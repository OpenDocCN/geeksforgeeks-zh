...
> 原文: [https://www.geeksforgeeks.org/integrating-bokeh-visualizations-in-django-application/](https://www.geeksforgeeks.org/integrating-bokeh-visualizations-in-django-application/)

Bokeh 是一个交互式可视化库，帮助我们创建数据集的可视化表示并与之交互。您可以创建各种类型的可视化效果，如条形图、水平图、时间序列等。有各种方法可以将 Bokeh 应用程序和小部件包含到网络应用程序和页面中。

在本教程中，我们将创建一个基本的 bokeh 图，并将其嵌入到我们的 Django web 应用程序中。为此，我们将从返回单个组件的 `bokeh.embed` 导入 `components`。函数 `bokeh.embed.components()` 返回一个脚本，其中包含带有 `<div>` 标记的绘图数据，绘图视图将加载到该脚本中。我们将详细了解一步一步的过程。
...
*   导航到地址 `http://127.0.0.1:8000/` 会看到类似这样的内容。
...
*   更新 `urls.py` 文件并添加 URL 模式。从我们的项目文件夹即 BokehDjango 打开 `urls.py`，在导入语句中加入 `include` 函数。此外，添加路径以包含我们新应用程序的 URL，如下所示:
...
*   现在在我们的 app 文件夹中创建一个新文件，即 BokehApp，并将其保存为 `urls.py`。
*   打开文件，为您的主页添加路径，如下所示，并且不要忘记导入 `path` 和 `views`。
...
*   现在让我们在 BokehApp 目录中创建一个模板文件夹，并将其保存为 `templates`。在模板目录中创建新文件，并将其保存为 `base.html`。
*   在你的 `base.html` 文件的头部添加以下 CSS 链接，并在 `bokeh-x.y.z.min` (下划线位置 `x.y.z`) 处替换你的 bokeh 版本
...
*   `components` 方法返回一个包含绘图数据的脚本，并提供一个 `<div>` 标签来显示绘图视图。这两个元素可以插入到 HTML 文本中，并且 `<script>` 在执行时会用剧情替换 div。
*   `circle` 法是字形法，是 `figure` 对象的一种方法。字形是博克图的基本视觉构件。这包括散点图中的直线、矩形、正方形、楔形或圆形等元素。
*   `plot` 变量使我们能够创建一个保存所有各种对象(如字形、注释等)的绘图。视觉化。
...
<body>
    <div class="container">
        <h1 class="mt-5">Our first Bokeh Graph</h1>
        <div class="mt-3">
            {{div| safe}}
        </div>
    </div>
