...
您也可以使用 `bokeh.plotting` 界面添加多个图形。为此，您只需要通过传递不同的数据作为参数多次调用 `line()` 函数，如示例所示。
...
`circle_cross()`: 此方法通过圆心添加一个带有“+”标记的圆字形，它取圆心的 x、y 坐标。
...
面片字形以特定的颜色给空间区域着色。我们使用 `patch()` 方法开发单个补丁，使用 `patches()` 方法开发多个补丁。
...
通过使用函数 `patches()` 而不是 `patch()`，可以以类似的方式创建多个修补程序。我们以数组列表的形式传递数据，以便在下面显示的示例中创建三个不同颜色的补丁。
