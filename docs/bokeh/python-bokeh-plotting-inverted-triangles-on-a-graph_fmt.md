...
> 原文: [https://www.geeksforgeeks.org/python-bokeh-plotting-inverted-triangles-on-a-graph/](https://www.geeksforgeeks.org/python-bokeh-plotting-inverted-triangles-on-a-graph/)
...
*   `y`: 倒三角标记中心的 y 坐标
*   `size`: 倒三角标记的直径，默认为 4
*   `angle`: 倒三角标记的旋转角度，默认为 0
*   `angle_units`: 角度的单位，默认为弧度
*   `fill_alpha`: 填充倒三角标记的α值
...
在此示例中，我们将绘制倒三角形，其中大小与它们的值和各种其他参数成比例。
