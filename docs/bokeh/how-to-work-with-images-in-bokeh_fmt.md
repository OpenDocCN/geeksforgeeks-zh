...
> 原文: [https://www.geeksforgeeks.org/how-to-work-with-images-in-bokeh/](https://www.geeksforgeeks.org/how-to-work-with-images-in-bokeh/)

在本文中，我们将看到如何在 Bokeh 中处理图像。

为此，我们将使用 `ImageURL` 字形。这个字形渲染从给定网址加载的图像。

> **语法:** `ImageURL('url', 'x', 'y', 'w', 'h', 'angle', 'dilate')`
>
> **论据:**
>
> *   **url:** 需要检索图像的网址。
> *   **x** = `NumberSpec(default=field("x"))`，定位图像锚点的 x 坐标。
> *   **y** = `NumberSpec(default=field("y"))`，定位图像锚点的 y 坐标。
...
> *   **angle** = `AngleSpec(default=0)`，图像旋转的角度，从水平方向测量。
> *   **global_alpha** = `Float(1.0)`，每个图像渲染时的整体不透明度。
> *   **dilate** = `Bool(False)`，是否总是对分数像素位置进行舍入，以使图像变大。如果像素舍入误差导致图像之间有间隙，则此设置可能很有用，此时它们应该看起来齐平。
> *   **anchor** = `Enum(Anchor)`，图像的什么位置应该锚定在‘x’，‘y’坐标。
> *   **retry_attempts** = `Int(0)`，尝试从指定的网址重新加载图像的次数。默认值为零。
