...
在本文中，我们将讨论如何使用 Python 中的 Bokeh 绘制多个图。我们将使用 `bokeh.layouts` 模块的 `row()` 方法，它在 `bokeh.io` 库的 `show()` 方法中作为参数使用，`bokeh` 用来描绘多个地块。

## 语法

> `show(row(图1, 图2, 图3, ...))`

其中 `图1`、`图2` 等是 `bokeh.plotting` 模块中类 `figure` 的对象。

## 方法

*   导入所需模块
*   使用 `figure` 类分配坐标和描绘地块。
*   在 `row()` 方法中使用 `figure` 对象作为参数。
*   使用 `show()` 方法描绘从 `row()` 方法返回的可视化。
