...
> 原文: [https://www.geeksforgeeks.org/python-bokeh-making-interactive-legends/](https://www.geeksforgeeks.org/python-bokeh-making-interactive-legends/)
...
通过将图例 `click_policy` 属性设置为 `"hide"`，可以从图例中隐藏字形。
...
隐藏字形使其完全消失，另一方面，静音字形只是根据参数取消强调字形。通过将图例 `click_policy` 属性设置为 `"mute"`，可以将图例中的字形静音。
