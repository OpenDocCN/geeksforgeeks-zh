...
> 原文：[https://www.geeksforgeeks.org/bokeh-plotting-figure-circle_x-function-in-python/](https://www.geeksforgeeks.org/bokeh-plotting-figure-circle_x-function-in-python/)
...
## `bokeh.plotting.figure.circle_x()`函数

`bokeh`库绘图模块中的`circle_x()`功能用于配置`circle_x`字形并添加到本图中。

**语法：**
```python
circle_x(x, y, size=4, angle=0.0, angle_units='rad', fill_alpha=1.0, fill_color='gray', line_alpha=1.0, line_cap='butt', line_color='black', line_dash=[], line_dash_offset=0, line_join='miter', line_width=1, name=None, tags=[], **kwargs)
```
...
以下示例说明了`bokeh.plotting`中的`bokeh.plotting.figure.circle_x()`函数：
