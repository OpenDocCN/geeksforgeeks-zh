...
> 原文: [https://www.geeksforgeeks.org/how-to-change-ticks-label-sizes-using-pythons-bokeh/](https://www.geeksforgeeks.org/how-to-change-ticks-label-sizes-using-pythons-bokeh/)
...
*   现在，要使用 bokeh 绘制任何数据集，最简单的方法是从`plotting`模块导入`figure`和`show`函数。其他是可选的导入，有助于自定义图表。
*   创建一个`figure`对象，将我们提供的数据集中的值绘制到图形上。
*   现在`plot`变量将是我们的图形对象，宽度为 700 像素，高度为 500 像素。
*   Bokeh 还为我们提供了定制每个轴的能力。
*   分配或创建数据集。
*   使用`label_text_font_size`属性决定刻度标签的大小。
*   最后，描绘可视化。
