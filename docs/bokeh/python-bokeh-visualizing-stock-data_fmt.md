...
> 原文: [https://www.geeksforgeeks.org/python-bokeh-visualizing-stock-data/](https://www.geeksforgeeks.org/python-bokeh-visualizing-stock-data/)
...
1.  导入所需的模块:
    *   `numpy`
    *   `figure`, `output_file` 和 `show` 来自 `bokeh.plotting`
    *   `AAPL`, `FB`, `GOOG`, `IBM` 和 `MSFT` 来自 `bokeh.sampledata.stocks`
2.  用标题和轴类型实例化图形对象。
3.  给 x 轴和 y 轴命名。
4.  绘制所有 5 家公司的折线图。
5.  显示模型。
