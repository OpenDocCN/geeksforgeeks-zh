...
> 原文: [https://www.geeksforgeeks.org/python-bokeh-plotting-dashes-on-a-graph/](https://www.geeksforgeeks.org/python-bokeh-plotting-dashes-on-a-graph/)
...
> *   `y` : 虚线标记中心的 y 坐标
> *   `size`: 虚线标记的直径，默认为 4
> *   `angle`: 虚线标记的旋转角度，默认值为 0
> *   `angle_unit`: 角度的单位，默认为弧度
> *   `fill_alpha`: 填充破折号的阿尔法值
> *   `fill_color`: 虚线标记的填充颜色值
