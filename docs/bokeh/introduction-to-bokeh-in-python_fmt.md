...
> 原文：`https://www.geeksforgeeks.org/introduction-to-bokeh-in-python/`

`Bokeh` 是一款 `Python` 交互数据可视化库。与 `Matplotlib` 和 `Seaborn` 不同，`Bokeh` 使用 `HTML` 和 `JavaScript` 渲染其图形。它以现代网络浏览器为呈现目标，提供优雅、简洁的新颖图形结构和高性能交互性。

![](img/cc262e14e6b170940f352415c3f7443c.png)

**Bokeh 的特征：** Bokeh 的一些重要特征如下：
...
*   **中级：** `bokeh.plotting` 接口是由所有可视字形组成的。
*   **高级：** `bokeh.charts` 接口，轻松搭建复杂图形。