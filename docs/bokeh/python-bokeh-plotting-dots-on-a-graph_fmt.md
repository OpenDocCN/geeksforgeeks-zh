...
> 原文: [https://www.geeksforgeeks.org/python-bokeh-plotting-dots-on-a-graph/](https://www.geeksforgeeks.org/python-bokeh-plotting-dots-on-a-graph/)
...
*   `y` : 点标记中心的 y 坐标
*   `size` : 点标记的直径，默认为 4
*   `angle` : 点标记的旋转角度，默认为 0
*   `angle_unit` : 角度的单位，默认为弧度
*   `fill_alpha` : 点标记的填充 alpha 值
*   `fill_color` : 点标记的填充颜色值
...
在本例中，我们将使用默认值绘制图表。我们提供了 `size` 属性来使字形可见。
...
在本例中，我们将使用 `random()` 函数生成点，并传递一些参数。
