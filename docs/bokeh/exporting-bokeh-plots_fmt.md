...
> **语法:** `export_png(obj, filename, width, height, webdriver)`
>
> **参数:**
>
> *   **`obj`**: `obj` 可以是我们要导出的任何 Bokeh 图。
> *   **`filename`**: 可选参数，默认绘图文件名为 python 文件名。
> *   **`width`**: 可选参数，用于设置导出的出图布局 `obj` 的宽度，默认情况下会被忽略。
> *   **`height`**: 可选参数，用于设置导出的出图布局 `obj` 的高度，默认忽略。
...
> **语法:** `export_svg(obj, filename, width, height, webdriver, timeout)`
>
> **参数:**
>
> *   **`obj`**: `obj` 可以是我们要导出的任何图。
> *   **`filename`**: 可选参数，默认绘图文件名为 python 文件名。
> *   **`width`**: 可选参数，用于设置导出的出图布局 `obj` 的宽度，默认忽略。
> *   **`height`**: 可选参数，用于设置导出的出图布局 `obj` 的高度，默认忽略。
> *   **`webdriver`**: 可选参数，用于设置导出图时使用的默认 `webdriver` 实例。默认情况下，如果我们没有指定任何内容，`selenium` web 驱动程序是可用的。
> *   **`timeout`**: 等待 Bokeh 初始化的最长时间(秒)。它的默认值是 5s。
...
**注意:** 导出的图将保存在运行 Python 代码文件的文件夹中。