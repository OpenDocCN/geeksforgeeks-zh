...
这是提供 [**Matplotlib**](https://www.geeksforgeeks.org/matplotlib-tutorial/) 或 **MATLAB** 类特征进行绘图的中级界面。它处理要绘制的数据，并创建有效的轴、网格和工具。这个界面的主类是`` `Figure` ``类。
...
在上面的例子中，我们创建了一个标题为`` `Bokeh Line Graph` ``的简单图。如果您正在使用 Jupyter，那么输出将在浏览器的新选项卡中创建。
...
| 财产 | 描述 |
| --- | --- |
| `legend.label_text_font` | 将默认标签字体更改为指定的字体名称 |
| `legend.label_text_font_size` | 字体大小，以磅为单位 |
| `legend.location` | 在指定位置设置标签。 |
| `legend.title` | 设置图例标签的标题 |
| `legend.orientation` | 设置为水平(默认)或垂直 |
| `legend.clicking_policy` | 指定单击图例时应该发生的情况 |
...
**Bokeh 术语中的字形**是指 Bokeh 地块的基本构建块，如直线、矩形、正方形等。Bokeh 绘图是使用 `` `bokeh.plotting` `` 界面创建的，该界面使用一组默认工具和样式。
...
一个**散点图**是一组在水平和垂直轴上代表单个数据的点。一种图表，其中两个变量的值沿 X 轴和 Y 轴绘制，结果点的模式揭示了它们之间的相关性。可以使用绘图模块的`` `scatter()` ``方法绘制。
...
**面片图**对一个区域进行着色，以显示具有相同属性的组。它可以使用绘图模块的`patch()`方法创建。
