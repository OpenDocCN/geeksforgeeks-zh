...
> 原文: [https://www.geeksforgeeks.org/how-to-create-a-covid-19-tracker-android-app/](https://www.geeksforgeeks.org/how-to-create-a-covid-19-tracker-android-app/)
...
世界正面临最严重的流行病之一，新冠肺炎的爆发，你们都知道这一点。因此，在封锁期间，让我们使用 REST API 创建一个**新冠肺炎追踪器安卓应用程序，它将只追踪全球统计数据。**
...
*   转到 `Gradle Scripts -> build.gradle (Module: app)` 部分，导入以下依赖项，然后单击上面弹出的“立即同步”。
...
<!--Text view to display the updated number when data will fetch from the API. For now default set to 0 -->
```xml
<TextView
    android:layout_width="match_parent"
    android:layout_height="wrap_content"
...
打开 `MainActivity.java` 文件里面的类，首先创建 `TextView` 类的对象。
...
在 `fetchdata()` 方法中，最重要的任务是如何从第三方应用编程接口获取数据并在我们的应用程序中实现它。我的要求是请通读安卓的 [Volley库](https://www.geeksforgeeks.org/volley-library-in-android/) 和 [REST API(简介)](https://www.geeksforgeeks.org/rest-api-introduction/) 两篇文章，了解以下概念。

使用 [Volley库](https://www.geeksforgeeks.org/volley-library-in-android/) 创建字符串请求，并用 `https://corona.lmao.ninja/v2/all` 链接分配“网址”。
...
请参考这个 [网站](https://corona.lmao.ninja/) 来查看请求的数据，它们是 [JSON](https://www.geeksforgeeks.org/javascript-json/) 格式。

[![](img/9ae5f783a7cb0c58e729e1b7124b1964.png)](https://media.geeksforgeeks.org/wp-content/uploads/20200428235348/f35.png)

所以接下来你要做的是，在 `onResponse()` 方法内部创建 `JSONObject` 类的对象，然后在文本视图中设置数据，这些数据在 `JSONObject` 的帮助下以 JSON 格式提供。确保你必须在一个 `try` 区内做这些事情。记住 `getString()` 内的参数必须与 JSON 格式给出的名称匹配。
