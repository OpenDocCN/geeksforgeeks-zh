...
> 原文: [https://www.geeksforgeeks.org/method-chaining-in-java-with-examples/](https://www.geeksforgeeks.org/method-chaining-in-java-with-examples/)
...
*   在这种情况下，在返回时，我们使用 `this`，它返回当前实例引用。检查[本](https://www.geeksforgeeks.org/this-reference-in-java/)中 `this` 参考变量的用途和值。
*   当在主方法中实现方法链接时，`setint(10)` 和 `setfloat(20)` 正在返回对象的引用，该引用进一步用于调用 `display()` 方法。