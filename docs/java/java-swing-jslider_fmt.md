...
3.  `setPaintTicks(boolean b)`: 布尔值决定记号是否画在滑块上
4.  `setPaintTrack(boolean b)`: 布尔值决定轨迹是否画在滑块上
5.  `setMajorTickSpacing(int n)`: 设置主要刻度的间距。
6.  `setMinorTickSpacing(int n)`: 设置次要刻度的间距。
7.  `setFont(Font f)`: 设置滑块的文本字体
8.  `setMaximum(int m)`: 设置滑块的最大值
9.  `setMinimum(int m)`: 设置滑块的最小值
10. `updateUI()`: 将 UI 属性重置为当前外观的值。
11. `setValueAdjusting(boolean b)`: 将模型的值调整属性设置为布尔值 `b`。
12. `setUI()`: 设置用户界面对象，实现这个组件的外观。
13. `setSnapToTicks(boolean b)`: 如果传递 `true`，则滑块位置被放置到最近的刻度。
14. `setModel(bounddrangemodel n)`: 设置处理滑块三个基本属性的 `bounddrangemodel`: 最小值、最大值、值。
15. `setLabelTable(Dictionary l)`: 用于指定将在任何给定值绘制什么标签。
16. `setInverted(boolean b)`: 如果传递 `true`，则滑块被设置为 Inverted。
17. `imageUpdate(Image img，int s，int x，int y，int w，int h)`: 当图像发生变化时，重新绘制组件。
18. `setExtent(int extent)`: 设置旋钮“覆盖”的范围大小。
19. `removeChangeListener()`: 从滑块中移除变更监听器。
...
31. `addChangeListener(ChangeListener l)`: 向滑块添加一个 `ChangeListener`。
...
## 程序 2：创建一个带有最小值、最大值以及绘制主要和次要刻度的滑块
...
## 程序 3：创建一个垂直滑块，绘制最小和最大值以及主要和次要刻度，并设置滑块的字体
