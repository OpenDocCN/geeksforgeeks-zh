...
### `Object` 类的 `equals()` 方法的一些原理

如果某个其他对象等于某个给定对象，那么它遵循以下规则：

*   **反身性：** 对于任何引用值 `a`，`a.equals(a)` 应该返回 `true`。
*   **对称性：** 对于任何引用值 `a` 和 `b`，如果 `a.equals(b)` 应该返回 `true`，那么 `b.equals(a)` 必须返回 `true`。
*   **传递性：** 对于任何引用值 `a`、`b` 和 `c`，如果 `a.equals(b)` 返回 `true`，`b.equals(c)` 返回 `true`，那么 `a.equals(c)` 应该返回 `true`。
*   **一致性：** 对于任何引用值 `a` 和 `b`，只要对象上用于 `equals` 比较的信息没有被修改，多次调用 `a.equals(b)` 会一致地返回 `true` 或一致地返回 `false`。

### 注意

对于任何非空引用值 `a`，`a.equals(null)` 应该返回 `false`。
...
引用 `obj` 也可以引用 `Geek` 子类的对象。行 (y) 确保如果传递的参数是 `Geek` 类子类的对象，它将返回 `false`。但是 `instanceof` 运算符如果发现传递的参数是 `Geek` 类的子类，则不会返回 `false`。阅读运算符 [`instanceof`](https://www.geeksforgeeks.org/java-instanceof-and-its-applications/)。
...
*   在应用程序执行期间，如果在同一对象上多次调用 `hashCode()`，那么它必须一致地返回相同的整数值，前提是在对象上的 `equals(Object)` 比较中使用的信息没有被修改。从应用程序的一次执行到同一应用程序的另一次执行，该整数值不必保持不变。
*   如果两个对象相等，根据 `equals(Object)` 方法，那么 `hashCode()` 方法必须在两个对象的每一个上产生相同的整数。
*   如果两个对象不相等，根据 `equals(Object)` 的方法，`hashCode()` 方法在两个对象上产生的整数值没有必要是不同的。它可以是相同的，但是在两个对象中的每一个上产生不同的整数对于提高基于散列的集合的性能更好，比如散列映射、散列表等等。

### 注意

只要相等，相等的对象必须产生相同的哈希代码，但是不相等的对象不需要产生不同的哈希代码。

### 相关链接

*   [在 Java 中覆盖 equals 方法](https://www.geeksforgeeks.org/overriding-equals-method-in-java/)
*   参考: [JavaRanch](http://www.javaranch.com/journal/2002/10/equalhash.html)
