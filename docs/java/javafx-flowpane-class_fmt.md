...
`FlowPane` 类是 JavaFX 的一部分。`FlowPane` 以在边界处换行的方式布局其子代。水平的 `FlowPane`（默认）将按行布局节点，按照 `FlowPane` 的宽度换行。垂直的 `FlowPane` 以列的形式排列节点，在 `FlowPane` 的高度换行。`FlowPane` 类继承了 [`Pane`](https://www.geeksforgeeks.org/javafx-pane-class/) 类。

## 类的构造函数

1.  `FlowPane()`: 创建新的水平流程窗格布局。
2.  `FlowPane(double h, double v)`: 创建新的水平流程窗格布局，具有指定的水平和垂直间隙。
3.  `FlowPane(double h, double v, Node... c)`: 创建一个新的水平流程窗格布局，具有指定的水平、垂直间隙和节点。
4.  `FlowPane(Node... c)`: 用指定的子节点创建流程窗格。
5.  `FlowPane(Orientation o)`: 创建具有指定方向的流程窗格。
6.  `FlowPane(Orientation o, double h, double v)`: 创建具有指定方向和指定水平和垂直间隙的流程窗格。
7.  `FlowPane(Orientation o, double h, double v, Node... c)`: 创建具有指定方向、指定水平和垂直间隙以及指定子级的流程窗格。
8.  `FlowPane(Orientation o, Node... c)`: 创建具有指定方向和指定节点的流程窗格。
...
| `getVgap()` | 返回流窗格的垂直间距。 |
| `setAlignment(Pos v)` | 设置窗格的对齐值。 |
| `setHgap(double v)` | 设置流动窗格的水平间距。 |
| `setOrientation(Orientation o)` | 设置窗格的方向。 |
| `setRowValignment(double v)` | 设置 `rowValignment` 属性的值。 |
| `setVgap(double v)` | 设置流动窗格的垂直间距。 |
...
**参考:** [https://docs.oracle.com/javase/8/javafx/api/javafx/scene/layout/FlowPane.html](https://docs.oracle.com/javase/8/javafx/api/javafx/scene/layout/FlowPane.html)