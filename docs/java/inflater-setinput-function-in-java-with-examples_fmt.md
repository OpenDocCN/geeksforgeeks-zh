...
使用`Inflater`类的`setInput()`函数来设置要解压缩的输入数据。当`needsInput()`返回 true 表示输入缓冲区为空时，应调用此函数。
...
## 参考:
[https://docs.oracle.com/javase/7/docs/api/java/util/zip/Inflater.html#setInput()](https://docs.oracle.com/javase/7/docs/api/java/util/zip/Inflater.html#setInput())