...
*   `return 0` --> 成功终止。
*   `return 1` 或任何其他**非零**值 --> 不成功终止。
*   返回不同的值，如 `return 1` 或 `return -1` 或任何其他**非零**值意味着程序返回错误。
...
| 3. | 它是一个用来返回值的关键字，所以不需要任何声明。它只需要返回带有值或变量的关键字。 | `java.lang.System.exit()` 方法的声明: `public static void exit(int status)` |
