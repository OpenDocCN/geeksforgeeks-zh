...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-class-in-jshell-of-java-9/](https://www.geeksforgeeks.org/how-to-create-a-class-in-jshell-of-java-9/)
...
一个 [`类`](https://www.geeksforgeeks.org/classes-objects-java/) 根据用户需求包含不同的方法和变量，但由于一些错误程序无法工作。但是在 `JShell` 的帮助下，这可以更有效地解决，并且耗时更少。

**示例：**

1.  在这个示例中，`类 A` 被成功创建，可以通过创建 `类 A` 的对象来调用类方法。
...
2.  在这个示例中，由于 `;` 导致错误发生，这可以很容易地解决，因为代码行很小且简单。
...
4.  在上面的例子中，创建了 `类 A` 的对象并用 `a` 表示，我们调用了 `get` 和 `show` 方法。我们也可以重写 `类 A` 的方法。重写方法后，将显示消息“方法已被修改”，现在我们可以调用修改后的方法并获得答案。
