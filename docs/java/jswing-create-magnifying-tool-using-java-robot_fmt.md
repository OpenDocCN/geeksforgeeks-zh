...
2.  `getLocation()` : 返回一个表示位置的 `Point` 实例
3.  `createScreenCapture(Rectangle r)` : 捕捉矩形 `r` 内的屏幕部分
4.  `drawImage(Image i, int x, int y, ImageObserver o)` : 在屏幕上的 `x`，`y` 位置绘制图像，指定图像观察者
5.  `drawImage(Image i, int x, int y, int w, int h, ImageObserver o)` : 在指定图像观察者的屏幕上，在 `x`，`y` 位置和指定的宽度和高度绘制图像
