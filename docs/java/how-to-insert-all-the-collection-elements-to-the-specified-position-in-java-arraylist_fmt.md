...
> 原文: [https://www.geeksforgeeks.org/how-to-insert-all-the-collection-elements-to-the-specified-position-in-java-arraylist/](https://www.geeksforgeeks.org/how-to-insert-all-the-collection-elements-to-the-specified-position-in-java-arraylist/)

可以使用 `java.util.ArrayList` 类中的 [`Collection.addAll()`](https://www.geeksforgeeks.org/collections-addall-method-in-java-with-examples/) 方法将元素插入到 ArrayList 中的集合元素指定位置。如果索引处存在任何元素，则该元素及其所有右侧元素都将移到右侧。这个方法接受两个参数：第一个参数是我们要插入元素的索引，第二个参数是另一个集合的对象。如果成功插入元素，此方法返回布尔值 `true`，否则返回 `false`。

## 语法

`public boolean addAll(int i, Collection c);`

这个方法将指定集合中的所有元素追加到这个列表的末尾，按照它们被指定集合的迭代器返回的顺序。如果在操作过程中修改了指定的集合，则此操作的行为是未定义的（意味着如果指定的集合是此列表，并且此列表是非空的，则此调用的行为是未定义的）。

## 异常

这里有两种可能的异常。

1.  `IndexOutOfBoundsException`: 如果索引超出范围，则抛出此异常。
2.  `NullPointerException`: 如果指定的集合为空，则抛出此异常。如果元素包含一个或多个空值，并且 `c` 不允许空元素，或者如果 `c` 或元素为空，该方法也会抛出此异常。
