...
> 原文：[https://www.geeksforgeeks.org/java-program-to-show-time-by-rolling-through-hours-and-months/](https://www.geeksforgeeks.org/java-program-to-show-time-by-rolling-through-hours-and-months/)

`java.util` 包有一个 `Calendar` 类，里面有一些预定义的方法，帮助我们做一些和日期时间相关的操作。日历类有一个预定义的方法叫做 `roll()` 方法，在这个方法的帮助下，我们可以滚动月份（不改变年份）和小时（不改变月份和年份）。
...
这里，`roll()` 方法取 2 个参数 `calendar_field` 和 `up_down`。

*   **calendar_field**：它是我们要滚动的小时或月或年。
