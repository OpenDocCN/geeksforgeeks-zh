...
> 原文：[https://www.geeksforgeeks.org/file-getparentfile-method-in-java-with-examples/](https://www.geeksforgeeks.org/file-getparentfile-method-in-java-with-examples/)

`getParentFile()` 方法是 [`File`](https://www.geeksforgeeks.org/File-class-in-java/) 类的一部分。该函数返回给定文件对象的父文件。该函数返回一个文件对象，该对象包含给定文件对象的父文件。如果抽象路径不包含任何父文件，则返回空值。
...
函数返回文件对象，它是给定文件对象的父文件。

下面的程序将说明 `getParentFile()` 函数的使用：

### 例 1

给我们一个文件的文件对象，我们要得到文件对象的父文件。
