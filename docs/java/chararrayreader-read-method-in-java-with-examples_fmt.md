...
Java 中 [`CharArrayReader`](https://www.geeksforgeeks.org/java-io-chararrayreader-class-java/) 类的 [`read()`](https://www.geeksforgeeks.org/java-io-chararrayreader-class-java/) 方法用于从流中读取单个字符。这个方法阻塞流直到:
...
## 返回值:
该方法返回一个 `int` 值，它是从流中读取的整数值。范围可以从 0 到 65535。否则，如果未读取任何字符，则返回-1。

## 异常:
如果输入输出时出现错误，该方法抛出 `IOException`。
