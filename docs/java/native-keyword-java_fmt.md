...
*   对于原生方法，实现已经在像 C、C++这样的旧语言中可用，我们不负责提供实现。因此，本机方法声明应以 `;` (分号)结尾。
*   我们不能将一个原生方法声明为 `abstract`。
*   我们不能将一个原生方法声明为 `strictfp`，因为不能保证旧语言(C，C++)遵循 IEEE 754 标准。因此，`native` `strictfp` 组合对于方法来说是非法的组合。
...
**参考:** [http://www.codejava.net/java-core/the-java-language/native-keyword](http://www.codejava.net/java-core/the-java-language/native-keyword)
