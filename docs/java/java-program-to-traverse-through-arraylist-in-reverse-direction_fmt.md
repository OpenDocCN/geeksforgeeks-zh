...
> 原文：[https://www.geeksforgeeks.org/java-program-to-traverse-through-arraylist-in-reverse-direction/](https://www.geeksforgeeks.org/java-program-to-traverse-through-arraylist-in-reverse-direction/)
...
*   使用 `List.stream()` 来获取流。
*   使用 `stream.collect()` 来将此流的元素收集到[链表](https://www.geeksforgeeks.org/linked-list-in-java/)中。
*   使用 `LinkedList.descendingIterator()` 方法以相反的顺序迭代链表。
*   使用 `forEachRemaining()` 来打印数组列表的每个元素。我们可以将方法引用 `System.out::println` 提供给 `forEachRemaining()`。
...
我们知道[列表](https://www.geeksforgeeks.org/list-interface-java-examples/)是一个有序集合，我们只需要通过它的索引就可以访问列表的元素，所以定义一个 `ArrayList`，从最后使用一个 [`for` 循环](https://www.geeksforgeeks.org/java-for-loop-with-examples/)迭代到第一个元素，打印每个元素。
...
## 方法 4：（使用 Apache Common 的 `ReverseListIterator`）

这个方法提供了 `ReverseListIterator`，我们可以用它以相反的顺序迭代 `List`。当我们使用 `ReverseListIterator` 时，`next()` 将返回数组列表中的最后一个元素，当我们调用下一个元素时，下一个括号将返回当前元素的前一个元素，而 `hasNext()` 将检查我们的数组列表是否包含元素。
...
class GFG {
    public static void main(String[] args)
    {
        ...
        ReverseListIterator<Integer> iterator
            = new ReverseListIterator<>(Arlist);
