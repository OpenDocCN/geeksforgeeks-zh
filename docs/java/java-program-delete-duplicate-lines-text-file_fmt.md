...
> 原文：[https://www.geeksforgeeks.org/java-program-delete-duplicate-lines-text-file/](https://www.geeksforgeeks.org/java-program-delete-duplicate-lines-text-file/)
...
一个更好的解决方案是使用 [`HashSet`](https://www.geeksforgeeks.org/hashset-in-java/) 来存储 `input.txt` 的每一行，由于 `Set` 会忽略重复的值，所以在存储一行时，检查它是否已经存在于 `HashSet` 中。仅当 `hashset` 中不存在时，才将其写入 `output.txt`。
