...
> 原文: [https://www.geeksforgeeks.org/linkedtransferqueue-trytransfer-method-in-java-with-examples/](https://www.geeksforgeeks.org/linkedtransferqueue-trytransfer-method-in-java-with-examples/)

`LinkedTransferQueue`类的`tryTransfer()`方法是 Java 中的一个内置函数，一般用于将一个元素传递给等待接收它的线程，如果没有等待的线程，那么它将终止，而不会将元素添加到队列中，或者您也可以通过将时间量及其单位作为参数传递给函数来使它等待一定的时间。
...
### 返回值:
该方法将返回一个`boolean`值，即如果元素被转移则为真，否则为假。
...
该方法接受三个强制参数:
*   这里`E e`是要传送到处于等待状态的线程的元素。
*   这里`long timeout`是指定终止前应该等待的时间量的时间。
*   这里`TimeUnit unit`是指长时间超时的单位。

### 返回值:
该方法将返回一个`boolean`值。如果元素被转移，则返回 true，否则返回 false。
...
### 程序 1:
通过设置等待作为其参数来说明 Java 中的`tryTransfer()`方法。
```java
// Java program to demonstrate
// the tryTransfer() Method.
