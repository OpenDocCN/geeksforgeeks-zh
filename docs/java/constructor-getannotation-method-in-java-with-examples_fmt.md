...
`Constructor`类的`getAnnotation()`方法用于获取指定类型的构造函数对象注释，如果存在这样的注释，则返回它；否则返回`null`。指定的类型作为参数传递。
...
该方法接受单个参数`annotationClass`，表示注解类型对应的类对象。

## 返回值

该方法返回指定注解类型的该元素的注解（如果存在于该元素中，否则为`null`）。

## 异常

如果给定的注解类为空，该方法抛出`NullPointerException`。
