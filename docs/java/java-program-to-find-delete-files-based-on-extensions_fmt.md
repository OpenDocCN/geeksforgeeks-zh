...
> 原文：[https://www.geeksforgeeks.org/java-program-to-find-delete-files-based-on-extensions/](https://www.geeksforgeeks.org/java-program-to-find-delete-files-based-on-extensions/)

我们可以在 Java 中使用 `FilenameFilter` 覆盖 `accept(File dir, String name)` 方法来执行文件过滤操作。`FilenameFilter` 接口有一个 `accept()` 的方法。它检查给定的文件是否应该包含在文件列表中。我们可以通过实现这个方法来检查传递的目录中的每个文件。如果文件具有所需的扩展名，则包含该文件，否则将被丢弃。
...
`list()` 方法是 [File 类](https://www.geeksforgeeks.org/File-class-in-java/)的一部分。函数返回一个字符串数组，表示给定抽象路径名中的文件，如果路径名是目录，则返回 null。该函数是一个重载函数。其中一个函数没有任何参数，另一个函数将 `FilenameFilter` 对象作为参数，如下所述。
...
## 参数
函数是一个重载函数。其中一个函数没有任何参数，另一个函数以 [`FilenameFilter`](https://www.geeksforgeeks.org/file-listfiles-method-in-java-with-examples/) 对象为参数。

## 返回值
函数返回字符串数组，如果文件对象是文件，则返回空值。

## 异常
如果不允许函数对文件进行写访问，这个方法抛出 [`SecurityException`](https://www.geeksforgeeks.org/why-does-canvas-todataurl-throws-a-security-exception/)。
...
class GFG {
    ...
    public void deleteFile(String folder, String extension) {
        ...
        // Creating an array of strings to
        ...
        // Base condition check when array of strings is
    ...
    public static void main(String args[]) {
        ...
        // FileChecker() method to check existence for the file

        // Delete the file with FILE_EXTENSION from
        // FILE_DIRECTORY using the deleteFile() method
        // created above
