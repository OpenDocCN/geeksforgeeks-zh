# Java 中的 CharBuffer.array() 方法

> 原文: [https://www.geeksforgeeks.org/charbuffer-array-method-in-java/](https://www.geeksforgeeks.org/charbuffer-array-method-in-java/)

`java.nio.CharBuffer` 类的 `array()` 方法用于返回支持该缓冲区的 `char` 数组。对此缓冲区内容所做的任何修改都会导致返回数组的内容也被修改，反之亦然。在调用此方法之前调用 `hasArray()` 方法，以确保此缓冲区具有可访问的后备数组。
