...
1.  `ToolBar()`: 创建一个空工具栏。
2.  `ToolBar(Node... n)`: 创建填充了指定节点的工具栏。
...
| `setOrientation(Orientation v)` | 设置对象方向的值。 |

下面程序举例说明 ToolBar 类的使用。

### 1. Java program to create a toolbar and add it to the scene
...
### 2. Java program to create a toolbar and add it to the scene and set the orientation of the toolbar
