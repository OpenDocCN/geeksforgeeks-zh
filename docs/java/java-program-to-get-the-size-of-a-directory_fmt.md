...
> 原文: [https://www.geeksforgeeks.org/java-program-to-get-the-size-of-a-directory/](https://www.geeksforgeeks.org/java-program-to-get-the-size-of-a-directory/)

Java 中文件的大小可以使用 `File` 类获得。`fileName.length()` 的内置函数用于查找以字节为单位的文件大小。目录可能包含 “N” 个文件，为了计算目录的大小，需要所有文件大小的总和。

**`length()` 方法:**

`File` 类对象中的 `length()` 方法返回的是 `long` (数据类型)格式的文件大小。文件大小以字节为单位。
