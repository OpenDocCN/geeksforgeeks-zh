...
`java.lang.Math.log10()` 是 Java Math Library 方法之一，用于返回给定双精度值的基数 10 对数值作为参数。有各种情况:

*   如果参数是正双精度值，`Math.log10()`方法将返回给定值的对数。
*   如果参数为 `NaN` 或小于零，`Math.log10()`方法将返回 `NaN`。
*   如果参数为正无穷大，则 `Math.log10()`方法将返回结果为正无穷大。
*   如果参数为正或负零，`Math.log10()`方法将返回结果为负无穷大。
