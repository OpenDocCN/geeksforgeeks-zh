...
> 原文：[https://www.geeksforgeeks.org/java-program-to-show-the-nesting-of-methods/](https://www.geeksforgeeks.org/java-program-to-show-the-nesting-of-methods/)
...
在下面的示例中，该类包含两种方法，即 `area()` 和 `volume()`，后者用于计算球体的面积和体积。`main()` 方法是调用 `area()` 方法计算球体的面积，调用 `volume()` 方法计算球体的体积，然后显示结果。
