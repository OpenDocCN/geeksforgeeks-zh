...
> 原文：[https://www.geeksforgeeks.org/how-to-make-a-simple-restcontroller-in-spring-boot/](https://www.geeksforgeeks.org/how-to-make-a-simple-restcontroller-in-spring-boot/)
...
> **rest controller**：`RestController` 用于借助 `@RestController` 注释制作 restful web 服务。此注释在类级别使用，允许类处理客户端发出的请求。让我们用一个例子来理解 `@RestController` 注释。
...
现在打开一个合适的 IDE，然后转到 **文件 > 从现有来源新建 > 项目 > Spring-boot-app** 并选择 `pom.xml`。在提示符下单击导入更改并等待项目同步，如下图所示：
...
转到 `src > main > java > com.gfg.spring.boot.app`，创建一个名为 `Controller` 的 Java 类，并添加注释 `@RestController`。
...
运行 `SpringBootAppApplication` 类，等待 Tomcat 服务器启动。
