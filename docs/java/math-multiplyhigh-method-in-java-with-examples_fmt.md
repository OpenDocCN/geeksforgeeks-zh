...
> 原文: [https://www.geeksforgeeks.org/math-multiplyhigh-method-in-java-with-examples/](https://www.geeksforgeeks.org/math-multiplyhigh-method-in-java-with-examples/)

`Math`类的`multiplyHigh(long x, long y)`方法用于从两个 64 位因子的 128 位乘积中返回最高有效的 64 位。此方法采用两个`long`值作为输入，一个`long`值是 64 位数字。此方法将两个`long`值相乘，并从乘积结果中返回最高有效的 64 位，其长度可能为 128 位。
...
## 参数:
这个方法接受两个`long`的`x`，`y`作为参数，其中`x`和`y`是我们想要相乘的参数。

## 返回值:
此方法从两个 64 位数字的 128 位乘积返回最高有效的 64 位`long`值，即 X * Y。

## 注:
此法在`JDK 9`中增加。因此，它不会在在线集成开发环境中运行。
