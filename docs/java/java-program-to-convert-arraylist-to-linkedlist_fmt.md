...
> 原文: [https://www.geeksforgeeks.org/java-program-to-convert-arraylist-to-linkedlist/](https://www.geeksforgeeks.org/java-program-to-convert-arraylist-to-linkedlist/)
...
`ArrayList` – 一个 `ArrayList` 是集合框架的一部分，存在于 `java.util` 包中。它用 Java 为我们提供了动态数组。尽管它可能比标准数组慢，但在需要对数组进行大量操作的程序中会很有帮助。

`LinkedList` – 一个 `LinkedList` 是一个线性数据结构，其中元素不存储在连续的内存位置。链表中的元素使用指针链接，如下图所示:
