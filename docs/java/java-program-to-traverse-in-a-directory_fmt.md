...
> 原文：[https://www.geeksforgeeks.org/java-program-to-traverse-in-a-directory/](https://www.geeksforgeeks.org/java-program-to-traverse-in-a-directory/)
...
`先决条件`要求是 `listFiles()` 并且考虑到没有路径遍历攻击。
...
从 Java 8 开始，引入了 `walk()` 方法来递归地遍历整个目录，并检索 `Stream<Path>` 作为返回值。
