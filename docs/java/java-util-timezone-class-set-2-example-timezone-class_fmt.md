...
**解释:**我们使用作为输入提供的标识获取该地点的时区对象，然后使用方法获取该地点的当前日期和时间。方法 `getDateTime` 返回 `LocalDateTime` 对象，我们可以使用 `LocalDateTime` 类的 `toLocalDate()` 方法获取 Date，使用 `LocalDateTime` 类的 `toLocalTime()` 方法获取 Time。
