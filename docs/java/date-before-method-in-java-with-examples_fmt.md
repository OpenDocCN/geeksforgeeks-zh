...
> 原文: [https://www.geeksforgeeks.org/date-before-method-in-java-with-examples/](https://www.geeksforgeeks.org/date-before-method-in-java-with-examples/)

`Date`类的`before()`方法测试日期是否在指定日期之前。
...
## 参数
该方法接受单个参数`when`，用于指定需要检查的日期。

## 返回值
返回布尔值。如果该日期在指定日期之前，它将返回`true`，否则返回`false`。

## 异常
当参数`when`为`null`时，函数会抛出`NullPointerException`异常。
