...
> 原文：[https://www.geeksforgeeks.org/abstractmap-putall-method-in-java-with-examples/](https://www.geeksforgeeks.org/abstractmap-putall-method-in-java-with-examples/)

`AbstractMap.putAll()` 是 `AbstractMap` 类的一个内置方法，用于复制操作。该方法将所有元素（即映射）从一个映射复制到另一个映射。
...
该方法取一个参数 `exist_abstract_map`，指的是我们要复制的现有地图。
...
如果我们要复制的地图为空，该方法抛出 `NullPointerException`。
