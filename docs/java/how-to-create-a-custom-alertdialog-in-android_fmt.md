...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-custom-alertdialog-in-android/](https://www.geeksforgeeks.org/how-to-create-a-custom-alertdialog-in-android/)
...
创建 `custom_layout.xml` 文件。在 `custom_layout.xml` 中添加以下代码。该代码定义 `AlertDialog` 框尺寸，并在其中添加一个 `EditText`。
...
## 步骤 2：在 activity_main.xml 中增加一个按钮

在 `activity_main.xml` 中增加一个按钮。单击该按钮将显示警报对话框。
...
在您想要显示自定义警报对话框的活动中添加 `custom_layout.xml`，它是在 `MainActivity.java` 中添加的。
