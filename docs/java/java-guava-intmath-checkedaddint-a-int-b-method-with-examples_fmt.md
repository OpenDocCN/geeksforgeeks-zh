...
> 原文: [https://www.geeksforgeeks.org/java-guava-intmath-checkedaddint-a-int-b-method-with-examples/](https://www.geeksforgeeks.org/java-guava-intmath-checkedaddint-a-int-b-method-with-examples/)

`checkedAdd(int a, int b)` 是番石榴的 [`IntMath`](https://www.geeksforgeeks.org/intmath-class-guava-java/) 类的一种方法，它接受两个参数 `a` 和 `b`，并返回它们的和。
...
## 参数:
该方法接受两个整数值 `a` 和 `b` 并计算它们的和。

## 返回值:
该方法返回传递给它的 `int` 值之和，前提是它不溢出。

## 异常:
如果和（即 `a + b`）在有符号整数运算中溢出，方法 `checkedAdd(int a, int b)` 将抛出 [`ArithmeticException`](https://docs.oracle.com/javase/8/docs/api/java/lang/ArithmeticException.html)。
