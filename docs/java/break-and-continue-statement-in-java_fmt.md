...
> 原文: [https://www.geeksforgeeks.org/break-and-continue-statement-in-java/](https://www.geeksforgeeks.org/break-and-continue-statement-in-java/)
...
`break` 语句用于从循环中立即终止。当在循环中遇到 `break` 语句时，循环迭代就此停止，控制立即从循环返回到循环后的第一条语句。基本上，`break` 语句用于我们不确定循环的实际迭代次数的情况，或者我们想要基于某种条件终止循环的情况。
...
`continue` 语句用于跳过循环的当前迭代。我们可以在任何类型的循环中使用 `continue` 语句，例如 `for`、`while` 和 `do-while` 循环。基本上，`continue` 语句用于我们想要继续循环，但不想要 `continue` 语句之后的剩余语句的情况。
