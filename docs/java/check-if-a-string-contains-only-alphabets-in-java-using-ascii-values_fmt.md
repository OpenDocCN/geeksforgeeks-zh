...
> 原文：[https://www.geeksforgeeks.org/check-if-a-string-contains-only-alphabets-in-java-using-ascii-values/](https://www.geeksforgeeks.org/check-if-a-string-contains-only-alphabets-in-java-using-ascii-values/)
...
2.  匹配字符串：
    ...
    *   如果字符串既不为`null`也不为空，则使用`ASCII`值逐个检查字符串字符的字母表。
3.  如果匹配，返回`true`。
