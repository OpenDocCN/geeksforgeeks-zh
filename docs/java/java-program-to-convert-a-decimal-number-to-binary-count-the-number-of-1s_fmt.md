# 将十进制数转换为二进制数的 Java 程序与计算 1 的个数

> 原文：[https://www.geeksforgeeks.org/java-program-to-convert-a-decimal-number-to-binary-count-the-number-of-1s/](https://www.geeksforgeeks.org/java-program-to-convert-a-decimal-number-to-binary-count-the-number-of-1s/)
...
在 Java 中，有两种方法可以转换，要么使用预定义的方法，要么使用三级逻辑构建，如下所示：

1.  使用 `Integer` 方法（整数包装类）
2.  使用蛮力方法（不使用任何预定义的类）
...
表示要转换为二进制的数字。Java 的 `Integer` 类提供了一些处理 `Integer` 的有用方法。其中一种方法是 `Integer.toBinaryString(int x)`。
...
字符串，它将转换后的整数的二进制表示或整数的简单二进制等价表示作为字符串对象。
...
要计算 1 的个数，检查得到的二进制字符串的每个字符是否等于 1。
