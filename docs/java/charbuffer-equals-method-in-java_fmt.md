...
> 原文: [https://www.geeksforgeeks.org/charbuffer-equals-method-in-java/](https://www.geeksforgeeks.org/charbuffer-equals-method-in-java/)
...
如果 `('a' == 'b') || (Character.isNaN(a) && Character.isNaN(b))`，此方法认为两个字符元素 `'a'` 和 `'b'` 相等。与 `Char.equals(Object)` 不同，值 `-0.0` 和 `+0.0` 被认为是相等的。
...
**返回值:** 当且仅当该缓冲区等于给定对象时，该方法返回 `true`。
