# AbstractMap.SimpleEntry hashCode() 方法示例

> 原文：[https://www.geeksforgeeks.org/abstractmap-simpleentry-hashcode-method-in-java-with-examples/](https://www.geeksforgeeks.org/abstractmap-simpleentry-hashcode-method-in-java-with-examples/)

`AbstractMap.SimpleEntry<K, V>` 用于维护一个键和值条目。可以使用 `setValue` 方法更改该值。这个类促进了构建自定义地图实现的过程。

`hashCode()` 方法返回 `AbstractMap.SimpleEntry<K, V>` 该地图条目的哈希码值。地图条目 `e` 的哈希代码可以通过以下方式计算：
...
[https://docs.oracle.com/javase/10/docs/api/java/util/AbstractMap.SimpleEntry.html#hashCode()](https://docs.oracle.com/javase/10/docs/api/java/util/AbstractMap.SimpleEntry.html#hashCode())