...
`IntSupplier`接口是从 Java 8 开始引入的`java.util.function`包的一部分，用于在 Java 中实现[函数式编程](https://www.geeksforgeeks.org/functional-programming-paradigm/)。它表示一个不接受任何参数但产生一个`int`值的函数。

分配给`IntSupplier`类型的对象的 lambda 表达式用于定义其`getAsInt()`，该表达式最终对其参数应用给定的操作。类似于使用[`Supplier<Integer>`](https://www.geeksforgeeks.org/supplier-interface-in-java-with-examples/)类型的对象。
