...
> 原文: [https://www.geeksforgeeks.org/add-ontouchlistener-to-imageview-to-perform-speech-to-text-in-android/](https://www.geeksforgeeks.org/add-ontouchlistener-to-imageview-to-perform-speech-to-text-in-android/)

**语音转文本**是一个特性，通常意味着用户说的任何话都会转换成文本。这个特性对于用户来说是一个非常常见和有用的特性。在像谷歌搜索这样实现搜索功能的地方，也可以在像谷歌键盘这样的应用程序中实现，因为它给用户带来了很好的体验。因此，在本文中，我们将使用[`ImageView`](https://www.geeksforgeeks.org/imageview-in-android-with-example/)的`OnTouchListener`实现语音到文本的转换，并将输入文本存储在[`EditText`](https://www.geeksforgeeks.org/edittext-widget-in-android-using-java-with-examples/)中。
...
最后，转到`app > manifest > AndroidManifest.xml`文件，并在那里添加下面写的权限。
