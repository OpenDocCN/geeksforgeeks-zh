...
> 原文：[https://www.geeksforgeeks.org/difference-between-getpath-and-getcononicalpath-in-java/](https://www.geeksforgeeks.org/difference-between-getpath-and-getcononicalpath-in-java/)
...
`getPath()` 是 `URL` 类的一个方法。它将给定的抽象路径名转换为路径名字符串。生成的字符串使用默认的名称分隔符来分隔名称序列中的名称。
...
`getAbsolutePath()` 返回一个 `Path` 对象，表示给定路径的绝对路径。如果给定的路径名已经是绝对的，那么路径名字符串就像通过 `getPath()` 方法一样简单地返回。如果该抽象路径名是空的抽象路径名，则是当前用户目录的路径名字符串，由系统属性 `user.dir()` 命名。否则，该路径名以系统相关的方式解析。

**返回：** 绝对路径名字符串，表示与该抽象路径名相同的文件或目录。

*   **在 Unix 系统上：** 通过相对于当前用户目录解析相对路径名，使其成为绝对路径名。
...
此方法返回给定抽象路径名的规范路径名字符串。规范路径名是绝对的和唯一的。此方法首先在必要时将路径名转换为绝对形式，就像通过调用 `getAbsolutePath()` 方法一样，然后以依赖于系统的方式将其映射到其唯一形式。
...
现在，在讨论了它们之后，让我们来深入探讨一下 `getPath()`、`getAbsolutePath()` 和 `getCanonicalPath()` 之间的差异，具体如下：
...
| **例**<br>*在窗口系统上*<br>`File path = new File("Notes/../demo.txt");`<br>**输出：** `Notes\..\demo.txt`<br>*在 Unix 系统上*<br>`File path = new File("Notes/../demo.txt");`<br>**输出：** `Notes/../demo.txt` | **例**<br>*窗口系统*<br>`File path = new File("Notes/../demo.txt");`<br>**输出：** `c:\user\ASPIRE\Desktop\Java\Notes\..\demo.txt`<br>*在 Unix 系统上*<br>`File path = new File("Notes/../demo.txt");`<br>**输出：** `home/pooja/桌面/笔记/../demo.txt` | **例**<br>*在车窗系统上*<br>`File path = new File("Notes/../demo.txt");`<br>**输出：** `c:\Users\ASPIRE\Desktop\Java\demo.txt`<br>*在 Unix 系统上*<br>`File path = new File("Notes/../demo.txt");`<br>**输出：** `/home/pooja/Desktop/Java/demo.txt` |