...
**4. `JFileChooser(FileSystemView)`** – 使用给定的文件系统视图。
...
**5. `JFileChooser(String, FileSystemView)`** – 使用给定的路径和文件系统视图。
...
**6. `JFileChooser(File, FileSystemView)`** – 使用给定的当前目录和文件系统视图。
...
注意：也可以使用`setApproveButtonText(字符串)`功能自定义审批按钮。这将把批准按钮的文本设置为所需的文本。