...
> 原文: [https://www.geeksforgeeks.org/iterate-list-in-java-using-loops/](https://www.geeksforgeeks.org/iterate-list-in-java-using-loops/)

在本文中，我们将看到如何遍历列表。在 Java 中，列表是集合框架的一个接口。列表可以是各种类型，如 `ArrayList`、`Stack`、`LinkedList` 和 `Vector`。有多种方法可以遍历 Java `List`，但这里我们只讨论使用循环遍历。因此，有标准的三个遍历可用，所以确实存在三个方法，但是随着 Java 8 和 `Stream` 的引入，其他方法也出现了。因此，所有四种方法讨论如下:
