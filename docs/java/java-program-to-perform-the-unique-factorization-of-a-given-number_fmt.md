...
> 原文：[https://www.geeksforgeeks.org/java-program-to-perform-the-unique-factorization-of-a-given-number/](https://www.geeksforgeeks.org/java-program-to-perform-the-unique-factorization-of-a-given-number/)
...
3.  假设一个数字低于 `sqrt(n)`，第二个数字高于 `sqrt(n)`。在这种情况下，总有至少一对数的乘积为 `n`，让 `x` 和 `y` 是乘积为 `n` 的两个数，我们可以写出 `x = sqrt(n) * sqrt(n)/y` 的第一个方程。

**有两个条件:**

*   当 `y < sqrt(n)` 即 `1 < sqrt(n)/y` 所以我们可以写出第一个方程 `x = sqrt(n)*(1+b)` 因此 `x > sqrt(n)`。
*   当 `y < sqrt(n)` 即 `1 > sqrt(n)/y` 时，我们可以写出第一个方程 `x = sqrt(n)*(1-b)`，因此 `x < sqrt(n)`。
