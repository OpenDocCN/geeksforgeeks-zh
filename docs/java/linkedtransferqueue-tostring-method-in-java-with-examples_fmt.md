...
> 原文：[https://www.geeksforgeeks.org/linkedtransferqueue-tostring-method-in-java-with-examples/](https://www.geeksforgeeks.org/linkedtransferqueue-tostring-method-in-java-with-examples/)

`java.util.concurrent.LinkedTransferQueue`的`toString()`方法是一个内置的 Java 函数，用于返回集合的字符串表示形式。字符串表示由包含在`"[]"`中并由`","`分隔的集合元素组成。字符串表示中元素的顺序按照迭代的顺序出现。
