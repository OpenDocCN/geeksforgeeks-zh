...
> 原文：[https://www.geeksforgeeks.org/commonly-asked-java-programming-interview-questions-set-1/](https://www.geeksforgeeks.org/commonly-asked-java-programming-interview-questions-set-1/)

## Java 为什么被称为‘平台无关编程语言’？
...
详见 [`final` 关键字](https://www.geeksforgeeks.org/final-keyword-java/)。
...
详见 [`super` 关键字](https://www.geeksforgeeks.org/super-keyword/)。

## StringBuffer 和 String 有什么区别？

`String` 是一个不可变的类，也就是说，一旦创建，您就不能修改它的内容。虽然 `StringBuffer` 是一个可变类，但这意味着您可以在以后更改它的内容。每当我们更改 `String` 对象的内容时，它都会创建一个新的字符串并引用它，而不会修改现有的字符串。这就是 `StringBuffer` 的性能优于 `String` 的原因。
详见 [`String` vs `StringBuilder` vs `StringBuffer`](https://www.geeksforgeeks.org/g-fact-27-string-vs-stringbuilder-vs-stringbuffer/)。
...
## Java 异常处理中的‘抛出’和‘抛出’有什么区别？

以下是两者之间的区别：

- `throw` 关键字用于从任何方法或静态块中抛出异常，而 `throws` 用于指示该方法可能抛出哪个异常。
- 如果任何方法抛出选中的异常，那么调用者可以处理这个异常（使用 `try catch` 块），或者通过在方法声明中声明另一个 `throws` 子句来重新抛出它。
...
与 C++ 不同，我们不需要在 Java 中显式销毁对象。[垃圾收集器](https://www.geeksforgeeks.org/garbage-collection-java/)自动为我们做。垃圾收集器检查是否不存在对某个对象的引用，该对象是否不再需要，以及该对象占用的内存是否可以释放。有时一个对象可以保存非 java 资源，如文件句柄或数据库连接，那么您需要确保这些资源在对象被销毁之前也被释放。为了执行这样的操作，Java 在对象类中提供了受保护的 `void finalize()`。您可以在类中重写此方法，并执行所需的任务。就在对象被释放之前，java 运行时调用该对象上的 `finalize()` 方法。详见[垃圾收集](https://www.geeksforgeeks.org/garbage-collection-java/)。

## Set 和 List 接口的区别？

`Set` 和 `List` 都是 `Collection` 接口的子接口。它们之间有以下两个主要区别：

- `List` 可以包含重复的值，但是 `Set` 不允许这样做。
- 在 `List` 接口中，数据以您插入的顺序出现，但在 `Set` 的情况下，插入顺序不会保留。

## 如果将 System.exit(0) 置于 try 或 catch block 上，会发生什么情况？最终会阻止执行吗？

通过在 `try` 或 `catch` block 中调用 `System.exit(0)`，我们可以跳过 `finally` block。`System.exit(int)` 方法可以引发 `SecurityException`。如果 `System.exit(0)` 退出 JVM 而没有抛出该异常，那么 `finally` 块将不会执行。但是，如果 `System.exit(0)` 确实引发了安全异常，那么将执行 `finally` block。

## 参考

- [常见 Java 编程面试问题 | 第二集](https://www.geeksforgeeks.org/commonly-asked-java-programming-interview-questions-set-2/)
- [Java 专业人士面试问题](https://www.geeksforgeeks.org/java-interview-questions/)
- 练习 Java 的[小测验](https://www.geeksforgeeks.org/quiz-corner-gq/)
- 爪哇[文章](https://www.geeksforgeeks.org/java/)

本文由**达尔梅什·辛格**整理。

您可能希望看到以下内容：

- [常见 C 编程面试问题 | 第 1 集](https://www.geeksforgeeks.org/commonly-asked-c-programming-interview-questions-set-1/)
- [常见 C 编程面试问题 | 第 2 集](https://www.geeksforgeeks.org/commonly-asked-c-programming-interview-questions-set-2/)
...
- [埃森哲最常问的面试问题](https://www.geeksforgeeks.org/accenture-interview-questions/)
...
- [常见的 DBMS 面试问题 | 集合 1](https://www.geeksforgeeks.org/commonly-asked-dbms-interview-questions-set-1/)
...
- [常见的数据结构面试问题](https://www.geeksforgeeks.org/commonly-asked-data-structure-interview-questions-set-1/)
