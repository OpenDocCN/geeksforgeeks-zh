...
> 原文:[https://www.geeksforgeeks.org/java-8-consumer-interface-in-java-with-examples/](https://www.geeksforgeeks.org/java-8-consumer-interface-in-java-with-examples/)

`Consumer`接口是`java.util.function`包的一部分，该包是从 Java 8 开始引入的，用于在 Java 中实现[函数式编程](https://www.geeksforgeeks.org/functional-programming-paradigm/)。它代表一个接受一个参数并产生一个结果的函数。然而，这类函数不返回值。
因此，该功能接口采用一个通用名称，即:-
...
**参数:**该方法接受`after`参数，该参数是当前参数之后要应用的消费者。
