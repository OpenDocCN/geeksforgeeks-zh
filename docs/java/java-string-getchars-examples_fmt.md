...
> 原文：[`https://www.geeksforgeeks.org/java-string-getchars-examples/`](https://www.geeksforgeeks.org/java-string-getchars-examples/)

`java`字符串`getChars()`方法将给定的字符串中的字符复制到目标字符数组中。
...
`StringIndexOutOfBoundsException` – 如果`srhStartIndex`、`srhEndIndex`不在适当的范围内。
