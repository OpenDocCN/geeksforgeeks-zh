...
> 原文: [https://www.geeksforgeeks.org/bigdecimal-longvalueexact-method-in-java/](https://www.geeksforgeeks.org/bigdecimal-longvalueexact-method-in-java/)

`BigDecimal.longValueExact()` 是一个内置函数，它将这个 `BigDecimal` 转换为一个 `long` 值，并检查丢失的信息。如果这个 `BigDecimal` 有任何小数部分，或者如果转换的结果太大而不能表示为 `long` 值，这个函数抛出 `ArithmeticException`。
...
如果这个 `BigDecimal` 中有非零小数部分或者其值太大无法表示为 `long`，则函数抛出 `ArithmeticException`。
...
下面的程序说明了 `BigDecimal.longValueExact()` 方法的使用：
...
**注意:** 与 `longValue()` 函数不同，`longValue()` 函数丢弃了任何小数部分，当转换结果太大而无法表示为 `long` 值时，该函数仅返回低阶 64 位，在这种情况下，该函数抛出 `ArithmeticException`。
