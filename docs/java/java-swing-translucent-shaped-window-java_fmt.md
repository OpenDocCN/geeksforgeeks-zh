...
> 原文: [https://www.geeksforgeeks.org/java-swing-translucent-shaped-window-java/](https://www.geeksforgeeks.org/java-swing-translucent-shaped-window-java/)

Java 提供了不同的函数，通过这些函数我们可以控制窗口或框架的半透明性。为了控制框架的不透明度，一定不要装饰。框架的不透明度是对框架或组件的半透明性的度量。
在 Java 中，我们可以通过两种方式创建有形状的窗口，第一种是使用 `AWTUtilities`，它是 `com.sun.awt` 包的一部分，`AWTUtilities` 类有一个函数 `setWindowShape(Window w, Shape s)`，它设置窗口的形状；第二种是使用 `setShape(Shape s)`，它将窗口的形状设置为指定的形状。
在本文中，我们将讨论窗口的均匀半透明性。

## 使用的方法

1.  `setWindowShape(Window w, Shape s)`：将窗口 `w` 的形状设置为指定的形状 `s`。
2.  `setShape(Shape s)`：将窗口的形状设置为指定的形状。
3.  `setOpacity(float f)`：设置帧的不透明度值。
...
## 2. 程序创建一个成型的半透明窗口(使用 AWTUtilities)并使用滑块控制其半透明性
...
## 3. 程序创建一个成型的半透明窗口(使用 setShape)并使用滑块控制其半透明性
