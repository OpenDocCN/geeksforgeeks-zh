# Java 中的 `Math.pow()` 方法示例

> 原文：[https://www.geeksforgeeks.org/math-pow-method-in-java-with-example/](https://www.geeksforgeeks.org/math-pow-method-in-java-with-example/)

`Math.pow()` 用于计算一个数与其他数的幂的乘积。该函数接受两个参数，并将第一个参数的值返回给第二个参数。下面列出了一些特殊情况：
