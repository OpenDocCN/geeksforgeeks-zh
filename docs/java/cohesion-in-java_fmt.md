...
**示例:** 假设我们有一个将两个数字相乘的类，但是同一个类创建了一个显示结果的弹出窗口。这是一个低内聚类的例子，因为窗口和乘法运算没有太多的共同点。为了使它具有高内聚性，我们必须创建一个类 `Display` 和一个类 `Multiply`。`Display` 将调用 `Multiply` 的方法来获得结果并显示它。这样才能开发出高内聚的解决方案。
...
*   高内聚是指你有一个做明确工作的类。低内聚是指一个类做很多没有多少共同点的工作。
*   高内聚给了我们更好的维护性，低内聚导致了难以维护、理解和降低重用性的单一类。
