...
> 原文：[https://www.geeksforgeeks.org/java-program-to-print-square-star-pattern/](https://www.geeksforgeeks.org/java-program-to-print-square-star-pattern/)
...
**步骤 2：** 对于矩形行，从 `1` 到 `rows` 运行外部循环。

```java
for (i = 1; i <= rows; i++)
```

**步骤 3：** 对于矩形的列，运行从 `1` 到 `columns` 的内部循环。
