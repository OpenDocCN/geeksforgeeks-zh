...
`BiMap` 即 **双向映射** 是一种既保留其值的唯一性又保留其键的唯一性的映射。`BiMap` 支持**反向视图**，这是另一个 `BiMap`，包含与此 `BiMap` 相同的条目，但具有反向的键和值。

**声明:** 声明为 `com.google.common.collect.BiMap<K, V>` 接口如下:
...
*   `putAll`: 如果尝试放入任何条目失败，将抛出 `IllegalArgumentException`。请注意，在引发异常之前，一些映射条目可能已添加到 `BiMap` 中。
*   `values`: 返回一个集合，而不是映射接口中指定的集合，因为 `BiMap` 具有唯一的值。
