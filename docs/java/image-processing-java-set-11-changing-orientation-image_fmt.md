...
主要思想是输入 `BufferedImage` 对象将被转换成 `Mat` 对象，然后将创建新的 `Mat` 对象，其中原始 `Mat` 对象值在方向修改后被放置。
...
*   `put(int row，int column，byte[] data)` / `get(int row，int column，byte[]data)` –用于将原始数据读取/写入 `Mat` 对象。
*   `flip(Mat mat1，Mat mat2，int flip_value)` –`mat1` 和 `mat2` 对应于输入和输出 `Mat` 对象，`flip_value` 决定方向类型，`flip_value` 可以是 `0`(沿 x 轴翻转)、`1`(沿 y 轴翻转)、`-1`(沿两个轴翻转)。
