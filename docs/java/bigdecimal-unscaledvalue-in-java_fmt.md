...
> 原文: [https://www.geeksforgeeks.org/bigdecimal-unscaledvalue-in-java/](https://www.geeksforgeeks.org/bigdecimal-unscaledvalue-in-java/)

`BigDecimal.unscaledValue()` 是 Java 中的一个内置方法，它返回一个 `BigInteger`，其值是 `BigDecimal` 值的未缩放值。该值计算为 `(this * 10^this.scale())`。
...
这个方法返回一个 `BigInteger`，它的值是这个 `BigDecimal` 值的未缩放值。
...
[https://docs.oracle.com/javase/7/docs/api/java/math/BigDecimal.html#unscaledValue()](https://docs.oracle.com/javase/7/docs/api/java/math/BigDecimal.html#unscaledValue())