# Java 中的 `Collections.frequency()`

> 原文: [https://www.geeksforgeeks.org/java-util-collections-frequency-java/](https://www.geeksforgeeks.org/java-util-collections-frequency-java/)

该方法是一个 `java.util.Collections` 类方法。它计算给定列表中指定元素的频率。它覆盖 `equals()` 方法来执行比较，以检查指定的对象和列表中的对象是否相等。
...
如果集合 `c` 为 `null`，则抛出 `NullPointerException`。
...
## 对自定义对象使用 `Collections.frequency()`
