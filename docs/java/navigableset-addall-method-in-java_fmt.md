...
> 原文: [https://www.geeksforgeeks.org/navigableset-addall-method-in-java/](https://www.geeksforgeeks.org/navigableset-addall-method-in-java/)

`java.util.NavigableSet.addAll(Collection C)`方法用于将上述集合中的所有元素追加到现有的`NavigableSet`中。元素是随机添加的，没有遵循任何特定的顺序。
...
参数`C`是要添加到导航集中的任何类型的集合。

## 返回值

如果该方法成功地将集合`C`的元素追加到该导航集中，则该方法返回真，否则返回假。

## 注

Java 中`NavigableSet`接口的`addAll()`方法是从`Set`接口继承而来的。

下面的程序说明了`java.util.NavigableSet.addAll()`方法:
