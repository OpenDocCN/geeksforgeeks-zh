...
> 原文：[https://www.geeksforgeeks.org/how-to-make-object-eligible-for-garbage-collection/](https://www.geeksforgeeks.org/how-to-make-object-eligible-for-garbage-collection/)

如果对象的引用变量在执行过程中从程序中丢失，则该对象有资格被[垃圾收集](https://www.geeksforgeeks.org/garbage-collection-java/)。有时它们也被称为**遥不可及的物体**。

## 什么是物体的参照物？

`new`运算符为对象动态分配内存，并返回对它的引用。这个引用是`new`分配的对象在内存中的地址。引用是一个地址，指示对象的变量、方法等在哪里被储存起来。
...
当一个方法被调用时，它会进入栈帧。当方法从栈中弹出时，它的所有成员都会消亡，如果在其中创建了一些对象，那么这些对象在方法执行后将变得不可达或匿名，因此符合垃圾收集的条件。
...
当一个对象的引用 ID 被引用到另一个对象的引用 ID 时，前一个对象就不再有任何引用指向它，变得不可达，因此符合垃圾收集的条件。
...
当一个对象的所有引用变量都被更改为`NULL`时，它变得不可达，因此符合垃圾收集的条件。
...
匿名对象的引用 ID 没有存储在任何地方。因此，它变得不可达。
