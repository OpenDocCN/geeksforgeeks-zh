...
> 原文: [https://www.geeksforgeeks.org/concurrentlinkeddeque-pop-method-in-java-with-examples/](https://www.geeksforgeeks.org/concurrentlinkeddeque-pop-method-in-java-with-examples/)

Java 中的 `java.util.ConcurrentLinkedDeque.pop()` 方法用于从 `ConcurrentLinkedDeque` 中弹出一个元素。该元素从 `ConcurrentLinkedDeque` 的顶部弹出，并从其中移除。
...
这个方法返回出现在 `ConcurrentLinkedDeque` 顶部的元素，然后删除它。

## 异常

方法抛出 `EmptyConcurrentLinkedDequeException`，如果 `ConcurrentLinkedDeque` 为空则抛出。

下面的程序说明了 `java.util.concurrentlinkeddeque.pop()` 方法：
