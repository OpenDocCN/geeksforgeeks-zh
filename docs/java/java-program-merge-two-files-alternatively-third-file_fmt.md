...
> 原文：[https://www.geeksforgeeks.org/java-program-merge-two-files-alternatively-third-file/](https://www.geeksforgeeks.org/java-program-merge-two-files-alternatively-third-file/)

## 先决条件
*   [`PrintWriter`](https://www.geeksforgeeks.org/java-io-printwriter-class-java-set-1/)
...
### 步骤
1.  为文件 `file3.txt` 创建一个 `PrintWriter` 对象。
2.  为文件 `file1.txt` 打开 `BufferedReader`。
3.  为文件 `file2.txt` 打开 `BufferedReader`。
4.  运行循环，将 `file1.txt` 的每一行复制到 `file3.txt`，然后将 `file2.txt` 的每一行复制到 `file3.txt`。
...
## 注意
如果当前工作目录（`cwd`）中存在 `file3.txt`，那么它将被上述程序覆盖，否则将创建新文件。
