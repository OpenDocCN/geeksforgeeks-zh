# CopyOnWriteArrayList 在 Java 中的 contains() 方法

> 原文: [https://www.geeksforgeeks.org/copyonwritearraylist-contains-method-in-java/](https://www.geeksforgeeks.org/copyonwritearraylist-contains-method-in-java/)

`contains(Object e)` 方法检查列表中是否存在给定的元素。如果元素至少出现一次，则函数返回 `true`，否则返回 `false`。
...
该函数接受单个强制参数 `o`，该参数指定要在列表中检查其出现的元素。

## 返回值

如果元素在列表中至少出现一次，函数返回 `true`，否则返回 `false`。
...
[https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/CopyOnWriteArrayList.html#contains-E-](https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/CopyOnWriteArrayList.html#contains-E-)