...
| 方法 | 说明 |
| --- | --- |
| `valueOf(String name)` | 返回指定名称的循环方法。 |
| `values()` | 返回一个包含 `CycleMethod` 类型值的数组。 |

## 程序示例

### 1. 创建 LinearGradient 对象并应用到矩形
以下程序说明了 `CycleMethod` 类的使用：创建一个 `LinearGradient` 对象，为其添加停止点，将 `CycleMethod` 设置为 `repeat`，将 `proportional` 设置为 `false` 并将其应用于矩形。
...
### 2. 创建 LinearGradient 对象并应用到圆形
Java 程序创建一个 `LinearGradient` 对象，为其添加停止点，将 `CycleMethod` 设置为 `reflect`，将 `proportional` 设置为 `false` 并将其应用于圆形。

1.  在这个程序中，我们将创建一个停止对象的数组，它们的偏移值范围从 0 到 1。
2.  然后创建一个带有指定停止点的线性渐变对象。将循环方法设置为反射，将比例设置为假。
...
### 3. 创建 LinearGradient 对象并应用到矩形（无循环）
Java 程序创建 `LinearGradient` 对象，为其添加停止点，将 `CycleMethod` 设置为 `no cycle`，将 `proportional` 设置为 `false` 并将其应用于矩形。
