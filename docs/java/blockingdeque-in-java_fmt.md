...
阻塞队列从阻塞队列接口延伸，阻塞队列接口又从 [`Queue` 接口](https://www.geeksforgeeks.org/queue-interface-java/) 和 [`Deque` 接口](https://www.geeksforgeeks.org/deque-interface-java-example/) 延伸。
...
封锁命令的实现类是 [`LinkedBlockingDeque`](https://www.geeksforgeeks.org/linkedblockingdeque-in-java-with-examples/) 。这个类是阻塞请求和[链表数据结构](https://www.geeksforgeeks.org/data-structures/linked-list/)的实现。`LinkedBlockingDeque` 可以选择使用构造函数进行绑定，但是，如果容量未指定，则为*整数。最大值*默认。节点在插入时按照容量限制动态添加。要在代码中使用 `BlockingDeque`，请使用以下导入语句。
