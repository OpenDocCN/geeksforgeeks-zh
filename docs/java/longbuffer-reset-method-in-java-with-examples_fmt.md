...
> 原文：[`https://www.geeksforgeeks.org/longbuffer-reset-method-in-java-with-examples/`](https://www.geeksforgeeks.org/longbuffer-reset-method-in-java-with-examples/)

[`java.nio.LongBuffer`](https://www.geeksforgeeks.org/tag/java-LongBuffer/)类的`reset()`方法用于将该缓冲区的位置重置到之前标记的位置。在此过程中，标记的值保持不变。
