...
警报对话框代码有三种`方法`:
*   `setTitle()`显示报警对话框标题的方法
*   `setMessage()`显示消息的方法
*   `setIcon()`方法用于在报警对话框中设置图标。

然后我们将两个按钮`setPositiveButton()`和`setNegativeButton()`添加到我们的警告对话框中，如下所示。
...
*   **第 6 步:**如果是肯定的，按完成按钮，应用程序从应用程序转到外面，如果是否定的，则`finish()`对话框或`dialog.cancel()`。
