...
> 原文：[https://www.geeksforgeeks.org/adding-images-to-a-word-document-using-java/](https://www.geeksforgeeks.org/adding-images-to-a-word-document-using-java/)
...
> `run.addPicture(InputStream imageData, int imageType, String imageFileName, int width, int height)`

## 参数

*   `imageData`：原始图像数据
*   `imageType`：图片的类型，例如 `XWPFDocument.PICTURE_TYPE_JPEG`
*   `imageFileName`：图像的文件名称
*   `width`：图像的宽度
*   `height`：图像的高度

## 实现步骤

1.  使用 Apache POI 包的 [`XWPFDocument`](https://www.geeksforgeeks.org/java-program-to-write-a-paragraph-in-a-word-document/) 创建一个空白文档。
...
3.  分别创建文字和图像的 [`FileOutputStream`](https://www.geeksforgeeks.org/fileoutputstream-in-java/) 和 [`FileInputStream`](https://www.geeksforgeeks.org/java-io-fileinputstream-class-java/)。
...
*   **步骤 1：** 创建空白文档
*   **步骤 2：** 创建段落
*   **步骤 3：** 在所需位置创建 word 文档的 `FileOutputStream`
*   **步骤 4：** 通过指定图像的路径来创建图像的 `FileInputStream`
*   **步骤 5：** 检索图像文件名和图像类型
*   **步骤 6：** 以像素为单位设置图像的宽度和高度
*   **步骤 7：** 使用 `addPicture()` 方法添加图片并写入文档
*   **步骤 8：** 关闭连接
