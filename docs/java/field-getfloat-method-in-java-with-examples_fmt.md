...
> 原文：[https://www.geeksforgeeks.org/field-getfloat-method-in-java-with-examples/](https://www.geeksforgeeks.org/field-getfloat-method-in-java-with-examples/)

[`Field`](https://www.geeksforgeeks.org/reflection-in-java/) 类的 `getFloat()` 方法用于获取浮点值，该字段必须是静态或实例字段类型。此方法还用于通过扩展转换获取另一个可转换为类型 `float` 的基元类型的值。当一个类包含一个静态或实例浮动字段，并且我们想要得到该字段的值，那么我们可以使用这个方法返回字段的值。
...
## 参数：
该方法接受单个参数 `obj`，该参数是提取浮点值的对象。

## 返回值：
该方法返回转换为类型 `float` 的字段的值。
...
4.  `ExceptionInInitializerError`：如果此方法引发的初始化失败，将引发此异常。
