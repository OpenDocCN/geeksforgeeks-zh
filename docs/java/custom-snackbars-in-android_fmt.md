...
`snackbar`在用户体验中起着非常重要的作用。可以包含或不包含要做的动作按钮的 Snackbar 立即显示由于用户交互而发生的消息。因此，在本文中，已经讨论了如何使用自定义布局来实现 SnackBars。请看下图，了解如何将定制 Snackbar 与常规（普通）Snackbar 区分开来。注意，我们将使用 `Java` 语言来实现这个项目。
