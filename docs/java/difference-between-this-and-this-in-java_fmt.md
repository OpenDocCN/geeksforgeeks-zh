...
> 原文：[https://www.geeksforgeeks.org/difference-between-this-and-this-in-java/](https://www.geeksforgeeks.org/difference-between-this-and-this-in-java/)

在Java中，`this`和`this()`是完全不同的。

*   `this`关键字是用来引用当前对象的，即通过它调用方法。
*   `this()`用于从同一个类的另一个构造函数调用一个构造函数。

下表显示了`this`关键字和`this()`之间的点对点差异。

| **this** | **this()** |
| --- | --- |
| 此关键字仅用于对象。 | `this()`仅用于构造函数。 |
| 它指的是当前对象。 | 它指的是参数与传递给`this()`的参数（即构造函数参数）匹配的同一个类的构造函数。 |
| 点（`.`）运算符用于访问成员。例如，`this.variableName` | 没有点（`.`）运算符。只传递匹配的参数。 |
| 它用于区分方法中的局部变量和实例变量。 | 它用于引用属于同一类的构造函数。 |

参见下面的代码，描述了`this`关键字的使用。
