...
> 原文：[https://www.geeksforgeeks.org/bigdecimal-setscale-method-in-java-with-examples/](https://www.geeksforgeeks.org/bigdecimal-setscale-method-in-java-with-examples/)

`setScale()` 用于设置 `BigDecimal` 的小数位数。此方法对调用此方法的当前大十进制数执行操作。
...
**注：** 自 Java 9 以来，`setScale(int newScale, int roundingMode)` 已被弃用。
...
该方法接受一个参数 `newScale`，用于设置该大十进制的刻度。

### 返回值

此方法返回一个大十进制数，其小数位数为指定值。

### 异常

如果指定的缩放操作需要舍入，则抛出 `ArithmeticException`。

### 注意

由于 `BigDecimal` 对象是不可变的，调用此方法不会导致原始对象被修改，所以 `setScale` 返回一个具有适当比例的对象。返回的对象可能是新分配的，也可能不是。
...
该方法接受两个参数 `newScale`，用于设置该大十进制的刻度，`roundingMode` 用于设置结果的舍入值。

### 返回值

该方法返回一个大十进制数，其小数位数为指定值。

### 异常

如果舍入模式是不必要的，并且指定的缩放操作需要舍入，则该方法抛出 `ArithmeticException`。如果舍入模式不代表有效的舍入模式，该方法还会抛出 `IllegalArgumentException`。
...
该方法接受两个参数 `newScale`，用于设置该大十进制的标度和 `RoundingMode` 类型的舍入模式，告知应用哪种舍入模式。
...
如果舍入模式是不必要的，并且指定的缩放操作需要舍入，则该方法抛出 `ArithmeticException`。
