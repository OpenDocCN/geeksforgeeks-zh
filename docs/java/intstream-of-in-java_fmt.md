...
`IntStream of(int t)` 返回一个包含单个指定元素的连续 `IntStream`。
...
`IntStream of(int... values)` 返回一个顺序的有序流，其元素是指定的值。
...
如果我们已经对流执行了终端操作，并再次尝试重用它们，则流是不可重用的，将生成 `IllegalStateException`。
...
要重用一个流，我们需要 `Supplier` 类，此时每次调用 `Supplier` 的 `get()` 方法都会生成一个新的实例并返回。
