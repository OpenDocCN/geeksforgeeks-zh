...
`java.lang`包的`Integer`类的`compare()`方法比较给定的两个整数值(`x`，`y`)作为参数，如果(`x==y`)则返回值零，如果(`x < y`)则返回值小于零，如果(`x > y`)则返回值大于零。
...
返回 (`Return`) :
此方法在(`x==y`)时返回值零，如果(`x < y`)则返回一个小于零的值，如果(`x > y`)则返回一个大于零的值。

## 示例:

展示`java.lang.Integer.compare()`方法的工作。
