...
*   `src`: 源图像
*   `dst`: 影像目标
*   `Size(w, h)`: 宽 `w` 像素和高 `h` 像素的内核大小
*   `Point(-1, -1)`: 指示相对于邻域的锚点位置。如果有负值，内核的中心被认为是锚点。
...
*   `src`: 源图像
*   `dst`: 影像目标
*   `Size(w, h)`: 要使用的内核的大小。如果 `w` 和 `h` 不是奇数和正数，将使用 `σx` 和 `σy` 参数计算大小。
*   `σx`, `σy`: x 和 y 中的标准差，写 0 意味着 `σx` 和 `σy` 是用核大小计算的。
