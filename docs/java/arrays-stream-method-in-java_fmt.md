...
该方法接受一个强制参数 `array`，该数组的元素将被转换为一个顺序流。

### 返回值

该方法从作为参数传递的数组中返回一个顺序流。
...
Java 中 `Arrays` 类的 `stream(T[] array, int startInclusive, int endExclusive)` 方法用于从作为参数传递的数组中获取一个顺序流，该数组只有一些特定的元素。这些特定元素取自作为参数传递给此方法的一系列索引。它返回一个以指定数组的指定范围为源的顺序流。
...
*   `array` 是其元素将被转换为连续流的数组。
*   `startInclusive` 是第一个覆盖的索引，包含。
*   `endExclusive` 是紧接着要覆盖的最后一个索引之后的索引。

### 返回值

该方法返回一个顺序流，该顺序流由作为参数传递的数组元素范围形成。
