...
> 原文: [https://www.geeksforgeeks.org/difference-between-servletconfig-and-servletcontext-in-java-servlet/](https://www.geeksforgeeks.org/difference-between-servletconfig-and-servletcontext-in-java-servlet/)
...
*   **举例:**
    假设，一个人的工作门户的名字是“NewWebsite.tg”。在不同 servlet 提供的网页顶部显示网站名称，需要将网站名称存储在每个 servlet 中，以避免冗余。由于 `ServletContext` 共享的信息可以被每个 Servlet 访问，所以最好使用 `ServletContext`，并在需要时使用 `getServletContext().getInitParameter(“名称”)` 检索网站名称。
...
*   如上图所示，不同的 servlet 由于在 `web.xml` 中附加了不同的值而获得了具有相同参数名“email”的不同的 email-id。每个 servlet 在包含 email-id 的 `<servlet>` 中都有自己的 `<init-param>`。
*   两个 servlets 都获得了 `ServletContext` 参数“网站名称”的访问权限。因为在 `web.xml` 中，`<context-param>` 在 `<servlet>` 之外，所以它独立于 servlet，可以从整个应用程序中访问。
...
| `servletConfig` 的参数以名称-值对的形式出现在 `<servlet>` 内的 `<init-param>` 中。 | `servletContext` 的参数在 `<servlet>` 外部和 `<web-app>` 内部的 `<context-param>` 中以名称-值对的形式出现 |
