...
> 原文: [https://www.geeksforgeeks.org/multi-threaded-chat-application-set-1/](https://www.geeksforgeeks.org/multi-threaded-chat-application-set-1/)
...
### 1. 服务器类
主服务器实现简单，与上一篇文章类似。以下几点将有助于理解服务器实施:
...
### 2. ClientHandler 类
与上一篇文章类似，我们创建了一个用于处理各种请求的助手类。这一次，随着套接字和流，我们引入了一个名称变量。这将保存连接到服务器的客户端的名称。以下几点将有助于理解 `ClientHandler` 的实现:
