# 如何在 Java 中使用 Comparable 接口对向量元素进行排序？

> 原文：[https://www.geeksforgeeks.org/how-to-sort-vector-elements-using-comparable-interface-in-java/](https://www.geeksforgeeks.org/how-to-sort-vector-elements-using-comparable-interface-in-java/)

`Vector` 是 `Collection` 的子接口。如果我们想把一组单独的对象表示成一个单一的实体，其中允许重复并且插入顺序必须保留，那么我们应该选择向量。它是一个可调整大小或可增长的数组。它实现了 `Serializable`、`Cloneable` 和 `RandomAccess` 接口。向量中的每个方法都是同步的，因此向量对象是线程安全的。

## Comparable 接口

`Comparable` 接口用于对自定义类的对象进行排序。这个接口存在于 `java.lang` 包中，它只包含一个方法 `compareTo(Object)`。但是它只提供了一个单一的排序序列，也就是说，我们可以根据单个数据成员对元素进行排序。

### compareTo() 方法

此方法用于将当前对象与指定对象进行比较，并根据两个对象的比较结果返回三个值。使用 `Comparable` 接口，我们可以对 `String` 类对象、用户定义的类对象和所有包装器类对象的元素进行排序。
...
*   一个负值当且仅当第一个对象必须排在第二个对象之前。
*   一个正值当且仅当第一个对象必须排在第二个对象之后。
*   仅当第一个对象等于第二个对象时才为零。

## Collections 类

`Collections` 是一个存在于 `java.util` 包中的实用程序类，用于定义排序、搜索和反转等操作集合对象的多种实用方法。

为了对列表的元素进行排序，`Collections` 类定义了如下两种排序方式：

*   `static void sort(List l)`: 根据默认的自然排序顺序进行排序。在这种情况下，列表应包含相似的对象或实现了 `Comparable` 的对象，否则我们将得到运行时异常，并且列表不应包含 `null` 值，否则我们将得到 `NullPointerException`。
*   `static void sort(List L, Comparator C)`: 根据自定义的排序顺序进行排序。

## 实现

考虑创建一个自定义类名 `Student`，它实现了 `Comparable` 接口，在这个接口中我们可以获得每个学生的 id。在驱动程序类内部，我们创建了一个 `Student` 类类型的向量，然后我们可以通过比较学生的 id，使用 `Comparable` 接口对向量元素进行排序。
...
学生姓名、标记和 id，其中我们将根据学生标记进行排序。

```java
// Java Program to Sort Vector Elements
// using Comparable Interface

// Importing Collection and Vector classes from
// java.util package
import java.util.Collections;
import java.util.Vector;
import java.util.*;
...
class Student implements Comparable<Student> {
    ...
    // Comparing the marks of two student
    // using the compareTo() method
    public int compareTo(Student otherStudent) {
