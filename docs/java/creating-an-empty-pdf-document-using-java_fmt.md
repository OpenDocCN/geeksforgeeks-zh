...
> 原文：[https://www.geeksforgeeks.org/creating-an-empty-pdf-document-using-java/](https://www.geeksforgeeks.org/creating-an-empty-pdf-document-using-java/)
...
`PdfWriter` 类属于 `com.itextpdf.kernel.pdf` 包。我们可以说这个类是一个 PDF 的文档编写器。在实例化 `PdfWriter` 对象的对象时，我们必须传递一个字符串值，它代表文件的路径，我们希望在这里创建 PDF。

**注意：** 在传递文件的路径时，确保你的 IDE 有**权限在该目录下写/读文件**，否则会给出 `FileNotFound` 错误。
...
`PdfDocument` 类属于包 `com.itextpdf.kernel.pdf`。此类代表 **PDF 文档**。在实例化这个类时，我们必须传递 `PdfWriter` 对象作为参数。
...
`Document` 类属于包 `com.itextpdf.layout`。是 `IText` 的核心类之一。如果你想从头开始生成一个 PDF，那么你必须使用 `Document` 类。在实例化 `Document` 类时，我们需要传递 `PdfDocument` 对象作为参数。
