...
该方法接受一个参数 `field`，指定要检查的临时字段，空返回假。

## 返回

如果该日期支持 `field`，则函数返回真，否则返回假。

下面的程序说明了 `时区数据时间.问题支持()` 方法：
...
[https://docs.oracle.com/javase/9/docs/api/java/time/chrono/ChronoZonedDateTime.html#isSupported-java.time.temporal.TemporalField-](https://docs.oracle.com/javase/9/docs/api/java/time/chrono/ChronoZonedDateTime.html#isSupported-java.time.temporal.TemporalField-)