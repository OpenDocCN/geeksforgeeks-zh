# 如何在安卓中使用 Retrofit 更新 API 中的数据？

> 原文：[https://www.geeksforgeeks.org/how-to-update-data-in-api-using-retrofit-in-android/](https://www.geeksforgeeks.org/how-to-update-data-in-api-using-retrofit-in-android/)

我们已经看到从 API 读取数据，以及在 API 的帮助下将数据发布到我们的数据库中。在本文中，我们将看看如何在我们的 API 中更新数据。我们将使用 Retrofit 库来更新我们的 API 中的数据。

## 本文我们要构建什么？

我们将构建一个简单的应用程序，在其中我们将显示一个简单的表单，通过该表单，我们将更新我们的数据，并将其传递给我们的 API 来更新该数据。我们将使用 `PUT` 请求和 Retrofit 库来更新我们的数据到 API。下面给出了一个示例视频，以了解我们将在本文中做什么。注意，我们将使用 **Java** 语言来实现这个项目。
...
下面是 Retrofit 的依赖，我们将使用它从 API 获取数据。要添加此依赖项，请导航至 `应用程序 > 渐变脚本 > build.gradle (应用程序)`，并在依赖项部分添加以下依赖项。
...
### 第六步：为我们的 API 调用创建一个接口类

导航到 `应用程序 > java > 你的应用程序的包名 > 右键单击它 > 新建 > Java 类` 选择它作为 `RetrofitAPI` 并添加下面的代码。代码中添加了注释，以便更详细地了解。
