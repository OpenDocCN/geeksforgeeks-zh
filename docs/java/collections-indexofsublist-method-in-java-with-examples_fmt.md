...
> 原文：[https://www.geeksforgeeks.org/collections-indexofsublist-method-in-java-with-examples/](https://www.geeksforgeeks.org/collections-indexofsublist-method-in-java-with-examples/)

`java.util.Collections` 类的 `indexOfSubList()` 方法用于返回指定目标列表在指定源列表中第一次出现的起始位置，如果没有则返回 -1。更正式地说，返回最低的索引 `i`，使得 `source.subList(i, i+target.size()).equals(target)`，如果没有这样的索引，则为 -1。（如果 `target.size() > source.size()` 则返回 -1。）
...
*   `source` – 要在其中搜索第一个目标的列表。
*   `target` – 要搜索作为源列表子列表的列表。

## 返回值

该方法返回指定源列表内指定目标列表第一次出现的起始位置，如果没有出现则返回 -1。

以下是示例，说明 `indexOfSubList()` 方法。
