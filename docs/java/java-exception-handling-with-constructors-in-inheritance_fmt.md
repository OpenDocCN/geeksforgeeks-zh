...
> 原文：[https://www.geeksforgeeks.org/java-exception-handling-with-constructors-in-inheritance/](https://www.geeksforgeeks.org/java-exception-handling-with-constructors-in-inheritance/)
...
现在，处理异常有两种方法，一种是捕获异常，另一种是抛出异常。但是在构造器的情况下，我们不能使用[try-catch机制](https://www.geeksforgeeks.org/flow-control-in-try-catch-finally-in-java/)来处理。原因是我们将代码放入 `try` 块中，这样可以引发异常，然后捕获它。引发异常的原因是对父类构造函数的调用，如 `super()`。这意味着如果我们想使用 `try-catch` 处理异常，如下图所示。
