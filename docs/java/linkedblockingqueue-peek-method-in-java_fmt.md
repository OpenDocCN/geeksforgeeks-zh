...
这个方法返回 `LinkedBlockingQueue` 的头。

下面的程序说明了 `LinkedBlockingQueue` 类的 `peek()`方法:

## 程序 1:

使用 `peek()`方法返回 `LinkedBlockingQueue` 的头部，其中容量为 7 的 `LinkedBlockingQueue` 包含名称列表
...
[`https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/LinkedBlockingQueue.html#peek--`](https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/LinkedBlockingQueue.html#peek--)