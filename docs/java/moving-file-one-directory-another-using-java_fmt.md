...
> 原文：[https://www.geeksforgeeks.org/moving-file-one-directory-another-using-java/](https://www.geeksforgeeks.org/moving-file-one-directory-another-using-java/)

Java 提供了在目录之间移动文件的功能。这里描述了实现这一点的两种方法。第一种方法利用 `Files` 包进行移动，而另一种方法首先将文件复制到目标，然后从源中删除原始副本。

## 方法一：使用 `Files.move()` 方法
...
## 方法二：使用 `File.renameTo()` 和 `File.delete()` 方法
...
本文由 **Mayank Kumar** 供稿。如果你喜欢 GeeksforGeeks 并想投稿，也可以使用 [contribute.geeksforgeeks.org](http://www.contribute.geeksforgeeks.org/) 写一篇文章，或者把文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
