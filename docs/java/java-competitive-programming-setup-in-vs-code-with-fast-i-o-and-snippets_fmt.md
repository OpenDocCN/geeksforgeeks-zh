# VS Code 中带有快速输入输出和代码片段的 Java 竞争性编程设置

> 原文：[https://www.geeksforgeeks.org/java-competitive-programming-setup-in-vs-code-with-fast-i-o-and-snippets/](https://www.geeksforgeeks.org/java-competitive-programming-setup-in-vs-code-with-fast-i-o-and-snippets/)
...
1.  安装 `VS Code` 并设置 `JDK`（如果没有安装）。让我们继续。
2.  为快速输入/输出设置一个片段。
3.  设置输入和输出文件。
4.  分割你的屏幕。
...
### 步骤 1：安装 VS Code 并设置 JDK

*   从[官方 `VS Code` 网站](https://code.visualstudio.com/download)安装 `VS Code`。
...
**2.1** 打开 `VS Code`，转到 **文件 -> 偏好设置 -> 用户片段**。
...
*   **`Template for CP`** 是片段的名称。它用于在代码完成时识别代码片段（用红色标记）。

![](img/f32de8b3a586a912b7d9ffabae7158f7.png)

*   **`template`** 是用于触发代码完成的前缀（用绿色标记）。
*   **`body`** 是代码片段所在的地方。这里需要注意的一些要点是：
...
*   **`description`** 用于对片段留一个简短的备注，以供日后参考。
...
*   搜索并安装名为 **`Code Runner`** 的扩展。
*   使用 **`Run Code`** 命令或使用 **`CTRL+ALT+N`** 运行代码。
...
*   打开 `VS Code`，打开包含输入、输出和 `Java` 程序文件的文件夹。为此，请转到 **文件 -> 打开文件夹** 并选择您的文件夹。
