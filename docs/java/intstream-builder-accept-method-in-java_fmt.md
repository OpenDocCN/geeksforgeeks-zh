# IntStream.Builder accept(int t) 方法

> 原文：[https://www.geeksforgeeks.org/intstream-builder-accept-method-in-java/](https://www.geeksforgeeks.org/intstream-builder-accept-method-in-java/)

`IntStream.Builder accept(int t)` 用于在流的构建阶段将元素插入到流中。它接受正在构建的流的元素。
...
**异常：** 当构建器已经转换到构建状态时，该方法抛出 `IllegalStateException`。这意味着该流已经进入构建阶段，现在不能更改。因此不能再接受更多的元素进入流。
