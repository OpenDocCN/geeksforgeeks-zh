...
> 原文: [https://www.geeksforgeeks.org/collections-singleton-method-java/](https://www.geeksforgeeks.org/collections-singleton-method-java/)

`java.util.Collections.singleton()` 方法是一个 [`java.util.Collections`](https://www.geeksforgeeks.org/java/#CollectionsClass) 类方法。它在单个指定的元素上创建一个不可变的集合。此方法的一个应用是从集合中移除一个元素，如列表和集合。
