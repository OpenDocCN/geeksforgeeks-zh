...
1.  `Throwable(Throwable cause)`：其中 `cause` 是导致当前异常的异常。
2.  `Throwable(String message, Throwable cause)`：其中 `message` 是异常消息，`cause` 是导致当前异常的异常。

## 支持 java 中链式异常的可抛出类的方法

1.  `getCause()` 方法：该方法返回异常的实际原因。
2.  `initCause(Throwable cause)` 方法：该方法设置调用异常的原因。
