...
> 原文：[https://www.geeksforgeeks.org/can-we-define-a-method-name-same-as-class-name-in-java/](https://www.geeksforgeeks.org/can-we-define-a-method-name-same-as-class-name-in-java/)
...
这里我们不通过一个对象调用 `void Main(int)`，而是通过 `this(5)` 调用，其显示错误，所以这不能是一个构造函数。这会给你一个错误，显示没有参数化的构造函数可用，但是我们认为我们已经有了它，那就是 `void Main(int a)`。但是 `void Main(int a)` 的行为并不是为了证明，删除上面代码中的注释部分就可以了。
