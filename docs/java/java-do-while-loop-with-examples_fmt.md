...
> 原文: [https://www.geeksforgeeks.org/java-do-while-loop-with-examples/](https://www.geeksforgeeks.org/java-do-while-loop-with-examples/)

当我们需要重复执行一段语句时，Java 中的[循环](https://www.geeksforgeeks.org/loops-in-java/)开始使用。

Java [边做边循环](https://www.geeksforgeeks.org/java/)是**出口控制循环**。因此，与 `for` 或 `while` 循环不同的是，在执行语句或循环体之后，执行 `while` 检查条件。
