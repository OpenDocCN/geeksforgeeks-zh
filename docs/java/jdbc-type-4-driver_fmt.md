...
为了更好地理解什么是类型 4 驱动程序，让我们对 JDBC 和以前的 ODBC 做一个概述。一个 [`JDBC 驱动程序`](https://www.geeksforgeeks.org/jdbc-drivers/) 使 Java 应用程序能够与数据库交互，我们可以从中获取或存储数据。JDBC 驱动程序类似于 ODBC 驱动程序。JDBC 类包含在 Java 包中，其功能包括：
*   连接到数据源，如数据库。
*   向数据库发送查询和更新语句。
