...
> 原文: [https://www.geeksforgeeks.org/buffer-position-methods-in-java-with-examples/](https://www.geeksforgeeks.org/buffer-position-methods-in-java-with-examples/)

[`java.nio.Buffer`](https://docs.oracle.com/javase/9/docs/api/java/nio/Buffer.html#) 类的 [`position(int newPosition)`](https://docs.oracle.com/javase/9/docs/api/java/nio/Buffer.html#position-int-) 方法用于设置该缓冲区的位置。如果标记被定义并且比新位置大，则它被丢弃。
...
**参数:** 该方法以 `newPosition` 为参数，为新位置值。它必须是非负的，并且不大于当前限制。
