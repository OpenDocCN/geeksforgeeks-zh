...
| 3。 | `JDBC` code needs to be written in a try catch block because it throws a checked exception (`SQLException`). | `Hibernate` manages the exception itself by marking the exception as unchecked. |
