...
> 原文: [https://www.geeksforgeeks.org/hashtable-containskey-method-in-java/](https://www.geeksforgeeks.org/hashtable-containskey-method-in-java/)

`java.util.Hashtable.containsKey()`方法用于检查哈希表中是否存在特定的键。它将键元素作为参数，如果表中存在该元素，则返回 `true`。
...
该方法只取一个参数 `key_element`，它指的是应该在哈希表中检查其存在的键。

## 返回值

如果键存在于哈希表中，该方法返回布尔真，否则返回假。

下面的程序用来说明 `java.util.Hashtable.containsKey()`方法：
