...
此方法接受一个 `Class<T>` 类型的参数 `annotationClass`，它是注释类型对应的类对象。

## 返回值

该方法返回指定注释类型的该元素的注释（如果存在于该元素中，否则为 `null`）。

## 异常

如果给定的注释类为空，该方法抛出 `NullPointerException`。
...
[https://docs.oracle.com/javase/8/docs/api/java/lang/reflect/Field.html#getAnnotation--](https://docs.oracle.com/javase/8/docs/api/java/lang/reflect/Field.html#getAnnotation--)