# Java 中的 IdentityHashMap `clone()` 方法

> 原文: [https://www.geeksforgeeks.org/identityhashmap-clone-method-in-java/](https://www.geeksforgeeks.org/identityhashmap-clone-method-in-java/)

`java.util.IdentityHashMap.clone()` 方法用于返回上述哈希映射的浅层副本。它只是创建了一个地图的副本。
...
以下程序用于说明 `java.util.IdentityHashMap.clone()` 方法：
