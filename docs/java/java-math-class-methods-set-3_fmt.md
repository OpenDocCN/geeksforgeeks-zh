...
> 原文：[https://www.geeksforgeeks.org/java-math-class-methods-set-3/](https://www.geeksforgeeks.org/java-math-class-methods-set-3/)
...
`java.math.ceil(double a)` 方法返回大于或等于传递的参数的最小可能值。返回值是一个数学整数。
...
`java.math.atan()` 方法返回方法参数值的反正切。返回的角度在 `-π/2` 到 `π/2` 的范围内。
...
`java.math.copySign()` 方法返回第一个浮点参数，但第二个参数有符号。
...
`java.math.cosh()` 方法返回传递的参数的双曲余弦值。
...
`java.math.decrementExact()` 方法将传递的参数值减 1。
...
`java.math.exp(double arg)` 方法返回欧拉数的双参数幂。
...
**在 `Math` 类中解释 `exp()`、`decrementExact()`、`cosh()` 方法的 Java 代码。**
...
`java.math.incrementExact()` 方法通过增加参数的值来返回参数。
...
`java.math.log10()` 方法返回传递的参数的 base10 对数值。
...
`java.math.pow(double b, double e)` 方法返回值为 `b<sup>e</sup>`。
