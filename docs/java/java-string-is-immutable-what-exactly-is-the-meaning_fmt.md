...
> 原文: [https://www.geeksforgeeks.org/java-string-is-immutable-what-exactly-is-the-meaning/](https://www.geeksforgeeks.org/java-string-is-immutable-what-exactly-is-the-meaning/)
...
随着应用程序的增长，`经常会占用大量内存，这甚至会导致冗余。`所以，为了让 Java 更高效，`JVM`留出了一个名为`"字符串常量池"`的特殊内存区域。
...
在`String`常量池中，`String`对象可能有一个或多个引用。`如果几个引用指向同一个String却不知道，如果其中一个引用修改了String值，那就不好了。这就是为什么String对象是不可变的。`

嗯，现在你可以说，`如果有人覆盖了String类的功能怎么办？`这就是`String`类被标记为`final`的原因，这样就没有人可以覆盖其方法的行为。