# 如何在安卓中使用Retrofit将数据发布到API？

> 原文：[https://www.geeksforgeeks.org/how-to-post-data-to-api-using-retrofit-in-android/](https://www.geeksforgeeks.org/how-to-post-data-to-api-using-retrofit-in-android/)

我们已经在安卓工作室看到了从我们的安卓应用中的API读取数据。为了从API读取数据，我们使用`GET`请求来读取我们的数据，该数据为`JSON`格式。在本文中，我们将了解如何在安卓工作室的安卓应用中向REST API添加数据。

## 我们将在本文中构建什么？

我们将构建一个简单的应用程序，其中我们将使用[Retrofit库](https://www.geeksforgeeks.org/introduction-retofit-2-android-set-1/)和`POST`请求向我们的REST API添加数据。我们将通过`EditText`字段添加数据，并通过响应代码验证我们的数据是否已添加到API中。当我们将数据添加到我们的API中时，我们将看到`201`是一个响应代码。`201`响应代码意味着数据已经添加到我们的API中。下面给出了一个示例视频，以了解我们将在本文中做什么。请注意，我们将使用`Java`语言来实现这个项目。
...
数据可以通过 [XML](https://www.geeksforgeeks.org/xml-basics/) 表单、 [JSON](https://www.geeksforgeeks.org/javascript-json/) 等不同格式添加到API中。大多数API都以`JSON`格式发布数据。因此，我们也将以`JSON`对象的形式将数据发布到我们的API。
...
导航到`App > AndroidManifest.xml`，并添加以下代码。
...
导航到`App > java > 你的应用的包名 > 右键点击它 > New > Java Class`并将其命名为`DataModal`并添加下面的代码。代码中添加了注释，以更详细地理解代码。
...
### 第六步：为我们的API调用创建一个接口类

导航到`App > java > 你的应用的包名 > 右键点击 > New > Java Class`选择为`Interface`并将文件命名为`RetrofitAPI`并添加下面的代码。代码中添加了注释，以更详细地理解代码。
