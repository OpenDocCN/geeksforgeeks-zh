# Java中的HashMap compute()方法示例

> 原文：[https://www.geeksforgeeks.org/hashmap-compute-method-in-java-with-examples/](https://www.geeksforgeeks.org/hashmap-compute-method-in-java-with-examples/)

`HashMap`类的`compute(key, BiFunction)`方法允许您更新哈希表中的值。`compute()`方法尝试计算指定键及其当前映射值的映射（如果找不到当前映射，则为`null`）。该方法用于**自动更新哈希表中给定键**的值。

*   如果在计算中传递的重新映射函数返回`null`值，映射将从映射中移除（或者如果最初不存在，则保持不存在）。
*   如果重新映射函数引发异常，将重新引发异常，并且当前映射保持不变。
*   **在计算过程中，重映射函数不应该能够修改这个映射。**

`compute()`方法可用于更新`HashMap`中的现有值。例如：
...
此方法的非并发实现应该覆盖此方法，如果在计算过程中检测到映射发生变化，则抛出`ConcurrentModificationException`。如果在计算过程中检测到映射发生变化，导致计算永远无法完成，那么并发实现应该覆盖这个方法，抛出`IllegalStateException`。
...
*   `NullPointerException`：如果键为`null`并且该映射不支持空键，或者`remappingFunction`为`null`。
*   `UnsupportedOperationException`：如果该地图不支持`put`操作。
