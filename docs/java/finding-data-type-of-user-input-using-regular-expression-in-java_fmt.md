...
> 原文：[https://www.geeksforgeeks.org/finding-data-type-of-user-input-using-regular-expression-in-java/](https://www.geeksforgeeks.org/finding-data-type-of-user-input-using-regular-expression-in-java/)

给定一个字符串，任务是使用 Java 中的[正则表达式](https://www.geeksforgeeks.org/regular-expressions-in-java/)找到它对应的数据类型。

我们可以将所有数据类型大致分为以下类型：

1.  **整数**：像 `byte`、`short`、`int`、`long` 这样的数字数据类型采用整数对象的形式。
2.  **Double**：像 `float` 和 `double` 这样的十进制数据类型采用 Double 对象的形式。
