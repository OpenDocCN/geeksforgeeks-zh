...
> 原文: [https://www.geeksforgeeks.org/implementing-rabin-karp-algorithm-using-rolling-hash-in-java/](https://www.geeksforgeeks.org/implementing-rabin-karp-algorithm-using-rolling-hash-in-java/)
...
借助 `滚动散列` 是可能的。 Rabin Karp 算法是朴素算法的优化算法之一，通过滚动字符串并搜索模式来执行搜索。
...
*   如果不匹配，通过跳过第一个字符并添加下一个字符的哈希值来移动到下一个字符，我们不计算字符串中下一个子字符串的哈希值，我们只滑动窗口跳过第一个字符并通过计算其哈希值添加窗口中的最后一个字符，即 `滚动哈希`。
...
4.  从 `temp` 变量中移除第一个元素，并在 `temp` 变量中添加下一个元素。`temp = temp - array[i] + array[i + pattern.length()]`
