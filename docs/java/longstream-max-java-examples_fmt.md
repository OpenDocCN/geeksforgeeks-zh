...
Java 8 中的 `java.util.stream.LongStream` 处理原语 `long`。它以一种新的方式帮助解决了求数组中最大值、求数组中最小值、求数组中所有元素的和、求数组中所有值的平均值等问题。`LongStream max()` 返回一个描述该流的**最大值**元素的 `OptionalLong`，如果该流为空，则返回一个空的 `OptionalLong`。
