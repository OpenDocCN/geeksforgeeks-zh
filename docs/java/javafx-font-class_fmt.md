...
字体类是 JavaFX 的一部分。字体类表示用于在屏幕上呈现文本的字体。字体的大小被描述为以磅为单位，实际测量值约为 1/72 英寸。字体类继承自 `Object` 类。

## 类的构造函数

1.  `Font(double s)`: 创建指定大小的字体对象。
...
| 方法 | 说明 |
| --- | --- |
| `Font(double s)` | 创建指定大小的字体对象。 |
| `Font(String f)` | 创建具有指定族名的字体对象。 |
| `Font(String f, double s)` | 创建具有指定族名和大小的字体对象。 |
| `Font(String f, FontPosture p, double s)` | 使用指定的族名、姿势和大小创建字体对象。 |
| `Font(String f, FontWeight w, double s)` | 使用指定的字体名称、字体粗细和大小创建字体对象。 |
| `Font(String f, FontWeight w, FontPosture p, double s)` | 使用指定的字体名称、字体粗细、字体姿态和大小创建字体对象。 |
| `getDefault()` | 返回默认字体。 |
| `getFamilies()` | 获取用户系统上安装的所有字体系列。 |
| `getFamily()` | 返回字体系列。 |
| `getFontNames()` | 获取用户系统上安装的所有字体的名称。 |
| `getFontNames(String f)` | 获取用户系统上安装的指定字体系列中所有字体的名称。 |
| `loadFont(InputStream in, double s)` | 从指定的输入流加载字体资源。 |
| `loadFont(String urlStr, double s)` | 从指定的网址加载字体资源。 |
| `getName()` | 完整的字体名称。 |
| `getSize()` | 此字体的磅值。 |
