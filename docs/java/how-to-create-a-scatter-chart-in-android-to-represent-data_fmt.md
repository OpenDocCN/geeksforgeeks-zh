...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-scatter-chart-in-android-to-represent-data/](https://www.geeksforgeeks.org/how-to-create-a-scatter-chart-in-android-to-represent-data/)
...
| `setZoom` | 它用于缩放 x 轴和 y 轴。 |
| `getLegend` | 此方法用于获取图表的图例。 |
| `getAxisLeft` | 返回左 y 轴对象。 |
| `getAxisRight` | 返回右 y 轴对象。 |
| `setDrawGridLines` | 此方法用于绘制网格线。 |
| `setScatterShape` | 设置此数据集应使用的散射形状。这将搜索可用的渲染器，并为数据集设置此渲染器。 |
| `setData` | 为图表设置新的数据对象。 |
| `invalidate` | 如果视图已启用，此方法用于使视图无效。 |
...
添加后，导航到 **build.gradle (Project)**，并在存储库部分添加下面的行。
