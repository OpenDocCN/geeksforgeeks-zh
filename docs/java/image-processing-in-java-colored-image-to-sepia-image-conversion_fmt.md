...
> 原文：[https://www.geeksforgeeks.org/image-processing-in-java-colored-image-to-sepia-image-conversion/](https://www.geeksforgeeks.org/image-processing-in-java-colored-image-to-sepia-image-conversion/)
...
1.  获取像素的 `RGB` 值。
2.  使用上述公式计算 `newRed`、`newGreen`、`newBlue`（取整数值）。
3.  按照以下条件设置像素的新 `RGB` 值：
    *   如果 `newRed` > 255，那么 `R` = 255，否则 `R` = `newRed`
    *   如果 `newGreen` > 255，那么 `G` = 255，否则 `G` = `newGreen`
    *   如果 `newBlue` > 255，则 `B` = 255，否则 `B` = `newBlue`
