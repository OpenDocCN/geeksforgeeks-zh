...
> 原文: [https://www.geeksforgeeks.org/how-to-add-custom-switch-using-iconswitch-library-in-android/](https://www.geeksforgeeks.org/how-to-add-custom-switch-using-iconswitch-library-in-android/)
...
1.  在 `build.gradle` 文件中添加支持库，并在依赖项部分添加依赖项。借助于此，我们可以直接在布局中使用小部件。
...
3.  现在在 `MainActivity.java` 文件中添加以下代码。这里我们在我们的开关上添加 `setCheckChangedListener`，如果开关改变，它将被调用。如果左开关打开，则执行情况 `LEFT`，如果右开关打开，则执行情况 `RIGHT`。
