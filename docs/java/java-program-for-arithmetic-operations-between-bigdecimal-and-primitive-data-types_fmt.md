# BigDecimal与原始数据类型算术运算的Java程序

> 原文：[https://www.geeksforgeeks.org/java-program-for-arithmetic-operations-between-bigdecimal-and-primitive-data-types/](https://www.geeksforgeeks.org/java-program-for-arithmetic-operations-between-bigdecimal-and-primitive-data-types/)

浮点数据类型（`float`和`double`）在财务计算中使用时不太准确。因此，Java提供了一个单独的类`BigDecimal`来执行操作，并避免计算中出现错误的可能性极小。[`BigDecimal`类](https://www.geeksforgeeks.org/bigdecimal-class-java/)提供对双精度数的运算，用于算术、小数位数处理、舍入、比较、格式转换和散列。它可以非常精确地处理非常大和非常小的浮点数，但会稍微增加时间复杂度。`BigDecimal`提供了许多可以实现的方法。在本文中，我们将讨论可以在`BigDecimal`上执行的基本算术运算，以及在`BigDecimal`和原始数据类型（如`int`、`short`、`long`等）之间执行相同的运算。
...
> 到目前为止，我们已经对两个`BigDecimal`对象执行了算术运算，现在我们将尝试对原始数据类型执行同样的操作。我们开始吧。
