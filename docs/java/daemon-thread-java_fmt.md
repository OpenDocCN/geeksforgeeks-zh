...
此方法将当前线程标记为守护线程或用户线程。例如，如果我有一个用户线程 `tU`，那么 `tU.setDaemon(true)` 将使它成为 Daemon 线程。另一方面，如果我有一个守护线程 `tD`，那么调用 `tD.setDaemon(false)` 会使它成为一个用户线程。
...
*   `IllegalThreadStateException`：如果仅此线程处于活动状态。
*   `SecurityException`：如果当前线程无法修改此线程。

#### 2. `boolean isDaemon()`

此方法用于检查当前线程是否是守护进程。如果线程是守护进程，则返回 `true`。否则，它返回 `false`。
...
如果此线程是守护程序线程，则此方法返回 `true`，否则返回 `false`。
...
class DaemonThread extends Thread
{
    ...
    public static void main(String[] args)
    {
        ...
        // Setting the threads to daemon threads
        t1.setDaemon(true);
        t3.setDaemon(true);
...
如果在启动线程后调用 `setDaemon()` 方法，它会抛出 `IllegalThreadStateException`。
