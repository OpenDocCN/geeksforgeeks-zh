...
> 原文: [https://www.geeksforgeeks.org/intermediate-methods-of-stream-in-java/](https://www.geeksforgeeks.org/intermediate-methods-of-stream-in-java/)
...
在这里，我们将讨论流应用编程接口的中间方法。这些方法都在 `java.util.stream` 中。中间操作符直到终端操作被调用时才执行，即直到实际需要处理结果时才执行。我们将讨论几个最重要和最常用的:

1.  [`filter(predicate)` 方法](https://www.geeksforgeeks.org/stream-filter-java-examples/)
2.  [`sorted()` 方法](https://www.geeksforgeeks.org/stream-sorted-in-java/)
3.  [`distinct()` 方法](https://www.geeksforgeeks.org/stream-distinct-java/)
4.  [`map()` 方法](https://www.geeksforgeeks.org/stream-map-java-examples/)

## 方法 1: [`filter(predicate)`](https://www.geeksforgeeks.org/stream-filter-java-examples/)

它返回一个新的流，该流由根据谓词(条件)调用它的流的元素组成。

> **注意:**
> *   中间函数返回一个流。
> *   在任何流上，你可以执行任意数量的中间操作，但终端操作应该是单一的并且最后写入。接下来是流。
> *   谓词是一个无干扰的无状态谓词，应用于每个元素以确定是否应包含它。
...
## 方法二: [`sorted()`](https://www.geeksforgeeks.org/stream-sorted-in-java/)
...
## 方法三: [`distinct()`](https://www.geeksforgeeks.org/stream-distinct-java/)
...
## 方法四: [`map()`](https://www.geeksforgeeks.org/stream-map-java-examples/)
