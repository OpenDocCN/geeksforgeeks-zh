...
> 原文: [https://www.geeksforgeeks.org/java-program-to-find-out-the-area-and-perimeter-of-rectangle-using-class-concept/](https://www.geeksforgeeks.org/java-program-to-find-out-the-area-and-perimeter-of-rectangle-using-class-concept/)
...
*   首先，我们将创建一个名为 `Rectangle` 的类，在其中我们将定义可变的 `length` 和 `width`。`Area()` 和 `Perimeter()` 方法计算矩形的面积和周长。
*   然后我们将创建另一个类，在其中我们将创建一个 `main` 方法。
*   在 `main` 方法中，`Rectangle` 的对象被创建，使用它我们可以调用赋值给 `Rectangle` 类的变量，在赋值之后，`Rectangle` 类的方法将被调用来计算矩形的面积和周长。
*   `Rectangle` 类的两个方法都将使用 `length` 和 `width` 的值，这些值是我们使用该类的对象从 `main` 方法传递过来的。结果会被打印出来。
