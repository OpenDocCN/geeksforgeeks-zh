...
> 原文: [https://www.geeksforgeeks.org/java-guava-ispoweroftwo-method-intmath-class/](https://www.geeksforgeeks.org/java-guava-ispoweroftwo-method-intmath-class/)

[番石榴的 `IntMath` 类](https://www.geeksforgeeks.org/intmath-class-guava-java/)的 `isPowerOfTwo()` 方法，用于检查一个数是否是二的幂。它接受要检查的数字作为参数，并根据该数字是否为 2 的幂返回布尔值 `true` 或 `false`。
...
如果 `x` 代表 2 的幂，则该方法返回 `true`，如果 `x` 不代表 2 的幂，则返回 `false`。
...
这与 `Integer.bitCount(x) == 1` 不同，因为 `Integer.bitCount(Integer.MIN_VALUE) = 1`，但 `Integer.MIN_VALUE` 不是 2 的幂。
