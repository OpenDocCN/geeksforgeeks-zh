...
> 原文：[https://www.geeksforgeeks.org/java-program-to-show-different-access-levels/](https://www.geeksforgeeks.org/java-program-to-show-different-access-levels/)
...
| 访问修饰符 | 描述 |
| --- | --- |
| `public` | 一个`public`修饰符的访问等级无处不在。它可以从类内、类外、包内甚至包外访问。 |
| `private` | `private`修饰符的访问级别只在类内，修饰符在。不能从该类外部访问它。 |
| `protected` | `protected`修饰符的访问级别是通过子类在包内（在中提到）和包外。如果不创建子类，则不能从包外部访问它。 |
| 默认 | 默认修饰符的访问级别仅在包内。不能从包外部访问它。如果不指定任何访问级别，它将被设置为默认值。 |
...
class GFG {
    ...
    public static void main(String[] args) {
        ...
        // Calling all the above methods in main() method body
        method1();
        method2();
        method3();
        method4();
