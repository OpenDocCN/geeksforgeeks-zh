...
**先决条件:** [Map 接口](https://www.geeksforgeeks.org/concurrentmap-interface-java/)
...
*   `ConcurrentHashMap` 的底层数据结构是[哈希表](https://www.geeksforgeeks.org/hashtable-in-java/)。
...
它实现了 `Serializable`、[`ConcurrentMap<K, V>`](https://www.geeksforgeeks.org/concurrentmap-interface-java/)、[`Map<K, V>`](https://www.geeksforgeeks.org/map-interface-java-examples/) 接口，扩展了 `AbstractMap<K, V>` 类。
...
### 1. `ConcurrentHashMap()`
创建一个新的空映射，默认初始容量(16)、负载系数(0.75)和 concurrencyLevel (16)。
...
使用指定的初始容量、默认负载系数(0.75)和并发级别(16)创建一个新的空映射。
...
使用指定的初始容量和加载因子以及默认的 concurrencyLevel (16)创建一个新的空映射。
...
创建一个新的映射，其映射与给定的映射相同。

```java
ConcurrentHashMap<K, V> CHM = new ConcurrentHashMap<>(Map<? extends K, ? extends V> m);
```
