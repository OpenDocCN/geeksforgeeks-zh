...
> 原文: [https://www.geeksforgeeks.org/javap-tool-in-java-with-examples/](https://www.geeksforgeeks.org/javap-tool-in-java-with-examples/)

## javap 工具

`javap`工具用于获取任何类或接口的信息。`javap`命令（也称为 **Java 反汇编器**）反汇编一个或多个类文件。其输出取决于所使用的**选项**（`-c`或`-verbose`分别表示字节码和字节码以及内部信息）。如果不使用任何选项，`javap`会打印出传递给它的类的包、受保护的和公共的字段和方法。
