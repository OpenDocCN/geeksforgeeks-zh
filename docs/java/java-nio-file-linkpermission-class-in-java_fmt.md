...
> 原文: [https://www.geeksforgeeks.org/java-nio-file-linkpermission-class-in-java/](https://www.geeksforgeeks.org/java-nio-file-linkpermission-class-in-java/)

`java.nio.file.LinkPermission` 类处理链接创建操作的权限。这些类允许权限如下:

| 权限名称 | 什么许可允许 | 授予此权限的风险 |
| --- | --- | --- |
| `hard` | 此权限允许将现有文件添加到目录中。此操作也称为创建硬链接。 | 攻击者可能有权访问所有文件，因为该权限允许链接到文件系统中的任何文件。 |
| `symbolic` | 该权限允许创建文件指针。此操作也称为创建软/符号链接。 | 攻击者可能有权访问所有文件，因为该权限允许链接到文件系统中的任何文件。 |
...
| 构造器 | 描述 |
| --- | --- |
| `LinkPermission(String name)` | 此构造函数用于创建具有指定名称的链接权限。 |
| `LinkPermission(String name, String actions)` | 此构造函数用于创建具有指定名称和操作的链接权限。 |

## 从 `java.security.BasicPermission` 类继承的方法

| 方法 | 描述 |
| --- | --- |
| `equals(Object obj)` | 此方法检查两个基本权限对象是否相等 |
| `getActions()` | 此方法以字符串格式返回操作 |
| `hashCode()` | 此方法返回此对象的哈希代码值 |
| `implies(Permission p)` | 此方法检查该对象是否暗示了给定的权限 |
