...
> 原文：[https://www.geeksforgeeks.org/abstractsequentiallist-retainall-method-in-java-with-example/](https://www.geeksforgeeks.org/abstractsequentiallist-retainall-method-in-java-with-example/)

`java.util.AbstractSequentialList`类的`retainAll()`方法用于从该列表中保留指定集合中包含的所有元素。
...
## 参数：
该方法将`Collection c`作为包含要从该列表中保留的元素的参数。
