...
`迭代器`是集合框架提供的一个接口，用于遍历集合以及对集合中的项目进行顺序访问。
...
我们将每个 `for` 循环中使用的“:”理解为“in”。所以循环读为“对于 `c` 中的每个元素 `e`”，这里的 `c` 是存储 `Element` 类型项的集合。

**注意：** 在使用 `lambda` 表达式的 Java 8 中，我们可以简单地用 `forEach` 替换 `for-each` 循环。
...
在 `for-each` 循环中，我们不能修改集合，它会抛出一个 `ConcurrentModificationException`。另一方面用迭代器我们可以修改集合。

修改集合只是意味着移除元素或更改存储在集合中的项的内容。这是因为 `for-each` 循环隐式地创建一个迭代器，但是它不向用户公开，因此我们不能修改集合中的项目。
...
如果您使用旧样式的 `C` 风格 `for` 循环进行迭代，那么我们可能会大大增加时间复杂度。这里 `l` 是 `List`，可以是 `ArrayList` / `LinkedList`，`n` 是 `List` 的大小。
...
这里，如果列表 `l` 是一个 `ArrayList`，那么我们可以在 `O(1)` 时间内访问它，因为它被分配了连续的内存块 (就像一个数组一样)，即随机访问是可能的。但是如果集合是 `LinkedList`，那么随机访问是不可能的，因为它没有被分配连续的内存块，所以为了访问一个元素，我们将不得不遍历链接列表，直到您到达所需的索引，因此在最坏的情况下访问一个元素所花费的时间将是 `O(n)`。
