...
> 原文：`https://www.geeksforgeeks.org/charbuffer-allocate-method-in-java-with-examples/`

`java.nio.CharBuffer` 类的 `allocate()` 方法用于在现有缓冲区旁边分配一个新的 `char buffer`。新缓冲区的位置将为零。它的极限将是它的容量。它的标记将是未定义的。它的每个元素都将被初始化为零。它将有一个后备数组，并且它的数组偏移量为零。
...
**参数：** 该方法以 `char` 中新缓冲区的 `capacity` 为参数。
**返回值：** 该方法返回 `新的字符缓冲区`。
