...
> 原文: [https://www.geeksforgeeks.org/modifier-isfinalmod-method-in-java-with-examples/](https://www.geeksforgeeks.org/modifier-isfinalmod-method-in-java-with-examples/)

`Modifier.isFinal()` 是 `java.lang.reflect.Modifier` 的方法，用于检查整数参数是否包含最终修饰符。如果此整数参数表示最终类型修饰符，则方法返回 `true`，否则返回 `false`。
...
如果 `mod` 包含最终修改器，则该方法返回 `true`，否则为 `false`。
