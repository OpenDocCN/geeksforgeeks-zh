...
> 原文：[https://www.geeksforgeeks.org/instanceof-operator-vs-isinstance-method-in-java/](https://www.geeksforgeeks.org/instanceof-operator-vs-isinstance-method-in-java/)

`instanceof` 运算符和 `isInstance()` 方法都用于检查对象的类别。但是主要的区别来自于我们想要动态检查对象的类。在这种情况下 `isInstance()` 方法会起作用。我们不可能通过 `instanceof` 运算符来做到这一点。

`instanceof` 运算符和 `isInstance()` 方法都返回布尔值。`isInstance()` 方法是 `java.lang.Class` 类的一个方法，而 `instanceof` 是一个运算符。
...
public class Test
{
    ...
    // instance of class whose name is passed as a string 'c'.
                      ...
                      throws ClassNotFoundException
    {
        ...
        /* print true as i is also instance of class
           Number as Integer class extends Number class*/
...
**相关文章：**
- [Java 中 `new` 运算符 vs `newInstance()` 方法](https://www.geeksforgeeks.org/new-operator-vs-newinstance-method-java/)
- [Java 中的反射](https://www.geeksforgeeks.org/reflection-in-java/)
