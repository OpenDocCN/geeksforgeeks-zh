...
> 原文链接：[https://www.geeksforgeeks.org/java-io-sequenceinputstream-java/](https://www.geeksforgeeks.org/java-io-sequenceinputstream-java/)
...
*   `read(byte[] b, int off, int len)`: 从该输入流中读取多达 `len` 字节的数据到一个数组中。
...
*   `available()`: 返回可以从当前底层输入流读取(或跳过)的字节数的估计值，而不会被当前底层输入流的方法的下一次调用阻塞。
...
注意：此程序不会在在线集成开发环境中运行，因为没有与之关联的文件。
