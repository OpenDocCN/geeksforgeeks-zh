...
1.  `AnchorPane()`: 创建新的锚定面板。
2.  `AnchorPane(Node… c)`: 创建具有指定节点的锚定面板。

## 常用方法

| 方法 | 说明 |
| --- | --- |
| `getBottomAnchor(Node c)` | 返回子级的底部锚点。 |
| `getLeftAnchor(Node c)` | 返回子级的左锚点。 |
| `getRightAnchor(Node c)` | 返回子级的右锚点。 |
| `getTopAnchor(Node c)` | 返回子级的顶部锚点。 |
| `setBottomAnchor(Node c, Double v)` | 设置孩子的底部锚点。 |
| `setLeftAnchor(Node c, Double v)` | 设置孩子的左锚点。 |
| `setRightAnchor(Node c, Double v)` | 设置孩子的右锚点。 |
| `setTopAnchor(Node c, Double v)` | 设置孩子的顶部锚点。 |

以下程序说明了 `AnchorPane` 类的使用:
