...
> 原文：[https://www.geeksforgeeks.org/java-io-input-output-in-java-with-examples/](https://www.geeksforgeeks.org/java-io-input-output-in-java-with-examples/)
...
2.  **[`System.out`](https://www.geeksforgeeks.org/java-lang-system-class-java/)**：这是**标准输出流**，用于在计算机屏幕等输出设备上产生程序的结果。

下面是我们用来输出语句的各种打印功能的列表：

### `print()`

[`print()`](https://www.geeksforgeeks.org/difference-between-print-and-println-in-java/) 方法在 Java 中用于在控制台上显示文本。此文本以 String 的形式作为参数传递给此方法。此方法将文本打印在控制台上，光标停留在控制台文本的末尾。下一次打印将从这里开始。
...
[`println()`](https://www.geeksforgeeks.org/difference-between-print-and-println-in-java/) 方法在 Java 中也用于在控制台上显示文本。它将文本打印在控制台上，光标移动到控制台下一行的开头。下一次打印将从下一行开始。
...
[`printf()`](https://www.geeksforgeeks.org/formatted-output-in-java/) 是所有方法中最简单的，因为这类似于 C 语言中的 `printf`。注意，`System.out.print()` 和 `System.out.println()` 采用单个参数，但 `printf()` 可能采用多个参数。这用于在 Java 中格式化输出。
...
3.  **[`System.err`](https://www.geeksforgeeks.org/java-lang-system-class-java/)**：这是**标准错误流**，用于在计算机屏幕或任何标准输出设备上输出程序可能抛出的所有错误数据。
...
*   **根据操作类型**，流可以分为两个主要类别：
    1.  **[输入流](https://www.geeksforgeeks.org/java-io-inputstream-class-in-java/)**：这些流用于从源数组或文件或任何外围设备读取必须作为输入的数据。例如，文件输入流、缓冲输入流、字节数组输入流等。
    ...
    2.  **[输出流](https://www.geeksforgeeks.org/java-io-outputstream-class-java/)**：这些流用于将数据作为输出写入数组或文件或任何输出外围设备。例如，文件输出流、缓冲输出流、字节数组输出流等。
...
*   **根据文件类型**，流可以分为两个主要类，这些主要类可以进一步分为其他类，这可以通过下图以及后面的解释看到。
...
这是用于按字节（8 位）处理数据。虽然它有很多类，但 `FileInputStream` 和 `FileOutputStream` 是最常用的。`FileInputStream` 用于从源读取，`FileOutputStream` 用于写入目标。以下是各种字节流类的列表：

| 流类 | 描述 |
| --- | --- |
| [`BufferedInputStream`](https://www.geeksforgeeks.org/java-io-bufferedinputstream-class-java/) | 它用于缓冲输入流。 |
...
| `PrintStream` | 这包含最常用的 `print()` 和 `println()` 方法。 |
| [`BufferedOutputStream`](https://www.geeksforgeeks.org/java-io-bufferedoutputstream-class-java/) | 这用于缓冲输出流。 |
| [`DataOutputStream`](https://www.geeksforgeeks.org/dataoutputstream-in-java/) | 这包含写入 Java 标准数据类型的方法。 |
| `FileOutputStream` | 这用于写入文件。 |
| `OutputStream` | 这是一个描述流输出的抽象类。 |
