...
> 原文：[https://www.geeksforgeeks.org/method-class-getdefaultvalue-method-in-java/](https://www.geeksforgeeks.org/method-class-getdefaultvalue-method-in-java/)

[`java.lang.reflect`](https://www.geeksforgeeks.org/reflection-in-java/) 的 `getDefaultValue()` 方法属于 `Method` 类。它返回由 `Method` 对象表示的[注释成员](https://www.geeksforgeeks.org/annotations-in-java/)的默认值。
当注释成员属于原语类型时，该方法返回该[包装类型](https://www.geeksforgeeks.org/wrapper-classes-java/)的实例。如果注释成员不包含任何默认值，或者如果 `Method` 对象没有注释类型的成员，则返回 `null`。
...
此方法返回此 `Method` 对象表示的注释成员的默认值。

## 异常

如果注释的类型为类，并且找不到默认类值的定义，该方法将抛出 `TypeNotPresentException`。

下面的程序说明了 `Method` 类的 `getDefaultValue()` 方法：

## 示例 1

下面的程序包含一个接口名称 `demo`，其中包含由接口方法表示的注释成员的一些默认值。程序的主方法为接口的每个方法创建 `Method` 对象，如果方法包含注释的默认值，则打印注释的默认值。
...
下面的程序包含一个接口名称 `demo` 和一个类名 `demo`，接口名称 `demo` 带有由接口方法表示的注释成员的一些默认值，类名 `demo` 没有注释的默认值。程序的 Main 方法为接口和类的每个方法创建 `Method` 对象，如果方法包含注释的默认值，则打印注释的默认值。
