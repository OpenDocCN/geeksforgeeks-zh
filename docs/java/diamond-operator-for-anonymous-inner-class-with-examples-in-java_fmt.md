...
## 钻石操作符
钻石操作符是在 Java 7 中作为一个新特性引入的。菱形运算符的主要目的是在创建对象时简化泛型的使用。它避免了程序中未经检查的警告，并使程序更可读。钻石运算符不能用于 JDK 7 中的匿名内部类。在 JDK 9 中，它可以与[匿名类](https://www.geeksforgeeks.org/anonymous-inner-class-java/)一起使用，以简化代码并提高可读性。在 JDK 7 之前，我们必须在表达式的两侧创建一个泛型类型的对象，如下所示:
