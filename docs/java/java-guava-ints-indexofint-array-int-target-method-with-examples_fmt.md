# Java 番石榴 | `Ints.indexOf(int[] array, int[] target)` 方法示例

> 原文: [https://www.geeksforgeeks.org/java-guava-ints-indexofint-array-int-target-method-with-examples/](https://www.geeksforgeeks.org/java-guava-ints-indexofint-array-int-target-method-with-examples/)

番石榴的 [`Ints` 类](https://www.geeksforgeeks.org/ints-class-guava-java/)的 `Ints.indexOf(int[] array, int[] target)` 方法接受两个参数 `array` 和 `target`。如果目标存在于数组中，该方法返回其第一次出现的开始位置。如果数组中不存在目标，则该方法返回 `-1`。
