...
1.  `MatteBorder(Icon tileIcon)`: 使用指定的平铺图标创建遮罩边框。
2.  `MatteBorder(Insets insets, Color matteColor)`: 用指定的插图和颜色创建哑光边框。
3.  `MatteBorder(Insets insets, Icon tileIcon)`: 使用指定的插图和平铺图标创建哑光边框。
4.  `MatteBorder(int top, int left, int bottom, int right, Color matteColor)`: 创建具有指定插图和颜色的哑光边框。
5.  `MatteBorder(int top, int left, int bottom, int right, Icon tileIcon)`: 用指定的插页和平铺图标创建一个哑光边框。

## 常用的方法有:

| 方法 | 说明 |
| --- | --- |
| `getBorderInsets()` | 返回边框的插页。 |
| `getBorderInsets(Component c, Insets insets)` | 用此边框的当前 insets 重新初始化 Insets 参数。 |
| `getMatteColor()` | 返回用于平铺边框的颜色，如果正在使用平铺图标，则返回 null。 |
...
我们将创建一个名为“框架”的框架 `f` ，并将创建一个充当容器的面板。我们将创建两个标签 `l1` 和 `l`。我们将使用 `setBorder()` 函数将两个框架的边框设置为无光泽边框。一个标签将有红色边框，另一个标签将有蓝色边框。我们将标签添加到面板，面板添加到框架。我们将使用 `setSize(400, 400)` 将框架的大小设置为 400, 400，并使用 `show()` 显示框架。
...
我们将创建一个标题为“框架”的框架 `f` ，并创建一个面板，作为一个容器。我们将创建两个标签 `l1` 和 `l`。我们将使用 `setBorder()` 函数将两个框架的边框设置为无光泽边框。一个标签将有一个图像图标作为边框，另一个标签将有另一个图像图标。我们将使用 `new ImageIcon()` 函数导入图像。我们将标签添加到面板，面板添加到框架。我们将使用 `setSize(400, 400)` 将框架的大小设置为 400, 400，并使用 `show()` 显示框架。
...
我们将创建一个标题为“框架”的框架 `f` ，并创建一个面板，该面板将充当容器。我们将创建两个标签 `l1` 和 `l`。我们将使用 `setBorder()` 函数将两个框架的边框设置为无光泽边框。一个标签将有一个图像图标作为边框，另一个标签将有另一个图像图标。我们将使用 `new ImageIcon()` 函数导入图像。我们将使用 `new Insets()` 函数指定边框的 inset 或宽度。我们将标签添加到面板，面板添加到框架。我们将使用 `setSize(400, 400)` 将框架的大小设置为 400, 400，并使用 `show()` 显示框架。
