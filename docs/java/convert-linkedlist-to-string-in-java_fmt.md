...
> 原文：[https://www.geeksforgeeks.org/convert-linkedlist-to-string-in-java/](https://www.geeksforgeeks.org/convert-linkedlist-to-string-in-java/)

一个[链表](https://www.geeksforgeeks.org/data-structures/linked-list/)是一个线性数据结构，其中元素不存储在连续的存储位置。

为了将链表转换成字符串，我们需要遍历链表，然后将链表的元素附加到字符串变量中。

我们可以使用`String`类、`StringBuilder`或`StringBuffer`将链表转换为字符串。

对于追加字符串，我们可以对`String`类使用`+`运算符，对`StringBuffer`和`StringBuilder`使用`append()`方法。
