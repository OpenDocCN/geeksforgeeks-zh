...
> 原文: [https://www.geeksforgeeks.org/localtime-plusnanos-method-in-java-with-examples/](https://www.geeksforgeeks.org/localtime-plusnanos-method-in-java-with-examples/)

`LocalTime` 类的 `plusNanos()` 方法，用于将指定数量的纳秒值添加到该 `LocalTime`，并将结果作为 `LocalTime` 对象返回。这一瞬间是不可改变的。计算大约在午夜进行。
...
该方法接受单个参数 `nanosecondsToAdd`，即需要添加的纳秒值，可以是负值。

## 返回值

这个方法返回一个基于这个时间加上纳秒的 `LocalTime`。
...
[https://docs.oracle.com/javase/10/docs/api/java/time/LocalTime.html#plusNanos(long)](https://docs.oracle.com/javase/10/docs/api/java/time/LocalTime.html#plusNanos(long))