...
*   在 Java 中，我们将使用`FTPClient`对象从 FTP 服务器读取文件。需要导入类`org.apache.commons.net.ftp.FTPClient`来创建`FTPClient`的实例。
*   为`FTPClient`对象调用`connect()`方法，以网址为参数，与建立连接。
*   使用`login()`方法发送凭证–用户名和密码。对于正确的凭据和成功的登录，返回`true`，允许用户访问服务器上的文件。
*   登录成功后，使用`listNames()`方法获取当前工作目录下所有文件的名称。返回的列表将是一个数组，可以迭代来读取名称。
*   由于登录是成功的，所以在读取文件名后需要`logout()`一次。最后调用`disconnect()`方法结束与连接的 FTP 服务器的关系。
