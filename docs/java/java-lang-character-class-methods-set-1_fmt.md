...
> 原文: [https://www.geeksforgeeks.org/java-lang-character-class-methods-set-1/](https://www.geeksforgeeks.org/java-lang-character-class-methods-set-1/)
...
1.  `charCount()`: `java.lang.Character.charCount()`方法使用 Unicode point 返回字符值的个数来表示参数字符值。一个 **Unicode 码位**用于 U+0000 到 U+10FFFF 范围内的字符值，以及作为 UTF-16 编码的编码单位的 16 位字符值。
...
2.  `charValue()`: `java.lang.Character.charValue()`方法返回已定义`Character`对象的原始字符值。
...
3.  `codePointAt()`: `java.lang.Character.codePointAt(char[] array, int position)`方法返回参数位置处的字符数组的 Unicode 码位。
...
4.  `codePointBefore()`: `java.lang.Character.codePointBefore(char[] array, int position)`方法返回出现在参数化位置之前的字符数组的 Unicode 码位。
...
**参数：**
- `array`：字符数组。
- `position`：你需要获取其Unicode码点值的字符之后的数组索引。
- `start`：字符数组的起始索引。
...
`codePointCount()`方法返回子字符数组的Unicode码点数。
...
`compareTo(Character argChar)`方法将给定的字符与此字符进行比较。
...
### 使用 `codePointBefore()`、`codePointCount()`、`compareTo()` 方法的 Java 代码示例
...
`equals()`方法将当前`char`对象与参数化的`char`对象进行比较。
...
`getNumericValue(char arg)`方法返回特定Unicode字符的`int`值。
...
**返回值：**
特定Unicode字符的`int`值。如果Unicode值不存在，则返回`-1`。

## getType()

`getType(char arg)`方法识别字符的通用类型。
- A – Z 值范围 u0041 至 u005A
- a -z 值范围 u0061 至 u007A
...
### 使用 `equals()`、`getNumericValue()`、`getType()` 方法的 Java 代码示例
