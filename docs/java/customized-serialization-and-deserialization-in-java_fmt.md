...
> 原文: [https://www.geeksforgeeks.org/customized-serialization-and-deserialization-in-java/](https://www.geeksforgeeks.org/customized-serialization-and-deserialization-in-java/)
...
*   `private void writeObject(ObjectOutputStream oos) throws Exception`：此方法将由 JVM（也称为回调方法）在序列化时自动执行。因此，要在序列化期间执行任何活动，必须仅在此方法中定义。
*   `private void readObject(ObjectInputStream ois) throws Exception`：此方法将由 JVM（也称为回调方法）在反序列化时自动执行。因此，要在反序列化期间执行任何活动，必须仅在此方法中定义。
