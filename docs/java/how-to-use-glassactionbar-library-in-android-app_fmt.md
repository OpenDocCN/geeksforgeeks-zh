...
> 原文：[https://www.geeksforgeeks.org/how-to-use-glassactionbar-library-in-android-app/](https://www.geeksforgeeks.org/how-to-use-glassactionbar-library-in-android-app/)
...
导航到 `Gradle Scripts > build.gradle (Module: app)`，在依赖项部分添加库，并同步项目。
...
导航至 `app > res > layout > activity_main.xml` 并参考以下代码。下面是 `activity_main.xml` 文件的代码。
...
导航至 `app > res > values > styles.xml` 并参考以下代码。下面是 `styles.xml` 文件的代码。
...
*   导航至 `app > res > values`
*   右键单击 `values` 并选择 `New > Values Resource File`
*   现在将出现一个对话框，然后将 `File name` 添加为 `themes`，并按确定。
...
导航到 `app > manifests > AndroidManifest.xml`，并在活动中为动作栏添加半透明主题。
