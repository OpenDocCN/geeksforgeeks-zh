# Java 中的 IntStream sequential()

> 原文:[https://www.geeksforgeeks.org/intstream-sequential-java/](https://www.geeksforgeeks.org/intstream-sequential-java/)

`IntStream sequential()`返回一个顺序的`IntStream`。它可能会自己返回，要么是因为流已经是顺序的，要么是因为基础流状态被修改为顺序的。`IntStream sequential()`是一个`中间操作`。中间操作在流实例上被调用，在它们完成处理后，它们给出一个流实例作为输出。
