...
> 原文: [https://www.geeksforgeeks.org/java-8-clock-getzone-method-with-examples/](https://www.geeksforgeeks.org/java-8-clock-getzone-method-with-examples/)

`java.time.Clock` 类是 Java 日期时间 API 的一部分。Java 日期时间 API 是从 Java 版本 8 中添加的。
`Clock` 类的 `getZone()` 方法返回用于创建 `Clock` 类的日期和时间的时区。每个 `Clock` 实例都需要一个时区来获取当前时刻，并将该实例转换为日期或时间。所以这个方法返回用于这个过程的时区。
