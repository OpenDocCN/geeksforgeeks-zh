...
> 原文：[https://www.geeksforgeeks.org/how-to-avoid-snackbar-overlap-floating-action-button-in-android/](https://www.geeksforgeeks.org/how-to-avoid-snackbar-overlap-floating-action-button-in-android/)
...
*   我们可以在下面的代码中观察到，在 `show_snackbar_button` 的点击监听器上，我们已经将 `AnchorView` 设置为父 `FloatingActionButton`。这样父 `FloatingActionButton` 将是可见的，即使在 `Snackbar` 弹出后，`Snackbar` 也将显示在 `FloatingActionButton` 的顶部。
