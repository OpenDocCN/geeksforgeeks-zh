...
> 原文：[https://www.geeksforgeeks.org/efficiently-reading-input-for-competitive-programming-using-java-8/](https://www.geeksforgeeks.org/efficiently-reading-input-for-competitive-programming-using-java-8/)
...
`java.io.BufferedReader` 类不提供任何读取原语数据输入的方法。`java.io.BufferedReader` 类从字符输入流中读取文本，缓冲字符，以便提供对字符序列的有效读取。尽管它抛出了一个名为 `IOException` 的检查异常。让我们看看如何处理这个异常并读取用户的输入。考虑如下自定义输入：
...
在前面的示例中，我们已经看到，在读取以空格分隔的数据时，我们首先将其存储在 `String` 数组中，然后遍历元素，然后使用 `java` 类型转换将其转换为所需的数据类型。一行代码让这成为可能怎么样？是的。Java 8 的流库提供了多种功能，使其变得简单和优化。考虑如下自定义输入：
...
// Using BufferedReader class via enhanced for loop
...
> **存入 `int` 数组：**
> 1.  创建传递的字符串数组流
> 2.  `br.readLine().split(" ")` – 根据分隔符值将输入字符串转换为字符串数组。(示例中为空格–“ ”)
> 3.  `mapToInt(Integer::parseInt)` – 使用合适的映射函数(示例中的 `Integer` 的 `parseInt()` 将字符串元素转换为所需的数据类型)
