# Comparator.comparingLong() 方法详解与示例

> 原文：[https://www.geeksforgeeks.org/comparator-comparinglong-method-in-java-with-examples/](https://www.geeksforgeeks.org/comparator-comparinglong-method-in-java-with-examples/)

Java 中 [`Comparator`](https://www.geeksforgeeks.org/comparator-interface-java/) 接口的 `comparingLong(ToLongFunction)` 方法接受一个函数作为参数，该函数从类型 `T` 中提取一个长整型排序关键字，并返回一个按该排序关键字进行比较的比较器。如果指定的函数也是可序列化的，则返回的比较器是可序列化的。
...
该方法接受单个参数 `keyExtractor`，该函数用于提取长整型排序键。
...
下面的程序说明了 `comparingLong(ToLongFunction)` 方法：
