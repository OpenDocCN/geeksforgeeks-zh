...
> 原文：[https://www.geeksforgeeks.org/java-program-to-extract-content-from-a-html-document/](https://www.geeksforgeeks.org/java-program-to-extract-content-from-a-html-document/)
...
1.  使用 [`FileReader`](https://www.geeksforgeeks.org/file-handling-java-using-filewriter-filereader/)
2.  使用 [`URL.openStream()`](https://www.geeksforgeeks.org/download-web-page-using-java/)

## 方法 1

该库称为 `FileReader`，它提供了读取任何文件的方法，而不考虑任何扩展名。将 HTML 行附加到 `StringBuilder` 的方法如下：

1.  使用 `FileReader` 从源文件夹读取文件，并进一步
2.  将每一行追加到 `StringBuilder` 中。
3.  当 HTML 文档中没有剩余内容时，使用功能 [`close()`](https://www.geeksforgeeks.org/java-io-bufferedreader-class-java/)。
4.  打印出字符串。
