...
> 原文: [https://www.geeksforgeeks.org/doublestream-findfirst-examples/](https://www.geeksforgeeks.org/doublestream-findfirst-examples/)

`DoubleStream findFirst()` 返回一个 `OptionalDouble`（可能包含也可能不包含非空值的容器对象），描述该流的 **first** 元素，如果该流为空，则返回一个空的 `OptionalDouble`。
...
函数返回一个描述该流第一个元素的 `OptionalDouble`，如果该流为空，则返回一个空的 `OptionalDouble`。

## 注意:

`findFirst()` 是一个 **端子-短路** 操作的 `Stream` 界面。此方法返回满足中间操作的任何第一个元素。
