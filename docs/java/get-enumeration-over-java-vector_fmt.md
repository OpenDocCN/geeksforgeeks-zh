...
> 原文: [https://www.geeksforgeeks.org/get-enumeration-over-java-vector/](https://www.geeksforgeeks.org/get-enumeration-over-java-vector/)
...
在 Java `Enumeration` 类中，默认情况下，所有列出的常量都是公共的、静态的和最终的。创建向量后，`enumeration()` 方法获取向量上的枚举。

> `public static <T> Enumeration<Object> enumeration(Collection<Object> c)`
>
> 是 `public class Collections extends Object` 的成员函数。

`enumeration()` 方法返回指定集合上的枚举对象，这里指定的集合是一个 `向量`。在通过 `向量` 获得枚举对象之后，我们将使用 [`hasMoreElements()`](https://www.geeksforgeeks.org/enumeration-hasmoreelements-method-in-java-with-examples/) 和 [`nextElement()`](https://www.geeksforgeeks.org/enumeration-nextelement-method-in-java-with-examples/) 方法通过 `向量` 进行枚举。
...
1.  [`hasMoreElements()`](https://www.geeksforgeeks.org/enumeration-hasmoreelements-method-in-java-with-examples/): 用来枚举元素是否多。
2.  [`nextElement()`](https://www.geeksforgeeks.org/enumeration-nextelement-method-in-java-with-examples/): 用于返回枚举中的下一个对象。
