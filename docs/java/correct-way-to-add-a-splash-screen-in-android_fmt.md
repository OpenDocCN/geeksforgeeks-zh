...
> 原文：[https://www.geeksforgeeks.org/correct-way-to-add-a-splash-screen-in-android/](https://www.geeksforgeeks.org/correct-way-to-add-a-splash-screen-in-android/)
...
要制作一个合适的闪屏，你必须首先建立一个背景，在屏幕上显示你的品牌标志。您必须为此在 `res/drawable` 文件夹中创建一个 XML 文件。
...
> **极客提示：**主题要继承`Theme.AppCompat.NoActionBar`。因为我们不希望在显示启动时状态栏可见。
...
现在我们已经做好了一切，现在唯一剩下的事情就是为我们的活动应用我们之前在步骤 2 中创建的主题，这很容易，只需弹出`AndroidManifest.xml`文件，按以下方式编辑。
