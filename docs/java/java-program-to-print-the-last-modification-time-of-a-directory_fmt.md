...
> 原文: [https://www.geeksforgeeks.org/java-program-to-print-the-last-modification-time-of-a-directory/](https://www.geeksforgeeks.org/java-program-to-print-the-last-modification-time-of-a-directory/)

可以使用 Java 显示任何包含多个文件的文件夹/目录的最后修改日期。使用 Java 中的 [`lastModified()`](https://www.geeksforgeeks.org/file-lastmodified-method-in-java-with-examples/) 方法，可以提取任意文件夹中任意文件的最后修改时间。
...
这里我们使用 [`Comparator`](https://www.geeksforgeeks.org/comparable-vs-comparator-in-java/) 将文件修改时间与同一文件夹中的其他文件进行比较。

**思路:**

1.  将所有文件放在一个名为 `File[]` 的数组中。
2.  使用 `Comparator` 以递增的顺序对最后修改日期/时间中基于数组的所有文件进行排序。
