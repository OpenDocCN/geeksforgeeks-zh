...
> 原文：[https://www.geeksforgeeks.org/customarrayadapter-in-android-with-example/](https://www.geeksforgeeks.org/customarrayadapter-in-android-with-example/)

在 Android 中，[`ArrayAdapter`](https://www.geeksforgeeks.org/arrayadapter-in-android-with-example/) 用于填充和控制 [`ListView`](https://www.geeksforgeeks.org/android-listview-in-java-with-example/) 和 [`Spinner`](https://www.geeksforgeeks.org/spinner-in-android-using-java-with-example/)。默认情况下，`ArrayAdapter` 提供仅包含单个信息或单个 [`TextView`](https://www.geeksforgeeks.org/textview-widget-in-android-using-java-with-examples/) 的列表项。为了有一个更复杂的布局，在一个列表项中包含多个信息，如图像、文本等，我们使用**自定义适配器**。像 Instagram、WhatsApp 等应用程序都在使用如此复杂的布局。

## 示例

在这个例子中，我们将为不同版本的 Android 操作系统制作一个应用程序，它有一个 [`ImageView`](https://www.geeksforgeeks.org/imageview-in-kotlin/) 和一个显示版本名称的 `TextView`。下面给出了一个示例 GIF，以了解我们将在本文中做什么。请注意，我们将使用 **Java** 语言来实现这个项目。
...
要在 Android Studio 创建新项目，请参考 [如何在 Android Studio 创建/启动新项目](https://www.geeksforgeeks.org/android-how-to-create-start-a-new-project-in-android-studio/)。注意选择 **Java** 作为编程语言。

**步骤 2：使用 `activity_main.xml` 文件**

在这一步中，我们将向我们的 `activity_main.xml` 文件添加一个 `ListView`，用于显示 `listItems` 的数据。转到 **app > res > layout > activity_main.xml** 并添加以下代码片段。
...
在这一步中，我们将为单个列表项视图创建一个布局文件。转到 **app > res > layout > 右键 > New > Layout Resource File** 并将其命名为 **list_item**。`list_item.xml` 包含一个 `ImageView` 和一个 `TextView`，用于填充 `ListView`。
...
我们将创建一个新的 Java 类，并将其命名为 **Item**。`Item.java` 包含三个私有成员变量 `androidVersionImage`、`androidVersionName` 和 `androidVersionNumber`。稍后，我们将创建这种 `Item` 类型的数组列表。进入 **app > java > 包名 > 右键 > New > Java Class**。
...
现在，我们将创建一个适配器类，作为用户界面组件和数据源之间的桥梁。进入 **app > java > 包名 > 右键创建一个新的 Java 类** 并命名为 `Adapter`。下面是给出的代码片段。
...
在 `MainActivity.java` 类中，我们创建了一个数组列表来存储图像和文本。这些图像被放置在可绘制文件夹 (**app > res > drawable**) 中。你可以用任何图像来代替它。我们获取 `ListView` 的引用，并在 `ListView` 上设置适配器。
