...
1.  抽象类是用 [`abstract`](https://www.geeksforgeeks.org/abstract-keyword-in-java/) 关键字声明的类。
...
5.  任何包含一个或多个抽象方法的类也必须用 `abstract` 关键字声明。
6.  抽象类不能有对象。也就是说，抽象类不能直接用 [`new`](https://www.geeksforgeeks.org/new-operator-java/) 操作符实例化。
