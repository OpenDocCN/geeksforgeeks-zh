...
> 原文：[https://www.geeksforgeeks.org/expandable-news-feed-in-android-with-example/](https://www.geeksforgeeks.org/expandable-news-feed-in-android-with-example/)
...
导航到 `应用 > java > 你的应用的包名 > 右键点击它 > 新建 > Java 类` 并将其命名为 `NewsModal` 并添加下面的代码。
...
同样，创建另一个 Java 类，并将其命名为 `CategoriesModal`，并向其中添加以下代码。
...
创建另一个 Java 类，并将其命名为 `NewsRVAdapter`，并向其中添加以下代码。代码中添加了注释，以便更详细地了解。
