...
> 原文：[https://www.geeksforgeeks.org/complete-guide-on-how-to-build-a-video-player-in-android/](https://www.geeksforgeeks.org/complete-guide-on-how-to-build-a-video-player-in-android/)
...
在安卓中插入视频，我们放入`raw`文件夹。`raw`文件夹出现在
中
...
2.  像播放、倒带、前进这样的图标只会在我们触摸 `VideoView` 的时候出现，它们只会出现 3 秒钟，然后就会消失。它是由谷歌提供的，这是它的默认行为。
3.  来到后端部分，即 Java 编码，我们通过以下方式获取媒体控件：
...
4.  然后，在 `ArrayList` 中添加原始文件夹的视频，并调用一个名为 `setVideo()` 的方法，给出第一个视频的参数。
    ...
    ```
5.  现在在定义 `setVideo()` 时，我们需要一个 `Uri` 对象，以便传递给一个名为 `setVideoURI()` 的方法。因此，
    ```
...
*   **播放第一段视频：**
    ...
    第一首歌《褪色》
*   **Dialog box after the first video:**
    [![After completion of first song,dialog box is getting generated](img/a261b8402788ace13f0af81680906153.png)](https://media.geeksforgeeks.org/wp-content/uploads/20191210103449/Screenshot_20191210-102206.png)
    After the first song is finished, the dialog box is generating
*   **Playing the second video:**
    [![When we click on "NEXT",then the second video starts running](img/50e07f1dd912edea79245dc79a626b0f.png)](https://media.geeksforgeeks.org/wp-content/uploads/20191210103629/Screenshot_20191210-102248.png)
