...
> 原文：[https://www.geeksforgeeks.org/compact-strings-in-java-9-with-examples/](https://www.geeksforgeeks.org/compact-strings-in-java-9-with-examples/)

**先决条件**：[字符串](https://www.geeksforgeeks.org/strings-in-java/)
...
*   [Java 9](https://www.geeksforgeeks.org/java-9-features-with-examples/) 引入了紧凑字符串的概念。紧凑字符串的主要目的是，每当我们创建一个字符串对象，并且对象内部的字符可以用 1 字节表示，这只是 `LATIN-1` 表示，那么在内部 `java` 将创建一个 `byte[]`。在其他情况下，如果任何字符需要 1 个以上的字节来表示，那么每个字符使用 2 个字节来存储，即 `UTF-16` 表示。
