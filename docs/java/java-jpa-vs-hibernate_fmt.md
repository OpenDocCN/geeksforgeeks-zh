...
| It is not an implementation. It is just a Java specification. | `Hibernate` 是一个 `JPA` 的实现。因此，`Hibernate` 遵循了**由 JPA 给出的**通用标准。 |
...
| 为持久化单元与实体管理器工厂互联，它使用 `EntityManagerFactory` 接口。因此，它给出了一个实体管理器。 | 创建一个会话实例并使用 `session factory` 接口。 |
