# Java 中的 `BigInteger.floatValue()` 方法

> 原文：[https://www.geeksforgeeks.org/biginteger-floatvalue-method-in-java/](https://www.geeksforgeeks.org/biginteger-floatvalue-method-in-java/)

`java.math.BigInteger.floatValue()` 将这个 `BigInteger` 转换成一个 `float` 值。如果这个函数返回的值对于一个数值来说太大而不能表示为浮点数，那么它将被转换为 `Float.NEGATIVE_INFINITY` 或 `Float.POSITIVE_INFINITY`，视情况而定。这种转换可能会丢失关于 `BigInteger` 值精度的信息。
...
### 例 2：当返回值太大，无法用一个量级表示为一个 `float` 时
...
[https://docs.oracle.com/javase/7/docs/api/java/math/BigInteger.html#floatValue()](https://docs.oracle.com/javase/7/docs/api/java/math/BigInteger.html#floatValue())