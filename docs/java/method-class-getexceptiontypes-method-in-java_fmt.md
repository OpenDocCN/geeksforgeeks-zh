...
> 原文: [https://www.geeksforgeeks.org/method-class-getexceptiontypes-method-in-java/](https://www.geeksforgeeks.org/method-class-getexceptiontypes-method-in-java/)

`Method`类的`getExceptionTypes()`方法返回一个由`Method`对象声明为抛出的异常类型的`Class`对象数组，以处理方法内部的异常。使用`throws`子句的方法处理的所有异常都将使用此方法作为`Class`对象的数组返回。如果应用此方法的方法在其`throws`子句中未声明任何异常，则此方法返回长度为 0 的数组。
...
这个方法返回一个异常类的数组，这个异常类是由`This method`对象使用`thrown`子句声明的。
