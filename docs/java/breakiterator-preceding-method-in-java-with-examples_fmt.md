# Java 中 BreakIterator 的 preceding() 方法示例

> 原文：[https://www.geeksforgeeks.org/breakiterator-preceding-method-in-java-with-examples/](https://www.geeksforgeeks.org/breakiterator-preceding-method-in-java-with-examples/)

`java.text.BreakIterator` 类的 `preceding()` 方法用于获取指定字符偏移量边界之前的最后一个边界的字符索引。它提供边界的第一个字符的偏移量，该偏移量在 `BreakIterator` 指向的当前边界之后或之前。
...
取字符的 `offset`，必须为其找到上一个边界。

## 返回值

该方法提供指定字符偏移量之前的边界的 `索引`。

## 异常

如果指定的索引小于第一个文本边界或大于最后一个文本边界，此方法将抛出 `IllegalArgumentException`。

以下是示例，说明 `preceding()` 方法：
...
[https://docs.oracle.com/javase/9/docs/api/java/text/BreakIterator.html#preceding-int-](https://docs.oracle.com/javase/9/docs/api/java/text/BreakIterator.html#preceding-int-)