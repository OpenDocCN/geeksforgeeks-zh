...
> 原文: [https://www.geeksforgeeks.org/java-program-delete-certain-text-file/](https://www.geeksforgeeks.org/java-program-delete-certain-text-file/)
...
一个更好的解决方案是使用 [HashSet](https://www.geeksforgeeks.org/hashset-in-java/) 来存储 `delete.txt` 的每一行，然后在循环 `input.txt` 的同时，检查它是否在 `HashSet` 中。如果不存在，将该行写入 `output.txt`。
