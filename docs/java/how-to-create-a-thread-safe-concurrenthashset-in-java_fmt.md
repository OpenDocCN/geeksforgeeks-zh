...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-thread-safe-concurrenthashset-in-java/](https://www.geeksforgeeks.org/how-to-create-a-thread-safe-concurrenthashset-in-java/)

创建线程安全的 `ConcurrentHashSet` 在 JDK 8 之前是不可能的，因为 `java.util.concurrent` 包没有一个名为 `ConcurrentHashSet` 的类。但是从 JDK 8 开始，新添加的 `keySet()`（默认）和 `newKeySet()` 方法在 Java 中创建了一个 `ConcurrentHashSet`，该类由 `ConcurrentHashMap` 支持。

`ConcurrentHashSet` 可以使用 `ConcurrentHashMap` 来创建，因为它允许我们使用 `keySet()`（默认值）和 `newKeySet()` 方法来返回 `Set`，这恰好是一个合适的 `Set`。这使我们能够访问必要的功能，如 `contains()`、`remove()` 等。这些方法只能在 `ConcurrentHashMap` 类中使用，不能在 `ConcurrentMap` 接口中使用，所以我们需要使用 `ConcurrentHashMap` 引用变量来保存引用，或者我们可以使用类型转换来转换存储在 `ConcurrentHashMap` 变量中的 `ConcurrentHashMap` 对象。
