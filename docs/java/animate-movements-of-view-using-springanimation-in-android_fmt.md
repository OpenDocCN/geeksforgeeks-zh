...
> 原文：[https://www.geeksforgeeks.org/animate-movements-of-view-using-springanimation-in-android/](https://www.geeksforgeeks.org/animate-movements-of-view-using-springanimation-in-android/)

在安卓系统中，我们可以给物体的运动添加动画。我们可以为这些动作添加一些很酷的动画，而不仅仅是更新一个物体从一点到另一点的位置。安卓提供了很多方法，可以通过[动画](https://www.geeksforgeeks.org/animation-in-android-with-example/)改变视图对象在屏幕上的位置。但是，这里我们要看一下`SpringAnimation`。`SpringAnimation`是基于安卓物理的动画应用编程接口的一部分。安卓弹簧动画允许你像弹簧一样动画视图的运动。动画是基于潮湿，僵硬，弹性等属性。一旦弹簧动画开始，在每一帧上，弹簧力将更新动画的值和速度。动画将继续运行，直到弹簧力达到平衡。
...
1.  调用`getSpring()`方法检索`SpringForce`对象来添加属性。
2.  调用`setDampingRatio()`/`setStiffness()`方法，传递要添加到弹簧中的值。

为了开始动画，我们或者调用`start()`或者`animateToFinalPosition(float finalPosition)`。此外，`updateListener`和`endListener`是各自的侦听器，用于在不再需要时侦听更新和删除侦听器。
...
要向可绘制文件夹添加图像，请参考[如何在安卓工作室向可绘制文件夹添加图像](https://www.geeksforgeeks.org/how-to-add-image-to-drawable-folder-in-android-studio/)。只需将图像复制并粘贴到可绘制文件夹中，即可将图像添加到可绘制文件夹中。
