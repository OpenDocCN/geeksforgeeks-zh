...
> 原文: [https://www.geeksforgeeks.org/java-program-to-implement-hash-tables-with-double-hashing/](https://www.geeksforgeeks.org/java-program-to-implement-hash-tables-with-double-hashing/)
...
`h2(key) = PM-(key%PM)*PM` // where PM is prime number
...
public HashTable(int ts)
{
    // Initially, initializing the parameters to some values
    size = 0;
    HASH_TABLE_SIZE = ts;
    table = new ValueEntry[HASH_TABLE_SIZE];
...
public int getPrime()
{
    // Iterating over hashtable using nested for loops in reverse order
    for (int i = HASH_TABLE_SIZE - 1; i >= 1; i--) {
...
```java
// Method 6
// In this function, 'myhash' function for double hashing after linear probing.
// A quadratic probing is used in which two 'myhash' functions are used as it is double chaining
private int myhash2(String y)
{
    int myhashVal2 = y.hashCode();
    myhashVal2 %= HASH_TABLE_SIZE;
...
public class GFG {
    ...
    public static void main(String[] args)
    {
        ...
        // Step 1: Creating an object of hashtable of custom size 100 which signifies table can hold 100 key-value pairs
        HashTable ht = new HashTable(100);
        ...
        // Calling print hash table function using object we call it with object.function_name
        ht.printHashTable();

        // Primarily goal of the program
        // Step 4: Removing elements with using key values using the remove() method
        ht.remove("prime");
        ht.printHashTable();
    }
}
