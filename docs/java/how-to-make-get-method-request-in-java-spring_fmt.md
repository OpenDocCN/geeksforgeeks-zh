...
> 原文: [https://www.geeksforgeeks.org/how-to-make-get-method-request-in-java-spring/](https://www.geeksforgeeks.org/how-to-make-get-method-request-in-java-spring/)
...
`GetMapping()` 注释主要用在 spring boot 应用中，通过匹配来自客户端的传入请求头来处理来自客户端的传入请求。
...
**第三步:** 转到 `src -> 主 -> java -> com.gfg.spring.boot.app` 创建一个名称为 `Controller` 的 Java 类，并添加注释。现在创建一个 GET 应用编程接口，如下所示:
...
**步骤 4:** 该应用程序现在可以运行了。运行 `SpringBootApp` 类，等待 Tomcat 服务器启动。

![](img/093e5180d109d63d4fd76cdf5ca54c15.png)

> **注意:** Tomcat 服务器的默认端口是 8080，可以在 `application.properties` 文件中更改。

**第五步:** 现在进入浏览器，输入网址 `localhost:8080`。观察输出，现在对 `localhost:8080/get/check` 进行同样的操作。
