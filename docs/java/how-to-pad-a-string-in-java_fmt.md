...
此方法用于使用给定的区域设置、指定的格式字符串和参数返回格式化的字符串。
...
## 2. 使用 Apache Common Lang

Apache Commons Lang 包提供了 `StringUtils` 类，其中包含 `leftPad()`, `center()` 和 `rightPad()` 方法，可以轻松地对字符串进行左填充、中心填充和右填充。
...
*   获取填充所在的字符串。
*   对于左填充，使用 `StringUtils.leftPad()` 方法的语法是:
...
public class GFG {
    ...
    public static String leftPadding(String input, char ch, int L) {
        // Left pad the string
        String result = StringUtils.leftPad(str, L, ch);
    ...
    public static String centerPadding(String input, char ch, int L) {
        ...
        String result = StringUtils.center(str, L, ch);
