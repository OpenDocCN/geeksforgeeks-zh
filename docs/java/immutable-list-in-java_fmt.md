...
*   顾名思义，不可变列表是一种不可变的列表类型。表示列表的内容在声明后是固定的或不变的，即为只读。
*   如果试图添加、删除和更新列表中的元素，将引发`UnsupportedOperationException`。
*   不变列表也不允许空元素。
*   如果试图创建具有空元素的不可替换列表，将引发`NullPointerException`。如果试图在列表中添加空元素，将引发`UnsupportedOperationException`。

## 不变列表的优势

*   它们是线程安全的。
*   它们的记忆效率很高。
*   由于它们是不可变的，因此它们可以毫无问题地被传递给第三方库。
...
在 Java 中，使用`of()`和集合、映射或列表来创建不可变列表。
...
在番石榴中，`ImmutableList` 类提供了一个函数 `Builder()`。通过该功能，可以创建一个新的不可替换列表，或者从现有列表中创建一个不可替换列表，或者两者都创建。
...
`Collections.unmodifiableList` 在相同的现有列表周围创建一个包装，这样包装就不能用于修改它。但是，我们仍然可以更改原始列表。
