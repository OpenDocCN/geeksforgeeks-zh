# Java中Matcher regionStart()方法，带示例

> 原文：[https://www.geeksforgeeks.org/matcher-regionstart-method-in-java-with-examples/](https://www.geeksforgeeks.org/matcher-regionstart-method-in-java-with-examples/)

`Matcher`类的`regionStart()`方法用于获取当前匹配器中模式要匹配区域的起始索引。此方法返回一个整数值，它是此匹配器区域的开始索引。
