# Java 中的比较器 reversed() 方法，带示例

> 原文：[https://www.geeksforgeeks.org/comparator-reversed-method-in-java-with-examples/](https://www.geeksforgeeks.org/comparator-reversed-method-in-java-with-examples/)

Java 中 [`Comparator`](https://www.geeksforgeeks.org/comparator-interface-java/) 接口的 `reversed()` 方法返回一个比较器，该比较器强制该比较器的逆序。如果您使用数组的排序方法，并在应用相反的方法后传递此比较器，那么它将按相反的顺序对数组进行排序。
