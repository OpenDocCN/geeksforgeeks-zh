...
> 原文: [https://www.geeksforgeeks.org/java-awt-mouseinfo-and-pointerinfo/](https://www.geeksforgeeks.org/java-awt-mouseinfo-and-pointerinfo/)

`MouseInfo` 和 `PointerInfo` 是 Java AWT 的一部分。`MouseInfo` 提供了有关指针位置和鼠标上按钮数量的信息。`PointerInfo` 提供信息返回指针和图形设备的位置信息。

## 鼠标信息的方法

| 方法 | 说明 |
| --- | --- |
| `getNumberOfButtons()` | 返回鼠标上的按钮数量。 |
| `getPointerInfo()` | 返回一个 `PointerInfo` 对象，该对象表示指针的当前位置和图形设备。 |

## 指针法

| 方法 | 说明 |
| --- | --- |
| `getDevice()` | 返回创建对象时鼠标所在的图形设备。 |
