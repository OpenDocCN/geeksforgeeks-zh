...
> 原文: [https://www.geeksforgeeks.org/how-to-validate-a-username-using-regular-expressions-in-java/](https://www.geeksforgeeks.org/how-to-validate-a-username-using-regular-expressions-in-java/)

给定一个代表用户名的字符串 `str`，任务是在[正则表达式](https://www.geeksforgeeks.org/write-regular-expressions/)的帮助下验证这个用户名。

如果满足以下所有约束，则用户名被视为有效:

*   用户名由 **6** 到 **30** 个字符组成。如果用户名少于 6 个字符或多于 30 个字符，则该用户名无效。
...
**输出:** 假
**解释:** 给定的用户名无效，因为它包含无效字符“？”。

## 方法

想法是使用[正则表达式](https://www.geeksforgeeks.org/regular-expressions-in-java/)来验证给定的用户名是否有效。可以按照以下步骤计算答案:

1.  获取字符串。
2.  构建一个正则表达式来验证给定的字符串。根据条件，正则表达式可以按如下方式构建：
...
*   `\\w{5,29}` 表示检查以确保剩余的项是单词字符（包括下划线），直到它到达末尾并用 `$` 表示。
*   `{5,29}` 表示给我们的 6-30 个字符的约束减去预定义的第一个字符。
3.  将字符串与正则表达式匹配。在 Java 中，这可以使用 `Pattern.matcher()` 来完成。
