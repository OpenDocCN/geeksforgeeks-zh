...
`Matcher`类的`hasAnchoringBounds()`方法用于检查该匹配器是否有锚定边界。通过锚定边界，这意味着匹配者将匹配像`^`这样的锚，如果锚定边界设置为真，匹配者将获得`$ 1`。此方法返回一个表示相同内容的布尔值。
...
[https://docs.oracle.com/javase/9/docs/api/java/util/regex/Matcher.html#hasAnchoringBounds--](https://docs.oracle.com/javase/9/docs/api/java/util/regex/Matcher.html#hasAnchoringBounds--)