...
`Java.lang.Cloneable` 接口是一个[标记接口](https://www.geeksforgeeks.org/marker-interface-java/)。它是在 JDK 1.0 中引入的。`Object` 类中有一个方法 [`clone()`](https://www.geeksforgeeks.org/clone-method-in-java-2/)。`Cloneable` 接口由一个类实现，以使 [`Object.clone()`](https://www.geeksforgeeks.org/clone-method-in-java-2/) 方法有效，从而进行逐字段复制。该接口允许实现类克隆其对象，而不是使用 `new` 运算符。
...
**示例 1：** 下面的程序解释说，如果你试图克隆一个没有实现 `Cloneable` 接口的对象，它将会抛出 `CloneNotSupportedException`，你可能想要处理这个异常。

## 示例 1：未实现 Cloneable 接口
...
**示例 2：** 下面的代码解释了如何正确使用 `Cloneable` 接口来使 `Object.clone()` 方法合法。实现此接口的类应该重写 `Object.clone()` 方法（`protected`），以便可以调用它。

## 示例 2：正确实现 Cloneable 接口
...
**注意：** 此接口不包含 `clone` 方法。因此，不可能仅仅因为一个对象实现了这个接口就克隆它。即使反射性地调用 `clone` 方法，也不能保证它会成功。
