...
> 原文：[https://www.geeksforgeks.org/dateformat-getavailablelocales-method-in-java-with-examples/](https://www.geeksforgeeks.org/dateformat-getavailablelocales-method-in-java-with-examples/)

Java 中 `DateFormat` 类的 `getAvailableLocales()` 方法用于返回该类的 `getInstance` 方法的本地化实例中可用的所有区域设置的数组。
...
该方法返回本地化实例可用的所有区域设置的数组。

以下程序说明了 `DateFormat` 类的 `getAvailableLocales()` 方法的工作：
...
**参考:** [`https://docs.oracle.com/javase/7/docs/api/java/text/DateFormat.html#getAvailableLocales()`](https://docs.oracle.com/javase/7/docs/api/java/text/DateFormat.html#getAvailableLocales())