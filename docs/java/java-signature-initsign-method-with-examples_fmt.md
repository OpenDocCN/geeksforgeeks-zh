...
> 原文: [https://www.geeksforgeeks.org/java-signature-initsign-method-with-examples/](https://www.geeksforgeeks.org/java-signature-initsign-method-with-examples/)

## `initSign(PrivateKey)`

`java.security.Signature` 类的 `initSign()` 方法用于初始化这个对象进行签名。如果使用不同的参数再次调用此方法，它会否定此调用的效果。
...
**参数:** 该方法将身份的 `privateKey` 作为将要生成签名的参数。
...
*   `privateKey` – 将要生成签名的身份的私钥。
*   `random` – 该签名的随机来源。

**异常:** 如果密钥无效，此方法抛出 `InvalidKeyException`。

下面是举例说明 `initSign(PrivateKey, SecureRandom)` 方法的例子:
