...
> 原文: [https://www.geeksforgeeks.org/adding-image-in-existing-pdf-file-using-java/](https://www.geeksforgeeks.org/adding-image-in-existing-pdf-file-using-java/)
...
3.  调用 `drawXObject()` 对象，并指定具有宽度和高度的坐标，将图像绘制到 PDF 文件上。
4.  关闭流，保存文档对象，并关闭文档。

## 实现

考虑输入图像样本来说明程序的工作，其中为了说明之前的工作 PDF 文档，如下所示:
