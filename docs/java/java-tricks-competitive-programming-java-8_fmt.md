...
> 原文：[https://www.geeksforgeeks.org/java-tricks-competitive-programming-java-8/](https://www.geeksforgeeks.org/java-tricks-competitive-programming-java-8/)
...
关于 java 中的快速输入/输出，请参考[这里](https://www.geeksforgeeks.org/fast-io-for-competitive-programming-java/)。

## 5) 对于字符串操作

使用 [`StringBuffer`](https://www.geeksforgeeks.org/stringbuffer-class-in-java/) 进行字符串操作，因为 java 中的 [`String`](https://www.geeksforgeeks.org/g-fact-27-string-vs-stringbuilder-vs-stringbuffer/) 是不可变的。在此参考。
...
Java 在 [`BigInteger`](https://www.geeksforgeeks.org/biginteger-class-in-java/) 类中内置了 GCD 方法。它返回一个 `BigInteger`，其值是 `abs(this)` 和 `abs(val)` 的最大公约数。如果该值 == 0 & 则返回 0。
...
Java 在 [`BigInteger`](https://www.geeksforgeeks.org/biginteger-class-in-java/) 类中有一个内置的 `isProbablePrime()` 方法。如果这个大整数可能是质数（有一定把握），它返回真；如果它肯定是复合的，它返回假。

```java
BigInteger.valueOf(1235).isProbablePrime(1)
```

## 10) 知道一个数是否是 2 的幂的高效技巧
...
1.  [`Arrays.binarySearch()`](https://www.geeksforgeeks.org/arrays-binarysearch-java-examples-set-1/)（[SET 1](https://www.geeksforgeeks.org/arrays-binarysearch-java-examples-set-1/) | [SET 2](https://www.geeksforgeeks.org/arrays-binarysearch-in-java-with-examples-set-2-search-in-subarray/)）用于在排序的数组上应用二分搜索法。
2.  [`Collections.binarySearch()`](https://www.geeksforgeeks.org/collections-binarysearch-java-examples/) 用于对基于比较器的集合应用二分搜索法。

## 13) 复制算法

1.  [`Arrays.copyOf()` 和 `copyOfRange()`](https://www.geeksforgeeks.org/array-copy-in-java/) 复制指定的数组。
...
## 16) 使用包装类函数获取一个数的基数转换

有时候你需要一个数的基数转换。为此，您可以使用包装类。
...
## 17) NullPointerException（为什么？）

[这里](https://www.geeksforgeeks.org/null-pointer-exception-in-java/)参考，[这里](https://www.geeksforgeeks.org/how-to-fix-nullpointerexception-in-java/)参考避开。
