# Java 中的 `DelayQueue` `take()` 方法示例

> 原文：[https://www.geeksforgeeks.org/delayqueue-take-method-in-java-with-examples/](https://www.geeksforgeeks.org/delayqueue-take-method-in-java-with-examples/)

[`DelayQueue`](https://www.geeksforgeeks.org/delayqueue-class-in-java-with-example/)的 `take()` 方法用于检索延迟队列的头部并将其移除。因此，延迟队列的大小减小了。如果此队列中有延迟过期的元素，此函数将等待。
...
**参数：** 该方法不接受参数。
**返回值：** 该函数返回延迟队列的头部。
**异常：** 如果在等待时被中断，此方法会抛出 `InterruptedException`。
