...
输入流用于从 Java 应用程序的源中读取数据。数据可以是任何东西，文件、阵列、外围设备或套接字。在 Java 中，`java.io.InputStream`类是所有 Java IO 输入流的基类。
...
2.  `mark(int arg)`：`mark(int arg)`方法用于标记输入流的当前位置。它将读取设置为限制，即在标记位置无效之前可以读取的最大字节数。
3.  `reset()`：`reset()`方法由`mark()`方法调用。它将输入流的位置更改回标记的位置。
4.  `close()`：`close()`方法用于关闭输入流，并将与该流相关联的系统资源释放给垃圾收集器。
5.  `read(byte[] arg)`：`read(byte[] arg)`方法用于从输入流读取`arg.length`的字节数到缓冲阵列`arg`。由`read()`方法读取的字节作为`int`返回。如果长度为零，则不读取任何字节，除非努力读取至少一个字节，否则返回 0。
6.  `skip(long arg)`：`skip(long arg)`方法用于跳过和丢弃输入流中的`arg`字节。
7.  `markSupported()`：`markSupported()`方法测试输入流是否支持标记和重置方法。默认情况下，Java IO `InputStream`的`markSupported`方法会产生`false`。
