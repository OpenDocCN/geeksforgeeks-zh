...
> 原文：[https://www.geeksforgeeks.org/collections-addall-method-in-java-with-examples/](https://www.geeksforgeeks.org/collections-addall-method-in-java-with-examples/)
...
如果集合因调用而改变，则该方法返回 `true`。

## 异常

如果 `elements` 包含一个或多个空值，并且 `c` 不允许空元素，或者如果 `c` 或 `elements` 为 `null`，则该方法抛出 `NullPointerException`。
