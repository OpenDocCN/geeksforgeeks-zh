...
[`ByteBuffer`](https://www.geeksforgeeks.org/tag/java-bytebuffer/) 类的 `putInt(int value)` 方法用于按照当前字节顺序将包含给定 `int` 值的四个字节写入当前位置的缓冲区，然后将该位置递增四。
...
下面是举例说明 `putInt(int value)` 方法的例子:
