...
每当我们需要遍历一个集合时，我们必须创建一个迭代器来遍历这个集合，然后我们可以将我们的业务逻辑放在集合中每个元素的循环中。如果执行不当，我们可能会遇到 `ConcurrentModificationException`。
...
## 参数:
该方法采用类型为 `Consumer` 的参数 `action`，代表每个元素要执行的动作。

## 返回:
`forEach` 的返回类型为 `void`。因此它不返回任何东西。

## 异常:
如果输入动作为空，则抛出 `NullPointerException`。
...
## 程序 3:
在这个程序中，我们将分别演示 `Consumer` 接口的实现，以便我们可以重用它。让我们创建一个实现 `Consumer` 接口并覆盖其 `accept` 方法的类 `CityConsumer`。
...
## 参考:
[https://www.geeksforgeeks.org/iterable-foreach-method-in-java-with-examples/](https://www.geeksforgeeks.org/iterable-foreach-method-in-java-with-examples/)