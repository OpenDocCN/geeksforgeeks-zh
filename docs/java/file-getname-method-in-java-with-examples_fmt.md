...
> 原文：[https://www.geeksforgeeks.org/file-getname-method-in-java-with-examples/](https://www.geeksforgeeks.org/file-getname-method-in-java-with-examples/)

`getName()` 方法是 [`File`](https://www.geeksforgeeks.org/File-class-in-java/) 类的一部分。该函数返回给定文件对象的名称。该函数返回一个包含给定文件对象名称的字符串对象。如果抽象路径不包含任何名称，则返回空字符串。
...
这个函数返回一个字符串值，它是给定文件对象的名称。

下面的程序将说明 `getName()` 函数的用法：

### 例 1

给我们一个文件的文件对象，我们要得到文件对象的名称。
