# Java 中 `FileInputStream` 类的 `available()` 方法示例

> 原文：[https://www.geeksforgeeks.org/fileinputstream-available-method-in-java-with-examples/](https://www.geeksforgeeks.org/fileinputstream-available-method-in-java-with-examples/)
...
*   **`FileNotFoundException`** – 如果该目录不可用，那么我们将获得 `FileNotFoundException`。
...
**步骤 2：** 现在，要从文件中读取有多少数据可供读取的数据，我们应该像下面这样使用 `FileInputStream` 对象调用一个可用的方法：
```java
