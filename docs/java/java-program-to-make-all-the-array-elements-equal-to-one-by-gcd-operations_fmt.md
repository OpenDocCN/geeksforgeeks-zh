...
> 原文: [https://www.geeksforgeeks.org/java-program-to-make-all-the-array-elements-equal-to-one-by-gcd-operations/](https://www.geeksforgeeks.org/java-program-to-make-all-the-array-elements-equal-to-one-by-gcd-operations/)

给你一个由 `N` 个整数组成的数组。您的任务是使最终数组的所有元素等于 `1`。您可以执行以下操作任意次数(可能为零)。选择 `A_i` 和 `A_j` 的两个指数、(0 <= i, j < n) 并将 `A_i` 和 `A_j` 都替换为它们的 [GCD(最大公约数)](https://www.geeksforgeeks.org/c-program-find-gcd-hcf-two-numbers/)。
...
**时间复杂度:** `O(N * log M)`，其中 `N` 是数组的长度，`M` 是数组的最小元素。