...
> 原文：[https://www.geeksforgeeks.org/encryption-and-decryption-application-in-android-using-caesar-cipher-algorithm/](https://www.geeksforgeeks.org/encryption-and-decryption-application-in-android-using-caesar-cipher-algorithm/)
...
在这个应用程序中，我们将提供一个空间(文本视图)来显示加密或解密消息的输出。消息、密文和密钥将作为用户的输入。将创建一个名为 `Utility` 的 java 类来编写加密和解密按钮的逻辑。注意，我们将使用 `Java` 语言来实现这个应用程序。下面给出了一个示例视频，以了解我们将在本文中做什么。
...
如下所示创建一个新的 java 类，并将其命名为 `Utility`。
...
在 `MainActivity.java` 文件中，`onClickListerner` 用于加密和解密按钮，`utility` 类的方法直接在这里传递。在 `MainActivity.java` 文件中使用以下代码。
