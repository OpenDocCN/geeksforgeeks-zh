...
`PrintWriter` 类是 `Writer` 类的实现。当我们必须打印很多项目时，使用 `PrintWriter` 比使用 `System.out.println` 更好，因为 `PrintWriter` 比其他打印机更快地将数据打印到控制台。

`System.out` 变量引用了一个包装缓冲区输出流的打印流类型的对象。当您在 `PrintStream` 上调用其中一个 `write()` 方法时，它会在内部刷新底层 `BufferedOutputStream` 的缓冲区。`PrintWriter` 不会发生这种情况。我们必须自己做。
