...
> 原文：[https://www.geeksforgeeks.org/deep-shallow-lazy-copy-java-examples/](https://www.geeksforgeeks.org/deep-shallow-lazy-copy-java-examples/)
...
我们可以将克隆定义为“创建对象的副本”。浅、深、懒复制与克隆过程有关。这其实是创建复制对象的三种方式。

## 浅拷贝

*   每当我们使用 `clone` 方法的默认实现时，我们都会得到对象的浅层副本，这意味着它会创建一个新实例，并将对象的所有字段复制到该新实例中，并将其作为对象类型返回，我们需要显式地将其转换回原始对象。这是对象的浅拷贝。
*   `Object` 类的 `clone()` 方法支持对象的浅拷贝。如果对象在浅拷贝中包含基元以及非基元或引用类型变量，则克隆的对象也引用原始对象引用的同一对象，因为只有对象引用被拷贝，而不是被引用的对象本身。
...
上面的代码显示了浅层复制。`data` 只是引用与 `values` 相同的数组。

![](img/b74f75b1fd41b448db6e70cea4339f25.png)

如果通过其他引用改变了 `values` 的元素，这可能会导致不愉快的副作用。
...
*   因此，对于深度复制，我们需要确保所有成员类也实现 `Cloneable` 接口，并覆盖 `Object` 类的 `clone()` 方法。
...
懒拷贝可以定义为浅复制和深复制的组合。该机制遵循一个简单的方法——在初始状态下，使用浅拷贝方法。计数器还用于跟踪有多少对象共享数据。当程序想要修改原始对象时，它会检查该对象是否被共享。如果对象是共享的，则启动深度复制机制。

## 汇总

在浅层复制中，只复制基本数据类型的字段，不复制对象引用。深度复制包括原始数据类型和对象引用的复制。没有硬性规定什么时候做浅拷贝，什么时候做深拷贝。懒惰复制是这两种方法的结合。
