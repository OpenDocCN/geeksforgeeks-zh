...
> 原文: [https://www.geeksforgeeks.org/maven-lifecycle-and-basic-maven-commands/](https://www.geeksforgeeks.org/maven-lifecycle-and-basic-maven-commands/)
...
下面是默认 Maven 生命周期及其 8 个步骤的表示：验证、编译、测试、打包、集成测试、验证、安装和部署。
...
2.  **编译**：它编译源代码，转换 `.java` 文件为 `.class` 文件并将这些类存储在 `target/class` 文件夹中。
3.  **测试**：为项目运行单元测试。
...
*   `mvn clean`：清理项目并移除上一次构建生成的所有文件。
*   `mvn compile`：编译项目的源代码。
*   `mvn test-compile`：编译测试源代码。
*   `mvn test`：为项目运行测试。
*   `mvn package`：为项目创建 `JAR` 或 `WAR` 文件，将其转换为可分发的格式。
*   `mvn install`：将打包的 `JAR`/`WAR` 文件部署到本地存储库。
*   `mvn deploy`：在编译、运行测试和构建项目后，将打包的 `JAR`/`WAR` 文件复制到远程存储库中。

通常，当我们运行上述任何命令时，我们会添加 `mvn clean` 步骤，以便在运行较新的构建之前移除从先前构建生成的 `target` 文件夹。这就是命令如何将 `clean` 步骤与 `install` 相结合：`mvn clean install`

同样，如果我们想在调试模式下运行该步骤以获得更详细的构建信息和日志，我们将在实际命令中添加 `-X`。因此，在调试模式打开的情况下，`install` 步骤将有以下命令：`mvn -X install`

考虑一个场景，我们不想在打包或安装 Java 项目时运行测试。在这种情况下，我们使用 `-DskipTests` 和实际命令。如果我们需要通过跳过与项目相关的测试来运行 `install` 步骤，那么命令将是：
