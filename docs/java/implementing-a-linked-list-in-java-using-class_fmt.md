...
在Java中，`LinkedList`可以表示为一个类，`Node`可以表示为一个单独的类。链接列表类包含节点类类型的引用。
...
3.  现在，3 个条件中的任何一个都可以存在:
    *   **情况 1:**`位置`为 0，即删除头部
        1.  在这种情况下，将节点的头更改为当前头的下一个节点。
        2.  释放被替换的头节点的内存。
    *   **情况 2:**`位置`大于 0 但小于列表大小，即在中间或最后，除了头部
        1.  在这种情况下，查找要删除的节点的上一个节点。
        2.  将上一个节点的下一个节点更改为当前节点的下一个节点。
        3.  释放被替换节点的内存。
    *   **情况 3:**`位置`大于列表大小，即列表中找不到位置
        1.  在这种情况下，不需要进行任何操作。
