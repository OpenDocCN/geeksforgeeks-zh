...
> 原文：[https://www.geeksforgeeks.org/gfact-52-java-object-creation-of-inherited-classes/](https://www.geeksforgeeks.org/gfact-52-java-object-creation-of-inherited-classes/)
...
我们可以看到，超类(`Fruit`)对象 `hashCode` 和子类(`Apple`)对象 `hashCode` 都是相同的，所以只创建了一个对象。这个对象属于 `Apple` 类(子类)，因为当我们试图打印创建的对象的类名时，它正在打印 `Apple` 子类。
