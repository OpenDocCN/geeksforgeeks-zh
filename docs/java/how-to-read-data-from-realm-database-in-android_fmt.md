...
> 原文：[https://www.geeksforgeeks.org/how-to-read-data-from-realm-database-in-android/](https://www.geeksforgeeks.org/how-to-read-data-from-realm-database-in-android/)

在前一篇文章中，我们已经看到[在安卓](https://www.geeksforgeeks.org/how-to-install-and-add-data-to-realm-database-in-android/)中向领域数据库添加数据。在本文中，我们将看一下在安卓应用程序中从我们的领域数据库中读取这些数据。

## 我们将在本文中构建什么？

在本文中，我们将简单地添加一个按钮来打开我们在上一篇文章中创建的项目中的一个新活动。在新活动中，我们将以[回收视图](https://www.geeksforgeeks.org/android-recyclerview/)的形式显示数据。下面是视频，我们将在其中看到我们将在本文中构建的内容。
...
所有的 Jetpack 组件都可以在 Google Maven 存储库中包括在 `build.gradle` 文件中。
...
转到 `activity_main.xml` 文件，添加一个新的按钮来打开一个新的活动来显示我们的课程列表。
...
导航到`app > res > layout > activity_read_courses.xml`并将下面的代码添加到该文件中。下面是活动文件的代码。
...
导航到`app > java > your package name > ReadCoursesActivity.java`文件，并添加下面的代码。代码中添加了注释，以更详细地理解代码。
