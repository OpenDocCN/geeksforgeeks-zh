...
> 原文: [https://www.geeksforgeeks.org/effectively-final-variable-in-java-with-examples/](https://www.geeksforgeeks.org/effectively-final-variable-in-java-with-examples/)

最终变量是用名为 `final` 的关键字声明的变量。
...
> **注意:** `Final` 和 `Effective Final` 类似，只是有效的 `Final` 变量没有用关键字 `final` 声明。
