...
> 原文：[https://www.geeksforgeeks.org/enhancements-for-switch-statement-in-java-13/](https://www.geeksforgeeks.org/enhancements-for-switch-statement-in-java-13/)
...
上面的代码通过匹配相应的案例并执行特定的代码块来工作。只要您提供必要的 `break` 语句，它就能正常工作。

但是，如果我们忘记了任何必需的 `break` 语句，会发生什么呢：
...
显然，这不是预期的输出。这是意外遗漏 `break` 语句的结果。
...
开关现在可以用作表达式。这意味着 `switch` 现在可以根据我们的输入返回值。为了适应这种变化，开关语法略有变化。开关块需要用分号分隔。`yield` 关键字用于返回值。收益表不需要 `break`。
...
开关表达式需要指定所有可能输入值的处理。我们要么提供所有可能的情况，要么指定 `default` 情况。这意味着，不管输入值是什么，`switch` 表达式应该总是返回某个值或者显式抛出一个异常。
...
为交换机引入了新的箭头 `->` 语法。它可以与 `switch` 一起用作表达和陈述。
...
这种语法的主要优点是我们不需要 `break` 语句来避免默认的失败。所以规则是，如果我们需要摔倒，用 `case:` 否则不用 `case` `->`。还要注意，对于所有案例分支，应该是 `case:` 或 `case ->`。开关中不能有不同的情况，否则会导致错误。
