...
`LinkedBlockingDeque`的`offer(E e)`方法，将参数中传递的元素插入到目标的末尾。如果容器的容量已经超出，那么它不会像`add()`和`addFirst()`函数那样返回异常。
...
[https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/LinkedBlockingDeque.html#offer(E)](https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/LinkedBlockingDeque.html#offer(E))