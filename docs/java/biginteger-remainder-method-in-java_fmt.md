# Java 中的 BigInteger remainder() 方法

> 原文: [https://www.geeksforgeeks.org/biginteger-remainder-method-in-java/](https://www.geeksforgeeks.org/biginteger-remainder-method-in-java/)

`java.math.BigInteger.remainder(BigInteger big)` 方法返回一个 `BigInteger`，其值等于（此 `BigInteger` % `big`（作为参数传递的 `BigInteger`））。余数运算在这个大整数被作为参数传递的另一个大整数除之后找到余数。
...
该方法返回的 `BigInteger` 等于这个 `BigInteger` % `big`（作为参数传递）。

## 异常

该方法抛出一个 `ArithmeticException` – 当 `big = 0` 时。
...
下面的程序说明了 `BigInteger` 类的 `remainder()` 方法。
...
当作为参数传递的 `BigInteger` 等于零时，程序显示异常。
...
[BigInteger remainder() 文档](https://docs.oracle.com/javase/7/docs/api/java/math/BigInteger.html#remainder(java.math.BigInteger))