...
`java.lang.Math.floorDiv()` 是 Java 中的内置数学函数，它返回小于或等于代数商的最大（最接近正无穷大）`int` 值。由于 `floorDiv()` 是**静态的**，所以不需要创建对象。
...
*   **算术异常**: 如果除数为零，则抛出 `ArithmeticException`。

## 返回值

此方法返回小于或等于代数商的最大（最接近于正无穷大）整数值。

下面的程序举例说明了 `java.lang.Math.floorDiv()` 方法:
