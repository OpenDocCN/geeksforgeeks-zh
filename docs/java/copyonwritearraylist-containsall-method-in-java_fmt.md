# Java 中的 CopyOnWriteArrayList containsAll() 方法

> 原文：[https://www.geeksforgeeks.org/copyonwritearraylist-containsall-method-in-java/](https://www.geeksforgeeks.org/copyonwritearraylist-containsall-method-in-java/)
...
该方法接受集合类型的强制参数 `col`。这是一个集合，需要检查它的元素是否在列表中。

## 返回值

如果集合 `col` 中的所有元素都出现在列表中，则该方法返回 `true`，否则返回 `false`。

## 异常

如果指定的集合为空，则方法抛出 `NullPointerException`。
...
[https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/CopyOnWriteArrayList.html#containsAll(java.util.Collection)](https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/CopyOnWriteArrayList.html#containsAll(java.util.Collection))