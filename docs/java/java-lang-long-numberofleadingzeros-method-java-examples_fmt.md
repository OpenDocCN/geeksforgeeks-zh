# Java 中的 `Long.numberOfLeadingZeros()` 方法示例

> 原文：[`https://www.geeksforgeeks.org/java-lang-long-numberofleadingzeros-method-java-examples/`](https://www.geeksforgeeks.org/java-lang-long-numberofleadingzeros-method-java-examples/)

## 方法介绍

`java.lang.Long.numberOfLeadingZeros()` 是 Java 中的一个内置函数，它返回最高阶集合位左边的前导零位数。简单来说，它返回 **(64 位)**，其中位置指的是从右侧开始的最高阶设置位。如果数字不包含任何设置位（换句话说，如果数字为零），它将返回 64。
...
下面的程序说明了 `java.lang.Long.numberOfLeadingZeros()` 函数：
