...
> 原文：[https://www.geeksforgeeks.org/collection-clear-method-in-java-with-examples/](https://www.geeksforgeeks.org/collection-clear-method-in-java-with-examples/)

`java.util.Collection` 接口的 `clear()` 方法用于清除调用它的 `Collection`。此方法不接受任何参数，也不返回任何值。
