# Java Math.IEEEremainder() 方法示例

> 原文：`https://www.geeksforgeeks.org/java-math-ieeeremainder-method-examples/`

`Math.IEEEremainder()` 方法按照 IEEE 754 标准的规定计算两个参数的余数运算。余数在数学上等于 `f1 - f2 * n`，其中 `n` 是最接近商 `f1/f2` 的精确数学值的数学整数，如果两个数学整数相等地接近 `f1/f2`，则 `n` 是偶数的整数。
...
**返回值：**
此方法返回 `dividend` 除以 `divisor` 的余数。

## 示例 1

展示 `Math.IEEEremainder()` 方法的工作。
