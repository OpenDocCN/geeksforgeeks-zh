...
`WeakHashMap`是`Map`接口的一个实现。`WeakHashMap`与`HashMap`几乎相同，除了`WeakHashMap`的情况，如果对象被指定为键不包含任何引用-即使它与`WeakHashMap`相关联，它也有资格进行垃圾收集。也就是说，垃圾收集器在地图上占据主导地位。
...
1.  **强引用 vs 弱引用**: 弱引用对象不是引用对象的默认类型/类别，使用时应明确指定。这种类型的引用在`WeakHashMap`中用于引用条目对象。
...
3.  **克隆方法实现**: `HashMap`实现`Cloneable`接口。
    `WeakHashMap`不实现`Cloneable`接口，只实现`Map`接口。因此，在`WeakHashMap`类中没有`clone()`方法。
