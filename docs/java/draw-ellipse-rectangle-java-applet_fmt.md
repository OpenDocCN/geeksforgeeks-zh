...
> 原文: [https://www.geeksforgeeks.org/draw-ellipse-rectangle-java-applet/](https://www.geeksforgeeks.org/draw-ellipse-rectangle-java-applet/)
...
示例:
### 我们将在 Applet 上的位置 100，100 处绘制一个高 200、宽 200 的矩形。
...
## 现在我们来看看如何在 Java 小程序中绘制一个矩形

我们可以通过两种方式在 java 小程序中绘制矩形。

### 1. 使用 `drawRect(int x, int y, int width, int height)` 绘制一个矩形
...
### 2. 使用 `drawLine(int x, int y, int x1, int y1)` 绘制一个矩形
