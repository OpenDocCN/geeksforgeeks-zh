...
> 原文：[https://www.geeksforgeeks.org/java-program-to-insert-a-new-node-at-the-beginning-of-the-circular-linked-list/](https://www.geeksforgeeks.org/java-program-to-insert-a-new-node-at-the-beginning-of-the-circular-linked-list/)

## 循环链表

循环链表是一个元素序列，其中每个元素都指向序列中的下一个元素，最后一个元素链接到第一个元素。这意味着循环链表类似于单个链表，只是最后一个节点指向列表中的第一个节点。

## 在循环链表开头插入新节点

如果链表是空的，那么`head`和`tail`都指向新添加的节点。如果列表不是空的，那么我们将最后一个节点指向新添加的节点，并将新添加的节点指向`head`节点，最后，我们将新添加的节点作为`head`节点。
...
1.  创建一个`Node`类，代表列表中的一个节点。它有两个变量`data`和`next`指针（指向下一个节点）。
2.  创建另一个类来创建循环链表，它有两个节点，即`head`和`tail`。
3.  当添加一个新节点到列表中时，我们将首先检查`head`是否为`null`。如果列表为空或者`head`为空，那么我们将插入节点作为`head`，`tail`也指向新添加的节点。
4.  如果列表不为空，则新添加的节点将指向`head`，`tail`将指向新添加的节点，新节点将作为`head`节点。
