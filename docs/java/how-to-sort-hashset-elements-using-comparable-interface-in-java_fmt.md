...
> 原文：[https://www.geeksforgeeks.org/how-to-sort-hashset-elements-using-comparable-interface-in-java/](https://www.geeksforgeeks.org/how-to-sort-hashset-elements-using-comparable-interface-in-java/)

[`HashSet`](https://www.geeksforgeeks.org/hashset-in-java/) 类实现了 [`Set`](https://www.geeksforgeeks.org/set-in-java/) 接口，由哈希表支持，哈希表实际上是一个 [`HashMap`](https://www.geeksforgeeks.org/java-util-hashmap-in-java-with-examples/) 实例。不能保证集合的迭代顺序，这意味着当我们迭代 `HashSet` 时，不能保证我们得到的元素是按照它们被插入的顺序。

要使用 Java 中的 [`Comparable`](https://www.geeksforgeeks.org/comparable-vs-comparator-in-java/) 接口对 `HashSet` 元素进行排序，首先，我们创建一个实现 `Comparable` 接口的类 `Student`。在这个类中，我们覆盖了 [`compareTo()`](https://www.geeksforgeeks.org/java-equals-compareto-equalsignorecase-and-compare/) 方法。
