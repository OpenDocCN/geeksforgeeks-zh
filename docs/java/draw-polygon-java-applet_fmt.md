...
## 1. `drawPolygon(int[] x, int[] y, int numberofpoints)`

`drawPolygon(int[] x, int[] y, int numberofpoints)` 使用给定的 x 和 y 坐标点集绘制一个多边形。
...
## 2. `drawPolygon(Polygon p)`

`drawPolygon(Polygon p)` 使用给定的 `Polygon` 类对象绘制一个多边形。
...
## 3. `drawLine(int x, int y, int x1, int y1)`

在这种方法中，我们将使用线段连接相邻的顶点，并连接第一个和最后一个顶点。
