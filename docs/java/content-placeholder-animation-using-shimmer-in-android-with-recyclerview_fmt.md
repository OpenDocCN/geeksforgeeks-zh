# 内容占位符动画在 Android 中使用微光与 RecyclerView

> 原文：[https://www.geeksforgeeks.org/content-placeholder-animation-using-shimmer-in-android-with-recyclerview/](https://www.geeksforgeeks.org/content-placeholder-animation-using-shimmer-in-android-with-recyclerview/)

我们已经看到了在 Android 中使用 Shimmer 实现内容占位符动画的一个简单视图。在这篇文章中，我们将看看如何在安卓系统中使用 RecyclerView 实现内容占位符动画。

## 我们将在本文中构建什么？

我们将构建一个简单的应用程序，在这个应用程序中，我们将从我们的 API 在我们的安卓 RecyclerView 中简单地显示数据。为了加载数据，我们将显示一个内容占位符用于加载。下面给出了一个示例视频，让我们了解一下在本文中要做什么。请注意，我们将使用 Java 语言来实现这个项目。
...
导航到 `app > AndroidManifest.xml`，并在我们从互联网加载数据时为其添加互联网权限。
...
导航到 `app > java > 你的应用的包名 > 右键点击它 > New > Java Class` 并将其命名为 `CourseModal` 并添加下面的代码。
...
### 第五步：在安卓中为我们的 RecyclerView 的项目创建布局文件

导航至 `app > res > layout > 右键点击 > New > Layout Resource File` 并将其命名为 `course_rv_item` 并添加以下代码。
...
导航至 `app > res > layout > 右键单击 > New > Layout Resource File` 并将其命名为 `shimmer_layout` 并添加以下代码。
...
            as a grey colour and not providing
            any view inside it-->
        <LinearLayout
            android:layout_width="match_parent"
            android:layout_height="match_parent"
