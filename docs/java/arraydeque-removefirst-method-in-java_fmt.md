...
> 原文: [https://www.geeksforgeeks.org/arraydeque-removefirst-method-in-java/](https://www.geeksforgeeks.org/arraydeque-removefirst-method-in-java/)

`java.util.ArrayDeque.removeFirst()`方法用于移除`Deque`的第一个元素。
...
方法抛出`NoSuchElementException`，如果`deque`为空则抛出。

下面的程序说明了`java.util.ArrayDeque.removeFirst()`方法：
