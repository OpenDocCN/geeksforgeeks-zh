...
> 原文：[https://www.geeksforgeeks.org/how-to-apply-different-styles-to-a-cell-in-a-spreadsheet-using-java/](https://www.geeksforgeeks.org/how-to-apply-different-styles-to-a-cell-in-a-spreadsheet-using-java/)

[Apache POI](https://www.geeksforgeeks.org/apache-poi-introduction/) 是一个功能强大的 API，用户可以使用 Java 程序基于 Microsoft Office 创建、操作和显示各种文件格式。使用 Apache POI，应该能够对以下文件格式执行创建、修改和显示/读取操作。例如，Java 不提供使用 Excel 文件的内置支持，所以我们需要为这项工作寻找开源的 API。它是由 Apache 软件基金会开发和分发的一个开源库，用于使用 Java 程序设计或修改 Microsoft Office 文件。它包含将用户输入数据或文件解码成微软办公文档的类和方法。这里可以应用不同的样式，如字体、颜色、单元格合并、对齐等。在 Java 程序中使用这个概念。

## Apache POI 架构

它由构成架构的各种组件组成，以形成一个工作系统：

*   **POIFS (Poor Obfuscation Implementation File System):** 这个组件是所有其他 POI 元素的基本因素。它用于显式读取不同的文件。
*   **HSSF (Horrible Spreadsheet Format):** 用来读写 MS-Excel 文件的 `xls` 格式。
*   **XSSF (XML Spreadsheet Format):** 用于 MS-Excel 的 `xlsx` 文件格式。
*   **HPSF (Horrible Property Set Format):** 用于提取 MS-Office 文件的属性集。
*   **HWPF (Horrible Word Processor Format):** 用于 MS-Word 文档扩展文件的读写。
*   **XWPF (XML Word Processor Format):** 用于 MS-Word 的 Docx 扩展文件的读写。
*   **HSLF (Horrible Slide Layout Format):** 用于阅读、创建和编辑 PowerPoint 演示文稿。
*   **HDGF (Horrible Diagram Format):** 包含 MS-Visio 二进制文件的类和方法。
*   **HPBF (Horrible Publisher Format):** 用于读写 MS-Publisher 文件。

Jar 文件是包含一个或多个 Java 类文件的 Zip 档案。这使得库的使用更加方便。目录和 Jar 文件被添加到构建路径中，并在运行时可供类加载器查找其中的特定类。我们一般用 `.jar` 文件以 Java 类文件和相关元数据和资源（文本、图像等）的形式分发 Java 应用程序或库。

> 人们可以说 JAR = Java ARchive

## 进场

*   **步骤 1:** 导入必要的 Jar 文件，像 `HSSF`、`XML` 这样的，并将它们添加到您的构建路径中。
*   **步骤 2:** 使用 `new XSSFWorkbook()` 创建工作簿，其中我们必须使用 `workbook.createSheet('Sheet1')` 创建电子表格或 Excel 文件，我们将在其中应用不同的样式。

![](img/93b099b3b3b70d34fedda0cfa20b228b.png)

*   **步骤 3:** 应用样式，这里使用命令电子表格合并单元格。添加合并区域。我们必须提供行和列的范围地址及其参数。
*   **步骤 4:** 现在确定单元格对齐的下一个样式。为此，我们有两个命令：
    *   `style1.setAlignment(XSSFCellStyle.ALIGN_...)` 用于确定对齐。
    *   `style1.setVerticalAlignment(XSSFCellStyle.VERTICAL_...)` 用于确定垂直对齐。
*   **步骤 5:** 对于给单元格应用边框，我们可以使用 `setBorderBottom/Left/Top/Right(XSSFCell borderBottom)`。
*   **步骤 6:** 更改参数中的边框名称，以经历不同的边框样式。
*   **步骤 7:** 填充颜色和添加图案，首先使用 `setFillBackgroundColor(HSSFColor.COLOR_NAME.index)` 设置单元格的背景颜色。
*   **步骤 8:** 然后通过 `setFillPattern(XSSFCellStyle.SOLID_FOREGROUND)` 设置你喜欢的模式。
*   **步骤 9:** 最后，使用 `setAlignment(XSSFCellStyle.ALIGN_TYPE)` 设置对齐方式。

## 实现

对已经创建的本地目录下的空 Excel 文件执行上述程序步骤。
...
style6.setFillPattern(XSSFCellStyle.FILL_HORIZONTAL_CROSS_HATCH);
...
style7.setFillPattern(XSSFCellStyle.THIN_VERTICAL_STRIPE);
...
# Step 10: Creating a new file in the local directory by creating object of FileOutputStream

```java
FileOutputStream out = new FileOutputStream(new File("C:/poiexcel/stlingcells.xlsx"));
```

# Step 11: Write to above workbook created in initial step
...
Display message for console window when program is successfully executed.
