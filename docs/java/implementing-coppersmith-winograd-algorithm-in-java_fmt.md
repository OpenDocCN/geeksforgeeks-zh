# 在 Java 中实现 Coppersmith-Winograd 算法

> 原文：[https://www.geeksforgeeks.org/implementing-coppersmith-winograd-algorithm-in-java/](https://www.geeksforgeeks.org/implementing-coppersmith-winograd-algorithm-in-java/)

`Coppersmith-Winograd` 算法是迄今为止已知的矩阵乘法算法中渐进最快的算法。在实际中很少使用具有优于 `Strassen` 计算的渐近运行时间的算法，因为它们运行场合中的巨大稳定因素使它们不合适。

它可以在 `O(n^{2.375477})` 时间内乘以两个 `n × n` 矩阵。它用于检查矩阵乘法。
