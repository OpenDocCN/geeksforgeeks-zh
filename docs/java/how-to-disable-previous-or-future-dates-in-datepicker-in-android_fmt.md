...
> 原文：[https://www.geeksforgeeks.org/how-to-disable-previous-or-future-dates-in-datepicker-in-android/](https://www.geeksforgeeks.org/how-to-disable-previous-or-future-dates-in-datepicker-in-android/)
...
用`getDatePicker()`从`DatePickerDialog`中获取日期选择器。使用`setMaxDate()`将最大日期设置为当前日期：
...
同样，用`getDatePicker()`从`DatePickerDialog`中获取日期选择器。使用`setMinDate()`将最小日期设置为当前日期。
