...
> 原文：[https://www.geeksforgeeks.org/java-program-to-implement-rolelist-api/](https://www.geeksforgeeks.org/java-program-to-implement-rolelist-api/)
...
1.  `RoleList()`：构建一个空的角色列表。
2.  `RoleList(int initial_capacity)`：用指定的初始能力构建一个空的角色列表。
3.  `RoleList(List<Role> list)`：按照列表迭代器返回的顺序，构建包含指定列表元素的角色列表。
