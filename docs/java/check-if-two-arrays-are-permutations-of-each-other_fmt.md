...
一个简单的解决方案是对两个数组进行排序，并比较排序后的数组。这个解决方案的时间复杂度是 `O(nLogn)`。

一个更好的解决方案是使用[哈希](https://www.geeksforgeeks.org/hashing-set-1-introduction/)。
...
该方法的时间复杂度为`O(n)`，假设我们有一个`哈希函数`在`O(1)`时间内插入和查找元素。
