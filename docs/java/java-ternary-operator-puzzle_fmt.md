...
2.  如果其中一个操作数是 `T` 类型，其中 `T` 是 `byte`、`short` 或 `char`，而另一个操作数是 `int` 类型的常量表达式，其值可在 `T` 类型中表示，则条件表达式的类型是 `T`。
...
条件表达式的类型决定了调用打印方法的哪个重载。对于第一个表达式，调用 `PrintStream.print(char)`；对于第二个，`PrintStream.print(int)`。前者重载将变量 `x` 的值打印为 Unicode 字符 (`X`)，而后者将其打印为十进制整数 (`88`)。
