...
> 原文: [https://www.geeksforgeeks.org/bigdecimal-bytevalueexact-method-in-java/](https://www.geeksforgeeks.org/bigdecimal-bytevalueexact-method-in-java/)

`BigDecimal.byteValueExact()` 是一个内置函数，它将 `BigDecimal` 转换为一个字节，并检查丢失的信息。任何大于 127 或小于 -128 的大十进制值都将产生异常，因为它不符合字节范围。
...
这个方法返回 `BigDecimal` 对象的字节值。

## 异常

如果大十进制有非零小数部分，即十进制值，或者超出字节结果的可能范围，该函数抛出 `ArithmeticException`。
...
[https://docs.oracle.com/javase/7/docs/api/java/math/BigDecimal.html#byteValueExact()](https://docs.oracle.com/javase/7/docs/api/java/math/BigDecimal.html#byteValueExact())