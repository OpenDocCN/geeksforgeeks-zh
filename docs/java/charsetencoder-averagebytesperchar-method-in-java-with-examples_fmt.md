...
`averageBytesPerChar()` 方法是 `java.nio.charset.CharsetEncoder` 的内置方法，它返回所提供的每个输入字符将产生的平均字节数。启发式值因此被用于估计给定输入序列所需的输出缓冲器的大小。
...
[`https://docs.oracle.com/javase/10/docs/api/java/nio/charset/CharsetEncoder.html#averageBytesPerChar()`](https://docs.oracle.com/javase/10/docs/api/java/nio/charset/CharsetEncoder.html#averageBytesPerChar())