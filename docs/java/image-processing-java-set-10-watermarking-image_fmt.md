...
> 原文：[https://www.geeksforgeeks.org/image-processing-java-set-10-watermarking-image/](https://www.geeksforgeeks.org/image-processing-java-set-10-watermarking-image/)
...
*   [`setFont(Font f)`](https://docs.oracle.com/javase/7/docs/api/java/awt/Graphics.html#setFont(java.awt.Font)) – 该方法在 awt 包的 `Font` 类中找到，构造函数以 (`FONT_TYPE`, `FONT_STYLE`, `FONT_SIZE`) 为参数。
...
*   [`drawString(String str, int x, int y)`](https://docs.oracle.com/javase/7/docs/api/java/awt/Graphics.html#drawString(java.lang.String,%20int,%20int)) – `Graphics` 类中的 `Fond` 将字符串文本和位置坐标作为 `x` 和 `y` 作为参数。
