...
> 原文:[https://www.geeksforgeeks.org/formatting-text-on-a-slide-in-a-ppt-using-java/](https://www.geeksforgeeks.org/formatting-text-on-a-slide-in-a-ppt-using-java/)
...
1.  使用阿帕奇`poi`包中的 `XMLSlideShow` 创建一个空演示对象。
2.  创建幻灯片母版对象，并使用 `XSLFSlideMaster` 获取第一张幻灯片。
3.  使用 `XSLFSlideLayout` 对象设置幻灯片的布局。
4.  使用布局创建幻灯片。
5.  使用 `XSLFTextShape` 对象获取幻灯片的第二个标题，并使用`XSLFTextParagraph`对象为其添加段落。
