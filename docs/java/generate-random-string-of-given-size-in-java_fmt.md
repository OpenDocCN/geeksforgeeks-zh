...
> 原文：[https://www.geeksforgeeks.org/generate-random-string-of-given-size-in-java/](https://www.geeksforgeeks.org/generate-random-string-of-given-size-in-java/)
...
这里函数 `getAlphaNumericString(n)` 生成一个长度为 `n` 的字符串的随机数。这个数字是一个字符的索引，这个字符附加在临时局部变量 `sb` 中。最后返回。
...
使用 `java.nio.charset` 包中的 `Charset` 随机生成 20 个字符长的字母数字字符串。
