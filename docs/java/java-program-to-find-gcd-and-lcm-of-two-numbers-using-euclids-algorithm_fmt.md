...
> 原文: [https://www.geeksforgeeks.org/java-program-to-find-gcd-and-lcm-of-two-numbers-using-euclids-algorithm/](https://www.geeksforgeeks.org/java-program-to-find-gcd-and-lcm-of-two-numbers-using-euclids-algorithm/)
...
这种计算 `gcd` 的方法是基于这样的原理，即两个数字 `A` 和 `B` 的 `GCD` 保持不变，即使用 `A` 和 `B` 的模代替较大的数字。在这种方法中，我们通过用 `A` 和 `B` 的模代替 `B` 来重复对 `A` 和 `B` 执行 `GCD` 操作，直到模变成 0。

下面是使用欧几里德算法求两个数的 `GCD` 和 `LCM` 的实现：
