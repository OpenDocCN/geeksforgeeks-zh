...
> 原文：[https://www.geeksforgeeks.org/difference-and-similarities-between-hashset-linkedhashset-and-treeset-in-java/](https://www.geeksforgeeks.org/difference-and-similarities-between-hashset-linkedhashset-and-treeset-in-java/)
...
| 何时使用 | 如果您不想保持插入顺序，但想存储唯一的对象 | 如果您想保持元素的插入顺序，那么您可以使用 `LinkedHashSet` | 如果你想根据一些比较器对元素进行排序，那么就使用 `TreeSet` |
...
## 根据插入顺序和时间，HashSet、LinkedHashSet 和 TreeSet 之间的差异
...
class GFG1 {
    ...
    private static void insertionTime()
    {
        ...
        System.out.println("Total time to insert"
                           + " 1000 elements in"
                           + " LinkedHashSet in nanoseconds: "
                           + (endTime - startTime));

        // TreeSet performance Test
        // inserting 1000 elements
...
Insertion Order of objects in HashSet :[Geek1, Geek2, Geek3]
Insertion Order of objects in LinkedHashSet :[Geek2, Geek1, Geek3]
Insertion Order of objects in TreeSet :[Geek1, Geek2, Geek3]

Insertion Time...

Total time to insert 1000 elements in HashSet in nanoseconds: 123456
Total time to insert 1000 elements in LinkedHashSet in nanoseconds: 234567
Total time to insert 1000 elements in TreeSet in nanoseconds: 345678
```

**注意：** 输出中的时间值是近似值，实际值会因硬件和 JVM 而异。
...
何时使用 `HashSet`、`TreeSet` 以及 Java 中的 `LinkedHashSet`:
