...
> 原文：[https://www.geeksforgeeks.org/image-processing-in-java-read-and-write/](https://www.geeksforgeeks.org/image-processing-in-java-read-and-write/)

Java 为图像实现了一种特殊类型的对象，称为 `BufferedImage`。可以从几种不同的图像类型（即 BMP、HEIC 等）中读取 `BufferedImage`。并非所有这些都由 `ImageIO` 本身支持，但是有插件来扩展 `ImageIO` 和其他库，如 Apache Imaging 和 JDeli。
