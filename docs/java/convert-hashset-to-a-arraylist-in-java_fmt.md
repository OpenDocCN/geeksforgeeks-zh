...
> 原文：[https://www.geeksforgeeks.org/convert-hashset-to-a-arraylist-in-java/](https://www.geeksforgeeks.org/convert-hashset-to-a-arraylist-in-java/)
...
还有一种将 `HashSet` 对象转换成 `ArrayList` 的方法，就是使用 Java 8 中引入的 Stream API。我们将使用 `Collectors` 类的 `toList()` 方法将输入数据转换为列表，但是我们希望它是一个数组列表，所以我们将使用前面的这个（`ArrayList<类型对象>`）将其类型转换为数组列表。
