...
| 方法 | 说明 |
| --- | --- |
| `equals(Object object)` | 返回二维对象是否相等 |
| `getHeight()` | 返回 `Dimension2D` 对象的高度 |
| `getWidth()` | 返回 `Dimension2D` 对象的宽度 |
| `hashCode()` | 返回 `Dimension2D` 对象的哈希代码值。 |

以下程序将说明 `Dimension2D` 类的使用:

### 1. Java program to create a Dimension2D object and display its contents

This program creates a `Dimension2D` object named `dimension`. The height and width of the dimension is passed as arguments. Then the height and width are extracted from the object using `getHeight()` and `getWidth()` function and the respective values are displayed.
...
public class Dimension_2 {
    ...
    public static void display(Dimension2D dimension)
    {
        ...
        // get the height and width of the dimension
        height = dimension.getHeight();
        width = dimension.getWidth();
...
**参考:** [https://docs.oracle.com/javase/8/javafx/api/javafx/geometry/Dimension2D.html](https://docs.oracle.com/javase/8/javafx/api/javafx/geometry/Dimension2D.html)