...
> 原文：[https://www.geeksforgeeks.org/bloom-filter-in-java-with-examples/](https://www.geeksforgeeks.org/bloom-filter-in-java-with-examples/)
...
## 为什么像 `HashSet` 或者 `HashTable` 这样的数据结构会失效？
...
### 测试输入 #1
假设我们要检查 IP `75.245.10.1`。使用与添加上述输入时相同的两个哈希函数来传递此 IP。
...
### 测试输入 #2
假设我们要检查 IP `75.245.20.30` 是否出现在集合中？过程将是相同的，使用与增加上述输入时同样的两个散列函数来传递这个 IP。
...
在这种情况下，索引 2 和 19 由其他输入设置为 1，而不是由该 IP `101.125.20.22` 设置。这被称为 **碰撞**，这就是为什么它是 **概率** 的，发生的几率不是 100%。

### 对布隆过滤器有什么期待？
1.  当布隆过滤器说某个元素不存在时，它肯定是 **不存在**。它 100% 保证给定的元素在集合中不可用，因为散列函数给出的索引的任一位都将被设置为 0。
2.  但是当 Bloom filter 说给定元素是 **present** 时，它是 **不是 100% 确定**，因为由于冲突，散列函数给出的索引的所有位都可能被其他输入设置为 1。

### 如何从一个 Bloom 滤镜得到 100% 准确的结果？
...
3.  **降低误报概率**
    在 Bloom filter 对象创建时引入另一个参数，如下所示：
    `BloomFilter<String> blackListedIps = BloomFilter.create(Funnels.stringFunnel(Charset.forName("UTF-8")), 10000, 0.005);`

现在假阳性概率已经从 **0.03 降低到了 0.005**。但是调整这个参数对布隆过滤器有影响。
