...
> 原文: [https://www.geeksforgeeks.org/java-string-class-strip-method-with-examples/](https://www.geeksforgeeks.org/java-string-class-strip-method-with-examples/)

Java `String` 类 `strip()` 方法返回一个字符串，该字符串提供了一个去掉了所有前导和尾随空格的字符串。这种方法类似于 `trim()` 方法。基本上有 3 种方法可以用不同的方式去除空白。
...
`stripTrailing()` 方法：返回一个字符串，去掉所有尾随空格。
