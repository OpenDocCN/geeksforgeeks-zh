...
> 原文: [https://www.geeksforgeeks.org/iterate-over-the-characters-of-a-string-in-java/](https://www.geeksforgeeks.org/iterate-over-the-characters-of-a-string-in-java/)
...
### 方法二: 使用 [`String.toCharArray()`](https://www.geeksforgeeks.org/java-string-tochararray-example/) 方法
...
## 方法7：使用`String.chars()`方法

在这种方法中，我们使用字符串类的`chars()`方法。由于性能原因，此方法不返回`Stream<Character>`对象。它返回一个`IntStream`对象，该对象可以转换为`Stream<Character>`。
...
在这种方法中，我们使用`String.codePoints()`方法返回一个Unicode值流。
