...
> 原文: [https://www.geeksforgeeks.org/difference-between-arrayblockingqueue-and-arraydeque/](https://www.geeksforgeeks.org/difference-between-arrayblockingqueue-and-arraydeque/)
...
[`ArrayDeque`](https://www.geeksforgeeks.org/arraydeque-in-java/) 是 Java 中一个同时实现 `Queue` 和 `Deque` 的类。它从两侧动态地重新调整大小。这是一种特殊的数组，它不断增长，允许用户在队列的两端添加或删除元素。又称 **数组双端队列** 或 **数组甲板**。

`ArrayDeque` 的一些重要特征如下:
...
| 序列号 | ArrayBlockingQueue | ArrayDeque |
| :--- | :--- | :--- |
| 1. | `ArrayBlockingQueue` 实现了 `BlockingQueue` 接口。 | `ArrayDeque` 实现了 `Deque` 接口。 |
| 2. | 这是一个固定大小的数组队列。因此，一旦我们创建了实例，我们就不能扩大和缩小队列的大小。 | 这是一个可调整大小的数组队列。因此，我们可以扩大和缩小队列的大小。 |
