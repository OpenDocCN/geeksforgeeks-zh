...
> 原文：[https://www.geeksforgeeks.org/java-program-to-remove-duplicate-elements-from-the-array/](https://www.geeksforgeeks.org/java-program-to-remove-duplicate-elements-from-the-array/)
...
1.  使用额外空间
2.  恒定额外空间
3.  使用[集合](https://www.geeksforgeeks.org/set-in-java/)
4.  使用频率数组
5.  使用[哈希表](https://www.geeksforgeeks.org/java-util-hashmap-in-java-with-examples/)

## 方法 1：（使用额外空间）

1.  创建一个临时数组 `temp[]` 来存储唯一元素。
2.  遍历数组并将 `a[]` 中的所有唯一元素复制到 `temp[]` 中。此外，记录唯一元素。让这个计数为 `j`。
3.  将 `j` 个元素从 `temp[]` 复制到 `a[]`。
