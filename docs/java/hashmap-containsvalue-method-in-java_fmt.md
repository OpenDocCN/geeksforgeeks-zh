# HashMap containsValue()方法

> 原文: [https://www.geeksforgeeks.org/hashmap-containsvalue-method-in-java/](https://www.geeksforgeeks.org/hashmap-containsvalue-method-in-java/)

`java.util.HashMap.containsValue()`方法用于检查特定值是由`HashMap`中的一个键映射还是由多个键映射。它将值作为参数，如果该值由映射中的任何键映射，则返回真。
...
下面的程序用来说明`java.util.HashMap.containsValue()`方法：
