...
> 原文: [https://www.geeksforgeeks.org/differences-between-interface-and-class-in-java/](https://www.geeksforgeeks.org/differences-between-interface-and-class-in-java/)

本文重点介绍了 Java 中的一个[类](https://www.geeksforgeeks.org/classes-objects-java/)和一个[接口](https://www.geeksforgeks.org/interfaces-in-java/)之间的区别。它们在语法上看起来很相似，都包含方法和变量，但它们在许多方面是不同的。
...
*   接口是关于功能的，比如玩家可以是一个接口，任何实现玩家的类都必须能够(或者必须实现)`move()`。所以它指定了类必须实现的一组方法。
...
| 它可以包含构造函数。 | 它不能包含构造函数。 |
| 它不能包含抽象方法。 | 它只包含抽象方法。 |
| 类中的变量和方法可以使用任何访问说明符(`public`、`private`、`default`、`protected`)来声明 | 接口中的所有变量和方法都声明为 `public` 的。 |
| 类中的变量可以是静态的、最终的或者两者都不是。 | 所有变量都是 `static` 和 `final` 的。 |