...
下图显示了形状、矩形和圆形三个类。形状是抽象的，而矩形和圆形是具体的，继承了形状。这是因为矩形和圆形实现了`area()`方法。
...
下面的代码说明了一个扩展抽象类的具体类。接口`X`中的方法`product()`由类`Product`实现，但它不实现方法`sum()`，因此它必须是抽象的。而类`Main`实现了未实现的方法`sum()`，因此没有未实现的方法。因此，它是一个具体的类。
