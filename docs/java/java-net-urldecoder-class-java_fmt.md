# Java 中的 URLDecoder 类

> 原文: [https://www.geeksforgeeks.org/java-net-urldecoder-class-java/](https://www.geeksforgeeks.org/java-net-urldecoder-class-java/)

这是一个用于 HTML 表单解码的实用程序类。它执行与 `URLEncoder` 类相反的操作，即给定一个编码字符串，它使用指定的方案对其进行解码。通常在 servlet 编程中使用 `getParameter()` 方法访问请求的内容时，值会在返回之前自动解码。但有时可能需要显式解码否则会被 URL 编码的字符串。
