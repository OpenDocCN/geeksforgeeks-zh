...
> 原文：`https://www.geeksforgeeks.org/field-togenericstring-method-in-java-with-examples/`

`java.lang.reflect.Field` 的 `toGenericString()` 方法用于返回表示该 Field 的字符串，包括其泛型类型。字符串的格式是字段的访问修饰符（如果有），后跟泛型字段类型、空格、声明字段的类的完全限定名、句点和字段名称。
