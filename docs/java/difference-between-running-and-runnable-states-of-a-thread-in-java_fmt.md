# Java中线程运行状态和可运行状态的区别

> 原文：[https://www.geeksforgeeks.org/difference-between-running-and-runnable-states-of-a-thread-in-java/](https://www.geeksforgeeks.org/difference-between-running-and-runnable-states-of-a-thread-in-java/)

## 线程状态概述

线程是Java多线程的支柱。多线程是一种允许程序的两个或多个部分同时执行的功能，以最大限度地利用CPU。这样一个程序的每个部分都被称为一个线程。所以线程是进程内的轻量级进程。

在Java中，一个线程可以有多个状态，并且在执行的任何时候都处于下列状态之一：

*   新的（New）
*   可运行的（Runnable）
*   运行（Running）
*   等待/阻塞（Waiting/Blocked）
*   终止/死亡（Terminated/Dead）
...
多线程程序为每个单独的线程分配固定的时间。每个线程运行一小段时间，然后暂停，将CPU让给另一个线程，这样其他线程就有机会运行。
...
**注意：** 以上代码的输出不一定每次执行都是一样的，因为这取决于CPU（线程调度器）分配给处理器的线程以及分配的时间。

## 输出解释

为了在上述程序的上下文中理解这一点，考虑当`301`被打印时，它意味着`Thread 3`处于运行状态，但是`Thread 1`和`Thread 2`的状态是什么？答案是，同时，当线程3在处理器中被执行时，线程2和线程1正在等待轮到它们被处理或执行，即它们当前处于可运行状态（或准备运行）。