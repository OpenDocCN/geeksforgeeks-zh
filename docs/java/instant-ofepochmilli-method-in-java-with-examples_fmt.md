...
`Instant`类的`ofEpochMilli()`方法有助于使用从 1970-01-01T00:00:00Z 的时期作为参数传递的毫秒来获得`Instant`。这个返回的毫秒用于通过转换获得不同的时间单位，如秒等。
...
## 参数:
该方法接受一个参数`epochMilli`为 1970-01-01T00:00:00Z 的毫秒值。

## 返回:
此方法返回`Instant`作为从纪元开始的时间，单位为毫秒。

## 异常:
如果结果超过最大或最小瞬间，该方法抛出`DateTimeException`。
...
参考文献: [https://docs.oracle.com/javase/10/docs/api/java/time/Instant.html#ofEpochMilli(long)](https://docs.oracle.com/javase/10/docs/api/java/time/Instant.html#ofEpochMilli(long))