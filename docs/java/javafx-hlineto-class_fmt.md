...
`HLineTo` 类是 JavaFX 的一部分。该类创建一条从当前位置到指定 x 坐标的水平线。`HLineTo` 类继承了 `PathElement` 类。
...
3.  然后创建一个名为 `moveto` 的 `MoveTo` 对象。
4.  现在将 `moveto` 和 `HLineto` 对象添加到路径中。
5.  将此路径添加到 `group` 对象，并将 `Group` 对象添加到场景，将场景添加到舞台，并调用 `show()` 函数显示最终结果。
...
4.  按顺序将所有 `MoveTo` 和 `HLineTo` 对象添加到路径中。
5.  之后，将此路径添加到 `group` 对象。
6.  将 `group` 对象添加到场景，将场景添加到舞台，调用 `show()` 函数显示最终结果。
