...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-dynamic-video-player-in-android-with-firebase-realtime-database/](https://www.geeksforgeeks.org/how-to-create-a-dynamic-video-player-in-android-with-firebase-realtime-database/)
...
我们将构建一个简单的应用程序，在其中我们将在一个 [`ExoPlayer`](https://www.geeksforgeeks.org/exoplayer-in-android-with-example/)中播放一个视频，并且我们将从 Firebase 加载一个视频到我们的 `ExoPlayer`中。与此同时，我们将在运行时更改我们的视频。下面给出了一个示例视频，让我们了解一下在本文中要做什么。注意，我们将使用 `Java`语言来实现这个项目。
...
现在验证您的应用程序是否连接到 Firebase。转到您的 `build.gradle`文件。导航到 `应用程序 > 渐变脚本 > build.gradle(应用程序)`文件，并确保以下依赖项已添加到您的依赖项部分。

> `implementation 'com.google.firebase:firebase-database:19.6.0'`

添加这个依赖项后，在你的 Gradle 文件中添加 `ExoPlayer`的依赖项。

### 第三步：在 build.gradle 文件中添加 ExoPlayer View 的依赖项

导航到 `应用程序 > 渐变脚本 > build.gradle(应用程序)`文件，并在其中添加以下依赖项。
    ...
    private void getVideoUrl() {
        ...
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                // after getting the value for our video url we are passing that value to our initialize exoplayer method to load our video
