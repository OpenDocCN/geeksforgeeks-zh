...
在 Java 中，`import`语句有助于在单个语句的帮助下获取一个[类](https://www.geeksforgeeks.org/classes-objects-java/)或在一个[包](https://www.geeksforgeeks.org/packages-in-java/)下指定的程序的所有可见类。这是非常有益的，因为程序员不需要编写整个类定义。因此，它提高了程序的可读性。

本文重点介绍 Java 程序中使用的`import`语句及其替代方法。
...
**编译描述:** 我们得到编译时错误。`javac`编译器在程序中找不到`ArrayList`类。`ArrayList`类是`java.util`包的一部分。所以我们需要在程序中包含`java.util`包下定义的`ArrayList`类。
