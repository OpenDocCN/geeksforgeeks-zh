...
> 原文: [https://www.geeksforgeeks.org/abstract-vs-public-access-modifier-in-java/](https://www.geeksforgeeks.org/abstract-vs-public-access-modifier-in-java/)

Java 中的 `Access Modifier` 是用于定义类、变量和方法的范围的保留关键字。它还告诉我们子类创建是否可能，或者对象创建是否可能。

`抽象访问修饰符`是一个只适用于类和方法而不适用于变量的修饰符。如果我们将任何方法声明为 `abstract` 方法，那么该方法必须在相应类的子类中实现，因为 `abstract` 方法从不谈论实现。如果任何修饰符谈论实现，那么它与 `abstract` 修饰符形成非法组合。以类似的方式，如果对于任何 `java` 类，如果我们不被允许创建一个对象(因为部分实现)，那么这种类型的类我们必须用 `abstract` 修饰符来声明。
