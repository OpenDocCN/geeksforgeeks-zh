...
> 原文: [https://www.geeksforgeeks.org/bigdecimal-plus-method-in-java/](https://www.geeksforgeeks.org/bigdecimal-plus-method-in-java/)

## `java.math.BigDecimal.plus()`

`java.math.BigDecimal.plus()` 是 Java 中的一个内置方法，它返回一个 `BigDecimal`，其值为 `(+this)`，其小数位数为 `this.scale()`。该方法简单地返回 `BigDecimal`，包含该方法是为了与一元减方法 `negate()` 保持对称。
...
## `java.math.BigDecimal.plus(MathContext mc)`

`java.math.BigDecimal.plus(MathContext mc)` 是 Java 中的一个内置方法，它返回一个 `BigDecimal`，其值为 `(+this)`，并根据上下文设置进行舍入。
...
该方法接受单个参数 `mc`，该参数是指要使用的舍入的上下文，即该值将被舍入到的位数。

### 返回值

该方法返回 `BigDecimal` Object 的值，必要时四舍五入。零结果的标度为 0。
