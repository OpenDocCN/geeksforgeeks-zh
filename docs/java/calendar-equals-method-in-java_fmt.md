...
> 原文: [https://www.geeksforgeeks.org/calendar-equals-method-in-java/](https://www.geeksforgeeks.org/calendar-equals-method-in-java/)

`java.util.Calendar.equals()` 是 `java.util` 包的 `Calendar` 类中的一个方法。方法将此日历与指定的对象进行比较。如果该对象等于 `Object`，则该方法返回 `true`。如果不是这样，即两个日历之间的参数有任何差异，则返回 `false`。
