...
> 原文: [https://www.geeksforgeeks.org/copyonwritearraylist-removeall-method-in-java-with-examples/](https://www.geeksforgeeks.org/copyonwritearraylist-removeall-method-in-java-with-examples/)

`CopyOnWriteArrayList` 类中的 `removeAll()` 方法，用于从调用的 `CopyOnArrayList` 对象中移除指定集合中包含的所有元素。
...
该方法只接受单个参数 `collection`，该集合将从调用对象中移除。

## 返回值

该方法返回一个 `boolean` 值。如果删除操作成功，则返回 `true`。
...
*   `NullPointerException`: 如果指定的集合为 `null`，或者如果此列表包含 `null` 元素，并且指定的集合不允许 `null` 元素。
