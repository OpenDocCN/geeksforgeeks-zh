...
*   `InflaterOutputStream(OutputStream out)`: 使用默认解压缩程序和缓冲区大小创建新的输出流。
*   `InflaterOutputStream(OutputStream out, Inflater input)`: 使用指定的解压缩程序和默认缓冲区大小创建新的输出流。
*   `InflaterOutputStream(OutputStream output, Inflater input, int internalBuffer)`: 使用指定的解压缩程序和缓冲区大小创建新的输出流。
