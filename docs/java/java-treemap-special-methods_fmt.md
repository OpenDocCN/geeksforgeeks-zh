...
**异常:**如果地图为空，该方法抛出`NoSuchElementException`。
...
`TreeMap`类的`java.util.TreeMap.headMap(key_point)`方法用于获取严格小于参数`key_value`的所有对或部分映射。上述参数不包括在新准备的树形图中。由于集合由地图支持，因此对地图的任何更改都会反映在其他地图中，反之亦然。
...
*   `ClassCastException`: 当`key_point`与地图的比较器不兼容或不可比较时，抛出此异常。
*   `NullPointerException`: 当`key_point`为空时，抛出此异常。
*   `IllegalArgumentException`: 当`key_point`超出范围或超出地图范围限制时，抛出此异常。
