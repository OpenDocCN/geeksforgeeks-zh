...
> 原文：[`https://www.geeksforgeeks.org/abstractcollection-addall-method-in-java-with-examples/`](https://www.geeksforgeeks.org/abstractcollection-addall-method-in-java-with-examples/)

[`AbstractCollection`](https://www.geeksforgeeks.org/abstractcollection-in-java-with-examples/) 的 `addAll(Collection C)` 方法用于将所述集合中的所有元素追加到*这个*集合中。元素是随机添加的，没有任何特定的顺序。
...
## 参数
参数 `C` 是要添加到集合中的任何类型的集合。

## 返回值
如果该方法成功地将集合 `C` 的元素追加到现有集合中，则该方法返回真，否则返回假。
