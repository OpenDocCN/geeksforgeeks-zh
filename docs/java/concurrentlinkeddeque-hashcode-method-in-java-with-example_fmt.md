...
> 原文：[https://www.geeksforgeeks.org/concurrentlinkeddeque-hashcode-method-in-java-with-example/](https://www.geeksforgeeks.org/concurrentlinkeddeque-hashcode-method-in-java-with-example/)

## 方法介绍

Java 中 `ConcurrentLinkedDeque` 的 `hashCode()` 方法用于获取这个 `ConcurrentLinkedDeque` 实例的 `hashCode` 值。它返回一个整数值，该整数值是这个 `ConcurrentLinkedDeque` 实例的哈希值。
...
该方法返回一个整数值，这是这个 `ConcurrentLinkedDeque` 实例的哈希值。
