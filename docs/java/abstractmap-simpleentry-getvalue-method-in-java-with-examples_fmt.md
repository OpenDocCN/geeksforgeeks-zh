# AbstractMap.SimpleEntry.getValue() 方法在 Java 中的示例

> 原文：[https://www.geeksforgeeks.org/abstractmap-simpleentry-getvalue-method-in-java-with-examples/](https://www.geeksforgeeks.org/abstractmap-simpleentry-getvalue-method-in-java-with-examples/)

`AbstractMap.SimpleEntry<K, V>` 用于维护一个键和值条目。可以使用 `setValue` 方法更改该值。这个类促进了构建自定义地图实现的过程。

`getValue()` 方法属于 `AbstractMap.SimpleEntry<K, V>`，返回该类中某个条目对应的值。
...
## 参考文献
[https://docs.oracle.com/javase/10/docs/api/java/util/AbstractMap.SimpleEntry.html#getValue()](https://docs.oracle.com/javase/10/docs/api/java/util/AbstractMap.SimpleEntry.html#getValue())