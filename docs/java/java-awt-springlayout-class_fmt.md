...
AWT (抽象窗口工具包) 中的一个 `SpringLayout` 类根据一组布局约束将子级布局到其关联的容器中。每个约束由一个弹簧对象表示，该对象控制两个组件边之间的垂直或水平距离。这些边可以属于容器的任何子级，也可以属于容器本身。默认情况下，`SpringLayout` 创建约束，使其关联组件具有最小、首选、最大和实际值。
...
*   `void addLayoutComponent(Component com, Object cons)`: 如果约束是 `SpringLayout.Constraints` 的一个实例，将约束与指定的组件相关联。
*   `getLayoutAlignmentX(Container c)`: 用于返回 0.5f (居中)。
*   `getLayoutAlignmentY(Container c)`: 用于返回 0.5f (居中)。
*   `getConstraint(String edgeName, Component c)`: 返回控制组件指定边缘与其父组件顶部或左侧边缘之间距离的弹簧。
*   `getConstraints(Component c)`: 返回指定组件的约束。
*   `layoutContainer(Container parent)`: 用于布局指定的容器。

下面的程序说明了 `SpringLayout` 类:

### Program 1

下面的程序在 `JFrame` 中排列组件。我们创建一个名为 `label` 的 `JLabel` 组件和一个名为 `textfield` 的 `JTextField`，并创建两个类，一个是 `JFrame` 类，另一个是 `SpringLayout` 类，然后通过 `add()` 方法将它们添加到 `JFrame` 中。我们使用 `setVisible()` 方法设置框架的可见性。布局通过 `setLayout()` 方法设置。
...
下面的程序在 `JFrame` 中排列组件。我们创建一个名为 `springlayout` 的类和四个名为 `b1`, `b2`, `b3`, `b4` 的 `JButton` 组件，然后通过 `add()` 方法将它们添加到 `JFrame` 中。我们使用 `setVisible()` 方法设置框架的可见性。布局通过 `setLayout()` 方法设置。
