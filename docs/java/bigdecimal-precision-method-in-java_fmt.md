# Java 中的 BigDecimal precision() 方法

> 原文: [https://www.geeksforgeeks.org/bigdecimal-precision-method-in-java/](https://www.geeksforgeeks.org/bigdecimal-precision-method-in-java/)

`BigDecimal.precision()` 方法返回这个 `BigDecimal` 的精度。精度是指未缩放值中的位数。
...
**返回值：** 该方法返回一个整数，表示这个 `BigDecimal` 对象的精度。
...
下面的程序说明了 `java.math.BigDecimal.precision()` 函数：
