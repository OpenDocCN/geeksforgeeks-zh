...
> 原文：[https://www.geeksforgeeks.org/factory-method-to-create-immutable-list-in-java-se-9/](https://www.geeksforgeeks.org/factory-method-to-create-immutable-list-in-java-se-9/)
...
在 Java SE 8 中，为了在 java 8 中创建不可变列表，我们使用了 `java.util.Collections.unmodifiableList(List list)` 方法。此方法返回指定列表的不可修改视图。该方法允许模块向用户提供对内部列表的“只读”访问。
...
Java SE 9 在 `List` 接口中引入了几个方法来创建不可变列表。为了在 Java 9 中创建不可变列表，我们使用了 `List.of()` 方法。
