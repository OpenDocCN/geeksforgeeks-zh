...
`LinkedHashMap` 类就像 `HashMap` 一样，有一个额外的特性，保持插入其中的元素的顺序。`HashMap` 提供了快速插入、搜索和删除的优势，但是它从来没有维护过 `LinkedHashMap` 提供的插入的轨迹和顺序，在那里元素可以按照它们的插入顺序被访问。
...
实现 `Map<K，V>` 接口，扩展 `HashMap <K，V>` 类。虽然链接哈希表的层次结构在以下媒体中描述为跟随:

![](img/bcb5be4b7476c8d0b485262d031fc896.png)

### LinkedHashMap 如何在内部工作？

`LinkedHashMap` 是 `HashMap` 类的扩展，它实现了 `Map` 接口。因此，该类被声明为:
...
在这个类中，数据以节点的形式存储。`LinkedHashMap` 的实现非常类似于双链表。因此，`LinkedHashMap` 的每个节点都表示为:

![LinkedHashMap-Node-in-Java](img/7ba24fe1556f31c23527a342c8a6697e.png)

*   **`hash`**: 所有的输入键都被转换成一个 Hash，Hash 是键的一种较短的形式，这样搜索和插入更快。
*   **`Key`**: 由于这个类扩展了 `HashMap`，数据以键值对的形式存储。因此，这个参数是数据的关键。
*   **`value`**: 每个键都有一个与之关联的值。此参数存储键的值。由于泛型，这个值可以是任何形式。
*   **`next`**: 由于 `LinkedHashMap` 存储了插入顺序，因此它包含到 `LinkedHashMap` 的下一个节点的地址。
*   **`Previous`**: 该参数包含 `LinkedHashMap` 的前一个节点的地址。

### 同步 LinkedHashMap

`LinkedHashMap` 的实现不同步。如果多个线程同时访问链接的哈希映射，并且至少有一个线程在结构上修改了该映射，则必须在外部同步该映射。这通常是通过同步一些自然封装地图的对象来实现的。如果不存在这样的对象，应该使用 `Collections.synchronizedMap` 方法“包装”地图。这最好在创建时完成，以防止对地图的意外非同步访问:
...
**3. `LinkedHashMap(Map<? extends K，? extends V> map)`:** 它用于用指定地图的元素初始化特定的链接散列表。
...
这里，对于 `Order` 属性，对于最后一个访问顺序传递 `true`，对于插入顺序传递 `false`。
...
| `removeEldestEntry(Map.Entry<K，V> eldest)` | 如果此地图应删除其最早的条目，则返回 `true`。 |
| `values()` | 返回此地图中包含的值的集合视图。 |

> **应用:** 由于 `LinkedHashMap` 使用了 Doubly LinkedList 来维护插入顺序，因此我们可以通过覆盖 `removeEldestEntry()` 方法来实现 LRU 缓存功能，以便在向映射中添加新映射时强制实施自动移除过时映射的策略。这允许您使用自己定义的一些标准来终止数据。
