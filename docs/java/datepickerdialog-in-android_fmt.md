...
安卓**日期选择器**是一个用户界面控件，用于在[安卓应用](https://www.geeksforgeeks.org/android-app-development-fundamentals-for-beginners/)中按日、月、年选择日期。日期选择器用于确保用户选择有效的日期。在具有两种模式的安卓日期选择器中，第一种显示完整的日历，第二种显示微调视图中的日期。您可以通过两种方式创建日期选择器控件，要么在 XML 文件中手动创建，要么以编程方式在活动文件中创建。我们将使用 [Java](https://www.geeksforgeeks.org/java/) 通过编程来实现。
...
在 `activity_main.xml` 文件中，仅添加一个[文本视图](https://www.geeksforgeeks.org/textview-widget-in-android-using-java-with-examples/)以显示所选日期，并添加一个[按钮](https://www.geeksforgeeks.org/button-in-kotlin/)以从日期选择日志中选择日期。下面是 `activity_main.xml` 文件的完整代码。
...
现在创建一个新的类，方法是转到该包，右键单击它，选择新建，然后选择 Java 类。将类命名为 `DatePicker`，将其超类命名为 `DialogFragment` (`androidx.fragment.app.DialogFragment`)，然后单击确定。
...
*   对于年、月和日月，创建一个 `Calendar` 类的实例，并将年、月和日月分配给 `int` 类型的变量。
...
实现 `DatePickerDialog` 类的 `OnDateSetListener`，并覆盖 `onDateSet()` 方法。`onDateSet()` 方法将在 `tvDate` 文本视图中设置选择后的日期。
...
在 `onClick()` 方法中实现设置 `btPickDate` 的。创建一个日期选择器的实例（我们的类）。使用实例的 `show` 方法，并传递 `getSupportFragmentManager()` 和一个 `tag`。`MainActivity.java` 文件的完整代码如下。
