...
> 原文：[https://www.geeksforgeeks.org/how-to-use-checkbox-in-android/](https://www.geeksforgeeks.org/how-to-use-checkbox-in-android/)
...
*   `public boolean isChecked()`: 如果 `CheckBox` 处于选中状态，则返回 `true`，否则返回 `false`。
*   `public void setChecked(boolean state)`: 它改变复选框的状态。
...
`activity_main.xml` 有一个 `TextView`、4 个 `CheckBoxes` 和一个按钮。文本视图提示用户选择他/她的爱好。首先用户选择其选项，然后按下提交按钮。按下提交按钮后，将生成一个显示所选爱好的吐司。
