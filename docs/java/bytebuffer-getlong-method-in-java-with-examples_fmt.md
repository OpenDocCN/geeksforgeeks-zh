...
### 抛出:
这个方法抛出 `BufferUnderflowException` – 如果这个缓冲区剩余的字节少于 8 个。
...
# `ByteBuffer`的`getLong(int index)`方法

`ByteBuffer`的`getLong(int index)`方法用于读取给定索引处的四个字节，根据当前字节顺序将它们组成一个浮点值。
...
[https://docs.oracle.com/javase/9/docs/api/java/nio/ByteBuffer.html#getLong-int-](https://docs.oracle.com/javase/9/docs/api/java/nio/ByteBuffer.html#getLong-int-)