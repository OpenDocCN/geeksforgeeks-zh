...
`MotionBlur` 是 JavaFX 的一部分。运动模糊类使用高斯卷积核实现运动模糊效果，具有可配置的半径和角度。运动模糊类继承 `Effect` 类。

## 类的构造函数

1.  `MotionBlur()`: 创建一个运动模糊的新对象。
2.  `MotionBlur(double a, double r)`: 用指定的角度和半径创建一个新的运动模糊对象。
...
| `getRadius()` | 返回运动模糊对象的半径 |
| `getInput()` | 返回运动模糊对象的输入 |
| `setAngle(double v)` | 设置运动模糊对象的角度 |
| `setRadius(double v)` | 设置运动模糊对象的半径 |
| `setInput(Effect v)` | 设置运动模糊对象的输入 |

下面的程序说明了运动模糊类的使用:

### 1. 导入图像并添加运动模糊效果

在这个程序中，创建了一个 `FileInputStream`，并从文件中获取图像输入。使用文件输入流的输入创建名为 `image` 的 `Image`。从该图像创建一个 `ImageView` 对象，并将其添加到 `VBox` 中。然后将 `VBox` 添加到场景，再将场景添加到舞台。创建一个指定参数级别的 `MotionBlur` 效果，并使用 `setEffect()` 函数将效果设置到图像视图上。
...
在这个程序中，创建了一个 `FileInputStream`，并从文件中获取图像输入。使用文件输入流的输入创建名为 `image` 的 `Image`。从该图像创建一个 `ImageView` 对象，并将其添加到 `VBox` 中。然后将 `VBox` 添加到场景，再将场景添加到舞台。创建一个指定参数级别的 `MotionBlur` 效果，并使用 `setEffect()` 函数将效果设置到图像视图上。使用 `setRadius()` 和 `setAngle()` 函数指定运动模糊的半径和角度。
...
**参考:** [https://docs.oracle.com/javase/8/javafx/api/javafx/scene/effect/MotionBlur.html](https://docs.oracle.com/javase/8/javafx/api/javafx/scene/effect/MotionBlur.html)