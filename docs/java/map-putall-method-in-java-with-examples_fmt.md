...
> 原文：[https://www.geeksforgeeks.org/map-putall-method-in-java-with-examples/](https://www.geeksforgeeks.org/map-putall-method-in-java-with-examples/)
...
这个方法只有一个参数映射 `m`，它包含要复制到给定映射的键值映射。

## 返回

该方法返回与该键相关的前一个值（如果存在），否则返回 `-1`。
