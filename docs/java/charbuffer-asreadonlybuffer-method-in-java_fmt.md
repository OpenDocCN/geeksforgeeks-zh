...
> 原文: [https://www.geeksforgeeks.org/charbuffer-asreadonlybuffer-method-in-java/](https://www.geeksforgeeks.org/charbuffer-asreadonlybuffer-method-in-java/)

`java.nio.CharBuffer` 类的 `asReadOnlyBuffer()` 方法用于用该缓冲区的内容创建一个新的**只读字符缓冲区**。新缓冲区是该缓冲区的副本。因此，对此缓冲区内容所做的更改将在新缓冲区中可见。
