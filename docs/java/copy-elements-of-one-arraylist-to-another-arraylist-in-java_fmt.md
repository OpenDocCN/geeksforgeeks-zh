...
> 原文：[https://www.geeksforgeeks.org/copy-elements-of-one-arraylist-to-another-arraylist-in-java/](https://www.geeksforgeeks.org/copy-elements-of-one-arraylist-to-another-arraylist-in-java/)

它是`List`接口的实现类。它允许复制对象/元素，并保持插入顺序。您可以通过数组列表的索引来获取数组列表中的元素，现在您需要将它传递给`get(index)`方法。您可以使用`add()`方法将元素添加到`ArrayList`中。
