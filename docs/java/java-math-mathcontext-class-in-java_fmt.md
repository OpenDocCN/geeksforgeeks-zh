...
> 原文: [https://www.geeksforgeeks.org/java-math-mathcontext-class-in-java/](https://www.geeksforgeeks.org/java-math-mathcontext-class-in-java/)

`java.math.MathContext`类提供了不可变的对象，这些对象封装了上下文设置并定义了那些数字运算符规则，例如`BigDecimal`类实现的规则。
...
> 有效位数由精度指定。舍入的默认模式是`HALF_UP`模式，这将在后面介绍。
...
*   `MathContext(int setPrecision, RoundingMode setRoundingMode)`: 此构造函数构造一个具有指定精度和舍入模式的新 math context。
...
| 方法 | 描述 |
| --- | --- |
| `equals(Object x)` | This method compares this MathContext with the specified Object to see if it is equal. |
