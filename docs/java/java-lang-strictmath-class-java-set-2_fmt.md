...
> 原文：[`https://www.geeksforgeeks.org/java-lang-strictmath-class-java-set-2/`](https://www.geeksforgeeks.org/java-lang-strictmath-class-java-set-2/)
...
### 13. `exp()`
`Java.lang.StrictMath.exp(double arg)` 方法返回欧拉数的双参数幂。
...
### 14. `cosh()`
`Java.lang.StrictMath.cosh()` 方法返回传递的参数的双曲余弦值。
...
### 15. `decrementExact()`
`java.lang.StrictMath.decrementExact()` 方法将传递的参数值递减 1。
...
**Java 代码，解释 `lang.StrictMath` 中的 `exp()`、`decrementExact()`、`cosh()` 方法。**
...
### 16. `log10()`
`Java.lang.StrictMath.log10()` 方法返回传递的参数的 base10 对数值。
...
### 17. `pow()`
`Java.lang.StrictMath.pow(double b, double e)` 方法返回值为 `b<sup>e</sup>`。
...
### 18. `incrementExact()`
`Java.lang.StrictMath.incrementExact()` 方法通过增加参数的值来返回参数。
...
**JAVA 代码，用 `lang.StrictMath` 解释 `incrementExact()`、`log10()`、`pow()` 方法。**
...
### 19. `signum()`
`Java.lang.StrictMath.signum()` 方法返回传递的参数的 signum 值。
...
### 20. `max()`
`Java.lang.StrictMath.max(double v1, double v2)` 方法返回两个传递的参数值中较大的值。
...
### 21. `round()`
`Java.lang.StrictMath.round()` 方法将传递的参数四舍五入到最接近的小数位。
...
**Java 代码解释 `lang.StrictMath` 中 `signum()`、`round()`、`max()` 方法。**
...
### 23. `log1p()`
`Java.lang.StrictMath.log1p()` 方法返回(传递的参数+ 1)的自然对数。
...
**Java 代码，解释 `lang.StrictMath` 中的 `ulp()`、`log1p()` 方法。**
