...
在 `org.javatuples` 包中，`getKey()` 方法用于从 `KeyValue` 类的 TupleClassObject 中获取键。此方法只能用于 javatuples 库的 `KeyValue` 类对象。它返回一个键，该键是出现在键值类对象的索引 0 处的元素。返回的键确保类型安全。
...
该方法返回一个键，该键是出现在 `KeyValueClassObject` 的索引 0 处的元素。
