...
> 原文：[https://www.geeksforgeeks.org/how-to-fix-java-lang-classcastexception-in-treeset-by-using-custom-comparator-in-java/](https://www.geeksforgeeks.org/how-to-fix-java-lang-classcastexception-in-treeset-by-using-custom-comparator-in-java/)
...
在像 `Integer`、`Double` 等包装类中，`Comparator` 接口已经实现，所以我们不需要担心。但是，每当我们在 `TreeSet` 中使用用户定义的类时，我们都必须用上述方法之一。

## 方法：通过在 `TreeSet` 构造函数中提供自定义比较器

与 `Comparable` 不同，`Comparator` 位于我们正在比较的元素类型的外部。这是一个单独的类。我们创建多个单独的类（实现 `Comparator`）来由不同的成员进行比较。

- 创建一个实现 `Comparator` 的类（从而创建 `compare()` 方法，该方法执行 `compareTo()` 以前完成的工作）。
- 创建比较器类的实例。
