...
> 原文：[https://www.geeksforgeeks.org/android-auto-complete-textbox-and-how-to-create-it/](https://www.geeksforgeeks.org/android-auto-complete-textbox-and-how-to-create-it/)
...
*   [安卓从第一个 app/安卓项目开始](https://www.geeksforgeeks.org/android-starting-with-first-app-android-project/)
*   [安卓运行你的第一个安卓应用](https://www.geeksforgeeks.org/android-running-your-first-android-app/)

[安卓](https://www.geeksforgeeks.org/android-app-development-fundamentals-for-beginners/) `AutoCompleteTextView` 用于自动完成单词。每当用户书写一个字符时，`AutoCompleteTextView` 将显示用户可以选择的单词的建议。`AutoCompleteTextView` 是 `EditText` 的子类。

**例如：**

![](img/e0a5e299f3c7bc8b37c5f9be8184636c.png)

`AutoCompleteTextView` 提供以下方法：
...
*   显示信息的 `TextView`
*   一个 `AutoCompleteTextView` 借助自动完成建议获取用户答案。

此外，将 `id` 以及其他属性分配给每个组件，如下图和代码所示。组件上分配的 `id` 有助于在 Java 文件中轻松找到和使用该组件。
...
**第三步：** 现在，在 UI 之后，这一步将创建 App 的后端。为此，请打开 `MainActivity.java` 文件，并使用 `findViewById()` 方法实例化在 XML 文件 (`AutoCompleteTextView`) 中制作的组件。此方法借助于分配的 `id` 将创建的对象绑定到用户界面组件。
**通用语法：**

> `组件类型 对象 = (组件类型) findViewById(R.id.id_of_the_component);`
