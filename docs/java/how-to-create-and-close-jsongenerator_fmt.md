...
> 原文：[https://www.geeksforgeeks.org/how-to-create-and-close-jsongenerator/](https://www.geeksforgeeks.org/how-to-create-and-close-jsongenerator/)
...
`createGenerator` 将我们希望写入 `JSON` 的目标作为第一个参数。如果我们使用字节流作为第一个参数，我们还可以选择指定字符集：
...
最后，我们调用生成器的 `close()` 方法。此方法通过关闭相应的输出流来释放资源。