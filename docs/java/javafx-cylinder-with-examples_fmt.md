...
> 原文:[https://www.geeksforgeeks.org/javafx-cylinder-with-examples/](https://www.geeksforgeeks.org/javafx-cylinder-with-examples/)

柱面是 JavaFX 的一部分。`Cylinder`类用于创建指定高度和半径的三维圆柱体。圆柱体以原点为中心。

### 该类的构造函数为

1.  `Cylinder()`:创建半径为 1.0、高度为 2.0 的圆柱体的新实例。
2.  `Cylinder(double r, double h)`:创建给定半径和高度的圆柱体的新实例。
3.  `Cylinder(double r, double h, int div)`:创建给定半径、高度和分割的圆柱体的新实例。
