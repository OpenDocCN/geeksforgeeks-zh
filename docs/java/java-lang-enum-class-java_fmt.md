...
2.  **`final int ordinal()`**：这个方法返回这个枚举常量的索引。
...
4.  **`final boolean equals(Object obj)`**：如果指定的对象等于这个枚举常量，这个方法返回 `true`，否则返回 `false`。
...
5.  **`final int hashCode()`**：此方法返回此枚举常量的哈希代码。实际上这个方法只包含一个语句，那就是 `return super.hashCode()`，它依次调用 `Object` 类 `hashCode()` 方法。
...
## 2. `final int compareTo(E obj)`

此方法将此枚举与订单的指定对象进行“比较”。枚举常量只能与相同枚举类型的其他枚举常量相比较。
...
## 3. `static <T extends Enum> T valueOf(Class enumType, String name)`

此方法返回具有指定名称的指定枚举类型的枚举常量。该名称必须与用于声明此类型的枚举常量的标识符完全匹配。
...
## 4. `final Class<E> getDeclaringClass()`

该方法返回该枚举常量的枚举类型对应的 `Class` 对象。如果此方法为两者返回相同的类对象，则任意两个枚举常数 `e1` 和 `e2` 属于相同的枚举类型。
...
## 5. `final Object clone()`

这种方法保证了枚举永远不会被克隆，这是保持其“单例”状态所必需的。编译器在内部使用它来创建枚举常量。
...
## 6. `final void finalize()`

此方法保证枚举类不能有 `finalize` 方法。
