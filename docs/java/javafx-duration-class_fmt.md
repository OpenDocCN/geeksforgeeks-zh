...
`Duration`类是JavaFX的一部分，用于定义一段时间。该类是不可变的，因此其对象是被替换而非修改。

## 创建方式

创建`Duration`对象有两种方法：
1.  使用构造器。
2.  使用静态构建方法之一，如`seconds(double)`或`minutes(double)`。

## 构造函数

*   `Duration(double m)`：以毫秒为单位创建一个指定持续时间的新`Duration`对象。

## 常用方法

| 方法 | 说明 |
| --- | --- |
| `add(Duration d)` | 将持续时间`d`添加到此`Duration`对象 |
| `subtract(Duration d)` | 从此`Duration`对象中减去持续时间`d` |
| `divide(Duration d)` | 将此`Duration`对象除以持续时间`d` |
| `multiply(Duration d)` | 将此`Duration`对象乘以持续时间`d` |
| `millis(double m)` | 以毫秒为单位返回指定持续时间的`Duration`对象 |
| `seconds(double m)` | 以秒为单位返回指定持续时间的`Duration`对象 |
| `minutes(double m)` | 以分钟为单位返回指定持续时间的`Duration`对象 |
| `hours(double m)` | 以小时为单位返回指定持续时间的`Duration`对象 |
| `toMillis()` | 返回此持续时间内的毫秒数 |
| `toSeconds()` | 返回此持续时间的秒数 |
| `toMinutes()` | 返回此持续时间的分钟数 |
| `toHours()` | 返回此持续时间的小时数 |
| `lessThan(Duration d)` | 如果指定的持续时间小于此实例的持续时间，则返回`true` |
| `greaterThan(Duration d)` | 如果指定的持续时间大于此实例的持续时间，则返回`true` |
| `greaterThanOrEqualTo(Duration d)` | 如果指定的持续时间大于或等于此实例的持续时间，则返回`true` |
| `lessThanOrEqualTo(Duration d)` | 如果指定的持续时间小于或等于此实例的持续时间，则返回`true` |

## 示例程序

### 示例1：创建并显示Duration对象

下面的程序创建两个`Duration`对象，并以毫秒、秒、分、小时为单位显示时间。程序创建两个`Duration`对象，命名为`duration_1`和`duration_2`。毫秒数作为参数传递。调用`display`函数，并将`Duration`对象作为参数传递。使用`toHours()`、`toMinutes()`、`toSeconds()`和`toMillis()`方法将持续时间转换为小时、分钟、秒和毫秒，并显示结果。
...
下面的程序创建四个`Duration`对象并显示它们，然后对它们进行加、乘、除、减运算并显示结果。程序创建四个`Duration`对象，命名为`duration_1`、`duration_2`、`duration_3`和`duration_4`。调用`display`方法，将`Duration`对象作为参数传递。使用`toHours()`、`toMinutes()`、`toSeconds()`和`toMillis()`方法将持续时间转换为小时、分钟、秒和毫秒，并显示结果。将`duration_1`和`duration_2`相加和相减，并显示结果对象。类似地，`duration_3`和`duration_4`被除和乘，结果对象被显示。
