...
> 原文: [https://www.geeksforgeeks.org/gfact-48-overloading-main-in-java/](https://www.geeksforgeeks.org/gfact-48-overloading-main-in-java/)
...
`要点:`
Java 的 `main()` 方法是静态的。除了 `main()` 就像任何其他方法一样可以以类似的方式重载之外，JVM 总是寻找特定的方法签名来启动程序。

*   正常的 `main()` 方法充当 JVM 开始执行程序的入口点。
*   我们可以在 Java 中重载 `main()` 方法。但是当我们运行程序时，JVM 并不会直接执行重载的 `main()` 方法，我们只需要从实际的 `main()` 方法中调用重载的 `main()` 方法。
