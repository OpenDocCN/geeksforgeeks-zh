...
> 原文：[https://www.geeksforgeeks.org/angle-bracket-in-java-with-examples/](https://www.geeksforgeeks.org/angle-bracket-in-java-with-examples/)
...
我们使用尖括号 `<>` 来指定泛型函数定义中的参数类型。然后为了调用这个函数，我们只需要传递期望的类型作为参数。我们还可以根据传递给泛型方法的参数类型，编写可以用不同类型的参数调用的泛型函数，编译器处理每个方法。

```java
// To create a generic function
public static <T> void func(T a, T b){}
