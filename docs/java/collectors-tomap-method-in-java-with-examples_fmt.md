...
> 原文：[https://www.geeksforgeeks.org/collectors-tomap-method-in-java-with-examples/](https://www.geeksforgeeks.org/collectors-tomap-method-in-java-with-examples/)
...
*   `T`：输入元素的类型。
*   `Map`：输出地图。
*   `Collector` 接口：将输入元素累积到可变结果容器中的可变约简操作，可选地在所有输入元素处理完毕后将累积的结果转换为最终表示。还原操作可以顺序执行，也可以并行执行。
*   `toMap()`：`Collectors` 类的静态方法，返回一个 `Collector`，它将元素收集到一个 `Map` 中，`Map` 的键和值是将映射函数应用于输入元素的结果。`Collectors` 类在 `java.util.stream` 包下。
...
**返回值：** 该方法返回一个 `Collector`，该 `Collector` 将元素收集到一个 `Map` 中，该 `Map` 的键和值是对输入元素应用映射函数的结果。
...
*   `keyMapper`：产生按键的映射功能
*   `valueMapper`：产生值的映射函数
*   `mergeFunction`：一个合并函数，用于解决与同一个键相关的值之间的冲突，如提供给 `Map.merge(Object, Object, BiFunction)`
...
## 函数键映射器，函数值映射器，二进制运算符（合并函数，供应商映射供应商）
...
*   `valueMapper`：产生值的映射函数
*   `mergeFunction`：一个合并函数，用于解决与同一个键相关的值之间的冲突，如提供给 `Map.merge(Object, Object, BiFunction)`
*   `mapSupplier`：返回一个新的空地图的函数，结果将被插入其中
