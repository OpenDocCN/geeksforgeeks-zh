...
大多数时候，在 java 中处理数字时，我们使用[原始数据类型](https://www.geeksforgeeks.org/data-types-in-java/)。但是，在 `java.lang` 包中的抽象类 `Number` 下，Java 还提供了各种数值[包装](https://www.geeksforgeeks.org/primitive-wrapper-classes-are-immutable-in-java/)子类。`Number` 类下主要有六个子类。这些子类定义了一些在处理数字时经常使用的有用方法。
...
**为什么在原始数据上使用 `Number` 类对象？**

*   由数字类定义的常数，如提供数据类型上限和下限的 `MIN_VALUE` 和 `MAX_VALUE`，非常有用。
*   `Number` 类对象可以用作需要对象的方法的参数（通常在操作数字集合时使用）。
*   类方法可用于将值转换成其他基元类型，转换成字符串，以及在数字系统（十进制、八进制、十六进制、二进制）之间进行转换。

## 所有数字子类通用的方法

1.  **`xxx xxxValue()`**：这里 `xxx` 代表基元数数据类型（`byte`、`short`、`int`、`long`、`float`、`double`）。该方法用于将[这个](https://www.geeksforgeeks.org/this-reference-in-java/)数字对象的值转换为指定的原始数据类型。
...
2.  **`int compareTo(Number subclass referenceName)`**：此方法用于将[这个](https://www.geeksforgeeks.org/this-reference-in-java/) `Number` 对象与指定的参数进行比较。但是，不能比较两种不同的类型，因此调用该方法的参数和 `Number` 对象应该属于同一类型。`referenceName` 可以是 `byte`、`double`、`integer`、`float`、`long` 或 `short`。
...
3.  **`boolean equals(Object obj)`**：此方法判断[这个](https://www.geeksforgeeks.org/this-reference-in-java/) `Number` 对象是否等于自变量。
...
4.  **`int parseInt(String s, int radix)`**：此方法用于获取[字符串](https://www.geeksforgeeks.org/string-class-in-java/)的原始数据类型。`radix` 用于返回十进制（10）、八进制（8）或十六进制（16）等表示作为输出。
...
5.  **`int parseInt(String s)`**：该方法是上述方法的另一个变体，默认情况下 `radix` 为 10（十进制）。
...
*   `String toString()`: `toString()`方法有两种变体。它们用于获取数字的字符串表示形式。这些方法的其他变体是`Integer.toBinaryString(int i)`、`Integer.toHexString(int i)`、`Integer.toOctalString(int i)`，它们将分别返回指定整数(i)的二进制、十六进制、八进制字符串表示。
...
`整数值 of()`: `valueOf()`方法有三种变体。所有这三种方法都返回一个包含原始整数值的整数对象。
...
radix - any radix value
Returns :
valueOf(int i) : an Integer object holding the value represented by the int argument.
