...
> 原文：[https://www.geeksforgeeks.org/check-whether-two-strings-are-anagram-of-each-other-using-hashmap-in-java/](https://www.geeksforgeeks.org/check-whether-two-strings-are-anagram-of-each-other-using-hashmap-in-java/)
...
`HashMap`也可以通过将每个字符串的字符映射到单个`HashMap`并将它们进行比较来发现任意两个给定的字符串是否是字谜。
