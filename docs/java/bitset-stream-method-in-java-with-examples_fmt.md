# Java 中的 BitSet stream() 方法示例

> 原文：[https://www.geeksforgeeks.org/bitset-stream-method-in-java-with-examples/](https://www.geeksforgeeks.org/bitset-stream-method-in-java-with-examples/)

Java `BitSet` 类的 `stream()` 方法用于为 `BitSet` 中包含的每个位返回一个索引流。索引以递增的顺序返回。流的大小是 `BitSet` 的 set 状态的位数，等于 [`cardinality()`](https://www.geeksforgeeks.org/java-bitset-cardinality/) 方法返回的值。
