...
> 原文：[https://www.geeksforgeeks.org/java-program-to-use-method-overloading-for-printing-different-types-of-array/](https://www.geeksforgeeks.org/java-program-to-use-method-overloading-for-printing-different-types-of-array/)

在[Java方法重载](https://www.geeksforgeeks.org/overloading-in-java/)中，可以定义为包含多个同名方法的类，但参数列表或参数类型或方法的参数顺序不应相同。我们可以使用Java中的方法重载打印不同类型的数组，方法是确保该方法包含具有相同方法名的不同参数。

让我们用三次重载的`printArray()`方法实现一个程序，以打印不同类型的数组。而且每次函数都有不同的参数。在`main`方法中，为每个数组提供所需的输入，然后通过调用它们各自的方法将数组打印在屏幕上。
