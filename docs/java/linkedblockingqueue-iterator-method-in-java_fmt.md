# Java 中的 `LinkedBlockingQueue` `iterator()` 方法
...
`LinkedBlockingQueue` 的 `iterator()` 方法以适当的顺序返回与这个 `LinkedBlockingQueue` 相同元素的迭代器。从这个方法返回的元素包含了从 `LinkedBlockingQueue` 的第一个（头）到最后一个（尾）的所有元素。返回的迭代器弱一致。
...
下面的程序说明了 `LinkedBlockingQueue` 类的 `iterator()` 方法：

### 程序 1

从 `LinkedBlockingQueue` 创建一个迭代器，其中包含一个类的不同学生的名字。
...
从包含员工列表的 `LinkedBlockingQueue` 创建迭代器。
...
**参考：** [https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/LinkedBlockingQueue.html#iterator--](https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/LinkedBlockingQueue.html#iterator--)