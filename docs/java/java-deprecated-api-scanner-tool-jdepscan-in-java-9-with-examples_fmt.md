...
> 原文: [https://www.geeksforgeeks.org/java-deprecated-api-scanner-tool-jdepscan-in-java-9-with-examples/](https://www.geeksforgeeks.org/java-deprecated-api-scanner-tool-jdepscan-in-java-9-with-examples/)

## Java 弃用 API Scanner 工具

Java 弃用 API Scanner 工具即 `jdeprscan` 是一款静态分析命令行工具，在 JDK 9 中推出，用于找出弃用 API 在给定输入中的用途。这里的输入可以是类文件名、目录或 JAR 文件。每当我们向 `jdeprscan` 命令行工具提供任何输入时，它都会生成系统控制台的依赖项。`jdeprscan` 引入了影响输出的各种选项。根据选项，`jdeprscan` 命令行工具生成输出。`jdeprscan` 工具识别由 Java SE 不推荐使用的 API 定义的不推荐使用的 API，但它不会列出第三方库使用的不推荐使用的 API。
...
### 1. `–release 6|7|8|9`
它将根据给定的版本列出给定参数中弃用 API 的使用情况。假设我们正在使用 JDK 9，并且我们想列出 Java 7 版本中的弃用 API，那么我们可以使用此选项。
...
### 6. `–list`
它将列出弃用的 API。除了使用 `--list`，我们也可以使用 `-l`。
