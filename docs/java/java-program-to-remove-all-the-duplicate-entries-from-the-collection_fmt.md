...
> 原文：[https://www.geeksforgeeks.org/java-program-to-remove-all-the-duplicate-entries-from-the-collection/](https://www.geeksforgeeks.org/java-program-to-remove-all-the-duplicate-entries-from-the-collection/)
...
**注意：** 由于我们使用的是 `HashSet`，所以插入顺序不会被保留，每次运行代码时，我们都会得到一些不同的输出（元素的顺序会有所不同）。所以如果我们想在插入时保持元素的顺序，那么我们应该使用 [`LinkedHashSet`](https://www.geeksforgeeks.org/linkedhashset-in-java-with-examples/)。
