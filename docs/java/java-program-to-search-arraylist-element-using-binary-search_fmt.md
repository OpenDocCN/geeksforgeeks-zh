...
> 原文：[https://www.geeksforgeeks.org/java-program-to-search-arraylist-element-using-binary-search/](https://www.geeksforgeeks.org/java-program-to-search-arraylist-element-using-binary-search/)

线性搜索可用于数据结构特定数据结构的排序和非排序元素，但平均案例时间复杂度为 `O(n)`。然而，由于二分搜索法只能在项目按排序顺序排列并且平均情况时间复杂度为 `O(logn)` 并且两个横向都具有最佳情况时间复杂度为 `O(1)` 时实现。现在，给定一个包含排序元素的数组列表，检查该元素是否存在于数组列表中。
...
使用[二分搜索法](https://www.geeksforgeeks.org/binary-search/)因为列表是按顺序排序的，与线性搜索相比，二分搜索法的平均时间复杂度更低，即 `O(logn)`。
...
假设为了找到最大元素的最大索引小于使用二分搜索法的数组列表的排序重复元素中的键。
