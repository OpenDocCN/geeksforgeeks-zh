...
> 原文：[`https://www.geeksforgeeks.org/longadder-sum-method-in-java-with-examples/`](https://www.geeksforgeeks.org/longadder-sum-method-in-java-with-examples/)

`Java` 中的 `LongAdder` 类创建一个初始和为零的新加法器。`LongAdder.sum()` 是 `Java` 中一个返回总和的内置方法。
