...
该函数接受单个参数 `ch` ，该参数是强制的。它指定要测试的字符。参数可以是 `char` 或 `int` 数据类型。

## 返回值:

函数返回一个布尔值。如果字符是国际标准化组织控制字符，布尔值为真，否则为假。
...
[https://docs.oracle.com/javase/7/docs/api/java/lang/Character.html#isISOControl(char)](https://docs.oracle.com/javase/7/docs/api/java/lang/Character.html#isISOControl(char))