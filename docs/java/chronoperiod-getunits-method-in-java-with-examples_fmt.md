# Java 中的 ChronoPeriod.getUnits() 方法示例

> 原文：[https://www.geeksforgeeks.org/chronoperiod-getunits-method-in-java-with-examples/](https://www.geeksforgeeks.org/chronoperiod-getunits-method-in-java-with-examples/)

`ChronoPeriod` 类的 `getUnits()` 方法用来获取该计时周期支持的单位集。支持的单位是列表中的年、月、日（仅按此顺序）。
