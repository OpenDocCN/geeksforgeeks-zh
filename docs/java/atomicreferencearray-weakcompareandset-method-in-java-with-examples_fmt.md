...
`AtomicReferenceArray`类的`weakCompareAndSet()`方法用于将索引`i`处的元素的值自动设置为新值，如果当前值等于作为参数传递的预期值，则设置为原子引用数组的新值。这个方法使用内存读取语义更新索引`i`处的值，就像变量被声明为可变类型的变量一样。如果向原子引用设置新值成功，则此方法返回`true`。
...
如果给`AtomicReference`设置新值成功，该方法返回`true`。
...
[https://docs.oracle.com/javase/10/docs/api/java/util/concurrent/atomic/AtomicReferenceArray.html#weakCompareAndSet(V, V)](https://docs.oracle.com/javase/10/docs/api/java/util/concurrent/atomic/AtomicReferenceArray.html#weakCompareAndSet(V, V))