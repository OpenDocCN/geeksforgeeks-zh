# Java 中的 `isTransient(int mod)` 方法及示例

> 原文：[https://www.geeksforgeeks.org/modifier-istransientmod-method-in-java-with-examples/](https://www.geeksforgeeks.org/modifier-istransientmod-method-in-java-with-examples/)

`Java.lang.reflect.Modifier` 的 `isTransient(int mod)` 方法用于检查整数参数是否包含瞬态修饰符。如果此整数参数表示瞬态类型修饰符，则方法返回 `true`，否则返回 `false`。
...
## 参数
该方法接受一个整数参数 `mod`，表示一组修饰符。

## 返回值
如果 `mod` 包含瞬态修饰符，则该方法返回 `true`，否则返回 `false`。
