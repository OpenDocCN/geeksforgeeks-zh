...
> 原文：[https://www.geeksforgeeks.org/how-to-create-google-lens-application-in-android/](https://www.geeksforgeeks.org/how-to-create-google-lens-application-in-android/)
...
导航到 `应用程序 > java > 你的应用程序的包名 > 右键单击它 > 新建 > Java 类` 并将你的类命名为 `DataModal`，并在其中添加下面的代码。在代码中添加注释，以更详细地理解代码。
...
### 第 8 步：为我们的回收视图创建一个适配器类

导航到`应用程序 > java > 你的应用程序的包名`，右键单击它，选择`新建 > Java 类`，并将其命名为`search resultsrvaadapter`，然后添加下面的代码。
...
去网站 [https://serpapi.com/search-api](https://serpapi.com/search-api) 用你的谷歌账号创建你的账号。这是一个类似的过程，因为你注册极客的极客。生成`API`密钥时，请确保选择免费试用选项并继续。进入上面显示的网站后，您将看到下面的屏幕。只需使用您的凭据登录，然后继续。

![](img/0816cea52daf0f4652de6ac386674a77.png)

继续操作后，您只需导航至`我的账户 > 仪表盘选项`即可打开以下屏幕。在此屏幕上，您将看到您的`API`密钥。
