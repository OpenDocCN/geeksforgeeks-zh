...
> 原文：[https://www.geeksforgeeks.org/fragment-lifecycle-in-android/](https://www.geeksforgeeks.org/fragment-lifecycle-in-android/)
...
> `<fragment>` 标签用于在安卓活动布局中插入片段。通过划分活动的布局，可以在其中添加多个片段。
...
| `onDetach()` | 系统执行此方法来解除片段与其主机活动的关联。 |
...
2.  **动态地：** `FragmentManager` 用于在片段中嵌入能够在运行时添加、删除或替换片段的活动。
...
> `<fragment>` 元素下的 `android:name` 标签包含活动打开时显示的默认片段的文件名。
