...
> 原文: [https://www.geeksforgeeks.org/linkedlist-addlast-method-in-java/](https://www.geeksforgeeks.org/linkedlist-addlast-method-in-java/)

`java.util.LinkedList.addLast()`方法用于在 `LinkedList` 的末尾插入特定的元素。
...
该函数接受单个参数 `element`，如上语法所示。由该参数指定的元素被附加在列表的末尾。
