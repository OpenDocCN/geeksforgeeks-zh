...
> 原文: [https://www.geeksforgeeks.org/bufferedreader-marksupported-method-in-java-with-examples/](https://www.geeksforgeeks.org/bufferedreader-marksupported-method-in-java-with-examples/)

Java 中 `BufferedReader` 类的 `markSupported()` 方法用于验证流是否支持 `mark()` 方法。如果流支持标记()，则返回布尔值 `true`，否则返回 `false`。
...
这个方法返回一个 `boolean` 值，表示流对 `mark()` 方法的可支持性。
