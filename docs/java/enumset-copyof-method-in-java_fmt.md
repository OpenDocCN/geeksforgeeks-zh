...
> 原文: [https://www.geeksforgeeks.org/enumset-copyof-method-in-java/](https://www.geeksforgeeks.org/enumset-copyof-method-in-java/)

## 1. 基于 `Collection` 的 `copyOf()` 方法

`java.util.EnumSet.copyOf(Collection collect)` 方法在 Java 中用于将一个集合中的所有内容复制到一个新的枚举集合中。首先，该集合由枚举的元素构成，然后创建一个新的枚举集，它是该集合的副本。
...
**参数:** 该方法接受一个枚举对象类型的参数 `Collection collect`，并引用其值将被复制到 `New_Enum_Set` 中的集合。
...
*   `NullPointerException`: 如果 `collect` 为空，则抛出该异常。
...
## 2. 基于 `EnumSet` 的 `copyOf()` 方法

`java.util.EnumSet.copyOf(EnumSet e_set)` 方法在 Java 中用于将一个现有的 `EnumSet`（即 `e_set`）中的所有内容复制到一个新的枚举集合中。
...
**参数:** 该方法接受一个枚举对象类型的参数 `EnumSet e_set`，并引用其值将被复制到 `New_Enum_Set` 中的集合。

**返回值:** 该方法不返回值。

**异常:** 当 `e_set` 为空时，该方法抛出 `NullPointerException`。
