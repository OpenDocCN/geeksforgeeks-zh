...
> 原文: [https://www.geeksforgeeks.org/bigdecimal-scalebypoweroften-method-in-java/](https://www.geeksforgeeks.org/bigdecimal-scalebypoweroften-method-in-java/)

`java.math.BigDecimal.scaleByPowerOfTen(int n)` 是 Java 上的一个内置方法，它返回一个数值等于 (`this * 10^n`) 的 `BigDecimal`。结果的等级是 `this.scale() - n`。
...
这个方法返回值为 `this * 10^n` 的 `BigDecimal` 对象。
...
[https://docs.oracle.com/javase/7/docs/api/java/math/BigDecimal.html#scaleByPowerOfTen(int)](https://docs.oracle.com/javase/7/docs/api/java/math/BigDecimal.html#scaleByPowerOfTen(int))