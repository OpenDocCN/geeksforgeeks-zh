...
17. `public int codePointAt(int index)` –它将一个`索引`作为参数，该索引必须从 0 到 `length()`–1。并返回索引处字符的`unicode`点。
18. `public int codePointBefore(int index)` –它将一个`索引`作为参数，该索引必须从 0 到 `length()`–1。并返回索引前一个字符的`unicode`点。
19. `public int codePointCount(int start_index, int end_index)` – 它接受参数`start_index`和`end_index`，并返回该范围内`Unicode code points`的数量。
...
20. `public CharSequence subSequence(int start_index, int end_index)` – 此方法返回一个`CharSequence`，它是调用此方法的`String`的一个子序列。

**注意:**它的行为类似于`subString(int start_index, int end_index)`，但是`subString()`返回`String`，而`subSequence`返回`CharSequence`。

21. `public boolean contains(CharSequence char_seq)`–如果调用它的字符串中存在给定的`CharSequence`，它将返回`true`。
22. `public boolean contentEquals(CharSequence char_seq)` – 仅当给定的`CharSequence`与调用它的`String`完全匹配时，它才返回`true`。
...
public class GFG_2 {
    public static void main(String[] args)
    {
        ...
        // contains()
        System.out.println("Result of contains on s_1 "
                           + "with charSequence cs " + s_1.contains(cs));
        System.out.println("Result of contains on s_1 "
                           + "with charSequence cs1 " + s_1.contains(cs1));

        // contentEquals()
        System.out.print("Result of contentEquals ");
...
```java
Result of contains on s_1 with charSequence cs true
Result of contains on s_1 with charSequence cs1 false
Result of contentEquals true
Result of contentEquals false
```

23. `public boolean endsWith(String suf)`–它接受参数字符串`后缀`，如果字符串具有相同的后缀，则返回`true`。
24. `public boolean startsWith(String pre)` – 它接受一个字符串参数`前缀`，如果字符串具有相同的前缀，则返回`true`。
...
```java
geeksforgeeks end with geeks true
geeksforgeeks end with eks true
geeksforgeeks end with for false
geeksforgeeks starts with geeks  true
geeksforgeeks starts with for  false
```

25. `public void getChars(int start, int end, char[] destination, int destination_start)` :取四个参数，`start`和`end`指的是要复制到字符数组的范围，`destination`是要复制到的字符数组，`destination_start`是目标数组的起始位置。
26. `public char[] toCharArray()` – 它将整个`String`转换为字符数组。

**注意:-** `getChars`提供更大的灵活性，当一个字符范围被复制到一个现有的数组或一个新的数组，而`toCharArray`将整个字符串转换成一个新的字符数组。
...
28. `public String intern()` – 它返回调用它的`String`对象的规范形式。
...
30. [`public static String format(String f, Object… arguments)`](https://www.geeksforgeeks.orgjava-string-format-examples/) – 根据格式说明符`f`返回格式化后的字符串，`arguments`的数量应与使用的格式说明符数量完全相等。
    **变体:**
    [`public static String format(Locale l, String f, Object… arguments)`](https://www.geeksforgeeks.org/java-string-format-examples/) – 根据使用的[`Locale`](https://docs.oracle.com/javase/8/docs/api/java/util/Locale.html)返回格式化后的字符串。
...
31. [`public boolean matches(String reg_exp)`](https://www.geeksforgeeks.org/java-lang-string-matches-java/) – 如果字符串匹配[正则表达式](https://www.geeksforgeeks.org/write-regular-expressions/) (`reg_exp`)，则返回 true。
32. `public boolean regionMatches(int start_OString, String another, int start_AString, int no_of_char)` – 如果原始字符串中从索引`start_OString`开始的区域与`another string`中从索引`start_AString`开始的区域匹配，则返回 true，`no_of_char`指的是要比较的字符数。

    **变体:**
    `public boolean regionMatches(boolean ignore_case, int start_OString, String another, int start_AString, int no_of_char)` – 当我们想要在比较子字符串时忽略大小写时，方法的这种变体提供了灵活性。如果第一个参数`ignore_case`为`true`则忽略大小写并进行比较，但是如果为`false`则与第一个版本的方法行为类似，不使用`ignore_case`。
...
33. [`public String[] split(String reg_exp)`](https://www.geeksforgeeks.org/split-string-java-examples/) – 它围绕正则表达式拆分字符串并返回一个`String`数组。

    **变体:**
    [`public String[] split(String reg_exp, int limit)`](https://www.geeksforgeeks.org/split-string-java-examples/) – 它在正则表达式周围拆分字符串，`limit`指的是应用`reg_exp`的次数，它是*结果数组的长度*，如果`reg_exp`为 n 则只应用 length-1 次。
...
34. [`public static String join(CharSequence de_limiter, CharSequence… elements)`](https://www.geeksforgeeks.org/java-string-join-examples/) – 它返回一个字符串，其中包含所有由`de_limiter`连接的`elements`。

    **变体:**
    [`public static String join(CharSequence de_limiter, Iterable elements)`](https://www.geeksforgeeks.org/java-string-join-examples/) – 它执行相同的功能，但第二个参数是`Iterable`，这使得它可以灵活地处理不同的集合类。
...
35. `public String replaceAll(String reg_exp, String replacement)` – 它用`replacement`替换与`reg_exp`匹配的原始字符串的所有子字符串，并返回修改后的字符串。
36. `public String replaceFirst(String reg_exp, String replacement)` – 它用`replacement`替换原始字符串中`reg_exp`的第一次出现，并返回修改后的字符串。
    **注意：** `replaceAll`和`replaceFirst`不会改变原始字符串，而是创建一个包含修改的新字符串。
