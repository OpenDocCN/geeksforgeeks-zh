...
2.  `CheckMenuItem(String t, Node g)`：用指定的文本和图形创建一个 `CheckMenuItem`。
...
| `selectedProperty()` | 表示此 `CheckMenuItem` 的当前状态 |
| `setSelected(boolean v)` | 设置选定属性的值 |

下面的程序说明了 JavaFX 的 `CheckMenuItem` 类：

## 程序1：创建菜单并添加CheckMenuItem
...
[https://docs.oracle.com/javafx/2/api/javafx/scene/control/CheckMenuItem.html](https://docs.oracle.com/javafx/2/api/javafx/scene/control/CheckMenuItem.html)