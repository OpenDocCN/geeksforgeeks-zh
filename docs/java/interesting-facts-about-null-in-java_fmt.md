...
> 原文: [https://www.geeksforgeeks.org/interesting-facts-about-null-in-java/](https://www.geeksforgeeks.org/interesting-facts-about-null-in-java/)
...
`null` 在 Java 中是字面量，因为关键字在 Java 中是区分大小写的，所以我们不能像 C 语言那样写 `NULL` 或 `0`。
...
与常见的误解不同，`null` 不是 `Object` 或者既不是类型。这只是一个特殊的值，可以分配给任何引用类型，您可以将空值转换为任何类型。
...
在自动装箱和取消装箱操作期间，如果将空值赋给基元装箱数据类型，编译器只需抛出空指针异常错误。
...
Java `instanceof` 运算符用于测试对象是否是指定类型（类或子类或接口）的实例。在运行时，如果表达式的值不为空，`instanceof` 运算符的结果为真。
...
我们不能对空值的引用变量调用非静态方法，它会抛出 `NullPointerException`，但是我们可以对空值的引用变量调用静态方法。因为静态方法是使用静态绑定绑定的，所以它们不会抛出空指针异常。
...
在 Java 中允许用 `null` 进行比较和不等于运算符。这对于用 Java 中的对象检查 `null` 非常有用。
