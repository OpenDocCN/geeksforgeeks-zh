...
> 原文：[https://www.geeksforgeeks.org/adding-list-in-a-pdf-using-java/](https://www.geeksforgeeks.org/adding-list-in-a-pdf-using-java/)
...
```java
// 创建 PdfWriter 对象
String path = "path/to/output.pdf";
...
`Document` 类是创建自给自足的 PDF 时的根元素。这个类的一个构造函数接受一个类 `PdfDocument`（即 `pdfdocument`）的对象。
