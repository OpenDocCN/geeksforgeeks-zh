...
`JToolBar` 是 Java Swing 包的一部分。`JToolBar` 是工具栏的一种实现。`JToolBar` 是一组常用的组件，如按钮或下拉菜单。用户可以将工具栏拖动到不同的位置。

## 类的构造函数有:

1.  `JToolBar()`: 创建一个水平方向的新工具栏
2.  `JToolBar(int o)`: 创建一个指定方向的新工具栏
3.  `JToolBar(String n)`: 创建一个具有指定名称的新工具栏
4.  `JToolBar(String n, int o)`: 创建一个具有指定名称和方向的新工具栏。

## 常用方法:

1.  `addSeparator()`: 将分隔符添加到工具栏的末尾
2.  `setFloatable(boolean b)`: 如果传递 `true`，则工具栏可以被拖动到其他位置，否则不可以。
3.  `setLayout(LayoutManager m)`: 设置工具栏的布局
4.  `setOrientation(int o)`: 设置工具栏的方向
5.  `add(Component c)`: 向工具栏添加组件
6.  `getMargin()`: 返回工具栏的边距
7.  `setMargin(Insets m)`: 将工具栏的边距设置为给定的 `Insets`
8.  `getOrientation()`: 返回工具栏的方向
9.  `updateUI()`: 来自 `UIFactory` 的通知，表示外观和感觉已经改变。
10. `setUI(ToolBarUI ui)`: 设置渲染这个组件的外观对象。
11. `setRollover(boolean b)`: 将此工具的翻转状态设置为布尔值 `b`。
12. `setFloatable(boolean b)`: 布尔值 `b` 决定工具栏的位置是否可以改变
13. `setBorderPainted(boolean b)`: 决定是否应该绘制边框。
14. `paintBorder(Graphics g)`: 绘制工具栏的边框
15. `isRollover()`: 返回翻转状态。
16. `isFloatable()`: 返回可浮动属性。
17. `isBorderPainted()`: 返回边框是否被绘制
18. `getComponentIndex(Component c)`: 返回指定组件的索引。
19. `getComponentAtIndex(int i)`: 返回指定索引处的组件。
20. `addSeparator(Dimension size)`: 追加指定维度的分隔符。
21. `addSeparator()`: 追加默认大小的分隔符。
22. `add(Action a)`: 在指定的动作之后添加一个新的 `JButton`。
