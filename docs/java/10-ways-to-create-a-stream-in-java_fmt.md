...
> 原文: [https://www.geeksforgeeks.org/10-ways-to-create-a-stream-in-java/](https://www.geeksforgeeks.org/10-ways-to-create-a-stream-in-java/)
...
`Arrays.stream()` 和 `Stream.of()` 方法是从指定数组创建顺序流的两种常用方法。当用非基元类型的数组调用时，这两个方法都返回一个流。
...
*   **使用 `Stream.of()` 创建流**
    一个非干扰操作，在元素从流中消费时执行，并且也返回一个新的流。
...
private static <T> void getStream(T[] arr)
{
    // Create stream from an array using Stream.of()
...
`iterate()`方法返回一个无限顺序有序流，该流是通过对初始元素种子迭代应用函数`f`而产生的。在下面的示例中，结果流的第一个元素是迭代方法的第一个参数。为了创建每一个后续元素，该函数应用于前一个元素。在下面的例子中，第二个元素是4。
...
class GFG {
    ...
    {
        // Create infinite stream using Stream.iterate() method
...
## 6. 使用`Pattern`和`Predicate`创建流

在java8中，`Pattern`的谓词`asPredicate()`方法创建了一个用于模式匹配的谓词布尔值函数。
...
class GFG {
    ...
    public static void main(String[] args)
    {
        // Create ArrayList of String that is backed by the specified array
