...
很多时候你会听到人们说 [`Java`](https://www.geeksforgeeks.org/java/) 比 Python 快或者 [`Python`](https://www.geeksforgeeks.org/python-programming-language/) 比 Java 更安全，以及它们之间的很多其他种类的比较。
...
在 Python 中，我们用 `.py` 文件，当您使用 `python` 命令执行该文件时 `.py` 然后它生成字节码，并将其存储在您的目录中的某个地方。这是对用户隐藏的，以使其不那么复杂。它会被编译，也会被解释，但是谁先经历呢？
