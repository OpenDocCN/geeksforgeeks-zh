...
> 原文：[https://www.geeksforgeeks.org/java-program-to-add-tables-to-a-word-document/](https://www.geeksforgeeks.org/java-program-to-add-tables-to-a-word-document/)
...
### 第一步：下载 Apache POI API

你可以从[这里](https://www.iotools.net/download/java-apis/apache-poi)下载。

### 第二步：导入 .jar 文件

提取项目的 jar 文件，并将以下 jar 添加到您的构建路径中：
...
下面是使用 Java 实现类来添加文档的步骤。

### 第一步：导入类

导入 `io` 的 `XWPFDocument`、`XWPFTable`、`XWPFTableRow` 等类。文件包，有助于创建文档，将表格添加到文档中，然后向其中添加行。

### 步骤 2：创建文档

使用类 `XWPFDocument` 创建一个文档，并将其存储在一个变量中。
