...
> 原文：`https://www.geeksforgeeks.org/character-offsetbycodepoints-method-in-java/`

`Character.offsetByCodePoints(CharSequence seq, int index, int codePointOffset)` 是 Java 中的一个内置方法，它返回给定字符序列内的索引，该索引从给定索引偏移了 `codePointOffset` 代码点。由索引和代码点偏移量给定的文本范围内的不成对的替代项各计为一个代码点。
...
*   `IndexOutOfBoundsException` – 如果 `index` 为负或大于字符序列的长度，或者如果 `codePointOffset` 为正并且以 `index` 开始的子序列的代码点少于 `codePointOffset`，或者如果 `codePointOffset` 为负并且 `index` 之前的子序列的代码点少于 `codePointOffset` 的绝对值。
