...
> 原文: [https://www.geeksforgeeks.org/matchresult-group-method-in-java-with-examples/](https://www.geeksforgeeks.org/matchresult-group-method-in-java-with-examples/)

使用`MatchResult`接口的`group()`方法获取与前一个匹配结果匹配的输入子序列。
...
**参数:** 该方法不取任何参数。
**返回值:** 该方法返回`String`，该字符串是上一次匹配的输入子序列。
