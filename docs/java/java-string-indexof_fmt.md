...
`indexOf()`方法有四个变体。本文对它们进行了描述，如下所示：

**1. `int indexOf()`**：此方法返回字符串中指定字符的第一个出现的索引，如果该字符没有出现，则返回-1。
...
**2. `int indexOf(char ch, int strt)`**：此方法返回字符串内指定字符的第一个出现的索引，从指定的索引处开始搜索，如果该字符没有出现，则返回-1。
