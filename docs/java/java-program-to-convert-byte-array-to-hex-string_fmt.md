...
> 原文：[https://www.geeksforgeeks.org/java-program-to-convert-byte-array-to-hex-string/](https://www.geeksforgeeks.org/java-program-to-convert-byte-array-to-hex-string/)
...
`Integer` 类有 [`toHexString()`](https://www.geeksforgeeks.org/java-lang-integer-tohexstring-method-examples/) 方法，可以将一个整数转换为它的十六进制等价物。我们现在需要将字节数组转换为整数（对于 4 大小的）或长（对于 8 大小的）并使用这个方法（因为这个方法存在于两个类中，即具有相同名称的 `Integer` 和 `Long`）。为了将字节数组转换为整数或长，我们可以使用 `ByteBuffer` 类的 [`wrap()`](https://www.geeksforgeeks.org/bytebuffer-wrap-method-in-java-with-examples/) 方法。
