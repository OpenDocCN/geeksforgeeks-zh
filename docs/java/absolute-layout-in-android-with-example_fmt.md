...
> 原文：[https://www.geeksforgeeks.org/absolute-layout-in-android-with-example/](https://www.geeksforgeeks.org/absolute-layout-in-android-with-example/)
...
1.  `android:id`：唯一指定绝对布局
2.  `android:layout_x`：它指定视图的 X 坐标（可能的值在[密度-像素或像素](https://www.geeksforgeeks.org/how-to-scale-different-views-to-all-screen-sizes-in-android-studio/)中）
3.  `android:layout_y`：它指定视图的 Y 坐标（可能的值是 dp 或 px）
...
为此，转到`应用程序 > res > 布局 > activity_main.xml`文件，并将约束布局更改为绝对布局，并添加[文本视图](https://www.geeksforgeeks.org/textview-widget-in-android-using-java-with-examples/)。下面是`activity_main.xml`文件的代码片段。
