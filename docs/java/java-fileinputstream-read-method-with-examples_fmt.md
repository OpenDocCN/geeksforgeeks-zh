# Java 文件输入流 read() 方法示例

> 原文: [https://www.geeksforgeeks.org/java-fileinputstream-read-method-with-examples/](https://www.geeksforgeeks.org/java-fileinputstream-read-method-with-examples/)

Java 中的 `FileInputStream` 类对于以字节序列的形式从文件中读取数据非常有用。`FileInputStream` 用于读取原始字节流，如图像数据。要读取字符流，请考虑使用 `FileReader`。
...
我们将在本文中使用 `available()` 方法的概念。`available()` 方法用于返回还有多少字节需要读取。我们将使用 `read()` 方法打印字符，直到剩下 0 个字符可供读取。
