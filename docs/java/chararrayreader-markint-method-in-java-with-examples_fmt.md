# Java 中的 CharArrayReader mark(int) 方法示例
...
该方法接受一个强制参数 `readAheadLimit`，它是在保留标记的同时可以读取的字符数的限制。读取这么多字符后，尝试重置流可能会失败。
...
如果在不支持输入输出或 `mark()` 方法时出现一些错误，此方法将引发 `IOException`。
...
[https://docs.oracle.com/javase/9/docs/api/java/io/CharArrayReader.html#mark-int-](https://docs.oracle.com/javase/9/docs/api/java/io/CharArrayReader.html#mark-int-)