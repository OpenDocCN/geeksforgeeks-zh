...
> 原文：[https://www.geeksforgeeks.org/character-isdigit-method-in-java-with-examples/](https://www.geeksforgeeks.org/character-isdigit-method-in-java-with-examples/)

## isDigit(char ch)方法

`java.lang.Character.isDigit(char ch)` 是 Java 中的一个内置方法，用于判断指定字符是否为数字。一个字符要被接受为数字，需要满足一些条件。也就是说，如果由 `getType(ch)` 提供的字符的一般类别类型是 `DECIMAL_DIGIT_NUMBER`，那么该字符就是一个数字。
...
该方法接受字符参数 `ch` 作为自变量，待测试。

### 返回值

这个方法返回一个布尔值。如果 `ch` 是数字，则返回 `true`，否则返回 `false`。

### 注意

此方法无法处理[补充字符](https://docs.oracle.com/javase/8/docs/api/java/lang/Character.html#supplementary)。若要支持所有 Unicode 字符，包括补充字符，请使用 `isDigit(int)` 方法。
...
## isDigit(int codePoint)方法

`java.lang.Character.isDigit(int codePoint)` 是 Java 中的一个内置方法，用于确定指定的整数类型的 Unicode 代码点字符是否为数字。

一个字符要被接受为一个数字，几乎没有必须满足的条件。也就是说，如果由 `getType(codePoint)` 提供的字符的一般类别类型是 `DECIMAL_DIGIT_NUMBER`，那么该字符就是一个数字。
...
该方法接受整数类型的 Unicode 字符参数 `codePoint` 作为参数，待测试。

### 返回值

这个方法返回一个布尔值。如果指定的字符是数字，则返回 `true`，否则返回 `false`。
