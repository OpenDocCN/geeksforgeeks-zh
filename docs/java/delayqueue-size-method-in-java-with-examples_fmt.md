...
> 原文: [https://www.geeksforgeeks.org/delayqueue-size-method-in-java-with-examples/](https://www.geeksforgeeks.org/delayqueue-size-method-in-java-with-examples/)

Java 中 `DelayQueue` 类的 `size()` 方法用于返回给定队列中存在的元素数量。如果元素数量大于 `Integer.MAX_VALUE`，则返回 `Integer.MAX_VALUE` 作为延迟队列的大小。
