...
> 原文：[https://www.geeksforgeeks.org/how-to-check-if-a-key-exists-in-a-hashmap-in-java/](https://www.geeksforgeeks.org/how-to-check-if-a-key-exists-in-a-hashmap-in-java/)
...
1.  获取哈希表和密钥。
2.  使用 `HashMap.iterator()` 方法创建一个迭代器来迭代 `HashMap`。
...
说明为什么不建议采用这种方法。如果 `HashMap` 包含空值，那么这个方法将抛出 `NullPointerException`。这使得该方法成为不可建议使用的方法。
