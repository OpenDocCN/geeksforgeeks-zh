...
> 原文：[https://www.geeksforgeeks.org/collections-sort-java-examples/](https://www.geeksforgeeks.org/collections-sort-java-examples/)
...
它的工作原理类似于 [`java.util.Arrays.sort()`](https://www.geeksforgeeks.org/arrays-sort-in-java-with-examples/) 方法，但是它更好，因为它可以对数组的元素以及链表、队列和其中的更多元素进行排序。

```java
public static void sort(List myList)
```
`myList`：我们想要排序的`List`类型对象。
此方法不返回任何内容。
...
## [`Arrays.sort()`](https://www.geeksforgeeks.org/arrays-sort-in-java-with-examples/) vs [`Collections.sort()`](https://www.geeksforgeeks.org/collections-in-java-2/)

`Arrays.sort()`适用于也可以是原始数据类型的数组。`Collections.sort()`适用于对象集合，如[`ArrayList`](https://www.geeksforgeeks.org/arraylist-in-java/)、[`LinkedList`](http://geeksquiz.com/linked-list-in-java/)等。
