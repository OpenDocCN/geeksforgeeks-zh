...
> 原文: [https://www.geeksforgeeks.org/how-to-take-input-from-user-separated-by-space-in-java/](https://www.geeksforgeeks.org/how-to-take-input-from-user-separated-by-space-in-java/)

有两种从用户处获取输入的方法，用空格隔开，如下所示:

1.  使用 `BufferedReader` 类，然后拆分和解析每个值。
...
使用 `Scanner` 类的 [`nextInt()` 方法](https://www.geeksforgeeks.org/scanner-nextint-method-in-java-with-examples/)。

**程序:**

1.  使用 `Scanner` 类的 [`nextInt()` 方法](https://www.geeksforgeeks.org/scanner-nextint-method-in-java-with-examples/) 进行扫描输入。
...
> **注意:** 第一种方法使用 `BufferedReader` 类，然后拆分解析每个值，比使用 `Scanner` 类的 `nextInt()` 方法要快得多。它比第二个快近 2 倍。现在我们提供了如何使用 `nanoTime()` 方法计算这两种方法的时间消耗。
