...
方法是执行特定任务并将结果返回给调用方的语句的集合。一个方法可以执行某些特定的任务而不返回任何东西。方法允许我们在不重新输入代码的情况下**重用**代码。在 Java 中，每个方法都必须是某个类的一部分，这不同于 C、C++ 和 Python 等语言。

> **注意:** 方法是省时的，帮助我们重用代码而不需要重新输入代码。

修饰符分为*访问修饰符*和*非访问修饰符*两组:
...
2.  **默认:** 默认修饰符的访问级别仅在包内。不能从包外部访问它。如果不指定任何访问级别，它将是默认的。
3.  **Protected:** 受保护修饰符的访问级别是通过子类在包内和包外。如果不创建子类，则不能从包外部访问它。
4.  **公共:** 公共修饰符的访问级别无处不在。可以从类内、类外、包内和包外访问它。
...
**如何命名一个方法？**: 方法名通常是一个单词，应该是小写的**动词**或多单词，以小写的**动词**开头，后跟**形容词、名词…..** 第一个单词后，每个单词的第一个字母要大写。比如 `findSum`、`computeMax`、`setX`、`getX`。
...
class GFG {
    ...
    public static void main(String[] args)
    {
        // Creating object of above class inside this class
        Test obj = new Test();
...
*   [Java 通过值严格通过](https://www.geeksforgeeks.org/g-fact-31-java-is-strictly-pass-by-value/)
