...
> 原文：[https://www.geeksforgeeks.org/how-to-use-balloon-library-in-android/](https://www.geeksforgeeks.org/how-to-use-balloon-library-in-android/)
...
| 属性 | 描述 |
| --- | --- |
| `.setWidthRatio()` | 用于根据水平屏幕大小的比例显示弹出窗口的宽度。 |
| `.showAlignBottom()` | 用于在底部显示弹出窗口。 |
| `.dismiss()` | 用于在弹出窗口显示后关闭它。 |
| `.setMargin()` | 用于从各个方面给与利润。 |
| `.setIsVisibleArrow()` | 用于显示箭头。 |
| `.setArrowPosition()` | 用于设置弹出窗口的箭头位置。 |
| `.setArrowOrientation()` | 用于设置弹出窗口的方向。 |
| `.setText()` | 用于显示文本。 |
| `.setTextColor()` | 用于显示文本颜色。 |
| `.setHeight()` | 用于给出弹出的高度。 |
| `.setCornerRadius()` | 用于使弹出窗口从四周变圆。 |
| `.setBalloonAnimation()` | 用于使动画弹出。 |
...
### 第二步：在 `build.gradle` 文件中添加气球库的依赖关系

然后导航到梯度脚本，然后导航到 `build.gradle (Module)` 级别。在 dependencies 部分的 `build.gradle` 文件中添加以下行。
