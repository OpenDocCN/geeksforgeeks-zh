# Java 中的 `LinkedTransferQueue.iterator()` 方法

> 原文: [https://www.geeksforgeeks.org/linkedtransferqueue-iterator-method-in-java/](https://www.geeksforgeeks.org/linkedtransferqueue-iterator-method-in-java/)

`LinkedTransferQueue.iterator()` 方法是 Java 中的一个内置函数，用于以适当的顺序返回这个队列中元素的迭代器。
...
该函数以适当的顺序在该队列中的元素上返回一个 `Iterator`。
