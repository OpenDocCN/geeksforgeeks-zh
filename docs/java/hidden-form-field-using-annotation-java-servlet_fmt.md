...
> 原文: [https://www.geeksforgeeks.org/hidden-form-field-using-annotation-java-servlet/](https://www.geeksforgeeks.org/hidden-form-field-using-annotation-java-servlet/)
...
2.  在 `index.html` 文件表单中，操作是 `FirstServlet`，因此当您点击提交按钮时，控制转到 `FirstServlet`，您的会话开始。
3.  在 `FirstServlet` 中，`request.getParameter("userName")` 从 `index.html` 获取输入，其中的字段是 `userName`，并将其打印在浏览器上。
4.  在 `FirstServlet` 中，有一个隐藏的表单域，它再次从浏览器获取输入，并将控件传递给 `SecondServlet`。
5.  这种方法适用于许多不同的 Servlet。
    注意: 这是隐藏表单字段方法的主要缺点，为了维护一个客户端会话，你必须一次又一次地填写表单。
6.  在 `SecondServlet` 中，我们再次使用 `request.getParameter` 从 `FirstServlet` 页面获取输入。
