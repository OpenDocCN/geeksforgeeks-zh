...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-java-date-to-xml-datetime-string/](https://www.geeksforgeeks.org/how-to-convert-java-date-to-xml-datetime-string/)

为了定义日期和时间，使用了`DateTime`数据类型。`DateTime`的定义格式为`YYYY-MM-DDTHH:MM:ss`，其中：
...
*   首先我们创建一个`SimpleDateFormat`的对象。这个类解析并格式化 Java 中的日期和时间。
*   然后，我们创建一个`StringBuffer`，它将保存 XML 格式的字符串。
