...
`StackPane` 类是 JavaFX 的一部分。`StackPane` 类以堆栈的形式布局其子类。新节点被放置在堆栈中前一个节点的顶部。`StackPane` 类继承自 [`Pane`](https://www.geeksforgeeks.org/javafx-pane-class/) 类。

## 类的构造函数

1.  `StackPane()`: 创建一个新的空堆叠面板。
2.  `StackPane(Node... c)`: 用指定的节点创建新的堆叠面板。
...
| `getMargin(Node c)` | 返回节点的 `Insets`。 |
| `setAlignment(Node n, Pos v)` | 设置作为堆栈平面一部分的节点的对齐方式。 |
| `setAlignment(Pos v)` | 设置堆栈的对齐方式。 |
| `setMargin(Node n, Insets v)` | 设置作为堆栈平面一部分的节点的边距。 |

下面的程序说明了 `StackPane` 类的使用:

### 示例 1：创建 StackPane 并添加组件
...
public class StackPane_2 extends Application {
    ...
    public void start(Stage stage) {
        try {
            ...
            // set alignment for the stack pane
...
**参考:** [https://docs.oracle.com/javase/8/javafx/api/javafx/scene/layout/StackPane.html](https://docs.oracle.com/javase/8/javafx/api/javafx/scene/layout/StackPane.html)