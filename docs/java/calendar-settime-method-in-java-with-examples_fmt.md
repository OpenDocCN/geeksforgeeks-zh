# Java 中的 Calendar.setTime() 方法示例

> 原文：[https://www.geeksforgeeks.org/calendar-settime-method-in-java-with-examples/](https://www.geeksforgeeks.org/calendar-settime-method-in-java-with-examples/)

`Calendar` 类中的 `setTime(Date dt)` 方法用于设置由该 `Calendar` 的时间值表示的 `Calendar` 时间，给定或过去的日期作为参数。
...
**参数：** 该方法取 `Date` 类型的一个参数 `dt`，指的是要设置的给定日期。
**返回值：** 该方法不返回值。

下面的程序说明了 `Calendar` 类的 `setTime()` 方法的工作：
