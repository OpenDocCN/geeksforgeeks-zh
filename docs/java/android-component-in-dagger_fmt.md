...
为了充分理解Dagger中组件的概念，我们必须先简单介绍一下Dagger本身。根据文献记载，Dagger的定义如下：

> 安卓应用程序中的手动依赖注入或服务定位器可能会有问题，具体取决于项目的大小。通过使用Dagger来管理依赖项，您可以在项目扩展时限制项目的复杂性。“Dagger会自动生成代码，模仿你原本手写的代码。”

让我们分析一下上面的说法是什么意思。

想象一下下面的场景：让我们假设我们有一个类，`Bike`类，和两个函数，它需要另外两个类，即`Engine`和`Wheels`。因此，我们说`Bike`类依赖于`Engine`和`Wheels`，为了正确的功能，应该实现这些依赖。现在，为了实例化`Bike`类，我们还必须创建和初始化，或者准确地说，只在活动的样板代码中注入另外两个类。这描述如下：

![](img/cef7051cdb8b6633528b39b02e0b7fe1.png)

实例化不带Dagger的`Bike`类

现在，当项目规模扩大时，各种类之间通常会有很多依赖关系，而上述方法会导致一些严重的问题和性能瓶颈。这就是Dagger发挥作用的地方，它自动处理所有的依赖和实例化。要详细了解Dagger和依赖注入：[安卓中Dagger 2的依赖注入](https://www.geeksforgeeks.org/dependency-injection-with-dagger-2-in-android/)

## 什么是Dagger

Dagger中的组件通过创建项目中所有依赖关系的图表来工作，这样它就可以在需要的时候找到应该从哪里获取这些依赖关系。为了实现这一点，需要创建一个接口，并用`@Component`进行注释。使用这个注释处理器，Dagger创建了一个依赖图，它包含了不同类之间的关系。这允许Dagger创建一个容器，就像我们手动依赖注入一样。

![](img/21eb8ff9cfa84271bbc3c140bb47f686.png)

函数也可以在`@Component`接口中定义，该接口返回所需类（`Bike`类）的实例。`@Component`通知Dagger生成一个包含所有必需依赖项的容器。这就是所谓的Dagger中的组件。让我们在实现中看到上面的例子。
...
为了在Dagger的帮助下使用依赖注入，我们需要在Gradle文件中添加一些依赖。转到`Gradle脚本 -> build.gradle (模块: 应用程序)`并添加以下依赖项。添加这两个依赖项后，您需要单击立即同步。
...
在这一步中，我们将创建一个带有注释`@Component`的*接口*，它将负责实例化自行车实体，从而创建所有依赖关系的图形。你可以参考这篇文章在安卓工作室中[创建接口。](https://www.geeksforgeeks.org/how-to-create-interfaces-in-android-studio/)
...
在这一步中，我们将使用注释`@Inject`注入所需的类，以创建自行车实体。下面是所有文件的代码片段。你可以参考这篇文章在安卓工作室中[创建类。](https://www.geeksforgeeks.org/how-to-create-classes-in-android-studio/)
...
在这一步中，我们将实例化`GFG_Bike`。如果步骤成功，文本视图将被更新，否则将显示错误[Toast](https://www.geeksforgeeks.org/android-what-is-toast-and-how-to-use-it-with-examples/)。下面是代码片段。
