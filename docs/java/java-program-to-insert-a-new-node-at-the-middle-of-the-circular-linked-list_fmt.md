...
> 原文：[https://www.geeksforgeeks.org/java-program-to-insert-a-new-node-at-the-middle-of-the-circular-linked-list/](https://www.geeksforgeeks.org/java-program-to-insert-a-new-node-at-the-middle-of-the-circular-linked-list/)
...
3.  对于非空列表，计算列表的长度。
4.  创建可变的中间并在其中存储中间长度。
5.  创建两个节点 `temporary` 和 `current`。
6.  现在使用中间变量遍历列表，直到 `temporary` 到达列表的中点。
7.  在当前节点后插入新节点。
8.  将 `current` 设为 `next`。指向 `New_node` 和 `New_node`。指向 `temporary`。
9.  节点已插入。
