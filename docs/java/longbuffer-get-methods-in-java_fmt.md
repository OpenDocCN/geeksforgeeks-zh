...
> 原文: [https://www.geeksforgeks.org/longbuffer-get-methods-in-java/](https://www.geeksforgeks.org/longbuffer-get-methods-in-java/)

## `get()`

`LongBuffer` 类的 `get()` 方法用于读取给定缓冲区当前位置的 `long`，然后增加位置。
...
**抛出:** 此方法抛出 `BufferUnderflowException` – 如果缓冲区当前位置不小于其限制，则抛出此异常。
...
`LongBuffer` 的 `get(long index)` 方法用于读取指定索引处的 `long`。
...
**参数:** 该方法以 `index` (读取 `long` 的索引)为参数。

**返回值:** 该方法返回给定索引处的 `Long` 值。

**异常:** 此方法抛出 `IndexOutOfBoundsException`。如果索引为负或不小于缓冲区的限制，则会引发此异常。

以下是说明 `get(long index)` 方法的示例:
