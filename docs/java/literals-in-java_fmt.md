...
简单来说，Java 中的文字是布尔、数字、字符或字符串数据的综合表示。它是在程序中表达特定值的一种媒介，例如名为 `x` 的整数变量在下面的语句中被赋予一个整数值。
...
对于整型数据类型（`byte`、`short`、`int`、`long`），我们可以用 4 种方式指定文字：
...
**十六进制文字（Base 16）:** 在这种形式中，允许的数字是 0-9，字符是 a-f，我们可以同时使用大写和小写字符，因为我们知道 Java 是一种区分大小写的编程语言，但是这里 Java 不区分大小写。
...
> **注意:** 默认情况下，每个文字都是 `int` 类型，我们可以通过以 `l` 或 `L` 为后缀来明确指定为 `long` 类型，没有办法明确指定 `byte` 和 `short` 文字，但是我们可以间接指定。每当我们给 `byte` 变量赋值时，如果该值在 `byte` 范围内，编译器会自动将其视为 `byte` 文字。
...
> **注意:** 默认情况下，每个浮点文字都是 `double` 类型，因此我们不能直接赋值给 `float` 变量。但是我们可以通过以 `f` 或 `F` 为后缀将浮点文字指定为 `float` 类型。我们可以通过以 `d` 或 `D` 为后缀将浮点文字显式指定为 `double` 类型。当然，这种约定不是必需的。
...
public class Test {
    public static void main(String[] args)
    {
        ...
        // char b = 0789; // This will cause an error
...
public class Test {
    public static void main(String[] args)
    {
        boolean b = true;
        boolean c = false;
        // boolean d = 0; // This will cause an error
        // boolean e = 1; // This will cause an error
