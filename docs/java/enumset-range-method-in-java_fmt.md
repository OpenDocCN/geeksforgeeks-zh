...
方法返回指定范围内提到的元素创建的枚举集。

## 异常

该方法抛出两种类型的异常：

*   如果任何开始或最后一个元素为空，将引发 `NullPointerException`。
