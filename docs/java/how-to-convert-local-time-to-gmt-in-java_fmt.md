...
> 原文: [https://www.geeksforgeeks.org/how-to-convert-local-time-to-gmt-in-java/](https://www.geeksforgeeks.org/how-to-convert-local-time-to-gmt-in-java/)

从 IST 时间或任何标准时间转换为格林尼治时间对于本地人理解和回报他们的国际客户是必要的，如果他们在工作或任何目的方面与海外有联系。今天，我们将看一个代码，其中我们将任何国家的标准时间转换为格林尼治标准时间。

在这里，我们将使用`SimpleDateFormat`来转换格林尼治标准时间中的当地时间。它可以在课堂上获得，如前所述:
...
1.  使用`SimpleDateFormat`类的`format()`方法
2.  使用`SimpleDateFormat`类的`Instance()`方法

**方法 1:** 使用`SimpleDateFormat`类的`format()`方法

Java 中`DateFormat`类的`format()`方法用于将给定日期格式化为日期/时间字符串。基本上，`format()`方法是将这个日期和时间转换成特定的格式，即 mm/dd/yyyy。
...
*   然后使用`SimpleDateFormat`将其转换为格林尼治标准时间
*   然后打印两个时区。
