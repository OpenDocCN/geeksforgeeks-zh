...
> 原文：[https://www.geeksforgeeks.org/java-program-to-interchange-any-two-rows-in-the-matrix/](https://www.geeksforgeeks.org/java-program-to-interchange-any-two-rows-in-the-matrix/)
...
*   如果 `K` 和 `L` 相同，则按原样打印矩阵。
*   否则在矩阵的第 `K` 行和第 `L` 行上循环。
*   遍历时用两行的索引交换元素。
*   现在循环结束后，打印矩阵。
