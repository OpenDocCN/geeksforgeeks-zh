...
> 原文：[https://www.geeksforgeeks.org/multi-language-programming-java-process-class-jni-and-io/](https://www.geeksforgeeks.org/multi-language-programming-java-process-class-jni-and-io/)
...
一个从 Java 运行文本到语音引擎的短程序。它由两个文件组成，即 Python 的 `tts.py` 和 Java 的 `GFG.java`。
...
从 Python 程序中，我们可以将我们想要的数据打印到 STDOUT 中。（使用 Python 中的简单 `print()` 函数。）现在，`Process` 类的实例 `ec` 有一个名为 `getInputStream()` 的函数。这将为 Java 程序返回一个 `InputStream` 对象。现在，来自输入流的输入可以很容易地用 `BufferedReader` 或 `Scanner` 类来处理。Python 类的标准输出连接到 `Process` 类实例的输入流。
...
class GFG {
    ...
    public static void main(String args[])
    {
        ...
        try {
            // Creating object of Processor class for python script
            Process ec = Runtime.getRuntime().exec(
                "python tts.py " + str);

            // Connect STDOUT of Python script to BufferedReader of Java
            BufferedReader br = new BufferedReader(
                new InputStreamReader(ec.getInputStream()));

            // Initially declaring and initializing empty string
            String st = " ";
        ...
        // Catch block to handle the exceptions
            ...
            // Print the exception/s along with line number using printStackTrace() method
            excep.printStackTrace();
        }
    }
}
