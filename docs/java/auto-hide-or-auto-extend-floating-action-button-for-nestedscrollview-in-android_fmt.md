...
> 原文:[https://www.geeksforgeeks.org/auto-hide-or-auto-extend-floating-action-button-for-nestedscrollview-in-android/](https://www.geeksforgeeks.org/auto-hide-or-auto-extend-floating-action-button-for-nestedscrollview-in-android/)
...
*   根布局应该是`activity_main.xml`中的`CoordinatorLayout`。
*   此外，嵌套滚动视图和一个扩展浮动动作按钮在布局中以重力`"bottom|end"`实现。
*   在`NestedScrollView`内嵌套的滚动视图布局样本布局是为了演示目的而包含的。
...
*   有一个需要处理的`NestedScrollView`与`OnScrollChangeListener`。如果嵌套滚动视图向下滚动，则扩展浮动操作按钮应处于收缩状态，当向上滚动时，FAB 应处于扩展状态。
...
### 现在用正常浮动动作按钮代替扩展浮动动作按钮

*   将扩展浮动动作按钮替换为正常浮动动作按钮，以隐藏向下滚动时的 FAB，向上滚动时 FAB 应该出现。
*   在`activity_main.xml`文件中调用以下代码。在这里，`ExtendedFloatingActionButton`被`FloatingActionButton`取代。
