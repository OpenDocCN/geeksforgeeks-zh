...
组布局是一个布局管理器，它对组件进行分层分组，并将它们排列在一个容器中。分组是通过使用 `Group` 类的实例来完成的。它通常用于开发图形用户界面（GUI）构建器，如 NetBeans IDE 提供的 GUI 构建器马蒂斯。`GroupLayout` 类支持两种类型的组：
...
*   `GroupLayout(Container host)`：用于为指定的容器创建组布局。

## 常用方法

1.  `addComponent(Component component, Object component)`：通知组件已添加到父容器。
2.  `getHonorsVisibility()`：返回在调整组件大小和定位组件时是否考虑组件可见性。
3.  `maximumLayoutSize(Container parent)`：返回指定容器的最大尺寸。
4.  `getLayoutAlignmentX(沿水平轴)`：返回沿 x 轴的对齐。
5.  `minimumLayoutSize(Container parent)`：返回指定容器的最小大小。
6.  `getLayoutStyle()`：返回用于计算组件之间首选间隙的 `LayoutStyle`。

下面的程序说明了 `GroupLayout` 类的使用：

下面的程序通过在一个 `JFrame` 中排列 `JLabel` 组件来说明 `GroupLayout` 的使用，该 `JFrame` 的实例类是 `GroupLayoutDemo`。我们创建 2 个名为 `headerLabel`、`statusLabel` 的 `JLabel` 组件，并创建 3 个名为 `btn1`、`btn2`、`btn3` 的 `JButton` 组件，然后使用 `add()` 方法将其添加到 `JFrame` 中。我们使用 `setSize()` 和 `setVisible()` 方法设置框架的大小和可见性。布局使用 `setLayout()` 方法设置。
...
public class GroupLayoutDemo {
    ...
    private void prepareGUI() {
        // Initialization of object "mainframe" of JFrame class.
        ...
        // Initialization of object "headerLabel" of JLabel class.
        headerLabel = new JLabel("", JLabel.CENTER);

        // Initialization of object "statusLabel" of JLabel class.
        ...
        // Initialization of object "controlPanel" of JPanel class.
        controlPanel = new JPanel();

        // Function to set the layout of JFrame.
        controlPanel.setLayout(new FlowLayout());

        // Adding Jlabel "headerlabel" on JFrame.
        mainFrame.add(headerLabel);

        // Adding JPanel "controlPanel" on JFrame.
        mainFrame.add(controlPanel);

        // Adding JLabel "statusLabel" on JFrame.
    ...
    private void showGroupLayoutDemo() {
        // Function to set the text on the header of JFrame.
        headerLabel.setText("Layout in action: GroupLayout");

        // Creating Object of "Panel" class
        JPanel panel = new JPanel();

        // Function to set the size of JFrame.
        panel.setSize(200, 200);

        // Creating Object of "layout" class
        ...
        // Creating Object of "btn1" class
        JButton btn1 = new JButton("Button 1");
```

# GroupLayout 使用示例

以下代码片段展示了如何使用 `GroupLayout` 来排列组件。

```java
// Creating Object of "btn2" class
...
下面的程序通过在一个 `JFrame` 中排列 `JLabel` 组件来说明 `GroupLayout` 的使用，该 `JFrame` 的实例类是 `GroupLayoutExample`。我们创建 1 个 `JLabel`、1 个 `JTextField` 和 2 个 `JCheckBox` 组件。两个 `JButton` 组件也创建为 `FindButton` 和 `CancelButton`，然后使用 `add()` 方法将它们添加到 `JFrame` 中。布局使用 `setLayout()` 方法设置。
...
public class GroupLayoutExample {
    ...
    public static void main(String[] args)
    {
        // Function to set the Default Look And Feel Decorated of JFrame.
        ...
        // Creating Object of "JTextField" class
...
**参考:** [https://docs.oracle.com/javase/7/docs/api/javax/swing/GroupLayout.html](https://docs.oracle.com/javase/7/docs/api/javax/swing/GroupLayout.html)