...
> 原文: [https://www.geeksforgeeks.org/java-if-statement-with-examples/](https://www.geeksforgeeks.org/java-if-statement-with-examples/)
...
`Java if` 语句是最简单的决策语句。它用于决定是否执行某个语句或语句块，即如果某个条件为真，则执行某个语句块，否则不执行。
...
如果我们没有在 `if( condition)` 之后提供花括号 `{` 和 `}`，那么默认情况下 `If` 语句会将紧接的 one 语句视为在其块内。例如，
...
4. "outside if-block" gets printed.
```
...
2.  [带有示例的 Java if-else 语句](https://www.geeksforgeeks.org/java-if-else-statement-with-examples/)
3.  [Java if-else-if 阶梯示例](https://www.geeksforgeeks.org/java-if-else-if-ladder-with-examples/)
