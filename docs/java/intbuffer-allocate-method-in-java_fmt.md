...
> 原文: [https://www.geeksforgeeks.org/intbuffer-allocate-method-in-java/](https://www.geeksforgeeks.org/intbuffer-allocate-method-in-java/)

`java.nio.IntBuffer` 类的 `allocate()` 方法用于在现有缓冲区旁边分配一个新的 `int buffer`。新缓冲区的位置将为零。它的极限将是它的容量。它的标记将是未定义的。它的每个元素都将被初始化为零。它将有一个后备数组，并且它的数组偏移量为零。
...
该方法以新缓冲区的 `capacity` 为参数，以 `int` 为单位。
...
如果容量为负整数，该方法抛出 `IllegalArgumentException`。
