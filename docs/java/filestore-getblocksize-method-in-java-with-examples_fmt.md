...
> 原文：[https://www.geeksforgeeks.org/filestore-getblocksize-method-in-java-with-examples/](https://www.geeksforgeeks.org/filestore-getblocksize-method-in-java-with-examples/)

`FileStore`类的`getBlockSize()`方法用于返回这个文件存储对象中每个块的字节数。每个文件存储都被组织成称为块的离散字节序列，块是文件存储的最小存储单元。每个读写操作都在多个块上执行。这种方法有助于获取块的大小。
...
## 返回值：
该方法返回一个正值，代表该文件存储的块大小，单位为字节。

## 异常：
该方法抛出以下异常：
* `IOException`：如果出现输入输出错误。
* `UnsupportedOperationException`：如果不支持操作。
