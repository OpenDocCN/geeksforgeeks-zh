# Java 中的 `Arrays.deepEquals()`

> 原文: [https://www.geeksforgeeks.org/java-util-arrays-deepequals-java/](https://www.geeksforgeeks.org/java-util-arrays-deepequals-java/)
...
*   两个可能为 `null` 的元素 `e1` 和 `e2` 在以下任一条件成立时被认为是深度相等的：
    *   `e1` 和 `e2` 都是对象引用类型的数组。
...
我们应该覆盖 `equals()` 方法来定义用户定义类中不同参数的相等性。
