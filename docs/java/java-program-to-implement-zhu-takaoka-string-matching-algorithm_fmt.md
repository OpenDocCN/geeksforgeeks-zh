...
> 原文：[https://www.geeksforgeeks.org/java-program-to-implement-zhu-takaoka-string-matching-algorithm/](https://www.geeksforgeeks.org/java-program-to-implement-zhu-takaoka-string-matching-algorithm/)
...
a)在索引 2 处，字符串包含字符“C”，由于`模式[2] == “C”`，因此找到了字符匹配。所以我们现在要检查以前的索引，即 1，0。所以在`字符串[1]`(等于“B”)处，`模式[1] != 'B'`所以找不到匹配，是时候移动字符了。

## 不良字符移位计算表(命名为 ZTBC 表)

该阶段是预处理阶段，即应该在开始比较之前完成。坏字符表是一个散列图，它以模式的所有字母作为关键字，该值代表模式应该给出的移动次数，以便:
...
// Method
    ...
    // print the index at which the pattern matches
    // occurs
...
> **注:**
> *   运行时复杂度为 `O(字符串*模式)`，用于搜索和 `O(模式+ (26*26))`。
> *   发现空间复杂度为 `O(26×26)`，对于大型测试用例，该复杂度几乎不变。