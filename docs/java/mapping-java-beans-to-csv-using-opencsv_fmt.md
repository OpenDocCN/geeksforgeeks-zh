...
> 原文: [https://www.geeksforgeeks.org/mapping-java-beans-to-csv-using-opencsv/](https://www.geeksforgeeks.org/mapping-java-beans-to-csv-using-opencsv/)

将 Java Bean（对象）转换为 CSV 文件的需求非常普遍。将 Bean 写入 CSV 文件的方法有很多，但是将 Java Bean 映射到 CSV 的最佳方法之一是使用 OpenCSV 库。在 OpenCSV 中有一个类名 `StatefulBeanToCsvBuilder`，它有助于将 Java Beans 转换为 CSV。
...
3.  使用列位置映射策略将已创建对象的列映射到 CSV 的列。这是可选步骤。如果未使用 `ColumnPositionMappingStrategy`，则对象将被写入 CSV，列名与对象的属性名相同。
...
其中 `Employee` 是要映射到 CSV 的对象。

4.  创建 `StatefulBeanToCsvBuilder` 后，通过调用 `StatefulBeanToCsvBuilder` 类的 `build` 方法，以 `writer` 对象为参数，创建 `StatefulBeanToCsv` 类的对象。根据需要用户也可以提供：
...
*   `ColumnPositionMappingStrategy` 借助 `StatefulBeanToCsvBuilder` 对象的 `withMappingStrategy` 函数。
*   借助 `StatefulBeanToCsvBuilder` 对象的 `withSeparator` 函数对生成的 CSV 文件进行分隔。
*   借助 `StatefulBeanToCsvBuilder` 对象的 `withQuotechar` 函数，设置生成 CSV 文件的引号字符。

5.  创建 `StatefulBeanToCsv` 类的对象后，可以借助 `StatefulBeanToCsv` 对象的 `write` 方法将对象或对象列表添加到 CSV 文件中。
...
在本例中，我们将创建员工对象列表，该列表具有姓名、年龄、公司、工资等属性。然后我们将生成一个包含雇员对象的 CSV 文件 `Employees.csv`。
...
public class BeanToCSV {
    public static void main(String[] args) {
        ...
        try {
            ...
            Employee emp5 = new Employee("Prakash", "29", "Sath", "51000");
...
**参考:** [BeanToCsv 官方文档](http://opencsv.sourceforge.net/)