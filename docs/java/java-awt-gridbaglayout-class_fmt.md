...
*   `removeLayoutComponent(Component cmp)`: 从布局中移除指定的组件。
*   `getLayoutAlignmentY(Container p)`: 返回沿 y 轴的对齐。
*   `addLayoutComponent(Component cmp, Object cons)`: 将具有指定名称的指定组件添加到布局中。
*   `toString()`: 返回此网格包布局值的字符串表示形式。
*   `getLayoutAlignmentX(Container p)`: 返回沿 x 轴的对齐。
*   `getConstraints(Component cmp)`: 获取指定组件的约束。
*   `maximumLayoutSize(Container target)`: 给定指定目标容器中的组件，返回该布局的最大尺寸。
*   `minimumLayoutSize(Container parent)`: 使用此网格袋布局确定父容器的最小尺寸。

下面的程序说明了 `GridBagLayout` 类:

### 程序 1

下面的程序将几个行列组件排列在一个 `JFrame` 中，其实例类命名为 `GridbagDemo`。我们创建了 4 个名为 `java`、`布局`、`管理器`、`演示` 的组件，然后通过 `add()` 的方法将其添加到 `JFrame` 中。我们通过 `setSize()` 和 `setVisible()` 的方法设置框架的大小和可见性。布局通过 `setLayout()` 的方法设置。
...
下面的程序将几个行列组件排列在一个 `JFrame` 中，其实例类命名为 `GridbagDemo`。我们创建 5 个 `JButton` 组件，然后通过 `add()` 方法将它们添加到 `JFrame` 中。我们通过方法 `setTitle`、`setSize()` 和 `setVisible()` 设置框架的标题、大小和可见性。布局通过 `setLayout()` 的方法设置。
