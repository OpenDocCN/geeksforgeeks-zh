...
> 原文：[https://www.geeksforgeeks.org/duration-gettemporalunit-method-in-java-with-examples/](https://www.geeksforgeeks.org/duration-gettemporalunit-method-in-java-with-examples/)

`java.time`包中`Duration`类的`get(TemporalUnit)`方法用于获取作为参数传递的单位值。该方法仅支持单位秒和纳米，其余均导致异常。
...
该方法接受一个参数`unit`，该单位是要获取其值的时间单位。

## 返回值

该方法返回作为参数传递的单位的一个`long`值。
