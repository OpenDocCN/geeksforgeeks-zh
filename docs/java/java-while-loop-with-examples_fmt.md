...
> 原文: [https://www.geeksforgeeks.org/java-while-loop-with-examples/](https://www.geeksforgeeks.org/java-while-loop-with-examples/)

当我们需要重复执行一个语句块时，Java 中的[循环](https://www.geeksforgeeks.org/loops-in-java/)就开始使用了。

`Java while` 循环是一个控制流语句，允许基于给定的布尔条件重复执行代码。`while` 循环可以看作是一个重复的 `if` 语句。
