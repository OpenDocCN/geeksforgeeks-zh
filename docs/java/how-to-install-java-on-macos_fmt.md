...
**第一步:** 点击下载选项下载 `Java 17`，如下图所示，在需要下载*.dmg*的地方点击并进行下载程序。
...
**第三步:** 点击**继续按钮** -> 选择目的地 -> 安装 -> 输入您的密码即可获得安装 `java 17` 的权限。

> **注意:** 您可以通过单击返回按钮来更改安装位置。它不允许我更改大苏尔上的安装位置。

![](img/d5eddfb757b51be9bc7ada6a3111dc37.png)

**第四步:** 现在通过右键单击卸载如图所示的 `DMG` 映像，基本上我们正在删除我们在 `Windows` 中不太可能做的设置。
...
**第一步:** 开放终端 -> 类型: `which java`。它应该显示类似`/usr/bin/java`的内容。

![](img/d480ac2b5f62c370575fc4e80520a641.png)

> `JAVA_HOME` 是目录的完整路径，该目录包含一个名为“bin”的子目录，其中包含 `Java`。对于 `Mac OSX`，它是`/Library/Java/Home`。
...
> **注意:** 这只是为这个会话设置了 `JAVA_HOME`。如果你想让它永久存在，你必须把这个命令添加到你的 `~/.zshenv` 或 `~/.zshrc` 文件与 `macOS 10.15 Catalina` 以及更高版本一样，`zsh` 是默认的 `Terminal shell`。为此，请遵循以下说明:
