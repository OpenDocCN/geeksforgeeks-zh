...
> 原文: [https://www.geeksforgeeks.org/interrupting-a-thread-in-java/](https://www.geeksforgeeks.org/interrupting-a-thread-in-java/)

在 Java Threads 中，如果任何线程处于睡眠或等待状态（即调用 `sleep()` 或 `wait()`），则调用线程上的 `interrupt()` 方法，会引发 `InterruptedException`，从而打破睡眠或等待状态。如果线程不处于睡眠或等待状态，调用 `interrupt()` 方法会执行正常行为，不会中断线程，但会将中断标志设置为 `true`。
