...
> 原文：[https://www.geeksforgeeks.org/integer-valueof-method-in-java/](https://www.geeksforgeeks.org/integer-valueof-method-in-java/)

## 1. `Integer.valueOf(int a)`

`Java.lang.Integer.valueOf(int a)` 是一个内置方法，用于返回表示指定 `int` 值 `a` 的整数实例。
...
`Java.lang.Integer.valueOf(String str)` 是一个内置方法，用于返回一个 `Integer` 对象，保存指定字符串 `str` 的值。
...
下面的程序说明了 `java.lang.Integer.valueOf(String)` 方法：
...
`Java.lang.Integer.valueOf(String s, int radix)` 是一个内置的方法，它返回一个 `Integer` 对象，当用第二个参数给出的基进行解析时，保存从指定字符串中提取的值。
...
下面的程序说明了 `java.lang.Integer.valueOf(String, int base)` 方法：
