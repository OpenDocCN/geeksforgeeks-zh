...
`java.lang.Math.subtractExact()`是 java 中的一个内置数学函数，它返回参数的差值。如果结果溢出一个 `int`，它将引发异常。由于 `subtractExact(int a，int b)`是**静态的**，所以不需要创建对象。
...
展示 `java.lang.Math.subtractExact()`方法的工作。
