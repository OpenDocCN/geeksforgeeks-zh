...
> 原文: [https://www.geeksforgeeks.org/java-util-concurrent-cyclicbarrier-java/](https://www.geeksforgeeks.org/java-util-concurrent-cyclicbarrier-java/)

`CyclicBarrier` 用于使线程相互等待。当不同的线程处理一部分计算，以及当所有线程都完成执行后需要在父线程中组合结果时，会使用它。换句话说，当多个线程执行不同的子任务，并且这些子任务的输出需要被组合以形成最终输出时，使用 `CyclicBarrier`。执行完成后，线程调用 `await()` 方法，等待其他线程到达屏障。一旦所有线程都到达了，屏障就为线程的继续进行让路。

## 循环屏障的工作原理

`CyclicBarrier` 在 `java.util.concurrent` 包中定义。首先，创建一个 `CyclicBarrier` 的新实例，指定屏障应该等待的线程数。

```java
CyclicBarrier newBarrier = new CyclicBarrier(numberOfThreads);
```

每个线程都进行一些计算，在完成计算后，调用 `await()` 方法，如图所示：
...
一旦调用 `await()` 的线程数等于构造时指定的线程数，屏障就会为等待的线程让路。`CyclicBarrier` 也可以通过一些操作来初始化，一旦所有线程都到达了该屏障，就会执行这些操作。这个动作可以组合或利用在屏障中等待的单个线程的计算结果。
...
**返回：**
无。但将屏障重置为初始状态。如果目前有任何一方在栅栏处等待，他们将带着一个 `BrokenBarrierException` 返回。
...
**返回：**
如果一方或多方由于构造或上次重置后的中断或超时，或由于异常导致屏障动作失败，则返回 `true`；否则为 `false`。
...
### await (带超时)
等待，直到所有参与方已在此屏障上调用 `await`，或者指定的等待时间已过。
...
## 说明
因为子线程还没有运行设置 `sum` 和 `product` 变量的值，所以 `(sum + product) = 0` 的值打印在控制台上。接下来，控制台上会打印 `(sum + product) = 36`，因为子线程运行时设置了 `sum` 和 `product` 的值。此外，屏障上的等待线程数达到 3，因此屏障允许所有线程通过，最终打印出 36。“此时在屏障处等待的参与方数量”的值 = 0，因为所有三个线程都已经调用了 `await()` 方法，因此屏障不再活动。最后，`newBarrier` 被重置，可以再次使用。

## 屏障损坏
当任何等待的线程离开屏障时，屏障就会被破坏。当一个或多个等待线程被中断或等待时间结束时（因为线程调用了带超时的 `await()` 方法），会发生这种情况：
...
当屏障由于一个或多个参与线程而被打破时，所有其他线程的 `await()` 方法都会抛出一个 `BrokenBarrierException`。而已经在屏障中等待的线程的 `await()` 调用被终止。
...
## 参考
[甲骨文文档](https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/CyclicBarrier.html)
