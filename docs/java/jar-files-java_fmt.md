...
一个 [JAR (Java Archive)](https://www.geeksforgeeks.org/jar-files-java/) 是一种包文件格式，通常用于聚合许多 Java 类文件和相关的元数据和资源（文本、图像等）整合到一个文件中，以在 Java 平台上分发应用软件或库。
简单来说，JAR 文件是包含压缩版本的类文件、音频文件、图像文件或目录的文件。我们可以把 `.jar` 文件想象成一个压缩文件（`.zip`），是通过使用 WinZip 软件创建的。甚至，WinZip 软件可以用来提取 `.jar` 的内容。因此，您可以将它们用于无损数据压缩、归档、解压缩和归档解包等任务。

让我们看看如何创建一个 `.jar` 文件和相关的命令来帮助我们工作。

## 1.1 创建 JAR 文件

为了创建一个 `.jar` 文件，我们可以使用 `jar cf` 命令，如下所述：
...
> 这里，`cf` 表示创建文件。例如，假设我们的包 `pack` 在 `C:\` 目录中可用，为了将它转换成一个名为 `pack.jar` 的 jar 文件，我们可以给出如下命令：
>
> ```java
...
现在，创建了 `pack.jar` 文件。为了查看一个 JAR 文件，我们可以使用如下命令：
...
这里，`class1`、`class2` 等是包 `pack` 中的类。前两个条目表示创建了一个清单文件并将其添加到 `pack.jar` 中。第三个条目表示名为 `pack` 的子目录，后两个条目表示目录 `pack` 中的文件名。

> **注：** 我们创建 `.jar` 文件时，它会自动接收默认的清单文件。归档文件中只能有一个清单文件，并且它总是有路径名 `META-INF/MANIFEST.MF`。
