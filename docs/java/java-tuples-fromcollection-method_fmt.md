...
> 原文: [https://www.geeksforgeeks.org/java-tuples-fromcollection-method/](https://www.geeksforgeeks.org/java-tuples-fromcollection-method/)
...
该方法以 `collection` 为参数，其中:

*   `X` – 表示集合中值的数据类型。
*   `collection` – 表示要插入到 `TupleClass` 中的值的集合。
*   `TupleClass` – 代表像 `Unit`、`Quintet`、`Decade` 等使用的 JavaTuple 类。

## 返回值

该方法返回 `TupleClass` 的对象，该对象调用方法，集合的值作为参数传递。
...
### 程序 2: 使用带有 `Decade` 类的 `fromCollection()`
