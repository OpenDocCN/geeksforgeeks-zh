...
> 原文：[https://www.geeksforgeeks.org/find-free-disk-space-using-java/](https://www.geeksforgeeks.org/find-free-disk-space-using-java/)
...
返回此抽象路径名所命名的分区的大小。
**返回值：**
分区的大小（以字节为单位），如果此抽象路径名未命名分区，则返回 `0L`。
**抛出：**
`SecurityException` - 如果已安装安全管理器且它拒绝 `RuntimePermission("getFileSystemAttributes")` 权限，或者其 `SecurityManager.checkRead(String)` 方法拒绝读取此抽象路径名所命名的文件。
...
## 2. `Java.io.File.getUsableSpace()`
获取驱动器可用的总可用空间。返回由该抽象路径名命名的分区上该虚拟机可用的字节数。如果可能，此方法会检查写权限和其他操作系统限制，因此通常会提供比 `getFreeSpace()` 更准确的新数据实际写入量估计。
...
**返回值：**
分区上可用的字节数，如果此抽象路径名未命名分区，则返回 `0L`。在系统上此信息不可用时，此方法将等效于调用 `getFreeSpace()`。
**抛出：**
`SecurityException` - 如果已安装安全管理器且它拒绝 `RuntimePermission("getFileSystemAttributes")` 权限，或者其 `SecurityManager.checkRead(String)` 方法拒绝读取此抽象路径名所命名的文件。
...
## 3. `Java.io.File.getTotalSpace()`
获取驱动器的总容量。此方法返回此抽象路径名所命名的分区的大小。
...
**所用参考：** [Oracle](https://docs.oracle.com/javase/8/docs/api/java/io/File.html)
