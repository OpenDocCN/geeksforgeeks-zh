...
> 原文: [https://www.geeksforgeeks.org/equals-and-deepequals-method-to-compare-two-arrays-in-java/](https://www.geeksforgeeks.org/equals-and-deepequals-method-to-compare-two-arrays-in-java/)

`Arrays.equals()` 方法如果一个数组包含另一个数组，则不递归比较。`Arrays.deepEquals()` 方法如果一个数组包含另一个数组，则递归比较。
...
*   `Arrays.equals()` 方法如果一个数组包含另一个数组，则不会递归比较。
*   在所有其他情况下，它返回 `False`。

## [`Arrays.deepEquals(Object[], Object[])`](https://www.geeksforgeeks.org/java-util-arrays-deepequals-java/)
...
**注意:** 总是建议用 `Arrays.equals()` 比较非嵌套数组和 `Arrays.deepEquals()` 比较嵌套数组，因为 `Arrays.equals()` 在非嵌套数组的情况下比 `Arrays.deepEquals()` 快。
