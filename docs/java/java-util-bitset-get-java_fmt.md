...
## 1. `boolean get(int value)`

如果位集中存在 `value`，则返回 `true`，否则返回 `false`。
...
## 2. `BitSet get(int fromval, int toval)`

该方法返回一个新的 `BitSet`，该 `BitSet` 由从 `fromval`（包含）到 `toval`（不包含）的 `BitSet` 中存在的元素组成。
