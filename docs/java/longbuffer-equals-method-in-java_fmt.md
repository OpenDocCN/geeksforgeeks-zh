...
> 原文: [https://www.geeksforgeks.org/longbuffer-equals-method-in-java/](https://www.geeksforgeks.org/longbuffer-equals-method-in-java/)
...
如果 `(a == b) || (Long.isNaN(a) && Long.isNaN(b))`，此方法认为两个 `Long` 元素 `a` 和 `b` 相等。与 `Long.equals(Object)` 不同，值 `-0` 和 `+0` 被认为是相等的。
