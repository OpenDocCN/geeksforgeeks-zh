# Java 中的 java.util.function.DoublePredicate 接口，带示例

> 原文：[https://www.geeksforgeeks.org/java-util-function-doublepredicate-interface-in-java-with-examples/](https://www.geeksforgeeks.org/java-util-function-doublepredicate-interface-in-java-with-examples/)

`DoublePredicate` 接口在 `JDK 8` 中引入。该接口封装在 `java.util.function` 包中。它对 `Double` 对象进行操作，并根据条件返回谓词值。这是一个[功能界面](https://www.geeksforgeeks.org/functional-interfaces-java/)，因此也可以用于[λ表达式](https://www.geeksforgeeks.org/lambda-expressions-java-8/)。
