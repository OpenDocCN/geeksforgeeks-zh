...
> 原文：[https://www.geeksforgeeks.org/java-string-literals-as-constants-or-singletons/](https://www.geeksforgeeks.org/java-string-literals-as-constants-or-singletons/)
...
**图 2：** 如果我们希望这两个字符串变量指向单独的字符串对象，那么我们使用 `new` 运算符如下：
...
上面的代码将在内存中创建两个不同的内存对象来表示它们，即使两个字符串的值相同。下图描述了 `new` 运算符的使用：
