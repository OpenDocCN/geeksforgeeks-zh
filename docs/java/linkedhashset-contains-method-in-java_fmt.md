# LinkedHashSet 在 Java 中 contains() 方法

> 原文: [https://www.geeksforgeeks.org/linkedhashset-contains-method-in-java/](https://www.geeksforgeeks.org/linkedhashset-contains-method-in-java/)
...
参数 `element` 属于 `LinkedHashSet` 类型。这是需要测试的元素，无论它是否存在于集合中。

## 返回值

如果元素存在于集合中，方法返回 `true`，否则返回 `false`。
