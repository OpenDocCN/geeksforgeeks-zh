...
> 原文：[https://www.geeksforgeeks.org/integer-signum-method-in-java/](https://www.geeksforgeeks.org/integer-signum-method-in-java/)
...
该方法取一个整数类型的参数 `a`，对其进行运算。

## 返回值

该方法返回指定整数值的 signum 函数。如果指定值为负值，则返回 -1；如果指定值为零，则返回 0；如果指定值为正值，则返回 1。
