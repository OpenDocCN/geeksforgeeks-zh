...
> 原文：[https://www.geeksforgeeks.org/convert-list-of-characters-to-string-in-java/](https://www.geeksforgeeks.org/convert-list-of-characters-to-string-in-java/)
...
**List–** Java 中的 `List` 实现了管理有序集合的能力。它包括基于索引的插入、更新、删除和搜索列表元素的技术。它也可以有重复的元素。列表接口位于 `java.util` 包中，继承了 `Collection` 接口。
...
在 Java 8 中，我们可以利用带有 `Collectors` 的流 API。
执行上述方法：
