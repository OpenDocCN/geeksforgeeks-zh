# Java Guava | IntMath.checkedPow(int b, int k) 方法带示例

> 原文: [https://www.geeksforgeeks.org/java-guava-intmath-checkedpowint-b-int-k-method-with-examples/](https://www.geeksforgeeks.org/java-guava-intmath-checkedpowint-b-int-k-method-with-examples/)

`checkedPow(int b, int k)` 是 Guava 的 [`IntMath`](https://www.geeksforgeeks.org/intmath-class-guava-java/) 类中的一个方法，接受两个参数 `b` 和 `k`，用来求 `b` 的 `k` 次方。
...
如果 `b` 的 `k` 次方在有符号整数运算中溢出，方法 `checkedPow(int b, int k)` 抛出 `ArithmeticException`。
