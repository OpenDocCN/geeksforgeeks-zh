...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-simple-spring-application/](https://www.geeksforgeeks.org/how-to-create-a-simple-spring-application/)
...
`第四步`：运行 `SpringBootAppApplication` 类，等待 Tomcat 服务器启动。

![](img/093e5180d109d63d4fd76cdf5ca54c15.png)

> **注意**：Tomcat 服务器的默认端口是 `8080`，可以在 `application.properties` 文件中更改。

`第五步`：最后现在进入浏览器，输入网址 `localhost:8080`。观察输出，现在对 `localhost:8080/hello/aayus/2` 执行同样的操作。
