...
## 先决条件
[静态变量](https://www.geeksforgeeks.org/static-keyword-in-java/)、[最终关键字](https://www.geeksforgeeks.org/final-keyword-java/)

**静态变量:** 当一个变量的值没有变化时，那么去做实例变量就不是一个好的选择。届时，我们可以向该变量添加 `static` 修饰符。每当我们将变量声明为静态时，就会在类级别创建一个与对象共享的变量。静态变量中任何变化都会反映到其他对象的操作中。如果我们不初始化静态变量，那么缺省情况下，JVM 将为静态变量提供一个默认值。
...
*   Declaring them as `static final`s will help you create a `constant`. There is only one copy of the variable that cannot be reinitialized.

## 关于最终静态变量的要点

1.  **变量初始化是强制的**: 如果静态变量被声明为 `final`，那么无论我们是否使用它，都必须显式地进行初始化，JVM 不会为 `final static` 变量提供任何默认值。
...
2.  **类加载前初始化**: 对于 `final static` 变量，强制要求我们在类加载完成前进行初始化。我们可以在声明时初始化一个 `final static` 变量。
...
3.  **在静态块内部初始化**: 我们也可以在静态块内部初始化一个 `final static` 变量，因为我们应该在类加载之前初始化一个 `final static` 变量，而且我们知道静态块是在 `main()` 方法之前执行的。
...
除了上面提到的方法，如果我们试图在其他地方初始化一个 `final static` 变量，那么我们会得到编译时错误。
