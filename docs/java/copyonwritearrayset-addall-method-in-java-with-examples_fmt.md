...
> 原文: [https://www.geeksforgeeks.org/copyonwritearrayset-addall-method-in-java-with-examples/](https://www.geeksforgeeks.org/copyonwritearrayset-addall-method-in-java-with-examples/)

[`CopyOnWriteArraySet`](https://www.geeksforgeeks.org/copyonwritearrayset-in-java/)的 `addAll()` 方法将指定集合中不存在的所有元素添加到该 `CopyOnWriteArraySet` 中。这些方法导致两个集合的联合。
...
该方法接受一个参数 `c`，该参数是包含要添加到该集合中的元素的集合。

## 返回值

这个方法返回一个布尔值，比如 `true`，如果 `CopyOnWriteArraySet` 被改变的话。否则此方法返回 `false`。
...
在下面的程序中，指定的集合元素被添加到 `CopyOnWriteArraySet` 中。由于 50 在两个集合中都很常见，所以只添加一次。
