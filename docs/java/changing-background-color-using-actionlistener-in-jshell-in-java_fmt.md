...
> 原文: [https://www.geeksforgeeks.org/changing-background-color-using-actionlistener-in-jshell-in-java/](https://www.geeksforgeeks.org/changing-background-color-using-actionlistener-in-jshell-in-java/)
...
使用 `Swing` 类的概念来处理帧，可以在 [JShell](https://www.geeksforgeeks.org/jshell-java-9-new-feature/) 中创建一个帧，并且可以在该帧上添加一个按钮，其工作方式与 Swing 中的 `ActionListener` 相同。
...
```java
f ==> java.awt.Frame[frame, 0, 0, 0, 0x0, invalid, hidden, layout...t, title =, resizable, normal]
```

### 步骤 3: 设置框架的可见性

因为创建框架对象后，框架在屏幕上仍然不可见。
...
**语法:** 这里第一个参数是高度，第二个是 `setSize()` 函数中的宽度。
...
b1 ==> java.awt.Button[button0, 0, 0, 0x0, invalid, label=Click!!]
```
...
```java
jshell> f.add(b1)
$8 ==> java.awt.Button[button0, 0, 0, 0x0, invalid, label=Click!!]
```
...
```java
jshell> b1.addActionListener(e -> f.setBackground(Color.cyan));
```
...
jshell> Frame f = new Frame();
f ==> java.awt.Frame[frame0, 0, 0, 0x0, invalid, hidden, layo...t, title=, resizable, normal]
