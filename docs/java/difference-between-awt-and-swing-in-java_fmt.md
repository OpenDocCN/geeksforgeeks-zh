...
> 原文：[https://www.geeksforgeeks.org/difference-between-awt-and-swing-in-java/](https://www.geeksforgeeks.org/difference-between-awt-and-swing-in-java/)

Java 是开发各种应用程序最受欢迎的编程语言之一。Java 的流行可以归因于它的多功能性，因为它可以用来设计轻量级和快速的定制应用程序，并服务于从网络服务到安卓应用程序的各种目的。Java 是快速、可靠、安全。用 Java 开发基于 `GUI` 的应用程序有多种方式，其中最流行的是 `AWT` 和 `Swing`。

在讨论这些差异之前，让我们先了解每一个到底是什么。

## 1. AWT

`AWT` 代表**抽象视窗工具包**。用 Java 开发图形用户界面或基于窗口的应用程序是一个依赖于平台的应用编程接口。由太阳微系统公司于 1995 年开发。它在使用中非常重要，因为它是由系统的主机操作系统生成的。它包含大量的类和方法，用于创建和管理图形用户界面。
