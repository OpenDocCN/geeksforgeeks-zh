...
> 原文：[https://www.geeksforgeeks.org/how-to-implement-youtubeplayerview-library-in-android/](https://www.geeksforgeeks.org/how-to-implement-youtubeplayerview-library-in-android/)
...
### 第二步：在安卓工作室的 libs 文件夹里面添加 jar 文件

从链接[这里](https://github.com/ChaitanyaMunje/YoutubePlayerView/blob/master/app/libs/YouTubeAndroidPlayerApi.jar)下载 jar 文件。要添加此文件，请在如下图所示的“项目”模式下打开您的安卓项目。
...
### 步骤 3：现在在你的 build.gradle 文件中添加依赖项

添加这种依赖关系。导航至 `你的应用名称 > app`，你将看到 `build.gradle` 文件。在该文件中，在依赖项部分添加依赖项。
...
现在把左上角的项目标签改成安卓。之后导航至 `app > res > layout > activity_main.xml`。在这里，我们将创建一个简单的按钮，它将重定向到一个新的活动，我们将在那里播放我们的 YouTube 视频。下面是 `activity_main.xml` 文件的 XML 代码片段。
...
现在我们将创建一个新的活动，在那里我们将展示我们的 YouTube 视频播放器。要创建新活动，请导航至 `app > java > 您的应用程序的包名称`，然后右键单击它 > 新建 > 活动 > 空活动 > 为您的活动命名，并确保其语言保持为 `Java`。现在您的新活动已经创建。（这里我们给活动命名为 `VideoPlayerActivity`）。

### 第六步：在新活动内实现 YoutubePlayerView

下面是 `activity_video_player.xml` 文件的代码。
...
这样，我们可以获得任何视频的网址。现在回到 `VideoPlayerActivity.java` 文件并参考以下代码。以下是 `VideoPlayerActivity.java` 文件的代码。代码中添加了注释，以更详细地理解代码。
...
导航到应用程序>`AndroidManifest.xml`文件，在那里您必须添加以下权限。
