...
*   它包括一个草图簿，这是 IDE 的一个极简替代物。这个 `sketchbook` 可以作为一个普通的 IDE 来组织项目。
*   加工中绘制的每个草图都是 [`Java类`](https://www.geeksforgeeks.org/classes-objects-java/) (`PApplet`)的子类。这个类实现了几乎所有的处理功能。
...
为了用加工语言编码，用户可以从[官网](https://processing.org/)下载加工画册。除此之外，用户还可以下载代码 [`jar文件`](https://www.geeksforgeeks.org/jar-files-java/)，并在任何一个 IDE 中设置使用处理。

## 示例

下面是一个了解如何在处理中编码的示例。让我们看看如何在加工中画一个圆。为了做到这一点，我们需要了解处理从其库中调用的主要功能。这意味着，我们只需要定义这个函数，而不需要调用它。
