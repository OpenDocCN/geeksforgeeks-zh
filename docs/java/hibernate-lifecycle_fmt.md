...
瞬态是实体对象的第一个状态。当我们使用 `new` 操作符实例化一个 [`POJO` 类](https://www.geeksforgeeks.org/pojo-vs-java-beans/)的对象时，该对象处于瞬态。此对象未与任何 Hibernate 会话连接。因为它没有连接到任何 Hibernate 会话，所以这个状态没有连接到任何数据库表。因此，如果我们对 `POJO` 类的数据进行任何更改，那么数据库表就不会改变。瞬态对象独立于 Hibernate，存在于 `堆内存` 中。
...
在这里，我们为 `Employee` 类创建了一个新对象。下面是显示 `Employee` 对象初始化的代码:
...
一旦对象与 Hibernate 会话连接，对象就进入持久状态。因此，有两种方法可以将瞬态转换为持久状态:

1.  使用 Hibernate 会话，将实体对象保存到数据库表中。
2.  使用 Hibernate 会话，将实体对象加载到数据库表中。
...
要将对象从持久状态转换为分离状态，我们要么关闭会话，要么清除其缓存。由于此处会话已关闭或缓存已清除，因此对数据所做的任何更改都不会影响数据库表。只要需要，分离的对象就可以重新连接到新的 Hibernate 会话。要将分离的对象重新连接到新的 Hibernate 会话，我们将使用以下方法:
...
在 Hibernate 生命周期中，它是最后一种状态。在移除状态下，当实体对象从数据库中删除时，就知道实体对象处于移除状态。通过调用 `delete()` 操作来完成。由于实体对象处于移除状态，因此如果对数据进行任何更改，都不会影响数据库表。
