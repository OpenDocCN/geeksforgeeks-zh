# Java中CharArrayReader类的read(char[])方法详解及示例
...
Java中 [`CharArrayReader`](https://www.geeksforgeeks.org/java-io-chararrayreader-class-java/) 类的 `read(char[])` 方法用于将指定的字符读入数组。这个方法阻塞流直到：
*   它从流中获取了一些输入。
*   发生了 `IOException`。
*   在读取时已到达流的末尾。
...
## 参数
该方法接受一个强制参数 `charArray`，它是要写入流中的字符数组。
