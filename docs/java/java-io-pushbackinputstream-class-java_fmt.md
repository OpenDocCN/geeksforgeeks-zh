# Java 中的 `PushbackInputStream` 类

> 原文：[https://www.geeksforgeeks.org/java-io-pushbackinputstream-class-java/](https://www.geeksforgeeks.org/java-io-pushbackinputstream-class-java/)

推回输入流允许读取一个字节，然后将其返回（即“推回”）到流中。`PushbackInputStream` 类实现了这个想法。它提供了一种在不中断输入流的情况下“窥视”输入流的机制。
它扩展了 `FilterInputStream`。

**字段：**

*   **`protected byte[] buf`**：这是推回缓冲区。
*   **`protected int pos`**：这是推回缓冲区内的位置，下一个字节将从该位置读取。
*   **`protected InputStream in`**：这是要被过滤的输入流。

**构造函数：**

*   **`PushbackInputStream(InputStream in)`**：这将创建一个流对象，允许向输入流返回一个字节。
*   **`PushbackInputStream(InputStream in, int numBytes)`**：这将创建一个推回缓冲区长度为 `numBytes` 的流。这允许多个字节被返回到输入流。
...
3.  **`boolean markSupported()`**：测试该输入流是否支持 `mark` 和 `reset` 方法，但它不支持。
...
6.  **`void mark(int readlimit)`**：标记该输入流中的当前位置。
    `PushbackInputStream` 的 `mark` 方法不做任何事情。
...
7.  **`void reset()`**：将此流重新定位到上次在此输入流上调用 `mark` 方法时的位置。
    `PushbackInputStream` 的 `reset` 方法除了抛出一个 `IOException` 之外什么都不做。
...
# PushbackInputStream 类 `unread()` 方法详解
...
## 方法说明：`void unread(int b)`

通过将一个字节复制到推回缓冲区的前面来推回该字节。此方法返回后，下一个要读取的字节将具有值（字节）`b`。

```java
Syntax: public void unread(int b)
Returns: NA
Exception: IOException - If there is not enough room in the pushback buffer for the byte, or this input stream has been closed by invoking its close() method.
```
