...
> 原文: [https://www.geeksforgeeks.org/instant-plusnanos-method-in-java-with-examples/](https://www.geeksforgeeks.org/instant-plusnanos-method-in-java-with-examples/)

`Instant` 类的 `plusNanos()` 方法将作为参数传递的纳秒值添加到该瞬间，并将结果作为即时对象返回。这个返回的即时消息是不可变的。
...
*   `DateTimeException`。
*   `ArithmeticException`: If there is a numerical overflow.
...
参考文献: [https://docs.oracle.com/javase/10/docs/api/java/time/Instant.html#plusNanos(long)](https://docs.oracle.com/javase/10/docs/api/java/time/Instant.html#plusNanos(long))