...
> 原文：[https://www.geeksforgeeks.org/java-program-to-sort-names-in-an-alphabetical-order/](https://www.geeksforgeeks.org/java-program-to-sort-names-in-an-alphabetical-order/)

例如，按字母顺序对名称进行排序有多种方式对数组进行排序，如使用内置的 `Arrays.sort()` 方法或使用普通排序算法，如冒泡排序、合并排序。这里我们使用冒泡排序和内置排序。
...
其思想是在 unicode 的基础上比较字符串，并使用 `compareTo()` 方法根据两个字符串之间的比较结果根据返回的 int 值交换字符串。
...
*   使用 `comparison()` 方法将一个字符串与其他字符串进行比较。
*   基于两个字符串之间的比较结果交换元素。
*   按字母顺序打印排序后的名称。
