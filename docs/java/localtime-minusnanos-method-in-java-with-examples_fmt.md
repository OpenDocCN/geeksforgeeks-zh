...
> 原文: [https://www.geeksforgeeks.org/localtime-minusnanos-method-in-java-with-examples/](https://www.geeksforgeeks.org/localtime-minusnanos-method-in-java-with-examples/)

## 方法介绍
`LocalTime`类的`minusNanos()`方法用于从该`LocalTime`中减去指定的纳秒数，并将结果作为`LocalTime`对象返回。这一瞬间是不可变的。计算大约在午夜进行。
...
## 参数
该方法接受单个参数`nanosecondsToSubtract`，即需要减去的纳秒值，可以是负值。

## 返回值
该方法返回一个`LocalTime`，基于该时间减去纳秒。

## 示例程序
以下程序说明了`minusNanos()`方法：
...
## 参考文献
[https://docs.oracle.com/javase/10/docs/api/java/time/LocalTime.html#minusNanos(long)](https://docs.oracle.com/javase/10/docs/api/java/time/LocalTime.html#minusNanos(long))