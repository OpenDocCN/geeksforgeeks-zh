...
`Java.util.List` 是 `Collection` 的子接口。它是对象的有序集合，其中可以存储重复的值。因为列表保留了插入顺序，所以它允许元素的位置访问和插入。列表接口由 `ArrayList`、`LinkedList`、`Vector` 和 `Stack` 类实现。
...
很少有实现了列表接口的类是 `Stack`、`ArrayList`、`LinkedList`、`Vector` 等。
...
`Arrays.asList()` 方法返回一个由指定数组支持的固定大小的列表。
...
`Collections` 类中有多种方法可以用来实例化列表。

### 2.1 使用 `Collections.addAll()`
...
### 2.2 使用 `Collections.unmodifiableList()`

`Collections.unmodifiableList()` 返回一个不能修改的列表，即既不能添加元素，也不能删除元素。任何修改列表的尝试都将导致不支持操作示例。
...
### 2.3 使用 `Collections.singletonList()`
