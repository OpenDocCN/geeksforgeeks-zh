...
> 原文: [https://www.geeksforgeeks.org/atomicintegerarray-getandadd-method-in-java-with-examples/](https://www.geeksforgeeks.org/atomicintegerarray-getandadd-method-in-java-with-examples/)

`AtomicIntegerArray.getAndAdd()`是 Java 中的一个内置方法，它在 `AtomicIntegerArray` 的指定索引处自动给元素添加给定值。此方法将原子数组的索引值和要添加的值作为参数，并在添加操作之前返回该值。`getAndAdd()` 方法与 `addAndGet()` 类似，但前者返回加法运算前的值，后者返回加法运算后的值。
