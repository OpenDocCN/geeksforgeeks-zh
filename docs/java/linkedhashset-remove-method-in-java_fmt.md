...
> 原文: [https://www.geeksforgeeks.org/linkedhashset-remove-method-in-java/](https://www.geeksforgeeks.org/linkedhashset-remove-method-in-java/)
...
参数`O`属于链接哈希集类型，指定要从链接哈希集中删除的元素。

## 返回值

如果指定的元素存在于`LinkedHashSet`中，该方法返回`True`，否则返回`False`。

## 示例

下面程序举例说明了`remove(Object O)`方法：
