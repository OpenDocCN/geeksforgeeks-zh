...
> 原文: [https://www.geeksforgeeks.org/count-the-pairs-of-vowels-in-the-given-string/](https://www.geeksforgeeks.org/count-the-pairs-of-vowels-in-the-given-string/)
...
**方法:** 从字符串的第一个字符开始到第二个最后一个字符，为每个字符递增 `count`，其中 `str[i]` 和 `str[i + 1]` 都是元音。打印最后的 `count`，这是所需的对的计数。
