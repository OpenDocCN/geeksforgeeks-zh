...
2.  `LineTo(double x, double y)`：用指定的 x，y 坐标创建一个新的 `LineTo` 对象。
...
在这个程序中，我们将创建一个名为 `path` 的路径对象。用指定的 X 和 Y 坐标创建一个 `HLineTo` 对象。然后创建一个名为 `moveto` 的移动到对象。现在将 `moveto` 和 `Lineto` 对象添加到路径中。将此路径添加到 `Group` 对象并将 `Group` 对象添加到场景并将场景添加到舞台并调用 `show()` 函数显示最终结果。
...
在这个程序中，我们将创建一个名为 `path` 的路径对象。用指定的 X 和 Y 坐标名称创建四个 `LineTo` 对象 `Lineto`、`Lineto1`、`Lineto2`、`Lineto3`。然后创建一个名为 `moveto` 的移动到对象。现在将 `moveto` 和 `Lineto` 对象添加到路径中。将此路径添加到 `Group` 对象并将 `Group` 对象添加到场景并将场景添加到舞台并调用 `show()` 函数显示最终结果。
...
**参考:** [https://docs.oracle.com/javase/8/javafx/api/javafx/scene/shape/LineTo.html](https://docs.oracle.com/javase/8/javafx/api/javafx/scene/shape/LineTo.html)