...
> 原文: [https://www.geeksforgeeks.org/java-sql-timestamp-setnanos-function-with-examples/](https://www.geeksforgeeks.org/java-sql-timestamp-setnanos-function-with-examples/)

`setNanos()` 函数是 Java SQL 的 `Timestamp` 类的一部分。该函数用于设置时间戳对象秒值的小数部分。该函数将对象的纳秒值设置为给定值。
...
如果传递的参数小于 0 或大于 999999999，则函数抛出 `IllegalArgumentException`。

以下示例说明了 `setNanos()` 函数的使用。

## 示例 1

创建时间戳并使用 `setNanos()` 设置时间戳对象的小数部分。
