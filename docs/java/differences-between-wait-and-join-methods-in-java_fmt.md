...
> 原文：[https://www.geeksforgeeks.org/differences-between-wait-and-join-methods-in-java/](https://www.geeksforgeeks.org/differences-between-wait-and-join-methods-in-java/)

`wait()` 和 `join()` 方法用于暂停当前线程。`wait()` 与 `notifyAll()` 和 `notify()` 方法一起使用，但 `join()` 在 Java 中用于等待直到一个线程完成其执行。
...
*   `wait()` 和 `join()` 之间一个最重要的区别是，`wait()` 必须在同步上下文中调用，即 `synchronized` 块或方法，否则它将抛出 `IllegalMonitorStateException`。另一方面，我们可以在 Java 中有或没有同步上下文的情况下调用 `join()` 方法。
