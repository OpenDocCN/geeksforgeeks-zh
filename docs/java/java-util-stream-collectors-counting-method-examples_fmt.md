...
> 原文：[`https://www.geeksforgeeks.org/java-util-stream-collectors-counting-method-examples/`](https://www.geeksforgeeks.org/java-util-stream-collectors-counting-method-examples/)

在 Java 8 中，有一个由 `Collectors` 类 `counting()` 方法返回的预定义 `counting()` 方法来计算流中的元素数量。

> **语法：**
> `public static Collector counting()` 其中，输出是 `Collector`，作用于类型为 `T` 的元素流，其整理器返回类型为 `Long` 的“已收集”值。这是一个终端操作，它返回流中经过各种流水线流操作（如过滤）后到达 `collect()` 方法的元素总数。
