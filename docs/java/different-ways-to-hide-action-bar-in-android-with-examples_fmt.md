...
> 原文：[https://www.geeksforgeeks.org/different-ways-to-hide-action-bar-in-android-with-examples/](https://www.geeksforgeeks.org/different-ways-to-hide-action-bar-in-android-with-examples/)
...
如果你想在整个应用程序中隐藏动作栏（从所有活动和片段中隐藏），那么你可以使用这个方法。只需转到 `res -> values -> style.xml` 并将基础应用程序更改为 `Theme.AppCompat.Light.NoActionBar`。
...
隐藏操作栏的另一种方法是通过窗口管理器设置窗口管理器标志。这种方法使得用户与应用程序交互时隐藏操作栏变得更加容易。您可以使用下面代码中描述的 `setFlags` 函数。此外，您必须在活动的 `setContentView()` 之前使用标志。下面是隐藏动作栏的 java 文件：
...
### 4. 使用 `try-catch` 隐藏动作栏，使其不参与任何特定活动
