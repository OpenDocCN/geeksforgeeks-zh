...
> 原文: [https://www.geeksforgeeks.org/mvvm-model-view-viewmodel-architecture-pattern-in-android/](https://www.geeksforgeeks.org/mvvm-model-view-viewmodel-architecture-pattern-in-android/)
...
创建一个名为 `Model` 的新类，该类将保存用户输入的电子邮件标识和密码。下面是实现适当的模型类的代码。
...
打开 `activity_main.xml` 文件，添加 2 个 [`EditText`](https://www.geeksforgeeks.org/edittext-widget-in-android-using-java-with-examples/) 获取电子邮件和密码的输入。还需要一个登录按钮来验证用户的输入并显示适当的吐司消息。以下是设计适当活动布局的代码。

> **注意:** 为了数据绑定库的正常运行，需要在顶部设置 `<layout>` 标签。在这种情况下，XML 的`约束布局`标签将不起作用。
