...
> 原文: [https://www.geeksforgeeks.org/java-math-abs-method-examples/](https://www.geeksforgeeks.org/java-math-abs-method-examples/)
...
**异常抛出:** `ArithmeticException`

> **提示:** 必须知道泛型返回类型，如下所示:
> 
> *   **如果参数是 `double` 型或 `float` 型:**
>     *   如果参数为正零或负零，则结果为正零。
>     *   如果参数是无限的，结果就是正无穷大。
>     *   如果参数是 `NaN`，结果就是 `NaN`。
> *   **如果参数是 `int` 或 `long` 类型:** 如果参数等于 `Integer.MIN_VALUE` 或 `Long.MIN_VALUE`，最负的可表示整数或长值，结果是相同的值，为负。
