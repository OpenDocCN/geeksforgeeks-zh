...
> 原文：[https://www.geeksforgeeks.org/arraylist-removerange-java-examples/](https://www.geeksforgeeks.org/arraylist-removerange-java-examples/)

Java 中 [`ArrayList`](https://www.geeksforgeeks.org/arraylist-in-java/) 的 `removeRange()` 方法用于从数组列表对象中移除指定范围内的所有元素。它将任何后续元素向左移动。此调用通过(`toIndex`-`fromIndex`)元素来缩短列表，其中 `toIndex` 是结束索引，`fromIndex` 是开始索引，所有元素都将从该索引中删除。(如果 `toIndex`==`fromIndex`，此操作无效)
...
**错误：**
`IndexOutOfBoundsException`：如果 `fromIndex` 或 `toIndex` 超出范围(`fromIndex` < 0 或 `toIndex` > `size()` 或 `toIndex` < `fromIndex`)
...
**注意：** `removeRange(int fromIndex, int toIndex)` 方法是 `ArrayList` 中**保护的方法**。受保护的方法可以在类、子类和包中访问，但不能是公共的。因此，我们将该类扩展到 `ArrayList`。