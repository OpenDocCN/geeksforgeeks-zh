...
> 原文：[https://www.geeksforgeeks.org/how-to-implement-queue-in-java-using-array-and-generics/](https://www.geeksforgeeks.org/how-to-implement-queue-in-java-using-array-and-generics/)

## 队列介绍
队列是遵循先进先出规则的线性数据结构。我们不仅可以为整数实现队列，还可以为字符串、浮点或字符实现队列。队列中有 5 个主要操作：

1.  `enqueue()` 将元素 x 添加到队列的前面
2.  `dequeue()` 移除队列的最后一个元素
3.  `front()` 返回前置元素
4.  `rear()` 返回后方元素
5.  `empty()` 返回队列是否为空
