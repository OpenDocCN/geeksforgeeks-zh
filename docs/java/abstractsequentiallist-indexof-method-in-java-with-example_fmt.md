# Java 中的抽象顺序列表 indexOf() 方法，示例

> 原文：[https://www.geeksforgeeks.org/abstractsequentiallist-indexof-method-in-java-with-example/](https://www.geeksforgeeks.org/abstractsequentiallist-indexof-method-in-java-with-example/)

`java.util.AbstractSequentialList` 类的 `indexOf()` 方法用于返回该列表中指定元素第一次出现的索引，如果该列表不包含该元素，则返回 -1。更正式地说，返回最低的索引 `i`，这样 `(o==null ? get(i)==null : o.equals(get(i)))`，如果没有这样的索引，则为 -1。
...
该方法以 `Object o` 为参数，该参数是要搜索的元素。

## 返回值

该方法返回该列表中指定元素第一次出现的索引，如果该列表不包含该元素，则返回 -1。
