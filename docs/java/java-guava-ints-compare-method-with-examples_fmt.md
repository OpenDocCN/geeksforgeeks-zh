...
> 原文: [https://www.geeksforgeeks.org/java-guava-ints-compare-method-with-examples/](https://www.geeksforgeeks.org/java-guava-ints-compare-method-with-examples/)

`Ints.compare()` 是番石榴库中 [`Ints`](https://www.geeksforgeeks.org/ints-class-guava-java/) 类的一种方法，用于比较两个给定的 `int` 值。它将两个整数作为要比较的参数。它根据指定整数的比较结果返回一个比较器值。
