...
> 原文：[https://www.geeksforgeeks.org/java-program-to-implement-concurrentlinkeddeque-api/](https://www.geeksforgeeks.org/java-program-to-implement-concurrentlinkeddeque-api/)

Java 中的 [`ConcurrentLinkedDeque`](https://www.geeksforgeeks.org/concurrentlinkeddeque-in-java-with-examples/) 类是一个无界的并发 deque，它将其元素存储为链接节点，其中每个节点包含前一个和下一个节点的地址。属于 `java.util.concurrent` 包。这个类是 Java 集合框架的成员。它还扩展了 `Object` 和 `AbstractCollection` 类。
...
2.  `public ConcurrentLinkedDeque(Collection<? extends E> c)`: 它创建一个包含集合 `c` 元素的 Deque。

## 方法

| 方法 | 类型 | 描述 |
| --- | --- | --- |
| `add(E e)` | `boolean` | 在文件尾部插入一个元素 |
| `addAll(Collection<? extends E> c)` | `boolean` | 插入指定集合中的所有元素 |
| `addFirst(E e)` | `void` | 在背景的前面添加一个元素 |
| `addLast(E e)` | `void` | 在 deque 的最后添加一个元素 |
| `clear()` | `void` | 从列表中移除所有元素 |
| `contains(Object o)` | `boolean` | 如果目标包含对象 `o`，则返回真 |
| `descendingIterator()` | `Iterator<E>` | 以相反的顺序返回 deque 中元素的迭代器。 |
| `element()` | `E` | 取回了德格的头，但没有拿走 |
| `getFirst()` | `E` | 检索数据的第一个元素 |
| `getLast()` | `E` | 检索数据的最后一个元素 |
| `isEmpty()` | `boolean` | 如果目标不包含任何元素，则返回 true |
| `iterator()` | `Iterator<E>` | 返回 deque 中元素的迭代器 |
| `peek()` | `E` | 取回了德格的头，但没有拿走 |
| `peekFirst()` | `E` | 检索并删除标题 |
| `peekLast()` | `E` | 检索并删除尾部 |
| `push(E e)` | `void` | 将元素推送到由 deque 表示的堆栈上 |
| `pop()` | `E` | 从由 deque 表示的堆栈中弹出一个元素。 |
| `remove()` | `E` | 检索并删除队列的头 |
| `removeFirst()` | `E` | 检索并删除第一个元素 |
| `removeLast()` | `E` | 检索并删除最后一个元素 |
| `size()` | `int` | 返回目标的大小 |
| `toArray()` | `Object[]` | 返回包含所有元素的数组 |
