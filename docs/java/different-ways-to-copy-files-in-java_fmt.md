...
> 原文：[https://www.geeksforgeeks.org/different-ways-to-copy-files-in-java/](https://www.geeksforgeeks.org/different-ways-to-copy-files-in-java/)
...
这是一个简单的方法，我们使用 `FileInputStream` 从第一个文件获取输入字符，使用 `FileOutputStream` 将输出字符写入另一个文件。这就像看到一个文件并写入另一个文件。
...
*   `transferFrom(ReadableByteChannel src, long position, long count)`：将字节传输到从 `src` 通道调用此方法的通道。这是由目标通道调用的。`position` 是指针开始复制操作的位置。`count` 指定文件的大小，该大小几乎等于它包含的内容量。
*   `transferTo(long position, long count, WritableByteChannel target)`：将字节从文件的源通道或方法调用通道传输到目标通道。这个方法主要是使用源通道调用的，`count` 提到源文件的大小和拷贝的位置。
...
*   `copy(InputStream in, Path target)`：将所有字节的数据从输入文件流复制到输出文件的输出路径。它不能用于制作源文件中指定部分的副本。这里我们不需要创建输出文件。它是在代码执行期间自动创建的。
*   `copy(Path source, OutputStream out)`：将 `path` 源中指定文件的所有字节复制到输出文件的输出流中。
*   `copy(Path source, Path target)`：使用源文件和目标文件的路径复制文件。这里也不需要创建输出文件。
...
> **注意：** 在所有这些方法中，`stream` 的处理速度很快，但是如果有人想要技术上比他们更先进，可以选择其他两种方法。另外，`FileChannel` 方法为我们提供了许多选项来控制要复制的文件部分并指定其大小。