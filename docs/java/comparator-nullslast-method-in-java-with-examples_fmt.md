...
> 原文：[https://www.geeksforgeeks.org/comparator-nullslast-method-in-java-with-examples/](https://www.geeksforgeeks.org/comparator-nullslast-method-in-java-with-examples/)

`nullsLast(Comparator<T> comparator)` 方法返回一个比较器，它是一个空友好的比较器，并且认为空值大于非空值。`null` 首先通过以下逻辑进行操作：
...
该方法返回一个比较器，该比较器认为空值大于非空值，并将非空对象与提供的比较器进行比较。
...
[https://docs.oracle.com/javase/10/docs/api/java/util/Comparator.html#nullsLast(java.util.Comparator)](https://docs.oracle.com/javase/10/docs/api/java/util/Comparator.html#nullsLast(java.util.Comparator))