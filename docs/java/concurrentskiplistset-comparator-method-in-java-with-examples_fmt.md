...
`java.util.concurrent.ConcurrentSkipListSet` 的 `comparator()` 方法是 Java 中的一个内置函数，它返回用于给定集合中元素排序的比较器。如果使用自然排序，则返回 `null`。
...
## 返回值:
函数返回用于给定集合中元素排序的比较器，如果使用自然排序，则返回 `null`。
...
## 参考:
[https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/ConcurrentSkipListSet.html#comparator--](https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/ConcurrentSkipListSet.html#comparator--)