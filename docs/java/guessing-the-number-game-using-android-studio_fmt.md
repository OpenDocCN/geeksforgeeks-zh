...
> 原文: [https://www.geeksforgeeks.org/guessing-the-number-game-using-android-studio/](https://www.geeksforgeeks.org/guessing-the-number-game-using-android-studio/)
...
*   在 `activity_main.xml` 文件中添加以下代码。这里添加了两个 `TextView`、一个 `EditText` 和一个 `Button`。`TextViews` 用于显示消息，`EditText` 小部件供用户输入猜测的数字，按钮用于提交输入的值。
...
在 `activity_main.xml` 文件中添加上述代码后，用户界面会是这样的:
...
*   使用以下代码从 `EditText` 获取值：

> `EditText variable = (EditText)findViewById(R.id.editId);`

这里的 `editId` 是 `EditText` 的 ID，`variable` 是变量名。

*   在 `variable` 中，存储的数据是字符串形式的，因此使用下面的代码将其转换为整数：
