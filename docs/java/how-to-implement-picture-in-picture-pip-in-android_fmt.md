...
> 原文：[https://www.geeksforgeeks.org/how-to-implement-picture-in-picture-pip-in-android/](https://www.geeksforgeeks.org/how-to-implement-picture-in-picture-pip-in-android/)

本文介绍了如何在[安卓应用](https://www.geeksforgeeks.org/components-android-application/)中实现`画中画`。我们在`谷歌地图`等很多应用中看到，在使用导航时，当我们关闭应用时，屏幕右下方会出现一个浮动屏幕，如下图所示。该屏幕被称为画中画模式。
...
2.  **声明画中画支持：** 默认情况下，没有活动启用画中画模式。这需要通过清单文件来完成。
...
4.  现在，让我们在`MainActivity.java`文件中添加一些代码。在这个应用程序中，我们将通过点击按钮将活动更改为画中画模式。
    ...
    *   之后使用功能`enterPictureInPictureMode()`，应该会提供一个`PictureInPictureParams.Builder`参数。
