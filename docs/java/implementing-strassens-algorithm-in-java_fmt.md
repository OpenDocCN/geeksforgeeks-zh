...
> 原文：[https://www.geeksforgeeks.org/implementing-strassens-algorithm-in-java/](https://www.geeksforgeeks.org/implementing-strassens-algorithm-in-java/)
...
*   首先，算法的时间复杂度为 `O(n^3)`，太高了。
*   其次，两个以上矩阵相乘不仅会增加程序的混乱和复杂度，还会相应增加时间复杂度。

**目的：**

沃尔克·斯特拉森是一个发表了他的算法来证明一般矩阵乘法的时间复杂度 `O(n^3)` 不是最优的名字。于是发表了 Strassen 的矩阵链乘法，降低了时间复杂度。这种算法比标准矩阵乘法更快，在日常计算大量大型矩阵乘法时非常有用。

## 斯特拉森矩阵乘法算法

### 第一步
取三个矩阵，假设 `A`、`B`、`C`，其中 `C` 是结果矩阵，`A` 和 `B` 是用斯特拉森方法相乘的矩阵。

![](img/63bfa5abdc61c3f61946710fba2bd3bf.png)

### 第二步
将 `A`、`B`、`C` 矩阵分为四个 `(n/2)×(n/2)` 矩阵，取各矩阵的第一部分，如下图。
...
### 第四步
求解第一部分后，计算第二、第三和第四部分，以及最终输出，生成一个相乘的矩阵作为结果，如上图所示。
