...
> 原文：[https://www.geeksforgeeks.org/longstream-allmatch-java-examples/](https://www.geeksforgeeks.org/longstream-allmatch-java-examples/)

`LongStream allMatch(longPredicate predicate)`返回此流的所有元素是否与提供的谓词匹配。如果不是确定结果所必需的，它可能不会计算所有元素上的谓词。这是一次短路终端操作。如果终端操作在无限输入时可能在有限时间内终止，则该操作为短路。
...
`allMatch()`函数，检查两个 `LongStream` 串接后得到的 `LongStream` 中的所有元素是否都小于 2。
