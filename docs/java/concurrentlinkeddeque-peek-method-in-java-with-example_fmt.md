# Java 中的 ConcurrentLinkedDeque peek() 方法示例

> 原文：[https://www.geeksforgeeks.org/concurrentlinkeddeque-peek-method-in-java-with-example/](https://www.geeksforgeeks.org/concurrentlinkeddeque-peek-method-in-java-with-example/)

`java.util.concurrent.ConcurrentLinkedDeque.peek()` 方法用于检索或获取 Deque 头部的元素。检索到的元素不会被删除或从 Deque 中移除，相反，该方法只是返回它。如果 deque 中没有元素，则返回 `null`。
...
下面的程序举例说明了 `java.util.concurrent.ConcurrentLinkedDeque.peek()` 方法：
