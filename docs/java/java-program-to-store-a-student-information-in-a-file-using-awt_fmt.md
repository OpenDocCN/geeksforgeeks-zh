...
> 原文：[https://www.geeksforgeeks.org/java-program-to-store-a-student-information-in-a-file-using-awt/](https://www.geeksforgeeks.org/java-program-to-store-a-student-information-in-a-file-using-awt/)

`Swing` 是 `JFC (Java 基础类)`的一部分。用 Java 构建图形用户界面需要使用 Swings。`Swing Framework` 包含大量组件，这些组件允许高级定制并提供丰富的功能，用于创建基于窗口的应用程序。Java swing 组件是轻量级的，独立于平台，提供强大的组件，如表格、滚动面板、按钮、列表、颜色选择器等。
...
2.  接下来创建 `JLabels`、`JTextFields`、`JComboBoxes`、`JButtons` 并分别设置它们的边界。
3.  相应地命名这些组件并设置它们的边界。
