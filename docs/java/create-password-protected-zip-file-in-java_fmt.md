...
> 原文：[https://www.geeksforgeeks.org/create-password-protected-zip-file-in-java/](https://www.geeksforgeeks.org/create-password-protected-zip-file-in-java/)

首先，Java 没有任何函数或包可以在不使用一些本机代码的情况下创建受密码保护的 zip 文件。Java 有一些有用的库，这很好，但是它们使用一些本机代码来执行它们的任务，这使得它们的使用在某种程度上依赖于平台。而在 `zip4j` 库中，我们只使用 Java 代码，不支持任何本机代码，这就是人们使用 `zip4j` 胜过其他内置库的原因。
...
2.  它支持类似 `ZipInputStream` 和 `ZipOutputStream`。
