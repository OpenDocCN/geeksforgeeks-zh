...
> 原文：[https://www.geeksforgeeks.org/javafx-combobox-with-examples/](https://www.geeksforgeeks.org/javafx-combobox-with-examples/)

`ComboBox` 是 JavaFX 库的一部分。JavaFX `ComboBox` 是一个简单 `ComboBox` 的实现，它显示了一个项目列表，用户最多可以从中选择一个项目，它继承了类 `ComboBoxBase`。

**组合框的构造函数：**

1.  `ComboBox()`：创建默认的空组合框
2.  `ComboBox(ObservableList i)`：用给定的项目创建一个组合框

**常用方法：**

| 方法 | 说明 |
| --- | --- |
| `getEditor()` | 此方法获取属性编辑器的值 |
| `getItems()` | 此方法返回组合框的项 |
| `getVisibleRowCount()` | 此方法返回 `visibleRowCount` 属性的值。 |
| `setItems(ObservableList v)` | 此方法设置组合框的项目 |
...
该程序创建一个名为 `combo_box` 的组合框，并使用 `ComboBox(FXCollections.observableArrayList(week_days))` 向其中添加字符串列表。我们会将组合框和标签（描述）添加到 `TilePane`（`getChildren().add()` 函数）。我们将创建一个舞台（容器）并将 `TilePane` 添加到场景中，并将场景添加到舞台中。我们将使用 `show()` 函数显示舞台。
...
## 程序创建一个组合框，并向其中添加一个事件处理程序

这个程序创建一个名为 `combo_box` 的组合框，并使用 `ComboBox(FXCollections.observableArrayList(week_days))` 向其中添加一个字符串列表。我们会将组合框和标签（描述）添加到 `TilePane`（`getChildren().add()` 函数）。我们将创建一个舞台（容器）并将 `TilePane` 添加到场景中，并将场景添加到舞台中。我们将使用 `show()` 函数显示舞台。我们将添加一个事件处理程序事件来处理 `combo_box` 的事件，这将把所选标签的文本更改为所选项目。我们还会将选定的标签添加到平铺窗格中。
