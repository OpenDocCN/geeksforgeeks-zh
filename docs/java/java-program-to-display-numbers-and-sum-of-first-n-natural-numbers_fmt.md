...
> 原文：`https://www.geeksforgeeks.org/java-program-to-display-numbers-and-sum-of-first-n-natural-numbers/`

使用迭代方法打印前 `N` 个自然数，即使用 `for` 循环。`For` 循环有三个参数：初始化、测试条件和递增/递减。
...
4.  用 `0` 初始化变量 `和`。
5.  在 `for` 循环的每次迭代中，用 `和` 开始添加 `I`，并打印 `I`。
6.  在循环结束时打印总和。
