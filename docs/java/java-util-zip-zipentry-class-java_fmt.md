...
> 原文: [https://www.geeksforgeks.org/java-util-zip-zipentry-class-java/](https://www.geeksforgeks.org/java-util-zip-zipentry-class-java/)

这个类用于表示一个 ZIP 文件条目。

## 构造方法

- `ZipEntry(String name)`: 用指定的名称创建一个新的 zip 条目。
- `ZipEntry(ZipEntry e)`: 创建一个新的 ZipEntry，其字段取自指定的 ZipEntry。
...
- `String getComment()`: 返回条目的注释字符串，如果没有，则返回 null。
```java
...
- `boolean isDirectory()`: 如果这是一个目录条目，则返回真。目录条目被定义为名称以“/”结尾的条目。
```java
...
- `void setComment(String comment)`: 设置条目的可选注释字符串。ZIP 条目注释的最大长度为 0xffff。如果编码后指定注释字符串的长度大于 0xFFFF 字节，则只有前 0xFFFF 字节输出到 ZIP 文件条目。
```java
...
- `void setSize(long size)`: 设置条目数据的未压缩尺寸。
```java
