...
> 原文：[https://www.geeksforgeeks.org/how-to-solve-illegalargumentexception-in-java/](https://www.geeksforgeeks.org/how-to-solve-illegalargumentexception-in-java/)
...
下面的程序有一个单独的线程，暂停一下，然后尝试打印一个句子。这种暂停是使用 `sleep()` 方法实现的，该方法接受以毫秒为单位的暂停时间。Java 明确定义这个时间必须是非负数。让我们看看传入负值的结果。
