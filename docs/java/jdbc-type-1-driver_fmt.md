...
JDBC 驱动程序使 Java 应用程序能够与数据库交互，我们可以从中获取或存储数据。JDBC 驱动程序类似于 ODBC 驱动程序。JDBC 类包含在 Java 包中，用于：
*   连接到数据源，如数据库。
*   向数据库发送查询和更新语句。
...
`Type-1` 驱动程序，即 JDBC 驱动程序，也称为桥驱动程序。它提供了一个桥来访问安装在每个客户端上的 ODBC 驱动程序。类型 1 驱动程序将对 JDBC 方法的调用转换为对开放数据库连接 (ODBC) 函数的调用。桥驱动程序允许 JDBC 应用程序立即访问由现有的 ODBC 驱动程序阵列提供的数据库连接。

[ODBC](https://www.geeksforgeeks.org/difference-odbc-jdbc/) 基于设备驱动模型，驱动封装了将一组标准的命令和函数转换成底层系统所需的特定调用所需的逻辑。使用 JDBC-ODBC 桥驱动程序，我们可以访问只支持 ODBC 的数据库。Java 应用程序向 JDBC-ODBC 桥驱动程序发送一个请求，该请求在内部调用 ODBC 等效函数，ODBC 驱动程序从底层数据库中检索结果并将其发送回 JDBC-ODBC 桥驱动程序。
