...
原文: [https://www.geeksforgeeks.org/java-program-to-implement-linkedblockingqueue-api/](https://www.geeksforgeeks.org/java-program-to-implement-linkedblockingqueue-api/)
...
容量参数是防止队列过度扩展的一种方法。容量(如果未指定)等于 `Integer.MAX_VALUE`。每次插入时都会动态创建链接节点，除非这会使队列超出容量。
...
2.  `public LinkedBlockingQueue(Collection<? extends E> c)` – 创建一个容量为 `Integer.MAX_VALUE` 的 `LinkedBlockingQueue`，最初包含指定集合的元素。
3.  `public LinkedBlockingQueue(int capacity)` - 创建具有给定容量的 `LinkedBlockingQueue`。
...
| **方法** | **类型** | **描述** |
| --- | --- | --- |
| `clear()` | 空的 | 移除 `LinkedBlockingQueue` 的所有元素 |
| `contains(Object o)` | 布尔 | 如果队列包含指定的元素，则返回 `true`。 |
| `iterator()` | `Iterator<E>` | 返回队列中元素的迭代器。 |
| `offer(E e)` | 布尔 | 如果可能，将指定的元素插入该队列的尾部，并在成功时返回 `true`，否则返回 `false`。 |
| `offer(E e, long timeout, TimeUnit unit)` | 布尔 | 在队列的尾部插入指定的元素，并等待指定的时间使空间变得可用。 |
| `peek()` | E | 检索但不移除队列头。 |
| `poll()` | E | 检索并移除队列头。 |
| `put(E e)` | 空的 | 在这个队列的尾部插入指定的元素，必要时等待空间变得可用。 |
| `remainingCapacity()` | `int` | 返回此队列在没有阻塞的情况下理想情况下可以接受的附加元素的数量。 |
| `size()` | `int` | 返回队列的大小 |
| `toArray()` | `Object[]` | 将队列转换为数组 |
| `drainTo(Collection<? super E> c)` | `int` | 从队列中移除所有可用元素，并将它们添加到指定的集合中。 |
| `take()` | E | 检索并移除该队列的头，如有必要，等待直到某个元素变得可用。 |
| `remove(Object o)` | 布尔 | 从队列中移除指定的元素(如果存在)。 |