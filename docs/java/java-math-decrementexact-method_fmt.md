...
> 原文: [https://www.geeksforgeeks.org/java-math-decrementexact-method/](https://www.geeksforgeeks.org/java-math-decrementexact-method/)

`java.lang.Math.decrementExact()` 是 Java 中的内置函数，它返回减 1 的参数，如果结果溢出指定的数据类型 `long` 或 `int`，将引发异常，具体取决于在方法参数上使用了哪种数据类型。因为这是减量，所以如果结果小于最小值，我们将遇到异常的唯一情况。最小值可以从 `Long.MIN_VALUE` 或 `Integer.MIN_VALUE` 导出。
