...
用指定的名称和值创建一个 cookie。该名称必须仅包含 ASCII 字母数字字符，并符合 RFC 2965。如果名称不正确，它将引发 `IllegalArgumentException` 异常；如果名称为空，它将引发 `NullPointerException` 异常。该值可以是 cookie 想要存储的任何内容。
...
返回从头字符串解析的 cookies 列表。标头必须以 `set-cookie` 或 `set-cookie2` 标记开头，或者根本不能包含任何标记。
...
用于设置描述 cookie 用途的简短描述。它用于何时向用户呈现 cookie。
...
返回 cookie 的描述，如果 cookie 没有注释，则返回 `null`。
...
用于设置描述 cookie 用途的简短评论 URL。当浏览器将 cookie 呈现给用户时使用。
...
返回 cookie 的 URL 注释，如果 cookie 没有 URL 注释，则返回 `null`。
...
返回 `setDiscard()` 方法设置的丢弃变量的状态。更具体地说，如果 UA 要丢弃此 cookie，则返回 `true`，否则返回 `false`。
...
指定该 cookie 应该可见的域。例如，从 `bali.vacations.com` 的 servlet 发送的 cookies 通常不会被浏览器返回到 `queensland.vacations.com` 的页面。如果站点希望发生这种情况，servlets 可以指定 `cookie.setDomain(".vacations.com")`。为了防止服务器设置适用于域外主机的 cookies，指定的域必须满足以下要求:它必须以点开始(例如 `.coreservlets.com`)。
...
用于设置饼干的最大年龄(秒)。它指定在创建 cookie 后它存在的最长时间。负值表示浏览器一退出，cookie 就会过期。
...
用于指定客户端返回 cookie 的路径。此 cookie 对指定路径的所有页面和子目录都可见。例如，如果服务器从 `http://ecommerce.site.com/toys/specials.html` 发送了 cookie，浏览器将在连接到 `http://ecommerce.site.com/to/beginners.html` 而不是 `http://ecommerce.site.com/c/classic.html` 时发回 cookie。
...
Returns the path set for this cookie.
...
### 18. `getSecure()`
如果该 cookie 必须通过安全协议发送，则返回 true，否则返回 false。
...
### 22. `getVersion()`
如果 cookie 符合原网景规范，则返回 0；如果符合 RFC 2965/2109，则返回 1。
...
### 23. `setVersion()`
用于设置该 cookie 使用的 cookie 协议版本。

```java
Syntax : public void setVersion(int v) throws IllegalArgumentException
Parameters :
...
### 24. `isHttpOnly()`
如果 cookie 只能被 http 使用，即不能被 JS、vb 等脚本语言使用，则返回 true。
...
```java
Syntax : public static boolean domainMatches(String domain, String host)
Parameters :
...
### 28. `equals()`
如果两个 http cookies 彼此相等，则返回 true，否则返回 false。
...
### 29. `hashCode()`
返回这个 http cookie 的哈希代码。结果是这个 cookie 的三个重要组成部分：名称、域和路径的哈希代码值的总和。覆盖类对象中的哈希代码。
