# Java 中的 BufferedInputStream mark() 方法示例
...
Java 中 `BufferedInputStream` 类的 `mark()` 方法用于标记输入流中的当前位置。同一类的 `reset()` 方法在 `mark()` 方法之后调用。`reset()` 将位置固定在最后标记的位置，以便可以再次读取相同的字节。
...
覆盖 `FilterInputStream` 类的 `mark()` 方法。

## 参数

该方法接受整数类型的一个参数 `readlimit`，该参数表示在标记位置无效之前可以读取的字节的最大限制。
