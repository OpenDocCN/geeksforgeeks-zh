...
关系运算符决定一个操作数与另一个操作数之间的关系。关系运算符评估两个操作之间的关系，如果关系存在，则返回 `true` 否则返回 `false`。
...
Java 提供了几种按位运算符来处理整数类型，`long`、`int`、`short`、`char`、`byte`。按位运算符对整数的二进制表示执行逐位运算。这些运算符作用于操作数的各个位。
...
*   **`~` (按位补码)**: 按位 `~` 运算符对操作数逐位执行二进制非运算。`~b = 1000`，也就是 8
*   **`<<` (左移)**: 该运算符将左操作数的位左移右操作数指定的次数。`a << 1 = 10010 = 18`
*   **`>>` (右移)**: 此运算符将左操作数的位向右移动右操作数指定的次数。`a >> 1 = 0100 = 4`
*   **`>>>` (零补右移)**: 右移零补符。左操作数值向右移动由右操作数指定的位数，移动的位数将被零替换。`a >>> 2 = 0010 = 2`

## 4. 赋值运算符

赋值运算符用于给变量赋值。赋值运算符的一般形式是:
...
逻辑运算符执行逻辑运算，如逻辑与、逻辑或等。让我们假设变量 `a` 保存布尔值 `true` 和 `b` 保存布尔值 `false`。以下是我们可以使用的一些逻辑运算符:

*   **逻辑“与” (`&&`)**: 如果左右操作数都为真，则该运算符返回真，否则返回假。比如 `a && b` 就是 `false`。
*   **逻辑或 (`||`)**: 如果左右操作数中的任何一个为真，该运算符将返回真。当左右操作数都为假时，它将返回假。比如 `a || b` 就是 `true`。
*   **逻辑非 (`!`)**: 这是一元运算符，可以和单个操作数一起使用。如果操作数为假，则返回真；如果操作数为真，则返回假。比如 `!a` 是 `false` 和 `!b` 为 `true`。
