# Java 中的 AtomicInteger getAndAccumulate() 方法示例

> 原文：[https://www.geeksforgeeks.org/atomicinteger-getandaccumulate-method-in-java-with-examples/](https://www.geeksforgeeks.org/atomicinteger-getandaccumulate-method-in-java-with-examples/)

`AtomicInteger.getAndAccumulate()` 方法是一种内置方法，通过对当前值和作为参数传递的值应用指定的操作来更新对象的当前值。它以一个整数作为参数，以一个 `IntBinaryOperator` 接口的对象作为参数，并将对象中指定的操作应用于这些值。它返回前一个值。
