...
> 原文：[https://www.geeksforgeeks.org/java-program-to-convert-date-to-string/](https://www.geeksforgeeks.org/java-program-to-convert-date-to-string/)
...
*   `Calendar`类用于提供在任意时刻与一组日历字段进行交互的功能。`Calendar`类有一个内置的方法`getInstance()`，用来获取当前的日期和时间。它返回存储在日期类型变量中的日期。
*   `DateFormat`类是一个内置的 Java 类，用于解析日期或时间值并设置其格式。我们可以使用`SimpleDateFormat`类对象来指定日期的格式，该对象将日期格式作为参数，并返回日期格式说明符。
...
1.  获取要转换的日期。
2.  创建一个`SimpleDateFormat`类的实例来格式化日期对象的字符串表示。
3.  使用`Calendar`对象获取日期。
4.  使用`format()`方法将给定日期转换为字符串。
5.  打印结果。
...
## 方法三：使用 `LocalDate.toString()` 方法

1.  从`Date`获取`LocalDate`的实例。
2.  使用`LocalDate`类的`toString()`方法将给定日期转换为字符串。
3.  打印结果。
...
class GFG {
    ...
    public static String convertDateToString(String date)
    {
        ...
        // Convert the given date into a string using toString() method
