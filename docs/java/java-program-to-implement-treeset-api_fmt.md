...
> 原文：[https://www.geeksforgeeks.org/java-program-to-implement-treeset-api/](https://www.geeksforgeeks.org/java-program-to-implement-treeset-api/)
...
1.  创建一个类，包含它的树集的所有方法都被命名为 `TreeSetImplementation`
2.  实现上述 `TreeSet` API
...
`E floor(E ele)`
返回集合中小于或等于给定元素的最大元素，如果不存在则返回 `null`。

`E ceiling(E ele)`
返回集合中大于或等于给定元素的最小元素，如果不存在则返回 `null`。

`SortedSet<E> headSet(E ele)`
返回集合中元素严格小于指定元素的部分的视图。

`NavigableSet<E> headSet(E ele, boolean inclusive)`
返回集合中元素小于（或等于，如果 `inclusive` 为 `true`）指定元素的部分的视图。

`E higher(E ele)`
返回集合中严格大于给定元素的最小元素，如果不存在则返回 `null`。
...
`E lower(E ele)`
返回集合中严格小于给定元素的最大元素，如果不存在则返回 `null`。

`E pollFirst()`
检索并移除第一个（最小）元素，如果集合为空则返回 `null`。

`E pollLast()`
检索并移除最后一个（最大）元素，如果集合为空则返回 `null`。
...
`NavigableSet<E> subSet(E lele, boolean lInclusive, E rele, boolean rInclusive)`
返回集合中元素范围从 `lele` 到 `rele` 的部分的视图。

`SortedSet<E> subSet(E lele, E rele)`
返回集合中元素范围从 `lele`（包含）到 `rele`（不包含）的部分的视图。

`SortedSet<E> tailSet(E ele)`
返回集合中元素大于或等于 `ele` 的部分的视图。

`NavigableSet<E> tailSet(E ele, boolean inclusive)`
返回集合中元素大于（或等于，如果 `inclusive` 为 `true`）`ele` 的部分的视图。
...
public class GFG {
    public static void main(String[] arg) {
        ...
        TreeSetImplementation<Integer> set = new TreeSetImplementation<Integer>();
        ...
        // Iterator for iterate over TreeSet
        ...
        System.out.println("The first element in the set: " + set.first());
        System.out.println();

        // Print floor value of 45
        System.out.println("Floor value of 45: " + set.floor(45));
        System.out.println();

        // Print last element in the set
        System.out.println("The last element in the set: " + set.last());
        System.out.println();
