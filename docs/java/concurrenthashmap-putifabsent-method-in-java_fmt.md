...
`java.util.concurrent.ConcurrentHashMap.putIfAbsent()`是 Java 中的内置函数，它接受一个键和值作为参数，如果指定的键没有映射到任何值，则映射它们。
...
*   `key_elem`: 如果`key_elem`没有与任何值关联，此参数指定要将指定的`val_elem`映射到的键。
*   `val_elem`: 此参数指定要映射到指定`key_elem`的值。

## 返回值:

函数返回映射到键的现有值，如果之前没有值映射到键，则返回`null`。

## 异常:

当指定参数为空时，函数抛出[空指针异常](https://docs.oracle.com/javase/7/docs/api/java/lang/NullPointerException.html)。
...
[https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/ConcurrentHashMap.html#putIfAbsent(java.lang.Object, java.lang.Object)](https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/ConcurrentHashMap.html#putIfAbsent(java.lang.Object, java.lang.Object))