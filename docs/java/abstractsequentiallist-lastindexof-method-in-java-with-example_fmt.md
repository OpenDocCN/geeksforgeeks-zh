...
> 原文: [https://www.geeksforgeks.org/abstractsequentiallist-lastindexof-method-in-java-with-example/](https://www.geeksforgeks.org/abstractsequentiallist-lastindexof-method-in-java-with-example/)

`java.util.AbstractSequentialList`类的`lastIndexOf()`方法用于返回该列表中指定元素第一次出现的索引，如果该列表不包含该元素，则返回-1。更正式地说，返回最低的索引`i`，这样`(o==null ? get(i)==null : o.equals(get(i)))`，如果没有这样的索引，则为-1。
