...
> 原文：[`https://www.geeksforgeeks.org/delayqueue-poll-method-in-java-with-examples/`](https://www.geeksforgeeks.org/delayqueue-poll-method-in-java-with-examples/)

[`DelayQueue`](https://www.geeksforgeeks.org/delayqueue-class-in-java-with-example/) 的 `poll()` 方法用于检索延迟队列的头部。该方法还从延迟队列中移除头部。因此，当调用 `poll()` 方法时，延迟队列的大小减少一。
...
**异常：**
**`NullPointerException`：** 如果头部不存在，此方法将返回 `null`。
