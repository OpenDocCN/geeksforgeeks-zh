...
`ExceptionInInitializerError` 是 `Error` 类的子类，因此它是一个未检查的异常。当 JVM 试图加载一个新类时， [JVM](https://www.geeksforgeeks.org/jvm-works-jvm-architecture/) 会自动引发这个异常，因为在类加载期间，所有 `static` 变量和静态初始化程序块都在被评估。这个异常还充当一个信号，告诉我们在静态初始化器块中或者在静态变量的赋值中发生了意外的异常。

在 Java 程序中可能出现 `ExceptionInInitializerError` 的情况基本上有两种:

## 1. 为静态变量赋值时出现异常初始化错误

在下面的例子中，我们将一个静态变量 `x` 赋值为 `20 / 0`，其中 `20 / 0` 给出了一个未定义的算术行为，因此在[静态变量](https://www.geeksforgeeks.org/static-keyword-java/)赋值中出现了一个异常，最终我们将得到 `ExceptionInInitializerError`。
...
在下面的例子中，我们已经声明了一个静态块，在其中我们创建了一个字符串 `s`，并为其分配了一个 `null` 值，然后我们正在打印字符串的长度，因此我们将获得 [`NullPointerException`](https://www.geeksforgeeks.org/null-pointer-exception-in-java/)，因为我们试图打印一个值为 `null` 的字符串的长度，并且我们看到这个异常发生在静态块中，因此我们将获得 `ExceptionInInitializerError`。
...
*   我们可以通过确保类的静态初始值设定项块不会引发任何运行时异常来解决 `java.lang.ExceptionInInitializerError` 错误。
