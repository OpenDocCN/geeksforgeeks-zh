...
> 原文：[https://www.geeksforgeeks.org/compare-two-hashmap-objects-in-java/](https://www.geeksforgeeks.org/compare-two-hashmap-objects-in-java/)

在本文中，我们将学习如何在 Java 中比较两个 `HashMap` 对象。在 `java.util` 包中存在 `HashMap` 类。`HashMap` 用于存储键值对，因此有不同的场景来比较 `HashMap` 的两个对象。以下哪一项：
1.  比较条目
2.  比较密钥
3.  比较值
...
条目是键值对。我们可以通过将条目与地图的 `equals()` 方法进行比较来比较两个哈希表，如果地图具有相同的键值对，则意味着相同的条目。
...
我们可以通过比较使用 `keySet()` 方法获得的键来检查两个 `HashMap` 对象是否有相同的键。我们用 `equals()` 的方法设置比较键。
...
通过使用 `values()` 方法将所有地图值转换为集合，然后将值与集合的 `equals()` 方法进行比较，我们可以比较地图对象中包含的值是否相同。
