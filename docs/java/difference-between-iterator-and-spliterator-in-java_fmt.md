...
> 原文：[`https://www.geeksforgeeks.org/difference-between-iterator-and-spliterator-in-java/`](https://www.geeksforgeeks.org/difference-between-iterator-and-spliterator-in-java/)

`Java`迭代器接口表示一个能够遍历一组 `Java` 对象的对象，一次一个对象。迭代器接口是 `Java` 中最古老的对象集合迭代机制之一（虽然不是最古老的——枚举器早于迭代器）。
