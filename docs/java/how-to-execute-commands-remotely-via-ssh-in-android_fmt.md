...
> 原文：[https://www.geeksforgeeks.org/how-to-execute-commands-remotely-via-ssh-in-android/](https://www.geeksforgeeks.org/how-to-execute-commands-remotely-via-ssh-in-android/)
...
下面的教程说明了如何在安卓系统中实现同样的效果。注意，我们将使用 **Java** 语言来实现这个项目。
...
### 第三步：在你的 AndroidManifest.xml 文件中添加访问互联网的权限

在您的 `<manifest></manifest>` 标签内的 `src/main/AndroidManifest.xml` 文件中添加以下两行。

### 第四步：修改 strings.xml 文件

打开 `app > src > main > res > values > strings.xml` 并添加以下代码：
...
*   **MainActivity**：这里我们输入登录详细信息
*   **sshActivity**：在这里，我们将看到 shell 输出。

**使用 activity_main.xml 文件：**

现在，导航到 `app > src > main > res > layout > activity_main.xml` 并将下面的代码添加到文件中。
...
首先，通过右键单击活动文件夹（`com.example.gfg_ssh`）创建一个新的活动 `sshActivity`，然后选择 **新建 > 活动 > 空白活动**。现在将下面的代码添加到 `activity_ssh.xml` 文件中，用于此活动的 UI。
...
这是我们导入 `Apache MINA SSHD` 库并通过 SSH 连接到我们的远程机器的活动。我们从意向中获取登录数据。下面是 `sshActivity.java` 文件的代码。代码中添加了注释，以更详细地理解代码。
