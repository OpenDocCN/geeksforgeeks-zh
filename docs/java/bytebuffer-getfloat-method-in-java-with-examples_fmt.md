...
`java.nio.ByteBuffer` 类的 `getFloat()` 方法用于读取该缓冲区当前位置的下四个字节，按照当前字节顺序组成一个 `Float` 值，然后将该位置递增四。
...
如果这个缓冲区中剩余的字节少于 4 个，这个方法抛出 `BufferUnderflowException`。
...
`java.nio.ByteBuffer` 的 `getFloat(int index)` 方法用于读取给定索引处的四个字节，根据当前字节顺序将其组成一个 `Float` 值。
...
如果索引为负或不小于缓冲区的限制，此方法将抛出 `IndexOutOfBoundsException`。
...
**参考：** [`ByteBuffer.getFloat()`](https://docs.oracle.com/javase/9/docs/api/java/nio/ByteBuffer.html#getFloat--)

参考：[`getFloat-int-`](https://docs.oracle.com/javase/9/docs/api/java/nio/ByteBuffer.html#getFloat-int-)