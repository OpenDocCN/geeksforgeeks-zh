...
`java.nio.CharBuffer`类的`read()`方法用于将字符读入指定的字符缓冲区。缓冲区按原样用作字符存储库:所做的唯一更改是 put 操作的结果。不执行缓冲区的翻转或倒回。
...
这个方法返回添加到缓冲区的字符数，如果这个字符源在它的末尾，则返回-1。
...
*   `ReadOnlyBufferException` -If the destination is a read-only buffer
...
### 示例 3: 为 readonly buffer exception
...
[https://docs.oracle.com/javase/9/docs/api/java/nio/CharBuffer.html#read-java.nio.CharBuffer-](https://docs.oracle.com/javase/9/docs/api/java/nio/CharBuffer.html#read-java.nio.CharBuffer-)