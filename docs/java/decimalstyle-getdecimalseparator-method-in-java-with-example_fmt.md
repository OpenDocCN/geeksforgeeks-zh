...
java 中`Java.time.format.DecimalStyle`类的`getDecimalSeparator()`方法用于获取用于表示该 decimal style 的 Locale 的十进制分隔符的字符。此方法返回该区域设置的十进制分隔符的字符。
...
返回值：这个方法返回一个字符，代表这个十进制样式的十进制分隔符。
