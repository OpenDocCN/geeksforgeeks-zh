...
> 原文: [https://www.geeksforgeeks.org/matcher-reset-method-in-java-with-examples/](https://www.geeksforgeeks.org/matcher-reset-method-in-java-with-examples/)

`Matcher`类的`reset()`方法是用来重置这个`Matcher`的。为了更好的理解，建议有`java regex`子包中[Pattern和Matcher类](https://www.geeksforgeeks.org/regular-expressions-in-java/)的先验知识。在这里，我们将借助Java程序来说明这一点。
