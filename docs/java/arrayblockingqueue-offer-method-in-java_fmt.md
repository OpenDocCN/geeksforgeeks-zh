...
`offer(E element)` 方法将作为参数传递的元素插入到此队列（`ArrayBlockingQueue`）的尾部，如果队列未满。当添加操作成功时返回 `true`，如果此队列已满则返回 `false`。此方法优先于 `add()` 方法，因为 `add` 方法在队列满时会抛出错误，而 `offer()` 方法在这种情况下返回 `false`。
...
**参数:** 该方法取一个参数，`e`。这是指要添加到队列中的元素。

**返回值:** 当加法运算成功时，该方法返回 `true`，如果该队列已满，则返回 `false`。

**异常:** 如果指定的元素为空，该方法抛出 `NullPointerException`。
...
public class GFG {
    ...
    public void offerExample() {
        ...
        System.out.println("adding user having name = " + user1.name + ": " + queue.offer(user1));
        System.out.println("adding user having name = " + user2.name + ": " + queue.offer(user2));
        System.out.println("adding user having name = " + user3.name + ": " + queue.offer(user3));
        System.out.println("adding user having name = " + user4.name + ": " + queue.offer(user4));
        System.out.println("adding user having name = " + user5.name + ": " + queue.offer(user5));
        ...
        // Add users in queue
        System.out.println("adding user having name = " + user6.name + ": " + queue.offer(user6));
        System.out.println("adding user having name = " + user7.name + ": " + queue.offer(user7));
    }
}
```
...
`offer(E element, long timeout, TimeUnit unit)` 方法将作为参数传递的元素插入到此队列（`ArrayBlockingQueue`）的尾部，如果队列未满。如果队列已满，它将**等待指定的时间直到有空间可用**。当添加操作成功时返回 `true`，如果此队列已满且在空间可用之前经过了指定的等待时间，则返回 `false`。当我们希望等待队列移除其元素，并且当队列未满时元素将被添加到队列中，但我们可以等待一个特定的时间（这个时间可以由我们定义）时，此方法非常有用。
...
*   `e` (对象) - 队列中要添加的元素。
*   `timeout` (长整型) - 放弃前等待的时间，单位为 `unit`。
*   `unit` (时间单位) - 决定如何解释 `timeout` 参数的时间单位。
...
下面的程序说明了 `ArrayBlockingQueue` 的 `offer(E element, long timeout, TimeUnit unit)` 方法:
...
public class GFG {
    public static void main(String[] args) throws InterruptedException {
        ...
        System.out.println("adding 423: " + queue.offer(433, 10, TimeUnit.SECONDS));
        System.out.println("adding 456: " + queue.offer(456, 10, TimeUnit.SECONDS));
        System.out.println("adding 987: " + queue.offer(987, 10, TimeUnit.SECONDS));
        System.out.println("adding 578: " + queue.offer(578, 10, TimeUnit.SECONDS));
        System.out.println("adding 852: " + queue.offer(852, 10, TimeUnit.SECONDS));
    }
}
```
...
// Try to add more elements
`System.out.println("adding 546: " + queue.offer(546, 10, TimeUnit.SECONDS))`;
}
}
```
