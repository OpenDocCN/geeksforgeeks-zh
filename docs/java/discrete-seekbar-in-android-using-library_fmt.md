...
> 原文：[https://www.geeksforgeeks.org/discrete-seekbar-in-android-using-library/](https://www.geeksforgeeks.org/discrete-seekbar-in-android-using-library/)
...
| `DSB_indicatorPopupEnabled` | 用于显示气泡指示器。 |
| `dsb_value` | 当前值。 |
| `DSB_trackColor` | 可绘制轨道的颜色。 |
...
**第二步：在 `build.gradle` 文件中添加离散 SeekBar 库的依赖关系**

然后导航到渐变脚本，然后导航到 `build.gradle(模块)` 级别。在依赖项部分的 [`build.gradle`](https://www.geeksforgeeks.org/android-build-gradle/) 文件中添加以下行。
