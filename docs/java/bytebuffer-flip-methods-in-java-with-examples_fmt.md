...
[`java.nio.ByteBuffer`](https://www.geeksforgeeks.org/tag/java-bytebuffer/) 类的 [`flip()`](https://www.geeksforgeeks.org/tag/java-bytebuffer/) 方法用来翻转这个缓冲区。极限被设置为当前位置，然后位置被设置为零。如果标记被定义，那么它被丢弃。在一系列通道读取或放置操作之后，调用此方法为一系列通道写入或相关的获取操作做准备。
...
下面是一些示例来说明 [`flip()`](https://www.geeksforgeeks.org/tag/java-bytebuffer/)方法:
