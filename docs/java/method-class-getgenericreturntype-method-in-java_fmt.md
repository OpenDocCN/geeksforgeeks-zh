...
> 原文: [https://www.geeksforgeeks.org/method-class-getgenericreturntype-method-in-java/](https://www.geeksforgeeks.org/method-class-getgenericreturntype-method-in-java/)

[java.lang.reflect](https://www.geeksforgeeks.org/reflection-in-java/) 的 `getGenericReturnType()` 方法。`Method` 类返回一个 `Type` 对象，该对象表示编码时在方法中声明的返回类型。因此，`getGenericReturnType()` 方法返回 `Method` 对象的返回类型。

如果形式返回类型是参数化类型，则为其返回的类型对象必须准确反映源代码中使用的实际类型参数。例如，对于方法 `public T getValue(){}`，如果用参数化类型（即 `List`）替换类型 `T`，那么 `getGenericReturnType()` 将返回 `"java.util.List<java.lang.String>"` 作为返回类型。
...
返回一个 `Type` 对象，表示 `Method` 对象的形式返回类型。
...
```
*Explanation:*
In the above method when we going to apply `getGenericReturnType()` method it is going to return `T` as a generic return type.
...
```
*Explanation:*
In the above method when we going to apply `getGenericReturnType()` method it is going to return `java.util.List<java.lang.String>` as a generic return type.

下面的程序说明了 `Method` 类的 `getGenericReturnType()` 方法。

## 程序 1

通过在方法上应用 `getGenericReturnType()` 打印 `Method` 对象的返回类型。
...
给出一个 return 参数类型作为输入，检查 `Method` 对象是否有相同的 Return 类型。
