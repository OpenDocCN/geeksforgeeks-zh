# Java 中的整数 shortValue() 方法

> 原文: [https://www.geeksforgeeks.org/integer-shortvalue-method-in-java/](https://www.geeksforgeeks.org/integer-shortvalue-method-in-java/)

`Integer.shortValue()` 是 `java.lang` 包中的一个内置方法，它以 `short` 类型返回该整数的值。
...
方法将该对象转换为 `short` 类型后，返回该对象表示的整数值。
