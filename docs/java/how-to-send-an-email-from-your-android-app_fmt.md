...
> 原文：[https://www.geeksforgeeks.org/how-to-send-an-email-from-your-android-app/](https://www.geeksforgeeks.org/how-to-send-an-email-from-your-android-app/)

在本文中，您将制作一个基本的[安卓应用程序](https://www.geeksforgeeks.org/android-app-development-fundamentals-for-beginners/)，可以用来通过您的安卓应用程序发送电子邮件。

您可以在的帮助下这样做，动作与带有额外字段的`ACTION_SEND`相同：

*   `EXTRA_EMAIL`：您要向其发送邮件的邮件ID，
*   `EXTRA_SUBJECT`：电子邮件主题和
*   `EXTRA_TEXT`：邮件正文。
...
`activity_main.xml`包含一个相对布局，其中包含三个用于收件人邮件ID的`EditText`、另一个用于邮件主题和最后一个用于邮件正文的`EditText`、三个用于标签的`TextView`和一个用于开始意图或发送邮件的`Button`：
...
在`MainActivity`中，创建`Intent`对象并将其`action`定义为`ACTION_SEND`以发送邮件，同时使用`putExtra`函数为`Intent`添加三个额外字段。这些字段是：
*   收件人的电子邮件
*   电子邮件主题
*   电子邮件正文
