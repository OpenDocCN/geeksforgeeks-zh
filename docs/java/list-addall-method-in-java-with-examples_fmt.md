...
> 原文: [https://www.geeksforgeeks.org/list-addall-method-in-java-with-examples/](https://www.geeksforgeeks.org/list-addall-method-in-java-with-examples/)
...
该函数只有一个参数，即 `Collection c`，其元素将被追加到列表中。

## 返回

如果指定列表的元素被追加，列表发生变化，则返回真。
...
下面是用 `Linkedlist` 展示 `list.addAll()` 实现的代码。
