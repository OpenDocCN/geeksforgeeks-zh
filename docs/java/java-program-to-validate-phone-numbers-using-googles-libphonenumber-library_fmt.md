...
> 原文：[https://www.geeksforgeeks.org/java-program-to-validate-phone-numbers-using-googles-libphonenumber-library/](https://www.geeksforgeeks.org/java-program-to-validate-phone-numbers-using-googles-libphonenumber-library/)
...
| 返回类型 | 方法 | 描述 |
| --- | --- | --- |
| `String` | `format(PhoneNumber number, PhoneNumberUtil.PhoneNumberFormat numberFormat)` | 使用默认规则将电话号码格式化为指定的格式。 |
| `String` | `formatNumberForMobileDialing(PhoneNumber number, String regionCallingFrom, boolean withFormatting)` | 以格式化字符串形式返回号码，以便可以从该地区的手机拨打。 |
| `boolean` | `isMobileNumberPortableRegion(String regionCode)` | 如果作为参数传递的区域支持移动号码可移植性，则该方法返回 `true`。 |
| `boolean` | `isNumberMatch(PhoneNumber firstNumberIn, PhoneNumber secondNumberIn)` | 它需要两个电话号码，并检查它们是否相等。 |
| `boolean` | `isPossibleNumber(CharSequence number, String regionDialingFrom)` | 验证电话号码是否是以字符串形式给出的可能号码，以及可以拨打该号码的地区。 |
| `boolean` | `isValidNumber(PhoneNumber number, String regionCode)` | 检查给定的数字对于特定地区是否有效。 |
| `boolean` | `isValidNumber(PhoneNumber number)` | 验证电话号码是否符合特定模式。 |
| `boolean` | `canBeInternationallyDialled(PhoneNumber number)` | 如果可以从指定区域外拨打该号码，则返回 `true`。 |
| `int` | `getCountryCodeForRegion(String regionCode)` | 返回特定地区的国家/地区呼叫代码。 |
| `PhoneNumberUtil.PhoneNumberType` | `getNumberType(PhoneNumber number)` | 此方法根据数字本身返回数字的类型。例如，免费、移动、固定电话等。 |
