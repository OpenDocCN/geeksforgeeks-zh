...
[MongoDB](https://www.geeksforgeeks.org/mongodb-an-introduction/) 是一个使用 C++开发的[开源](https://www.geeksforgeeks.org/open-source-freeware-and-shareware-softwares/)跨平台文档数据库。MongoDB 的一些特性包括:
...
4.  最后，使用 `getDatabase()` 来访问数据库。
5.  用户名为:`GFGUser`，数据库名为`mongoDb`。
6.  `toCharArray()` 的功能用于将密码转换为字符数组。
7.  `getDatabase()` 的功能用于获取数据库。
...
创建集合使用 `com.mongodb.client.MongoDatabase` 类和 `createCollection()` 方法。这里，`database.createCollection()` 创建一个名为`GFGCollection`的集合。以下是创建集合的代码:
...
只有文档类型的数据可以插入 MongoDB。因此，我们可以使用 `append()` 方法创建一个要插入值的文档，或者使用 `insertOne()` 方法将文档直接传递到 MongoDB 中。

在这里，首先，我们创建了一个新文档作为`title`，然后添加`about`部分。然后，我们给文档赋予了各自的值。`insertOne()` 的功能用于将文档插入集合中。
