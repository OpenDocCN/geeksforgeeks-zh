...
> 原文：[https://www.geeksforgeeks.org/how-to-get-all-the-values-of-the-linkedhashmap-in-java/](https://www.geeksforgeeks.org/how-to-get-all-the-values-of-the-linkedhashmap-in-java/)
...
这里，`LHM` 是 `LinkedHashMap` 的名字。`list` 是我们列表的名字。
...
这里，`LHM` 是 `LinkedHashMap` 值的名称，`values` 是包含所有值的列表的名称。
...
由 [`values()`](https://www.geeksforgeeks.org/hashmap-values-method-in-java/) 方法返回的集合视图由原始的 `LinkedHashMap` 对象支持。
