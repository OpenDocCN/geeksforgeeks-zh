...
> 原文：[https://www.geeksforgeeks.org/how-to-find-the-minimum-or-maximum-element-from-the-vector-in-java/](https://www.geeksforgeeks.org/how-to-find-the-minimum-or-maximum-element-from-the-vector-in-java/)
...
1.  使用 [`Collections.min()`](https://www.geeksforgeeks.org/collections-min-method-in-java-with-examples/) 和 [`Collections.max()`](https://www.geeksforgeeks.org/collections-max-method-in-java-with-examples/) 方法。
2.  使用迭代向量元素来设置最小-最大值。
...
*   取一个变量，比如 `minNumber`，在比较和更新之前，将其初始化为最大值。
*   取一个变量，比如 `maxNumber`，然后将其初始化为最小值以便比较和更新。
*   遍历向量，并将每个元素与上述两个变量进行比较：
    *   `minNumber`
    *   `maxNumber`
*   如果数字小于 `minNumber`，则更新 `minNumber` 的值。
*   如果值大于 `maxNumber`，则更新 `maxNumber` 的值。
