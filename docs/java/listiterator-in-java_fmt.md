...
列表迭代器中没有**当前元素**。它的光标总是位于上一个和下一个元素之间。`previous()` 将返回到上一个元素，`next()` 将返回到下一个元素。因此，对于 n 长度的列表，有 n+1 个可能的游标。
...
**参数:** 该方法将第一个元素的 `索引` 作为从列表迭代器返回的参数(通过调用 `next`)。
...
| 迭代器 | **列表迭代器** |
| :--- | :--- |
| 它可以遍历任何类型的集合。 | 它只遍历列表集合实现的类，如[链表](https://www.geeksforgeeks.org/linked-list-in-java/)、[数组列表](https://www.geeksforgeeks.org/arraylist-in-java/)等。 |
| 遍历只能向前进行。 | 元素的遍历可以向前和向后进行。 |
| 迭代器对象可以通过调用集合类的 `iterator()` 方法来创建。 | `ListIterator` 对象可以通过调用集合类的 `listIterator()` 方法来创建。 |
| 不允许删除元素。 | 允许删除元素。 |
| 做加法运算时抛出 `ConcurrentModificationException`。因此，不允许添加。 | 允许添加元素。 |
| 在迭代器中，我们不能访问遍历元素的索引。 | 在 `ListIterator` 中，我们有 `nextIndex()` 和 `previousIndex()` 方法来访问被遍历或下一个遍历元素的索引。 |
| 不允许修改任何元素。 | 允许修改。 |
...
| 方法 | 描述 |
| :--- | :--- |
| `void add(E e)` | 此方法在列表中插入指定的元素。 |
| `boolean hasNext()` | 如果列表有更多元素要遍历，则返回 true。 |
| `boolean hasPrevious()` | 如果列表迭代器在向后遍历列表时有更多的元素，则返回 true。 |
| `E next()` | 此方法返回下一个元素，并将光标增加一个位置。 |
| `int nextIndex()` | 此方法返回元素的索引，该索引将在调用 `next()` 方法时返回。 |
| `E previous()` | 此方法返回列表的前一个元素，并将光标向后移动一个位置。 |
| `int previousIndex()` | 此方法返回调用 `previous()` 方法时将返回的元素的索引。 |
| `void remove()` | 此方法从调用 `next()` 或 `previous()` 方法元素时返回的列表中移除最后一个元素。 |
| `void set(E e)` | 此方法用指定的元素替换调用 `next()` 或 `previous()` 方法时返回的最后一个元素。 |

## 在接口 `java.util.Iterator` 中声明的方法

| 方法 | 描述 |
| :--- | :--- |
| `default void forEachRemaining(Consumer<? super E> action)` | 对剩余的每个元素执行给定的操作，直到所有元素都被处理完或者该操作引发异常。 |
