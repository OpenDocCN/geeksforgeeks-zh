...
> 原文：[https://www.geeksforgeeks.org/how-to-update-contents-of-a-table-using-jdbc-connection/](https://www.geeksforgeeks.org/how-to-update-contents-of-a-table-using-jdbc-connection/)
...
**1. 创建数据库：** 可以使用 `SQLyog` 创建一个数据库，并在其中创建一些表，并在其中填充数据，以便更新表的内容。这里比如我的数据库名称是 `hotelman`，表名是 `cuslogin` 和 `adminlogin`。我们将以 `cuslogin` 表为例。
