...
> 原文：[https://www.geeksforgeeks.org/java-thread-priority-multithreading/](https://www.geeksforgeeks.org/java-thread-priority-multithreading/)
...
*   我们将使用 [`currentThread()`](https://www.geeksforgeeks.org/naming-thread-fetching-name-current-thread-java/) 方法获取当前线程的名称。用户也可以使用 [`setName()`](https://www.geeksforgeeks.org/naming-thread-fetching-name-current-thread-java/) 方法，如果他/她为了理解的目的想要按照选择来命名线程的话。
*   [`getName()`](https://www.geeksforgeeks.org/method-class-getname-method-in-java/) 方法将用于获取线程的名称。
...
2.  `public final void setPriority(int newPriority)`：`java.lang.Thread.setPriority()` 方法将线程的优先级改为 `newPriority` 值。如果参数 `newPriority` 的值超过最小（1）和最大（10）限制，此方法将引发 `IllegalArgumentException`。
