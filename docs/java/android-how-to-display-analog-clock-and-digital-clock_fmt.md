...
1.  **模拟时钟:**模拟时钟是`View`类的子类。它代表一个圆形时钟。在圆圈的周围，数字 1 到 12 似乎代表小时，两只手用来显示时间的瞬间——短的表示小时，长的表示分钟。
...
### Step1
首先创建一个新的安卓应用程序。这将创建一个 XML 文件 `activity_main.xml` 和一个 Java 文件 `MainActivity.java`。请参考先决条件以了解更多关于此步骤的信息。

![](img/67ce776ee5bb4c4c7603e79a354914a9.png)

### Step2
打开 `activity_main.xml` 文件并在一个 `RelativeLayout` 中添加以下小部件：
*   一个**模拟时钟**
*   一个**数字时钟**
