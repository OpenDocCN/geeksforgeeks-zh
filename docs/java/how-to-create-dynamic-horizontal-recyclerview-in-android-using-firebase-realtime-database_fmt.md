...
> 原文: [https://www.geeksforgeeks.org/how-to-create-dynamic-horizontal-recyclerview-in-android-using-firebase-realtime-database/](https://www.geeksforgeeks.org/how-to-create-dynamic-horizontal-recyclerview-in-android-using-firebase-realtime-database/)
...
为了从 Firebase Firestore 数据库中读取数据，我们必须创建一个 Object 类，我们将在这个类中读取数据。要创建一个对象类，导航到`应用程序> java >你的应用程序的包名>右键点击它，然后点击新建> Java 类`并给你的类命名。这里我们已经给了名称为`CardModel`并添加了下面的代码。
...
要创建一个新的适配器类，导航到`应用程序> java >你的应用程序的包名>右键单击它，然后单击新建> java 类`，将你的 Java 类命名为`AdapterCard`，并添加下面的代码。代码中添加了注释，以更详细地理解代码。
