# Java 中 `EnumSet.of()` 方法的枚举

> 原文: [https://www.geeksforgeeks.org/enumset-of-method-in-java/](https://www.geeksforgeeks.org/enumset-of-method-in-java/)

## 1. `java.util.EnumSet.of(E ele1, E ele2, E ele3, …)` 方法

`java.util.EnumSet.of(E ele1, E ele2, E ele3, …)` 方法在 Java 中用于创建一个最初包含参数中指定元素的枚举集。当同时添加多个元素时，随着新元素的添加，旧元素会被向下推入集合。当在不同时间或迭代中添加不同元素时，旧元素会被替换。

### 语法

```java
EnumSet = EnumSet.of(E ele1, E ele2, E ele3, ...)
```
...
该方法返回一个枚举集，该枚举集最初包含通过参数传递的指定元素。

### 异常

如果传递的任何元素为 `null`，该方法将引发 `NullPointerException`。
...
## 2. `java.util.EnumSet.of(E_first, E_rest)` 方法

`java.util.EnumSet.of(E_first, E_rest)` 用于创建一个最初包含所有指定元素的枚举集。这个工厂方法的参数列表使用了可变参数特性，也可用于创建最初包含任意数量元素的枚举集，但其缺点是会使程序运行速度比不使用可变参数的重载方法慢。
...
下面的程序说明了 `of(E_first, E_rest)` 方法的工作原理：
...
public class Enum_Set_Demo {
    public static void main(String[] args) {
        // Creating the list that will be used as args
