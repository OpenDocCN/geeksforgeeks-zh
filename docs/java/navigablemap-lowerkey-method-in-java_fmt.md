...
`NavigableMap`接口的`lowerKey()`方法用于返回严格小于给定键的最大键，作为参数传递。更简单地说，这个方法用于在作为参数传递的元素之后查找下一个最大的元素。
...
该方法取一个强制参数`key`，即这是要匹配的键。

## 返回值

这个方法返回`最大的键严格小于到键`，或者`空值`如果没有这样的键。
...
*   `NullPointRexception`:当映射中指定的键为空并且使用自然的排序时，这意味着比较器不允许空键。
