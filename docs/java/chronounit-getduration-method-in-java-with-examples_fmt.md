...
> 原文: [https://www.geeksforgeks.org/chronounit-getduration-method-in-java-with-examples/](https://www.geeksforgeks.org/chronounit-getduration-method-in-java-with-examples/)

计时单位枚举的 `getDuration()` 方法用于返回该计时单位在国际标准化组织日历系统中的估计持续时间。白天因夏令时而异，而月份有不同的长度。
