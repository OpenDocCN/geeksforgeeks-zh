...
> 原文：[https://www.geeksforgeeks.org/file-isabsolute-method-in-java-with-examples/](https://www.geeksforgeeks.org/file-isabsolute-method-in-java-with-examples/)

## 方法介绍

`isAbsolute()` 方法是 [`File`](https://www.geeksforgeeks.org/File-class-in-java/) 类的一部分。该函数返回抽象路径名是否为绝对路径名。
...
该函数返回一个布尔值，该值告诉抽象路径名是否是绝对的。

下面的程序将说明 `isAbsolute()` 函数的使用。

### 例 1

给我们一个文件的文件对象，我们要检查它是不是绝对的。
