...
> 原文：[https://www.geeksforgeeks.org/how-to-create-swipe-navigation-in-an-android-app/](https://www.geeksforgeeks.org/how-to-create-swipe-navigation-in-an-android-app/)

## 介绍

说起[安卓应用](https://www.geeksforgeeks.org/top-programming-languages-for-android-app-development/)，首先想到的就是其丰富性。有太多种类的安卓应用为用户提供了漂亮的动态用户界面。其中一个功能是在我们的安卓应用程序中使用左右滑动来导航，而不是点击按钮。它不仅看起来更简单和优雅，而且为用户提供了方便的访问。有许多应用程序使用这种滑动功能来滑动应用程序中的不同活动。例如，流行的聊天应用 Snapchat 使用它来浏览镜头、聊天和故事。在这里，我们将学习如何在我们自己的安卓应用程序中实现滑动视图。

## 核心概念

我们可以通过使用两个特性来实现这一点：

1.  [`Fragment`](https://www.geeksforgeeks.org/introduction-fragments-android/)：一个片段只是一个活动的一部分。我们可以让一个片段占据屏幕的一部分或整个屏幕。或者我们可以同时展示多个片段，组成一个完整的屏幕。在一个活动中，我们也可以互相交换不同的片段。
2.  [`ViewPager`](https://developer.android.com/reference/androidx/viewpager/widget/ViewPager)：`ViewPager`是 Java 中的一个类，与 Fragments 结合使用。主要用于设计应用的 UI。

## 工作原理

首先我们需要使用`setAdapter()`方法在`ViewPager`上设置一个`Adapter`。我们设置的适配器叫做`FragmentPagerAdapter`，它是 Java 中的一个抽象类。因此，我们创建了自己的`SampleFragmentPagerAdapter`，它从`FragmentPagerAdapter`延伸出来，并在屏幕上显示我们的片段。当我们在设备中启动应用程序时，`ViewPager`会询问`SampleFragmentPagerAdapter`要刷过多少页。适配器的`getCount()`方法将这个答案返回给`ViewPager`。然后，`ViewPager`请求位于第 0 个位置的片段，适配器返回该特定片段，然后由`ViewPager`在我们的屏幕上显示。当我们向左滑动时，`ViewPager`向适配器询问第一个位置的片段，类似地，它会显示在屏幕上，并继续下去。

## 逐步实现

我们将创建三个片段，也就是用户可以刷过的三个屏幕。然后我们将这些片段添加到我们的`FragmentPagerAdapter`中，最后将其设置在`ViewPager`上。
...
*   我们把我们的片段命名为`Fragment1`。
*   `Fragment1`显示我们现在将要制作的布局`fragment_1.xml`。
...
### 步骤 3：创建 FragmentPagerAdapter

现在我们已经拥有了三个片段以及与之关联的三个布局，我们将构建我们的`FragmentPagerAdapter`并称之为`SimpleFragmentPagerAdapter`。这可以通过首先创建一个新的 Java 类并将其命名为`SimpleFragmentPagerAdapter`来完成。以下是它将包含的代码：
...
说明：
*   `getItem()`方法返回指定位置的片段。所以首先是`Fragment1`，向左滑动后，我们会得到相同顺序的其他碎片。
*   `getCount()`方法返回要显示的碎片数量，在本例中是三个。

### 步骤 4：创建 MainActivity 和布局
