# Java 中的 Collections.disjoint() 方法，带示例

> 原文：[https://www.geeksforgeeks.org/java-util-collections-disjoint-method-java-examples/](https://www.geeksforgeeks.org/java-util-collections-disjoint-method-java-examples/)

## 方法介绍

`java.util.Collections.disjoint()` 方法存在于 `java.util.Collections` 类中。它用于检查两个指定的集合是否不相交。更正式地说，如果两个集合没有共同的元素，它们就是不相交的。
...
- `NullPointerException` - 如果一个集合包含 `null` 元素，而 `null` 不是另一个集合的合格元素。
- `ClassCastException` - 如果一个集合包含一个元素，而该元素的类型对于另一个集合是不合格的。
...
`Arrays` 类没有不相交的方法。我们可以使用 `Collections.disjoint()` 来快速检查两个数组的分离。
