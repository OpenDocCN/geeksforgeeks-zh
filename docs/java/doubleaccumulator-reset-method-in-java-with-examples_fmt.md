# Java 中的 DoubleAccumulator.reset() 方法示例

> 原文：[https://www.geeksforgeeks.org/doubleaccumulator-reset-method-in-java-with-examples/](https://www.geeksforgeeks.org/doubleaccumulator-reset-method-in-java-with-examples/)

`DoubleAccumulator.reset()` 是 Java 中的一个内置方法，用于重置变量，同时保持对标识值的更新。这意味着它只返回复位值，不接受任何参数。返回类型是 `void`。
