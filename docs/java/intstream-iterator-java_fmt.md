...
`IntStream.iterator()`返回该流元素的迭代器。这是一个`终端操作`，也就是说，它可能穿越河流产生结果或副作用。
...
Where, `PrimitiveIterator.OfInt` is an Iterator specialized for int values.

## 返回值:

`IntStream.iterator()`返回该流的元素迭代器。
