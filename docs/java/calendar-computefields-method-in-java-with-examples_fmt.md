...
> 原文：[https://www.geeksforgeeks.org/calendar-computefields-method-in-java-with-examples/](https://www.geeksforgeeks.org/calendar-computefields-method-in-java-with-examples/)

`Calendar` 类中的 `computeFields()` 方法用于将当前毫秒时间值 `time` 转换为 `field[]` 中提到的日历字段值的转换操作。
