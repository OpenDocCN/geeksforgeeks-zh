...
> 原文：[https://www.geeksforgeeks.org/java-program-to-convert-double-to-string/](https://www.geeksforgeeks.org/java-program-to-convert-double-to-string/)

**问题陈述：** 使用内置函数在 Java 中将 `double` 数据类型转换为 `String` 的 Java 程序。这样做的主要目标是存储大的数字流，这些数字流来自于即使是数据类型也无法存储数字流的地方。
...
### 方法 1：使用 `String.valueOf()` 方法

`String.valueOf()` 是 Java 中的一个方法，它只是简单地将下面给定的参数类型化为字符串，因为它是 Java 中的一个内置方法，属于 `String` 类。请记住，在使用此方法执行一次后，任何执行都可能会改变结果，因为它将导致无指导的输出。
...
`Double.toString()`：在 Java 中每当调用 `print` 时，总是直接或间接地调用 Java 中 `Object` 类的 `toString()` 方法。即使没有使用这个内置函数，并且打印了双精度数字，也会调用 `toString()` 方法。`String` 是 Java 中的一个类。Java 中的所有类都是从 `Object` 类派生的。每当访问未创建 Java 程序中对象的打印时。每当用户用 Java 给出打印命令时，总是调用 Java 中 `Object` 类的 `toString()` 方法。
