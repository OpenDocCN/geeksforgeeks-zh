...
> 原文：[https://www.geeksforgeeks.org/augmented-faces-with-arcore-in-android/](https://www.geeksforgeeks.org/augmented-faces-with-arcore-in-android/)
...
我们将创建 Snapchat、Instagram 和抖音等人脸滤镜。下面给出了一个 `GIF` 示例，来了解一下在本文中我们要做什么。注意，我们将使用 `Java` 语言来实现这个项目。
...
在样本数据/模型文件夹中添加任何 3D 模型。我们可以通过在项目文件目录中创建一个新文件夹或者直接从安卓工作室创建一个新文件夹来实现这一点。允许的 3D 模型扩展包括 `.fbx`，`.obj`。网上有很多免费的模型。您可以访问[这里](https://poly.google.com/)或更多。您可以从[这里](https://drive.google.com/drive/folders/1KgTbs6SwiEJN-j0gP_IzWMGKZyitT3GH?usp=sharing)下载本例中使用的资产。请参考这篇文章[在安卓工作室创建一个原始文件夹](https://www.geeksforgeeks.org/resource-raw-folder-in-android-studio/)。然后只需将 `fox_face.sfb` 文件复制粘贴到 `raw` 文件夹即可。同样，将 `fox_face_mesh_texture.png` 文件复制粘贴到[可绘制文件夹](https://www.geeksforgeeks.org/how-to-add-image-to-drawable-folder-in-android-studio/amp/?ref=rp)。
...
将以下代码片段添加到 `build.gradle` 文件中。这是转换所必需的（仅一次）。`.fbx` 资产进入 `.sfb` 并将其保存在原始文件夹中。或者您可以按照步骤 2 中的操作自己添加它们。

> ```groovy
> //需要（仅一次）转换 .fbx 资产进入 .sfb
> //并保存在原始文件夹中
> sceneform.asset('sample data/models/fox_face.fbx',
>                   'default',
...
创建一个新的类，并将文件命名为 `CustomArFragment`，它扩展了 `Fragment`。以下是 `CustomArFragment.java` 文件的代码。
...
[output.mp4](https://media.geeksforgeeks.org/wp-content/uploads/20201120194043/output.mp4)

**Github 项目链接：** `https://github.com/raghavtilak/AugmentedFaces`

## ARCore 的局限性

1.  增强人脸仅适用于前置摄像头。
2.  并非所有设备都支持 `ARCore`。仍有一小部分设备不支持增强现实核心。您可以在 `https://developers.google.com/ar/discover/supported-devices` 查看 `ARCore` 支持的设备列表。
