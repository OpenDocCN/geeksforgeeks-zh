...
> 原文: [https://www.geeksforgeeks.org/enumset-noneof-method-in-java/](https://www.geeksforgeeks.org/enumset-noneof-method-in-java/)

`java.util.EnumSet.noneOf(Class<E> elementType)` 方法用于创建类型为 `elementType` 的空集合。
...
## 参数:
该方法接受一个元素类型的参数 `elementType`，并引用该枚举集的元素类型的类对象。
