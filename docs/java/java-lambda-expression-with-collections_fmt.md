...
> 原文: [https://www.geeksforgeeks.org/java-lambda-expression-with-collections/](https://www.geeksforgeeks.org/java-lambda-expression-with-collections/)
...
我们可以使用[`Comparator`](https://www.geeksforgeeks.org/comparator-interface-java/)接口进行排序，它只包含一个抽象方法：`compare()`。一个只包含一个抽象方法接口被称为函数式接口。

### `Comparator`接口的使用

要定义我们自己的排序，即我们自己定制的排序，那么我们应该使用`Comparator`的概念。
...
在定义我们自己的排序时，JVM 总是要调用`Comparator`的`compare()`方法。

- 当且仅当`obj1`必须在`obj2`之前时，返回负值(-1)。
- 当且仅当`obj1`必须在`obj2`之后时，返回正值(+1)。
- 当且仅当`obj1`和`obj2`相等时，返回零(0)。

在列表、集合、映射或其他任何地方，当我们想要定义自己的排序方法时，JVM 总是在内部调用`compare()`方法。

**当使用了函数式接口概念时，那么我们可以在它的位置使用 Lambda 表达式。**

## 使用 Lambda 表达式对列表的元素进行排序

使用 Lambda 表达式代替`Comparator`对象来定义我们自己在集合中的排序。
