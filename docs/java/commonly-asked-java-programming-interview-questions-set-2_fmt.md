...
> 原文：[https://www.geeksforgeeks.org/commonly-asked-java-programming-interview-questions-set-2/](https://www.geeksforgeeks.org/commonly-asked-java-programming-interview-questions-set-2/)
...
## Q5。什么是 Java 中的“this”关键词？

在实例方法或构造函数中，`this` 是对当前对象的引用，当前对象的方法或构造函数正在被调用。通过使用 `this`，可以从实例方法或构造函数中引用当前对象的任何成员。
该关键词的用法
...
## Q6。什么是抽象类？Java 中的抽象类和 C++有什么相似或不同？
...
`Java` 的主要方法是无地法。除了 `main()` 就像任何其他方法一样&可以以类似的方式重载之外，`JVM` 总是寻找方法签名来启动程序。
...
## Q10。C++中的继承与 Java 有何不同？

1.  在 `Java` 中，所有的类都直接或间接地继承自 `Object` 类。因此，`Java` 中总是有一个单一的类继承树，`Object` 类是树的根。
2.  在 `Java` 中，祖父母类的成员是不可直接访问的。详见本 G-Fact。
3.  在 `Java` 中，`protected` 成员访问说明符的含义有些不同。在 `Java` 中，类“A”的 `protected` 成员在同一个包的其他类“B”中是可访问的，即使 `B` 不是从 `A` 继承的(它们都必须在同一个包中)。
4.  `Java` 使用 `extends` 关键字进行继承。与 `C++` 不同，`Java` 不提供像 `public`、`protected` 或 `private` 这样的继承说明符。因此，我们不能在 `Java` 中更改基类成员的保护级别，如果某些数据成员在基类中是 `public` 或 `protected` 的，那么它在派生类中仍然是 `public` 或 `protected` 的。像 `C++` 一样，基类的 `private` 成员在派生类中是不可访问的。
    与 `C++` 不同，在 `Java` 中，我们不需要记住那些继承规则，它们是基类访问说明符和继承说明符的组合。
5.  在 `Java` 中，默认情况下方法是虚拟的。在 `C++` 中，我们显式地使用 `virtual` 关键字。详见本 G-Fact。
6.  `Java` 使用单独的关键字 `interface` 作为接口，`abstract` 关键字作为抽象类和抽象函数。
7.  与 `C++` 不同，`Java` 不支持多重继承。一个类不能从多个类继承。一个类可以实现多个接口。
8.  在 `C++` 中，父类的默认构造函数是自动调用的，但是如果我们要调用父类的参数化构造函数，就必须使用 Initializer list。像 `C++` 一样，父类的默认构造函数在 `Java` 中被自动调用，但是如果我们想调用参数化构造函数，那么我们必须使用 `super` 来调用父构造函数。
...
不，`private` 方法不能被覆盖，因为它在任何其他类中都不可见。阅读更多

## Q13。什么是空白最终变量？

`Java` 中的一个 `final` 变量只能赋值一次，我们可以在声明中赋值，也可以稍后赋值。
...
`Java` 中的一个**空白最终**变量是一个 `final` 变量，在声明过程中没有初始化。下面是一个简单的空白期末的例子。
...
## Q14。【超】在 java 中是什么关键词？

`Java` 中的 `super` 关键字是一个引用变量，用来引用父类对象。关键词“超级”带着继承的概念进入画面。无论何时创建子类的实例，都会隐式创建父类的实例，即由 `super` 引用变量引用。
使用 `java` 超级关键词的各种场景：
...
`Java` 中的 `static` 关键字主要用于内存管理。我们可以将 `java` `static` 关键字应用于变量、方法、块和嵌套类。`static` 关键字属于类，而不是类的实例。
...
## Q16。哈希表与 Java 中的哈希表的区别。

1.  `HashMap` 是非同步的。它不是线程安全的，没有适当的同步代码就不能在许多线程之间共享，而 `Hashtable` 是同步的。它是线程安全的，可以与许多线程共享。
2.  `HashMap` 允许一个空键和多个空值，而 `Hashtable` 不允许任何空键或空值。
3.  如果不需要线程同步，`HashMap` 通常比 `Hashtable` 更受欢迎。
