...
> 原文: [https://www.geeksforgeeks.org/differences-treemap-hashmap-linkedhashmap-java/](https://www.geeksforgeeks.org/differences-treemap-hashmap-linkedhashmap-java/)

先决条件: [Java 中的 HashMap 和 TreeMap](https://www.geeksforgeeks.org/hashmap-treemap-java/)
...
1.  **HashMap:** `HashMap` 提供 `O(1)` 查找和插入。但是，如果您遍历键，键的顺序基本上是任意的。它是由一组链表实现的。
    **语法:**
    ```java
...
2.  **LinkedHashMap:** `LinkedHashMap` 提供 `O(1)` 查找和插入。键按插入顺序排序。它由双链接桶实现。
    **语法:**
    ```java
...
3.  **TreeMap:** `TreeMap` 提供 `O(log N)` 查找和插入。键是有序的，所以如果需要按排序顺序遍历键，可以这样做。这意味着键必须实现 `Comparable` 接口。`TreeMap` 由红黑树实现。
    **语法:**
    ```java
    public class TreeMap extends AbstractMap implements
    NavigableMap, Cloneable, Serializable
    ```
    *   `TreeMap` 包含基于关键字的值。它实现了 `NavigableMap` 接口并扩展了 `AbstractMap` 类。
    *   它只包含独特的元素。
    *   它不能有空键，但可以有多个空值。
    *   它与 `HashMap` 相同，而是**保持升序(使用其键**的自然顺序排序)。

4.  **Hashtable:** “哈希表”是基于哈希的映射的通称。
    **语法:**
    ```java
...
1.  假设您正在创建一个名称到 `Person` 对象的映射。您可能希望定期按姓名的字母顺序输出人员。`TreeMap` 可以让你做到这一点。
2.  `TreeMap` 还提供了一种方法，给定一个名字，输出下 10 个人。这对于许多应用中的“更多”功能可能是有用的。
