...
1.  `add(Component c)`: 向容器中添加组件。
2.  `addActionListener(ActionListener d)`: 添加指定组件的 `ActionListener`。
3.  `setBackground(Color c)`: 设置指定容器的背景颜色。
4.  `setSize(int a, int b)`: 将容器的大小设置为指定的尺寸。
5.  `setText(String s)`: 将标签的文本设置为 `s`。
6.  `getText()`: 返回标签的文本。

## Java程序使用 java swing 元素创建一个带有 basic +、-、/、*的简单计算器
