...
> 原文: [https://www.geeksforgeeks.org/iterate-through-linkedhashmap-using-an-iterator-in-java/](https://www.geeksforgeeks.org/iterate-through-linkedhashmap-using-an-iterator-in-java/)
...
1.  创建一个链接哈希表并添加键、值对。
2.  我们使用 `Set s = lhm.entrySet();` 将链接的哈希表转换为 [`entrySet`](https://www.geeksforgeeks.org/hashmap-entryset-method-in-java/)。
3.  我们为集合定义迭代器：`Iterator it=s.iterator();`。
4.  使用 `while` 循环，我们遍历我们的 `linkedHashMap`：`while(it.hasNext()) System.out.println(it.next());`。
