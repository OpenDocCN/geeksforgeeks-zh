...
> 原文：[https://www.geeksforgeeks.org/bigdecimal-add-method-in-java-with-examples/](https://www.geeksforgeeks.org/bigdecimal-add-method-in-java-with-examples/)

`BigDecimal.add(BigDecimal val)` 用于计算两个 `BigDecimal` 的算术和。这种方法用于寻找比 Java 的最大数据类型 `double` 的范围大得多的大量范围的算术加法，而不损害结果的精度。此方法对调用此方法的当前 `BigDecimal` 数和作为参数传递的 `BigDecimal` 数执行操作。
...
**参数：** 该方法接受一个参数 `val`，该值是要添加到此 `BigDecimal` 中的值。

**返回值：** 这个方法返回一个 `BigDecimal`，它保存 `sum (this + val)`，其小数位数是 `max(this.scale(), val.scale())`。
...
**参数：** 该方法接受两个参数，一个是 `val`，该值是要添加到此 `BigDecimal` 中的值，另一个是类型为 `MathContext` 的 `mc`。
