...
> 原文：[https://www.geeksforgeeks.org/how-to-get-size-minimum-and-maximum-value-of-data-types-in-java/](https://www.geeksforgeeks.org/how-to-get-size-minimum-and-maximum-value-of-data-types-in-java/)

数据类型的**大小**由`(数据类型名称).SIZE`给出。其可存储的**最大值**由`(数据类型名称).MAX_VALUE`给出。其可存储的**最小值**由`(数据类型名称).MIN_VALUE`给出。

> 总是用大写字母写数据类型的第一个单词。

**示例：**

**1.** 如果要打印浮点数据类型的大小，请使用`Float.SIZE`。
