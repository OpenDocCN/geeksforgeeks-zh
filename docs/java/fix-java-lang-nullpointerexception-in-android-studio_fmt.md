# 修复安卓工作室中的“java.lang.NullPointerException”

> 原文: [https://www.geeksforgeeks.org/fix-java-lang-nullpointerexception-in-android-studio/](https://www.geeksforgeeks.org/fix-java-lang-nullpointerexception-in-android-studio/)

嘿，极客们，今天我们将看到 `NullPointerException` 是什么意思，以及我们如何在 Android Studio 中修复它。要理解 `NullPointerException`，我们必须理解 `null` 的含义。

## 什么是 null？

“`null`”是所有程序员中非常熟悉的关键词。它基本上是引用数据类型或变量的文字，如数组、类、接口和枚举。每个基本数据类型都有一个默认值（例如：布尔值为 `true` 和 `false`）。同样，如果在声明过程中没有初始化引用数据类型变量，则默认为 `null` 值。
...
同样重要的是要注意，我们不能直接将 `null` 值存储在基本数据类型变量或对象中，如下所示。
...
当应用程序或程序试图访问存储有 `null` 值的对象引用（访问方法）时，会出现运行时异常。当我们如下所示声明后不初始化它时，`null` 值会自动存储在引用变量中。
...
Android Studio 中的 `NullPointerException` 在下面的截图中以黄色突出显示。

![](img/4e9b475b02ca6a5953d92c84380aa179.png)

如上图所示，它包含一个初始化为 `null` 的 `TextView`。

```java
TextView textview = null;
```

`TextView` 引用变量（即 `textview`）被访问，这给出了一个空指针异常。
...
![](img/a8d059a1b364d3b639ed9731722f4b7a.png)
...
正如您在初始化 `TextView` 组件后看到的，我们已经解决了 `NullPointerException`。因此，通过这种方式，我们可以摆脱安卓工作室中的空指针异常。