...
> 原文: [https://www.geeksforgeeks.org/java-io-pipedoutputstream-class-java/](https://www.geeksforgeeks.org/java-io-pipedoutputstream-class-java/)
...
*   `PipedOutputStream(PipedInputStream in)` : 创建一个管道输出流，它连接到管道输入流–“输入流”。

## 方法

*   `write(int byte)` : `Java.io.PipedOutputStream.write(int byte)` 将指定的字节写入管道输出流。
...
*   `connect(PipedInputStream destination)` : `Java.io.PipedOutputStream.connect(PipedInputStream destination)` 将管道输出流连接到“目的地”管道输入流，如果“目的地”是带有其他流的管道，则会引发 IO 异常。
