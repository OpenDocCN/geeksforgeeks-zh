...
`isBefore()` 方法的 `Instant` 类检查这个即时时间线位置是否在作为参数传递的即时之前。如果该时刻时间线位置在作为参数传递的时刻之前，则该方法将返回 true 或 false。比较是基于瞬间的时间线位置。
...
该方法取一个参数 `otherInstant` ，作为比较的另一个时刻。它不应为空。

## 返回:

如果该时刻在指定时刻之前，则该方法返回 `true`。

## 异常:

如果 `otherInstant` 为空，该方法抛出 `NullPointerException`。
