...
`HashMap` 类的 `replaceAll(BiFunction)` 方法将每个值替换为对相应值应用给定函数（执行特定操作）的结果。这个过程以同样的方式继续，直到所有条目都被处理完，或者直到函数抛出一个异常。它会重新引发替换函数引发的异常。
...
*   `NullPointerException`（可选）：当给定函数指向空值或新值尚未初始化，因此引用空值时，它会抛出一个空指针异常。
    **示例：** 调用空对象的实例方法。
*   `UnsupportedOperationException`（可选）：抛出，表示此映射不支持请求的操作。
    **例：** 在 `ArraysList` 类中如果我们使用 `add` 或者 `remove` 方法就会抛出 `UnsupportedOperationException`。

以下程序说明了 `replaceAll(BiFunction)` 方法：
...
**参考：** [https://docs.oracle.com/javase/10/docs/api/java/util/Map.html#replaceAll(java.util.function.BiFunction)](https://docs.oracle.com/javase/10/docs/api/java/util/Map.html#replaceAll(java.util.function.BiFunction))