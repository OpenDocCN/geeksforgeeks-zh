...
> 原文:[https://www.geeksforgeeks.org/how-to-change-the-whole-app-language-in-android-programmatically/](https://www.geeksforgeeks.org/how-to-change-the-whole-app-language-in-android-programmatically/)

安卓 7.0(API 级别 24)为多语言用户提供支持，允许用户在设置中选择多个语言环境。一个`Locale`对象代表一个特定的地理、政治或文化区域。需要这些区域设置来执行任务的操作称为**区域敏感**，并使用区域设置为用户定制信息。例如，显示一个数字是一项区分区域设置的操作，因此应该根据用户的原生区域、文化或国家的惯例来设置数字的格式。
...
在这一步中，我们将实现 Java 代码来在`string.xml`文件之间切换，以使用各种语言。首先，我们将初始化所有视图，并在[警报对话框](https://www.geeksforgeeks.org/android-alert-dialog-box-and-how-to-create-it/)上设置点击行为，以便在`LocaleHelper`类的帮助下选择所需的语言。下面是为`MainActivity.java`类给出的代码片段。
