...
`java.text.Bidi` 类的 `baseIsLeftToRight()` 方法用于检查这个 `Bidi` 实例是否有从左到右的基本方向。
...
如果此 `Bidi` 已经向左到**向右**基本方向，则此方法返回 `true` 否则返回 `false`。
...
[https://docs.oracle.com/javase/9/docs/api/java/text/Bidi.html#baseIsLeftToRight--](https://docs.oracle.com/javase/9/docs/api/java/text/Bidi.html#baseIsLeftToRight--)