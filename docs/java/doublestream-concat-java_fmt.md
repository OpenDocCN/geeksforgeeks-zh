...
`DoubleStream concat()` 方法创建一个串联流，其中元素是第一个流的所有元素，然后是第二个流的所有元素。如果两个输入流都是有序的，则结果流是有序的；如果两个输入流中的任何一个是并行的，则结果流是并行的。
...
`DoubleStream.concat()` 方法返回的元素是有序的。例如，以下两行返回相同的结果:
