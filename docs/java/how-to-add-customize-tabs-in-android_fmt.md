...
> 原文：[https://www.geeksforgeeks.org/how-to-add-customize-tabs-in-android/](https://www.geeksforgeeks.org/how-to-add-customize-tabs-in-android/)

本文在安卓中增加了**自定义标签**。[`TabLayout`](https://www.geeksforgeeks.org/how-to-implement-a-tablayout-in-android-using-viewpager-and-fragments/) 提供水平布局来显示选项卡。[`TabLayout`](https://www.geeksforgeeks.org/how-to-implement-a-tablayout-in-android-using-viewpager-and-fragments/) 也可以用 [`ViewPager`](https://www.geeksforgeeks.org/how-to-implement-a-tablayout-in-android-using-viewpager-and-fragments/) 添加，这里勾选，但是不能自定义。每当用户点击标签时，它将导致一个片段到另一个片段的事务。可以创建自定义选项卡来完成同样的任务。图标，动画，文本等根据我们的需要可以添加标签。下图显示了自定义选项卡的示例：
...
通过右键单击 java 包创建一个 `AlgorithmFragment`，选择新建 > **片段(空白)**。
...
现在在 `fragment_algorithm.xml` 文件中添加以下代码。这里，布局中添加了一个文本视图。
...
现在在 `fragment_course.xml` 文件中添加以下代码。这里，布局中添加了一个文本视图。
...
现在在 `fragment_profile.xml` 文件中添加以下代码。这里，布局中添加了一个文本视图。
...
现在在 `tab_bar.xml` 文件中添加以下代码。在此文件中，设计自定义选项卡的布局。这里为每个片段添加了一个文本视图和一个图标(图像视图)。
...
现在在 `activity_main.xml` 文件中添加以下代码。在这个文件中，添加自定义选项卡的布局和片段的容器。
...
现在在 `MainActivity.java` 文件中添加以下代码。在这个文件中，在导航中添加帮助在片段之间导航。当用户点击图标时，它将切换片段。
