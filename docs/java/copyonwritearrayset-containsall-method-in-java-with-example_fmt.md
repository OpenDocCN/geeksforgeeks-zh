# CopyOnWriteArraySet containsAll() 方法详解（Java）

> 原文：[https://www.geeksforgeeks.org/copyonwritearrayset-containsall-method-in-java-with-example/](https://www.geeksforgeeks.org/copyonwritearrayset-containsall-method-in-java-with-example/)

## 方法介绍

`CopyOnWriteArraySet` 的 `containsAll()` 方法用于检查两个集合是否包含相同的元素。它将一个集合作为参数，如果该集合的所有元素都存在于另一个集合中，则返回 `true`。
...
参数 `C` 为集合。该参数是指需要在该集合中检查其元素出现的集合。

## 返回值

如果这个集合包含其他集合的所有元素，方法返回 `true`，否则返回 `false`。

下面的程序说明了 `CopyOnWriteArraySet.containsAll()` 方法：
