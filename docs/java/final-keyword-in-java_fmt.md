...
众所周知，最终变量不能被重新赋值。但是在引用最终变量的情况下，该引用变量指向的对象的内部状态可以改变。请注意，这不是重新分配。`final`的这个性质叫做`非及物性`。要理解对象内部状态的含义，如下例所示:
...
`非传递性`属性也适用于数组，因为[数组是 java](https://www.geeksforgeeks.org/arrays-in-java/) 中的对象。带有`final`关键字的数组也称为[最终数组](https://www.geeksforgeeks.org/final-arrays-in-java/)。
