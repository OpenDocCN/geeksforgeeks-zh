...
> 原文：[https://www.geeksforgeeks.org/creating-first-java-application-in-intellij-idea/](https://www.geeksforgeeks.org/creating-first-java-application-in-intellij-idea/)

`IntelliJ IDEA` 是一个 Java IDE，是世界上 3 个最大、最受欢迎的 IDE 之一。它有两个版本，即免费开源社区版和付费终极版。`IntelliJ IDEA` 的一些特性如下：
...
*   当你第一次打开 `IntelliJ` 时，你会看到一个欢迎屏幕，有三个选项。在这三个选项中，点击 `New Project` 开始。
*   在下一个窗口中，您必须给出项目的名称。
    *   `GeeksForGeeks` 是一个自定义输入，在本文中用来说明。
*   如果您想使用额外的库或任何类型的模板，那么您可以从下拉菜单中选择一个选项来简化您的任务。
*   单击 `Finish` 后，您的 `IDE` 将为您的项目创建一个虚拟环境。

![](img/354e2ffc485d1755dbcef8a606e5049d.png)

*   将项目命名为 `Geeks`（您可以选择任何名称），然后单击 `Finish`。

![](img/1dede472a9c8137cc6d269dcdd92d417.png)

## 步骤 2：在项目内部创建一个包，并创建与该包对应的类

*   在项目窗口的左侧，选择 `src → New → Java Class → Write "geeksforgeeks.gfg"`。
*   它将创建一个名为 `geeksforgeeks` 的包，并创建一个 `Geeksforgeeks` 类。

![](img/107430224271e18392d818a7705329fa.png)

## 步骤 3：编写要在文本编辑器中执行的代码

> 在文本编辑器中编写一个只应该打印 `This is my first project in java` 的演示程序。程序成功编译运行后。
...
*   完成编写部分后，您现在可以将应用程序打包到一个 `JAR` 文件中。
...
*   现在进入导航栏的 `Build`，从下拉列表中选择 `Build Artifacts`。

![](img/1890671ee9eb528d6d7a0de6bf313017.png)

## 步骤 5：测试您的应用程序

*   现在，从右侧的字段中选择 `Edit Configurations`，如下所示，用黄色框突出显示。

![](img/cb548a8a7f1f235ac010aec7fe3a6cdd.png)

*   现在点击加号 `(+)` 并从菜单中选择 `JAR Application`。在名称字段中，写下 `GeeksForGeeksJar`。
*   从目录中选择 `geeksforgeeks.jar` 文件。

![](img/b52a46c3e89b1ed47b35e1df470f1b8a.png)

*   在 `JAR path` 字段中，单击文件夹符号并指定 `.jar` 文件。
*   单击 `Before launch` 字段中的加号 `(+)`，然后从给定选项中选择 `Build Artifacts`。

![](img/1afd7a581fbb4466b6f418ac23dc0978.png)

*   从菜单中选择 `geesforgeeks:jar`，然后按 `OK`。
