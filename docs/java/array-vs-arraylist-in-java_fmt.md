...
数组是 Java 提供的基本功能。`ArrayList` 是 Java 中集合框架的一部分。因此，数组成员是使用 `[]`，而数组列表有一组方法来访问和修改元素。
...
数组是固定大小的数据结构，而 `ArrayList` 不是。在创建对象时，不必提及数组列表的大小。即使我们指定了一些初始容量，我们也可以添加更多的元素。
...
根据数组的定义，数组既可以包含基元数据类型，也可以包含类的对象。但是，数组列表只支持对象条目，不支持基本数据类型。
...
由于不能为基元数据类型创建数组列表，数组列表的成员总是对不同内存位置的对象的引用（详见[本](https://www.geeksforgeeks.org/g-fact-46/)）。因此，在数组列表中，实际的对象从不存储在连续的位置。实际对象的引用存储在连续的位置。
...
| --- | --- | --- |
| 维度 | 可以是一维或多维。 | 只能是一维。 |
| 遍历元素 | 通常使用 `for` 循环或增强 `for` 循环。 | 使用 `size()` 方法来计算数组列表的大小。 |
| 特性 | 它是静态的，具有固定长度。 | 它是动态的，可以在必要时增加或减少大小。 |
| 速度 | 更快。如上所示的固定大小。 | 速度相对较慢，因为其动态性质。 |
| 原始数据类型存储 | 可以直接存储原始数据类型，而不是对象。 | 不能像数组那样直接添加原始数据类型，而是通过自动装箱和拆箱。 |
| 添加元素 | 仅使用赋值运算符。 | 使用称为 `add()` 方法的特殊方法。 |
