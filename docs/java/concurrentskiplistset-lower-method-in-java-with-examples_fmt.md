...
该方法只取一个参数，`e`，即指定要匹配的值。

## 返回值

该方法返回一个值，该值是集合中存在的最大元素，严格来说小于指定元素。如果不存在这样的元素，那么它将返回空值。
...
*   `NullPointerException`：如果指定的集合为空，则抛出。
...
在下面的程序中，指定的元素是 67，我们的集合也包含了元素 67，但是没有返回，因为 `lower()` 方法返回的值严格来说比较少。
