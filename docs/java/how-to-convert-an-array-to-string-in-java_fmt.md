...
> 原文：`https://www.geeksforgeeks.org/how-to-convert-an-array-to-string-in-java/`

以下是在 Java 中将数组转换为字符串的各种方法：

## 1. `Arrays.toString()` 方法

`Arrays.toString()` 方法用于返回指定数组内容的字符串表示形式。字符串表示形式由包含在方括号（“[]”）中的数组元素列表组成。相邻的元素由字符“，”（逗号后跟空格）分隔。如果数组为空，则返回“null”。
...
`java.lang.StringBuilder.append(char[])` 是将 `char` 数组参数的字符串表示形式附加到这个 `StringBuilder` 序列的内置方法。
