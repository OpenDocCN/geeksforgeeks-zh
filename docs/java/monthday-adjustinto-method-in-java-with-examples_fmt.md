...
> 原文：[https://www.geeksforgeeks.org/monthday-adjustinto-method-in-java-with-examples/](https://www.geeksforgeeks.org/monthday-adjustinto-method-in-java-with-examples/)

`adjustInto()`方法属于`MonthDay`类，用于调整传递的时态对象，使其具有应用该方法的`MonthDay`。此实例是不可变的，不受此方法调用的影响。
...
## 参数：
该方法接受`temporal`作为参数，该参数是要调整的目标时间对象。它不应为`null`。
