...
> 原文：[https://www.geeksforgeeks.org/establishing-jdbc-connection-in-java/](https://www.geeksforgeeks.org/establishing-jdbc-connection-in-java/)
...
这里我们在运行时将驱动程序的类文件加载到内存中。不需要使用 `new` 或创建对象。以下示例使用 `Class.forName()` 加载 Oracle 驱动程序，如下所示：
...
*   `user`：Username，从中可以访问您的 SQL 命令提示符。
...
所以最后我们已经将数据发送到指定的位置，现在我们即将完成我们的任务。通过关闭连接，语句和结果集的对象将自动关闭。`Connection` 接口的 `close()` 方法用于关闭连接。如下图所示：
