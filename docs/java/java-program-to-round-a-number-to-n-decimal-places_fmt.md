...
> 原文：[https://www.geeksforgeeks.org/java-program-to-round-a-number-to-n-decimal-places/](https://www.geeksforgeeks.org/java-program-to-round-a-number-to-n-decimal-places/)
...
十进制数可以通过 Java 支持的内置 `format()` 方法进行舍入。该方法具有以下语法：
...
`DecimalFormat` 是 `NumberFormat` 的子类，用于在 java 中执行十进制数字的格式化。我们创建这个类的一个对象，并将以 `#` 形式指定的格式作为参数传入，小数点后的 `#` 表示我们希望输出的位数。默认情况下，该数字四舍五入到最高值本身。抽取 `DecimalFormat` 类的对象调用一个方法 `format()`，该方法将待格式化的数字作为参数。
...
如果我们希望将数字舍入到楼层，我们调用 Java 内置类 `RoundingMode`。它具有以下属性值：
...
此方法可以在支持 `DecimalFormat` 类的内置方法 `setRoundingMode()` 上调用，该方法将 `RoundingMode` 作为参数。地板或天花板，并相应地给我们结果。
