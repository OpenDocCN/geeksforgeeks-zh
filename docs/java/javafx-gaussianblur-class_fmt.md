...
1.  `GaussianBlur()`: 创建 `GaussianBlur` 的新对象。
2.  `GaussianBlur(double r)`: 以指定的半径创建 `GaussianBlur` 的新对象。

## 常用方法

| 方法 | 说明 |
| --- | --- |
| `getRadius()` | 返回属性 `radius` 的值。 |
| `getInput()` | 返回属性 `input` 的值。 |
| `setInput(Effect val)` | 设置属性 `input` 的值。 |
| `setRadius(double value)` | 设置属性 `radius` 的值。 |
...
**参考:** [https://docs.oracle.com/javase/8/javafx/api/javafx/scene/effect/GaussianBlur.html](https://docs.oracle.com/javase/8/javafx/api/javafx/scene/effect/GaussianBlur.html)