...
> 原文：[https://www.geeksforgeeks.org/abstractmap-containsvalue-method-in-java-with-examples/](https://www.geeksforgeeks.org/abstractmap-containsvalue-method-in-java-with-examples/)

`AbstractMap`的`containsValue()`方法用于检查抽象映射中的单个或多个键是否映射了特定的值。它将值作为参数，如果该值由映射中的任何键映射，则返回真。
