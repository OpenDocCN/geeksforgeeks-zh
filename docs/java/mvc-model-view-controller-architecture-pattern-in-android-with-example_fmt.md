...
> 原文: [https://www.geeksforgeeks.org/mvc-model-view-controller-architecture-pattern-in-android-with-example/](https://www.geeksforgeeks.org/mvc-model-view-controller-architecture-pattern-in-android-with-example/)
...
*   **控制器:** 该组件建立**视图**和**模型**之间的关系。包含核心应用逻辑，获知用户行为，根据需要更新模型。
...
创建一个名为 `Model` 的新类来分离所有数据及其操作。这个类不会知道视图类的存在。
