...
> 原文: [https://www.geeksforgeeks.org/java-awt-cursor-class-with-examples/](https://www.geeksforgeeks.org/java-awt-cursor-class-with-examples/)

`Cursor`类是 Java AWT 包的一部分，用于创建自定义游标或继承系统或预定义游标。`Cursor`类主要用于封装鼠标光标的位图表示。

## 构造函数

`Cursor`类的构造函数为:

1.  `Cursor(int t)`: 用指定的类创建光标。
...
| `createCustomCursor(Image i, Point p, String name)` | 使用指定的图像和名称创建自定义光标。 |
