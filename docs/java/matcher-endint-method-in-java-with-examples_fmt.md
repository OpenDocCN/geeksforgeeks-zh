...
> 原文: [https://www.geeksforgeeks.org/matcher-endint-method-in-java-with-examples/](https://www.geeksforgeeks.org/matcher-endint-method-in-java-with-examples/)

`Matcher`类的`end(int group)`方法用于从指定的组中获取已经完成的匹配结果的结束索引之后的偏移量。
...
## 参数:
该方法取一个参数`group`，需要匹配图案结束索引后的偏移量。

## 返回值:
该方法返回指定组匹配结束索引后的`偏移量`。
...
## 参考:
[Oracle 文档](https://docs.oracle.com/javase/9/docs/api/java/util/regex/Matcher.html#end-int-)