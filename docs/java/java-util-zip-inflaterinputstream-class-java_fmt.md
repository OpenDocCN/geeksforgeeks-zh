# Java 中的 `InflaterInputStream` 类

> 原文：[https://www.geeksforgeeks.org/java-util-zip-inflaterinputstream-class-java/](https://www.geeksforgeeks.org/java-util-zip-inflaterinputstream-class-java/)

这个类实现了一个流过滤器，用于以“deflate”压缩格式解压缩数据。它还被用作其他解压缩过滤器的基础，如 `GZIPInputStream`。

## 构造方法

*   **`InflaterInputStream(InputStream in)`**：用默认的解压缩程序和缓冲区大小创建一个新的输入流。
*   **`InflaterInputStream(InputStream in, Inflater inf)`**：使用指定的解压缩程序和默认缓冲区大小创建新的输入流。
*   **`InflaterInputStream(InputStream in, Inflater inf, int size)`**：用指定的解压缩程序和缓冲区大小创建一个新的输入流。
...
*   **`void mark(int readlimit)`**：标记该输入流中的当前位置。
...
*   **`boolean markSupported()`**：测试该输入流是否支持标记和重置方法。
...
**下一篇文章：** [Java 中的 `InflaterOutputStream` 类](https://www.geeksforgeeks.org/java-util-zip-inflateroutputstream-class-java/)
