...
> 原文: [https://www.geeksforgeeks.org/java-program-to-find-the-most-repeated-word-in-a-text-file/](https://www.geeksforgeeks.org/java-program-to-find-the-most-repeated-word-in-a-text-file/)

[`Map`](https://www.geeksforgeeks.org/java-util-hashmap-in-java/) 和 [`Map.Entry`](https://www.geeksforgeeks.org/map-entry-interface-java-example/) 接口将被使用，因为 `Map` 接口将唯一键映射到值。键是用于在以后检索值的对象。`Map.Entry` 接口使您能够使用 `Map` 的条目。此外，我们将使用 `HashMap` 类来存储“键/值”对中的项目，并通过另一种类型的索引来访问它们。
...
public class GFG {
        ...
        throws FileNotFoundException
    {
        ...
        for (Entry<String, Integer> word :
            ...
            // Comparing values using getValue() method
            if (word.getValue() == max) {
