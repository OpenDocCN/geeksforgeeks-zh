...
> 原文: [https://www.geeksforgeeks.org/java-rmi-marshalledobject-class-in-java/](https://www.geeksforgeeks.org/java-rmi-marshalledobject-class-in-java/)

`java.rmi.MarshalledObject`是一个 Java 类，一个`MarshalledObject`包含一个字节流，对象的序列化表示被赋予其构造函数，所包含的对象被序列化和反序列化，序列化语义与封送和解封参数的序列化语义相同。
...
`MarshalledObject(T object)`–`MarshalledObject(T object)`初始化`MarshalledObject`类的新实例，该实例包含所提供对象的当前状态的序列化表示。
...
`MarshalledObject`类包含三个方法：

1.  `hashCode()`
2.  `get()`
3.  `equals(Object obj)`

让我们单独讨论这门课的三种方法，以便更好地理解。
...
```java
// Java program to get the hash code
import java.io.*;
import java.rmi.*;
...
class GFG {
    ...
    {
        // invoke getHashCode method to get hashCode
        getHashCode(new tmp());
    }
...
```java
// Java program to get copy of marshalledObject
import java.io.*;
import java.rmi.*;
...
class GFG {
    public static void main(String[] args)
    {
        // invoke get method to get copy of marshalledObject
        get(new tmp());
    }
    @SuppressWarnings("unchecked")
    public static void get(tmp t)
    {
        try {
            ...
            // marshalledObject to get copy of marshalled object
...
**方法返回类型:** `equals()`方法具有`boolean`返回类型，如果参数引用的`MarshalledObject`包含与此对象完全相同的对象序列化表示形式，它将返回`true`。

**方法参数:** `equals()`方法有一个`Object`类型的参数。
...
```java
// Java program to compare two marshalled object
import java.io.*;
import java.rmi.*;
...
class GFG {
    public static void main(String[] args)
    {
        // invoke compare method to compare the marshalled object
    ...
    public static void compare(tmp a,tmp b)
    {
        try {
            ...
            // marshalledObject to compare the marshalled object
