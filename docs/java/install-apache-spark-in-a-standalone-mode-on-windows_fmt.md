...
> 原文：[https://www.geeksforgeeks.org/install-apache-spark-in-a-standalone-mode-on-windows/](https://www.geeksforgeeks.org/install-apache-spark-in-a-standalone-mode-on-windows/)
...
4.  **Hadoop YARN：** 在这种模式下，驱动程序在应用程序的主节点内部运行，并由集群上的 YARN 处理。

在本文中，我们将探讨 Apache Spark 在独立模式下的安装。Apache Spark 是用 Scala 编程语言开发的，运行在 JVM 上。Java 安装是 Spark 中必不可少的事情之一。所以让我们从 Java 安装开始。

## 安装 Java

**第一步：** [下载](https://www.oracle.com/in/java/technologies/javase-jdk15-downloads.html) Java JDK。
...
**步骤 3：** 在 Windows 搜索栏中键入“环境变量”，打开笔记本电脑上的环境变量。
...
*   单击用户变量，将 `JAVA_HOME` 添加到路径，值为：`C:\Program Files\Java\jdk1.8.0_261`。
*   单击系统变量，将 `C:\Program Files\Java\jdk1.8.0_261\bin` 添加到 `PATH` 变量。
*   打开命令提示符，输入 `java -version`，将显示如下所示以验证 Java 安装。
...
*   在系统变量中，将 `C:\Program Files (x86)\scala\bin` 添加到 `PATH` 变量中。
...
*   在用户变量中，用值 `C:\spark\spark-2.4.6-bin-hadoop2.7` 将 `SPARK_HOME` 添加到路径中。
...
**第二步：** 将文件复制到 `C:\spark\spark-1.6.1-bin-hadoop2.6\bin`。

**步骤 3：** 现在在 `cmd` 上执行 `spark-shell` 以验证 Spark 安装，如下所示：
