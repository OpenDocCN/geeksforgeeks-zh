# Java 中的 FloatBuffer rewind() 方法示例

> 原文：[https://www.geeksforgeeks.org/floatbuffer-rewind-methods-in-java-with-examples/](https://www.geeksforgeeks.org/floatbuffer-rewind-methods-in-java-with-examples/)

[`java.nio.FloatBuffer`](https://www.geeksforgeeks.org/tag/java-floatbuffer/) 类的 `rewind()` 方法用来倒带这个缓冲区。此方法将位置设置为零，限制保持不变，如果有任何先前标记的位置，将被丢弃。当需要进行通道写或获取操作时，应调用此方法。这意味着，如果缓冲区数据已经写入，则需要将其复制到另一个数组中。例如：
...
**参考：** [https://docs.oracle.com/javase/9/docs/api/java/nio/FloatBuffer.html#rewind--](https://docs.oracle.com/javase/9/docs/api/java/nio/FloatBuffer.html#rewind--)