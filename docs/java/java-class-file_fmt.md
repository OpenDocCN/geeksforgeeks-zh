...
一个 **Java 类文件**是一个包含 Java 字节码并且有`.class`扩展名的文件，可以由 [JVM](https://www.geeksforgeeks.org/jvm-works-jvm-architecture/) 执行。一个 Java 类文件是由一个 Java 编译器从`.java`文件作为成功编译的结果创建的。正如我们所知，单一的 Java 编程语言源文件（或者我们可以说`.java`文件）可能包含一个类或多个类。所以如果一个`.java`文件有多个类，那么每个类都会编译成一个单独的类文件。
...
类文件的前 4 个字节被称为`magic_number`。这是一个预定义的值，JVM 用它来识别`.class`文件是否由有效的编译器生成。预定义的值将是十六进制形式，即`0xCAFEBABE`。

现在让我们看看当 JVM 找不到有效的 magic number 时会发生什么。假设我们有一个名为`Sample.java`的`.java`文件，并在你的系统上遵循逐步过程。
...
**第 1 步：** 使用`javac Sample.java`编译。
**第 2 步：** 现在打开`Sample.class`文件。看起来像是在跟踪。

![Sample.class File](img/9df76452171509f56d29f6f856d95069.png)

**第三步：** 现在从文件开始，从这个`Sample.class`文件中删除至少一个符号并保存。
...
**注意：** 这取决于你移除`.class`文件数据的程度。

### minor_version & major_version

这两个一起表示`.class`文件版本。JVM 将使用这些版本来识别哪个版本的编译器生成了当前的`.class`文件。我们将类文件的版本表示为`M.m`，其中`M`代表`major_version`，`m`代表`minor_version`。

**注：** 由较低版本编译器生成的`.class`文件可以由较高版本 JVM 执行，但由较高版本编译器生成的`.class`文件不能由较低版本的 JVM 执行。如果我们试图执行，我们会得到运行时异常。
...
**第三步：** 打开第二个命令提示窗口，设置第二步安装的已安装 JDK 的 bin 文件夹路径。并检查 Java 编译器版本和 JVM 版本。
...
它表示常量池中（当编译一个 Java 文件时，对变量和方法的所有引用都作为符号引用存储在类的常量池中）存在的常量数量。
...
表示当前类的直属超类的全限定名。考虑以上`Sample.java`文件。当我们编译它时，我们可以说`this_class`将是`Sample`，`super_class`将是`Object`。
...
表示当前类文件中存在的字段数量（静态变量）。

### fields[]

表示当前类文件中存在的字段（静态变量）信息。
