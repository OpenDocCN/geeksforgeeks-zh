...
> 原文：[https://www.geeksforgeeks.org/eye-detecting-video-player-in-android/](https://www.geeksforgeeks.org/eye-detecting-video-player-in-android/)
...
> implementation 'com.github.pradyuman7:LookAtMe:2.0'
...
转到 `res`，创建一个新的安卓资源目录，并将其命名为 `raw`。这个文件是用来粘贴我们要在应用程序中播放的离线视频的（对于在线视频，不需要创建它，我们可以用 java 代码提供视频的链接，我们将在后面看到）。

![](img/9045804812d7949e85d6f55a2125ace0.png) ![](img/c4d8d68ce12e8198dd29ef0747ccecbb.png)

> **参考文章：** [安卓工作室中的资源 Raw 文件夹](https://www.geeksforgeeks.org/resource-raw-folder-in-android-studio/)

### 第四步：使用 activity_main.xml 文件

导航到 `app > res > layout > activity_main.xml` 并将下面的代码添加到该文件中。
...
下面是 `MainActivity.java` 文件的完整代码。代码中添加了注释，以更详细地理解代码。
...
> **注意：** 你可以在这个应用里同时播放线上和线下的视频。为了播放离线视频，你必须把它粘贴在原始文件夹中，然后用代码给出它的名字，就像这里提到的 `gfg_eye_detecting`，但是如果你想播放在线视频，你必须用上面提到的代码给出视频的 Url。

### 第六步：使用 AndroidManifest.xml 文件
