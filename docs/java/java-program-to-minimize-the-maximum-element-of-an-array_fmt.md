...
> 原文：[https://www.geeksforgeeks.org/java-program-to-minimize-the-maximum-element-of-an-array/](https://www.geeksforgeeks.org/java-program-to-minimize-the-maximum-element-of-an-array/)
...
假设，在这两种情况下，最优和是 `S`。因此，给 `N-1` 个元素赋予 `S/N` 的底值，给其余元素赋予 `S/K` 的顶值，将使和等于 `S`，并且它也能被 `K` 整除

现在，在 `sum/N` 的最低值和 `sum/N` 的最高值之间，最高值会更大。最后，最大元素将是 `sum/N` 的上限值
