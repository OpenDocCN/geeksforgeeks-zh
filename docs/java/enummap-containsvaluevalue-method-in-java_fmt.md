# Java中EnumMap的containsValue(value)方法

> 原文：[https://www.geeksforgeeks.org/enummap-containsvaluevalue-method-in-java/](https://www.geeksforgeeks.org/enummap-containsvaluevalue-method-in-java/)

`java.util.EnumMap.containsValue(value)`方法用于确定映射的一个或多个键是否与给定值相关联。它将该值作为参数，如果该值由枚举映射中的任何键映射，则返回真。
...
## 参数
该方法接受一个参数`value`，该值是指其映射将由任意键检查的值。
