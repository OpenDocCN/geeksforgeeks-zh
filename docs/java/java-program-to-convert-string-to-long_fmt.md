...
> 原文：[https://www.geeksforgeeks.org/java-program-to-convert-string-to-long/](https://www.geeksforgeeks.org/java-program-to-convert-string-to-long/)
...
Java 中的字符串是内部由字符数组支持的对象。由于数组是不可变的，而字符串也是一种保存字符的特殊数组，因此字符串也是不可变的。

## Long

长数据类型是 64 位二进制补码整数。其值范围介于 (-2<sup>63</sup>) 至 (2<sup>63</sup> -1)（含）之间。其默认值为 `0`。当您需要的值范围超过 `int` 提供的值范围时，可以使用长数据类型。
