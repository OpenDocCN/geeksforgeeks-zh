...
> 原文: [https://www.geeksforgeeks.org/method-class-getdeclaringclass-method-in-java/](https://www.geeksforgeeks.org/method-class-getdeclaringclass-method-in-java/)

`Method`类的`getDeclaringClass()`方法返回一个`Class`对象，该对象代表声明了此`Method`的类或接口。
...
通过应用`getDeclaringClass()`方法，找到定义该方法的类对象的详细信息。
...
要查找`Object`类方法的细节，通过应用`getDeclaringClass()`方法。
