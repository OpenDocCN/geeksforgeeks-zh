...
> 原文：`https://www.geeksforgeeks.org/arraylist-retainall-method-in-java/`

[`ArrayList`](https://www.geeksforgeeks.org/arraylist-in-java/) 的 `retainAll()` 方法用于移除指定集合中未包含的所有数组列表元素，或者保留当前数组列表实例中所有匹配元素，这些元素匹配作为参数传递给该方法的集合列表中的所有元素。
...
如果调用导致列表发生变化，则该方法返回布尔值 `true`，否则返回 `false`。
...
下面的程序用来说明 `java.util.ArrayList.retainAll()` 方法：

## 程序 1

将 `ArrayList` 集合作为参数传递给方法。
