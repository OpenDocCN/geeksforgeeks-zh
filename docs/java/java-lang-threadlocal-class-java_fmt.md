...
> 原文: [https://www.geeksforgeeks.org/java-lang-threadlocal-class-java/](https://www.geeksforgeeks.org/java-lang-threadlocal-class-java/)
...
这个方法返回这个线程局部变量在当前线程副本中的值。如果变量没有当前线程的值，则首先将其初始化为通过调用 `initialValue()` 方法返回的值。
...
此方法将当前线程对此线程局部变量的副本设置为指定值。大多数子类不需要重写这个方法，只需要依赖 `initialValue()` 方法来设置线程局部变量的值。
