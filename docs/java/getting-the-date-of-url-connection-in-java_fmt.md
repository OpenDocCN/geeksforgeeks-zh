...
> 原文：[https://www.geeksforgeeks.org/getting-the-date-of-url-connection-in-java/](https://www.geeksforgeeks.org/getting-the-date-of-url-connection-in-java/)

`HttpURLConnection`类是 Java 中处理所有与 URL 连接相关的操作的类。`getDate()`方法是`HttpURLConnection`类中用来获取 URL 连接的日期和时间的方法。
...
该方法不取任何参数。它只是与一个`HttpURLConnection`对象一起使用，我们希望从该对象中获取连接的日期和时间。
