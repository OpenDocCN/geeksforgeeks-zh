# Java中的AbstractSet.removeAll()方法示例

> 原文：[https://www.geeksforgeeks.org/abstractset-removeall-method-in-java-with-examples/](https://www.geeksforgeeks.org/abstractset-removeall-method-in-java-with-examples/)

## 方法描述

`AbstractSet`类的`removeAll()`方法用于从该集中移除指定集合中包含的所有元素。
...
该方法将`Collection c`作为包含要从该集合中移除的元素的参数。
...
*   `UnsupportedOperationException` – 如果该集合不支持该操作，则抛出该异常。
*   `ClassCastException` – 当这个集合的元素的类与指定的集合不兼容时抛出。
*   `NullPointerException` – 当该集合包含空元素并且指定的集合不允许空元素时，或者如果集合为空，则会引发该异常。
