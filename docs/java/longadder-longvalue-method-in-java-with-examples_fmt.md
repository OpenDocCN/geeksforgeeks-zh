...
> 原文: [https://www.geeksforgeeks.org/longadder-longvalue-method-in-java-with-examples/](https://www.geeksforgeeks.org/longadder-longvalue-method-in-java-with-examples/)

## Java 中的 LongAdder 类

`LongAdder` 类创建一个初始和为零的新加法器。`LongAdder.longValue()` 是 Java 中的一个内置方法，返回 `sum()`。这个方法相当于 `sum()` 方法。创建类的对象时，其初始值为零。
