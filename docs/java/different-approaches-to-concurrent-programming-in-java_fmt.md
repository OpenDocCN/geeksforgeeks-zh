...
> 原文：[https://www.geeksforgeeks.org/different-approaches-to-concurrent-programming-in-java/](https://www.geeksforgeeks.org/different-approaches-to-concurrent-programming-in-java/)

本文展示了如何使用 Java 线程框架执行并发编程。让我们首先分析并发编程：

## 并发编程

这意味着任务似乎是同时运行的，但在幕后，系统可能真的在任务之间来回切换。并发编程的要点是，即使在单处理器机器上，它也是有益的。
...
1.  首先要排队一个任务。调用 `ExecutorService` 指向新的固定线程池并提供一个大小。此大小表示同时进行的任务的最大数量。例如，如果一个人向队列中添加了一千个东西，但是池的大小是 50，那么在任何时候都只有 50 个在运行。只有当前五十个中的一个完成执行时，第五十一个才会被执行。像池大小为 100 这样的数字不会使系统过载。

    ```java
    ExecutorService taskList = Executors.newFixedThreadPool(poolSize);
    ```

2.  然后，用户必须将一些 `Runnable` 类型的任务放到任务队列中。`Runnable` 只是一个接口，它有一个叫做 `run` 的方法。当系统通过启动单独的线程在任务之间来回切换时，它会在适当的时间调用 `run` 方法。
...
## 方法一：实现 `Runnable` 的独立类

1.  首先要做的是创建一个单独的类，一个完全单独的类，实现 `Runnable` 的接口。
...
3.  接下来，使用暂停方法 `wait`，以便系统来回切换。因此，打印语句将是交错的。
