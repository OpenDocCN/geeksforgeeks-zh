# Java 中的 TemporalAdjusters 类

> 原文：[https://www.geeksforgeeks.org/java-time-temporal-temporaladjusters-class-in-java/](https://www.geeksforgeeks.org/java-time-temporal-temporaladjusters-class-in-java/)
...
| `firstDayOfYear()` | 此方法用于返回“年初一”调整器，该调整器返回设置为当年年初一的新日期。 |
| `firstInMonth(DayOfWeek dayOfWeek)` | 此方法用于返回月内第一个调整器，该调整器返回与第一个匹配的星期几在同一个月中的新日期。 |
| `lastDayOfMonth()` | 此方法用于返回“每月的最后一天”调整器，该调整器返回设置为当月最后一天的新日期。 |
| `lastDayOfYear()` | 此方法用于返回“一年中的最后一天”调整器，该调整器返回一个设置为当年最后一天的新日期。 |
| `lastInMonth(DayOfWeek dayOfWeek)` | 此方法用于返回月中最后一个调整器，该调整器返回与最后一个匹配的星期几在同一个月中的新日期。 |
| `next(DayOfWeek dayOfWeek)` | 此方法用于返回下一个星期几调整器，该调整器将日期调整到被调整日期之后指定星期几的第一次出现。 |
| `nextOrSame(DayOfWeek dayOfWeek)` | 此方法用于返回下一个或同一天的星期调整器，该调整器将日期调整到正在调整的日期之后指定星期的第一次出现，除非它已经在返回相同对象的那一天。 |
| `ofDateAdjuster(UnaryOperator<LocalDate> dateBasedAdjuster)` | 此方法用于获取包装日期调整器的临时调整器。 |
| `previous(DayOfWeek dayOfWeek)` | 此方法用于返回前一个星期几调整器，该调整器将日期调整到被调整日期之前指定星期几的第一次出现。 |
| `previousOrSame(DayOfWeek dayOfWeek)` | 此方法用于返回前一个或同一天的星期调整器，该调整器将日期调整到被调整日期之前的指定星期的第一次出现，除非它已经在返回相同对象的那一天。 |
