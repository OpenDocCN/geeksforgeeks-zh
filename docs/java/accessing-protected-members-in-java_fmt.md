...
> 原文：[https://www.geeksforgeeks.org/accessing-protected-members-in-java/](https://www.geeksforgeeks.org/accessing-protected-members-in-java/)
...
它会给出编译时错误。在下面的例子中，我们将创建两个类。包 1 中的 `Sample` 类和包 2 中的 `Test` 类，并尝试访问 `Test` 类中 `Sample` 类的受保护成员。上面两个例子证明了这一点。

## 情况 5：访问不同包中子类中的受保护成员

我们可以访问存在于不同包中的子类的受保护成员。在下面的例子中，我们将创建两个类。包 1 中的 `Sample` 类和包 2 中的子类。子类扩展了 `Sample` 类。
