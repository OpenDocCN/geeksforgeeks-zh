...
> 原文: [https://www.geeksforgeeks.org/java-net-cacheresponse-class-in-java/](https://www.geeksforgeeks.org/java-net-cacheresponse-class-in-java/)

`CacheResponse` 是一个抽象类，表示从响应缓存中检索资源的通道。这个类的对象提供了一个返回实体主体和相关响应头的输入流。此类从 `java.lang.Object` 继承方法，如 `clone()`、`equals()`、`finalize()`、`getClass()`、`hashCode()`、`notify()`、`notifyAll()`、`toString()`、`wait()`。
...
`getBody()` 方法返回一个 `InputStream`，可以从中访问响应体。
...
`getHeaders()` 方法返回从响应头字段名称到字段值列表的不可变映射。
...
public class GFG {
        ...
        throws IOException
    {
        ...
        // Returning a Map containing the response headers
        cr.getHeaders();
    }
}
```
