...
> 原文：[https://www.geeksforgeeks.org/java-net-urlconnection-class-in-java/](https://www.geeksforgeeks.org/java-net-urlconnection-class-in-java/)

`URLConnection` 类在 Java 中是一个抽象类，表示由相应 `URL` 指定的资源的连接。由 `java.net` 包导入。`URLConnection` 类用于服务两个不同但相关的目的：首先，它提供对与服务器（尤其是 HTTP 服务器）交互的控制，而不是 `URL` 类。其次，通过一个 `URLConnection`，我们可以检查服务器发送的报头并做出相应的响应，我们可以配置客户端请求中使用的报头字段。我们也可以使用 `URLConnection` 下载二进制文件。
...
| 方法 | 描述 |
| --- | --- |
| `addRequestProperty(String key, String value)` | 此方法用于添加由键值对指定的常规请求属性。 |
| `connect()` | 此方法用于建立到由 `URL` 指定的资源的连接，如果这种连接尚未建立的话。 |
...
| `getContent(Class[] classes)` | 检索此 `URLConnection` 的内容。 |
| `getContentEncoding()` | 返回内容编码报头字段的值。 |
| `getContentLength()` | 返回内容长度报头字段的值。 |
| `getContentLengthLong()` | 将内容长度报头字段的值作为长整型返回。 |
| `getContentType()` | 返回内容类型报头字段的值。 |
| `getDate()` | 返回日期报头字段的值。 |
...
| `getRequestProperty(String key)` | 返回此连接的命名常规请求属性值。 |
...
| `setDefaultUseCaches(boolean defaultusecache)` | 将“使用缓存”字段的默认值设置为指定值。 |
...
| `setFileNameMap(FileNameMap map)` | 设置文件名映射。 |
...
| `setRequestProperty(String key, String value)` | 设置常规请求属性。 |
| `setUseCaches(boolean usecache)` | 将此 `URLConnection` 的 `useCaches` 字段的值设置为指定值。 |
| `toString()` | 返回此 `URLConnection` 的字符串表示形式。 |
