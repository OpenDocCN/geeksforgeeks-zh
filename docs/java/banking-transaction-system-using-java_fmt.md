...
> 原文: [https://www.geeksforgeeks.org/banking-transaction-system-using-java/](https://www.geeksforgeeks.org/banking-transaction-system-using-java/)
...
我们已经在类 `Bank` 中声明了 `withdrawn()` 和 `deposit()` 方法，并通过创建 `Bank` 类的对象 `obj` 从驱动程序类 `GFG` 中访问它们。
...
**同步**提供对象锁并声明敏感区域(`withdrawn` & `deposit`方法)。一个对象可以有多个线程，但是敏感区域一次只能被一个线程访问。线程调度器选择线程的执行顺序。由于这是一个随机的过程，每个解释的输出是不同的。
