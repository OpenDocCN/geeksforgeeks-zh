...
> 原文：[https://www.geeksforgeeks.org/dateformatsymbols-getavailablelocales-method-in-java-with-examples/](https://www.geeksforgeeks.org/dateformatsymbols-getavailablelocales-method-in-java-with-examples/)

Java 中 `DateFormatSymbols` 类的 `getAvailableLocales()` 方法用于返回该类的 `getInstance` 方法的本地化日期格式符号实例中可用的所有区域设置的数组。
