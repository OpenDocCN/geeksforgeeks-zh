...
> 原文: [https://www.geeksforgeeks.org/linkedtransferqueue-offer-method-in-java/](https://www.geeksforgeeks.org/linkedtransferqueue-offer-method-in-java/)

## offer(E e, long timeout, TimeUnit unit)

`LinkedTransferQueue` 类的 `offer(E e, long timeout, TimeUnit unit)` 方法是一个内置的 Java 函数，如果队列没有满，它会在这个队列的尾部插入作为参数传递给方法的元素。
...
*   `e` – 要插入的元素。
*   `timeout`
*   `unit` – 决定如何解释 `timeout` 参数的时间单位

**返回值:** 该方法返回一个布尔值 `true`。由于队列是无界的，这个方法永远不会阻塞或返回 `false`。

**异常:** 当指定元素为空时，函数显示 `NullPointerException`。

下面的程序说明了 `java.util.concurrent.LinkedTransferQueue.offer()` 的使用:

**程序 1:** 通过使用 `offer(E e, long timeout, TimeUnit unit)` 方法插入学生姓名来创建 `LinkedTransferQueue`，该方法的时间单位参数以秒为单位，超时参数为 5 秒。
...
**参考:**
[https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/LinkedTransferQueue.html#offer(E)](https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/LinkedTransferQueue.html#offer(E))
[https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/LinkedTransferQueue.html#offer(E,%20long,%20java.util.concurrent.TimeUnit)](https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/LinkedTransferQueue.html#offer(E,%20long,%20java.util.concurrent.TimeUnit))