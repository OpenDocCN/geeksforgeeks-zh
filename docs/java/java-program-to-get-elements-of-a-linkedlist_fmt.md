...
> 原文: [https://www.geeksforgeeks.org/java-program-to-get-elements-of-a-linkedlist/](https://www.geeksforgeeks.org/java-program-to-get-elements-of-a-linkedlist/)

[`链表`](https://www.geeksforgeeks.org/data-structures/linked-list/)是一种线性数据结构，其中元素不存储在连续的存储位置。这里的任务是获取链表的元素。

## 方法一：使用 `get(int index)` 方法

我们可以使用 `get(int index)` 方法从 `LinkedList` 的特定索引中访问元素。

在给定的例子中，我们已经使用了 [`get(i)` 方法](https://www.geeksforgeeks.org/linkedlist-get-method-in-java/)。这里，该方法返回位于第 *i*<sup>th</sup> 索引处的元素。
...
**参数:** 参数 `index` 是整数数据类型，指定要从链接列表中提取的元素的位置或索引。

**返回值:** 该方法返回出现在参数 `index` 指定位置的元素。
...
我们可以使用 [`iterator()`](https://www.geeksforgeeks.org/iterators-in-java/) 方法。
...
## 方法三：使用 `listIterator()` 方法

我们可以使用 [`listIterator()`](https://www.geeksforgeeks.org/linkedlist-listiterator-method-in-java/) 方法。

- `ListIterator()` 是 `Iterator()` 方法的子接口。
- 它为我们提供了访问列表元素的功能。
