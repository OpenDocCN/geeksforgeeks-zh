...
> 原文: [https://www.geeksforgeeks.org/how-to-take-input-from-user-in-java/](https://www.geeksforgeeks.org/how-to-take-input-from-user-in-java/)

[Java](https://www.geeksforgeeks.org/java/) 带来了各种流及其 I/O 包，帮助用户执行所有的输入输出操作。这些流支持所有类型的对象、数据类型、字符、文件等，以完全执行输入/输出操作。有两种方式我们可以从用户或文件中获取输入：
...
它是 `BufferedReader` 的高级版本，是在 Java 的更高版本中添加的。`Scanner` 可以读取格式化的输入。它对于不同类型的数据类型有不同的功能。

*   `Scanner` 更容易阅读，因为我们不需要写抛出，因为它没有抛出异常。
*   它是在后来的 Java 版本中添加的。
...
**导入 Scanner 类的语法:** 要使用 `Scanner`，我们需要导入 `Scanner` 类。
...
*   浮点数: [`nextFloat()`](https://www.geeksforgeeks.org/scanner-nextfloat-method-in-java-with-examples/)
*   字符串: [`readLine()`](https://www.geeksforgeeks.org/bufferedreader-readline-method-in-java-with-examples/)

因此，在 `Scanner` 中的整数和字符串的情况下，我们不需要像在 `BufferedReader` 中那样进行解析。
...
*   当我们阅读更大的输入时，这两个因素会起作用。一般来说，`Scanner` 类提供输入。
*   `BufferedReader` 是首选的，因为它是同步的。当处理多个线程时，它是首选的。
*   为体面的输入，容易可读性。`Scanner` 比 `BufferedReader` 更受欢迎。