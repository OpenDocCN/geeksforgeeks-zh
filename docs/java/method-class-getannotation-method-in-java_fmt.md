...
> 原文：`https://www.geeksforgeeks.org/method-class-getannotation-method-in-java/`

`Method` 类的 `getAnnotation()` 方法返回作为参数传递的指定类型的注释（如果存在这样的注释，则返回 `null`）。这是获取方法对象注释的重要方法。
...
该方法取一个强制参数 `annotationClass`，它是注释类型的 `Class` 对象。

## 返回值

此方法返回指定注释类型的方法注释（如果存在于此元素中），否则为 `null`。

## 异常

如果给定的注释类为 `null`，该方法抛出 `NullPointerException`。

下面的程序说明了 `Method` 类的 `getAnnotation()` 方法：

## 示例 1

该程序打印指定注释类型的方法注释，该注释类型作为参数提供给表示 `GFG` 类的 `getCustomAnnotation()` 方法的 `Method` 对象的 `getAnnotation()`。
