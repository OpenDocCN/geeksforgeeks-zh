...
> 原文: [https://www.geeksforgeeks.org/java-8-linkedblockingqueue-spliterator-method-with-examples/](https://www.geeksforgeeks.org/java-8-linkedblockingqueue-spliterator-method-with-examples/)

`LinkedBlockingQueue`的`spliterator()`方法返回一个与`LinkedBlockingQueue`元素相同的[拆分器](https://www.geeksforgeeks.org/java-util-interface-spliterator-java8/)。返回的迭代器弱一致。它可以和 Java 8 中的 Streams 一起使用。此外，它还可以单独和批量遍历元素。
...
从`LinkedBlockingQueue`创建一个分割器，该分割器包含一个班级不同学生的名字。
...
从包含员工对象列表的`LinkedBlockingQueue`创建一个拆分器。
