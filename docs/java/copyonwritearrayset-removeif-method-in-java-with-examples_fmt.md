...
> 原文：[https://www.geeksforgeeks.org/copyonwritearrayset-removeif-method-in-java-with-examples/](https://www.geeksforgeeks.org/copyonwritearrayset-removeif-method-in-java-with-examples/)

`CopyOnWriteArraySet`的`removeIf()`方法从该`CopyOnWriteArraySet`中移除满足指定条件的元素。
...
该方法接受一个强制参数`filter`，它是谓词值，基于该值从该集合中移除元素。
