...
> 原文: [https://www.geeksforgeeks.org/implement-splash-screen-and-authentication-in-social-media-android-app/](https://www.geeksforgeeks.org/implement-splash-screen-and-authentication-in-social-media-android-app/)

这是“在安卓工作室上构建社交媒体应用”教程的第 1 部分，我们将在本文中介绍以下功能:
...
转到 `工具 > Firebase > 认证` 将你的应用连接到 Firebase 并且将 `Firebase` 认证 `SDK` 添加到你的应用中，如下图所示。

![](img/181e476cbc0df3780ceb56b11999f60a.png)

然后到 [Firebase 控制台](https://console.firebase.google.com/) `> 认证 > 登录方式` 启用邮件/密码。

![](img/b281eaf401887fccbf3e6e1a67ae85a8.png)

**第三步: 新建 4 个空活动**

转到 `包名称 > 右键单击 > 新建 > 活动 > 清空活动` 并将活动命名为 `SplashScreen`、`RegistrationActivity`、`LoginActivity` 和 `DashboardActivity`。你也可以参考这篇文章[如何在安卓工作室](https://www.geeksforgeeks.org/how-to-create-constructor-getter-setter-methods-and-new-activity-in-android-studio-using-shortcuts/)创建新的空活动。

**步骤 4: 使用闪屏活动**

导航到 `应用程序 > res > 布局 > activity_splash_screen.xml` 并将下面的代码添加到该文件中。下面是活动文件的代码。
...
使用 `SplashScreen.java` 文件。这里我们检查如果用户为空，则转到 `LoginActivity`。否则移至 `DashboardActivity`。
...
导航到 `应用程序 > res > 布局 > activity_registration.xml`，并将下面的代码添加到该文件中。下面是 `activity_registration.xml` 文件的代码。这里我们为姓名、电子邮件和密码创建了三个[编辑文本](https://www.geeksforgeeks.org/edittext-widget-in-android-using-java-with-examples/)，并为注册用户创建了一个[按钮](https://www.geeksforgeeks.org/button-in-kotlin/)。
