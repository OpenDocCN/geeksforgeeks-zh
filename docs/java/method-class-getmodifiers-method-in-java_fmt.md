# Java 中的 Method 类 | getModifiers() 方法

> 原文: [https://www.geeksforgeeks.org/method-class-getmodifiers-method-in-java/](https://www.geeksforgeeks.org/method-class-getmodifiers-method-in-java/)

`Method` 类的 `getModifiers()` 方法返回该 `Method` 对象表示的方法的修饰符。它返回一个 `int` 值。然后可以使用 `Modifier` 类，获取对应于该值的修饰符的名称。
...
当在类的方法对象上应用 `getModifiers()` 时，该程序打印方法的修饰符。
...
程序演示如何应用 `Method` 类的 `getModifiers()` 方法。程序打印 `demo` 类的所有方法的修饰符名称。
    ...
    public static void main(String[] args)
    {
        try {
            ...
            for (Method method : methods) {
                ...
                // print method name
                System.out.println("Modifier of method name:" + method.getName());
...
*解释:* 这个程序的输出还显示了从 `Object` 类继承而来的方法（如 `wait`、`equals`、`toString`、`hashCode`、`getClass`、`notifyAll`）的修饰符信息，这些方法是通过类对象从 `java.lang` 包的超类 `Object` 继承而来的。

## 参考

[https://docs.oracle.com/javase/8/docs/api/java/lang/reflect/Method.html#getModifiers--](https://docs.oracle.com/javase/8/docs/api/java/lang/reflect/Method.html#getModifiers--)