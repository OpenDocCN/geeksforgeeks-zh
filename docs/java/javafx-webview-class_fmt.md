...
| `prefHeight(double v)` | 设置 webview 的首选高度。 |
| `prefWidth(double v)` | 设置 webview 的首选宽度。 |
| `setFontScale(double v)` | 设置 webview 的字体比例。 |
...
| `setZoom(double v)` | 设置 webview 的缩放比例。 |

以下程序说明了 Webview 类的使用:

### Java Program to create a WebView and load a website and display it on the stage:
