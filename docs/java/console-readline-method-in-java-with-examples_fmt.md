# Java 中的 `Console` `readLine()` 方法，带示例

> 原文：[https://www.geeksforgeeks.org/console-readline-method-in-java-with-examples/](https://www.geeksforgeeks.org/console-readline-method-in-java-with-examples/)

Java 中 `Console` 类的 `readLine()` 方法有两种类型：

## 1. `readLine()` 方法

Java 中 `Console` 类的 `readLine()` 方法用于从控制台读取单行文本。
...
以下程序说明了 `Console` 类的 `readLine()` 方法：
...
**Output：**
![](img/19605beb5b5c30b42157e4e90286deb0.png)
...
**Output：**
![](img/63d48ab02d77eb839e231440e58f74f8.png)

## 2. `readLine(String, Object)` 方法

Java 中 `Console` 类的 `readLine(String, Object)` 方法用于通过提供格式化提示从控制台读取单行文本。
...
下面的程序说明了 `Console` 类的 `readLine(String, Object)` 方法：
