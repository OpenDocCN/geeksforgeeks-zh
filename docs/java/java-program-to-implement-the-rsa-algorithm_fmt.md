...
> 原文：[https://www.geeksforgeeks.org/java-program-to-implement-the-rsa-algorithm/](https://www.geeksforgeeks.org/java-program-to-implement-the-rsa-algorithm/)
...
5.  计算 `d`，例如 `e*d mod ϕ(n) = 1`
6.  公钥 `{e, n}` 私钥 `{d, n}`
7.  密文 `C = P^e mod n` 其中 `P` 是明文
8.  对于解密，`D = C^d mod n`，其中 `D` 将返还明文。
