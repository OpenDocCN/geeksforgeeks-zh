...
> 原文: [https://www.geeksforgeeks.org/how-to-compile-and-run-c-c-plus-plus-java-programs-in-linux/](https://www.geeksforgeeks.org/how-to-compile-and-run-c-c-plus-plus-java-programs-in-linux/)
...
这里，`filename.c` 是 C 程序文件，用 `-o` 选项显示代码中的错误。如果没有发现错误，这将生成一个可执行文件作为 `executable_file_name`。
...
这里，`filename.cpp` 是 C++ 程序文件，使用 `-o` 选项显示代码中的错误。如果没有发现错误，这将生成一个可执行文件作为 `executable_file_name`。
