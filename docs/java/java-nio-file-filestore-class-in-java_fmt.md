...
> 原文：[https://www.geeksforgeeks.org/java-nio-file-filestore-class-in-java/](https://www.geeksforgeeks.org/java-nio-file-filestore-class-in-java/)
...
| 方法 | 描述 |
| --- | --- |
| `getAttribute(String attribute)` | 此方法读取文件存储属性值。 |
| `getFileStoreAttributeView(Class<V> type)` | 此方法返回给定类型的文件属性视图。 |
...
| `name()` | 此方法返回此文件存储的名称。 |
| `supportsFileAttributeView(Class<? extends FileAttributeView> type)` | 此方法告知此文件存储是否支持由给定文件属性视图标识的文件属性。 |
| `supportsFileAttributeView(String name)` | 此方法告知此文件存储是否支持由给定文件属性视图标识的文件属性。 |
| `type()` | 此方法返回此文件存储的类型。 |
