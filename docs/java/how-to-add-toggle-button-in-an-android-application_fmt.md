...
> 原文: [https://www.geeksforgeeks.org/how-to-add-toggle-button-in-an-android-application/](https://www.geeksforgeeks.org/how-to-add-toggle-button-in-an-android-application/)
...
新建一个项目，填写 app 所需的所有细节，如 app 名称、包名等。

> 选择 `文件` -> `新建` -> `新建项目` -> 填写所需的详细信息，然后单击 `完成`。

### 步骤 2

在这一步中，打开 XML 文件并添加代码以显示切换按钮和文本视图。
...
在这一步中，打开 `MainActivity` 并添加下面的代码来初始化切换按钮，并添加一个当用户点击切换按钮时将被调用的 `onToggleClick` 方法。此方法更改 `textview` 中的文本。

> 打开 `应用` -> `Java` -> `包` -> `MainActivity.java`
