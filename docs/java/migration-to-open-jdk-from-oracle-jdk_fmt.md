...
> 原文：[https://www.geeksforgeeks.org/migration-to-open-jdk-from-oracle-jdk/](https://www.geeksforgeeks.org/migration-to-open-jdk-from-oracle-jdk/)
...
在比较甲骨文 Java v/s 开放 JDK 之前，让我们先了解一下技术要求。

对于比 `Oracle Java JE 8` 补丁程序 `211` 和 `JDK 11` 更新的任何内容，Oracle 都引入了订阅费，从安全漏洞的角度来看，将版本保持在旧的、不受支持的补丁级别并不理想。因此，我们必须确定安装的 `Oracle Java JE` 或 `JDK` 是否真的有必要在服务器上运行任何应用程序。
