...
`java.util.concurrent.ConcurrentHashMap` 的 `isEmpty()` 方法是 Java 中的一个内置函数，它检查这个映射是否包含任何键值映射，并返回一个布尔值。
...
该函数返回一个 `布尔值`。如果 `ConcurrentHashMap` 为空，则返回 `true`，否则返回 `false`。

以下程序说明了 `isEmpty()` 方法的使用：

### 程序 1

在这个程序中，`ConcurrentHashMap` 是非空的。
...
[https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/ConcurrentHashMap.html#isEmpty()](https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/ConcurrentHashMap.html#isEmpty())