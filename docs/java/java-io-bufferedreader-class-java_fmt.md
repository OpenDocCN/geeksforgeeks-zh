...
> 原文：[https://www.geeksforgeeks.org/java-io-bufferedreader-class-java/](https://www.geeksforgeeks.org/java-io-bufferedreader-class-java/)
...
### `void close()`
关闭流并释放与其相关联的任何系统资源。一旦流被关闭，进一步的 `read()`、`ready()`、`mark()`、`reset()` 或 `skip()` 调用将引发 `IOException`。关闭以前关闭的流没有效果。
...
将字符读入数组的一部分。这个方法实现了 `Reader` 类对应的 `read` 方法的总约定。为了方便起见，它试图通过重复调用底层流的 `read` 方法来读取尽可能多的字符。这种迭代读取一直持续到下列条件之一变为真：
