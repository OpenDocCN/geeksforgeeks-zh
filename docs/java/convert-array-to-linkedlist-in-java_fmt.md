...
> 原文：[https://www.geeksforgeeks.org/convert-array-to-linkedlist-in-java/](https://www.geeksforgeeks.org/convert-array-to-linkedlist-in-java/)

数组是连续的内存分配，而链表是随机放置在内存中的元素块，它们被链接在一起，其中一个块保存内存中另一个块的地址。有时根据需求或者因为内存中的空间问题，在进取世界中有更大的代码块，有必要将数组转换为列表。这里演示了数组到链表的转换。

## 方法

1.  使用 [`asList()`](https://www.geeksforgeeks.org/arrays-aslist-method-in-java-with-examples/) 方法来收集类。
2.  使用 [`addAll()`](https://www.geeksforgeeks.org/java-util-linkedlist-addall-method-in-java/) 方法来收集类。

### 方法 1：使用 `asList()` 方法的集合类

这个方法结合 `Collection.toArray()` 充当基于数组和基于集合的 API 之间的桥梁。返回的列表被序列化并实现随机访问。这在 O(1) 时间内运行。
...
1.  定义数组。
2.  创建一个空的链表。
3.  使用 `Collections` 类的 `addAll()` 方法，以两个对象作为参数。
    *   第一个对象是要转换的列表。
    *   第二个对象是要转换的数组。
