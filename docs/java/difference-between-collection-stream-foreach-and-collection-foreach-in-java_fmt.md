# Collection.stream().forEach() 与 Collection.forEach() 在 Java 中的差异

> 原文：[https://www.geeksforgeeks.org/difference-between-collection-stream-foreach-and-collection-foreach-in-java/](https://www.geeksforgeeks.org/difference-between-collection-stream-foreach-and-collection-foreach-in-java/)

`Collection.forEach()` 和 `Collection.stream().forEach()` 都用于迭代集合，两者之间没有太大的区别，因为两者给出了相同的结果，尽管它们的内部工作方式有些不同。
...
## `Collection.stream().forEach()` 与 `Collection.forEach()` 的差异
