# Long.toBinaryString()方法，带示例

> 原文：[https://www.geeksforgeeks.org/java-lang-long-tobinarystring-method-examples/](https://www.geeksforgeeks.org/java-lang-long-tobinarystring-method-examples/)
...
参数：该函数接受一个强制参数：
`num` - 此参数指定要转换为二进制字符串的数字。其数据类型为 `Long`。

## 返回值

该函数返回由二进制（基数为 2）中的参数表示的无符号 `Long` 值的字符串表示形式。
