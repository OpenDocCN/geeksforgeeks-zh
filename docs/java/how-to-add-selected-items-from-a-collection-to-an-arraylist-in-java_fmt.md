...
> 原文：[https://www.geeksforgeeks.org/how-to-add-selected-items-from-a-collection-to-an-arraylist-in-java/](https://www.geeksforgeeks.org/how-to-add-selected-items-from-a-collection-to-an-arraylist-in-java/)

给定一个具有一些值的集合，任务是将这个集合的选定项添加到 Java 中的数组列表中。

## 示例

> **输入：** 集合 = `[1, 2, 3]`，条件 = `(item != 2)`
> **输出：** 数组列表 = `[1, 3]`
>
> **输入：** 集合 = `["GFG", "Geeks", "GeeksForGeeks"]`，条件 = `(item != "GFG")`
> **输出：** 数组列表 = `["Geeks", "GeeksForGeeks"]`
...
*   使用 [`filter()` 方法](https://www.geeksforgeeks.org/stream-filter-java-examples/)从流中选择所需项目。
...
> 包含精选集合元素的数组列表 `["GFG", "Geeks", "GeeksForGeeks"]`： `["Geeks", "GeeksForGeeks"]`