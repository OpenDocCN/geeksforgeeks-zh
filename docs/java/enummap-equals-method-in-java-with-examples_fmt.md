# EnumMap的equals()方法在Java中的示例

> 原文：[https://www.geeksforgeeks.org/enummap-equals-method-in-java-with-examples/](https://www.geeksforgeeks.org/enummap-equals-method-in-java-with-examples/)

Java中的`java.util.EnumMap.equals(Object obj)`方法用于将传递的对象与**这个EnumMap**进行比较以判断相等性。必须记住，传递的对象必须是与`EnumMap`类型相同的映射。
...
**参数：** 该方法接受一个`Object`类型的参数`obj`，用于参照该地图进行比较。
**返回值：** 如果指定对象等于地图，则方法返回`true`，否则返回`false`。

以下程序说明了`equals()`方法的工作方式：
