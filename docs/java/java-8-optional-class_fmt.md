...
[Java 8 可选类](https://www.geeksforgeeks.org/java-8-optional-class/)

可选是一个容器对象，它可以包含也可以不包含非空值。您必须导入 `java.util` 包才能使用该类。如果存在一个值，`isPresent()` 将返回 `true`，`get()` 将返回该值。提供了依赖于包含值存在与否的其他方法，例如 `orElse()`，如果值不存在，则返回默认值；以及 `ifPresent()`，如果值存在，则执行一个代码块。这是一个基于 `值的` 类，即它们的实例是：
...
静态方法是 Java 中不需要创建类的对象就可以调用的方法。它们由类名本身或对该类的对象的引用来引用。
...
由于静态方法属于类，因此可以在不创建类对象的情况下调用它们。下面给出了一些关于静态方法的要点：
...
实例方法是指在调用之前需要创建其类的对象的方法。要调用实例方法，我们必须创建一个定义它的类的对象。
...
实例方法可以在它们所在的同一个类中调用，也可以从同一个包或其他包中定义的不同类中调用，具体取决于提供给所需实例方法的访问类型。下面给出了一些关于实例方法的要点：
...
一个具体的方法意味着，该方法有**完整的定义**，但是它可以在继承的类中被覆盖。如果我们把这个方法**做成最终的**，那么它就不能被覆盖。将方法或类声明为“final”意味着它的实现已经完成。必须重写抽象方法。具体方法如果不是最终的，可以在继承的类中被覆盖。下表为可选类提供的具体方法列表：
...
[Java 8 可选类](https://docs.oracle.com/javase/8/docs/api/java/util/Optional.html)
