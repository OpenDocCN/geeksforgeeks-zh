# Java 中的 AbstractSequentialList isEmpty() 方法示例

> 原文：[https://www.geeksforgeeks.org/abstractsequentiallist-isempty-method-in-java-with-example/](https://www.geeksforgeeks.org/abstractsequentiallist-isempty-method-in-java-with-example/)

Java 中 `AbstractSequentialList` 的 `isEmpty()` 方法用来检查这个抽象顺序列表是否为空。它返回一个布尔值，表示相同的内容。
...
该方法返回一个布尔值，表示 `AbstractSequentialList` 的这个实例是否为空。
