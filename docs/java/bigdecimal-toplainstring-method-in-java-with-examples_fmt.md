...
> 原文:[https://www.geeksforgeks.org/bigdecimal-toplainstring-method-in-java-with-examples/](https://www.geeksforgeks.org/bigdecimal-toplainstring-method-in-java-with-examples/)

`toPlainString()` 方法用于表示当前的 `BigDecimal`，通过该方法将该方法调用为不带指数字段的 `String` 形式。如果数值为正刻度，小数点右边的位数用于表示刻度。对于具有零刻度或负刻度的值，生成的结果字符串就好像该值被转换为具有零刻度的数值相等的值，并且好像零刻度值的所有尾随零都出现在结果中。

**注:** 如果将此方法的结果传递给 [`String`](https://www.geeksforgeeks.org/strings-in-java/) 构造函数，则只会恢复此 `BigDecimal` 的数值，新 `BigDecimal` 的表示可能会有不同的小数位数。特别是，该方法的行为类似于 [`toString()`](https://www.geeksforgeeks.org/object-tostring-method-in-java/) 方法。
...
[https://docs.oracle.com/en/java/javase/12/docs/api/java.base/java/math/BigDecimal.html#toPlainString()](https://docs.oracle.com/en/java/javase/12/docs/api/java.base/java/math/BigDecimal.html#toPlainString())