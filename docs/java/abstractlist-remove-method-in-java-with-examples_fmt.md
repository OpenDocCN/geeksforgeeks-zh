...
> 原文：[https://www.geeksforgeeks.org/abstractlist-remove-method-in-java-with-examples/](https://www.geeksforgeeks.org/abstractlist-remove-method-in-java-with-examples/)

`AbstractList`类的`remove(int index)`方法用于从特定位置或索引从抽象列表中移除元素。
...
参数`index`是整数数据类型，指定要从抽象列表中删除的元素的位置。
