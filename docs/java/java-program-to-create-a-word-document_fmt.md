...
> 原文: [https://www.geeksforgeeks.org/java-program-to-create-a-word-document/](https://www.geeksforgeeks.org/java-program-to-create-a-word-document/)

Word 文件应该是在没有 Word 的情况下，借助 Java 应用程序协议接口创建的。

**相关概念:** **Apache POI 和 Maven**

[Apache POI](https://www.geeksforgeeks.org/apache-poi-introduction/) 是 Apache 基金会提供的一个 API，是不同 java 库的集合。该工具使库可以读取、写入和操作不同的微软文件，如 Excel 工作表、PowerPoint 和 Word 文件。有两种类型，基本上是旧版本，包括`.doc`，`.ppt`而较新版本的文件为`.docx`，`.pptx`。有两种方法可以处理 Apache POI，如下所述:
...
**Spire.Doc for Java** 是一个专业的 Java Word API，它使 Java 应用程序能够在不使用 Microsoft Office 的情况下创建、转换、操作和打印 Word 文档。它将被导入作为该程序的参考。
...
| 可怕的文字处理器格式( **HWPF** ) | MS-Word 的`.doc`扩展文件读写 |
