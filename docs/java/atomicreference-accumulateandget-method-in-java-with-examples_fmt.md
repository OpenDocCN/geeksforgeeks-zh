...
> 原文：[https://www.geeksforgeeks.org/atomicreference-accumulateandget-method-in-java-with-examples/](https://www.geeksforgeeks.org/atomicreference-accumulateandget-method-in-java-with-examples/)

一个`AtomicReference`类的`accumulateAndGet()`方法被用来自动更新原子引用的当前值，将给定的累计函数应用于当前值和给定值，并返回更新后的值。累积函数应该没有副作用，因为当尝试的更新由于线程之间的争用而失败时，它可以被重新应用。
...
[https://docs.oracle.com/javase/10/docs/api/java/util/concurrent/atomic/AtomicReference.html#accumulateAndGet(V, java.util.function.BinaryOperator)](https://docs.oracle.com/javase/10/docs/api/java/util/concurrent/atomic/AtomicReference.html#accumulateAndGet(V, java.util.function.BinaryOperator))