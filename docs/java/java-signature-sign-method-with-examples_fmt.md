...
> 原文: [https://www.geeksforgeeks.org/java-signature-sign-method-with-examples/](https://www.geeksforgeeks.org/java-signature-sign-method-with-examples/)

## `sign(byte[] dataBuffer, int offset, int length)`

`java.security.Signature` 类的 `sign()` 方法用于完成签名操作，并将结果签名字节存储在提供的缓冲区 `dataBuffer` 中，从 `offset` 开始。签名的格式取决于基础签名方案。
...
> **参数:** 该方法将以下参数作为参数
> - `dataBuffer` – 签名结果的缓冲区为 `byte[]` 数组。
> - `offset` – 偏移到存储签名的数据缓冲器。
> - `length` – 分配给签名的数据缓冲器中的字节数。
