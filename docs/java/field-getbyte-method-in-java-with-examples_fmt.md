...
> 原文：[https://www.geeksforgeeks.org/field-getbyte-method-in-java-with-examples/](https://www.geeksforgeeks.org/field-getbyte-method-in-java-with-examples/)

[`Field`](https://www.geeksforgeeks.org/reflection-in-java/) 类的 `getByte()` 方法用于获取字节的值，该字段必须是静态或实例的 `byte` 类型字段。当一个类包含一个静态或实例 `byte` 字段，并且我们想要得到该字段的值时，我们可以使用这个方法返回字段的值。
...
## 参数
该方法接受单个参数 `obj`，该参数是提取字节值的对象。

## 返回值
该方法返回转换为类型 `byte` 的字段值。

## 异常
该方法抛出以下异常：

1.  `IllegalAccessException`：如果 `Field` 对象正在执行 Java 语言访问控制，并且基础字段不可访问，则会引发此异常。
2.  `IllegalArgumentException`：如果指定的对象不是声明基础字段的类或接口的实例，或者如果字段值不能通过扩展转换转换为类型 `byte`，则会引发此异常。
...
## 参考文献
[https://docs.oracle.com/javase/8/docs/api/java/lang/reflect/Field.html#getByte-java.lang.Object-](https://docs.oracle.com/javase/8/docs/api/java/lang/reflect/Field.html#getByte-java.lang.Object-)