...
*   `SessionFactory` 是一个存在于 `org.hibernate` 包中的接口，用于创建 `Session` 对象。
*   它本质上是不可变的和线程安全的。
...
*   `Session` 是 `org.hibernate` 包中的一个接口。`Session` 对象是基于 `SessionFactory` 对象（即工厂）创建的。
*   它通过 Hibernate 框架打开与数据库软件的连接/会话。
*   这是一个重量轻的物体，它不是线程安全的。
*   `Session` 对象用于执行 CRUD 操作。
...
*   每当我们执行任何操作时都会使用 `Transaction` 对象，并且基于该操作，数据库中会有一些变化。
*   `Transaction` 对象用于向数据库发出指令，通过使用 `commit()` 方法，使由于操作而发生的更改永久化。
...
*   `Query` 是 `org.hibernate` 包中的一个接口。
*   `Query` 实例通过调用 `Session.createQuery()` 获得。
...
*   `Criteria` 是一个简化的应用编程接口，用于通过组合 `Criteria` 对象来检索实体。
*   进程是标准工厂。`Criteria` 实例通常通过工厂限制方法获得。
