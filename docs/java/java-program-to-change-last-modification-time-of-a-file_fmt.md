...
> 原文：[https://www.geeksforgeeks.org/java-program-to-change-last-modification-time-of-a-file/](https://www.geeksforgeeks.org/java-program-to-change-last-modification-time-of-a-file/)

使用 Java 的 `File` 类，即 `File.setLastModified()` 方法，可以通过 Java 修改文件的日期。

## Java 文件类

`File` 类是 Java 对文件或目录路径名的表示。`File` 类包含几种方法，用于处理路径名、删除和重命名文件、创建新目录、列出目录内容以及确定文件和目录的几种常见属性。

## 设置最后修改的方法

`setLastModified()` 函数是一个在 Java `File` 类中预先定义的方法。该函数设置文件或目录的最后修改时间。该函数以毫秒为单位设置文件的最后修改值（`long` 类型）。

**参数** – 由新的上次修改时间（毫秒）组成的字符串。

**返回值** – 返回一个布尔值。（如果操作成功，则为 `true`，否则为 `false`）。
...
4.  最后，使用 `file.setLastModified(date.getTime())` 方法来设置我们文件的新的“上次修改”日期。
