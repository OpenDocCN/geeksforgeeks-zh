# Java 字符串类 `lines()` 方法示例

> 原文: [https://www.geeksforgeeks.org/java-string-class-lines-method-with-examples/](https://www.geeksforgeeks.org/java-string-class-lines-method-with-examples/)

`lines()` 方法是一种静态方法，它返回从给定的多行字符串中提取的行流，由行终止符分隔，如下所示：
...
在这里，我们将讨论三个例子，以更好地理解字符串类 `lines()` 方法对数据结构的作用。
...
### 示例 2: 使用 `forEach` 将行流转换为 `ArrayList`
