...
> 原文: [https://www.geeksforgeeks.org/java-lang-character-class-methods-set-2/](https://www.geeksforgeeks.org/java-lang-character-class-methods-set-2/)
...
## 1. `toUpperCase()`
`java.lang.Character.toUpperCase(char arg)`方法基于 Unicode 值将参数中的给定字符转换为大写。
...
## 2. `toLowerCase()`
`java.lang.Character.toLowerCase(char arg)`方法基于 Unicode 值将参数中的给定字符转换为小写。
...
## 4. `reverseBytes()`
`java.lang.Character.reverseBytes(char cP)`方法通过反转参数化字符中字节的顺序返回一个字符。

**语法:**
```java
public static char reverseBytes(char cP)
Parameters : 
...
## 5. `isAlphabetic()`
`java.lang.Character.isAlphabetic(int cP)`方法检查论证字符(代码点)是否是字母表
...
## 6. `isValidCodePoint()`
`java.lang.Character.isValidCodePoint(int cP)` method checks whether the argumented Unicode is actually valid or not
...
### 解释 `isValidCodePoint()`，`reverseBytes()`，`isAlphabetic()`方法使用的 Java 代码
```java
// Java program explaining Character class methods
// isValidCodePoint(), reverseBytes(), isAlphabetic()
import java.lang.Character;
public class NewClass
{
    public static void main(String[] args)
