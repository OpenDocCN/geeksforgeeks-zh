...
> 原文: [https://www.geeksforgeeks.org/for-loop-java-important-points/](https://www.geeksforgeeks.org/for-loop-java-important-points/)

[编程语言中的循环](https://www.geeksforgeeks.org/loops-in-java/)是一种功能，当某个条件评估为真时，它有助于重复执行一组指令/功能。在 Java 中，任何编程语言都不可能提供四种执行循环的方式，即 `while` 循环、`for` 循环、`for-each` 循环、`do-while` 循环，或者我们基本上可以称之为**三种类型的循环**在一些书籍中，`for-each` 循环都被视为增强 `for` 循环。让我们详细讨论循环。
...
根据变量范围的[概念](https://www.geeksforgeeks.org/variable-scope-in-java/)，初始化块中声明的变量只能在循环中访问。
