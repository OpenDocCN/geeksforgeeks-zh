...
2.  `JSpinner(SpinnerModel model)`: 用作为参数传递的指定微调器模型创建微调器。

## 常用方法

1.  `SpinnerListModel(List l)`: 用列表 `l` 的元素创建一个 spinner 模型。这个 spinner 模型可以用来设置为 spinner 的模型。
2.  `SpinnerNumberModel(int value, int max, int min, int step)`: 返回一个 spinner 模型，其初始值设置为 `value`，有最小值和最大值，以及一个确定的 `step` 值。
3.  `setValue(Object v)`: 将微调器的值设置为作为参数传递的对象。
...
## 程序示例2：带ChangeListener的JSpinner

程序创建一个 `JSpinner`，并向其中添加 `ChangeListener`。使用 `JSpinner` 程序选择您的出生日期。
