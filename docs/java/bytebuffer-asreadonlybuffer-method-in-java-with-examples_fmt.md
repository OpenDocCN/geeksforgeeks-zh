...
> 原文: [https://www.geeksforgeeks.org/bytebuffer-asreadonlybuffer-method-in-java-with-examples/](https://www.geeksforgeeks.org/bytebuffer-asreadonlybuffer-method-in-java-with-examples/)
...
如果此缓冲区本身是只读的，则此方法的行为方式与 `copy()` 方法完全相同。
...
该方法返回新的只读字节缓冲区，其内容与该缓冲区的内容相同。

以下是举例说明 `asReadOnlyBuffer()` 方法的例子:
