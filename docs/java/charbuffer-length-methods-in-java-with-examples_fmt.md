...
> 原文：[https://www.geeksforgeeks.org/charbuffer-length-methods-in-java-with-examples/](https://www.geeksforgeeks.org/charbuffer-length-methods-in-java-with-examples/)

`java.nio.CharBuffer` 类的 `length()` 方法用来返回这个字符缓冲区的长度。当视为字符序列时，字符缓冲区的长度只是位置（包括）和限制（不包括）之间的字符数；也就是说，它相当于 `remaining()`。
