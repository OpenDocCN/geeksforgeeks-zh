...
`java.util.concurrent.ConcurrentSkipListSet` 的 `clone()` 方法是 Java 中的一个内置函数，它返回这个 `ConcurrentSkipListSet` 实例的一个浅层副本。
...
函数返回一个 `ConcurrentSkipListSet` 的浅拷贝。
...
**参考：** [https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/ConcurrentSkipListSet.html#clone()](https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/ConcurrentSkipListSet.html#clone())