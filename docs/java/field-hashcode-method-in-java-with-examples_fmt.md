# Java 中的字段 hashCode()方法

> 原文：[https://www.geeksforgeeks.org/field-hashcode-method-in-java-with-examples/](https://www.geeksforgeeks.org/field-hashcode-method-in-java-with-examples/)

`java.lang.reflect.Field` 的 `hashCode()` 方法用于获取该 `Field` 的 `hashCode`。最终的哈希码被计算为基础字段的声明类名及其名称的哈希码的异或。如果对象没有改变，哈希码总是相同的。哈希码是 JVM 在创建对象时生成的唯一代码。它可以用来对哈希相关算法进行一些操作，比如哈希表。也可以用这个唯一的代码搜索一个对象。
...
[https://docs.oracle.com/javase/8/docs/api/java/lang/reflect/Field.html#hashCode--](https://docs.oracle.com/javase/8/docs/api/java/lang/reflect/Field.html#hashCode--)