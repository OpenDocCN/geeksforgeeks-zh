...
`IntStream.mapToDouble()` 是一个中间操作。这些操作总是懒惰的。中间操作在流实例上被调用，在它们完成处理后，它们给出一个流实例作为输出。
...
1.  **`DoubleStream`**: 原始双值元素的序列。这就是流的双原始特化。
2.  **`mapper`**: 应用于每个元素的无状态函数。

## 返回值

该函数返回一个 `DoubleStream`，该流由给定函数应用于该流元素的结果组成。
...
// Driver code
    public static void main(String[] args)
    {
        ...
        // Using DoubleStream mapToDouble(IntToDoubleFunction mapper)
