# Java 中的 `isEnum()` 方法，带示例

> 原文：[`https://www.geeksforgeeks.org/class-isenum-method-in-java-with-examples/`](https://www.geeksforgeeks.org/class-isenum-method-in-java-with-examples/)

[`java.lang.Class`](https://www.geeksforgeeks.org/java-lang-class-class-java-set-1/) 类的 `isEnum()` 方法用于检查该类是否是枚举类。如果此类是枚举类，则方法返回 `true`。否则返回 `false`。
...
如果这个类是枚举类，这个方法返回 `true`。否则返回 `false`。
