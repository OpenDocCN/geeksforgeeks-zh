...
> 原文：[https://www.geeksforgeeks.org/java-program-to-illustrate-a-method-without-parameters-but-with-return-type/](https://www.geeksforgeeks.org/java-program-to-illustrate-a-method-without-parameters-but-with-return-type/)

任务是说明一个不包含任何参数但应该返回值的方法。方法用于将一个完整的程序分解成可以逐个执行的模块。考虑任何随机形状来说明：圆形。

**例：** 首先要创建一个名为 `Circle` 的类，然后借助这个类，它可以计算圆的面积和周长。

1.  面积计算
2.  周长计算

在类 `Circle` 中，我们将声明一个名为 `radius` 的变量来获取用户的半径值，这样通过使用这个半径值，我们将能够计算圆的面积和周长。现在，为了计算面积和周长，我们需要创建两个单独的方法，称为 `area()` 和 `circumference()`，这两个方法没有将任何值作为参数，但肯定会返回一些值。这种方法返回的值是圆的面积和周长的最终值。这些方法将在变量值的帮助下计算后返回值。最重要的是计算面积和周长，使用这些方法首先需要的是调用那些被称为调用函数的方法，为此我们必须创建一个 `Circle` 类的对象，因为在主方法中，我们想要访问它们，另一个类的方法因此我们需要使包含我们的方法的类的对象 `area()` 和 `circumference()` 并且通过使用类的对象 `Circle`，我们将容易地调用方法并且那些方法将返回一些值，并且我们将通过使用我们的预定义函数来显示它。
