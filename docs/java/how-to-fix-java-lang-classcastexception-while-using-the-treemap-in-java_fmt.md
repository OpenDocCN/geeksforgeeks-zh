# 如何在Java中使用TreeMap的同时修复java.lang.ClassCastException？

> 原文：[https://www.geeksforgeeks.org/how-to-fix-java-lang-classcastexception-while-using-the-treemap-in-java/](https://www.geeksforgeeks.org/how-to-fix-java-lang-classcastexception-while-using-the-treemap-in-java/)

`java.lang.ClassCastException`是Java中**未检查的异常**之一。当我们试图将**一个类类型**的对象转换为**另一个类类型**的对象时，可能会出现这种情况。

当我们在`TreeMap`中使用自定义类对象作为键，并且既没有实现[`Comparable`接口](https://www.geeksforgeeks.org/comparable-vs-comparator-in-java/)也没有实现[`Comparator`接口](https://www.geeksforgeeks.org/comparator-interface-java/)时，那么`java.lang.ClassCastException`就出现了。

所以有**两种方式**来修复`java.lang.ClassCastException`同时使用Java中的`TreeMap`：

*   使用[`Comparable`](https://www.geeksforgeeks.org/comparable-vs-comparator-in-java/)
*   使用[`Comparator`](https://www.geeksforgeeks.org/comparator-interface-java/)
