...
> 原文: [https://www.geeksforgeeks.org/java-program-to-implement-attribute-api/](https://www.geeksforgeeks.org/java-program-to-implement-attribute-api/)
...
3.  分配属性。
4.  使用 `putValue()` 方法在该属性中存储与键名关联的值 `Value`。
5.  使用 `keySet()` 方法返回包含在该属性中找到的所有键的集合。
6.  遍历对象类型的集合接口的迭代器。
7.  使用 `hasNext()` 方法检查条件，该方法保持真，直到只剩下一个元素来打印元素。
8.  使用 `clear()` 方法清除集合中的所有属性对象。
