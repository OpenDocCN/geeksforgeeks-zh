# Java 中的 `String.getBytes()`

> 原文：[https://www.geeksforgeeks.org/java-lang-string-getbyte-java/](https://www.geeksforgeeks.org/java-lang-string-getbyte-java/)

`getBytes()` 函数用于将字符串转换为字节序列，并返回字节数组。该功能可以通过两种方式实现。本文将讨论这两种方法。

## 语法 1 – `public byte[] getBytes()`

此函数不接受参数，使用默认字符集将字符串编码为字节。
...
## 语法 2 – `public byte[] getBytes(Charset charset)`

此实现接受一个字符集参数，指定在转换为字节时用于编码字符串的字符集。定义了许多字符集，如下所述。
...
public class GetBytecharset {
    public static void main(String args[]) {
        ...
        try {
            // converting the string into byte using getBytes ( converts into UTF-16 values )
