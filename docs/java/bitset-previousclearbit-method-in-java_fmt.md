...
> 原文: [https://www.geeksforgeeks.org/bitset-previousclearbit-method-in-java/](https://www.geeksforgeeks.org/bitset-previousclearbit-method-in-java/)
...
该方法取 `fromIndex` 作为开始检查的索引(含)。

### 返回值

该方法返回前一个清零位的索引，如果没有该位则返回 -1。

### 异常

如果指定的索引小于 -1，该方法抛出 `IndexOutOfBoundsException`。

下面的程序举例说明了 `previousClearBit()` 方法：
...
显示 `IndexOutOfBoundsException` 异常:
