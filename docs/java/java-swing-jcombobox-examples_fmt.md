...
> 原文：`https://www.geeksforgeeks.org/java-swing-jcombobox-examples/`
...
4.  `JComboBox(Vector 项目)`：用指定矢量中的项目创建一个新的 `JComboBox`。

常用方法有：

1.  `addItem(E 项)`：将项目添加到 `JComboBox`。
...
6.  `removeItem(int i)`：移除索引 `i` 处的元素。
7.  `setEditable(boolean b)`：布尔 `b` 决定组合框是否可编辑。如果传递 `true`，则组合框是可编辑的，反之亦然。
8.  `setSelectedIndex(int i)`：选择索引 `i` 处的 `JComboBox` 的元素。
9.  `showPopup()`：使组合框显示其弹出窗口。
10. `setUI(ComboBoxUI)`：设置渲染这个组件的 L&F 对象。
11. `setSelectedItem(Object a)`：将组合框显示区的选中项设置为参数中的对象。
12. `setSelectedIndex(int a)`：选择索引处的项目。
13. `setPopupVisible(boolean v)`：设定弹出窗口的可见性。
14. `setModel(ComboBoxModel a)`：设置 `JComboBox` 获取项目列表时使用的数据模型。
15. `setMaximumRowCount(int count)`：设置 `JComboBox` 显示的最大行数。
16. `setEnabled(boolean b)`：启用组合框，以便可以选择项目。
17. `removeItem(Object orObject)`：从项目列表中移除项目。
18. `removeAllItems()`：从项目列表中移除所有项目。
19. `removeActionListener(ActionListener l)`：删除 `action listener`。
20. `isPopupVisible()`：确定弹出窗口的可见性。
21. `addPopupMenuListener(PopupMenuListener l)`：添加一个弹出项目侦听器，它将从组合框的弹出部分侦听通知消息。
...
29. `firePopupMenuWillBecomeInvisible()`：通知 `PopupMenuListeners` 组合框的弹出部分已经变得不可见。
30. `firePopupMenuWillBecomeVisible()`：通知 `PopupMenuListeners` 组合框的弹出部分将变得可见。
31. `setEditor(ComboBoxEditor a)`：设置用于在 `JComboBox` 字段中绘制和编辑所选项目的编辑器。
32. `setActionCommand(String a)`：设置发送给 `actionListeners` 的事件中应该包含的动作命令。
...
以下程序将说明 `JComboBox` 的使用。程序创建一个简单的 `JComboBox` 并向其中添加元素。
