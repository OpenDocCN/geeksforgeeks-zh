...
> 原文: [https://www.geeksforgeeks.org/matcher-replacefirststring-method-in-java-with-examples/](https://www.geeksforgeeks.org/matcher-replacefirststring-method-in-java-with-examples/)

[`Matcher`](https://www.geeksforgeeks.org/regular-expressions-in-java/) 类的 `replaceFirst()` 方法表现为一种追加和替换方法。此方法读取输入字符串，并用匹配器字符串中的第一个匹配模式替换它。
