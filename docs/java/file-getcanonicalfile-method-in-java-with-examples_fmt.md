...
> 原文：[https://www.geeksforgeeks.org/file-getcanonicalfile-method-in-java-with-examples/](https://www.geeksforgeeks.org/file-getcanonicalfile-method-in-java-with-examples/)

`getCanonicalFile()` 方法是 [`File`](https://www.geeksforgeeks.org/File-class-in-java/) 类的一部分。这个函数返回给定文件对象的规范文件。如果文件对象的文件路径是规范的，那么它只返回当前文件对象的文件。
...
这里的 `program.txt` 是存在于当前工作目录中的文件
