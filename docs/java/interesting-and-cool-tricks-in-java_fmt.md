...
> 原文：[https://www.geeksforgeeks.org/interesting-and-cool-tricks-in-java/](https://www.geeksforgeeks.org/interesting-and-cool-tricks-in-java/)
...
## 1. Executing Comments
Most of the developers think [comments](https://www.geeksforgeeks.org/comments-in-java/) are never executed in a program and are used for ease in understanding the code. But, they are executed. For example:
...
In Java SE 7 and above, underscores can be used in [numeric literals](https://www.geeksforgeeks.org/g-fact-45-using-underscore-in-numeric-literals/) without generating any warning or error in the output.
...
## 4. Finding a position to insert the numeric element in the array
There is a small cool trick to find the position where the requested element can be inserted in the sorted array.
...
## 5. 包装类 vs 数据类型
在下面的示例中，第二个 print 语句不会显示 true，因为包装类对象的引用正在被比较，而不是它们的值。
