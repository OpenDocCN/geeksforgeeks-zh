...
> 原文：[https://www.geeksforgeeks.org/finding-minimum-and-maximum-element-of-a-collection-in-java/](https://www.geeksforgeeks.org/finding-minimum-and-maximum-element-of-a-collection-in-java/)
...
## 使用 Collections.min() 和 Collections.max() 方法

使用 `Collections.min()` 和 `Collections.max()` 方法可以很容易地找到集合的最小和最大元素。这些是 Java 中 `Collections` 类的静态方法。因此，它们可以通过类名直接访问，如下所示。
...
如上面的语法所示，`Collections.min()` 和 `Collections.max()` 方法将 `Collection` 作为参数。这是要找到最大/最小元素的集合。因此，任何实现 `Collection` 接口的类都可以作为参数在这些函数中传递，如数组、链表、数组列表、集合等。因此，任何集合的最大和最小元素都很容易找到。
...
`Map` 是 Java 集合框架中不同类型的实体，其中元素被视为键值对，而不是直接值。所以 `Map` 类**不实现 `Collection` 接口**。它们改为实现 `Map` 接口。因此 `Collections.min()` 和 `Collections.max()` 不适用于 `Map`，并将抛出**编译错误**。
...
no suitable method found for min(Map)
            int minMap = Collections.min(map);
                                    ^
method Collections.<T>min(Collection<? extends T>) 
is not applicable
```

### 如何求 Map 的最小和最大元素？

虽然 `Map` 不实现 `Collection` 接口，但是 `Map` 的元素类型 `键` 和 `值` 类**分别实现 `Collection` 接口**。因此，最小和最大元素可以根据它们的键或值在 `Map` 中找到。

**示例 1：** 根据键值查找 Map 的最小和最大元素。
