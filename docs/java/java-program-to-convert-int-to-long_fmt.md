...
> 原文：[https://www.geeksforgeeks.org/java-program-to-convert-int-to-long/](https://www.geeksforgeeks.org/java-program-to-convert-int-to-long/)
...
1.  用一个简单的赋值。这被称为**隐式类型转换**或类型提升，编译器会自动将较小的数据类型转换为较大的数据类型。
2.  使用 `java` 中 `long` 包装类的 `valueOf()` 方法将 `int` 转换为 `Long`。

## 1. 隐式类型转换

在这种情况下，我们只是将整数数据类型赋值给长数据类型。
由于与 `long` 相比，`int` 是一种较小的数据类型，编译器会自动将 `int` 转换为 `long`，这称为隐式类型转换或类型提升。
...
在本例中，我们使用 `long` 包装类的 `valueOf()` 方法将 `int` 转换为 `Long`。
