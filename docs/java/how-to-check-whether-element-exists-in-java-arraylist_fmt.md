...
> 原文: [https://www.geeksforgeeks.org/how-to-check-whether-element-exists-in-java-arraylist/](https://www.geeksforgeeks.org/how-to-check-whether-element-exists-in-java-arraylist/)

[Java `ArrayList`](https://www.geeksforgeeks.org/arraylist-in-java/) 是一个可调整大小的数组，可以在 `java.util` 包中找到。与内置数组不同，我们可以随时从数组列表中添加或删除元素。

我们可以通过两种方式检查 `java` 中的 `ArrayList` 中是否存在元素：
...
`java.util.ArrayList` 类的 [`contains()`](https://www.geeksforgeeks.org/arraylist-contains-java/) 方法，可以用来检查一个元素是否存在于 Java `ArrayList` 中。
...
**返回：** 如果在列表中找到指定的元素，则返回 `true`，否则返回 `false`。
...
class GFG {
    public static void main(String[] args)
    {
        ...
        // passing 5 as parameter to contains() function
        if (list.contains(5))
...
*   `contains()` 方法内部使用 `indexOf()` 方法来确定列表中是否存在指定的元素。
...
class GFG {
    public static void main (String[] args) {
        ...
        // passing 5 as parameter to indexOf() function
        if(list.indexOf(5) >= 0)
            System.out.println("5 exists in the ArrayList");
        else
            System.out.println("5 does not exist in the ArrayList");
