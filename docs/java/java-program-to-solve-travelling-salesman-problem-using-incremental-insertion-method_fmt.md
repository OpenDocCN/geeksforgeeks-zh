...
> 原文：[https://www.geeksforgeeks.org/java-program-to-solve-travelling-salesman-problem-using-incremental-insertion-method/](https://www.geeksforgeeks.org/java-program-to-solve-travelling-salesman-problem-using-incremental-insertion-method/)
...
1.  从仅包含节点“I”的子图开始。
2.  找到节点 `r`，使得 `c_ir` 最小，形成子游 `i-r-i`。
3.  （选择步骤）给定子路线，找到不在子路线中最靠近子路线中任何节点 `j` 的节点 `r`；即具有最小的 `c_rj`。
4.  （插入步骤）在子流程中找到使 `c_ir + c_rj - c_ij` 最小化的弧 `(i, j)`，在 `i` 和 `j` 之间插入 `r`。
