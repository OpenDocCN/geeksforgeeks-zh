...
> 原文：[https://www.geeksforgeeks.org/implementing-bit-matrix-in-java/](https://www.geeksforgeeks.org/implementing-bit-matrix-in-java/)

`BitMatrix` 是一个二维数组，其中每个元素不是 0 就是 1。我们将实现一个位矩阵，促进基本的位操作，如 `or`、`and`、`xor`。

**进场:**

1.  从 `java.util` 包导入 `BitSet` 类。
2.  使用 `BitSet` 初始化一个大小为 `rows x cols` 的位数组。默认情况下，集合中的所有位最初的值都为 `false`。
3.  现在，通过使用 `BitSet` 类提供的方法，我们可以操作数组。
4.  `BitSet` 类中的方法有 `set`、`clear`、`xor`、`or` 和 `and`。
5.  使用 `display()` 方法显示位矩阵。
