# Java.util.LinkedList.get()、getFirst()、getLast() 方法详解

> 原文链接：https://www.geeksforgeeks.org/java-util-linklist-get-first-get-last-java/

允许**获取特定索引**处的元素的常规方法是 `get()`。虽然在 `LinkedList` 中，如果没有完整的遍历，永远不可能实现这一点，但是这个方法允许同样的情况。这里有三种变体，所有这些都将在本文中讨论，当然也有例外。
