...
> 原文：[https://www.geeksforgeeks.org/java-program-to-print-smallest-and-biggest-possible-palindrome-word-in-a-given-string/](https://www.geeksforgeeks.org/java-program-to-print-smallest-and-biggest-possible-palindrome-word-in-a-given-string/)

## 简介
一个**回文串**是反串等于原串的串。在这个 Java 程序中，我们将看到在字符串中打印最小和最大回文单词的方法。
...
## 算法思路
*   `lengthPalindrome()` 函数通过提取字符串的每个单词并将其传递给 `checkPalin()` 函数来查找最长和最小的回文单词，然后通过将其与原始字符串的长度(如果是回文)进行比较来更新最小和最长字符串的答案字符串。
