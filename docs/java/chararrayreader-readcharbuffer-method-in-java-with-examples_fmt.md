...
Java 中 [`CharArrayReader`](https://www.geeksforgeeks.org/java-io-chararrayreader-class-java/) 类的 [`read(CharBuffer)`](https://www.geeksforgeeks.org/java-io-chararrayreader-class-java/) 方法用于将指定的字符读入 `CharBuffer` 实例。这个方法阻塞流直到:
...
这个方法接受一个强制参数 `charBuffer` ，它是要写入流中的 `charBuffer` 实例。

## 返回值

该方法返回一个整数值，即从流中读取的字符数。如果未读取任何字符，则返回-1。
...
下面的方法说明了 [`read(CharBuffer)`](https://www.geeksforgeeks.org/java-io-chararrayreader-class-java/)方法的工作原理:
...
[https://docs.oracle.com/javase/9/docs/api/java/io/Reader.html#read-java.nio.CharBuffer-](https://docs.oracle.com/javase/9/docs/api/java/io/Reader.html#read-java.nio.CharBuffer-)