...
> 原文：[https://www.geeksforgeeks.org/how-to-update-data-in-firebase-firestore-in-android/](https://www.geeksforgeeks.org/how-to-update-data-in-firebase-firestore-in-android/)

在前一篇文章中，我们已经看到了如何在安卓中向 Firebase Firestore 添加数据，以及如何从 Firebase Firestore 读取数据。现在我们将看到如何在我们的 Firebase Firestore 中更新这些添加的数据。现在我们将在 Android Firebase 中实现这种更新数据。
...
要创建新活动，请导航至 `应用程序 > res > 布局`，右键单击并单击 `新建 > 空活动` 创建新活动，我们将其命名为 `UpdateCourse`。创建新活动后，导航至 `应用程序 > res > 布局 > Activity_update_course.xml`，并添加以下代码。
...
在之前的文章中，我们已经看到创建了我们的 Modal 类。在本文中，我们将更新我们的 `Courses` 类，这样我们就可以将我们的对象类从适配器传递给我们的活动进行导航。随着 `Serializable` 的实现，我们必须创建一个新的字符串变量，并为该变量创建一个 `getter` 和 `setter`。我们将使用该变量来存储文档的 id。以下是更新后的 `Courses.java` 类的代码。代码中添加了注释，以便更详细地了解。
...
#### 步骤 3：为我们的回收视图添加 `onClickListener()`
