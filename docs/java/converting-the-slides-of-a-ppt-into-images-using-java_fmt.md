...
> 原文：[https://www.geeksforgeeks.org/converting-the-slides-of-a-ppt-into-images-using-java/](https://www.geeksforgeeks.org/converting-the-slides-of-a-ppt-into-images-using-java/)
...
6.  `java.io.File`：它包含了多种处理路径名、删除和重命名文件等方法。
7.  `java.io.FileInputStream`：用于从文件中读取数据。
8.  `java.io.FileOutputStream`：用于将数据写入文件。
9.  `java.io.IOException`：它是一个异常，在代码中用于在输入和输出操作中引发故障。

## 从 Apache POI 导入

1.  `org.apache.poi.xslf.usermodel.XMLSlideShow`：用于创建读取或写入幻灯片显示的对象。
2.  `org.apache.poi.xslf.usermodel.XSLFSlide`：用于创建和管理演示文稿中的幻灯片。
