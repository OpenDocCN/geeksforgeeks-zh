# 如何修复 java.net.ConnectException: 连接被拒绝: 用 Java 连接？

> 原文: [https://www.geeksforgeeks.org/how-to-fix-java-net-connectexception-connection-refused-connect-in-java/](https://www.geeksforgeeks.org/how-to-fix-java-net-connectexception-connection-refused-connect-in-java/)

`java.net.ConnectException: Connection refused` 是 Java 中最常见的一种网络异常，每当软件处于客户端-服务器体系结构中并试图建立从客户端到服务器的 TCP 连接时出现。为了解决通信问题，我们需要小心处理异常。首先，让我们看看 `java.net.ConnectException` 出现的可能原因：连接被拒绝。
...
5.  错误地，在给定的端口或随机端口号中提到了错误的端口。
6.  连接字符串信息错误。例如:

> `Connection conn = DriverManager.getConnection("JDBC:MySQL://localhost/:3306<dbname>?" + "user=<username>&password=<password>");`

**实现:** 这里我们使用的是 MySQL 数据库连接，连接信息应该是这种格式。现在让我们看看修复 `java.net.ConnectException` 的方法：连接被拒绝。
