# Java 中 `complementOf()` 方法的枚举补充

> 原文：[https://www.geeksforgeeks.org/enumset-complementof-method-in-java/](https://www.geeksforgeeks.org/enumset-complementof-method-in-java/)

`java.util.EnumSet.complementOf(Enum_Set)` 方法用于创建一个包含与指定 `EnumSet` 类型相同的元素的 `EnumSet`，这些元素的值存在于枚举中，但不包含在指定的 `EnumSet` 中。
...
该方法接受一个参数 `枚举集`，其值在将值填充到新的补充枚举集中时将被忽略。
...
如果值 `枚举集` 为空，则该方法抛出 `NullPointerException`。

以下程序说明了 `complementOf()` 方法的工作原理：
...
public class Enum_Set_Demo {
    public static void main(String[] args) {
        ...
        EnumSet<GFG> e_set = EnumSet.of(GFG.To, GFG.Welcome, GFG.Geeks);
