# Java 程序反转一个数字并检查它是否是回文

> 原文: [https://www.geeksforgeeks.org/java-program-to-reverse-a-number-check-if-it-is-a-palindrome/](https://www.geeksforgeeks.org/java-program-to-reverse-a-number-check-if-it-is-a-palindrome/)

如果给定数的反序与给定数本身相同，则该数可以称为回文。数字的长度是 log<sub>10</sub>(n)，也就是说，对于使用字符串操作（如创建反转和检查回文）的大整数，需要 log<sub>10</sub>(n) 的时间。
