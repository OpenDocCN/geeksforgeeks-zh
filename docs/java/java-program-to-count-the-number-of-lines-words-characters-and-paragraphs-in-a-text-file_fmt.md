...
> 原文：[https://www.geeksforgeeks.org/java-program-to-count-the-number-of-lines-words-characters-and-paragraphs-in-a-text-file/](https://www.geeksforgeeks.org/java-program-to-count-the-number-of-lines-words-characters-and-paragraphs-in-a-text-file/)
...
该功能出现在 `java.io.FileInputStream` 包下。它通过打开到文件系统中由文件对象文件命名的实际文件的连接来创建文件输入流。
...
*   `FileNotFoundException` – 如果文件不存在，是目录而不是常规文件，或者由于某种其他原因无法打开进行读取。
*   `SecurityException` – 如果安全管理器存在，并且其 `checkRead` 方法拒绝对文件的读取访问。
...
该功能存在于 `java.io.BufferedReader` 包下。它创建一个使用默认大小的输入缓冲区的缓冲字符输入流。
