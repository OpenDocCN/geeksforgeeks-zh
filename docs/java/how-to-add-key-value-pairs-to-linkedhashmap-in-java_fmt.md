...
`LinkedHashMap` 是一个哈希表和链表实现的 Map 接口。在 `LinkedHashMap` 中，键值对的顺序取决于键插入到映射中的顺序。插入顺序不影响键是否重新插入到映射中。
...
2.  [`keySet()`](https://www.geeksforgeeks.org/hashmap-keyset-method-in-java/) : Create a set of key elements included in the hash map.
3.  [`values()`](https://www.geeksforgeeks.org/hashmap-values-method-in-java/) : Create a set of hash table values.
