...
*   **`protected Writer(Object lock)`:** 创建一个新的字符流编写器，其关键部分将在给定的对象上同步。

## 方法

*   **`Writer append(char c)`:** 将指定的字符追加到此 `Writer`。对形式 `out.append(c)` 的这个方法的调用与调用 `out.write(c)` 的行为完全相同。
*   **`Writer append(CharSequence csq)`:** 向该编写器追加指定的字符序列。对形式 `out.append(csq)` 方法的调用与调用 `out.write(csq.toString())` 的行为完全相同，根据字符序列 `csq` 的 `toString` 规范，整个序列可能不会被追加。例如，调用字符缓冲区的 `toString` 方法将返回一个子序列，其内容取决于缓冲区的位置和限制。
...
*   **`Writer append(CharSequence csq, int start, int end)`:** 向该编写器追加指定字符序列的子序列。将指定字符序列的子序列附加到该书写器。
...
*   **`abstract void close()`:** 关闭流，先刷新。一旦流被关闭，进一步的 `write()` 或 `flush()` 调用将导致引发 `IOException`。关闭以前关闭的流没有效果。
...
*   **`abstract void flush()`:** 刷新流。如果流已将各种 `write()` 方法中的任何字符保存在缓冲区中，请立即将它们写入预期目标。然后，如果目的地是另一个字符或字节流，则刷新它。因此，一个 `flush()` 调用将刷新一系列写入器和输出流中的所有缓冲区。
...
*   **`void write(int c)`:** 写单个字符。要写入的字符包含在给定整数值的 16 个低位中；16 个高位被忽略。想要支持高效单字符输出的子类应该覆盖这个方法。
