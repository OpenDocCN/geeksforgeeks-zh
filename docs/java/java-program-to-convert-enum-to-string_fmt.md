...
> 原文：[https://www.geeksforgeeks.org/java-program-to-convert-enum-to-string/](https://www.geeksforgeeks.org/java-program-to-convert-enum-to-string/)
...
*   我们将遵循与前面使用的相同的过程，但是这里唯一的区别是我们将使用`toString()`方法。所以就把`name()`方法换成`toString()`方法。
