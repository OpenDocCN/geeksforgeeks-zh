...
> 原文：[https://www.geeksforgeeks.org/field-getdouble-method-in-java-with-examples/](https://www.geeksforgeeks.org/field-getdouble-method-in-java-with-examples/)

`getDouble()` 方法是 Java 反射机制中 `Field` 类的一部分，用于获取一个 `double` 值，该值必须是静态或实例字段的类型。此方法还用于通过加宽转换获取可转换为 `double` 类型的另一个基元类型的值。当一个类包含一个静态或实例 `double` 字段，并且我们想要得到该字段的值时，我们可以使用这个方法返回字段的值。
...
该方法接受单个参数 `obj`，该参数是提取 `double` 值的对象。

## 返回值

该方法返回转换为 `double` 类型的字段值。
