# Java中LinkedBlockingQueue的`put()`方法示例
...
如果队列未满，则`LinkedBlockingQueue`的`put(E e)`方法将作为参数传递的元素插入到该`LinkedBlockingQueue`尾部。如果队列已满，则此方法将等待空间变得可用，在空间可用后，它将元素插入到`LinkedBlockingQueue`中。
...
## 程序1：使用`put()`方法向队列添加新名称后，打印`LinkedBlockingQueue`的元素
...
## 程序2：在`LinkedBlockingQueue`中使用`put`方法添加`Employee`对象
...
## 程序3：显示`put()`方法抛出的`NullPointerException`
...
**参考：** [https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/LinkedBlockingQueue.html#put-E-](https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/LinkedBlockingQueue.html#put-E-)