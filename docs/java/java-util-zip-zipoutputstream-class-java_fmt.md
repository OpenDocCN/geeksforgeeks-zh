...
> 原文：[`https://www.geeksforgeeks.org/java-util-zip-zipoutputstream-class-java/`](https://www.geeksforgeeks.org/java-util-zip-zipoutputstream-class-java/)

这个类实现了一个输出流过滤器，用于以 ZIP 文件格式写入文件。包括对压缩和未压缩条目的支持。

## 建造师

*   **`ZipOutputStream(OutputStream out)`**：创建新的压缩输出流。
...
### `void finish()`
在不关闭底层流的情况下完成 ZIP 输出流的内容写入。当对同一输出流连续应用多个过滤器时，请使用此方法。
...
开始写入一个新的 ZIP 文件条目，并将流定位到条目数据的开头。如果当前条目仍处于活动状态，则将其关闭。如果没有为条目指定压缩方法，将使用默认压缩方法，如果条目没有设置修改时间，将使用当前时间。
...
为后续被放气的条目设置压缩级别。默认设置是 `DEFAULT_COMPRESSION`。
...
设置后续条目的默认压缩方法。只要没有为单个 ZIP 文件条目指定压缩方法，并且最初设置为放气，就会使用此默认值。
...
向当前 ZIP 条目数据写入一个字节数组。此方法将阻塞，直到所有字节都被写入。
