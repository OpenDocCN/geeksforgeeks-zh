...
`java.util.concurrent.ConcurrentSkipListSet`的`subSet()`方法是 Java 中的内置函数，其中元素在该方法定义的范围内返回。
...
第二个方法变体返回该集合的一部分的视图，该集合的元素范围是从`fromElement`到`toElement`，由布尔包含参数定义。
...
在本例中，因为布尔包含为`true`，所以也返回了元素 400。
