...
> 原文：[https://www.geeksforgeeks.org/how-to-extract-data-from-pdf-file-in-android/](https://www.geeksforgeeks.org/how-to-extract-data-from-pdf-file-in-android/)
...
导航到 **Gradle Scripts > `build.gradle`（模块：应用）**，并在依赖项部分添加以下依赖项。
...
由于我们从 PDF 文件中提取数据，所以我们将在我们的应用程序中添加 PDF 文件。要将 PDF 文件添加到您的应用程序中，我们必须首先创建 **raw** 文件夹。请参考安卓工作室中的[资源 raw 文件夹](https://www.geeksforgeeks.org/resource-raw-folder-in-android-studio/)，在安卓中创建一个原始文件夹。创建新的原始目录后，将您的 PDF 文件复制并粘贴到“原始”文件夹中。在您的应用程序中添加该 PDF 文件后，现在我们将在 XML 部分实现。
