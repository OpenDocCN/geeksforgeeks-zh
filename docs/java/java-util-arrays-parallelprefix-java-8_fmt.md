# Java 8 中的 `Arrays.parallelPrefix`

> 原文: [https://www.geeksforgeeks.org/java-util-arrays-parallelprefix-java-8/](https://www.geeksforgeeks.org/java-util-arrays-parallelprefix-java-8/)
...
Java 8 中的 [`Arrays`](https://www.geeksforgeeks.org/array-class-in-java/) 类引入了 `parallelPrefix` 方法。`parallelPrefix` 方法对数组的元素累计执行给定的数学函数，它们同时修改数组。
...
让我们用一个例子来理解 `parallelPrefix(int[] array, IntBinaryOperator op)`。

**例子 1:** 它说明了传递第二个参数（即 `IntBinaryOperator`）的一些方法。
...
**示例 3:** 这说明了 `parallelPrefix(T[] array, BinaryOperator op)` 和 `parallelPrefix(T[] array, int fromIndex, int toIndex, BinaryOperator op)`。
