...
> 原文: [https://www.geeksforgeeks.org/java-string-equalsignorecase-method-with-examples/](https://www.geeksforgeeks.org/java-string-equalsignorecase-method-with-examples/)

[`String`](https://www.geeksforgeeks.org/string-class-in-java/) 类的 `equalsIgnoreCase()` 方法比较两个字符串，而不管字符串的大小写。此方法返回一个布尔值，如果参数不为空并且表示忽略大小写的等效字符串，则为 `true`，否则为 `false`。
