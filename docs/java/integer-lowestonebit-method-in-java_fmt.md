...
> 原文：[https://www.geeksforgeeks.org/integer-lowestonebit-method-in-java/](https://www.geeksforgeeks.org/integer-lowestonebit-method-in-java/)

`java.lang.Integer.lowestOneBit()` 方法是一个内置函数，它返回一个最多只有一个一位的 `int` 值，该值位于指定 `int` 值中最低（即最右边）一位的位置。如果指定的值在其二进制补码二进制表示中没有一位，即如果数字的二进制表示等于零。
...
下面的程序说明了 `java.lang.Integer.lowestOneBit()` 方法：
