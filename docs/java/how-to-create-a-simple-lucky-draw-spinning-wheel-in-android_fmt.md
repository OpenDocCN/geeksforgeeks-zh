...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-simple-lucky-draw-spinning-wheel-in-android/](https://www.geeksforgeeks.org/how-to-create-a-simple-lucky-draw-spinning-wheel-in-android/)
...
我们已经为正在构建的应用程序添加了必要的资源文件。现在，让我们为我们的应用程序设计用户界面。将此 XML 文件添加到`app > res > 布局`中。下面是 `activity_main.xml` 文件的代码。
...
现在是时候初始化 `MainActivity` 中的所有内容了。我们在 Java 中使用 `Random` 类，生成一个随机数进行旋转，`CountDownTimer` 类每毫秒将图像旋转 2 度。下面是 `MainActivity` 的完整代码。以下是 `MainActivity.java` 文件的代码。代码中添加了注释，以更详细地理解代码。
