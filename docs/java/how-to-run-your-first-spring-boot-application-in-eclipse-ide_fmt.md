...
> 原文：[https://www.geeksforgeeks.org/how-to-run-your-first-spring-boot-application-in-eclipse-ide/](https://www.geeksforgeeks.org/how-to-run-your-first-spring-boot-application-in-eclipse-ide/)
...
2.  在您的 `pom.xml` 文件中添加 `spring-web` 依赖项。
...
**步骤 2：** 在您的 `pom.xml` 文件中添加 `spring-web` 依赖项。转到项目内部的 `pom.xml` 文件，并添加以下 `spring-web` 依赖项。
...
**步骤 3：** 在你的项目中创建一个包，并将这个包命名为“控制器”。在控制器中，包创建一个类，并将其命名为 `DemoController`。
...
我们在控制器层使用了下面的注释。在这个例子中，URI 路径是`/hello`。
...
> 试试这个运行在 Tomcat URL 上：`http://localhost:8989/hello`
