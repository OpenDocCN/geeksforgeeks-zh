# ArrayDeque contains() 方法在 Java 中

> 原文: [https://www.geeksforgeeks.org/arraydeque-contains-method-in-java/](https://www.geeksforgeeks.org/arraydeque-contains-method-in-java/)
...
```java
ArrayDeque.contains(Object element)
```

## 参数

参数 `element` 属于 `Object` 类型。这是需要测试的元素，无论它是否存在于 `deque` 中。

## 返回值

如果元素存在于对象中，该方法返回 `true`，否则返回 `false`。
