...
> 原文: [https://www.geeksforgeeks.org/hashtable-putifabsent-method-in-java-with-examples/](https://www.geeksforgeeks.org/hashtable-putifabsent-method-in-java-with-examples/)

`HashTable`类的`putIfAbsent(Key, value)`方法，如果给定的键不与值相关联或映射为空，则允许将值映射到给定的键。如果哈希表中已经存在这样的键值集，则返回空值。
...
该方法返回**映射到键的现有值，如果之前没有值映射到键，则返回空值**。

## 异常

此方法抛出:

*   `NullPointerException`: when the specified parameter is empty.
