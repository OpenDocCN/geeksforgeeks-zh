...
> 原文: [https://www.geeksforgeeks.org/java-tuples-fromiterable-method/](https://www.geeksforgeeks.org/java-tuples-fromiterable-method/)
...
该方法以 `Iterable` 为参数，其中:

*   `X` – 表示可迭代表中值的数据类型。
*   `Iterable` – 表示要插入到 `TupleClass` 中的值的可迭代。
*   `TupleClass` – 代表像 `Unit`、`Quintet`、`Decade` 等使用的 JavaTuple 类。

## 返回值

该方法返回 `TupleClass` 的对象，该对象调用方法，可迭代的值作为参数传递。

下面的程序说明了使用 `fromIterable()` 方法的各种方法:

### 程序 1: 使用 `fromIterable()` 和 `Unit` 类
...
### 程序 2: 使用 `fromIterable()` 配合 `Decade` 类
