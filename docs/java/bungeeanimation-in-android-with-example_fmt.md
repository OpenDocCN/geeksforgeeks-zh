...
> 原文：[https://www.geeksforgeeks.org/bungeeanimation-in-android-with-example/](https://www.geeksforgeeks.org/bungeeanimation-in-android-with-example/)
...
**第三步：** 现在创建一个新的空活动（转到 `app->new->activity->空活动`，并将其命名为 `SecondActivity`，同时生成布局文件。将以下代码添加到 `activity_second.xml` 文件中。在这个文件中添加一个 [`TextView`](https://www.geeksforgeeks.org/textview-in-kotlin/) 到布局中。
...
**步骤 4：** 现在将以下代码添加到 `SecondActivity.java` 文件中。在这个文件中添加一个 **蹦极动画** 到 `onBackPressed()` 功能。所以每当用户点击后退按钮时，就会调用 `slideLeft()` 功能。
...
**第五步：** 在 `activity_main.xml` 文件中添加以下代码。在该文件中，在布局中添加各种 [`Button`](https://www.geeksforgeeks.org/button-in-kotlin/)，当用户点击时，将打开 `SecondActivity` 并显示不同的动画。
...
**第 6 步：** 在 `MainActivity.java` 文件中增加以下代码。现在点击任意 `Button`，`Open()` 功能开始，相应的动画将被执行。
