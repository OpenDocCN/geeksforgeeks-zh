...
> 原文：[https://www.geeksforgeeks.org/internal-storage-in-android-with-example/](https://www.geeksforgeeks.org/internal-storage-in-android-with-example/)
...
*   `OpenFileOutput()`：用于创建和保存文件。此方法返回一个文件输出流实例。
    ...
    > **语法：** `OpenFileOutput(String fileName, int mode)`
    >
    > **参数：**
    >
    > **模式：**
    >
    > `Context.MODE_PRIVATE`：如果文件存在，那么它就在上面，否则会创建一个新文件。
    >
    > `Context.MODE_APPEND`：如果文件存在，那么数据将被附加到文件的末尾。
    >
    > **返回：**
    >
    > `FileOutputStream` 对象
...
*   `OpenFileInput()`：用于从文件中读取数据，这将返回一个 `FileInputStream` 实例。
    ...
    > **语法：** `openFileInput(String fileName)`
    >
    > **返回：**
    >
    > `FileInputStream` 对象
...
1.  一个 [EditText](https://www.geeksforgeeks.org/edittext-widget-in-android-using-java-with-examples/) 接受用户输入
2.  两个按钮，一个用于读取数据，另一个用于写入
3.  一个 TextView 显示文件内容
...
将要创建的文件是 `demoFile.txt`，可以在 **设备文件浏览器 > data > data > 应用包 > files** 中找到。
