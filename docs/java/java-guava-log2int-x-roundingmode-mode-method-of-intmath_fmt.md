# Java Guava `IntMath.log2(int x, RoundingMode)` 方法

> 原文: [https://www.geeksforgeeks.org/java-guava-log2int-x-roundingmode-mode-method-of-intmath/](https://www.geeksforgeeks.org/java-guava-log2int-x-roundingmode-mode-method-of-intmath/)

[Guava 的 `IntMath`](https://www.geeksforgeeks.org/intmath-class-guava-java/) 类的 `log2(int x, RoundingMode mode)` 方法接受两个参数，根据第二个参数指定的舍入方式计算第一个参数舍入后的基数-2 对数值。
...
该方法取 2 个参数，其中 `x` 为整数，`mode` 为指定取整模式。
...
*   `ArithmeticException`: 如果 `mode` 为 `RoundingMode.UNNECESSARY` 且 `x` 不是 2 的幂。
...
下面的例子说明了 Guava `IntMath.log2()` 方法的实现:
...
[https://google.github.io/guava/releases/20.0/api/docs/com/google/common/math/IntMath.html#log2-int-java.math.RoundingMode-](https://google.github.io/guava/releases/20.0/api/docs/com/google/common/math/IntMath.html#log2-int-java.math.RoundingMode-)