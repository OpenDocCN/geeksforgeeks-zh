...
> 原文: [https://www.geeksforgeeks.org/java-class-dependency-analyzer-in-java-8-with-examples/](https://www.geeksforgeeks.org/java-class-dependency-analyzer-in-java-8-with-examples/)

`jdeps` 是在 `JDK 8` 中引入的一个新的命令行工具，用于理解应用程序的静态依赖关系和库，即 `jdeps` 命令显示 Java 类文件的包级或类级依赖关系。`jdeps` 的输入可以是一个 `.class` 文件路径名，一个 [JAR 文件](https://www.geeksforgeeks.org/jar-files-java/) 或者可以是一个全限定类名来分析所有的类文件。每当我们向 `jdeps` 命令行工具提供任何输入时，它都会生成系统控制台的依赖项。`jdeps` 引入了影响输出的各种选项。根据选项，`jdeps` 命令行工具生成输出。
...
我们必须编译上面的 `Geeks.class` 文件，编译器会生成 `Geeks.class` 文件，我们必须使用这个文件作为 `jdeps` 的输入。在这里，我运行的是 Java 9，我们将看到 `jdeps` 的输出，按照 Java 9，这个 `Geeks.class` 文件有不同的选项。
...
### `-dotoutput directoryPath`

如果我们使用此选项，`jdeps` 将为每个分析的存档生成一个 `.dot` 文件到指定目录，列出依赖关系。它还会生成一个名为 `summary.dot` 的摘要文件，指定存档之间的依赖关系。
...
它将打印所有类级依赖关系。我们可以使用 `-v` 代替 `-verbose`。
...
打印包级依赖关系，不包括同一归档内的依赖关系。

### `-verbose:class`

打印类级依赖关系，不包括同一归档内的依赖关系。
