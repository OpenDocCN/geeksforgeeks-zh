...
`SplitPane` 类是 JavaFX 的一部分。`SplitPane` 类是一个包含两个或多个由分隔符分隔的边的控件。用户可以拖动两侧，以给一侧更多的空间，从而使另一侧收缩相同的量。`SplitPane` 类继承 `Control` 类。

## 类的构造者

*   `SplitPane()`: 创建新的分割窗格。
*   `SplitPane(Node... n)`: 创建具有指定节点的分割窗格。
...
| `setDividerPosition(int dividerIndex, double position)` | 设置分隔线在指定索引处的位置。 |
| `setDividerPositions(double... positions)` | 设置分隔线的位置。 |
| `setOrientation(Orientation o)` | 设置 `SplitPane` 的方向。 |
...
*   使用 `getItems().add()` 功能创建标签并将其添加到拆分窗格。
...
public class SplitPane_1 extends Application {
    ...
    public void start(Stage stage) {
        try {
            ...
            for (int i = 1; i < 5; i++) {
                split_pane.getItems().add(new Label("\tLabel no " + i + "\t"));
            }
...
*   在这个程序中，我们将创建一个分割面板名称 `split_pane`。
*   使用 `getItems().add()` 功能创建标签并将其添加到拆分窗格。
*   将 `split_pane` 添加到场景中，并将场景添加到舞台中。
*   使用 `setOrientation()` 功能设置 `split_pane` 的方向。
*   调用 `show()` 功能显示最终结果。
