# Java 8 | BigInteger `divideAndRemainder()` 方法带示例

> 原文：[https://www.geeksforgeeks.org/java-8-biginteger-divideandremainder-method-with-examples/](https://www.geeksforgeeks.org/java-8-biginteger-divideandremainder-method-with-examples/)

## 介绍
在 Java 8 中引入了 `java.math.BigInteger.divideAndRemainder(BigInteger val)`。在调用此方法的 `BigInteger` 和作为参数传递给另一个方法的 `BigInteger` 之间应用除法运算后，此方法返回两个 `BigInteger` 的数组。这里，数组的第一个大整数表示除法运算的结果 (`this / val`)，第二个大整数表示除法运算的余数 (`this % val`)。
...
## 参数
该方法取 `BigInteger` 类型的强制参数 `val`，作为除数进行除法运算。

## 返回值
这个方法返回一个两个 `BigInteger` 的数组，包含商 (`this / val`) 作为第一个元素，余数 (`this % val`) 作为第二个元素。

## 异常
如果参数中传入了 `null` 或 `0`，该方法将抛出 `ArithmeticException`。
...
## 参考
[https://docs.oracle.com/javase/8/docs/api/java/math/BigInteger.html#divideAndRemainder-java.math.BigInteger-](https://docs.oracle.com/javase/8/docs/api/java/math/BigInteger.html#divideAndRemainder-java.math.BigInteger-)