...
> 原文：[https://www.geeksforgeeks.org/multiautocompletetextview-in-android-with-example/](https://www.geeksforgeeks.org/multiautocompletetextview-in-android-with-example/)
...
标记器设置在方法 `setTokenizer()` 中。默认情况下，我们有 `CommaTokenizer`。在本例中，我们使用了两个多自动完成文本视图实例。一个（`MultiAutoCompleteTextView` 默认），带有默认命令生成器。

> `multiAutoCompleteTextViewDefault.setTokenizer(new MultiAutoCompleteTextView.CommaTokenizer());`

这里，当用户完成键入或选择一个子字符串时，会在该子字符串的末尾添加一个逗号。对于第二个实例（`MultiAutoCompleteTextView` 自定义），我们使用的是空间标记器。它不过是一个 `Java` 类，因为我们需要在 3 个方法中编写代码，即 `findTokenStart`、`findTokenEnd` 和 `terminateToken`。
...
*   对于`multipautocomplettextviewcactivity`的第二个实例，这里使用了空格标记器，默认情况下只有逗号标记器，如果我们使用其他标记器，需要用java编写代码，它应该实现3个方法，即`findTokenStart`、`findTokenEnd`和`terminateToken`。
*   现在创建另一个Java文件(`app > java >您的包名> New > Java Class`)并将其命名为`SpaceTokenizer`。以下是`SpaceTokenizer.java`文件的完整代码。代码中添加了注释，以更详细地理解代码。
...
在许多应用程序中，有必要拥有多自动完成文本视图，这将有助于提供有价值的信息，并使用户的生活更容易避免选择不相关的信息。收集需求时，用户必须键入java文件中所有必要的文本，以便在下拉列表中显示必要的建议文本。