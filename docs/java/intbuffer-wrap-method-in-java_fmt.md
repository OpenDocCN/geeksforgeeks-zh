...
> 原文: [https://www.geeksforgeeks.org/intbuffer-wrap-method-in-java/](https://www.geeksforgeeks.org/intbuffer-wrap-method-in-java/)
...
**参数:** 该方法以 `array` 作为参数，该数组是将备份该缓冲区的数组。

**返回值:** 该方法返回创建的新的 `IntBuffer`。
...
**返回值:** 这个方法返回新的 `IntBuffer`。

**抛出:** 如果偏移和长度参数的前提条件不成立，该方法抛出 `IndexOutOfBoundsException`。
