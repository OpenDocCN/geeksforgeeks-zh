...
> 原文: [https://www.geeksforgeeks.org/delayqueue-remainingcapacity-method-in-java-with-examples/](https://www.geeksforgeeks.org/delayqueue-remainingcapacity-method-in-java-with-examples/)

[`DelayQueue`](https://www.geeksforgeeks.org/delayqueue-class-in-java-with-example/)的`remainingCapacity()`方法总是返回`Integer.MAX_VALUE`，因为延迟队列不受容量限制。这意味着，不管延迟队列的大小如何，它都会返回相同的结果，即`Integer.MAX_VALUE`。
