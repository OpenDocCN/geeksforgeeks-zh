...
Java 中 [`CharArrayReader`](https://www.geeksforgeeks.org/java-io-chararrayreader-class-java/) 类的 [`ready()`](https://www.geeksforgeeks.org/java-io-chararrayreader-class-java/) 方法用来检查这个 `CharArrayReader` 是否准备好被读取。它返回一个布尔值，表示读取器是否准备好了。
...
## 返回值：
该方法返回一个布尔值，该值告诉 `CharArrayReader` 是否准备好被读取。如果它准备好了，它就会变成真的。否则返回假。

## 异常：
如果输入输出时出现错误，该方法抛出异常。

下面的方法说明了 [`ready()`](https://www.geeksforgeeks.org/java-io-chararrayreader-class-java/)方法的工作原理：
...
## 参考：
[https://docs.oracle.com/javase/9/docs/api/java/io/CharArrayReader.html#ready--](https://docs.oracle.com/javase/9/docs/api/java/io/CharArrayReader.html#ready--)