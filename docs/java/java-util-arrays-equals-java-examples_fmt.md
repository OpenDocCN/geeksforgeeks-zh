...
> 原文: [https://www.geeksforgeeks.org/java-util-arrays-equals-java-examples/](https://www.geeksforgeeks.org/java-util-arrays-equals-java-examples/)
...
`Arrays.equals()` 方法有针对不同基本数据类型的重载版本：
...
我们也可以使用 `Arrays.equals()` 来检查用户定义类的对象数组的相等性。看看 `Arrays.equals()` 方法的最后一个变体。

**注意：** 在对象数组的情况下，必须[覆盖 `equals` 方法](https://www.geeksforgeeks.org/overriding-equals-method-in-java/)来提供您自己的相等定义，否则您将获得输出取决于 `Object` 类的 `equals()` 方法的返回。在下面的程序中，我们正在检查学生的 `rollno`、`name` 和 `address` 是否相等。
