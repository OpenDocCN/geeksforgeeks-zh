...
> 原文: [https://www.geeksforgeeks.org/intbuffer-hasarray-method-in-java/](https://www.geeksforgeeks.org/intbuffer-hasarray-method-in-java/)
...
当且仅当该缓冲区由数组支持且不是只读时，该方法将返回 ``true``。否则返回 ``false``。
