...
> 原文：[https://www.geeksforgeeks.org/collections-min-method-in-java-with-examples/](https://www.geeksforgeeks.org/collections-min-method-in-java-with-examples/)
...
**异常：** 如果集合为空，此方法抛出 `NoSuchElementException`。
...
`java.util.Collections` 类的 `min(Collection, Comparator)` 方法用于根据指定的比较器引发的顺序返回给定集合的最小元素。集合中的所有元素必须可以通过指定的比较器相互比较。
...
*   `coll` - 最小元素待定的集合。
*   `comp` - 用来确定最小元素的比较器。`null` 值表示应该使用元素的自然排序。
