...
在 Java 8 中引入了流应用编程接口，用于处理对象集合。一个流是一个支持各种方法的对象序列，这些方法可以被流水线化以产生期望的结果。在继续之前，让我们讨论一下集合和流之间的区别，以便理解为什么引入这个概念。
...
在上面的代码中，“c”指的是集合。所以在集合上，我们调用 `stream()` 方法，同时，我们将其存储为 stream 对象。从今以后，我们用这种方式获得流对象。

> **注意:** 流存在于名为 `java.util.stream` 的 java 实用程序包中。
...
这三个都将在下面讨论，因为它们几乎在大多数场景中都是并行的，并且通过在下面的干净 java 程序中实现它们来提供更好的理解。正如我们在上面的例子中已经研究过的，我们试图过滤处理过的对象可以解释为在流上操作的 `filter()` 操作。稍后，从处理对象的过滤元素开始，我们使用 Collectors 将元素收集回列表，我们已经在 `Collectors.toList()` 方法的帮助下为其导入了一个名为 `java.util.stream` 的特定包。这在 streams 中被称为 `collect()` 操作，所以这里我们不再单独讨论它们。
...
现在，极客们，你们很清楚“为什么”流被引入，但是你们应该想知道“在哪里”使用它。答案很简单，因为我们在日常生活中确实经常使用它们。因此，用更简单的话来说，极客可以直接把流放在集合概念适用的地方，流概念也可以应用在那里。
