...
[`hashCode()` 方法](https://www.geeksforgeeks.org/equals-hashcode-methods-java/)用于获取对象的哈希代码。`object` 类的 `hashCode()` 方法以整数形式返回对象的内存引用。`hashCode()` 方法的定义是 `public native hashCode()`。它表明 `hashCode()` 的实现是本机的，因为在 `java` 中没有任何直接的方法来获取对象的引用。可以提供自己的 `hashCode()` 实现。
在 `HashMap` 中，`hashCode()` 用于计算桶，从而计算索引。

### equals()法

[`equals()` 方法](https://www.geeksforgeeks.org/equals-hashcode-methods-java/)用于检查两个对象是否相等。此方法由 `object` 类提供。您可以在类中重写它，以提供自己的实现。
...
4.  如果索引 6 中没有其他对象，则将该对象放在索引 6 中。
...
4.  如果索引 6 中没有其他对象，则将该对象放在那里。
5.  在这种情况下，节点对象是在索引 6 处找到的**–这是一个碰撞的情况。**
6.  在这种情况下，通过 `hashCode()` 和 `equals()` 方法检查这两个键是否相同。
7.  如果键相同，用当前值替换该值。
8.  否则，通过链表将这个节点对象连接到前一个节点对象，两者都存储在索引 6 中。
