...
> 原文：[https://www.geeksforgeeks.org/can-two-variables-refer-to-the-same-arraylist-in-java/](https://www.geeksforgeeks.org/can-two-variables-refer-to-the-same-arraylist-in-java/)
...
上图显示了对象 `list` 如何指向内存中的数组列表。现在让我们看看如何让另一个对象指向上面实例化的同一个数组列表。
...
现在，当把 `list` 分配给 `secondList` 时，`secondList` 也开始指向内存中的同一个数组列表。请注意，这里只需创建一个新实例。现在，为了更好地理解，让我们深入代码并看几个例子。
