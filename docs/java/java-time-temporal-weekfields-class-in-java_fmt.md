...
`WeekFields` 类表示周的定义，以提供临时字段实例。`WeekFields` 提供了五个字段，`weekOfYear()`、`weekOfMonth()`、`dayOfWeek()`、`weekOfWeekBasedYear()` 和 `weekBasedYear()`，它们提供了对任何时态对象的值的访问。
...
| 方法 | 描述 |
| --- | --- |
| `dayOfWeek()` | 此方法返回一个字段，以基于此周字段访问一周中的某一天。 |
| `equals(Object object)` | 此方法检查该周字段是否等于指定的对象。 |
| `getFirstDayOfWeek()` | 此方法获取一周的第一天。 |
| `getMinimalDaysInFirstWeek()` | 此方法在第一周获取最少天数。 |
| `hashCode()` | 此方法返回此 `WeekFields` 的哈希代码。 |
| `of(DayOfWeek firstDayOfWeek, int minimalDaysInFirstWeek)` | 此方法从一周的第一天和最小天数获取 `WeekFields` 的实例。 |
| `of(Locale locale)` | 此方法获取适用于区域设置的 `WeekFields` 实例。 |
| `toString()` | 此方法获取此 `WeekFields` 实例的字符串表示形式。 |
| `weekBasedYear()` | 此方法返回一个字段，以基于该周字段访问基于周的年份。 |
| `weekOfMonth()` | 此方法返回一个字段，以基于此周字段访问一个月中的周。 |
| `weekOfWeekBasedYear()` | 此方法返回一个字段，以访问基于该周字段的一年中的周。 |
| `weekOfYear()` | 此方法返回一个字段，以基于此周字段访问一年中的周。 |
