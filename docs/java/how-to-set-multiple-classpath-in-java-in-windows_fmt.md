...
> 原文: [https://www.geeksforgeeks.org/how-to-set-multiple-classpath-in-java-in-windows/](https://www.geeksforgeeks.org/how-to-set-multiple-classpath-in-java-in-windows/)
...
4.  如果系统变量中已经存在类路径，单击编辑按钮，然后放一个分号(；)结尾。粘贴 `MySQL-Connector Java.jar` 文件的路径。否则点击新建按钮。
    ![](img/ff7dcc9b11b04c7781d3eede60096cb7.png)
5.  将变量名键入为 `CLASSPATH`，变量值键入为 `C:\ Program Files \ Java \ JRE 1.8 \ MySQL-Connector Java . jar；。；`。
    注:放 `；。；` 在类路径的末尾。
...
1.  安装最新的 JDK。
2.  打开命令提示符，输入命令: `javac -version`，显示如下错误。
