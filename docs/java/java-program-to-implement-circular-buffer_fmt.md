...
> 原文：[https://www.geeksforgeeks.org/java-program-to-implement-circular-buffer/](https://www.geeksforgeeks.org/java-program-to-implement-circular-buffer/)
...
由于添加的元素类型未知，空的 `Object` 数组及其容量将在构造函数中初始化。保持两个指针，即 `head` 和 `tail`，用于元素的插入和删除。`head` 指向第一个元素，`tail` 指向最后一个元素。
...
插入元素后，`tail` 指针和 `size` 增加 1。当数组的 `size` 等于其 `capacity` 时，缓冲区已满，无法容纳更多的元素。

**删除元素：**

`head` 指针处的元素被检索，`head` 指针增加 1，如果缓冲区的 `size` 减少 1。
...
创建了一个通用 `Node` 类，作为创建循环缓冲区的助手类。

保持两个指针，即 `head` 和 `tail`，用于元素的插入和删除。`head` 指向第一个元素，`tail` 指向最后一个元素。
...
*   最初 `head` 和 `tail` 为空，`size` 为 0。
*   元素被添加到链表的尾部，`tail` 的引用被更改为 `head` 指针。
*   当元素被添加到链表中时，缓冲区的 `size` 增加。
*   当数组的 `size` 等于其 `capacity` 时，缓冲区已满，无法容纳更多的元素。

**删除元素：**

`head` 指针处的元素被检索，并且 `head` 指针的引用改变到下一个元素，并且缓冲区的 `size` 如果减 1。
