...
> 原文：[https://www.geeksforgeeks.org/how-to-run-spring-boot-application/](https://www.geeksforgeeks.org/how-to-run-spring-boot-application/)
...
一般来说，要开发一个 Spring Boot 应用程序，我们选择 `Eclipse`、`Spring Tool Suite` 和 `IntelliJ IDEA` IDE。因此，在本文中，我们将在这 3 个 IDEs 中运行我们的应用程序。
...
一个人应该知道如何在 Eclipse IDE 中[创建和设置 Spring Boot 项目](https://www.geeksforgeeks.org/how-to-create-and-setup-spring-boot-project-in-eclipse-ide/)，并在 Eclipse IDE 中创建你的第一个 Spring Boot 应用程序。
...
**步骤 3：** 在您的项目中创建一个包，并将该包命名为“控制器”。在控制器包中创建一个类，并将其命名为 `DemoController`。以下是 `DemoController.java` 文件的代码。
...
我们已经在我们的控制器层中使用了下面的注释。在这个例子中，URI 路径是 `/hello`。
...
试试这个运行在 Tomcat URL：`http://localhost:8989/hello`
...
**步骤 2：** 成功创建或导入 spring boot 项目后，将自动创建一个名为 `Application.java`（这里是 `DemoApplication`）的文件，这是您的入口点。您可以将其视为 Spring Boot 应用程序的 `main` 方法。

![](img/004a4c32ae825e4e740440a58600480f.png)

**步骤 3：** 运行 Spring Boot 应用程序

**方法 1：** 要立即运行此应用程序，右键单击 `Application.java` > 运行 “`DemoApplication.main()`” 如下图所示。或者您可以键入快捷键组合（`Ctrl + Shift + F10`）来运行应用程序。

![](img/f240b9fe699ee571325f03cd709f2fd9.png)

**方法二：** 直接点击下图所示的绿色三角形按钮，然后选择运行 “`DemoApplication.main()`”。
...
2.  将项目导入到 STS IDE 中
3.  将为 STS 创建一个名为 `Application` 文件的条目文件
4.  在服务器上运行应用程序。

**步骤 1：** 在 Spring 工具套件中创建您的 Spring Boot 项目

您可以参考本文**如何在 Spring Tool Suite 中创建和设置 Spring Boot 项目**，并创建您的第一个 Spring Boot 应用程序。或者你可以在[春季初始化器](https://www.geeksforgeeks.org/spring-initializr/)中创建你的 Spring Boot 项目，并将该项目导入到你的 STS IDE 中。
...
**2.2：** 将出现如下弹出窗口。在这里，您必须选择在 Spring Initializer 中创建 spring boot 项目时生成的目录。然后点击完成按钮。

![](img/4541356a22e3ebb42aa5aa623d15f07e.png)

**步骤 3：** 成功创建或导入 spring boot 项目后，将自动创建一个文件名 `Application.java`（这里是 `DemoApplication`），这是您的入口点。您可以将其视为 Spring Boot 应用程序的主要方法。
...
现在，极客们，你们一定想知道，如果我们不希望它在默认端口号上运行，那会怎么样，因为 Tomcat 服务器的默认端口是 `8080`。我们可以使用下面一行代码来更改 `application.properties` 文件中的端口号，如下所示：
