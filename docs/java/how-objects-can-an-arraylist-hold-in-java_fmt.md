...
> 原文：[https://www.geeksforgeeks.org/how-objects-can-an-ArrayList-hold-in-Java/](https://www.geeksforgeeks.org/how-objects-can-an-ArrayList-hold-in-Java/)
...
> 这里 `<String>` 代表要存储在数组列表中的元素的数据类型。请记住，这不能是任何原始数据类型，如 `int` 或 `float`。
...
Java 中 `List` 接口的 [`List.get()` 方法](https://www.geeksforgeeks.org/list-get-method-in-java-with-examples/) 用于获取该列表中给定特定索引处存在的元素。
