# Java 中的 Double.compare() 方法示例

> 原文：[https://www.geeksforgeeks.org/double-compare-method-in-java-with-examples/](https://www.geeksforgeeks.org/double-compare-method-in-java-with-examples/)

`Double` 类的 `compare()` 方法是 Java 中的一个内置方法，用于比较两个指定的双精度值。返回的整数值的符号与函数调用返回的整数的符号相同。
