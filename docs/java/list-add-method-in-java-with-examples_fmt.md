...
> 原文：[https://www.geeksforgeeks.org/list-add-method-in-java-with-examples/](https://www.geeksforgeeks.org/list-add-method-in-java-with-examples/)
...
该函数只有一个参数，即 `e`——要附加到该列表的元素。

## 返回

如果指定的元素被追加，列表发生变化，则返回真。
...
下面是使用 `Linkedlist` 展示 `list.add()` 实现的代码。
