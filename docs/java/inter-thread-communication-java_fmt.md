...
> 原文：[`https://www.geeksforgeeks.org/inter-thread-communication-java/`](https://www.geeksforgeeks.org/inter-thread-communication-java/)
...
为了避免轮询，Java 使用了三种方法，即 `wait()`，`notify()`，和 `notifyAll()`。所有这些方法作为 `final` 都属于 `Object` 类，这样所有的类都有它们。它们只能在同步块中使用。
