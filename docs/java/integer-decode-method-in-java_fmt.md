# Java 中的 `Integer.decode()` 方法

> 原文：[https://www.geeksforgeeks.org/integer-decode-method-in-java/](https://www.geeksforgeeks.org/integer-decode-method-in-java/)
...
该方法取 `String` 数据类型的一个参数 `str`，引用需要解码的字符串。

## 返回值

该方法返回一个整数对象，该对象保存由字符串 `str` 表示的整数值。

## 异常

当字符串包含无法解析的整数时，该方法抛出 `NumberFormatException`。
...
当字符串值被传递时，抛出 `NumberFormatException`。
