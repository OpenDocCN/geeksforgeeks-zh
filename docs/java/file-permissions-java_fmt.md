...
```
如果用户没有权限更改此抽象路径名的访问权限，操作将失败。如果 `executable` 为 `false`，并且基础文件系统没有实现执行权限，则操作将失败。

### `setReadable`
一个用于设置此抽象路径名所有者读取权限的便捷方法。
```java
