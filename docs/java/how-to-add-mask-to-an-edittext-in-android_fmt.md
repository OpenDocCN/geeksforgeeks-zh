...
> 原文：[https://www.geeksforgeeks.org/how-to-add-mask-to-an-edittext-in-android/](https://www.geeksforgeeks.org/how-to-add-mask-to-an-edittext-in-android/)

`EditText` 是一个安卓小部件。这是一个用于输入和修改数据的用户界面元素。它以字符串格式返回数据。
...
1.  将支持库添加到您的根 `build.gradle` 文件中（不是您的模块 `build.gradle` 文件）。这个库 `jitpack` 是一个新颖的包库。它是为 JVM 而做的，因此 [github](https://www.geeksforgeeks.org/ultimate-guide-git-github/) 和 [bigbucket](https://www.geeksforgeeks.org/bitbucket-vs-github-vs-gitlab/) 中存在的任何库都可以直接在应用程序中使用。
...
3.  现在在 `activity_main.xml` 文件中添加以下代码。它将在 `activity_main.xml` 中创建三个掩码编辑文本和一个按钮。
