...
> 原文：[https://www.geeksforgeeks.org/how-to-change-font-of-toolbar-title-in-an-android-app/](https://www.geeksforgeeks.org/how-to-change-font-of-toolbar-title-in-an-android-app/)
...
在方法 1 中，只需转到 `activity_main.xml` 文件，并在工具栏小部件中添加一个具有 `fontFamily` 属性的 [`TextView`](https://www.geeksforgeeks.org/textview-in-kotlin/)。`activity_main.xml` 文件的完整代码如下。
...
首先，在项目的 `src/main/assets/font/` 中添加一个字体文件。然后为工具栏和文本标题创建变量，并调用方法 `findViewById()`。从指定的字体数据创建一个新的字体。最后在文本标题中设置字体。以下是 `MainActivity.java` / `MainActivity.kt` 文件的完整代码。
