...
> 原文: [https://www.geeksforgeeks.org/navigableset-isempty-method-in-java/](https://www.geeksforgeeks.org/navigableset-isempty-method-in-java/)

`java.util.NavigableSet.isEmpty()` 方法用于检查 `NavigableSet` 是否为空。如果导航集为空，则返回真，否则返回假。
...
如果导航集为空，则该方法返回真，否则返回假。

## 示例程序

下面的程序说明了 `java.util.NavigableSet.isEmpty()` 方法：
