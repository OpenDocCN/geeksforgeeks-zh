...
> 原文: [https://www.geeksforgeeks.org/longstream-summarystatistics-java/](https://www.geeksforgeeks.org/longstream-summarystatistics-java/)
...
`LongSummaryStatistics summaryStatistics()` 返回一个 `LongSummaryStatistics`，描述关于这个流的元素的各种汇总数据。

## 注

`LongStream summaryStatistics()` 是 `约简` 的特例。约简操作，也称为 `折叠`，获取一系列输入元素，并通过重复应用组合操作将它们组合成单个汇总结果。组合运算可以是求一组数的和或最大值。

## 示例 1

使用 `LongStream summaryStatistics()` 获取给定 `LongStream` 中存在的元素的 `LongSummaryStatistics`。
...
使用 `LongStream summaryStatistics()` 获取给定范围内存在的元素的 `LongSummaryStatistics`。
