# 将 `HashMap` 转换为 Java 中的 `LinkedList`

> 原文：[https://www.geeksforgeeks.org/convert-hashmap-to-linkedlist-in-java/](https://www.geeksforgeeks.org/convert-hashmap-to-linkedlist-in-java/)
...
[`LinkedList`](https://www.geeksforgeeks.org/linked-list-in-java/) 是 [`java.util`](https://www.geeksforgeeks.org/java-util-package-java/) 包中的集合框架的一部分。这个类是 `LinkedList` 数据结构的实现，它是一个线性数据结构，其中元素不存储在连续的位置，每个元素都是一个独立的对象，有数据部分和地址部分。这些元素使用指针和地址进行链接。每个元素都被称为一个节点。
...
class GFG {
    ...
    { 
        ...
        // list of values
        List<String> listOfValue = new LinkedList<>(l.values());
