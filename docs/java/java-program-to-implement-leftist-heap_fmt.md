...
> 原文: [https://www.geeksforgeeks.org/java-program-to-implement-leftist-heap/](https://www.geeksforgeeks.org/java-program-to-implement-leftist-heap/)
...
**示例:** 下面的左树显示了通过上述过程为每个节点计算的距离。最右边的节点的等级为 0，因为该节点的右子树为空，其父节点的距离为 1，即 `dist( i ) = 1 + dist( right( i))`。每个节点都遵循相同的方法，并计算它们的 s 值（或等级）。
...
2.  如果最右边的叶子的路径有 `x` 个节点，那么左边的堆至少有 `2^x – 1` 个节点。这意味着对于具有 `n` 个节点的左边堆，到最右边叶的路径长度是 `O(log n)`。
...
*   我们将首先获取一个 `Node` 类，并创建它的构造函数和各种参数。
