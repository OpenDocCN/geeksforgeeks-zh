...
> 原文:[https://www.geeksforgeeks.org/android-how-to-open-camera-through-intent-and-display-captured-image/](https://www.geeksforgeeks.org/android-how-to-open-camera-through-intent-and-display-captured-image/)
...
这篇文章的目的是展示如何从一个应用程序中打开相机，点击图像，然后在同一个应用程序中显示这个图像。本文开发了一个 android 应用程序来实现这一点。在`MediaStore`类的`ACTION_IMAGE_CAPTURE`意图的帮助下，从我们的应用程序内部打开摄像头。

### 例如:
![](img/19bd4f69fed6ce00d67b5da86701c7e5.png)

此图像显示了摄像机点击并在图像视图中设置的图像。当应用程序打开时，它会显示“相机”按钮来打开相机。当按下时，`ACTION_IMAGE_CAPTURE`意图由`MediaStore`类启动。捕获图像后，它会显示在`ImageView`中。
...
**第三步:** 现在，在 UI 之后，这一步将创建 App 的后端。为此，打开`MainActivity.java`文件，并使用`findViewById()`方法实例化在 XML 文件(`camera_button`，`ImageView`)中制作的组件。此方法借助于分配的标识将创建的对象绑定到用户界面组件。
