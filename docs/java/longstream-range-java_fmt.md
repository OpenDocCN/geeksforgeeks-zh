# Java 中的 LongStream range()

> 原文: [https://www.geeksforgeeks.org/longstream-range-java/](https://www.geeksforgeeks.org/longstream-range-java/)

`LongStream range(long startInclusive, long endExclusive)` 返回从 `startInclusive`（包含）到 `endExclusive`（排除）的顺序长流，增量为 1。
...
`LongStream range(long startInclusive, long endExclusive)` 基本上像 `for` 循环一样工作。递增值的等价序列可以按如下顺序产生:
