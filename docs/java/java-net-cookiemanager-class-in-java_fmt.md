...
> 原文：[https://www.geeksforgeeks.org/java-net-cookiemanager-class-in-java/](https://www.geeksforgeeks.org/java-net-cookiemanager-class-in-java/)

[`CookieManager`](https://www.geeksforgeeks.org/java-net-httpcookie-java/) 类提供了 [`CookieHandler`](https://www.geeksforgeeks.org/java-net-httpcookie-java/) 的精确实现。这将 cookie 的存储与接受和拒绝 cookie 的策略分开。一个 `CookieManager` 用一个 `CookieStore` 和一个 `CookiePolicy` 初始化。`CookieStore` 管理存储，而 `CookiePolicy` 对象对 cookie 的接受/拒绝做出策略决定。
...
| `setCookiePolicy(CookiePolicy cookiePolicy)` | 此方法设置此 cookie 管理器的 cookie 策略。 |
| `get(URI uri, Map<String, List<String>> requestHeaders)` | 此方法将从 cookie 缓存中获取请求头中指定 URI 的所有适用 cookie。 |
| `put(URI uri, Map<String, List<String>> responseHeaders)` | 此方法设置所有适用的 cookie。 |
...
### 参数
作为参数传递的 `URI` 指定了 cookies 的预期用途。特别是，该方案应该反映 cookies 是通过 HTTP、HTTPS 发送，还是在另一个上下文中使用，如 JavaScript。
