...
> 原文: [https://www.geeksforgeeks.org/java-program-to-get-todays-date/](https://www.geeksforgeeks.org/java-program-to-get-todays-date/)
...
## 方法 1: 使用 `LocalDate` 类的 `now()` 方法

`now()` 方法是 `LocalDate` 类用来从默认时区的系统时钟中获取当前日期。此方法将根据系统时钟和默认时区返回本地日期，以获取当前日期。
