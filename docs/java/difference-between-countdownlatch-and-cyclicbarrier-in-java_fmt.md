...
> 原文: [https://www.geeksforgeeks.org/difference-between-countdownlatch-and-cyclicbarrier-in-java/](https://www.geeksforgeeks.org/difference-between-countdownlatch-and-cyclicbarrier-in-java/)
...
`CountDownLatch` 是等待多个线程完成或调用 `countDown()` 的线程。当所有线程都调用了 `countDown()` 时，等待线程继续执行。
