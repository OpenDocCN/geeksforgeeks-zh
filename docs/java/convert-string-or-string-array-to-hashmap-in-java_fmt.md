...
> 原文：[https://www.geeksforgeeks.org/convert-string-or-string-array-to-hashmap-in-java/](https://www.geeksforgeeks.org/convert-string-or-string-array-to-hashmap-in-java/)
...
*   首先，我们用逗号分割字符串，并将其存储在数组 `parts` 中。拆分后我们有以下内容：

```java
"Aashish:1","Bina:2", "Chintu:3"
```

*   然后在 `parts` 上迭代，分割学生数据，得到学生姓名和学号。并将 `roll no` 设置为键，将名称设置为 `HashMap` 的值。
