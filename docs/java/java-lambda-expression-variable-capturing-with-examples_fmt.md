...
由 lambda 表达式的封闭范围定义的变量可以在 lambda 表达式中访问。例如，lambda 表达式可以使用由其封闭类定义的实例或静态变量。lambda 表达式还可以访问(显式和隐式)`this`，这是指 lambda 表达式的封闭类的调用实例。因此，lambda 表达式可以获取或设置内部变量或静态变量的值，并调用由其封闭类定义的方法。
...
如注释所示，`number`实际上是最终的，因此可以在`myLambda`中使用。但是，如果要修改`number`，无论是在 lambda 内部还是外部，`number`都将失去其有效的最终状态。这将导致错误，程序将无法编译。
