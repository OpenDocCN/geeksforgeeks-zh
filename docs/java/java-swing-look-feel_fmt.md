...
Java 的 JRE 提供了以下 L&F：

1.  **跨平台观感**：这是在所有平台上看起来都一样的“Java L&F”，也叫“Metal”。它是 Java API (`javax.swing.plaf.metal`) 的一部分，是默认的。
2.  **系统观感**：在这里，应用程序使用的 L&F 是它所运行的系统的默认值。系统 L&F 在运行时确定，应用程序要求系统返回适当的 L&F 的名称。
...
还可以使用观感的实际类名作为 `UIManager.setLookAndFeel()` 的参数。例如：
...
2.  **Motif 观感**：只需将第 14 行和第 79 行更改为：
...
4.  **Windows 经典观感**：将第 14 行和第 79 行更改为：
