...
> 原文：[https://www.geeksforgeeks.org/java-program-to-convert-outputstream-to-string/](https://www.geeksforgeeks.org/java-program-to-convert-outputstream-to-string/)

`OutputStream` 是一个抽象类，可在 `java.io` 包中获得。因为它是一个抽象类，为了使用它的功能，我们可以使用它的子类。一些子类是 `FileOutputStream`、`ByteArrayOutputStream`、`ObjectOutputStream` 等。字符串只不过是一个字符序列，用双引号来表示。方法使用字符集转换流。

**方法 1：**

1.  创建 `ByteArrayOutputStream` 的对象。
2.  创建一个字符串变量并初始化它。
3.  使用 `write()` 方法，将字符串的内容复制到 `ByteArrayOutputStream` 的对象中。
4.  打印出来。
...
2.  创建 `ByteArrayOutputStream` 的对象。
3.  使用 `write()` 方法将内容从字节数组复制到对象中。
4.  打印出来。
