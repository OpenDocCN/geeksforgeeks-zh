...
> 原文：[https://www.geeksforgeeks.org/java-util-hashmap-in-java-with-examples/](https://www.geeksforgeeks.org/java-util-hashmap-in-java-with-examples/)
...
`HashMap` 实现 `Serializable`、`Cloneable`、[`Map<K,V>`](https://www.geeksforgeeks.org/map-interface-java-examples/) 接口。`HashMap` 扩展 `AbstractMap<K,V>` 类。直接子类是 [`LinkedHashMap`](https://www.geeksforgeeks.org/linkedhashmap-class-java-examples/)、`PrinterStateReasons`。
...
**构造函数 2：** `HashMap(int initialCapacity)`
...
**构造函数 3：** `HashMap(int initialCapacity, float loadFactor)`
...
**构造函数 4：** `HashMap(Map map)`

它创建一个 `HashMap` 实例，其映射与指定的映射相同。
...
Java 中的 `HashMap` 是基于哈希表实现的 `Map` 接口。它存储键值对，并允许快速检索。下面通过示例演示其基本用法。
...
**3. 移除元素:** 为了从地图中移除元素，我们可以使用 [`remove()`](https://www.geeksforgeeks.org/hashmap-remove-method-in-java/) 方法。此方法获取键值，并从该映射中移除键的映射(如果它存在于该映射中)。
...
*   还实现了 [`Cloneable`](https://docs.oracle.com/javase/7/docs/api/java/lang/Cloneable.html) 和 [`Serializable`](https://docs.oracle.com/javase/7/docs/api/java/io/Serializable.html) 接口。上述定义中的 `k` 和 `V` 分别代表键和值。
...
| 方法 | 描述 |
| --- | --- |
| [`clear()`](https://www.geeksforgeeks.org/hashmap-clear-method-in-java/#:~:text=HashMap.,mappings%20from%20a%20specified%20HashMap.&text=Parameters%3A%20The%20method%20does%20not,does%20not%20return%20any%20value.) | 从此映射中移除所有映射。 |
| [`clone()`](https://www.geeksforgeeks.org/hashmap-clone-method-in-java/) | 返回这个 `HashMap` 实例的浅拷贝:键和值本身没有被克隆。 |
| [`compute(K key, BiFunction<? super K, ? super V, ? extends V> remappingFunction)`](https://www.geeksforgeeks.org/hashmap-compute-method-in-java-with-examples/) | 尝试计算指定键及其当前映射值的映射(如果没有当前映射，则为空)。 |
| [`computeIfAbsent(K key, Function<? super K, ? extends V> mappingFunction)`](https://www.geeksforgeeks.org/hashmap-computeifabsent-method-in-java-with-examples/) | 如果指定的键尚未与值相关联(或映射为 `null`)，会尝试使用给定的映射函数计算其值，并将其输入到此映射中，除非为 `null`。 |
| [`computeIfPresent(K key, BiFunction<? super K, ? super V, ? extends V> remappingFunction)`](https://www.geeksforgeeks.org/hashmap-computeifpresentkey-bifunction-method-in-java-with-examples/) | 如果指定键的值存在且不为空，将尝试计算给定键及其当前映射值的新映射。 |
| [`containsKey(Object key)`](https://www.geeksforgeeks.org/hashmap-containskey-method-in-java/) | 如果此映射包含指定键的映射，则返回 `true`。 |
| [`containsValue(Object value)`](https://www.geeksforgeeks.org/hashmap-containsvalue-method-in-java/) | 如果此映射将一个或多个键映射到指定值，则返回 `true`。 |
| [`entrySet()`](https://www.geeksforgeeks.org/hashmap-entryset-method-in-java/) | 返回此映射中包含的映射的集合视图。 |
| [`get(Object key)`](https://www.geeksforgeeks.org/hashmap-get-method-in-java/) | 返回指定键映射到的值，如果此映射不包含键映射，则返回 `null`。 |
| [`isEmpty()`](https://www.geeksforgeeks.org/hashmap-isempty-method-in-java/) | 如果此映射不包含键值映射，则返回 `true`。 |
| [`keySet()`](https://www.geeksforgeeks.org/hashmap-keyset-method-in-java/#:~:text=util.,the%20key%20elements%20in%20them.) | 返回此地图中包含的键的集合视图。 |
| [`merge(K key, V value, BiFunction<? super V, ? super V, ? extends V> remappingFunction)`](https://www.geeksforgeeks.org/hashmap-mergekey-value-bifunction-method-in-java-with-examples/#:~:text=The%20merge(Key%2C%20Value%2C,as%20bucket1%20and%20so%20on.) | 如果指定的键尚未与值相关联或与 `null` 相关联，则将其与给定的非 `null` 值相关联。 |
| [`put(K key, V value)`](https://www.geeksforgeeks.org/hashmap-put-method-in-java/) | 将指定值与此映射中的指定键相关联。 |
| [`putAll(Map<? extends K, ? extends V> m)`](https://www.geeksforgeeks.org/hashmap-putall-method-in-java/#:~:text=HashMap.,from%20one%20map%20into%20another.&text=Parameters%3A%20The%20method%20takes%20one,we%20want%20to%20copy%20from.) | 将指定映射中的所有映射复制到此映射。 |
| [`remove(Object key)`](https://www.geeksforgeeks.org/hashmap-remove-method-in-java/) | 从该映射中删除指定键的映射(如果存在)。 |
| [`size()`](https://www.geeksforgeeks.org/hashmap-size-method-in-java/#:~:text=HashMap.,or%20mappings%20in%20the%20Map.&text=Parameters%3A%20The%20method%20does%20not,pairs%20present%20in%20the%20map.) | 返回此映射中键值映射的数量。 |
| [`values()`](https://www.geeksforgeeks.org/hashmap-values-method-in-java/) | 返回此地图中包含的值的集合视图。 |

### 从 `java.util.AbstractMap` 类继承的方法

| 方法 | 描述 |
| --- | --- |
| [`equals()`](https://www.geeksforgeeks.org/abstractmap-equals-method-in-java-with-examples/#:~:text=The%20AbstractMap.,of%20this%20map%20or%20not.) | 将指定的对象与此映射进行比较，看是否相等。 |
...
| 方法 | 描述 |
| --- | --- |
| [`equals()`](https://www.geeksforgeeks.org/map-equals-method-in-java-with-examples/) | 将指定的对象与此映射进行比较，看是否相等。 |
| [`forEach(BiConsumer<? super K, ? super V> action)`](https://www.geeksforgeeks.org/map-foreach-method-in-java-with-examples/) | 对此映射中的每个条目执行给定的操作，直到所有条目都已处理完毕或该操作引发异常。 |
| [`getOrDefault(Object key, V defaultValue)`](https://www.geeksforgeeks.org/map-getordefault-method-in-java-with-examples/) | 返回指定键映射到的值，如果此映射不包含键映射，则返回默认值。 |
| [`hashCode()`](https://www.geeksforgeeks.org/map-hashcode-method-in-java-with-examples/) | 返回此映射的哈希代码值。 |
| [`putIfAbsent(K key, V value)`](https://www.geeksforgeeks.org/map-putifabsent-method-in-java-with-examples/) | 如果指定的键还没有与值相关联(或者被映射为 `null`)，则将它与给定值相关联并返回 `null`，否则返回当前值。 |
| [`remove(Object key, Object value)`](https://www.geeksforgeeks.org/map-removekey-value-method-in-java-with-examples/) | 仅当指定项当前映射到指定值时，才移除该项。 |
| [`replace(K key, V value)`](https://www.geeksforgeeks.org/map-replacekey-value-method-in-java-with-examples/) | 仅当指定键当前映射到某个值时，才替换该项。 |
| [`replace(K key, V oldValue, V newValue)`](https://www.geeksforgeeks.org/map-replacekey-oldvalue-newvalue-method-in-java-with-examples/) | 仅当当前映射到指定值时，替换指定键的条目。 |
| [`replaceAll(BiFunction<? super K, ? super V, ? extends V> function)`](https://www.geeksforgeeks.org/map-replaceall-method-in-java-with-examples/) | 将每个条目的值替换为对该条目调用给定函数的结果，直到所有条目都已处理完毕或函数引发异常。 |
