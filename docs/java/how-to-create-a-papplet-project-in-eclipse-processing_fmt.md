...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-papplet-project-in-eclipse-processing/](https://www.geeksforgeeks.org/how-to-create-a-papplet-project-in-eclipse-processing/)
...
*   `PApplet` 类必须是要嵌入网页或由 Java Applet Viewer 查看的 Applet 的超类。
*   `PApplet` 类有两种主要方法，即 `setup()` 和 `draw()` 方法。
*   `setup` 方法运行一次，而 `draw` 功能将重复运行，在画布上绘制图像。
...
下面是一个了解如何用 Java 创建 applet 的示例。在这个小程序中，我们正在画布上渲染图像。该图像可以来自互联网或本地电脑。众所周知，有两个函数需要定义，在每个函数中定义如下：
