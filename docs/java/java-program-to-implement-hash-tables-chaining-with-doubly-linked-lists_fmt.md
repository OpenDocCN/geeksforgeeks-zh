...
> 原文：[https://www.geeksforgeeks.org/java-program-to-implement-hash-tables-chaining-with-doubly-linked-lists/](https://www.geeksforgeeks.org/java-program-to-implement-hash-tables-chaining-with-doubly-linked-lists/)
...
[Java 中的哈希表链接](https://www.geeksforgeeks.org/implementing-our-own-hash-table-with-separate-chaining-in-java/)可以同时使用单链表和 [`双链表`](https://www.geeksforgeeks.org/doubly-linked-list/)。虽然实现是相同的，但唯一的区别是双向链表允许双向遍历，即节点包含指向下一个和上一个节点的指针。因此，与单链表（`O(n)`）相比，在已知位置插入和删除的复杂性降低到 `O(1)`。
