...
> 原文：`https://www.geeksforgeeks.org/bigdecimal-tobigintegerexact-method-in-java/`

`java.math.BigDecimal.toBigIntegerExact()`是 Java 中的一个内置方法，它将这个 `BigDecimal` 转换为 `BigInteger`，检查丢失的信息。如果这个 `BigDecimal` 有非零的小数部分，就会引发异常。
