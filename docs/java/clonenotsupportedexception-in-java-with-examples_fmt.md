...
> 原文：[https://www.geeksforgeeks.org/clonenotsupportedexception-in-java-with-examples/](https://www.geeksforgeeks.org/clonenotsupportedexception-in-java-with-examples/)

`CloneNotSupportedException`被抛出，表示已经调用了`Object`类中的`clone()`方法来克隆一个对象，但是该对象的类没有实现`Cloneable`接口。
