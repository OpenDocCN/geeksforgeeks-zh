...
> 原文: [https://www.geeksforgeeks.org/introduction-to-the-spring-data-framework/](https://www.geeksforgeeks.org/introduction-to-the-spring-data-framework/)

在之前的文章中，我们已经讨论过[Spring框架](https://www.geeksforgeeks.org/introduction-to-spring-framework/)和[Spring Boot](https://www.geeksforgeeks.org/introduction-to-spring-boot/?ref=rp)。在本文中，我们将了解`Spring Data`框架。

## 对 Spring Data Framework 的需求
...
`Spring Data Framework` 是包含多个子框架的父项目。所有这些子框架都处理特定于数据库的数据访问。该框架的设计目标是为所有数据访问技术(如关系或非关系数据库、基于云的技术或地图缩减框架)提供一个基于 `Spring` 的熟悉且一致的模型。简而言之，`Spring Data` 是一个让 `Spring` 开发人员轻松进入 `NoSQL` 新兴世界的计划。以下是 `Spring Data` 积极支持的一系列技术。
...
以下是 `Spring Data` 框架中使用的一些关键术语及其定义。
...
5.  **HBase:** 是一个面向列的数据库。其中行指向作为键值对的“列”。列可以分组为列族。
