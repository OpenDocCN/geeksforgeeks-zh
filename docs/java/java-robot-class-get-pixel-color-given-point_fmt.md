# Java Robot类：获取给定点的像素颜色

> 原文：[https://www.geeksforgeeks.org/java-robot-class-get-pixel-color-given-point/](https://www.geeksforgeeks.org/java-robot-class-get-pixel-color-given-point/)

`Robot`类是`java.awt`包的一部分。`Robot`类基本上用于生成本机系统输入事件，用于自运行演示、测试自动化和其他使用鼠标和键盘控制的应用程序。
...
在下面的程序中，我们将打印一个标签，该标签将包含输入像素的RGB值，并且标签的文本将是像素颜色。
