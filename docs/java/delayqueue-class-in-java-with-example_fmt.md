...
> 原文: [https://www.geeksforgeeks.org/delayqueue-class-in-java-with-example/](https://www.geeksforgeeks.org/delayqueue-class-in-java-with-example/)

`DelayQueue`类是 [Java 集合框架](https://www.geeksforgeeks.org/collections-in-java-2/)的成员，属于`java.util.concurrent`包。`DelayQueue`实现[`BlockingQueue`](https://www.geeksforgeeks.org/blockingqueue-interface-in-java/)接口。`DelayQueue`是一个专门的[`PriorityQueue`](https://www.geeksforgeeks.org/priority-queue-class-in-java-2/)，它根据元素的延迟时间对元素进行排序。这意味着只有那些延迟已经过期的元素可以从队列中取出。

`DelayQueue`头包含在最短时间内过期的元素。如果没有延迟过期，则没有 head，轮询将返回`null`。`DelayQueue`只接受属于`Delayed`类的元素，或者实现`java.util.concurrent.Delayed`接口的元素。延迟队列在内部阻塞元素，直到某个延迟过期。延迟队列实现了获取延迟(`TimeUnit`。`getDelay()`)方法返回剩余的延迟时间。传递给`getDelay()`方法的`TimeUnit`实例是一个枚举，它告诉延迟应该以哪个时间单位返回。`TimeUnit`枚举可能需要天、小时、分钟、秒、毫秒、微秒、纳秒。此队列不允许空元素。这个类及其迭代器实现了`Collection`和`Iterator`接口的所有可选方法。方法[`iterator()`](https://www.geeksforgeeks.org/delayqueue-iterator-method-in-java-with-examples/)中提供的迭代器不能保证以任何特定的顺序遍历延迟队列的元素。
...
{
    /**
     * 返回与此对象相关联的剩余延迟，以给定时间单位。
     *
     * @param unit 时间单位
     * @return 剩余延迟；零值或负值表示延迟已经过去
     */
...
它实现了`Iterable<E>`、`Collection<E>`、[`BlockingQueue<E>`](https://www.geeksforgeeks.org/blockingqueue-interface-in-java/)、[`Queue<E>`](https://www.geeksforgeeks.org/queue-interface-java/)接口。
...
要构建一个`DelayQueue`，我们需要从`java.util.concurrent.DelayQueue`导入。
...
2.  **`DelayQueue(Collection<? extends E> c)`**：这个构造函数用来构造一个延迟队列，将`Delayed`实例集合的元素作为参数传递。
    ```java
    DelayQueue<E> dq = new DelayQueue<E>(Collection<? extends E> c);
    ```
...
`DelayQueue` 类的 [`remove()`](https://www.geeksforgeeks.org/delayqueue-remove-method-in-java/) 方法用于从这个 `DelayQueue` 中移除给定对象的单个实例，比如 `obj`(如果它存在的话)。如果给定元素被成功移除，则返回 `true`，否则返回 `false`。
...
`DelayQueue` 的 [`iterator()`](https://www.geeksforgeeks.org/delayqueue-iterator-method-in-java-with-examples/) 方法用于返回 `DelayQueue` 中所有元素的迭代器。
...
| 方法 | 描述 |
| --- | --- |
| [`add(E e)`](https://www.google.com/url?client=internal-element-cse&cx=009682134359037907028:tj6eafkv_be&q=https://www.geeksforgeeks.org/delayqueue-add-method-in-java-with-examples/&sa=U&ved=2ahUKEwjV9JDj-_vrAhVFjuYKHb0uA84QFjADegQICRAC&usg=AOvVaw0kOXPgzZbcG3tBkRDZRB9C) | 将指定的元素插入这个延迟队列。 |
| [`clear()`](https://www.geeksforgeeks.org/delayqueue-clear-method-in-java/#:~:text=The%20clear()%20method%20of,after%20this%20call%20is%20returned.) | 从这个延迟队列中自动移除所有元素。 |
| [`drainTo(Collection<? super E> c)`](https://www.google.com/url?client=internal-element-cse&cx=009682134359037907028:tj6eafkv_be&q=https://www.geeksforgeeks.org/delayqueue-drainto-method-in-java-with-examples/&sa=U&ved=2ahUKEwjV9JDj-_vrAhVFjuYKHb0uA84QFjACegQIBxAC&usg=AOvVaw3NAiE9KS7xxoiIG4YA7xyn) | 从此队列中移除所有可用元素，并将它们添加到给定集合中。 |
...
| [`iterator()`](https://www.google.com/url?client=internal-element-cse&cx=009682134359037907028:tj6eafkv_be&q=https://www.geeksforgeeks.org/delayqueue-iterator-method-in-java-with-examples/&sa=U&ved=2ahUKEwjV9JDj-_vrAhVFjuYKHb0uA84QFjAIegQIBRAC&usg=AOvVaw1DjM9nWHanh-0xEmlF4gt2) | 返回该队列中所有元素(过期和未过期)的迭代器。 |
| [`offer(E e)`](https://www.google.com/url?client=internal-element-cse&cx=009682134359037907028:tj6eafkv_be&q=https://www.geeksforgeeks.org/delayqueue-offer-method-in-java-with-examples/&sa=U&ved=2ahUKEwjV9JDj-_vrAhVFjuYKHb0uA84QFjAHegQIAhAC&usg=AOvVaw0mkCYjX6fKDBbAoJXZuTBJ) | 将指定的元素插入这个延迟队列。 |
| `offer(E e, long timeout, TimeUnit unit)` | 将指定的元素插入这个延迟队列。 |
...
| [`poll()`](https://www.geeksforgeeks.org/delayqueue-poll-method-in-java-with-examples/) | 检索并移除该队列的头，或者如果该队列没有具有过期延迟的元素，则返回 `null`。 |
| `poll(long timeout, TimeUnit unit)` | 检索并删除该队列的头，如有必要，等待直到具有过期延迟的元素在该队列中可用，或者指定的等待时间过期。 |
| [`put(E e)`](https://www.google.com/url?client=internal-element-cse&cx=009682134359037907028:tj6eafkv_be&q=https://www.geeksforgeeks.org/delayqueue-put-method-in-java-with-examples/&sa=U&ved=2ahUKEwjV9JDj-_vrAhVFjuYKHb0uA84QFjAGegQIARAC&usg=AOvVaw1Zw0Eunud0Zthn3ZiQyhVO) | 将指定的元素插入这个延迟队列。 |
| [`remainingCapacity()`](https://www.google.com/url?client=internal-element-cse&cx=009682134359037907028:tj6eafkv_be&q=https://www.geeksforgeeks.org/delayqueue-remainingcapacity-method-in-java-with-examples/&sa=U&ved=2ahUKEwiT7-WW_PvrAhVN63MBHdG8A684ChAWMAB6BAgAEAI&usg=AOvVaw3XnICzSRZNdNBQgF8Zw7a3) | 总是返回 `Integer.MAX_VALUE`，因为延迟队列不受容量限制。 |
| [`remove(Object o)`](https://www.google.com/url?client=internal-element-cse&cx=009682134359037907028:tj6eafkv_be&q=https://www.geeksforgeeks.org/delayqueue-remove-method-in-java/&sa=U&ved=2ahUKEwjV9JDj-_vrAhVFjuYKHb0uA84QFjAEegQIBBAC&usg=AOvVaw040T6VPyHNpe1K5Duv1wXO) | 从该队列中移除指定元素的单个实例(如果存在),无论它是否已过期。 |
| [`take()`](https://www.google.com/url?client=internal-element-cse&cx=009682134359037907028:tj6eafkv_be&q=https://www.geeksforgeeks.org/delayqueue-take-method-in-java-with-examples/&sa=U&ved=2ahUKEwjV9JDj-_vrAhVFjuYKHb0uA84QFjABegQICBAC&usg=AOvVaw1A-2FlV8CwkLx_827Qkdt5) | 检索并删除该队列的头，如有必要，等待直到具有过期延迟的元素在该队列中可用。 |
| [`toArray()`](https://www.google.com/url?client=internal-element-cse&cx=009682134359037907028:tj6eafkv_be&q=https://www.geeksforgeeks.org/delayqueue-toarray-method-in-java-with-examples/&sa=U&ved=2ahUKEwjV9JDj-_vrAhVFjuYKHb0uA84QFjAFegQIAxAC&usg=AOvVaw1dGOrSfUHw3AhrwWxX8vbi) | 返回包含该队列中所有元素的数组。 |
...
| 方法 | 描述 |
| --- | --- |
| [`contains(Object o)`](https://www.geeksforgeeks.org/blockingqueue-contains-method-in-java-with-examples/#:~:text=The%20contains(Object%20o)%20method,it%20returns%20a%20false%20value.) | 如果此队列包含指定的元素，则返回 `true`。 |

## 接口 java.util.Collection 中声明的方法

| 方法 | 描述 |
| --- | --- |
| [`addAll(Collection<? extends E> c)`](https://www.geeksforgeeks.org/collection-addall-method-in-java-with-examples/#:~:text=The%20addAll(Collection%20collection)%20of,the%20successfulness%20of%20the%20operation.) | 将指定集合中的所有元素添加到此集合中(可选操作)。 |
...
| `size()` | 返回此集合中的元素数量。 |
| `spliterator()` | 在此集合中的元素上创建一个[拆分器](https://www.geeksforgeeks.org/java-util-interface-spliterator-java8/)。 |
| `stream()` | 返回以此集合为源的顺序流。 |
