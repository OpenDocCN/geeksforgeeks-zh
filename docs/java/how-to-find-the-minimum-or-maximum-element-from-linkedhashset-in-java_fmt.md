...
> 原文: [https://www.geeksforgeeks.org/how-to-find-the-minimum-or-maximum-element-from-linkedhashset-in-java/](https://www.geeksforgeeks.org/how-to-find-the-minimum-or-maximum-element-from-linkedhashset-in-java/)

给定一个 `LinkedHashSet`，任务是在 Java 中从这个 `LinkedHashSet` 中找到最小和最大元素。有两种方法可以从 Java 中的 `LinkedHashSet` 中找到最小或最大元素，如下所示：

1.  使用 `Collections` 类的最小和最大方法。
...
*   `Collections.max(Object obj)`: 接受集合对象。
*   `Collections.min(Object obj)`: 接受集合对象。
