...
> 原文：[https://www.geeksforgeeks.org/java-program-to-convert-long-to-int/](https://www.geeksforgeeks.org/java-program-to-convert-long-to-int/)

`Long`是一种比`int`更大的数据类型，我们需要显式地对转换执行类型转换。类型转换是通过类型转换运算符执行的。将`long`转换为`int`基本上有三种方法：

1.  通过显式类型转换
2.  使用`Math.toIntExact()`方法
...
`target_datatype`：是我们要将源数据类型转换成的目标数据类型。
...
**异常：**
它抛出`ArithmeticException`——如果结果溢出一个整数。
