# Java 中的 LongBuffer duplicate() 方法

> 原文: [https://www.geeksforgeeks.org/longbuffer-duplicate-method-in-java/](https://www.geeksforgeeks.org/longbuffer-duplicate-method-in-java/)

`java.nio.LongBuffer` 类的 `duplicate()` 方法用于创建共享给定缓冲区内容的新的长缓冲区。
...
**返回值:** 该方法返回携带前一个长缓冲区内容的新长缓冲区。

以下是说明 `duplicate()` 方法的示例:

**示例 1:** 使用直接 LongBuffer
