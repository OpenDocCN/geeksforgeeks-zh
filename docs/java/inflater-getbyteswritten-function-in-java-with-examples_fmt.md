...
> 原文：[https://www.geeksforgeeks.org/inflater-getbyteswritten-function-in-java-with-examples/](https://www.geeksforgeeks.org/inflater-getbyteswritten-function-in-java-with-examples/)

`Inflater` 类的 `getBytesWritten()` 函数返回到目前为止提供的未压缩字节输出的总数。
...
## 返回类型
函数返回 `Long` 值，即输出的未压缩字节总数。

## 异常
函数不抛出任何异常

## 示例 1：`getBytesWritten()` 函数的使用
...
## 参考
[https://docs.oracle.com/javase/7/docs/api/java/util/zip/Inflater.html#getBytesWritten()](https://docs.oracle.com/javase/7/docs/api/java/util/zip/Inflater.html#getBytesWritten())