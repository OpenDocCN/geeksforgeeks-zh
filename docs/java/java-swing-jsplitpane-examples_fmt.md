...
> 原文: [https://www.geeksforgeeks.org/java-swing-jsplitpane-examples/](https://www.geeksforgeeks.org/java-swing-jsplitpane-examples/)
...
6.  `setRightComponent(Component c)`：拆分窗格的右侧组件设置为 `c`。
...
10. `setUI(SplitPaneUI ui)`：设置渲染此组件的观感对象。
11. `setResizeWeight(double v)`：指定拆分窗格大小变化时如何分配额外空间。
12. `setOneTouchExpandable(boolean n)`：设置 `oneTouchExpandable` 属性的值，该属性为 `true` 时提供一个用户界面小部件，可以在单击时折叠或展开组件。
13. `setDividerLocation(int l)`：设置分割线的位置。
14. `setDividerSize(int n)`：设置分割线的大小。
15. `setLastDividerLocation(int n)`：设置分割线的最后一个位置。
16. `setDividerLocation(double p)`：将分隔符位置设置为 `JSplitPane` 大小的百分比。
17. `setContinuousLayout(boolean n)`：设置 `continuousLayout` 属性的值，对于要连续重新显示的子组件，该值必须为真。
18. `remove(int index)`：移除指定索引处的组件。
19. `remove(Component c)`：从窗格中移除子组件。
20. `isOneTouchExpandable()`：返回 `oneTouchExpandable` 属性。
21. `isContinuousLayout()`：返回 `ContinuousLayout` 属性。
22. `getMinimumDividerLocation()`：返回分割线的最小位置。
23. `getMaximumDividerLocation()`：返回分割线的最大位置。
24. `getDividerSize()`：返回分割线的大小。
25. `getDividerLocation()`：返回传递给 `setDividerLocation` 的最后一个值。
26. `add(Component c, Object co, int i)`：将指定的组件添加到该拆分窗格中。
