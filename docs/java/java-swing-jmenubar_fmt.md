...
`JMenuBar`、`JMenu` 和 `JMenuItem` 是 Java Swing 包的一部分。`JMenuBar` 是菜单栏的一个实现。`JMenuBar` 包含一个或多个 `JMenu` 对象，当选择 `JMenu` 对象时，它们会显示一个弹出窗口，显示一个或多个 `JMenuItem`。
...
3.  `JMenu(String s)`：用指定的名称创建新菜单。
4.  `JMenu(String s, boolean b)`：用指定的名称创建一个新菜单，布尔值指定它是否为可撕下菜单。可撕掉的菜单可以被打开并从其父菜单栏或菜单中拖出来。

**常用方法:**

1.  `add(JMenu c)`：向菜单栏添加菜单。将对象添加到菜单栏。
2.  `add(Component c)`：在 `JMenu` 的末尾添加组件。
3.  `add(Component c, int index)`：将组件添加到 `JMenu` 的指定索引中。
4.  `add(JMenuItem menuItem)`：将菜单项添加到菜单的末尾。
5.  `add(String s)`：用指定的字符串创建一个菜单项，并将其附加到菜单的末尾。
6.  `getItem(int index)`：返回给定索引处的指定 `JMenuItem`。
