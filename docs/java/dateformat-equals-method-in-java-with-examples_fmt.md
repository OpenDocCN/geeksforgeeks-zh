# DateFormat.equals()方法详解（Java示例）

> 原文：[https://www.geeksforgeeks.org/dateformat-equals-method-in-java-with-examples/](https://www.geeksforgeeks.org/dateformat-equals-method-in-java-with-examples/)

`DateFormat`类的`equals()`方法用于比较两个日期格式对象。如果此日期格式等于传递的对象，则方法返回真，否则返回假。
...
参数：该方法取一个对象类型的参数`obj`，并引用要与该日期格式对象进行比较的对象。

返回值：如果两个对象相等，则该方法返回布尔真，否则该方法返回假。

以下程序说明了`DateFormat`的`equals()`方法：
