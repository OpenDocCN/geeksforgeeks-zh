...
> 原文：[https://www.geeksforgeeks.org/convert-an-iterable-to-collection-in-java/](https://www.geeksforgeeks.org/convert-an-iterable-to-collection-in-java/)

[`Iterable`](https://www.geeksforgeeks.org/java-implementing-iterator-and-iterable-interface/) 和 [`Collection`](https://www.geeksforgeeks.org/collections-in-java-2/) 在 Java 中有很大的用处。迭代器在 Java 的 Collection 框架中用于逐个检索元素，Collection 是一组表示为单个单元的单个对象。Java 提供了集合框架，该框架定义了几个类和接口，将一组对象表示为一个单元。
...
创建一个实用函数意味着创建一个函数，通过显式地考虑每个项目，将 `iterable` 转换为集合。这也可以通过多种方式实现，如下所述：
...
`forEach` 循环在后台使用迭代器。因此，它可以通过以下方式显式完成。
...
随着 Java 8 中 [`Stream`](https://www.geeksforgeeks.org/stream-in-java/) 的引入，这样的工作已经变得相当容易了。要将可迭代转换为集合，首先将 `Iterable` 转换为 `Spliterator`。然后借助 `StreamSupport.stream()` 可以遍历分割器，然后借助 `collect()` 进行收集。
