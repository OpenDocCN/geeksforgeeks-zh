...
> 原文: [https://www.geeksforgeeks.org/java-program-to-convert-octal-to-binary/](https://www.geeksforgeeks.org/java-program-to-convert-octal-to-binary/)
...
`Integer.parseInt()` 是 Java 中的内置函数，用于将字符串解析为由基数值(方法的第二个参数)指定的数字系统。在这种方法中，八进制数的输入将被视为字符串。八进制字符串将被解析为八进制的整数值。此外，八进制数将使用另一种内置方法 `Integer.toBinaryString()` 将其转换为等效的二进制数。结果值也将是字符串数据类型。下面是实现。
