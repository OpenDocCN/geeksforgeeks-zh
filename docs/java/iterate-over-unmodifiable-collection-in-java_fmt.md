...
> 原文：[https://www.geeksforgeeks.org/iterate-over-unmodifiable-collection-in-java/](https://www.geeksforgeeks.org/iterate-over-unmodifiable-collection-in-java/)
...
**示例：** 我们可以在任何数据结构之前初始化可修改的类，这样我们就可以防止它更新给定数据结构中的任何元素。我们可以使用 `Collections.unmodifiableCollection(Collection)` 创建集合的不可修改视图。
...
## 例 3：演示 `UnsupportedOperationException`
