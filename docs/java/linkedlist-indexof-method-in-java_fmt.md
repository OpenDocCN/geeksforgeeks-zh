...
> 原文: [https://www.geeksforgeeks.org/linkedlist-indexof-method-in-java/](https://www.geeksforgeeks.org/linkedlist-indexof-method-in-java/)
...
参数`element`属于链接列表类型。它指定需要在链接列表中检查其出现的元素。

## 返回值

如果列表中不存在`element`，则该方法返回列表中第一个出现的`element`的索引或位置。返回值是整数类型。
