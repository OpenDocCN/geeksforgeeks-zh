...
> 原文: [https://www.geeksforgeeks.org/fail-fast-fail-safe-iterators-java/](https://www.geeksforgeeks.org/fail-fast-fail-safe-iterators-java/)
...
Java 中的迭代器用于迭代 Collection 对象。如果集合中有**结构修改**，Fail-Fast 迭代器会立即抛出`ConcurrentModificationException`。结构修改意味着在线程迭代集合时，从集合中添加、移除任何元素。`ArrayList`上的迭代器，`HashMap`类是快速失败迭代器的一些例子。
