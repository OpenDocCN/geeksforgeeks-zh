# Java中的`Stream.of()`和`Arrays.stream()`方法的区别

> 原文：[https://www.geeksforgeeks.org/difference-between-stream-of-and-arrays-stream-method-in-java/](https://www.geeksforgeeks.org/difference-between-stream-of-and-arrays-stream-method-in-java/)

## `Arrays.stream()`

Java中`Arrays`类的`stream(T[] array)`方法用于从作为参数传递的数组及其元素中获取一个顺序流。它返回一个顺序流，数组的元素作为参数传递，作为它的源。
...
`Stream.of(T... values)`返回一个顺序的有序流，其元素是指定的值。`Stream.of()`方法只是为非基元类型调用`Arrays.stream()`方法。
...
这两种方法是从指定数组创建顺序流时最常用的两种方法。当用非基元类型`T`调用时，这两种方法都返回一个`Stream<T>`。

## `Arrays.stream()`和`Stream.of()`之间的差异

即使`Stream.of()`是`Arrays.stream()`方法的包装器，也有一些特定的差异点，明确了何时使用`Arrays.stream()`或何时使用`Stream.of()`。以下是上述两种方法之间的一些区别：

1.  **不同的返回类型**：

    对于基元数组（如`int[]`、`long[]`等），`Arrays.stream()`和`Stream.of()`具有不同的返回类型。

    **示例：**传递一个整数数组，`Stream.of()`方法返回`Stream<int[]>`，而`Arrays.stream()`返回一个`IntStream`。
...
2.  **`Stream.of()`需要展平，而`Arrays.stream()`不需要**：
    ...
    因为用于处理基元类型的流的理想类是它们的基元流类型（如`IntStream`、`LongStream`等）。因此，在使用之前，需要将`Stream.of()`返回的流显式展平为其原始流。
...
3.  **`Stream.of()`是泛型，而`Arrays.stream`不是**：
    ...
    另一方面，`Stream.of()`返回一个类型为`T`的通用流（`Stream<T>`）。因此，它可以用于任何类型。
    ...
    *   **对于`Arrays.stream()`方法：**
        ...
        **输出：**
        ...
        > Java代码中的编译错误：-
        > prog.java:20: 错误: 找不到适合 stream(char[])的方法
        >         Arrays.stream(arr);
        >                ^
        > 方法 Arrays.stream(T[])不适用于参数 (char[])
