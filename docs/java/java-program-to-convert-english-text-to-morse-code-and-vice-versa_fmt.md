...
> 原文: [https://www.geeksforgeeks.org/java-program-to-convert-english-text-to-morse-code-and-vice-versa/](https://www.geeksforgeeks.org/java-program-to-convert-english-text-to-morse-code-and-vice-versa/)

**莫尔斯电码** 是电信中使用的一种方法，将文本字符编码为两种不同信号持续时间的标准化序列，称为点和破折号。莫尔斯电码定义了一种标准编码，其中每个字母都映射到一系列点和破折号，如下所示: `a` 映射到 `.-`，`b` 映射到 `-...`，`c` 映射到 `-.-.`，等等。
...
1.  将所有字母表存储在一个数组中，该数组在本代码中的名称为 `letter[]`，在此将所有字母表存储在另一个数组中，该数组的名称为 `code[]`。
2.  将输入字符串转换为字符数组 `string[]`。
3.  将字符位置与字母数组中的一个字母反复匹配。
4.  找到它在 `letter[]` 数组中的字符位置，并使用该位置从 `code[]` 数组中返回出现在该位置的字符。
