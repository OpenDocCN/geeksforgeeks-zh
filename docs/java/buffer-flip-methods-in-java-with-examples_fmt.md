# Java 中的缓冲区 flip() 方法示例

> 原文: [https://www.geeksforgeeks.org/buffer-flip-methods-in-java-with-examples/](https://www.geeksforgeeks.org/buffer-flip-methods-in-java-with-examples/)

`java.nio.ByteBuffer` 类的 `flip()` 方法用来翻转这个缓冲区。极限被设置为当前位置，然后位置被设置为零。如果标记被定义，那么它被丢弃。在一系列通道读取或放置操作之后，调用此方法为一系列通道写入或相关的获取操作做准备。
...
[https://docs.oracle.com/javase/9/docs/api/java/nio/Buffer.html#flip--](https://docs.oracle.com/javase/9/docs/api/java/nio/Buffer.html#flip--)