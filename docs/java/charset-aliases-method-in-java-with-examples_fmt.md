# Java 中的 `Charset.aliases()` 方法，带示例

> 原文：[https://www.geeksforgeeks.org/charset-aliases-method-in-java-with-examples/](https://www.geeksforgeeks.org/charset-aliases-method-in-java-with-examples/)

`aliases()` 方法是 `java.nio.charset` 包的内置方法，它返回一个包含该 `Charset` 别名的集合。当一个对象可以被多个引用引用时，这意味着它们存储同一个对象的引用值，那么这样的引用被称为别名。
...
该方法返回一个 `Set`，该集合包含代表字符集内置别名的字符串。
...
## PCK (日文 Shift_JIS 的另一种表示)

`pck`

## Unicode 字节顺序标记 (BOM) 变体
...
**参考:** [https://docs.oracle.com/javase/9/docs/api/java/nio/charset/Charset.html#aliases--](https://docs.oracle.com/javase/9/docs/api/java/nio/charset/Charset.html#aliases--)