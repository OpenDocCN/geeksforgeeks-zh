...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-basic-intro-slider-of-an-android-app/](https://www.geeksforgeeks.org/how-to-create-a-basic-intro-slider-of-an-android-app/)

当我们下载任何应用程序并第一次使用该应用程序时，然后我们将看到应用程序中的介绍滑块。在这个滑块的帮助下，我们教育用户如何使用这个应用程序，它详细地讲述了这个应用程序。在这篇文章中，我们将看看应用程序中 Intro Slider 的实现。现在，让我们开始在应用程序中实现这一功能。
...
现在导航到（项目的）`gradle 脚本 > build.gradle` 文件，并在存储库部分添加下面的代码。
...
**第三步：创建一个新的 Java 类来显示滑块的幻灯片**

要创建一个新的 Java 类，导航到 `应用程序 > java > 你的应用程序的包名 > 右键单击它，然后单击新建 > Java 类`，并将其命名为 `IntroSlider`。创建此类后，向其中添加以下代码。代码中添加了注释，以更详细地理解代码。
...
当我们创建一个新的活动来显示介绍滑块时，我们将这个活动添加到 `AndroidManifest.xml` 文件中。将以下几行添加到你的 `AndroidManifest.xml` 文件中。
