...
> 原文: [https://www.geeksforgeeks.org/logger-finer-method-in-java-with-examples/](https://www.geeksforgeeks.org/logger-finer-method-in-java-with-examples/)

`Logger`类的`finer()`方法用于记录更精细的消息。此方法用于将`FINER`类型日志传递给所有注册的输出处理程序对象。
...
此方法用于记录`finer`消息。如果记录器被启用来记录更精细级别的消息，那么给定的消息将被转发到所有注册的输出处理程序对象。
...
**参数:** 该方法接受单个参数`String msg`，即字符串消息。

**返回值:** 此方法不返回任何内容。

下面的程序说明了`finer(String msg)`方法:
...
## finer(Supplier msgSupplier)

此方法用于记录`FINER`消息，仅在日志级别确实会记录该消息时才构造。这意味着如果记录器被启用到`FINER`消息级别，则通过调用提供的`Supplier`函数来构造消息，并转发给所有已注册的输出`Handler`对象。
