...
> 原文：[https://www.geeksforgeeks.org/field-getdeclaringclass-method-in-java-with-examples/](https://www.geeksforgeeks.org/field-getdeclaringclass-method-in-java-with-examples/)

[`getDeclaringClass()`](https://www.geeksforgeeks.org/reflection-in-java/) 方法属于 `Field` 类，用于获取声明该 `Field` 对象所代表的字段的类对象。如果 `Field` 对象存在，并且我们想要获取其声明类对象，那么我们可以使用这个方法。
