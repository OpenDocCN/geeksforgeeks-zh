...
局部内部类是在*块*中定义的内部类。通常，这个块是一个方法体。有时这个块可以是 `for` 循环或 `if` 子句。本地内部类不是任何封闭类的成员。它们属于定义它们的块，因此本地内部类不能有任何关联的访问修饰符。但是，它们可以标记为 `final` 的或 `abstract` 的。这个类可以访问包含它的类的字段。本地内部类必须在定义它们的块中实例化。
...
编译包含本地内部类的程序时，编译器会生成两个 `.class` 文件，一个用于外部类，另一个用于引用外部类的内部类。编译器将这两个文件命名为:

*   `Outer.class`
*   `Outer$1InnerClass.class`
...
例如，如果您在上述示例中的`Inner`类构造函数或`Inner`类的任何方法中添加突出显示的赋值语句:
...
因为这个赋值语句，变量`sum`不再是*有效的最终*。因此，Java 编译器会生成类似于“从内部类引用的局部变量必须是最终的或者实际上是最终的”的错误消息
...
**解释:** 上面的程序由于内部类不能声明为 `static`，导致编译错误。内部类与它们在其中定义的块相关联，而不是与外部类相关联(在本例中是 `Outer` 类)。
