...
> 原文: [https://www.geeksforgeeks.org/charbuffer-duplicate-method-in-java/](https://www.geeksforgeeks.org/charbuffer-duplicate-method-in-java/)

`java.nio.CharBuffer` 类的 `duplicate()` 方法用于创建共享给定缓冲区内容的新的 `char buffer`。
...
该方法返回 `新的 char 缓冲区`，其携带 `以前的 char 缓冲区` 内容。

以下是说明 `duplicate()` 方法的示例：

## 示例 1：使用直接 CharBuffer
