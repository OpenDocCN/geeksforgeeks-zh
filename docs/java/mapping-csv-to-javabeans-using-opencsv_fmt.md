...
> 原文: [https://www.geeksforgeeks.org/mapping-csv-to-javabeans-using-opencsv/](https://www.geeksforgeeks.org/mapping-csv-to-javabeans-using-opencsv/)
...
1.  创建一个 `HashMap`，用于映射列 id 和 bean 属性。
...
2.  创建 `HeaderColumnNameTranslateMappingStrategy` 对象，将映射 `HashMap` 传递给 `setColumnMapping` 方法。
...
3.  创建 `CSVReader` 和 `CsvToBean` 类的对象。
...
4.  调用 `CsvToBean` 类的 `parse` 方法，并传递 `HeaderColumnNameTranslateMappingStrategy` 和 `CSVReader` 对象。
...
让我们将包含学生数据的 csv 文件转换为具有属性 `Name`、`RollNo`、`Department`、`Result`、`Pointer` 的学生对象。
...
首先创建一个具有属性 `Name`、`RollNo`、`Department`、`Result`、`Pointer` 的 `Student` 类。然后创建一个将 csv 数据映射到 JavaBeans 对象的主类。
...
**参考:**
*   [OpenCSV 文档](http://opencsv.sourceforge.net/)
*   [`CsvToBean` 文档](http://opencsv.sourceforge.net/apidocs/com/opencsv/bean/CsvToBean.html)
*   [`MappingStrategy` 文档](http://opencsv.sourceforge.net/apidocs/com/opencsv/bean/MappingStrategy.html)