# Java中的AtomicReferenceArray.set()方法示例
...
`AtomicReferenceArray`类的`set()`方法用于将索引`i`处的元素的值设置为新值。索引`i`和`newValue`都作为参数传递给该方法。这个方法用内存读取语义设置值，就像变量被声明为易失性类型的变量一样。
...
[https://docs.oracle.com/javase/10/docs/api/java/util/concurrent/atomic/AtomicReferenceArray.html#set(int,E)](https://docs.oracle.com/javase/10/docs/api/java/util/concurrent/atomic/AtomicReferenceArray.html#set(int,E))