# Java 中的抽象列表 subList() 方法示例

> 原文：[https://www.geeksforgeeks.org/abstractlist-sublist-method-in-java-with-examples/](https://www.geeksforgeeks.org/abstractlist-sublist-method-in-java-with-examples/)

[`java.util.AbstractList`](https://www.geeksforgeeks.org/abstractlist-in-java-with-examples/) 类的 `subList()` 方法用于返回此列表中指定的 `fromIndex`（包含）和 `toIndex`（不包含）之间的部分的视图。（如果 `fromIndex` 和 `toIndex` 相等，则返回的列表为空。）
...
*   `IndexOutOfBoundsException`：如果端点索引值超出范围（`fromIndex < 0 || toIndex > size || fromIndex > toIndex`）
...
### 示例 3：适用于 IllegalArgumentException
