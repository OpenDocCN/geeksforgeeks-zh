...
> 原文: [https://www.geeksforgeeks.org/inflater-gettotalout-function-in-java-with-examples/](https://www.geeksforgeeks.org/inflater-gettotalout-function-in-java-with-examples/)

`Inflater` 类的 `getTotalOut()` 函数返回到目前为止提供的未压缩字节输出总数。
...
函数返回 `int` 值，即输出的未压缩字节总数。

## 异常

函数不抛出任何异常。

## 示例 1: `getTotalOut()` 函数的使用
...
当输出的长度大于 `Integer.MAX_VALUE` 时就会出现问题，那么结果可能会溢出，我们应该用 `getBytesRead()` 函数代替。

## 参考

[https://docs.oracle.com/javase/7/docs/api/java/util/zip/Inflater.html#getTotalOut()](https://docs.oracle.com/javase/7/docs/api/java/util/zip/Inflater.html#getTotalOut())