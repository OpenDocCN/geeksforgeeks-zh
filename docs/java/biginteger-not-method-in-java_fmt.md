...
> 原文：`https://www.geeksforgeeks.org/biginteger-not-method-in-java/`

`BigInteger.not()`方法用于查找`BigInteger`的按位非。当且仅当此`BigInteger`为非负值时，此方法返回负值。方法对当前的大整数应用按位非运算。
...
`https://docs.oracle.com/javase/7/docs/api/java/math/BigInteger.html#and(java.math.BigInteger)`