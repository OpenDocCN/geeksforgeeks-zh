...
`LinkedBlockingQueue` 的 `toString()` 方法返回该集合的字符串表示形式。字符串表示由集合元素的列表组成，这些元素按照迭代器返回的顺序排列，用方括号括起来（`"[]"`）。相邻的元素由字符 `", "`（逗号和空格）分隔。元素被转换为字符串，如 `String.valueOf(Object)`。
...
以下程序说明了 `LinkedBlockingQueue` 的 `toString()` 方法：
...
public class GFG {
    public static void main(String[] args) {
        ...
        System.out.println("Linked Blocking Queue in string " + LBQ.toString());
    }
}
```
...
[https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/LinkedBlockingQueue.html#toString--](https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/LinkedBlockingQueue.html#toString--)