...
> 原文: [https://www.geeksforgeeks.org/java-8-arraydeque-removeif-method-in-java-with-examples/](https://www.geeksforgeeks.org/java-8-arraydeque-removeif-method-in-java-with-examples/)
...
如果谓词返回 `true` 且某些元素被移除，则此方法返回 `true`。

## 异常

如果指定的过滤器为 `null`，该方法抛出 `NullPointerException`。

下面的程序说明了 `ArrayDeque` 的 `removeIf()` 方法：

### 示例 1

演示 `ArrayDeque` 上的 `removeIf()` 方法，该方法包含一组 `String`，移除以 'A' 开头的字符串。
...
演示 `ArrayDeque` 上的 `removeIf()` 方法，该方法包含一组学生对象，用于移除所有分数低于 40 分的学生。
...
演示 `removeIf()` 方法中的 `NullPointerException`。
...
[https://docs.oracle.com/javase/10/docs/api/java/util/ArrayDeque.html#removeIf(java.util.function.Predicate)](https://docs.oracle.com/javase/10/docs/api/java/util/ArrayDeque.html#removeIf(java.util.function.Predicate))