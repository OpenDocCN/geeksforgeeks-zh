...
> 原文：`https://www.geeksforgeeks.org/java-swing-jpanel-with-examples/`
...
3.  `JPanel(boolean isDoubleBuffered)`：用指定的缓冲策略创建一个新的 `JPanel`
4.  `JPanel(LayoutManager l, boolean isDoubleBuffered)`：使用指定的 `LayoutManager` 和指定的缓冲策略创建一个新的 `JPanel`

### `JPanel`常用功能

1.  `add(Component c)`：向指定容器添加组件
2.  `setLayout(LayoutManager l)`：将容器的布局设置为指定的布局管理器
3.  `updateUI()`：用当前观感的值重置 `UI` 属性。
4.  `setUI(PanelUI ui)`：设置渲染此组件的对象的外观和感觉。
