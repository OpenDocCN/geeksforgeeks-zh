...
> 原文: [https://www.geeksforgeeks.org/dagger-2-android-example-using-retrofit/](https://www.geeksforgeeks.org/dagger-2-android-example-using-retrofit/)
...
*   **组件:** 我们的依赖项提供者和依赖项消费者之间的连接是通过一个接口提供的，该接口使用 `@Component` 对其进行注释。剩下的事情将由匕首完成。
...
> 实现 'com.squareup.retrofit2:converter-gson:2.7.2'
> //这两行是为匕首增加的
...
下面是 `activity_main.xml` 文件的代码。这将在 `app > src > 主 > res > 布局` 文件夹下。取决于不同的分辨率，有时我们可能需要在 `布局 > hdpi` 或 `布局 > xhdpi` 文件夹等下拥有它。
