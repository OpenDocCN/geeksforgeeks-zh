...
> 原文: [https://www.geeksforgeeks.org/filestore-isreadonly-method-in-java-with-examples/](https://www.geeksforgeeks.org/filestore-isreadonly-method-in-java-with-examples/)

如果文件存储是只读的，则使用`FileStore`类的`isReadOnly()`方法返回真，否则返回假。如果文件存储不支持对文件的写操作或其他更改，则称为只读。如果试图创建一个文件、打开一个现有文件进行写入等，将引发一个`IOException`。
