...
`ColorAdjust` 类是 JavaFX 的一部分。颜色调整类允许对色调、饱和度、亮度和对比度进行逐像素调整。颜色调整类继承 `Effect` 类。

## 类的构造函数

1.  `ColorAdjust()`: 创建颜色调整类的新对象。
2.  `ColorAdjust(double hue, double saturation, double brightness, double contrast)`: 用指定的色调、饱和度、亮度和对比度值创建颜色调整类的新对象。
...
| `setBrightness(double v)` | 设置颜色调整效果的亮度值 |
| `getHue()` | 返回颜色调整效果的色调值 |
| `setHue(double v)` | 设置颜色调整效果的色调值 |
| `getContrast()` | 返回颜色调整效果的对比度值 |
| `setContrast(double v)` | 设置颜色调整效果的对比度值 |
| `getSaturation()` | 返回颜色饱和度的值调整效果 |
| `setSaturation(double v)` | 设置颜色调整效果的饱和度值 |
| `getInput()` | 返回属性输入的值 |
| `setInput(Effect v)` | 设置属性输入的值 |

下面的程序说明了 `ColorAdjust` 类的使用：

1.  **Java 程序，对指定色调、亮度、对比度和饱和度的图像应用颜色调整效果：** 在该程序中，创建一个 `FileInputStream`，并从文件中获取图像作为输入。名为 `Image` 的对象是使用来自文件输入流的输入创建的。从图像中创建 `ImageView` 对象，并将其添加到 `VBox` 中。然后 `VBox` 被添加到场景中，场景被添加到舞台中。创建 `ColorAdjust` 效果，使用功能 `setHue()`、`setBrightness()`、`setSaturation()`、`setContrast()` 设置色调、饱和度、对比度和亮度的值，并使用 `setEffect()` 功能将效果设置为图像视图。
...
1.  **将色彩调整效果应用于具有色调、亮度、对比度和饱和度的图像的 Java 程序被作为来自用户的输入（使用文本字段）：** 在该程序中，创建一个 `FileInputStream`，并且图像被作为来自文件的输入。名为 `Image` 的对象是使用来自文件输入流的输入创建的。从图像中，创建一个 `ImageView` 对象，并将其添加到 `VBox` 中。然后 `VBox` 被添加到场景中，场景被添加到舞台中。创建 `ColorAdjust` 效果，使用功能 `setHue()`、`setBrightness()`、`setSaturation()`、`setContrast()` 设置色调、饱和度、对比度和亮度的值，并使用 `setEffect()` 功能将效果设置为图像视图。我们将创建四个文本字段（色调、饱和度、对比度和亮度）和一个按钮。用户将给出必要的色调、饱和度、对比度和亮度值，当按下按钮时，这些值将应用于图像。创建 `EventHandler` 来处理按钮事件。
