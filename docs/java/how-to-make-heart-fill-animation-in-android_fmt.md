...
> 原文：[https://www.geeksforgeeks.org/how-to-make-heart-fill-animation-in-android/](https://www.geeksforgeeks.org/how-to-make-heart-fill-animation-in-android/)

`AnimatedVectorDrawable`类在`API 21`中引入，用于将`Vector drawable`制作成漂亮且轻松的动画。使用`AnimatedVectorDrawable`，可以：
*   旋转、缩放、平移矢量图形
*   将矢量绘制成动画，如填充颜色等。
*   绘制路径并进行路径变形

一个`动画矢量可绘制元素`有一个`矢量可绘制属性`，以及一个或多个目标元素。目标元素可以通过`android:name`属性指定其目标，并通过`android:animation`属性将目标与适当的对象动画器或动画器集链接。

## 绘制心填充动画的方法：

1.  在`values directory`中创建新的`heart.xml`文件，并添加以下矢量可绘制路径数据和路径命令：
