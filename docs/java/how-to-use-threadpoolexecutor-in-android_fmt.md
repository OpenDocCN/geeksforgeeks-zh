...
> 原文：[https://www.geeksforgeeks.org/how-to-use-threadpoolexecutor-in-android/](https://www.geeksforgeeks.org/how-to-use-threadpoolexecutor-in-android/)
...
`ThreadPoolExecutor`使用线程池中的一个线程来完成特定的作业。
...
*   `workQueue`：任务队列，只保存可以执行的作业。这将是一个阻塞队列。
...
必须创建优先级未来任务，它将实现`Comparable<PriorityFutureTask>`。
