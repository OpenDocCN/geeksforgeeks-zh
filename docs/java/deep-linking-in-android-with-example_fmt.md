...
> 原文：[https://www.geeksforgeeks.org/deep-linking-in-android-with-example/](https://www.geeksforgeeks.org/deep-linking-in-android-with-example/)
...
导航到`应用程序 > AndroidManifest.xml`，并在其中添加以下代码。因为我们正在为我们的`MainActivity.java`文件创建一个深度链接，所以我们必须在主活动部分添加这段代码。下面是将要添加到`AndroidManifest.xml`文件中的代码。代码中添加了注释，以便更详细地了解。
