# Java中使用静态修饰符的类加载和静态块执行

> 原文：[https://www.geeksforgeeks.org/class-loading-and-static-blocks-execution-using-static-modifier-in-java/](https://www.geeksforgeeks.org/class-loading-and-static-blocks-execution-using-static-modifier-in-java/)
...
静态块甚至在`main`方法之前就被调用了，`main`方法只不过是一个静态方法，即每个类的执行点。
...
当方法比类的实例与类更相关时使用。`java`中的`Math`类是`Java`中的一个很好的例子。它有所有的静态方法，你可以用一个类名比如`max`，`min`，`pow`来调用，因为这些函数不需要不同的对象有不同的值。您可以使用`ClassName.methodName()`传递参数并获得答案。
...
*   静态方法只能调用其他静态方法，也就是说，它们不能从静态方法（如`main`方法）中调用非静态方法。
*   静态块可以直接使用唯一的静态变量。

单例设计模式是最广泛使用的设计模式，其中对于类的每个实例请求，只创建和返回类的一个实例。`static`关键字用于只创建一次对象，因为`static`只占用一次内存，因此每次调用都会保持同步。

下面的例子清楚地显示了执行的顺序。静态块首先被调用，即使它写在`main`方法之后。它证明了静态块是在`main`方法之前首先被调用的东西。
