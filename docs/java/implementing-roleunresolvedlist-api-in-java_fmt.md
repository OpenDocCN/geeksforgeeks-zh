# 在Java中实现RoleUnresolvedList API

> 原文：[https://www.geeksforgeeks.org/implementing-roleunresolvedlist-api-in-java/](https://www.geeksforgeeks.org/implementing-roleunresolvedlist-api-in-java/)

应用程序编程接口（API）是允许两个应用程序相互通信的软件。[RoleUnresolvedList](https://www.geeksforgeeks.org/java-program-to-implement-rolelist-api/) 代表角色未解析对象的列表，这些对象代表由于试图访问、读取或写入角色时遇到一些问题而未从关系中检索到的角色。因此，RoleUnresolvedList API 是一个与内部文件通信以检索对象含义的程序，代表由于某些问题而不能从关系中检索的角色。

## 先决条件

### 包

在 Java 的 `RoleUnresolvedList` API 程序中，为了检索程序中使用的对象的含义和用途，使用了各种包。下图显示了程序中导入的包：
...
*   `RoleUnresolvedList()`：用于构造空的 `RoleUnresolvedList`。
*   `RoleUnresolvedList(int initialCapacity)`：用于构建指定了初始容量的空 `RoleUnresolvedList`。
*   `RoleUnresolvedList(List<RoleUnresolved> list)`：用于构建包含指定列表元素的 `RoleUnresolvedList`。它是按照列表迭代器返回的顺序构造的。

### 方法

*   `add(int index, Object element)`：此方法用于将特定元素插入特定位置。
*   `add(int index, RoleUnresolved role)`：此方法在指定位置插入指定为元素的 `RoleUnresolved`。
*   `add(Object o)`：在列表末尾追加指定元素。
*   `add(RoleUnresolved role)`：将添加列表最后一个元素指定的 `RoleUnresolved`。
*   `addAll(Collection<?> c)`：它会在末尾追加指定列表中的元素。
*   `addAll(int index, Collection<?> c)`：它会在指定位置插入元素。
*   `addAll(int index, RoleUnresolvedList roleList)`：从指定位置开始，按照指定 `RoleUnresolvedList` 的迭代器返回的顺序，将指定 `RoleUnresolvedList` 中的所有元素插入此列表。
