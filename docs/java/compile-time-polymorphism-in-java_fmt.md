...
> 原文: [https://www.geeksforgeeks.org/compile-time-polymorphism-in-java/](https://www.geeksforgeeks.org/compile-time-polymorphism-in-java/)
...
> **注:** 运行时多态性通过方法覆盖实现。而编译时多态性是通过方法重载和运算符重载实现的。

在本文中，我们将看到编译时多态性。

## 编译时多态性

编译时多态性也称为静态多态性或早期绑定。编译时多态性是在编译过程中解决的多态性。方法的重载是通过类的引用变量调用的。编译时多态性是通过方法重载和运算符重载实现的。
...
在给定的示例中，第一个 `show` 方法有一个参数，第二个 `show` 方法有两个参数。调用函数时，编译器会查看参数的数量，并决定如何解析方法调用。
...
class Test {
    ...
    public static void main(String args[])
    {
        ...
        // Calls show() with no parameter
        obj.show();

        // Calls show() with two parameters
        obj.show(4, 5);
    }
}
```
