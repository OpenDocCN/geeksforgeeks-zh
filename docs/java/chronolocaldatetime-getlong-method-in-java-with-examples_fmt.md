...
Java 中`ChronoLocalDateTime`接口的`getLong()`方法用于将作为输入从此`ChronoLocalDateTime`传递的指定字段的值作为长值返回。此方法在此`ChronoLocalDateTime`中查询该字段的值，返回值将始终在该字段的有效值范围内。当字段不受支持并且方法无法返回`int`值时，将引发异常。
...
## 参数
该方法接受单个参数`field`，代表要获取的字段。这是一个强制参数，不应为`null`。
