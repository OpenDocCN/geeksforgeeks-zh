...
我们在按钮组中添加单选按钮，这样我们一次只能选择一个单选按钮。我们使用 `ButtonGroup` 类来创建一个按钮组，并在一个组中添加单选按钮。
...
### `ButtonGroup()`
用于创建一个组，我们可以在其中添加 `JRadioButton`。我们只能在一个按钮组中选择一个按钮。

**将单选按钮组合在一起的步骤。**
*   使用 `ButtonGroup()` 方法创建一个按钮组实例。
```java
...
### `isSelected()`
它将返回一个布尔值 `true` 或 `false`，如果一个 `JRadioButton` 被选中，它将返回 `true`，否则返回 `false`。
示例:
```java
