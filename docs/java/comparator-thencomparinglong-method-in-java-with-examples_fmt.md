# Java 中的 Comparator.thenComparingLong 方法及示例

> 原文：[https://www.geeksforgeeks.org/comparator-thencomparinglong-method-in-java-with-examples/](https://www.geeksforgeeks.org/comparator-thencomparinglong-method-in-java-with-examples/)

Java 中的 [`Comparator`](https://www.geeksforgeeks.org/comparator-interface-java/) 接口的 `thenComparingLong(ToLongFunction)` 方法返回一个字典顺序比较器，该比较器带有一个提取 `Long` 排序关键字的函数。
...
### 参数：
该方法接受 `keyExtractor`，该函数用于提取长排序键。

### 返回值：
这个方法返回一个字典顺序比较器，由这个和 `Long` 排序键组成。
...
在示例中可以看到，首先排序是在部门上进行的，如果部门相同，则在 ID 上进行排序。
