...
> 原文: [https://www.geeksforgeeks.org/charbuffer-compareto-method-in-java/](https://www.geeksforgeeks.org/charbuffer-compareto-method-in-java/)

`java.nio.CharBuffer` 类的 `compareTo()` 方法用于比较一个缓冲区和另一个缓冲区。两个字符缓冲区是通过按字典顺序比较剩余元素的序列来进行比较的，而不考虑每个序列在其相应缓冲区内的起始位置。像调用 `Char.compare(char, char)` 一样对成对的字符元素进行比较。任何其他类型的对象都不能与 `CharBuffer` 相比。
...
该方法将一个 `CharBuffer` 对象作为参数，与该缓冲区进行比较。
