...
> 原文：[https://www.geeksforgeeks.org/java-util-treemap-pollfirstentry-polllastentry-java/](https://www.geeksforgeeks.org/java-util-treemap-pollfirstentry-polllastentry-java/)

`java.util.TreeMap` 还包含支持在值的高端和低端进行检索和删除的功能，因此在适用性和日常使用方面具有很大的灵活性。这个函数是 `poll()`，在本文中讨论了 2 个变体。

## 1. `pollFirstEntry()`
它**移除并检索一个键值对**，其中键是地图中键值最小的，如果地图为空则返回`null`。
...
## 2. `pollLastEntry()`
它**移除并检索一个键值对**，其中键是地图中最大的键值，如果地图为空则返回`null`。
...
## 实际应用
使用去重或优先排队的概念可以想到很多应用。下面的代码显示了一个这样的例子。
