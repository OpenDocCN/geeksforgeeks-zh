...
> 原文: [https://www.geeksforgeeks.org/identityhashmap-remove-method-in-java/](https://www.geeksforgeeks.org/identityhashmap-remove-method-in-java/)

`java.util.IdentityHashMap.remove()` 是 `IdentityHashMap` 类的内置方法，用于从映射中移除任何特定键的映射。它基本上删除了地图中任何特定键的值。
...
该方法采用一个参数 `key`，其映射将从地图中移除。

## 返回值

该方法返回之前映射到指定键的值，如果该键不存在，则该方法返回 `null`。

下面的程序说明了 `java.util.IdentityHashMap.remove()` 方法的工作方式:
