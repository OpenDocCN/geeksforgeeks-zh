...
> 原文: [https://www.geeksforgeeks.org/gregoriancalendar-add-method-in-java/](https://www.geeksforgeeks.org/gregoriancalendar-add-method-in-java/)

`java.util.GregorianCalendar.add(int calendarfield, int time)` 是 Java 中 `GregorianCalendar` 类的内置方法。该函数接受日历字段和要添加到该特定日历字段的时间量作为参数。根据日历规则，该方法根据其符号将指定的时间量添加或推断到指定的字段。
...
如果 `calendarfield` 具有值 `ZONE_OFFSET`、`DST_OFFSET`，或者未知，或者如果任何日历字段具有超出范围的值，该方法将引发 `IllegalArgumentException`。
...
下面的程序说明了 Java 中 `GregorianCalendar.add()` 方法的使用:
...
[https://docs.oracle.com/javase/7/docs/api/java/util/GregorianCalendar.html#add(int, int)](https://docs.oracle.com/javase/7/docs/api/java/util/GregorianCalendar.html#add(int, int))