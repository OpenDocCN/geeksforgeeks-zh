...
> 原文：[https://www.geeksforgeeks.org/android-networking-bare-skin-understanding-jpost/](https://www.geeksforgeeks.org/android-networking-bare-skin-understanding-jpost/)
...
*   与之前的发布-订阅库相比，它对订阅者的引用是弱的。因此，它不会导致内存泄漏。
*   一条消息可以发送给某些用户。这可以防止事件的发布在不利的位置被接收。因此，减少了应用程序行为异常的可能性。
*   可以使用专用通道来管理用户添加。它减少了无意中订阅用户并接收不需要的通信的可能性。
*   是一个小库（55kb）。因此，应用程序的总大小不受影响。
*   支持同步和异步消息传输和处理。
*   它提供了一种异步运行程序的技术。
...
> **极客提示**：它有一个 `ListView`，可以在垂直列表中显示元素。
...
### 步骤#4：创建一个扩展 `android.app` 的类，名为 `GeeksforGeeks`
...
> **geek tip**：`@SerializedName` 是一个将变量映射到 JSON 键的 JSON 库注释。

### 步骤#6：创建 `GitRepoMsg` 类
...
> *   `@OnUiThread`: it is a comment for Android. For more information about annotation in Android, click [here](https://www.geeksforgeeks.org/android-annotation-processing/).
