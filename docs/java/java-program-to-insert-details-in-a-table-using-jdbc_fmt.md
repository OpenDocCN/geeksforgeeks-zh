...
> 原文: [https://www.geeksforgeeks.org/java-program-to-insert-details-in-a-table-using-jdbc/](https://www.geeksforgeeks.org/java-program-to-insert-details-in-a-table-using-jdbc/)

`JDBC`（Java 数据库连接）基本上是 Java 编程语言和各种数据库（如 Oracle、`SQL`、Postgress 等）之间的标准 `API`（应用程序接口）。它连接前端（用于与用户交互）和后端（用于存储数据）。

## 算法

在 [`JDBC`](https://www.geeksforgeeks.org/jdbc-drivers/) 中搜索/插入/删除/更新。
