...
> 原文: [https://www.geeksforgeeks.org/charset-contains-method-in-java-with-examples/](https://www.geeksforgeeks.org/charset-contains-method-in-java-with-examples/)
...
该功能接受单个强制参数 `second`，指定要检查的字符集。

## 返回值

该函数返回一个布尔值。如果它包含给定的字符集，则返回 `true`，否则返回 `false`。
