...
> 原文：[https://www.geeksforgeeks.org/java-util-bitset-set-method-java/](https://www.geeksforgeeks.org/java-util-bitset-set-method-java/)

`set()` 方法有四个变体。本文描述了所有这些，如下所示：

## 1. `set(int index)`

此方法将指定索引处的位设置为 `true`，即添加一个值。
...
## 2. `set(int num, boolean value)`

此方法将指定索引处的位设置为指定值，如果布尔值为 `true`，则添加值，否则不添加。
...
## 3. `set(int fromnum, int tonum)`

此方法将从指定 `fromnum`（包括）到指定 `tonum`（不包括）的位设置为 `true`，即**将 `fromnum` 的值与 `tonum-1`** 相加。
...
## 4. `set(int fromnum, int tonum, boolean value)`

此方法将指定 `fromnum`（包括）到指定 `tonum`（不包括）的位设置为指定值，即插入 `fromnum` 到 `tonum-1` **如果传递的布尔值为真，否则不插入**。
