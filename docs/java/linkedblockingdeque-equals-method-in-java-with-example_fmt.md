...
`java.util.LinkedBlockingDeque`类的`equals()`方法用于将指定的对象与该`LinkedBlockingDeque`进行比较以获得相等性。当且仅当指定对象也是`LinkedBlockingDeque`，两个`LinkedBlockingDeque`具有相同的大小，并且两个`LinkedBlockingDeque`中所有对应的元素对相等时，返回 true。(两个元素 e1 和 e2 相等，如果(e1==null？e2==null : e1.equals(e2))。)换句话说，如果两个`LinkedBlockingDeque`包含相同顺序的相同元素，则它们被定义为相等。
...
**参数:**该方法将的对象作为参数，与该`LinkedBlockingDeque`进行相等性比较。

**返回值:**如果指定对象等于该`LinkedBlockingDeque`，则该方法返回真。
