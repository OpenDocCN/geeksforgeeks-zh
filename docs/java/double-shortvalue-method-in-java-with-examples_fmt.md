...
> 原文：[https://www.geeksforgeks.org/double-shortvalue-method-in-java-with-examples/](https://www.geeksforgeks.org/double-shortvalue-method-in-java-with-examples/)

## 方法描述
`Double` 类的 `shortValue()` 方法是一个内置方法，用于在类型转换后将调用对象指定的值作为 `short` 返回。
...
## 返回值
它将 `DoubleObject` 的值返回为 `short` 类型。
