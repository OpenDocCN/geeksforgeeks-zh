...
> 原文：[https://www.geeksforgeeks.org/java-program-without-making-class/](https://www.geeksforgeeks.org/java-program-without-making-class/)

## 不做类可以运行 Java 程序吗？

这个想法对我们来说[枚举就是 Java](https://www.geeksforgeeks.org/enum-in-java/)。每个枚举常数总是隐式`public static final`。由于它是`static`的，我们可以通过使用枚举名称来访问它。因为它是`final`的，我们不能创建子枚举。
