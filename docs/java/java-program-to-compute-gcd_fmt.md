...
**注意:**如果 `A`、`B` 是[互素](https://www.geeksforgeeks.org/check-two-numbers-co-prime-not/)，则 `gcd(A, B) = 1`。

**通用方法:**

在计算 GCD 的通用方法中，我们实际上实现了 GCD 的定义。

*   首先，分别找出 `A` 和 `B` 的所有因数。
*   然后列出 `A` 和 `B` 的公因数。
*   这些公因数中最大的就是 `A` 和 `B` 的 GCD。
...
这种方法的原理是，即使我们用 `A` 和 `B` 之间的差替换较大的数字，两个数字 `A` 和 `B` 的 GCD 也将相同。在这种方法中，只要差大于 0，我们就通过用差(`A`，`B`)替换 `B` 来重复对 `A` 和 `B` 执行 GCD 操作。
...
这种方法类似于重复减法。但是，在这种方法中，我们用 `A` 和 `B` 的模代替 `B`，而不是差值。
