...
> 原文: [https://www.geeksforgeeks.org/java-program-to-create-directories-recursively/](https://www.geeksforgeeks.org/java-program-to-create-directories-recursively/)
...
2.  使用 [`createDirectories()`](https://docs.oracle.com/javase/tutorial/essential/io/dirs.html) 的方法（[`java.nio`](https://www.geeksforgeeks.org/introduction-to-java-nio-with-examples/) 包）

## 方法#1

第一种方法是导入 `java.io.File` 类，并定义一个名为 `file()` 的方法，该方法在内部利用 `mkdir()` 函数递归创建目录。`file()` 方法中使用的算法描述如下。
...
2.  该方法采用三个参数：
    *   代表主目录的字符串 `md`。
    *   代表目录结构的字符串 `path`，其中每个字符表示一个新目录。
    *   表示要创建的目录数量的 `int` `depth`。
3.  将终止条件声明为 `(depth == 0)` 返回。
4.  减少每次递归调用的 `depth`。
5.  检查 `path` 字符串的长度是否为 0，并相应地显示消息。
6.  将 `path` 字符串的第一个字符附加到 `md`，并为每个递归调用从 `path` 字符串中移除第一个字符。
7.  用 `md` 作为参数创建 `File` 类的对象。
...
这种方法利用 `java.nio` 包来实现代码。我们在这里部署 `createDirectories()` 方法来创建新的目录。我们还利用 `try-catch` 块来捕获输入输出错误。算法可以在下面找到。

### 算法

1.  按照方法 1 中的算法，重复步骤 1 至 6。
2.  现在，使用 `Paths.get()` 方法将字符串 `md` 转换为 `Path` 实例。
3.  再次，使用 `exists()` 方法检查目录是否已经存在。
4.  如果目录不存在，请打开一个 `try-catch` 块，并使用 `createDirectories()` 方法创建一个新目录。
5.  否则，显示目录已经存在。
6.  进行递归调用。
