# Java 中 LinkedBlockingQueue 的 contains() 方法
...
如果队列包含作为参数传递的对象 `o`，则 `LinkedBlockingQueue` 的 `contains(Object o)` 方法返回 `true`。它返回真，当且仅当，这个队列包含至少一个元素 `e`，它等于作为参数传递的对象 `o`，即 `e.equals(o)`。这个方法对于检查队列是否有元素非常有帮助。
...
该方法取一个强制参数 `o`，即队列中要检查的对象。

## 返回值

如果该队列包含作为参数传递的对象，则该方法返回 `true`。否则返回 `false`。

下面的程序说明了 `contains()` 方法的 `LinkedBlockingQueue` 类：

## 程序 1

检查一个数字队列是否包含数字 `n`。数字 `n` 作为输入传递给 `LinkedBlockingQueue` 的 `contains()`。
...
检查一个员工队列是否包含某个员工 `e`。员工 `e` 的详细信息作为输入传递给 `LinkedBlockingQueue` 的 `contains()`。
