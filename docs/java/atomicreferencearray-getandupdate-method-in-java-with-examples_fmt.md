# Java 中的 AtomicReferenceArray getAndUpdate() 方法示例
...
`AtomicReferenceArray` 类的 `getAndUpdate()` 方法用于原子更新，通过对当前值应用指定的更新函数操作来更新原子引用数组的当前值。它将 `UnaryOperator` 接口的一个对象作为参数，并将该对象中指定的操作应用于当前值。它返回前一个值。
