...
> 原文: [https://www.geeksforgeeks.org/localdate-plusweeks-method-in-java-with-examples/](https://www.geeksforgeeks.org/localdate-plusweeks-method-in-java-with-examples/)
...
该方法接受单个参数 `weeksToAdd`，代表要添加的周，可以是负数。

## 返回值

此方法返回一个基于此日期加上周数的 `LocalDate`，不为空。
