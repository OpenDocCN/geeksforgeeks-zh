...
> 原文：[https://www.geeksforgeeks.org/how-to-integrate-facebook-audience-network-fan-interstitial-ads-in-android/](https://www.geeksforgeeks.org/how-to-integrate-facebook-audience-network-fan-interstitial-ads-in-android/)
...
*   现在在`onCreate()`方法中，初始化`脸书观众网 SDK`。
...
*   在`MainActivity.java`类中创建一个`InterstitialAd`的对象。
...
*   接下来创建一个私有方法`loadInterstitial()`在`onCreate()`方法外并定义它。

```java
private void loadInterstitial()
{
    // 初始化 Interstitial 对象
    // InterstitialAd 初始化构造函数接受 2 个参数
    // 1) 上下文
    // 2) 放置标识
...
*   初始化 SDK 后在`onCreate()`方法中调用`loadInterstitial()`方法。

> **注：** 将`"IMG_16_9_APP_INSTALL#YOUR_PLACEMENT_ID"`替换为自己的 placement id，即可展示真实广告。

*   接下来创建一个方法`showInterstitial()`在`onCreate()`方法外，我们后面称之为显示广告。
...
*   所以接下来的事情就是当用户点击一个 show ad 按钮时调用`showInterstitial()`方法。
*   现在在`onCreate()`方法中为按钮创建一个`点击监听器`并调用`showInterstitial()`。
...
*   现在为间隙广告添加`InterstitialAdListener`，这样用户就知道广告的状态了。要添加`InterstitialAdListener`，打开`loadInterstitial()`方法，并在`fbInterstitialAd.loadAd();`之前添加下面的代码。
...
*   而在`InterstitialAdListener Override`方法中会显示一条[祝酒信息](https://www.geeksforgeeks.org/android-what-is-toast-and-how-to-use-it-with-examples/)，让用户知道广告的状态。以下是`MainActivity.java`文件的完整代码。
