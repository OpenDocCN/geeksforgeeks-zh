...
`org.javatuples` 中的 `getLabel()` 方法用于从标签值类中的 TupleClassObject 获取标签。此方法只能用于 javatuples 库的 `LabelValue` 类对象。它返回一个标签，该标签是标签值类对象的索引 0 处的元素。返回的标签确保类型安全。
