# 修饰符：Java 中的 `isSynchronized(int mod)` 方法示例

> 原文：[https://www.geeksforgeeks.org/modifier-issynchronizedmod-method-in-java-with-examples/](https://www.geeksforgeeks.org/modifier-issynchronizedmod-method-in-java-with-examples/)

`isSynchronized(int mod)` 是 `java.lang.reflect.Modifier` 类的方法，用于检查整数参数是否包含 `synchronized` 修饰符。如果此整数参数表示同步类型修饰符，则方法返回 `true`，否则返回 `false`。
...
## 参数
该方法接受一个整数名称 `mod`，表示一组修饰符。

## 返回值
如果 `mod` 包含同步修改器，则该方法返回 `true`，否则为 `false`。
...
## 参考文献
[https://docs.oracle.com/javase/10/docs/api/java/lang/reflect/Modifier.html#isSynchronized(int)](https://docs.oracle.com/javase/10/docs/api/java/lang/reflect/Modifier.html#isSynchronized(int))