...
> 原文: [https://www.geeksforgeeks.org/java-program-to-access-the-part-of-list-as-list/](https://www.geeksforgeeks.org/java-program-to-access-the-part-of-list-as-list/)

列表是存储在一起形成集合的有序元素序列。列表可以包含重复条目，也可以包含空条目。列表允许我们执行基于索引的操作，即添加、删除、操作和位置访问。Java 提供了一个内置接口 `java.util` 来执行列表以及其他基于类的功能。

**方法:**

1.  维护开始和结束索引的简单方法。
2.  使用 [`subList()`](https://www.geeksforgeeks.org/arraylist-sublist-method-in-java-with-examples/) 方法。
...
> 时间复杂度= O(n)
>
> 空间复杂度= O(n)，其中 n 是列表的大小。

**方法 2:** Java 为我们提供了一个内置的方法 [`subList()`](https://www.geeksforgeeks.org/arraylist-sublist-method-in-java-with-examples/) 来访问属于指定范围的索引值的元素。该方法由数组列表包提供。
...
**返回类型:** 所需元素的列表。该方法访问从 `fromIndex` 到 `toIndex-1` 范围内的所有元素。如果 `fromIndex` 等于 `toIndex`，则返回一个空列表。
