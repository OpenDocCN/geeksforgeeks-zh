...
> 原文：[https://www.geeksforgeeks.org/java-program-to-create-set-of-pairs-using-hashset/](https://www.geeksforgeeks.org/java-program-to-create-set-of-pairs-using-hashset/)
...
从示例中，我们可以清楚地看到重复对仍然存在。这是因为对于集合来说，`Pair` 是同一类型的对象，无法区分。所以为了克服这个问题，我们可以把 `Pair` 做成一串索引和值，以字符串的形式存储在 `Set` 中，然后打印出来，就可以得到所有唯一的对了。
...
因此，从代码和输出中，您可以清楚地看到重复的对被删除，我们可以在不覆盖 [`hashCode()` 方法](https://www.geeksforgeeks.org/equals-hashcode-methods-java/) 和 [`equals()` 方法](https://www.geeksforgeeks.org/compare-two-strings-in-java/) 的情况下获得所有唯一的对。