...
Java 中的[枚举](https://www.geeksforgeeks.org/enum-in-java/)、[`==` vs `equals()`](https://www.geeksforgeeks.org/difference-equals-method-java/)
...
`equals()` 方法在内部使用 `==` 运算符来检查两个枚举是否相等。这意味着，您可以使用 `==` 和 `equals()` 方法来比较枚举。但问题是，如果我们有两种方法来比较 `enum` 的两个成员，那么哪一种更好呢？

`==` 运算符和 `equals()` 方法都仅用于比较，但区别不大：
...
**解释：** 在上面的程序中，我们正在比较一个枚举成员和一个 `null`。当我们使用 `==` 运算符进行比较时，我们不会得到任何异常，而当我们使用 `equals()` 方法时，我们得到的是 `NullPointerException`。

因此我们可以得出结论，当我们使用 `==` 运算符进行枚举成员比较时，它不会抛出任何异常，但是 `equals()` 方法会引发异常。
...
**说明：** 在上面的程序中，我们正在比较一个枚举成员和另一个枚举成员。当我们使用 `equals()` 方法进行比较时，我们不会得到任何编译失败（CF），而当我们使用 `==` 运算符时，我们会得到编译失败（CF），表示“无与伦比的类型”。

因此，我们可以得出结论，当我们使用 `==` 运算符进行枚举成员比较时，它会在编译时执行类型兼容性检查。`equals()` 方法永远不会担心这两个参数的类型（对于不可比较的类型，它只会给出 `false`）。