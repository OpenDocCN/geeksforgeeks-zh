# Duration.compareTo(Duration) 方法示例
...
`java.time` 包中 `Duration` 类的 `compareTo(Duration)` 方法用于将该持续时间与作为参数传递的持续时间进行比较。
...
该方法返回一个 `int` 值，其中负值表示其他持续时间大于该值，零表示其他持续时间等于该持续时间，正值表示该持续时间大于其他持续时间。
...
[Oracle 文档](https://docs.oracle.com/javase/9/docs/api/java/time/Duration.html#compareTo-java.time.Duration-)