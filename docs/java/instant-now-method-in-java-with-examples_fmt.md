# Java 中的 `Instant.now()` 方法示例

> 原文：[https://www.geeksforgeeks.org/instant-now-method-in-java-with-examples/](https://www.geeksforgeeks.org/instant-now-method-in-java-with-examples/)

在 `Instant` 类中，根据传递给它的参数，有两种 `now()` 方法。

## `now()`

`now()` 是 `Instant` 类的一个方法，用于从系统 UTC 时钟获取当前的 `Instant`。此方法将根据系统协调世界时时钟返回即时消息。
...
`now(Clock clock)` 是 `Instant` 类的一个方法，用于根据作为参数传递的指定时钟返回当前的 `Instant`。
