...
> 原文：[https://www.geeksforgeeks.org/java-program-to-blur-image-using-smoothing/](https://www.geeksforgeeks.org/java-program-to-blur-image-using-smoothing/)
...
*   要读写图像文件，我们必须导入`File`类 [`import java.io.File;`]。这个类通常表示文件和目录路径名。
*   为了处理错误，我们使用`IOException`类 [`import java.io.IOException;`]。
*   为了保存图像，我们使用`BufferedImage`类 [`import java.awt.image.BufferedImage;`]。该对象用于在内存中存储图像。
*   为了执行图像读写操作，我们将导入`ImageIO`类 [`import javax.imageio.ImageIO;`]。这个类有读写图像的静态方法。

## 方法

*   以 2D 矩阵平滑图像的每个像素。
*   使用`BufferedImage`类和`Color`的一些内置方法。
