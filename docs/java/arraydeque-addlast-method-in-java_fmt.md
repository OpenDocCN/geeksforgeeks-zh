...
> 原文: [https://www.geeksforgeeks.org/arraydeque-addlast-method-in-java/](https://www.geeksforgeeks.org/arraydeque-addlast-method-in-java/)

`java.util.ArrayDeque.addLast(Object element)`方法是用来在这个 deque 的末尾插入一个特定的元素。它类似于 Java 中的 `add()`方法。

## 语法:
```java
ArrayDeque.addLast(Object element)
```

## 参数:
参数 `element` 属于数组类型，指的是要添加的元素。
