...
> 原文：[https://www.geeksforgeeks.org/how-to-return-an-array-in-java/](https://www.geeksforgeeks.org/how-to-return-an-array-in-java/)
...
返回数组的方法的返回类型必须与被返回数组的数据类型相同。返回类型可能是通常的整数、双精度、字符、字符串或用户定义的类对象。
...
考虑到方法和返回数组的用法，必须准确使用访问修饰符。静态和非静态声明也必须考虑在内。
...
在方法调用中必须有任何相同数据类型或类似的变量数组来处理返回的数组。例如，从任何方法返回的整数数组都可以存储如下。
