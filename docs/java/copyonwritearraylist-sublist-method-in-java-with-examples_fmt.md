...
> 原文: [https://www.geeksforgeeks.org/copyonwritearraylist-sublist-method-in-java-with-examples/](https://www.geeksforgeeks.org/copyonwritearraylist-sublist-method-in-java-with-examples/)

`CopyOnWriteArrayList`类的`subList()`方法用于返回列表中指定的`fromIndex`（包含）和`toIndex`（不包含）之间的部分的视图。（如果`fromIndex`和`toIndex`相等，则返回的列表为空。）
...
## 异常:
如果端点索引值超出范围（从`index < 0`或`toIndex > size()`开始），此方法将抛出`IndexOutOfBoundsException`。
