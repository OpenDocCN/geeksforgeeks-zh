...
> 原文: [https://www.geeksforgeeks.org/bigdecimal-movepointleft-method-in-java/](https://www.geeksforgeeks.org/bigdecimal-movepointleft-method-in-java/)

**先决条件:** [大十进制基础知识](https://www.geeksforgeeks.org/bigdecimal-class-java/)

`BigDecimal.movePointLeft(int n)`方法用于将当前`BigDecimal`的小数点向左移动`n`位。
...
该方法取整数型的一个参数`n`，指小数点需要左移的位数。

## 返回值

方法返回相同的`BigDecimal`值，小数点左移`n`位。
...
[https://docs.oracle.com/javase/7/docs/api/java/math/BigDecimal.html#movePointLeft(int)](https://docs.oracle.com/javase/7/docs/api/java/math/BigDecimal.html#movePointLeft(int))