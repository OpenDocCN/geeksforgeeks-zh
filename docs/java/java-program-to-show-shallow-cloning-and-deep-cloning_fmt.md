...
> 原文：[https://www.geeksforgeeks.org/java-program-to-show-shallow-cloning-and-deep-cloning/](https://www.geeksforgeeks.org/java-program-to-show-shallow-cloning-and-deep-cloning/)
...
一般来说，深度复制和克隆被称为相同的深度克隆，因为它们之间的区别是一条细线，焦点放在该细线上，以便于使用 [`clone()`](https://www.geeksforgeeks.org/clone-method-in-java-2/) 方法手动复制对象。
...
*   [`clone()`](https://www.geeksforgeeks.org/clone-method-in-java-2/) 方法的对象类支持对象的浅拷贝。如果对象在浅拷贝中包含基元以及非基元或引用类型变量，则克隆的对象也引用原始对象引用的同一对象，因为只有对象引用被拷贝，而不是被引用的对象本身。简单来说，这里创建了一个对象，而堆栈内存中有两个引用。
