...
> 原文：[https://www.geeksforgeeks.org/java-program-to-find-the-index-of-the-treeset-element/](https://www.geeksforgeeks.org/java-program-to-find-the-index-of-the-treeset-element/)

与像`ArrayList`或`LinkedList`这样的列表类不同，`TreeSet`类不允许使用索引访问元素。没有使用索引访问`TreeSet`元素的直接方法，因此找到元素的索引并不简单。
...
2.  使用`Iterator`
3.  使用`TreeSet`类的 [`headSet()`](https://www.geeksforgeeks.org/treeset-headset-method-in-java/) 方法

### 方法 1：通过将`TreeSet`转换为`List`

像`ArrayList`或`LinkedList`这样的`List`提供了 [`indexOf()`](https://www.geeksforgeeks.org/list-indexof-method-in-java-with-examples/) 方法来查找元素索引。我们可以将`TreeSet`转换为`ArrayList`，然后使用`indexOf()`方法。此方法返回此列表中指定元素的第一个匹配项的索引，如果此列表不包含该元素，则返回-1。
...
1.  使用`iterator()`方法获取`TreeSet`元素的迭代器。
2.  一旦获取了迭代器，迭代元素并根据要求从用户或自定义输入中搜索指定的元素，如下所示，以便理解。
...
### 方法 3：使用`TreeSet`类的 [`headSet()`](https://www.geeksforgeeks.org/treeset-headset-method-in-java/) 方法

`TreeSet`类的 [`headSet()`](https://www.geeksforgeeks.org/treeset-headset-method-in-java/) 方法返回元素少于指定元素的部分`TreeSet`的视图。由于`TreeSet`的元素会按照元素的自然顺序或通过自定义比较器自动排序，因此`headSet`的大小将等于比指定元素小或相等的元素数量。如果我们将`TreeSet`元素放在一个列表中，那么这个数字将等于元素的索引。

**说明：**

> 如果`TreeSet`包含`[1, 2, 3, 4]`，那么元素`3`的`headSet`将包含元素`[1, 2]`。`headSet`的大小将是`2`，这将是元素`3`的索引。因此，如果我们得到`headSet`的大小，那么它将等于我们必须找到索引的元素的位置。
