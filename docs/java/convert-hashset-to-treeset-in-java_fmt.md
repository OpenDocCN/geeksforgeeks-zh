...
> 原文：[https://www.geeksforgeeks.org/convert-hashset-to-treeset-in-java/](https://www.geeksforgeeks.org/convert-hashset-to-treeset-in-java/)

`HashSet`：`HashSet` 在 Java 中一般用于搜索、插入、删除等操作。这些操作平均需要恒定的时间。`HashSet` 比 `TreeSet` 快。`HashSet` 是使用哈希表实现的。

`TreeSet`：Java 中的 `TreeSet` 取 O(log n) 进行搜索、插入、删除，比 `HashSet` 高。但是 `TreeSet` 会保留已排序的数据。此外，它还支持 `higher()`（返回最少更高的元素）、`floor()`、`ceiling()` 等操作。这些操作在 `TreeSet` 中也是 O(log n)，在 `HashSet` 中不支持。使用自平衡二叉查找树（红黑树）实现树集。`TreeSet` 由 Java 中的 `TreeMap` 支持。
...
### 1. 通过调用带参构造函数并将 HashSet 对象作为参数传递
...
### 2. 通过使用 addAll 方法构造包含相同元素的 TreeSet
...
### 3. 通过使用 for-each 循环（此方法常用于两种不兼容类型之间的转换）
