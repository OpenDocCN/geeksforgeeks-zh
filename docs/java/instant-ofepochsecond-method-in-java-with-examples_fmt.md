...
> 原文：[https://www.geeksforgeeks.org/instant-ofepochsecond-method-in-java-with-examples/](https://www.geeksforgeeks.org/instant-ofepochsecond-method-in-java-with-examples/)
...
**返回值：** 该方法以秒为参数返回 `Instant`。

**异常：** 如果瞬间超过最大或最小瞬间，该方法抛出以下异常 `DateTimeException`。
...
一个 `Instant` 类的方法，用于返回 `Instant` 的一个实例，使用秒作为参数传递给从 1970-01-01T00:00:00Z 的纪元开始计算的方法，纳秒的分数秒也作为参数传递，这将改变秒和纳秒的值，以确保存储的纳秒在 0 到 999,999,999 的范围内。
...
**参考文献：**
[https://docs.oracle.com/javase/10/docs/api/java/time/Instant.html#ofEpochSecond(long, long)](https://docs.oracle.com/javase/10/docs/api/java/time/Instant.html#ofEpochSecond(long, long))
[https://docs.oracle.com/javase/10/docs/api/java/time/Instant.html#ofEpochSecond(long)](https://docs.oracle.com/javase/10/docs/api/java/time/Instant.html#ofEpochSecond(long))