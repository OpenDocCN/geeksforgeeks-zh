# AbstractSet contains() 方法详解（Java 示例）

> 原文：[https://www.geeksforgeks.org/abstractset-contains-method-in-java-with-example/](https://www.geeksforgeks.org/abstractset-contains-method-in-java-with-example/)

Java `AbstractSet` 的 `contains()` 方法用于检查集合中是否存在元素。它将元素作为参数，如果元素存在于集合中，则返回 `true`。
...
参数 `element` 为 `Set` 类型。该参数是指需要在集合中检查其出现的元素。
