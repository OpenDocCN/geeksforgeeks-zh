...
> 原文: [https://www.geeksforgeeks.org/doublebuffer-asreadonlybuffer-method-in-java-with-examples/](https://www.geeksforgeeks.org/doublebuffer-asreadonlybuffer-method-in-java-with-examples/)

`DoubleBuffer`类的`asReadOnlyBuffer()`方法用于用这个缓冲区的内容创建一个新的只读双缓冲区。新缓冲区是该缓冲区的副本。因此，对此缓冲区内容所做的更改将在新缓冲区中可见。
