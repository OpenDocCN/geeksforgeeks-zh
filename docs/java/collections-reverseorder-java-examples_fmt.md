# Collections.reverseOrder()方法详解（Java示例）

> 原文：[https://www.geeksforgeeks.org/collections-reverseorder-java-examples/](https://www.geeksforgeeks.org/collections-reverseorder-java-examples/)

[`Collections`](https://www.geeksforgeeks.org/collections-class-in-java/)类的`reverseOrder()`方法本身存在于 [`java.util`](https://www.geeksforgeeks.org/java-util-package-java/)包中，它返回一个比较器，使用这个比较器我们可以按相反的顺序对`Collection`进行排序。自然排序是由对象自己的`compareTo`方法强加的排序。
...
## 参数
一个比较器，其顺序将被返回的比较器反转（也可以为空）。

## 返回类型
一个比较器，对实现[`Comparable`](https://www.geeksforgeeks.org/comparator-interface-java/)接口的对象集合施加与自然顺序相反的顺序。
...
> **提示**：但是对于像`Integer`数组这样的“对象数组”来说很好，但是对于像`int`数组这样的原始数组就不行了。
