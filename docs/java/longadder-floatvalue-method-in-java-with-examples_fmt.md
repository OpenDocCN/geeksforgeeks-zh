...
> 原文: [https://www.geeksforgeeks.org/longadder-floatvalue-method-in-java-with-examples/](https://www.geeksforgeeks.org/longadder-floatvalue-method-in-java-with-examples/)

`LongAdder`类创建一个初始和为零的新加法器。`LongAdder.floatValue()` 是 Java 中的一个内置方法，它将总和作为浮点值返回。创建类的对象时，其初始值为零。
...
**返回值:** 该方法返回一个`float`值，它代表这个长加法器的和。
