# Java 中 BreakIterator following() 方法示例

> 原文：[https://www.geeksforgeeks.org/breakiterator-following-method-in-java-with-examples/](https://www.geeksforgeeks.org/breakiterator-following-method-in-java-with-examples/)

`java.text.BreakIterator` 类的 `following()` 方法用于返回文本行中指定偏移量之后出现的第一个边界的索引。它提供下一个边界的第一个字符的偏移量，下一个边界跟随传递的偏移量的边界。
...
该方法将 `offset` 作为参数，必须找到第一个参数之后的所需边界。
...
如果偏移量小于第一个边界且大于最后一个边界，该方法抛出 `IllegalArgumentException`。

以下是举例说明 `following()` 方法：
