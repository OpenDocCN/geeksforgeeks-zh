...
> 原文：[https://www.geeksforgeeks.org/atomicinteger-for-lock-free-algorithms-in-java/](https://www.geeksforgeeks.org/atomicinteger-for-lock-free-algorithms-in-java/)
...
我们需要编写线程安全的应用程序，但同时，它们应该给我们带来高性能和并发执行的好处。因此，为了最大限度地减少同步的使用并降低锁的复杂性，Java 在 `java.util.concurrent` 包中有一整套类，它提供了许多无锁和原子的高级操作。
...
现在，为了将上面的例子转换成一个简单的无锁示例，我们使用了 Java 的 `AtomicInteger` 类，它是 `java.util.concurrent.atomic` 包的一部分。这是一个非常有用的类，可以很容易地在并发应用程序中使用，如下所示：

## Java 代码示例（使用 AtomicInteger）
