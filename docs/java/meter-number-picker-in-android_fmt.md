...
> 原文：[https://www.geeksforgeeks.org/meter-number-picker-in-android/](https://www.geeksforgeeks.org/meter-number-picker-in-android/)
...
| `MNP_padding垂直` | 用于给小部件提供从顶部和底部的填充 |
...
### 第二步：在 build.gradle 文件中添加电表编号选择器库的依赖关系

首先导航到渐变脚本，然后导航到 `build.gradle` (项目)级别。在 `allprojects{}` 部分添加下面给出的行。

> mavenuentral _)

然后导航到分级脚本，然后导航到 `build.gradle` (模块)级别。在依赖项部分的 `build.gradle` 文件中添加以下行。

> 实现' com . Alex-zait sev:meter number picker:1 . 0 . 5 '

现在点击 `立即同步`，它将同步你在 `build.gradle()` 中的所有文件。

### 第三步：在 themes.xml 文件中添加样式代码
...
### 第四步：在你的 activity_main.xml 文件中创建新的 ShadowImageView
