...
> 原文：[https://www.geeksforgeeks.org/bytebuffer-putshort-methods-in-java-with-examples/](https://www.geeksforgeeks.org/bytebuffer-putshort-methods-in-java-with-examples/)

### `putShort(int value)`

`java.nio.ByteBuffer` 类的 `putShort(int value)` 方法用于将包含给定 short 值的两个字节按当前字节顺序写入当前位置的缓冲区，然后将该位置增加 2。
...
*   `BufferOverflowException` - If the current position of this buffer is not less than its limit
*   `ReadOnlyBufferException` - If this buffer is read-only.

下面是举例说明 `putShort()` 方法的例子：
