...
> 原文：[https://www.geeksforgeeks.org/how-to-sort-arraylist-using-comparator/](https://www.geeksforgeeks.org/how-to-sort-arraylist-using-comparator/)

[`Comparator`](https://www.geeksforgeeks.org/comparator-interface-java/)是一个接口，用于对[`ArrayList`](https://www.geeksforgeeks.org/arraylist-in-java/)进行排序和重新排列。比较器用于对用户定义对象的`ArrayList`进行排序。在Java中，比较器在[`java.util`](https://www.geeksforgeeks.org/java-util-package-java/)包中提供。使用比较器，我们可以根据多个变量对`ArrayList`进行排序。我们可以简单地实现`Comparator`，而不影响原始的用户定义类。要使用比较器对`ArrayList`进行排序，我们需要重写`Comparator`接口提供的`compare()`方法。重写`compare()`方法后，我们需要调用`Collections.sort()`方法，如下所示。
...
在上面的例子中，我们按照可用库存数量对`Shop`类进行排序。我们也可以根据名称和产品编号进行排序。让我们根据名称对上面的`ArrayList`进行排序。
