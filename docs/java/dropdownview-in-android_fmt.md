...
`DropDownView`是大多数安卓应用中使用的另一个令人兴奋的功能。这是以动画形式表示菜单和其他选项的独特方式。我们可以在`DropDownView`的一个标题下看到选项列表。在本文中，我们将看到如何在安卓中实现`DropDownView`。下面给出了一个GIF示例，来了解一下我们将在本文中做什么。注意，我们将使用`Java`语言来实现这个项目。
...
| --- | --- |
| `layout_width` | 赋予布局宽度。 |
| `layout_height` | 给布局赋予高度。 |
| `containerBackgroundColor` | 给容器赋予背景色。 |
| `overlayColor` | 给覆盖颜色。 |
...
将JitPack存储库添加到构建文件中。将其添加到`allprojects{ }`部分内存储库末尾的`root build.gradle`中。
...
导航到`app > res > layout > 右键单击 > New > Layout Resource File`，并将文件命名为`header`和`footer`。
...
现在点击`运行`选项。`Build Gradle`需要一些时间。之后，您将获得如下所示的设备输出。
