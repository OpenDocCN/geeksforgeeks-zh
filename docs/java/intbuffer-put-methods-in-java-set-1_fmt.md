# Java 中的 IntBuffer put()方法

> 原文: [https://www.geeksforgeks.org/intbuffer-put-methods-in-java-set-1/](https://www.geeksforgeks.org/intbuffer-put-methods-in-java-set-1/)

## put(int i)

`java.nio.IntBuffer` 类的 `put(int i)` 方法用于将给定的 `int` 写入当前位置新创建的 `IntBuffer`，然后递增该位置。
...
该方法将 `int` 值 `i` 作为参数写入 `IntBuffer`。

### 返回值

这个方法返回这个缓冲区，其中插入了 `int` 值。
