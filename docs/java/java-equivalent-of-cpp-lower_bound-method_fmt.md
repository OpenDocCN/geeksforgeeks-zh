...
> 原文：[https://www.geeksforgeeks.org/java-equivalent-of-cpp-lower_bound-method/](https://www.geeksforgeeks.org/java-equivalent-of-cpp-lower_bound-method/)

C++的`lower_bound()`方法返回`数组`中第一个元素的索引，其值不小于`key`。这意味着该函数返回仅大于或等于下一个最小数字的索引。如果有多个值等于该数，`lower_bound()`将返回第一个值的索引。
...
现在让我们讨论一下方法，以便使用`lower_bound()`方法来获得仅大于或等于该数的下一个最小数的索引。
...
2.  迭代使用二分搜索法
3.  递归使用二分搜索法
4.  使用`Arrays`实用类的`binarySearch()`方法

### 方法 1：使用线性搜索

#### 进场

我们可以用线性搜索来寻找下界。我们将从第 0 个索引开始迭代数组，直到找到一个等于或大于`key`的值。
...
1.  将`low`初始化为 0，`high`初始化为`n`。
2.  将`key`与中间元素进行比较(`arr[mid]`)。
3.  如果中间元素大于或等于`key`，则将`high`值更新为中间索引(`mid`)。
4.  否则更新`low`至`mid + 1`。
5.  重复步骤 2 至步骤 4，直到`low`小于`high`。
6.  经过以上所有步骤后，`low`就是给定数组中某个`key`的下限。
...
### 方法 4：使用`Arrays`实用类的`binarySearch()`方法

我们还可以使用`Arrays`实用类（或`Collections`实用类）的内置二分搜索法实现。函数返回搜索关键字的索引，如果它包含在数组中；否则，`(-(插入点) - 1)`。插入点被定义为将键插入数组的点。

#### 进场

1.  在排序的数组中搜索`key`的索引，返回该`key`的索引，因为它的正值存在于数组中，否则，返回指定的负值。
2.  排序数组中应该添加`key`的位置。
3.  现在，如果该`key`出现在数组中，我们向左移动以找到它的第一次出现，否则递减索引以找到该`key`的第一次出现。
4.  在应用二分搜索法之前对数组进行排序。
5.  打印出来。
