...
> 原文: [https://www.geeksforgeeks.org/java-program-to-output-query-results-in-tabular-format-in-jdbc/](https://www.geeksforgeeks.org/java-program-to-output-query-results-in-tabular-format-in-jdbc/)
...
3.  以表格格式输出结果。
    ...
    *   单击“源”选项卡并导入以下库
    *   下载一个 `rs2xml.jar` 文件。如果单独的 JAR 文件不起作用，可以额外导入 `DbUtils`。[下载链接](https://hacksmile.com/rs2xml-jar-free-download/)
...
#### 3.4 下载一个 `rs2xml.jar` 文件。如果单独的 jar 文件不起作用，另外导入 `DbUtils`。现在，只需按照以下步骤解压缩并上传 jar 文件:
