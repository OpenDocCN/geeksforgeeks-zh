...
> 原文: [https://www.geeksforgeeks.org/creating-multiple-pools-of-objects-of-variable-size-in-java/](https://www.geeksforgeeks.org/creating-multiple-pools-of-objects-of-variable-size-in-java/)
...
有几个对象的新对象的生产成本似乎略高，因为它们不被认为是轻量级的。数据库连接对象、解析器对象、线程生成等等都是例子。在任何应用程序中，我们可能都必须构建许多这样的对象池。因为这种对象的生成是昂贵，它将对任何程序的性能产生负面影响。我们可以再次使用同一个对象。
...
*   线程类型的 `Thread myThreads[]` 是一个数组引用，将引用线程类对象(线程池数组)
*   布尔类型的 `isUsed[]` 是一个数组，用于跟踪当前正在使用的线程对象
*   用于记录线程池数量的 `noOfThreadPools` &是一个类级属性
*   `ThreadreferenceCount` 用于记录特定池中线程引用的数量
*   `poolNum`，用于存储为每个创建的池指定的唯一编号
...
class Thread {
    ...
    // Constructor that initializes the objects of Thread class
...
class ThreadPool {
    ...
    // To keep true whenever 1st object of a pool is created,
    ...
    public boolean initialize(int size)
    {
        ...
        if (true == isFirst) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                // we are assigning thread name to each thread
            ...
            for (int i = 0; i < n; i++) {
                ...
                // any of the objects created (from o to size-1)
                isUsed[i] = false;
            }
        ...
        if (ThreadreferenceCount > 0) {
            for (int i = 0; i < n; i++) {
                ...
                if (true == thread.compare(myThreads[i])) {
                    ...
                    // reference from outside the class is referring to
                    // object myThread[i]
                    isUsed[i] = false;
    ...
    public boolean deInitialize()
    {
        ...
        // deinitialize() even if the references are present outside
...
public class Main {
    ...
    public static void main(String[] args)
    {
        ...
        // Creating object of class 2 in class 3 main() method
        ...
        // Now getting 2 references to the 2 Objects from this pool
```
...
if (t1 != null) {
    ...
    System.out.println(" t1 do not refer any object");
}
...
if (t2 != null) {
    ...
    System.out.println(" t2 do not refer any object");
}
...
if (t3 != null) {
    ...
    System.out.println(" t3 do not refer any object");
}
...
if (tp1 != null) {
    ...
    System.out.println("tp1 dont refer to any object");
}
