# Java 中的 ChronoLocalDateTime query() 方法示例
...
`query()` 是 `ChronoLocalDateTime` 接口的一个方法，用于使用指定的查询作为参数来查询这个 `ChronoLocalDateTime`。作为参数传递的 `TemporalQuery` 对象定义了用于从该 `ChronoLocalDateTime` 获取结果的逻辑。
...
[https://docs.oracle.com/javase/9/docs/api/java/time/chrono/ChronoLocalDateTime.html#query-java.time.temporal.TemporalQuery-](https://docs.oracle.com/javase/9/docs/api/java/time/chrono/ChronoLocalDateTime.html#query-java.time.temporal.TemporalQuery-)