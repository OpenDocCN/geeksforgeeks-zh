...
`IntStream skip()` 是一个**有状态的中间操作**，也就是说，在处理新元素时，它可能会合并来自之前看到的元素的状态。
...
`IntStream` 是一个原始的 `int` 值元素序列。这是[流](https://www.geeksforgeeks.org/stream-in-java/)的内部原始特化。`n` 是要跳过的主导元素数量。
