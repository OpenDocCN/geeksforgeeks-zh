...
> 原文: [https://www.geeksforgeeks.org/abstractqueue-addall-method-in-java-with-examples/](https://www.geeksforgeeks.org/abstractqueue-addall-method-in-java-with-examples/)

## `AbstractQueue` 的 `addAll(E e)` 方法
...
该方法接受一个强制参数 `Collection c`，该集合包含要添加到该队列的元素。

### 返回

如果该队列由于调用而改变，则该方法返回 `true`。
