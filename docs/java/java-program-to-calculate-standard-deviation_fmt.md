...
> 原文：[https://www.geeksforgeeks.org/java-program-to-calculate-standard-deviation/](https://www.geeksforgeeks.org/java-program-to-calculate-standard-deviation/)
...
7.  之后，平均值将通过 `mean = sum / n` 来计算，其中 `n` 是数组中的元素数量。
8.  现在，标准偏差将在平均值的帮助下计算，这是通过再次迭代 `for` 循环并在数学预定义方法（如 `Math.pow` 和 `Math.sqrt`）的帮助下完成的。
9.  之后，该类将返回该值，然后打印。
