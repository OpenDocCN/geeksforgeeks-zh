...
Java 中`ChronoPeriod`接口的`plus()`方法是将给定的周期量加到指定的周期上。该功能在年、月和日分别运行。
...
此功能接受单个参数`amountToAdd`，即`TemporalAmount`添加至期间。它不能为空。

## 返回值

该函数返回一个基于给定周期的`ChronoPeriod`，加上请求的周期，该值不能为空。
...
[https://docs.oracle.com/javase/9/docs/api/java/time/chrono/ChronoPeriod.html#plus-java.time.temporal.TemporalAmount-](https://docs.oracle.com/javase/9/docs/api/java/time/chrono/ChronoPeriod.html#plus-java.time.temporal.TemporalAmount-)