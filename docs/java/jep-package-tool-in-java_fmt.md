...
JEP 包工具是在 Java 14 中作为孵化工具引入的。直到 Java 15 之前，它一直是一个孵化工具。它将 Java 应用程序打包成特定于平台的特性。基本上，这个工具将我们的 Java 源文件转换成可执行文件。在 Windows 中，可执行文件有两种类型。在 Ubuntu 中，有 `.rpm` 和 `.deb`。JEP 包工具将 Java 源文件转换成这些文件。
...
在你的 IDE 中创建一个 Maven 项目，并创建一个新的 Java 类，众所周知。

![](img/1c2643311e1770fa6db0490377c31848.png)

### 步骤 2

将以下代码插入下面提供的 Java 文件。

**示例 1:** Java 应用程序

```java
// Java Program to be Associated with JAR File to
// Illustrate JEP Package Tool

// Importing all classes from java.Swing package
...
现在我们需要定义 JAR 文件名，我们将在其中包装类文件和相关配置。它包含在如下所示的构建路径中：
