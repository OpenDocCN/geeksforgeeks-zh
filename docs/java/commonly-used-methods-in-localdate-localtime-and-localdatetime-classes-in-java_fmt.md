...
> 原文：[https://www.geeksforgeeks.org/commonly-used-methods-in-localdate-localtime-and-localdatetime-classes-in-java/](https://www.geeksforgeeks.org/commonly-used-methods-in-localdate-localtime-and-localdatetime-classes-in-java/)

Java 提供了与日期相关的三个最重要的类，即 `LocalDate`、`LocalTime`、`LocalDateTime`，这使得在 Java 中处理日期变得非常容易。为了使用这些类，我们需要导入 `java.time` 包，该包是日期、时间、瞬间和持续时间的主要 API。
...
| 类 | 描述 |
| --- | --- |
| `LocalDate` | 本地日期类只能保存日期。例如，假设是 2021-02-28 |
| `LocalTime` | 本地时间类只能保存时间。例如，假设是 19:32:25.457826 |
| `LocalDateTime` | 本地日期时间类保存日期和时间。比如说是 2021-02-28T19:32:25.457826。在这种格式中，T 之前是日期，T 之后是时间。 |

一般来说，导入包中所有不必要的文件被认为是不好的做法。因此，我们应该只导入需要的类。这些类中有各种方法可以根据类处理日期和时间。让我们讨论被称为 `LocalDate` 类中最常见的方法，即 `now()` 方法。
...
| 方法 | 描述 |
| --- | --- |
| `getDayOfMonth()` | 返回当前月份中的某一天。例如，假设是 28。 |
| `getDayOfWeek()` | 返回星期几。例如，假设是 SUNDAY。 |
| `getDayOfYear()` | 返回一年中的第几天。假设是 59。 |
| `getMonth()` | 返回月份的名称。例如，假设是 FEBRUARY。 |
| `getMonthValue()` | 返回月份的数值。例如，假设是 2。 |
| `isLeapYear()` | 返回一个布尔值（true/false）。例如，假设是 false。 |
| `lengthOfYear()` | 返回当前年份的天数。例如，假设是 365。 |
| `lengthOfMonth()` | 返回当前月份的天数。例如，假设是 28。 |
| `plusDays(天数)` | 将指定天数加到当前日期并返回新日期。 |
| `plusMonths(月数)` | 将指定月数加到当前日期并返回新日期。 |
| `plusYears(年数)` | 将指定年数加到当前日期并返回新日期。 |

类似的方法也可以用来减去日期：`minusDays()`、`minusMonths()`、`minusYears()`。这些函数也可用于 `LocalDateTime` 类，增加了一些与时间相关的函数，比如 `plusHours()`、`plusMinutes()`、`plusSeconds()`、`plusNanos()` 等等。
