...
> 原文：[https://www.geeksforgeeks.org/java-program-to-add-two-numbers-without-using-arithmetic-operator/](https://www.geeksforgeeks.org/java-program-to-add-two-numbers-without-using-arithmetic-operator/)

这里，我们需要写一个函数，返回两个指定整数的和。并且该函数不能使用任何算术运算符，如`++`、`--`、`-`等等。这将是一个非常基本的初级程序，通过简单地使用`+`运算符计算总和，从而简单地打印和显示结果。但是如果我们不约束不使用任何算术运算符，那么只有一种方法，那就是使用`异或`运算符，它可以对两个给定的位执行加法运算。对于进位位，我们可以使用`和` (`&`)运算符。

使用`位操作符`完成。这里我们将主要使用以下三种操作：
