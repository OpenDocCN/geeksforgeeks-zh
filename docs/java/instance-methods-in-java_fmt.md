...
> 原文：`https://www.geeksforgeeks.org/instance-methods-in-java/`

实例方法是执行特定任务的一组代码。有时程序的大小会增加，我们希望将`main`方法的逻辑与其他方法分开。方法是在类内部编写的函数。由于 java 是一种面向对象的编程语言，我们需要在一些类中编写一个方法。
...
您不能直接调用静态方法中的实例方法，因此可以使用类的对象调用实例方法。我们知道 java 程序的执行是从`main`方法开始的，`main`方法是`static`的，所以不能直接调用`instance`方法。我们必须创建类对象；然后，我们可以在`main`方法中调用`instance`方法。
