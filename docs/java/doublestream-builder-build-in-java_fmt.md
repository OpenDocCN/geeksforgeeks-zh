...
> 原文：`https://www.geeksforgeeks.org/doublestream-builder-build-in-java/`

`DoubleStream.Builder` 的 `build()` 方法用于构建流，将该构建器转换到*构建状态*。
...
一个流构建器有一个生命周期，从*构建阶段*开始，在此期间可以添加元素，然后过渡到*构建阶段*，之后便不能再添加元素。构建阶段在调用 `build()` 方法时开始，该方法创建一个有序流，其元素是按照添加顺序添加到流构建器中的元素。
...
在调用 `build()` 方法后尝试在其中添加元素（当流处于构建阶段时）。
