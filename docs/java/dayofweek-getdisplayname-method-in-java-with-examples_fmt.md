...
> 原文: [https://www.geeksforgeeks.org/dayofweek-getdisplayname-method-in-java-with-examples/](https://www.geeksforgeeks.org/dayofweek-getdisplayname-method-in-java-with-examples/)
...
*   `样式` – 是 `TextStyle`，可以是“FULL”、“SHORT”和“窄带”三个元素。
*   `地区` – 代表世界上特定的语言和地区。默认区域设置是美国。
*   `dayOfWeekObject` – 是 `DayOfWeek` 的一个实例。

## 返回值:
函数返回根据指定的 `Locale` 类参数和 `TextStyle` 返回星期几的文本表示。
