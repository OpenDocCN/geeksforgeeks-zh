# 如何在安卓中添加淡入淡出的 TextView 动画

> 原文：[https://www.geeksforgeeks.org/how-to-add-fading-textview-animation-in-android/](https://www.geeksforgeeks.org/how-to-add-fading-textview-animation-in-android/)

`TextView` 是用户界面组件的基本构建模块。它用于设置文本并将其显示给用户。它是一个非常基本的组件，使用了很多。

一个**渐变文本视图**是一个每几秒钟自动改变其内容的文本视图。如果我们想设计一个更漂亮的界面，可以使用衰落文本视图。

**进场:**

1.  将此添加到您的根 `build.gradle` 文件中（不是您的模块 `build.gradle` 文件）：
...
2.  在模块的 `build.gradle` 文件中添加支持库，并在依赖项部分添加依赖项。
...
3.  现在在 `activity_main.xml` 文件中添加以下代码。
...
4.  现在在 `MainActivity.java` 文件中添加以下代码。
