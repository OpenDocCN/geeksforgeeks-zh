...
> 原文：`https://www.geeksforgeeks.org/concurrentskiplistset-polllast-method-in-java/`

`java.util.concurrent.ConcurrentSkipListSet`的`pollLast()`方法是 Java 中的一个内置函数，它返回检索并删除最后一个（最高的）元素，如果这个集合为空，则返回`null`。
...
函数返回检索并删除最后一个（最高的）元素，如果该集合为空，则返回`null`。
