...
> 原文: [https://www.geeksforgeeks.org/automate-mouse-events-using-java-robot-class/](https://www.geeksforgeeks.org/automate-mouse-events-using-java-robot-class/)
...
1.  `mouseMove(int x, int y)` : 将鼠标移动到屏幕的指定位置
2.  `keyPress(int k)` : 用指定的键码按给定的键
3.  `keyRelease(int k)` : 用指定的键码释放给定的键
4.  `mousePress(int b)` : 按下一个或多个鼠标按钮。
5.  `mouseRelease(int b)` : 释放一个或多个鼠标按钮。
