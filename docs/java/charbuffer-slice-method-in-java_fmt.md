...
> 原文：[https://www.geeksforgeks.org/charbuffer-slice-method-in-java/](https://www.geeksforgeks.org/charbuffer-slice-method-in-java/)

`slice()`方法的`java.nio.CharBuffer`类用于创建一个新的`CharBuffer`，其内容是给定缓冲区内容的共享子序列。
