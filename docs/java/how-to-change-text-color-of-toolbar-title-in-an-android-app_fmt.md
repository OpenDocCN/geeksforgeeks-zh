...
> 原文：[https://www.geeksforgeeks.org/how-to-change-text-color-of-toolbar-title-in-an-android-app/](https://www.geeksforgeeks.org/how-to-change-text-color-of-toolbar-title-in-an-android-app/)
...
在方法 1 中，只需转到 `activity_main.xml` 文件，并在工具栏小部件中添加一个带有文本颜色属性的 `TextView`。`activity_main.xml` 文件的完整代码如下。
...
### 步骤 2：`themes.xml` 文件的更改

转到 `应用程序 > res > 值 > 主题 > themes.xml` 文件并在 `<resources>` 标签中添加以下行。
