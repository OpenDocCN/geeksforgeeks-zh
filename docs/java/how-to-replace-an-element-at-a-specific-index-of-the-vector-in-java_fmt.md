...
> 原文：[https://www.geeksforgeeks.org/how-to-replace-an-element-at-a-specific-index-of-the-vector-in-java/](https://www.geeksforgeeks.org/how-to-replace-an-element-at-a-specific-index-of-the-vector-in-java/)

`Vector`类实现了一个可生长的对象数组。向量基本上属于遗留类，但现在它与集合完全兼容。在`java.util`包中找到，实现了`List`接口，所以我们可以在这里使用`List`接口的所有方法。
...
向量中的一个元素可以在特定/指定的索引处用另一个元素替换，使用`set()`方法，或者我们可以说`java.util.Vector.set()`方法。
...
该函数接受两个强制参数，如上语法所示，如下所述。

*   `index`：这是整型的，指的是向量中要替换的元素的位置。
*   `element`：是替换现有元素的新元素，与矢量属于同一对象类型。
