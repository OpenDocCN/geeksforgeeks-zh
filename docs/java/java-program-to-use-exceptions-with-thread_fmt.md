...
> 原文：[https://www.geeksforgeeks.org/java-program-to-use-exceptions-with-thread/](https://www.geeksforgeeks.org/java-program-to-use-exceptions-with-thread/)
...
类实现一个 `Thread` 类或 `Runnable` 接口，然后扩展类有 `start()` 方法运行线程，`sleep()` 方法使当前执行的线程休眠指定的毫秒数等等。

![](img/2dcdf7ded45ef09283db07bc92ce12b9.png)

在讨论方法之前，陈述。为了更好地理解，应该知道线程的事务来进一步处理异常。Java 中任意时间点的线程存在于以下任意一种状态。线程在任何时刻都只处于所示状态之一：
...
1.  一个类名 `RunnableThread` 实现了 `Runnable` 接口，该接口给出了由线程执行的 `run()` 方法。这个类的对象现在可以运行了。
...
2.  实现可运行接口（`java.lang.Runnable`）

### 异常和线程异常处理

这里，在扩展线程类的类中创建了一个新的线程，其中 `run()` 方法被覆盖。这会调用在扩展线程类的类中创建的新线程的入口点。此外，`start()` 方法用于启动和运行程序中的线程。
...
线程类的 `sleep()` 方法用于为了代码的正确工作流而要求线程休眠特定时间段的情况。

**语法：**

> `public static void sleep(long millis);` //常用
>
> `public static void sleep(long millis, int nanos);` //仅用于说明精度
...
| -------- | ------------------------------------------------ |
| `millis` | 线程休眠的持续时间                               |
| `nanos`  | 线程休眠的额外持续时间，但限于 0 — 999999 之间 |
...
2.  `InterruptedException` 在线程被正在进行的线程中断时抛出，正如所讨论的，Java 支持多线程的概念。
