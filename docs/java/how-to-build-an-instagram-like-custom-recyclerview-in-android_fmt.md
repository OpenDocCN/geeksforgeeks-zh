...
> 原文：[https://www.geeksforgeeks.org/how-to-build-an-instagram-like-custom-recyclerview-in-android/](https://www.geeksforgeeks.org/how-to-build-an-instagram-like-custom-recyclerview-in-android/)

我们已经看到在我们的应用程序中用简单的数据在安卓系统中实现[回收视图](https://www.geeksforgeeks.org/android-recyclerview/)。在本文中，我们将看一下在安卓中的 Instagram 实现。
...
### 第八步：创建一个适配器类，用于将该数据设置到我们的回收视图的每个项目中

导航到 `应用程序 > java > 你的应用程序的包名 > 右键单击它 > 新 java 类` 并将其命名为 `InstagramFeedRVAdapter` 并添加下面的代码。
...
第九步：使用`MainActivity.java`文件。转到`MainActivity.java`文件，参考以下代码。以下是`MainActivity.java`文件的代码。代码中添加了注释，以更详细地理解代码。
