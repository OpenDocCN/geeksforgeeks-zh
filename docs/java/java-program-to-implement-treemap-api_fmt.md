...
> 原文：[https://www.geeksforgeeks.org/java-program-to-implement-treemap-api/](https://www.geeksforgeeks.org/java-program-to-implement-treemap-api/)

[`TreeMap`](https://www.geeksforgeeks.org/treemap-in-java/) 用于在 Java 中实现 [`Map`](https://www.geeksforgeeks.org/map-interface-java-examples/) 接口和 [`NavigableMap`](https://www.geeksforgeeks.org/navigablemap-interface-in-java-with-example/) 接口以及 [`AbstractMap`](https://www.geeksforgeeks.org/abstractmap-in-java/) 类。映射根据其键的自然顺序进行排序，或者根据映射创建时提供的 [`Comparator`](https://www.geeksforgeeks.org/comparator-interface-java/) 进行排序，具体取决于使用的构造函数。

现在，在实现 `TreeMap` API 时，任务分为两部分：

1.  创建一个类，包含它的 `TreeMap` 的所有方法，被命名为 `TreeMapImplementation`。
2.  实现上面的 `TreeMap` API。
...
class TreeMapImplementation<K, V> {
    ...
    public TreeMapImplementation(Map<? extends K, ? extends V> map1) {
        map = new TreeMap<K, V>(map1);
    }

    // Constructor creates a new Treemap
    // according to given sortedMap
    public TreeMapImplementation(SortedMap<K, ? extends V> m) {
    ...
    public NavigableMap<K, V> headMap(K key, boolean include) {
    ...
    public NavigableMap<K, V> subMap(K lKey, boolean lInclude, K rKey, boolean rInclude) {
...
public class GFG {
    public static void main(String[] arg) {
        TreeMapImplementation<String, Integer> student = new TreeMapImplementation<>();
        ...
        System.out.println("The key value pairs of TreeMap:");
        ...
        System.out.println("Size of the TreeMap after insertion: " + student.size());
    }
}
```
...
System.out.println("TreeMap contains Akshay: " + student.containsKey("Akshay"));
...
System.out.println("Size of the treemap: " + student.size());
}
}
```
