# Java 中的 `DelayQueue` `put()` 方法示例

> 原文：[https://www.geeksforgeeks.org/delayqueue-put-method-in-java-with-examples/](https://www.geeksforgeeks.org/delayqueue-put-method-in-java-with-examples/)
...
## 参数
该方法只接受一个参数 `ele`。它是将被插入延迟队列的元素。

## 返回值
该方法的返回类型为 `void`，不返回值。

## 异常
*   `NullPointerException`：如果给定值为空，则抛出。
