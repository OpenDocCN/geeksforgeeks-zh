# Java 中的 `String.replace()` 方法

> 原文：[https://www.geeksforgeeks.org/java-lang-string-replace-method-java/](https://www.geeksforgeeks.org/java-lang-string-replace-method-java/)

以下是 `replace()` 方法的三个变体。本文对它们全部进行了描述，如下：
...
该方法用给定的 `replace_str` 替换该字符串中与给定正则表达式匹配的第一个子字符串。
