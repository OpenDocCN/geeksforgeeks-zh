# 比较器用示例比较 Java 中的 `comparingDouble()` 方法

> 原文：[https://www.geeksforgeeks.org/comparator-comparingdouble-method-in-java-with-examples/](https://www.geeksforgeeks.org/comparator-comparingdouble-method-in-java-with-examples/)

Java 中 [`Comparator`](https://www.geeksforgeeks.org/comparator-interface-java/) 接口的 `comparingDouble(ToDoubleFunction)` 方法接受一个函数作为参数，该函数从类型 `T` 中提取一个双精度排序键，并返回一个按该排序键进行比较的比较器。如果指定的函数也是可序列化的，则返回的比较器是可序列化的。
...
该方法接受单个参数 `keyExtractor`，该函数用于提取双精度排序键。
...
下面的程序说明了 `comparingDouble(ToDoubleFunction)` 方法：
