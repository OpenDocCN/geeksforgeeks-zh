...
> 原文: [https://www.geeksforgeeks.org/navigablemap-interface-in-java-with-example/](https://www.geeksforgeeks.org/navigablemap-interface-in-java-with-example/)

`NavigableMap` 接口是 [Java 集合框架](https://www.geeksforgeeks.org/collections-in-java-2/)的成员。它属于 `java.util` 包，是 [`SortedMap`](https://write.geeksforgeeks.org/sortedmap-in-java-with-examples/) 的扩展，提供了 `lowerKey`、`floorKey`、`ceilingKey`、`higherKey` 等便捷的导航方法。它还提供了从 Java 中的现有映射创建子映射的方法，例如键小于指定键的头映射，键大于指定键的尾映射，以及严格包含介于 `toKey` 和 `fromKey` 之间的键的子映射。

实现 `NavigableMap` 的一个示例类是 [`TreeMap`](https://write.geeksforgeeks.org/hashmap-treemap-java/)。
...
`NavigableMap` 实现 [`Map<K,V>`](https://www.geeksforgeeks.org/map-interface-java-examples/) 和 [`SortedMap<K,V>`](https://www.geeksforgeeks.org/sortedmap-java-examples/) 接口。[`ConcurrentNavigableMap<K,V>`](https://www.geeksforgeeks.org/concurrentnavigablemap-interface-in-java/) 扩展了 `NavigableMap`。`ConcurrentSkipListMap` 和 [`TreeMap`](https://www.geeksforgeeks.org/treemap-in-java/) 是 `NavigableMap` 的实现类。
...
`NavigableMap` 有两个实现类，分别是 `ConcurrentSkipListMap` 和 `TreeMap`。[`TreeMap`](https://www.geeksforgeeks.org/treemap-in-java/) 是一个基于红黑树的 `NavigableMap` 实现，它根据其键的自然顺序进行排序，或者通过地图创建时提供的 [`Comparator`](https://www.geeksforgeeks.org/comparator-interface-java/) 进行排序，具体取决于使用的构造函数。`TreeMap` 具有用于插入、删除和访问操作的 `O(log n)` 的预期时间成本。`TreeMap` 不同步，必须在外部完成同步。
...
要向 `NavigableMap` 添加元素，我们可以使用 `Map` 接口的任何方法。下面的代码展示了如何使用它们。您可以在代码中观察到插入顺序没有被保留。如果在构建时没有提供比较器，则遵循自然顺序。
...
为了移除元素，我们也使用 `Map` 接口的方法，因为 `NavigableMap` 是 `Map` 的后代。我们可以使用 [`remove()`](https://www.geeksforgeeks.org/map-remove-method-in-java-with-examples/) 方法，该方法获取键值，并从该 `TreeMap` 中移除键值的映射（如果该映射存在于地图上的话）。我们可以使用 [`clear()`](https://www.geeksforgeeks.org/navigablemap-clear-method-in-java/) 来移除地图的所有元素。
...
我们可以使用 [`Iterator`](https://www.geeksforgeeks.org/iterators-in-java/) 接口遍历集合框架的任何结构。因为迭代器处理一种类型的数据，所以我们使用 `Map.Entry<?, ?>` 将两个独立的类型解析为兼容的格式。然后使用 `next()` 方法打印 `NavigableMap` 的元素。另一个著名的方法是使用 `for-each` 循环并获得钥匙。使用 `getValue()` 方法可以找到该键的值。
...
| 方法 | 描述 |
| --- | --- |
| [`ceilingEntry(K key)`](https://www.geeksforgeeks.org/navigablemap-ceilingentry-method-in-java/) | 返回与大于或等于给定键的最小键相关联的键值映射，如果没有这样的键，则返回 `null`。 |
| [`ceilingKey(K key)`](https://www.geeksforgeeks.org/navigablemap-ceilingkey-method-in-java/) | 返回大于或等于给定键的最小键，如果没有这样的键，则返回 `null`。 |
...
| [`floorEntry(K key)`](https://www.geeksforgeeks.org/navigablemap-floorentry-method-in-java/) | 返回与小于或等于给定键的最大键相关联的键值映射，如果没有这样的键，则返回 `null`。 |
| [`floorKey(K key)`](https://www.geeksforgeeks.org/navigablemap-floorkey-method-in-java/#:~:text=The%20floorKey()%20method%20of,there%20is%20no%20such%20key.) | 返回小于或等于给定键的最大键，如果没有这样的键，则返回 `null`。 |
| [`headMap(K toKey)`](https://www.geeksforgeeks.org/navigablemap-headmap-in-java/) | 返回该地图关键点严格小于 `toKey` 的部分的视图。 |
| [`headMap(K toKey, boolean inclusive)`](https://www.geeksforgeeks.org/navigablemap-headmap-in-java/) | 返回此地图中键小于(或等于，如果 `inclusive` 为真)`toKey` 的部分的视图。 |
| [`higherEntry(K key)`](https://www.geeksforgeeks.org/navigablemap-higherentry-method-in-java/#:~:text=The%20higherEntry()%20method%20of,is%20no%20such%20key%20existed.&text=Where%2C%20K%20is%20the%20type,values%20mapped%20to%20the%20keys.) | 返回与严格大于给定键的最小键相关联的键值映射，如果没有这样的键，则返回 `null`。 |
| [`higherKey(K key)`](https://www.geeksforgeeks.org/navigablemap-higherkey-method-in-java/) | 返回严格大于给定键的最小键，如果没有这样的键，则返回 `null`。 |
| `lastEntry()` | 返回与此映射中最大键相关联的键值映射，如果映射为空，则返回 `null`。 |
| [`lowerEntry(K key)`](https://www.geeksforgeeks.org/navigablemap-lowerentry-method-in-java/) | 返回与严格小于给定键的最大键相关联的键值映射，如果没有这样的键，则返回 `null`。 |
| [`lowerKey(K key)`](https://www.geeksforgeeks.org/navigablemap-lowerkey-method-in-java/) | 返回严格小于给定键的最大键，如果没有这样的键，则返回 `null`。 |
...
| [`pollLastEntry()`](https://www.geeksforgeeks.org/navigablemap-polllastentry-method-in-java/) | 移除并返回与此映射中最大键关联的键值映射，如果映射为空，则返回 `null`。 |
...
| 方法 | 描述 |
| --- | --- |
| [`comparator()`](https://www.geeksforgeeks.org/sortedmap-comparator-method-in-java-with-examples/) | 返回用于对该映射中的键进行排序的比较器，如果该映射使用其键的自然排序，则返回 `null`。 |
| [`entrySet()`](https://www.geeksforgeeks.org/sortedmap-entryset-method-in-java-with-examples/) | 返回此映射中包含的映射的集合视图。 |
| [`firstKey()`](https://www.geeksforgeeks.org/sortedmap-firstkey-method-in-java/) | 返回当前地图中的第一个(最低的)键。 |
| [`keySet()`](https://www.geeksforgeeks.org/sortedmap-keyset-method-in-java-with-examples/) | 返回此地图中包含的键的集合视图。 |
| [`lastKey()`](https://www.geeksforgeeks.org/sortedmap-lastkey-method-in-java/) | 返回当前地图中的最后一个(最高的)键。 |
| [`values()`](https://www.geeksforgeeks.org/sortedmap-values-method-in-java-with-examples/#:~:text=The%20values()%20method%20of,the%20values%20in%20the%20Map.&text=Parameters%3A%20The%20method%20does%20not%20accept%20any%20parameters.) | 返回此地图中包含的值的集合视图。 |
...
| [`equals(Object o)`](https://www.geeksforgeeks.org/map-equals-method-in-java-with-examples/#:~:text=Map.,of%20this%20map%20or%20not.) | 将指定的对象与此映射进行比较，看是否相等。 |
...
| `putIfAbsent(K key, V value)` | 如果指定的键还没有与值相关联(或者被映射为 `null`)，则将它与给定值相关联并返回 `null`，否则返回当前值。 |
| [`remove(Object key)`](https://www.geeksforgeeks.org/map-remove-method-in-java-with-examples/) | 从该映射中删除键的映射(如果存在)(可选操作)。 |
| `remove(Object key, Object value)` | 仅当指定项当前映射到指定值时，才移除该项。 |
| `replace(K key, V value)` | 仅当指定键当前映射到某个值时，才替换该项。 |
| `replace(K key, V oldValue, V newValue)` | 仅当当前映射到指定值时，替换指定键的条目。 |
| `replaceAll(BiFunction<? super K,? super V,? extends V> function)` | 将每个条目的值替换为对该条目调用给定函数的结果，直到所有条目都已处理完毕或函数引发异常。 |
| `size()` | 返回此映射中键值映射的数量。 |

</figure>

**参考:**[https://docs.oracle.com/javase/8/docs/api/java/util/NavigableMap.html](https://docs.oracle.com/javase/8/docs/api/java/util/NavigableMap.html)
