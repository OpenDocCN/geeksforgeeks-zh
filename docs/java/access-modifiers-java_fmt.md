...
当没有为类、方法或数据成员指定访问修饰符时——它被称为具有**默认**访问修饰符。
...
在这个例子中，我们将创建两个包 `p1` 和 `p2`。`p1` 中的 `A` 类被公开，以便在 `p2` 中访问它。`A` 类中的方法 `display` 是受保护的，`B` 类是从 `A` 类继承的，然后通过创建 `B` 类的对象来访问这个受保护的方法。
