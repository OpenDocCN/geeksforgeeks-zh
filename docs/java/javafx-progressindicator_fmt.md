...
该程序创建由名称 `pb` 指示的进度指示器。进度指示器将在场景中创建，而场景又将托管在舞台中。函数 `setTitle()` 用于为舞台提供标题。然后创建一个平铺窗格，在该窗格上调用 `getChildren().add()` 方法来附加进度指示器和场景内的按钮。最后调用 `show()` 方法显示最终结果。
...
**参考**：[https://docs.oracle.com/javase/8/javafx/api/javafx/scene/control/ProgressIndicator.html](https://docs.oracle.com/javase/8/javafx/api/javafx/scene/control/ProgressIndicator.html)