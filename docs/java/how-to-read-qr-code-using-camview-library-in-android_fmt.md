...
> 原文：[https://www.geeksforgeeks.org/how-to-read-qr-code-using-camview-library-in-android/](https://www.geeksforgeeks.org/how-to-read-qr-code-using-camview-library-in-android/)

`CAMView` 库是访问用户设备摄像头的简单解决方案。通过使用这个库，我们可以访问用户的相机，并使用来执行相机的许多功能，例如扫描条形码，这些都是通过使用内置的 `ZXing` 解码引擎来完成的。该库包含一组组件，这些组件已准备好放入您的布局中，以便即时访问如此多的功能。该库处理低级例程，如相机、配置、流、方向等。我们只需要遵循一些基本步骤来使用这个库。
...
| `onScannerError()` | 当扫描仪出现错误时，使用这种方法。 |
...
我们正在清单文件中添加相机和震动权限。导航到 `App > manifests` 找到 `AndroidManifest.xml`。下面是 `AndroidManifest.xml` 的代码片段。
