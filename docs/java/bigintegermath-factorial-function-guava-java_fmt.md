# BigIntegerMath.factorial() 函数 | Guava | Java

> 原文：[https://www.geeksforgeeks.org/bigintegermath-factorial-function-guava-java/](https://www.geeksforgeeks.org/bigintegermath-factorial-function-guava-java/)

[Guava 的 `BigIntegerMath` 类](https://www.geeksforgeeks.org/bigintegermath-class-guava-java/) 的方法 `factorial(int n)` 被用来寻找给定数的阶乘。它返回 `n!`，即前 `n` 个正整数的乘积。
