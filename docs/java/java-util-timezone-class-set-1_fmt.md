...
> 原文: [https://www.geeksforgeeks.org/java-util-timezone-class-set-1/](https://www.geeksforgeeks.org/java-util-timezone-class-set-1/)
...
*   `getAvailableIDs(int rawOffset)` – 使用此方法，您可以获得一个 ID 数组，其中该 ID 的时区具有以毫秒为单位的指定 GMT 偏移。
...
*   `getTimeZone(String ID)` – 该方法用于获取给定标识的时区。
...
*   `getDSTSavings()` – 方法返回要添加到本地标准时间的时间量，以获得本地挂钟时间。
...
*   `getOffset(long date)` – 该方法用于在方法中返回该时区相对于经过日期的 UTC 的偏移量。
...
*   `inDaylightTime(Date date)` – 如果给定日期在该时区的夏令时，则该方法返回真，否则返回假。

```java
Syntax : public abstract boolean inDaylightTime(Date date)
...
*   `setDefault(TimeZone zone)` – 用于设置 `getDefault` 方法返回的时区。
