...
> 原文: [https://www.geeksforgeeks.org/character-isvalidcodepoint-method-in-java-with-examples/](https://www.geeksforgeeks.org/character-isvalidcodepoint-method-in-java-with-examples/)

`Character.isValidCodePoint()` 是 Java 中的一个内置方法，用于确定参数中提到的指定代码点是否是有效的 Unicode 代码点值。
...
## 参数:
参数 `codePoint` 为整数数据类型，指待测试的 Unicode 码点。

## 返回值:
如果指定的代码点值在最小代码点和最大代码点之间，则该方法返回 `true`，否则返回 `false`。
...
## 参考:
[https://docs.oracle.com/javase/7/docs/api/java/lang/Character.html#isValidCodePoint(int)](https://docs.oracle.com/javase/7/docs/api/java/lang/Character.html#isValidCodePoint(int))