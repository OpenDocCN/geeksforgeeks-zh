...
`Java . text . choice format`类的`previousDouble()`方法用于获取仅小于传递的双精度值的双精度值。
...
## 参数:
该方法接受双精度值`d`作为参数，返回之前的双精度值。

## 返回值:
这个方法返回一个双精度值，刚好小于传递的双精度值。
...
**参考:**[https://docs . Oracle . com/javase/9/docs/API/Java/text/choice format . html # previous double-double-T4](https://docs.oracle.com/javase/9/docs/api/java/text/ChoiceFormat.html#previousDouble-double-)