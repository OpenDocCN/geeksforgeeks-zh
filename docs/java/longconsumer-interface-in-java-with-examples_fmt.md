...
> 原文：[https://www.geeksforgeeks.org/longconsumer-interface-in-java-with-examples/](https://www.geeksforgeeks.org/longconsumer-interface-in-java-with-examples/)
...
分配给 `LongConsumer` 类型的对象的 lambda 表达式用于定义其 `accept()`，该表达式最终将给定的操作应用于其唯一的参数。这类似于使用类型为[消费者](https://www.geeksforgeeks.org/java-8-consumer-interface-in-java-with-examples/) `<Long>` 的对象。
...
**参数：** 该方法接受 `after` 参数，该参数是当前参数之后要应用的 `LongConsumer`。

**返回值：** 该方法返回一个组合 `LongConsumer`，首先应用当前操作，然后应用 `after` 操作。
