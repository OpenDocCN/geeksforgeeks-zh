...
> 原文: [https://www.geeksforgeeks.org/identityhashmap-containsvalue-method-in-java/](https://www.geeksforgeeks.org/identityhashmap-containsvalue-method-in-java/)

`java.util.IdentityHashMap.containsValue()` 方法用于检查特定值是由 `IdentityHashMap` 中的一个键映射还是由多个键映射。它将值作为参数，如果该值由映射中的任何键映射，则返回真。
...
该方法只取一个 `Object` 类型的参数 `value`，并引用其映射应该由映射内任何键检查的值。

## 返回值

如果检测到值的映射，则该方法返回布尔值 `true`，否则返回 `false`。

下面的程序说明了 `java.util.IdentityHashMap.containsValue()` 方法：
