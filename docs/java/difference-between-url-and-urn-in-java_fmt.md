...
1.  `HTTPS` 是我们正在使用的协议。
2.  `media.geeksforgeeks.org` 是服务器名称，因安全和维护组织隐私而被拒绝访问。
3.  `/WP-content/uploads/2021030515058/` 是目录名，`g.png` 是文件名。
...
1.  `urn:isbn:0451450523` 是一个统一资源名称。
...
| 网址包含协议、IP 地址、端口号、目录名、主机等。 | 它以前缀 `urn` 开始，后跟一个命名空间标识符，然后是一个命名空间特定的字符串。 |