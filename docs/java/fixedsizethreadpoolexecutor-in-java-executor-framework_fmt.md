...
在这里，主线程生成 5 个线程，每个线程都有一个 `Runnable` 实现的实例。当我们调用 `thread.start()` 时，`Runnable` 实现的 `run` 方法被调用，任务被执行。
...
在这里，我们创建了一个包含 3 个线程的线程池，在这里我们向执行器服务提交了 5 个任务。这个执行器服务负责执行线程池中的线程提交的任务。执行器服务负责创建线程池。所以这里线程名是 `pool-X-thread-Y`。