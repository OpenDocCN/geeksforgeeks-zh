...
> 原文: [https://www.geeksforgeeks.org/how-to-read-write-objects-data-in-csv-format-using-notepad-in-java/](https://www.geeksforgeeks.org/how-to-read-write-objects-data-in-csv-format-using-notepad-in-java/)
...
1.  使用文件阅读器&缓存器打开一个文件(使用文件的完全限定名)。
...
这里我们考虑一个 `Customer` 类，它有两个数据成员，即“姓名”和“账号”。在读取时，我们需要读取文件中以 CSV 格式写入的数据，分割读取的数据。在编写时，我们需要将 `Customer` 类型(CSV 格式)的每个对象的每个数据成员的值保存在一个名为 `CustomerData.txt` 的文件中。我们正在使用 `String` 类的 `format()` 方法创建一个字符串格式，该格式包含 30 字节的客户名称和 10 字节的客户账号(将每个对象的数据成员保存为相同的格式)。例如:
```java
