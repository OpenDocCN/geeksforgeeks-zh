...
> `K` 必须扩展 [`Enum`](https://www.geeksforgeeks.org/java-lang-enum-class-java/)，这强制要求密钥必须是指定的[枚举](https://www.geeksforgeeks.org/enum-in-java/)类型。
...
1.  `EnumMap(Class<K> keyType)`：构造函数用于用指定的键类型创建一个空枚举映射。
2.  `EnumMap(EnumMap<K,? extends V> m)`：构造函数用于创建一个枚举映射，该映射与指定的枚举映射具有相同的键类型，初始映射与 `EnumMap` 相同。
3.  `EnumMap(Map<K,? extends V> m)`：构造函数用于从参数中指定的映射创建一个带有初始化的枚举映射。
...
| 方法 | 已执行的操作 |
| --- | --- |
| [`clear()`](https://www.geeksforgeeks.org/enummap-clear-method-in-java/) | 从此映射中删除所有映射。 |
| [`clone()`](https://www.geeksforgeeks.org/enummap-clone-method-in-java/) | 返回此枚举映射的浅层副本。 |
| [`containsKey(Object key)`](https://www.geeksforgeeks.org/enummap-containskey-method-in-java/) | 如果此映射包含指定键的映射，则返回 `true`。 |
| [`containsValue(Object value)`](https://www.geeksforgeeks.org/enummap-containsvaluevalue-method-in-java/) | 如果此映射将一个或多个键映射到指定值，则返回 `true`。 |
| [`entrySet()`](https://www.geeksforgeeks.org/enummap-entryset-method-in-java/) | 返回此映射中包含的映射的集合视图。 |
| [`equals(Object o)`](https://www.geeksforgeeks.org/enummap-equals-method-in-java-with-examples/) | 将指定的对象与此映射进行比较，看是否相等。 |
| [`get(Object key)`](https://www.geeksforgeeks.org/enummap-get-method-in-java/) | 返回指定键映射到的值，如果此映射不包含键映射，则返回 `null`。 |
| `hashCode()` | 返回此映射的哈希代码值。 |
| [`keySet()`](https://www.geeksforgeeks.org/enummap-keyset-method-in-java/) | 返回此地图中包含的键的集合视图。 |
| [`put(K key, V value)`](https://www.geeksforgeeks.org/enummap-put-method-in-java/) | 将指定值与此映射中的指定键相关联。 |
| [`putAll(Map<? extends K,? extends V> m)`](https://www.geeksforgeeks.org/enummap-putallmap-method-in-java/) | 将指定映射中的所有映射复制到此映射。 |
| [`remove(Object key)`](https://www.geeksforgeeks.org/enummap-remove-method-in-java/) | 从该映射中删除该键的映射(如果存在)。 |
| [`size()`](https://www.geeksforgeeks.org/enummap-size-method-in-java/) | 返回此映射中键值映射的数量。 |
| [`values()`](https://www.geeksforgeeks.org/enummap-values-method-in-java/) | 返回此地图中包含的值的集合视图。 |
...
| 属性 | EnumMap | EnumSet |
| --- | --- | --- |
| *内部表示* | `EnumMap` 在内部表示为数组。该表示是紧凑和有效的。 | `EnumSet` 在内部表示为位向量或位序列。 |
| *允许空元素？* | 不允许空键，但允许空值。 | 不允许空元素。 |
| *是抽象类吗？* | 否 | 是 |
| *实例化* | 由于 `EnumMap` 不是一个抽象类，因此可以使用 `new` 运算符来实例化它。 | 它是一个抽象类，没有构造函数。`EnumSet` 是使用它的预定义方法创建的，如 `allOf()`、`noneOf()`、`of()` 等。 |
