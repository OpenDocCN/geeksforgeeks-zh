...
> 原文：`https://www.geeksforgeeks.org/how-to-print-hashset-elements-in-java/`
...
*   我们可以使用 `Collection` 接口的 `iterator()` 方法创建一个迭代器对象。

> `public Iterator iterator();` // `Collection` 接口的 `iterator()` 方法。

*   创建迭代器对象

> `Iterator itr = c.iterator();` // 其中 `c` 是任何集合对象，如 `ArrayList`、`HashSet` 等。
...
我们可以使用 `HashSet` 对象引用变量直接打印 `HashSet` 元素。它将打印完整的 `HashSet` 对象。
