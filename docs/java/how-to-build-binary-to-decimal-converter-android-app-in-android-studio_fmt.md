...
> 原文：[https://www.geeksforgeeks.org/how-to-build-binary-to-decimal-converter-android-app-in-android-studio/](https://www.geeksforgeeks.org/how-to-build-binary-to-decimal-converter-android-app-in-android-studio/)
...
导航到`app/res/layout/activity_main.xml`，并将下面的代码添加到该文件中。在`activity_main.xml`文件中，我们要添加**两个 EditText** 用于**输入和输出**文本显示，**两个按钮**用于**提交**提交输入，**清除**重置，我已经添加了**两个 TextView** 显示一些文本，这样我们就可以指导用户如何使用这个应用了。下面是`activity_main.xml`文件的代码。
