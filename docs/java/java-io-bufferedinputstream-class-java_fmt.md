...
> 原文: [https://www.geeksforgeeks.org/java-io-bufferedinputstream-class-java/](https://www.geeksforgeeks.org/java-io-bufferedinputstream-class-java/)
...
### `int available()`
返回可以从该输入流中读取（或跳过）的字节数的估计值，而不会被该输入流的方法的下一次调用所阻塞。
...
Overrides: markSupported in class FilterInputStream
...
从给定的偏移量开始，将该字节输入流中的字节读入指定的字节数组。
...
**下一篇:** [Java 中的 `Java.io.BufferedOutputStream` 类](https://www.geeksforgeeks.org/java-io-bufferedoutputstream-class-java/)
