...
### 步骤 1
在你的根 `build.gradle` 文件（不是你的模块 `build.gradle` 文件）中添加支持库。这个库 `jitpack` 是一个新颖的包存储库。它是为 JVM 而做的，这样在 [github](https://www.geeksforgeeks.org/ultimate-guide-git-github/) 和 [bigbucket](https://www.geeksforgeeks.org/bitbucket-vs-github-vs-gitlab/) 中存在的任何库都可以直接在应用程序中使用。
...
### 步骤 2
在 `build.gradle` 文件中添加支持库，并在依赖项部分添加依赖项。
...
### 步骤 3
在 `activity_main.xml` 文件中添加以下代码。在这个文件中，我们将`加载按钮`添加到布局中。
...
### 步骤 4
在 `MainActivity.java` 文件中添加以下代码。在这个文件中，将加载按钮附加到根布局，然后添加一个`监听器`。因此，每当用户点击它时，就会自动调用 `setOnButtonLoadingListener`。在侦听器内部，有三种方法：`onClick()`、`onStart()` 和 `onFinish()`。在 `onClick` 方法里面添加按钮被点击时应该执行的代码，在 `onStart()` 方法里面添加按钮被点击时应该立即执行的代码，在 `onFinish()` 方法里面添加其他两个功能执行完之后执行的代码。
...
## 参考
[https://github.com/andreasagap/LoadingButtonLibrary](https://github.com/andreasagap/LoadingButtonLibrary)