...
> 原文:[https://www.geeksforgeeks.org/keyfactory-generatepublic-method-in-java-with-examples/](https://www.geeksforgeeks.org/keyfactory-generatepublic-method-in-java-with-examples/)
...
## 参数:
该方法以 `keySpec`(私钥的规格(密钥材料))为参数。

## 返回值:
该方法返回 `公钥`。

## 异常:
如果给定的密钥规范不适合此密钥工厂生成私钥，则此方法将引发 `InvalidKeySpecException`。
