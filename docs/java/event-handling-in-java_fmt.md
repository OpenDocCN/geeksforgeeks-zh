...
这是一种机制，用于`控制事件`和`决定事件`发生后应该发生什么。为了处理事件，Java遵循`委托事件模型`。
...
**例1:** 对于`按键事件`我们使用`addKeyListener()`进行注册。

**例2:** 说明对于`动作事件`我们使用`addActionListener()`进行注册。
...
| 事件类 | 监听器接口 | 描述 |
| --- | --- | --- |
| `ActionEvent` | `ActionListener` | 一种事件，指示组件定义的操作发生，如按钮单击或从菜单项列表中选择一项。 |
| `AdjustmentEvent` | `AdjustmentListener` | 调整事件由像滚动条这样的可调整对象发出。 |
| `ComponentEvent` | `ComponentListener` | 指示组件移动、大小改变或可见性改变的事件。 |
| `ContainerEvent` | `ContainerListener` | 当组件被添加到容器(或从容器中移除)时，该事件由容器对象生成。 |
| `FocusEvent` | `FocusListener` | 这些是与焦点相关的事件，包括焦点、聚焦、聚焦输出和模糊。 |
| `ItemEvent` | `ItemListener` | 一个事件，指示某项是否被选中。 |
| `KeyEvent` | `KeyListener` | 由于键盘上的一系列按键而发生的事件。 |
| `MouseEvent` | `MouseListener` & `MouseMotionListener` | 由于用户与鼠标(定点设备)的交互而发生的事件。 |
| `MouseWheelEvent` | `MouseWheelListener` | 指定鼠标滚轮在组件中旋转的事件。 |
| `TextEvent` | `TextListener` | 当对象的文本更改时发生的事件。 |
...
| `WindowListener` | * `windowActivated()` * `windowDeactivated()` * `windowOpened()` * `windowClosing()` * `windowClosed()` * `windowIconified()` * `windowDeiconified()` |
...
> **注意:**使用任何IDE或安装JDK运行代码，在线编译器可能会因为某些包不可用而抛出错误。
