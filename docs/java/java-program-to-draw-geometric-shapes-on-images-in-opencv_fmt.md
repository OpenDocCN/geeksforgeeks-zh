...
> 原文：[https://www.geeksforgeeks.org/java-program-to-draw-geometric-shapes-on-images-in-opencv/](https://www.geeksforgeeks.org/java-program-to-draw-geometric-shapes-on-images-in-opencv/)

Java 中的 `OpenCV` 库包含一个名为 `Improc` 的类，该类提供了各种方法，如 `resize()`、`wrapAffine()`、`filter2D` 来处理输入图像。我们将在这里使用 `javax.swing` 包。`javax.swing` 包为 java swing API 提供类，如 `JButton`、`JTextField`、`JTextArea`、`JRadioButton`、`JCheckbox`、`JMenu`、`JColorChooser` 等。这里涉及到的 java 概念是 Swing 类和抽象窗口工具包，如下所述：

## Swing 类

这里我将解释一下 `javax.swing` 包的所有类，这些类将在后面的程序中使用：
...
*   `javax.swing.JMenuBar`：类 `JMenuBar` 用于在窗口或框架上显示菜单栏。它可能有几个菜单。
*   `javax.swing.JMenuItem`：`JMenuItem` 类表示菜单中的实际项目。菜单中的所有项目都应该从类或其子类中派生出来。
...
在 `OpenCV` 中在图像上绘制几何形状的步骤：

1.  创建一个项目并添加 `OpenCV` 库
2.  创建包
3.  创建一个类
4.  创建一个名为“images”的文件夹，并放置一个 `.jpg` 或 `.png` 文件在里面。
5.  在 java 文件中编写以下代码。
...
*   点击文件 > 新建 > Java 项目。
*   为您的项目命名，并在如下图所示的 `JRE` 和项目布局选项下勾选必要的选项。在这里，我已经将这个项目命名为 `OpenCVShape`。
*   标记选项后，单击下一步。
...
### 步骤 4：创建一个名为“images”的文件夹，放一个 `.jpg` 或 `.png` 文件在里面。

*   右键点击项目名称（在本例中为 `OpenCVShape`）。转到新建 -> 文件夹。键入文件夹的名称（图片）并点击确定。
*   放一张扩展名为 `.jpg` 或 `.png` 的图片在这个文件夹里。

### 步骤 5：在 java 文件中编写以下代码

实现：计算上述步骤，得到不同的几何形状，如下例所示：
...
public class OpenCVShapes extends JFrame {
    ...
    private void setUpView() {
        ...
        imageView.addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent e) {
                super.mouseReleased(e);
                // when mouse release replace tempimage to image
                image = tempImage.clone();
            }
        });
        ...
        saveMenuItem.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                // TODO Auto-generated method stub
                Imgcodecs.imwrite("images/ind1.png", image);
            }
        });
    }
