...
> 原文：[https://www.geeksforgeeks.org/java-program-to-create-an-object-for-class-and-assign-value-in-the-object-using-constructor/](https://www.geeksforgeeks.org/java-program-to-create-an-object-for-class-and-assign-value-in-the-object-using-constructor/)
...
1.  首先，定义一个名称为`SampleClass`的类，并定义一个构造函数方法。
2.  **构造器**将始终具有与类名称相同的名称，并且它不具有返回类型。
3.  构造函数用于实例化类的变量。
4.  现在，使用构造函数，我们可以赋值。
5.  在构造函数方法之后，实现一个返回任何变量的值的函数。
6.  现在在主功能中使用`new`关键字创建一个对象。如果该类没有自定义构造函数，则调用该类的默认构造函数，否则在创建对象时根据与该类构造函数匹配的参数类型和数量调用自定义构造函数。
7.  我们可以使用对象调用上面声明的函数。
8.  最后使用`System.out.println()`语句打印函数的值。
9.  一旦你完成了实现，运行程序并得到输出。
