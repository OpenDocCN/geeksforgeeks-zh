...
> 原文: [https://www.geeksforgeeks.org/java-net-interfaceaddress-class-java/](https://www.geeksforgeeks.org/java-net-interfaceaddress-class-java/)
...
2.  `getBroadcast()`：返回该接口地址的广播地址的 `InetAddress`。由于只有 IPv4 地址有广播地址，使用 IPv6 地址时将返回空值。

    ```java
    Syntax : public InetAddress getBroadcast()
    ```

3.  `getNetworkPrefixLength()`：返回该接口地址的前缀长度，即该地址的子网掩码。

    ```java
    Syntax : public short getNetworkPrefixLength()
    ```

4.  `equals()`：用于指定对象与该接口地址的比较。仅当给定对象不为空并且表示与此对象相同的接口地址时，才返回 `true`。
...
5.  `hashCode()`：返回该接口地址的 `hashCode`。

    ```java
    Syntax : public int hashCode()
    ```

6.  `toString()`：返回此接口地址的字符串表示形式。该字符串的形式为：接口地址/前缀长度。
