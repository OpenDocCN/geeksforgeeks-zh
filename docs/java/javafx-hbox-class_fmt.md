...
`HBox` 是 JavaFX 的一部分。`HBox` 以水平列的形式显示其子代。如果 `HBox` 设置了边框和/或填充，那么内容将在这些插页中显示。`HBox` 类扩展了 `Pane` 类。

## 类的构造函数

*   `HBox()`：创建一个没有节点的 `HBox` 对象。
*   `HBox(double s)`：创建节点之间有间距的 `HBox`。
...
### 1. Java Program to create a HBox and add it to the stage
...
### 2. Java Program to create a HBox, add spaces between its elements and add it to the stage
...
### 3. Java Program to create a HBox, add spaces between its elements, set an alignment and add it to the stage
