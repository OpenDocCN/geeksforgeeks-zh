...
> 原文：[https://www.geeksforgeeks.org/java-program-to-illustrate-the-availability-of-default-constructor-of-the-super-class-to-the-sub-class-by-default/](https://www.geeksforgeeks.org/java-program-to-illustrate-the-availability-of-default-constructor-of-the-super-class-to-the-sub-class-by-default/)

Java 中的[构造函数](https://www.geeksforgeeks.org/constructors-in-java/)是一种用于初始化对象的特殊方法。每当使用 `new()` 关键字创建对象时，至少会调用一个构造函数。构造函数名称必须与类名匹配，并且不能有返回类型。如果在这种情况下类中没有可用的构造函数，Java 编译器默认提供一个**默认构造函数（无参数构造函数）**。
...
在下面给出的代码中，我们创建了一个名为 `defaultConstructor` 的类，在这个类中，我们创建了一个默认构造函数。
