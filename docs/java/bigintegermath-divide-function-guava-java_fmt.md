# BigIntegerMath.divide() 函数 | Guava | Java

> 原文：[https://www.geeksforgeeks.org/bigIntegermath-divide-function-guava-java/](https://www.geeksforgeeks.org/bigIntegermath-divide-function-guava-java/)

[Guava 的 `BigIntegerMath` 类](https://www.geeksforgeeks.org/bigintegermath-class-guava-java/)的方法 `divide(BigInteger p, BigInteger q, RoundingMode mode)` 返回 `p` 除以 `q` 的结果，使用指定的舍入模式进行舍入。
...
*   `p`：被除数（大整数）。
*   `q`：除数（大整数）。
*   `mode`：用于计算 `p` 和 `q` 除法的舍入模式。

## 返回值

此方法返回 `p` 除以 `q` 的结果，使用指定的舍入模式进行舍入。

## 异常

如果 `q == 0`，或者如果 `mode == UNNECESSARY` 且 `a` 不是 `b` 的整数倍，则方法抛出 `ArithmeticException`。
