...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-covid-19-tracker-android-app-to-see-details-of-any-city-and-state-in-india/](https://www.geeksforgeeks.org/how-to-create-a-covid-19-tracker-android-app-to-see-details-of-any-city-and-state-in-india/)
...
*   转到 `app -> res -> values -> colors.xml` 部分，设置应用的颜色。
*   这些颜色用于不同地方的应用程序。
...
*   转到 `Gradle Scripts -> Build.gradle(Module: app)` 部分，导入以下依赖项，点击 `立即同步` 按钮同步应用。
*   要了解更多关于 `volley` 库的信息[请点击此处](https://developer.android.com/training/volley)。

> implementation 'com.android.volley:volley:1.1.1'

*   现在转到 `app -> manifests -> AndroidManifest.xml` 部分，允许 `INTERNET` 权限，因为此应用将使用互联网权限。
...
1.  转到 `app -> res -> layout -> activity_main.xml`
2.  在布局中添加一个 [`TextView`](https://www.geeksforgeeks.org/textview-widget-in-android-using-java-with-examples/)：在屏幕顶部显示极客。
3.  在布局中添加一个 [`ListView`](https://www.geeksforgeeks.org/android-listview-in-java-with-example/)：在屏幕上显示城市追踪详情列表。
...
1.  右键单击资源文件夹中的 `layout` 文件夹，创建新布局。
2.  然后点击 `Layout Resource File`，设置名称 `testing.xml`。
3.  点击 `完成` 保存。
4.  单击 `testing.xml` 并开始设计列表视图的用户界面布局。
5.  `testing.xml` 中使用的可绘制文件是“`向上箭头`”。转到 `drawable -> New -> Vector Asset` 并搜索“`向上箭头`”并将其添加到您的文件中。
...
在这个文件中，我们将使用`凌空`库。参考代码内部的注释来理解代码。
