...
> 原文：[https://www.geeksforgeeks.org/how-to-replace-a-value-for-the-given-key-in-the-treemap/](https://www.geeksforgeeks.org/how-to-replace-a-value-for-the-given-key-in-the-treemap/)

Java 中的 [`TreeMap`](https://www.geeksforgeeks.org/treemap-in-java/) 与抽象地图类一起用于实现 [`Map` 接口](https://www.geeksforgeeks.org/map-interface-java-examples/) 和 [`NavigableMap` 接口](https://www.geeksforgeeks.org/navigablemap-interface-in-java-with-example/)。地图根据其键的自然顺序进行排序，或者通过地图创建时提供的 [`Comparator`](https://www.geeksforgeeks.org/comparator-interface-java/) 进行排序，具体取决于使用的构造函数。
...
**参数：** 该方法采用两个参数，都是 `TreeMap` 的对象类型。

*   `key`：这是指需要插入到 `Map` 中进行映射的关键元素。
*   `value`：这是指上述键将映射到的值。

**返回值：** 如果传递了一个现有的键，则返回前一个值。如果传递了新的对，则返回 `null`。
...
*   在 `replace` 的情况下，如果密钥不存在，它不会创建新的记录。
...
*   [`NullPointerException`](https://www.geeksforgeeks.org/null-pointer-exception-in-java/) 如果指定的键或值为空，并且此映射不允许空键或值，并且
