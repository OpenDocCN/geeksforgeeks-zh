...
> 原文: [https://www.geeksforgeeks.org/can-we-override-default-method-in-java/](https://www.geeksforgeeks.org/can-we-override-default-method-in-java/)
...
如果我们在一个程序中只使用一个接口，那么我们每次只使用一个默认方法，此时不需要覆盖，如下程序所示：
...
但是当使用的多于两个接口并且两个接口都作为父类时，这时需要覆盖默认方法。如果我们使用多个接口并且在两个接口中，如果两个接口具有相同的名称和相同的结构。所以在那个时候，必须重写缺省方法中的任何一个，否则会导致错误。
