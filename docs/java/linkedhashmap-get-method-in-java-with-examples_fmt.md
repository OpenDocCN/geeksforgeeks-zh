...
> 原文: [https://www.geeksforgeeks.org/linkedhashmap-get-method-in-java-with-examples/](https://www.geeksforgeeks.org/linkedhashmap-get-method-in-java-with-examples/)

在 Java 中，[`LinkedHashMap` 类](https://www.geeksforgeeks.org/linkedhashmap-class-java-examples/)的 `get()` 方法用于检索或获取参数中提到的特定键映射的值。当映射不包含键的这种映射时，它返回空值。
...
一个对象类型的参数 `key_element`，指的是应该取关联值的键。

## 返回类型

参数中与 `key_element` 关联的值。
