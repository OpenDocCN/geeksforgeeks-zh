...
*   为了获得可绘制的星星，使用了 `rt.getProgressDrawable()` 方法。然后修改星星的颜色，使用方法 `setColorFilter()` 和参数 `Color.YELLOW`。最后，编写 `Call` 方法，通过 `rt.getRating()` 方法提取用户选择的评级值。
...
**注意:** 对于布局，如果你是初学者，`ConstraintLayout` 很好用，因为它可以根据屏幕调整视图。这个 XML 文件定义了应用程序的视图。
...
这里我们不需要更改清单文件，不需要 `RatingBar` 的许可。默认情况下，清单文件中会提到所有创建的新活动。
