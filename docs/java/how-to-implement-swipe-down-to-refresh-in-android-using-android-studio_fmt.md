...
在开始编写代码之前，有必要在 `build.gradle` 文件中添加一个**滑动刷新布局依赖项**以启用滑动布局。这种依赖性是：
...
重要的是从前端 `activity_main.xml` 开始。创建一个 `SwipeRefreshLayout` 刷新布局并添加一个 `TextView` 在屏幕上显示字符串并为其提供一定的 id。
