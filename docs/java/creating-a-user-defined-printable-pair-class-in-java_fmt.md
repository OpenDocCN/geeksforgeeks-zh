...
> 原文：[https://www.geeksforgeeks.org/creating-a-user-defined-printable-pair-class-in-java/](https://www.geeksforgeeks.org/creating-a-user-defined-printable-pair-class-in-java/)

C++标准库中的 `pair` 类被大量使用。我们可以在 Java 中实现我们自己的用户定义的 `pair` 类，它的对象可以像任何其他参数一样在任何地方使用。

**注：**

> 这个类相当于 java 中的 `pair<int>` 类。您可以为其他数据类型创建自己的模板或类。
...
*   我们现在可以玩这个类，甚至创建一个对的数组，类似于 C++中的 `vector<pair>`。
