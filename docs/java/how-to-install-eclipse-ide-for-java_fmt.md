...
> 原文：[https://www.geeksforgeeks.org/how-to-install-eclipse-ide-for-java/](https://www.geeksforgeeks.org/how-to-install-eclipse-ide-for-java/)
...
2.  前往 [`Eclipse` 网站下载](https://www.eclipse.org/downloads)，选择符合操作系统和位数要求的版本。
3.  打开下载的文件，并遵循标准软件安装过程。
4.  根据开发人员的语言需求选择软件包。
...
要在工作区运行 `Eclipse`，首先需要下载并安装 `JDK`（`Java` 开发工具包）。打开你的默认浏览器，搜索“`JDK` 下载”，点击 `oracle.com` 链接。甲骨文为您提供最新版本的 `JDK`。您可以根据需要的配置从这里下载。
...
单击链接 `https://www.eclipse.org/downloads`，根据您的 `Windows32` 或 `64` 位版本选择兼容的 `IDE` 版本。

![](img/7b8dea49b00fffe60bd28956d2ad09cf.png)

### 第三步

下载完成后，打开下载的文件，从屏幕上打开的弹出窗口中选择 `RUN`。

### 第 4 步

现在从这里开始，我们必须为 `Java` 开发者选择包。并给出 `IDE` 的目录路径，以便进一步处理。
...
现在你已经准备好在 `Eclipse` 中做 `Java` 编程了。我们可以走了。