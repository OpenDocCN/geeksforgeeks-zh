...
> 原文: [https://www.geeksforgeeks.org/date-compareto-method-in-java-with-examples/](https://www.geeksforgeeks.org/date-compareto-method-in-java-with-examples/)

`Date`类的`compareTo()`方法比较两个日期并排序。
...
该函数接受单个参数`anotherDate`，该参数指定要比较的日期。
...
如果`anotherDate`为`null`，函数抛出一个异常，即`NullPointerException`。
