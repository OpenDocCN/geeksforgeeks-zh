...
> 原文：[https://www.geeksforgeeks.org/actionbar-in-android-with-example/](https://www.geeksforgeeks.org/actionbar-in-android-with-example/)
...
随着 ActionBar 的推出，谷歌宣布了一个**支持库**。该库是 **AppCompat** 的一部分，其目的是为旧版安卓提供向后兼容性，并支持选项卡式界面。所有使用安卓提供的默认主题的应用程序（`Theme.Default`）下，包含一个动作栏。但是，开发人员可以根据自己的需要以多种方式对其进行定制。动作栏中包含的组件有：
...
*   `android:orderInCategory`：该属性的值指定了项目在 ActionBar 中的位置。有两种方法可以定义不同菜单项的位置。第一种方法是为所有项目提供相同的属性值，并且位置将按照代码中声明的顺序进行定义。第二种方法是为所有项目提供一个不同的数值，然后项目将根据该属性的值的升序来定位自己。
*   `app:showAsAction`：该属性定义了项目将如何出现在动作栏中。有四种可能的标志可供选择：
    *   `always`：始终在动作栏中显示项目。
    *   `ifRoom`：如果有空间，保留物品。
    *   `never`：有了这个标志，该项目不会在 ActionBar 中显示为图标，而是会出现在溢出菜单中。
    *   `withText`：要将一个项目既表示为图标又表示为标题，可以在该标志后附加一个 `always` 或 `ifRoom` 标志（`always|withText` 或 `ifRoom|withText`）。
*   `android:icon`：一个项目的图标通过该属性在可绘制目录中被引用。
...
动作栏的项目是为执行某些操作而设计的。项目的那些操作/动作是在活动文件中声明的，活动文件是为动作栏设计的。在本例中，目标活动是 `MainActivity` 文件。此外，自定义标题、副标题和应用程序徽标也在该文件中定义。下面是正确的代码，用于设计所有提到的项目，并在用户单击操作栏的项目时显示一条祝酒信息。
...
前往位于 `res` 文件夹的**值目录**中的 `style.xml` 文件。要更改动作栏的默认颜色，必须更改 `colorPrimary` 资源。下面是使动作栏颜色为“绿色”的代码。
...
*   `ActionBar` 的所有特性不是一次引入的，而是随着不同 `API` 级别(如 `API 15`、`17` 和 `19`)的发布而引入的。
