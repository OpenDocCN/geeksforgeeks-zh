...
> 原文: [https://www.geeksforgeeks.org/java-swing-jtogglebutton-class/](https://www.geeksforgeeks.org/java-swing-jtogglebutton-class/)
...
2.  `JToggleButton(Action a)`: 创建一个切换按钮，从提供的动作中获取属性。
3.  `JToggleButton(Icon icon)`: 用指定的图像创建一个最初未选择的切换按钮，但没有文本。
4.  `JToggleButton(Icon icon, boolean selected)`: 创建具有指定图像和选择状态的切换按钮，但没有文本。
5.  `JToggleButton(String text)`: 用指定的文本创建一个未选中的切换按钮。
6.  `JToggleButton(String text, boolean selected)`: 创建具有指定文本和选择状态的切换按钮。
7.  `JToggleButton(String text, Icon icon)`: 创建具有指定文本和图像的切换按钮，该按钮最初未被选中。
...
### 1. 用项目侦听器实现 JToggleButton 事件的 Java 程序

在这个程序中，我们使用 `JFrame()` 创建框架。这里，`setDefaultCloseOperation()` 用于设置框架的关闭选项。使用 `JToggleButton()` 创建一个按钮。实例化 `ItemListener`，它只包含 `itemStateChanged()` 方法，当点击按钮时自动调用。事件在按钮上生成，相应地，输出被打印到控制台。附加所有监听器，并在按钮上添加 `addItemListener()`。向框架添加按钮并设置框架的大小。
...
**输出:**
[视频演示](https://media.geeksforgeeks.org/wp-content/uploads/20180826_023018.mp4)

### 2. 使用 ActionListener 实现 JToggleButton 事件的 Java 程序

这里，在 `JFrame` 上创建了一个 `JToggleButton`。然后，我们定义 `ActionListener`。`actionPerformed()` 是 `ActionListener()` 中唯一的方法，只要点击注册的组件就会被调用。`abstractButton.getModel().isSelected()` 如果选择了按钮返回 `true`，否则返回 `false`。将监听器连接到跳转按钮。
...
**输出:**
[视频演示](https://media.geeksforgeeks.org/wp-content/uploads/20180826_031430.mp4)
