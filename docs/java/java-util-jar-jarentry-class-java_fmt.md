...
> 原文：[https://www.geeksforgeeks.org/java-util-jar-jarentry-class-java/](https://www.geeksforgeeks.org/java-util-jar-jarentry-class-java/)
...
`equals`，`finalize`，`getClass`，`notify`，`notifyAll`，`wait`

**注意**：由于无法读取文件，程序不会在在线 IDE 上运行。
