...
> 原文：[https://www.geeksforgeeks.org/java-program-to-define-ordinal-method-using-enum-concept/](https://www.geeksforgeeks.org/java-program-to-define-ordinal-method-using-enum-concept/)

Java 枚举，也称为 Java 枚举类型，是一种其字段由一组固定常数组成的类型。

`java.lang.Enum.ordinal()`讲述了`序数`（它是其`枚举`声明中的位置，其中初始常数被赋予零的序数）用于特定的枚举。
...
> 比如我们在枚举类中声明了三个索引，那么枚举索引的`ordinal()`值是 0，1，2
...
// value remain same whether we mention
