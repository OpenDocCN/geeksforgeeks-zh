# Java 中的 LongStream noneMatch() 方法详解

> 原文：[https://www.geeksforgeeks.org/longstream-nonematch-java-examples/](https://www.geeksforgeeks.org/longstream-nonematch-java-examples/)

`LongStream noneMatch(LongPredicate predicate)` 返回此流中是否没有元素与提供的谓词匹配。如果不是确定结果所必需的，它可能不会计算所有元素上的谓词。这是一次**短路端子操作**。如果终端操作在无限输入时可能在有限时间内终止，则该操作为短路。
...
1.  `LongPredicate`：一个长值参数的谓词（布尔值函数）。

## 返回值

如果流的所有元素都与提供的谓词匹配，或者流为空，则函数返回 `true`，否则返回 `false`。

## 注意

如果流为空，则返回 `true`，不计算谓词。

## 示例 1

`noneMatch()` 函数，检查 `LongStream` 的元素是否都不能被 5 整除。
...
`noneMatch()` 函数，用于检查连接两个 `LongStream` 后得到的 `LongStream` 中是否没有元素小于 2。
