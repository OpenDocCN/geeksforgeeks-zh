...
> 原文：[https://www.geeksforgeeks.org/convert-an-array-into-collection-in-java/](https://www.geeksforgeeks.org/convert-an-array-into-collection-in-java/)

Java `集合`提供了一个存储和操作对象组的架构。数据的数据类型可以更改为常规数据类型，如数组到集合。要将基于数组的数据转换为集合，我们可以使用 `java.util.Arrays` 类。这个类提供一个静态方法 `asList(T… a)`，将数组转换为集合。
