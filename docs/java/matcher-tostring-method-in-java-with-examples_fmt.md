# Java中Matcher toString()方法，带示例

> 原文：[https://www.geeksforgeks.org/matcher-tostring-method-in-java-with-examples/](https://www.geeksforgeks.org/matcher-tostring-method-in-java-with-examples/)

`Matcher`类的`toString()`方法用于获取该匹配器的字符串表示。此方法从`Object`类派生而来，其行为方式类似。
...
这个方法返回一个`String`值，这是这个匹配器的字符串表示。
