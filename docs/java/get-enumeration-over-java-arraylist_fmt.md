...
> 原文：[https://www.geeksforgeeks.org/get-enumeration-over-java-arraylist/](https://www.geeksforgeeks.org/get-enumeration-over-java-arraylist/)

[`ArrayList`](https://www.geeksforgeeks.org/arraylist-in-java/)类是存在于`java.util`包中的一个可重定大小的数组。与内置数组不同，`ArrayList`可以动态改变其大小，在数组列表中可以添加和删除元素。
...
`ArrayList`可以是如下所示的任何类型。`java.util.Collections`类的`enumeration()`方法用于返回指定集合的枚举。

*   整数、字符串、双精度等的`ArrayList`。
*   `ArrayList`的`ArrayList`，或者仅仅是`HashMap`的`ArrayList`。
*   任何用户定义对象的`ArrayList`。
...
**使用的方法：** [`hasMoreElements()`](https://www.geeksforgeeks.org/enumeration-hasmoreelements-method-in-java-with-examples/) 方法

实现`Enumeration`接口的对象生成一系列元素，一次一个。`hasMoreElements()`枚举法用于测试该枚举是否包含更多元素。如果枚举包含的元素多于它将返回`true`，否则返回`false`。
...
**返回值：** 当且仅当此枚举对象至少包含一个要提供的元素时，此方法返回`true`，否则为`false`。
