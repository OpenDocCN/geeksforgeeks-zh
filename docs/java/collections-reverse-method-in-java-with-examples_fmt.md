...
> 原文: [https://www.geeksforgeeks.org/collections-reverse-method-in-java-with-examples/](https://www.geeksforgeeks.org/collections-reverse-method-in-java-with-examples/)

`reverse()` 是 `Collections` 类的方法（顾名思义）用于反转存储元素的对象中的元素。它颠倒了作为参数传递的列表中元素的顺序。这个类存在于 `java.util` 包中，所以语法如下：
...
如果指定列表或其列表迭代器不支持 `set` 操作，则抛出 `UnsupportedOperationException`。
...
让我们通过在 Java 代码中实现 `Collections` 类的这个方法，如下所示：
...
如果我们浏览一下上面的程序，那么只有一个微小的签名细节，那就是我们正在创建一个 `LinkedList` 类的对象，而不是 `ArrayList` 类，如示例 1 所示。对于 `LinkedList`，我们只是在上面的代码中进行了如下所示的更改：
...
`Arrays` 类（[Java 中的 Arrays 类](https://www.geeksforgeeks.org/array-class-in-java/)）没有反转方法。我们可以使用 `Collections.reverse()` 来反转数组，如下所示：
