# Java 中的 `BufferedReader reset()` 方法示例

> 原文：[https://www.geeksforgeeks.org/bufferedreader-reset-method-in-java-with-examples/](https://www.geeksforgeeks.org/bufferedreader-reset-method-in-java-with-examples/)

Java 中 `BufferedReader` 类的 `reset()` 方法是将位置固定或标记在最后标记的位置，这样就可以再次读取同一个字节。
...
如果从未在流上调用 `mark()` 方法，或者如果 mark 的值无效，该方法将抛出 `IOException`。
