...
该方法返回德格容器的**第一个元素**，如有必要，等待直到某个元素可用。

## 异常:

如果在等待时被中断，该功能会抛出**中断异常**。

## 注:

`blocking request`的 `takeFirst()` 方法继承自 Java 中的 `LinkedBlockingDeque` 类。
...
[https://docs . Oracle . com/javase/7/docs/API/Java/util/concurrent/blockingdequee . html # take first()](https://docs.oracle.com/javase/7/docs/api/java/util/concurrent/BlockingDeque.html#takeFirst())