...
`DoubleStream skip(long n)` 在丢弃流的第 n 个元素后，返回由该流的剩余元素组成的流。如果此流包含的元素少于 n 个，则将返回一个空流。

## 注意

`DoubleStream skip()` 是一个`有状态的中间操作`，也就是说，在处理新元素时，它可能会合并来自之前看到的元素的状态。有状态的中间操作可能需要在产生结果之前处理整个输入。例如，在看到流的所有元素之前，无法对流进行排序。
...
如果 `n` 为负，函数抛出`IllegalArgumentException`。
