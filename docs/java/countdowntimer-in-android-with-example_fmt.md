...
> 原文：[https://www.geeksforgeeks.org/countdowntimer-in-android-with-example/](https://www.geeksforgeeks.org/countdowntimer-in-android-with-example/)

CountDownTimer 应用程序是关于设置一个时间，它以相反的顺序移动，因为它显示了即将到来的事件的剩余时间。`CountDownTimer` 是一种精确的计时器，可用于网站或博客，显示任何特殊事件的倒计时，如生日或周年纪念日。同样，这里让我们创建一个[安卓应用](https://www.geeksforgeeks.org/introduction-to-android-development/)来学习如何创建一个简单的`倒计时`应用。因此，让我们开始一步一步地创建应用程序，直到它完成。
...
`PulseCountdown` 中的 [安卓](https://www.geeksforgeeks.org/introduction-to-android-development/) 是 `CountDownTimer` 的替代品。用 `PulseCountdown` 代替 `CountDownTimer` 非常容易实现，因为 `PulseCountdown` 提供了一个默认布局，带有一些漂亮的动画。默认情况下 `PulseCountdown` 的 `起始值` 为 10，而 `结束值` 为 0。假设有需要一个 `竞猜 app`，而在那个加时限回答问题的地方有 `PulseCountdown` 可以使用。要在安卓系统中实现 `PulseCountdown` 请参考 [`PulseCountdown` 并举例说明。](https://www.geeksforgeeks.org/pulsecountdown-in-android-with-example/)
