...
Java 中的局部变量是一个在方法体中声明的变量。那么您只能在该方法中使用该变量。该类中的其他方法甚至不知道该变量的存在。如果我们声明一个局部变量，那么我们应该在使用它之前在块中初始化它。在局部变量的情况下，JVM 不会提供任何默认值。

当你“不小心”试图修改一个值时，一个最终局部变量会作为一个警告，并向编译器提供信息，从而可以更好地优化类文件。
...
1.  **变量的初始化不是强制性的**：即使局部变量是 `final`，我们也只在想要使用它时才执行初始化，即如果我们不使用它，那么即使它是 `final`，也不需要执行初始化。
...
2.  **`final` 是局部变量唯一适用的修饰符**：局部变量唯一适用的修饰符是 `final`。如果我们错误地尝试应用任何其他修饰符，我们将得到编译时错误。
