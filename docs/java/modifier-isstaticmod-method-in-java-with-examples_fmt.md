...
> 原文:[https://www.geeksforgeeks.org/modifier-isstaticmod-method-in-java-with-examples/](https://www.geeksforgeeks.org/modifier-isstaticmod-method-in-java-with-examples/)

`java.lang.reflect.Modifier`的 `isStatic(int mod)` 方法用于检查整数参数是否包含静态修饰符。如果此整数参数表示静态类型修饰符，则方法返回 `true`，否则返回 `false`。
...
## 返回:
如果 `mod` 包含静态修饰符，则该方法返回 `true` 否则为假。
