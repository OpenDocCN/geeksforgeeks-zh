# Java中CharArrayReader markSupported()方法示例
...
Java中`CharArrayReader`类的`markSupported()`方法用来检查这个`CharArrayReader`是否支持`mark()`操作。它返回一个布尔值，该值表示读取器是否被标记为受支持。
...
这个方法返回一个布尔值，告诉这个`CharArrayReader`是否支持`mark()`操作。如果它是`markSupported`，则返回`true`。否则返回`false`。
...
[https://docs.oracle.com/javase/9/docs/api/java/io/CharArrayReader.html#markSupported--](https://docs.oracle.com/javase/9/docs/api/java/io/CharArrayReader.html#markSupported--)