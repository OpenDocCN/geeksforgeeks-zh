...
> 原文：[https://www.geeksforgeeks.org/how-to-check-if-linkedhashmap-contains-a-value-in-java/](https://www.geeksforgeeks.org/how-to-check-if-linkedhashmap-contains-a-value-in-java/)
...
使用 For-each 循环遍历 `LinkedHashMap`。创建 `check()` 函数检查 `LinkedHashMap` 中是否存在任何值。如果我们得到任何返回值 `true`，则迭代 `LinkedHashMap`，否则返回 `false`。
...
使用函数 `containsValue()` 来查找我们的值是否存在于我们的 `LinkedHashMap` 中。
...
**参数：** 该方法只取一个 `Object` 类型的参数 `value`，并引用其映射应该由映射内任何键检查的值。
