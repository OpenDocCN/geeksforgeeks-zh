...
Java 没有 `goto` 语句，因为它提供了一种以任意和非结构化方式进行分支的方法。这通常会使 `goto` 代码难以理解和维护。它还禁止某些编译器优化。然而，在一些地方，`goto` 对于流控制来说是一个有价值且合法的构造。例如，当您退出一组深度嵌套的循环时，`goto` 会很有用。为了处理这种情况，Java 定义了 `break` 语句的扩展形式。
