...
> 原文：[https://www.geeksforgeeks.org/how-to-sort-a-treeset-with-user-defined-objects-in-java/](https://www.geeksforgeeks.org/how-to-sort-a-treeset-with-user-defined-objects-in-java/)

[`Comparator`](https://www.geeksforgeeks.org/comparator-interface-java/) 接口用于对自定义类的对象进行排序。`Comparator` 类的一个对象能够比较两个不同类的对象。以下功能比较 `obj1` 和 `obj2`。

*   `TreeSet` 实现 `SortedSet` 接口。因此，不允许重复值。
*   `TreeSet` 中的对象以排序和升序存储。
*   `TreeSet` 不保留元素的插入顺序，但元素按键排序。
*   如果我们依赖于默认的自然排序顺序，那么插入到树中的对象应该是同类的和可比较的。`TreeSet` 不允许插入异类对象。它会抛出一个 `ClassCastException`。

由于 `TreeSet` 实现了 `SortedSet` 接口，因此元素默认按升序排序。但是如果我们想在排序中进行改变，比如按照递减的顺序进行排序，或者按照姓名、分数、工资（这些都是用户定义的类）进行排序，比如任何方式。

所以解决这一切的办法就是我们可以通过显式实现 `Comparator` 类来进行排序。
...
*   `Comparator` 接口用于对自定义类的对象进行排序。
*   该接口存在于 `java.util` 包中，包含 2 个方法 `compare(Object obj1, Object obj2)` 和 `equals(Object element)`。
