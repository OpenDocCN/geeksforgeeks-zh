# Java中的java.util.zip.DeflaterOutputStream类

> 原文：[https://www.geeksforgeeks.org/java-util-zip-deflateroutputstream-class-java/](https://www.geeksforgeeks.org/java-util-zip-deflateroutputstream-class-java/)
> [Java中的java.util.zip.DeflaterInputStream类](https://www.geeksforgeeks.org/java-util-zip-deflaterinputstream-class-java/)

这个类实现了一个输出流过滤器，用于以“deflate”压缩格式压缩数据。它还被用作其他类型的压缩过滤器的基础，例如`GZIPOutputStream`。

## 构造器及说明

*   `DeflaterOutputStream(OutputStream out)`: 使用默认压缩器和缓冲区大小创建新的输出流。
...
**注意：** 由于无法在此处读取`file2.txt`，因此在线IDE上将看不到程序的输出。
