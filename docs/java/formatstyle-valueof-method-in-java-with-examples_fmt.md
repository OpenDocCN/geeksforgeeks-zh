...
> 原文: [https://www.geeksforgeeks.org/formatstyle-valueof-method-in-java-with-examples/](https://www.geeksforgeeks.org/formatstyle-valueof-method-in-java-with-examples/)

`FormatStyle` 枚举的 `valueOf()` 方法用于获取具有指定名称的该类型 `FormatStyle` 的枚举值。
...
该方法接受一个 `name` 作为参数，该参数是要返回的枚举常量的名称。
