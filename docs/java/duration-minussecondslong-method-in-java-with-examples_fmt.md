...
`java.time`包中`Duration`类的`minusSeconds(long)`方法用于获取该持续时间的不变副本，减去指定的秒数，作为参数传递。
...
## 参数:
该方法接受一个参数`numberOfSeconds`，这是要减去的秒数。它可以是正数或负数，但不能为空。

## 返回值:
该方法返回一个`Duration`，它是一个现有持续时间的不可变副本，并减去了参数秒数。
...
## 参考:
[https://docs . Oracle . com/javase/9/docs/API/Java/time/duration . html # mins seconds-long-](https://docs.oracle.com/javase/9/docs/api/java/time/Duration.html#minusSeconds-long-)