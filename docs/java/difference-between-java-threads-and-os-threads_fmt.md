...
> 原文: [https://www.geeksforgeeks.org/difference-between-java-threads-and-os-threads/](https://www.geeksforgeeks.org/difference-between-java-threads-and-os-threads/)
...
| 死锁检测 | 1) 如果资源只有一个实例 2) 如果有多个资源实例 | 1) 嵌套同步块或试图获取不同对象的锁，或从另一个同步方法调用同步方法 2) 使用 `jstack`。它允许我们上传一个线程转储并分析它。3) 也可以使用 `jConsole` 或 `VisualVM` 来检测死锁 |
| 避免死锁 | 可以用银行家算法完成。 | 1) 避免不必要的锁定 2) 避免嵌套锁 3) 使用 `Thread.join()` 方法 4) 使用锁排序 5) 锁定超时 |
...
> **注意:** 从 1.3 版本开始，`Linux` 的 `Sun JVM` 中放弃了 `green threads`，使用由 OS 管理的 `native threads`。详见 [本](https://stackoverflow.com/questions/5713142/green-threads-vs-non-green-threads)。