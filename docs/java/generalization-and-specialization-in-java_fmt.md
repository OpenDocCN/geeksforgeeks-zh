...
> 原文: [https://www.geeksforgeeks.org/generalization-and-specialization-in-java/](https://www.geeksforgeeks.org/generalization-and-specialization-in-java/)
...
*   `Vehicle` 和 `Bus` 引用**可以相互类型转换。**
*   `Vehicle` 和 `Car` 的引用**可以相互进行类型转换。**
*   `Vehicle` 和 `Truck` 的引用**可以相互进行类型转换。**
*   `Bus`、`Car` 和 `Truck` 的引用**不能相互进行类型转换。**

## 泛化

将子类类型转换为超类类型称为“**泛化**”，因为我们正在使子类变得更加通用，其范围也在不断扩大。这也被称为**加宽或向上转换**。加宽是安全的，因为类将变得更加通用。
比如我们说 `Car` 是 `Vehicle`，就不会有异议。因此，Java 编译器一般不会要求强制转换运算符。
...
将超类类型转换为子类类型称为“**专门化**”。在这里，我们从更一般的形式下降到特定的形式，因此范围缩小了。因此，这被称为**缩小**或**向下转换**。
收窄是**不安全**的，因为类会越来越具体从而引起越来越多的质疑。例如，如果我们说 `Vehicle` 是一辆 `Car`，我们需要一个证据。因此，在这种情况下，Java 编译器特别要求强制转换。这叫做**显式转换**。
