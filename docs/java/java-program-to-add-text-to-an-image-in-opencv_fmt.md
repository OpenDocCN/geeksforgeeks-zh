...
> 原文: [https://www.geeksforgeeks.org/java-program-to-add-text-to-an-image-in-opencv/](https://www.geeksforgeeks.org/java-program-to-add-text-to-an-image-in-opencv/)
...
| 参数的数据类型 | 参数名称 | 功能 |
| --- | --- | --- |
| `Mat` 对象 | `image` | 要添加到输入图像对象的文本 |
| `String` | `text` | 要插入到输入图像中的文本 |
| `Point` (元组) | `org` | 图像中文本字符串左下角的坐标 |
| `int` | `fontType` | 描绘字体的风格 |
| `double` | `fontSize` | 要添加到输入图像上的文本大小 |
| `Scalar` | `color` | 要在输入图像上绘制的文本字符串的颜色 |
| `int` | `thickness` | 单位中线条的粗细，默认情况下是统一的 |
