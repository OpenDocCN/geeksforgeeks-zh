...
> 原文: [https://www.geeksforgeeks.org/introduction-to-hibernate-framework/](https://www.geeksforgeeks.org/introduction-to-hibernate-framework/)
...
1.  `JDBC` 代码依赖于所使用的数据库软件，即我们的持久化逻辑因为使用 `JDBC` 而具有依赖性。这里我们正在向 `Employee` 表插入记录，但我们的查询依赖于数据库软件，即这里我们使用的是 `MySQL`。但如果我们更换数据库，这个查询将无法工作。

    ![](img/aecec3acb4471a47fa1782490e7f00ee.png)

2.  如果与 `JDBC` 合作，在项目过程中更改数据库的成本非常高。
3.  `JDBC` 代码不是跨多个数据库软件的可移植代码。
4.  在 `JDBC` 中，异常处理是强制性的。这里我们可以看到，我们正在为连接处理大量的异常。

    ![](img/41bad8d22874e74650ec57d8382bc46c.png)

5.  与 `JDBC` 合作时，没有支持对象级关系。
6.  在 `JDBC` 中，存在样板代码问题，即对于每个项目，我们都必须编写下面的代码。这增加了代码长度并降低了可读性。
