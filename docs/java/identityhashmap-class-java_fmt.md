...
实现 `Serializable`、`Cloneable`、`Map<K, V>` 接口，扩展 `AbstractMap<K, V>` 类。
...
要移除映射，我们使用 [`remove()`](https://www.geeksforgeeks.org/identityhashmap-remove-method-in-java/)，一种内置的 `IdentityHashMap` 类方法，用于从映射中移除任何特定键的映射。
