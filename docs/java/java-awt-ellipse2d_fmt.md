...
*   `Ellipse2D.Float` 以浮动精度定义椭圆。

## 该类的构造函数为:

1.  `Ellipse2D.Double()`: 在位置 0，0 和大小 0，0 处创建一个椭圆。
2.  `Ellipse2D.Double(double x, double y, double w, double h)`: 在 x，y 位置以及宽度 w 和高度 h 创建一个椭圆。
3.  `Ellipse2D.Float()`: 在位置 0，0 和大小 0，0 处创建一个椭圆。
4.  `Ellipse2D.Float(float x, float y, float w, float h)`: 在位置 x，y，宽度 w 和高度 h 创建一个椭圆。

## 常用方法

| 方法 | 说明 |
| --- | --- |
| `contains(double x, double y)` | 返回该点是否在椭圆内 |
| `contains(double x, double y, double w, double h)` | 返回矩形是否在椭圆内 |
| `equals(Object o)` | 确定指定的对象是否等于此 `Ellipse2D`。 |
| `intersects(double x, double y, double w, double h)` | 返回矩形是否与椭圆相交 |
| `setFrame(double x, double y, double w, double h)` | 将此形状的框架矩形的位置和大小设置为指定的矩形值。 |

下面的程序说明了 `Ellipse2D` 类:

## 1. Java program to create two ellipses and draw them to a java applet

To create ellipse shape on Java applet, we will initialize `Ellipse2D` class objects named `ed` and `ed1`. The 4 parameters passed in the constructor of the `ed` object are the X coordinate of the upper-left corner of the framing rectangle, the Y coordinate of the upper-left corner of the framing rectangle, the width of the framing rectangle and the height of the framing rectangle. In the constructor of `ed`, we pass nothing, which means the ellipse is initialized to location (0, 0) and size (0, 0). To show them on screen, we create an object of `Graphics2D` class `g1` and call `g1.draw()` method.
