...
将多个 PDF 文档合并成一个 PDF 文档是使用 `PDFMergerUtility` 类完成的。这个类将把一个 PDF 文档列表合并，结果将保存到新文档中。这个题目的前提是你已经安装了 [**阿帕奇 PDFBox 库**](https://pdfbox.apache.org/download.cgi)。

**按照这些步骤合并多个 PDF 文档：**

1.  首先，我们必须实例化 `PDFMergerUtility` 类。
