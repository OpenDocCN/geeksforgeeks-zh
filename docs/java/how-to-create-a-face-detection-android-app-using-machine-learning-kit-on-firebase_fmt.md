# 如何在 Firebase 上使用机器学习工具包创建人脸检测安卓应用

> 原文: [https://www.geeksforgeeks.org/how-to-create-a-face-detection-android-app-using-machine-learning-kit-on-firebase/](https://www.geeksforgeeks.org/how-to-create-a-face-detection-android-app-using-machine-learning-kit-on-firebase/)
...
`Firebase ML KIT` 旨在通过提供一系列可以在 iOS 和安卓应用中使用的预训练模型，让机器学习变得更加容易。让我们使用 ML Kit 的人脸检测 API 来识别照片中的人脸。在本文的最后，我们将有一个应用程序，可以识别图像中的人脸，然后显示关于这些人脸的信息，例如这个人是否在微笑，或者他们的眼睛是否闭上了美妙的图形用户界面。
...
1.  登录或注册 [Firebase 控制台](https://console.firebase.google.com/)。
...
*   要增强图形用户界面，请选择 `.png` 格式，并将其添加到 `res` 文件夹中，并将其设置为 `main.xml` 文件的背景，或者进入布局的设计视图，在**声明属性**下自定义背景，设置背景颜色，如下图:
...
*   现在转到**布局 -> 新建 -> 布局资源文件 -> 名称: `fragment_resultdialog.xml`**。这个文件是为了自定义输出屏幕而创建的，它将显示一个名为结果对话框的对话框，其中有一个名为 `result_text` 的文本视图，包含检测到的图像的所有属性。下面是创建的 XML 文件的 XML 文件。
...
*   创建一个新的 java 类，即 `ResultDialog.java` 和超类，`DialogFragment`，这是 `fragment_resultdialog.xml` 的 java 文件。下面是 java 文件的示例代码。
...
| 环境 | 描述 |
| --- | --- |
| 性能模式 | 快速(默认) \| 准确 在检测人脸时，更注重速度或准确性。 |
| 检测地标 | 无地标(默认) \| 所有地标 是否尝试识别面部“地标”:眼睛、耳朵、鼻子、脸颊、嘴巴等等。 |
| 检测轮廓 | 无轮廓(默认) \| 所有轮廓 是否检测面部特征的轮廓。仅对图像中最突出的面部检测轮廓。 |
| 分类人脸 | 无分类(默认) \| 所有分类 是否将人脸分类比如“微笑”和“睁大眼睛”。 |
| 最小表面尺寸 | 浮点 (默认值:0.1f) 要检测的人脸相对于图像的最小尺寸。 |
| 启用面部跟踪 | false(默认) \| true 是否给人脸分配一个标识可用于跟踪图像中的人脸。请注意，当启用轮廓检测时，只检测到一张脸，所以人脸跟踪不会产生有用的结果。出于这个原因，并加以改进检测速度，不要同时启用轮廓检测和人脸跟踪。 |
