...
> 原文：[https://www.geeksforgeeks.org/interpolator-in-android-with-example/](https://www.geeksforgeeks.org/interpolator-in-android-with-example/)
...
**设置对象动画师对象：** 在本例中，我们将使用单个 `ObjectAnimator` 来激活不同的按钮。我们还将使用 2 秒的固定持续时间来播放动画，这给了我们足够的时间来观察动画行为。您可以用下面的两行代码来设置它。
