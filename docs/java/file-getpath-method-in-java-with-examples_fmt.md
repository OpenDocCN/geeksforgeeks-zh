...
`getPath()` 方法是 [`File`](https://www.geeksforgeeks.org/File-class-in-java/) 类的一部分。这个函数返回给定文件对象的路径。该函数返回一个包含给定文件对象路径的字符串对象。
...
## 返回值:
该函数返回一个字符串值，该值是给定文件对象的路径。

下面的程序将说明 `getPath()` 函数的使用:

## 例 1:
给我们一个文件的文件对象，我们要得到文件对象的路径。
