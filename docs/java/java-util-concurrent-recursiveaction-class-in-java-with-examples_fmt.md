...
> 原文：[https://www.geeksforgeeks.org/java-util-concurrent-recursiveaction-class-in-java-with-examples/](https://www.geeksforgeeks.org/java-util-concurrent-recursiveaction-class-in-java-with-examples/)

## 简介

`RecursiveAction` 是一个抽象类，封装了一个不返回结果的任务。它是 `ForkJoinTask` 的一个子类，这是一个抽象类，代表了一个可以在多核系统中独立内核上执行的任务。`RecursiveAction` 类被扩展以创建一个具有 `void` 返回类型的任务。代表任务计算部分的代码保存在 `RecursiveAction` 的 `compute()` 方法中。
...
1.  **`RecursiveAction()`**：使用默认设置创建 `RecursiveAction` 对象。
...
[https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/RecursiveAction.html](https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/RecursiveAction.html)