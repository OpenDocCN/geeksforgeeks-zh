...
`IntStream.asDoubleStream()` 返回一个由该流的元素组成的 `DoubleStream`，转换为 double。这是一个 ***的中级操作。*** 在流实例上调用中间操作，在它们完成处理后，它们给出一个流实例作为输出。
...
```
Where, `DoubleStream` is a sequence of primitive double-valued element.
Return Value : `IntStream.asDoubleStream()` returns a `DoubleStream` consisting of the elements of this stream, converted to double.
