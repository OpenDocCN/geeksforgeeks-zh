...
> 原文：[https://www.geeksforgeeks.org/java-program-to-display-current-hour-and-current-minute/](https://www.geeksforgeeks.org/java-program-to-display-current-hour-and-current-minute/)
...
它用于在 java 中显示日期和时间以及操作日期和时间，除此之外，它还用于在 java 中格式化跨时区关联数据的日期和时间类。所以为了从一个名为 `java.util` 的包中导入这个类。导入该类后，可以创建日期类的对象，以便打印当前日期和时间。现在为了打印默认的日期和时间只需调用打印命令使用 `toString()` 方法获取当前的日期和时间。
...
这里，`field` 代表给定的日历字段，函数返回给定字段的值。

### 接近

1.  使用 `Calendar` 类搭配 `格式说明符`
2.  使用 `Calendar` 类不带 `格式说明符`

## 方法 1：使用带有格式说明符的日历类

`get()` 方法是 `java.util.Calendar` 类的一个方法。`Calendar` 类提供了一些在包外实现具体日历系统的方法。格式说明符以百分比字符（`%`）开头，以表示数据类型（`int`、`float` 等）的“类型字符”结尾。将以表示数据的基本方式（十进制、十六进制等）进行转换。
...
class GFG {
    ...
    public static void main(String args[])
    {
        ...
        // Displaying hour using Format class using format specifiers
