...
> 原文：[https://www.geeksforgeeks.org/abstractcollection-remove-method-in-java-with-examples/](https://www.geeksforgeeks.org/abstractcollection-remove-method-in-java-with-examples/)

[Java 抽象集合](https://www.geeksforgeeks.org/abstractcollection-in-java-with-examples/) 的 `remove(Object O)` 方法是从集合中移除特定元素。
...
参数 `O` 属于集合类型，指定要从集合中移除的元素。

## 返回值

如果参数中指定的元素最初出现在集合中并被成功移除，则该方法返回 `true`，否则返回 `false`。

下面的程序说明了 `java.util.AbstractCollection.remove()` 方法：
