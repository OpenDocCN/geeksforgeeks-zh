...
> 原文: [https://www.geeksforgeeks.org/java-program-to-change-a-collection-to-an-array/](https://www.geeksforgeeks.org/java-program-to-change-a-collection-to-an-array/)

数组是包含元素的线性数据结构，元素的大小是在创建时定义的，并且可以保存对象和原始同类数据。集合是一个预定义的类，只包含异构对象类型，但包含原语。方法可用于将一个集合更改为 java 中的数组。以下方法可用于将集合转换为数组:

1.  使用 [`list.add()`](https://www.geeksforgeeks.org/list-add-method-in-java-with-examples/) 方法
2.  使用 [`list.toArray()`](https://www.geeksforgeeks.org/list-array-java/) 方法
...
*   [`IndexOutOfBoundsException`](https://www.geeksforgeeks.org/understanding-array-indexoutofbounds-exception-in-java/): 当索引超出范围时抛出。
...
是 [`List`](https://www.geeksforgeeks.org/list-interface-java-examples/) 接口中存在的一种方法，将列表中的所有元素按顺序作为数组返回。
...
*   它覆盖了类 `AbstractCollection` 中的 `toArray`
