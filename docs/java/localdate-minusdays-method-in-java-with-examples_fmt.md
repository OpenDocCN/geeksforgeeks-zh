...
> 原文：[https://www.geeksforgeeks.org/localdate-minusdays-method-in-java-with-examples/](https://www.geeksforgeeks.org/localdate-minusdays-method-in-java-with-examples/)
...
该方法接受单个参数 `daysToSubtract`，表示要减去的天数，可以是负数。

## 返回值

该方法返回基于该日期减去天数的 `LocalDate`，不为空。

## 异常

如果结果超出支持的日期范围，该方法抛出 `DateTimeException`。

以下程序说明了 `minusDays()` 方法：
...
[https://docs.oracle.com/javase/10/docs/api/java/time/LocalDate.html#minusDays(long)](https://docs.oracle.com/javase/10/docs/api/java/time/LocalDate.html#minusDays(long))