...
> 原文: [https://www.geeksforgeeks.org/how-to-change-default-user-and-password-in-spring-security/](https://www.geeksforgeeks.org/how-to-change-default-user-and-password-in-spring-security/)
...
现在打开一个合适的 `IDE`，然后转到 `文件 > 从现有资源 > 新建 > 项目` 并选择 `pom.xml`。在提示符下单击导入更改并等待项目同步，如下图所示:
...
转到 `src > main > java > com.gfg.spring.boot.app`，运行主应用 `SpringBootAppApplication.java`。
...
`Spring security` 生成默认密码。为了覆盖，我们必须在 `applications.properties` 文件中配置我们自己的用户名和密码。
...
转到 `src > main > java > com.gfg.spring.boot.app`，创建两个 `Java` 文件，一个是 `controller.java`，另一个是 `config.java`。
...
`WebSecurityConfigureAdapter` 类用于配置传入的请求。主要有两种方法用于配置。第一种方法用于为 `spring` 应用服务器添加角色，另一种方法用于根据角色区分请求。现在运行 `spring` 应用程序的主应用程序。
...
去找邮递员，输入 `localhost:8080/delete`。
