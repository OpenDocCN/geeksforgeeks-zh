...
> 原文：[https://www.geeksforgeeks.org/edittext-widget-in-android-using-java-with-examples/](https://www.geeksforgeeks.org/edittext-widget-in-android-using-java-with-examples/)
...
*   现在我们将在 `MainActivity.java` 文件中添加代码，使我们的布局具有交互性或响应性。我们的应用程序将在点击按钮时生成一个带有文本“**欢迎来到 geeks forgeeks【*****用户输入的名称*****】**”的[祝酒词](https://www.geeksforgeeks.org/android-what-is-toast-and-how-to-use-it-with-examples/)
...
| --- | --- |
| `android:id` | 用于唯一标识控件 |
| `android:gravity` | 用于指定如何对齐文本，如左、右、中、上等。 |
| `android:hint` | 用于在文本为空时显示提示文本 |
| `android:text` | 用于设置编辑文本的文本 |
| `android:textSize` | 用于设置文本的大小。 |
| `android:textColor` | 用于设置文本的颜色。 |
| `android:textStyle` | 用于设置文本的样式。例如，粗体、斜体、粗斜体等。 |
| `android:textAllCaps` | 使用此属性以大写字母显示文本。 |
| `android:width` | 它使文本视图正好有这么多像素宽。 |
| `android:height` | 它使文本视图正好有这么多像素高。 |
| `android:maxWidth` | 用于使文本视图最多有这么多像素宽。 |
| `android:minWidth` | 用于使文本视图至少有这么多像素宽。 |
| `android:background` | 用于设置此视图的背景。 |
| `android:backgroundTint` | 用于设置此视图背景的色调。 |
| `android:clickable` | 用于当您想要使此视图可点击时设置为真。否则，设置为 `false`。 |
| `android:drawableBottom` | 用于在此视图中将可绘制设置为文本的底部。 |
| `android:drawableEnd` | 用于在此视图中将可绘制设置为文本的结尾。 |
| `android:drawableLeft` | 用于在此视图中将可绘制设置在文本的左侧。 |
| `android:drawablePadding` | 用于将填充设置为可在视图中绘制。 |
| `android:drawableRight` | 用于在此视图中将可绘制设置为文本的右侧。 |
| `android:drawableStart` | 用于在此视图中将可绘制设置为文本的开头。 |
| `android:drawableTop` | 用于在此视图中设置可绘制到文本的顶部。 |
| `android:elevation` | 用于设置此视图的高程。 |