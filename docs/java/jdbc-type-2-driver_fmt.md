...
一个 [`JDBC`](https://www.geeksforgeeks.org/jdbc-drivers/) 驱动程序使 Java 应用程序能够与数据库交互，我们可以从中获取或存储数据。JDBC 驱动程序类似于 `ODBC` 驱动程序。`JDBC` 类包含在 Java 包中，其功能包括：
...
## 类型 2 或 Partial Java 驱动程序

也称为 `Native API`，将 `JDBC` 调用转换为特定于数据库的本机库调用，这些调用直接被数据库引擎理解。这种类型的驱动程序将 `JDBC` 调用转换为对 `Oracle`、`Sybase`、`Informix`、`DB2` 或其他数据库管理系统的客户端 `API` 的调用。
