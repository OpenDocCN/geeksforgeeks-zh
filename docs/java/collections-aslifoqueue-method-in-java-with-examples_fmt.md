# Java 中的集合 `asLifoQueue()` 方法示例

> 原文：[https://www.geeksforgeeks.org/collections-aslifoqueue-method-in-java-with-examples/](https://www.geeksforgeeks.org/collections-aslifoqueue-method-in-java-with-examples/)

`Collections` 类的 `asLifoQueue()` 方法用于返回一个作为后进先出（LIFO）队列的视图。方法 `add` 被映射到 `push`，`remove` 被映射到 `pop` 等等。当您想要使用需要队列的方法，但又需要后进先出排序时，这个视图会很有用。
...
## 参数
该方法以 `Deque` 为参数，将其转换为后进先出队列。

## 返回值
该方法从 `deque` 返回一个 `LifoQueue`。
