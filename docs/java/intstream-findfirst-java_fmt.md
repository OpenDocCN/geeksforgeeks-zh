...
其中，`OptionalInt` 是一个容器对象，它可能包含也可能不包含一个非空值。该函数返回一个描述此流第一个元素的 `OptionalInt`，如果流为空，则返回一个空的 `OptionalInt`。

## 注意

`findFirst()` 是 `Stream` 接口的一个 **端子-短路** 操作。此方法返回满足中间操作的任何第一个元素。
