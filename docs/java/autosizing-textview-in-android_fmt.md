...
> 原文：[https://www.geeksforgeeks.org/autosizing-textview-in-android/](https://www.geeksforgeeks.org/autosizing-textview-in-android/)
...
*   将应用程序名称重命名为 `Auto Size TextView`，布局名称为 `activity_main.xml`。
*   并选择 `Java` 作为语言。

**步骤 2：通过使用 `activity_main.xml` 文件构建一个合适的布局**
...
*   在上面的代码中可以看到 `TextView` 的属性：`app:autoSizeMaxTextSize="80sp"`，这是 `TextView` 的初始大小。
*   `app:autoSizeMinTextSize="10sp"` 使用此属性，`TextView` 的大小将被调整到 `10sp` 的大小，`app:autoSizeStepGranularity="2sp"` 使用此属性，当 `TextView` 退出屏幕时，我们将它的大小统一减少为 `2sp`。
*   `app:autoSizeTextType="uniform"` 通过使用这个属性，我们只是统一调整了 `TextView` 的大小。
*   需要记住的一点是，不应该将文本视图的高度和宽度设置为 `wrap_content`。
*   要禁用文本视图的大小调整，可以将此属性设置为 `none`。如下图所示。
...
**步骤 3：现在使用 `onClick` XML 属性分别处理每个按钮**
