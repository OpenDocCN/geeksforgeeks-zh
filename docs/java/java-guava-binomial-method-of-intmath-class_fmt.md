# Java Guava IntMath 类的 binomial() 方法

> 原文：[https://www.geeksforgeeks.org/java-guava-binomial-method-of-intmath-class/](https://www.geeksforgeeks.org/java-guava-binomial-method-of-intmath-class/)

[Guava 的 `IntMath` 类](https://www.geeksforgeeks.org/intmath-class-guava-java/)的 `binomial(int n, int k)` 方法接受两个参数 `n` 和 `k`，计算二项式系数 $\binom{n}{k}$ 的值。

如果计算值溢出整数的最大值，则该方法返回 `Integer.MAX_VALUE`，或者换句话说，一个整数的最大值。
...
`binomial(int n, int k)` 抛出 `IllegalArgumentException`，如果 `n < 0`，`k < 0` 或 `k > n`。

以下示例说明了 `IntMath` 类的 `binomial()` 方法：
