...
Java 程序创建一个路径并向其添加多个 `LineTo` 对象，并使用 `ClosePath` 对象关闭路径并显示:

*   在这个程序中，我们将创建一个名为 `path` 的路径对象。
*   用指定的 X 和 Y 坐标名称创建四个 `LineTo` 对象 `Lineto`、`Lineto1`、`Lineto2`、`Lineto3`。
*   最初路径没有关闭，所以我们将通过添加名为 `close` 的 `ClosePath` 对象来关闭路径。
*   然后创建一个名为 `moveto` 的 `MoveTo` 对象。
...
[https://docs.oracle.com/javase/8/javafx/api/javafx/scene/shape/ClosePath.html](https://docs.oracle.com/javase/8/javafx/api/javafx/scene/shape/ClosePath.html)