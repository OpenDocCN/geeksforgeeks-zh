...
> 原文: [https://www.geeksforgeeks.org/java-equals-compareto-equalsignorecase-and-compare/](https://www.geeksforgeeks.org/java-equals-compareto-equalsignorecase-and-compare/)
...
双等于（`==`）运算符用于比较两个或两个以上的对象，如果它们指的是同一个对象，则返回 `true`，否则返回 `false`。字符串在 Java 中是不可变的。当创建两个或多个没有 `new` 关键字的对象时，两个对象引用相同的值。双等于运算符实际上比较对象引用。
...
在 Java 中，`String`类的`equals()`方法根据字符串的数据/内容比较两个给定的字符串。如果两个字符串的所有内容都相同，则返回 `true`。如果所有字符都不匹配，则返回 `false`。

![](img/6c51ac83d2290e5a51249e9987e90cff.png)

下面的例子说明了 Java 中字符串比较的 `equals()`:
...
在 Java `Comparable`接口中比较值并返回一个 `int`，这些 `int`值可能小于、等于或大于。Java 比较两个字符串基于字符串中每个字符的 Unicode 值。如果两个字符串不同，则它们在对两个字符串都有效的某个索引处具有不同的字符，或者它们的长度不同，或者两者都不同。
假设索引`'i'`是字符不同的地方，那么`compareTo()`将返回`firstString.charAt(i) - secondString.charAt(i)`。

![](img/2fa64dbd5bd155d58f625423ff5cb39a.png)

下面的例子说明了 Java 中字符串比较的 `compareTo()`:
...
## 方法 4: 使用 `equalsIgnoreCase()`方法

Java `String`类的`equalsIgnoreCase()`方法与`equals()`方法非常相似，除了它忽略了大小写。如上面的示例中，`String`对象`s4`比较`s3`，`equals()`方法返回 `false`，但是在`equalsIgnoreCase()`的情况下，它将返回 `true`。因此`equalsIgnoreCase()`方法不区分大小写。
