# LinkedHashSet contains() 方法详解（Java 示例）

> 原文：[https://www.geeksforgeeks.org/linkedhashset-contains-method-in-java-with-examples/](https://www.geeksforgeeks.org/linkedhashset-contains-method-in-java-with-examples/)

在 Java 中，`LinkedHashSet` 类包含名为 `contains()` 的方法。如果该集合包含指定的元素，则该方法返回 `true`，否则返回 `false`。
...
**返回类型：** 一个布尔值，如果这个集合包含指定的元素，则返回 `true`。
