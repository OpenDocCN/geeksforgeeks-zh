# Java 中的 `length` vs `length()`

> 原文: [https://www.geeksforgeeks.org/length-vs-length-java/](https://www.geeksforgeeks.org/length-vs-length-java/)

**数组长度:** `length` 是适用于[数组](https://www.geeksforgeeks.org/arrays-in-java/)的最终变量。借助 `length` 变量，我们可以得到数组的大小。
...
3.  要直接访问数组的字段成员，我们可以使用 `.length`；鉴于 `.length()` 调用方法来访问字段成员。
...
**说明:** 这里的 `str` 是类型 `String` 的数组，这就是为什么用 `str.length` 来求它的长度。
...
**说明:** 这里的 `str` 是类型 `String` 的数组，这就是为什么 `str.length()` 不能用来求其长度的原因。
...
**解释:** 这里 `str[0]` 指向 `String` 即 `GEEKS`，因此可以使用 `length()` 访问。
