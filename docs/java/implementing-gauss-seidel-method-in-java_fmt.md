...
> 原文：[https://www.geeksforgeeks.org/implementing-gauss-seidel-method-in-java/](https://www.geeksforgeeks.org/implementing-gauss-seidel-method-in-java/)
...
这里，`D` 代表矩阵 `A` 的对角线部分，`-L` 严格来说是 `A` 的下三角部分，`U` 严格来说是 `A` 的上三角部分。

## 编程算法

1.  以方程中的变量个数和增广矩阵（由两个给定矩阵的列相加而成）的值为输入。
2.  检查给定的增广矩阵是否对角占优。
