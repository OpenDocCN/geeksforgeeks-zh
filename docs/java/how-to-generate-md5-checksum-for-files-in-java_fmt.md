...
> 原文：[https://www.geeksforgeeks.org/how-to-generate-md5-checksum-for-files-in-java/](https://www.geeksforgeeks.org/how-to-generate-md5-checksum-for-files-in-java/)
...
> **注意：**
> 1.  文件中的微小变化也可能产生不同的校验和。
> 2.  然而，这些加密哈希函数并不完美。安全研究人员发现了 MD5 和 SHA-1 函数之间的“冲突”。他们找到了两个不同的文件，它们产生了相同的 MD5 或 SHA-1 哈希值，但文件本身是不同的。虽然这种情况偶然发生的可能性极低，但攻击者可以利用这种策略将恶意文件伪装成有效文件。

## 在 Java 中生成校验和

Java 通过 `java.security` 包中的 `MessageDigest` 类提供了生成这些散列函数的内置功能。`MessageDigest` 是加密的单向散列函数，它接受任意大小的数据并产生固定长度的散列值。

1.  我们首先通过传递任何有效的哈希算法字符串来实例化 `MessageDigest` 对象。
2.  然后我们更新这个对象，直到读取完整个文件。虽然我们可以使用 `digest(byte[] input)` 方法通过一次性读取整个文件来完成 `MessageDigest` 对象的最终更新，但当文件过大时，我们可能没有足够的内存将整个文件读入字节数组，这可能导致 `java.lang.OutOfMemoryError: Java heap space` 错误。
3.  因此，最好分段读取数据并更新 `MessageDigest`。

更新完成后，将调用其中一个 `digest` 方法来完成哈希计算。每当调用 `digest` 方法时，`MessageDigest` 对象都会重置为其初始化状态。`digest` 方法返回一个字节数组，该数组包含十进制格式的字节，因此我们将其转换为十六进制格式。最后一个字符串就是校验和。
...
public class GFG {
    ...
    // this method return the complete hash of the file passed
        ...
        throws IOException
    {
        ...
        // since it allows us to update the string i.e. its mutable
