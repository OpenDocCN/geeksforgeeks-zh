...
> 原文：[https://www.geeksforgeeks.org/creating-an-arraylist-with-multiple-object-types-in-java/](https://www.geeksforgeeks.org/creating-an-arraylist-with-multiple-object-types-in-java/)

[`ArrayList`](https://www.geeksforgeeks.org/arraylist-in-java/) 类在 Java 中基本上是一个可调整大小的数组，即它可以根据我们添加到其中的值动态地增长和收缩大小。它存在于 `java.util` 包中。
...
更常见的是创建一个确定类型的数组列表，如 `Integer`、`Double` 等。但是也有一种方法可以创建能够保存多种类型的对象的数组列表。

我们将讨论如何使用 [`Object`](https://www.geeksforgeeks.org/object-class-in-java/) 类来创建数组列表。

`Object` 类是类层次结构的根。Java 中的每个类都在直接或间接地扩展它。如果一个类没有从任何其他类继承，那么它直接扩展 `Object` 类，否则如果它扩展任何类，那么它从它的基类间接扩展 `Object` 类。我们可以使用下面提到的语法使用 `Object` 类来声明我们的数组列表。
