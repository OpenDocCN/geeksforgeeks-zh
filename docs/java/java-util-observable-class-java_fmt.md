# Java中的java.util.Observable类

> 原文：[https://www.geeksforgeeks.org/java-util-observable-class-java/](https://www.geeksforgeeks.org/java-util-observable-class-java/)
...
观察类必须实现 `Observer` 接口，其中定义了 `update()` 方法。

被观察的物体必须遵循两个简单的规则：

1.  如果改变，必须调用 `setChanged()` 方法。
2.  当它准备通知观察者这个变化时，它必须调用 `notifyObservers()` 方法。这导致调用观察对象中的 `update()` 方法。
...
## 4. `notifyObservers()`
通知被观察对象的所有观察者它已经发生了改变，通过调用 `update()` 方法实现。第二个参数传递 `null` 给 `update()`。
...
## 5. `notifyObservers(Object obj)`
通知被观察对象的所有观察者它已经发生了改变，通过调用 `update()` 方法实现。参数 `obj` 会被传递给 `update()`。
...
## 7. `deleteObserver(Observer observer)`
从观察被观察对象的对象列表中移除 `observer`。向此方法传递 `null` 将没有效果。
