...
我们调用`JOptionPane`类的静态`showMessageDialog()`方法来创建消息对话框。我们提供对话框的父项、消息文本、标题和消息类型。消息类型是以下常量之一：
...
3.  `setVisible(…)`：方法用于设置`JFrame`的可见性状态。
