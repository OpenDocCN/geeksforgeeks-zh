...
`DateFormat`类中的`setLenient(boolean leniency)`方法用于指定对该日期格式对象的日期和时间的解释是否宽松。
...
## 参数:
该方法采用布尔类型的一个参数，该参数表示日期格式对象的日历模式。布尔值`true`打开宽大处理模式，`false`关闭宽大处理模式。

## 返回值:
该方法不返回值。

下面的程序说明了`DateFormat`类的`setlained()`方法的工作:
