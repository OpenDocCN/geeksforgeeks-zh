...
`java.lang.Math.negateExact()` 是 Java 中的一个内置函数，它返回参数的否定结果。如果结果溢出了指定的数据类型 `long` 或 `int`，将引发异常，具体取决于在方法参数上使用了哪种数据类型。当数据类型 `int` 的最小值为 `-2147483648`，最大值为 `2147483647` 时，它会引发错误。所以如果我们否定 `-2147483648`，结果将是 `2147483648`，它已经超过了最大值。
...
函数返回参数的否定结果。如果结果溢出指定的数据类型（`long` 或 `int`），则抛出异常，具体取决于方法参数使用了哪种数据类型。
