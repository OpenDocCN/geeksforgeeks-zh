...
`Bytes.indexOf(byte[]数组，byte[]目标)`方法，番石榴的 [`Bytes 类`](https://www.geeksforgeeks.org/bytes-class-guava-java/)接受两个参数 `数组` 和 `目标`。如果目标存在于数组中，该方法返回其第一次出现的开始位置。如果数组中不存在目标，则该方法返回 `-1`。

## 语法:

> `public static int indexOf(byte[]数组，byte[]目标)`
...
[https://Google . github . io/guava/releases/19.0/API/docs/com/Google/common/primitive/bytes . html # indexOf(byte[]，%20byte[])](https://google.github.io/guava/releases/19.0/api/docs/com/google/common/primitives/Bytes.html#indexOf(byte[], %20byte[]))