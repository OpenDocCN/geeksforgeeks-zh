...
*   **mod:** `ArithmeticException` 如果 m <= 0
*   **gcd:** `IllegalArgumentException` 如果 a < 0 或 b < 0
*   **checkedAdd:** `ArithmeticException` 如果 a + b 在有符号整数运算中溢出
*   **checkedSubtract:** `ArithmeticException` 如果 a – b 在有符号整数算法中溢出
*   **checkedMultiply:** `ArithmeticException` 如果 a * b 在有符号整数算术中溢出
*   **checkedPow:** `ArithmeticException` 如果 b 到 k 次方在有符号整数算法中溢出
