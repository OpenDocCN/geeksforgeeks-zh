...
**先决条件:** [`Stream in Java`](https://www.geeksforgeeks.org/stream-in-java/)
...
### 示例 2
`Arrays.stream()` 将 `int` 数组转换为 `stream`。
...
### 示例 3
`Arrays.stream()` 将长数组和双数组转换为 `stream`。
...
## 使用 `Stream.of()`, `IntStream.of()`, `LongStream.of()`, & `DoubleStream.of()`
...
### 示例 2
`IntStream.of()` 将 `int` 数组转换为 `stream`。
...
### 示例 3
`LongStream.of()` 和 `DoubleStream.of()` 将长数组和双数组转换为 `stream`。
