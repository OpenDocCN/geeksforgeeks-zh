# Java 中的 `LongStream reduce(long identity, LongBinaryOperator op)` 示例

> 原文: [https://www.geeksforgeeks.org/longstream-reducelong-identity-longbinaryoperator-op-java/](https://www.geeksforgeeks.org/longstream-reducelong-identity-longbinaryoperator-op-java/)
...
*   `identity`: 累加函数的标识值。
*   `LongBinaryOperator`: 对两个长值操作数进行运算并产生长值结果的操作。
*   `op`: 用于组合两个值的关联无状态函数。
