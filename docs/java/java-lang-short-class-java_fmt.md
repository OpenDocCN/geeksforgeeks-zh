...
*   **`Short(short b)`**：用提供的值创建一个初始化的短对象。
...
*   **`Short(String s)`**：用字符串表示提供的短值创建一个 `Short` 对象。默认基数为 10。
...
2.  **`valueOf()`**：返回用提供的值初始化的 `Short` 对象。
...
3.  另一个重载函数 `valueOf(String val, int radix)`，提供类似于 `new Short(Short.parseShort(val, radix))` 的功能。
...
4.  另一个重载的函数 `valueOf(String s)`，它提供了类似于 `new Short(Short.parseShort(s, 10))` 的功能。
...
5.  **`parseShort()`**：通过解析提供的基数字符串返回短值。与 `valueOf()` 不同，它返回一个简单的短值，而 `valueOf()` 返回一个短对象。
...
6.  另一个重载方法只包含字符串作为参数，默认情况下基数设置为 10。
...
7.  **`decode()`**：返回一个 `Short` 对象，保存所提供字符串的解码值。提供的字符串必须是以下形式，否则将抛出 `NumberFormatException` 异常：
    *   十进制 - `(符号)十进制_数字`
    *   十六进制 - `(符号)"0x"十六进制_数字`
    *   十六进制 - `(符号)"0X"十六进制_数字`
    *   八进制 - `(符号)"0"八进制_数字`
...
8.  **`byteValue()`**：返回该短对象对应的字节值。

```java
Syntax : public byte byteValue()
```

9.  **`shortValue()`**：返回该短对象对应的短值。

```java
Syntax : public short shortValue()
```

10. **`intValue()`**：返回与此短对象对应的 `int` 值。

```java
Syntax : public int intValue()
```

11. **`longValue()`**：返回该短对象对应的长值。

```java
Syntax : public long longValue()
```

12. **`doubleValue()`**：返回该短对象对应的双数值。

```java
Syntax : public double doubleValue()
```

13. **`floatValue()`**：返回该短对象对应的浮点值。

```java
Syntax : public float floatValue()
```

14. **`hashCode()`**：返回该短对象对应的 `hashCode`。

```java
Syntax : public int hashCode()
```

15. **`equals()`**：用于比较两个 `Short` 对象的相等性。如果两个对象包含相同的短值，则此方法返回 `true`。仅当检查是否相等时才应使用。在所有其他情况下，应首选 `compareTo` 方法。
...
16. **`compareTo()`**：用于比较两个 `Short` 对象的数值是否相等。当比较两个数值相等的短数值时，应该使用这种方法，因为它可以区分较小值和较大值。返回小于 0，0 的值，对于小于、等于和大于，返回大于 0 的值。
...
17. **`compare()`**：用于比较两个原始短值的数值是否相等。由于它是一个静态方法，因此可以在不创建任何 `Short` 对象的情况下使用。
...
18. **`reverseBytes()`**：返回一个原语短值，该值反转给定短值的二进制补码形式中的位顺序。
