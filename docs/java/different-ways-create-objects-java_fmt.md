...
> 原文：`https://www.geeksforgeeks.org/different-ways-create-objects-java/`

我们可以用几种方法在 Java 中创建一个类的对象，因为我们都知道一个类提供了对象的蓝图，你可以从一个类创建一个对象。这个概念被低估了，有时被证明是有益的，因为这个概念被许多程序员绕过了，有时甚至在面试中被问到。
...
5.  Constructor class using `newInstance()` method
...
如果我们知道类的名字并且它有一个公共的默认构造函数，我们可以创建一个对象 [`Class.forName()`](https://www.geeksforgeeks.org/new-operator-vs-newinstance-method-java/)。我们可以用它来创建类的对象。`Class.forName()` 实际上在 Java 中加载该类，但不创建任何对象。要创建类的对象，必须使用类的 `newInstance()` 方法。
