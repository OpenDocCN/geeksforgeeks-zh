...
> 原文: [https://www.geeksforgeeks.org/check-for-string-rotation-in-java/](https://www.geeksforgeeks.org/check-for-string-rotation-in-java/)
...
一个**有效的解决方案**是把 `s1` 和它自己连接起来。当且仅当 `s2` 是旋转字符串的子串时，`s2` 是 `s1` 的旋转。在 Java 中，我们可以使用 `String` 类的 `contains()` 方法或者 `indexOf()` 方法来检查子字符串。
