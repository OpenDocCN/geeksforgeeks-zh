...
如前所述，Java 编程语言的`java.net`包包括各种类和接口，它们提供了一种易于使用的方法来访问网络资源。除了类和接口之外，`java.net`包还支持两种著名的网络协议。这些是：
...
> **注：** 可以从 [TCP 和 UDP 的区别](https://www.geeksforgeeks.org/differences-between-tcp-and-udp/)中了解更多关于 TCP 和 UDP 的知识。
...
4.  [`DatagramPacket`](https://www.geeksforgeeks.org/java-net-datagrampacket-class-java/) – `DatagramPacket` 类用于为消息从一个系统到另一个系统的无连接传输提供便利。这个类提供了使用数据报套接字类为无连接传输生成数据报包的工具。
...
6.  [`ServerSocket`](https://www.geeksforgeeks.org/java-net-serversocket-class-in-java/) – `ServerSocket` 类用于实现客户机/服务器套接字连接的服务器端的独立于系统的实现。如果 `ServerSocket` 类的构造函数无法侦听指定的端口，它将引发异常。例如，如果端口已经被使用，它将抛出一个异常。
...
9.  [`Proxy`](https://www.geeksforgeeks.org/java-net-proxy-class-in-java/) – `Proxy` 是一种不变的对象，是一种工具或方法或程序或系统，用来保存其用户和计算机的数据。它的行为就像电脑和互联网用户之间的一堵墙。`Proxy` 对象表示要应用于连接的代理设置。
10. [`URL`](https://www.geeksforgeeks.org/url-class-java-examples/) – Java 中的 `URL` 类是互联网上任何可用资源的入口点。类 `URL` 描述了统一资源定位符，它是万维网上“资源”的信号。源可以表示简单的文件或目录，也可以表示更难的对象，例如对数据库或搜索引擎的查询。
11. [`URLConnection`](https://www.geeksforgeeks.org/java-net-urlconnection-class-in-java/) – Java 中的 `URLConnection` 类是一个抽象类，描述了由类似 `URL` 定义的资源的连接。`URLConnection` 类用于辅助两个不同但相互关联的目的。首先，它提供了对与服务器（尤其是 HTTP 服务器）交互的控制，而不是 `URL` 类。此外，使用 `URLConnection`，用户可以验证服务器传输的标头，并可以做出相应的反应。用户还可以使用 `URLConnection` 配置客户端请求中使用的标头字段。
...
3.  `FileNameMap` – `FileNameMap` 接口是一个简单的接口，它实现了一个工具来概述文件名和 MIME 类型字符串。文件名称映射从数据文件中收取文件名映射（称为模拟表）。
4.  `SocketOptions` – `SocketOptions` 界面帮助用户控制套接字的行为。通常，在套接字中开发必要的特性是非常必要的。`SocketOptions` 允许用户设置各种标准选项。
5.  `SocketImplFactory` – `SocketImplFactory` 接口为 `SocketImpl` 实例定义了一个工厂。套接字类使用它来创建实现各种策略的套接字实现。
6.  `ProtocolFamily` – 该接口代表一个通信协议家族。`ProtocolFamily` 接口包含一个名为 `name()` 的方法，该方法返回协议家族的名称。

## 套接字编程

[**Java Socket 编程**](https://www.geeksforgeeks.org/socket-programming-in-java/) 用于在不同 JRE 上工作的应用程序之间的通信。套接字使用 TCP 实现两台计算机之间的通信工具。Java Socket 编程可以是面向连接的，也可以是无连接的。在套接字编程中，`Socket` 和 `ServerSocket` 类是为面向连接的套接字编程而管理的。然而，`DatagramSocket` 和 `DatagramPacket` 类被用于无连接套接字编程。

客户端应用程序在其通信端生成一个套接字，并努力将该套接字与服务器相结合。当连接建立时，服务器在其通信端生成一个 `Socket` 类的对象。客户端和服务器现在可以通过读写套接字进行通信。
...
1.  **步骤 1** – 服务器实例化一个 `ServerSocket` 对象，指示将在哪个端口号进行通信。
2.  **步骤 2** – 实例化 `ServerSocket` 对象后，服务器请求 `ServerSocket` 类的 `accept()` 方法。该程序暂停，直到客户端连接到给定端口上的服务器。
3.  **步骤 3** – 服务器空闲后，客户端实例化一个 `Socket` 类的对象，定义服务器名称和要连接的端口号。
4.  **步骤 4** – 经过上述步骤后，`Socket` 类的构造函数努力将客户端连接到指定的服务器和端口号。如果通信通过验证，客户端立即拥有一个擅长与服务器交互的套接字对象。
5.  **步骤 5** – 在服务器端，`accept()` 方法返回对服务器上连接到客户端套接字的新套接字的引用。

连接稳定后，可以使用输入/输出流进行通信。`Socket` 类的每个对象都有一个输出流和一个输入流。客户机的输出流与服务器的输入流相关联，客户机的输入流与服务器的输出流相结合。传输控制协议是一种双向通信协议。因此，信息可以在相应的时间通过两个流传输。

# Java Socket编程

## Socket类

`Socket`类用于创建套接字对象，帮助用户实现所有基本的套接字操作。用户可以执行各种网络操作，如发送、读取数据和关闭连接。使用`java.net.Socket`类创建的每个`Socket`对象都专门与一个远程主机相关联。如果用户想要连接到另一个主机，那么他必须构建一个新的套接字对象。

### Socket类方法

在Socket编程中，客户机和服务器都有一个`Socket`对象，因此`Socket`类下的所有方法都可以被客户机和服务器调用。`Socket`类中有很多方法。

| 序号 | 方法 | 描述 |
| --- | --- | --- |
| 1 | `public void connect(SocketAddress host, int timeout)` | 此方法用于将套接字连接到特定的主机。仅当用户实例化套接字时应用了无参构造函数，才需要此方法。 |
| 2 | `public int getPort()` | 此方法用于返回套接字所绑定的远程机器上的端口。 |
| 3 | `InetAddress getInetAddress()` | 此方法用于返回连接到套接字的另一台计算机的地址。 |
| 4 | `public int getLocalPort()` | 此方法用于返回本地机器上套接字加入的端口。 |
| 5 | `SocketAddress getRemoteSocketAddress()` | 此方法返回远程套接字的地址。 |
| 6 | `InputStream getInputStream()` | 此方法用于返回套接字的输入流。输入流与远程套接字的输出流相结合。 |
| 7 | `public OutputStream getOutputStream()` | 此方法用于返回套接字的输出流。输出流与远程套接字的输入流相结合。 |
| 8 | `public void close()` | 此方法用于关闭`Socket`，这将导致套接字类的对象无法再次连接到任何服务器。 |

## ServerSocket类

`ServerSocket`类用于提供客户端/服务器套接字连接的服务器端的独立于系统的实现。如果`ServerSocket`类的构造函数无法侦听指定的端口，它将引发异常。例如，如果端口已经被使用，它将抛出一个异常。
...
| 序号 | 方法 | 描述 |
| --- | --- | --- |
| 1 | `public int getLocalPort()` | 此方法用于返回服务器套接字正在监听的端口。如果用户在构造函数中传递0作为端口号，并要求服务器为他找一个端口，此方法将非常有用。 |
| 2 | `public void setSoTimeout(int timeout)` | 此方法用于设置服务器套接字在`accept()`方法期间挂起时的超时值。 |
| 3 | `public Socket accept()` | 此方法等待传入的客户端。考虑到已使用`setSoTimeout()`方法设置了超时值，此方法将被阻塞，直到客户端在指定端口与服务器连接或套接字超时。否则，此方法将被无限期地阻塞。 |
| 4 | `public void bind(SocketAddress host, int backlog)` | 此方法用于将套接字绑定到`SocketAddress`对象中的特定服务器和端口。如果用户已经使用无参构造函数实例化了服务器套接字，则应使用此方法。 |
...
| 序号 | 方法 | 描述 |
| :--- | :--- | :--- |
| 1 | `getByAddress(byte[])` | 根据给定的原始 IP 地址创建 `InetAddress` 对象。 |
| 2 | `static InetAddress getByAddress(String host, byte[] addr)` | 此方法用于根据给定的主机名和 IP 地址创建 `InetAddress`。 |
| 3 | `static InetAddress getByName(String host)` | 此方法用于在给定主机名时确定主机的 IP 地址。 |
| 4 | `static InetAddress getLocalHost()` | 此方法用于返回本地主机。 |
| 5 | `String getHostName()` | 此方法用于获取 IP 地址的主机名。 |
| 6 | `String getHostAddress()` | 此方法以文本显示形式返回 IP 地址字符串。 |
| 7 | `String toString()` | 此方法用于将 IP 地址转换为字符串。 |

#### InetAddress 类方法示例

下面是 `InetAddress` 类的 Java 实现，用来说明方法的用法：
