...
7.  按位运算符
8.  [移位运算符](https://www.geeksforgeeks.org/shift-operator-in-java/?ref=gcse)
...
该运算符是二元运算符，用 `&` 表示。它逐位返回输入值的与，即如果两位都是 1，它给出 1，否则显示 0。
...
这个运算符是二元运算符，用 `^` 表示。它返回输入值的逐位异或，即如果对应的位不同，它给出 1，否则显示 0。
...
此运算符是一元运算符，用 `~` 表示。它返回输入值的补码表示，即所有位反转，这意味着它将 0 变为 1，将 1 变为 0。
...
> **注意：** 编译器会给出该数的 2 的补码，即 10 的 2 的补码为 -6。
