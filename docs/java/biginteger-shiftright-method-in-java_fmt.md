...
> 原文: [https://www.geeksforgeeks.org/biginteger-shiftright-method-in-java/](https://www.geeksforgeeks.org/biginteger-shiftright-method-in-java/)

**先决条件:** [大整数基础知识](https://www.geeksforgeeks.org/biginteger-class-in-java/)

方法返回一个大整数，其值为 `(this >> n)`。移位距离 `n` 可以是负的，在这种情况下，该方法执行左移。`shiftRight()`方法将一个数字的二进制表示中的每个数字向右移动 `n` 次，并且移位方向上的最后一位被替换为 `0`。这个 `shiftRight()`方法计算 `floor(this / 2^n)`。
...
该方法取整数型的一个参数 `n`，该参数指的是以比特为单位的移位距离。
...
如果移动距离是整数，该方法可能会抛出 `ArithmeticException`。
...
下面的程序说明了 `BigInteger` 的 `shiftRight(int)`方法:
...
[https://docs.oracle.com/javase/7/docs/api/java/math/BigInteger.html#shiftRight(int)](https://docs.oracle.com/javase/7/docs/api/java/math/BigInteger.html#shiftRight(int))