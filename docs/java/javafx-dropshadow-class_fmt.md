...
`DropShadow` 类是 JavaFX 的一部分。`DropShadow` 类创建一个高级效果，以指定的半径、扩散、模糊类型和偏移量呈现内容后面的对象的阴影。`DropShadow` 类继承 `Effect` 类。
...
3.  `DropShadow(double radius, Color color)`：用指定的半径和颜色值创建 `DropShadow` 的新对象。
4.  `DropShadow(double radius, double offsetX, double offsetY, Color color)`：用指定的半径，偏移，偏移和颜色值创建 `DropShadow` 的新对象。
...
| `getColor()` | 返回效果的颜色。 |
| `getInput()` | 返回属性输入的值。 |
| `getRadius()` | 返回 `dropShadow` 效果的半径。 |
| `setBlurType(BlurType v)` | 设置 `dropShadow` 效果的模糊类型。 |
| `setColor(Color v)` | 设置阴影效果的颜色。 |
| `setInput(Effect v)` | 设置属性输入的值。 |
| `setRadius(double v)` | 设置阴影效果的半径。 |
| `setOffsetX(double v)` | 设置 `offsetX` 的值。 |
| `setOffsetY(double v)` | 设置 `offsetY` 的值。 |
| `getOffsetX()` | 返回 `offsetX` 的值。 |
| `getOffsetY()` | 返回 `offsetY` 的值。 |
| `getSpread()` | 返回投影效果的扩散值。 |
| `setSpread(double v)` | 设置投影效果的扩散值。 |
...
**参考：** [https://docs.oracle.com/javase/8/javafx/api/javafx/scene/effect/DropShadow.html](https://docs.oracle.com/javase/8/javafx/api/javafx/scene/effect/DropShadow.html)