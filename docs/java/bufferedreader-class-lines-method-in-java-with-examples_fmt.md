# Java 中的 `BufferedReader` 类 `lines()` 方法，带示例

> 原文: [https://www.geeksforgeeks.org/bufferedreader-class-lines-method-in-java-with-examples/](https://www.geeksforgeeks.org/bufferedreader-class-lines-method-in-java-with-examples/)

`BufferedReader.lines()` 是 Java 库中 `BufferedReader` 类的方法，它根据 `Stream` 从这个缓冲读取器类返回行。在流的帮助下，有很多方法可以根据我们的需要模拟输出。
...
**示例:** 找到包含 "Hello" 作为单词的行，它只会在连续扫描中工作。
