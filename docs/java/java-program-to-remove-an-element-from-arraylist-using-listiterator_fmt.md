...
> 原文:[https://www.geeksforgeeks.org/java-program-to-remove-an-element-from-arraylist-using-listiterator/](https://www.geeksforgeeks.org/java-program-to-remove-an-element-from-arraylist-using-listiterator/)

`ListIterator.remove()` 方法从列表中移除由 `next()` 或 `previous()` 光标位置返回的最后一个元素。每次调用 `next()` 或 `previous()` 后只能调用一次 `remove()`。只有在最后一次调用 `next()` 或 `previous()` 操作之后，还没有调用 `add(E)` 操作时，才能执行该操作。
...
使用列表迭代器从数组列表中移除元素的过程如下:

1.  创建数组列表实例 `new ArrayList<String>()`。
2.  使用颜色在数组列表中添加元素。
3.  创建 `colors.listIterator()` 的 `ListIterator` 实例。
4.  删除元素前打印列表元素。
5.  将迭代器增加 `listIterator.next()` 并移动到要移除的元素。
