...
> 原文：[https://www.geeksforgeeks.org/how-to-make-post-request-in-java-spring/](https://www.geeksforgeeks.org/how-to-make-post-request-in-java-spring/)
...
提取 zip 文件。现在打开一个合适的 IDE，然后转到 **文件 -> 从现有来源新建 -> 项目 -> Spring-boot-app** 并选择 `pom.xml`。在提示符下单击导入更改并等待项目同步。
...
Postman 是一个 API 开发工具，用于帮助构建、测试和修改 API。几乎任何开发人员可能需要的任何功能都封装在这个工具中。每月有 500 多万开发人员使用它，使他们的 API 开发变得简单易行。它能够发出各种类型的 HTTP 请求（`GET`、`POST`、`PUT`、`PATCH`），保存环境以备后用，将 API 转换为各种语言的代码（如 JavaScript、Python）。

现在我们将使用 [Postman](https://www.geeksforgeeks.org/introduction-postman-api-development/) 进行邮寄请求，我们可以以 JSON 的形式发送数据，如下图所示。
