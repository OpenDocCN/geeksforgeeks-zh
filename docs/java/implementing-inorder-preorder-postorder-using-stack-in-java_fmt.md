...
> 原文：[https://www.geeksforgeeks.org/implementing-inorder-preorder-postorder-using-stack-in-java/](https://www.geeksforgeeks.org/implementing-inorder-preorder-postorder-using-stack-in-java/)

两种类型的数据结构：[线性数据结构](https://www.geeksforgeeks.org/overview-of-data-structures-set-1-linear-data-structures/)和[非线性数据结构](https://www.geeksforgeeks.org/overview-of-data-structures-set-2-binary-tree-bst-heap-and-hash/)。这些数据结构之间的主要区别是元素的横向方式。线性数据结构在一次运行中跨越所有元素，而非线性数据结构以层次方式跨越所有元素。
...
**3.** 将当前节点推至 `S`，向左设置 `current = current->` 直到 `current` 为空。

**4.** 如果 `current` 为空，并且堆栈不为空，则

*   从堆栈中弹出顶部项目。
*   打印弹出项，设置 `current = popped_item->right`
*   转到步骤 3。

**5.** 如果 `current` 为空，堆栈为空，那么我们就完成了。
...
1.  创建一个空堆栈 `s`。
2.  将当前节点初始化为根节点。
3.  将当前节点推至 `S`，设置 `current = current->left` 打印堆栈中的 `peek` 元素，直到 `current` 为 `NULL`。
4.  如果 `current` 为空且堆栈不为空，则
    a) 从堆栈中弹出顶部项目。
    b) 设置 `current = popped_item->right`。
    c) 转到步骤 3。
5.  如果 `current` 为空，并且堆栈为空，那么我们就完成了。
...
**2.** 当 `root` 不为空时，执行以下操作

*   推动 `root` 的右子级，然后推动 `root` 进行堆叠。
*   将 `root` 设置为 `root` 的左子级。

**3.** 从堆栈中弹出一个项目，并将其设置为 `root`。

*   如果弹出的项有一个右子项，并且右子项在堆栈顶部，则从堆栈中移除右子项，将 `root` 推回，并将 `root` 设置为 `root` 的右子项。
*   否则打印 `root` 的数据，并将 `root` 设置为 `NULL`。
