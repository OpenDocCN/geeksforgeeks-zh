...
> 原文: [https://www.geeksforgeeks.org/how-to-use-multiline-collapsingtoolbar-library-in-android-app/](https://www.geeksforgeeks.org/how-to-use-multiline-collapsingtoolbar-library-in-android-app/)
...
> **注意:** 在创建新项目时请确保使用安卓支持库，因为 `multiline-collapsingtoolbar` library 不支持 `AndroidX`。
...
导航到 `应用程序 > res > 值 > styles.xml`，并将下面的代码添加到该文件中。下面是 `styles.xml` 文件的代码。
...
转到 `MainActivity.java` 文件并参考以下代码。以下是 `MainActivity.java` 文件的代码。
...
导航到 `应用程序 > 清单 > AndroidManifest.xml` 并在主要活动标签中添加以下代码。
