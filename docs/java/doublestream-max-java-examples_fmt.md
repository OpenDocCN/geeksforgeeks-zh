...
> 原文: [https://www.geeksforgeeks.org/doublestream-max-java-examples/](https://www.geeksforgeeks.org/doublestream-max-java-examples/)

Java 8 中的 `java.util.stream.DoubleStream` 处理原语的双打。它以一种新的方式解决了求数组中的最大值、求数组中的最小值、求数组中所有元素的和、求数组中所有值的平均值等问题。`DoubleStream max()` 返回描述该流最大元素的 `OptionalDouble`，如果该流为空，则返回空的 `OptionalDouble`。
