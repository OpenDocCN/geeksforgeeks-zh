...
> 原文: [https://www.geeksforgeeks.org/matcher-startstring-method-in-java-with-examples/](https://www.geeksforgeeks.org/matcher-startstring-method-in-java-with-examples/)

`Matcher`类的`start(String)`方法用于从指定的字符串中获取已经完成的匹配结果的开始索引。
...
该方法取一个参数`string`，即需要匹配模式的起始索引的字符串。

## 返回值

该方法返回从指定字符串匹配的第一个字符的`索引`。
...
> Current Matcher:java.util.regex.Matcher[pattern=\b(?<GFG>[A-Za-z\s]+) region=0,22 lastmatch=]
> 3

**参考:** [Oracle 文档](https://docs.oracle.com/javase/9/docs/api/java/util/regex/Matcher.html#start-java.lang.String-)