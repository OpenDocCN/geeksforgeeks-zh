...
然而，对于新的和有经验的程序员来说，一个常见的缺点是通过允许不再使用的对象被访问来防止内存被释放。这对于项目来说是一个非常大的缺点，因为内存泄漏会阻塞资源并降低应用程序的性能。甚至会导致[Java.lang.OutOfMemoryError](https://www.geeksforgeeks.org/understanding-outofmemoryerror-exception-java/)。

常见情况有:

*   **静态字段** 声明一个`静态字段`，在不再需要其数据后忘记将其设置为`null`
...
```java
List<String> geeksList = new ArrayList<String>();
```
