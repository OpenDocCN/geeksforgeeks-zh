...
> 原文：[`https://www.geeksforgeeks.org/finding-maximum-element-of-java-hashset/`](https://www.geeksforgeeks.org/finding-maximum-element-of-java-hashset/)
...
*   使用 for 循环（简单方法）
*   使用 `HashSet` 的 [`Collections.max()`](https://www.geeksforgeeks.org/collections-max-method-in-java-with-examples/) 方法（最优方法）
...
1.  创建 `HashSet` 的对象
2.  向创建的对象添加元素
3.  创建一个变量，并赋予其值 [`Integer.MIN_VALUE`](https://www.geeksforgeeks.org/integer-max_value-and-integer-min_value-in-java-with-examples/)
4.  对每个循环重复使用
...
## 方法二：使用 `HashSet` 的 [`Collections.max()`](https://www.geeksforgeeks.org/collections-max-method-in-java-with-examples/) 方法
...
*   [`NoSuchElementException`](https://www.geeksforgeeks.org/java-util-linkedlist-get-getfirst-getlast-java/) 如果集合为空。

**进场：**

1.  创建 `HashSet` 的对象
2.  向创建的对象添加元素
3.  使用 `Collections.max()` 方法显示创建后生成的非重复列表中最大的字符串。注意，不会有单一的重复元素。
