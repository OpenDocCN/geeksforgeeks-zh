...
> 原文: [https://www.geeksforgeeks.org/creating-an-server-client-application-using-the-datagrampacket-and-datagramsocket-classes/](https://www.geeksforgeeks.org/creating-an-server-client-application-using-the-datagrampacket-and-datagramsocket-classes/)
...
**注意:** 要执行 `UDPServerEx` 和 `UDPClient` 类，请在两个单独的命令提示符窗口中运行 `UDPServerEx.java` 和 `UDPClient.java`。请记住，`UDPServerEx` 类在 `UDPClient` 类之前执行。图 1 显示了 `UDP服务器.java` 和 `UDPClient.java` 文件的输出:
