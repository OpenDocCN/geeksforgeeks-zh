...
> 原文: [https://www.geeksforgeks.org/enumeration-interface-in-java/](https://www.geeksforgeks.org/enumeration-interface-in-java/)
...
枚举接口的子接口为 `NamingEnumeration`，实现类为 `StringTokenizer`。
...
| **修改器和类型** | **方法** | **解释** |
| --- | --- | --- |
| 默认迭代器 | [`asIterator()`](https://www.geeksforgeeks.org/enumeration-asiterator-method-in-java-with-examples/) | 这个方法返回一个迭代器，遍历这个枚举覆盖的所有剩余元素。 |
| 布尔 | [`hasMoreElements()`](https://www.geeksforgeeks.org/enumeration-hasmoreelements-method-in-java-with-examples/) | 在实现时，如果有更多的元素要提取或不提取，它将返回布尔值，当所有的元素都被枚举后，它将返回 `false`。 |
| `E` | [`nextElement()`](https://www.geeksforgeeks.org/enumeration-nextelement-method-in-java-with-examples/) | 此方法返回枚举的下一个元素。当没有更多元素时，它会抛出 `NoSuchElementException`。 |