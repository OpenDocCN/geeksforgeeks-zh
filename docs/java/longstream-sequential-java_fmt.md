# Java 中的 LongStream sequential()

> 原文:[https://www.geeksforgeeks.org/longstream-sequential-java/](https://www.geeksforgeeks.org/longstream-sequential-java/)

**`LongStream sequential()`** 返回一个连续的长流。它可能会自己返回，要么是因为流已经是顺序的，要么是因为基础流状态被修改为顺序的。`LongStream sequential()` 是一个 `中间操作`。中间操作在流实例上被调用，在它们完成处理后，它们给出一个流实例作为输出。
