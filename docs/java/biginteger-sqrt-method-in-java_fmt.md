...
> 原文: [https://www.geeksforgeeks.org/biginteger-sqrt-method-in-java/](https://www.geeksforgeeks.org/biginteger-sqrt-method-in-java/)

`java.math.BigInteger.byteValueExact()` 是在 Java SE 9 & JDK 9 中添加的内置函数，该函数返回应用了 `sqrt()` 方法的 `BigInteger` 的平方根的 `BigInteger` 值。它与 `floor(sqrt(n))` 相同，其中 `n` 是一个数字。如果实平方根不能表示为整数值，则该平方根小于实平方根。
