# Java 中的 `DoubleStream filter()` 示例

> 原文: [https://www.geeksforgeeks.org/doublestream-filter-java-examples/](https://www.geeksforgeeks.org/doublestream-filter-java-examples/)

`DoubleStream filter(DoublePredicate predicate)` 返回一个由该流中与给定谓词匹配的元素组成的流。这是一个**中级操作**。这些操作总是懒惰的，即执行诸如 `filter()` 的中间操作实际上并不执行任何过滤，而是创建一个新的流，当遍历该流时，它包含与给定谓词匹配的初始流的元素。
