...
```java
public static <A, B, ..> TupleClass<A, B, ..> with(A a, B b, ..)
```
...
*   **B b** – 代表 B 中第二个值的类型及其在 b 中的对应值。
*   等等。

## 返回值

该方法返回调用该方法的*类*的对象，值作为参数传递。

## 异常

该方法在以下情况下抛出 `RuntimeException`:
