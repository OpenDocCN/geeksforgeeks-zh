...
> 原文: [https://www.geeksforgeeks.org/java-functionconstructor-overloading-puzzle/](https://www.geeksforgeeks.org/java-functionconstructor-overloading-puzzle/)
...
在我们的程序中，两个构造函数都是可访问和适用的。构造函数`GFG(Object)`接受传递给`GFG(double[])`的任何参数，因此`GFG(Object)`不太具体。(每个双数组都是一个对象，但不是每个对象都是双数组。)
