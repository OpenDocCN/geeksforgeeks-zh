...
> 原文：[https://www.geeksforgeeks.org/how-to-perform-crud-operations-in-room-database-in-android/](https://www.geeksforgeeks.org/how-to-perform-crud-operations-in-room-database-in-android/)

应用程序中的数据可以通过不同的方式保存在用户的设备上。我们可以通过 `SQLite` 表、共享首选项等多种方式将数据存储在用户的设备中。在本文中，我们将了解一下在 `Android` 上保存数据、读取、更新和删除房间数据库中的数据。我们将在安卓系统上使用房间数据库执行 `CRUD` 操作。在本文中，我们将了解如何在安卓的房间数据库中执行 `CRUD` 操作。
...
导航到`应用程序 > 渐变脚本 > 构建.gradle`文件，并在依赖项部分添加以下依赖项。

> //在下面添加使用房间的依赖项。
> 
> 实现' `androidx.room:room-runtime:2.2.5` '
> 
> 注释处理器' `androidx.room:room-compiler:2.2.5` '
> 
> //在下面添加使用房间生命周期扩展的依赖项。
> 
> 实现“`androidx.lifecycle:lifecycle-extensions:2.2.0`”
> 
> 注释处理器“`androidx.lifecycle:lifecycle-compiler:2.2.0`”
