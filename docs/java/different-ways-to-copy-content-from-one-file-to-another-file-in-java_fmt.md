...
> 原文：[https://www.geeksforgeeks.org/different-ways-to-copy-content-from-one-file-to-another-file-in-java/](https://www.geeksforgeeks.org/different-ways-to-copy-content-from-one-file-to-another-file-in-java/)

在 Java 中，我们可以将一个文件的内容复制到另一个文件中。这可以通过[文件输入流](https://www.geeksforgeeks.org/java-io-fileinputstream-class-java/#:~:text=FileInputStream%20is%20useful%20to%20read,bytes%20such%20as%20image%20data.&text=FileInputStream(File%20file)%20%3ACreates,from%20the%20specified%20File%20object.)和`FileOutputStream`类来完成。
...
这将为给定的文件名创建一个文件输出流对象`fout`，并将读取的内容写入该文件名。
...
**注意：** 如果这里给定的文件名不存在，那么`File`类将创建一个同名的新文件。
