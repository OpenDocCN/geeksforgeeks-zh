...
> **提示:**极客们你们一定想知道有没有像`lower_bound()`或`upper_bound()`这样的函数，很可能在 C++ STL 中找到。所以直截了当的答案是，直到 Java 9 才出现了函数，后来它们才被添加进来。
...
它适用于对象集合，如`ArrayList`和`LinkedList`。
