# Java 中的 `ChronoLocalDateTime.range()` 方法示例
...
`ChronoLocalDateTime` 接口的 `range()` 方法用于获取作为参数传递的字段的有效值范围。此方法返回一个 `ValueRange` 对象，该对象包含字段的最小和最大有效值。这个 `ChronoLocalDateTime` 有助于提高返回范围的准确性。当字段不受支持并且方法无法返回范围值时，将引发异常。
