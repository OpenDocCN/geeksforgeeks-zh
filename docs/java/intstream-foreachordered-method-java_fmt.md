...
> 原文: [https://www.geeksforgeeks.org/intstream-foreachordered-method-java/](https://www.geeksforgeeks.org/intstream-foreachordered-method-java/)
...
## 参数:
`IntConsumer`表示接受单个`int`值参数且不返回结果的操作。这是消费者对`int`的原始类型专门化。

## 注意:
`forEachOrdered(IntConsumer action)`对该流的每个元素执行一个操作，如果该流具有定义的遭遇顺序，则在该流的`遭遇顺序`中执行。
