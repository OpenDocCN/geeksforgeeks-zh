...
Joda-Time 是由`joda.org`创建的一个 API，它提供了比 `java.util` 包中的类（如 `Calendar`、`GregorianCalendar`、`Date` 等）更好的类，并且有更有效的方法来处理日期和时间。这个应用编程接口包含在 Java 8.0 的 `java.time` 包中。
...
*   它支持 7 种日历系统，如佛教、科普特语、埃塞俄比亚语、公历、格里高利历、伊斯兰教、儒略历。
*   有一个条款来创建我们自己的日历系统。
*   它提供了一套丰富的日期和时间计算方法。
...
### 2. LocalDate
`LocalDate`：这个类以年-月-日的形式表示日期，用于表示没有时间和时区的日期。

```java
LocalDate today = LocalDate.now(); // 使用 now 方法将系统日期存入 LocalDate 对象。
System.out.println(today); // 2018-08-03
int d = today.getDayOfMonth(); // 03
```

### 3. LocalTime
`LocalTime`：这个类代表一天中没有时区的时间。

```java
LocalTime time = LocalTime.now(); // 将系统时间存入 LocalTime 对象
System.out.println(time); // 10:19:58
```

### 4. LocalDateTime
`LocalDateTime`：这个类处理日期和时间，不考虑时区。
...
3.  在 Eclipse 中，在包资源管理器中查找您的项目，右键单击它，然后选择 新建->文件夹-> `libs`。
4.  将 `joda-time-2.1.jar` 复制/拖动到新创建的 `libs` 文件夹中。
5.  再次右键单击您的项目（在包资源管理器中），然后选择 属性-> Java 构建路径->库->添加 JAR-> `joda-time-2.1.jar`。
6.  现在你可以用这个代码进行测试：
