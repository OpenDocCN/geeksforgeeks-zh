...
> 原文: [https://www.geeksforgeeks.org/java-signature-getinstance-method-with-examples/](https://www.geeksforgeeks.org/java-signature-getinstance-method-with-examples/)
...
如果没有提供程序支持指定算法的签名实现，此方法将抛出 `NoSuchAlgorithmException`。
...
## 签名实例(`String algorithm`, `Provider provider`)
...
*   `NoSuchAlgorithmException` – 如果指定提供程序对象中没有指定算法的 `SignatureSpi` 实现。
