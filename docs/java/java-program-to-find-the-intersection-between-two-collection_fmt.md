...
> 原文: [https://www.geeksforgeeks.org/java-program-to-find-the-intersection-between-two-collection/](https://www.geeksforgeeks.org/java-program-to-find-the-intersection-between-two-collection/)
...
1.  使用 [`ArrayList.contains()`](https://www.geeksforgeeks.org/arraylist-contains-java/#:~:text=ArrayList%20contains()%20method%20in,the%20given%20list%20or%20not.&text=Returns%3A,list%20else%20it%20returns%20false.) 方法
2.  使用 [`Vector.retainAll()`](https://www.geeksforgeeks.org/vector-retainall-method-in-java-with-examples/) 方法

### 方法 1

*   将元素存储在第一个集合和第二个集合（`ArrayList`）中。
...
public class GFG {
    ...
    static ArrayList<Integer> RemoveElements(ArrayList<Integer> A, ArrayList<Integer> B) {
...
public class GFG {
    // Method 1 of this class
    // To remove the elements from the collection
    static Vector<Integer> RemoveElements(Vector<Integer> A, Vector<Integer> B) {
        A.retainAll(B);
        // Returning the update ArrayList
        return A;
    }
    ...
    public static void main(String[] args) {
        ...
        Vector<Integer> UpdatedCollection = RemoveElements(A, B);
