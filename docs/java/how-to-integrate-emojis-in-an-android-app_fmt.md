...
> 原文：[https://www.geeksforgeeks.org/how-to-integrate-emojis-in-an-android-app/](https://www.geeksforgeeks.org/how-to-integrate-emojis-in-an-android-app/)
...
### 第二步：添加以下依赖项进行 `build.gradle(:app)`
...
创建一个布局来定义表情应该是什么样子。它的主要目的是定义表情符号的大小。它还会显示我们发送的信息。点击 **app** -> **res** -> **layout** (右键) -> **新建** -> **布局资源文件**，新建布局。

![newLayout](img/9669e610f6e442bf1f17ede4bf73ee14.png)

将其命名为 `text_view_emoji`。这就是 `text_view_emoji.xml` 的样子：
...
在这里，编写一个函数来膨胀 `EmojiTextView`。`LayoutInflater` 用于将用 XML 编写的 `View` 或 [`ViewGroup`](https://www.geeksforgeeks.org/android-ui-layouts/) 转换为可以在代码中使用的 Java 视图。另外，在这里设置 `onCreate()` 功能。经历了所有这些变化后，这就是 `MainActivity.java` 的样子：
