...
> 它是一个类型为 `javax.servlet.ServletConfig` 接口的对象。
...
| 方法 | 说明 |
| --- | --- |
| `getServletContext()` | 此方法从会话中提取一个名为的对象。 |
| `String getInitParameter(String name)` | 此方法接收保存在会话中的名为或 null 的对象。 |
| `Enumeration getInitParameterNames()` | 在会话中，此方法放置一个具有名称的对象。 |
| `String getServletName()` | 这个方法返回一个 `RequestDispatcher`，它在路径上充当资源的掩护。 |
