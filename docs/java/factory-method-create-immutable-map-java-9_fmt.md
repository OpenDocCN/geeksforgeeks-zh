...
> 原文：[https://www.geeksforgeeks.org/factory-method-create-immutable-map-java-9/](https://www.geeksforgeeks.org/factory-method-create-immutable-map-java-9/)
...
为了在 java 8 中创建不可变的映射，我们使用了 `java.util.Collections.unmodifiableMap(Map map)` 方法。

### 不可修改地图(Map map)

此方法返回指定地图的不可修改视图。该方法允许模块向用户提供对内部地图的“只读”访问。
...
为了在 java 9 中创建不可变的地图，我们使用了 `Map.of()` 和 `Map.ofEntries()` 方法。
...
class ImmutableListDemo
{
    public static void main(String args[])
    {
        ...
        Map<Integer, String> immubatleMap = Map.ofEntries(entry1, entry2);
    }
}
```
