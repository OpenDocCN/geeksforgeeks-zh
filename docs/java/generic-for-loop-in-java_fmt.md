...
当我们知道我们必须`迭代整个集合或列表`时，那么我们可以使用通用 For 循环。Java 的 Generic 有一个名为`for each`循环，也叫`增强for循环`。这个 for-each 循环使得`更容易迭代``数组``或``泛型集合类`。
...
但是如果我们查看每个循环的通用循环或 T2 循环，回路的通用由三个参数组成:
