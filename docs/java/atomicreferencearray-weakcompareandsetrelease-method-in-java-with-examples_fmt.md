# Java 中的原子引用数组 weakCompareAndSetRelease() 方法示例
...
如果当前值等于作为参数传递的预期值，则使用`AtomicReferenceArray`类的`weakCompareAndSetRelease()`方法自动将索引`i`处的元素值设置为原子引用数组的新值。此方法更新该值，并确保先前的加载和存储在此访问后不会重新排序。如果向`AtomicReference`设置新值成功，则此方法返回`true`。
...
**输出：**
![](img/3249d6cd983e1acafc1162631506b149.png)
...
**输出：**
![](img/f040e4188d6a1a4ef38dfac7b08ec3f5.png)

## 参考文献

[https://docs.oracle.com/javase/10/docs/api/java/util/concurrent/atomic/AtomicReferenceArray.html#weakCompareAndSetRelease(V, V)](https://docs.oracle.com/javase/10/docs/api/java/util/concurrent/atomic/AtomicReferenceArray.html#weakCompareAndSetRelease(V, V))