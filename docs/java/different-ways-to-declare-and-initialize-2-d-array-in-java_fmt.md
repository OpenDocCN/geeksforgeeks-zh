...
> 原文: [https://www.geeksforgeeks.org/different-ways-to-declare-and-initialize-2-d-array-in-java/](https://www.geeksforgeeks.org/different-ways-to-declare-and-initialize-2-d-array-in-java/)
...
*   `no_of_rows`：一个数组可以存储的行数。例如 `no_of_rows = 3`，那么该阵列将有三行。
*   `no_of_columns`：一个数组可以存储的行数。例如，`no_of_columns = 4`，那么该阵列将有四列。
...
我们可以用 `arr.length` 可用于查找行的大小（第一维）和 `arr[0].length` 可以用来计算列的大小（第二维）。
