...
> 原文: [https://www.geeksforgeeks.org/class-getprotectiondomain-method-in-java-with-examples/](https://www.geeksforgeeks.org/class-getprotectiondomain-method-in-java-with-examples/)
...
该方法以 `ProtectionDomain` 对象的形式返回该类的指定 `ProtectionDomain`。

## 异常

如果安全管理器存在并且其 `checkPermission` 方法不允许获取 `ProtectionDomain`，则此方法抛出:
