...
> 原文：[https://www.geeksforgeeks.org/java-program-to-convert-binary-to-hexadecimal/](https://www.geeksforgeeks.org/java-program-to-convert-binary-to-hexadecimal/)

十六进制数字系统顾名思义由 16 个实体组成。这 16 个实体由 10 位数字组成，0-9 也代表十六进制系统的前 10 位数字。对于剩下的 6 个数字，我们使用从 A 到 F 的英文字母来表示数字 10 到 15。应该注意的是，十六进制系统中最低的数字是 0，最高的是 15，用 f 表示。十六进制数可以通过将 4 个数字组合成十六进制数的单个字符而从二进制数中导出。
...
*   在上面的例子中，二进制数 10001101 可以分解成 4 位的块，例如 1000 和 1101，它们作为对应的十六进制数的 2 个字符。
*   所得的十六进制数将是 8D，其中每个字符都是通过计算其在十进制系统中的相应值来确定的，如果它是两位数，则用字母表来替换它，在这种情况下，D 代表 13。十六进制系统也称为基数-16。

对于二进制到十六进制的转换，我们将使用以下两种方法：

1.  使用内置 Java 方法 `toHexString()`
2.  重复获取余数，并将转换后的十进制数除以 16

## 方法 1

使用这种方法，我们首先将二进制数转换成存储为整数的十进制数。然后，我们简单地使用 Java 的 [`toHexString()`](https://www.geeksforgeeks.org/java-lang-integer-tohexstring-method-examples/) 方法来生成想要的输出字符串。
...
*   `num` – 该参数指定将转换为十六进制字符串的数字。数据类型是 `int`。
