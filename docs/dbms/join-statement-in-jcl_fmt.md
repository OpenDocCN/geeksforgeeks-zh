...
1.  `JOINKEYS FILE=F1,FIELDS=(1,10,A)` – 指定文件 1 的连接标准。
2.  `JOINKEYS FILE=F2,FIELDS=(1,10,A)` – 指定文件 2 的连接标准。
3.  `REFORMAT FIELDS=(F1:1,71,F2:1,9)` – `REFORMAT` 字段告诉排序实用程序，通过指定开始和结束位置，哪些字段将从输入文件写入输出文件。

## JCL SORT 连接两个文件，并从两个文件中写入记录

1.  **匹配记录(内部连接)–**
    如果排序卡中没有指定 `JOIN` 语句，`F1` 和 `F2` 的配对记录将被写入输出文件。
...
3.  **来自文件 1 的不匹配–**
    排序卡保留来自 `F1` 文件的未配对记录，不能在 `F2` 中使用 `REFORMAT`。
...
5.  **来自文件 2 的不匹配–**
    排序卡保留来自 `F2` 文件的未配对记录，不能在 `F1` 中使用 `REFORMAT`。
