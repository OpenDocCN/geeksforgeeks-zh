...
> 原文: [https://www.geeksforgeeks.org/database-management-systems-set-10/](https://www.geeksforgeeks.org/database-management-systems-set-10/)
...
## 2) 让 `E1` 和 `E2` 是一个简单单值属性的 E/R 图中的两个实体。`R1` 和 `R2` 是 `E1` 和 `E2` 之间的两种关系，其中 `R1` 是一对多，`R2` 是多对多。`R1` 和 `R2` 没有自己的特点。在关系模型中表示这种情况所需的最小表数是多少？
(a) 2
(b) 3
(c) 4
(d) 5
...
## 3) 考虑一个关系方案 `R = (A、B、C、D、E、H)`，以下函数依赖项在该关系方案上保持: `{ A–>B、BC–>D、E–>C、D–>A }`。`R` 的候选键有哪些？
(a) `AE`、`BE`
(b) `AE`、`BE`、`DE`
(c) `AEH`、`BEH`、`BCH`
(d) `AEH`、`BEH`、`DEH`
