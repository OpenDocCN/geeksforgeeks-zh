...
> 原文：[https://www.geeksforgeeks.org/creating-materialized-view-using-table-definition-in-cassandra/](https://www.geeksforgeeks.org/creating-materialized-view-using-table-definition-in-cassandra/)

在本文中，我们将讨论如何基于一个特定的问题创建一个物化视图，并介绍同样的例子。我们一个一个来讨论。

## 先决条件
– [物化视图](https://www.geeksforgeeks.org/materialized-views-in-cassandra/)

在本文中，您将看到如何基于表定义创建物化视图。让我们通过下面给出的步骤来理解这一点。

## 创建表格定义

现在，为了理解这些场景，考虑一个例子，其中 `cricket_team` 是一个表名，`team_match_wins`、`team_match_losses` 和 `team_match_ties` 是字段。
...
| 团队名称 | 团队_比赛_失败 | 团队_比赛_联系 | 团队_匹配_获胜 |
| --- | --- | --- | --- |
...
现在，如果您要执行这个查询，您将会得到一个错误，因为没有名为 `cricket_team_by_team_match_wins` 的表。因此，首先，您可以使用以下 CQL 查询创建一个物化视图。
...
| 团队_匹配_获胜 | 团队名称 | 团队_比赛_失败 | 团队_比赛_联系 |
| --- | --- | --- | --- |
