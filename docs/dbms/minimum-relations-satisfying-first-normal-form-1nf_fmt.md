...
> 原文: [https://www.geeksforgeeks.org/minimum-relations-satisfying-first-normal-form-1nf/](https://www.geeksforgeeks.org/minimum-relations-satisfying-first-normal-form-1nf/)
...
我们很清楚如何画一个 ER 图。但是，当我们问诸如满足第一范式(`1NF`)需要多少最小关系之类的问题时，有时会有点困惑。我们建立了一些简单的规则，这些规则是在深入分析每个案例后形成的，因此可以直接使用。

## 注意

不鼓励你去记忆规则，而是理解每个案例背后的逻辑会帮助你轻松地长时间记住它们。

## 注释

1.  如果双方都完全参与；将涉及的两个实体和关系合并到 `1` 个表中。
...
*   **Way-1:** 将两个实体和关系合并成一个表。这是不正确的，因为 `(AD)` 将成为该表的主键，但主键不能有空值。
    ...
    ```
*   **Way-2:** 合并 `1` 侧关系。这是不正确的，因为 `(AD)` 将成为该表的主键，但主键不能有空值。
    ```
    | A  | B  | C  | D  |
    |----|----|----|----|
    | a1 | b1 | c1 | d1 |
