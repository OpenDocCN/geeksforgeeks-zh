...
> 原文：[https://www.geeksforgeeks.org/difference-between-rdbms-and-hive/](https://www.geeksforgeeks.org/difference-between-rdbms-and-hive/)
...
| --- | --- |
| 它用于维护数据库。 | 用于维护数据仓库。 |
| 它使用结构化查询语言。 | 它使用 HQL（Hive 查询语言）。 |
| 模式在 RDBMS 中是固定的。 | 模式各不相同。 |
| 存储标准化数据。 | 存储标准化和非标准化两种类型的数据。 |
| RDBMS 中的表是稀疏的。 | Hive 里表很密集。 |
| 它不支持分区。 | 它支持自动化分区。 |
| 没有使用分区方法。 | 分割方法用于分区。 |