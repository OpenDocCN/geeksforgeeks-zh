...
> 原文：[https://www.geeksforgeeks.org/consistency-levels-in-cassandra/](https://www.geeksforgeeks.org/consistency-levels-in-cassandra/)
...
*   `LOCAL_SERIAL`：
    此一致性级别为 Like `LOCAL_QUORUM`。在这种一致性级别下，我们需要本地 DC 的 2 个节点才能成功。但是，In `LOCAL_SERIAL` 一致性级别有很多额外的流量(至少 4 倍往返)，以加强 Cassandra 中的顺序和一致性。
