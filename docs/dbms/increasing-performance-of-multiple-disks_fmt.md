...
> 原文：[https://www.geeksforgeeks.org/increasing-performance-of-multiple-disks/](https://www.geeksforgeeks.org/increasing-performance-of-multiple-disks/)
...
支持性能提升的主要思想是：`数据剥离`。这包括在不同磁盘之间分发数据，并以同步方式访问所有磁盘以检索数据。在多个磁盘的情况下，提高性能的主要概念包括使用并行数据访问的概念，将数据分发到几个磁盘中，而不是只保存在一个磁盘中。要做到这一点，有两个概念可以遵循。它们是`位级数据条带化`和`块级数据条带化`。
...
在`位级条带化`和`块级条带化`中，数据访问是并行的，速度也提高了。我们使用大量磁盘，这提高了速度和性能。但是，当任何磁盘出现故障时，整个操作都会受到影响。因此，该系统证明其可靠性非常低。