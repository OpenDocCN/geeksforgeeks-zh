...
> 原文: [https://www.geeksforgeeks.org/user-defined-function-in-cassandra/](https://www.geeksforgeeks.org/user-defined-function-in-cassandra/)
...
现在，为了传递该值作为输入，我们将创建一个简单的表，即 `UDF_Function_test`，其中 `function_id`、`num1` 和 `num2` 是字段值。我们来看看。
...
现在，我们将在这里插入一些数据，我们将在 UDF 函数中使用这些数据来计算最大值，其中 `function_id` 将在 101 到 104 的范围内。我们来看看。
...
现在，我们将尝试将在空输入时返回空的子句更改为在空输入时调用，并将看到结果。我们来看看。
