...
## 2. NewSQL
`NewSQL` 这个术语对数据库进行了分类，这些数据库是关系模型与数据类型的可扩展性、灵活性的进步的结合。这些数据库侧重于 `NoSQL` 没有的功能，这提供了强有力的一致性保证。这包括两层数据：一层是关系数据，一层是键值存储。
...
| 9. | 例如：`DynamoDB`、`MongoDB`、`RavenDB` 等。 | 例如：`VoltDB`、`CockroachDB`、`NuoDB` 等。 |