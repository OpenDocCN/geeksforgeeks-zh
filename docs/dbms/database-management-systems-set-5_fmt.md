...
> 原文:[https://www.geeksforgeeks.org/database-management-systems-set-5/](https://www.geeksforgeeks.org/database-management-systems-set-5/)

GATE CS 2010 考试中提出了以下问题。

## 1) 列车预订数据库的关系模式如下。
`Passenger(pid，pname，年龄)`
`Reservation(pid，等级，tid)`
...
`T3` 应该可以在 `T2` 之前完成，因为 `T3` 的 `Read(Y)` 和 `T2` 的 `Read(Y)` 不冲突。同样，`T3` 的 `Write(X)` 与 `T2` 的 `Read(Y)` 和 `Write(Y)` 操作并不冲突。
