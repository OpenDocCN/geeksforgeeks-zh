...
例如，`学生` `Geeks` `报名参加` `第 100 期` `课程`。
这里，`报名`是关系的名称。

这只是关系的一个实例。可以有几个例子和几个其他人加入几个课程。关系`注册`将`学生`和`课程`作为参与实体集。

从数学上讲，我们把上面的表示为，`注册`是`学生`和`课程`的交叉积的一个子集。它有关于哪个学生注册了哪个课程的信息。如果`(s，c)`属于`注册`，则意味着学生`s`已注册课程`c`。`注册`表中的元组称为关系实例，而`注册`称为关系类型或关系集。关系类型包含同类关联的集合。
