...
> 原文：`https://www.geeksforgeeks.org/storage-snapshot-technology/`

## 存储

存储定义为通过计算技术将数字数据保存在数据存储设备中的过程。存储是使计算机能够暂时或永久保留数据的技术。
...
*   分割镜像将正在为其创建快照的存储实体（如文件系统、卷或`LUN`）物理克隆到另一个同类且大小完全相同的实体上。
