...
Java 提供[原子类](https://www.geeksforgeeks.org/tag/java-util-concurrent-atomic-package/)，如 `AtomicInteger`、`AtomicLong`、`AtomicBoolean` 和 `AtomicReference`。这些类的对象分别表示 `int`、`long`、`boolean` 和对象引用的原子变量。这些类包含以下方法。
