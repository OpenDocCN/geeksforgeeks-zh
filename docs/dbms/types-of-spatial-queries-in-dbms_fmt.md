...
> 原文：[https://www.geeksforgeeks.org/types-of-spatial-queries-in-dbms/](https://www.geeksforgeeks.org/types-of-spatial-queries-in-dbms/)
...
1.  **近似查询：**
    它请求位于指定位置附近的对象。查询给定点一定距离内的所有酒店就是近似查询的一个例子。最近邻查询请求距离指定点最近的对象。

例如，我们可能想找到最近的火车站。请注意，该查询不必指定距离限制，因此即使我们不知道最近的火车站有多远，我们也可以询问它。

2.  **区域查询：**
    处理空间区域。例如，查询可以要求部分或全部出现在固定区域内的对象。查询一个特定城镇地理范围内的所有药店，或者我们可以找到特定城市的所有学校。
3.  **联合/交集：**
    在这种类型的查询中，我们也可以请求区域的交集和并集。

例如，给定区域信息，例如年降雨量和人口密度，查询可以请求年降雨量低和人口密度高的所有区域。

一般来说，在空间数据的查询中有`空间需求`和`非空间需求`的组合。例如，我们可能想找到最近的餐馆，那里有素食选择，每顿饭收费不到 10 美元。
