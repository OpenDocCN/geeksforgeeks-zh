...
在许多情况下，您可能需要创建一个已经定义(或创建)的表的精确副本。`MySQL`可以让你执行这个操作。因为我们可能需要这样的重复表来测试数据，而不会对原始表和存储在其中的数据产生任何影响。
...
要克隆表，请使用下面的查询。使用此查询，可以创建一个空的表模式(结构)，其属性与原始表相同:
...
如果你想创建一个表，这个表被你的原始表的所有内容装饰起来，使用这个 `MySQL` 查询:
