...
> 原文：[https://www.geeksforgeeks.org/attribute-subset-selection-in-data-mining/](https://www.geeksforgeeks.org/attribute-subset-selection-in-data-mining/)
...
### 逐步向前选择
此过程以一组空属性作为最小集开始。选择最相关的属性（具有最小 `p` 值）并将其添加到最小集合中。在每次迭代中，一个属性被添加到一个缩减的集合中。

### 逐步向后淘汰
这里所有的属性都考虑在初始属性集中。在每次迭代中，从 `p` 值高于显著性水平的属性集中删除一个属性。

### 向前选择和向后消除的组合
将逐步向前选择和向后消除相结合，以便最有效地选择相关属性。这是最常见的技术，通常用于属性选择。

### 决策树归纳
该方法使用决策树进行属性选择。它构建了一个类似流程图的结构，其中的节点表示对属性的测试。每个分支对应于测试的结果，叶节点是一个类预测。不是树的一部分的属性被认为是不相关的，因此被丢弃。