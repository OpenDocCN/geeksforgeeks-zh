...
*   SQL 是关系数据库的编程语言（解释如下），如 `MySQL`、`Oracle`、`Sybase`、`SQL Server`、`PostgreSQL` 等。其他非关系数据库（也称为 `NoSQL`）如 `MongoDB`、`CouchDB` 等不使用 SQL。
...
**度:** 关系中属性的个数称为关系的度。上面定义的 `STUDENT` 关系为 5 度。

**基数:** 关系中元组的数量称为基数。上面定义的 `STUDENT` 关系的基数为 4。

**列:** 列表示特定属性的一组值。`ROLL_NO` 一栏摘自关系 `STUDENT`。
...
**数据查询语言:** 用于从关系中提取数据。例如: `SELECT`

因此，首先我们将考虑数据查询语言。从关系数据库中检索的一般查询是:

1.  `SELECT` [ `DISTINCT` ] 属性列表 `FROM` R1, R2...
2.  [ `WHERE` 条件]
3.  [ `GROUP BY` (属性) [ `HAVING` 条件]]
4.  [ `ORDER BY` (属性) [ `DESC` ]];
...
如果要检索所有学生的属性 `ROLL_NO` 和 `NAME`，查询将为:
...
如果要检索 `ROLL_NO` 大于 2 的学生的 `ROLL_NO` 和 `NAME`，查询将为:
...
如果我们想要检索学生的所有属性，我们可以写 `*` 来代替将所有属性写成:
...
如果我们想用 `AGE` 来表示关系的升序，我们可以用 `ORDER BY` 子句表示为:
...
**注:** `ORDER BY AGE` 相当于 `ORDER BY AGE ASC`。如果我们想要按照 `AGE` 的降序检索结果，我们可以使用 `ORDER BY AGE DESC`。

### 案例 5

如果我们想要检索属性或属性组的不同值，`DISTINCT` 的用法如下:
...
聚合函数用于对关系的数据值进行数学运算。SQL 中使用的一些常见聚合函数有:
...
`GROUP BY` 用于根据属性或属性组对关系的元组进行分组。它总是与在组上计算的聚合函数相结合。例如:
...
在这个查询中，`SUM(AGE)` 将被计算，但不是针对整个表，而是针对每个地址。即: 地址 `DELHI` (18+18=36) 的年龄总和，其他地址也是如此。输出结果是:
...
**注意:** 不属于 `GROUP BY` 子句的属性不能用于 `SELECT`。作为 `GROUP BY` 子句一部分的任何属性都可以用于 `SELECT`，但不是强制性的。但是我们可以在聚合函数中使用不属于 `GROUP BY` 子句的属性。
