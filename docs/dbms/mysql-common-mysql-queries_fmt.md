...
**注:** 数据库 `information_schema`、`mysql`、`performance_schema` 为 `MySQL` 服务器内部使用的系统数据库。一个 `test` 数据库用于安装期间提供的测试目的。
...
### 9. CREATE DATABASE 数据库名称

**数据库名称:** 数据库名称
该语句创建一个新的数据库。

### 10. DROP DATABASE_name
...
### 16. ALTER TABLE TABLE_name DROP(column 1)

**TABLE_name:** 现有表的名称。
**第 1 列:** 第 1 列名称。
此语句从现有表中删除指定的列。

### 17. INSERT INTO 表名(第 1 列，第 2 列，第 3 列..) VALUES(value1，value2，value3..)
...
### 19. DELETE FROM 表中删除_名称 WHERE 条件

**表_名称:** 表的名称。
**条件:** 条件陈述。
此语句从表中删除记录。

### 20. SELECT 列 1、列 2、列 3..FROM 表_name WHERE 条件
...
**注意:** 聚合函数允许您使用 `SELECT` 查询对数据进行计算并提供信息。
