...
从表 `Student` 中删除 `Age` 为 20 的行。这将删除 2 行（第三行和第五行）。
...
**输出：**
上面的查询将删除两行（第三行和第五行）表 `Student` 现在看起来像，
...
**输出：**
表中的所有记录都将被删除，没有剩余的记录可以显示。表 `Student` 会变空！
