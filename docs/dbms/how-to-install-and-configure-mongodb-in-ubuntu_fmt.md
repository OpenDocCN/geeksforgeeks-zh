...
**第二步：** 现在，使用 [`apt`](https://www.geeksforgeeks.org/apt-command-in-linux/) 安装 `MongoDB` 包。键入以下命令，然后按回车键。
...
阅读更多关于 `Unix` 中 [`systemctl`](https://www.geeksforgeeks.org/systemctl-in-unix/) 命令的信息。