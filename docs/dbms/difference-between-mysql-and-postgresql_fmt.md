...
> 原文: [https://www.geeksforgeeks.org/difference-between-mysql-and-postgresql/](https://www.geeksforgeeks.org/difference-between-mysql-and-postgresql/)
...
| 3. | 仅当与 `InnoDB` 和 `NDB` 集群引擎一起使用时，才是 `ACID` 兼容的。 | 这是 `ACID` 兼容的。 |
| 4. | 实现语言为 **C/C++**。 | 实现语言为 **C**。 |
...
| 10. | `SQL` 提供**有限的 `MVCC` 支持**(在 `InnoDB` 中)。 | **完全的 `MVCC`** 支持。 |