...
> 原文：[https://www.geeksforgeeks.org/timestamp-based-concurrency-control/](https://www.geeksforgeeks.org/timestamp-based-concurrency-control/)
...
如前所述，`时间戳`是由数据库管理系统创建的唯一标识符，用于识别事务。它们通常按照提交给系统的顺序进行分配。交易的时间戳 `T` 为`TS(T)`。时间戳的基础知识，可以参考这里的。
...
根据事务进入系统的时间，为每个事务分配一个时间戳。假设，如果旧的事务 `T_i` 具有时间戳 `TS(T_i)`，则新的事务 `T_j` 被分配时间戳 `TS(T_j)`，使得`TS(T_i) < TS(T_j)`。该协议管理并发执行，使得时间戳确定可串行化顺序。时间戳排序协议确保任何冲突的读写操作都按照时间戳顺序执行。每当某些事务 `T` 试图发出 `R_item(X)` 或 `W_item(X)` 时，基本 TO 算法会将 `T` 的时间戳与 `R_TS(X)` 和 `W_TS(X)` 进行比较，以确保不违反时间戳顺序。这在以下两种情况下描述了基本 TO 协议。

1.  每当事务 `T` 发出 `W_item(X)` 操作时，检查以下条件：
    *   如果 `R_TS(X) > TS(T)` 或如果 `W_TS(X) > TS(T)`，则中止并回滚 `T` 并拒绝该操作。否则，
    *   执行 `T` 的 `W_item(X)` 操作，并将 `W_TS(X)` 设置为 `TS(T)`。
2.  每当事务 `T` 发出 `R_item(X)` 操作时，检查以下条件：
    *   如果 `W_TS(X) > TS(T)`，那么中止并拒绝 `T` 并拒绝操作，否则
    *   如果 `W_TS(X) <= TS(T)`，那么执行 `T` 的 `R_item(X)` 操作，并将 `R_TS(X)` 设置为 `TS(T)` 和当前 `R_TS(X)` 中较大的那个。

每当基本 TO 算法检测到两个以不正确顺序发生的冲突操作时，它都会通过中止发出操作的事务来拒绝后一个操作。基本 TO 生成的计划保证是*冲突可串行化的*。已经讨论过，使用时间戳可以确保我们的日程将是 [*无死锁*](https://www.geeksforgeeks.org/dbms-introduction-timestamp-deadlock-prevention-schemes/)。

基本 TO 协议的一个缺点是`级联回滚`仍然是可能的。假设我们有一个事务 `T_1` 和 `T_2` 使用了一个由 `T_1` 写的值。如果 `T_1` 中止并重新提交给系统，那么 `T_2` 也必须中止并回滚。因此级联中止的问题仍然普遍存在。
