...
> 原文：[https://www.geeksforgeeks.org/select-operation-in-relational-algebra/](https://www.geeksforgeeks.org/select-operation-in-relational-algebra/)
...
σ c(R)
```
其中 `c` 是布尔表达式（条件）的选择条件，我们可以有一个像 `Roll = 3` 这样的条件或者像 `X > 2` 和 `Y < 1` 这样的条件的组合。符号 `σ`（sigma）用于表示选择（choose）运算符。

`r` 是一个关系代数表达式，其结果是一个关系。条件 `c` 中指定的布尔表达式可以用以下形式编写：
```
<attribute name> <comparison operator> <constant value> or <attribute name>
```
其中，`<attribute name>` 显然是关系属性的名称，`<comparison operator>` 是任意一个运算符 `{=, <, <=, >, >=, !=}` 并且，`<constant value>` 是取自关系域的常量值。
...
## 示例-2
给定具有以下元组的关系学生（Roll，Name，Class，Fees，Team）：

| Roll | Name | Department | Fees | Group |
| --- | --- | --- | --- | --- |
...
```
σ Fees >= 10000 (σ Class != 'A' (Student))
```
