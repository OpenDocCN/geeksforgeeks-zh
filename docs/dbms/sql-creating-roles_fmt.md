...
首先创建一个`manager`角色，然后允许`manager`创建表和视图。然后，它授予`SAM`和`STARK` `manager`的角色。现在`SAM`和`STARK`可以创建表格和视图。如果用户被授予多个角色，他们将获得与所有角色相关联的所有权限。然后使用`REVOKE`从角色“管理器”中删除创建表权限。使用`DROP`从数据库中删除该角色。