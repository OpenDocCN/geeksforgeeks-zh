...
- 使用 `Stream` 作为别名获取 `Branch`，并使用 `CGPA` 作为别名从 `Student_Details` 表中获取 `Grade`。
...
本文由 `Pratik Agarwal` 供稿。如果你喜欢 `GeeksforGeeks` 并想投稿，你也可以使用 `contribute.geeksforgeeks.org` 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
