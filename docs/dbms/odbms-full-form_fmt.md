...
> 原文：`https://www.geeksforgeeks.org/odbms-full-form/`
...
*   **处理各种数据的能力：** 与其他数据库管理系统不同，`ODBMS` 还可以存储非文本数据，如：图像、视频和音频。

## 不足之处

*   **没有通用的标准：** 操作 `ODBMS` 没有普遍认可的标准。这是最显著的缺点，因为用户可以随心所欲地操作数据模型，这在处理海量数据时可能是个问题。
