...
> 原文: [https://www.geeksforgeeks.org/components-of-storage-area-network-san/](https://www.geeksforgeeks.org/components-of-storage-area-network-san/)
...
1.  **多模光纤:**
    也称为`MMF`，因为它同时将多束以不同角度投射的光线传输到电缆纤芯中。在`MMF`传输中，光束在电缆内部传播时会分散并发生碰撞。这种碰撞在信号传播一定距离后会削弱信号强度，这被称为`模态色散`。

`MMF`电缆用于 500 米的距离，因为模态色散会导致信号衰减。

2.  **单模光纤:**
    也叫`SMF`，因为它携带一束光通过光纤的纤芯。电缆中的小纤芯减少了模态色散。由于衰减较小，`SMF`电缆可用于长达 10 公里的距离。`SMF`比`MMF`贵。
