...
> 原文: [https://www.geeksforgeeks.org/difference-between-super-key-and-candidate-key/](https://www.geeksforgeeks.org/difference-between-super-key-and-candidate-key/)
...
通过使用键 `AB` 我们可以识别表格的其余属性 `(CDEF)`。同样，关键 `CD`。但是，使用 `CB` 键，我们只能识别 `D` 和 `F` 而不能识别 `A` 和 `E`。同理键 `D`。
...
这里我们可以看到两个候选键 `ID` 和 `{First_name, Last_name, DOB, Phone_no}`。所以在这里，存在不止一个候选关键字，它可以唯一地标识关系中的元组。
