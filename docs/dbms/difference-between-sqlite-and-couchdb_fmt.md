...
> 原文: [https://www.geeksforgeeks.org/difference-between-sqlite-and-couchdb/](https://www.geeksforgeeks.org/difference-between-sqlite-and-couchdb/)
...
**2. CouchDB:**
Apache CouchDB 是一个开源的面向文档的 [`NoSQL`](https://www.geeksforgeeks.org/introduction-to-nosql/) 数据库，使用多种格式和协议来存储、传输和处理它的数据，它使用 `JSON` 来存储数据， [`JavaScript`](https://www.geeksforgeeks.org/javascript-tutorial/) 作为它的查询语言使用 `MapReduce`，`HTTP` 作为一个 `API`。它由 Apache 软件基金会开发，最初于 2005 年发布。是用 `Erlang` 写的。
...
| 9. | 它不支持任何复制方法。 | 它支持两种复制方式–<br>• `Master slave copy`<br>• `Master slave copy` |
