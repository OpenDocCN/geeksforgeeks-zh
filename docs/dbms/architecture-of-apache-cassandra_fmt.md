...
> 原文：[https://www.geeksforgeeks.org/architecture-of-apache-cassandra/](https://www.geeksforgeeks.org/architecture-of-apache-cassandra/)
...
在该策略中，它允许定义单个整数 `rf`（复制因子）。它确定应该包含每行副本的节点数。例如，如果 `replication_factor` 为 `2`，则两个不同的节点应该存储每行的副本。它对所有节点一视同仁，忽略任何已配置的数据中心或机架。
