...
> 原文: [https://www.geeksforgeeks.org/types-of-normal-forms-in-dbms/](https://www.geeksforgeeks.org/types-of-normal-forms-in-dbms/)
...
使用这个更容易查询的功能 [`NoSQL` 数据库](https://www.geeksforgeeks.org/introduction-to-nosql/)像[`MongoDB`](https://www.geeksforgeeks.org/mongodb-an-introduction/)、[`Apache POI`](https://www.geeksforgeeks.org/apache-poi-introduction/)等。因此像谷歌、亚马逊和脸书这样的科技巨头用它来处理大量难以存储的日常数据。
...
*   `X -> Y` 是任何非平凡的函数依赖，`R` 是 `BCNF` 如果 `X` 是候选键或超键。

或者

*   `X -> Y` 是微不足道的函数依赖(即 `Y` 是 `X` 的子集)，
...
假设 `R` 是关系模式，`F` 是单值和多值依赖关系 `X ->-> Y` 在 `4NF`，如果:
