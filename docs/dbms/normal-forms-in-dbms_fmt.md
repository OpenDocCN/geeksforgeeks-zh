...
*   **示例 1 –** 表 1 中的关系 `STUDENT` 因多值属性 `STUD_PHONE` 而不符合 1NF。其分解为 1NF 的形式已在表 2 中展示。
...
在上表中，`Courses` 是一个多值属性，因此它不在 1NF 中。
...
因此，
`COURSE_FEE` 将是一个非质数属性，因为它不属于唯一的候选关键字 `{STUD_NO, COURSE_NO}`；
...
*   **示例 1 –** 在表 4 给出的关系 `STUDENT` 中，

FD 集: `{STUD_NO->STUD_NAME, STUD_NO->STUD_STATE, STUD_STATE->STUD_COUNTRY, STUD_NO->STUD_AGE}`
候选密钥: `{STUD_NO}`

对于表 4 中的这个关系，`STUD_NO->STUD_STATE` 和 `STUD_STATE->STUD_COUNTRY` 为真。所以 `STUD_COUNTRY` 是传递依赖于 `STUD_NO` 的，它违反了第三范式。为了将其转换为第三范式，我们将关系 `STUDENT (STUD_NO, STUD_NAME, STUD_PHONE, STUD_STATE, STUD_COUNTRY, STUD_AGE)` 分解为:
...
`B -> D`,
`E -> A`
所有可能的候选键在上述关系中是 `{A, E, CD, BC}`。所有属性都在所有函数依赖的右侧，是素属性。

## 4. Boyce-Codd 范式(BCNF)

一个关系 `R` 在 BCNF，如果 `R` 在第三范式，并且对于每个 FD，LHS 是超级关键。在每个非平凡的函数依赖 `X-->Y` 中，BCNF 要求 `X` 是一个超级键。

*   **示例 1–** 求一个关系 `R(A, B, C, D, E)` 的最高范式，FD 设置为 `{BC->D, AC->BE, B->E}`。
步骤 1。我们可以看到，`(AC)+ = {A, C, B, E, D}` 但是它的子集都不能决定关系的所有属性，所以 `AC` 将是候选键。`A` 或 `C` 不能从关系的任何其他属性派生，因此将只有 1 个候选键 `{AC}`。
步骤 2。在本例中，质数属性是候选关键字 `{A, C}` 的一部分，其他属性在本例中是非质数 `{B, D, E}`。
...
**解决上述类型问题的要点**。
**1)** 先从 BCNF 开始查，然后 3NF，以此类推，总是个好主意。
...
**2NF:** 在 2NF 中，我们需要检查部分依赖。`CD` 是候选关键字的适当子集，它确定 `E`，`E` 是非质数属性。所以，给定的关系也不在 2NF。所以，最高范式是 1NF。
