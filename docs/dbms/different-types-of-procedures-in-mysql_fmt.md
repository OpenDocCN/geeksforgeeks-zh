...
> 原文：[https://www.geeksforgeeks.org/different-types-of-procedures-in-mysql/](https://www.geeksforgeeks.org/different-types-of-procedures-in-mysql/)
...
采用性别类型输入（此处为`"Male"`/`"Female"`）的程序，输入/输出参数反映了属于该性别类别/类型的作者数量：
...
根据表格`author`的插入，我们有两个作者，一个是男性，一个是女性。因此，一名男性作者的产出为`1`，一名女性作者的产出为`1`。
