...
**隔离：** 事务提交之前，事务的结果不应该对其他人可见。例如，让我们假设 `A` 的余额是 Rs。5000 和 `T1` 借记 Rs。1000 到 `A`。`A` 的新余额将是 4000。如果 `T2` 信用卢比。500 到 `A` 的新余额，`A` 会变成 4500，之后这个 `T1` 就失效了。然后我们还必须回滚 `T2`，因为它正在使用 `T1` 产生的价值。因此，事务结果在提交之前不会对其他事务可见。
