...
> 原文：[https://www.geeksforgeeks.org/materialized-views-with-practical-approach-in-cassandra/](https://www.geeksforgeeks.org/materialized-views-with-practical-approach-in-cassandra/)
...
为了创建物化视图（物化视图表 – `everytime_high_marks`），我们提供了一个简单的 `select` 语句和用于该视图的主键。指定 `CLUSTERING ORDER BY` 允许我们对最高分进行反向排序，这样我们只需选择分区中的第一项就可以获得最高分。
