...
> 原文: [https://www.geeksforgeeks.org/er-diagram-of-bank-management-system/](https://www.geeksforgeeks.org/er-diagram-of-bank-management-system/)
...
*   `银行主体`: 银行主体的属性为`银行名称`、`代码`、`地址`。
    `代码`是银行实体的主键。
*   `客户实体`: 客户实体的属性为`Customer_id`、`姓名`、`电话号码`、`地址`。
    `客户标识`是客户实体的主键。
*   `分支实体`: 分支实体的属性为`Branch_id`、`名称`、`地址`。
    `分行标识`是分行实体的主键。
*   `账户主体`: 账户主体的属性有`账号`、`账户类型`、`余额`。
    `账号`是账户主体的主键。
*   `贷款主体`: 贷款主体的属性为`贷款_id`、`贷款_类型`、`金额`。
    `贷款标识`是贷款主体的主键。
