...
`ALTER SCHEMA` 语句用于将对象从一个模式转移到同一数据库中的另一个模式。
...
```sql
INSERT dbo.geektab (id, name, address)
...
```sql
ALTER SCHEMA geek TRANSFER OBJECT::dbo.geektab;
```
...
```text
Msg 208, Level 16, State 1, Procedure sp_get_id, Line 3
