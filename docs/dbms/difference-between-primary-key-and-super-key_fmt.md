...
> 原文: [https://www.geeksforgeeks.org/difference-between-primary-key-and-super-key/](https://www.geeksforgeeks.org/difference-between-primary-key-and-super-key/)
...
通过使用键 `AB` 我们可以识别表格的其余属性 `(CDEF)`。同样关键 `CD`。但是，使用 `CB` 键，我们只能识别 `D` 和 `F` 而不能识别 `A` 和 `E`。同理键 `D`。
