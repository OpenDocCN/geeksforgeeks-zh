...
> 原文：[`https://www.geeksforgeeks.org/difference-between-data-warehousing-and-data-mining/`](https://www.geeksforgeeks.org/difference-between-data-warehousing-and-data-mining/)

数据仓库的建立是为了支持管理功能，而数据挖掘是用来从数据中提取有用的信息和模式。数据仓库是将信息编译成数据仓库的过程。

## [数据仓库](https://www.geeksforgeeks.org/data-warehousing/)
它是一种聚合来自一个或多个来源的结构化数据的技术，以便进行比较和分析，而不是事务处理。数据仓库旨在通过提供一个数据清理、数据集成和数据整合的平台来支持管理决策过程。数据仓库包含面向主题的、集成的、时变的和非易失的数据。
