...
> 原文：[https://www.geeksforgeeks.org/data-distribution-in-cassandra/](https://www.geeksforgeeks.org/data-distribution-in-cassandra/)
...
`令牌`：令牌是哈希值，`murmur3` 哈希算法用于 `Cassandra` 中的哈希，分区器使用它来确定在环中的每个节点上将行存储在哪里。
...
我们取 `RF = 2`，简单的说就是每行有两个副本。`Cassandra` 没有主副本或主副本。
