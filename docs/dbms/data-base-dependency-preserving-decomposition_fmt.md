...
分解 `D = { R1, R2, R3, ..., Rn }` 是一组函数依赖 `F` 的依赖保持分解，如果：

```
(F1 ∪ F2 ∪ … ∪ Fm)+ = F+.
```

考虑一个关系 `R`，`R ---> F{...with some functional dependency(FD)....}`。

`R` 被分解或划分为带有 FD `{ f1 }` 的 `R1` 和带有 `{ f2 }` 的 `R2`，那么可能存在三种情况：
...
设一个关系 `R(A, B, C, D)` 和函数依赖 `{ AB-->C, C-->D, D-->A }`。关系 `R` 分解为 `R1(A, B, C)` 和 `R2(C, D)`。检查分解是否保持依赖关系。
...
[https://www.geeksforgeeks.org/dbms-gq/database-design-normal-forms-gq/](https://www.geeksforgeeks.org/dbms-gq/database-design-normal-forms-gq/)
