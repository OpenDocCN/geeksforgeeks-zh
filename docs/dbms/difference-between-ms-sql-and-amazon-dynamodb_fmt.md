...
> 原文: [https://www.geeksforgeeks.org/difference-between-ms-sql-and-amazon-dynamodb/](https://www.geeksforgeeks.org/difference-between-ms-sql-and-amazon-dynamodb/)
...
```
| 编号 | 微软 SQL | 亚马逊 DynamoDB |
| --- | --- | --- |
| 1. | 微软的 `SQL` 是一个关系数据库模型。 | 亚马逊 `DynamoDB` 是一个可扩展的数据库，数据存储在亚马逊云上。 |
| 2. | 主要的数据库模型是关系数据库管理系统。 | 主要的数据库模型是亚马逊 `DynamoDB` 中的文档存储和键值存储。 |
...
| 8. | 它不是基于云的。 | 它基于云。 |
| 9. | 就排名而言，它被认为比亚马逊 `DynamoDB` 更好。 | 就排名而言，被认为不如 `MS SQL`。 |
