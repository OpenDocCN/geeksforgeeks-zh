...
此模型不支持的一些操作有：`日志传送`、`AlwaysOn` 或 `镜像` 和 `时间点恢复`。

在这种情况下，数据库仅用于测试和开发。此操作中的数据是静态的。它没有提供 `时间点恢复`。

## 2. 完全恢复

与简单恢复不同，它支持事务日志备份。不会因为文件损坏或丢失而导致工作损失，因为这种模型可以跟踪在数据库上执行的每个操作。

它支持数据库恢复的 `时间点`，因此可以恢复到任意点。当这种模型被数据库使用时，事务日志将大量（无限）增长，这将导致系统崩溃等问题。因此为了防止这种情况，我们必须定期备份事务日志。

此设置提供了更多选项。

## 3. 大容量日志

该模型与完全恢复模型相似，因为两个事务日志都是备份的。它对批量操作有很高的性能。它有助于比其他模型更快地导入大容量数据，并保持事务文件大小较低。它不支持 `时间点恢复`。
