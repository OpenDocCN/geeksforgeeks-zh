...
*   `NOT NULL`: 这个约束告诉我们不能在列中存储空值。也就是说，如果一个列被指定为非空，那么我们将不能再在这个特定的列中存储空值。
*   `UNIQUE`: 当用一列指定时，这个约束告诉列中的所有值必须是唯一的。也就是说，列的任何一行中的值都不能重复。
*   `PRIMARY KEY`: 主键是一个可以唯一标识表中每一行的字段。此约束用于将表中的字段指定为主键。
*   `FOREIGN KEY`: 外键是一个字段，可以唯一地标识另一个表中的每一行。此约束用于将字段指定为外键。
...
如果我们在表中指定一个字段为 `NOT NULL`。那么该字段将永远不接受空值。也就是说，在没有为该字段指定任何值的情况下，不允许在表中插入新行。

例如，下面的查询创建了一个学生表，字段 `ID` 和 `NAME` 不为空。也就是说，每次我们希望插入新行时，我们都必须为这两个字段指定值。
...
主键是唯一标识表中每一行的字段。如果表中的某个字段作为主键，则该字段将不能包含空值，并且该字段的所有行都应该具有唯一值。换句话说，我们可以说这是 `NOT NULL` 约束和 `UNIQUE` 约束的组合。
...
我们可以清楚地看到，`Orders` 表中的字段 `C_ID` 是 `Customers` 表中的主键，即它唯一标识 `Customers` 表中的每一行。因此，它是 `Orders` 表中的外键。
...
例如，下面的查询创建了一个表 `Student`，并将字段 `AGE` 的条件指定为 `(AGE >= 18)`。也就是说，用户将不被允许在表中输入任何 `AGE` < 18 的记录。[详细检查约束](https://www.geeksforgeeks.org/sql-check-constraint/)
...
该约束用于为字段提供默认值。也就是说，如果在表中输入新记录时，用户没有为这些字段指定任何值，那么默认值将被分配给它们。
