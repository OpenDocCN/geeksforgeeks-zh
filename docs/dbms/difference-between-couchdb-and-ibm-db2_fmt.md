...
> 原文：[https://www.geeksforgeeks.org/difference-between-couchdb-and-ibm-db2/](https://www.geeksforgeeks.org/difference-between-couchdb-and-ibm-db2/)

**1. CouchDB：**
CouchDB 是一个开源的面向文档的 NoSQL 数据库，它使用多种格式和协议来存储、传输和处理其数据。它使用 `JSON` 来存储数据，使用 `MapReduce`，将 `JavaScript` 作为其查询语言，使用 `HTTP` 作为 API。它由 `Apache` 软件基金会开发，最初于 2005 年发布。是 Erlang 写的。
...
| four | 是用 Erlang 语言写的。 | 它是用 `C`、`C++`、汇编语言、`Java` 语言编写的。 |
