...
> 原文: [https://www.geeksforgeeks.org/ms-sql-server-type-conversion/](https://www.geeksforgeeks.org/ms-sql-server-type-conversion/)
...
从一种数据类型转换为另一种数据类型的过程称为类型转换或类型转换。有以下两种类型转换。

*   **隐式转换:** 一种数据类型可以自动转换为另一种数据类型的转换称为隐式转换。
*   **显式转换:** 必须手动转换的数据类型转换称为显式转换。

显式转换必须使用`CAST`或`CONVERT`函数在 SQL Server 中完成。语法如下。

```sql
cast(old_datatype as new_datatypename);
