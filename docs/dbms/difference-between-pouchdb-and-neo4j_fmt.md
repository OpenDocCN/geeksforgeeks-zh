...
> 原文: [https://www.geeksforgeeks.org/difference-between-pouchdb-and-neo4j/](https://www.geeksforgeeks.org/difference-between-pouchdb-and-neo4j/)

## 1. PouchDB
`PouchDB` 是一个开源的 `NoSQL` 在线数据库。它是根据为 `CouchDB` 提供动力的 `NoSQL` 数据库设计的。它是用 `JavaScript` 语言编写的。不需要通过网络执行查询，因为 `PouchDB` 位于浏览器内部，因此速度非常快。它使用浏览器中的 `IndexedDB` 和 `WebSQL` 在本地存储数据。
...
| 10. | 数据库的主要模型是文档存储。 | `Neo4j` 的主要数据库模型是 `Graph DBMS`。 |
| 11. | 它支持 `Javascript` 编程语言。 | `Neo4j` 支持 `.Net`、`Clojure`、`Elixir`、`Go`、`Groovy`、`Haskell`、`Java`、`JavaScript`、`Perl`、`PHP`、`Python`、`Ruby`、`Scala` 等编程语言。 |
