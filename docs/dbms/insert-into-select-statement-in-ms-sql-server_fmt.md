# INSERT INTO...SELECT 语句在 MS SQL Server 中的应用
...
**先决条件**：[MS SQL Server 中的 INSERT 语句](https://www.geeksforgeeks.org/insert-statement-in-ms-sql-server/)、[MS SQL Server 中的 SELECT 语句](https://www.geeksforgeeks.org/select-statement-in-ms-sql-server/)

考虑一个大学数据库。有两张表，即 `Student` 表和 `Marks` 表。
在这种情况下，需要将少数学生的分数从 `Marks` 表转移到 `Student` 表，有很多种方法。
...
如果用户想将 `Marks` 表的数据转移到 `Student` 表，查询如下：
...
INTO Student (Roll_No, Name, Course)
SELECT Roll_No, Name, GPA
FROM Marks
ORDER BY Roll_No;
```
