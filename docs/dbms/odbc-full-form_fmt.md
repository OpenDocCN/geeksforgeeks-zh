...
它是一个开放标准`应用程序编程接口`也称为 `API`，用于访问数据库。第一个 `ODBC` 驱动程序是在 1992 年建立的，当时微软与辛巴合作，命名为 `Simba`。借助程序中的 `ODBC` 语句。我们可以访问许多不同或公共数据库中的不同文件。
...
*   `Error Codes` –
    `ODBC` 基本上支持错误代码机制来显示在处理 `SQL` 语句时导致错误发生的问题。
*   `Attributes` –
    除了错误代码功能外，它还提供不同类型的函数来获取驱动程序中使用的属性和函数的详细信息。
*   `Rich metadata` –
    `ODBC` 也提供了对关于数据的数据（也称为元数据）的巨大支持。`ODBC` 还提供不同的函数来获取有关所用函数和数据类型的数据。
*   `Inter-operability` –
    `ODBC` 最重要的特性是其互操作性，这意味着使用 `ODBC` 驱动程序，我们可以开发不同的应用程序，这些应用程序可以与不同的 `DB`（也称为数据库管理系统）通信，并且将我们的应用程序从一个数据库切换到另一个不会产生任何问题。
*   `SQL` 语法–
