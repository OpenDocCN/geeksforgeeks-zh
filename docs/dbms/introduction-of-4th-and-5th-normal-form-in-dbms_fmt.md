...
> 原文: [https://www.geeksforgeeks.org/introduction-of-4th-and-5th-normal-form-in-dbms/](https://www.geeksforgeeks.org/introduction-of-4th-and-5th-normal-form-in-dbms/)
...
**示例 –** 考虑一个班级的数据库表，有两个关系 `R1` 包含学号(`SID`)和学号(`SNAME`)，`R2` 包含课程号(`CID`)和课程名(`CNAME`)。
...
**联合依赖 –** 连接分解是多值依赖的进一步推广。如果 `R1` 和 `R2` 在 `C` 上的连接等于关系 `R`，那么我们可以说存在一个连接依赖(JD)，其中 `R1` 和 `R2` 是给定关系 `R (A，B，C，D)` 的分解 `R1(A，B，C)` 和 `R2(C，D)`。或者，`R1` 和 `R2` 是 `R` 的无损分解，如果 `R1，R2，…，Rn` 是无损连接分解。如果 join 的属性 join 等于关系 `R`，则 `(A，B，C，D)`，`(C，D)` 将是 `R` 的 JD。这里，`(R1，R2，R3)` 用于表示关系 `R1`，`R2`，`R3` 等是 `R` 的 JD。

设 `R` 是 `R1、R2、R3……..Rn` 的关系模式，当且仅当 `R = R1 ⋈ R2 ⋈ ... ⋈ Rn` 时，称 `Rn` 为 `R` 的分解。
