...
> 原文: [https://www.geeksforgeeks.org/clausal-form-in-deductive-databases/](https://www.geeksforgeeks.org/clausal-form-in-deductive-databases/)
...
*   公式中的所有变量都是全称量化的。因此，没有必要为所有变量显式地包含全称量词。量词被移除，公式中的所有变量都隐式地由全称量词量化。
*   因为公式是由许多子句组成的，并且每个子句都是由许多仅通过或逻辑连接词连接的文字组成的。因此，每个子句都是文字的`析取`。
*   为了形成一个公式，子句本身仅由“与”逻辑连接词连接。因此，公式的从句形式是子句的`连词`。
...
上面的子句有 **n** 个负文字量和 **m** 个正文字量。该子句可转换为以下等效逻辑公式:
