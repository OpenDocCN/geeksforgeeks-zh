...
1NF 到 2NF 关系的[规范化](https://www.geeksforgeeks.org/database-normalization-normal-forms/)包括**去除部分依赖**。如果存在部分依赖，我们通过将部分依赖属性与它们的行列式副本放在一个新的关系中，从关系中移除部分依赖属性。
...
因此，
`COURSE_FEE` 将是一个非素数属性，因为它不属于唯一的候选关键字 `{STUD_NO, COURSE_NO}`；
但是，`COURSE_NO -> COURSE_FEE`，即 `COURSE_FEE` 依赖于 `COURSE_NO`，它是候选关键字的适当子集。非素数属性 `COURSE_FEE` 依赖于候选关键字的一个适当子集，这是一个部分依赖，因此这种关系不在 2NF 中。
