...
> 原文:[https://www.geeksforgeeks.org/database-management-systems-set-6/](https://www.geeksforgeeks.org/database-management-systems-set-6/)
...
## 2) 让 `R` 和 `S` 为关系模式，使得 `R={a，b，c}` 和 `S={c}`。现在考虑数据库上的以下查询:
...
既然 `(sname，city)` 形成了一个候选键，就没有非平凡的依赖 `X ? Y` 其中 `X` 不是超级键
