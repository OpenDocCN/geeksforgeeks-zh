...
> 原文：[https://www.geeksforgeeks.org/difference-between-indexing-and-hashing-in-dbms/](https://www.geeksforgeeks.org/difference-between-indexing-and-hashing-in-dbms/)

## 索引
索引顾名思义，是一种普遍用于加快数据访问速度的技术或机制。索引基本上是一种数据结构，用于快速定位和访问数据库表中的数据。使用数据库表的一列或多列可以很容易地开发或创建索引。

## 散列
散列顾名思义，是一种使用以搜索关键字为参数的散列函数来生成数据记录地址的技术或机制。它计算数据记录在磁盘上的直接位置，而不使用索引结构。一个好的散列函数只使用单向散列算法，散列不能转换回原始密钥。简单来说，就是把给定的密钥转换成另一个值的过程，这个值被称为哈希值或者简称为散列。
...
| 索引的类型包括有序索引、主索引、辅助索引和聚集索引。 | 散列的类型包括静态和动态散列。 |
