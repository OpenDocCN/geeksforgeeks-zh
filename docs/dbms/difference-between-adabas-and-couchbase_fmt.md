...
> 原文：[https://www.geeksforgeeks.org/difference-between-adabas-and-couchbase/](https://www.geeksforgeeks.org/difference-between-adabas-and-couchbase/)

## 1. Adabas
`Adabas` 代表适应性数据库系统。它是由软件公司开发的，运行在 `IBM` 大型机上。它于 1971 年推出。这是一个面向大型机和 `Linux/Unix/Windows` 环境的 `OLTP-DBMS`。`ADABAS` 是一个企业数据库管理系统（`DBMS`），旨在实现可靠性、高性能、可扩展性和低总拥有成本。
...
| 3. | `Adabas` 是用自然语言写的。 | `Couchbase` 是用 `C++`、`Erlang`、`C` 和 `Go` 语言编写的。 |
...
| 11. | `Adabas` 的许可证是商业的。 | `Couchbase` 的许可证是开源的。 |
