...
> 原文: [https://www.geeksforgeeks.org/replication-strategy-in-cassandra/](https://www.geeksforgeeks.org/replication-strategy-in-cassandra/)
...
这是一个简单的策略，建议在单个数据中心的多个机架上的多个节点使用。
...
让我们考虑一个例子，`cluster1` 是一个关键空间名称，其中 `NetworkTopologyStrategy` 是一种复制策略，有两个数据中心：一个是 `east` 数据中心，`replication_factor` = 2，第二个是 `west` 数据中心，`replication_factor` = 3。
...
`system` 键空间包含有关可用的柱族、柱和簇的信息。`system_auth` 密钥空间主要包含身份验证信息、用户凭据和权限。