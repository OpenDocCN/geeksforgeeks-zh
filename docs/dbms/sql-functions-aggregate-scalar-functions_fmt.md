...
### `COUNT()`
它用于计算 `SELECT` 语句中返回的行数。它不能在 MS ACCESS 中使用。
...
### `LAST()`
`LAST()` 函数返回所选列的最后一个值。它只能在 MS ACCESS 中使用。
...
```
（指定 `length` 是可选的，`start` 表示起始位置，从 1 开始）
...
注意：许多数据库系统采用 IEEE 754 算术运算标准，该标准规定当任何数值以 .5 结尾时，四舍五入到最接近的偶数整数，即 5.5 和 6.5 都会四舍五入为 6。
