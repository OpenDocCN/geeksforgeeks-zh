...
术语频率(`tf_{ij}`)是针对第 I 个术语和第 j 个文档计算的:

`$ tf_{i, j} = \frac{n_{i, j}}{\sum_k n_{k, j}} $`

其中`$ n_{i, j} $`是第 j 个文档中第 I 个术语的出现。
...
第 j 个文档中第 I 个术语的最终得分`w_{i, j}`由一个简单的乘法组成:`tf_{ij}*idf_{i}`。由于文档/查询只包含集合中所有不同术语的子集，因此对于大量术语，术语频率可以为零:这意味着需要稀疏向量表示来优化空间需求。
...
旋转规范化的思想是使比经验值(旋转长度:`l_{p}`)短的文档不太相关，而使长的文档更相关，如下图所示:旋转规范化
