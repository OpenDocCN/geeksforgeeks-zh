...
删除模式前，删除模式中的所有对象。如果模式中有任何对象，输出将是一个错误。

## 示例

让我们创建一个名为 `geeksh` ([https://www.geeksforgeeks.org/create-schema-in-sql-server/](https://www.geeksforgeeks.org/create-schema-in-sql-server/))的新模式；
...
现在在 `geeksh` 模式中创建一个名为 `geektab` 的表:
