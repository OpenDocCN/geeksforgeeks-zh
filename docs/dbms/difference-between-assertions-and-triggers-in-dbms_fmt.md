...
> 原文：[https://www.geeksforgeeks.org/difference-between-assertions-and-triggers-in-dbms/](https://www.geeksforgeeks.org/difference-between-assertions-and-triggers-in-dbms/)
...
```
在上面的例子中，我们强制执行 `CHECK` 约束，即船只和水手的数量应该少于 100。因此，在这里，我们能够同时检查两个表的约束。
...
$
```
在上面的例子中，我们在更新之前创建了触发器。所以，如果 `new.age` 大于 60 岁我们就不应该更新，否则我们就应该更新。我们可以使用 `NEW` 和 `OLD` 来调用这个触发器符号。
