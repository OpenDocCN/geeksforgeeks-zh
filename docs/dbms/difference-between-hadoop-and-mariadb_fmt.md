...
> 原文: [https://www.geeksforgeeks.org/difference-between-hadoop-and-mariadb/](https://www.geeksforgeeks.org/difference-between-hadoop-and-mariadb/)
...
## 2. MariaDB
`MariaDB` 是 `MySQL` 的增强版，由 `MySQL` 的原开发者制作，增加了功能。`MariaDB` 是开源软件。它在世界范围内被广泛使用，因为它速度快，性能强。2009 年由 `MariaDB` 企业开发。它是安全的，并且有一个内置的数据库防火墙。它是一个关系数据库，有一个我们可以从中访问数据的 `SQL` 接口。

## Hadoop 和 MariaDB 的区别

| 序号 | Hadoop | MariaDB |
| :--- | :--- | :--- |
| 1. | `Hadoop` 是一种开源软件，用于存储数据并同时运行应用程序或进程。 | `MariaDB` 是 [`MySQL`](https://www.geeksforgeeks.org/sql-tutorial/#mysql) 的增强版。 |
