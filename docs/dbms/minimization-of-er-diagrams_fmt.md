...
但是我们可以将“学生”和“注册”表合并起来，重命名为“学生_注册”。
...
因此，`Student_Enroll`和`Course`至少需要两个数据库管理系统表。

**注意：**在一对多关系中，我们至少可以有两个表。

## 2) 当在`ER图`中有多对多`基数`时
我们来考虑一下上面的例子，变化是现在一个学生也可以报1门以上的课程。
...
现在，同样的问题注册关系的主键是什么，如果我们仔细分析`Enroll`表，主键是（`SID`，`CID`）。

但是在这种情况下，我们不能将`Enroll`表与`Student`和`Course`中的任何一个合并。如果我们试图将`Enroll`与`Student`和`Course`中的任何一个合并，将会创建冗余数据。
...
由于`E1`是全部参与，`E1`的每个条目只与`E2`中的一个条目相关，但并非`E2`中的所有条目都与`E1`的一个条目相关。
...
但是我们可以把`R`合并成`E1`或者`E2`。所以至少需要2张表。

下面是大门往年问题。
[https://www.geeksforgeeks.org/gate-gate-cs-2008-question-82/](https://www.geeksforgeeks.org/gate-gate-cs-2008-question-82/)
[https://www.geeksforgeeks.org/gate-gate-cs-2008-question-83/](https://www.geeksforgeeks.org/gate-gate-cs-2008-question-83/)
