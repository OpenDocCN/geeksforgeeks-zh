...
`5NF`，又称`PJNF`。这是一种数据库规范化级别，旨在减少关系数据库中的冗余。当且仅当一个关系满足 4NF 且不存在连接依赖时，该关系才被称为 `5NF`。如果一个关系可以通过连接多个子关系来重新创建，并且这些子关系中的每一个都具有原始关系的属性子集，则称该关系具有连接依赖性。
...
考虑下面的关系 `R` 具有模式 `R(供应商、产品、消费者)`。主键是关系的所有三个属性的组合。
