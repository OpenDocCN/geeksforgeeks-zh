...
> 原文: [https://www.geeksforgeeks.org/conditionally-updating-columns/](https://www.geeksforgeeks.org/conditionally-updating-columns/)
...
在上面的 CQL 查询中，如果该行存在，它将返回 `true`。让我们看看结果。

### 输出:

![](img/13a6f0513e5ec1c0bdb872febeb1d80f.png)

在上面的 CQL 查询中，如果不存在行，它将返回 `true`。让我们看看结果。
...
在 CQL 查询中，条件更新是轻量级事务的例子，它们会带来不可忽略的性能，这是有代价的，应该谨慎使用。