...
> 原文：[https://www.geeksforgeeks.org/difference-between-entity-constraints-referential-constraints-and-semantic-constraints/](https://www.geeksforgeeks.org/difference-between-entity-constraints-referential-constraints-and-semantic-constraints/)
...
它给出了一个错误，因为`rollnumber`是一个防止重复的主键约束。这些约束确保保持表中的唯一性，以避免重复。
...
创建表格时有一个限制，即分数应奖励给那些只学习`student`表格中所列课程的学生。如果用户试图输入一个不存在的值，它会返回一个错误。
...
| **语法** | 主键：<br>`create table (col1 datatype1 primary key…)` | 外键：<br>`create table (col1 datatype1 references tablename…)` | `col1 varchar2(30)` |
| **示例** | 没有两个学生可以被指定同一个学号。 | `Rollnumber`指的是标记表。 | 名称被分配给精度为50的`varchar2`。 |