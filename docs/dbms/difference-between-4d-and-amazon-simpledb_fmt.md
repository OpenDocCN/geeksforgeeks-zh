...
> 原文：[https://www.geeksforgeeks.org/difference-between-4d-and-amazon-simpledb/](https://www.geeksforgeeks.org/difference-between-4d-and-amazon-simpledb/)

## 1. 第四维（4D）
第四维是一个关系数据库管理系统，由 Laurent Ribardière 开发的 `IDE`。`4D` 可以作为服务器运行，这样多个客户端可以管理单个[数据库](https://www.geeksforgeeks.org/what-is-database/)。`4D` 创建于 1984 年，1987 年略微推迟了麦金塔的公开发行。是一个集成了[数据库管理系统](https://www.geeksforgeeks.org/dbms/)的应用开发环境。

## 2. 亚马逊 SimpleDB
它是由亚马逊托管的简单数据库服务，数据存储在亚马逊云中。亚马逊是一个实时运行结构化数据查询的网络服务。这个数据库是 `Amazon.com` 用 `Erlang` 写的。它具有以下特点——高可用性和灵活性，几乎没有或根本没有管理负担。
