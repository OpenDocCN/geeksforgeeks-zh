...
> 原文: [https://www.geeksforgeeks.org/number-of-possible-superkeys-in-dbms/](https://www.geeksforgeeks.org/number-of-possible-superkeys-in-dbms/)
...
## Example-1
Let a Relation `R` have attributes `{a1,a2,a3}` and `a1` is the candidate key. Then how many super keys are possible?
...
## Example-3
Let a Relation `R` have attributes `{a1, a2, a3,…, an}` and the candidate key is “`a1 a2 a3`” then the possible number of super keys?

按照前面的公式，我们有 3 个属性，而不是一个。所以，这里可能的超级按键数量是 `2^(N-3)`。

## Example-4
Let a Relation `R` have attributes `{a1, a2, a3,…, an}` and the candidate keys are “`a1`”, “`a2`” then the possible number of super keys?
...
## Example-5
Let a Relation `R` have attributes `{a1, a2, a3,…, an}` and the candidate keys are “`a1`”, “`a2 a3`” then the possible number of super keys?
...
## Example-7
Let a Relation `R` have attributes `{a1, a2, a3,…, an}` and the candidate keys are “`a1 a2`”, “`a1 a3`” then the possible number of super keys?
...
**第二步:-** 找到超级键
由于只有一个候选键属性，有两种可能性：要么我们选择要么不选择，因此会有两次机会。
所以，`A _ _ D _ _ _ _ = _ B _ D _ _ _ _ = _ _ _ DE _ _ = _ _ _ D _ F _ _ = 2^6`

**步骤 3:-** 由于两个候选键的组合，找到超键。
所以，`n(AD∪BD) = n(AD∪ED) = n(AD∪FD) = n(BD∪ED) = n(BD∪FD) = n(ED∪FD) = 2^5`

**步骤 4:-** 由于 3 个候选键的组合，找到超级键，
所以，`n(AD∪BD∪ED) = n(AD∪ED∪FD) = n(ED∪BD∪FD) = n(BD∪FD) = n(BD∪FD∪AD) = 2^4`

**第五步:-** 因全求超键，
所以，`n(AD∪BD∪ED∪FD) = AB _ DEF _ _ = 2^3`

所以，根据包含-排除原理:-
```
| W∪X∪Y∪Z | = | W |+| X |+| Y |+| Z |–| W∪X |–| W∪Y |–| W∪Z |–| X∪Y |–| X∪Z |–| Y∪Z |+| W∪X∪Y |+| W∪X∪Z |+| W∪Y∪Z |+| X∪Y∪Z |–| W∪X∪Y∪Z |
```
超级按键 = `4 * 2^6 – 6 * 2^5 + 4 * 2^4 – 2^3 = 120`

所以超级键的数量是 120。这个解释是由 YaduvanshiRishi 贡献的。

## Example-10
让一个关系 `R` 有属性 `{a1，a2，a3…… an}` 和 `{ a1a2a3…… ak }` 作为候选键，其中 `k <= n`，那么可以有多少个超级键？
超级钥匙的可能数量是 `2^(n-k)`。