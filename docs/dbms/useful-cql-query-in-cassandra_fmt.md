...
> 原文: [https://www.geeksforgeeks.org/useful-cql-query-in-cassandra/](https://www.geeksforgeeks.org/useful-cql-query-in-cassandra/)
...
现在，我们要创建一个表 `web_info`，其中 `url`、`user_id`、`net_location`、`visitor_info` 是表的字段。我们来看看。
...
此示例显示了 `ALLOW FILTERING` 在非分区列的 CQL 查询中的工作方式。我们来看看。
