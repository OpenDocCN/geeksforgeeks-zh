...
> 原文：[https://www.geeksforgeeks.org/equivalence-of-functional-dependencies-sets/](https://www.geeksforgeeks.org/equivalence-of-functional-dependencies-sets/)

为了理解函数依赖集（`FD`集）的等价性，在[这篇文章](https://www.geeksforgeeks.org/finding-attribute-closure-and-candidate-keys-using-functional-dependencies/)中给出了关于属性闭包的基本思想。
...
*   `A->C`存在于`FD2`中，但不直接存在于`FD1`中，但我们会检查是否可以导出它。对于集`FD1`，`(A)+ = {A，B，C，D}`。这意味着`A`可以在功能上确定`A`、`B`、`C`和`D`。因此`A->C`也将保留在集`FD1`中。
*   `A->D`存在于`FD2`中，但不直接存在于`FD1`中，但我们会检查是否可以导出它。对于集`FD1`，`(A)+ = {A，B，C，D}`。这意味着`A`可以在功能上确定`A`、`B`、`C`和`D`。因此`A->D`也将保留在集`FD1`中。
...
*   集`FD1`中的`A->B`存在于集`FD2`中。
*   集`FD1`中的`B->C`也存在于集`FD2`中。
*   `A->C`存在于`FD1`中，但不直接存在于`FD2`中，但我们会检查是否可以导出它。对于集合`FD2`，`(A)+ = {A，B，C，D}`。这意味着`A`可以在功能上确定`A`、`B`、`C`和`D`。所以`A->C`也将在集合`FD2`中保持。
...
*   集`FD2`中的`A->B`存在于集`FD1`中。
*   集`FD2`中的`B->C`也存在于集`FD1`中。
*   `A->D`存在于`FD2`中，但不直接存在于`FD1`中，但我们会检查是否可以导出它。对于集`FD1`，`(A)+ = {A，B，C}`。这意味着`A`不能在功能上决定`D`，所以`A->D`在`FD1`中不成立。

由于集`FD2`中的所有`FDs`在集`FD1`中不成立，因此`FD2 ⊄ FD1`。
