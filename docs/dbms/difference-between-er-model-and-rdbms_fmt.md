...
## 1. ER(实体-关系)模型
[ER(实体-关系)模型](https://www.geeksforgeeks.org/introduction-of-er-model/)代表实体关系模型。它通过给出系统实体之间的关系来指定数据库的结构。实体集是一组彼此相似的实体。它被认为是一种概念设计，可以进一步用于实现数据库。它有助于我们在绘画作品的帮助下以一种简单的方式理解不同实体之间的联系。ER 模型一般是在制作数据库之前构建的。ER 模型用于描述真实世界的实体以及它们之间的关联。
