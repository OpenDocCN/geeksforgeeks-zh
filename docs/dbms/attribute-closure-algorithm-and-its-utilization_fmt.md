...
> 原文: [https://www.geeksforgeeks.org/attribute-closure-algorithm-and-its-utilization/](https://www.geeksforgeeks.org/attribute-closure-algorithm-and-its-utilization/)

一组功能描述符的闭包是从 `F` 可以推断出的所有功能描述符的集合 `F+` ，也称为完整的[功能依赖](https://www.geeksforgeeks.org/functional-dependency-and-attribute-closure/)集。用 `F+` 表示。
...
步骤 4 之前的结果是 `ABCEF`，步骤 4 之后的结果是 `ABCEF`，二者相同，所以停止。

**所以 `{A, B}+` 的闭包为 `{A, B, C, E, F}`。**

**结论:** 对于每个属性/属性集，我们都能找到闭包。这导致 `F+` 。