...
## 触发器
触发器是数据库中的存储过程，每当数据库中发生特殊事件时，它都会自动调用。例如，当一行被插入到指定的表中或者当某些表列被更新时，可以调用触发器。
...
2.  `[before | after]`: 指定触发器将在何时执行。
3.  `{insert | update | delete}`: 这指定了 DML 操作。
4.  `on [table_name]`: 指定与触发器关联的表的名称。
5.  `[for each row]`: 这指定了行级触发器，即触发器将针对受影响的每一行执行。
