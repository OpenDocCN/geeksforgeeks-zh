...
> 原文: [https://www.geeksforgeeks.org/database-management-system-set-1/](https://www.geeksforgeeks.org/database-management-system-set-1/)

GATE CS 考试中提出了以下问题。

## 1. 给定关系

`员工(姓名、工资、部门)`和`部门(部门、部门、地址)`

以下哪个查询不能用基本关系代数运算(`U`，`-`，`x`，`π`，`σ`，`p`)来表示？(GATE CS 2000)
(a) 每位员工的部门地址
(b) 姓名与其部门名称相同的员工
(c) 所有员工的工资总和
(d) 给定部门的所有员工
...
[http://faculty.ksu.edu.sa/zitouni/203%20Haseb%20%20Lecture%20Notes/Relional%20Algebra.pdf](http://faculty.ksu.edu.sa/zitouni/203%20Haseb%20%20Lecture%20Notes/Relional%20Algebra.pdf)
...
## 3. 给定关系 `r(w，x)` 和 `s(y，z)`，从 `r，s` 中选择不同的 `w，x` 的结果保证与 `r` 相同，前提是(GATE CS 2000)
(a) `r` 无重复且 `s` 非空
(b) `r` 和 `s` 无重复
(c) `s` 无重复且 `r` 非空
(d) `r` 和 `s` 具有相同数量的元组
...
## 4. 在 SQL 中，关系可以包含空值，与空值的比较被视为未知。假设所有带有空值的比较都被视为假。下列哪对不等价？(GATE CS 2000)
(a) `x = 5`，`not (not (x = 5))`
...
**参考文献:**
[http://www.cs.sfu.ca/CC/354/han/material/notes/354notes-chapter6/node1.html](http://www.cs.sfu.ca/CC/354/han/material/notes/354notes-chapter6/node1.html)