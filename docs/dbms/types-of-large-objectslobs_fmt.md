...
*   **NCLOBs (National Character LOBs)** 用于存储来自国家字符集的大块字符数据。

## 持久和临时`lob`

内部`lob`可以是持久的，也可以是临时的。持久性`lob`是存在于数据库表行中的`lob`实例。当您只在本地应用程序范围内实例化一个`lob`时，会创建一个临时`lob`实例。

当您将临时实例插入到表行中时，该实例将成为持久实例。

持久`LOBs`使用复制语义方法，也参与数据库事务。您还可以在任何事务或系统故障事件中恢复持久性`lob`，可以轻松提交或回滚。换句话说，根据与使用数据库对象相关的`ACID`属性，使用持久的`lob`。

## 外部`lob`和`BFILE`数据类型

外部`lob`是存储在数据库表空间之外的操作系统文件中的数据对象，没有数据库服务器的事务支持。

`BFILE`具有只读数据类型。数据库允许只读字节流访问存储在`BFILEs`中的数据。您不能从应用程序内部写入`BFILE`。
...
*   `DML`（数据操作语言）语句用于授予和撤销目录对象的对象权限。

一个会话中最多可以同时打开`10`个`BFILES`。

| **大对象数据类型** | **描述** |
| --- | --- |
| `BLOB` (二进制大对象) | 以二进制格式存储任何类型的数据，如图像、音频和视频。 |
| `CLOB` (字符大对象) | 将字符串数据存储在字符集格式的数据库中。用于使用数据库字符的大字符集/字符串或文档。 |
| `NCLOB` (国家字符大对象) | 以国家字符集格式存储字符串数据。用于国家字符集的大字符集/字符串或文档。支持不同宽度格式的字符。 |
| `BFILE` (外部二进制文件) | `BFILEs`可以在只读的基础上从您的应用程序中访问。使用`BFILEs`存储静态数据，如图像数据，这些数据不需要在应用程序中进行操作。 |