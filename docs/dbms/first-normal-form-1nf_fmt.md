...
如果一个表有数据冗余，并且没有适当地规范化，那么在不面临数据丢失的情况下，将很难处理和更新数据库。它还会消耗额外的内存空间，如果数据库没有正常化，插入、更新和删除异常会非常频繁。
...
如果关系包含复合或多值属性，则违反第一范式，或者如果不包含任何复合或多值属性，则关系处于第一范式。如果一个关系中的每个属性都是单值属性，那么这个关系就是第一范式。
