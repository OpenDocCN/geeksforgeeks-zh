...
在[卡珊德拉](https://www.geeksforgeeks.org/introduction-to-apache-cassandra/)列表中是[采集数据类型](https://www.geeksforgeeks.org/collection-data-type-in-apache-cassandra/)。在本文中，我们将讨论如何以不同的方式更新列表，以便我们可以在列表中的任何位置添加元素。我们还可以附加一个列表。我们一个一个来讨论。

## 创建键空间

考虑集群 `cluster1` 是一个键空间。
...
现在，我们将向 `Food_App` 表中插入一些数据。让我们看看。
