...
> 原文: [https://www.geeksforgeeks.org/monitoring-cluster-in-cassandra/](https://www.geeksforgeeks.org/monitoring-cluster-in-cassandra/)
...
有关所用 `nodetool` 选项的完整列表，请使用以下 `CQL` 查询。
...
要检查 `nodetool status`，请使用以下 `CQL` 查询。
...
要检查 `nodetool info`，请使用以下 `CQL` 查询。
...
该工具已被重命名为 `nodetool tablestats`。`nodetool tablestats` 提供一个或多个表的统计信息。
...
## 5. nodetool tablehistograms

该工具已更名为 `nodetool tablehistograms`。它提供了初始故障排除性能指标和过去十五分钟内表的读写延迟的当前性能统计数据，以监控 `Cassandra` 中的集群。
要检查 `nodetool tablehistograms`，请使用以下 `CQL` 查询。
