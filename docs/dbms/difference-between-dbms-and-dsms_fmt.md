...
> 原文: [https://www.geeksforgeeks.org/difference-between-dbms-and-dsms/](https://www.geeksforgeeks.org/difference-between-dbms-and-dsms/)
...
下面的查询是 `DBMS` 查询的一个例子，这是一个一次性的查询，给出了确切的答案。
...
下面的查询是 `DSMS` 查询的一个例子，它是一个连续的查询，给出了精确/近似的答案。
...
| 编号 | DBMS | DSMS |
| :--- | :--- | :--- |
| 01. | `DBMS` 是指数据库管理系统。 | `DSMS` 提到数据流管理系统。 |
| 02. | `DBMS` 处理持久数据。 | 数据流管理系统处理流数据。 |
| 03. | 在 `DBMS` 中，会发生随机数据访问。 | 在 `DSMS`，发生顺序数据访问。 |
| 04. | 它基于查询驱动的处理模型，即所谓的基于拉的模型。 | 它基于数据驱动处理模型，即所谓的基于推送的模型。 |
| 05. | 在 `DBMS` 中，查询计划在开始/固定时进行优化。 | `DSMS` 基于自适应查询计划。 |
| 06. | `DBMS` 中的数据更新率相对较低。 | `DSMS` 的数据更新率相对较高。 |
| 07. | 在 `DBMS` 中，查询是一次性查询。 | 但在 `DSMS`，这种质疑是持续不断的。 |
| 08. | 在 `DBMS` 中，查询给出了确切的答案。 | 在 `DSMS`，这个问题给出了确切/近似的答案。 |
| 09. | `DBMS` 不提供实时服务。 | `DSMS` 提供实时服务。 |
| 10. | `DBMS` 使用无界磁盘存储意味着无限的二级存储。 | `DSMS` 使用有限主存意味着有限主存。 |