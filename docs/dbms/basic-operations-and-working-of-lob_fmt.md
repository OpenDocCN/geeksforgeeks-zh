...
> 原文：[https://www.geeksforgeeks.org/basic-operations-and-working-of-lob/](https://www.geeksforgeeks.org/basic-operations-and-working-of-lob/)
...
*   您可以通过调用 `isTemporary()` 方法来测试业务线是否是临时的。如果 LOB 是通过调用 `createTemporary` 方法创建的，那么 `isTemporary()` 方法返回 `true`，否则返回 `false`。
...
*   您可以通过调用 `isOpen` 方法来检查 LOB 是打开还是关闭。每当通过调用 `open(int)` 方法打开 LOB 时，参数的值必须是 `MODE_READONLY` 或 `MODE_READWRITE`，如 `oracle.sql.BLOB` 和 `oracle.sql.CLOB` 类中所定义的。如果您使用 `MODE_READONLY` 打开业务线，那么任何写入业务线的尝试都会导致一个 SQL 异常。