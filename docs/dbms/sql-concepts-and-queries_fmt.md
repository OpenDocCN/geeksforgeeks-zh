...
让我们考虑现有的数据库，如 `information_schema`、`mysql`、`performance_schema`、`sakila`、`student`、`sys` 和 `world`。如果您想显示现有的数据库，那么我们将使用如下所示的 `SHOW DATABASES` 查询。
...
这里，`Account` 表中的 `AccNo` 列由 `Customer` 表中的 `CustID` 列引用。这里，`Account` 表是子表，`Customer` 表是父表。
...
| --- | --- | --- | --- | --- |
| 1111 | 储蓄 | 1000 | 1990-11-09 | 6000 |
| 1114 | 目前的 | 2000 | 1992-10-07 | 1000 |
| 1113 | 目前的 | 7000 | 1992-11-03 | 4000 |
| 1112 | 储蓄 | 1000 | 2003-12-12 | 3000 |
...
![左连接](img/4c4c19ba7c5abaedcd0f62eceeb32fa9.png)

### RIGHT JOIN

`RIGHT JOIN` 关键字返回右表(`table2`)中的所有记录以及左表(`table1`)中的匹配记录。
...
![右连接](img/90e414248df6e020fc2e467f79bca53f.png)
...
![内部连接](img/8283354f47f9ef4539dc0ddfdcb17f93.png)
...
![完全连接](img/1eeffd88ac4ba0308beb69f5c1662396.png)
...
这是必需的，因为 `WHERE` 子句不支持诸如 `COUNT`、`MIN`、`MAX`、`AVG`、`SUM` 等聚合函数。
