...
> 原文：[https://www.geeksforgeeks.org/high-availability-mechanism-in-cassandra/](https://www.geeksforgeeks.org/high-availability-mechanism-in-cassandra/)
...
![img](img/947721cf46da35ebe0207103c714022f.png)
...
在节点间通信中，我们使用 `listener_address` 和 `storage_port` 在 Cassandra 中进行通信。Cassandra 的默认端口 id 是 `7000`。在 Cassandra 中，集群中每个节点的端口 id 必须相同。
...
**关键：** 任何数据中心的种子节点列表在集群中的每个节点上都必须相同，这是一项重要的任务。
...
**注意：** 在卡珊德拉中，当负责该令牌范围的节点再次启动时，提示将被传递。
...
![img](img/a8ad82eb6df3aa9c455280f406549ba9.png)
