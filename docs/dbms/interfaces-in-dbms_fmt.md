...
图形用户界面通常以图表形式向用户显示模式。然后，用户可以通过操作图表来指定查询。在许多情况下，图形用户界面利用菜单和表单。大多数图形用户界面使用鼠标等定点设备来选择显示的模式图的特定部分。

## 4. 自然语言接口

这些接口接受请求，这些请求以英语或其他语言书写，并尝试理解它们。自然语言接口有自己的模式，该模式类似于数据库概念模式，以及一个包含重要单词的字典。
...
语音的使用是有限的，无论是作为查询、对问题的回答还是作为请求的结果，它正变得普遍。词汇量有限的应用程序，例如电话簿查询、航班到达/Departure和银行账户信息查询，允许使用语音进行输入和输出，以使普通人能够访问此信息。
