...
最极端的情况是在分布式系统的每个站点复制整个数据库。这将提高系统的可用性，因为只要至少有一个站点启动，系统就可以继续运行。
...
在这种类型的复制中，数据库的某些片段可能会被复制，而其他片段可能不会。片段的拷贝数量可以从一个到分布式系统中站点的总数。片段复制的描述有时被称为复制模式。
