...
数据库由表组成，在`关系数据库管理系统`的情况下，每个表称为一个关系。

让我们考虑一个名为“大学”的示例数据库，它有两个名为“学生”和“分数”的表。如果用户想要传输某一组行，则查询时将使用`insert in select`语句。但是，如果用户想要从这两个表中提取一整组行，或者如果用户只想从这两个表中提取一组选定的行，那么在查询中使用联接是完美的匹配。
