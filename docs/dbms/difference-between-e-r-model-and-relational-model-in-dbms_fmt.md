...
> 原文：`https://www.geeksforgeeks.org/difference-between-e-r-model-and-relational-model-in-dbms/`
...
| 3. | 它表示实体的集合并描述它们之间的关系。 | 它以表格的形式表示数据，并描述它们之间的关系。 |
| 4. | 它由实体、实体类型、实体集等组件组成。 | 它由域、属性、元组等组件组成。 |
| 5. | 理解实体之间的关系很容易。 | 导出不同表之间的关系不太容易。 |
| 6. | 它描述基数。 | 它没有描述基数。 |