...
我们可以观察上面的表，看到 `registry_no` 不能是表的主键，因为它与表的所有记录都不匹配，尽管它保存了表的所有唯一值。现在，在这种情况下，我们必须人为地为这个表创建一个主键。我们可以通过在表中添加一个包含匿名整数且与其他列没有直接关系的列 `surr_no` 来实现这一点。`surr_no` 的这个附加列是表的代理键。
