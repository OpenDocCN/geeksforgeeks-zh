...
我们用一个例子来理解死锁的概念:
假设事务 `T1` 持有学生表中某些行的锁，需要更新成绩表中的某些行。同时，事务 `T2` 在成绩表中持有锁，但需要更新事务 `T1` 持有的学生表中的行。
...
- [等待-死亡和创伤-等待的区别](https://stackoverflow.com/questions/32794142/what-is-the-difference-between-wait-die-and-wound-wait)
- 本书 – [纳瓦斯数据库系统基础](https://amzn.to/2TjTV7l)