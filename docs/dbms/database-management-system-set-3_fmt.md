...
> 原文: [https://www.geeksforgeeks.org/database-management-system-set-3/](https://www.geeksforgeeks.org/database-management-system-set-3/)
...
`ALL` 的意思是 `A.age` 应该大于子查询返回的所有值。表 `B` 中没有名为 `"arun"` 的条目。因此子查询将返回空值。如果子查询返回空值，则 `A` 的所有行的条件都为真(详情参见[本](http://publib.boulder.ibm.com/infocenter/iseries/v5r3/index.jsp?topic=%2Fsqlp%2Frbafyhowsub.htm))。所以表 `A` 的所有行都被选中。
