...
> 原文：[https://www.geeksforgeeks.org/difference-between-primary-key-and-unique-key/](https://www.geeksforgeeks.org/difference-between-primary-key-and-unique-key/)
...
号码属性已经分配了主键，并且`公民标识`可以有唯一的约束，其中`公民标识`列中的每个条目都应该是唯一的，因为一个国家的每个公民都必须有他或她的唯一标识号，如阿拉伯数字。但是，如果学生在这种情况下迁移到另一个国家，他或她将没有任何`公民标识`，并且条目可能具有空值，因为在唯一约束中只允许一个空值。
...
2.  定义主键时自动创建的`聚集索引`，而唯一键生成`非聚集索引`。
