...
#### 1. 向表中的用户授予 Select 权限

要向用户名为 `Amit` 的名为 `Users` 的表授予 `SELECT` 权限，应执行以下 `GRANT` 语句。
...
要向表 `Users` 中名为 `Amit` 的用户授予多个权限，应执行以下 `GRANT` 语句。
...
要向表 `Users` 中名为 `Amit` 的用户授予所有权限，应执行以下 `GRANT` 语句。
...
要向表 `Users` 中的所有用户授予特定权限，应执行以下 `GRANT` 语句。
...
在使用函数和过程时，`GRANT` 语句可用于授予用户执行 MySQL 中函数和过程的能力。
...
要查看表中授予用户的权限，使用 `SHOW GRANTS` 语句。要检查授予名为 `Amit` 的用户和作为 `localhost` 的主机的权限，将执行以下 `SHOW GRANTS` 语句：
...
要撤销用户名为 `Amit` 的 `Users` 表的 `SELECT` 权限，应执行以下 `REVOKE` 语句。
...
要撤销表 `Users` 中名为 `Amit` 的用户的多个权限，应执行以下 `REVOKE` 语句。
...
要撤销表 `Users` 中名为 `Amit` 的用户的所有权限，应执行以下 `REVOKE` 语句。
...
要撤销表 `Users` 中所有用户的特定权限，应执行以下 `REVOKE` 语句。
...
在使用函数和过程时，`REVOKE` 语句可用于撤销过去授予用户的 `EXECUTE` 权限。
...
*   **撤销用户在 MySQL 过程中的执行权限**：如果有一个名为 `DBMSProcedure` 的过程，并且您想要撤销对名为 `Amit` 的用户的 `EXECUTE` 访问，那么应该执行下面的 `REVOKE` 语句。

    ```
    REVOKE EXECUTE ON PROCEDURE DBMSProcedure FROM 'Amit'@'localhost';
    ```

*   **撤销所有用户对 MySQL 中某个过程的执行权限**：如果有一个名为 `DBMSProcedure` 的过程，并且您想要撤销对所有用户的 `EXECUTE` 访问权限，那么应该执行下面的 `REVOKE` 语句。
