...
> 原文: [https://www.geeksforgeeks.org/functional-dependency-and-attribute-closure/](https://www.geeksforgeeks.org/functional-dependency-and-attribute-closure/)
...
*   我们知道 `STUD_NO` 对每个学生来说都是独一无二的。所以 `STUD_NO->STUD_NAME`，`STUD_NO->STUD_PHONE`，`STUD_NO->STUD_STATE`，`STUD_NO->STUD_COUNTRY` 和 `STUD_NO ->STUD_AGE` 都将为真。
*   同样，如果两个记录具有相同的 `STUD_STATE`，那么 `STUD_STATE->STUD_COUNTRY` 将为真，它们也将具有相同的 `STUD_COUNTRY`。
*   对于关系 `学生_课程`，`课程_编号->课程_名称` 将为真，因为具有相同 `课程_编号` 的两个记录将具有相同的 `课程_名称`。

## 功能依赖集

关系的功能依赖集或 FD 集是关系中存在的所有 FD 的集合。例如，表 1 中所示的关系学生的功能描述集为:
...
属性集的属性闭包可以定义为一组属性，这些属性可以由属性集在功能上确定。
...
考虑关系方案 `R = {E，F，G，H，I，J，K，L，M，N}` 以及 R 上的函数依赖集 `{{E，F} -> {G}、{F} -> {I，J}、{E，H} -> {K，L}、K -> {M}、L -> {N}}`，R 的关键是什么？(GATE-CS-2014)
...
在具有属性 `A、B、C、D` 和 `E` 的模式中，给出了以下一组功能依赖关系
`{A -> B、A -> C、CD -> E、B -> D、E -> A}`
以上一组不暗示以下哪一个功能依赖关系？(GATE IT 2005)
...
`(BD)+ = {BD}` 意思是 `BD -> CD` 不成立。所以这个 FD 在 FD 集合中没有隐含。所以 (B) 是必选项。
其他可以用同样的方法检查。
...
考虑一个关系方案 `R = (A，B，C，D，E，H)`，以下函数依赖项在该关系方案上保持:`{ A–>B，BC–>D，E–>C，D–>A }`。R 的候选键有哪些？【GATE 2005】
