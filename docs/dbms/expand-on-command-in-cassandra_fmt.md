# 在Cassandra中使用Expand命令

> 原文：[https://www.geeksforgeeks.org/expand-on-command-in-cassandra/](https://www.geeksforgeeks.org/expand-on-command-in-cassandra/)

## 先决条件
先决条件是了解[Cassandra](https://www.geeksforgeeks.org/introduction-to-apache-cassandra/)。在本文中，我们将讨论`cqlsh` shell命令，如`expand on`、`expand off`、`describe keyspace`、`describe table`，还将讨论已实现的`cqlsh`查询。
...
## 创建表
现在，在成功创建`cluster1`键空间之后，我们将使用它来创建一个表。
...
现在，我们还可以使用`expand off`命令不展开结果。通常情况下，`expand off`是默认设置。下面给出了一个实现的`cqlsh`查询来验证它的工作结果。
