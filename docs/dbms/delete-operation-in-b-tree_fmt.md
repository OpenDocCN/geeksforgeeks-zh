...
**a)** 如果节点 `x` 中 `k` 前面的子节点 `y` 至少有 `t` 个键，那么在以 `y` 为根的子树中找到 `k` 的前身 `k'`，递归删除 `k'`，用 `x` 中的 `k'` 替换 `k`(我们可以找到 `k'`，一次向下删除。)

**b)** 如果 `y` 的键少于 `t`，那么，对称地，检查节点 `x` 中跟在 `k` 后面的子 `z`，如果 `z` 至少有 `t` 个键，那么在以 `z` 为根的子树中找到 `k` 的后继 `k'`，递归删除 `k'`，用 `x` 中的 `k'` 替换 `k`(我们可以找到 `k'`，在一次向下传递中删除它。)
...
本文由 [**Balasubramanian 供稿。**](http://in.linkedin.com/pub/balasubramanian-nagasundaram/3a/361/97b) 如果你发现任何不正确的地方，或者你想分享更多关于上面讨论的话题的信息，请写评论。