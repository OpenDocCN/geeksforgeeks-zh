...
MySQL 还支持用户定义变量的概念，允许将一个值从一条语句传递到另一条语句。MySQL 中的用户定义变量被写成 `@var_name`，其中，`var_name` 是变量的名称，可以由字母数字字符、下划线（_）和美元符号（$）组成。
...
*   未声明的变量也可以在 SQL 语句中访问，但是它们的值被设置为 `NULL`。
...
**注意:** 在上面的查询中，注意 `select` 语句中列的顺序。如果“标记”列写在“排名”列之前，那么我们得不到想要的输出。因为每次 `@prev_mark` 被分配当前学生的分数，结果评价 `@prev_mark != mark` 为 `假`。因此，每个学生的排名都显示为不递增，即它将保持为 0。