...
> 原文：[https://www.geeksforgeeks.org/introduction-of-shared-memory-segment/](https://www.geeksforgeeks.org/introduction-of-shared-memory-segment/)
...
这些类型的 IPC 通常需要从进程到内核的数据传输。在大多数情况下，需要四份数据。此外，这四个副本是在内核和进程之间制作的，这通常是一个昂贵的副本（比在内核或单个进程内复制数据更昂贵）。客户机和服务器之间通过内核的数据传输如图 1 所示。
