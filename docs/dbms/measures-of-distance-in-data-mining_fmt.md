...
> 原文: [https://www.geeksforgeeks.org/measures-of-distance-in-data-mining/](https://www.geeksforgeeks.org/measures-of-distance-in-data-mining/)
...
```
d(p, q) = d(q, p) = sqrt((q1 - p1)^2 + (q2 - p2)^2 + ... + (qn - pn)^2) = sqrt(Σ_{i=1}^{n} (qi - pi)^2)
```
