...
> 原文：[https://www.geeksforgeeks.org/graph-based-concurrency-control-protocol-in-dbms/](https://www.geeksforgeeks.org/graph-based-concurrency-control-protocol-in-dbms/)
...
该协议的一个先决条件是我们知道访问数据库项目的顺序。为此，我们对一组**数据库项目(`D`) *{`d<sub>1</sub>` 、`d<sub>2</sub>` 、`d<sub>3</sub>` 、…..，`d<sub>n</sub>` }*** 进行部分排序。实现后的协议表述为：
...
*   `Cascading Rollbacks` 仍然是一个问题。我们不遵循解锁操作何时可以发生的规则，所以这个问题在这个协议中持续存在。
