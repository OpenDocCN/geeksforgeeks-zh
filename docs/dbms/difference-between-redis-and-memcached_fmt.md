...
> 原文：`https://www.geeksforgeeks.org/difference-between-redis-and-memcached/`

**1. Redis：**
Redis 是一个开源、键值、`NoSQL` 数据库。它是一种内存中的数据结构，存储从内存提供的所有数据，并使用磁盘进行存储。它提供了独特的数据模型和高性能，支持各种数据结构，如字符串、列表、集合、哈希，它将其用作数据库缓存或消息代理。它也被称为数据结构服务器。它不支持模式关系数据库管理系统、`SQL` 或 `ACID` 事务。
