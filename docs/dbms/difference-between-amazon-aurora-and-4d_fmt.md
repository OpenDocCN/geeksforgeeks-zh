...
> 原文: [https://www.geeksforgeeks.org/difference-between-amazon-aurora-and-4d/](https://www.geeksforgeeks.org/difference-between-amazon-aurora-and-4d/)

## 1. 亚马逊极光
这是一个来自 `Amazon Web Services` 的完全托管关系数据库引擎，兼容 `PostgreSQL` 和 `MySQL`。它比标准的 `MySQL` 数据库快 5 倍，比标准的 `PostgreSQL` 数据库快 3 倍。极光的特点是分布式、容错和自愈存储系统。
