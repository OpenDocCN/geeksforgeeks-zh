...
> 原文：[https://www.geeksforgeeks.org/lossless-join-and-dependency-preserving-decomposition/](https://www.geeksforgeeks.org/lossless-join-and-dependency-preserving-decomposition/)
...
1.  `Att(R1) U Att(R2) = ABCD = Att(R)` 成立。
2.  `Att(R1) ∩ Att(R2) = φ`，违反了无损连接分解的条件。因此，分解不是无损的。

对于依赖保持分解，在 `R1(AB)` 可以保证 `A->B`，在 `R2(CD)` 可以保证 `C->D`。因此它是依赖保持分解。
