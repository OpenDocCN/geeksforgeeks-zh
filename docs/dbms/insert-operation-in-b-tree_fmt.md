...
1.  初始化 `x` 为根。
2.  当 `x` 不是叶子时，按照以下步骤操作：
    *   找到下一个要穿越的 `x` 的孩子。让孩子成为 `y`。
    *   如果 `y` 未满，将 `x` 更改为指向 `y`。
    *   如果 `y` 满了，拆分它，把 `x` 改为指向 `y` 的两个部分之一。如果 `k` 小于 `y` 中的 `mid key`，那么把 `x` 设为 `y` 的第一部分，否则设为第二部分。当我们拆分 `y` 时，我们把一个 `key` 从 `y` 移到它的父 `x`。
