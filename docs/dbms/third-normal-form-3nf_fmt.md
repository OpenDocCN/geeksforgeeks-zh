...
对于表 4 中的这个关系，`STUD_NO` -> `STUD_STATE` 和 `STUD_STATE` -> `STUD_COUNTRY` 为真。所以 `STUD_COUNTRY` 是过渡依赖于 `STUD_NO` 的，它违反了第三范式。为了将其转换为第三范式，我们将关系 `STUDENT` (`STUD_NO`，`STUD_NAME`，`STUD_PHONE`，`STUD_STATE`，`STUD_COUNTRY_STUD_AGE`)分解为:
