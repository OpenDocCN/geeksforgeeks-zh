...
> 原文: [https://www.geeksforgeeks.org/configuring-clusters-in-cassandra/](https://www.geeksforgeeks.org/configuring-clusters-in-cassandra/)
...
*   `cassandra.yaml` 文件的安装位置是 `<install_location>/resources/cassandra/conf`。
...
*   使用命令 `ctrl+f` 在文件中搜索。
*   搜索 `cluster_name`。
*   您将在文件中看到以下属性: `cluster_name: 'Test Cluster'`。这是集群节点的默认设置。
...
*   使用命令 `ctrl+f` 在文件中搜索。
*   搜索 `listen_address`。
*   您将在文件中看到以下属性: `listen_address: localhost`。这是群集节点的默认设置。
...
*   打开 `cassandra.yaml` 文件。
*   使用命令 `ctrl+f` 在文件中搜索。
*   搜索 `native_transport_address`。
*   您将在文件中看到以下属性: `native_transport_address: localhost`。这是群集节点的默认设置。
*   您可以根据需要更改 IP `native_transport_address`，然后按 `ctrl+s` 保存文件。
...
*   使用命令 `ctrl+f` 在文件中搜索。
*   搜索 `seeds`。
*   您将在文件中看到以下属性: `seeds: "127.0.0.1"`。这是群集节点的默认设置。
...
| 本地_传输_地址 | 192.168.10.71 | 192.168.10.72 | 192.168.10.73 |
