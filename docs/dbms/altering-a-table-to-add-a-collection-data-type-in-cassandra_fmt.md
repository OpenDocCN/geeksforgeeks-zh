...
> 原文：[https://www.geeksforgeeks.org/altering-a-table-to-add-a-collection-data-type-in-cassandra/](https://www.geeksforgeeks.org/altering-a-table-to-add-a-collection-data-type-in-cassandra/)

在本文中，我们将讨论如何更改表以添加 `MAP` 集合数据类型，以及如何在 `UPDATE` 命令的帮助下更改表后插入数据。我们一个一个来讨论。
首先我们要创建一个表，让我们考虑 `E_book` 是一个表名，`Book_name`、`Author_name`、`Selling_price` 是 `E_book` 表中的字段。让我们看看。
