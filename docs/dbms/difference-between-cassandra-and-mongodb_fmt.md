...
> 原文：[https://www.geeksforgeeks.org/difference-between-cassandra-and-mongodb/](https://www.geeksforgeeks.org/difference-between-cassandra-and-mongodb/)
...
| 4. | 卡珊德拉的读取性能非常高效，因为它需要 `O(1)` 时间。 | 与 `Cassandra` 相比，`MongoDB` 的读取性能没有那么快。 |
...
| 10. | 像 `Hulu`、`Instagram`、`Intuit`、`Netflix`、`Reddit` 等知名公司都使用 `Cassandra`。 | `Adobe`、`Amadeus`、`Lyft`、`ViaVarejo`、`Craftbase` 等知名公司都使用 `MongoDB`。 |