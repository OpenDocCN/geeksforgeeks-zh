...
> 原文:[https://www.geeksforgeeks.org/generalization-specialization-and-aggregation-in-er-model/](https://www.geeksforgeeks.org/generalization-specialization-and-aggregation-in-er-model/)
...
例如，为项目工作的员工可能需要一些机器。因此，需要在关系 `WORKS_FOR` 和实体 `MACHINERY` 之间建立需求关系。使用聚合，`WORKS_FOR` 与其实体 `EMPLOYEE` 和 `PROJECT` 的关系被聚合为单个实体，并且在聚合的实体和 `MACHINERY` 之间创建关系 `REQUIRE`。
