...
*   用于生成、操作和检索数据的 `SQL` 数据语句
*   创建数据库对象
*   使用 `SQL` 的内置函数来转换和操作数据
*   数据语句中的条件逻辑等等
...
*   学习解决复杂问题，修正问题，准备数据，操作、分组和报告。
*   数据库特定功能，例如 `SQL-PIVOT` 和 `UNPIVOT` 运算符、Oracle 的 `MODEL` 子句和 PostgreSQL 有用的 `GENERATE_SERIES` 函数。
*   窗口函数、将行旋转为列以及将列旋转回行。
*   如何创建直方图、生成累计值和小计等。
