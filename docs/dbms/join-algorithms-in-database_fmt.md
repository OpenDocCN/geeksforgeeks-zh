...
为了理解这些算法，我们将假设有两个关系，关系 `R` 和关系 `S`。关系 `R` 具有 `T_R` 元组并占据 `B_R` 块。关系 `S` 有 `T_S` 元组，占用 `B_S` 块。我们还将假设关系 `R` 是外部关系，`S` 是内部关系。
...
即 `min(B_R, B_S) + 1` 是主内存中连接两个关系所需的最小块数，这样就不会有块被传输超过一次。
