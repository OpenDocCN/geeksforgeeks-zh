...
> 原文: [https://www.geeksforgeeks.org/granting-permissions-to-roles-in-cassandra/](https://www.geeksforgeeks.org/granting-permissions-to-roles-in-cassandra/)
...
要解决此错误“`Unauthorized: Error from server: code=2100 [Unauthorized]: Message=User user_access has no SELECT permission on <all> or any of its parents`”，我们可以授予访问权限。

考虑一下，如果我们只想访问大学键空间上的“学生”表，那么我们可以使用下面的 `cqlsh` 查询。

```
cassandra@cqlsh> GRANT ALL PERMISSIONS ON TABLE university.student TO user_access;
```

将对 `university.student` 的所有权限授予 `user_access`。
...
```
cassandra@cqlsh> GRANT ALL PERMISSIONS ON KEYSPACE university TO user_access;
```

将 keyspace 大学的所有权限授予 `user_access`。
...
```
cassandra@cqlsh> GRANT ALL PERMISSIONS ON ALL KEYSPACES TO user_access;
```

将所有密键空间上的所有权限授予 `user_access`。
