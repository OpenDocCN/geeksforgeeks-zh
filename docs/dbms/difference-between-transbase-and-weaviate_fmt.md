...
`2。weavete:`
这是一个开源的基于 [`GraphQL`](https://www.geeksforgeeks.org/graphql/) 的智能图，其核心特征是:语义搜索、自动分类和知识表示。主数据库模型是搜索引擎，辅助数据库模型是图形数据库管理系统。它允许我们的数据以基于 [`GraphQL`](https://www.geeksforgeeks.org/graphql/) 查询语言的大型图形格式表示。这是一个基于图嵌入机制的搜索图，称为 Contextionary。
...
| 7. | ADO.NET、 [`JDBC`](https://www.geeksforgeeks.org/introduction-to-jdbc/) 、ODBC、专有原生 API 是 Transbase 的 API 和其他访问方式。 | [`GraphQL`](https://www.geeksforgeeks.org/graphql/) 查询语言和 [`RESTful`](https://www.geeksforgeeks.org/rest-api-introduction/) [`HTTP`](https://www.geeksforgeeks.org/what-is-http/)/[`JSON`](https://www.geeksforgeeks.org/json/) API 是 Weaviate 的 API 和其他访问方式。 |
| 8. | Transbase 有数据模式。 | 弱化数据方案基于到 [`GraphQL`](https://www.geeksforgeeks.org/graphql/) 接口的映射。 |
