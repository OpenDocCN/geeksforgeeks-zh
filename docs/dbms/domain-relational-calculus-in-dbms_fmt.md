...
> 原文: [https://www.geeksforgeeks.org/domain-relational-calculus-in-dbms/](https://www.geeksforgeeks.org/domain-relational-calculus-in-dbms/)
...
其中，`< x1, x2, x3, …, xn >` 代表结果域变量，`P (x1, x2, x3, …, xn )` 代表等价于谓词演算的条件或公式。
...
**注意：** 将出现在结果关系中的域变量必须出现在 `≺` 和 `≻` 之前，并且所有域变量必须按照它们在原始关系或表中的顺序出现。