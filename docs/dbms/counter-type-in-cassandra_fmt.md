...
在本文中，我们将讨论如何在 [Cassandra](https://www.geeksforgeeks.org/introduction-to-apache-cassandra/) 中创建计数器类型列，以及在将计数器列用作表中的数据类型时有什么限制。我们将看到如何通过使用 `update` 查询来更新计数器列。让我们一个一个来讨论。
...
现在，我们将借助 `Update` 子句为计数器列插入值。我们来看看。
...
现在，我们将在 `update` 子句的帮助下更新计数器列的值。我们来看看。
...
卡珊德拉拒绝在命令中使用时间戳或使用 TTL 来更新计数器列。因此，让我们创建一个 `View_Counts` 表。我们创建了 `count_view`，这是一个 `counter` 类型的计数器值。这非常重要，我们有一个 `name` 和 `blog_name`，我们可以看到主键是 `(name, blog_name)`。

如果我们试图创建一个类似的表，比如说 `copy_View_Counts`，主键将只有 `name`，我们会得到一个错误。因为如果计数器列和非计数器列不是主键，则不能在同一个表中混合使用它们。让我们来看看。
...
这里，所有非计数器列都必须是主键的一部分。但是，`primary key` 只有 `name`，这就是它给出错误的原因。让我们来看看。
