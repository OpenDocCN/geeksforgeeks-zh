...
> 原文：[https://www.geeksforgeeks.org/finding-additional-functional-dependencies-in-a-relation/](https://www.geeksforgeeks.org/finding-additional-functional-dependencies-in-a-relation/)
...
`B→Φ`：可导出，因为`Φ`是`B`的子集。
`B→A`：可从给定的函数依赖关系`B→A`推导出来。
...
因此，关系`R`上给定的函数依赖`A→B`和`B→A`可以确定的附加函数依赖总数为`13`，包括`A→B`，`B→A`如下：
