...
> 原文：[https://www.geeksforgeeks.org/difference-between-mongodb-and-amazon-cloudsearch/](https://www.geeksforgeeks.org/difference-between-mongodb-and-amazon-cloudsearch/)

## 1. MongoDB
MongoDB 是一个开源的面向文档的数据库，用于大容量数据存储。属于 `NoSQL` 数据库分类。`NoSQL` 工具意味着它不使用通常的行和列。MongoDB 使用 `BSON`（文档存储格式），这是 `JSON` 文档的二进制风格。
