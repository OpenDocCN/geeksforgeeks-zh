...
> 原文: [https://www.geeksforgeeks.org/export-and-import-data-in-cassandra/](https://www.geeksforgeeks.org/export-and-import-data-in-cassandra/)

先决条件 – [Cassandra](https://www.geeksforgeeks.org/introduction-to-apache-cassandra/)

在本文中，我们将讨论如何通过 `cqlsh` 查询导出和导入数据。我们一个一个来讨论。

## 创建表

首先，我们将创建一个表，即 `Data`，其中 `id`、`firstname`、`lastname` 是示例练习的字段。我们来看看。
...
开始复制列为 [`id`, `firstname`, `lastname`] 的数据。
...
现在，我们将从表 `Data` 中删除数据，以便从已经创建的 CSV 文件中再次导入。
...
正在启动集群 1 的副本。带有列 [`id`, `firstname`, `lastname`] 的数据。
...
要复制表中的特定行，请使用下面给出的 `cqlsh` 查询。首先，从表中导出数据，然后在这两个步骤之后截断，按照下面给出的步骤进行。
