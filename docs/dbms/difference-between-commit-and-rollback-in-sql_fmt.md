...
> 原文：[https://www.geeksforgeeks.org/difference-between-commit-and-rollback-in-sql/](https://www.geeksforgeeks.org/difference-between-commit-and-rollback-in-sql/)

## COMMIT

`COMMIT` 在 SQL 中是一种事务控制语言，用于将事务中所做的更改永久保存在表/数据库中。执行`提交`后，数据库无法恢复到之前的状态。
...
|    | COMMIT | ROLLBACK |
| --- | --- | --- |
| 1. | `COMMIT` 永久保存当前事务所做的更改。 | `ROLLBACK` 撤消当前事务所做的更改。 |
