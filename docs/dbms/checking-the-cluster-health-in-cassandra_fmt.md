...
> 原文: [https://www.geeksforgeeks.org/checking-the-cluster-health-in-cassandra/](https://www.geeksforgeeks.org/checking-the-cluster-health-in-cassandra/)
...
在 Cassandra 中，`nodetool status` 命令真的很有帮助。此命令使您能够检查群集节点的运行状况。`nodetool status` 命令对于提供关于节点间数据分布的信息也很有用。例如:
