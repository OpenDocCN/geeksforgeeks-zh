...
在数据库管理系统(DBMS)中，数据通过属性和元组(即列和行)以表格形式表示。有各种操作(插入、删除、更新、修改等。)我们可以在桌子上表演。一个这样的操作是连接。当我们将两个表合并成一个时，我们称之为连接，用`⨝`表示。
...
检查给定关系是否包含虚假元组的示例。设`R`为关系，`R1`和`R2`为分解`R`后得到的关系。
