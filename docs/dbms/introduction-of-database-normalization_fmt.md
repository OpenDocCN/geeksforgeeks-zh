...
> 原文：[https://www.geeksforgeeks.org/introduction-of-database-normalization/](https://www.geeksforgeeks.org/introduction-of-database-normalization/)
...
例如，`employee_id → name` 表示 `employee_id` 在功能上决定了员工的姓名。作为时间表数据库中的另一个例子，`{student_id, time} → {lecture_room}`，学号和时间决定了学生应该在哪个讲课室。
