...
> 原文：[https://www.geeksforgeeks.org/how-to-solve-relational-algebra-problems-for-gate/](https://www.geeksforgeeks.org/how-to-solve-relational-algebra-problems-for-gate/)
...
在本例中，我们将 `Order` 重命名为 `Order1` 和 `Order2`（两者表示相同的关系 `Order`）。然后我们在 `Order1` 和 `Order2` 之间应用了条件连接。它将返回那些 `Order1` 和 `Order2` 的 `Prod_Id` 和 `Order_Month` 相同但 `Order1` 和 `Order2` 的 `Agent_Id` 不同行。这意味着同一产品在同一个月内被两个不同的代理商订购。然后我们投影 `Prod_Id`。

因此，最终输出将返回不同代理商在同一个月订购的产品的产品标识。我们可以通过获取样本数据来做到这一点。令 `Order` 关系由以下数据组成。
...
投影 `Order1.Prod_Id` 后，输出将是 **P002**，这是同月至少有两个不同代理商订购的产品的 `Prod_Id`。

### 注意

如果我们想找到同一个月至少有三个不同代理商订购的 `Prod_Id`，可以按如下方式进行：
...
考虑以下带下划线主键的关系 `R(A, B, C)` 和 `S(B, D, E)`。关系 `R` 包含 200 个元组，关系 `S` 包含 100 个元组。自然连接 `R` 和 `S` 中可能的最大元组数量是多少？
...
如果明确提到 `S.B` 是 `R.B` 的外键，那么上面讨论的 Case-3 和 Case-4 是不可能的，因为 `S.B` 的值将来自 `R.B` 的值。因此，自然连接中元组的最小和最大数量将是 100。

### 参考

- [Navathe 数据库系统基础](https://amzn.to/3h4jBNg)
