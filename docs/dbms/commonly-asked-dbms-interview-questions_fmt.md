...
> 原文：[https://www.geeksforgeeks.org/commonly-asked-dbms-interview-questions/](https://www.geeksforgeeks.org/commonly-asked-dbms-interview-questions/)

## 1. 与传统的基于文件的系统相比，DBMS 有什么优势？

开发数据库管理系统是为了解决传统操作系统支持的典型文件处理系统的以下困难。
1. 数据冗余和不一致性
2. 数据访问困难
3. 数据隔离–多个文件和格式
4. 诚信问题
...
一个[主键](https://www.geeksforgeeks.org/difference-between-primary-and-candidate-key/)是候选键之一。其中一个候选键被选为最重要的，并成为主键。一个表中不能有多个主键。
一个外键是一个表中唯一标识另一个表的一行的字段(或字段集合)。
...
**DML** 代表数据操作语言。像 `SELECT`、`INSERT` 和 `UPDATE` 这样的 SQL 查询就属于这一类。
...
视图[视图](http://en.wikipedia.org/wiki/View_(SQL))是一个基于 SQL 语句结果集的虚拟表。我们可以使用 `CREATE VIEW` 语法来创建。
...
触发器是与 `INSERT`、`UPDATE` 或 `DELETE` 操作相关联的代码。每当对表执行关联查询时，代码都会自动执行。触发器对于维护数据库的完整性非常有用。
...
数据库事务是一组必须作为整体对待的数据库操作，这意味着要么执行所有操作，要么不执行任何操作。一个例子可以是从一个账户到另一个账户的银行交易。借方和贷方操作必须同时执行，否则不执行。`ACID` ([http://en.wikipedia.org/wiki/ACID](http://en.wikipedia.org/wiki/ACID)) (原子性、一致性、隔离性、持久性)是一组保证数据库事务得到可靠处理的属性。
...
1.  在`2NF`中。
2.  一个表的每个非质数属性都不依赖于该表的每个键。

## 40. 什么是数据库管理系统中的`BCNF`？

`BCNF`是比`3NF`更严格的`Boyce Codd`范式。

如果满足以下两个条件，则称任何表在`BCNF`中：

1.  在`3NF`中。
2.  对于存在的每个函数依赖项`X->Y`，`X`是表的超级键。
...
```
SELECT * FROM (SELECT rownum, empno FROM emp) WHERE MOD(rownum,2)=1;
```

如果要取偶数，可以使用以下查询：

```
SELECT * FROM (SELECT rownum, empno FROM emp) WHERE MOD(rownum,2)=0;
```

## 43. `SQL`中的模式匹配是如何完成的？

在`LIKE`运算符的帮助下，模式匹配在`SQL`中是可能的。“`%`”在与`0`个或更多字符匹配时与`LIKE`运算符一起使用，而“`_`”用于与一个特定字符匹配。
...
| **标准的选择** | 映射可以分两个阶段进行：<br>1. 独立于系统的映射，但依赖于数据模型<br>2. 根据特定的数据库管理系统定制模式 | 以下标准通常用于指导物理数据库设计选项的选择：<br>1. 响应时间<br>2. 空间利用<br>3. 交易吞吐量 |
...
`RAID`代表**R**edundant **A**rray of **I**nexpensive（或有时“独立”）**D**isks。
