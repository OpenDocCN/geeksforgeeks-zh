...
> 原文：[PostgreSQL 和 MongoDB 的区别](https://www.geeksforgeeks.org/difference-between-postgresql-and-mongodb/)
...
| :--- | :--- | :--- |
| **数据库类型** | 一种数据库系统 | 一种数据库系统 |
| **开发语言** | `MongoDB` 是用 `C++` 编写的 | `PostgreSQL` 是用 `C` 语言编写的 |
| **创立与维护** | `MongoDB` 是由 `10gen` 在 `2007` 年创立的，该公司基于*这个巨大的词*创造了这个产品 | `PostgreSQL` 是一个开源项目，由 `PostgreSQL` 全球开发小组及其多产社区维护 |
| **支持** | `MongoDB` 提供了社区支持论坛、服务器故障和 `StackOverflow`。用户还可以通过企业级支持获得 `24×7` 的企业级支持。 | `PostgreSQL` 为用户提供了各种各样的社区和商业支持选项。社区支持包括邮件列表和 `IRC` |
| **数据库模型** | 非关系数据库管理系统 | 关系数据库管理系统 |
| **数据模型** | 面向文档 | 面向对象的 |
| **语言支持** | 仅提供英语版本 | 有多种语言版本 |
