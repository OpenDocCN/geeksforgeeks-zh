...
> 原文: [https://www.geeksforgeeks.org/how-to-reset-mysql-root-password-in-windows-using-cmd/](https://www.geeksforgeeks.org/how-to-reset-mysql-root-password-in-windows-using-cmd/)
...
使用“文件 > 另存为”菜单将文件保存到硬盘的根目录。选择一个有意义的文件名，如 `mysql-init.txt`。

因此，`localhost` 命令将在您的本地系统上更改密码。如果您试图通过网络更改系统密码，请将主机名替换为 `localhost`。
...
反复检查以确保它有效。如果你有独特的配置选项(比如用 `--defaults-file` 选项启动 MySQL)，那就去做吧。
