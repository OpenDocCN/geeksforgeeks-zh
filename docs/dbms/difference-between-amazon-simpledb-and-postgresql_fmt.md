...
> 原文：[https://www.geeksforgeeks.org/difference-between-amazon-simpledb-and-postgresql/](https://www.geeksforgeeks.org/difference-between-amazon-simpledb-and-postgresql/)
...
这是一个由亚马逊托管的简单数据库服务，数据存储在亚马逊云中。亚马逊是一个实时运行结构化数据查询的网络服务。这个数据库是 Amazon.com 用 Erlang 写的。它具有以下特点——高可用性和灵活性，几乎没有或根本没有管理负担。
...
| 5. | 它有预定义的数据类型，如日期或浮点数。 | 它没有预定义的数据类型，如日期或浮点数。 |
| 6. | 它支持 SQL。 | 它不支持 SQL。 |
