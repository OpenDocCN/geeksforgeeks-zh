...
`CREATE TABLE` 命令中的 `column_name` 将告诉列的名称，相应的 `datatype` 将指定该列的数据类型。在此表中，三个列名即–`Student_id` 为 `INT` 类型，`Name` 为 `VARCHAR` 类型，`Marks` 为 `INT` 类型。
...
这将从表中删除所有记录。例如，下面的命令将从 `Student` 表中删除所有记录。
