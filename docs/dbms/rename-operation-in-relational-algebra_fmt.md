...
> 原文: [https://www.geeksforgeeks.org/rename-operation-in-relational-algebra/](https://www.geeksforgeeks.org/rename-operation-in-relational-algebra/)
...
*   **示例-1:** 查询将关系 `Student` 重命名为 `MaleStudent`，并将学生的属性 `RollNo`，`SName` 重命名为 (`Sno`, `Name`)。
...
*   **示例-2:** 查询将表 `Department` 的属性 `Name`、`Age` 重命名为 `A`、`B`。
