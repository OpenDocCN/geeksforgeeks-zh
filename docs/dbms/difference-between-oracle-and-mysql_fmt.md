...
甲骨文是一个关系型`数据库管理系统`(`RDBMS`)。它是由甲骨文公司在 1980 年开发的。它是第一个为网格计算设计的数据库，为管理信息和应用程序提供了最灵活和最经济的方式。它运行在主要平台上，如视窗、`Unix`、`Linux` 和 `macOS`。它是一个关系数据库，用户通过应用程序或称为 `SQL` 的查询语言访问其中的数据。

## 2. MySQL

`MySQL` 是基于`结构化查询语言`(`SQL`) 的开源关系数据库管理系统(`RDBMS`)。它运行在 `Linux`、`UNIX` 和 `Windows` 等平台上。
