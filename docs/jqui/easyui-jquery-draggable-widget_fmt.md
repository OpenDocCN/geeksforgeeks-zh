...
*   `proxy`: 类型为字符串或函数。用于设置“克隆”的代理元素。克隆元素用作代理。如果指定了一个函数，它必须返回一个 jQuery 对象。
*   `revert`: 是布尔类型。当拖动停止时，元素将返回到其起始位置。
*   `cursor`: 为字符串类型。拖动时要显示的 CSS 光标。
*   `deltaX`: 类型号。对应于当前光标的拖动元素位置 x。
*   `deltaY`: 类型号。对应于当前光标的拖动元素位置 y。
*   `handle`: 它的类型是选择器。启动可拖动的句柄。
*   `disabled`: 类型为布尔型。如果设置为 `true`，可拖动功能将被禁用。
*   `edge`: 它的类型是数字。可用于启动可拖动属性的拖动宽度。
*   `axis`: 它的类型是字符串。它设置被拖动元素移动的轴。
*   `delay`: 它的类型是延迟。它以毫秒为单位定义时间延迟。
...
*   `onEndDrag`: 参数是一个事件。拖动结束时会触发。
*   `onStopDrag`: 参数是一个事件。拖动停止时会触发。

**方法:**

*   `options`: 返回选项属性。
*   `proxy`: 如果设置了代理属性，则返回拖动代理。
*   `enable`: 启用拖动动作。
*   `disable`: 禁用拖动动作。
