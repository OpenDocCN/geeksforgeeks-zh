# jQuery UI Datepicker monthNames 选项

> 哎哎哎:# t0]https://www . geeksforgeeks . org/jquery-ui-date picker-monthname 选项/

## 简介

jQuery UI 由 GUI 小部件、视觉效果和使用 jQuery、CSS 和 HTML 实现的主题组成。jQuery 用户界面非常适合为网页构建用户界面。 jQueryUI 中的 `jQueryUI Datepickers` 小部件允许用户轻松直观地输入日期。在本文中，我们将看到如何在 jQuery UI Datepicker 中使用 `monthNames` 选项。`monthNames` 选项用于在 jQuery UI 日期选择器中设置月份名称。
