...
jQuery UI 由 GUI 小部件、视觉效果和使用 HTML、CSS 和 jQuery 实现的主题组成。jQuery 用户界面非常适合为网页构建用户界面。 jQuery UI 选项卡小部件帮助我们将一些内容放在不同的选项卡中，允许我们在它们之间切换。在本文中我们将看到如何在 jQuery UI 滑块中使用`load()`方法。`load()`方法强制重新加载 jQuery UI 中的索引选项卡。
...
[https://api.jqueryui.com/tabs/#method-load](https://api.jqueryui.com/tabs/#method-load)