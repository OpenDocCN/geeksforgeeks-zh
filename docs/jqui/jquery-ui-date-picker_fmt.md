...
`管理工作日:` 默认情况下，从周日开始显示一周的第一天(`firstDay`=0)。我们可以通过更改 `firstDay` 的值来更改开始日期。
...
`minDate:` 最小可选日期。`maxDate:` 最大可选日期。在关于日期选择器的上一篇文章的末尾有一个例子。又来了。

两个日历的互锁
`日历的变化功能:` 我们将使用 `change` 功能来触发事件。每当启动日历的 `change` 功能被触发时，我们将为结束日历设置 `minDate`。
同样，当触发结束日历的任何 `change` 功能时，我们将设置开始日历的 `maxDate`。

`getDate()` 方法返回日历的选定日期。这里有一个例子
