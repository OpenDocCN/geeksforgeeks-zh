...
在本文中，我们将学习如何使用 `jQuery EasyUI` 设计数据列表。数据列表呈现列表中的项目。它是一个特殊的数据网格，在一列中显示数据。
...
*   `rows`: 该属性定义显示行数。
*   `checkbox`: 定义是否为每行显示一个复选框。
*   `valueField`: 要绑定到行的字段值名称。
*   `textField`: 要在该行上显示的字段名称。
*   `groupField`: 指示要分组的字段。
*   `textFormatter`: `textField` 格式化程序功能。
*   `groupFormatter`: 格式化程序函数返回组内容。

## CDN 链接

首先，添加你的项目所需的 `jQuery EasyUI` 脚本，链接是本地路径。
