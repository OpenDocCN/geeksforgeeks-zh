# jQuery UI 进度条小部件完整参考

> 原文：[https://www.geeksforgeeks.org/jquery-ui-progressbar-widget-complete-reference/](https://www.geeksforgeeks.org/jquery-ui-progressbar-widget-complete-reference/)

`jQuery UI Progressbar` 小部件用于添加确定或不确定过程的显示状态。这个小部件中有很多选项、方法和事件，它们都在下面列出和分类。
...
*   `jQuery UI Progressbar` `更改`事件
*   `jQuery UI Progressbar` `完成`事件
*   `jQuery UI Progressbar` `创建`事件