...
按钮的可视化分组使用 jQuery UI `buttonset()`方法处理。
...
以下示例演示了带有选项`text`和`icon`的 jQuery UI `button()`方法的使用。还处理其他选项，例如通过`iconPosition`选项将图标位置设置为“开始”或“结束”。按钮上有一两个图标。如下图所示，`primary`图标设置在左侧，`secondary`图标设置在右侧。jQuery UI `button()`的库中有这么多可用的选项，程序员可以根据项目需求选择或自定义其他选项。
...
jQuery UI `button()`提供了许多方法和选项，可用于控制可主题化的小部件。程序员可以根据需要使用它们中的任何一个。该示例仅演示了其中一些作为指南。
以下代码演示了按钮触发的`click`和`change`事件，以及使用`icons`、`destroy`和`disable`等选项。jQuery UI `button()` 方法也支持已实现的事件管理。还支持 jQuery UI 按钮的`enable`和`disable`状态。
...
*   **点击按钮前:**
    ![](img/c4aa579c57bbfeab720a1412707644cd.png)
*   **点击按钮后:**
