...
> 原文：[https://www.geeksforgeeks.org/how-to-display-date-picker-in-the-page-using-jquery-ui/](https://www.geeksforgeeks.org/how-to-display-date-picker-in-the-page-using-jquery-ui/)
...
下面的代码演示了一个简单的日期选择器。我们将从输入元素中启动日期选择器。
...
下面的代码演示了一个定制的日期选择器。我们的日期选择器没有年或月更改按钮。另外，格式也不好看。所以我们改变它们如下。

*   `changeMonth`：是布尔类型，如果设置为 `true`，我们可以更改月份。
*   `changeYear`：是布尔类型，如果设置为 `true`，我们可以更改年份。
*   `minDate`：设置最小日期。
*   `maxDate`：设置最大日期。设为 `+1` 表示明天，设为 `-1` 表示昨天。如果您想将日期设置为明年，请使用 `+1y`，反之亦然。
*   `dateFormat`：设置日期格式。`"dd/mm/yy"` 首先将日期设置为日期，然后是月份，然后是年份。
