...
> 原文: [https://www.geeksforgeeks.org/jquery-ui-sortable-update-event/](https://www.geeksforgeeks.org/jquery-ui-sortable-update-event/)
...
jQuery UI 可排序 `update` 事件用于在用户停止排序且 DOM 位置发生变化时触发。

## 语法

用 `update` 回调函数初始化可排序的小部件。
...
*   `event`: 当用户用 DOM 的位置变化停止排序时，触发该事件。
*   `ui`: 这是一个具有以下选项的对象类型。
    *   `helper`: 表示排序后的助手的 jQuery 对象。
    *   `item`: 表示当前拖动项的 jQuery 对象。
    *   `offset`: 表示为 `{top, left}` 的辅助对象的当前绝对位置。
    *   `position`: 表示为 `{top, left}` 的辅助对象的当前位置。
    *   `originalPosition`: 辅助对象的原始位置，表示为 `{top, left}`。
    *   `sender`: jQuery 对象类型的可排序项来自从一个可排序项移动到另一个可排序项。
    *   `placeholder`: 用作占位符的元素。这是 jQuery 对象的类型。
...
[https://api.jqueryui.com/sortable/#event-update](https://api.jqueryui.com/sortable/#event-update)