...
*   `duration`: 悬停在按钮上时显示菜单的持续时间(毫秒)。
*   `menu`: 创建相应菜单的选择器。
*   `plain`: 定义效果是一个 `boolean` 值，默认值为 `true` 表示素色效果。

**方法:**

*   `options`: 此方法返回选项对象。
*   `disable`: 已用于禁用 `splitbutton`。
*   `enable`: 已用于启用 `splitbutton`。
*   `destroy`: 该方法用于销毁 `splitbutton`。

**CDN 链接:** 首先，添加你的项目所需的 jQuery EasyUI 脚本，链接是`本地路径`。
