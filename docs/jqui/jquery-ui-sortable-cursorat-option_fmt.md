...
jQuery UI 由 GUI 小部件、视觉效果和使用 HTML、CSS 和 jQuery 实现的主题组成。jQuery 用户界面非常适合为网页构建用户界面。jQuery UI 可排序光标选项用于在排序时将光标移动到给定的位置。此选项与 `cursor` 选项一起使用。光标位置的值由 `{top, left, right, bottom}` 选项设置。
...
**参考:** [https://api.jqueryui.com/sortable/#option-cursorAt](https://api.jqueryui.com/sortable/#option-cursorAt)