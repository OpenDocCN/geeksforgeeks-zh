...
jQuery UI 由 GUI 小部件、视觉效果和使用 jQuery、CSS 和 HTML 实现的主题组成。jQuery 用户界面非常适合为网页构建用户界面。jQuery UI 日期选择器的 `shortYearCutoff` 选项用于设置日期的世纪。小于或等于截止年的输入年份值被视为当前世纪。比那个大的年份被认为是在上个世纪。
