...
`HTTP Location` 头是在两种情况下使用的响应头，用于要求浏览器重定向 URL(状态代码 3xx)或提供关于新创建的资源的位置的信息(状态代码 201)。它的用法经常与另一个 HTTP 头 `HTTP Content-Location` 头混淆。它们之间的主要区别在于位置给出了页面重定向发生的资源的 URL，而 `HTTP Content-Location` 用于指示传输的资源的 URL。
