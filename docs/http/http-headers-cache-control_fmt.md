...
`Cache-Control` 头是一个通用头，它指定了服务器响应和客户端请求的缓存策略。基本上，它给出了关于特定资源的缓存方式、缓存资源的位置及其在过期前达到的最大寿命（即生存时间）的信息。
...
*   `Maximum outdated [= < seconds >]`: means that the client only accepts outdated responses. The optional time in seconds indicates the maximum acceptable obsolescence limit for the client.
...
浏览器兼容 **HTTP Cache-Control 头**列表如下: