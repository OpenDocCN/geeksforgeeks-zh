...
这个头接受 2 个指令，但是在指令上可以用其他指令重定向，如下所述:
...
*   `accelerometer`: 用于获取设备加速度信息
*   `ambient-light-sensor`: 用于获取周围光线是否充足的信息
*   `autoplay`: 用于控制媒体`自动播放`设置。主要用于音频和视频元素
*   `battery`: 用于使用电池状态 API 获取电池状态
*   `camera`: 用于通过设备的摄像头控制视频输入
*   `display-capture`: 通过截图捕捉屏幕内容
*   `document-domain`: 通过设置或`取消设置`来控制当前文档的文档域
*   `encrypted-media`: 用于控制加密媒体扩展接口(EME)
*   `fullscreen`: 用于控制全屏访问
*   `geolocation`: 使用`地理定位` API 在地图上显示用户位置
*   `gyroscope`: 用于获取访问设备陀螺仪的设备方位信息
*   `layout-animations`: 用于显示布局动画和过渡
*   `legacy-image-formats`: 以传统格式显示图像
*   `magnetometer`: 用于获取设备磁方位信息
*   `microphone`: 用于通过设备的麦克风进行音频输入
*   `midi`: 用于访问 Web MIDI API
*   `oversized-images`: 用于显示和下载超大图片
*   `payment`: 使用支付请求 API 控制所有支付相关活动
*   `picture-in-picture`: 允许视频以画中画模式播放
*   `publickey-credentials-get`: 使用网络身份验证应用编程接口并检索公钥凭证
*   `sync-xhr`: 用于制作同步`XMLHTTPRequest`
*   `usb`: 控制`网络 usb`接口进行 USB 媒体访问
*   `wake-lock`: 使用唤醒锁定 API 通知设备不要进入省电模式
*   `xr-spatial-tracking`: 用于利用`WebXR`设备 API 与`WebXR`会话进行交互
