...
在本例中，第一个 pin `pin-sha256="cUPcTAZWKaASuYWhhneY3oBAkE3h2+soZS7sWs="` 是生产中使用的服务器公钥。
第二个针脚 `pin-sha256="M8HztCzM3elS5P4hhyBNf6lHkmjAHKhpGPWE="` 用作备用钥匙。
第三针 `max-age=51000` 告知客户端将此信息保存两个月(此时限由 IETF RFC 提供)。
第四个引脚 `includeSubDomains` 表示密钥对所有子域有效。
最后最后一个 pin `report-uri="https://www.geeksforgeeks.org/hpkp-report"` 说明了在哪里报告 pin 验证失败。

## 浏览器兼容性

与 `Public-Key-Pins` 头兼容的浏览器如下:
