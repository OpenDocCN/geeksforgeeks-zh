...
**推荐人策略** HTTP 头设置了请求时与推荐人头一起发送的信息量参数。`Referrer-Policy` 策略用于在获取资源或执行导航时维护源帐户的安全性和隐私性。这是通过修改用于填充推荐人`标题`的算法来完成的。
...
*   **`no-referrer-when-downgrade`**: This will send the complete website information to a potential trusted website, which comes from *modern* HTTPS state or non-modern **HTTPS** state. Message sent to **https-> https** and **http-> https** transitions. This is the default referrer-policy.
...
*   **`origin-when-cross-origin`**: When making a request at the same origin, send complete URL information, and when making a request across origin, send only the origin information.
...
*   **`same-origin`**: When the origin is in the same website, but there is no cross-origin information sent, the referral information will be sent.
*   **`unsafe-url`**: It sends complete URL information regardless of any standard.

## 示例

这是万维网联盟给出的标准示例。此处的示例列出了请求作为导航网站发送到的网站以及与其一起发送的推荐人信息。为方便起见，`https://example.com/page.html`将被视为每个示例的原点。
...
下面列出了 **HTTP 头 `Referrer-Policy`** 支持的浏览器