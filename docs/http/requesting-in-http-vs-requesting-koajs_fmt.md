...
> 原文:[https://www.geeksforgeeks.org/requesting-in-http-vs-requesting-koajs/](https://www.geeksforgeeks.org/requesting-in-http-vs-requesting-koajs/)
...
导入 `http` 模块，并将返回的 `http` 实例存储到变量中。
...
使用 `createServer()` 方法创建一个服务器实例，并使用 `listen()` 方法将其绑定到某个端口。
...
它与客户端和服务器通信请求和响应。它需要 `PORT` < *号* >和 `IP` < *号* >进行通信。
