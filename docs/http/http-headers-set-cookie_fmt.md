...
*   `Expires=<date>`: 这是一个可选指令，包含 `cookie` 的过期日期。
*   `Max-Age=<non-zero-digit>`: 它以秒的数字格式包含寿命，零或负值会使 `cookie` 立即过期。
*   `Domain=<domain-value>`: 该指令定义了发送 `cookie` 的主机。这是一个可选指令。
*   `Path=<path-value>`: 该指令定义了一个必须存在于请求的网址中的路径，否则浏览器无法发送 `cookie` 头。
