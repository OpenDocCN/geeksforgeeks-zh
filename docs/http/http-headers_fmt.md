...
`HTTP` 头用于通过 `请求` 和 `响应` 头在客户端和服务器之间传递附加信息。所有标题都不区分大小写，标题字段用冒号分隔，键值对采用明文字符串格式。由空字段标题表示的标题部分的结尾。有几个标题字段可以包含注释。一些头可以包含用等号分隔的质量(q)键值对。
...
| [`语用学`](#) | 它是一般类型的头，但是没有指定响应行为，因此是特定于实现的。 |
...
| 页眉 | 描述 |
| :--- | :--- |
| [`期待`](https://www.geeksforgeeks.org/http-headers-expect/) | 这是一个请求类型头。它用于指示服务器为了响应客户端而需要满足的特定行为或期望。通常，期望:`100-继续`是为标题字段定义的唯一期望。 |
...
| 页眉 | 描述 |
| :--- | :--- |
| [`严格-传输-安全`](https://www.geeksforgeeks.org/http-headers-strict-transport-security/) | 它是一个响应头，用于告知用户代理应该仅使用 `HTTPS` 与给定服务器进行通信，从而保护用户免受中间人攻击。 |
| [`X-内容-类型-选项`](https://www.geeksforgeeks.org/http-headers-x-content-type-options/) | 它是一个响应头，由服务器用于指示 `MIME` 类型在 `Content-Type` 标头中声明的 `MIME` 类型是否应遵循并被尊重。 |
| [`X-帧-选项`](https://www.geeksforgeeks.org/http-headers-x-frame-options/) | 它是一个响应头，可以用于指示是否应允许浏览器在 `<frame>`、`<iframe>`、`<embed>` 或 `<object>` 中呈现页面。 |
| [`X-XSS-保护`](https://www.geeksforgeeks.org/http-headers-x-xss-protection/) | 这是一个响应头，可用于配置 `XSS` 筛选器的运行。 |
...
| [`X-框架-选项`](https://www.geeksforgeeks.org/http-headers-x-frame-options/) | 这是一个响应头。它用于防止网站遭受点击劫持攻击。它定义了是否允许浏览器在 `<iframe>`、`<embed>` 或 `<object>` 中呈现页面。 |
...
| [`重试-在`](https://www.geeksforgeeks.org/http-headers-retry-after/) | 它是响应类型的头，用于通过 HTTP 请求或响应传递附加信息。HTTP 重试后标头是一个 HTTP 响应标头，它指示在发出另一个请求之前要等待多长时间。 |
...
**端到端头部：** 这种类型的头部应该传输给消息的最终接收者，以便服务器可以向客户端发出请求，客户端可以响应该请求。中间代理必须原样重传这些头部。

**逐跳报头：** 这种类型的报头仅适用于单个传输级连接。这种头部不应该被代理重传或缓存。