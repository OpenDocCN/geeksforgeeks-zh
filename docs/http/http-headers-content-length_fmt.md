# HTTP 头：Content-Length

> 原文：[https://www.geeksforgeeks.org/http-headers-content-length/](https://www.geeksforgeeks.org/http-headers-content-length/)

`HTTP` 头用于在 `HTTP` 请求或 `HTTP` 响应中传递附加信息。`HTTP Content-Length` 实体头用于以八进制数（即字节数）表示实体体的大小，并将其发送给收件人。这是一个禁止的标题名称。基本上是请求或响应正文中的数据字节数。正文在标题下面的空行之后。
