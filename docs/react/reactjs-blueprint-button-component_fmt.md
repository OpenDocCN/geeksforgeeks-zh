...
> 原文: [https://www.geeksforgeeks.org/reactjs-blueprint-button-component/](https://www.geeksforgeeks.org/reactjs-blueprint-button-component/)
...
*   `active`: 如果设置为真，按钮将显示激活状态。
*   `alignText`: 用于按钮内的文本对齐。
*   `className`: 用于表示传递给子元素的以空格分隔的类名列表。
*   `disabled`: 用于表示该动作是否非交互。
*   `elementRef`: 用于表示接收该组件呈现的原生 HTML 元素的引用处理程序或引用对象。
*   `fill`: 用于指示该按钮是否应该展开以填充其容器。
*   `icon`: 用于表示要在文本前渲染的蓝图 UI 图标(或图标元素)的名称。
*   `intent`: 用于表示应用于元素的视觉意图颜色。
*   `large`: 用于表示该按钮是否应该使用大样式。
*   `loading`: 当设置为真时，按钮将显示居中的加载微调器，而不是其内容。
*   `minimal`: 用于表示该按钮是否应该使用最小样式。
*   `onClick`: 用于表示点击事件处理程序。
*   `outlined`: 用于表示该按钮是否应该使用轮廓样式。
*   `rightIcon`: 用于表示文本后要渲染的蓝图 UI 图标(或图标元素)的名称。
*   `small`: 用于表示该按钮是否应该使用小样式。
*   `text`: 用于表示动作文本。
*   `type`: 用于表示按钮的 HTML 类型属性。
