# React Suite Radio 组件

> 原文: [https://www.geeksforgeeks.org/react-suite-radio-component/](https://www.geeksforgeeks.org/react-suite-radio-component/)

React Suite 是一个流行的前端库，包含一组为中间平台和后端产品设计的 React 组件。Radio 组件允许用户从一组中选择一个选项。我们可以在 ReactJS 中使用以下方法来使用 React Suite 无线电组件。

## Radio Props

*   `checked`: 表示收音机是否被选择。
*   `defaultChecked`: 用于指定是否选择收音机的初始状态。
*   `disabled`: 当设置为真时，用于禁用收音机。
*   `inline`: 用于内联布局。
*   `inputRef`: 用于表示输入元素的 Ref。
*   `name`: 用于表示无线电组件的名称属性。
*   `onChange`: 是状态变化时触发的回调函数。
*   `title`: 用来表示 HTML 标题。
*   `value`: 用于表示 RadioGroup 的值。
...
[https://rsuitejs.com/components/radio/](https://rsuitejs.com/components/radio/)