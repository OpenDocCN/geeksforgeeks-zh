...
> 原文: [https://www.geeksforgeeks.org/reactjs-onsen-ui-listitem-component/](https://www.geeksforgeeks.org/reactjs-onsen-ui-listitem-component/)
...
*   `expandable`: 用于指定列表项是否可以展开以显示隐藏内容。
*   `expanded`: 用于指定可展开列表项是否展开。

## 预设修改器

*   `tappable`: 用于列表项被点击时改变外观。
*   `chevron`: 用于在列表项的右端显示一个人字形，点击后外观会发生变化。
*   `longdivider`: 用于显示项目之间的长水平分隔线。
*   `nodivider`: 用于移除列表项之间的分隔线。
*   `material`: 用于显示材料设计列表项。
...
[https://onsen.io/v2/api/react/ListItem.html](https://onsen.io/v2/api/react/ListItem.html)