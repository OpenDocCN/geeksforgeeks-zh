...
> 原文: [https://www.geeksforgeeks.org/consuming-a-rest-api-github-users-using-fetch-react-client/](https://www.geeksforgeeks.org/consuming-a-rest-api-github-users-using-fetch-react-client/)
...
自定义组件驻留在 `components` 文件夹中，所有的东西都放在 `MainComponent.js` 中，我们将这个组件放在 `App.js` 中，它本身位于“根”DOM 节点下，这个节点中的所有东西都将由 React DOM 管理。
...
## 第四步: 在 MainComponent.js 组件中

我们有两个状态变量， `username` (来自用户的输入)和 `userData`(来自 REST API 的响应)。我们需要获取用户数据，每次 `username` 有更新。为了实现这一点，我们将使用 `useEffect` 钩子从 React。
...
## 第五步: 获取 GitHub 用户 API 的响应

现在为了得到 GitHub 用户 API 的响应，我们将使用 Fetch 发出 `GET` 请求，这将是 `getUserData()` 函数的角色。
...
`getUserData()` 是一个 `async` 函数，其中 `fetch(gitHubUrl)` 发出请求并返回一个承诺。请求完成后，承诺以 `response` 对象解决。这个对象基本上是各种响应格式的通用占位符。 `response.json()` 用于从响应中提取 json 对象，它返回一个承诺，因此出现 `await`。
...
现在，进入搜索栏组件，用于接收 `username` 的用户输入。是一个简单的组件，有一个文本类型的输入字段。一旦有任何变化，它将使用更新后的值触发 `setUsername`。
...
我们最后一个组件，是一个可复用的 UI 组件，基本上是一个卡片组件，接收 `userData` 作为道具，只是以任意选择的格式显示。你可以调整 `App.css` 文件来了解各种设计方面。
...
**源代码:** `https://github.com/notnotaparas/github-usersAPI`