...
蚂蚁设计库预建了这个组件，也很容易集成。自动完成组件用于用选项值自动完成自由文本值。我们可以在 ReactJS 中使用以下方法来使用 Ant 设计自动完成组件。
...
*   `backfill`: 如果该值为真，则在使用键盘输入时，它会回填选定的项目。
*   `children(用于自定义输入元素)`: 用于自定义输入元素。
*   `children(对于数据源)`: 用于数据源自动完成。
*   `defaultActiveFirstOption`: 用于表示第一个选项是否默认激活。
*   `defaultOpen`: 表示下拉菜单的初始打开状态。
*   `defaultValue`: 用于表示最初选择的选项。
*   `disabled`: 用于禁用选择。
*   `dropdownClassName`: 用于传递下拉菜单样式的类名。
*   `dropdownMatchSelectWidth`: 用于判断选择输入和下拉菜单的宽度是否相同。
*   `filterOption`: 如果设置为真，则用于对其应用过滤选项。
*   `notFoundContent`: 用于显示没有结果匹配时的内容。
*   `open`: 用于控制下拉菜单的打开状态。
*   `options`: 用于传递选择选项。
*   `placeholder`: 表示输入的占位符。
*   `value`: 用于表示选择的选项值。
*   `onBlur`: 是离开组件时触发的回调函数。
