...
*   **`required`**: 当设置为真时，`textarea` 元素是必需的。
*   **`disabled`**: 当设置为真时，`textarea` 元素禁用。
*   **`isInvalid`**: 用于将 *only* 文本输入的视觉样式设置为无效。
*   **`spellCheck`**: 用于浏览器的原生拼写检查功能。
*   **`grammarly`**: 用于允许语法浏览器扩展附加到后台文本区。
*   **`placeholder`**: 用于表示占位符文本。
*   **`appearance`**: 用于文本区的外观。
*   **`width`**: 用于表示文本区域的宽度。
*   **`className`**: 用于将类名传递给按钮。

## TextareaField Props

*   **`label`**: 用于表示输入元素上方使用的标签。
*   **`labelFor`**: 用于表示标签上作为 `htmlFor` prop 传递的。
*   **`isRequired`**: 用于表示标签后是否显示星号。
*   **`description`**: 定义标签下方和输入元素上方字段的可选描述。
*   **`hint`**: 用于定义输入元素下的可选提示。
...
![输出](img/23148b9803a9e9be5754f92199c237aa.png)

## 参考

[https://evergreen.segment.com/components/textarea](https://evergreen.segment.com/components/textarea)