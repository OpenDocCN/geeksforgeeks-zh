...
Reaction Desktop 是一个将原生桌面体验带到 Web 上的受欢迎的库。该库提供 MacOS 和 Windows OS 组件。窗口组件用于允许用户创建窗口。我们可以在 ReactJS 中使用以下方法来使用 Reaction Desktop Windows 窗口组件。

## 窗口道具

*   `background`：用于设置组件背景颜色。
*   `chrome`：用于设置窗口的铬。
*   `color`：用于设置组件及其子组件的主颜色。
*   `height`：用于设置元件高度。
*   `hidden`：用于设置零部件可见性。
...
[https://reactdesktop.js.org/docs/windows/window](https://reactdesktop.js.org/docs/windows/window)