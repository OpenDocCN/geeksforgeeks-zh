...
> 原文：[https://www.geeksforgeeks.org/reactjs-reactstrap-collapse-component/](https://www.geeksforgeeks.org/reactjs-reactstrap-collapse-component/)
...
*   `innerRef`: 用于表示内部参照元素。
*   `mountOnEnter`: 用于安装组件，等待第一个*进入*转换被触发。
*   `unmountOnExit`: 用于卸载组件。
*   `appear`: 组件挂载时，用于运行收拢动画。
*   `enter`: 用于启用或禁用输入转换
*   `exit`: 用于启用或禁用退出转换
*   `timeout`: 用于表示折叠动画的持续时间，单位为毫秒。
*   `addEndListener`: 用于表示监听结束事件的函数。
*   `onEnter`: 是组件在中折叠之前触发的回调函数。
*   `onEntered`: 它是一个回调函数，在组件收拢后触发。
*   `onEntering`: 是组件开始收拢后触发的回调函数。
*   `onExit`: 是组件崩出前触发的回调函数。
*   `onExited`: 是组件折叠出来后触发的回调函数。
...
[https://reactstrap.github.io/components/collapse/](https://reactstrap.github.io/components/collapse/)