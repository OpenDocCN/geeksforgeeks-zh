...
*   `bottom`: 当位置为`bottomRight`或`bottomLeft`时，用于表示与视口底部的距离。
*   `btn`: 用于自定义关闭按钮。
*   `className`: 用于自定义 CSS 类。
*   `closeIcon`: 用于自定义关闭图标。
...
*   `key`: 用于通知的唯一标识。
*   `message`: 用于表示通知框的标题。
*   `placement`: 用于放置通知。值可以是`top`、`topRight`、`bottomLeft`和`bottomRight`。
*   `style`: 用于定制的直列样式。
*   `top`: 当放置在`top`或`topRight`时，用于表示与视口顶部的距离。
