...
> 原文:[https://www.geeksforgeeks.org/how-to-create-a-translucent-text-input-in-reactjs/](https://www.geeksforgeeks.org/how-to-create-a-translucent-text-input-in-reactjs/)
...
*   `Wrapper`、`Input`、`Label`、`Underline` 是 *Component.jsx* 文件中用来组成文本输入框的样式组件。
*   在 *Component.jsx* 文件中，我们使用来自 *Component.motion.js* 文件的带有自定义动画变体的 `framer-motion` 来制作文本输入框的动画。
*   React **useState** hook 用于管理 `value` 的状态，该值用作占位符属性，在激活时也将其设置为标签。
*   `framer-motion` 使用 `useCycle` 钩子类似于 `useState` 钩子。它在一系列用于动画的视觉属性中循环。它用于在动画变体之间切换或循环。
