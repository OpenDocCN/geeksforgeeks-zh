...
> 原文：[https://www.geeksforgeeks.org/what-is-reactor-pattern-in-node-js/](https://www.geeksforgeeks.org/what-is-reactor-pattern-in-node-js/)
...
2.  **非阻塞 I/O**：应用程序将进行函数调用，并且不等待结果就继续执行。它被称为“异步”。
...
2.  事件解复用器由 `libuv` 运行，`libuv` 是一个允许 JavaScript 代码（通过 `V8`）执行 I/O、网内、文件等的库。它是一个异步 I/O 库，允许 Node.js 执行 I/O。
