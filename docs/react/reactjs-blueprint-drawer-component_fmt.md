...
> 原文: [https://www.geeksforgeeks.org/reactjs-blueprint-drawer-component/](https://www.geeksforgeeks.org/reactjs-blueprint-drawer-component/)
...
*   `backdropClassName`: 用于表示应用于背景元素的 CSS 类名。
*   `backdropProps`: 用于表示背景元素的 HTML 道具。
*   `canEscapeKeyClose`: 用于指示按 esc 键是否应调用 `onClose`。
*   `canOutsideClickClose`: 用于指示在覆盖元素外单击是否应该调用 `onClose`。
*   `className`: 用于表示传递给子元素的以空格分隔的类名列表。
*   `enforceFocus`: 用于表示叠加是否要防止焦点自行离开。
*   `hasBackdrop`: 用于指示是否应该在内容后面呈现跨容器的 background 元素。
*   `icon`: 用于表示要在抽屉标题中呈现的图标或图标元素的名称。
*   `isCloseButtonShown`: 用于指示是否在对话框的表头显示关闭按钮。
...
*   `position`: 用于抽屉的位置。
*   `shouldReturnFocusOnClose`: 用于指示在此抽屉关闭后，应用程序是否应将焦点返回到文档中的最后一个活动元素。
*   `size`: 用来表示抽屉的 CSS 大小。
*   `style`: 用于表示应用于对话框的 CSS 样式。
*   `title`: 用于表示对话框的标题。
*   `transitionDuration`: 用于指示叠加的进入/离开过渡需要多长时间，以毫秒为单位。
*   `transitionName`: 用于表示内部 `CSSTransition` 的转换名称。
*   `usePortal`: 用于指示覆盖图是否应该包装在门户中。
*   `vertical`: 用于表示抽屉是否应该以垂直造型出现。
...
[https://blueprintjs.com/docs/#core/components/drawer](https://blueprintjs.com/docs/#core/components/drawer)