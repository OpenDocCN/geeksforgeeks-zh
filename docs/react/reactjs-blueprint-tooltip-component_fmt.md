...
> 原文: [https://www.geeksforgeeks.org/reactjs-blueprint-tooltip-component/](https://www.geeksforgeeks.org/reactjs-blueprint-tooltip-component/)

Blueprint 是一个基于 React 的网络用户界面工具包。该库非常适合构建桌面应用程序的复杂数据密集型界面，并且非常受欢迎。

工具提示组件为用户提供了一种在悬停交互期间显示附加信息的方式，因为这是一个轻量级弹出窗口。我们可以在 ReactJS 中使用以下方法来使用 ReactJS Blueprint 工具提示组件。

## 工具提示道具

*   **autoFocus**: 用于指示叠加首次打开时是否应获取应用焦点。
*   **boundary**: 用于确定 Popper 用于其翻转和 preventOverflow 修改器的边界元素。
*   **canEscapeKeyClose**: 用于指示按 esc 键是否应该调用 `onClose`。
*   **captureDismiss**: 当用户点击一个类时，捕捉消失。当这个参数设置为真时，`popover` 元素将只关闭当前 `POPOVER`，而不关闭外部 `POPOVER`。
*   **className**: 用于表示传递给子元素的以空格分隔的类名列表。
*   **content**: 用于表示工具提示内部将要显示的内容。
*   **defaultIsOpen**: 用于表示不受控制时的初始打开状态。
*   **disabled**: 用于防止真时爆音出现。
*   **enforceFocus**: 用于表示叠加是否要防止焦点自行离开。
*   **hoverCloseDelay**: 用于表示用户悬停在触发器上后，工具提示应保持打开的时间，以毫秒为单位。
...
*   **modifiers**: 用于表示 `popper` 修改器选项，直接传递给内部 `Popper` 实例。
...
*   **usePortal**: 用于指示弹出窗口是否应该在附加到门户容器道具的门户中呈现。
...
![输出](img/0b3b3c5bdb46f62c8df16a299e2af58e.png)

## 参考

[https://blueprintjs.com/docs/#core/components/tooltip](https://blueprintjs.com/docs/#core/components/tooltip)