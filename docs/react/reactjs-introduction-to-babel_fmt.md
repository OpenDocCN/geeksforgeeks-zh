...
> 原文：`https://www.geeksforgeeks.org/reactjs-introduction-to-babel/`
...
**注意：** 同样值得注意的是，编译器与 `transpiler` 完全不同，`transpiler` 将源代码转换为另一个相同抽象层次的源代码，而编译器一般将代码转换为较低层次的代码。像在 Java 中，源代码被转换成字节码，这是较低的级别，不等同。
