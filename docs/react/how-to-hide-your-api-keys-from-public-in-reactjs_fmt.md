...
> 原文: [https://www.geeksforgeeks.org/how-to-hide-your-api-keys-from-public-in-reactjs/](https://www.geeksforgeeks.org/how-to-hide-your-api-keys-from-public-in-reactjs/)
...
### 1. 创造 `.env` 文件
只需创建一个 `.env` 在你的 React 项目根目录下命名文件如下所示：
![](img/384b623a8a0f76d61fae6d92b6893d63.png)

### 2. 在 `.env` 文件中创建环境变量
使用前缀 `REACT_APP`，创建您自己的 `env` 变量并分配密钥，如下所示：
...
这样可以防止 Github 推 `.env` 文件到远程存储库。避免在公共存储库中推送应用编程接口密钥。
...
### 4. 访问 API 密钥
API 密钥可以使用 `process.env` 对象轻松访问。只需简单地将文件导入到您的主文件，并使用 `process.env.your_key_name` 访问它。
