...
**说明:** 组件包含标题、段落和状态变量 `subject`。该状态变量的值绑定到标题和段落元素，状态变量(即 `subject`)的任何变化都将反映在视图部分。
...
**解释**: 组件包含标题、段落、输入字段和状态变量 `subject`。这里我们使用 `onChange` 事件，当用户在输入字段中输入值时，变化反映在状态变量 `subject` 中，我们可以在视图中看到变化的值。