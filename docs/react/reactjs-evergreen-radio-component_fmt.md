...
> 原文: [https://www.geeksforgeeks.org/reactjs-evergreen-radio-component/](https://www.geeksforgeeks.org/reactjs-evergreen-radio-component/)
...
*   `disabled`: 当设置为真时，收音机禁用。
*   `checked`: 当设置为真时，检查收音机。
*   `size`: 用来表示无线电圈的大小。
*   `isRequired`: 当设置为真时，收音机获得必需的属性。
*   `isInvalid`: 当该属性设置为真时，咏叹调无效属性为真。
*   `appearance`: 用于复选框的外观。

**RadioGroup 道具:**

*   `options`: 用于传递电台组电台的选项。
*   `value`: 用于表示控制时选择的项目值。
*   `defaultValue`: 用于表示无线电组不受控制时的默认值。
*   `onChange`: 是状态改变时调用的回调函数。
*   `label`: 用于表示收音机的标签。
*   `size`: 用来表示无线电圈的大小。
*   `isRequired`: 当设置为真时，收音机获得必需的属性。
...
**参考:** [https://evergreen.segment.com/components/radio](https://evergreen.segment.com/components/radio)