...
React Suite 是一个流行的前端库，包含一组为中间平台和后端产品设计的 React 组件。`Input` 组件允许用户创建一个基本的小部件，用于获取用户输入的是一个文本字段。我们可以在 ReactJS 中使用以下方法来使用 React Suite 输入组件。
...
[https://rsuitejs.com/components/input/](https://rsuitejs.com/components/input/)