...
以下方法介绍了如何使用[React Native](https://www.geeksforgeeks.org/introduction-react-native/)中的按钮。为此，我们将使用按钮组件。它基本上是一个可点击的组件，用于在用户点击时执行一些操作。
...
*   `onPress`: 当用户点击按钮时调用。
*   `title`: 显示在按钮内部的文本。
*   `accessibilityLabel`: 显示失明可访问性特征的文本。
*   `color`: 用于设置按钮的颜色。
*   `disabled`: 如果为真，则用户无法按下按钮。
*   `hasTVPreferredFocus`: 电视首选对焦，仅适用于电视。
...
*   `touchSoundDisabled`: 如果为真，触摸时不播放系统声音。
...
现在让我们实现按钮。这里我们创建了一个按钮，当有人点击该按钮时，计数将会改变。
