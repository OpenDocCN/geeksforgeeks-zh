...
构造函数在反应方面没有什么不同。这可以将事件处理程序连接到组件和/或初始化组件的本地状态。在挂载组件之前，会启动`constructor()`，与 Reaction 中的大多数内容一样，它有几条规则，您可以在使用它们时遵循这些规则。
...
相反，我们将道具的值从`constructor()`传递给`Super()`函数：
...
## 步骤 2：不要在`constructor()`内调用 `setState()`
