...
*   变量、对象、数组、函数和类。
*   常见的数组方法，如 `.map()`，`.filter()`，`.reduce()`。
*   ES6 功能如箭头功能、`let`、`const`。了解 `this` 关键字、`call`、`apply` 和 `bind` 方法。
*   异步 JavaScript 的概念，如 `Promise`、回调、`async/await`。
...
*   **`import` 关键字：** 您必须在 React 项目中使用 `import` 关键字才能使用已安装的 Node 模块。
*   **`export` 关键字：** 创建模块/组件时使用这个关键字，只需要返回一部分，而不是所有的方法和变量。
...
*   学习 React 中的 `state` 概念？它是如何工作的？基本上，状态保存同步变量，状态值的改变将反映在整个应用程序中，无论它在哪里使用。了解如何在状态中存储和更新数据。
*   在 React 中，你将使用 `props`（嘿！到底是什么？）。我们知道这对你来说可能是个新词。基本上，`props` 就像传递给函数或方法的参数。知道什么时候用，怎么用。您将使用 `props` 作为 HTML 标签的输入参数。
*   函数组件、类组件，以及这两者之间的区别。
...
你会如何从“主页”页面重定向到“博客”页面？您需要在您的应用程序中设置路由来达到这个目的。React 开发者使用名为 `react-router-dom` 的第三方库为不同页面创建链接，加载不同页面、重定向到其他页面。

下面是 `react-router-dom` 的一些常见特性，您应该知道如何使用您的应用程序。

*   如何使用 `<Route>`、`<Switch>`、`<BrowserRouter>` 组件。
*   如何使用 `<Link>` 组件和 `useHistory()` 钩子进行导航。
*   使用 `path` 属性创建动态路由，即 `<Route path="/posts/:post-slug" component={Post} />` 并使用 `useParams()` 钩子获取路径值。
*   React Router 中 `<Redirect />` 的工作原理。

## 5. React Hooks

因此，您已经完成了基本的构建模块和 React 基础。现在是时候学习 React Hooks 了。这个新特性是在 React 版本中添加的。在 React Hooks 的帮助下，你可以不用类来编写 React 特性。React hooks 允许您编写更少的代码，这使得您的应用程序更加高效、简单和强大。

您将主要学习 React Hooks 的 5 个核心概念。这五个核心概念如下：

*   **`useState`：** 在你的 React 应用程序中，大部分时间你都会使用这个概念来存储和管理数据。
*   **`useEffect`：** 您将在像 HTTP 请求和使用浏览器 API 的操作中使用这个概念。
*   **`useRef`：** 这将用于引用 [JSX](https://www.geeksforgeeks.org/reactjs-introduction-jsx/) 元素。
*   **`useContext`：** React Hooks 的这个属性将被用来访问来自 React Context 的数据，以便在组件之间轻松共享数据（而不是传递 `props`）。
...
主要有两种方法来处理来自后端的数据。最常见和标准的方式是使用 **REST API**。另一个也是更新的是 **GraphQL API**。在您的工作中，您将会遇到必须同时使用这两个 API 的问题。了解如何使用 React 处理两种 API。

## 7. React 中的样式

在开发的大部分时间里，我们使用普通的 CSS 给我们的应用程序赋予样式。但是在 React 中，大多数开发人员使用组件库来简化样式。有很多组件库可以使用，但是 React 中最流行的是 **Material UI**。

您也可以使用 **实用类库** 来设置元素的样式。流行的实用类库是 **Tailwind CSS**。作为一名开发人员，你会遇到这两种情况。所以熟悉这两种类型的库是很好的。
...
我们讨论了在 React 中应该学习的主要概念。一旦你开始在某个 React 项目上工作，你将会遇到许多问题。根据问题的不同，你必须在中搜索高效的解决方案。
