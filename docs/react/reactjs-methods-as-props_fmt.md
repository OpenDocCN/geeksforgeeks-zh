...
为此，让我们创建一个名为 `ParentComponent.js` 的新组件，现在让我们在这个文件中创建一个类组件的基本布局。
...
现在让我们设置一个状态来问候我们的父母每当这个组件被渲染的时候，设置一个状态是不必要的，但是我们将这样做只是为了让一个应用程序更加动态。此外，让我们创建一个事件，每当父组件被呈现时发出警报。不要忘记绑定事件，这样`this`关键字就不会返回“`undefined`”。
...
现在我们来制作一个新的组件。我们称之为 `ChildComponent.js`，并制作一个简单的功能组件。让我们制作一个简单的按钮，然后将方法`greetParent()`作为属性。本质上，当按钮被点击时，它必须问候父级。
...
别忘了导入`ParentComponent`中的`ChildComponent`。因此`ParentComponent`的最终代码如下。
