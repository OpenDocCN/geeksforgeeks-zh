...
> 原文: [https://www.geeksforgeeks.org/reactjs-reactstrap-form-component/](https://www.geeksforgeeks.org/reactjs-reactstrap-form-component/)
...
*   `state`: 用于表示该部件的状态。
*   `valid`: 设置为真时，用于应用 is-valid 类。
*   `invalid`: 设置为真时，用于应用 is-invalid 类。
*   `label`: 用于传入自定义元素使用。
*   `innerRef`: 用于获取对 DOM 元素的引用。
*   `static`: 表示是否应用静态类。
*   `plaintext`: 用于表示是否应用明文类。
*   `addon`: 用于表示插件是否添加。
*   `className`: 用于表示造型的类名。
...
*   `children`: 用于将 children 元素传递给这个组件。
*   `row`: 设置为真时，用于应用行类。
*   `check`: 设置为真时，用于套用表单检查类。
*   `inline`: 设置为真时，用于应用 linline 类。
*   `disabled`: 当检查和禁用道具为真时应用禁用类，当为假时不做任何事情。
*   `label`: 用于传入自定义元素使用。
*   `className`: 用于表示造型的类名。
*   `cssModule`: 用来表示造型用的 CSS 模块。
...
**参考:** [https://reactstrap.github.io/components/form/](https://reactstrap.github.io/components/form/)