...
*   `children`：用于指示覆盖的子代属性。
*   `defaultShow`：用于指示覆盖的初始可见状态。
*   `delay`：用于指示覆盖被触发后显示和隐藏的延迟时间。
*   `flip`：用于指示覆盖的初始翻转状态。
*   `onHide`：用于指示覆盖图的非隐藏属性。
*   `onToggle`：它是一个回调函数，在用户更改工具提示的可见性时触发。
*   `overlay`：它是目标旁边的文本或元素覆盖。
*   `placement`：用于设置叠层的定位方向。
*   `popperConfig`：它是 `Popper.js` 的配置对象，传递给 Popper 的底层实例。
*   `show`：它表示覆盖的可见性。
*   `target`：它是需要为覆盖设置目标时使用的目标属性。
*   `trigger`：它用于指定哪些动作触发动作或覆盖可见性。

## 覆盖道具

*   `children`：用于指示覆盖的子代属性。
*   `container`：它基本上是一个组件实例或一个返回或 DOM 节点的函数。
*   `onEnter`：它是在覆盖过渡之前触发的回调函数。
*   `onEntered`：它是一个回调函数，在 `Overlay` 完成转换后触发。
*   `onEntering`：它是一个回调函数，在覆盖开始过渡时触发。
...
[`https://react-bootstrap.github.io/components/overlays/#overlay-trigger-props`](https://react-bootstrap.github.io/components/overlays/#overlay-trigger-props)