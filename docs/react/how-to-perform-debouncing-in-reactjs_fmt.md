...
> 原文: [https://www.geeksforgeeks.org/how-to-perform-debouncing-in-reactjs/](https://www.geeksforgeeks.org/how-to-perform-debouncing-in-reactjs/)
...
### 第 2 步
创建项目文件夹后，即`react-debouncing`，使用以下命令移动到它:
```jsx
cd react-debouncing
```
...
*   我们有一个 React `WidgetText` 组件，它基本上只有输入和文本区域，然后 `App` 组件本身有切换按钮。
*   有条件地，如果我们点击切换按钮，它将进入处理程序抓取，前一个状态看它显示 `bool` off 的状态，翻转它，所以如果它是真的，那么它将渲染 `WidgetText`，如果它是假的，我们不渲染 `WidgetText`。
