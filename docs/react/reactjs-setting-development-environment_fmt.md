...
`第一步`: 安装 `NodeJS`。您可以访问 `NodeJS` 的[官方下载链接](https://nodejs.org/en/download/current/)下载并安装最新版本的 `NodeJS`。一旦我们在个人电脑上设置了 `Node`，接下来我们需要做的就是设置 `React` 样板。

`第二步`: 为旧版本和最新版本设置 `React` 环境，根据你的 `Node` 版本跟随任何人。

### 对于包含 `Node < 8.10` 和 `npm < 5.6` 的旧版本:
设置 `React` 样板。我们将在全球范围内安装样板。在您的终端或命令提示符下运行以下命令来安装 `React` 样板。
...
现在，在成功安装样板文件之后，我们接下来要做的是创建我们的 `React` 应用程序。我们可以使用 `create-react-app` 命令来创建一个 `React` 应用程序。

### 对于包含 `Node >= 8.10` 和 `npm >= 5.6`:
为了使用提供良好开发体验的 `JavaScript` 最新特性，机器应该包含 `Node >= 8.10` 和 `npm >= 5.6` 的版本。
...
**现在，我们已经成功地建立了开发环境。最后剩下的就是启动开发服务器。**
