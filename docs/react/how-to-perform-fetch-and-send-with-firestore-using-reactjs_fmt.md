...
> 原文：[https://www.geeksforgeeks.org/how-to-perform-fetch-and-send-with-firestore-using-reactjs/](https://www.geeksforgeeks.org/how-to-perform-fetch-and-send-with-firestore-using-reactjs/)
...
我们使用 `create-react-app` 来创建我们的应用程序。
...
使用 `npm` 在项目中安装 `firebase` 包。

```jsx
npm install firebase@8.3.1 --save
```

## 步骤 3：创建 Firebase 项目并查看数据格式

通过填写必要的详细信息并检查存储在 Firestore 中的数据格式，从 Firebase 仪表板创建一个新项目。在本例中，我们有一个对象，其中 `name`、`age` 和 `courseEnrolled` 是存储数据的字段。

![](img/8525bba6fc651f6a0f0ac9910bbb3814.png)

## 步骤 4：初始化 Firebase

通过从 Firebase 仪表盘复制相关凭证，初始化项目中的 Firebase。
...
创建一个基本的用户界面，用于显示商店数据。我们将使用 `get()` 方法从商店获取数据。数据然后循环通过并在页面上显示。
