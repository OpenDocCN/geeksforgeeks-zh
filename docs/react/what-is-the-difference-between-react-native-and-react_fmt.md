...
### React 或 ReactJS 基本介绍:
是脸书为了更好的 UI 开发和高效的 DOM 操作而创建的开源 Javascript 库。`React` 有一个虚拟 `DOM` 的概念。当从服务器接收到任何数据时，该虚拟 `Dom` 已经相应地被修改，然后该更新的虚拟 `DOM` 通过某种算法与真实 `DOM` 匹配，并且只有真实 `DOM` 的不同于虚拟 `DOM` 的那些部分被更新。
