...
> 原文：[https://www.geeksforgeeks.org/how-to-create-an-avatar-generator-app-in-reactjs/](https://www.geeksforgeeks.org/how-to-create-an-avatar-generator-app-in-reactjs/)
...
现在，转到 `src` 文件夹，创建一个 `Components` 文件夹和一个 `Styles` 文件夹。在 `Components` 文件夹下，创建文件 `Avatar.jsx`，在 `Styles` 文件夹下，创建文件 `Avatar.css`。
...
这个文件包含了所有的逻辑。我们将使用一个名为 [DiceBear Avatars](https://avatars.dicebear.com/) 的免费开源 API（无需授权）根据几个参数获取随机头像。
...
在浏览器中打开 `http://localhost:3000/` URL。它会显示结果。我们的应用程序现在已经完成，现在应该可以工作了。
