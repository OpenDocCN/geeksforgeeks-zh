...
*   像 `atom`、`sublime text` 或 `Visual Studio Code` 这样的代码编辑器。
*   `node` 也应该安装在带有 `npm` 的机器上。

我们将使用 `node` 安装巴别塔。打开你的文本编辑器，然后创建如下所示的目录结构:
...
如果你知道 `node` 是如何工作的，那么你就知道 `node_modules`、`package.json` 和 `package-lock.json`。一旦我们运行一些命令，它们就会自动形成。
