...
> 原文：[https://www.geeksforgeeks.org/how-to-set-parent-state-from-children-component-in-reactjs/](https://www.geeksforgeeks.org/how-to-set-parent-state-from-children-component-in-reactjs/)

我们可以使用以下方法从 ReactJs 中的子组件设置父状态。

## 先决条件
[ReactJS 中的状态介绍](https://www.geeksforgeeks.org/reactjs-state-react/)

*   我们实际上将在父组件本身中设置父组件的状态，但是子组件将负责设置。
*   我们将在 `Parent` 组件中创建一个函数，用给定的输入设置状态。
*   我们将把这个函数作为一个道具（prop）传递给子组件。
*   然后，子组件将使用新值调用该函数。
*   我们将在函数中设置父组件的状态。
...
### 第四步
创建函数 `setStateOfParent` 在父组件中设置父组件的状态，同时在子组件中传递 `setStateOfParent` 函数。
...
### 第 5 步
现在，只要您想设置父项的状态，就可以访问并调用子项中的 `setStateOfParent` 功能。
