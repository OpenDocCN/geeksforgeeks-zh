...
> 原文：[https://www.geeksforgeeks.org/how-to-focus-on-the-next-field-input-in-reactjs/](https://www.geeksforgeeks.org/how-to-focus-on-the-next-field-input-in-reactjs/)
...
## 示例
我们在代码中可以看到，每当我们在输入字段中键入时，都会调用 `handleChange` 函数，它会根据当前输入字段中键入的字符数来决定下一个字段是否应该是焦点。如果它达到最大字符，那么它会找到下一个字段，并使其成为焦点。这是当前输入字段达到最大字符数限制时下一个输入字段的聚焦方式。
