...
> 哎哎哎: https://www.geeksforgeeks.org/react-js-static-getderivedstatefromprops/

当组件的状态依赖于道具的变化时，使用 `getDerivedStateFromProps()` 方法。

`getDerivedStateFromProps(props, state)` 是一个静态方法，在 React 的挂载和更新阶段都会在 `render()` 方法之前调用。它以更新的道具和当前状态作为参数。
