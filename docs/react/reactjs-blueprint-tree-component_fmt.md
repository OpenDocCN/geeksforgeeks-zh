...
> 原文: [https://www.geeksforgeeks.org/reactjs-blueprint-tree-component/](https://www.geeksforgeeks.org/reactjs-blueprint-tree-component/)

Blueprint 是一个基于 React 的 Web 用户界面工具包。该库非常适合构建桌面应用程序的复杂数据密集型界面，并且非常受欢迎。树组件为用户提供了一种显示树结构数据的方式。它用于显示分层数据。我们可以在 ReactJS 中使用以下方法来使用 ReactJS Blueprint 树组件。

## Tree Props

*   `className`: 用于表示传递给子元素的以空格分隔的类名列表。
*   `contents`: 用于表示指定树的内容和外观的数据。
...
*   `depth`: 用来表示节点的深度。
*   `disabled`: 表示该树节点是否非交互。
*   `hasCaret`: 用于指示是否显示展开/折叠节点的插入符号。
*   `icon`: 用于表示节点标签旁边要渲染的图标或图标元素的名称。
*   `id`: 用于表示节点的唯一标识符。
*   `isExpanded`: 继承自 `ITreeNode` 的 `isExpanded` 属性。
*   `isSelected`: 表示是否选择该节点。
*   `key`: 用于表示唯一的键属性。
*   `label`: 用于表示节点的主标签。
*   `nodeData`: 用于表示与节点关联的可选自定义用户对象。
*   `onClick`: 是点击 `TreeNode` 时触发的回调函数。
*   `onCollapse`: 这是一个回调函数，在树节点崩溃时触发。
...
*   `path`: 用于表示树节点的路径属性。
*   `secondaryLabel`: 用于表示显示在节点右侧的次要标签/组件。
...
### 示例 1
现在在 `App.js` 文件中写下以下代码。这里，我们演示了应用了图标元素的树组件。
