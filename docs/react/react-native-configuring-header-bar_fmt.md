...
> 原文: [https://www.geeksforgeeks.org/react-native-configuring-header-bar/](https://www.geeksforgeeks.org/react-native-configuring-header-bar/)

要配置 React Native 应用程序的标题栏，请使用导航选项。导航选项是屏幕组件的静态属性，它是一个对象或函数。
...
*   **步骤 4:** 使用以下命令安装所需的软件包:
    ...
    ```bash
    npm install --save react-navigation react-navigation-material-bottom-tabs react-native-paper react-native-vector-icons
    ```
...
在我们的示例中，我们将了解如何设置标题栏的样式，如何向其添加标题按钮/图标，并学习如何从一个屏幕动态发送数据并将其作为标题显示在另一个屏幕上。
...
注意导航选项。在这里，我们在标题栏中配置了一个标题按钮组件，它将我们带到设置屏幕。此外，请注意，当我们单击“转到用户屏幕”按钮时，我们会发送用户输入。
...
这里我们接收用户输入，我们通过主屏幕将它设置为输出标题栏中的标题。
