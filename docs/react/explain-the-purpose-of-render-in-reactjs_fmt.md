# 解释 ReactJS 中 render() 的目的

> 原文: [https://www.geeksforgeeks.org/explain-the-purpose-of-render-in-reactjs/](https://www.geeksforgeeks.org/explain-the-purpose-of-render-in-reactjs/)

React.js 库有两个组件:
*   类组件
*   功能组件

类组件使用 `render()` 函数。该函数的作用是：接受两个参数，一个是 HTML 代码，一个是 HTML 元素。
...
*   在 `render()` 方法中，我们不能更改 `state`，也不能导致副作用（例如向 web 服务器发出 HTTP 请求）。

让我们用一个例子来理解 `render()` 函数。
