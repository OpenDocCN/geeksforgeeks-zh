...
*   **受控**: `受控`属性用于使受控 `TransitionablePortal`。对于控件，它用于定义转换。
*   **bsPrefix**: 这个道具用来表示 CSS 中某个组件的变化。它用于自定义引导 CSS。默认值是微调器，类型是字符串。
*   **大小**: 道具的 `大小` 用来演示旋转器的大小。道具的 `大小` 用于组件大小的变化。尺码的类型是 `sm`。
*   **颜色**: `颜色` 道具用来表示旋转器的颜色。
*   **类名**: `类名` 道具用于表示 CSS 中组件样式的类名。
...
这是展示如何使用 `TransitionablePortal` 插件的基本示例。
