...
**速记片段：** 第一个代码的输出和上面的代码是一样的，但是使用的主要原因是，与内部带有 `div` 标签的代码相比，它要快一点，因为我们没有创建任何 DOM 节点。此外，它需要较少的内存。上述方法还有另一种简写，即我们用 `<>` 和 `</>` 代替 `<React.Fragment>`。

**注意：** 简写语法不接受 `key` 属性，在这种情况下你必须使用 `<React.Fragment>` 标记。
