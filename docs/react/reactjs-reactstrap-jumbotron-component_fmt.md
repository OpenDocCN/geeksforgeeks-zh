...
> 原文: [https://www.geeksforgeeks.org/reactjs-reactstrap-jumbotron-component/](https://www.geeksforgeeks.org/reactjs-reactstrap-jumbotron-component/)
...
*   `fluid`: Reactstrap `Jumbotron` 中的 `fluid` 属性用于获取屏幕的全宽。它采用默认值 `false`，类型为布尔值。
*   `as`: 属性用于查找自定义元素类型。取默认值为 `<div>`，类型为元素类型。
*   `className`: Reactstrap 中的 `className` 属性用于指定 CSS 属性。它将默认值作为字符串。
...
**参考:** [https://reactstrap.github.io/components/jumbotron](https://reactstrap.github.io/components/jumbotron)