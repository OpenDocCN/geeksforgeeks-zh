...
在 React 中，我们可以对 JSX 使用正常的 JavaScript 表达式。要将任何 JavaScript 表达式嵌入到一段在 JSX 编写的代码中，我们必须将该表达式用大括号`{}`括起来。考虑以下写在`index`文件中的程序：
...
JSX 允许我们使用 HTML 元素的属性，就像我们使用普通的 HTML 一样。但是，JSX 没有使用 HTML 的常规命名约定，而是使用 camelcase 约定来表示属性。比如 HTML 中的`class`变成了 JSX 的`className`。这背后的主要原因是 HTML 中的一些属性名，比如“class”，是 JavaScripts 中的保留关键字。所以，为了避免这个问题，JSX 对属性使用了 camel case 命名约定。我们也可以在 JSX 使用自定义属性。对于自定义属性，此类属性的名称应以`data-`为前缀。在下面的例子中，我们为`<h2>`标签使用了一个名为`data-sampleAttribute`的自定义属性。
