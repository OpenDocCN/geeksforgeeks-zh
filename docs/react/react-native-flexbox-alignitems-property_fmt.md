...
*   `stretch`: 是 `alignItems` 的默认值。在这种情况下，儿童组件被拉伸以适应容器的副轴的高度。
*   `center`: 子组件的对齐应在容器副轴的中心。
*   `flex-start`: 子组件将与容器副轴的开始对齐。
*   `flex-end`: 子组件将与容器副轴的末端对齐。
*   `baseline`: 儿童组件将与容器的基线对齐。
...
在此示例中，`flex-direction` 设置为 `row`，`alignItems` 属性值为 `stretch`。当我们为直线设置值 `stretch` 时，我们没有给我们的次轴标注尺寸。因此，在下面的代码中，我们没有给出项目的高度。
...
在本例中，整个代码将保持不变，我们只需将 `alignItems` 属性的值更改为 `center`，并为项目提供高度。
...
**参考:** [https://reactnative.dev/docs/flexbox#align-items](https://reactnative.dev/docs/flexbox#align-items)