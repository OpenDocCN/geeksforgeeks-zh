...
*   `addAfter`: 用于表示输入字段后显示的标签文本。
*   `addBefore`: 用于表示输入字段前显示的标签文本。
*   `allowClear`: 如果设置为 `true`，则用于移除带有清晰图标的输入内容。
*   `bordered`: 表示是否有边框样式。
*   `defaultValue`: 用于表示初始输入内容。
*   `disabled`: 用于指示输入是否禁用。
*   `id`: 用于表示输入的 `id`。
*   `maxLength`: 用于表示最大长度。
*   `prefix`: 用于表示输入的前缀图标。
...
*   `onPressEnter`: 是按回车键时触发的回拨功能。

## 输入。文本区道具

*   `allowClear`: 如果设置为 `true`，则用于移除带有清晰图标的输入内容。
*   `autoSize`: 用于表示高度自动调整功能。
*   `bordered`: 表示是否有边框样式。
*   `defaultValue`: 用于表示初始输入内容。
*   `maxLength`: 用于表示最大长度。
...
*   `enterButton`: 表示输入后是否显示回车按钮。
*   `loading`: 表示是否显示加载效果的搜索框。
...
*   `iconRender`: 用于自定义切换按钮。
*   `visibilityToggle`: 用于指示是否显示切换按钮。
