...
这是一个使用 `Alert` 方法来显示警报对话框的应用编程接口。该对话框可以有三个按钮，分别是正、负和中性，用于执行不同的操作。
...
*   `prompt()`: 此方法用于创建并显示提示，以提醒的形式输入一些文本。

### 报警方式参数:

*   `title`: 这是显示为对话框标题的必需参数。
*   `message`: 是显示对话框消息的可选参数。
*   `buttons`: 也是一个可选参数，显示对话框上的按钮。
*   `options`: 也是可选参数，只有安卓才有。

### 提示方式参数:

*   `title`: 这是显示为对话框标题的必需参数。
*   `message`: 是显示对话框消息的可选参数。
*   `callbackOrBackButton`: 该参数可以作为函数或按钮传递，如果作为函数传递，当用户同意该条件时，将使用提示值调用该参数。如果它作为按钮传递，那么它将基于数组内容进行配置。
*   `type`: 该参数配置文本输入。
*   `defaultValue`: 该参数是定义文本输入中默认文本的字符串。
*   `keyboardType`: 此参数是一个字符串，用于定义第一个文本字段的键盘类型(如果存在)。
...
现在让我们实现警报功能。这里我们创建了一个按钮，当有人点击该按钮时，会弹出一个警告。
