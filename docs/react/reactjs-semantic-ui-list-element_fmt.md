...
*   `bulleted`: 使用此属性，列表项可以标记为带有项目符号。
*   `ordered`: 列表可以按照的顺序进行数字排序。
*   `link`: 列表可以针对导航链接进行特殊格式化。
...
在本例中，我们将使用基本列表元素，通过使用 ReactJS 语义 UI `List` 元素来显示一组相关内容。
...
在本例中，我们将使用列表元素，通过使用 ReactJS 语义 UI `List` 元素，用图标显示一组相关内容。
...
**参考:** [https://react.semantic-ui.com/elements/list](https://react.semantic-ui.com/elements/list)