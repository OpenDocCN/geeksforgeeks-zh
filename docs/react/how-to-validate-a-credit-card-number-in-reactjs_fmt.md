...
> 原文: [https://www.geeksforgeeks.org/how-to-validate-a-credit-card-number-in-reactjs/](https://www.geeksforgeeks.org/how-to-validate-a-credit-card-number-in-reactjs/)
...
**步骤 2:** 创建项目文件夹（即 `creditcardvalidatedemo`）后，使用以下命令移动到该文件夹:

```jsx
cd creditcardvalidatedemo
```

**步骤 3:** 创建 ReactJS 应用程序后，使用以下命令安装 `validator` 模块:
...
现在在 `App.js` 文件中写下以下代码。在这里，`App` 是我们的默认组件，我们已经编写了代码来使用基本的用户界面验证信用卡号。
