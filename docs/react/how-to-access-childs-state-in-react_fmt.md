...
> 原文: [https://www.geeksforgeeks.org/how-to-access-childs-state-in-react/](https://www.geeksforgeeks.org/how-to-access-childs-state-in-react/)

在 React 中，我们可以使用 Refs 访问孩子的状态。我们将为父组件中的子组件指定一个参照。然后使用 Refs 我们可以访问孩子的状态。

## 创建参照

参照是使用 `React.createRef()` 创建的，并通过 `ref` 属性附着到 React 元素。
...
当我们在渲染中将引用分配给元素或子组件时，我们可以使用引用的 `current` 属性来访问该元素。
