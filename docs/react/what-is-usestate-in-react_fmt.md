...
要使用 `useState`，需要从 `react` 导入 `useState`，如下所示:
...
首先使用随机函数用随机数初始化计数变量 `count`，并用 `setRandomCount` 更新计数的状态。每次我们点击 `onClick` 按钮，它都会调用 `clickHandler` 函数，该函数会用随机数再次设置计数变量。