...
> 原文：[https://www.geeksforgeeks.org/how-to-install-reactjs-on-windows/](https://www.geeksforgeeks.org/how-to-install-reactjs-on-windows/)
...
2.  **调试**：React app 可以使用 `React开发者工具` 轻松调试。这是一个浏览器扩展，可以用于 `Chrome` 和 `Firefox`。
...
**第 6 步**：现在打开你选择的 `IDE`，比如 `Visual Studio Code`，打开你已经安装了 `react app` 的文件夹 `newfolder`（在上面的例子中）。在文件夹里面你会看到你的 `app` 的名字 `reactfirst`（在我们的例子中）。使用终端并进入你的应用程序名称文件夹。使用命令 `cd reactfirst`（您的应用名称）。
