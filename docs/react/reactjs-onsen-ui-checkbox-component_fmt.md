...
ReactJS Onsen-UI 是一个受欢迎的前端库，具有一组 React 组件，旨在以一种美观高效的方式开发 HTML5 混合和移动网络应用程序。`Checkbox` 组件允许用户从给定选项中进行二进制选择。我们可以在 ReactJS 中使用以下方法来使用 Onsen-UI 复选框组件。

## 复选框道具

*   `modifier`：用于复选框的外观。
*   `disabled`：用于指定复选框是否禁用。
*   `onChange`：是复选框状态变化时触发的回调函数。
*   `value`：用于表示复选框的值。
*   `checked`：用于控制复选框（受控）的状态。
*   `defaultChecked`：用于定义未控制输入首次渲染时单选按钮的状态。
*   `inputId`：用于指定内部 `<input>` 元素的 `id` 属性。

## 预设修改器

*   `material`：用于显示材质设计复选框。
*   `no-border`：用于 iOS 无边框复选框。
...
![输出](img/6be7ddc2277376f90792088b9002ef2d.png)

### 参考
[https://onsen.io/v2/api/react/Checkbox.html](https://onsen.io/v2/api/react/Checkbox.html)