...
> 原文：[https://www.geeksforgeeks.org/reactjs-ui-ant-design-datepicker-component/](https://www.geeksforgeeks.org/reactjs-ui-ant-design-datepicker-component/)
...
*   `autoFocus`：如果设置为真，则用于在安装组件时获取焦点。
*   `bordered`：表示是否有边框样式。
*   `className`：用于传递拾取器的类名。
*   `dateRender`：用作日期单元格的自定义渲染函数。
*   `disabled`：用于确定日期选择器是否禁用。
...
*   `defaultPickerValue`：用于设置默认提货日期。
*   `defaultValue`：用于设置默认日期。
*   `disabled`：表示是否禁用开始或结束。
*   `disabledTime`：用于指定不能选择的时间。
*   `format`：用于设置日期格式。
*   `ranges`：用于快速选择的预设范围。
*   `renderExtraFooter`：用于在面板中渲染额外的页脚。
*   `separator`：用于设置输入之间的分隔符。
