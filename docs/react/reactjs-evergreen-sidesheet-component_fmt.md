# React Evergreen SideSheet 组件

> 原文: [https://www.geeksforgeeks.org/reactjs-evergreen-sidesheet-component/](https://www.geeksforgeeks.org/reactjs-evergreen-sidesheet-component/)
...
*   `shouldCloseOnOverlayClick`: 用于指示叠加是否应在点击时关闭叠加。
*   `shouldCloseOnEscapePress`: 用于指示按下 ESC 键是否应关闭覆盖。
*   `width`: 用于定义侧页的宽度。
*   `containerProps`: 用于定义通过侧页容器窗格的属性。
*   `position`: 用于定位板材。
*   `preventBodyScroll`: 表示是否防止外体滚动。
...
*   **步骤 3:** 创建 ReactJS 应用程序后，使用以下命令安装所需的 `evergreen-ui` 模块:
...
[https://evergreen.segment.com/components/side-sheet](https://evergreen.segment.com/components/side-sheet)