...
> 原文：[https://www.geeksforgeeks.org/how-to-specify-a-port-to-run-a-create-react-app-based-project/](https://www.geeksforgeeks.org/how-to-specify-a-port-to-run-a-create-react-app-based-project/)

当我们使用`npx create-react-app`命令创建一个新的 react app 时，该 app 的默认端口为 3000。我们可以从`localhost:3000`访问该应用。
...
有几种方法可以改变 react app 的默认端口。在本教程中，我们将逐一介绍每种方法。
...
这是更改 react app 默认端口的最简单方法。我们需要在项目目录内创建`.env`文件并添加环境变量。用户需要在`.env`文件内添加以下代码。
...
在这个方法中，我们必须在`package.json`文件中编辑一行代码。在这里，用户会在“scripts”对象中找到类似`"start": "react-scripts start"`的代码。在下图中，可以看到“scripts”对象的默认视图。

![](img/4c87acadd40f10e789bcb9fc2c1835b7.png)

用户需要编辑“scripts”对象的第一行，他们必须在那里添加下面的代码。
...
编辑`package.json`文件后，您的“scripts”对象应该如下图所示。

![](img/6792a86de2a7209a59e92c8fdce5b3a5.png)

### 方法 3: 安装并添加 cross-env 包
...
安装完`cross-env`包后，用户需要编辑`package.json`文件内“scripts”对象的第一行。用户需要通过删除“scripts”对象中的第一行来更改下面的代码。
...
在代码内部进行更改后，您的“scripts”对象应该如下图所示。

![](img/89628abe994b880d4fa79ceac5eb8e3a.png)

### 方法 4: 用运行命令指定端口

在这种方法中，我们不需要在 react 应用程序中编辑任何文件。我们只需要提到带有 react 项目的 run 命令的端口。用户必须使用下面的命令来运行项目，而不是`npm start`。
