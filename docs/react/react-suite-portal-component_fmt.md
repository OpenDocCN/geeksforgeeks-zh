...
React Suite 是一个流行的前端库，包含一组为中间平台和后端产品设计的 React 组件。`Portal` 组件将其子组件渲染到当前 DOM 层次结构外的新*子树*中。我们可以在 ReactJS 中使用以下方法来使用 React Suite `Portal` 组件。

**门户道具:**

*   `children`：用来表示子成分。
*   `container`：用于表示渲染子组件容器。
...
**参考：** [https://rsuitejs.com/components/portal/](https://rsuitejs.com/components/portal/)