...
> 原文: [https://www.geeksforgeeks.org/how-to-update-the-state-of-a-component-in-reactjs/](https://www.geeksforgeeks.org/how-to-update-the-state-of-a-component-in-reactjs/)
...
## 1. 更新基于类的组件的状态
现在我们将学习如何更新基于类的组件的状态。下面讨论这些步骤。

*   进入 `App.js` 文件，清除所有内容。
*   在 `App.js` 文件导入的顶部，从 `'React'` 中选择 `{Component}`。
*   创建一个名为 `App` 的基于类的组件。这是我们重建的默认应用组件。
...
*   在 `App.js` 文件导入的顶部，从 `'react'` 中导入 `{useState}`。
*   在应用程序组件中，使用以下语法创建一个名为 `text` 的状态。这是内置的 `useState` 方法，用于对功能组件进行反应。：
...
*   使用 `setState` 方法更新特定事件(如按钮点击)的状态。语法如下:
