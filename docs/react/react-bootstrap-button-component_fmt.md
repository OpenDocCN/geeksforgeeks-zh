...
Reaction-Bootstrap 是一个在设计时考虑到 Reaction 的前端框架。 Bootstrap 是为 Reaction 重新构建和修改的，因此被称为 Reaction-Bootstrap。 按钮用于在网站上执行操作，它们在前端部分起着至关重要的作用。
...
现在使用该文件中的以下代码片段并导出该组件，以便我们可以在 index.js 中导入以进行呈现。
