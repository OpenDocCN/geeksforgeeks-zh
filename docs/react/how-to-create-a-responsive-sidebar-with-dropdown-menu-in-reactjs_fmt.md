...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-responsive-sidebar-with-dropdown-menu-in-reactjs/](https://www.geeksforgeeks.org/how-to-create-a-responsive-sidebar-with-dropdown-menu-in-reactjs/)
...
现在，通过在终端中键入给定的命令，转到您的 `react-sidebar-dropdown` 文件夹：
...
现在在 `src` 中创建 `components` 文件夹，然后转到组件文件夹，创建三个文件 `Sidebar.js`、`SidebarData.js` 和 `SubMenu.js`。
...
打开&关闭侧边栏视图，这就是 `useState()` hook 发挥作用的地方。

我们创建一个状态，第一个元素 `sidebar` 作为初始状态，值为 `false`，第二个元素作为函数 `setSidebar()` 来更新状态。然后创建一个名为 `showSidebar()` 的函数，每当调用该函数时，它都会将 `sidebar` 的值设置为与其当前值相反。
...
由于每个侧边链接都没有下拉菜单，它首先检查一个侧边链接是否有一个 `subNav` 属性，该属性是从我们在 `SidebarData.js` 文件中创建的对象数组中用它定义的。如果该属性存在，那么它执行上述逻辑，否则侧链接正常出现，没有任何打开和关闭图标。
...
现在我们完成了组件文件夹，现在我们操作页面。在 `src/pages` 中编辑项目侧边栏的各种页面：

## 文件名 - AboutUs.js

# React组件文件结构

## AboutUs, OurAim, OurVision 组件
