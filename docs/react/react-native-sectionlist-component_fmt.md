...
> 原文: [https://www.geeksforgeeks.org/react-native-sectionlist-component/](https://www.geeksforgeeks.org/react-native-sectionlist-component/)
...
*   `onEndReachedThreshold`: 它是一个值，确切地告诉列表的底部应该离屏幕末端有多远，以便触发 `onEndReached`。
*   `refreshControl`: 如果给定，刷新控制将被插入“拉至刷新”功能。
*   `onViewableItemsChanged`: 它是在行可视性发生变化时调用的函数。
*   `refreshing`: 等待新数据刷新屏幕时设置为真。
*   `renderSectionFooter`: 在每个部分的底部渲染。
*   `renderSectionHeader`: 在每个部分的顶部呈现。
*   `SectionSeparatorComponent`: 渲染在每个部分的顶部和底部，以区分它们。
...
在这里，我们将使用 Expo CLI 版本，这将更加流畅地运行您的 React Native 应用程序。按照以下步骤逐一设置您的 React 原生环境。
...
*   **步骤 3:** 现在进入你的项目文件夹，即 `sectionlist-demo`
...
在本例中，我们将看到如何使用基本的 `SectionList` 组件。
...
[https://reactnative.dev/docs/sectionlist](https://reactnative.dev/docs/sectionlist)