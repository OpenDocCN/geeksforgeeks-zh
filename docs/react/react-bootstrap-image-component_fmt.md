...
Reaction-Bootstrap 是一个前端框架，其设计考虑到了 Reaction。 `Image` 组件提供了一种方法，可以在此 `Image` 组件的帮助下将图像放入我们的应用程序中。 我们可以在 ReactJS 中使用以下方法来使用 Reaction-Bootstrap `Image` 组件。

**图像道具：**

*   `fluid`：它提供了一种将我们的图像设置为流体图像的方法。
*   `rounded`：它提供了一种将图像设置为圆形的方法。
*   `roundedCircle`：它提供了一种将图像设置为圆形的方法。
*   `thumbnail`：它提供了一种将图像设置为缩略图形状的方法。
