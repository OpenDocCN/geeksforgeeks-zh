# 如何在React Native中的标签导航底部添加图标？

> 原文：[https://www.geeksforgeeks.org/how-to-add-icons-at-the-bottom-of-tab-navigation-in-react-native/](https://www.geeksforgeeks.org/how-to-add-icons-at-the-bottom-of-tab-navigation-in-react-native/)

在“React Native”中的选项卡导航底部添加图标是一项相当简单的任务。在本文中，我们将实现一个基本的应用程序来学习在选项卡导航中使用图标。为此，我们首先需要设置应用程序并安装一些软件包。

## 实现

现在让我们从实现开始：

1.  打开终端，通过以下命令安装`expo-cli`。
    ```bash
...
2.  现在通过以下命令创建一个项目。
    ...
    ```
3.  现在进入你的项目文件夹，即`tab-navigation-icons`。
    ```bash
...
现在，让我们设置选项卡导航器并添加图标，以及一些基本的CSS样式，以使图标看起来像样。在我们的演示应用程序中将有3个屏幕，即主屏幕、用户屏幕和设置屏幕。因此，我们将有3个标签在这3个屏幕之间导航，我们的主屏幕是默认屏幕。
