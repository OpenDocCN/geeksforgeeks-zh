...
> 原文: [https://www.geeksforgeeks.org/reactjs-reactstrap-carousel-component/](https://www.geeksforgeeks.org/reactjs-reactstrap-carousel-component/)
...
*   `previous`: 是点击上一个按钮时触发的回调函数。
*   `keyboard`: 用于指示转盘是否应对键盘事件做出反应。
*   `pause`: 用于根据不同的鼠标事件暂停幻灯片。
*   `ride`: 用于用户手动循环第一个项目后自动播放转盘。
*   `interval`: 用于延迟这些物品自动循环运动之间的时间。
*   `children`: 用于将 children 元素传递给这个组件。
*   `mouseEnter`: 是鼠标进入转盘时触发的回调函数。
*   `mouseLeave`: 是鼠标退出转盘时触发的回调函数。
*   `slide`: 用于启用幻灯片之间的动画。
*   `cssModule`: 用来表示造型用的 CSS 模块。
*   `enableTouch`: 用于指示转盘上的触摸手势是否有效。
...
*   `direction`: 用来表示像下一个或上一个这样的方向。
*   `onClickHandler`: 是一个回调函数，点击时触发。
*   `cssModule`: 用来表示造型用的 CSS 模块。
*   `directionText`: 用于表示方向文字。
...
*   `captionHeader`: 用于表示字幕标题值。
*   `captionText`: 用于表示字幕文本值。
*   `cssModule`: 用来表示造型用的 CSS 模块。

## 不受控制的转盘道具

*   `items`: 用于表示项数组。
*   `indicators`: 用于显示一组滑块位置指示器。
*   `controls`: 表示是否有控件。
*   `autoPlay`: 表示是否可以自动播放。
...
现在在 `App.js` 文件中写下以下代码。这里，我们使用了带有转盘控制按钮的转盘组件。
...
现在在 `App.js` 文件中写下以下代码。这里，我们使用了没有转盘控制按钮的转盘组件。
