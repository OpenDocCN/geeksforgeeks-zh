...
> 原文：[https://www.geeksforgeeks.org/how-to-validate-a-date-in-reactjs/](https://www.geeksforgeeks.org/how-to-validate-a-date-in-reactjs/)
...
### 步骤 2
创建项目文件夹后，即 `datevalidatedemo`，使用以下命令移动到该文件夹：
```jsx
cd datevalidatedemo
```

### 步骤 3
创建 ReactJS 应用程序后，使用以下命令安装 `validator` 模块：
```jsx
...
### `App.js`
现在在 `App.js` 文件中写下以下代码。在这里，`App` 是我们的默认组件，我们已经编写了代码来用基本的用户界面验证日期。
