...
> 原文：[https://www.geeksforgeeks.org/how-reactjs-es6-syntax-is-different-compared-to-es5/](https://www.geeksforgeeks.org/how-reactjs-es6-syntax-is-different-compared-to-es5/)
...
*   这里允许的数据类型有 `number`、`string`、`null`、`boolean`、`undefined` 和 `symbol`。
*   ES5 使用 `require()` 模块导入我们脚本文件中的任何成员模块。
...
*   使用 `module.exports` 关键字将任何组件导出为模块。
...
*   ES5 使用 `function` 关键字和 `return` 关键字来定义函数。
...
*   ES5 使用 `createReactClass()` 或 `React.createClass()` API 创建 react 组件类，并使用 `getInitialState()` 方法定义 react 组件状态。
*   对象操作的处理速度很慢。
...
![项目结构](img/e3dd5555048e70b07ee15f52e1c03e9f.png)
...
[视频链接](https://media.geeksforgeeks.org/wp-content/uploads/20210930204713/es.mp4)

## ES6

*   ES6 的完整形式是 ECMA 剧本 6，于 2015 年出版。
*   这里允许的数据类型有 `number`、`string`、`null`、`boolean`、`undefined` 和 `symbol`。
*   ES6 使用 `import` 模块来导入我们脚本文件中的任何成员模块。
...
*   ES6 使用 `export default` 关键字将任何组件导出为模块。
...
*   在 ES6 中，我们不需要使用 `function` 关键字来定义函数。在 ES6 中使用 Arrow 函数使其更加紧凑。这是一个用 `=>` 语法描述的函数。
...
*   ES6 使用扩展 `React.Component` 的 ES6 类，并使用构造函数中的 `this.state` 定义一个 react 组件状态。
...
| 5. | ES5 uses the `require()` module to contain react modules or components. | ES6 uses `import` modules to contain react modules or components. |
...
| 11. | ES5 performance lacks new features, so it is relatively low. | With the addition of advanced functions and code optimization, ES6 provides high performance. |
| 12. | ES5 has huge community support because it has been used for a long time. | ES6 has less community support than ES5 because it is the latest update of ES5. |