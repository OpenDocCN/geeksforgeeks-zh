...
> 原文: [https://www.geeksforgeeks.org/reactjs-onsen-ui-button-component/](https://www.geeksforgeeks.org/reactjs-onsen-ui-button-component/)
...
*   `modifier`: 用于按钮的外观。
*   `disabled`: 用于指定按钮是否禁用。
*   `ripple`: 用于指定按钮是否有波纹效果。
*   `onClick`: 是点击按钮时触发的回调函数。

## 预设修改器

*   `outline`: 用于显示具有轮廓和透明背景的按钮。
*   `light`: 用于显示不突出的按钮。
*   `quiet`: 用于显示没有轮廓和/或背景的按钮。
*   `cta`: 用来展示一个真正突出的按钮。
*   `large`: 用于显示覆盖屏幕宽度的按钮。
*   `large--quiet`: 用于显示大静音按钮。
*   `large--cta`: 用于显示大的行动号召按钮。
*   `material`: 用于显示材质设计按钮。
*   `material--flat`: 用于显示材质设计扁平按钮。
...
**参考:** [https://onsen.io/v2/api/react/Button.html](https://onsen.io/v2/api/react/Button.html)