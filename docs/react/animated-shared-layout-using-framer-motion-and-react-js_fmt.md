...
> 原文: [https://www.geeksforgeeks.org/animated-shared-layout-using-framer-motion-and-react-js/](https://www.geeksforgeeks.org/animated-shared-layout-using-framer-motion-and-react-js/)
...
*   我们将使用 react `useState` hook 和 `framer-motion` 组件 `motion` 和 `AnimatePresence` 创建一个 `Item` 组件，即动画布局。
*   `Content` 组件用于使用 HTML `img` 标签和 `div` & 成帧器-运动组件 `motion` 创建项目的(动画共享布局)内容。
*   `toggleOpen` 是一个实用函数，用于设置“isOpen”值，而不是(`!`)的最后一个值。
*   在 `App.js` 中，`itemList` 代表我们想要创建的动画共享布局的数量，在我们的例子中是 3。
*   在 `App.js` 中，我们将使用 `framer-motion` 的 `AnimateSharedLayout` 组件来包装导入的 `Item` 组件，并通过“itemList”数组进行映射来渲染动画布局。
