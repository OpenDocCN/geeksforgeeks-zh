...
Ant Design 库预建了这个组件，也很容易集成。图标组件非常有用，因为图标是每个用户界面应用程序所必需的。我们可以在 ReactJS 中使用以下方法来使用 Ant Design 图标组件。
...
*   `rotate`: 用于将图标旋转 n 度。
*   `spin`: 用于随动画旋转图标。
*   `style`: 用于将样式属性传递给大小、边框等图标。
*   `twoToneColor`: 用于双色图标。

## 自定义图标属性

*   `component`: 用于表示用于根节点的组件。
*   `rotate`: 用于将图标旋转 n 度。
*   `spin`: 用于随动画旋转图标。
*   `style`: 用于将样式属性传递给大小、边框等图标。
...
*   `scriptUrl`: 用于表示 `iconfont.cn` 项目生成的网址。

## 自定义 SVG 图标属性

*   `className`: 用于传递这个元素的类名。
*   `fill`: 用于定义 SVG 图标中填充的颜色。
*   `height`: 用于设置 SVG 图标的高度。
*   `style`: 用于将大小、边框等样式属性传递给 SVG 图标。
*   `width`: 用于设置 SVG 图标的宽度。
