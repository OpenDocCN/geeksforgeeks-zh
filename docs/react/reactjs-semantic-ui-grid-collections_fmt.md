...
*   **分割:** `Grid` 可以在它的列之间有分割器来将它们彼此分开。
*   **垂直分割:** `Grid` 也可以垂直使用行与行之间的垂直分割器进行分割。
*   **Celled:** `Grid` 也可以包含的行被划分为细胞。
*   **内部划分:** `Grid` 只能在内部行之间有行划分。
...
在本例中，我们将使用 ReactJS 语义 UI Grid 集合，使用网格和图标元素来显示基本网格。
...
在本例中，我们将使用网格和图标元素，但我们使用的是 Importing `_` from lodash。
...
**参考:** [https://react.semantic-ui.com/collections/grid](https://react.semantic-ui.com/collections/grid)