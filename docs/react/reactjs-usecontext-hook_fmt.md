...
上下文应用编程接口使用 `Context.Provider` 和 `Context.Consumer` 组件传递数据，但是编写长函数代码来使用这个上下文应用编程接口是非常麻烦的。因此，`useContext` 钩子有助于使代码更易读、不那么冗长，并且不需要引入消费者组件。`useContext` 钩子是 React 16.8 中新增的。
...
演示 `useContext` Hook 使用的程序。在这个例子中，我们有一个按钮，只要我们点击这个按钮，`onClick` 处理程序就会被触发，它会在 `useContext` 钩子的帮助下改变身份验证状态（默认值为 `Nopes`）。让我们看看上面代码的输出：
