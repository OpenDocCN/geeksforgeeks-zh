...
Reaction-Bootstrap 是一个前端框架，其设计考虑到了 Reaction。`Fade`组件提供了一种向子组件或元素添加淡入淡出动画的方法。`Collapse`组件提供了一种将折叠切换动画添加到组件或元素的方法。我们可以在 ReactJS 中使用以下方法来使用 Reaction-Bootstrap 淡入淡出和折叠组件。

## 淡出道具

*   `show`：当组件安装时，它用于运行淡入动画。
*   `in`：用于显示组件。
*   `mountOnEnter`：它用于在组件等待时装载组件，直到触发第一个`enter`转换。
*   `onEnter`：它是在组件淡入之前触发的回调函数。
*   `onEntered`：它是在组件淡入后触发的回调函数。
*   `onEnching`：它是组件开始淡入后触发的回调函数。
*   `onExit`：它是在组件淡出之前触发的回调函数。
*   `onExited`：它是在组件淡出后触发的回调函数。
*   `onExiting`：它是组件开始淡出后触发的回调函数。
*   `timeout`：表示淡入淡出动画的时长，单位为毫秒。
*   `unmountOnExit`：用于卸载组件。

## 折叠道具

*   `show`：当组件安装时，它用于运行展开动画。
*   `children`：用于定义该组件的子元素。
*   `className`：用于为该组件的样式添加类名。
*   `dimension`：表示高度和宽度形式的尺寸。
...
*   `role`：定义可折叠元素的 ARIA 角色。
*   `timeout`：表示折叠动画的时长，单位为毫秒。
*   `unmountOnExit`：用于卸载组件。

## 创建 Reaction 应用程序并安装模块

### 步骤 1
使用以下命令创建 Reaction 应用程序：
```jsx
