...
> 原文：[https://www.geeksforgeeks.org/how-to-check-the-version-of-reactjs/](https://www.geeksforgeeks.org/how-to-check-the-version-of-reactjs/)
...
2.  使用命令行
3.  使用从 `React` 默认导入的 `version` 属性

## 使用 `package.json` 文件

[`package.json`](https://www.geeksforgeeks.org/node-js-package-json/) 包含关于我们项目的元数据。默认情况下，它是在我们创建 React 项目时创建的。我们可以使用下面提到的命令创建一个 react 应用程序。
...
## 使用从 `React` 默认导入的 `version` 属性

从 `react` 库中默认导入的是一个具有 `version` 属性的对象。我们可以用我们想要的方式在 JSX 元素中使用这个属性。
...
`app.js` 文件的内容在下面给出的代码中有所提及，我们在其中演示了版本属性在 `React` 对象上的使用。
