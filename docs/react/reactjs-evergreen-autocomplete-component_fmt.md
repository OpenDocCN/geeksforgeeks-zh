...
React Evergreen 是一个受欢迎的前端库，它有一组 React 组件来构建漂亮的产品，因为这个库是灵活的、合理的默认值和用户友好的。自动完成组件用于自动完成带有选项值的自由文本值。基本上允许用户从选项列表中键入和选择项目。我们可以在 ReactJS 中使用以下方法来使用长青自动完成组件。

## 自动完成道具

*   `title`：用于为项目提供标题。
*   `items`：用于表示作为选择选项的项目数组。
*   `selectedItem`：用于表示自动完成时要显示的选定项目。
*   `itemToString`：它是一个在每个项目上使用的函数，用于在项目数组不是字符串数组的情况下返回将在过滤器上显示的字符串。
*   `children`：是用来渲染‘滤镜’组件的函数。
*   `itemSize`：用于表示列表中每个项目的高度。
*   `renderItem`：它是一个返回组件来渲染项目的函数。
*   `position`：用于波波头的位置。
*   `itemFilter`：用于过滤项目的功能。
*   `isFilterDisabled`：用于启用和禁用过滤。
*   `popoverMinWidth`：用于表示结果容器的最小高度。
*   `popoverMaxHeight`：用于表示结果容器的最大高度。
*   `allowOtherValues`：用于表示输入是否接受超出提供项目的任意用户输入。

## 自动复合命题

*   `children`：用于传递该组件的子元素。
*   `style`：用于传递组件的样式。
*   `isSelected`：表示项目是否被选择。
*   `isHighlighted`：用于表示是否高亮显示该项目。
...
[https://evergreen.segment.com/components/autocomplete/props](https://evergreen.segment.com/components/autocomplete/props)