...
React Suite 是一个流行的前端库，包含一组为中间平台和后端产品设计的 React 组件。标签组件允许用户提供标签进行分类或标记。我们可以在 ReactJS 中使用以下方法来使用 React Suite 标记组件。

## 标签道具

*   `children`：用于表示组件的 children。
*   `classPrefix`：用于表示组件 CSS 类的前缀。
*   `closable`：表示该组件的可关闭属性。
...
[https://rsuitejs.com/components/tag/](https://rsuitejs.com/components/tag/)