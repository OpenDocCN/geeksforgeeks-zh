...
> 原文：[https://www.geeksforgeeks.org/reactjs-evergreen-toaster-component/](https://www.geeksforgeeks.org/reactjs-evergreen-toaster-component/)
...
*   `isShown`: 用于关闭吐司并调用 `onCloseComplete` 上的回调，当完成时，该选项设置为假。

**毛巾管理员提议:**

*   `bindNotify`: 是通过 `this.notify` 功能触发的功能。
*   `bindRemove`: 是通过 `this.remove` 功能触发的功能。
*   `bindGetToaster`: 是通过 `this.getToaster` 功能触发的功能。
...
**参考:** [https://evergreen.segment.com/components/toaster](https://evergreen.segment.com/components/toaster)