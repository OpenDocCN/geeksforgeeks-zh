# ReactJS Blueprint OverflowList 组件

> 原文：[https://www.geeksforgeeks.org/reactjs-blueprint-overflowlist-component/](https://www.geeksforgeeks.org/reactjs-blueprint-overflowlist-component/)

Blueprint 是一个基于 React 的 Web 用户界面工具包。该库非常适合构建桌面应用程序的复杂数据密集型界面，并且非常受欢迎。`OverflowList` 组件为用户提供了一种方法，让用户接受通用项目列表，然后它会尽可能多地渲染其内部的项目。我们可以在 ReactJS 中使用以下方法来使用 ReactJS Blueprint OverflowList 组件。

## OverflowList 属性

*   `alwaysRenderOverflow`：用于指示是否强制总是调用 `overflowRenderer`，即使溢出的项目为零。
*   `className`：用于表示传递给子元素的以空格分隔的类名列表。
*   `collapseFrom`：它用于表示项目应该从哪个方向折叠，即子项目的开始或结束。
*   `items`：用于传递项，列表中显示所有项，容器中放不下的项将改为在溢出中渲染。
...
*   `style`：用于表示应用于根元素的 CSS 属性。
*   `tagName`：用于表示容器元素的 HTML 标记名。
*   `visibleItemRenderer`：是一个回调函数，被触发来渲染每个可见的项目。
...
[https://blueprintjs.com/docs/#core/components/overflow-list](https://blueprintjs.com/docs/#core/components/overflow-list)