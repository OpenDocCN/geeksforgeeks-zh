...
React Router 5 提供 4 个挂钩，您可以在 React 应用中使用:
...
这是 React Router 提供的最流行的钩子之一。它允许您访问 React Router 使用的历史实例。使用历史实例，您可以将用户重定向到另一个页面。React Router 创建的历史实例使用一个堆栈(称为“历史堆栈”)，它存储用户访问过的所有条目。
...
*   `length`:返回一个数字。历史堆栈中的条目数
*   `action`:返回表示当前动作(推送、替换或弹出)的字符串。
*   `location`:返回表示当前位置的对象。它可能具有以下属性:
    *   `pathname`:包含网址路径的字符串
    *   `search`:包含网址查询字符串的字符串
    *   `hash`:包含网址哈希片段的字符串
    *   `state`:包含位置特定状态的对象，当该位置被推送到堆栈上时，该对象被提供给 `push(path, state)`。仅在浏览器和内存历史中可用。

**方法:**

*   `push(path, [state])`: 将新条目推送到历史堆栈上。用于将用户重定向到页面
*   `replace(path, [state])`: 替换历史堆栈上的当前条目
*   `go(n)`: 将历史堆栈中的指针移动 n 个条目
*   `goBack()`: 相当于 `go(-1)`。
*   `goForward()`: 相当于 `go(1)`。
*   `block(prompt)`:区块导航。它将回调作为参数，并在导航被阻止后调用它。当您想首先确认用户是否真的想离开页面时，这非常有用。
...
仔细检查登录组件，函数 `handleClick` 获取用户名和密码，并调用 `authenticateUser` 函数，该函数以某种方式验证用户。身份验证完成后，我们希望将用户重定向到 `/profile/John` (假设用户名为“John”)页面。这就是 `handleClick` 函数的最后一行。`useHistory()` hook 返回 React Router 创建的历史实例，`history.push("/profile/John")` 将给定的 URL 添加到历史堆栈中，从而将用户重定向到给定的 URL 路径。同样，您可以根据需要使用历史对象的其他方法和参数。

检查下一个钩子，看看重定向到动态网址是如何工作的。

### 2. `useParams`

这个钩子返回一个包含 URL 中所有参数的对象。
...
**示例:**登录后，我们希望用户被重定向到 `/profile/username` URL。用户名取决于用户的给定名称。因此，我们需要根据用户给定的用户名动态设置网址路径。这很容易做到，我们需要稍微更新一下 `App.js` 文件。
