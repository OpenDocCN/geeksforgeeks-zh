...
我们可以使用 `React Native Tools`，这是 VS Code 中提供的扩展，用于 React Native 应用中的调试。安装扩展后，只需新建一个名为 `Launch.json` 的文件，用于调试配置文件的创建。最后还要启用 `远程调试 JS` 并启用 `直播重装`。现在可以用 VS Code 开始调试了。
