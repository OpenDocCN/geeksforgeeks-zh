...
> 原文: [https://www.geeksforgeeks.org/how-to-use-files-in-public-folder-in-reactjs/](https://www.geeksforgeeks.org/how-to-use-files-in-public-folder-in-reactjs/)
...
如下图。现在创建一个资产文件夹，并把任何样本图像放入其中，就像这里我们保留了 `gfg.png` 文件。此外，我们还将 `gfgPublic.png` 文件保存在公共文件夹中。
...
对于上述文件结构，为了在 `Intro` 组件中显示 `src/assets/gfg.png`，可以简单导入，如下所示:
...
用于显示公用文件夹中的图像，请使用 `public/gfgPublic.png`。`src` 属性的 URL 将是 `process.env.PUBLIC_URL + "/gfgPublic.png"`。这里，`PUBLIC_URL` 将在构建过程中被替换为“PUBLIC”文件夹的 URL。
