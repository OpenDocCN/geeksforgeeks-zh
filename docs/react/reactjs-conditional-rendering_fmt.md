...
为此，我们将创建一个名为“主页”的父组件、两个名为“登录”和“注销”的组件以及一个名为“消息”的组件。我们将使用一个名为`isLoggedIn`的状态变量来存储用户是否登录的信息。该变量的值将根据用户点击按钮而改变。`Homepage`组件将呈现`Message`组件以显示消息，并且它还将基于存储在`this.state`中的值呈现`Login`和`Logout`中的组件之一。`Message`组件也会根据状态`props`的值返回不同的消息。
