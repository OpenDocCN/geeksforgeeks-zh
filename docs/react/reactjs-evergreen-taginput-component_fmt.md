...
*   `isRemovable`: 用于指示标签是否可以移除。

## 切削输入命题

*   `addOnBlur`: 用于指示输入模糊时是否应该将 `inputValue` 添加到标签中。
*   `className`: 用于传递要应用于容器组件的类名。
*   `disabled`: 用于指示输入是否应禁用。
*   `height`: 用于表示输入的垂直尺寸。
*   `inputProps`: 用于表示传递给输入组件的属性。
*   `inputRef`: 用于表示输入元素的 Ref 处理程序。
*   `onAdd`: 是添加新标签时触发的回调函数。
*   `onBlur`: 是一个回调函数，当输入的焦点模糊时触发。
*   `onChange`: 是一个回调函数，在标签值发生变化时触发。
*   `onFocus`: 是输入接收焦点时触发的回调函数。
*   `onInputChange`: 是输入值发生变化时触发的回调函数。
*   `onRemove`: 是一个回调函数，在标签被移除时触发。
*   `separator`: 用于表示粘贴文本或按回车键时要拆分的数值或正则表达式。
*   `tagProps`: 用于为 `tag` 组件提供道具。
*   `tagSubmitKey`: 用于表示打字时为了提交新标签而按下的键。
*   `values`: 用于受控标签值。
...
[https://evergreen.segment.com/components/tag-input](https://evergreen.segment.com/components/tag-input)