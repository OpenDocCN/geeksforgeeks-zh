...
*   `isShown`: 设置为真时显示对话框。
*   `title`: 用于表示对话框的标题。
*   `hasHeader`: 如果设置为真，将显示带有标题和关闭图标按钮的标题。
*   `header`: 用来用我们自己的自定义组件覆盖默认表头。
*   `hasFooter`: 设置为真时，用取消确认按钮显示页脚。
*   `footer`: 用来用我们自己的自定义组件覆盖默认页脚。
*   `hasCancel`: 设置为真时，显示取消按钮。
*   `hasClose`: 设置为真时显示关闭按钮。
*   `onCloseComplete`: 这是退出转换完成时会触发的功能。
*   `onOpenComplete`: 是进入过渡完成时会触发的功能。
*   `onConfirm`: 是点击确认按钮时会触发的功能。
*   `confirmLabel`: 用于表示确认按钮的标签。
*   `isConfirmLoading`: 当设置为真时，确认按钮设置为加载状态。
*   `isConfirmDisabled`: 当设置为真时，确认按钮设置为禁用状态。
...
*   `minHeightContent`: 用于表示身体内容物的最小高度。
*   `containerProps`: 用于表示传递给对话容器的道具。
*   `contentContainerProps`: 用于表示传递给内容容器的道具。
*   `preventBodyScroll`: 表示是否防止外体滚动。
*   `overlayProps`: 用于表示传递给叠加组件的道具。
