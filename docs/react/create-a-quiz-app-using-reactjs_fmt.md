...
> 原文: [https://www.geeksforgeeks.org/create-a-quiz-app-using-reactjs/](https://www.geeksforgeeks.org/create-a-quiz-app-using-reactjs/)
...
这个文件将把我们的应用程序呈现为 HTML 文件，该文件位于 `public` 文件夹中。现在，用文件 `src/components/QuestionBox.js` 和 `src/components/ResultBox.js` 创建一个文件夹名 `components`，用文件 `src/question/index.js` 保存我们的问题。
...
```jsx
mkdir question && cd question && touch index.js
```
...
### 编辑 `src/components/QuestionBox.js` 文件
...
### 编辑 `src/components/ResultBox.js` 文件
...
在浏览器中打开 `http://localhost:3000/` URL。它会显示结果。
