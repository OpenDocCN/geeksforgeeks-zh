...
### 第 4 步:
对于 `picker`，我们在 React Native 中有一个 `picker` 组件，但是该组件现在已经被**弃用了**，所以作为替代，我们将使用一个名为 `react-native-picker` 的外部包。使用以下命令安装该软件包。
...
## 示例:
现在让我们实现 `Picker`。在这里，我们创建了一个精选课程。
...
## 参考:
[https://reactnative.dev/docs/picker](https://reactnative.dev/docs/picker)