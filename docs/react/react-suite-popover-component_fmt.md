...
> 原文: [https://www.geeksforgeeks.org/react-suite-popover-component/](https://www.geeksforgeeks.org/react-suite-popover-component/)
...
*   `title`: 用于表示部件的标题。
*   `visible`: 用于表示默认情况下组件可见。

### Whisper 道具:
*   `container`: 用于设置渲染容器。
*   `delay`: 用于表示延迟时间。
*   `delayHide`: 用于表示隐藏的延迟时间。
*   `delayShow`: 用于表示演出延迟时间。
*   `enterable`: 用于表示触发值设置为悬停时，是否允许鼠标进入 popover 的浮动层。
*   `full`: 用于表示容器中的内容物装满。
*   `onBlur`: 是失去焦点时触发的回调函数。
*   `onClick`: 是点击事件触发的回调函数。
*   `onClose`: 是关闭组件时触发的回调函数。
*   `onEnter`: 是一个回调函数，在叠加转换之前触发。
*   `onEntered`: 这是一个回调函数，在覆盖完成转换后触发。
*   `onEntering`: 这是一个回调函数，当覆盖开始转换时触发。
...
*   `speaker`: 用于显示组件。
*   `trigger`: 用于触发事件。
*   `triggerRef`: 用于表示触发器的参考。

### Whisper 方法:
*   `open`: 是用于显示弹出窗口的功能。
*   `close`: 是用来隐藏 popover 的功能。
...
### 参考:
[https://rsuitejs.com/components/popover/](https://rsuitejs.com/components/popover/)