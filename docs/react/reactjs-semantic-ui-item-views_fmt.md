...
> 原文: [https://www.geeksforgeeks.org/reactjs-semantic-ui-item-view/](https://www.geeksforgeeks.org/reactjs-semantic-ui-item-view/)

语义用户界面是一个现代框架，用于为网站开发无缝设计。它为用户提供了轻量级的组件体验。它使用预定义的 CSS、JQuery 语言来整合到不同的框架中。

在本文中，我们将了解如何在 ReactJS 语义用户界面中使用项目视图。`Item` 视图用于向用户呈现一组项目。
...
## 示例
在本例中，我们将使用 ReactJS 语义 UI 分割线元素向用户显示项目集。这里我们有一个项目，其样本内容包含一个描述和一个项目元素的额外部分。
...
## 参考
[https://react.semantic-ui.com/views/item](https://react.semantic-ui.com/views/item)