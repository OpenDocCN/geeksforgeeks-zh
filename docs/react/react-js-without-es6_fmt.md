...
如果我们在 react 中不使用 ES6，那么还有一种替代方法可以执行。我们用`create-react-class`代替 ES6。让我们看看 ES6 和 create-react-class 方法之间的一些变化。
...
使用 ES6，默认道具可以作为 reactjs 类组件的属性。
...
与声明默认道具相同，设置初始状态相同。为了设置初始状态，我们需要在构造器中分配`this.state`。为此制作`Counter.js`文件。
