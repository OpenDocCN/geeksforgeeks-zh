...
> 原文：[https://www.geeksforgeeks.org/reactjs-reactstrap-alert-component/](https://www.geeksforgeeks.org/reactjs-reactstrap-alert-component/)

`Reactstrap` 是一个流行的前端库，可以轻松使用 `React Bootstrap 4` 组件。该库包含 `Bootstrap 4` 的无状态 `React` 组件。警报组件用于紧急中断，要求确认或通知用户情况。我们可以在 `ReactJS` 中使用以下方法来使用 `ReactJS Reactstrap` 警报组件。
