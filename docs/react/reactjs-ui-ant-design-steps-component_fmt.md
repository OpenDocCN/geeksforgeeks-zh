...
蚂蚁设计库预建了这个组件，也很容易集成。`Steps` 组件是一种导航栏，它通过引导用户完成任务的步骤来帮助。我们可以在 ReactJS 中使用以下方法来使用 Ant 设计步骤组件。
...
*   `className`: 用于定义该组件的附加类。
*   `current`: 从 0 开始计数，设置当前步数。
*   `direction`: 用于指定步骤栏的方向，垂直或水平。
*   `initial`: 用于从 0 开始计数，设置初始步长。
*   `labelPlacement`: 用于在垂直或水平方向放置标题和描述。
*   `percent`: 用于表示流程状态下当前步骤的进度圈百分比。
*   `progressDot`: 用于应用进度点样式的步骤。
*   `responsive`: 用于屏幕宽度小于 532 像素时改为垂直方向。
*   `size`: 用于指定步骤栏的尺寸。
*   `status`: 用于指定当前步骤的状态。
*   `type`: 用于表示步骤的类型。
...
*   `description`: 用于定义步骤的描述。
*   `disabled`: 用于禁用点击事件。
*   `icon`: 用于定义步骤的图标。
*   `status`: 用于指定状态。
*   `subTitle`: 用于表示该步骤的副标题。
*   `title`: 用于表示步骤的标题。
