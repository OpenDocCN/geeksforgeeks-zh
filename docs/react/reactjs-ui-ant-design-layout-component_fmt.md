...
Ant Design库预建了这个组件，也很容易集成。布局组件用于处理页面的整体布局。我们可以在ReactJS中使用以下方法来使用Ant Design布局组件。

## 布局属性

*   `className`：用于传递容器类名。
*   `hasSider`：表示子元素体内是否含有`Sider`。
*   `style`：用于自定义样式。

## Layout.Sider属性

*   `breakpoint`：用于表示响应布局的断点。
*   `className`：用于传递容器类名。
*   `collapsed`：用于设置当前状态。
*   `collapsedWidth`：用于表示折叠侧边栏的宽度。
*   `collapsible`：表示是否可以折叠。
*   `defaultCollapsed`：用于设置初始状态。
*   `reverseArrow`：用于反转箭头的方向，表示从右侧展开的边。
*   `style`：用于自定义样式。
*   `theme`：用来表示侧边栏的Color主题。
*   `trigger`：用于指定自定义触发。
*   `width`：用来表示侧边栏的宽度。
*   `zeroWidthTriggerStyle`：当`collapsedWidth`为0时，用于自定义特殊触发器的样式。
*   `onBreakpoint`：是断点改变时触发的回调函数。
*   `onCollapse`：是点击触发器或激活响应布局时触发的回调函数。
