...
> 原文: [https://www.geeksforgeeks.org/how-to-set-background-images-in-reactjs/](https://www.geeksforgeeks.org/how-to-set-background-images-in-reactjs/)
...
在这个方法中，我们在元素本身内部添加 `style` 属性。

**文件名: `App.js`**

在 `App.js` 中，我们将在 `div` 元素中添加一个 `style` 属性，并使用内嵌 CSS 为 `div` 元素设置背景图像。
...
在这个方法中，我们添加一个外部 CSS 文件，为 react 组件设置一个背景图像。
...
用户可以使用环境变量通过绝对 URL 直接从 `public/` 文件夹访问背景图片。在使用此方法之前，不要忘记在公共文件夹中添加图像，否则会显示编译错误。
...
用户可以通过相对路径 URL 访问 `public/` 文件夹或公共文件夹的任意子文件夹中的图像。访问图片的网址应该类似 `<host_name>/image.png`。
...
如果图像驻留在 `src` 目录内，用户可以将其导入组件文件管理器内，并将其设置为背景图像。
...
**运行 React 应用程序的步骤:** 您可以使用以下命令运行 React 应用程序。
