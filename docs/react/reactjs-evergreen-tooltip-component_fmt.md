...
*   `appearance`: 用于工具提示的外观。
*   `position`: 用于定位波波夫。
*   `content`: 用于表示波波夫的含量。
*   `hideDelay`: 用于表示隐藏工具提示前的时间，单位为毫秒。
*   `showDelay`: 用于表示工具提示显示前的时间，单位为毫秒。
*   `isShown`: 当设置为真时，手动显示工具提示。
*   `children`: 用于工具提示的目标按钮。
*   `statelessProps`: 用于定义传递给工具提示的属性。
...
[https://evergreen.segment.com/components/tooltip](https://evergreen.segment.com/components/tooltip)