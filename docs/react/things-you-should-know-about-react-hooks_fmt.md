...
> 原文: [https://www.geeksforgeeks.org/things-you-should-know-about-react-hooks/](https://www.geeksforgeeks.org/things-you-should-know-about-react-hooks/)
...
`render()`，`componentDidMount()`，`componentDidUpdate()`，`componentWillUnmount()`所有这些都是生命周期方法。React hooks 是状态管理和生命周期方法的替代方法。有许多不同用途的钩子。其中一些是 `useState`、`useEffect`、`useCallback`、`useMemo`、`useRef`、`useContext` 等。
...
如果你和 Redux 合作过，你可能听过这个词。Reducer 钩子作为 `useState` 钩的替代物。当一个状态被改变时，它被捆绑在一个名为 Reducer 的中心函数中。之后，状态将根据操作和现有状态进行更新。下面给出了其中一个例子…你可以重写下面给出的状态挂钩例子…
...
引用钩子 `useRef` 引用由渲染方法创建的反应元素。下面给出了一个例子…
...
我们已经通过代码示例讨论了主要的钩子及其用法。这些钩子不限于此。一旦你有了“反应”的经验，你将会使用更多的钩子，比如 `useDispatch`，`useSelector` 等等。每一个都有自己的应用。