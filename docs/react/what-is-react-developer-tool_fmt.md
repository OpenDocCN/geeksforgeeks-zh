...
React 开发工具是 React JavaScript 库的 `Chrome 开发工具` 扩展。如果您正在处理 `React.js` 应用程序，这是一个非常有用的工具。这个扩展将 React 调试工具添加到 `Chrome 开发工具` 中。它帮助您检查和编辑构建页面的 React 组件树，对于每个组件，您可以检查道具、状态、钩子等。
...
跟随链接 [React 开发者工具](https://chrome.google.com/webstore/detail/react-developer-tools/fmkadmapgofadopljbjfkapdkoienihi?hl=en)
...
打开要检查的应用程序，现在单击扩展选项卡，然后单击 React 开发工具。如果扩展仍然是彩色的，这意味着它是用 `React.js` 构建的；如果扩展是无色的，这意味着应用程序没有使用 `React.js`。
