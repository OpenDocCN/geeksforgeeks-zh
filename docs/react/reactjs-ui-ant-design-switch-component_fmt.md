...
Ant Design 库预建了这个组件，也很容易集成。开关组件是用来切换单个设置的状态开或关。我们可以在 ReactJS 中使用以下方法来使用 Ant Design 开关组件。
...
*   `className`: 用于传递该交换机的类名。
*   `defaultChecked`: 表示是否设置初始状态。
*   `disabled`: 用于禁用开关。
*   `loading`: 用于显示开关的加载状态。
*   `size`: 用来表示开关的大小。
