...
> 原文: [https://www.geeksforgeeks.org/reactjs-ui-ant-design-tabs-component/](https://www.geeksforgeeks.org/reactjs-ui-ant-design-tabs-component/)

蚂蚁设计库预建了这个组件，也很容易集成。标签组件用作标签，可以方便地在不同视图之间切换。我们可以在 ReactJS 中使用以下方法来使用 Ant 设计选项卡组件。

**标签道具:**

*   `activeKey:` 用于表示当前标签面板的键。
*   `addIcon:` 用于自定义添加图标。
*   `animated:` 用于指示是否用动画改变标签页。
*   `centered:` 用于制表符居中。
*   `defaultActiveKey:` 用于表示初始激活的 `TabPane` 的键。
*   `hideAdd:` 表示是否隐藏加号图标。
...
*   `closeIcon:` 用于自定义标签面板头部的关闭图标。
*   `forceRender:` 用于选项卡中内容的强制渲染。
*   `key:` 用于表示 `TabPane` 的键。
...
**参考:** [https://ant.design/components/tabs/](https://ant.design/components/tabs/)