...
> 原文: [`https://www.geeksforgeeks.org/how-to-use-divider-component-in-reactjs/`](https://www.geeksforgeeks.org/how-to-use-divider-component-in-reactjs/)

分隔线是一条细线，将列表和布局中的内容分组。`React` 的 `Material UI` 有这个组件可供我们使用，非常容易集成。我们可以使用下面的方法在 `ReactJS` 中使用分频器组件。

### 创建反应应用程序并安装模块

1.  **步骤 1:** 使用以下命令创建一个 `React` 应用程序。
    ```jsx
    npx create-react-app foldername
    ```

2.  **步骤 2:** 在创建项目文件夹(即 `foldername`)后，使用以下命令移动到该文件夹。
    ```jsx
    cd foldername
    ```

3.  **步骤 3:** 创建 `ReactJS` 应用程序后，使用以下命令安装 `material-ui` 模块。
    ```jsx
