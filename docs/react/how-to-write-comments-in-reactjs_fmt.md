...
> 原文: [https://www.geeksforgeeks.org/how-to-write-comments-in-reactjs/](https://www.geeksforgeeks.org/how-to-write-comments-in-reactjs/)
...
### 示例 1: React 组件注释
我们可以在 `React` 中使用双正斜杠 `//` 或星号格式 `/* */` 编写注释，类似于常规的 `JavaScript`。
...
### 示例 2: 在 JSX 中注释
当我们想要评论**渲染块**内部的东西时，上面的示例不起作用。这是因为我们在渲染块中使用了 `JSX`，并且必须在大括号 `{/* */}` 中使用多行注释。
