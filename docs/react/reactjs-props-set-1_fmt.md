...
React 允许我们使用名为 **props** (代表属性) 向组件传递信息。道具基本上是一种全局变量或对象。我们将在本文中详细了解这些内容。
...
在上面的代码片段中，我们将名为 `sampleProp` 的道具传递给名为 `DemoComponent` 的组件。这个道具的值是 `"HelloProp"`。现在让我们看看如何才能获得这些道具。
...
我们可以使用上面的语法从组件类内部访问任何 prop。`this.props` 是一种存储所有组件道具的全局对象。`propName`，即道具的名称是这个物体的关键。
...
在上面的例子中，我们使用了一个基于类的组件来说明道具。但是我们可以将道具传递给基于功能的组件，就像我们在上面的例子中所做的那样。但是要从函数中访问一个道具，我们不再需要使用 `this` 关键字。功能组件接受道具作为参数，可以直接访问。下面是与上面相同的示例，但是这次使用了基于函数的组件而不是基于类的组件。
