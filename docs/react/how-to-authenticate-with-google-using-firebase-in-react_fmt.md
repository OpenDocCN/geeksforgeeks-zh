...
下面的方法涵盖了如何在 react 中使用 firebase 向 Google 进行身份验证。我们已经使用`firebase`模块来实现所以。
...
**步骤 2:** 在创建项目文件夹(即`myapp`)后，使用以下命令将`cd`移动到该文件夹:
...
**第六步:**进入你的 firebase 仪表盘，启用`Google`登录方法，如下图所示。
