...
> 原文：`https://www.geeksforgeeks.org/reactjs-semantic-ui-radio-addons/`

语义用户界面是一个现代框架，用于为网站开发无缝设计，它给用户一个轻量级的组件体验。它使用预定义的 CSS、JQuery 语言来整合到不同的框架中。

在本文中，我们将了解如何在 ReactJS 语义用户界面中使用 `Radio` 插件。单选按钮用于制作单选按钮。

## 属性

*   `toggle`：通过这个属性，我们可以使用单选作为切换。
*   `slider`：通过这个属性，我们可以使用 `radio` 作为滑块。
*   `Radio Group`：我们可以使用这个属性将一个单选按钮组合在一起。

## 状态

*   `readOnly`：在这种状态下，收音机将是只读的。
*   `checked`：在此状态下，收音机将被预先检查。
*   `disabled`：在这种状态下，可以禁用收音机。
*   `remote control`：在这种状态下，可以远程控制收音机。
...
在这个示例中，我们展示了如何使用 ReactJS 语义 UI 无线电插件来使用基本的无线电插件。
...
在本例中，我们显示了无线电插件 ReactJS Semantic UI 无线电插件中的 `disabled`、`checked` 和 `readOnly` 状态。
