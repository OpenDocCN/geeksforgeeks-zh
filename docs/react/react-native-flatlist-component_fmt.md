...
> 原文: [https://www.geeksforgeeks.org/react-native-flatlist-component/](https://www.geeksforgeeks.org/react-native-flatlist-component/)
...
*   `ListFooterComponentStyle`: 用于为 `ListFooterComponent` 设置内部视图样式。
*   `ListHeaderComponent`: 它呈现在所有项目的顶部。
*   `ListHeaderComponentStyle`: 用于设置内部视图的样式 `ListHeaderComponent`。
*   `columnWrapperStyle`: 这是多项目行的可选自定义样式。
*   `extraData`: 是告诉列表重新渲染的属性。
*   `getItemLayout`: 这是一个可选的优化，如果你知道项目的大小，可以跳过动态内容的测量。
*   `horizontal`: 如果这是真的，那么项目将被水平渲染而不是垂直渲染。
*   `initialNumToRender`: 它告诉在初始批次中要渲染多少个项目。
*   `initialScrollIndex`: 如果提供，那么它将从 `initialScrollIndex` 项目开始，而不是从最上面的项目开始。
*   `inverted`: 反转卷轴方向。
*   `keyExtractor`: 用于提取给定项目的唯一密钥。
*   `numColumns`: 用于渲染多列。
*   `onEndReached`: 当滚动位置进入渲染内容时调用一次。
*   `onEndReachedThreshold`: 它告诉我们离终点还有多远。
*   `refreshControl`: 如果提供，将添加标准刷新控制。
*   `onViewableItemsChanged`: 当行的可见性发生变化时调用。
*   `progressViewOffset`: 设置需要加载时的偏移量。它只在安卓系统上可用。
*   `refreshing`: 设置为真，同时等待刷新的新数据。
*   `removeClippedSubviews`: 这可能会提高大型列表的滚动性能。在安卓系统上，默认值为真。
*   `viewabilityConfigCallbackPairs`: 显示配对列表。

## 列表中的方法

*   `flashScrollIndicators()`: 它会瞬间显示滚动指示器。
...
*   `scrollToEnd()`: 滚动到内容末尾。
*   `scrollToIndex()`: 滚动到提供索引的特定项目。
*   `scrollToItem()`: 滚动到提供的项目。它需要对数据进行线性扫描。
*   `scrollToOffset()`: 滚动到列表中特定的内容像素偏移量。
...
现在让我们实现 `FlatList`。在这里，我们创建了一个课程平面列表。
