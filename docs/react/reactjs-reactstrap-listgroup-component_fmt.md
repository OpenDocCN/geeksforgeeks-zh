...
> 原文: [https://www.geeksforgeeks.org/reactjs-reactstrap-listgroup-component/](https://www.geeksforgeeks.org/reactjs-reactstrap-listgroup-component/)
...
*   `flush`: 列表组道具中的同花顺属性。它用于指示是否在父容器中边对边地呈现列表组项。
*   `horizontal`: 列表组属性中的水平属性用于指示是否水平渲染列表组项目。
*   `className`: 类名用于表示组件样式的类名
*   `cssModule`: `ListGroup` 道具中的 `ccsModule` 属性用于表示样式化的 CSS 模块。
...
现在在 `App.js` 文件中写下以下代码。这里，我们使用了没有禁用和活动道具的列表组组件。
...
现在在 `App.js` 文件中写下以下代码。在这里，我们使用了禁用和活动道具的列表组组件。
...
**参考:** [https://reactstrap.github.io/components/listgroup/](https://reactstrap.github.io/components/listgroup/)