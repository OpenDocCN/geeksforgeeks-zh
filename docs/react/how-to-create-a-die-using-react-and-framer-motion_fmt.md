# 如何使用 React 和 framer-motion 创建骰子？

> 原文：[https://www.geeksforgeeks.org/how-to-create-a-die-using-react-and-framer-motion/](https://www.geeksforgeeks.org/how-to-create-a-die-using-react-and-framer-motion/)

我们可以使用 `react`（配合纯 `CSS`）和 `framer-motion` 库来创建一个骰子，并为其制作动画。
...
**步骤 2：** 创建项目文件夹后，即 `rolling-die`，使用以下命令移动到项目文件夹：

```jsx
cd rolling-die
```

**步骤 3：** 创建 ReactJS 应用程序后，使用以下命令安装 `framer-motion` 模块：
