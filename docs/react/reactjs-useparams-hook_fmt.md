...
### 步骤 2:
创建项目文件夹(即 `useparams_react`)后，使用以下命令移动到该文件夹。
...
### 步骤 3:
创建 ReactJS 应用程序后，使用以下命令安装 `react-router-dom` 和 `react-dom` 包。
...
## 输出:
现在打开浏览器，转到 `http://localhost:3000/`，会看到如下输出。你可以看到你在网址中传递了什么:`id` 显示在屏幕上。通过这种方式，您可以访问当前路线的网址参数。
