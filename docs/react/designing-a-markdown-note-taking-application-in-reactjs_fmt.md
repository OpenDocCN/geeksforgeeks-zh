...
> 原文：[https://www.geeksforgeeks.org/designing-a-markdown-note-taking-application-in-reactjs/](https://www.geeksforgeeks.org/designing-a-markdown-note-taking-application-in-reactjs/)

在本文中，我们将学习如何在 `React` 中设计一个 `markdown` 笔记应用程序，它可以帮助用户快速记下笔记并下载渲染输出。使用 `markdown` 可以减少格式化文本的时间，从而只使用 [`markdown`](https://www.geeksforgeeks.org/introduction-to-markdown/) 语法创建丰富的文本内容。
...
我们将从使用 `create-react-app` 工具创建一个新的 `react` 项目开始。您可以使用以下命令创建一个具有所需名称的项目。请确保系统上安装了 `node` 和 `npm`。
...
我们将从将 `bootstrap` CDN 添加到 `index.html` 页面开始。CDN 链接可以在[这里](https://getbootstrap.com/docs/5.0/getting-started/introduction/)找到，可以直接添加到索引页的头部。

我们还需要两个 `npm` 包，一个是 [`markdown-it`](https://www.npmjs.com/package/markdown-it) 包，它负责解析 `markdown` 内容并以 HTML 形式向我们提供渲染输出。我们可以简单地使用这个 HTML 来显示我们的输出。另一个模块是 [`file-saver`](https://www.npmjs.com/package/file-saver) 包，它简化了将给定的 `blob` 保存到系统的任务。以下命令用于安装这两个软件包：

```jsx
npm install markdown-it file-saver
```

## 步骤 3：使用 `bootstrap` 创建简单的用户界面

我们将使用 `bootstrap` 网格系统来创建布局，以确保我们的应用程序是移动响应的，并且在不编写大量 CSS 的情况下看起来很好。

应用程序可分为 3 个部分；允许用户在 `markdown` 中输入注释的 `markdown` 编辑器，显示渲染的 HTML 的输出区域，以及包含文件名输入字段和保存内容按钮的文件保存选项。使用 `bootstrap` 的 `card` 组件嵌套每个部分，并将相关组件添加到输入元素中。
...
我们将使用 [`useState`](https://www.geeksforgeeks.org/what-is-usestate-in-react/) 方法创建 3 个钩子来管理我们应用程序的 3 个部分中的状态。每个钩子返回变量本身及其初始状态，以及可用于设置该变量状态的函数。当组件的状态改变时，`React` 将自动重新呈现该组件。
...
`markdownText` 变量用在 `textarea` 中，我们将在这里写 `markdown` 文本。我们将在 `textarea` 的 `value` 属性中使用该值。使用 `setMarkDownText` 功能，我们可以在这里更改文本的值，它将通过这个 `value` 属性显示。

在 `div` 元素中使用了 `renderedHTML` 变量，该变量将显示 `markdown` 的渲染输出。由于 `markdown-it` 模块将从 `markdown` 返回 HTML，我们需要直接设置这个元素的内部 HTML。这将需要使用元素的 `dangerouslySetInnerHTML` 属性，并传递一个带有 `__html` 键的对象。
...
**`handleTextInput()` 函数：** 该函数将获取在 `markdown` 编辑器中输入的文本，并使用 `markdown-it` 渲染所需的 HTML。

我们将首先通过获取目标的值来获取输入的文本。然后，我们初始化 `MarkdownIt` 的一个新实例，并使用它的 `render` 方法。然后我们将使用 `setRenderedHTML` 功能在输出区域设置这个，我们的最终 HTML 将在输出区域显示。该功能将在 `markdown` 编辑器的 `onChange` 属性中使用。

**`saveHTML()` 和 `saveMarkdown()` 函数：** 这些函数将用给定的文件名将内容保存到我们的系统中。在这两个功能中，我们将使用要保存的内容创建一个 `Blob` 对象。我们可以使用 `renderedHTML` 和 `markdownText` 变量来获取应用程序的当前文本内容。

接下来指定 `blob` 的类型。我们可以使用 `text/html` 作为渲染输出，这样它就被保存为一个 html 文件，而只有 `text/plain` 作为 `markdown` 文本。

我们现在将使用 `file-saver` 模块的 `saveAs()` 方法将文件保存到系统中。这个方法使用两个参数，第一个是必须保存的 `blob` 对象，第二个是应该用来保存文件的文件名。我们可以通过使用 `fileName` 变量获得文件名，并传递我们之前创建的 `blob` 对象。
...
### 源代码:
[https://github.com/sayantanm19/react-markdown-notes-gfg](https://github.com/sayantanm19/react-markdown-notes-gfg)