...
> 原文：[https://www.geeksforgeeks.org/10-reasons-to-learn-reactjs-in-2021/](https://www.geeksforgeeks.org/10-reasons-to-learn-reactjs-in-2021/)
...
`React` 不需要很长的代码来编写应用程序。在 `React` 中，我们说一切都是一个组成部分。用户编写称为组件的小代码，并将它们组合成另一个组件。简单来说，任何用 `React` 制作的应用程序都是一个组件树。
...
### 4. 使用一项技术构建网页、移动应用、和虚拟现实应用

有了 `React`，你可以建立一个更好的网站、反应灵敏的移动应用和虚拟现实应用。`Airbnb`、`Cloudflare`、`Dropbox`、`BBC`、`Facebook`、`Instagram` 等热门网站都是使用 `ReactJS` 搭建的。`Instagram`、`Facebook`、`纽约时报` 等一些热门应用。也是使用 `ReactJS` 构建的。

### 5. 反应堆上的迁移

大多数不了解 `ReactJS` 的社区现在都迁移到 `ReactJS`，因为它非常容易理解和实现。他们已经开始在 `ReactJS` 上重写代码。一些顶级公司有 `Reddit`、`BBC`、`Netflix` 等。这增加了世界上对 `ReactJS` 技能的工作要求。
...
这是一份世界上顶尖科技公司的名单，他们正在使用 `ReactJS` 作为一项非常重要的技术。
...
**注：** `Facebook` 于 2011 年首次开始使用 `React JS`，到现在已经用了十年。在 `Facebook` 开始使用它之后，所有其他公司都开始跟随它。
