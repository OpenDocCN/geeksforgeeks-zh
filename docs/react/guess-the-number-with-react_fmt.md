...
我们创建了两个组件`App`和`Result`。`App`组件包含所有的逻辑，它是有状态的，`Result`组件只根据用户的猜测在页面上显示适当的消息。`App`组件有一个受控的输入字段，用户可以在其中输入和猜测数字。我们给`App`组件设置了一个默认道具`secret`，它保存了所需的秘密号，并且是随机生成的。`App`组件将输入字段的值和密码传递给`Result`组件。`Result`组件相应地显示猜测的数字是高或低或正确的适当消息。
