...
现在，通过运行以下命令进入项目文件夹，即 `ip-finder`:
...
让我们抓取这个项目所需的 Mapbox API 键。遵循以下简单步骤:
...
在这个文件中，我们将使用名为 `https://ipapi.co/*`的免费开源 API 获取用户的 IP 地址(免费计划:1000 个请求/天)。`App.js` 分为两部分，一部分用于显示用户的 IP 地址、位置& ISP(互联网服务提供商)，另一部分用于在动态地图上显示用户的大概位置。现在，在 `App.js` 组件中写下以下代码。
