...
> 原文: [https://www.geeksforgeeks.org/reactjs-semantic-ui-dimmer-module/](https://www.geeksforgeeks.org/reactjs-semantic-ui-dimmer-module/)

语义用户界面是一个现代框架，用于为网站开发无缝设计。它为用户提供了轻量级的组件体验。它使用预定义的 CSS、JQuery 语言来整合到不同的框架中。

在本文中，我们将了解如何在 ReactJS 语义用户界面中使用调光器模块。`Dimmer`模块用于将用户的注意力集中在页面的特定部分或某个片段上。

## 属性

*   `content`调光器：调光器可以显示内容。
*   `page`调光器：调光器可以格式化为固定在页面上。
...
在这个例子中，我们展示了一个暗淡的效果来集中用户的注意力。当用户单击“调光”按钮时，会显示这种调光效果。原来的按钮也要呈现才能回到正常状态。
...
在这个示例中，我们展示了一个加载效果来集中用户的注意力。当用户点击加载按钮时，显示这种加载效果。原来的按钮也要呈现才能回到正常状态。
...
**参考：** [https://react.semantic-ui.com/modules/dimmer/](https://react.semantic-ui.com/modules/dimmer/)