...
*   **第四步:**现在进入设置，点击 `js`。选择 `巴贝尔` 作为 `javascript` 预处理器，在搜索栏中输入 `React` 导入 `React` 和 `React-dom` 为 `React-dom` `CDNjs`，然后保存并关闭。![](img/9b7ef7c40473648a80bdb08ad5dd97da.png)

现在在相应的块中编写 `JavaScript`、`HTML` 和 `CSS` 代码。编写完代码后，点击保存按钮，现在如果您的代码没有错误，那么它将成功运行并显示输出。
```
编写完代码后，点击保存按钮，现在如果您的代码没有错误，那么它将成功运行并显示输出。
```