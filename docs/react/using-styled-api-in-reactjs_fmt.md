...
在继续下一步之前，首先我们必须通过在您的项目目录中运行以下命令，借助于您的 `src` 文件夹中的终端，或者您也可以在您的项目文件夹中的 Visual Studio Code 的终端中运行该命令来安装样式化的组件。
...
安装完模块后，现在打开您的 `App.js` 文件，该文件位于项目目录中的 `src` 文件夹下。

### 第 3 步

现在导入 React 和 styled 模块。

### 第 4 步

在你的 `App.js` 文件中，添加这个代码片段来导入 React 和 styled 模块。
