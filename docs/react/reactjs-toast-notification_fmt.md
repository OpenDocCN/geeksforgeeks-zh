...
**吐司通知**是为了向用户显示消息而添加的弹出消息。它可以是成功消息、警告消息或自定义消息。吐司通知也叫**Toast通知**。所有的吐司通知都在`react-toastify`模块中，所以要使用它们，我们需要导入这个模块。
...
*   **第三步:** 现在导入`react-toastify`模块，Toastify CSS 文件，以及一个 toast 通知的调用者方法。
*   **第四步:** 在你的`app.js`文件中，通过添加下面的代码来导入 Toast 模块。
