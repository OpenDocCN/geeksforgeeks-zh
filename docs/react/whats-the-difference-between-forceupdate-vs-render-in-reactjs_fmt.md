...
> 原文: [https://www.geeksforgeeks.org/whats-the-difference-between-forceupdate-vs-render-in-reactjs/](https://www.geeksforgeeks.org/whats-the-difference-between-forceupdate-vs-render-in-reactjs/)
...
| **`render()`** | **`forceUpdate()`** |
| --- | --- |
| 在以下条件下调用此方法: <br> *   当一个组件被实例化时。 <br> *   或者当状态道具被更新时。 | 当状态或道具以外的一些数据发生变化时调用。 |
| 它不会跳过任何生命周期方法。 | 它会跳过生命周期 `shouldComponentUpdate` 方法。 |
| 不可由用户调用。 | 手动调用它。 |
| 它将在需要时自动重新渲染。 | 它有效地重新渲染组件。 |
| 强制使用，因为这是类组件中唯一需要的方法。 | 不推荐。应避免使用。 |