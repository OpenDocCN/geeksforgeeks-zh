...
Ant Design 库已经预建了这个组件，它也很容易集成。我们可以使用下面的方法轻松地使用这个自旋组件。
...
*   `delay`: 定义加载动画的延迟时间，单位为毫秒。
*   `indicator`: 旋转指示器的 React 节点。
*   `size`: 定义微调器的大小，即 `small`、`default` 和 `large`。
*   `spinning`: 这是一个布尔值，定义旋转是否应该旋转。
*   `tip`: 当 `Spin` 有子元素时自定义描述内容。
...
**步骤 2:** 创建 React 应用程序后，使用以下命令进入该应用程序:
...
**参考:** [https://ant.design/components/spin/](https://ant.design/components/spin/)