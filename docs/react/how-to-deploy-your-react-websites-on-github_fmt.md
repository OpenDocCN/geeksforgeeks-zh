...
> 原文：[https://www.geeksforgeeks.org/how-to-deploy-your-react-websites-on-github/](https://www.geeksforgeeks.org/how-to-deploy-your-react-websites-on-github/)

构建 web 应用程序对开发人员来说总是令人兴奋的，尤其是当你第一次踏入编程世界的时候。经过大量的努力，你建立了应用程序的前端，你想向世界展示你的技能，你的创造力，当然还有你的辛勤工作。有些人可能熟悉免费托管网站，他们可以在那里托管他们的应用程序。你们中的一些人可能也熟悉 [GitHub Pages](https://pages.github.com/) 来部署静态网页。如果你不知道，那么这里有一个链接来检查……[使用 GitHub 来托管一个免费的静态网站](https://www.geeksforgeeks.org/using-github-to-host-a-free-static-website/)。嗯，这些资源非常受欢迎，可以让你的网络应用在服务器上运行，如果它非常有用并且免费可用，为什么不呢？

![Deploy Your React Websites on Github](img/60e4461f203e677a4a37b09ca88eb9c2.png)

你知道你可以用流行的 JavaScript 库 `React` 做一个酷酷的网站，也可以部署在 `GitHub Pages` 上吗？你不信吗……？？在系统中安装 [React Developer Tool](https://chrome.google.com/webstore/detail/react-developer-tools/fmkadmapgofadopljbjfkapdkoienihi?hl=en)，自行检查链接 [**Portfolio**](https://anuupadhyay.github.io/portfolio/)。用 `React` 学习和构建一个 web 应用不是很酷很神奇吗？在 `GitHub Pages` 上免费部署你的 `React` web app，并向全世界展示，这不是很酷很神奇吗？

现在，如果你是一个对学习 `ReactJS` 感到兴奋的人，那么这里是你的链接 [**geeksforgeeks ReactJS**](https://www.geeksforgeeks.org/reactjs/)。但是要在 `GitHub Pages` 上部署你漂亮的应用程序，我们只需要再学习一些东西。因此，在这个博客中，我们将讨论在 `GitHub Pages` 上部署 `React` 应用程序的分步过程。

## 基本术语

1.  **[React:](https://www.geeksforgeeks.org/reactjs/)** 由 Facebook 开发的一个流行的 JavaScript 库，用于创建和处理用户界面的灵活组件。
...
2.  应安装足够版本的 [Node](https://nodejs.org/en/) 和 `NPM`。下面是检查安装和版本的命令。
...
3.  安装了适当版本的 [create-react-app](https://github.com/facebook/create-react-app)。下面是检查安装和版本的命令。
...
2.  现在输入您的新应用程序，并运行以下命令来启动应用程序。您将看到您的应用程序正在本地开发服务器上运行。
...
5.  给 app 的 `package.json` 文件添加一些属性。在顶层，添加一个 `homepage` 属性。将其值定义为字符串 `http://{username}.github.io/{repo-name}`，其中 `{username}` 是您的 `GitHub` 用户名，`{repo-name}` 是您在步骤 3 中创建的 `GitHub` 存储库 (`my-app`) 的名称。
...
现在您需要再添加两个属性。在现有 `scripts` 属性中，添加一个 `predeploy` 属性和一个 `deploy` 属性，每个属性的值如下所示：
...
一旦完成以上操作，您的文件 `package.json` 将看起来像这样…

![package-json](img/b478575002f15095e512ae92aaf7e60c.png)

接下来，如果您从 `react-router-dom` 使用路由器，那么您需要将 `BrowserRouter` 标签更改为
...
7.  现在魔法来了。按照下面的命令，生成应用程序的生产版本，在 `GitHub Pages` 上部署代码。
...
就这样。您的 `React` 应用程序发布在 `Github Pages` 上，如果您想验证它，只需转到 `GitHub` 存储库中应用程序的设置选项卡并向下滚动。你会看到下面这样的东西…

![published site](img/34842dc50b91f224b4b5ce515c6a5704.png)

在我们的例子中，该应用可以在 `https://anuupadhyay.github.io/my-app/` 访问。
