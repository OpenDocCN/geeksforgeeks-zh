...
*   `flex direction`: CSS 的默认值为`row`，而 React Native 中的默认值为`column`。
*   `align content`: CSS 的默认值是`stretch`，React Native 中的默认值是`flex-start`。
...
**`flexDirection` 属性:** `flexDirection`属性控制节点子节点的布局方向，即主轴。以下给出了四种类型的弹性方向:

*   `column` (默认值): 这以垂直方式从上到下对齐子代。
*   `row`: 这以水平方式从左到右对齐孩子。
*   `column-reverse`: 这将在垂直空间中从下向上对齐子对象。
*   `row-reverse`: 这将在水平空间中从右向左对齐子对象。
...
**参考:** [https://reactnative.dev/docs/flexbox](https://reactnative.dev/docs/flexbox)