...
> 原文:[https://www.geeksforgeeks.org/how-to-use-map-to-create-lists-in-reactjs/](https://www.geeksforgeeks.org/how-to-use-map-to-create-lists-in-reactjs/)
...
**示例:**在使用地图创建组件时，react 期望将一个关键道具传递给正在构建的每个组件。如果没有传递密钥，它仍然会渲染，但是您会在控制台中看到来自 React 的警告。在 `App.js` 文件中，我们定义了一个水果列表，然后映射到一个 div 列表。
...
**说明:**我们声明了一个将用于渲染草皮的水果列表。`styles`对象包含每个项目的 CSS 样式，它只是添加了一些边距、填充和一些阴影。最后，`App`函数返回一个 div 列表，这些 div 是通过调用水果数组上的 `map()`返回的。我们在每个 div 的`style`属性中添加了`styles`对象。