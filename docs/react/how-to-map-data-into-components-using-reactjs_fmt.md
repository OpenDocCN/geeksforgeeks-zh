...
> 原文: [https://www.geeksforgeeks.org/how-to-map-data-into-components-using-reactjs/](https://www.geeksforgeeks.org/how-to-map-data-into-components-using-reactjs/) 将数据映射到组件中
...
### 示例
使用 `map` 函数将 `list.js` 文件中的数据映射到自定义 `Info` 组件。

### App.js
导入 `App.js` 文件中的用户数组和 `Info` 组件。使用映射功能将用户数组的每个元素映射到信息组件。
...
### 示例 2
不使用映射功能，将 `list.js` 文件中的数据映射到 `Info` 组件。
