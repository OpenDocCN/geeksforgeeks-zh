...
> 原文: [https://www.geeksforgeeks.org/how-to-publish-a-reactjs-component-to-npm/](https://www.geeksforgeeks.org/how-to-publish-a-reactjs-component-to-npm/)
...
使用终端 & 进入 `components` 文件夹，输入 `npm init`。您将在命令行中看到以下内容。
...
```
现在转到 `package.json` 文件 & 将依赖项更改为对等依赖项，并从 `components` 文件夹中删除 `node_modules` 文件夹。
