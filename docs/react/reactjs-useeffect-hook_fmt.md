...
引入 `useEffect` Hook 的动机是为了消除使用基于类的组件的副作用。例如，像更新 DOM、从 API 端点获取数据、设置订阅或计时器等任务可能会导致不必要的副作用。由于渲染方法会很快产生副作用，因此需要使用生命周期方法来观察副作用。
...
让我们通过设置计时器来考虑另一个副作用。在 `componentDidMount()` 方法中，我们设置了一个计时器，每 5 秒记录一个字符串“hello”。当组件从 DOM 中移除时，我们可以清除这个计时器。我们在 `componentWillUnmount()` 生命周期方法中做到了这一点。计时器的代码如下所示：
...
`useEffect` Hook 可以让你在函数组件中执行副作用。它是 `componentDidMount()` 方法、`componentDidUpdate()` 方法和 `componentWillUnmount()` 方法的近似替代。

## 渲染后的 useEffect

我们知道，`useEffect()` 用于在函数组件中引起副作用，它还能够将基于类的组件的 `componentDidMount()`、`componentDidUpdate()` 和 `componentWillUnmount()` 生命周期方法处理到函数组件中。让我们看一个例子，说明如何使用 `useEffect` Hook 作为一个函数，它可以模仿上面提到的生命周期方法，但是在函数组件中。
...
现在，让我们尝试用一个函数组件来替换这个功能。出于同样的目的，创建一个新文件并命名它。
...
当我们指定 `useEffect` 时，我们基本上是请求 React 执行我们在 `useEffect` 函数中作为参数传递的函数，每次组件呈现时。需要注意的第二件事是 `useEffect` 在组件内部使用，因为这样做我们可以轻松地访问组件状态和 props，而无需编写任何额外的代码。