...
使用[创建-反应-应用程序](https://www.geeksforgeeks.org/reactjs-setting-development-environment/)创建一个反应应用程序，让我们称之为**极客**。
...
反应路由器可以通过 [npm](https://www.geeksforgeeks.org/node-js-npm-node-package-manager/) 在您的反应应用程序中安装。按照下面给出的步骤在您的 React 应用程序中安装路由器:
...
*   **`Routes`**: 这是 v6 中引入的新组件，也是`<Switch>`组件的升级版。`Routes` 相对于 `Switch` 的主要优势是:
    *   相对 `<Route>`s 和`<Link>` s
    *   路由是根据最佳匹配选择的，而不是按顺序遍历的。
*   **`Route`**: `Route` 是有条件显示的组件，当其路径与当前 URL 匹配时，会呈现一些 UI。
*   **`Link`**: `Link` 组件用于创建不同路线的链接，并围绕应用程序实现导航。它的工作原理就像 HTML [锚定标签](https://www.geeksforgeeks.org/html-a-tag/)。
...
**注意:** `BrowserRouter` 别名为 `Router`。

## 使用 React Router

要使用 React Router，让我们首先在 React 应用程序中创建几个组件。在项目目录中，在 `src` 文件夹中创建一个名为**`components`**的文件夹，现在将名为 **`home.js`** 、 **`about.js`** 和 **`contact.js`** 的 3 个文件添加到 `components` 文件夹中。
...
*   **`BrowserRouter`:** 将别名为 `Router` 的 `BrowserRouter` 添加到 `app.js` 文件中，以便包装所有其他组件。`BrowserRouter` 是一个父组件，只能有一个子组件。
...
现在，在本地主机上运行您的应用程序，并单击您创建的链接。你会注意到 `Link` 组件的 **`to`** 道具中的值会改变网址。
...
*   **1. `exact`:** 用于将精确值与网址进行匹配。例如，精确 `path='/about'` 将仅在组件与路径完全匹配时才呈现组件，但是如果我们从语法中移除 `exact`，则即使结构类似 `/about/10`，用户界面仍将呈现。
*   **2. `path`:** `path` 指定我们分配给组件的路径名。
*   **3. `element`:** 它指的是将在匹配路径时呈现的组件。

**注意:** 默认情况下，路由是包含的，这意味着多个 `Route` 组件可以匹配 URL 路径并同时渲染。如果我们想要渲染单个组件，我们需要使用 **`Routes`**。
...
`Routes` 将几条路由组合在一起，遍历它们并找到与路径匹配的第一条路由。由此，呈现路径的对应组件。
