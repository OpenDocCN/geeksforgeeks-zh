...
> 原文：[https://www.geeksforgeeks.org/playing-with-destructors-in-c/](https://www.geeksforgeeks.org/playing-with-destructors-in-c/)
...
由于引用给出了变量的左值，通过使用引用返回，程序将输出“i = 10”。
...
函数 `foo()` 返回变量 `i` 的左值。因此，`i` 的地址将被复制到返回值中。因为引用会自动解引用，它将输出“i = 10”。
