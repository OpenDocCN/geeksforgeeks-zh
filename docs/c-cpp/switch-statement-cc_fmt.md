...
### 1) Break
此关键字用于停止开关块内部的执行。它有助于终止开关块并将其断开。

### 2) Default
这个关键字用于指定在没有大小写匹配的情况下要执行的语句集。

> **注意：** 有时候当 `default` 没有放在 `switch case` 程序末尾的时候，我们要用 `break` 语句搭配默认 case。

## 关于交换机案例陈述的要点

### 1) 表达式必须为常量值
开关中提供的表达式应得到一个**常量值**，否则无效。开关情况的一些有效表达式是，
...
### 3) 默认语句是可选的
`default` 语句是可选的。即使 `switch case` 语句没有 `default` 语句，它也能毫无问题地运行。

### 4) 使用 break 终止序列
开关内部使用 `break` 语句来终止语句序列。当到达 `break` 语句时，开关终止，控制流跳到开关语句之后的下一行。

### 5) break 语句是可选的
`break` 语句是可选的。如果省略，执行将继续到下一个案例。控制流将延续到后续案例，直到达到 `break`。

### 6) 允许 switch 语句嵌套
允许 `switch` 语句嵌套，也就是说你可以在另一个 `switch` 里面有 `switch` 语句。然而，应该避免嵌套的 `switch` 语句，因为它会使程序更加复杂，可读性更差。
...
*   [关于 C 中开关情况的有趣事实](https://www.geeksforgeeks.org/interesting-facts-about-switch-statement-in-c/)
