...
> 原文：[https://www.geeksforgeeks.org/variable-length-arguments-for-macros/](https://www.geeksforgeeks.org/variable-length-arguments-for-macros/)
...
让我们举个例子来看看。下面的宏采用可变长度参数，如`printf()`函数。此宏用于错误记录。宏打印文件名后跟着行号，最后打印信息/错误信息。第一个参数`prio`决定了消息的优先级，即它是信息消息还是错误，`stream`可能是`stdout`或`stderr`。它在`stdout`上显示`INFO`消息，在`stderr`流上显示`ERROR`消息。
