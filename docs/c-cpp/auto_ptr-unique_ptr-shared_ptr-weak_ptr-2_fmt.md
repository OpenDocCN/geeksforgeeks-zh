...
> 原文：[https://www.geeksforgeeks.org/auto_ptr-unique_ptr-shared_ptr-weak_ptr-2/](https://www.geeksforgeeks.org/auto_ptr-unique_ptr-shared_ptr-weak_ptr-2/)
...
`auto_ptr` 是一个智能指针，用于管理通过 `new` 表达式获得的对象，并在 `auto_ptr` 本身被破坏时删除该对象。
...
**`auto_ptr` 为什么被弃用？**
它以这样一种方式获得指针的所有权，即没有两个指针应该包含相同的对象。赋值转移所有权并将右值 `auto_ptr` 重置为空指针。因此，由于前面提到的无法复制，它们不能在 STL 容器中使用。
...
`weak_ptr` 被创建为 `shared_ptr` 的副本。它提供对由一个或多个 `shared_ptr` 实例拥有但不参与引用计数的对象的访问。`weak_ptr` 的存在或销毁对 `shared_ptr` 或其其他副本没有影响。在某些情况下，需要中断 `shared_ptr` 实例之间的循环引用。
...
**参考：**
[https://www.quora.com/When-should-I-use-shared_ptr-and-unique_ptr-in-C++-and-what-are-they-good-for](https://www.quora.com/When-should-I-use-shared_ptr-and-unique_ptr-in-C++-and-what-are-they-good-for)
