...
C/C++ 中的`strcspn()`函数将两个字符串作为输入，`string_1`和`string_2`作为参数，并通过遍历`string_2`中的任何字符来搜索`string_1`。如果在`string_1`中找不到`string_2`的任何字符，该函数将返回`string_1`的长度。该功能在`cstring`头文件中定义。
...
**返回值:** 该函数返回`string_1`中在`string_2`中匹配的任何字符之前遍历的字符数。
