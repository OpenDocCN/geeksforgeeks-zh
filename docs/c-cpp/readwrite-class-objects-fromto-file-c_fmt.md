...
> 原文: [https://www.geeksforgeeks.org/readwrite-class-objects-fromto-file-c/](https://www.geeksforgeeks.org/readwrite-class-objects-fromto-file-c/)
...
数据传输通常使用`>>`和`<<`运算符完成。但是，如果你有一个包含4个数据成员的类，并希望直接从其对象将所有4个数据成员写入文件，或反之亦然，我们可以使用以下语法：

要将对象的数据成员写入文件：
// 这里`file_obj`是`ofstream`的一个对象
`file_obj.write((char *) & class_obj, sizeof(class_obj));`

要将文件的数据成员读入对象：
// 这里`file_obj`是`ifstream`的一个对象
