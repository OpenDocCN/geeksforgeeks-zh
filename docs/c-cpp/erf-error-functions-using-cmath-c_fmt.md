...
> 原文: [https://www.geeksforgeeks.org/erf-error-functions-using-cmath-c/](https://www.geeksforgeeks.org/erf-error-functions-using-cmath-c/)
...
该内置函数计算 x 输入值的误差函数，参数 x 可以是`int`或`float`或`double`。它返回一个双精度值，即`erf(x)`。

## erfc(x)

此内置函数计算 x 输入值的误差函数的余数，参数 x 可以是`int`或`float`或`double`。它返回一个`1 - erf(x)`的双精度值。
