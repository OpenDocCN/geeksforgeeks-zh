# C++ STL 中的无序多集 `load_factor()` 函数

> 原文: [https://www.geeksforgeeks.org/unordered_multiset-load_factor-function-in-c-stl/](https://www.geeksforgeeks.org/unordered_multiset-load_factor-function-in-c-stl/)

`unordered_multiset::load_factor()` 是 C++ STL 中的内置函数，返回无序多集容器中的当前加载因子。负载系数是容器中元素的数量（其大小）和桶的数量（`bucket_count`）之间的比率：
...
以下程序说明了 `unordered_multiset::load_factor()` 功能：
