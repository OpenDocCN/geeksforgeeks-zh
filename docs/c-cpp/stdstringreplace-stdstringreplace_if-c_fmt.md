...
它将等于`old_value`的范围`[first, last]`中的每个元素替换为`new_value`。该函数使用运算符`==`将各个元素与`old_value`进行比较。
...
`replace_if()`用`new_value`替换一元谓词(即`op(elem)`)为其生成`true`的范围`[first, last]`中的每个元素。
...
**相关文章:**[`STD::string::replace_copy()`，`STD::string::replace_copy_if()`](https://www.geeksforgeeks.org/stdstringreplace_copy-stdstringreplace_copy_if-cpp/)
