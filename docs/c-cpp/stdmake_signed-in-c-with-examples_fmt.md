# std::make_signed C++ 示例

> 原文: [https://www.geeksforgeeks.org/stdmake_signed-in-c-with-examples/](https://www.geeksforgeeks.org/stdmake_signed-in-c-with-examples/)

[C++ STL](https://www.geeksforgeeks.org/the-c-standard-template-library-stl/) 的 `std::make_signed` 模板存在于 `<type_traits>` 头文件中。C++ STL 的 `std::make_signed` 模板用于通过保留任何 cv 限定符来获取 `T` (特质类)对应的有符号类型。可以使用 `std::is_same` 功能检查给定类型 `T` 是否为有符号类型。
...
模板 `std::make_signed` 接受单个参数 `T` (特质类)并将类型 `T` 作为有符号类型。
...
[http://www.cplusplus.com/reference/type_traits/make_signed/](http://www.cplusplus.com/reference/type_traits/make_signed/)