...
递归函数只是一种循环结构。不同的是，它自己维护一个[内存堆栈](https://www.geeksforgeeks.org/memory-layout-of-c-program/)。显然，它必须有一个像[循环](https://www.geeksforgeeks.org/loops-in-c-and-cpp/)一样的[终止条件](https://www.geeksforgeeks.org/break-statement-cc/)。因此，递归函数具有以下结构：
...
**解释:** 在这里，函数如何识别变量 `n`，即使它不是作为参数传递的。带有空 capture 子句 `[ ]` 的 lambda 只能访问其本地的变量。这里，使用了捕获子句 `[ & ]`，这允许函数访问变量 `n`(变量 `n` 的实际值正在被改变)。有两种方法可以解决上面的错误：
...
**解释:** 在阶乘函数中，使用 `[ & ]` capture 子句直接访问 `n`。在 `factorial2` 函数中，`n` 作为参数传递。因此，不需要 `[ & ]` 子句。
