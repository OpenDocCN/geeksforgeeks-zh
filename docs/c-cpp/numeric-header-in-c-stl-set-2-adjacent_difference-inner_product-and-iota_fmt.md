# C++ STL 中的数值头文件 | 集合 2 (`adjacent_difference()`、`inner_product()` 和 `iota()`)

> 原文: [https://www.geeksforgeeks.org/numeric-header-in-c-stl-set-2-adjacent_difference-inner_product-and-iota/](https://www.geeksforgeeks.org/numeric-header-in-c-stl-set-2-adjacent_difference-inner_product-and-iota/)

[C++ STL 中的数字表头文件 | 集合 1](https://www.geeksforgeeks.org/numeric-header-in-c-stl-set-1-accumulate-and-partial_sum/)

## `adjacent_difference`

这个函数将一个数组的对应元素之间的差异分配给另一个数组。它返回位于 `[first, last]` 之间的所有值集的相邻差值。

例如: 如果 `a[]` 代表提供的范围 `[first, last]` 中的元素，`b[]` 代表结果。
...
这个函数返回 `var` 与从 `first1` 和 `first2` 开始的两个范围的元素形成的对的内积相加的结果。
...
该函数为数组范围 `[first, last]` 中的元素赋值，该值在每一步递增 `val++`。
