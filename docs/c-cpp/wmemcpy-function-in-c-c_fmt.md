...
`wmemcpy()` 函数在头文件`cwcchar . h`中指定，并将指定数量的字符从一个字符串复制到另一个字符串。该函数不会检查第一个名为`source`的字符串中是否有任何终止的空宽字符，它总是精确地将 `n` 个字符复制到第二个名为`destination`的字符串中。
