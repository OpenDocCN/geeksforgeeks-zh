...
> 原文：[https://www.geeksforgeeks.org/finding-median-of-unsorted-array-in-linear-time-using-c-stl/](https://www.geeksforgeeks.org/finding-median-of-unsorted-array-in-linear-time-using-c-stl/)
...
## 方法
思路是在 [C++ STL](https://www.geeksforgeeks.org/the-c-standard-template-library-stl/) 中使用 [`nth_element()`](https://www.geeksforgeeks.org/stdnth_element-in-cpp/) 函数。

1.  如果数组中的元素数量是奇数，则使用如下所示的 `nth_element()` 函数找到第 `(N/2)` 个元素，然后索引 `(N/2)` 处的值是给定数组的中值。

    ```
    nth_element(arr.begin(), arr.begin() + N/2, arr.end())
    ```

2.  否则，使用如下所示的 `nth_element()` 函数找到第 `(N/2)` 个和 `((N–1)/2)` 个元素，并找到索引 `(N/2)` 和 `((N–1)/2)` 处值的平均值，即给定数组的中值。
