...
> 原文: [https://www.geeksforgeeks.org/program-to-design-a-brick-game-using-computer-graphics/](https://www.geeksforgeeks.org/program-to-design-a-brick-game-using-computer-graphics/)
...
*   [`setfillstyle(pattern, color)`](https://www.geeksforgeeks.org/setfillstyle-floodfill-c/): `graphics.h` 头文件中的一个函数，通过它可以给出一个绘图的图案和一种特定的颜色。
*   [`floodfill(a, b, c)`](https://www.geeksforgeeks.org/setfillstyle-floodfill-c/): `graphics.h` 头文件中的一个函数，通过它可以给以(`a`, `b`)为中心、`c` 为边框颜色的特定有界区域着色。
...
*   [`settextstyle(int font, int direction, int font_size)`](https://www.geeksforgeeks.org/settextstyle-function-c/): `graphics.h` 头文件中的一个函数，可用于可打印文本的样式，其中 `font` 参数指定了文本的字体。方向可以是水平方向(从左到右)或垂直方向(从下到上)。
