...
## 语法 2
从索引 `str_idx` 开始最多分配字符串的 `str_num` 个字符。如果 `str_idx > str.size()`，则抛出范围错误。
...
## 语法 3
指定 C 字符串 `cstr` 的字符。如果结果大小超过最大字符数，它将引发 `length_error`。
...
## 语法 4
分配字符数组字符的 `chars_len` 字符。如果结果大小超过最大字符数，它将引发 `length_error`。
...
## 语法 5
指定字符 `c` 出现的次数。如果次数等于 `string::npos`，将引发 `length_error`。
...
## 语法 6
分配范围内的所有字符 `[first, last)`。如果范围超出字符串的实际内容，它将抛出 `length_error`。
