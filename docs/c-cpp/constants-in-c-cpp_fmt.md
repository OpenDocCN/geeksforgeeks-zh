...
分配给每个常量变量的值被称为*文字*。通常，术语、常数和文字可以互换使用。例如，`const int = 5;` 是一个常量表达式，值 5 被称为常量整数文字。
...
该指令用于声明现有变量或任何值的别名。我们可以用它来声明一个常数，如下所示:
...
*   `value`: 指分配给 `identifierName` 的任何值。

### 使用 `const` 关键字

使用 `const` 关键字定义常量就像定义变量一样简单，不同的是您必须在定义之前使用 `const` 关键字。

![](img/999a1d61113f5802085f51d7646b70a0.png)
