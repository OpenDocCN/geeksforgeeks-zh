...
配对容器是在`<utility>`头中定义的简单容器，由两个数据元素或对象组成。

*   第一个元素被称为`first`，第二个元素被称为`second`，顺序是固定的（`first`，`second`）。
*   `Pair`用于将两个类型可能不同的值组合在一起。`Pair`提供了一种将两个异构对象存储为一个单元的方法。
*   配对可以被分配、复制和比较。默认情况下，映射或`hash_map`中分配的对象数组属于“对”类型，其中所有`first`元素都是与其`second`值对象相关联的唯一键。
