...
**(B)** GeeksQuiz 将被打印 2147483647 次，即 (2 的 31 次方)–1。
**(C)** 它将无限次打印 GeeksQuiz，即程序将永远继续运行，直到它被其他方式终止，如 CTRL+C 或 CTRL+Z 等。
**(D)** GeeksQuiz 只会打印一次。因为当 `main()` 在 `main()` 内部使用时，编译器会在运行时忽略它。这是为了确保 `main()` 只被调用一次。
**(E)** GeeksQuiz 将被打印，直到该程序发生堆栈溢出。
