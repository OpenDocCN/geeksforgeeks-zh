...
先决条件:[设置在 C++ ](https://www.geeksforgeeks.org/set-in-cpp-stl/) ，[无序设置在 C++ ](https://www.geeksforgeeks.org/unorderd_set-stl-uses/)
...
*   我们必须打印/访问数据(按排序顺序)。
*   我们需要元素的前身/继承者。
*   由于集合是有序的，我们可以在集合元素上使用类似 [`binary_search()`，`lower_bound()`和`upper_bound()`](https://www.geeksforgeeks.org/binary-search-functions-in-c-stl-binary_search-lower_bound-and-upper_bound/)这样的函数。这些函数不能用于`unordered_set()`。
*   更多情况参见[BST 相对 Hash table 的优势](https://www.geeksforgeeks.org/advantages-of-bst-over-hash-table/)。
...
如果要看 c++ STL 中 `set` 和`unordered_set`的实现细节，参见 [Set Vs Map](https://www.geeksforgeeks.org/set-vs-map-c-stl/) 。`Set`允许按排序顺序遍历元素，而`Unordered_set`不允许按排序顺序遍历元素。
...
在`set`中，可以修改集合来查找前任或继任者，而`unordered_set`不允许查找前任/继任者。
