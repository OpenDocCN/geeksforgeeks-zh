...
> 原文: [https://www.geeksforgeeks.org/list-in-c-set-2-some-useful-functions/](https://www.geeksforgeeks.org/list-in-c-set-2-some-useful-functions/)
...
该功能在列表的末尾添加`value`。它与`push_back()`的不同之处在于，它直接在位置创建元素，而`push_back()`首先创建一个临时副本并从那里复制。在大多数情况下，`emplace_back()`在实现上比`push_back()`更快。

## 3. `emplace_front(value)`
该功能在列表的开始处增加`value`。它与`push_front()`的不同之处在于，它直接在位置创建元素，而`push_front()`首先创建一个临时副本并从那里复制。在大多数情况下，`emplace_front()`在实现上比`push_front()`更快。
...
## 4. `merge(list2)`
该功能用于合并`list2`和`list1`。如果两个列表都是按排序顺序排列的，那么结果列表也是按排序的。

## 5. `remove_if(condition)`
该功能根据其参数中给出的条件从列表中删除元素。
...
## 6. `unique()`
该功能用于删除重复出现的号。列表必须经过排序才能执行该功能。

## 7. `splice(position, list2)`
该功能用于将元素从一个列表转移到另一个列表。
