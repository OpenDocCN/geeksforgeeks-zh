...
**无序映射:** 无序映射是存储键值和映射值组合而成的元素的关联容器。键值用于唯一标识元素，映射值是与键相关联的内容。键和值都可以是预定义或用户定义的任何类型。

**`unordered_map::at()`:** 这个函数在 C++ 中为 `unordered_map` 返回对以元素为键 `k` 的值的引用。
...
## 在 unordered_map 中 at() 与运算符[]有何不同

`at()` 和运算符 `[]` 都用于引用给定位置处存在的元素，唯一的区别是，`at()` 抛出超范围异常，而运算符 `[]` 显示**未定义的行为**。即如果运算符 `[]` 用于查找键对应的值，并且如果键不存在于无序映射中，它将首先将键插入映射中，然后为该键分配默认值 `0`。