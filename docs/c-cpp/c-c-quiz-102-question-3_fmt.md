...
假设一个 C 程序有一个浮点常量 1.414，把它转换为`float`数据类型的最好方法是什么？
...
**说明:**默认浮动常量为`double`数据类型。通过在它后面加上`F`或`F`，可以将其转换为`float`数据类型。更多详情，可参考本帖[此处](https://www.geeksforgeeks.org/sizeof-floating-constant-in-c/)。
