...
**差异** :
`set` 的差异仅用于存储键，而 `map` 用于存储键值对。例如，在[打印排序的不同元素](https://www.geeksforgeeks.org/print-sorted-distinct-elements-array-c/)的问题中，我们使用 `set`，因为某个键需要值。如果我们改变问题来打印不同排序元素的频率，我们使用 `map`。我们需要映射来将数组值存储为键，将频率存储为值。
