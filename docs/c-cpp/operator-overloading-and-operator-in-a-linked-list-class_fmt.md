...
- [C++ 中的链表](https://www.geeksforgeeks.org/linked-list-set-1-introduction/)
...
我们可以重载 `>>` 和 `<<` 操作符，在[链表](https://www.geeksforgeeks.org/data-structures/linked-list/)中进行输入，在 C++ 中打印[链表](https://www.geeksforgeeks.org/data-structures/linked-list/)中的元素。它能够为操作符提供一种特殊的数据类型含义，这种能力被称为[操作符重载](https://www.geeksforgeeks.org/operator-overloading-c/)。
...
**解释：**
上述函数的返回类型是对 `istream` 对象的引用。在声明 `cin >> head;` 中，`cin` 是第一个参数，对 `head` 的引用是函数的第二个参数。执行任何此类语句时，都会调用上述函数。
...
**说明：**
上述函数的返回类型是对 `ostream` 对象的引用。在声明 `cout << head;` 中，`cout` 是第一个参数，对 `head` 的引用是函数的第二个参数。执行任何此类语句时，都会调用上述函数。

## 完整重载代码

下面是 `>>` 和 `<<` 运算符重载的代码，连续以一个数字 `N` 作为输入，在链表中插入数字 `N`，直到 `N = -1`。
