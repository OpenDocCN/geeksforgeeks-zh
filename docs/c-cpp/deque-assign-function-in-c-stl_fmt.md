...
> 原文: [https://www.geeksforgeeks.org/deque-assign-function-in-c-stl/](https://www.geeksforgeeks.org/deque-assign-function-in-c-stl/)
...
*   `iterator1`: 它指定迭代器，该迭代器指向容器（`deque`，`array`，…）的起始元素，该容器的元素将被转移到 `deque1`。
*   `iterator2`: 它指定指向容器（`deque`，`array`，…）的最后一个元素的迭代器，该容器的元素将被转移到 `deque1`。
