...
C/C++ 中的 `wcstoimax()` 和 `wcstoumax()` 函数的工作原理与 C++ 中的 [strtoimax()](https://www.geeksforgeeks.org/strtoimax-function-in-c/) 和 [strtoumax()](https://www.geeksforgeeks.org/strtoumax-function-in-c/) 函数完全相同，但用于将宽字符串(`wstring`) 的内容转换为指定基数的整数。该功能在 `cinttypes` 头文件中定义。
