...
> 原文：`https://www.geeksforgeeks.org/set-get_allocator-in-c-stl/`

`C++ STL` 中的 `set::get_allocator()` 是一个内置函数，它返回与集合相关联的分配器对象的副本。
...
下面是说明 `set::get_allocator()` 方法的示例：

### 示例 1

下面的程序展示了如何使用集合的分配器来分配 7 个元素的数组。
