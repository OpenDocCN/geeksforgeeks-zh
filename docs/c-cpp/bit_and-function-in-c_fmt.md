...
1.  这个类的对象可以在标准算法上使用，如 `transform` 或 `accumulate`。
2.  成员函数（`operator()`）返回其参数的 `位与`。

我们必须包含库 `<functional>` 和 `<algorithm>` 来分别使用 `bit_and` 和 `transform`，否则它们将不起作用。

下面是显示 `bit_and` 功能工作的程序：
