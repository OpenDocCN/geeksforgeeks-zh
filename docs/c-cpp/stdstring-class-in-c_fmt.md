...
在其定义中，C++ 有一种方式将字符序列表示为类的对象。这个类叫做 `std::string`。`String` 类将字符存储为字节序列，其功能是允许访问单字节字符。

## std::string 与字符数组

*   字符数组只是字符的数组，可以以空字符结束。字符串是一个类，它定义了表示为字符流的对象。
*   字符数组的大小必须静态分配，如果需要，在运行时不能分配更多的内存。在字符数组的情况下，未使用的已分配内存被浪费。如果是字符串，内存是动态分配的。可以在运行时按需分配更多内存。由于没有预分配内存，没有浪费内存。
*   存在数组衰减的风险以防字符数组。由于字符串被表示为对象，不会发生数组衰减。
*   字符数组的实现比 `std::string` 更快。`std::string` 在实现上比字符数组慢。
*   字符数组不提供太多内置功能来操纵字符串。字符串类定义了多个允许对字符串进行多种操作的功能。
...
**2. `push_back()`**：该功能用于在字符串的末端输入一个字符。
**3. `pop_back()`**：从 C++ 11 引入（针对字符串），此功能用于删除字符串中最后一个字符。
...
**4. `capacity()`**：该功能返回分配给字符串的容量，该容量可以是等于或大于字符串大小的。分配额外的空间，以便当新字符被添加到字符串中时，操作可以有效地完成。
**5. `resize()`**：此功能改变字符串的大小，大小可以增减。
**6. `length()`**：此函数求字符串的长度。
**7. `shrink_to_fit()`**：此函数减小字符串的容量，使其等于字符串的最小容量。如果我们确定不需要进一步添加字符，该操作对于节省额外的内存是有用的。
...
**9. `end()`**：该函数返回一个迭代器到字符串的末尾。
**10. `rbegin()`**：这个函数返回一个反向迭代器指向字符串的端。
**11. `rend()`**：这个函数返回一个反向迭代器，指向字符串的开头。
...
**12. `copy("char 数组", len, pos)`**：此函数复制其参数中提到的目标字符数组中的子串。需要 3 个参数，目标字符数组，需要复制的长度和字符串中的起始位置才能开始复制。
**13. `swap()`**：此功能将一个字符串与另一个字符串交换。
