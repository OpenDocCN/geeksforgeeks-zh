...
C/C++ 中的`strtoul()`函数，该函数根据给定的基数将`str`中字符串的初始部分转换为无符号长 int 值，该值必须介于 2 和 36 之间(含 2 和 36)，或者是特殊值 0。该函数丢弃任何空白字符，直到找到第一个非空白字符，然后尽可能多的字符形成一个有效的基数为 n 的无符号整数表示，并将它们转换为整数值。
