...
1.  `double sin(double)`：该函数以角度（以度为单位）为自变量，返回其正弦值，可以使用正弦曲线进行验证。
2.  `double cos(double)`：该函数以角度（以度为单位）为自变量，返回其余弦值，可以使用余弦曲线进行验证。
3.  `double tan(double)`：此函数以角度（以度为单位）为自变量，返回其正切值。这也可以用三角法来验证，如 `Tan(x) = Sin(x)/Cos(x)`。
4.  `double sqrt(double)`：此函数以数字为自变量，返回其平方根值。数字不能为负值。
5.  `int abs(int)`：此函数以整数为自变量，返回其绝对值。这意味着，不管输入的符号是什么，输出总是正的。
6.  `double pow(double, double)`：此函数以一个自变量为基数，另一个为指数。
7.  `double hypot(double, double)`：该功能要求直角三角形的两条边作为其斜边给出输出。
8.  `double floor(double)`：此函数返回小于或等于函数中传递的参数的整数值。
9.  `double fabs(double)`：此函数返回任意数字的绝对值。
...
17. `double log(double)`：该函数取一个数，返回该数的自然对数。
