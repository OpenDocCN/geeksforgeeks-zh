...
给定两个基数和指数，`pow()`函数找到 `x` 的 `y` 次方，即 x <sup>y</sup> 。基本上在 C 中，指数值是用`pow()`函数计算的。`pow()`是获取一个数的幂的函数，但是我们必须在 c/c++ 中使用`#include <math.h>`才能使用那个`pow()`函数。然后传递两个数字。示例–`pow(4, 2)`；然后我们将得到 4^2 的结果，这是 16。
...
`pow()`函数将`double`作为参数，并返回一个`double`值。这个函数并不总是适用于整数。一个这样例子是`pow(5, 2)`。当分配给一个整数时，它在一些编译器上输出 24，在其他一些编译器上运行良好。但是`pow(5, 2)`没有任何分配给整数输出 25。

*   这是因为 5 <sup>2</sup> (即 25)可能存储为 24.99999999 或 25.00000000001，因为返回类型是`double`的。当赋给`int`时，25.00000000001 变成 25，但 24.9999999 将给出输出 24。
