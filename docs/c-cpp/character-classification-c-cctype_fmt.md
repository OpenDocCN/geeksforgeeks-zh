...
> 原文：[https://www.geeksforgeeks.org/character-classification-c-cctype/](https://www.geeksforgeeks.org/character-classification-c-cctype/)

使用 `<cctype>` 函数库中指定的函数，C++ 中的字符分类是可能的。本文讨论了字符分类的众多功能。

## 1. `isalpha()`
如果字符是字母表则此函数返回 `true`，否则返回 `false`。`a-z` 和 `A-Z` 中的所有字符根据该功能返回真。

## 2. `isalnum()`
如果字符是字母或数字则此函数返回 `true`，否则返回 `false`。`a-z`、`A-Z` 的所有字符和 `0-9` 的所有数字根据此功能返回真。

## 3. `isdigit()`
如果字符是数字则此函数返回 `true`，否则返回 `false`。所有 `0-9` 的数字按此功能返回真。

```cpp
// C++ program to demonstrate isalpha(), isalnum()
// and isdigit()
#include <cctype>
#include <iostream>
using namespace std;
...
## 4. `isblank()`
如果字符是空格或制表符则该函数返回 `true`，否则返回 `false`。

## 5. `isspace()`
如果字符是空格、制表符或空白控制码（例如，`\n`，`\r`）则该函数返回 `true`，否则返回 `false`。

## 6. `iscntrl()`
如果字符是 tab 或任何控制代码则此函数返回 `true`，否则返回 `false`。
...
## 7. `isprint()`
如果字符可在控制台上打印，即除控制代码以外的所有字符，则该函数返回 `true`，否则返回 `false`。

## 8. `isxdigit()`
如果字符是十六进制的，即 `0-9` 和 `a-f` / `A-F`，则此函数返回 `true`，否则返回 `false`。

## 9. `ispunct()`
如果字符是标点符号则此函数返回 `true`，否则返回 `false`。
...
**参考：**[http://www.cplusplus.com/reference/cctype/](http://www.cplusplus.com/reference/cctype/)
