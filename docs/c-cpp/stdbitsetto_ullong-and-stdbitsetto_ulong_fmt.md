# C++ STL 中的 std::bitset::to_ullong 和 std::bitset::to_ulong

> 原文: [https://www.geeksforgeeks.org/stdbitsetto_ullong-and-stdbitsetto_ulong/](https://www.geeksforgeeks.org/stdbitsetto_ullong-and-stdbitsetto_ulong/)

## 位集

位集是 `bool` 的数组，但每个布尔值并不单独存储，而是位集优化了空间，使得每个 `bool` 只占用 1 位空间，因此位集 `bs` 占用的空间小于 `bool bs[N]` 和向量 `bs(N)` 占用的空间。然而，位集的一个限制是，`N` 在编译时必须是已知的，即常数（向量和动态数组没有这个限制）。
