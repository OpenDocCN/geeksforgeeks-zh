...
* [`lower_bound`](https://www.geeksforgeeks.org/lower_bound-in-cpp/) 也是一个选项，它花费 `log(n)` 的时间。它返回一个迭代器，指向第一个不小于 `k` 的元素。但由于它返回一个迭代器，我们永远无法知道索引或小于 `k` 的元素数量。
