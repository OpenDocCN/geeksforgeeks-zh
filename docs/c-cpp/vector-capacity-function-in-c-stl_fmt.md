...
`vector::capacity()` 函数是一个内置函数，它返回当前为向量分配的存储空间的大小，用元素表示。这个容量不一定等于向量大小。它可以等于或大于，额外的空间可以适应增长，而无需在每次插入时重新分配。容量并不限制向量的大小。当容量耗尽并且需要更多容量时，它会被容器自动扩展(重新分配存储空间)。向量大小的理论极限由成员 `max_size` 给出。
