# 在 C++ STL 中 `accumulate()` 和 `partial_sum()`: 数值头

> 原文: [https://www.geeksforgeeks.org/accumulate-and-partial_sum-in-c-stl-numeric-header/](https://www.geeksforgeeks.org/accumulate-and-partial_sum-in-c-stl-numeric-header/)
...
该函数用变量 `sum` 返回位于 `[first, last]` 之间的所有值的总和。
...
该函数返回变量 `sum` 位于 `[first, last]` 之间的所有值的总和。
...
该函数将数组中相应元素的部分和赋给第二个数组的每个位置。它返回位于 `[first, last]` 之间的所有值的部分和，并将其存储在另一个数组 `b` 中。
例如，如果 `x` 表示 `[first, last]` 中的元素，`y` 表示结果中的元素，则 `y` 可以计算为:
