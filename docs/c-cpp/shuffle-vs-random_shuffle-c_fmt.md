...
int main()
{
    ...
    // set some values:
    for (int j = 1; j < 10; ++j)
        arr.push_back(j); // 1 2 3 4 5 6 7 8 9
...
使用 `g` 作为统一随机数生成器，随机重新排列范围 `[first, last]` 中的元素。
...
## shuffle 和 random_shuffle 在 C++ 中的区别
...
本文由 [Shambhavi Singh](https://www.facebook.com/shambhavi.singh.1217) 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 [contribute.geeksforgeeks.org](http://www.contribute.geeksforgeeks.org) 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
