...
> 原文: [https://www.geeksforgeeks.org/set-of-tuples-in-c-with-examples/](https://www.geeksforgeeks.org/set-of-tuples-in-c-with-examples/)

## 什么是元组？

一个[元组](https://www.geeksforgeeks.org/tuples-in-c/#:~:text=A%20tuple%20is%20an%20object,which%20they%20will%20be%20accessed.)是一个可以容纳多个元素的对象。元素可以是不同的数据类型。元组的元素按照被访问的顺序被初始化为参数。
...
[集合](https://www.geeksforgeeks.org/set-in-cpp-stl/)是一种关联的容器，其中每个元素必须是唯一的，因为元素的值标识了它。元素的值一旦添加到集合中就不能修改，尽管可以移除和添加该元素的修改值。
...
3.  [`size()`](https://www.geeksforgeeks.org/setsize-c-stl/): 返回集合中元素的个数。
4.  [`max_size()`](https://www.geeksforgeeks.org/set-max_size-function-in-c-stl/): 返回集合可以容纳的最大元素数。
5.  [`empty()`](https://www.geeksforgeeks.org/setempty-c-stl/): 返回集合是否为空。
