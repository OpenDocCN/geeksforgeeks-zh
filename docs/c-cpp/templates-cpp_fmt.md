...
C++ 新增两个关键词支持模板：`template` 和 `typename`。第二个关键字总是可以被关键字 `class` 替换。
...
了解更多关于 C++ 中[泛型的信息](https://www.geeksforgeeks.org/generics-in-c/)。
...
和函数模板一样，类模板在类定义独立于数据类型的东西时很有用。对于链接列表、二进制树、堆栈、队列、数组等类非常有用。
...
你可能还想参加一个关于模板的[测验](https://www.geeksforgeeks.org/c-plus-plus-gq/templates-gq/)。

Java 也支持这些功能。Java 称之为[泛型](https://www.geeksforgeeks.org/generics-in-java/)。
