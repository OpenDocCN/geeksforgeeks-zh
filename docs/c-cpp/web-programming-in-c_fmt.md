...
在使用 CGI 编程之前，程序员应该确保 Web 服务器支持 CGI，并为处理 CGI 程序进行了良好的配置。按照惯例，CGI 文件的扩展名为 `.cgi`，尽管它们是 C++ 可执行文件。默认情况下，Apache 网络服务器被配置为在 `/var/www/cgi-bin` 中运行 CGI 程序。
...
*   这个文件需要被保存在 `/var/www/cgi-bin` 目录中。

它包含以下内容。

*   CGI 程序中经常使用的其他一些 HTTP 头有:
    1.  `Content-Type`: 定义返回文件格式的 MIME 字符串。
    2.  `Expires: Date`: 定义当前网页信息失效的日期。
    3.  `Location: URL`: 必须返回的网址，而不是请求的网址。
    4.  `Last-Modified: Date`: 资源上次修改的日期。
    5.  `Content-Length: N`: 返回数据的长度，以字节为单位。浏览器使用该值 `N` 来报告估计的下载时间。
    6.  `Set-Cookie: String`: 用于设置通过字符串传递的 Cookie。
