...
1.  使用 `emplace_back` 功能向矢量添加元素。
2.  检查向量的大小是否为 0，如果不是，递增初始化为 0 的计数器变量，并 `pop_back` 元素。
3.  重复此步骤，直到向量的大小变为 0。
4.  打印变量的最终值。
...
2.  **`emplace_back()`** 就地构造一个字符串，因此不会创建临时字符串，而是直接用 `char*` 参数调用 `emplace_back()`。然后，它将创建一个字符串存储在用这个 `char*` 初始化的向量中。因此，在这种情况下，我们避免构造和销毁不必要的临时字符串对象。

详见 C++ STL 中[emplace 与 insert。](https://www.geeksforgeeks.org/emplace-vs-insert-c-stl/)
