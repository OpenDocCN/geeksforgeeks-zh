...
> 原文：[https://www.geeksforgeeks.org/opencv-understanding-brightness-in-an-image/](https://www.geeksforgeeks.org/opencv-understanding-brightness-in-an-image/)

在 OpenCV 中，改变图像的亮度是一项非常基础的任务。通过改变图像亮度，意味着改变每个图像像素的值。这种变化可以通过增加或减少图像的像素值来实现，增加或减少任何常数。
...
所以，假设我们希望将图像亮度增加 `60`。然后，我们为每个图像像素值添加 `60`，但严格禁止超出像素范围，即允许的最大限制是 `255`，我们的像素值不应增加超过 `255`。假设如果在任何情况下，该图像像素值超过 `255`，那么应该只给它最大允许值，而不是新增加的值。
...
这里，我们将像素值降低 `20`，并从输入图像中的每个图像像素中减去 `20`，但唯一重要的是任何像素的值都不应低于最小允许像素值。
