...
> 原文: [https://www.geeksforgeeks.org/complex-declarations-in-c/](https://www.geeksforgeeks.org/complex-declarations-in-c/)
...
含义: `f` 是一个 10 个指针的数组，指向返回 `void` 的函数(它接受 `int` 类型的 2 个参数)
...
后缀: `arr[5] * () * () * int`
含义: `arr` 是 5 个指向函数的指针的数组，该函数返回指向函数的指针，该函数返回指向整数的指针
...
## 示例 8：`void (*bsd_signal(int sig, void (*func)(int)))(int)`
...
含义: `bsd_signal` 是一个函数，它取整数 `sig` 及作为函数的指针 `func`(它取整数作为参数并返回 `void`)并返回函数的指针(它取整数作为参数并返回 `void`)
