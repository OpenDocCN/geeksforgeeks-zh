...
> 原文：[https://www.geeksforgeeks.org/searching-map-using-stdmap-functions-c/](https://www.geeksforgeeks.org/searching-map-using-stdmap-functions-c/)
...
还有一个在地图中搜索的功能，它**返回包含搜索关键字**的范围。由于映射包含唯一元素，返回范围最多包含 1 个元素。此函数**返回一个对的迭代器，其第一个元素指向搜索到的键对的`lower_bound()`，第二个元素指向搜索到的键的`upper_bound()`。如果键不存在，则第一个元素和第二个元素都指向下一个更大的元素。**
