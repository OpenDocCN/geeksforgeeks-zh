...
## 与 `sort()` 相比，什么时候更喜欢 `stable_sort`？

有时我们希望确保排序数组中相等元素的顺序与原始数组中相同。如果这些值与其他字段相关联，这将非常有用。例如，考虑按分数对学生进行排序，如果两个学生有相同的分数，我们可能希望保持他们出现在输入中的顺序相同。详见排序算法中的[稳定性](https://www.geeksforgeeks.org/stability-in-sorting-algorithms/)。

考虑以下例子。我们有按结束时间排序的时间间隔，我们想按开始时间排序。此外，如果两个开始时间相同，我们希望保持它们按结束时间排序。`sort()` 不能保证这一点。
...
对于 `stable_sort()` 它是 `O(n*log^2(n))` 如果没有与长度成线性比例的额外内存可用。如果可用，则为 `O(n * log(n))`。