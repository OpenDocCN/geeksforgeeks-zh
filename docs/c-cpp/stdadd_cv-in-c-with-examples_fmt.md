...
[C++ STL](https://www.geeksforgeeks.org/the-c-standard-template-library-stl/) 的 `std::add_cv` 模板存在于 `<type_traits>` 头文件中。C++ STL 的 `std::add_cv` 模板用于获取具有常量和变量资格的类型 `T`。功能 `std::is_same::value` 用于检查 `T` 是否为常量和易失性资质。
...
模板 `std::add_cv` 接受单个参数 `T`(性状类)检查 `T` 是否常量和挥发合格。
...
[http://www.cplusplus.com/reference/type_traits/add_cv/](http://www.cplusplus.com/reference/type_traits/add_cv/)