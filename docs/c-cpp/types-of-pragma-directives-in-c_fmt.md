...
> 原文：[https://www.geeksforgeeks.org/types-of-pragma-directives-in-c/](https://www.geeksforgeeks.org/types-of-pragma-directives-in-c/)
...
`pragma COPYRIGHT` 的**语法**为：

> `#pragma COPYRIGHT "字符串"`

*   这里，字符串指定了包含在目标文件的版权消息中的字符集。
*   如果在 `pragma COPYRIGHT` 期间未指定任何数据，则在版权消息中使用当前年份。
...
`pragma COPYRIGHT_DATE` 的**语法**为：

> `#pragma COPYRIGHT_DATE "string"`

在这里，字符串是一个将被 `COPYRIGHT` `pragma` 使用的日期。
考虑下面给出的例子：
...
像 `OPTIMIZE` `pragma` 一样，即使是 `OPT_LEVEL` 也不能在函数中使用。最后，`OPT_LEVEL` 3 和 4 只允许出现在文件的开头。

下面是 C 语言中的一个示例代码片段，演示了 `pragma OPT_LEVEL` 的使用：
...
*   `pragma OPTIMIZE` 基本上是用来打开/关闭源程序部分的优化。
*   但是，当使用 `pragma` 时，请在 `aCC` 命令上指定一个优化选项（同时给出编译程序的命令），否则，该 `pragma` 将被忽略。
*   另外，请记住，`pragma OPTIMIZE` 不能在函数中使用。

下面是 C 语言中的一个示例代码片段，演示了 `OPTIMIZE` 实用程序的使用：
...
用于创建不同版本共享库例程的 `HP_SHLIB_VERSION` 的语法是：

> `#pragma HP_SHLIB_VERSION ["]日期["]`
...
用于指定与写入可重定位对象模块的代码相关联的名称的 `pragma LOCATION` 的**语法**是：

> `#pragma LOCATION "string"`

**示例：**

> `#pragma LOCATION "极客为极客"`

*   在上面的例子中，字符串（“极客的极客”）指定了一个用于编码子空间的名称。
*   在此指令之后，该指令后面的所有代码都与字符串中指定的名称相关联。
*   唯一 `LOCATION` `pragma` 的最小范围是一个函数。

### `pragma VERSION_ID`

`pragma VERSION_ID` 的**语法**可以给出如下：

> `#pragma VERSION_ID "string "`

这里的字符串是放在目标文件中的字符串。

**示例：**

> `#pragma VERSION_ID "GFG 私人有限公司，版本 1234 5.8.0 1.10"`

在上例中，`pragma` 将 **GFG 私人有限公司版本 1234 5.8.0 1.10** 的字符串放入对象文件中。

### `pragma ONCE`

`pragma ONCE` 指定了包含该 `pragma` 目录的文件将只被编译器在特定文件的构建中打开一次，其**语法**可以给出为：
