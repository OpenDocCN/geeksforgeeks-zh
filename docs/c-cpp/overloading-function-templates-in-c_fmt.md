...
> 原文: [https://www.geeksforgeeks.org/overloading-function-templates-in-c/](https://www.geeksforgeeks.org/overloading-function-templates-in-c/)
...
在[函数重载](https://www.geeksforgeeks.org/function-overloading-c/)中，函数可能有相同的定义，但参数不同。下面是 [C++ 程序](https://www.geeksforgeeks.org/c-plus-plus/)来说明函数重载:
...
*   在上面的代码中，`square`重载了不同的参数。
*   函数`square`也可以重载其他参数，每次都需要相同的名称和不同的参数。
*   为了减少这些努力，[C++](https://www.geeksforgeeks.org/c-plus-plus/)引入了一个名为`函数模板`的泛型类型。

## 函数模板

函数模板的语法与常规函数相同，但它以一个[关键字](https://www.geeksforgeeks.org/cc-tokens/)`template`开始，后面是包含在角括号`< >`内的模板参数。
