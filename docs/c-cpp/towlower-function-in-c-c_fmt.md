...
`towlower()`是 C/C++ 中的一个内置函数，它将给定的宽字符转换成小写。在 C++ 的`<cwctype>`头文件中定义。

*   它是头文件`<cwctype>`中的一个函数，所以如果使用这个函数，就必须使用这个头文件，它是`tolower()`函数的宽字符等价物。
...
函数接受单个强制参数`ch`，指定我们必须转换成小写的宽字符。

### 返回值

如果存在 c 值，函数返回小写的 c 值，否则返回 c 值(不变)。该值作为`wint_t`值返回，可以隐式转换为`wchar_t`。
