...
`bitset::flip()` 是 C++ 中的内置 STL，用于翻转位。如果函数中没有传递参数，那么它会翻转所有的位值，将 0 转换为 1，将 1 转换为 0。如果传递了参数位置，它只会翻转该位置的位。
...
该功能接受一个参数`pos`，该参数不是强制的。如果参数位置被传递，它只翻转索引位置的位(索引位置是从右边开始计算的)。如果没有传递参数，它会翻转所有的位值，将 0 转换为 1，将 1 转换为 0。

## 返回值

该函数根据传入或未传入的参数翻转所有位值，并返回数字的新二进制表示。
