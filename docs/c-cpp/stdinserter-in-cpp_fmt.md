...
`std::inserter` 构造一个插入迭代器，从 `x` 指向的位置开始，在 `x` 的连续位置插入新元素。它在头文件中定义。

插入迭代器是一种特殊类型的输出迭代器，旨在允许通常覆盖元素(如复制)的算法在容器的特定位置自动插入新元素。
...
2.  **`insert()` vs `std::inserter()`**: 现在，您可能会认为 `insert()` 和 `std::inserter()` 相似，但实际上并不相似。当你必须在算法中传递一个迭代器时，你应该像上面的例子一样使用 `std::inserter()` 方法，而对于在容器中正常插入值，可以使用 `insert()` 方法。
