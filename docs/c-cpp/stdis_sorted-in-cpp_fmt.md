...
### 示例
给定一个仅由字符组成的字符串。检查字符是否按排序顺序排列。此外，在比较时忽略大小写，即 `"f" > "A"`。
...
**时间复杂度:** 复杂度在 `first` 和 `last` 之间的距离是线性的：比较元素对，直到发现不匹配。
