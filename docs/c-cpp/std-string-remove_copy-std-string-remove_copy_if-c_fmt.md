...
它是算法库中定义的 c++ 中的一个 STL 函数。它将范围`[first, last]`中的元素复制到从`result`开始的范围，除了那些与给定元素比较相等的元素。

*   得到的范围比`[first, last]`短，因为序列中匹配的元素一样多，这些元素被“移除”。
*   未被移除的元素的相对顺序被保留。
...
它是算法库中定义的 c++ 中的一个 STL 函数。它将范围`[first, last]`中的元素复制到从`result`开始的范围，满足给定条件(如奇数、偶数、素数、非素数等)的元素除外。

*   将范围`[first, last]`中的元素复制到从`result`开始的范围，条件函数返回 `true` 的元素除外。
*   得到的范围比`[first, last]`短，因为匹配的元素一样多，这些元素被“移除”。
