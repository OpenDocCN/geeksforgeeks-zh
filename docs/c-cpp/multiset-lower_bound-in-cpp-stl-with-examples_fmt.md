# C++ STL 中的 multiset::lower_bound()，示例

> 原文：[`https://www.geeksforgeeks.org/multiset-lower_bound-in-cpp-stl-with-examples/`](https://www.geeksforgeeks.org/multiset-lower_bound-in-cpp-stl-with-examples/)

`multiset::lower_bound()` 是 C++ STL 中的一个内置函数，它返回一个指向容器中第一个元素的迭代器，该元素相当于参数中传递的 `k`。如果集合容器中不存在 `k`，函数返回一个迭代器，指向刚好大于 `k` 的下一个元素。如果参数中传递的键超过容器中的最大值，则返回的迭代器打印容器中的元素数。
