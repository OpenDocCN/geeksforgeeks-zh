# C++中的std::string::crbegin()和std::string::crend()示例

> 原文：[https://www.geeksforgeeks.org/stdstringcrbegin-and-stdstringcrend-in-c-with-examples/](https://www.geeksforgeeks.org/stdstringcrbegin-and-stdstringcrend-in-c-with-examples/)
...
函数`std::string::crbegin()`返回一个引用字符串中最后一个元素的常量反向迭代器。

### 示例程序

下面是说明`std::string::crbegin()`的程序：
...
函数`std::string::crend()`返回一个常量反向迭代器，指向字符串中第一个元素之前的元素。

### 示例程序

下面是说明`std::string::crend()`的程序：
