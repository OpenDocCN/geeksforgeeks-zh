...
> 原文: [https://www.geeksforgeeks.org/taking-string-input-space-c-3-different-methods/](https://www.geeksforgeeks.org/taking-string-input-space-c-3-different-methods/)
...
**说明:** 此处，`[]` 为[扫描集](https://www.geeksforgeeks.org/scansets-in-c/)字符。`^\n` 告诉输入，直到没有遇到新行。然后，用这个 `%*c` ，它读取换行符，这里使用的 `*` 表示这个换行符被丢弃。
