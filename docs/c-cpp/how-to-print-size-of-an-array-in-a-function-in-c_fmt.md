...
> 原文：[https://www.geeksforgeeks.org/how-to-print-size-of-an-array-in-a-function-in-c/](https://www.geeksforgeeks.org/how-to-print-size-of-an-array-in-a-function-in-c/)

如何计算函数中数组参数的大小？
考虑下面的 C++ 程序：

## 问题：在函数中使用 `sizeof` 计算数组大小
...
上面的输出是针对整数大小为 4 字节，指针大小为 8 字节的机器。`main` 函数中的 `cout` 语句打印 40，而 `findSize` 函数中的 `cout` 语句打印 8。原因是，数组总是在函数中作为指针传递，即 `findSize(int arr[])` 和 `findSize(int *arr)` 的意思完全一样。因此，`findSize()` 中的 `cout` 语句打印的是指针的大小。详见[这篇文章](https://www.geeksforgeeks.org/using-sizof-operator-with-array-paratmeters/)和[这篇文章](https://www.geeksforgeeks.org/why-c-treats-array-parameters-as-pointers/)。
