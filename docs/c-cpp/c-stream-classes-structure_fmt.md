...
1.  `ios` 类是流类层次结构中最顶层的类。它是 `istream`、`ostream`、`iostream`、`ifstream`、`ofstream` 和 `fstream` 的基类。
2.  `istream` 和 `ostream` 是 `iostream` 类的直接基类。`istream` 类负责输入，`ostream` 类负责输出。
3.  类 `ios` 使用 `istream` 和 `ostream` 间接继承到 `iostream` 类。为避免 `ios` 类数据和成员函数的重复，在继承时声明为虚拟基类，在 `istream` 继承时声明为 `virtual public ios`，在 `ostream` 继承时声明为 `virtual public ios`。
...
`_withassign` 类为赋值操作提供了额外的功能，这就是为什么存在 `istream_withassign` 和 `ostream_withassign` 类。

## 各流类提供的设施

### `ios` 类
`ios` 类负责向所有其他流类提供所有输入和输出设施。

### `istream` 类
这个类负责处理输入流。它提供了许多处理字符、字符串和对象的功能，如 `get()`、`getline()`、`read()`、`ignore()`、`putback()` 等。
...
### `ostream` 类
这个类负责处理输出流。它提供了许多处理字符、字符串和对象的功能，如 `write()`、`put()` 等。
...
### `iostream` 类
这个类负责处理输入和输出流，因为 `istream` 类和 `ostream` 类都继承到其中。它提供了 `istream` 类和 `ostream` 类的功能，用于处理字符、字符串和对象，如 `get()`、`getline()`、`read()`、`ignore()`、`putback()`、`put()`、`write()` 等。
...
### `istream_withassign` 类
这个类是 `istream` 的变种，允许对象赋值。预定义的对象 `cin` 是这个类的一个对象，因此可以在运行时被重新分配给不同的 `istream` 或 `istream_withassign` 对象。

**举例：** 说明 `cin` 是 `istream` 类的对象。
...
### `ostream_withassign` 类
这个类是 `ostream` 的变种，允许对象赋值。预定义的对象 `cout`、`cerr`、`clog` 是这个类的对象，因此可以在运行时重新分配给不同的 `ostream` 对象。
