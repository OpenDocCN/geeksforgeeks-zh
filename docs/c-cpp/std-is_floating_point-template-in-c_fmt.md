# std::is_floating_point 模板

> 原文: [https://www.geeksforgeeks.org/std-is_floating_point-template-in-c/](https://www.geeksforgeeks.org/std-is_floating_point-template-in-c/)

C++ STL 的 `std::is_floating_point` 模板用于检查给定类型是否为浮点值。它返回一个显示相同内容的布尔值。
...
该模板接受单个参数 `T` (Trait 类)检查 `T` 是否为浮点类型。
...
下面的程序说明了 C++ STL 中的 `std::is_floating_point` 模板:
