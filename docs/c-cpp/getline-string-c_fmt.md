...
C++ `getline()` 是一个标准库函数，用于从输入流中读取字符串或行。它是 `<string>` 表头的一部分。`getline()` 函数从输入流中提取字符，并将其追加到 `string` 对象中，直到遇到定界字符。这样做时，字符串对象中先前存储的值将被输入字符串（如果有）替换。
...
**注意:** 在上面的例子中如果 `#define MAX_NAME_LEN 6`，那么在这种情况下如果你越过了定义的限制，你的程序将停止执行并退出。这适用于你使用 `getline()` 函数的每个宏。你会得到如下输出：
...
第二个声明和第一个几乎一样。唯一的区别是，后者有一个定界字符，默认情况下是换行符(`\n`)。
...
我们可以使用 `getline()` 函数，在一个字符的基础上拆分一个句子。让我们看一个例子来了解如何做到这一点。
...
本文由 **Harsh Agarwal** 贡献，由 **Faisal Al Mamun** 改进。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 [write.geeksforgeeks.org](https://write.geeksforgeeks.org) 写一篇文章或者把你的文章邮寄到 review-team@geeksforgeeks.org。看到你的文章出现在极客博客主页上，帮助其他极客。
