# std::adjacent_find 在 C++ 中查找

> 原文: [https://www.geeksforgeeks.org/stdadjacent_find-in-c/](https://www.geeksforgeeks.org/stdadjacent_find-in-c/)

在范围 `[first, last]` 中搜索匹配的两个连续元素的第一次出现，并返回这两个元素中第一个元素的迭代器，如果没有找到这样的对，则返回 `last`。使用给定的二元谓词 `p` 或使用 `==` 对元素进行比较。
该函数有两种可能的实现方式，如下所示：
...
返回值 :
一个指向第一对相同元素中第一个元素的迭代器，即第一个迭代器 `it` 使得对于第一个版本有 `*it == *(it+1)`，或者对于第二个版本有 `p(*it, *(it + 1)) != false`。
如果未找到此类元素，则返回 `last`。
