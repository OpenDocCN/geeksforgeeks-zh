...
> 原文：[https://www.geeksforgeeks.org/c-program-to-draw-the-taj-mahal-using-graphics/](https://www.geeksforgeeks.org/c-program-to-draw-the-taj-mahal-using-graphics/)

在 Turbo C 图形中，`graphics.h`函数用于绘制不同的形状（如圆形、矩形等），以不同的格式（不同的字体和颜色）显示文本（任何消息）。通过使用`graphics.h`程序，可以设计动画和游戏。这些对初学者很有用。

## 使用的功能

*   `arc(x, y, start_ang, end_ang, r)`：`graphics.h`头文件中的一个函数，它画一个以`(x, y)`为中心的弧，`start_ang`为其起始角，`end_ang`为其终止角，`r`为半径。
...
*   在`main()`函数中，调用第一个`taj_body()`函数。在这个函数中，泰姬陵的建筑是使用`rectangle()`函数从矩形基底开始创建的。
*   然后使用`rectangle()`函数制作两个矩形。在较小的矩形中，门是使用`line()`函数制作的。
