...
> 原文: [https://www.geeksforgeeks.org/unordered_multimap-and-its-application/](https://www.geeksforgeeks.org/unordered_multimap-and-its-application/)

**允许重复:**
我们已经在[之前的帖子](https://www.geeksforgeeks.org/unordered_map-in-stl-and-its-applications/)中讨论了`unordered_map`，但是有一个限制，我们不能在`unordered_map`中存储重复，也就是说，如果我们的`unordered_multimap`中已经有一个键值对，并且插入了另一个键值对，那么两个键值对都会在那里，而在`unordered_map`的情况下，对应于键值的先前值会被只会在那里的新值更新。甚至可以在`unordered_multimap`中存在两次。

**内部表示:**
`unordered_multimap`的内部实现与`unordered_map`的内部实现相同，但是对于重复的键，每个键值对维护另一个计数值。由于对存储在哈希表中，它们之间没有特定的顺序，但是具有相同键的对在数据结构中聚集在一起，而具有相同值的对不能保证聚集在一起。

**时间复杂度:**
`unordered_multimap`上的所有操作平均花费恒定的时间量，但是在最坏的情况下，时间可能会变成线性的，这取决于内部使用的哈希函数，但是从长远来看，`unordered_multimap`优于`multimap`（基于树的`multimap`）。

**函数:**
`unordered_multimap`支持许多函数，如下代码所示:
...
正如我们在上面的代码中看到的，大部分操作工作类似于`unordered_map`，但是需要注意的是:
我们可以使用初始化器列表一次初始化并插入许多对。
`unordered_multimap`没有`[]`运算符，因为对应于一个键的值不是唯一的，一个键可以有许多值，因此`[]`运算符不能应用于它们。
`erase`功能删除与所提供的键相关联的值的所有实例。
...
# C++ STL unordered_multimap 成员函数

*   `begin()` – 返回一个迭代器，该迭代器指向容器中的第一个元素或其存储桶中的第一个元素。
*   `end()` – 返回一个迭代器，该迭代器指向容器中最后一个元素之后的位置或其存储桶中最后一个元素之后的位置。
*   `count()` – 返回容器中键等于参数中传递的键的元素数。
*   `CBE gin()` – 返回指向容器中第一个元素或其存储桶中第一个元素的常量迭代器。
*   `cend()` – 返回一个常量迭代器，该迭代器指向容器中最后一个元素之后的位置或其存储桶中最后一个元素之后的位置。
*   `清除()` – 清除无序 _ 多映射容器的内容。
*   `size()` – 返回无序 _ 多映射的大小。它表示容器中元素的数量。
*   `交换()` – 交换两个无序 _ 多映射容器的内容。两个容器的尺寸可以不同。
*   `find()` – 返回一个迭代器，该迭代器指向具有关键字 k 的元素之一。
*   `bucket _ size()` – 返回 bucket 中元素的个数 n。
*   `empty()` – 如果无序 _multimap 容器为空，则返回 true。否则，它返回 false。
*   `equal _ range()` – 返回所有元素的键等于一个键的范围。
*   `操作符=` – 从不同容器复制/分配/移动元素。
*   `max _ size()` – 返回无序多映射容器可以容纳的最大元素数。
*   `load _ factor()` – 返回无序多映射容器中的当前加载因子。
*   `key _ eq()` – 根据比较结果返回布尔值。
*   `侵位()` – 在无序 _ 多映射容器中插入一个新的{key，element}。
*   `侵位 _ 提示()` – 在无序 _ 多映射容器中插入一个新的{key:element}。
*   `bucket _ count()` – 返回无序多映射容器中的桶总数。
*   `桶()` – 返回给定键所在的桶号。
*   `max _ load _ factor()` – 返回无序 _multimap 容器的最大加载因子。
*   `重新灰化()` – 将容器中的铲斗数量设置为 N 或更多。
*   `reserve()` – 将容器中的桶数(bucket_count)设置为最合适的数量，使其至少包含 n 个元素。
*   `hash _ function()` – 该 hash 函数是一元函数，只接受单个参数，并基于该参数返回 size_t 类型的唯一值。
*   `max _ bucket _ count()` – 返回无序多映射容器可以拥有的最大桶数。

`最近关于无序 _ 多地图的文章`
本文由乌卡什·特里维迪供稿。如果您发现任何不正确的地方，或者您想分享关于上面讨论的更多信息，请写评论