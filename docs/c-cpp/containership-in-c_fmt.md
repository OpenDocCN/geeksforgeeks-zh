...
**说明：** 在类`second`中我们有一个类`first`的对象。这是我们正在见证的另一种类型的遗传。这种类型的继承被称为**具有 _a** 关系，因为我们说类`second`有一个第一类`first`的对象作为它的成员。从对象`f`我们先调用类`first`的函数`showf()`。
...
**解释：** 在这个程序中我们没有继承类`first`变成类`second`，但是因为我们有一个类`first`的对象作为类`second`的成员。所以调用类`second`的默认构造函数时，由于`second`中存在`first`类的对象`f`，首先调用类`first`的默认构造函数，然后调用类`second`的默认构造函数。
...
**说明：** 在集装箱船的帮助下，我们只能使用`public`成员/功能类，而不能使用`protected`或`private`。在`first`课中，我们借助`getnum`返回了参考。然后我们通过调用`showf`来显示它。
