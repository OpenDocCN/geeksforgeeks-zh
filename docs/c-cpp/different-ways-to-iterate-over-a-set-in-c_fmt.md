...
> 原文：[https://www.geeksforgeeks.org/different-ways-to-iterate-over-a-set-in-c/](https://www.geeksforgeeks.org/different-ways-to-iterate-over-a-set-in-c/)
...
2.  使用[反向迭代器](https://www.geeksforgeeks.org/vector-rbegin-and-rend-function-in-c-stl/)反向迭代一个集合。
3.  使用[基于范围的 for 循环](https://www.geeksforgeeks.org/range-based-loop-c/)迭代一个集合。
4.  使用[`for_each`循环](https://www.geeksforgeeks.org/for_each-loop-c/)迭代一个集合。
...
>
> `range_declaration`：
> 一个命名变量的声明，其类型是 `range_expression` 表示的序列元素的类型，或者是对该类型的引用。通常使用 `auto` 说明符进行自动类型推导。
>
> `range_expression`：
...
## 使用 `for_each` 循环迭代一个集合

在这种方法中，`for_each` 循环接受一个在每个容器元素上执行的函数。

语法：

> `for_each(input_iterator start_iter, input_iterator last_iter, function fnc)`
>
