...
`std::swap` 用于两个容器之间的元素交换。它的一个变体是 `std::swap_ranges`，顾名思义，它用于交换范围内的元素。
...
当我们必须找出一个给定的容器在前半部分和后半部分是否包含相同的元素时，即两个半部分是否相同时，可以使用它。
...
因此，在第一个向量 `1 2 3 4 5 1 2 3 4` 中，它在两个半部分中包含相同的元素，因此打印“Yes”，而在第二个向量 `1 2 3 4 1 2 3 5` 中，后半部分包含一个与前半部分不同的元素，即 `4` 代替 `5`，因此打印“No”。
