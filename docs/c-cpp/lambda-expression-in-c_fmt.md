...
C++ 11 引入了 lambda 表达式，允许我们编写一个内联函数，该函数可以用于那些不会被重用且不值得命名的简短代码片段。在其最简单的形式中，lambda 表达式可以定义如下:
...
通常 lambda 表达式中的返回类型由编译器自己计算，我们不需要指定，可以忽略显式的 `-> return-type` 部分。但是在一些复杂的情况下，如条件语句中，编译器无法识别返回类型，我们需要指定它。

## 示例

标准函数 lambda 表达式的各种用法如下:
