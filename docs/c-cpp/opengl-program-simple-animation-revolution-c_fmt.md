...
> 原文：[`https://www.geeksforgeeks.org/opengl-program-simple-animation-revolution-c/`](https://www.geeksforgeeks.org/opengl-program-simple-animation-revolution-c/)
...
**进场:**
要让画面动起来，需要了解一个用来显示的功能的工作流程，即 `glClear(GL_COLOR_BUFFER_BIT)`。它的任务是在一定时间后（通常在 1/30 秒或 1/60 秒后）用默认值清除屏幕。因此，如果坐标发生任何变化，那么它将看起来是移动的，因为人眼只能分辨相隔 1/16 秒的图像（视觉暂留）。
...
*   因为，我们必须使所有的点以相同的运动类型运动，以保持图形在一起，所以运动方程，即 `glVertex2i(x/2–600 * cos(j), y/2–100 * sin(j))` 在循环的每个内部给出，因此它可以应用于所有的点。
