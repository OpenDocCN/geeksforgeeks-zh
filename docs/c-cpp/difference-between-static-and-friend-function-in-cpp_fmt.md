...
> 原文: [https://www.geeksforgeeks.org/difference-between-static-and-friend-function-in-cpp/](https://www.geeksforgeeks.org/difference-between-static-and-friend-function-in-cpp/)

`静态函数`：它基本上是一个`成员函数`，即使类的对象没有初始化也可以调用。这些函数与任何对象相关联，用于在类的不同对象之间维护类成员函数的单一副本。这个函数用 `static` 关键字表示。

`好友功能`：基本上是访问类的`非公开成员`特别需要的功能。它有权访问该类的所有私有和受保护成员。它通常提供一些通常不被类使用的附加功能，并允许共享由非成员函数提供的类信息。
