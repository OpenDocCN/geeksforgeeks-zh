# C++ 中的 `unordered_map::count()` 方法

> 原文：[https://www.geeksforgeeks.org/unordered_map-count-in-c/](https://www.geeksforgeeks.org/unordered_map-count-in-c/)

`unordered_map::count()` 是 C++ 中的一个内置方法，用于计算具有给定键的无序映射中的元素数量。

**注意**：由于 `unordered_map` 不允许存储重复键的元素，所以 `count()` 函数基本上是检查给定键在 `unordered_map` 中是否存在。
...
该函数接受单个参数 `Key`，需要在给定的无序映射容器中进行检查。

## 返回值

如果给定键在映射中存在值，则该函数返回 `1`，否则返回 `0`。

下面的程序说明了 `unordered_map::count()` 函数：
