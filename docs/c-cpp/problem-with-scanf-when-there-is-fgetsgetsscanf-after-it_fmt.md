...
> 原文：[https://www.geeksforgeeks.org/problem-with-scanf-when-there-is-fgetsgetsscanf-after-it/](https://www.geeksforgeeks.org/problem-with-scanf-when-there-is-fgetsgetsscanf-after-it/)
...
1.  我们可以使用额外的 `"\n"` 让 `scanf()` 读取换行符，即 `scanf("%d\n", &x)`。实际上，`scanf("%d", &x)` 也可以（注意末尾的空格）。
2.  我们可以在 `scanf()` 后添加一个 `getchar()` 来读取多余的换行符。

## 修正程序

修正程序见[此链接](https://ide.geeksforgeeks.org/nnfP2h)和[此链接](https://ide.geeksforgeeks.org/BAD6dR)。
