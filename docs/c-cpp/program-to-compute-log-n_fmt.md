...
写一个计算并返回 $\log_2 n$ 的单行 C 函数。例如，如果 `n = 64`，那么你的函数应该返回 6，如果 `n = 128`，那么你的函数应该返回 7。
...
**时间复杂度:** O(log n)
**辅助空间:** O(log n) 如果在递归过程中考虑了堆栈大小，否则为 O(1)
...
写一个返回 $\lfloor\log_r n\rfloor$ 的单行函数 `Logn(n, r)`。
...
我们只需要用对数性质来求任意基数 `r` 上 `log(n)` 的值。即 $\log_r n = \dfrac{log_k (n)}{\log_k (r)}$，其中 `k` 可以是任何东西，对于标准日志功能，可以是 `e` 或 `10`。
