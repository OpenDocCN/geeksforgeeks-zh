# C++ STL 中的 unordered_set::get_allocator() 示例

> 原文: [https://www.geeksforgeeks.org/unordered_set-get_allocator-in-cpp-stl/](https://www.geeksforgeeks.org/unordered_set-get_allocator-in-cpp-stl/)

[`unordered_set`](https://www.geeksforgeeks.org/unorderd_set-stl-uses/) 的 `get_allocator()` 方法是 C++ **标准模板库 (STL)** 的一部分。此方法获取存储的分配器对象并返回它。
...
其中 `Allocator_type` 是容器使用的分配器的类型。
...
在这个方法中，如果有任何元素比较对象抛出异常，就会抛出异常。

下面的程序说明了 `unordered_set::get_allocator()` 函数。
