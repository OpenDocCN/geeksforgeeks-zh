...
任何数据类型都用于在变量中存储任何类型的值。像 JavaScript、TypeScript 这样的脚本语言提供了任何数据类型的功能。C++ 也提供了这个功能，但只是在 boost 库的帮助下。只要将变量的数据类型设为任意，就可以将任何类型的值赋给变量。以下是用任何数据类型声明变量所需的语法：
...
要使用 `boost::any` 数据类型，`"boost/any.hpp"` 需要包含在程序中。
...
[http://www.boost.org/doc/libs/1_66_0/doc/html/any.html](http://www.boost.org/doc/libs/1_66_0/doc/html/any.html)