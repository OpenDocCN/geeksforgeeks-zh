...
`std::multimap::key_comp()` 是 C++ STL 中的一个内置函数，它返回容器使用的比较对象的副本。默认情况下，这是一个 `less` 对象，返回与运算符“<”相同的值。它是一个函数指针或函数对象，接受两个与容器元素类型相同的参数，如果第一个参数被认为在它定义的严格弱排序中位于第二个参数之前，则返回 `true`，否则返回 `false`。如果 `key_comp` 反身返回 `false`（即，无论键作为参数传递的顺序如何），则两个键被认为是等效的。
