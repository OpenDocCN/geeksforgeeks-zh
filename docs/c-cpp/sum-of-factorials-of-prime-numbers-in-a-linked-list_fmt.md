...
**输出：** `8`
**说明：**
质数为 `2` 和 `3`，所以是 `2! + 3! = 2 + 6 = 8`。

**输入：** `L1 = 7->4->5`
**输出：** `5160`
**解释：**
质数为 `7` 和 `5`，所以是 `7! + 5! = 5160`。

## 方法

按照以下步骤解决上述问题：

*   实现一个函数 `factorial(n)`，用于[找到 `N` 的阶乘](https://www.geeksforgeeks.org/program-for-factorial-of-a-number/)。
*   初始化一个变量 `sum = 0`。现在，遍历给定的列表，检查每个节点是否是[素数](https://www.geeksforgeeks.org/prime-numbers/)。
*   如果节点是质数，则更新 `sum = sum + factorial(node)`，否则将节点移到下一个。
*   最后打印计算出的 `sum`。
