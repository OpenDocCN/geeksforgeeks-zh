...
一种解决方案是创建一个用户定义的类或结构。我们创建一个有三个成员的结构，然后创建这个结构的向量。
...
另一种解决方案是在 C++ STL 中使用对类。我们将第一个元素作为普通元素，第二个元素作为另一个元素对，因此同时存储 3 个元素。
...
本文由 **Jatin Goyal** 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 `contribute.geeksforgeeks.org` 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
