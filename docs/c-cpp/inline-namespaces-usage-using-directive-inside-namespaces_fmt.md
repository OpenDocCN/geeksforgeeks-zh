...
> 原文：[https://www.geeksforgeeks.org/inline-namespaces-usage-using-directive-inside-namespaces/](https://www.geeksforgeeks.org/inline-namespaces-usage-using-directive-inside-namespaces/)

先决条件：[C++ 中的命名空间](https://www.geeksforgeeks.org/namespace-in-c/)

内联命名空间是在其原始名称空间定义中使用可选关键字 `inline` 的名称空间。
...
这种相同的行为（与内联命名空间相同）也可以通过在命名空间中使用 `using` 声明性来实现。命名内联命名空间的 `using` 指令隐式插入到封闭命名空间中（类似于未命名命名命名空间的隐式 `using` 指令）。考虑以下 C++ 代码：
