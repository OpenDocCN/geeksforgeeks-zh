...
> 原文：[https://www.geeksforgeeks.org/difference-between-const-int-const-int-const-and-int-const/](https://www.geeksforgeeks.org/difference-between-const-int-const-int-const-and-int-const/)
...
*   `int const* const` 是 **const** **指针** 到 **const** **int**

使用这个规则，即使是复杂的声明也可以被解码，

*   `int ** const` 是一个 **const** **指针** 到 **指针** 到一个 **int**。
*   `int * const *` 是指向 **const** **指针** 的 **指针** 和指向 **int** 的指针。
*   `int const **` 是一个 **指针** 指向一个 **指针** 指向一个 **const** **int**。
