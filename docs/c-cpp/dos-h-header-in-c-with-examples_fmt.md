...
> 原文: [https://www.geeksforgeeks.org/dos-h-header-in-c-with-examples/](https://www.geeksforgeeks.org/dos-h-header-in-c-with-examples/)
...
**参数：** 它接受一个以毫秒为单位的 `time`，将程序执行停止到那个时间段。
...
**参数：** 它接受在 `dos.h` 库中定义的结构 `date`。使用功能 `da_day()`、`da_mon()` 和 `da_year()` 打印日期。
...
**参数：** 它接受在 `dos.h` 库中定义的结构 `time`。使用功能 `ti_hour()`、`ti_min()` 和 `ti_sec()` 打印时间。
...
## sound()
`sound()` 函数在 C 中用于在我们的系统上播放不同频率的声音。
...
**参数：** 它接受一个 `frequency`，并在我们的系统中播放该频率的声音。
...
## nosound()
`nosound()` 函数在 C 中用于停止由 `sound()` 函数播放的声音。
