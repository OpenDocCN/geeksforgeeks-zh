...
> 原文：[https://www.geeksforgeeks.org/non-blocking-io-with-pipes-in-c/](https://www.geeksforgeeks.org/non-blocking-io-with-pipes-in-c/)

先决条件：[`pipe()`系统调用](https://www.geeksforgeeks.org/pipe-system-call/)
...
此函数成功运行后，如果管道**为空/满**并设置`errno`为`EAGAIN`，则读/写调用返回`-1`。

**示例：** 子节点每 3 秒向父节点写一次“你好”，父节点每秒执行一次非阻塞读。
...
在这里，在`while`循环第一个 5 秒报警后设置`write()`调用在管道中写一个字符`x`。`count`变量用于计数管道中的字符写入。`alarm(0)`表示取消 5 秒的设定报警。一段时间后当管道容量满时，`write()`调用被阻塞，程序不执行下一条指令，5 秒后设置报警振铃并发送信号`SIGALRM`。之后`alram_action`处理程序执行并打印管道最多可以写多少个字符。
