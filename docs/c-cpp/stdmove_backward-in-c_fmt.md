...
> 原文: [https://www.geeksforgeeks.orgstdmove_backward-in-c/](https://www.geeksforgeeks.org/stdmove_backward-in-c/)

将范围 `[first, last]` 中的元素从末尾移动到结果处结束的范围。
该函数从将 `*(last-1)` 移动到 `*(result-1)` 开始，然后向后跟随这些元素之前的元素，直到到达 `first` (并包括它)。
...
`result`
指向目标序列中 past-the-end 位置的双向迭代器。此迭代器不应指向范围 `[first, last)` 内的任何元素。

**返回类型：**
一个指向目标序列中第一个元素的迭代器，元素已被移动至此。
