...
> 原文: [https://www.geeksforgeeks.org/database-connectivity-using-cc/](https://www.geeksforgeeks.org/database-connectivity-using-cc/)

SQL（结构化查询语言）是第四代语言（4GL），用于定义、操作和控制关系数据库管理系统。

在开始主要文章之前，让我们熟悉一下所使用的工具。

## 1. 编译器：Code::Blocks IDE

**下载链接:** [二进制下载](http://www.codeblocks.org/downloads/5)

`Code::Blocks` 是交叉编译器（可以在 Windows、Linux、Mac 等任何平台上运行），可以免费下载。这个 IDE 是专门为 C 和 C++ 设计的，使用方便。
...
1.  打开 `Code::Blocks` IDE，转到或点击 **设置** -> **编译器和调试器设置**（您现在将看到全局编译器设置）。
2.  现在点击 **链接器设置**，在链接器设置中点击添加按钮并添加以下内容。
...
c:\Program Files\CodeBlocks\MinGW\lib\libkernel32.a
```
...
现在，我们将看看如何执行一个简单的 SQL 查询。首先，为数据库创建一个表：

```sql
create table tb1(id number, name varchar(20));
```

现在，在您的 `con.Connect` 之后建立与数据库的连接；您应该使用 `cmd.setCommandText` 方法将查询传递给数据库，如下所示：
