...
C++11 引入了多线程支持。在 C++11 之前，我们不得不在 C 中使用 POSIX 线程或者 `pthreads` 库。虽然这个库完成了工作，但是缺少任何标准语言提供的特性集导致了严重的可移植性问题。C++11 抛弃了所有这些，给了我们 `std::thread`。线程类和相关函数在 `<thread>` 头文件中定义。
...
要等待线程，请使用 `std::thread::join()` 函数。该功能使当前线程等待，直到该标识的线程执行完毕。
...
**参考**
[C++ reference - 线程库](https://en.cppreference.com/w/cpp/thread)