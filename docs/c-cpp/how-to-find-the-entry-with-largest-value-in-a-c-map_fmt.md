...
> 原文：`https://www.geeksforgeeks.org/how-to-find-the-entry-with-largest-value-in-a-c-map/`

给定一个 C++ 中的`map`，任务是找到这个`map`中值最高的条目。
...
1.  在`iterator`的帮助下，逐个条目迭代`map`条目。
    ```cpp
    map::iterator itr;
