...
`map::insert()` 是 C++ STL 中的内置函数，用于在映射容器中插入带有特定键的元素。
...
```cpp
// C++ program to illustrate
// map::insert(iterator position, {key, element})
#include <bits/stdc++.h>
using namespace std;
...
**参数:** 该函数接受两个参数 `position1` 和 `position2`，指定元素的范围。范围 `[position1, last)` 中的所有元素都被插入到另一个地图容器中。
...
```cpp
// C++ program to illustrate
// map::insert(iterator position1, iterator position2)
#include <bits/stdc++.h>
using namespace std;
