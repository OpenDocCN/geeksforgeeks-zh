...
这是`C++`的`标准模板库(STL)`的一部分。要使用这个`STL`，使用`namespace std`并在程序中包含`<map>`头文件。
返回比较键的`函数对象`或`比较对象`或`排序委托`，这是这个容器的构造函数参数的副本。
是一个`函数指针`或者一个`对象`取两个同类型的参数作为元素键，决定容器中元素的顺序。

## 语法:

> `key_compare key_comp();`

这里`key_compare`是`比较对象`的类型，与容器相关联。
...
> 返回键比较`函数对象`或`排序委托`，在`multimap`中定义为其第三个模板参数的别名。
