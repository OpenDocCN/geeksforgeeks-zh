# Boost C++ 库的 `std::boost::is_array` 模板

> 原文：[https://www.geeksforgeeks.org/boosttype_traitsis_array-template-in-c/](https://www.geeksforgeeks.org/boosttype_traitsis_array-template-in-c/)
...
该模板接受单个参数 `T`（Trait 类）检查 `T` 是否为指针类型。
...
下面的程序说明了 `std::boost::type_traits::is_array` 模板：
