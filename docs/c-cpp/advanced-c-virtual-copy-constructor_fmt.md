...
> 原文：[https://www.geeksforgeeks.org/advanced-c-virtual-copy-constructor/](https://www.geeksforgeeks.org/advanced-c-virtual-copy-constructor/)
...
`User`类在虚拟构造器的帮助下创建对象。要创建的对象基于用户输入。`Action()`正在复制正在创建的对象并修改其属性。借助`Clone()`虚拟函数创建的复制对象，该函数也被认为是*虚拟复制构造器*。`Clone()`方法背后的概念是*原型模式*的构建块。
