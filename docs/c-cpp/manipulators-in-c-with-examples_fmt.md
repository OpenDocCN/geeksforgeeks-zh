...
### 1. 无参数操纵器
下面提供了**iOS stream 库**定义的最重要的操纵器。
*   `endl` :在`ostream`中定义。它用于输入一个新行，在输入一个新行后，它会刷新输出流(即，它会强制将所有输出写入屏幕或文件中)。
*   `ws` :在 `istream` 中定义，用于忽略字符串序列中的空格。
*   `ends`:也是在 `ostream` 中定义的，它在输出流中插入一个空字符。它通常与 `std::ostream` 一起使用，当相关的输出缓冲区需要以空终止以作为 C 字符串处理时。
...
*   **`<iomanip>` 中的一些重要操纵者是:**
    ...
    3.  `setprecision(val)`: 它将 `val` 设置为浮点值精度的新值。
    4.  `setbase(val)`: 用于设置数值的数值基值。
    5.  `setiosflags(标志)`:用于设置参数掩码指定的格式标志。
    6.  `resetiosflags(m)`: 用于重置参数掩码指定的格式标志。
*   **`<ios>` 中一些重要的操控者有:**
    1.  `showpos`:它强制在正数上显示一个正号。
    2.  `noshowpos`: 它强制不要在正数上写正号。
    3.  `showbase`:表示数值的数值基数。
    4.  `uppercase`:它强制数字值使用大写字母。
    5.  `nouppercase`:它强制数值使用小写字母。
    6.  `fixed`:对浮点值使用十进制表示法。
    7.  `scientific`: 它使用科学浮点记数法。
    8.  `hex`:读写整数的十六进制值，其工作方式与`setbase(16)`相同。
