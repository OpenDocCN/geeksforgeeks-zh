...
2.  The function (`operator ()`) returns the `位或` of its parameter.
...
## 某些点要记住
1.  The `bit_or` result of a number with`零`enters the number itself.
2.  The bitwise OR of two`相同的数字`is also equal to the number itself.
3.  If the number is`奇数`，its `bit_or` and `1` will produce the same number.
