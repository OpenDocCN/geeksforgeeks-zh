...
> 原文: [https://www.geeksforgeeks.org/draw-an-ellipse-divided-by-straight-line-into-two-colored-part-in-c-graphics/](https://www.geeksforgeeks.org/draw-an-ellipse-divided-by-straight-line-into-two-colored-part-in-c-graphics/)
...
*   [`ellipse(int x, int y, int from_angle, int to_angle, int x_rad, int y_rad)`](https://www.geeksforgeeks.org/draw-ellipse-c-graphics/): 一个来自 `graphics.h` 头文件的函数负责在屏幕上创建椭圆。
*   [`line(int x1, int y1, int x2, int y2)`](https://www.geeksforgeeks.org/draw-line-c-graphics/): 一个函数从 `graphics.h` 头文件里面画一条线。
*   [`floodfill(int x, int y, int border_color)`](https://www.geeksforgeeks.org/setfillstyle-floodfill-c/): 一个来自 `graphics.h` 头文件的负责函数用于填充一个封闭区域。当前填充图案和填充颜色用于填充该区域。
