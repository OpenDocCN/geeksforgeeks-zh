...
为了创建堆栈，我们必须在代码中包含`<stack>`头文件。然后我们用这个语法来定义`std::stack`：

```cpp
template <class T, class Container = deque<T> > class stack;
```

- **`T`** – 是包含在`std::stack`中的元素类型。它可以是任何有效的 C++ 类型，甚至是用户定义的类型。
- **`Container`** – 是底层容器对象的类型。

## 成员类型

- `value_type` – 第一个模板参数，它表示元素类型。
- `container_type` – 第二个模板参数，容器。它表示底层容器类型。
- `size_type` – 无符号整数类型。
...
- [`push(g)`](https://www.geeksforgeeks.org/stack-push-and-pop-in-c-stl/) – 在堆栈顶部添加元素`'g'` – 时间复杂度：`O(1)`
- [`pop()`](https://www.geeksforgeeks.org/stack-push-and-pop-in-c-stl/) – 移除堆栈顶部的元素 – 时间复杂度：`O(1)`
...
1.  在我们的代码中包含`iostream`头文件或`<bits/stdc++.h>`来使用它的功能。
2.  在我们的代码中包含`stack`头文件来使用它的函数，如果已经包含了`<bits/stdc++.h>`，那么就不需要`stack`头文件，因为它已经内置了函数。
...
- [`stack::top()` 在 C++ STL 中](https://www.geeksforgeeks.org/stacktop-c-stl/)
- [`stack::empty()` 和 `stack::size()` 在 C++ STL 中](https://www.geeksforgeeks.org/stackempty-stacksize-c-stl/)
- [`stack::push()` 和 `stack::pop()` 在 C++ STL 中](https://www.geeksforgeeks.org/stackpush-stackpop-c-stl/)
- [`stack::swap()` 在 C++ STL 中](https://www.geeksforgeeks.org/stackswap-c-stl/)
- [`stack::emplace()` 在 C++ STL 中](https://www.geeksforgeeks.org/stackemplace-c-stl/)
