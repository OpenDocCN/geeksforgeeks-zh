...
思路是在 C 中使用`system()`。该函数用于从 C 程序调用操作系统命令。
...
`system()`函数的参数是通往操作系统的路径，而`/i`是我们可用的大量选项中的一个实体。要查看选项，我们运行`cmd`并键入:

```
C:\Users\User>shutdown
```

`shutdown`命令为我们提供了一个选项列表。这些是:
...
本文由**萨哈布拉****阿玛蒂亚·兰詹·赛基亚**供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用`contribute.geeksforgeeks.org`写一篇文章或者把你的文章邮寄到`contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
