# C++ 中的 regex_replace | 使用 regex_replace 替换字符串的匹配项

> 原文: [https://www.geeksforgeeks.org/regex_replace-in-cpp-replace-the-match-of-a-string-using-regex_replace/](https://www.geeksforgeeks.org/regex_replace-in-cpp-replace-the-match-of-a-string-using-regex_replace/)
...
1.  `$&` 或者 `{content}amp;` 用于插入整个正则表达式匹配。
...
假设一个正则表达式对象 `re("((geeks)(.*))")` 被创建，主题字符串为 `subject("its all about geeksforgeeks")`，你想用任何[捕获组](https://www.geeksforgeeks.org/smatch-regex-regular-expressions-in-c/)的内容替换匹配（例如 `$0`，`$1`，…最多 9）。

> **示例-1:**
> 将匹配替换为 `$1` 的内容。
> 这里匹配的是 “geeksforgeeks”，将被 `$1` (“geeks”) 取代。
> 因此，结果 “its all about geeks”。
>
> **示例-2:**
> 将匹配替换为 `$2` 的内容。
> 这里 match 是 “geeksforgeeks”，将被 `$2` (“forgeeks”) 代替。
> 因此，结果 “its all about forgeeks”。
