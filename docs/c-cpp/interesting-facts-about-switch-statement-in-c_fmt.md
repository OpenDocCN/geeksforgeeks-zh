...
> 原文: [https://www.geeksforgeeks.org/interesting-facts-about-switch-statement-in-c/](https://www.geeksforgeeks.org/interesting-facts-about-switch-statement-in-c/)

先决条件 – [switch语句在C](https://www.geeksforgeeks.org/switch-statement-cc/)
`switch`是一个控制语句，允许一个值改变对执行的控制。
...
## 1) `switch`中使用的表达式必须是整型(`int`、`char`和`enum`)。
任何其他类型的表达式都是不允许的。
...
在Java中，`switch`中也允许字符串(参见[这个](http://docs.oracle.com/javase/tutorial/java/nutsandbolts/switch.html))

## 2) 匹配`case`之后的所有语句执行，直到到达`break`语句。
...
## 5) `case`以上编写的语句从不执行。
`switch`语句后，控制转移到匹配`case`，`case`前编写的语句不执行。
