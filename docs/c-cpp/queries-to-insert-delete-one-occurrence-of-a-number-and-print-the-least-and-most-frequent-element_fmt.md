...
*   **类型-3:** 打印最不频繁的元素，如果存在多个元素，则打印其中最大的。
...
一种**有效的方法**是使用[集合](https://www.geeksforgeeks.org/set-in-cpp-stl/)容器来回答每个查询。使用两个集合，一个散列表，上述问题可以在每个查询的 **O(log N)** 中解决。使用两套 `s1` 和 `s2` ，一套存储 `{num，frequency}` ，另一套存储 `{frequency，number}` 。使用存储每个数字的频率的散列图。使用运算符重载设计集合 `s2` ，使其按照第一个元素的升序排序。如果第一个元素看起来与一个或多个元素相同，则集合将按第二个元素的降序排序。用户定义的[运算符重载](https://www.geeksforgeeks.org/operator-overloading-c/)功能将是:
...
```
注意：运算符重载仅适用于用户定义的数据类型。`pr` 是一个结构体，其中 `first` 和 `second` 是两个整数。

## 算法

下面是求解每种类型查询的算法:

*   **类型 1:** 使用哈希表检查元素是否存在。如果不存在，则在[哈希表](https://www.geeksforgeeks.org/hashing-set-1-introduction/)中标记该数字。使用 [`insert()`](https://www.geeksforgeeks.org/set-insert-function-in-c-stl/) 将 `{num，1}` 插入集合 `s1`，将 `{num}` 插入集合 `s2`。如果之前存在，则从哈希表中获取频率，并使用 [`find()`](https://www.geeksforgeeks.org/set-find-function-in-c-stl/) 和 [`erase()`](https://www.geeksforgeeks.org/seterase-c-stl/) 功能从 `s1` 中删除 `{num，frequency}` ，从 `s2` 中删除 `{frequency，num}` 。在集合 `s1` 中插入 `{num，frequency+1}` ，在集合 `s2` 中插入 `{frequency+1，num}` 。另外，增加哈希表中的计数。
*   **类型 2:** 在查询类型 1 中遵循与上面相同的过程。唯一不同的是减少哈希表中的计数，在集合 `s1` 中插入 `{num，frequency-1}` ，在集合 `s2` 中插入 `{frequency-1，num}` 。
*   **类型 3:** 打印可以使用 [`begin()`](https://www.geeksforgeeks.org/setbegin-setend-c-stl/) 函数获得的开始元素，因为集合的设计方式是 `begin()` 返回最不频繁的元素。如果有多个，则返回最大的。
