...
> 原文：[https://www.geeksforgeeks.org/what-happens-if-loop-runs-till-maximum-of-signed-and-unsigned-in-c/](https://www.geeksforgeeks.org/what-happens-if-loop-runs-till-maximum-of-signed-and-unsigned-in-c/)
...
我们知道字符变量的大小是 8 位或 1 字节。因此，通过十进制数的 2 进制表示，8 位中的最大数字是 `11111111`。这是因为 8 位无符号数的范围是从 `0` 到 `2^8 - 1`。

现在 `(11111111)_2 = (255)_10`。

如果我们从 `0` 开始将循环驱动到 `255`，它将在循环中执行该语句 `256` 次（包括 `0` 和 `255` 次）。当循环到达 `(255)_10` 时，在执行之后，变量 `i` 增加 `1`，即通过 2s 补码运算，

`(11111111)_2 + (0000001)_2 = (0000000)_10`
...
带符号的字符范围从 `-2^7` 到 `2^7 - 1`，因此如果限制为 `< 128`，也是无限执行。

注意 `(127)_10` 的 2s 补码是 `(01111111)_2` 加 `1` 得到 `(10000000)_2`，从 2s 补码形式计算是 `–(128)_10`。
