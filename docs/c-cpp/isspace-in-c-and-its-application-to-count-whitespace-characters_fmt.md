...
> 原文：[https://www.geeksforgeeks.org/isspace-in-c-and-its-application-to-count-whitespace-characters/](https://www.geeksforgeeks.org/isspace-in-c-and-its-application-to-count-whitespace-characters/)

## isspace() 函数
在 C++ 中，`isspace()` 是用于字符串和字符处理的预定义函数。`cstring` 是字符串函数所需的头文件，`cctype` 是字符函数所需的头文件。
...
本文由 **Ayush Saxena** 和 **Kanchan Ray** 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 [contribute.geeksforgeeks.org](http://www.contribute.geeksforgeeks.org) 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
