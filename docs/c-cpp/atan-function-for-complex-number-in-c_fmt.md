...
> 原文：[https://www.geeksforgeeks.org/atan-function-for-complex-number-in-c/](https://www.geeksforgeeks.org/atan-function-for-complex-number-in-c/)

复数的 `atan()` 函数在 `<complex>` 头文件中定义。该功能是 `atan()` 功能的复数版本。该函数用于计算复数 `z` 的复反正切，并返回复数 `z` 的反正切。
