...
> 原文：[https://www.geeksforgeeks.org/thread-joinable-function-in-c/](https://www.geeksforgeeks.org/thread-joinable-function-in-c/)

`std::thread::joinable` 是 C++ `std::thread` 中的内置函数。它是一个观察者函数，这意味着它观察一个状态，然后返回相应的输出，并检查线程对象是否可连接。
...
是布尔型函数，当线程对象为可连接时返回 `true`。如果线程对象不可连接，则返回 `false`。
...
**注意：** 第三个输出将在 1 秒后出现，因为线程被休眠了 1 秒钟。