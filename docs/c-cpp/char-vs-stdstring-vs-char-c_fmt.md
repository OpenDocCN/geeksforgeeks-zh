...
1.  这在 C 中可以很好地工作，但是在 C++ 中用这种形式编写是一个坏主意。这就是为什么编译器显示“不赞成从字符串常量转换为‘char *’”的警告，因为在 C 语言中，字符串文字是字符数组，而在 C++ 中是字符常量数组。因此在 `char*` 前使用 `const` 关键字。

    ```cpp
    const char* str = "This is GeeksForGeeks";
    ```

2.  我们不能在程序的后期修改字符串。我们可以改变 `str` 指向其他东西，但不能改变 `str` 处存在的值。更多细节请参考 [storage-for-strings-in-c](https://www.geeksforgeeks.org/storage-for-strings-in-c/)。
...
在 C++ 中处理字符串时，`std::string` 是最好的选择，因为它有更好的搜索、替换和操作功能。下面讨论一些有用的 `std::string` 函数。
...
1.  当处理较低级别的访问时，比如与操作系统对话，但是通常，如果您将字符串传递给操作系统，那么 `std::string::c_str` 会覆盖它。
...
2.  如果我们想要连接或操作其他字符串，我们需要取数组的大尺寸，因为字符串的大小是固定的。我们可以使用 C++ 标准库 `cstring` 或 `string.h` 来实现这一点。
