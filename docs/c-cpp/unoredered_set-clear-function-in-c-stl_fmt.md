# C++ STL 中的 `unordered_set::clear()` 函数

> 原文：[https://www.geeksforgeeks.org/unordered_set-clear-function-in-c-stl/](https://www.geeksforgeeks.org/unordered_set-clear-function-in-c-stl/)

`unordered_set::clear()` 函数是 C++ STL 中的一个内置函数，用来清除一个 `unordered_set` 容器。也就是说，这个函数从无序集中移除所有元素并清空它。所有迭代器、指针和对容器的引用都将失效。这将容器的大小减小到零。
...
以下程序说明了 `unordered_set::clear()` 功能：
