...
> 原文：[https://www.geeksforgeeks.org/convert-character-array-to-string-in-c/](https://www.geeksforgeeks.org/convert-character-array-to-string-in-c/)
...
[`std::string`](https://www.geeksforgeeks.org/stdstring-class-in-c/) 有一个内置的构造函数可以为我们完成这项工作。这个构造函数接受一个以空字符结尾的字符序列作为输入。然而，我们只能在声明字符串时使用此方法，并且不能对同一个字符串再次使用，因为它使用的构造函数仅在声明字符串时调用。

### 方法思路

1.  获取字符数组及其大小。
2.  声明一个字符串（即字符串类的一个对象），并在这样做的同时将字符数组作为其构造函数的参数。
3.  使用语法：`string_name(character_array_name)`。
4.  返回字符串。
...
另一种方法是使用重载的 `=` 运算符，它在 [C++ `std::string`](https://www.geeksforgeeks.org/stdstring-class-in-c/) 中也是可用的。
