...
> 原文:[https://www.geeksforgeeks.org/where-is-an-object-stored-if-it-is-created-inside-a-block-in-c/](https://www.geeksforgeeks.org/where-is-an-object-stored-if-it-is-created-inside-a-block-in-c/)
...
*   但是如果我们在运行时创建对象，即通过`动态内存分配`，那么该对象将存储在`堆`上。这是在`new`操作员的帮助下完成的。在这种情况下，我们需要使用`delete`操作符显式销毁对象。
