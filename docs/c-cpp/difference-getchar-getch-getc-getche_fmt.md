...
> 原文：[https://www.geeksforgeeks.org/difference-getchar-getch-getc-getche/](https://www.geeksforgeeks.org/difference-getchar-getch-getc-getche/)

所有这些函数从输入中读取一个字符并返回一个整数值。返回该整数以容纳用于指示失败的特殊值。`EOF` 值通常用于此目的。

## getc()

它从给定的输入流中读取单个字符，并在成功时返回相应的整数值（通常是读取字符的 ASCII 值）。失败时返回 `EOF`。
