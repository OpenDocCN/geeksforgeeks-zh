...
给定字符串 `str`，任务是检查一个字符串在 C++ 中是否是 pangram。

> 如果一个字符串包含所有的英文字母，那么这个字符串就是一个 [Pangram](https://www.geeksforgeeks.org/pangram-checking/)。
...
>
> **输入:** `str = "We promptly judged antique ivory buckles for prize"`
> **输出:** 否
...
3.  在完成字符串的遍历和标记后，遍历哈希并查看是否所有字符都存在，即每个索引都为 `true`。如果都标记了，那么返回 `true`，否则返回 `false`。
...
STL 的 [`transform()`](https://www.geeksforgeeks.org/transform-c-stl-perform-operation-elements/) 方法可以用来检查给定的字符串是否是 Pangram。
...
> **步骤 4:** 现在检查计数是否等于 27，然后字符串包含所有 26 个字母。
