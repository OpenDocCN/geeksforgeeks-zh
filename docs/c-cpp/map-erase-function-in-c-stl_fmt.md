...
**参数:** 该功能接受一个强制参数 `key`，指定要在 `map` 容器中擦除的键。

**返回值:** 如果在 `map` 中找到关键元素，函数返回 1，否则返回 0。
...
**参数:** 函数接受一个强制参数 `position`，该位置指定了迭代器，该迭代器是对要擦除的元素位置的引用。
...
**参数:** 该功能接受两个强制参数，如下所述:
*   `position1` – 指定迭代器，该迭代器是要移除的元素的引用。
*   `position2` – 指定迭代器，该迭代器是要移除的元素的引用。
