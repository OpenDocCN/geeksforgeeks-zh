...
> 原文：[Error Handling During File Operations in C/C++](https://www.geeksforgeeks.org/error-handling-during-file-operations-in-c-c/)
...
*   当试图读取不存在的文件时。
*   尝试使用尚未打开的文件时。
*   当试图以不适当的模式使用文件时，即，将数据写入已打开进行读取的文件。
*   写入写保护文件时，即试图写入只读文件时。
...
`ferror()` 函数检查流中的任何错误。如果没有发生错误，则返回零值，如果有错误，则返回非零值。除非通过 `clearerr()` 函数清除，否则错误指示将持续到文件关闭。
...
`clearerr()` 清除流所指向的流的错误。使用该函数是因为错误指示器不会自动清除。一旦设置了特定流的错误指示器，对该流的操作将继续返回错误值，直到调用 `clearerr()` 、 `fseek()` 、 `fsetpos()` 或 `rewind()` 为止。
