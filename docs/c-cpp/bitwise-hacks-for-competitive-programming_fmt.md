...
> 原文: [https://www.geeksforgeeks.org/bitwise-hacks-for-competitive-programming/](https://www.geeksforgeeks.org/bitwise-hacks-for-competitive-programming/)

建议先参考[关于按位运算符的有趣事实](https://www.geeksforgeeks.org/interesting-facts-bitwise-operators-c/)。

## 1. 如何在数字 `num` 中设置一位

如果我们想在数字 `num` 的第 `n` 个位置设置一位，可以使用“OR”运算符 (`|`) 来完成。

*   首先，我们通过 `(1 << pos)` 将 `1` 左移 `pos` 位。
...
请注意，我们首先左移了“1”，然后使用“与”运算符在该位置获取位。因此，如果 `num` 中的 `pos` 位置有“1”，则“AND”之后的 `bit` 变量将存储“1”，否则，如果数字 `num` 中的 `pos` 位置有“0”，则“AND”之后的 `bit` 变量将存储“0”。
