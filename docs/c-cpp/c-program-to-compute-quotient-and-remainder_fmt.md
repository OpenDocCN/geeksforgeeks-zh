...
> 原文:[https://www.geeksforgeeks.org/c-program-to-compute-quotient-and-remainder/](https://www.geeksforgeeks.org/c-program-to-compute-quotient-and-remainder/)
...
在下面的程序中，为了找到这两个数字的商和余数，首先要求用户输入两个数字。使用 `scanf()` 功能扫描输入，并存储在变量 `A` 和 `B` 中。然后，使用算术运算符 `/` 对变量 `A` 和 `B` 进行除法，以获得作为结果存储在变量 `商` 中的商；并使用算术运算符 `%` 将余数作为结果存储在变量 `余数` 中。
