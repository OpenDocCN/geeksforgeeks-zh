# `std::is_standard_layout` 是 C++ 中的模板

> 原文: [https://www.geeksforgeeks.org/is_standard_layout-template-in-c/](https://www.geeksforgeeks.org/is_standard_layout-template-in-c/)

C++ STL 的 `std::is_standard_layout` 模板用于检查类型是否为标准布局。它返回一个布尔值来显示检查结果。
...
此模板包含单参数 `T` (Trait 类) 检查 `T` 是否为标准布局类型。
