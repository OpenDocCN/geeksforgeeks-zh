...
在上面的程序中，有一个泛型类 `A`，它取一个非类型参数 `N`，类 `B` 继承自泛型类 `A` 的一个实例，`A` 的这个实例的 `N` 值为 2。`B` 类超越了 `A<2>` 的 `fun()`。`C` 类从 `B` 继承而来，在 `main()` 中，有一个 `A` 类型的指针 `A<2>*` 指向 `C` 的一个实例，调用 `a->fun()` 时，`B` 类的函数被执行，因为 `fun()` 是虚函数，虚函数是根据实际对象调用的，而不是根据指针调用的。`C` 类中没有函数 `fun()`，所以在层次结构中查找，在 `B` 中找到。
