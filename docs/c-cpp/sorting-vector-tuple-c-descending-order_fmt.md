...
> 原文: [https://www.geeksforgeeks.org/sorting-vector-tuple-c-descending-order/](https://www.geeksforgeeks.org/sorting-vector-tuple-c-descending-order/)

## 什么是元组向量？
一个`元组`是一个可以容纳多个元素的对象，一个包含多个这样的元组的`向量`被称为元组向量。元素可以是不同的数据类型。元组的元素按照被访问的顺序被初始化为参数。
...
### 情况 2: 在元组的第二元素的基础上，按降序对向量元素进行排序。
