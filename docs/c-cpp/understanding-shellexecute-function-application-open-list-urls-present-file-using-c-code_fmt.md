# 了解 ShellExecute 函数及其应用

> 原文：[https://www.geeksforgeeks.org/understanding-shellexecute-function-application-open-list-urls-present-file-using-c-code/](https://www.geeksforgeeks.org/understanding-shellexecute-function-application-open-list-urls-present-file-using-c-code/)
...
**输出：**
```
The site opened in your browser.
```
...
**lpOperation**
类型：`LPCTSTR`
指向空终止字符串的指针，在本例中称为动词，它指定要执行的操作。可用动词集取决于特定的文件或文件夹。通常，对象快捷菜单中可用的操作是可用的动词。**在我们的例子中，使用“打开”来由 `lpFile` 参数指定的项目。项目可以是文件或文件夹。**
...
**输入：**
```
A list of URLs stored in a text file.
```
**输出：**
```
All the URLs opened in different tabs.
```
