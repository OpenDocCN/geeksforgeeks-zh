...
> 原文: [https://www.geeksforgeeks.org/multidimensional-arrays-c-cpp/](https://www.geeksforgeeks.org/multidimensional-arrays-c-cpp/)

[数组基础](https://www.geeksforgeeks.org/arrays-in-c-language-set-1-introduction/)
...
`data_type`：要存储在数组中的数据类型。这里 `data_type` 是有效的 C/C++ 数据类型。
`array_name`：数组的名称。
...
类似地，数组 `int x[5][10][20]` 可以存储总计 `(5*10*20) = 1000` 个元素。
