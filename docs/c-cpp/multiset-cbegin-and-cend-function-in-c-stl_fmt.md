...
> 原文：[https://www.geeksforgeeks.org/multiset-cbegin-and-cend-function-in-c-stl/](https://www.geeksforgeeks.org/multiset-cbegin-and-cend-function-in-c-stl/)

`multiset::cbegin()` 是 C++ STL 中的内置函数，它返回指向容器中第一个元素的常量迭代器。迭代器不能用于修改集合容器中的元素。迭代器可以相应地增加或减少来遍历集合。
...
`multiset::cend()` 是 C++ STL 中的一个内置函数，它返回一个指向容器中最后一个元素之后的位置的常量迭代器。迭代器不能用于修改集合容器中的元素。迭代器可以相应地增加或减少遍历集合。
