# __builtin_inf() GCC 编译器的功能

> 原文: [https://www.geeksforgeeks.org/__builtin_inf-functions-of-gcc-compiler/](https://www.geeksforgeeks.org/__builtin_inf-functions-of-gcc-compiler/)
...
**注意:** 如果编译器显示溢出警告，在 `__builtin_inf()` 函数之前使用[类型转换](https://www.geeksforgeeks.org/type-conversion-c/)作为：

```cpp
(data_type)__builtin_inf()
```

## 1. `__builtin_inf(void)`
这个函数返回正无穷大，然后转换成 C 语言 `limits.h` 头文件宏的 `DBL_MAX`。它的返回数据类型是 `double`。
...
## 2. `__builtin_infd32(void)`
此函数返回 4 字节整数的最大值，返回的数据类型为 32 位整数。
...
## 3. `__builtin_infd64(void)`
这个函数返回最高值，可以用 8 字节整数来表示，这是我们在 C++ 中可以使用的最高整数值。
...
## 4. `__builtin_infd128(void)`
由于在 C/C++ 中整数可以使用的最大大小是 64 位，所以这个函数给出的结果与 `__builtin_infd64()` 相同。
...
## 5. `__builtin_inff(void)`
此函数返回的浮点最大值表示 4 字节的最大小数点值。
...
## 7. `__builtin_infn(void)` & `__builtin_infnx(void)`
在 GCC 编译器的最新版本中，这是两个新的函数。可以用 `32` 或 `64` 代替 `n` ，分别返回 32 位、64 位的 `inf` 值。
