...
> 原文：[https://www.geeksforgeeks.org/overloading-subscript-or-array-index-operator-in-c/](https://www.geeksforgeeks.org/overloading-subscript-or-array-index-operator-in-c/)
...
**解释：**
在上面的例子中，由于下标操作符的排他属性，两个“cout”语句都提供了类似的输出。编译器以类似的方式读取两个语句，因此 `*(name + 5)` 和 `*(5 + name)` 没有区别。

**正负下标**
数组的第一个元素存储在索引 0 处。C++ 数组的范围是从 `array[0]` 到 `array[size–1]`。但是，C++ 支持正下标和负下标。负数下标必须在数组边界内；如果他们不这样做，结果是不可预测的。下面的代码显示了正负数组下标：
