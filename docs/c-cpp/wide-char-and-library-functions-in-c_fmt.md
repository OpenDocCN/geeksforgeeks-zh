...
> 原文：[https://www.geeksforgeeks.org/wide-char-and-library-functions-in-c/](https://www.geeksforgeeks.org/wide-char-and-library-functions-in-c/)
...
语法：`size_t wcslen(const wchar_t* wcs);`
...
语法：`wchar_t* wcscpy(wchar_t* strDestination, const wchar_t* strSource);`
...
语法：`wchar_t* wcscat(wchar_t* strDestination, const wchar_t* strSource);`
...
语法：`int wcscmp(const wchar_t* wcs1, const wchar_t* wcs2);`
...
语法：`wchar_t* wcstok( wchar_t* str, const wchar_t* delim, wchar_t** ptr);`
...
`ptr` – `wchar_t*` 类型对象的指针，`wcstok` 使用它来存储其内部状态。
这是 `strtok()` 的宽字符等价物。
...
语法：`wchar_t* wcsncpy(wchar_t* destination, const wchar_t* source, size_t n);`

将 `source` 的前 `n` 个宽字符复制到 `destination`。如果在复制 `n` 个字符之前找到了源宽字符串的结尾，目标将被填充额外的空宽字符，直到总共 `n` 个字符。这是 `strncpy()` 的宽字符等价物。
...
语法：`const wchar_t* wcsstr(const wchar_t* wcs1, const wchar_t* wcs2);`
...
**参考：** [http://www.cplusplus.com/reference/cwchar/](http://www.cplusplus.com/reference/cwchar/)
