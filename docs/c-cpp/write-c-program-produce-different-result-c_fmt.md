...
> 原文：`https://www.geeksforgeeks.org/write-c-program-produce-different-result-c/`
...
字符文字在 C 和 C++ 中有不同的处理方式。在像`'a'`、`'b'`这样的 C 字符文字中，被视为整数，而在 C++ 中被视为字符。（详见[本](https://www.geeksforgeeks.org/g-fact-19/)）
...
在 C 语言中，每当我们声明一个结构变量时，都需要使用`struct tag`。在 C++ 中，结构标记不是必需的。例如，让`Student`有一个结构。在 C 语言中，我们必须使用`struct Student`来表示`Student`变量。在 C++ 中，我们可以省略`struct`，只使用`Student`。
...
布尔结果的类型在 C 和 C++ 中是不同的。感谢 Gaurav Jain 提出这一点。
