...
就像 C++ STL 中的 `unordered_map` 一样，我们不能像下面这样获取值：
...
cout << "Key : " << key;
cout << "Value : " << map[key]; // 错误：multimap 不支持下标操作符
```
...
找到给定键的[下界](https://www.geeksforgeeks.org/upper_bound-and-lower_bound-for-vector-in-cpp-stl/)和[上界](https://www.geeksforgeeks.org/upper_bound-and-lower_bound-for-vector-in-cpp-stl/)，并在它们之间遍历。

- `lower_bound(key)`：返回指向大于或等于 `key` 的第一个元素的迭代器。
