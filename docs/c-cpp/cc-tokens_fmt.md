...
关键词是编程语言中预先定义或保留的词。每个关键字都意味着在程序中执行特定的功能。因为关键字是编译器的参考名称，所以它们不能用作变量名，因为这样做，我们试图给关键字赋予一个不允许的新含义。您不能重新定义关键字。但是，您可以在编译前使用 `C/C++` 预处理器指令指定要替换关键字的文本。
...
常量也像正态变量。但是，唯一的区别是，一旦它们被定义，它们的值就不能被程序修改。常数是指固定值。它们也被称为文字。
常量可以属于任何数据类型

**语法:**
`const data_type 变量_name` (或) `const data_type * variable_name；`
...
字符串只不过是以空字符(`“\0”`)结尾的字符数组。这个空字符表示字符串的结尾。字符串总是用双引号括起来。而在 `C` 和 `C++` 中，字符用单引号括起来。
...
以下特殊符号用于 `C` 中，具有特殊含义，因此不能用于其他目的。`[] () {}, ;* = #`
...
运算符是应用于 `C` 变量和其他对象时触发操作的符号。运算符所作用的数据项称为操作数。
