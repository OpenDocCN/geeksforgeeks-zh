...
> 原文：[https://www.geeksforgeeks.org/dynamic-initialization-of-object-in-c/](https://www.geeksforgeeks.org/dynamic-initialization-of-object-in-c/)
...
*   内存在运行时使用[`new`](https://www.geeksforgeeks.org/new-and-delete-operators-in-cpp-for-dynamic-memory/)操作符分配，同样，内存在运行时使用[`delete`](https://www.geeksforgeeks.org/new-and-delete-operators-in-cpp-for-dynamic-memory/)操作符释放。
...
2.  `geeks`类的对象调用该函数，它显示动态分配变量的值，即`ptr`。

以下是使用`new`运算符动态初始化对象的程序：
...
1.  在下面的代码中，`delete`用于动态[释放](https://www.geeksforgeeks.org/g-fact-30/)内存。
2.  使用[赋值运算符](https://www.geeksforgeeks.org/assignment-operators-in-c-c/)将对象`obj2`中的`obj1`的内容[覆盖](https://www.geeksforgeeks.org/overloading-new-delete-operator-c/)，然后使用`delete`运算符解除`obj1`的分配。

下面是使用`delete`操作符动态释放内存的代码。
