...
*   **描述:** `strstr(str1, str2)`返回一个指向`str1`中第一个出现的字符串`str2`的指针。这里`s2`是“for”，`strstr()`返回它在`s1`中第一次出现的地址。
...
*   **描述:** `memset(string, c, n)`将`string`的前 n 个字符设置为‘c’。在本程序中，`string`中的第一个“8”字符将被设置为“*”。我们经常在支票上看到这种文本，我们想在那里隐藏一些数据。关于`memset`的更多细节，请参考这里的
