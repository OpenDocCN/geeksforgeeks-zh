...
> 原文: [https://www.geeksforgeeks.org/difference-between-static-and-dynamic-memory-allocation-in-c/](https://www.geeksforgeeks.org/difference-between-static-and-dynamic-memory-allocation-in-c/)

`内存分配`是为计算机程序和服务分配物理或虚拟内存空间的过程。内存分配在程序执行之前或执行时完成。内存分配有两种类型:
...
**静态内存分配:** 编译器为声明的变量分配静态内存。使用 `&` 运算符的地址可以找到该地址，并且可以将其分配给指针。内存在编译时分配。

**动态内存分配:** 执行时(运行时)完成的内存分配称为`动态内存分配`。函数 `calloc()` 和 `malloc()` 支持分配动态内存。在动态分配中，当值由函数返回并分配给指针变量时，通过使用这些函数来分配内存空间。

**C 中静态和动态内存分配的表格差异:**

| 序列号 | Static memory allocation | Dynamic memory allocation |
| :--- | :--- | :--- |
...
| three | It uses `stack` to manage the static allocation of memory. | It uses `heap` to manage the dynamic allocation of memory. |
...
| Eleven | **Example:** This static memory allocation is generally used for `array`. | **Example:** This dynamic memory allocation is generally used for `linked list`. |