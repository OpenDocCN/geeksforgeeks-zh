...
C 中常规标签有函数范围。其中作为本地标签的范围可以是内部嵌套块。因此，在一个函数中不能多次声明传统标签，而这正是使用本地标签的地方。
...
在这样的函数 `foo()` 中，函数宏将被扩展两次。这会导致一个函数中有多个标签 `x` 的定义，给编译器带来混乱，导致编译错误。在这种情况下，本地标签是有用的。
...
本文由喀什巴蒂亚供稿。如果您发现任何不正确的地方，或者您想分享更多关于上面讨论的主题的信息，请写评论。