## c++ STL 中的无序集 `begin()` 函数

> 原文：[https://www.geeksforgeeks.org/unordered_set-begin-function-in-c-stl/](https://www.geeksforgeeks.org/unordered_set-begin-function-in-c-stl/)

`unordered_set::begin()` 方法是 C++ STL 中的一个内置函数，用于返回指向无序集容器中第一个元素的迭代器。无序集的所有迭代器只能用来访问元素，迭代器不允许修改无序集容器中的元素。
...
下面程序举例说明 `unordered_set::begin()` 功能：
```cpp
