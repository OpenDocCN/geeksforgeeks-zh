...
很多时候我们在编程的时候，需要计算两个数之间的最小公倍数（LCM）。在这个帖子中，我们已经讨论了如何找到 LCM。代替定义然后使用计算 LCM 的函数，我们可以简单地使用一个内置的函数 [`boost::math::lcm()`](http://contribute.geeksforgeeks.org/advance-c-with-boost-library/) C++ 库。
...
**参数：** `m`, `n`
**返回值：** 如果 `m` 或 `n` 为零则返回 0，否则返回 `mod(m)` 和 `mod(n)` 的 LCM。
...
2.  如果任何一个数字是非整数数据类型，那么此函数将抛出一个错误。
...
3.  在 C++ 17 中，引入了一个用于计算两个数字 LCM 的新 STL 函数 [`std::lcm()`](https://www.geeksforgeeks.org/stdlcm-in-cpp17/)，可以在支持 C++ 17 特性的任何编译器上使用。
