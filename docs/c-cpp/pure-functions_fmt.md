...
如果一个函数总是为相同的参数值返回相同的结果，并且它没有像修改参数（或全局变量）或输出某些东西那样的副作用，那么它被称为[纯函数](http://en.wikipedia.org/wiki/Pure_function)。调用纯函数的唯一结果是返回值。纯函数的例子有 `strlen()`，`pow()`，`sqrt()` 等。不纯函数的例子有 `printf()`，`rand()`，`time()` 等。

## 编译器优化

如果一个函数被称为纯函数，那么[循环优化](http://en.wikipedia.org/wiki/Loop_optimization)和[子表达式消除](http://en.wikipedia.org/wiki/Common_subexpression_elimination)可以应用于它。在 GCC 中，我们可以使用 `pure` 属性将函数标记为纯函数。
