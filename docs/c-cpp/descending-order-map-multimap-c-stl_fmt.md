...
> 原文：[https://www.geeksforgeeks.org/descending-order-map-multimap-c-stl/](https://www.geeksforgeeks.org/descending-order-map-multimap-c-stl/)

我们已经讨论了 C++ STL 中的[`map`](https://www.geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/)和 C++ STL 中的[`multimap`](https://www.geeksforgeeks.org/multimap-associative-containers-the-c-standard-template-library-stl/)。这些数据结构的默认行为是以升序存储元素。在 `map` 和 `multimap` 中插入元素时如何保证逆序或降序？

想法是当 `map`/`multimap` 的一个实例。

## Map 中降序

`map` 存储键值对。一个自平衡的 BST（通常是红黑树）被用来实现它。
...
`multimap` 类似于 `map`，只是多元素可以有相同的键。在这种情况下，键值和映射值对必须是唯一的，而不是每个元素都是唯一的。
