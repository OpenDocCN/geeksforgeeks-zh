...
从头到尾遍历链表。对于每个新遇到的元素，检查`node->data > node->next->data`。如果为真，对每个节点执行相同的操作，否则返回 0 并打印“No”。
...
递归检查`node->data > node->next->data`，如果不是，返回 0，这是我们从递归中得出的终止条件，否则为下一个节点递归调用 `Check_List` 函数。
