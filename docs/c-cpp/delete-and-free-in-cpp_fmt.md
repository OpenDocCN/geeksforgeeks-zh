...
`delete`和`free()`在编程语言中具有相似的功能，但它们不同。在 C++ 中，`delete`操作符应该只用于指向使用`new`操作符分配的内存的指针或空指针，而`free()`应该只用于指向使用`malloc()`分配的内存的指针或空指针。
...
| It should only be used as a pointer to the memory allocated by the `new` operator, or as a null pointer. | It should only be used as a pointer or null pointer to the memory allocated using `malloc()`. |
