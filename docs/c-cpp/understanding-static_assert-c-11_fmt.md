# 理解 C++ 11 中的 static_assert

> 原文：[https://www.geeksforgeeks.org/understanding-static_assert-c-11/](https://www.geeksforgeeks.org/understanding-static_assert-c-11/)
...
这个 `#error` 指令很好地处理简单的任务。然而，当需要为复杂的任务进行编译时断言时，例如使用 `sizeof` 检查数据类型的大小，它会失败。这是因为像 `sizeof` 这样的标记在预处理转换阶段之后才会转换为源标记，此时您不能再使用预处理指令。
...
在上面的代码中，我们创建了一个名为 `Vector` 的模板类，我们不想创建一个小于 4 的向量。因此，在模板主体内部，我们放入一个 `static_assert` 语句来检查正在生成的向量的大小是否大于 3。如果失败，断言将失败，并显示错误消息：“Vector size is too small!”。
这正是在向量类对象 `two` 的声明中发生的情况。传递给它的大小为 `2`，这将导致无法检查条件，从而产生编译时错误，并因此停止编译过程。

## `static_assert` 相比 `#error` 有什么优势？

*   与 `#error` 不同，使用 `static_assert` 的断言发生在预处理转换阶段之后。因此，可以使用 `static_assert` 用 `sizeof` 检查数据类型的大小。
...
"code relies on 'long' being exactly 8 bytes");
int main()
{
    return 0;
}
...
int main()
{
    cout << "Assertion passed. The program didn't produce an error";
    return 0;
}
```
...
`static_assert` 中传递的常量表达式需要是有效的表达式。例如，考虑以下代码：
