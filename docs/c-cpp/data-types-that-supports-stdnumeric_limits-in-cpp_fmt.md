...
> 原文：[https://www.geeksforgeeks.org/data-types-that-supports-stdnumeric_limits-in-cpp/](https://www.geeksforgeeks.org/data-types-that-supports-stdnumeric_limits-in-cpp/)

[`numeric_limits`](https://www.geeksforgeeks.org/stdnumeric_limitsmax-and-stdnumeric_limitsmin-in-c/) 类模板提供了一种简单且标准化的方式来查询算术类型的各种属性。例如，一个类型 `T` 可以存储的最大值是 `std::numeric_limits<T>::max()`。

**示例：**

*   `std::numeric_limits<int>::max()` 给出了我们可以在 `int` 类型中存储的最大可能值。
*   `std::numeric_limits<unsigned int>::max()` 给出了我们可以在 `unsigned int` 类型中存储的最大可能值。

`numeric_limits` 有许多成员函数、成员常量和助手[类](https://www.geeksforgeeks.org/classes-objects-java/)，其中一些是特定于类型的。例如 `std::numeric_limits<T>::lowest()` 仅适用于浮点数据类型。最常见的数据类型查询有：

*   `std::numeric_limits<T>::is_integer`：如果 `T` 是整数类型，则为 `true`，浮点类型则为 `false`。
*   `std::numeric_limits<T>::is_signed`：所有有符号算术类型 `T` 为 `true`，无符号类型为 `false`。
*   `std::numeric_limits<T>::digits`：是基于基数的位数。例如，-1 代表 `bool`，7 代表 `char`，31 代表 `int`，等等。
*   `std::numeric_limits<T>::digits10`：是 `T` 类型可以无变化表示的基数为 10 的位数。
*   `std::numeric_limits<T>::max_digits10`：是唯一表示类型 `T` 所有不同值所需的基数为 10 的位数。
*   此外，还会查询最小值、最大值、最低值、字节大小和位数。

浮点数具有与 `epsilon` 相关的查询。除此之外，`numeric_limits` 还有一些功能。例如：`has_infinity`、`traps`、`round_error` 等。
