...
> 原文：[https://www.geeksforgeeks.org/output-of-c-programs-set-25-macros/](https://www.geeksforgeeks.org/output-of-c-programs-set-25-macros/)
...
*   **说明：** 与第一个问题不同，这里程序中的值需要在宏中替换。一旦控件到达 `area`，它就被宏代码替换，

```cpp
cout<< "macro expression area: " << length * width;
```

然后 `length` 和 `width` 的值被替换并计算。
...
*   **描述：** 是不是 `sqrt(8)`？但是 `sqrt(a+5)` 将替换为 `(a+5 * a+5)`；结果为 23。为了使其完美，宏命令应替换为
