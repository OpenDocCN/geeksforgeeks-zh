...
> 原文：[https://www.geeksforgeeks.org/installing-mingw-tools-for-c-c-and-changing-environment-variable/](https://www.geeksforgeeks.org/installing-mingw-tools-for-c-c-and-changing-environment-variable/)
...
**第四步：** 在安装管理器中右键单击每个选项，然后单击 `标记安装`。

![Mark for installation](img/f409c636b0810d289db3fd6947a76559.png)

**第五步：** 在安装中，选项-单击应用更改。然后选择 `应用`。将开始下载所有文件。需要 5-6 分钟。完成后点击 `关闭`。
...
**步骤 1：** 转到设备上的 `C 盘` 并搜索 `MinGW` 文件夹。在 `MinGW` 文件夹中，转到 `bin` 文件夹并复制其路径。
...
**第三步：** 在系统变量中搜索 `Path`，然后选择 `编辑`。现在通过点击 `新建` 向其添加新路径。现在粘贴路径，点击确定。
