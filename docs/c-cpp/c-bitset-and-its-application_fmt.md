...
一个位集是一个布尔数组，但是每个布尔值并不是单独存储的，而是位集优化了空间，使得每个布尔只占用 1 位空间，因此位集`bs`占用的空间小于布尔`bs[N]`和向量`bs(N)`的空间。然而，位集的一个限制是，`N`在编译时必须是已知的，即常数(向量和动态数组没有这个限制)。
...
为位集类定义的主要功能是运算符`[]`、`count()`、`size()`、`set()`、`reset()`以及更多，它们将在下面的代码中解释–
