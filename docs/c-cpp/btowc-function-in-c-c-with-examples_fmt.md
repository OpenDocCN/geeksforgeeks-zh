...
> 原文：[https://www.geeksforgeeks.org/btowc-function-in-c-c-with-examples/](https://www.geeksforgeeks.org/btowc-function-in-c-c-with-examples/)

`btowc()` 是 C/C++ 中的一个内置函数，它将一个字符转换为它的宽字符等价物。在 C++ 的 `<cwchar>` 头文件中定义。
...
该函数接受单个强制参数 `ch`，该参数指定要转换为其宽字符的单字节字符。
