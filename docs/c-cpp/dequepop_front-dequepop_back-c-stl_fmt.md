...
> 原文链接：[https://www.geeksforgeeks.org/dequepop_front-dequepop_back-c-STL/](https://www.geeksforgeeks.org/dequepop_front-dequepop_back-c-STL/)

[Deque](https://www.geeksforgeeks.org/deque-cpp-stl/)（双端队列）是具有两端伸缩特性的序列容器。它们类似于向量，但是在末尾和开头插入和删除元素的情况下效率更高。与向量不同，可能无法保证连续的存储分配。
...
**结果：**
移除名为 `dequename` 的给定 deque 最前端的值。
...
使用 `push_front()` 功能输入一个空的 deque，并打印 deque 的反面。
...
**结果：**
移除名为 `dequename` 的给定 deque 最末尾的值。
...
使用 `push_front()` 函数输入一个空的 deque，输入以下数字和顺序，打印 deque 的反面。
