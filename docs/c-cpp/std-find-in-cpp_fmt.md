...
在给定的数字范围内查找元素。返回范围 `[first, last)` 中比较等于 `val` 的第一个元素的迭代器。如果没有找到这样的元素，函数返回 `last`。
...
**first, last:**
将迭代器输入到序列的初始和最终位置。搜索的范围是 `[first, last)`，包含 `first` 和 `last` 之间的所有元素，包括 `first` 指向的元素，但不包括 `last` 指向的元素。

**val:**
要在范围内搜索的值

**返回值:**
范围内第一个元素的迭代器，比较等于 `val`。如果没有元素匹配，函数返回 `last`。
