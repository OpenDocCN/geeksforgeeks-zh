...
1.  **Zero ( 0 )**：当发现两个字符串完全相同时，返回值等于零。也就是说，两个字符串中的所有字符都相同。
...
2.  **Greater than zero ( >0 )**：当`leftStr`中第一个不匹配字符的ASCII值大于`rightStr`中对应字符的ASCII值时，返回一个大于零的值。或者我们也可以说：
...
3.  **Less than Zero ( <0 )**：当`leftStr`中第一个不匹配字符的ASCII值小于`rightStr`中对应字符的ASCII值时，返回一个小于零的值。
...
**重要提示：** 当字符串不同时，你会发现`strcmp()`函数返回的值是两种情况下`leftStr`和`rightStr`中第一个不匹配字符的ASCII值之差。
