...
`data()` 函数将字符串的字符写入数组。它返回一个指向数组的指针，该指针是从字符串到数组的转换中获得的。它的返回类型不是有效的 C 字符串，因为没有 `"\0"` 字符被追加到数组的末尾。
...
*   `std::string::data()` 返回字符串所属的数组。因此，调用者不能修改或释放内存。
    ...
    ```cpp
    ptr[2] = 'a';
    // 这将引发错误，因为 ptr 指向一个由字符串拥有的数组，
    // 换句话说，ptr 现在指向一个常量数组，为其赋新值是不允许的。
    ```
...
```cpp
// 将 str 转换为由指针 ptr 指向的数组。
const char* ptr = str.data();

// 在将原始字符串修改为数组后错误地访问 str
str += "hello"; // 使 ptr 失效

// 现在用 ptr 的引用显示 str 将给出垃圾值
cout << ptr; // 将给出垃圾值
```
...
```cpp
// CPP code to illustrate std::string::data()
#include <iostream>
#include <string>
#include <cstring>
using namespace std;
...
{
    // Converts str1 to str2 without appending '\0' at the end
    const char* str2;
    str2 = str1.data();
