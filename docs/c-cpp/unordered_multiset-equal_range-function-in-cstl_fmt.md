...
`unordered_multiset::equal_range()`是 C++ STL 中的内置函数，返回所有元素等于给定的`value`的范围。它返回一对迭代器，其中第一个迭代器指向范围的下限，第二个迭代器指向范围的上限。如果容器中没有等于给定`value`的元素，则返回一对，其中下限和上限都指向容器结束后的位置。
