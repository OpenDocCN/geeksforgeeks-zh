...
> 原文：[https://www.geeksforgeeks.org/fibonacci-sum-subset-elements/](https://www.geeksforgeeks.org/fibonacci-sum-subset-elements/)

给定一个由 `n` 个元素组成的数组，任务是求数组子集的斐波那契和，其中子集的每个元素 `<= k`。
精确地说，求 `F(A_i1)+F(A_i2)+F(A_i3)+…+F(A_ix)`，其中 `(A_i1, A_i2, …, A_ix) <= K` 和 `1 <= (i_1, i_2, …, i_x) <= n`。这里，`F(i)` 是 `i` 第个斐波那契数。
...
**说明：**
在查询 1 中，子集 `{1, 2, 2}` 就是这样一个子集，在这种情况下，子集内的所有元素都是 `<= k`，即 `<= 2`。子集的斐波那契和= `F(1)+F(2)+F(2)= 1+1+1 = 3`。
在查询 2 中，子集 `{1, 2, 3, 4, 2}` 是这样一个子集，其中子集内的所有元素都是 `<= k`，即本例中的 `<= 6`。子集的斐波那契和= `F(1)+F(2)+F(3)+F(4)+F(2)= 1+1+2+3+1 = 8`。

使用两种不同的查询技术来解决这个问题，即：
1.  在线查询，
2.  离线查询

在这两种方法中，唯一常见的步骤是生成第 `n` 个斐波那契数。使用[这个方法](https://www.geeksforgeeks.org/program-for-nth-fibonacci-number/)生成第 `n` 个斐波那契数的有效技术。
...
所以，重点关注的子集是排序数组 `A` 中的 `A_1, A_2, A_3, …, A_x`，斐波那契和为：`F(A_1)+F(A_2)+F(A_3)+…+F(A_x)`。
使用[前缀和数组](https://www.geeksforgeeks.org/prefix-sum-array-implementation-applications-competitive-programming/)高效地求子集 `A_1, A_2, …, A_x` 的和。
如果 `prefixFibSum[i]` 存储斐波那契和直到排序数组 `A` 的第 `i` 个索引，那么，数组子集从 `1` 到 `x` 的斐波那契和，就是 `prefixFibSum[x]`。
这样，子集 `[1…x]` 的斐波那契和 = `prefixFibSum[x]`，`prefixFibSum[x]` 可以计算如下：
`prefixFibSum[x] = prefixFibSum[x-1] + fib(A[x])`。
...
在离线查询中，按照特定的顺序存储查询并计算每个查询的答案，并按照原来指定的顺序存储和输出查询的结果。

将每个查询存储为一对整数，其中该对的第一个成员是该查询的查询参数 `K`，该对的第二个成员是查询最初发生的索引。
...
然后，将查询 1 存储为 `(13, 1)`，其中 `13` 是该查询的 `K` 值，`1` 是指定它是第 `1` 个查询的索引，类似地，查询 2 和查询 3 分别表示为 `(3, 2)` 和 `(8, 3)`。

一旦所有的单个查询被表示为整数对，以递增的方式在 `K` 的基础上对查询对的数组进行排序。

例如，排序后的上述查询看起来像 `{(3, 2), (8, 3), (13, 1)}`。

**查询排序背后的思想:**

查询排序背后的主要思想是，当某个子集的元素对于某些查询 `q_i` 小于 `K` 时，那么对于所有查询 `q_j` (其中 `i < j`，从而 `K_i <= K_j`) 这些元素都存在，因此如果元素数组和查询(按 `K` 排序)都被排序，那么在一个上维护两个指针
...
```
while (arr[i] <= query[j].K) {
     fibonacciSum  = fibonacciSum + fib(arr[i])
     i = i + 1
}
```

因此，当子集中的元素小于或等于当前查询对的第一个成员(即 `K`)时，继续前进到下一个元素，同时将所需的斐波那契数添加到当前总和中。一旦当前元素变得大于当前查询的参数 `K`，则将 `sum` 的当前值存储在大小为 `q` 的辅助数组 `ans` 中(即查询数)，该数组位于当前查询对的第 `2` 个成员所指向的索引处(即当前查询发生的原始索引处)。

```
ans[query[j].original_index] = fibonacciSum
```

最后，打印 `ans` 数组，该数组按照最初出现的顺序存储所有查询的结果。
