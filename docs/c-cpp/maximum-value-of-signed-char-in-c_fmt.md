...
> 原文: [https://www.geeksforgeeks.org/maximum-value-of-signed-char-in-c/](https://www.geeksforgeeks.org/maximum-value-of-signed-char-in-c/)
...
*   可存储在带符号字符数据类型中的最大值通常为 `127`，约为 `2^7–1`（但与[编译器相关](https://www.geeksforgeeks.org/compile-32-bit-program-64-bit-gcc-c-c/)）。
*   可存储在带符号字符中的最大值和最小值作为常数存储在 [`climits`](https://www.geeksforgeeks.org/climits-limits-h-cc/) [头文件](https://www.geeksforgeeks.org/header-files-in-c-cpp-and-its-uses/)中，可分别命名为 `SCHAR_MAX` 和 `SCHAR_MIN`。
*   可以存储在带符号字符数据类型中的最小值通常为 `-128`，即 `-2^7`（但取决于编译器）。
*   在数据类型的[溢出](https://www.geeksforgeeks.org/overflow-in-arithmetic-addition-in-binary-number-system/)或下溢的情况下，该值被环绕。例如，如果 `-128` 存储在有符号字符数据类型中，并从中减去 1，则该变量中的值将等于 `127`。同样，在溢出的情况下，该值将舍回 `-128`。
