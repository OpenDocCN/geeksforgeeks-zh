...
> 原文：[https://www.geeksforgeeks.org/difference-set-multiset-unordered_set-unordered_multiset/](https://www.geeksforgeeks.org/difference-set-multiset-unordered_set-unordered_multiset/)
...
(v) 使用迭代器遍历。
(vi) `set`执行为[二叉查找树](https://www.geeksforgeeks.org/binary-search-tree-set-1-search-and-insertion/)。
...
## 结论
简单来说，`set` 是一个存放 **已排序且唯一的** 元素的容器。如果添加了 `unordered`，则表示元素是 **无序的**。如果增加 `multiset` 表示允许 **重复元素** 存储。
