...
> 原文：[https://www.geeksforgeeks.org/commonly-asked-oop-interview-questions/](https://www.geeksforgeeks.org/commonly-asked-oop-interview-questions/)

## 什么是面向对象编程？
**O**bject-**O**riented **P**rogramming（OOP）是一种编程范式，在这种编程范式中，完整的软件就像一堆相互交谈的对象一样运行。对象是数据和对其数据进行操作的方法的集合。
...
## 什么是封装？
封装是指以下两个概念之一。
1.  数据隐藏：限制对对象成员访问的语言功能。例如，`C++` 中的 `private` 和 `protected` 成员。
...
*运行时多态性：* 虚函数支持运行时多态性。思路是，虚函数是根据指向或引用的对象类型调用的，而不是根据指针或引用的类型调用的。换句话说，虚函数在运行时被延迟解析。
...
## 什么是析构函数？
析构函数是当对象由作用域组成或被破坏时自动调用的方法。析构函数名称也与类名相同，但名称前带有波浪符（`~`）。
...
- [DBMS 面试常见问题|第二集](https://www.geeksforgeeks.org/commonly-asked-dbms-interview-questions-set-2/)
