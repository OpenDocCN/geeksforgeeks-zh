...
*   可以存储在 `int` 数据类型中的最大整数值通常是 `2，147，483，647` ，位于`2^31–1`周围，但与[编译器相关](https://www.geeksforgeeks.org/compile-32-bit-program-64-bit-gcc-c-c/)。
*   可以存储在 `int` 中的最大值作为常量存储在 `<climits>` [头文件](https://www.geeksforgeeks.org/write-header-file-c/)中，头文件的值可以作为 `INT_MAX`。
*   可以存储在 `int` 数据类型中的最小整数值通常为 `-2，147，483，648` ，在 `-2^31` 周围，但与编译器有关。
*   在数据类型的[溢出](https://www.geeksforgeeks.org/check-for-integer-overflow/)或[下溢](https://www.geeksforgeeks.org/check-for-integer-overflow/)的情况下，该值被环绕。例如，如果将 `-2，147，483，648` 存储在 `int` 数据类型中，并从中减去 `1` ，则该变量中的值将等于 `2，147，483，647` 。同样，在溢出的情况下，该值将舍 `-2，147，483，648` 。
