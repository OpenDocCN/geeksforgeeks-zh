...
> 原文:[https://www.geeksforgeeks.org/base-class-pointer-pointing-to-derived-class-object/](https://www.geeksforgeeks.org/base-class-pointer-pointing-to-derived-class-object/)
...
*   要访问基类的`变量`，将使用基类指针。
*   因此，指针是基类的类型，它可以访问基类的所有公共`函数`和`变量`，因为指针是基类的，这被称为绑定指针。
*   在这个指针中，基类归基类所有，但指向派生类`对象`。
*   同样适用于派生类指针，值被更改。
...
*   基类的这个指针将能够锤炼自己类的`函数`和`变量`，并且仍然可以指向派生类对象。