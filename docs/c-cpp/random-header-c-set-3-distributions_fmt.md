...
> 原文: [https://www.geeksforgeeks.org/random-header-c-set-3-distributions/](https://www.geeksforgeeks.org/random-header-c-set-3-distributions/)

我们已经在[第 1 集](https://www.geeksforgeeks.org/random-header-c-set-1generators/)中讨论了生成器。我们还在[第二集](https://www.geeksforgeeks.org/random-header-set-2-distributions/)中讨论了三个分布。在这篇文章中，将讨论更多的分布。
...
- `min`: 返回成员 `operator()` 给出的取值范围的最大下界，在本例中始终为零。
- `max`: 返回成员 `operator()` 给定值范围的最小上限。
- `reset`: 它重置分布，这样对象的后续使用就不依赖于它已经产生的值。
- `param`: 它获取或设置分布参数对象。
...
- `min`: 返回成员 `operator()` 给定范围的最大下界，对于 `exponential_distribution` 总是零。
- `max`: 返回成员 `operator()` 给定范围的最小上限。
- `reset`: 它重置分布，这样在后续的使用中，结果不依赖于它已经产生的值。
- `param`: 它获取或设置分布参数对象。
...
- `operator()`: 它返回一个遵循分布参数的新随机数。
- `min`: 返回成员 `operator()` 给定范围的最大下界，对于 `gamma_distribution`，该下界始终为零。
- `max`: 返回成员 `operator()` 给定范围的最小上限。
- `reset`: 它重置分布，这样在后续的使用中，结果不依赖于它已经产生的值。
- `param`: 它获取或设置分布参数对象。
...
- **`operator()`**: 它返回一个遵循分布参数的新随机数。
- **`min`**: 返回算子`operator()`给定范围的最大下界，对于威布尔分布总是零。
- **`max`**: 返回运算符`operator()`给定范围的最小上限。
- **`reset`**: 它重置分布，这样在后续的使用中，结果不依赖于它已经产生的值。
...
- **`operator()`**: 它生成根据概率函数分布的随机数。
- **`min`**: 返回成员运算符`operator()`给定范围的最大下界。
- **`max`**: 返回成员运算符`operator()`给定范围的最小上限。
- **`reset`**: 它重置分布，这样在后续的使用中，结果不依赖于它已经产生的值。
- **`param`**: It gets or sets the distribution parameter object.
...
- **`operator()`**: 它生成一个遵循这个分布的随机数。
- **`min`**: 返回运算符`operator()`给定范围的最大下界，对数正态分布总是零。
- **`max`**: 返回运算符`operator()`给定范围的最小上限。
- **`reset`**: 它重置分布，这样在后续的使用中，结果不依赖于它已经产生的值。
- **`param`**: It gets or sets the distribution parameter object.
...
- **`operator()`**: 它生成根据概率函数分布的随机数。
- **`min`**: 返回运算符`operator()`给定范围的最大下界，对于卡方分布始终为零。
- **`max`**: 返回运算符`operator()`给定范围的最小上限。
- **`reset`**: 它重置分布，这样在后续的使用中，结果不依赖于它已经产生的值。
- **`param`**: It gets or sets the distribution parameter object.
...
是根据一个 Fisher F-distribution 产生浮点值的随机数分布，由:
![ p \left ( x | m,n \right ) = \frac{\Gamma \left ( \frac{m+n}{2} \right )}{\Gamma\left ( \frac{m}{2} \right ). \Gamma \left ( \frac{n}{2} \right )}. \frac{\left ( \frac{mx}{n} \right )^{\frac{m}{2}}}{x.\left ( 1 + \frac{mx}{n} \right )^{\frac{m+n}{2}}},\ x \geq 0  ](img/db299ebb63edcfd961ef7b0e866afff8.png "Rendered by QuickLaTeX.com")给出，它通过划分 `m` 和 `n` 个自由度的两个独立卡方分布产生随机数。

- **`operator()`**: 它生成根据概率函数分布的随机数。
- **`min`**: 返回运算符`operator()`给定范围的最大下界，对于`fisher_f_distribution`，该下界始终为零。
- **`max`**: 返回运算符`operator()`给定范围的最小上限。
- **`reset`**: 它重置分布，这样在后续的使用中，结果不依赖于它已经产生的值。
- **`param`**: It gets or sets the distribution parameter object.
...
*   **min**: 返回运算符()给定范围的最大下限。
*   **max**: 返回运算符()给定范围的最小上限。
*   **重置**: 它重置分布，这样在后续的使用中，结果不依赖于它已经产生的值。
*   **param**: It gets or sets the distribution parameter object.
