...
> 原文：[https://www.geeksforgeeks.org/how-to-write-long-strings-in-multi-lines-cc/](https://www.geeksforgeeks.org/how-to-write-long-strings-in-multi-lines-cc/)
...
输出：
```
这些是 C 语言中的保留字，有 int、float、if、else、for、while 等。标识符是字母和数字的序列，但必须以字母开头。下划线(_)被视为字母。标识符区分大小写。标识符用于命名变量、函数等。
```
