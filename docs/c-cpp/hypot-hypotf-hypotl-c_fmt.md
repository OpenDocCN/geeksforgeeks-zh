...
C++ 中的 `hypot()` 函数返回传递的参数平方和的平方根。它找到了斜边，`斜边`是直角三角形最长的边。其计算公式如下：
...
> **头文件**：`cmath`
> **参数**：`hypot()` 取 2 或 3 个整型或浮点型参数。
> **返回**：
> 1. 如果传递两个参数，直角三角形的斜边。
> 2. 如果传递了三个参数，从原点到 `(x, y, z)` 的距离。

### 异常或错误

1. `hypot(x, y)`、`hypot(y, x)` 和 `hypot(x, -y)` 是等价的。
2. 如果其中一个参数为 0，则 `hypot(x, y)` 相当于用非零参数调用的 `fabs()`。
3. 如果其中一个参数为 `无穷大` 或未定义，则 `hypot(x, y)` 返回 `undefined`。
...
`hypotf()` 的 C++ 程序实现这里，变量被赋值为浮点类型否则 `类型不匹配` 错误发生。
...
`hypotl()` 函数与 `hypot` 函数相同。唯一的区别是函数的参数和返回类型是长双精度类型。附加到 `hypotl` 的 `l` 字符代表 `long double`，它表示函数的参数类型和返回类型。
...
`hypotl()` 的 C++ 程序实现这里，变量被赋值为长双类型否则 `类型不匹配` 错误发生。
