...
> 原文：[https://www.geeksforgeeks.org/find-a-number-containing-n-1-set-bits-at-even-positions-from-the-right/](https://www.geeksforgeeks.org/find-a-number-containing-n-1-set-bits-at-even-positions-from-the-right/)

给定一个正整数 `N`，任务是找到一个包含 `(N–1)` 组位的数字，该数字在从右边开始的每个偶数索引处（基于 1）以二进制形式设置位。

**例：**

> **输入：** `N = 2`
> **输出：** `2`
> 2 的二进制表示为 `10`，其中 `1` 设置位位于右侧的偶数位置。
> **输入：** `N = 4`
> **输出：** `42`
> 42 的二进制表示为 `101010`
...
# Function to return the generated string
# by appending "10" n-1 times
...
| n | Decimal equivalent | Binary equivalent | Base 4 |
| :--- | :--- | :--- | :--- |
| 1 | 0 | 0 | 0 |
| 2 | 2 | 10 | 2 |
| 4 | 42 | 101010 | 222 |
...
我们实际上是在 base4 中为每个 `n` 追加 `2`，即对于 `n = 7` 而言，我们在 base4 中的数字应该是 `(n–1)` 个连续的 `2`。

现在我们要记住一点，因为我们知道，如果我们从任何基数 `m` 转换到基数 `10` 即小数，那么解就是 `(n0 * m^0 + n1 * m^1 + N2 * m^2 + … + n * m^n)`。所以由于我们的基数是 `4`，通过进一步的计算我们可以发现我们需要的个数 `n` 可以利用 `O(1)` 时间复杂度中的推导公式来找到。
