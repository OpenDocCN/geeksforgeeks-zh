...
> 原文：[https://www.geeksforgeeks.org/wcsspn-function-in-c-c-with-examples/](https://www.geeksforgeeks.org/wcsspn-function-in-c-c-with-examples/)

`wcsspn()` 是 C/C++ 中的内置函数，它返回宽字符串的最大初始段长度，该长度由另一个宽字符串中存在的字符组成。在 C++ 的 `<cwchar>` 头文件中定义。
...
该函数返回 `des` 的最大初始段长度，该长度只包含 `src` 指向的宽字符串中的宽字符。
