...
> 原文: [https://www.geeksforgeeks.org/synchronous-chatting-application-using-c-boostasio/](https://www.geeksforgeeks.org/synchronous-chatting-application-using-c-boostasio/)

[`Boost 库`](https://www.geeksforgeeks.org/advanced-c-boost-library/) 由 `asio` 组成，这是一个免费的跨平台 C++ 库，用于网络和低级 I/O 编程，使用现代 C++ 方法提供一致的异步模型。本文将帮助开发一个使用 `boost::asio` 的客户端-服务器同步聊天应用程序。我们明确提到“同步”，因为在同步模型中，我们的一个客户机或服务器必须等待另一个。
...
*   `read_until()` 方法在通信过程中从存储数据的缓冲区获取消息。这里我们使用 `"\n"` 作为输出分隔符，这意味着我们将继续从缓冲区读取数据，直到遇到 `"\n"` 并存储它。
...
*   `read_until()` 和 `write()` 对于我们的客户端应用程序以及服务器端都将保持不变。
