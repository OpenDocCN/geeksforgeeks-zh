...
`wmemset()` 函数是 C/C++ 中的一个内置函数，它将一个宽字符复制指定的次数到一个宽字符数组中。它在 C++ 中的 `<cwchar>` 头文件中定义。
...
*   如果 `count` 大于 0，则函数返回 `des`。
*   如果 `count` 小于 0，可能会出现 `Segmentation Fault`。
*   如果 `count` 等于零，则该函数不做任何事情。
