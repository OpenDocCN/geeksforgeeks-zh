# C/C++ 中的决策（if, if..else, Nested if, if-else-if）

> 原文：[https://www.geeksforgeeks.org/decision-making-c-c-else-nested-else/](https://www.geeksforgeeks.org/decision-making-c-c-else-nested-else/)
...
1.  [if 语句](#if)
2.  [if..else 语句](#if-else)
3.  [嵌套 if 语句](#nested-if)
4.  [if-else-if 阶梯](#if-else-if)
5.  [switch 语句](https://www.geeksforgeeks.org/switch-statement-cc/)
6.  [跳转语句](#jmp)
    1.  [break](#brk)
    2.  [continue](#cont)
    3.  [goto](#got)
...
这里评估后的 `condition` 不是真就是假。`if` 语句接受布尔值——如果值为真，那么它将执行它下面的语句块，否则不执行。如果我们没有在 `if(condition)` 之后提供花括号 `{ }`，那么默认情况下，`if` 语句会将紧接在它下面的第一个语句视为在其块内。
...
如果 `if` 语句为假，则 `else` 语句后面的代码块将被执行。
...
这些语句在 C 或 C++ 中用于程序中所有函数的无条件控制流。它们支持四种类型的跳转语句：

### 1. break 语句

[**C break:**](https://www.geeksforgeeks.org/break-statement-cc/) 此循环控制语句用于终止循环。一旦在循环中遇到 `break` 语句，循环迭代就停止在那里，控制立即从循环返回到循环后的第一个语句。
...
[**C continue:**](https://www.geeksforgeeks.org/continue-statement-cpp/) 这个循环控制语句就像 [break 语句](https://www.geeksforgeeks.org/break-statement-cc/)一样。`continue` 语句与 `break` 语句相反，它不是终止循环，而是强制执行循环的下一次迭代。
...
在上面的语法中，第一行告诉编译器转到或跳转到标记为 `label` 的语句。这里的 `label` 是用户定义的标识符，表示目标语句。紧跟在“`label:`”后面的语句是目标语句。“`label:`”也可以出现在“`goto label`”之前。
...
2.  [**C 返回:**](https://www.geeksforgeeks.org/return-statement-in-c-cpp-with-examples/) C 或 C++ 中的 `return` 将执行的流程返回到调用它的函数。这个语句不一定需要任何条件语句。语句一执行，程序流就立即停止，并从调用它的地方返回控件。`return` 语句可以为 `void` 函数返回任何东西，也可以不返回任何东西，但是对于非 `void` 函数，必须返回一个返回值。
    **语法:**
