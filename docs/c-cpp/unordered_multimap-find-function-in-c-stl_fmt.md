# C++ STL 中 `unordered_multimap::find()` 函数

> 原文：[https://www.geeksforgeeks.org/unordered_multimap-find-function-in-c-stl/](https://www.geeksforgeeks.org/unordered_multimap-find-function-in-c-stl/)

`unordered_multimap::find()` 是 C++ STL 中的内置函数，它返回一个迭代器，该迭代器指向具有关键字 `k` 的元素之一。如果容器不包含任何带有关键字 `k` 的元素，那么它返回一个迭代器，该迭代器指向容器中最后一个元素之后的位置。
