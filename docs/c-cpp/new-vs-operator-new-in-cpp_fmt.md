# C++ 中 `new` vs `operator new`

> 原文: [https://www.geeksforgeeks.org/new-vs-operator-new-in-cpp/](https://www.geeksforgeeks.org/new-vs-operator-new-in-cpp/)

创建新对象时，使用 `new` 运算符分配内存，然后调用构造函数来初始化内存。这里，`new` 运算符执行分配和初始化，而 `operator new` 只执行分配。
让我们看看这两者是如何独立工作的。

## `new` 关键字

`new` 运算符是一个**运算符**，表示对堆内存分配的请求。如果有足够的内存，`new` 运算符初始化内存，并将新分配和初始化的内存的地址返回给指针变量。当您使用 `new` 关键字（普通 `new`）创建类对象时：

*   使用堆中的 `operator new` 来分配对象的内存。
*   调用类的构造函数来正确初始化这个内存。
...
*   这是覆盖默认堆分配逻辑的机制。
*   它不初始化内存，即不调用构造函数。然而，在我们重载的 `operator new` 返回之后，编译器会自动调用构造函数。
*   也有可能在全球范围内或针对特定类别使 `operator new` 超载。
...
2.  **`new` 调用 `operator new`:** `new` 运算符调用 `operator new()`，就像 `+` 运算符调用 `operator+()`。
3.  **`operator new` 可以重载:** `operator new` 可以重载，就像允许我们执行定制任务的函数一样。
4.  **内存分配:** `new` 表达式调用 `operator new` 分配原始内存，然后调用构造函数。
