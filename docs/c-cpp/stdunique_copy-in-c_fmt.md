...
[`std::unique`](https://www.geeksforgeeks.org/stdunique-in-cpp/) 用于删除某个范围内连续出现的任何元素的重复项 `[first, last]`。它对连续出现相同元素的范围内的所有子组执行此任务。

但是，如果我们不想改变原来的范围，只想将 `std::unique` 的结果复制到另一个容器中，对此我们定义了另一个函数，即 `std::unique_copy()`。在这种情况下，仅复制范围 `[first, last]` 中每个连续等价元素组的第一个元素。
...
### 1. 移除所有重复元素
`std::unique_copy` 可用于移除所有重复元素（无论是否连续）并将结果存储在另一个容器中，而不影响原容器。
...
### 2. 检查容器是否只包含唯一元素
我们也可以使用 `std::unique_copy` 来检查一个容器是否只包含唯一元素。这只是上述将唯一元素存储在另一个容器中的概念的延伸。之后，我们可以比较这两个容器是否相等。
