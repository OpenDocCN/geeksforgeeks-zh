...
> 原文: [https://www.geeksforgeeks.org/draw-a-chess-board-using-graphics-programming-in-c/](https://www.geeksforgeeks.org/draw-a-chess-board-using-graphics-programming-in-c/)

## 先决条件
[`graphics.h`](https://www.geeksforgeeks.org/add-graphics-h-c-library-gcc-compiler-linux/) | [如何在 CodeBlocks 中包含 `graphics.h`？](https://www.geeksforgeeks.org/include-graphics-h-codeblocks/)
...
1.  [`rectangle(left, top, right, bottom)`](https://www.geeksforgeeks.org/intersecting-rectangle-when-bottom-left-and-top-right-corners-of-two-rectangles-are-given/) : 来自 `graphics.h` 头文件的一个函数，用于绘制矩形。绘制矩形需要左上角和右下角的坐标。`left` 指定左上角的 X 坐标，`top` 指定左上角的 Y 坐标，`right` 指定右下角的 X 坐标，`bottom` 指定右下角的 Y 坐标。
...
5.  [`floodfill()`](https://www.geeksforgeeks.org/setfillstyle-floodfill-c/) : 来自 `graphics.h` 头文件的函数，用于填充封闭区域。
