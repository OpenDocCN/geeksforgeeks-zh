...
*   **复数的极坐标形式：** 可视化和表示复数的另一种方式是极坐标形式。极坐标形式使用复数的模长 `r` 和方向 `θ`。
...
5.  **点积：** 向量 `P` 和 `Q` 的点积是 `(ac + bd)`。
    这与上述构造的实部相同。因此，返回 `z1'z2` 的实部。

6.  **叉积：** 向量 `P` 和 `Q` 的叉积的模长是 `(ad - bc)`。
    这与上述构造的虚部相同。因此，返回 `z1'z2` 的虚部。
