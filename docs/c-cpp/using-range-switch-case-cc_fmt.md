# C/C++ 中 switch case 语句的范围使用

> 原文: [https://www.geeksforgeeks.org/using-range-switch-case-cc/](https://www.geeksforgeeks.org/using-range-switch-case-cc/)

大家都很熟悉 C/C++ 中的[switch 语句](https://www.geeksforgeeks.org/switch-statement-cc/)，但是你知道`在 case 语句中可以用数字范围`代替单个数字或字符。

*   这是 GNU C 编译器的用例范围扩展，而不是标准的 C 或 C++。
*   您可以在单个 case 标签中指定连续值的范围，如下所示:
    `case low ... high:`
...
**时间复杂度:** `O(n)`，其中 `n` 是数组 `arr` 的大小。
**辅助空间:** `O(1)`

**练习:** 可以通过修改 `char` 数组和 `case` 语句来尝试上面的 `char` 数组程序。

**错误条件:**

1.  **`low > high`:** 编译器给出错误信息。
2.  **重叠 case 值:** 如果 `case` 标签的值在 `switch` 语句中已经使用过的 `case` 范围内，编译器会给出错误消息。
