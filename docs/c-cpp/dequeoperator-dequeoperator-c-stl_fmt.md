...
该运算符用于通过替换现有内容向容器分配新内容。还会根据新内容修改大小。
...
该运算符用于引用运算符内部给出的位置处的元素。它与 `at()` 函数类似，唯一的区别是 `at()` 函数在位置不在 deque 大小的界限内时抛出一个超范围异常，而这个运算符导致**未定义的行为**。
