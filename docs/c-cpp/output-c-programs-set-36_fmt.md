...
`setWidth(10.0)` 设置宽度为 10，`printWidth(box)` 打印（宽度 * 2）即 20。
...
**说明：**
类或结构 `c` 继承类 `a` 的同时也继承类 `b`。当我们创建 `c` 类的对象 `p` 时，`a` 和 `b` 都是自动继承的，这意味着我们可以访问 `a` 和 `b` 类的属性。
因此，`p->value` 将被设置为 0，如问题中所给，然后打印 `Inherited` 并退出代码。
