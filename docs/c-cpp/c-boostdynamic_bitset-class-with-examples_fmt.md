...
> 原文: [https://www.geeksforgeeks.org/c-boostdynamic_bitset-class-with-examples/](https://www.geeksforgeeks.org/c-boostdynamic_bitset-class-with-examples/)

[`boost`](https://www.geeksforgeeks.org/advanced-c-boost-library/) 中有 150 多个库，其中一些最常用的库已经包含在 C++ 标准库中。`dynamic_bitset` 是一个用于位操作的强大库。`dynamic_bitset` 类用于表示 0（复位）或 1（设置）形式的一组位。`dynamic_bitset` 是对 `bitset`（[`std::bitset`](https://www.geeksforgeeks.org/c-bitset-and-its-application/) 和 `boost::bitset`）的改进，后者在运行时分配任何所需长度的位，而 `bitset` 的位长度必须在编译时确定。
头文件位于 `boost/dynamic_bitset.hpp`。
...
*   `set()`：它为每个位分配 1。
*   `reset()`：如果参数中传递了 n，它会将第 n 个位指定为 0，否则它会清除整个位集对象。
*   `flip()`：它反转任何给定的位，即从 0 切换到 1，反之亦然。
*   `size()`：返回 `dynamic_bitset` 对象的大小。
*   `resize()`：用于增加或减少对象的大小。
*   `push_back()`：将 `dynamic_bitset` 对象的大小增加 1，并在 MSB 处推送值。
*   `pop_back()`：从 MSB 中删除一位。
*   `num_blocks()`：返回位集中的块数。
*   `append()`：追加最高有效位的位。这将位集的大小增加了每块位。对于范围 [0, bits_per_block) 中的 i，位置 (size + i) 处的位被设置为 ((value >> i) & 1)。
...
*   `test()`：测试第 i 位是否置位。如果设置为 true，则返回 false。
...
*   `operator[]`：返回第 n 位的引用。
*   `operator&=()`：对参数中传递的当前位集对象和位集对象进行按位“与”运算。例如 `B1.operator&=(B2)`；`B1` 和 `B2` 的按位“与”，即 `B1 & B2`（`B1` 和 `B2` 必须具有相同的位数）。
*   `operator|=()`：对参数中传递的当前位集对象和位集对象进行按位“或”运算。例如 `B1.operator|=(B2)`；`B1` 和 `B2` 的按位“或”，即 `B1 | B2`（`B1` 和 `B2` 必须具有相同的位数）。
*   `operator^=()`：它对参数中传递的当前位集对象和位集对象进行按位异或运算。例如 `b1.operator^=(b2)`；`B1` 和 `B2` 的逐位异或，即 `B1 ^ B2`（`B1` 和 `B2` 必须具有相同的位数）。
*   `operator-=`：与参数中传递的当前位集对象和位集对象执行集合差运算。例如 `B1.operator-=(B2)`；设置 `B1` 和 `B2` 的差值，即 `B1–B2`（`B1` 和 `B2` 必须具有相同的位数）。
*   `operator=()`：用传入参数的对象分配当前位集对象。
*   `operator==()`：验证当前位集对象是否与作为参数传递的对象完全相同。
*   `operator!=()`：验证当前位集对象是否不等于作为参数传递的对象。
*   `operator<()`：如果当前位集在字典序上小于作为参数传递的 `bitset` 对象，则返回真，否则返回假。
*   `operator>()`：如果当前位集在字典序上大于作为参数传递的 `bitset` 对象，则返回真，否则返回假。
*   `operator<=()`：如果当前位集在字典序上小于或等于作为参数传递的 `bitset` 对象，则返回真，否则返回假。
*   `operator>=()`：如果当前位集在字典序上大于或等于作为参数传递的 `bitset` 对象，则返回真，否则返回假。
*   `operator~()`：它创建当前位集的副本，其所有位都被翻转。
*   `operator<<()`：创建当前位集对象的一个拷贝，该拷贝左移 n 位。
*   `operator>>()`：创建当前位集对象的一个拷贝，向右移动 n 位。
*   `operator<<=()`：将当前位集对象左移 n 位。
*   `operator>>=()`：将当前位集对象向右移动 n 位。

## 例 2：卡片打印处理机（Card Print Processor 的缩写）
