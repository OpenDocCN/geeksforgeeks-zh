...
> 原文：[`https://www.geeksforgeeks.org/advantages-and-disadvantages-of-function-overloading-in-cpp/`](https://www.geeksforgeeks.org/advantages-and-disadvantages-of-function-overloading-in-cpp/)
...
在上面的例子中，所有被命名为 `print` 的函数都是相同的，但是打印不同，我们可以感觉到对它们进行了不同的调用。这是可能的，因为传递了参数，根据这些参数执行函数调用，而不管这些函数是否共享一个公共名称。此外，请记住，我们可以通过在返回类型中更改它们的签名来重载函数。例如，这里 `bool print()` 函数可以对它们三个都调用。