...
每个参数必须是一个内存地址，转换后的结果将被写入其中。成功后，函数返回填充的变量数。在输入失败的情况下，在能够成功读取任何数据之前，返回 `EOF`。
可用于扫描的类型说明符:
...
成功后，函数返回填充的变量数。在输入失败的情况下，在能够成功读取任何数据之前，返回 `EOF`。
...
`fscanf_s()` 与 `fscanf()` 的区别，和 `scanf()` 与 `scanf_s()` 的区别相同。`fscanf_s()` 是安全函数，安全函数要求将每个 `c`、`C`、`s`、`S` 和 `[` 类型字段的大小作为紧随变量之后的参数传递。
...
`sscanf_s()` 是 `sscanf()` 的安全函数，安全函数要求将每个 `c`、`C`、`s`、`S` 和 `[` 类型字段的大小作为紧随变量之后的参数传递。
