# C++中的`is_permutation()`及其在字谜搜索中的应用

> 原文：[https://www.geeksforgeeks.org/is_permutation-c-application-anagram-search/](https://www.geeksforgeeks.org/is_permutation-c-application-anagram-search/)

`is_permutation()`用于检查两个容器（如字符串和向量）是否相互置换。它接受三个参数，前两个参数是第一个对象的开始和结束位置，第三个参数是第二个对象的开始位置。
...
Input : text = "forxxorfxdofr"
        pat = "for"
Output :  3
There are three anagrams of "for" in text.
...
我们已经讨论了一个解决方案。但是在这篇文章中，它是使用`is_permutation()`完成的。虽然复杂度高于[之前讨论的方法](https://www.geeksforgeeks.org/anagram-substring-search-search-permutations)，但目的是解释`is_permutation()`的应用。
...
本文由[**萨哈布拉**](https://www.facebook.com/sahil.chhabra.965)供稿。如果你喜欢GeeksforGeeks并想投稿，你也可以使用[write.geeksforgeeks.org](https://write.geeksforgeeks.org)写一篇文章或者把你的文章邮寄到`review-team@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
