...
`deque::crbegin` 的意思是 constant _ reverse _ begin，顾名思义，它返回一个指向 `deque` 最后一个元素的 `constant_reverse_iterator`。
...
返回一个 `const_reverse_iterator` 到序列的反向开始。
...
> **代码：**
> ```cpp
> for (auto reverse_it = deque.crbegin(); reverse_it != deque.crend(); ++reverse_it)
>     cout << ' ' << *reverse_it;
> ```
> **输出：** `6 5 4 3 2 1`
...
> **prog.cpp:14:8: 错误：** 赋值只读位置 `‘rit.std::reverse_iterator<_Iterator>::operator*() const’`
> `*rit = 10;`
> `^~~~~`