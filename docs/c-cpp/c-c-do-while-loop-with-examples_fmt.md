...
> 原文: [https://www.geeksforgeeks.org/c-c-do-while-loop-with-examples/](https://www.geeksforgeeks.org/c-c-do-while-loop-with-examples/)

当我们需要重复执行一个语句块时，C/C++ 中的循环开始使用。与 `while` 一样，`do-while` 循环的执行也基于测试条件而终止。`do-while` 循环和 `while` 循环的主要区别在于 `do-while` 循环在循环体的末端测试条件，即 `do-while` 循环是出口控制的，而其他两个循环是入口控制的循环。
