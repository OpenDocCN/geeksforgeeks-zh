...
> 原文: [https://www.geeksforgeeks.org/fill-function-in-c-stl-with-examples/](https://www.geeksforgeeks.org/fill-function-in-c-stl-with-examples/)

C++ STL 中的 `fill()` 函数用于填充容器中的一些默认值。`fill()` 函数也可用于填充容器中某个范围内的值。它接受两个迭代器 `begin` 和 `end`，并从 `begin` 所指的位置开始并刚好在 `end` 所指的位置之前填充容器中的值。
...
*   `End`: This function fills the value to the position before the position pointed by the end of the iterator `end`.
*   `Value`: This parameter indicates the value to be filled by the function in the container.

`注意`: 注意 `begin` 包含在范围内，但 `end` 不包含在内。
