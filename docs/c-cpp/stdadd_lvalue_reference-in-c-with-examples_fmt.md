...
> 原文: [https://www.geeksforgeeks.org/stdadd_lvalue_reference-in-c-with-examples/](https://www.geeksforgeeks.org/stdadd_lvalue_reference-in-c-with-examples/)

`std::add_lvalue_reference` 模板存在于 `<type_traits>` 头文件中，属于 [C++ STL](https://www.geeksforgeeks.org/the-c-standard-template-library-stl/)。`std::add_lvalue_reference` 模板用来获取引用 `T` 类型的左值引用类型。一个左值引用是通过在某个类型后放置一个 `&` 而形成的。右值引用是通过在某个类型后放置一个 `&&` 而形成的。左值不能将（非常数）左值引用绑定到右值。
模板检查 `std::add_lvalue_reference` `std::is_lvalue_reference::value`。
...
**参数:** 模板 `std::add_lvalue_reference` 接受单个参数 `T`（性状类）。

下面是在 [C++](https://www.geeksforgeeks.org/c-plus-plus/) 中演示 `std::add_lvalue_reference` 的程序:
...
**参考:** [http://www.cplusplus.com/reference/type_traits/add_lvalue_reference/](http://www.cplusplus.com/reference/type_traits/add_lvalue_reference/)