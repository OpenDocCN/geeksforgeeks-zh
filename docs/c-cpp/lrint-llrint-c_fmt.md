...
`lrint()` 函数接受 `double`、`float` 或整数值作为参数。

## 返回

`lrint()` 函数使用当前舍入模式将参数中给出的小数值舍入为整数值，并以 `long int` 类型返回结果。
这里，当前模式由函数 `fesetround()` 决定。默认情况下，舍入方向设置为“就近舍入”（`to-nearest`）。
可以使用 `fesetround()` 更改此设置。
...
`llrint()` 函数接受 `double`、`float` 或整数值作为参数。

## 返回

`llrint()` 函数使用当前舍入模式将参数中给出的小数值舍入为整数值，并以 `long long int` 类型返回结果。
这里，当前模式由函数 `fesetround()` 决定。默认情况下，舍入方向设置为“就近舍入”（`to-nearest`）。
可以使用 `fesetround()` 更改此设置。

## 错误

必须给出一个参数，否则会给出错误 **“没有匹配的函数来调用像这样的‘llrint()’”**。
