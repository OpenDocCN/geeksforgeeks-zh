...
> 原文：`https://www.geeksforgeeks.org/atanh-function-for-complex-number-in-c/`

复数的 `atanh()` 函数在 `<complex>` 头文件中定义。该功能是 `atanh()` 功能的复数版本。该函数用于计算复数 `z` 的复圆弧双曲正切值，并返回复数 `z` 的圆弧双曲正切值。
