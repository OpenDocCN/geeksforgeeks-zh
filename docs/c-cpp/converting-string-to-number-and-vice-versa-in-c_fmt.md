...
> 原文: [https://www.geeksforgeeks.org/converting-string-to-number-and-vice-versa-in-c/](https://www.geeksforgeeks.org/converting-string-to-number-and-vice-versa-in-c/)
...
Boost 库提供了一个内置函数 `lexical_cast()`，直接将字符串转换为数字。如果输入无效，它会返回一个异常 `bad_lexical_cast`。
...
在该方法中，`stringstream` 声明一个流对象，该对象首先将一个数字作为流插入到一个对象中，然后使用 `str()` 完成数字到字符串的内部转换。
...
类似于字符串转换，`lexical_cast()` 函数保持不变，但这次参数列表修改为 `lexical_cast(numeric_var)`。
