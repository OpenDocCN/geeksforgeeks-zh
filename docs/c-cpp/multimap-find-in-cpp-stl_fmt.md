...
``multimap::find()`` 是 C++ STL 中的一个内置函数，它返回一个迭代器，一个常量迭代器，引用键在 multimap 中出现的位置。在存在多个相同键的情况下，引用其中一个键(通常是第一个键)的迭代器。如果我们希望用给定的密钥获得所有项目，我们可以使用 ``equal_range()``。如果键不在 multimap 容器中，它将返回一个迭代器，一个引用 ``multimap.end()`` 的常量迭代器。
...
**参数:** 该函数接受一个强制参数 `key`，该参数指定要在多映射容器中搜索的键。
