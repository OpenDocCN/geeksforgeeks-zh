...
> **模板:**
> `void make_heap(random_access_iterator first, random_access_iterator last);`
> **参数:**
> `first`: 必须转换为堆的序列起始元素的指针。
> `last`: 指向序列最后一个元素的下一个地址的指针，必须转换为堆。
...
> **模板:**
> `void make_heap(random_access_iterator first, random_access_iterator last, comp);`
> **参数:**
> `first`: 指向必须转换为堆的序列起始元素的指针。
...
该功能可用于调度。在调度中，在迭代中动态插入新元素。一次又一次地排序以获得最大值需要非常复杂的 O(nlogn)，相反，我们使用 `push_heap()` 函数来堆积 O(logn) 时间内产生的堆。下面的代码描述了它的实现。
