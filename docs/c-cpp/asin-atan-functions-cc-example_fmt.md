...
> 原文: [https://www.geeksforgeeks.org/asin-atan-functions-cc-example/](https://www.geeksforgeeks.org/asin-atan-functions-cc-example/)
...
**注意:** 传递给此函数的参数必须在 `[-1, 1]` 范围内，`asin()` 函数返回的值在 `[-π/2, π/2]`。
...
int main()
{
    ...
    k = 0.5;
    // asin() returns value in the range of [-π/2,π/2]
    ret = asin(k);
    val = (ret * 180) / PI;
...
`atan()` 函数用来求一个数的反正切，意思是给这个函数一个正切值，它会返回这个值对应的弧度角。反正切是切线的逆运算。该函数接受所有实数，`atan()` 函数返回的值在 `[-π/2, π/2]`。
