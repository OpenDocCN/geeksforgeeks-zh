...
`strtod()` 是 C 和 C++ STL 中的一个内置函数，它将字符串的内容解释为浮点数，并将其值返回为双精度值。它设置一个指针指向字符串最后一个有效字符之后的第一个字符，如果有的话，否则将指针设置为空。
...
- `str`: 指定表示浮点数的字符串。
- `end`: 指定的参数是指已经分配的 `char*` 类型的对象。

## 返回值

返回一个 `double` 值，由字符串转换而来，如果无法进行有效转换，则返回 0。
