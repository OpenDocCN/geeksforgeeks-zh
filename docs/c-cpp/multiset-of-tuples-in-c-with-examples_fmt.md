...
> 原文：[`https://www.geeksforgeeks.org/multiset-of-tuples-in-c-with-examples/`](https://www.geeksforgeeks.org/multiset-of-tuples-in-c-with-examples/)
...
> `tuple<datatype1, datatype2, datatype3, ...> myTuple`
>
> 这里，`datatype1`，`datatype2`，`datatype3`...是相似或不相似的数据类型。

**与元组相关联的函数：**

1.  **`make_tuple()`：** `make_tuple()`用于给`tuple`赋值。传递的值应该与元组中声明的值保持一致。
    **语法：**
...
2.  **`get()`：** `get()`用于访问元组值并修改它们，它接受索引和元组名称作为参数来访问特定的元组元素。
    **语法：**
    > `get<index>(myTuple)`
    >
    > 这里，`index`是我们想要在`myTuple`中访问的元素的索引。元组中的索引从零开始。

## 什么是多集？

一个[多集合](https://www.geeksforgeeks.org/multiset-in-cpp-stl/)是一种类型的[关联容器](https://www.geeksforgeeks.org/sequence-vs-associative-containers-cpp/)，它类似于一个[集合](https://www.geeksforgeeks.org/set-in-cpp-stl/)，但是在多集合的情况下，所有的元素都必须是成对不同的。简单来说，可以有多个元素具有相同的值。

**语法：**

> `multiset<datatype> myMultiset;`
>
> 这里，`datatype`可以是相似或不相似的数据类型。
...
*   [`max_size()`](https://www.geeksforgeeks.org/multiset-max_size-in-cpp-stl-with-examples/)：返回多集可以容纳的最大元素数。
...
> `multiset<tuple<datatype1, datatype2, datatype3>> myMultiset;`
>
> 在这里，`datatype1`、`datatype2`、`datatype3`是相似或不相似的数据类型。
