...
C/C++ 中的 [`remove()`](http://www.cplusplus.com/reference/cstdio/remove/) 功能可以用来删除文件。如果成功删除文件，函数返回 0，否则返回非零值。
...
**说明:** 这可以使用 C 中的 [`remove()`](https://www.geeksforgeeks.org/c-program-delete-file/) 功能来完成，注意，这是在 `Linux` 环境下完成的。因此，`remove` 函数被输入命令行参数中的第一个参数，即编译后创建的 `a.out` 文件（可执行文件）。因此程序将被销毁。
