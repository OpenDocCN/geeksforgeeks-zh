...
> 原文：[https://www.geeksforgeeks.org/c-program-demonstrate-fork-and-pipe/](https://www.geeksforgeeks.org/c-program-demonstrate-fork-and-pipe/)
...
我们首先关闭第一个管道的读取端(`fd1[0]`)，然后通过管道的写入端(`fd1[1]`)写入字符串。现在家长将**等待**直到子进程完成。在子进程之后，父进程将关闭第二个管道的写入端(`fd2[1]`)，并通过管道的读取端(`fd2[0]`)读取字符串。

## 子进程内部

子进程通过关闭管道的写端(`fd1[1]`)读取父进程发送的第一个字符串，读取后连接两个字符串，并通过`fd2`管道将字符串传递给父进程，并将退出。
