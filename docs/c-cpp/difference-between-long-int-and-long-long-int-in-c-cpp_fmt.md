...
> 原文:[https://www . geeksforgeks . org/long-int-和 long-long-int-in-c-cpp/](https://www.geeksforgeeks.org/difference-between-long-int-and-long-long-int-in-c-cpp/) 之间的差异
...
**递减顺序为:**`long` >= `long long` >= `int` >= `short` >= `char`

### 程序 1:

在各种竞争性编码平台中，约束条件介于`10^7`到 `10^18`之间。下面是理解这个概念的程序:
...
如上，输出不正确，因为 `int` 不能存储一个`10^10`值(超出其范围)。在这种情况下，应该使用 `long long`。
...
**说明:**之后给出正确的输出，是`10^10`，当范围达到 `10^18`时，可以很容易的存储成很长很长的数据类型。