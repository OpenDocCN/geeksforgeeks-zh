# std::to_address 用 C++ 举例

> 原文: [https://www.geeksforgeeks.org/stdto_address-in-c-with-examples/](https://www.geeksforgeeks.org/stdto_address-in-c-with-examples/)
...
这个方法接受一个参数 `p`，这个参数是需要找到地址的花式或原始指针。

## 返回值

该方法返回代表指针 `p` 地址的原始指针。

以下示例演示了 `std::to_address` 的用法。
...
**参考:** [https://en.cppreference.com/w/cpp/memory/to_address](https://en.cppreference.com/w/cpp/memory/to_address)