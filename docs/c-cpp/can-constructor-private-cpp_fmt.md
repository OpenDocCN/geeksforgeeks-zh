...
`构造函数`是初始化类的对象的类的特殊成员函数。在 C++ 中，当创建类的对象时，会自动调用构造函数。

默认情况下，构造函数在类的公共部分定义。那么，问题是构造函数可以在类的私有部分定义吗？

**回答：可以，构造函数可以在类的私有部分定义。**

## 如何在私有部分使用构造函数？

1.  **Using Friend Class：** 如果我们希望类不能被其他任何对象实例化，而只能由友元类实例化。
...
3.  **Named Constructor Idiom：** 由于构造函数与类同名，不同的构造函数通过其参数列表来区分，但如果构造函数的数量很多，实现可能会变得容易出错。
...
## 参考
1.  [命名构造函数成语](https://isocpp.org/wiki/faq/ctors#named-ctor-idiom)
2.  [一个构造函数在 C++ 中可以是私有的吗](https://stackoverflow.com/questions/18132590/can-a-constructor-be-private-in-c)
