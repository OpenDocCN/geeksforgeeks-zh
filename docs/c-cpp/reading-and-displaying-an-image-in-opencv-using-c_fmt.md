...
> 原文: [https://www.geeksforgeeks.org/reading-and-displaying-an-image-in-opencv-using-c/](https://www.geeksforgeeks.org/reading-and-displaying-an-image-in-opencv-using-c/)

在本文中，我们将讨论如何在 [C++](https://www.geeksforgeeks.org/c-plus-plus/) 中使用 [OpenCV(开源计算机视觉)](https://www.geeksforgeeks.org/top-7-most-popular-computer-vision-tools-in-2020/) 打开图像。与 python 不同，C++ 中不需要任何额外的库。OpenCV C++ 自带这个神奇的图像容器 `Mat`，可以为我们处理一切。从标准 C++ 程序中看到的唯一变化是包含了命名空间 `cv`，它包含了所有的 OpenCV 函数、[类](https://www.geeksforgeeks.org/c-classes-and-objects/)和[数据结构](https://www.geeksforgeeks.org/data-structures/)。在 OpenCV 中读取和显示图像需要以下功能:
...
这有助于我们在 `imread()` 函数无法加载图像或图像不存在于指定路径时进行错误处理，并告诉我们 `Mat` 容器是否为空。

## WaitKey()

该功能通过保持窗口打开直到用户按下某个键来帮助长时间显示图像。
