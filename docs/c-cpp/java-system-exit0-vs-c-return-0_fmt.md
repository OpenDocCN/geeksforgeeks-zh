...
> 原文: [https://www.geeksforgeeks.org/java-system-exit0-vs-c-return-0/](https://www.geeksforgeeks.org/java-system-exit0-vs-c-return-0/)

Java 和 C++ 是具有不同应用和设计目标的语言。C++ 是过程编程语言 C 的扩展，Java 依赖于 Java 虚拟机来实现安全性和高度可移植性。这导致他们有许多不同。在本文中，我们将看到 `C++ return 0` 和 `Java System.exit(0)` 的区别。在讨论这些差异之前，让我们先了解它们各自的真正含义。
...
在 `stdlib.h` 中，宏 `EXIT_SUCCESS` 和 `EXIT_FAILURE` 是这样定义的:
...
*   `return 0` --> 成功终止。
*   `return 1` 或任何其他非零值 --> 不成功终止。
*   返回不同的值，如 `return 1` 或 `return -1` 或任何其他非零值意味着程序返回错误。
...
*   `exit(0)` --> 成功终止。
*   `exit(1)` 或 `exit(-1)` 或任何其他非零值 --> 不成功终止。
...
**注:** 两个 `return 0` 和 `System.exit(0)` 的工作与 `main()` 功能返回类型的区别相同。

**下表描述了差异:**

| 序号 | `C++ return 0` | `Java System.exit(0)` |
| :--- | :--- | :--- |
...
| 3. | `return` is a keyword used to return a value, so no declaration is required. It only needs to return keywords with values or variables. | Declared as `java.lang.System.exit()` method: `public static void exit(int status)` |
