...
该功能包含在`boost/algorithm/string`库中。`boost`字符串算法库提供了字符串相关算法的通用实现，这些算法在`STL`中缺失。`trim`功能用于从字符串中移除所有前导或尾随空格。输入序列被就地修改。
...
> **解释:**
> `trim_left()`函数删除所有前导空格。
> `trim_right()`函数删除所有尾随空格。
> `trim()`函数删除所有前导和尾随空格。
