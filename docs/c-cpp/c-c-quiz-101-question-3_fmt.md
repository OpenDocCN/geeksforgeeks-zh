...
(B) `free(*ppInt2)`不正确。这会导致编译时错误。
(C) `free(*ppInt2)`不正确。它会产生运行时错误。
(D) `malloc()`和`free()`都没有问题，即没有编译/运行时错误。

回答: (D)

说明: `ppInt2`是指向指针的指针。`*ppInt2`是指向`int`的指针。只要`free()`的参数是指针，就没有问题。这就是为什么 B)和 C)都不正确的原因。根据其数据类型，`ppInt1`和`ppInt2`的分配都很好。所以 A)也是不正确的。正确的说法是 D)。
