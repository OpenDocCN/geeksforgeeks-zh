...
> 原文：[https://www.geeksforgeeks.org/erosion-and-dilation-morphological-transformations-in-opencv-in-cpp/](https://www.geeksforgeeks.org/erosion-and-dilation-morphological-transformations-in-opencv-in-cpp/)
...
`结构元素`是用于与给定图像交互的形状。它帮助我们根据它如何错过或符合图像得出结论。用于形态学操作，如腐蚀、膨胀、打开、关闭、渐变、[黑帽/高帽变换](https://www.geeksforgeeks.org/top-hat-and-black-hat-transform-using-python-opencv/)。OpenCV 为内核提供了 3 种形状：矩形、十字形和椭圆形。
...
*   `形状`：结构元素的形状可以是 `MORPH_RECT`、`MORPH_ELLIPSE`、`MORPH_CROSS` 中的任何一个。
*   `ksize`：结构元素的大小。
...
> `erode(src, dst, 内核, 锚, 迭代, 边界类型, 边界值)`
...
*   `锚`：结构元素内部的锚位置。默认值为 `[-1, -1]` 表示位置为结构元素的中心。
*   `迭代`：应用侵蚀的次数。
*   `边框类型`：边框类型(`BORDER_CONSTANT`、`BORDER_REPLICATE` 等)。
*   `边框值`：边框值。
...
> `dilate(src, dst, 内核, 锚, 迭代, borderType, borderValue)`
