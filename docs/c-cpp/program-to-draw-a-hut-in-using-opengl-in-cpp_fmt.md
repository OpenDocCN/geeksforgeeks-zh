...
> 原文：[https://www.geeksforgeeks.org/program-to-draw-a-hut-in-using-opengl-in-cpp/](https://www.geeksforgeeks.org/program-to-draw-a-hut-in-using-opengl-in-cpp/)
...
*   使用函数 `glutInit(&argc, argv)` 初始化工具包。
*   使用功能设置显示模式并指定配色方案。
*   使用功能 `glutInitWindowSize(1200, 740)` 设定窗口大小。
*   使用功能设置窗口的起始位置。
*   使用功能 `glutCreateWindow("Basic hut like structure")` 初始化窗口并设置标题。
*   初始化 `myInit()` 功能，并执行以下步骤：
    *   使用功能 `glClearColor(1.0, 0.5, 0.0, 1.0)` 将背景颜色设置为橙色。
    *   使用功能 `gluOrtho2D(0.0, 400.0, 0.0, 400.0)` 指定显示区域。
