...
条件编译指令有助于编译程序的特定部分，或者让我们根据某些条件跳过程序某些特定部分的编译。在我们的[上一篇文章](https://www.geeksforgeeks.org/cc-preprocessors/)中，我们已经讨论了两个这样的指令`#ifdef`和`#endif`。在这篇文章中，我们将讨论`#ifndef`、`#if`、`#else`、`#elif`。
