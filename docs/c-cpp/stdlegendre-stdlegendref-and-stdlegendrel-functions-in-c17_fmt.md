# C++17 中的 `std::legendre`、`std::legendref` 和 `std::legendrel` 函数

> 原文: [https://www.geeksforgeeks.org/stdlegendre-stdlegendref-and-stdlegendrel-functions-in-c17/](https://www.geeksforgeeks.org/stdlegendre-stdlegendref-and-stdlegendrel-functions-in-c17/)

`legendre`、`legendref` 和 `legendrel` 是 C++ STL 中的内置函数，用于计算 n 次和参数 x 的无关联多项式的值。x 的 n 阶无关联[勒让德多项式](https://en.wikipedia.org/wiki/Legendre_polynomials)的值由下式给出:
...
函数返回参数 `x` 的 `n` 阶无关联勒让德多项式的值，返回类型取决于传递的参数。

## 注

函数运行在 C++ 17 (7.1) 及以上版本。
