...
`reverse()`是头文件`<algorithm>`中的预定义函数。它被定义为上述头文件中的一个模板。它颠倒任何容器的范围`[first, last]`中元素的顺序。时间复杂度为`O(n)`。

**注意:**使用的范围是`[first, last]`，包含`first`和`last`之间的所有元素，包括`first`指向的元素，但不包括`last`指向的元素。
...
```
`BidirectionalIterator`是一个迭代器，可用于以正向和反向两种方向访问容器的任何元素。
