...
> 原文：[https://www.geeksforgeeks.org/when-do-we-pass-arguments-by-reference-or-pointer/](https://www.geeksforgeeks.org/when-do-we-pass-arguments-by-reference-or-pointer/)
...
**2)** **对于传递大尺寸的参数：** 如果一个参数很大，通过引用（或指针）传递效率更高，因为真正传递的只是一个地址，而不是整个对象。例如，让我们考虑下面的 `Employee` 类和一个打印雇员详细信息的函数 `printEmpDetails()`。
...
**3)** **避免对象切片：** 如果我们将子类的对象传递给一个期望超类对象的函数，那么传递的对象如果是按值传递的，那么就是[切片的](http://en.wikipedia.org/wiki/Object_slicing)。例如，考虑以下程序，它打印“This is Pet class”。
...
如果我们在上面的程序中使用了按引用传递，那么它会正确打印“This is Dog class”。参见以下修改后的程序。
...
感谢 [Venki](https://www.geeksforgeeks.org/forums/users/venki/) 加了这个点。
