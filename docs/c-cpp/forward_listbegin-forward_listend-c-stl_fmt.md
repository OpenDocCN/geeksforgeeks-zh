...
> 原文：`https://www.geeksforgeeks.org/forward_listbegin-forward_listend-c-stl/`
...
`begin()`函数用于返回指向转发列表容器第一个元素的迭代器。`begin()`函数返回一个双向迭代器到容器的第一个元素。
...
`end()`函数用于返回指向列表容器最后一个元素的迭代器。`end()`函数返回一个双向迭代器到容器的最后一个元素。
...
**Returns:**
This function returns a bidirectional iterator pointing to the last element.
