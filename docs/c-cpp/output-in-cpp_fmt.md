...
`cout`经常用于打印输出，即在监视器上。预定义对象 `cout` 是 [`iostream` 类](https://www.geeksforgeeks.org/c-stream-classes-structure/)的一个实例。对于格式化的输出操作，`cout` 与[插入运算符](https://www.geeksforgeeks.org/overloading-stream-insertion-operators-c/)一起使用，该运算符被写成`<<`（即两个“小于”符号）。
...
在这个例子中，用户被要求输入他/她的城市名称，当用户输入他/她的城市时，它会将城市名称存储在 `name` 变量中。之后，控制台将打印输出字符串。下面是同样的程序：
