# C++ STL 中的无序映射 `hash_function()` 函数

> 原文：[https://www.geeksforgeeks.org/unordered_map-hash_function-function-in-c-stl/](https://www.geeksforgeeks.org/unordered_map-hash_function-function-in-c-stl/)

`unordered_map::hash_function()` 是 C++ STL 中的一个内置函数，用来获取哈希函数。这个散列函数是一个一元函数，只接受一个参数，并根据它返回一个 `size_t` 类型的唯一值。
...
下面的程序说明了 `unordered_map::hash_function()` 函数。
