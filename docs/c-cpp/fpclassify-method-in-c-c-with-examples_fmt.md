# C/C++ 中的 `fpclassify()` 方法示例

> 原文：[https://www.geeksforgeeks.org/fpclassify-method-in-c-c-with-examples/](https://www.geeksforgeeks.org/fpclassify-method-in-c-c-with-examples/)

`fpclassify()` 函数在 C++ 中的 [`math.h` 头文件](https://www.geeksforgeeks.org/c-library-math-h-functions/) 和 [`<cmath>` 库](https://www.geeksforgeeks.org/c-mathematical-functions/) 中定义。该函数用于获取与分类宏常量之一相匹配的 `int` 类型的值（取决于 `x` 的值）。
...
该方法接受一个参数 `x`，该参数是与该方法的一个宏常数相匹配的值。它可以是整数、浮点、双精度或长双精度。
