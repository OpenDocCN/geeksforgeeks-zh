...
1. 自我分配检查。
2. 如果任务不是给自己的，那么它会遵循。
    a) 释放分配给 `this->ptr` 的内存
    b) 分配新内存给 `this->ptr` 并复制值
    c) 返回 `*this`
...
本文由**碧华纳特**供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 `contribute.geeksforgeeks.org` 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
