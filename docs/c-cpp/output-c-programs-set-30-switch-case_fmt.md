...
> 原文: [https://www.geeksforgeeks.org/output-c-programs-set-30-switch-case/](https://www.geeksforgeeks.org/output-c-programs-set-30-switch-case/)
...
**说明:** 我们可以写任何顺序的 `case` 语句，包括默认的 `case`。在 `switch case` 语句中，默认案例可以是第一个案例、最后一个案例或介于两者之间的任何案例。表达式 `movie << (2 + movie)` 的值是 8。
...
**说明:** 在 `C` 中，逗号也是优先级最低的运算符。所以如果 `x = (a, b);` 那么 `x = b`。
注意: `Case` 表达式可以是宏常量。
