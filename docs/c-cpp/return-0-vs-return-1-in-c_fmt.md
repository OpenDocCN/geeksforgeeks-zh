...
下面是一个程序来说明[主函数](https://www.geeksforgeeks.org/executing-main-in-c-behind-the-scene/)中 `返回 0` 和 `返回 1` 的用法：
...
*   C++ 将[布尔型](https://www.geeksforgeeks.org/bool-data-type-in-c/)视为完全独立的数据类型，只有两个不同的值，即 `true` 和 `false`。
*   值 `1` 和 `0` 属于 `int` 类型，不能隐式转换为布尔值，这意味着：
    *   **返回 0:** 从函数返回 `false`。
    *   **返回 1:** 从一个函数返回 `true`。

下面是一个程序来说明用户自定义函数中 `返回 0` 和 `返回 1` 的用法：
...
| In the custom function | `0` indicates that the custom function is returning `false`. | `1` indicates that the custom function is returning `true`. |