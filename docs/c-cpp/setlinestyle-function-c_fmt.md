...
### 线型
指定后续线条将以哪种样式绘制(如实线、虚线、居中、虚线)。
![](img/b74e5828bd99cd0bd4c8a9afa6c3059b.png)

### `up pattern`
是16位模式，仅当`linestyle`为`USERBIT_LINE` (4)时适用。在这种情况下，每当模式字中的一位为1时，该行中相应的像素就以当前的绘图颜色绘制。必须始终提供`upattern`的值。如果`linestyle`不是`USERBIT_LINE` (4)，则简单地忽略它。
