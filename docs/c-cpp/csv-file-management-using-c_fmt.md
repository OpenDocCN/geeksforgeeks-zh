...
## 2. Read a particular record (读取特定记录)
...
4.  通过`row[索引]`检索所需的列数据。在这里，`row[0]`总是存储学生的卷号，因此将`row[0]`与用户输入的卷号进行比较，如果匹配，则显示学生的详细信息并脱离循环。
...
2.  要求用户为要更新的记录输入新值。
3.  用新数据更新`row[索引]`。这里，索引是指需要更新的列字段。
...
**参考文献:**
[T2】https://www.geeksforgeeks.org/file-handling-c-classes/
[https://www.geeksforgeeks.org/stringstream-c-applications/](https://www.geeksforgeeks.org/stringstream-c-applications/)