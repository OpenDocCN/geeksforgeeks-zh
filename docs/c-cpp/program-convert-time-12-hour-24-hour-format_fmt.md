...
> 原文: [https://www.geeksforgeeks.org/program-convert-time-12-hour-24-hour-format/](https://www.geeksforgeeks.org/program-convert-time-12-hour-24-hour-format/)
...
```
Input : A single string containing a time in 12-hour clock format(hh:mm:ss AM or hh:mm:ss PM where 01 <= hh <= 12 or 01 <= mm,ss <= 59)
...
```python
# Python3 program to convert 12 hour to 24 hour format
def print24(s):
...
```javascript
// javascript program to convert 12 hour format to 24 hour format
