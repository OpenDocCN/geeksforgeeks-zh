...
> 原文：[https://www.geeksforgeeks.org/working-with-array-and-vectors-using-stl-in-c/](https://www.geeksforgeeks.org/working-with-array-and-vectors-using-stl-in-c/)
...
`sort(start_index, last_index)` – 对给定数组/向量进行排序。`sort()` 函数适用于快速排序算法。C++ STL 提供了一个类似的函数 `sort`，可以对向量或数组（随机访问的项目）进行排序。它的复杂性是 O(nlogn)。
...
*   `accumulate(first_index, last_index, initial_sum)`: 这个函数返回一个数组/向量的所有元素的和。
