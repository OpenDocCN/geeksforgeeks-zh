...
> 原文：[https://www.geeksforgeeks.org/stdgslice-valarray-generalized-slice-selector/](https://www.geeksforgeeks.org/stdgslice-valarray-generalized-slice-selector/)

## Valarray 广义切片选择器
这个类表示 Valarray 广义切片选择器（一个多维切片）。它不包含也不引用任何元素——它只描述了要在 `valarray::operator[]` 中用作索引的元素的选择。
...
给定起始值 `s`、步幅列表 `i_j` 和尺寸列表 `d_j`，由这些值构建的 `std::gslice` 选择一组指数：
...
- `size_t (stride)`：分隔所选元素的跨度。

在给定语法中，默认构造函数等效于 `gslice(0, std::valarray(), std::valarray())`。此构造函数仅允许构造切片数组。使用 `start`、`size` 和 `stride` 参数构造一个新的切片。
