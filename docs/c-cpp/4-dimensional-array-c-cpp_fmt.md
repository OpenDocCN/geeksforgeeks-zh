...
先决条件: [C/C++ 中的数组](https://www.geeksforgeeks.org/arrays-in-c-language-set-1-introduction/)，[更多关于数组](https://www.geeksforgeeks.org/arrays-in-c-language-set-2-interesting-array-properties/)

四维(4D)数组是数组的数组，或者换句话说，4D 数组是三维数组的数组。数组中的维度越多，意味着要容纳的数据越多，但也意味着管理和理解数组的难度越大。
...
1.  `int student[4][5][6][7];`
    `int` 指定数组类型整数。`student` 是我们 4D 阵的名字。我们的数组可以容纳 840 个整型元素。这个数字是通过乘以每个维度的值得出的。在这种情况下: `4x5x6x7=840`。

2.  `float country[5][6][5][6][5];`
    阵 `country` 是五维阵。可以容纳 4500 个浮点元素 (`5x6x5x6x5=4500`)。
