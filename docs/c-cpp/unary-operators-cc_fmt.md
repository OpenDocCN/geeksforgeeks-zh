...
#### 2.1 前缀增量 (`++a`)
在此方法中，运算符位于操作数之前。在使用之前，操作数的值将被改变。
...
#### 2.2 后缀增量 (`a++`)
在这个方法中，运算符跟在操作数后面。使用后，操作数的值将被更改。
...
### 4. 逻辑非 (`!`)
用于反转其操作数的逻辑状态。如果条件为真，则逻辑非运算符将使其为假。
...
### 5. 取址运算符 (`&`)
它给出了一个变量的地址。它用于返回变量的内存地址。由取址运算符返回的这些地址被称为指针，因为它们“指向”内存中的变量。
...
### 6. `sizeof()`
此运算符以字节为单位返回其操作数的大小。`sizeof` 运算符总是位于其操作数之前。操作数是一个表达式，也可以是一个类型转换。
