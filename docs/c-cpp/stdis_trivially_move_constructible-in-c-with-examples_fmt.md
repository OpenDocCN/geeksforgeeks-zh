# std::is_trivially_move_constructible 在 C++ 中的构造示例

> 原文: [https://www.geeksforgeeks.org/stdis_trivially_move_constructible-in-c-with-examples/](https://www.geeksforgeeks.org/stdis_trivially_move_constructible-in-c-with-examples/)

[C++ STL](https://www.geeksforgeeks.org/the-c-standard-template-library-stl/) 的 `std::is_trivially_move_constructible` 模板存在于 `<type_traits>` 头文件中。C++ STL 的 `std::is_trivially_move_constructible` 模板用于检查 `T` 是否为 trivially move constructible。如果 `T` 是普通移动可构造类型，则返回布尔值 `true`，否则返回 `false`。
...
模板 `std::is_trivially_move_constructible` 接受单个参数 `T` (Trait 类) 检查 `T` 是否为 trivially move constructible 类型。

## 返回值

模板 `std::is_trivially_move_constructible` 返回如下所示的布尔变量:
*   **真:** 如果类型 `T` 是一个 trivially move constructible。
*   **假:** 如果类型 `T` 不是一个 trivially move constructible。

下面是用 C++ 演示 `std::is_trivially_move_constructible` 的程序:
...
**参考:** [http://www.cplusplus.com/reference/type_traits/is_trivially_move_constructible/](http://www.cplusplus.com/reference/type_traits/is_trivially_move_constructible/)