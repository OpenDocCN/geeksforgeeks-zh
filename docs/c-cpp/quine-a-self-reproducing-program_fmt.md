...
> 原文：[https://www.geeksforgeeks.org/quine-a-self-reproducing-program/](https://www.geeksforgeeks.org/quine-a-self-reproducing-program/)
...
该程序使用 `printf` 函数，但不包含其对应的头文件(`#include <stdio.h>`)，这可能导致未定义的行为。此外，`main` 的返回类型声明被省略，以减少程序的长度。两个 `34` 用于在字符串 `s` 周围打印双引号。
