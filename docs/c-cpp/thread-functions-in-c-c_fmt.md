...
在一个**Unix/Linux操作系统**中，**C/C++语言**为所有线程相关的功能提供了[POSIX 线程(pthread)](https://www.geeksforgeeks.org/multithreading-c-2/)标准API（应用程序接口）。它允许我们为并发进程流创建多个线程。它在多处理器或多核系统上最有效，在这些系统中，线程可以在内核级实现，以达到执行速度。在单处理器系统中，通过利用输入输出或其他可能导致进程停止的系统功能的延迟，也可以获得收益。

我们必须在脚本的开头包含`pthread.h`头文件，才能使用`pthreads`库的所有功能。要执行c文件，我们必须在编译文件时在命令行中使用`-pthread`或`-lpthread`。
...
*   `thread`: 指向无符号整数值的指针，该整数值返回所创建线程的线程 id。
...
## 5. pthread_equal
比较两个线程是否相同。如果两个线程相等，函数返回非零值，否则返回零。
...
**参数:** 此方法接受一个强制参数`thread`，它是被发送取消请求的线程的线程 id。

## 7. pthread_detach
用于分离一个线程。分离的线程在终止时不需要其他线程来等待。如果线程被分离，其资源会在终止后自动释放。
...
**参数:** 该方法接受一个强制参数`thread`，即必须分离的线程的线程 id。
