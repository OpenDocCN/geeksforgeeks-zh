...
> 原文: `https://www.geeksforgeeks.org/stdremove_const-in-c-with-examples/`

[C++ STL](https://www.geeksforgeeks.org/the-c-standard-template-library-stl/) 的 `std::remove_const` 模板存在于 `<type_traits>` 头文件中。C++ STL 的 `std::remove_const` 模板用于获取没有 const 限定的类型 `T`。如果 `T` 没有常量限定，则返回布尔值 `true`，否则返回 `false`。下面是相同的语法：
...
## 参数:
此模板 `std::remove_const` 接受单个参数 `T`（类型特征类），检查 `T` 是否无 const 限定。

## 返回值:
模板 `std::remove_const` 返回布尔值：
*   `true`: 如果类型 `T` 没有被 const 限定。
*   `false`: 如果类型 `T` 是 const 限定的。
...
## 参考:
`http://www.cplusplus.com/reference/type_traits/remove_const/`