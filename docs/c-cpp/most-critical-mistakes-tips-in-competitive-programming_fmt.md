...
*   当从另一个字符串组成一个新字符串时，不使用像 `str = str + str2[i]`这样的语法，即添加字符来获得一个字符串，这个方法的时间复杂度是 **`O(size(string))`**、因此列出了向一个字符串添加字符，我们使用 **`push_back(char)`**，其时间复杂度是 **`O(n)`。**
...
> **注意:**这些是一些可以让你的生活变得轻松的流行算法，它们都有相同的使用模式，即函数名 **(下 lim，上 lim+1)**