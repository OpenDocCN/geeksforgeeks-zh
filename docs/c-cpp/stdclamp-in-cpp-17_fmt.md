...
**注意：** 这个函数从 C++ 17 开始在 `<algorithm>` 头文件中定义。
...
```cpp
template< class T >
constexpr const T& clamp( const T& num, const T& low, const T& high );
...
- `num` - 要被限制的值。
- `low`, `high` - 限制 `num` 的边界。
- `comp` - 比较函数对象，如果 `low` 小于 `num` 且 `num` 小于 `high` 则返回 `true`。

**返回值**
- 如果 `num` 小于 `low`，则返回对 `low` 的引用。
- 如果 `high` 小于 `num`，则返回对 `high` 的引用。
- 否则，返回对 `num` 的引用。
