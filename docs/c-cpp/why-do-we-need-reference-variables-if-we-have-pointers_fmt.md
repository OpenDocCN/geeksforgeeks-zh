...
> 原文: [https://www.geeksforgeeks.org/why-do-we-need-reference-variables-if-we-have-pointers/](https://www.geeksforgeeks.org/why-do-we-need-reference-variables-if-we-have-pointers/)
...
在引用中要访问实际变量的值，我们不需要显式取消引用引用变量，它们会自动被取消引用。
...
*   引用变量为`现有变量`提供了一个`新名称`。它是隐式去引用的，不需要去引用运算符 `(*)` 来检索引用的值。`鉴于`，要检索指针指向的`值`，我们需要去引用操作符 `(*)`，也就是所谓的`显式去引用`。
*   引用可以被视为一个`常量指针`。在声明时必须是`初始化`，不能更改内容。
...
*   当指针 `ptr1` 指向 `num1` 变量时，它会打印 `20`
*   当 `*ptr1` 被取消引用并更改值时，它会打印 `26`
*   类似地，对于另一个名为 `num2` 的变量，打印 `23`
*   当引用 `ref1` 被 `num1` 初始化时，它打印的是 `26` 而不是 `20`
*   最后当 `18` 被分配到 `ref1` 中时，由于隐式去引用，它会打印 `18`

**引用变量说明：**
引用变量最好的例子是 [`复制构造函数`](https://www.geeksforgeeks.org/copy-constructor-in-cpp/)的概念。复制构造函数以一个`引用变量`为参数，这里不能使用指针。
...
**说明：** 在上面的例子中，如果我们在复制构造函数的参数中取`指针`，那么 `complex` 类的`对象将被一次又一次地创建`而永远不会被停止，这是`在 OOPs` 概念中的错误。`选择引用`只是这种情况下的解决方案。