# C++ STL 中的 unordered_map::load_factor

> 原文：[https://www.geeksforgeeks.org/unordered_map-load_factor-in-c-stl/](https://www.geeksforgeeks.org/unordered_map-load_factor-in-c-stl/)

## 介绍

`unordered_map::load_factor()` 是 C++ STL 中的内置函数，它返回 `unordered_map` 容器中的当前加载因子。加载因子是容器中元素的数量（其大小）与桶的数量（桶计数）之间的比率：
`加载因子 = 大小 / 桶计数`
