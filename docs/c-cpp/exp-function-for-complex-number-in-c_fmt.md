...
> 原文：[https://www.geeksforgeeks.org/exp-function-for-complex-number-in-c/](https://www.geeksforgeeks.org/exp-function-for-complex-number-in-c/)

复数的`exp()`函数在`<complex>`头文件中定义。该功能是`exp()`功能的复杂版本。该函数用于计算复数`z`的基数`e`指数，并返回复数`z`的基数-e指数。
