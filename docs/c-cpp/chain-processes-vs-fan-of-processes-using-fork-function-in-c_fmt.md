# 链流程与使用 C 中的 fork()函数的流程风扇

> 原文: [https://www.geeksforgeeks.org/chain-processes-vs-fan-of-processes-using-fork-function-in-c/](https://www.geeksforgeeks.org/chain-processes-vs-fan-of-processes-using-fork-function-in-c/)

`Fork System Call`：`Fork System Call` 用于创建新的[进程](https://www.geeksforgeeks.org/difference-between-program-and-process/)，称为 `子进程`，与进行 `fork()` 调用的进程（父进程）并发运行。创建新的子进程后，两个进程都将执行 `fork()` 系统调用之后的下一条指令。子进程使用与父进程相同的程序计数器、相同的中央处理器寄存器和相同的打开文件。

要创建流程的扇形或链条，首先插入[头文件](https://www.geeksforgeeks.org/header-files-in-c-cpp-and-its-uses/) `unistd.h`。使用 `fork()` 功能创建流程。使用 `exit()` 方法需要包括 `stdlib.h`，程序中有 3 种方式可以使用 `exit` 语句：
...
**说明：** 在上面的程序中， `getpid()` 用来获取进程 id。或者，也可以使用 `getppid()` 来获取父进程标识。每次程序运行时，输出都不一样，因为不是每次操作系统都给进程分配相同的进程 id，而是每次程序运行时总是以非递减顺序获得 PID（进程 id）。
