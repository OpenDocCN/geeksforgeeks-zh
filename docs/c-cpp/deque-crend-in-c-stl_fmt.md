...
`deque::crend()`是 C++ STL 中的一个内置函数，它返回一个常量反向迭代器，指向`decue`第一个元素之前的位置。
...
> prog.cpp:15:13: 错误: 分配只读位置 ‘it. std::reverse_iterator<_Iterator>::operator*() const [with _Iterator = std::_Deque_iterator<const char, const char&, const char*>]’
>      *it = 'g';
>             ^
> prog.cpp:17:5: 错误: 应为 ‘;’ 在 ‘return’ 之前
>      return 0
>           ^~~~~