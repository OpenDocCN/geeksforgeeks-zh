# C++ STL 中的 unordered_map::reserve()

> 原文: [https://www.geeksforgeeks.org/unordered_map-reserve-in-c-stl/](https://www.geeksforgeeks.org/unordered_map-reserve-in-c-stl/)

正如我们所知，`Bucket`是容器内部哈希表中的一个槽，所有元素都是根据它们的键的哈希值分配给它的。桶的编号从0到桶计数。现在作为一个桶持有可变数量的项目。该数字基于术语`负载系数`。当`load_factor`达到某个阈值时，容器会增加铲斗数量并重新提升地图。但是当我们调用`重新散列(n)`时，它直接将桶的数量设置为`n`，并触发整个哈希表的重建。但是当我们调用`reserve(n)`时，它会创建足够的`Buckets`来保存至少`n`个项目。如果我们将`n`个项目添加到地图中，根据负载系数，可能会触发一次重挂。通过为无序的map容器调用我们期望的大小的`reserve`，我们避免了容器大小的增加可能产生的多次重散列，并优化了哈希表的大小。C++函数`std::unordered_map::reserve()`将容器中的桶数(`bucket_count`)设置为最适合包含至少`n`个元素的值。
...
### 参数:
该函数接受单个强制参数`N`，该参数将请求的元素数量指定为最小容量。
