# C++ STL 中的 unordered_multimap::equal_range() 函数

> 原文：[https://www.geeksforgeeks.org/unordered_multimap-equal_range-function-in-c-stl/](https://www.geeksforgeeks.org/unordered_multimap-equal_range-function-in-c-stl/)

`unordered_multimap::equal_range()` 是 C++ STL 中的内置函数，返回所有元素的键等于一个 `key` 的范围。它返回一对迭代器，其中第一个迭代器指向范围的下限，第二个迭代器指向范围的上限。如果容器中没有等于给定 `value` 的元素，则它返回一对，其中下限和上限都指向容器结束后的位置或 `unordered_multimap::end()`。
...
## 参数：
该功能接受强制参数 `k`。返回的范围将包含带有关键字 `k` 的元素。
