...
> 原文：[https://www.geeksforgeeks.org/difference-while1-while0-c-language/](https://www.geeksforgeeks.org/difference-while1-while0-c-language/)
...
但实际上，在现实世界中使用 `while(1)` 是不可取的，因为它增加了 CPU 的使用，也阻止了代码，即在程序手动关闭之前，不能从 `while(1)` 中出来。`while(1)` 可以用在条件总是需要为真的地方。
