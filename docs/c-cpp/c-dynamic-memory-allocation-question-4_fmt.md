...
> 原文:[https://www.geeksforgeeks.org/c-dynamic-memory-allocation-question-4/](https://www.geeksforgeeks.org/c-dynamic-memory-allocation-question-4/)

以下哪一项是正确的

(A) `calloc()`分配内存，并将分配的内存初始化为零，而使用`malloc()`分配的内存有未初始化的数据。

(B) `malloc()`和`memset()`可以用来获得和`calloc()`一样的效果。

(C) `calloc()`取两个参数，但`malloc`只取1个参数。
