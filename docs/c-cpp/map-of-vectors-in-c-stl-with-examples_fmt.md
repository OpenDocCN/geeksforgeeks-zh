...
> 原文：[https://www.geeksforgeeks.org/map-of-vectors-in-c-stl-with-examples/](https://www.geeksforgeeks.org/map-of-vectors-in-c-stl-with-examples/)

[`STL`](https://www.geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/) 中的映射是以映射方式存储元素的关联容器。每个元素都有一个键值和一个映射值。没有两个映射值可以具有相同的键值。

[`STL`](https://www.geeksforgeeks.org/vector-in-cpp-stl/) 中的向量与动态数组相同，能够在插入或删除元素时自动调整自身大小，其存储由容器自动处理。向量元素被放在连续的存储中，这样就可以使用迭代器访问和遍历它们。
