...
C 中的 `fopen()` 方法是一个库函数，用于打开文件以执行各种操作，包括读取、写入等。以及各种模式。如果该文件存在，则打开该特定文件，否则创建一个新文件。
...
现在，如果我们希望查看文件，那么我们需要运行以下代码，该代码将打开文件并显示其内容。
...
3.  [C 中的 EOF、`getc()` 和 `feof()`](https://www.geeksforgeeks.org/eof-and-feof-in-c/)
