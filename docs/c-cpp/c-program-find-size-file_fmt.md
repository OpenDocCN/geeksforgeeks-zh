...
这个想法是在 C 使用 [`fseek()`](https://www.geeksforgeeks.org/fseek-in-c-with-example/)，在 C 使用 [`ftell()`](https://www.geeksforgeeks.org/ftell-c-example/)。使用 `fseek()`，我们将文件指针移动到末尾，然后使用 `ftell()`，我们找到它的位置，实际上是以字节为单位的大小。
