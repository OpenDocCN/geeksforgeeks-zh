...
## 1. 实例化与分组
命名空间是对标识符进行分组的一种方式，这样它们就不会发生冲突。使用类意味着您可以创建该类的实例，而对于命名空间则不是这样。

## 2. using-declaration
您可以对命名空间使用`using-declaration`，除非从类派生，否则对类来说这是不可能的。

## 3. 重新打开与扩展
您可以重新打开一个名称空间，并跨翻译单元添加内容。你不能在课堂上这样做。例如：
```cpp
namespace A {
int f1();
}
...
## 5. 别名与typedef
如果名称的长度使代码难以阅读，或者在不能使用指令的头文件中键入代码很繁琐，我们可以创建一个名称空间别名，作为实际名称的缩写。例如：
```cpp
#include <iostream>
