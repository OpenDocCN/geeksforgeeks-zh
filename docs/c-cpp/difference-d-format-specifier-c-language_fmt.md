...
> 原文：[https://www.geeksforgeeks.org/difference-d-format-specifier-c-language/](https://www.geeksforgeeks.org/difference-d-format-specifier-c-language/)
...
- **`%i`** 取整数值为十进制、十六进制或八进制类型的整数值。
    - 以十六进制格式输入值 – 值应在 `0x` 之前提供。
    - 以八进制格式输入值 – 值应在 `0` 之前提供。
