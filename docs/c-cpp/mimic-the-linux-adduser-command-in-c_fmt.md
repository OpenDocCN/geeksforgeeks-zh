# 模仿 C 中的 Linux adduser 命令

> 原文：[https://www.geeksforgeeks.org/mimic-the-linux-adduser-command-in-c/](https://www.geeksforgeeks.org/mimic-the-linux-adduser-command-in-c/)
...
模仿 Linux 上的 [adduser](https://www.geeksforgeeks.org/adduser-command-in-linux-with-examples/) 命令，使用 [C 程序](https://www.geeksforgeeks.org/c-language-set-1-introduction/)作为编程语言创建一个普通或系统用户。以下是这个问题作为一个解决方案的隐含要求，以便程序员坚持命令的正式执行：
...
*   一个人必须处理一个 [`passwd`](https://www.geeksforgeeks.org/passwd-command-in-linux-with-examples/) 文件。
*   还必须处理一个 `shadow` 文件。
*   两个文件的执行必须通过指定 `PFILE` 环境变量来控制。

## 先决条件

在开始处理代码之前，让我们快速了解一下先决条件。首先，从把握 `passwd` 文件背后的思想开始。另外，请随意参考 Linux 手册页。

### 1. passwd 文件

技术上表示为 `/etc/passwd` 文件，该文件的主要目的是监视或存储已被授予系统访问权限的注册用户的记录。它记录了 7 个重要领域。
...
正式定义为 `/etc/shadow` 文件，该文件扮演类似 `passwd` 文件的角色。但是，它关注的是 `passwd` 文件中与加密密码相关的所有内容。与 `passwd` 文件不同，它只对根用户可读。通常，它包含 9 个字段的记录。
...
环境变量的目的是影响程序或软件的行为方式。例如，`$LANG` 变量将与用户理解特定语言的软件进行沟通，因此它必须在接口级别提供相同的语言。这里我们将使用一个 `PATH` 变量来模拟系统查找 `passwd` 和影子文件的位置。我们将把它命名为 `PFILE`。

## 命令选项

在终端上输入 `useradd` 后，通常会指定另一个值，称为“选项”。选项可以是各种字母。但是，要限制该程序，只能使用 `-r` 和 `-m` 选项。
...
这里指定所有头文件和变量，并给出所有函数调用。它将作为编译器的蓝图。

![main.c](img/241048e153be016260a6e0fc75aaf2e0.png)

快速查看要遵循的代码布局。

这里，定义一个[结构](https://www.geeksforgeeks.org/structures-c/)，它将保存所有必须更新的数据。任何其他方法都会引入额外的代码行，并使跟踪代码变得困难。此外，将您创建的 `struct` 重新命名为 `Struct` 也是一个很好的建议。当使用 `struct` 作为参数和返回类型时，不重命名将在编译期间导致大量引用和标识符错误。

`int main()` 是调用所有[函数](https://www.geeksforgeeks.org/functions-in-c/)并加载程序的基础。`void files_and_env()` 是 `PATH` 变量分配给 `PFILE` 并初始化文件的地方。请记住插入为 `f1` 和 `f2` 创建的文件的有效位置或路径。接下来，`Struct chk_getU_op()` 是一个检查终端上输入命令的函数，从命令和选项中获取用户名。它返回一个类型为 `Struct` 的对象。
