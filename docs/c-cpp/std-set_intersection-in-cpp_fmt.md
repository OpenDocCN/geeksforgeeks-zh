...
2.  也可以用来查找两个列表中的共同元素。程序如上。

## 3. 复杂性

复杂性在 `[first1, last1]` 和 `[first2, last2]` 之间的距离是线性的: 执行高达 `2*(count1+count2)-1` 的比较。其中 `count1 = last1 - first1`，`count2 = last2 - first2`。
