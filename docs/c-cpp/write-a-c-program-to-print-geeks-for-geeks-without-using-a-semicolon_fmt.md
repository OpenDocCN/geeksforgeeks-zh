...
> 原文：[https://www.geeksforgeeks.org/write-a-c-program-to-print-geeks-for-geeks-without-using-a-semicolon/](https://www.geeksforgeeks.org/write-a-c-program-to-print-geeks-for-geeks-without-using-a-semicolon/)
...
*   `format`：这是一个包含要写入 `stdout` 的文本的字符串。
*   附加参数：……（三个点称为省略号），根据格式字符串指示可变的参数数量。

**`printf()` 返回写入 `stdout` 的字符总数。因此，它可以用作 `if` 条件、`while` 条件、`switch` 情况和宏中的条件检查。**
