# C++ STL 中 unordered_multimap::count() 函数

> 原文: [https://www.geeksforgeeks.org/unordered_multimap-count-function-in-c-stl/](https://www.geeksforgeeks.org/unordered_multimap-count-function-in-c-stl/)

`unordered_multimap::count()` 是 C++ STL 中的一个内置函数，它返回容器中其键等于参数中传递的键的元素数。
...
该函数接受单个强制参数 `key`，该参数指定要返回无序多映射容器中其计数的键。

## 返回值

返回一个无符号整数类型，表示 `key` 在容器中出现的次数。
