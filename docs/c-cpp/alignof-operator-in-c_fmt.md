...
在 C++ 11 中，`alignof` 运算符用于返回指定类型的对齐方式，以字节为单位。
...
*   `alignof` 运算符返回类型实例所需的字节对齐方式，类型可以是完整类型、数组类型或引用类型。
*   对于数组类型，返回元素类型的对齐要求。
*   对于引用类型，运算符返回引用类型的对齐方式。

## 返回值

`alignof` 运算符通常用于返回类型为 `std::size_t` 的值。
...
上述情况下，`alignof(S)` 的值是结构中最大元素的对齐要求。
