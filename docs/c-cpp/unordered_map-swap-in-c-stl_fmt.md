# C++ STL 中的 unordered_map swap

> 原文: [https://www.geeksforgeeks.org/unordered_map-swap-in-c-stl/](https://www.geeksforgeeks.org/unordered_map-swap-in-c-stl/)

`std::unordered_map::swap()` 是 C++ STL 中的内置函数，它将一个容器的元素交换到另一个容器。调用此函数后，调用方无序映射的元素将是被调用方无序映射的元素，而被调用方无序映射的元素将是调用方无序映射的元素。
