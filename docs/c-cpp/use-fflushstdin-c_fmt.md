...
我们可以像 `stdin` 一样用于输入流吗？按照 C 标准，使用 `fflush(stdin)`是未定义的行为。然而一些编译器，像微软的 visual studio 允许它。它在这些编译器中是如何使用的？在获取带有空格的输入字符串时，缓冲区不会为下一个输入清除，而是考虑前一个输入。要解决这个问题，`fflush(stdin)`被用于清除流/缓冲区。
