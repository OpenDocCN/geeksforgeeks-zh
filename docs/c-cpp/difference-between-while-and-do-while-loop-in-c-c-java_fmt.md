...
> 原文: [https://www.geeksforgeeks.org/difference-between-while-and-do-while-loop-in-c-c-java/](https://www.geeksforgeeks.org/difference-between-while-and-do-while-loop-in-c-c-java/)
...
| `while` 结尾没有分号。`while(条件)` | `while` 结尾的分号。`while(条件)；` |
| 如果只有一条语句，则不需要括号。 | 括号总是必需的。 |
| 条件中的变量在循环执行前初始化。 | 变量可以在循环之前或循环中初始化。 |
| `while` 循环是入口控制循环。 | `do-while` 循环是出口控制循环。 |
