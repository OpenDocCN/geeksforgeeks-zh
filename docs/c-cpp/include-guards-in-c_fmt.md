...
创建一个动物类并将其保存为`Animal.h`。下面是同样的程序:
...
创建一个狗类并保存为`Dog.h`。请记住包含上面声明的`Animal.h`头文件:
...
创建一个`main.cpp`文件，并包含以上两个头文件。下面是同样的程序:
...
**说明:**当 `main.cpp` 运行时，包含 `Animal.h` ，并申报动物等级。这里第一行`Animal.h` 头在执行时和 as `_ANIMALS_` 未定义时代码正常执行。当 `Dog.h` 头文件被包含，进而包含 `Animal.h` 时，这一次 `_ANIMALS_` 是在程序中定义的，所以第一行 `#ifndef` 条件为真，整个代码从**跳过**到最后一行，即 `#endif` 。简单来说，如果预处理器定义了这个名字，那么它会跳过整个文件，转到 `#endif` ，换句话说，它不处理文件。