...
[STL](https://www.geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/)中的`map`用于散列键值。我们通常将`map`用于标准数据类型。我们也可以用`map`来配对。

例如，考虑一个简单的问题，给定一个矩阵和访问的位置，打印哪些位置没有被访问。
