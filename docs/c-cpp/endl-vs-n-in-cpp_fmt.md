...
`endl` 和 `\n` 似乎都在做同样的事情，但它们之间有细微的区别。

`cout << endl` 插入新行并刷新流（输出缓冲区），而 `cout << "\n"` 只是插入新行。
...
| We can't write `'endl'` between double quotes. | We can write `"\n"` between double quotes like `cout << "\n"`. |
