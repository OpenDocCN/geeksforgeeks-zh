...
> 原文: [https://www.geeksforgeeks.org/maximum-value-of-unsigned-int-in-c/](https://www.geeksforgeeks.org/maximum-value-of-unsigned-int-in-c/)
...
*   关键字 `unsigned` 是一个[数据类型](https://www.geeksforgeeks.org/data-types-in-c/)说明符，它只表示非负整数，即正数和零。

无符号 `int` 数据类型的一些属性包括:

*   无符号数据类型只能存储正值。
*   它需要 32 位的大小。
*   可以存储在无符号 `int` 数据类型中的最大整数值通常为 `4,294,967,295`（即 `2^32 - 1`，但与[编译器相关](https://www.geeksforgeeks.org/compile-32-bit-program-64-bit-gcc-c-c/)）。
*   无符号 `int` 可以存储的最大值作为常量存储在 [`<climits>`](https://www.geeksforgeeks.org/climits-limits-h-cc/) 头文件中。其值可以作为 [`UINT_MAX`](https://www.geeksforgeeks.org/int_max-int_min-cc-applications/)。
