...
> 原文：[`https://www.geeksforgeeks.org/atm-management-system-using-cpp/`](https://www.geeksforgeeks.org/atm-management-system-using-cpp/)
...
该程序使用了 C++ 中[类](https://www.geeksforgeeks.org/c-classes-and-objects/)、[访问修饰符](https://www.geeksforgeeks.org/access-modifiers-in-c/)、[数据类型](https://www.geeksforgeeks.org/c-data-types/)、[变量](https://www.geeksforgeeks.org/variables-in-c/)、[switch 语句](https://www.geeksforgeeks.org/switch-statement-cc/)等基本概念。以下是将要实现的功能：

*   `setvalue()`：此处使用此功能，使用 C++ 中的基本输入输出方法设置数据，即 `cout` 和 `cin` 语句，分别从键盘(即用户)显示和获取输入。
*   `showdata()`：该功能用于打印数据。
*   `deposit()`：该功能有助于将钱存入特定账户。
*   `showbal()`：此功能显示沉积后可用的总余额。
*   `withdrawl()`：该功能有助于从账户中取款。
*   `main()`：这个功能在一个无限的 `while` 循环里面有一个简单的 `switch` 盒(进行选择)，这样每次用户都可以选择。
