...
> 原文：[https://www.geeksforgeeks.org/declare-a-cc-function-returning-pointer-to-array-of-integers/](https://www.geeksforgeeks.org/declare-a-cc-function-returning-pointer-to-array-of-integers/)
...
4.  带有参数 `int **` 的函数，返回指向 4 个整数指针数组的指针
    ```cpp
    int *(*function(int *))[4];
    ```
...
我们如何确保上述声明是正确的？以下程序可以交叉检查我们的声明：
...
宏 `SIZE_OF_ARRAY` 用于数组大小的符号表示。`p_array_t` 类型定义为“指向 4 个整数的数组的指针”。如果我们的声明是错误的，程序会在 `function` 的定义中抛出一个错误。
