...
> 原文：[https://www.geeksforgeeks.org/sort-array-using-socket-programming/](https://www.geeksforgeeks.org/sort-array-using-socket-programming/)
...
使用`gcc`命令（`gcc client.c -o client` 和 `gcc server.c -o server`）编译这些文件。使用`./server`和`./client`运行程序（请注意：首先要运行服务器程序，等待客户端的响应，然后是客户端代码）。
