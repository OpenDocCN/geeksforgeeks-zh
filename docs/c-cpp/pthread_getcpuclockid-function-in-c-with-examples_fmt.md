...
> 原文: [https://www.geeksforgeeks.org/pthread_getcpuclockid-function-in-c-with-examples/](https://www.geeksforgeeks.org/pthread_getcpuclockid-function-in-c-with-examples/)
...
*   `thread`: 要获取时钟 ID 的线程的 ID，调用 `pthread_create()` 或 `pthread_self()` 都可以获取。
*   `clock_id`: 指向 `clockid_t` 对象的指针，函数可以在该对象中存储时钟 id。
...
下面的程序创建一个线程，然后使用 `clock_gettime(2)` 检索两个线程消耗的总进程 CPU 时间和每线程 CPU 时间。
...
下面的程序创建一个进程，然后使用 `clock_gettime(2)` 检索两个进程消耗的总进程 CPU 时间以及父进程和子进程 CPU 时间。
