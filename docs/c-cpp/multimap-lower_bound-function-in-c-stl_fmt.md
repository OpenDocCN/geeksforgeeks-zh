# multimap::lower_bound() 函数在 C++ STL 中

> 原文: [https://www.geeksforgeeks.org/multimap-lower_bound-function-in-c-stl/](https://www.geeksforgeeks.org/multimap-lower_bound-function-in-c-stl/)
...
函数返回一个迭代器，指向容器中的键，相当于参数中传递的 `k`。如果 `k` 不在 `multimap` 容器中，函数返回一个迭代器，指向刚好大于 `k` 的下一个元素。如果参数中传递的键超过了容器中的最大键，则返回的迭代器指向键+1 和元素=0。
