...
> 原文: [https://www.geeksforgeeks.org/internal-linkage-external-linkage-c/](https://www.geeksforgeeks.org/internal-linkage-external-linkage-c/)
...
1.  **内部链接:** 实现内部链接的标识符在其声明的翻译单元外部不可访问。单元内的任何标识符都可以访问具有内部链接的标识符。它通过关键字`static`实现。内部链接的标识符存储在 RAM 的已初始化或未初始化段中。(**注意:** `static` 在作用域方面也有含义，但这里不讨论)。
    一些例子:
...
有 3 个翻译单元(Animals、Feed、Wash)使用`animals`代码。
...
2.  **外部链接:** 实现外部链接的标识符对**每个翻译单元**都是可见的。外部链接的标识符在翻译单元之间*共享*，并被认为位于程序的最外层。在实践中，这意味着您必须在一个对所有地方都可见的位置定义一个标识符，使其只有一个可见定义。它是全局作用域变量和函数的默认链接。因此，具有外部链接的特定标识符的所有实例都引用程序中的同一个标识符。关键字`extern`实现外部链接。

当我们使用关键字`extern`时，我们告诉链接器在其他地方寻找定义。因此，外部链接标识符的声明不占用任何空间。`extern`标识符通常存储在内存的初始化/未初始化或文本段中。

**请务必在进入以下示例之前，先通过 [了解 C 中的 extern 关键字](https://www.geeksforgeeks.org/understanding-extern-keyword-in-c/)。**
...
本文由 **西姆兰·达米亚** 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 [contribute.geeksforgeeks.org](http://www.contribute.geeksforgeeks.org) 写一篇文章或者把你的文章邮寄到 contribute@geeksforgeeks.org。看到你的文章出现在极客博客主页上，帮助其他极客。
