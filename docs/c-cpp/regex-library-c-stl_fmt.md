...
*   [`regex_replace`](https://www.geeksforgeeks.org/regex-regular-expression-in-c/): 用格式化的替换文本替换正则表达式。(功能模板)

## 迭代器

regex 迭代器用于遍历序列中找到的整个正则表达式匹配集。

*   `regex_iterator`: 遍历字符序列中的所有正则表达式匹配项。(类模板)
...
*   `regex_traits`: 提供正则表达式库所需的关于字符类型的元信息。(类模板)
