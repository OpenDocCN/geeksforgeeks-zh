...
> 原文: [https://www.geeksforgeeks.org/how-to-iterate-through-a-vector-without-using-iterators-in-c/](https://www.geeksforgeeks.org/how-to-iterate-through-a-vector-without-using-iterators-in-c/)
...
这里 `itr` 是存储在向量中的值，用于遍历向量。下面是同样的程序来说明:
...
用于在不使用迭代器的情况下更新向量中的值，使用引用遍历存储在向量中的值并更新该值。下面是相同的语法:
...
这里 `itr` 是存储在向量中的值的地址，用于遍历向量。下面是同样的程序来说明:
...
我们也可以在 C++ 中的许多不同[容器](https://www.geeksforgeeks.org/containers-cpp-stl/)中使用相同的遍历进行迭代。以下是相同的插图:
