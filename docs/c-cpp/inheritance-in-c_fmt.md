...
1.  [为什么用继承，什么时候用继承？](#为什么用继承什么时候用继承)
2.  [继承方式](#继承方式)
3.  [继承类型](#继承类型)
...
您可以清楚地看到，上述过程导致相同的代码重复 3 次。这增加了出错和数据冗余的机会。为了避免这种情况，使用了继承。如果我们创建一个 `Vehicle` 类，并在其中写入这三个函数，然后从 `Vehicle` 类继承其余的类，那么我们就可以简单地避免数据的重复，并增加可重用性。请看下图，其中三个类继承自 `Vehicle` 类:
...
**注意**: 派生类不继承对私有数据成员的访问。但是，它确实继承了一个完整的父对象，该对象包含该类声明的任何私有成员。
