...
> 原文: [https://www.geeksforgeeks.org/returning-multiple-values-from-a-function-using-tuple-and-pair-in-c/](https://www.geeksforgeeks.org/returning-multiple-values-from-a-function-using-tuple-and-pair-in-c/)

在某些情况下，您需要在解决问题时返回多个值(可能是不同的数据类型)。一种方法是使用指针、结构或全局变量，这里已经讨论过了。还有一种有趣的方法不用上述方法也可以这样做，使用元组(用于返回多个值)和对(用于两个值)。
