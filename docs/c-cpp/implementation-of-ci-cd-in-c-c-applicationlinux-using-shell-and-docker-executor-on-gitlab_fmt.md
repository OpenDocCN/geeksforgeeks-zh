...
> 原文: [https://www.geeksforgeeks.org/implementation-of-ci-cd-in-c-c-applicationlinux-using-shell-and-docker-executor-on-gitlab/](https://www.geeksforgeeks.org/implementation-of-ci-cd-in-c-c-applicationlinux-using-shell-and-docker-executor-on-gitlab/)
...
| 软件 | 描述 |
| --- | --- |
| `git` | 这是第一个要求，在 GitLab 上提交变更。这是一个版本控制软件，跟踪文件的变化 |
| `cmake` | 要构建自动化、测试和打包 c/c++ 应用程序，需要在 Linux 机器上安装 `cmake`。 |
| `gcc` | 它是一个需要编译 c/c++ 程序的编译器 |
...
| 变量/文件 | 路径 |
| --- | --- |
| `g++` | `export G++=/usr/bin/g++` |
| `gcc` | `export CC=/usr/bin/gcc` |
...
*   **输入此 runner 的 gitlab-ci 令牌**: 这将是注册时需要的每个项目的唯一令牌，可以在**找到**
...
**10.** 验证 GitLab Runner 已经注册了相应的存储库，并且 Runner 已经启动。转到 GitLab 帐户→选择要向跑步者注册的存储库→设置→ CI/CD →展开跑步者，将有绿色圆圈可用，并显示消息“跑步者将为此项目激活”。注意: 如果圆圈是灰色的，表示跑步者还没有开始，再次开始。
...
| `sudo gitlab-runner stop` | Stop the Runner of the project and replace the test runner with your runner name, which can be found in the `config.toml` file (where your `gitlab-runner` is located). |
| `sudo gitlab-runner unregister --url http://gitlab.example.com/ --token t0k3n` | Remove Runner by URL and token |
| `sudo gitlab-runner unregister --all-runners` | To log off all Runner |
| `sudo gitlab-runner restart` | This command stops and then starts the GitLab Runner service. |
| `sudo gitlab-runner run` | Actuator, running |
| `sudo gitlab-runner run-single` | By execution |
| `sudo gitlab-runner list` | You can see the name of the environment variable |
| `sudo gitlab-runner verify` | Run commands in debug mode. |

**.gitlab-ci.yml (Shell 执行者):**

以下是 `.gitlab-ci.yml` 的内容。外壳执行器模式下的 `.gitlab-ci.yml`。但是，如果需要，可以更改它。
