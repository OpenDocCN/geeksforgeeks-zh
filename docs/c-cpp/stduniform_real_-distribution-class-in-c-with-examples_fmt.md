# `std::uniform_real_distribution` 类用 C++ 举例

> 原文：[https://www.geeksforgeeks.org/stduniform_real_-distribution-class-in-c-with-examples/](https://www.geeksforgeeks.org/stduniform_real_-distribution-class-in-c-with-examples/)
...
[C++](https://www.geeksforgeeks.org/c-plus-plus/) 在`<random>`库中引入了`uniform_real_distribution`类，其成员函数以均匀概率给出来自给定输入范围的随机实数或连续值。

## `uniform_real_distribution` 类中的公共成员函数

1.  **`operator()`**：该函数返回给定范围内的随机值。返回值的数据类型是在模板类初始化期间指定的。任何值的概率都是一样的。该操作的时间复杂度为 O(1)。
