...
> 原文：[https://www.geeksforgeeks.org/c-program-for-ceiling-in-a-sorted-array/](https://www.geeksforgeeks.org/c-program-for-ceiling-in-a-sorted-array/)
...
int ceilSearch(int arr[], int low, int high, int x)
{
    ...
    /* Otherwise, linearly search for the ceiling value */
    for(i = low; i < high; i++){
        if(arr[i] == x)
            return i;
...
这里不用线性搜索，而是用二分搜索法来找出索引。二分搜索法将时间复杂度降低到 `O(Logn)`。
...
int ceilSearch(int arr[], int low, int high, int x)
{
    ...
    /* If x is greater than last element, then return -1 */
    if(x > arr[high])
        return -1;
...
更多详情请参考完整文章：[排序数组中的上限](https://www.geeksforgeeks.org/ceiling-in-a-sorted-array/)！