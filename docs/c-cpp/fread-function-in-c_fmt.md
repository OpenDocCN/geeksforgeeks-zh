...
C++ 中的 `fread()` 函数从流中读取数据块。这个函数首先从给定的输入流中读取对象的计数，每个对象的大小为 `size` 字节。成功读取的字节总数为 (`size` * `count`)。根据读取的字符数，文件位置指示器递增。如果读取的对象不是可复制的，那么行为是未定义的，如果 `size` 或 `count` 的值等于零，那么这个程序将简单地返回 0。
...
*   `计数`：它指定元素的数量，每个元素的大小为 `size` 字节。
*   `流`：指定读取数据的文件流。

**返回值：** 函数返回成功读取的对象数。如果发生错误，返回值可能小于 `count`。
