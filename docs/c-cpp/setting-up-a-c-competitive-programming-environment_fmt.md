...
> 原文：[https://www.geeksforgeeks.org/setting-up-a-c-competitive-programming-environment/](https://www.geeksforgeeks.org/setting-up-a-c-competitive-programming-environment/)
...
我个人最喜欢的是 `Sublime Text 3`，因为它重量轻，审美最低，功能高。

[下载 Sublime Text 3](https://www.sublimetext.com/3)
...
如果您正在使用 `Sublime Text`，那么按照以下步骤获取一个模板：
打开 `Sublime`，转到 `工具 > 开发人员 > 新代码片段`，点击它。
...
我已经抓取了一些代码来自动化提交过程(它需要 `Sublime Text`、`Linux` 操作系统和 `Git`)。
...
要使用上述代码；转到 `Sublime Text > 工具 > 构建系统 > 新建系统` 并粘贴此代码。用一个名称保存文件，然后再次转到 `工具 > 构建系统`，选择您为我们的新代码选择的名称。
全部完成……
