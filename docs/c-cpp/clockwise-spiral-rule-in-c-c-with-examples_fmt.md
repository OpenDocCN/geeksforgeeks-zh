...
> 原文: [https://www.geeksforgeeks.org/clockwise-spiral-rule-in-c-c-with-examples/](https://www.geeksforgeeks.org/clockwise-spiral-rule-in-c-c-with-examples/)
...
### 第三步: 现在，沿着顺时针的路径，用一些单词替换“|”，试着得到语法的意思，用“(”替换“|”。圆括号说明其功能，表示“`a`是功能”。
...
### 答案:
`f` 是一个以参数为 `int` 的函数，`p` 是一个以参数为 `int` 的函数，返回一个指向 `nothing (void)` 的常量指针返回一个指向[常量函数](https://www.geeksforgeeks.org/difference-between-static-and-constant-function-in-cpp/)的指针，以[字符数组](https://www.geeksforgeeks.org/difference-between-string-and-character-array-in-java/)为参数返回一个[字符串](https://www.geeksforgeeks.org/string-data-structure/)。
