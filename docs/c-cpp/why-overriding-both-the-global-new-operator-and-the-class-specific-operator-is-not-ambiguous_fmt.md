...
> 原文: [https://www.geeksforgeeks.org/why-overriding-both-the-global-new-operator-and-the-class-specific-operator-is-not-ambiguous/](https://www.geeksforgeeks.org/why-overriding-both-the-global-new-operator-and-the-class-specific-operator-is-not-ambiguous/)
...
编译器会调用内置的新函数。下面的例子演示了它是如何工作的。它确实包含特定于类的运算符 `new`，但不包含全局运算符 `new`。此后，在调用 `::new` 时，编译器调用标准库 `new` 函数。
