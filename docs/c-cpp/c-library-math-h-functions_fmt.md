...
`math.h` 头文件定义了各种数学函数和一个宏。这个库中所有可用的函数都以 `double` 为自变量，返回 `double` 为结果。让我们逐一讨论一些重要的功能。

## `double ceil(double x)`

C 库函数 `double ceil(double x)` 返回大于等于 `x` 的最小整数值。
...
C 库函数 `double floor(double x)` 返回小于或等于 `x` 的最大整数值。
...
C 库函数 `double log(double x)` 返回 `x` 的自然对数（以 `e` 为底的对数）。
...
C 库函数 `double log10(double x)` 返回 `x` 的公共对数（以 `10` 为底的对数）。
...
C 库函数 `double pow(double x, double y)` 返回 `x` 升到 `y` 的幂，即 x<sup>y</sup>。
...
C 库函数 `double modf(double x, double *integer)` 返回分数分量（小数后的部分），将 `integer` 设置为整数分量。
...
C 库函数 `double exp(double x)` 返回 `e` 的升至 `x` 次方的值。
...
C 库函数 `double cos(double x)` 返回一个弧度角 `x` 的余弦值。
...
C 库函数 `double acos(double x)` 以弧度为单位返回 `x` 的弧余弦。
...
C 库函数 `double tanh(double x)` 返回 `x` 的双曲正切值。
...
本文由 **Avinash Kumar Singh** 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 [contribute.geeksforgeeks.org](http://contribute.geeksforgeeks.org) 写一篇文章或者把你的文章邮寄到 contribute@geeksforgeeks.org。看到你的文章出现在极客博客主页上，帮助其他极客。
