# C++ STL 中的 unordered_map::end() 函数

> 原文：[https://www.geeksforgeeks.org/unordered_map-end-function-in-c-stl/](https://www.geeksforgeeks.org/unordered_map-end-function-in-c-stl/)

`unordered_map::end()` 是 C++ STL 中的内置函数，它返回一个迭代器，指向 `unordered_map` 容器中最后一个元素之后的位置。在无序映射对象中，不能保证哪个特定元素被认为是它的第一个元素。但是容器中的所有元素都被覆盖了，因为范围从它的开始到它的结束一直到无效。
...
该函数接受一个参数 `n`，该参数是一个可选参数，用于指定桶编号。如果设置了，迭代器将检索指向桶的结束元素的指针，否则，它将指向容器的结束元素。

## 返回值

该函数返回一个迭代器到容器末尾之后的元素。
