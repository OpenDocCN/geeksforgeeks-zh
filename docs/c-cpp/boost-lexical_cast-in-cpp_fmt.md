...
Boost 库中定义在 `boost/lexical_cast.hpp` 的 `lexical_cast` 提供了一个转换操作符 `boost::lexical_cast`，它可以将数字从字符串转换为数字类型，如 `int` 或 `double`，反之亦然。

`boost::lexical_cast` 是类似于 [`std::stoi()`](https://www.geeksforgeeks.org/stdstod-stdstof-stdstold-c/)、[`std::stod()`](https://www.geeksforgeeks.org/stdstod-stdstof-stdstold-c/) 和 [`std::to_string()`](https://www.geeksforgeeks.org/stdto_string-in-cpp/) 等函数的替代函数，这些函数在 C++11 中被添加到标准库中。
...
如果转换失败，将引发从 `bad_cast` 派生的 `bad_lexical_cast` 类型的异常。它引发了一个异常，因为 `float` 52.50 和字符串 “GeeksforGeeks” 不能转换为 `int` 类型的数字。
