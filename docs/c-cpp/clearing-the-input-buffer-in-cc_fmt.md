...
> 原文：[`https://www.geeksforgeeks.org/clearing-the-input-buffer-in-cc/`](https://www.geeksforgeeks.org/clearing-the-input-buffer-in-cc/)
...
**1. 使用 `while ((getchar()) != '\n');`**：键入 `while ((getchar()) != '\n');` 读取缓冲区字符直到结束，并丢弃它们(包括换行符)，并在 `scanf()` 语句清除输入缓冲区并允许在所需容器中输入后使用它。
...
**1. 使用 `cin.ignore(numeric_limits<streamsize>::max(),'\n');`**：- 键入 `cin.ignore(numeric_limits<streamsize>::max(),'\n');` 在 `cin` 语句之后，丢弃输入流中的所有内容，包括换行符。
