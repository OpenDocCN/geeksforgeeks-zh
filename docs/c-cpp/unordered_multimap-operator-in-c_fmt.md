# `unordered_multimap`的`operator=`在C++中

> 原文：[https://www.geeksforgeeks.org/unordered_multimap-operator-in-c/](https://www.geeksforgeeks.org/unordered_multimap-operator-in-c/)

`unordered_multimap::operator=`是C++ STL中的一个内置函数，它执行三种类型的任务，如下所述。
...
**参数：** 函数不接受任何参数。右边的容器是要将元素复制到左边容器的容器。
**返回值：** 不返回任何东西。
...
**参数：** 函数不接受任何参数。右边的容器是要将元素从其中移到左边容器的容器。使用`operator=`后，右侧容器中的元素将被销毁。
**返回值：** 不返回任何东西。
...
## 语法（用于分配来自不同`initializer_list`的元素）

```cpp
*unordered_multimap_name1* operator= (*initializer_list il*)
```

**参数：** 它不接受任何参数，它右边的列表将被分配给容器。
**返回值：** 不返回任何东西。
