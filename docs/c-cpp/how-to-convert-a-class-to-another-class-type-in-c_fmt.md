...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-a-class-to-another-class-type-in-c/](https://www.geeksforgeeks.org/how-to-convert-a-class-to-another-class-type-in-c/)
...
**例：**
设 `A` 和 `B` 两类。如果我们想将属于 `A` 类的细节分配给 `B` 类的对象，那么这可以通过以下方式实现：

> `B`（类 `B` 的对象）= `A`（类 `A` 的对象）
> 其中 `=` 已经为类类型 `B` 的对象重载。

类转换可以通过转换函数来实现，转换函数是通过使用[运算符重载](https://www.geeksforgeeks.org/operator-overloading-c/)来实现的。
