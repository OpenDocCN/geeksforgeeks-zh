...
*   这里包括 `<unistd.h>`[头文件](https://www.geeksforgeeks.org/write-header-file-c/)为 [`sleep()`](https://www.geeksforgeeks.org/java-concurrency-yield-sleep-and-join-methods/) 功能。
...
### `setup()`
使用 `rand()` 函数编写代码生成边界内的果实。

*   使用 `rand()%20` 因为边界的大小是 `height= 20` 和 `width= 20` 所以果实会在边界内生成。
...
### `input()`
在该功能中，程序员编写代码从键盘(W、A、S、D、X 键)获取输入。
...
### `logic()`
在这里，为这个程序写下所有的逻辑就像为蛇的运动，为增加分数，当蛇会触碰到边界游戏就结束了，为退出游戏而随机生成的果实一旦蛇会吃到果实。
...
### `sleep()`
C 中的这个函数是将程序执行延迟给定秒数的函数。在这段代码中，`sleep()`用于减慢蛇的移动速度，这样用户就可以轻松地玩耍。

### `main()`
从 `main()`函数开始执行程序。它调用所有的函数。
