# C++ 中的 ostream::seekp(pos) 方法示例

> 原文: [https://www.geeksforgeeks.org/ostreamseekppos-method-in-c-with-exmaples/](https://www.geeksforgeeks.org/ostreamseekppos-method-in-c-with-exmaples/)
...
该方法返回**该 ostream 实例**，位置设置为指定的新位置。

## 异常

如果操作设置了一个内部状态标志（除了 `eofbit`）注册了成员异常，函数抛出一个成员类型的异常 `failure`。

以下示例演示了 `seekp()` 方法在 C++ 中的使用:

## 示例 1: 演示 seekp() 在二进制文件中的使用
