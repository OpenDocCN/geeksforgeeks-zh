...
## std::find_first_of 用于比较两个容器之间的元素

它将范围 `[first1, last1)` 中的所有元素与范围 `[first2, last2)` 中的元素进行比较，如果在第一个范围中找到第二个范围中的任何元素，那么它将返回该元素的迭代器。
...
- `first1`: 前向迭代器，指向第一个范围中的第一个元素。
- `last1`: 前向迭代器，指向第一个范围中的最后一个元素。
- `first2`: 前向迭代器，指向第二个范围中的第一个元素。
- `last2`: 前向迭代器，指向第二个范围中的最后一个元素。

返回值: 它返回一个迭代器，指向 `[first1, last1)` 中第一个属于 `[first2, last2)` 的元素。如果没有找到匹配项或 `[first2, last2)` 为空，则函数返回 `last1`。
...
这里，`first1`, `last1`, `first2` 和 `last2` 与之前的情况相同。

- `pred`: 二元函数，接受两个参数（每个序列中的一个元素，顺序相同），并返回可转换为 `bool` 的值。返回的值表示在此函数的上下文中元素是否被视为匹配。该函数不得修改其任何参数。这可以是函数指针或函数对象。
...
`std::find_first_of` 可用于查找另一个容器中出现的任何元素的第一次出现。
...
`O(n1 * n2)`，其中，`n1` 为第一范围的元素个数，`n2` 为第二范围的元素个数。
