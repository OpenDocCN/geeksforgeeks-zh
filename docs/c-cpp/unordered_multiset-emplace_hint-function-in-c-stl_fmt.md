# C++ STL 中的 unordered_multiset::emplace_hint() 函数

> 原文: [https://www.geeksforgeeks.org/unordered_multiset-emplace_hint-function-in-c-stl/](https://www.geeksforgeeks.org/unordered_multiset-emplace_hint-function-in-c-stl/)

`unordered_multiset::emplace_hint()` 是 C++ STL 中的一个内置函数，它在 `unordered_multiset` 容器中插入一个新元素。它从参数中提供的位置开始搜索元素的插入点。位置只是一个提示，它并不决定插入的位置。插入会根据容器的标准在该位置自动完成。它将容器的尺寸增加了一个。
