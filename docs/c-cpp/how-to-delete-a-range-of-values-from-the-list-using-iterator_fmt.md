...
> 原文: [https://www.geeksforgeeks.org/how-to-delete-a-range-of-values-from-the-list-using-iterator/](https://www.geeksforgeeks.org/how-to-delete-a-range-of-values-from-the-list-using-iterator/)

给定一个`列表`，任务是使用迭代器从该列表中删除一系列值。
...
在该方法中，从列表中删除一系列元素。这在两个`迭代器`的帮助下完成。第一个迭代器指向范围的开始元素，第二个迭代器指向范围的最后一个元素。第一个迭代器是`独占`，而最后一个迭代器是`包含`，这意味着最后一个迭代器指向的元素也会被删除。
