...
> 原文：[https://www.geeksforgeeks.org/time-h-header-file-in-c-with-examples/](https://www.geeksforgeeks.org/time-h-header-file-in-c-with-examples/)
...
*   它还包含 `CLOCKS_PER_SEC` 宏，该宏保存系统时钟每秒滴答的次数。
*   **预定义功能**及时

| S.No | 函数名 | 说明 |
| --- | --- | --- |
| 1. | `asctime()` | 该函数以 `Day Month Date Hours:Minutes:Seconds Year` 的格式返回日期和时间。例如：`Sat Jul 27 11:26:03 2019`。`asctime()` 函数通过将 `struct tm` 变量作为参数返回一个字符串。 |
| 2. | `clock()` | 该函数返回程序消耗的处理器时间。 |
| 3. | `ctime()` | 此函数以 `Day Month Hours:Minutes:Seconds Year` 的格式返回日期和时间，例如：`Sat Jul 27 11:26:03 2019`。时间根据日历时间返回的指针打印。 |
| 4. | [`difftime()`](https://www.geeksforgeeks.org/difftime-c-library-function/) | 该函数返回提供的时间之间的差值。 |
| 5. | [`gmtime()`](https://www.geeksforgeeks.org/gmtime-function-in-c-c/) | 此功能打印世界协调时时间和日期。`gmtime()` 和 `asctime()` 的格式相同。 |
| 6. | `mktime()` | 此函数使用 `struct tm` 返回等效日历时间。 |
| 7. | `time()` | 此函数使用数据类型 `time_t` 返回等效日历时间。 |
