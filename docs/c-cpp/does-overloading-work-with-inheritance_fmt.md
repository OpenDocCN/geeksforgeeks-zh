...
> 原文: [https://www.geeksforgeeks.org/does-overloading-work-with-inheritance/](https://www.geeksforgeeks.org/does-overloading-work-with-inheritance/)
...
**注**: `Console.ReadKey()` 用于暂停控制台。它类似于 `C/C++` 中的 `getch`。
...
**解释:** 这里，我们正在创建派生类的对象，因此编译器将首先优先考虑派生类，如果需要，将执行隐式类型转换。所以一旦编译器来到 `Console.WriteLine(obj.f(3));` 这一行代码它会检查参数的兼容性。这里的 `3` 是 `int` ，兼容 `派生类函数 f` 的 `double`。所以编译器会执行 `int` 到 `double` 的隐式类型转换。于是输出 `f(double):6.3` 就来了。

现在当编译器进入 `Console.WriteLine(obj.f(3.3));` 同样，它会优先选择派生类，并发现它是可调用的。所以它将评估派生类函数 `f` 。于是输出 `f(double):6.6` 就来了。
...
**看到预期产量，你是否感到震惊？怎么可能？**
好了，这些问题我们都有答案了。因为我们已经创建了派生类的对象，所以 `C#` 编译器将首先优先考虑派生类，如果它没有找到任何兼容性，那么它将选择基类。所以当编译器来到 `Console.WriteLine(obj.f(3));` 一行代码，它将检查 `派生类方法 f` ，并发现它是可调用的，因此执行它，并输出 `f (int): 6` 。现在当 `Console.WriteLine(obj.f(3.3));` 一行代码将执行它会发现派生类方法不适合，因为 `3.3(double)` 与 `int` 数据类型不兼容，因此编译器现在更喜欢基类，在那里它找到了最佳匹配，并执行那个。所以那个的输出将是 `f(double):6.6` 。
