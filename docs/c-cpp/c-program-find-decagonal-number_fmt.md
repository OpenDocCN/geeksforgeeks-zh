...
> 原文: [https://www.geeksforgeeks.org/c-program-find-decagonal-number/](https://www.geeksforgeeks.org/c-program-find-decagonal-number/)

## 十边形数的定义

十边形数是将三角形和正方形的概念扩展到十边形（十边形）的一种图形数。第 n 个十边形数字计算 n 个嵌套十边形图案中的点数，所有十边形共享一个公共角，其中图案中的第 i 个十边形具有由彼此间隔一个单位的 i 个点组成的边。第 n 个十边形数由公式 `D(n) = 4n^2 - 3n` 给出。
