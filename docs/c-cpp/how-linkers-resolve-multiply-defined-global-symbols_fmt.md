...
> 原文：[https://www.geeksforgeeks.org/how-linkers-resolve-multiply-defined-global-symbols/](https://www.geeksforgeeks.org/how-linkers-resolve-multiply-defined-global-symbols/)
...
在这种情况下，链接器将生成错误消息，因为强符号 `main` 被定义了多次：
...
同样，链接器将为以下模块生成错误消息，因为强符号 `x` 被定义了两次：
...
但是，如果 `x` 在一个模块中未初始化，则链接器将悄悄地选择在另一个模块中定义的强符号，如以下程序中的情况：
...
规则 2 和 3 的应用会引入一些阴险的运行时错误，对于粗心的程序员来说是不可理解的，尤其是如果重复的符号定义有不同的类型。例：“x”在一个模块中定义为 `int`，在另一个模块中定义为 `double`。
