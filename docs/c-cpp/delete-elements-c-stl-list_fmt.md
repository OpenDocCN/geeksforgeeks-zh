...
## 1. 使用 `list::erase()`
此函数的目的是从列表中移除元素。可以使用此函数移除单个或多个连续范围内的元素。此函数接受 2 个参数，起始迭代器和结束迭代器。
**时间复杂度：** `O(n)`，其中 `n` 是列表的大小。
...
## 2. 使用 `list::pop_front()` 和 `list::pop_back()`
*   `pop_back()`：此功能从列表中删除最后一个元素。这会将列表的大小减少 1。
    时间复杂度：`O(1)`
*   `pop_front()`：此功能从列表中移除第一个元素，并移动后续元素。这会将列表的大小减少 1。
    时间复杂度：`O(1)`
...
## 3. 使用 `remove()` 和 `remove_if()`
*   `remove()`：此函数删除在其参数中传递的值的所有出现。与 `erase()` 不同的是，`erase()` 按位置删除值，而 `remove()` 删除传递的值。列表的大小会因删除的事件数量而减小。
    时间复杂度：`O(n)`
*   `remove_if()`：该函数删除出现的值，该值返回“真”给在其参数中传递的函数。
    时间复杂度：`O(n)`
