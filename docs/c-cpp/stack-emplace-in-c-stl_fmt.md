...
## stack::emplace()和stack::push()函数的区别

`push()`函数将值或传递给函数的参数的副本插入到顶部的容器中，而`emplace()`函数构造一个新元素作为参数的值，然后将其添加到容器的顶部。

## 应用

给定若干个整数，使用`emplace()`将它们加到栈中，不用`size`函数就能求出栈的大小。
