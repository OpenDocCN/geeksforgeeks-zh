...
> 原文：[https://www.geeksforgeeks.org/difference-between-include-and-include-in-c-c-with-examples/](https://www.geeksforgeeks.org/difference-between-include-and-include-in-c-c-with-examples/)

**先决条件：** [C/C++ 头文件及其用途](https://www.geeksforgeeks.org/header-files-in-c-cpp-and-its-uses/)

这两种类型的区别在于预处理器搜索要包含在代码中的文件的位置。

> **`#include <filename>`** // 标准库头文件
> **`#include "filename"`** // 用户自定义头文件

## `#include <filename>`

`#include < >` 是针对预定义的头文件。如果头文件是预定义的，那么只需将头文件名称写在尖括号中。
...
预处理器在包含指令的文件所在的目录中进行搜索。编译器将在当前文件夹或用户定义的文件夹中搜索这些头文件。此方法通常用于包含程序员定义的头文件。
...
| **序号** | **`#include <filename>`** | **`#include "filename"`** |
| :--- | :--- | :--- |
| 1 | 预处理器在编译器/IDE 预先指定的搜索目录中进行搜索。 | 预处理器在包含该指令的文件所在目录中进行搜索。 |
| 2 | 头文件可以在默认位置找到，例如 `/usr/include` 或 `/usr/local/include`。 | 头文件可以在用户定义的文件夹中找到。 |
| 3 | 此方法通常用于标准库头文件（如 `stdlib.h`）。 | 此方法通常用于程序员自定义的头文件。 |

## 示例演示

**情况 1：** 使用符号 `#include " "` 包含标准库头文件。
...
**解释：**
`#include ""` 将先搜索 `./`。然后它将搜索默认的包含路径。可以使用下面的命令打印包含路径。

> `gcc -v -o a filename.c`

**情况 2：** 使用符号 `#include < >` 包含标准头文件。
...
假设存在一个用户自定义的 `stdio.h` 文件：

```c
// stdio.h (user-defined)
int add(int a, int b)
{
  return(a+b);
}
...
1.  当在当前目录中创建 `stdio.h` 时，**情况 1** 中的代码将产生错误，但**情况 2** 中的代码将正常工作。
