...
> 原文：[https://www.geeksforgeeks.org/comparator-class-in-c-with-examples/](https://www.geeksforgeeks.org/comparator-class-in-c-with-examples/)
...
上面的比较器函数`operator()`一次取两对对象，如果两个运算符的数据成员相同，则返回 `true`。根据问题的需要，在比较器函数中可以有任何条件。在上面的例子中，如果数据成员相同，函数返回 `true`。
...
*   在上面的程序中，创建了一个`student`对象列表，其中最初插入了两个学生对象。
*   现在，为了搜索特定的学生，编写了一个模板线性搜索函数，它利用了比较器类。
*   比较器类根据学生的`name`属性比较要从学生列表中搜索的学生。
*   如果要搜索的对象的`name`属性等于列表中对象的任何`name`属性，则返回 `true`，否则返回 `false`。
...
*   在上面的程序中，创建了一个`student`对象列表，其中最初插入了 3 个学生对象。
*   现在，如果必须根据学生的`rollnum`属性对学生列表进行排序，那么必须将一个自定义比较器类作为参数传递给 [`sort()`](https://www.geeksforgeeks.org/sort-c-stl/) 函数。
