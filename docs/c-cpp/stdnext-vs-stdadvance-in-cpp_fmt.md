...
[`std::advance`](https://www.geeksforgeeks.org/stdadvance-in-cpp/) 不返回任何内容，而 [`std::next`](https://www.geeksforgeeks.org/stdnext-in-cpp/) 从给定的基本位置前进 n 个位置后返回一个迭代器。
...
[`std::advance`](https://www.geeksforgeeks.org/stdadvance-in-cpp/) modifies it arguments such that it points to the desired position, whereas, [`std::next`](https://www.geeksforgeeks.org/stdnext-in-cpp/) does not modify its argument, infact it returns a new iterator pointing to the desired position.
...
[`std::next`](https://www.geeksforgeeks.org/stdnext-in-cpp/) 要求作为参数传递的迭代器至少是[前向迭代器](https://www.geeksforgeeks.org/forward-iterators-in-cpp/)的类型，而 [`std::advance`](https://www.geeksforgeeks.org/stdadvance-in-cpp/) 没有这样的限制，因为它可以与任何迭代器一起工作，甚至与[输入迭代器](https://www.geeksforgeeks.org/input-iterators-in-cpp/)一起工作，或者比它更好。
