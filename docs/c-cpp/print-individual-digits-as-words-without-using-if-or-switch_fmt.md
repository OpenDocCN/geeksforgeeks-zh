...
> 原文：[https://www.geeksforgeeks.org/print-individual-digits-as-words-without-using-if-or-switch/](https://www.geeksforgeeks.org/print-individual-digits-as-words-without-using-if-or-switch/)
...
2.  创建另一个数组 `digits[]` 来存储 `n` 的单个数字。
3.  遍历 `n` 个数字，并将其存储在 `digits[]` 中。请注意，通过重复存储 `n%10` 并做 `n = n/10`，遍历的标准方式是以相反的顺序遍历数字。
