...
> 原文：[https://www.geeksforgeeks.org/train-a-support-vector-machine-to-recognize-facial-features-in-c/](https://www.geeksforgeeks.org/train-a-support-vector-machine-to-recognize-facial-features-in-c/)
...
有三种特征类型：年龄、情感和性别。四个年龄段，六种情绪，两种性别类型。因此，n 类分类器被实现来识别面部数据上的每个特征。
...
**步骤#2：** 每次运行时，迭代特征类型中的特征值，将图像变成向量或数组，即从文件夹中获取所有图像（例如，性别->男，性别->女）。
...
**步骤#8：** 将训练数据向量转换为 OpenCV Mat 对象以训练 SVM。
...
[使用 OpenCV 2.4 的 HOG SVM 的结果日志](https://github.com/manid2/FacialFeaturesRecognizer/blob/cv2.4/results_log.txt)
[使用 OpenCV 4.0 的 HOG SVM 的结果日志](https://github.com/manid2/ProgramsForFun/blob/master/LearnOpenCV/ModifiedSamples/cv4_hog_svm_success_log.txt)
