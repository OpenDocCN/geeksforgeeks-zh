...
> 原文：[https://www.geeksforgeeks.org/core-dump-segmentation-fault-c-cpp/](https://www.geeksforgeeks.org/core-dump-segmentation-fault-c-cpp/)
...
*   **`scanf()` 使用不当：**
    `scanf()` 函数需要一个变量的地址作为输入。在本程序中，`n` 取值 2，并假设其地址为 1000。如果我们将 `n` 传递给 `scanf()`，从 STDIN 获取的输入将被放置在无效内存 2 中，该内存应该是 1000。这是导致分段故障的内存损坏。
