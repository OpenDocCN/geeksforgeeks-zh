...
> 原文: [https://www.geeksforgeeks.org/ratio_less_equal-function-in-c/](https://www.geeksforgeeks.org/ratio_less_equal-function-in-c/)

`ratio_less_equal()` 是 C++ 中的一个内置函数，它检查比率 `R1` 是否小于或等于比率 `R2`。如果比率小于或等于比率 2，则返回真，否则返回假。
...
该功能接受两个要比较的模板参数 `ratio1` 和 `ratio2`。

## 返回值

如果比率 1 小于或等于比率 2，则该函数返回布尔值 `true`，否则返回 `false`。
