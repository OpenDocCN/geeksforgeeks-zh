...
`rand()` 函数在 C/C++ 中用于生成范围为 `[0, RAND_MAX]` 的随机数。
...
```cpp
int rand(void);
// 返回一个在范围 [0, RAND_MAX) 内的伪随机数。
// RAND_MAX: 是一个常量，其默认值可能因实现而异，但保证至少为 32767。
```
...
// 使用值 seed 为 rand() 使用的伪随机数生成器设置种子。
```
