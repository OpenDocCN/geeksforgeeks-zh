...
> 原文: [https://www.geeksforgeeks.org/access-is-denied-error-in-c-visual-studio-and-how-to-resolve-it/](https://www.geeksforgeeks.org/access-is-denied-error-in-c-visual-studio-and-how-to-resolve-it/)
...
安装 `VS Code` 和 [`MinGW` (C 编译器)](https://www.geeksforgeeks.org/complete-guide-to-install-c17-in-windows/)的方法有很多种，但是当这些方法被用来运行一个代码时，它可能会在输出或 `VS Code` 终端显示一些错误。但安装过程不正确的情况可能不会出现。这是因为系统的反病毒阻止了 `VS Code` 访问 `MinGW` 的 `bin` 文件夹。
