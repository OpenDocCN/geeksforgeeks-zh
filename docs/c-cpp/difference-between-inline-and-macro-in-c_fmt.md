...
> 原文：[https://www.geeksforgeeks.org/difference-between-inline-and-macro-in-c/](https://www.geeksforgeeks.org/difference-between-inline-and-macro-in-c/)

## 内联

内联函数是由 `inline` 关键字定义的普通函数。内联函数是由编译器扩展的短函数。它的参数只计算一次。内联函数是短长度函数，在类内不使用 `inline` 关键字，自动成为内联函数。
...
也叫预处理器指令。宏由 `#define` 关键字定义。在程序编译之前，只要预处理器检测到宏，预处理器就检查程序，然后预处理器用宏定义替换宏。
...
| S.NO | 内联 | 宏 |
| --- | --- | --- |
| 1. | 内联函数由 `inline` 关键字定义。 | 而宏是由 `#define` 关键字定义的。 |
