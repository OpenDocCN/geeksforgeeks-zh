...
> 原文：[https://www.geeksforgeeks.org/forward-list-and-list-of-pairs-in-c-with-examples/](https://www.geeksforgeeks.org/forward-list-and-list-of-pairs-in-c-with-examples/)

## 转发列表

[转发列表](https://www.geeksforgeeks.org/forward-list-c-set-1-introduction-important-functions/) 在 STL 中实现单链表。前向列表是从 C++ 11 引入的，在插入、移除和移动操作（如排序）方面比其他容器更有用，并且允许元素的时间常数插入和移除。它与列表的不同之处在于，转发列表只跟踪下一个元素的位置，而列表同时跟踪下一个和上一个元素，因此增加了存储每个元素所需的存储空间。向前列表的缺点是不能向后迭代，并且不能直接访问它的单个元素。当只需要向前遍历时，向前列表优于列表（与单链表优于双链表相同），因为我们可以节省空间。一些例子是，散列中的链接，图的邻接表表示等。
...
[列表](https://www.geeksforgeeks.org/list-cpp-stl/) 是允许非连续内存分配的序列容器。与 vector 相比，列表遍历速度慢，但是一旦找到位置，插入和删除就很快。通常，当我们说一个列表时，我们说的是一个双重链表。为了实现单链表，我们使用了一个转发列表。
...
一个 [对](https://www.geeksforgeeks.org/pair-in-cpp-stl/) 容器是一个简单的容器，在由两个数据元素或对象组成的 `<utility>` 头中定义。在一对中，第一个对象以 `first` 引用，第二个对象以 `second` 引用，顺序固定为 `{first, second}`。

**语法 1：**

> `forward_list<pair<data_type1, data_type2>> forward_list;`
>
> 这里 `data_type1` 和 `data_type2` 为数据类型。
...
// Function to print list
// contents
void print(list<pair<int, string>>& listOfPairs)
{
    cout << "List : " << '\n';
    for (auto currentPair : listOfPairs)
    {
...
int main()
{
    ...
    // Push the pair at the back in the list
    ...
    // Push the pair at the front in the list
