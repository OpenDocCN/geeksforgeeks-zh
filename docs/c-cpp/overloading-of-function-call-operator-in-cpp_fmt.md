...
> 原文：[https://www.geeksforgeeks.org/overloading-of-function-call-operator-in-cpp/](https://www.geeksforgeeks.org/overloading-of-function-call-operator-in-cpp/)
...
*   它修改了对象获取运算符的方式。
*   在[面向对象语言](https://www.geeksforgeeks.org/object-oriented-programming-in-cpp/)中，运算符 `()` 可以被视为普通运算符，类类型的[对象可以像对任何其他重载运算符进行函数调用一样调用函数(命名为 `operator()` )。](https://www.geeksforgeeks.org/c-classes-and-objects/)
*   当函数调用运算符重载时，不会创建调用函数的新方法，而是创建一个可以传递任意数量参数的 `operator()` 函数。
