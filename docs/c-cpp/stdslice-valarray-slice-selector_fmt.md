# std::slice (Valarray 切片选择器)

> 原文: [https://www.geeksforgeeks.org/stdslice-valarray-slice-selector/](https://www.geeksforgeeks.org/stdslice-valarray-slice-selector/)
...
- `size_t start` : 选择中第一个元素的索引。
- `size_t size` : 选择中的元素数量。
- `stride` : 分隔所选元素的跨度。

- 在给定的语法中，默认构造函数等同于 `slice(0, 0, 0)`。此构造函数的存在只是为了允许构造切片数组。
- 它用参数 `start`、`size`、`stride` 构建一个新的切片。该切片将参考元素的 `size` 编号，每个元素的位置为:
...
Explanation : starting from index 1 then next index `1 + 1 * 4 = 5`, next index `1 + 2 * 4 = 9`, next index `1 + 3 * 4 = 13`, next index `1 + 4 * 4 = 17`.

因此，步长大于 1 的切片不会选择 valarray 中的连续元素。例如，`slice(3, 4, 5)` 选择元素 3，8，13 和 18。
