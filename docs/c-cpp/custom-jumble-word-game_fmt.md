...
思路是使用[文件处理](https://www.geeksforgeeks.org/tag/c-file-handling/)的概念和一个包含所有有意义单词的文本文件(比如 `words.txt`)。以下是步骤:

1.  [给定字符串排序](https://www.geeksforgeeks.org/sort-string-characters/)。
2.  打开 `words.txt` 文件使用文件处理 `ifstream` 到[在 C++ 中读取文件为:
    > `ifstream word("words.txt");`
    >
    > *   对于文件中的每个单词 `words.txt` 对单词进行排序，并将其与给定的排序字符串进行比较。
    > *   如果在上述步骤中两个字符串都匹配，则在文件 `words.txt` 中打印当前单词。
    > *   检查完所有文字后关闭文件。](https://www.geeksforgeeks.org/readwrite-structure-file-c/)
