...
C++ 的 [`lower_bound()`](https://www.geeksforgeeks.org/lower_bound-in-cpp/) 方法返回[数组](https://www.geeksforgeeks.org/introduction-to-arrays/)中第一个元素的索引，其值不小于 `key`。这意味着该函数返回仅大于或等于下一个最小数字的索引。如果有多个值等于该数，`lower_bound()` 将返回第一个值的索引。
...
2.  迭代使用二分搜索法
3.  递归使用二分搜索法
4.  使用 `Arrays` 类的 `binarySearch()` 方法
...
2.  将键与中间元素进行比较 (`array[mid]`)。
3.  如果中间元素大于或等于键，则将 `high` 值更新为中间索引 (`mid`)。
4.  否则更新 `low` 至 `mid + 1`。
5.  重复步骤 2 至步骤 4，直到 `low` 小于 `high`。
6.  经过以上所有步骤后，`low` 就是给定数组中某个键的下界。
...
## 方法 4：使用 `Arrays` 类的 `binarySearch()` 方法

我们还可以使用 `Arrays` 类（或 `Collections` 类）的内置二分搜索实现。函数返回搜索关键字的索引，如果它包含在数组中；否则，返回 `(-(插入点) - 1)`。插入点被定义为将键插入数组的点。
