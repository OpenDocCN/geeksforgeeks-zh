...
> 原文: [https://www.geeksforgeeks.org/maximum-value-of-unsigned-char-in-c/](https://www.geeksforgeeks.org/maximum-value-of-unsigned-char-in-c/)
...
*   可以存储在无符号字符数据类型中的最大值通常为 `255`，约为 `2^8–1`（但取决于编译器）。
*   可以存储在无符号字符中的最大值作为常量存储在 [`<climits>`](https://www.geeksforgeeks.org/climits-limits-h-cc/) 头文件中，其值可以用作 `UCHAR_MAX`。
