...
> 原文：[https://www.geeksforgeeks.org/wcslen-function-in-cpp-with-examples/](https://www.geeksforgeeks.org/wcslen-function-in-cpp-with-examples/)

`wcslen()` 函数在 `cwcchar.h` 头文件中定义。函数 `wcslen()` 返回给定宽字符串的长度。
...
## 参数：
此方法采用单个参数 `str`，该参数表示指向要计算长度的宽字符串的指针。
