...
> 原文：[https://www.geeksforgeeks.org/different-ways-to-setting-up-environment-for-cpp-programming-in-mac/](https://www.geeksforgeeks.org/different-ways-to-setting-up-environment-for-cpp-programming-in-mac/)
...
**步骤 2：** 现在点击链接“`新建一个 Xcode 项目`”。

**步骤 3：** 同样，如果有一个现有项目，您将单击“`打开一个项目或文件`”，或者如果该项目在源代码管理中，则单击“`克隆一个现有项目`”。

**步骤 4：** 点击 `macOS` 标签，然后点击 `命令行应用`，因为这是我们唯一可以用 C 和 C++ 开发应用的地方。

![](img/9354ee22a539368bdc326a7c8424ee00.png)

第二步

**步骤 5：** 现在它将显示一个界面，给产品或应用程序一个名称和包名，作为一个普通的 IDE。在给出该名称后，将有一个下拉列表来选择一种语言。因此，您可以选择 `C` 或 `C++` 并单击“下一步”按钮，而不是 `swift`。

![](img/fa621d05b9e6eec480809f2d0c5307cf.png)

第三步。

**步骤 6：** 然后它会询问项目在你机器上的位置，并且，完成！，您现在可以打开或创建 C 或 C++ 文件。

![](img/c43089916212c9121340a434d264373b.png)

## 方式二：安装 Xcode 命令行工具并使用任何首选 IDE
...
**步骤 2：** 现在点击安装它会询问你的设备密码，所以输入密码后它会安装 Xcode 命令行工具。现在我们可以创建一个 C 或 C++ 文件并编辑它。

**步骤 3：** 现在使用 `VScode` 或者由你选择，因为它是最好的免费代码，并为 `C++` 安装两个扩展。
