# 如何在 C 中读取 PGMB 格式的图像

> 原文: [https://www.geeksforgeeks.org/how-to-read-a-pgmb-format-image-in-c/](https://www.geeksforgeeks.org/how-to-read-a-pgmb-format-image-in-c/)
...
[![](img/4e642b84fd57fbf4a351211ac2bf2cbd.png)](https://media.geeksforgeeks.org/wp-content/uploads/20201219090246/gfg200x200min.png)
...
在标题信息之后，有一个尺寸 `高度*宽度` 的网格，包含二进制格式图像的灰度像素值。
...
*   `ignoreComments()` 函数用于跳过文件中的任何注释。注释通常出现在标题部分，因此，我们会在读取每个属性后检查它们。
*   在 `openPGM()` 函数中，读取文件头信息，即文件类型、高度、宽度等如上所述。
