...
`getch()`是一个非标准函数，存在于 [`conio.h 头文件`](http://en.wikipedia.org/wiki/Conio.h)中，该文件主要由 [MS-DOS 编译器使用，如 Turbo C](https://www.geeksforgeeks.org/editors-types-system-programming/)。它不是 C 标准库或 ISO C 的部分，也不是 POSIX 定义的。
...
### 示例:使用 `getch()`
### 接受隐藏密码注意:
下面的代码不会在 Online 编译器上运行，而是在像 Turbo IDE 这样的 MS-DOS 编译器上运行。
