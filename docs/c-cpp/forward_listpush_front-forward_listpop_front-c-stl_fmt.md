...
> 原文：[https://www.geeksforgeeks.org/forward_listpush_front-forward_listpop_front-c-stl/](https://www.geeksforgeeks.org/forward_listpush_front-forward_listpop_front-c-stl/)
...
**结果：**
将名为`forwardlistname`的正向列表的开头添加参数中提到的值。
...
使用`push_front()`函数输入一个空的正向列表，该列表包含以下数字和顺序，并对给定的正向列表进行排序。
...
**结果：**
移除名为`forwardlistname`的给定正向列表开头存在的值。
...
使用`push_front()`功能，输入一个空的正向列表，列表的编号和顺序如下，打印列表的反面。
