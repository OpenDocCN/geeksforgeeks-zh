...
**std::move**
将范围 `[first, last]` 中的元素移动到从 `result` 开始的范围。
将 `[first, last]` 中元素的值转移到 `result` 指向的元素。调用后，范围 `[first, last]` 中的元素保持未指定但有效的状态。
...
本文由 **[沙钦·毕斯特](https://www.linkedin.com/in/sachin-bisht-984b5013a/)** 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 `contribute.geeksforgeeks.org` 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
