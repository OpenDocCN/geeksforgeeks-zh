...
> 原文: [https://www.geeksforgeeks.org/why-global-array-has-a-larger-size-than-the-local-array/](https://www.geeksforgeeks.org/why-global-array-has-a-larger-size-than-the-local-array/)
...
**程序 1:** 下面是 [C++ 程序](https://www.geeksforgeeks.org/c-plus-plus/)，其中在本地声明了一个大小为 `10^7` 的 1D 数组。
...
**说明:** 在上面的程序中，当局部声明一个一维数组时会出现一个[分割错误](https://www.geeksforgeeks.org/core-dump-segmentation-fault-c-cpp/)，那么这个数组大小的限制就是 `10^5` 的顺序。数组的[大小不能声明为大于 `10^5`](https://www.geeksforgeeks.org/how-to-find-size-of-array-in-cc-without-using-sizeof-operator/)。在本例中，声明了大小为 `10^7` 的数组，因此出现了错误。

**程序 2:** 下面是一个一维数组大小 `10^5` 的初始化程序:
...
**说明:** 上述程序中，代码编译成功，输出为 `1`。这是因为大小为 `10^5` 的 1D 数组是本地初始化的，这是有效的。

**程序 3:** 下面是一个一维数组大小 `10^7` 在全球范围内声明的程序。
...
**说明:** 以上代码中编译成功，输出为 `1`。这是因为大小为 `10^7` 的一维数组是全局声明的，这是有效的。

**注意:** 如果全局声明大小为 `10^8` 的一维数组，那么将再次遇到*分段错误*错误，因为一维数组的全局声明也有限制，也就是说，只能全局声明大小为 `10^7` 的一维数组。
...
*   当数组被全局声明时，它存储在[数据段](https://www.geeksforgeeks.org/memory-layout-of-c-program/)中，并且该数据段没有大小限制。因此，当数组被声明为大尺寸(即大于 `10^7`)时，堆栈内存变满并导致[堆栈溢出错误](https://www.geeksforgeeks.org/heap-overflow-stack-overflow/)，因此遇到分段错误。因此，对于声明更大大小的数组，最好是全局声明。