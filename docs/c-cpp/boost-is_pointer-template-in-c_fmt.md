...
> 原文: [https://www.geeksforgeeks.org/boost-is_pointer-template-in-c/](https://www.geeksforgeeks.org/boost-is_pointer-template-in-c/)
...
如果 `T` 是指针类型，则从 `true_type` 继承，否则从 `false_type` 继承。

## 语法

```cpp
boost::is_pointer<T>::value
boost::is_pointer<T>::value_type
```
