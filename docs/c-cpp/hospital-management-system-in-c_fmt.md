...
> 原文：[https://www.geeksforgeeks.org/hospital-management-system-in-c/](https://www.geeksforgeeks.org/hospital-management-system-in-c/)
...
1.  `PrintHospitalData()`：它将打印所有医院的数据。
2.  `PrintPatientData()`：将打印所有医院的数据。
3.  `SortHospitalByName()`：从名称排序所有医院
4.  `SortHospitalByRating()`：根据等级对医院进行排序
5.  `SortByBedsAvailable()`：根据可用床位对医院进行排序
6.  `SortByBedsPrice()`：按照最低价格对医院进行排序。

**实现思路：**

*   为`Hospital`数据集和`Patient`数据创建[类](https://www.geeksforgeeks.org/c-classes-and-objects/)。
*   初始化存储医院数据集和患者数据的变量。
*   为访问医院数据集和患者数据的`Hospital`和`Patient`类别创建[对象](https://www.geeksforgeeks.org/object-oriented-programming-in-cpp/)。
