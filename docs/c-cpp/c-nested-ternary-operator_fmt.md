...
这个三元运算符类似于 `if-else` 语句。所以可以用 `if-else` 语句的形式来表达。
...
这个嵌套三元运算符可以分解为 `if`、`else` 和 `else-if` 语句。表达式可以在三进制运算符和 `if else` 语句中分成更小的部分，如下所示:
