...
> 原文: [https://www.geeksforgeeks.org/similarities-between-java-and-c/](https://www.geeksforgeeks.org/similarities-between-java-and-c/)
...
它是程序执行的入口点。然而，函数声明是不同的，但名称是相同的。
...
这些数据类型包括 `int`、`float`、`char`、`double` 等。有一些区别，比如布尔数据类型在 Java 中被称为 `boolean`，但在 C++ 中被称为 `bool`。
