...
`atan2()` 是 C++ STL 中的一个内置函数，返回 (`y/x`) 的正切倒数，其中 `y` 是 y 坐标的比例，`x` 是 x 坐标的比例。数值介于 –π 和 π 之间，代表 (`x`, `y`) 点和正 x 轴的角度 θ。它是正 X 轴和点 (`X`, `y`) 之间的逆时针角度，以弧度为单位。
...
参数可以是 `double`、`float` 或 `long double` 数据类型。

## 返回值

该函数返回一个介于 –π 和 π 之间的数值，代表 (`x`, `y`) 点与正 x 轴的角度 θ。它是正 X 轴和点 (`X`, `y`) 之间的逆时针角度，以弧度为单位。
...
当字符串或字符作为参数传递时，函数返回 `no matching function call` 错误。
