...
> 原文: [https://www.geeksforgeeks.org/nested-structure-in-c-with-examples/](https://www.geeksforgeeks.org/nested-structure-in-c-with-examples/)

**先决条件:** [结构体](https://www.geeksforgeeks.org/structures-c/)
...
外部和内部结构变量被声明为普通变量，外部结构的数据成员使用单个点(`.`)访问，内部结构的数据成员使用两个点访问。下面是实现这个概念的 C 程序:
...
声明结构的一个普通变量和一个指针变量来解释两者的区别。在指针变量的情况下，点(`.`)和箭头(`->`)将用于访问数据成员。下面是实现上述方法的 C 程序:
