...
2.  根据 HOF 的定义，如果一个函数参数被应用到任何地方，通过调用一个专用的一阶函数来恢复该应用的参数，该函数将解释这个参数现在代表的去函数化符号。通常，该函数被称为 `apply` ，有时被称为 `eval` 。
...
现在，为了解除上述程序的功能，我们将用 `Lam` 数据类型值替换所有高阶函数(即 `o` ，这是这里唯一的高阶函数)，我们将启动 `apply` 函数来简化数据类型–
