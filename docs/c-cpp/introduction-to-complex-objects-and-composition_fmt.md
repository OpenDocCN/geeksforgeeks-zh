...
> 原文：[https://www.geeksforgeeks.org/introduction-to-complex-objects-and-composition/](https://www.geeksforgeeks.org/introduction-to-complex-objects-and-composition/)
...
在这里给出的类中，`Location`使用类`Point`的对象作为其数据成员。因此，`Location`是一个复杂的类，它使用一个简单的类`Point`。让我们看看利用作文的程序。
...
**说明：** 注意在程序中，`Two`类有一个`One`类的对象。要访问`One`的成员，我们必须使用`Two`的对象，如在命令`T.O.set(100)`中。此外，因为`num`是`One`的私有成员，所以我们必须使用公共函数来访问它的值。