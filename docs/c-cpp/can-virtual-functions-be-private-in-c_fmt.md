...
> 原文：[https://www.geeksforgeeks.org/can-virtual-functions-be-private-in-c/](https://www.geeksforgeeks.org/can-virtual-functions-be-private-in-c/)
...
上面的程序没什么值得注意的。`b_ptr` 是 `Base` 类型的指针，指向派生的类对象。调用 `ptr->print()` 时，执行派生的 `print()`。这段代码之所以有效，是因为基类定义了一个公共接口，而派生类在其实现中重写了它，即使派生类有一个私有的虚拟函数。
