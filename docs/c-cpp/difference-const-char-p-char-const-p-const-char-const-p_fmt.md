...
这是一个指向常量字符的指针。**你不能改变 `ptr` 挌向的值，但是你可以改变指针本身**。`"const char *"` 是指向 `const char` 的（非常数）指针。
...
这是一个指向非常数字符的常量指针。**你不能改变指针 `p`，但是可以改变 `ptr` 挌向的值。**
...
这是一个指向常量字符的常量指针。**你既不能改变指针指向的数值，也不能改变指针指向的数值。**
