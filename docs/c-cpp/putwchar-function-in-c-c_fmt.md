...
C/C++ 中的 `putwchar()` 函数将一个宽字符写入 `stdout`（标准输出）。在 `<cwchar.h>` 头文件中定义。
...
函数接受一个强制参数 `ch`，指定要写入的宽字符。

## 返回值

该函数返回如下两个值：
*   成功时，此函数返回由 `ch` 表示的宽字符。
*   如果出现写入错误，则返回 `WEOF` 并设置错误指示器。
...
```cpp
All Hebrew Alphabets : א ב ג ד ה ז ח ט י כ ך ל ם מ ן נ ס ע ף פ ץ צ ק ר ש ת
```
