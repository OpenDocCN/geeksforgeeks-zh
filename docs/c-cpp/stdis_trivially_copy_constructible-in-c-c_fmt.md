...
> 原文: [https://www.geeksforgeeks.org/stdis_trivially_copy_constructible-in-c-c/](https://www.geeksforgeeks.org/stdis_trivially_copy_constructible-in-c-c/)

`std::is_trivially_copy_constructible` 模板是一种类型，可以从相同类型的值或引用中进行普通构造。这包括标量类型、普通的可复制构造类以及这类类型的数组。这个算法将要测试一个类型是否可以简单地复制和构造。它返回显示相同的布尔值。
...
如果 `T` 是平凡可复制可构造的，那么它继承自 `true_type` 否则继承自 `false_type`。
...
该模板接受单个参数 `T` (性状类) 来检查 `T` 是否可以简单复制构建。
...
下面的程序说明了 C/C++ 中的 `std::is_trivially_copy_constructible` 模板:
