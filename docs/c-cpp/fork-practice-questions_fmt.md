...
**解说:**
1. 它将创建两个进程，一个是父进程(具有子进程的进程标识)，另一个是子进程 C1(进程标识= 0)。
2. 当条件为真时，父 P 执行 `if` 语句，子 C1 执行 `else` 语句并打印 3。父进程检查下一个 `if` 语句并创建两个进程(一个父进程和一个子进程 C2)。在 `if` 语句中，我们使用的不是运算符(即，`!`)，它为子进程 C2 执行，父进程 P 执行其他部分并打印值 2。子 C2 进一步创建了两个新流程(一个是父 C2，另一个是子 C3)。
