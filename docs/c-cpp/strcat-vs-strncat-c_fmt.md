...
[C++ 中的 `strncat()` 函数](https://www.geeksforgeeks.org/strncat-function-in-c-cpp/)将一个字符串中给定数量的字符追加到另一个字符串的末尾。`strncat()`函数将接受这三个参数：
1. `dest`
2. `src`
3. `count`
...
许多程序员建议，与 `strcat()` 相比，`strncat()` 是安全的，因为 `strcat()` 不会检查复制数据的大小，并且会一直复制到空终止符，这可能会导致缓冲区溢出，而 `strncat()` 会检查复制数据的大小，并且只会复制“n”个字节。
