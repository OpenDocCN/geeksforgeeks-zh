...
> 原文: [https://www.geeksforgeeks.org/stdfind_if-stdfind_if_not-in-c/](https://www.geeksforgeeks.org/stdfind_if-stdfind_if_not-in-c/)

## std::find_if

返回范围 `[first, last]` 中第一个元素的迭代器，`pred`（一元函数）为该元素返回 `true`。如果没有找到这样的元素，函数返回 `last`。
...
将迭代器返回到 `pred`（一元函数）返回 `false` 的 `[first, last]` 范围内的第一个元素。如果没有找到这样的元素，函数返回 `last`。
