...
2.  **段错误：** 程序可以访问一些不属于它的内存块，这可能导致程序崩溃，例如段错误。
...
*   然而，C++ 提供了 [`std::vector`](https://www.geeksforgeeks.org/vector-in-cpp-stl/) 类模板，它不需要执行边界检查。向量也有 [`std::at()`](https://www.geeksforgeeks.org/stdbasic_stringat/) 成员函数，可以执行边界检查。
