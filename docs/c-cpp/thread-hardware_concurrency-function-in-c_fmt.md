...
> 原文:`https://www . geesforgeks . org/thread-hardware _ concurrency-function-in-c/`

`Thread::hardware_concurrency`是 C++ `std::thread` 中的内置函数。它是一个**观测器函数**，这意味着它观测一个状态，然后返回相应的输出。该函数返回**可用硬件实现支持的并发线程数**。该值可能并不总是准确的。
