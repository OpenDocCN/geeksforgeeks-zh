# C++ STL 中的 multiset erase()

> 原文: [https://www.geeksforgeeks.org/multiset-erase-in-c-stl/](https://www.geeksforgeeks.org/multiset-erase-in-c-stl/)

**先决条件:** [multiset](https://www.geeksforgeeks.org/multiset-in-cpp-stl/)

`multiset::erase()` 是 C++ 中从 multiset 移除指定元素的 STL 函数。
...
*   `position_of_iterator`: 是指需要借助迭代器移除的特定元素的位置。

**返回值:** 这个方法返回被移除元素之后的迭代器。

以下示例说明了 `multiset::erase()` 方法的工作原理：
...
```cpp
size_type erase (const value_type& constant_value);
```

**参数:** 该方法接受以下参数：

*   `constant_value`: 指借助其值从 multiset 中移除的特定元素。它必须是恒定的。这个值的所有实例都会被这个方法删除。
...
*   `starting_iterator`: 指要从 multiset 中移除的值范围的起始迭代器。
*   `ending_iterator`: 是指要从 multiset 中移除的值范围的结束迭代器。
