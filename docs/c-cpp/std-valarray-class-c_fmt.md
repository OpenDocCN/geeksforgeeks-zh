...
*   支持元素数学运算和各种形式的广义下标运算符、切片和间接访问。
*   与 `vector` 相比，在某些数学运算中，`valarray` 也比 `vector` 更高效。

## `valarray` 类中的公共成员函数:

### 1. `apply()`
该函数将其参数中给出的操作一次应用于所有的 `valarray` 元素，返回一个带有操作值的新 `valarray`。

### 2. `sum()`
这个函数一次返回 `valarray` 所有元素的总和。
...
### 3. `min()`
该函数返回 `valarray` 的最小的元素。

### 4. `max()`
该函数返回 `valarray` 中最大的元素。
...
### 5. `shift()`
该函数通过参数中提到的号返回移动元素后的新 `valarray`。如果号为正，左移，如果号为负，则右移。

### 6. `cshift()`
该函数通过参数中提到的号循环移位(旋转)元素后返回新的 `valarray`。如果号为正，则应用左循环换档，如果号为负，则应用右循环换档。
...
### 7. `swap()`
此功能将一个 `valarray` 与另一个 `valarray` 互换。
