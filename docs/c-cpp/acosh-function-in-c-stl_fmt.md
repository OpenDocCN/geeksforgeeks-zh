...
该函数接受一个强制参数 `x`，该参数指定弧度的反双曲角应大于或等于 1。如果参数小于 1，则返回 `-nan`。参数可以是 `double`、`float` 或 `long double` 数据类型。

## 返回

`acosh()` 函数返回参数的反双曲正弦值，单位为弧度，范围为 `[0，inf]`。

根据 C++ 11 标准，有各种原型可供 `acosh()` 功能使用。

| 数据类型 | 原型 |
| --- | --- |
| For `double` | `double acosh(double x);` |
| For `float` | `float acosh(float x);` |
| For `long double` | `long double acosh(long double x);` |
...
| To `float` | `float a = 0; double b = acosh(double(a));` |
