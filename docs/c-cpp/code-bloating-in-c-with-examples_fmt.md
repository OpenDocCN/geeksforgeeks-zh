...
> 原文：[https://www.geeksforgeeks.org/code-bloating-in-c-with-examples/](https://www.geeksforgeeks.org/code-bloating-in-c-with-examples/)
...
在 C++ 中，我们有用于函数和容器的内置[模板](https://www.geeksforgeeks.org/templates-cpp/)。模板是具有不同数据类型的功能的泛化形式。模板的每个实例都是编译器生成的完全独立的代码片段。
...
在上面的模板函数 `findSum()` 中，每使用一种模板参数 `T` 就编译一次。对不同的 `T` 调用 `run()` 方法时，会多次编译 `GfG::run()` 中的函数。该功能可以花更多时间更改 `N` 的值。有各种技术来克服这个问题。头文件的问题也适用于模板，因为模板几乎总是在头文件中定义的。
...
上述程序将完美运行，并将打印 `GeeksForGeeks`。但是在主函数的第一行上面的程序中有一个代码膨胀的问题。以上程序的议程是打印字符串 `GeeksForGeeks`，那么为什么我们要创建字符串类的对象，因为它导致创建字符串类对象。我们可以简单地写：
