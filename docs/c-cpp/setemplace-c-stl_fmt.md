# 在 C++ STL 中使用 `set::emplace()`

> 哎哎哎: [https://www.geeksforgeeks.org/setemplace-c-STL/](https://www.geeksforgeeks.org/setemplace-c-STL/)
...
1.  它具有很强的异常保证，因此，如果抛出异常，则不会进行任何更改。
...
使用 `emplace()` 函数输入一个空的 `multiset`，输入以下数字和顺序，求元素的和。
...
当我们使用 `insert()` 时，我们创建一个对象，然后将其插入 `multiset`。通过 `emplace()`，对象被就地构建。
