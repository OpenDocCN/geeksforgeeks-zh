# 在 C++ 中生成 `generate_canonical` 函数模板

> 原文: [https://www.geeksforgeeks.org/stdgenerate_canonical-function-template-in-c/](https://www.geeksforgeeks.org/stdgenerate_canonical-function-template-in-c/)

C++ 中的这个模板用于将 `g` 生成的值转换为范围 `[0, 1]` 内的浮点值，保留用 `g` 生成的序列的一致性属性。为了生成足够的熵，`generate_canonical()` 将精确地调用 `g()` k 次，其中，
...
`generate_canonical()` 函数接受单参数 `g`，用作统一随机数生成器对象。它用于获取熵。
...
[http://www.cplusplus.com/reference/random/generate_canonical/](http://www.cplusplus.com/reference/random/generate_canonical/)