...
*   如果返回值等于 `GL_NO_ERROR` OpenGL 符号常量，一切都好。
*   如果有任何其他返回值，则表明出现了问题。

基本的 [OpenGL 库](https://www.geeksforgeeks.org/getting-started-with-opengl/)为代表不同误差条件的多个符号常数提供了定义。GLU 库也定义了一些错误代码，但是大多数都有几乎没有意义的名字，比如 `GLU_NURBS_ERROR_1`、`GLU_NURBS_ERROR_2`等等。
...
**解释:**由`gluErrorString`返回的值指向位于 GLU 库中的一个字符串。因为它不是一个动态分配的[字符串，所以它不能被程序显式解除分配。另外，它不能被程序修改(因此，字符串声明上的 const 修饰符)。在程序中将这些函数调用封装成一个通用的错误报告函数是非常容易的。](https://www.geeksforgeeks.org/what-is-dynamic-memory-allocation/)
...
如果这些`glCheckError`调用中的任何一个被分组到我们的数据库中，那么更精确地知道哪个`glCheckError`调用返回了错误是有帮助的。
