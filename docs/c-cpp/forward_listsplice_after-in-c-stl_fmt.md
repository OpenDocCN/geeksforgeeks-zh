...
> 原文: [https://www.geeksforgeeks.org/forward_listsplice_after-in-c-stl/](https://www.geeksforgeeks.org/forward_listsplice_after-in-c-stl/)

`forward_list::splice_after()` 是 CPP STL 中的一个内置函数，它将第一个+1到最后一个范围内的元素从一个给定的 `forward_list` 传输到另一个 `forward_list`。元素被插入到参数中位置所指向的元素之后。
