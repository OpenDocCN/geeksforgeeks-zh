...
> 原文：[https://www.geeksforgeeks.org/count-set-bits-floating-point-number/](https://www.geeksforgeeks.org/count-set-bits-floating-point-number/)

给定一个`浮点数`，编写一个函数来计算其二进制表示中的设置位。
例如，`0.15625`的浮点表示有`6`个设定位（见[本](http://en.wikipedia.org/wiki/Single_precision#IEEE_754_single-precision_binary_floating-point_format:_binary32)）。典型的`C`编译器使用`单精度浮点格式`。
