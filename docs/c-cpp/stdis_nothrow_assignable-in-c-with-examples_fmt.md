# std::is_nothrow_assignable 可在 C++ 中赋值，示例

> 原文: [https://www.geeksforgeeks.org/stdis_nothrow_assignable-in-c-with-examples/](https://www.geeksforgeeks.org/stdis_nothrow_assignable-in-c-with-examples/)

[C++ STL](https://www.geeksforgeeks.org/the-c-standard-template-library-stl/) 的 `std::is_nothrow_assignable` 模板存在于 `<type_traits>` 头文件中。C++ STL 的 `std::is_nothrow_assignable` 模板用于检查 `A` 是否类型可赋值给 `B`，这是众所周知的不抛出任何异常。如果 `A` 是可分配给 `B` 的类型，则返回布尔值 true，否则返回 false。
...
**参考:** [http://www.cplusplus.com/reference/type_traits/is_nothrow_assignable/](http://www.cplusplus.com/reference/type_traits/is_nothrow_assignable/)