...
这些都是最常出现的错误，只要掌握非常基本的编程知识就可以修复。但是如果出现这样的情况，其中[头文件](https://www.geeksforgeeks.org/write-header-file-c/)被包括在程序中，并且无意中完成了函数的正向声明，该函数已经是已经包括的头文件的一部分。在这种情况下，头文件的内容不会被记住，无论它是用户定义的还是预定义的。
...
*   让另一个头文件名为`mathematics.h`包含头文件`pentagon.h`。
...
*   现在包含`pentagon.h`的`mathematics.h`头文件修改为:
