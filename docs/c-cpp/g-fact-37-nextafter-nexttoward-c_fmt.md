...
## nextafter() 和 nexttoward()

> 原文: [https://www.geeksforgeeks.org/g-fact-37-nextafter-nexttoward-c/](https://www.geeksforgeeks.org/g-fact-37-nextafter-nexttoward-c/)
...
**`nextafter(x, y)` 和 `nexttoward(x, y)`**

在 C 和 C++ 中，`nextafter(x, y)` 和 `nexttoward(x, y)` 都是在 `math.h` 或 `cmath` 头文件中定义的类似函数。它们都沿 `y` 方向返回 `x` 之后的下一个可表示值。`nexttoward()` 具有更精确的第二个参数 `y`。
