...
让我们通过使用扫描集来实现 `gets()` 函数。`gets()` 函数将一行从 `stdin` 读入 `s` 指向的缓冲区，直到找到终止换行符或 `EOF`。
...
顺便提一下，使用 `gets()` 通常不是一个好主意。从 Linux 手册页查看下面的注释。
`千万不要用 gets()。因为如果不事先知道数据，就不可能知道 gets() 将读取多少个字符，而且因为 gets() 将继续存储超过缓冲区末尾的字符，所以使用起来极其危险。它已经被用来破坏计算机安全。请改用 fgets()。`
另见[本](https://www.geeksforgeeks.org/gets-is-risky-to-use/)帖。
