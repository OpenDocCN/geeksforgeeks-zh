...
头文件：`cmath`
参数：`rint()`函数接受单个参数值进行舍入。

返回值：默认情况下，舍入方向设置为“最接近的整数”，否则`rint()`函数使用`fegetround()`指定的舍入方向将参数舍入为整数值，并返回该值。
...
*   如果`rint()`函数与参数`3.7`一起使用，并且还使用了`fesetround(FE_DOWNLOAD)`，则输出将是`3`
*   如果`rint()`函数与参数`3.3`一起使用，并且还使用了`fesetround(FE_UPDATE)`，则输出将是`4`
