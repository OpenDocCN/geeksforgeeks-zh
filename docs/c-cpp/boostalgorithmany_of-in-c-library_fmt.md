...
[C++ boost 库](https://www.geeksforgeeks.org/advanced-c-boost-library/) 中的 `any_of()` 函数位于头文件 `"boost/algorithm/cxx11/any_of.hpp"` 下，该函数测试序列的元素，如果其中任何元素共享给定的属性，则返回 `true`。它接受一个序列和一个谓词，如果谓词为序列中的任何给定元素返回 `true`，则返回 `true`。
...
如果给定的谓词在序列的任何元素上为真，则函数返回真，否则返回假。
