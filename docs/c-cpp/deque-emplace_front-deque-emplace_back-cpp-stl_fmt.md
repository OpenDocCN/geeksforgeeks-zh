...
Parameters :
The element to be inserted into the deque is passed as the parameter.
Result :
The parameter is added to the deque at the beginning.
...
给定一个空的 deque，使用 `emplace_front()` 函数向其添加整数，然后不使用 `size()` 函数计算其大小。
...
1.  使用 `emplace_front()` 功能将元素添加到 deque。
2.  检查 deque 是否为空，如果不是，递增初始化为 0 的计数器变量，并弹出 back 元素。
3.  重复此步骤，直到 deque 变空。
4.  打印变量的最终值。
...
The element to be inserted into the deque is passed as the parameter.
Result :
The parameter is added to the deque at the end.
...
给定一个空的 deque，使用 `emplace_back()` 函数向其添加整数，然后计算元素的和。
...
1.  使用 `emplace_back()` 功能将元素添加到 deque。
2.  检查 deque 是否为空，如果不是，将 back 值添加到初始化为 0 的 `sum` 变量中，并弹出 back 元素。
3.  重复此步骤，直到 deque 变空。
4.  打印变量的最终值。
