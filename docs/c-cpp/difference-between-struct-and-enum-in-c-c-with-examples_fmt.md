...
> 原文: [https://www.geeksforgeeks.org/difference-between-struct-and-enum-in-c-c-with-examples/](https://www.geeksforgeeks.org/difference-between-struct-and-enum-in-c-c-with-examples/)

## C++ 中的结构

一个[结构](https://www.geeksforgeeks.org/structures-c/)是 C/C++ 中用户自定义的数据类型。结构创建的数据类型可用于将可能不同类型的项组合成单一类型。`struct`关键字用于创建结构。
