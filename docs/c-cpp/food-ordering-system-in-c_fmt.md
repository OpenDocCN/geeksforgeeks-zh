...
问题陈述: 编写[C 程序](https://www.geeksforgeeks.org/c-programming-language/)实现订餐，功能如下:
*   登录和注册
*   按食物搜索
*   按酒店搜索
*   购物车和订单确认
...
这个函数实现了我们项目的登录功能。登录时，会验证电子邮件和密码，并检查其是否已经注册。成功登录后，可以选择`search_by_food()`或`search_by_hotels()`功能。

### 酒店订单

在此功能中，食物订单是在选择任何酒店后下的。一旦选择了酒店，食物列表就会显示出它们各自的价格。一旦选择了食物，您需要进入选项`cart`以成功订购食物。

在酒店订购之前，初始化一些默认酒店，我们通过C中的[结构](https://www.geeksforgeeks.org/structures-c/)在本地给出这些酒店，以供我们演示。一旦酒店及其各自的食物被动态初始化，它将显示酒店列表。

### 按食物排序

一旦在上面的步骤中选择了酒店，选择食物并输入要订购的项目数量。一旦完成所有食物的选择，它将显示所选食物的总量。移至`cart`成功订购食物。
...
在 `main()` 函数中，程序通过一个 `while(1)` 循环持续运行，使用 `printf()` 和 `scanf()` 与用户交互，根据 `choice` 变量的值，通过 `switch` 语句调用 `signup()`、`login()` 函数或退出程序。

`signup()` 函数负责收集用户信息，包括 `temp_name`、`temp_age`、`temp_email`、`temp_password1`、`temp_password2` 和 `temp_mobile`，然后调用 `validate()` 函数进行验证。

`validate()` 函数对用户输入进行多项验证：
1.  使用 `for` 循环检查 `temp_name` 是否只包含字母。
2.  检查 `temp_email` 是否包含至少一个 `@` 和一个 `.`，且总长度不小于5。
3.  使用 `strcmp()` 函数比较 `temp_password1` 和 `temp_password2` 是否一致。
4.  检查密码长度是否在8到12位之间，并统计其中大写字母（`caps`）、小写字母（`small`）、数字（`numbers`）和特殊字符（`special`，如 `@`、`&`、`#`、`*`）的数量，要求每类至少一个。
5.  验证 `temp_age` 是否大于0。
6.  验证 `temp_mobile` 是否为10位数字。

如果所有验证通过，`validate()` 函数将返回1，`signup()` 函数随后会调用 `account_check()` 函数。
