...
> 原文：[https://www.geeksforgeeks.org/c-program-to-print-cross-or-x-pattern/](https://www.geeksforgeeks.org/c-program-to-print-cross-or-x-pattern/)
...
我们需要打印 `n` 行 `n` 列。所以我们运行两个嵌套循环。外部循环逐个打印所有行（运行 `i = 1` 到 `n`）。内部循环（运行 `j = 1` 到 `n`）运行当前行的所有列。现在一行可以包含空格和“$”。我们如何决定将空间放在哪里，将“$”放在哪里？

对于 `i = 1`：第一列和最后一列应包含“$”；
对于 `i = 2`：第二个和第二个最后一列应包含“$”；
一般来说，第 `i` 列和第 `(n+1–i)` 列应该包含“$”；
