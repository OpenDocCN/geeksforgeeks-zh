...
[`itoa()`](http://www.cplusplus.com/reference/cstdlib/itoa/) 函数将整数转换为空终止字符串。它也可以转换负数。`itoa()` 函数的标准定义如下：
...
第三个参数 `base` 指定转换基数。例如：如果 `base` 是 2，那么它将把整数转换成它的二进制兼容字符串；或者如果 `base` 是 16，那么它将创建整数的十六进制转换字符串形式。
如果基数为 10 且值为负，则结果字符串前面会有一个减号 (`-`)。对于任何其他基数，值总是被认为是无符号的。
参考: [http://www.cplusplus.com/reference/cstdlib/itoa/?kw=itoa](http://www.cplusplus.com/reference/cstdlib/itoa/?kw=itoa)
