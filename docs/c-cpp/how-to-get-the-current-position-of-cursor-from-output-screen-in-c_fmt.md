...
> 原文: [https://www.geeksforgeeks.org/how-to-get-the-current-position-of-cursor-from-output-screen-in-c/](https://www.geeksforgeeks.org/how-to-get-the-current-position-of-cursor-from-output-screen-in-c/)

给定的任务是从 C 语言的输出屏幕中获取光标的当前位置。

## 方法

C 语言中有一个预定义的函数 `wherex()`，返回光标在当前输出屏幕中的 x 坐标。以及 `wherey()` 函数，返回当前输出屏幕中光标的 y 坐标。这两个功能都在 `conio.h` 头文件中定义。
