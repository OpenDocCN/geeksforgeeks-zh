...
> 原文：[https://www.geeksforgeeks.org/c-program-for-finding-the-middle-element-of-a-given-linked-list/](https://www.geeksforgeeks.org/c-program-for-finding-the-middle-element-of-a-given-linked-list/)
...
将中间元素初始化为 `head`，将一个计数器初始化为 `0`。从头开始遍历列表，遍历时递增计数器，当计数器为奇数时，将中间变为下一个 `mid->next`。所以 `mid` 只会移动列表总长度的一半。
感谢纳伦德拉·康拉尔卡尔提出这个方法。
