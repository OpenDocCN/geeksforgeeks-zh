...
> 原文: [https://www.geeksforgeeks.org/lexicographical_compare-in-cpp/](https://www.geeksforgeeks.org/lexicographical_compare-in-cpp/)

C++ STL 提供了许多实用程序来解决基本的常见生活问题。比较值总是必要的，但有时我们也需要比较字符串。因此，本文旨在解释关于`lexicographical_compare()`，它允许比较字符串。该功能在`<algorithm>`表头定义。它有两个实现。
