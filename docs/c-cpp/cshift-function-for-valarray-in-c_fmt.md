...
> 原文：`https://www.geeksforgeeks.org/cshift-function-for-valarray-in-c/`

`cshift()` 函数在 `valarray` 头文件中定义。该函数返回一个相同大小的新 `valarray`，其元素的位置被 `n` 个元素循环移位。如果 `n` 为负，则应用右移，如果 `n` 为正，则应用左移。
