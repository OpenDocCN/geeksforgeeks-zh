...
> 原文：[https://www.geeksforgeeks.org/commonly-asked-c-programming-interview-questions-set-3/](https://www.geeksforgeeks.org/commonly-asked-c-programming-interview-questions-set-3/)

## 问题 1 写下最小的可执行代码？
Ans。`main` 是执行代码所必需的。代码是
...
Ans。分号是编译器需要的，顾名思义 Preprocessors 就是在编译前处理我们源代码的程序。因此不需要分号。

## 问题 4 包含带内角括号的头文件 `< >` 和双引号 `""` 有什么区别？
...
一个**远指针**通常为 32 位，可以访问当前段外的内存。为此，编译器分配一个段寄存器来存储段地址，然后分配另一个寄存器来存储当前段内的偏移量。
...
| 例如：简单的指针，我们通常在 C 和 C++ 中学习 | 在运行程序的设备中使用的指针，即通过该指针攻击其他计算机。 |

## 问题 6 为什么“类型降级”不存在，而“类型提升”不存在？此外，它将消耗更少的空间资源比做它从类型推广。？
Ans。我们举个例子来理解一下。

假设 `double a = 1.5; int b = 10;`，我们要计算 `a + b`。
...
## 问题 8 C 中的 `#include` 和 Java 中的 `import` 的区别？
Ans。

| #include | import |
| --- | --- |
| `#include` 是一个语句，不是关键字。 | `import` 是关键字。 |
| 由预处理器处理。 | 由编译器处理。 |
| 它会增加代码的大小。 | 它不会增加代码的大小。这里，即使我们写 `import java.lang.*;`，它也不会附加所有类。相反，它将允许访问 `java.lang` 的类。 |
...
详情请参考 `++*p`、`*p++` 与 `*++p` 的[区别](https://www.geeksforgeeks.org/difference-between-p-p-and-p/)。

## 问题 10 用例子解释深抄和浅抄？
在下面的 C 程序中，`struct` 变量 `st1` 包含指向动态分配内存的指针。当我们将 `st1` 分配给 `st2` 时，`st2` 的 `str` 指针也开始指向相同的存储位置。这种复制称为`浅复制`。
...
1.  [https://www.geeksforgeeks.org/commonly-asked-c-programming-interview-questions-set-1/](https://www.geeksforgeeks.org/commonly-asked-c-programming-interview-questions-set-1/)
2.  [https://www.geeksforgeeks.org/commonly-asked-c-programming-interview-questions-set-2/](https://www.geeksforgeeks.org/commonly-asked-c-programming-interview-questions-set-2/)