...
> 原文：[https://www.geeksforgeeks.org/how-to-restrict-dynamic-allocation-of-objects-in-c/](https://www.geeksforgeeks.org/how-to-restrict-dynamic-allocation-of-objects-in-c/)

C++ 编程语言允许自动（或堆栈分配）和动态分配对象。在 Java 和 C# 中，必须使用 `new` 动态分配所有对象。
出于运行时效率的考虑，C++ 支持栈分配对象。基于堆栈的对象由 C++ 编译器隐式管理。它们在超出范围时被销毁，动态分配的对象必须使用 `delete` 操作符手动释放，否则会发生内存泄漏。C++ 不支持像 Java 和 C# 这样的语言使用的自动垃圾收集方法。
...
的思想是保持 `new` 的运算符函数私有，这样就不能调用 `new`。请参见以下程序。无法使用 `new` 创建 `Test` 类的对象，因为 `new` 运算符函数在 `Test` 中是私有的。如果我们取消 `main()` 第二行的注释，程序将产生编译时错误。
