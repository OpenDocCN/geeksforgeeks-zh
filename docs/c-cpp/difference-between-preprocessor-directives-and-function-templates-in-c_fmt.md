...
> 原文：[https://www.geeksforgeeks.org/difference-between-preprocessor-directives-and-function-templates-in-c/](https://www.geeksforgeeks.org/difference-between-preprocessor-directives-and-function-templates-in-c/)
...
但是，该任务也可以使用使用预处理器指令 [`#define`](https://www.geeksforgeeks.org/interesting-facts-preprocessors-c/) 创建的[预处理器指令](https://www.geeksforgeeks.org/cc-preprocessors/)来执行。所以，这两个数字的最小值可以定义为：
...
| **S 号** | **预处理器指令** | **函数模板** |
| :--- | :--- | :--- |
| **1** | 没有完整的类型检查。 | 有完整的类型检查。 |
| **2** | 它们与 `#define` 一起使用。 | 它们使用 `template` 关键字定义。 |
| **3** | 在编译前处理。 | 在编译期间处理。 |
| **4** | 它们可能导致意外结果。 | 没有这样的意外结果。 |
| **5** | 它们不保证示例中的类型安全。 | 它们保证类型安全。 |
| **6** | 它们没有清晰和完整的特化。 | 它们有清晰和完整的特化。 |