...
> 原文：[https://www.geeksforgeeks.org/incompatibilities-between-c-and-cpp/](https://www.geeksforgeeks.org/incompatibilities-between-c-and-cpp/)
...
在 C 语言中，可以使用一种语法来定义函数，该语法可以在参数列表之后选择性地指定参数类型：
...
在 C 语言中，一个全局数据对象可以多次声明，而无需使用`extern`说明符。只要最多有一个这样的声明提供了初始值设定项，对象就被认为只定义了一次。
...
在 C 语言中，`void*`可用作任何指针类型变量赋值或初始化的右手操作数。
...
在 C 语言中，数组可以由初始值设定项初始化，该初始值设定项包含的元素比数组要求的多。
...
在 C 语言中，一个没有指定任何参数类型的函数可以接受任意数量的任何类型的参数。点击[这里](https://www.geeksforgeeks.org/difference-int-main-int-mainvoid/)了解更多。
