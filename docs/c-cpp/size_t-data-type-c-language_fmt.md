...
它是一种用于以字节表示对象大小的类型，因此被`sizeof`运算符用作返回类型。它保证足够大，以容纳主机系统可以处理的最大对象的大小。基本上，最大允许大小取决于编译器；如果编译器是 32 位，那么它只是一个用于`unsigned int`的 typedef(即别名)，但是如果编译器是 64 位，那么它将是用于`unsigned long`的 typedef。`size_t`数据类型从不为负。
