# c++ STL 中的 multiset::upper_bound()，示例

> 原文：[`https://www.geeksforgeeks.org/multiset-upper_bound-in-cpp-stl-with-examples/`](https://www.geeksforgeeks.org/multiset-upper_bound-in-cpp-stl-with-examples/)

`multiset::upper_bound()`是 C++ STL 中的一个内置函数，它返回一个迭代器，该迭代器指向刚好大于 `k` 的下一个元素。如果参数中传递的键超过了容器中的最大键，则迭代器返回的元素指向容器中最后一个元素之后的位置。
...
**参数：** 该函数接受一个强制参数 `key`，指定要返回其上限的元素。
