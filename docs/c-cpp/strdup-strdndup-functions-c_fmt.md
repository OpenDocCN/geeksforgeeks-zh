...
该函数返回一个指向空终止字节字符串的指针，该字符串与`s`指向的字符串重复。获得的内存使用`malloc`动态完成，因此可以使用`free()`释放。
它返回一个指向重复字符串`s`的指针。
...
语法：`char * strndup(const char * s, size_t n);`
...
参考：[Linux man page](http://man7.org/linux/man-pages/man3/strdupa.3.html)
