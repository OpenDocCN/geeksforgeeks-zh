...
> 原文: [https://www.geeksforgeeks.org/unordered_set-operators-in-c-stl/](https://www.geeksforgeeks.org/unordered_set-operators-in-c-stl/)
...
[C++ STL 中的无序集合==运算符](https://www.geeksforgeeks.org/unordered_set-operator-in-c-stl-3/)

`==` 是 C++ STL 中的一个运算符，在两个无序集之间执行相等比较运算。`unordered_set::operator==` 是相同的对应运算符函数。
...
**参数:** 该运算符函数引用两个无序集 `uset1` 和 `uset2` 作为要比较的参数。

**返回值:** 该方法在比较两个集合后返回一个布尔结果值。比较程序如下:

*   首先比较它们的大小。
*   然后在 `uset2` 中寻找 `uset1` 中的每个元素。

如果两个条件都满足则返回 `true` 值，如果条件不满足，则返回 `false` 值。

下面的程序说明了 C++ 中的 `unordered_set::operator==` 的用法。
...
[无序集合!= c++ STL 中的运算符](https://www.geeksforgeeks.org/unordered_set-operator-in-c-stl-2/)
...
`=` 是 C++ STL 中的一个运算符，它将一个无序集合复制(或移动)到另一个无序集合，而 `unordered_set::operator=` 是对应的运算符函数。这个函数有三个版本。
...
uset.operator= ( unordered_set&& us )
uset.operator= ( initializer_list )
```
...
下面的程序说明了 C++ 中的 `unordered_set::operator=`。
