...
> 原文：[https://www.geeksforgeeks.org/difference-between-type-casting-and-type-conversion/](https://www.geeksforgeeks.org/difference-between-type-casting-and-type-conversion/)
...
`()` 是一个转换运算符。
`target_datatype` 是我们要将源数据类型转换成的数据类型。
...
在第 5 行：可以看到，我们正在将`float`（源）数据类型转换为`byte`（目标）数据类型。
