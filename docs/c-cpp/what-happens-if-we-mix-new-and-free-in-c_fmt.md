# 如果我们在 C++ 中混合 new 和 free 会发生什么？

> 原文：[https://www.geeksforgeeks.org/what-happens-if-we-mix-new-and-free-in-c/](https://www.geeksforgeeks.org/what-happens-if-we-mix-new-and-free-in-c/)
...
我们知道 `new` 关键字创建的内存是使用 `delete` 关键字删除的，而 `malloc()` 创建的内存是由 `free()` 删除的。`new` 调用[构造函数](https://www.geeksforgeeks.org/constructors-c/)，`delete` 调用[析构函数](https://www.geeksforgeeks.org/destructors-c/)进行内存释放。现在，使用 `new` 关键字创建内存，并尝试使用 `free()` 删除它，那么析构函数将不会被调用，因此内存和资源也不会被释放。而且会导致内存和资源泄露。下面是分析 `free()` 和 `delete` 如何表现的程序：
...
在上面的代码中，使用 `new` 关键字创建了一个对象，我们试图使用 `free()` 删除这个对象。它不会调用这个对象的析构函数，这个对象的内存和资源也不会释放。因此，总是建议不要在 [C++ 程序](https://www.geeksforgeeks.org/c-plus-plus/)中混合 `new` 和 `free`，以避免额外的努力和时间。