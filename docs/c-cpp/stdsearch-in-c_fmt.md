...
`std::search` 在头文件 `<algorithm>` 中定义，用于找出满足关于另一个序列的条件（如果没有定义这样的谓词，等于）的子序列的存在。
...
>
> **first1:** 向前迭代器，到达要搜索的第一个容器的开头。
> **last1:** 向前迭代器到要搜索的第一个容器的末尾。
> **first2:** 向前迭代器到要搜索的第二个容器的子序列的开头。
> **last2:** 前向迭代器到要搜索的第二个容器的子序列的结尾。
> **返回:** 第一个元素的迭代器，`[first2, last2)` 在 `[first1, last1)` 中第一次出现，如果没有发现出现，则返回 `last1`。
...
> **返回:** 一个迭代器，返回满足谓词的 `[first2, last2)` 在 `[first1, last1)` 中第一次出现的第一个元素，如果没有发现出现，则返回 `last1`。
