...
> 原文: [https://www.geeksforgeeks.org/setting-c-development-environment/](https://www.geeksforgeeks.org/setting-c-development-environment/)
...
`ide.geeksforgeeks.org` 就是 GeeksforGeeks 提供的这样一个 IDE。
可以点击 IDE 上的运行按钮运行程序。
...
*   `filename.cpp` 是你的源代码文件的名称。在我们的例子中，名字是“helloworld.cpp”和 `any-name` 可以是你选择的任何名字。该名称将分配给编译后由编译器创建的可执行文件。在我们的例子中，我们选择 `any-name` 作为“你好”。
    我们将运行上述命令如下:
...
*   **Mac OS X 安装**:如果你是 Mac 用户，就要下载 Xcode。要下载 Xcode，你必须访问苹果网站，或者你可以在苹果应用商店搜索它。您可以通过 [developer.apple.com/technologies/tools/](https://developer.apple.com/technologies/tools/) 链接下载 Xcode。您将在那里找到所有必要的安装说明。
