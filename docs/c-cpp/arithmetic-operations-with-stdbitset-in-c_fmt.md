...
一个 [bitset](https://www.geeksforgeeks.org/c-bitset-and-its-application/) 是一个由布尔值组成的 [array](https://www.geeksforgeeks.org/array-data-structure/)，但是每个布尔值并不是单独存储的。相反，位集优化了空间，使得每个 `bool` 只占用 1 位空间，因此位集占用的空间，比如说，`bs` 小于 `bool bs[N]` 和 [`vector<bool> bs(N)`](https://www.geeksforgeeks.org/vector-in-cpp-stl/)。然而，位集的一个限制是，`N` 在编译时必须是已知的，即常数（`vector` 和动态数组没有这个限制）。

## 重要提示

> *   注意整数溢出。例如，如果位集声明为大小 3 而加法结果是 9，这就是整数溢出的情况，因为 9 无法存储在 3 位中。
> *   注意负结果，因为位集被转换为无符号长整型，所以负数无法存储。

## 增加 2 个位组

按照以下步骤解决问题:

*   初始化一个 `bool` `carry` 到 `false`。
*   创建一个位集 `ans` 来存储两个位集 `x` 和 `y` 的总和。
*   遍历位组 `x` 和 `y` 的长度，使用 [全加器](https://www.geeksforgeeks.org/full-adder-in-digital-logic/) 功能确定 `ans` 中当前位的值。
*   返回 `ans`。
...
*   创建一个位集 `ans` 来存储两个位集 `x` 和 `y` 之间的差异。
*   遍历位组 `x` 和 `y` 的长度，使用 [全减法器](https://www.geeksforgeeks.org/full-subtractor-in-digital-logic/) 功能确定 `ans` 中当前位的值。
*   返回 `ans`。
