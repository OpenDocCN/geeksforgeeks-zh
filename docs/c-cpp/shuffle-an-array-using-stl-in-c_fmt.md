...
STL 包含两种方法，可以用来得到一个混洗的数组。这就是 [`shuffle()`](https://www.geeksforgeeks.org/shuffle-vs-random_shuffle-c/) 和 `random_shuffle()`。
...
*   `shuffle` 在 C11++ 之后推出，使用的函数比 `random_shuffle` 使用的 `rand()` 要好。
