...
> 原文：[https://www.geeksforgeeks.org/rearrange-characters-in-a-string-such-that-no-two-adjacent-are-same-using-hashing/](https://www.geeksforgeeks.org/rearrange-characters-in-a-string-such-that-no-two-adjacent-are-same-using-hashing/)

给定具有重复字符的字符串 `str`，任务是重新排列字符串中的字符，使得没有两个相邻的字符相同。如果可能，则打印 `Yes` 否则打印 `No`。

**示例：**

> **输入：** `str = "geeksforgeeks"`
> **输出：** Yes
> “egeksforgeks”就是这样一种安排。
>
> **输入：** `str = "bbbbbbb"`
> **输出：** No

**方法：** 思路是将每个字符的频率存储在一个 `unordered_map` 中，将字符的最大频率与字符串长度和最大频率数的差值进行比较。如果最大频率小于该差值，则可以不这样安排。

1.  让我们开始交替放置所有具有最大频率的字符。那么至少，我们需要它们之间的 `(max_freq - 1)` 个空间来解决这个问题，这样它们就不会彼此相邻。
2.  但是我们还剩下 `(length of the string – max_freq)` 个空格。因此，`(length of the string – max_freq)` 应该至少为 `(max_freq - 1)`，这样就不会有两个字符相同。
