...
ASP 中的`缓冲属性`用于指定输出是否缓冲。当页面缓冲时，服务器不响应或发送数据到客户端，直到剩余的服务器脚本被成功处理，或直到调用`Flush`或`End`方法。

*   **Flush方法：** 此用于立即发送缓冲的 HTML 输出。
*   **End方法：** 此为停止脚本的工作进程。它通常返回当前结果。

当服务器开始向服务器发送数据后，此属性不能中断。`缓冲属性`应该设置为`.asp`文件的第一行代码。
...
*   `flag`： 它包含指定输出是否应该缓冲的布尔值。
    1.  `true`： 定义页面被缓冲。
    2.  `false`： 指定输出不会被缓冲。

**注意：** 如果设置了`response.Buffer=true`在无限循环中，输出永远不会发送到客户端。
