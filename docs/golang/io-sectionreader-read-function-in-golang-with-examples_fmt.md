...
用 Go 语言，`io` 包为 I/O 原语提供基本接口。它的主要工作是封装这类原语之正在进行的实现。Go 语言中的 `io.SectionReader.Read()` 函数用于返回 `NewSectionReader` 读取的字节数，该方法以缓冲区为参数。该函数是在 `io` 包下定义的，需要导入 `"io"` 包才能使用。
...
返回指定长度的指定缓冲区内容的字节数，如果有错误则返回错误，如果没有错误则返回 `nil`。
