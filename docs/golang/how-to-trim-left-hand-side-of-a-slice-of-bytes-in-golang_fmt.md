...
> 原文：[https://www.geeksforgeeks.org/how-to-trim-left-hand-side-of-a-slice-of-bytes-in-golang/](https://www.geeksforgeeks.org/how-to-trim-left-hand-side-of-a-slice-of-bytes-in-golang/)
...
在字节的 Go 切片中，您可以使用 `TrimLeft()` 函数从给定切片中修剪所有 UTF-8 编码的起始码点。这个函数通过切掉给定字符串中指定的所有 UTF-8 编码码点，返回原始切片的子切片。如果给定的字节片段在其左侧不包含指定的字符串，则此函数返回原始片段，不做任何更改。它是在 `bytes` 包下定义的，所以你必须在你的程序中导入 `bytes` 包来访问 `TrimLeft` 函数。
