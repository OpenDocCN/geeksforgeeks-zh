...
Golang 中的 `strings.Replace()` 函数用于返回给定字符串的副本，其中前 `n` 个不重叠的 `old` 实例被 `new` 实例替换。
...
这里，`s` 是原始或给定的字符串，`old` 是要替换的字符串。`new` 是替换旧的，`n` 是替换 `old` 的次数。

## 注意：

如果 `old` 为空，则在字符串开头和每个 UTF-8 序列之后匹配，从而为 `k` 符文字符串生成最多 `k+1` 个替换。 如果 `n<0`，则对替换次数没有限制。
