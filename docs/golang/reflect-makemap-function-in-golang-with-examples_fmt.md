...
> 原文：`https://www.geeksforgeeks.org/reflect-makemap-function-in-golang-with-examples/`

Go 语言提供了运行时反射的内置支持实现，并允许程序在反射包的帮助下操作任意类型的对象。`MakeMap()` 函数用于创建指定类型的新地图。要访问这个函数，需要在程序中导入 `reflect` 包。
