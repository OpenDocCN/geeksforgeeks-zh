...
> 原文：[https://www.geeksforgeeks.org/how-to-find-the-last-index-value-in-slice-of-bytes-in-golang/](https://www.geeksforgeeks.org/how-to-find-the-last-index-value-in-slice-of-bytes-in-golang/)
...
在 Go 分片字节中，可以使用 `LastIndex()` 函数找到给定分片的指定实例的最后一个索引值。该函数返回原始字节片段中给定值的最后一个实例的索引。如果给定值在原始切片中不可用，则返回 `-1`。它是在 `bytes` 包下定义的，所以您必须在程序中导入 `bytes` 包来访问 `LastIndex` 函数。
