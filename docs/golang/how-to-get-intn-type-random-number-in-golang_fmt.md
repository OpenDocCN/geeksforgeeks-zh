...
原文: [https://www.geeksforgeeks.org/how-to-get-intn-type-random-number-in-golang/](https://www.geeksforgeeks.org/how-to-get-intn-type-random-number-in-golang/)
...
在 `math/rand` 包提供的 `Intn()` 函数的帮助下，您可以从默认来源生成 `int` 类型的 `[0，n]` 中的非负伪随机数。因此，您需要在程序中添加一个 `math/rand` 包，借助 `import` 关键字来访问 `Intn()` 函数。如果 `n` 的值为 `<= 0`，这个方法会出现恐慌。
