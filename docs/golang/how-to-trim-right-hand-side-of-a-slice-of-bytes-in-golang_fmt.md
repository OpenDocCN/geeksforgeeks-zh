# 如何在 Golang 中修剪一片字节的右侧？

> 原文: [https://www.geeksforgeeks.org/how-to-trim-right-hand-side-of-a-slice-of-bytes-in-golang/](https://www.geeksforgeeks.org/how-to-trim-right-hand-side-of-a-slice-of-bytes-in-golang/)
...
在字节的 Go 切片中，您可以使用 `TrimRight()` 功能修剪给定切片中所有结束的 UTF-8 编码码点。该函数通过切断给定字符串中指定的所有 UTF-8 编码的尾随码点，返回原始切片的子切片。如果给定的字节片段在其右侧不包含指定的字符串，则此函数返回原始片段，不做任何更改。它是在 `bytes` 包下定义的，所以您必须在程序中导入 `bytes` 包来访问 `TrimRight` 函数。
