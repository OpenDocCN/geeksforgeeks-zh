...
在 Golang 中，我们可以使用 `os` 包读写环境变量。

**1.** 用 `os.Setenv()` 设置环境变量。此方法接受两个字符串参数。它会返回一个错误(如果有)。

```go
os.Setenv(key, value)
```

**2.** 用 `os.Getenv()` 获取环境变量值。如果变量存在，此方法返回该变量的值，否则返回空值。

```go
os.Getenv(key)
```

**3.** 使用 `os.Unsetenv()` 删除或取消设置单个环境变量。此方法返回一个错误(如果有)。

```go
os.Unsetenv(key)
```

**4.** 用 `os.LookupEnv()` 获取环境变量值和一个布尔值。布尔值表示键是否存在。如果键不存在，则返回 `false`。

```go
os.LookupEnv(key)
```

**5.** 用 `os.Environ()` 列出所有环境变量及其值。此方法返回“`键=值`”格式的字符串副本。

```go
os.Environ()
```

**6.** 用 `os.Clearenv()` 删除所有环境变量。
