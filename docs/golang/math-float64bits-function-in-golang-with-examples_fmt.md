...
Go 语言提供了对基本常量和数学函数的内置支持，以便在 `math` 包的帮助下对数字执行运算。此软件包提供 `Float64bits()` 函数，该函数返回 `a` 的 IEEE 754 二进制表示，符号位为 `a`，结果位于相同的位位置。因此，需要在 `import` 关键字的帮助下在程序中添加一个 `math` 包来访问 `Float64bits()` 函数。
