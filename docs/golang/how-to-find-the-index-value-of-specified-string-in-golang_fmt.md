...
> 原文：[https://www.geeksforgeeks.org/how-to-find-the-index-value-of-specified-string-in-golang/](https://www.geeksforgeeks.org/how-to-find-the-index-value-of-specified-string-in-golang/)
...
此函数用于从原始字符串中查找给定字符串的第一个实例的索引值。如果给定的字符串在原始字符串中不可用，则此方法将返回 `-1`。
...
此方法从原始字符串中的字符返回任何 Unicode 代码点的第一个实例的索引。如果字符的 Unicode 代码点在原始字符串中不可用，则此方法将返回 `-1`。
...
该函数返回原始字符串中给定字节的第一个实例的索引。如果给定的字节在原始字符串中不可用，则此方法将返回 `-1`。
