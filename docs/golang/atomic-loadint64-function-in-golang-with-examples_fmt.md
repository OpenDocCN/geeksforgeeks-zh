...
> 原文：[https://www.geeksforgeeks.org/atomic-loadint64-function-in-golang-with-examples/](https://www.geeksforgeeks.org/atomic-loadint64-function-in-golang-with-examples/)

在 Go 语言中，`atomic` 包提供低级原子内存操作，有助于实现同步算法。Go 语言中的 `LoadInt64()` 函数用于原子地加载 `addr` 处的值。这个函数是在 `atomic` 包下定义的。在这里，您需要导入 `sync/atomic` 包才能使用这些功能。
...
`*int64` 是指向 `int64` 值的指针。然而，`int64` 包含从 -9223372036854775808 到 922337203685475807 的所有有符号 64 位整数的集合。
