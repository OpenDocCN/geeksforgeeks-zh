...
> 原文: [https://www.geeksforgeeks.org/finding-hyperbolic-sine-of-specified-number-in-golang/](https://www.geeksforgeeks.org/finding-hyperbolic-sine-of-specified-number-in-golang/)

Go 语言提供了对基本常数和数学函数的内置支持，以便在 `math` 包的帮助下对数字进行运算。您可以借助 `math` 包提供的 `Sinh()` 函数找到指定数字的双曲正弦。因此，您需要在导入关键字的帮助下在程序中添加一个 `math` 包来访问 `Sinh()` 函数。
