...
Go 语言提供了对基本常量和数学函数的内置支持，以便在 `math` 包的帮助下对数字执行运算。在 `math` 包提供的 `Expm1()` 函数的帮助下，可以求出指定数减 1 的以 e 为底的指数，即 *e**a-1*。因此，您需要借助 `import` 关键字在程序中添加一个 `math` 包来访问 `Expm1()` 函数。
