...
> 原文：[https://www.geeksforgeeks.org/atomic-loadint32-function-in-golang-with-examples/](https://www.geeksforgeeks.org/atomic-loadint32-function-in-golang-with-examples/)

在 Go 语言中，`原子`包提供低级原子内存，有助于实现同步算法。Go 语言中的 `LoadInt32()` 函数用于自动加载 `addr`。这个函数是在 `原子`包下定义的。在这里，您需要导入 `sync/atomic` 包才能使用这些功能。
...
`(*int32)` 是指向 `int32` 值的指针。但是，`int32` 包含从 -2147483648 到 2147483647 的所有带符号 32 位整数的集合。
