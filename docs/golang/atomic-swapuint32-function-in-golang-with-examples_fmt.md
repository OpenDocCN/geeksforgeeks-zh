# Golang 中 atomic.SwapUint32() 函数功能示例

> 原文：[https://www.geeksforgeeks.org/atomic-swapuint32-function-in-golang-with-examples/](https://www.geeksforgeeks.org/atomic-swapuint32-function-in-golang-with-examples/)

在 Go 语言中，`atomic` 包提供低级原子内存操作，有助于实现同步算法。Go 语言中的 `SwapUint32()` 函数用于将新值自动存储到 `addr` 中，并返回之前的 `addr` 值。这个函数是在 `atomic` 包下定义的。在这里，您需要导入 `sync/atomic` 包才能使用这些功能。
