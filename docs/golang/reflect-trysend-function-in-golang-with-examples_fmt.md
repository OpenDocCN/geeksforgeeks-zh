# 反射：用例子在 Golang 中尝试 `TrySend()` 函数

> 原文：[https://www.geeksforgeeks.org/reflect-trysend-function-in-golang-with-examples/](https://www.geeksforgeeks.org/reflect-trysend-function-in-golang-with-examples/)

Go 语言提供了运行时反射的内置支持实现，并允许程序在 `reflect` 包的帮助下操作任意类型的对象。`reflect.TrySend()` 函数尝试在 `v` 通道上发送 `x`，但不会阻塞。要访问这个函数，需要在程序中导入 `reflect` 包。
