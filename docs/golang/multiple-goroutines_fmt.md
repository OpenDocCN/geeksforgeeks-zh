...
[多戈鲁汀](https://golangbot.com/goroutines/)
...
在上面的例子中，我们有两个主 goroutine 之外的 goroutine，即 `Aname` 和 `Aid`。在这里， `Aname` 打印作者姓名， `Aid` 打印作者 id。

## 工作

在这里，我们有两个 goroutine，即 `Aname` 和 `Aid`，以及一个主 goroutine。当我们首先运行主 goroutine 并打印“`!...Main Go-routine Start...!`”时，这里的主 goroutine 就像一个父级，其他 goroutine 是它的子级，所以首先运行主 goroutine，然后其他 goroutine 启动，如果主 goroutine 终止，那么其他 goroutine 也会终止。所以，在主 goroutine 之后， `Aname` 和 `Aid` goroutine 同时开始工作。`Aname` 从 `150ms` 开始工作，`Aid` 从 `500ms` 开始工作，如下图所示:
