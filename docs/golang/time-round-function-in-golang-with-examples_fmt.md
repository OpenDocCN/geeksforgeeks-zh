...
在 Go 语言中，`time` 包提供了确定和查看时间的功能。Go 语言中的 `Round()` 函数用于查找将所述持续时间 `d` 四舍五入为 `m` 持续时间的最接近倍数的结果。这里，中间值的舍入方式是从 0 开始舍入。而且，这个函数是在 `time` 包中定义的。在这里，你需要导入 `time` 包才能使用这些函数。
...
如果结果超过了在持续时间内可以存储的最大（或最小）值，则返回最大（或最小）持续时间。但如果 `m` 小于或等于 0，则返回未更改的 `d`。
...
这里，首先形成 `d` 数组，然后使用 `range` 迭代 `d` 的所有值，最后使用 `Round()` 方法打印 `d` 的所有四舍五入的值。