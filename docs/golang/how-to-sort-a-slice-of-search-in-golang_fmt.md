...
> 原文：[https://www.geeksforgeeks.org/how-to-sort-a-slice-of-search-in-golang/](https://www.geeksforgeeks.org/how-to-sort-a-slice-of-search-in-golang/)

Go 语言提供了基本常量和运行时反射的内置支持实现来操作排序包。Golang 具有功能相互独立运行的能力。借助该功能，我们可以通过导入`"sort"`包，轻松对整数和字符串进行排序。这些函数是在排序包下定义的，因此您需要在程序中导入排序包。排序函数可以搜索任何重要的 Golang 排序函数列表，如下所示：
...
此函数返回`[0, n]`中`f(i)`为真的最小索引`i`，假设在范围`[0, n]`上，`f(i) == true`意味着`f(i+1) == true`。
