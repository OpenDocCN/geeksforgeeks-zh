# 反射：用例子在Golang中理解Append()函数

> 原文：[https://www.geeksforgeeks.org/reflect-append-function-in-golang-with-examples/](https://www.geeksforgeeks.org/reflect-append-function-in-golang-with-examples/)

Go 语言提供了运行时反射的内置支持实现，并允许程序在 `reflect` 包的帮助下操作任意类型的对象。`reflect.Append()` 函数用于将值 `x` 追加到一个切片 `s` 中。要访问这个函数，需要在程序中导入 `reflect` 包。
