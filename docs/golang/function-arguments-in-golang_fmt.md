...
**例 1：** 在下面的程序中，可以看到 `Z` 的值是不能被 `modify` 功能修改的。
...
**示例 2：** 在下面的程序中，`swap` 函数无法交换值，因为我们正在使用按值调用。
...
在这里，您将使用[指针](https://www.geeksforgeeks.org/pointers-in-golang/)的概念。解引用运算符 `*` 用于访问地址中的值。地址运算符 `&` 用于获取任何数据类型的变量的地址。实际参数和形式参数都引用相同的位置，因此函数内部所做的任何更改实际上都反映在调用方的实际参数中。

**示例 1：** 在函数调用中，我们传递变量的地址，并使用解引用运算符 `*` 来修改值。所以在函数 `modify` 后，你会发现更新后的值。
...
**示例 2：** 通过使用引用调用，`swap` 函数将能够交换如下所示的值。
