...
> 原文: [https://www.geeksforgeeks.org/mutex-in-golang-with-examples/](https://www.geeksforgeeks.org/mutex-in-golang-with-examples/)
...
在调用 `Lock()` 和 `Unlock()` 之间出现的任何代码都将只由一个 Goroutine 执行。
...
**说明:** 在上面的程序中，第 12 行的 `worker()` 函数将 `GFG` 的值增加 1，然后调用 `WaitGroup` 上的 `Done()` 通知其完成。`worker()` 函数被调用 1000 次。当多个 Goroutine 试图同时访问 `GFG` 的值时，这些 Goroutine 中的每一个同时运行，并且当试图增加第 13 行中的 `GFG` 时，比赛状态发生。由于竞争条件的原因，多次运行同一个程序每次会给出不同的输出。
...
**说明:** `Mutex` 是结构类型，变量 `m` 的类型 `Mutex` 是在第 31 行创建的。更改 `worker()` 函数，以便在 `m.Lock()` 和 `m.Unlock()` 之间的第 18 行增加 `GFG` 的代码。现在，在任何时间点，只允许一个 Goroutine 执行这段代码，因此可以处理竞争条件。
