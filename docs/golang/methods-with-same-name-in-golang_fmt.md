...
> 原文：[`https://www.geeksforgeeks.org/methods-with-same-name-in-golang/`](https://www.geeksforgeeks.org/methods-with-same-name-in-golang/)
...
在上面的例子中，我们有两个同名的方法，即`show()`但是有不同类型的接收器。这里第一个`show()`方法包含学生类型的`s`接收器，第二个`show()`方法包含教师类型的`t`接收器。而在`main()`函数中，我们借助两个方法各自的结构变量来调用这两个方法。如果您试图用相同类型的接收器创建这个`show()`方法，那么编译器会抛出一个错误。
