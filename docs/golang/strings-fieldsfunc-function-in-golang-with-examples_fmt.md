# Golang 中的 `strings.FieldsFunc()` 函数示例
...
Golang 中的函数用于在每次运行满足 `f(C)` 的 Unicode 代码点 `c` 时拆分给定的字符串，并返回一个字符串切片数组。
...
如果字符串中的所有代码点都满足 `f(C)` 或字符串为空，则返回空切片。

## 注意事项

此函数不保证其调用 `f(C)` 的顺序。如果 `f` 没有为给定的 `c` 返回一致的结果，`FieldsFunc` 可能会崩溃。
