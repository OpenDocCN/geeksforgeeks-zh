...
> 原文：[https://www.geeksforgeeks.org/how-to-split-a-slice-of-bytes-after-the-given-separator-in-golang/](https://www.geeksforgeeks.org/how-to-split-a-slice-of-bytes-after-the-given-separator-in-golang/)
...
在字节的 Go 切片中，您可以使用 `SplitAfter()` 函数在指定的分隔符后分割切片。该函数在给定分隔符的每个实例之后将一个字节片分割成所有子片，并返回一个包含这些子片的片。它是在 `bytes` 包下定义的，所以你必须在程序中导入 `bytes` 包来访问 `SplitAfter` 函数。
