...
> 原文: [https://www.geeksforgeeks.org/golang-checking-the-string-for-the-specified-regular-expression/](https://www.geeksforgeeks.org/golang-checking-the-string-for-the-specified-regular-expression/)

正则表达式是定义搜索模式的字符序列。Go 语言支持正则表达式。正则表达式用于解析、过滤、验证和从大文本中提取有意义的信息，如日志、其他程序生成的输出等。

在 Go `regexp` 包中，允许您借助 `MatchString()` 函数检查给定的字符串是否包含指定正则表达式模式的任何匹配。这个函数是在 `regexp` 包下定义的，所以要访问这个方法，你需要在程序中导入 `regexp` 包。
...
这里 `pattern` 代表图案，`s` 代表字符串。如果图案匹配，该函数返回 `true`，如果图案不匹配，则返回 `false`。并且如果发现也返回一个错误。
