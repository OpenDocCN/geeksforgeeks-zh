# Golang 中的 `time.Time.MarshalBinary()` 函数
...
在围棋语言中，`time` 包提供了确定和查看时间的功能。 Go 语言中的 `MarshalBinary()` 函数用于实现 `encoding.BinaryMarshaler` 接口。 而且，这个函数是在 `time` 包中定义的。 在这里，您需要导入 `"time"` 包才能使用这些函数。
...
它返回一个表示接收器以二进制形式编码的字节片，还返回一个已发生的错误，但如果没有错误，则返回 `nil`。
