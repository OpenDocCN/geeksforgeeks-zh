...
> 原文: [https://www.geeksforgeeks.org/returning-pointer-from-a-function-in-go/](https://www.geeksforgeeks.org/returning-pointer-from-a-function-in-go/)

## 先决条件

[Go 中的指针](https://www.geeksforgeeks.org/pointers-in-golang/)和[将指针传递给函数](https://www.geeksforgeeks.org/pointers-to-a-function-in-go/)
...
在下面的程序中，代码行 `int* lv = n1 * n1;` 将给出警告，因为它是该功能的本地。为避免警告，将其设为静态。
...
Golang 不像 C/C++那样对指针算法提供任何支持。如果你要执行，那么编译器会抛出一个错误作为无效操作。