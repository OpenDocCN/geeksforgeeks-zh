# Golang 中的 LeadingZeros() 函数示例

> 原文: [https://www.geeksforgeeks.org/bits-leadingzeros-function-in-golang-with-examples/](https://www.geeksforgeeks.org/bits-leadingzeros-function-in-golang-with-examples/)

`bits.LeadingZeros()` 函数用于查找给定数字中前导零位的个数。如果给定的数字等于零，那么这个函数将返回 `UintSize`。要访问这个函数，需要在程序中导入 `math/bits` 包。
