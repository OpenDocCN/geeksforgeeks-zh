...
| 方法 | 描述 |
| --- | --- |
| `New()` | 这种方法用于创建 n 个元素的环。 |
| `Do(func(*Ring))` | 此方法用于以正向顺序调用环的每个元素上的函数 f。 |
| `Len()` | 此方法用于计算 r 环中的元素数量。 |
| `Link(*Ring)` | 这种方法用来连接环 r 和环 s。 |
| `Move(n int)` | 此方法用于在环中向后移动 `n % r.Len()` 元素(`n < 0`) or forward (`n >= 0`)，并返回该环元素。 |
| `Next()` | 此方法用于返回下一个环元素。 |
| `Prev()` | 此方法用于返回前一个环元素。 |
| `Unlink(n int)` | 此方法用于从给定的环中移除 `n % r.Len()` 元素，从 `r.Next()` 开始。 |
