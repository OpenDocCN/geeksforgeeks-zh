...
> 原文: [https://www.geeksforgeeks.org/how-to-convert-a-slice-of-bytes-in-lowercase-in-golang/](https://www.geeksforgeeks.org/how-to-convert-a-slice-of-bytes-in-lowercase-in-golang/)
...
在 Go 切片字节中，您可以使用 `ToLower()` 函数转换小写切片。此函数返回给定字节片的副本（视为 UTF-8 编码字节），其中所有 Unicode 字母都映射为小写。它是在 `bytes` 包下定义的，所以您必须在程序中导入 `bytes` 包来访问 `ToLower` 函数。
