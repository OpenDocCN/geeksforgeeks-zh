...
> 原文：[https://www.geeksforgeeks.org/math-yn-function-in-golang-with-examples/](https://www.geeksforgeeks.org/math-yn-function-in-golang-with-examples/)
...
*   如果 `a < 0` 的值像 `Yn(a < 0, 0)`，那么这个函数将返回 `+Inf`（如果 `a` 是奇数）或 `-Inf`（如果 `a` 是偶数）。
