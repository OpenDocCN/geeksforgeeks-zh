...
可以借助 `IsUpper()` 函数检查给定符文是否为大写字母。如果给定的符文是大写字母，此函数返回 `true`；如果给定的符文不是大写字母，则返回 `false`。该函数是在 `Unicode` 包下定义的，因此要访问该方法，您需要在程序中导入 `unicode` 包。
...
这个函数的返回类型是 `bool` 型的。让我们借助给定的例子来讨论这个概念:
