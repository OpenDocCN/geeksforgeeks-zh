# Golang 中的 io.SectionReader.Size() 函数及示例
...
用 Go 语言，`io` 包为 I/O 原语提供基本接口。它的主要工作是封装这类原语之正在进行的实现。Go 语言中的 `SectionReader.Size()` 函数用于查找 `NewSectionReader()` 方法返回的节的大小，以字节为单位。此外，该函数是在 `io` 包下定义的。在这里，您需要导入 `io` 包才能使用这些函数。
