...
> 原文: [https://www.geeksforgeeks.org/how-to-get-int63n-type-random-number-in-golang/](https://www.geeksforgeeks.org/how-to-get-int63n-type-random-number-in-golang/)
...
在 `math/rand` 包提供的 `Int63n()` 函数的帮助下，您可以从缺省源生成一个 63 位整数 `n` 的非负伪随机数 in [0，n]作为 `int64` 类型。因此，您需要借助 `import` 关键字在程序中添加一个 `math/rand` 包来访问 `Int63n()` 函数。如果 `n` 的值为 `<= 0`，这个方法会出现恐慌。
