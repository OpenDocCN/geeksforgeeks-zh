...
> 原文: [https://www.geeksforgeeks.org/go-pointer-to-pointer-double-pointer/](https://www.geeksforgeeks.org/go-pointer-to-pointer-double-pointer/)

## 先决条件: [Go指针](https://www.geeksforgeeks.org/pointers-in-golang/)
...
声明指针指向指针类似于在 Go 中声明[指针](https://www.geeksforgeeks.org/pointers-in-golang/)。不同的是，我们必须在指针名称的名称前附加一个 `*`。这通常是在我们使用 [`var` 关键字](https://www.geeksforgeeks.org/var-keyword-in-go/)和类型一起声明指针变量时完成的。下面的例子和图片将更好地解释这个概念。

### 例 1

在下面的程序中，指针 `pt2` 存储 `pt1` 指针的地址。解引用 `pt2` 即 `*pt2` 会给出变量 `v` 的地址，也可以说指针 `pt1` 的值。如果您尝试 `**pt2`，那么这将给出变量 `v` 的值，即 100。
...
我们对上面的程序做一些修改。通过使用如下所示的解引用来更改指针的值，从而为指针分配一些新值:
