...
在 Go 字符串中，也可以使用 `IndexRune()` 函数找到给定字符串中指定符文的第一个索引。此函数返回 Unicode 代码点的第一个实例的索引，即指定的符文，如果指定的符文不在给定的字符串中，则返回-1。如果符文是 `utf8.RuneError`，然后它返回任何无效的 UTF 8 字节序列的第一个实例。它是在字符串包下定义的，所以你必须在你的程序中导入字符串包来访问索引函数。
