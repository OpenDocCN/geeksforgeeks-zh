...
> 原文：[https://www.geeksforgeeks.org/how-to-install-golang-debugger/](https://www.geeksforgeeks.org/how-to-install-golang-debugger/)

安装 golang 调试器是检测或分析并消除任何软件类型代码中现有和潜在错误(bug)的有力步骤，这些错误甚至会导致其异常行为或导致崩溃。

为了防止我们的软件/系统出现这些不正确的操作和异常工作，我们使用调试工具来发现和解决这些类型的错误。因此，Go-develop 是一个调试工具(称为调试器)，用于在开发的各个阶段查找编码错误。我们推荐您使用深究 golang 调试器，因为它是最好的、功能强大的调试器工具，并且使用简单。Delve 是第三方调试器，用于 Go 编程语言，可在 [GitHub](https://github.com/go-delve/delve) 上获得。
...
Go 吕德 e 只需在工作区内使用 `go get` 命令即可轻松下载并安装，但如果您使用的是 `go mod`，则可能需要在项目目录外执行此命令(如下图所示)，以避免 Delve 被添加到现在已经执行的 `go mod` 文件中。
...
调试完 `dlv` 命令后，您可以从 `help` 命令选项中获取帮助。如果您将键入命令列表的帮助(用于进一步调试)。
...
如果你想获得帮助命令选项，那么你可以使用 `dlv exit` 或 `clear` 命令，该命令将带你回到你之前所在的地方。

由于您有一个正在运行的 go 安装，因此应该已经设置了以下内容：

始终确保全局环境变量设置正确，因为这将指示命令 `dlv` 的存储目录。你也可以通过输入 `env` 来检查。

并且还要确保包含 `GOBIN` 的路径使得 run go 二进制可执行文件没有绝对路径规范。
...
该命令将允许您在 OSX 内部安装深究 golang 调试器。
