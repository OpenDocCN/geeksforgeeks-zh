...
> 原文：[https://www.geeksforgeeks.org/how-to-install-golang-on-macos/](https://www.geeksforgeeks.org/how-to-install-golang-on-macos/)
...
这里，我们添加 `export GOPATH=/user/anki/Documents/go` 到 `.bash_profile`。那个 `.bash_profile` 是一个在您登录 Mac 帐户时自动加载的文件，它包含您的命令行界面（CLI）的所有启动配置和首选项。
...
**第一步:** 根据自己的选择下载安装一个文本编辑器。安装后，在“文档”中（或系统中的任何位置）创建一个名为 `go` 的文件夹，并转到（或任何您想要的名称）。在这个文件夹中，创建另一个名为 `src` 的文件夹，并在这个源文件夹中创建另一个名为 `welcome` 的文件夹。在这个文件夹中，您所有的 go 程序都将被存储。
