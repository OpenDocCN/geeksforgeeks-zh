...
[JSON](https://www.geeksforgeeks.org/javascript-json/) 是一种广泛使用的数据交换格式。Golang 使用 `encoding/json` 包提供了多种编码和解码 API 来处理 JSON，包括往返于内置和自定义数据类型。
...
*   JSON 空值对应 `nil`
*   JSON 数组对应 `slice`
*   JSON 对象对应 `map` 或 `struct`

## 1. 编码/封送结构

包 `encoding/json` 中的 `Marshal()` 函数用于将数据编码成 JSON。
...
包 `encoding/json` 中的 `Unmarshal()` 函数用于将 JSON 中的数据解组或解码为结构。
