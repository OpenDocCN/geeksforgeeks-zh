...
> 原文：[https://www.geeksforgeeks.org/atomic-compareandswapuintptr-function-in-golang-with-examples/](https://www.geeksforgeeks.org/atomic-compareandswapuintptr-function-in-golang-with-examples/)

在 Go 语言中，`atomic`包提供低级原子内存，有助于实现同步算法。Go 语言中的`CompareAndSwapUintptr()`功能用于执行`uintptr`值的比较和交换操作。这个函数是在`atomic`包下定义的。在这里，您需要导入`sync/atomic`包才能使用这些功能。
...
`(*uintptr)`是指向`uintptr`值的指针。而`uintptr`是一个太大的无符号整数类型，包含任何指针的位模式。
