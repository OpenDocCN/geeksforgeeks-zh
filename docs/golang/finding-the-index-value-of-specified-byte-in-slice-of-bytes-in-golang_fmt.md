...
> 原文：`https://www.geeksforgeeks.org/finding-the-index-value-of-specified-byte-in-slice-of-bytes-in-golang/`
...
在字节的 Go 切片中，可以使用 `IndexByte()` 函数找到给定切片中指定字节的第一个索引值。此函数返回给定字节片中指定字节的第一个实例的索引。如果给定字节在原始切片中不可用，则该方法将返回 `-1`。它是在 `bytes` 包下定义的，所以您必须在程序中导入 `bytes` 包来访问 `IndexByte` 函数。
