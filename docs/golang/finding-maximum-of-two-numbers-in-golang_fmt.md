...
> 原文：[https://www.geeksforgeeks.org/finding-maximum-of-two-numbers-in-golang/](https://www.geeksforgeeks.org/finding-maximum-of-two-numbers-in-golang/)

Go 语言提供了对基本常数和数学函数的内置支持，以便在 `math` 包的帮助下对数字进行运算。借助 `math` 包提供的 `Max()` 函数，可以在给定的两个数字中找到最大的数字。所以，你需要在程序中添加一个 `math` 包，借助 `import` 关键字来访问 `Max()` 函数。
