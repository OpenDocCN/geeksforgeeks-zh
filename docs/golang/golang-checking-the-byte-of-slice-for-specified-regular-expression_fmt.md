...
> 原文: [https://www.geeksforgeeks.org/golang-checking-the-byte-of-slice-for-specified-regular-expression/](https://www.geeksforgeeks.org/golang-checking-the-byte-of-slice-for-specified-regular-expression/)
...
这里，`p` 代表模式，`s` 代表字节片。如果模式匹配，此函数返回 `true`；如果模式不匹配，则返回 `false`。并且如果发现也返回一个 `error`。
