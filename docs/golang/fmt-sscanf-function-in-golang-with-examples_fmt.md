...
在 Go 语言中，`fmt` 包实现了格式化的输入输出，其功能类似于 C 的 `printf()` 和 `scanf()` 函数。`fmt.Sscanf()` 函数扫描指定的字符串，并将连续的空格分隔值存储到由格式确定的连续参数中。此外，该功能在 `fmt` 包中定义。在这里，您需要导入 `"fmt"` 包才能使用这些功能。
