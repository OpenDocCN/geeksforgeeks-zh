...
Golang 提供了多个 API 来使用 [JSON](https://www.geeksforgeeks.org/json-full-form/) 工作，包括使用`encoding/json`包往返于内置和自定义数据类型。为了解析 json，我们使用包编码/json 中的`Unmarshal()`函数将 JSON 中的数据解包或解码为[结构](https://www.geeksforgeeks.org/structures-in-golang/)。
...
**注意:**如果 `v` 为零或者不是指针，`Unmarshal`将返回`InvalidUnmarshalError`。
