...
> 原文：[`https://www.geeksforgeeks.org/how-to-sort-a-slice-stable-in-golang/`](https://www.geeksforgeeks.org/how-to-sort-a-slice-stable-in-golang/)
...
在 Go 语言中，允许使用 `SliceStable()` 函数对切片进行稳定排序。给定指定的 `less` 函数，该函数对指定的切片进行排序，同时保持相等元素的原始顺序。如果指定的接口不是一个切片，这个函数就会死机。它是在 `sort` 包下定义的，所以您必须在程序中导入 `sort` 包来访问 `SliceStable` 函数。
