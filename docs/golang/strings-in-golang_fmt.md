...
| escape character | description / description / category |
| :--- | :--- |
| **\** | backslash. ) |
...
这里，`index`是存储 UTF-8 编码码点的第一个字节的变量，`chr`存储给定字符串的字符，而`str`是一个字符串。
...
在 Go 语言字符串中，你可以使用两个函数来获取字符串的长度：一个是 `len()`，另一个是 `RuneCountInString()`。`RuneCountInString()` 函数由 `utf8` 包提供，该函数返回字符串中存在的 rune（Unicode 码点）的总数。而 `len()` 函数返回的是字符串的字节数。
