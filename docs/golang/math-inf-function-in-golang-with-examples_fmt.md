...
Go 语言提供了对基本常量和数学函数的内置支持，以便在 `math` 包的帮助下对数字执行运算。您可以找到 `math` 包提供的正无穷大（如果 `sign >= 0`）或负无穷大（如果 `sign < 0`）。因此，您需要在 `import` 关键字的帮助下在程序中添加一个 `math` 包来访问 `Inf()` 函数。
