...
> 原文: [https://www.geeksforgeeks.org/how-to-count-specific-characters-present-in-the-slice-in-golang/](https://www.geeksforgeeks.org/how-to-count-specific-characters-present-in-the-slice-in-golang/)
...
在 Go 切片字节中，可以借助`计数`功能对其中存在的元素进行计数。此函数返回给定切片中可用元素的总数或某些指定元素的总数。这个函数是在`字节`包下定义的，所以你必须在程序中导入`字节`包来访问`计数`函数。
