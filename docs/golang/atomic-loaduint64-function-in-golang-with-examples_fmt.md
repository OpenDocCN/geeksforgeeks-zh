...
> 原文：[https://www.geeksforgeeks.org/atomic-loaduint64-function-in-golang-with-examples/](https://www.geeksforgeeks.org/atomic-loaduint64-function-in-golang-with-examples/)

在 Go 语言中，`atomic` 包提供较低级别的原子内存，这有助于实现同步算法。Go 语言中的 `LoadUint64()` 函数用于自动加载 `addr`。这个函数是在 `atomic` 包下定义的。在这里，您需要导入 `sync/atomic` 包才能使用这些功能。
...
`*uint64` 是指向 `uint64` 值的指针。但是，`uint64` 包含从 0 到 18446744073709551615 的所有无符号 64 位整数的集合。
