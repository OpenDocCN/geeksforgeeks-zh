...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-struct-instance-using-a-struct-literal-in-golang/](https://www.geeksforgeeks.org/how-to-create-a-struct-instance-using-a-struct-literal-in-golang/)
...
1.  键必须是结构类型中声明的字段名。
2.  不带任何键的元素列表必须按照字段声明的顺序，为每个结构字段列出一个元素。
3.  包含键的元素列表不需要为每个结构字段都有一个元素。省略的字段将获得该字段的零值。
4.  文字可以省略元素列表；这种文字的计算结果是其类型的零值。
5.  为属于不同包的结构的非导出字段指定元素是错误的。
