...
> 原文：[https://www.geeksforgeeks.org/atomic-compareandswapuint32-function-in-golang-with-examples/](https://www.geeksforgeeks.org/atomic-compareandswapuint32-function-in-golang-with-examples/)

在 Go 语言中，`atomic`包提供低级原子内存，有助于实现同步算法。Go 语言中的`CompareAndSwapUint32()`功能用于执行`uint32`值的比较和交换操作。这个函数是在原子包下定义的。在这里，您需要导入`"sync/atomic"`包才能使用这些功能。
...
`(*uint32)`是指向`uint32`值的指针。`uint32`是位大小为 32 的整数类型。此外，`int32`包含范围从 0 到 4294967295 的所有无符号 32 位整数的集合。
