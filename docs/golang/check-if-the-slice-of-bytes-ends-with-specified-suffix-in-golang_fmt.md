# 检查字节片是否以 Golang 中指定的后缀结束

> 原文: [https://www.geeksforgeeks.org/check-if-the-slice-of-bytes-ends-with-specified-suffix-in-golang/](https://www.geeksforgeeks.org/check-if-the-slice-of-bytes-ends-with-specified-suffix-in-golang/)
...
在字节的 Go 切片中，可以借助 `HasSuffix()` 函数检查给定切片是否以指定后缀结束。如果给定的切片以指定的后缀结束，则该函数返回 `true`；如果给定的切片不以指定的后缀结束，则该函数返回 `false`。它是在 `bytes` 包下定义的，所以你必须在你的程序中导入 `bytes` 包来访问 `HasSuffix` 函数。
