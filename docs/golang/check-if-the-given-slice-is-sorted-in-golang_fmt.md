...
> 原文: [https://www.geeksforgeeks.org/check-if-the-given-slice-is-sorted-in-golang/](https://www.geeksforgeeks.org/check-if-the-given-slice-is-sorted-in-golang/)
...
在 Go 语言中，可以借助`SliceIsSorted()`函数检查给定切片是否排序。如果给定切片已排序，此函数返回 `true`。如果给定的切片没有排序，则返回 `false`。如果指定的接口不是切片类型，这个函数会死机。它是在 `sort` 包下定义的，所以您必须在程序中导入 `sort` 包来访问 `SliceIsSorted` 函数。
