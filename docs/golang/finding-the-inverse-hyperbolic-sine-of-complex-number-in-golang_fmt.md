...
> 原文: [https://www.geeksforgeeks.org/finding-the-inverse-hyperbolic-sine-of-complex-number-in-golang/](https://www.geeksforgeeks.org/finding-the-inverse-hyperbolic-sine-of-complex-number-in-golang/)

Go 语言借助 `cmplx` 包，为复数的基本常数和数学函数提供内置支持。允许你借助 `math/cmplx` 包提供的 `Asinh()` 函数求指定复数的反双曲正弦。所以，你需要在你的程序中添加一个 `math/cmplx` 包在 `import` 关键字的帮助下访问 `Asinh()` 函数。
