...
> 原文: [https://www.geeksforgeeks.org/how-to-declare-and-access-pointer-variable-in-golang/](https://www.geeksforgeeks.org/how-to-declare-and-access-pointer-variable-in-golang/)
...
为此，您需要使用地址操作符用另一个变量的内存地址初始化指针，如下例所示:
...
**说明:** 这里(`&`)符号是用来获取存储值的变量的地址为“Geeks For Geeks”。该变量拥有的地址与指针中存储的地址相同。因此`&dummyVar`和`pointerVariable`的输出是一样的。要访问存储在地址中的值，该值存储在指针中，我们使用(`*`)符号。