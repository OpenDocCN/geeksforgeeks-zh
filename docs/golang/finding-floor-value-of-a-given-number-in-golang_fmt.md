...
> 原文: [https://www.geeksforgeeks.org/finding-floor-value-of-a-given-number-in-golang/](https://www.geeksforgeeks.org/finding-floor-value-of-a-given-number-in-golang/)

Go 语言提供了对基本常数和数学函数的内置支持，以便在数学包的帮助下对数字进行运算。借助`math`包提供的`Floor()`函数，可以找到小于等于指定数的最大整数值。因此，您需要在导入关键字的帮助下在程序中添加一个数学包来访问`Floor()`函数。
