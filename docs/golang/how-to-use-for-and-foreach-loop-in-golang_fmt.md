...
> 原文: [https://www.geeksforgeeks.org/how-to-use-for-and-foreach-loop-in-golang/](https://www.geeksforgeeks.org/how-to-use-for-and-foreach-loop-in-golang/)

Golang 中只有一个循环构造，那就是 `for` 循环。Golang 中的 `for` 循环有三个组件，必须用分号（`;`）分隔，这些是：

*   **初始化语句**：在第一次迭代之前执行。例如 `i := 0`
*   **条件表达式**：就在每次迭代之前执行。例如：`i < 5`
*   **Post 语句**：在每次迭代结束时执行。例如 `i++`

不需要任何括号来包含这三个组件，但是要定义一个块，我们必须使用花括号 `{ }`。
...
您可以使用 `for` 作为 Golang 中的 `while` loop。去掉所有的分号。
...
## 在 Golang 没有 foreach 循环

在 Golang 没有 `foreach` 循环，取而代之的是 `for` 循环可以与 `range` 结合使用，作为 `foreach`。有一个关键字 `range`，可以将 `for` 和 `range` 组合在一起，在循环内可以选择使用键或值。
