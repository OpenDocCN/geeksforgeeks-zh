...
> 原文：[https://www.geeksforgeeks.org/go-decision-making-if-if-else-nested-if-if-else-if/](https://www.geeksforgeeks.org/go-decision-making-if-if-else-nested-if-if-else-if/)
...
*   `if`语句可以有零个或一个`else`，它必须在任何其他`if`之后。
*   `if`语句可以有零到多个其他`if`，并且它必须在其他`if`之前。
*   如果成功，其余的`if`或`else if`都不会被测试。
