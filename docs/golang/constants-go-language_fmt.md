...
数值常量为 `高精度值` 。As Go 是一种静态类型的语言，不允许混合数字类型的操作。您不能将 `float64` 添加到 `int` 中，甚至不能将 `int32` 添加到 `int` 中。虽然，写`1e6 * time.Second` 是合法的。第二 或 `math.Exp(1)` 甚至`1<<('\t'+2.0)`。在围棋中，常数与变量不同，表现得像正则数。
...
复常数的行为很像浮点常数。它是整数常量(或参数)的*有序对*或*实数对*，常量之间用逗号隔开，该对用括号括起来。第一个常数是实部，第二个是虚部。复数常量 `COMPLEX8` 使用 `8字节`的存储空间。
...
*   可以用十进制或指数形式表示浮动常数。
*   当用十进制形式表示时，必须包括小数点、指数或两者。
*   当使用指数形式表示时，必须包括整数部分、小数部分或两者。
...
布尔常量类似于字符串常量。它应用与字符串常量相同的规则。区别只是它有两个非类型化的常量 `真` 和 `假`。
