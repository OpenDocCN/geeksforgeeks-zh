# `bits.OnesCount64()`函数在Golang中的示例

> 原文：[https://www.geeksforgeeks.org/bits-onescount64-function-in-golang-with-examples/](https://www.geeksforgeeks.org/bits-onescount64-function-in-golang-with-examples/)

Go 语言在 `bits` 包的帮助下，为位操作提供内置支持，以实现预声明的无符号整数类型的位计数和操作功能。这个包提供了 `OnesCount64()` 函数，用于查找 `a` 中的置位数。要访问 `OnesCount64()` 函数，需要在导入关键字的帮助下，在程序中添加一个 `math/bits` 包。
...
## 参数：
该函数取 `uint64` 类型的一个参数，即 `a`。

## 返回值：
该函数返回用于表示一个数的二进制形式中1的个数。
