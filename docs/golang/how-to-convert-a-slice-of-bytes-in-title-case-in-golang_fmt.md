# 如何在 Golang 中转换一个字节切片为标题大小写？

> 原文: [https://www.geeksforgeeks.org/how-to-convert-a-slice-of-bytes-in-title-case-in-golang/](https://www.geeksforgeeks.org/how-to-convert-a-slice-of-bytes-in-title-case-in-golang/)
...
在 Go 分片字节中，允许使用`ToTitle()`函数转换标题情况下的分片。此函数返回给定字节片的副本（视为 UTF-8 编码字节），其中所有 Unicode 字母都映射到它们的标题大小写中。它是在`bytes`包下定义的，所以你必须在你的程序中导入`bytes`包来访问`ToTitle`函数。
