...
> 原文：`https://www.geeksforgeeks.org/golang-extracting-all-the-regular-expression-from-the-string/`

正则表达式是定义搜索模式的字符序列。Go 语言支持正则表达式。正则表达式用于解析、过滤、验证和从大文本中提取有意义的信息，如日志、其他程序生成的输出等。

在 Go `regexp` 中，可以借助 `FindAllString()` 方法找到给定字符串中的所有正则表达式。此方法返回指定正则表达式的所有连续匹配的片段，如包注释中的“全部”描述所定义。或者，如果没有找到匹配项，这个方法将返回零，这里 `count` 表示返回片段的子串数。这个方法是在 `regexp` 包下定义的，所以要访问这个方法，需要在程序中导入 `regexp` 包。
