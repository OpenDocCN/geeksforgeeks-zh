...
在 Go 语言中，恐慌就像一个例外，它也在运行时出现。或者换句话说，恐慌意味着在你的 Go 程序中出现了一个意外情况，由于这个情况，你的程序的执行被终止了。有时，当出现某些特定情况时，如越界数组访问等，运行时会出现死机。如示例 1 所示，或者有时是程序员故意抛出的，以借助示例 2 所示的`panic()`函数来处理 Go 程序中的最坏情况。

`panic()`函数是内置函数，在 Go 语言的内置包中定义。该函数终止控制流并开始恐慌。
