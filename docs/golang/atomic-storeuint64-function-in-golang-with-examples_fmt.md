...
> 原文：[https://www.geeksforgeeks.org/atomic-storeuint64-function-in-golang-with-examples/](https://www.geeksforgeeks.org/atomic-storeuint64-function-in-golang-with-examples/)

在 Go 语言中，`原子`包提供低级原子内存，有助于实现同步算法。Go 语言中的 `StoreUint64()` 函数用于将 `val` 自动存储到 `addr` 中。这个函数是在 `原子` 包下定义的。在这里，您需要导入 `sync/atomic` 包才能使用这些功能。
...
`*uint64` 是指向 `uint64` 值的指针。`uint64` 是位大小为 64 的整数类型。但是，`int64` 包含从 0 到 18446744073709551615 的所有无符号 64 位整数的集合。

## 返回值：

将 `val` 存入 `addr`，需要时可以返回。
