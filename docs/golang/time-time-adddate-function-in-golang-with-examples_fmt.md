...
在 Go 语言中，`time` 包提供了确定和查看时间的功能。`Time.AddDate()` 函数用于计算时间，这相当于将指定的年数、月数和天数与给定的 `t` 相加。
例如，如果该方法的参数类似于 `(-2, 4, 5)`，并且声明的 `t` 为 2018 年 2 月 3 日，则输出将为 2016 年 6 月 8 日。与这里的日期方法一样，如果月、日或年的范围超出正常范围，则它会自动转换为归一化范围。而且，这个函数是在 `time` 包中定义的。在这里，您需要导入 `time` 包才能使用这些功能。
