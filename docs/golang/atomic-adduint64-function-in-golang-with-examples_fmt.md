...
> 原文：[https://www.geeksforgeeks.org/atomic-adduint64-function-in-golang-with-examples/](https://www.geeksforgeeks.org/atomic-adduint64-function-in-golang-with-examples/)

在 Go 语言中，`原子`包提供较低级别的原子内存，这有助于实现同步算法。Go 语言中的 `AddUint64()` 函数用于将 `delta` 自动添加到 `addr` 中。这个函数是在 `原子`包下定义的。在这里，您需要导入 `sync/atomic` 包才能使用这些功能。
...
这里，`addr` 表示地址，`delta` 表示大于零的少量位。此外，如果你想从 `x` 中减去一个有符号的正常数值 `c`，那么你可以通过 `AddUint64(&x, ^uint64(c-1))` 来实现。如果你特别想减少 `x`，那么可以通过 `AddUint64(&x, ^uint64(0))` 来完成。

**注意：** `*uint64` 是指向 `uint64` 值的指针。此外，`uint64` 包含从 0 到 18446744073709551615 的所有无符号 64 位整数的集合。

**返回值：** 它自动添加 `地址` 和 `增量`，并返回一个新值。
