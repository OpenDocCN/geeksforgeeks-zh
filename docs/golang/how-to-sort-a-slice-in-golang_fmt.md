...
> 原文：[`https://www.geeksforgeeks.org/how-to-sort-a-slice-in-golang/`](https://www.geeksforgeeks.org/how-to-sort-a-slice-in-golang/)
...
在 Go 语言中，可以借助 `Slice()` 函数对切片进行排序。给定提供的较少函数，此函数对指定的切片进行排序。这个函数的结果不稳定。所以对于稳定排序，可以使用 `SliceStable`。如果指定的接口不是片类型，这个函数就会死机。它是在 `sort` 包下定义的，所以您必须在程序中导入 `sort` 包来访问切片函数。
