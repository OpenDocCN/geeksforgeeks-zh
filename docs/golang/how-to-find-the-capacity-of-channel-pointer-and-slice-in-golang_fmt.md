...
> 原文：[https://www.geeksforgeeks.org/how-to-find-the-capacity-of-channel-pointer-and-slice-in-golang/](https://www.geeksforgeeks.org/how-to-find-the-capacity-of-channel-pointer-and-slice-in-golang/)

Go 语言中，容量定义了一个特定元素可以容纳的最大数量。这里的任务是在 Golang 中找到[通道](https://www.geeksforgeeks.org/channel-in-golang/)、[指针](https://www.geeksforgeeks.org/pointers-in-golang/)、[切片](https://www.geeksforgeeks.org/slices-in-golang/)的容量，我们可以使用 `cap()` 函数。
...
本例使用 `cap()` 函数求 Golang 中 Pointer 的容量。
...
本例使用 `cap()` 函数求 Golang 中 Channel 的容量。
...
在本例中，使用 `cap()` 函数来查找 Golang 中 Slice 的容量。
