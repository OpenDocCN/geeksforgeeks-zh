...
> 原文：`https://www.geeksforgeeks.org/how-to-add-a-method-to-struct-type-in-golang/`
...
在上面的代码中，`Rect` 的实例作为值传递给 `r`，以调用方法 `Area()`。也可以参考 `(&r)`。无论您用来调用方法的实例是指针还是值，调用方法都没有区别，Go 会自动为您进行转换。
