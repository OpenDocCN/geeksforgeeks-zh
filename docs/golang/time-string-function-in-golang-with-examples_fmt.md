...
在 Go 语言中，`time` 包提供了确定和查看时间的功能。`time.String()` 函数用于将 `Duration` 格式表示为“24h6m0.7s”的字符串。在这里，最前面的零单位被删除。并且规定的少于一秒格式的持续时间将使用较小的单位，例如毫秒、微秒或纳秒，以确保最前面的数字不是零。持续时间 0 的格式为 0。这个函数是在 `time` 包中定义的，你需要导入 `time` 包才能使用这些函数。
