...
遍历字符串时，通过检查数组 `pos` 来检查字符串中是否已经存在当前字符。如果不存在，则将当前字符的索引存储在数组中，并将值作为当前索引。如果它已经存在于数组中，这意味着当前字符可以在当前子字符串中重复。为此，请检查字符的前一次出现是在当前子字符串的起始点 `st` 之前还是之后。如果是在 `st` 之前，那么只更新数组中的值。如果在 `st` 之后，那么找到当前子串 `currlen` 的长度为 `i - st` ，其中 `i` 为当前索引。

比较 `currlen` 和 `maxlen` 。如果 `maxlen` 小于 `currlen`，那么将 `maxlen` 更新为 `currlen` 并 `start` 为 `st`，字符串遍历完成后，需要的最长不重复字符的子串是从 `s[start]` 到 `s[start + maxlen - 1]`。
...
### 注意:
不使用数组，我们也可以使用 `string` 到 `int` 的 `map`。