...
> 原文：[https://www.geeksforgeeks.org/how-to-find-the-length-of-channel-pointer-slice-string-and-map-in-golang/](https://www.geeksforgeeks.org/how-to-find-the-length-of-channel-pointer-slice-string-and-map-in-golang/)
...
## 通道
在 Go 语言中，通道是一个 goro tine 与另一个 goro tine 通信的媒介，这种通信是无锁的。
...
## 指针
Go 编程语言或 Golang 中的指针是一个变量，用于存储另一个变量的内存地址。
...
## 切片
切片是存储相似类型元素的变长序列，不允许在同一个切片中存储不同类型的元素。
...
## 字符串
它是一个可变宽度字符序列，其中每个字符都由一个或多个字节使用 UTF-8 编码来表示。
