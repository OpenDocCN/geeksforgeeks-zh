# 如何在 Golang 中找到字节片内任意元素的最后一个索引值？

> 原文：[https://www.geeksforgeeks.org/how-to-find-the-last-index-value-of-any-element-in-slice-of-bytes-in-golang/](https://www.geeksforgeeks.org/how-to-find-the-last-index-value-of-any-element-in-slice-of-bytes-in-golang/)
...
在字节的 Go 切片中，可以使用 `LastIndexAny()` 函数找到给定切片中任何指定实例的最后一个索引值。此函数返回字符中任何 Unicode 代码点的原始片段中最后一次出现的字节索引。如果字符的 Unicode 码点在原始切片中不可用或为空，则该方法将返回 `-1`。它是在 `bytes` 包下定义的，所以你必须在程序中导入 `bytes` 包来访问 `LastIndexAny` 函数。
