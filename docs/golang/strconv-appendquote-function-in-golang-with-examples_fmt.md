# Golang 中的 strconv.AppendQuote() 函数及示例
...
Go 语言提供内置支持，通过`strconv`包实现与基本数据类型字符串表示的转换。该包提供了一个`AppendQuote()`函数将表示`str`的双引号 Go 字符串文字（由`Quote`生成）附加到`num`，并返回扩展缓冲区，如下语法所示。要访问`AppendQuote()`函数，您需要在程序中导入`strconv`包。
