...
> 原文: [https://www.geeksforgeeks.org/how-to-get-current-date-and-time-in-various-format-in-golang/](https://www.geeksforgeeks.org/how-to-get-current-date-and-time-in-various-format-in-golang/)

Golang 中的包 `time` 提供了测量和显示时间的功能。日历计算总是采用公历，没有闰秒。使用 `time.Now()` 函数你可以得到 `yyyy-mm-dd hh:mm:ss .毫秒时区` 格式中的日期和时间。这是在 golang 获取日期和时间的最简单方法。
...
使用您的时间变量或标识符上的 `Format()` 函数，您可以获得各种格式的输出，如下例所示。您需要在 `Format()` 函数中指定所需输出的格式。这是 Golang 编程中最灵活的日期和时间格式化方式之一。
