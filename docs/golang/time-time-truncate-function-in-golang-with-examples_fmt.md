...
在围棋语言中，`Time` 包提供了确定和查看时间的功能。 GO 语言中的 `Time.Truncate()` 函数用于查找将指定时间 `t` 四舍五入为给定持续时间 `d` 从零开始的最接近倍数的输出。 而且，这个函数是在 `time` 包中定义的。 在这里，您需要导入 `time` 包才能使用这些函数。
...
`truncate()` 方法在时间上以从零开始的绝对持续时间的形式工作。 然而，它在当时的布局形式上不起作用。
