...
> 原文: [https://www.geeksforgeeks.org/different-ways-to-find-the-type-of-an-object-in-golang/](https://www.geeksforgeeks.org/different-ways-to-find-the-type-of-an-object-in-golang/)
...
**注意:** 一个空的[接口](https://www.geeksforgeeks.org/interfaces-in-golang/)由 `interface{}` 表示，可以用来保存任何类型的值。

### 2. 使用反射包

您也可以使用[反射包](https://golang.org/pkg/reflect/)，它实现运行时反射，从而允许程序操作任意类型的对象。`reflect.TypeOf(x)` 和 `reflect.ValueOf(x).Kind()` 函数帮助我们找到需要的变量类型。
...
您可以使用一个[类型开关](https://www.geeksforgeeks.org/switch-statement-in-go/)来做几个类型断言。类型开关串联使用几个类型断言，并运行第一个匹配的类型。在此开关中，`case` 包含要与开关表达式中的类型进行比较的类型，如果没有匹配的 `case`，则计算 `default`。
