...
> 原文: [https://www.geeksforgeeks.org/reflect-indirect-function-in-golang-with-examples/](https://www.geeksforgeeks.org/reflect-indirect-function-in-golang-with-examples/)

Go 语言提供了运行时反射的内置支持实现，并允许程序在反射包的帮助下操作任意类型的对象。`reflect.Indirect()`函数用于获取`v`指向的值，即如果`v`是一个零指针，`Indirect`返回一个零值。如果`v`不是指针，`Indirect`返回`v`。要访问这个函数，需要在程序中导入`reflect`包。
