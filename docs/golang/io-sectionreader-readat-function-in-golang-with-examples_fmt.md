...
用围棋语言，`io`包为 I/O 原语提供基本接口。它的主要工作是封装这类原语之王正在进行的实现。Go 语言中的`SectionReader.ReadAt()`函数用于返回`NewSectionReader`读取的字节数方法，该方法以缓冲区和偏移量为参数，并且该函数在`io`包下定义，这里需要导入`"io"`包才能使用这些函数。
