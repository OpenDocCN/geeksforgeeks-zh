...
**解释:**
变量`GFG`没有作为参数传递给匿名函数，但是函数可以访问它。在这个例子中，有一个小问题，因为任何其他将在 main 中定义的函数都可以访问全局变量`GFG`，并且它可以在不调用`counter`函数的情况下更改它。因此关闭还提供了另一个方面，即**数据隔离**。
...
**解释:**
闭包引用了变量`GFG`，即使在`newCounter()`函数已经完成运行，但是`newCounter()`函数之外没有其他代码可以访问这个变量。这就是如何在函数调用之间保持数据的持久性，同时将数据与其他代码隔离开来。