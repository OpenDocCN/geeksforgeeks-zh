...
> 原文: [https://www.geeksforgeeks.org/check-if-the-string-ends-with-specified-suffix-in-golang/](https://www.geeksforgeeks.org/check-if-the-string-ends-with-specified-suffix-in-golang/)
...
在 Go strings 中，可以借助 `HasSuffix()` 函数检查字符串是否以指定后缀结尾。如果给定的字符串以指定的后缀结束，该函数返回 `true`，如果给定的字符串不以指定的后缀结束，该函数返回 `false`。它是在 `strings` 包下定义的，所以您必须在程序中导入 `strings` 包才能访问 `HasSuffix` 函数。
