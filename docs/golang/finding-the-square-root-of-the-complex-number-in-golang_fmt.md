...
> 原文：`https://www.geeksforgeeks.org/finding-the-square-root-of-the-complex-number-in-golang/`

Go 语言借助 `cmplx` 包，为复数的基本常数和数学函数提供内置支持。允许你借助 `math/cmplx` 包提供的 `Sqrt()` 函数求指定复数的平方根。在这个函数中，选择 `q`，使得实数 `q` >= 0，并且 `imag(q)` 具有与 `imag(y)` 相同的符号。因此，要访问 `Sqrt()` 函数，您需要借助 `import` 关键字在程序中添加一个 `math/cmplx` 包。
