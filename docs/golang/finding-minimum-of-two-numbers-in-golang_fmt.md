...
> 原文：[https://www.geeksforgeeks.org/finding-minimum-of-two-numbers-in-golang/](https://www.geeksforgeeks.org/finding-minimum-of-two-numbers-in-golang/)

Go 语言提供了对基本常数和数学函数的内置支持，以便在`math`包的帮助下对数字进行运算。借助`math`包提供的`Min()`函数，可以在给定的两个数字中找到最小的数字。因此，您需要在导入关键字的帮助下在程序中添加一个`math`包来访问`Min()`函数。
