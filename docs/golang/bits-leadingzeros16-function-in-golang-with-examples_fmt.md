# `bits.LeadingZeros16()` 函数详解（附Golang示例）

> 原文：`https://www.geeksforgeeks.org/bits-leadingzeros16-function-in-golang-with-examples/`

`bits.LeadingZeros16()` 函数用于查找给定数字中的前导零位数。如果给定的数字等于零，那么这个方法将返回 `16`。要访问这个函数，需要在程序中导入 `math/bits` 包。
