...
> 原文：[https://www.geeksforgeeks.org/golang-splitting-the-slice-after-the-specified-separator/](https://www.geeksforgeeks.org/golang-splitting-the-slice-after-the-specified-separator/)

在 Go 语言中，切片比数组更强大、灵活、方便，是一种轻量级的数据结构。切片是存储相似类型元素的可变长度序列，不允许在同一切片中存储不同类型的元素。

在 `bytes` 包的 Go 切片中，您可以使用 `SplitAfterN()` 函数在指定的分隔符后分割切片。此函数在给定分隔符的每个实例之后将一个切片拆分为所有子切片，并返回包含这些子切片的切片。如果给定的分隔符是空的，那么它在每个 UTF-8 序列之后拆分，并且计数指示要返回的子片段的数量。它是在 `bytes` 包下定义的，所以你必须在程序中导入 `bytes` 包来访问 `SplitAfterN` 函数。
...
这里，`o_slice` 是原始字符串，`sep` 是分隔符，`m` 用来查找要返回的子字符串个数。这里如果 `m > 0`，那么最多返回 `m` 个子切片，最后一个子切片不会被分割。如果 `m == 0`，则返回零。如果 `m < 0`，则返回所有子切片。让我们借助给定的例子来讨论这个概念。
