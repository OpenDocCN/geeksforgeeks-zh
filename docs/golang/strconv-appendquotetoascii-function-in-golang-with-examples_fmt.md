...
Go 语言提供内置支持，通过 `strconv` 包实现基本数据类型的字符串表示形式之间的转换。此包提供了一个 `AppendQuoteToASCII()` 函数，用于将由 `QuoteToASCII` 生成的表示字符串的双引号 GO 字符串文字附加到 `num` 并返回扩展缓冲区。或者换句话说，将字符串 `str` 转换为由“双引号”产生的 ASCII 字符串，将结果附加到 `num` 的末尾并返回附加的 `[]byte`。要访问 `AppendQuoteToASCII()` 函数，您需要在程序中导入 `strconv` 包。
...
这里，`num` 是 `[]byte`，`str` 是 `string`。`str` 的结果将追加到 `num` 的末尾。
