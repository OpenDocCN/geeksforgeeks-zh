...
> 原文: [https://www.geeksforgeeks.org/finding-mod-of-given-number-in-golang/](https://www.geeksforgeeks.org/finding-mod-of-given-number-in-golang/)

Go 语言提供了对基本常数和数学函数的内置支持，以便在`math`包的帮助下对数字进行运算。借助`math`包提供的`Mod()`函数，可以找到指定 a/b 的 Mod 或浮点余数。因此，您需要在程序中添加一个`math`包，借助`import`关键字来访问`Mod()`函数。
