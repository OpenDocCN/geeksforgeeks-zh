...
> 原文: [https://www.geeksforgeeks.org/check-if-the-given-characters-is-present-in-golang-string/](https://www.geeksforgeeks.org/check-if-the-given-characters-is-present-in-golang-string/)
...
该函数用于检查给定字符串中是否存在给定的字母。如果给定字符串中有字母，则返回 `true`，否则返回 `false`。
...
这里 `str` 是原字符串，`chstr` 是你要检查的字符串。让我们借助一个例子来讨论这个概念。
...
此函数用于检查给定字符串中是否存在字符中的任何 Unicode 代码点。如果在给定的字符串中可以使用字符中的任何 Unicode 代码点，则此方法返回 `true`，否则返回 `false`。
