...
> 原文：[https://www.geeksforgeeks.org/how-to-iterate-over-an-array-using-for-loop-in-golang/](https://www.geeksforgeeks.org/how-to-iterate-over-an-array-using-for-loop-in-golang/)

[数组](https://www.geeksforgeeks.org/arrays-in-go/)是存储在连续位置的相似类型项目集合的数据结构。为了在数组上执行操作，需要遍历它。循环用于在编程语言中迭代数据结构。这里可以通过以下方式使用它：
...
**说明：** 关键字 `range` 设置迭代范围至 `arr` 长度。变量 `index` 和 `element` 分别存储数组的索引和值。

## 例 3

当我们不需要索引时，可以用空白标识符 `_`，忽略它。
