...
[MongoDB](https://www.geeksforgeeks.org/mongodb-an-introduction/) 是一个开源的 NoSQL 数据库。它是一个面向文档的数据库，使用一个类似 JSON 的叫做 BSON 的结构来存储文档（即键值对）。MongoDB 提供了集合的概念来对文档进行分组。在本文中，我们将讨论如何将 MongoDB 与 Golang 连接起来。

## 先决条件

需要安装 MongoDB，并在默认端口（即 `27017`）上启动。

## 安装

Package `mongo` 为 Go 提供了一个 MongoDB 驱动 API，可以用来和 MongoDB API 进行交互。使用下面的命令安装软件包 `mongo`。
...
包 `context` 是 `context.Context` 类型，它保存截止日期、取消信号和其他请求范围的值，跨越应用编程接口边界和进程之间。

### 将 Go 驱动程序与 MongoDB 连接

现在，要将 Go 驱动程序与 MongoDB 连接，您需要执行以下步骤：

1.  创建 `mongo.Client`。`mongo.Connect` 功能返回 `mongo.Client`，它处理与 MongoDB 的连接。
2.  `mongo.Client` 有一个名为 `Ping` 的方法，在成功连接时返回 pong。
3.  最后，使用 `mongo.Client.Disconnect` 以关闭数据库连接。
...
1.  创建 `mongo.Client`。`mongo.Connect` 功能返回 `mongo.Client`，它处理与 MongoDB 的连接。
2.  `mongo.Client.Database` 返回一个指向数据库的指针类型。
3.  指向数据库的指针有一个方法 `Collection` 来选择要使用的集合。
4.  `Collection` 类型提供了两种将文档插入 MongoDB 的方法。
5.  `Collection.InsertOne()` 方法可以向数据库中插入一个文档。
6.  `Collection.InsertMany()` 方法可以插入文档列表。
7.  然后最后使用 `mongo.Client.Disconnect` 以关闭数据库连接。
...
7.  然后最后使用 `mongo.客户端。断开连接` 以关闭数据库连接。
...
1.  创建`Mongo`客户端。`Mongo`客户端处理与`MongoDB`的连接。
2.  `Mongo`数据库返回一个指向数据库的指针类型。
3.  指向数据库的指针有一个方法`集合`来选择要使用的集合。
4.  集合提供两种方法来删除集合中的文档。
