...
> 原文：[https://www.geeksforgeeks.org/how-to-split-a-slice-of-bytes-in-golang/](https://www.geeksforgeeks.org/how-to-split-a-slice-of-bytes-in-golang/)
...
在字节的 Go 切片中，您可以使用`Split()`功能分割给定的切片。该函数将一个字节片分割成由给定分隔符分隔的所有子片，并返回一个包含所有这些子片的片。它是在`bytes`包下定义的，所以您必须在程序中导入`bytes`包来访问`Split`函数。
