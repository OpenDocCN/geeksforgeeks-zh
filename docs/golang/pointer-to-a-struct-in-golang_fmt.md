...
> 原文: [https://www.geeksforgeeks.org/pointer-to-a-struct-in-golang/](https://www.geeksforgeeks.org/pointer-to-a-struct-in-golang/)
...
这里，我们正在创建一个名为 `Employee` 的结构，它有两个变量。在主功能中，创建结构的实例，即 `emp`。之后，您可以将结构的地址传递给指针，该指针表示指向结构概念的指针。没有必要显式使用取消引用，因为它会给出与您在下面的程序中看到的相同的结果（两次 ABC）。
