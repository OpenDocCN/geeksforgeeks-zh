...
> 原文：`https://www.geeksforgeeks.org/how-to-trim-white-spaces-from-the-slice-of-bytes-in-golang/`
...
在字节的 Go 切片中，您可以使用 `TrimSpace()` 功能修剪给定切片的所有前导和尾随空格。该函数通过去除所有前导和尾随空格来返回原始切片的子切片。它是在 `bytes` 包下定义的，所以你必须在你的程序中导入 `bytes` 包来访问 `TrimSpace` 函数。
