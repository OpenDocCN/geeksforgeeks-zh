...
> 原文：`https://www.geeksforgeeks.org/bits-mul32-function-in-golang-with-examples/`

**位.Mul32()函数**用来求`x`和`y`的 64 位乘积，这个函数的执行时间不依赖于输入。要访问这个函数，需要在程序中导入`math/bits`包。
