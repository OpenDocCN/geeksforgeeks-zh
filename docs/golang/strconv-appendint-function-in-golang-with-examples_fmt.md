# Golang 中的 strconv.AppendInt() 函数及示例
...
Go 语言提供内置支持，通过`strconv`包实现与基本数据类型字符串表示的转换。该包提供`AppendInt()`函数用于将`FormatInt`生成的整数`val`的字符串形式追加到`num`，并返回扩展缓冲区，如下语法所示。要访问`AppendInt()`函数，需要在程序中导入`strconv`包。
