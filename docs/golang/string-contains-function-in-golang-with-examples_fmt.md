...
`strings.Contains()` 函数用于检查给定字符串中是否存在给定的字母。如果字母出现在给定的字符串中，则返回 `TRUE`，否则返回 `FALSE`。
...
**解释：**在上面的示例中，我们检查不同字符串中是否存在子串 `'for'` 和 `'Science'`。由于 `strings.Contains()` 函数返回布尔值，因此在这两种情况下都返回 `TRUE`。

## 示例 2：

下面的示例说明了用户如何打印所需的结果，而不是布尔输出：
