...
> 原文：[https://www.geeksforgeeks.org/how-to-find-the-first-index-value-in-slice-of-bytes-in-golang/](https://www.geeksforgeeks.org/how-to-find-the-first-index-value-in-slice-of-bytes-in-golang/)
...
在字节的 Go 切片中，可以使用 `Index()` 函数找到给定切片中指定实例的第一个索引值。此函数返回给定值在原始字节片中的第一个实例的索引。如果原始切片中没有给定值，则返回 `-1`。它是在 `bytes` 包下定义的，所以您必须在程序中导入 `bytes` 包来访问 `Index` 函数。
