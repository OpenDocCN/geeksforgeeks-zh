# io.PipeWriter.Close() 函数详解

> 原文：[https://www.geeksforgeeks.org/io-pipewriter-close-function-in-golang-with-examples/](https://www.geeksforgeeks.org/io-pipewriter-close-function-in-golang-with-examples/)

在 Go 语言中，`io` 包为输入/输出原语提供基本接口。它的主要工作是封装这种原语之王正在进行的实现。`PipeWriter` 的 `Close()` 函数用于关闭编写器。但是，从 `PipeReader` 连续读取，即读取管道的一半，不会返回任何字节，并且会返回一个 `EOF` 错误。此外，该函数在 `io` 包中定义。在这里，您需要导入 `io` 包才能使用这些函数。
...
**返回值：** 返回 `Close` 方法之前写的内容。并且从 `PipeReader` 连续读取，即读取管道的一半，在调用 `Close()` 方法后不会返回任何字节，并且会返回一个 `EOF` 错误。
