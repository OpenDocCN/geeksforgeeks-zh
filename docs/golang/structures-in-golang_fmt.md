...
上面的代码创建了一个`Address`类型的变量，该变量默认设置为零。 对于结构，零表示所有字段都设置为其对应的零值。 因此，字段`Name`、`Street`、`City`、`State`被设置为`""`，`Pincode`被设置为`0`。
...
[Golang中的指针](https://www.geeksforgeeks.org/pointers-in-golang/)是用于存储另一个变量的内存地址的变量。 您还可以创建指向结构的指针，如下例所示：
...
Golang为我们提供了使用`emp8.firstName`而不是显式解引用`(*emp8).firstName`来访问`firstName`字段的选项。 下面是说明这一点的示例：
