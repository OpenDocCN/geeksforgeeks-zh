...
> 原文：[https://www.geeksforgeeks.org/golang-splitting-the-string-after-the-specified-separator/](https://www.geeksforgeeks.org/golang-splitting-the-string-after-the-specified-separator/)

在 Go 语言中，字符串不同于其他语言，如 Java、C++、Python 等。它是一个可变宽度字符序列，其中每个字符都由一个或多个字节使用 UTF-8 编码来表示。

在 Go 字符串中，您可以使用 `SplitN()` 函数在指定的分隔符后拆分字符串。此函数在给定分隔符的每个实例之后将一个片段拆分为所有子字符串，并返回这些分隔符之间的子字符串片段。计数表示要返回的子策略的数量。它是在 `strings` 包下定义的，所以您必须在程序中导入 `strings` 包才能访问 `SplitN` 函数。
...
这里 `str` 是字符串，`sep` 是分隔符。如果 `str` 不包含给定的 `sep` 和 `sep` 非空，那么它将返回一个长度为 1 的切片，该切片只包含 `str`。或者如果 `sep` 是空的，那么它将在每个 UTF-8 序列之后分裂。或者如果 `str` 和 `sep` 都为空，则返回一个空切片。
