...
> 原文: [https://www.geeksforgeeks.org/how-to-find-the-type-of-struct-in-golang/](https://www.geeksforgeeks.org/how-to-find-the-type-of-struct-in-golang/)
...
方法 `reflect.TypeOf()` 返回 `main.Student` 作为类型，而 `reflect.ValueOf().Kind()` 返回一个 `struct`。正是因为方法 `reflect.TypeOf()` 返回类型为 `reflect.Type` 的变量。`reflect.Type` 包含定义传递的变量的类型的所有信息，在本例中是 `Student`。`Kind()` 告诉我们这个类型最初是由什么组成的——指针、`int`、字符串、`struct`、接口或其他内置数据类型。在我们的例子中，类型是 `Student`，类型是 `struct`。
