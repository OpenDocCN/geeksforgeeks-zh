...
> 原文：[https://www.geeksforgeeks.org/how-to-print-specific-date-time-in-golang/](https://www.geeksforgeeks.org/how-to-print-specific-date-time-in-golang/)

Golang 通过基于模式的布局支持时间格式化和解析。在 Go 中，当前时间可以通过使用 `time.Now()` 来确定，该函数由 `time` 包提供。`time` 包提供测量和显示时间的功能。

要打印当前的日期时间，您需要在您的 Go 程序中导入 `"time"` 包来处理日期和时间。
...
要在 Golang 中打印特定的日期时间，请使用 `time` 包中提供的格式常量。常用的格式有：
