# Golang 中的 `strings.NewReplacer()` 函数
...
Golang 中的 `strings.NewReplacer()` 函数从以前的新字符串集列表中返回一个新的 `Replacer`。 替换是按照它们在目标字符串中显示的顺序执行的，没有重叠匹配。 旧字符串比较是按参数顺序进行的。 旧字符串比较是按参数顺序进行的。
