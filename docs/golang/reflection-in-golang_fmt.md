...
反射是程序在**运行时**内省和分析其结构的能力。在围棋语言中，主要通过**类型**进行反思。`reflect`包提供了用于此目的的所有必需的原料药/方法。反射通常被称为**元编程的一种方法。**为了更好地理解反射，让我们先了解一下空接口：
...
**反思包：**
围棋反思的基础是围绕`reflect.Value`、`reflect.Type`和`reflect.Kind`展开的。这些在包中定义，属于`reflect`类型。`reflect.Value`体现。`reflect.Type`和`reflect.Kind`并且可以使用方法获得：
...
| 使用`reflect.TypeOf(x interface{})`可以获得类型。 | 使用`Kind()`可以获得种类。 |

</figure>

`reflect`包为我们提供了许多其他方法：

1.  `NumField()`：此方法返回结构中存在的字段数。如果传递的参数不是`reflect.Struct`，那么它就会恐慌。
2.  `Field()`：这个方法允许我们使用索引变量访问结构中的每个字段。

在下面的例子中，我们将发现一个结构和另一个自定义数据类型的种类和类型之间的区别。除此之外，我们还将使用`reflect`包中的方法来获取和打印结构中的字段以及自定义数据类型中的值。
...
我们已经使用了`reflect.TypeOf(i interface{})`和`Type.Kind()`获取这些字段的方法，我们可以看到输出的差异。

*   结构`person`是`main.details`类型和`reflect.Struct`种类。
*   变量`iD`为`main.myType`类型和`string`种类。

因此，类型和种类之间的区别变得清晰，并根据他们的定义。这是《围棋语言》反思中的一个基本概念。此外，我们使用了`reflect.ValueOf()`、`.NumField()`和`.Field()`方法也可以从`reflect`包中获取空接口中的值、结构体的字段数，然后分别获取每个字段。这是可能的，因为在运行时使用了反射，这允许我们确定参数的类型和种类。

**注：**`NumField()`和`.Field()`只适用于结构体。如果元素不是结构，将导致死机。格式说明符`%T`不能用于打印种类。它会打印`reflect.Kind`。如果我们在带有`%T`的`Printf`语句中传递`i.Kind()`，它将打印`reflect.Kind`，本质上是围棋中所有种类的类型。

值得注意的是，`.Field(i)`的值的类型是`reflect.Value`，是类型而不是种类。“种类”显示在下面一行。因此，我们看到了反思在《围棋》中的重要性和作用。知道运行时变量的类型使我们能够编写大量的泛型代码。因此，反思是格朗不可或缺的根本。