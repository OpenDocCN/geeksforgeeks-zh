...
*   在地图中，一个键必须是唯一的，并且必须是使用 `==` 运算符可比较的类型或支持 `!=` 操作员的类型。因此，大多数内置类型可以用作一个键，如 `int`、`float64`、`rune`、`string`、可比数组和结构、指针等。像切片和不可比较的数组和结构这样的数据类型或不可比较的自定义数据类型不用作映射键。
*   在地图中，值不像键一样是唯一的，可以是任何类型，如 `int`、`float64`、`rune`、`string`、指针、引用类型、地图类型等。
...
也可以使用 `make()` 函数创建地图。这个函数是一个内置函数，在这个方法中，您只需要传递映射的类型，它就会返回一个初始化的映射。
