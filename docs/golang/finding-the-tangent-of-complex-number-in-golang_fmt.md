...
> 原文: [https://www.geeksforgeeks.org/finding-the-tangent-of-complex-number-in-golang/](https://www.geeksforgeeks.org/finding-the-tangent-of-complex-number-in-golang/)

Go 语言借助 `cmplx` 包，为复数的基本常数和数学函数提供内置支持。借助 `math/cmplx` 包提供的 `Tan()` 函数，可以求出指定复数的正切值。因此，您需要借助 `import` 关键字在程序中添加一个 `math/cmplx` 包来访问 `Tan()` 函数。
