...
在 Golang 中，`range`关键字用于不同类型的数据结构，以便迭代元素。`range`关键字主要用于`for`循环，以便迭代一个[地图](https://www.geeksforgeeks.org/golang-maps/)、[切片](https://www.geeksforgeeks.org/slices-in-golang/)、[通道](https://www.geeksforgeeks.org/channel-in-golang/)或一个[数组](https://www.geeksforgeeks.org/arrays-in-go/)的所有元素。当它遍历数组的元素并进行切片时，它以整数形式返回元素的索引。当它遍历一个映射的元素时，它返回下一个键值对的键。此外，`range`可以返回一个值或两个值。让我们看看在 Golang 中迭代不同种类的集合时`range`返回什么。
