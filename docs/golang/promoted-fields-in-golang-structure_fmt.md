...
> 原文: [https://www.geeksforgeeks.org/promoted-fields-in-golang-structure/](https://www.geeksforgeeks.org/promoted-fields-in-golang-structure/)
...
在上面的例子中，我们有两个结构，分别命名为`details`和`student`。其中`details`结构是正常结构，`student`结构是嵌套结构，它包含`details`结构作为字段，就像匿名字段一样。现在，详细信息结构的字段，即`name`、`age`和`gender`，被提升到`student`结构中，称为提升字段。现在，您可以借助`student`结构直接访问它们，如`values.name`、`values.age`、`values.gender`。