...
> 原文:[https://www . geesforgeks . org/reflect-swapper-function-in-golang-with-examples/](https://www.geeksforgeks.org/reflect-swapper-function-in-golang-with-examples/)

Go 语言提供了运行时反射的内置支持实现，并允许程序在反射包的帮助下操作任意类型的对象。`reflect.Swapper()`函数用于交换所提供切片中的元素。要访问这个函数，需要在程序中导入`reflect`包。
