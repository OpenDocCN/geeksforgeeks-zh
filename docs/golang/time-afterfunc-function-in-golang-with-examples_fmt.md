...
在围棋语言中，`Time`包提供了确定和查看时间的功能。 Go 语言中的`AfterFunc()`函数用于等待经过的持续时间，之后，它在自己的 Go 例程中调用定义的函数`f`。 此外，该函数在`time`包下定义。 在这里，您需要导入`time`包才能使用这些函数。
...
它返回一个`*Timer`，然后在其`stop()`方法的帮助下使用该计时器取消调用。
