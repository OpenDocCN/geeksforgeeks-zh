...
> 原文: [https://www.geeksforgeeks.org/how-to-trim-prefix-from-the-slice-of-bytes-in-golang/](https://www.geeksforgeeks.org/how-to-trim-prefix-from-the-slice-of-bytes-in-golang/)
...
在字节的 Go 切片中，您可以使用 `TrimPrefix()` 函数从给定切片中修剪前缀。这个函数通过切断给定的前导前缀字符串来返回原始切片的子切片。如果给定的字节片段不包含指定的前缀字符串，则此函数返回原始片段，不做任何更改。它是在 `bytes` 包下定义的，所以您必须在程序中导入 `bytes` 包来访问 `TrimPrefix` 函数。
