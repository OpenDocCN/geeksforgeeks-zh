...
> 原文: [https://www.geeksforgeeks.org/counting-number-of-repeating-words-in-a-golang-string/](https://www.geeksforgeeks.org/counting-number-of-repeating-words-in-a-golang-string/)
...
要计算重复单词的数量，首先将字符串作为输入，然后使用 `strings.Fields()` 函数来拆分字符串。定义了一个函数 `repetition()` 来计算重复的单词数。
...
在上面的程序中，我们首先将一个字符串作为输入，然后使用 `strings.Fields()` 函数来分割该字符串。如果出现了相同的单词，计数将增加一个，否则将返回值 1，这意味着该单词在字符串中只出现一次。