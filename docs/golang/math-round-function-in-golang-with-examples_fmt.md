# Golang 中的 Round() 函数示例

> 原文：[https://www.geeksforgeeks.org/math-round-function-in-golang-with-examples/](https://www.geeksforgeeks.org/math-round-function-in-golang-with-examples/)

Go 语言提供了对基本常数和数学函数的内置支持，以便在 `math` 包的帮助下对数字进行运算。借助 `math` 包提供的 `Round()` 函数，可以将给定的数字四舍五入到最接近的整数（四舍五入到离零一半）。因此，您需要在导入关键字的帮助下在程序中添加一个 `math` 包来访问 `Round()` 函数。
