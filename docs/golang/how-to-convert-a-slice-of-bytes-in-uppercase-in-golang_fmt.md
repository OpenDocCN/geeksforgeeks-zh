...
> 原文: [https://www.geeksforgeeks.org/how-to-convert-a-slice-of-bytes-in-uppercase-in-golang/](https://www.geeksforgeeks.org/how-to-convert-a-slice-of-bytes-in-uppercase-in-golang/)
...
在 Go 分片字节中，允许使用 `ToUpper()` 函数将分片转换为大写。此函数返回给定字节片的副本（视为 UTF-8 编码字节），其中所有 Unicode 字母都映射为大写。它是在 `bytes` 包下定义的，所以你必须在程序中导入 `bytes` 包来访问 `ToUpper` 函数。
