...
> 原文：`https://www.geeksforgeeks.org/how-to-count-the-number-of-repeated-characters-in-a-golang-string/`
...
在 Go 字符串中，可以借助 `Count()` 函数统计字符串中某些特定的 Unicode 码点或 `costr`（重复字符）的非重叠实例数。此函数返回一个值，该值代表给定字符串或字符串中存在的 Unicode 代码点的总数。它是在 `strings` 包下定义的，所以您必须在程序中导入 `strings` 包来访问 `Count` 函数。
...
这里 `str` 是原字符串，`costr` 是我们要计数的字符串。如果 `costr` 的值为空，则该函数返回 1 + 字符串中的 Unicode 码点数。
