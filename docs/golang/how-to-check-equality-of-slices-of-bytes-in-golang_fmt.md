...
> 原文：[https://www.geeksforgeeks.org/how-to-check-equality-of-slices-of-bytes-in-golang/](https://www.geeksforgeeks.org/how-to-check-equality-of-slices-of-bytes-in-golang/)
...
在 bytes 的 Go 切片中，可以借助 `Equal()` 函数检查切片的相等性。如果两个切片相等，此函数返回 `true`；如果两个切片不相等，则返回 `false`。它是在 `bytes` 包下定义的，所以您必须在程序中导入 `bytes` 包来访问 `Equal` 函数。
