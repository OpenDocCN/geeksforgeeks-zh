...
> 原文：[https://www.geeksforgeeks.org/io-copyn-function-in-golang-with-examples/](https://www.geeksforgeeks.org/io-copyn-function-in-golang-with-examples/)
...
返回拷贝到 `dst` 的 `int64` 类型的字节总数，还返回从 `src` 拷贝到 `dst` 时遇到的第一个错误（如果有）。此外，当且仅当错误为“无”时，返回的字节数为“n”。
