...
在围棋语言中，`time`包提供了确定和查看时间的功能。 GO 语言中的`LoadLocation()`函数用于查找指定名称的位置。 因此，如果声明的名称是`"UTC"`，则返回`UTC`，如果声明的名称是`"Local"`，则返回`Local`。 否则，假定要使用的名称是相当于 IANA 时区数据库中的文件的位置。 其中此数据库仅存在于 Unix 系统上。 而且，这个函数是在`time`包中定义的。 在这里，你需要导入`"time"`包才能使用这些函数。
...
这里，`name`是要使用的位置的名称，`*Location`是指向该位置的指针。 其中`Location`形成使用中的时间偏移量集合。 而`error`是恐慌性错误。
