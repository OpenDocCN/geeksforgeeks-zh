...
> 原文: [https://www.geeksforgeeks.org/partition-the-array-in-k-segments-such-that-bitwise-and-of-individual-segment-sum-is-maximized/](https://www.geeksforgeeks.org/partition-the-array-in-k-segments-such-that-bitwise-and-of-individual-segment-sum-is-maximized/)

给定一个大小为 `N` 的数组和一个整数 `K`。任务是在 `K` 段中划分数组，使得单个段和的位“与”最大化。求按位“与”的最大值，可以得到。
...
> `(2+5+6)&(9+1)&(3+10+12)= 8`
> **输入:** `a[] = {1, 2, 7, 10, 23, 21, 6, 8, 7, 3}`，`K = 2`
> **输出:** `41`
> **解释** :
...
**方法:**
首先，试着回答一个更简单的问题:给定一个整数 `x`，并确定是否有可能将给定的数组划分为 `K` 段，使得段和的位“与”具有 `x` 的所有设置位？
我们用 `S_i` 来表示第 `i` 段的元素之和。另外，如果 `S_i` 有 `x` 的所有设定位，我们称一个段为“好”。现在很明显，对于一个好的段 `i`，`S_i & x = x`。
还有，所有 `K` 段都应该是“好的”才能得到 `x` 的按位 AND。现在来检查一下我们是否可以把数组分割成 `k` 个“好的”段。我们可以在这里使用[动态编程](https://www.geeksforgeeks.org/dynamic-programming/)。
让 `dp[i][j] = true`，表示可以先将前 `i` 个元素分割成 `j` 段，这样所有 `j` 段都是“好的”段，否则为 `false`。
上述 dp 的递推关系为:

> `dp[i][j]` 是 `true`，如果对于某些索引 `l`，`dp[l][j-1]` 为 `true` 且 `(prefix[i] - prefix[l]) & mask == mask`。
...
bool checkpossible(int mask, int* arr, int* prefix, int n, int k)
{
...
class GFG
{
    // Function to check whether a k segment partition
    // is possible such that bitwise AND is 'mask'
    static boolean checkpossible(int mask, int arr[], int prefix[], int n, int k)
    {
        int i, j;
