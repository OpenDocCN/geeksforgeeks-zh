...
> 原文: [https://www.geeksforgeeks.org/reverse-words-in-a-given-string-set-2/](https://www.geeksforgeeks.org/reverse-words-in-a-given-string-set-2/)

给定字符串 `str`，任务是通过将字符串的每个单词 `str` 视为一个单元来反转字符串。

**示例:**

> **输入:** `str` = "geeks quiz practice code"
> **输出:** code practice quiz geeks
> **解释:**
> 给定字符串中的单词为["geeks", "quiz", "practice", "code"]。
> 因此，颠倒单词顺序后，需要的输出是"code practice quiz geeks"。
>
> **输入:** `str` = "coding is fun to do"
> **输出:** do to fun is coding
...
1.  创建一个 `栈` 来存储字符串 `str` 的每个单词。
2.  遍历字符串 `str`，用空格分隔符分隔 `str` 的每个单词。
3.  将 `str` 的所有单词推入堆栈。
4.  逐一打印堆栈的所有元素。
...
*   因为一个句子中的所有单词都用空格隔开。
*   我们必须使用 [`split()`](https://www.geeksforgeeks.org/python-string-split/) 通过空格来分割句子。
