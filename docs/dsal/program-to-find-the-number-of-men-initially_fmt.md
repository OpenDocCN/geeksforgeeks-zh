...
> 原文: [https://www.geeksforgeeks.org/program-to-find-the-number-of-men-initially/](https://www.geeksforgeeks.org/program-to-find-the-number-of-men-initially/)
...
假设初始人数为 `M`，天数为 `D`。
在 `D` 天内完成的工作数量为 `M` 人，则 `M * D`。即 **已完成的工作 = `M * D`** ……(1)
如果有 `M + m` 人，则 `d` 天内完成的工作数量相同。
即 **完成的功 = `(M + m) * (D - d)`** ……(2)
等式 1 和等式 2，得：
