...
> 原文：[https://www.geeksforgeeks.org/minimum-removals-required-to-make-frequency-of-each-array-element-equal-to-its-value/](https://www.geeksforgeeks.org/minimum-removals-required-to-make-frequency-of-each-array-element-equal-to-its-value/)
...
**方法：** 使用贪婪算法可以解决问题。按照以下步骤解决问题：

*   初始化一个图（map）`mp` 来存储数组中每个不同元素的频率。
*   遍历数组并存储数组中每个不同元素的频率。
*   初始化一个变量 `cntMinRem` 来存储需要移除的数组元素的最小数量，使得 `arr[i]` 的频率等于 `arr[i]`。
