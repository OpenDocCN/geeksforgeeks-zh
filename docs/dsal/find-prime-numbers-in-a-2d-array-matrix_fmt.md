...
> 原文: [https://www.geeksforgeeks.org/find-prime-numbers-in-a-2d-array-matrix/](https://www.geeksforgeeks.org/find-prime-numbers-in-a-2d-array-matrix/)
...
**说明:** 第一个质数在第 1 行第 2 列的位置，数值为 2。第二个质数位于第 2 行和第 1 列的位置，值为 2。
...
按照以下步骤实施此方法:
*   从矩阵中找到[最大元素，并将其存储在变量 `maxNum` 中。](https://www.geeksforgeeks.org/program-to-find-the-maximum-element-in-a-matrix/)
*   现在使用[埃拉托斯特尼的筛子找到从 `1` 到 `maxNum` 的质数，并将结果存储在数组 `prime[]` 中。](http://www.geeksforgeeks.org/sieve-of-eratosthenes/)
*   现在遍历矩阵，使用 `prime[]` 数组检查每个数是否是素数。
