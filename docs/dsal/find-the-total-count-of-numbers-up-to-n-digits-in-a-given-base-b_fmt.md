...
> 原文: [https://www.geeksforgeeks.org/find-the-total-count-of-numbers-up-to-n-digits-in-a-given-base-b/](https://www.geeksforgeeks.org/find-the-total-count-of-numbers-up-to-n-digits-in-a-given-base-b/)
...
> 第 `N` 项 = 基数 `B` 中 `N` 位数的自然数 = `(B–1) * B^(N–1)`

最后，通过从 `1` 到 `N` 迭代一个循环，并使用上述公式计算第 `i` 项和第 `i` 项，可以计算出基数 `B` 中所有自然数的计数，直到 `N` 位。
