...
> 原文: [https://www.geeksforgeeks.org/sort-an-alphanumeric-string-such-that-the-positions-of-alphabets-and-numbers-remain-unchanged/](https://www.geeksforgeeks.org/sort-an-alphanumeric-string-such-that-the-positions-of-alphabets-and-numbers-remain-unchanged/)
...
**方法:** 我们将字符串转换为字符数组，然后对字符数组 `c[]` 进行排序。对字符数组进行排序后，数字字符将占据数组的起始索引，字母将占据数组的剩余部分。数字部分将被排序，字母部分也将被排序。我们将保留两个索引，一个在字母部分 `al_c` 的起始索引处，一个在数字部分 `nu_c` 的起始索引处，现在我们将检查原始字符串，如果某个位置被字母占据，那么我们将使用 `c[nu_c]` 替换它，并增加 `nu_c`，否则我们将使用 `c[al_c]` 替换它，并增加 `al_c`。
