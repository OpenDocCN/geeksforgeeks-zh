...
> 原文：[https://www.geeksforgeeks.org/queries-on-array-with-disappearing-and-reappearing-elements/](https://www.geeksforgeeks.org/queries-on-array-with-disappearing-and-reappearing-elements/)

给定一个大小为 `N` 的数组 `arr[]`。每一秒钟，一个整数消失 `N` 秒钟，在 `N` 秒钟后，它重新出现在原来的位置。整数从左到右依次消失 `arr[0]`，`arr[1]`，…，`arr[N–1]`。所有整数消失后，它们开始重新出现，直到所有整数重新出现。一旦 `N` 元素再次出现，过程再次开始。
现在给出 `Q` 查询，每个查询由两个整数 `t` 和 `M` 组成。任务是在第 `t` 秒处从左侧确定第 `M` 个元素。如果在 `M` 之前数组不存在，则打印 `-1`。
...
主要的方式是要求检查数组是空的还是满的，用圈数除以数组的大小就可以看出来。如果余数为 `0`，则它可以是两种情况之一（空或满）。
