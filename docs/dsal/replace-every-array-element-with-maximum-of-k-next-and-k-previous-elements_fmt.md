...
> 原文：[https://www.geeksforgeeks.org/replace-every-array-element-with-maximum-of-k-next-and-k-previous-elements/](https://www.geeksforgeeks.org/replace-every-array-element-with-maximum-of-k-next-and-k-previous-elements/)
...
一个[线段树](https://www.geeksforgeeks.org/segment-tree-set-2-range-maximum-query-node-update/)可以用来解决这个问题。因此，构建一个最大范围线段树，其中：

*   叶节点是输入数组的元素。
*   每个内部节点代表其所有子节点的最大值。

现在，在建立线段树之后，找到从 `(i-K)` 到 `(i-1)` 的最大值，称为**左边的**，和 `(i+1)` 到 `(i+K)` 的最大值，称为**右边的**，在这个线段树上使用查询。将 `arr[i]` 换成最大的**左**和**右**。
