...
> 原文：[https://www.geeksforgeeks.org/bitwise-xor-of-elements-having-odd-frequency/](https://www.geeksforgeeks.org/bitwise-xor-of-elements-having-odd-frequency/)
...
遍历数组，将所有元素的频率存储在 `unordered_map` 中。现在，使用上一步创建的映射计算奇数频率元素的异或。
...
这个解决方案需要 `O(n)` 时间和 `O(n)` 空间。

## 有效方法

这种方法使用 XOR 的两个重要属性——`a ^ a = 0` 和 `0 ^ a = a`。对数组中的所有元素进行 XOR。结果将是出现奇数次的数字的异或，因为出现偶数次的元素最终会相互抵消。
