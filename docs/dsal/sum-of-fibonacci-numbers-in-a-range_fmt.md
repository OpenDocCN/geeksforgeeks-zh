...
> 原文: [https://www.geeksforgeeks.org/sum-of-fibonacci-numbers-in-a-range/](https://www.geeksforgeeks.org/sum-of-fibonacci-numbers-in-a-range/)

给定一个范围`【l，r】`，任务是求`fib(l) + fib(l + 1) + fib(l + 2) + …..+ fib(r)`的和，其中`fib(n)`是第`n`个斐波那契数。
...
简单计算`fib(l) + fib(l + 1) + fib(l + 2) + …..+ fib(r)`，时间复杂度为`O(r–l)`。
...
> `fib(n)=(φⁿ–ψⁿ)/√5`
> 其中，
> `φ=(1+√(5))/2` 大致等于 1.61803398875
> `ψ = 1–φ=(1–√(5))/2` 大致等于 0.61803398875
...
思路是找到斐波那契数之和与第`n`个斐波那契数的关系，用**比奈公式**计算其值。
...
为了找到`S(n)`，只需计算`(n + 2)ᵗʰ`斐波那契数，从结果中减去`1`。
因此，
`S(l，r)= S(r)–S(l–1)`
