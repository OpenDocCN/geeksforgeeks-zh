...
#### 注意:
我们可以只包含一个库，即`#include <bits/stdc++.h>`包括所有的 [STL](https://www.geeksforgeeks.org/the-c-standard-template-library-stl/) 库，但是在某些比赛中，包括这个库会让代码变慢。为了克服这个问题，我们可以添加特定的库来访问 **STL** 的特定数据结构。此外，在移除元素时，需要注意数据结构是否为空。对空数据结构调用 remove 函数会导致错误。以下是一些[数据结构](https://www.geeksforgeeks.org/data-structures/)及其图示
...
#### 向量最常见的函数:
1.  `push_back()`: 用于推送向量末尾的元素。要获得更快的方法，请使用`emplace_back()`。
2.  `pop_back()`: 用于从向量中移除最后一个元素。
3.  `size()`: 返回向量的大小。
4.  `clear()`: 删除矢量的所有内容。
5.  `erase()`: 删除指定的索引或数据。
6.  `empty()`: 如果向量为空，返回布尔值 True，否则返回 False。
7.  `lower_bound(iterator first, iterator last, const val)`: 下界返回一个迭代器，指向范围[first, last]中值不小于‘val’的第一个元素。
8.  `upper_bound(iterator first, iterator last, const val)`: 上界返回一个迭代器，指向范围[first, last]中的第一个元素，该元素的值大于“val”。
