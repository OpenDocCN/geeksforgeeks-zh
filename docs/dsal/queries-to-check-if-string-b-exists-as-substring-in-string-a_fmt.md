...
> 原文：[https://www.geeksforgeeks.org/queries-to-check-if-string-b-exists-as-substring-in-string-a/](https://www.geeksforgeeks.org/queries-to-check-if-string-b-exists-as-substring-in-string-a/)

给定两个字符串 `A`、`B` 和一些由整数 `i` 组成的查询，任务是检查从索引 `i` 开始到索引 `i + length(B) - 1` 的子字符串是否等于 `B`。如果相等，则打印 `Yes`，否则打印 `No`。注意 `i + length(B)` 永远小于 `length(A)`。
...
一种简单的方法是对每个查询逐个字符地比较字符串，回答每个查询需要 `O(length(B))` 的时间。

## 高效方法

我们将使用[滚动哈希](https://www.geeksforgeeks.org/rabin-karp-algorithm-for-pattern-searching/)算法优化查询处理。
...
从 `i` 开始的子串 `A` 的散列可以被发现为 `(hash_A[i + len_b - 1] - hash_A[i - 1]) / d^i`，或者更具体地：
