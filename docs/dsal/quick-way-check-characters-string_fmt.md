...
> 原文: [https://www.geeksforgeeks.org/quick-way-check-characters-string/](https://www.geeksforgeeks.org/quick-way-check-characters-string/)
...
想法是在 C++ STL 中使用 `find_first_not_of()` 方法。`find_first_not_of()` 查找并返回第一个与指定字符 (或字符串中任何指定字符) 不匹配的字符的位置。
