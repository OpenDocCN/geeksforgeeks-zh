...
4.  从队列开始弹出顶点，直到队列为空，并对每个弹出的顶点执行以下步骤（让 `U` 为弹出的顶点）：
    *   将顶点 `U` 设置为不存在于队列中。
    *   对于 `U` 的每个相邻顶点 `V`，检查其当前的最小 `Distance[V]` 是否大于通过 `U` 的距离，即 `Distance[U] + 连接 U 和 V 的边的重量`。
    *   如果是，则更新 `Distance[V] = Distance[U] + weight of edge connecting U and V`。
    *   借助布尔数组检查 `V` 是否不在队列中：
        1.  如果 `V` 首次进入队列，请在队列的**后面**附加 `V`，并借助布尔数组将顶点 `V` 设置为队列中存在的值。
