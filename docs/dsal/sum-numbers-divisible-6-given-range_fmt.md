...
> 原文: [https://www.geeksforgeeks.org/sum-numbers-divisible-6-given-range/](https://www.geeksforgeeks.org/sum-numbers-divisible-6-given-range/)

给定一个 `L`-`R` 范围，在 `L`-`R` 范围内找到所有可被 6 整除的数的和。`L` 和 `R` 都很大。
...
一种天真的方法是从 `L` 到 `R` 循环，将所有能被 6 整除的数字相加。一个有效的方法是将所有可被 6 整除的数加起来直到和为 `R`，将所有可被 6 整除的数加起来直到 `L-1`。然后相减就会得到答案。
