...
> 原文：[https://www.geeksforgeeks.org/queries-to-find-minimum-sum-of-array-elements-from-either-end-of-an-array/](https://www.geeksforgeeks.org/queries-to-find-minimum-sum-of-array-elements-from-either-end-of-an-array/)

给定一个由 `N` 个不同整数组成的数组 `arr[]` 和一个由 `Q` 个查询组成的数组 `query[]`，每个查询的任务是在数组中找到 `query[i]`，并计算从数组开始和结束到 `query[i]` 的数组元素之和的最小值。

**示例：**

> **输入：** `arr[] = {2, 3, 6, 7, 4, 5, 30}`，`Q = 2`，`query[] = {6, 5}`
> **输出：** `11 27`
> **解释：**
> 查询 1：从开始的总和 = `2 + 3 + 6 = 11`。结束时的总和 = `30 + 5 + 4 + 7 + 6 = 52`。因此，`11` 是必需的答案。
> 查询 2：从开始的总和 = `27`。结束时的总和 = `35`。因此，`27` 是必需的答案。
>
> **输入：** `arr[] = {1, 2, -3, 4}`，`Q = 2`，`query[] = {4, 2}`
> **输出：** `4 2`

## 简单方法

最简单的方法是遍历数组直到每个查询的 `query[i]`，并从数组的末尾和开始计算总和。最后，打印所得总和的最小值。
...
上述方法可以通过使用额外空间和前缀和与后缀和的概念进行优化，并以恒定的计算复杂度回答每个查询。其思想是对每个索引的前缀和后缀总和进行预处理。按照以下步骤解决问题：

*   初始化两个变量，说 `prefix` 和 `suffix`。
*   初始化一个无序映射，比如说 `mp`，将数组元素作为键映射成对，其中第一个值给出前缀和，第二个值给出后缀和。
*   遍历数组并继续添加 `arr[i]` 为前缀，以 `arr[i]` 为键，`prefix` 为值存储在地图中。
*   反向遍历数组并不断添加 `arr[i]` 为后缀，以 `arr[i]` 为键，以 `suffix` 为值存储在地图中。
*   现在遍历数组 `query[]`，对于每个查询 `query[i]`，打印 `mp[query[i]].first` 和 `mp[query[i]].second` 的最小值作为结果。
