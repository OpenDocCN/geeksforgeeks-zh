...
> 原文：[https://www.geeksforgeeks.org/nth-number-in-a-set-of-multiples-of-a-b-or-c/](https://www.geeksforgeeks.org/nth-number-in-a-set-of-multiples-of-a-b-or-c/)

给定四个整数 `N`、`A`、`B` 和 `C`。任务是打印包含 `A`、`B` 或 `C` 倍数的集合中的第 `N` 号。
...
**天真的方法：** 从 `1` 开始遍历，直到找到可被 `A`、`B` 或 `C` 整除的第 `N` 个元素。

**有效方法：** 给定一个数，我们可以求出 `A`、`B` 或 `C` 的除数。现在，[二分搜索法](https://www.geeksforgeeks.org/binary-search/)可以用来找第 `N` 个数，这个数可以被 `A`、`B` 或者 `C` 整除。
