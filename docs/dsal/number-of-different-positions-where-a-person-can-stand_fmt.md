...
> 原文: [https://www.geeksforgeeks.org/number-of-different-positions-where-a-person-can-stand/](https://www.geeksforgeeks.org/number-of-different-positions-where-a-person-can-stand/)
...
我们对每一项进行迭代，检查是否适合条件 `a <= i-1` 和 `n-i <= b` (对于 `i` 从 1 到 `n`)。第一个条件可以转换成 `a+1 <= i`，条件 `n-i <= b` 在 `n-b <= i`，那么一般条件可以写成 `max(a+1, n-b) <= i`，然后我们的答案就可以用公式 `n - max(a+1, n-b) + 1` 来计算了。
