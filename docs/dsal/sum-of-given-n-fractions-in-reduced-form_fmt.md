...
> 原文: [https://www.geeksforgeeks.org/sum-of-given-n-fractions-in-reduced-form/](https://www.geeksforgeeks.org/sum-of-given-n-fractions-in-reduced-form/)

给定两个长度为 `N` 的[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `arr1[]` 和 `arr2[]`，它们分别包含 `N` 个分数的分子和分母，任务是以简化形式找到给定的 `N` 个分数的和。
...
*   查找存储在 `arr2[]` 中的所有分母的[最小公倍数(LCM)](https://www.geeksforgeeks.org/program-to-find-lcm-of-two-numbers/)。
