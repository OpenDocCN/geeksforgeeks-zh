...
> 原文: [https://www.geeksforgeeks.org/kth-lexicographically-smallest-distinct-string-from-given-array-of-strings/](https://www.geeksforgeeks.org/kth-lexicographically-smallest-distinct-string-from-given-array-of-strings/)

给定一个具有 `N` 个字符串和一个整数 `K` 的数组 `arr`，任务是找到字典序上最小的第 `K` 个不同字符串。如果不存在，则打印空字符串。
...
1.  对给定的字符串数组排序。
2.  创建一个映射来存储每个字符串的频率。
3.  现在，遍历映射，每次找到一个频率为 1 的字符串时，减少 `K` 的值。
