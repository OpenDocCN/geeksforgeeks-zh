...
> 原文: [https://www.geeksforgeeks.org/sum-subsets-set-formed-first-n-natural-numbers/](https://www.geeksforgeeks.org/sum-subsets-set-formed-first-n-natural-numbers/)
...
一个**高效的解决方案**是基于从 1 到 `n` 的每个数字恰好出现 `2^(n-1)` 次的事实。所以我们需要的和是 `(1 + 2 + 3 + ... + n) * 2^(n-1)`。总和可以写成 `(n * (n + 1) / 2) * 2^(n-1)`。
