...
> 原文: [https://www.geeksforgeeks.org/construct-smallest-n-digit-number-possible-by-placing-digits-at-positions-specified-by-given-array/](https://www.geeksforgeeks.org/construct-smallest-n-digit-number-possible-by-placing-digits-at-positions-specified-by-given-array/)
...
> **输入:** `N = 3`，`arr[]={{0，7}，{2，2}}`
> **输出:** `702`
> **说明:** 根据条件，第 1 位和第 3 位数字应等于 7 和 2。编号应为 `7_2` 的形式。因此，最小可能整数是 `702`。
>
> **输入:** `N = 3`，`arr[]={{1，1}，{1，3}}`
> **输出:** 否

**天真方法:** 由于位数总是从 `0` 到 `9` 的，最简单的方法是根据给定的条件放置位数，检查超过 `0` 且小于 `10^N` 的每个整数的[组合](https://www.geeksforgeeks.org/permutation-and-combination/)。
...
*   如果需要放置的第一个数字是 `0`，则打印`"No"`，因为数字不能包含前导零。
*   [遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]`，并在`地图`中的相应索引处插入数字。
*   运行从 `1` 到 `N` 的循环，对于每个 `i`，检查在`地图`中的 `i` 位置是否分配了一个数字。如果出现在`地图`中，将其添加到答案中。
