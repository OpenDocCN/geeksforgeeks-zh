...
> 原文: [https://www.geeksforgeeks.org/find-prime-factors-of-array-elements-whose-sum-of-exponents-is-divisible-by-k/](https://www.geeksforgeeks.org/find-prime-factors-of-array-elements-whose-sum-of-exponents-is-divisible-by-k/)

给定一个由正整数 `N` 和整数 `K` 组成的数组 `arr[]`，任务是创建一组素数，使得所有数组元素的素数因式分解中素数的所有幂的和可被 `K` 整除。
...
想法是找到所有小于或等于数组最大元素 `arr[]` 的素数。对于每个素数，计算它除以数组元素的次数。如果计数值可被 `K` 整除，则将质数插入结果集中。在集合的最后打印元素。
...
优化上述方法的思路是预先计算所有数字的所有质因数的计数。以下是步骤:

1.  创建最小素因子分解数组 `spf[]` 直到数组中的最大数。这一步用于预先计算一个数的质因数。
2.  遍历给定的数组 `arr[]`，对于每个元素，找出存储在 `spf[]` 数组中的所有因子的总数。
3.  对于上述步骤中质数的每次幂和，将其频率存储在图中。
