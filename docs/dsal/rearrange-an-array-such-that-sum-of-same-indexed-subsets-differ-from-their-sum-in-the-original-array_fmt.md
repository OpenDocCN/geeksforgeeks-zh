...
> 原文：[https://www.geeksforgeeks.org/rearrange-an-array-such-that-sum-of-same-indexed-subsets-differ-from-their-sum-in-the-original-array/](https://www.geeksforgeeks.org/rearrange-an-array-such-that-sum-of-same-indexed-subsets-differ-from-their-sum-in-the-original-array/)
...
*   现在，遍历排序后的顺序，将每个元素插入其下一个较大元素的原始索引处（即索引 `v[(i + 1) % n].second` 处）。这确保了除了一个索引之外的每个索引现在都有一个比存储在其中的前一个值更小的元素。

> **证明:**
> 让 `S = { arr_1, arr_2, …, arr_k }` 成为子集。
