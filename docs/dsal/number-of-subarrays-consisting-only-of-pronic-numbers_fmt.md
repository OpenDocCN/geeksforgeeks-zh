...
> 原文: [https://www.geeksforgeeks.org/number-of-subarrays-consisting-only-of-pronic-numbers/](https://www.geeksforgeeks.org/number-of-subarrays-consisting-only-of-pronic-numbers/)

给定一个由 `N` 个正整数组成的数组 `arr[]`，任务是计算仅由[个复数](https://www.geeksforgeeks.org/check-given-number-pronic/)组成的[个子数组](https://www.geeksforgeeks.org/subarraysubstring-vs-subsequence-and-programs-to-generate-them/)的数量。
...
**时间复杂度:** `O(√M * N^3)`，其中 `M` 是数组中出现的[最大元素](https://www.geeksforgeeks.org/c-program-find-largest-element-array/)
**辅助空间:** `O(N)`
...
*   初始化一个变量，比如说 `count`，存储[子阵](https://www.geeksforgeeks.org/generating-subarrays-using-recursion/)的总计数；初始化一个变量 `C`，存储连续数组元素的计数，这些元素是[字根](https://www.geeksforgeeks.org/check-given-number-pronic/)。
*   [遍历给定数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]`，并执行以下步骤:
    *   如果当前元素 `arr[i]` 是一个**音位数字**，那么将 `C` 增加 `1`。
    *   否则，将 `count` 增加 `C * (C - 1) / 2` 来计数具有 `C` 元素的子阵数量，并将 `C` 更新为 `0`。
*   增加 `count` 的值，将 `count` 计为 `C * (C - 1) / 2`。
...
**时间复杂度:** `O(N*sqrt(M))`，其中 `M` 是数组中出现的[最大元素](https://www.geeksforgeeks.org/c-program-find-largest-element-array/)。
**辅助空间:** `O(1)`