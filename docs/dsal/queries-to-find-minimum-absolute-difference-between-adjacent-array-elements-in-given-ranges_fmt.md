...
> 原文: [https://www.geeksforgeeks.org/queries-to-find-minimum-absolute-difference-between-adjacent-array-elements-in-given-ranges/](https://www.geeksforgeeks.org/queries-to-find-minimum-absolute-difference-between-adjacent-array-elements-in-given-ranges/)

给定一个由 `N` 个整数组成的[数组](https://www.geeksforgeeks.org/introduction-to-arrays/)`arr[]`和一个由形式为 `{L, R}` 个查询组成的数组`queries[]`，每个查询的任务是找到在范围`[L, R]`内相邻元素之间的绝对差的最小值。

**示例:**

> **输入:** `arr[] = {2, 6, 1, 8, 3, 4}`, `queries[] = {{0, 3}, {1, 5}, {4, 5}}`
> **输出:**
> 4
> 1
> 1
...
> **输入:** `arr[] = [10, 20, 1, 1, 5]`, `queries[] = [0, 1], [1, 4], [2, 3]`
> **输出:**
> 10
> 0
> 0
...
上述方法也可以通过使用[稀疏表](https://www.geeksforgeeks.org/sparse-table/)进行优化，该稀疏表支持恒定时间内的查询 `O(1)` 带额外空间 `O(N log N)`。不通过原件 `arr[]` 通过 `diff[]` 得到需要的答案。按照以下步骤解决问题:

1.  初始化一个[全局数组](https://www.geeksforgeeks.org/initialization-global-static-variables-c/) `lookup[][]` 为稀疏数组。
2.  定义一个[函数](https://www.geeksforgeeks.org/functions-in-c/) `preprocess(arr, N)` ，并执行以下操作:
    *   使用变量 `i` 迭代范围 `[0, N]`，并将 `lookup[i][0]` 的值设置为 `i` 。
    *   [嵌套使用变量 `j` 和 `i` 迭代范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[1, N]`，如果 `arr[lookup[i][j-1]]` 小于 `arr[lookup[i+(1<<(j-1))][j-1]`，则将 `lookup[i][j]` 设置为 `lookup[i][j-1]`。
3.  定义函数 `query(int arr[], int L, int M)` 并执行以下操作:
    *   将变量 `j` 初始化为 `(int)log2(R–L+1)`。
...
4.  定义一个[函数](https://www.geeksforgeeks.org/functions-in-c/) `Min_difference(arr, n, q, m)` ，并执行以下操作:
    *   调用函数 `preprocess(arr, n)` 对稀疏数组进行预处理。
    *   [遍历给定的查询数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `Q[]`，函数 `query(arr, L, R–1)` 返回的值给出当前查询的结果。
