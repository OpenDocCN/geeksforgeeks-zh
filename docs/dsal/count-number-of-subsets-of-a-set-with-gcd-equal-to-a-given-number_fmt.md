...
> 原文：[https://www.geeksforgeeks.org/count-number-of-subsets-of-a-set-with-gcd-equal-to-a-given-number/](https://www.geeksforgeeks.org/count-number-of-subsets-of-a-set-with-gcd-equal-to-a-given-number/)
...
The one subset with GCD equal to 3 is {3}.
...
如何获取给定 GCD `i` 的子集数量，其中 `i` 位于从 1 到 `arrMax` 的范围内。想法是使用内置的 `freq`（步骤 2）计算 `i` 的所有倍数。假设有 `add` 个 `i` 的倍数。所有可能的具有 `add` 个数的子集的数量将是 `pow(2, add) – 1`，不包括空集。例如，如果给定的数组是 `{2, 3, 6}` 并且 `i = 3`，则有 3 的 2 个倍数（3 和 6）。因此，将有 3 个子集 `{3}`、`{3, 6}` 和 `{6}` 的 `i` 倍数为 GCD。这些子集还包括 `{6}`，它没有 3 作为 GCD，而是 3 的倍数。所以我们需要减去这样的子集。我们将每个 GCD 的子集计数存储在另一个哈希映射 `subsets` 中。假设 `sub` 是具有多重 `i` 作为 GCD 的子集的数量。`i` 的任何倍数的 `sub` 值可以直接从 `subsets[]` 获得，因为我们正在评估从 `arrMax` 到 1 的计数。
...
扩展上述解决方案，使所有计算都在模 1000000007 下进行，以避免溢出。
