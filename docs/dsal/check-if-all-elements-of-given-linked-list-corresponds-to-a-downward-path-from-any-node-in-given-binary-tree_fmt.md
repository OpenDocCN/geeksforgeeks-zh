...
> 原文: [https://www.geeksforgeeks.org/check-if-all-elements-of-given-linked-list-corresponds-to-a-downward-path-from-any-node-in-given-binary-tree/](https://www.geeksforgeeks.org/check-if-all-elements-of-given-linked-list-corresponds-to-a-downward-path-from-any-node-in-given-binary-tree/)
...
借助[二叉树](https://www.geeksforgeeks.org/binary-tree-data-structure/)的 [DFS 遍历](https://www.geeksforgeeks.org/dfs-traversal-of-a-tree-using-recursion/)可以解决给定的问题。在 DFS 遍历过程中，如果二叉树的任意节点等于[链表](https://www.geeksforgeeks.org/data-structures/linked-list/)的头部，则可以调用[递归函数](https://www.geeksforgeeks.org/recursive-functions/) `isPathUtil()` 递归检查链表的其他元素是否也作为从该节点开始的路径存在。如果已经遍历了完整的链表，则存在有效的所需路径，因此返回 `true`。否则，返回 `false`。按照以下步骤解决给定的问题:

*   声明一个函数，说 `isSubPathUtil(root, head)`，在这个[函数](https://www.geeksforgeeks.org/functions-in-c/)中执行以下步骤:
    *   如果 `root` 为 `NULL`，则返回 `false`。
    *   如果 `head` 为 `NULL`，则返回 `true`。
    *   如果当前根节点的值与 LL 的当前头的值相同，则[递归调用](https://www.geeksforgeeks.org/recursion/)为 `isSubPathUtil(root->left, head->next)` 和 `isSubPathUtil(root->right, head->next)`。如果这些递归调用之一返回的值为真，则返回真。否则，返回 `false`。
*   声明一个函数，说 `isSubPath(root, head)`，在这个函数中执行以下步骤:
    *   如果 `root` 为 `NULL`，则返回 `false`。
    *   如果 `head` 为 `NULL`，则返回 `true`。
    *   如果当前根节点的值与 LL 的当前头的值相同，则递归调用 `isSubPath(root->left, head->next)` 和 `isSubPath(root->right, head->next)`，如果这些递归调用之一返回的值为真，则返回真。否则，递归调用 `isSubPath(root->left, head)` 和 `isSubPath(root->right, head)` 的返回值。
*   完成上述步骤后，如果函数返回的值为 `true`，则打印 `Yes`。否则，打印 `No`。
