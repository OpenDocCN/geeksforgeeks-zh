...
> 原文: [https://www.geeksforgeeks.org/python-program-to-reverse-a-stack/](https://www.geeksforgeeks.org/python-program-to-reverse-a-stack/)
...
这个方法在栈底追加元素，`BottomInsertion` 接受两个值作为参数第一个是 `stack`，第二个是 `elements`，这是一个递归方法。
...
def BottomInsert(s, value):
    ...
    # if stack is not empty then execute else block
        ...
        # remove the element and store it to popped  
        popped = s.pop()

        # invoke it self and pass stack and value as an argument.
...
方法是对栈的元素进行反转，这个方法接受栈作为参数 `Reverse()` 也是一个 `Recursive()` 函数。`Reverse()` 是调用 `BottomInsertion()` 方法来完成堆栈上的反向操作。
...
def Reverse(s):
    ...
    # if stack is not empty then 
        ...
        # call BottomInsert() method and pass stack and popped element as an argument
