...
吊杆编号是仅由数字 2 和 3 组成的编号。给定一个整数 k，找到第 k 个吊杆编号。
...
**参考:** [http://www.spoj.com/problems/TSHOW1/](http://www.spoj.com/problems/TSHOW1/)

思路很简单，就像[生成二进制数](https://www.geeksforgeeks.org/interesting-method-generate-binary-numbers-1-n/)一样。这里我们也使用同样的方法，我们使用队列数据结构来解决这个问题。首先入队 `"2"`，然后入队 `"3"`，这两个分别是第一个和第二个吊杆编号。现在设置 `count=2`，每次在队列前弹出 `()` 并在弹出数中追加 `"2"` 并增加计数 `++`，如果 `count==k` 然后打印当前吊杆号，否则在弹出数中追加 `"3"` 并增加计数 `++`，如果 `count==k` 然后打印当前吊杆号。重复该过程，直到我们到达第 K 个吊杆编号。
