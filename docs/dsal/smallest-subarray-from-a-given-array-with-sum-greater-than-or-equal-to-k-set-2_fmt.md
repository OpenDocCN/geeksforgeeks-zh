...
> 原文: [https://www.geeksforgeeks.org/smallest-subarray-from-a-given-array-with-sum-greater-than-or-equal-to-k-set-2/](https://www.geeksforgeeks.org/smallest-subarray-from-a-given-array-with-sum-greater-than-or-equal-to-k-set-2/)

给定一个由 `N` 个正整数和一个整数 `K` 组成的数组 `A[]`，任务是找出和大于等于 `K` 的最小子阵的长度。如果没有这样的子阵列，打印 `-1`。
