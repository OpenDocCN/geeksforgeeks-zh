...
> 原文：[https://www.geeksforgeeks.org/check-grid-can-become-row-wise-column-wise-sorted-adjacent-swaps/](https://www.geeksforgeeks.org/check-grid-can-become-row-wise-column-wise-sorted-adjacent-swaps/)
...
做上面这道题的思路真的很简单。我们可以简单的对同一行的字符进行排序，然后只需检查列是否新网格是否排序。请不要以为相邻互换可以排序（[冒泡排序](https://www.geeksforgeeks.org/bubble-sort/)例如只做相邻互换）。
...
// Driver code
let v = ["ebcda", "ihgfj", "klmno", "pqrst", "yvwxu" ];
