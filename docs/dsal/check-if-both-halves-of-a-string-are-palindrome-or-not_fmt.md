...
> 原文: [https://www.geeksforgeeks.org/check-if-both-halves-of-a-string-are-palindrome-or-not/](https://www.geeksforgeeks.org/check-if-both-halves-of-a-string-are-palindrome-or-not/)
...
*   通过以下两种情况同时检查两半是否回文:
    *   如果 `S[i]` 与 `S[N/2–1–i]` 不相等，则前半部分不回文。
    *   如果 `S[N/2+i]` 不等于 `S[N–1–i]`，则后半部分不回文。
