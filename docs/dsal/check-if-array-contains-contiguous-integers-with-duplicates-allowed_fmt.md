...
> 原文：[https://www.geeksforgeeks.org/check-if-array-contains-contiguous-integers-with-duplicates-allowed/](https://www.geeksforgeeks.org/check-if-array-contains-contiguous-integers-with-duplicates-allowed/)

给定一个由`N`个整数组成的数组`Arr`（允许重复）。如果是一组连续的整数，则打印“Yes”，否则打印“No”。
...
5.  再次遍历循环，检查是否有任何元素是正的，这意味着元素的差值大于 1。断开循环并打印“No”。
6.  如果没有发现阳性元素，打印“Yes”。
