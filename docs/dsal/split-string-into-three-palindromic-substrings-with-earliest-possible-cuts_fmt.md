...
> 原文：[https://www.geeksforgeeks.org/split-string-into-three-palindromic-substrings-with-earliest-possible-cuts/](https://www.geeksforgeeks.org/split-string-into-three-palindromic-substrings-with-earliest-possible-cuts/)
...
3.  与第一步类似，从给定字符串的最后一个索引 `str` 生成所有的子字符串，并检查给定的子字符串是否是回文。如果任何子串作为子串出现，那么将子串的最后一个索引存储在向量 `lastPal` 中，其中 `lastPal` 将存储第三个回文，从存储的值开始，到给定串`字符串`的`(N–1)`第个索引结束。
