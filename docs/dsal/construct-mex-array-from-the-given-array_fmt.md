...
> 原文：[https://www.geeksforgeeks.org/construct-mex-array-from-the-given-array/](https://www.geeksforgeeks.org/construct-mex-array-from-the-given-array/)

给定一个具有 `N` 个不同正元素的数组 `arr[]`，任务是生成另一个数组 `B[]`，使得对于数组 `arr[]` 中的每个第 `i` 个索引，`B[i]` 是从 `arr[]` 中缺失的最小正数，不包括 `arr[i]`。
...
解决这个问题最简单的方法就是遍历数组 `arr[]`。对于每个索引 `i`，初始化一个数组 `hash[]`。对于每个索引 `j`（其中 `j <= i`），更新 `hash[arr[j]] = 1`。现在从索引 `1` 遍历数组 `hash[]`，找到 `hash[k] = 0` 的最小索引 `k`，更新 `B[i] = k`。最后，完成上述步骤后，打印数组 `B[]`。
...
对上述途径进行优化，思路是计算数组 `arr[]` 的 `MEX`。遍历数组 `arr[]`。如果 `arr[i]` 小于数组 `arr[]` 的 `MEX`，那么排除该元素的 `MEX` 将是 `arr[i]` 本身。如果数组 `arr[]` 的 `arr[i]` 大于数组 `MEX`，那么排除后数组的 `MEX` 不会改变。

按照以下步骤解决问题：

1.  初始化一个数组，比如说 `hash[]`，来存储数组 `arr[]` 中是否存在值 `i`。如果存在 `i`，`hash[i] = 1`，否则 `hash[i] = 0`。
