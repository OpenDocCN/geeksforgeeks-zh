...
> 原文: [https://www.geeksforgeeks.org/queries-to-answer-the-number-of-ones-and-zero-to-the-left-of-given-index/](https://www.geeksforgeeks.org/queries-to-answer-the-number-of-ones-and-zero-to-the-left-of-given-index/)

给定一个二进制数组和 `Q` 查询。每个查询都由一个数字 `K` 组成，任务是打印索引 `K` 左边的 1 和 0 的数字。
...
*   迭代数组，每一步用左边的 1 和 0 初始化 `left[i]`。
*   每个查询的 1 的数量将是 `left[k].first`，零的数量将是 `left[k].second`。
