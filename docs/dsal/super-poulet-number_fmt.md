...
**超素数**是基数为 2 的素数(伪素数)，如果每个除数 `D` 除`2^{D} - 2`。
...
**方法:**思路是[生成数字](https://www.geeksforgeeks.org/find-divisors-natural-number-set-1/) `N` 的所有除数，对于所有除数检查 `D` 除`2^{D}-2`。如果所有除数都满足这个条件，那么这个数就是超级波特数。
