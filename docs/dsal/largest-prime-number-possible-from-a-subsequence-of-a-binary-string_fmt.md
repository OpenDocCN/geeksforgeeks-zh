...
> 原文: [https://www.geeksforgeeks.org/largest-prime-number-possible-from-a-subsequence-of-a-binary-string/](https://www.geeksforgeeks.org/largest-prime-number-possible-from-a-subsequence-of-a-binary-string/)
...
解决问题的思路是[生成字符串](https://www.geeksforgeeks.org/print-subsequences-string/)的所有可能的子序列，并将每个子序列转换为其等价的十进制形式。打印由此子序列获得的最大素数。

按照以下步骤解决此问题:

*   分别在 `Pair.first` 和 `Pair.second` 中初始化一个成对的[向量](https://www.geeksforgeeks.org/sorting-vector-of-pairs-in-c-set-1-sort-by-first-and-second/)，比如说 `vector`，用于存储成对的字符串及其等价的十进制值。
...
*   迭代一个循环，从 `i = 0` 到[字符串的长度](https://www.geeksforgeeks.org/c-program-to-find-the-length-of-a-string/) `s`:
    *   迭代一个从 `j = 0` 到 `vec` 长度的循环:
        *   将第 `j` 对储存在 `temp` 下。
        *   如果字符串 `s` 的 `i` 中带有字符的是 `"1"`:
            *   首先在 `temp` 中添加字符
            *   通过[向左移动](https://www.geeksforgeeks.org/left-shift-right-shift-operators-c-cpp/)当前值并添加 `1` 来更新 `temp.second` 的值。
        *   否则:
            *   首先在 `temp` 中添加字符
            *   通过[向左移动](https://www.geeksforgeeks.org/left-shift-right-shift-operators-c-cpp/)当前值并添加 `0` 来更新 `temp.second` 的值。
        *   将此 `temp` 对储存到 `vec` 中。
        *   如果 `temp.second` 是[质数](https://www.geeksforgeeks.org/tag/prime-number/):
            *   在 `ans` 中存储 `ans` 和 `temp.second` 的最大值。
    *   如果 `ans` 等于 `0`:
        *   从字符串 `s` 中得不到质数。
    *   否则:
        *   打印 `ans`。
