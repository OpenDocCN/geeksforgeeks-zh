...
> 原文: [https://www.geeksforgeeks.org/remove-minimum-elements-from-either-side-such-that-2min-becomes-more-than-max-set-2/](https://www.geeksforgeeks.org/remove-minimum-elements-from-either-side-such-that-2min-becomes-more-than-max-set-2/)
...
> **输出:** `4`
> 我们需要去掉 4 个元素 (`4`, `5`, `100`, `200`) 这样 `2*min` 就变得大于 `max` 了。
>
> **输入:** `arr[] = {4, 7, 5, 6}`
> **输出:** `0`
> 我们不需要移除任何元素作为 `4*2 > 7`
...
我们在[上一篇](https://www.geeksforgeeks.org/remove-minimum-elements-either-side-2min-max/)中的 `O(n^3)`、`O(n^2 * logn)` 和 `O(n^2)` 时间中讨论了解决这个问题的各种方式。在本文中，我们将讨论使用[滑动窗口](https://www.geeksforgeeks.org/window-sliding-technique/)和[线段树](https://www.geeksforgeeks.org/segment-tree-set-1-sum-of-given-range/)概念的 `O(n * logn)` 时间解决方案。

1.  为给定输入数组的范围最小值查询和范围最大值查询构造线段树。
2.  取两个指针 `start` 和 `end`，将两者初始化为 `0`。
3.  当 `end` 小于输入数组的长度时，执行以下操作:
    *   使用步骤 1 中构建的线段树在当前窗口中找到 `min` 和 `max`。
    *   检查 `2 * min <= max`，如果是，则递增 `start` 指针，否则更新到目前为止的最大有效长度（如果需要）。
    *   增量 `end`。
4.  `length(arr[]) - maxValidLength` 为必选项。
...
```csharp
// Function to return the minimum number of elements to remove
// such that the remaining array has no invalid window.
public static int removeMinElements(int[] a)
{
    int n = a.Length;
    RangeMinimumQuery rMimQ = new RangeMinimumQuery();
    int[] minTree = rMimQ.createSegmentTree(a);
...
class RangeMinimumQuery
{
    // Creates a new segment tree from the given input array
    public int[] createSegmentTree(int[] input)
    {
        int n = input.Length;
        int segTreeSize = 2 * getNextPowerOfTwo(n) - 1;
    ...
    public void createSegmentTreeUtil(int[] segmentTree, int[] input, int low, int high, int pos)
    {
        if (low == high)
        {
            // Its a leaf node
    ...
    static int rangeMinimumQueryUtil(int[] segmentTree, int low, int high, int from, int to, int pos)
    {
        // Total overlap
        if (from <= low && to >= high)
        {
...
class RangeMaximumQuery
{
    ...
    public int rangeMaximumQueryUtil(int[] segmentTree, int low, int high, int from, int to, int pos)
    {
        // Total overlap
        if (from <= low && to >= high)
        {
...
// Function to return the minimum power of 2 which is greater than n
static int getNextPowerOfTwo(int n)
{
    int logPart = (int)Math.Ceiling(Math.Log(n) / Math.Log(2));
    return (int)Math.Pow(2, logPart);
}
