...
> 原文: [https://www.geeksforgeeks.org/find-the-gcd-that-lies-in-given-range/](https://www.geeksforgeeks.org/find-the-gcd-that-lies-in-given-range/)

给定两个正整数 `a` 和 `b` 和一个范围 `[low, high]`。任务是找出在给定范围内的 `a` 和 `b` 的最大公约数。如果范围内没有除数，打印 -1。
