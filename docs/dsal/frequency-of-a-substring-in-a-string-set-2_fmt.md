...
## 天真的做法:
参考[之前的帖子](https://www.geeksforgeeks.org/frequency-substring-string/)最简单的解决问题的方法。
**时间复杂度:** `O(N*M)`
**辅助空间:** `O(1)`

## 使用 [KMP 算法的方法:](https://www.geeksforgeeks.org/searching-for-patterns-set-2-kmp-algorithm/)
参考本文[之前的帖子](https://www.geeksforgeeks.org/frequency-substring-string/)使用 [KMP 算法](https://www.geeksforgeeks.org/kmp-algorithm-for-pattern-searching/)解决问题。
**时间复杂度:** `O(N + M)`
**辅助空间:** `O(M)`

## 使用[正则表达式](https://www.geeksforgeeks.org/write-regular-expressions/)的方法:
按照以下步骤解决问题:

*   使用[正则表达式()函数](https://www.geeksforgeeks.org/regex-regular-expression-in-c/)形成字符串`模式`的正则表达式。
*   使用功能 `smatch()` 创建一个 `smatch` `M` 。
*   使用函数 `regex_match()` 检查字符串`字符串`中字符串`模式`是否存在，如下所示:
...
*   创建一个变量`numberOfMatches`[数据类型](https://www.geeksforgeeks.org/c-data-types/) `ptrdiff_t` 来存储出现次数。
*   使用函数 `regex_iterator()` 函数如下:
