...
> 原文: [https://www.geeksforgeeks.org/sum-of-every-kth-prime-number-in-an-array/](https://www.geeksforgeeks.org/sum-of-every-kth-prime-number-in-an-array/)

给定一个整数数组（小于 `10^6`），任务是找出出现在每个（`k-1`）素数之后的所有素数的和，即数组中的每个第 `k` 个素数。
...
高效方法
我们将创建一个存储一个数是否是质数的筛子。然后，它可以用在 `O(1)` 时间内检查一个数字是否符合质数。这样，我们只需要跟踪每一个第 `K` 个素数，并保持运行和。
