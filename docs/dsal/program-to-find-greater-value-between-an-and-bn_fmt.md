...
> 原文：[https://www.geeksforgeeks.org/program-to-find-greater-value-between-an-and-bn/](https://www.geeksforgeeks.org/program-to-find-greater-value-between-an-and-bn/)

给定三个整数 `a`、`b` 和 `n`，任务是找出 `a^n` 和 `b^n` 之间的较大值。

**例：**

> **输入：** `a = 3`，`b = 4`，`n = 5`
> **输出：** `b^n` 大于 `a^n`
> 值 `a^n` 为 243，`b^n` 为 1024。
> 所以，`b^n` 大于 `a^n`。
> **输入：** `a = -3`，`b = 2`，`n = 4`
> **输出：** `a^n` 大于 `b^n`
> `a^n` 值为 243，`b^n` 值为 16。
> 所以，`a^n` 大于 `b^n`。

## 基本方法

对于 `a`、`b` 和 `n` 的每个值，计算 `a^n` 和 `b^n` 的值。然后比较获得的结果，并根据输出显示。
当 `a`、`b`、`n` 的值较大时，就会出现这种方法的问题，对于 `a`、`n` 的值较大时，计算 `a^n` 会超出整数的限制，从而导致整数溢出。
