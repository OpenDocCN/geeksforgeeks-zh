...
> 原文: [https://www.geeksforgeeks.org/number-of-binary-strings-of-length-n-with-k-adjacent-set-bits/](https://www.geeksforgeeks.org/number-of-binary-strings-of-length-n-with-k-adjacent-set-bits/)

给定 `n` 和 `k`。任务是从 $2^n$ 中找出长度为 `n` 的二进制串的数量，使得它们满足 `f(位串) = k`。
其中，
...
### 方法 2(高效)
在方法 1 中，有重叠的子问题需要移除，对此我们可以应用动态规划(memoicing)。
