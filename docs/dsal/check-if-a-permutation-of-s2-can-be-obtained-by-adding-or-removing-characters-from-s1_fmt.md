...
> 原文: [https://www.geeksforgeeks.org/check-if-a-permutation-of-s2-can-be-obtained-by-adding-or-removing-characters-from-s1/](https://www.geeksforgeeks.org/check-if-a-permutation-of-s2-can-be-obtained-by-adding-or-removing-characters-from-s1/)

给定由 `N` 和 `M` 字符组成的两个[字符串](https://www.geeksforgeeks.org/string-data-structure/) `S1` 和 `S2`，任务是在从字符串 `S1` 中添加或删除一个字符素数次后，检查字符串 `S1` 是否可以等于 `S2` 的任意排列。如果可能，则打印`“是”`。否则，打印`“否”`。
