...
> 原文: [https://www.geeksforgeeks.org/average-odd-numbers-till-given-odd-number/](https://www.geeksforgeeks.org/average-odd-numbers-till-given-odd-number/)
...
我们可以通过将每个奇数相加到 `n`，然后将总和除以计数来计算平均值。以下是实施办法。
...
奇数的平均值只能通过单步求出，使用以下公式 `(n+1)/2`，其中 `n` 是最后一个奇数。
