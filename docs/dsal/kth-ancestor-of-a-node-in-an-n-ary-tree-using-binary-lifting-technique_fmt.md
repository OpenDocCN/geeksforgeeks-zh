...
> 原文: [https://www.geeksforgeeks.org/kth-ancestor-of-a-node-in-an-n-ary-tree-using-binary-lifting-technique/](https://www.geeksforgeeks.org/kth-ancestor-of-a-node-in-an-n-ary-tree-using-binary-lifting-technique/)

给定一个 N 元树的顶点 `V` 和一个整数 `K`，任务是打印树中给定顶点的第 `K` 祖先。如果不存在这样的祖先，那么打印 `-1`。
...
思路是使用[二元提升技术](https://www.geeksforgeeks.org/lca-in-a-tree-using-binary-lifting-technique/)。这种技术基于这样一个事实，即每个整数都可以用二进制形式表示。通过预处理，可以计算出一个稀疏表 `table[v][i]`，该表存储了顶点 `v` 的第 `2^i` 父级，其中 `0 ≤ i ≤ log₂N`。该预处理需要 `O(NlogN)` 时间。

要找到顶点 `V` 的第 `K` 父项，让 `K = b₀b₁b₂…bₙ` 成为二进制表示中的一个 `n` 比特数，让 `p₁, p₂, …, pⱼ` 是位值为 `1` 的指数，那么 `K` 可以表示为 `K = 2^{p₁} + 2^{p₂} + … + 2^{pⱼ}`。因此，要到达 `V` 的第 `K` 父级，我们必须跳至第 `2^{p₁}`，`2^{p₂}`，…，`2^{pⱼ}` 父级。这可以通过先前在 `O(logN)` 中计算的稀疏表来有效地完成。
