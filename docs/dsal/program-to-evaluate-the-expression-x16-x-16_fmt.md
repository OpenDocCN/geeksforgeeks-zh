...
> 原文: [https://www.geeksforgeeks.org/program-to-evaluate-the-expression-x16-x-16/](https://www.geeksforgeeks.org/program-to-evaluate-the-expression-x16-x-16/)

给定一个数字 `X`。任务是为 `X` 的给定值找到下面表达式的值。

**例:**

> **输入** : `X = √2`
> **输出** : `198`
> **解释** : `2[(\sqrt[]{2})^6 + 15 (\sqrt[]{2})^4 + 15\sqrt[]{2})^2 + 1] = 198`
> **输入** : `X = 3`
> **输出** : `4160`

**方法:** 思路是使用二项式表达式。我们可以把这两个项看作两个二项式。通过扩展这些项，我们可以找到期望的和。下面是术语的扩展。

> `(X+1)^6 = {6_C}_0 X^6+6_c_1 X^5+{6_C}_2 X^4+{6_C}_3 X^3+{6_C}_4 X^2+{6_C}_5 X+{6_C}_6`
> `(X-1)^6 = {6_C}_0 X^6-6_c_1 X^5+{6_C}_2 X^4-{6_C}_3 X^3+{6_C}_4 X^2-{6_C}_5 X+{6_C}_6`
> `(X+1)^6+(X-1)^6 =2[{6_C}_0 X^6+{6_C}_2 X^4+{6_C}_4 X^2+{6_C}_6]`
> `(X+1)^6+(X-1)^6=2[X^6 + 15 X^4 + 15 X^2 +1] ---EQ(1)`
