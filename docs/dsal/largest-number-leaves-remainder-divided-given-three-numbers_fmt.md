...
> 原文: [https://www.geeksforgeeks.org/largest-number-leaves-remainder-divided-given-three-numbers/](https://www.geeksforgeeks.org/largest-number-leaves-remainder-divided-given-three-numbers/)
...
这个想法是基于这样一个事实，如果一个数剩下相同的余数 `a`，`b` 和 `c`，那么它将划分它们的差异。让我们理解假设 `x` 是我们的结果。设 `a = x*d1 + r` 其中 `r` 是 `a` 除以 `x` 的余数，同样我们可以写出 `b = x*d2 + r` 和 `c = x*d3 + r`。这里的逻辑是，我们首先找到所有三对的差，然后找到差的最大公约数，以使结果最大化。
