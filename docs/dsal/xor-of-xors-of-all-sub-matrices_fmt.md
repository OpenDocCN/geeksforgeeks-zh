...
> 原文: [https://www.geeksforgeeks.org/xor-of-xors-of-all-sub-matrices/](https://www.geeksforgeeks.org/xor-of-xors-of-all-sub-matrices/)
...
一个**简单的方法**是生成所有可能的子矩阵，唯一地找到每个子矩阵的异或，然后把它们全部异或起来。这种方法的时间复杂度为 `O(n^6)`。
...
假设基于 0 的索引，索引 `(R, C)` 所在的子矩阵数等于 `(R + 1)*(C + 1)*(N - R)*(N - C)`。
