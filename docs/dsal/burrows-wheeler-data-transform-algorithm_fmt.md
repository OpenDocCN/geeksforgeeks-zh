...
*   **第二步:** 下一步是按字典顺序对旋转进行排序。**“`$`”**符号在词典中被视为第一个字母，甚至在**‘`a`’**之前。
...
## 为什么是“`$`”符号嵌入在文本中？
即使我们的文本没有与任何 EOF 字符（**‘`$`’**）连接，我们也可以计算 BWT。**“`$`”**符号在计算 BWT 逆时出现。

## 实施方式
1.  让我们实例化**“`banana$`”**作为我们的 `input_text` 并实例化字符数组 `bwt_arr` 作为我们的输出。
2.  让我们得到**“`banana$`”**的所有后缀并计算它的 `suffix_arr` 来存储每个后缀的索引。
...
3.  迭代 [`suffix_arr`](https://www.geeksforgeeks.org/suffix-array-set-1-introduction/) ，现在让我们添加到我们的输出数组 `bwt_arr` ，每个旋转的最后一个字符。
4.  `input_text` 从后缀数组中当前值表示的位置开始的每次旋转的最后一个字符可以用 `input_text[(suffix_arr[i]–1+n)% n]` 来计算，其中 `n` 是 `suffix_arr` 中的元素数量。
...
## 时间复杂度
O( n^2 Logn)。这是因为上面用于构建后缀数组的方法具有 O( n^2 Logn) 时间复杂度，这是由于 O(nLogn) 排序算法中字符串比较的 O(n) 时间。
