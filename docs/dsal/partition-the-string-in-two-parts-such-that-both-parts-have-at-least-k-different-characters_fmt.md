...
> 原文：[https://www.geeksforgeeks.org/partition-the-string-in-two-parts-such-that-both-parts-have-at-least-k-different-characters/](https://www.geeksforgeeks.org/partition-the-string-in-two-parts-such-that-both-parts-have-at-least-k-different-characters/)

给定一串小写英文字母和一个整数 `0 < K <= 26`。任务是将字符串分成两部分（并打印它们），使得两部分都至少有 `k` 个不同的字符。如果有多个可能的答案，打印左边部分最小的那个。如果没有这样的答案，打印 “Not Possible”。

**示例：**

> **输入：** `str = "geeksforgeeks"`，`k = 4`
> **输出：** `geeks`，`forgeeks`
> 字符串可以分为“geeks”和“forgeeks”两部分。由于“geeks”有四个不同的字符‘g’、‘e’、‘k’和‘s’，并且这是最小的左边部分，“forgeeks”也至少有四个不同的字符。
> **输入：** `str = "aaababb"`，`k = 2`
> **输出：** `Not Possible`
