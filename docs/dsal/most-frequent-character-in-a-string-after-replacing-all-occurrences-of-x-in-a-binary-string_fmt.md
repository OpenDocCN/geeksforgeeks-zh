...
> 原文: [https://www.geeksforgeeks.org/most-frequent-character-in-a-string-after-replacing-all-occurrences-of-x-in-a-binary-string/](https://www.geeksforgeeks.org/most-frequent-character-in-a-string-after-replacing-all-occurrences-of-x-in-a-binary-string/)

## 问题描述

给定一个由 `1`、`0` 和 `X` 组成的长度为 `N` 的[字符串](https://www.geeksforgeeks.org/string-data-structure/)，任务是在每次出现 `X` 后，以最大频率打印[字符](https://www.geeksforgeeks.org/return-maximum-occurring-character-in-the-input-string/)（`"1"` 或 `"0"`）：
...
**输出：** `1`
**解释：**
操作 1: `S = "X11001100X1XX"`
操作 2: `S = "111001100X1XX"`
无法再进行替换。
...
*   位于 `1` 和 `0` 之间（如 `1XX0`）的所有 `X` 都没有意义，因为 `1` 和 `0` 都不能转换。
*   位于 `0` 和 `1` 之间（如 `0XXX1`）的所有 `X` 也没有意义，因为它对 `1` 和 `0` 的贡献相等。考虑 `"0X…X1"` 形式的任意子串，然后从字符串的开始到结束改变[第一次出现 `X`](https://www.geeksforgeeks.org/find-first-and-last-positions-of-an-element-in-a-sorted-array/) 后，子串中 `0` 和 `1` 的实际出现频率保持不变。

从以上观察可以得出结论，结果取决于以下条件：

*   原字符串中 `'1'` 和 `'0'` 的计数。
*   在两个连续的 `0` 或两个连续的 `1` 之间出现的 `X` 的频率，即分别为 `"0XX0"` 和 `"1XX1"`。
*   出现在字符串开始处的连续的 `'X'` 的数量，其右端为 `'1'`，即 `"XXXX1…"`。
*   出现在字符串末尾并具有左端 `"0"` 的连续 `"X"` 的数量，即 `"…0XXX"`。

因此，根据上述条件计算 `1` 和 `0` 的数量，并打印结果计数。
