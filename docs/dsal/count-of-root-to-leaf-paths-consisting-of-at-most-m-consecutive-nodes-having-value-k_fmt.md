...
以 [`二叉树`](https://www.geeksforgeeks.org/binary-tree-data-structure/) 的形式给出 [`无环无向图`](https://www.geeksforgeeks.org/detect-cycle-undirected-graph/) ，其根在顶点`1`和值在每个由数组`arr[]`表示的顶点`[1，N]`上，任务是找到根到叶路径的数量，这些路径最多包含`m`个连续节点，其值为`K`。
