...
> 原文: [https://www.geeksforgeeks.org/check-whether-k-th-bit-set-not/](https://www.geeksforgeeks.org/check-whether-k-th-bit-set-not/)

给定一个数字 `n`，检查 `n` 的第 `k` 位是否置位。
...
如果我们将 `n` 右移 `k-1`，如果第 `k` 位设置为 1 则最后一位为 1，否则为 0。
...
```python
# Python3 program to check if k-th bit of
