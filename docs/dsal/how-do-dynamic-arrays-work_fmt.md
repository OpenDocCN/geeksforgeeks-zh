...
当我们试图进行插入并且没有更多空间留给新项目时，动态数组（C++中的`向量`、Java中的`数组列表`）会自动增长。通常面积会翻倍。
...
在下面的代码中，数组将变得满满的，我们将所有元素复制到新的双大小数组（可变大小数组）中。示例代码如下：
...
## 添加元素方法
// function add an element at end of array
```java
public void add(int data)
{
    // check no of element is equql to size of array
