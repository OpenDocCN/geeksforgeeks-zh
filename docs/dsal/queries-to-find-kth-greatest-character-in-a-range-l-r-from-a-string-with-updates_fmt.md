...
给定长度为 `N` 和 `Q` 的字符串 `str`，查询以下两种类型:

1.  `(1 L R K)`: 从索引范围 `[L, R]` (基于 1 的索引) 中找到第 `K` 最伟大的角色 (非独特的)
2.  `(2 J C)`: 用字符 `C` 替换字符串中的第 `J` 字符
...
*   对于每一个类型为 `(1 L R K)` 的查询，从索引 `[L, R]` 的范围中找到 `S` 的[子串](https://www.geeksforgeeks.org/length-of-the-longest-valid-substring/)，并[按照非递增顺序对该子串](https://www.geeksforgeeks.org/substring-sort/)进行排序。打印子字符串中第 `K` 索引处的字符。
*   对于类型为 `(2 J C)` 的每个查询，将 `S` 中的第 `J` 字符替换为 `C`。

**时间复杂度:** `O ( Q * ( N log(N))`，其中 `N logN` 是排序每个子串的计算复杂度。
**辅助空间:** `O(N)`
