...
> 原文: [https://www.geeksforgeeks.org/priority-queue-using-binary-heap/](https://www.geeksforgeeks.org/priority-queue-using-binary-heap/)
...
### 时间复杂度:
除了时间复杂度为 `O(1)` 的 `getMax()` 外，所有操作的时间复杂度均为 `O(log N)`。
