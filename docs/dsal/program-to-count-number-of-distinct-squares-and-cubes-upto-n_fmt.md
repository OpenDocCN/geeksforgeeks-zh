# 计算不超过N的不同正方形和立方体数量的程序

> 原文：[https://www.geeksforgeeks.org/program-to-count-number-of-distinct-squares-and-cubes-upto-n/](https://www.geeksforgeeks.org/program-to-count-number-of-distinct-squares-and-cubes-upto-n/)
...
> 输入：`N = 70`
> 输出：10 个完美正方形或完美立方体的数字 1，4，8，9，16，25，27，36，49，64
> 输入：`N = 25`
> 输出：6 个完美正方形或完美立方体的数字 1，4，8，9，16，25
...
1.  从 1 到 `N` 的方块数为 `floor(sqrt(N))`。
2.  立方体数量从 1 到 `N` 为 `floor(cbrt(N))`。
3.  去掉既有正方形又有立方体的数字（比如 1，64…）通过从中减去 `floor(sqrt(cbrt(N)))`。
