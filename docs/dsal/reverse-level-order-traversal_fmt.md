...
> 原文：[https://www.geeksforgeeks.org/reverse-level-order-traversal/](https://www.geeksforgeeks.org/reverse-level-order-traversal/)

我们在前一篇文章中已经讨论了[层级顺序遍历](https://www.geeksforgeeks.org/level-order-tree-traversal/)。思路是先打印最后一层，再打印倒数第二层，以此类推。像层级顺序遍历一样，每一层都是从左到右打印的。

![Example Tree](img/c8cf26aa3839f4c631be334e5430e6e2.png)

上述树的逆序遍历是“4 5 2 3 1”。

两种方法对于[正常层级顺序遍历](https://www.geeksforgeeks.org/level-order-tree-traversal/)都可以很容易地修改来做反向层级顺序遍历。

## 方法 1（递归函数打印给定层级）

我们可以轻松修改用于正常[层级顺序遍历](https://www.geeksforgeeks.org/level-order-tree-traversal/)的方法 1。在方法 1 中，我们有一个 `printGivenLevel()` 方法，它打印给定的层级号。我们唯一需要改变的是，不是从第一层到最后一层调用 `printGivenLevel()`，而是从最后一层到第一层调用。
