...
> 原文：[https://www.geeksforgeeks.org/reduce-string-by-removing-outermost-parenthesis-from-each-primitive-substring/](https://www.geeksforgeeks.org/reduce-string-by-removing-outermost-parenthesis-from-each-primitive-substring/)
...
1.  初始化一个变量 `count` 来存储**圆括号**的个数，即 `"("`。
2.  如果 `count` 大于 `0`，则在结果中添加每一个 `"("`，即在遇到一个原始子串的第一个 `"("` 之后添加所有 `"("`。
3.  如果 `count` 大于 `0`，则在结果中添加每一个 `')'`，即在遇到一个原始子串的最后一个 `')'` 之前添加所有 `')'`。
4.  最后，打印得到的结果字符串。
