...
> 原文: [https://www.geeksforgeeks.org/modify-given-array-to-make-sum-of-odd-and-even-indexed-elements-same/](https://www.geeksforgeeks.org/modify-given-array-to-make-sum-of-odd-and-even-indexed-elements-same/)

给定大小为`N`的二进制数组`arr[]`，从数组中最多移除`N/2`个元素，使得奇数和偶数索引处的元素之和相等。任务是打印修改后的数组。
...
> **解释:**
> 对于给定数组`arr[] = {1，1，1，0}`
> 奇数位置元素之和，`Odd_Sum`= `arr[1]+arr[3]`= 1+1 = 2。
> 偶位置元素之和，`Even_Sum` = `arr[2] + arr[4]` = 1 + 0 = 1。
> 由于`Odd_Sum` & `Even_Sum`不相等，去掉一些元素使其相等。
...
3.  如果`count_0 ≥ N/2`，则去掉所有的`1s`，打印所有的零`count_0`次。
4.  否则，如果`count_0 < N/2`，通过去掉所有的`0s`，可以使偶数和奇数指数之和按照以下条件相同:
    *   如果`count_1`为奇数，则打印`1`作为新数组的元素`(count_1–1)`次。
    *   否则，打印`1`作为新数组的元素`count_1`的次数。
5.  根据上述条件打印结果数组。
