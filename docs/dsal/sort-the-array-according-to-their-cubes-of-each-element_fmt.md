...
> 原文: [https://www.geeksforgeeks.org/sort-the-array-according-to-their-cubes-of-each-element/](https://www.geeksforgeeks.org/sort-the-array-according-to-their-cubes-of-each-element/)

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是根据每个元素的立方体对数组进行排序。
...
想法是使用内置了排序函数 `sort()` 的 `Comparator` 函数，根据数组元素的立方体对数组进行排序。下面是使用的比较器功能：
