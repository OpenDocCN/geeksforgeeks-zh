...
> 原文: [https://www.geeksforgeeks.org/adaptive-huffman-coding-and-decoding/](https://www.geeksforgeeks.org/adaptive-huffman-coding-and-decoding/)
...
2.  对于固定代码: 作为 `k < 2r` 即 `1 < 2*10`，满足条件 (1)
...
3.  对于树中已经存在的符号 `'a'`。遍历树直到符号 `'a'`，我们得到代码=`"1"`
...
5.  对于固定代码: 当 `k > 2r`，即 `18 > 2*10` 时，满足条件(2)
...
7.  对于固定代码: 作为 `k < 2r` 即 `4 < 2*10`，满足条件 (1)
...
9.  对于固定码: 当 `k > 2r`，即 `22 > 2*10` 时，满足条件(2)
...
10. 交换左子树和右子树的节点，因为树违反了属性
11. 对于树中已经存在的符号 `'a'`。遍历树直到符号 `'a'`，我们得到代码=`"0"`

```
Huffman Code for symbol for 'a' is "0"
```

12. 对于树中已经存在的符号 `'r'`。遍历树直到符号 `'a'`，我们得到代码=`"10"`
...
14. 对于固定代码: 作为 `k < 2r` 即 `11 < 2*10`，满足条件 (1)
...
*   读取给定代码的下一组位并遍历树。我们有 `0` 作为 `NYT` 节点。到达 `NYT` 节点后，读取 `1000` 位。将 `1000` 转换为十进制是 8。如 `8 < r` 满足条件 (1)。
    现在将 `e+1` 位转换为十进制并加 1。
...
*   读取下一组位并遍历树，我们在 `00` 到达 `NYT` 节点。读取 `0001` 的 `e` 位。将 `0001` 转换为十进制是 1。如 `1 < r` 满足条件 (1)。
    现在将 `e+1` 位转换为十进制并加 1。
...
*   读取下一组位并遍历树，我们在 `000` 到达 `NYT` 节点。读取 `e` 位，即 `1011`。将 `1011` 转换为十进制是 11。如 `11 > r` 满足条件(2)。
...
*   读取下一组位并遍历树，我们在 `1100` 到达 `NYT` 节点。读取 `e` 位，即 `0101`。将 `0101` 转换为十进制是 9。如 `9 < r` 满足条件 (1)。
