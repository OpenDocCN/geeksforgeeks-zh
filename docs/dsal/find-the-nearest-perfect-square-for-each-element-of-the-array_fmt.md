...
> 原文: [https://www.geeksforgeeks.org/find-the-nearest-perfect-square-for-each-element-of-the-array/](https://www.geeksforgeeks.org/find-the-nearest-perfect-square-for-each-element-of-the-array/)

给定一个由 `N` 个正整数组成的数组 `arr[]`，任务是为每个数组元素打印最近的完美正方形。
...
*   对于每个数组元素，找到最近的完美正方形。
    *   如果 `N` 是完美正方形，那么打印 `N`。
    *   否则，找到第一个完美方块编号 `> N` 并注意其与 `N` 的区别。
    *   然后，找到第一个完美平方数 `< N`，并注意其与 `N` 的区别。
    *   并打印完美的正方形，得到这两个差值的最小值。
