...
> 原文：[https://www.geeksforgeeks.org/count-divisors-set-bits-quotient-dividing-n/](https://www.geeksforgeeks.org/count-divisors-set-bits-quotient-dividing-n/)
...
所以，随着 `d` 的增加，我们会得到递减的 `q`，所以，我们得到了两个排序的序列，一个用于增加 `d`，另一个用于减少 `q`。现在，观察一下，最初，`d` 的设置位数大于 `q`，但是在一个点之后，`d` 的设置位数小于或等于 `q`。
