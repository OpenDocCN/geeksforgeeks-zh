...
> 原文：`https://www.geeksforgeeks.org/k-th-ancestor-of-a-node-in-binary-tree-set-3/`

给定一个二叉树，其中节点从 `1` 到 `N` 编号。给定一个节点和一个正整数 `K`。我们必须打印二叉树中给定节点的 `Kth` 祖先。如果不存在这样的祖先，则打印 `-1`。

例如下面给定二叉树中的 `2nd` 节点的祖先 `4` `5` 就是 `1`。`3rd` 节点的祖先 `4` 将为 `-1`。

![](img/0b5eaaad45e1f0a3fcf1d9265341611c.png)

**方法：** 首先我们从根中找到给定关键数据的路径，并将它存储到一个 `vector` 中，然后我们简单地返回最后一个向量的第 k 个索引。
