...
> 原文：[https://www.geeksforgeeks.org/root-to-leaf-path-sum-equal-to-a-given-number-in-bst/](https://www.geeksforgeeks.org/root-to-leaf-path-sum-equal-to-a-given-number-in-bst/)

给定一个 `BST` 和一个数字。任务是检查给定的数字是否等于给定的[二叉查找树](https://www.geeksforgeeks.org/binary-search-tree-set-1-search-and-insertion/)中从根到叶路径的所有节点的总和。

![](img/02e3437e5b98b886833bbac172d24b4e.png)

**方法**：想法是从根到所有叶子以自上而下的方式遍历，保持一个 `path[]` 数组来存储当前根到叶子的路径。遍历时，将当前路径的所有节点的数据存储在数组 `path[]` 中。每当到达叶节点时，使用数组 `path[]` 计算当前路径上所有节点的总和，并检查它是否等于给定的总和。
以下是上述办法的实现：
