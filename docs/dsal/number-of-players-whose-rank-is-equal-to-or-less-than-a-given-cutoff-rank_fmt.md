...
> 原文: [https://www.geeksforgeeks.org/number-of-players-whose-rank-is-equal-to-or-less-than-a-given-cutoff-rank/](https://www.geeksforgeeks.org/number-of-players-whose-rank-is-equal-to-or-less-than-a-given-cutoff-rank/)

给定一个由 `N` 个整数和一个表示截止秩的整数 `R` 组成的数组 `arr[]`，任务是计算秩为最多 `R` 的数组元素的数量，使得相等的数组元素被排序相同，并且不同的数组元素根据它们在数组 `arr[]` 中的位置进行排序。
...
利用排序的概念可以解决给定的问题。请按照以下步骤解决此问题:

*   按降序排列给定数组 `arr[]`。
*   初始化两个变量，比如 `rank` 存储数组元素的排序，初始化为 `1`；比如 `count` 存储需要的结果，初始化为 `0`。
*   使用变量 `i` 遍历给定数组 `arr[]`，并执行以下步骤:
    *   如果 `arr[i]` 等于前一个元素，则将与前一个等级相同的等级分配给当前元素。
    *   否则，将 `(count + 1)` 等级的值赋给当前元素。
    *   如果 `rank` 大于 `R`，则中断。否则，将 `count` 增加 `1`。
*   完成以上步骤后，打印 `count` 的数值作为答案。
