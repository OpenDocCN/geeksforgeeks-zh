...
> 原文: [https://www.geeksforgeeks.org/minimum-substring-flips-required-to-convert-a-binary-string-to-another/](https://www.geeksforgeeks.org/minimum-substring-flips-required-to-convert-a-binary-string-to-another/)

给定两个大小分别为 `N` 和 `M` 的[二进制字符串](https://www.geeksforgeeks.org/tag/binary-string/) `S1` 和 `S2`，任务是找到将字符串 `S1` 转换为 `S2` 所需的相等字符的[子字符串的最小反转次数](https://www.geeksforgeeks.org/program-print-substrings-given-string/)。如果无法将字符串 `S1` 转换为 `S2`，则打印 `-1`。
...
*   如果给定字符串 `S1` 和 `S2` 的[长度不相同](https://www.geeksforgeeks.org/5-different-methods-find-length-string-c/)，则打印 `-1`。
