...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-possible-triplet-products-from-given-ranges/](https://www.geeksforgeeks.org/sum-of-all-possible-triplet-products-from-given-ranges/)
...
> **输入:** `A = 1`，`B = 1`，`C = 2`
> **输出:** `3`
> **说明:** 给定表达式的值为: `(1 * 1 * 1 + 1 * 1 * 2) % (10^9 + 7) = 3`。因此，所需输出为 `3`。
>
> **输入:** `A = 10`，`B = 100`，`C = 1000`
> **输出:** `13874027`

**天真方法:** 解决这个问题最简单的方法是生成所有可能的三元组 `(i, j, k)` 并打印所有可能的乘积之和 `(i * j * k) mod (10^9 + 7)`。
...
*   初始化一个变量，比如 `MMI` 存储 `8` 的[模乘逆](https://www.geeksforgeeks.org/multiplicative-inverse-under-modulo-m/)。
*   初始化一个变量，比如说 `M` 来存储 `10^9 + 7` 的值。
