...
> 原文: [https://www.geeksforgeeks.org/split-a-numeric-string-into-fibonacci-sequence/](https://www.geeksforgeeks.org/split-a-numeric-string-into-fibonacci-sequence/)
...
1.  初始化一个[向量](https://www.geeksforgeeks.org/vector-in-cpp-stl/) `seq[]` 来存储**斐波那契序列**。
2.  初始化一个指向字符串当前索引的变量 `pos`（`S`），最初为 `0`。
3.  重复索引 `[pos, length–1]`:
    *   如果 `seq` 的长度小于 `2` 或者当前数字等于 `seq` 的最后两个数字之和，则将数字 `S[pos: i]` 添加到斐波那契序列 `seq` 中。重复索引 `i + 1` 并继续。
    *   如果最后添加的数字 `S[pos: i]` 没有形成斐波那契数列，并且在递归后返回 `false`，则将其从 `seq` 中移除。
    *   否则，结束循环，并在形成斐波那契序列时返回真。
4.  如果 `pos` 超过了 `S` 的长度，那么:
    *   如果序列 `seq` 的长度大于或等于 `3`，则找到斐波那契序列，因此返回真。
    *   否则，斐波那契数列是不可能的，因此返回假。
5.  最后，如果 `seq` 的长度大于或等于 `3`，则打印 `seq` 中的数字作为所需的斐波那契序列，否则打印 `-1`。
