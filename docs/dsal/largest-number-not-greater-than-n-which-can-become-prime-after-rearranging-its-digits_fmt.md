...
> 原文: [https://www.geeksforgeeks.org/largest-number-not-greater-than-n-which-can-become-prime-after-rearranging-its-digits/](https://www.geeksforgeeks.org/largest-number-not-greater-than-n-which-can-become-prime-after-rearranging-its-digits/)
...
生成一个小于或等于给定数字 `N` 的所有质数的列表，这可以使用厄拉多塞的[筛](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)高效地完成。

## 主要步骤

主要思路是检查从 `N` 到 `1` 的所有数字，如果有任何一个数字可以重新洗牌形成质数。找到的第一个这样的数字将是答案。

为此，对每个数字运行一个从 `N` 到 `1` 的循环:
