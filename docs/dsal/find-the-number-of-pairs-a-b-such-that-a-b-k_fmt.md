...
> 原文:[https://www.geeksforgeeks.org/find-the-number-of-pairs-a-b-such-that-a-b-k/](https://www.geeksforgeeks.org/find-the-number-of-pairs-a-b-such-that-a-b-k/)
...
## 简单方法
运行从 `1` 到 `n` 的两个循环，并计算所有对 `(i，j)`，其中 `i % j = K`。该方法的时间复杂度为 `O(n^2)`。

## 有效方法
最初总计`计数= N–K`，因为范围内的所有数字都是 `> K` 除以后会给出 `K` 作为余数。之后，对于所有的 `i > K` 都有精确的`(N–K)/i`数，这些数在除以 `i` 后会给出余数作为 `K`。
