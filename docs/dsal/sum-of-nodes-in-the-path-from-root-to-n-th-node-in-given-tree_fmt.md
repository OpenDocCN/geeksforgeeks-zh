...
> 原文: [https://www.geeksforgeeks.org/sum-of-nodes-in-the-path-from-root-to-n-th-node-in-given-tree/](https://www.geeksforgeeks.org/sum-of-nodes-in-the-path-from-root-to-n-th-node-in-given-tree/)
...
*   递减 `ind` 直到小于或等于 `1`，并继续更新 `val = temp - prefix[ind - 1]`。
*   如果 `ind` 为**奇数**，则将 `temp` 更新为 `prefix[ind - 2] + (val + 1) / 2`。
*   否则，如果 `ind` 为**偶数**，则更新 `temp = prefix[ind - 2] + (val + 3) / 4`。
