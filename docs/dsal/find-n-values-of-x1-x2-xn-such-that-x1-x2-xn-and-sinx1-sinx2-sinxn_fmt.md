...
> 原文: [https://www.geeksforgeeks.org/find-n-values-of-x1-x2-xn-such-that-x1-x2-xn-and-sinx1-sinx2-sinxn/](https://www.geeksforgeeks.org/find-n-values-of-x1-x2-xn-such-that-x1-x2-xn-and-sinx1-sinx2-sinxn/)

给定一个数 `N`，任务是求 `X_i` 的 `N` 个整数值，使得 `X1 < X2 < … < XN` 和 `sin(X1) < sin(X2) < … < sin(XN)`。
...
思路是用 `PI` 的分数值（`π`）；即，`π = 355/113`，因为它给出了精确度为 `0.000009%` 的 `π` 的最佳合理值。
...
按照上面的等式得到数值 `X1 < X2 < … < XN` 和 `sin(X1) < sin(X2) < … < sin(XN)`，我们必须找到 `sin(X)` 的值，增量为 `710`。
