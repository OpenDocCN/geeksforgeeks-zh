...
> 原文：[https://www.geeksforgeeks.org/find-minimum-number-of-steps-to-reach-the-end-of-string/](https://www.geeksforgeeks.org/find-minimum-number-of-steps-to-reach-the-end-of-string/)

给定一个长度为 `N` 的二进制字符串 `str` 和一个整数 `K`，任务是找到从 `str[0]` 移动到 `str[N-1]` 所需的最小步数，移动如下：

1.  从指数 `i` 来看，唯一有效的移动是 `i + 1`、`i + 2` 和 `i + K`。
2.  只有当 `str[i] = '1'` 时，才能访问索引 `i`。
...
思路是用[动态规划](https://www.geeksforgeeks.org/dynamic-programming/)解决问题。
