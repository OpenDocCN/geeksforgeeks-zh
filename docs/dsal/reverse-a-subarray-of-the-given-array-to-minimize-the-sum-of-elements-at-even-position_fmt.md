...
> 原文：[https://www.geeksforgeeks.org/reverse-a-subarray-of-the-given-array-to-minimize-the-sum-of-elements-at-even-position/](https://www.geeksforgeeks.org/reverse-a-subarray-of-the-given-array-to-minimize-the-sum-of-elements-at-even-position/)

给定正整数的数组 `arr[]`。任务是反转一个子阵列，使偶数位置的元素之和最小，并打印最小和。

**注意：** 只执行一次移动。子阵列可能不会反转。
...
> 现在偶数位置的元素之和 = `arr[0] + arr[2] + arr[4] = 1 + 4 + 2 = 7`
>
> **输入：** `arr[] = {0, 1, 4, 3}`
> **输出：** 1
> **解释：**
...
> 现在偶数位置的元素之和 = `arr[0] + arr[2] = 0 + 1 = 1`，即最小和。

**朴素法：** 思路是应用蛮力法，[生成所有的子阵](https://www.geeksforgeeks.org/generating-subarrays-using-recursion/)，检查偶数位置的元素之和。打印所有总和中的最小值。
...
*   反转一个**偶数长度**子阵列将使奇数索引的所有元素进入偶数索引，偶数索引的元素进入奇数索引。
