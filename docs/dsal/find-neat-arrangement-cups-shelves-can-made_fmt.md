...
> 原文:[https://www.geeksforgeeks.org/find-neat-arrangement-cups-shelves-can-made/](https://www.geeksforgeeks.org/find-neat-arrangement-cups-shelves-can-made/)
...
整理杯子和茶托，找出杯子总数 `a` 和茶托总数 `b`。因为同一个架子上不能超过 5 个杯子，所以通过公式 `(a+5-1)/5` 找出杯子需要的最大架子数，通过公式 `(b+10-1)/10` 找出茶碟需要的最大架子数。如果这两个值之和等于或小于 `n`，则这种安排是可能的，否则是不可能的。
