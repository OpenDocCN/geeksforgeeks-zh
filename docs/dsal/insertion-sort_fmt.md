...
`i = 1`。由于 `11` 小于 `12`，在 `12` 之前移动 `12` 并插入 `11`，`11，12`，13，5，6
...
如果元素按相反顺序排序，插入排序需要最长时间进行排序。并且当元素已经排序时，它花费的时间最少(`n` 的顺序)。
...
元素数量少时使用插入排序。当输入数组几乎被排序，只有很少的元素被错放在完整的大数组中时，它也很有用。

### 什么是二进制插入排序？

我们可以使用二分搜索法来减少正常插入排序中的比较次数。二进制插入排序在每次迭代中使用二分搜索法查找插入所选项目的正确位置。在正常插入中，排序在最坏的情况下取 `O(i)` (在第一次迭代时)。我们可以用二分搜索法把它简化为 `O(log i)`。由于每次插入都需要一系列的交换，该算法作为一个整体仍然具有 `O(n^2)` 的最坏情况运行时间。实施请参考[本](https://www.geeksforgeeks.org/binary-insertion-sort/)。
