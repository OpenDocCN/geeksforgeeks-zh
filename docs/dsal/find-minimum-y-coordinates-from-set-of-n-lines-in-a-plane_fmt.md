...
> 原文: [https://www.geeksforgeeks.org/find-minimum-y-coordinates-from-set-of-n-lines-in-a-plane/](https://www.geeksforgeeks.org/find-minimum-y-coordinates-from-set-of-n-lines-in-a-plane/)

以 2D [阵列](https://www.geeksforgeeks.org/introduction-to-arrays/) `arr[][]` 的形式给定平面中的 **N** 条直线，使得每行由 2 个整数组成（比如说 `m` 和 `c`），其中 `m` 是直线的斜率，`c` 是该直线的 **y 截距**。给你的 **Q** 查询各由 **x 坐标**组成。任务是为每个查询找到每一行对应的最小可能 **y 坐标**。
...
1.  **L1** 和 **L2** 是两条线，相交于 `(x1, y1)`。如果 **L1** 比 **L2** 低 `x = x1`，那么 **L2** 会比 **L1** 低 `x = x1`。这意味着对于某些连续范围，线条给出了较低的值。
