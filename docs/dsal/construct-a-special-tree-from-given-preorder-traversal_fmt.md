...
> 原文: [https://www.geeksforgeeks.org/construct-a-special-tree-from-given-preorder-traversal/](https://www.geeksforgeeks.org/construct-a-special-tree-from-given-preorder-traversal/)

给定一个数组`pre[]`，它表示一个特殊二叉树的 Preorder 遍历，其中每个节点都有 0 或 2 个子节点。又给出了一个数组`preLN[]`，它只有两个可能的值`L`和`N`。`preLN[]`中的值`L`表示二叉树中对应的节点是叶节点，`N`表示对应的节点是非叶节点。编写一个函数，根据给定的两个数组构造树。
