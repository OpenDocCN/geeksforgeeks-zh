...
我们可以将循环修改为 `i = a`，`i <= n`，`i = i + a`，以减少迭代次数。但是如果有 `a` 的 `m` 倍，也需要 `O(m)` 时间。为了在 `O(1)` 时间内得到结果，我们可以使用 `n` 个自然数求和的公式。对于上面的例子，`a = 4`，`N = 23`，`a` 的倍数，`m = N/a`（整数除法）。倍数是 `4`，`8`，`12`，`16`，`20`。我们可以写成 `4 * [1, 2, 3, 4, 5]`。所以我们可以得到倍数之和为：
