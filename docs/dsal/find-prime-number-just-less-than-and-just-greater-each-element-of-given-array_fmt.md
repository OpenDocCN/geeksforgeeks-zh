...
> 原文: [https://www.geeksforgeeks.org/find-prime-number-just-less-than-and-just-greater-each-element-of-given-array/](https://www.geeksforgeeks.org/find-prime-number-just-less-than-and-just-greater-each-element-of-given-array/)

给定一个整数数组`A[]`的大小`N`，任务是找到素数刚好小于且刚好大于`A[i]`（对于所有`0 <= i < N`）。
...
**观察:** 小于 10<sup>9</sup> 的[最大原始间隙](https://en.wikipedia.org/wiki/Prime_gap#:~:text=As%20of%20September%202017%2C%20the%20largest%20known%20prime%20gap%20with%20confirmed%20endpoints%20has%20length%201550%2C%20while%20the%20largest%20known%20maximal%20prime%20gap%20(with%20unknown%20endpoints)%20has%20length%201132.%20The%20prime%20gap%20of%20length%201550%20has%20merit%20M%20%3D%2013.1829.) 为 292。
...
1.  按照降序从`A[i]-1`开始迭代，对于每个当前索引`j`，执行以下操作：
    ...
    2.  如果`j`是质数，打印`j`并终止内环。这给出的质数只比`A[i]`少一点。
...
### 时间复杂度
`O(N*G*√M)`，其中 `G` 为最大原始间隙，`M` 为 `A`。
### 辅助空间
`O(1)`

## 有效方法 1
不用检查单个数字，所有素数都可以用厄拉多塞的[筛重新计算。](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)
