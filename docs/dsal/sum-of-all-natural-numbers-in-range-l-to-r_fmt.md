...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-natural-numbers-in-range-l-to-r/](https://www.geeksforgeeks.org/sum-of-all-natural-numbers-in-range-l-to-r/)
...
一种**有效的方法**是使用前 `N` 个自然数之和的公式。包含-排除原则的思想有助于解决上述问题。求自然数的和直到 `R` 和 `L-1`，再减去 `sum(R) - sum(L-1)`。
