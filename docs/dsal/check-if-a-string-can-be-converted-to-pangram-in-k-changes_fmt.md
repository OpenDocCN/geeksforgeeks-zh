...
> 原文: [https://www.geeksforgeeks.org/check-if-a-string-can-be-converted-to-pangram-in-k-changes/](https://www.geeksforgeeks.org/check-if-a-string-can-be-converted-to-pangram-in-k-changes/)

给定一个只包含小写英文字母的字符串和一个整数 `K`。任务是通过最多执行 `K` 次更改来检查字符串是否可以转换为 Pangram。在一个变化中，我们可以删除任何现有的字符，并添加一个新的字符。
...
[Recommended: Please solve it on “PRACTICE” first, before moving on to the solution.](https://practice.geeksforgeeks.org/problems/k-pangrams/0)

**进场:**

1.  使用布尔访问数组逐字符遍历字符串以跟踪数组中的所有字符。
2.  使用变量 `count`，遍历 `visited` 数组以记录丢失的字符。
3.  如果 `count` 值小于或等于 `K`，则打印“真”。
4.  否则打印假。
