...
> 原文：[https://www.geeksforgeeks.org/change-bits-make-specific-value/](https://www.geeksforgeeks.org/change-bits-make-specific-value/)
...
经过上述程序后，如果`改变的比特数大于K`，那么最多改变 `K` 个比特就不可能得到 `A` 和 `B` 的或作为 `T`。
如果`改变的比特小于K`，那么我们可以通过使用剩余的 `K` 值来进一步最小化 `A` 的值。为此我们将再次循环比特，并且如果在任何时候：

*   第 `i` 个 `A` 位是 `1`，第 `i` 个 `B` 位是 `0`，那么我们将进行 `2` 次更改，并翻转两者。
*   第 `i` 个 `A` 位和第 `i` 个 `B` 位都是 `1`，那么我们将再次进行 `1` 次更改并翻转 `A` 位。

上述解决方案的总时间复杂度为 `O(最大位数)`。
