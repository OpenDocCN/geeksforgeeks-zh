...
> 原文: [https://www.geeksforgeeks.org/xor-two-binary-strings-of-unequal-lengths/](https://www.geeksforgeeks.org/xor-two-binary-strings-of-unequal-lengths/)
...
**做法:** 思路是先使两个字符串长度相等，然后对每个字符逐一进行 XOR 运算，存储到结果字符串中。以下是上述办法的实施情况:
...
**时间复杂度:** `O(len)`，其中 `len = Max(长度 a, 长度 b)`
