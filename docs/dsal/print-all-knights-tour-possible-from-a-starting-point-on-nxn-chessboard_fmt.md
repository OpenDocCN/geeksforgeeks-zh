# 在NxN棋盘上打印所有可能的骑士之旅起点

> 原文：[https://www.geeksforgeeks.org/print-all-knights-tour-possible-from-a-starting-point-on-nxn-chessboard/](https://www.geeksforgeeks.org/print-all-knights-tour-possible-from-a-starting-point-on-nxn-chessboard/)

给定一个`N x N`棋盘，一个骑士最初站在`X`行和`Y`列上，任务是打印所有可能的路径，这样骑士必须精确地访问每个方块一次。
...
> 7 2 15 22 19
> ……302 更
> `说明:` 最初，骑士在(1，2)号牢房。按照第1条路径，骑士将按照以下顺序访问细胞：(1，1)-(3，2)-(5，3)-(4，5)-(2，4)……以此类推。
>
> `输入:` N = 3，X = 1，Y = 3
> `输出:` -1
...
*   迭代骑士所有可能的8个动作，检查它们是否可以安全访问。如果是，递归调用下一步，直到访问的方块数等于方块总数。
