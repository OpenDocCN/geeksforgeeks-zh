# 阵列 B 的最小可能和，使得所有 1 ≤ i < j ≤ N 的 a[i]*b[i] = a[j]*b[j]

> 原文: [https://www.geeksforgeeks.org/minimum-possible-sum-of-array-b-such-that-aibi-ajbj-for-all-1-i-j-n/](https://www.geeksforgeeks.org/minimum-possible-sum-of-array-b-such-that-aibi-ajbj-for-all-1-i-j-n/)

给定一个大小为 `N` 的数组 `a[]`。任务是找到数组 `b[]` 元素的最小可能和，使得所有 `1 ≤ i < j ≤ N` 的 `a[i] * b[i] = a[j] * b[j]`。答案可能很大。所以，打印答案 modulo `10^9+7`。
...
假设确定满足给定条件的 `B_i`。然后让 `K = A_1 * B_1`，然后约束 `K = A_1 * B_1 = A_j * B_j` 全部保持 `j > 1`。因此 `K` 是 `A_1, …, A_N` 的公倍数。

反过来，让 `lcm` 为 `A_1, …, A_N` 的最小公倍数，让 `B_i = LCM / A_i`。则这样的 `B` 满足条件。

所以，想要的答案是 `∑ lcm / A_i`。但是 `lcm` 可以是一个很大的数字，所以不能直接计算。现在，让我们考虑计算，将 `lcm` 保持在因子分解的形式。设 `p_i` 为素数，假设因式分解由 `X = ∏ p_i^{g_i}`、`Y = ∏ p_i^{f_i}`。那么 `X` 和 `Y` 的最小公倍数由 `∏ p_i^{max(g_i, f_i)}` 给出。通过使用这个，可以以因子化的形式获得 `A_1, …, A_N` 的最小公倍数。所以这个问题总共可以在 `O(N*sqrt(A))` 时间内解决，其中 `A = max(A_i)`。同样，通过适当的预计算加速素分解，也可以在总共 `O(A + N*logA)` 的时间内得到答案。
