...
> 原文: [https://www.geeksforgeeks.org/print-all-safe-primes-below-n/](https://www.geeksforgeeks.org/print-all-safe-primes-below-n/)

给定一个整数 `N`，任务是打印 `N` 以下所有的安全素数。一个**安全素数**是 `(2 * p) + 1` 形式的素数，其中 `p` 也是素数。
...
首先使用厄拉多塞的筛法对所有素数进行预计算直到 `N`，然后从 `2` 开始检查当前素数是否也是安全素数。如果是，则打印，否则跳到下一个质数。
