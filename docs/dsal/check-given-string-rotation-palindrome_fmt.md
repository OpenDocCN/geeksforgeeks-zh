...
> 原文：[https://www.geeksforgeeks.org/check-given-string-rotation-palindrome/](https://www.geeksforgeeks.org/check-given-string-rotation-palindrome/)
...
一个简单的解决方法是取输入字符串，尝试它的每一个可能的旋转，如果一个旋转是回文，返回 `true`。如果没有旋转是回文，那么返回 `false`。
以下是本办法的实施情况。
