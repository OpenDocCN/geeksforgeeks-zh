...
> 原文：[https://www.geeksforgeeks.org/smallest-number-divisible-n-least-k-trailing-zeros/](https://www.geeksforgeeks.org/smallest-number-divisible-n-least-k-trailing-zeros/)
...
蛮力法是从 `result = 10^k` 开始。检查 `result` 是否除以 `n`，如果是，就是答案，否则增加 `10^k`。

## 方法 2

高效的方法是计算 `10^k` 和 `n` 的 LCM。假设 `n = 375`，`k = 4`。`result = 10000`。现在 `375` 和 `10000` 的 LCM 是两者除以的最低数。它将包含 `k` 个或更多的零（因为它是 `10^k` 的倍数）并且也是 `n` 的倍数。
