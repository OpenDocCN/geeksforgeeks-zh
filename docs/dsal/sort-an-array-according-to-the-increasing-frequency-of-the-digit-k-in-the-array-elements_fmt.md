...
> 原文: `https://www.geeksforgeeks.org/sort-an-array-according-to-the-increasing-frequency-of-the-digit-k-in-the-array-elements/`

给定一个大小为 `N` 的`数组``arr[]`，以及一个代表一个数字的整数`K`，任务是根据数字`K`在数组元素中的递增频率，以递增的顺序打印给定的数组。
...
**方法:** 思路是统计数组每个元素的数字`K`的出现次数，根据它对数组进行排序。按照以下步骤解决问题:

*   初始化一个`多重映射`，比如说`mp`，将`K`的出现按排序顺序存储在每个元素中。
*   使用变量`i`遍历给定数组`arr[]`，并执行以下操作:
    *   将数字`K`在`arr[i]`中的出现存储在变量`cnt`中。
    *   在`mp`中插入一对`cnt`和`arr[i]`。
*   打印`mp`中的数组元素，得到需要的排序顺序。
