...
> 原文: [https://www.geeksforgeeks.org/check-for-an-array-element-that-is-co-prime-with-all-others/](https://www.geeksforgeeks.org/check-for-an-array-element-that-is-co-prime-with-all-others/)

给定正整数数组 `arr[]`，其中 `2≤arr[i]≤10^6`，所有可能的值为 `i`。任务是检查给定数组中是否至少有一个元素与该数组的所有其他元素形成同素对。如果不存在这样的元素，则打印 `否`，否则打印 `是`。
...
一个简单的解决方法是检查每个元素与所有其他元素的 `gcd` 是否等于 1。本方案时间复杂度为 `O(n^2)`。

## 高效方法

高效的解决方案是生成给定数组中整数的所有素因子。使用 `hash`，存储每个元素的计数，它是数组中任何数字的质因数。如果元素与其他元素不包含任何公共素因子，它总是与其他元素形成共素对。
