...
> 原文: [https://www.geeksforgeeks.org/reduce-a-number-n-by-at-most-d-to-maximize-count-of-trailing-nines/](https://www.geeksforgeeks.org/reduce-a-number-n-by-at-most-d-to-maximize-count-of-trailing-nines/)
...
解决这个问题最简单的方法是使用变量 `i` 迭代范围 `[0, D]`，并将 `N` 的值递减 `i`。最后，打印 `N` 的值，该值包含尾随 `9` 的最大可能计数。
...
> `N % pow(10, i)` 给出 `N` 的最后 `i` 位。

按照以下步骤解决问题:

*   初始化一个变量，比如说 `res` 存储 `N` 的递减值，最大计数跟随 `9`。
*   初始化一个变量，比如说 `count` 将[计数存储在 `N`](https://www.geeksforgeeks.org/program-count-digits-integer-3-different-methods/) 中。
*   使用变量 `i` 迭代范围 `[1, count]`，检查 `N % pow(10, i)` 是否大于或等于 `D`。如果发现为真，则打印 `res` 的值。
*   否则，将 `res` 更新为 `N % pow(10, i) - 1`。
