...
> 原文：[https://www.geeksforgeeks.org/sum-of-nodes-having-sum-of-subtrees-of-opposite-parities/](https://www.geeksforgeeks.org/sum-of-nodes-having-sum-of-subtrees-of-opposite-parities/)
...
思路是[递归](https://www.geeksforgeeks.org/recursion/)计算**左子树**之和**右子树**之和，然后检查给定条件。按照以下步骤解决问题:

*   初始化一个变量`sum`为 `0` 来存储所有这些节点的总和。
*   [在给定的树](https://www.geeksforgeeks.org/iterative-postorder-traversal-using-stack/)中执行后置遍历。
*   求每个节点左右子树的和，检查和是否为`非零`，检查两个和的和是否为奇数。如果发现为真，则在`sum`中包含当前节点值。
