...
将数字的所有因子存储在数据结构中(`向量或数组`)。按升序排列。一旦因子被存储，动态编程可以用来检查任何组合是否形成 `N`。问题变得类似于[子集和问题](https://www.geeksforgeeks.org/dynamic-programming-subset-sum-problem/)。我们可以用同样的方法检查这个数是否是半完全数。
