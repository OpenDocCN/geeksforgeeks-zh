...
> 原文：[https://www.geeksforgeeks.org/count-number-of-smallest-elements-in-given-range/](https://www.geeksforgeeks.org/count-number-of-smallest-elements-in-given-range/)
...
3.  `min(left_subtree) = min(right_subtree)`:
    `node.min = min(left_subtree)` 或 `min(right_subtree)`，`node.count = left_subtree.count + right_subtree.count`
