...
> 原文: [https://www.geeksforgeeks.org/check-if-n-can-be-divided-into-k-consecutive-elements-with-a-sum-equal-to-n/](https://www.geeksforgeeks.org/check-if-n-can-be-divided-into-k-consecutive-elements-with-a-sum-equal-to-n/)
...
为了解决上面提到的问题，让我们将整数 `N` 分成 `i` 个连续的数字。序列的术语看起来像 `(d+1)、(d+2)、(d+3)…..(d+i)`，其中 `d` 是每个整数中存在的公共差，该序列的和应等于 `N`。
