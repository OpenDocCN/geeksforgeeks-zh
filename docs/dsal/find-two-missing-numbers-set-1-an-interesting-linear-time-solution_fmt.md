...
> 原文: [https://www.geeksforgeeks.org/find-two-missing-numbers-set-1-an-interesting-linear-time-solution/](https://www.geeksforgeeks.org/find-two-missing-numbers-set-1-an-interesting-linear-time-solution/)
...
*   其中一个数字将小于或等于 `avg`，而另一个数字将严格大于 `avg`。两个数字永远不可能相等，因为所有给定的数字都是不同的。
