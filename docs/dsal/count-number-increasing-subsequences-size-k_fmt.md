...
> 原文: [https://www.geeksforgeeks.org/count-number-increasing-subsequences-size-k/](https://www.geeksforgeeks.org/count-number-increasing-subsequences-size-k/)
...
> `dp[i][j] = sum(dp[i-1][j])`，其中 `1 < i <= k`，`i <= j <= n` 和 `arr[m] < arr[j]` 为 `(i-1) <= m < j`。
