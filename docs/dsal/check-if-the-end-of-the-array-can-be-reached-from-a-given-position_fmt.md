...
> 原文: [https://www.geeksforgeeks.org/check-if-the-end-of-the-array-can-be-reached-from-a-given-position/](https://www.geeksforgeeks.org/check-if-the-end-of-the-array-can-be-reached-from-a-given-position/)

给定一个正整数的数组 `arr[]` 和一个数 `S`，任务是从索引 `S` 到达数组的末尾。我们只能从当前索引 `i` 移动到索引 `(i + arr[i])` 或 `(i - arr[i])`。如果有办法到达数组末尾，则打印 `"是"` 否则打印 `"否"`。
...
> **说明:**
> 初始位置: `arr[S] = arr[2] = 2`。
> 到达终点的可能跳跃: `4 -> (index 7)` 或 `4 -> (index 2)`
> 由于两者都在界内，因此无法到达终点。
