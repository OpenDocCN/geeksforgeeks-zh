...
> 原文: [https://www.geeksforgeeks.org/count-frequency-of-digit-k-in-given-array/](https://www.geeksforgeeks.org/count-frequency-of-digit-k-in-given-array/)
...
想法是遍历数组，对于每个单独的数组元素，计算其中数字 `K` 的出现次数，并更新总计数。按照以下步骤解决问题:

1.  初始化数字 `K` 的总出现次数，比如 `count`，为 0。
2.  从起点元素到终点遍历给定数组。
3.  对于每个遍历的元素，找到该元素中数字 `K` 的频率。
4.  将 `count` 加到 `sum` 中。
5.  返回 `sum` 作为答案。
