...
> 原文: [https://www.geeksforgeeks.org/find-permutation-with-maximum-remainder-sum/](https://www.geeksforgeeks.org/find-permutation-with-maximum-remainder-sum/)

给定一个整数 `N`，任务是找到从 `1` 到 `N` 的整数排列，使得 `\sum_{i=1}^{N}P_i\mod i` 最大。
...
对上述数组中的表达式 `P_i\mod i` 求值后，输出数组将是 `{0，1，2，3，…。，N–1 }`，这是可以获得的最大值。
以下是上述方法的实施:
