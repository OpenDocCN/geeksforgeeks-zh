...
> 原文: [https://www.geeksforgeeks.org/check-if-gcd-of-all-composite-numbers-in-an-array-divisible-by-k-is-a-fibonacci-number-or-not/](https://www.geeksforgeeks.org/check-if-gcd-of-all-composite-numbers-in-an-array-divisible-by-k-is-a-fibonacci-number-or-not/)

给定由 `N` 个非负整数和一个整数 `K` 组成的数组 `arr[]`，任务是检查数组中所有可被 `K` 整除的复合数的 GCD 是否为斐波那契数。如果发现是真的，打印 `"是"`。否则，打印 `"否"`。
...
> **输出:** 是
> **说明:** 数组中可被 2 整除的复合数为 `{34, 2, 4, 8}`。这些元素的 GCD 等于 2，是斐波那契数。

**方法:** 按照以下步骤解决问题:

1.  创建一个函数 `isComposite()` 来检查一个数字是否是复合数字。
2.  创建另一个函数 `isFibonacci()` 来检查一个数字是否是斐波那契数。
3.  初始化一个整数向量，比如 `compositeset`，和另一个整数变量 `gcd` 来存储数组中可被 `K` 整除的复合数的 GCD。
4.  遍历数组 `arr`。
5.  对于每一个元素 `arr[i]`，检查它是否是复合的，是否能被 `K` 整除。如果发现是真的，将其插入向量 `compositeset`。
6.  计算向量中所有元素的 GCD `compositeset` 并存储在变量 `gcd` 中。
7.  检查 `gcd` 是否为斐波那契数。
