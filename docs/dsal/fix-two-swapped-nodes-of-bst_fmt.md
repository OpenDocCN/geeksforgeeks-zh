...
**如何解决？** 我们会保持三个指针，`first`，`middle`，`last`。当我们找到当前节点值小于前一个节点值的第一个点时，我们用前一个节点更新 `first`，用当前节点更新 `middle`。当我们找到当前节点值小于前一个节点值的第二个点时，我们用当前节点更新 `last`。在#2 的情况下，我们永远找不到第二点。所以，`last` 指针不会被更新。处理后，如果 `last` 节点值为空，则 BST 的两个交换节点相邻。
...
# 修正二叉搜索树（BST）中两个交换节点的代码实现

## C语言实现

### 代码说明

本实现通过中序遍历来找出二叉搜索树（BST）中被交换的两个节点，并进行修正。关键函数包括：
- `swap()`：用于交换两个整数的值。
- `newNode()`：创建一个新的树节点。
- `correctBSTUtil()`：执行中序遍历，通过`first`、`middle`、`last`和`prev`四个指针来定位违规节点。
- `correctBST()`：主函数，调用`correctBSTUtil()`并交换被错误放置的节点数据。
- `printInorder()`：打印树的中序遍历结果。
...
本实现是上述C语言版本的Java翻译。核心逻辑位于`BinaryTree`类中：
- `Node`类：定义了树节点的结构。
- `correctBSTUtil()`：执行中序遍历，使用类成员变量`first`、`middle`、`last`和`prev`来追踪节点。
- `correctBST()`：修正BST的主函数，通过交换节点数据来修复树。
- `printInorder()`：打印中序遍历序列。
