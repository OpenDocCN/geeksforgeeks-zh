...
解决方案是基于这样一个事实，即“[如果一个图没有奇数长度的循环，那么它必须是二分的，即它可以用两种颜色](https://www.geeksforgeeks.org/check-graphs-cycle-odd-length/)”着色。这个想法是将给定的问题转化为一个更简单的问题，我们只需要检查是否有奇数长度的循环。要转换，我们按照进行：
