...
> 原文: [https://www.geeksforgeeks.org/xor-of-elements-in-a-given-range-with-updates-using-fenwick-tree/](https://www.geeksforgeeks.org/xor-of-elements-in-a-given-range-with-updates-using-fenwick-tree/)

给定整数数组 `A` 和由以下两种类型的查询组成的数组 `Q`：

*   **(1，L，R)**：返回索引 `L` 和 `R` 之间存在的所有元素的异或。
*   **(2，I，val)**：将 `A[I]` 更新为 `A[I] XOR val`。

任务是使用[芬威克树](https://www.geeksforgeeks.org/binary-indexed-tree-or-fenwick-tree-2/)求解每个查询并打印第一类每个查询的异或。
...
> 3 到 8 范围内的元素异或为 3
> **说明：**
> 子数组 `{ 3，2，3，4，5，6 }` 的异或为 5。
> 第二次查询后，`arr[4]` 被 4 替换。
> 子数组 `{ 3，4，3，4，5，6 }` 的异或为 3。
...
*   对于类型 1 的查询，使用 `getXor()` 返回范围 `[1，R]` 和范围 `[1，L-1]` 中元素的异或。
*   在 `getXor()` 中，对于 `i` 从 `index` 开始到其所有祖先直到 1，继续计算异或和 `BITree[i]`。为了在 `getXor()` 视图中得到第 `i` 个索引的祖先，我们只需要通过 `i = i–i&(-i)` 从 `i` 中减去最低有效位。最后返回最终的异或值。
*   对于类型 2 的查询，将 `A[index]` 更新为 `A[index] ^ val`。通过调用 `updateBIT()`，更新 `BITree[]` 中包含此元素的所有范围。
*   在 `updateBIT()` 中，对于从 `index` 开始直到 `N` 的每个 `i`，将 `BITree[i]` 更新为 `BITree[i] ^ val`。为了在 `updateBit()` 视图中得到第 `i` 个索引的祖先，我们只需要通过 `i = i + i & (-i)` 从 `i` 中添加最低有效位。
