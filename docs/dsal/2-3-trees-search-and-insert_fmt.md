...
2-3 树是一种树数据结构，其中每个内部节点（非叶节点）要么有一个数据元素和两个子节点，要么有两个数据元素和三个子节点。如果一个节点包含一个数据元素 `leftVal`，则它有两个子树（子树），即 `左` 和 `中`。而如果一个节点包含两个数据元素 `leftVal` 和 `rightVal`，则它有三个子树，即 `左`、`中` 和 `右`。

2-3 棵树的主要优点是，它在本质上是平衡的，而二叉查找树树的高度在最坏的情况下可能是 O(n)。因此，搜索、插入和删除等操作的最坏情况时间复杂度为 O(log(n))，因为 2-3 棵树的高度为 O(log(n))。

## 搜索

要在给定的 2-3 树 `T` 中搜索一个键 `K`，我们遵循以下步骤：
...
1.  如果 `K` < `current node.leftVal`，我们探索当前节点的左子树。
2.  否则如果 `current node.leftVal` < `K` < `current node.rightVal`，我们探索当前节点的中间子树。
3.  否则如果 `K` > `current node.rightVal`，我们探索当前节点的右子树。
