...
> 原文：[https://www.geeksforgeeks.org/previous-smaller-integer-one-less-number-set-bits/](https://www.geeksforgeeks.org/previous-smaller-integer-one-less-number-set-bits/)
...
1.  在`n`的二进制表示中，找到最右边设置位的位置（考虑位置 1 的最后一位，位置 2 的第二个最后一位，以此类推）。让`pos`代表位置。参考[这篇](https://www.geeksforgeeks.org/position-of-rightmost-set-bit/)帖子。
2.  在`pos`位置关闭或解除钻头。参考[这篇](https://www.geeksforgeeks.org/how-to-turn-off-a-particular-bit-in-a-number/)帖子。
