...
> 原文: [https://www.geeksforgeeks.org/number-subsequences-form-ai-bj-ck/](https://www.geeksforgeeks.org/number-subsequences-form-ai-bj-ck/)

给定一个字符串，计算 `a^i b^j c^k` 形式的子序列数，即它由 `i` 个 `'a'` 字符组成，后跟 `j` 个 `'b'` 字符，后跟 `k` 个 `'c'` 字符，其中 `i >= 1`，`j >= 1`，`k >= 1`。
...
1.  初始化由不同的 `'a'` 组合引起的不同子序列的计数。让这个计数成为一个 `aCount`。
2.  初始化由 `'b'` 的不同组合引起的不同子序列的计数。让这个计数成为 `bCount`。
3.  初始化由不同的 `'c'` 组合引起的不同子序列的计数。让这个计数被 `cCount`。
4.  遍历给定字符串的所有字符。对当前字符 `s[i]` 做以下操作:
    *   **如果当前字符是 `'a'`**，那么有以下可能:
        a) 当前字符开始一个新的子序列。
        b) 当前字符是 `aCount` 子序列的一部分。
        c) 当前字符不是 `aCount` 子序列的一部分。
        因此我们做一个 `aCount = (1 + 2 * aCount)`；
    *   **如果当前字符是 `'b'`**，那么有以下可能:
        a) 当前字符以一个 `aCount` 子序列开始一个新的 `b` 的子序列。
...
*   `aCount` 是字母 `'a'` 的子序列数。
*   考虑这个例子: `aa`。
*   我们可以看到这个的 `aCount` 是 3，因为我们可以选择这些可能性: (`xa`, `ax`, `aa`) (`x` 表示我们没有使用那个字符)。还要注意，这与中间的字符无关，即 `aa` 和 `ccbabbbcac` 的 `aCount` 是相同的，因为两者都正好有 2 个 `a`。
*   现在，加上 1 个 `a`，我们现在有了以下新的子序列: 每个旧的子序列，每个旧的子序列+新的 `a`，以及新的字母 `a`。所以总共有 `aCount + aCount + 1` 个子序列。
*   现在，让我们来考虑 `bCount`，一些 `a`，然后一些 `b` 的子序列的数量。在 `'aab'` 中，我们看到 `bCount` 应该是 3 (`axb`, `xab`, `aab`)，因为这只是我们可以选择前两个 `a` 的子序列，然后是 `b` 的方法的数量，所以每次我们添加一个 `ab`，方法的数量就会增加一个 `aCount`。
*   让我们找到 `"aabb"` 的 `bCount`。我们已经确定 `aab` 有 3 个子序列，所以我们肯定还有这些。此外，我们可以将新的 `b` 添加到这些子序列中的任何一个上，以获得另外 3 个。最后，我们必须计算不使用任何其他 `b` 的子序列，根据最后一段的逻辑，这只是一个 `aCount`。所以，这之后的 `bCount` 只是 `旧的 bCount * 2 + aCount`；
*   `cCount` 类似。
