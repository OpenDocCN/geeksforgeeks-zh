...
> 原文: [https://www.geeksforgeeks.org/reduce-all-array-elements-to-zero-by-performing-given-operations-thrice/](https://www.geeksforgeeks.org/reduce-all-array-elements-to-zero-by-performing-given-operations-thrice/)

给定一个大小为 `N` 的数组 `arr[]`，任务是通过应用以下操作恰好三次将每个数组元素转换为 `0`：

*   选择一个子数组。
*   将子数组的每个元素增加其长度的整数倍。

最后，打印所涉及的子数组的第一个和最后一个索引，以及在每个步骤中添加到子数组的每个索引中的元素。
...
添加元素: -3 -6 -6
**解释:**
步骤 1: 选择子数组 `{arr[1]}` 并将 -1 添加到其中唯一的元素。因此，数组 `arr[]` 修改为 `{0, 3, 2, 4}`。
步骤 2: 选择子数组 `{arr[3], arr[4]}` 并分别向子数组元素添加 `{4, 2}`。因此，数组 `arr[]` 修改为 `{0, 3, 6, 6}`。
步骤 3: 选择子数组 `{arr[2], arr[3], arr[4]}` 并分别向子数组元素添加 `{-3, -6, -6}`。因此，数组 `arr[]` 修改为 `{0, 0, 0, 0}`。
...
**操作 1:** 选择子数组 `{arr[1], ..., arr[N]}`。设置 `A[i] = A[i] - N * A[i] = (N-1)*(-A[i])`。
操作 1 后，每个元素都是 `(N-1)` 的倍数。
**操作 2:** 选择子数组 `{arr[1], ..., arr[N-1]}`。将子数组的所有值加/减 `(N-1)` 的倍数，直到它们减少到 0。
**操作 3:** 选择尺寸为 1 的子数组 `{arr[N]}`。加/减 1 的倍数使 `A[N] = 0`。
...
*   否则，执行以下操作：
    ...
    *   打印 `1 N-1`。从子数组的每个元素中减去 `(N-1) * arr[i]` 并打印减去的值。
    *   最后打印 `N N`。从元素中减去 `arr[i-1]`。打印 `arr[i-1]`。
