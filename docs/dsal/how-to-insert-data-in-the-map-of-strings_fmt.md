...
`Map`是[关联容器](https://www.geeksforgeeks.org/sequence-vs-associative-containers-cpp/)，以特定顺序存储元素。它将元素存储在键值和映射值的组合中。
...
要在`map`中插入数据，使用`map`中的`insert()`功能。它用于在`map`容器中插入具有特定键的元素。
...
它接受一个由要插入到`map`容器中的键和元素组成的对，但它只插入唯一的键。这意味着，如果键已经存在于`map`中，则该函数不会在`map`中插入键和元素。

## 返回值

返回一个迭代器，指向`map`中的新元素。
