...
> 原文: [https://www.geeksforgeeks.org/implement-undo-and-redo-features-of-a-text-editor/](https://www.geeksforgeeks.org/implement-undo-and-redo-features-of-a-text-editor/)
...
> 对文档执行 `"WRITE B"`。因此，该文档包含 `"AB"`。
> 对文档执行 `"WRITE C"`。因此，该文件包含 `"ABC"`。
> 对文档执行 `"UNDO"`。因此，该文档包含 `"AB"`。
> 打印文档内容，即 `"AB"`。
> 对文档执行 `"REDO"`。因此，该文件包含 `"ABC"`。
> 打印文档内容，即 `"ABC"`。
...
使用[栈](https://www.geeksforgeeks.org/stack-data-structure/)可以解决问题。按照以下步骤解决问题:

*   初始化两个栈，记为 `Undo` 和 `Redo`。
*   [遍历字符串数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `Q`，执行以下操作:
    *   如果遇到 `"WRITE"` 字符串，将字符推到 `Undo` 栈。
    *   如果遇到 `"UNDO"` 字符串，从 `Undo` 栈[弹出](https://www.geeksforgeeks.org/stack-push-method-in-java/)顶部元素，并将其推至 `Redo` 栈。
    *   如果遇到 `"REDO"` 字符串，弹出 `Redo` 栈的顶部元素，推入 `Undo` 栈。
    *   如果遇到 `"READ"` 字符串，[按相反顺序打印 `Undo` 栈的所有元素](https://www.geeksforgeeks.org/print-stack-elements-from-bottom-to-top/)。
