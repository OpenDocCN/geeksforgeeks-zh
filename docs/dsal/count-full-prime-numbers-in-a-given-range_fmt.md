...
> 原文：[https://www.geeksforgeeks.org/count-full-prime-numbers-in-a-given-range/](https://www.geeksforgeeks.org/count-full-prime-numbers-in-a-given-range/)

给定两个整数 `L` 和 `R`，任务是计算给定范围内存在的全素数的数量。
...
*   否则，检查它的所有数字是否都是质数。如果发现为真，增加 `cnt`。
*   最后，在完成范围遍历后，打印 `cnt` 的值。
