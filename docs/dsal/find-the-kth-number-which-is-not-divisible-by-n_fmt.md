...
> 原文: [https://www.geeksforgeeks.org/find-the-kth-number-which-is-not-divisible-by-n/](https://www.geeksforgeeks.org/find-the-kth-number-which-is-not-divisible-by-n/)

给定两个整数 `N` 和 `K`，任务是找到不能被 `N` 整除的第 `K` 个数。

> **注:** `N` 的值大于 1，因为每个数都可以被 1 整除。
...
一个简单的解决方案是迭代一个循环来寻找第 `K` 个不可被 `n` 整除的数，下面是寻找第 `K` 个数的步骤:

*   将不可分数和当前数的计数初始化为 0。
*   循环的同时使用 [while](https://www.geeksforgeeks.org/c-c-while-loop-with-examples/) 进行迭代，直到不可分数字的计数不等于 `K`。
...
这个想法是利用二分搜索法来解决这个问题。这个问题的搜索空间将从 1 到最大整数值，中间值计算为搜索空间的中间值与 `n` 的倍数之差。
...
问题中的关键观察是，从 1 到 N-1 的每个数都不能被 N 整除，然后类似地，N + 1 到 2 * N–1 也不能被 N 整除。记住这一点，不能被 N 整除的第 K 个<sup>数</sup>将是:
