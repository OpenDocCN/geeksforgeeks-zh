...
> 原文: [https://www.geeksforgeeks.org/given-equation-circle-find-area-circle/](https://www.geeksforgeeks.org/given-equation-circle-find-area-circle/)

给出圆心在原点`(0, 0)`且半径为`R`的圆`X^2 + Y^2 = R^2`的方程，任务是求圆的面积。
...
*   给定一个方程`X^2 + Y^2 = R^2`，并将其存储为字符串`str`。
*   计算字符串的长度并将其存储到`len`中。
*   从`0`到`len-1`开始循环，并检查字符串`str[i] == '='`。
*   将`=`后的字符存储到字符串变量`st`中。
*   将字符串`st`转换为数字并存储到`radius_square`中。
*   用公式`π * R^2`求圆的面积（乘以`π`）。
