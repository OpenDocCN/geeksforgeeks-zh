...
> 原文:[https://www.geeksforgeeks.org/program-to-find-sum-of-the-series-13-35/](https://www.geeksforgeeks.org/program-to-find-sum-of-the-series-13-35/)
...
现在，系列 1、3、5 的第 n 项由 `2*n-1` 给出，系列 3、5、7 的第 n 项由 `2*n+1` 给出，将这两个值放入 t<sub>n</sub> :

> t<sub>n</sub> = (2 * n-1) * (2 * n+1) = 4 * n * n-1

现在，前 n 项的总和将由下式给出:

> s<sub>n</sub> = ∑(4 * n * n–1) = ∑4 * { n * n } - ∑(1)

现在已知级数 n*n (1，4，9，…)的前 n 项之和由下式给出: n*(n+1)*(2*n+1)/6
而 1 的 n 个数之和就是 n 本身。
现在，将值放入 S<sub>n</sub>:

> s<sub>n</sub> = 4 * n *(n+1)*(2 * n+1)/6 – n = n *(4 * n * n+6 * n–1)/3
