...
给定一个长度为 `N` 的数组 `arr[]` 和另一个包含{a <sub>1</sub> 、a <sub>2</sub> 、… a <sub>k</sub> }的数组 `P[]`,该数组代表给定数组 `arr[]` 的位置，任务是检查该数组是否只能通过交换元素 `arr[a_i]` 和 `arr[a_{i+1}]` 来排序，其中 `i` 属于数组 `P[]`。
...
*   最初，我们创建一个大小为 `N` 的位置数组 `pos[]`。该数组将用于标记数组中的给定位置 `P[]`。那就是:
...
*   在这个增量之后，我们对从 `i` 到 `j+1` 得到的这个片段进行[排序](https://www.geeksforgeeks.org/sorting-algorithms/)，最后，在我们必须检查的向量中的位置 `j` 之后进行检查，因为我们已经排序到这个片段。
