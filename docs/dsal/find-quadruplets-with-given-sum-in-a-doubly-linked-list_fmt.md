...
> 原文：[https://www.geeksforgeeks.org/find-quadruplets-with-given-sum-in-a-doubly-linked-list/](https://www.geeksforgeeks.org/find-quadruplets-with-given-sum-in-a-doubly-linked-list/)
...
给定的问题可以使用本文[中讨论的思路](https://www.geeksforgeeks.org/find-four-numbers-with-sum-equal-to-given-sum/)使用[4 指针技术](https://www.geeksforgeeks.org/find-four-elements-that-sum-to-a-given-value-two-pointer-approach/)来解决。按照以下步骤解决问题：

1.  初始化四个变量，`first` 作为双链表的开始即 `head`，`second` 到 `first` 指针的下一个，`third` 到 `second` 指针的下一个，以及 `fourth` 到存储排序的双链表中所有 4 个元素的双链表的最后一个节点。
2.  [迭代一个循环](https://www.geeksforgeeks.org/loops-in-c-and-cpp/)，直到 `first` 节点和 `fourth` 节点不为 `NULL`，并且它们不相等，这两个节点不交叉，执行以下步骤：
    *   将 `second` 指针初始化到 `first->next`。
    *   [迭代一个循环](https://www.geeksforgeeks.org/loops-in-c-and-cpp/)直到 `second` 和 `fourth` 节点不是 `NULL`，它们不相等也不交叉。
        *   初始化一个变量 `reqsum` 为 `(X – (first->data + second->data))`，将 `third` 指针指向 `second` 指针的下一个，取另一个 `temp` 指针初始化为 `last` 节点，即指针 `fourth`。
        *   [迭代一个循环](https://www.geeksforgeeks.org/loops-in-c-and-cpp/)而 `temp` 和 `third` 不是 `NULL`，它们不相等也不交叉：
            *   如果 `reqsum` 的值为 `third->data + temp->data`，则打印四元组并将 `third` 指针增加到当前 `third` 指针的下一个，将 `temp` 增加到当前 `temp` 的前一个。
            *   如果 `reqsum` 的值小于 `third->data + temp->data`，则增加 `third` 指针，即 `third = third->next`。
            *   否则，递减 `temp` 指针，即 `temp = temp->prev`。
        *   将 `second` 指针移动到下一个指针。
    *   将 `first` 指针移动到下一个指针。
