...
> 原文: [https://www.geeksforgeeks.org/find-the-maximum-length-of-the-prefix-set-2/](https://www.geeksforgeeks.org/find-the-maximum-length-of-the-prefix-set-2/)

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是找到数组的最大前缀长度，这样从前缀中删除一个元素将使剩余前缀元素的频率相同。
...
本文的[集 1](https://www.geeksforgeeks.org/find-the-maximum-length-of-the-prefix/) 中已经讨论了散列和基于排序的方法。

**方法:** 想法是使用两个 `map` 来跟踪元素的频率，并使用以下四个条件来检查有效前缀。
...
*   初始化两个 `map`，分别称为 `mp1` 和 `mp2`，分别存储前缀中元素的频率和存储元素的频率。
*   另外，初始化一个变量，比如说 `ans` 为 `0` 来存储前缀的最大长度。
*   使用变量 `i` 遍历范围 `[0，N-1]`，并遵循以下步骤:
    *   如果 `arr[i]` 的计数不等于 `0`，则从 `map` `mp2` 中递减该频率的计数，如果变为 `0`，则从 `map` `mp2` 中清除值。
    *   将 `map` `mp1` 中 `arr[i]` 的计数增加 `1`，然后将 `map` `mp2` 中 `arr[i]` 即 `mp1[arr[i]]` 的新频率的计数增加 `1`。
    *   如果当前元素的计数等于其前缀长度，即 `(i+1)` 或者每个元素在前缀中出现一次，那么将 `ans` 的值更新为 `max(ans，i+1)`。
    *   现在，如果 `mp2` 的大小等于 `2`，则执行以下步骤:
        *   将数组元素的频率，即 `map` 的键 `mp2` 分别存储在变量中，如 `freq1` 和 `freq2`。
        *   将 `freq1` 和 `freq2` 的频率计数存储在变量中，如 `count1` 和 `count2`。
