...
进行有序遍历，然后取一个临时变量，如果我们向左走，则`curr`值降低，如果向右走，则`curr`值升高。在这里声明一个条件，如果`minimum`大于`curr`，那么`minimum`=`curr`，如果`maximum`小于`curr`，那么`maximum`=`curr`。最后，打印`minimum`+`maximum`，即树的垂直宽度。
