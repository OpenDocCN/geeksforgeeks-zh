...
> 原文: [https://www.geeksforgeeks.org/check-large-number-divisible-20/](https://www.geeksforgeeks.org/check-large-number-divisible-20/)
...
如果一个数能被 5 和 4 整除，它就能被 20 整除。我们可以通过检查最后两位数是否能被 4 整除来检查一个数是否能被 4 整除。我们可以通过检查最后一个数字来检查除以 5 的可分性。另外，如果一个数的最后一位是零，第二个最后一位是 2 的倍数，那么这个数可以被 20 整除。
...
```
// Java program to check if a large number is divisible by 20.
import java.io.*;
