...
我们已经讨论了岛屿的 [DFS 解决方案](https://www.geeksforgeeks.org/find-number-of-islands/)。这个问题也可以通过在每个组件上应用 `BFS()` 来解决。在每个 `BFS()` 调用中，都会访问一个组件或子图。我们将在下一个未访问的部分调用 `BFS`。对 `BFS()` 的调用数给出了连接的组件数。`BFS` 也可以用。
