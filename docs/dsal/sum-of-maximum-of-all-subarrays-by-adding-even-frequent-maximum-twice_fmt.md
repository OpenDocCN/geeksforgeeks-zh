...
> 原文：[https://www.geeksforgeeks.org/sum-of-maximum-of-all-subarrays-by-adding-even-frequent-maximum-twice/](https://www.geeksforgeeks.org/sum-of-maximum-of-all-subarrays-by-adding-even-frequent-maximum-twice/)

## 问题描述

给定一个由 `N` 个整数组成的[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `arr[]`（所有数组元素都是 2 的[完美幂](https://www.geeksforgeeks.org/calculate-sum-of-all-integers-from-1-to-n-excluding-perfect-power-of-2/)），任务是计算所有[子数组](https://www.geeksforgeeks.org/generating-subarrays-using-recursion/)中最大元素的和。
...
- 子阵 1：`{4}`。最大值 = 4。总和 = 4。
- 子阵 2：`{4，4}`。最大值 = 4。由于最大值在子阵列中出现两次，总和 = 4 + 8 = 12。
- 子阵 3：`{4}`。最大值 = 4。总和 = 16。
因此，要求输出为 16。
...
解决这个问题最简单的方法是[生成给定数组](https://www.geeksforgeeks.org/generating-subarrays-using-recursion/)的所有可能的子数组，并找到所有子数组中的最大元素及其出现的次数。最后，打印获得的所有最大元素的总和。按照以下步骤解决问题：

- 初始化一个变量，比如 `sum`，以存储所有子阵列的最大值的所需[和](https://www.geeksforgeeks.org/sum-of-maximum-of-all-subarrays-divide-and-conquer/)。
- 生成给定阵列 `arr[]` 的所有可能子阵列。
- 对于生成的每个子阵，找到最大元素的[频率](https://www.geeksforgeeks.org/python-find-frequency-of-largest-element-in-list/)，[检查频率是否均匀](https://www.geeksforgeeks.org/check-whether-given-number-even-odd/)。如果发现是真的，将 `2 * maxNumber` 加到 `sum` 上。否则，将 `maxNumber` 加到 `sum` 上。
...
**高效方法:** 优化上述方法，思路是利用所有数组元素都是 2 的次方的属性，利用该属性解决问题。按照以下步骤解决问题:

*   按照降序遍历 2 的所有次幂。将任意 2 次方视为掩码。
*   将数组分成子数组，这样就不会有子数组包含 `arr[index] = -1`，其中 `index` 是数组中的任何有效位置。
*   让上述步骤得到的子阵列为 `S`。遍历 `S` 和从外环添加值，该值仅由 `S` 中具有当前掩码的子阵列贡献。同时将相应的位置 `arr[index] = mask` 设置为 `arr[index] = -1`。
*   计算包含掩码的 `S` 中所有子阵列贡献的值，并将三个计数器维护为奇数、偶数计数，以及掩码的频率。
*   指针 `prev` 指向前一个索引，这样 `arr[prev] = mask`。
*   在任何 `index` 处，其中 `arr[index] = mask`，通过从索引中减去 `prev` 获得掩码的最后一次出现和当前出现之间的整数的计数。使用此计数和掩码频率的奇偶校验，使用公式 `count = (index - prev)` 获得包含掩码的所有连续子阵列贡献的值，并将计数添加到答案中。
*   如果最大频率为偶数或奇数，并且如果奇偶校验为奇数:
    *   掩码的频率为奇数的所有相邻子阵列贡献的值为 `(count - 1) * evenCount + oddCount`。
    *   由所有相邻子阵列贡献的值，这些子阵列具有与掩码相同的频率: `2 * (count - 1) * oddCount + 2 * evenCount`。
*   否则，如果奇偶为偶数:
    *   掩码的频率为奇数的所有相邻子阵列贡献的值为 `(count - 1) * oddCount + evenCount`。
    *   由所有相邻子阵列贡献的值，这些子阵列具有与掩码相同的频率: `2 * (count - 1) * evenCount + 2 * oddCount`。
*   将所有相应的值添加到答案中。如果奇偶校验为偶数，也将计数加到偶数。否则，将计数加到奇数上。
