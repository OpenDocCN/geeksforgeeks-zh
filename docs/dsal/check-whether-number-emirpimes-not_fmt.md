...
> 原文：[https://www.geeksforgeeks.org/check-whether-number-emirpimes-not/](https://www.geeksforgeeks.org/check-whether-number-emirpimes-not/)

给定一个数字 `n`，检查它是否是一个 Emirpimes。

一个 Emirpimes（“半素数”的倒写）的定义来源于它的拼写方式。所以，一个素数本身就是一个[半素数](https://www.geeksforgeeks.org/check-whether-number-semiprime-not/)（两个素数的乘积）的数，它的数字的反转给出了另一个新的数，它也是一个半素数。因此，根据定义，我们可以得出结论，没有一个回文数字可以是 Emirpimes，因为它们的数字反转不会产生任何新的数字，但会再次形成相同的数字。
...
5.  如果是，则最初输入的数字被报告为一个 Emirpimes。
