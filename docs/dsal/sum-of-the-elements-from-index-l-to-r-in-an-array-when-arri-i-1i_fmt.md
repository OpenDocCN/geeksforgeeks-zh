# 当 `arr[i] = i * (-1)^i` 时，数组中从索引 `l` 到 `r` 的元素之和

> 原文: [https://www.geeksforgeeks.org/sum-of-the-elements-from-index-l-to-r-in-an-array-when-arri-i-1i/](https://www.geeksforgeeks.org/sum-of-the-elements-from-index-l-to-r-in-an-array-when-arri-i-1i/)

给定两个整数 `L` 和 `R` 以及一个数组 `arr[]`，其在索引 `i` 处的每个元素计算为 `arr[I] = I * (-1)^I`。任务是在索引范围 `[L, R]` 内找到数组这些元素的和。
...
根据数组元素的定义，数组的每个奇元素为负，偶元素为正。所以，为了找到和，运行一个从 `L` 到 `R` 的循环，保持所有奇数（负）和偶数（正）的和。最后，返回总和。

## 有效途径

可以注意到本系列所有奇数元素的和将始终等于 `(totalOdd)^2`，其中 `totalOdd` 是奇数元素的总数，偶数元素的和将为 `totalEven * (totalEven + 1)`。现在，我们要做的就是找到 `L` 和 `R` 上所有奇数元素的和，存储两者的差，得到 `L` 和 `R` 之间所有奇数元素的和，对偶数做同样的操作，最后返回偶数和奇数的差。
