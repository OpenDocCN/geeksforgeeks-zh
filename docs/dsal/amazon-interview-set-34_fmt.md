...
20 道客观题:
5 道数学题，其中 3 道是概率题
一些 `c` 输出问题很容易在[`geeksquiz.com`](http://geeksquiz.com/)中找到
关于堆、散列、递归函数时间复杂度的问题
...
输出::1- > 3- > 2- > 5- >空
```
**2.** [给定一个数组和两个数字 `x` 和 `y`，求两个数字 `x` 和 `y` 之间的最小距离，假设 `x` 和 `y` 总是存在于数组中，`x` 和 `y` 也可能相同……](https://practice.geeksforgeeks.org/problems/minimum-distance-between-two-numbers/1)
