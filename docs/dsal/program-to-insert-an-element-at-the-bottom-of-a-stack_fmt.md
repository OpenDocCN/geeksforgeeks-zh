...
> 原文：[https://www.geeksforgeeks.org/program-to-insert-an-element-at-the-bottom-of-a-stack/](https://www.geeksforgeeks.org/program-to-insert-an-element-at-the-bottom-of-a-stack/)
...
1.  [初始化堆栈](https://www.geeksforgeeks.org/stack-in-cpp-stl/)，比如 `temp`。
2.  继续从给定堆栈 `S` [弹出](https://www.geeksforgeeks.org/stack-push-and-pop-in-c-stl/)，将弹出的元素推入 `temp`，直到堆栈 `S` 变空。
3.  [将 `N` 推入堆栈 `S`](https://www.geeksforgeeks.org/stack-push-and-pop-in-c-stl/)。
4.  现在，继续从堆栈 `temp` 弹出，并将弹出的元素推入堆栈 `S`，直到[堆栈 `temp` 变为空的](https://www.geeksforgeeks.org/stack-empty-and-stack-size-in-c-stl/)。
...
3.  否则，去掉 `S` 的[顶元素](https://www.geeksforgeeks.org/stack-top-c-stl/)，存储在一个变量中，比如说 `X`。
4.  使用新堆栈再次递归。
5.  将 `X` 推入 `S`。
