...
给定一个由字符 `K` 或 `k`、`Q` 或 `q`、`B` 或 `b`、`N` 或 `n`、`R` 或 `r`、以及 `P` 或 `p` 组成的矩阵 `board[][]`，代表棋盘。棋子分别为黑和白颜色，空格由 `-` 表示。任务是检查哪个王（黑的或白的）不安全，即是否受到其他任何棋子的攻击（可以被吃掉），并据此打印答案。
...
1.  检查白人和黑人国王的位置。
2.  对于每个国王，检查对面颜色的车、主教、骑士、国王、女王、棋子，看他们是否在攻击国王。
