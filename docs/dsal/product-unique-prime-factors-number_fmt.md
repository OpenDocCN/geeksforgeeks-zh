...
> 原文: [https://www.geeksforgeeks.org/product-unique-prime-factors-number/](https://www.geeksforgeeks.org/product-unique-prime-factors-number/)
...
使用从 `i = 2` 到 `n` 的循环，检查 `i` 是否是 `n` 的因子，然后检查 `i` 是否是质数本身，如果是，则将乘积存储在 `product` 变量中，并继续该过程，直到 `i = n`。
