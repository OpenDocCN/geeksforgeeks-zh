...
> 原文: [https://www.geeksforgeeks.org/smallest-number-whose-product-with-n-has-sum-of-digits-equal-to-that-of-n/](https://www.geeksforgeeks.org/smallest-number-whose-product-with-n-has-sum-of-digits-equal-to-that-of-n/)
...
按照步骤解决问题:
1.  既然 `N` 可以很大，就把 `N` 的输入当成字符串。计算 `N` 的位数之和，存储在变量中，比如 `S`。
