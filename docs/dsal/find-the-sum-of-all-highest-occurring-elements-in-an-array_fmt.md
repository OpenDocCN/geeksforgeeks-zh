...
> 原文：[https://www.geeksforgeeks.org/find-the-sum-of-all-highest-occurring-elements-in-an-array/](https://www.geeksforgeeks.org/find-the-sum-of-all-highest-occurring-elements-in-an-array/)
...
*   遍历数组，用 C++中的`unordered_map`存储数组元素的频率，这样 map 的键就是数组元素，值就是它在数组中的频率。
*   然后，遍历地图以找到最大出现元素的频率。
*   现在，为了找到总和，再次遍历地图，对于所有具有最大频率的元素，找到`frequency_of_max_occuring_element * max_occuring_element`，并找到它们的总和。
