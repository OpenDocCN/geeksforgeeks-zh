...
> 原文: [https://www.geeksforgeeks.org/leftmost-and-rightmost-indices-of-the-maximum-and-the-minimum-element-of-an-array/](https://www.geeksforgeeks.org/leftmost-and-rightmost-indices-of-the-maximum-and-the-minimum-element-of-an-array/)
...
*   初始化变量 `leftMin = rightMin = leftMax = rightMax = arr[0]` 和 `min = max = arr[0]`。
*   开始从 `1` 到 `n–1` 遍历数组。
    *   如果 `arr[i] < min`，则找到一个新的最小值。更新 `leftMin = rightMin = i`。
    *   否则 `arr[i] = min` 则找到当前最小值的另一个副本。更新 `rightMin = i`。
    *   如果 `arr[i] > max`，则找到一个新的最大值。更新 `leftMax = rightMax = i`。
