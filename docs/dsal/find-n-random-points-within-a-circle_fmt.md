...
> 原文：[https://www.geeksforgeeks.org/find-n-random-points-within-a-circle/](https://www.geeksforgeeks.org/find-n-random-points-within-a-circle/)

给定四个整数 `N`、`R`、`X` 和 `Y`，使其代表一个以 `(X, Y)` 为圆心坐标的半径为 `R` 的圆。任务是在圆内或圆上随机找 `N` 个点。
...
要在圆内或圆上找到一个随机点，我们需要两个分量，一个角 (`theta`) 和距中心的距离 (`D`)。之后，点 `(xi, yi)` 可以表示为：
