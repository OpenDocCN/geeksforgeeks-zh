...
> 原文: [https://www.geeksforgeeks.org/sum-of-every-kth-prime-number-in-an-array-2/](https://www.geeksforgeeks.org/sum-of-every-kth-prime-number-in-an-array-2/)
...
**有效方法:** 创建一个筛子，用来存储一个数是否是质数。然后，它可以在 `O(1)` 时间内检查一个数字是否符合质数。这样，我们只需要跟踪每一个第 `K` 个素数，并保持运行和。
以下是上述方法的实施:
