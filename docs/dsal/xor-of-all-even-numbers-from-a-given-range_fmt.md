...
> 原文: [https://www.geeksforgeeks.org/xor-of-all-even-numbers-from-a-given-range/](https://www.geeksforgeeks.org/xor-of-all-even-numbers-from-a-given-range/)

给定两个整数 `L` 和 `R`，任务是计算 `[L, R]` 范围内所有偶数的按位异或。
...
### 天真法
解决问题最简单的方法是遍历 `[L, R]` 范围内的所有偶数，打印所有偶数的按位异或。
...
1.  从范围 `[1, (R) / 2]` 中找到所有数字的按位异或，并将其存储在一个变量中，比如 `xor_r`。
2.  从范围 `[1, (L–1)/2]` 中找到所有数字的按位异或，并将其存储在一个变量中，比如 `xor_l`。
3.  最后，打印 `xor_r ^ xor_l` 的值。
