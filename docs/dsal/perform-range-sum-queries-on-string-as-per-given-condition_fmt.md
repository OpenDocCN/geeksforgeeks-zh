...
> 原文: [https://www.geeksforgeeks.org/perform-range-sum-queries-on-string-as-per-given-condition/](https://www.geeksforgeeks.org/perform-range-sum-queries-on-string-as-per-given-condition/)

给定一个只包含小写字母的字符串 `S` 和 `Q` 个查询，其中每个查询包含一对 `{L, R}`。对于每个查询 `{L, R}`，都存在一个子串 `S[L, R]`，任务是找到子串中每个字符的频率与它们在字母顺序中的位置的乘积的值。
**注:** 考虑基于 1 的索引。
...
天真的想法是遍历查询中的每个范围 `{L, R}`，并保持数组中每个字符的计数。遍历范围后，找到表达式 `1*(出现'a') + 2*(出现'b') + 3*(出现'c') + ... + 26*(出现'z')` 的值。
**时间复杂度:** `O(N*Q)`，其中 `N` 为给定字符串的长度。
**辅助空间:** `O(1)`

## 高效方法

思路是使用整个字符串的[前缀和](https://www.geeksforgeeks.org/prefix-sum-array-implementation-applications-competitive-programming/)，通过这个前缀和我们可以在恒定时间内执行每个查询。以下是步骤:
