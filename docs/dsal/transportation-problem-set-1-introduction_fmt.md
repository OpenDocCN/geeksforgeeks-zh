...
上表中`D1`、`D2`、`D3`和`D4`是产品/货物从不同来源发货的目的地。`S1`、`S2`、`S3`和`S4`是源头。`S<sub>i</sub>`是从源头`O<sub>i</sub>`的供应。`D<sub>j</sub>`是目的地`D<sub>j</sub>`的需求。`C<sub>ij</sub>`是产品从源头`S<sub>I</sub>`到目的地`D<sub>j</sub>`的成本。