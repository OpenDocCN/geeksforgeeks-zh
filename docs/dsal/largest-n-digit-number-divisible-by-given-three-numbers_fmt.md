...
> 原文: [https://www.geeksforgeeks.org/largest-n-digit-number-divisible-by-given-three-numbers/](https://www.geeksforgeeks.org/largest-n-digit-number-divisible-by-given-three-numbers/)
...
*   找到最大的 `n` 位数，即 `pow(10, n) - 1`，并将其存储在变量 `largestN` 中。
*   找出给定的三个数字 `x`，`y`，`z` 的 LCM，说 `LCM`。
*   计算 `largestN` 除以 `LCM` 即 `largestN % LCM` 时的余数，并将其存储在变量 `remainder` 中。
*   从 `largestN` 中减去 `remainder`。如果结果仍然是 `n` 数字，则打印结果。
*   否则打印 `Not possible`。
