...
> 原文: [https://www.geeksforgeeks.org/choose-x-such-that-a-xor-x-b-xor-x-is-minimized/](https://www.geeksforgeeks.org/choose-x-such-that-a-xor-x-b-xor-x-is-minimized/)
...
一个简单的解决方案是将 `A` 和 `B` 与 `X ≤ min(A，B)` 的所有可能值进行异或运算，生成所有可能的和。要生成所有可能的和，需要 `O(N)` 时间，其中 `N = min(A，B)`。

一个有效的解决方案是基于这样的事实，即数字 `X` 将只包含该索引处的设置位，其中 `A` 和 `B` 都包含一个设置位，从而在与 `X` 进行异或运算后，该位将被取消设置。这将只需要 `O(log N)` 时间。
