...
> 原文: [https://www.geeksforgeeks.org/remove-spaces-from-a-given-string/](https://www.geeksforgeeks.org/remove-spaces-from-a-given-string/)
...
一个更好的解决方案可以在 O(n) 时间内解决。这个想法是记录到目前为止看到的非空格字符的数量。
...
Divyam Madaan 建议的另一种解决方案是使用预定义函数。下面是实现：
