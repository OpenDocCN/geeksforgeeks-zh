...
> 原文: [https://www.geeksforgeeks.org/find-n-distinct-integers-with-sum-n/](https://www.geeksforgeeks.org/find-n-distinct-integers-with-sum-n/)
...
思路是像 `(+x, -x)` 一样打印 `N/2` 个[对称对](https://www.geeksforgeeks.org/given-an-array-of-pairs-find-all-symmetric-pairs-in-it/)，这样合成的和永远是 `0`。
现在如果整数 `N` 为奇数，则打印 `N` 连同这些整数组，使所有整数之和等于 `N`。如果 `N` 为偶数，则打印 `0` 和 `N` 连同这些整数组，使所有整数之和等于 `N`。
