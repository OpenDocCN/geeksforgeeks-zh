...
给定维度为 `N*M` 的矩阵 `mat[][]`，正整数 `K` 和源单元格 `(X，Y)`，任务是通过在最多移动 `K` 个的每次移动中向四个方向移动，打印从源单元格 `(X，Y)` 移出矩阵的所有可能路径。
...
*   初始化一个数组，比如说 `arrayOfMoves` 存储从源单元格到矩阵外的所有移动。
*   定义一个递归函数，比如 `printAllmoves(N，M，moves，X，Y，arrayOfMoves)`，并执行以下步骤:
    *   **基本情况:**
        *   如果 `moves` 的值为非负数，并且当前单元格 `(X，Y)` 不在矩阵中，则打印存储在 `arrayOfMoves` 中的所有移动。
        *   如果 `moves` 的值小于 `0`，则从功能返回。
    *   将当前单元格 `(X，Y)` 插入到数组 `arrayOfMoves[]` 中。
    *   通过将 `moves` 的值递减 `1`，在当前单元格 `(X，Y)` 的所有四个方向递归调用该函数。
    *   如果数组 `arrayOfMoves` 的大小大于 `1`，则删除为回溯步骤插入的最后一个单元格。
*   调用功能 `printAllmoves(N，M，moves，X，Y，arrayOfMoves)` 打印所有可能的移动。
