...
> 原文：[https://www.geeksforgeeks.org/find-the-count-of-subsequences-where-each-element-is-divisible-by-k/](https://www.geeksforgeeks.org/find-the-count-of-subsequences-where-each-element-is-divisible-by-k/)
...
由于每个元素必须能被 `K` 整除，总的子序列等于 `2^cnt`，其中 `cnt` 是数组中能被 `K` 整除的元素数。**注意**将从结果中减去 `1`，以排除空的子序列。所以，最终结果将是 `2^cnt - 1`。
