...
> 原文：[https://www.geeksforgeeks.org/number-of-times-an-array-can-be-partitioned-repetitively-into-two-subarrays-with-equal-sum/](https://www.geeksforgeeks.org/number-of-times-an-array-can-be-partitioned-repetitively-into-two-subarrays-with-equal-sum/)

给定一个大小为 `N` 的数组 `arr[]`，任务是找出该数组可以重复划分为两个子数组的次数，使得两个子数组的元素之和相同。
...
> **输出：** 3
> **解释：**
> 1.  创建索引 1 之后的第一个分区。其余的数组都在 `{2, 2}` 的右侧和左侧。
> 2.  考虑左侧子数组 `{2, 2}`。在这个左子数组的索引 0 后做一个分区。现在形成了两个相似的子数组，每个子数组有一个元素，即 `{2}`，不能再细分。
> 3.  考虑右侧的子数组 `{2, 2}`。在这个左子数组的索引 0 后做一个分区。现在形成了两个相似的子数组，每个子数组有一个元素，即 `{2}`，不能再细分。
>     因此输出为 3，因为数组被分割了 3 次。
>
> **输入：** `arr[] = {12, 3, 3, 0, 3, 3}`
> **输出：** 4
> **解释：**
...
> 4.  第四个分区在数组 `{0, 3, 3}` 中的 1 之后，剩下的数组是 `{0, 3}` 和 `{3}`，不能再细分。
>     因此输出为 4。
...
2.  从开始位置迭代到结束位置。
3.  对于每个可能的分区索引（比如 `K`），如果 `prefix_sum[K] - prefix_sum[start-1] = prefix_sum[end] - prefix_sum[k]`，则该分区有效。
4.  如果在上述步骤中分区有效，则分别进行左右子数组，并确定这两个子数组是否形成有效分区。
