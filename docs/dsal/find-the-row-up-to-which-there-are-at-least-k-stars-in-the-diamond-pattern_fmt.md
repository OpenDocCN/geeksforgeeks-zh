...
> 原文：[https://www.geeksforgeeks.org/find-the-row-up-to-which-there-are-at-least-k-stars-in-the-diamond-pattern/](https://www.geeksforgeeks.org/find-the-row-up-to-which-there-are-at-least-k-stars-in-the-diamond-pattern/)
...
给定的问题可以通过简单地[迭代每行](https://www.geeksforgeeks.org/row-wise-vs-column-wise-traversal-matrix/)并保持每行星星数量的计数来解决。打印星星数超过 `k` 的前两个。
...
上述方法可以使用[二分搜索法](https://www.geeksforgeeks.org/binary-search/)对来自 `[0, 2 * n - 1]` 的行数值进行优化。按照以下步骤解决问题：

*   初始化变量 `start = 0`，`end = (2 * N) - 1` 和 `ans = 0`。
*   当 `start` 的值小于 `end` 的值时，请遵循以下步骤。
    *   计算 `mid`，等于 `(start + end) / 2`。
    *   数星星的数量直到 `mid`。
    *   如果到 `mid` 为止的星数大于或等于 `K`，将 `mid` 存入变量并向 `mid` 左侧移动 `end = mid - 1`。
    *   否则，通过 `start = mid + 1` 移动到中间右侧。
*   返回所需值 `ans`。
