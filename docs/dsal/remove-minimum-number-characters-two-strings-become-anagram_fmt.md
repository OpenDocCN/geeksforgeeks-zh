...
> 原文: [https://www.geeksforgeeks.org/remove-minimum-number-characters-two-strings-become-anagram/](https://www.geeksforgeeks.org/remove-minimum-number-characters-two-strings-become-anagram/)
...
其思想是为字符串和每个字符的存储频率创建字符计数数组。现在迭代两个字符串的计数数组和任意字符的频率差 `abs(count1[str1[i]-'a'] - count2[str2[i]-'a'])` 在两个字符串中是要在任一字符串中删除的字符数。
