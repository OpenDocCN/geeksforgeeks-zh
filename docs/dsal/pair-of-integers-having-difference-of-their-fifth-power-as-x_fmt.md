...
> 原文：[https://www.geeksforgeeks.org/pair-of-integers-having-difference-of-their-fifth-power-as-x/](https://www.geeksforgeeks.org/pair-of-integers-having-difference-of-their-fifth-power-as-x/)

给定一个整数 `X`，任务是找到一对 `A` 和 `B`，使得它们的五次方差为 `X`，即 `A^5 - B^5 = X`。如果没有这样的对，输出“不可能”。
...
**天真方法：** 一个简单的解决方案是使用两个 `for` 循环，一个用于 `A`，一个用于 `B`，范围从 `-10^9` 到 `10^9`。
**高效途径：** 思路是利用数学技术**缩小 `A` 和 `B`** 的范围。

自 `A^5 - B^5 = X` => `A^5 = X + B^5`。要使 `A` 尽可能高，`B` 也必须尽可能高，这从不等式中可以明显看出。
