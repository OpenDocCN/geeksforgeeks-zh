...
*   `que[]` 数组存储到达的顶点，只有在尚未考虑所有顶点的情况下，才将顶点排入队列。
*   为了区分是否已访问该节点，我们将在 `Visited[]` 数组中的相应索引处放置 `1`，以表明该节点已被访问；如果给定索引处为 `0`，则表示尚未访问它。
*   `parent` 数组存储每个顶点的父节点。例如，如果 0 连接到 2，则 2 将是 0 的父节点，我们将 2 放在 `parent` 数组的索引 0 处。
