...
> 原文：[https://www.geeksforgeeks.org/move-all-special-char-to-the-end-of-the-string/](https://www.geeksforgeeks.org/move-all-special-char-to-the-end-of-the-string/)
...
是遍历输入字符串并维护两个字符串，一个字符串包含普通字符（`A`，`1`，`' '`等），另一个字符串维护特殊字符（`@`，`$`等）。最后，连接两个字符串并返回。
