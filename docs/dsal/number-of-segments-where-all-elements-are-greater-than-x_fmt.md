...
> 原文：[https://www.geeksforgeeks.org/number-of-segments-where-all-elements-are-greater-than-x/](https://www.geeksforgeeks.org/number-of-segments-where-all-elements-are-greater-than-x/)
...
**方法：** 初始化一个 `flag` 计数器为 `false`，当任何大于 `X` 的元素出现时标记为 `true`。当任何元素不大于 `X` 时，如果 `flag` 为 `true`，则对该段进行计数。每当任何元素 `<= X` 出现时，重新初始化 `flag` 为 `false`。
以下是上述办法的实施情况。
