...
> 原文:[https://www.geeksforgeeks.org/clockwise-spiral-traversal-of-binary-tree/](https://www.geeksforgeeks.org/clockwise-spiral-traversal-of-binary-tree/)
...
# A utility function to prclockwise spiral traversal of tree

```python
def printPattern(node):
    # max, min has taken for calculating width of tree
    max_value = [INT_MIN]
    min_value = [INT_MAX]
    hd = 0
    ...
    # initialize complete matrix with MAX INTEGER(purpose garbage)
    for i in range(width):
    ...
    # Print the circular clockwise spiral traversal of the tree
