...
**有效方法:** 从 Hermite 序列可以明显看出，如果 `n` 为**奇数**，那么**第 n 个 Hermite 数**将为 `0`。现在，可以使用下式找到第 n 个埃尔米特数：

![Hermite number equation](img/8ec47d178332d7b753742e9f4a871b01.png)

其中 `(n–1)!! = 1 * 3 * 5 * (n–1)` 即 `(n–1)` 的[双因子](https://www.geeksforgeeks.org/double-factorial/)。以下是上述方法的实现：
