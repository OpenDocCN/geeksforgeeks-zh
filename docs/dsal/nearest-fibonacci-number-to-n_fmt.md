...
> 原文: [https://www.geeksforgeeks.org/nearest-fibonacci-number-to-n/](https://www.geeksforgeeks.org/nearest-fibonacci-number-to-n/)
...
*   初始化一个变量，比如说 `ans`，来存储离 `N` 最近的[斐波那契数](https://www.geeksforgeeks.org/program-for-nth-fibonacci-number/)。
*   初始化两个变量，将 `first` 设为 `0`，将 `second` 设为 `1`，存储[斐波那契数列](https://www.geeksforgeeks.org/program-to-print-first-n-fibonacci-numbers/)的第一项和第二项。
*   将 `first` 和 `second` 的总和存储在一个变量中，比如说 `third`。
*   迭代至 `third` 的值最多为 `N`，执行以下步骤:
    *   将 `first` 的值更新为 `second` 和 `second` 到 `third` 的值。
    *   将 `first` 和 `second` 的总和存储在变量 `third` 中。
*   如果 `second` 和 `N` 的[绝对](https://www.geeksforgeeks.org/abs-labs-llabs-functions-cc/)差最多是 `second` 和 `N` 的值，那么更新 `ans` 的值为 `second`。
*   否则，将 `ans` 的值更新为 `third`。
