...
给定 `n` 个整数的数组。如果数组所有元素的 GCD 大于 1，则认为该数组是最佳的。如果数组不是最好的，我们可以通过选择一个索引 `i` (`1 <= i < n`) 并分别替换数字 `a_i` 和 `a_{i+1}` 为 `a_i - a_{i+1}` 和 `a_i + a_{i+1}`。找到要在数组上完成的最小操作数，使其达到最佳状态。
...
以下是一个用于寻找“最佳数组”的JavaScript代码示例。
