# 通过顺时针方向旋转第 i 行正好 i 次来修改矩阵

> 原文: [https://www.geeksforgeeks.org/modify-a-matrix-by-rotating-ith-row-exactly-i-times-in-clockwise-direction/](https://www.geeksforgeeks.org/modify-a-matrix-by-rotating-ith-row-exactly-i-times-in-clockwise-direction/)

给定一个尺寸为 `M * N` 的矩阵 `mat[][]`，任务是将矩阵的第 `i` 行顺时针旋转 `i` 次后得到的矩阵打印出来。
...
> 8 9 7
> **说明:**
> 第 0 行旋转 0 次。因此，第 0 行保持为 `{1, 2, 3}`。
> 第 1 行旋转 1 次。因此，第 1 行修改为 `{6, 4, 5}`。
> 第 2 行旋转 2 次。因此，第 2 行修改为 `{8, 9, 7}`。
> 完成上述操作后，给定矩阵修改为 `{{1, 2, 3}, {6, 4, 5}, {8, 9, 7}}`。
>
...
*   以逐行方式遍历给定矩阵，对于每一个第 `i` 行，执行以下步骤:
    *   反转矩阵的当前行。
    *   反转当前行的前 `i` 个元素。
    *   反转当前行的最后 `(N - i)` 个元素，其中 `N` 是当前行的大小。
*   完成以上步骤后，打印矩阵 `mat[][]`。
