# 根据给定的规则找到从点 0 到达点 N 所需的时间

> 原文: [https://www.geeksforgeeks.org/find-time-required-to-reach-point-n-from-point-0-according-to-given-rules/](https://www.geeksforgeeks.org/find-time-required-to-reach-point-n-from-point-0-according-to-given-rules/)

给定两个正整数 `X` 和 `Y` 以及 `N` 个排成一行的点数，任务是根据以下规则找到从 `点0` 到达 `点N` 所需的时间：
...
> **输出:** `28`
> **解释:**
> 从 `点号0` 到 `点号1` 需要 `4` 个时间单位，再多等 `2` 分钟（直到第 `4` 个时间单位屏障在 `3` 个时间单位关闭，必须等待第 `6` 个时间单位才能重新打开）。因此，当前跳跃的总时间是 `6` 个单位。
>
