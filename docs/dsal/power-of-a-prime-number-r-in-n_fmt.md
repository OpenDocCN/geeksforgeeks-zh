...
> 原文：[https://www.geeksforgeeks.org/power-of-a-prime-number-r-in-n/](https://www.geeksforgeeks.org/power-of-a-prime-number-r-in-n/)
...
一个简单的方法是，首先计算 `n` 的阶乘，然后对其进行因子分解，求出一个素数的幂。由于一个数的阶乘是一个大数，上述方法会导致稍大的数溢出。这个想法是考虑一个因子 `n` 的素因子。
