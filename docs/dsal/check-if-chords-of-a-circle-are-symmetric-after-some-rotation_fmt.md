...
> 原文：[https://www.geeksforgeeks.org/check-if-chords-of-a-circle-are-symmetric-after-some-rotation/](https://www.geeksforgeeks.org/check-if-chords-of-a-circle-are-symmetric-after-some-rotation/)
...
旋转范围 `[1, N]` 内的每一个距离 `K`，检查每一个点 `[a, b]` 旋转的点 `[a + K, b + K]` 是否存在。
如果有 `k`，则打印是，否则打印否。
**时间复杂度:** `O(N*M)`

## 有效途径

检查 `N` 的[因子](https://www.geeksforgeeks.org/find-divisors-natural-number-set-1/)就够了。
让我们假设如果我们将图像旋转 `K` 个单位，那么整个图像将被分成 `N/K` 个块。那么如果 `K` 不是 `N` 的除数，就会出现长度小于 `K` 的非对称块，图像永远不会与原图对称。
因此[计算](https://www.geeksforgeeks.org/find-all-divisors-of-a-natural-number-set-2/) `N` 的所有除数，并检查每个和弦是否存在旋转和弦。
