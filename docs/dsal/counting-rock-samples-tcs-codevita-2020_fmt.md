...
> 原文：[https://www.geeksforgeeks.org/counting-rock-samples-tcs-codevita-2020/](https://www.geeksforgeeks.org/counting-rock-samples-tcs-codevita-2020/)

约翰是一名地质学家，他需要对岩石样本进行计数，以便将其送往化学实验室。他有问题。实验室只接受以 ppm（百万分之几）为单位的各种尺寸的岩石样品。约翰需要你的帮助。你的任务是开发一个程序，以获得实验室接受的每个范围内的岩石数量。

## 问题陈述

给定一个表示岩石样本大小的数组 `samples[]` 和一个 2D 数组 `ranges[]`，任务是为每一个可能的 `1 <= i <= N` 计算范围 `ranges[i][0]` 到 `ranges[i][1]` 的岩石样本。

## 示例

**输入：**
`samples[] = {345, 604, 321, 433, 704, 470, 808, 718, 517, 811}`，`ranges[] = {{300, 380}, {400, 700}}`

**输出：**
`2 4`

**解释：**
范围 `[300, 380]`：样本 `{345, 321}` 位于范围内。因此，计数为 2。
范围 `[400, 700]`：样本 `{433, 604, 517, 470}` 位于范围内。因此，计数为 4。

**输入：**
`samples[] = {400, 567, 890, 765, 987}`，`ranges[] = {{300, 380}, {800, 1000}}`
...
想法是针对每个 `ranges[i]` 迭代 `samples[]`，并计算位于指定范围内的样本数量。按照以下步骤解决问题：

*   遍历数组 `ranges[]`。
*   对于每一行 `ranges[i]`，遍历数组 `samples[]` 并计算位于范围 `[ranges[i][0], ranges[i][1]]` 内的岩石样本数量。
