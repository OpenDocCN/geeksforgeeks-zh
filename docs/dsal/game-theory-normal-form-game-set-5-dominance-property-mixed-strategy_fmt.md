...
> 原文: [https://www.geeksforgeeks.org/game-theory-normal-form-game-set-5-dominance-property-mixed-strategy/](https://www.geeksforgeeks.org/game-theory-normal-form-game-set-5-dominance-property-mixed-strategy/)
...
这里 `Minimax` 值不等于 `马希民`，所以这个游戏没有 `鞍点`。现在继续使用优势属性来减少行和列。
...
**注:** 我们还可以将特定行 `X` 的元素与其他两行或更多行的平均值进行比较，如果取平均值后行 `X` 的元素小于或等于相应的元素，则删除行 `X`。**或者**我们还可以将特定列 `X` 的元素与两列或更多列的平均值进行比较，如果取平均值后 `X` 列的元素大于相应的元素，则删除 `X` 列。
