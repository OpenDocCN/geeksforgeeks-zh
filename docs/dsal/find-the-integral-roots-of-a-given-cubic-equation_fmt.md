...
> 原文: [https://www.geeksforgeeks.org/find-the-integral-roots-of-a-given-cubic-equation/](https://www.geeksforgeeks.org/find-the-integral-roots-of-a-given-cubic-equation/)

给定 5 个整数表示 `A`、`B`、`C`、`D` 和 `E`，代表三次方程 `f(x) = A*x^{3} + B*x^{2} + C*x + D = E`，任务是找到这个方程的积分解。如果不存在任何积分解，则打印 `"NA"`。
...
思路是用[二分搜索法](https://www.geeksforgeeks.org/binary-search/)。以下是步骤:

1.  将 `start` 和 `end` 变量分别初始化为 `0` 和 `10^5`。
2.  找到 `start` 和 `end` 的中间(比如 `mid`)值，检查它是否满足给定的等式。
3.  如果当前 `mid` 值满足给定的等式，打印 `mid` 值。
4.  否则如果 `f(x)` 的值小于 `E`，那么更新 `start` 为 `mid + 1`。
5.  否则更新 `end` 为 `mid - 1`。
6.  如果我们找不到上述方程的任何积分解，那么打印 `"NA"`。
