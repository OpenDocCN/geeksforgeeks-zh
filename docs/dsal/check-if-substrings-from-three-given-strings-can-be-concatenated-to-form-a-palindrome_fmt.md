...
> 原文：[https://www.geeksforgeeks.org/check-if-substrings-from-three-given-strings-can-be-concatenated-to-form-a-palindrome/](https://www.geeksforgeeks.org/check-if-substrings-from-three-given-strings-can-be-concatenated-to-form-a-palindrome/)

给定三个字符串 `S1`、`S2` 和 `S3`，其长度分别为 `L`、`M` 和 `N`，任务是检查是否有可能从 `S1`、`S2` 和 `S3` 中选择一些非空的子字符串，使得它们的串联形成一个回文。如果发现是真的，打印“YES”。否则，打印“NO”。
...
> **解释：**
> 可能的方法之一如下：
> 从字符串 `S1` 中选择子字符串“ad”，从字符串 `S2` 中选择“d”，从字符串 `S3` 中选择“a”。因此，结果字符串是 `S = "adda"`，这是一个回文。因此，输出应该是“YES”。
>
> **输入：** `S1 = "a"`, `S2 = "BC"`, `S3 = "c"`
> **输出：** NO
...
2.  迭代字符串 `S1` 的字符，并在 `maskA` 中设置 `(i-'a')` 位，表示字符串 `S1` 中存在相应的字符。
3.  迭代字符串 `S3` 的字符，并在 `maskC` 中设置 `(i-'a')` 位，表示相应的字符出现在字符串 `S3` 中。
4.  如果 `maskA` 和 `maskC` 的位“与”大于零，则打印“YES”。否则，打印“NO”。
