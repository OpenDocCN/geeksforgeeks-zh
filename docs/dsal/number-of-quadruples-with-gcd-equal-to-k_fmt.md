...
> **输入:** `N = 10`，`K = 2`
> **输出:** `5`
> 有效四元组为`(2，4，6，8)`、`(2，4，6，10)`、`(2，4，8，10)`、`(2，6，8，10)`和`(4，6，8，10)`
> **输入:** `N = 8`，`K = 1`
> **输出:** `69`
...
*   `MC<sub>4</sub>`四倍都是可能的总量。`<sup>(M/2)</sup>C<sub>4</sub>`四倍体的 gcd 是 `2` 的倍数。(使用 2 的 M/2 倍数)。同样的，`<sup>(M/3)</sup>C<sub>4</sub>`四倍体的 gcd 是 `3` 的倍数。但是如果两个量都减，那么 `6` 的倍数 gcd 要减两次，所以必须包含`<sup>(M/6)</sup>C<sub>4</sub>`才能加一次。
