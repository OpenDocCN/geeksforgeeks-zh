...
> 原文：[https://www.geeksforgeeks.org/sort-given-string-using-character-search/](https://www.geeksforgeeks.org/sort-given-string-using-character-search/)

给定一个大小为 `n` 的字符串 `str`。问题是在不使用任何排序技术（如[冒泡排序](https://www.geeksforgeeks.org/bubble-sort/)、[选择排序](https://www.geeksforgeeks.org/selection-sort/)等）的情况下对给定字符串进行排序。该字符串仅包含小写字符。
...
在上面的方法中，我们每次都要遍历整个字符串，寻找‘a’到‘z’集合中的每个字符。我们可以通过维护一个字符数组并用字符串中所有字符的出现次数来填充它来克服这个缺点。稍后，我们可以从字符数组中构造所需的排序字符串。
