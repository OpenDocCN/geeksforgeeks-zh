...
> 原文：[https://www.geeksforgeeks.org/sorting-vector-of-pairs-in-c-set-1-sort-by-first-and-second/](https://www.geeksforgeeks.org/sorting-vector-of-pairs-in-c-set-1-sort-by-first-and-second/)

## 什么是对的向量？

一个`pair`是一个存储相互映射的两个值的容器，一个包含多个这样的对的`vector`被称为一个对向量。
...
以对的第一个元素为基础，按升序对向量元素进行排序。这种类型的排序可以使用简单的`sort()`函数来实现。默认情况下，排序函数根据第一个元素对向量元素进行排序。
...
[按第一和第二降序排序](https://www.geeksforgeeks.org/sorting-vector-of-pairs-in-c-set-2-sort-in-descending-order-by-first-and-second/)
