...
> 原文：[https://www.geeksforgeeks.org/generate-a-matrix-having-each-element-equal-to-the-sum-of-specified-submatrices-of-a-given-matrix/](https://www.geeksforgeeks.org/generate-a-matrix-having-each-element-equal-to-the-sum-of-specified-submatrices-of-a-given-matrix/)

## 问题描述

给定一个维度为 `M*N` 的矩阵 `mat[][]` 和一个整数 `K`，任务是生成一个矩阵 `ans[][]`，其中 `ans[i][j]` 等于所有元素 `mat[r][c]` 的和，使得 `r ∈ [i–K, i + K]`，`c ∈ [j - K, j + K]`。
...
**解释：**
`ans[0][0] = mat[0][0] + mat[0][1] + mat[1][0] + mat[1][1] = 1 + 2 + 4 + 5 = 12`
`ans[0][1] = mat[0][0] + mat[0][1] + mat[0][2] + mat[1][0] + mat[1][1] + mat[1][2] = 1 + 2 + 3 + 4 + 5 + 6 = 21`
同样，`ans[0][2] = 16`，`ans[1][0] = 27`，`ans[1][1] = 45`，`ans[1][2] = 33`，`ans[2][0] = 24`，`ans[2][1] = 39`，`ans[2][2] = 28`
...
最简单的方法是使用两个嵌套循环迭代矩阵元素，并计算所有元素的和 `mat[r][c]`（`i-K ≤ r ≤ i+K`，`j-K ≤ c ≤ j+K`），对于每一个可能的 `ans[i][j]`。最后，遍历矩阵后打印矩阵。
...
为了优化上述方法，想法是初始化一个辅助矩阵 `aux[M][N]` 来存储矩阵 `mat[][]` 的所有行的前缀和。一旦构造了 `aux[][]`，`ans[i][j]` 可以通过遍历 `[i-K ≤ r ≤ i+K]` 范围内的所有行并添加 `aux[r][max_col] - aux[r][min_col] + mat[r][min_col]` 来计算。该增加值表示行 `r` 对 `ans[i][j]` 的贡献之和。
...
为了优化上述方法，想法是初始化辅助矩阵 `aux[M][N]`，使得 `aux[i][j]` 使用以下表达式将子矩阵 `mat[0][0]` 至 `mat[i][j]` 中的元素之和存储起来：
