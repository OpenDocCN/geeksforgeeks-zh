...
> 原文: [https://www.geeksforgeeks.org/find-the-maximum-occurring-character-after-performing-the-given-operations/](https://www.geeksforgeeks.org/find-the-maximum-occurring-character-after-performing-the-given-operations/)

给定一个由 `0`、`1` 和 `*` 组成的字符串 `str`，任务是在执行给定操作后，从 `0` 和 `1` 中找出最大出现字符:

> *   将 `*` 替换为 `0`，其中 `*` 出现在字符串中现有 `0` 的左侧。
> *   将 `*` 替换为 `1`，其中 `*` 出现在字符串中现有 `1` 的右侧。
> *   如果 `*` 可以同时被 `0` 和 `1` 替换，则保持不变。
...
**输出:** `0`
**说明:**
由于 `0` 可以代替其左侧的 `*`，而 `1` 可以代替其右侧的 `*`，因此字符串变为 `0001***0`。
...
生成最终结果字符串，然后比较 `0` 和 `1` 的频率的想法。以下是步骤:
