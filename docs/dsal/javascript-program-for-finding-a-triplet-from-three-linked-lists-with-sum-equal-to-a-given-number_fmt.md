...
> 原文:[https://www.geeksforgeeks.org/javascript-program-for-finding-a-triplet-from-three-linked-lists-with-sum-equal-to-a-given-number/](https://www.geeksforgeeks.org/javascript-program-for-finding-a-triplet-from-three-linked-lists-with-sum-equal-to-a-given-number/)
...
2) 在对 `b` 和 `c` 进行排序后，逐个从列表 `a` 中选择一个元素，并通过遍历 `b` 和 `c` 来找到这一对。思路类似于 [3SUM 问题](http://en.wikipedia.org/wiki/3SUM)的二次算法。
...
`时间复杂度:` 链表 `b` 和 `c` 可以使用合并排序在 `O(nLogn)` 时间内排序(参见[本](https://www.geeksforgeeks.org/merge-sort-for-linked-list/))。第二步耗时 `O(n * n)`。所以整体时间复杂度是 `O(nlogn) + O(nlogn) + O(n*n) = O(n*n)`。
