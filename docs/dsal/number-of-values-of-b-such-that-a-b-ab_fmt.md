...
> 原文: [https://www.geeksforgeeks.org/number-of-values-of-b-such-that-a-b-ab/](https://www.geeksforgeeks.org/number-of-values-of-b-such-that-a-b-ab/)

给定一个整数 `a`。求 `b` 的解数，满足方程:
...
一个**天真的解决方案**是从 0 迭代到 `a`，并计算满足给定方程的值的数量。我们只需要遍历到 `a`，因为任何大于 `a` 的值都会给出异或值 > `a`，因此不能满足等式。
