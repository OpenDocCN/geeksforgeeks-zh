...
> 原文: [https://www.geeksforgeeks.org/check-whether-bits-set-given-range/](https://www.geeksforgeeks.org/check-whether-bits-set-given-range/)
...
1.  计算`num = ((1 << r) - 1) ^ ((1 << (l - 1)) - 1)`。这将产生一个数字`num`，其具有`r`数量的比特，并且范围`l`到`r`中的比特是唯一设置的比特。
2.  计算`new_num = n & num`。
3.  如果`num == new_num`，返回`"Yes"`（所有位都设置在给定范围内）。
4.  否则返回`"No"`（并非所有位都设置在给定范围内）。
