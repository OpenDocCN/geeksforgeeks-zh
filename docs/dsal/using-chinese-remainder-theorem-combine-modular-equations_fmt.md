...
> 原文：[https://www.geeksforgeeks.org/using-chinese-remainder-theorem-combine-modular-equations/](https://www.geeksforgeeks.org/using-chinese-remainder-theorem-combine-modular-equations/)
...
在方程``a≅xmod(m<sub>1</sub>* m<sub>2</sub>* ... * m<sub>n</sub>)``中找到 x。
...
1.  ``≅ 3mod(4)``和``≅ 4mod(7)``是我们最初得到的两个方程。让结果方程为``a<sub>0</sub>≅x<sub>0</sub>mod(m<sub>1</sub>* m<sub>2</sub>)``。
    *   ``A<sub>0</sub>``由``m<sub>1</sub>' * m<sub>1</sub>* x<sub>0</sub> + m<sub>0</sub>' * m<sub>0</sub>* x<sub>1</sub>``给出，其中``m<sub>1</sub>' = m<sub>1</sub>``模``m<sub>0</sub>``和``m<sub>0</sub>'``的模逆=模逆。
    *   我们可以使用扩展欧几里德算法计算模逆。
