...
> 原文: [https://www.geeksforgeeks.org/convert-given-upper-triangular-matrix-to-1d-array/](https://www.geeksforgeeks.org/convert-given-upper-triangular-matrix-to-1d-array/)
...
矩阵中索引 `(i, j)` 处的元素位于 `(n * (i - 1) - (((i - 2) * (i - 1)) / 2)) + (j - i)`，其中 `1 ≤ i, j ≤ N` 和 `i ≤ j`。
...
以下公式用于找出非零矩阵元素的正确位置:
矩阵中索引 `(i, j)` 处的元素位于 `(j * (j - 1) / 2) + i - 1` 处，其中 `1 ≤ i, j ≤ N` 和 `i ≤ j`。
