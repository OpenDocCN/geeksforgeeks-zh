...
> 原文: [https://www.geeksforgeeks.org/insert-an-adjacent-k-in-place-for-every-occurrence-of-it-in-the-array-keeping-the-size-of-the-array-intact/](https://www.geeksforgeeks.org/insert-an-adjacent-k-in-place-for-every-occurrence-of-it-in-the-array-keeping-the-size-of-the-array-intact/)

给定一个由 `N` 个整数和一个整数 `K` 组成的数组 `arr`，任务是为它在原始序列中的每次出现插入一个相邻的 `K`，然后使用一个 `O(1)` 辅助空间将数组截断到原始长度。
...
> **输出:** `{1, 0, 0, 2, 3, 0, 0, 4}`
> **解释:**
> 给定数组 `{1, 0, 2, 3, 0, 4, 5, 0}` 修改为 `{1, 0, 0, 2, 3, 0, 0, 4}`。
>
> **输入:** `arr[] = {7, 5, 8}`，`K = 8`
> **输出:** `{7, 5, 8}`
> **解释:**
...
*   由于每个 `K` 需要用两个相邻的 `K` 个条目进行更新，**数组的长度将增加一个数量，该数量等于原始数组 `arr[]` 中存在的 `K` 的数量**。
*   求 `K` 的总数，然后我们假设我们有一个有足够空间容纳每个元素的数组。
*   初始化一个变量 `write_idx`，该变量将指向这个虚拟数组末尾的索引和当前数组末尾的另一个指针 `curr`，即 `arr[N-1]`。
