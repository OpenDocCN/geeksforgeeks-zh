...
> 原文：[https://www.geeksforgeeks.org/flatten-binary-tree-in-order-of-level-order-traversal/](https://www.geeksforgeeks.org/flatten-binary-tree-in-order-of-level-order-traversal/)
...
4.  应用级别顺序遍历。假设`curr`是队列中最前面的元素。然后，
    *   如果`curr`为空，则继续。
    *   否则在队列中推送`curr->left`和`curr->right`。
    *   设置`prev = curr`。
