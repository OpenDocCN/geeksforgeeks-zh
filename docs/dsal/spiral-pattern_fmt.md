...
常见的观察是这样形成的正方形将具有`(2*N-1)x(2*N-1)`的大小。用 `N` 填充第一行和第一列，最后一行和第一列，然后逐渐减少 `N`，类似地填充剩余的行和列。填充 2 行 2 列后，每次减少 `N`。
...
从 `i = 1` 和 `j = 1` 开始索引，可以观察到所需矩阵的每个值都将是 `max(ABS(I–n)，ABS(j–n)+1`。
