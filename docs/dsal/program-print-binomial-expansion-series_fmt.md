...
> 原文: [https://www.geeksforgeeks.org/program-print-binomial-expansion-series/](https://www.geeksforgeeks.org/program-print-binomial-expansion-series/)

给定三个整数，`A`，`X` 和 `n`，任务是打印下面二项式表达式系列的术语。
`(A+X)^n = nC0*A^n*X^0 + nC1*A^(n-1)*X^1 + nC2*A^(n-2)*X^2 + ... + nCn*A^0*X^n`
...
我们知道，对于 `n` 的每个值，二项式级数中都会有 `(n+1)` 项。所以现在我们使用一个简单的方法，计算系列中每个元素的值并打印出来。

```
nCr = (n!) / ((n-r)! * (r)!)

Below is value of general term. 
Tr+1 = nCn-r * A^(n-r) * X^r
