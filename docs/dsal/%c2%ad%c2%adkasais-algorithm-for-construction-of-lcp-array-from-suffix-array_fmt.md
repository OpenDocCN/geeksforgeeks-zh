# Kasai算法：从后缀数组构造LCP数组

> 原文: [https://www.geeksforgeeks.org/kasais-algorithm-for-construction-of-lcp-array-from-suffix-array/](https://www.geeksforgeeks.org/kasais-algorithm-for-construction-of-lcp-array-from-suffix-array/)
...
这里讨论的基于二分搜索法的解决方案花费 `O(m*Logn)` 时间，其中 `m` 是要搜索的模式的长度，`n` 是文本的长度。借助 LCP 数组，我们可以在 `O(m + Log n)` 时间内搜索到一个模式。例如，如果我们的任务是在“banana”中搜索“ana”，则 `m = 3`，`n = 5`。

***LCP 数组是一个大小为 n 的数组（类似后缀数组）。值 `lcp[i]` 表示由 `suffix[i]` 和 `suffix[i+1]` 索引的后缀中最长公共前缀的长度。*** `suffix[n-1]` 没有定义，因为后面没有后缀。
...
LCP 数组的构建有两种方式:
1.  计算 LCP 数组作为后缀数组的副产品（Manber & Myers算法）。
...
我们首先计算文本中第一个后缀为“香蕉”的 LCP。我们需要后缀数组中的下一个后缀来计算 LCP（记住 `lcp[i]` 定义为 `suffix[i]` 和 `suffix[i+1]` 的最长公共前缀）。要在 `suffixArr` 中找到下一个后缀，我们使用 `sufixar[]`。下一个后缀是“na”。由于“香蕉”和“na”之间没有共同的前缀，“香蕉”的 LCP 值为 0，在后缀数组中位于索引 3，因此我们将 `lcp[3]` 填充为 0。

接下来我们计算第二个后缀“anana”的 LCP。后缀数组中“anana”的下一个后缀是“banana”。由于没有公共前缀，所以“anana”的 LCP 值为 0，位于后缀数组的索引 2 处，因此我们将 `lcp[2]` 填充为 0。

接下来我们计算第三个后缀“nana”的 LCP。由于没有下一个后缀，所以没有定义“nana”的 LCP 值。我们将 `lcp[5]` 填充为 0。

文本中的下一个后缀是“ana”。后缀数组中“ana”的下一个后缀是“anana”。因为有一个长度为 3 的公共前缀，所以“ana”的 LCP 值为 3。我们将 `lcp[1]` 填充为 3。

现在我们处理文本中的下一个后缀“na”。这就是 Kasai 的算法使用 LCP 值必须至少为 2 的技巧的地方，因为以前的 LCP 值是 3。由于“na”后没有字符，LCP 的最终值为 2。我们将 `lcp[4]` 填充为 2。
...
- [http://www.mi.fu-berlin.de/wiki/pub/ABI/RnaSeqP4/suffix-array.pdf](http://www.mi.fu-berlin.de/wiki/pub/ABI/RnaSeqP4/suffix-array.pdf)
- [http://codeforces.com/blog/entry/12796](http://codeforces.com/blog/entry/12796)

本文由普拉哈尔·阿格沃尔供稿。如果你发现任何不正确的地方，请写评论，或者你想分享更多关于上面讨论的话题的信息。