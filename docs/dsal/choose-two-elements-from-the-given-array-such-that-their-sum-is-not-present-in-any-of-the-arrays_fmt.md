...
> 原文: [https://www.geeksforgeeks.org/choose-two-elements-from-the-given-array-such-that-their-sum-is-not-present-in-any-of-the-arrays/](https://www.geeksforgeeks.org/choose-two-elements-from-the-given-array-such-that-their-sum-is-not-present-in-any-of-the-arrays/)
...
> **输出:** `3 9`
> `3 + 9 = 12`，而`12`不存在于任何给定的数组中。
>
> **输入:** `A[] = {1, 3, 5, 7}`，`B[] = {7, 5, 3, 1}`
> **输出:** `7 7`

**思路:** 从`A[]`中选择`X`作为最大元素，从`B[]`中选择`Y`作为最大元素。现在，很明显`(X + Y)`将大于两个数组的最大值，即它不会出现在任何数组中。
