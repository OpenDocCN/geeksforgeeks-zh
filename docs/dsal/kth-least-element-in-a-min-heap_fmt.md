...
> 原文：[https://www.geeksforgeeks.org/kth-least-element-in-a-min-heap/](https://www.geeksforgeeks.org/kth-least-element-in-a-min-heap/)

给定一个大小为 `n` 的最小堆，在最小堆中找到第 `k` 个最小的元素。
...
我们可以从[最小堆](https://www.geeksforgeeks.org/binary-heap/)中提取最小元素 `k` 次，最后提取的元素将是第 `k` 个最小元素。每次删除操作需要 `O(log n)` 的时间，因此这种方法的总时间复杂度为 `O(k * log n)`。
...
我们可以注意到一个关于最小堆的有趣观察。`i` 级的元素 `x` 有 `i–1` 个祖先。根据最小堆的属性，这些 `i-1` 祖先保证小于 `x`。这意味着 `x` 不能是堆中最小的 `i-1` 个元素。利用这个属性，我们可以得出结论，第 `k` 个最小元素最多可以有 `k` 个级别。

我们可以减小最小堆的大小，使它只有 `k` 个级别。然后，我们可以通过之前提取 `k` 次最小元素的策略来获得第 `k` 个最小元素。请注意，堆的大小减少到最大 `2^k – 1`，因此每个堆操作将花费 `O(log 2^k) = O(k)` 时间。总时间复杂度为 `O(k^2)`。如果 `n >> k`，那么这种方法的性能会比前一种更好。
