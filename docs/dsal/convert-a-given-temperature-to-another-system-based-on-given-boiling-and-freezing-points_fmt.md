...
> 原文：[https://www.geeksforgeeks.org/convert-a-given-temperature-to-another-system-based-on-given-boiling-and-freezing-points/](https://www.geeksforgeeks.org/convert-a-given-temperature-to-another-system-based-on-given-boiling-and-freezing-points/)

有两个温度计，给定五个整数 `F1`、`B1`、`F2`、`B2` 和 `T`，其中 `F1` 和 `B1` 是**温度计 1** 上的水的冰点和沸点，`F2` 和 `B2` 是**温度计 2** 上的水的冰点和沸点。任务是在温度计 2 上找到温度。
...
考虑第一个温度计采用 `U1` 单位制，第二个温度计采用 `U2` 单位制。
