...
> 原文：[https://www.geeksforgeeks.org/largest-substring-of-str2-which-is-a-prefix-of-str1/](https://www.geeksforgeeks.org/largest-substring-of-str2-which-is-a-prefix-of-str1/)

给定两个字符串`str1`和`str2`，任务是找到`str1`的最长前缀，该前缀作为字符串`str2`的子串出现。如果可能，打印前缀，否则打印-1。

## 举例：

> **输入：** `str1 = "geeksfor"`, `str2 = "forgeeks"`
> **输出：** `geeks`
> `str2`中出现的`str1`的所有前缀都是`"g"`、`"ge"`、`"gee"`、`"geek"`和`"geeks"`。
> **输入：** `str1 = "abc"`, `str2 = "def"`
> **输出：** `-1`

## 方法：

检查`str1`是否作为子串出现在`str2`中。如果是，则`str1`是所需字符串，否则从`str1`中删除最后一个字符，并重复这些步骤，直到字符串`str1`变为空或找到所需字符串。
