# 求子阵列[i，N-1]中每个索引 I 的最小子阵列和

> 原文: [https://www.geeksforgeeks.org/find-minimum-subarray-sum-for-each-index-i-in-subarray-i-n-1/](https://www.geeksforgeeks.org/find-minimum-subarray-sum-for-each-index-i-in-subarray-i-n-1/)

给定一个大小为 `N` 的数组 `arr[]`，任务是为 `[0，N-1]` 中的所有 `i` 找到子数组 `[i，N-1]` 中的最小子数组和。
...
**方法:** 使用标准的[Kadane最大子数组和](https://www.geeksforgeeks.org/largest-sum-contiguous-subarray/)算法可以解决这个问题。按照以下步骤解决此问题:

*   [反转数组 `arr[]` 的元素](https://www.geeksforgeeks.org/how-to-invert-the-elements-of-a-boolean-array-in-python/)，即将正号更改为负号，反之亦然。
*   使用变量 `i` : 在范围 `[0，N-1]` 中迭代
    *   使用[Kadane算法](https://www.geeksforgeeks.org/largest-sum-contiguous-subarray/)求子数组 `arr[i，N-1]` 的[最大子数组和](https://www.geeksforgeeks.org/largest-sum-contiguous-subarray/)。
    *   再次将结果反相，打印结果。
