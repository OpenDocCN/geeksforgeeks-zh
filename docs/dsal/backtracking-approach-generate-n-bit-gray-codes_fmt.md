...
> 原文: [https://www.geeksforgeeks.org/backtracking-approach-generate-n-bit-gray-codes/](https://www.geeksforgeeks.org/backtracking-approach-generate-n-bit-gray-codes/)
...
我们在[中讨论了一种生成 `n` 位格雷码](https://www.geeksforgeeks.org/given-a-number-n-generate-bit-patterns-from-0-to-2n-1-so-that-successive-patterns-differ-by-one-bit/)的方法，本文提供了一种**回溯方法**来解决同样的问题。我们的想法是，对于 `n` 位中的每一位，我们可以选择忽略它，或者反转该位，这意味着对于 `n` 位，我们的灰度序列高达 `2^n`。因此，我们进行两次递归调用，要么反转位，要么保持位不变。
