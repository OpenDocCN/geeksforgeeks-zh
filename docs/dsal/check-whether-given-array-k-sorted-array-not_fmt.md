...
> 原文：[https://www.geeksforgeeks.org/check-whether-given-array-k-sorted-array-not/](https://www.geeksforgeeks.org/check-whether-given-array-k-sorted-array-not/)
...
排序 `aux[]`。现在，对于 `arr[]` 中索引 `i` 处的每个元素，使用二分搜索法查找其在 `aux[]` 中的索引 `j`。如果对于任何元素 `k < abs(i-j)`，则 `arr[]` 不是 `k` 排序数组。否则就是一个 `k` 排序数组。这里 `abs` 是绝对值。
...
**另一种方法**可以是**将相应的元素索引存储到辅助数组**中。然后简单检查一下**`abs(i - aux[i].second) <= k`**，不满足条件**返回“否”**。它比上面提到的方法稍快，因为**我们不必执行二分搜索法来检查与原始索引的距离**，虽然**时间复杂度**保持不变。
