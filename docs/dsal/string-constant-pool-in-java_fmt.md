...
在这种情况下，两个字符串对象都在堆栈中创建，但是 `"Hello"` 值的另一个实例没有在堆中创建。相反，`"Hello"` 的前一个实例被重复使用。字符串常量池是一个驻留在堆中的小缓存。Java 将所有的值存储在直接分配的字符串常量池中。这样，如果需要再次访问类似的值，在堆栈中创建的新字符串对象可以在指针的帮助下直接引用它。换句话说，字符串常量池的存在主要是为了减少内存使用，提高内存中现有实例的重用。当字符串对象被赋予不同的值时，新值将作为单独的实例注册到字符串常量池中。让我们用下面的例子来理解这一点：
