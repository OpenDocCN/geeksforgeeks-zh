...
> 原文: [https://www.geeksforgeeks.org/smallest-integer-with-digit-sum-m-and-multiple-of-n/](https://www.geeksforgeeks.org/smallest-integer-with-digit-sum-m-and-multiple-of-n/)
...
**逼近:**
从 `N` 开始，迭代 `N` 的所有倍数，检查其位数和是否等于 `m`。这个问题可以在 `log_n(INT_MAX) * log_10(INT_MAX)` 时间解决。这种方法的效率可以通过从一个至少有 `m/9` 位数的数字开始迭代来提高。
