...
> 原文：[https://www.geeksforgeeks.org/java-program-for-smallest-k-digit-number-divisible-by-x/](https://www.geeksforgeeks.org/java-program-for-smallest-k-digit-number-divisible-by-x/)
...
计算 `MIN`：最小的 `K` 位数（`1000...K-times`）。
如果 `MIN % X` 为 0，则答案 `ans = MIN`。
否则，`ans = (MIN + X) - ((MIN + X) % X)`。
这是因为在范围 `[MIN...MIN+X]` 内会有一个数能被 `X` 整除。
