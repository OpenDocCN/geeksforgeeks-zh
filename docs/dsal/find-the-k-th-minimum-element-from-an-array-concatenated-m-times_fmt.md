...
> 原文：[https://www.geeksforgeeks.org/find-the-k-th-minimum-element-from-an-array-concatenated-m-times/](https://www.geeksforgeeks.org/find-the-k-th-minimum-element-from-an-array-concatenated-m-times/)

给定一个数组 `arr[]` 和两个整数 `K` 和 `M`。问题是在将数组连接到自身 `M` 次后找到第 `K` 个最小元素。
...
如果 `M` 的值很小，上述方法可以很好地工作，但是对于 `M` 的值很大，会产生内存错误或超时错误。
想法是观察给定数组排序后，当我们将它串联 `M` 次并再次排序时，每个元素现在都会在新的串联数组中出现 `M` 次。所以，

1.  给定数组按升序排序。
2.  返回给定数组 `arr` 的索引 `( (K-1) / M )` 处的值，即返回 `arr[((K-1) / M)]`。
