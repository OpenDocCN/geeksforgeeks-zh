...
为了实现双端队列，我们​​需要跟踪两个指针，即`front`和`rear`。我们**从队列的后端或前端放入（推入）**，**从队列的后端和前端弹出（弹出）**。
...
诸如`insertFront()`，`insertRear()`，`deleteFront()`，`deleteRear()`之类的操作的时间复杂度为`O(1)`。`erase()`的时间复杂度为`O(n)`。
