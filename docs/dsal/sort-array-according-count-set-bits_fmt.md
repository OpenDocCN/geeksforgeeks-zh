...
> 原文：[https://www.geeksforgeeks.org/sort-array-according-count-set-bits/](https://www.geeksforgeeks.org/sort-array-according-count-set-bits/)
...
使用 `std::sort` 的自定义比较器根据设置的位计数对数组进行排序。
...
1.  创建一个大小为 32 的向量 `count`。每个计数单元，即 `count[i]` 是另一个向量，它存储其设定位计数为 `i` 的所有元素。
2.  遍历数组，并对每个元素执行以下操作：
    1.  计算这个元素的设置位数。让它成为 `setbitcount`。
    2.  `count[setbitcount].push_back(element)`。
3.  以相反的方式遍历 `count`（因为我们需要以非递增的顺序排序）并修改数组。
...
本文由**尼克尔·查克拉瓦图拉**供稿，经**阿肖克·卡瓦**修改。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用`write.geeksforgeeks.org`写一篇文章或者把你的文章邮寄到`review-team@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
