...
> 原文: [https://www.geeksforgeeks.org/sum-of-bitwise-or-of-all-possible-subsets-of-given-set/](https://www.geeksforgeeks.org/sum-of-bitwise-or-of-all-possible-subsets-of-given-set/)
...
是取数组 `arr[]` 元素的所有可能的 `OR` 组合，然后执行所有值的求和。这种方法的时间复杂度呈指数增长，因此对于大的 `n` 值来说不是更好的方法。

## 一种高效的方法

是找到关于 `or` 属性的模式。现在再次考虑二进制形式的子集，如:
...
现在，考虑所有结果 `ORs` 中的第 `i`th 位，只有当子集中元素的第 `i`th 位都为 `0` 时，它才为零。

第 `i` 位子集的数量 = 所有可能子集的总数 – 所有第 `i` 位为 `0` 的子集。这里，全部子集 = `2^n – 1`，所有第 `i` 位为 `0` 的子集 = 数组所有元素的第 `i` 位的 `2^(零计数) - 1`。现在，总子集 `OR` 具有第 `i` 位 = `(2^n - 1) - (2^(count) - 1)`。值为 `1` 的位所贡献的总值 = 总子集或值为 `1` 的位 * `(2^i)`。

现在，总和 = (具有第 `0` 位的总子集) * `2^0` + (具有第 `1` 位的总子集) * `2^1` + …… + (具有第 `31` 位的总子集) * `2^31`。
