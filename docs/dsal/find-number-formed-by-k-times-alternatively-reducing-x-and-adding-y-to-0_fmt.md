...
> 原文: [https://www.geeksforgeeks.org/find-number-formed-by-k-times-alternatively-reducing-x-and-adding-y-to-0/](https://www.geeksforgeeks.org/find-number-formed-by-k-times-alternatively-reducing-x-and-adding-y-to-0/)
...
### 天真法
给定的问题可以通过执行给定的操作 `K` 次数并打印得到的结果来解决。

**时间复杂度:** `O(K)`
**辅助空间:** `O(1)`

### 高效方法
上述方法也可以通过查找 `K` 移动次数中递减(使用 `X` 的值)和递增(使用 `Y` 的值)的总值，然后打印这些值的总和作为结果来优化。
