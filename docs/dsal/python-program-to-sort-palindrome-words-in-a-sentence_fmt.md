...
> 原文: [https://www.geeksforgeeks.org/python-program-to-sort-palindrome-words-in-a-sentence/](https://www.geeksforgeeks.org/python-program-to-sort-palindrome-words-in-a-sentence/)
...
> **输入:** `S` = “Please refer to the madam to know the level”
> **输出:** please level to the madam to know the refer
> **说明:** 这里的 “level”、“madam”、“refer” 都是回文词。对它们进行排序会生成序列{“level”、“madam”、“refer”}。
>
> **输入:** `S` = “refer dad”
> **输出:** dad refer
...
*   将句子 `S` 中出现的所有[回文单词](https://www.geeksforgeeks.org/remove-palindromic-words-given-sentence/)存储在一个列表中，比如说 `newlist[]`。
*   使用 [`sort()`](https://www.geeksforgeeks.org/python-list-sort/) 功能对列表 `newlist` 进行排序。
*   初始化一个指针，说 `j = 0`。
*   [遍历列表](https://www.geeksforgeeks.org/iterate-over-words-of-a-string-in-python/) `lis`，将所有[回文](https://www.geeksforgeeks.org/java-program-count-number-palindrome-words-sentence/)替换为 `newlist[j]`，并将 `j` 增加 `1`。
