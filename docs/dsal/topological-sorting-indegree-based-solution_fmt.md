...
> **输出:** 5 4 2 3 1 0
> **解释:** DAG 的拓扑排序是按照这样的顺序进行的，即对于每个有向边 `uv`，顶点 `u` 在排序中位于 `v` 之前。5 没有传入边。4 没有输入边，2 和 0 有来自 4 和 5 的输入边，最后放置 1。
>
> **输入:**
...
已经讨论了一个基于深度优先搜索的解决方案来寻找拓扑排序。

**解决方案:** 在本文中，我们将看到另一种方法来寻找有向无环图（DAG）中顶点的线性顺序。该方法基于以下事实：
**一个 DAG `G` 至少有一个入度为 0 的顶点和一个出度为 0 的顶点**。
...
1.  取一个将跟踪的度内数组，遍历数组的边，简单地将目的节点的计数器增加 1。
