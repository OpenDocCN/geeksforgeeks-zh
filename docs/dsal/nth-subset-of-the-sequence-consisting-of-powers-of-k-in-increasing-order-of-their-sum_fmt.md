...
> 原文: [https://www.geeksforgeeks.org/nth-subset-of-the-sequence-consisting-of-powers-of-k-in-increasing-order-of-their-sum/](https://www.geeksforgeeks.org/nth-subset-of-the-sequence-consisting-of-powers-of-k-in-increasing-order-of-their-sum/)

给定两个整数 `N` 和 `K`，任务是从 K 的幂生成的子集序列中找到第 `N` 个子集，即 `{1, K^1, K^2, K^3, …..}`，这样子集按其和的递增顺序排列。
...
按照以下步骤解决问题:
*   计算小于或等于 `N` 的最近的 2 的幂，记为 `p`。因此，`p = log₂(N)`。
...
*   将计数 `count` 和 `x` 初始化为 0。此外，一个存储子集元素的向量。
*   当 `n` 大于 0 时，执行以下操作。
    *   设置 `x = n & 1`，用于查找数字的最后一位是否被设置。
    *   如果 `x` 不是 0，现在将元素 `K^count` 计入子集。
