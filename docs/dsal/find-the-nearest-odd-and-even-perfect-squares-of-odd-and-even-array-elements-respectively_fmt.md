...
> 原文：[https://www.geeksforgeeks.org/find-the-nearest-odd-and-even-perfect-squares-of-odd-and-even-array-elements-respectively/](https://www.geeksforgeeks.org/find-the-nearest-odd-and-even-perfect-squares-of-odd-and-even-array-elements-respectively/)
...
按照以下步骤解决问题：
- 遍历数组并执行以下操作：
    - 找到当前数组元素的平方根并将其存储在一个变量中，比如 `sr`。
    - 如果 `sr` 与 `arr[i]` 相同，那么 `sr*sr` 就是最近的完美方块。
    - 否则，`(sr + 1)^2` 就是最近的完美方块。
    - 打印上一步中获得的最近的完美正方形。
