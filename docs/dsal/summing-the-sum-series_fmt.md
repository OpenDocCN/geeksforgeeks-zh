...
定义了一个函数，该函数将第一个 `N` 个自然数之和的乘以和(`N`)。你的任务是将函数修改为 `sumX(N, M, K)` 计算 `和(K + 和(K + 和(K+…和(K+N)……)))`，继续 `M` 项。对于给定的 `N`、`M` 和 `K` 计算 `sumX(N, M, K)` 的值。

**注意:** 由于答案可能会很大，所以在中打印答案取模 `10^9 + 7`。
...
*   运行循环 `M` 次，每次将 `K` 加到前面的答案上，并应用 `和(prev_ans + K)` ，每次取 `10^9 + 7` 的模。
*   最后打印 `sumX(N, M, K)` 的值。
