# 查询统计给定范围内具有单个设置位的数组元素

> 原文：[https://www.geeksforgeeks.org/queries-to-count-array-elements-from-a-given-range-having-a-single-set-bit/](https://www.geeksforgeeks.org/queries-to-count-array-elements-from-a-given-range-having-a-single-set-bit/)

给定一个由正整数组成的数组`arr[]`和一个由查询组成的数组`Q[][]`，每个第`i`次查询的任务是从只有一个设置位的范围`[Q[i][0], Q[i][1]]`中计数数组元素。

## 示例

> **输入：** `arr[] = {12, 11, 16, 8, 2, 5, 1, 3, 256, 1}`，`queries[][] = {{0, 9}, {4, 9}}`
> **输出：** `6 4`
> **解释：**
> 在索引`[0, 9]`的范围内，元素`arr[2]`（`= 16`）、`arr[3]`（`= 8`）、`arr[4]`（`= 2`）、`arr[6]`（`= 1`）、`arr[8]`（`= 256`）和`arr[9]`（`= 1`）只有1个设置位。
> 在`[4, 9]`范围内，元素`arr[4]`（`= 2`）、`arr[6]`（`= 1`）、`arr[8]`（`= 256`）、`arr[9]`（`= 1`）只有1个设置位。

> **输入：** `arr[] = {2, 1, 101, 11, 4}`，`queries[][] = {{2, 4}, {1, 4}}`
> **输出：** `1 2`

## 天真方法

每个查询最简单的方法是迭代范围`[l, r]`，并使用[布莱恩·克尼根的算法](https://www.geeksforgeeks.org/count-set-bits-in-an-integer/)计算只有一个集合位的数组元素的数量。
...
*   第`i`个索引存储到第`i`个索引为止只有一个设置位的数组元素的计数。
