...
> 原文：[https://www.geeksforgeeks.org/check-if-an-array-element-is-concatenation-of-two-elements-from-another-array/](https://www.geeksforgeeks.org/check-if-an-array-element-is-concatenation-of-two-elements-from-another-array/)

给定两个分别由 `N` 和 `M` 个正整数组成的数组 `arr[]` 和 `brr[]`，任务是找到数组 `brr[]` 中的所有元素，这些元素等于数组 `arr[]` 中任意两个元素的串联。如果不存在这样的元素，则打印 `-1`。
...
解决问题最简单的方法是从给定的数组中生成所有可能的对，并检查数组 `arr[]` 中的元素对的连接是否存在于数组 `brr[]` 中。如果发现是真的，则打印形成的串联数字。
...
上述方法可以通过检查数组 `brr[]` 中的每个元素 `brr[i]` 是否可以分为 2 个部分 `左` 和 `右`，使得这两个部分都存在于数组 `arr[]` 中。
...
*   初始化一个 `HashMap` `M` 并存储数组 `arr[]` 中存在的所有元素。
*   遍历数组 `brr[]` 并执行以下步骤：
    *   生成 `左侧` 和 `右侧` 零件的所有可能组合，以便它们的连接结果为 `brr[i]`。
    *   如果在上述组合之一中的 `地图` `M` 中同时存在 `左侧` 和 `右侧` 零件，则打印 `brr[i]` 的值。否则，继续下一次迭代。
