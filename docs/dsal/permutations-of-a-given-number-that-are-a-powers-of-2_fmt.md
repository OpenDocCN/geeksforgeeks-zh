...
> 原文: [https://www.geeksforgeeks.org/permutations-of-a-given-number-that-are-a-powers-of-2/](https://www.geeksforgeeks.org/permutations-of-a-given-number-that-are-a-powers-of-2/)

给定一个由 `N` 个数字组成的字符串 `S`，任务是打印 `S` 个数字的所有可能组合，这是 2 的完美幂。
...
给定的问题可以通过回溯来解决。想法是生成字符串 `S` 的所有可能排列，如果是 2 的完美幂，那么打印出来。按照以下步骤解决问题:

*   定义函数 `check(int number)` 以检查给定的 `number` 是否为 2 的幂，并执行以下任务:
    *   如果 `number` 等于 `0`，则返回 `false`。
    *   如果 `number` 和 `number-1` 的位与运算结果为 0，则返回 `true`，否则返回 `false`。
*   定义一个函数 `calculate(int arr[], string ans)` 并执行以下任务:
    *   如果字符串 `ans` 的长度不等于 `0` 且函数 `check(Integer.parseInt(ans.trim()))` 的值返回 `true`，则将该值添加到 HashSet `H` 中。
    *   使用变量 `i` 迭代一个范围 `[0, 10]`，并执行以下任务:
        *   如果 `arr[i]` 等于 `0`，则 `continue`。
        *   否则，通过 `1` 减少 `arr[i]` 的值，调用函数 `calculate(temp, ans + to_string(i))` 找到字符串 `S` 的其他可能排列，并通过 `1` 增加 `arr[i]` 的值。
*   初始化一个 HashSet `H` 来存储可能的 2 的幂的数字字符串。
*   初始化一个数组，比如 `temp[]`，来存储字符串 `S` 中整数的频率。
*   循环迭代直到 `N` 不等于 `0`，执行以下步骤:
    *   将变量 `rem` 初始化为 `N % 10`，并将 `temp[rem]` 的值增加 `1`。
    *   将 `N` 的值除以 `10`。
*   调用函数 `calculate(temp, "")` 找到字符串 `S` 的可能排列。
*   执行上述步骤后，打印哈希表作为结果。
...
### 【高效方法】
想法是最初生成 2 的所有幂，并将它们存储在适当的数据结构中(在这种情况下为“CPP 的集合”)，并检查字符串中是否存在 2 的幂。
