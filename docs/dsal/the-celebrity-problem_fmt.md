...
我们可以把输入的问题描述为代表聚会中人物的数字/字符数组。我们还有一个假设函数 `hasQuantity(A, B)`，如果 A 知道 B，则返回 `true`，否则返回 `false`。我们如何解决这个问题。
...
### 方法:
使用图形建模解决方案。将每个顶点的内倾角和外倾角初始化为 0。如果甲知道乙，画一条从甲到乙的有向边，把乙的度数和甲的度数增加 1。为每个可能的对 `[i, j]` 构造图的所有可能的边。有 `N C2` 对。如果聚会中有一个名人，图中会有一个外倾角为零、内倾角为 `N-1` 的汇聚节点。

### 算法:

1.  创建两个数组 `indegree` 和 `outdegree`，以存储增量和增量。
2.  运行嵌套循环，外环从 0 到 `n`，内环从 0 到 `n`。
3.  对于每一对 `i, j` 检查 `i` 是否知道 `j`，然后增加 `i` 的外倾角和 `j` 的内倾角。
...
上述方法使用递归在 `n` 个人中寻找潜在名人，递归调用 `n-1` 个人，直到达到 `0` 个人的基本情况。对于 `0` 个人-返回 `-1`，表示没有可能的名人，因为有 `0` 个人。在递归的第 `I` 阶段，比较第 `I` 个人和第 `(i-1)` 个人，以检查他们中是否有人知道对方。使用上面的逻辑(在要点中)，潜在的名人回到第 `(i+1)` 阶段。
...
2.  运行一个循环，直到 `i` 小于 `j`。
3.  如果 `i` 认识 `j`，`i` 就不能成为名人。所以增量 `i`，也就是 `i++`
...
1.  写代码找名人。不要使用任何数据结构，如图形、堆栈等……您只能访问 `N` 和`hasQuality(int, int)`。
2.  使用队列实现算法。你的观察是什么？将您的解决方案与[在数组中查找最大值和最小值](https://www.geeksforgeeks.org/maximum-and-minimum-in-an-array/)和[锦标赛树](https://www.geeksforgeeks.org/tournament-tree-and-binary-heap/)进行比较。我们需要的最少比较次数是多少(打给`N`的最佳电话数是多少？