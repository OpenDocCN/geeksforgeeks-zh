...
> 原文：[https://www.geeksforgeeks.org/find-the-node-with-maximum-value-in-a-binary-search-tree/](https://www.geeksforgeeks.org/find-the-node-with-maximum-value-in-a-binary-search-tree/)

给定一个二叉查找树（`BST`），任务是找到 `BST` 中具有最大值的节点。
...
这个挺简单的。只需递归地从根到右遍历节点，直到右侧为空。右边为空的节点是值最大的节点。
