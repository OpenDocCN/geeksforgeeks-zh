...
1)  Repeat following k times:
      ...
      c) If a^(n-1) ≠ 1 (mod n), then return false
...
注意，即使我们增加迭代次数(更高的`k`)，上述方法也可能失败。存在一些复合数，其性质是每一个`a < n`，`gcd(a，n) = 1`和`a <sup>n-1</sup> ≡ 1 (mod n)`。这样的数字被称为[卡迈克尔数字](https://en.wikipedia.org/wiki/Carmichael_number)。如果需要快速方法进行过滤，例如在RSA公钥密码算法的密钥生成阶段，通常会使用费马的素性测试。
