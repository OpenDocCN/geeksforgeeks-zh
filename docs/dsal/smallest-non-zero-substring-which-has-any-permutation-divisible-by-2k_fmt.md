...
> 原文：[https://www.geeksforgeeks.org/smallest-non-zero-substring-which-has-any-permutation-divisible-by-2k/](https://www.geeksforgeeks.org/smallest-non-zero-substring-which-has-any-permutation-divisible-by-2k/)

给定一个长度为 `N` 的二进制字符串 `S` 和一个整数 `K`，任务是找到 `S` 的最小非零子串，该子串可以被混在一起生成一个可被 `2^K` 整除的二进制字符串。如果不存在这样的子字符串，则打印 `-1`。**注意**：`K` 始终大于 `0`。
