...
> 原文: [https://www.geeksforgeeks.org/costs-required-to-move-all-1s-to-each-index-of-a-given-binary-array/](https://www.geeksforgeeks.org/costs-required-to-move-all-1s-to-each-index-of-a-given-binary-array/)

给定一个**二进制数组**，其中，将一个元素从索引 `i` 移动到索引 `j` 需要 `ABS(i–j)` 成本。任务是找到将所有 `1` 移动到给定数组的每个索引的成本。
...
**朴素方法:** 解决这个问题最简单的方法是遍历给定数组，对于给定数组的每个数组元素，打印将给定数组的所有 `1` 移动到当前索引的成本。
...
**高效方法:** 为了优化上述方法，思路是遍历给定数组并使用前缀求和技术找到从给定数组的每个索引的左侧和右侧移动所有 `1` 的成本。按照以下步骤解决问题:

*   初始化一个数组，说 `cost[]` 来存储在给定数组的每个索引处移动所有 `1` 的成本。
*   初始化两个变量，说 `costLeft`、`costRight` 存储成本，分别从每个索引的左侧和右侧移动所有 `1`。
*   从左到右遍历给定数组，将 `costLeft` 的值增加左侧的 `1` 的数量，将 `cost[i]` 的值增加 `costLeft`。
*   从右到左遍历给定数组，将 `costRight` 的值增加右侧 `1` 的个数，将 `cost[i]` 的值增加 `costRight`。
*   最后打印 `cost[]` 数组。
