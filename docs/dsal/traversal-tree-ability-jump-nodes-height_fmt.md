...
想法是使用 [DFS](https://www.geeksforgeeks.org/tree-traversals-inorder-preorder-and-postorder/) 创建节点的高度邻接表并存储父级。 然后使用另一个 DFS 计算使用以下`dp`状态可以达到的最大特殊节点数：
...
`O(n * n * k)`（最坏的情况，例如：2 层树，其根具有`n-1`个子节点）