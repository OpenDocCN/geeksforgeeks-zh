...
> 原文：[https://www.geeksforgeeks.org/smallest-number-divisible-first-n-numbers/](https://www.geeksforgeeks.org/smallest-number-divisible-first-n-numbers/)
...
如果你仔细观察，答案一定是数字 1 到 `n` 的 **LCM**。
寻找从 1 到 `n` 的数的 LCM：

1.  初始化 `ans = 1`。
2.  迭代从 `i = 1` 到 `i = n` 的所有数字。在第 `i` 次迭代中，`ans = LCM(1, 2, ….., i)`。这可以通过 `LCM(1, 2, …, i) = LCM(ans, i)` 轻松完成。
    因此在第一次迭代中，我们只需要做：
