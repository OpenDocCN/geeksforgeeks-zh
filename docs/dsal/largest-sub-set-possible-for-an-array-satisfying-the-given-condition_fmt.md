...
> 原文: [https://www.geeksforgeeks.org/largest-sub-set-possible-for-an-array-satisfying-the-given-condition/](https://www.geeksforgeeks.org/largest-sub-set-possible-for-an-array-satisfying-the-given-condition/)

给定一个数组 `arr[]` 和一个整数 `K`。任务是找到最大子集的大小，使得子集 `(X, Y)` 中的每一对都是 `Y != (X * K)` 的形式，其中 `X < Y`。
