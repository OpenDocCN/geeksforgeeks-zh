...
> 原文：[https://www.geeksforgeeks.org/find-minimum-value-of-y-for-the-given-x-values-in-q-queries-from-all-the-given-set-of-lines/](https://www.geeksforgeeks.org/find-minimum-value-of-y-for-the-given-x-values-in-q-queries-from-all-the-given-set-of-lines/)
...
天真法是将每一行的 `x` 值代入，计算所有行的最小值。对于每个查询，将花费 `O(N)` 个时间，因此解决方案的复杂性变为 `O(Q * N)`，其中 `N` 是行数，`Q` 是查询数。

## 高效方法

思路是用[凸包](https://www.geeksforgeeks.org/convex-hull-using-divide-and-conquer-algorithm/)的招数：
