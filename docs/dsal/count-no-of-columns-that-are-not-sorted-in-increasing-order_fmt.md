...
> 原文：`https://www.geeksforgeeks.org/count-no-of-columns-that-are-not-sorted-in-increasing-order/`
...
**方法：** 逐个遍历每一列，检查下一个元素是否大于同一列中的前一个元素。如果不是，将列数增加 1，并继续遍历，直到遍历完所有列。打印计数的值。
