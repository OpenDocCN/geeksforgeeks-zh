...
> 原文: [https://www.geeksforgeeks.org/pairs-involved-balanced-parentheses/](https://www.geeksforgeeks.org/pairs-involved-balanced-parentheses/)
...
这里，在段树中，对于每个节点，保留一些简单的元素，如整数或集合或向量等。为每个节点保留三个整数:
1. `t` =区间内的答案。
2. `o` =删除括号后剩余的左括号“`(`”的数量，这些括号属于长度为 `t` 的间隔中的正确括号序列。
3. `c` =删除括号后剩余的结束括号'`)'`的数量在这个长度为 `t` 的间隔中属于正确括号序列的那些。
...
# To build a segmenttree we pass 1 as
# 'id' 0 as 'l' and l as 'n'.
# Here, we consider query's interval as [x, y)
def build(id, l, r, s):
    global tree_arr
