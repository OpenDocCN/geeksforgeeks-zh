...
> 原文: [https://www.geeksforgeeks.org/sub-strings-having-exactly-k-characters-that-have-ascii-value-greater-than-p/](https://www.geeksforgeeks.org/sub-strings-having-exactly-k-characters-that-have-ascii-value-greater-than-p/)

给定一个字符串 `str`，两个整数 `k` 和 `p`。任务是统计 `str` 的所有子字符串，这些子字符串包含 ASCII 值大于 `p` 的 `k` 个字符。
...
我们只需要遍历所有索引，取所有可能长度的子串，然后只需检查子串是否正好有 ASCII 值大于 `'p'` 的 `'k'` 个字符。
