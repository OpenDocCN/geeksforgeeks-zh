...
> 原文: [https://www.geeksforgeeks.org/check-if-a-number-can-be-represented-as-the-sum-of-numbers-with-at-least-one-digit-equal-to-k/](https://www.geeksforgeeks.org/check-if-a-number-can-be-represented-as-the-sum-of-numbers-with-at-least-one-digit-equal-to-k/)
...
*   将一个变量`temp`初始化为`k`，同时取一个计数器说`count`，赋 0
*   迭代至`temp`和`N`的最后一位不相等，每次迭代
    *   将`temp`的值增加`k`
    *   保持迭代次数`count`，如果`count`大于 10，则中断循环
*   检查`temp`和`N`的最后一位数字是否相等，如果`temp <= N`的值:
    *   如果满足上述条件，返回真
    *   否则返回 false
*   同样如果`k * 10 <= N`，返回真
*   否则返回 false
