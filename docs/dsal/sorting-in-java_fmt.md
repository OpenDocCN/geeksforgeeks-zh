...
### 方式 2：使用 `Arrays` 类的 `sort()` 方法

[`Arrays.sort()`](https://www.geeksforgeeks.org/arrays-sort-in-java-with-examples/) 适用于原始数据类型的数组，默认情况下，这些数组也按升序排序。
...
[`Collections.sort()`](https://www.geeksforgeeks.org/collections-sort-java-examples/) 适用于数组列表和链表等对象集合。
...
>     以前，Java 的 `Arrays.sort()` 方法对基元数组使用快速排序，对对象数组使用合并排序。在最新版本的 Java 中，`Arrays.sort()` 方法和 `Collections.sort()` 使用的是 Timsort。
> *   **默认做哪种排序顺序？**
>     默认按升序排序。
> *   **数组或列表如何降序排序？**
