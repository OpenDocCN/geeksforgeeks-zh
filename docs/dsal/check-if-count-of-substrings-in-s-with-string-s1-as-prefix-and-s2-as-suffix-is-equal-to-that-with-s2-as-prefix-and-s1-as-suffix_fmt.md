...
> 原文: [https://www.geeksforgeeks.org/check-if-count-of-substrings-in-s-with-string-s1-as-prefix-and-s2-as-suffix-is-equal-to-that-with-s2-as-prefix-and-s1-as-suffix/](https://www.geeksforgeeks.org/check-if-count-of-substrings-in-s-with-string-s1-as-prefix-and-s2-as-suffix-is-equal-to-that-with-s2-as-prefix-and-s1-as-suffix/)

给定三个字符串 `S`、`S1` 和 `S2`，任务是检查以 `S1` 和 `S2` 开始和结束的子字符串数量是否等于以 `S2` 和 `S1` 开始和结束的子字符串数量。如果发现是真的，则打印“是”。否则，打印“否”。

## 示例

**输入:** `S = "hello world"`，`S1 = "hello"`，`S2 = "world"`
**输出:** No
**解释:**
...
**输入:** `S = "opencloseopencloseopen"`，`S1 = "open"`，`S2 = "close"`
**输出:** 是
...
*   将字符串 `S`、`S1` 和 `S2` 的长度存储在变量中，分别表示为 `N`、`N1` 和 `N2`。
*   初始化一个变量，比如 `count`，将子串 `S1` 的出现次数存储在字符串 `S` 中。
*   初始化一个变量，比如说 `ans`，来存储分别以 `S1` 和 `S2` 开始和结束的子串。
*   遍历给定的字符串 `S` 并执行以下步骤:
    *   将字符串 `S` 的子字符串存储在字符串 `prefix` 中，从大小为 `N1` 的索引 `i` 开始。
    *   将字符串 `S` 的子字符串存储在字符串 `suffix` 中，从大小为 `N2` 的索引 `i` 开始。
    *   如果 `prefix` 与字符串 `S1` 相同，那么将 `count` 的值增加 1。
    *   如果 `suffix` 与字符串 `S2` 相同，则通过 `count` 增加 `ans` 的值。
*   使用以上步骤，分别找到以 `S2` 和 `S1` 开始和结束的子串数量。将获得的计数存储在变量 `ans2` 中。
