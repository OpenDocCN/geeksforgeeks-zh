...
`Math.exp()` 是 JavaScript 中的一个内置函数，用于获取 `e^p` 的值，其中 `p` 是任意给定的数字。
...
**说明:**
这里参数 `p` 的值为 0，所以在 `e^p` 中放入值 0 而不是 `p` 后，其值变为 1。
...
**说明:**
这里参数 `p` 的值是 2，所以在 `e^p` 中放入值 2 而不是 `p` 之后，那么它的值就变成了 7.38905609893065。
...
这里的参数应该是一个数字，否则会给出一个错误或 `NaN`，即不是一个数字。
...
由 `JavaScript Math.exp()` 功能支持的浏览器如下:
