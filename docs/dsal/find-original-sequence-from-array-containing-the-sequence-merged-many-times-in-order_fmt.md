...
> 原文：[https://www.geeksforgeeks.org/find-original-sequence-from-array-containing-the-sequence-merged-many-times-in-order/](https://www.geeksforgeeks.org/find-original-sequence-from-array-containing-the-sequence-merged-many-times-in-order/)
...
**方法：** 想法是观察在给定序列中首先出现的元素是合成的恢复序列的第一个元素。在我们恢复的序列中获取该元素，不要包含给定序列的副本。对其余元素执行相同的操作，直到我们到达终点。这个想法可以通过 C++ 中的 [`map`](https://www.geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/) 和 [`set`](https://www.geeksforgeeks.org/set-in-cpp-stl/) 来实现。

## 使用 `map`

1.  从左到右遍历给定的序列。
2.  序列中第一次出现的元素会被考虑在内，并使用 `map` 进行标记。
3.  遍历时标记的元素被忽略。
