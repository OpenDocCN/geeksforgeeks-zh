...
> 原文: [https://www.geeksforgeeks.org/find-the-number-of-solutions-to-the-given-equation/](https://www.geeksforgeeks.org/find-the-number-of-solutions-to-the-given-equation/)

给定三个整数 `A`、`B` 和 `C`，任务是求 `X` 的计数值，使得满足以下条件：
`X = B * Sm(X)^A + C`，其中 `Sm(X)` 表示 `X` 的位数之和，且 `1 < X < 10^9`。
...
> **输出:** `3`
> 对于 `X = 10`，`2 * (1)^3 + 8 = 10`
> 对于 `X = 2008`，`2 * (10)^3 + 8 = 2008`
> 对于 `X = 13726`，`2 * (23)^3 + 8 = 13726`

**方法：** 这里可以做一个重要的观察，位数之和最多为 81（即 `X = 999999999` 时）。对应每个位数之和，我们得到一个单值 `X`。所以我们可以迭代每个数字的和，检查 `X` 的结果值是否有效。
