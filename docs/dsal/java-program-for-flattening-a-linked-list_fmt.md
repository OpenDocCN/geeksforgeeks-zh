...
> 原文：[`https://www.geeksforgeeks.org/java-program-for-flattening-a-linked-list/`](https://www.geeksforgeeks.org/java-program-for-flattening-a-linked-list/)
...
想法是使用 [`Merge()`](https://www.geeksforgeeks.org/merge-sort-for-linked-list/) 过程对链表进行合并排序。我们使用`merge()`逐个合并列表。我们递归`merge()`当前列表和已经展平的列表。`向下`指针用于链接展平列表的节点。
