...
> 原文: [https://www.geeksforgeeks.org/possible-two-sets-from-first-n-natural-numbers-difference-of-sums-as-d/](https://www.geeksforgeeks.org/possible-two-sets-from-first-n-natural-numbers-difference-of-sums-as-d/)
...
> 把上面 2 个方程相加，我们得到
> `2 * 和(s1) = N*(N+1)/2 + D`
> 如果 `和(S1)` 和 `和(s2)` 都是整数，那么只有我们能把前 `N` 个自然数拆分成两组。为此，`N*(N+1)/2 + D` 必须是偶数。
