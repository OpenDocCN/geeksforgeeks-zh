...
其思想是使用给定的一组规则形成数组元素的有向图。当形成图时，我们不进行自循环，因为值 `arr[i]%n` 等于 0 意味着没有移动。最后，我们的任务简化为有向图中的[检测周期](https://www.geeksforgeeks.org/detect-cycle-in-a-graph/)。为了检测循环，我们使用 DFS，在 DFS 中，如果到达一个被访问的节点，且该节点在递归调用栈中，我们说有一个循环。
