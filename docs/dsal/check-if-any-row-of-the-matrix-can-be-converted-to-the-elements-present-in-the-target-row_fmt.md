...
> 原文：[https://www.geeksforgeeks.org/check-if-any-row-of-the-matrix-can-be-converted-to-the-elements-present-in-the-target-row/](https://www.geeksforgeeks.org/check-if-any-row-of-the-matrix-can-be-converted-to-the-elements-present-in-the-target-row/)

给定维度为 `N*M` 的矩阵 `mat[][]` 和整数为 `M` 的数组 `target[]`，任务是通过选择矩阵的任意两行来检查矩阵的任意一行是否可以等于数组 `target[]`，并将任意一行的每个元素更新为所选两行的相应索引处的元素的最大值。如果可以，则打印 `Yes`。否则，打印 `No`。
...
> `[max(1, 2), max(5, 5), max(7, 3)] = [2, 5, 7]` 等于 `target[]`。
>
> **输入:** `mat[][] = {{3, 4, 5}, {4, 5, 6}}`, `target[] = {3, 2, 5}`
> **输出:** No

**方法:** 给定的问题可以通过使用贪婪方法来解决，其思想是创建一个辅助的数组 `comp[]` 和遍历矩阵的每一行，如果这些行的元素小于或等于目标数组中相应的元素，则对该行的每一个元素执行 `Math.max()` 操作，并执行 `comp`。可以遵循以下步骤:

*   初始化一个数组，说 `comp[]` 大小 `M` 每个元素等于 `INT_MIN`。
*   遍历矩阵 `mat[][]` 并针对每一行检查该行中的所有值是否小于或等于 `target` 数组中的相应值。如果发现为真，则更新 `comp[]` 数组，取 `comp[]` 中每个元素与行中相应元素的最大值。
*   经过以上步骤，如果数组 `comp[]` 与数组 `target[]` 相同，则打印 `Yes`。否则，打印 `No`。
