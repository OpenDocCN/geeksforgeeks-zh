...
> 原文: [https://www.geeksforgeeks.org/convert-characters-of-string1-to-characters-present-in-string2-by-incrementing-or-decrementing-lexicographically/](https://www.geeksforgeeks.org/convert-characters-of-string1-to-characters-present-in-string2-by-incrementing-or-decrementing-lexicographically/)
...
给定的问题可以使用[贪婪的方法](https://www.geeksforgeeks.org/greedy-algorithms/)来解决。其思想是将字符串 `A` 中不在字符串 `B` 中的所有字符转换为字符串 `B` 中存在的最接近的可能字符，这可以通过以下步骤来完成:
...
*   将字符 `x` 转换为 `y` 的方法可以有以下两种不同类型:
    *   第一是增加字符 `x` 直到达到 `y`，即顺时针旋转。
    *   第二是递减字符 `x` 直到达到 `y`，即逆时针旋转。
*   因此，在变量 `clockwise` 和 `anticlockwise` 中，保持 `A` 中每个字符顺时针和逆时针旋转的所有最小移动的总和，这是所需的值。
