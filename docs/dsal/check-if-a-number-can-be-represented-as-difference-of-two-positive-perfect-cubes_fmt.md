...
> 原文: [https://www.geeksforgeeks.org/check-if-a-number-can-be-represented-as-difference-of-two-positive-perfect-cubes/](https://www.geeksforgeeks.org/check-if-a-number-can-be-represented-as-difference-of-two-positive-perfect-cubes/)
...
> **输入:** `N = 124`
> **输出:** 是
> **说明:** 既然 124 可以表示为 `(125–1) = (5^3 – 1^3)`。因此，打印“是”。
>
> **输入:** `N = 4`
> **输出:** 否

**方法:** 解决给定问题的思路是将 1 到 `X` 的所有数字的[完美立方体](https://www.geeksforgeeks.org/perfect-cubes-range/)存储在一张[图](https://www.geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/)中，其中 `X` 是最大整数，`X^3` 和 `(X–1)^3` 之间的差值最多为 `N`。

按照以下步骤解决问题:

*   初始化一个[有序图](https://www.geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/)，比如说 `cubes`，按照排序顺序存储前 `X` 个自然数的完美立方体。
