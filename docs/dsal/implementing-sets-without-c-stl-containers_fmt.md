...
> 原文：[https://www.geeksforgeeks.org/implementing-sets-without-c-stl-containers/](https://www.geeksforgeeks.org/implementing-sets-without-c-stl-containers/)
...
*   `add(data)` – 向集合中添加“数据”
*   `unionSet()` – 返回集合与集合的联合
*   `intersectionSet()` – 返回具有集合的相交集
*   `complementSet(U)` – 返回具有通用集“U”的集的补集
*   `operator-(s)` – 返回集合和集合的差
*   `operator==(s)` – 返回集合是否等于集合
*   `displayProduct()` – 将集合和集合的笛卡尔乘积打印到控制台
*   `displayPowerSet()` – 将电源集打印到控制台
*   `toArray()` – 将集合作为其元素的数组返回
*   `contains()` – 返回集合是否包含元素
*   `displaySet()` – 将设置打印到控制台
*   `getSize()` – 返回集合的大小
...
int main()
{
    ...
    // Set F contains the difference of the Sets A and B
    Set<int> F = A - B;
    cout << "A - B = ";
    F.displaySet();
    cout << endl;

    // Set D contains the union of the Sets A and B
    Set<int> D = A.unionSet(B);
    cout << "A union B = ";
    D.displaySet();
    cout << endl;

    // Set E contains the intersection of the Sets A and B
