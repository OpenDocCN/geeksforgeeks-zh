...
我们知道大小为 `n` 的集合的子集数是 `2^n`。

**这个公式是如何工作的？**
对于每一个元素，我们都有两个选择，要么挑，要么不挑。所以我们总共有 `2 * 2 *……(n 次)`选择，即 `2^n`。
...
大小为 2 的子集数量=`^nC_2`
...
子集总数=`^nC_0 + ^nC_1 + ^nC_2 + …… + ^nC_n = 2^n`
详见[二项式系数之和](https://www.geeksforgeeks.org/sum-binomial-coefficients/)。
