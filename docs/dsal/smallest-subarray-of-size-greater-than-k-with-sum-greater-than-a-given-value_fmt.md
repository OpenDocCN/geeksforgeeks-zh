...
给定一个数组，大小为 `N` 的 `arr[]` ，两个正整数 `K` 和 `S` ，任务是找出大小大于 `K` 的最小子数组的长度，其和大于 `S` 。
...
8.  此时，我们检查子阵列的大小是否可以减小(通过增加 `i` ) 使得它仍然大于 `K`，并且总和也大于 `s`。我们不断地从总和中移除数组的第 `i` 个元素，以减小子阵列大小，然后增加 `j` ，使得我们移动到数组中的下一个元素。
