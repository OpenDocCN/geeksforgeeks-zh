...
> 原文: [https://www.geeksforgeeks.org/check-if-the-number-is-balanced/](https://www.geeksforgeeks.org/check-if-the-number-is-balanced/)
...
**方法:**
从头开始迭代数字的一半长度。将 `s[i]` 和 `s[位数–1–i]` 分别加到 `leftSum` 和 `rightSum` 上，同时计算前半部分和后半部分的位数之和。最后，检查 `leftSum` 和 `rightSum` 是否相等。
