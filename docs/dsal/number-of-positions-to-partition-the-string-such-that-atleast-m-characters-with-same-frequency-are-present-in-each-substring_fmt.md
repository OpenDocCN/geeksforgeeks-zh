...
> 原文：[https://www.geeksforgeeks.org/number-of-positions-to-partition-the-string-such-that-atleast-m-characters-with-same-frequency-are-present-in-each-substring/](https://www.geeksforgeeks.org/number-of-positions-to-partition-the-string-such-that-atleast-m-characters-with-same-frequency-are-present-in-each-substring/)

给定一个由小写英文字母组成的字符串和一个整数 `m`。任务是计算字符串中有多少个位置，这样，如果将字符串划分为两个非空的子字符串，则两个子字符串中至少有 `m` 个频率相同的字符。
字符串中需要出现字符。
...
> 即 `a|a|b|b|c|c|a|a`
> 只有两个满足给定约束的分区是可能的。
> `aab | bccaa` – 在分隔符的左半部分，“a”的频率为 2，“b”的频率为 1，与右半部分相同。
> `aabb | ccaa` – 在分隔符的左半部分，“a”为频率 2，“c”为频率 1，与右半部分相同。
> **输入：** `str = "aabbaa"`，`m = 2`
> **输出：** 1
