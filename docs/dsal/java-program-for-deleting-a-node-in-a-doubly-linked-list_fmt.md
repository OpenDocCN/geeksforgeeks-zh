...
> 原文：[https://www.geeksforgeeks.org/java-program-for-deleting-a-node-in-a-doubly-linked-list/](https://www.geeksforgeeks.org/java-program-for-deleting-a-node-in-a-doubly-linked-list/)
...
*   如果存在上一个 `del`，则将上一个的 `next` 设置为 `del` 的 `next`。
...
*   如果 `del` 的 `previousNode` 存在，则设置 `del` 旁边的 `prev`。
...
List after Deleting middle node: 8 2
```
