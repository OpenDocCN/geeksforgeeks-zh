...
> 原文：[https://www.geeksforgeeks.org/find-triplet-sum-closest-to-x-in-a-sorted-doubly-linked-list-dll/](https://www.geeksforgeeks.org/find-triplet-sum-closest-to-x-in-a-sorted-doubly-linked-list-dll/)
...
解决给定问题最简单的方法是[使用三个嵌套循环](https://www.geeksforgeeks.org/print-all-triplets-with-given-sum/)生成所有可能的三元组，然后选择和最接近 `X` 的三元组，并打印三元组的和。
...
为了优化上述方法，思路是使用[3 指针技术](https://www.geeksforgeeks.org/find-a-triplet-in-an-array-whose-sum-is-closest-to-a-given-number/)。按照以下步骤解决此问题：

### 步骤

1.  初始化 4 个变量，`first` 和 `second` 指向双链表的头节点，即 `first = head`、`second = head`，以及 `tail` 和 `third` 都初始化到双链表的最后一个节点。
2.  初始化一个变量 `diff`，初始化为 `INT_MAX`，存储最接近 `X` 的和。
3.  当 `first` 节点不是 `null` 时迭代，并执行以下步骤：
    *   初始化 `second` 到 `first` 的下一个，即 `second = first->next`、`third = tail`（双链表的最后一个节点）。
    *   迭代当 `second` 和 `third` 不是 `null` 且 `third` 不等于 `second`。
        *   初始化一个变量，说 `sum` 求和为 `(first->data + second->data + third->data)`。
        *   如果 `X - sum` 的绝对值小于 `X - diff` 的[绝对值](https://www.geeksforgeeks.org/abs-labs-llabs-functions-cc/)，则将 `diff` 的值更新为 `sum`。
        *   如果 `sum` 小于 `X`，则增加 `second` 指针，即 `second = second->next`。
        *   否则，递减 `third`，即 `third = third->prev`。
    *   将 `first` 指针移动到下一个指针，即 `first = first->next`。
4.  完成以上步骤后，打印 `diff` 的值作为结果。
