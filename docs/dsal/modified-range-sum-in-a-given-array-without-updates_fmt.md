...
> 原文: [https://www.geeksforgeeks.org/modified-range-sum-in-a-given-array-without-updates/](https://www.geeksforgeeks.org/modified-range-sum-in-a-given-array-without-updates/)
...
> **输出:** `4 2 5 4`
> **解释:**
> 对于 `i = 0` -> 输入数组中的元素 = 4。因此，`L = 1`，`R = min(4, N-1) = 3`。因此，从 `arr[1]` 到 `arr[3]` 的所有元素都增加 1。更新操作后的元素是 `{4, 2, 4, 3}`。
...
**天真方法:** 天真方法是为每个元素运行一个循环，并将 `arr[i+1]` 到 `arr[min(i+arr[i], N-1)]` 的所有值增加 1。这个方法的时间复杂度是 `O(N^2)`。
...
*   对于给定数组中的每个元素 `arr[i]`，1 被加到 `b[i+1]` 上，并从 `b[min(i + arr[i], N–1)+1]` 中减去。
*   然后，计算数组 `b[]` 的前缀和。
