...
> 原文: [https://www.geeksforgeeks.org/jacobsthal-and-jacobsthal-lucas-numbers/](https://www.geeksforgeeks.org/jacobsthal-and-jacobsthal-lucas-numbers/)

雅可比序列是一个类似于斐波那契序列的加法序列，由递推关系 `J_n = J_{n-1} + 2J_{n-2}` 定义，初始项 `J_0 = 0`，`J_1 = 1`。序列中的一个数叫做 [**雅可比数**](https://en.wikipedia.org/wiki/Jacobsthal_number)。它们是特定类型的[卢卡斯序列](https://en.wikipedia.org/wiki/Lucas_sequence) `U_n(P, Q)`，其中 `P = 1`，`Q = -2`。
...
雅各布斯塔尔-卢卡斯数表示互补的卢卡斯序列 `V_n(1, -2)`。它们满足与雅可比数相同的递推关系，但具有不同的初始值：
![{\displaystyle L_{n}=\left\{\begin{matrix} 2 & & if n=0\\1&&ifn=1 \\ L_{n-1} + 2L_{n-2}&&ifn>1 \end{matrix}\right.}](img/38664653688e22f3f257963bca59a1f1.png "Rendered by QuickLaTeX.com")
