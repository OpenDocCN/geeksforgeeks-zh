# 在[2, 3, ..n]与[2, 3, ..m]中寻找最大互质数

> 原文: [https://www.geeksforgeeks.org/largest-number-in-2-3-n-which-is-co-prime-with-numbers-in-2-3-m/](https://www.geeksforgeeks.org/largest-number-in-2-3-n-which-is-co-prime-with-numbers-in-2-3-m/)
...
## 方法 1
创建一个从 2 到 `n` 的数字列表，然后运行 `i = 2` 到 `m` 的循环，标记所有 `i` 的倍数的数字，如果 `i` 已经被标记，不要运行循环，因为它的倍数已经被标记了。当循环终止时，运行一个从 `n` 到 2 的循环，直到找到一个未标记的数字。第一个未标记的数字就是答案，如果没有未标记的数字，那么这个数字就不存在。这种方法需要 `O(n)` 个辅助空间，因此如果 `n` 的值太大，这种方法将不起作用。

## 方法 2
运行一个从 `n` 到 `p+1` 的循环，检查每个数是否不能被 2 到 `m` 之间的任何数整除。
