...
> 原文：[https://www.geeksforgeeks.org/generate-a-random-permutation-of-1-to-n/](https://www.geeksforgeeks.org/generate-a-random-permutation-of-1-to-n/)
...
创建一个由 `N` 个元素组成的数组，并将这些元素初始化为 `1, 2, 3, 4, …, N`，然后使用 Fisher-Yates 洗牌算法对数组元素进行洗牌。

[费希尔-耶茨洗牌算法](https://www.geeksforgeeks.org/shuffle-a-given-array-using-fisher-yates-shuffle-algorithm/)在 `O(n)` 时间复杂度下工作。这里的假设是，给我们一个函数 `rand()`，它在 `O(1)` 时间内生成一个随机数。
