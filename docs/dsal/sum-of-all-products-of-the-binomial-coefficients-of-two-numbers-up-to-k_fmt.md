...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-products-of-the-binomial-coefficients-of-two-numbers-up-to-k/](https://www.geeksforgeeks.org/sum-of-all-products-of-the-binomial-coefficients-of-two-numbers-up-to-k/)
...
解决这个问题最简单的方法就是简单的迭代范围 `[0, K]`，计算 `C(N, i)` 和 `C(M, K - i)` 为每一个 `i`，并通过相加它们的乘积更新 `sum`。
...
> 根据**范德蒙身份**，总共 `(N + M)` 项中的 `K` 项的任意组合都应该有 `M` 中的 `r` 项和 `N` 中的 `K - r` 项。
