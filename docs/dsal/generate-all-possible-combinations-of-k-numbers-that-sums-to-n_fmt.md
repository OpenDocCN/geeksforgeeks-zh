...
> 原文: [https://www.geeksforgeeks.org/generate-all-possible-combinations-of-k-numbers-that-sums-to-n/](https://www.geeksforgeeks.org/generate-all-possible-combinations-of-k-numbers-that-sums-to-n/)

给定两个整数 `N` 和 `K`，任务是根据以下条件找到所有有效的 `K` 个数的组合，这些组合加起来就是 `N`：
*   仅使用 `[1, 9]` 范围内的数字。
*   每个号码最多只能使用一次。
...
最简单的思路就是用[回溯](https://www.geeksforgeeks.org/backtracking-introduction/)来解决问题。按照以下步骤解决问题:
*   如果 `N * 9` 小于 `K`，打印 `"Impossible"`。
*   初始化两个 `vector<bool>` `vis`，来存储组合中是否已经使用了一个数字，以及 `vector<int>` `sub_vector`，来存储其和等于 `K` 的子集。
*   初始化一个 `vector<vector<int>>`，说 `output`，存储所有可能的组合。
*   现在，定义一个函数，比如 `Recurrence(N, K, sub_vector, vis, output, last)` 来查找所有组合，其中 `last` 代表已经使用的最后一个数字:
    *   定义一个基本情况，如果 `N = 0` 和 `K = 0`，则将 `sub_vector` 推入 `output` 矢量。
    *   现在如果 `N` 或 `K` 小于 `0`，则返回。
    *   使用变量迭代范围 `[last, 9]`，说出 `i`，并将 `i` 推入 `sub_vector` 并将 `i` 标记为已访问。现在，调用递归函数 `Recurrence(N - 1, K - i, sub_vector, vis, output, i + 1)`。
    *   在上述步骤的每次迭代中，将 `i` 标记为未访问，并从矢量 `sub_vector` 中弹出 `i`。
*   现在，调用递归函数 `Recurrence(N, K, sub_vector, vis, output, 1)`。
*   最后，完成以上步骤后，[打印矢量的矢量 `vector<vector<int>>`](https://www.geeksforgeeks.org/vector-of-vectors-in-c-stl-with-examples/) `output`。
