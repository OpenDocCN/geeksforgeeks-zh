...
一个简单的解决方案是遍历从 1 到 `n-6` 的所有数字。对于每个数字 `i`，检查 `i，i+2，i+6` 或者 `i，i+4，i+6` 是否是素数。如果是，打印三胞胎。

一个有效的解决方法是使用[厄拉多塞的筛](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)首先找到所有的素数，这样我们就可以快速检查一个数是否是素数。以下是实施办法。
