...
> 原文: [https://www.geeksforgeeks.org/python-program-for-sorting-an-array-of-0s-1s-and-2s/](https://www.geeksforgeeks.org/python-program-for-sorting-an-array-of-0s-1s-and-2s/)
...
1.  保持三个指数 `低= 1`，`中= 1` 和 `高= N`，有四个范围，1 到低 (包含 0 的范围)，低到中 (包含 1 的范围)，中到高 (包含未知元素的范围) 和高到 N (包含 2 的范围)。
2.  从头到尾遍历数组，`mid` 小于 `high`。(循环计数器为 1)
3.  如果元素为 0，则用索引 `低` 的元素交换元素，并更新 `低=低+ 1` 和 `中=中+ 1`
4.  如果元素为 1，则更新 `mid = mid + 1`
5.  如果元素为 2，则用索引 `高` 的元素交换元素，更新 `高=高–1`，更新 `I = I–1`。因为交换的元素没有被处理
