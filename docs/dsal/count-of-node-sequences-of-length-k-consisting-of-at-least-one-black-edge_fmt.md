...
给定一棵树，该树由从 `[1，N]` 编号的 `N` 个节点组成，并被着色为黑色（由 `1` 表示）或绿色（由 `0` 表示），任务是计算长度为 `K` 的序列数 `[a1, a2, .. aK]`，使得连续节点之间的路径最短，并且覆盖的边至少由一个黑色边组成。 由于答案可能很大，因此将其打印为 `10^9 + 7` 的模。
...
> **输出**：`24`
> **说明：**
> 答案中包括除 `(1, 1, 1)`，`(2, 2, 2)` 和 `(3, 3, 3)` 之外的所有 `3^3` 序列。

**方法**：

的想法是计算长度 `K` 的序列数，以便不覆盖任何黑色边。 令计数为 `temp` 。 然后 `(N^K) – temp` 是必需的答案。 `temp` 可以通过去除黑边，然后计算所得图形的不同成分的大小来轻松计算。

请按照以下步骤操作：

1.  将 `ans` 的值初始化为 `N^K` 。
2.  通过仅添加绿色边来构造图 `G`。
3.  执行图形的 [DFS 遍历](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/)，并从 `ans` 减去 `(size^K)`，其中 `size` 为图 `G` 的不同组成部分中的节点数。
...
class GFG{
    ...
    static int ans;
    // List to store the graph
    static List<int> []G = new List<int>[N];
    // Iterative Function to
    // calculate (a<sup>b</sup>) % mod in O(log b)
    static int power(int a, int b)
    {
        // Initialize result
        int res = 1;
    ...
    // DFS traversal of the nodes
    static void dfs(int i, bool []visited)
    {
    ...
    // Function to calculate the readonly answer
    static void totalSequences(int n, int k)
    {
        // Mark all the vertices as not visited initially
        bool []visited = new bool[n + 1];
        // Subtract (size^k) from the answer for different components
        for(int i = 1; i <= n; i++)
        {
            if (!visited[i])
            {
    ...
    static void addEdge(int x, int y, int color)
    {
        // If the colour is green, include it in the graph
        if (color == 0)
        {
            G[x].Add(y);
            G[y].Add(x);
