...
> 原文: [https://www.geeksforgeeks.org/replace-every-consonant-sequence-with-its-length-in-the-given-string/](https://www.geeksforgeeks.org/replace-every-consonant-sequence-with-its-length-in-the-given-string/)

给定一个由小写字符组成的字符串 `str`。任务是用辅音的长度替换连续的辅音序列。
