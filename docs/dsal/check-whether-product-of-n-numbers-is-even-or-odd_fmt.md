...
> 原文: [https://www.geeksforgeeks.org/check-whether-product-of-n-numbers-is-even-or-odd/](https://www.geeksforgeeks.org/check-whether-product-of-n-numbers-is-even-or-odd/)

给定一个包含 `n` 个数字的数组 `arr[]` 。问题是检查给定的 `n` 个数的乘积是偶数还是奇数。
...
基于以上事实，如果单个偶数出现，那么 `n` 个数的乘积将是偶数，否则是奇数。
