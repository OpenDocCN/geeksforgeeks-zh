...
> 原文：[https://www.geeksforgeeks.org/sort-a-linked-list-after-converting-elements-to-their-squares/](https://www.geeksforgeeks.org/sort-a-linked-list-after-converting-elements-to-their-squares/)

给定一个非递减的[链表](https://www.geeksforgeeks.org/data-structures/linked-list/)。任务是在不使用任何额外空间的情况下，将链表的元素排列成排序的顺序。
...
**逼近**：任务可以通过将给定的列表从转换点（负向正）划分为两个不同的链表来解决，一个只包含负元素的链表表示 `l1`，另一个包含正元素的链表表示 `l2`。将列表 `l1` 的所有元素进行平方，将其[反转](https://www.geeksforgeeks.org/reverse-a-linked-list/)，同时将列表 `l2` 的所有元素进行平方，[合并两个排序列表](https://www.geeksforgeeks.org/merge-two-sorted-linked-lists/)得到结果列表。
...
n = 6;
let arr2 = [-2, -1, 0, 1, 2];
head = makeList(arr2.length, arr2);
let arr3 = [-5, -4, -3, -2, -1];
head = makeList(arr3.length, arr3);
print_list(squaresList(head));
