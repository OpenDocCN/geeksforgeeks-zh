...
> 原文: [https://www.geeksforgeeks.org/check-if-lowercase-and-uppercase-characters-are-in-same-order/](https://www.geeksforgeeks.org/check-if-lowercase-and-uppercase-characters-are-in-same-order/)
...
1.  遍历字符串。
2.  将小写字母和大写字母分别存储在两个单独的字符串 `lowerStr` 和 `upperStr` 中。
3.  将小写字符串转换为大写。
4.  检查两个大写字符串是否相等。
5.  如果相等，则打印 `Yes`，否则打印 `No`。
