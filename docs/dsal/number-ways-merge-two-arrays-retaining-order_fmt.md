...
> 原文：[https://www.geeksforgeeks.org/number-ways-merge-two-arrays-retaining-order/](https://www.geeksforgeeks.org/number-ways-merge-two-arrays-retaining-order/)
...
合并后我们知道合并后元素总数会是 `(m + n)` 个元素。所以，现在我们只需要从 `(m + n)` 中选择 `m` 个位置的方法，您将按照数组 `A` 的实际顺序放置数组 `A` 的元素，即 `^{m + n}C_n`。
放置数组 `A` 的 `m` 个元素后，会留下 `n` 个空格，可以用数组 `B` 的 `n` 个元素按其实际顺序填充。
因此，合并两个数组以使它们在合并数组中的顺序相同的方法总数是 `^{m+n}C_n`。
