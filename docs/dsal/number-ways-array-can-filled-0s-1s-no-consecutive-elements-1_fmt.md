...
> 原文: [https://www.geeksforgeeks.org/number-ways-array-can-filled-0s-1s-no-consecutive-elements-1/](https://www.geeksforgeeks.org/number-ways-array-can-filled-0s-1s-no-consecutive-elements-1/)
...
由于每个元素有 `2` 个可能值，`1` 和 `0`，并且总共有 `n` 个元素，所以没有任何限制的数组总数将是指数级的，即 `2^n`。
...
设 `T(n)` 是给出大小为 `n` 的数组可以填充的次数的函数，那么我们得到以下递推关系

> `T(n) = T(n-1) + T(n-2)`

而这就是 [斐波那契数列](https://www.geeksforgeeks.org/program-for-nth-fibonacci-number/) 的递推关系。
因此，任意 `n` 的输出等于斐波那契数列从 `1` 开始的第 `(n+2)` 项。
即 `1 1 2 3 5 8 11……`
所以现在我们只需要计算直到第 `(n+2)` 个元素的斐波那契数列，这就是答案。
