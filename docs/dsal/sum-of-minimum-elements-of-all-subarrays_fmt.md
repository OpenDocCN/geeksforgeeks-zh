...
> 原文：[https://www.geeksforgeeks.org/sum-of-minimum-elements-of-all-subarrays/](https://www.geeksforgeeks.org/sum-of-minimum-elements-of-all-subarrays/)
...
**方法：** 天真的方法是生成所有可能的（连续的）子阵列，找到它们的最小值，并将其添加到结果中。时间复杂度为 `O(N^2)`。

**有效途径：** 解决问题的一般直觉是求 `sum(A[i] * f(i))`，其中 `f(i)` 是子阵数，其中 `A[i]` 最小。

为了找到 `f(i)`，我们需要找出：
`left[i]`：`A[i]` 左边上严格较大数的长度。
`right[i]`：`A[i]` 右边上较大数的长度。

我们制作 `left[]` 和 `right[]` 两个数组，使得：
`left[i] + 1` 等于以 `A[i]` 结尾的子阵个数，`A[i]` 只是单个最小值。
同样，`right[i] + 1` 等于以 `A[i]` 开始的子阵个数，`A[i]` 为第一最小值。
最后，`f(i) = (left[i]) * (right[i])`，其中 `f[i]` 等于子阵总数，其中 `A[i]` 最小。
...
1.  当前元素是右侧所有元素中最小的元素。所以 `ans` 会是 `((curr_index - 0) * arr[curr_index])`。因为它将是从 `curr_index` 开始的所有子阵列中的最小元素。
2.  右边有一个元素，比当前元素小。较小元素的答案已经存储在 `dp` 中。只需计算从当前索引到较小的右索引的答案。
    *   `up_small = (right_index - curr_index) * arr[curr_index]` // 从 `curr_index` 求和到 `right_index`。
    *   `curr_sum = up_small + dp[right_index]`
