...
> 原文：[https://www.geeksforgeeks.org/split-array-into-minimum-number-of-subsets-such-that-elements-of-all-pairs-are-present-in-different-subsets-at-least-once/](https://www.geeksforgeeks.org/split-array-into-minimum-number-of-subsets-such-that-elements-of-all-pairs-are-present-in-different-subsets-at-least-once/)
...
**方法:** 想法是始终将数组拆分为两个大小的子集 `floor(N / 2)` 和 `ceil(N / 2)`。在每个分区之前，只需将 `arr[i]` 的值与 `arr[N / 2 + i]` 交换即可。按照下面给出的步骤解决问题:
