...
> 原文：[https://www.geeksforgeeks.org/find-the-count-of-palindromic-sub-string-of-a-string-in-its-sorted-form/](https://www.geeksforgeeks.org/find-the-count-of-palindromic-sub-string-of-a-string-in-its-sorted-form/)
...
例如，字符串 `“aabbbcd”` 的回文子串将为 `“a”、“aa”、……、“bbb”、“c”、……` 等。这种方法的时间复杂度为 `O(n)`。

*   创建一个哈希表，用于存储字符串 `str` 中每个字符的频率。
*   遍历哈希表，对于每个非零频率，将 `(hash[i] * (hash[i]+1)) / 2` 添加到 `sum` 中。
*   最后打印 `sum`。
