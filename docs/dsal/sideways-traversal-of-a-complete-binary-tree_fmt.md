...
> 原文：[https://www.geeksforgeeks.org/sideways-traversal-of-a-complete-binary-tree/](https://www.geeksforgeeks.org/sideways-traversal-of-a-complete-binary-tree/)
...
想法是使用修改后的[广度优先搜索](https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/)函数将每一级的所有节点存储在向量数组中。与此同时，树需要遍历的最大级别也存储在一个变量中。完成预计算任务后，按照以下步骤得到所需答案：

1.  创建一个向量`tree[]`，其中`tree[i]`将存储该树在`i`级别的所有节点。
2.  取一个整数变量`k`来记录正在遍历的级别号，取另一个整数变量`path`来记录已经完成的循环数。还创建了一个`flag`变量来跟踪树被遍历的方向。
3.  现在，开始打印每个级别最右边的节点，直到达到`maxLevel`。
4.  由于达到了最大水平，必须改变方向。在最后一层，从最右边到左边打印元素。`maxLevel`变量的值必须递减。
