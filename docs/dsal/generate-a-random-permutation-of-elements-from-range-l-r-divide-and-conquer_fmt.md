# 从范围 [L, R] 生成元素的随机排列（分而治之）

> 原文：[https://www.geeksforgeeks.org/generate-a-random-permutation-of-elements-from-range-l-r-divide-and-conquer/](https://www.geeksforgeeks.org/generate-a-random-permutation-of-elements-from-range-l-r-divide-and-conquer/)

给定范围 `[L, R]`，其中 `L ≤ R`，任务是生成序列 `[L, L + 1, L + 2, …, R]` 的随机排列。
...
**方法：** 我们将使用[分治](https://www.geeksforgeeks.org/divide-and-conquer-algorithm-introduction/)算法进行求解。我们将创建一个全局[向量](https://www.geeksforgeeks.org/vector-in-cpp-stl/)，它将存储由我们的递归函数生成的随机数。
我们将把两个参数 `L`（开始）和 `R`（结束）传递给我们的递归函数。在函数内部，它将调用另一个函数 `give_random_number()`，该函数将返回一个介于 `X` 和 `Y` 之间的数字。让我们称之为 `N`。我们将把这个随机数存储在我们的向量中，然后我们将对范围 `[L, N–1]` 递归调用 `generate_random_permutation()` 函数，然后对范围 `[N+1, R]` 递归调用。
如果 `L` 变得大于 `R`，那么我们将从函数中返回，不执行任何任务，因为这是我们的基础条件。
以下是上述方法的实现：
