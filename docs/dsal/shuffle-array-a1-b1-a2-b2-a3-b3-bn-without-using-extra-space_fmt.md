...
这里讨论了一种在线性时间内有效的新解决方案。我们知道第一个和最后一个数字不会离开它们的位置。并且，我们跟踪从中挑选任何数字的索引以及目标索引的位置。我们知道，如果我们选择`ai`，它必须进入索引`2 * i–1`，如果是`bi`，它必须进入`2 * i`。我们可以根据选择索引检查我们从哪里选择了某个数字，如果它大于或小于`n`。
