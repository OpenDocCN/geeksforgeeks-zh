...
> 原文: [https://www.geeksforgeeks.org/absolute-difference-between-the-xor-of-non-prime-numbers-and-prime-numbers-of-an-array/](https://www.geeksforgeeks.org/absolute-difference-between-the-xor-of-non-prime-numbers-and-prime-numbers-of-an-array/)
...
## 天真的方法
一个简单的解决方案是遍历数组，并不断检查每个元素是否是质数。如果这个数是质数，那么将其异或到 `X1` 代表质数的异或，否则将其异或到 `X2` 代表非质数异或。遍历整个数组后，取 `X1` 和 `X2` 的绝对差值。

## 高效方法
使用厄拉多塞的[筛](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)生成数组最大元素之前的所有素数。现在，遍历数组并检查当前元素是否是质数。如果元素是质数，则将其与 `X1` 异或，否则将其与 `X2` 异或。最终打印 `ABS(X1–X2)`。
