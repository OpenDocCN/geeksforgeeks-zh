...
> 原文: [https://www.geeksforgeeks.org/sort-a-nearly-sorted-or-k-sorted-array-set-2-gap-method-shell-sort/](https://www.geeksforgeeks.org/sort-a-nearly-sorted-or-k-sorted-array-set-2-gap-method-shell-sort/)

给定一个由 `N` 个元素组成的数组 `arr[]`，其中每个元素最多离开其目标位置 `K`，任务是设计一个算法，在 `O(N*log(K))` 时间内进行排序。
...
>         4 7 8 9 `10 50` 60 70，不要交换索引 4 和 5 的元素。
...
*   初始化一个变量，用值 `K` 表示 `间隙`，对每个子列表的每个 `间隙` 元素进行排序。
