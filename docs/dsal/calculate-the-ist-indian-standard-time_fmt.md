...
> 原文：[https://www.geeksforgeeks.org/compute-the-ist-Indian-standard-time/](https://www.geeksforgeeks.org/calculate-the-ist-indian-standard-time/)

给定两个整数 `H` 和 `R`，其中 `H` 是一个地点 `X` 的小时时间，`R` 是从地点 `X` 到印度的度数距离，任务是在 [IST](https://en.wikipedia.org/wiki/Indian_Standard_Time) 找到当前时间。
...
> `IST = (24/360) * 82.50`
> `= 5.5`
> `= 0.5 * 60`（即 60 分钟 = 360 度旋转和 1 分钟 = 6 度，所以 0.5 小时 * 60 = 30）
> `IST = 5:30`
> **输入：** `H = 12`，`R = 60`
