...
> 原文：[https://www.geeksforgeeks.org/sum-of-consecutive-bit-differences-of-first-n-non-negative-integers/](https://www.geeksforgeeks.org/sum-of-consecutive-bit-differences-of-first-n-non-negative-integers/)
...
- 数字的连续位差遵循一种模式，即每个等于 `2^i` 的值 `X` 与其前一个数字的位差为 `(i + 1)`，`X` 以上的 `(2^i – 1)` 个数字和 `X` 以下的 `(2^i – 1)` 个数字遵循相同的模式。
- 对于 `X = 4` (`2^2`，`i = 2`) 有一个位差是 `(2 + 1)`，数字 (1，2，3) 和 (5，6，7) 遵循相同的位差模式。
...
**例如：**
对于 `N = 10`，使用 `Count(3)` 计算最接近的 2 的幂即 `M = 8` 的和，然后对大于 8 的剩余数重复该过程。
