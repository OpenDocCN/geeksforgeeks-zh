...
> 原文: [https://www.geeksforgeeks.org/kth-space-seperated-number-from-a-given-string/](https://www.geeksforgeeks.org/kth-space-seperated-number-from-a-given-string/)

给定一个由空格分隔的整数组成的[字符串](https://www.geeksforgeeks.org/string-data-structure/) `S`，任务是提取字符串中的第 `K` 个数字。
...
> **输入:** `S = "12 13 15"`, `K = 3`
> **输出:** 15
> **说明:** 以上字符串中的第 3 个整数为 15。
>
> **输入:** `S = "10 20 30 40"`, `K = 2`
> **输出:** 20
> **说明:** 上述字符串中的第 2 个整数为 20。

## 天真方法

解决问题最简单的方法是[遍历字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-python/)并记录遇到的空格数。一旦遇到 `K–1` 个空格，将数字打印到下一个空格作为所需答案。
...
想法是在 C++ 中使用 [stringstream](https://www.geeksforgeeks.org/stringstream-c-applications/)，它将字符串对象与流相关联，并允许我们从字符串中读取，就像它是流一样（就像 `cin` 一样）。
...
思路是使用 [strtok()](https://www.geeksforgeeks.org/strtok-strtok_r-functions-c-examples/) 函数提取按键处的字符串。使用 [c_str()](https://www.geeksforgeeks.org/basic_string-c_str-function-in-c-stl/) 函数获取字符数组的[字符指针](https://www.geeksforgeeks.org/char-vs-stdstring-vs-char-c/)引用。
