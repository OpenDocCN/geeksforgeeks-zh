...
> 原文: [https://www.geeksforgeeks.org/find-the-total-marks-obtained-according-to-given-marking-scheme/](https://www.geeksforgeeks.org/find-the-total-marks-obtained-according-to-given-marking-scheme/)

给出 `N` 个 MCQ 的答案键和学生标注的答案。任务是计算学生的分数。
...
1.  开始遍历 `studentAnswer[]`。
2.  如果 `studentAnswer[]` 的当前索引值 `= 0`，则不尝试该问题。
3.  否则如果当前索引处的值等于 `answerKey[]` 中相应索引处的值，则递增肯定答案的计数。
4.  否则增加否定答案的数量。
5.  通过计算正分数和负分数来打印总分数。
