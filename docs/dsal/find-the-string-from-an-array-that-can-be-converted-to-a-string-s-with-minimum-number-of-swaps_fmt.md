...
> 原文: [https://www.geeksforgeeks.org/find-the-string-from-an-array-that-can-be-converted-to-a-string-s-with-minimum-number-of-swaps/](https://www.geeksforgeeks.org/find-the-string-from-an-array-that-can-be-converted-to-a-string-s-with-minimum-number-of-swaps/)
...
> **输出:** `acb`
> **解释:**
> 字符串 `"acb"` 可以通过交换 1 对字符 `"acb"` -> `"abc"` 转换为 `"abc"`。
> 字符串 `"xyz"` 不能转换为 `"abc"`。
...
这个问题可以通过[从给定的字符串数组](https://www.geeksforgeeks.org/check-whether-two-strings-are-anagram-of-each-other/)中搜索 `S` 的字谜来解决，然后，对于每个这样的字符串，找到将字符串转换为 `S` 所需的最少字符互换次数。
...
*   将 `S1` 中的字符位置存储在 26 个列表中。对于 `S1` 中的每个字符，将其索引附加到其对应的列表中，即列表 0 存储字符 `'a'` 的所有位置。同样的，列表 1 存储 `'b'` 等所有位置。
*   在完成字符串 `S1` 的遍历后，[反过来遍历字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-python/) `S` 的字符，对于每个字符，说 `S[j]`，从给定的列表数组中获取其各自的索引，说 `temp`。
