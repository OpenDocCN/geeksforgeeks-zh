...
> 原文: [https://www.geeksforgeeks.org/find-words-which-are-greater-than-given-length-k-using-stringstream/](https://www.geeksforgeeks.org/find-words-which-are-greater-than-given-length-k-using-stringstream/)

给定一个包含空格分隔的单词和数字 `K` 的字符串。任务是使用 C++ 中的 [`stringstream`](https://www.geeksforgeeks.org/stringstream-c-applications/) 查找并打印所有长度大于 `K` 的单词。
