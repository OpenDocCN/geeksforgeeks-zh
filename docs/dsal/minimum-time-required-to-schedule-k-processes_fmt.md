...
> 原文：[https://www.geeksforgeeks.org/minimum-time-required-to-schedule-k-processes/](https://www.geeksforgeeks.org/minimum-time-required-to-schedule-k-processes/)

## 问题描述

给定一个正整数 `K` 和一个由 `N` 个正整数组成的数组 `arr[]`，使得 `arr[i]` 是处理器可以在 `1` 秒内调度的进程数 `i`。任务是最小化调度 `K` 过程所需的总时间，使得在由 `i` 处理器调度之后，`arr[i]` 减少到 `floor(arr[i]/2)`。
...
**说明:**
预定流程顺序如下:
1.  首先安排第 `3` 次流程。数组 `arr[]` 修改为 `{3, 1, 3, 2, 4}`，因为 `arr[2] = floor(arr[2]/2) = floor(7/2) = 3`。
2.  接下来安排第 `5` 流程。`arr[]` 数组修改为 `{3, 1, 3, 2, 2}`。
3.  接下来安排 `1` 流程。`arr[]` 数组修改为 `{1, 1, 3, 2, 2}`。
4.  接下来安排第二个和第一个过程。数组 `arr[]` 修改为 `{3, 0, 3, 2, 4}`。
...
解决给定问题最简单的方法是对给定列表进行升序排序，选择能力最高的处理器，将 `K` 的值减少该值，从列表中删除该处理器，再将该处理器的一半添加到排序列表中。重复以上过程，直到至少安排 `K` 个过程，并打印安排至少 `K` 个过程后所需的时间。
...
上述方法也可以使用哈希的概念进行优化。按照以下步骤解决问题:

1.  初始化给定数组中存在的最大元素大小的辅助数组 `tmp[]`。
2.  初始化一个变量，说 `count` 分别存储调度所有进程的最短时间。
3.  从头到尾遍历给定数组 `tmp[]`，并执行以下步骤:
    *   如果 `tmp[]` 中的电流元素大于 `0`，`i * tmp[i]` 小于 `K`。
        *   将 `K` 的值减少 `i * tmp[i]` 的值。
        *   将 `tmp[i/2]` 增加 `tmp[i]`，因为处理器的能力会减半。
        *   将 `count` 的值增加 `tmp[i]` 的值。
        *   如果 `K` 的值已经小于或等于 `0`，则打印 `count` 的值作为结果。
    *   如果数组 `tmp[]` 中的当前元素至少为 `0`，并且 `i * tmp[i]` 的值至少为 `K`，则执行以下步骤:
        *   如果 `K` 可被当前指数整除，那么将 `count` 的值增加 `K / i`。
        *   否则，将 `count` 的值增加 `K/i + 1`。
4.  完成上述步骤后，如果无法安排所有流程，打印 `-1`。否则，打印 `count` 作为所需的最短时间。
