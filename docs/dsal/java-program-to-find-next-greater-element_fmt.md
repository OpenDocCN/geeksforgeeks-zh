...
> 原文：[https://www.geeksforgeeks.org/java-program-to-find-next-greater-element/](https://www.geeksforgeeks.org/java-program-to-find-next-greater-element/)
...
4.  对于输入数组 `[13，7，6，12]`，每个元素的下一个较大元素如下。
...
*   一个接一个地拾取剩余的元素，并按照以下步骤循环。
    1.  将当前元素标记为 `next`。
    2.  如果堆栈不为空，将堆栈的顶部元素与 `next` 进行比较。
    3.  如果 `next` 大于 `top` 元素，则从堆栈中弹出元素。`next` 是弹出元素的下一个更大的元素。
    4.  当弹出的元素小于 `next` 时，继续从堆栈中弹出。`next` 成为所有这些弹出元素的下一个更大的元素。
*   最后，推送 `next` 到堆栈中。
