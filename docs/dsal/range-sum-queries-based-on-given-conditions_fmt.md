...
> 原文: [https://www.geeksforgeeks.org/range-sum-queries-based-on-given-conditions/](https://www.geeksforgeeks.org/range-sum-queries-based-on-given-conditions/)

给定一个由 `N` 个整数组成的[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `arr[]` 和由 `Q` 个查询组成的矩阵 `Queries`，每个查询的形式为 `{m, a, b}`。对于每个查询，任务是根据以下条件找到数组元素的总和：
...
> **输入**：`arr[] = {6, 4, 2, 7, 2, 7}`，`Q = 3`，`Queries[][3] = {{2, 3, 6}, {1, 3, 4}, {1, 1, 6}}`
> **输出**：`24 9 28`
> **解释：**
> 对于查询 1：
> `m` 为 2，则排序后的数组为 `arr[] = {2, 2, 4, 6, 7, 7}`，范围 `[3, 6]` 内的元素之和为 `4 + 6 + 7 + 7 = 24`。
> 对于查询 2：
> `m` 为 1，则原始数组为 `arr[] = {6, 4, 2, 7, 2, 7}`，范围 `[3, 4]` 内的元素之和为 `2 + 7 = 9`。
> 对于查询 3：
> `m` 为 1，则原始数组为 `arr[] = {6, 4, 2, 7, 2, 7}`，范围 `[1, 6]` 内元素之和为 `6 + 4 + 2 + 7 + 2 + 7 = 28`。

> **输入**：`arr[] = {5, 5, 2, 3}`，`Q = 3`，`Queries[][10] = {{1, 2, 4}, {2, 1, 4}, {1, 1, 1}, {2, 1, 4}, {2, 1, 2}, {1, 1, 1}, {1, 3, 3}, {1, 1, 3}, {1, 4, 4}, {1, 2, 2}}`
> **输出**：`10 14 5 14 10 5 2 10 3 5`

## 朴素方法

思想是遍历给定的查询，根据给定的条件找到所有元素的和：
...
上述方法可以通过使用[前缀和数组](https://www.geeksforgeeks.org/prefix-sum-array-implementation-applications-competitive-programming/)减少一个循环来优化。以下是步骤:
