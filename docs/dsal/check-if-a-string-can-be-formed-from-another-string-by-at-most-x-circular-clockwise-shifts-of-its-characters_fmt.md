...
> 原文: [https://www.geeksforgeeks.org/check-if-a-string-can-be-formed-from-another-string-by-at-most-x-circular-clockwise-shifts-of-its-characters/](https://www.geeksforgeeks.org/check-if-a-string-can-be-formed-from-another-string-by-at-most-x-circular-clockwise-shifts-of-its-characters/)
...
## 方法
思路是遍历字符串并针对每个索引，找到两个字符串各自索引处字符的 [ASCII 值](https://www.geeksforgeeks.org/program-print-ascii-value-character/)之间的差异。如果差值小于 0，那么对于循环移位，加 26 得到实际差值。如果对于任何一个指标，差值超过 `X`，那么 `S2` 就不能由 `S1` 形成，否则有可能。
