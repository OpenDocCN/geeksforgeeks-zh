...
一个简单解决方案是一个一个地找到链表中所有给定键的出现。 对于每个发现的事件，将其插入末尾。 直到所有出现的给定键都移到结尾为止。
...
将维护单独的键列表。 我们将此键列表初始化为空。 我们遍历给定的列表。 对于找到的每个键，我们将其从原始列表中删除，然后插入单独的键列表中。 最后，我们在剩余给定列表的末尾链接关键字列表。 该解决方案的时间复杂度也是`O(n)`，并且它只需要遍历列表。
