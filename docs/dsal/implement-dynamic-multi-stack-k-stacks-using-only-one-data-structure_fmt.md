...
> 原文: [https://www.geeksforgeeks.org/implement-dynamic-multi-stack-k-stacks-using-only-one-data-structure/](https://www.geeksforgeeks.org/implement-dynamic-multi-stack-k-stacks-using-only-one-data-structure/)
...
*   将每个堆栈的`size`和`topIndex`存储在数组`size[]`和`topIndex[]`中。
...
*   如果堆栈的大小达到`max`保留容量，`expand`保留大小(乘以 2)
*   如果堆栈的大小降至保留大小的四分之一，则`shrink`保留大小(除以 2)
...
*   O(1)为`top()`功能。
*   [摊销](https://www.geeksforgeeks.org/analysis-algorithm-set-5-amortized-analysis-introduction/) O(1)为`push()`和`pop()`功能。
