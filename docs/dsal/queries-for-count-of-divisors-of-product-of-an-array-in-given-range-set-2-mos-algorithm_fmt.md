...
> 原文: [https://www.geeksforgeeks.org/queries-for-count-of-divisors-of-product-of-an-array-in-given-range-set-2-mos-algorithm/](https://www.geeksforgeeks.org/queries-for-count-of-divisors-of-product-of-an-array-in-given-range-set-2-mos-algorithm/)

给定一个大小为 `N` 和 `Q` 个查询 `【L，R】`，任务是在给定的范围内找到这个数组乘积的除数。

## 先决条件:
- [莫氏算法](https://www.geeksforgeeks.org/mos-algorithm-query-square-root-decomposition-set-1-introduction/)
- [模乘逆](https://www.geeksforgeeks.org/modular-multiplicative-inverse-1-n/)
- [使用筛子进行素分解](https://www.geeksforgeeks.org/prime-factorization-using-sieve-olog-n-multiple-queries/)
...
### 步骤 1
对所有查询进行排序，从 `0` 到 `√n–1` 的 `L` 值查询放在一起，然后从 `√n` 到 `2×√n–1` 的所有查询依次类推。一个块中的所有查询都按照 `R` 值的递增顺序进行排序。

### 步骤 2
逐个处理所有查询，每个查询都使用上一个查询中计算的结果。让 `result` 是先前查询的结果。

### 步骤 3
一个数 `n` 可以表示为 `n = ![\prod_{i=1}^{n} a_{i}^{p_{i}}](img/ca040f0723633d817f1fa7f4d7ed1ce0.png "Rendered by QuickLaTeX.com")`，其中 `a_i` 为质因数，`p_i` 为它们的整数次幂。
所以，对于这个因式分解，我们有公式可以求出除数 `n` 的总数，也就是:
> `![\prod_{i=1}^{n} (p_{i}+1)](img/a3b5d2d56d2975a6106c22f8d8961e50.png "Rendered by QuickLaTeX.com")`

### 步骤 4
在 `add()` 函数中，我们将计数器数组递增为 `counter[a[i]] = counter[a[i]] + p_i`。让 `prev` 存储 `counter[a[i]]` 的前一个值。现在随着计数器阵列的变化，结果变化为:
> *   `result = result / (prev + 1)` (除以 `prev + 1` 中和了前一个 `p_i` )
> *   `result = (result * (counter[a[i]] + 1))` (现在前一个结果被中和了，所以我们用新的计数相乘，即 `counter[a[i]] + 1`)

### 步骤 5
在 `remove()` 函数中，我们将计数器阵列递减为 `counter[a[i]] = counter[a[i]] – p_i`。现在随着计数器阵列的变化，结果变化为:
> *   `result = result / (prev + 1)` (除以 `prev + 1` 中和了前一个 `p_i` )
> *   `result = (result * (counter[a[i]] + 1))` (现在前一个结果被中和了，所以我们乘以新的计数，即 `counter[a[i]] + 1`)
