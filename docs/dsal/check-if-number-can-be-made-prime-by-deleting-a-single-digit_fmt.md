# 通过删除单个数字检查数字是否可以变成质数

> 原文: [https://www.geeksforgeeks.org/check-if-number-can-be-made-prime-by-deleting-a-single-digit/](https://www.geeksforgeeks.org/check-if-number-can-be-made-prime-by-deleting-a-single-digit/)
...
**进场:** 思路是把 `N` 转换成字符串。现在对字符串的每个数字进行迭代并[从字符串](https://www.geeksforgeeks.org/stdstringerase-in-cpp/)中删除索引 `i` 处的字符，然后在删除索引 `i` 处的字符后将字符串转换为整数，现在检查该整数是否为质数，然后返回 `true`。否则，最后返回 `false`。
