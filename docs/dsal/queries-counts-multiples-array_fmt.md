...
> 原文：[https://www.geeksforgeeks.org/queries-counts-multiples-array/](https://www.geeksforgeeks.org/queries-counts-multiples-array/)
...
**简单方法**是遍历整个数组中 `k` 的每个值，通过检查数组每个元素的模来计算总倍数，即对于 `i` (0 < i < n)的每个元素，检查 `arr[i] % k == 0` 与否。如果它能被 `k` 完全整除，那么递增计数。该方法的时间复杂度为 `O(n * k)`，对于 `k` 的大量查询效率不高。

**高效方法**是使用厄拉多塞的[筛的概念](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)。让我们定义数组 `arr[]` 中的最大值为 `MAX`。由于数组 `arr[]` 中所有数字的倍数总是小于最大值，因此我们将只迭代到 `MAX`。
现在对每个值（比如 `q`）迭代 **q，2q，3q，… t.k** (`tk <= MAX`)，因为所有这些数字都是 `q` 的倍数。同时在 `ans[]` 数组中为 `q`(1，2，… `MAX`)的每个值存储所有这些数字的计数。之后我们可以在 `O(1)` 时间内回答每个查询。
