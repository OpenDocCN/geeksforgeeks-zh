...
> 原文: [https://www.geeksforgeeks.org/number-of-pairs-of-lines-having-integer-intersection-points/](https://www.geeksforgeeks.org/number-of-pairs-of-lines-having-integer-intersection-points/)

给定两个整数数组 `P[]` 和 `Q[]`，其中 `P[i]` 和 `Q[j]` 对于每个 `0 <= i < size(P)` 和 `0 <= j < size(Q)` 表示直线方程 `x - y = -P[i]` 和 `x + y = Q[j]`。任务是从有整数交点的 `P[]` 和 `Q[]` 中找出对的个数。
...
让 `p0` 和 `p1` 分别为偶数和奇数 `p[i]` 的个数。
同理，偶数和奇数的数量分别为 `q0` 和 `q1`。
所以需要的答案是 `p0 * q0 + p1 * q1`。
