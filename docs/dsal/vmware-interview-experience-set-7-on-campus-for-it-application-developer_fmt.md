# VMware 面试体验|第 7 集(面向 IT 应用程序开发人员的校园内)
...
## 第 4 轮:技术第 2 轮
Q1) `安卓`讨论。(因为我在 `安卓`系统上实习过)
Q2)关于 `安卓`系统的`应用编程接口`安全性和认证的问题。
Q3)关于数据库的一般性讨论
Q4)`java` 中的`序列化`
Q5)`Java` 中的`瞬态`修饰符
Q6)什么时候使用`抽象类`，什么时候使用 `Java` 中的`接口`
...
`Armstrong Numbers` `Sieve of Eratosthenes` `Infix to Postfix` `All Practice Problems for VMWare`