...
> 原文：[https://www.geeksforgeeks.org/program-to-implement-inverse-interpolation-using-lagrange-formula/](https://www.geeksforgeeks.org/program-to-implement-inverse-interpolation-using-lagrange-formula/)

给定的任务是求未知函数 `y = f(x)` 的给定 `y` 的 `x` 值，其中给出了某些点 `(x, y)` 对的值。
设，`y = f(x)` 为未知函数，其中 `x` 为自变量。
对于不同的 `x` 值，给出相应的 `y_k = f(x_k)`，`k=0, 1, 2, 3…m` 值。
...
> 
> ![k, x_k, y_k 表格](img/482b8d8196842700de4c1da15b663d1e.png)
> 
> **输出：** `2.79501`
...
> STEP–1：初始化最终值 `x = 0`
> STEP–2：`I = 1` 到 `n` do
> STEP–3：初始化 `xi = data[i].x`
> STEP–4：`FOR j = 1` 到 `n` do
> STEP–5：`IF I != j` do
> STEP–6：将 `xi` 乘以 `(y - data[j].y)` 和除以 `(data[i].y - data[j].y)`
> ENDIF
> END for
> STEP–7：将 `xi` 加到 `x`
> END for
