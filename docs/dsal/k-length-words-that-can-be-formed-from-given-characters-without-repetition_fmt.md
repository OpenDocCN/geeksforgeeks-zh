...
> 原文：`https://www.geeksforgeeks.org/k-length-words-that-can-be-formed-from-given-characters-without-repetition/`
...
**进场：** 统计`str`中不同字符的个数并存储在`cnt`中，现在的任务是将`cnt`字符中的`k`字符排列出来，即`P(n, r) = n! / (n - r)!`。
