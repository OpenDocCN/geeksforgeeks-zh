...
> 原文: [https://www.geeksforgeeks.org/solving-sudoku-using-bitwise-algorithm/](https://www.geeksforgeeks.org/solving-sudoku-using-bitwise-algorithm/)
...
步骤 (3.1) 可以通过遍历相应的行、列和 3×3 子矩阵来执行。然而，我们可以通过在回溯之前预处理这些数字来使这一步更快，这是本文的主要观点。因此，让我们考虑下面的矩阵作为例子：
...
然后在步骤 (3.1) 中，我们可以使用按位运算来确定数字 `i` 是否在行、列和 3×3 子矩阵中。假设 `rowDigits[r]` 是包含行 `r` 的数字的整数，那么我们可以用下面的表达式来检查数字 `i` 是否在行 `r` 中：
...
一旦步骤 (3.1) 的条件为真，就执行步骤 (3.1a)，然后我们需要将数字 `i` 插入到 `rowDigits`、`columnDigits` 和 `subMatrixDigits` 中，我们可以用以下表达式完成：
...
在步骤 (3.1c) 的条件为假的情况下，执行步骤 (3.1d)，然后我们需要从 `rowDigits`、`columnDigits` 和 `submatrixDigits` 中移除数字 `i`，我们可以用以下表达式完成：
