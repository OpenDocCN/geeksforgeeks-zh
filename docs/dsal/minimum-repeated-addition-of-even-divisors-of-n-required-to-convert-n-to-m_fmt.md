...
> 原文: [https://www.geeksforgeeks.org/minimum-repeated-addition-of-even-divisors-of-n-required-to-convert-n-to-m/](https://www.geeksforgeeks.org/minimum-repeated-addition-of-even-divisors-of-n-required-to-convert-n-to-m/)
...
**输出:** `4`
**解释:**
第一步: 加 `2`（`2` 是偶数除数且 `2 != 6`）到 `6`。现在 `N` 变成了 `8`。
第二步: 加 `4`（`4` 是偶数除数且 `4 != 8`）到 `8`。现在 `N` 变成了 `12`。
第三步: 加 `6`（`6` 是偶数除数且 `6 != 12`）到 `12`。现在 `N` 变成了 `18`。
第四步: 加 `6`（`6` 是偶数除数且 `6 != 18`）到 `18`。现在 `N` 变成 `24 = M`。
因此，需要 `4` 个步骤。
...
最简单的解决方法是考虑一个数的所有可能的偶因子，递归地为它们计算答案，最后返回它的最小值。
