...
> 原文：[https://www.geeksforgeeks.org/print-strings-in-reverse-dictionary-order-using-trie/](https://www.geeksforgeeks.org/print-strings-in-reverse-dictionary-order-using-trie/)

`Trie`是一种高效的`信息检索数据结构`。使用`Trie`，搜索复杂性可以达到最佳极限。
给定`一串字符串`。任务是使用`[Trie](https://www.geeksforgeeks.org/trie-insert-and-search/)`以相反的字典顺序打印所有字符串。如果输入数组中有重复项，我们只需要打印一次。
