...
> 原文：[https://www.geeksforgeeks.org/traversal-of-a-graph-in-lexicographical-order-using-bfs/](https://www.geeksforgeeks.org/traversal-of-a-graph-in-lexicographical-order-using-bfs/)

给定一个由 `N` 节点、一个源 `S` 和一个类型为 `{u, v}` 的 `Edges[]` 数组组成的图 `G`，表示在节点 `u` 和 `v` 之间有一条无向边，任务是使用 [BFS 按照字典顺序遍历该图](https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/)。
...
*   初始化一个图，比如说 `G`，按照节点的字典顺序存储一个节点的所有相邻节点。
*   初始化一个图，比如说 `vis`，来检查一个节点是否已经被遍历。
*   遍历 `Edges[][2]` 数组，将图中每个节点的所有相邻节点存储在 `G` 中。
...
void CreateGraph(int N, int M, char S,
                 char Edges[][2])
{
