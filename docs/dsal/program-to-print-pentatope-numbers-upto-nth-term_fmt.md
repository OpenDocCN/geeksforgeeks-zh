...
> 原文: [https://www.geeksforgeeks.org/program-to-print-pentatope-numbers-upto-nth-term/](https://www.geeksforgeeks.org/program-to-print-pentatope-numbers-upto-nth-term/)

## 先决条件

*   [四面体数](https://www.geeksforgeeks.org/tetrahedral-numbers/)
*   [五元数](https://www.geeksforgeeks.org/pentatope-number/)

给定一个值 `n`，任务是打印直到第 `n` 项的五元数列。
...
这个问题很容易用第 `N` 个五元数等于前 `N` 个四面体数之和来解决。我们来看看五元和四面体数系列。
...
使用公式计算第 `N` 个四面体数：`\frac{N\times (N+1)\times (N+2)}{6}`。

因此，通过生成四面体数并将其与之前生成的所有四面体数之和相加来打印五元数系列。以下是上述方法的实现：
...
该公式求第 `N` 个五元数：`\frac{N\times (N+1)\times (N+2)\times (N+3)}{24}`。
