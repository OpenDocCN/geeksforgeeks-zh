...
> 原文: [https://www.geeksforgeeks.org/permutations-of-string-such-that-no-two-vowels-are-adjacent/](https://www.geeksforgeeks.org/permutations-of-string-such-that-no-two-vowels-are-adjacent/)
...
假设在一个字符串中，元音的数量是 `vowelCount`，辅音的数量是 `consonantCount`。

> 因此，
> **总方式** = (`consonantCount!` / `copyConsonant!`) * `C(consonantCount + 1, vowelCount)` * (`vowelCount!` / `copyVowel!`)
