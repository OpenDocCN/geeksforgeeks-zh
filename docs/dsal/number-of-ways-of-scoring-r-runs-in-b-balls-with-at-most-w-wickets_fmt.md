...
> 原文: [https://www.geeksforgeeks.org/number-of-ways-of-scoring-r-runs-in-b-balls-with-at-most-w-wickets/](https://www.geeksforgeeks.org/number-of-ways-of-scoring-r-runs-in-b-balls-with-at-most-w-wickets/)

给定三个整数 `R`、`B` 和 `W`，表示**运行**、**球**和**检票口**的数量。在板球比赛中，一个人可以在单个球中得分 **0、1、2、3、4、6** 或**一个小门**。任务是计算一个团队可以精确得分的方式数量 `R` 跑在精确的 `B` 球中最多有 `W` 个检票口。由于途径数量较多，打印答案时取模 `1000000007`。
...
*   如果一个队**在一个球**上失去了 1 个小门，那么 `跑=跑`、`球=球+ 1` 和 `小门=小门+ 1`。

DP 将由三种状态组成，运行状态最大为 `6 *球`，因为这是最大可能。因此 `DP[i][j][k]` 表示 `i` 跑的次数可以精确地在 `j` 球中得分，同时失去 `k` 个检票口。
