...
> 原文：[https://www.geeksforgeeks.org/find-prime-factors-of-z-such-that-z-is-product-of-all-even-numbers-till-n-that-are-product-of-two-distinct-prime-numbers/](https://www.geeksforgeeks.org/find-prime-factors-of-z-such-that-z-is-product-of-all-even-numbers-till-n-that-are-product-of-two-distinct-prime-numbers/)
...
2.  因此，`Z` 的素数因式分解将是 `2^X * 3^1 * 5^1 * … * P^1` 的形式，其中 `P` 是最后一个素数 ≤ `N/2`，`X` 是素数在 `[3, N/2]` 范围内的个数。

## 方法

按照步骤解决问题：

1.  将所有素数 ≤ `N / 2` 存储，使用厄拉多塞的[筛在一个向量中，说 `prime`。](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)
