# 检查移动任意一个坐标是否可以形成直角三角形

> 原文: [https://www.geeksforgeeks.org/check-if-a-right-angled-triangle-can-be-formed-by-moving-any-one-of-the-coordinates/](https://www.geeksforgeeks.org/check-if-a-right-angled-triangle-can-be-formed-by-moving-any-one-of-the-coordinates/)
...
众所周知，对于边长为 `a`、`b` 和 `c` 的三角形，如果下列等式成立，则三角形将为**直角**：`a^2 + b^2 = c^2`。
所以对于三角形的每个坐标，找出所有的边和。
如果上述条件不成立，那么需要进行以下操作：
我们需要将所有坐标逐一更改 1，并检查它是否是直角三角形的有效组合。
看一下，可以有 4 种可能的组合将每个坐标改变 1。它们是 `(-1, 0)`、`(0, 1)`、`(1, 0)`、`(0, -1)`。因此，运行一个循环，对每个坐标逐一应用这些更改，并检查公式 `a^2 + b^2 = c^2` 是否正确。
