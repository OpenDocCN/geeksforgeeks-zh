...
> 原文: [https://www.geeksforgeeks.org/check-if-the-remainder-of-n-1-factorial-when-divided-by-n-is-n-1-or-not/](https://www.geeksforgeeks.org/check-if-the-remainder-of-n-1-factorial-when-divided-by-n-is-n-1-or-not/)

给定一个整数 `N`，其中 `1≤N≤10^5`，任务是查找 `(N-1)! % N = N–1` 与否。
...
**天真法:** 解决上面提到的问题，天真法就是找 `(N–1)!` 并检查是否 `(N-1)! % N = N–1` 或不是。但是这种方法会导致溢出，因为 `1 ≤ N ≤ 10^5`。
