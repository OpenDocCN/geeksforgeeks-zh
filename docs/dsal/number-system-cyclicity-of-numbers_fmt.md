...
> 原文: [https://www.geeksforgeeks.org/number-system-cyclicity-of-numbers/](https://www.geeksforgeeks.org/number-system-cyclicity-of-numbers/)
...
任何数的循环性主要集中在其单位数上。当上升到任何[幂](https://www.geeksforgeeks.org/modular-exponentiation-power-in-modular-arithmetic/)时，每个单位数字都有自己的重复模式。这个概念在解决智能问题时非常有用。数字的循环性的概念可以通过计算所有从 0 到 9 的一位数的单位位数来学习。这些数字可大致分为以下三类:
...
**问题 1:** 找到 `416 ^ 345` 的单位数字。

**答案:** 只需找到 `6 ^ 345` 即可给出 6 作为单位数字，因此 `416 ^ 345` 的单位数字为 6。

**问题 2:** 找到 `235 ^ 34566` 的单位数字。

**答案:** 找到 `5 ^ 34566` 会给出 5 作为单位数字，因此 `235 ^ 34566` 的单位数字是 5。
...
*   `4 ^ 奇数 = 4`: 如果 4 是奇数的幂，那么单位数字就是 4。
*   `4 ^ 偶数 = 6`: 如果 4 的幂是偶数，那么单位数字就是 6。
*   `9 ^ 奇数 = 9`: 如果 9 是奇数的幂，那么单位数字就是 9。
*   `9 ^ 偶数 = 1`: 如果 9 的幂是偶数，那么单位数字就是 1。

以下是基于上述概念的一些问题:

**问题 1:** 找到 `414 ^ 23` 的单位数字。

**回答:** 23 是奇数，所以 `4 ^ 奇数 = 4`，因此单位数字是 4。

**问题 2:** 找到 `29 ^ 82` 的单位数字。

**回答:** 82 是偶数，所以 `9 ^ 偶数 = 1`，因此单位数字为 1。
...
**问题 1:** 找到 `257 ^ 345` 的单位数字。

**回答:** `345 % 4 = 1`，所以 `7 ^ 1`，因此单位数字是 7。

**问题 2:** 找到 `423 ^ 43` 的单位数字。

**回答:** `43 % 4 = 3`，所以 `3 ^ 3`，因此 7 是单位数字。

**问题 3:** 找到 `28 ^ 146` 的单位数字。

**回答:** `146 % 4 = 2`，所以 `8 ^ 2`，因此单位数字是 4。
