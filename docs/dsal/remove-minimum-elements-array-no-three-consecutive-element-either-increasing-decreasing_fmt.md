...
> 原文: [https://www.geeksforgeeks.org/remove-minimum-elements-array-no-three-consecutive-element-either-increasing-decreasing/](https://www.geeksforgeeks.org/remove-minimum-elements-array-no-three-consecutive-element-either-increasing-decreasing/)

给定一组不同的正整数。任务是找到要移除的元素的最小数量，使得数组中没有三个连续的元素在增加或减少。也就是说，拆除后要么是一个`a[i-1] > a[i] < a[i+1]`，要么是一个`a[i-1] < a[i] > a[i+1]`。
...
观察，对于 `n < 3`, `output` 将为 0，因为不需要移除任何元素。

**方法 1 O(n^2 ):**
如果我们仔细观察，我们可以注意到 `arr` 中的元素移除后，剩下的是 `arr` 的之字形子序列。假设新数组是 `arr'`。那么我们为到达 `arr'` 所执行的删除次数就是 `arr` 的大小 – `arr'` 的大小。我们的目标是最小化这种差异。注意 `arr` 的大小是固定的，所以我们必须最小化 `arr'` 的大小或最大化 `arr'` 的大小。这样问题就简化为寻找[最长的之字形子序列](https://www.geeksforgeeks.org/longest-zig-zag-subsequence/)。
...
**这是怎么工作的？**
我们来定义一个 `a[i]` 为：
*   峰 (Peak)：如果 `0 < i < n–1` 且 `a[i-1] < a[i] > a[i+1]`。
*   谷 (Valley)：若 `0 < i < n–1` 且 `a[i-1] > a[i] < a[i+1]`。

设 `P(arr)` 和 `V(arr)` 分别为峰数和谷数。
观察到，在任何之字形排列中，除端点外的所有元素都是峰或谷，即 `n = P(arr) + V(arr) + 2`。

现在，我们可以表明任何删除都不会增加任何数组中峰谷的总数:
1.  假设我们去掉一个既不是峰也不是谷的元素 `a[i]`:
    *   如果 `a[i]` 是一个端点，那么显然没有峰或谷可以形成。
    *   如果 `a[i-1] < a[i] < a[i+1]`，那么在移除 `a[i]` 之后，元素 `a[i-1]` 和 `a[i+1]` 将相邻。但是由于 `a[i-1] < a[i+1]`，`a[i-1]` 或 `a[i+1]` 的状态不会改变，即如果 `a[i-1]` 或 `a[i+1]` 最初是一个峰/谷/两者都不变，那么在移除之后它将保持一个峰/谷/两者都不变，因为它们与其邻居的比较将是相同的。特别是，没有形成新的峰或谷。
    *   类似的情况还有 `a[i-1] > a[i] > a[i+1]`。
2.  假设我们去掉一个峰或谷:
    *   假设我们去掉一个峰，那么 `a[i-1] < a[i] > a[i+1]`。现在有可能 `a[i-1]` 或 `a[i+1]` 会变成一个峰或谷。但是我们可以表明，它们不能同时改变，因为要么 `a[i-1] < a[i+1]`，这意味着 `a[i-1]` 的状态永远不变，要么 `a[i-1] > a[i+1]`，这意味着 `a[i+1]` 的状态永远不变。因此，将形成一个峰或谷，但是由于我们移除了一个峰，峰/谷的总数不会增加。
    *   当我们移除一个山谷，即 `a[i-1] > a[i] < a[i+1]` 时，同样的事情也会发生。
...
2. For each element arr[i] from index i = 1 to n - 2.
   (i) if (arr[i-1] < arr[i] && arr[i] < arr[i+1]) or (arr[i-1] > arr[i] && arr[i] > arr[i+1])
