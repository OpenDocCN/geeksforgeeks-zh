...
> 原文: [https://www.geeksforgeeks.org/find-the-number-of-primitive-roots-modulo-prime/](https://www.geeksforgeeks.org/find-the-number-of-primitive-roots-modulo-prime/)

给定一个质数 `p`。任务是统计 `p` 的所有原始根。

一个[**本原根**](https://en.wikipedia.org/wiki/Primitive_root_modulo_n)是整数 `x` (`1 <= x < p`)，使得整数 `x-1`，`x^2-1`，…，`x^{p-2}-1` 可被 `p` 整除，但 `x^{p-1}-1` 可被 `p` 整除。
