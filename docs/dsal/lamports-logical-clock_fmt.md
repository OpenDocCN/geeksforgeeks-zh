...
*   **逻辑时钟:** 逻辑时钟的标准是:
    *   【C1】: `C_I(a) < C_I(b)` 【`C_I`->逻辑时钟，如果‘a’发生在‘b’之前，那么在特定过程中‘a’的时间会小于‘b’。]
    *   【C2】: `C_I(a) < C_j(b)` 【`C_I(a)`的时钟值小于 `C_j(b)`】

## 参考

*   **流程:** `P_i`
*   **事件:** `E_ij`，其中 I 为过程数，j: `j_{T8}` 事件中 `i^{th}` 过程。
*   `t_m`: 消息 m 的向量时间跨度
*   `C_i` 与进程 `P_i` 关联的矢量时钟， `j^{th}` 元素为 `Ci[j]`，包含 `P_i` 的进程中当前时间的最新值 `P_j`。
*   `d`: 漂移时间，一般 `d` 为 1。

## 实施细则【IR】

*   **【IR1】:** 如果 `a -> b` ['a '发生在同一过程中的' b '之前]，那么，`C_I(b) = C_I(a) + d`
*   **【IR2】:** `C_j = max(C_j, t_m + d)` 【如果工序数较多，则 `t_m = C_I(a)` 的值，`C_j = max(C_j, t_m + d)`】
...
*   如果没有输入值，即遵循[IR1]，下一个点的值将继续增加 `d` (`d = 1`)。
    ...
    *   `e24 = e23 + d = 3 + 1 = 4`
    *   `e26 = e25 + d = 6 + 1 = 7`
*   当有进值时，则跟随**【IR2】**，即取 `C_j` 和 `t_m + d` 之间的最大值。
    *   `e17 = max(7, 5) = 7`，[`e16 + d = 6 + 1 = 7`，`e24 + d = 4 + 1 = 5`，7 和 5 之间的最大值为 7]
    *   `e23 = max(3, 3) = 3`，[`e22 + d = 2 + 1 = 3`，`e12 + d = 2 + 1 = 3`，3 和 3 之间的最大值为 3]
    *   `e25 = max(5, 6) = 6`，[`e24 + 1 = 4 + 1 = 5`，`e15 + d = 5 + 1 = 6`，5 和 6 之间的最大值为 6]
