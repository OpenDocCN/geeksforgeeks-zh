...
> 原文: [https://www.geeksforgeeks.org/find-the-last-player-to-be-able-to-remove-a-string-from-an-array-which-is-not-already-removed-from-other-array/](https://www.geeksforgeeks.org/find-the-last-player-to-be-able-to-remove-a-string-from-an-array-which-is-not-already-removed-from-other-array/)

给定两个大小分别为 `N` 和 `M` 的字符串数组 `arr[]` 和 `brr[]`，任务是当两个玩家按照以下规则进行最佳游戏时，找到游戏的赢家：

*   **玩家 1** 开始游戏。
*   **玩家 1** 从数组 `arr[]` 中移除一个字符串，如果它还没有从数组 `brr[]` 中移除。
*   **玩家 2** 从数组 `brr[]` 中移除一个字符串，如果它还没有从数组 `arr[]` 中移除。
*   不能从数组中移除字符串的玩家将输掉游戏。

## 示例

> **输入:** `arr[] = {"geeks", "geek"}`, `brr[] = {"geeks", "geeksforgeeks"}`
> **输出:** 玩家 1
> **解释:**
> **回合 1:** 玩家 1 从 `arr[]` 中移除了 "geeks"。
> **第二回合:** 玩家 2 从 `brr[]` 移除 "geeks"。
> **第三回合:** 玩家 1 从 `brr[]` 移除 "geek"。
> 现在，玩家 2 不能移除任何字符串。
> 因此，需要的输出是玩家 1。
...
这个想法是基于这样一个事实，即两个数组中的公共字符串只能从其中一个数组中移除。按照以下步骤解决问题：

*   如果两个数组中公共字符串的计数都是奇数，那么从数组 `brr[]` 中移除一个字符串，因为玩家 1 开始游戏，第一个公共字符串被玩家 1 移除。
