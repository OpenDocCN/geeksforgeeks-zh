...
> 原文: [https://www.geeksforgeeks.org/print-characters-having-prime-frequencies-in-order-of-occurrence/](https://www.geeksforgeeks.org/print-characters-having-prime-frequencies-in-order-of-occurrence/)

给定一个只包含小写字符的字符串 `str`。任务是按照出现的顺序打印具有质数频率的字符。

**注意**：具有质数频率的重复元素按照其出现的顺序被打印的次数与它们出现的次数一样多。
...
**方法:** 创建一个频率数组来存储给定字符串 `str` 中每个字符的频率。再次遍历字符串，使用厄拉多塞[筛](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)检查该字符的频率是否为质数。
