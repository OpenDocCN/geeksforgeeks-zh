...
> 原文: [https://www.geeksforgeeks.org/count-1s-present-in-a-range-of-indices-l-r-in-a-given-array/](https://www.geeksforgeeks.org/count-1s-present-in-a-range-of-indices-l-r-in-a-given-array/)

给定一个由单个元素`N`（`1≤N≤10^6`）和两个整数`L`和`R`（`1 ≤ L ≤ R ≤ 10^5`）组成的[数组`arr[]`](https://www.geeksforgeeks.org/array-data-structure/)，任务是使所有数组元素成为`0`或`1`。

- 从数组`arr[]`中选择一个元素`P`使得`P > 1`。
- 将`P`替换为同一位置的三个元素，依次为`floor(P/2)`、`P%2`、`floor(P/2)`。因此，每次操作后数组`arr[]`的大小增加`2`。

执行完所有操作后，在数组`arr[]`中打印索引`[L, R]`范围内`1`的总数的计数。
