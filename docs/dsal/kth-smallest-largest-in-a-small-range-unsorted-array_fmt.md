...
> 原文: [https://www.geeksforgeeks.org/kth-smallest-largest-in-a-small-range-unsorted-array/](https://www.geeksforgeeks.org/kth-smallest-largest-in-a-small-range-unsorted-array/)

在未排序的数组中查找第 `k` 个最小或最大的元素，其中 `k <= size of array`。已知数组元素的范围较小。
...
由于给定的数组元素在小范围内，我们可以利用索引表做一些类似`计数排序`的事情。我们存储元素的计数，然后遍历计数数组并打印第 `k` 个元素。
