...
> 原文: [https://www.geeksforgeeks.org/calculate-sum-of-scores-after-n-days-based-on-given-conditions/](https://www.geeksforgeeks.org/calculate-sum-of-scores-after-n-days-based-on-given-conditions/)
...
解决问题最简单的方法就是遍历 `[1, N]` 的范围，每天不停的加分数。最后，打印获得的所有分数的总和。
...
> 前 `F` 周得分之和:
> 第一周得分之和 = `1 + 2 + 3 + … + 7`
> 第二周得分之和 = `2 + 3 + 4 + … + 8 = 7*1 + (1 + 2 + 3 + … + 7)`
> 第三周的分数总和 = `3+4+5+……+9 = 7 * 2+(1+2+3+……+7)`
> ……
> 第 `F` 周的分数总和 = `7 *(F–1)+(1+2+3+……+7)`
