...
> 原文：[https://www.geeksforgeeks.org/optimum-location-point-minimize-total-distance/](https://www.geeksforgeeks.org/optimum-location-point-minimize-total-distance/)

给定一组点和一条线 `ax+by+c = 0`。我们需要在给定的线上找到一个点，对于这个点，与给定的点集的距离之和是最小的。
...
因此，解决方案如下进行，我们从 `low` 和 `high` 开始，分别初始化为一些最小和最大的值，然后我们开始迭代，在每次迭代中，我们计算两个 `mid`，`mid1` 和 `mid2`，它们代表搜索空间中的 1/3 和 2/3 位置，我们用 `mid1` 和 `mid2` 计算所有点的总距离，并通过比较这些距离成本来更新 `low` 或 `high`，这个迭代一直持续到 `low` 和 `high` 变得近似相等。
