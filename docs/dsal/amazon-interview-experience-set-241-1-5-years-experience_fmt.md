...
## 第 2 轮(海德拉巴 Q 市)–酒吧募捐者

1.  关于项目，你的贡献，你的角色和责任，面临的挑战等。

2.  为什么是亚马逊？

3.  给定一个只包含`I`s 和`D`s 的模式。`I` 代表增加，`D` 代表减少。设计一种算法，按照该模式打印最小数量。1-9 的数字和数字不能重复。
...
1.  [给定一个`BST`的预序遍历。构建`BST`。](https://practice.geeksforgeeks.org/problems/construct-tree-from-preorder-traversal/1)
