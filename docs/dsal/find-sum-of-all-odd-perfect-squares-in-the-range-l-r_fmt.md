...
> 原文: [https://www.geeksforgeeks.org/find-sum-of-all-odd-perfect-squares-in-the-range-l-r/](https://www.geeksforgeeks.org/find-sum-of-all-odd-perfect-squares-in-the-range-l-r/)
...
> **输入**: `L = 1`, `R = 9`
> **输出**: `10`
> **说明**: 范围内奇数为 1, 3, 5, 7, 9，只有 1, 9 是完美平方。所以，`1 + 9 = 10`。
>
> **输入**: `L = 50`, `R = 10000`
> **输出**: `166566`
...
解决这个问题的基本思路是遍历 `L` 到 `R` 范围内的数字，对于每个奇数，也要检查它是否是一个完美的正方形。
...
解的逼近基于序列的数学概念。想法是使用第一个 `N` 奇数的平方和。

> **前 n 个奇数自然数的平方和**:
> ![\sum (2n-1)^{2} = \frac{n(2n+1)(2n-1)}{3}](img/59d1b914cabe92e86624e387aa61618c.png "Rendered by QuickLaTeX.com")

按照以下步骤解决问题：

1.  检查 1 和刚好大于或等于 `L` 的完美平方奇数之间的完美平方的计数。
2.  检查 `[1, L]` 范围内奇数正方体的计数。
