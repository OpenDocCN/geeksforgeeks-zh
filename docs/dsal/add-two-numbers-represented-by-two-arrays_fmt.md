...
> 原文: [https://www.geeksforgeeks.org/add-two-numbers-represented-by-two-arrays/](https://www.geeksforgeeks.org/add-two-numbers-represented-by-two-arrays/)

给定两个数组 `A[0..n-1]` 和 `B[0..m-1]`，大小分别为 `n` 和 `m`，代表两个数字，使得数组的每个元素代表一个数字。例如，`A[] = { 1, 2, 3}` 和 `B[] = { 2, 1, 4 }` 分别代表 123 和 214。任务是找出两个数的和。在上述情况下，答案是 337。
