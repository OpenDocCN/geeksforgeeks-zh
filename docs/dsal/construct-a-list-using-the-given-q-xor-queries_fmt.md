...
> 原文: [https://www.geeksforgeeks.org/construct-a-list-using-the-given-q-xor-queries/](https://www.geeksforgeeks.org/construct-a-list-using-the-given-q-xor-queries/)
...
> `[0 6 3 2]` (列表增加 2)
> `[4 2 7 6]` (每个元素用 4 异或)
> `[4 2 7 6]` (每个元素用 5 异或) -> 排序后为 `[1 2 3 7]`
>
> **输入:** `Q[][] = {{0, 2}, {1, 3}, {0, 5}}`
> **输出:** `1 3 5`
> **解释:**
...
1.  用 `0` 初始化一个列表，然后遍历每个查询，并执行以下操作:
    *   对于查询类型 `0`，在列表中添加给定值。
    *   对于查询类型 `1`，遍历列表并使用它们各自的[值按位异或](https://www.geeksforgeeks.org/bitwise-operators-in-c-cpp/)来更新每个元素。
...
通过跟踪从右到左工作的累积[逐位异或](https://www.geeksforgeeks.org/bitwise-operators-in-c-cpp/)，以相反的顺序处理数字要容易得多。按照以下步骤解决问题:
