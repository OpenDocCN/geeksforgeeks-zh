...
> 原文: [https://www.geeksforgeeks.org/replace-all-elements-by-difference-of-sums-of-positive-and-negative-numbers-after-that-element/](https://www.geeksforgeeks.org/replace-all-elements-by-difference-of-sums-of-positive-and-negative-numbers-after-that-element/)
...
这种方法的时间复杂度为 `O(N^2)`，其中 `N` 是数组中的元素数量。
...
**有效方法:** 将正负和初始化为 0。现在从最后一个元素到第一个元素运行一个 `for` 循环，并计算 `diff = ABS(pos_sum) - ABS(neg_sum)`。
