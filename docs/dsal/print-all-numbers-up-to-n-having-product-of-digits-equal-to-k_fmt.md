...
> 原文：[https://www.geeksforgeeks.org/print-all-numbers-up-to-n-having-product-of-digits-equal-to-k/](https://www.geeksforgeeks.org/print-all-numbers-up-to-n-having-product-of-digits-equal-to-k/)

给定两个整数 `N` 和 `K`，任务是打印范围 `[1, N]` 中的所有数字，这些数字的各位数字之积等于 `K`。如果没有找到这样的数字，则打印 `-1`。
...
>
> **输入：** `N = 500`，`K = 10`
> **输出：** `25 52 125 152 215 251`

## 方法

按照以下步骤解决问题：

*   初始化一个变量，比如 `flag`，来存储是否存在满足给定条件的数字。
*   声明一个函数 `productOfDigits()`，求数字各位数字的乘积。
*   迭代范围 `[1, N]`：
    *   如果 `i` 的数字乘积等于 `K`，则打印该数字并设置 `flag = 1`。
*   如果 `flag` 等于 `0`，则表示在 `[1, N]` 范围内找不到这样的数字。因此，打印 `-1`。
