...
> 原文：[https://www.geeksforgeeks.org/program-for-finding-the-integral-of-a-given-function-using-booles-rule/](https://www.geeksforgeeks.org/program-for-finding-the-integral-of-a-given-function-using-booles-rule/)
...
本文讨论了用[布尔规则](https://en.wikipedia.org/wiki/Boole%27s_rule)计算给定函数`f(x)`的近似整数值。
布尔规则是一种求积分近似值的数值积分技术。它以自学成才的数学家、哲学家和逻辑学家乔治·布尔的名字命名。布尔技术的思想是在一些等间距的值（给定图像中的`h`）下，使用`f_k`的值来近似积分。下图显示了如何考虑各种`f_k`：
