...
> 原文: [https://www.geeksforgeeks.org/weighted-job-scheduling-set-2-using-lis/](https://www.geeksforgeeks.org/weighted-job-scheduling-set-2-using-lis/)
...
我们首先需要根据开始时间对工作进行分类。让作业 `[0..n-1]` 是排序后的作业数组。我们定义向量 `L`，使得 `L[i]` 本身是存储以作业 `i` 结束的加权作业调度的向量。因此，对于索引 `i`，`L[i]` 可以递归地写成–
...
**上述动态规划解的时间复杂度**为 `O(n^2)`，其中 `n` 为作业数。
**程序使用的辅助空间**为 `O(n^2)`。
