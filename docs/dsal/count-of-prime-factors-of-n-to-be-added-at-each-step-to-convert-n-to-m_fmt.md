...
给定两个整数`N`和`M`，任务是找出将`N`转换为`M`所需的最小操作数。每个操作都涉及将`N`当前值的[质因子](http://www.geeksforgeeks.org/print-all-prime-factors-of-a-given-number/)之一相加。如果有可能获得`M`，则打印操作数。否则，打印`-1`。
...
该问题可以通过使用 [BFS](https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/) 获得达到`M`的最小步长和[埃拉托斯特尼筛法](http://www.geeksforgeeks.org/sieve-of-eratosthenes/)来预先计算质数来解决。

请按照以下步骤解决问题：

*   使用筛法存储并预计算所有素数。
*   现在，如果`N`已经等于`M`，则打印 0，因为不需要加法运算。
*   将此问题可视化为执行 **BFS** 的图问题。在每个级别上，通过添加质数来存储上一级别中`N`的值可获得的数字。
*   现在，首先在队列中插入`(N, 0)`，其中`N`表示该值，0 表示达到该值的操作数。
*   在队列的每个级别，通过提取 `front()` 处的元素来逐个遍历所有元素，然后执行以下操作：
    1.  将 `q.front().first` 存储在 `newNum` 中，将 `q.front().second` 存储在 `distance` 中，其中 `newNum` 是当前值，`distance` 是达到该值所需的操作次数。
    2.  将 `newNum` 的所有质因子存储在[集合](https://www.geeksforgeeks.org/set-in-cpp-stl/)中。
    3.  如果 `newNum` 等于`M`，则打印`distance`，因为它是所需的最小操作。
    4.  如果 `newNum` 大于`M`，则中断。
    5.  否则，`newNum` 小于`M`。因此，通过逐个添加其质因子`i`来更新`newNum`，并将`(newNum + i, distance + 1)`存储在队列中，并为下一级重复上述步骤。
