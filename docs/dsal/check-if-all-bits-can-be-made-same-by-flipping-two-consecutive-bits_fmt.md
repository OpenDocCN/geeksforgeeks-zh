...
> 原文: [https://www.geeksforgeeks.org/check-if-all-bits-can-be-made-same-by-flipping-two-consecutive-bits/](https://www.geeksforgeeks.org/check-if-all-bits-can-be-made-same-by-flipping-two-consecutive-bits/)
...
仔细观察，第 `i` 位和第 `j` 位的切换可以通过从第 `i` 位开始切换来完成，比如(`i`，`i+1`)，(`i+1`，`i+2`) …。(`j-1`，`j`)这里，除了 `i` 和 `j` 之外，每个位都切换两次(如果位切换两次，则它到达初始值)，然后最终第 `i` 位和 `j` 位切换。因此，可以说，当 1 和 0 的计数都是奇数时，不可能使二进制字符串中的所有数字相等。
