...
> 原文: [https://www.geeksforgeeks.org/count-all-grandparent-parent-child-triplets-in-a-binary-tree-whose-sum-is-greater-than-x/](https://www.geeksforgeeks.org/count-all-grandparent-parent-child-triplets-in-a-binary-tree-whose-sum-is-greater-than-x/)

给定一个整数 `X` 和一个二叉树，任务是计算节点的三元组的数量，使得它们的和大于 `X`，并且它们具有祖父母->父母->子女关系。
...
**高效方法:** 这个问题可以通过维护 3 个名为 `grandParent`、`parent` 和 `child` 的变量来解决。它可以在不使用其他数据结构的情况下在常数空间中完成。

1.  按顺序遍历树。
2.  维护 3 个名为 `grandParent`、`parent` 和 `child` 的变量。
3.  每当我们的总和超过目标时，我们就可以增加计数或打印三元组。
