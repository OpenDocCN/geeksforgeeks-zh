...
> 原文: [https://www.geeksforgeeks.org/convert-array-gcd-array-becomes-1/](https://www.geeksforgeeks.org/convert-array-gcd-array-becomes-1/)
...
> 输入: `arr = {10, 15, 30}`, `k = 6`
> 输出: 是
> 将数组的所有元素除以 5，因为它是所有元素的除数，也小于 `k`，即 6。给出类似 `{2, 3, 6}` 的序列。现在，把 2 除以 2，3 除以 3，6 除以 2。然后，序列变成 `{1, 1, 3}`。现在，数组的 gcd 是 1。

> 输入: `arr = {5, 10, 20}`, `k = 4`
> 输出: No
> 这里，10 除以 2 的顺序变成 `{5, 5, 20}`。然后，20 除以 2，再除以 2。最后，序列变成 `{5, 5, 5}`。要使成为该序列的 gcd 1，将每个元素除以 5。但是 5 > 4 所以这里不可能做成 gcd 1。
