...
> 原文: [https://www.geeksforgeeks.org/sum-of-elements-of-a-geometric-progression-gp-in-a-given-range/](https://www.geeksforgeeks.org/sum-of-elements-of-a-geometric-progression-gp-in-a-given-range/)

给定几何级数 `arr[]` 和 `Q` 个以 `[L, R]` 形式的查询，其中 `L` 是范围的左边界，`R` 是右边界。任务是找到给定范围内几何级数元素的和。
...
> 7
> **说明:**
> 范围 1: `arr = {7, 7, 7, 7, 7, 7}`。因此总和= 42
> 范围 2: `arr = {7, 7, 7}`。因此总和= 21
> 范围 3: `arr = {7}`。因此总和= 7

## 逼近

由于给定的序列是一个几何级数，所以可以很容易地分两步高效地求出和:
1.  获取范围的第一个元素。
2.  如果 `d = 1`，则乘以 `d*k` 到它，否则乘以 `(d^k – 1)/(d – 1)` 到它，其中 `d` 是 GP 的公比，`k` 是范围内的元素数。
