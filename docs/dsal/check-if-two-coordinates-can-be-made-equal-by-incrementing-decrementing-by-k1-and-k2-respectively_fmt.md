...
> 原文: [https://www.geeksforgeeks.org/check-if-two-coordinates-can-be-made-equal-by-incrementing-decrementing-by-k1-and-k2-respectively/](https://www.geeksforgeeks.org/check-if-two-coordinates-can-be-made-equal-by-incrementing-decrementing-by-k1-and-k2-respectively/)
...
如果可以使 `(X1, Y1)` 和 `(X2, Y2)` 相等，则打印 `是`。否则，打印 `否`。
...
> 通过以上操作，可以使两个坐标相等，然后打印 `是`。

> **输入:** `X1 = 10`, `Y1 = 10`, `X2 = 18`, `Y2 = 13`, `K1 = 10`, `K2 = 10`
> **输出:** `否`

**进场:** 这个问题可以用[贪心进场](https://www.geeksforgeeks.org/greedy-algorithms/)来解决，基于观察：`(X1, Y1)` 点在 x 方向可以走位 `n1` 次，而 `(X2, Y2)` 点在 x 方向可以走位 `n2` 次，那么表达式可以写成：
