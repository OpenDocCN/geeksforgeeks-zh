...
> 原文: [https://www.geeksforgeeks.org/count-lexicographically-increasing-k-length-strings-possible-from-first-n-alphabets/](https://www.geeksforgeeks.org/count-lexicographically-increasing-k-length-strings-possible-from-first-n-alphabets/)
...
解决问题最简单的方法是使用[递归](https://www.geeksforgeeks.org/recursion/)和[回溯](https://www.geeksforgeeks.org/backtracking-algorithms/)来生成所有可能的字符排列，对于每个字符串，检查字符是否遵循字典顺序递增。打印所有此类字符串的计数。
...
*   初始化大小为 `(N+1)` 的[数组](https://www.geeksforgeeks.org/array-data-structure/) `columnSum[]`，其中 `columnSum[i]` 是数组 `dp[][]` 列 `j` 中所有值的总和。
*   初始化大小为 `(K + 1)*(N + 1)` 的 `dp[][]` 表。
*   将 `dp[0][i]` 初始化为 `1`，随后更新数组 `columnSum[]`。
*   分别使用变量 `i` 和 `j` 在 `K` 上迭代两个嵌套循环:
    *   将 `dp[i][j]` 存储为 `columnSum[j-1]`。
    *   将 `columnSum[j]` 更新为 `columnSum[j] + dp[i][j]`。
*   完成上述步骤后，打印 `dp[K][N]` 的值作为最终的合成数。
