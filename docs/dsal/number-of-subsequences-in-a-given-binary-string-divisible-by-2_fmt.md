...
> 原文: [https://www.geeksforgeeks.org/number-of-subsequences-in-a-given-binary-string-divisible-by-2/](https://www.geeksforgeeks.org/number-of-subsequences-in-a-given-binary-string-divisible-by-2/)
...
**天真的方法:** 天真的方法是生成所有可能的子序列，并检查它们是否能被 2 整除。时间复杂度为 `O(2^N * N)`。

**有效方法:** 可以观察到，任何二进制数只有在以一个 `0` 结尾时才能被 `2` 整除。现在的任务是计算以 `0` 结束的子序列的数量。因此，对于每个索引 `i`，使得 `str[i] = '0'`，找到以 `i` 结束的子序列的数量。该值等于 `2^i` (基于 0 的索引)。因此，最终答案将等于所有 `i` 的 `2^i` 的总和，使得 `str[i] = '0'`。
