...
上述函数的时间复杂度可以写成 `θ(log 1) + θ(log 2) + θ(log 3) + ... + θ(log n)` 也就是 `θ(log n!)`。
`log n` 的增长顺序对于大的 `n` 值，即 `θ(log n!) = θ(n log n)`。所以 `fun()` 的时间复杂度是 `θ(n log n)`。
表达式 `θ(log n!) = θ(n log n)` 可以很容易地根据[斯特林近似（或斯特林公式）](http://en.wikipedia.org/wiki/Stirling%27s_approximation)推导出来。
