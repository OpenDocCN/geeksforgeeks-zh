...
> 原文: [https://www.geeksforgeeks.org/number-of-ways-to-write-n-as-a-sum-of-k-non-negative-integers/](https://www.geeksforgeeks.org/number-of-ways-to-write-n-as-a-sum-of-k-non-negative-integers/)
...
3.  假设我们到达第 `i` 行第 `j` 列的 `dp[i][j]` 处，即我们可以选择 `i` 个元素，我们需要得到总和 `j`。要计算直到 `dp[i][j]` 的路数，请首先选择 `(i–1)` 个元素，然后选择 `(j–x)` 个元素，其中 `x` 是第一个 `(i–1)` 个元素的总和。
4.  重复以上步骤填充 `dp[][]` 数组。
5.  值 `dp[n][m]` 将给出所需的结果。
...
计算总和然后存储计数的想法增加了时间复杂度。我们可以通过将总和存储在上面的 `dp[][]` 表中来减少它。以下是上述办法的实施情况:
