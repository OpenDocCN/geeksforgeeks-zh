...
> 原文: [https://www.geeksforgeeks.org/printing-brackets-matrix-chain-multiplication-problem/](https://www.geeksforgeeks.org/printing-brackets-matrix-chain-multiplication-problem/)
...
其思想是将每个子表达式 `(i，j)` 的最佳断点存储在 2D 数组 `bracket[n][n]` 中。一旦我们构造了 `bracket` 数组，我们就可以使用下面的代码打印括号。
