...
> 原文: [https://www.geeksforgeeks.org/smallest-number-greater-than-or-equal-to-n-using-only-digits-1-to-k/](https://www.geeksforgeeks.org/smallest-number-greater-than-or-equal-to-n-using-only-digits-1-to-k/)
...
*   为了获得有效的解决方案，我们需要了解一个事实，即最多可以形成 9 位数，直到 10<sup>10</sup>。因此，我们将反向迭代数字并检查:
    1.  如果 `当前数字 >= K`，则使 `数字 = 1`。
    2.  如果 `当前数字 < K` 之后没有比 `K` 更大的数字，则 `将数字增加 1`，并原样复制所有剩余的数字。
