...
> 原文: [https://www.geeksforgeeks.org/visualizing-the-complement-of-a-graph-using-networkx/](https://www.geeksforgeeks.org/visualizing-the-complement-of-a-graph-using-networkx/)
...
*   然后我们将调用 `draw()` 函数传递 `G_C` 作为一个参数，它将显示补码图。
...
通过使用 `complement()` 方法，`G` 中的所有边都被移除，所有其他可能性都被添加到 `G` 的补码中并被打印。