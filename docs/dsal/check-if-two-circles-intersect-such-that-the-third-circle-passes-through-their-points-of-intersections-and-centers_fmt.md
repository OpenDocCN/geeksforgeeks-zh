...
> 原文: [https://www.geeksforgeeks.org/check-if-two-circles-intersect-such-that-the-third-circle-passes-through-their-points-of-intersections-and-centers/](https://www.geeksforgeeks.org/check-if-two-circles-intersect-such-that-the-third-circle-passes-through-their-points-of-intersections-and-centers/)

给定三个圆的中心和半径 `A`、`B` 和 `C`，形式为 `{X, Y, R}`，其中 `(X, Y)` 是圆的中心，`R` 是该圆的半径。任务是检查任意两个圆是否相交，使得第三个圆穿过两个圆的交点和中心。如果发现为真，则打印“是”。否则，打印“否”。
...
*   假设 `C1C2`、`C1C3`、`C2C3` 为圆心距离 `C1` 与 `C2`、`C1` 与 `C3`、分别为 `C2` 与 `C3`。
