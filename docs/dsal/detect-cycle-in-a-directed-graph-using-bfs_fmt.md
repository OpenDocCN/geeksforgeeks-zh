...
想法是简单地将 [Kahn 的算法用于拓扑排序](https://www.geeksforgeeks.org/topological-sorting-indegree-based-solution/)应用于有向图中检测循环。

使用 BFS 在有向图中检测循环的步骤如下：

**步骤 1**：计算图中存在的每个顶点的入度（传入边数），并将访问的节点数初始化为 0。

**步骤 2**：选取入度为 0 的所有顶点并将它们添加到队列中（入队操作）。

**步骤 3**：从队列中移除一个顶点（出队操作），然后：
1.  访问的节点数增加 1。
2.  将其所有相邻节点的入度减少 1。
3.  如果将相邻节点的入度降低到零，则将其添加到队列中。

**步骤 4**：重复步骤 3，直到队列为空。

**步骤 5**：如果访问的节点数**不等于**图中的节点数，则图中有循环，否则返回 `false`。

**如何找到每个节点的入度？**

有两种计算每个顶点的入度的方法：

1.  取一个入度数组，该数组将跟踪每个节点的入度。遍历所有边，只需将目标节点的计数器加 1。
...
2.  遍历每个节点的邻接表，然后将与其连接的所有节点的入度增加 1。
