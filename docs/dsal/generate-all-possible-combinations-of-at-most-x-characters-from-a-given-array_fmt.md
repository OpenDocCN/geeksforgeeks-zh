...
> 原文：`https://www.geeksforgeeks.org/generate-all-possible-combinations-of-at-most-x-characters-from-a-given-array/`

给定一个由 `N` 个字符组成的数组 `arr[]`，任务是生成最多 `X` 个元素（1 ≤ X ≤ N）的所有可能组合。

**示例：**

> **输入：** `N = 3`，`X = 2`，`arr[] = {'a', 'b', 'c'}`
> **输出：** `a b c bc ca ab cb ac ba`
> **解释：** 使用 1 个字符的所有可能组合为 3 个：`{'a', 'b', 'c'}`。使用 2 个字符的所有可能组合为 `{"bc", "ca", "ab", "cb", "ac", "ba"}`。
>
