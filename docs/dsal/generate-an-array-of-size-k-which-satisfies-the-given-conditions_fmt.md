...
> 原文: [https://www.geeksforgeeks.org/generate-an-array-of-size-k-which-satisfies-the-given-conditions/](https://www.geeksforgeeks.org/generate-an-array-of-size-k-which-satisfies-the-given-conditions/)

给定两个整数 `N` 和 `K`，任务是生成一个长度为 `K` 的数组 `arr[]`：

1.  `arr[0]+arr[1]+…+arr[K–1]= N`。
2.  `arr[i] > 0` 对于 `0 ≤ i < K`。
3.  `arr[i] < arr[i + 1] ≤ 2 * arr[i]` 对于 `0 ≤ i < K – 1`。
...
**进场：** 让 `r = n–k *(k+1)/2`。如果 `r < 0` 那么答案是 `-1` 已经。否则，我们构造数组 `arr[]`，其中所有 `arr[i]` 都是 `floor(r / k)` 除了最右边的 `r % k` 值外，都是 `ceiling(r / k)`。
