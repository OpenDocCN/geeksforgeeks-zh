...
> 原文：[https://www.geeksforgeeks.org/count-number-of-subsets-having-a-particular-xor-value/](https://www.geeksforgeeks.org/count-number-of-subsets-having-a-particular-xor-value/)
...
**动态规划方法 O(n*m)：**
我们定义一个数 `m`，使得 `m = pow(2, (log2(max(arr)) + 1)) - 1`。这个数字实际上是任何异或子集将获得的最大值。我们通过计算最大数的位数得到这个数。我们创建一个 2D 数组 `dp[n+1][m+1]`，使得 `dp[i][j]` 等于来自 `arr[0…i-1]` 的子集的具有异或值 `j` 的子集的数量。

我们按如下方式填充 `dp` 阵列：

1.  我们将 `dp[i][j]` 的所有值初始化为 0。
2.  设定 `dp[0][0]` 的值 = 1，因为空集合的异或为 0。
3.  从左到右迭代 `arr[i]` 的所有值，对于每个 `arr[i]`，迭代 XOR 的所有可能值，即从 0 到 `m`（包括 0 和 `m`）并填充 `dp` 数组如下：
    对于 `i = 1` 到 `n`：
    对于 `j = 0` 到 `m`：
    `dp[i][j] = dp[i-1][j] + dp[i-1][j ^ arr[i-1]]`
    这可以解释为，如果存在具有 XOR 值 `j` 的子集 `arr[0…i-2]`，则还存在子集 `arr[0…i-2]` 具有 XOR 值 `j ^ arr[i-1]`。那么显然存在具有 XOR 值 `j` 的子集 `arr[0…i-1]`，如 `j ^ arr[i-1] ^ arr[i-1] = j`。
4.  计算具有异或值 `k` 的子集的数量：因为 `dp[i][j]` 是从 `arr[0…i-1]` 的子集中具有 `j` 作为异或值的子集的数量，则集合 `arr[0…n-1]` 中的子集数将异或值作为 `K` 将是 `dp[n][K]`。
