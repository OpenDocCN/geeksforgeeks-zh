...
> 原文: [https://www.geeksforgeeks.org/analysis-algorithms-set-5-practice-problems/](https://www.geeksforgeeks.org/analysis-algorithms-set-5-practice-problems/)
...
### 解:
我们可以根据关系 `s_i = s_{i-1} + i` 来定义术语‘s’，每次迭代‘i’的值增加一。在第 `i` 次迭代中包含在“s”中的值是第一个“i”正整数的和。如果 `k` 是程序的总迭代次数，那么 `while` 循环在以下情况下终止: `1 + 2 + 3 …+ k = k(k+1)/2 > n`。所以 `k = O(√n)`。
上述函数 `O(√n)` 的时间复杂度。
