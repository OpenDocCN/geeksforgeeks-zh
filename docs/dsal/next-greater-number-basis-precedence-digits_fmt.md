...
> 原文：[https://www.geeksforgeeks.org/next-greater-number-basis-precedence-digits/](https://www.geeksforgeeks.org/next-greater-number-basis-precedence-digits/)

给定一个包含数字的字符串 `num`。问题是根据给定的数字优先级，在数字中使用相同的数字组找到下一个更大的数字。例如数字的优先顺序为 `1、6、4、5、2、9、8、0、7、3`，简单来说就是 `1<6<4<5<2<9<8<0<7<3`。如果无法形成下一个更大的数字，则打印原始数字。
...
1.  创建一个大小为 `10` 的 `priority[]` 数组。借助优先级数组 `pre[]` 为 `priority[]` 中的每个数字分配一个优先级编号，其中 `1` 被视为最小优先级，`10` 被视为最高优先级。
2.  使用带有手动定义的比较函数的 [STL C++ `next_permutation`](https://www.geeksforgeeks.org/stdnext_permutation-prev_permutation-c/) 找到下一个更大的置换。
