...
> 原文：[https://www.geeksforgeeks.org/check-if-a-word-is-present-in-a-sentence/](https://www.geeksforgeeks.org/check-if-a-word-is-present-in-a-sentence/)

给定一个句子作为字符串 `str` 和一个单词 `word`，任务是检查该单词是否存在于 `str` 中。一个句子是由多个单词组成的字符串，每个单词用空格隔开。
