...
> 原文：[https://www.geeksforgeeks.org/check-number-permutable-prime/](https://www.geeksforgeeks.org/check-number-permutable-prime/)

给定一个数 `N`，任务是检查它是否是一个可置换的素数。

一个`可置换素数`是通过任意置换变换数字位置后也是素数的数。一些可置换的质数是 2、3、5、7、11 等。

**先决条件：** `素性测试` | `CPP next_permute()`
...
1.  构造厄拉多塞的`筛`，高效寻找素数。
2.  要么通过切换数字来生成数字的每个排列，要么使用内置的 C++ 函数`next_permutation()`来检查它是否是素数。
