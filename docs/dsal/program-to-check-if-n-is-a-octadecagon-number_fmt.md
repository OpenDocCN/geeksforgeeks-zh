...
> 原文：[`https://www.geeksforgeeks.org/program-to-check-if-n-is-a-octadecagon-number/`](https://www.geeksforgeeks.org/program-to-check-if-n-is-a-octadecagon-number/)
...
3.  如果用上述公式计算的 `K` 的值是一个整数，那么 `N` 就是一个十八进制数。
4.  否则 `N` 不是十八进制数。
