...
> 原文：[https://www.geeksforgeeks.org/javascript-program-for-rearranging-a-linked-list-such-that-all-even-and-odd-positioned-nodes-are-together/](https://www.geeksforgeeks.org/javascript-program-for-rearranging-a-linked-list-such-that-all-even-and-odd-positioned-nodes-are-together/)
...
下面的程序为奇数和偶数位置的当前节点维护两个指针`odd`和`even`。我们还存储了偶链表的第一个节点`evenFirst`，这样我们就可以在两个不同的列表中将所有奇数和偶数节点连接在一起后，将偶列表附加在奇数列表的末尾。
