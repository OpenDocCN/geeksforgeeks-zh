...
**方法:**
创建新图形（ `G'` ）。对于初始图形 `G` 中的每个节点 `V`，创建两个新节点 `V_even` 和 `V_odd`。

> 这里，`V_odd` 表示为 `((V * 10)+1)`，`V_even` 表示为 `((V * 10) + 2)`。
> 例如，如果节点 `V = 4`，则 `V_odd = 41`，`V_even = 42`。

现在，对于 `G` 中的每条边（ `U, V` ），在 `G'` 中添加两条新边：`(U_even, V_odd)` 和 `(U_odd, V_even)`。最后使用 [Dijkstra 最短路径算法](https://www.geeksforgeeks.org/dijkstras-shortest-path-algorithm-greedy-algo-7/) 找到 `(源_偶)` 节点到 `(目的_偶)` 节点的最短路径。
对于上述输入 1 中给出的图形，`G'` 可以表示为:

![Graph G'](img/30374498d27916be9bbe92bf95bc8541.png)

从图 `G'` 可以观察到，从 `(1_偶)` 到 `(5_偶)` 只有偶长路径。因此，奇数长度的路径在 `G'` 中被分开，并且可以获得所需的最短路径。
