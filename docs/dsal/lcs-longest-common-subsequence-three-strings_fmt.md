...
> 原文：[https://www.geeksforgeeks.org/lcs-longest-common-subsequence-three-strings/](https://www.geeksforgeeks.org/lcs-longest-common-subsequence-three-strings/)

给定 3 个长度均小于 100 的字符串，任务是找出所有三个给定序列中最长的公共子序列。例如：
...
这个问题只是 [LCS](https://www.geeksforgeeks.org/dynamic-programming-set-4-longest-common-subsequence/) 的一个扩展。让输入序列为 `X[0..m-1]`，`Y[0..n-1]` 和 `Z[0..o-1]`，长度分别为 `m`、`n` 和 `o`。并让 `L(X[0..m-1], Y[0..n-1], Z[0..o-1])` 是三个序列 `X`、`Y` 和 `Z` 的 LCS 长度。下面是实现思路：
