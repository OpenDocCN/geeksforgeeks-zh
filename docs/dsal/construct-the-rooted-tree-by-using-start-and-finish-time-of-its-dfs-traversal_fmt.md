...
> 原文: [https://www.geeksforgeeks.org/construct-the-rooted-tree-by-using-start-and-finish-time-of-its-dfs-traversal/](https://www.geeksforgeeks.org/construct-the-rooted-tree-by-using-start-and-finish-time-of-its-dfs-traversal/)
...
*   将 `Identity[i]` 定义为起点等于 `i` 的顶点的索引
*   由于 `Start[v]` 和 `End[v]` 是顶点 `v` 的开始和结束时间，`v` 的第一个孩子是 `Identity[Start[v]+1]`，第 `(i+1)` 个是 `Identity[End[chv[i]]`，其中 `chv[i]` 是 `v` 的第一个孩子
