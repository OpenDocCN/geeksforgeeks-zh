...
> 原文: [https://www.geeksforgeeks.org/index-of-kth-set-bit-in-a-binary-array-with-update-queries/](https://www.geeksforgeeks.org/index-of-kth-set-bit-in-a-binary-array-with-update-queries/)

给定二进制数组 `arr[]` 和 `q` 以下类型的查询:

1.  `k`: 求数组中第 `k` 组位即第 `1` 的索引。
...
一个简单的解决方案是运行从 `0` 到 `n–1` 的循环，并找到第 `k` 个 `1`。要更新一个值，只需执行 `arr[i] = x`。第一次操作需要 `O(n)` 时间，第二次操作需要 `O(1)` 时间。

**另一种解决方案**是创建另一个数组，并将每个 1 的索引存储在该数组中的第 1 个索引处。第 `k` 个 `1` 的索引现在可以在 `O(1)` 时间计算，但是更新操作现在需要 `O(n)` 时间。如果查询操作的数量很大，并且更新很少，这种方法就能很好地工作。
