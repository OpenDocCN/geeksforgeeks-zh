...
> 原文: [https://www.geeksforgeeks.org/construct-an-array-such-that-cube-sum-of-all-element-is-a-perfect-square/](https://www.geeksforgeeks.org/construct-an-array-such-that-cube-sum-of-all-element-is-a-perfect-square/)
...
> **输入:** `N = 6`
> **输出:** `5 10 5 10 5`
> **解释:** 数组元素的三次和为 `(5)^3 + (10)^3 + (5)^3 + (10)^3 + (5)^3 + (5)^3`

**方法:** 思路是构造大小为 `N` 的数组，其中首先包含 `N` 个自然数。如果仔细观察，

> 第一 `N` 自然数的立方和: `(1)^3 + (2)^3 + (3)^3 + (4)^3 + ... + (N)^3` 永远是完美的正方形。
> 为 `1^{3} + 2^{3} + 3^{3} + ... + N^{3} = (\frac{N(N+1)}{2})^{2}`
