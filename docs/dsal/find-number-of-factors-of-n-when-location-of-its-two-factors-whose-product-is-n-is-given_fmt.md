...
> 原文: [https://www.geeksforgeeks.org/find-number-of-factors-of-n-when-location-of-its-two-factors-whose-product-is-n-is-given/](https://www.geeksforgeeks.org/find-number-of-factors-of-n-when-location-of-its-two-factors-whose-product-is-n-is-given/)

给定 `a` 和 `b`，当因子按升序排列时，它们表示乘积等于 `N` 的 `N` 的两个因子的位置。任务是找到 `N` 的因子总数。
