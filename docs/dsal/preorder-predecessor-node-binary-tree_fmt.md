...
> 原文: [https://www.geeksforgeeks.org/preorder-predecessor-node-binary-tree/](https://www.geeksforgeeks.org/preorder-predecessor-node-binary-tree/)
...
一个简单的解决方案是首先将给定树的前序遍历存储在一个数组中，然后线性搜索给定节点并打印它旁边的节点。
时间复杂度: `O(n)`
辅助空间: `O(n)`
...
1.  如果给定的节点是根，那么返回 `null` 作为前序前驱。
2.  如果节点是其父节点的左子节点，或者父节点的左子节点为空，则返回父节点作为其前序前驱节点。
3.  如果节点是其父节点的右子节点，并且父节点的左子节点存在，则前驱节点将是父节点的左子树的最右边的节点（最大值）。
4.  如果节点是其父节点的右子节点，并且父节点没有左子节点，则前驱节点将是父节点（最大值）。
