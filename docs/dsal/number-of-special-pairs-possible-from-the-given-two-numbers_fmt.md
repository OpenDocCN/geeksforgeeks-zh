...
> 原文：[https://www.geeksforgeeks.org/number-of-special-pairs-possible-from-the-given-two-numbers/](https://www.geeksforgeeks.org/number-of-special-pairs-possible-from-the-given-two-numbers/)

给定两个数字 `A`，`B`，任务是找出 `A`，`B` 的特殊对的数字。两个数字 `A`，`B` 的特殊对是满足两个给定条件的一对数字 `X`，`Y`：
`A = X | Y`，`B = X & Y`。
