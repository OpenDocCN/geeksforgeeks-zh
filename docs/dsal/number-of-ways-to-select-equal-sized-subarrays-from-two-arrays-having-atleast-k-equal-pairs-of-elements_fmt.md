...
> 原文: [https://www.geeksforgeeks.org/number-of-ways-to-select-equal-sized-subarrays-from-two-arrays-having-atleast-k-equal-pairs-of-elements/](https://www.geeksforgeeks.org/number-of-ways-to-select-equal-sized-subarrays-from-two-arrays-having-atleast-k-equal-pairs-of-elements/)
...
*   让我们考虑一个正方形子矩阵，左上角为`(I，j)`，右下角为`(i + size，j + size)`。这相当于考虑子阵列 `A[i: i + size]` 和 `B[j: j + size]`。可以观察到，如果这两个子矩阵将有 `x` 对相等的元素，那么子矩阵将有 `x 1`。
