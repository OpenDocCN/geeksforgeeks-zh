...
> 原文: [https://www.geeksforgeeks.org/check-if-the-count-of-inversions-of-two-given-types-on-an-array-are-equal-or-not/](https://www.geeksforgeeks.org/check-if-the-count-of-inversions-of-two-given-types-on-an-array-are-equal-or-not/)
...
*   因此**1 型**的反演基本上是**2 型**的反演，加上所有对指标 `(i, j)` 和小于 `(j-1)` 的 `i` 和 `a[i] > a[j]`。
*   所以，对于任何指标 `j`，任务是检查是否有指标 `i`，比 `j-1` 小和 `a[i] > a[j]`。如果找到任何一对这样的索引 `(i, j)`，则打印 `"No"`。否则，打印 `"是"`。
