...
> 原文：[https://www.geeksforgeeks.org/queries-to-count-sum-of-rows-and-columns-of-a-matrix-present-in-given-ranges/](https://www.geeksforgeeks.org/queries-to-count-sum-of-rows-and-columns-of-a-matrix-present-in-given-ranges/)

给定大小为 `N * M` 的矩阵 `A[][]` 和由 `{L, R}` 形式的 `Q` 查询组成的 2D 数组 `queries[][]`，任务是统计行和列和的数量，它们是从 `[L, R]` 范围内的整数。

**示例：**

> **输入：** `N = 2`，`M = 2`，`A[][] = {{1, 4}, {2, 5}}`，`Q = 2`，`queries[][] = {{3, 7}, {3, 9}}`
> **输出：** `3 4`
> **解释：**
> 第一行之和 = `1 + 4 = 5`。
> 第二行之和 = `2 + 5 = 7`。
...
> 因此，计数为 `4`。
>
> **输入：** `N = 3`，`M = 2`，`A[][] = {{13, 3}, {9, 4}, {6, 10}}`，`Q = 2`，`queries[][] = {{10, 20}, {25, 35}}`
> **输出：** `4 1`

**高效方法：** 思路是逐行逐列遍历矩阵，分别预先计算每行每列的和。遍历数组 `queries[][]` 并使用二分搜索法计算给定范围内存在的行和或列和的数量。按照以下步骤解决问题：

1.  初始化两个辅助数组，比如说 `row_sum[]` 和 `col_sum[]`，来存储行和列的总和。
2.  初始化另一个数组，比如 `sum_list[]`，来存储所有行和列和元素的组合。
3.  按升序排列数组 `sum_list[]`。
4.  遍历数组 `queries[]`，对于每个查询：
    *   执行二分搜索法找到索引，说出最左边和的 `i`，即 `L`。
    *   再次执行二分搜索法找到最右边和的指标 `j`，即 `R`。
    *   打印 `j - i + 1` 作为查询结果。
...
class GFG
{
    ...
    static void totalCount(int[,] A, int N, int M, int[,] queries, int Q)
    {
        // Stores the sum of each row
        int[] row_sum = new int[N];
        // Stores the sum of each col
