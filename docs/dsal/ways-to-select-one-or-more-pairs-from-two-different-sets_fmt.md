...
> 原文: [https://www.geeksforgeeks.org/ways-to-select-one-or-more-pairs-from-two-different-sets/](https://www.geeksforgeeks.org/ways-to-select-one-or-more-pairs-from-two-different-sets/)

给定两个正数`n`和`m` (`n <= m`)，分别表示第一类集合和第二类集合的总项目数。求通过从第一类(I)中选取一个项目、从第二类(II)中选取另一个项目来组成至少一对的总方法数。在任何排列中，一个项目不应在任意两对之间重复出现。
**注:** 由于答案可能很大，以模`1000000007`输出。
...
方法很简单，我们只需要从`n`型中选择`i`项和从`m`型中选择`i`项并将它们相乘([积法则](https://en.wikipedia.org/wiki/Rule_of_product))的组合，其中`i`从`1`变化到`n`。但是我们也可以用“I”的方式排列结果，因此我们需要乘以`i!`。然后取所有结果乘积的和([求和规则](https://en.wikipedia.org/wiki/Rule_of_sum))得到最终答案。
