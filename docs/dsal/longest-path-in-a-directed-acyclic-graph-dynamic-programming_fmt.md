...
给定有向图`G`，其中`N`个顶点和`M`个边。任务是在 Graph 中找到最长的有向路径的长度。
...
**简单方法**：朴素的方法是使用 [DFS](http://www.geeksforgeeks.org/depth-first-traversal-for-a-graph/) 计算来自每个节点的最长路径的长度。该方法的时间复杂度为`O(N^2)`。
