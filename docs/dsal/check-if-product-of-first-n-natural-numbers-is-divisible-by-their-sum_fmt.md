...
> 原文: [https://www.geeksforgeeks.org/check-if-product-of-first-n-natural-numbers-is-divisible-by-their-sum/](https://www.geeksforgeeks.org/check-if-product-of-first-n-natural-numbers-is-divisible-by-their-sum/)
...
求第一个 `N` 个自然数的和与积，检查积是否能被和整除。

## 高效进场

我们知道第一个 `N` 个自然的和与积是 `和 = (N * (N + 1)) / 2`，`积 = N!` 分别为。现在为了检查乘积是否能被和整除，我们需要检查下面等式的余数是否为 0。
