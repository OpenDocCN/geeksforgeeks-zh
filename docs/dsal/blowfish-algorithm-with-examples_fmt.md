...
> 原文：[https://www.geeksforgeeks.org/blowfish-algorithm-with-examples/](https://www.geeksforgeeks.org/blowfish-algorithm-with-examples/)

**Blowfish**是**Bruce Schneier**在 1993 年设计的一种加密技术，作为 [DES 加密技术](https://www.geeksforgeeks.org/data-encryption-standard-des-set-1/)的替代品。它比 DES 快得多，并且提供了良好的加密速率，迄今为止没有发现有效的[密码分析技术](https://www.geeksforgeeks.org/introduction-to-crypto-terminologies/)。它是第一个不受任何专利限制的安全密码，因此任何人都可以免费使用。

1.  **区块大小**：64 位
2.  **密钥大小**：32 位至 448 位可变大小
3.  **子密钥数量**：18【P 数组】
4.  **轮数**：16
5.  **替换盒**的数量：4【每个替换盒有 512 个条目，每个条目 32 位】
...
*   这 18 个子密钥存储在一个 `P` 数组中，每个数组元素是一个 32 位的条目。
*   它是用圆周率（π）的十六进制表示初始化的。
*   每个子密钥的十六进制表示由下式给出：
...
*   在加密和解密过程中都需要 4 个替换盒（S 盒）`{S[0]...S[3]}`，每个 S 盒有 256 个条目 `{S[i][0]...S[i][255], 0 ≤ i ≤ 3}`，其中每个条目都是 32 位的。
*   它是用圆周率（π）的十六进制表示初始化 `P` 数组后，再用同样的方法初始化的。[你可以在这里找到 **S 盒**！](https://github.com/Ray784/Blowfish-S-boxes)

## 第三步：加密

*   加密函数由两部分组成：
    **a.** 加密由 16 轮组成，每一轮（`Ri`）输入前一轮的明文（`P.T.`）和相应的子密钥（`Pi`）。每一轮的描述如下：
...
（注：由于原文内容在此处截断，后续代码和内容未提供，因此排版也相应结束。）
...
*   解密功能也由两部分组成:
    1.  **轮次:** 解密也由 16 轮次组成，每轮(`Ri`)(如上所述)从上一轮输入密文(`C.T .`)和相应的子密钥(`P[17-i]`)(即，对于解密，子密钥反向使用)。
    2.  **后处理:**16 回合后的输出处理如下:
