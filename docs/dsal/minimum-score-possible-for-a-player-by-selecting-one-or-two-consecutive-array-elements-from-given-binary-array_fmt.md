...
> 原文: [https://www.geeksforgeeks.org/minimum-score-possible-for-a-player-by-selecting-one-or-two-consecutive-array-elements-from-given-binary-array/](https://www.geeksforgeeks.org/minimum-score-possible-for-a-player-by-selecting-one-or-two-consecutive-array-elements-from-given-binary-array/)

给定一个二进制数组 `arr` 大小 `N` 和两个玩家 `A` 和 `B`。任务是通过根据给定的约束选择玩家的分数来最小化玩家的分数 `A`：
...
最简单的方法是尝试所有可能的组合，从给定数组中移除元素。每次都有两个可能的选项，即可以删除一个或两个连续的元素。每个位置，从 `1` 到 `N–1`，都有 `2` 的选择。因此，可以进行 `2^N` 的可能组合。每个组合的处罚可以找到，其中，打印最低处罚。
...
优化上述途径，思路是使用动态规划。可以通过以下 `dp` 转换来解决，其中 `DP[i][0]` 存储从 `i` 到 `N–1` 的最小惩罚。如果玩家 `A` 从索引 `i` 开始选择。同样，可以为玩家 `B` 定义 `dp[i][1]`。

> 轮到玩家 A 时:
> `dp[i][0] = min(dp(i+1, 1) + arr[i], dp(i+2, 1) + arr[i] + arr[i+1])`
> 其中，
> `i` 表示当前位置。
> `1` 表示下一个状态是 B 的开启。
...
*   带记忆的递归可以使用。对于基础条件，检查当前位置是否超过或变成 `N`，返回 `0`。
*   根据玩家的回合应用上面定义的转场，并返回最小答案。
*   用玩家 `A` 的回合和惩罚初始化递归函数为 `0`。
*   对于每个递归调用，将计算出的最小惩罚存储在图 `M` 中，以避免为重叠子问题计算。
*   在上述递归调用结束后，打印玩家 `A` 的最低分数。
