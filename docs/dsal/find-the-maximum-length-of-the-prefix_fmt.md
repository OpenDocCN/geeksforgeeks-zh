...
> 原文：[`https://www.geeksforgeeks.org/find-the-maximum-length-of-the-prefix/`](https://www.geeksforgeeks.org/find-the-maximum-length-of-the-prefix/)

给定一个由 `N` 个整数组成的数组 `arr[]`，其中数组的所有元素都来自范围 **[0, 9]** 即一个数字，任务是找到这个数组前缀的最大长度，这样从前缀中删除一个元素将使前缀中剩余元素的出现相同。
