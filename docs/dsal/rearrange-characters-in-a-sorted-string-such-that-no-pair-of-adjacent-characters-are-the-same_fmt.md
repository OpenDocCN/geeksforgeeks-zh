...
> 原文:[https://www.geeksforgeeks.org/rearrange-characters-in-a-sorted-string-such-that-no-pair-of-adjacent-characters-are-the-same/](https://www.geeksforgeeks.org/rearrange-characters-in-a-sorted-string-such-that-no-pair-of-adjacent-characters-are-the-same/)
...
*   当 `k` 小于 `N` 时迭代，并执行以下步骤:
    ...
    *   否则，[交换](https://www.geeksforgeeks.org/swapping-characters-string-java/) `S[j]` 和 `S[k]` 并将 `i` 和 `j` 增加 `1`，如果 `j` 等于 `k`，则将 `k` 增加 `1`。
*   完成以上步骤[后，反转字符串](https://www.geeksforgeeks.org/reverse-a-string-in-java/) `S`。
