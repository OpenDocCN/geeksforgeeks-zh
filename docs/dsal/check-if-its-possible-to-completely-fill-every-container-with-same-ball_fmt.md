...
> 原文: [https://www.geeksforgeeks.org/check-if-its-possible-to-completely-fill-every-container-with-same-ball/](https://www.geeksforgeeks.org/check-if-its-possible-to-completely-fill-every-container-with-same-ball/)

给定两个[数组](https://www.geeksforgeeks.org/arrays-in-java/)，`arr[]C`（容器）和`arr[]B`（球），任务是找出是否有可能用给定的球完全填满每个容器，如果每个容器只能储存相同类型的球的话。在数组`C`中，`C[i]`存储第`i`个容器可以存储的最大球数。在数组`B`中，`B[i]`存储第`i`个球的类型。

## 示例

**输入:** `C = [1, 2, 3]`, `B = [1, 2, 2, 3, 3, 4, 4]`
**输出:** YES
**解释:** 用1号球填充第一个容器，用2号球填充第二个容器，用2号球填充第三个容器
...
想法是使用[回溯](https://www.geeksforgeeks.org/backtracking-algorithms/)来检查是否有可能填满每个容器。可以观察到，每种球的`频率`只需要一个，所以我们把每种球的`频率`存储在一个`图`中。让我们看看实施我们的方法所涉及的步骤:

*   将同类型球的频率存储在`图`中。
*   调用函数`getans()`检查是否可以填充容器。
*   试着用`频率`大于等于容器容量的球填充容器。如果可能的话返回`真`，否则`回溯`并检查其他球。
*   如果不存在组合，返回`假`。
