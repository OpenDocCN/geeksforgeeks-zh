...
一个简单的解决方案是将所有的数字从 1 迭代到 `sqrt(n)`，检查该数字是否除以 `n`，并增加除数。这种方法需要 `O(sqrt(n))` 时间。
...
对于 `n = p1^E1 * p2^E2 * p3^E3…` 其中 `p1`、`p2`、`p3`..是质因数，除数由 `(e1+1) * (e2+1) * (e3+1)` 给出…
...
1.  1 个质因数，如果它是指数为 1 的质数(`y`)
2.  1 个素数因子，如果它是素数的平方(`sqrt(y)`)，指数为 2
3.  指数为 1 和 1 的复合(`p1`，`p2`)的 2 个质因数
