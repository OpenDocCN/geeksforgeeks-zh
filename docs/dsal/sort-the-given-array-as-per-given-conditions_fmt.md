...
> 原文: [https://www.geeksforgeeks.org/sort-the-given-array-as-per-given-conditions/](https://www.geeksforgeeks.org/sort-the-given-array-as-per-given-conditions/)
...
*   创建三个[向量](https://www.geeksforgeeks.org/vector-in-cpp-stl/) `v1`、`v2`、`v3`，其中 `v1` 存储偶数且可被 `5` 整除的数字，`v2` 存储偶数但不可被 `5` 整除的数字，`v3` 存储奇数。
*   [使用变量 `i` 在范围](https://www.geeksforgeeks.org/range-based-loop-c/)`[0, N-1]`中迭代，并执行以下步骤:
    *   如果 `arr[i]%2 == 0` 和 `arr[i]%5 == 0`，则在 `v1` 中插入 `arr[i]`。
    *   如果 `arr[i]%2 == 0` 和 `arr[i]%5 != 0`，则在 `v2` 中插入 `arr[i]`。
    *   如果 `arr[i]%2`，则在 `v3` 中插入 `arr[i]`。
*   [按降序排列向量](https://www.geeksforgeeks.org/sorting-a-vector-in-c/) `v1`。
*   执行上述步骤后，打印[向量](https://www.geeksforgeeks.org/vector-in-cpp-stl/) `v1`，然后打印向量 `v2`，然后打印 `v3` 作为答案。
