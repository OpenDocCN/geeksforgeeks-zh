...
> 原文：[https://www.geeksforgeeks.org/generate-an-array-of-maximum-sum-such-that-each-element-exceeds-all-elements-present-either-on-its-left-or-right/](https://www.geeksforgeeks.org/generate-an-array-of-maximum-sum-such-that-each-element-exceeds-all-elements-present-either-on-its-left-or-right/)

给定一个由 `N` 个正整数组成的数组 `A[]`，任务是构造一个大小为 `N` 的数组 `B[]`，数组元素的最大可能和满足每个索引 `i` 的以下标准：
