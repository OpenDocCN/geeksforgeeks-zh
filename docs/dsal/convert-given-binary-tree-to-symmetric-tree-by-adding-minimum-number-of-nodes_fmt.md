...
> 原文: [https://www.geeksforgeeks.org/convert-given-binary-tree-to-symmetric-tree-by-adding-minimum-number-of-nodes/](https://www.geeksforgeeks.org/convert-given-binary-tree-to-symmetric-tree-by-adding-minimum-number-of-nodes/)
...
1.  创建一个函数 `buildSymmetric`，该函数将接受两个参数 `root1` 和 `root2`。
2.  这里，`root1` 和 `root2` 是彼此对称的节点。
3.  所以最初，`root1` 和 `root2` 都包含根的值，并且在每个递归调用中:
    *   如果 `root1` 存在但 `root2` 不存在，则创建一个与 `root1` 值相同的新节点，并将其放置在 `root2` 的位置。
    *   如果 `root2` 存在而 `root1` 不存在，也要按照上述步骤进行。
    *   如果 `root1` 和 `root2` 的值不相同，那么将两个节点的值改为它们的总和。
    *   现在，对 `(root1->left, root2->right)` 和 `(root1->right, root2->left)` 的对称位置进行下两次递归调用。
4.  在所有递归调用完成后，树将变得对称。
...
// into a symmetric
Node* buildSymmetricTree(Node* root1,
                         Node* root2)
{
    // Base Case
...
// Function to convert the given tree
// into a symmetric
static Node buildSymmetricTree(Node root1,
                               Node root2)
{
    // Base Case
    if (root1 == null && root2 == null) {
...
# Function to convert the given tree
# into a symmetric
def buildSymmetricTree(root1, root2):
    # Base Case
    ...
    # Recurring to the left
    root1.left = buildSymmetricTree( root1.left, root2.right)

    # Recurring to the right
    root1.right = buildSymmetricTree(root1.right, root2.left)
