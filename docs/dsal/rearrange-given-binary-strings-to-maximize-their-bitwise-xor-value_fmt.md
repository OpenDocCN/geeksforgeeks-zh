...
> 原文：[https://www.geeksforgeeks.org/rearrange-given-binary-strings-to-maximize-their-bitwise-xor-value/](https://www.geeksforgeeks.org/rearrange-given-binary-strings-to-maximize-their-bitwise-xor-value/)
...
最简单的方法是生成所有可能的方式来重新排列 `S1`、`S2` 和 `S3`。假设字符串中分别有 `O1`、`O2` 和 `O3` 设定位 `S1`、`S2` 和 `S3`。要检查以获得最大[位异或](https://www.geeksforgeeks.org/calculate-xor-1-n/)值的重新排列总数如下：

> 重新排列 `S1` = `NC_{O1}`
> 重新排列 `S2` = `NC_{O2}`
> 重新排列 `S3` = `NC_{O3}`
>
> 因此，要检查的所有可能的重新排列 = `NC_{O1} * NC_{O2} * NC_{O3}`
...
想法是找到一个合适的重排 `S1`、`S2` 和 `S3`，这样他们的**位异或**值最大化，使用[动态规划](https://www.geeksforgeeks.org/dynamic-programming/)。子问题可以存储在 `dp[][][][]` 表中，其中 `dp[i][o1][o2][o3]` 存储从索引 `i` 开始直到位置 `N-1` 的**最大异或**值，其中 `o1`、`o2` 和 `o3` 为**1**的编号。
...
> `dp[i][o1][o2][o3] = max(dp(将 1 分配给所有三个字符串), dp(将 1 分配给两个字符串中的任何一个), dp(将 1 分配给任何一个字符串), dp(不将 1 分配给任何字符串))`
> 其中，
> `i` = 当前位置
> `o1` = 要放入琴弦 `S1` 的剩余位置
> `o2` = 要放入琴弦 `S2` 的剩余位置
