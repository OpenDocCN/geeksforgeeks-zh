...
> 原文：[https://www.geeksforgeeks.org/booths-multiplication-algorithm/](https://www.geeksforgeeks.org/booths-multiplication-algorithm/)
...
> 将被乘数放入 `BR` 中，将乘数放入 `QR` 中，算法按照以下条件工作：
>
> **1.** 如果 `Qn` 和 `Qn+1` 相同，即 `00` 或 `11`，则执行 1 位算术移位。
> **2.** 如果 `QnQn+1 = 10`，则执行 `A = A + BR` 并执行 1 位算术移位。
> **3.** 如果 `QnQn+1 = 01`，则执行 `A = A - BR` 并执行 1 位算术移位。
