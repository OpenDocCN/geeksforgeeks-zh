...
> 原文：[https://www.geeksforgeeks.org/generate-a-sequence-determined-by-the-characters-of-a-given-string/](https://www.geeksforgeeks.org/generate-a-sequence-determined-by-the-characters-of-a-given-string/)

给定一个长度为`N`（`1≤N≤10^5`）的[二进制字符串](https://www.geeksforgeeks.org/tag/binary-string/) `S`，任务是打印一个由满足以下条件的整数 `0` 到 `N` 组成的排列 `A`：
...
*   [遍历字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-c/)并将值附加到结果数组中，例如 `res`，相应地使用两个指针。
