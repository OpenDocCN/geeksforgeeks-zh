...
> 原文：[https://www.geeksforgeeks.org/array-range-queries-to-find-the-number-of-perfect-square-elements-with-updates/](https://www.geeksforgeeks.org/array-range-queries-to-find-the-number-of-perfect-square-elements-with-updates/)

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是执行以下两个查询：

*   **查询(`start`, `end`)**：从 `start` 到 `end` 打印子数组中完美平方数的个数。
...
**说明：**
在**查询 1** 中，子数组 `[0…4]` 有 `2` 个完美的正方形数字 `16` 和 `9`，即 `[16, 15, 8, 9, 14]`。
在**查询 2** 中，索引 `3` 处的值更新为 `11`，数组 `arr` 现在为 `[16, 15, 8, 11, 14, 25]`。
在**查询 3** 中，子数组 `[0…4]` 具有 `1` 个完美平方数 `16`，即 `[16, 15, 8, 11, 14]`。
...
*   每当我们从头到尾收到一个查询时，我们可以在段树中查询从 `start` 到 `end` 范围内的节点的总和，这又表示从 `start` 到 `end` 范围内的完美平方数的数量。
...
void `query(int* st, int n, int qs, int qe)`
{
    int `perfectSquareInRange`
        = `queryUtil(st, 0, n - 1, qs, qe, 0)`;
...
class GFG{
    ...
    /* where
      ...
      qs & qe  -. Starting and ending indexes
                  of query range   */
    static int `queryUtil(int []st, int ss, int se, int qs, int qe, int index)`
    {
        // If segment of this node is a part
        // of given range, then return
        // the number of perfect square numbers
        ...
        // overlaps with the given range
        int `mid` = `getMid(ss, se)`;
        return `queryUtil(st, ss, mid, qs, qe, 2 * index + 1)` +
               `queryUtil(st, mid + 1, se, qs, qe, 2 * index + 2)`;
    }
...
递归函数，用于为数组 `arr[ss..se]` 构建线段树。`si` 是线段树 `st` 中当前节点的索引。
...
如果数组中只有一个元素，检查它是否为完全平方数，然后在线段树中存储 1，否则存储 0 并返回。
...
如果有多于一个元素，则递归处理左右子树，并将两个值的和存储在此节点中。
...
从给定数组构建线段树的函数。此函数为线段树分配内存，并调用 `constructSTUtil()` 来填充分配的内存。

```python
def constructST(arr: List[int], n: int) -> List[int]:
```

为线段树分配内存。线段树的高度为 `x = (ceil(log2(n)))`。线段树的最大大小为 `max_size = 2 * pow(2, x) - 1`。
...
```csharp
    static bool isPerfectSquare(int x)
    {
        ...
        double sr = Math.Sqrt(x);
        // If square root is an integer
        return ((sr - Math.Floor(sr)) == 0) ? true : false;
    }
```

## 获取中间索引

一个工具函数，用于从角落索引获取中间索引。
...
更新输入数组和线段树中值的函数。它使用 `updateValueUtil()` 来更新线段树中的值。
...
```csharp
    static void updateValue(int []arr, int[] st, int n, int i, int new_val)
    {
        ...
        if (isPerfectSquare(oldValue) && isPerfectSquare(new_val))
            return;
