...
一种天真的方法是迭代到 `N`，找到所有的因子和它们的数字和。存储其中最大的并打印出来。
**时间复杂度:** `O(N)`
一种高效的方法是循环直到 `sqrt(N)`，那么因子将是 `i` 和 `n/i`，检查其中最大的[数字和](https://www.geeksforgeeks.org/finding-sum-of-digits-of-a-number-until-sum-becomes-single-digit/)，如果数字和相似，则存储较大的因子。迭代完成后，打印它们。
