...
> 原文：[https://www.geeksforgeeks.org/count-arrays-consecutive-element-different-values/](https://www.geeksforgeeks.org/count-arrays-consecutive-element-different-values/)
...
让状态 `f(i)` 表示填充数组 `A` 的范围 `[1, i]` 的方式数，使得 `A[1] = 1`，`A[i] ≠ 1`。
因此，如果 `x ≠ 1`，那么问题的答案就是 `f(n) / (k - 1)`，因为 `f(n)` 是 `A[n]` 填充 2 到 `k` 的数字的方式数，对于所有这样的值 `A[n]` 答案都是相等的，所以单个值的答案就是 `f(n) / (k - 1)`。
否则，如果 `x = 1`，答案就是 `f(n - 1)`，因为 `A[n - 1] ≠ 1`，而我们唯一能填入 `A[n]` 的数字就是 `x = 1`。

现在，主要的问题是如何计算 `f(i)`。考虑 `A[i - 1]` 可以是的所有数字。我们知道它一定在 `[1, k]` 中。

*   如果 `A[i - 1] ≠ 1`，那么有 `(k - 2) * f(i - 1)` 种方式来填充数组的剩余部分，因为 `A[i]` 不能是 1 或者 `A[i - 1]`（所以我们乘以 `(k - 2)`），对于范围 `[1, i - 1]`，递归地有 `f(i - 1)` 种方式。
*   如果 `A[i - 1] = 1`，那么有 `(k - 1) * f(i - 2)` 种方式来填充数组的剩余部分，因为 `A[i - 1] = 1` 表示 `A[i - 2] ≠ 1`，这意味着有 `f(i - 2)` 种方式来填充范围 `[1, i - 2]`，并且 `A[i]` 唯一不能为 1 的值，所以我们有 `A[i]` 的 `(k - 1)` 种选择。
