...
让 `S(N)` 代表斐波那契数列的前 `N` 项之和。现在，为了简单地找到 `S(N)`，[计算第(`N` + 2) <sup>个</sup>斐波那契数](https://www.geeksforgeeks.org/program-for-nth-fibonacci-number/)，并从结果中减去 `1`。本系列的 `N` <sup>第</sup>项可由下式计算:
...
*   第 `N` <sup>个</sup>斐波那契数也可以使用[生成函数](https://www.geeksforgeeks.org/discrete-maths-generating-functions-introduction-prerequisites/)来计算。
*   第 `N` <sup>个</sup>斐波那契数的生成函数由下式给出:
