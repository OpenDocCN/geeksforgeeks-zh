...
> 原文: [https://www.geeksforgeeks.org/python-program-to-check-if-given-array-is-monotonic/](https://www.geeksforgeeks.org/python-program-to-check-if-given-array-is-monotonic/)

给定一个包含整数的数组。任务是检查数组是否为`单调`。如果数组是`单调递增`或`单调递减`，则数组是单调的。
如果对于所有 `i <= j`，`A[i] <= A[j]`，数组 `A` 是单调递增的。如果对于所有 `i <= j`，`A[i] >= A[j]`，数组 `A` 是单调递减的。
返回 `true` 如果给定数组 `A` 是单调的，则返回 `false` (不带引号)。
...
**注:** 带`单元素的数组`可以认为既是`单调递增`也是`单调递减`，因此返回 `true`。
