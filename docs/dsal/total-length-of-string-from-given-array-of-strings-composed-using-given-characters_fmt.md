...
> 原文: [https://www.geeksforgeeks.org/total-length-of-string-from-given-array-of-strings-composed-using-given-characters/](https://www.geeksforgeeks.org/total-length-of-string-from-given-array-of-strings-composed-using-given-characters/)

给定一个`字符列表`和一个`字符串数组`，找出字符串数组中所有可以使用给定字符组成的字符串的总长度。

**例:**

> **输入:** `strings = ["mouse", "me", "bat", "lion"]`, `chars = "oushame"`
> **输出:** 10
> **解释:**
> 使用字符`"oushame"`可以形成的字符串有`"mouse"`和`"me"`以及`"bat"`。
> 鼠的长度为 5，“我”的长度为 2，“蝙蝠”的长度为 3
> 所有长度之和= 5 + 2 + 3 = 10。
> **输入:** `strings = ["hi", "data", "geeksforgeeks"]`, `chars = "tiadha"`
> **输出:** 6
> **解释:**
...
*   我们可以在形成字符串时使用给定字符串中的字符，即`chars`。我们还可以重用使用过的字符来形成下一个字符串。
*   通过跟踪字符串中每个字符的频率，维护一个以字符为关键字和值的`无序映射`。
*   每次我们扫描字符串列表中的字符时，我们都会减少`无序映射`中的字符频率，但我们必须维护原始映射的副本，以便检查第二个字符串。
