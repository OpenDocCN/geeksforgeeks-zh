...
> 原文: [https://www.geeksforgeeks.org/find-the-coordinate-that-does-not-belong-to-any-square/](https://www.geeksforgeeks.org/find-the-coordinate-that-does-not-belong-to-any-square/)
...
**输出:** `1 1`
**解释:** 正方形有四条边: `x = 0`，`x = 2`，`y = 0`，`y = 2`，现在除了
...
*   [使用变量 `i` 迭代一个范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[0，N]`，并执行以下步骤:
    ...
    *   [使用变量 `j` 迭代一个范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[0，N]`，并执行以下步骤:
        *   如果 `i` 不等于 `j`，则执行以下步骤:
            *   将 `x1` 设置为 `x1` 或 `p[j].fi` 的最小值。
            *   将 `x2` 设置为 `x2` 或 `p[j].fi` 的最大值。
            *   将 `y1` 设置为 `y1` 或 `p[j].se` 的最小值。
            *   将 `y2` 设置为 `y2` 或 `p[j].se` 的最小值。
    *   初始化一个变量，将 `ok` 设为 `true`，将变量 `cnt1、cnt2、cnt3、cnt4` 设为 `0`，将最大和最小点数存储为 `x1、x2、y1、y2`。
    *   [使用变量 `j` 迭代一个范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[0，N]`，并执行以下步骤:
        *   如果 `i` 不等于 `j`，则执行以下步骤:
            *   若 `p[j].fi` 等于 `x1`，然后将 `cnt1` 的值增加 `1`。
            *   若 `p[j].fi` 等于 `x2`，然后将 `cnt2` 的值增加 `1`。
            *   若 `p[j].se` 等于 `y1`，然后将 `cnt3` 的值增加 `1`。
            *   若 `p[j].se` 等于 `y2`，然后将 `cnt4` 的值增加 `1`。
        *   否则，将 `ok` 的值设置为 `false`。
    *   如果 `ok` 的值为 `true` 且 `cnt1、cnt2、cnt3、cnt4` 的值大于等于 `N`，且 `x2–x1` 等于 `y2–y1`，则 `p[i]` 为要求点。打印答案。
