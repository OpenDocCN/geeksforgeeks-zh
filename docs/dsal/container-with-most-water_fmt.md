...
给定 `n` 个非负整数 `a_1, a_2, ..., a_n`，其中每个整数代表坐标 `(i, a_i)` 上的一个点。画出 `n` 条垂直线，使得线 `i` 的两个端点在 `(i, a_i)` 和 `(i, 0)` 处。
...
*   **算法:**
    ...
    3.  找出边界高度为 `array[i]` 和 `array[j]` 的容器中可以容纳的水，即 `面积 = (j - i) * min(array[i], array[j])`，如果该面积大于当前最大值，则更新当前最大值。
    4.  打印当前最大值。
*   **实施:**
...
*   **算法:**
    1.  保留两个索引，`first = 0` 和 `last = n-1` 以及一个存储最大面积的 `max_area` 值。
    2.  运行一个循环，直到 `first` 小于 `last`。
    3.  用 `max_area` 和 `min(array[first], array[last]) * (last - first)` 的最大值更新 `max_area`。
    4.  如果 `array[first]` 的值大于 `array[last]`，则将 `last` 更新为 `last - 1`，否则将 `first` 更新为 `first + 1`。
    5.  打印最大面积。
*   **实施:**
