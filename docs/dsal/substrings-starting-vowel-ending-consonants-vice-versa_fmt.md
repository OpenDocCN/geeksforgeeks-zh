...
> 原文：[https://www.geeksforgeeks.org/substrings-starting-vowel-ending-consonants-vice-versa/](https://www.geeksforgeeks.org/substrings-starting-vowel-ending-consonants-vice-versa/)
...
一个简单的解决方案是[生成所有子串](https://www.geeksforgeeks.org/program-print-substrings-given-string/)。对于每个子串，检查特殊串的条件。如果是，增加计数。
一个高效的解决方案是统计字符串每个后缀中的元音和辅音。数完这些之后，我们从头开始遍历字符串。对于每一个辅音，我们在它后面加上元音数作为结果。同样，对于每一个元音，我们在其后加一些辅音。
