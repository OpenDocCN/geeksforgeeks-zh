...
> 原文: [https://www.geeksforgeeks.org/given-number-n-decimal-base-find-number-digits-base-base-b/](https://www.geeksforgeeks.org/given-number-n-decimal-base-find-number-digits-base-base-b/)
...
**简单方法:** 将十进制数转换成给定的基数 `r`，然后计算位数。

**有效的方法:** 它基于数字的基数和该数字的位数之间的关系。

通常: 让 `n` 为正整数。如果是 `b^{d-1}\leq n < b^d`，则 `n` 的基础 `b` 表示有 `d` 位数字，如果是 `d-1 \leq \log_b n < d` 或 `\lfloor log_b n \rfloor = d-1` 就是这种情况。因此，`n` 的基数 `b` 表示中的位数是
