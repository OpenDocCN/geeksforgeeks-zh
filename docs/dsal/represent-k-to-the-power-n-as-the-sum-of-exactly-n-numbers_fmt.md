...
> 原文: [https://www.geeksforgeeks.org/represent-k-to-the-power-n-as-the-sum-of-exactly-n-numbers/](https://www.geeksforgeeks.org/represent-k-to-the-power-n-as-the-sum-of-exactly-n-numbers/)

给定两个数字 `N` 和 `K`，任务是将 `K^N` 表示为恰好 `N` 个数字的和。如果没有这样的数字，打印 `NA`。

**示例:**

> **输入:** `N = 5`，`K = 2`
> **输出:** `2 2 4 8 16`
> **解释:** `2+2+4+8+16 = 32 = 2^5`
> **输入:** `N = 4`，`K = 3`
> **输出:** `3 6 18 54`
> **解释:** `3+6+18+54 = 81 = 3^4`

**方法:** 为了得到其和是 K 的幂的数，我们可以选择满足以下条件的数:
`K^1`, `(K^2 - K^1)`, `(K^3 - K^2)`, ..., `(K^N - K^(N-1))`。
这将总是给出 k 的幂的和。
