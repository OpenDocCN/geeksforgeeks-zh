...
给定图`G`，其中包含节点`[0，N – 1]`，一个源`S`和一个数组`Edges[][3]`（类型为`{u，v，w}`），表示节点`u`和`v`之间存在有向边，权重为`w`。任务是检查给定来源是否存在负循环。如果发现是真的，则打印`“是”`。否则，打印`“否”`。
...
**方法**：这个想法是使用最短路径更快算法（SPFA）查找是否存在负循环，并且该负循环可以从图中的源顶点到达。请按照以下步骤解决问题：
...
*   停止算法并在第`N`次某个顶点放松后立即打印`是`，因为只有`N`个顶点（即从`0`到`N – 1`）。
*   否则，打印`“否”`。
