...
> 原文：[https://www.geeksforgeeks.org/count-distinct-substrings-string-using-suffix-array/](https://www.geeksforgeeks.org/count-distinct-substrings-string-using-suffix-array/)
...
我们在下面的帖子中讨论了一个基于后缀特里的解决方案：[使用后缀特里对字符串的不同子字符串进行计数](https://www.geeksforgeeks.org/kasais-algorithm-for-construction-of-lcp-array-from-suffix-array/)。

我们可以使用[后缀数组](https://www.geeksforgeeks.org/suffix-array-set-1-introduction/)和最长公共前缀概念来解决这个问题。后缀数组是给定字符串的所有后缀的排序数组。

用于字符串 `"ababa"` 的后缀有：`"ababa"`、`"baba"`、`"aba"`、`"ba"`、`"a"`。将这些后缀按排序后，我们得到后缀数组为 `[4, 2, 0, 3, 1]`。

然后使用 [kasai 的算法](https://www.geeksforgeeks.org/kasais-algorithm-for-construction-of-lcp-array-from-suffix-array/)计算 `lcp` 数组。对于字符串 `"ababa"`，`lcp` 数组是 `[1, 3, 0, 2, 0]`。
