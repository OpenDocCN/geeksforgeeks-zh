...
下面的例子说明了 JavaScript 中的 Math tanh()方法:

### 例 1:

这里计算任意数的双曲正切的公式 `e` 是一个数学常数，其近似值等于 `2.718`。
...
同样，只要用所需的数代替 `p`，就可以计算出任意数的双曲正切。这里与上面的计算相同，当我们用 `18` 代替 `x` 时，该值变成如上所示的输出。
...
因为复数不能作为函数的参数，只能取整数值作为参数，所以是错误的情况。
...
除了整数之外，没有任何东西被接受为函数的参数，这就是为什么这里字符串作为参数给出 `NaN`，即不是数字。
...
它的实际应用是每当我们需要求一个数的双曲正切值的时候，那时候我们就借助 JavaScript 中的`Math.tanh()`函数。
