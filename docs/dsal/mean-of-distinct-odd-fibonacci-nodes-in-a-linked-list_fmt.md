...
// Remove current fibonacci 
        // number from hashmap so 
        // that duplicate elements 
        // can't be counted
...
System.out.println(meanofnodes());
}
}
```
...
public static void Main(String[] args)
{
    ...
    Console.WriteLine(meanofnodes());
}
}
```

**时间复杂度**：`O(N + M)`，其中`N`是链表中的节点数，`M`是链表中的最大元素。
**辅助空间**：`O(M)`，其中`M`是链表中的最大元素。
