...
> 原文: [https://www.geeksforgeeks.org/convert-all-numbers-in-range-l-r-to-binary-number/](https://www.geeksforgeeks.org/convert-all-numbers-in-range-l-r-to-binary-number/)
...
**时间复杂度:** `O(N * LogR)`，其中 `N` 是范围 `[L, R]` 内数字的计数。
**辅助空间:** `O(N * logR)`