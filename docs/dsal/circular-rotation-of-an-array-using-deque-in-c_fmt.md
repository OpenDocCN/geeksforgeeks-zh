...
> 原文：[https://www.geeksforgeeks.org/circular-rotation-of-an-array-using-deque-in-c/](https://www.geeksforgeeks.org/circular-rotation-of-an-array-using-deque-in-c/)

给定一个整数数组 `arr[]` 和另一个整数 `D`，任务是在数组上执行 `D` 次圆周旋转并打印修改后的数组。
...
如果旋转的次数超过了 `deque` 的大小，那么就用 `N` 取模，这样，`D = D % N`。其中 `D` 为转数，`N` 为 `deque` 大小。
