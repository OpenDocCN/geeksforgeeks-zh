...
2.  在着色的同时，还要跟踪用两种颜色着色的节点数。 设两个计数为 `count_color[0]` 和 `count_color[1]` 。

3.  现在我们知道二元图可以具有的最大边数是 `count_color[0] * count_color[1]` 。

4.  我们还知道具有 `n` 个节点的树具有 `n-1` 个边。

5.  因此，我们的答案是 `count_color[0] * count_color[1] - (n - 1)` 。
