...
**天真的方法:** 天真的想法是[找到给定数 `N` 的所有可能因子](https://www.geeksforgeeks.org/find-divisors-natural-number-set-1/)，并计算每个[因子是否为完美立方体](https://www.geeksforgeeks.org/perfect-cube/)。如果是，那么计算这个因素，并检查下一个因素。
...
> 其中 `a1`，`a2`，`a3`，...，`an` 是 `n` 的不同素因子的计数

在一个完美的立方体中，不同质因数的计数必须能被 3 整除。因此，完美立方体的因子数由下式给出:

> `立方体的 N 的因子 = (1+a1/3)*(1+a2/3)*……*(1+an/3)`
> 其中 `a1`，`a2`，`a3`，...，`an` 是 `n` 的不同素因子的计数
