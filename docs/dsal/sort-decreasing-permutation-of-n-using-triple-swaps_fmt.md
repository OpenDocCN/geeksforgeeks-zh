...
> 原文：[https://www.geeksforgeeks.org/sort-decreasing-permutation-of-n-using-triple-swaps/](https://www.geeksforgeeks.org/sort-decreasing-permutation-of-n-using-triple-swaps/)

给定一个由递减排列的 `N` 个数字组成的数组 `A[]`，任务是使用三重交换对数组进行排序。如果无法对数组进行排序，则打印 `-1`。
...
要解决上面提到的问题，我们必须选择三个指标，这样我们才能在正确的位置带上至少一个元素。我们的意思是，我们必须在索引 `0` 处引入 `1`，在索引 `1` 处引入 `2`，以此类推。
