...
> 原文: [https://www.geeksforgeeks.org/subarray-permutation-that-satisfies-the-given-condition/](https://www.geeksforgeeks.org/subarray-permutation-that-satisfies-the-given-condition/)
...
天真的方法是生成所有 `M` 大小的子阵列，看看是否存在这样的子阵列。但是如果给定的排列太大，这种方法会很耗时，因为它是在 `O(N^3)` 中运行的。

### 高效方法

更好的解决方案是使用 [哈希](https://www.geeksforgeeks.org/hashing-data-structure/)。
...
4.  因此可以计算出每个整数 `1` 到 `n` 的“位置之和”，其中 `位置之和(k) = sumcur = 位置_of_1 + 位置_of_2 + ... + 位置_of_k`。
5.  让上述系列的最小元素为 `x`。当位置被排序时，这将是第一个元素，其余的将是连续的。
6.  那么如果需要的子阵列存在，那么 `位置之和(m)` 必须是 `x + (x+1) + ... + (x+m-1)` `{m 个连续项}` = `x * m - m + m * (m+1)/2`。
