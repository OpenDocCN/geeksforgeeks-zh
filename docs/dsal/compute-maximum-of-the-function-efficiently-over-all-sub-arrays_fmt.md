...
> 原文: [https://www.geeksforgeeks.org/compute-maximum-of-the-function-efficiently-over-all-sub-arrays/](https://www.geeksforgeeks.org/compute-maximum-of-the-function-efficiently-over-all-sub-arrays/)

给定一个数组，`arr[]`和一个函数`F(i, j)`。任务是计算所有子阵列`arr[i..j]`上的`最大值{F(i, j)}`。
函数`F()`定义为:
