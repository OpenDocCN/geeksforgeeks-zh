# 对 0、1 和 2 的数组进行排序（简单计数）

> 原文：[https://www.geeksforgeeks.org/sort-array-0s-1s-2s-simple-counting/](https://www.geeksforgeeks.org/sort-array-0s-1s-2s-simple-counting/)

给定一个由 0、1 和 2 组成的数组 `A[]`，编写一个对 `A[]` 进行排序的函数。这些函数应该将全 0 放在第一位，然后全 1，全 2 放在最后。
...
计算 0、1 和 2 的数量。计数后，将所有 0 放在第一位，然后是 1，最后是 2。我们遍历数组两次。时间复杂度将为 `O(n)`。
...
```python
# Python program to sort an array of 0s
# 1s and 2s.
import math
