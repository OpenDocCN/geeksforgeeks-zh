...
> 原文: [https://www.geeksforgeeks.org/solve-the-linear-equation-of-single-variable/](https://www.geeksforgeeks.org/solve-the-linear-equation-of-single-variable/)
...
`方法:` 思路是用两个指针更新两个参数：所用变量的系数 `coeff` 和总和 `total`。在“=”的左侧和右侧，对每个数字使用相反的符号，由符号变量 `sign` 处理，一旦看到“=”，符号变量将翻转。

现在，在唯一解的情况下，有效总数 `total` 和系数 `coeff` 的比值给出了所需的结果。在无限解的情况下，有效总数和系数都为零，例如 `x + 1 = x + 1`。在无解的情况下，`x` 的系数原来是零，但有效总数是非零的。
