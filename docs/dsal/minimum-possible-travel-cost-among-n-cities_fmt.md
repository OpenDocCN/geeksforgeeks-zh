...
> 原文：[https://www.geeksforgeeks.org/minimum-possible-travel-cost-among-n-cities/](https://www.geeksforgeeks.org/minimum-possible-travel-cost-among-n-cities/)

直线道路上有 `N` 个城市，每个城市之间相隔 `1` 个单位的距离。你必须乘车到达 `(N + 1)` 第个城市。从第 `i` 个城市出发将花费 `C[i]` 美元来行驶 `1` 单位的距离。换句话说，从第 `i` 个城市到第 `j` 个城市的旅行费用是 `ABS(i–j) * C[i]` 美元。任务是找到从城市 `1` 到城市 `(N + 1)` 即最后一个城市以外的最小出行成本。

## 示例

> **输入:** `C[] = {3, 5, 4}`
> **输出:** `9`
> 从第一个城市乘车的公交成本最低，所以将用于出行 `(N + 1)` 单位。
>
> **输入:** `C[] = {4, 7, 8, 3, 4}`
> **输出:** `18`
> 在第一个城市上车然后在第四个城市换乘公交车。
> `(3 * 4) + (2 * 3) = 12 + 6 = 18`

## 方法

途径很简单，只需乘坐目前为止成本最低的公交车出行即可。每当发现一辆成本更低的公共汽车，就从那个城市换一辆。以下是解决步骤：

1.  先从第一个城市开始，成本 `C[1]`。
2.  前往下一个城市，直到找到一个费用低于上一个城市（我们旅行的城市，比如说 `i`）的城市。
