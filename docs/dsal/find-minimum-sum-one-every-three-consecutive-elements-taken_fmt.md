...
> 原文: [https://www.geeksforgeeks.org/find-minimum-sum-one-every-three-consecutive-elements-taken/](https://www.geeksforgeeks.org/find-minimum-sum-one-every-three-consecutive-elements-taken/)
...
当 `arr[i]` 是解和的一部分(不一定是结果)并且是最后选择的元素时，设 `sum(i)` 是最小可能和。那么我们的结果是 `sum(n-1)`、`sum(n-2)` 和 `sum(n-3)` 的最小值[我们必须选择最后三个元素中的至少一个]。
我们可以递归地计算 `sum(i)` 作为 `arr[i]` 和 `minimum(sum(i-1), sum(i-2), sum(i-3))` 的和。由于问题的递归结构中存在[重叠子问题](https://www.geeksforgeeks.org/dynamic-programming-set-1/)，我们可以使用动态规划来解决这个问题。
以下是以上思路的实现。
