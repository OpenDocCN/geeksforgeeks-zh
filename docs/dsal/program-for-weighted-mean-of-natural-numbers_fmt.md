...
> 原文: [https://www.geeksforgeeks.org/program-for-weighted-mean-of-natural-numbers/](https://www.geeksforgeeks.org/program-for-weighted-mean-of-natural-numbers/)
...
计算前 `n` 个自然数的加权平均值。这是计算前 `n` 个自然数加权平均值的公式。在这个方法中，我们给出了前 `n` 个自然数，它们的权重也是自然数。然后我们生成公式。
...
本文由 `达曼德拉·库马尔` 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 `contribute.geeksforgeeks.org` 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
