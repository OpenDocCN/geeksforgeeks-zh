...
## 树边
在图中应用 DFS 后得到的树中存在的边。所有的绿色边都是树边。

## 前进边
这是一个边`(u，v)`，这样 `v` 是后代，但不是 DFS 树的一部分。从 `1 到 8` 的边缘是向前的边缘。

## 后沿
是一条边`(u，v)`，这样 `v` 就是节点 `u` 的祖先，但不是 DFS 树的一部分。从 `6 到 2` 的边缘是后边缘。[后沿的出现表示有向图](https://www.geeksforgeeks.org/detect-cycle-in-a-graph/)中的循环。

## 交叉边
是连接两个节点的边，使得它们之间没有任何祖先和后代关系。节点 `5 至 4` 的边为交叉边。
