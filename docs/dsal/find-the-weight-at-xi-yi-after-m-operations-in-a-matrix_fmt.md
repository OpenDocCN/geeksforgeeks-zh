...
给定笛卡尔平面上的 `n x n` 点，任务是在 `m` 次操作后找到 `(x_i, y_j)` 处的重量。`x_i`，`y_j`，`w` 表示在线条 `x = x_i` 和 `y = y_j` 上的所有点上添加重量 `w` 的操作。
...
**简单方法：**
考虑一个二维数组 `arr[n][n] = {0}` 并执行给定的操作，然后在 `(x_i, y_j)` 处检索权重。这种方式需要 `O(n*m)` 时间。
