...
[亚马逊面试经历](https://www.geeksforgeeks.org/amazon-interview-experience-set-359-on-campus/)

### 简单方法

将运行`for`循环，以检查每个元素是否在给定范围内并保持其计数。 运行每个查询的时间复杂度为`O(n)`。
...
将是首先对数组进行排序，然后使用经过修改的二分搜索函数找到两个索引，第一个元素大于或等于范围下限，另一个元素小于或等于上限。 运行每个查询的时间为`O(logn)`，对数组进行一次排序的时间为`O(nlogn)`。
