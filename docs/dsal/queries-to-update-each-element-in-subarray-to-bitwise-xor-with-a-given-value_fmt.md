...
> 原文: [https://www.geeksforgeeks.org/queries-to-update-each-element-in-subarray-to-bitwise-xor-with-a-given-value/](https://www.geeksforgeeks.org/queries-to-update-each-element-in-subarray-to-bitwise-xor-with-a-given-value/)
...
> **输出:** `0 5 0 1 4`
> **解释:**
> 1<sup>st</sup> 查询: 关注子数组 `{2, 3, 6}`，在用其与 `val(= 2)` 的异或替换每个元素后，修改为 `{0, 1, 4}`。
> 2<sup>nd</sup> 查询: 关注子数组 `{3, 6, 5}`，在将每个元素替换为其与 `val(= 4)` 的异或运算后，修改为 `{5, 0, 1}`。
> 因此，最终数组为 `{0, 5, 0, 1, 4}`。
>
...
按照以下步骤通过以恒定的时间复杂度处理每个查询来解决问题:

*   用全零初始化数组 `temp[]`。
*   对于表单 `(l, r, val)` 的每个查询，更新 `temp[l-1]` 和 `temp[r]`，将其各自的与值异或。
*   对每个查询完成上述步骤后，将 `temp[]` 转换为**前缀异或数组**。
*   最后，通过将第 i<sup>个</sup>元素替换为其异或来执行更新 `arr[]`。
