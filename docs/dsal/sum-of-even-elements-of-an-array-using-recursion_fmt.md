...
> 原文：[https://www.geeksforgeeks.org/sum-of-even-elements-of-an-array-using-recursion/](https://www.geeksforgeeks.org/sum-of-even-elements-of-an-array-using-recursion/)
...
编写一个递归函数，将数组作为参数，用 `sum` 变量存储所考虑元素的和和索引。如果所需索引处的当前元素被添加到 `sum` 中，则不要更新 `sum`，并再次为下一个索引调用相同的方法。终止条件将是当没有剩余元素需要考虑时，即传递的索引超出了给定数组的界限，打印总和，并在这种情况下返回。
