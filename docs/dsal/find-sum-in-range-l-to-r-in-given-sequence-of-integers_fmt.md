...
> 原文: [https://www.geeksforgeeks.org/find-sum-in-range-l-to-r-in-given-sequence-of-integers/](https://www.geeksforgeeks.org/find-sum-in-range-l-to-r-in-given-sequence-of-integers/)

给定一个整数的数组 `arr[]` 和一个形式为:
`[2, 3, 0, 1, 6, 7, 4, 5, …]` 的序列。

还给出了两个整数 `L` 和 `R`，这样 `0 \le L \le R \le 10^{6}`。任务是找出从 `L` 到 `R` 的给定范围内所有数字的和。
