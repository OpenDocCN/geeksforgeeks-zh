...
给定一个 `n` 乘 `n` 的板，其中 `n` 的形式为 `2^k`，其中 `k >= 1`（基本上 `n` 是 2 的幂，最小值为 2）。该板缺少一个单元（大小为 1 x 1）。用 L 形瓷砖填充木板。L 形瓷砖是一个 2×2 的正方形，缺少一个 1×1 大小的单元。
...
`T(n) = 4T(n/2) + C`
以上递归可以用[主方法](https://www.geeksforgeeks.org/analysis-algorithm-set-4-master-method-solving-recurrences/)求解，时间复杂度为 `O(n^2)`。

### 这是如何工作的？

使用数学归纳法可以证明分治算法的工作原理。让输入方块的大小为 `2^k x 2^k` ，其中 `k >= 1`。
基本情况:我们知道对于 `k = 1` 问题是可以解决的。我们有一个 `2 x 2` 的正方形，少了一个单元格。
归纳假设:让问题可以为 `k-1` 解决。
现在我们需要证明的是，如果问题对于 `k-1` 可以解，那么对于 `k` 可以解。对于 `k`，我们在中间放置一个 L 形瓷砖，我们有四个子曲面，尺寸为 `2^{k-1} x 2^{k-1}` ，如上图 2 所示。所以如果我们能解 4 个子正方形，我们就能解完整的正方形。
