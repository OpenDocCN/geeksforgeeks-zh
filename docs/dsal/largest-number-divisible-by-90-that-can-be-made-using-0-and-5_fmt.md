...
因为我们可以选择和置换任意数量的元素，所以只有数组中 `0` 和 `5` 的数量是重要的。让我们将计数分别存储为 `c0` 和 `c5`。
...
*   如果以上两个条件都为假。让我们把 `5` 的数量分成 `9` 组。将会有完全填满的 `floor(c5/9)` 组，我们可以使用所有组的所有 `5` 来获得 `9` 的倍数，这也使得数字和是 `9` 的倍数。因为增加零的数量不会影响整除性，所以我们可以使用所有的零。
