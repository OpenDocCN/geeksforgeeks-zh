...
> 原文：[https://www.geeksforgeeks.org/sort-n-numbers-range-0-n2-1-linear-time/](https://www.geeksforgeeks.org/sort-n-numbers-range-0-n2-1-linear-time/)
...
如果我们使用[计数排序](https://www.geeksforgeeks.org/counting-sort/)，将花费 O(n^2) 时间，因为给定的范围是 n^2。使用任何基于比较的排序，如[合并排序](http://geeksquiz.com/merge-sort/)、[堆排序](http://geeksquiz.com/heap-sort/)等，需要 O(nLogn) 时间。

现在问题来了，如何在 O(n) 中做到这一点？首先，可能吗？我们可以使用问题中给出的数据吗？`n` 个数字，范围从 0 到 n<sup>2</sup>–1？
...
如果范围从 1 到 n<sup>2</sup>，则不能直接应用上述过程，必须进行更改。考虑 `n = 100`，范围从 1 到 10000。因为基数是 100，所以一个数字必须从 0 到 99，并且数字中应该有 2 个数字。但是数字 10000 有超过 2 位数。所以要在 1 到 n<sup>2</sup> 的范围内对数字进行排序，我们可以使用以下过程。
