...
> 原文: [https://www.geeksforgeeks.org/calculate-the-square-of-euclidean-distance-traveled-based-on-given-conditions/](https://www.geeksforgeeks.org/calculate-the-square-of-euclidean-distance-traveled-based-on-given-conditions/)

给定一个数组`commands[]`，该数组由表示沿坐标行进的距离和方向的有符号整数和表示无法访问的坐标的数组`obstacles[]`组成，任务是按照在序列中指定的命令，按照`commands[]`数组中的顺序，找到可从原点`(0, 0)`开始向北行进的最大[欧几里德距离](https://en.wikipedia.org/wiki/Euclidean_distance#:~:text=In%20mathematics%2C%20the%20Euclidean%20distance,%20metric%20as%20the%20Pythagorean%20metric.)的**正方形的正方形**
...
4.  制作 `2` 个数组 `(dx[], dy[])` 并根据方向的变化将所有可能的运动存储在 `x` 和 `y` 坐标中。
5.  如果遇到方向改变，参照 `2` 个数组改变当前方向。
...
**时间复杂度：** `O(N)`
**辅助空间：** `O(N + M)`，其中 `M` 是这个`obstacles`数组的长度。