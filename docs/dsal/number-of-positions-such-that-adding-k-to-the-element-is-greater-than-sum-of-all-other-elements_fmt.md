...
> 原文：[https://www.geeksforgeeks.org/number-of-positions-such-that-adding-k-to-the-element-is-greater-than-sum-of-all-other-elements/](https://www.geeksforgeeks.org/number-of-positions-such-that-adding-k-to-the-element-is-greater-than-sum-of-all-other-elements/)

给定一个数组 `arr[]` 和一个数字 `K`。任务是找出有效位置 `i` 的数量，使得 `(arr[i] + K)` *大于*除 `arr[i]` 之外的数组所有元素的和。
