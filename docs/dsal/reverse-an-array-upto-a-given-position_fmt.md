...
> 原文: [https://www.geeksforgeeks.org/reverse-an-array-upto-a-given-position/](https://www.geeksforgeeks.org/reverse-an-array-upto-a-given-position/)

给定数组 `arr[]` 和数组 `k` 中的一个位置，写一个函数名 `reverse(a[], k)`，这样它就可以反转子数组 `arr[0..k-1]`。使用的额外空间应为 `O(1)`，时间复杂度应为 `O(k)`。
