...
这里，我们需要保留两个相邻列表的副本，一个用于正差，另一个用于负差。像在[迪克斯特拉算法](https://www.geeksforgeeks.org/dijkstras-shortest-path-algorithm-greedy-algo-7/)中一样取一个[优先级队列](https://www.geeksforgeeks.org/priority-queue-set-1-introduction/)，一次保留四个变量，即：
...
对于每个顶点，根据所需条件（`阶段`值）推相邻顶点。请参阅代码以获得更好的理解。
