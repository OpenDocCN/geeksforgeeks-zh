...
> 原文: [https://www.geeksforgeeks.org/sort-string-of-characters-using-stack/](https://www.geeksforgeeks.org/sort-string-of-characters-using-stack/)
...
*   迭代字符串中的所有字符
    ...
    2.  如果第 i <sup>个</sup>字符不大于，则将`栈`的所有元素推入`tempstack`，然后将该字符推入`栈`。之后，将`tempstack`的所有大元素推至`栈`。
