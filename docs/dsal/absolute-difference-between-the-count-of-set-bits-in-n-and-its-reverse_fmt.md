...
> 原文: [https://www.geeksforgeeks.org/absolute-difference-between-the-count-of-set-bits-in-n-and-its-reverse/](https://www.geeksforgeeks.org/absolute-difference-between-the-count-of-set-bits-in-n-and-its-reverse/)

给定一个整数 `N`，任务是找出存在于数字 `N` 和数字 `N` 相反的[设定位的数量之间的绝对差异](https://www.geeksforgeeks.org/count-total-set-bits-in-all-numbers-from-1-to-n/)。
...
> 集合比特数 = `4`
> `135` 的倒数为 `531`
> 二进制表示的 `(531)₁₀ = (100001101)₂`

## 方法

主要思路是使用 [STL 库](https://www.geeksforgeeks.org/the-c-standard-template-library-stl/)的[位集功能](https://www.geeksforgeeks.org/c-bitset-and-its-application/)。

按照以下步骤解决给定的问题:

1.  [反转数字 `N`](https://www.geeksforgeeks.org/write-a-program-to-reverse-digits-of-a-number/) 的数字并将其存储在变量中，比如 `revN`。
2.  使用[位设置](https://www.geeksforgeeks.org/c-bitset-and-its-application/)功能[统计 `N` 中的设置位数](https://www.geeksforgeeks.org/count-set-bits-in-an-integer/)。
