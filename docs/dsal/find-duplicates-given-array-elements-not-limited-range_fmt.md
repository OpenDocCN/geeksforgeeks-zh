...
想法是使用嵌套循环，并对每个元素检查数组中元素是否存在多次。如果存在，则将其存储在哈希映射中。否则，请继续检查其他元素。
...
使用[`unordered_map`](https://www.geeksforgeeks.org/unordered_map-in-stl-and-its-applications/)进行哈希处理。打印每个元素的出现频率和频率大于 1 的元素。`unordered_map`因为整数范围未知而被使用。对于 Python，使用字典将数字存储为键，将频率存储为值。字典可以用作整数范围是未知的。
