...
> 原文: [https://www.geeksforgeeks.org/check-if-all-the-1s-in-a-binary-string-are-equidistant-or-not/](https://www.geeksforgeeks.org/check-if-all-the-1s-in-a-binary-string-are-equidistant-or-not/)

给定一个二进制字符串 `str`，任务是检查字符串中所有的 `1` 是否等距。术语等距是指每两个相邻的 `1` 之间的距离相同。注意字符串至少包含两个 `1`。
...
> **输入:** `str = "0101001"`
> **输出:** 否
> 第1个与第2个 `1` 的距离为 2，第2个与第3个 `1` 的距离为 3。

**方法:** 将字符串中所有 `1` 的位置存储在一个向量中，然后检查每两个连续位置之间的差异是否相同。
