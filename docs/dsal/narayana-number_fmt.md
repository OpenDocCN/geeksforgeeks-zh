...
在组合学中，[纳拉亚纳数](https://en.wikipedia.org/wiki/Narayana_number) `N(n, k)`，其中 `n = 1, 2, 3 …`，`1 ≤ k ≤ n`，组成自然数的三角数组，称为纳拉亚纳三角。由以下公式给出：
![{\displaystyle N(n,k)={\frac {1}{n}}{n \choose k}{n \choose k-1}.}  ](img/ace85f196ac66ce457b8f84f24e331e3.png "Rendered by QuickLaTeX.com")

纳拉亚纳数 `N(n, k)` 可用于查找包含括号的 `n` 对的表达式的数量，这些表达式匹配正确且包含 `k` 个不同的嵌套。
