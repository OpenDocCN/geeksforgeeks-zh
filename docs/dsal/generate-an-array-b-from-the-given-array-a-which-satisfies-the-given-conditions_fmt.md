...
> 原文: [https://www.geeksforgeeks.org/generate-an-array-b-from-the-given-array-a-which-satisfies-the-given-conditions/](https://www.geeksforgeeks.org/generate-an-array-b-from-the-given-array-a-which-satisfies-the-given-conditions/)

给定一个由 `N` 个整数组成的数组 `A[]`，使得 `A[0] + A[1] + A[2] + … + A[N-1] = 0`。任务是为所有有效的 `i` 生成一个数组 `B[]`，使得 `B[i]` 要么是 `⌊a[i/2⌋` 要么是 `⌈a[i/2⌉`，并且 `b[0] + b[1] + b[2] + … + b[n-1] = 0`。
...
对于偶数整数，可以安全地假设 `B[i]` 将是 `A[i] / 2`。但是对于奇数整数，为了保持和等于零，取正好一半奇数整数的上限和正好另一半奇数整数的下限。既然奇–奇=偶且偶–偶=偶且 0 也是偶，可以说 `A[]` 将始终包含偶数个奇整数，这样和就可以是 `0`。所以对于有效的输入，总会有答案。
