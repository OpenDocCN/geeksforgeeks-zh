...
> 原文：[https://www.geeksforgeeks.org/remove-nodes-root-leaf-paths-length-k/](https://www.geeksforgeeks.org/remove-nodes-root-leaf-paths-length-k/)

给定一棵二叉树和一个数字 `k`，移除所有仅位于长度小于 `k` 的根到叶路径上的节点。如果一个节点 `X` 位于多个根到叶路径上，并且如果任何路径的路径长度 >= `k`，则 `X` 不会从二叉树中删除。换句话说，如果经过一个节点的所有路径的长度都小于 `k`，则该节点被删除。考虑下面的二叉树示例：
...
只有一条路径 `1->2->5` 的长度小于 4。节点 `5` 是唯一仅位于此路径上的节点，因此节点 `5` 被移除。节点 `2` 和 `1` 没有被移除，因为它们也是其他长度为 4 的路径的一部分。

如果 `k` 是 5 或大于 5，则整棵树被删除。

如果 `k` 是 3 或小于 3，则没有任何节点被删除。

**我们强烈建议尽量减少你的浏览器，先自己试试这个。**

这里的想法是使用后序遍历树。在移除一个节点之前，我们需要检查该节点在较短路径中的所有子节点是否已经被移除。有 2 种情况：
