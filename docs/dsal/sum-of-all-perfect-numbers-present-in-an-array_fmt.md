...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-perfect-numbers-present-in-an-array/](https://www.geeksforgeeks.org/sum-of-all-perfect-numbers-present-in-an-array/)
...
**方法:** 初始化 `sum = 0`。对于数组的每个元素，找到其适当因子的和，比如 `sumFactors`。如果 `arr[i] = sumFactors`，则更新结果总和为 `sum = sum + arr[i]`。最后打印 `sum`。
