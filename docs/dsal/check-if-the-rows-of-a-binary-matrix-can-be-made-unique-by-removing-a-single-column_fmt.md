# 通过删除一列检查二进制矩阵的行是否可以唯一

> 原文: [https://www.geeksforgeeks.org/check-if-the-rows-of-a-binary-matrix-can-be-made-unique-by-removing-a-single-column/](https://www.geeksforgeeks.org/check-if-the-rows-of-a-binary-matrix-can-be-made-unique-by-removing-a-single-column/)

给定大小为 `M * N` 的二元矩阵 `mat[][]`。任务是检查从矩阵中删除一列后矩阵的行是否唯一。

**示例:**

> **输入:** `mat[][] = { {1, 0, 1}, {0, 0, 0}, {1, 0, 0} }`
> **输出:** 是
> 删除第 2 列后，矩阵每行变得唯一。
>
> **输入:** `mat[][] = { {1, 0}, {1, 0} }`
