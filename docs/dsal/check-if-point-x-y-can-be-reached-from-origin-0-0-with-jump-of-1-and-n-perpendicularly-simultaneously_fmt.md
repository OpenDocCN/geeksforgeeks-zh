...
*   **情况 1–其中 N 是偶数** : 在这种情况下，任何坐标`(X，Y)`都是可达的。让我们一次考虑 X 和 Y。对于一个坐标 X，可以遵循 `(1，N)`、`(1，-N)`、`(1，N)`、`(1，-N)`、… 跳跃的顺序。生成的坐标将是所需坐标 `(X，0)` ，或 `(X，N)` 。由于 `N` 为偶数，所以也可以按照跳跃的顺序 `(N，-1)`、`(-N，-1)`、`(N，-1)`、……到达 `(X，0)` 。同样可以到达 `(0，Y)` ，结合跳转的顺序到达 `(X，0)` 、 `(0，Y)`、`(X，Y)` 。
*   **情况 2–其中 N 是奇数，X 和 Y 都是偶数或奇数** : 这些情况可以通过遵循类似于**情况 1** 的操作序列来处理。
*   **情况 3–其中 N 是奇数，X 和 Y 具有不同的奇偶校验** : 在这种情况下，没有可能的跳跃序列到达 `(X，Y)` ，因为根据情况 2， `(X + 1，Y)` 和 `(X，Y + 1)` 必须是可达的，因为它们都将具有相同的奇偶校验，并且没有可能的操作序列覆盖 `(1，0)` 或`的距离`
...
<script>
    // Function to check if (X, Y) is reachable
// from (0, 0) using the jumps of given type
function checkReachability(N, X, Y)
{
