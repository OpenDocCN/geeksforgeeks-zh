...
> 原文: [https://www.geeksforgeeks.org/number-subarrays-maximum-value-given-range/](https://www.geeksforgeeks.org/number-subarrays-maximum-value-given-range/)
...
*   大小为 `N` 的阵列的所有可能子阵列的数量是 `N * (N + 1)/2`。让 `countSubarrys(N) = N * (N + 1)/2`。

我们跟踪当前子阵列中的两个计数。
1) 所有小于或等于 `r` 的元素的计数。我们称之为 `inc`。
2) 所有小于 `l` 的元素计数，我们称之为 `exc`。
我们对当前子阵列的回答是 `countSubarrys(inc) – countSubarrys(exc)`。我们基本上去除了所有那些仅由小于 `l` 的元素构成的子阵列。
