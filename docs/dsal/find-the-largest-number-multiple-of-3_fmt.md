...
> 原文: [https://www.geeksforgeeks.org/find-the-largest-number-multiple-of-3/](https://www.geeksforgeeks.org/find-the-largest-number-multiple-of-3/)
...
简单 & 直接方法是生成元素的所有组合，并跟踪形成的可被 3 整除的最大数。
时间复杂度: `O(n×2^n)`。将会有数组元素的 `2^n` 组合。将每个组合与迄今为止最大的数字进行比较可能需要 `O(n)` 个时间。
...
**2.** 走三个队列。一个用于存储被 3 除后余数为 0 的元素。第二个队列存储的数字除以 3 得到的余数为 1。第三个队列存储的数字除以 3 得到的余数为 2。称它们为 `queue0`、`queue1` 和 `queue2`。
...
从 `queue1` 中删除一个项目。如果 `queue1` 为空，请从 `queue2` 中删除两个项目。如果 `queue2` 包含的项目少于两个，则该数量是不可能的。
…… **4.3** 数字之和除以 3 后得出余数 2。
从 `queue2` 中删除一个项目。如果 `queue2` 为空，请从 `queue1` 中删除两个项目。如果 `queue1` 包含的项目少于两个，则该数量是不可能的。
