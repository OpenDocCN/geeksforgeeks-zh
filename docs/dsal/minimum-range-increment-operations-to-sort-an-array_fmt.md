...
> 原文：[https://www.geeksforgeeks.org/minimum-range-increment-operations-to-sort-an-array/](https://www.geeksforgeeks.org/minimum-range-increment-operations-to-sort-an-array/)

给定一个包含 `N` 个元素的数组。允许在数组上执行以下任意次数的移动：
...
考虑一个排序的数组，递增数组的所有元素仍然会得到一个排序的数组。所以想法是从最后一个索引开始，从右开始遍历数组的元素，并跟踪最小的元素。如果在任何时候，元素的顺序被发现是增加的，通过从当前元素中减去右边的最小元素来计算移动的次数。
