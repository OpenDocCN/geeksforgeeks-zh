...
我们知道，`LCM(a, b) = (a * b) / gcd(a, b)`。上面的关系只适用于两个数字，`LCM(a, b, c) != (a * b * c) / gcd(a, b, c)`。

这里的想法是将我们的关系扩展到 2 个以上的数字。假设我们有一个数组 `arr[]` 包含 n 个元素，需要计算它们的 LCM。
我们算法的主要步骤是：

1.  初始化 `ans = arr[0]`。
2.  迭代数组的所有元素，即从 `i = 1` 到 `i = n-1`。在第 `i` 次迭代中 `ans = LCM(arr[0], arr[1], ..., arr[i-1])`。这可以通过 `LCM(arr[0], arr[1], ..., arr[i]) = LCM(ans, arr[i])` 轻松完成。因此在第 `i` 次迭代中，我们只需要做 `ans = LCM(ans, arr[i]) = ans * arr[i] / gcd(ans, arr[i])`。
