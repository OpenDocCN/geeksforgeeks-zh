...
> 原文: [https://www.geeksforgeeks.org/rectangle-with-minimum-possible-difference-between-the-length-and-the-width/](https://www.geeksforgeeks.org/rectangle-with-minimum-possible-difference-between-the-length-and-the-width/)
...
任务是找到两个整数`l`和`b`，使得`l * b = 面积`和`| l - b |`尽可能最小。因子分解可以用来解决问题，但是从`1`到`N`只做简单的因子分解，对于`N`的较大值，需要很长时间才能得到需要的输出。
要克服这一点，只需迭代到。考虑到`√N < l ≤ N`，那么对于`l`、`b`的所有值将始终为`< √N`。
