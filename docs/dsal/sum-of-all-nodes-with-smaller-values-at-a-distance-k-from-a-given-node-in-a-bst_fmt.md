...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-nodes-with-smaller-values-at-a-distance-k-from-a-given-node-in-a-bst/](https://www.geeksforgeeks.org/sum-of-all-nodes-with-smaller-values-at-a-distance-k-from-a-given-node-in-a-bst/)

给定一个[二叉查找树](https://www.geeksforgeeks.org/binary-search-tree-set-1-search-and-insertion/)、一个`目标节点`和一个整数值 `K`，任务是找出距离`目标节点` `K` 距离值小于`目标节点`的所有节点的和。
...
**方法:** 给定的问题可以通过对`目标节点`下方的 `K` 距离执行 [DFS 遍历](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/)并对`目标节点`上方的 `K` 距离执行 DFS 遍历来解决。按照以下步骤解决问题:
...
*   将变量 `dl` 初始化为 `-1`，如果`目标`小于`根`，则将 `dl` 的值设置为函数`kdistansum(根- >左，目标 k，求和)`返回的值。
