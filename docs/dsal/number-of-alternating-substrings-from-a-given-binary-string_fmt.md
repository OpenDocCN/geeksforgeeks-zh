...
> 原文：[https://www.geeksforgeeks.org/number-of-alternating-substrings-from-a-given-binary-string/](https://www.geeksforgeeks.org/number-of-alternating-substrings-from-a-given-binary-string/)
...
> **输出：** 7
> **解释：**
> 字符串 `S` 的所有子串为：`{"0", "00", "001", "0010", "0", "01", "010", "1", "10", "0"}`
> 其中为交替出现的子串为：`{"0", "0", "01", "010", "1", "10", "0"}`。
> 因此，答案是 7。
>
> **输入：** `S = "010"`
> **输出：** 6

**天真法：** 解决这个问题最简单的方法就是首先找到[字符串 `S` 的所有子串](https://www.geeksforgeeks.org/program-print-substrings-given-string/)，然后检查每一个[字符串](https://www.geeksforgeeks.org/category/data-structures/c-strings/)是否交替。

**时间复杂度：** `O(N^3)`
**辅助空间：** `O(N^2)`

**有效方法：** 该问题具有[重叠子问题属性](https://www.geeksforgeeks.org/overlapping-subproblems-property-in-dynamic-programming-dp-1/)和[最优子结构属性](https://www.geeksforgeeks.org/optimal-substructure-property-in-dynamic-programming-dp-2/)。所以这个问题可以用[动态规划](https://www.geeksforgeeks.org/dynamic-programming/)来解决。按照以下步骤解决此问题：

*   声明一个 `dp` 数组，其中 `dp[i][j]` 存储以字符 `i` 开始且在 `[j，N-1]` 范围内的交替字符串的数量。
...
*   初始化一个变量，比如说 `ans` 为 `0`，以存储交替出现的子串的数量。
*   [使用变量 `i` 在范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[0，N-1]` 中迭代，并执行以下步骤：
    *   将 `ans` 更新为 `dp[0][i]` 和 `dp[1][i]` 的最大值。
*   最后，打印在 `ans` 中获得的值作为答案。
