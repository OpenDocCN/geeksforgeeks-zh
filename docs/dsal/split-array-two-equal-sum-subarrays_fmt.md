...
> 原文：[https://www.geeksforgeeks.org/split-array-two-equal-sum-subarrays/](https://www.geeksforgeeks.org/split-array-two-equal-sum-subarrays/)
...
时间复杂度:O(n)<sup>2</sup>)
辅助空间:O(1)
`一个高效的解决方案`是先从左到右计算整个数组的和。现在我们从右边遍历数组并跟踪右边的和，左边的和可以通过从整个和中减去当前元素来计算。
以下是以上思路的实现。
