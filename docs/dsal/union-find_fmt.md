...
联合查找算法可以用来检查无向图是否包含循环。注意，我们已经讨论了检测周期的[算法](http://www.geeksforgeeks.org/archives/18516)。这是另一种基于联合-寻找的方法。该方法假设图形不包含任何自循环。

我们可以跟踪 1D 数组中的子集，让我们称之为 `父[]`。
让我们考虑以下图表：
...
最初，`父`数组的所有槽都被初始化为 `-1`（意味着每个子集中只有一个项目）。
