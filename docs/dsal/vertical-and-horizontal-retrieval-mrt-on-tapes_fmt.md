...
> 原文：[https://www.geeksforgeeks.org/vertical-and-horizontal-retrieval-mrt-on-tapes/](https://www.geeksforgeeks.org/vertical-and-horizontal-retrieval-mrt-on-tapes/)

**先决条件：** [磁带上的最佳存储](https://www.geeksforgeeks.org/optimal-storage-tapes/)

您将获得不同大小的 `tape[]` 和不同卷的 `records[]`。任务是水平或垂直填充磁带中的卷，以便最大限度地减少检索时间。

**举例：**

> **输入：** `tape[] = { 25, 80, 160 }`, `records[] = { 15, 2, 8, 23, 45, 50, 60, 120 }`
> **输出：**
> **水平填充：**
> 第一盘磁带：`[ 2 8 15 ]` MRT：12.3333
> 第二盘磁带：`[ 23 45 ]` MRT：45.5
> 第三盘磁带：`[ 50 60 ]` MRT：80
> **垂直填充：**
> 第一盘磁带：`[ 2 23 ]` MRT：13.5
> 第二盘磁带：`[ 8 45 ]` MRT：
...
我们有，
`tape[] = { 25, 80, 160 }`
`records[] = { 2, 8, 15, 23, 45, 50, 60, 120 }` *(按递增顺序)*
...
比较两种结果，我们可以使用一种特殊的胶带填充技术，这种技术可以提供最好的结果(最小的 MRT)。