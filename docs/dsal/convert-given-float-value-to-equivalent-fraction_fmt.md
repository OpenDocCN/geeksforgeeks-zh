...
> 原文: [https://www.geeksforgeeks.org/convert-given-float-value-to-equivalent-fraction/](https://www.geeksforgeeks.org/convert-given-float-value-to-equivalent-fraction/)

给定一个形式为[字符串](https://www.geeksforgeeks.org/string-data-structure/) `N` 的[浮点数](https://www.geeksforgeeks.org/introduction-of-floating-point-representation/)，任务是将给定的[浮点数](https://www.geeksforgeeks.org/introduction-of-floating-point-representation/)转换为[分数](https://www.geeksforgeeks.org/program-compare-two-fractions/)。
...
*   如果没有循环序列，那么将给定的数字乘以 `10^x`，让 `10^x` 和结果数的 `GCD` 为 `g`。打印结果除以 `g` 作为分子，`10^x` 除以 `g` 作为分母。
> 例如，如果 `N = "1.5"` 那么 `x = 1`。
> 将 `N` 乘以 `10`，结果为 `15`，`10` 和 `15` 的 `GCD` 为 `3`。
> 因此，打印 `15/3 = 5` 作为分子，`10/3` 作为分母。

*   如果存在重复序列，则将 `N` 乘以 `10^x`。比如 `N = 23.98(231)` 乘以 `N*(10^2)`。
*   让序列中的总位数为 `y`。对于 `10^2 * N = 2398.(231)`，`y` 变为 `3`。
*   将 `10^y` 与 `N * 10^x` 相乘。对于 `10^2 * N = 2398.(231)`，乘以 `10^3` 即 `10^2 * N * 10^3 = 2398231.(231)`。
*   现在，用 `N * 10^(y+x)` 减去 `N * 10^x`，结果为 `M`。上例中，`10^2 * N * (10^3 - 1) = 2395833`。
*   因此，`N = M / ((10^x) * (10^y – 1))`。上例 `N = 2395833 / 999000`。
*   找到 `M` 和 `((10^x) * (10^y – 1))` 的 `GCD`，打印 `M / gcd` 作为分子，`((10^x) * (10^y – 1))` 作为分母。
