# 在 O(1) 空间的数组中寻找出现次数超过 n/3 的元素

> 原文: [https://www.geeksforgeeks.org/n3-repeated-number-array-o1-space/](https://www.geeksforgeeks.org/n3-repeated-number-array-o1-space/)

给定一个包含 `n` 个整数的只读数组。请在线性时间和恒定的附加空间中，找出数组中出现次数超过 `n/3` 的任何元素。如果不存在这样的元素，返回 `-1`。
