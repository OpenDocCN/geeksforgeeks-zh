...
> 原文: [https://www.geeksforgeeks.org/generate-a-combination-of-minimum-coins-that-results-to-a-given-value/](https://www.geeksforgeeks.org/generate-a-combination-of-minimum-coins-that-results-to-a-given-value/)

给定一个代表可用面额的大小为 `N` 的数组 `arr[]` 和一个整数 `X`。任务是找到可用面额的最小硬币数量的任意组合，使得硬币的总和为 `X`。如果可用面额无法获得给定金额，打印 `-1`。
...
最简单的方法是尝试给定面额的所有可能组合，使得在每个组合中，硬币的总和等于 `X`。从这些组合中，选择硬币数量最少的一个并打印出来。如果任意组合之和不等于 `X`，打印 `-1`。
**时间复杂度:** `O(X^N)`
**辅助空间:** `O(N)`

## 高效方法

上述方法可以使用动态规划来优化找到最小硬币数。在找到最小硬币数的同时，可以使用回溯来跟踪使其总和等于 `X` 所需的硬币。按照以下步骤解决问题:

1.  初始化一个辅助数组 `dp[]`，其中 `dp[i]` 将存储使总和等于 `i` 所需的最小硬币数。
2.  使用这篇文章中讨论的方法，找出使它们的总和等于 `X` 所需的最小硬币数。
3.  找到最小硬币数后，使用回溯技术追踪使用的硬币，使总和等于 `X`。
4.  回溯时，遍历数组选择一个小于当前和的硬币，使得 `dp[当前和]` 等于 `dp[当前和–选择的硬币] + 1`。将所选硬币存储在一个数组中。
