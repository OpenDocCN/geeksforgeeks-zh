...
> 原文：[https://www.geeksforgeeks.org/partition-array-into-minimum-number-of-equal-length-subsets-consisting-of-a-single-distinct-value/](https://www.geeksforgeeks.org/partition-array-into-minimum-number-of-equal-length-subsets-consisting-of-a-single-distinct-value/)

给定一个大小为 `N` 的数组 `arr[]`，任务是打印最小计数的等长子集数组可以被划分成这样，每个子集只包含一个不同的元素。
...
解决问题最简单的方法是存储每个不同数组元素的频率，使用变量 `i` 迭代范围 `[N, 1]`，检查数组中所有不同元素的频率是否可以被 `i` 整除。如果发现为真，则打印 `(N / i)` 的值。
...
优化上述方法的思路是使用 GCD 的概念。按照以下步骤解决问题：

*   初始化一个映射，比如 `freq`，来存储数组中每个不同元素的频率。
*   初始化一个变量，比如说 `freqGCD`，来存储数组中每个不同元素的频率的 GCD。
*   遍历地图找到 `freq` 的值。
*   最后打印 `(N / freqGCD)` 的值。
