...
> 原文: [https://www.geeksforgeeks.org/check-if-a-given-string-can-be-converted-to-a-balanced-bracket-sequence/](https://www.geeksforgeeks.org/check-if-a-given-string-can-be-converted-to-a-balanced-bracket-sequence/)

给定大小为 `N` 的字符串 `S`，由 `(`、`)` 和 `$` 组成，任务是通过用 `)` 或 `(` 替换每次出现的 `$` 来检查给定字符串是否可以转换为**平衡括号序列**。

> 一个**平衡括号序列**是每个开括号 `(` 有对应的闭括号 `)` 的序列。
...
**方法:** 使用栈可以解决上述问题。想法是检查所有 `)` 是否可以与 `(` 或 `$` 平衡，反之亦然。按照以下步骤解决此问题:

*   存储 `(`、`)`、`$` 的频率，分别出现在变量 `countOpen`、`countClosed` 和 `countSymbol` 中。
*   将变量 `ans` 初始化为 `1` 以存储所需结果，并初始化一个堆栈 `stack_1` 以检查所有 `)` 是否可以与 `(` 或 `$` 平衡。
*   使用变量 `i` 遍历字符串 `S`，并执行以下操作:
    *   如果当前字符 `S[i]` 为 `)`，如果 `stack_1` 为空，则将 `ans` 设置为 `0`，否则从 `stack_1` 弹出字符。
    *   否则将字符 `S[i]` 推至 `stack_1`。
*   反转 `S`，按照同样的程序检查所有 `(` 是否能与 `)` 或 `$` 平衡。
*   如果 `countSymbol` 的值小于 `countOpen` 和 `countClosed` 的绝对差值，则将 `ans` 设置为 `0`。否则用符号平衡多余的括号。平衡后，如果 `countSymbol` 为奇数，则将 `ans` 设置为 `0`。
