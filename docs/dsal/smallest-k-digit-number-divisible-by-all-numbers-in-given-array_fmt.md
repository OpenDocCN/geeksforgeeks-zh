...
> 原文: [https://www.geeksforgeeks.org/smallest-k-digit-number-divisible-by-all-numbers-in-given-array/](https://www.geeksforgeeks.org/smallest-k-digit-number-divisible-by-all-numbers-in-given-array/)

给定数组 `arr[]`。任务是创建最小的 `K` 位数，可被 `arr[]` 的所有数字整除。
...
*   求 `arr[]` 所有[阵元](https://www.geeksforgeeks.org/lcm-of-given-array-elements/)的 `LCM`。
*   找到具有 `K` 位数的 `LCM` 的倍数。
*   第一个有 `K` 位的数字将是最终答案。
*   最后，返回答案。
