...
第二种方法使用哈希处理来获得结果，并在较短的`O(n^2)`上求解。
...
该方法使用排序来得出正确的结果，并以 `O(n^2)` 的时间求解。
...
*   **辅助空间**：`O(n)`。
    由于需要一个哈希映射，因此空间复杂度是线性的。

## 算法

上述方法需要额外的空间。该想法基于此帖子的[方法 2](https://www.geeksforgeeks.org/find-a-triplet-that-sum-to-a-given-value/)。对于每个元素，检查是否存在一对总和等于该元素的负值的对。
...
6.  如果总和大于零，则 `r` 的值递减，通过减少 `r` 的值，总和将随着对数组进行排序而减小，因此 `array[r-1] < array[r]`。
