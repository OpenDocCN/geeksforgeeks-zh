...
1.  从头到尾遍历给定列表，并将每个访问的节点压入栈。
2.  再次遍历列表。对于每个访问的节点，从栈中弹出一个节点，并将弹出的节点的数据与当前访问的节点进行比较。
...
当节点数为偶数时，前一半和后一半恰好包含一半节点。这种方法的挑战在于处理节点数为奇数的情况。我们不希望中间节点成为任何列表的一部分，因为我们将比较它们是否相等。对于奇数的情况，我们使用单独的变量`midnode`。
