...
> 原文：[https://www.geeksforgeeks.org/program-to-calculate-ex-by-recursion/](https://www.geeksforgeeks.org/program-to-calculate-ex-by-recursion/)
...
我们知道 `n` 个自然数的和 = `n*(n+1)/2`，其阶为 `O(n^2)`。

因此，如果该方法为 `O(n^2)`，则时间复杂度为 `O(n^2)`。
