...
> 原文: [https://www.geeksforgeeks.org/largest-smaller-number-possible-using-one-swap-operation/](https://www.geeksforgeeks.org/largest-smaller-number-possible-using-one-swap-operation/)
...
[推荐:请先在“<u>”上解，再进行解。](https://practice.geeksforgeeks.org/problems/previous-number-in-one-swap/0)

## 算法步骤

1.  从右边开始遍历，找到一个大于右边数字之一的数字 `is`。让这个索引这样的元素成为索引。
2.  然后在 `str[index]` 的右边找到另一个索引，它的最大值小于 `str[index]`。
3.  交换上面的两个值。
