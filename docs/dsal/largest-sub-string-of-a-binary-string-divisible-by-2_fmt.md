...
> 原文：[https://www.geeksforgeeks.org/largest-sub-string-of-a-binary-string-divisible-by-2/](https://www.geeksforgeeks.org/largest-sub-string-of-a-binary-string-divisible-by-2/)
...
天真的方法是生成所有这样的子字符串，并检查它们是否能被 2 整除。这种方法的时间复杂度为 `O(N^3)`。

## 更好的方法

一个简单的方法是从字符串末尾删除字符，而最后一个字符是 `1`。当遇到 `0` 时，当前字符串将被 `2` 整除，因为它在 `0` 结束。这种方法的时间复杂度将是 `O(N)`。
