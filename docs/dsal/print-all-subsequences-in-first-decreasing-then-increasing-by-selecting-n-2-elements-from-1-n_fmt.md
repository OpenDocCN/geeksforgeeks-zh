# 通过从 1 到 N 中选择 N/2 个元素，以先减后增的方式打印所有子序列

> 原文: [https://www.geeksforgeeks.org/print-all-subsequences-in-first-decreasing-then-increasing-by-selecting-n-2-elements-from-1-n/](https://www.geeksforgeeks.org/print-all-subsequences-in-first-decreasing-then-increasing-by-selecting-n-2-elements-from-1-n/)

给定一个正整数 `N`，任务是通过从 `1` 到 `N` 中选择 `ceil(N/2)` 个元素，以先减后增的方式打印[数组](https://www.geeksforgeeks.org/array-data-structure/)的所有子序列。
...
**方法:** 该方法基于使用 Python 的内置函数 [`itertools.permutations`](https://www.geeksforgeeks.org/python-itertools-permutations/) 来生成大小为 `ceil(N/2)` 的所有子序列。按照以下步骤解决问题:

*   初始化一个[数组](https://www.geeksforgeeks.org/array-data-structure/) `arr[]`，插入从 `1` 到 `N` 的所有元素。
*   将变量 `K` 初始化为 `ceil(N/2)`。
*   使用名为 [`itertools.combinations`](https://www.geeksforgeeks.org/itertools-combinations-module-python-print-possible-combinations/) 的内置函数插入数组 `arr` 中的所有子序列。
*   [使用变量 `seq` 迭代数组](https://www.geeksforgeeks.org/iterating-arrays-java/) `sequences`，并执行以下步骤:
    *   检查 `seq[1] > seq[0]` 或 `seq[-1] < seq[-2]` 或 `seq` 是增加还是减少，然后继续，否则该序列不满足上述条件。
    *   如果元素多于 `seq[i] < seq[i-1]` 和 `seq[i] < seq[i+1]` 那么也继续，不打印数组。
    *   如果 `seq` 没有遵循以上任何一点，则打印 `seq`。
...
**时间复杂度:** O(C(N, ceil(N/2)) * ceil(N/2)! * N)
**辅助空间:** O(C(N, ceil(N/2)) * ceil(N/2)!)