...
> 原文:[https://www.geeksforgeeks.org/check-if-any-permutation-of-a-given-string-is-lexicographically-larger-than-the-other-given-string/](https://www.geeksforgeeks.org/check-if-any-permutation-of-a-given-string-is-lexicographically-larger-than-the-other-given-string/)

给定两个相同长度的字符串 `str1` 和 `str2`，任务是检查在任何给定的字符串中是否存在任何可能的排列，使得一个字符串的每个字符在相应的索引处大于或等于另一个字符串的每个字符。如果存在置换，则返回 `true`，否则返回 `false`。

**示例:**

> **输入:** `str1 = "ADB"`, `str2 = "CDA"`
> **输出:** `true`
> **解释:** 在置换 `str1 = "ABD"` 和 `str2 = "ACD"` 之后，所以 `str2` 中的每个字符都大于或等于 `str1` 中的每个字符。
>
> **输入:** `str1 = "gfg"`, `str2 = "AGD"`
> **输出:** `true`
...
*   将给定字符串转换为字符数组
*   对两个字符数组进行排序
*   现在，遍历这些数组，检查一个数组中的每个字符是否大于或等于另一个数组中的相应字符
*   如果发现词典较大，打印为 `true`，否则为 `false`。
...
*   为两个给定的字符串制作频率图
*   创建变量 `count1` 和 `count2` 以指示各个字符串的累计频率
*   遍历频率图，检查任何字符串的值是否大于另一个。
*   如果是，则打印 `true`。否则打印为 `false`。
