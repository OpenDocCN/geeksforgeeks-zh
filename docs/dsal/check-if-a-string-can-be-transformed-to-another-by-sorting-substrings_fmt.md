...
给定两个字符串`str1`和`str2`，每个字符串的长度`N`，并且仅由小写英文字母组成，任务是检查字符串`str1`通过多次执行以下操作，可以将转换为字符串`str2`。

*   在`str1`中选择一个非空的子字符串，按字典顺序原地排序，以不降序排列子字符串的字符。
...
*   遍历字符串`str2`，对于每个字符，检查`str1`中的相同字符是否可以移到该位置。
