...
给定一个数组，对于每个元素，找到最右边的元素的值，该元素的频率大于当前元素的频率。如果不存在答案，则将值设为`-1`。
...
一种简单的哈希技术是使用值作为索引来存储每个元素的频率。假设创建一个列表，以存储数组中每个数字的频率（需要单遍历）。现在使用两个循环。
...
3.  逐个拾取其余元素的位置，然后循环执行以下步骤。
    1.  将当前元素的位置标记为`i`。
    2.  如果栈顶部指向的元素的频率比当前元素的频率高，则将当前位置`i`推入栈。
    3.  如果栈顶部指向的元素的频率比当前元素的频率小，并且栈不为空，则请执行以下步骤：
        1.  继续弹出栈。
        2.  如果步骤`c`中的条件失败，则将当前位置`i`推入栈。
4.  步骤 3 中的循环结束后，从栈中弹出所有元素并将`-1`打印，因为不存在下一个更高频率元素。
