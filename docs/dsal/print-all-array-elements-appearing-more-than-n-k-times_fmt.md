...
> 原文: [https://www.geeksforgeeks.org/print-all-array-elements-appearing-more-than-n-k-times/](https://www.geeksforgeeks.org/print-all-array-elements-appearing-more-than-n-k-times/)

给定一个大小为 `N` 的数组 `arr[]` 和一个整数 `K`，任务是找出出现次数超过 `(N / K)` 次的所有数组元素。
...
解决这个问题最简单的方法是遍历数组，对于每个不同的数组元素，统计其频率，检查是否超过 `N / K`。如果发现为真，则打印数组元素。
...
使用二分搜索法手法可以解决问题。其思想是遍历数组，通过计算数组元素的上限来统计每个不同数组元素的频率。最后检查阵元频率是否大于 `N / K`。如果发现为真，则打印数组元素。按照以下步骤解决问题:

*   排序数组，`arr[]`。
*   使用变量 `i` 遍历数组，找到 `arr[i]` 的上界，说 `X`，检查 `(X - i)` 是否大于 `N / K`。如果发现为真，则打印 `arr[i]`。
*   最后更新 `i = X`。
...
- 使用 [`Counter()`](https://www.geeksforgeeks.org/python-counter-objects-elements/) 功能计算每个元素的频率。
