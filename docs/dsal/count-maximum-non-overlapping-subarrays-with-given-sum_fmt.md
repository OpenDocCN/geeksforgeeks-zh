...
> 原文：[https://www.geeksforgeeks.org/count-maximum-non-overlapping-subarrays-with-given-sum/](https://www.geeksforgeeks.org/count-maximum-non-overlapping-subarrays-with-given-sum/)

给定一个由 `N` 个整数和一个整数 `目标` 组成的数组 `arr[]`，任务是找到最大数量的非空非重叠子数组，使得每个子数组中的数组元素之和等于 `目标`。
...
**方法:** 为了获得具有和 `目标` 的最小不重叠子阵列，目标是使用前缀和技术。按照以下步骤解决问题：

1.  将到目前为止计算的所有和存储在一个映射 `mp` 中，键作为前缀的和，直到该索引和值作为具有该和的子阵列的结束索引。
2.  如果前缀和直到索引 `i`，比如 `sum`，等于 `目标`，检查 `sum - 目标` 是否存在于映射中。
3.  如果映射 `mp` 中存在 `sum - 目标` 且 `mp[sum - 目标] = idx`，则表示来自 `[idx + 1, i]` 的子阵具有等于 `目标` 的 `sum`。
4.  现在对于不重叠的子阵列，维护一个附加变量 `availIdx`（初始设置为 -1），只有当 `mp[sum - 目标] >= availIdx` 时，才能从 `[idx + 1, i]` 获取子阵列。
5.  每当找到这样的子阵列时，增加答案并将 `availIdx` 的值变为当前索引。
6.  同样，对于不重叠的子阵列，贪婪地将子阵列取尽可能小总是有益的。因此，对于找到的每个前缀和，更新其在地图中的索引，即使它已经存在。
7.  完成上述步骤后，打印 `计数` 的值。
