...
> 原文: [https://www.geeksforgeeks.org/print-binary-tree-vertical-order/](https://www.geeksforgeeks.org/print-binary-tree-vertical-order/)
...
上述算法的时间复杂度为 `O(w*n)`，其中 `w` 为二叉树的宽度，`n` 为二叉树中的节点数。在最坏的情况下，`w` 的值可以是 `O(n)`（例如考虑一个完整的树），时间复杂度可以变成 `O(n^2)`。
