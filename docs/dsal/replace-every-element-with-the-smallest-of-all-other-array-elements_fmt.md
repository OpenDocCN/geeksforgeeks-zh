...
> 原文: [https://www.geeksforgeeks.org/replace-every-element-with-the-smallest-of-all-other-array-elements/](https://www.geeksforgeeks.org/replace-every-element-with-the-smallest-of-all-other-array-elements/)
...
**高效方法:**
想法是保持前缀和后缀最小数组。维护 `leftMin[]` 和 `rightMin[]` 数组，该数组存储每个数组元素的左子数组和右子数组的最小值。一旦计算完成，通过存储最小的 `leftMin[i]` 和 `rightMin[i]` 来替换原始数组的每个 `i` 索引。
