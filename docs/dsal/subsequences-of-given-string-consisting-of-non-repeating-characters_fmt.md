...
> 原文: [https://www.geeksforgeeks.org/subsequences-of-given-string-consisting-of-non-repeating-characters/](https://www.geeksforgeeks.org/subsequences-of-given-string-consisting-of-non-repeating-characters/)
...
*   初始化一个[集合](https://www.geeksforgeeks.org/set-in-cpp-stl/)，比如 `sub`，来存储所有可能的由非重复字符组成的子序列。
...
def FindSub(sub, ch, str, res, i):
    ...
    if (str[i] not in ch):
        ...
        # Insert str[i] into the
        # current subsequence
        FindSub(sub, ch, str, res+str[i], i + 1)
        res += str[i]
...
# Utility function to print all subsequences
# of string with non-repeating characters
def printSubwithUniqueChar(str, N):
