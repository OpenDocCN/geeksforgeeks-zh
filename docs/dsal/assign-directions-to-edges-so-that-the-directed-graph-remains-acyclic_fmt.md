...
> 原文：`https://www.geeksforgeeks.org/assign-directions-to-edges-so-that-the-directed-graph-remains-acyclic/`
...
思路是用`拓扑排序`。以下是算法中使用的两个步骤。

**1)** 只考虑有向边的子图，求子图的`拓扑排序`。在上面的例子中，`拓扑排序`是`{0，5，1，2，3，4}`。下图显示了上述示例图的`拓扑排序`。
[![second](img/b6fedf12a50179f05abf289ffef56197.png)](https://media.geeksforgeeks.org/wp-content/cdn-uploads/second.png)

**2)** 使用上述`拓扑排序`为无向边指定方向。对于每条无向边`(u，v)`，如果在`拓扑排序`中 `u` 在 `v` 之前，则为其指定从 `u` 到 `v` 的方向，否则为其指定从 `v` 到 `u` 的方向。
下图显示了示例图中的指定方向。
[![third](img/16b7cd2d083772f2fccb3cea4f4252b0.png)](https://media.geeksforgeeks.org/wp-content/cdn-uploads/third.png)

来源：`http://courses.csail.mit.edu/6.006/oldquizzes/solutions/q2-f2009-sol.pdf`
