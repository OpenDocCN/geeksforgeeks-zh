# C++标准库中的`uniform_int_distribution`类

> 原文：[https://www.geeksforgeeks.org/stduniform_int_distribution-class-in-c/](https://www.geeksforgeeks.org/stduniform_int_distribution-class-in-c/)
...
C++在`<random>`库中引入了`uniform_int_distribution`类，其成员函数以均匀的概率给出来自给定输入范围的随机整数或离散值。

## `uniform_int_distribution`类中的公共成员函数

### 1. `operator()`
该函数返回给定分布范围内的随机数。从这个函数中得到任何数的概率都是一样的。`operator()`函数需要恒定的生成时间。
...
### 2. `a()`
返回范围的下限参数。这指定了其成员`operator()`可能返回的值范围的下限。

### 3. `b()`
返回范围的较高参数。这指定了其成员`operator()`可能返回的值范围的上限。

### 4. `max()`
此函数返回`operator()`函数可能输出的最小上界。

### 5. `min()`
此函数返回`operator()`函数可能输出的最高下限。

### 6. `reset()`
此功能重置分布，使得后续分布不依赖于之前生成的数字。
...
**参考：**[https://en.cppreference.com/w/cpp/numeric/random/uniform_int_distribution](https://en.cppreference.com/w/cpp/numeric/random/uniform_int_distribution)