...
> 原文: [https://www.geeksforgeeks.org/check-whether-an-array-can-be-made-strictly-decreasing-by-modifying-at-most-one-element/](https://www.geeksforgeeks.org/check-whether-an-array-can-be-made-strictly-decreasing-by-modifying-at-most-one-element/)
...
对于每个元素 `arr[i]`，如果大于 `arr[i-1]` 和 `arr[i+1]` 或者小于 `arr[i-1]` 和 `arr[i+1]`，那么需要修改 `arr[i]`。
即 `arr[i] = (arr[i-1] + arr[i+1]) / 2`。如果修改后，`arr[i] = arr[i-1]` 或 `arr[i+1]`，则数组不能严格递减而不影响**最多**一个元素，否则计算所有这样的修改，如果最后修改的计数小于或等于 `1` 则打印**是**，否则打印**否**。
