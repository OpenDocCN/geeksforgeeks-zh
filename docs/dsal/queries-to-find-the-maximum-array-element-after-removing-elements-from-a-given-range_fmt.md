...
> 原文: [https://www.geeksforgeeks.org/queries-to-find-the-maximum-array-element-after-removing-elements-from-a-given-range/](https://www.geeksforgeeks.org/queries-to-find-the-maximum-array-element-after-removing-elements-from-a-given-range/)

给定一个由形式为 `{L, R}` 的查询组成的数组 `arr[]` 和一个数组 `Q[][]`，每个查询的任务是在从索引范围 `[L, R]` 中移除数组元素后找到最大数组元素。如果从给定范围的索引中删除元素后数组变为空，则打印 `0`。
...
**天真法:** 最简单的方法是遍历数组，在每个查询中移除给定范围内的数组元素后，找到最大元素。

**时间复杂度:** `O(N*Q)`
**辅助空间:** `O(1)`

**高效方式:** 优化上述方式，思路是使用两个辅助数组: 一个存储前缀最大值（范围 `[0, i]` 中的最大元素），另一个存储后缀最大值（范围 `[i, N–1]` 中的最大元素）。那么对于每个查询 `[L, R]`，答案将是 `prefixMax[L–1]` 和 `suffixMax[R+1]` 的最大值。按照以下步骤解决问题:

*   初始化两个辅助数组: `prefixMax` 和大小为 `N + 1` 的 `suffixMax`，并将 `prefixMax[0]` 初始化为 `arr[0]`，将 `suffixMax[N–1]` 初始化为 `arr[N–1]`。
*   使用变量 `i` 迭代范围 `[1, N–1]`，并将 `prefixMax[i]` 设置为 `prefixMax[i–1]` 和 `arr[i]` 的最大值。
*   使用变量 `i` 迭代范围 `[N–2, 0]`，并将 `suffixMax[i]` 设置为 `suffixMax[i + 1]` 和 `arr[i]` 的最大值。
*   遍历数组 `Q[][]`，对于每个查询，检查以下条件:
    *   如果 `L = 0` 且 `R = N–1`，则打印 `0` 作为结果。
    *   否则，如果 `L = 0`，则打印 `suffixMax[R+1]` 作为结果。
    *   否则，如果 `R = N–1`，则打印 `prefixMax[L–1]` 作为结果。
    *   对于所有其他情况，打印 `prefixMax[L–1]` 和 `suffixMax[R+1]` 的最大值。
