...
> 原文: [https://www.geeksforgeeks.org/xor-sum-every-possible-pair-array/](https://www.geeksforgeeks.org/xor-sum-every-possible-pair-array/)
...
一种有效的方法基于相同值的异或为 0 的事实。像 `(a[i]、a[j])` 和 `(a[j]、a[i])` 这样的所有对将具有相同的和。所以，它们的异或值将为 0。只有像 `(a[i]，a[i])` 这样的配对才会给出不同的结果。所以，对给定数组的所有元素进行异或运算，然后乘以 2。
