...
给定一个整数 `n`，检查数字 `n` 是否为超完美数字。超完美数是满足 `σ^2(n) = σ(σ(n)) = 2n` 的正整数，其中 `σ` 是[除数求和](https://en.wikipedia.org/wiki/Divisor_summatory_function)函数。
...
1.  如果 `n` 是偶数超完全数，那么 `n` 必须是 2 的幂，即 `2^k`，这样 `2^(k+1)–1` 就是一个[梅森素数](https://en.wikipedia.org/wiki/Mersenne_prime)。
2.  不知道是否有奇数个超完美数。奇数超完美数 `n` 必须是平方数，这样 `n` 或 `σ(n)` 至少可以被三个不同的素数整除。`7×10^24` 以下没有奇数个超完美数。
