...
> 原文: [https://www.geeksforgeeks.org/print-all-internal-nodes-of-a-binary-tree/](https://www.geeksforgeeks.org/print-all-internal-nodes-of-a-binary-tree/)

给定一个二叉树，任务是打印树中的所有内部节点。内部节点是携带至少一个子节点的节点，或者换句话说，内部节点不是叶节点。这里我们打算按级别顺序打印所有这样的内部节点。考虑以下二叉树:
...
*   通过逐个推送`队列`中的节点来执行级别顺序遍历。
*   逐个弹出`队列`中的元素，并跟踪以下情况:
    1.  该节点只有一个左子节点。
    2.  该节点只有一个右子节点。
    3.  该节点既有左子节点也有右子节点。
    4.  该节点根本没有子节点。
