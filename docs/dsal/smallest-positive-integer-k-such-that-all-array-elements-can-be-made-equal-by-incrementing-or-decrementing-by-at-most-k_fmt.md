# 最小正整数 K，通过增加或减少最多 K 个可以使所有数组元素相等

> 原文: [https://www.geeksforgeeks.org/smallest-positive-integer-k-such-that-all-array-elements-can-be-made-equal-by-incrementing-or-decrementing-by-at-most-k/](https://www.geeksforgeeks.org/smallest-positive-integer-k-such-that-all-array-elements-can-be-made-equal-by-incrementing-or-decrementing-by-at-most-k/)

给定一个大小为 `N` 的数组 `arr[]`，任务是找到最小正整数 `K`，使得将每个数组元素递增或递减 `K` 至多一次使所有元素相等。如果无法使所有数组元素相等，则打印 `-1`。
...
*   初始化一个集合来存储数组中存在的所有不同元素。
*   计算数组中不同的元素，等于集合的大小，比如 `M`。
*   如果 `M > 3`，则打印 `-1`。
