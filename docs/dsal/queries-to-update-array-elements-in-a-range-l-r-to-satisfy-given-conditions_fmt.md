...
> 原文: [https://www.geeksforgeeks.org/queries-to-update-array-elements-in-a-range-l-r-to-satisfy-given-conditions/](https://www.geeksforgeeks.org/queries-to-update-array-elements-in-a-range-l-r-to-satisfy-given-conditions/)

给定一个由 `N` 个整数和一个数组 `Q[][]` 组成的数组 `arr[]`，每一行的形式为 `(L，R)`。每个查询的任务是更新范围 `[L，R]` 内的所有数组元素，使得 `arr[i] = i - L + 1`。

## 示例

**输入:**
`arr[] = { 0, 0, 0, 0 }`, `Q[][] = { { 1, 2 }, { 0, 1 } }`
**输出:**
`1 3 2 0`
**解释:**
查询 1: 更新 `arr[1] = 1 - 1 + 1`, `arr[2] = 2 - 1 + 1`，将 `arr[]` 修改为 `{ 0, 1, 2, 0 }`
查询 2: 更新 `arr[0] = 0 - 0 + 1`, `arr[1] = 1 - 0 + 1`，将 `arr[]` 修改为 `{ 1, 3, 2, 0 }`

**输入:**
`arr[] = { 0, 0, 0, 0 }`, `Q[][] = { { 1, 3 }, { 0, 1 } }`
**输出:**
`1 3 2 3`

## 天真方法

解决问题最简单的方法是遍历数组 `Q[][]`。对于每个查询，遍历给定范围内的所有数组元素并更新 `arr[i] += i - L + 1`。最后，打印数组元素。
...
上述方法可以使用差分数组的概念进行优化。按照以下步骤解决问题:

*   初始化两个数组 `arr1[]` 和 `arr2[]`，并将所有元素初始化为 `0`。
*   遍历查询数组 `Q[][]`。对于类型为 `(L，R)` 的每个查询，更新 `arr1[L] += 1`、`arr1[R + 1] -= 1` 和 `arr2[R + 1] -= R - L + 1`。
*   遍历数组 `arr1[]`，存储 `arr1[]` 的前缀和，即 `arr1[i] += arr1[i - 1]`。
*   遍历数组 `arr2[]` 并更新 `arr2[i] += arr2[i - 1] + arr1[i]`。
*   最后打印数组 `arr2[]`。
