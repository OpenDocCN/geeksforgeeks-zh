...
> 原文:[https://www.geeksforgeeks.org/sum-of-main-diagonal-elements-in-a-matrix-which-are-prime/](https://www.geeksforgeeks.org/sum-of-main-diagonal-elements-in-a-matrix-which-are-prime/)

## 问题陈述

给定一个由 `R` 行和 `C` 列组成的矩阵 `mat[][]`。任务是从主对角线找到所有元素的和，这些元素是[质数](https://www.geeksforgeeks.org/prime-numbers/)。
...
我们可以通过优化数字的[初级度测试来优化朴素方法。以下是优化初级测试的步骤:](https://www.geeksforgeeks.org/primality-test-set-1-introduction-and-school-method/)
...
3.  因为 `2` 除`(6k + 0)`，`(6k + 2)`，`(6k+4)`；和 `3` 除`(6k + 3)`。所以更有效的方法是测试 `N` 是否能被 `2` 或 `3` 整除，然后检查表格 `6k ± 1` 的所有数字。
