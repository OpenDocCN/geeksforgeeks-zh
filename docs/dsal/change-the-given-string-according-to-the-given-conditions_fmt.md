...
> 原文: [https://www.geeksforgeeks.org/change-the-given-string-according-to-the-given-conditions/](https://www.geeksforgeeks.org/change-the-given-string-according-to-the-given-conditions/)

给定一个字符串 `S`，任务是更改字符串，使其遵循下面给出的规则，并打印更新后的字符串。校对的规则是：

1.  如果有三个连续的字符相同，那么它是一个错误的拼写。删除其中一个字符。**比如**字符串 `"oooops"` 可以改成 `"oops"`。
2.  如果两对相同的字符（AABB）连接在一起，这是一个错误的拼写。删除第二对字符中的一个。**比如**字符串 `"hheelllloo"` 可以改成 `"hheelllo"`。
3.  规则遵循从左到右的优先级。
