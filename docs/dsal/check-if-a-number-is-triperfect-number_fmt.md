...
> 原文: [https://www.geeksforgeeks.org/check-if-a-number-is-triperfect-number/](https://www.geeksforgeeks.org/check-if-a-number-is-triperfect-number/)

给定一个数字`N`。任务是检查给定的数字是否是三分位数。

## 三分位数

一个数等于其除数之和的三倍，也就是其正除数之和的三倍，就是三分位数。
