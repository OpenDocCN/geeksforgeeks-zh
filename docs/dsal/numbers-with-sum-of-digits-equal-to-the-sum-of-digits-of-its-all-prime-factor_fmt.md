...
> 原文：[https://www.geeksforgeeks.org/numbers-with-sum-of-digits-equal-to-the-sum-of-digits-of-its-all-prime-factor/](https://www.geeksforgeeks.org/numbers-with-sum-of-digits-equal-to-the-sum-of-digits-of-its-all-prime-factor/)
...
4.  然后通过将最小质因数的第个数字相加，即 `sum_digits[n] = sum_digits[n / spf[n]] + Digit_Sum(spf[n])`，检查该数字的位数是否等于质因数的位数位数 _ SPF 之和[n] +位数 _ SPF 之和(不适用)
5.  现在制作前缀和数组，计算一个数字 n 的有效数字。对于每个查询，打印：`ans[r] - ans[l-1]`
