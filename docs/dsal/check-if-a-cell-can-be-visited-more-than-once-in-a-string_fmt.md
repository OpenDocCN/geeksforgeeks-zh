...
> 原文: [https://www.geeksforgeeks.org/check-if-a-cell-can-be-visited-more-than-once-in-a-string/](https://www.geeksforgeeks.org/check-if-a-cell-can-be-visited-more-than-once-in-a-string/)

给定由`.`组成的字符串或者任何数字。一个`.`在字符串中表示单元格为空，如果任意单元格中有数字`x`，则表示可以在字符串中向右或向左移动`x`步。
...
其思想是取一个数组`visited[]`来记录字符串的第`i`个单元格可以被访问的次数。现在遍历字符串并检查当前字符是`.`或者一个数字`x`。如果当前字符是`.`，则不做任何事情；如果它是一个数字，则将`[i-x, i+x]`范围内的访问数组中的访问次数增加1。
最后，遍历`visited[]`数组，检查是否有任何单元格被访问过不止一次。
