...
> 原文: [https://www.geeksforgeeks.org/find-nth-positive-number-whose-digital-root-is-x/](https://www.geeksforgeeks.org/find-nth-positive-number-whose-digital-root-is-x/)
...
简易法就是从 1 开始计算每个数字的`数字根`，每当我们遇到一个`数字根`等于 `X` 的数字，我们就把计数器加 1。当我们的计数器等于 `n` 时，我们将停止搜索。
...
我们可以直接使用公式找到数字 `K` 的`数字根`:

```
digitalRoot(k) = (k - 1) mod 9 + 1
```

由此我们可以找到`数字根`为 `K` 的第 `N` 个数为，
