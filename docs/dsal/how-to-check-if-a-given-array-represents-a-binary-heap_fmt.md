...
> 原文：[https://www.geeksforgeeks.org/how-to-check-if-a-given-array-represents-a-binary-heap/](https://www.geeksforgeeks.org/how-to-check-if-a-given-array-represents-a-binary-heap/)
...
一个简单的解决方法是首先检查根是否大于它的所有后代。然后检查根的孩子。这个解的时间复杂度为 `O(n^2)`。

## 高效解法

一个高效解是只比较根与其子代（不是所有的后代），如果根大于其子代，并且所有节点都是如此，那么树就是 max-heap（这个结论是基于 `>` 算子的传递性，即如果 `x > y` 和 `y > z`，那么 `x > z`）。

假设索引从 0 开始，最后一个内部节点出现在索引 `(n-2)/2` 处。下面是这个解决方案的实现。
...
一个迭代解是遍历所有内部节点，检查 `i` 节点是否大于其子节点。
