...
> 原文: [https://www.geeksforgeeks.org/railway-station-tcs-codevita-2020/](https://www.geeksforgeeks.org/railway-station-tcs-codevita-2020/)
...
> 1 号列车(出发地)—>2 号列车(出发地 2 号车站)—>目的地
> 1 号列车(出发地)—>3 号列车(出发地 2 号车站)—>目的地
>
> **输入:** `N = 0`
> **输出:** `1`
...
解决问题的主要思路是用[递归](https://www.geeksforgeeks.org/recursion/)配合[记忆](https://www.geeksforgeeks.org/memoization-1d-2d-and-3d/)来解决这个问题。循环关系如下:

> **F(N)= F(N–1)+F(N–2)+F(N–3)，**
>
> 其中，
> `F(N–1)` 计算到达第 `(N–1)` 站的方式。
> `F(N–2)` 计算到达第 `(N–2)` 站的行程。
> `F(N–3)` 计算到达第 `(N–3)` 站的行程。

按照以下步骤解决问题:

1.  初始化数组 `dp[]` 进行记忆。最初将所有指数设置为 `-1`。
2.  定义一个[递归函数](https://www.geeksforgeeks.org/recursive-functions/) `findWays()` 来计算到达第 `N` 站的路数。
3.  需要考虑以下基本情况:
    *   对于 `x < 0` 返回 `0`。
    *   对于 `x = 0`，返回 `1`。
    *   对于 `x = 1`，返回 `2`。
...
5.  否则，递归计算 `findWays(x–1)`、`findWays(x–2)` 和 `findWays(x–3)`，并将它们的和存储在 `dp[x]` 中。
6.  返回 `dp[x]`。
