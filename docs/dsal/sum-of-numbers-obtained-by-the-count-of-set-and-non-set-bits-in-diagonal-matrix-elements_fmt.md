...
> 原文：[https://www.geeksforgeeks.org/sum-of-numbers-obtained-by-the-count-of-set-and-non-set-bits-in-diagonal-matrix-elements/](https://www.geeksforgeeks.org/sum-of-numbers-obtained-by-the-count-of-set-and-non-set-bits-in-diagonal-matrix-elements/)
...
> `5 = (0101)₂`
> `9 = (1001)₂`
> 在比特位置 0，设置比特数(=0) < 未设置比特数(=3)
> 在比特位置 1，设置比特数(=1) < 未设置比特数(=2)
> 在比特位置 2，设置比特数(=1) < 未设置比特数(=2)
> 在比特位置 3，设置比特数(=1) < 未设置比特数(=2)
> 因此，在处理主对角线之后，生成的数字是 `(0001)₂ = 1`。
> 对于次对角线，每个元素的二进制表示为：
> `3 = (011)₂`
> `5 = (101)₂`
> `7 = (111)₂`
> 在比特位置 0，设置比特数(=3) > 非设置比特数(=0)
> 在比特位置 1，设置比特数(=2) > 非设置比特数(=1)
> 在比特位置 2，设置比特数(=3) > 非设置比特数(=0)
> 因此，生成的数字是 `(111)₂ = 7`。
> 所需的总和 = `1 + 7 = 8`。
>
> **输入：** `mat[][] = [[2, 3], [3, 9]]`
