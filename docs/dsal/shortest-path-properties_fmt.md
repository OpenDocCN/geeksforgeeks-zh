...
*   上面列出的所有算法都基于这个属性。
*   例如，让 `P1` 成为从图 `G` 的【最短路径】 `(S →X →Y → V)` 的 `(X → Y)` 的子路径。并让 `P2` 成为图形 `G` 中的任意其他路径 `(X → Y)`。那么，`P1` 的成本必须小于或等于 `P2` 的成本。否则路径 `(S →X →Y → V)` 将不是节点 `S` 和 `V` 之间的最短路径。![](img/fb7c7f235b4195eb5ebd3b0b2e1745b6.png)
