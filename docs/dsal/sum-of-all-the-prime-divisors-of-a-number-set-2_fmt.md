...
> 原文：[https://www.geeksforgeeks.org/sum-of-all-the-prime-divisors-of-a-number-set-2/](https://www.geeksforgeeks.org/sum-of-all-the-prime-divisors-of-a-number-set-2/)
...
这个问题可以通过找到这个数的所有[素因子](https://www.geeksforgeeks.org/print-all-prime-factors-of-a-given-number/)来解决。按照以下步骤解决此问题：

*   将变量 `sum` 初始化为 `0`，以存储 `N` 的素数之和。
*   如果 `N` 被 `2` 整除，将 `2` 加到 `sum` 上，再将 `N` 除以 `2` 直到整除。
*   [使用变量 `i`，在范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[3, sqrt(N)]` 中迭代，增量为 `2`：
    *   如果 `N` 被 `i` 整除，将 `i` 加到 `sum` 上，再将 `N` 除以 `i` 直到整除。
*   如果 `N` 是大于 `2` 的素数，将 `N` 加到 `sum`。
*   完成以上步骤后，打印 `sum` 作为答案。
