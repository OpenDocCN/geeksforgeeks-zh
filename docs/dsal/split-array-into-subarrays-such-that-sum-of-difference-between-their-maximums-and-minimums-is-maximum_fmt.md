...
> 原文: [https://www.geeksforgeeks.org/split-array-into-subarrays-such-that-sum-of-difference-between-their-maximums-and-minimums-is-maximum/](https://www.geeksforgeeks.org/split-array-into-subarrays-such-that-sum-of-difference-between-their-maximums-and-minimums-is-maximum/)

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是将该数组拆分为子数组，使得所有子数组的最大元素和最小元素之差之和最大。
...
**方法:** 给定的问题可以用动态规划解决。按照以下步骤解决问题:

*   初始化一个数组，比如说 `dp[]`，其中 `dp[i]` 代表第一个 `i` 数组元素的所有子数组的最大和最小元素之差的最大和。
*   将 `dp[0]` 初始化为 `0`。
*   在范围 `[1, N - 1]` 内遍历给定数组，并执行以下步骤:
    *   初始化一个变量，说 `min` 为 `arr[i]`，该变量存储范围 `[0, i]` 内的最小元素。
    *   初始化一个变量，比如说 `max` 为 `arr[i]`，该变量存储范围 `[0, i]` 内的最大元素。
    *   使用变量 `j` 以相反的顺序迭代范围 `[0, i]`，并执行以下步骤:
        *   将 `min` 的值更新为 `min` 和 `arr[j]` 的最小值。
        *   将 `max` 更新为 `max` 和 `arr[j]` 的最大值。
        *   将 `dp[i]` 的值更新为 `dp[i]` 和 `max - min + dp[j - 1]` 的最大值。
