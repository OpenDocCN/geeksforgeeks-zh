...
> 原文：[https://www.geeksforgeeks.org/pairs-of-strings-which-on-concatenating-contains-each-character-of-string/](https://www.geeksforgeeks.org/pairs-of-strings-which-on-concatenating-contains-each-character-of-string/)
...
> **输入：** `arr[]= {“s”，“ng”，“stri”}`
> **输出：** 1
> `(arr[1]，arr[2])` 是在串联时唯一包含字符串“string”的每个字符的对，即 `arr[1]+arr[2]=“ngstri”`
...
将给定字符串存储为位掩码，即字符串 `"srin"` 将存储为 `101110`，因为 `t` 和 `g` 缺失，所以它们对应的位为 `0`。现在，创建一个大小为 `64` 的数组，这是获得的位掩码的最大可能值（`0`（`000000`）到 `63`（`111111`））。现在，问题简化为找到这些位掩码对的计数，这些位掩码将 `63`（二进制 `111111`）作为它们的或值。
