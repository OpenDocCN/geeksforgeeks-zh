...
`display_surface` 变量就像一张画布，我们将在上面添加实体(图像、文本等)按照要求。通过 `display.set_caption` 函数传递的参数将是模拟窗口的标题。首先要显示的图像——所有三扇门都已关闭，已加载。一个布尔变量，`change`，用于记录变化。功能 `music` 是可选的。它的功能是当用户按下一扇门时产生*点击*的声音。一定要有任何想要播放的音乐的 mp3 文件。该函数执行初始化、加载和播放三个步骤。
...
`show_car` 功能可能看起来有点麻烦，但是它的功能相当琐碎。它旨在显示适当方向的图像，即，如果汽车在门 3 后面，则显示对应于该方向(汽车在位置 3)的图像。所有的图片(以及代码中使用的名称)都在本文的末尾。`state` 参数告诉模拟的结果。
...
每个游戏都包含一个无限循环(当为真时为 `True`)。检查注册的事件类型是否为 `QUIT`，即用户关闭了 Pygame 窗口。在这种情况下，程序终止。`MOUSEBUTTONDOWN` 事件对应鼠标左键点击鼠标的事件。`if` 条件用于确定用户已按下的门(基于坐标)，门号被分配给变量 `user`。例如，如果发生点击的点的 X 坐标 (`event.pos[0]`) 在 71 和 203 之间，Y 坐标 (`event.pos[1]`) 在 387 和 633 之间，则登记点击在门 1 上有点击。单击布尔型 `clicked` 设置为真，只要单击三扇门中的一扇门。
