...
> 原文: [https://www.geeksforgeeks.org/sum-of-elements-from-an-array-having-even-parity/](https://www.geeksforgeeks.org/sum-of-elements-from-an-array-having-even-parity/)
...
**方法:** 初始化一个变量 `sum = 0` 并遍历数组从 `0` 到 `n-1`，同时使用[布莱恩·克尼根算法](https://www.geeksforgeeks.org/count-set-bits-in-an-integer/)计算 `arr[i]` 中的设置位数。如果 `count` 为`偶数`，则更新 `sum = sum + arr[i]`。最后打印 `sum`。
