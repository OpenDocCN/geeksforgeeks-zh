...
> 原文: [https://www.geeksforgeeks.org/sum-of-the-distances-from-every-node-to-all-other-nodes-is-maximum/](https://www.geeksforgeeks.org/sum-of-the-distances-from-every-node-to-all-other-nodes-is-maximum/)
...
*   要找出两边的节点数，找出 `dp[node1]` 或 `dp[node2]` 的较小值，其中 `node1` 和 `node2` 是边两边的节点。
*   如果一侧有 `min(dp[node1], dp[node2])`，那么另一侧就会有 `(N - min(dp[node1], dp[node2]))`。
