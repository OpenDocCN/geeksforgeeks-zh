...
> 原文：[https://www.geeksforgeeks.org/amazon-interview-experience-set-134-campus-sde/](https://www.geeksforgeeks.org/amazon-interview-experience-set-134-campus-sde/)
...
**第 2 轮:**
1. [给定 `m` 和 `n` (`m<n`)，按级别顺序打印 `m` 和 `n` 级别之间的所有节点。](https://practice.geeksforgeeks.org/problems/level-order-traversal-line-by-line/1)
...
**F2F–第 1 轮:**
1. [给定一个单链表和值 `k`，使得 `k < length of ll`，编写一个递归方法来反转列表中每 `k` 个节点。](https://practice.geeksforgeeks.org/problems/reverse-a-linked-list-in-groups-of-given-size/1)
    输入：`A -> B -> C -> D -> E`，`k=2`
    输出：`E->C->D->A->B`

    输入：`A->B->C->D->E->F`，`k=2`

2. 什么是寻呼？页面错误是什么意思？您将如何处理页面错误？

**F2F–第 2 轮:**
1. [给定一个 BST，在树中交换两个节点。识别两个交换的节点](https://practice.geeksforgeeks.org/problems/fixed-two-nodes-of-a-bst/1)。
...
3. 给出了一个数组，它的每个 `i` 索引都是 `[i]` 的子节点，如下例所示。根节点由 `-1` 表示。找到树的高度。我是在直线时间里做的。
