...
> 原文：[https://www.geeksforgeeks.org/sort-an-array-of-points-by-their-distance-from-a-reference-point/](https://www.geeksforgeeks.org/sort-an-array-of-points-by-their-distance-from-a-reference-point/)
...
思想是将每个元素存储在一个[对](https://www.geeksforgeeks.org/pair-in-cpp-stl/)中，包含距离给定点 `P` 的距离，然后根据存储的距离对[向量](https://www.geeksforgeeks.org/vector-in-cpp-stl/)的所有元素进行排序。
...
**时间复杂度：** 和上面的方法一样，有一个长度为 `N` 的数组的排序，在最坏的情况下需要 `O(N*logN)` 的时间。因此，时间复杂度将是 `O(N * logN)`。
