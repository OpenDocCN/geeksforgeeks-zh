...
> 原文：[https://www.geeksforgeeks.org/find-missing-elements-from-an-array/](https://www.geeksforgeeks.org/find-missing-elements-from-an-array/)

给定范围`【1，N】`的整数列表，其中缺少一些元素。任务是找到丢失的元素。注意列表中可以有重复项。
...
**方法：** 在给定的范围`【1，N】`应该有一个元素对应每个指标。如果一个元素丢失了，那么它的索引将永远不会被访问。
