...
> 原文: [https://www.geeksforgeeks.org/program-best-fit-algorithm-memory-management/](https://www.geeksforgeeks.org/program-best-fit-algorithm-memory-management/)

前提条件: [分区分配方式](https://www.geeksforgeeks.org/partition-allocation-methods-in-memory-management/)

`最佳匹配`将进程分配给一个分区，该分区是可用空闲分区中最小的分区。
...
3- Start by picking each process and find the
   ...
   current process i.e., find min(blockSize[1], 
...
虽然`Best-Fit`最小化了空间浪费，但是它消耗了大量处理器时间来搜索接近所需大小的块。此外，在某些情况下，`Best-Fit`的性能可能比其他算法差。例如，见下面的练习。
