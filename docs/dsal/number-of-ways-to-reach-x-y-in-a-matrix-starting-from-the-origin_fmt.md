...
> 原文: [https://www.geeksforgeeks.org/number-of-ways-to-reach-x-y-in-a-matrix-starting-from-the-origin/](https://www.geeksforgeeks.org/number-of-ways-to-reach-x-y-in-a-matrix-starting-from-the-origin/)
...
> **输入:** `X = 3`, `Y = 3`
> **输出:** 2
> 唯一可能的方式是 `(0, 0) -> (1, 2) -> (3, 3)` 和 `(0, 0) -> (2, 1) -> (3, 3)`
>
> **输入:** `X = 2`, `Y = 3`
> **输出:** 0

**接近:** 一次移动 `x` 坐标 + `y` 坐标的值增加 `3`。所以当 `X + Y` 不是 `3` 的倍数时，答案是 `0`。当 `(+1, +2)` 的移动次数为 `n`，`(+2, +1)` 的移动次数为 `m` 时，则 `n + 2m = X`，`2n + m = Y`。答案是 `0` 当 `n < 0` 或 `m < 0` 时。如果没有，答案是 `C(n+m, n)`，因为只需要决定 `n + m` 中的哪个 `n` 次移动 `(+1, +2)`。这个值可以通过 `O(n + m + log mod)` 计算阶乘及其倒数来计算。也可用 `O(min{n, m})` 计算。
