...
> 原文: [https://www.geeksforgeeks.org/nth-term-of-a-custom-fibonacci-series/](https://www.geeksforgeeks.org/nth-term-of-a-custom-fibonacci-series/)

给定三个整数 `A`、`B` 和 `N`。自定义斐波那契数列定义为 `F(x)= F(x–1)+F(x+1)`，其中 `F(1) = A`，`F(2) = B`，现在的任务是找到这个数列的 `N` 第项。
...
**进场:** 可以观察到该系列会像 `A、B、B–A、-A、-B、A–B、A、B、B–A、…` 一样进行下去。下图为上述进场的实施:
