...
> 原文：[https://www.geeksforgeeks.org/check-whether-n-can-be-a-perfect-cube-after-adding-or-subtracting-k/](https://www.geeksforgeeks.org/check-whether-n-can-be-a-perfect-cube-after-adding-or-subtracting-k/)
...
> **输入：** `N = 7`，`K = 1`
> **输出：** 是
> `7 + 1 = 8` 这是一个完美立方体（`2^3 = 8`）
> **输入：** `N = 5`，`K = 4`
> **输出：** 是
> `5–4 = 1` 这是一个完美立方体（`1^3 = 1`）

## 方法

解决这个问题最简单的方法是检查 `(N + K)` 或 `(N–K)` 是否是完美立方体。
