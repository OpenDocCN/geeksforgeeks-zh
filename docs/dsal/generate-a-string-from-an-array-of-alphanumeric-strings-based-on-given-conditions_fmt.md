...
> 原文：[https://www.geeksforgeeks.org/generate-a-string-from-an-array-of-alphanumeric-strings-based-on-given-conditions/](https://www.geeksforgeeks.org/generate-a-string-from-an-array-of-alphanumeric-strings-based-on-given-conditions/)
...
> **输出：** `tiX`
> **解释：**
> 对于第一个字符串 `"Robert:36787"`：长度 `"Robert"` 为 6。由于字符串 `"36787"` 中存在 6，因此答案后会附加 `"Robert"` 的第 6 个字符，即 `t`。
> 对于第二个字符串 `"Tina:68721"`：`"Tina"` 的长度为 4。`"68721"` 中出现的小于或等于 4 的最高数字是 2。因此，`"Tina"` 的第二个字符，即 `i`，被附加到答案中。
> 对于第三个字符串 `"Jo:56389"`：`"Jo"` 的长度为 2。由于 `"56389"` 中不存在小于或等于 2 的数字，因此答案后会附加一个 `T`（`T = 'X'`）。
> 因此，上述操作后的最终字符串为 `"tiX"`。

> **输入：** `arr[] = {"Geeks:89167", "gfg:68795"}`，`T = 'X'`
> **输出：** `GX`
> **解释：**
> 对于第一个字符串 `"Geeks:89167"`，`"Geeks"` 的长度 = 5，`"89167"` 的数字有小于 5 的数字 1。
...
1.  遍历字符串数组，在 `":"` 周围拆分每一个字符串。第一部分包含名称，第二部分包含编号。
2.  将 `名称` 的长度存储在一个变量中，找到小于或等于 `数字` 长度的最大数字。
3.  如果找到任何这样的数字，提取名称为 `d` 的 `索引` 处的字符，并附加到结果字符串中。否则，将 `T` 追加到结果字符串中。
