...
给定严格增加的正整数数组`A`，其中，`1 <= A[i] <= 10^18`。

任务是找到`A`的最长斐波那契式子序列的长度。 如果这样的子序列不存在，则返回 0。
...
斐波那契式序列使得每个序列都有两个相邻的项来确定下一个预期项。

> 例如，对于 1、1，我们希望序列必须继续 2、3、5、8、13 等。

*   使用集合或映射可以快速确定数组`A`中是否存在斐波那契数列的下一项。 由于这些项的指数增长，因此每次迭代都只能进行`log(M)`搜索以获取下一个元素。
...
为了优化上述方法，该想法是实现[动态规划](https://www.geeksforgeeks.org/dynamic-programming/)。 初始化代表斐波那契序列长度的`dp`表`dp[a, b]`以`(a, b)`结尾。 然后将表更新为`dp[a, b] = (dp[b – a, a] + 1)`或 2。
