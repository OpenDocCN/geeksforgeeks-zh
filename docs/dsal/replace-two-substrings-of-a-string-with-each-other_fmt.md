...
> 原文：[https://www.geeksforgeeks.org/replace-two-substrings-of-a-string-with-each-other/](https://www.geeksforgeeks.org/replace-two-substrings-of-a-string-with-each-other/)

给定 3 个字符串 `S`、`A` 和 `B`。任务是用 `B` 替换 `S` 中等于 `A` 的每个子串，用 `A` 替换 `S` 中等于 `B` 的每个子串。匹配 `A` 或 `B` 的两个或多个子串有可能重叠。为了避免这种情况的混淆，您应该找到与 `A` 或 `B` 匹配的最左边的子字符串，将其替换，然后继续字符串的其余部分。
例如将 `A = "aa"` 与 `S = "aaa"` 匹配时，`A[0, 1]` 将优先于 `A[1, 2]`。

**注意**：`A` 和 `B` 的长度相同，且 `A != B`。
...
**方法：** 从长度为 `len(S)` 的 `S` 开始，遍历所有可能的长度为 `len(A)` 的子串。如果有子字符串匹配 `A` 或 `B`，则根据需要更新字符串，并最终打印更新后的字符串。
