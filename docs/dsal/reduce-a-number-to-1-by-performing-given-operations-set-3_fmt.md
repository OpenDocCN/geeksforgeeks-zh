...
> 原文: [https://www.geeksforgeeks.org/reduce-a-number-to-1-by-performing-given-operations-set-3/](https://www.geeksforgeeks.org/reduce-a-number-to-1-by-performing-given-operations-set-3/)

给定一个整数 `N`，任务是通过执行以下操作，找到将给定数 `N` 减少到 `1` 所需的步数:

1.  如果数字是 2 的幂，那么将数字除以 `2`。
2.  否则，从 `N` 中减去比 `N` 小 2 的最大幂。
...
*   如果数是 2 的幂，那么只允许我们用 `2` 除这个数。
*   否则我们可以从 `N` 中减去小于 `N` 的 2 的最大幂。
...
**方法:** 给定的问题可以使用按位运算符来解决。按照以下步骤解决问题:

*   初始化两个变量，表示 `res` 值为 `0` 和 `MSB` 值为 `log₂(N)` 以存储将该数减少到 `1` 所需的步数和 `N` 的最高有效位。
*   迭代而 `N` 不等于 `1`:
    *   检查数字是否为 2 的幂，然后将 `N` 除以 `2`。
    *   否则，从 `N` 中减去小于 `N` 的 2 的最大幂，即更新 `N` 为 `N=N-2^MSB`。
