# 求数列 1 + x + x^2 + x^3 + ... + x^n 之和的程序

> 原文: [https://www.geeksforgeeks.org/program-to-find-the-sum-of-the-series-1-x-x2-x3-xn/](https://www.geeksforgeeks.org/program-to-find-the-sum-of-the-series-1-x-x2-x3-xn/)

给定一个整数 `X`，任务是打印数列，求数列之和 `1 + x^{1} + x^{2} + x^{3} + x^{4} ... x^{N}`。
...
**方法:** 思路是遍历数列，计算数列 `N` 项之和。该系列的第 `N` 项可计算为:
