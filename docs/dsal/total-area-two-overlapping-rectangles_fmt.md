...
> 原文：[https://www.geeksforgeeks.org/total-area-two-overlapping-rectangles/](https://www.geeksforgeeks.org/total-area-two-overlapping-rectangles/)
...
第一个矩形的 `y_distance = ABS(l1.y – r1.y)`

同样，我们可以计算第二个矩形的面积。

对于相交部分的面积，
相交矩形的 `x_distance = min(r1.x, r2.x) – max(l1.x, l2.x)`
相交矩形的 `y_distance = min(r1.y, r2.y) – max(l1.y, l2.y)`
