...
> 原文: [https://www.geeksforgeeks.org/reduce-array-to-a-single-element-by-repeatedly-replacing-adjacent-unequal-pairs-with-their-maximum/](https://www.geeksforgeeks.org/reduce-array-to-a-single-element-by-repeatedly-replacing-adjacent-unequal-pairs-with-their-maximum/)
...
最简单的方法是[遍历给定的数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)，对于每个元素 `arr[i]`，根据给定的约束开始连续执行给定的操作。如果对于任何元素，数组变成单个元素，打印索引 `i`。否则，打印 `-1`。
...
想法是使用[排序算法](https://www.geeksforgeeks.org/sorting-algorithms/)。请注意，如果所有元素都相同，答案将始终是 `-1`。否则，[可以选择具有最大元素的索引](https://www.geeksforgeeks.org/python-maximum-minimum-elements-position-list/)来开始执行操作。按照以下步骤解决问题:

*   创建与给定数组相同的另一个数组 `B[]`，并创建一个变量 `save` 用 `-1` 初始化来存储答案。
*   [排序数组](https://www.geeksforgeeks.org/c-program-to-sort-an-array-in-ascending-order/) `B[]`。
*   [使用变量 `i` 在 `[N–1]` 到 `[0]` 的范围内遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)，如果发现两个连续的不等元素，即 `B[i]` 不等于 `B[i–1]`，则更新 `save` 为 `save = i`。
*   遍历数组后:
    *   如果 `save` 是 `-1`，打印 `-1` 并返回。
    *   否则如果 `save` 等于 `arr[0]` 且 `save` 不等于 `arr[1]`，则更新 `save` 为 `1`。
    *   否则如果 `save` 等于 `arr[N–1]` 且 `save` 不等于 `arr[N–2]`，则更新 `save` 为 `N`。
    *   否则，在范围 `[1, N–1]` 内循环，检查 `save` 是否等于 `arr[i]`，使得 `arr[i]` 不等于 `arr[i–1]` 和 `arr[i+1]`，然后将 `save` 更新为 `save = i+1`。
*   完成上述步骤后，打印变量 `save` 中存储的索引。
