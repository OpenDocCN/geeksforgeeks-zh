...
> 原文: [https://www.geeksforgeeks.org/program-to-find-last-two-digits-of-2n/](https://www.geeksforgeeks.org/program-to-find-last-two-digits-of-2n/)

给定一个数字 `n`，我们需要找到 `2^n` 的最后两位数字。
...
**注意:** 这种方式只会在一定范围内对 `2^n` 起作用，因为[溢出](https://www.geeksforgeeks.org/check-for-integer-overflow/)发生。
...
高效的方法是每次乘法后只保留 2 位数字。这个想法与[模幂](https://www.geeksforgeeks.org/modular-exponentiation-power-in-modular-arithmetic/)中讨论的非常相似，这里讨论了一种通用的方法来寻找 `(a^b)%c`，这里 `c` 是 `10^2`，因为只需要最后两位数字。
...
Professional Hypertext Preprocessor 的缩写。
