...
> 原文:[https://www.geeksforgeeks.org/two-odd-occurring-elements-array-occur-even-times/](https://www.geeksforgeeks.org/two-odd-occurring-elements-array-occur-even-times/)
...
一个**简单的解决方案**是使用两个嵌套循环。外部循环遍历所有元素。内部循环计算当前元素的出现次数。我们打印出现次数为奇数的元素。如果此解决方案为 O(n<sup>2</sup>)，则时间复杂度为 O(n<sup>2</sup>)。
