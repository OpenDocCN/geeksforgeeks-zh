...
> 原文: [https://www.geeksforgeeks.org/sliding-window-maximum-maximum-of-all-subarrays-of-size-k-using-stack-in-on-time/](https://www.geeksforgeeks.org/sliding-window-maximum-maximum-of-all-subarrays-of-size-k-using-stack-in-on-time/)
...
对于每个指标，计算子阵列从该指标开始时当前元素最大的指标，即对于每个指标 `i` 一个指标 `j ≥ i`，使得 `max(a[i], a[i + 1], … a[j]) = a[i]`。我们称之为 `max_up[i]`。
然后从 `i` 索引开始的长度 `k` 子数组中的最大元素，可以通过检查从 `i` 到 `i+k–1` 的每个索引来找到，其中 `max_up[i] ≥ i+k–1`（该窗口的最后一个索引）。

`Stack` 数据结构可用于将值存储在窗口中，即最后访问的元素或前一个插入的元素将位于顶部（索引最接近当前元素的元素）。

## 算法

1.  创建一个数组 `max_up` 和一个堆栈来存储索引。在堆栈中按 0。
2.  运行从索引 1 到索引 n-1 的循环。
3.  从堆栈中弹出所有索引，哪些元素 `(arr[s.top()])` 小于当前元素并更新 `max_up[s.top()] = i–1`，然后在堆栈中插入 `i`。
4.  从堆栈中弹出所有索引，并分配 `max_up[s.top()] = n–1`。
5.  创建变量 `j = 0`
6.  运行从 0 到 n–k 的循环，循环计数器为 `i`
7.  运行嵌套循环，直到 `j < i` 或 `max_up[j] < i+k–1`，在每次迭代中递增 `j`。
8.  打印 `j` 索引处的数组元素。
...
https://youtu.be/m-Dqu7csdJk?list=plqm7alhxfysqdk2mdfbwedjd2svvjh9p