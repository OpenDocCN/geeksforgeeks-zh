...
> 原文: [https://www.geeksforgeeks.org/multiply-number-4-using-bitwise-operator/](https://www.geeksforgeeks.org/multiply-number-4-using-bitwise-operator/)

我们有一个数字 `n`，我们的任务是使用逐位运算符将数字乘以 4。
示例:
...
**解释** 情况 1: - `n = 4`。4 的二进制数是 `100`，现在向左移动两位，然后是 `10000`，现在数字是 16，乘以 4*4=16，`ans`。
**接近**: - (`n << 2`) 向左移动两位。
...
**推广:** 一般来说，我们可以使用按位运算符乘以 2 的幂。例如，假设我们希望与 16 相乘 (这是 2<sup>4</sup>)，我们可以通过向左移动 4 位来实现。