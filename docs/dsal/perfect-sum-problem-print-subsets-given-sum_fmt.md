...
> 原文：[https://www.geeksforgeeks.org/perfect-sum-problem-print-subsets-given-sum/](https://www.geeksforgeeks.org/perfect-sum-problem-print-subsets-given-sum/)
...
就像[之前的帖子](https://www.geeksforgeeks.org/dynamic-programming-subset-sum-problem/)一样，我们构建了一个二维数组 `dp[][]`，这样 `dp[i][j]` 就可以存储 `true`，前提是数组元素从 `0` 到 `i` 的和 `j` 是可能的。

在填充 `dp[][]` 之后，我们从 `dp[n-1][sum]` 递归遍历它。对于被遍历的单元，我们在到达之前存储路径，并考虑元素的两种可能性。
