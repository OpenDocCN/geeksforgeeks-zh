...
最简单的方法是生成所有可能的四倍体，并检查给定条件是否成立。如果发现是真的，则增加最终计数。打印获得的最终计数。
...
为了优化上述方法，其思想是使用[散列](http://www.geeksforgeeks.org/hashing-data-structure/)。步骤如下：
...
// Function to count total number
// of required tuples
static int countTuples(int arr[], int N)
{
  int ans = 0, val = 0;
