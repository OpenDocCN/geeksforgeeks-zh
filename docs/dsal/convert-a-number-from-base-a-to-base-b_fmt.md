...
> 原文: [https://www.geeksforgeeks.org/convert-a-number-from-base-a-to-base-b/](https://www.geeksforgeeks.org/convert-a-number-from-base-a-to-base-b/)

给定两个正整数 `A` 和 `B` 以及一个大小为 `N` 的[字符串](https://www.geeksforgeeks.org/string-data-structure/) `S`，其中 `S` 表示基数 `A` 中的一个数字，任务是将给定字符串 `S` 从基数 `A` 转换为基数 `B`。
...
[数字系统是](https://www.geeksforgeeks.org/number-system-and-base-conversions/)在[计算机系统架构](https://www.geeksforgeeks.org/characteristics-of-computer-system/)中表示数字的技术。计算机体系结构支持以下数字系统:

*   **二进制数系统(基数 2):** 二进制数系统只由两位数组成，`0` 和 `1`，这个数系统的基数是 `2`。
...
**从任意基数到十进制的转换:** 在基数 `base` 中数字 `str` 的十进制等效值等于 `str[len-1] * 1 + str[len-2] * base + str[len-3] * (base)^2 + ...`

**从十进制到任意基数的转换:**
十进制数 `inputNum` 可以通过重复将 `inputNum` 除以 `base` 转换为基数 `base` 上的数字，并存储余数。最后[反转得到的字符串](https://www.geeksforgeeks.org/reverse-a-string-in-c-cpp-different-methods/)得到想要的结果。
