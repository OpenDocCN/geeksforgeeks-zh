# 通过插入加法运算符对一个数字串的所有可能表达式求和

> 原文：[https://www.geeksforgeeks.org/sum-of-all-possible-expressions-of-a-numeric-string-possible-by-inserting-addition-operators/](https://www.geeksforgeeks.org/sum-of-all-possible-expressions-of-a-numeric-string-possible-by-inserting-addition-operators/)

给定一个长度为 `N` 的数字字符串 `str`，任务是通过多次在字符串字符之间插入 `+` 运算符来找到所有可能表达式的总和。
...
想法是以所有可能的方式在字符串的所有可能的索引处插入 `+` 运算符，并计算总和。最后，打印得到的总和。按照以下步骤解决问题：
