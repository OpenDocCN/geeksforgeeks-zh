...
> 原文: [https://www.geeksforgeeks.org/program-to-print-the-fish-pattern/](https://www.geeksforgeeks.org/program-to-print-the-fish-pattern/)
...
1.  首先，创建上部:
    *   对 `i=0` 到 `i < N` 运行一个循环，在循环的每次迭代中:
        *   如上图所示，首先出现一串具有 `spaces1` (最初为 `M = N`) 空间的 `spaces1`，然后出现一层恒星，假设 `stars1` 只有一颗恒星，然后出现 2 次 `spaces1`，然后出现另一层恒星，最初为 0 颗恒星 (`stars2`)。
        *   现在在每次迭代中，`spaces1` 减少一个空间，`stars1` 增加 2 颗恒星，`stars2` 增加 1 颗恒星。
2.  中间部分，打印 `stars1` 和 `stars2`，因为这一行没有空格。
3.  现在，为了得到下部，颠倒上部的算法。
4.  循环结束后，将创建鱼的图案。
