...
> 原文: [https://www.geeksforgeeks.org/nearest-greater-number-by-interchanging-the-digits/](https://www.geeksforgeeks.org/nearest-greater-number-by-interchanging-the-digits/)
...
*   使用整数设置 `min1` 的最小值。
*   用上述置换方法交换 `A` 的数字。
*   检查 `A` 的排列值是否小于 `min1`。如果小于，则将 `min1` 更新为该值。
*   对 `A` 的所有排列重复此操作，找到最小的较大值。
