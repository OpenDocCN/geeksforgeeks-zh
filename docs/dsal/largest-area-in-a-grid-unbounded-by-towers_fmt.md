...
> 原文：[https://www.geeksforgeeks.org/largest-area-in-a-grid-unbounded-by-towers/](https://www.geeksforgeeks.org/largest-area-in-a-grid-unbounded-by-towers/)
...
1.  用 `0` 初始化尺寸为 `L * W` 的矩阵。
2.  遍历 `X[]` 和 `Y[]`，对于每一个 `(X[i], Y[i])`，将 `X[i]` 第行和 `Y[i]` 第列中的所有单元格标记为 `1`，表示在 `(X[i], Y[i])` 被塔守护。
3.  然后遍历矩阵，对于每个单元，找到未被保护的最大子矩阵，即由 `0` 组成的最大子矩阵。打印相应的区域。
...
上述方法可以使用[贪婪技术](https://www.geeksforgeeks.org/greedy-algorithms/)进行优化：

1.  对坐标列表 `X[]` 和 `Y[]` 进行排序。
2.  计算 X 坐标之间的空格，即 `dX[] = {X_1, X_2–X_1, …, X_N–X_(N-1), (L+1)–X_N}`。同样，计算 Y 坐标之间的间距，`dY[] = {Y_1, Y_2–Y_1, …, Y_N–Y_(N-1), (W+1)–Y_N}`。
3.  遍历 `dX[]` 和 `dY[]` 并计算它们各自的最大值。
4.  计算它们最大值的乘积，并将其打印为所需的最长无界区域。
