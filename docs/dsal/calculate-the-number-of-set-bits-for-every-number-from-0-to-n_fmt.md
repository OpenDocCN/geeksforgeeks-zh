...
> 原文: [https://www.geeksforgeeks.org/calculate-the-number-of-set-bits-for-every-number-from-0-to-n/](https://www.geeksforgeeks.org/calculate-the-number-of-set-bits-for-every-number-from-0-to-n/)
...
> **输入:** `N = 3`
> **输出:** `0 1 1 2`
> `0`、`1`、`2`、`3` 可以写成二进制 `0`、`1`、`10`、`11`。
> `1` 在二进制表示中的个数是 `0`、`1`、`1` 和 `2`。
> **输入:** `N = 5`
> **输出:** `0 1 2 1 2`

**天真的方法:** 运行一个从 `0` 到 `N` 的循环，并使用内置的位计数函数 `__builtin_popcount()`，找到所有所需整数中的设置位数。
以下是上述方法的实施:
...
在 `5(101)` 中 `1` 的数等于在 `2` 的二进制表示 + `1` 中 `1` 的数。所以在任何奇数 `(2 * i + 1)` 的情况下，都将是 `i` 的二进制表示中 `1` 的个数 + `1`。
以下是上述方法的实施:
