...
> 原文：[https://www.geeksforgeeks.org/count-ancestors-with-smaller-value-for-each-node-of-a-binary-tree/](https://www.geeksforgeeks.org/count-ancestors-with-smaller-value-for-each-node-of-a-binary-tree/)

给定一个由 `N` 个节点组成的[二叉树](https://www.geeksforgeeks.org/binary-tree-data-structure/)，值从 `1` 到 `N`，根在节点 `1`，每个节点的任务是计算值小于当前节点的[祖先](https://www.geeksforgeeks.org/print-ancestors-of-a-given-node-in-binary-tree/)的数量。
...
想法是从树的根节点执行 [DFS 遍历](https://www.geeksforgeeks.org/dfs-traversal-of-a-tree-using-recursion/)，并将每个节点的直接父节点存储在一个数组中。然后迭代每个节点，并使用父数组，将其值与其所有祖先进行比较。最后，打印结果。

按照以下步骤解决问题：

- 初始化一个[数组](https://www.geeksforgeeks.org/array-data-structure/)，比如说 `par[]`，大小为 `N`，用 `-1` 来存储每个节点的直接父节点。
- 从根节点开始执行 [DFS 遍历](https://www.geeksforgeeks.org/dfs-traversal-of-a-tree-using-recursion/)，并执行以下步骤：
    - 更新当前节点的父节点。
    - 递归调用当前节点的子节点。
- 现在，[使用变量 `i` 迭代范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[1，N]`，并执行以下步骤：
    - 将当前节点存储在一个变量中，比如 `node`。
    - [迭代](https://www.geeksforgeeks.org/loops-in-c-and-cpp/)直到 `par[node]` 不等于 `-1`：
        - 如果 `par[node]` 小于 `i`，那么将 `cnt` 增加 `1`。
        - 将 `node` 更新为 `par[node]`。
