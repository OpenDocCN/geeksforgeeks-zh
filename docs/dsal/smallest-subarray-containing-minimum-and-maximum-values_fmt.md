...
> 原文: [https://www.geeksforgeeks.org/smallest-subarray-containing-minimum-and-maximum-values/](https://www.geeksforgeeks.org/smallest-subarray-containing-minimum-and-maximum-values/)
...
思路是使用[双指技术](https://www.geeksforgeeks.org/two-pointers-technique/)这里:

*   求数组的最大值和最小值。
*   遍历数组并存储最近出现的最大值和最小值。
*   如果最大值最后一次出现的时间是 `pos_max`，最小值是 `pos_min`，那么 `ABS(pos_min - pos_max) + 1` 的最小值就是我们需要的答案。
