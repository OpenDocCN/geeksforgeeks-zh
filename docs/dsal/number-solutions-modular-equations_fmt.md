...
> 原文：`https://www.geeksforgeeks.org/number-solutions-modular-equations/`
...
如果我们仔细分析方程式 `A mod X = B`，很容易注意到，如果 `(A = B)`，那么 `X` 可以取比 `A` 大无限多的值。在 `(A < B)` 的情况下，不存在任何可能的 `X` 值能使模方程成立。所以，我们唯一需要研究的情况是 `(A > B)`。因此，我们现在深入关注这个案例。
...
因此，我们可以得出结论：`(A–B)` 大于 `B` 的除数，是 `X` 满足 `A mod X = B` 所能取的所有可能值。
...
上述方法的时间复杂度无非是求`A–B`的除数的时间复杂度`O(√(A–B))`。