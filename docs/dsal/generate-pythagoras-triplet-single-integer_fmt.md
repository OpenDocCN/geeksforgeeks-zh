...
> 原文：[https://www.geeksforgeeks.org/generate-pythagoras-triplet-single-integer/](https://www.geeksforgeeks.org/generate-pythagoras-triplet-single-integer/)
...
定义：“毕达哥拉斯三元组”是毕达哥拉斯定理的整数解，即它们满足方程 `a^2 + b^2 = c^2`。
我们的任务是从整数值生成三元组。这可能是一个令人困惑的任务，因为给我们的边可以是斜边或非斜边。
通过把三胞胎放在一个公式中开始计算三胞胎，可以推导出只有对于 1 和 2，才可能没有三胞胎。
再进一步，
如果 `n` 为偶数，我们的三胞胎用公式 `[(n^2/4 - 1)^2 + n^2 = (n^2/4 + 1)^2]` 计算。
如果 `n` 为奇数，我们的三胞胎用公式 `[(n^2-1)^2/2 + n^2 = (n^2+1)^2/2]` 计算。

### 计算证明

勾股定理也可以写成 `c^2 - b^2 = a^2`。
即 `a*a = (c-b)(c+b)`。
`a*a x 1 = a*a`，由此可见 `c = (a^2+1)/2` 和 `b = (a^2 - 1)/2` 一样，如果 `n` 为奇数，这种解法也有效。
对于偶数解，`c+b = n^2/2, c-b=2`，由此，我们得到 `n` 为偶数时的上式。
