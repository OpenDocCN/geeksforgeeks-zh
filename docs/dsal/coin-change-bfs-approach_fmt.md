...
给定一个整数 `X` 和一个由正整数组成的长度为 `N` 的数组 `arr[]`，任务是从数组中选取最小数量的整数，使它们的总和达到 `X`。任何数字都可以选择无限次。如果没有答案，则打印 `-1`。

**示例:**

> **输入:** `X = 7`，`arr[] = {3，5，4}`
> **输出:** `2`
> 最小元素数为 2，因为 3 和 4 可以选择达到 7。
>
> **输入:** `X = 4`，`arr[] = {5}`
> **输出:** `-1`
...
在此之前，我们先来定义一个状态。一个状态 `S_X` 可以定义为我们需要从数组中取的整数的最小数量，以得到总共 `X`。
现在，如果我们开始将每个状态视为图中的一个节点，使得每个节点都连接到 `(S_{X-arr[0]}, S_{X-arr[1]}, ..., S_{X-arr[N-1]})`。
