...
> 原文: [https://www.geeksforgeeks.org/program-to-check-if-a-number-can-be-expressed-as-an-even-power-of-2-or-not/](https://www.geeksforgeeks.org/program-to-check-if-a-number-can-be-expressed-as-an-even-power-of-2-or-not/)

给定一个正整数 `N`，任务是检查给定的整数是否为 `2` 的偶数次幂。

## 示例

> **输入:** `N = 4`
> **输出:** 是
> **说明:** `4` 可以表示为 `2^2 = 4`，是 `2` 的偶数次方。
>
> **输入:** `N = 8`
> **输出:** No
> **说明:** `8` 可以表示为 `2^3 = 8`，是 `2` 的奇次幂。

## 天真法

最简单的方法是迭代 `2` 的所有指数值，检查对应的值是否为 `N`，指数是否为偶数。如果两者都为真，则打印“是”。否则，如果 `2` 的当前指数超过 `N`，则打印“否”。
...
上述方法也可以使用二分搜索法实现。按照以下步骤解决问题:

*   初始化两个变量，`low` 为 `0` 和 `high` 为 `N`。
*   重复直到 `low` 超过 `high`:
    *   求 `mid` 的值为 `low + (high - low) / 2`。
    *   如果 `2^mid` 的值为 `N`，而 `mid` 的值为偶数，则打印“是”并跳出循环。
    *   如果 `2^mid < N` 的值，更新 `low` 的值为 `(mid + 1)`。
    *   否则，将 `high` 值更新为 `(mid - 1)`。
