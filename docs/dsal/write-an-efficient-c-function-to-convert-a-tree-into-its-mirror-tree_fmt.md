...
> 原文: [https://www.geeksforgeeks.org/write-an-efficient-c-function-to-convert-a-tree-into-its-mirror-tree/](https://www.geeksforgeeks.org/write-an-efficient-c-function-to-convert-a-tree-into-its-mirror-tree/)
...
最坏的情况时间复杂度是 `O(n)`，对于空间复杂度，如果我们不考虑函数调用的递归栈的大小，那么 `O(1)`否则 `O(h)`，其中 `h` 是树的高度。这个程序类似于树的遍历空间和时间复杂度将与树遍历相同更多信息请参见我们的[树遍历](https://www.geeksforgeeks.org/tree-traversals-inorder-preorder-and-postorder/)帖子了解详情。
