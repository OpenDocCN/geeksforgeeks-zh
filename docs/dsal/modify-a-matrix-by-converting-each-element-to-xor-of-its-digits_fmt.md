...
> 原文：[https://www.geeksforgeeks.org/modify-a-matrix-by-converting-each-element-to-xor-of-its-digits/](https://www.geeksforgeeks.org/modify-a-matrix-by-converting-each-element-to-xor-of-its-digits/)

给定维度为 `M*N` 的矩阵 `arr[][]`，任务是将每个矩阵元素转换为元素中数字的位异或。
...
**时间复杂度:** `O(M*N*log10 K)`，其中 `K` 是[矩阵中存在的最大元素](https://www.geeksforgeeks.org/program-to-find-the-maximum-element-in-a-matrix/)。
**辅助空间:** `O(1)`