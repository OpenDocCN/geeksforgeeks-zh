...
> 原文: [https://www.geeksforgeeks.org/find-the-gcd-of-lcm-of-all-unique-pairs-in-an-array/](https://www.geeksforgeeks.org/find-the-gcd-of-lcm-of-all-unique-pairs-in-an-array/)
...
- 起始元素为 `A[i]` 的所有对的 `LCM` 可以写成 `LCM(A[i], GCD(I+1 到 n-1 范围内的所有 j))`。
- 为此，我们构建了一个[后缀数组](https://www.geeksforgeeks.org/suffix-array-set-1-introduction/)。假设 `suffix[]` 存储属于范围 `[i+1, N-1]` 的元素的 `gcd`。
- 然后创建一个 `LCM` 数组来存储 `A[i]` 的 `LCM` 和其后所有元素的 `GCD`，即 `LCM[i] = LCM(A[i], suffix[i+1])`，其中 `suffix[i+1]` 存储元素 `[i+1, n-1]` 的 `GCD`。
