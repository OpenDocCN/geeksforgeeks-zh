...
是使用两个嵌套循环。外循环从最左边的元素开始一个接一个地选择一个元素。内部循环检查元素的左侧是否存在。如果存在，则忽略该元素。
...
是使用排序技术，我们首先按升序对数组的所有元素进行排序，然后在数组中逐个查找不同的元素。
...
是使用`unordered_set`，我们运行一个`for`循环，并且该值首次出现在其`add sum`变量中，并存储在哈希表中，下一次我们不使用该值。
