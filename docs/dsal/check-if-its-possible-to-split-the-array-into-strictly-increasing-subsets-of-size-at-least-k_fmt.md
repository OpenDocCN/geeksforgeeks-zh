...
> 原文：[https://www.geeksforgeeks.org/check-if-its-possible-to-split-the-array-into-strictly-increasing-subsets-of-size-at-least-k/](https://www.geeksforgeeks.org/check-if-its-possible-to-split-the-array-into-strictly-increasing-subsets-of-size-at-least-k/)

给定一个大小为 `N` 的数组 `arr[]` 和一个整数 `K`，任务是检查是否有可能将数组拆分为大小至少为 `K` 的严格递增子集。如果可能，则打印“是”。否则，打印“否”。
...
利用 `Map` 存储每个元素的频率，将数组划分为 `X` 个子集，其中 `X` 为数组中出现次数最大的元素的频率即可解决问题。按照以下步骤解决问题：

*   初始化一个映射表示 `m` 来存储元素的频率，也初始化一个变量 `mx` 为 `0` 来存储数组中最大出现元素的频率 `arr[]`。
*   使用变量 `i` 遍历数组 `arr[]`，并将 `m[arr[i]]` 增加 `1`，并将 `mx` 的值更新为 `max(mx, m[arr[i]])`。
