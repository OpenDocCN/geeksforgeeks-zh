...
标准的河内塔问题在这里解释。在标准问题中，所有的盘交易都被认为是相同的。

给定一个 3×3 的矩阵 `costs[][]` 包含棒之间的盘转移成本，其中 `costs[i][j]` 存储将盘从 `棒 i` 转移到 `棒 j` 的成本。同一杆之间的转移成本为 `0`。因此成本矩阵的对角线元素都是 `0`。任务是打印所有 `N` 盘从 `杆 1` 转移到 `杆 3` 的最小成本。
...
假设 `mincost(idx, src, dest)` 是将 `idx` 到 `N` 的索引盘从 `杆 src` 转移到 `杆 dest` 的最小成本。既不是源棒也不是目的棒的第三棒的值为 `rem = 6 –( src+dest)`，因为棒号是 1、2 和 3，它们的和是 6。如果第一个和第三个杆分别是源杆和目的杆，那么辅助杆的编号为 `6 –( 1+3)= 2`。
...
> `cost = mincost(idx + 1, src, dest) + costs[src][rem] + mincost(idx + 1, dest, src) + costs[rem][dest] + mincost(idx + 1, src, dest)`
