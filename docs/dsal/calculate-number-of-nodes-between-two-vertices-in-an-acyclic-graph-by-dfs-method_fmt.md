...
给定一个由 `V` 个顶点和 `E` 条边、一个源顶点 `src` 和一个目的顶点 `dest` 组成的连通非循环图，任务是计算图中给定源顶点和目的顶点之间的顶点数。

## 示例

**输入:** `V = 8`，`E = 7`，`src = 7`，`dest = 8`，`edges[][] ={{1, 4}, {4, 5}, {4, 2}, {2, 6}, {6, 3}, {2, 7}, {3, 8}}`
**输出:** `3`
**解释:** `7` 和 `8` 之间的路径是 `7->2->6->3->8`。所以，`7` 到 `8` 之间的节点数是 `3`。
...
**输出:** `1`
**解释:** `5` 和 `2` 之间的路径是 `5 -> 4 -> 2`。所以，`5` 到 `2` 之间的节点数是 `1`。

## 方法

这个问题也可以使用[这篇](https://www.geeksforgeeks.org/number-nodes-two-vertices-acyclic-graph-disjoint-union-method/)文章中所述的[不相交联合](https://www.geeksforgeeks.org/union-find/)方法来解决。解决这个问题的另一种方法是使用[深度优先搜索](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/)方法来解决。按照以下步骤解决此问题:

*   初始化一个访问过的数组 `vis[]` 来标记哪些节点已经被访问过。将所有节点标记为 `0`，即未访问。
*   执行一个 DFS，找出 `src` 和 `dest` 之间路径中存在的节点数。
*   `src` 和 `dest` 之间的节点数等于它们与 `2` 之间的路径长度差，即 `(path_length(src, dest) - 2)`。
*   由于图是无环且连通的，因此在 `src` 和 `dest` 之间总会有一条单一的路径。
