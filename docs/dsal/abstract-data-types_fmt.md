...
数据一般存储在列表中的键序列中，该列表具有一个头部结构，由 `count`、`pointers` 和用于比较列表中数据的 `compare function` 的地址组成。
...
在堆栈 ADT 实现中，数据指针不是存储在每个节点中，而是存储在数据中。程序为 `data` 分配内存，并将 `address` 传递给堆栈 ADT。

![](img/ee1655c8f8c9bb501e24e78a11a399ad.png)

头节点和数据节点封装在 ADT 中。调用函数只能看到指向堆栈的指针。堆栈头结构还包含一个指针，指向当前堆栈中条目数量的 `顶部` 和 `计数`。
...
**参考：** [https://en.wikipedia.org/wiki/Abstract_data_type](https://en.wikipedia.org/wiki/Abstract_data_type)
