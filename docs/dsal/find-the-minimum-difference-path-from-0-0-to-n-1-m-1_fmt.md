# 找出从(0,0)到(N-1,M-1)的最小差路径

> 原文: [https://www.geeksforgeeks.org/find-the-minimum-difference-path-from-0-0-to-n-1-m-1/](https://www.geeksforgeeks.org/find-the-minimum-difference-path-from-0-0-to-n-1-m-1/)

给定两个 2D 阵列 `b[][]` 和 `c[][]`，它们有 `N` 行和 `M` 列。任务是沿着从 `(0,0)` 到 `(N–1,M–1)` 的路径，最小化 `b[i][j]` 之和与 `c[i][j]` 之和的绝对差。
...
答案独立于决定 `b[i][j]` 和 `c[i][j]` 的顺序和路径。因此，让我们考虑一个布尔表，如果 `(i,j)` 能够以 `k` 的最小差异达到，则 `dp[i][j][k]` 将为真。

如果是真的，那么对于单元格 `(i+1,j)` 来说，要么是 `k + |b_{i+1,j}–c_{i+1,j}|` 要么是 `| k –| b_{i+1,j}–c_{i+1,j}||`。正方形 `(i,j+1)` 也是如此。因此，表格可以按照 `i` 和 `j` 的递增顺序填写。
