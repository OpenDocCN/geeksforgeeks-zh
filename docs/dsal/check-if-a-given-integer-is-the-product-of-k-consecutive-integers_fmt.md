...
> 原文: [https://www.geeksforgeeks.org/check-if-a-given-integer-is-the-product-of-k-consecutive-integers/](https://www.geeksforgeeks.org/check-if-a-given-integer-is-the-product-of-k-consecutive-integers/)
...
*   初始化两个整数，比如 `Kthroot` 和 `product`，分别存储整数 `N` 的[K<sup>th</sup>根](https://www.geeksforgeeks.org/n-th-root-number/)和 `K` 的乘积。
*   将 `[1，K]` 范围内整数的乘积存储在变量 `product` 中。
*   否则，[迭代范围](https://www.geeksforgeeks.org/python-program-to-print-all-positive-numbers-in-a-range/) `[2，Kthroot]` 并执行以下步骤:
    *   如果产品价值等于 `N`，则打印`“是”`并[跳出循环](https://www.geeksforgeeks.org/break-statement-cc/)。
    *   将产品价值更新为 `(product * (i + K – 1)) / (i – 1)`。
*   完成上述步骤后，如果上述情况都不满足，则打印`“否”`，因为 `N` 不能表示为 `K` 连续整数的乘积。
