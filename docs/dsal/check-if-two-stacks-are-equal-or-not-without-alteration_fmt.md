...
> 原文: [https://www.geeksforgeeks.org/check-if-two-stacks-are-equal-or-not-without-alteration/](https://www.geeksforgeeks.org/check-if-two-stacks-are-equal-or-not-without-alteration/)
...
给定的问题可以通过在给定的两个[堆栈](https://www.geeksforgeeks.org/stack-class-in-java/)之间移动一些元素来解决，用于检查两个堆栈中的每个对应元素。按照以下步骤解决问题:

*   将堆栈大小 `S1` 和 `S2` 分别存储在变量 `N` 和 `M` 中。
*   如果 `N` 不等于 `M`，则打印 `"No"` 返回。
*   迭代范围 `[1，N]` 并执行以下操作:
    *   [将](https://www.geeksforgeeks.org/stack-push-and-pop-in-c-stl/)[顶](https://www.geeksforgeeks.org/stack-top-c-stl/) `(N–I)` 元素叠加 `S1` 推至叠加 `S2`。
