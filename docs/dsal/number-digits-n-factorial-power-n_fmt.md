...
> 原文：[https://www.geeksforgeeks.org/number-digits-n-factorial-power-n/](https://www.geeksforgeeks.org/number-digits-n-factorial-power-n/)

给定一个正整数 `N`，我们要求 `N` 的阶乘中的位数的总数升到 `N` 的幂，即 `(N!)^N`。
...
> **蛮力法：** 通过蛮力，我们可以简单的算出 `N!` 在 `O(N)` 时间内，然后乘以 `N` 倍，但这将是一个非常慢的方法，并且会超过时间和空间，因为 `(N!)^N` 将是一个巨大的数字。
>
> **高效法：**
> 我们再仔细看看。我们可以打破 `(N!)^N` 转化为更简单的易于计算的术语。通过取普通对数，
>
> 我们得到 `N * log10(N!)`。
>
> 我们知道，`N! = 1 * 2 * 3 * 4 *....* N`。
