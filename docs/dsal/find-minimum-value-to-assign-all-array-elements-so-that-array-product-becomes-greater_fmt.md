...
> 原文: [https://www.geeksforgeeks.org/find-minimum-value-to-assign-all-array-elements-so-that-array-product-becomes-greater/](https://www.geeksforgeeks.org/find-minimum-value-to-assign-all-array-elements-so-that-array-product-becomes-greater/)

给定一个由 `n` 个元素组成的数组 `arr[]`，将给定数组的所有元素更新为某个最小值 `x`，即 `arr[i] = x` (`0 <= i < n`)，使得新数组所有元素的乘积严格大于初始数组所有元素的乘积，其中 `1 <= arr[i] <= 10^10` 且 `1 <= n <= 10^5`。
...
**方法 1 : O(n log n)**
我们在 `n` 的极限上使用二分搜索法，在每个 `mid`，我们检查 `mid^n` 的积是否大于数组的原积。
我们使用乘积对数来避免溢出。因此，我们计算当前乘积的对数和二分搜索法期间 `mid^n` 的对数来比较值。
