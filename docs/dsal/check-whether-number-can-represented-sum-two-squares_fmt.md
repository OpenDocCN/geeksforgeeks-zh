...
> 原文：[https://www.geeksforgeeks.org/check-whether-number-can-represented-sum-two-squares/](https://www.geeksforgeeks.org/check-whether-number-can-represented-sum-two-squares/)
...
我们用两个循环来表示，循环一直运行到 `n` 的平方根，每次我们都要找到循环的两个数的平方和是否等于 `n`，如果找到那个组合，那么我们就打印 `Yes`，否则就打印 `No`。
...
我们也可以在 `O(sqrt(n))` 复杂度中解决这个问题。为了解决这个问题，我们使用了一个 `hashmap`，在这个 `hashmap` 中我们将存储数字的平方直到 `sqrt(n)`。每次我们都会在 `hashmap` 中搜索 `(n - i*i)`，如果存在，那么返回 `Yes`，否则返回 `No`。
...
我们也可以在 `O(sqrt(n)log n)` 中找到这个问题，这个方法是由[萨加尔·舒克拉](https://auth.geeksforgeeks.org/user/Sagar%20Shukla)贡献的。

### 二分搜索法方法

检查 `c - a^2` 是否是完美正方形的另一种方法是利用二分搜索法。该方法与典型的二分搜索法查找数字的方法相同。唯一的区别在于我们需要在 `[0, c - a^2]` 范围内找到一个整数 `mid`，这样这个数就是 `c - a^2` 的平方根，或者换句话说，我们需要在 `[0, c - a^2]` 范围内找到一个整数 `mid`，这样 `mid * mid` = `c - a^2`。
...
要做到这一点，我们只需通过重复除法找到给定数字 `n` 的所有质因数，范围可以从`[2, sqrt(n)]`到这些因数的计数。如果在任何一步，我们发现形式的任何质因数`(4k + 3)`出现的次数是奇数次，我们可以返回一个假值。
如果 `n` 本身是质数，它就不能被`[2, sqrt(n)]`中的任何一个质数整除。因此，我们需要检查 `n` 是否可以用`(4k + 3)`的形式表示。如果是这样，我们需要返回一个假值，表示这个素数出现了奇数(1)次。
否则，我们可以返回真值。
