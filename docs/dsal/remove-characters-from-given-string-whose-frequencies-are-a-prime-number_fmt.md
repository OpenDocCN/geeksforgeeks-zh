...
> 原文: [https://www.geeksforgeeks.org/remove-characters-from-given-string-whose-frequencies-are-a-prime-number/](https://www.geeksforgeeks.org/remove-characters-from-given-string-whose-frequencies-are-a-prime-number/)
...
解决这个问题最简单的方法是找出字符串中每个不同字符的[频率](https://www.geeksforgeeks.org/print-characters-frequencies-order-occurrence/)，对于每个字符，检查其频率是否为[质数](https://www.geeksforgeeks.org/c-program-to-check-whether-a-number-is-prime-or-not/)。如果发现频率是质数，跳过那个字符。否则，将其追加到新形成的字符串中。
...
上述方法可以通过使用厄拉多塞的[筛](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)预计算[素数](https://www.geeksforgeeks.org/prime-numbers/)来优化。按照以下步骤解决问题:

*   使用厄拉多塞的筛，生成直到 `N` 的所有质数，并将其存储在数组 `prime[]` 中。
*   初始化一张[地图](https://www.geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/)，存储每个字符的[频率](https://www.geeksforgeeks.org/frequency-of-each-character-in-a-string-using-unordered_map-in-c/)。
*   然后，[遍历字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-python/)，借助地图和 `prime[]` 数组，找出哪些字符有质数频率。
