...
> 原文: [https://www.geeksforgeeks.org/largest-sum-contiguous-increasing-subarray/](https://www.geeksforgeeks.org/largest-sum-contiguous-increasing-subarray/)
...
一个简单的解决方案是[生成所有子阵列](https://www.geeksforgeeks.org/subarraysubstring-vs-subsequence-and-programs-to-generate-them/)并计算它们的和。最后返回和最大的子阵。该方案的时间复杂度为 `O(n^2)`。

一个高效的解决方案是基于所有元素都是正的事实。所以我们考虑最长递增子阵，并比较它们的和。为了增加子阵不能重叠，所以我们的时间复杂度变成了 `O(n)`。
