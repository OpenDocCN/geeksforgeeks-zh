...
解决这个问题的思路是[遍历字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-c/)找到字符串中每个字符的[出现频率，然后按其频率递增每个字符。按照以下步骤解决问题:](https://www.geeksforgeeks.org/frequency-of-each-character-in-a-string-using-unordered_map-in-c/)
*   初始化一个[数组](https://www.geeksforgeeks.org/array-data-structure/)，比如`frequency[]`，存储字符串 `S` 中每个字符的出现次数。
