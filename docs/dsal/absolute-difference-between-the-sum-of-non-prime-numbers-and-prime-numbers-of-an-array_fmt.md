...
> 原文: [https://www.geeksforgeeks.org/absolute-difference-between-the-sum-of-non-prime-numbers-and-prime-numbers-of-an-array/](https://www.geeksforgeeks.org/absolute-difference-between-the-sum-of-non-prime-numbers-and-prime-numbers-of-an-array/)
...
一个简单的解决方案是遍历数组，并不断检查每个元素是否是质数。如果数是质数，那么把它加到代表质数之和的 `S2` 和上，否则检查它是否不是 `1`，然后把它加到非质数之和上，比如说 `S1`。遍历整个数组后，取两者的绝对差值(`S1-S2`)。
**时间复杂度** : `O(Nsqrt(N))`

## 高效方法

使用厄拉多塞的[筛](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)生成数组最大元素之前的所有素数，并将其存储在哈希中。现在，遍历数组并检查该数字是否出现在哈希映射中。然后，把这些数字加起来和 `S2` 相加，如果不是 `1`，就加起来和 `S1` 相加。
遍历整个数组后，显示两者的绝对差值。
**时间复杂度** : `O(Nlog(log(N)))`
