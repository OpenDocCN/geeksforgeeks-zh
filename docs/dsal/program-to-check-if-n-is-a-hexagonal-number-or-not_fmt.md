...
> 原文: [https://www.geeksforgeeks.org/program-to-check-if-n-is-a-hexagonal-number-or-not/](https://www.geeksforgeeks.org/program-to-check-if-n-is-a-hexagonal-number-or-not/)

给定一个数 `N`，检查是否为[六边形](https://www.geeksforgeeks.org/hexagonal-number/)。如果是，则打印“是”，否则输出“否”。
...
为了解决上面提到的问题，我们必须注意到**第 n 个六边形数**由公式给出: `H(n)= n *(2n–1)`。公式表明，第 `N` 个六边形数二次依赖于 `N`，因此，求 `N = H(n)` 方程的正整数根。
