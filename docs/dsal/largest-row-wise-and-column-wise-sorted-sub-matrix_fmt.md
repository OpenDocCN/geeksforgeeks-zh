...
**方法:**解决这个问题的方法有很多，从 `O(N^3 * M^3)` 到 `O(N * M)` 不等。在本文中，将讨论一种使用[堆栈](https://www.geeksforgeeks.org/stack-data-structure/)的时间复杂度为 `O(N * M)` 的方法。
在进一步进行之前，建议先解决[这个问题](https://www.geeksforgeeks.org/largest-rectangle-under-histogram/)。
...
*   一旦我们有了这个数组，对于每个列 `j`。
    ...
    *   当 `i` 小于 `N` 时，在 `i` 上运行循环
        ...
        *   将子阵列 `pre[i][j]` 上的直方图问题应用于 `pre[k-1][j]`，以找到其下的最大区域。让我们称这个值为 `val`。将最终答案更新为 `ans = max(ans, val)`。
        *   更新 `i` 等于 `k-1`。
