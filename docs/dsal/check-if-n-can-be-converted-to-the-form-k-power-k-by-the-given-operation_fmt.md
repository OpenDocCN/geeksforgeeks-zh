# 检查 N 是否可以通过给定的操作转换成 K 的 K 次方形式

> 原文: [https://www.geeksforgeeks.org/check-if-n-can-be-converted-to-the-form-k-power-k-by-the-given-operation/](https://www.geeksforgeeks.org/check-if-n-can-be-converted-to-the-form-k-power-k-by-the-given-operation/)

给定一个`正数 N`，我们必须使用以下任意次数的运算，找出 `N` 是否可以转换成形式 `K^K`，其中 `K` 也是正整数:
...
> **输出:** 是
> **解释:**
> 对于整数 `13` 选择 `d = 3`: `N = 13–3^2 = 4`，`4` 是 `2^2` 的形式。因此，输出为 `4`。
>
> **输入:** `N = 90`
> **输出:** 否
> **说明:**
...
为了解决上面提到的问题，我们将使用[递归](https://www.geeksforgeeks.org/recursion/)。在每个递归步骤中，遍历 `N` 的当前值的所有数字，并选择它作为 `d`。这样，所有的搜索空间都将被探索，如果其中任何一个 `N` 的形式为 `K^K` 则停止递归并返回 `true`。要检查数字是否是给定的形式，请将所有这样的数字预先存储在一个集合中。该方法取 `O(D^N)`，其中 `D` 为 `N` 的位数，可以进一步优化。
