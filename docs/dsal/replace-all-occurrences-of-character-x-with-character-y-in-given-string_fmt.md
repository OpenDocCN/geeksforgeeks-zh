...
> 原文: [https://www.geeksforgeeks.org/replace-all-occurrences-of-character-x-with-character-y-in-given-string/](https://www.geeksforgeeks.org/replace-all-occurrences-of-character-x-with-character-y-in-given-string/)
...
**递归方法:** 思想是递归遍历给定的字符串，用值 `Y` 替换字符 `X`。以下是步骤:

1.  获取字符串 `str`，字符 `X`，以及 `Y`。
2.  递归地从索引 `0` 迭代到 `str.length`。
...
*   **递归调用:** 如果不满足基本大小写，则检查第 `0` 个索引处的字符，如果是 `X`，则用 `Y` 替换该字符，并递归迭代下一个字符。
