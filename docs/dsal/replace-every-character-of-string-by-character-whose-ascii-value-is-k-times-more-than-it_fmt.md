...
> 原文: [https://www.geeksforgeeks.org/replace-every-character-of-string-by-character-whose-ascii-value-is-k-times-more-than-it/](https://www.geeksforgeeks.org/replace-every-character-of-string-by-character-whose-ascii-value-is-k-times-more-than-it/)

给定字符串 `str` 仅由小写字母和整数 `k` 组成，任务是用 ASCII 值比它大 `k` 倍的字符替换给定字符串的每个字符。如果 ASCII 值超过 `'z'`，则以循环方式从 `'a'` 开始检查。
