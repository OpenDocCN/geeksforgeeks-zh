...
一个**简单的解决方案**是为每个查询找到整个子矩阵的异或。因此，每个查询的最坏情况时间复杂度将是 `0(n^2)`。

## 高效方法:

我们都很熟悉线性阵列上前缀异或的思想，即

> 如果 `arr[] = {1，2，3，4}` 并且我们计算 `prefixXOR[] = {1，3，0，4}`，其中 `prefixXOR[i]` 存储从 `arr[0]` 到 `arr[i]` 的值的 XOR，那么子阵列 `arr[i]` 到 `arr[j]` 的 XOR 可以被发现为 `prefixXOR[j]^ prefixXOR[i–1]`
...
一旦我们有了所需的前缀异或矩阵，我们就可以简单地回答查询。从 `(R1，C1)` 到 `(R2，C2)` 的子矩阵的 XOR 可以计算为 `prefix_xor[R2][C2] ^ prefix_xor[R1–1][C2] ^ prefix_xor[R2][C1–1] ^ prefix_xor[R1–1][C1–1]`。
