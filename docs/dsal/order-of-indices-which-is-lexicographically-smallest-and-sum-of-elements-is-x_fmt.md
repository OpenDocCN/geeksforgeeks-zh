...
> 原文：[https://www.geeksforgeeks.org/order-of-indices-which-is-lexicographically-smallest-and-sum-of-elements-is-x/](https://www.geeksforgeeks.org/order-of-indices-which-is-lexicographically-smallest-and-sum-of-elements-is-x/)
...
> 所选索引`A[0] + A[2] = 6 + 5 = 11`之和≤ `11`。
> 这里，`[2，3]`、`[2，2]`或`[3，3]`也给出了最大数量的所选索引，但它们在词典学上不是最小的。
...
设第一个最小值的索引为`m`，那么所做索引的最大值为`n = X / A[m]`。所以`ans = [m，m，m，…，m]`，其中`m`的个数为`n`，是选择的最大索引个数的解，总和= `n×A[m]`。我们也确定最优答案中有`n`个值，并且不超过这个值。
...
> 例如，`X = 11`，`A = [6，8，5，4，7]`。
> 最小值在索引`3`，`A[3] = 4`。所以`n = 11 / 4 = 2`，`ans = [3，3]`，总和= `4 x 2 = 8`。
