...
> 原文：[https://www.geeksforgeeks.org/pairwise-swap-leaf-nodes-binary-tree/](https://www.geeksforgeeks.org/pairwise-swap-leaf-nodes-binary-tree/)
...
现在，如果我们仔细观察，那么我们看到，当遍历时，如果遍历的叶节点数是偶数，这意味着我们可以形成一对叶节点。为了跟踪这一对，我们采用两个指针`firstPtr`和`secondPtr`，如上所述。每次我们遇到一个叶节点，我们就用这个叶节点初始化`secondPtr`。现在如果`count`为奇数，我们用`secondPtr`初始化`firstPtr`，否则我们简单地交换这两个节点。
