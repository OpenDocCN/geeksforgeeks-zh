...
> 原文: [https://www.geeksforgeeks.org/find-permutation-of-first-n-natural-numbers-that-satisfies-the-given-condition/](https://www.geeksforgeeks.org/find-permutation-of-first-n-natural-numbers-that-satisfies-the-given-condition/)
...
保持最后 `K` 个元素就位。移动其余的元件，使得第 `i` 个元件放置在第 `(i + 1)` 个位置，并且第 `(N–K)` 个元件保持在位置 `1`，因为 `gcd(x, x + 1) = 1`。
