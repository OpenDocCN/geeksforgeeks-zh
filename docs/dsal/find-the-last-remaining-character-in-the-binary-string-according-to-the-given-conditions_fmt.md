...
> 原文: [https://www.geeksforgeeks.org/find-the-last-remaining-character-in-the-binary-string-according-to-the-given-conditions/](https://www.geeksforgeeks.org/find-the-last-remaining-character-in-the-binary-string-according-to-the-given-conditions/)
...
1.  所有数字都被添加到队列中。
2.  两个计数器被保持为大小为 `2` 的数组 `del`，这将表示每个数字存在的浮动删除数。
3.  遍历队列，直到这两种类型中至少有一个数字存在。
4.  那么对于队列中的每个数字，如果该数字的 `del` 计数器不是 `0`，则删除该数字。
5.  否则，相反数字的 `del` 计数器递增并放回队列。
