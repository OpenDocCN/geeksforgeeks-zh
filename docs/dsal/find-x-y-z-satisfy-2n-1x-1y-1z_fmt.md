...
> 原文: [https://www.geeksforgeeks.org/find-x-y-z-satisfy-2n-1x-1y-1z/](https://www.geeksforgeeks.org/find-x-y-z-satisfy-2n-1x-1y-1z/)

给定 `n`，求 `x`，`y`，`z`，使 `x`，`y`，`z` 满足等式 `2/n = 1/x+1/y+1/z`。
有多个 `x`，`y` 和 `z` 满足等式，打印其中任何一个，如果不可能，则打印 `-1`。
