...
给定一个长度为 `N` 的 `arr[]` 数组，任务是找到包含负整数的数组的大小为 `K` 的子数组的最小乘积。
...
**方法:**给定的问题可以使用两点技巧和滑动窗口方法来解决。可以遵循以下步骤来解决问题:

*   迭代数组 `arr` 直到 `K-1`，将每一个非零数字相乘，找到乘积，并计算子数组中的零的数量。
*   迭代数组 `arr` 从 `K` 直到数组结束，每次迭代:
