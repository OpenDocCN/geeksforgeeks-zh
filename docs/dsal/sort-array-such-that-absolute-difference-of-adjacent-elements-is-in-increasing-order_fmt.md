...
> 原文：[https://www.geeksforgeeks.org/sort-array-such-that-absolute-difference-of-adjacent-elements-is-in-increasing-order/](https://www.geeksforgeeks.org/sort-array-such-that-absolute-difference-of-adjacent-elements-is-in-increasing-order/)

给定一个长度为 `N` 的未排序数组，任务是对数组进行排序，使得所有 `0 <= i < N-1` 满足 `ABS(arr[i]-arr[i+1]) <= ABS(arr[i+1]-arr[i+2])`，即 `ABS(arr[0]-arr[1]) <= ABS(arr[1]-arr[2]) <= ABS(arr[2]-arr[3])` 等等。
...
为了解决上面提到的问题，我们按照升序对给定的未排序数组进行排序。然后运行从 `i = 1` 到 `i < n/2` 的循环，并分别从前半部分和后半部分交替地在**堆栈**中推送元素，即推送一次 `a[i]` 和一次 `a[n-i-1]`，直到整个数组元素被推入堆栈。
