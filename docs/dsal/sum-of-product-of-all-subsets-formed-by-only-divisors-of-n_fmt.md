...
> 原文: [https://www.geeksforgeeks.org/sum-of-product-of-all-subsets-formed-by-only-divisors-of-n/](https://www.geeksforgeeks.org/sum-of-product-of-all-subsets-formed-by-only-divisors-of-n/)
...
**天真方法:** 这个问题的天真方法是[从其除数生成所有可能的子集](https://www.geeksforgeeks.org/find-distinct-subsets-given-set/)，然后计算每个子集的乘积。计算每个子集的乘积后，将所有乘积相加，找到所需的答案。这种方法的时间复杂度是 `O(2^D)`，其中 `D` 是 `n` 的除数。
...
显然，从上面的观察，我们可以得出结论，如果 `{D1, D2, D3, … Dn}` 是 `N` 的除数，那么要求的答案将是:
