...
**方法:** 因为已知一个数在给定大小的数组 `N` 的所有子集中出现 `2^(N-1)` 次。所以如果一个数 `X` 是质数，那么 `X` 的贡献将是 `X * X * X * …..* 2^(N-1)` 次，即

![](img/1ac84cfe8594a6de74ef95b3f10542e1.png)

由于 `2^(N-1)` 也将是一个很大的数字，不能直接计算。[费马定理](https://www.geeksforgeeks.org/find-abm-where-b-is-very-large/)将用于计算此处的功率。
