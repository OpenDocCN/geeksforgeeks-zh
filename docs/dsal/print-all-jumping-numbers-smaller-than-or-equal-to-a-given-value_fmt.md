...
一个简单的解决方法是遍历从 0 到 `x` 的所有数字，对于每个遍历的数字，检查它是否是一个跳跃的数字。如果是，那就打印出来。否则忽略它。该解决方案的时间复杂度为 `O(x)`。

一个高效解可以在 `O(k)` 时间内解决这个问题，其中 `k` 是小于或等于 `x` 的跳数个数，思路是使用 [BFS](https://www.geeksforgeeks.org/breadth-first-traversal-for-a-graph/) 或 [DFS](https://www.geeksforgeeks.org/depth-first-traversal-for-a-graph/) 。
