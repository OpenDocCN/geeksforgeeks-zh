...
> 原文: [https://www.geeksforgeeks.org/largest-palindromic-prime-in-an-array/](https://www.geeksforgeeks.org/largest-palindromic-prime-in-an-array/)
...
> **输出:** 11
> 11，5 和 7 是数组中唯一的回文素数。其中 11 个最大。
> **输入:** `arr[] = {2, 4, 6, 8, 10}`
> **输出:** 2

一个简单的方法是遍历每一个数组元素，[检查是否是素数](https://www.geeksforgeeks.org/primality-test-set-1-introduction-and-school-method/)，[检查是否是回文](https://www.geeksforgeeks.org/check-if-a-number-is-palindrome/)。如果是，如果结果也大于当前结果，则更新结果。

## 大量元素的高效方法

*   使用厄拉多塞的[筛计算一个数是否是素数，直到数组中的最大元素。](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)
*   现在，初始化一个变量 `currentMax = -1` 并开始遍历数组 `arr[]`。
*   对于每一个 `i`，如果 `arr[i]` 是 `prime` 以及 `回文` 和 `arr[i] > currentMax` 那么更新 `currentMax = arr[i]`。
*   最后打印 `currentMax`。
