...
> 原文: [https://www.geeksforgeeks.org/queries-for-dfs-of-a-subtree-in-a-tree/](https://www.geeksforgeeks.org/queries-for-dfs-of-a-subtree-in-a-tree/)
...
- 一个节点的子树的 DFS 将永远是一个从节点（比如索引 `ind`）到 `(ind + under[node])` 的连续子数组。
- 使用散列法获取已存储的节点的索引，并打印原始 DFS 中的节点，直到索引等于 `ind + under[node]`。
