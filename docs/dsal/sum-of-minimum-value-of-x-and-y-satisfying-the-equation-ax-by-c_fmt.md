# 满足方程 ax + by = c 的 x 和 y 的最小值之和

> 原文: [https://www.geeksforgeeks.org/sum-of-minimum-value-of-x-and-y-satisfying-the-equation-ax-by-c/](https://www.geeksforgeeks.org/sum-of-minimum-value-of-x-and-y-satisfying-the-equation-ax-by-c/)

给定表示为 `ax + by = c` 的线性方程的三个整数 `a`、`b` 和 `c`，任务是找到给定方程的解 `(x, y)`，使得 `(x + y)` 最小化。如果上述方程不存在解，则打印 `-1`。
**注:** `x` 和 `y` 为非负整数。
...
为了解决上述问题，找到给定的[线性丢番图方程](https://www.geeksforgeeks.org/linear-diophantine-equations/)的任意解 `(x, y)`，然后相应地找到 `x` 的值并最小化和。
下面是给定方程的解 `(x', y')`:

> ![x' = x + k * \frac{b}{g}](img/c0567e22a1870a5126bd84208e4dd338.png "Rendered by QuickLaTeX.com")
> ![y' = y - k * \frac{a}{g}](img/7dcc58f95dba9da012cf99dfe84994a7.png "Rendered by QuickLaTeX.com")
>
> 其中 `g` 是 `gcd(a, b)`，`k` 是任意整数。
>
> ![x' + y' = x + y + k*(\frac{b}{g} - \frac{a}{g})](img/7dcc58f95dba9da012cf99dfe84994a7.png "Rendered by QuickLaTeX.com")
