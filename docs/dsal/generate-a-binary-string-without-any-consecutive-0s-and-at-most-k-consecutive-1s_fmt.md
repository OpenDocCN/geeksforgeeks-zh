...
> 原文: [https://www.geeksforgeeks.org/generate-a-binary-string-without-any-consecutive-0s-and-at-most-k-consecutive-1s/](https://www.geeksforgeeks.org/generate-a-binary-string-without-any-consecutive-0s-and-at-most-k-consecutive-1s/)

给定两个整数 `N` 和 `M`，任务是用以下条件构造一个二进制字符串：

*   二进制字符串由 `N` 个 `0` 和 `M` 个 `1` 组成。
*   二进制字符串最多有 `K` 个连续的 `1`。
*   二进制字符串不包含任何相邻的 `0`。
...
*   因此，对于 `N` 个 `0`，至少应有 `N-1` 个 `1` 存在，以生成所需类型的字符串。
*   由于不超过 `K` 个连续的 `1` 可以放在一起，对于 `N` 个 `0`，最多可以有 `(N+1) * K` 个 `1` 个可能。
*   因此，`1` 的数量应该在范围内：
...
*   否则，请按照以下步骤解决问题：
    *   将 `0` 附加到最终字符串中。
    *   在每对 `0` 之间插入 `1`，从 `M` 中减去 `N–1`，因为 `N–1` 个 `1` 已经放置。
    *   对于剩余的 `1`，将 `min(K–1, M)` 个 `1` 放置在每个已放置的 `1` 旁边，以确保不超过 `K` 个 `1` 放置在一起。
...
// Function to construct the binary string
static String ConstructBinaryString(int N, int M, int K)
{
