...
> 原文: [https://www.geeksforgeeks.org/how-to-determine-if-a-binary-tree-is-balanced/](https://www.geeksforgeeks.org/how-to-determine-if-a-binary-tree-is-balanced/)
...
以上高度平衡方案用于 `AVL` 树。下图显示了两棵树，其中一棵树是高度平衡的，另一棵树不是。第二棵树是高度不均衡的，因为左边子树的高度比右边子树的高度多 2。
...
`O(n^2)`在[全二叉树](https://www.geeksforgeeks.org/binary-tree-set-3-types-of-binary-tree/)的情况下。

### 优化实现

上述实现可以通过在同一个递归中计算高度来优化，而不是单独调用`height()`函数。感谢 Amar 提出这个优化版本。这种优化将时间复杂度降低到`O(n)`。
