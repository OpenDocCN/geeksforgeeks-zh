...
*   **第二步:**找出极小极大和马希民值。
    ...
    由于这款游戏的 `minimax` 和 `马希民值` 不相等，所以这款游戏没有鞍点。
...
*   **Step 4:** Now find the probabilities for each row.
    ...
    **计算概率:**对应的 `odds`/行或列 `odds` 之和，即
    对于 `P1`，行 1 的 `odd` 值为 6，两行 `odd` 值之和为 8，所以 `P1 = 6/(6+2) = 3/4`
    对于 `P2`，行 2 的 `odd` 值为 2，两行 `odd` 值之和为 8，所以 `P2 = 2/(6+2) = 1/4`
...
*   **第五步:**找到游戏的价值。
    ...
    *不使用公式*
    有 4 种方法可以找到游戏的价值。
    取第一列。现在将第一列的元素与相应的行 `odd` 相乘，然后将两次相乘相加，再除以行的总 `odd`。
    `V =(9 * 6+5 * 2)/(6+2)=(54+10)/8 = 64/8 = 8`。
    **OR、**
    取第二列。现在将第二列的元素与相应的行 `odd` 相乘，然后将两次相乘相加，然后除以行的总 `odd`。
    `V =(7 * 6+11 * 2)/(6+2)=(42+22)/8 = 64/8 = 8`。
    **OR、**
    拿第一排。现在将第一行的元素与相应的列 `odd` 相乘，然后将两次相乘相加，再除以列的总 `odd`。
    `V =(9 * 4+7 * 4)/(4+4)=(36+28)/8 = 64/8 = 8`。
    **OR，**
    取第二排。现在将第二行的元素与相应的列 `odd` 相乘，然后将两次相乘相加，再除以列的总 `odd`。
