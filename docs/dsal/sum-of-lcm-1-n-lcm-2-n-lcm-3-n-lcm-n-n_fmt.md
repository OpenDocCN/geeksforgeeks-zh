...
> 原文: [https://www.geeksforgeeks.org/sum-of-lcm-1-n-lcm-2-n-lcm-3-n-lcm-n-n/](https://www.geeksforgeeks.org/sum-of-lcm-1-n-lcm-2-n-lcm-3-n-lcm-n-n/)
...
两个数的 LCM `a` 和 `b` = `(a * b) / gcd(a，b)`，其中 `gcd(a，b)` 是 `a` 和 `b` 的最大公约数。
...
使用[欧拉全能性函数](https://www.geeksforgeeks.org/eulers-totient-function/)，
`∑LCM(i，n) = (∑(d * ETF(d)) + 1) * n / 2`
其中 `ETF(d)` 是 `d` 的欧拉全能性函数，`d` 属于 `n` 的除数集合。
