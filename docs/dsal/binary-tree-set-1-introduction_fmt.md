...
### 树:
与数组、链表、栈和队列是线性数据结构不同，树是分层数据结构。

### 树词汇:
最顶端的节点叫树根。直接位于元素下的元素称为其子元素。某物正上方的元素称为它的父元素。例如，“a”是“f”的子代，“f”是“a”的父代。最后，没有子元素的元素叫做叶子。
...
### 二叉树:
元素最多有 2 个子元素的树称为二叉树。由于二叉树中的每个元素只能有两个子元素，我们通常将它们命名为左右子元素。

### C 语言中的二叉树表示法:
树是由指向树中最顶端节点的指针来表示的。如果树为空，则根的值为空。
树节点包含以下部分。
1. 数据
2. 指针指向左子
