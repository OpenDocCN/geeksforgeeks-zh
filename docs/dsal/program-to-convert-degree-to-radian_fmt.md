...
> 原文：[https://www.geeksforgeeks.org/program-to-convert-degree-to-radian/](https://www.geeksforgeeks.org/program-to-convert-degree-to-radian/)

给定`degree`中的角度，任务是将其转换为`radian`。
...
**度：** 度通常用（度符号）表示，是平面角度的度量，定义为整个旋转为 360 度。因为完整的旋转等于 `2 * pi` 弧度，所以一个度数相当于 `pi/180` 弧度。
弧度转换成度数的公式为：
