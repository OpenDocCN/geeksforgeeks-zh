...
> 原文: [https://www.geeksforgeeks.org/check-if-a-string-is-made-up-of-k-alternating-characters/](https://www.geeksforgeeks.org/check-if-a-string-is-made-up-of-k-alternating-characters/)
...
为了使字符串由 `K` 个交替字符组成，它必须满足以下条件:

1.  `(i + mK)` 索引处的所有字符必须相同，其中 `i` 是当前索引，`mK` 代表 `K` 的 `m` 次倍数，这意味着每 `K` 个索引后，字符必须重复。
