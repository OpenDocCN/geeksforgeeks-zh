...
> 原文: [https://www.geeksforgeeks.org/find-product-of-all-elements-at-indexes-which-are-factors-of-m-for-all-possible-sorted-subsequences-of-length-m/](https://www.geeksforgeeks.org/find-product-of-all-elements-at-indexes-which-are-factors-of-m-for-all-possible-sorted-subsequences-of-length-m/)
...
由于不可能找到所有集合，所以想法是统计每个元素在所需位置出现的总次数。如果找到计数，则:

> 乘积 = `(arr[1]^count) * (arr[2]^count) * ….. * (arr[n]^count) % (10^9 + 7)`

*   为了找到 `arr[i]` 的计数，我们必须找到不同集合的总数，这些集合可以通过将 `arr[i]` 放在每个可能的索引处（将 `M` 完全除）来形成。
*   因此，将 `arr[i]` 放置在 `j` 第 `j` 整除 `m` 索引处形成的集合数将为:

> `C(小于 arr[i] 的元素数量, j-1) * C(大于 arr[i] 的元素数量, M-j)`

*   由于任何元素的计数都可能很大，所以求 `(arr[i]^count) % (10^9 + 7)`，[使用费马小定理](https://www.geeksforgeeks.org/fermats-little-theorem/)。
