...
> 原文: [https://www.geeksforgeeks.org/check-if-any-k-ranges-overlap-at-any-point/](https://www.geeksforgeeks.org/check-if-any-k-ranges-overlap-at-any-point/)

给定 `N` 个范围 `[L，R]` 和整数 `K`，任务是检查是否有任何 `K` 个范围在任何点重叠。

**示例:**

> **输入:** `ranges[][] = {{1，3}，{2，4}，{3，4}，{7，10}}`，`K = 3`
> **输出:** 是
> 3 个范围 `{1，3}`，`{2，4}` 和 `{3，4}` 中的一个公共点。
>
> **输入:** `ranges[][] = {{1，2}，{3，4}，{5，6}，{7，8}}`，`K = 2`
> **输出:** 否
