...
> 原文：[https://www.geeksforgeeks.org/biggest-number-by-arranging-numbers-in-certain-order/](https://www.geeksforgeeks.org/biggest-number-by-arranging-numbers-in-certain-order/)
...
这个问题是[将给定的数字排列成最大的数字](https://www.geeksforgeeks.org/given-an-array-of-numbers-arrange-the-numbers-to-form-the-biggest-number/)问题的变型。在这个问题中，我们找到了具有一定限制的最大数，即奇数和偶数的顺序需要在最终结果中保持，因此目标是有一个 `O(n)` 时间复杂度的解。

以下是寻找保持奇数和偶数顺序的最大数的步骤。

1.  将原数组的个数分成 2 个数组 `even[]` 和 `odd[]`。同时划分数字的顺序应该保持。
2.  合并 `even` 和 `odd` 数组，合并时遵循条件。设 `X` 是一个数组的元素，`Y` 是另一个数组的元素。比较 `XY`（`X` 后加 `Y`）和 `YX`（`Y` 后加 `X`）。如果 `XY` 较大，则将 `X` 加到最终结果上，否则将 `Y` 加到最终结果上。
