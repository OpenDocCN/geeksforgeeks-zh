...
> 原文：[https://www.geeksforgeeks.org/count-arithmetic-progressions-having-sum-n-and-common-difference-equal-to-1/](https://www.geeksforgeeks.org/count-arithmetic-progressions-having-sum-n-and-common-difference-equal-to-1/)
...
*   一个 AP 系列的和由公式 `S = (N/2) * (2 * A + N - 1)` 给出，其中第一项为 `A` 而公差为 `1`。
*   求解上述方程：
...
*   初始化一个变量，比如 `count`，来存储给定条件下可能的 AP 系列的数量。
*   遍历 `2 * N` 的所有因子，对于每个 `i` 因子，检查 `(2 * N / i) - i + 1` 是否为偶数。
*   如果发现为真，则增加 `count`。
*   打印 `count - 1` 作为所需答案，因为仅由 `N` 组成的序列需要丢弃。
