# 查询数组的范围【L，R】内是否存在任何非重复元素

> 原文: [https://www.geeksforgeeks.org/queries-to-check-if-any-non-repeating-element-exists-within-range-l-r-of-an-array/](https://www.geeksforgeeks.org/queries-to-check-if-any-non-repeating-element-exists-within-range-l-r-of-an-array/)

给定一个由整数和查询组成的 `(L, R)` 形式的数组 `arr[]`，任务是检查索引 `[L, R]`（*基于 1 的索引*）中是否存在任何非重复元素。如果至少有一个非重复元素，则打印 `"Yes"`。否则，打印 `"No"`。
...
> **输出:** No Yes
> **解释:**
> 对于第一个查询，子数组是 `{1, 2, 1, 2}`，我们可以看到两个数都有频率 2。所以答案是 No。
> 对于第二个查询，子数组是 `{1, 2, 1, 2, 3}`，我们可以看到 3 有频率 1，所以答案是 Yes。
>
> **输入:** `arr[] = {1, 2, 3, 4, 5}`, `query[][] = { {1, 4} }`
> **输出:** Yes
> **解释:** 子数组为 `{1, 2, 3, 4}`，所有元素频率为 1，所以答案为 Yes。

## 天真方法

解决这个问题最简单的方法是为每个查询迭代一个给定的子数组，并维护一个 `map` 来存储每个元素的频率。迭代 `map`，检查是否有频率为 1 的元素。
...
解决方案的关键观察是，对于给定数组中频率为 1 的元素，数组中该数字的**前一次出现**严格小于查询的 `l`，元素的**后一次出现**严格大于某个查询的 `r`。用这个观察来寻找秩序。以下是使用合并排序树方法解决给定问题的步骤:

1.  将数组中每个带有元素的**前一次出现和下一次出现**存储为 `pair`。
2.  构建合并排序树，并根据前一个匹配项合并其中的节点。`merge` 功能用于合并范围。
