...
注意: 探针工具是测量工具，如果可以达到 6 根管子，但是探针工具的长度是 4 根。如果可以到达 3 个管道，但探测工具的长度为 4，则答案为 4。答案将是 3。
...
找到一种方法，通过元素进行处理，找到下一组可以访问的元素 (遍历图表)。应该注意的是，我们希望到达距离小于 `L` 的从起始节点可到达的所有节点。重点不是到达很多节点，而是从起点 `(X, Y)` 以较少的移动到达它们。
...
在每次迭代中，取出队列的一个成员，并分析其相连的邻居。如果邻居未被访问，并且当前元素的深度不大于 `L`，则连接的元素被放入队列，标记为已访问，并且已访问的节点数增加 `1`。
...
# search for connection
# We start by pushing the start node (X, Y) into a queue
# with depth 1 and then start processing the queue till it is empty.
def `bfs(x, y, d)`:
    `push(x, y, d)`
    while (`r` > `f`) :
        `temp` = `pop()`
...
if `__name__`=='__main__': # main function
    ...
    for `i` in `range(row_size)` :
        for `j` in `range(col_size)` :
            # visited matrix for BFS set to unvisited for every cell
            `vi[i][j]` = 0
            # depth set to max initial value
            `depth[i][j]` = `Max`
