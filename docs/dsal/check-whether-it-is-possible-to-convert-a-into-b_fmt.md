...
> 原文: [https://www.geeksforgeeks.org/check-whether-it-is-possible-to-convert-a-into-b/](https://www.geeksforgeeks.org/check-whether-it-is-possible-to-convert-a-into-b/)

给定两个整数 `A` 和 `B`。任务是检查是否可以通过多次执行以下操作将 `A` 转换成 `B`。
...
在其他情况下(如果 `B` 以除 1 以外的`奇数`结尾)，答案是`否`。
