...
> 原文: [https://www.geeksforgeeks.org/sum-of-maximum-element-minimum-element-for-all-the-subsets-of-an-array/](https://www.geeksforgeeks.org/sum-of-maximum-element-minimum-element-for-all-the-subsets-of-an-array/)
...
一个**天真的解决方案**是生成所有子集，遍历每个子集，找到最大和最小元素，并将它们的差加到当前和上。这个解决方案的时间复杂度是 `O(n * 2^n)`。
...
> =`(4–3)+(4–1)+(3–1) +(4–1)`
> 前 3 个“V”值可以忽略，因为它们的计算结果为 0 (因为它们来自 1 大小的子集)。
> 重新排列总和，我们得到:
> =`(4–3)+(4–1)+(3–1)+(4–1)`
...
例如，我们分析值为 6 的元素。3 个元素小于 6，4 个元素大于 6。因此，如果我们考虑所有子集，其中 6 与 3 个较小的元素一起出现，那么在所有这些子集中，6 将是最大的元素。这些子集的数量将为 `2^3`。当 4 个元素大于 6 时，6 是最小元素，类似的论点也成立。

> 因此，
> 元素在所有子集中出现次数最大= `2^位置–1`
> 元素在所有子集中出现次数最小= `2^(n–1–位置)–1`
> 其中 `位置` 是排序数组中元素的索引。
