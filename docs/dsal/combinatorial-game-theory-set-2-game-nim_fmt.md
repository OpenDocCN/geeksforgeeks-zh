...
> 原文: [https://www.geeksforgeeks.org/combinatorial-game-theory-set-2-game-nim/](https://www.geeksforgeeks.org/combinatorial-game-theory-set-2-game-nim/)
...
> "*给定一定数量的堆，其中每堆包含一定数量的石头/硬币。在每一回合，玩家只能选择一堆，并从该堆中移除任意数量的石头(至少一个)。不能移动的玩家被认为输掉游戏(即拿走最后一块石头的玩家是赢家)。*”
...
那么 `A` 在这个游戏中有很强的专长吗？还是他/她先发制人比 `B` 有优势？
...
游戏任意一点每堆/堆中硬币/石头数量的累计 XOR 值在该点称为尼姆-Sum。*否则，如果尼姆-Sum 评估为零，那么玩家 `A` 肯定会输。”*

上述定理的证明见 - [https://en.wikipedia.org/wiki/Nim#Proof_of_the_winning_formula](https://en.wikipedia.org/wiki/Nim#Proof_of_the_winning_formula)

## 最优策略

*   如果“n”个数字的异或和已经为零，则不可能通过一个数字的单个减少使异或和为零。
...
在下面的程序中，我们玩了电脑和人类(用户)之间的 Nim-Game。下面的程序使用了两个功能：
*   `knowWinnerBeforePlaying()`: 在玩之前告知结果。
...
当前游戏状态 -> 3 4 5
计算机从索引为 0 的堆中移除 2 颗石子。
当前游戏状态 -> 1 4 5
人类从索引为 1 的堆中移除 3 颗石子。
当前游戏状态 -> 1 1 5
计算机从索引为 2 的堆中移除 5 颗石子。
当前游戏状态 -> 1 1 0
人类从索引为 1 的堆中移除 1 颗石子。
当前游戏状态 -> 1 0 0
计算机从索引为 0 的堆中移除 1 颗石子。
当前游戏状态 -> 0 0 0
