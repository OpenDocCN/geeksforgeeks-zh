...
> 原文：[https://www.geeksforgeeks.org/largest-n-digit-octal-number-which-is-a-perfect-square/](https://www.geeksforgeeks.org/largest-n-digit-octal-number-which-is-a-perfect-square/)
...
正如我们所知，当一个数大于 `8^k` 时，八进制中的位数增加，其中 `k` 表示数中的位数。所以对于八进制数字系统中的任何 `N` 位数，都必须小于 `8^(N+1)` 的值。因此，利用这一观察可以得出的一般术语是–

> `N` 位八进制数 = `octal(pow(ceil(sqrt(pow(8, N))) - 1, 2))`
