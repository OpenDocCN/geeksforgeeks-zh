...
> 原文: [https://www.geeksforgeeks.org/java-program-to-find-a-good-feedback-vertex-set-in-a-graph/](https://www.geeksforgeeks.org/java-program-to-find-a-good-feedback-vertex-set-in-a-graph/)
...
class Edge
{
    ...
    public void set_Edge(int end, int start)
    {
        ...
        if (end > adjacentList.size() || start > adjacentList.size())
    ...
    public Edge check()
    {
        ...
        Iterator<Integer> iterator = this.adjacentList.keySet().iterator();
        ...
        while (iterator.hasNext()) {
            ...
            List<Integer> adjList = this.adjacentList.get(i);

            // checking for equal size we would return the same
            if (count == size) {
                return this;
            }
            ...
            if (adjList.size() == 0) {
                ...
                Iterator<Integer> iteratorR = this.adjacentList.keySet().iterator();

                // Again iterating completely over the new Iterator
                while (iteratorR.hasNext()) {
                    // Having the new R , help us to iterate the new Iterator
                    Integer R = iteratorR.next();

                    // New List is taken
                    List<Integer> lit = this.adjacentList.get(R);
```
...
在构建图时，以下代码片段用于处理邻接表，确保没有重复值：

```java
// This if condition will help not to
// have any duplicate values inside the new List
if (lit.contains(i)) {
...
在迭代过程中，会移除已处理的顶点对应的邻接表条目：

```java
// The below line would help us to remove the values
// Of the adjacent List as we move on to make the vertices
// The other values are simultaneously removed.
this.adjacentList.remove(i);

iterator = this.adjacentList.keySet().iterator();
```

最终，`printGraph()` 方法用于打印图的结构：
...
public void printGraph() {
    ...
    // Now let us take an iterator and try to print it
    // This iterator contains the values of the new graph
    Iterator<Integer> iterator = this.adjacentList.keySet().iterator();
    ...
    while (iterator.hasNext()) {
        ...
        // Taking a list which will help us have the side edges.
...
`getFeedbackArcSet()` 方法用于查找并输出图中的反馈弧集：
        ...
        if (list.size() != 0) {
            ...
            for (int j = 0; j < list.size(); j++) {
                ...
                if (visited[list.get(j)] == 1) {
                    flag = true;
                    // Now taking the output of feedback arc
...
`GFG` 类包含 `main` 方法，用于演示图的创建、打印以及反馈弧集的检测：

```java
// Now let us declare the class GFG
class GFG {
    public static void main(String[] args) {
        // Now let us declare and initialize all the variables
        ...
        try {
            ...
            if (modified.getFeedbackArcSet(vertices) == false) {
