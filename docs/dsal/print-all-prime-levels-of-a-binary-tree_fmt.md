...
> 原文: [https://www.geeksforgeeks.org/print-all-prime-levels-of-a-binary-tree/](https://www.geeksforgeeks.org/print-all-prime-levels-of-a-binary-tree/)

给定一个`二叉树`，任务是打印该树的所有`质数`。
...
为了检查一个级别是否是 `Prime` 级别，

*   我们首先需要对二叉树进行`级的顺序遍历`，得到每一级的节点值。
*   这里一个`队列数据结构`用于存储树的级别，同时进行级别顺序遍历。
*   然后对于每一级，`检查是否是质数`级。
...
# or not
def isLevelPrime(queue, index, size):
    for i in range(index, size):
        # Check value of node is
        # pPrime or not
...
# Utility function to get Prime
# Level of a given Binary tree
def findPrimeLevels(node, queue, index, size):
    # Print root node value, if Prime
    if (isPrime(queue[index].key)):
