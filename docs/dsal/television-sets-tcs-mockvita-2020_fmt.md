...
> 原文: [https://www.geeksforgeeks.org/television-sets-tcs-mockvita-2020/](https://www.geeksforgeeks.org/television-sets-tcs-mockvita-2020/)

## 问题描述

毗湿奴医生正在一个小镇上开设一家新的世界级医院，旨在成为该市患者的首选。医院有带电视和不带电视两种类型的 `N` 房间，每日房价分别为 `R1` 和 `R2`。然而，从他的经验来看，毗湿奴医生知道病人的数量不是全年不变的，而是遵循一种模式。一年中任何一天的患者人数由以下公式给出:
...
思路是遍历全年，每天产生收益。找出第一年每一个可能有电视的房间的收入。按照以下步骤解决问题:

1.  假设有电视的房间总数为 `tvs`，其中 `0 ≤ tvs ≤ N`。
2.  对于每一个数字的 `tvs`，第一年的收入可以通过每天的遍历找到。
