...
> 原文：[https://www.geeksforgeeks.org/find-the-original-array-using-xor-values-of-all-adjacent-elements/](https://www.geeksforgeeks.org/find-the-original-array-using-xor-values-of-all-adjacent-elements/)

给定一个序列`arr[]`，它包含`N-1`个元素，代表一个数组中所有相邻对的异或值，任务是从`arr[]`中找到原始的**数组**。
**注：** 给出的`N`总是奇数，`arr[]`包含`N`个自然数的[排列](https://www.geeksforgeeks.org/find-the-good-permutation-of-first-n-natural-numbers/)。
...
3.  现在对于元素的其余部分，连续地，对最后一个元素进行异或运算，我们将得到最后一个第二个元素，即 `d`。
4.  反复更新最后一个元素，最后得到第一个元素，即 `a`。
