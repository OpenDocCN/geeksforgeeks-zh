...
一个数 `N` 如果 `N` 是一个[丰数](https://www.geeksforgeeks.org/abundant-number/)而它所有的真因数都是[亏数](https://www.geeksforgeeks.org/deficient-number/)，那么这个数就是本原丰数。
前几个原始丰富数是:
...
> **输出:** YES
> **解释:**
> 20 的真因数之和为 `1+2+4+5+10 = 22 > 20`，
> 所以，20 是一个丰数。
> 1、2、4、5 和 10 的真因数分别是 0、1、3、1 和 8，
> 这些数都是亏数。
> 因此，20 是本原丰数。
> **输入:** `N = 17`
> **输出:** NO
...
1.  检查该数是否为[丰数](https://www.geeksforgeeks.org/abundant-number/)，即由 `sum(N)` 表示的数的所有真因数之和大于数 `N` 的值。
2.  如果不是丰数，则返回 `false`，否则执行以下操作。
3.  检查 `N` 的所有真因数是否都是[亏数](https://www.geeksforgeeks.org/deficient-number/)，即 `divisorSum(N)` 所表示的数的所有因数之和小于 `N` 值的两倍。
4.  如果以上两个条件都成立，打印 `YES` 否则打印 `NO`。
...
**参考文献:** [https://en.wikipedia.org/wiki/Primitive_abundant_number](https://en.wikipedia.org/wiki/Primitive_abundant_number)