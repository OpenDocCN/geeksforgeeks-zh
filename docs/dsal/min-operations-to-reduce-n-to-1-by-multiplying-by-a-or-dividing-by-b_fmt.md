...
*   如果 `B%A != 0` ，那么只有当 `N` 完全被 `B` 整除，并且需要 `N/B` 步骤才能将 `N` 转换为 `1` 。而如果 `N = 1` 则需要 `0` 步，否则不可能打印 `-1`。
*   如果 `B%A == 0` ，那么考虑一个变量 `C` ，其值为 `B/A` 。用 `B` 除 `N`，用第二次运算直到不能再除为止，我们把除法的个数叫做 `x`。
*   再把剩下的 `N` 除以 `C` 直到不能再除为止，我们把这个运算中的除法数叫做 `y`。
*   如果上述操作后 `N` 不等于 `1`，则无法使用上述操作将 `N` 转换为 `1` ，答案将是 `-1`，但如果它等于 `1`，则我们可以使用公式 `total_steps = x+(2 * y)` 来计算所需的总最小步数。
