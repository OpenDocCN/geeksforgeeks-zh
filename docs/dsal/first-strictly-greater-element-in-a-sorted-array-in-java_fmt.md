...
> 原文: [https://www.geeksforgeeks.org/first-strictly-greater-element-in-a-sorted-array-in-java/](https://www.geeksforgeeks.org/first-strictly-greater-element-in-a-sorted-array-in-java/)
...
请注意，在每一次迭代中，我们都会考虑至少一半的剩余元素。如果顶部分支执行，那么`[low, (low+high)/2]`范围内的元素全部被丢弃，导致我们失去 `floor((low+high)/2) – low + 1 >= (low+high)/2 – low = (high-low)/2` 个元素。

如果底部分支执行，则`[(low+high)/2 + 1, high]`范围内的元素都将被丢弃。这使我们失去了 `high – floor((low+high)/2) + 1 >= high – (low+high)/2 = (high–low)/2` 个元素。
