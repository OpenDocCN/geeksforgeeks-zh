...
> 原文: [https://www.geeksforgeeks.org/find-the-prime-p-using-given-four-integers/](https://www.geeksforgeeks.org/find-the-prime-p-using-given-four-integers/)

给定四个整数 `X`，`Y`，`X^2 % P`，`Y^2 % P`，其中 `P` 为素数。任务是找到质数 `P`。
...
> **输出:** `3`
> 当 `X = 3`，`X^2 = 9`，`9` 模 `P` 为 `0`。所以 `P` 的可能值是 `3`。
> 当 `Y = 5`，`Y^2 = 25`，`25` 模 `P` 为 `1`。所以 `P` 的可能值是 `3`。
>
