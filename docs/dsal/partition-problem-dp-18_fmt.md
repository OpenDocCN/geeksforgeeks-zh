...
### 时间复杂度
O(2^n)在最坏的情况下，这个解决方案为每个元素尝试了两种可能性(是包含还是排除)。
...
以下是一个基于动态规划的 Python 程序，用于解决分区问题。
