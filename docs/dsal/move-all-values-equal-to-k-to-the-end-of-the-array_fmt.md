...
> 原文: [https://www.geeksforgeeks.org/move-all-values-equal-to-k-to-the-end-of-the-array/](https://www.geeksforgeeks.org/move-all-values-equal-to-k-to-the-end-of-the-array/)
...
> **输出:** `[4, 1, 3, 2, 2, 2, 2, 2]`
> **解释:**
> `2` 是必须移动到数组 `arr[]` 末尾的数字。因此，进行更改后，数组为 `[4, 1, 3, 2, 2, 2, 2, 2]`。数字 `4`、`1` 和 `3` 可以有不同的顺序。
>
...
为了解决上面提到的问题，我们使用[双指针技术](https://www.geeksforgeeks.org/two-pointers-technique/)。

1.  初始化两个指针，左边的指针标记数组的开始，右边的指针分别标记数组的结束。
2.  只要右指针指向 `K` 就递减计数，只要左指针不指向整数 `m` 就递增计数。
