...
> 原文: [https://www.geeksforgeeks.org/construct-an-array-from-its-pair-product/](https://www.geeksforgeeks.org/construct-an-array-from-its-pair-product/)

给定一个对积数组 `pair[]`，任务是找到原始数组。一个数组的对积数组 `arr[]` 是包含有序形式的所有对的积的数组，即 `{(arr[0] * arr[1])、(arr[0] * arr[2])、…、(arr[1] * arr[2])、(arr[1] * arr[3])、…、(arr[n–2]* arr[n–1])}`。

## 举例:

> **输入:** `pair[] = {2，3，6}`
> **输出:** 1 2 3
> **输入:** `pair[] = {48，18，24，24，32，12}`
> **输出:** 6 8 3 4

## 方法:

首先从给定的对积数组中找到所需数组的大小。假设原阵的大小为 `N`，对积阵的大小为 `X`。因此，通过求解 `(N *(N–1))/2 = X`，`N` 的值可以计算为 `N =(1+(int)sqrt(1+8 * X))/2`。
