...
> 原文：[https://www.geeksforgeeks.org/modify-binary-tree-by-replacing-each-node-with-the-product-of-all-remaining-nodes/](https://www.geeksforgeeks.org/modify-binary-tree-by-replacing-each-node-with-the-product-of-all-remaining-nodes/)
...
首先计算[给定树](https://www.geeksforgeeks.org/product-of-all-nodes-in-a-binary-tree/)中所有节点的乘积，然后对给定树执行[任意树遍历](https://www.geeksforgeeks.org/tree-traversals-inorder-preorder-and-postorder/)，并用值 `(P / root->data)` 更新每个节点，即可解决给定问题。按照以下步骤解决问题：

*   使用[前序遍历](https://www.geeksforgeeks.org/tree-traversals-inorder-preorder-and-postorder/)找到二叉树的所有节点的乘积，并将其存储在一个变量中，比如 `P`。
*   定义一个[函数](https://www.geeksforgeeks.org/functions-in-c/)，比如 `updateTree(root, P)`，执行以下步骤：
    *   如果 `root` 的值为 `NULL`，则从函数返回。
    *   将根节点的当前值更新为值 `(P / root->data)`。
    *   [递归调用](https://www.geeksforgeeks.org/recursion/)左右子树的 `updateTree(root->left, P)` 和 `updateTree(root->right, P)`。
*   完成上述步骤后，打印修改后的树的[中序遍历](https://www.geeksforgeeks.org/tree-traversals-inorder-preorder-and-postorder/)。
