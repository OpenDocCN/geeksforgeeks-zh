...
> 原文: [https://www.geeksforgeeks.org/java-program-expand-string-range-given/](https://www.geeksforgeeks.org/java-program-expand-string-range-given/)
...
*   首先，我们必须将字符串分割成`String[]`数组。我们必须在找到符号的地方分割字符串。
*   现在我们有一个包含元素的`String[]`数组。现在我们只去第一个索引最后一个元素，即 1，`String[]`数组的前一个索引第一个元素说它是 5。
