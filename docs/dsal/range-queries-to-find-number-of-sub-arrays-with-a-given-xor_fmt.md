...
> 原文：[https://www.geeksforgeeks.org/range-queries-to-find-number-of-sub-arrays-with-a-given-xor/](https://www.geeksforgeeks.org/range-queries-to-find-number-of-sub-arrays-with-a-given-xor/)

给定大小为 `n` 和 `q` 个查询的数组 `arr[]` 和整数 `k`。每个查询由一个索引范围 `[l, r]` 组成，任务是统计索引对 `i` 和 `j` 的数量，使得 `l ≤ i ≤ j ≤ r`（基于 1 的索引）和元素 `a[i], a[i + 1], …, a[j]` 的异或等于 `k`。
...
[**这篇文章**](https://www.geeksforgeeks.org/count-number-subarrays-given-xor/) 讨论了在每个查询中寻找具有给定异或的子数组，时间复杂度为 `O(n)`，因此如果 `q` 很大，我们的总运行时间复杂度将是 `O(n * q)`。
既然给我们离线查询，想法就是用莫队算法。我们将数组划分为 `sqrt(n)` 个块，并根据块索引对查询进行排序，首先用较少的 `r` 值断开连接。
我们将保留一个数组 `xorr[]`，其中 `xorr[i]` 存储从 0 到 `i`（基于索引 0）的数组元素前缀的异或。
我们还将保留另一个数组 `cnt[]`，其中 `cnt[y]` 存储带有 `xorr[i] = k` 的前缀数量。
假设，我们在索引 `x` 处添加一个元素，让 `xorr[x] ^ k = y`，现在如果 `cnt[y]` 大于 0，则存在带有相同异或的前缀，因此我们得到在索引 `x` 处结束的带有给定异或 `k` 的 `y` 个子数组。
