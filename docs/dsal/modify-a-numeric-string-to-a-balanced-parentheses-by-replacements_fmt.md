...
> 原文: [https://www.geeksforgeeks.org/modify-a-numeric-string-to-balanced-by-replacements/](https://www.geeksforgeeks.org/modify-a-numeric-string-to-a-balanced-parentheses-by-replacements/)
...
> **输入:** `S = "1123"`
> **输出:** 是，`(()`
> **解释:** 将字符`“1”`的替换为`“(”`、`“2”`替换为`“)”`和`“3”`替换为`“)”`。因此，得到的括号序列是`”(())`，这是平衡的。
>
> **输入:** `S = "1121"`
> **输出:** 否
...
*   如果一个字符串的`第一个`和`最后一个`字符不同，则分别由`打开`和`关闭`括号来代替。第三个`字符由`打开`或`关闭`支架代替。
