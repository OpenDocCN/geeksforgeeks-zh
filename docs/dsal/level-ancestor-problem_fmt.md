...
解决上述查询的想法是使用**跳转指针算法**并在 `O(n log n)` 时间内对树进行预处理，并在 `O(log n)` 时间内回答级别祖先查询。在跳转指针中，有一个从节点 `N` 到其第 `j` 个祖先的指针，对于 `j = 1, 2, 4, 8, ...`，依此类推。我们将这些指针称为 `jump_N[i]`，其中 `jump_u[i] = LA(N, depth(N) - 2^i)`。

当要求算法处理查询时，我们使用这些跳转指针反复跳转树。跳转次数最多为 `log n`，因此可以在 `O(log n)` 时间内回答查询。

因此，我们存储每个节点的 **`2^i` 祖先**，并且还从树的根中找到每个节点的深度。

现在我们的任务简化为找到节点 `N` 的第 `(depth(N) - 2^i)` 个祖先。让我们将 `X` 表示为 `(depth(N) - 2^i)` 并让 `X` 的 `b` 位是由 `s1, s2, s3, ... sb` 表示的**设置位**（1）。

**`X = 2^(s1) + 2^(s2) + ... + 2^(sb)`**

现在的问题是如何从树的根中找到每个节点的 `2^j` 祖先以及每个节点的深度？

最初，我们知道每个节点的第 `2^0` 个祖先是其父节点。我们可以递归计算第 `2^j` 个祖先。我们知道第 `2^j` 祖先是第 `2^(j-1)` 祖先的第 `2^(j-1)` 祖先。为了计算每个节点的深度，我们使用祖先矩阵。如果我们发现第 `k` 个元素的根节点在第 `j` 个索引处存在，则该节点的深度仅为 `2^j`，但如果第 `k` 个元素的祖先数组中不存在根节点，则第 `k` 个元素的深度为 `2^(第 k 行的最后一个非零祖先的索引) + 第 k 行的最后索引处的祖先的深度`。
...
在给定图中，我们需要计算值为 `8` 的节点的第一级祖先。首先，我们创建祖先矩阵，该矩阵存储节点 `2^j` 的第 `j` 个祖先。现在，节点 `8` 的 **`2^0`** 的祖先是 **10**，类似的 **`2^0`** 节点 **10** 的祖先是 `9`，节点 `9` 的祖先是 `1`，节点 `1` 的祖先是 `5`。基于上述算法，节点 8 的第一级祖先是节点 8 的 **`(depth(8) - 1)` 祖先**。我们已经预先计算了每个节点的深度，深度 8 为 5，所以我们最终需要找到 **`(5 - 1) = 4`** 节点 8 的第 4 个祖先，它等于节点 `8` 的 **`2^1` 个祖先** 的 **`2^1` 个祖先** 和 **`2^1`** 的祖先是 **`2^0`** 的祖先是节点 `8` 的 **`2^0`** 祖先。因此，节点 `8` 的 **`2^0`** 的祖先的 **`2^0`** 的祖先是值为 `9` 的节点，而节点 `9` 的 **`2^1`** 祖先是值为 `5` 的节点。因此，通过这种方式，我们可以以 `O(log n)` 时间复杂度来计算所有查询。
