...
> 原文：[https://www.geeksforgeeks.org/find-n-integers-given-difference-product-sum/](https://www.geeksforgeeks.org/find-n-integers-given-difference-product-sum/)
...
一个棘手的解决方案是保留差 `D`，选择 `N` 个数字作为 `N-2` 个 `'1'`，一个 `'2'` 和一个剩余的数字作为 `'N+D'`。
和 = `(N-2)*(1) + 2 + (N+D) = 2*N + D`。
积 = `1*2*(N+D) = 2*N+2*D`
差 = `(2 * N+2 * D)–(2 * N+D)= D`。
