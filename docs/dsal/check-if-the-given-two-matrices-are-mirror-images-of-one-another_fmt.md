...
> 原文: [https://www.geeksforgeeks.org/check-if-the-given-two-matrices-are-mirror-images-of-one-another/](https://www.geeksforgeeks.org/check-if-the-given-two-matrices-are-mirror-images-of-one-another/)

给定两个大小为 ``NxN`` 的矩阵 ``mat1[][]`` 和 ``mat2[][]``。任务是找出给定的两个矩阵是否是彼此的镜像。如果给定的两个矩阵是镜像，则打印“``是``”，否则打印“``否``”。

> 如果对于矩阵的任何有效索引 `(i, j)`，大小为 `N*N` 的两个矩阵 ``mat1`` 和 ``mat2`` 被称为彼此的镜像
>
> ```
