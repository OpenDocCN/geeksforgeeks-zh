...
*   定义一个[递归函数](https://www.geeksforgeeks.org/recursive-functions/)，比如说`RecursiveCloneStack(stack<int> S, stack<int> Des)`，其中`S`是源栈，`Des`是目的栈:
    *   定义一个基本情况，好像`S.size()`是`0`，然后从函数返回。
    *   将源栈的[顶元素存储在一个变量中，说`val`](https://www.geeksforgeeks.org/stack-top-c-stl/)，然后[移除栈的顶元素](https://www.geeksforgeeks.org/stack-push-and-pop-in-c-stl/)`S`。
    *   现在用更新的源栈`S`调用递归函数，即`RecursiveCloneStack(S, Des)`。
    *   完成上述步骤后，将`val`推入`Des`堆。
*   初始化一个[栈](https://www.geeksforgeeks.org/stack-data-structure/)，说`Des`来存储目的栈。
*   现在调用函数`RecursiveCloneStack(S, Des)`将源栈的元素复制到目标栈。
