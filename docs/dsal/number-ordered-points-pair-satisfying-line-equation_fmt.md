...
> 原文：[https://www.geeksforgeeks.org/number-ordered-points-pair-satisfying-line-equation/](https://www.geeksforgeeks.org/number-ordered-points-pair-satisfying-line-equation/)
...
**方法 2(高效):**
给定一个点的 x 坐标，对于每个 x 都有一个唯一的 y 值，而 y 的值只不过是 `m * x + c`。因此，对于数组 `arr` 的每个可能的 x 坐标，计算满足直线方程的 y 的唯一值在该数组中出现多少次。存储数组中所有整数的计数。现在，对于每个值 `arr[i]` ，将 `m * arr[i] + c` 的出现次数加到答案中，对于给定的 `i`，`m * arr[i] + c` 在数组中出现 `x` 次，然后，将 `x` 加到我们的计数器中以获得总有效点，但是需要检查如果 `arr[i] = m * arr[i] + c`，那么， 很明显，由于这在数组中出现了 `x` 次，因此一次出现在第 `i` 索引处，其余 `(x–1)` 次出现是有效的 y 坐标，因此将 `(x–1)` 添加到我们的点数计数器中。
