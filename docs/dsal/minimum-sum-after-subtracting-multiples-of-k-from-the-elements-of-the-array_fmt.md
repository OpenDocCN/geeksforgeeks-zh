...
> 原文: [https://www.geeksforgeeks.org/minimum-sum-after-subtracting-multiples-of-k-from-the-elements-of-the-array/](https://www.geeksforgeeks.org/minimum-sum-after-subtracting-multiples-of-k-from-the-elements-of-the-array/)

给定一个整数 `K` 和一个整数数组 `arr`，任务是通过从每个元素中减去一个 `K` 的倍数来找到数组中所有元素被约简后的最小可能和（结果必须是正的，并且数组中的每个元素在此约简后必须相等）。如果数组无法缩小，则打印 `-1`。**注意**一个元素在数组的最终状态下可以减少，也可以不减少。
...
函数 `min_sum(n, k, a)` 用于计算经过变换后的数组最小和。其逻辑如下：

1.  对数组 `a` 进行排序。
2.  如果最小元素 `a[0]` 小于 0，则返回 -1。
3.  如果 `k` 等于 0：
    *   若所有元素相同（`a[0] == a[n - 1]`），则返回 `n * a[0]`。
    *   否则返回 -1。
4.  如果 `k` 不等于 0：
    *   检查数组中每个元素 `a[i]` 与 `a[0]` 的差值 `p` 是否能被 `k` 整除（`p % k == 0`）。
    *   如果存在元素不能被整除，则返回 -1。
    *   如果所有元素都能被整除：
        *   若 `k` 等于 1，则返回 `n`。
        *   否则返回 `n * (a[0] % k)`。
