...
> 原文: [https://www.geeksforgeeks.org/find-minimum-steps-required-to-reach-the-end-of-a-matrix-set-1/](https://www.geeksforgeeks.org/find-minimum-steps-required-to-reach-the-end-of-a-matrix-set-1/)

给定一个由正整数组成的 2d 矩阵，任务是找到到达矩阵末端（最右边的底部单元格）所需的最小步数。如果我们在单元格 `(i, j)`，我们可以去单元格 `(i, j+arr[i][j])` 或 `(i+arr[i][j], j)`。我们不能出界。如果不存在路径，打印 `-1`。
...
一个简单的解决方案就是探索所有可能的解决方案。这需要指数级的时间。

### 更好的做法

我们可以用动态规划在多项式时间内解决这个问题。我们来决定一下 `DP` 的状态。我们将在 2d DP 上构建我们的解决方案。假设我们在 `{i, j}` 号单元格。我们将尝试找到从该单元格到达终点单元格 `(n-1, n-1)` 所需的最小步数。我们只有两条可能的路径，即到达单元格 `{i, j+arr[i][j]}` 或 `{i+arr[i][j], j}`。一个简单的递归关系是：
