...
> 原文：`https://www.geeksforgeeks.org/check-whether-n-is-a-factorion-or-not/`
...
创建一个大小为 `10` 的数组 `fact[]` 来存储所有可能数字的阶乘，其中 `fact[i]` 存储 `i!`。现在，对于给定数字的所有数字，使用前面计算的 `fact[]` 数组，找到这些数字的阶乘之和。如果和等于给定的数，那么这个数就是一个因子，否则就不是。
