...
> 原文：[https://www.geeksforgeeks.org/find-the-initial-array-from-given-array-after-range-sum-queries/](https://www.geeksforgeeks.org/find-the-initial-array-from-given-array-after-range-sum-queries/)
...
> **输出:** `3 5 8`
> 如果对数组 `{3, 5, 8}` 执行查询 `[0, 1, 2]`，得到的数组将是 `{5, 7, 8}`。
> **输入:** `arr[] = { 20, 30, 20, 70, 100 }`，`l[] = { 0, 1, 3 }`，`r[] = { 2, 4, 4 }`，`x[] = { 10, 20, 30 }`

## 朴素法

对于从 `l` 到 `r` 的每个范围，减去相应的 `x` 得到初始数组。以下是实施办法：
