...
> 原文: [https://www.geeksforgeeks.org/number-of-solutions-for-x-y-where-a-x-b-and-c-y-d-and-x-y-are-integers/](https://www.geeksforgeeks.org/number-of-solutions-for-x-y-where-a-x-b-and-c-y-d-and-x-y-are-integers/)

给定四个整数 `a`，`b`，`c`，`d`（直到 `10^6`）。任务是找到 `x < y` 的解的个数，其中 `a <= x <= b` 和 `c <= y <= d` 且 `x`，`y` 是整数。
...
**方法**: 让我们显式迭代 `x` 的所有可能值，对于 `x` 的一个这样的固定值，问题简化为有多少个 `y` 值使得 `c <= y <= d` 和 `y > x`（即 `y >= max(c, x + 1)`）和 `y <= d`。假设 `c <= d`，否则当然没有 `y` 的有效值。因此，对于一个固定的 `x`，有 `d - max(c, x + 1) + 1` 个 `y` 的有效值，因为一个范围 `[R1, R2]` 内的整数个数是由 `R2 - R1 + 1` 给出的。
