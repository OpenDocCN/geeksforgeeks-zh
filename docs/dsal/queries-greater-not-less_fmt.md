...
给定一组`N`个整数。将有`Q`个查询，每个查询包括两个整数形式的`q`和`x`，`0 <= q <= 1`。查询有两种类型:
...
## 方法 1:
一种朴素方法可以针对每个查询，遍历整个数组并计算小于或大于`x`的整数，具体取决于`Q`。这种方法的时间复杂度为`O(Q*N)`。

## 方法 2:
一种高效的方法可以对数组进行排序，并对每个查询使用二分搜索法。这需要`O(NlogN + QlogN)`。
以下是该方法的实现情况:
