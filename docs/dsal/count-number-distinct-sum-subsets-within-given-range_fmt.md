...
[位集](https://www.geeksforgeeks.org/c-bitset-and-its-application/) | [位操作](https://www.geeksforgeeks.org/bits-manipulation-important-tactics/)

## 方法 1(简单)

一种天真的方法是生成给定集的所有可能子集，按子集方式计算它们的和，并将它们推送到散列表中。迭代整个给定的范围，计算 hashmap 中存在的数字。

## 方法 2(高效)

解决这个问题的有效方法是使用大小为 10<sup>5</sup> 的位组。通过左移位集并与前一位集按位“或”来更新每个元素 `X` 的位集，以便新的可能和的位集变为 `1`。然后使用**前缀和**的概念，预计算前缀`[1]`在 `1` 和 `i` 之间所需的数字计数..`i]`如果同时有多个查询被询问，则回答 `O(1)`中的每个查询。对于查询 `L` 和 `R`，答案只是**前缀【R】–前缀【L–1】**
