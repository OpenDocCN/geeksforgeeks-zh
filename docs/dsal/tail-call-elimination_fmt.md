...
我们还讨论了尾部递归优于非尾部递归，因为尾部递归可以被现代编译器优化。现代编译器基本上是做`尾调用消去`来优化尾递归代码。
...
递归使用栈来跟踪函数调用。每次函数调用时，都会有一个新的框架被推送到堆栈上，其中包含该调用的局部变量和数据。假设一个堆栈帧需要 `O(1)`，即恒定的内存空间，那么对于 `N` 递归调用，所需的内存将是 `O(N)`。
