...
> 原文：[https://www.geeksforgeeks.org/klees-algorithm-length-union-segments-line/](https://www.geeksforgeeks.org/klees-algorithm-length-union-segments-line/)
...
1.  将所有线段的所有坐标放入一个辅助数组 `points[]`。
2.  根据坐标值排序。
3.  排序的附加条件——如果有相等的坐标，插入任一线段的左坐标，而不是右坐标。
4.  现在遍历整个数组，计数器 `Counter` 记录重叠的段数。
5.  如果 `Counter` 大于零，则结果累加到 `points[i] – points[i-1]` 之间的差值上。
6.  如果当前元素属于左端，我们增加 `Counter`，否则减少。
