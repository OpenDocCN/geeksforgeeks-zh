...
> 原文: [https://www.geeksforgeeks.org/given-n-standard-deviation-find-n-elements/](https://www.geeksforgeeks.org/given-n-standard-deviation-find-n-elements/)

给定 `N` 和[标准差](https://en.wikipedia.org/wiki/Standard_deviation)求 `N` 个元素。

> 平均值是元素的平均值。
> 表示 `arr[0..n-1]` 的平均值为 `σ(arr[i])/n`，其中 `0 <= i < n`。
> 方差是平均值除以元素数的平方差之和。
> **方差** = `σ(arr[i]–mean)^2 / n`
> 标准差为方差的平方根。
> **标准差** = `σ(variance)`
...
如果我们看公式，我们有两个未知项一个是 `xi`，另一个是 `mean`。主要动机是使平均值为 `0`，这样我们就可以得到 `X` 元素的公式。会有两种情况，一种是偶数，一种是奇数。

### 当 N 为偶数时

要使 `N` 元素的平均值为 `0`，最好的方法是将 `N` 元素表示为 `-X +X -X +X`。公式将是 `sqrt(x^2)/n`、`x2+x^2+x^2+………N` 项之和)，所以公式原来是 `sqrt (N*(x^2)/N)`。`n` 相互抵消，所以 `sqrt (x^2)` 变成了 `SD`。所以，我们得到 `N` 元素为 **-SD +SD -SD +SD……** 得到平均值 `0`。我们需要打印 `-SD +SD -SD +SD……`。

### N 为奇数时

`N` 元素的平均值为 `0`。所以，一个元素是 `0`，其他 `N-1` 个元素是 `-X +X -X`。公式将是 `sqrt(x^2)/n`、`x2+x^2+x^2+………N-1` 项之和，所以公式原来是 `sqrt((N-1)*(x^2)/N)`，所以 `X= SD * sqrt(n/(n-1))`。`n` 个元素是 `0 -X +X -X +X……`。
