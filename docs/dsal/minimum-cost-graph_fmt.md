...
给定表示为二维平面上的 `N` 个节点 `(x_i, y_i)`。如果节点之间的曼哈顿距离为 `1`，则称这些节点已连接。您可以连接两个未连接的节点，但要以它们之间的欧式距离为代价。任务是连接图，以便每个节点都有一条从任何节点到其路径的路径，且成本最低。

## 示例

> **输入**：`N = 3`，`edges[][] = {{1, 1}, {1, 1}, {2, 2}, {3, 2}}`
> **输出**：`1.41421`
> 由于 `(2, 2)` 和 `(2, 3)` 已经连接。
> 因此，我们尝试将 `(1, 1)` 与 `(2, 2)` 或 `(1, 1)` 与 `(2, 3)` 连接，但将 `(1, 1)` 与 `(2, 2)` 连接产生最小的成本。
>
...
蛮力方法是将每个节点与每个其他节点连接，类似地，将其他 `N` 个节点连接起来，但在最坏的情况下，时间复杂度将为 `N^N`。
