...
> 原文：[https://www.geeksforgeeks.org/search-a-string-in-matrix-using-split-function-in-java/](https://www.geeksforgeeks.org/search-a-string-in-matrix-using-split-function-in-java/)
...
如果搜索字符串出现在矩阵的任何一行，`split()` 函数将产生以下结果：

1.  如果字符串占据了整行，那么 `split()` 函数将返回一个长度为零的数组。
2.  如果字符串出现在字符之间，那么数组长度将大于 1。
3.  如果数组长度是 1，那么可能有三种情况：
    *   字符串出现在前半部分。
    *   字符串出现在后半部分。
    *   该字符串不在该行中。
4.  要逐列搜索字符串，转置矩阵并重复第一步。
5.  打印 `Yes` 如果找到字符串，则打印 `No`。
    ...
    public static int check(String[] matrix, String string) {
        ...
        for (String input : matrix) {
            ...
            if (value.length >= 2 || value.length == 0) {
                return 1;
            } else if (value.length == 1 && input.length() != value[0].length()) {
                return 1;
            }
        }
        return 0;
