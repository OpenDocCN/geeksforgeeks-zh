...
> 原文: [https://www.geeksforgeeks.org/subtract-1-from-a-number-represented-as-linked-list/](https://www.geeksforgeeks.org/subtract-1-from-a-number-represented-as-linked-list/)

给定代表正整数的[链表](https://www.geeksforgeeks.org/data-structures/linked-list/)的`head`，任务是在减去 1 后打印更新的链表。
...
*   定义一个[函数](https://www.geeksforgeeks.org/functions-in-c/)，比如 `subtractOneUtil` 以链表头为参数，执行以下步骤:
    *   **基本情况:** 如果链表头节点为 `NULL`，则从该递归调用返回 `-1`。
    *   [**递归调用**](https://www.geeksforgeeks.org/recursion/): 递归调用链表的下一个节点，让这个递归调用返回的值是 `borrow`。
    *   如果 `borrow` 的值是 `-1`，头节点的值是 `0`，那么将头节点的值更新为 `9`，并从当前递归调用中返回 `-1`。
    *   否则，将头节点的值递减 `1`，并从当前递归调用中返回 `0`。
*   调用上述函数作为 `subtractOne(head)`，从链表中减去 `1`。
...
`时间复杂度:` O(N)，N 是[给定链表](https://www.geeksforgeeks.org/find-length-of-a-linked-list-iterative-and-recursive/)的长度。
`辅助空间:` O(1)