...
> 原文: [https://www.geeksforgeeks.org/program-to-determine-the-octant-of-the-axial-plane/](https://www.geeksforgeeks.org/program-to-determine-the-octant-of-the-axial-plane/)

给定3个坐标`x`，`y`和`z`，任务是确定轴向平面的八分之一。
...
下面给出的是为了确定轴向平面的八分之一需要检查的条件。

*   检查 `x >= 0` 和 `y >= 0` 和 `z >= 0`，那么点位于第一个八分点。
*   检查 `x < 0` 和 `y >= 0` 和 `z >= 0`，那么点位于第二个八分点。
*   检查 `x < 0` 和 `y < 0` 和 `z >= 0`，则点位于第三个八分点。
*   检查 `x >= 0`，`y < 0` 和 `z >= 0`，那么点位于第四个八分点。
*   检查 `x >= 0` 和 `y >= 0` 和 `z < 0`，那么点位于第五个八分点。
*   检查 `x < 0` 和 `y >= 0` 且 `z < 0`，则点位于第六个八分点。
