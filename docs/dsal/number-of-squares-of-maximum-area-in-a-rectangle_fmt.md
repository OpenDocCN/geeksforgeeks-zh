...
> 原文: [https://www.geeksforgeeks.org/number-of-squares-of-maximum-area-in-a-rectangle/](https://www.geeksforgeeks.org/number-of-squares-of-maximum-area-in-a-rectangle/)
...
任务是将矩形切割成边长为 `s` 的正方形，矩形没有剩余的部分，因此 `s` 必须将 `m` 和 `n` 分开。同样，正方形的边应该是最大可能的，因此，`s` 应该是 `m` 和 `n` 的最大公约数。
所以， `s = gcd(m, n)` 。
