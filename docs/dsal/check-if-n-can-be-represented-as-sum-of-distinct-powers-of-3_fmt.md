...
> 原文: [https://www.geeksforgeeks.org/check-if-n-can-be-represented-as-sum-of-distinct-powers-of-3/](https://www.geeksforgeeks.org/check-if-n-can-be-represented-as-sum-of-distinct-powers-of-3/)

给定一个正整数 `N`，任务是检查给定的数字 `N` 是否可以表示为 `3` 的相异次幂的和。如果发现是真的，则打印`“是”`。否则，`“否”`。
...
> **输出:** *是*
> **说明:**
> 数字 `N`(= 28)可以表示为`(1 + 7) = (3^0 + 3^3)`，是一个完美的幂 2。
>
> **输入:** `N = 6`
> **输出:** 否

**方法:** 解决给定问题的最简单方法是[生成所有不同幂 `3` 的所有可能排列](https://www.geeksforgeeks.org/write-a-c-program-to-print-all-permutations-of-a-given-string/)，如果存在这样的组合，其和是 `3` 的[完美幂](https://www.geeksforgeeks.org/perfect-power-1-4-8-9-16-25-27/)。正如`3^15 > 10^7`一样，也只有 `16` 两种截然不同的权力，即`[0, 15]`。
...
通过观察三元中的 `N` 形成（基 3）也可以优化上述方法。每个数字是 `3^i`，三进制数 `N` 是它们的和。

要拥有 `3` 的不同次方，以三进制形式总结为 `N`，则第 `i` 位数字可以是 `0`，`1` 或 `2`（二进制为 `0` 和 `1`）。因此，在和位置：每个数字可以有三种情况：

*   数字可以是 `0`，即 `N` 中无 `3^i` 项。
*   数字可以是 `1`，即 `N` 中有一项 `3^i`。
*   数字不能是 `2`，因为 `3^i` 中有 `2` 项和，所以不是截然不同的。
