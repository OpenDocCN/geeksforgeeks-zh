...
> 原文: [https://www.geeksforgeeks.org/largest-number-m-less-than-n-such-that-xor-of-m-and-n-is-even/](https://www.geeksforgeeks.org/largest-number-m-less-than-n-such-that-xor-of-m-and-n-is-even/)
...
`(10 XOR 8) = 2`，所以 `M = 8` 是可能的，因为它是满足必要条件的 `M` 的最大可能值。

**输入:** `N = 5`
**输出:** `3`
`(5 XOR 4) = 1`，所以 `M = 4` 是不可能的，因为 `1` 不是偶数。
`(5 XOR 3) = 6`，所以 `M = 3` 是可能的，因为 `6` 是偶数，`3` 是 `M` 的最大可能值(小于 `N`)。
