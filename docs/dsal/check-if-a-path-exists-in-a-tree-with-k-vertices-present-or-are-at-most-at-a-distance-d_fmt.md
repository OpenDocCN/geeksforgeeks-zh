...
> 原文: [https://www.geeksforgeeks.org/check-if-a-path-exists-in-a-tree-with-k-vertices-present-or-are-at-most-at-a-distance-d/](https://www.geeksforgeeks.org/check-if-a-path-exists-in-a-tree-with-k-vertices-present-or-are-at-most-at-a-distance-d/)

给定一棵树，其 `N` 个顶点编号为 `0` 到 `N-1`、`K` 个顶点，距离为 `D`，任务是找出是否存在从根到某个顶点的路径，使得每个 `K` 个顶点都属于该路径，或者最多与该路径有一段距离 `D`。
...
*   如果当前的一组 `K` 顶点可以形成一条连续路径，那么答案是“是”，否则为“否”。
