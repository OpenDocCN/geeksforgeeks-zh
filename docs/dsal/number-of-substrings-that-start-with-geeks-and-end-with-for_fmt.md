...
> 原文: [https://www.geeksforgeeks.org/number-of-substrings-that-start-with-geeks-and-end-with-for/](https://www.geeksforgeeks.org/number-of-substrings-that-start-with-geeks-and-end-with-for/)

给定一个由小写英文字母组成的字符串 `str`，任务是找出以`"geeks"`开头，以`"for"`结尾的子字符串的数量。

**举例:**

> **输入:** `str = "geeksforgeeks"`
> **输出:** 3
> `"geeksfor"`, `"geeksforgeeksfor"`, `"geeksfor"` 是唯一有效的子字符串。
> **输入:** `str = "geeksforgeeks"`
> **输出:** 1

**天真的方法:** 首先将计数器设置为 0，然后遍历字符串，每当遇到子字符串`"geeks"`时，从下一个索引再次遍历字符串，并尝试找到`"for"`的子字符串。如果`"for"`存在，则递增计数器并最终打印。

**高效方法:** 为子串`"geeks"`和`"for"`设置两个计数器，比如 `c1` 和 `c2`。迭代时，每当遇到子串`"geeks"`时，递增 `c1`，每当遇到`"for"`时，设置 `c2 = c2 + c1`。这是因为`"geeks"`的每一次出现都会与当前找到的`"for"`组成一个有效的子串。最后打印 `c2`。
