...
> 原文: [https://www.geeksforgeeks.org/bitonic-traveling-salesman-problem/](https://www.geeksforgeeks.org/bitonic-traveling-salesman-problem/)
...
上述问题可以使用[动态规划](https://www.geeksforgeeks.org/dynamic-programming/)解决。为了理解，问题可以换成两个人。两者应该同时从最左边的点开始。沿着两条不同的路径走，最后到达最右边的点，除了起点和终点。
...
*   迭代表的最后一行 `dp` ，并将 `DP[N-1][i]` 更新为 `distance(N-1, N)` 和 `distance(i, N)` 之和，其中 `distance(x, y)` 表示 `xth` 和 `yth` 点之间的[欧氏距离](https://www.geeksforgeeks.org/calculate-the-euclidean-distance-using-numpy/)。
*   创建一个[递归函数](https://www.geeksforgeeks.org/recursive-functions/) `findTour(i, j)` 来填充所有其他单元格
    *   将 `dp[i][j]` 更新为最小的 `findTour(i+1, j) + distance(i, i+1)` 和 `findTour(i+1, i) + distance(j, i+1)` 。
