...
> 原文：[https://www.geeksforgeeks.org/count-all-prefixes-of-the-given-binary-array-which-are-divisible-by-x/](https://www.geeksforgeeks.org/count-all-prefixes-of-the-given-binary-array-which-are-divisible-by-x/)

给定一个二进制数组 `arr[]` 和一个整数 `x`，任务是计算给定数组中可被 `x` 整除的所有前缀。
**注：** 从 `arr[0]` 到 `arr[i]` 的第 `i` 个前缀解释为二进制数（从最高有效位到最低有效位）。
...
从 `0` 迭代到 `i`，将每个二进制前缀转换为十进制，并检查该数是否可被 `x` 整除。
以下是上述方法的实施：
...
正如我们在上面的方法中看到的，我们将每个二进制前缀转换为十进制数，如 `0, 01, 010, 0101……`。但是随着 `n`（数组的大小）值的增加，结果数将会非常大，可能会超出数据类型的范围，因此我们可以利用模块化属性。
代替做 `number = number * 2 + arr[i]`，我们可以做得更好，作为 `number = (number * 2 + arr[i]) % x`。
