...
> 原文：[https://www.geeksforgeeks.org/program-generate-captcha-verify-user/](https://www.geeksforgeeks.org/program-generate-captcha-verify-user/)
...
生成验证码的字符集存储在字符数组`chrs[]`中，该数组包含(a-z，A-Z，0-9)，因此`chrs[]`的大小为62。
为了每次生成唯一的验证码，使用`rand()`函数（`rand()%62`）生成随机数，该函数生成0到61之间的随机数，并将生成的随机数作为字符数组`chrs[]`的索引，从而生成`captcha[]`的新字符，并且该循环运行`n`次（验证码的长度）以生成给定长度的验证码。
