...
> 原文: [https://www.geeksforgeeks.org/program-binomial-coefficients-table/](https://www.geeksforgeeks.org/program-binomial-coefficients-table/)
...
二项式系数用于研究二项式分布和多分量冗余系统。`B(m, x) = m! / x!(m - x)!, m >= x`
...
**问题分析:**
二项式系数可以递归计算如下–
`B(m, x) = B(m, x - 1) [m - x + 1 / x], x = 1, 2, 3 .... m`
此外，`b(0, 0) = 1`
也就是说，当 `x` 为零或 `m` 为零时，二项式系数为 1。程序打印 `m = 10` 的二项式系数表。
