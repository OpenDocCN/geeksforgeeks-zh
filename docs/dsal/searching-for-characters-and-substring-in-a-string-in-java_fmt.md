...
> 原文:[https://www.geeksforgeeks.org/searching-for-characters-and-substring-in-a-string-in-java/](https://www.geeksforgeeks.org/searching-for-characters-and-substring-in-a-string-in-java/)
...
### 方式 3: `indexOf(char c, int indexFrom)`
...
### 方式 4: `lastIndexOf(char c, int fromIndex)`
...
### 方式 8: `contains(CharSequence seq)`

如果字符串包含指定的字符值序列，则返回真，否则返回假。它的参数指定要搜索的字符序列，如果 `seq` 为 `null`，则抛出 `NullPointerException`。
...
*   `boolean startsWith(String)`:如果字符串存在于给定字符串的开头，则返回 true，否则返回 false。
*   `boolean startsWith(String, int indexNum)`: 如果字符串存在于给定字符串的索引 `indexNum` 的开始处，则返回 true，否则返回 false。
*   `boolean endsWith(String)`:如果字符串存在于给定字符串的结尾，则返回真，否则返回假。
