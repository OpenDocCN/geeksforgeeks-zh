...
> 原文: [https://www.geeksforgeeks.org/sort-a-string-in-decreasing-order-of-values-associated-after-removal-of-values-smaller-than-x/](https://www.geeksforgeeks.org/sort-a-string-in-decreasing-order-of-values-associated-after-removal-of-values-smaller-than-x/)

给定一个交替放置的整数 `X` 和一个由空格分隔的文本和数字组成的字符串 `str`，任务是对该字符串进行排序，以便在移除所有小于 `X` 的数字后，文本和数字按照相关数字的降序显示。如果两个字符串具有相同的值，则按照字符串的字典顺序对它们进行排序。

**示例:**

> **输入:** `X = 79`，`str = "Geek 78 Eve 99 Geeks 88 Gfg 86"`
> **输出:**
> Eve 99 Geeks 88 Gfg 86
> **说明:**
> “Eve 99”被移除，因为与其关联的数字小于 `X`(= 79)。
> 现在，基于关联数字降序的重新排序的字符串是“Eve 99 Geeks 88 Gfg 86”。
>
> **输入:** `X = 77`，`str = "Axc 78 Czy 60"`
> **输出:** `Axc 78`

**方法:** 想法是使用[冒泡排序](https://www.geeksforgeeks.org/bubble-sort/)技术。按照以下步骤解决问题:

*   将字符串分割成一个列表，然后删除小于给定值的条目，即 `X`。
