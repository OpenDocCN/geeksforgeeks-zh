...
> 原文: [https://www.geeksforgeeks.org/implementation-k-nearest-neighbors/](https://www.geeksforgeeks.org/implementation-k-nearest-neighbors/)

**先决条件:** [K 最近的邻居](https://www.geeksforgeeks.org/k-nearest-neighbors/)

## 简介

假设给我们一组数据项，每个数据项都有数值特征(如身高、体重、年龄等)。如果特征的计数是 `n`，我们可以将项目表示为一个 `n` 维网格中的点。给定一个新项目，我们可以计算从该项目到集合中每一个其他项目的距离。我们挑选最接近的 `k` 邻居，看看这些邻居的分类。我们在那里分类新的项目。
...
将这些数据文件放置在工作目录中。[数据2](https://media.geeksforgeeks.org/wp-content/uploads/data2.txt) 和 [数据](https://media.geeksforgeeks.org/wp-content/uploads/data.txt)。选择一个，将内容原样粘贴到名为 `data.txt` 的文本文件中。
...
然后我们继续看数据集本身。我们将项目保存到一个名为 `items` 的列表中，其元素是字典(每个项目一个)。这些条目字典的关键是特征名，加上“类”来保存条目类。最后，我们想打乱列表中的项目(这是一种安全措施，以防项目顺序不正常)。
...
随着数据存储到 `items` 中，我们现在开始构建我们的分类器。对于分类器，我们将创建一个新的功能，`Classify`。它将把我们想要分类的项目、项目列表和最近邻居的数量 `k` 作为输入。

如果 `k` 大于数据集的长度，我们就不进行分类，因为我们不能有比数据集中项目总数更多的最近邻居。(或者，我们可以将 `k` 设置为 `items` 的长度，而不是返回错误消息)
...
我们要计算待分类项目与训练集中所有项目之间的距离，最终保持 `k` 最短距离。为了保持当前最近的邻居，我们使用了一个名为 `neighbors` 的列表。最少的每个元素都有两个值，一个是距离要分类项目的距离，另一个是邻居所在的类别。我们将通过广义欧几里德公式计算距离(对于 `n` 维)。然后，我们将选择在 `neighbors` 中出现时间最多的类，这将是我们的选择。代码:
...
我们需要实现的外部功能有 `EuclideanDistance`、`UpdateNeighbors`、`CalculateNeighborsClass`、`FindMax`。
...
我们有我们的邻居列表(最多应该有 `k` 的长度)，我们想添加一个给定距离的项目到列表中。首先，我们将检查 `neighbors` 的长度是否为 `k`。如果它有更少的，我们就不考虑距离而添加项目(因为在我们开始拒绝项目之前，我们需要将列表填充到 `k`)。如果没有，我们将检查该项目的距离是否比列表中距离最大的项目短。如果是真的，我们将用新的物品替换最大距离的物品。
