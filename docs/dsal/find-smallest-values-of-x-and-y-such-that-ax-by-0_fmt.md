...
> 原文: [https://www.geeksforgeeks.org/find-smallest-values-of-x-and-y-such-that-ax-by-0/](https://www.geeksforgeeks.org/find-smallest-values-of-x-and-y-such-that-ax-by-0/)
...
一个简单解法是从 1，1 开始尝试`x`和`y`的每一个可能值，当方程满足时停止。
一个直接解决方案是使用最小公倍数(LCM)。`a`和`b`的 LCM 代表可以使两边相等的最小值。我们可以用下面的公式找到 LCM。
