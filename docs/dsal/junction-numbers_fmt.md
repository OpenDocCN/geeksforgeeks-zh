...
`结号`是一个数字 `N`，如果它可以写成 `K + SumOfDIgits(k)` 且至少有两个 `K` 的话。
...
**方法:** 由于`结数`是一个数 `N`，如果它可以写成 `K + SumOfDIgits(k)` 且至少有两个 `K`，那么对于一个循环中从 `1` 到 `N` 的所有数，我们将检查 `I+SumOfDIgits(I)` 是否等于 `N`。如果等于 `N` 打印`“是”`否则打印`“否”`。
...
参考: [http://www.numbersaplenty.com/set/junction_number/](http://www.numbersaplenty.com/set/junction_number/)