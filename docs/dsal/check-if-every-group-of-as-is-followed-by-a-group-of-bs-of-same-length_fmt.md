...
> 原文: [https://www.geeksforgeeks.org/check-if-every-group-of-as-is-followed-by-a-group-of-bs-of-same-length/](https://www.geeksforgeeks.org/check-if-every-group-of-as-is-followed-by-a-group-of-bs-of-same-length/)

给定字符串 `str`，任务是检查每组连续的 `a` 的后面是否跟有一组相同长度的连续的 `b` 的。如果每组条件为真，则打印 `1` 否则打印 `0`。
