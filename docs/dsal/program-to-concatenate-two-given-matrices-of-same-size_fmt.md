...
> 原文：[https://www.geeksforgeeks.org/program-to-concatenate-two-given-matrices-of-same-size/](https://www.geeksforgeeks.org/program-to-concatenate-two-given-matrices-of-same-size/)
...
思想是声明一个大小为 `M x 2 * N` 的矩阵，然后遍历矩阵 `A` 并存储矩阵前半部分的元素，然后类似地迭代矩阵 `B` 并存储矩阵后半部分的元素，借助于以下公式：
