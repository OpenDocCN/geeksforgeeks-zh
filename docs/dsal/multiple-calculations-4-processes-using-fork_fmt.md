...
> 原文：[https://www.geeksforgeeks.org/multiple-calculations-4-processes-using-fork/](https://www.geeksforgeeks.org/multiple-calculations-4-processes-using-fork/)
...
`解释–` 这里，我们已经使用了 `fork()` 函数创建了 4 个进程（三个子进程和一个父进程）。因此，这里我们使用两个 `fork()` 函数来创建 4 个进程：`n1=fork()` 和 `n2 = fork()`。

*   如果 `n1` 和 `n2` 都大于零，则是父进程执行计算一个数字出现频率的任务。
*   如果 `n1` 等于零且 `n2` 大于零，则是第一个子进程对给定数组进行排序。
*   如果 `n1` 大于零且 `n2` 等于零，则是第二个子进程计算数组中偶数的总个数。
*   如果 `n1` 和 `n2` 都等于零，则是第三个子进程计算数组中所有元素的和。
