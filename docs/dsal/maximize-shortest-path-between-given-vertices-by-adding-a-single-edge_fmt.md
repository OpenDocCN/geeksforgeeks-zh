...
给定`N`个节点和`M`条边的无向图。你还给出一个`K`条边的**已选择[]**集合。任务是在给定选定边的任意两个顶点之间添加单个边，以最大化节点`1`到节点`N`之间的最短路径长度。

**注意**：您可以在任意两个选定的顶点之间添加一条边，即使这些顶点之间已经有一条边。
...
思路是使用[广度优先搜索](https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/)查找从顶点`1`和`N`到每个选定顶点的距离。对于选定的顶点`i`，令`x_i`表示到节点`1`的距离，`y_i`表示到节点`N`的距离。步骤如下：

1.  维护一个二维矩阵（例如`dist[2][]`），该矩阵具有`2`行和`N`列。
2.  在第一行中，使用`BFS`遍历保持节点`1`与图中其他顶点之间的最短距离。
3.  在第二行中，使用`BFS`遍历保持节点`N`与图中其他顶点之间的最短距离。
4.  现在，从已选择的顶点中选择两个顶点`a`和`b`以最小化`min(x_a + y_b, y_a + x_b)`的值。为此，请执行以下操作：
    *   创建一个向量对，并将`(x_i – y_i)`的值与它们各自的选定节点一起存储。
    *   [对上述向量进行排序](https://www.geeksforgeeks.org/sorting-vector-of-pairs-in-c-set-1-sort-by-first-and-second/)。
    *   最好将`best`初始化为`0`，将`max`初始化为`-INF`。
    *   现在遍历上述向量对，并为每个选定的节点（例如`a`）将`best`的值更新为`max(best, max + dist[1][a])`，并将`max`更新为`max(max, dist[0][a])`。
5.  经过上述操作后，`max(dist[0][N-1], best + 1)`给出了最短路径的最小值。
