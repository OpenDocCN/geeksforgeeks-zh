...
> 原文: [https://www.geeksforgeeks.org/calculate-mdas-factorial-of-given-number/](https://www.geeksforgeeks.org/calculate-mdas-factorial-of-given-number/)
...
> MDAS_Factorial(N) = (N) * (N-1) / (N-2) + (N-3) - (N-4) - ... - 最多 1。
...
想法是对每个操作周期(`*`, `/`, `+`, `-`)使用一个循环，并计算 `N` 的 MDAS 阶乘。但如果 `N` 非常大，这可能会运行缓慢。这种方法的时间复杂度为 `O(N)`。
