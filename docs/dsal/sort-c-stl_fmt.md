...
我们已经在 C 语言中讨论了 [`qsort()`](https://www.geeksforgeeks.org/comparator-function-of-qsort-in-c/)。C++ STL 提供了一个类似的函数排序，对向量或数组（具有随机访问的项）进行排序。
...
例如：在下面的代码中，假设间隔 `{6，8}` 和 `{1，9}` 作为参数在 `compareInterval` 函数（比较器函数）中传递。现在因为 `i1.first`（`=6`） > `i2.first`（`=1`），所以我们的函数返回 `false`，这告诉我们“first”参数不应该放在“second”参数之前，因此排序将按照 `{1，9}` 优先，然后 `{6，8}` 作为下一个的顺序进行。
