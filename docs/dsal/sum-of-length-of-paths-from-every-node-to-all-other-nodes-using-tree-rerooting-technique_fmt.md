...
> 原文: [https://www.geeksforgeeks.org/sum-of-length-of-paths-from-every-node-to-all-other-nodes-using-tree-rerooting-technique/](https://www.geeksforgeeks.org/sum-of-length-of-paths-from-every-node-to-all-other-nodes-using-tree-rerooting-technique/)

对于无向树中的每个节点，使用树重跟踪技术找到从该节点到所有其他节点的路径长度之和。
...
>
> **输出:** 12 9 17 14 10 15 15
>
> **说明:** 为节点 `i` : `i1 + i2 + i3...`，其中 `ij` 是从 `i` 到 `j` 的路径长度。
...
> 路径长度总和的树 dp
>
> **注:** 图中 `dp[node]` 表示从 `node` 到其所有子树的路径之和。
>
> 现在考虑节点 `a` 的子节点 `b`，
>
> 1.  子树 `b`，`size[b]` 和子树 `b` 的路径长度之和给出为 `dp[b]`。
> 2.  考虑在 `a` 开始并且在 `b` 的子树的某个节点结束的所有路径。
> 3.  每条路径可以分解如下: `path(a, x) = edge(a, b) + path(b, x)`
> 4.  由于 `path(b, x)` 的所有路径的长度已经被 `dp[b]` 覆盖，因此只需要为 `b` 的子树中的所有节点 `x` 添加 `edge(a, b)`。
> 5.  这意味着 `size[b]` 需要添加到 `dp[b]` 中，因此子树 `b` 对 `dp[a]` 的贡献是 `(dp[b] + size[b])`。
...
### `重新搜索方法`
通过计算一个根的答案，并每次重新搜索树来计算其他节点，可以进一步优化解决方案。
