...
> 原文：[https://www.geeksforgeeks.org/find-the-minimum-capacity-of-the-train-required-to-hold-the-passengers/](https://www.geeksforgeeks.org/find-the-minimum-capacity-of-the-train-required-to-hold-the-passengers/)

给定进出列车的乘客数量，任务是找到列车的最小容量，以在整个旅程中保持所有乘客。

## 例：

> **输入：** `enter[] = {3, 5, 2, 0}`， `exit[] = {0, 2, 4, 4}`
> **输出：** `6`
> 车站 1：列车通过能力 = `3`
> 车站 2：列车通过能力 = `3 + 5 - 2 = 6`
> 车站 3：列车通过能力 = `6 + 2 - 4 = 4`
...
> 任何时刻列车最多可容纳 `6` 名乘客。
>
> **输入：** `enter[] = {5, 2, 2, 0}`， `exit[] = {0, 2, 2, 5}`
> **输出：** `5`

## 进场：

某站列车当前通过能力可通过将列车进站人数相加，再减去出站人数计算得出。所需的最小容量将是所有站点当前容量的最大值。
