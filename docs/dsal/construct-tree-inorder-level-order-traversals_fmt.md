...
> 原文: [https://www.geeksforgeeks.org/construct-tree-inorder-level-order-traversals/](https://www.geeksforgeeks.org/construct-tree-inorder-level-order-traversals/)
...
上述方法的时间复杂度上限为 O(n^3)。在主递归函数中，`constructTree()`被调用，这需要 O(n^2)时间。
