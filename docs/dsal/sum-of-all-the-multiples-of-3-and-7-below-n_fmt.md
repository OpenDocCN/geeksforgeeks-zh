...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-the-multiples-of-3-and-7-below-n/](https://www.geeksforgeeks.org/sum-of-all-the-multiples-of-3-and-7-below-n/)
...
*   我们知道 `3` 的倍数组成 [AP](https://www.geeksforgeeks.org/progressions-ap-gp-hp-and-practice-problems/) 为 `S₃ = 3+6+9+12+15+18+21+…`
*   而 `7` 的倍数组成一个 AP 为 `S₇ = 7 + 14 + 21 + 28 + …`
*   现在，`Sum = S₃ + S₇` 即 `3+6+7+9+12+14+15+18+21+21+……`
*   从上一步开始， `21` 重复两次。事实上， `21` (或 3*7)的所有倍数将重复计算两次，一次在系列 `S₃` 中，另一次在系列 `S₇` 中。所以 `21` 的倍数需要从结果中剔除。
*   所以，最终结果将是 `S₃ + S₇ – S₂₁`
