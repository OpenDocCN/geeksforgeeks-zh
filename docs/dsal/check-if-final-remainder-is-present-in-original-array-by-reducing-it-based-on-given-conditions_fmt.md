# 根据给定条件减少最终余数，检查原始数组中是否存在最终余数

> 原文: [https://www.geeksforgeeks.org/check-if-final-remainder-is-present-in-original-array-by-reducing-it-based-on-given-conditions/](https://www.geeksforgeeks.org/check-if-final-remainder-is-present-in-original-array-by-reducing-it-based-on-given-conditions/)
...
在以下操作后，`arr` 的大小为=2，所以 `(3*0) % 6 = 0`，并且 `0` 不在 `arr` 中，因此返回“否”。
...
*   以 `mid` 为中间元素，前后弹出元素，取其积除以 `mid`。
*   把我们从后面分成数组后得到的余数加上去。
*   重复步骤 2 直到数组长度等于 `2`。
*   最后取剩余元素的乘积除以 `n`。
*   如果原始数组中存在余数则打印“是”，否则打印“否”。
...
*   取 `start` 为 `0`，`end` 为 `n-1`，取 `start` 和 `end` 之和求中间指标，除以 2。
*   求运算后的余数，用余数替换最后一个元素。
...
*   将原始数组复制到另一个数组，并对该数组执行操作。
*   初始 `start` 为 `0`，`end` 为 `n-1`。
*   求中间指数为 `start` 和 `end` 的和除以 `2`。
*   通过将 `start` 和 `end` 元素相乘，除以中间元素来求余数。
*   用余数替换 `end` 元素，从 `1` 开始递增 `start`，保持 `end` 与 `n-1` 相同。
*   重复步骤 3 直到 `start` 等于 `end-1`。
*   取数组索引 `start` 和 `end` 处元素的乘积，除以 `n`。
*   如果原始数组中存在余数则打印“是”，否则打印“否”。