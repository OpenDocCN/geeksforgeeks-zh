...
给定一个整数 `n`（可能非常大），求其阶乘中出现的位数，其中阶乘定义为，`factorial(n) = 1*2*3*4……..*n` 和 `factorial(0) = 1`。
...
我们已经讨论了在[阶乘|集合 1](https://www.geeksforgeeks.org/count-digits-factorial-set-1/) 中计算数字的对于 `n` 的小值的解决方案。然而，该解决方案无法处理 `n > 10^6` 的情况，那么，我们能改进我们的解决方案吗？
是的！我们可以。
我们可以用 Kamenetsky 的公式找到我们的答案！
...
就这样！
我们的解决方案可以处理非常大的输入，可以容纳在 32 位整数中，甚至更大！。
以下是以上想法的实现:
