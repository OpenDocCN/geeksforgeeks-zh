...
> 原文: [https://www.geeksforgeeks.org/print-the-indices-for-every-row-of-a-grid-from-which-escaping-from-the-grid-is-possible/](https://www.geeksforgeeks.org/print-the-indices-for-every-row-of-a-grid-from-which-escaping-from-the-grid-is-possible/)

给定维度为 `M * N` 的二进制[2D数组](https://www.geeksforgeeks.org/multidimensional-arrays-c-cpp/) `arr[][]` 表示网格，其中 `0` 表示在单元的主对角线上有一堵墙，而 `1` 表示在单元的交叉对角线上有一堵墙。任务是针对每一个 `i` 第行打印一个可以逃离网格的行的索引，假设一个人不能逃离网格的顶部或底部。
...
>
> 第 0、1 或 4 行不存在转义路径。
> 如果一个人从第 2 行第行进入网格，那么他将按照上图所示的路径从单元 `(2, 3)` 中出来。
> 如果一个人从 3 第行进入网格，那么他将按照上图所示的路径从单元 `(3, 3)` 中出来。
>
...
*   [使用变量 `row` 迭代范围 `[0, M–1]`](https://www.geeksforgeeks.org/range-based-loop-c/)，并执行以下操作:
    *   初始化两个变量，比如 `i` 和 `j` 来存储该人当前所在单元格的行索引和列索引。
    *   将 `row` 分配给 `i`，将 `0` 分配给 `j`。
