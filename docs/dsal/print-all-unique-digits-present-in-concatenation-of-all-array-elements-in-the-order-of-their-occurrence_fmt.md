...
> 原文: [https://www.geeksforgeeks.org/print-all-unique-digits-present-in-concatenation-of-all-array-elements-in-the-order-of-their-occurrence/](https://www.geeksforgeeks.org/print-all-unique-digits-present-in-concatenation-of-all-array-elements-in-the-order-of-their-occurrence/)
...
想法是[将所有数组元素转换成它们的等价字符串](https://www.geeksforgeeks.org/different-ways-for-integer-to-string-conversions-in-java/)并连接这些字符串，然后使用[哈希](https://www.geeksforgeeks.org/hashing-data-structure/)来查找所获得的数字中存在的唯一数字。
...
*   使用[类型转换](https://www.geeksforgeeks.org/typecasting-in-c/)将字符串 `S` 转换为等效整数(比如 `N`)(去掉前导 0)。
...
*   对于数字 `N` 的每个数字，执行以下操作:
    *   检查是否是 `visited`。
    *   如果该数字未被访问，且其频率是 `1`，则将该数字追加到 `list` 中。将该指标的值设为 `visited`。
*   [反转列表](https://www.geeksforgeeks.org/python-reversing-list/) `list` 并打印。
