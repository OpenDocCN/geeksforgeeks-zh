...
> 原文: [https://www.geeksforgeeks.org/check-possible-transform-one-string-another/](https://www.geeksforgeeks.org/check-possible-transform-one-string-another/)
...
**方法:**
如果可以将 `s1` 的前 `i` 个字符转换为 `s2` 的前 `j` 个字符，则令 `DP[i][j]` 为 `1`，否则 `DP[i][j] = 0`。近距离观察给了我们两个需要处理的条件。
最初 `DP[0][0] = 1`，如果 `DP[i][j] = 1`，则可以使用以下条件检查下一组。

**1.** 如果大写的 `s1[i]` 等于 `s2[j]`，则可以将 `s1` 的前 `i+1` 个字符转换为 `s2` 的前 `j+1` 个字符，因此 `DP[i+1][j+1] = 1`。

**2.** 如果 `s1[i]` 是小写，则可以删除该元素，因此前 `i+1` 个字符可以转换为 `s2` 的前 `j` 个字符。因此 `DP[i+1][j] = 1`。

如果 `DP[n][m] = 1`，则可以通过上述条件将 `s1` 转换为 `s2`。
