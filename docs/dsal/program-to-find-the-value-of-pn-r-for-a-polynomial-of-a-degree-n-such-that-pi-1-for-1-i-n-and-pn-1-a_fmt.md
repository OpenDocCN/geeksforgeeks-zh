...
> 原文: [https://www.geeksforgeeks.org/program-to-find-the-value-of-pn-r-for-a-polynomial-of-a-degree-n-such-that-pi-1-for-1-i-n-and-pn-1-a/](https://www.geeksforgeeks.org/program-to-find-the-value-of-pn-r-for-a-polynomial-of-a-degree-n-such-that-pi-1-for-1-i-n-and-pn-1-a/)

给定三个正整数 `N`、`R`、`A` 和一个多项式 `P(X)` 的次数 `N`、`P(i) = 1` 对于 `1 ≤ i ≤ N`，`P(N + 1)` 的值为 `A`，任务是求 `P(N + R)` 的值。
...
给定问题可以通过求给定多项式的表达式 `P(X)` 然后计算 `P(N + R)` 的值来求解。次数为 `N` 的多项式的一般形式是:
...
*   初始化一个变量，比如 `ans`，作为 `(A–1)/N!` 的值。
*   使用变量 `i` 迭代范围 `[1，N]`，并将 `ans` 的值更新为 `ans *(N+R–i)`。
