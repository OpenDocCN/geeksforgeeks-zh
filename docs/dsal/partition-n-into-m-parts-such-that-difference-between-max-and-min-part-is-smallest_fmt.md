...
> 原文：[https://www.geeksforgeeks.org/partition-n-into-m-parts-such-that-difference-between-max-and-min-part-is-smallest/](https://www.geeksforgeeks.org/partition-n-into-m-parts-such-that-difference-between-max-and-min-part-is-smallest/)
...
为了尽量缩小各部分之间的差异，我们应该让所有部分尽可能地相互接近。假设我们可以打印任何浮点值而不是整数，在这种情况下，答案是 `0`（打印 `N/M` 共 `M` 次）。但是因为我们需要打印整数，所以我们可以把它分成两部分，`floor(N/M)` 和 `floor(N/M) + 1`，这样我们得到的答案最多是 `1`。

我们需要打印每种类型的多少个项？
假设我们打印 `floor(N/M)` 共 `M` 次，总和等于 `N - (N % M)`。所以我们需要选择 `N % M` 项，将其值增加 `1`。
