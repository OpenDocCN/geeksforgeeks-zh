...
class GFG 
{
    ...
    public static void Main(String []args) 
    {
        ...
        Console.WriteLine("Linked list before replacing"
            + " duplicates\n"); 
        printList(head);

        replaceDuplicates(head);

        Console.WriteLine("Linked list after replacing"
            + " duplicates\n"); 
        printList(head); 
    } 
}
...
本文由 [Chhavi](https://auth.geeksforgeeks.org/profile.php?user=chhavi%20saini%201&list=practice) 贡献。如果您喜欢 GeeksforGeeks 并希望做出贡献，则还可以使用 [contribute.geeksforgeeks.org](http://www.contribute.geeksforgeeks.org) 撰写文章，或将您的文章邮寄至 contribute@geeksforgeeks.org。查看您的文章出现在 GeeksforGeeks 主页上，并帮助其他 Geeks。
