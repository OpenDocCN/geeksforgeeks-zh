...
> 原文：[https://www.geeksforgeeks.org/minimum-sprinklers-required-to-be-turned-on-to-water-the-plants/](https://www.geeksforgeeks.org/minimum-sprinklers-required-to-be-turned-on-to-water-the-plants/)

给定一个由 `N` 个整数组成的数组 `arr[]`，其中第 `i` 个元素代表喷水器的范围，即 `[i-arr[i], i+arr[i]]` 它可以喷水，任务是找到最少数量的要打开的喷水器来给画廊中的每一株植物喷水。如果不可能给每一株植物浇水，那么打印 `-1`。
...
以上问题可以使用贪婪手法解决。其思想是首先通过左边界对范围进行排序，然后从左开始遍历范围，并在每次迭代中选择洒水喷头可以覆盖的最右边的边界，使左边界在当前范围内。按照以下步骤解决问题：

*   初始化一个 `vector<pair<int, int>>` 说 `V` 将每个洒水车的射程存储为一对。
*   遍历数组 `arr[]`，如果 `arr[i]` 不等于 `-1`，则推矢量 `V` 中的对 `(i-arr[i], i+arr[i])`。
*   按第一个元素对成对向量进行升序排序。
*   初始化 `2` 变量表示 `res`，和 `maxRight` 来存储要打开的最小洒水喷头和存储阵列的最右边界。
*   初始化一个变量，比如说 `i` 为 `0` 来迭代 `V`。
*   重复直到 `maxRight` 小于 `N`，并执行以下步骤：
    *   如果 `i` 等于 `V.size()` 或 `V[i].first` 大于 `maxRight`，则打印 `-1` 并返回。
    *   将当前洒水喷头的右边界存储在变量中，比如 `currMax`。
    *   现在迭代直到 `i+1` 小于 `V.size()` 和 `V[i+1].first` 小于或等于 `maxRight`，然后在每次迭代中以 `1` 递增 `i`，并将 `currMax` 更新为 `currMax = max(currMax, V[i].second)`。
    *   如果 `currMax` 小于 `maxRight`，则打印 `-1` 并返回。
    *   将 `maxRight` 更新为 `maxRight = currMax + 1`，然后将 `res` 和 `i` 增加 `1`。
