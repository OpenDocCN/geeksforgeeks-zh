...
> `10 * 3 * 6 * 5` 是除 `2` 外所有数组元素的乘积为 `900`。
>
> **输入:** `arr[] = {1, 2, 1, 3, 4}`
> **输出:** `24 12 24 8 6`

**方法:** 思路是用 [`log()`](https://www.geeksforgeeks.org/log-function-cpp/) 和 [`exp()`](https://www.geeksforgeeks.org/exp-function-cpp/) 函数代替 `log10()` 和 [`pow()`](https://www.geeksforgeeks.org/power-function-cc/)。以下是一些关于这方面的观察:

*   假设 `M` 是所有数组元素的乘积，那么在 `i`<sup>`th`</sup> 位置的输出数组元素将等于 `M/arr[i]`。
...
*   初始化两个变量，比如 `product = 1` 和 `Z = 1`，存储数组的乘积和零元素的个数。
*   [如果 `arr[i]` 不等于 `0`，则遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)并将乘积乘以 `arr[i]`。否则，将 `Z` 的计数增加 `1`。
*   [遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]` 并执行以下操作:
    ...
    *   否则，如果 `Z` 为 `1` 且 `arr[i]` 为 `0`，则更新 `arr[i]` 为 `product`，[继续](https://www.geeksforgeeks.org/continue-statement-cpp/)。
    *   否则，如果 `Z` 大于 `1`，则将 `arr[i]` 指定为 `0` 并继续。
    *   现在使用上面讨论的公式找到 `abs(product)/abs(arr[i])` 的值，并将其存储在一个变量中，比如 `curr`。
    *   如果 `arr[i]` 和 `product` 的值为负，或者如果 `arr[i]` 和 `product` 为正，则将 `arr[i]` 指定为 `curr`。
