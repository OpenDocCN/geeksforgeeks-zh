...
第二个实现在时间复杂度方面更好，但是非常复杂，因为我们已经实现了自己的优先级队列。STL 提供[优先级队列](http://geeksquiz.com/priority-queue-container-adaptors-the-c-standard-template-library-stl/)，但提供的优先级队列不支持减键操作。在 Prim 的算法中，我们需要一个[优先级队列](http://geeksquiz.com/binary-heap/)以及以下对优先级队列的操作：
...
**使用加权图的向量数组表示的更快实现**：[https://www.geeksforgeeks.org/graph-implementation-using-stl-for-competitive-programming-set-2-weighted-graph/](https://www.geeksforgeeks.org/graph-implementation-using-stl-for-competitive-programming-set-2-weighted-graph/)
