...
> 原文: [https://www.geeksforgeeks.org/print-sum-of-matrix-and-its-mirror-image/](https://www.geeksforgeeks.org/print-sum-of-matrix-and-its-mirror-image/)
...
当找到矩阵的镜像时，每个元素的行将保持不变，但其列的值将重新排列。对于任何元素 `A_ij`，其在镜像中的新位置将是 `A_i(n-j)`。获得矩阵的镜像后，将其添加到原始矩阵并打印结果。

## 注意事项

1.  矩阵的索引将从 `0, 0` 开始，在 `n-1, n-1` 结束，因此任何元素 `A_ij` 的位置将是 `A_i(n-1-j)`。
2.  打印结果时，请注意正确的输出格式。
