...
> 原文: [https://www.geeksforgeeks.org/0-1-knapsack-using-least-count-branch-and-bound/](https://www.geeksforgeeks.org/0-1-knapsack-using-least-count-branch-and-bound/)
...
本文讨论了 [0/1 背包问题](https://www.geeksforgeeks.org/0-1-knapsack-problem-dp-10/)使用最小成本(LC)的**分支定界法**的实现。
...
>
> 位于 `i` 级别的左分支和右分支存储包括和不包括 `i` 元素获得的最大值。
> 下图显示了每一步后优先级队列的状态：
>
...
以下是关于0/1背包问题分支定界算法实现的代码示例，包含C++和Java两种语言的实现。
