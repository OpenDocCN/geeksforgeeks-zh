...
> 原文: [https://www.geeksforgeeks.org/modify-n-by-adding-its-smallest-positive-divisor-exactly-k-times/](https://www.geeksforgeeks.org/modify-n-by-adding-its-smallest-positive-divisor-exactly-k-times/)
...
解决这个问题最简单的方法是使用变量 `i` 迭代范围 `[1, K]`，在每次运算中，[找到大于 N](https://www.geeksforgeeks.org/find-divisors-natural-number-set-1/) 的 `1` 的最小除数，并将 `N` 的值增加大于 `N` 的 `1` 的最小除数。最后打印 `N` 的值。
...
*   否则，找到大于 `N` 的 `1` 的最小正除数，记为 `smDiv` 并将值 `N` 更新为 `(N + smDiv + (K - 1) * 2)`。
*   最后，打印 `N` 的值。
