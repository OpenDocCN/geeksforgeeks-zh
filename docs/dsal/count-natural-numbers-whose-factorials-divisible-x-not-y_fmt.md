...
> 原文：[https://www.geeksforgeeks.org/count-natural-numbers-whose-factorials-divisible-x-not-y/](https://www.geeksforgeeks.org/count-natural-numbers-whose-factorials-divisible-x-not-y/)

给定两个数字 `x` 和 `y` (`x <= y`)，找出所有满足其阶乘 `i!` 能被 `x` 整除但不能被 `y` 整除的自然数 `i` 的总数。
...
一个**简单的解决方法**就是从 1 迭代到 `y-1`，对于每一个数 `i`，检查 `i!` 是否可被 `x` 整除且不能被 `y` 整除。如果我们应用这种朴素的方法，我们就不会超过 `20!` 或者 `21!`（`long long int` 会有上限）。

一个**更好的解决方案**是基于下述帖子：
[求第一个阶乘可被 x 整除的自然数](https://www.geeksforgeeks.org/find-first-natural-number-whose-factorial-divisible-x/)

我们使用上述方法分别求出第一个阶乘可被 `x` 整除的自然数和第一个阶乘可被 `y` 整除的自然数。设阶乘可被 `x` 和 `y` 整除的第一个自然数分别为 `xf` 和 `yf`。我们的最终答案将是 `yf - xf`。这个公式是基于这样一个事实：如果 `i!` 可被数字 `x` 整除，那么 `(i+1)!`，`(i+2)!`，... 也可以被 `x` 整除。
...
本文由 `Shubham Gupta` 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用`contribute.geeksforgeeks.org`写一篇文章或者把你的文章邮寄到`contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
