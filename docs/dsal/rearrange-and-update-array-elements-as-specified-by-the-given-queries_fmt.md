...
> 原文：[https://www.geeksforgeeks.org/rearrange-and-update-array-elements-as-specified-by-the-given-queries/](https://www.geeksforgeeks.org/rearrange-and-update-array-elements-as-specified-by-the-given-queries/)

给定一个大小为 `N` 的数组 `arr[]`，并查询 `Q[][]`，任务是对给定的数组执行以下类型的查询。
- `0`：将数组左移一个位置。
- `1`：将数组右移一个位置。
- `2 X Y`：更新 `arr[X] = Y` 的值。
- `3 X`：打印 `arr[X]`。
...
**处理方法：** 使用[deque（双端队列）](https://www.geeksforgeeks.org/deque-cpp-stl/)在 O(1) 中的队列前后执行插入和删除操作，可以解决这个问题。按照以下步骤解决问题。
