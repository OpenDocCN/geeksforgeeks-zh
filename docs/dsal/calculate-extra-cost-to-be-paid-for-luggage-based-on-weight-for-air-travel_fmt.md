...
> 原文: [https://www.geeksforgeeks.org/calculate-extra-cost-to-be-paid-for-luggage-based-on-weight-for-air-travel/](https://www.geeksforgeeks.org/calculate-extra-cost-to-be-paid-for-luggage-based-on-weight-for-air-travel/)

给定一个包含行李重量的 `N` 大小的数组 `weights[]`。如果重量在 `W` 的阈值内，则不需要任何额外的成本。但是根据下表，在重量超过阈值后，他们需要支付额外的费用。任务是计算所需行李的额外费用。
...
> **输入:** `weights[] = {5, 4, 3, 6}`，`W = 8`
> **输出:** 0
> **说明:** 无权重过阈值。因此不会产生额外的成本。
>
> **输入:** `weights[] = {120, 135, 280, 60, 300}`，`W = 90`
> **输出:** 1700
