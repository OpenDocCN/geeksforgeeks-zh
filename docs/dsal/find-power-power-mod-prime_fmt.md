...
给定四个数字 `A`，`B`，`C` 和 `M`，其中 `M` 是质数。我们的任务是找到 `A^(B^C) (mod M)`。
...
`天真法`是先计算 `res = B^C` ，然后用模指数计算 `A^res % M`。这种方法的问题是，我们不能直接在 `B^C` 上应用 `mod M`，所以我们必须在没有 `mod M` 的情况下计算这个值。但是如果我们直接求解它，那么我们会得出 `A` 的指数的大值，这肯定会在最终答案中溢出。

一种`有效的方法`是利用[费马小定理](https://en.wikipedia.org/wiki/Fermat's_little_theorem)将 `B^C` 缩小到一个较小的值，然后应用模指数。
