...
给定`n`行和`m`列的矩阵。任务是将每个矩阵元素替换为其行或列的最大公因数，以最大者为准。也就是说，对于每个元素`(i, j)`，请从第`i`行的 GCD 或第`j`列的 GCD 中将其替换，以较大者为准。
...
Output :  1 1 3 3
          1 1 3 3
          1 1 3 3
For index (0,2), GCD of row 0 is 1, GCD of column 2 is 3.
...
使用`暴力`方法，我们可以遍历矩阵的元素，找到与该元素对应的行和列的 GCD 并将其替换为两者中的最大值。

一种有效的`方法`是分别为行和列制作大小为`n`和`m`的两个数组。并存储每一行和每一列的 GCD。大小为`n`的数组将包含每一行的 GCD，大小为`m`的数组将包含每一列的 GCD。并用其对应的行 GCD 或列 GCD 的最大值替换每个元素。
