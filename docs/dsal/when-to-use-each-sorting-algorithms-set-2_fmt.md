...
> 原文：[https://www.geeksforgeeks.org/when-to-use-each-sorting-algorithms-set-2/](https://www.geeksforgeeks.org/when-to-use-each-sorting-algorithms-set-2/)
...
[堆排序](https://www.geeksforgeeks.org/heap-sort/)从可用元素构建二进制堆数据结构，然后使用堆进行排序。这是一种基于比较的（就地）排序算法，没有二次最坏情况运行时间。Heapsort 可以按照以下限制使用：
...
[希尔排序](https://www.geeksforgeeks.org/shellsort/)是原地比较排序。该算法避免了大的移位，如在插入排序的情况下，如果较小的值在最右边并且必须移动到最左边。与插入排序或冒泡排序相比，壳排序更有效，并且在以下情况下使用：
...
[桶排序](https://www.geeksforgeeks.org/bucket-sort-2/)的工作原理是将数组的元素分配到多个桶中。然后，每个存储桶被单独排序，并被追加到一个数组中。这是一个相当稳定的算法，因为一旦元素被分配到桶中，每个桶都可以独立于其他桶进行处理。桶排序可以按照以下约束使用：
