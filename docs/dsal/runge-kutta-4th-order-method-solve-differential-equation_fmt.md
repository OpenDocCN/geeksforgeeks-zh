...
> 原文：[https://www.geeksforgeeks.org/runge-kutta-4th-order-method-solve-differential-equation/](https://www.geeksforgeeks.org/runge-kutta-4th-order-method-solve-differential-equation/)
...
任务是求未知函数 `y` 在给定点 `x` 的值。龙格-库塔法求给定 `x` 的 `y` 的近似值，只有一阶常微分方程可以用龙格-库塔四阶法求解。

以下是用于根据前一个值 `y_n` 计算下一个值 `y_{n+1}` 的公式。`n` 的值是 0，1，2，3，…`(x–x0)/h`。此处 `h` 为步高，`x_{n+1}= x_0+h`。较小的步长意味着更高的精度。
...
该公式基本上使用当前 `y_n` 加上四个增量的加权平均值来计算下一个值 `y_{n+1}`。
*   `k_1` 是基于区间开始时斜率的增量，使用 `y`。
*   `k_2` 是基于区间中点斜率的增量，使用 `y + hk_1 /2`。
*   `k_3` 同样是基于中点斜率的增量，使用 `y + hk_2 /2`。
*   `k_4` 是基于区间末端斜率的增量，使用 `y + hk_3`。

该方法为四阶方法，即局部截断误差为 `O(h^5)` 阶，而总累积误差为 `O(h^4)` 阶。
...
[http://w3.gazi.edu.tr/~balbasi/mws_gen_ode_txt_runge4th.pdf](http://w3.gazi.edu.tr/~balbasi/mws_gen_ode_txt_runge4th.pdf)
[https://www.youtube.com/watch?v=kUcc8vAgoQ0](https://www.youtube.com/watch?v=kUcc8vAgoQ0)
