# 检查 K 个 '0' 是否可以翻转，使得二进制字符串不包含一对相邻的 '1'

> 原文: [https://www.geeksforgeeks.org/check-if-k-0s-can-be-flipped-such-that-binary-string-contains-no-pair-of-adjacent-1s/](https://www.geeksforgeeks.org/check-if-k-0s-can-be-flipped-such-that-binary-string-contains-no-pair-of-adjacent-1s/)

给定一个长度为 `N` 的[二进制字符串](https://www.geeksforgeeks.org/tag/binary-string/) `S` 和一个整数 `K`，任务是检查是否有可能翻转 `K` 个 `0`，使得生成的字符串不包含任何一对相邻的 `1`。如果可以，则打印 `"Yes"`。否则，打印 `"No"`。

> **输入:** `S = "01001001000"`, `K = 1`
> **输出:** Yes
> **解释:**
> 修改 `S = "01001001000"` 到 `"01001001001"` 或者修改 `S = "01001001000"` 到 `"01001001010"`。
>
> **输入:** `S = "000001000"`, `K = 4`
> **输出:** No
...
想法是[遍历字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-python/)并用两个相邻字符都为 `'0'` 的 `'0'` 替换那些 `'1'`，并翻转其中一个 `'0'`，以计算所有可能的翻转位置，从而不影响原始字符串。按照以下步骤解决问题:

*   初始化一个变量，比如 `cnt` 为 `0`，存储可以翻转的位置数。
...
*   完成上述步骤后，如果 `cnt` 的值至少为 `K`，则打印 `"Yes"`，否则打印 `"No"`。
