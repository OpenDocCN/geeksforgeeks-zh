...
> 原文: [https://www.geeksforgeeks.org/convert-a-binary-string-to-another-by-flipping-prefixes-minimum-number-of-times/](https://www.geeksforgeeks.org/convert-a-binary-string-to-another-by-flipping-prefixes-minimum-number-of-times/)
...
给定的问题可以通过逐个固定位来解决。要固定第 `i` 位，当 `A[i]` 和 `B[i]` 不相等时，翻转长度为 `i` 的前缀，然后翻转长度为 `1` 的前缀。现在，翻转长度为 `i` 的前缀。这三个操作不会改变 `A` 中的任何其他位。在 `A[i]` 不等于 `B[i]` 的所有指数上执行这些操作。由于每一位使用 3 次操作，总体来说将使用 `3 * N` 次操作。

为了最小化操作的数量，可以通过以相反的顺序逐个固定比特来修改上述方法。要固定第 `i` 位，要么需要翻转长度为 `i` 的前缀，要么需要翻转第一位，然后需要翻转长度为 `i` 的前缀。但是按照相反的顺序，先前固定的位不会通过该过程再次翻转，并且每个位最多需要 `2` 次操作。因此，所需的最小操作次数为 `2 * N`。
