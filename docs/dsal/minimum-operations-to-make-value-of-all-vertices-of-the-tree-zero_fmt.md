...
对于下面的树，我们执行以下 3 个操作以使所有顶点的值等于零：
...
现在，如果我们在某个非叶节点中说 `v`，我们看一下它的所有子节点，如果说对 `V` 的子节点 `i` 进行 `X_i` 次加法运算，则需要应用 `max(X_i)` 对于节点 `v` 的所有子节点 `i`，增加以 `v` 为根的任何子树的操作。类似地，对于节点 `V` 的减少操作，我们执行相同的操作。
