...
> 原文: [https://www.geeksforgeeks.org/find-reminder-of-array-multiplication-divided-by-n/](https://www.geeksforgeeks.org/find-reminder-of-array-multiplication-divided-by-n/)
...
## 天真的方法:
首先乘以所有的数，然后取 `%` 乘 `n`，然后求余数，但是在这种方法中，如果数是 `2^64` 的最大值，那么它给出错误的答案。

## 避免溢出的方法:
首先取一个余数或像 `arr[i] % n` 这样的单个数，然后将余数与当前结果相乘。乘法后，再次取余数，避免溢出。这是因为模运算的分布特性。`(a * b) % c = ( ( a % c ) * ( b % c ) ) % c`
