...
> 原文: [https://www.geeksforgeeks.org/smallest-subarray-whose-product-leaves-remainder-k-when-divided-by-size-of-the-array/](https://www.geeksforgeeks.org/smallest-subarray-whose-product-leaves-remainder-k-when-divided-by-size-of-the-array/)

给定一个由 `N` 个整数和一个整数 `K` 组成的数组 `arr[]`，任务是找出最小子阵列的长度，该子阵列的乘积除以 `N` 得到余数 `K`。如果不存在这样的子阵列，打印 `-1`。
...
**方法:**
想法是生成给定阵列的所有可能的子阵列并打印最小子阵列的长度，当除以 `N` 时，其所有元素的乘积给出余数 `K`。
