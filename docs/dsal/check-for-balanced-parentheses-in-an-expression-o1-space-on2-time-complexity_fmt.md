...
> 原文：[https://www.geeksforgeeks.org/check-for-balanced-parentheses-in-an-expression-o1-space-on2-time-complexity/](https://www.geeksforgeeks.org/check-for-balanced-parentheses-in-an-expression-o1-space-on2-time-complexity/)
...
*   遇到左括号时，设置 `j = i`、`i = i + 1` 和 `count++`。
*   当遇到右括号时，递减计数并在 `i` 和 `j` 比较括号，
    *   如果 `i` 和 `j` 处的括号匹配，则在 `i` 和 `j` 位置替换字符串中的 `'#'`。递增 `i`，递减 `j`，直到遇到非 `'#'` 值或 `j >= 0`。
