...
> 原文：[https://www.geeksforgeeks.org/find-median-of-two-given-vectors/](https://www.geeksforgeeks.org/find-median-of-two-given-vectors/)

给定两个向量，不同大小的 `a` 和 `b`，其中数组 `a` 有 `m` 个元素，数组 `b` 有 `n` 个元素。任务是找到两个向量的中值。这个问题是两个不同大小排序数组[中值](https://www.geeksforgeeks.org/median-of-two-sorted-arrays-of-different-sizes/)问题的扩展。
...
**时间复杂度:** `O(m + n)`，因为合并两个向量 `O(m + n)` 需要时间。
**空间复杂度:** `O(1)`，因为不需要额外空间。