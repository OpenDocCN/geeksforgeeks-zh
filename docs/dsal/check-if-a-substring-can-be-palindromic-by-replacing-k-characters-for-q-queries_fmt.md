...
> 原文: [https://www.geeksforgeeks.org/check-if-a-substring-can-be-palindromic-by-replacing-k-characters-for-q-queries/](https://www.geeksforgeeks.org/check-if-a-substring-can-be-palindromic-by-replacing-k-characters-for-q-queries/)

给定一个字符串 `str` 和 `Q` 以 `【L，R，K】` 形式查询，任务是从最多允许 `K` 变化的 `【L，R】` 的字符串中找出字符是否可以重新排列，使字符串回文。对于每个查询，如果可以变成回文串，则打印 `“是”`，否则打印 `“否”`。
...
*   获取不匹配对的计数。
*   现在我们需要将一半不匹配的字符转换为剩余的字符。如果一半不匹配字符的计数小于或等于 `K`，则打印 `“是”`，否则打印 `“否”`。
