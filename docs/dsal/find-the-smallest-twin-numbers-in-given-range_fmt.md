...
> 原文: [https://www.geeksforgeeks.org/find-the-smallest-twin-numbers-in-given-range/](https://www.geeksforgeeks.org/find-the-smallest-twin-numbers-in-given-range/)

给定范围 [`low`..`high`]，打印给定范围（包括 `low` 和 `high`）内最小的孪生数。两个数是孪生的，如果它们是素数，差是 2。
...
一个**简单的解决方法**是从 `low` 开始，对于每个数字 `x`，检查 `x` 和 `x + 2` 是否是素数。这里 `x` 从 `low` 到 `high` 不等-2。
