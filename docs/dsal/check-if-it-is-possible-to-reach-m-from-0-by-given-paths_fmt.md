...
> 原文: [https://www.geeksforgeeks.org/check-if-it-is-possible-to-reach-m-from-0-by-given-paths/](https://www.geeksforgeeks.org/check-if-it-is-possible-to-reach-m-from-0-by-given-paths/)
...
*   初始化一个[数组](https://www.geeksforgeeks.org/array-data-structure/)，比如 `rightMost[]` 和 `dp[]`，分别存储从 `i` 点到最远点的距离，并用 `0` 初始化 `rightMost[]` 的每个值。
*   [迭代范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[0, N-1]` 并将 `rightMost[a[i][0]]` 更新为 `rightMost[a[i][0]]` 和 `b` 的[最大值](https://www.geeksforgeeks.org/stdmax_element-in-cpp/)。
*   [使用变量 `i` 迭代范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[M, 0]`:
    *   将 `dp[i]` 的值更新为 `i`。
    *   [使用变量 `j` 迭代范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[min(M, rightMost[i]), i+1]`，并将 `dp[i]` 更新为 `dp[i]` 和 `dp[j]` 的最大值。
