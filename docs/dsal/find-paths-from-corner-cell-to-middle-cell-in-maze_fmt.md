...
> 原文：[https://www.geeksforgeeks.org/find-paths-from-corner-cell-to-middle-cell-in-maze/](https://www.geeksforgeeks.org/find-paths-from-corner-cell-to-middle-cell-in-maze/)
...
1.  如果到达目的地，打印路径。
2.  否则：
    *   将当前单元格标记为已访问，并将其添加到路径数组中。
    *   在所有 4 个允许的方向上前进，并递归地检查它们中是否有任何一个导致解决方案。
    *   如果以上解决方案都不起作用，则将此单元格标记为不可见，并将其从路径数组中删除。
