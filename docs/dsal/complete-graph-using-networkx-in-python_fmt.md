# 使用 Python 中的 NetworkX 创建完全图

> 原文: [https://www.geeksforgeeks.org/complete-graph-using-networkx-in-python/](https://www.geeksforgeeks.org/complete-graph-using-networkx-in-python/)
...
我们将使用 `networkx` 模块来实现一个完整的图形。它带有一个内置函数 `networkx.complete_graph()`，可以使用 `networkx.draw()` 方法进行说明。Python 中的这个模块用于可视化和分析不同类型的图形。
...
>
> *   `n`: 完全图中的节点数。
> *   返回一个 `networkx.Graph` 完整对象。
> *   节点的索引从零到 `n-1`。
>
> **用于通过传递图形对象来实现图形。**
>
> `draw(G, node_size, node_color)`
>
> **参数:**
>
> *   `G`: 指完整的图形对象
