...
> 原文: [https://www.geeksforgeeks.org/check-if-a-given-string-is-a-comment-or-not/](https://www.geeksforgeeks.org/check-if-a-given-string-is-a-comment-or-not/)
...
*   If `line[0] == '/'`:
    *   如果 `line[1] == '/'`，则打印“这是单行注释”。
    *   如果 `line[1] == '*'`，则遍历字符串，如果找到任何相邻的 `'*'` 和 `'/'` 对，则打印“这是多行注释”。
*   否则，打印“这不是评论”。
