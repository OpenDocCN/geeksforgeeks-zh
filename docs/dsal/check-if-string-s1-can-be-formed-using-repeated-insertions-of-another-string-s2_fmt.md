# 检查字符串 S1 是否可以通过重复插入另一根字符串 S2 形成

> 原文：[https://www.geeksforgeeks.org/check-if-string-s1-can-be-formed-using-repeated-insertions-of-another-string-s2/](https://www.geeksforgeeks.org/check-if-string-s1-can-be-formed-using-repeated-insertions-of-another-string-s2/)
...
> *   在 `"a"` 之后插入 `"ab"`。最后一串将是 `"aabb"`
>
> **输入：** `S1 = "ababcd"`, `S2 = "ABC"`
> **输出：** 否
> **说明：** 任何一个系列的操作都不可能获得上面的字符串。

**方法：** 给定的问题可以使用[栈数据结构](https://www.geeksforgeeks.org/stack-data-structure/)来解决。想法是将 `S1` 的字符插入栈，直到找到 `S2` 的最后一个字符。然后从栈中弹出长度为 `len(S2)` 的字符，并与 `S2` 进行比较。如果不一样，停止并返回 `false`。否则重复这个过程，直到 `S1` 变空。

以下是上述方法的步骤：
*   首先，检查以下案例，如果发现为真，则返回 `false`：
    *   `S1` 独特字符的数量必须与 `S2` 相同
    *   字符串 `S1` 的长度必须是 `S2` 的倍数
*   维护一个[栈](https://www.geeksforgeeks.org/stack-data-structure-introduction-program/)来存放所有字符
*   遍历[字符串](https://www.geeksforgeeks.org/string-class-in-java/) `S1` 并将字符压入栈中
*   如果当前字符是字符串 `S2` 的最后一个字符，则匹配栈顶的所有字符
*   如果任何位置的栈为空或字符不匹配，则返回 `false`
*   在字符串上完成迭代后，检查栈是否为空。如果栈不为空，则返回 `false`，否则返回 `true`
