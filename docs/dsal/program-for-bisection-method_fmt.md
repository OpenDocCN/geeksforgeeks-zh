...
给定浮点数 `x` 上的函数 `f(x)` 和两个数字 `a` 和 `b`，使得 `f(a)*f(b) < 0` 且 `f(x)` 在 `[a, b]` 上连续。这里 `f(x)` 代表代数或超越方程。在区间 `[a, b]` 内找到函数的根（或找到一个 `x` 值使得 `f(x)` 为 0）。
...
代数函数是可以用多项式形式表示的函数，如 `f(x)= a<sub>1</sub>x<sup>3</sup>+a<sub>2</sub>x<sup>2</sup>+…..+ e` 其中 `a<sub>1</sub>`，`a<sub>2</sub>`，…为常数，`x` 为变量。
超越函数是非代数函数，例如 `f(x)= sin(x)* x–3` 或 `f(x) = e <sup>x</sup> + x <sup>2</sup>` 或 `f(x)= ln(x)+x`……。

## 什么是等分法？

该方法也称为区间减半法、二分搜索法或二分法。这种方法用于在给定的区间内寻找方程的根，该区间的值为 `f(x) = 0` 的 `x`。
