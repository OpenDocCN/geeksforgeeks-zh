...
> 原文: [https://www.geeksforgeeks.org/time-until-distance-gets-equal-to-x-between-two-objects-moving-in-opposite-direction/](https://www.geeksforgeeks.org/time-until-distance-gets-equal-to-x-between-two-objects-moving-in-opposite-direction/)
...
> **输入:** `U = 3`，`V = 3`，`X = 3`
> **输出:** `0.5`
> `0.5` 秒后，A 民警距离 `1.5` 米与 B 民警距离 `1.5` 米方向相反，两民警距离为 `1.5 + 1.5 = 3`
> **输入:** `U = 5`，`V = 2`，`X = 4`
> **输出:** `0`

## 进场

可以用 `距离 = 速度 * 时间` 求解。这里，距离将等于给定的范围，即 `距离 = X`，速度将是两个速度的总和，因为它们以相反的方向移动，即 `速度 = U + V`。
