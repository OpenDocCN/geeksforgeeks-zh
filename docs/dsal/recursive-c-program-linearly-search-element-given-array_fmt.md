...
> 原文: [https://www.geeksforgeeks.org/recursive-c-program-linearly-search-element-given-array/](https://www.geeksforgeeks.org/recursive-c-program-linearly-search-element-given-array/)

给定一个未排序的数组和一个元素 `x`，在给定数组中搜索 `x`。为此编写递归 C 代码。如果元素不存在，返回 `-1`。

`方法:` 想法是将 `x` 与 `arr[]` 中的最后一个元素进行比较。如果在最后一个位置找到一个元素，返回它。否则对剩余数组和元素 `x` 重复 `elmntSrch()`
