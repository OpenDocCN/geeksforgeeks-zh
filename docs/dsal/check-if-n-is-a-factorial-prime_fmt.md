...
> 原文: [https://www.geeksforgeeks.org/check-if-n-is-a-factorial-prime/](https://www.geeksforgeeks.org/check-if-n-is-a-factorial-prime/)
...
*   如果 `N` 不是质数，则打印 `No`。
*   否则设置`fact = 1` 并从 `i = 1` 开始更新`fact = fact * i`，如果`fact = N–1`或`fact = N + 1` 则打印`是`。
*   重复上述步骤，直到`fact ≤ N + 1`，如果条件不满足，则最终打印`否`。
