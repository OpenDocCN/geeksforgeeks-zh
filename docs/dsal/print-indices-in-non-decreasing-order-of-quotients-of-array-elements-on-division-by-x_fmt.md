...
> 原文：[https://www.geeksforgeeks.org/print-indices-in-non-decreasing-order-of-quotients-of-array-elements-on-division-by-x/](https://www.geeksforgeeks.org/print-indices-in-non-decreasing-order-of-quotients-of-array-elements-on-division-by-x/)

## 问题描述

给定一个由 `N` 个整数和一个整数 `X` 组成的数组 `arr[]`，任务是通过 `X` 对数组元素执行整数除法，并按照获得的商的非递减顺序打印数组的索引。

**示例:**

> **输入:** `N = 3`，`X = 3`，`order[] = {2，7，4}`
> **输出:** `1 3 2`
> **解释:**
> 将数组元素除以 3 后，数组修改为 `{0，2，1}`。因此，所需的输出顺序是 `1 3 2`。

> **输入:** `N = 5`，`X = 6`，`order[] = {9，10，4，7，2}`
> **输出:** `3 5 1 2 4`
> **解释:**
...
*   遍历数组。
*   初始化 `pair` 的 `vector`。
*   对于每个数组元素，将除以 `X` 得到的商的值存储为 `vector` 中的 `pair` 的第一个元素，将第二个元素存储为所需顺序中整数的位置。
*   遍历结束后，对 `pair` 向量进行排序，最后打印出对的所有第二个元素。
