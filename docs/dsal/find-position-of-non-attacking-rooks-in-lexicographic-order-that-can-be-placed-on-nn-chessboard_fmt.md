...
> 原文：[https://www.geeksforgeeks.org/find-position-of-non-attacking-rooks-in-lexicographic-order-that-can-be-placed-on-nn-chessboard/](https://www.geeksforgeeks.org/find-position-of-non-attacking-rooks-in-lexicographic-order-that-can-be-placed-on-nn-chessboard/)
...
初始化一个大小为 `N*N` 的二维矩阵 `mat[][]`，每个单元格为 `0`，并以 `1` 标记初始位置的车。然后遍历矩阵 `mat[][]`，检查第 `i` 行和第 `j` 列是否包含任何车，记录放置的车数。如果任何一行和一列都不包含任何放置的车，那么在那里放置一个车，并将此单元格添加到结果字符串中。最后，打印放置的车数和车位置。
...
想法是创建两个大小各为 `N` 的数组，以存储第 `i` 行或第 `i` 列是否已被占用。
...
问题中的关键观察是，可以放置的最大车数是 `N-K`。也就是说，如果两个车在同一排或同一列，它们就会互相攻击。因为没有两个给定的车互相攻击，所以输入中给定的所有行都是唯一的。同样，输入中给出的所有列都是唯一的。因此，我们剩下 `N-K` 个未使用的行和 `N-K` 个未使用的列来放置新的车。

换句话说，如果我们试图通过[鸽子洞原理](https://www.geeksforgeeks.org/discrete-mathematics-the-pigeonhole-principle/)放入 `N-K` 个以上的车，如果有 `N+1` 只鸽子和 `N` 个地方要填充，那么至少有一个地方包含 `1` 只以上的鸽子。
