...
> 原文: [https://www.geeksforgeeks.org/replace-every-element-of-array-with-sum-of-elements-on-its-right-side/](https://www.geeksforgeeks.org/replace-every-element-of-array-with-sum-of-elements-on-its-right-side/)
...
一个简单的方法是运行两个[循环](https://www.geeksforgeeks.org/loops-in-c-and-cpp/)，外循环逐个固定每个元素，内循环计算固定元素右侧的元素之和。

**时间复杂度:** `O(N^2)`

### 高效方法

想法是计算数组的总和，然后将当前元素更新为 `sum - arr[i]`，并在每个步骤中将总和更新为当前元素。
