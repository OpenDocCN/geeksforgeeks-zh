...
> 原文: [https://www.geeksforgeeks.org/paths-from-entry-to-exit-in-matrix-and-maximum-path-sum/](https://www.geeksforgeeks.org/paths-from-entry-to-exit-in-matrix-and-maximum-path-sum/)

给定一个迷宫，它是一个 `N * N` 网格 `grid[][]`。迷宫的每个单元包含数字 `1`、`2` 或 `3`，其定义动作为:

*   如果 `grid[i][j] = 1`，那么唯一有效的移动就是 `grid[i][j + 1]`。
*   如果 `grid[i][j] = 2`，那么唯一有效的移动就是 `grid[i + 1][j]`。
*   如果 `grid[i][j] = 3`，那么有效的移动是 `grid[i][j + 1]` 和 `grid[i + 1][j]`。

现在，任务是找到从 `grid[0][0]` 到 `grid[N–1][N–1]` 的所有路径的计数，以及所有路径中的最大可能和，即当访问单元的所有值加到和上时。
...
> 最大和: 7
> 两个有效路径为
> `(0, 0) -> (0, 1) -> (1, 1)`，和为 `3 + 2 + 1 = 6`
> 和 `(0, 0) -> (1, 0) -> (1, 1)` 以和为 `3 + 3 + 1 = 7`

> **输入:** `grid[][] = { {1, 1, 3, 2, 1}, {3, 2, 2, 1, 2}, {1, 3, 3, 1, 3}, {1, 2, 3, 1, 2}, {1, 1, 3, 1} }`
> **输出:**
> 总计

## 方法

这个问题可以用[动态规划](https://www.geeksforgeeks.org/dynamic-programming/)解决。
要计算路径数，创建一个 `dp[][]` 数组，其中 `dp[i][j]` 将存储从 `grid[i][j]` 到 `grid[N–1][N–1]` 的路径数，循环关系为:

1.  如果 `grid[i][j] = 1`，那么 `dp[i][j] = dp[i][j + 1]`。
2.  如果 `grid[i][j] = 2`，那么 `dp[i][j] = dp[i + 1][j]`。
3.  如果 `grid[i][j] = 1`，那么 `dp[i][j] = dp[i][j + 1] + dp[i + 1][j]`。

而终止条件将是当源和目的地相同时，即 `dp[N–1][N–1] = 1`。

现在，为了找到最大和路径，可以创建另一个 `dp[][]` 数组，其中 `dp[i][j]` 将存储从 `grid[i][j]` 到 `grid[N–1][N–1]` 的路径的最大和，并且递归关系为:

1.  如果 `grid[i][j] = 1`，那么 `dp[i][j] = grid[i][j] + dp[i][j + 1]`。
2.  如果 `grid[i][j] = 2`，那么 `dp[i][j] = grid[i][j] + dp[i + 1][j]`。
3.  如果 `grid[i][j] = 1`，那么 `dp[i][j] = grid[i][j] + max(dp[i][j + 1] + dp[i + 1][j])`。

而终止条件将再次是当源和目的地相同时，即 `dp[N–1][N–1] = grid[N–1][N–1]`。
