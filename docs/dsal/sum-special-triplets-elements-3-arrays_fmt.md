...
> 原文：[https://www.geeksforgeeks.org/sum-special-triplets-elements-3-arrays/](https://www.geeksforgeeks.org/sum-special-triplets-elements-3-arrays/)

给定三个数组 `A`、`B` 和 `C`，任务是找出所有`特殊三元组`的值之和。特殊三元组定义为三元组 `(X, Y, Z)`，其中条件：`X ≤ Y 且 Z ≤ Y` 始终成立。每个三元组 `(X, Y, Z)` 的值由下式给出：
...
我们生成所有三元组并检查三元组是否是特殊三元组，我们使用 `f(x, y, z)` 计算三元组的值，其中 `(x, y, z)` 是特殊三元组，并将其添加到所有这些特殊三元组的最终总和中。
