# 从 L 到 R 的所有自然数的和（适用于 L 和 R 的大值）

> 原文：[https://www.geeksforgeeks.org/sum-of-all-natural-numbers-from-l-to-r-for-large-values-of-l-and-r/](https://www.geeksforgeeks.org/sum-of-all-natural-numbers-from-l-to-r-for-large-values-of-l-and-r/)

给定两个非常大的数字 `L` 和 `R`，其中 `L ≤ R`，任务是计算从 `L` 到 `R` 的所有自然数的和。和可能很大，所以打印`和 % 1000000007`。
...
*   设 `sum(N)` 是一个返回前 `N` 个自然数之和的函数。
*   前 `N` 个自然数之和为 `sum(N) = (N * (N + 1)) / 2`。
*   `L` 到 `R` 范围内的数字之和将是 `范围和 = sum(R) – sum(L - 1)`。

*   答案用模 `10^9 + 7` 计算，所以，

> `mod = 10^9 + 7`
> `范围和 = (sum(R) – sum(L - 1) + mod) % mod;`
> 这个也可以写成 `范围和 = (sum(R) % mod – sum(L - 1) % mod + mod) % mod;`
> 现在，`sum(R) % mod` 可以写成 `((R * (R + 1)) / 2) % mod`
> 或 `((R % mod) * ((R + 1) % mod) * invmod(2)) % mod`
> 由于 `R` 较大，所以 `R` 的模可以按[这里](https://www.geeksforgeeks.org/how-to-compute-mod-of-a-large-number/)所述进行计算。
> `invmod(2)` 的值 = `500000004`，可以用[费马小定理](https://en.wikipedia.org/wiki/Fermat%27s_little_theorem)计算。
> 同样，也可以计算 `sum(L – 1) % mod`。
