...
> 原文: [https://www.geeksforgeeks.org/compute-score-of-a-string-compound-of-balanced-parentheses/](https://www.geeksforgeeks.org/calculate-score-of-a-string-consisting-of-balanced-parentheses/)

给定一个由成对的平衡括号组成的字符串，任务是根据以下规则计算给定字符串的分数：
...
> **输入：** `str = "()"`
> **输出：** 2
> **说明：** 有两个平衡括号“()()”的单个对。因此，得分 = 得分为“()” + 得分为“()” = 1 + 1 = 2
>
> **输入：** `str = "(())"`
> **输出：** 2
> **说明：** 由于输入为“(x)”形式，总分 = 2 * 得分为“()” = 2 * 1 = 2

**方法：** 使用栈可以解决这个问题。这个想法是迭代字符串的字符。对于每个 `i` 字符，检查该字符是否为 `'('`。如果发现为真，则将该字符插入堆栈。否则，计算内括号的分数，并将分数的两倍插入堆栈。按照以下步骤解决问题：

*   初始化一个栈来存储当前遍历的字符或者内部平衡括号的分数。
*   迭代字符串的字符。对于每个 `i` 字符，检查以下条件：
    *   如果当前字符是 `'('`，则将当前字符推入堆栈。
    *   否则，检查堆栈的顶部元素是否为 `'('`。如果发现为真，则在代表内部平衡括号得分的堆栈中插入 `"1"`。
