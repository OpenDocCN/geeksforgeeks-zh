...
> 原文: [https://www.geeksforgeeks.org/closest-next-smaller-greater-numbers-number-set-bits/](https://www.geeksforgeeks.org/closest-next-smaller-greater-numbers-number-set-bits/)
...
观察: 给定一个数字 `n` 和两个比特位置 `i` 和 `j`，假设我们把比特 `i` 从 1 翻转到 0，比特 `j` 从 0 翻转到 1。如果 `i > j`，那么 `n` 就会减少。如果 `i < j`，那么 `n` 就会增加。
