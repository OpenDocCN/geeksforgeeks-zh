...
> 原文：[https://www.geeksforgeeks.org/find-the-indices-which-will-hold-the-array-sum-after-given-operations/](https://www.geeksforgeeks.org/find-the-indices-which-will-hold-the-array-sum-after-given-operations/)
...
*   在 `max(arr[i], arr[j])` 的索引处分配 `K`，在 `min(arr[i], arr[j])` 的索引处分配 `0`。
...
对给定的数组元素进行排序，检查直到 `i-1` 的和是否大于具有元素的直到 `i` 的和，如果是大于，则该索引有可能是有效索引。

1.  取一个 `vector<pair>`，表示 `v`，将所有元素和各自的索引存储成对，还取一个变量，表示 `sum`，将存储整个数组的和。
2.  将向量按升序排序，取一个计数器，比如说 `c`，用 `1` 初始化，因为一个索引总是有可能保存数组的和。
3.  迭代从 `倒数第二个` 元素开始的向量，检查 `的和直到倒数第二个元素` 是否大于的和直到 `的和直到最后`（即数组的和），如果大于则递增计数器，这意味着它有保持和的可能性，否则中断循环。
4.  取一个向量说 `ans` 存储有效的索引，通过从末尾到计数器的迭代来存储索引，然后对 `ans` 向量进行排序并打印出来。
