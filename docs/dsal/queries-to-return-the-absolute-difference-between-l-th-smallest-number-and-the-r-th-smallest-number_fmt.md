...
> 原文：[https://www.geeksforgeeks.org/queries-to-return-the-absolute-difference-between-l-th-smallest-number-and-the-r-th-smallest-number/](https://www.geeksforgeeks.org/queries-to-return-the-absolute-difference-between-l-th-smallest-number-and-the-r-th-smallest-number/)

## 问题描述

给定一个由 `N` 个独特元素组成的数组和 `Q` 个查询。每个查询由两个整数 `L` 和 `R` 组成。任务是打印第 `L` 小和第 `R` 小元素索引之间的绝对差值。
...
4
```
对于第一个查询，第二个最小的数字是 2，位于索引 3 处，第五个最小的数字是 6，位于索引 5 处。因此这两个索引之间的绝对差值为 2。
...
*   将元素及其索引存储在一对数组中。
*   根据第一个元素对数组进行排序。
*   对于每个查询，答案都是 `ABS(arr[l-1].second - arr[r-1].second)`。
