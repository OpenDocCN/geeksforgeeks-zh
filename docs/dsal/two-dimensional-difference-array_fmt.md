...
> 原文：[https://www.geeksforgeeks.org/two-dimensional-difference-array/](https://www.geeksforgeeks.org/two-dimensional-difference-array/)

给定维度为 `N * M` 的[矩阵](https://www.geeksforgeeks.org/matrix/)和具有形式为 `{k, r1, c1, r2, c2}` 的每个查询的 2D 数组 `Queries[][]`，任务是将 `k` 添加到子矩阵 `(r1, c1)` 到 `(r2, c2)` 中的所有单元格。

## 示例

**输入：**
`A[][] = {{1, 2, 3}, {1, 1, 0}, {4, -2, 2}}`，`Queries[][] = {{2, 0, 0, 1, 1}, {-1, 1, 0, 2, 2}}`
**输出：**
```
3 4 3
2 -1 1
...
```
**解释：**
查询 1：矩阵修改为 `{{3, 4, 3}, {3, 2, 0}, {4, -2, 2}}`。
查询 2：矩阵修改为 `{{3, 4, 3}, {2, -1, 1}, {3, -3, 1}}`。

**输入：**
`A[][] = {{1, 2, 3}, {4, 5, 6}, {7, 8, 9}}`，`Queries[][] = {{1, 1, 1, 2, 2}, {2, 0, 1, 0, 2}}`
**输出：**
```
1 4 5
4 6 7
...
思路基于 O(1) 中的[差分数组 | 范围更新查询](https://www.geeksforgeeks.org/difference-array-range-update-query-o1/)。按照以下步骤解决问题：
...
*   最后，将修改后的数组打印为：对于 `j > 0` 是 `D[i][j] + A[i][j-1]`，对于 `j = 0` 是 `D[i][j]`。
