...
> 原文：`https://www.geeksforgeeks.org/number-of-subarrays-with-positive-product/`
...
寻找负积子阵的方法已经在[这篇](https://www.geeksforgeeks.org/number-of-sub-arrays-with-negative-product/)文章中讨论过了。如果 `cntNeg` 是负积子阵列的计数，而 `total` 是给定阵列的所有可能子阵列的计数，那么正积子阵列的计数将是 `cntPos = total–cntNeg`。
