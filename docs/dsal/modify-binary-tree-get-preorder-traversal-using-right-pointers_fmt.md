...
> 原文：[https://www.geeksforgeeks.org/modify-binary-tree-get-preorder-traversal-using-right-pointers/](https://www.geeksforgeeks.org/modify-binary-tree-get-preorder-traversal-using-right-pointers/)

给定一棵二叉树。以这样一种方式修改它，在修改之后，您可以只使用右指针对它进行一次前序遍历。在修改过程中，您可以使用右指针和左指针。
...
**方法 1（递归）**
需要使根节点的右指针指向其左子树。
如果节点刚刚离开其子节点，那么只需将子节点向右移动，即可完成对该节点的处理。
如果也有一个右子树，那么它应该是原左子树最右边的节点。
