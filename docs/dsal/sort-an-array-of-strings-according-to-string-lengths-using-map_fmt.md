...
> 原文: [https://www.geeksforgeeks.org/sort-an-array-of-strings-according-to-string-lengths-using-map/](https://www.geeksforgeeks.org/sort-an-array-of-strings-according-to-string-lengths-using-map/)

给定一个字符串数组，我们需要使用[映射数据结构](https://www.geeksforgeeks.org/map-interface-java-examples/)按照字符串长度的递增顺序对数组进行排序。

## 示例:

> **输入:** `str[] = {"GeeksforGeeks", "I", "from", "am"}`
> **输出:** I am from GeeksforGeeks
>
> **输入:** `str[] = {"You", "are", "beautiful", "looking"}`
> **输出:** You are beautiful looking

## 方法:

给定的方法是使用[树形图](https://www.geeksforgeeks.org/treemap-in-java/)的 Java。

*   取一个包含字符串长度的`TreeMap`作为键，取一个与键长度相同的`ArrayList`。
*   由于我们知道一个`TreeMap`已经被排序，所以在插入到`TreeMap`之后，我们再次从`ArrayList`中一个接一个地获取值到`String`数组中。
