...
> 原文: [https://www.geeksforgeeks.org/check-if-a-matrix-is-bitonic-or-not/](https://www.geeksforgeeks.org/check-if-a-matrix-is-bitonic-or-not/)

给定一个矩阵 `m[][]`，任务是检查给定的矩阵是否为[双调的](https://www.geeksforgeeks.org/program-to-check-if-an-array-is-bitonic-or-not/)。如果给定的矩阵是`双调的`，则打印`是`。否则，打印`否`。
...
> `输出:` NO
> `解释:`
> 由于列 `{2, 5, 5}` 不满足三个条件中的任何一个，所以给定的矩阵不是双调的。

`方法:`
按照以下步骤解决问题:
*   逐个检查矩阵每行的元素，看它是否形成一个双调序列。如果发现任何一行不是双调的，打印`否`。
*   同样，逐一检查每一列的元素，看它是否形成一个双调序列。如果发现任何一列不是双调的，打印`否`。
*   如果发现所有的行和列都是`双调的`，则打印`是`。
