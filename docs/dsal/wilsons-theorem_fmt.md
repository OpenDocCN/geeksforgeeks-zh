...
考虑一个接近输入数的素数模下的阶乘计算问题，即我们要求 `n！% p` 使得 `n < p`，`p` 是素数，`n` 接近 `p` 例如 `(25！% 29)`。从威尔逊定理，我们知道 `28！` 是 `-1`。所以我们基本上需要求 `[ (-1) *逆(28，29) *逆(27，29) *逆(26) ] % 29`。反函数 `逆(x，p)` 返回模 `p` 下 `x` 的逆（详见[本](https://www.geeksforgeeks.org/multiplicative-inverse-under-modulo-m/)）。

威尔逊定理的更多应用见[本](https://en.wikipedia.org/wiki/Wilson's_theorem#Applications)。

## 参考文献

- [https://en.wikipedia.org/wiki/Wilson's_theorem](https://en.wikipedia.org/wiki/Wilson's_theorem)
- [https://primes.utm.edu/notes/proofs/Wilsons.html](https://primes.utm.edu/notes/proofs/Wilsons.html)
