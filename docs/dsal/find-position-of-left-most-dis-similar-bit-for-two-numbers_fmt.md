...
> 原文: [https://www.geeksforgeeks.org/find-position-of-left-most-dis-similar-bit-for-two-numbers/](https://www.geeksforgeeks.org/find-position-of-left-most-dis-similar-bit-for-two-numbers/)

给定两个数字 `n1` 和 `n2`。任务是从左边找到两个数的二进制表示中第一个不匹配位的位置。在使两个数字的二进制表示长度相同后，我们需要找到这个位。我们通过在较小的数字中附加 0 来使长度相同。

**注**:

*   **例如**: `n1 = 1`，`n2 = 7`。`n1` 和 `n2` 的按位表示将分别为“1”和“111”。将两个零附加到 `n1`，使其为 100。
*   如果 `n1` 等于 `n2`，则打印零。
