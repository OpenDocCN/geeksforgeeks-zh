...
对于每一列，找出最小值，然后是第二个最小值，取这两个最小值的绝对差值，然后将其写入相应的列差值中，如图所示。在`D1`栏中， `2` 为最小值， `3` 为第二小值，它们的绝对差值为 `1` 。同样，对于 `D2` 、 `D3` 和 `D4` 栏，绝对差值分别为 `2` 、 `2` 和 `2` 。

行差和列差的这些值也被称为罚分。现在选择最大惩罚。最高罚 `3` 对应排 `O2` 。现在在行 `O2` 中找到成本最小的单元格，并在相应行的供应和相应列的需求之间分配最小值。需求小于供应，因此将列的需求，即 `250` 分配给单元。然后取消 `D1` 一栏。
...
再次选择与第 `O1` 行对应的最大处罚 `3` 。第 `O1` 行成本最低的单元格为 `(D2, O1)`，成本为 `1` 。将各个行和列中的最小供应和需求分配给单元格。取消零值行或列。
...
现在选择最大处罚为 `7` 对应栏 `D4` 。 `D4` 列中成本最小的单元格是 `(D4, O3)`，成本为 `2` 。单元格 `(D4, O3)` 的需求量小于供应量。将 `200` 分配给单元格，取消列。
...
没有余额了。所以将单元格的分配值乘以它们对应的单元格成本，然后全部相加，得到最终成本。
