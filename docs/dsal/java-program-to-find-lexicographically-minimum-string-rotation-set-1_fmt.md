...
> 原文: [https://www.geeksforgeeks.org/java-program-to-find-lexicographically-minimum-string-rotation-set-1/](https://www.geeksforgeeks.org/java-program-to-find-lexicographically-minimum-string-rotation-set-1/)
...
3. 通过在索引 0、1、2...n-1 处获取 `concat` 的子串，找到 `str` 的所有旋转。将这些旋转存储在 `arr[]` 中。
4. 排序 `arr[]` 并返回 `arr[0]`。
...
// This function return lexicographically 
...
在假设我们使用了一个 O(n<sup>2</sup>Logn) 排序算法的情况下，上述解的时间复杂度为 O(n<sup>2</sup>Logn)。
