...
> 原文: [https://www.geeksforgeeks.org/the-lottery-ticket-hypothesis/](https://www.geeksforgeeks.org/the-lottery-ticket-hypothesis/)
...
*   需要一个`启发式`来定义一个结构是否多余。这些试探法是`幅度`、`梯度`或`激活`。这里我们选择了`星等`。我们用最低的幅度修剪权重。
...
*   像`MNIST`这样具有超过`600K 个参数`的全连接神经网络被认为被简化为具有与原始网络相同精度的`21K 个参数`的`子网`
