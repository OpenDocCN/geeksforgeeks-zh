...
> 原文: [https://www.geeksforgeeks.org/find-number-of-subarrays-ending-with-arri-where-arri-is-the-minimum-element-of-that-subarray/](https://www.geeksforgeeks.org/find-number-of-subarrays-ending-with-arri-where-arri-is-the-minimum-element-of-that-subarray/)
...
想法是通过维护[栈](https://www.geeksforgeeks.org/stack-data-structure/)来使用用于找到[下一个更大元素](https://www.geeksforgeeks.org/next-greater-element/)的方法。逐步解决问题的方法是：

*   将数组的第一个元素 (`arr[0]`) 推入栈中，计数为 `1`，因为第一个元素将是子数组本身，以当前元素 `arr[0]` 和子数组的最小值结束。
*   那么对于数组中的每个元素 `arr[i]`
    1.  从栈中弹出元素，直到栈顶大于当前元素，并将弹出元素的计数添加到当前元素的计数中。
    2.  将当前元素和计数作为一对推入栈中。
