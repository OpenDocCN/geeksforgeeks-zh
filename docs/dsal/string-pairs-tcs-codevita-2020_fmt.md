...
给定一个由 `N` (`1 ≤ N ≤ 100`)整数组成的数组`arr[]`，任务是将每个数组元素(`1 ≤ arr[i] ≤ 100`)转换为它们各自的文本表示，并打印数组中所有可能对的计数的小写表示，这些对的总和等于它们的文本表示中存在的元音总数。如果计数超过 100，打印“大于 100”。
