...
> 原文: [https://www.geeksforgeeks.org/check-if-a-line-at-45-degree-can-divide-the-plane-into-two-equal-weight-parts/](https://www.geeksforgeeks.org/check-if-a-line-at-45-degree-can-divide-the-plane-into-two-equal-weight-parts/)

给定一组 `n` 个点 `(x_i, y_i)` 在 2D 坐标。每个点都有一些权重 `w_i`。任务是检查是否可以画一条 45 度的线，使每一边的点的权重之和相等。
...
现在，遍历从最小 `x` 坐标到最大 `x` 坐标的所有 `x` 坐标。因此，制作一个数组 `prefix_sum[]`，它将存储权重之和直到点 `x = i`。

所以，可以有两个选项，答案可以是“是”:

*   `prefix_sum[1, 2, …, i-1] = prefix_sum[i+1, …, n]`
*   或者存在一个点 `i`，使得一条线在 `x = i` 和 `x = i+1` 之间通过，且 `prefix_sum[1, …, i] = prefix_sum[i+1, …, n]`，其中 `prefix_sum[i, …, j]` 是从 `i` 到 `j` 的点的权重之和。
...
我们可以忽略 `sqrt(2)`，因为它是比例因子。另外，我们不需要关心旋转后的 `y` 坐标，因为垂直线无法区分具有相同 `x` 坐标的点。`(x, y1)` 和 `(x, y2)` 将位于直线 `x = k` 的右侧、左侧或直线上。
