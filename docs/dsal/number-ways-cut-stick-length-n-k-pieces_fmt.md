...
> 原文：[https://www.geeksforgeeks.org/number-ways-cut-stick-length-n-k-pieces/](https://www.geeksforgeeks.org/number-ways-cut-stick-length-n-k-pieces/)
...
解这道题相当于解数学方程 `x1 + x2 + …..+ xK = N`。
我们可以用组合学中的“条和星”方法来求解，从中我们得到这个方程的正积分解个数为 `(N–1)C(K–1)`，其中 `N C K` 为 `N!/((N–K)! * (K!))`，这里 `!` 代表阶乘。
在 `C++` 和 `Java` 中，对于阶乘的大值，可能会出现溢出错误。在这种情况下，我们可以引入一个大质数如 `10^7 + 7` 来修改答案。我们可以利用[卢卡斯定理](https://www.geeksforgeeks.org/compute-ncr-p-set-2-lucas-theorem/)计算 `nCr % p`。
...
## 练习
用允许的 0 个长度片扩展上述问题。提示:同样可以通过将每个 `x_i` 写成 `y_I–1` 来找到解的个数，我们得到一个等式 `y_1 + y_2 + …..+ y_K = N + K`。该方程解的个数为 `(N+K–1)C_(K–1)`。
