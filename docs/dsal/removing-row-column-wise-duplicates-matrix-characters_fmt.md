...
> 原文: [https://www.geeksforgeeks.org/removing-row-column-wise-duplicates-matrix-characters/](https://www.geeksforgeeks.org/removing-row-column-wise-duplicates-matrix-characters/)
...
遍历整个矩阵，对于每个元素，检查其对应的行和列。如果有重复项，则将另一个矩阵中的特定单元格标记为 `true`。最后，将另一个矩阵中有假值的所有字符连接起来形成字符串。
