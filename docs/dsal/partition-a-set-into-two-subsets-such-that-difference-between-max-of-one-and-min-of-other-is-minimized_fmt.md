...
> 原文: [https://www.geeksforgeeks.org/partition-a-set-into-two-subsets-such-that-difference-between-max-of-one-and-min-of-other-is-minimized/](https://www.geeksforgeeks.org/partition-a-set-into-two-subsets-such-that-difference-between-max-of-one-and-min-of-other-is-minimized/)
...
为了解决上述问题，我们必须找到两个整数 `m` 和 `n`，使得第一组的最大值为 `m`，第二组的最小值为 `n`。其思想是[按照给定的数组升序](https://www.geeksforgeeks.org/c-program-to-sort-an-array-in-ascending-order/)进行排序，在对数组进行排序后，连续元素之间的最小差就是将数组元素划分为子集后所需的最小差。
