...
> 原文: [https://www.geeksforgeeks.org/rearrange-the-given-string-such-that-all-prime-multiple-indexes-have-same-character/](https://www.geeksforgeeks.org/rearrange-the-given-string-such-that-all-prime-multiple-indexes-have-same-character/)

给定一个大小为 `N` 的字符串 `str`。任务是找出是否可能重新排列字符串中的字符，以便对于任何质数 `p <= N` 和范围从 `1` 到 `N/p` 的任何整数 `i`，必须满足条件 `str[p] = str[p*i]`。如果不可能进行任何这样的重新排列，则打印 `-1`。
...
*   剩余位置可以有任何符号。该位置保持使用[筛法](https://www.geeksforgeeks.org/sieve-eratosthenes-0n-time-complexity/)。
