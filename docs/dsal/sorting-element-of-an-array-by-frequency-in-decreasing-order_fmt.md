...
> 原文: [https://www.geeksforgeeks.org/sorting-element-of-an-array-by-frequency-in-decreasing-order/](https://www.geeksforgeeks.org/sorting-element-of-an-array-by-frequency-in-decreasing-order/)

给定一个由 `N` 个整数组成的数组 `arr[]`。任务是将数组 `arr[]` 按照元素出现的频率降序排序。
...
*   将所有元素的频率存储在数组 `freq[]` 中。
*   对于 `arr[] = { 4, 4, 5, 6, 4, 2, 2, 8, 5 }`
    上述数组的频率数组为:
    `freq[] = { 0, 0, 2, 0, 3, 2, 0, 1, 0, 1}`
*   遍历频率数组，对于频率大于 `1` 的所有元素，更新数组 `arr[]` 中的值如下:
    `freq[2] = 2`
    `arr[0] = 100000 * freq[2] + (100000 - 2) = 299998`
    `freq[4] = 3`
    `arr[1] = 100000 * freq[4] + (100000 - 4) = 399996`
    `freq[5] = 2`
    `arr[2] = 100000 * freq[5] + (100000 - 5) = 299995`
*   按降序排列数组 `arr[]`。
*   遍历数组 `arr[]` 并根据步骤 2 中数组元素的更新获得该元素的元素和频率，执行以下操作:
    得到当前元素的频率:
    ...
    元素 `4` 具有频率 `3`。
*   对于 `arr[]` 中的每个元素，在每个索引处找到 `值` 和频率(比如说 `f`)，并打印出数值 `value` `f` 次。
