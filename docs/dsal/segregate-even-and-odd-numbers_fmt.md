...
> 原文: [https://www.geeksforgeeks.org/segregate-even-and-odd-numbers/](https://www.geeksforgeeks.org/segregate-even-and-odd-numbers/)
...
这个问题和我们以前的帖子很像，把 0 和 1 分隔成一个数组，这两个问题都是著名的荷兰国旗问题的变种。

## 算法: `segregateEvenOdd()`

1.  初始化两个索引变量 `left` 和 `right`:
    `left = 0`, `right = size -1`
2.  持续递增 `left` 索引直到我们看到一个偶数。
3.  持续递减 `right` 索引直到我们看到一个奇数。
4.  如果 `left < right` 则交换 `arr[left]` 和 `arr[right]`
...
1.  使用两个指针 `i` 和 `j`，`i` 将指向索引 `0`，`j` 将指向最后一个索引。
