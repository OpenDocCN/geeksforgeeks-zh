...
在这种情况下，按顺序遍历列表或数组，并检查每个元素。例如：[线性搜索](https://www.geeksforgeeks.org/linear-search/)。

## 区间搜索

这些算法是专门为在有序数据结构中搜索而设计的。这些类型的搜索算法比线性搜索高效得多，因为它们反复瞄准搜索结构的中心并将搜索空间分成两半。例如：[二分搜索法](https://www.geeksforgeeks.org/binary-search/)。

### 线性搜索

思路是遍历给定数组`arr[]`找到元素所在的索引。以下是步骤：
