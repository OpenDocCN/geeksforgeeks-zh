...
> 原文：[https://www.geeksforgeeks.org/rearrange-all-elements-of-array-which-are-multiples-of-x-in-increasing-order/](https://www.geeksforgeeks.org/rearrange-all-elements-of-array-which-are-multiples-of-x-in-increasing-order/)

给定一个整数数组`arr`和一个数字`x`，任务是按照相对位置升序对数组中`x`的倍数的所有元素进行排序，即其他元素的位置不得受到影响。
