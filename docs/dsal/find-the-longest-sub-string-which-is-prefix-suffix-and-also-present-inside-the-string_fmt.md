...
> 原文：[https://www.geeksforgeeks.org/find-the-longest-sub-string-which-is-prefix-suffix-and-also-present-inside-the-string/](https://www.geeksforgeeks.org/find-the-longest-sub-string-which-is-prefix-suffix-and-also-present-inside-the-string/)

给定字符串 `str`。任务是找到最长的子串，它是给定串的前缀、后缀和子串。如果不存在这样的字符串，则打印 `-1`。

**举例：**

> **输入：** `str = "fixprefixsuffix"`
> **输出：** `fix`
...
让我们计算字符串所有前缀的最长前缀后缀。最长前缀后缀 `lps[i]` 是前缀的最大长度，也是子串 `str[0…i]` 的后缀。关于最长前缀后缀的更多信息，可以在 [KMP 算法](https://www.geeksforgeeks.org/kmp-algorithm-for-pattern-searching/)的描述中看到。
