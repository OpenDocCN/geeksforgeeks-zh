...
> 原文: [https://www.geeksforgeeks.org/count-number-of-pairs-with-the-given-comparator/](https://www.geeksforgeeks.org/count-number-of-pairs-with-the-given-comparator/)
...
迭代每对元素，使得 `i < j` 并检查每对自定义比较器是否为真。如果是，则增加计数。
**时间复杂度:** `O(N^2)`

## 高效方法

想法是定制[合并排序](https://www.geeksforgeeks.org/merge-sort/)，在合并两个子数组时计算这样的对。每个数组都有两种类型的计数，即：

*   **数组间对:** 存在于左子数组本身的对。
*   **数组内对:** 出现在右子数组中的对。

对于左子数组，计数可以从下到上递归计算，而主要任务是计数数组内对。
...
下面是从左子数组到右子数组的数组内对的图示：

### 基本情况

这个问题的基本情况是两个子数组中只有一个元素，我们想要检查数组内对。然后，检查这两个元素是否形成一个这样的对，然后增加计数，并将较小的元素放在其位置上。
...
如果大于或等于，如果我们找到任何这样的对，那么该子数组右边的所有元素也将与当前元素形成对。因此这种对的计数增加左子数组中剩余的元素数量。
...
如果小于或等于，如果我们找到任何这样的对，那么该子数组右侧的所有元素也将与当前元素形成对。因此这种对的计数增加了右子数组中剩余的元素数量。
...
在情况等于的情况下，如果我们找到任何这样的对，那么我们试图借助于 [while 循环](https://www.geeksforgeeks.org/loops-in-c-and-cpp/)在左子数组中找到所有这样的对。在每个这样的可能对中，将计数增加 1。
...
最后，合并两个子数组，就像在[合并排序](https://www.geeksforgeeks.org/merge-sort/)中一样。
