...
## 方法
假设采用从源到中间的路径 P1，以及从中间到目标的路径 P2。 这两个路径之间可能存在一些共同的边。 因此，最佳路径将始终具有以下形式：对于任何节点 U，步行都包括从源到 U，从中间到 U，从目的地到 U 的最短路径上的边。因此，如果 `dist(a, b)` 是节点 a 和节点 b 之间的最短路径的成本，所需的最小成本路径为 `min {dist(Source, U) + dist(intermediate, U) + dist(destination, U)}`。通过对这 3 个节点执行 [`Dijkstra最短路径算法`](https://www.geeksforgeeks.org/greedy-algorithms-set-6-dijkstras-shortest-path-algorithm/)，可以找到所有节点距源，中间和目标的最小距离。
