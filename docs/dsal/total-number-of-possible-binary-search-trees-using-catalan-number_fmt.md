# 使用加泰罗尼亚数计算可能的二叉搜索树总数

> 原文：[https://www.geeksforgeeks.org/total-number-of-possible-binary-search-trees-using-catalan-number/](https://www.geeksforgeeks.org/total-number-of-possible-binary-search-trees-using-catalan-number/)

给定一个整数 `N`，任务是用 `N` 个键统计可能的[二叉搜索树](https://www.geeksforgeeks.org/binary-search-tree-data-structure/)的数量。
...
**方法：** 用 `N` 个键将要形成的二叉搜索树的数量可以通过简单地计算[加泰罗尼亚数](https://www.geeksforgeeks.org/program-nth-catalan-number/)系列中相应的数字来计算。
前几个加泰罗尼亚数为 `n = 0, 1, 2, 3, …` 是 `1, 1, 2, 5, 14, 42, 132, 429, 1430, 4862, …`
加泰罗尼亚数满足以下递归公式：
...
在计算方面，我们需要从较低的数字开始，因为 `G(n)` 的值取决于 `G(0) ... G(n-1)` 的值。
