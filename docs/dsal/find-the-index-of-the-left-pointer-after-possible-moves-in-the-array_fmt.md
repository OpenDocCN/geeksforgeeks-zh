...
> 原文：[https://www.geeksforgeeks.org/find-the-index-of-the-left-pointer-after-possible-moves-in-the-array/](https://www.geeksforgeeks.org/find-the-index-of-the-left-pointer-after-possible-moves-in-the-array/)

给定一个大小为`N`的数组。移动两个指针，一个从数组的左侧移动，一个从数组的右侧移动。一个指针只有在它已经通过的所有数字的总和小于另一个指针已经通过的数字的总和时才会向前移动。当左指针小于右指针或不能移动时，继续该过程。打印最后左边指针的位置。
