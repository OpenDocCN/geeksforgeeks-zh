...
> 原文：[https://www.geeksforgeeks.org/optimal-partition-of-an-array-into-four-parts/](https://www.geeksforgeeks.org/optimal-partition-of-an-array-into-four-parts/)

给定 `n` 个非负整数的数组。从数组中选择三个索引，即 `(0 <= index_1 <= index_2 <= index_3 <= n)`，组成四个子集，使得表达式 `sum(0, index_1) - sum(index_1, index_2) + sum(index_2, index_3) - sum(index_3, n)` 最大可能。

这里，两个索引 `l` 和 `r` 的意思是，`sum(l, r)` 将是从 `l` 到 `r` 不包含的位置上的子集的所有数字的和（第 `l` 个元素不计算，第 `r` 个元素计算）。例如，如果 `arr = {-5, 3, 9, 4}`，那么从 `0` 到 `4` 的每个 `i` 的 `sum(0, 1) = -5`，`sum(0, 2) = -2`，`sum(1, 4) = 16` 和 `sum(i, i) = 0`。对于索引 `l` 和 `r` 保持 `0 <= l <= r <= n`。数组中的索引从 `0` 开始编号。
