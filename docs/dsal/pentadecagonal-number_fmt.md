...
给定一个数 `N`，任务是找到第 `N` 个五边形数。

> 五边形数是将三角形和正方形的概念扩展到五边形（一个 15 边的多边形）的图形数。第 `N` 个五边形数计算 `N` 个嵌套五边形图案中的点数，所有五边形共享一个公共角，其中图案中的第 `i` 个三边形具有由相互间隔一个单位的“`I`”个点组成的边。前几个十边形数字是 **1、15、42、82、135、201、280……**
