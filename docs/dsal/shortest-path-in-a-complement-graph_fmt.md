...
给定一个`无向非加权图 G`。对于给定的节点`开始`，返回最短路径，即从`开始`到`G`的补图中所有节点的边数。
...
一个简单的解决方案是创建补码图，并在该图上使用[广度优先搜索](https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/)来找到到所有节点的距离。
**时间复杂度:** `O(n^2)` 用于创建补图，`O(n + m)` 用于广度优先搜索。

## 高效途径

思路是用**修正广度优先搜索**计算答案，然后就不需要构造补图了。
