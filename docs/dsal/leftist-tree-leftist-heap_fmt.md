...
左树或左堆是用二进制堆的变体实现的优先级队列。每个节点都有一个 `s值(或等级或距离)`，它是到最近的叶子的距离。与二进制堆(总是一个[完整的二叉树](https://www.geeksforgeeks.org/binary-tree-set-3-types-of-binary-tree/))相比，左树可能非常不平衡。
...
左树是一棵二叉树，它具有以下特性:
1.  **正常最小堆属性:** `键(i) >= 键(父(I))`
2.  **Heavier on left side :** `dist(right(i)) <= dist(left(i))`。这里，`dist(i)` 是在扩展二叉树表示中（在此表示中，null子节点被视为外部或节点），从节点 `i` 到叶子的最短路径上的边数。每个子树也是左倾的，并且 `dist( ) = 1 + dist( right( i ) )`。

**示例:** 下面的左树显示了通过上述过程为每个节点计算的距离。最右边的节点的等级为 0，因为该节点的右子树为空，其父节点的距离为 `1 + dist( right( i ) )`。每个节点都遵循相同的方法，并计算它们的 `s值(或等级)`。
...
2.  如果最右边的叶的路径有 `x` 个节点，那么左边的堆至少有 `2^x – 1` 个节点。这意味着对于具有 `n` 个节点的左边堆，到最右边叶的路径长度是 `O(log n)`。
...
4.  从递归返回之前:
    – 更新合并根的 `dist()`。
    – 如果需要，交换根下的左右子树，以保持合并后的结果的左属性。

来源: [http://courses.cs.washington.edu/courses/cse326/08sp/lectures/05-leftist-heaps.pdf](http://courses.cs.washington.edu/courses/cse326/08sp/lectures/05-leftist-heaps.pdf)
