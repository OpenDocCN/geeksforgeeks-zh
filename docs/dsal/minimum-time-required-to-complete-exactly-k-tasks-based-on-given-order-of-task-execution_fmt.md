...
> 原文: [https://www.geeksforgeeks.org/minimum-time-required-to-complete-exactly-k-tasks-based-on-given-order-of-task-execution/](https://www.geeksforgeeks.org/minimum-time-required-to-complete-exactly-k-tasks-based-on-given-order-of-task-execution/)

给定一个大小为 `N` 的 `2D` 数组 `arr[][]`，每一行由完成 `3` 种不同类型任务所需的时间组成：`A`、`B` 和 `C`。行元素 `arr[i][0]`、`arr[i][1]` 和 `arr[i][2]` 分别是完成类型为 `A`、`B` 和 `C` 的第 `i` 个任务所需的时间。任务是根据以下条件从数组中找到准确完成 `K` 个任务的最短完成时间：

*   同一类型的任务将同时运行。
*   只有当 `K` 个类型为 `A` 的任务已经完成时，才能执行类型为 `B` 的任务。
*   只有当 `K` 个类型为 `B` 的任务已经完成时，才能执行类型为 `C` 的任务。
...
**输出:** `7`
**说明:**
完成 `A` 类 `K` 个任务所需的最短时间 = `min(max(arr[0][0], arr[2][0]), max(arr[0][0], arr[1][0]), max(arr[1][0], arr[2][0]))` = `3`
因此，完成 `B` 类 `K` 个任务的时间 = `max(arr[0][1], arr[2][1])` = `2`
因此，完成 `C` 类 `K` 个任务的时间 = `max(arr[0][2], arr[2][2])` = `2`
因此，完成所有类型 `K(= 2)` 个任务的最短时间 = `3 + 2 + 2 = 7`
...
1.  初始化两个变量，比如 `X` 和 `Y`，同时使用上述递归关系递归遍历每一行。
2.  `X` 通过选择给定数组的第 `i` 行来存储最小完成时间。
3.  `Y` 通过不选择给定数组的第 `i` 行来存储最小完成时间。
...
// completion time to select
// exactly K items
function MinTime(arr, i, Ta, Tb, Tc, K)
{
    ...
    // Select the ith item
    let X = MinTime(arr, i - 1,
                    Math.max(Ta, arr[i - 1][0]),
                    Math.max(Tb, arr[i - 1][1]),
                    Math.max(Tc, arr[i - 1][2]), K - 1);

    // Do not select the ith item
    let Y = MinTime(arr, i - 1, Ta,
                    Tb, Tc, K);
...
document.write(MinTime(arr, n, 0, 0, 0, K));
