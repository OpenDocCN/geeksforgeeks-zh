...
> 原文: [https://www.geeksforgeeks.org/smallest-index-in-given-range-of-indices-which-is-not-equal-to-x/](https://www.geeksforgeeks.org/smallest-index-in-given-range-of-indices-which-is-not-equal-to-x/)
...
从 2 到 3 的所有元素都等于 1，因此答案是 -1。

**输入:**
`arr[] = { 1, 2, 3, 2, 1, 1, 3, 5 }`, `query[][] = { { 1, 4, 2 }, { 4, 5, 1 }, { 2, 3, 1 }, { 4, 6, 1 } }`
**输出:**
```
2
-1
...
3.  否则意味着 `arr[L] = X`。在这种情况下，我们需要找到下一个与 `arr[L]` 值不同的指数。这可以从 `nextpos[L]` 获得。
4.  如果 `nextpos[L]` 小于等于 `R`，则打印 `nextpos[L]`。
