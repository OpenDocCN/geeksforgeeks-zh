...
> 原文: [https://www.geeksforgeeks.org/smallest-number-to-make-array-sum-at-most-k-by-dividing-each-element/](https://www.geeksforgeeks.org/smallest-number-to-make-array-sum-at-most-k-by-dividing-each-element/)
...
> **输出:** `4`
> **解释:**
> 当每个元素除以 4: `2/4 + 3/4 + 4/4 + 9/4 = 1 + 1 + 1 + 3 = 6`
> 当每个元素除以 3: `2/3 + 3/3 + 4/3 + 9/3 = 1 + 1 + 2 + 3 = 7` 大于 `K`。
>
> **输入:** `arr[] = {5, 6, 7, 8}`, `K = 4`
> **输出:** `8`
...
1.  输入数组。
2.  假设最大可能答案为 `10^9`，将最大值初始化为 `10^9`，最小值初始化为 `1`。
...
**时间复杂度:** `O(N * 30)`，其中 `N` 是数组的大小，因为找到 `1` 到 `10^9` 之间的任意数字在二分搜索法最多需要 30 次运算。
**辅助空间:** `O(1)`