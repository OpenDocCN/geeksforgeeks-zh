...
Q1。给定一个在线电影票预订系统，他想知道我们如何处理对同一座位的同时请求，以便它只分配给一个用户。如何使用`MySQL`完成，然后如何使用`REDIS`。问了一些`mysql`中的`select`和`update`查询。
...
[Sub-Array sum divisible by K](https://practice.geeksforgeeks.org/problems/sub-array-sum-divisible-by-k/0)
