...
> 原文：[https://www.geeksforgeeks.org/print-the-two-possible-permutations-from-a-given-sequence/](https://www.geeksforgeeks.org/print-the-two-possible-permutations-from-a-given-sequence/)

给定一个包含 `N` 个正整数的数组 `arr`，任务是检查给定的数组是否可以分解成两个排列，如果可能的话打印排列。如果从 `1` 到 `M` 的所有整数恰好只包含一次，则一个 `M` 个整数序列被称为置换。
