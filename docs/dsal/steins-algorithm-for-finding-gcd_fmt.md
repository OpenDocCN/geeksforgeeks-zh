...
> 原文: [https://www.geeksforgeeks.org/steins-algorithm-for-finding-gcd/](https://www.geeksforgeeks.org/steins-algorithm-for-finding-gcd/)
...
4.  如果 `a` 为偶数，`b` 为奇数，则 `gcd(a, b) = gcd(a/2, b)`。同样，如果 `a` 是奇数，`b` 是偶数，那么 `gcd(a, b) = gcd(a, b/2)`。因为 2 不是公约数。
5.  如果 `a` 和 `b` 都是奇数，那么 `gcd(a, b) = gcd(|a-b|/2, b)`。注意两个奇数的差是偶数。
6.  重复步骤 3–5，直到 `a = b`，或者直到 `a = 0`。在任一种情况下，GCD 都是 `pow(2, k) * b`，其中 `pow(2, k)` 是 2 的幂，`k` 是在步骤 2 中找到的 2 的公共因子的数量。
...
*   [http://www.cse.unt.edu/~tarau/teaching/PP/NumberTheoretical/GCD/Binary%20GCD%20algorithm.pdf](http://www.cse.unt.edu/~tarau/teaching/PP/NumberTheoretical/GCD/Binary%20GCD%20algorithm.pdf)
