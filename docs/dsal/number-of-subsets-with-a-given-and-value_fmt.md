...
> 原文: [https://www.geeksforgeeks.org/number-of-subsets-with-a-given-and-value/](https://www.geeksforgeeks.org/number-of-subsets-with-a-given-and-value/)

给定一个长度为 `N` 的数组和一个整数 `X`，任务是找出“与”值为 `X` 的子集的数量。
...
递归关系是合理的，因为只有路径。要么取当前元素，用 `curr_and` AND，要么忽略，继续前进。
