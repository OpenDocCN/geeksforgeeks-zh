...
> 原文：[https://www.geeksforgeeks.org/check-if-the-elements-of-stack-are-pairwise-sorted/](https://www.geeksforgeeks.org/check-if-the-elements-of-stack-are-pairwise-sorted/)

给定一个整数堆栈，编写一个检查堆栈中的数字是否成对排序的函数 `pairWiseSorted()`。对必须是递增的，如果栈有奇数个元素，则顶部的元素从一对中被遗漏。该函数应保留原始堆栈内容。

> 堆栈上只允许以下标准操作。
> 1.  `push(X)`: 在栈顶输入一个元素 `X`。
> 2.  `pop()`: 移除堆栈顶部元素。
> 3.  `empty()`: 检查堆栈是否为空。
