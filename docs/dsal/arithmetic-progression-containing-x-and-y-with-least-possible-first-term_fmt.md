...
> 原文: [https://www.geeksforgeeks.org/arithmetic-progression-containing-x-and-y-with-least-possible-first-term/](https://www.geeksforgeeks.org/arithmetic-progression-containing-x-and-y-with-least-possible-first-term/)
...
最简单的方法是迭代从 `1` 到 `abs(X-Y)` 的所有可能公共差的值，并检查是否存在第一项大于 `0` 且包含 `X` 和 `Y` 的 `N` 长度 AP。
...
该方法基于这样的思想，即要将 `X` 和 `Y` 都包括在系列中，AP 的共同差异必须是 `abs(X-Y)` 的一个因素。以下是解决问题的步骤:
