...
> 原文: [https://www.geeksforgeeks.org/javascript-program-to-find-if-there-is-a-subarray-with-0-sum/](https://www.geeksforgeeks.org/javascript-program-to-find-if-there-is-a-subarray-with-0-sum/)
...
一个**简单的解决方案**就是逐个考虑所有子阵，检查每个子阵的和。我们可以运行两个循环: 外部循环选择一个起点 `i`，内部循环尝试从 `i` 开始的所有子阵列(实现参见[本](https://www.geeksforgeeks.org/find-subarray-with-given-sum/))。该方法的时间复杂度为 `O(n^2)`。
