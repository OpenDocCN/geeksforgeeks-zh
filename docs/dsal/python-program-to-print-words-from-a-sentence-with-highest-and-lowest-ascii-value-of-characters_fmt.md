...
> 原文：[https://www.geeksforgeeks.org/python-program-to-print-words-from-a-sentence-with-highest-and-lowest-ascii-value-of-characters/](https://www.geeksforgeeks.org/python-program-to-print-words-from-a-sentence-with-highest-and-lowest-ascii-value-of-characters/)
...
**输入:** `S = "every moment is fresh beginning"`
**输出:**
ASCII 平均值最小的单词为“beginning”。
ASCII 平均值最大的单词为“every”。
**说明:**
单词“every”的字符 ASCII 值平均值=((101+118+101+114+121)/5 = 111)
单词“moment”的字符 ASCII 值平均值=((109+111+109+101+110+116)/6 = 109.33333)
ASCII 值平均值
因此，ASCII 值平均值最小的单词为“beginning”，ASCII 值平均值最大的单词为“every”。

**输入:** `S = "sky blue"`
**输出:**
ASCII 平均值最小的单词为“blue”。
ASCII 平均值最大的单词为“sky”。

## 方法

思路是使用 [`split()`](https://www.geeksforgeeks.org/python-string-split/) 功能。按照以下步骤解决问题：

*   使用 [`split()`](https://www.geeksforgeeks.org/python-string-split/) 函数拆分字符串中由空格分隔的所有单词。将其存储在[列表](https://www.geeksforgeeks.org/python-list/)中，比如 `lis[]`。
*   初始化四个变量 `maxi`、`mini`、`maxId` 和 `minId`，分别存储 ASCII 值平均值的最大值、ASCII 值平均值的最小值、列表 `lis[]` 中 ASCII 值平均值最大的单词的索引、列表 `lis[]` 中 ASCII 值平均值最小的单词的索引。
*   定义一个函数，比如 `averageValue()`，来[找到一个字符串的平均 ASCII 值](https://www.geeksforgeeks.org/average-of-ascii-values-of-characters-of-a-given-string/)。
*   [遍历列表](https://www.geeksforgeeks.org/iterate-over-a-list-in-python/) `lis[]` 并执行以下操作：
    *   对于列表 `lis[]` 中的每个第 `i` 个单词，并将其存储在变量中，比如 `curr`。
    *   如果 `curr > maxi`，则将 `maxi` 更新为 `maxi = curr`，并分配 `maxId = i`。
