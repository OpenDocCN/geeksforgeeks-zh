...
> 原文: [https://www.geeksforgeeks.org/modulo-power-for-large-numbers-represented-as-strings/](https://www.geeksforgeeks.org/modulo-power-for-large-numbers-represented-as-strings/)

给定两个表示为字符串的数字 `sa` 和 `sb`，找到 `a^b % MOD`，其中 `MOD` 为 `1e9 + 7`。数字 `a` 和 `b` 最多可包含 `10^6` 个数字。
...
**减少 b:**
如何减少 `b`，我们已经在 [Find (a^b)%m where 'b' is very large](https://www.geeksforgeeks.org/find-abm-where-b-is-very-large/) 中讨论过了。
现在终于我们既有 `a` 又有 `b` 都在 `1 < a, b <= 10^9+7` 的范围内。因此，我们现在可以使用模幂运算来计算所需的答案。
