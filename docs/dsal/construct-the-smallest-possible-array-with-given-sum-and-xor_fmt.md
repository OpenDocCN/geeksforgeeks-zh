...
> 原文：[https://www.geeksforgeeks.org/construct-the-smallest-possible-array-with-given-sum-and-xor/](https://www.geeksforgeeks.org/construct-the-smallest-possible-array-with-given-sum-and-xor/)

给定两个正整数 `S` 和 `X`，表示数组所有元素的和与[按位异或](https://www.geeksforgeeks.org/bitwise-operators-in-c-cpp/) `arr[]`。任务是找到数组的元素 `arr[]`。如果无法生成这样的数组，请打印-1。
