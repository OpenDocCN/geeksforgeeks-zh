...
> 原文: [https://www.geeksforgeeks.org/check-if-a-subarray-of-size-k-exists-whose-elements-form-a-number-divisible-by-3/](https://www.geeksforgeeks.org/check-if-a-subarray-of-size-k-exists-whose-elements-form-a-number-divisible-by-3/)

给定一个大小为 `N` 的数组 `arr[]` 和一个正整数 `K`，任务是找到一个大小为 `K` 的子数组，其元素可用于生成一个可被 3 整除的数。如果不存在这样的子阵列，则打印 `-1`。
...
**天真方法:** 最简单的方法是从给定的阵列中生成所有可能的子阵列大小 `K`，对于每个子阵列，检查该子阵列形成的数是否能被 `3` 整除。
...
1.  将数组前 `K` 个元素的和存储在一个变量中，比如 `sum`。
2.  遍历数组的剩余元素。
3.  使用滑动窗口技术，从 `sum` 中减去子阵列的第一个元素，并将下一个阵列元素添加到子阵列中。
4.  每一步，检查 `sum` 是否能被 3 整除。
