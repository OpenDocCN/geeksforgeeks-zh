...
> 原文: [https://www.geeksforgeeks.org/replace-in-a-string-such-that-no-two-adjacent-characters-are-same/](https://www.geeksforgeeks.org/replace-in-a-string-such-that-no-two-adjacent-characters-are-same/)

给定一根长度为 `N` 的绳子 `S`，由小写字母和`"?"`组成，任务是替换`"?"`用小写字母，这样没有相邻的字符是相同的。如果存在多个可能的组合，请打印其中任何一个。
...
> 由于`"baba"`中没有相邻的字符相同，所以打印字符串作为答案。
...
最简单的方法是尝试[生成由小写字母组成的给定字符串](https://www.geeksforgeeks.org/write-a-c-program-to-print-all-permutations-of-a-given-string/)的所有可能排列。可以有 `26^N` 种弦。在每个字符串中，检查相邻字符是否匹配，给定字符串中的所有小写字符是否匹配所选的字符串排列。

**时间复杂度:** `O(N * 26^N)`，其中 `N` 为给定字符串的长度。
**辅助空间:** `O(N)`

## 高效途径

优化上述途径，思路是替换每一个`"?"`通过字符`'a'`检查该字符是否等于相邻字符。如果它等于相邻字符，则递增当前字符。以下是步骤:
