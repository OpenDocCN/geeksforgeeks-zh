# 通过将范围[a，b]和[b，c]中的任意两个数字相加，获得范围[1，b+c]中每个数字的方法数

> 原文：[https://www.geeksforgeeks.org/number-of-ways-to-obtain-each-numbers-in-range-1-bc-by-adding-any-two-numbers-in-range-a-b-and-b-c/](https://www.geeksforgeeks.org/number-of-ways-to-obtain-each-numbers-in-range-1-bc-by-adding-any-two-numbers-in-range-a-b-and-b-c/)
...
*   我们将用零初始化大小为 `b + c + 1` 的数组 `arr`。现在在循环中，我们将在 `i+j` 处增加索引，即 `(arr[i+j]++)`。
*   我们将在最后简单地打印数组。
...
思路是用[前缀和](https://www.geeksforgeeks.org/prefix-sum-array-implementation-applications-competitive-programming/)逻辑来解决这个问题。
