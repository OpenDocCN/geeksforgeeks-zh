...
请先在 IDE 上尝试您的方法，然后再进入解决方案。

一种**天真的方法**是遵循递归来寻找数字，并使用递归来求解。以下是上述办法的实施情况。
...
一个**更好的解决方案**是使用[动态规划](https://www.geeksforgeeks.org/dynamic-programming/)(记忆化)，因为有多个重叠。下面给出的是 `N=10` 的递归树。
...
在上面的部分递归树中，`rec(8)`、`rec(7)`、`rec(6)`已经被求解了两次。在绘制完整的递归树时，已经观察到有许多子问题被反复解决。因此，该问题具有重叠子结构性质，用记忆法或制表法都可以避免同一子问题的重新计算。以下是上述办法的实施情况。
