...
> 原文：[https://www.geeksforgeeks.org/check-whether-product-of-integers-from-a-to-b-is-positive-negative-or-zero/](https://www.geeksforgeeks.org/check-whether-product-of-integers-from-a-to-b-is-positive-negative-or-zero/)

给定两个整数 `a` 和 `b`，任务是检查来自范围 `[a, b]` 即 `a * (a+1) * (a+2) * …… * b` 的整数乘积是正、负还是零。
...
我们可以运行一个从 `a` 到 `b` 的循环，将所有从 `a` 到 `b` 的数字相乘，检查乘积是正、负还是零。对于 `a` 和 `b` 的较大值，该解决方案将失败，并将导致溢出。

## 有效方法

有三种可能情况：

1.  如果 `a > 0` 和 `b > 0`，则所得乘积为正。
