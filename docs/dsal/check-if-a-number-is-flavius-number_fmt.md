...
> 原文: [https://www.geeksforgeeks.org/check-if-a-number-is-flavius-number/](https://www.geeksforgeeks.org/check-if-a-number-is-flavius-number/)

给定一系列从 `1` 到`无穷大`的整数和一个数字 `N`。
任务是在每第`i`次迭代时，从剩余系列中移除每第 `(i + 1)`个元素，并发现系列中是否存在给定数量的 `N`。
...
*   重复这个过程。
    *   否则移除第一次迭代中移除的元素数量，即数量的 `1/2`，然后检查如果它能被 `3` 整除，答案应该是“否”，否则减去之前移除的数量，即数量的 `1/3`，以此类推。
