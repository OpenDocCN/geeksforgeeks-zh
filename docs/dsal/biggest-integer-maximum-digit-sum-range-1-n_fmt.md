...
> 原文: [https://www.geeksforgeeks.org/maximum-integer-maximum-digits-sum-range-1-n/](https://www.geeksforgeeks.org/maximum-integer-maximum-digits-sum-range-1-n/)
...
是对从 1 到 `n` 的所有数字进行迭代，找出哪个数字的数字和最大。这个解的时间复杂度是 `O(n)`。

## 一种有效的方法

是从 `n` 迭代到 1。对当前数字的每个数字执行以下操作，如果该数字不为零，将其减少 1，并将所有其他数字更改为其右侧的 9。如果结果整数的位数总和严格大于当前答案的位数总和，则使用结果整数更新答案。如果结果整数之和与当前答案相同，则如果结果整数大于当前答案，则用结果整数更新当前答案。
