...
> 原文: [https://www.geeksforgeeks.org/program-to-calculate-bitonicity-of-an-array/](https://www.geeksforgeeks.org/program-to-calculate-bitonicity-of-an-array/)

给定一组`N`个整数。任务是找到给定数组的比特数。
数组`arr[]`的比特性可以定义为:
...
想法是取一个变量`bt = 0`并开始遍历数组，如果下一个元素大于当前元素，则增加`bt` 1，如果下一个元素小于当前元素，则减少`bt` 1。
以下是上述方法的实施:
