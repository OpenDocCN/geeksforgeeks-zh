...
> 原文: [https://www.geeksforgeeks.org/find-the-largest-possible-value-of-k-such-that-k-modulo-x-is-y/](https://www.geeksforgeeks.org/find-the-largest-possible-value-of-k-such-that-k-modulo-x-is-y/)
...
解决问题最简单的方法就是检查 `K` 在 `[0, N]` 范围内的每个可能值，是否满足条件 `K % X = Y`。如果没有这样的 `K`，打印 `-1`。否则，从满足条件的范围打印最大可能值 `K`。
...
上述方法可以基于观察到 `K` 可以获得以下值之一来满足等式而进行优化:
