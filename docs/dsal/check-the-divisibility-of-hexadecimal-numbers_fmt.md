...
> 原文: [https://www.geeksforgeeks.org/check-the-divisibility-of-hexadecimal-numbers/](https://www.geeksforgeeks.org/check-the-divisibility-of-hexadecimal-numbers/)

给定一个由一个大的十六进数组成的字符串 `S`，任务是用一个给定的十进制数 `M` 检查它的可除性。如果可分，则打印`是`否则打印`否`。
...
本[篇](https://www.geeksforgeeks.org/check-divisibility-binary-stream/)采用的一种进场方式将用于避免溢出。从背面迭代整个字符串。
