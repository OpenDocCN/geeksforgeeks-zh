...
> 原文: [https://www.geeksforgeeks.org/check-if-k-can-be-obtained-by-performing-arithmetic-operations-on-any-permutation-of-an-array/](https://www.geeksforgeeks.org/check-if-k-can-be-obtained-by-performing-arithmetic-operations-on-any-permutation-of-an-array/)

## 问题描述

给定一个由 `N` 个整数组成的数组 `arr[]` 和一个整数 `K`，任务是检查在分配算术运算符（`+`、`-`、`/`、`*`）后，为给定数组的任何排列形成的表达式是否给出值 `K`。如果发现为真，则打印执行操作的顺序。否则，打印 `-1`。
...
思路是使用回溯，通过生成赋值运算符的所有可能组合，找到结果方程的值。以下是步骤：

1.  遍历数组并选择前两个元素，应用所有可能的操作。让上述操作的结果为 `X`。
