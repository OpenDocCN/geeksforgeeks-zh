...
我们知道整个数组的 [GCD](https://www.geeksforgeeks.org/c-program-find-gcd-hcf-two-numbers/) 将会是**最大整数**来划分数组的每个元素。如果 `GCD = 1`，那么没有整数可以分割整个数组。然而，如果 `GCD > 1` 则存在整数，该整数完全划分数组。例如，
