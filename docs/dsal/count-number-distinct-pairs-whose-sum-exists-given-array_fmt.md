...
> 原文:[https://www.geeksforgeeks.org/count-number-distinct-pairs-whose-sum-exists-given-array/](https://www.geeksforgeeks.org/count-number-distinct-pairs-whose-sum-exists-given-array/)
...
想法是使用`map`来寻找独特的元素。我们首先在地图中存储元素及其数量。然后我们遍历数组元素，对于每对元素`(arr[i]，arr[j])`，我们检查数组中是否存在`(arr[i] + arr[j])`。如果存在，那么我们使用配对图检查它是否已经被计数。如果还没有计数，那么我们增加计数。
