...
> 原文：[https://www.geeksforgeeks.org/minimum-removals-in-a-number-to-be-divisible-by-10-power-raised-to-k/](https://www.geeksforgeeks.org/minimum-removals-in-a-number-to-be-divisible-by-10-power-raised-to-k/)

## 问题描述

给定两个正整数 `N` 和 `K`。找出可以从数字 `N` 中删除的最小位数，这样删除后的数字可以被 `10^K` 整除，否则打印 `-1`。
...
想法是从最后一位开始遍历数字，同时保留一个计数器。如果当前数字不为零，递增计数器变量，否则递减变量 `K`。当 `K` 变为零时，返回计数器作为答案。遍历整数后，检查 `K` 的当前值是否为零。如果是零，返回计数器作为答案，否则返回答案作为 `N` 中的位数减一，因为我们需要将整个数字减少到一个可被任何数字整除的零。此外，如果给定的数字不包含任何零，则返回 `-1` 作为答案。
