...
> 原文: [https://www.geeksforgeeks.org/check-if-a-given-matrix-can-be-converted-to-another-given-matrix-by-row-and-column-exchanges/](https://www.geeksforgeeks.org/check-if-a-given-matrix-can-be-converted-to-another-given-matrix-by-row-and-column-exchanges/)
...
> `final[][] = {{1, 4, 3, 2}, {5, 8, 7, 6}, {13, 16, 15, 14}, {9, 12, 11, 10}}`

**方法:**
为了解决这个问题，我们只需要检查 `startMatrix` 中所有行和列中的元素是否保留在 `finalMatrix` 矩阵中，而不管它们的顺序如何。任何违反此条件的行为都将确保无法获得 `finalMatrix` 矩阵。遍历每一行的循环，检查该行在 `startMatrix` 中的所有元素是否在 `finalMatrix` 中的一行中。然后转置 `startMatrix` 和 `finalMatrix` 并重复相同的验证。
