...
> 原文: [https://www.geeksforgeeks.org/product-of-the-maximums-of-all-subsets-of-an-array/](https://www.geeksforgeeks.org/product-of-the-maximums-of-all-subsets-of-an-array/)

给定一个由 `N` 个正整数组成的[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `arr[]`，任务是找到给定数组的所有可能子集的[最大值](https://www.geeksforgeeks.org/c-program-find-largest-element-array/)的乘积。由于乘积可以很大，打印到[模 `(10^9 + 7)`](https://www.geeksforgeeks.org/modulo-1097-1000000007/)。
...
解决给定问题最简单的方法是[生成给定数组的所有可能子集](https://www.geeksforgeeks.org/print-subsets-given-size-set/)，求所有生成子集最大值的乘积模 `(10^9 + 7)` 作为结果乘积。
...
*   因此，由小于或等于每个数组元素的所有元素形成的子集的数量 `arr[i]` 有助于具有 `arr[i]` 作为最大元素的子集的计数。
...
*   [使用变量 `i` 从头到尾遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)，并执行以下步骤:
    *   找出小于当前元素 `arr[i]` 的子集数作为 `(2^i – 1)`，并将其存储在一个变量中，比如 `P`。
    *   由于数组元素 `arr[i]` 贡献 `P` 次，因此将值 `arr[i]` `P` 次乘以变量 `最大乘积`。
*   用[求数组元素的乘积](https://www.geeksforgeeks.org/program-for-product-of-array/)，`最大乘积` 包括大小 `1` 的所有子集。
