...
> 原文：[https://www.geeksforgeeks.org/find-the-minimum-absolute-difference-in-two-different-bsts/](https://www.geeksforgeeks.org/find-the-minimum-absolute-difference-in-two-different-bsts/)
...
1.  为两个 `BST` 创建前向迭代器。假设它们所指向的节点的值分别为 `v1` 和 `v2`。
2.  现在在每一步：
    *   最后更新 `ans` 为 `min(ans, abs(v1 - v2))`。
    *   如果 `v1 < v2`，移动第一个 `BST` 的迭代器，否则移动第二个 `BST` 的迭代器。
