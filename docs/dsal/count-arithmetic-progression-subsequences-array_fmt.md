...
> 原文: [https://www.geeksforgeeks.org/count-arithmetic-progression-subsequences-array/](https://www.geeksforgeeks.org/count-arithmetic-progression-subsequences-array/)

给定一组 `n` 个正整数。任务是计算数组中算术级数子序列的数量。注意：空序列或单元素序列是算术级数。`1 <= arr[i] <= 1000000`。
...
现在，我们需要找到长度大于等于 2 的算术级数子序列。设数组的最小值和最大值分别为 `minarr` 和 `maxarr`。注意，在所有算术级数子序列中，公共差的范围将从 (`minarr` – `maxarr`) 到 (`maxarr` – `minarr`)。现在，对于每个常见的差异，比如 `d`，使用动态编程计算长度大于或等于 2 的子序列。
设 `dp[i]` 是以 `arr[i]` 结尾的子序列的数目，并且具有 `d` 的公共差。所以，

![](img/9f746dc79e29c575b5b41f939968a7f6.png)

长度大于或等于 2 且具有公共差 `d` 的子序列的数目是 `dp[i]` – 1 的和，其中 `0 <= i < n`。为了加速，存储所有满足 `arr[j] + d = arr[i]` 且 `j < i` 的 `dp[j]` 的和。
以下是上述思想的实现：
