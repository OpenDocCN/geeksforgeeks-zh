...
> 原文: [https://www.geeksforgeeks.org/find-two-non-repeating-elements-in-an-array-of-repeating-elements/](https://www.geeksforgeeks.org/find-two-non-repeating-elements-in-an-array-of-repeating-elements/)
...
**方法:**
**步骤 1:** 将数组中的所有元素异或为一个变量 `sum`，这样数组中出现两次的所有元素都将被移除，例如，`4 =“100”`，如果 `4` 异或 `4` => `“100”`异或`“100”`，则答案为`“000”`。
**步骤 2:** 因此，在 `sum` 中，最终答案将是 `3` 异或 `5`，因为 `2` 和 `4` 都与自身异或，给出 `0`，因此 `sum =“011”`异或`“101”`，即 `sum =“110”= 6`。
**第三步:** 现在我们取 `sum` 的 `2` 的补码，即`(-sum)=“010”`。
**第四步:** 现在将 `sum` 的 `2` 与 `sum` 进行按位 `And` 运算，即`“110”&“010”`给出答案`“010”`(按位`&`的目的是我们想要得到一个只包含 `sum` 的最右边集合位的数字)。
**第五步:** 按位`&`将数组的所有元素与这个得到的 `sum` 相加，`2 =“010”&“010”= 2`，`3 =“011”&“010”=“010”`，`4 =“100”&“010”=“000”`，`5 =“101”&“010”=“000”`。
