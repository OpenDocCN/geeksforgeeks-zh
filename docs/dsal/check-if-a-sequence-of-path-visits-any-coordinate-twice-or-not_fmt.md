...
> 原文: [https://www.geeksforgeeks.org/check-if-a-sequence-of-path-visits-any-coordinate-twice-or-not/](https://www.geeksforgeeks.org/check-if-a-sequence-of-path-visits-any-coordinate-twice-or-not/)

给定长度为 `N` 的字符串 `str`，仅由字符 `N`、`S`、`E` 或 `W` 组成，每个字符分别代表移动一个单位北、南、东或西。一个人从 2D 平面的原点 `(0, 0)` 开始，按照字符串上的方向行走。任务是检查这个人在行走中是否越过了他已经访问过的任何坐标。如果发现是真的，则打印 `Crossed`。否则，打印 `Not Crossed`。
...
想法是为遇到的坐标保持一个集合，以检查已经访问的坐标是否出现。以下是步骤:

1.  初始化两个变量 `X` 和 `Y`，并将数值初始化为零。这些变量将表示 x 和 y 坐标。
2.  在集合中插入 `X` 和 `Y`。
3.  在 `[0, N)` 范围内循环迭代，按照以下条件增加 X 或 Y 坐标:
    ...
    *   如果字符为 `E`，则用 1 递增 `X`。
    *   如果字符为 `W`，则以 1 递减 `X`。
4.  为集合中的每个字符插入上一步更新的 `X` 和 `Y` 坐标。
5.  在上述步骤中插入坐标时，如果遇到任何当前坐标，则打印 `Crossed`。
6.  否则，打印 `Not Crossed`，因为所有坐标点都是唯一的。
