# 级数 `K^n + (K^(n-1) * (K-1)^1) + (K^(n-2) * (K-1)^2) + …… + (K-1)^n`

> 原文: [https://www.geeksforgeeks.org/sum-of-the-series-kn-kn-1-k-11-kn-2-k-12-k-1n/](https://www.geeksforgeeks.org/sum-of-the-series-kn-kn-1-k-11-kn-2-k-12-k-1n/)

## 问题描述

给定一个 `K` 和 `n` 的值，任务是求以下级数的和：

> **`K^n + (K^(n-1) * (K-1)^1) + (K^(n-2) * (K-1)^2) + …… + (K-1)^n`**
