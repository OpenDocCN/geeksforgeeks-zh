...
> 原文：[https://www.geeksforgeeks.org/count-magic-squares-in-a-grid/](https://www.geeksforgeeks.org/count-magic-squares-in-a-grid/)
...
让我们分别检查每 3×3 个子网格。对于每个网格，所有数字必须是唯一的，并且在 `1` 和 `9` 之间，以及每`行`、`列`之间，并且两条`对角线`必须具有相等的总和。
...
*   中间元素必须是 5。
*   网格的总和必须是 45 并且包含从 1 到 9 的所有不同值。
*   每行和每列的和必须为 15。
*   两条对角线的和也必须为 15。
