...
> 原文: [https://www.geeksforgeeks.org/check-binary-representation-number-palindrome/](https://www.geeksforgeeks.org/check-binary-representation-number-palindrome/)

给定一个整数`x`，写一个 C 函数，如果`x`的二进制表示是回文，则返回 true，否则返回 false。
例如二进制表示为 `10..01` 的数字是回文，而二进制表示为 `10..00` 的数字不是回文。
