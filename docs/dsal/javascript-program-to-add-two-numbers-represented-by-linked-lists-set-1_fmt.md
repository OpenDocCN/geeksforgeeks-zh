...
> 原文: [https://www.geeksforgeeks.org/javascript-program-to-add-two-numbers-represented-by-linked-lists-set-1/](https://www.geeksforgeeks.org/javascript-program-to-add-two-numbers-represented-by-linked-lists-set-1/)
...
遍历两个列表，逐个选择两个列表的节点，并添加值。如果总和大于 10，则进位为 1 并减少总和。如果一个列表中的元素比另一个列表中的多，则认为该列表的剩余值为 0。
...
4.  继续下去，直到两个列表都结束。
5.  如果两位数之和大于 9，则将进位设置为 1，将当前数字设置为 `sum % 10`。
