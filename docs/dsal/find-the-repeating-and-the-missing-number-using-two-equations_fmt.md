...
> 原文: [https://www.geeksforgeeks.org/find-the-repeating-and-the-missing-number-using-two-equations/](https://www.geeksforgeeks.org/find-the-repeating-and-the-missing-number-using-two-equations/)

给定一个大小为 `N` 的数组 `arr[]`，除了出现两次的 `A` 和缺少一次的 `B` 之外，范围 `[1, N]` 中的每个整数都恰好出现一次。任务是找到数字 `A` 和 `B`。
