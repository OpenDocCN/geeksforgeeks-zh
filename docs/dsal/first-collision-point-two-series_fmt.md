...
> 原文：[https://www.geeksforgeeks.org/first-collision-point-two-series/](https://www.geeksforgeeks.org/first-collision-point-two-series/)

给定五个数字 `a`、`b`、`c`、`d` 和 `n`（其中 `a`、`b`、`c`、`d`、`n > 0`）。这些值代表两个系列的 `n` 项。这两个数列分别是 `b`，`b+a`，`b+2a`…`b+(n-1)a` 和 `d`，`d+c`，`d+2c`，…..`d+(n-1)c`。
...
function point(a, b, c, d, n)
{
    ...
    if(flag == 0)
    {
            document.write("No collision point");
    }
