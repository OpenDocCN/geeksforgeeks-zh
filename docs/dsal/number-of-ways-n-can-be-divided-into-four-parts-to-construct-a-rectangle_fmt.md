...
**方法:** 由于数字必须被分割，这样矩形由被分割的四个部分组成，所以如果数字是奇数，那么的方式数将为零，因为矩形的周长总是偶数。

现在，如果 `n` 是偶数，那么只有`(n–2)/4`的方式来分割数字，例如，如果必须将 8 分成四份，那么只有`(8–2)/4 = 1`种方式，即`【1，1，3，3】`，没有其他方式。因为你只能取边长`<= n/2–1`组成一个有效的矩形，然后从那些`n/2–1`的矩形中再除以 2，避免重复计数。
