# Proto Van Emde Boas 树：查询——继任者与前任

> 原文：[https://www.geeksforgeeks.org/proto-van-emde-boas-tree-set-6-query-successor-and-predecessor/](https://www.geeksforgeeks.org/proto-van-emde-boas-tree-set-6-query-successor-and-predecessor/)
...
后续查询和前置查询的重复关系:
```
T(u) = 2T(√u) + O(log₂(log₂(u)))
```

**时间复杂度:** 每个查询 `O(log₂(u) * log₂(log₂(u)))`
**辅助空间:** `O(N)`。