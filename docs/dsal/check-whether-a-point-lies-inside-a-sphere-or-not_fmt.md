...
> 原文: [https://www.geeksforgeeks.org/check-whether-a-point-lies-inside-a-sphere-or-not/](https://www.geeksforgeeks.org/check-whether-a-point-lies-inside-a-sphere-or-not/)
...
如果，一个点 `(x, y, z)` 位于具有中心 `(cx, cy, cz)` 和半径 `r` 的球体上

```
( x-cx ) ^2 + (y-cy) ^2 + (z-cz) ^ 2 = r^2
```

如果，一个点 `(x, y, z)` 在具有中心 `(cx, cy, cz)` 和半径 `r` 的球体之外
