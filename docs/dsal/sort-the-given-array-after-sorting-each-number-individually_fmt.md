...
> 原文: [https://www.geeksforgeeks.org/sort-the-given-array-after-sorting-each-number-individually/](https://www.geeksforgeeks.org/sort-the-given-array-after-sorting-each-number-individually/)
...
1.  创建一个函数 `toString`，该函数将接受整数 `N` 作为参数，并将返回字符串形式的 `N`。
2.  将函数 `toString` 中数组 `arr` 的每个元素传递给字符串将其转换为字符串。对该字符串进行排序，并将其转换回整数。之后，用数组 `arr` 中转换后的整数替换每个元素。
