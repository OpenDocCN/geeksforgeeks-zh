...
> [**十边形数**](https://www.geeksforgeeks.org/decagonal-numbers/) 是一个将三角形和正方形的概念扩展到十边形（10边多边形）的图形数。第 `n` 个十边形数字计算 `n` 个嵌套十边形图案中的点数，所有十边形共享一个公共角，其中图案中的第 `i` 个十边形具有由相互间隔一个单位的 `i` 个点组成的边。前几个十边形数字是 1，10，27，52，85，126，175，…
...
2.  因为我们必须检查给定的数字是否可以表示为十边形数字。可以勾选为：
...
3.  如果使用上述公式计算的 `K` 的值是整数，那么 `N` 是十边形数。
4.  否则 `N` 不是十进制数。
