...
> 原文: [https://www.geeksforgeeks.org/count-characters-in-a-string-whose-ascii-values-are-prime/](https://www.geeksforgeeks.org/count-characters-in-a-string-whose-ascii-values-are-prime/)

给定一个字符串 `S`。任务是计算并打印字符串中 ASCII 值为质数的字符数。
...
思路是使用[厄拉多塞的筛](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)生成字符串 `S` 字符最大 ASCII 值以内的所有素数。现在，迭代字符串并获取每个字符的 ASCII 值。如果 ASCII 值是质数，则增加计数 `count`。最后，打印 `count`。
