...
> 原文：[https://www.geeksforgeeks.org/partition-of-a-set-into-k-subsets-with-equal-sum-using-bitmask-and-dp/](https://www.geeksforgeeks.org/partition-of-a-set-into-k-subsets-with-equal-sum-using-bitmask-and-dp/)
...
*   想法是用**掩码**确定当前状态。当前状态将告诉我们已经形成的子集（已经选择了哪些数字）。
    **例如：** `arr[] = [2，1，4，3，5，6，2]`，`掩罩 = (1100101)`，这意味着当前掩罩中已经选择了 `{2，1，5，2}`。
*   对于任何当前状态 `掩罩`，将基于以下两个条件向其添加 `j` 元素：

> 1.  `掩罩` 的第 `j` 位未被设置（`掩罩 & (1 << j) == 0`）
> 2.  `和(掩罩) + arr[j] <= 目标值`（其中 `目标值 = (数组元素之和) / K`）

*   维护一个表 `dp[]`，这样 `dp[i]` 会存储掩码 `i` 中元素的总和。因此，动态规划转换将为：

> `DP[i | (1 << j)] = (dp[i] + arr[j]) % 目标值`
> **图解：**
> `arr[] = [4，3，2，3，5，2，1]`，`K = 4`，`目标值 = 5`
> `DP["1100101"]` 暗示选择了 `{4，3，5，1}`
> 因此，`和 = 4 + 3 + 5 + 1 = 13`，`13 % 5 = 3`。
> 因此，`DP["1100101"] = 3`
