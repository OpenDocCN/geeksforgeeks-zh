...
> 原文：[https://www.geeksforgeeks.org/queries-to-replace-subarrays-by-equal-length-arrays-with-at-most-p-replacements-allowed-for-any-array-element/](https://www.geeksforgeeks.org/queries-to-replace-subarrays-by-equal-length-arrays-with-at-most-p-replacements-allowed-for-any-array-element/)

给定大小为 `N` 的数组 `arr[]`，整数 `P` 和由以下类型的查询组成的 2D 数组 `Q[][]`：

*   `1 L R B[R–L+1]`：此查询的任务是将子数组 `{arr[L], …, arr[R]}` 替换为数组 `B[]`，假设任何数组元素最多可以替换 `P` 次。
*   `2 X`：该查询的任务是打印 `arr[X]`。

**示例：**

> **输入：** `arr[] = {3, 10, 4, 2, 8, 7}`，`P = 1`，`Q[][] = {{1, 0, 3, 3, 2, 1, 11}, {2, 3}, {1, 2, 3, 5, 7}, {2, 2}}`
> **输出：** `11`
> **解释：**
> 查询 1：将子数组 `{arr[0], …, arr[3]}` 替换为数组 `{3, 2, 1, 11}`。
> 查询 2：打印 `arr[3]`，即 `11`。
...
问题可以使用并查集算法解决。思路是遍历数组 `Q[][]`，检查 `Q[0]` 是否等于 `1`。如果发现为真，则用新数组替换子数组，并且每当任何数组元素被替换 `P` 次时，则使用并查集创建新子集。按照以下步骤解决问题：

*   初始化一个数组，比如 `visited[]`，其中 `visited[i]` 检查索引 `i` 是否存在于任何不相交的子集中。
*   初始化一个数组，比如 `count[]`，其中 `count[i]` 存储 `arr[i]` 被替换的次数。
*   初始化一个数组，比如 `last[]`，来存储每个不相交子集的最大元素。
*   初始化一个数组，比如 `parent[]`，来存储每个不相交子集的最小元素。
*   遍历 `Q[][]` 数组，对于每个查询，检查 `Q[i][0] == 1` 是否存在。如果发现为真，则执行以下操作：
    *   使用变量 `low` 迭代范围 `[Q[i][1], Q[i][2]]`，检查 `visited[low]` 是否为真。如果发现为真，则找到存在 `low` 的子集的父元素，找到该子集中存在的最大元素。
    *   否则，检查 `count[low]` 是否小于 `P`。如果发现为真，则用相应的值替换 `arr[low]` 的值。
    *   否则，检查 `count[low]` 是否等于 `P`。如果发现为真，则创建当前索引的新的不相交子集。
*   否则，打印 `arr[Q[i][1]]`。
...
class GFG {
    ...
    public static void main(String[] args)
    {
        ...
        // Initialize parent and last arrays
        for (int i = 0; i < N; i++) {
            parent[i] = i;
            last[i] = i;
        }
