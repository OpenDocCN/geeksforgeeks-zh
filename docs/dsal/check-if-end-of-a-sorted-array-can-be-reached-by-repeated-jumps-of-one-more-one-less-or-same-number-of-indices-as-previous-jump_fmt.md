...
> 原文: [https://www.geeksforgeeks.org/check-if-end-of-a-sorted-array-can-be-reached-by-repeated-jumps-of-one-more-one-less-or-same-number-of-indices-as-previous-jump/](https://www.geeksforgeeks.org/check-if-end-of-a-sorted-array-can-be-reached-by-repeated-jumps-of-one-more-one-less-or-same-number-of-indices-as-previous-jump/)

给定一个大小为 `N` 的排序后的数组 `arr[]`，任务是通过在每次移动中跳转 `arr[i]+k–1`、`arr[i] + k` 或 `arr[i] + k + 1` 来检查是否有可能从 `arr[1]` 到达给定数组的末尾，其中 `k` 表示索引的数量最初考虑 `K = 1`。如果可能，则打印 `"是"`。否则，打印 `"否"`。
...
想法是使用递归。从索引 `i`，递归移动到具有值 `A[i]+K–1`、`A[i] + K` 或 `A[i] + K + 1` 的索引，并检查是否有可能到达终点。如果存在到达终点的路径，则打印 `"是"`否则打印 `"否"`。
...
优化上述途径，思路是使用动态规划。创建一个 2D 表 `memo[N][N]` 来存储记忆的结果。对于任何索引 `(i，j)`，`memo[i][j]` 表示是否有可能从索引 `i` 移动到结束 `(N-1)` 和先前采取的 `j` 步骤。`memo[i][j] = 1` 表示可能，`memo[i][j] = 0` 表示不可能。按照以下步骤解决问题:

*   创建一个大小为 `N*N` 的记忆表 `memo[][]`。
*   对于任何索引 `(i，j)` 更新 `memo[][]` 表格，如下所示:
    *   如果 `i` 等于 `(N–1)`，到达终点位置时返回 `1`。
    *   如果 `memo[i][j]` 已经计算，则返回 `memo[i][j]`。
    *   检查任何具有 `A[i] + j`、`A[i]+j–1` 或 `A[i] + j + 1` 值的索引，并递归检查是否有可能到达终点。
    *   将该值存储在 `memo[i][j]` 中并返回该值。
*   如果有可能到达终点，打印 `"是"`。
*   否则，打印 `"否"`。
