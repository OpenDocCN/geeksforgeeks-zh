...
> 原文：[https://www.geeksforgeeks.org/minimum-removals-required-to-make-any-interval-equal-to-the-union-of-the-given-set/](https://www.geeksforgeeks.org/minimum-removals-required-to-make-any-interval-equal-to-the-union-of-the-given-set/)
...
> **输出：** `2`
> **解释：**
> 删除区间 `[1, 3]` 和 `[13, 20]` 将集合修改为 `{[4, 12], [5, 8]}`。区间 `[4, 12]` 是集合的并集。
>
...
1.  遍历给定的一组区间。
2.  对于集合中的每个区间，找出其左边界大于或等于其左边界以及其右边界小于或等于其右边界的所有区间。将这些区间的计数存储在一个变量中，比如 `Count`。
3.  找出每个区间的所有 `N - Count` 值的最小值（因为 `N - Count` 会给出删除的区间数）。
4.  打印获得的最小值作为所需答案。
