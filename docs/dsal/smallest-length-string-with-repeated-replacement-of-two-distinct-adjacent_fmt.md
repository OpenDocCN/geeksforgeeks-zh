...
> 原文：[https://www.geeksforgeeks.org/smallest-length-string-with-repeated-replacement-of-two-distinct-adjacent/](https://www.geeksforgeeks.org/smallest-length-string-with-repeated-replacement-of-two-distinct-adjacent/)
...
**归纳步骤**：取两个长度为 `k + 1` 的字符串，每个字母出现的次数相同。找出一对相邻的字母在两个弦上。这里出现了两种情况：
...
在最坏的情况下，每个字母出现在整个字符串的 1/3 处。这导致辅助空间=`O(N^3)`和时间复杂度=`O(N^3)`。
