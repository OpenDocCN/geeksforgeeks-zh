...
> 原文: [https://www.geeksforgeeks.org/check-number-can-written-sum-k-prime-numbers/](https://www.geeksforgeeks.org/check-number-can-written-sum-k-prime-numbers/)
...
如果 `N >= 2K` 且 `K = 1`: 如果 `N` 是素数则答案为是。

如果 `N >= 2K` 且 `K = 2`: 如果 `N` 是偶数则答案为是(哥德巴赫猜想)。如果 `N` 是奇数则答案为否，如果 `N-2` 不是素数则答案为是。这是因为我们知道奇数+奇数=偶数，偶数+奇数=奇数。所以当 `N` 是奇数，`K = 2` 时，一个数必须是 2，因为它是唯一的偶数素数，所以现在答案取决于 `N-2` 是否是奇数。
