# 在 Python 中使用 `update()` 实现 n 个数组的并集

> 原文：[https://www.geeksforgeeks.org/set-update-python-union-n-arrays/](https://www.geeksforgeeks.org/set-update-python-union-n-arrays/)
...
`anySet.update(iterable)`，此方法将名为 `anySet` 的集合与任何给定的 `iterable` 进行并集，并且不像 `union()` 方法那样返回集合的任何浅拷贝，它将结果更新为前缀集，即 `anySet`。
