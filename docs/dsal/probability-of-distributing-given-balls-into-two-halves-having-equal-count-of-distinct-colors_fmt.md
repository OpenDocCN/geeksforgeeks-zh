...
> 原文: [https://www.geeksforgeeks.org/probability-of-distributing-given-balls-into-two-halves-having-equal-count-of-distinct-colors/](https://www.geeksforgeeks.org/probability-of-distributing-given-balls-into-two-halves-having-equal-count-of-distinct-colors/)

给定一个大小为 `N` 的数组 `arr[]`，代表每个 `N` 不同颜色的球的数量，任务是找到将所有球分配到两个盒子中的概率，使得两个盒子包含相同数量的不同颜色的球。
...
> **输出:** `1.00000`
> **说明:** 两种可能的分配方式如下:
> 将第 0 个色型的球放入第 1 个盒子，将第 1 个色型的球放入第 2 个盒子。
> 将 1 st 型的颜色放在 1 st 框中，将 0 th 型的颜色放在第二个框中。
> 因此，概率等于 `(2 / 2) = 1.00000`
>
> **输入:** `arr[] = {2，1，1}`，`N = 3`
> **输出:** `0.66667`
...
思路是用组合学和回溯来解决问题。给定的问题可以基于以下观察来解决:

> *   令 `x` 为将 `k` 个球分成两半的总方法数。
> *   可以观察到 `x` 等同于从 `k` 个球中选择 `k/2` 个球，等于 `<sup>k</sup> c <sub>(k/2)</sub>`。
> *   对于每个 `0 ≤ i < n`，从 `arr[i]` 中选择 `j` 个球放入一个盒子，这可以通过回溯来计算。
> *   因此，所需的概率是 `y/x`。

按照以下步骤解决问题:

1.  计算数组 `arr[]` 中所有元素的总和到一个变量中，比如 `K`。
2.  如果 `K` 是奇数，打印 `0`。
3.  计算选择 `K/2` 个球的方式数并存储在变量中，比如 `X`。
4.  定义一个递归函数，比如 `validPermutations(pos, usedBalls, box1, box2)`，并执行以下步骤:
    *   定义基本情况: 如果 `usedBalls` 等于 `K/2`，那么如果 `box1 == box2`，则返回 `1`。否则，返回 `0`。
    *   如果 `pos >= N`，则返回 `0`。
    *   现在，初始化一个变量，比如 `res`，来存储剩余球的分配方式。
    *   迭代范围 `[0, arr[pos]]`:
        *   将 `box1` 和 `box2` 分配给变量，分别表示 `newBox1` 和 `newBox2`。
        *   如果 `j > 0` 和 `newBox2`，如果 `j < arr[pos]`，则将 `newBox1` 增加 1。
        *   现在，更新 `res` 为 `res = res + <sup>arr[pos]</sup>c<sub>j</sub> * validPermutations(pos + 1, usedBalls + j, newBox1, newBox2)`。
    *   返回 `res` 的值。
5.  调用函数 `validPermutations(0, 0, 0, 0)` 并将其存储在变量中，比如 `Y`。
6.  最后，将得到的结果打印为 `Y/X`。
