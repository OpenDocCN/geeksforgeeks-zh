...
> 原文：[https://www.geeksforgeeks.org/priority-queue-using-array-in-c/](https://www.geeksforgeeks.org/priority-queue-using-array-in-c/)
...
*   `dequeue()`:
    *   使用 `peek()` 函数找到优先级最高的索引。让我们将该位置称为 `ind`，然后将位置 `ind` 之后的所有元素的位置向左移动一个位置。
    *   将尺寸减小一。
...
*   在分时计算机系统中，等待CPU时间的过程被加载到[优先级队列](https://www.geeksforgeeks.org/priority-queue-set-1-introduction/)中。
