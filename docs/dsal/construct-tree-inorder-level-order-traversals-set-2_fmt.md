...
以下算法使用`O(N ^ 2)`时间复杂度来解决上述问题，它使用 C++ 中的[`unordered_set`](https://www.geeksforgeeks.org/unorderd_set-stl-uses/)数据结构（基本上是创建哈希表）来放入当前根的左子树的值，之后我们将以`O(1)`的复杂度检查，以确定当前的`levelOrder`节点是否是左子树的一部分。
