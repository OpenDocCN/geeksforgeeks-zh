...
> 原文: [https://www.geeksforgeeks.org/program-chocolate-wrapper-puzzle/](https://www.geeksforgeeks.org/program-chocolate-wrapper-puzzle/)

给定以下三个值，任务是找到你能吃的最大巧克力总数。

1.  `money`: 买巧克力的钱
2.  `price`: 一块巧克力的价格
3.  `wrap`: 多拿一块巧克力需要退回的包装数量。
...
据观察，我们只需将巧克力和包装纸的值减 1，然后除以即可得到结果 `(choc-1)/(wrap-1)`
