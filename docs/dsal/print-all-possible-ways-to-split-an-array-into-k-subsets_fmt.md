...
> 原文：[https://www.geeksforgeeks.org/print-all-possible-ways-to-split-an-array-into-k-subsets/](https://www.geeksforgeeks.org/print-all-possible-ways-to-split-an-array-into-k-subsets/)

给定一个大小为 `N` 的数组 `arr[]` 和一个整数 `K`，任务是打印所有可能的方式将给定数组拆分为 `K` 个子集。
...
>
> **输入：** `arr[] = { 1, 2, 3, 4 }`，`K = 2`
> **输出：** `{ {{ 1, 2, 3 }, { 4 }}, {{ 1, 2, 4 }, { 3 }}, {{ 1, 2 }, { 3, 4 }}, {{ 1, 3, 4 }, { 2 }}, {{ 1, 3 }, { 2, 4 }}, {{ 1, 4 }, { 2, 3 }}, {{ 1 }, { 2, 3, 4 }} }`

**方法：** 使用回溯生成并打印所有子集可以解决问题。按照以下步骤解决问题：

1.  遍历数组并使用以下递归关系将元素插入任意一个 `K` 子集：
