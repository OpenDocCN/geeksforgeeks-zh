...
> 原文: [https://www.geeksforgeeks.org/sort-a-string-in-increasing-order-of-given-priorities/](https://www.geeksforgeeks.org/sort-a-string-in-increasing-order-of-given-priorities/)
...
1.  初始化两个变量，比如 `digits` 和 `character`，分别存储字符和数字。
2.  根据 ASCII 表对字符串 `digits` 和 `character` 进行排序。
3.  现在，遍历字符中的所有字符，并将每个具有奇数奇偶校验的字符附加到一个变量上，比如说 `OddChar`，将每个具有偶数奇偶校验的字符附加到另一个变量上，比如说 `EvenChar`。
4.  同样，对于字符串数字，将奇偶校验位分开，比如 `OddDigit` 和 `EvenDigit`。
5.  最后，[将字符串连接为 `OddChar + EvenChar + OddDigit + EvenDigit`。](https://www.geeksforgeeks.org/python-string-concatenation/)
