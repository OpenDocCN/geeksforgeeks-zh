...
> 原文: [https://www.geeksforgeeks.org/count-number-of-rotated-strings-which-have-more-number-of-vowels-in-the-first-half-than-second-half/](https://www.geeksforgeeks.org/count-number-of-rotated-strings-which-have-more-number-of-vowels-in-the-first-half-than-second-half/)
...
一个有效的方法是制作字符串 `s = str + str`，那么 `s` 的大小将为 `2 * N`。现在，制作一个前缀数组来存储从第 0 个索引到第 `i` 个索引的元音数量。然后从 `N–1` 到 `2 * N–2` 循环，得到 `str` 的所有旋转弦。查找所需旋转字符串的计数。
...
为了将上述方法的空间复杂度降低到常数，将两部分元音的数量存储在`两个变量`中，并通过改变索引来迭代所有旋转。
