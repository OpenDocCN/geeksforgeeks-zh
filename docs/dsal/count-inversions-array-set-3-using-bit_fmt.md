...
> 原文: [https://www.geeksforgeeks.org/count-inversions-array-set-3-using-bit/](https://www.geeksforgeeks.org/count-inversions-array-set-3-using-bit/)
...
*   **算法:**
    1.  创建一个位，找出给定数字的位中较小元素的计数，以及一个变量 `result = 0`。
    2.  从头到尾遍历数组。
...
*   **时间复杂度:** 更新函数和 `getSum` 函数为 `O(log(maxElement))` 运行。必须为数组中的每个元素运行 `getSum` 函数。所以总的时间复杂度是: `O(nlog(maxElement))`。
...
遍历数组，对于每个索引，找到数组右侧较小元素的数量。这可以使用 `BIT` 来完成。合计数组中所有索引的计数，并打印总和。方法保持不变，但前一种方法的问题是，它不适用于负数，因为指数不能是负数。其思想是将给定的数组转换为值从 1 到 n 的数组，并且较小和较大元素的相对顺序保持不变。
...
1.  创建一个 `BIT`，找出给定数字的 `BIT` 中较小元素的计数，以及一个变量 `结果` = 0。
2.  前面的解决方案不适用于包含负元素的数组。因此，将数组转换为包含元素相对编号的数组，即复制原始数组，然后对数组的副本进行排序，并用排序数组中相同元素的索引替换原始数组中的元素。例如，如果数组是 `{-3, 2, 0}`，那么数组被转换为 `{1, 3, 2}`。
3.  从头到尾遍历数组。
4.  对于每个索引，检查比当前元素少多少个数字出现在 `BIT` 中，并将其添加到 `结果` 中。
