...
> 原文: [https://www.geeksforgeeks.org/zigzag-level-order-traversal-of-an-n-ary-tree/](https://www.geeksforgeeks.org/zigzag-level-order-traversal-of-an-n-ary-tree/)
...
给定的问题可以通过使用 [BFS 遍历](https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/)来解决。该方法非常类似于 N 元树的[级顺序遍历](https://www.geeksforgeeks.org/generic-tree-level-order-traversal/)。可以观察到，在水平顺序遍历期间反转偶数水平的顺序时，获得的序列是锯齿形遍历。基于这些观察，以下是要遵循的步骤:

*   在 BFS 遍历期间，将每一级的节点存储到一个[向量](https://www.geeksforgeeks.org/vector-in-cpp-stl/)中，比如说 `curLevel[]`。
*   对于每个相应的级别，将 `level` 存储到矢量的[矢量中，例如 `result[]`。](https://www.geeksforgeeks.org/vector-of-vectors-in-c-stl-with-examples/)
*   反转 `result[]` 中偶数位置的向量。
*   完成上述步骤后，所有向量存储在 `result[]` 所需的结果中。
