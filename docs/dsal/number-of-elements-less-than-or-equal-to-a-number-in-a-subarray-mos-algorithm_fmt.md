...
> 原文: [https://www.geeksforgeeks.org/number-of-elements-less-than-or-equal-to-a-number-in-a-subarray-mos-algorithm/](https://www.geeksforgeeks.org/number-of-elements-less-than-or-equal-to-a-number-in-a-subarray-mos-algorithm/)

给定一个数组`arr[]`、`N`和`Q`个形式为`L`、`R`和`X`的查询，任务是打印由`L`到`R`表示的子数组中小于或等于`X`的元素数量。
...
> **第 1 步** – 添加`arr[0]`，并将其频率增加为`freq[arr[0]]++`即`freq[3]++`和`freq[]=[0，0，0，1，0，0，0，0]`
> **第 2 步** – 添加`arr[1]`，并增加`freq[arr[1]]++`，即`freq[4]++`和`freq[]=[0，0，0，1，1，0，0，0]`
> **步骤 3** – 添加`arr[2]`并增加`freq[arr[2]]++`即`freq[6]++`和`freq[]=[0，0，0，1，1，0，1，0]`
> **步骤 4** – 添加`arr[3]`并增加`freq[arr[3]]++`即`freq[2]++`和`freq[]=[0，0，1，1，1，0，1，0]`
> **步骤 5** – 添加`arr[4]`并增加`freq[arr[4]]++`即`freq[7]++`和`freq[]=[0，0，1，1，1，0，1，1]`
> **第 6 步** – 添加`arr[5]`并增加`freq[arr[5]]++`即`freq[1]++`和`freq[]=[0，1，1，1，1，0，1，1]`
> **第 7 步** – 答案等于`\sum_{i=0}^{X} freq[i]`
> 为了计算第 7 步中的和，我们不能进行迭代，因为这样会导致每个查询的时间复杂度为`O(N)`，所以我们将使用**sqrt 分解技术**来寻找每个查询的和，时间复杂度为`O(√n)`。
