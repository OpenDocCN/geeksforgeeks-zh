...
> 原文:[https://www.geeksforgeeks.org/goldman-sachs-interview-experience-set-28-on-campus-for-full-time/](https://www.geeksforgeeks.org/goldman-sachs-interview-experience-set-28-on-campus-for-full-time/)

高盛第一次来到我们的校园进行全职招聘和实习。

选拔过程包括一轮在线选拔，随后是 4 轮 F2F 技术选拔。

## 线上轮
线上轮在 HackerRank 平台托管。它有 5 个 `MCQ`（每个 10 分）和两个编码问题（15+35 分）。
**注：**`MCQ` 和编码部分（均为 50-50）有各自的分段截止值。因此，相应地回答，以清除两个部分中的截止点。

- `MCQ` 是基本的 `OOPS` 和数据结构，但不要掉以轻心。
...
2.  给定一个大小为 `N` 的数组和数组的 `N` 个元素。给出了 `k` 值。找出子数组乘积小于或等于给定值 `k` 的子数组数量。
    例如：
    ```
    N = 3
    array -> 1, 2, 3
...
我提到了 `Dijkstra` 斐波那契算法。此外，他还要求我证明这个代码的准确性。
是的，他似乎对我印象深刻。
