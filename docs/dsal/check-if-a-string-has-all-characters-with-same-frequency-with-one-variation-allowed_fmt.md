...
> 原文: [https://www.geeksforgeeks.org/check-if-a-string-has-all-characters-with-same-frequency-with-one-variation-allowed/](https://www.geeksforgeeks.org/check-if-a-string-has-all-characters-with-same-frequency-with-one-variation-allowed/)

给定一串小写字母，通过删除 1 或 0 个字符来查找它是否可以转换为有效字符串。“有效”字符串是字符串，这样对于中的所有不同字符，每个这样的字符在其中出现的次数都相同。
...
*   使用 `Counter()` 功能计算所有字符的频率。
*   将这些频率转换到列表中。
*   使用 `Counter()` 再次计算此列表的频率。
*   如果 `Counter()` 的长度为 1，则返回真。
*   如果 `Counter()` 的长度是 2，如果最小值是 1，则返回真。
*   否则返回假。
...
本文由 **Nishant_singh(pintu)** 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用`write.geeksforgeeks.org`写一篇文章或者把你的文章邮寄到`review-team@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
