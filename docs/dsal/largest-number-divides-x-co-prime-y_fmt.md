...
> 原文: [https://www.geeksforgeeks.org/largest-number-divides-x-co-prime-y/](https://www.geeksforgeeks.org/largest-number-divides-x-co-prime-y/)

给定两个正数 `x` 和 `y`，求最大整数 `a`，这样:
...
在这里，首先我们将通过找到 `x` 和 `y` 的最大公约数(`gcd`)并将 `x` 除以 `gcd` 来从 `x` 中移除 `x` 和 `y` 的公因数。
数学上:
