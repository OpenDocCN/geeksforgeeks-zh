...
> 原文: [https://www.geeksforgeeks.org/queries-to-calculate-sum-of-the-path-from-root-to-a-given-node-in-given-binary-tree/](https://www.geeksforgeeks.org/queries-to-calculate-sum-of-the-path-from-root-to-a-given-node-in-given-binary-tree/)

给定一个根在节点 `1` 的[无限完全二叉树](https://www.geeksforgeeks.org/binary-tree-set-3-types-of-binary-tree/)，其中每个 `i` 节点有两个子节点，值为 `2 * i` 和 `2 * (i + 1)`。给定另一个由 `N` 个正整数组成的[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `arr[]`，每个数组元素 `arr[i]` 的任务是找出从根节点到节点 `arr[i]` 的路径中出现的节点值之和。
