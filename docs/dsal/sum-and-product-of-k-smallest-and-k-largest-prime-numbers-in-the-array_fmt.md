...
> 原文：[`https://www.geeksforgeeks.org/sum-and-product-of-k-smallest-and-k-largest-prime-numbers-in-the-array/`](https://www.geeksforgeeks.org/sum-and-product-of-k-smallest-and-k-largest-prime-numbers-in-the-array/)
...
`时间复杂度`:`O(N * log(N)+max_val * log(log(max_val)))`其中`N`为元素总数，`max_val`为数组中的最大值。
`辅助空间`:`O(N + max_val)`