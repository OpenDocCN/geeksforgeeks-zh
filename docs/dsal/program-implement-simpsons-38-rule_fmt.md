...
> 原文: [https://www.geeksforgeeks.org/program-implement-simpsons-38-rule/](https://www.geeksforgeeks.org/program-implement-simpsons-38-rule/)

写一个程序来实现辛普森的 3/8 法则。
辛普森一家的 3/8 法则是由托马斯·辛普森提出的。该方法用于执行数值积分。这种方法一般用于定积分的数值逼近。这里，抛物线用于逼近曲线的每个部分。

**辛普森的 3/8 公式:**
积分结果等于 `(3h/8)` 乘以 `[F(a) + 3F((2a+b)/3) + 3F((a+2b)/3) + F(b)]`。
这里，
`h` 是区间大小，由 `h = (b–a)/n` 给出。
`n` 是区间数量。
