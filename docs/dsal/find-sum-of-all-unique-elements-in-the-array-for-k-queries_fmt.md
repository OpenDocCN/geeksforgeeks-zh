...
> 原文：[https://www.geeksforgeeks.org/find-sum-of-all-unique-elements-in-the-array-for-k-queries/](https://www.geeksforgeeks.org/find-sum-of-all-unique-elements-in-the-array-for-k-queries/)

给定一个数组`arr[]`，其中最初所有元素都是`0`，和另一个数组`Q[][]`包含`K`个查询，其中每个查询代表一个范围`[L, R]`，任务是将`1`添加到每个由范围`[L, R]`定义的子数组中，并返回所有**唯一**元素的总和。
