...
> 原文：[https://www.geeksforgeeks.org/find-min-and-max-values-among-all-maximum-leaf-nodes-from-all-possible-binary-max-heap/](https://www.geeksforgeeks.org/find-min-and-max-values-among-all-maximum-leaf-nodes-from-all-possible-binary-max-heap/)

给定一个正整数 `N`，任务是从每个可能的[二进制最大堆](https://www.geeksforgeeks.org/binary-heap/)的最大叶节点中找到最大和最小的元素，该最大堆是以第一个 `N` 自然数作为二进制最大堆的节点值而形成的。
...
**天真方法：** 最简单的方法是[生成所有可能的最大二进制堆](https://www.geeksforgeeks.org/number-ways-form-heap-n-distinct-integers/)，该堆可以首先由 `N` 自然数形成，并跟踪所有堆中所有最大叶节点中最小和最大的叶节点值。
...
*   如上所述，计算最大堆 `N` 个节点中叶节点的[计数，并将其存储在一个变量中，比如 `numberOfLeaves`](https://www.geeksforgeeks.org/height-complete-binary-tree-heap-n-nodes/)。
...
*   [如果 `N+1` 不是 2 的幂](https://www.geeksforgeeks.org/program-to-find-whether-a-no-is-power-of-two/)且 `D` 大于 `1`，那么在深度 `D-1` 处必须存在一个叶节点。因此，然后将 `D` 减 `1`。
*   最后，完成上述步骤后，将最大值打印为 `(N-D)`，最小值打印为 `numberOfLeaves`。
