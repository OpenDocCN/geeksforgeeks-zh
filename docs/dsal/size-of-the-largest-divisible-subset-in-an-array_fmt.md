...
> 原文: [https://www.geeksforgeeks.org/size-of-the-largest-divisible-subset-in-an-array/](https://www.geeksforgeeks.org/size-of-the-largest-divisible-subset-in-an-array/)
...
1.  让我们把所有的数字按升序排列。
2.  注意集合 `X = {x_i, ..., x_k}` 是可接受的，如果 `x_i` 整除 `x_{i+1}` (1 ≤ i ≤ k–1)。
3.  因此，`dp[x]` 等于从数字 `x` 开始的最长合适的递增子序列的长度。
4.  状态转移关系：`dp[x] = max(dp[x], 1 + dp[y])` 如果 `x` 整除 `y`。
