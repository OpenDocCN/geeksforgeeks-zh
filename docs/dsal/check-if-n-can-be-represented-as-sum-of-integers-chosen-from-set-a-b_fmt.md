...
给定三个整数 `N`、`A` 和 `B`，任务是找出 `N` 是否可以表示为 `A` 的和 `B` 的之和。
...
一个有效的解决方法是调用以零开始的递归函数(因为零总是可能的)。如果函数调用是 `fun(x)` 那么递归调用 `fun(x + a)` 和 `fun(x + b)` (因为如果 `x` 是可能的，那么 `x + a` 和 `x + b` 也是可能的)。如果 `x > n`，则退出该功能。
