...
[渐近符号](https://www.geeksforgeeks.org/analysis-of-algorithms-set-3asymptotic-notations/)
...
*例:*
如果 `f(n) = n^2` 和 `g(n) = n^2` 那么 `f(n)=θ(n^2)` 和 `g(n)=θ(n^2)`

***证明:***

*   ***必要部分:***
    `f(n)=θ(g(n))` ⇒ `g(n)=θ(f(n))`
    根据θ的定义，存在正常数 `c1`，`c2`，使得 `c1.g(n) ≤ f(n) ≤ c2.g(n)` 对于所有 `n≥n0`
    `g(n)≤(1/c1).f(n)` 和 `g(n) ≥ (1/c2).f(n)`
    `(1/c2).f(n) ≤ g(n) ≤ (1/c1).f(n)`
    由于 `c1` 和 `c2` 是正常数，`1/c1` 和 `1/c2` 定义得很好。因此，根据θ的定义，`g(n)=θ(f(n))`
*   ***充分性部分:***
    `g(n)=θ(f(n))` ⇒ `f(n)=θ(g(n))`
    根据θ的定义，存在正常数 `c1`、`c2`，使得 `c1.f(n) ≤ g(n) ≤ c2.f(n)` 对于所有 `n≥n0`
...
*例:*
如果 `f(n) = n`，`g(n) = n^2` 和 `h(n)= n^3`
`n` 是 `O(n^2)` 而 `n^2` 是 `O(n^3)` 那么 `n` 就是 `O(n^3)`

***证明:***
存在正常数 `c1`，`c2` 使得对于所有 `n ≥ n0` 的 `f(n)≤c1.g(n)`
`f(n)≤c1.g(n)`
`g(n)≤c2.h(n)`
`f(n)≤c1.c2.h(n)`
...
*例:*
若 `f(n) = n` 且 `g(n) = n^2` 则 `n` 为 `O(n^2)` 且 `n^2` 为 `ω(n)`

***证明:***

*   ***必要部分:***
    `f(n)= O(g(n))` ⇒ `g(n)=ω(f(n))`
    根据 Big-Oh (`O`)的定义 `f(n) ≤ c.g(n)` 对于某些正常数 `c`
    `g(n)≥(1/c).f(n)`
    根据ω(`ω`)的定义，`g(n)=ω(f(n))`
*   ***充分部分:***
    `g(n)=ω(f(n))` ⇒ `f(n)= O(g(n))`
    根据ω(`ω`)的定义，对于某些正常数 `c` `g(n)≥c.f(n)`
    `f(n)≤(1/c).g(n)`
...
因为这些性质适用于渐近表示法，所以可以在函数 `f(n)` 和 `g(n)` 与两个实数 `a` 和 `b` 之间进行类比。
*   `g(n) = O(f(n))` 类似于 `a ≤ b`
*   `g(n)=ω(f(n))` 类似于 `a ≥ b`
*   `g(n)=θ(f(n))` 类似于 `a = b`
*   `g(n) = o(f(n))` 类似于 `a < b`
...
`≤(c1+c2)g(n)`
`≤c.g(n)` 其中 `c = c1+c2`
因此，`O(f(n)) + O(g(n)) = O(g(n)) = O(max(f(n), g(n)))`

## 注

1.  如果 `lim_{n→∞} f(n)/g(n)= c`，`c ∈ R+` 则 `f(n)=θ(g(n))`
2.  如果 `lim_{n→∞} f(n)/g(n)≤c`，`c ∈ R` (`c` 可以为 0) 那么 `f(n) = O(g(n))`
3.  如果 `lim_{n→∞} f(n)/g(n)= 0`，那么 `f(n) = O(g(n))` 和 `g(n) = ω(f(n))`
4.  如果 `lim_{n→∞} f(n)/g(n)≥c`，`c ∈ R` (`c` 可以是 ∞) 那么 `f(n)=ω(g(n))`
5.  如果 `lim_{n→∞} f(n)/g(n)= ∞`，那么 `f(n)=ω(g(n))` 和 `g(n)=o(f(n))`