...
[意大利面堆栈](http://en.wikipedia.org/wiki/Spaghetti_stack)是一种 N 元树数据结构，其中子节点有指向父节点的指针（但不是相反）。
...
资料来源：[`http://en.wikipedia.org/wiki/Spaghetti_stack`](http://en.wikipedia.org/wiki/Spaghetti_stack)