...
在数学中，[古尔德序列](https://en.wikipedia.org/wiki/Gould%27s_sequence)是一个整数序列，其第 `n` 项表示帕斯卡三角形第 `n-1` 行中奇数的计数。这个序列仅由 2 的幂组成。
...
一个更好的解决方案(使用动态规划)是基于这样的观察，即在前面两项的每一次幂都翻倍之后。
例如
...
因此，我们可以在 2 <sup>i</sup> 之后，通过将先前项的值加倍来计算古尔德的序列项。以下是上述方法的实现-
