...
> 原文：[https://www.geeksforgeeks.org/convert-string-palindrome-string-changing-one-character/](https://www.geeksforgeeks.org/convert-string-palindrome-string-changing-one-character/)
...
思路很简单，我们用 `str[n-i-1]` 检查 `str[i]`。如果有不匹配，我们增加 `count`。如果不匹配的 `count` 超过 1，我们返回 `false`。否则我们返回 `true`。
