...
> 原文：[https://www.geeksforgeeks.org/sort-the-sides-of-triangle-on-the-basis-of-increasing-area/](https://www.geeksforgeeks.org/sort-the-sides-of-triangle-on-the-basis-of-increasing-area/)

给定一个由 `N` 个三角形的边组成的数组 `arr[]`，任务是根据面积的递增顺序对三角形的给定边进行排序。
...
**方法：** 给定的可以通过将三角形的区域的边存储在另一个数组中，然后按照存储区域的递增顺序对数组进行排序，然后打印存储在另一个数组中的边作为结果来解决。
...
上述方法也可以在空间方面进行优化，思路是利用比较器功能对给定数组按照面积递增顺序进行排序。下面是使用的比较器功能：
