...
> 原文: [https://www.geeksforgeeks.org/check-possible-make-array-equal-doubling-tripling/](https://www.geeksforgeeks.org/check-possible-make-array-equal-doubling-tripling/)
...
这是如何工作的？我们知道每一个整数都可以表示为质数`2^a * 3^b * 5^c * 7^d ...`。所以，在我们的问题中，我们可以通过加倍(`*2`)或三倍(`*3`)来增加 `a` 和 `b`。我们可以通过乘以 2 或 3 使数组中所有元素的 `a` 和 `b` 相等。但是这些数在它们的乘积表示中也有其他质数，我们不能改变它们的幂。所以为了使所有的数相等，它们应该从一开始就对其他质数具有相等的幂。我们可以通过将所有的数字尽可能多地除以二或三来检查它。那么他们都应该是平等的。
