...
> 原文：[https://www.geeksforgeeks.org/python-program-for-quicksort-on-singly-linked-list/](https://www.geeksforgeeks.org/python-program-for-quicksort-on-singly-linked-list/)
...
在`quick_sort_recursive()`中，我们首先调用 `partition()`将 `pivot` 放在正确的位置并返回 `pivot`。在枢轴被放置在正确的位置之后，我们找到左侧（枢轴之前的列表）的尾部节点，并为左侧列表重现。最后，我们重现正确的列表。
