# 构建从 x^1 到 x^n 获得的数值的数字频率数组

> 原文: [https://www.geeksforgeeks.org/construct-a-frequency-array-of-digits-of-the-values-obtained-from-x1-x2-xn/](https://www.geeksforgeeks.org/construct-a-frequency-array-of-digits-of-the-values-obtained-from-x1-x2-xn/)

给定两个整数(`x` 和 `n`)。任务是找到一个数组，使它包含出现在(`x^1`，`x^2`，…，`x^(n-1)`，`x^n`)的数字的频率。
