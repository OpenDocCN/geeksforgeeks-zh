...
制作一个 `m` 大小的数组，并用零初始化它。对于每个范围 `{L，R}` 做 `array[L]++` 和 `array[R+1]–`。现在，在取 `sum` 的同时迭代数组，如果 `sum = x` 在索引 `I` 处，这表示数字 `I` 在 `sum` 范围内，例如，如果索引 `2` 的 `sum = 3` 的值表示数字 `2` 在 `3` 范围内。因此，当总和为 `0` 时，表示给定的数字不在任何给定的范围内，打印这些数字。
