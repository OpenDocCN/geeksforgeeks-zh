...
> 原文: [https://www.geeksforgeeks.org/subset-array-sum-by-generating-all-the-subsets/](https://www.geeksforgeeks.org/subset-array-sum-by-generating-all-the-subsets/)

给定一个大小为 `N` 的数组和一个和，任务是检查一些数组元素是否可以加起来和 `N` 相加。

## 注意

至少要包含一个元素才能组成和。(即总和不能为零)
...
思想是使用[生成所有子集，生成数组](https://www.geeksforgeeks.org/subarraysubstring-vs-subsequence-and-programs-to-generate-them/)的所有子序列，并相应地检查是否有任何子序列的和等于给定的和。
