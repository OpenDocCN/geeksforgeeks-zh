...
来到[朱莉娅](https://www.geeksforgeeks.org/julia-language-introduction/)，朱莉娅的神奇之处之一就是它的类型系统。这是一种严格输入的语言。在 Julia 中，数组可以包含同构`[1, 2, 3]`或异构类型`[1, 2.5, "3"]`的值。默认情况下，Julia 会尝试将这些值提升为常见的具体类型。如果 Julia 不能提升包含的类型，结果数组将是抽象类型`Any`。
...
所以可以说一个`Array{Any}` 相当于一个 Matlab 单元数组。它可以保存不同数据类型的元素。
