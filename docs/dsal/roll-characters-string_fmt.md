...
给定一个字符串 `s` 和一个数组 `roll`，其中 `roll[i]` 表示字符串中滚动的第一个 `roll[i]` 字符。我们需要在弦上应用每一个 `roll[i]` 并输出最终的弦。滚动意味着增加字符的 ASCII 值，如滚动“z”会得到“a”，滚动“b”会得到“c”等。
...
简单方法: 继续为每一卷改变字符串的字符 `roll[i]`
时间复杂度 = O(卷数组的大小 * 字符串的长度) = O(n^2)
