...
> 原文: [https://www.geeksforgeeks.org/check-if-a-matrix-is-reverse-bitonic-or-not/](https://www.geeksforgeeks.org/check-if-a-matrix-is-reverse-bitonic-or-not/)

给定一个矩阵 `m[][]`，任务是检查给定的矩阵是否为[反向双调和](https://www.geeksforgeeks.org/count-of-all-possible-reverse-bitonic-subarrays/)。如果给定的矩阵是`反向双调和`，则打印`是`。否则，打印`否`。
...
**输出:** 否
**说明:**
由于列 `{1, 4, 2}` 不满足三个条件中的任何一个，因此给定的矩阵不是反向双调和。

## 方法

按照以下步骤解决问题:
*   逐个检查矩阵每行的元素，看是否形成[反向双调和](https://www.geeksforgeeks.org/count-of-all-possible-reverse-bitonic-subarrays/)序列。如果发现任何一行不是`反向双调和`，打印`否`。
*   同样，逐个检查每一列的元素，看其是否形成[反向双调和](https://www.geeksforgeeks.org/count-of-all-possible-reverse-bitonic-subarrays/)序列。如果发现任何一列不是`反向双调和`，打印`否`。
*   如果发现所有的行和列都是`反向双调和`，则打印`是`。
