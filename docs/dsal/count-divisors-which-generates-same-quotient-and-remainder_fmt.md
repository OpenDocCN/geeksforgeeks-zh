...
> 原文：[https://www.geeksforgeeks.org/count-divisors-which-generates-same-quotient-and-remainder/](https://www.geeksforgeeks.org/count-divisors-which-generates-same-quotient-and-remainder/)

给定一个正整数 `N`，任务是找出所有数字 `M` 的计数，使得当数字 `N` 除以 `M` 时，商等于其余数，即 `(⌊N/M⌋ = N mod M)`，其中 `⌊ ⌋` 表示给定数字的向下取整值。
...
最简单的方法是基于这样一个事实：`M` 不能大于 `N`。至于大于 `N` 的任何数，商都为零，而其与 `N` 的模永远是 `N`。因此，迭代从 `1` 到 `N` 的所有数字，并计算所有满足给定条件的数字。
...
*   找到 `N` 的所有因子，并将其存储在数组中。这可以用 `O(√N)` 复杂度来计算。
*   通过迭代数组检查所有除数，如果除数减 `1` 满足给定条件 `⌊N / M⌋ = N mod M`，则增加计数。
