...
> 原文：[`https://www.geeksforgeeks.org/search-in-a-sorted-2d-matrix-stored-in-row-major-order/`](https://www.geeksforgeeks.org/search-in-a-sorted-2d-matrix-stored-in-row-major-order/)
...
任务是找出整数`K`是否存在于矩阵中。如果存在，则打印`Found`，否则打印`Not found`。
...
我们已经讨论了排序矩阵中[搜索元素的一个实现](https://www.geeksforgeeks.org/search-element-sorted-matrix/)。这篇文章提供了一个更好的实现。

**思路：** 思路是用各个击破的方式解决这个问题。
