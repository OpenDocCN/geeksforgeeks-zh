...
> 原文：[https://www.geeksforgeeks.org/smallest-substring-occurring-only-once-in-a-given-string/](https://www.geeksforgeeks.org/smallest-substring-occurring-only-once-in-a-given-string/)

## 问题描述

给定一个由 `N` 个小写字母组成的字符串 `S`，任务是在 `S` 中找到最小的子字符串的长度，其出现时间正好是 `1`。
...
解决问题的思路是生成给定字符串 `S` 的所有可能的子串，并将每个子串的频率存储在 `HashMap` 中。现在，遍历 `HashMap`，打印最小长度的子串，其频率为 `1`。
