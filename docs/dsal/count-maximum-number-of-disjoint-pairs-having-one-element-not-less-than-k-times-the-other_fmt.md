...
> 原文:[https://www.geeksforgeeks.org/count-maximum-number-of-disjoint-pairs-having-one-element-not-less-than-k-times-the-other/](https://www.geeksforgeeks.org/count-maximum-number-of-disjoint-pairs-having-one-element-not-less-than-k-times-the-other/)
...
2.  将两个变量 `i` 和 `j` 分别初始化为 `0` 和 `(N / 2)`，并将变量`count`存储成对结果的最大计数。
