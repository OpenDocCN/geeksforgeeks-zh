...
一个**天真的解决方案**是首先计算 `pow = pow(2, n)`，然后用 `pow % 10` 找到 `pow` 的最后一位数字。这个解决方案是低效的，并且对于稍大的 `n` 也有一个整数算术问题。
...
我们能把它推广到任何输入数吗？大数请参考 [查找 a^b 最后一位数字](https://www.geeksforgeeks.org/find-last-digit-of-ab-for-large-numbers/)。