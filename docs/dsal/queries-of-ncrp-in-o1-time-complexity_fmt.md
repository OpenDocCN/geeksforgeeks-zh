...
> 原文: [https://www.geeksforgeeks.org/queries-of-ncrp-in-o1-time-complexity/](https://www.geeksforgeeks.org/queries-of-ncrp-in-o1-time-complexity/)
...
一种**天真的方法**是通过在任何时候应用模块化运算，使用[公式](https://www.geeksforgeeks.org/program-calculate-value-ncr/)计算 `nCr`。因此时间复杂度为 `O(q * n)`。

一个**更好的方法**是使用[费马小定理](https://www.geeksforgeeks.org/fermats-little-theorem/)。根据它，`nCr` 也可以写成 `(n!/(r!*(n-r)!)) mod p` 相当于 `(n! * inverse(r!) * inverse((n-r)!)) mod p`。因此，预计算从 1 到 `n` 的数字的阶乘将允许以 `O(log n)` 回答查询。唯一需要做的计算就是计算 `r!` 的倒数和 `(n-r)!` 的倒数。因此整体复杂性将为 `O(q * log(n))`。
...
由于 `N` 的最大值是 `10^6`，预计算到 `10^6` 就可以了。
