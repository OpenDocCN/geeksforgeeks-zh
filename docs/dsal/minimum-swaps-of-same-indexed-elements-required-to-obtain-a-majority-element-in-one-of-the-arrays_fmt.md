...
> 原文: [https://www.geeksforgeeks.org/minimum-swaps-of-same-indexed-elements-required-to-obtain-a-majority-element-in-one-of-the-arrays/](https://www.geeksforgeeks.org/minimum-swaps-of-same-indexed-elements-required-to-obtain-a-majority-element-in-one-of-the-arrays/)
...
> **解释:**
> 以下是所需的交换:
> **交换 1:** 交换 `arr[1]` 和 `brr[1]` 将 `arr[]` 修改为 `{3, 2, 3, 4, 9}`，将 `brr[]` 修改为 `{5, 5, 1, 5, 3}`。
...
*   使用变量 `i` 遍历数组 `crr[]`，并执行以下操作:
    ...
    *   如果地图 `A` 和 `B` 中 `crr[i]` 的频率之和至少为 `N/2`，则更新 `res` 至 `res` 和 `min(N/2, A[crr[i]])` 的最小值。
