...
> 原文: [https://www.geeksforgeeks.org/area-of-circle-inscribed-in-a-isosceles-trapezoid/](https://www.geeksforgeeks.org/area-of-circle-inscribed-in-a-isosceles-trapezoid/)

给定等腰梯形 `ABCD` 的两个底为 `a` 和 `b`，任务是找到内接在该梯形中的圆的面积。
...
给定一个内接于梯形 `ABCD`（边 `AB = n`，`CD = m`）的圆，我们需要求出梯形的高度（即 `AL`），它是圆半径的一半，才能求出圆的面积。
...
3.  现在在三角形 `ACL` 中应用[勾股定理](https://www.geeksforgeeks.org/find-the-hypotenuse-of-a-right-angled-triangle-with-given-two-sides/)。
    ...
    ```
4.  因此梯形的高度 `AL = Square_root(m * n)`。
5.  现在圆的半径只是高度的一半，因此面积很容易计算。

## 算法

1.  求梯形的高度为 `Square_root(m * n)`。
2.  求内圆半径
    ```
    R = height / 2 
      = square_root(m * n) / 2
