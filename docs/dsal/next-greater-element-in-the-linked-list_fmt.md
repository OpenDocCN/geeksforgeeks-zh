...
朴素的方法是遍历链表`L`，对于链表中的每个元素，通过遍历当前元素的整个字符串，可以找到列表中的下一个更大的元素 。

**时间复杂度**：`O(N ^ 2)`

## 高效方法

可以优化上述朴素的方法来，通过维护遍历过的元素的单调递减[栈](http://www.geeksforgeeks.org/stack-data-structure/)。 如果找到更大的元素，则将其附加到结果链表`L`上，否则将`0`附加。 步骤如下：
