...
> 原文: [https://www.geeksforgeeks.org/sort-permutation-of-n-natural-numbers-using-triple-cyclic-right-swaps/](https://www.geeksforgeeks.org/sort-permutation-of-n-natural-numbers-using-triple-cyclic-right-swaps/)
...
**方法:** 思路是遍历数组，找到数组中不在实际排序位置的元素，可以通过 `if (arr[i] != i)` 进行检查。因为数组中只有 N 个自然元素。最后，找出数组中需要的奇数长度循环旋转，得到数组的排序形式。如果需要任何偶数长度的循环旋转，那么就不可能对数组的元素进行排序。
