...
> 原文：[https://www.geeksforgeeks.org/number-of-subsequences-with-even-and-odd-sum/](https://www.geeksforgeeks.org/number-of-subsequences-with-even-and-odd-sum/)
...
**天真方法：**
一个简单的方法是递归生成所有可能的子序列，用偶数和计数子序列的数量，然后从总子序列中减去，这个数量将是奇数子序列。这种方法的时间复杂度将是 $O(2^{N})$。

**更好的方法：**
更好的方法是使用 `动态编程`。

*   当我们遍历数组时，我们会计算偶数子序列的计数。我们创建了两个数组 `countODD[N]` 和 `countEVEN[N]`，其中 `countODD[i]` 表示范围 $(0, i)$ 内奇数和的子序列数，`countEVEN[i]` 表示范围 $(0, i)$ 内偶数和的子序列数。
*   如果我们在位置 `i`，并且数是*奇数*，那么具有偶数和的子序列的总数将是
    *   对于 `countEVEN[i]`，第 `i` 个数字不与任何其他子序列配对（即直到 $(i-1)$ 位置的偶数子序列）**+** 第 `i` 个数字与所有其他奇数子序列配对，直到 $(i-1)$ 位置（奇数+奇数=偶数）
    *   对于 `countODD[i]`，第 `i` 个数字不与任何其他子序列配对（即奇数子序列直到 $(i-1)$ 位置）**+** 第 `i` 个数字与所有其他偶数子序列配对直到 $(i-1)$ 位置（奇数+偶数=奇数）**+** 一个只有 1 个元素的子序列，即第 `i` 个数字本身
*   如果我们在位置 `i`，并且数是*偶数*，那么具有偶数和的子序列的总数将是
    *   对于 `countEVEN[i]`，第 `i` 个数不与任何其他子序列配对（即直到 $(i-1)$ 位置的偶数子序列）**+** 第 `i` 个数与所有其他偶数子序列配对，直到 $(i-1)$ 位置（偶数+偶数=偶数）**+** 一个只有 1 个元素的子序列，即第 `i` 个数本身
    *   对于 `countODD[i]`，第 `i` 个数不与任何其他子序列配对（即奇数子序列直到 $(i-1)$ 位置）**+** 第 `i` 个数与所有其他奇数子序列配对直到 $(i-1)$ 位置（偶数+奇数=奇数）
...
**高效的方法:**
我们不需要制作 `countEVEN[N]` 和 `countODD[N]` 数组，我们只需要 `countEVEN` 变量和 `countODD` 变量，并像前面一样进行更改。
以下是上述方法的实施：
