...
> 原文: [https://www.geeksforgeeks.org/form-largest-number-using-one-swap-operation/](https://www.geeksforgeeks.org/form-largest-number-using-one-swap-operation/)
...
创建一个数组 `rightMax[]`。`rightMax[i]` 包含位于 `num[i]` 右侧且大于 `num[i]` 的最大数字的索引。如果不存在这样的数字，则 `rightMax[i] = -1`。现在，遍历 `rightMax[]` 数组，从 `i = 0` 到 `n-1`（其中 `n` 是 `num` 中的总位数），找到第一个有 `rightMax[i] != -1` 的元素。执行 `swap(num[i], num[rightMax[i]])` 操作并断开。
