...
> 原文：[https://www.geeksforgeeks.org/number-of-ways-of-choosing-k-equal-substrings-of-any-length-for-every-query/](https://www.geeksforgeeks.org/number-of-ways-of-choosing-k-equal-substrings-of-any-length-for-every-query/)

给定一个字符串`str`和`Q`个查询。每个查询由一个整数`K`组成。任务是为每个查询找到选择任意长度的相等子串的方法数量。注意，`K`个子串的集合必须唯一。

**举例：**

> **输入：** `str = "aabaab"`，`que[] = {3}`
> **输出：** 4
> “a”是唯一出现 3 次以上的子串，即 4。
...
*   对于每个查询，答案将是选择出现超过`K`次的每个字符串的`K`个子字符串（比如`X`）。答案将是`C(X, K)`。
