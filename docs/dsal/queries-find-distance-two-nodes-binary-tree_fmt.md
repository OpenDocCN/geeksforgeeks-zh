...
2.  通过存储[二叉树的欧拉游程](https://www.geeksforgeeks.org/euler-tour-binary-tree/)到数组并借助每个节点的层次和欧拉游程计算另外两个数组，在`O(logn)`时间内找到二叉树中两个节点的LCA。
    这些步骤说明如下：
    （I）首先，找到[二叉树](https://www.geeksforgeeks.org/euler-tour-binary-tree/)的Euler游程。
    ![](img/2708b1df3a48f61766a2991e20a9827a.png)
    （II）然后，将每个节点的层次存储在Euler数组中。
    ![](img/46c3681a944153e098ee85c4a7d527ff.png)
    （III）然后，将二叉树所有节点的第一个匹配项存储在Euler数组中。`H`存储来自Euler数组的节点的索引，因此可以最小化查找最小值的查询范围，并且可以通过进一步优化查询时间来最小化它们的查找范围。
    ![](img/df28d4f97f3049a97ec62a66b102db30.png)
3.  然后`在L数组上构建稀疏表`，并找到范围（`H[A]到H[B]`）内的最小值，记为X。然后，`我们使用值X的索引作为Euler数组的索引来获取LCA`，即`Euler[index(X)]`。
    设`A = 8`，`B = 5`。
    （I）`H[8] = 1`和`H[5] = 2`。
    （II）当`X = 0`，`index = 7`时，`L`数组中的最小值在1和2之间，然后，`LCA = Euler[7]`，即`LCA = 1`。
