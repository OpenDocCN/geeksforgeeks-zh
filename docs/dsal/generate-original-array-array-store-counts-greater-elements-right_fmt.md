...
> 原文：[https://www.geeksforgeeks.org/generate-original-array-array-store-counts-greater-elements-right/](https://www.geeksforgeeks.org/generate-original-array-array-store-counts-greater-elements-right/)

给定一个`greater[]`的整数数组，其中数组的每个值都表示在未知数组`arr[]`中有多少元素大于其右侧的元素。我们的任务是生成原始数组`arr[]`。可以假设原始数组包含范围从 1 到 n 的元素，并且所有元素都是唯一的。
...
我们考虑一个元素数组`temp[] = {1, 2, 3, 4, ..n}`。我们知道`greater[0]`的值表示元素计数大于`arr[0]`。我们可以观察到`temp[]`的第`(n–greater[0])`个元素可以放在`arr[0]`处。所以我们将它放在`arr[0]`处，并从`temp[]`中删除它。我们对剩余元素重复上述过程。对于每一个`greater[i]`的元素，我们将`temp[]`的第`n–greater[i]–i`个元素放入`arr[i]`中，并将其从`temp[]`中移除。
...
**时间复杂度：** O(n<sup>2</sup>)（擦除操作在向量中为 O(n)）