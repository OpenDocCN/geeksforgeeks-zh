...
> 原文：[https://www.geeksforgeeks.org/count-array-elements-having-sum-of-digits-equal-to-k/](https://www.geeksforgeeks.org/count-array-elements-having-sum-of-digits-equal-to-k/)
...
*   初始化一个变量，比如说 `count`，来存储具有等于 `K` 的[数字总和](https://www.geeksforgeeks.org/program-for-sum-of-the-digits-of-a-given-number/)的元素。
*   声明一个函数 `sumOfDigits()` 来[计算一个数字的位数之和](https://www.geeksforgeeks.org/program-for-sum-of-the-digits-of-a-given-number/)。
*   [遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]`，对于每个数组元素，检查位数之和是否等于 `K`。如果发现为真，则将 `count` 增加 `1`。
*   打印 `count` 的值作为所需答案。
