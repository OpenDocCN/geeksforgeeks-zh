...
> 原文：[https://www.geeksforgeeks.org/program-to-print-a-doormat-pattern-having-a-string-written-in-the-center-in-python/](https://www.geeksforgeeks.org/program-to-print-a-doormat-pattern-having-a-string-written-in-the-center-in-python/)
...
整个图案可以分为 3 个不同的部分。第一部分包含一个有三角形的图案。第二部分包含一个字符串 `"GeeksforGeeks"`，第三部分包含一个倒三角形的模式。通过使用内置的字符串对齐功能，可以轻松打印出顶部为倒三角形的图案。

1.  **[`ljust()`](https://www.geeksforgeeks.org/python-string-ljust-rjust-center/)**：此方法返回长度为 `l` 的左对齐字符串。
2.  **[`center()`](https://www.geeksforgeeks.org/string-center-python/)**：该方法返回长度为 `l` 的居中对齐字符串。
