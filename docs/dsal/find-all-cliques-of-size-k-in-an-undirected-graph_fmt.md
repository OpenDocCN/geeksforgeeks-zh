...
给定具有 `N` 个节点和 `E` 条边且值为 `K` 的无向图，任务是打印形成大小为 `K` 的所有[集团](https://en.wikipedia.org/wiki/Clique_problem)。
...
**方法**：的想法是使用`递归`解决上述问题。 找到其度大于或等于（`K-1`）的所有顶点，并检查 `K` 个顶点的哪个子集形成一个集团。 将另一条边添加到当前列表时，将检查是否通过添加该边来确定列表是否仍然是集团。
