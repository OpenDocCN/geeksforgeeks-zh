...
> 原文：[https://www.geeksforgeeks.org/minimum-time-required-to-rot-all-oranges-dynamic-programming/](https://www.geeksforgeeks.org/minimum-time-required-to-rot-all-oranges-dynamic-programming/)
...
> **输出：** `2`
> 在第一个单位时间内，除了最后一排的第一个橘子会在第二个单位时间内腐烂外，所有的橘子都会腐烂。
>
...
>
> 如果 `dp(i, j) > 0` 和 `dp(i, j) < INT_MAX` 则 `dp(i, j) = min(dp(i, j), 1 + min(dp(i + 1, j), dp(i–1, j), dp(i, j + 1), dp(i, j–1)))`
>
