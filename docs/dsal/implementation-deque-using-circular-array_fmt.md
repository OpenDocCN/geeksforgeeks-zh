...
`insertRear()`:在德格后面增加一项。
`deleteFront()`:从德清前面删除一个项目。
`deleteRear()`:从德清后方删除一个物品。
除以上操作外，还支持以下操作
`getFront()` :从队列中获取前置项。
...
像 `insertfront()`、`insertlast()`、`deletefront()`、`deletelast()` 等所有操作的时间复杂度都是 `O(1)`。

在下一个帖子中，我们将讨论使用双链表的 deque 实现。
