...
> 原文: [https://www.geeksforgeeks.org/count-numbers-from-a-given-range-that-are-not-divisible-by-any-of-the-array-elements/](https://www.geeksforgeeks.org/count-numbers-from-a-given-range-that-are-not-divisible-by-any-of-the-array-elements/)

给定一个由 `N` 个正整数和 `L` 与 `R` 组成的数组 `arr[]`，任务是找出 `[L, R]` 范围内不能被任何数组元素整除的数字的个数。
...
简单的方法是迭代给定范围 `[L, R]` 内的所有数字，对于每个数字，检查它是否可以被任何数组元素整除。如果它不能被任何数组元素整除，则增加计数。检查所有数字后，打印计数。
...
上述方法可以通过使用厄拉多塞的筛进行优化，标记一个数的所有倍数并将其存储在高效的数据结构中，比如 `Set`，它提供了几乎恒定时间的查找操作。按照以下步骤解决问题:

*   首先，对于每个数组元素，说 `arr[i]`，使用厄拉多塞的筛，将其所有小于 `R` 的倍数存储在 `Set` 中。
*   在范围 `[1, R]` 中不能被给定数组中的任何数字整除的整数数量将等于 `(R–Set的大小)`。就这样吧 `A`。
