...
> 原文: [https://www.geeksforgeeks.org/print-all-integers-that-are-sum-of-powers-of-two-given-numbers/](https://www.geeksforgeeks.org/print-all-integers-that-are-sum-of-powers-of-two-given-numbers/)

给定三个非负整数 `x`、`y` 和 `bound`，任务是打印所有强大的整数 `?` 按排序顺序 `bound`。
一个强大的整数的形式是 `x^i + y^j` 对于所有的 `i, j >= 0`。
...
**方法:** 初始化 `i = 0` 为外环，`j = 0` 为内环，如果 `x^i >= bound` 则脱离外环（因为对其加 `y` 的任何幂都会使其脱离 `bound`）。如果 `x^i + y^j > bound` 然后脱离内环，在内环的每隔一次迭代中，将 `x^i + y^j` 保存在[集合](https://www.geeksforgeeks.org/set-in-cpp-stl/)中，以保持强大整数的清晰有序列表。最后打印集合的内容。
