...
> 原文: [https://www.geeksforgeeks.org/bell-numbers-number-of-ways-to-partition-a-set/](https://www.geeksforgeeks.org/bell-numbers-number-of-ways-to-partition-a-set/)
...
以上问题的解决方案是[贝尔数](https://en.wikipedia.org/wiki/Bell_number)。
...
一个**比较好的方法**就是用[贝尔三角](https://en.wikipedia.org/wiki/Bell_triangle)。下面是前几个贝尔数的贝尔三角形示例。
...
上述解的时间复杂度为 `O(n^2)`。我们将很快讨论计算贝尔数的其他更有效的方法。

另一个可以通过贝尔数字解决的问题。
如果一个数不能被 `1` 以外的正平方数整除，则该数为[无平方数](https://en.wikipedia.org/wiki/Square-free_integer)。例如，`6` 是一个平方自由数，但是 `12` 能被 `4` 整除。
给定一个无平方数 `x`，求 `x` 的不同乘法分区的个数，乘法分区的个数为 `Bell(n)`，其中 `n` 为 `x` 的素因子个数，例如 `x = 30`，有 `3` 个素因子 `2、3、5`。所以答案是 `Bell(3)`，也就是 `5`。`5` 个分区分别是 `1 x 30、2 x 15、3 x 10、5 x 6` 和 `2 x 3 x 5`。
