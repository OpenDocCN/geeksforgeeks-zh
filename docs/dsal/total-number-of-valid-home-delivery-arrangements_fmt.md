...
> 原文：[https://www.geeksforgeeks.org/total-number-of-valid-home-delivery-arrangements/](https://www.geeksforgeeks.org/total-number-of-valid-home-delivery-arrangements/)

给定订单数量 `N`，找出订单的有效安排数量，其中订单的交付总是在订单接收之后。
...
3.  如果我们只考虑皮卡事件，对皮卡之间的安排没有限制。所以，总共可能安排 `4!`。
...
1.  对于 `N` 个订单，我们有 `Total events = 2 * N`。
2.  所以可能的排列总数是 `( 2 * N )!`。
3.  现在，每个订单只有在提货后才有效。

对于每一个 `(Pi, Di)`，我们不能改变这种安排。每个这样的订单只有一个有效的安排。所以我们需要对每个订单除以 2。所以总的有效排列是 `( 2 * N )! / 2 ^ N`。
