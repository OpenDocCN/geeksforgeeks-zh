...
> 原文: [https://www.geeksforgeeks.org/check-if-given-array-can-be-divided-into-subsequences-of-k-increasing-consecutive-integers/](https://www.geeksforgeeks.org/check-if-given-array-can-be-divided-into-subsequences-of-k-increasing-consecutive-integers/)

给定一个由 `N` 个整数和一个正整数 `K` 组成的数组 `arr[]`，任务是检查是否有可能将该数组划分为 `K` 个连续整数的递增的子序列，使得每个元素只能在单个子序列中起作用。

**例**:

> **输入:** `arr[] = {1, 2, 1, 3, 2, 3}`，`K = 3`
> **输出:** 是
> **说明:** 给定数组可分为 `{1, 2, 3}` 和 `{1, 2, 3}` 两个子序列，每个子序列都有 3 个递增顺序的连续整数。
>
> **输入:** `arr[] = {4, 3, 1, 2}`，`K = 2`
> **输出:** 否

**进场:** 以上问题可以使用贪婪进场使用二分搜索法解决。可以观察到，对于任意整数 `arr[i]`，最优选择是选择子阵 `arr[i+1, N)` 中 `arr[i] + 1` 的最小索引。根据这一观察，按照以下步骤解决给定的问题:

*   如果 `K` 不是 `N` 的除数，则不存在所需子序列的可能集合。于是，印 `否`。
*   将每个整数的索引存储在集合数据结构中。使用具有密钥集对结构的地图可以有效地存储它。
*   维护一个 `visited` 数组，以跟踪已经包含在子序列中的索引。
*   对范围 `[0, N)` 中的每个 `i` 进行迭代，如果当前索引处的整数尚未被访问，请执行以下步骤:
    *   使用 `upper_bound` 函数，在范围 `[i+1, N]` 中找到 `arr[i] + 1` 的最小索引，并用它更新当前子序列最后一个元素的值。
