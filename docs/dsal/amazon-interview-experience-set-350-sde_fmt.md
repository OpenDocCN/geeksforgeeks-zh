...
> 原文:[https://www.geeksforgeeks.org/amazon-interview-experience-set-350-sde/](https://www.geeksforgeeks.org/amazon-interview-experience-set-350-sde/)
...
1.  [相对排序](https://practice.geeksforgeeks.org/problems/relative-sorting/0)
    这个问题可以通过自定义的比较函数结合合并排序来轻松解决，这个我后来想通了。我提出了一个基于 2 个 `hashmaps` 的不同解决方案。时间复杂度为`O(n log n)`。我的方法有点复杂和冗长，所以面试官没有让我编写代码。
...
1.  [设计 LRU 缓存。](https://practice.geeksforgeeks.org/problems/lru-cache/1)
    我用`unordered_map`和双链表设计的。我编写了自己的 `DLL` 类。之后，他们设计了不同的测试用例，并要求我解释我的代码是如何处理这些用例的。
...
我用括号和记忆解决了这个问题。时间复杂度–`O(n^3)`。

我用三维数组来记忆。
`dp[i][j][0]`:使用子串`(i，j)`形成 0 的方式数
`dp[i][j][1]`:使用子串`(i，j)`形成 1 的方式数

面试官强调只使用一个二维数组，所以我必须包括以下预计算来计算长度为 `n` 的字符串括号的总方法数，比如 `F( n)`。

`F( n ) = 1 + sum(F(i)* F(n–i))`，`i` 从 1 到 `n-1` 不等。
...
1.  [用和`>= k`移除所有不在任何路径中的节点](https://www.geeksforgeeks.org/remove-all-nodes-which-lie-on-a-path-having-sum-less-than-k/)
