...
> 原文: [https://www.geeksforgeeks.org/arithmetic-expression-evalution/](https://www.geeksforgeeks.org/arithmetic-expression-evalution/)

堆栈组织在计算算术表达式时非常有效。表达式通常用所谓的中缀符号来表示，其中每个运算符都写在两个操作数之间(即 `A + B`)。使用这种符号，我们必须通过使用括号或一些运算符优先约定来区分`(A + B) * C`和`A + (B * C)`。因此，算术表达式中运算符和操作数的顺序并不能唯一确定操作的执行顺序。
