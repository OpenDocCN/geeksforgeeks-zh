...
> 原文: [https://www.geeksforgeeks.org/check-if-all-array-elements-can-be-reduced-to-less-than-x/](https://www.geeksforgeeks.org/check-if-all-array-elements-can-be-reduced-to-less-than-x/)

给定一个由 `N` 个正整数和一个整数 `X` 组成的数组 `A[]`，任务是通过执行以下操作来确定是否有可能将所有数组元素转换为小于 `X`：

*   选择 2 个不同的索引 `j` 和 `k`。
*   选择一个索引 `i`，其中 `A[i] > X`。
*   当且仅当 `gcd(A[j], A[k]) ≠ 1` 时，替换 `A[i] = gcd(A[j], A[k])`。
...
1.  找到两个分别有 `gcd ≠ 1` 和 `gcd ≤ X` 的数字，然后，通过使用这两个数字，所需的数字 `A[i]` 可以替换为 `gcd(A[j], A[k])`。
