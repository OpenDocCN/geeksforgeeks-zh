...
提问:
1。详细解释项目。
2。[使用`链表`实现`队列`](https://practice.geeksforgeeks.org/problems/implement-queue-using-linked-list/1)。
3。`单链表`和`双链表`的区别(深度分析)。
...
1.绘制流程状态图。
2。[给定一棵`二叉树`，检查是否是`二叉查找树`](https://practice.geeksforgeeks.org/problems/check-for-bst/1)(覆盖所有角例)。
