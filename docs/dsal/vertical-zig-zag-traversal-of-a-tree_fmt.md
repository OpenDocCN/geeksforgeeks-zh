...
> 原文: [https://www.geeksforgeeks.org/vertical-zig-zag-traversal-of-a-tree/](https://www.geeksforgeeks.org/vertical-zig-zag-traversal-of-a-tree/)
...
创建一个向量`tree[]`，其中`tree[i]`将存储该树在第`i`级的所有节点。取两个指针`p1`指向第一级，`p2`指向最后一级。现在，使用这两个指针以交替的方式开始打印节点(即从右向左为`p1`，从左向右为`p2`)。如果在`p1`所指的层级中没有剩余节点，则移动到下一层级，如果在`p2`所指的层级中没有剩余节点，则移动到上一层级。
