...
> 原文: [https://www.geeksforgeeks.org/queries-for-characters-in-a-repeated-string/](https://www.geeksforgeeks.org/queries-for-characters-in-a-repeated-string/)

## 问题描述

给定一个字符串 `X`。通过多次重复字符串 `X` 形成一个字符串 `S`，即多次将字符串 `X` 附加到自身。有 `Q` 个查询，每个查询包含两个索引 `i` 和 `j`。任务是判断字符串 `S` 中索引 `i` 的元素是否与索引 `j` 的元素相同，如果相同则为每个查询打印 "Yes"，否则打印 "No"。
...
设字符串 `X` 的长度为 `n`，观察索引 `0`，`n`，`2n`，`3n`，…处的元素。它们都是一样的。类似地，对于索引 `i`，位置 `i`，`n+i`，`2n+i`，`3n+i`，…..包含相同的元素。

因此，对于每个查询，查找 `(i % n)` 和 `(j % n)`，如果两者对于字符串 `X` 都相同，则说明原索引处的字符相同。
