...
> 原文：[https://www.geeksforgeeks.org/perfect-digital-invariants-number/](https://www.geeksforgeeks.org/perfect-digital-invariants-number/)

如果一个正整数的数字的某个固定幂的和等于数字本身，则称之为`完美数字不变数`。
对于任意数，`abcd… = power(a, n) + power(b, n) + power(c, n) + power(d, n) + ...`，
其中 `n` 可以是大于 0 的任意整数。
