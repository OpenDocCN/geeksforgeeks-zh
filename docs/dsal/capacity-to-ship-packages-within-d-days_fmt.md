...
> 原文: [https://www.geeksforgeeks.org/capacity-to-ship-packages-within-d-days/](https://www.geeksforgeeks.org/capacity-to-ship-packages-within-d-days/)

给定一个由表示 `N` 个项目的重量的正整数和一个正整数 `D` 组成的数组，任务是找到一艘船的最小载重量（比如说，`K`）以在 `D` 天内装运所有重量，从而使装载在船上的重量的顺序与数组元素在 `arr[]` 中的顺序一致。
...
**第 1 天:** 将第一天数值 `1` 和数值 `2` 的重量作为重量 `1 + 2 = 3` 的总和装运。
...
给定的问题可以用[贪心法](https://www.geeksforgeeks.org/greedy-algorithms/)和[二分搜索法](https://www.geeksforgeeks.org/binary-search/)来解决。问题的单调性可以观察到，如果所有的包裹都可以在 `D` 天内以 `K` 的容量成功发货，那么肯定可以以任何大于 `K` 的容量发货。按照以下步骤解决问题：

*   初始化一个变量，将 `sum` 表示为 `-1`，以存储所需船只的最小容量。
*   初始化两个变量，比如说 `s` 和 `e`，给定数组中的[最大元素](https://www.geeksforgeeks.org/c-program-find-largest-element-array/)和数组的[总和](https://www.geeksforgeeks.org/program-find-sum-elements-given-array/)分别表示搜索空间的下限和上限。
*   [迭代直到](https://www.geeksforgeeks.org/c-c-while-loop-with-examples/) `s` 的值小于或等于 `e`，并执行以下步骤：
    *   初始化一个变量，说 `mid` 为 `(s + e)/2`。
    *   检查是否有可能在 `D` 天内装运所有包裹，此时允许的最大容量为 `mid`。如果发现 `为真`，则将 `sum` 的值更新为 `mid`，将 `e` 的值更新为 `(mid - 1)`。
    *   否则，将 `s` 的值更新为 `(mid + 1)`。
...
`O(N * log(S–M))`，其中 `S` 为数组元素之和，`M` 为数组最大元素。
辅助空间: `O(1)`