...
> 原文：[https://www.geeksforgeeks.org/program-to-find-greatest-common-divisor-gcd-of-n-strings/](https://www.geeksforgeeks.org/program-to-find-greatest-common-divisor-gcd-of-n-strings/)
...
思路是用[递归](https://www.geeksforgeeks.org/recursion/)。以下是步骤：

1.  创建递归函数 `gcd(str1，str2)`。
2.  如果 `str2` 的长度大于 `str1`，那么我们将使用 `gcd(str2，str1)` 来重现。
3.  现在，如果 `str1` 不是以 `str2` 开始，那么返回一个空字符串。
