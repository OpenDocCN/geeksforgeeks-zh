...
> 原文：[https://www.geeksforgeeks.org/find-three-closest-elements-from-given-three-sorted-arrays/](https://www.geeksforgeeks.org/find-three-closest-elements-from-given-three-sorted-arrays/)
...
一个`简单的解决方案`是运行三个嵌套循环来考虑来自 `A`、`B` 和 `C` 的所有三元组，计算每个三元组的 `max(ABS(A[i]–B[j]), ABS(B[j]–C[k]), ABS(C[k]–A[i]))` 的值，并返回所有值的最小值。该解决方案的时间复杂度为 `O(n^3)`。

一个更好的解决方案是使用二分搜索法。
1) 迭代 `A[]` 的所有元素，求 `B[]`、`C[]` 中刚好小于等于的元素二分搜索法，注意区别。
2) 对 `B[]` 和 `C[]` 重复步骤 1。
3) 返回整体最小值。
该方案的时间复杂度为 `O(nLogn)`。
