...
> 原文：[https://www.geeksforgeeks.org/minimum-swaps-required-to-make-a-binary-string-alternating/](https://www.geeksforgeeks.org/minimum-swaps-required-to-make-a-binary-string-alternating/)
...
1. 计算字符串奇数位置和偶数位置的零个数。让它们的计数分别为 `odd_0` 和 `even_0`。
2. 计算字符串奇数位置和偶数位置的 1 的数量。让它们的计数分别为 `odd_1` 和 `even_1`。
3. 我们总是用 0 交换 1（永远不要用 1 交换 1，也不要用 0 交换 0）。所以我们只需要检查我们的交替字符串是否以 0 开始，那么交换的数量是最小的（`even_0`，`odd_1`），如果我们的交替字符串以 1 开始，那么交换的数量是最小的（`even_1`，`odd_0`）。答案是这两者中的最小值。
...
2. 字符串长度为偶数:
我们将计算奇数位置(`one_odd`)的 `1` 数和偶数位置(`one_even`)的 `1` 数，然后答案是 `min(N/2 - one_odd, N/2 - one_even)`
3. 字符串是奇数长度:
这里我们考虑两种情况:
I) `1` 的总数 > `0` 的总数(那么我们已经把 `1` 放在偶数位置)所以，答案是 `((N+1)/2 - even位置的1的数量)`。
ii) `0` 的总数 > `1` 的总数(那么我们已经在偶数位置放了零)所以，答案是 `((N+1)/2 - even位置的0的数量)`。
