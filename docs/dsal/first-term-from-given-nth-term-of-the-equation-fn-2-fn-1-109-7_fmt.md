# 等式 f(n)=(2 * f(n–1))% 10^9+7 的给定第 n 项的第一项

> 原文: [https://www.geeksforgeeks.org/first-term-from-given-nth-term-of-the-equation-fn-2-fn-1-109-7/](https://www.geeksforgeeks.org/first-term-from-given-nth-term-of-the-equation-fn-2-fn-1-109-7/)

给定一个整数 `N` 和一个整数 `F_N`，表示线性方程 `F(N)=(2 * F(N–1))% M`，其中 `M` 为 `10^9 + 7`，任务是求 `F(1)` 的值。
...
> `F(2)=(2 * 500000005)% M = 3`
> `F(3)=(2 * 3)% M = 6`
> 对于 `F(1)= 500000005`，给定的线性方程满足 `F(3)` 的值。
> 因此，`F(1)` 的值为 `500000005`。

## 天真法

解决这个问题最简单的方法是在 `[1，M–1]` 范围内尝试 `F(1)` 的所有可能值，检查是否有任何值满足给定的线性方程。如果发现为真，则打印 `F(1)` 的值。
...
*   在模 `M` 下计算 `2^(N–1)` 的模乘逆元，记为 `modInv`。
*   最后打印 `F(N) * modInv` 的值。
