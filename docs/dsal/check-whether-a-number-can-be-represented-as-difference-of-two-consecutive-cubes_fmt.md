...
> 原文：[https://www.geeksforgeeks.org/check-whether-a-number-can-be-represented-as-difference-of-two-consecutive-cubes/](https://www.geeksforgeeks.org/check-whether-a-number-can-be-represented-as-difference-of-two-consecutive-cubes/)

给定一个数字 `N`，任务是检查数字 `N` 是否可以表示为两个连续立方体的差。如果是，则打印这些数字，否则打印“否”。
...
问题中的关键观察是，一个数可以表示为两个连续立方体的差，当且仅当：
...
因此，如果上述条件成立，那么我们将通过检查 `(I+1)^3 - I^3 = N` 来打印数字，并打印数字 `i` 和 `i + 1`。
