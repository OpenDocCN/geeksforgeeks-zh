...
> 原文: [https://www.geeksforgeeks.org/sort-an-array-of-0s-1s-2s-and-3s/](https://www.geeksforgeeks.org/sort-an-array-of-0s-1s-2s-and-3s/)

给定一个由 **0**、**1**、**2** 和 **3** 组成的大小为 **N** 的数组 `arr[]`，任务是按照升序排列给定数组。
...
*   初始化三个变量，比如 `i`、`mid` 和 `j`。将 `i` 和 `mid` 的值设置为 `0`，将 `j` 设置为 `(N–1)`。
*   重复一个循环直到 `mid ≤ j`，并执行以下步骤:
    *   如果 `arr[mid]` 的值为 `0`，则交换 `arr[i]` 和 `arr[mid]`，并将 `i` 和 `mid` 的值增加 `1`。
    *   否则，如果 `arr[mid]` 的值为 `3`，则交换 `arr[mid]` 和 `arr[j]`，并将 `j` 减 `1`。
    *   否则，如果 `arr[mid]` 的值为 `1` 或 `2`，则将 `mid` 的值增加 `1`。
*   现在对范围 `[i, j]` 内的子数组进行排序，通过迭代直到 `i ≤ j`，并执行以下操作:
    *   如果 `arr[i]` 的值为 `2`，则用 `1` 交换 `arr[i]` 和 `arr[j]` 并将 `j` 的值减去 `1`。
    *   否则，将 `i` 的值增加 `1`。
*   完成上述步骤后，打印数组 `arr[]` 作为结果排序数组。
