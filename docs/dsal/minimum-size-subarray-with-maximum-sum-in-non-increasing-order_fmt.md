...
> 原文: [https://www.geeksforgeeks.org/minimum-size-subarray-with-maximum-sum-in-non-increasing-order/](https://www.geeksforgeeks.org/minimum-size-subarray-with-maximum-sum-in-non-increasing-order/)
...
1.  首先，我们将对数组进行排序，并定义一个名为 `A` 的向量。现在我们先计算整个数组的和，然后在连续更新 `temp` 的同时，从后面迭代整个循环。循环运行直到 `temp` 变为零。
2.  实际上，当我们从后面迭代循环时，我们考虑的是最大值。因此，我们可以说，我们可以在更短的时间内达到目标条件，也通过采取更少的变量。
3.  现在，随着循环的运行，我们继续更新 `temp` 的值，只需将 `nums[i]` 添加到其中，并将 `nums[i]` 添加到向量 `A` 中。最后我们返回向量 `A`，它代表我们想要的输出结果。
