...
> 原文：[https://www.geeksforgeeks.org/generate-an-alternate-increasing-and-decreasing-array/](https://www.geeksforgeeks.org/generate-an-alternate-increasing-and-decreasing-array/)

给定一个大小为 `N` 的字符串 `str`，该字符串仅包含两种类型的字符，即 `"I"` 或 `"D"`。任务是生成一个数组 `arr[0, 1, ... N]`，尺寸为 `N + 1`，满足以下条件：

*   如果 `str[i] == "I"`，则 `arr[i] < arr[i+1]`
*   如果 `str[i] == "D"`，则 `arr[i] > arr[i+1]`
...
> **输出：** `0 4 1 3 2`
> **解释：**
> `str[0] = "I"` 因此 `arr[0] < arr[1]`
> `str[1] = "D"` 因此 `arr[1] > arr[2]`
> `str[2] = "I"` 因此 `arr[2] < arr[3]`
> `str[3] = "D"` 因此 `arr[3] > arr[4]`
>
> **输入：** `str = "III"`
> **输出：** `0 1 2 3`
...
1.  将变量 `START` 初始化为 `0`，将 `END` 初始化为 `N`。
2.  迭代整个数组。
3.  如果 `str[i] == "I"`，则在 `arr[i]` 处分配 `START`，并增加 `START`。
4.  如果 `str[i] == "D"`，则在 `arr[i]` 处分配 `END`，并递减 `END`。
5.  现在数组的最后一个元素没有赋值，所以在 `arr[N]` 处赋值 `START`。
