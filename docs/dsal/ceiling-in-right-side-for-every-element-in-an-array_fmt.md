...
> 原文: [https://www.geeksforgeeks.org/ceiling-in-right-side-for-every-element-in-an-array/](https://www.geeksforgeeks.org/ceiling-in-right-side-for-every-element-in-an-array/)
...
一个更好的解决方案是使用排序。我们对所有元素进行排序，然后对每个元素向右遍历，直到找到一个更大的元素(请注意，一个元素可以多次出现)。

一个高效的解决方案是使用自平衡`BST`(实现为`C++`中的`set`和`Java`中的`TreeSet`)。在自平衡`BST`中，我们可以在`O(Log n)`时间内完成插入和上限操作。
