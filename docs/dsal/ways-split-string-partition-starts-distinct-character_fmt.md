...
> 原文：[https://www.geeksforgeeks.org/ways-split-string-partition-starts-distinct-character/](https://www.geeksforgeeks.org/ways-split-string-partition-starts-distinct-character/)

给定一个字符串 `s`。假设 `k` 是给定字符串中可能的最大分区数，每个分区以不同的字符开头。任务是找出将字符串 `s` 拆分成 `k` 个分区（非空）的方法，以便每个分区以不同的字符开始。
...
现在，要找到将字符串拆分成 `k` 个部分的方法，每个分区从不同的字符开始。首先，观察第一个分区将总是固定字符串的第一个字符，不管它有多长。现在，我们需要处理除了第一个字符之外的所有其他字符。
...
因此概括观察，让 `count_i` 是字符 `i` 在 `s` 中出现的次数。因此我们的答案将是所有 `i` 的 `count_i` 的乘积，这样 `i` 出现在字符串中，并且 `i` 不等于字符串的第一个字符。
