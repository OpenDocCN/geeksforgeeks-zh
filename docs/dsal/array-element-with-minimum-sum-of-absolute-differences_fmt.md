...
> 原文：[https://www.geeksforgeeks.org/array-element-with-minimum-sum-of-absolute-differences/](https://www.geeksforgeeks.org/array-element-with-minimum-sum-of-absolute-differences/)
...
一个**有效的方法：**总是选择 `x` 作为数组的中值。如果 `n` 为偶数，有**两个中位数**，那么两个中位数都是最优选择。该方法的时间复杂度为 `O(n * log(n))`，因为为了找到中间值，必须对数组进行排序。当找到 `x`（数组的中间值）时，计算并打印最小和。
