# 范围[L，R]内非常大的二进制数的异或运算

> 原文: [https://www.geeksforgeeks.org/xor-of-very-large-binary-numbers-in-range-l-r/](https://www.geeksforgeeks.org/xor-of-very-large-binary-numbers-in-range-l-r/)
...
*   `(l ^(l+1)^……^(r–1)^ r)` 可以写成 `(1^2^3 ^….^(R-1)^R) ^ (1^2^3 …..^(L-2) ^ (L-1))` 其中 `^` 表示元素的按位异或。所以问题简化为[求 1 到 `n` 的异或](https://www.geeksforgeeks.org/calculate-xor-1-n/)。
...
*   通过执行以下步骤，创建一个函数 `sub`，从大小为 `N` 的二进制字符串中减去 1:
    ...
    *   返回[字符串](https://www.geeksforgeeks.org/string-data-structure/) `S` 作为答案。
*   通过执行下面提到的步骤，创建一个函数 `ad`，将 1 添加到大小为 `N` 的二进制字符串 `S` 中:
    *   初始化一个变量，说 `carry` 为 `0`。
    *   [使用变量 `i` 在范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[N-1，0]` 中迭代，并执行以下步骤:
        *   如果 `S[i] = '1'`，则将 `S[i]` 的值修改为 `0`，将 `carry` 的值修改为 `1`。
        *   如果 `S[i] = '0'`，则将 `S[i]` 的值修改为 `1`，将 `carry` 的值修改为 `0`，终止循环。
    *   如果 `carry = 1`，则将 `S` 的值修改为 `'1' + S`，并返回字符串 `S`。
...
*   创建一个函数 `final_xor`，通过执行以下步骤计算两个二进制字符串 `L` 和 `R` 的 xor:
    ...
    *   初始化一个字符串，比如说 `ans`，它们将存储二进制字符串 `L` 和 `R` 的异或值。
