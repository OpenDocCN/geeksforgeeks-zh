...
> 原文：[https://www.geeksforgeeks.org/construct-sum-array-sum-elements-given-range/](https://www.geeksforgeeks.org/construct-sum-array-sum-elements-given-range/)

给你一个 `n` 元素数组和一个奇数 `m`。你必须从给定的数组构造一个新的 `sum_array`，使得 `sum_array[i] = Σarr[j]` 为 `(i-(m/2)) < j < (i+(m/2))`。
**注：** 为 `j < 0` 或 `j >= n` 取 `arr[j] = 0`。
...
```
calculate sum of first (m/2)+1 elements
sum_array[0] = sum
for i=1 to i<n
    if( (i-(m/2)-1) >= 0 )
        sum -= arr[(i-(m/2)-1)]
