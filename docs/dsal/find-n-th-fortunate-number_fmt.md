...
幸运数是最小的整数 `m > 1`，因此，对于给定的正整数 `n`，`p_n + m` 是素数。这里 `p_n` 是前 `n` 个素数的乘积，即 `n` 阶的素阶乘(或[素阶乘](https://www.geeksforgeeks.org/primorial-of-a-number/))例如:
...
现在，质因数 `p_n` 和大于 `p_n` 的第一个质数之间的最小差 `m` (`m>1`)是一个质数。
**例:**
