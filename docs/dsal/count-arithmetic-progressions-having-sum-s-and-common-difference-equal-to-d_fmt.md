...
> 原文: [https://www.geeksforgeeks.org/count-arithmetic-progressions-having-sum-s-and-common-difference-equal-to-d/](https://www.geeksforgeeks.org/count-arithmetic-progressions-having-sum-s-and-common-difference-equal-to-d/)
...
1.  初始化一个变量，说 `answer`，用和 `S` 和公差 `D` 存储 `AP`s 的计数。
2.  [迭代范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[1, √(2 * S)]` 并检查 `2 * S` 是否被 `i` 整除，然后执行以下步骤:
    *   如果 `((2 * S / i) + 1 - i * D)` 的值可被 `2` 整除，则用 `1` 递增 `answer`。
    *   如果 `(i * D - S / i + 1)` 的值可被 `2` 整除，则用 `1` 递增 `answer`。
3.  完成以上步骤后，打印 `answer` 的值作为 `AP`s 的结果计数。
