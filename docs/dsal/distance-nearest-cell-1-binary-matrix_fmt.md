...
给定 `N x M` 的二进制矩阵，至少包含一个值 1。该任务是为每个单元查找矩阵中最接近 1 的距离。 距离计算为 `| i1 – i2 | + | j1 – j2 |`，其中 `i1`，`j1` 是当前单元格的行号和列号，`i2`，`j2` 是具有值 1 的最近单元格的行号和列号。
...
- **时间复杂度**：`O(N^2 * M^2)`。
  对于矩阵中的每个元素，都要遍历整个矩阵，并且有 `N * M` 个元素，因此时间复杂度为 `O(N^2 * M^2)`。
...
此方法使用 [BFS 或广度优先搜索](https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/)技术获得解决方案。

### 方法
想法是使用多源广度优先搜索。将每个单元视为一个节点，并将任何两个相邻单元之间的每个边界作为一条边。从 1 到 `N * M` 编号每个单元。现在，将对应单元格值为 1 的所有节点推入队列中的矩阵。使用此队列应用 BFS，以查找相邻节点的最小距离。
...
public:
    ...
    // 创建具有 N*M 个节点的图，将每个单元格视为一个节点，每个边界视为一条边
    void createGraph()
    {
        int k = 1; // 分配给单元格的数字
...
本文由 **Anuj Chauhan** 提供。如果您喜欢 GeeksforGeeks 并希望做出贡献，则还可以使用 [contribute.geeksforgeeks.org](http://www.contribute.geeksforgeeks.org) 撰写文章，或将您的文章邮寄至 contribute@geeksforgeeks.org。查看您的文章出现在 GeeksforGeeks 主页上，并帮助其他 Geeks。
