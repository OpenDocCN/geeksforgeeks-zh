...
> 原文: [https://www.geeksforgeeks.org/find-the-final-sequence-of-the-array-after-performing-given-operations/](https://www.geeksforgeeks.org/find-the-final-sequence-of-the-array-after-performing-given-operations/)

给定一个大小为 `N` 的数组 `arr[]`，任务是精确地执行以下操作 `N` 次，创建一个整数的空列表 `b[]`，在第 `i` 次操作中，
...
数组的最后一个元素只会反转一次。倒数第二个元素将被反转两次。因此，它到达最终结果数组`b`中的最后一个位置。因此，我们可以通过从头到尾迭代原始数组，并将元素放在未填充的第一个索引和未填充的最后一个索引处来填充`b`数组。下面实现了同样的想法。
