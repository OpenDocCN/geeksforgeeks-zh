...
> 原文: [https://www.geeksforgeeks.org/path-from-the-root-node-to-a-given-node-in-an-n-ary-tree/](https://www.geeksforgeeks.org/path-from-the-root-node-to-a-given-node-in-an-n-ary-tree/)

## 问题描述

给定一个整数 `N` 和一个如下形式的 [N 元树](https://www.geeksforgeeks.org/generic-treesn-array-trees/)：

*   每个节点按顺序编号，从 `1` 开始，直到最后一级，包含节点 `N`。
*   每个奇数层的节点包含 `2` 个子节点，每个偶数层的节点包含 `4` 个子节点。
...
*   使用 [`lower_bound()`](https://www.geeksforgeeks.org/lower_bound-in-cpp/) 找到前缀和数组中大于或等于 `N` 的索引 `ind`。因此 `ind` 表示到达节点 `N` 需要经过的层数。
*   初始化一个变量，比如说，`temp = N` 和一个数组 `path[]` 来存储从根到 `N` 的节点。
*   递减 `ind` 直到其小于或等于 `1` 并继续更新 `val = temp - prefix[ind - 1]`。
*   如果 `ind` 为奇数，更新 `temp = prefix[ind - 2] + (val + 1) / 2`。
*   否则，如果 `ind` 为偶数，则更新 `temp = prefix[ind - 2] + (val + 3) / 4`。
*   将 `temp` 追加到 `path[]` 数组中。
*   最后打印数组 `path[]`。
