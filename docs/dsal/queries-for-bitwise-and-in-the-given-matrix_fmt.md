...
> 原文：[https://www.geeksforgeeks.org/queries-for-bitwise-and-in-the-given-matrix/](https://www.geeksforgeeks.org/queries-for-bitwise-and-in-the-given-matrix/)
...
> **输入：** `mat[][] = {{12, 23, 13}, {41, 15, 46}, {75, 82, 123}}`, `q[] = {{0, 0, 2, 2}, {1, 1, 2, 2}}`
> **输出：**

### 天真的方法

遍历子矩阵，找到该范围内所有数字的逐位“与”。在最坏的情况下，每次查询将花费 `O(n^2)` 时间。

### 高效的方法

如果我们把整数看作二进制数，我们很容易看到我们的答案的 `i` 位被设置的条件是子矩阵中所有整数的 `i` 位都应该被设置。

因此，我们将计算每个位的前缀计数。我们将使用这个来寻找子矩阵中设置了 `i` 位的整数个数。如果它等于子矩阵的总元素，那么我们的答案的 `i` 位也将被设置。

为此，我们将创建一个 3d 数组，`prefix_count[][][]`，其中 `prefix_count[i][x][y]` 将存储子矩阵所有元素的计数，左上角位于 `{0, 0}`，右下角位于 `{x, y}` 和 `i` 位集。参考[这篇](https://www.geeksforgeeks.org/submatrix-sum-queries/)文章了解矩阵情况下的前缀计数。
