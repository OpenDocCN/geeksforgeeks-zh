...
> 原文: [https://www.geeksforgeeks.org/sum-of-numbers-in-a-range-l-r-whose-count-of-divisors-is-prime/](https://www.geeksforgeeks.org/sum-of-numbers-in-a-range-l-r-whose-count-of-divisors-is-prime/)

给定 `Q` 个查询，其中每个查询由整数范围 `[L，R]` 组成，任务是从给定范围中找到除数为素数的整数之和。
...
**方法:** 使用[厄拉多塞的筛](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)找出每个元素的所有素数和除数，直到极限 `N`。现在创建一个前缀和数组 `sum[]`，其中 `sum[i]` 将存储范围 `[0，i]` 中的元素的和，这些元素的除数是使用前面创建的筛数组的质数。现在每个查询都可以在 `O(1)` 中回答为 `sum[r] – sum[l – 1]`。
