...
> 原文: [https://www.geeksforgeeks.org/number-from-a-given-range-that-requires-kth-smallest-number-of-steps-to-get-reduced-to-1/](https://www.geeksforgeeks.org/number-from-a-given-range-that-requires-kth-smallest-number-of-steps-to-get-reduced-to-1/)

给定三个正整数 `L`、`R` 和 `K`，任务是从范围 `[L, R]` 中找到该数，该范围要求 `Kth` 最小步数通过执行以下操作减少到 1:
...
> *   同样，`10` 步所需的步数是 `6`。
>
> 因此，来自给定范围的所有值(按升序排序)所需的步数对于值 `{8, 10, 7, 9}` 分别为 `{3, 6, 16, 19}`。因此，数字 `9` 需要第 `K` 个最小步数。
>
> **输入:** `L = 7`，`R = 10`，`K = 2`
> **输出:** `10`

**方法:** 想法是制作单独的函数来计算范围中每个数字的步数，并打印所获得的值中的最小的 `Kth`。按照以下步骤解决问题:
*   定义一个函数 `power_value()` 来计算一个数字所需的步数:
    *   基础条件: 如果 `number` 为 `1`，则返回 `0`。
    *   如果数字为偶数，则用数值 `number/2` 调用函数 `power_value()`。
    *   否则，用数值 `number * 3 + 1` 调用函数 `power_value()`。
*   现在，为了找到第 `K` 个最小步数，执行以下步骤:
    *   初始化一对的向量，比如 `ans`。
    *   遍历 `[L, R]` 范围内的数组，计算当前数 `i` 的幂值，在对 `ans` 的向量中插入一对步数和 `i`。
    *   按照该数步数的升序对向量进行排序。
*   完成上述步骤后，从开始打印 `Kth` 最小步数，即 `ans[K-1].second` 作为结果。
