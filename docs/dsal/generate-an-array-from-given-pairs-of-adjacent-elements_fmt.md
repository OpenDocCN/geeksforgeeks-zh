...
> 原文: [https://www.geeksforgeeks.org/generate-an-array-from-given-pairs-of-adjacent-elements/](https://www.geeksforgeeks.org/generate-an-array-from-given-pairs-of-adjacent-elements/)
...
使用 [**哈希**](https://www.geeksforgeeks.org/hashing-data-structure/) 和 [**DFS**](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/) 可以解决问题。按照以下步骤解决问题:

1.  使用 [地图](https://www.geeksforgeeks.org/unordered_map-in-cpp-stl/)，比如 `map` 初始化 [邻接表](https://www.geeksforgeeks.org/graph-and-its-representations/)，以存储分配给每个元素的相邻元素。
2.  初始化一个 [向量](https://www.geeksforgeeks.org/vector-in-cpp-stl/)，比如说 `res`，来存储数组中的原始元素。
3.  从角元素开始创建原始数组。因此，找到只有一个邻居的元素。这可以是原始数组的第一个 `元素` 或最后一个 `元素`。
4.  将获得的元素插入 `res`。
