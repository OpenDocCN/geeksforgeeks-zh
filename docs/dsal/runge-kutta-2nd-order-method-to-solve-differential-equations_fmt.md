...
> 原文: [https://www.geeksforgeeks.org/runge-kutta-2nd-order-method-to-solve-differential-equations/](https://www.geeksforgeeks.org/runge-kutta-2nd-order-method-to-solve-differential-equations/)
...
> **输入:** `x0 = 0`，`y0 = 1`，`x = 2`，`h = 0.2`
> **输出:** `y(x) = 0.645590`
> **输入:** `x0 = 2`，`y0 = 1`，`x = 4`，`h = 0.4`
> **输出:** `y(x) = 4.122991`
...
该公式基本上使用当前的 `yn` 加上两个增量的加权平均值来计算下一个值 `yn+1`。

*   `K1` 是基于区间开始时斜率的增量，使用 `y`
*   `K2` 是基于区间中点斜率的增量，使用 `(y + h*K1 /2)`。

该方法为二阶方法，意味着局部截断误差在 `O(h^3)` 量级，而总累积误差为 `O(h^4)` 量级。
以下是上述方法的实施:
