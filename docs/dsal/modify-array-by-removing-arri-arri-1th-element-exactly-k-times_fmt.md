...
> 原文: [https://www.geeksforgeeks.org/modify-array-by-removing-arri-arri-1th-element-exactly-k-times/](https://www.geeksforgeeks.org/modify-array-by-removing-arri-arri-1th-element-exactly-k-times/)

给定由第一个 `N` [自然数](https://www.geeksforgeeks.org/natural-numbers/)组成的[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `arr[]`，其中 `arr[i] = i` (基于 1 的索引)和正整数 `K`，任务是[打印在移除每个 `(arr[i] + arr[i + 1])` 个元素后获得的数组](https://www.geeksforgeeks.org/c-program-to-print-an-array-using-recursion/) `arr[]`。
...
> **操作 1:** 从数组 `arr[]` 中删除每 `(arr[1] + arr[2])` 个元素，即每 3 个元素。现在数组修改为 `{1, 2, 4, 5, 7, 8}`。
> **操作 2:** 从数组 `arr[]` 中删除每 `(arr[2] + arr[3])` 个元素，即每 6 个元素。现在数组修改为 `{1, 2, 4, 5, 7}`。
> 执行上述操作后，得到的数组为 `{1, 2, 4, 5, 7}`。

> **输入:** `N = 10`，`K = 3`
> **输出:** `1 2 4 5 7 10`

**方法:** 通过从每个 `i` 操作中的数组中删除每个 `(arr[i] + arr[i + 1])` 第项，精确地执行给定的操作 `K` 次，可以解决给定的问题。按照以下步骤解决问题:
