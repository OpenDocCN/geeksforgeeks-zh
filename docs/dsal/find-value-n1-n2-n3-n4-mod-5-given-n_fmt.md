...
> 原文: [https://www.geeksforgeeks.org/find-value-n1-n2-n3-n4-mod-5-given-n/](https://www.geeksforgeeks.org/find-value-n1-n2-n3-n4-mod-5-given-n/)

给你一个函数 `f(n) = n + n^2 + n^3 + n^4`，对于任意给定值正整数 `n`，你都要求 `f(n) mod 5` 的值。
注意：`n` 可能足够大，这样 `f(n) > 10^18`。
...
首先为了解决这个问题，你可以借助任意幂函数和模运算符直接找到 `(n + n^2 + n^3 + n^4) mod 5` 的值。
...
also as f(n) mod 5 is dependent on n%5, (n+1)%5 & (n^2+1)%5,
...
经过上面的分析，我们可以看到，如果 `n` 的形式是 `5k+1` 或者说 `5k-4`，那么 `f(n) mod 5 = 4`，其他方面 `f(n) mod 5 = 0`。
即如果 `(n % 5 == 1)` 结果 = 4，
否则结果 = 0。
