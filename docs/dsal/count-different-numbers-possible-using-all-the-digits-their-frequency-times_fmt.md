...
> 原文: [https://www.geeksforgeeks.org/count-different-numbers-possible-using-all-the-digits-their-frequency-times/](https://www.geeksforgeeks.org/count-different-numbers-possible-using-all-the-digits-their-frequency-times/)
...
**逼近**: 使用排列组合可以轻松解决问题。数组 `arr[]` 的索引给出了对应数字的频率。这个问题可以认为是求 `X` 对象的全排列，其中 `X0` 对象是一种类型，`X1` 对象是另一种类型，依此类推。那么可能的数字计数由下式给出:

> 总计数 = `X! / (X0! * X1! * ..... * X9!)`

其中 `X` = 所有数字的频率之和，`Xi` = 第 `i` 个数字的频率。
