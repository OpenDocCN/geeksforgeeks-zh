...
> 原文：[https://www.geeksforgeeks.org/rearrange-a-string-so-that-all-same-characters-become-at-least-d-distance-away/](https://www.geeksforgeeks.org/rearrange-a-string-so-that-all-same-characters-become-at-least-d-distance-away/)
...
4.  当 `MH` 不为空时，执行以下操作。
    ...
    *   让第一个位置为 `p`。在 `p`，`p+d`，...，`p+(f-1)d` 处填充 `x`。
