...
> 原文：[https://www.geeksforgeeks.org/comparing-leading-zeros-in-binary-representations-of-two-numbers/](https://www.geeksforgeeks.org/comparing-leading-zeros-in-binary-representations-of-two-numbers/)
...
### 解决方案 1
天真的方法是先找到数字的二进制表示，然后计算前导零的个数。

### 解决方案 2
找到比给定数字小的[最大二次幂](https://www.geeksforgeeks.org/highest-power-2-less-equal-given-number/)，比较这些二次幂决定答案。

### 解决方案 3
一种有效的方法是按位异或和与运算符。

> 情况 1：如果两者都有相同数量的前导零，那么 `(x ^ y) <= (x & y)`，因为相同数量的前导 `0` 会在 `x & y` 的更高位置导致一个 `1`。
