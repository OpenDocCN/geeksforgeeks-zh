...
> 原文：[https://www.geeksforgeeks.org/bitwise-and-of-n-binary-strings/](https://www.geeksforgeeks.org/bitwise-and-of-n-binary-strings/)
...
类似于[添加两个位串](https://www.geeksforgeeks.org/add-two-bit-strings/)。给定一个 N 个二进制字符串的数组。我们首先计算前两个二进制字符串的与运算，然后用第三个二进制字符串执行这个“结果”，以此类推，直到最后一个二进制字符串。
例如字符串 `arr[] = {"101", "110110", "111"}`：
第一步：`result = 101 AND 110110`；
步骤 2：`result = result(步骤 1) AND 111`；
依此类推..
...
找出最小和最大字符串的大小。我们需要这个来给我们的结果加上`(最大-最小)`个零。例如，如果我们有 `0010` 和 `11`，那么这些字符串上的“与”将是 `0010`（因为我们可以将 `11` 写成 `0011`）。然后对每个位执行“与”运算。
