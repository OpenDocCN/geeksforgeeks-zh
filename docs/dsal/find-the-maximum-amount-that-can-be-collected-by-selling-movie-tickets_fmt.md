...
> 原文：[https://www.geeksforgeeks.org/find-the-maximum-amount-that-can-be-collected-by-selling-movie-tickets/](https://www.geeksforgeeks.org/find-the-maximum-amount-that-can-be-collected-by-selling-movie-tickets/)

给定一个整数 `N` 和一个数组 `seats[]`，其中 `N` 是排队买电影票的人数，`seats[i]` 是电影院第 `i` 排的空座数。任务是找到影院老板通过向 `N` 人出售电影票可以赚到的最大金额。一张票的价格等于所有排中空座位的最大数量。
...
> 4 + 3 + 2 = 9
>
> **输入:** `seats[] = {2, 3, 5, 3}`，`N = 4`
> **输出:** 15

**方法:** 这个问题可以通过使用[优先级队列](https://www.geeksforgeeks.org/priority-queue-set-1-introduction/)来解决，该队列将存储每行的空座位数，其中最大值将在顶部可用。

*   创建一个空的优先级队列 `q` 并遍历 `seats[]` 数组，并将所有元素插入优先级队列。
*   初始化两个整数变量 `ticketSold = 0` 和 `ans = 0`，这两个变量将存储售出的门票数量和迄今为止的总收藏量。
*   现在在 `ticketSold < N` 和 `q.top() > 0` 时检查，然后从 `priority_queue` 中移除顶部元素，并通过添加优先级队列的顶部元素来更新 `ans`。也将该最高值存储在变量 `temp` 中，并将 `temp - 1` 插入优先级队列。
