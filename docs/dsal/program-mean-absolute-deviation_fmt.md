...
> 原文: [https://www.geeksforgeeks.org/program-mean-absolute-deviation/](https://www.geeksforgeeks.org/program-mean-absolute-deviation/)
...
Input : arr[] = {23.54, 56, 34.56, 67, 45.34, 56.78, 78, 21}
Output : 16.6675
```

> 数据集的平均绝对偏差或平均绝对偏差是与平均值或中心点的绝对差异的平均值。平均绝对偏差可以用下面给出的方法计算。
> 设 `arr[n]` 为 `n` 大小的数组，任务是求平均绝对偏差。
> 平均绝对偏差 = (`abs(arr[0] – mean) + abs(arr[1] – mean) + ... + abs(arr[n-1] – mean)`) / `n`；
