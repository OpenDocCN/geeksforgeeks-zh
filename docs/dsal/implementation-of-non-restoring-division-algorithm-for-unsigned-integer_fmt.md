...
> 原文：[https://www.geeksforgeeks.org/implementation-of-non-restoring-division-algorithm-for-unsigned-integer/](https://www.geeksforgeeks.org/implementation-of-non-restoring-division-algorithm-for-unsigned-integer/)
...
| 步骤 | 累加器-A | 被除数-商Q | 状态 |
| :--- | :--- | :--- | :--- |
| **初始值** | `0000` | `0111` | `M`=`0101` |
| **步骤1：左移** | `0000` | `111_` | |
| 操作：A - M | `1011` | `1110` | 步骤不成功（负）<br>A + M |
| **步骤2：左移** | `0111` | `110_` | |
| 操作：A + M | `1100` | `1100` | 步骤不成功（负）<br>A + M |
| **步骤3：左移** | `1001` | `100_` | |
| 操作：A + M | `1110` | `1000` | 步骤不成功（负）<br>A + M |
| **步骤4：左移** | `1101` | `000_` | |
| 操作：A + M | `0010` | `0001` | 步骤成功（正） |
| | **余数(2)** | **商(1)** | |

## 方法

从上面的解决方案来看，思路是观察计算所需商和余数所需的步数等于被除数中的位数。最初，假设被除数为 `Q`，除数为 `M`，累加器 `A` = `0`。因此：

1.  每走一步，向左移动被除数 `Q` 1 个位置。
2.  从 `A` (`A - M`) 中减去除数。
