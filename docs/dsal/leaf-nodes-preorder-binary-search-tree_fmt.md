...
> 原文:[https://www . geesforgeks . org/leaf-nodes-preorder-binary-search-tree/](https://www.geeksforgeks.org/leaf-nodes-preorder-binary-search-tree/)
...
这个想法是利用二叉查找树和栈的性质。
使用指向数组的两个指针 `i` 和 `j` 遍历数组，最初 `i = 0`，`j = 1`。每当 `a[i] > a[j]` 时，我们可以说 `a[j]` 是 `a[i]` 的左边部分，因为前序遍历遵循 访问 -> 左 -> 右。因此，我们在栈中插入 `a[i]`。
对于那些违反规则的点，我们开始从栈中弹出元素，直到栈顶元素小于 `a[j]`，当它没有弹出时，我们会中断循环，并打印相应的 `j` 值。
...
前序遍历按顺序遍历：访问、左、右。
并且我们知道 BST 中任意节点的左子节点总是小于该节点。所以前序遍历将首先从根节点遍历到最左边的节点。因此，前序序列将首先按降序排列。现在，在降序之后，可能有一个节点更大或者打破了降序。所以，可能会有这样的情况：
...
这是使用栈解决的。
当在情况 1 和情况 2 上运行上述算法时，当 `i = 2` 和 `j = 3` 时，两种情况下栈的状态将相同：

![](img/be5ce52738aaa480a6de251d29f7b956.png)

因此，节点 65 将从栈中弹出 20 和 50。这是因为 65 是 20 之前的节点的右子节点。我们使用 `found` 变量存储这些信息。所以，20 是根节点。
而在情况 2 中，40 将不能从栈中弹出任何元素。因为 40 是 20 之后的节点的右节点。所以，20 不是叶节点。
注意：在算法中，我们将无法检查前序最右节点或最右元素的叶节点的情况。所以，只需打印最右边的节点，因为我们知道这将始终是一个叶节点在前序遍历中。
以下是本办法的实施情况：
