...
> 原文：[https://www.geeksforgeeks.org/find-partition-line-such-that-sum-of-values-on-left-and-right-is-equal/](https://www.geeksforgeeks.org/find-partition-line-such-that-sum-of-values-on-left-and-right-is-equal/)

考虑笛卡尔坐标平面上的 `n` 点。让点 `(X_i, Y_i)` 被赋予一个值 `V_i`。平行于 `y轴`的线，如果其左侧点值之和等于其右侧点值之和，则称为好分割线。**注意**如果一个点位于分割线上，则该点不在分割线的两侧。任务是打印`是`如果有好的分割线，则打印`否`。
...
`x = c`，其中 `5 < c < 8` 是一条好的分割线，因为两侧值之和为 `10`。
...
2.  从最小 `X` 坐标开始，在`X_i`检查是否满足以下条件之一:
