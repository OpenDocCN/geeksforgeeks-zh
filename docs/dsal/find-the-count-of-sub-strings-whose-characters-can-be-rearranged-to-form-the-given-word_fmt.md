...
> 原文：[https://www.geeksforgeeks.org/find-the-count-of-sub-strings-whose-characters-can-be-rearranged-to-form-the-given-word/](https://www.geeksforgeeks.org/find-the-count-of-sub-strings-whose-characters-can-be-rearranged-to-form-the-given-word/)

给定一个字符串 `str`，任务是找到所有长度为 4 的子字符串的计数，这些子字符串的字符可以重新排列以形成单词 `“clap”`。
...
**方法:** 对于每一个长度为 4 的子串，从单词 `“clap”` 开始计算字符出现的次数。如果每个字符在子字符串中都恰好出现一次，则增加 `cnt`。最后打印 `cnt`。
