...
> 原文：[https://www.geeksforgeeks.org/check-divisibility-binary-stream/](https://www.geeksforgeeks.org/check-divisibility-binary-stream/)
...
**如何帮助得到余数？**
任何数（`m`）都可以写成 `m = a*n + r` 的形式，其中 `a`、`n` 和 `r` 是整数，`r` 是余数。所以当 `m` 乘以任何数 `x`，余数。假设 `m` 乘以 `x`，那么 `m*x` 将是 `m*x = x*a*n + x*r`。所以 `(m*x) % n = (x*a*n) % n + (x*r) % n = 0 + (x*r) % n = (x*r) % n`；
