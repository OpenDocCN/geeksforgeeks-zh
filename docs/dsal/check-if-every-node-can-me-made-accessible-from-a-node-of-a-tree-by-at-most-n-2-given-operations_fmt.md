...
> 原文: [https://www.geeksforgeeks.org/check-if-every-node-can-be-made-accessible-from-a-node-of-a-tree-by-at-most-n-2-given-operations/](https://www.geeksforgeeks.org/check-if-every-node-can-be-made-accessible-from-a-node-of-a-tree-by-at-most-n-2-given-operations/)

## 问题描述

给定一个由 `N` 个节点组成的有向树，任务是检查给定树中是否存在一个节点，以便通过从树中移除任何有向边并最多在树中的任何一对节点之间添加另一个有向边 `floor(N/2)` 次来到达所有其他节点。如果存在这样的节点，则打印 `"是"`。否则，打印 `"否"`。
...
因此，剩余的两个节点 (`2`, `3`) 现在都可以从节点 `1` 访问。所需操作数为 `1`，即 `<= floor(3/2)` (`= 1`)。
...
*   每个节点应该至少有一个父节点，即每个节点应该至少有 `1` 个入度，以使树可以从所需节点访问。
*   可以得出结论，如果每个节点的入度至少有 `1` 个，那么所有其他节点都可以被访问。
*   因此，任务简化为寻找具有 `0` 入度的节点数量，并检查其是否至多为 `N / 2`。

按照以下步骤解决问题:

1.  将树中每个节点的入度存储在大小为 `(N + 1)` 的辅助数组 `A[]` 中。
2.  将此数组初始化为地图中所有 `(键, 值)` 对的 `A[键] = 值`。
3.  初始化一个变量，比如说 `count` 为 `0`，来存储入度等于 `0` 的节点数。
4.  遍历数组 `A[]` 和计算值为 `0` 的数组元素数量，并将其存储在变量 `count` 中。
5.  完成上述步骤后，如果 `count` 的值最多为 `floor(N/2)`，则打印 `"是"`。否则，打印 `"否"`。
