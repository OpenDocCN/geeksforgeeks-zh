...
> 原文：[https://www.geeksforgeeks.org/subtract-two-numbers-without-using-arithmetic-operators/](https://www.geeksforgeeks.org/subtract-two-numbers-without-using-arithmetic-operators/)

编写一个减法函数`subtract(x, y)`，返回`x - y`，其中`x`和`y`是整数。该函数不应使用任何算术运算符（`+`、`++`、`--`、`-`等）。
