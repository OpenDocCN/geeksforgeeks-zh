...
> 原文：[https://www.geeksforgeeks.org/average-of-first-n-even-natural-numbers/](https://www.geeksforgeeks.org/average-of-first-n-even-natural-numbers/)
...
在这个程序中迭代循环，找到前 `N` 个偶数的总和并除以 `N`，需要 `O(N)` 个时间。
...
思路是前 `N` 个偶数之和为 `n(n+1)`，用于求前 `N` 个偶数除以 `N` 的平均值，故公式为 `n(n + 1) / n = ( n + 1)`。即前 `n` 个偶数的平均值为 `n+1`。它需要 `O(1)` 时间。
