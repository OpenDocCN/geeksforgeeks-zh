...
> 原文: [https://www.geeksforgeeks.org/check-if-the-sum-of-perfect-squares-in-an-array-is-divisible-by-x/](https://www.geeksforgeeks.org/check-if-the-sum-of-perfect-squares-in-an-array-is-divisible-by-x/)
...
从 `i` 到 `n–1` 运行一个循环，检查 `arr[i]` 是否是完美的正方形。如果 `arr[i]` 是一个完美的正方形，那么更新 `sum = sum + arr[i]`。如果最后 `sum % x = 0` 则打印`是`否则打印`否`。要检查一个元素是否是完美的正方形，请遵循以下步骤:
