...
> 原文: [https://www.geeksforgeeks.org/remaining-array-element-after-repeated-removal-of-last-element-and-subtraction-of-each-element-from-next-adjacent-element/](https://www.geeksforgeeks.org/remaining-array-element-after-repeated-removal-of-last-element-and-subtraction-of-each-element-from-next-adjacent-element/)

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是在从其下一个相邻元素中减去每个元素并重复移除最后一个数组元素后，找到剩余的数组元素。
...
> **输出:** `-11`
> **解释:**
> 操作 1: 数组 `arr[]` 修改为 `{ 8-1, 4-8 } = { 7, -4}`。
> 操作 2: 数组 `arr[]` 修改为 `{ -4-7 } = { -11 }`。
> 因此，最后剩余的数组元素是 `-11`。

## 天真方法

最简单的方法是遍历数组直到其大小减少到 1，并对数组执行给定的操作。完成遍历后，打印剩余的元素。
**时间复杂度:** `O(N^2)`
**辅助空间:** `O(1)`
...
*   从以上两个观察，可以得出结论: 答案是 `(x-y)^(N-1)` 和每个数组元素 `arr[i]` 的展开式中的项的系数的乘积之和。
*   因此，思路是将每个数组元素更新为 `(arr[i] * (N-1)C(i-1) * (-1)^i)` 后，求数组 `arr[]` 之和。

按照以下步骤解决问题:

*   遍历数组 `arr[]` 并使用帕斯卡三角形计算 `N^Cr` 后，更新 `arr[i]` 为 `arr[i] = arr[i] * (N-1)C(i-1) * (-1)^i`。
*   打印数组和 `arr[]`。
