...
> 原文：`https://www.geeksforgeeks.org/immediate-smaller-element-n-ary-tree/`
...
元素 `2`、`15`、`20` 和 `25` 小于 `x`，即 `30`，但 `25` 是直接较小的元素，因此是答案。
...
`O(N)`，其中 `N` 为 N 元树中的节点数。

## 辅助空间

`O(N)`，用于递归调用（当一个节点有 `N` 个子节点时的最坏情况）。