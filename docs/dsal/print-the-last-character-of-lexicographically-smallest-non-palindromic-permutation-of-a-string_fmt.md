...
给定[字符串](https://www.geeksforgeeks.org/string-data-structure/)`str`，任务是打印给定字符串的字典最小的[非回文排列](https://www.geeksforgeeks.org/make-the-string-lexicographically-smallest-non-palindromic-by-replacing-exactly-one-character/)的最后一个字符。 如果不存在这样的排列，请打印 -1。
...
解决该问题的最简单方法是[生成给定字符串的所有可能排列](https://www.geeksforgeeks.org/write-a-c-program-to-print-all-permutations-of-a-given-string/)，并针对每个排列检查它是否是回文集。 在所有获得的非回文排列中，按字典顺序打印最小的排列的最后一个字符。 请按照以下步骤操作：
...
为了优化上述方法，其思想是存储给定字符串`str`的每个字符的[频率](https://www.geeksforgeeks.org/print-characters-frequencies-order-occurrence/)。 如果所有字符都相同，则打印 -1。 否则，打印给定字符串`str`的最大字符。
