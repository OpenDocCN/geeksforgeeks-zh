...
[**绳索**](https://www.geeksforgeeks.org/ropes-data-structure-fast-string-concatenation/)是可伸缩的[字符串](https://www.geeksforgeeks.org/string-data-structure/)实现。它们是为整个字符串的高效操作而设计的。赋值、连接和子字符串等操作花费的时间几乎与字符串的长度无关。
...
**注:** `rope`类和`crope`为 **SGI 扩展**；它们不是 C++标准库的一部分。

**声明:**
绳索的定义方式与[向量](https://www.geeksforgeeks.org/vector-in-cpp-stl/)相同为`vector<int>`。但是对于字符绳，我们可以用`crope`作为`rope<char>`。
...
*   `push_back()`: 此功能用于在绳子末端输入一个字符。 ***时间复杂度:*** O(对数 N)。
*   `pop_back()`: 从 C++11 引入(针对字符串)，这个函数用来删除绳子上的最后一个字符。 ***时间复杂度:*** O(对数 N)。
*   `insert(int x, crope r1)`: 在第 `x` 个元素之前插入 `r1` 的内容。 ***时间复杂度:*** 对于**最佳情况:** O(log N)而对于**最坏情况:** O(N)。
*   `erase(int x, int l)`: 擦除 `l` 个元素，从第 `x` 个元素开始。 ***时间复杂度:*** O(log N)。
*   `substr(int x, int l)`: 返回一个新的绳子，其元素是从位置 `x` 开始的 `l` 个字符。 ***时间复杂度:*** O(log N)。
*   `replace(int x, int l, crope r1)`: 用 `r1` 中的元素替换以第 `x` 个开头的 `l` 个元素。 ***时间复杂度:*** O(log N)。
