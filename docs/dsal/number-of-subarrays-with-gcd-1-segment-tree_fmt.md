...
> **输入:** `arr[] = {1，1，1}`
> **输出:** `6`
> 给定阵列的每个单个子阵列的 GCD 为 `1`，总共有 `6` 个子阵列。
> **输入:** `arr[] = {2，2，2}`
> **输出:** `0`

## 方法

这个问题可以在 `O(NlogN)` 中使用[段树](https://www.geeksforgeeks.org/segment-tree-set-1-sum-of-given-range/)数据结构来解决。将要构建的段可以用来回答范围-gcd 查询。
...
经过以上观察，双指针技术完全有道理，即如果最小的 `R` 的长度对于索引 `L` 是已知的，那么对于索引 `L + 1`，搜索需要从 `R` 开始。
