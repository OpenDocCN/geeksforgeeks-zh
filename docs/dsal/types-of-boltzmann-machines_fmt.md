...
∈_i - Energy of system in state i
T - Temperature of the system
k - Boltzmann constant
∑e^(-∈_j/kT) - Sum of values for all possible states of the system
```

玻尔兹曼分布描述了系统的不同状态，因此玻尔兹曼机器使用这种分布创建了机器的不同状态。从上面的方程可以看出，随着系统能量的增加，系统处于状态“I”的概率降低。因此，系统在最低能量状态下最稳定（气体扩散时最稳定）。这里，在玻尔兹曼机器中，系统的能量是根据突触的**权重**来定义的。一旦系统被训练并且权重被设置，系统总是试图通过调整权重来为自己找到最低能量状态。
...
在一个完整的玻尔兹曼机器中，每个节点都与其他节点相连，因此连接呈指数增长。这就是我们使用受限玻尔兹曼机的原因。受限玻尔兹曼机中节点连接的限制如下：
...
```
E(v, h) = -∑ a_i v_i - ∑ b_j h_j - ∑∑ v_i w_{i,j} h_j
...
```
d/dw_{ij} (log(P(v^0))) = <v_i^0 * h_j^0> - <v_i^∞ * h_j^∞>
v - visible state, h- hidden state
<v_i^0 * h_j^0> - initial state of the system
<v_i^∞ * h_j^∞> - final state of the system
P(v^0) - probability that the system is in state v^0
w_{ij} - weights of the system
```

上述等式告诉我们——系统权重的变化将如何改变系统处于特定状态的对数概率。系统试图以尽可能低的能量状态（最稳定）结束。我们也可以只考虑前几个暂停，而不是继续调整权重过程，直到当前输入与前一个输入相匹配。了解如何调整我们的曲线以获得最低的能量状态就足够了。因此，我们调整权重，重新设计系统和能量曲线，以便获得当前位置的最低能量。这被称为**辛顿的捷径**。
...
考虑一下——玛丽看了六部电影中的四部，并对其中的四部进行了评分。说着，她看了 `m_1`、`m_3`、`m_4` 和 `m_5` 并且喜欢 `m_3`、`m_5`（评分 1）而不喜欢另外两部，也就是 `m_1`、`m_4`（评分 0），而另外两部电影——`m_2`、`m_6` 都没有评分。现在，用我们的 RBM，我们将推荐其中一部电影给她看。说：
...
*   塔伦蒂诺导演的《第四季》。
*   `m_2` 属于‘动作’类型。
*   `m_6` 同时属于《动作》和《戏剧》两种类型，《迪卡普里奥》在其中出演，并获得了“奥斯卡奖”。

我们有以下观察结果：

*   玛丽喜欢 `m_3`，`m_5`，它们都属于“戏剧”类型，她可能**喜欢“戏剧”**电影。
*   玛丽不喜欢 `m_1`，`m_4` 这些动作片，她可能**不喜欢《动作》**电影。
*   玛丽喜欢 `m_3`，`m_5` 他们已经获得了一个‘奥斯卡’，她可能**喜欢**一个**【奥斯卡】**电影。
*   既然《迪卡普里奥》在《第五季》中出演，而且玛丽喜欢，她可能会喜欢《T2》这部由 T4《迪卡普里奥》在《第五季》中出演的电影。
*   玛丽不喜欢塔伦蒂诺导演的《第四季》，她可能不喜欢 T4《塔伦蒂诺》导演的任何电影《第五季》。

因此，基于 `m_2`、`m_6` 的观察和细节；我们的 RBM **推荐 `m_6`** 给玛丽（《戏剧》，《迪卡普里奥》和《奥斯卡》都符合玛丽的兴趣和 `m_6`）。这就是 RBM 是如何工作的，因此被用于推荐系统。
...
假设我们将几个受限玻尔兹曼机堆叠在一起，这样第一个 RBM 输出就是第二个 RBM 的输入，以此类推。这种网络被称为深度信念网络。每一层中的连接都是无方向的（因为每一层都是 RBM）。同时，层与层之间的连接是定向的（除了上面两层——上面两层之间的连接是无定向的）。有两种方法可以训练 DBN 网络：

1.  **贪婪分层训练算法：** RBM 是逐层训练的。一旦训练了单个 RBM（即设置了参数-权重、偏差），就在 DBN 层之间设置了方向。
...
DBM 类似于 DBN，除了层内的连接之外，层与层之间的连接也是**无方向的**（不像 DBN 那样，层与层之间的连接是有方向的）。DBM 可以提取更复杂的特征，因此可以用于更复杂的任务。