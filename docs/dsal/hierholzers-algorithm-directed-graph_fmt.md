...
以下是对上面代码的改进:
*   上面的代码记录了每个顶点的边数。这是不必要的，因为我们已经在维护邻接表。我们只是删除了 `edge_count` 数组的创建。在算法中，我们将 `if edge_count[curr_v]` 替换为 `if adj[curr_v]`。
*   上面的代码将初始节点推送到堆栈中两次。尽管他对结果进行编码的方式是正确的，但这种方法令人困惑且效率低下。我们通过将下一个顶点附加到堆栈上来消除这个问题，而不是当前的顶点。
*   在作者测试算法的主要部分，邻接表 `adj1` 和 `adj2` 的初始化有点怪异。那部分也是改良过的。
...
本文由 [阿舒托什·库马尔](https://in.linkedin.com/in/ashutosh-kumar-9527a7105) 供稿。文章还包含来自尼提什·库马尔的输入。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 [contribute.geeksforgeeks.org](http://www.contribute.geeksforgeeks.org) 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
