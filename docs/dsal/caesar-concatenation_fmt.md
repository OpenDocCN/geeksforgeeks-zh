...
> **解释:**
> 移位为 4 的字符串 `str1` 的凯撒文本为 `"kiojskiow"`
> 在所有偶数索引处移位为 4 的字符串 `str2` 的凯撒文本为 `"keikw 163"`
> 结果字符串为 `"kiojskiow" + "keikw 163"`

> **输入:** `str1 = "jklmn12"`, `str2 = "abc1w"`, `N = 9`
> **输出:** `JKlmN12nfma1w`
> **解释:**
...
思路是遍历给定的字符串 `str1` 和 `str2`，在下面 3 种情况的基础上移位 `N` 将 `str1` 每个索引处和 `str2` 偶数索引处的所有字符进行转换。
...
3.  **情况 3:** 如果字符位于 `'0'` 和 `'9'` 之间，则当前字符被加密为:
    ```
...
void printCaesarText(string str1, string str2, int N)
{
