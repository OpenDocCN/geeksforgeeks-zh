...
> 原文: [https://www.geeksforgeeks.org/job-selection-problem-loss-minimization-strategy-set-2/](https://www.geeksforgeeks.org/job-selection-problem-loss-minimization-strategy-set-2/)
...
在计算生产结束时的总数量时，请记住第一天生产的货物将会有 `(1-P)^{N-i}` 乘以它的左音量。显然，从 `(1-P)^{N-N} = 1` 开始，在第 N 天(最后一天)生产的货物将保持其体积不变。
...
## 复杂性–
我们精确地执行 N 个 `push()` 和 `pop()` 操作，每个操作都需要 `log(N)` 时间。因此，时间复杂度为 `O(N * log(N))`。
下面是解决方案的实现。
