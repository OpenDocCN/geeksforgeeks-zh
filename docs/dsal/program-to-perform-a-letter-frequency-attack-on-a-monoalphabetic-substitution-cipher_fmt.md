...
> 原文: [https://www.geeksforgeeks.org/program-to-perform-a-letter-frequency-attack-on-a-monoalphabetic-substitution-cipher/](https://www.geeksforgeeks.org/program-to-perform-a-letter-frequency-attack-on-a-monoalphabetic-substitution-cipher/)
...
4.  因此，想法是找出给定字符串中第 `i` 个最大出现字母和字符串 `T` 之间的差异，然后用该差异移动给定字符串的所有字母。获得的字符串将是可能的解密字符串之一。
...
*   [使用变量 `i` 迭代范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[0, 5]`，并执行以下步骤:
    *   找到字符串 `S` 中出现次数最多的第 `i` 个元素，并将其存储在一个变量中，比如 `ch`。
    *   找出字符串 `T` 的 `ch` 和第 `i` 个字符之间的区别，并将其存储在一个变量中，比如说 `x`。
    *   [迭代字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-c/) `S` 的字符，将所有字符移动 `x`，然后将得到的字符串推入一个数组 `plaintext[]`。
*   最后，经过以上步骤，打印数组 `plaintext[]` 中获得的字符串。
