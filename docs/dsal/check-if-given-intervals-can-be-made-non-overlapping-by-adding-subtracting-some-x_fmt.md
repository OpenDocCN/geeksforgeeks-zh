# 通过加减一些 X 检查给定的间隔是否可以不重叠

> 原文: [https://www.geeksforgeeks.org/check-if-given-intervals-can-be-made-non-overlapping-by-adding-subtracting-some-x/](https://www.geeksforgeeks.org/check-if-given-intervals-can-be-made-non-overlapping-by-adding-subtracting-some-x/) 使其不重叠

给定一个包含 `N` 个间隔的数组 `arr[]`，任务是检查间隔是否可以加或减 `X`，之后是否没有重叠的间隔。这里 `X` 可以是任何实数。
...
> **输出:** YES
> **说明:**
> 我们可以在第1个和第3个区间中加上 `X = 1000`，在第2个和第4个区间中减去 `X = 1000`。
>
> **输入:** `arr[] = {[1，2]，[3，4]，[5，6]}`
> **输出:** YES
> **说明:**
...
想法是在嵌套循环的帮助下，将每个间隔作为一对进行比较，然后针对每个间隔检查它们是否重叠。如果任何三个间隔彼此重叠，那么就没有办法增加任何值 `X` 来形成不重叠。
我们可以使用联合查找或不相交集合数据结构来发现三个间隔中是否存在相互重叠。
