...
> 原文: [https://www.geeksforgeeks.org/queries-to-find-the-count-of-characters-preceding-the-given-location/](https://www.geeksforgeeks.org/queries-to-find-the-count-of-characters-preceding-the-given-location/)

给定长度为 `N` 的字符串 `S`，其中只包含小写字母。另外，给定 `Q` 个查询，其中每个查询由整数 `P` 组成，使得 `1≤ P ≤ N`。任务是找出给定位置 `P` 前相同字母出现的次数。
...
对于给出位置 `p` 的每个查询 `Q_i`，运行从 `1` 到 `p-1` 的循环，并检查该索引处存在的元素是否等于索引 `p` 处的元素，如果是，则将计数器变量的值增加 `1`。循环完成后，在新行中打印计数器变量的值。
