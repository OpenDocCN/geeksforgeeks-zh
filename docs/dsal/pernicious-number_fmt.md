...
一个`流毒数`是一个正整数，在其二进制表示中有个素数。第一个`流毒数`是 3，因为 3 = (11)(在二进制表示中)和 1 + 1 = 2 是一个素数。

## 流毒数属性

1.  没有任何`流毒数`也是 2 的次幂，因为二进制形式的 2 的次幂表示为 1 后跟 0。所以，1 不被认为是质数。
2.  形式 `2^n + 1` 的每一个带有 n 的数字> 0 都是`流毒数`，因为二进制形式的 1 的数量是 2，它是质数。
3.  带素数 p 的形式为 `2^p – 1` 的数是一个`流毒数`，称为[梅森数](https://en.wikipedia.org/wiki/Mersenne_prime)。

## 打印前 n 个有害数字的想法

打印前 n 个`流毒数`的想法很简单。对从 1 到 n 的每个数字执行以下操作。
...
下面是打印前 25 个`流毒数`的程序。以下是上述办法的实施情况。
