...
最初我们用最低可能的配置形成阵列，用 `1`、`2`、`3`、`4` 填充阵列..其满足给定的条件。如果总和为 `1..k` 大于 `N`，则不能形成数组。为了形成阵列，首先用 `1`、`2`、`3`，..`k`。再次向数组中的每个元素添加 `(n-sum)/k`，因为在这样添加时，没有任何条件是无效的，因为我们正在向每个数字添加相等的元素。
剩余的数字 `rem` 贪婪地从后面加上，使每一个数字都是前一个数字的两倍。填充数组后，如果任何给定的条件不满足，打印`-1`，否则形成的数组将是我们想要的答案。
以下是上述方法的实现:
