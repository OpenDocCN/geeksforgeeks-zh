...
> 原文：[https://www.geeksforgeeks.org/sort-array-according-absolute-difference-given-value-using-constant-extra-space/](https://www.geeksforgeeks.org/sort-array-according-absolute-difference-given-value-using-constant-extra-space/)
...
上面的问题在之前的帖子[这里](https://www.geeksforgeeks.org/sort-an-array-according-to-absolute-difference-with-given-value/)已经解释过了。它需要 `O(n^2)` 的时间和 `O(n)` 的额外空间。下面的解决方案虽然具有相对较差的时间复杂度（即 `O(n^2)`），但它在不使用任何额外空间或内存的情况下完成工作。
