...
> 原文: [https://www.geeksforgeeks.org/total-number-of-subsets-of-size-at-most-k/](https://www.geeksforgeeks.org/total-number-of-subsets-of-size-at-most-k/)
...
> **输出:** `6`
> 包含 1 个元素的子集= `{1}`、`{2}`、`{3}`
> 包含 2 个元素的子集= `{1, 2}`、`{1, 3}`、`{2, 3}`
> 由于 `K = 2`，因此对于长度最多 `K`，将只考虑上述子集，因此计数为 `6`。
>
> **输入:** `N = 5`，`K = 2`
> **输出:** `15`
...
1.  因为可以由 `N` 个项目构成的恰好 `K` 个元素的子集的数量是 `^N C_K`。因此对于“最多”，所需的计数将是
...
2.  为了计算 `^N C_K` 的值，使用了[二项式系数](https://www.geeksforgeeks.org/dynamic-programming-set-9-binomial-coefficient/)。请[参考这篇文章，看看它是如何工作的](https://www.geeksforgeeks.org/binomial-coefficient-dp-9/)。

3.  为了得到长度为 `K` 的所需子集，运行一个从 `1` 到 `K` 的循环，并为 `i` 的每个值加上 `^N C_i`。
