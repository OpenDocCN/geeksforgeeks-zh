...
> 原文：[https://www.geeksforgeeks.org/find-sum-of-factorials-in-an-array/](https://www.geeksforgeeks.org/find-sum-of-factorials-in-an-array/)
...
**方法：** 实现一个函数 `factorial(n)`，该函数找到 `n` 的阶乘，并初始化 `sum = 0`。现在，遍历给定的数组，对于每个元素 `arr[i]` 更新 `sum = sum + factorial(arr[i])`。最后打印计算出的 `sum`。
