...
[Shell Sort](http://en.wikipedia.org/wiki/Shellsort) 主要是 [Insertion Sort](https://www.geeksforgeeks.org/insertion-sort/) 的变体。在插入排序中，我们只将元素向前移动一个位置。当一个元素必须向前移动很远时，会涉及到许多移动。`shellSort` 的思想是允许交换远程项目。在 `shellSort` 中，我们对数组 `h` 进行排序，得到一个大的 `h` 值。我们不断减少 `h` 的值，直到它变成 1。如果第 `h` 个元素的所有子列表都被排序，那么这个数组被称为 `h` 排序的。
...
### 时间复杂度
上述 `shellsort` 实现的时间复杂度为 O(n<sup>2</sup>)。在上面的实现中，间隙在每次迭代中减少一半。还有许多其他方法可以减少间隙，从而提高时间复杂度。详见[本](http://en.wikipedia.org/wiki/Shellsort#Gap_sequences)。

### 参考资料
*   [https://www.youtube.com/watch?v=pGhazjsFW28](https://www.youtube.com/watch?v=pGhazjsFW28)
