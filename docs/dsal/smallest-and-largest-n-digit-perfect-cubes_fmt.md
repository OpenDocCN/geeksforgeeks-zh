...
> 原文: [https://www.geeksforgeeks.org/smallest-and-largest-n-digit-perfect-cubes/](https://www.geeksforgeeks.org/smallest-and-largest-n-digit-perfect-cubes/)
...
**方式:** 从 `N = 1` 开始增加 `N` 的数值，系列将像 `8、64、729、9261、…...` 为**最大的 N 位完美立方体**，其 `N` 第项将为 `pow(ceil(cbrt(pow(10, (N))))-1, 3)`。
和 `1，27，125，1000，…...` 表示最小的 N 位完美立方体，其 `N` 第项为 `pow(ceil(cbrt(pow(10, (N–1)))))、3)`。
以下是上述方法的实施:
