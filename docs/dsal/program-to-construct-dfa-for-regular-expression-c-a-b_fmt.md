...
给定一个[字符串](https://www.geeksforgeeks.org/string-data-structure/) `S`，任务是设计一个[确定性有限自动机(DFA)](https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-1/)来接受语言 `L = C(A+B)+`。如果给定字符串被DFA接受，则打印`“是”`。否则，打印`“否”`。
...
**方法：** 思路是解释给定的语言 `L = C(A+B)+`。对于`C(A+B)+`形式的[正则表达式](https://www.geeksforgeeks.org/write-regular-expressions/)，下面是[DFA状态转移图](https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-2/)：
