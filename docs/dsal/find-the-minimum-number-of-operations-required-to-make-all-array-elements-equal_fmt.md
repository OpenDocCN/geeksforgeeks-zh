...
> 原文: [https://www.geeksforgeeks.org/find-the-minimum-number-of-operations-required-to-make-all-array-elements-equal/](https://www.geeksforgeeks.org/find-the-minimum-number-of-operations-required-to-make-all-array-elements-equal/)
...
我们来找数组中最频繁的元素（使用 `map` 存储所有元素的频率）。让这个元素为 `x`。如果我们更仔细地观察这些操作，我们可以看到这些操作的部分意味着**将元素 `p` 设置为元素 `q`**。如果 `p < q` 则需要进行第一次操作，否则进行第二次操作。
