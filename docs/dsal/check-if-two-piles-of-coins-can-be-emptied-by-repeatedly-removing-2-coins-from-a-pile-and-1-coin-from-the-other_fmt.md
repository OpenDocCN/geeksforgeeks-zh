...
> 原文: [https://www.geeksforgeeks.org/check-if-two-piles-of-coins-can-be-emptied-by-repeatedly-removing-2-coins-from-a-pile-and-1-coin-from-the-other/](https://www.geeksforgeeks.org/check-if-two-piles-of-coins-can-be-emptied-by-repeatedly-removing-2-coins-from-a-pile-and-1-coin-from-the-other/)

给定两堆由硬币数量 `N` 和 `M` 组成的硬币，任务是通过重复从一堆中取出 `2` 枚硬币和从另一堆中取出 `1` 枚硬币来检查两堆硬币是否可以同时清空。如果两堆都可以清空，则打印`“是”`。否则，打印`“否”`。
...
> **输出:** `是`
> **说明:**
> 从第 1 堆中取出 1 枚硬币，从另一堆中取出 2 枚硬币。因此，两堆硬币的数量都是 0。
>
> **输入:** `N = 2`，`M = 2`
> **输出:** `否`
...
因此，思路是[检查硬币总数之和是否为 `3` 的倍数，最大硬币数最多为最小硬币数的两倍](https://www.geeksforgeeks.org/check-whether-large-number-given-form-multiple-3/)，然后打印`“是”`。否则，打印`“否”`。
