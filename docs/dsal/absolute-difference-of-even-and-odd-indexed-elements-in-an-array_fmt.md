...
> 原文：[https://www.geeksforgeeks.org/absolute-difference-of-even-and-odd-indexed-elements-in-an-array/](https://www.geeksforgeeks.org/absolute-difference-of-even-and-odd-indexed-elements-in-an-array/)

给定一个整数数组 `arr`，任务是分别找到偶数和奇数索引位置元素的运行绝对差。

### 注：
数组考虑基于 0 的索引。也就是说，数组中第一个元素的索引为零。
...
### 方式：
遍历数组，保留两个变量 `even` 和 `odd` 分别存储偶、奇索引元素的绝对差。遍历时，检查当前索引是否为偶数，即 `(i%2) == 0`。最后打印结果。
以下是上述方法的实施：
