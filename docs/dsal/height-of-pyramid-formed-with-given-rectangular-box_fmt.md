...
> 原文: [https://www.geeksforgeeks.org/height-of-pyramid-formed-with-given-rectangular-box/](https://www.geeksforgeeks.org/height-of-pyramid-formed-with-given-rectangular-box/)
...
*   我们必须为每个区块制作尺寸 `l * l` 或 `b * b`。
*   因为我们只能修剪而不能连接，所以我们选择方块的尺寸为 `min(l, b)`。
*   用每个矩形块上的 `min(l, b)` 创建一个数组 `a`。
*   将数组 `a` 排序，并将高度初始化为 0。
*   遍历数组 `a`，如果数组中的元素大于 `height + 1`，则将高度值增加 1。
*   返回高度。
