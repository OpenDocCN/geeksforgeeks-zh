...
> 原文: [https://www.geeksforgeeks.org/sort-permutation-of-1-to-n-by-removing-any-element-and-inserting-it-to-front-or-back/](https://www.geeksforgeeks.org/sort-permutation-of-1-to-n-by-removing-any-element-and-inserting-it-to-front-or-back/)

给定一个大小为 `N` 的数组 `arr[]`，该数组具有从 `1` 到 `N` 的 `N` 个不同的整数，任务是通过移除任何元素并将其插入到数组的前面或后面来计算按升序对数组排序所需的最小步骤数。
...
**方法:** 利用以下事实可以解决这个问题: 要使数组以最小步数排序，必须将最小数量的元素移到前面或后面。此外，必须改变位置的元素最初不会按照增加的顺序排列。因此，问题简化为寻找最长的递增子阵列，因为只有阵列中的那些元素不会被移动。
