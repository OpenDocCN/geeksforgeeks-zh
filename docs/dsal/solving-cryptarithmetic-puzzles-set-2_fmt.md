...
> 原文: [https://www.geeksforgeeks.org/solving-cryptarithmetic-puzzles-set-2/](https://www.geeksforgeeks.org/solving-cryptarithmetic-puzzles-set-2/)

给定一个大小为 `N` 的字符串数组 `arr[]` 和一个字符串 `S`，任务是找出是否有可能将范围 `[0, 9]` 中的整数值映射到字符串中出现的每个字母表，使得对数组中所有字符串编码形成的数字求和后得到的总和等于字符串 `S` 形成的数字。
...
> **输入:** `arr[] = {“SIX”, “SEVEN”, “SEVEN”}`, `S = “TWENTY”`
> **输出:** Yes
> **解释:**
> 可能的方式之一是:
> 1.  将字符映射如下: `S`→6、`I`→5、`X`→0、`E`→8、`V`→7、`N`→2、`T`→1、`W`→3、`Y`→4。
> 2.  现在，在对字符串“SIX”、“SEVEN”和“TWENTY”编码后，分别修改为 650、68782 和 138214。
> 3.  因此，“SIX”、“SEVEN”和“SEVEN”的值之和等于(650+ 68782+ 68782 = 138214)，这等于字符串“TWENTY”的值。
>
> 因此，打印“Yes”。

**本文第 1 集**已经讨论过[这里](https://www.geeksforgeeks.org/c-code-article-backtracking-set-8-solving-cryptarithmetic-puzzles/)中的字符串数组大小为 `2` 的情况。

**方法:** 给定的问题可以使用回溯来解决。按照以下步骤解决问题:

*   初始化三个数组，比如 `MP[26]`、`Hash[26]` 和 `CharAtfront[26]`，来存储字母表的映射值，每个字符串中字母表的位置值之和，以及一个字符是否在任何字符串的起始索引处。
*   初始化一个数组 `used[]`，该数组用于存储一个数字是否映射到任何字母表。
*   初始化一个字符串缓冲区 `uniq` 来存储每个字母出现一次的字符串。
*   将 `-1` 分配给 `mp` 的每个数组元素。
*   使用变量 `i` 迭代数组 `arr[]`，并执行以下操作:
    *   将字符串的长度 `arr[i]` 存储在变量 `M` 中。
    *   使用变量 `j` 迭代字符串 `arr[i]`，并执行以下操作:
        *   如果 `MP[arr[i][j] - 'A']` 是 `-1`，则在 `uniq` 中追加 `arr[i][j]`，并将 `0` 分配给 `MP[arr[i][j] - 'A']`。
        *   现在用 `10^(M-j-1)` 来增加 `Hash[arr[i][j] - 'A']` 的值。
        *   如果 `arr[i].length() > 1` 和 `j` 为 `0`，然后在 `CharAtfront[arr[i][j] - 'A']` 中标记为真。
*   迭代字符串 `S`，执行与每个数组字符串相同的任务。
*   将 `-1` 填充到 `mp` 的每个数组元素中。
*   定义一个递归函数 `solve(string word, int i, int S, int[] mp, int[] used)` 进行回溯:
    *   如果 `i` 等于 `word.length()`，则返回 `true` 如果 `S` 为 `0`。否则，返回 `false`。
    *   如果 `mp[word[i] - 'A']` 不等于 `-1`，则递归调用函数 `solve(word, i+1, S + mp[word[i] - 'A'] * Hash[word[i] - 'A'], mp, used)` 然后返回其返回的值。
    *   否则，将变量 `X` 初始化为 `false`，并使用变量 `j` 迭代范围 `[0, 9]`，并执行以下操作:
        *   如果满足以下任一条件，继续循环中的下一次迭代:
            *   如果 `used[j]` 是 `true`。
            *   如果 `CharAtfront[word[i] - 'A']` 为 `1` 且 `j` 为 `0`。
        *   现在将 `used[j]` 标记为 `true`，并将 `j` 分配给 `mp[word[i] - 'A']`。
        *   以上步骤完成后，调用函数 `solve(word, i+1, S + j * Hash[word[i] - 'A'], mp, used)`，然后用其返回的值执行 `X` 的按位 OR。
        *   现在将 `used[j]` 标记为 `false`，并将 `-1` 分配给 `MP[word[i] - 'A']` 进行回溯。
    *   返回 `X` 的值。
*   最后，完成以上步骤后，如果 `solve(uniq, 0, 0, mp, used)` 返回的值为真，则打印“Yes”。否则，打印“No”。
