...
给定一个二叉查找树(BST)，将其转换为二叉树，这样原始 BST 的每个键都被更改为键加上 BST 中所有较小键的总和。给定一个有 `N` 个节点的 BST，我们必须转换成二叉树。

上面给出了 `N=5` 个节点的 BST。节点处的值为 **9、6、15、3、21**。转换后的二叉树，节点处的值为 **18，9，33，3，54**。

**解决方案:** 我们将执行一个常规的有序遍历，其中我们跟踪访问的节点的总和。让这个总和成为 `sum`。正在访问的节点，将该节点的键添加到 `sum` 中，即 `sum = sum + node->key`。将当前节点的键改为 `sum`，即 `node->key = sum`。当顺序遍历一个 BST 时，对于当前被访问的每个键，所有已经被访问的键都是较小的键。
