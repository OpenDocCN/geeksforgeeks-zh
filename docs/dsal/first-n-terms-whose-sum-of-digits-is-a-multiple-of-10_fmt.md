...
> 原文: [https://www.geeksforgeeks.org/first-n-terms-whose-sum-of-digits-is-a-multiple-of-10/](https://www.geeksforgeeks.org/first-n-terms-whose-sum-of-digits-is-a-multiple-of-10/)
...
可以观察到，要得到所需系列的第 `N` 项，求 `N` 位数之和。如果总和已经是 `10` 的倍数，则在 `N` 的末尾追加数字 `0`，否则在末尾追加最小可能数字，使得新的数字总和是 `10` 的倍数。

> 例如，要获得第 `19` 项，由于位数之和已经是 `10` 的倍数，那么追加 `0` 和 `190` 就是该系列的第 `19` 项。
> 对于 `N = 5`，可追加使位数之和为 `10` 倍数的最小位数为 `5`，`55` 为该系列的第 `5` 项。
