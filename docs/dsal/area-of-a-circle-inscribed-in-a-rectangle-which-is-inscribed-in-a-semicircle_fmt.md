...
> 原文：[https://www.geeksforgeeks.org/area-of-a-circle-inscribed-in-a-rectangle-which-is-inscribed-in-a-semicircle/](https://www.geeksforgeeks.org/area-of-a-circle-inscribed-in-a-rectangle-which-is-inscribed-in-a-semicircle/)

给定一个半径为 `R` 的半圆，该半圆内接一个长度为 `L` 和宽度为 `B` 的矩形，该矩形又内接一个半径为 `r` 的圆。任务是找到半径为 `r` 的圆的面积。
...
> 我们知道半圆内可内接的最大矩形有长度 `l=√2R/2` 和宽度 `b=R/√2`（[请参考](https://www.geeksforgeeks.org/area-of-a-square-inscribed-in-a-circle-which-is-inscribed-in-an-equilateral-triangle/)）。
