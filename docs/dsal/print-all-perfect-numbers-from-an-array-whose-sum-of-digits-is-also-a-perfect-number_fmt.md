...
> 原文：[https://www.geeksforgeeks.org/print-all-perfect-numbers-from-an-array-whose-sum-of-digits-is-also-a-perfect-number/](https://www.geeksforgeeks.org/print-all-perfect-numbers-from-an-array-whose-sum-of-digits-is-also-a-perfect-number/)

给定一个大小为 `N` 的数组 `arr[]`，任务是从一个数组中打印所有的完全数，该数组的位数总和也是一个完全数。
...
1.  声明一个函数 `isPerfect()` 检查这个数字是不是一个完全数。
2.  声明另一个函数 `sumOfDigits()` 来计算一个数的所有数字的和。
3.  遍历数组 `arr`：
    *   如果 `arr[i]` 是一个完全数：
