...
> 原文: [https://www.geeksforgeeks.org/area-square-circumscribed-circle/](https://www.geeksforgeeks.org/area-square-circumscribed-circle/)
...
> 用来计算外接圆面积的公式是:
> `2 * r^2` 其中，`r` 是正方形被圆外切的圆的半径。
> **这个公式是如何工作的？**
> 假设正方形的对角线为 `d`，边长为 `a`.
> 我们从勾股定理中知道，一个正方形的对角线是边长的√(2)倍。
> 即 `d^2 = a^2 + a^2` -> `d = 2 * a^2` -> `d = √(2) * a`
> 现在，
> `a = d / √2`
> 我们知道被圆外切的正方形对角线等于圆的直径。
> 所以正方形的面积 = `a * a`
> = `d/√(2) * d/√(2)`
> = `d^2 / 2`
> = `(2 * r)^2 / 2` (我们知道 `d = 2 * r`)
