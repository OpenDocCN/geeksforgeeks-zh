...
> 原文: [https://www.geeksforgeeks.org/sum-of-nodes-in-a-binary-tree-having-only-the-left-child-nodes/](https://www.geeksforgeeks.org/sum-of-nodes-in-a-binary-tree-having-only-the-left-child-nodes/)
...
*   使用后序遍历遍历二叉树
    *   如果`root`为空，则返回 0
    *   在左边的子树上使用[递归](https://www.geeksforgeeks.org/recursion/)，并将其答案存储在变量`left`中
    *   在右子树上使用[递归](https://www.geeksforgeeks.org/recursion/)，并将其答案存储在变量`right`中
    *   如果`root->left != NULL && root->right == NULL`，然后返回 `root->val + left + right` 的值
    *   否则返回`left + right`
