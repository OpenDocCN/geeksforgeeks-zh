...
> 原文: [https://www.geeksforgeeks.org/smallest-value-of-n-such-that-the-sum-of-all-natural-numbers-from-k-to-n-is-at-least-x/](https://www.geeksforgeeks.org/smallest-value-of-n-such-that-the-sum-of-all-natural-numbers-from-k-to-n-is-at-least-x/)

给定两个正整数 `X` 和 `K`，任务是找到 `N` 的最小值，使得范围 `[K, N]` 中所有自然数的和至少为 `X`。如果不存在 `N` 的可能值，则打印 `-1`。
...
**朴素法:** 解决这个问题最简单的方法是检查 `[K, X]` 范围内的每个值，并从该范围返回第一个值，该值至少有 `X` 个第一个 `N` 个自然数的和。
...
使用[二分搜索法](https://www.geeksforgeeks.org/binary-search/)可以优化上述途径。按照以下步骤解决给定的问题:

*   初始化一个变量，将 `res` 设为 `-1`，以存储满足给定条件的 `N` 的最小可能值。
*   初始化两个变量，`low` 为 `K`，`high` 为 `X`，通过执行以下步骤在这个范围内执行[二分搜索法](https://www.geeksforgeeks.org/binary-search/):
    *   求 `mid` 的值为 `low + (high - low) / 2`。
    *   如果[从 `K` 到 `mid` 的自然数](https://www.geeksforgeeks.org/sum-of-natural-numbers-using-recursion/)之和大于或等于 `X` 则为真。
    *   如果发现为真，则将 `res` 更新为 `mid`，并将 `high` 设置为 `(mid - 1)`。否则，将 `low` 更新为 `(mid + 1)`。
