...
> 原文：[https://www.geeksforgeeks.org/find-the-count-of-unvisited-indices-in-an-infinite-array/](https://www.geeksforgeeks.org/find-the-count-of-unvisited-indices-in-an-infinite-array/)

给定一个无限长的数组和两个整数 `M` 和 `N`，它们是互质的，任务是从第一个位置开始，当从 `arr[i]` 单次移动时，可以到达 `arr[i+M]` 或 `arr[i+N]`，找到不能访问的位置数。**注意**结果总是有限的。
...
*   使用[弗罗贝纽斯数](https://en.wikipedia.org/wiki/Coin_problem)公式 `X = (M * N) – M – N` 找到无法使用 `M` 和 `N` 任意组合获得的最大索引。
*   因为 `X` 是不能访问的最大索引，所以大于它的每个索引都不需要检查。
*   现在，对于小于 `X` 的索引，如果 `X` 不可见，那么 `Y = X – M` 和 `Z = X – N` 也不可见，同样的还有 `Y – M` 和 `Z – N` 等等，直到索引大于 `0`。
