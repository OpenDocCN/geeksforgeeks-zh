...
> **1.** 用第一种颜色给第一个顶点上色。
> **2.** 对剩余的 `V-1` 顶点执行以下操作。
> ….. **a)** 考虑当前拾取的顶点，并使用最低编号的颜色对其进行着色，该颜色尚未在与其相邻的任何先前着色的顶点上使用。如果所有以前使用的颜色都出现在与 `v` 相邻的顶点上，则为其指定一种新的颜色。
...
所以顶点的选取顺序很重要。许多人提出了不同的方法来找到比基本算法平均效果更好的排序。最常见的是`威尔士-鲍威尔算法`，它按照度数降序考虑顶点。
