...
如果任意两个连续项的比率总是相同的，那么一系列数字被称为几何级数。简单地说，它意味着数列中的下一个数是通过将一个固定数乘以数列中的前一个数来计算的。例如，2，4，8，16 是一个 GP，因为序列中任意两个连续项的比率(公差)是相同的(4 / 2 = 8 / 4 = 16 / 8 = 2)。
