...
> 原文：[https://www.geeksforgeeks.org/what-are-the-c-programming-concepts-used-as-data-structures/](https://www.geeksforgeeks.org/what-are-the-c-programming-concepts-used-as-data-structures/)

## 数据类型

简单来说，数据类型为我们提供了关于数据类型的信息。举例、整数、字符等。C 语言中的数据类型是`变量`的声明。数据类型分为：
...
名为`ch`的变量指的是内存地址 100，已经占用了 1 个字节的内存，其中保存了字符数据类型‘A’的值。`num`为整数型，指内存地址 200，已占用内存 4 字节，保存值 123456。`标记`为双型，指内存位置 300，已占用 8 字节内存，保存值 97.123456。
...
当我们定义一个结构时，没有分配内存。结构的大小等于每个数据成员消耗的空间总量。
...
*   它们用于变量的间接访问。
*   如果`var`是变量的名称，那么`&var`给出了`var`的地址。
...
这是因为我们将扫描的值分配到`var`的存储位置。我们对地址不感兴趣，但对存储在该地址的值感兴趣。
...
指针可以指向任何数据类型。它可以保存它所指向的数据类型的任何变量的地址。未初始化的指针变量的值为空。
...
要获取指针指向的地址值，我们使用星号（`*`）运算符。因此，在上面的例子中，`ptr`保存地址 250，地址处的值是 5。因此，`*ptr`等于 5。
...
这里`p`是指针，`*p`是结构。因此，要访问数据成员，我们必须使用
