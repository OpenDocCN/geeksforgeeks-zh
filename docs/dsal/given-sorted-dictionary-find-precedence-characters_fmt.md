...
2.  对给定排序数组中的每对相邻单词执行以下操作。
    a) 让当前一对单词为 `word1` 和 `word2`。逐一比较两个单词的字符，找出第一个不匹配的字符。
    b) 在 `g` 中创建一条从 `word1` 到 `word2` 的不匹配字符的边。
...
**练习:** 当输入无效时，上述代码不起作用。例如 `{"ABA", "bba", "AAA"}` 是无效的，因为从前两个词，我们可以推断 `'a'` 应该出现在 `'b'` 之前，但是从最后两个词，我们可以推断 `'b'` 应该出现在 `'a'` 之前，这是不可能的。扩展上述程序以处理无效输入，并将输出生成为 `"Invalid"`。
...
1.  一次比较两个相邻的单词（即 `word1` 和 `word2`，`word2` 和 `word3`，...，`word(startIndex)` 和 `word(startIndex+1)`）。
2.  然后，我们对所选的两个单词一次比较一个字符。
    a) 如果两个字符不同，我们在这里停止比较，并得出结论，来自 `word(startIndex)` 的字符在另一个之前。
    b) 如果两个字符相同，我们将继续进行比较直到 (2a) 出现，或者如果其中一个单词已经用完。
