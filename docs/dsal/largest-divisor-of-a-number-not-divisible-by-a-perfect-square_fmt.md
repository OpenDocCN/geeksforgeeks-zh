...
一个简单的方法是通过迭代到`N`的平方根找到给定数`N`的所有除数，并在列表中保持它们的排序顺序(降序)。这里，我们按照降序将它们插入到[集合](https://www.geeksforgeeks.org/set-in-cpp-stl/)中，以保持它们的排序。此外，通过从 1 到 10 <sup>5</sup> 的迭代，列出最多 10 个 <sup>10</sup> 的所有完美方块。
