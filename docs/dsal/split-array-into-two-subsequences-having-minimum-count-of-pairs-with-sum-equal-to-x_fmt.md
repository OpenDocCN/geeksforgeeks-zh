...
> 原文: [https://www.geeksforgeeks.org/split-array-into-two-subsequences-having-minimum-count-of-pairs-with-sum-equal-to-x/](https://www.geeksforgeeks.org/split-array-into-two-subsequences-having-minimum-count-of-pairs-with-sum-equal-to-x/)

给定一个由 `N` 个整数和一个整数 `X` 组成的数组 `arr[]`，任务是将该数组拆分为两个子序列，使得两个数组中总和等于 `X` 的对的数量最小。
