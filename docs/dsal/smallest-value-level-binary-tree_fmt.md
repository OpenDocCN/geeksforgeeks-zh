...
> 原文：[https://www.geeksforgeeks.org/smallest-value-level-binary-tree/](https://www.geeksforgeeks.org/smallest-value-level-binary-tree/)
...
# into vector array.
def printPerLevelMinimum(root, res, level):
    if (root != None):
...
def perLevelMinimumUtility(root):
    ...
    res = printPerLevelMinimum(root, res, 0)
...
方法:- 想法是使用队列执行二叉树的迭代层级顺序遍历。遍历时使用 `min` 变量，该变量存储正在处理的树的当前级别的最小元素。当层级完全通过时，打印该最小值。
