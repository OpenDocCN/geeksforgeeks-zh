...
> 原文: [https://www.geeksforgeeks.org/range-product-queries-in-an-array/](https://www.geeksforgeeks.org/range-product-queries-in-an-array/)
...
一种简单的方法是从下到上遍历数组中的每个元素，并将所有元素相乘以找到乘积。每个查询的时间复杂度为 `O(n)`。

一种有效的方法是获取另一个数组，并将所有元素的乘积存储到数组第 `i` 个索引中索引为 `i` 的元素。但是这里有个问题。如果数组中的任何元素为 `0`，则 `dp` 数组从那时起将包含 `0`。因此，我们应该得到 `0` 作为我们的答案，即使 `0` 不在下限和上限之间。

为了解决这种情况，我们使用了另一个名为 `countZeros` 的数组，该数组将包含“0”的数字，该数组包含直到索引 `i`。

现在，如果 `countZeros[upper] - countZeros[lower] > 0`，则表示该范围内有 `0`，因此答案将是 `0`。否则我们将照此进行。
...
**时间复杂度:** `O(n)`在创建 dp 数组期间和`O(1)`每个查询