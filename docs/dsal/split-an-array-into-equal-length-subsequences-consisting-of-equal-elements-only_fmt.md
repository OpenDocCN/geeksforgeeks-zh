...
> 原文:[https://www.geeksforgeeks.org/split-an-array-into-equal-length-subsequences-consisting-of-equal-elements-only/](https://www.geeksforgeeks.org/split-an-array-into-equal-length-subsequences-consisting-of-equal-elements-only/)
...
思路基于以下观察:让`arr[i]`的频率为 `C_i`，那么这些元素必须分解为 `X` 的子序列，使得 `C_i % X = 0`。对于每个索引 `i`，该值必须为是。为此， `X` 的值应等于所有 `C_i (1≤i≤N)` 的[最大公约数(GCD)](https://www.geeksforgeeks.org/c-program-find-gcd-hcf-two-numbers/)。如果 `X` 大于 1，则打印是，否则打印否。
...
*   将`mp`中所有频率的[最大公约数存储在变量 `X` 中。](https://www.geeksforgeeks.org/gcd-two-array-numbers/)
*   如果 `X` 大于 1，那么答案是肯定的。
*   否则，答案是否定的。
