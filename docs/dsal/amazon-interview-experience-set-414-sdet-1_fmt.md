...
### 问题 1
Simulate an Android mode lock.

### 问题 2
[Generate all words](https://practice.geeksforgeeks.org/problems/possible-words-from-phone-digits/0) that can be formed in the given pattern. The keyboard specification is mobile keyboard. I will explain it clearly.
...
### 问题 1
[Generate all possible test cases for an addition function that takes two strings as input and returns an integer, which is the sum of two numbers given as string input](https://practice.geeksforgeeks.org/problems/key-pair/0) .

我为这个问题给出了大约 10 个测试用例，因为它非常直接。我通过使用 `JUnit` 测试编写了一些代码，这非常简单。

### 问题 2
给定编辑器中的打开文件模块，打开文件模块可能会出现哪些错误。
...
### 问题 1
[Knight's position problem](https://practice.geeksforgeeks.org/problems/steps-by-knight/0) .
...
### 问题 2
[给定一个链表，找到两个链表的交点，这两个链表汇聚在一个公共点](https://practice.geeksforgeeks.org/problems/intersection-point-in-y-shapped-linked-lists/1)。
...
### 问题 1
[Given a set of integers, print pairs that add up to the given sum.](https://practice.geeksforgeeks.org/problems/key-pair/0)
...
### 问题 2
[给定一棵二叉树打印它的底视图](https://practice.geeksforgeeks.org/problems/bottom-view-of-binary-tree/1)。
...
### 问题 1
[Given a binary tree, print the Kth largest element](https://practice.geeksforgeeks.org/problems/kth-largest-element-in-bst/1) .

我只是立即开始了一个蛮力解决方案，通过寻找树的有序遍历。然后对数组进行排序，找到排序后的数组中第 `k` 个最大的元素，也就是从数组的起始索引开始的第 `(n-k)` 个元素，其中 `n` 是树中的节点总数。这使用了 `O(n)` 时间和 `O(n)` 空间。
...
### 问题 2
[给定一个有向图，找出是否存在循环。如果它确实存在，打印出来。](https://practice.geeksforgeeks.org/problems/detect-cycle-in-a-directed-graph/1)
