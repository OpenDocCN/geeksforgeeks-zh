...
给定一个非负数 `n` 和两个值 `l` 和 `r`。问题是在 `n` 的二进制表示中切换 `l` 到 `r` 范围内的位，即从最右边的 `l` 位切换到最右边的 `r` 位。拨动操作将一位 `0` 翻转至 `1`，一位 `1` 翻转至 `0`。
...
1.  将 `num` 计算为 `((1<<r)–1)^((1<<(l-1))–1)` 或 `((1 << r)-l)`。这将产生一个数字 `num`，其具有 `r` 数量的比特，并且范围 `l` 到 `r` 中的比特是唯一设置的比特。
2.  现在，执行 `n = n ^ num`。这将切换 `l` 到 `r` 到 `n` 范围内的位。
