...
> 原文：[https://www.geeksforgeeks.org/unique-element-in-an-array-where-all-elements-occur-k-times-except-one-set-2/](https://www.geeksforgeeks.org/unique-element-in-an-array-where-all-elements-occur-k-times-except-one-set-2/)
...
假设我们有每个元素 `K` 次，那么给定数组中所有元素的**和**与所有唯一元素的 `K * 和` 之差就是 `(K-1)次` 唯一元素。
...
上述问题可以根据恒定的空间复杂度进行优化。使用[位操纵](https://www.geeksforgeeks.org/bits-manipulation-important-tactics/)方法解决此问题。
...
我们的结果是 `(100)₂ = 4₁₀`。所以最后的答案会是 `4`，因为它呈现了 `1` 次。
