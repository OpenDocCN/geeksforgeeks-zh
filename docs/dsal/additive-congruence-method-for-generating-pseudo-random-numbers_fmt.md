...
> 原文: [https://www.geeksforgeeks.org/additive-congruence-method-for-generating-pseudo-random-numbers/](https://www.geeksforgeeks.org/additive-congruence-method-for-generating-pseudo-random-numbers/)
...
> **m** (> 0)：模数
> **c** ∈ [0, m)：增量
> **X<sub>0</sub>** ∈ [0, m)：序列初始值 – 称为**种子**

应适当选择 `m`、`c`、`X_0`，以获得几乎等于 `m` 的周期。

**方法步骤:**

*   选择种子值 `X_0`，模量参数 `m`，增量项 `c`。
*   初始化需要生成的随机数数量（比如，一个整数变量 `nofrandomnums`）。
*   定义存储以保持生成的随机数（这里，考虑 `vector`）的大小 `nofrandomnums`。
*   用种子值初始化向量的第 0 个索引。
