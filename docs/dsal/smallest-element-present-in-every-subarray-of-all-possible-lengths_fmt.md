...
> 原文: [https://www.geeksforgeeks.org/smallest-element-present-in-every-subarray-of-all-possible-lengths/](https://www.geeksforgeeks.org/smallest-element-present-in-every-subarray-of-all-possible-lengths/)

## 问题描述

给定一个长度为 `N` 的数组 `arr[]`，任务是对于每一个可能长度的子数组，找到该长度的每个子数组中存在的最小元素。
...
**解释:**
对于长度= 1，长度为 1 的每个子数组中没有共同的元素。因此，输出为-1。
对于长度= 2，在长度为 2 的每个子数组中没有共同的元素。因此，输出为-1。
对于长度= 3，每个子数组中的公共元素为 3。因此，输出为 3。
对于长度= 4，2 和 3 在长度为 4 的每个子数组中都是常见的。2 是较小的，是所需的输出。
同样，对于长度 5 和 6，这些长度的每个子数组中最小的公共元素是 2。
对于长度 7、8、9 和 10，这些长度的每个子数组中最小的公共元素是 1。
...
思路是针对 `K` ( 1 ≤ K ≤ N)的每个可能值，在所有大小为 `K` 的子数组中找到公共元素，打印出最小的公共元素。按照以下步骤解决问题:

1.  将长度为 `K` 的每个子数组的每个唯一数字的计数相加。
2.  检查计数是否等于子数组的数量，即 `N–K+1`。
3.  如果发现是真的，那么该特定元素出现在大小为 `K` 的每个子数组中。
...
如果使用一个数组存储数组中存在特定数字的所有索引，并找到最小长度，使其出现在每个长度子数组中 `1 ≤ K ≤ N`，则可以优化上述方法。

按照以下步骤解决问题:

1.  初始化一个数组，比如说`indices[]`，以存储与该索引号对应的特定数字所在的索引。
2.  现在，对于给定数组中的每个数字，求最小长度，使它出现在该长度的每个子数组中。
3.  最小长度可以通过找到给定数组中特定数字重复的最大间隔来找到。同样，对于数组的其他数字也要找到相同的值。
4.  用 `-1` 初始化大小为 `N+1` 的`answer[]`数组，其中`answer[i]`代表长度为 `K` 的子数组的答案。
5.  现在，`indices[]`数组给出了每个长度子数组中存在的数字，比如说 `K`，然后如果`answer[K]`是 `-1`，则用相同的数字更新`answer[K]`。
6.  遍历之后，更新`answer[]`数组，这样如果一个数字出现在所有长度为 `K` 的子数组中，那么这个特定的数字也会出现在所有长度大于 `K` 的子数组中。
7.  更新`answer[]`数组后，打印该数组中存在的所有元素，作为每个长度子数组 `K` 的答案。
