...
> 原文: [https://www.geeksforgeeks.org/modify-a-string-by-performing-given-shift-operations/](https://www.geeksforgeeks.org/modify-a-string-by-performing-given-shift-operations/)

给定一个包含小写英文字母的[字符串](https://www.geeksforgeeks.org/string-data-structure/) `S`，以及一个由成对的形式 `{方向，量}` 组成的[矩阵](https://www.geeksforgeeks.org/matrix/) `shift[][]`，其中方向可以是 `0`（用于左移位）或 `1`（用于右移位），`量` 是字符串 `S` 需要移位的索引数。任务是返回执行给定操作后可以获得的修改后的字符串。
...
解决问题最简单的方法是[遍历矩阵](https://www.geeksforgeeks.org/traverse-a-given-matrix-using-recursion/) `shift[][]`，按指定方向的指数数量移位 `S[0]`。完成所有移位操作后，打印获得的最终字符串。
...
*   [遍历矩阵](https://www.geeksforgeeks.org/row-wise-vs-column-wise-traversal-matrix/) `shift[][]` 并在每 `i` 行执行以下操作:
    *   如果 `shift[i][0] = 0`（*左档*），则通过 `-shift[i][1]` 降低 `val`。
    *   否则（*左移*），通过 `shift[i][1]` 增加 `val`。
*   更新 `val = val % len`（进一步优化有效班次）。
*   初始化一个[字符串](https://www.geeksforgeeks.org/string-data-structure/) `result = ""`，存储修改后的字符串。
*   现在，检查 `是否 val > 0`。如果发现是真的，则通过 `val` 在弦上执行正确的旋转。
*   否则，按 `|val|` 量向左旋转琴弦。
*   打印 `result`。
