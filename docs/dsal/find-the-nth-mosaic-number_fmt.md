...
给定一个整数 `N`，任务是找到第 `N` 个[马赛克号](http://oeis.org/A000026)。一个马赛克号可以表示如下：
如果 `N = A^A * B^B * C^C…` 其中 `A`，`B`，`C` ..是 `N` 的质因数，那么第 `N` 个马赛克号将是 `A * A * B * B * C * C……`。
...
**方法:** 我们必须通过将数除以因子，直到因子除以数，来找到数中所有的素因子以及因子的幂。第 `N` 个马赛克号将是已发现的主要因素及其力量的产物。
