...
> 原文：[https://www.geeksforgeeks.org/lcs-formed-consecutive-segments-least-length-k/](https://www.geeksforgeeks.org/lcs-formed-consecutive-segments-least-length-k/)
...
创建 `LCS[][]` 数组，其中 `LCS[i][j]` 表示由长度至少为 `k` 的连续段的 `s1` 到 `i` 和 `s2` 到 `j` 的字符形成的最长公共子序列的长度。`cnt[i][j] = cnt[i-1][j-1] + 1` 当 `s1[i-1]==s2[j-1]` 时。如果字符不相等，则段不相等，因此将 `cnt[i][j]` 标记为 `0`。
当 `cnt[i][j] >= k` 时，则通过将 `lcs[i-a][j-a]` 的值相加来更新 `lcs` 值，其中 `a` 为线段的长度 `a <= cnt[i][j]`。长度至少为 `k` 的连续片段的最长子序列的答案将存储在 `lcs[n][m]` 中，其中 `n` 和 `m` 是 `string1` 和 `string2` 的长度。
