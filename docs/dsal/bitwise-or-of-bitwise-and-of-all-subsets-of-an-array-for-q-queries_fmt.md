...
> 原文: [https://www.geeksforgeeks.org/bitwise-or-of-bitwise-and-of-all-subsets-of-an-array-for-q-queries/](https://www.geeksforgeeks.org/bitwise-or-of-bitwise-and-of-all-subsets-of-an-array-for-q-queries/)

给定两个大小为 `N` 的 `arr[]` 和大小为 `Q` 的查询，任务是找到数组的子集的按位与的按位或。在每个查询中，给你一个索引和值，你必须用给定值替换数组给定索引处的值，并在每个查询后打印数组所有子集的按位与的按位或。

**示例:**

> **输入:** `arr[] = {3, 5, 7}`，`N = 3`，`queries[] = {{1, 2}, {2, 1}}`，`Q = 2`
> **输出:** 7
> 3
> **解释:**
> 对于第一个查询将索引 1 处的值替换为值 2，更新后的数组 `arr[]` 为 `{3, 2, 7}`。
> 然后取所有子集的 AND，即 `AND(3) = 3`，`AND(2) = 2`，`AND(7) = 7`，`AND(3, 2) = 2`，`AND(3, 7) = 3`，`AND(3, 2, 7) = 2`。
> 所有子集的“与”的“或”为 `OR(3, 2, 7, 2, 3, 2) = 7`。
> 现在，对于第二个查询用值 1 替换索引 2 处的值，更新后的数组 `arr[]` 为 `{3, 2, 1}`。
> 然后取所有子集的 AND，即 `AND(3) = 3`，`AND(2) = 2`，`AND(1) = 1`，`AND(3, 2) = 2`，`AND(3, 1) = 1`，`AND(3, 2, 1) = 0`。
> 所有子集 `OR(3, 2, 1, 2, 1, 0) = 3` 的“与”的“或”。

> **输入:** `arr[] = {1, 2, 3}`，`N = 3`，`queries[] = {{2, 4}, {1, 8}}`，`Q = 2`
> **输出:** 7
> 13

**方法:** 这个问题可以用贪婪算法解决。按照以下步骤解决问题:

*   初始化大小为 32 的数组 `bits[]`，并存储所有元素的设置位的计数。
*   使用变量 `p` 在范围 `[0, Q-1]` 中迭代
    *   首先减去先前值的位，然后加上新值的位。
    *   使用变量 `i` 在范围 `[0, 31]` 中迭代
        *   如果当前位被设置为前一个值，则用索引处的 `bits[]` 数组中减去一位。
        *   如果当前位被设置为新值，则在具有索引的 `bits[]` 数组中添加一位。
    *   用给定数组中的前一个值替换新值 `arr[]`。
    *   初始化一个变量 `ans` 来存储 `bits[]` 数组的按位或。
    *   使用变量 `i` 在范围 `[0, 31]` 中迭代
        *   如果当前位不等于 0，则将当前位的按位或加到 `ans` 中。
    *   完成上述步骤后，打印 `ans` 作为每个查询所需的答案。
