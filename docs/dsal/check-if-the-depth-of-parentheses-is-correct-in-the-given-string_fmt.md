...
> 原文：[https://www.geeksforgeeks.org/check-if-the-depth-of-parentheses-is-correct-in-the-given-string/](https://www.geeksforgeeks.org/check-if-the-depth-of-parentheses-is-correct-in-the-given-string/)

给定一个由左圆括号、右圆括号和整数组成的字符串 `S`，如果在这个字符串中，整数正确地指示了它的深度，那么任务就是打印 `Yes`。深度是指围绕该整数的嵌套括号集的数量。否则打印 `No`。
...
1.  逐个字符地迭代字符串
2.  如果是左括号或右括号，则追加到数组 `arr[]` 中
3.  如果字符是数字，则迭代直到整个数字被读取，然后追加到 `arr[]` 中
4.  逐元素迭代数组 `arr[]`
    *   如果元素是 `'('`，增加 `depth` 并 `open` 加 1
    *   如果元素为 `')'`，则 `depth` 减 1，`close` 加 1
    *   如果元素是整数，检查“整数 != `depth`”，如果为真，将 `flag` 设置为 0 并中断循环
