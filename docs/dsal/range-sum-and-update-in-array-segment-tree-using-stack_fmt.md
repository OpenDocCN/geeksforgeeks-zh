...
> 原文: [https://www.geeksforgeeks.org/range-sum-and-update-in-array-segment-tree-using-stack/](https://www.geeksforgeeks.org/range-sum-and-update-in-array-segment-tree-using-stack/)
...
> 给定范围内的更新值之和= 114
> **解释:**
> 给定范围内的值之和 `2 至 6` 为 5 + 7 + 19 + 11 + 8 = 50。
...
2.  **为建筑段树:**
    *   将根节点作为元组推送到[栈](https://www.geeksforgeeks.org/stack-data-structure/)
...
*   从[堆栈](https://www.geeksforgeeks.org/stack-data-structure/)弹出元素，并执行以下操作，直到[堆栈](https://www.geeksforgeeks.org/stack-data-structure/)变为空:
    ...
    2.  否则，插入当前节点为的标志元组，放置 `(current_node，INF，INF)` 以反转求值顺序，并插入左右子节点值为:

> `mid = (start + end) / 2`
> `S.emplace(curr_node * 2, start, mid)`
> `S.emplace(curr_node * 2 + 1, mid + 1, end)`

3.  如果开始索引和结束索引与 `INF` 相同，那么将当前索引处的[段树](https://www.geeksforgeeks.org/segment-tree-set-1-sum-of-given-range/)值更新为:
...
*   对于查询 : O(log N)，要查询一个和，我们在每个级别处理四个节点，级别数为 log N
*   对于更新 : O(log N)，用惰性传播更新树是 O(log N)，因为我们更新树的根，然后只更新树的范围在每个级别重叠的那部分。