...
> 原文:[https://www.geeksforgeeks.org/check-if-an-array-can-be-sorted-by-swapping-pairs-from-indices-consisting-of-unequal-elements-in-another-array/](https://www.geeksforgeeks.org/check-if-an-array-can-be-sorted-by-swapping-pairs-from-indices-consisting-of-unequal-elements-in-another-array/)

给定大小为 `N` 的数组 `A[]` 和大小为 `N` 的二进制数组 `B[]`，任务是检查如果 `B[i]` 不等于 `B[j]` 的话，数组 `A[]` 是否可以通过交换对 `(A[i], A[j])` 转换为排序数组。如果数组 `A[]` 可以排序，则打印“是”。否则，打印“否”。
...
> **输出:** 是
> **说明:**
> 从 `B[1] != B[2]` 开始交换 `A[]` 的位置 1 和位置 2 的元素。所以，`A[] = {1, 3, 2}`，`B[] = {1, 0, 1}`
> 现在，从 `B[2] != B[3]` 开始交换 `A[]` 的位置 2 和位置 3 的元素。所以，`A[] = {1, 2, 3}`。因此，它被排序。
>
> **输入:** `A[] = {5, 15, 4}`，`B[] = {0, 0, 0}`
> **输出:** 否
...
*   检查给定数组 `A[]` 是否已经按照升序排序。如果发现属实，则打印“是”。
