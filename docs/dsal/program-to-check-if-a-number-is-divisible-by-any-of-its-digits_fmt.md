...
> 原文：[https://www.geeksforgeeks.org/program-to-check-if-a-number-is-divisible-by-any-of-its-digits/](https://www.geeksforgeeks.org/program-to-check-if-a-number-is-divisible-by-any-of-its-digits/)

给定一个整数 `N`，其中 $1 \leq n \leq 10^{18}$。任务是检查这个数是否不能被它的任何一个数字整除。如果给定的数字 `N` 可以被它的任何一个数字整除，则打印“是”，否则打印“否”。
