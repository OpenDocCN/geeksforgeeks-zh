...
> 原文: [https://www.geeksforgeeks.org/reduce-sum-of-any-subset-of-an-array-to-1-by-multiplying-all-its-elements-by-any-value/](https://www.geeksforgeeks.org/reduce-sum-of-any-subset-of-an-array-to-1-by-multiplying-all-its-elements-by-any-value/)

给定一个由 `N` 个正整数组成的数组 `arr[]`，任务是检查给定数组的任意子集的元素之和在将其所有元素乘以任意整数后是否可以简化为 `1`。如果不可能，则打印 `"否"`。否则，打印 `"是"`。
...
最简单的方法是生成给定数组的所有可能子集，如果数组中存在任何子集，那么它的元素之和在乘以任何整数后等于 `1`，然后打印 `"是"`。否则，打印 `"否"`。
...
上述方法也可以通过使用贝祖特的恒等式（贝祖特引理）进行优化，其中规定如果任意两个整数 `a` 和 `b` 的 GCD 等于 `d`，则存在整数 `x` 和 `y`，使得 `a * x + b * y = d`。

因此，想法是检查给定数组 `arr[]` 的 GCD 是否可以做成 `1`。因此，要满足给定的条件，必须存在任意两个元素的 GCD 为 `1`，那么数组的 GCD 将等于 `1`。因此，打印 `"是"`。否则，打印 `"否"`。
