# 求解范围 [0, N-1] 内 x 值的线性同余 Ax = B (mod N)

> 原文: [https://www.geeksforgeeks.org/solve-linear-congruences-ax-b-mod-n-for-values-of-x-in-range-0-n-1/](https://www.geeksforgeeks.org/solve-linear-congruences-ax-b-mod-n-for-values-of-x-in-range-0-n-1/)

## 问题描述

给定三个正整数 `A`、`B` 和 `N`，它们表示形式 `AX=B (mod N)` 的一个[线性同余](https://www.geeksforgeeks.org/linear-congruence-method-for-generating-pseudo-random-numbers/)，任务是打印 `X (mod N)` 的所有可能值，即在满足此等式的范围 `[0, N-1]` 内。如果没有解决方案，请打印 -1。
...
该想法基于以下观察:
*   当且仅当 `B` 可被 [`GCD(A, N)`](https://www.geeksforgeeks.org/basic-and-extended-euclidean-algorithms/) 整除，即 `B%GCD(A, N)=0` 时，解存在。
*   `X (mod N)` 的解数为 `GCD(A, N)`。

## 证明

1.  Given that `ax = b (mod n)` exists a number `y` so that `ax = b+ny`，即 `ax-ny = b` (1)。这是一个[线性丢番图方程](https://www.geeksforgeeks.org/linear-diophantine-equations/)。
2.  Now, using [扩展欧几里得算法](https://www.geeksforgeeks.org/euclidean-algorithms-basic-and-extended/)，`u` and `v` can be found as follows: `Au+NV = GCD(a, n) = D` (say)。
3.  Suppose `d = GCD(a, n)`。因此，`u * b/d` 是方程 (1) 的解。
4.  Let `x0` become `u * b/d`。因此，方程 (1) 的 `d` 个解将是 `x0, x0+(N/d), x0+2*(N/d), ..., x0+(d-1)*(n/d)`。

## 解决步骤

按照以下步骤解决问题:
*   使用[扩展欧几里得算法](https://www.geeksforgeeks.org/euclidean-algorithms-basic-and-extended/)将变量 `d` 初始化为 `GCD(A, N)` 以及 `u`。
