...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-possible-strings-obtained-by-removal-of-non-empty-substrings/](https://www.geeksforgeeks.org/sum-of-all-possible-strings-obtained-by-removal-of-non-empty-substrings/)

给定由 `N` 个整数组成的数值字符串 `str`，任务是在移除非空的子字符串后，找到所有可能的结果字符串的和。
...
*   预先计算 10 的幂并存储在数组 `power[]` 中。
*   将给定数值串的数字的前缀和存储在数组 `pref[]` 中。
