...
> 原文：[https://www.geeksforgeeks.org/program-to-find-the-maximum-element-in-a-matrix/](https://www.geeksforgeeks.org/program-to-find-the-maximum-element-in-a-matrix/)
...
想法是使用两个嵌套循环遍历矩阵，一个用于行，一个用于列，并找到最大元素。用最小值初始化一个变量 `maxElement`，遍历矩阵，如果当前元素大于 `maxElement`，则每次进行比较。如果是，那么用当前元素更新 `maxElement`。
