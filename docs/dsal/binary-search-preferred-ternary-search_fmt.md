...
> 原文：[https://www.geeksforgeeks.org/binary-search-preferred-ternary-search/](https://www.geeksforgeeks.org/binary-search-preferred-ternary-search/)
...
在二分搜索法，最坏的情况下有 `2Log`<sub>2</sub>`n + 1` 的比较。在三元搜索中，最坏情况下有 `4Log`<sub>3</sub>`n + 1` 个比较。
...
因此，三进制和二进制搜索的比较归结为表达式 `2Log`<sub>3</sub>`n` 和 `Log`<sub>2</sub>`n` 的比较。`2Log`<sub>3</sub>`n` 的值可以写成 `(2 / Log`<sub>2</sub>`3) * Log`<sub>2</sub>`n`。由于 `(2 / Log`<sub>2</sub>`3)` 的值大于 1，所以在最坏的情况下，三进制搜索比二分搜索法做更多的比较。
