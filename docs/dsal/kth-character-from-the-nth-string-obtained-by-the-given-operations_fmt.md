...
> 原文：[https://www.geeksforgeeks.org/kth-character-from-the-nth-string-obtained-by-the-given-operations/](https://www.geeksforgeeks.org/kth-character-from-the-nth-string-obtained-by-the-given-operations/)

给定两个正整数 `N` 和 `K`，任务是通过对字符串 `S`（最初为 `"A"`）执行 `N` 次以下操作得到的字符串的第 `K` 个字符。

> 每次第 `i` 次操作都会生成以下字符串（`S_i`）：
> `S_i = S_{i-1} + 'B' + rev(comp(S_{i-1}))`
> 其中，
> `comp()` 表示字符串的补码，即 `A` 改为 `B`，`B` 改为 `A`。
> `rev()` 返回字符串的反向。
...
**方法：** 想法是使用[递归](https://www.geeksforgeeks.org/recursion/)从先前生成的字符串生成新的字符串，并重复该过程，直到执行 `N` 次操作。以下是步骤：

1.  将两个字符串 `prev` 初始化为 `"A"`，将 `curr` 初始化为空字符串。
2.  如果 `N = 1` 则返回 `A`，否则执行以下操作。
3.  循环 `(N-1)` 次，每次更新 `curr` 为 `prev + "B"`。
4.  反转字符串 `prev`。
5.  再次将字符串 `curr` 更新为 `curr + prev`。
6.  将字符串 `prev` 更新为 `curr`。
7.  完成上述步骤后，返回字符串 `curr` 的第 `(K-1)` 个字符。
