...
设 `A[][]` 为图的邻接矩阵表示。如果我们计算 `A^3`，那么无向图中三角形的个数等于 `trace(A^3) / 6`。其中 `trace(A)` 是矩阵 `A` 主对角线上元素的和。
...
## 这是如何工作的？
如果我们为图的邻接矩阵表示计算 `A^n` ，那么值 `A^n[i][j]` 表示图中顶点 `i` 到 `j` 之间的不同步数。在 `A^3` 中，我们得到每对顶点之间长度为 3 的所有不同路径。
三角形是长度为三的循环路径，即在同一个顶点开始和结束。所以 `A^3[i][i]` 代表一个以顶点 `i` 开始和结束的三角形，由于一个三角形有三个顶点，并且是为每个顶点计数的，所以我们需要将结果除以 3。此外，由于图是无向的，每个三角形都是 `i-p-q-i` 和 `i-q-p-i` 的两倍，所以我们也除以 2。所以三角形的数量是 `trace(A^3) / 6`。

## 时间复杂度
上述算法的时间复杂度为 `O(V^3)` 其中 `V` 为图中顶点数，我们可以利用[斯特拉森矩阵乘法](https://www.geeksforgeeks.org/strassens-matrix-multiplication/)算法将性能提升到 `O(V^2.8074)`。

## 另一种方法
使用[位集](https://www.geeksforgeeks.org/c-bitset-and-its-application/)作为邻接表。
...
## 时间复杂度
首先我们有两个用于嵌套循环的 `O(V^2)` 流的位集运算 `&` 和 `count`，两者的时间复杂度都为 `O(V / Word RAM)`，其中 `V` =图中的节点数，`Word RAM` 通常为 32 或 64。所以最后的时间复杂度是 `O(V^2 * V / 32)` 或者 `O(V^3)`。

## 参考文献

[http://www.d.umn.edu/math/Technical%20reports/Technical%20reports%202007-/TR%202012/yang.pdf](https://scse.d.umn.edu/about/departments-and-programs/mathematics-statistics-department)
