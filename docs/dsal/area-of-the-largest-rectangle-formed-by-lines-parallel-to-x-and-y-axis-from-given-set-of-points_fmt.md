...
> 原文: [https://www.geeksforgeeks.org/area-of-the-largest-rectangle-formed-by-lines-parallel-to-x-and-y-axis-from-given-set-of-points/](https://www.geeksforgeeks.org/area-of-the-largest-rectangle-formed-by-lines-parallel-to-x-and-y-axis-from-given-set-of-points/)

给定一个由代表 `N` 点坐标的 `N` 对整数组成的数组 `arr[]`，任务是从给定的一组点中找出由平行于 `X` 和 `Y` 轴的直线形成的最大矩形的面积。
...
**方法:** 使用排序技术可以解决问题。按照以下步骤解决问题:

1.  将 `X` 和 `Y` 坐标存储在两个不同的数组中，比如说 `x[]` 和 `y[]`。
2.  排序数组 `x[]` 和 `y[]`。
