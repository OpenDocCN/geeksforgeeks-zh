...
*   [遍历给定数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]`，并执行以下步骤:
    *   找到 `log2(arr[i])` 的值并存储在变量中，比如 `power`。
    *   找到 `2^(power)` 和 `2^(power + 1)` 的值，并存储在变量中，分别说 `LesserValue` 和 `LargerValue`。
    *   如果 `(arr[i] - LesserValue)` 的值等于 `(LargerValue - arr[i])`，那么将 `res` 的值增加 `arr[i]`。
