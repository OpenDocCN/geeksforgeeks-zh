...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-divisors-from-1-to-n-set-3/](https://www.geeksforgeeks.org/sum-of-all-divisors-from-1-to-n-set-3/)
...
*   因此，通过公式 `x * floor(N / x)` 计算每个 `x` 的贡献。
*   可以观察到 `floor(N/i)` 对于一系列连续的数字 `l1, l2, l3, …, lr` 是相同的。因此，与其计算每个 `i` 的 `li * floor(N/li)`，不如计算 `(l1 + l2 + l3 + … + lr) * floor(N/l1)`，降低了计算复杂度。
