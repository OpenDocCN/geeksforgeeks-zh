...
> 原文: [https://www.geeksforgeeks.org/sum-of-lengths-of-all-12-edges-of-any-rectangular-parallelepiped/](https://www.geeksforgeeks.org/sum-of-lengths-of-all-12-edges-of-any-rectangular-parallelepiped/)
...
给定的区域是 `s1`、`s2` 和 `s3`。设 `a`、`b` 和 `c` 是有一个公共顶点的边的长度。其中 `s1 = a * b`、`s2 = b * c`、`s3 = c * a`。很容易找到脸面积的长度: `a = \sqrt{s1s3/s2}`、`b = \sqrt{s1s2/s3}`、`c = \sqrt{s2s3/s1}`。答案将是所有 4 条边的总和，有 4 条边的长度等于 `a`、`b` 和 `c`。
...
**参考资料:** [https://en.wikipedia.org/wiki/Parallelepiped](https://en.wikipedia.org/wiki/Parallelepiped)