...
> 原文：[https://www.geeksforgeeks.org/python3-program-for-segregate-0s-and-1s-in-an-array/](https://www.geeksforgeeks.org/python3-program-for-segregate-0s-and-1s-in-an-array/)
...
感谢纳文提出这个方法。
1. 统计 0 的个数。让计数为 `C`。
2. 一旦我们有了计数，我们可以将 `C` 个 0 放在数组的开头，将 1 放在剩余的 `n–C` 位置。
...
维护两个指标。将第一索引 `left` 初始化为 0，将第二索引 `right` 初始化为 `n-1`。
当 `left` < `right`
a) 保持递增索引 `left` 当其中有 0 时
b) 保持递减索引 `right` 当其中有 1 时
c) 如果 `left` < `right`，则交换 `arr[left]` 和 `arr[right]`
...
1. 取两个指针 `type0`（对于元素 0）从开始（索引=0）开始，`type1`（对于元素 1）从结束（索引=`array.length-1`）开始。
