...
> 原文: [https://www.geeksforgeeks.org/check-if-a-number-can-be-expressed-as-product-of-a-prime-and-a-composite-number/](https://www.geeksforgeeks.org/check-if-a-number-can-be-expressed-as-product-of-a-prime-and-a-composite-number/)

给定一个数字 `N`，任务是检查 `N` 是否可以表示为一个[素数](https://www.geeksforgeeks.org/prime-numbers/)和一个复合数的乘积。如果可以，那么打印`是`，否则`否`。
...
这个问题可以借助[厄拉多塞算法的筛](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)来解决。现在，要解决这个问题，请遵循以下步骤:

1.  创建一个布尔数组 `isPrime`，其中 `isPrime[i]` 为`真`如果是素数，否则是`假`。
2.  使用筛选算法找到所有质数直到 `N`。
3.  现在为 `i=2` 到 `i < N` 运行一个[循环](https://www.geeksforgeeks.org/loops-in-c-and-cpp/)，在每次迭代中:
    *   检查这两种情况:
        *   如果 `N` 被 `i` 整除。
        *   如果 `i` 是素数而 `N/i` 不是，或者如果 `i` 不是素数而 `N/i` 是。
    *   如果以上两个条件都满足，返回`真`。
    *   否则，返回`假`。
4.  打印答案，根据以上观察。
