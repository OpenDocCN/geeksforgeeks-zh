...
> 原文：[https://www.geeksforgeeks.org/largest-sub-string-where-all-the-characters-appear-at-least-k-times/](https://www.geeksforgeeks.org/largest-sub-string-where-all-the-characters-appear-at-least-k-times/)

给定一个字符串 `str` 和一个整数 `K`，任务是找到最长子字符串 `S'` 的长度，使得 `S'` 中的每个字符至少出现 `K` 次。

> **输入：** `s = "xyyz"`，`k = 2`
> **输出：** 5
> “xyy”是最长的子串，其中每个字符至少出现两次。
