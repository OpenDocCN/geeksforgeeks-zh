...
> 原文：[https://www.geeksforgeeks.org/find-next-greater-element-with-no-consecutive-1-in-its-binary-representation/](https://www.geeksforgeeks.org/find-next-greater-element-with-no-consecutive-1-in-its-binary-representation/)

给定 `Q` 个查询，其中每个查询由整数 `N` 组成，任务是找到大于 `N` 的最小整数，使得在其二进制表示中没有连续的 `1s`。
