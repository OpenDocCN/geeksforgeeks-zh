...
> 原文: [https://www.geeksforgeeks.org/java-program-for-reversing-a-doubly-linked-list/](https://www.geeksforgeeks.org/java-program-for-reversing-a-doubly-linked-list/)

给定一个[双向链表](https://www.geeksforgeeks.org/doubly-linked-list/)，任务是反转给定的双向链表。
...
这里有一个简单的方法来反转双向链表。我们所需要做的就是交换所有节点的`prev`和`next`指针，改变头（或开始）节点的`prev`，最后改变`head`指针。
...
我们也可以交换数据而不是指针来反转双向链表。[反转数组的方法](https://www.geeksforgeeks.org/write-a-program-to-reverse-an-array-or-string/)可以用来交换数据。与指针相比，如果数据项的大小更大，交换数据的成本会更高。
...
class LinkedList {
    ...
    void reverse() {
        ...
        // Popped all the elements and added in the linked list,
...
在这个方法中，我们遍历链表一次，并将元素添加到栈中，然后再次遍历整个链表来更新所有元素。整体耗时 `2n`，是 `O(n)` 的时间复杂度。

更多详情请参考[反转双向链表](https://www.geeksforgeeks.org/reverse-a-doubly-linked-list/)整篇文章！