...
> 原文: [https://www.geeksforgeeks.org/check-if-sum-of-digits-of-a-number-exceeds-the-product-of-digits-of-that-number/](https://www.geeksforgeeks.org/check-if-sum-of-digits-of-a-number-exceeds-the-product-of-digits-of-that-number/)

给定一个正整数 `N`，任务是检查 `N` 的位数之和是否严格大于 `N` 的位数之积。如果发现为真，则打印“是”。否则，打印“否”。
...
*   迭代直到 `N` 大于 `0` 并执行以下步骤:
    *   找到 `N` 的最后一位数字，并将其存储在一个变量中，比如 `rem`。
    *   将 `sumofdigt` 的值增加 `rem`。
    *   将 `prodofdigt` 的值更新为 `prodofdigt * rem`。
