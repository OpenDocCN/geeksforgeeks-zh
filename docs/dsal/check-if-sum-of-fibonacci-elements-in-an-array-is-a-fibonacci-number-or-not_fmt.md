...
> 原文: [https://www.geeksforgeeks.org/check-if-sum-of-fibonacci-elements-in-an-array-is-a-fibonacci-number-or-not/](https://www.geeksforgeeks.org/check-if-sum-of-fibonacci-elements-in-an-array-is-a-fibonacci-number-or-not/)

给定一个包含 `N` 个元素的数组 `arr[]`，任务是检查该数组的斐波那契元素之和是否为[斐波那契数](https://www.geeksforgeeks.org/program-for-nth-fibonacci-number/)。
...
**方法:** 想法是使用[散列](https://www.geeksforgeeks.org/hashing-data-structure/)来预计算并存储[斐波那契数](https://www.geeksforgeeks.org/program-for-nth-fibonacci-number/)直到最大数量，以使检查变得容易且高效（在 `O(1)` 时间内）。
