...
> 原文：[https://www.geeksforgeeks.org/minimum-steps-to-come-back-to-origin-in-a-circular-tour/](https://www.geeksforgeeks.org/minimum-steps-to-come-back-to-origin-in-a-circular-tour/)
...
**天真方法：** 用 `k` 和 `count = 0` 初始化计数器 `i`。此外，对于每次迭代增量 `count`，将 `m` 添加到 `i`。取其模数 `n`，即 `i = ((i + m) % n)`，如果 `i > n`，如果 `i` 等于 `k`，那么 `count` 就是我们的答案。
**时间复杂度：** `O(n)`。
...
本文由 [Shivam Pradhan(anuj_charm)](https://www.facebook.com/anuj.charm) 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 [contribute.geeksforgeeks.org](http://www.contribute.geeksforgeeks.org) 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
