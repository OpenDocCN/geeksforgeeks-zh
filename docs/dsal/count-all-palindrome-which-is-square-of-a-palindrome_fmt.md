...
> 原文: [https://www.geeksforgeeks.org/count-all-palindrome-which-is-square-of-a-palindrome/](https://www.geeksforgeeks.org/count-all-palindrome-which-is-square-of-a-palindrome/)

给定两个正整数 `L` 和 `R`（表示为字符串），其中 $1\leq L \leq R\leq10^{18}$。任务是找到包含范围 `[L, R]` 内的超级回文总数。
...
因此我们迭代每个 `i` 直到 $10^{5}$ 并创建关联回文 `R`，并检查 $R^{2}$ 是否为回文。
同样我们会单独处理奇数和偶数回文，每当回文超出 `R` 的时候就中断。
既然 $P \leq 10^{18}$，$R \leq (10^{18})^{\frac{1}{2}} \equiv 10^{9}$，$R = i||i^{'}$（在拼接上），$i^{'}$ 是 `i` 的反向（双向），那么我们的 `LIMIT` 就不会大于 $i\leq10^{5}$。
