...
> 原文: [https://www.geeksforgeeks.org/program-to-check-whether-a-number-is-proth-number-or-not/](https://www.geeksforgeeks.org/program-to-check-whether-a-number-is-proth-number-or-not/)

给定一个正整数 `N`，任务是检查它是否是一个 Proth 数。如果给定的号码是原号码，则打印“是”，否则打印“否”。

`普罗瑟数`：在数学中，普罗瑟数是一个正整数的形式

> `n = k * 2^n + 1`

其中 `k` 是奇数正整数，`n` 是正整数，使得 `2^n > k`。
...
1.  从数字中扣除 1。这将给出一个形式为 `k*2^n` 的数字，如果给定的数字是一个原数字。
