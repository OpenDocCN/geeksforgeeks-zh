...
> 原文: [https://www.geeksforgeeks.org/minimum-prime-number-operations-to-convert-a-to-b/](https://www.geeksforgeeks.org/minimum-prime-number-operations-to-convert-a-to-b/)
...
**天真的做法：** 若素因式分解 `A = p₁^q₁ * p₂^q₂ * … * pₙ^qₙ`。如果我们将 `A` 乘以某个质数，那么该质数的 `qᵢ` 将增加 1，如果我们将 `A` 除以其质数之一，那么该质数的 `qᵢ` 将减少 1。所以对于一个素数 `p`，如果它在 `A` 的素数因式分解中出现 `q_A` 次，在 `B` 的素数因式分解中出现 `q_B` 次，那么我们只需要求 `| q_A – q_B |` 的和即可。
