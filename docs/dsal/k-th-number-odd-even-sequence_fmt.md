...
> 原文: [https://www.geeksforgeeks.org/k-th-number-odd-even-sequence/](https://www.geeksforgeeks.org/k-th-number-odd-even-sequence/)

给定两个数字 `n` 和 `k`，在由 `n` 组成的奇-偶序列中找到第 `k` 个数字。奇-偶序列首先包含从 1 到 `n` 的所有奇数，然后包含集合 1 到 `n` 中的所有偶数。
...
第一种方法是简单地制作一个奇偶序列，然后在其中找到第 `k` 个元素。
