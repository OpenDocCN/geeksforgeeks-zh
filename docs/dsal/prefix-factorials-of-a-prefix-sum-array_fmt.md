...
> 原文：[https://www.geeksforgeeks.org/prefix-factorials-of-a-prefix-sum-array/](https://www.geeksforgeeks.org/prefix-factorials-of-a-prefix-sum-array/)

给定一个由 `N` 个正整数组成的[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `arr[]`，任务是找到给定数组的[前缀和数组](https://www.geeksforgeeks.org/prefix-sum-array-implementation-applications-competitive-programming/)的前缀阶乘，即 `prefix[i] = (\sum_{0}^{i}arr[i])!`。
...
解决给定问题最简单的方法是先求给定数组的[前缀和](https://www.geeksforgeeks.org/prefix-sum-array-implementation-applications-competitive-programming/)，然后[求前缀和数组中每个数组元素](https://www.geeksforgeeks.org/factorial-of-an-array-of-integers/)的阶乘。计算前缀和后，打印阶乘数组。
...
**时间复杂度：** `O(N*M)`，其中 `M` 为[数组元素之和](https://www.geeksforgeeks.org/program-find-sum-elements-given-array/)。
**辅助空间：** `O(1)`

## 高效方法

上述方法也可以通过预先计算[数组元素之和](https://www.geeksforgeeks.org/program-find-sum-elements-given-array/)的[阶乘](https://www.geeksforgeeks.org/program-for-factorial-of-a-number/)进行优化，使得每个索引 `i` 的阶乘计算可以在 `O(1)` 时间进行。
