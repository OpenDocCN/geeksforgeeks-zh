...
> 原文: [https://www.geeksforgeeks.org/check-if-a-number-can-be-represented-as-a-sum-of-a-prime-number-and-a-perfect-square/](https://www.geeksforgeeks.org/check-if-a-number-can-be-represented-as-a-sum-of-a-prime-number-and-a-perfect-square/)
...
**高效方法:** 上述方法可以通过使用厄拉多塞的筛将所有小于`N`的素数存储在数组中来优化。如果有质数，说 `X`，检查`(N–X)`是否为正方。如果发现是真的，则打印`"是"`。否则，打印`"否"`。
