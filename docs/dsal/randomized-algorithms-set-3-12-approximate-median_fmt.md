...
**问题陈述:**给定一个由 `n` 个数字和 `ε > 0` 组成的未排序数组 `A[]`，计算一个元素，该元素的秩(在已排序数组 `A[]`)在 `[(1–ε)n/2，(1 + ε)n/2]` 的范围内。
求近似中值算法 `ε` is 1/2 = >秩应在 `[n/4，3n/4]` 范围内
...
以下步骤代表的算法为 `O((Log n) x (Log Log n))` 时间，产生不正确结果的概率小于或等于 `2/n^2` 。
...
**时间复杂度:**
我们使用 C++中 STL 提供的一个集合。在 STL 集中，每个元素的插入都需要 `O(log k)`。所以对于 `k` 次插入，花费的时间是 `O (k log k)`。
现在将 `k` 替换为 `c log n`
...
从左四分之一选择至少 `k/2` 个元素的概率)<= `1/n^2`
从左四分之一或右四分之一选择至少 `k/2` 个元素的概率)<= `2/n^2`

因此，算法产生不正确的结果，概率小于或等于 `2/n^2` 。
