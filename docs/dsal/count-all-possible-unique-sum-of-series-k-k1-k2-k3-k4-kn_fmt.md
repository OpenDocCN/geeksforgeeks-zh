...
> 原文: [https://www.geeksforgeeks.org/count-all-possible-unique-sum-of-series-k-k1-k2-k3-k4-kn/](https://www.geeksforgeeks.org/count-all-possible-unique-sum-of-series-k-k1-k2-k3-k4-kn/)
...
*   既然 `K` 是常数，它唯一的意义就是大小不同的两个子集不能有相同的和。
*   唯一和可以从子集的最小可能和到大小为 `X` 的子集的最大可能和获得，其中 `(2 ≤ X ≤ (N + 1))`。

> 例如: `N = 4`
> 级数为= `{K，K + 1，K + 2，K + 3，K + 4}`
> 对于 `K = 2`，最小和= `(K，K + 1) = 2*K + 1`
> 和最大和= `(K + 3，K + 4) = 2*K + 7`
> 与 `K = 2` 可能的最大相异和为 `(最大和–最小和+ 1)`。

## 按照以下步骤解决问题

1.  初始化两个数组 `fsum[]` 和 `rsum[]`，每个数组大小为 `N + 1` 并初始化为 `0`。
2.  对于数组 `fsum[]` 和 `rsum[]` 的每个元素，用 `ar[i]+fsum[i–1]` 更新 `fsum[i]`，用 `ar[i] + rsum[i + 1]` 更新 `rsum[i]`。
3.  将变量 `ans` 初始化为 `1`，该变量存储给定序列的不同可能和的计数。
4.  对于每个可能的子集大小 `X`，其中 `(2 ≤ X ≤ (N + 1))`，将值 `1+rsum[N+1–k]-fsum[k-1]` 加到 `ans` 上。
5.  `ans` 的值是必需的答案，因此返回它。
