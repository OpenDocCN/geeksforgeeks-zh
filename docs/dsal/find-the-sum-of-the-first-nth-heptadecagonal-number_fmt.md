...
1.  首先，我们需要创建一个函数来帮助我们计算第 `N` 个[十七进制数](https://www.geeksforgeeks.org/heptadecagonal-number/)。
2.  现在，运行一个从 1 到 `N` 的循环，找到第 `i` 个[七进制数](https://www.geeksforgeeks.org/heptadecagonal-number/)。
3.  把上面计算的所有七进制数相加。
4.  最后，显示前 `N` 个七进制数的和。
