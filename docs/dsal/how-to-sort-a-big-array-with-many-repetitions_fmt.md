...
> 原文: [https://www.geeksforgeeks.org/how-to-sort-a-big-array-with-many-repetitions/](https://www.geeksforgeeks.org/how-to-sort-a-big-array-with-many-repetitions/)
...
a) 如果`arr[i]`不在树中，则插入它并初始化计数为 1
b) 否则增加其在树中的计数。
3) 进行树的有序遍历。这样做时，请将每个键的计数次数放在`arr[]`。
第 2 步和第 3 步耗时。所以整体时间复杂度为 O(n Log m)
下面是上述思想的 C++实现。
...
```java
// inorder traversal in BST always give sorted order. Put the sorted elements back into the array
...
我们也可以使用[哈希](http://geeksquiz.com/hashing-set-1-introduction/)在 O(n + m Log m)时间内解决上述问题。
...
在哈希搜索和插入花费 O(1)时间的假设下，步骤 2 的时间复杂度为 O(n)。步骤 3 花费 O(m Log m)时间，其中 m 是输入数组中不同键的总数。第四步耗时 O(n)。所以总的时间复杂度是 O(n + m Log m)。
...
void sort(int arr[], int n)
{
   //1. Create an empty hash table.
    ...
    //2. Input array values are stores as key and their counts are stored as value in hash table.
    for (int i=0; i<n; i++)
        count[arr[i]]++;
