...
> 原文: [https://www.geeksforgeeks.org/find-the-missing-value-from-array-b-formed-by-adding-some-value-x-to-array-a/](https://www.geeksforgeeks.org/find-the-missing-value-from-array-b-formed-by-adding-some-value-x-to-array-a/)

给定两个数组 `arr1[]` 和 `arr2[]`，大小分别为 `N` 和 `N-1`。`arr2[]` 中的每一个值都是通过在 `N-1` 个任意的 `arr1[i]` 上加上一个隐藏值 `X` 得到的，这意味着对于任意 `i`，`arr1[i]+X` 不包含在 `arr2[]` 中，也就是 `arr2[]` 中的缺失值。任务是找到隐藏值 `X` 和未从 `arr1[]` 中选取的缺失值。
...
> **说明:** 第二个数组的元素是这样得到的:
> 第一个元素: `(10 + 7) = 17`，10 是 `arr1[]` 索引 4 处的元素。
> 第二个元素: `(3 + 7) = 10`，3 是 `arr1[]` 索引 0 处的元素。
> 第三个元素: `(6 + 7) = 13`，6 是 `arr1[]` 索引 1 处的元素。
> 所以，7 是隐藏值，5 是缺失值。

> **输入:** `arr1[] = {4, 1, 7}`，`arr2[] = {4, 10}`
> **输出:** Hidden = 3，Missing = 4

**做法:** 这个问题可以用以下方法解决：

*   [按非递减顺序对两个给定数组进行排序](https://www.geeksforgeeks.org/sort-c-stl/)。
*   创建一个[无序映射](https://www.geeksforgeeks.org/unordered_map-in-cpp-stl/)来存储差值。
*   [遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)直到 `N-1`，通过存储两个数组元素的差值来检查值。
    *   令 `a = arr2[i] - arr1[i]` 和 `b = arr2[i] - arr1[i+1]`。
    *   如果 `a != b`，那么检查是否
        *   `a > 0`，然后存储在映射中。
        *   `b > 0`，然后存入映射。
    *   否则检查 `a > 0`，然后将其存储在映射中。
*   [遍历映射](https://www.geeksforgeeks.org/traversing-a-map-or-unordered_map-in-cpp-stl/)找到隐藏值并打印出来。
*   通过添加隐藏值遍历 `arr1[]`，检查是否存在于 `arr2[]` 中，否则打印缺失值。
