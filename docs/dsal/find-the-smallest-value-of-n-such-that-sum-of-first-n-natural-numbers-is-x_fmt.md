...
> 原文: [https://www.geeksforgeeks.org/find-the-smallest-value-of-n-such-that-sum-of-first-n-natural-numbers-is-x/](https://www.geeksforgeeks.org/find-the-smallest-value-of-n-such-that-sum-of-first-n-natural-numbers-is-x/)

给定一个正整数 `X` *(1≤X≤10<sup>6</sup>)*，任务是求最小值 `N`，使得前 `N` 个自然数的和为≥ `X`。
...
**天真方法:** 解决这个问题最简单的方法是检查范围 `[1，X]` 中的每个值，并从该范围返回第一个值，对于该值，发现第一个 `N` 自然数的和为≥ `X`。
...
2.  初始化变量 `low = 1`，`high = X` 并在此范围内执行[二分搜索法](https://www.geeksforgeeks.org/binary-search/)。
3.  计算 `mid = low + (high – low) / 2`，检查第一个 `mid` 数之和是否大于等于 `x`。
4.  如果 `总和 ≥ X`，将其存储在变量 `res` 中，并将 `high` 设置为 `mid - 1`。
5.  否则，将 `low` 设置为 `mid + 1`。
6.  打印 `res`，这是所需答案。
