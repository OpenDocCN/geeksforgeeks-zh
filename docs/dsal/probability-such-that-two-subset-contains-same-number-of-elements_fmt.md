...
> 原文: [https://www.geeksforgeeks.org/probability-such-that-two-subset-contains-same-number-of-elements/](https://www.geeksforgeeks.org/probability-such-that-two-subset-contains-same-number-of-elements/)
...
一个集合包含的子集总数 `N` 元素个数为 `2^N`，同时选择 `X` 和 `Y` 的总可能方式将是 `2^N * 2^N = 2^{2N}`。

令 `P` = 选择 `X` 和 `Y` 的总可能方式，使两者具有相同的元素数量。
那么 `P = \sum_{r=0}^{n} ^nC_r * ^nC_r = C_0^2+C_1^2+C_2^2+....+C_n^2 = ^{2n}C_{n}`。
那么要求的概率就是 `\frac{^{2n}C_{n}}{2^{2n}}`。
