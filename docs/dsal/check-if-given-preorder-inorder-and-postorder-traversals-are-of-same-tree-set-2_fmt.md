...
> 原文: [https://www.geeksforgeeks.org/check-if-given-preorder-inorder-and-postorder-traversals-are-of-same-tree-set-2/](https://www.geeksforgeeks.org/check-if-given-preorder-inorder-and-postorder-traversals-are-of-same-tree-set-2/)
...
在[之前的文章](https://www.geeksforgeeks.org/check-if-given-preorder-inorder-and-postorder-traversals-are-of-same-tree/)中，我们已经讨论了通过使用任意两个遍历来构建树来解决上述问题的方法。本文讨论了一种不使用任何额外空间的方法。

**进场:**

1.  在 `inorder` 数组中搜索 `preorder` 数组的第一个元素，并将其索引存储为 `idx`，如果它不存在，则返回 `False`。另外，检查该元素是否出现在后置数组中。如果不是，则返回假。
...
4.  递归地重复步骤 1 到 3，直到数组的长度变成 0(在这种情况下，我们返回真)或 1(在这种情况下，只有当所有三个数组相等时，我们才返回真，否则返回假)。
