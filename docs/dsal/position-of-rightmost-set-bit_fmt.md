...
> 原文: [https://www.geeksforgeeks.org/position-of-rightmost-set-bit/](https://www.geeksforgeeks.org/position-of-rightmost-set-bit/)
...
**算法:** (例 12(1100))
设 I/P 为 12 (1100)
1. 取给定数的二进制补码，因为所有位都被还原除了从右到左的第一个“1”【0100】
2. 用原来的数做按位与，这将返回数，而只需要一个(0100)
3. 取数的对数 2，你将得到(位置–1)(2)
4. 加 1 (3)
...
对于所有包含的二进制数，只有最右边的位设置为 1，如 2，4，8，16，32……。
...
用 `1` 初始化 `pos`，向上迭代到 `INT_SIZE`（此处为 `32`）并检查是否设置了位，如果设置了位则中断循环，否则递增 `pos`。
