...
> 原文：[https://www.geeksforgeeks.org/second-best-minimum-spanning-tree/](https://www.geeksforgeeks.org/second-best-minimum-spanning-tree/)
...
`T` 和 `T'` 的区别仅在于一个边替换。因此，我们应该找到一个不在 `T` 中的边 `e_new` 并用 `T` 中的边替换它（比如 `e_old`），这样 `T' = T union { e_new } – { e_old }` 就是一棵生成树，并且（`e_new` – `e_old`）的权重差最小（`e_new`，`e_old` 是 `T` 中的边）。
