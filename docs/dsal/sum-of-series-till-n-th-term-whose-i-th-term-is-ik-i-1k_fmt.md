...
> 原文：[https://www.geeksforgeeks.org/sum-of-series-till-n-th-term-whose-i-th-term-is-ik-i-1k/](https://www.geeksforgeeks.org/sum-of-series-till-n-th-term-whose-i-th-term-is-ik-i-1k/)

给定 `N` 和 `k` 的值，任务是找到直到第 `N` 项的级数和，第 `I` 项由 `T_I = I^k + (I–1)^k` 给出。因为级数的和可能非常大，所以以 `1000000007` 为模计算它的和。
...
**天真的方法**
一个简单的解决方案是生成 `n` 个以下的所有术语并添加它们。时间复杂度将为 `O(N)`。由于 `N` 非常大，该方法将产生 `TLE`。
...
> 考虑 `N = 4`， `K = 3`
> 则系列为：
> `1^3 – (1–1)^3 + 2^3 – (2–1)^3 + 3^3 – (3–1)^3 + 4^3 – (4–1)^3`
> 即 `1^3 – 0^3 + 2^3 – 1^3 + 3^3 – 2^3 + 4^3 – 3^3`
> 将相同的异能改写在一起，我们得到。
> `0^3 + 1^3 – 1^3 + 2^3 – 2^3 + 3^3 – 3^3 + 4^3`
> 即 `0^3 + (1^3 – 1^3) + (2^3 – 2^3) + (3^3 – 3^3) + 4^3`
> 因此最终结果将是 `4^3` (即 `n^k` )
> 因此最终公式为 `N^K`
...
**时间复杂度**：`O(K)`
`N^K` 可以用[模幂](https://www.geeksforgeeks.org/modular-exponentiation-power-in-modular-arithmetic/)在 `O(log N)` 中计算。
