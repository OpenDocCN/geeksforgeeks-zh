...
一个简单的解决方案是使用两个嵌套的[循环](https://www.geeksforgeeks.org/loops-in-c-and-cpp/)。对于每个索引，将索引左侧的所有元素与该索引中存在的元素进行比较，并找到小于该索引中存在的元素的最大元素。
...
思路是使用[自平衡 BST](http://www.geeksforgeeks.org/tag/self-balancing-bst/) 在 `O(LogN)` 中找到数组中任意元素之前的最大元素。

自平衡 BST 实现为 C++中的[`set`](https://www.geeksforgeeks.org/set-in-cpp-stl/) 和 Java 中的[`TreeSet`](https://www.geeksforgeeks.org/treeset-in-java-with-examples/)。
