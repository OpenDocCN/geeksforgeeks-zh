...
> 原文: [https://www.geeksforgeeks.org/sort-an-array-according-to-count-of-set-bits-set-2/](https://www.geeksforgeeks.org/sort-an-array-according-to-count-of-set-bits-set-2/)
...
上一节我们已经用各种方法讨论了基于集合比特计数的排序方法。本文包含使用 `map` 的实现。
众所周知，`map`/`multimap` 以有序的方式存储数据。因此，如果我们在 `map` 中为 `arr[i]` 存储 `(32 - countsetbits(arr[i]))`，那么输出将按照设置位计数的递减顺序输出，这就是所需的输出。
以下是上述方法的实施:
