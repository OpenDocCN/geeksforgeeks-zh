...
> 原文：[https://www.geeksforgeeks.org/sum-of-bitwise-xor-of-each-array-element-with-all-other-array-elements/](https://www.geeksforgeeks.org/sum-of-bitwise-xor-of-each-array-element-with-all-other-array-elements/)
...
> **输出:** `5 4 3`
> **解释:**
> 对于 `arr[0]`：`arr[0] ^ arr[0] + arr[0] ^ arr[1] + arr[0] ^ arr[2] = 1^1 + 1^2 + 1^3 = 0 + 3 + 2 = 5`
> 对于 `arr[1]`：`arr[1] ^ arr[0] + arr[1] ^ arr[1] + arr[1] ^ arr[2] = 2^1 + 2^2 + 2^3 = 3 + 0 + 1 = 4`
> 对于 `arr[2]`：`arr[2] ^ arr[0] + arr[2] ^ arr[1] + arr[2] ^ arr[2] = 3^1 + 3^2 + 3^3 = 2 + 1 + 0 = 3`
>
> **输入:** `arr[] = {2, 4, 8}`
> **输出:** `16 18 22`
> **解释:**
> 对于 `arr[0]`：`arr[0] ^ arr[0] + arr[0] ^ arr[1] + arr[0] ^ arr[2] = 2^2 + 2^4 + 2^8 = 0 + 6 + 10 = 16`
> 对于 `arr[1]`：`arr[1] ^ arr[0] + arr[1] ^ arr[1] + arr[1] ^ arr[2] = 4^2 + 4^4 + 4^8 = 6 + 0 + 12 = 18`
> 对于 `arr[2]`：`arr[2] ^ arr[0] + arr[2] ^ arr[1] + arr[2] ^ arr[2] = 8^2 + 8^4 + 8^8 = 10 + 12 + 0 = 22`

## 天真方法

想法是[遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)，对于每个数组元素，[遍历数组](https://www.geeksforgeeks.org/iterating-arrays-java/)，计算其与所有其他数组元素的[位异或](https://www.geeksforgeeks.org/bitwise-operators-in-c-cpp/)之和。
...
为了优化上述方法，其思想是使用**逐位异或**的属性，即异或上的相似位给出 `0`，否则给出 `1`。按照以下步骤解决问题：

*   计算位置 `i` 处设置位的频率，其中 `0 <= i <= 32`，跨越频率数组中数组的所有元素。
*   对于数组的每个元素 `X`，通过运行从 `i=0` 到 `32` 的循环来计算异或和，并检查是否设置了 `X` 的 `i` 位。
    *   如果是，那么将 `(N - frequency[i]) * 2^i` 加到异或和上，因为在这个位置上的 `X` 的设置位将使所有的设置位为零，所有的未设置位为 `1`。
    *   否则，将 `frequency[i] * 2^i` 加到异或和上。
