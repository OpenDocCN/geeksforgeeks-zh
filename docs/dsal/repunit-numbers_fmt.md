...
我们会统计给定号码 `N` 的基数 `B` 中的 1 的个数，也会统计给定号码 `N` 的基数 `B` 中的位数。如果相同，则打印“是”，否则打印“否”。

### 例如

`N = 31`，`B = 5`
31 可以写成 5 中的 `111`，所以给定数 `N` 的基数 `B` 中的 1 的个数 = 3 和给定数 `N` 的基数 `B` 中的位数 = 3。
...
**参考:** [http://www.numbersaplenty.com/set/repunit/](http://www.numbersaplenty.com/set/repunit/)