...
最后一个正方形子矩阵是整个矩阵 `M`，其中 `r=c=a=1`。我们可以看到 `M(1, 1) + M(2, 2) = 1+3 = 4` 和 `M(1, 2) + M(2, 1) = 2+4 = 6` 都是偶数。
...
我们知道当两个数的奇偶性相同时，两个数的和是偶数。假设 `M(i, j)` 的奇偶性是奇数，这意味着 `M(i+1, j+1)`、`M(i+1, j-1)`、`M(i-1, j+1)`、`M(i-1, j-1)` 的奇偶性必须是奇数。
