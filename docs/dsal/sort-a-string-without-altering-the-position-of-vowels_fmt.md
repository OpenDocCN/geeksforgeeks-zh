...
> 原文：[https://www.geeksforgeeks.org/sort-a-string-without-altering-the-position-of-vowels/](https://www.geeksforgeeks.org/sort-a-string-without-altering-the-position-of-vowels/)

## 问题描述与示例

给定一个大小为 `N` 的字符串 `S`，任务是在不改变元音位置的情况下对字符串进行排序。
...
*   按照字典顺序对字符串 `temp` 进行排序。
*   初始化一个指针，比如 `ptr = 0`，指向字符串 `temp` 中的当前字符。
*   现在，遍历字符串 `S`，用 `temp[ptr]` 替换字符串 `S` 的每个辅音。递增 `ptr`。
