...
> 原文: [https://www.geeksforgeeks.org/queries-to-check-if-any-pair-exists-in-an-array-having-values-at-most-equal-to-the-given-pair/](https://www.geeksforgeeks.org/queries-to-check-if-any-pair-exists-in-an-array-having-values-at-most-equal-to-the-given-pair/)

给定一个[对](https://www.geeksforgeeks.org/pair-in-cpp-stl/) `arr[]` 和 `Q` 的[向量](https://www.geeksforgeeks.org/vector-in-cpp-stl/)，以数组 `queries[]` 中的对的形式进行查询，每个查询的任务是检查是否存在两个值都小于当前查询对中的值的对。如果发现是真的，则打印 `"Yes"`。否则，打印 `"No"`。

## 示例

**输入:** `arr[][] = {{3, 5}, {2, 7}, {2, 3}, {4, 9}}`, `queries[][] = {{3, 4}, {3, 2}, {4, 1}, {3, 7}}`
**输出:**
Yes
No
No
...
**输入:** `arr[][] = {{2, 1}, {4, 2}, {4, 4}, {7, 2}}`, `queries[][] = {{2, 1}, {1, 1}}`
**输出:**
Yes
No
...
最简单的方法是[遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `queries[][]`。对于每一对，遍历给定的对数组，检查是否存在对应值大于或等于对 `{p1, p2}` 的对，然后打印 `"Yes"`。否则，打印 `"No"`。
...
优化上述途径，思路是使用[二分搜索法](https://www.geeksforgeeks.org/binary-search/)。按照以下步骤解决此问题:
...
*   现在，遍历给定的 `queries[]` 数组，并对数组中的每一对执行二分搜索法运算 `arr[]`。
*   如果以上步骤得到的对小于 `queries[]` 中的当前对，则打印 `"Yes"` 否则打印 `"No"`。
