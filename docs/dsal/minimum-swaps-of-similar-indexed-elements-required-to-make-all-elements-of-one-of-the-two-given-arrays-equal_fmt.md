...
> 原文: [https://www.geeksforgeeks.org/minimum-swaps-of-similar-indexed-elements-required-to-make-all-elements-of-one-of-the-two-given-arrays-equal/](https://www.geeksforgeeks.org/minimum-swaps-of-similar-indexed-elements-required-to-make-all-elements-of-one-of-the-two-given-arrays-equal/)

给定两个大小为 `N` 的数组 `A[]` 和 `B[]`，任务是计算使两个给定数组中的一个相等所需的相似索引元素的最小交换次数。如果不可能使一个数组的所有元素相等，则打印 `-1`。
...
使用哈希可以解决问题。按照以下步骤解决问题:

*   初始化一个变量，比如 `minSwaps`，以存储所需的最小交换次数，这样至少一个数组只包含一个不同的值。
*   初始化另一个变量，比如 `swapsRequired`，以存储所需的交换次数，这样至少一个数组只包含一个不同的值。
*   初始化一个数组，比如 `countA[]`，来存储数组 `A[]` 中每个不同元素的频率。
*   初始化一个数组，比如 `countB[]`，来存储数组 `B[]` 中每个不同元素的频率。
*   初始化一个数组，比如说 `countAB[]`，以存储出现在 `A[]` 和 `B[]` 相同索引处的每个不同元素的频率。
*   使用变量 `i` 遍历两个数组，检查 `(countA[i] + countB[i] - equalAB[i] == N)` 的值是否为真。如果发现为真，则更新 `swapsRequired = min(countA[i], countB[i]) - equalAB[i]` 并更新 `minSwaps = min(minSwaps, swapsRequired)`。
*   最后，打印 `minSwaps` 的值。
