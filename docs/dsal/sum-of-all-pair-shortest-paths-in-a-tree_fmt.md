...
给定一个加权无向图 `T`，该图由值为 `[0, N–1]` 的节点和类型为 `{u, v, w}` 的数组 `edges[][3]` 组成，该数组表示顶点 `u` 和 `v` 之间具有权重 `w` 的边。任务是找到给定树中所有成对最短路径的总和。
...
最简单的方法是使用弗洛伊德·沃肖尔算法找到每对顶点之间的最短路径。在预计算每对节点之间最短路径的成本后，打印所有最短路径的总和。
...
int sumOfshortestPath(int N, int E, int edges[][3])
{
    int g[N][N];
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
...
class GFG{
    ...
    public static void main(String[] args)
    {
        ...
        // Function Call
        System.out.print(sumOfshortestPath(N, E, Edges));
    }
}
...
想法是使用[DFS 算法](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/)，使用 [DFS](https://www.geeksforgeeks.org/depth-first-traversal-for-a-graph/) ，对于每个顶点，从这个顶点访问每隔一个顶点的成本可以在线性时间内找到。按照以下步骤解决问题:

1.  遍历节点 `0` 到 `N–1`。
2.  对于每个节点 `i` ，使用 [DFS](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/) 找到访问每隔一个顶点的成本总和，其中源将是节点 `i` ，让我们用 `S_i` 来表示这个总和。
3.  现在，计算 `S = S_0 + S_1 + … + S_{N-1}`。并将 `S` 除以 `2` ，因为每条路径都要计算两次。
