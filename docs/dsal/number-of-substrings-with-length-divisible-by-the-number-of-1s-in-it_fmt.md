...
> 原文: [https://www.geeksforgeeks.org/number-of-substrings-with-length-divisible-by-the-number-of-1s-in-it/](https://www.geeksforgeeks.org/number-of-substrings-with-length-divisible-by-the-number-of-1s-in-it/)
...
> 子串 `[l:r]` 中的 1 个数 = `prefix_sum[r] – prefix_sum[l–1]`
...
> `r – l + 1 = k * (prefix_sum[r] – prefix_sum[l-1])`，其中 `k` 是某个整数

*   这也可以写成:

> `r – k * prefix_sum[r] = l – 1 – k * prefix_sum[l-1]`

观察这个等式，我们可以为 `0 <= k < n` 创建另一个 `B[i] = i – k * prefix_sum[i]` 的数组。所以，现在的问题是计算 `B` 中每 `k` 个相等整数对的数量。
