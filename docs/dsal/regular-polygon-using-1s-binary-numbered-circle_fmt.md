...
> 原文：[https://www.geeksforgeeks.org/regular-polygon-using-1s-binary-numbered-circle/](https://www.geeksforgeeks.org/regular-polygon-using-1s-binary-numbered-circle/)
...
如果给定的数组大小是 `N`，那么 `K` 应该划分 `N`，否则 `K` 个顶点不能以相等的方式划分 `N` 个顶点成为正多边形。下一件事是在所选多边形的每个顶点都应该有一个。
