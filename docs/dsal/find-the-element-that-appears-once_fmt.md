...
> 原文: [https://www.geeksforgeeks.org/find-the-element-that-appears-once/](https://www.geeksforgeeks.org/find-the-element-that-appears-once/)
...
`ones` 和 `twos` 初始化为 0。对于数组中的每个新元素，找出新元素中的公共集合位和 `ones` 的前一个值。这些公共设置位实际上是应该加到 `twos` 上的位。所以用 `twos` 对公共集合位进行按位异或运算。`twos` 还会获得一些第三次出现的额外位。这些额外的比特稍后被移除。
用 `ones` 的前一个值对新元素进行异或运算，更新 `ones`。有些位可能会出现第三次。这些额外的位稍后也会被移除。
`ones` 和 `twos` 都包含第三次出现的额外位。通过找出 `ones` 和 `twos` 中常见的设置位来删除这些多余的位。
