...
> 原文：[https://www.geeksforgeeks.org/minimum-steps-to-make-the-product-of-the-array-equal-to-1/](https://www.geeksforgeeks.org/minimum-steps-to-make-the-product-of-the-array-equal-to-1/)

给定一个包含整数 `N` 的数组 `arr[]`。在一个步骤中，数组的任何元素都可以增加或减少一。任务是找到所需的最小步骤，使数组元素的乘积成为 `1`。
...
> 因此，总共需要 5 个步骤来更新元素，使得最终数组的乘积为 `1`。
...
3.  同样，所有负数都可以更新为 `-1`。
4.  如果数组中存在 `0`，则可以根据情况将其减少为 `1` 或 `-1`（`1` 的计数必须为偶数）。
5.  如果负数的计数是偶数，那么它们总是会产生 `1`。
6.  但是如果有奇数个负数，那么它们将产生奇数个 `-1`。要解决这个问题，有两种可能：
    *   先试着找到数组中的计数 `0`，因为需要 `1` 运算才能成为 `-1`。
