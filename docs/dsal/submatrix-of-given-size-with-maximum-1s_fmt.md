...
> 原文:[https://www.geeksforgeeks.org/submatrix-of-given-size-with-maximum-1s/](https://www.geeksforgeeks.org/submatrix-of-given-size-with-maximum-1s/)
...
思路是使用[滑动窗口技术](https://www.geeksforgeeks.org/window-sliding-technique/)来解决这个问题，在这个技术中，我们一般计算一个窗口的值，然后逐个滑动窗口来计算每个大小为 `K` 的窗口的解。
...
在该技术中，我们使用给定的 `mat[][]` 数组计算 `dp[][]` 矩阵。在 `dp[][]` 数组中，我们使用先前的 `dp[][]` 值计算直到索引 `(i，j)` 的 1 的个数，并将其存储在 `dp[i][j]` 中。
