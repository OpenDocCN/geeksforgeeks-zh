...
给定由范围`[0, N)`中的值组成的`N`节点组成的[树](https://www.geeksforgeeks.org/generic-treesn-array-trees/)和由范围`[0, N)`中的值组成的`Q`整数的数组`Queries[]`。每个查询的任务是移除顶点值`Queries[i]`，并计算结果图中的连通分量。

**示例:**

> **输入:** `N = 7`, `Edges[][2] = {{0, 1}, {0, 2}, {0, 3}, {1, 4}, {1, 5}, {3, 6}}`, `Queries[] = {0, 1, 6}`
>
...
>
> **输入:** `N = 7`, `Edges[][2] = {{0, 1}, {0, 2}, {0, 3}, {1, 4}, {1, 5}, {3, 6}}`, `Queries[] = {5, 3, 2}`
>
> ![](img/e52ab83d38307572f6627b99c71e0980.png)
>
> **输出:** 1 2 1
...
**方法:**想法是观察在[树](https://www.geeksforgeeks.org/generic-treesn-array-trees/)中，每当删除一个节点时，连接到该节点的节点就会分离。因此，连接组件的[计数等于删除节点](https://www.geeksforgeeks.org/program-to-count-number-of-connected-components-in-an-undirected-graph/)的[度](https://www.geeksforgeeks.org/find-degree-particular-vertex-graph/)。
因此，方法是预先计算并存储数组中每个节点的[度](https://www.geeksforgeeks.org/print-the-degree-of-every-node-from-the-given-prufer-sequence/)。对于每个查询，连接组件的计数只是查询中对应节点的度。
