...
*   玩家 1 总是第一个开始。
*   两个玩家轮流选择一整块连续相等的字符，并从给定的二进制字符串 `S` 中删除它们。
...
> 转 1：(玩家 1) `**111**0011001010` → `00110010` `Score_1` `0000**1**010` → `0000010` `Score_1 = 4`
> 回合 4：(玩家 2) `0000010`
> 他不能做任何事情，因为只有一个‘1’存在，这是一个奇数。
> 同样，他不能选择‘0’，因为它们是奇数(5 和 1)，因此，`Score_2 = 2`
> 回合 5：(玩家 1) `00000**1**0` → `000000` `Score_1 = 5`
> 回合 6：(玩家 2) `**000000**` → `Score_2 = 2` (本回合未删除‘1’)
>
...
> **输出：** 选手 2
> **解说：**
> 转 1：(选手 1) `11111110**1**` → `111110` `评分_1 = 3`
> 转 2：(选手 2) `**111111**0` → `0`
>
