...
> 原文: [https://www.geeksforgeeks.org/check-if-a-palindromic-matrix-can-be-formed-from-the-given-array-elements/](https://www.geeksforgeeks.org/check-if-a-palindromic-matrix-can-be-formed-from-the-given-array-elements/)

给定一个由 `N^2` 个整数组成的数组 `arr[]`，任务是检查是否可以由给定的数组元素形成维度为 `N * N` 的矩阵，即回文矩阵。如果可能，打印回文矩阵。
          ...
          vector<vector<int> >& grid)
{
    ...
    // to the matrix
    for (auto c : temp) {
        grid[c.first][c.second] = it.second;
    }
