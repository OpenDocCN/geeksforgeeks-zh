...
> 原文: [https://www.geeksforgeeks.org/total-time-for-which-the-hero-will-be-in-shock/](https://www.geeksforgeeks.org/total-time-for-which-the-hero-will-be-in-shock/)

给定一个包含非递减顺序的 `N` 个整数和一个整数 `D` 的数组 `attack`。数组 `attack` 中的每一个元素都表示英雄在游戏中被攻击的时间，`D` 表示英雄被震到无法反击的持续时间。任务是找到英雄被电击的总时间。
...
> `shockTime[i] = (attack[i] + D) – max(前一次攻击结束的时间, attack[i])`

现在要解决这个问题，请按照以下步骤操作:

1.  创建一个变量 `totalTime`，存储这个问题的答案。
2.  创建一个变量 `prev` 并将其初始化为 `attack[0]`。该变量将存储每次单独攻击前一次攻击结束的时间。
3.  运行从 `i = 0` 到 `i < N` 的循环，在每次迭代中:
    *   将 `attack[i]` 即 `(attack[i] + D) – max(prev, attack[i])` 的电击时间加到 `totalTime` 上。
    *   将 `prev` 更改为前一次攻击结束的时间，因此 `prev = attack[i] + D`。
