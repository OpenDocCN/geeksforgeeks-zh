...
> 原文: [https://www.geeksforgeeks.org/check-if-it-is-possible-to-make-two-matrices-strictly-increasing-by-swapping-corresponding-values-only/](https://www.geeksforgeeks.org/check-if-it-is-possible-to-make-two-matrices-strictly-increasing-by-swapping-corresponding-values-only/)

给定两个 `n * m` 矩阵 `A[][]` 和 `B[][]`，任务是仅通过交换不同矩阵中的两个元素（如果它们位于相应的位置，即 `A[i][j]` 只能与 `B[i][j]` 交换）来使两个矩阵严格递增（行和列）。如果可能，则打印 `是`，否则，打印 `否`。
