...
> 原文: [https://www.geeksforgeeks.org/given-a-string-find-its-first-non-repeating-character/](https://www.geeksforgeeks.org/given-a-string-find-its-first-non-repeating-character/)
...
**方法 1 :** [哈希映射](https://www.geeksforgeeks.org/java-util-hashmap-in-java/)和双字符串方法遍历。
...
### 方法二：`HashMap` 和单串遍历。
**方法:**制作一个计数数组，而不是最大字符数(256)的 `hash_map`。我们不仅可以存储计数，还可以存储第一次遇到字符时的索引，例如“a”的`(3，26)`，这意味着“a”被计数了 3 次，第一次出现在位置 26。所以说到找到第一个非中继器，我们只需要扫描计数数组，而不是字符串。感谢本提出这种方法。

**算法:**
1.  制作一个 `count_array`，它有两个字段，即`频率，第一次出现一个字符`。
2.  `count_array`的大小为`256`。
3.  使用指针遍历给定的字符串。
4.  增加当前字符的计数并更新出现次数。
