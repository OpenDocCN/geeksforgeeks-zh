...
**说明**：删除 1 和两个 2。因此，仅剩 3 个，因此元素的不同数目为 1。

**输入**：`arr[] = {2, 4, 1, 5, 3, 5, 1, 3}`，`m = 2`

**输出**：3

**说明**：完全卸下 2 和 4。因此，剩下的 1、3 和 5，即 3 个元素。

对于`O(N * log N)`方法，请参考[先前的文章](https://www.geeksforgeeks.org/minimum-number-of-distinct-elements-after-removing-m-items/)。

## 高效方法

这个想法是将每个元素的出现存储在[哈希](https://www.geeksforgeeks.org/hashing-data-structure/)中，然后再次计算每个频率在哈希中的出现。 步骤如下：
