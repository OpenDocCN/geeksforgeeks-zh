...
> 原文: [https://www.geeksforgeeks.org/calculate-sum-of-all-integers-from-1-to-n-excluding-perfect-power-of-2/](https://www.geeksforgeeks.org/calculate-sum-of-all-integers-from-1-to-n-excluding-perfect-power-of-2/)
...
2.  因为 2 的所有完美幂的和形成了一个几何级数。因此，小于 `N` 的 2 的所有幂的和由以下公式计算:
    > 2 的完美幂小于 `N` 的元素数由 `log2(N)` 给出，
    > 令 `r = log2(N)`
    > 所有 2 的完美幂的数之和由 `2^r – 1` 给出。
3.  从第一个 `N` 数的和中减去上面计算的 2 的所有完美幂的和，得到结果。
