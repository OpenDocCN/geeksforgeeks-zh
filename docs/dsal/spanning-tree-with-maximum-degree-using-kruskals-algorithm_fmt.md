...
给定一个由 `n` 个顶点和 `m` 个边组成的无向未加权连通图。任务是找到这个图的任何生成树，使得所有顶点上的最大度是最大可能的。打印输出边的顺序并不重要，边也可以反向打印，即`(u, v)`也可以打印为`(v, u)`。
...
[Kruskal 算法求最小生成树](https://www.geeksforgeeks.org/kruskals-minimum-spanning-tree-algorithm-greedy-algo-2/)

## 方法:

利用 Kruskal 算法寻找最小生成树，可以解决给定的问题。
