...
> 原文：`https://www.geeksforgeeks.org/sliding-window-maximum-maximum-of-all-subarrays-of-size-k/`
...
### 方法
为了在子数组的`k`个元素中找到最大值，先前的方法使用遍历元素的循环。为了减少该时间，我们的想法是使用 [AVL 树](https://www.geeksforgeeks.org/avl-tree-set-1-insertion/)，该树返回`log n`时间中的最大元素。因此，遍历数组并将`k`个元素保留在 BST 中，并在每次迭代中打印最大值。`AVL树`是一种合适的数据结构，因为在平均和最坏情况下查找，插入和删除都需要`O(log n)`时间，其中`n`是操作之前树中的节点数。
