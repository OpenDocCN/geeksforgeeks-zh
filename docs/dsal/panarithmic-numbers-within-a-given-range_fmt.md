...
> 原文：[https://www.geeksforgeeks.org/panarithmic-numbers-within-a-given-range/](https://www.geeksforgeeks.org/panarithmic-numbers-within-a-given-range/)

给定两个正数 `A` 和 `B`。任务是打印所有的[实用数](https://en.wikipedia.org/wiki/Practical_number)（两个数字之间的对数，含）。

**实用数**或**实数**是正整数 `N`，这样所有小于 `N` 的正整数都可以表示为 `N` 的不同除数之和。

**例如：** 12 是一个实用数字，因为从 1 到 11 的所有数字都可以表示为其除数 1，2，3，4 和 6 的和（`5 = 3 + 2`，`7 = 6 + 1`，`8 = 6 + 2`，`9 = 6 + 3`，`10 = 6 + 3 + 1`，`11 = 6 + 3 + 2`）。
