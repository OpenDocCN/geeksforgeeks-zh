...
> 原文：[https://www.geeksforgeeks.org/program-to-print-step-pattern/](https://www.geeksforgeeks.org/program-to-print-step-pattern/)
...
1.  设置一个标志来表示是增加还是减少。
2.  设置一个变量 `x` 来表示要打印的 `*` 的数量，并将其初始化为 0。
3.  遍历字符串中的所有字符。
4.  对于每个字符打印 `x` 个 `*`。
5.  如果设置了标志，则增加。
6.  否则减量。
