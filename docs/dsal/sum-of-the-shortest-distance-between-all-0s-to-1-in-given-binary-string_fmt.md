...
> 原文: [https://www.geeksforgeeks.org/sum-of-the-shortest-distance-between-all-0s-to-1-in-given-binary-string/](https://www.geeksforgeeks.org/sum-of-the-shortest-distance-between-all-0s-to-1-in-given-binary-string/)

给定一个二进制[字符串](https://www.geeksforgeeks.org/string-data-structure/) `S`，任务是找出给定字符串 `S` 中所有 `0` 到 `1` 的最短距离之和。
...
给定的问题可以用[贪婪方法](https://www.geeksforgeeks.org/greedy-algorithms/)解决。其思想是为从左侧距离其最近的每个 `0` 搜索 `1`。同样，从右侧开始，为最靠近的每个 `0` 搜索 `1`。最后，根据左右两侧的计算值，计算任意 `0` 至 `1` 的最小距离之和。按照以下步骤解决给定的问题。

*   初始化两个数组 `prefixDistance(N)`、`suffixDistance(N)` 分别存储左右距离。
*   初始化一个变量，比如说 `cnt = 0`，它存储任何一个 `0` 到最近的 `1` 之间的距离。
*   初始化一个变量，比如说 `haveOne = false`，标记字符 `1`。
*   初始化一个变量，比如说 `sum = 0`，它存储所有 `0` 到最近的 `1` 之间的总和。
*   使用变量 `i` 迭代范围 `[0, N-1]` 执行以下步骤:
    *   如果 `S[i]` 的值为 `1`，则将 `haveOne` 指定为 `true`、`cnt` 指定为 `0`、`prefixDistance[i]` 指定为 `0`。
    *   否则，如果 `haveOne` 为 `true`，则将 `cnt` 的值增加 `1`，并将 `prefixDistance[i]` 的值指定为 `cnt`。
*   使用变量 `i` 迭代范围 `[0, N-1]`，执行以下步骤:
    *   如果 `S[i]` 的值为 `1`，则将 `haveOne` 指定为 `true`、`cnt` 指定为 `0`、`suffixDistance[i]` 指定为 `0`。
    *   否则，如果 `haveOne` 为 `true`，则将 `cnt` 的值增加 `1`，并将 `suffixDistance[i]` 的值指定为 `cnt`。
*   使用变量 `i` 迭代范围 `[0, N-1]`，如果 `S[i]` 的值为 `0`，则将 `sum` 更新为 `sum += min(prefixDistance[i], suffixDistance[i])`。
