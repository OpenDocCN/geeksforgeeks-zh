...
> 原文: [https://www.geeksforgeeks.org/program-to-reverse-columns-in-given-2d-array-matrix/](https://www.geeksforgeeks.org/program-to-reverse-columns-in-given-2d-array-matrix/)
...
## 时间复杂度
`O(N^2)`，使用两个 `for` 循环，一个循环遍历所有列，内部循环交换每列的开始和结束索引。

## 辅助复杂度
`O(1)`，作为无额外空间，甚至不用于交换两个元素。