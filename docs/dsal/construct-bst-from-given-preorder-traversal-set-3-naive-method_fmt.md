...
> 原文：[https://www.geeksforgeeks.org/construct-bst-from-given-preorder-traversal-set-3-naive-method/](https://www.geeksforgeeks.org/construct-bst-from-given-preorder-traversal-set-3-naive-method/)
...
**时间复杂度：** 排序需要 `O(nlogn)` 时间，使用前序和中序遍历进行排序和构造需要线性时间。因此，上述解决方案的总时间复杂度为 `O(nlogn)`。
**辅助空间：** `O(n)`。