...
> 超 D 数是一个数字 `N`，使得 `D * N^D` 包含一个由仅包含 `D` 的 `D` 个数字组成的子串，其中 `D` 大于 1 且小于 10
...
**方法:** 想法是创建每一个可能的字符串串联数字 `D`，`D` 的次数，然后将检查串联是否作为子串出现在 `D * N^D` 中，其中 `D` 将在范围 `[2, 9]` 内。
以下是上述方法的实施:
...
**参考文献:** [OEIS](http://oeis.org/A032743)