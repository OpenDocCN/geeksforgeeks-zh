...
> 原文: [https://www.geeksforgeeks.org/check-whether-n-is-a-dihedral-prime-number-or-not/](https://www.geeksforgeeks.org/check-whether-n-is-a-dihedral-prime-number-or-not/)
...
预先计算素数筛，用于素数测试。厄拉多塞的筛可以用 `n*logn*logn` 时间来计算。对数字及其不同方向进行素性测试。如果数字通过了素性测试，检查是否有任何数字属于排除集 `[3, 4, 6, 7, 9]`。如果数字通过了这两个测试，则返回真。
