...
> 原文: [https://www.geeksforgeeks.org/calculate-time-required-to-type-a-word-using-given-single-row-keyboard/](https://www.geeksforgeeks.org/calculate-time-required-to-type-a-word-using-given-single-row-keyboard/)

给定一个大小为 `26` 的[字符串](https://www.geeksforgeeks.org/string-data-structure/) `keyboardLayout` 和一个[字符串](https://www.geeksforgeeks.org/string-data-structure/) `word`，任务是计算输入单词所花费的总时间，从 `0` 第键开始，如果移动到相邻的键需要单位时间。
...
*   初始化两个变量，说 `last`，存储最后一次更新的索引，`result`，存储输入单词的总时间。
*   [迭代字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-c/)中的字符 `word`:
    *   初始化两个变量，说 `destination`，存储需要输入的下一个字符的索引，`distance`，存储该索引与当前索引的距离。
    *   将 `distance` 的值加到 `result` 上。
    *   更新 `last` 到 `destination`。
*   完成上述操作后，打印 `result` 的值。
