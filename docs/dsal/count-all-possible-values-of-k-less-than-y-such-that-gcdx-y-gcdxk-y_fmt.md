...
> 原文: [https://www.geeksforgeeks.org/count-all-possible-values-of-k-less-than-y-such-that-gcdx-y-gcdxk-y/](https://www.geeksforgeeks.org/count-all-possible-values-of-k-less-than-y-such-that-gcdx-y-gcdxk-y/)

给定两个整数 `X` 和 `Y`，任务是求整数个数 `K`，使得 [`gcd(X，Y)`](https://www.geeksforgeeks.org/euclidean-algorithms-basic-and-extended/) 等于 [`gcd(X+K，Y)`](https://www.geeksforgeeks.org/euclidean-algorithms-basic-and-extended/)，其中 `0 < K < Y`。

**例:**

> **输入:** `X = 3`，`Y = 15`
> **输出:** `4`
> **说明:** `K` 的所有可能值为 `{0，3，6，9}`，其中 [`GCD(X，Y)`](https://www.geeksforgeeks.org/stdgcd-c-inbuilt-function-finding-gcd/) = `GCD(X + K，Y)`。
>
> **输入:** `X = 2`，`Y = 12`
> **输出:** `2`
> **说明:** `K` 的所有可能值都是 `{0，8}`。
