...
> 原文: [https://www.geeksforgeeks.org/maximum-product-subarray-size-k/](https://www.geeksforgeeks.org/maximum-product-subarray-size-k/)
...
**方法 1 (简单: O(n*k))**
一种简单的方法是逐个考虑 `k` 大小的所有子阵列。这种方法需要两个循环，因此复杂度为 `O(n * k)`。
