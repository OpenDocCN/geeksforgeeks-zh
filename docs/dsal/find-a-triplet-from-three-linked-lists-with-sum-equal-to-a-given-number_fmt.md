...
链表`b`和`c`可以使用归并排序以`O(nLogn)`时间排序（请参阅[这里](https://www.geeksforgeeks.org/merge-sort-for-linked-list/)）。 步骤 2 花费`O(n * n)`时间。 因此，总体时间复杂度为`O(nlogn) + O(nlogn) + O(n * n) = O(n * n)`。
