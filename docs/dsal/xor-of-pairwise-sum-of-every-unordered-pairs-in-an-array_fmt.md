...
> 原文: [https://www.geeksforgeeks.org/xor-of-pairwise-sum-of-every-unordered-pairs-in-an-array/](https://www.geeksforgeeks.org/xor-of-pairwise-sum-of-every-unordered-pairs-in-an-array/)
...
*   为了获得我们在所有对和中看到的最终异或值的第 K 位，它们的第 k 位被置位或未被置位。如果甚至有许多对设置了第 K 位，那么对于第 K 位，它们的异或是零。
*   为了找到设置了第 K 位的对和的计数，我们注意到我们可以通过 `2^(K+1)` 来修改所有数组元素。这是因为 `X` 和 `Y` 属于输入数组，**和= X + Y**。然后 `X + Y` 相加可以设置它们的 K 位，这意味着 `Sum >= 2^K`。还可以观察到，它们可以有加法的遗留，这使得范围 `[2^(K+1), 2^(K+1) + 2^K)` 中的数字的 K 位未置位。所以我们只关心所有数字的第 K 位和第(K+1)位来检查第 K 位的异或。
*   执行 mod 操作后，对于设置了 kth 位的 sum，其值将在范围–`[2^K, 2^(K+1)) U [2^(K+1)+2^K, 最大值-Sum-Can-Take]`内。

*   要找到上述范围内的数字，制作另一个包含 `arr[]` 的修改数组元素的数组 `B` 并对它们进行排序。那么 `Sum` 可以假设为 `Sum = B_i + B_j`。最后用二分搜索法求 `j` 的最大界(C++内置[下界](https://www.geeksforgeeks.org/upper_bound-and-lower_bound-for-vector-in-cpp-stl/))。修复 `I`，因为数组是排序的，所以找到满足给定条件的最后一个 `j`，并且索引范围内的所有数字都可以添加到计数中以检查 xor。
