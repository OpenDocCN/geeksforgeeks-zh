...
> 原文：[https://www.geeksforgeeks.org/program-for-variance-and-standard-deviation-of-an-array/](https://www.geeksforgeeks.org/program-for-variance-and-standard-deviation-of-an-array/)
...
> 平均值是元素的平均值。
> `arr[0..n-1]` 的平均值 = `∑(arr[i]) / n`，其中 `0 <= i < n`。
> 方差是平均值除以元素数的平方差之和。
> **方差** = `∑(arr[i]–mean)² / n`。
> 标准差为方差的平方根。
> **标准差** = `√(variance)`。
