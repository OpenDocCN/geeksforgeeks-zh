...
要解决上面提到的问题，解决方法是检查每个元素是否是质数。如果它是质数，那么检查它是否只出现一次。一旦找到一个出现一次的主元素，就打印它。

**时间复杂度:** `O(N^2)`

## 高效方法

上述方法可以使用[筛选](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)和[哈希算法](https://www.geeksforgeeks.org/hashing-in-java/)进一步优化。
