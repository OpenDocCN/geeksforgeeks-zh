...
给定一个正整数 `N`，任务是构造第一个 `N` 自然数的[排列](https://www.geeksforgeeks.org/check-if-an-array-is-a-permutation-of-numbers-from-1-to-n/)，使得相邻元素之间的绝对差为 `2`、`3` 或 `4`。如果无法构造这样的排列，则打印`-1`。
...
给定的问题可以通过将连续的奇偶元素组合在一起构造[排列](https://www.geeksforgeeks.org/permutation-coefficient/)来解决。按照以下步骤解决问题:
...
*   初始化一个变量，说 `i` 为 `N`，执行以下步骤:
    ...
    *   [迭代直到](https://www.geeksforgeeks.org/c-c-while-loop-with-examples/) `i` 的值至少为 `1` 并打印 `i` 的值，将 `i` 的值递减 `2`。
