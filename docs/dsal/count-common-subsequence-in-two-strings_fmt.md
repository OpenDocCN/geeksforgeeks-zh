...
> 原文：[https://www.geeksforgeeks.org/count-common-subsequence-in-two-strings/](https://www.geeksforgeeks.org/count-common-subsequence-in-two-strings/)

给定两个字符串 `S` 和 `T`。任务是统计 `S` 和 `T` 中公共子序列的个数。
...
为了找到两个字符串（比如 `S` 和 `T`）中公共子序列的数量，我们通过定义一个二维数组 `dp[][]`，使用动态规划，其中 `dp[i][j]` 是字符串 `S[0…i-1]` 和 `T[0…j-1]` 中公共子序列的数量。

现在，我们可以将 `dp[i][j]` 定义为：

- 当 `S[i-1]` 等于 `T[j-1]` 时：
  `dp[i][j] = dp[i][j-1] + dp[i-1][j] + 1`
  这是因为当 `S[i-1] == T[j-1]` 时，利用上述事实，所有先前的公共子序列在被追加一个字符时会加倍。`dp[i][j-1]` 和 `dp[i-1][j]` 都包含 `dp[i-1][j-1]`，因此在我们的递归中它被添加了两次，这使得所有先前的公共子序列的计数加倍。递归中加 1 是为了最新的字符匹配：由 `S[i-1]` 和 `T[j-1]` 组成的公共子序列。

- 当 `S[i-1]` 不等于 `T[j-1]` 时：
  `dp[i][j] = dp[i-1][j] + dp[i][j-1] - dp[i-1][j-1]`
  这里我们减去 `dp[i-1][j-1]` 一次，因为它同时存在于 `dp[i][j-1]` 和 `dp[i-1][j]` 中，并且被加了两次。
