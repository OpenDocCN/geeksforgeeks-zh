...
> 原文：[https://www.geeksforgeeks.org/sort-an-array-having-first-n-elements-sorted-and-last-m-elements-are-unsorted/](https://www.geeksforgeeks.org/sort-an-array-having-first-n-elements-sorted-and-last-m-elements-are-unsorted/)

给定两个正整数 `N` 和 `M`，以及一个由 `(N + M)` 个整数组成的[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `arr[]`，使得前 `N` 个元素按[升序排序](https://www.geeksforgeeks.org/c-program-to-sort-an-array-in-ascending-order/)，最后 `M` 个元素未排序，任务是[按升序排序给定数组](https://www.geeksforgeeks.org/c-program-to-sort-an-array-in-ascending-order/)。
...
**高效方式：** 上述方法也可以利用[归并排序](https://www.geeksforgeeks.org/merge-sort/)的思路进行优化。其思想是对最后 `M` 个数组元素执行归并排序操作，然后对数组的前 `N` 个和最后 `M` 个元素使用[合并两个已排序数组](https://www.geeksforgeeks.org/merge-two-sorted-arrays/)的概念。按照以下步骤解决问题：

*   使用本文中讨论的方法，对最后 `M` 个数组元素执行归并排序操作。
*   经过上述步骤后，子数组 `arr[0...N-1]` 和 `arr[N...N+M-1]` 按升序排序。
*   使用本文中讨论的方法，合并两个已排序的子数组 `arr[0...N-1]` 和 `arr[N...N+M-1]`。
