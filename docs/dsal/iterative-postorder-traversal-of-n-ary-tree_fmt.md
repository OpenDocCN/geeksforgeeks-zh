...
> 原文: [https://www.geeksforgeeks.org/iterative-postorder-traversal-of-n-ary-tree/](https://www.geeksforgeeks.org/iterative-postorder-traversal-of-n-ary-tree/)
...
*   从根开始。
*   重复以下所有步骤，直到任`root`!=空或堆栈不为空。
    1.  如果`root`!= null，然后将`root`作为索引推入堆栈，并继续向左侧节点推进。
    2.  从堆栈中弹出元素并打印出来。
    3.  从堆栈中弹出所有元素，直到堆栈不为空&&弹出的节点是的最后一个子节点，它是父节点。
    4.  将`root`分配给堆栈顶部节点的下一个子节点。
