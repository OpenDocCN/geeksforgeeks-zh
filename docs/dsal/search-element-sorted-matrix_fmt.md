...
给定一个排序的矩阵 `mat[n][m]` 和一个元素 `x`。找到 `x` 在矩阵中的位置（如果存在），否则打印 `-1`。矩阵的排序方式是，一行中的所有元素按递增顺序排序，对于行 `i`，其中 `1 <= i <= n-1`，行 `i` 的第一个元素大于或等于行 `i-1` 的最后一个元素。该方法应该具有 `O(log n + log m)` 的时间复杂度。
...
1.  首先我们会找到正确的行，其中 `k` 可能存在。为此，我们将在第一列和最后一列同时应用二分搜索法。
    - `低=0，高=n-1`
    - I) 如果 (`k < first element of row(a[mid][0])` ) => `k` 必须存在于上一行中 => `高=中-1`；
    - ii) 如果 (`k >行的最后一个元素(a[mid][m-1])`) => `k` 必须存在于上面的行中 => `低=中+1`；
    - iii) if( `k >行的第一个元素(a[mid][0])` & `k <行的最后一个元素(a[mid][m-1])` ) => `k` 必须存在于此行中 => 像在一维数组中一样在这一行中应用二分搜索法
