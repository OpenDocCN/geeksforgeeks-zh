...
> 原文: [https://www.geeksforgeeks.org/smallest-number-greater-than-n-that-can-be-represented-as-a-sum-of-distinct-power-of-k/](https://www.geeksforgeeks.org/smallest-number-greater-than-n-that-can-be-represented-as-a-sum-of-distinct-power-of-k/)
...
3.  在这种情况下，该幂被加 `k`（`k` 的位置值）次，这使得它多一次幂（`((k-1)+1)`）。`k^x = k * k^x = k^(x+1)`。
4.  因为必须找到最小的数，所以在这一步之后，`k` 的所有低次幂都被减少到 `0`，因为相加 `(k-b)k^x`（`b` = 基础 `k` 表示中该位置的值）已经使大于前一个数。
5.  最后，将数字转换回十进制形式。
