...
> 原文: [https://www.geeksforgeeks.org/sum-of-absolute-differences-of-indices-of-occurrences-of-each-array-element/](https://www.geeksforgeeks.org/sum-of-absolute-differences-of-indices-of-occurrences-of-each-array-element/)
...
最简单的方法是遍历给定数组，对于每个元素 `arr[i]` (`0≤i≤N`)，遍历数组，检查 `arr[i]` 是否与 `arr[j]` (`0≤j≤N`) 相同。如果发现为真，将 `abs(i–j)` 加到总和中，打印每个数组元素的总和。
...
思路是利用地图数据结构对上述途径进行优化。按照以下步骤解决问题:

1.  初始化一个映射来存储数组中每个唯一元素的重复索引的向量。
2.  遍历从 `i = 0` 到 `N–1` 的给定数组，对于每个数组元素 `arr[i]`，用 `0` 初始化和，遍历向量 `mp[arr[i]]`，该映射存储元素 `arr[i]` 出现的索引。
