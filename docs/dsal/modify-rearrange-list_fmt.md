...
首先按照上述说明修改链表，即，如果下一个有效数字与当前数字相同，则将其值加倍并将下一个数字替换为 0。
...
```cs
// C# implementation to modify and rearrange list
using System;
...
class GFG
{
    ...
    static Node segValidInvalidNum(Node head)
    {
        // valid (non-zero numbers) list start and end pointers
        Node validStart = null,
             validEnd = null;
        // invalid (zero numbers) list start and end pointers
        ...
        while (currentNode != null)
        {
            ...
            // if it is a non-zero element, then add it to valid list
            if (element != 0)
            {
                if (validStart == null)
                {
            ...
            // else it is a zero element, so add it to invalid list
            else
            {
                if (invalidStart == null)
                {
    ...
    // function to modify and rearrange list
    static Node modifyAndRearrangeList(Node head)
    {
        // if list is empty or contains a single element only
        if (head == null ||
            head.next == null)
            return head;
        ...
        // segregate valid (non-zero) and invalid (non-zero) numbers
        return segValidInvalidNum(head);
    }
