...
> 原文: [https://www.geeksforgeeks.org/sort-an-array-by-swapping-elements-of-different-type-specified-by-another-array/](https://www.geeksforgeeks.org/sort-an-array-by-swapping-elements-of-different-type-specified-by-another-array/)

给定两个分别包含整数元素及其各自类型（类型 0 或类型 1）的数组 `a[]` 和 `b[]`，任务是检查是否有可能仅通过交换不同类型的元素以非递减顺序对数组进行排序。
...
*   如果数组 `a[]` 没有排序并且所有元素都是同一类型，那么答案是 **否**，因为不可能交换。
