...
> 原文: [https://www.geeksforgeeks.org/find-the-number-of-divisors-of-all-numbers-in-the-range-1-n/](https://www.geeksforgeeks.org/find-the-number-of-divisors-of-all-numbers-in-the-range-1-n/)
...
创建一个大小为 `(N + 1)` 的数组 `arr[]`，其中 `arr[i]` 存储 `i` 的除数。现在，对于范围 `[1, N]` 中的每一个 `j`，递增所有可被 `j` 整除的元素。
