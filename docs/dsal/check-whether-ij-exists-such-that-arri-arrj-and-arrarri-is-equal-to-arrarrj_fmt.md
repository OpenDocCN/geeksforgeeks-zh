# 检查是否存在索引 (i, j) 使得 arr[i] != arr[j] 且 arr[arr[i]] 等于 arr[arr[j]]

> 原文: [https://www.geeksforgeeks.org/check-whether-ij-exists-such-that-arri-arrj-and-arrarri-is-equal-to-arrarrj/](https://www.geeksforgeeks.org/check-whether-ij-exists-such-that-arri-arrj-and-arrarri-is-equal-to-arrarrj/)

给定数组 `A[]`。任务是确定是否有可能选择两个索引 `i` 和 `j`，从而满足以下条件：

1.  `A[i]` 不等于 `A[j]`。
2.  `A[A[i]]` 等于 `A[A[j]]`。

**注意:** 数组中元素的值小于 `N` 的值，即对于每个 `i`，`arr[i] < N`。
...
2.  变量 `i` 指向索引 0，变量 `j` 指向下一个 `i`。
3.  如果 `Arr[i]` 不等于 `Arr[j]`，则检查 `Arr[Arr[i]–1]` 是否等于 `Arr[Arr[j]–1]`。如果是，则返回真。否则也检查其他索引的 `Arr[Arr[i]- 1]` 和 `Arr[Arr[j]–1]`。
