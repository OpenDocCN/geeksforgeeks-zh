...
> 原文：[https://www.geeksforgeeks.org/find-smallest-positive-number-y-such-that-bitwise-and-of-x-and-y-is-zero/](https://www.geeksforgeeks.org/find-smallest-positive-number-y-such-that-bitwise-and-of-x-and-y-is-zero/)

给定一个整数 `X`。任务是找到最小的正数 `Y` ( > 0)，使得 `X` [和](https://www.geeksforgeeks.org/bitwise-operators-in-c-cpp/) `Y` 的按位与为零。
...
*   如果 `X` 的二进制表示包含全 1，那么 `Y` 的所有位都应该是 0，使按位与运算的结果为零。那么 `X+1` 就是我们的答案，也就是第一个正整数。
*   如果 `X` 的二进制表示不包含全 1，那么在 `X` 中找到第一个位置，该位置的位是 0。那么我们的答案将是 `pow(2, position)`。
