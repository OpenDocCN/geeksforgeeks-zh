...
> 原文: [https://www.geeksforgeeks.org/find-the-index-of-the-array-elements-after-performing-given-operations-k-times/](https://www.geeksforgeeks.org/find-the-index-of-the-array-elements-after-performing-given-operations-k-times/)

给定一个数组 `arr[]` 和一个整数 `K`，任务是打印数组元素的位置，其中结果中的第 `i` 个值是在精确地应用以下操作 `K` 次后原始数组中第 `i` 个元素的索引：
...
> 操作 4 -> `arr[] = {2, 2, 2}` (位置 `{3, 0, 2}`) -> `{2, 2, 1}` (位置 `{0, 2, 3}`)。
>
> **输入：** `arr[] = {1, 2, 3}`，`K = 3`
> **输出：** `{1, 2}`
> **解释：**
...
**方法：** 思路是用一个队列来模拟 `K` 次操作。按照以下步骤解决问题：

1.  初始化一个队列来存储这对 `{arr[i], i}`。
