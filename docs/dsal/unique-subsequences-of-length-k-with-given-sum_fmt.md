...
> 原文: [https://www.geeksforgeeks.org/unique-subsequences-of-length-k-with-given-sum/](https://www.geeksforgeeks.org/unique-subsequences-of-length-k-with-given-sum/)

给定一个由 `N` 个整数和两个数字 `K` 和 `S` 组成的数组 `arr[]`，任务是打印长度为 `K` 的所有子序列，其和为 `S`。
...
> **输入:** `N = 10`，`K = 5`，`S = 20`，`arr[] = {1, 4, 2, 8, 10, 6, 12, 1, 5, 7}`
> **输出:**
> `{10, 1, 2, 5, 7}`
> `{4, 8, 1, 5, 7}`
> `{4, 8, 10, 1, 2}`
