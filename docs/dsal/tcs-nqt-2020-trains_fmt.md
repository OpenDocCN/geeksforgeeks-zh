...
现在，假设铁路管理局想增加更多的列车来增加频率。因此，启动其他**列车 B**，与**列车 A** 的车站相同。如果**列车 B** 在 `t` 时刻发车，他们想知道**列车 B** 的时间。程序应该像**列车 A** 一样，从第一站到最后一站，在每个车站为**列车 B** 返回一个字符串数组 `S`（时间戳（在 [float](https://www.geeksforgeeks.org/difference-float-double-c-cpp/) 中））。
...
> **输入:** `t = -26.15`
> **输出:** `Invalid Input`
> **说明:** 不存在 -26.15 这样的时间。因此，打印“Invalid Input”。

## 方法

思路是根据给定的 A 列车时刻计算各站之间的时差，按照以下步骤解决问题:
...
*   如果 `t` 的整数部分不在 `[0, 24]` 范围内，或者 `t` 的小数部分不在 `[0, 60]` 范围内，则打印 `"Invalid Input"`，因为整数部分代表小时，小数部分代表分钟。
*   否则，遍历 `[0, 5]` 范围，打印 `t + train_B[i]` 表示**列车 B** 到达 `i` 站的时间。然后更新 `t` 为 `t = t + train_B[i]`。
