...
> 原文: [https://www.geeksforgeeks.org/java-8-arrays-parallelsort-method-with-examples/](https://www.geeksforgeeks.org/java-8-arrays-parallelsort-method-with-examples/)

Java 8 在 `java.util.Arrays` 类中引入了一个名为 `parallelSort()` 的新方法。它使用数组元素的并行排序。

## 并行排序算法

1.  数组被划分为子数组，子数组再次被划分为更小的子数组，直到达到最小的细节层级。
2.  数组由多个线程单独排序。
3.  并行排序使用 Fork/Join 概念进行排序。
4.  排序后的子数组随后被合并。
...
`parallelSort()` 方法使用**多线程**的概念，这使得排序**比正常排序方法更快**。

## 示例

![img](img/5fec5ecb1e8eddff87544f43f37e8cbd.png)
...
注意: 不同的时间间隔将被打印，但并行排序将在串行排序之前完成。

**环境:** 2.6 GHz Intel Core i7，Java 版本 8