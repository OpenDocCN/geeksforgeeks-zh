...
> 原文: [https://www.geeksforgeeks.org/series-largest-gcd-sum-equals-n/](https://www.geeksforgeeks.org/series-largest-gcd-sum-equals-n/)
...
**逼近:**
最常见的观察是数列的 gcd 永远是 `n` 的除数，最大可能的 gcd (`b`) 将是 `n/sum`，其中 `sum` 是 `1+2+..+m` 的和。
如果 `b` 结果是 0，那么 `1+2+3+..+k` 的和超过 `n`，这是无效的，因此输出“-1”。
...
取可能除数的最大值 (`r`)，该值应小于或等于 `b`，并将序列打印为 `r`，`2*r`，… `(m-1)*r`，`n-s`。
如果没有找到这样的除数，只需输出“-1”。
