...
> 原文: [https://www.geeksforgeeks.org/check-root-leaf-path-given-sequence/](https://www.geeksforgeeks.org/check-root-leaf-path-given-sequence/)
...
*   如果`当前节点`等于`arr[index]`，这意味着直到该级别的树路径都是相同的。
*   现在剩下的路径要么在`左子树`中，要么在`右子树`中。
*   如果任何节点与`arr[index]`不匹配，这意味着当前路径与根到叶路径的给定序列不相同，因此我们返回并移入右子树。
*   当我们到达`叶节点`并且它等于`arr[index]`，同时给定的根到叶路径序列中没有更多元素时，这意味着该路径存在于给定的树中。
