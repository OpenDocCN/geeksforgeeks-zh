...
> 原文：[https://www.geeksforgeeks.org/comparison-among-bubble-sort-selection-sort-and-insertion-sort/](https://www.geeksforgeeks.org/comparison-among-bubble-sort-selection-sort-and-insertion-sort/)

## 1. [冒泡排序](https://www.geeksforgeeks.org/bubble-sort/)

[冒泡排序](https://www.geeksforgeeks.org/bubble-sort/)在每一遍中重复比较和交换（如果需要）相邻元素。在冒泡排序的第`I`遍（升序），最后`(I-1)`个元素已经排序，第`I`个最大的元素被放置在第`(N-i)`个位置，即第`I`个最后位置。
...
**算法优化：** 检查内循环（传递执行循环）是否发生交换操作。如果在任何过程中没有交换，这意味着数组现在已经完全排序，因此不需要继续，停止排序操作。因此，当数组在所有遍数完成之前排序时，我们可以优化遍数。它还可以在第一遍中检测给定/输入数组是否排序。
...
*   **最差情况**：反向排序/很少元素在合适的位置。`O(N^2)`。`O(N^2)`互换。
*   **平均情况**：`O(N^2)`。`O(N^2)`互换。

**空间复杂度：** 一个临时变量用于交换【辅助，`O(1)`】。因此它是就地排序。
...
*   **最佳案例**：`O(N^2)`。和`O(1)`互换。
*   **最差情况**：反向排序，内循环最大比较时。`O(N^2)`。此外，`O(N)`互换。
*   **平均情况**：`O(N^2)`。同样`O(N)`互换。

**空间复杂度：** 【辅助，`O(1)`】。就地排序。（当元素被移动而不是被交换时（即 `temp=a[min]`，然后将元素从 `ar[i]` 移动到 `ar[min-1]` 一个位置，然后放入 `a[i]=temp`）。如果选择交换，则算法不在原位。）
...
1.  所有情况下的时间复杂度都是`O(N^2)`，没有最佳情况场景。

## 3. [插入排序](https://www.geeksforgeeks.org/insertion-sort/)
...
*   **最差情况**：反向排序，内环最大比较时，`O(N^2)`。和`O(N^2)`互换。
*   **平均情况**：`O(N^2)`。和`O(N^2)`互换。
