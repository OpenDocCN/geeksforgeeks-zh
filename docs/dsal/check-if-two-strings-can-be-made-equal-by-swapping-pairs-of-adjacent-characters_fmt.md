...
> 原文：[https://www.geeksforgeeks.org/check-if-two-strings-can-be-made-equal-by-swapping-pairs-of-adjacent-characters/](https://www.geeksforgeeks.org/check-if-two-strings-can-be-made-equal-by-swapping-pairs-of-adjacent-characters/)

给定长度分别为 `N` 和 `M` 的两个[字符串](https://www.geeksforgeeks.org/string-data-structure/) `A` 和 `B` 以及由 `K` 个整数组成的[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `arr[]`，任务是通过交换字符串 `A` 的任意一对相邻字符来检查字符串 `B` 是否可以从字符串 `A` 中获得。如果可以将字符串 `A` 转换为字符串 `B`，打印“是”。否则，打印“否”。
...
> **输出：** 是
> **说明：**
> 交换 `A[1]` 和 `A[2]`。现在字符串变成了 `A = "acbabka"`。
> 互换 `A[4]` 和 `A[5]`。现在字符串变成了 `A = "acbakba"`，和字符串 `B` 一样。
>
...
*   如果两个字符串的长度不相等，那么字符串 `A` 就不能转化为字符串 `B`。因此，打印“否”。
*   [遍历给定数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]`，检查 `A[arr[i]]` 和 `B[arr[i]]` 处的字符是否相同。如果发现属实，则打印“否”。否则，请执行以下步骤：
    *   如果 `arr[i]` 的第一个元素不是 `0`：
        *   将从 `0` 到 `arr[i]` 的 `A` 和 `B` 的所有字符存储在两个字符串中，比如 `X` 和 `Y`。
        *   [查找字符串 `X` 和 `Y`](https://www.geeksforgeeks.org/print-characters-frequencies-order-occurrence/) 的字符频率。
        *   如果所有字符的频率相同，则打印“否”。
