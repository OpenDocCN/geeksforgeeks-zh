...
> 原文: [https://www.geeksforgeeks.org/sum-of-absolute-differences-of-pairs-from-the-given-array-that-satisfy-the-given-condition/](https://www.geeksforgeeks.org/sum-of-absolute-differences-of-pairs-from-the-given-array-that-satisfy-the-given-condition/)
...
(5, 2) -> `abs(12 - 3) = 9`
(5, 3) -> `abs(12 - 5) = 7`
11 + 4 + 2 + 5 + 10 + 4 + 9 + 7 = 52 (注：原文示例计算有误，此处按逻辑修正)
...
初始化 `sum = 0` 并运行两个嵌套循环，对于每对 `arr[i]`, `arr[j]`，如果 `(j - i)` 为素数，则将 `sum` 更新为 `sum = sum + abs(arr[i] - arr[j])`。最后打印 `sum`。
