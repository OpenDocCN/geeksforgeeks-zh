...
> 原文: [https://www.geeksforgeeks.org/absolute-difference-between-the-product-of-non-prime-numbers-and-prime-numbers-of-an-array/](https://www.geeksforgeeks.org/absolute-difference-between-the-product-of-non-prime-numbers-and-prime-numbers-of-an-array/)
...
一个简单的解决方案是遍历数组，并不断检查每个元素是否是质数。如果数是质数，那么乘以 `P2` 积，它代表质数的乘积，否则检查它是否不是 1，然后乘以非质数的乘积，比如说 `P1`。遍历整个数组后，取两者的绝对差值`abs(P1-P2)`。
时间复杂度: `O(N*sqrt(N))`

## 高效方法

使用厄拉多塞筛生成数组最大元素之前的所有素数，并将其存储在哈希中。现在，遍历数组并检查该数字是否出现在哈希映射中。然后，将这些数字乘以 `P2` 乘积，检查它是否不是 1，然后乘以 `P1` 乘积。遍历整个数组后，显示两者之间的绝对差异。
以下是上述方法的实施:
