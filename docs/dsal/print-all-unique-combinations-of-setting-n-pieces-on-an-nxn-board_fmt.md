...
> 原文：[https://www.geeksforgeeks.org/print-all-unique-combinations-of-setting-n-pieces-on-an-nxn-board/](https://www.geeksforgeeks.org/print-all-unique-combinations-of-setting-n-pieces-on-an-nxn-board/)
...
这个问题可以通过使用递归生成所有可能的解来解决。现在，按照以下步骤解决这个问题：

1.  创建一个名为 `allCombinations` 的函数，它将生成所有可能的解。
2.  它将取一个整数 `piecesPlaced` 表示放置的总件数，整数 `N` 表示需要放置的件数，两个整数 `row` 和 `col` 表示当前件将要放置的行和列，以及一个字符串 `ans` 作为参数，用于存储放置件的矩阵。
3.  现在，对 `allCombinations` 的初始调用将通过 `0` 作为 `piecesPlaced`、`N`、`0` 和 `0` 作为 `row` 和 `col`，以及一个空字符串作为 `ans`。
4.  在每次通话中，检查基本情况，即：
    *   如果行变成 `N` 并且所有的片都被放置，即 `piecesPlaced == N`。然后打印 `ans` 返回。否则如果 `piecesPlaced` 不是 `N`，那么就从这个调用返回。
5.  现在打两个电话：
