...
> 原文: [https://www.geeksforgeeks.org/modify-array-by-removing-characters-from-their-hexadecimal-representations-which-are-present-in-a-given-string/](https://www.geeksforgeeks.org/modify-array-by-removing-characters-from-their-hexadecimal-representations-which-are-present-in-a-given-string/)

给定一个大小为 `N` 的数组 `arr[]` 和一个字符串 `S`，任务是通过从 `S` 中存在的十六进制表示中移除所有字符，然后将等效的十进制元素替换回数组来修改给定数组。
...
> **输出:** `{4, 5, 15, 7}`
> **解释:**
> 74 -> (4A)<sup>16</sup> -> (4)<sub>16</sub> -> 4
> 91 -> (5B)<sub>16</sub> -> (5)<sub>16</sub> -> 5
>
