...
> 原文：[https://www.geeksforgeeks.org/find-n-geometric-means-between-a-and-b/](https://www.geeksforgeeks.org/find-n-geometric-means-between-a-and-b/)
...
**进场：**
让 `A₁`，`G₂`，`G₃`，`G₄` ……`Gₙ` 为两个给定数字 `A` 和 `B` 之间的 `N` 个几何平均数，然后 `A`、`G₁`，`G₂` …..`Gₙ`，`B` 将处于几何级数。
所以 `B` = (`N`+2)ᵗʰ 几何级数的第项。
那么这里 `R` 是公比
`B` = `A` * `R`<sup>`N`+1</sup>
`R`<sup>`N`+1</sup> = `B`/`A`
`R` = (`B`/`A`)<sup>`1`/(`N`+1)</sup>
现在我们有了 `R` 的值，也有了第一项 `A`
`G₁` = `A` * `R` = `A` * (`B`/`A`)<sup>`2`/(`N`+1)</sup>
`G₂` = `A` * `R`<sup>`2`</sup> = `A` * (`B`/`A`)<sup>`2`/(`N`+1)</sup>
`G₃` = `A` * `R`<sup>`3`</sup> = `A` * (`B`/`A`)<sup>`3`/(`N`+1)</sup>
。
。
。
**`Gₙ` = `A` * `R`<sup>`N`</sup> = `A` * (`B`/`A`)<sup>`N`/(`N`+1)</sup>**
