...
——现在，请注意等式 1 和等式 2——我们正试图重新解决同一个问题。所以我们要递归到 `m > 0`。要使这样的解存在，`(m-ci)` 必须是 `w` 的倍数，其中 `ci` 是方程的系数。`ci` 可以是-1，0，1。因此，我们必须检查所有三种可能性 `((m–1)% w = = 0)`、`((m + 1 ) % w == 0)` 和 `(m % w == 0)`。如果不是，那么就不会有任何解决办法。
