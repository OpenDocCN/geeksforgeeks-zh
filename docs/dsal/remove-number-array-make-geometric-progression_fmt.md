...
> 原文: [https://www.geeksforgeeks.org/remove-number-array-make-geometric-progression/](https://www.geeksforgeeks.org/remove-number-array-make-geometric-progression/)

给定一个由 `N` 个正元素组成的数组 `arr[]`，任务是通过最多移除一个元素来确定是否有可能将该数组转换为几何级数（GP）。如果是，则找到将数组转换为几何级数的移除数的索引。
