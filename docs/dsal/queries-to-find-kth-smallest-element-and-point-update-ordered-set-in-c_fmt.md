# 查询以查找第k个最小元素和点更新：C++中的有序集

> 原文：[https://www.geeksforgeeks.org/queries-to-find-kth-smallest-element-and-point-update-ordered-set-in-c/](https://www.geeksforgeeks.org/queries-to-find-kth-smallest-element-and-point-update-ordered-set-in-c/)

给定大小为 `N` 的数组 `arr[]` 和包含 `M` 个查询的集合 `Q[][]`，任务是在给定的数组上执行查询，这样可以有两种类型的查询：

*   `类型1`：`[i, x]` – 将第 `i` 个索引处的元素更新为 `x`。
*   `类型2`：`[k]` – 找到数组中第 `k` 个最小元素。
...
> **输出：** `5 2`
> **解释：**
> 对于第 1 个查询：`arr[] = {4, 5, 6, 2}`
> 对于第 2 个查询：第 3 个最小元素为 5。
> 对于第 3 个查询：`arr[] = {7, 5, 6, 2}`
> 对于第 4 个查询：第 1 个最小元素为 2。
...
这个问题的天真方法是在恒定时间内更新数组中的第 `i` 个元素，并使用排序来[找到第 K 个最小元素](https://www.geeksforgeeks.org/kth-smallestlargest-element-unsorted-array/)。

**时间复杂度：** `O(M * (N * log(N)))`，其中 `M` 为查询数，`N` 为数组大小。

## 高效方法

想法是使用类似于[`set`](https://www.geeksforgeeks.org/set-in-cpp-stl/)的[基于策略的数据结构](https://www.geeksforgeeks.org/policy-based-data-structures-g/)。
...
### `类型1`
对于这个查询，我们更新数组的第 `i` 个元素。因此，我们需要更新数组和数据结构中的值。为了更新树容器中的值，在树中找到 `arr[i]` 值，将其从树中删除，并将更新后的值插回到树中。

### `类型2`
为了找到第 `K` 个最小的元素，在树上使用 `find_by_order(K-1)`，因为数据是已排序的数据。这类似于排序数组上的[二分搜索法](https://www.geeksforgeeks.org/binary-search/)操作。
