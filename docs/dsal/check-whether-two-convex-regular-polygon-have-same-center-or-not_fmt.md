...
> 原文: [https://www.geeksforgeeks.org/check-whether-two-convex-regular-polygon-have-same-center-or-not/](https://www.geeksforgeeks.org/check-whether-two-convex-regular-polygon-have-same-center-or-not/)

给定两个正整数 `N` 和 `M`，表示凸正多边形的边，其中 `N < M`，任务是检查当 `N` 边多边形内接于 `M` 边多边形时，这两个多边形是否具有相同的中心。
