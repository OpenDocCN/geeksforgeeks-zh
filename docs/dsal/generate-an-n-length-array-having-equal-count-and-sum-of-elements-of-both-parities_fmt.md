...
> 原文: [https://www.geeksforgeeks.org/generate-an-n-length-array-having-equal-count-and-sum-of-elements-of-both-parities/](https://www.geeksforgeeks.org/generate-an-n-length-array-having-equal-count-and-sum-of-elements-of-both-parities/)

给定一个整数 `N` *(3≤N≤10<sup>5</sup>)*，任务是生成一个由 `N` 个不同的正元素组成的数组，这样双方元素的个数和和即偶数和奇数是相同的。如果无法构建这样的阵列，请打印 `-1`。
