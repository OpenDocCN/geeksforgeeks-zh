...
> 原文: [https://www.geeksforgeeks.org/smallest-positive-integer-that-divides-all-array-elements-to-generate-quotients-with-sum-not-exceeding-k/](https://www.geeksforgeeks.org/smallest-positive-integer-that-divides-all-array-elements-to-generate-quotients-with-sum-not-exceeding-k/)

## 问题描述

给定一个大小为 `N` 的数组 `arr[]` 和一个正整数 `K`，任务是找到最小正整数，使得通过将所有数组元素除以该最小正整数得到的剩余数组元素的和不超过 `K`。

**注意:** 将数组元素除以最小正整数必须是 Cell 类型。
...
解决这个问题最简单的方法就是找到数组中最大的元素，记为 `Max`，使用变量 `i` 迭代范围 `[1, Max]`，检查所有数组元素除以 `i` 后剩余数组元素的和是否小于等于 `K`。如果发现是真的，那么打印 `i`。
...
优化上述方法的思路是使用二分搜索法。按照以下步骤解决问题:

*   初始化一个变量，比如 `Max`，来存储数组中最大的元素。
*   将所有数组元素相除得到剩余数组元素之和小于或等于 `K` 的最小正整数的值必须在范围 `[1, Max]` 内。因此，在 `[1, Max]` 范围内应用二分搜索法。
*   初始化两个变量，比如 `left = 1` 和 `right = Max`，以存储所需输出值所在的范围。
*   检查是否有可能通过将数组元素除以 `(left + right) / 2` 得到小于或等于 `K` 的数组元素之和。如果发现属实，则更新 `right = (left + right) / 2`。
*   否则，更新 `left = (left + right) / 2`。
*   最后，打印 `left` 的值。
