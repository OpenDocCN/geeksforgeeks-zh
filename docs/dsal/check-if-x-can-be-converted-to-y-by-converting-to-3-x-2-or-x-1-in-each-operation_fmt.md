...
> 原文: [https://www.geeksforgeeks.org/check-if-x-can-be-converted-to-y-by-converting-to-3-x-2-or-x-1-in-each-operation/](https://www.geeksforgeeks.org/check-if-x-can-be-converted-to-y-by-converting-to-3-x-2-or-x-1-in-each-operation/)
...
*   如果 `X` 为偶数，那么可以转换为 `(3 * (X / 2))`，大于 `X` 的所有偶数都大于 0。
*   如果 `X` 的值为奇数，则 `X` 可以转换为 `(X - 1)`，再转换为 `(3 * (X - 1) / 2)`，大于 `X`。
