...
使用两个循环从两个数组中选取元素，并检查该对的总和是否等于`x`。
...
对于每个元素`arr1[i]`，其中`1 <= i <= m`，搜索`arr2[]`中的`x – arr1[i]`值。如果搜索成功，则增加**计数**。
...
`O(m)`，应创建具有较小大小的数组的哈希表，以降低空间复杂度。

### 方法 4（有效方法）

此方法使用两个指针的概念，一个指针从左到右遍历第一个数组，另一个指针从右到左遍历第二个数组。
