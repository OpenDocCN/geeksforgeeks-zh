...
*   [使用变量`i`、](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)[遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)`A[]`，并执行以下步骤:
...
*   现在[迭代](https://www.geeksforgeeks.org/how-to-traverse-a-c-set-in-reverse-direction/)`St`的集合，检查树`A`中当前节点的子节点的向量是否不等于树`B`中当前节点的子节点的向量，然后打印`No`作为答案，然后[返回](https://www.geeksforgeeks.org/return-statement-in-c-cpp-with-examples/)。
*   最后，如果以上情况都不满足，则打印`Yes`作为答案。
