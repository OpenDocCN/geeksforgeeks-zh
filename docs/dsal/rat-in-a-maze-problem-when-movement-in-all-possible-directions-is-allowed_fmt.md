...
> 原文：[https://www.geeksforgeeks.org/rat-in-a-maze-problem-when-movement-in-all-possible-directions-is-allowed/](https://www.geeksforgeeks.org/rat-in-a-maze-problem-when-movement-in-all-possible-directions-is-allowed/)

假设一只老鼠被放在一个正方形矩阵中的 `(0,0)` 处 `m[][]`，大小为 `n`，并且必须在 `(n-1, n-1)` 到达目的地。任务是找到一个排序的字符串数组，表示老鼠到达目的地 `(n-1, n-1)` 的所有可能方向。老鼠可以移动的方向是“U”(向上)、“D”(向下)、“L”(向左)、“R”(向右)。
