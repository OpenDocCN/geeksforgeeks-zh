...
> 原文：[https://www.geeksforgeeks.org/check-if-the-array-has-an-element-which-is-equal-to-xor-of-remaining-elements/](https://www.geeksforgeeks.org/check-if-the-array-has-an-element-which-is-equal-to-xor-of-remaining-elements/)
...
首先，对数组的所有元素进行异或运算，并将其存储在变量 `xorArr` 中。现在再次遍历整个数组，对于每个元素，计算数组元素的异或，不包括当前元素 `arr[i]`，即 `x = xorArr ^ arr[i]`。
