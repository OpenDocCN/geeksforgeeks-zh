...
**时间复杂度**：`O(N^2)`。最坏的情况发生在所有元素按降序排列的时候。
...
这个方法也很简单，如果你知道段树和二分搜索法。让我们考虑一个数组 `a`，假设 `a_i` 的神经元特异性烯醇化酶是 `a_j`，我们只需要在 `i + 1` 到 `n - 1` 的范围内为 `j` 到二分搜索法。`j` 将是第一个索引 `k`，这样从索引 `i + 1` 到 `k`（`∀k∈ [i+1, n - 1]`）的元素范围最小值小于 `a_i`。

**时间复杂度**：`O(N(log(N))^2)`。对于每个 `n` 数组元素，我们做一个二分搜索法，包括 `log(N)` 步骤，每个步骤花费 `log(N)` 操作【范围最小查询】。
...
1.  沿着这条线，我们将构建一个数组`aux` - 使得`aux[i]`是输入数组中`i`出现的最小索引。
2.  很容易看出，我们需要压缩输入数组来构建这个数组`aux`，因为如果`i`超过`10^7`(在线裁判的内存限制)，我们很可能会出现分割错误。
3.  为了压缩，我们对输入数组进行排序，然后对于数组中的每个新值，如果可能的话，我们将其映射到相应的较小值。使用这些映射值生成一个与输入数组顺序相同的`compressed`数组。
4.  现在我们已经完成了压缩，我们可以从查询部分开始:
    *   假设在上一步中，我们将数组压缩为`ctr`个不同的值。最初设置`aux[i] = -1 \forall i \in [0, ctr)`，这表示到目前为止，任何索引都没有处理任何值。
    *   以相反的顺序遍历压缩数组，这意味着在过去，我们只处理右侧的元素。
        *   对于`compressed[i]`，使用段树查询(并存储在`ans[i]`中)值的最小索引`[0, compressed[i])`，这一定是`compressed[i]`的 NSE！
        *   将`compressed[i]`的索引更新为`i`。
...
int query(int node, int tl, int tr, int l, int r)
{
    ...
    int q1 = query(2 * node, tl, tm, l, min(r, tm));
    int q2 = query(2 * node + 1, tm + 1, tr, max(l, tm + 1), r);
...
2.  一个接一个地拾取剩余的元素，并按照以下步骤循环。
    *   将当前元素标记为`next`。
    *   如果栈不为空，则比较`next`与栈顶。如果`next`比顶部小，那么`next`就是顶部的 NSE。当顶部大于`next`时，继续从堆栈中弹出。`next`成为所有此类弹出元素的神经元特异性烯醇化酶。
    *   将`next`推入堆栈。
