...
> 原文: [https://www.geeksforgeeks.org/find-the-maximum-sum-ab-for-a-given-input-integer-n-satisfying-the-given-condition/](https://www.geeksforgeeks.org/find-the-maximum-sum-ab-for-a-given-input-integer-n-satisfying-the-given-condition/)
...
> **输入:** `N = 10`
> **输出:** 9
> **解释:** 数字 `a = 3` 和 `b = 6` 导致和=9 也是 `6 * 3 = 18`，它可被 `6 + 3 = 9` 整除
>
> **输入:** `N = 20`
> **输出:** 27
...
1.  运行从 1 到 `n` 的嵌套循环
2.  对于 [1, `N`] 范围内的每个数字 `a`，再找一个整数 `b`，这样 `a != b` 且 (`a+b`) 除 `a * b`。
...
1.  如果 `a` 可以表示为 `k * x` 而 `b` 可以表示为 `k * y` 使得 `x` 和 `y` 为[互质](https://www.geeksforgeeks.org/check-two-numbers-co-prime-not/)，那么:
...
5.  显然，`k` 可被 (`x + y`) 整除的最小值是 (`x + y`)。这意味着 `(x+y)*y <= N` 和 `(x+y)*x <= N`。
