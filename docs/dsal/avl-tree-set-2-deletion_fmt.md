...
document.write("</br>");
document.write("Preorder traversal after deletion of 10 :" + "</br>");
preOrder(root);
```
...
**时间复杂度:**
旋转操作（`leftRotate`和`rightRotate`）需要恒定的时间，因为只有很少的指针在那里被改变。更新高度和获得平衡系数也需要恒定的时间。因此，AVL 删除的时间复杂度与 BST 删除相同，BST 删除是 `O(h)`，其中 `h` 是树的高度。由于 AVL 树是平衡的，所以高度为 `O(Log n)`。所以 AVL 删除的时间复杂度是 `O(Log n)`。

**参考文献:**
[CS251 Slides - Chapter 7b](https://www.cs.purdue.edu/homes/ayg/CS251/slides/chap7b.pdf)
