...
> 原文：[https://www.geeksforgeeks.org/java-program-for-finding-a-triplet-from-three-linked-lists-with-sum-equal-to-a-given-number/](https://www.geeksforgeeks.org/java-program-for-finding-a-triplet-from-three-linked-lists-with-sum-equal-to-a-given-number/)
...
2.  在对 `b` 和 `c` 进行排序后，逐个从列表 `a` 中选择一个元素，并通过遍历 `b` 和 `c` 来找到这一对。思路类似于 [3SUM](http://en.wikipedia.org/wiki/3SUM) 问题的二次算法。
