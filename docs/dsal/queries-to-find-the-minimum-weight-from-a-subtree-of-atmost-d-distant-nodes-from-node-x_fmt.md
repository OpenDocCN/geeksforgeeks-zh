...
> 原文: [https://www.geeksforgeeks.org/queries-to-find-the-minimum-weight-from-a-subtree-of-atmost-d-distant-nodes-from-node-x/](https://www.geeksforgeeks.org/queries-to-find-the-minimum-weight-from-a-subtree-of-atmost-d-distant-nodes-from-node-x/)
...
*   在这个范围内，找到距离最大为 `D` 的最小加权节点。构建 `合并排序树`，按照 `深度` 的递增顺序合并两个范围，在 `合并排序树` 的每个节点的值中找到 `前缀最小值`。
