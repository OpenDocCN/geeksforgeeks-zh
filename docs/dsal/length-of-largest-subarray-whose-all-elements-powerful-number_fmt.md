...
> 原文: [https://www.geeksforgeeks.org/length-of-largest-subarray-whose-all-elements-powerful-number/](https://www.geeksforgeeks.org/length-of-largest-subarray-whose-all-elements-powerful-number/)

给定一个整数元素的数组 `arr[]`，任务是找到 `arr[]` 的最大子数组的长度，使得子数组的所有元素都是[强子数](https://www.geeksforgeeks.org/powerful-number/)。

> 如果一个数 `n` 的每一个质因数 `p`，`p^2` 也对其进行除法运算，那么这个数 `n` 就被称为**强数**。
...
*   从左到右遍历数组。用 0 初始化一个 `max_length` 和 `current_length` 变量。
*   如果当前元素是一个[强数](https://www.geeksforgeeks.org/powerful-number/)，那么增加 `current_length` 变量并继续。否则，设置 `current_length = 0`。
*   在每一步，将最大长度指定为 `max_length = max(current_length, max_length)`。
*   最后打印 `max_length` 的值。
