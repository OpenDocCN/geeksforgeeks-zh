...
> 原文：[https://www.geeksforgeeks.org/count-array-elements-exceeding-sum-of-preceding-k-elements/](https://www.geeksforgeeks.org/count-array-elements-exceeding-sum-of-preceding-k-elements/)
...
3.  对于每个数组元素，检查 `arr[i]` 是否超过 `(arr[i-1] + arr[i-2] + … + arr[i-K])`。因此，任务是检查 `arr[i]` 是否超过 `prefix[i-1] - prefix[i-(K+1)]`（对于 `K < i < N`），或者 `arr[i]` 是否超过 `prefix[i-1]`（对于 `i = K`）。
