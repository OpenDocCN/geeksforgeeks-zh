...
这个问题可以使用 [BFS](https://www.geeksforgeeks.org/breadth-first-traversal-for-a-graph/) 解决。我们可以将给定的数组视为未加权图，其中每个顶点都有两条边指向下一个和上一个数组元素，更多的边指向具有相同值的数组元素。现在为了快速处理第三类边缘，我们保留了 10 个[向量](https://www.geeksforgeeks.org/vector-in-cpp-stl/)，用于存储数字 0 到 9 所在的所有索引。在上面的例子中，对应于 0 的向量将存储 `[0，12]` 两个索引，其中 0 出现在给定的数组中。
