...
解决问题的最简单方法是[生成给定字符串所有可能长度的所有可能排列](https://www.geeksforgeeks.org/write-a-c-program-to-print-all-permutations-of-a-given-string/)，并检查是否发生任何排列`N`次。 如果发现是真的，则打印`Yes`。 否则，打印`No`。
...
想法是存储所有字符的频率，并计算至少`N`次出现的字符数。 请按照以下步骤解决问题：
