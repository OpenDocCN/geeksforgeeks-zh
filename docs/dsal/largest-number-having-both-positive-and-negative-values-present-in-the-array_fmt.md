...
> 原文：[https://www.geeksforgeeks.org/largest-number-having-both-positive-and-negative-values-present-in-the-array/](https://www.geeksforgeeks.org/largest-number-having-both-positive-and-negative-values-present-in-the-array/)

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是找到最大的数 `K` (`> 0`)，使得值 `K` 和 `-K` 都出现在给定数组 `arr[]` 中。如果没有，则打印 `-1`。
