...
> 原文: [https://www.geeksforgeeks.org/check-if-any-point-exists-in-a-plane-whose-manhattan-distance-is-at-most-k-from-n-given-points/](https://www.geeksforgeeks.org/check-if-any-point-exists-in-a-plane-whose-manhattan-distance-is-at-most-k-from-n-given-points/)

给定两个数组`A[]`和`B[]`，由平面内`N`个不同点的`X`和`Y`坐标以及正整数`K`组成，任务是检查平面内是否存在点`P`，使得该点与所有给定点之间的曼哈顿距离至多为`K`。如果存在这样的点`P`，则打印`"是"`。否则，打印`"否"`。
...
**方法:** 给定的问题可以通过求每对`N`给定点之间的曼哈顿距离来解决。检查所有点对后，如果点对之间的距离计数最多为`K`，则打印`"是"`。否则，打印`"否"`。
