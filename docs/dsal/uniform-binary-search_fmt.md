...
均匀二分搜索法是对[二分搜索法](https://www.geeksforgeeks.org/binary-search/)算法的优化，在同一个数组或多个大小相同的数组上进行多次搜索。在正常的二分搜索法，我们做算术运算来寻找中点。这里，我们预计算中点，并将其填入查找表。数组查找通常比算术运算(加法和移位)更快找到中点。
...
**参考文献:** [https://en.wikipedia.org/wiki/Uniform_binary_search](https://en.wikipedia.org/wiki/Uniform_binary_search)