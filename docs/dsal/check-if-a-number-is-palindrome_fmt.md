...
> 原文：[https://www.geeksforgeeks.org/check-if-a-number-is-palindrome/](https://www.geeksforgeeks.org/check-if-a-number-is-palindrome/)

给定一个整数，写一个函数，如果给定的数字是回文，则返回 `true`，否则返回 `false`。比如 `12321` 是回文，`1451` 不是回文。

![](img/c731721083c87a35c6b927c4946c4b6a.png)

让给定的数为 `num`。这个问题的一个简单方法是先[倒 `num` 的位数](https://www.geeksforgeeks.org/write-a-c-program-to-reverse-digits-of-a-number/)，然后比较 `num` 和 `num` 的倒位数。如果两者相同，则返回真，否则返回假。

以下是一个有趣的方法，灵感来自[这篇帖子的方法 2](https://www.geeksforgeeks.org/function-to-check-if-a-singly-linked-list-is-palindrome/)。想法是创建一个 `num` 的副本，并通过引用递归传递该副本，通过值传递 `num`。在递归调用中，沿着递归树向下移动时，将 `num` 除以 `10`。在递归树上向上移动时，将副本除以 `10`。当他们在一个所有子调用都结束的函数中相遇时，`num` 的最后一个数字将从开始的第一个数字开始，副本的最后一个数字将从结束的第一个数字开始。
