...
> 原文: [https://www.geeksforgeeks.org/print-numbers-such-that-no-two-consecutive-numbers-are-co-prime-and-every-three-consecutive-numbers-are-co-prime/](https://www.geeksforgeeks.org/print-numbers-such-that-no-two-consecutive-numbers-are-co-prime-and-every-three-consecutive-numbers-are-co-prime/)

给定一个整数 `N`，任务是打印 `N` 个整数 `≤ 10^9`，使得这些整数中没有两个连续的是同素的，并且每 3 个连续的是同素的。
...
*   我们可以将连续的素数相乘，最后一个数只需乘以 `gcd(last，last-1) * 2`。我们这样做是为了使 `(n–1)第号`、`第n号` 和 `第1号` 也能遵循问题陈述中提到的属性。
...
**另一种方法:** 使用厄拉多塞的[筛列出所有 600 万以内的质数](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)。我们知道基本条件，即 `N = 3` 形式 `{6，10，15}`。
因此，我们使用这三个值来生成序列的进一步项。
像 `{2，3，5}` 一样，这些素数不能用来生成序列，因为它们已经在 `{6，10，15}` 中使用了。我们也不能使用 `{7，11}`，我们稍后会看到。
现在我们有了一个质数列表 `{13，17，19，23，29，…}`。我们取第一个素数乘以 6，第二个乘以 15，第三个乘以 10，第四个乘以 6，以此类推…
