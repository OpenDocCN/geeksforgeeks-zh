...
> 原文：[https://www.geeksforgeeks.org/check-if-the-sum-of-primes-is-divisible-by-any-prime-from-the-array/](https://www.geeksforgeeks.org/check-if-the-sum-of-primes-is-divisible-by-any-prime-from-the-array/)

给定一个数组 `arr[]`，任务是检查数组中的素数之和是否能被数组中的任意素数整除。如果是则打印 `YES`，否则打印 `NO`。
...
*   再次遍历数组，检查 `sum % arr[i] = 0`，其中 `arr[i]` 是质数。如果是，则打印 `YES`。否则最后打印 `NO`。
