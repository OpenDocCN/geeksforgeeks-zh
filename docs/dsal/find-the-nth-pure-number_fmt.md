...
我们将假设 2 个数字组成一个单个块。每个区块有 `2^区块` 个纯数字。对于 1 块的纯数字，有 `2^1` 个纯数字；对于有 2 个区块的编号，有 `2^2` 个编号，以此类推。

*   从 4 开始的纯数字，从位置 `2^区块 - 1` 开始，例如，4444 位于 (`2^2 - 1 = 3`)，这意味着它位于系列中的第三个位置。
*   以 5 开头的纯数字从位置 `2^区块 + 2^(区块-1) - 1` 开始，例如，5555 位于 (`2^2 + 2^1 - 1 = 5`)，这意味着它位于系列中的第五个位置。
