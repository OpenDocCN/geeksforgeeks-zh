...
> 原文：[https://www.geeksforgeeks.org/reorder-characters-of-a-string-to-valid-english-representations-of-digits/](https://www.geeksforgeeks.org/reorder-characters-of-a-string-to-valid-english-representations-of-digits/)

给定长度为 `N` 的字符串 `S`，该字符串由包含数字的重新排序的英文表示的小写字符【0–9】组成，任务是以升序打印这些数字。

**示例：**

> **输入：** `S = "fviefuro"`
> **输出：** `45`
> **说明：** 给定的字符串可以重新洗牌为“four five”，所以字符串所代表的数字是 4 和 5。
>
> **输入：** `S = "owoztnoer"`
> **输出：** `012`
> **说明：** 给定的字符串可以重新洗牌得到“zero one two”，所以字符串所代表的数字是 0、1、2。

**天真方法：** 最简单的方法是生成给定字符串的所有排列，对于每个排列，检查是否有可能找到字符串表示的有效数字。如果发现为真，则按升序打印这组数字。
...
> 在“six”中，字符“x”是独一无二的。
> 在“eight”中，字符“g”是唯一的。
> 在“three”中，“h”字是独一无二的，因为已经考虑到“eight”字有“h”字。
> 在“one”中，字符“o”是唯一的，因为已经考虑了具有字符“o”的单词。
> 在“five”中，字符“f”是唯一的，因为已经考虑了具有字符“f”的“four”字。
> 在“seven”中，字符“v”是独一无二的。
> 在“nine”中，字符“i”是唯一的，因为已经考虑了具有字符“i”的单词。

按照以下步骤解决问题：

*   初始化一个空的字符串 `ans` 来存储需要的结果。
*   将字符串的每个字符的频率存储在 `M` 中。
*   创建一个唯一字符到其对应字符串的映射。
*   遍历地图，执行以下步骤：
    *   将数字对应的唯一字符存储在变量 `x` 中。
