...
> 原文: [https://www.geeksforgeeks.org/count-non-equidistant-triplets-of-distinct-array-elements-having-indices-in-increasing-order/](https://www.geeksforgeeks.org/count-non-equidistant-triplets-of-distinct-array-elements-having-indices-in-increasing-order/)

给定一个大小为 `N` 的数组 `arr[]`，该数组仅由 `0`、`1` 和 `2` 组成，任务是找到包含不同数组元素的索引 `(i, j, k)` 的三元组的计数，使得 `i < j < k` 和数组元素不等距，即 `(j - i) != (k - j)`。
...
思想是将数组元素 `0`、`1` 和 `2` 的索引存储在三个独立的数组中，然后找到满足给定条件的计数三元组。按照以下步骤解决问题:

1.  初始化两个数组，比如 `zero_i[]` 和 `one_i[]`，分别存储给定数组中 `0` 和 `1` 的索引。
2.  初始化一个地图，比如说 `mp`，来存储给定数组中 `2` 的索引。
3.  通过将 `zero_i[]`、`one_i[]` 和 `mp` 的大小相乘，找出所有可能的三胞胎的总数。
4.  现在，减去所有违反给定条件的三胞胎。
5.  为了找到这样的三元组，遍历数组 `zero_i[]` 和 `one_i[]` 并尝试在地图中找到违反条件的第三个索引。
6.  要找到违反条件的第三个指标，会出现以下三种情况:
