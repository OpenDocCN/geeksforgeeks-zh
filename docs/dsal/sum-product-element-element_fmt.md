...
所以对于每个 `i`，`0≤i≤n–2`，我们必须计算 `arr[i] * (arr[j] + arr[j+1] + …..+arr[n–2]+arr[n–1])`，其中 `j = i + 1`，求这些的和。我们可以降低寻找 `(arr[j] + arr[j+1] + …..+arr[n–2]+arr[n–1])` 的复杂度，方法是预先计算数组的后缀和。

让 `sum[]` 存储给定数组的后缀和，即 `sum[i]` 有 `arr[i] + arr[i+1] + …..+arr[n–2]+arr[n–1]`。
所以，在预计算 `sum[]` 之后，我们需要求 `arr[i]*sum[i + 1]` 的和，其中 `0≤i≤n–2`。
