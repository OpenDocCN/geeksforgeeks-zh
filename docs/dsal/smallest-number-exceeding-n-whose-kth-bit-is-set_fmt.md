...
> 原文: [https://www.geeksforgeeks.org/smallest-number-exceeding-n-whose-kth-bit-is-set/](https://www.geeksforgeeks.org/smallest-number-exceeding-n-whose-kth-bit-is-set/)
...
> **输出:** `20`
> **解释:**
> `(20)₁₀`的二进制表示为`(10100)₂`。从左数的 2<sup>nd</sup>位（*0 基分度*）设置在`(20)₁₀`中。因此，`20` 是设置了 2<sup>和</sup>位的大于 `15` 的最小数字。
>
> **输入:** `N = 16`，`K = 3`
> **输出:** `24`
...
1.  检查是否设置了 `N` 的 `K<sup>第</sup>` 位。如果发现为真，则找到最低的未置位位并置位，将其右侧的所有位更改为 `0`。
