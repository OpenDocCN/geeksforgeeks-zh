...
> 原文: [https://www.geeksforgeeks.org/print-n-terms-of-withoff-sequence/](https://www.geeksforgeeks.org/print-n-terms-of-withoff-sequence/)
...
如果 `A_{m,n}` 表示第 `m` 行和第 `n` 列中的元素，则

*   `A_{m,1} = floor(mφ)φ`
*   `A_{m,2} = floor(mφ)φ^2`
*   `A_{m,n} = A_{m,n-2} + A_{m,n-1}` 代表 `n > 2`
*   `φ = (1 + √5) / 2`
...
*   `T(n, -1) = n-1`，如果 `k = -1`
*   `T(n, 0) = floor(n*φ)`，如果 `k = 0`
*   `T(n, k) = T(n, k-1) + T(n, k-2)`，如果 `k > 0`
*   `φ = (1 + √5) / 2`

所以我们可以递归地找到 `T(n, k)` 的值，对于 `k = 0` 和对于 `k = -1` 有两种基本情况。我们将这些值存储在**图**中，并在需要时使用它来减少计算。在我们得到数组后，我们必须以反对角线的方式遍历它，所以我们设置 `i=0` 和 `j=0` 并减少 `j` 并增加 `i`。当 `j < 0` 时，我们初始化 `j = i` 和 `i = 0`。
...
[https://oeis.org/A035513](https://oeis.org/A035513)