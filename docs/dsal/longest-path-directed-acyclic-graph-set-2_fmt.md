...
我们已经讨论了如何在集合 1 中找到有向无环图（DAG）中的[最长路径](https://www.geeksforgeeks.org/find-longest-path-directed-acyclic-graph/)。在本文中，我们将讨论另一种有趣的解决方案，以使用算法来查找 [DAG](https://www.geeksforgeeks.org/shortest-path-for-directed-acyclic-graphs/) 中的最短路径。

## 算法思想

想法是**否定路径的权重并在图形中找到最短路径**。加权图 `G` 中两个给定顶点 `s` 和 `t` 之间的最长路径与图 `G'` 中从 `G` 得出的最短路径（通过将每个权重更改为负值）相同。因此，如果可以在 `G'` 中找到最短路径，那么也可以在 `G` 中找到最长路径。

## 分步过程

以下是寻找最长路径的分步过程：

1.  我们将给定图的每条边的权重更改为负值。
2.  将到所有顶点的距离初始化为无穷大，到源的距离初始化为 `0`。
3.  找到该图的拓扑排序，该图表示该图的线性排序。当我们按拓扑顺序考虑顶点 `u` 时，可以确保我们考虑了它的每个传入边。也就是说，我们已经找到了到达该顶点的最短路径，并且可以使用该信息来更新其所有相邻顶点的更短路径。
4.  一旦有了拓扑顺序，我们就会按照拓扑顺序逐一处理所有顶点。对于每个要处理的顶点，我们使用当前顶点到源顶点及其边权重的最短距离来更新其相邻顶点的距离。即：
...
5.  一旦找到了源顶点的所有最短路径，最长路径就是对最短路径的求反。
...
本文由 **Aditya Goel** 提供。如果您喜欢 GeeksforGeeks 并希望做出贡献，则还可以使用 [contribute.geeksforgeeks.org](http://www.contribute.geeksforgeeks.org) 撰写文章，或将您的文章邮寄至 contribute@geeksforgeeks.org。查看您的文章出现在 GeeksforGeeks 主页上，并帮助其他 Geeks。
