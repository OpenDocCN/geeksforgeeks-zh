...
> 原文: [https://www.geeksforgeeks.org/nth-positive-number-whose-absolute-difference-of-adjacent-digits-is-at-most-1/](https://www.geeksforgeeks.org/nth-positive-number-whose-absolute-difference-of-adjacent-digits-is-at-most-1/)

给定一个数字 `N`，任务是找到第 `N` 个数字，其每对相邻数字之间的绝对差值为 1。
...
*   现在执行以下操作 `N` 次。
    ...
    *   如果 (`arr[i] % 10 != 0`)，然后将 `10 * arr[i] + (arr[i] % 10) - 1` 入队。
    *   入队 `10 * arr[i] + (arr[i] % 10)`。
    *   如果 (`arr[i] % 10 != 9`)，然后将 `10 * arr[i] + (arr[i] % 10) + 1` 入队。
*   返回 `arr[N]` 作为答案。
