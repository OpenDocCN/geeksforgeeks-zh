...
> 原文: [https://www.geeksforgeeks.org/number-of-perfect-cubes-between-two-given-numbers/](https://www.geeksforgeeks.org/number-of-perfect-cubes-between-two-given-numbers/)
...
一个比较天真的做法是检查 `a` 和 `b`（包括 `a` 和 `b`）之间的所有数字，每当我们遇到一个完美的立方体时，就把计数增加一。
...
我们可以简单的取 `a` 的立方根和 `b` 的立方根，向上取 `a` 的立方根和向下取 `b` 的立方根，利用以下公式统计它们之间的完美立方体数量：
