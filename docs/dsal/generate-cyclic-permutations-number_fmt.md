...
> 原文: [https://www.geeksforgeeks.org/generate-cyclic-permutations-number/](https://www.geeksforgeeks.org/generate-cyclic-permutations-number/)

给定一个数 `N`，我们的任务是生成该数所有可能的循环排列。
循环置换将集合的所有元素移动固定的偏移量。对于具有元素 `a_0`、`a_1`、…、`a_n` 的集合，向左一个位置的循环排列将产生 `a_1`、…、`a_n`、`a_0`，向右一个位置的循环排列将产生 `a_n`、`a_0`、`a_1`、…。
