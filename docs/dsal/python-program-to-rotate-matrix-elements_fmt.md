...
> 原文：[https://www.geeksforgeeks.org/python-program-to-rotate-matrix-elements/](https://www.geeksforgeeks.org/python-program-to-rotate-matrix-elements/)
...
旋转矩阵的核心在于逐层、逐边地移动元素。我们定义 `top`、`bottom`、`left`、`right` 四个边界，然后在一个 `while` 循环中依次处理上边、右边、下边、左边的元素移动，每完成一圈，边界向内收缩，直到 `left >= right` 或 `top >= bottom`。
