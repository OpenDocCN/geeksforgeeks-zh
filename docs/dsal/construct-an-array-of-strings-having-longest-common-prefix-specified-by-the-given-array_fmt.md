...
> 原文: [https://www.geeksforgeeks.org/construct-an-array-of-strings-having-longest-common-prefix-specified-by-the-given-array/](https://www.geeksforgeeks.org/construct-an-array-of-strings-having-longest-common-prefix-specified-by-the-given-array/)

给定一个大小为 `N` 的整数数组 `arr[]`，任务是构造一个由长度为 `N` 的 `N+1` 个字符串组成的数组，使得 `arr[i]` 等于 `i<sup>th</sup>` 字符串和 `(i+1)<sup>th</sup>` 字符串的[最长公共前缀](https://www.geeksforgeeks.org/longest-common-prefix-using-binary-search/)。
...
*   其思想是观察如果 `i<sup>th</sup>` 字符串已知，那么 `(i-1)<sup>th</sup>` 字符串可以由 `i<sup>th</sup>` 字符串通过将 `N–arr[i-1]` 个字符从 `i<sup>th</sup>` 字符串更改而成。
*   从右向左开始构造字符串，生成 `N + 1` 个字符串。

> **图解:**
> `N = 3`，`arr[] = {2, 0, 3}`
> 让第 `(N + 1)<sup>th</sup>` 个字符串为 `"aaa"`
