...
### 方法
一种简单有效的方法是为数据生成霍夫曼树，并使用类似于 java 中 `TreeMap` 的数据结构来存储符号和位长，以便信息始终保持排序。然后，可以使用递增和按位左移操作来获得规范代码。
...
class Canonical_Huffman {
    ...
    static void testCanonicalHC(int n, char chararr[], int freq[]) {
        ...
        PriorityQueue<Node> q = new PriorityQueue<Node>(n, new Pq_compare());
