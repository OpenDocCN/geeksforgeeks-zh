...
> 原文：[https://www.geeksforgeeks.org/the-ubiquitous-binary-search-set-1/](https://www.geeksforgeeks.org/the-ubiquitous-binary-search-set-1/)
...
给定 `N` 个不同元素的排序数组。使用最少的比较次数在数组中查找一个键。(您认为二分搜索法是在排序数组中搜索关键字的最佳选择吗？)

没有太多理论，这里是典型的二分搜索法算法。

```
// Returns location of key, or -1 if not found
int BinarySearch(int A[], int l, int r, key)
{
    int m;
...
给定一个 `N` 个不同整数的数组，求输入 `key` 的 `floor` 值。比方说，`A = {-1，2，3，5，6，8，9，10}` 和 `key = 7`，我们应该返回 `6` 作为结果。
...
int Floor(int A[], int l, int r, key)
{
    int m;
...
给定一个可能有重复元素的排序数组。在 `log N` 时间中查找输入 `key` 的出现次数。
...
// Invariant: A[l] <= key and A[r] > key
int GetRightPosition(int A[], int l, int r, key)
{
    int m;
...
int GetLeftPosition(int A[], int l, int r, key)
{
    int m;
...
int CountOccurances(int A[], int size, key)
{
...
给定不同元素的排序数组，数组在未知位置旋转。查找数组中的最小元素。
...
1.  名为 `signum(x, y)` 的函数定义为：
    ...
    您是否遇到过任何指令集，其中的比较行为类似于 `signum` 函数？它能使二分搜索法的第一次实施达到最佳吗？
