...
给定两个由`正值节点`组成的 `N` 和 `M` 大小的`链表`，它们有一个公共的交点，任务是`找到这两个链表的交点，它们在这里合并`。
...
**方法:**思路是遍历第一个链表，将每个节点的值乘以-1，使其为负。然后，`遍历第二个链表`，打印第一个节点的负值。按照以下步骤解决问题:

*   `遍历第一个链表` `L1`，将每个节点的值乘以 `-1`。
*   现在，遍历第二个链表 `L2`，如果存在任何具有负值的节点，则打印该节点值的绝对值，作为链表和`脱离循环`的结果交集。
