...
> 原文: [https://www.geeksforgeeks.org/count-numbers-from-range-whose-prime-factors-are-only-2-and-3-using-arrays-set-2/](https://www.geeksforgeeks.org/count-numbers-from-range-whose-prime-factors-are-only-2-and-3-using-arrays-set-2/)
...
*   将 2 的所有小于等于 `R` 的幂存储在一个数组 `power2[]` 中。
*   同样，将所有小于或等于 `R` 的 3 的幂存储在另一个数组 `power3[]` 中。
*   初始化第三个数组 `power23[]`，并存储 `power2[]` 的每个元素与小于或等于 `R` 的 `power3[]` 的每个元素的成对乘积。
*   现在对于任何范围 `[L, R]`，我们将简单地迭代数组 `power23[]` 并计算范围 `[L, R]` 中的数字。
