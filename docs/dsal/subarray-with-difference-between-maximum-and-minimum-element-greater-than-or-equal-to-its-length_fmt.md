...
> 原文: [https://www.geeksforgeeks.org/subarray-with-difference-between-maximum-and-minimum-element-greater-than-or-equal-to-its-length/](https://www.geeksforgeeks.org/subarray-with-difference-between-maximum-and-minimum-element-greater-than-or-equal-to-its-length/)
...
找到至少有两个元素的所有可能的子阵列，然后检查满足给定条件的每个子阵列，即 `max(子阵列) - min(子阵列) >= len(子阵列)`。

## 高效的方法

找到长度为 2 的子阵列，其中仅有的两个元素之间的绝对差大于或等于 2。这将涵盖几乎所有的情况，因为只有三种情况下没有这样的子阵列:
