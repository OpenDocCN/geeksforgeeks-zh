...
> 原文：[`https://www.geeksforgeeks.org/xor-of-all-possible-pairwise-sum-from-two-given-arrays/`](https://www.geeksforgeeks.org/xor-of-all-possible-pairwise-sum-from-two-given-arrays/)
...
**天真方法：** 解决问题最简单的方法是[从两个给定的数组](https://www.geeksforgeeks.org/find-all-pairs-possible-from-the-given-array/)中生成所有可能的对，并计算它们各自的和，然后用对的和更新异或。最后，打印得到的异或。
...
*   仅考虑第 `K` 位，任务是统计对 `(i, j)` 的数量，使得 `(A_i + B_j)` 的第 `K` 位被设置为 `1`。
*   如果这个数字是奇数，在答案中加上 `X = 2^k`。我们只对模 `2X` 中的 `(a_i, b_j)` 的值感兴趣。
*   因此，将 `a_i` 替换为 `a_i % (2X)` 并将 `b_j` 替换为 `b_j % (2X)`，并假设 `a_i` 和 `b_j < 2X`。
*   设置 `(a_i + b_j)` 的 `k` 位时有两种情况：
    *   `X <= a_i + b_j < 2X`
    *   `3X <= a_i + b_j < 4X`
*   因此，按照递增的顺序对 `b[]` 进行排序。对于一个固定的 `i`，满足 `X <= (a_i + b_j) < 2X` 的 `j` 的集合形成一个区间。
