# 检查给定的字符串是否可以通过仅删除单一类型的字符而成为回文

> 原文: [https://www.geeksforgeeks.org/check-if-given-string-can-be-made-palindrome-by-removing-only-single-type-of-character/](https://www.geeksforgeeks.org/check-if-given-string-can-be-made-palindrome-by-removing-only-single-type-of-character/)
...
*   使用[双指针](https://www.geeksforgeeks.org/two-pointers-technique/)技术，检查不匹配，将 `l` 放置在字符串的`起点`处，将 `r` 放置在字符串的`终点`处
*   如果 `S[l] == S[r]`，递增 `l`，递减 `r`。
*   如果 `S[l] != S[r]`，检查 `S[l] == char`，则 `l++`，否则如果 `S[r] == char`，则 `r--`
