...
> 原文: [https://www.geeksforgeeks.org/find-the-number-of-cells-in-the-table-contains-x/](https://www.geeksforgeeks.org/find-the-number-of-cells-in-the-table-contains-x/)

给定两个整数 `N` 和 `X`。`N` 代表表格的行数和列数。表中第 `i` 行第 `j` 列的元素为 `i*j`。任务是在包含 `X` 的表格中找到单元格的数量。
...
很容易看出数字 `x` 在一行中只能出现一次。如果 `x` 包含在第 `i` 行中，那么列号将是 `x/i`。`x` 包含在第 `i` 行中，如果 `x` 可被 `i` 整除。让我们检查一下 `x` 除以 `i` 和 `x/i <= n`。如果满足这些条件，请更新答案。
...
接下来，循环遍历所有数字 `i` 从 1 到 `x` 的平方根，如果 `i` 是 `x` 的因子且 `x/i <= n`，那么由于关联属性: `i*(x/i)` 和 `(x/i)*i` 在 `n` 平方表上至少还有两个额外的位置 `x`。
