...
> 原文: [https://www.geeksforgeeks.org/check-if-a-string-contains-only-alphabets-in-java-using-lambda-expression/](https://www.geeksforgeeks.org/check-if-a-string-contains-only-alphabets-in-java-using-lambda-expression/)
...
思路是使用 `Character` 类的 `isLetter()` 方法。

## 算法

1.  获取字符串。
2.  匹配字符串：
    *   检查字符串是否为 `null`。如果为 `null`，返回 `false`。
    *   检查字符串是否为空。如果为空，则返回 `false`。
    *   如果字符串既不为 `null` 也不为空，则使用 Lambda 表达式 `Character::isLetter()` 进行检查。
3.  如果匹配，返回 `true`。
...
**[使用 Regex 检查字符串是否只包含 Java 中的字母](https://www.geeksforgeeks.org/check-if-a-string-contains-only-alphabets-in-java-using-regex/)**