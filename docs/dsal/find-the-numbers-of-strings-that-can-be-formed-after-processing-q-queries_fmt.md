...
> 原文：[https://www.geeksforgeeks.org/find-the-numbers-of-strings-that-can-be-formed-after-processing-q-queries/](https://www.geeksforgeeks.org/find-the-numbers-of-strings-that-can-be-formed-after-processing-q-queries/)

给定一个数字 `N` (`1 <= N <= 2000`)，任务是通过处理给定的 `q` (`1 <= q <= 200000`) 查询，找到使用从“a”到“z”的字符后可以获得的大小为 `N` 的字符串个数。
...
一个有效的解决方案是使用[联合查找](https://www.geeksforgeeks.org/union-find-algorithm-set-2-union-by-rank/)算法。
...
*   处理完所有查询后，设不相交集组件的数量为 `x`，那么答案是 `26^x`。
