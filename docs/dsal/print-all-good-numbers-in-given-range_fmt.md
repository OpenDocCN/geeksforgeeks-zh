...
> 原文：[https://www.geeksforgeeks.org/print-all-good-numbers-in-given-range/](https://www.geeksforgeeks.org/print-all-good-numbers-in-given-range/)

给定一个数字`d`和一个范围`[L, R]`，其中 `L < R`，请打印给定范围内所有不包含数字`d`的“好号”。如果一个数字的每一位都大于其右侧所有数字之和，则该数字是“好号”。例如，`9620` 是一个好号，因为 `2 > 0`，`6 > 2+0` 且 `9 > 6+2+0`。
...
这个想法是遍历给定范围内的所有数字。对于每个数字，遍历其所有数字。遍历时，记录到目前为止的数字和。在任何时候，如果当前数字大于或等于该和，则返回 `false`。此外，如果当前数字变为`d`，则返回 `false`。
