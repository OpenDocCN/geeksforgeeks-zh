...
> 原文：[https://www.geeksforgeeks.org/find-the-n-th-binary-string-in-sorted-order/](https://www.geeksforgeeks.org/find-the-n-th-binary-string-in-sorted-order/)

给定一个正整数 `n`，任务是在由两个符号 `a` 和 `b` 构成的所有可能字符串的无限列表中，找到按字典顺序排序后的第 `n` 个字符串。

> a, b, aa, ab, ba, bb, aaa, aab, aba, abb, baa, bab, bbb, ...
...
一种**简单的方法**是生成直到 `n` 的所有字符串，然后确定第 `n` 个字符串。然而，该方法不适用于大数值的 `n`。

一种**有效的方法**是基于这样的事实：使用 2 个符号可以生成的长度为 `k` 的字符串的数量是 `2^k`。基于此，我们可以根据列表中字符串长度的实际索引 (`n`) 来计算相对索引。当列表被排序时，在第 `n` 个索引处的字符串可以很容易地使用相对索引的二进制形式来确定。以下公式用于计算：

> 相对索引 = `n + 1 - 2^floor(log(n + 1))`

**考虑以下示例：**

> 令 `n = 11`，则 `floor(log(n + 1)) = 3`。
> 这表明索引 `n` 由长度为 3 的字符串组成。长度为 3 的字符串从 `(2^3 - 1) = 第 7 个索引`开始，第 7 个索引包含字符串 `"aaa"`。
> 因此，`相对索引 = 11 + 1 - 2^3 = 4`。
> 这是相对于索引 `7` 的偏移量。现在，索引 `n = 11` 处的字符串可以简单地从相对索引 `4` 的二进制解释中获得。
