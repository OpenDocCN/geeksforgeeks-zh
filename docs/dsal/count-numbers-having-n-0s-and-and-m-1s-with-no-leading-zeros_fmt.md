...
> 原文: [https://www.geeksforgeeks.org/count-numbers-having-n-0s-and-m-1s-with-no-leading-zeros/](https://www.geeksforgeeks.org/count-numbers-having-n-0s-and-m-1s-with-no-leading-zeros/)

给定两个整数 `N` 和 `M`，任务是找出不含前导零的 `N` 个 0 和 `M` 个 1 以及 `N + M` 总位数的不同数字。
...
**方法:** 通过求 `N` 个相似项和 `M–1` 个相似项的全排列，可以很容易地解决问题。由于不允许前导零，`1` 始终固定在数字的开头。剩余的 `M–1` 个 `1` 和 `N` 个 `0` 以不同的排列方式排列。现在 `(A + B)` 个对象的排列数，其中 `A` 个同类型对象和 `B` 个其他类型对象由 `(A + B)! / (A! * B!)` 给出。因此，相异数的个数由 `(N + M - 1)! / (N! * (M - 1)!)` 给出。
