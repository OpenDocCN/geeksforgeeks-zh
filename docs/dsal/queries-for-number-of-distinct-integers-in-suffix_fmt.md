...
> 原文: [https://www.geeksforgeeks.org/queries-for-number-of-distinct-integers-in-suffix/](https://www.geeksforgeeks.org/queries-for-number-of-distinct-integers-in-suffix/)

给定一个由 `N` 元素和 `Q` 查询组成的数组，其中每个查询包含一个整数 `K`。对于每个查询，任务是在从 `K` 元素到 `N` 元素的后缀中找到不同整数的数量。
...
辅助阵列 `aux[]` 从阵列右侧保持。数组 `mark[]` 存储一个元素是否已经出现在已经遍历的后缀中。如果元素没有出现，则更新 `aux[i] = aux[i+1] + 1`。否则 `aux[i] = aux[i+1]`。
每个查询的答案都是 `aux[q[i]]`。
以下是上述办法的实施情况。
