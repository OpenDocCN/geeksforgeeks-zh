# 通过旋转数字检查是否所有数组元素都可以转换成音位数字

> 原文: [https://www.geeksforgeeks.org/check-if-all-array-elements-can-be-converted-to-pronic-numbers-by-rotating-digits/](https://www.geeksforgeeks.org/check-if-all-array-elements-can-be-converted-to-pronic-numbers-by-rotating-digits/)

给定一个大小为 `N` 的数组 `arr[]`，任务是通过旋转数组元素的数字任意多次来检查是否有可能将所有数组元素转换为音位数字。
...
> `arr[1]` → 向右旋转一次将 `arr[1]` 修改为 `240` (= 15 × 16)。
...
*   遍历数组并检查每个数组元素，是否可以将其转换为音位数字。
*   对于每个数组元素，应用所有可能的旋转，并在每次旋转后检查生成的数字是否为音位数。
