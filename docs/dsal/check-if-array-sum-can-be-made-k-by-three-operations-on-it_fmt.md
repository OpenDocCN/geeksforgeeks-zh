...
> 原文: [https://www.geeksforgeeks.org/check-if-array-sum-can-be-made-k-by-three-operations-on-it/](https://www.geeksforgeeks.org/check-if-array-sum-can-be-made-k-by-three-operations-on-it/)

给定一个由 `N` 个整数和一个正整数 `K` 组成的数组。此数组只能执行三个操作：
1. 用整数的负值替换一个整数，
2. 将元素的索引号（基于 1 的索引）添加到元素本身，以及
3. 从元素本身减去元素的索引号。

任务是检查给定的数组是否可以被转换，使用上面三个允许的操作中的任何一个，在每个元素上只执行一次，使得数组的总和变成 `k`。
