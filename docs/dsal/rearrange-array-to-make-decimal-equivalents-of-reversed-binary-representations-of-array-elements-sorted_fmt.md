...
> 原文: [https://www.geeksforgeeks.org/rearrange-array-to-make-decimal-equivalents-of-reversed-binary-representations-of-array-elements-sorted/](https://www.geeksforgeeks.org/rearrange-array-to-make-decimal-equivalents-of-reversed-binary-representations-of-array-elements-sorted/)

给定一个由正整数 `N` 组成的数组 `arr[]`，任务是重新排列数组，以便对所有数组元素的反向二进制表示进行排序。
...
按照以下步骤解决问题:
*   初始化一个数组，比如说 `newArr[]`，插入所有数组元素作为数字的反向二进制表示。
*   按递增顺序排列数组 `newArr`。
*   遍历数组 `newArr` 并将所有元素复制到数组 `arr[]`。
*   现在，将所有数组元素更新为数字的反向二进制表示。
*   完成以上步骤后，打印数组 `arr[]` 的元素作为输出。
