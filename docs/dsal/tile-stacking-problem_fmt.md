...
此外，我们可以使用这样一个事实，即对于固定的 `j`，我们使用 `i` 的先前 `k` 值的连续值。因此，我们可以为每个状态维护前缀和数组 `presum[][]`。现在我们已经去掉了每个状态的 `k` 因子。
...
以下JavaScript程序用于计算构建给定高度的稳定塔的方法数。
