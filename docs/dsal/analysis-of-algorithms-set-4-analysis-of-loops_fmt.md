...
> 原文:[https://www.geeksforgeeks.org/analysis-of-algorithms-set-4-analysis-of-loops/](https://www.geeksforgeeks.org/analysis-of-algorithms-set-4-analysis-of-loops/)
...
## 1) `O(1)`:
如果一个函数(或一组语句)不包含循环、递归和对任何其他非常数时间函数的调用，则它的时间复杂度被认为是 `O(1)`。
...
## 2) `O(n)`:
如果循环变量递增/递减一个常量，则循环的时间复杂度被认为是 `O(n)`。例如，以下函数的时间复杂度为 `O(n)`。
...
## 3) `O(n^c)`:
嵌套循环的时间复杂度等于最里面的语句被执行的次数。例如，以下示例循环的时间复杂度为 `O(n^2)`。
...
例如[选择排序](https://www.geeksforgeeks.org/selection-sort/)和[插入排序](https://www.geeksforgeeks.org/insertion-sort/)的时间复杂度为 `O(n^2)`。

## 4) `O(Logn)`
如果循环变量除以/乘以一个常量，则循环的时间复杂度被认为是 `O(Logn)`。
...
例如，[二分搜索法(参考迭代实现)](http://geeksquiz.com/binary-search/)具有 `O(Logn)`时间复杂度。让我们从数学上看看它是如何为 `O(log n)`的。我们在第一个循环中得到的级数是 `1，c，c^2，c^3，… c^k`。如果我们把 `k` 等于 `Log_c n`，我们得到 `c^(Log_c n)`也就是 `n`。

## 5) `O(logn)`
如果循环变量按指数规律减少/增加一个常量，那么循环的时间复杂度就被认为是 `O(logn)`。
