...
给定`N`个对，任务是找到`K`个第`个`最小对，且对的数量少于给定对`(x，y)`。
...
上述问题可以通过从数组中找出所有对，然后分别打印第 k 个最小对和小于`(x，y)`的对的个数来解决。

## 基于策略的数据结构方法

以下是将基于策略的数据结构显示为`MAP`的代码。它可以添加/移除元素对，可以在`O(log N)`时间内找到小于`(x，y)`的元素对数量、第 k 个最小的元素对等。这张地图的特别之处在于，我们可以访问这一对元素在有序的 2D 数组中的索引。如果这一对没有出现在地图上，我们就得到这一对在地图上的位置。
