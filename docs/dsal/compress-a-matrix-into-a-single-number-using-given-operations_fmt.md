...
> 原文：[https://www.geeksforgeeks.org/compress-a-matrix-into-a-single-number-using-given-operations/](https://www.geeksforgeeks.org/compress-a-matrix-into-a-single-number-using-given-operations/)

给定维度为 `M * N` 的矩阵 `mat[][]`，任务是首先压缩它以获得一个数组，然后使用以下操作再次压缩它以获得单个整数：

*   当矩阵被压缩时，其值的二进制表示被压缩。
*   因此，考虑每个比特，如果一个比特的位置具有 `S` 设置比特和 `NS` 未设置比特，则如果 `S > NS` 则该比特被设置为该位置，否则不被设置。
*   每一列被压缩，把矩阵变成一个数组，然后这个数组被进一步压缩成一个数字。

> 例如，如果 `5`、`2`、`3` 和 `1` 被压缩，那么它们的二进制表示 `(101)₂`、`(010)₂`、`(011)₂` 和 `(001)₂` 被压缩，那么对于 `0ᵗʰ` 和...
