...
以下函数将给定的链表顺时针旋转 `k` 个位置，并返回更新后的头指针。
...
class GFG 
{
    ...
    static Node rightRotate(Node head, int k) 
    {
        ...
        // len is used to store length of the linked list, tmp will point 
        ...
        // If current is null then k is equal to the count of nodes in the list 
        // Don't change the list in this case 
        if (current == null) 
            return head;
