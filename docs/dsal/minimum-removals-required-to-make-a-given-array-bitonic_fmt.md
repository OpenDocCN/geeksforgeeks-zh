...
> 原文: [https://www.geeksforgeeks.org/minimum-removals-required-to-make-a-given-array-bitonic/](https://www.geeksforgeeks.org/minimum-removals-required-to-make-a-given-array-bitonic/)
...
> **输出:** 0
> **解释:**
> 给定的数组已经是双离子数组了。因此，所需的输出为 0。

## 方法

问题可以基于[最长递增子序列问题](https://www.geeksforgeeks.org/longest-increasing-subsequence-dp-3/)的概念来解决。按照以下步骤解决问题:

*   初始化一个变量，比如说 `left[]`，这样 `left[i]` 存储**最长递增子序列** ce 到 `i` 索引的长度。
*   初始化一个变量，比如说 `right[]`，这样 `right[i]` 存储**最长递减子序列**在 `[i，N]` 范围内的长度。
*   使用变量 `i` 遍历 `left[]` 和 `right[]` 数组，找到 `(left[i] + right[i] – 1)` 的最大值，并将其存储在一个变量中，比如 `maxLen`。
*   最后，打印 `N – maxLen` 的值。
