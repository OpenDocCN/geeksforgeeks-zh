...
一个**简单解**是从 `L` 遍历到 `R`，检查当前数是否为质数。如果是，将其添加到`sum`。最后，打印总和。

一个**高效的解决方案**是使用厄拉多塞的[筛来查找给定限制内的所有素数。然后，计算一个前缀和数组来存储和，直到每个值都在极限之前。一旦我们有了前缀数组，我们只需要返回`prefix[R] – prefix[L-1]`。

**注**：`prefix[i]` 将存储从 1 到`i`的所有素数之和。
