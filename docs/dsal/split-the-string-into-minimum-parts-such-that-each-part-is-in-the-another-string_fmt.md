# 将字符串分成最小的部分，使每个部分都在另一个字符串中

> 原文：[https://www.geeksforgeeks.org/split-the-string-into-minimum-parts-such-that-each-part-is-in-the-another-string/](https://www.geeksforgeeks.org/split-the-string-into-minimum-parts-such-that-each-part-is-in-the-another-string/)
...
> **输出：** -1
> **解释：**
> 无法将字符串 `A` 拆分为子字符串，使得每个字符串也存在于 `B` 中。

## 进场思路

*   构造一个 `B` 的每个子串的 [Trie](https://www.geeksforgeeks.org/trie-insert-and-search/)。
*   之后，我们将使用[动态规划](https://www.geeksforgeeks.org/dynamic-programming/)找到最小的部分数来断开字符串 `A`，使得每个部分都是 `B` 的子串。
...
dp[S1.length] is the minimum number of parts.
```
