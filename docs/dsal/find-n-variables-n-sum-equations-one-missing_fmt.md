...
> 原文: [https://www.geeksforgeeks.org/find-n-variables-n-sum-equations-one-missing/](https://www.geeksforgeeks.org/find-n-variables-n-sum-equations-one-missing/)
...
方程 `n1` => `X2 + X3 +…..+Xn = a1`
`Eqn2` => `X1 + X3 +…..+Xn = a2`
`Eqni` => `X1+X2+…+X_{i-1}+X_{i+1}..+Xn = ai`
`Eqnn` => `X1 + X2 +…..+ X_{n-1} = an`
