...
> 原文：[https://www.geeksforgeeks.org/find-nth-root-of-a-number-using-bisection-method/](https://www.geeksforgeeks.org/find-nth-root-of-a-number-using-bisection-method/)

给定两个正整数 `N` 和 `P`。任务是找到 `P` 的 `N` 次根。
...
有多种方法可以解决给定的问题。下面的算法基于数学概念[求根二等分法](https://www.geeksforgeeks.org/program-for-bisection-method/)。为了求出给定数 `P` 的 `N` 次幂根，我们将在 `x` 中形成一个方程作为 `x^N - P = 0`，目标是用二等分法求出这个方程的正根。

**等分法是如何工作的？**
取一个区间 `(a, b)` 这样就已经知道根是存在于那个区间的。之后，找到区间的**中间**，检查函数值及其在 `x = 中间` 的导数。
