...
> 原文: [https://www.geeksforgeeks.org/split-an-array-into-groups-of-3-such-that-x3-is-divisible-by-x2-and-x2-is-divisible-by-x1/](https://www.geeksforgeeks.org/split-an-array-into-groups-of-3-such-that-x3-is-divisible-by-x2-and-x2-is-divisible-by-x1/)

## 问题描述

给定一个包含 `N` 个元素的数组 `A`（`N` 可被 `3` 整除），任务是将数字拆分为 3 个一组，使得该组有 3 个元素 `X1`、`X2` 和 `X3`，并且该组应该满足以下条件：
*   `X1`、`X2` 和 `X3` 是成对不同的
*   `X3` 可以被 `X2` 整除
*   `X2` 可以被 `X1` 整除

如果无法将数组拆分成 `N/3` 个这样的组，打印 `-1`。
