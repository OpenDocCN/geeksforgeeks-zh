...
> 原文: [https://www.geeksforgeeks.org/print-first-n-terms-of-lower-wythoff-sequence/](https://www.geeksforgeeks.org/print-first-n-terms-of-lower-wythoff-sequence/)

给定一个整数 `N`，任务是打印[下 Wythoff 序列](https://en.wikipedia.org/wiki/Beatty_sequence#Examples)的第一个 `N` 项。
...
**方法:** 下 Wythoff 序列是这样一个序列，其第 `n` 项为 `a(n) = floor(n * φ)`，其中 `φ = (1 + sqrt(5)) / 2`。所以，我们运行一个循环，找到序列的第一个 `n` 项。
