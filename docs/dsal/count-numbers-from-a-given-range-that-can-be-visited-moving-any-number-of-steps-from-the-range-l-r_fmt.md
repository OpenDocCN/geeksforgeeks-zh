...
> 原文: [https://www.geeksforgeeks.org/count-numbers-from-a-given-range-that-can-be-visited-moving-any-number-of-steps-from-the-range-l-r/](https://www.geeksforgeeks.org/count-numbers-from-a-given-range-that-can-be-visited-moving-any-number-of-steps-from-the-range-l-r/)
...
*   构建一个大尺寸的数组来标记范围。
*   初始化一个变量，比如 `count = 0`，来计算可到达的点。
*   首先，使 `diff_arr[x] = 1` 和 `diff_arr[x+1] = -1` 将起点 `X` 标记为已访问。
*   从 `X` 迭代到 `Y`，对于每个 `i` 更新 `diff_arr[i] += diff_arr[i-1]`，如果 `diff_arr[i] >= 1`，则更新 `diff_arr[i+L] = 1` 和 `diff_arr[i + R + 1] = -1` 和 `count = count + 1`。
*   最后，返回 `count`。
