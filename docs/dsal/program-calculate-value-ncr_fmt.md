...
1.  一个[二项式系数](http://en.wikipedia.org/wiki/Binomial_coefficient) `C(n, k)` 可以定义为 `(1 + X)^n` 展开式中的 `X^k` 系数。
2.  二项式系数 `C(n, k)` 也给出了从 `n` 个对象中选择 `k` 个对象的方法，不考虑顺序；更正式地说，`n` 元素集的 `k` 元素子集（或 `k` 组合）的数量。

给定两个数字 `n` 和 `r`，求 `nCr` 的值。示例：
...
- [动态编程 | 集合 9 (二项式系数)](https://www.geeksforgeeks.org/dynamic-programming-set-9-binomial-coefficient/)
