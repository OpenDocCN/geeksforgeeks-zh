...
> 原文: [https://www.geeksforgeeks.org/kth-ancestor-of-all-nodes-in-an-n-ary-tree-using-dfs/](https://www.geeksforgeeks.org/kth-ancestor-of-all-nodes-in-an-n-ary-tree-using-dfs/)

给定一个 [N 元树](https://www.geeksforgeeks.org/generic-treesn-array-trees/)和一个整数 `K`，任务是以级别顺序的方式打印树的所有节点的第 `K` 个祖先。如果某个节点不存在 `K` 个祖先，则打印该节点的 `-1`。
...
方法是用 [DFS](https://www.geeksforgeeks.org/depth-first-traversal-for-a-graph/) 找到所有节点的祖先。以下是步骤:

1.  任何节点的第 `K` 个父节点都可以通过使用 `DFS` 找到，并将一个节点的所有父节点存储在一个临时向量中，比如 `temp[]`。
2.  每当在 `DFS` 中访问一个节点时，它就会被推入 `temp` 向量中。
3.  在 `DFS` 结束时，从 `temp` 向量中弹出当前访问的节点。
4.  对于当前访问的节点，向量包含该节点的所有祖先。
5.  向量末尾的第 `K` 节点是当前访问节点的第 `K` 祖先，因此将其存储在一个 `ancestor[]` 数组中。
