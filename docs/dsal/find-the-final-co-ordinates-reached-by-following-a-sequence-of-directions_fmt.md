...
> 原文：[https://www.geeksforgeeks.org/find-the-final-coordinates-reached-by-following-a-sequence-of-directions/](https://www.geeksforgeeks.org/find-the-final-coordinates-reached-by-following-a-sequence-of-directions/)
...
> **输入:** `SX = 2`，`SY = 3`，`D = "SEWW"`
> **输出:** `(1, 2)`
> **解释:** 从给定点开始，会定向到 `S(2, 2)` —> `E(3, 2)` —> `W(2, 2)` —> `W(1, 2)`。因此最终位置为 `(1, 2)`。
>
> **输入:** `SX = 2`，`SY = 2`，`D = "NSSE"`
> **输出:** `(3, 1)`
...
遍历字符串 `D` 并相应地增加或减少坐标 `SX` 和 `SY` 并打印它们的最终值。
