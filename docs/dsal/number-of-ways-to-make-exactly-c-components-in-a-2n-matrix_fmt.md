...
> 原文: [https://www.geeksforgeeks.org/number-of-ways-to-make-exactly-c-components-in-a-2n-matrix/](https://www.geeksforgeeks.org/number-of-ways-to-make-exactly-c-components-in-a-2n-matrix/)
...
**方法:** 解决这个问题的思路是使用[动态规划](https://www.geeksforgeeks.org/dynamic-programming/)。构建一个 `(N+1) * (C+1) * 2 * 2` 大小的 4D 动力定位矩阵，其中 `N` 是列数，`C` 是组件数，2*2 维是最后一列的排列。最后一列可以有四种不同的组合。他们是 00，01，10，11。
