...
> 原文: [https://www.geeksforgeeks.org/print-n-lines-of-4-numbers-such-that-every-pair-among-4-numbers-has-a-gcd-k/](https://www.geeksforgeeks.org/print-n-lines-of-4-numbers-such-that-every-pair-among-4-numbers-has-a-gcd-k/)
...
第一个观察是，如果我们能解出 `K=1` 的给定问题，那么我们可以通过简单地将答案与 `K` 相乘来解决带有 GCD `K` 的问题，我们知道任何三个连续的奇数在配对时总是有一个 GCD 1，所以每一行的三个数字都可以很容易地得到。因此，这些行看起来像:
...
偶数不能总是插入，因为在第三行插入 6 会使 `GCD(6，9)` 为 3。因此，可以插入的最佳数字是每行前两个奇数之间的数字。因此，模式如下所示:
