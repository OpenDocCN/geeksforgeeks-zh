...
> 原文: [https://www.geeksforgeeks.org/smallest-and-largest-n-digit-number-starting-and-ending-with-n/](https://www.geeksforgeeks.org/smallest-and-largest-n-digit-number-starting-and-ending-with-n/)

给定一个整数 `N`，任务是找到以数字 `N` 开始和结束的最小和最大的 `N` 位数字。
...
**进场:**
我们知道最大最小的 `N` 位号是 `9999…9`，其中 `9` 重复 `N` 次和 `1000…0`，其中 `0` 分别重复 `N-1` 次。
现在要得到最小和最大的 `N` 位数字以 `N` 开始和结束，我们需要将最小和最大的 `N` 位数字的第一个和最后一个数字替换为 `N`。
