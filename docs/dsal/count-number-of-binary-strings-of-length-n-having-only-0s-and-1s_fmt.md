...
> 原文：[https://www.geeksforgeeks.org/count-number-of-binary-strings-of-length-n-having-only-0s-and-1s/](https://www.geeksforgeeks.org/count-number-of-binary-strings-of-length-n-having-only-0s-and-1s/)

给定一个整数 `N`，任务是计算长度为 `n` 的二进制字符串的数量，这些字符串只有 0 和 1。注意：
由于计数可能非常大，所以返回模 `10^9+7` 的答案。
...
**方法：** 使用排列组合可以很容易地解决问题。在字符串的每个位置只能有两种可能性，即 0 或 1。因此，长度为 `n` 的字符串中 0 和 1 的置换总数由 `2 * 2 * 2 *……(n 次)` 给出，即 `2^N`。答案可能非常大，因此返回 `10^9+7` 的模。
