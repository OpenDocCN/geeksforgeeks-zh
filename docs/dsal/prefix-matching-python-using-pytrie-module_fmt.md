...
> 原文: [https://www.geeksforgeeks.org/prefix-matching-python-using-pytrie-module/](https://www.geeksforgeeks.org/prefix-matching-python-using-pytrie-module/)
...
*   **创造:** `trie = StringTrie()` 创建一个空的 Trie 数据结构。
*   **插入:** `trie[key] = value`，`key` 是我们要在 trie 中插入的数据，`value` 类似于一个桶，刚好在插入的 key 的最后一个节点后追加，这个桶包含插入的 key 的实际值。
*   **搜索:** `trie.values(prefix)`，返回包含给定前缀的所有键的列表。
*   **删除:** `del trie[key]`，从特里数据结构中删除指定键。

![](img/53291873013341dc4b350f449912630a.png)

**注意:** 要安装 `pytrie` 包，使用这个 `pip install pytrie --user` 命令从 linux 中的终端安装。
