...
> 原文: [https://www.geeksforgeeks.org/number-of-ways-to-reach-nth-floor-by-taking-at-most-k-leaps/](https://www.geeksforgeeks.org/number-of-ways-to-reach-nth-floor-by-taking-at-most-k-leaps/)
...
设 `combo[i]` 为到达 `i` 层的路数。因此，通过跳跃 `i-j` 从 `combo[j]` 到达 `combo[i]` 的方法数量将是 `combo[i] += combo[j]`。所以迭代所有可能的跳跃，并为每个可能的跳跃不断添加可能的组合到组合数组。最终答案将存储在 `combo[N]` 中。
