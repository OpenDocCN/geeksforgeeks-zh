...
> 原文：[https://www.geeksforgeeks.org/optimized-euler-totient-function-multiple-evaluations/](https://www.geeksforgeeks.org/optimized-euler-totient-function-multiple-evaluations/)

[`Euler Totient Function(ETF)`](https://www.geeksforgeeks.org/eulers-totient-function/) `φ(n)` 对于输入 `n` 是 `{1, 2, 3, …, n}` 中与 `n` 相对素数的数的计数，即 [`GCD`](https://www.geeksforgeeks.org/c-program-find-gcd-hcf-two-numbers/)（最大公约数）与 `n` 为 1 的数。
...
我们已经讨论了[计算欧拉全能函数](https://www.geeksforgeeks.org/eulers-totient-function/)的不同方法，这些方法适用于单个输入。在像 `10^5` 时报那样我们不得不多次调用欧拉全能函数的问题中，简单的解决方案将导致 `TLE`（超过时间限制）。想法是用厄拉多塞的[筛子](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)。

使用[埃拉托斯特尼的筛](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)找出所有达到最大极限的质数，比如 `10^5`。
