...
> 原文：[`https://www.geeksforgeeks.org/check-encoding-represents-unique-binary-string/`](https://www.geeksforgeeks.org/check-encoding-represents-unique-binary-string/)
...
一种 `天真的方法` 是取一个空字符串，遍历 `n` 个整数到 `编码[0]` 中给定的 1 个数，然后加 1 个零，然后 `编码[1]` 个 1，最后检查字符串长度是否等于 `k`，然后打印 `“Yes”` 或打印 `“No”`。
