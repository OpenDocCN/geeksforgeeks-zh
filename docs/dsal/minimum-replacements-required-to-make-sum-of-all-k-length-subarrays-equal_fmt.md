...
原文: [https://www.geeksforgeeks.org/minimum-replacements-required-to-make-sum-of-all-k-length-subarrays-equal/](https://www.geeksforgeeks.org/minimum-replacements-required-to-make-sum-of-all-k-length-subarrays-equal/)

给定一个由 `N` 个正整数和一个整数 `K` 组成的数组 `arr[]`，任务是通过用任意整数替换最小数量的数组元素，使所有 `K`-长度子数组的和相等。
...
> **输出:** `2`
> **解释:**
> 操作 1: 用 `4` 替换 `arr[3]`，将 `arr[]` 修改为 `{3, 4, 3, 4, 6}`。
...
**方法:** 这个想法是基于这样的观察：所有子阵将具有相等的和，当所有被距离 `K` 隔开的元素相等时。

因此，通过计算相隔一段距离 `K` 的元素出现频率，找出出现次数最多的数量，就可以解决这个问题。按照以下步骤解决问题:

*   初始化一个变量 `ans` 来存储需要的结果。
*   使用变量 `i` 在范围 `[0, K-1]` 中迭代
    *   创建一个映射 `freq` 来存储从 `i` 开始相隔一段距离 `K` 的元素的频率。
    *   遍历映射找到出现次数最多的元素。
    *   再次，遍历映射，如果元素不等于最大出现元素，则将其频率添加到 `ans` 中。
*   打印 `ans` 的值作为结果。
