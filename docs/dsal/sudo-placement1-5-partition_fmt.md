...
给定一组正数和负数。任务是找到一个分割点，使得左数组的元素都不在右数组中。如果有多个分区，则求左数组之和与右数组之和(`|sum左–sum右|`)相对于分区点的绝对差最小的分区。如果有多个点，打印从左数第一个分区点，即(左数组的最后一个索引和右数组的第一个索引)/2。考虑基于 1 的索引。分区上的左右数组必须至少有 1 个元素，最多有 `n-1` 个元素。如果没有分区，打印`-1`。
...
一种简单的方法是从每个索引开始向左和向右遍历，并检查分区在该索引处是否可能。如果分区是可能的，那么检查左数组的一个元素和右数组的元素的和之间的绝对差是否小于之前在分区获得的值。找到分割点后，贪婪地找到 `|sum左–sum右|`。
时间复杂度:`O(N^2)`

一种有效的解决方案是将每个出现元素的最后一个索引存储在哈希映射中。因为元素值很大，所以不能使用直接索引。创建一个`前缀[]` 和`后缀[]` 数组，分别存储前缀和后缀和。将变量`count`初始化为 0。迭代数组中的所有元素。一个常见的观察点是，当遍历时，如果当前元素(`A_i`)的最后一次不出现不是 `i` 本身，那么我们不能在 `i` 和元素的最后一次出现之间有一个划分。遍历存储元素最后一次出现的最大值时，因为分区在此之前无法完成。
一旦`count`是 `i` 本身，我们就可以有一个分区，现在如果有多个分区那么选择 `min |sum左–sum右|`。
注意:使用`map`代替`unordered_map`可能会导致 TLE。
以下是上述办法的实施情况。
...
class GFG
{
    ...
    static void partition(int []a, int n)
    {
        ...
        // stores the last index till which there can not be any partition
        long count = 0;
        ...
        // Check if partition is possible or not
        // donot check for the last element as partition is not possible
        for (int i = 0; i < n - 1; i++)
        {
            // takes an element and checks its last occurrence, stores the maximum of the last occurrence where partition can be done
...
function partition(a, n) {
    ...
    // donot check for the last element as partition is not possible
    for (let i = 0; i < n - 1; i++) {
        // takes an element and checks it last occurrence, stores the maximum of the last occurrence where partition can be done
        count = Math.max(count, mpp.get(a[i]));
