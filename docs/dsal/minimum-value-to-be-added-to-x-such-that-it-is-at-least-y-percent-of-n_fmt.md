...
> 原文：[https://www.geeksforgeeks.org/minimum-value-to-be-added-to-x-such-that-it-is-at-least-y-percent-of-n/](https://www.geeksforgeeks.org/minimum-value-to-be-added-to-x-such-that-it-is-at-least-y-percent-of-n/)
...
**方法：** 找到 `val = (N * Y) / 100`，这是 `N` 的 `Y` 百分比。现在为了使 `X` 等于 `val`，`val - X` 必须加到 `X` 上，前提是 `X < val`。
