...
> 原文：[https://www.geeksforgeeks.org/scheduling-priority-tasks-limited-time-minimizing-loss/](https://www.geeksforgeeks.org/scheduling-priority-tasks-limited-time-minimizing-loss/)
            ...
            int units[],  int prior[])
{
    ...
    for (int i = 1; i <= t; i++) {
        ...
        // are already in pq.
        while (ptr < n and v[ptr].first == i) {
            pq.push({ prior[v[ptr].second], units[v[ptr].second] });
            ptr++;
        }
...
本文由 **Parth Trehan** 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 `contribute.geeksforgeeks.org` 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
