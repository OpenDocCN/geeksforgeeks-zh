...
> 原文: [https://www.geeksforgeeks.org/primitive-root-of-a-prime-number-n-modulo-n/](https://www.geeksforgeeks.org/primitive-root-of-a-prime-number-n-modulo-n/)
...
一个简单的解决方法就是尝试从 `2` 到 `n-1` 的所有数字。对于每个数字 `r`，计算 `r^x(mod n)` 的值，其中 `x` 在 `[0, n-2]` 的范围内。如果所有这些值都不同，则返回 `r`，否则继续下一个 `r` 值。如果所有 `r` 值都尝试过，则返回 `-1`。
