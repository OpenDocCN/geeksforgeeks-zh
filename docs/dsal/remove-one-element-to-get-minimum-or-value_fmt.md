...
> 原文: [https://www.geeksforgeeks.org/remove-one-element-to-get-minimum-or-value/](https://www.geeksforgeeks.org/remove-one-element-to-get-minimum-or-value/)
...
## 天真的方法
一种方法是逐个移除每个元素，然后找到剩余元素的 OR。这种方法的时间复杂度为 `O(N^2)`。

## 高效的方法
为了高效地解决问题，`(OR(arr[0…i-1]) | OR(arr[i+1…N-1]))` 的值必须为任意元素 `arr[i]` 确定。为此，可以计算前缀和后缀或数组，如 `pre[]` 和 `suf[]`，其中 `pre[i]` 存储 `OR(arr[0…i])` 和 `suf[i]` 存储 `OR(arr[i…N-1])`。那么删除 `i` 元素后数组的 OR 值可以计算为 `(pre[i-1] | suf[i+1])`，答案将是所有可能的 OR 值中的最小值。
