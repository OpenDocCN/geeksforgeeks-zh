...
> 原文: [https://www.geeksforgeeks.org/rearrange-a-string-so-that-all-same-characters-become-atleast-d-distance-away/](https://www.geeksforgeeks.org/rearrange-a-string-so-that-all-same-characters-become-atleast-d-distance-away/)
...
1.  假设给定的字符串是 `str`，字符串的大小是 `n`，字母的大小假定为 `256`（一个常数）。
...
4.  循环运行 `n` 次
    *   搜索具有最大频率和 `dist[j] <= 0` 的下一个符合条件的字符。
