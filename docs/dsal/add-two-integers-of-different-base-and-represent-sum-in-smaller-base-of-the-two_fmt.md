...
给定两个整数 `X`、`Y` 分别在基数 `B1` 和 `B2`，任务是求整数 `X` 和 `Y` 的和，并表示为 `B1` 和 `B2` 中较小基数的数。
...
思路是利用[基数转换](https://www.geeksforgeeks.org/convert-base-decimal-vice-versa/)将两个整数转换为**基数 10**（小数），然后求出两个数的**和**。然后使用[基数转换](https://www.geeksforgeeks.org/convert-base-decimal-vice-versa/)将**和**转换成较小的基数 `B`（`B1` 和 `B2` 中最小的）。
...
我们可以减少之前方法的转换次数。这里我们需要找到基数较小的 `B` (最小的 `B1` 和 `B2` ，然后将基数较大的整数转换成基数较小的 `B` ，并借助于给定基数的两个整数相加得到整数。
