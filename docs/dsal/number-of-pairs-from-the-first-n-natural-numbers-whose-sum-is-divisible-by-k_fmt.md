...
> 原文: [https://www.geeksforgeeks.org/number-of-pairs-from-the-first-n-natural-numbers-whose-sum-is-divisible-by-k/](https://www.geeksforgeeks.org/number-of-pairs-from-the-first-n-natural-numbers-whose-sum-is-divisible-by-k/)

给定 `N` 和 `K` 的整数值。任务是从 `{1, 2, 3, ..., N}` 以内的自然数集合中找出其和可被 `k` 整除的对的数目。
