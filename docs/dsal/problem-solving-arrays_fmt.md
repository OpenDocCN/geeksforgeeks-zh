...
对于访问 `X[i][j][k]` 的 C 程序，以下中间代码由编译器生成。假设整数的大小是 32 位，字符的大小是 8 位。(GATE-CS-2014)
...
*   在一维数组 `a[100]` 中，元素 `a[i]` 可以作为 `a[i]` 或 `*(a+i)` 或 `*(i+a)` 来访问
*   `a[i]` 的地址可以作为 `&a[i]` 或 `(a+i)` 或 `(i+a)` 来访问
*   在二维数组 `a[100][100]` 中，元素 `a[i][j]` 可以作为 `a[i][j]` 或 `*(*(a+i)+j)` 或 `*(a[i]+j)` 来访问
*   `a[i][j]` 的地址可以作为 `&a[i][j]` 或 `a[i]+j` 或 `*(a+i)+j` 来访问
*   在二维数组中，第 `i` 行的地址可以作为 `a[i]` 或 `*(a+i)` 来访问
...
由于 `A[2]` 代表整数指针，它可以将整数数组的地址存储为: `A[2] = C`；因此，`I` 是有效的。

由于 `A[2]` 代表 `C` 的基址，`A[2][3]` 可以修改为: `A[2][3] = *(C+3) = 0`；它会将 `C[3]` 的值更改为 0。因此，`II` 也是有效的。

由于 `B` 是 2D 阵，`B[2][3]` 可以修改为: `B[2][3] = 5`；它会将 `B[2][3]` 的值更改为 5。因此，`IV` 也是有效的。

由于 `B` 是 2D 数组，`B[2]` 代表第二行地址，不能在语句的 LHS 使用，因为修改地址无效。因此 `III` 是无效的。