...
## 先决条件
[NP-完全性](https://www.geeksforgeeks.org/np-completeness-set-1/)，[哈密顿圈](https://www.geeksforgeeks.org/hamiltonian-cycle-backtracking-6/)。

## 哈密顿圈
无向图`G =(V，E)`中恰好遍历每个顶点一次的圈。

## 问题陈述
给定一个图`G(V，E)`，问题是确定该图是否包含由属于`V`的所有顶点组成的哈密顿圈。
...
*   让我们假设图`G`包含覆盖图的`V`顶点的哈密尔顿路径，从一个随机的顶点开始说`V <sub>开始</sub>`并在`V <sub>end</sub>`结束，现在既然我们将所有的顶点连接到图`G`中任意的新顶点`V <sub>new</sub>`。
    我们通过使用边`V<sub>end</sub>`到`V<sub>new</sub>`和`V <sub>new</sub>`到`V <sub>start</sub>`分别将原来的哈密顿路径扩展到一个哈密顿圈。图形`G’`现在包含遍历所有顶点一次的闭合循环。
...
## 结论
既然，*哈密顿圈*既是，一个**NP-问题**又是 **NP-Hard**。所以是一个 **NP-Complete** 问题。