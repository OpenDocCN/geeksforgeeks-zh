...
接着讨论记忆结构。`bytes` 变量将存储在哪里。堆和栈在内存分配中的作用。
...
3) Java+SQL
某电商希望抽取最近 10 小时内销售的前 100 名产品。对此提出质疑。现在用 Java 存储检索到的数据。(`结果集`用于此。)他让我解释`结果集`的数据结构。一旦数据被检索，如何显示它根据不同的标准排序，如成本，模型等。
...
[Find the missing no in string](https://practice.geeksforgeeks.org/problems/find-the-missing-no-in-string/1)
