...
> 原文: [https://www.geeksforgeeks.org/msd-most-significant-digit-radix-sort/](https://www.geeksforgeeks.org/msd-most-significant-digit-radix-sort/)
...
*   根据元素的`I`<sup>th</sup>数字将元素存储在不同的桶中。
...
该实现是针对基于[计数排序()](https://www.geeksforgeeks.org/counting-sort/)方法的字符串。作为 `C 风格 ASCII 字符`是 `1 字节`。因此， `256 大小数组`用于[统计字符](https://www.geeksforgeeks.org/program-count-occurrence-given-character-string/)的出现次数，并按字典顺序对字符串进行排序。

下面是使用`计数排序()`方法实现的 MSD 基数排序:
