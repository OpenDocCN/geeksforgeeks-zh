...
> 原文: [https://www.geeksforgeeks.org/sum-nodes-longest-path-root-leaf-node/](https://www.geeksforgeeks.org/sum-nodes-longest-path-root-leaf-node/)

给定一个包含 `n` 个节点的二叉树。问题是找到从根节点到叶节点的最长路径上所有节点的和。如果两条或多条路径竞争最长的路径，则考虑具有最大节点总数的路径。
