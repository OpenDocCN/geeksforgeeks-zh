...
> 原文：[https://www.geeksforgeeks.org/permutation-of-an-array-that-has-smaller-values-from-another-array/](https://www.geeksforgeeks.org/permutation-of-an-array-that-has-smaller-values-from-another-array/)

给定两个大小相等的数组 `A` 和 `B`。任务是打印 `A` 的任何排列，使得 `A[i] > B[i]` 的索引 `i` 数量最大化。
...
如果 `A` 中的最小元素打败了 `B` 中的最小元素，我们应该把它们配对。否则对我们的分数没用，因为打不过 `B` 的任何其他元素。

通过上述策略，我们为 `A` 和 `B` 制作了两对向量 `Ap` 以及它们各自的索引。然后对两个向量进行排序并模拟。每当我们在向量 `Ap` 中找到任何元素，使得 `Ap[i].first > Bp[j].first` 对于一些 `(i, j)`，我们将它们配对，即我们将我们的答案数组更新为 `ans[Bp[j].second] = Ap[i].first`。然而如果 `Ap[i].first < Bp[j].first` 对于一些 `(i, j)`，然后我们将它们存储在向量 `remain` 中，最后将它们与任意一个配对。
