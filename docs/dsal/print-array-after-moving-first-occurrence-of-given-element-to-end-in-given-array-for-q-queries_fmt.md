...
> 原文: [https://www.geeksforgeeks.org/print-array-move-first-occurrence-of-given-element-to-end-in-given-array-for-q-query/](https://www.geeksforgeeks.org/print-array-after-moving-first-occurrence-of-given-element-to-end-in-given-array-for-q-queries/)

给定一个由 `N` 个整数组成的数组 `arr[]` 和一个具有 `Q` 个整数的数组 `query[]`，任务是在将第一个出现的 `query[i]` 移动到数组 `arr[]` 的末尾后，为范围 `[0, Q]` 中的每个 `i` 打印数组 `arr[]`。

**示例:**

> **输入:** `arr[] = {1, 3, 1, 3}`，`query[] = {3, 1}`
> **输出:** `1 3 3 1`
> **解释:** 在第一次迭代中，将 `query[0]` 的第一次出现发送到末尾，即，将 `3` 的第一次出现发送到末尾。
> 因此，数组变成 `arr[] = {1, 1, 3, 3}`。
> 在第二次迭代中，将 `query[1]` 的第一次出现发送到最后。
> 因此，数组 `arr[] = {1, 3, 3, 1}` 是必需的数组。
>
> **输入:** `arr[] = {1, 2, 3, 4, 5}`，`query[] = {4, 3}`
> **输出:** `1 2 5 4 3`
...
*   使用变量 `i` 迭代数组 `query[]`，并执行以下操作:
    *   从集合 `m[query[i]]` 中移除代表第一个出现的第一个元素。
    *   将最后一个元素的索引，即 `N+i` 插入到集合 `m[query[i]]` 中。
