...
> 原文: [https://www.geeksforgeeks.org/analysis-algorithms-big-o-analysis/](https://www.geeksforgeeks.org/analysis-algorithms-big-o-analysis/)

在我们之前关于[算法分析](https://www.geeksforgeeks.org/analysis-of-algorithms-set-3asymptotic-notations/)的文章中，我们已经讨论了渐近符号、它们的最坏和最好情况性能等。简而言之。在本文中，我们使用[大 O 渐近符号](https://www.geeksforgeeks.org/analysis-of-algorithms-set-3asymptotic-notations/)详细讨论了算法的分析。

## 算法大 O 分析

我们可以用大 O 符号来表达算法的复杂性。对于大小为 `N` 的问题:

*   恒定时间函数/方法是“1 阶”: `O(1)`
*   线性时间函数/方法是“N 阶”: `O(N)`
*   二次时间函数/方法是“N 阶平方”: `O(N^2)`

**定义**: 设 `g` 和 `f` 是从自然数集合到自身的函数。如果存在常数 `c > 0` 和自然数 `n_0` 使得 `f(n) <= c*g(n)` 为所有 `n >= n_0`，则函数 `f` 被称为 `O(g)` (读 `g` 的 big-oh)。
...
> `f(n) = O(g(n))` 如果存在正整数 `n_0` 和正常数 `c`，使得 `f(n) <= c*g(n)` ∀ `n >= n_0`
...
>
> **多项式函数**:
> 如果 `f(n) = a_0 + a_1*n + a_2*n^2 + ... + a_m*n^m`，则 `O(f(n)) = O(n^m)`。
>
> **求和函数**:
> 如果 `f(n) = f_1(n) + f_2(n) + ... + f_m(n)` 和 `f_i(n) <= f_{i+1}(n)` ∀ `i = 1, 2, ..., m`，则 `O(f(n)) = O(max(f_1(n), f_2(n), ..., f_m(n)))`。
>
> **对数函数**:
> 如果 `f(n) = log_a(n)` 和 `g(n) = log_b(n)`，则 `O(f(n)) = O(g(n))`；就大 O 而言，所有对数函数都以相同的方式增长。
...
在实际情况下，一个算法的性能(运行时间)取决于 `n`，即输入的大小或每个输入项需要的运算次数。
...
> **线性算法** – `O(n)` – 线性搜索。
>
> **超线性算法** – `O(n log n)` – 堆排序，合并排序。
>
> **多项式算法** – `O(n^c)` – strassen 矩阵乘法、冒泡排序、选择排序、插入排序、桶排序。
>
> **指数算法** – `O(c^n)` – 河内塔。
>
> **阶乘算法** – `O(n!)` – 未成年人行列式展开，旅行商问题的蛮力搜索算法。
