...
> 原文: [https://www.geeksforgeeks.org/sum-minimum-maximum-elements-subarrays-size-k/](https://www.geeksforgeeks.org/sum-minimum-maximum-elements-subarrays-size-k/)
...
思路是使用双端队列数据结构和滑动窗口概念。我们创建了两个大小为 `k` 的空双端队列 (`S`，`G`)，它们只存储当前窗口中并非无用的元素的索引。如果一个元素不能是下一个子阵列的最大值或最小值，那么它就是无用的。
