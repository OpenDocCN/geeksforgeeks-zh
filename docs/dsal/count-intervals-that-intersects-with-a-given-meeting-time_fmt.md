...
> 原文：[https://www.geeksforgeeks.org/count-intervals-that-intersects-with-a-given-meeting-time/](https://www.geeksforgeeks.org/count-intervals-that-intersects-with-a-given-meeting-time/)

给定一个由代表开始和结束时间（以 [12 小时格式](https://en.wikipedia.org/wiki/24-hour_clock)）的 `N` 对字符串和代表会议时间的字符串 `P` 组成的数组 `arr[]`，任务是找出包含时间 `P` 的间隔计数。
...
思路是先[把所有时间从 12 小时格式转换成 24 小时格式](https://www.geeksforgeeks.org/program-convert-time-12-hour-24-hour-format/)，然后和时间 `P` 进行范围对比。按照以下步骤解决问题：
...
*   [遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]`，若 `(P >= L && P <= R)` 或 `(P >= R && P <= L)`，则将 `ans` 的计数增加 `1`。
*   最后，完成以上步骤后，打印 `ans`。
