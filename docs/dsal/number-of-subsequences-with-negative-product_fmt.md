...
> 原文: [https://www.geeksforgeeks.org/number-of-subsequences-with-negative-product/](https://www.geeksforgeeks.org/number-of-subsequences-with-negative-product/)
...
- 计算数组中正负元素的数量。
- 可以为子序列选择奇数个负元素，以保持负乘积。具有奇数个负元素的子序列的不同组合的数量将是 `pow(2, neg_count - 1)`。
- 可以为子序列选择任意数量的正元素，以保持负乘积。具有所有正元素的子序列的不同组合的数量将是 `pow(2, pos_count)`。
...
我们也可以通过从**子序列总数**中减去**正子序列总数**来计算负积子序列的数量。使用本文中讨论的方法来寻找具有正积的子序列的总数。