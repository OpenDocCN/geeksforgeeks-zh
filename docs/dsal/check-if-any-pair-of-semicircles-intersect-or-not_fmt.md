...
> 原文: [https://www.geeksforgeeks.org/check-if-any-pair-of-semicircles-intersect-or-not/](https://www.geeksforgeeks.org/check-if-any-pair-of-semicircles-intersect-or-not/)

给定由 `N` 个整数组成的数组 `arr[]`，使得每对点 `(x, y)` 将半圆的端点表示为 `(x, 0)` 和 `(y, 0)`，任务是检查是否有任何一对半圆相交。如果发现为真，则打印 `"是"`。否则，打印 `"否"`。
...
给定的问题可以通过检查任意一对半圆是否相交，然后相应地打印元素来解决。按照以下步骤解决问题:

*   将所有可能的半圆存储在一个向量中，它们对应的是 `x` 坐标和 `y` 坐标。
*   现在，生成所有可能的上述向量对，并执行以下步骤:
    *   让这对坐标为 `X` 和 `Y`，使用以下条件检查圆是否相交:
        *   如果 `(X[0] < Y[0]`、`X[1] < Y[1]`、`Y[0] < X[2])` 或 `(Y[0] < X[0]`、`Y[1] < X[1]`、`X[0] < Y[1])`，否则，不会相交。
        *   如果以上两个圆相交，则打印 `"是"` 并跳出循环。
