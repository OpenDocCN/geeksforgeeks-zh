...
> 原文: [https://www.geeksforgeeks.org/minimum-removal-of-elements-from-end-of-an-array-required-to-obtain-sum-k/](https://www.geeksforgeeks.org/minimum-removal-of-elements-from-end-of-an-array-required-to-obtain-sum-k/)

给定一个整数 `K` 和一个大小为 `N` 的数组 `A[]`，任务是用最少的操作数创建一个和 `K` 的新数组，其中在每个操作中，一个元素可以从 `A[]` 的开始或结束处移除并追加到新数组中。如果无法用总和 `K` 生成新数组，则打印 `-1`。如果有多个答案，打印任意一个。
...
1.  任务是找到两个最小长度的子数组，一个从数组的开始，一个从数组的结束（可能是空的），这样它们的和等于 `K`。
2.  从左侧遍历数组并计算需要从右侧移除的子数组，使得总和为 `K`。
...
1.  计算数组`A`元素的和并存储在变量中，比如`Total`。
2.  问题可以看作是用总和`(Total - K)`找到最大尺寸子阵。
3.  剩下的元素加起来就是`K`。
