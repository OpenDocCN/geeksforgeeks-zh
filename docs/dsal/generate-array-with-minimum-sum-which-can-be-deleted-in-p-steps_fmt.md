...
> 原文: [https://www.geeksforgeeks.org/generate-array-with-minimum-sum-which-can-be-deleted-in-p-steps/](https://www.geeksforgeeks.org/generate-array-with-minimum-sum-which-can-be-deleted-in-p-steps/)
...
### 方法
遵循贪婪的方法可以解决问题。首先，我们将首先放置 `P` 个自然数，对于其余 `(N–P)` 个位置，我们将用 1 填充它，因为我们必须最小化总和。
所以总和将是 `P * (P + 1) / 2 + (N - P)`。
