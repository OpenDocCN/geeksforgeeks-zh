...
*   **设 `G` 为一个图，其中所有顶点的度至少为 `2`。那么 `G` 包含一个循环:** 假设 `G` 简单，让 `P` 为最长路径为 `(`v`<sub>0</sub>`v`<sub>1</sub>`v`<sub>2</sub>…`v`<sub>a 1</sub>`v`<sub>a</sub>)`。鉴于 `v`<sub>a</sub>` 的度数为 `2`，`v`<sub>a</sub>` 必须与至少一个不是 `(`v`<sub>a</sub>–1)` 的顶点 `v` 相邻，如果 `v ∉ {v` <sub>0</sub> `，…，v` <sub>然而，`P` 被选择为最大长度，因此这是不可能的，因此 `v ∈ {v` <sub>0</sub> `…，v`<sub>a–2</sub>`}`。因此，如果 `v = v` <sub>i</sub> ``，那么 `v` <sub>i+1</sub> `，…，v` <sub>a、</sub>`v`<sub>I</sub></sub>` 就是一个循环。
*   **凯莱公式:**
    > 对于 `N` ≤ `1`，在 `N`-`1` 个钟形顶点上有精确的 `N`<sup>`N`–2</sup>` 树形图。