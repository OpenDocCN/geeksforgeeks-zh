...
> 原文：[https://www.geeksforgeeks.org/check-if-given-array-can-be-rearranged-such-that-mean-is-equal-to-median/](https://www.geeksforgeeks.org/check-if-given-array-can-be-rearranged-such-that-mean-is-equal-to-median/)

给定排序浮点[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `arr[]`。检查 `arr[]` 是否可以重新排列，使其平均值等于其中位数。
...
*   初始化一个变量，比如说，`mean` 来存储 `arr[]` 的平均值。
*   检查 `arr[]` 的大小是偶数还是奇数。
    *   如果 `arr[]` 的大小是偶数
        *   使用双指针方法搜索平均值 = `mean` 的两个元素。
        *   将两个指针初始化为 `i=0，j=n-1`。
        *   执行两个指针方法来搜索 `arr[]` 中的中间值。
    *   如果 `arr[]` 的大小是奇数
        *   应用二分搜索法查找 `mean` 是否存在于 `arr[]` 中。
*   如果是 `mean` 被发现返回 `是`，否则 `否`。
