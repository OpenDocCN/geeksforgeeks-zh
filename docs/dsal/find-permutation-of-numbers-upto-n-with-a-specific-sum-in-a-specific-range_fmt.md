...
> 原文：[https://www.geeksforgeeks.org/find-permutation-of-numbers-upto-n-with-a-specific-sum-in-a-specific-range/](https://www.geeksforgeeks.org/find-permutation-of-numbers-upto-n-with-a-specific-sum-in-a-specific-range/)
...
给定的问题可以通过使用[贪婪方法](https://www.geeksforgeeks.org/greedy-algorithms/)来解决，该方法基于这样的观察，即假设 `X` 个数字必须从第一个 `N` 个自然数的排列中选择，因此使和为 `S`，让 `X` 个数字的最小和最大和分别为 `minSum` 和 `maxSum`，然后：

*   `X = R–L+1`
*   `minSum = X(X+1)/2`
*   `maxSum = X(2 * N+1–X)/2`

因此，如果给定的和 `S` 位于范围 `[minSum, maxSum]` 内，则存在一种排列，使得从 `L` 到 `R` 的所有数字的和为 `S`。否则，不存在任何这样的排列。按照以下步骤解决问题：

*   定义一个[函数](https://www.geeksforgeeks.org/functions-in-c/) `possible(int x, int S, int N)` 并执行以下任务：
    *   将变量 `minSum` 初始化为 `x*(x + 1)/2` 并将 `maxSum` 初始化为 `(x *(2 * N)–x+1)/2`。
    *   如果 `S` 小于 `minSum` 或 `S` 大于 `maxSum`，则返回 `false`。否则，返回 `true`。
*   初始化变量，说 `x` 为 `(R–L+1)`。
*   如果函数 `possible(x, S, N)` 返回的[值](https://www.geeksforgeeks.org/c-function-argument-return-values/)返回 `false`，则打印 `-1` 并[从函数](https://www.geeksforgeeks.org/return-statement-in-c-cpp-with-examples/)返回。
...
*   [使用变量 `i` 迭代范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[N, 1]`，并执行以下任务：
    *   如果 `S–i` 大于或等于 `0` 且函数 `possible(x–1, S–i, i–1)` 返回 `true`，则将 `S` 设置为 `(S–i)`，将 `x` 的值减少 `1`，[将 `i` 的值推入矢量](https://www.geeksforgeeks.org/vectorpush_back-vectorpop_back-c-stl/)。
