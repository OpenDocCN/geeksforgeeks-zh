...
> 原文: [https://www.geeksforgeeks.org/number-of-ways-to-remove-a-sub-string-from-s-such-that-all-remaining-characters-are-same/](https://www.geeksforgeeks.org/number-of-ways-to-remove-a-sub-string-from-s-such-that-all-remaining-characters-are-same/)
...
*   现在有两种情况:
    *   当 `str[0] == str[n-1]` 时，前缀的每个字符(包括前缀结束后的字符)将作为子字符串的起始字符，后缀的每个字符(包括后缀开始前的字符)将作为子字符串的结束字符。因此，总的有效子串将是 `count = (count_left + 1) * (count_right + 1)`。
    *   当 `str[0] != str[n-1]`。然后 `count = count_left + count_right + 1`。
