...
> 原文：[https://www.geeksforgeeks.org/rearrange-a-string-according-to-the-given-indices/](https://www.geeksforgeeks.org/rearrange-a-string-according-to-the-given-indices/)

给定一个字符串 `S` 和一个数组 `index[]`，任务是通过将每个字符 `S[i]` 放置到 `index[i]` 来重新排列字符串 `S`。

**例**

> **输入：** `S = "geeksforgeeks"`, `index[] = {5, 6, 7, 0, 1, 2, 8, 9, 10, 3, 4, 11, 12}`
> **输出：** `ksfeegeorgks`
