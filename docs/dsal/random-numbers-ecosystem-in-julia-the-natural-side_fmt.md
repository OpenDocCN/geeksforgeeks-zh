...
> 原文：[https://www.geeksforgeeks.org/random-numbers-ecosystem-in-julia-the-natural-side/](https://www.geeksforgeeks.org/random-numbers-ecosystem-in-julia-the-natural-side/)
...
在[上一篇文章](https://www.geeksforgeeks.org/random-numbers-ecosystem-in-julia-the-pseudo-side/)中，我们谈到了**数学/算法**生成随机数的方法，因此是**伪**随机的。伪随机数发生器非常适合一般应用。但是它们有一个缺点。如果第三个人知道 RNG 初始化时使用的**种子**值，他/她可以很容易地预测 RNG 可能给出的数字序列。这可能会导致申请阶段出现严重失误。

## 真随机数发生器

要解决这个问题，一种方法是使用**真随机数生成器**代替**伪随机数生成器**。
...
从自然界生成随机数是一种经典而有效的方法。[**Random.org**](http://Random.org)是一个从大气噪音中产生随机性的组织。您可以查看以上链接并使用他们提供的服务。但是，如果我们运行的是一个**模拟**，需要在特定的步骤中自动连续地输入随机数，会怎么样呢？我们只是不能每次都去**random.org**网站抄号。幸运的是**Random.org**提供了一个**API**，允许开发人员为他们的界面编写**客户端**。**API客户端**将直接对 Random.org 服务器进行 **HTTP 调用**，获取所需数据。

## 在 Julia 中使用 TRNG

来到 **Julia** ，它提供了利用 **JSON-RPC** 请求和 **HTTP** 调用（分别通过 **HTTP.jl** 和 **JSON.jl** ）的能力。但是手动发出 **POST** 请求，每次我们需要随机数的时候可能会变得很无聊。幸运的是`RandomOrg_API.jl`包为我们处理了这个。带有详细自述文件的软件包链接可在此处获得。
...
> *   `max`：这是将从中挑选随机数的范围的上限。
> *   `replacement`：指定随机数是否应该替换拾取。缺省值（`true`）将导致数字被替换，即结果数字可能包含重复值。如果您希望选取的数字是唯一的，请将此值设置为`false`。
...
该方法通过用户定义的小数位数在`[0, 1]`区间内均匀分布，生成真正随机的**小数**。
