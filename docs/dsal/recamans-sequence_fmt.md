...
给定一个整数 `n`，打印[雷卡曼序列](http://mathworld.wolfram.com/RecamansSequence.html)的前 `n` 个元素。
...
下面是一个简单的实现，我们将所有 `n` 个雷卡曼序列号存储在一个数组中。我们用上面提到的递归公式计算下一个数。
...
本文由 **Kishlay 维尔马**供稿。如果你喜欢 `GeeksforGeeks` 并想投稿，你也可以用 `write.geeksforgeeks.org` 写一篇文章或者把你的文章邮寄到 `review-team@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
