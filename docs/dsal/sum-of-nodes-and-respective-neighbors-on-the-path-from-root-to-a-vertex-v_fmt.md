...
> 原文: [https://www.geeksforgeeks.org/sum-of-nodes-and-respective-neighbors-on-the-path-from-root-to-a-vertex-v/](https://www.geeksforgeeks.org/sum-of-nodes-and-respective-neighbors-on-the-path-from-root-to-a-vertex-v/)

给定具有 `N` 个顶点的根树、代表分配给每个节点的值的数组 `values[]`，以及顶点 `V`，任务是计算位于从根（总是 `0`）到 `V` 的路径中的节点和直接邻居的值的总和。

## 示例

> **输入:** `N = 8`，`values = {1, 2, 3, 0, 0, 4, 3, 6}`，`V = 7`
>
