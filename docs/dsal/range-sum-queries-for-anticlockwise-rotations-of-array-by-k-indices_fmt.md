...
> 原文: [https://www.geeksforgeeks.org/range-sum-queries-for-anticlockwise-rotations-of-array-by-k-indices/](https://www.geeksforgeeks.org/range-sum-queries-for-anticlockwise-rotations-of-array-by-k-indices/)
...
*   创建一个 [**前缀数组**](https://www.geeksforgeeks.org/prefix-sum-array-implementation-applications-competitive-programming/) ，其大小是 `arr` 的两倍，并将 `arr` 的 `I<sup>th</sup>` 索引处的元素复制到 `I<sup>th</sup>` 和 `N+I<sup>th</sup>` 索引处的元素，用于 `【0，N】` 中的所有 I。
...
*   对于类型 1 的查询，将 `开始` 切换到 `((start + K) % N)th` position。
*   对于类型 2 的查询，计算 `prefix[start + R] - prefix[start + L- 1 ]`。
*   如果 `开始+ L >= 1` 或打印的值 `prefix[start + R]`。
*   否则。
