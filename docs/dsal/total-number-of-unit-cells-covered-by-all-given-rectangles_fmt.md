...
> 原文: [https://www.geeksforgeeks.org/total-number-of-unit-cells-covered-by-all-given-rectangles/](https://www.geeksforgeeks.org/total-number-of-unit-cells-covered-by-all-given-rectangles/)

给定一个由 `N` 个矩形的坐标组成的矩阵 `A[][]`，使得 `{A[i][0], A[i][1]}` 表示矩形的左下坐标，`{A[i][2], A[i][3]}` 表示矩形的右上坐标，任务是找出所有矩形覆盖的单元格总数。

**例:**

> **输入:** `N = 2`，`A[][] = {{1, 1, 3, 3}, {2, 1, 4, 2}}`
> **输出:** `5`
> **说明:**
>
...
>
> 第一个矩形包含以下 4 个单元格:
> `{(1, 1), (1, 2), (2, 2), (2, 1)}, {(1, 2), (1, 3), (2, 3), (2, 2)}, {(2, 1), (2, 2), (3, 1)}, {(2, 2), (2, 3), (3, 3), (3, 2)}`
> 第二个矩形包含 2 个单元格:
> `{(2, 1), (2, 2), (3, 2), (3, 1)}, {(3, 1)}`
>
> **输入:** `N = 3`，`A[][] = {{1, 3, 4, 5}, {3, 1, 7, 4}, {5, 3, 8, 6}}`
> **输出:** `24`
...
3.  最后，我们将维护一个变量 `area` 来存储包围的单元格数量。增加 `area` 如果 `arr[i][j] == 1`。
4.  打印 `area` 的最终值。
