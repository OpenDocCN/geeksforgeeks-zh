...
> 原文: [https://www.geeksforgeeks.org/number-of-k-length-subsequences-with-minimum-sum/](https://www.geeksforgeeks.org/number-of-k-length-subsequences-with-minimum-sum/)
...
> **输入:** `arr[] = {1, 2, 3, 4}`，`K = 2`
> **输出:** 1
> 长度为 2 的子序列为 `(1, 2)`，`(1, 3)`，`(1, 4)`，`(2, 3)`，`(2, 4)` 和 `(3, 4)`。
> 最小和为 3，唯一有这个和的子序列为 `(1, 2)`。
>
> **输入:** `arr[] = {2, 1, 2, 2, 2, 1}`，`K = 3`
> **输出:** 4

**方法:** 给定数组中长度为 `K` 的子序列的最小可能和是数组中最小元素 `K` 的和。设 `X` 为数组的 `K` 个最小元素中的最大元素，设其在数组的 `K` 个最小元素中出现的次数为 `Y`，其在整个数组中的总出现次数为 `cntX`。现在有 `C(cntX, Y)` 的方式选择这个元素，在 `K` 最小的元素中，就是需要的子序列的个数。
