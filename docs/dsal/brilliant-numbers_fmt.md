...
思路是利用厄拉多塞的[筛找出所有小于等于给定数 `N` 的素数](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)。一旦我们有了一个告诉所有素数的数组，我们就可以遍历这个数组来找到一个给定乘积的对。我们将使用厄拉多塞的筛子找到给定产品的[两个质数，并检查这对质数是否具有相同的位数](https://www.geeksforgeeks.org/find-two-distinct-prime-numbers-with-given-product/)。
以下是上述方法的实现:
