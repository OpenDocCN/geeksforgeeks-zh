...
使用 [`strtok()`](https://www.geeksforgeeks.org/strtok-strtok_r-functions-c-examples/) 将字符串分解成单词，现在每个单词取两个指针， `i` 和 `j` 分别指向字符串的第二个和第二个最后一个字符。交换这些字符，然后**增加 i** 和**减少 j** 。重复这些步骤，同时 `i < j` 。
