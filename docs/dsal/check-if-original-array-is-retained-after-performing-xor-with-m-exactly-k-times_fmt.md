...
> 原文: [https://www.geeksforgeeks.org/check-if-original-array-is-retained-after-performing-xor-with-m-exactly-k-times/](https://www.geeksforgeeks.org/check-if-original-array-is-retained-after-performing-xor-with-m-exactly-k-times/)

给定一个数组 `A` 和两个整数 `M` 和 `K`，任务是检查并打印“`Yes`”，如果通过对数组元素执行精确的“`K`”数量的[位异或](https://www.geeksforgeeks.org/tag/xor/)运算可以保留原始数组，则带有“`M`”。否则打印“`No`”。
...
这个问题可以使用[异或属性](https://www.geeksforgeeks.org/bitwise-hacks-for-competitive-programming/)来解决。

> `A XOR B = C` 和 `C XOR B = A`
