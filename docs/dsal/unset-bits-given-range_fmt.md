...
给定一个非负数 `n` 和两个值 `l` 和 `r`。问题是在 `n` 的二进制表示中 `l` 到 `r` 范围内的位不设置，即从最右边的 `l` 位到最右边的 `r` 位不设置位。
**约束：** `1 <= l <= r <=` 二进制表示的位数 `n`。
...
1.  计算 `num = (1 << (sizeof(int) * 8 - 1)) - 1`。这将产生最高正整数 `num`。`num` 中的所有位都将被设置。
2.  在 `num` 中的 `l` 至 `r` 范围内切换钻头。参考[这个](https://www.geeksforgeeks.org/toggle-bits-given-range/)岗位。
3.  现在，执行 `n = n & num`。这将解除 `l` 至 `r` 范围内 `n` 的位。
4.  返回 `n`。

**注意：** `sizeof(int)` 已被用作输入为 `int` 数据类型。对于大输入，您可以使用 `long int` 或 `long long int` 数据类型来代替 `int`。
