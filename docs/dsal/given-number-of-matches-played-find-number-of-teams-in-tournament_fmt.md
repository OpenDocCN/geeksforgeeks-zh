...
> 原文: [https://www.geeksforgeeks.org/given-number-of-matches-played-find-number-of-teams-in-tournament/](https://www.geeksforgeeks.org/given-number-of-matches-played-find-number-of-teams-in-tournament/)
...
**进场:**
由于每场比赛都是两队之间进行。所以这个问题类似于从给定的 `N` 个对象中选择 2 个对象。因此比赛总数将为 `C(N, 2)`，其中 `N` 为参赛队伍数。因此，

> `M = C(N, 2)`
> `M = (N * (N – 1)) / 2`
> `N² – N – 2 * M = 0`
> 这是一个 `ax² + bx + c = 0` 型二次方程。这里 `a = 1`，`b = -1`，`c = 2 * M`。因此，应用公式
> `x = (-b + sqrt(b² – 4ac)) / 2a` 和 `x = (-b – sqrt(b² – 4ac)) / 2a`
