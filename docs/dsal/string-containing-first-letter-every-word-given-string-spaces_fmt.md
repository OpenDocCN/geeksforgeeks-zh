...
> 原文: [https://www.geeksforgeeks.org/string-containing-first-letter-every-word-given-string-spaces/](https://www.geeksforgeeks.org/string-containing-first-letter-every-word-given-string-spaces/)

给定一个包含小写英文字母和空格的字符串。它可能包含多个连续空格。任务是获取每个单词的第一个字母，并将结果作为字符串返回。结果不应包含任何空格。
...
其思想是遍历字符串中的每个字符，并维护一个布尔变量，该变量最初被设置为 `true`。每当我们遇到空格，我们设置布尔变量为 `true`。如果我们遇到除了空格之外的任何字符，我们将检查布尔变量，如果它被设置为 `true`，就将该字符复制到输出字符串并将布尔变量设置为 `false`。如果布尔变量设置为 `false`，则不执行任何操作。

### 算法步骤

```
1. 遍历字符串 `str`。并初始化一个变量 `v` 为 `true`。
2. 如果 `str[i] == ' '`。设置 `v` 为 `true`。
3. 如果 `str[i] != ' '`。检查 `v` 是否为 `true`。
   a) 如果为 `true`，将 `str[i]` 复制到输出字符串并将 `v` 设置为 `false`。
   b) 如果为 `false`，不执行任何操作。
```
