...
> 原文: [https://www.geeksforgeeks.org/check-whether-an-array-can-be-made-strictly-increasing-by-modifying-atmost-one-element/](https://www.geeksforgeeks.org/check-whether-an-array-can-be-made-strictly-increasing-by-modifying-atmost-one-element/)

给定一个正整数数组 `arr[]`，任务是通过修改**至多**一个元素来确定这个数组是否可以严格递增。
...
对于每个元素 `arr[i]`，如果它大于 `arr[i-1]` 和 `arr[i+1]` 或者小于 `arr[i-1]` 和 `arr[i+1]`，则需要修改 `arr[i]`。
