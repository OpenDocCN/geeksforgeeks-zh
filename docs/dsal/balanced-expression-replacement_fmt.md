...
> 原文: [https://www.geeksforgeeks.org/balanced-expression-replacement/](https://www.geeksforgeeks.org/balanced-expression-replacement/)

给定一个只包含以下内容的字符串：`{`、`}`、`(`、`)`、`[`、`]`。有些地方用`X`代替任何括号。确定是否可以通过用适当的括号替换所有的`X`来建立有效的括号序列。

## 先决条件
[平衡括号表达式](https://www.geeksforgeeks.org/check-for-balanced-parentheses-in-an-expression/)
...
## 方法
我们讨论了一个验证给定的[括号表达式是否平衡的解决方案](https://www.geeksforgeeks.org/check-for-balanced-parentheses-in-an-expression/)。
