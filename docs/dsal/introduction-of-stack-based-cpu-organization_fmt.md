...
> 原文：[https://www.geeksforgeeks.org/introduction-of-stack-based-cpu-organization/](https://www.geeksforgeeks.org/introduction-of-stack-based-cpu-organization/)
...
此操作导致在堆栈顶部插入一个操作数，并减少堆栈指针寄存器。`PUSH`指令的格式是：
...
此操作导致从堆栈顶部删除一个操作数，并增加堆栈指针寄存器。`POP`指令的格式是：
...
`PDP-11`、`英特尔的 8085` 和 `惠普的 3000` 都是堆栈组织计算机的一些例子。
