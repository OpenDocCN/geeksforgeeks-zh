...
> 原文：[https://www.geeksforgeeks.org/print-all-sequences-of-given-length/](https://www.geeksforgeeks.org/print-all-sequences-of-given-length/)
...
递归函数 `printSequenceRecur` 生成并打印长度为 `k` 的所有序列，其思想是多使用一个参数索引。函数 `printSequenceRecur` 将 `arr[]` 中的所有项保持在正常状态直到索引，更新索引处的值，并在索引后递归调用自身以获取更多项。
