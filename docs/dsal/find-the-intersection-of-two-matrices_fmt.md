...
> 原文：[https://www.geeksforgeeks.org/find-the-intersection-of-two-matrices/](https://www.geeksforgeeks.org/find-the-intersection-of-two-matrices/)

给定两个相同顺序的矩阵 `A[][]` 和 `B[][]`，维度为 `m * n`。任务是找到两个矩阵的交集 `C`，其中：

*   `C<sub>ij</sub> = A<sub>ij</sub>` 如果 `A<sub>ij</sub> = B<sub>ij</sub>`
*   `C = "*"`，否则。
