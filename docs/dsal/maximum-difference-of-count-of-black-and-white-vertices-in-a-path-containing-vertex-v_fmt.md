...
的想法是使用[动态编程](http://www.geeksforgeeks.org/dynamic-programming/)的概念来解决此问题。
...
class GFG{
    ...
    // Function to perform DFS on the given tree
    static void dfs(int v, int p)
    {
        dp[v] = c[v];
        foreach (int i in g[v]) 
        {
            if (i == p)
                continue;
            dfs(i, v);
            // Returning calculated maximum difference between white and black for current vertex
            dp[v] += Math.Max(0, dp[i]);
        }
    }

    // Function that prints the maximum difference between white and black vertices
    void maximumDifference(int v, char []color, int n)
    {
        for (int i = 0; i < n; i++) 
        {
            // Condition for white vertex
        ...
        // Calling dfs function for vertex v
        dfs(v, -1);
        // Printing maximum difference between white and black vertices
        Console.Write(dp[v] + "\n");
    }
