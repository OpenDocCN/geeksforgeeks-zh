...
是保持大小为`k`的数组。 想法是保持数组排序，以便在`O(1)`时间内找到第`k`个最大元素（如果数组按升序排序，我们只需要返回数组的第一个元素）
...
是使用大小为`k`的自平衡二分搜索树。 可以在`O(Logk)`时间中找到第`k`个最大元素。
...
是使用大小为`k`的最小堆存储流的`k`个最大元素。 第`k`个最大元素始终位于根，可以在`O(1)`时间找到。
