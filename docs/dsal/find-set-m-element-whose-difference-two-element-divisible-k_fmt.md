...
> 原文：[https://www.geeksforgeeks.org/find-set-m-element-whose-difference-two-element-divisible-k/](https://www.geeksforgeeks.org/find-set-m-element-whose-difference-two-element-divisible-k/)
...
**方法：** 要解决这个问题，只需记录一个元素被 `k` 除时的余数。创建一个大小为 `k` 的多维数组 `remainder_set[][]`，其索引显示余数，当被 `k` 除时，该数组的元素将按照它们相应的余数排列。例如，如果 `arr[i] % k = 3`，那么 `arr[i]` 将是所有元素的 `remainder_set[3]` 的元素，依此类推。现在，遍历 `remainder_set`，可以很容易地得到一个大于或等于所需大小 `m`（如果存在）的集。当然，集合中任何元素的差都可以被 `k` 整除。
