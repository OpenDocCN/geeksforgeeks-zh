# 通过交换不相等的字符对检查两个二进制字符串是否可以相等

> 原文: [https://www.geeksforgeeks.org/check-if-two-binary-strings-can-be-made-equal-by-swapping-pair-of-unequal-characters/](https://www.geeksforgeeks.org/check-if-two-binary-strings-can-be-made-equal-by-swapping-pair-of-unequal-characters/)

给定两个长度为`N`（`1≤N≤10^5`）的[二进制字符串](https://www.geeksforgeeks.org/tag/binary-string/) `S1`和`S2`，任务是通过执行以下任意次数的操作来检查是否有可能将字符串`S1`转换为`S2`：
