...
给定大小为`N`的数组`arr[]`，任务是检查该数组是否仅包含一个不同的元素。 如果仅包含一个不同的元素，则打印`Yes`，否则打印`No`。
...
想法是对给定的数组排序，然后针对每个有效索引检查当前元素和下一个元素是否相同。 如果它们不相同，则表示该数组包含多个不同的元素，因此打印`No`，否则打印`Yes`。
...
可以通过使用集合数据结构解决此问题。 自设置以来，不允许重复。 步骤如下：
...
也可以在不使用任何额外空间的情况下解决此问题。 步骤如下：

1.  假定数组的第一个元素是数组中唯一的唯一元素，并将其值存储在变量中，例如`X`。
