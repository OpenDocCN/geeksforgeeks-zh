...
> 原文：[https://www.geeksforgeeks.org/partition-an-array-into-two-subsets-with-equal-count-of-unique-elements/](https://www.geeksforgeeks.org/partition-an-array-into-two-subsets-with-equal-count-of-unique-elements/)

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是将数组划分为两个子集，使得两个子集中唯一元素的计数相同。对于每个元素，如果该元素属于第一个子集，则打印 `1`。否则，打印 `2`。如果做不到这样的分区，那么打印 `-1`。
...
*   初始化一张图，记为 `M`，存储所有阵元的频率。
*   初始化数组，比如 `ans[]`，存储每个数组元素所属的子集号。
*   通过在地图 `M` 中计数具有频率 `1` 的元素，找到数组中唯一元素的计数。让这个数成为 `C`。
*   如果 `C` 的值为偶数，则通过将这些元素标记为 `1`，将这些元素的一半移动到第一个子集中，并将数组中的所有元素标记为 `2` 到 `ans[]`。
