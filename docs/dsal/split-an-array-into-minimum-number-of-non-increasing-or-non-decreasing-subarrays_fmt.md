...
> 原文: [https://www.geeksforgeeks.org/split-an-array-into-minimum-number-of-non-increasing-or-non-decreasing-subarrays/](https://www.geeksforgeeks.org/split-an-array-into-minimum-number-of-non-increasing-or-non-decreasing-subarrays/)
...
按照以下步骤解决问题:
*   初始化一个变量，说 `ans`，用 `1` 存储需要的结果，用 `N` 存储 `current` 跟踪电流序列的顺序，是不递减 (`I`)、不递增 (`D`)，还是无 (`N`)。
*   现在，[迭代范围为 `[1，N–1]` 的数组](https://www.geeksforgeeks.org/iterating-arrays-java/):
    *   如果 `current` 等于 `N`，执行以下操作:
        *   如果 `arr[i] < arr[i-1]` 则更新 `current` 为 `D`。
        *   否则，如果 `arr[i] > arr[i-1]`，则更新 `current` 为 `I`。
        *   否则，将 `current` 更新为 `N`。
    *   如果 `current` 等于 `I`，执行以下操作:
        *   如果 `arr[i] >= arr[i-1]`，则更新 `current` 为 `I`。
        *   否则，将 `current` 更新为 `N`，并将 `ans` 增加 `1`。
    *   否则，请执行以下操作:
        *   如果 `arr[i] <= arr[i-1]`，则更新 `current` 为 `D`。
        *   否则，将 `current` 更新为 `N`，并将 `ans` 增加 `1`。
