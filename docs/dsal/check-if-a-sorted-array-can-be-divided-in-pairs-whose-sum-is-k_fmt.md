...
> 原文: [https://www.geeksforgeeks.org/check-if-a-sorted-array-can-be-divided-in-pairs-whose-sum-is-k/](https://www.geeksforgeeks.org/check-if-a-sorted-array-can-be-divided-in-pairs-whose-sum-is-k/)
...
一个简单的解决方案是迭代每个元素 `arr[i]`。查找是否有另一个尚未访问的值为 `(k–arr[i])` 的元素。如果没有这样的元素，则返回 `false`。如果找到一对，则将两个元素都标记为已访问。该解决方案的时间复杂度为 `O(n^2)`，需要 `O(n)` 个额外空间。

更好的解决方案是使用哈希。这篇文章给出的解决方案可以很容易地修改为工作。这个解决方案的时间复杂度是 `O(n)`，但是它需要额外的哈希表空间。
