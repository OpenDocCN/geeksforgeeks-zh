...
> 原文：[https://www.geeksforgeeks.org/sum-of-fourth-power-of-first-n-even-natural-numbers/](https://www.geeksforgeeks.org/sum-of-fourth-power-of-first-n-even-natural-numbers/)
...
在这个简单的求前 n 个偶数自然数的四次幂的方法中，迭代一个从 1 到 n 的循环。每次迭代都存储在变量中，并持续到`(i != n)`。这是一个 `O(N)` 时间复杂度。
...
一个有效的解决方案是使用直接的数学公式，其推导如下，这是只取 `O(1)` 时间复杂度。

> 前 n 个偶数自然数的四次幂之和 = `8 * (n * (n + 1) * (2 * n + 1) * (3 * n^2 + 3 * n - 1)) / 15`

**这个公式是怎么用的？**

[自然数的四次幂之和](https://www.geeksforgeeks.org/sum-fourth-powers-first-n-natural-numbers/) 是 = `(n * (n + 1) * (2 * n + 1) * (3 * n^2 + 3 * n - 1)) / 30`

我们需要偶数个自然数，所以我们把每个项乘以 `2^4`。
