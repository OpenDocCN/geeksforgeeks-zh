...
> 原文：[https://www.geeksforgeeks.org/rearrange-numbers-in-an-array-such-that-no-two-adjacent-numbers-are-same/](https://www.geeksforgeeks.org/rearrange-numbers-in-an-array-such-that-no-two-adjacent-numbers-are-same/)
...
1.  建立一个存储元素及其频率的优先级队列或最大堆。`Priority_queue` 或 `max_heap` 是基于元素的频率构建的。
