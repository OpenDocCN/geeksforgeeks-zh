...
> 原文: [https://www.geeksforgeeks.org/sum-of-the-sums-of-all-possible-subsets/](https://www.geeksforgeeks.org/sum-of-the-sums-of-all-possible-subsets/)
...
天真法是利用幂集找到所有子集，然后对所有可能的子集求和得到答案。
**时间复杂度:** `O(2^N)`

## 高效方法

一个高效的方法就是用观察来解决问题。如果我们写出所有的子序列，一个共同的观察点是每个数在一个子集中出现 `2^(N–1)` 次，因此将导致 `2^(N-1)` 作为对总和的贡献。遍历数组，在答案中添加 `(arr[i] * 2^(N-1))`。
