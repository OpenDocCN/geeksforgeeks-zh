...
**天真法:**这种方法是基于[本文](https://www.geeksforgeeks.org/sum-divisors-1-n/)中实现的方法，上述问题可以在 `O(N^(1/k))` 中解决，对于任何 `K^(th)` 幂除数，其中 `N` 是我们要求其和的数。这是因为，在这个总和中，每个数字将贡献`floor(N/p)` 或 `int(N/p)` 次。因此，在迭代这些完美幂的时候，我们只需要将`p * int(N/p)`加到和上。

**时间复杂度:** `O(√N)`

**有效方法:**

*   让我们从 `start = 2` 开始，找到哪个 `floor(N/(start^2)) = floor(N/(end^2))` 的最大范围(`start` 到 `end`)
*   区间 `[start, end]` 中所有完美方块的贡献将贡献 `floor(N/(start^2))` 次，因此我们可以立即对该范围进行更新。
*   范围 `[start, end]` 的贡献可以表示为:

> `floor(N/(start^2)) * (end - start - 1)`
...
**时间复杂度:** `O(N^(1/3))` 作为 `N/(x^2)` 不能取大于 `N^(1/3)` 的不同值为 `N` 。
