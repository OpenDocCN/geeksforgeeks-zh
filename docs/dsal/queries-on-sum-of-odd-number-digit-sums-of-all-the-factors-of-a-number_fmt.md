...
给出 `Q` 个查询。每个查询包含一个正整数 `n`。任务是输出 `n` 的所有除数中包含的奇数位数之和的和。
...
> `6` 中的是 `0`，`9` 中的是 `9`，`12` 中的是 `1`，`18` 中的是 `1`，`36` 中的是 `3`。
> 于是，总和变成了 `18`。
...
现在，要求因子奇数位数之和的和，我们可以利用厄拉多塞的[筛的跳跃现象](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)。所以，对于所有可能的因素，把它们的贡献加到倍数上。
