...
我们可以观察到上面的递归实现做了很多重复的工作(我们通过绘制递归树也可以做到这一点)。由于有重叠的子问题，我们可以对此使用动态规划。下面是一个基于动态编程的实现。
