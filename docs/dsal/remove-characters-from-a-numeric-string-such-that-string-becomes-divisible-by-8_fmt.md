...
> 原文: [https://www.geeksforgeeks.org/remove-characters-from-a-numeric-string-such-that-string-becomes-divisible-by-8/](https://www.geeksforgeeks.org/remove-characters-from-a-numeric-string-such-that-string-becomes-divisible-by-8/)
...
**方法:** 考虑到 8 的可除性规则，我们只需要检查 `str` 最后 3 个字符组成的数是否能被 8 整除。因此，我们可以迭代 8 到 1000 的所有倍数，并检查在给定的字符串中是否有任何倍数作为子序列存在，那么这个倍数就是我们需要的答案。否则，不存在答案，因为所有大于 1000 的 8 的倍数也需要有已经检查过的数字(由最后 3 位数字组成)。
