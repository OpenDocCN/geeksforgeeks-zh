...
> 原文: [https://www.geeksforgeeks.org/find-number-from-its-divisors/](https://www.geeksforgeeks.org/find-number-from-its-divisors/)
...
对给定的 `N` 除数进行排序，编号 `X` 将是排序数组中的**第一个编号*最后一个编号**。通过将除 `1` 和 `X` 之外的所有 `X` 的除数存储在另一个数组中，交叉检查 `X` 是否与给定语句矛盾，如果形成的数组与给定数组不相同，则打印 `-1`，否则打印 `X`。
