...
> 原文：[https://www.geeksforgeeks.org/program-to-print-a-string-in-vertical-zigzag-manner/](https://www.geeksforgeeks.org/program-to-print-a-string-in-vertical-zigzag-manner/)

给定大小为`N`的字符串`S`和若干行`R`，任务是相对于给定的行数以垂直之字形打印给定的字符串，如示例所示。

**示例：**

> **输入：** `S = "123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"`，`R = 9`
> **输出：**
...
*   初始化一个变量`interval`为`2*R-2`来存储主列之间的间隙。
*   使用变量`i`在范围`[0, R-1]`中迭代：
    *   将变量`step`初始化为`interval - 2*i`，以存储每行的步长值。
    *   使用变量`j`在范围`[i, N-1]`中迭代，在每次迭代中将`j`递增`interval`：
        *   打印字符`S[j]`。
        *   如果`step`的值在`[1, interval-1]`和`step + j < N`的范围内，则打印`(interval - R - i)`的空格数，然后打印`s[j+step]`，最后打印`(i-1)`空格。
        *   否则打印`(interval - R)`的空格数。
    *   每次外循环迭代后打印换行符。
