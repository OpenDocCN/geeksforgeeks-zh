...
> 原文：[https://www.geeksforgeeks.org/count-full-nodes-binary-tree-iterative-recursive/](https://www.geeksforgeeks.org/count-full-nodes-binary-tree-iterative-recursive/)
...
1.  创建一个空的队列 `Node` 并将根节点推入队列。
2.  当 `nodeQueue` 不为空时，执行以下操作：
    a. 从队列中弹出一个项目并进行处理。
        a.1) 如果它是完整节点，则增加 `count++`。
    b. 如果弹出项目的左子节点可用，则将其推入队列。
    c. 如果弹出项目的右子节点可用，则将其推入队列。
