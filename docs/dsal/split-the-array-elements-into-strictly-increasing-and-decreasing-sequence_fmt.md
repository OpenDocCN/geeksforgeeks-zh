...
> 原文: [https://www.geeksforgeeks.org/split-the-array-elements-into-strictly-increasing-and-decreasing-sequence/](https://www.geeksforgeeks.org/split-the-array-elements-into-strictly-increasing-and-decreasing-sequence/)

## 问题描述

给定一组 `N` 个元素。任务是将元素分成两个数组，比如 `a1[]` 和 `a2[]`，一个包含**严格的递增的**元素，另一个包含**严格的递减的**元素，且满足 `a1.size() + a2.size() = a.size()`。如果不能这样做，打印 `-1` 或打印两个数组。
