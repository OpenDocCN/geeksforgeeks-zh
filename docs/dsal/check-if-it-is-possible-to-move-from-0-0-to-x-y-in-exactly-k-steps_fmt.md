# 检查是否可以从(0,0)移动到(X,Y)正好K步

> 原文: [https://www.geeksforgeeks.org/check-if-it-is-possible-to-move-from-0-0-to-x-y-in-exactly-k-steps/](https://www.geeksforgeeks.org/check-if-it-is-possible-to-move-from-0-0-to-x-y-in-exactly-k-steps/)
...
很明显，从 `(0，0)` 到达 `(X，Y)` 的最短路径将是 `最小移动 = (|X| + |Y|)`。所以，如果 `K < 最小移动` 那么就不可能达到 `(X，Y)`。但是如果 `K ≥ 最小移动` 那么在达到 `最小移动` 中的 `(X，Y)` 步数之后，剩余的 `(K–最小移动)` 步数必须是偶数，以便在剩余的步数中保持在该点。
