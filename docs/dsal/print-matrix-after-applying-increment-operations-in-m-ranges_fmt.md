...
> 原文: [https://www.geeksforgeeks.org/print-matrix-after-applying-increment-operations-in-m-ranges/](https://www.geeksforgeeks.org/print-matrix-after-applying-increment-operations-in-m-ranges/)
...
> 第一次查询后: `mat[][] = { {1, 1}, {1, 1} }`
> 第二次查询后: `mat[][] = { {2, 2}, {1, 1} }`
> **输入:** `N = 5`，`q[][] = { { 0, 0, 1, 2 }, { 1, 2, 3, 4 }, { 1, 4, 3, 4 } }`
> **输出:**
> 1 1 1 1 1
> 1 1 2 1 2
> 2 2 2 2 2
...
对于每个查询 `(X1, Y1)` 表示子矩阵的左上角单元格，`(X2, Y2)` 表示子矩阵的右下角单元格。对于每个左上角单元格，在左上角元素中添加 `1`，并从右下角单元格旁边的元素（如果有）中减去 `1`。
