...
> 原文: [https://www.geeksforgeeks.org/check-if-there-exists-a-number-with-x-factors-out-of-which-exactly-k-are-prime/](https://www.geeksforgeeks.org/check-if-there-exists-a-number-with-x-factors-out-of-which-exactly-k-are-prime/)
...
*   假设一个数 `N` 有 `X` 因子，其中 `K` 为素数，比如 `K_{1}, K_{2}, K_{3}, ...K_{M}`。
*   因此，数可以写成 `N = K_{1}^{a}, K_{2}^{b}, K_{3}^{c}, ...K_{M}^{K}`，其中因子的总数由 `X = (a+1) * (b+1) * (c+1) * ... *(K+1)` 计算。
