...
*   它包括一个 `sketchbook`，这是 `IDE` 的一个极简替代物。这个 `sketchbook` 可以作为一个普通的 `IDE` 来组织项目。
...
## 安装加工
为了用加工语言编码，用户可以从[官网](https://processing.org/)下载加工画册。除此之外，用户还可以下载代码 [`jar` 文件](https://www.geeksforgeeks.org/jar-files-java/)，并在任何一个 `IDE` 中设置使用处理。
