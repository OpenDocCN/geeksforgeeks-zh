...
> 原文：[https://www.geeksforgeeks.org/sort-the-array-using-slow-sort/](https://www.geeksforgeeks.org/sort-the-array-using-slow-sort/)
...
**进场：** 像[合并排序](https://www.geeksforgeeks.org/merge-sort/)一样，`慢速排序`是一个[分治](https://www.geeksforgeeks.org/divide-and-conquer-introduction/)算法。它将输入数组分成两半，称自己为两半，然后比较两半的最大元素。它将子数组的最大元素存储在子数组的顶部位置，然后递归调用没有最大元素的子数组。按照以下步骤解决问题：
...
**最佳病例时间复杂度：** `O(N^(log_2 N / (2 + e)))`，其中 `e > 0`
**平均病例时间复杂度：** `O(N^(log_2 N / 2))`
**辅助空间：** `O(1)`