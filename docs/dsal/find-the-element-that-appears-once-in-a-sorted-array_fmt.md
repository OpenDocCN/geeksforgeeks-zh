...
> 原文: [https://www.geeksforgeeks.org/find-the-element-that-appears-once-in-a-sorted-array/](https://www.geeksforgeeks.org/find-the-element-that-appears-once-in-a-sorted-array/)
...
另一个简单的解决方案是使用 `XOR` 的属性(`a ^ a = 0` & `a ^ 0 = a`)。想法是找到完整数组的 `XOR`。数组的 `XOR` 就是需要的答案。
...
一个高效解可以在 `O(Log n)` 时间内找到需要的元素。想法是用[二分搜索法](http://geeksquiz.com/binary-search/)。下面是对输入数组的观察。
...
3.  如果 `mid` 是奇数，则比较 `arr[mid]` 和 `arr[mid – 1]`。如果两者相同，则所需元素在 `mid` 之后，否则在 `mid` 之前。
