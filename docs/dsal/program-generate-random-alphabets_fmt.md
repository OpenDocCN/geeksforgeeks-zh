...
> 原文：[https://www.geeksforgeeks.org/program-generate-random-alphabets/](https://www.geeksforgeeks.org/program-generate-random-alphabets/)

先决条件：[`rand()`和`srand()`](https://www.geeksforgeeks.org/rand-and-srand-in-ccpp/)

给定字符数组中的所有字母，打印给定大小的随机字符串。我们将使用`rand()`函数打印随机字符。它返回随机整数值。这个数字是由一种算法生成的，这种算法在每次调用时都会返回一系列明显不相关的数字。
