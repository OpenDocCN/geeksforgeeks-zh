...
> 原文: [https://www.geeksforgeeks.org/multiply-n-complex-numbers-given-as-strings/](https://www.geeksforgeeks.org/multiply-n-complex-numbers-given-as-strings/)

给定字符串形式的 `N` 个复数，任务是打印这些 `N` 个复数的乘法。
...
*   接下来，将字符串转换为带有适当符号的数字。将字符串的实部和虚部存储在单独的变量中。取 `a` 为第一个复数的实部，`b` 为第一个复数的虚部。取 `c` 为第二个复数的实部，`d` 为第二个复数的虚部。
*   接下来，我们将通过计算 `a` 和 `c` 的乘积，并将其与 `b` 和 `d` 的乘积相减，来计算实部的合成值。对于虚部，我们将计算 `a` 和 `d` 的乘积之和，以及 `b` 和 `c` 的乘积之和。
*   然后，我们将生成一个临时字符串来获取之前计算的实部和虚部值之和。
