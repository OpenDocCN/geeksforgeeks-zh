...
> 原文: [https://www.geeksforgeeks.org/find-out-the-prime-numbers-in-the-form-of-anb-or-bna/](https://www.geeksforgeeks.org/find-out-the-prime-numbers-in-the-form-of-anb-or-bna/)

## 问题描述

给定两个整数 `A` 和 `B` 和一个整数 `N`。任务是找出 `A + nB` 或 `B + nA` 形式的 `N` 个素数（`n` = 1, 2, 3...）。如果不可能，请打印-1。
...
既然 `A + nB` 是一个素数，有一点可以肯定的是 `A` 和 `B` 之间不应该存在任何公因数，也就是说 `A` 和 `B` 应该是互素数。
最好最有效的方法是使用**狄利克雷定理**。
**狄利克雷定理**说如果 `a` 和 `b` 是相对素数的正整数，那么算术序列 `a`、`a + b`、`a + 2b`、`a + 3b`……包含无限多的素数。
所以，首先检查 `A` 和 `B` 是否互素。
如果 `A` 和 `B` 是互素的，那么检查 `A + nB` 和 `B + nA` 的素性，其中 `n` = 1, 2, 3... 打印其中第一个 `N` 个素数。
