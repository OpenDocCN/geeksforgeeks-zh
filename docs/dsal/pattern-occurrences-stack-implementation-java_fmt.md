...
> 原文：[https://www.geeksforgeeks.org/pattern-occurrences-stack-implementation-java/](https://www.geeksforgeeks.org/pattern-occurrences-stack-implementation-java/)

假设我们有两个字符串：`Pattern` 和 `Text`。
**`Pattern`**：由独特的字符组成。
**`Text`**：由任意长度组成。

我们需要从 `Text` 中找到 `Pattern` 的数量，去掉文本中模式的每一个出现。
...
想法是使用堆栈数据结构。
1.  初始化一个 `pointer` 开始匹配模式中出现的 0 和 `counter` 到 0。
2.  检查模式和文本在当前索引中是否具有相同的字符。
3.  如果 `pointer` 指向模式的末尾，这意味着所有先前的字符都已按照递增的顺序找到，则将 `counter` 递增 1。
4.  如果不是，如果字符相同，继续将 `pointer` 增加 1。
5.  如果两个字符串中的字符不同，请检查字符是否与模式的第一个字符相同（即 `pointer = 0`）。
6.  如果是，将当前 `pointer` 中的剩余字符添加到堆栈的模式长度中，并检查它们是否存在，以便从堆栈中形成模式。另外，现在将 `pointer` 初始化为 1，因为我们已经检查过 `pointer = 0`（在步骤 5 中）。
7.  如果匹配，清空堆栈至空。否则，删除第一个字符，并继续添加子字符串的剩余部分，以检查后续步骤。
8.  如果任何添加到堆栈中的字符串与模式相匹配，将 `counter` 增加 1，并将 `pointer` 初始化 0。
9.  对文本长度的所有索引重复所有这些步骤。
10. 打印 `counter` 和事件。
11. 堆栈的基本任务是处理可能发生的未决操作。
...
先决条件：[Java 中的 Stack 类](https://www.geeksforgeeks.org/stack-class-in-java/)
...
1.  [Stack Java 文档。](https://docs.oracle.com/javase/7/docs/api/java/util/Stack.html)
2.  [Java 中的自定义 ArrayList 和类。](https://www.geeksforgeeks.org/custom-arraylist-java/)
3.  [更多 Stack 操作。](https://www.geeksforgeeks.org/data-structures/#Stack)
