...
> 原文: [https://www.geeksforgeeks.org/reversing-first-k-elements-queue/](https://www.geeksforgeeks.org/reversing-first-k-elements-queue/)

给定一个整数 `k` 和一个整数的[队列](https://www.geeksforgeeks.org/queue-data-structure/)，我们需要颠倒队列前 `k` 个元素的顺序，让其他元素保持相同的相对顺序。
仅允许在队列中执行以下标准操作。

*   `enqueue(x)`: 将项目 `x` 添加到队列的后面
*   `dequeue()`: 从队列前面移除项目
*   `size()`: 返回队列中的元素数。
*   `front()`: 查找前端项目。
...
想法是用一个辅助[栈](https://www.geeksforgeeks.org/stack-data-structure/)。
