...
让我们从一个算法开始，来解决给定问题的一个更简单的版本，其中所有边的权重都是 1。

*   选择一个随机的好节点，并从这一点执行 `BFS`，并在第一级停止，说 `s` 包含另一个好节点。
*   我们知道任意两个好节点之间的最小距离不能超过 `s`。所以我们再次随机选取一个之前没有选取过的好节点，再次执行 `BFS`。如果我们在 `s` 的距离内没有发现任何特殊节点，我们将终止搜索。如果我们这样做了，那么我们更新 `s` 的值，并且用随机获取的一些其他特殊节点重复该过程。
