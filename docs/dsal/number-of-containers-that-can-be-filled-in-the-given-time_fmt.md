...
> 原文：[https://www.geeksforgeeks.org/number-of-containers-that-can-be-filled-in-the-given-time/](https://www.geeksforgeeks.org/number-of-containers-that-can-be-filled-in-the-given-time/)
...
这个问题用[贪婪进场](https://www.geeksforgeeks.org/greedy-algorithms/)解决。以下是步骤：

*   容器装满后，液体从容器的两侧均匀地溢出，并充满下面的容器。
*   把这个容器金字塔看作一个矩阵。所以，`cont[i][j]` 是第 `i` 排第 `j` 个容器中的液体。
...
2.  如果 `cont[i][j]` 的值大于或等于 `1`（表示已加满），则计数增加 `1`，然后在 `cont[i+1][j]` 和 `cont[i+1][j+1]` 中，液体被倒出，它们的值分别增加 `(cont[i][j]-1)/2` 的值，因为液体被分成一半量。
