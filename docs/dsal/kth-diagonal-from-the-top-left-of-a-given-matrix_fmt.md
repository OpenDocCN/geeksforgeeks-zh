...
> 原文: [https://www.geeksforgeeks.org/kth-diagonal-from-the-top-left-of-a-given-matrix/](https://www.geeksforgeeks.org/kth-diagonal-from-the-top-left-of-a-given-matrix/)

给定一个 `N * N` 维度的平方矩阵 `M[][]`，任务是找到矩阵的 `K` 第条对角线，从**左上角**开始向**右下角**移动，并打印其内容。
...
**天真方法:**
解决这个问题最简单的方法是从给定矩阵的左上角到右下角生成所有对角线。生成第 `K` 条对角线后，打印其内容。
**时间复杂度:** `O(N^2)`
**辅助空间:** `O(N)`

**高效途径:**
以上途径可以通过避免遍历矩阵来优化。相反，找到 `K` 第对角线的边界，即左下和右上索引。获得这些索引后，只需遍历对角线的索引即可打印所需的对角线。
需要以下观察来找到对角线的位置:
...
*   如果 `(K–1)` < `N`，设置起始行，`startRow` 为 `K–1`，列，`startCol` 为 `0`。
