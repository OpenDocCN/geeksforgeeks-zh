...
> 原文：[https://www.geeksforgeeks.org/construct-bst-from-given-preorder-traversal-set-2/](https://www.geeksforgeeks.org/construct-bst-from-given-preorder-traversal-set-2/)
...
Java 是一种计算机语言，尤用于创建网站。以下是一个从给定先序遍历构造二叉搜索树（BST）的 Java 程序示例。
...
以下是一个从给定先序遍历构造二叉搜索树（BST）的 Python 3 程序示例。
...
以下是一个从给定先序遍历构造二叉搜索树（BST）的 C# 程序示例。
...
时间复杂度为 `O(n)`。从第一眼看，复杂程度更高。如果我们仔细观察，我们可以观察到每个项目只被推送和弹出一次。因此，在 `constructTree()` 的主循环中最多执行 `2n` 次推/弹出操作。因此，时间复杂度为 `O(n)`。
