...
> 原文: [https://www.geeksforgeeks.org/check-given-matrix-hankel-not/](https://www.geeksforgeeks.org/check-given-matrix-hankel-not/)
...
因此，为了检查给定的矩阵是否是 Hankel 矩阵，我们需要检查每个 `m[i][j] == a_{i+j}`。现在，`a_{i+j}` 可以定义为：
...
以下是一个 JavaScript 程序，用于检查给定的矩阵是否为汉克尔矩阵。
