...
> 原文：[https://www.geeksforgeeks.org/find-the-index-in-a-circular-array-from-which-prefix-sum-is-always-non-negative/](https://www.geeksforgeeks.org/find-the-index-in-a-circular-array-from-which-prefix-sum-is-always-non-negative/)
...
*   如果数组元素的[和为负](https://www.geeksforgeeks.org/program-find-sum-elements-given-array/)，则不存在从该点开始前缀和为非负的索引。
...
*   初始化一个变量，比如说 `sum` 为 `0`，存储数组元素的[和](https://www.geeksforgeeks.org/program-find-sum-elements-given-array/)。
*   初始化一个变量，比如说 `in` 为 `0`，存储循环遍历的起始索引。
*   初始化一个变量，说 `min` 为 [`INT_MAX`](https://www.geeksforgeeks.org/int_max-int_min-cc-applications/)，存储数组 `arr[]` 的最小前缀和。
*   [遍历给定数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)并执行以下步骤：
    *   将 `sum` 的值更新为 `sum` 和当前元素 `arr[i]` 的总和。
    *   如果 `sum` 的值小于 `min`，则将 `min` 更新为 `sum`，`in` 为 `(i + 1)`。
*   如果数组的[和为负](https://www.geeksforgeeks.org/program-find-sum-elements-given-array/)，则打印 `-1`。否则，打印 `(in % N)` 的值作为最终可能的索引。
