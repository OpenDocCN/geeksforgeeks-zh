...
> 原文: [https://www.geeksforgeeks.org/string-k-distinct-characters-no-characters-adjacent/](https://www.geeksforgeeks.org/string-k-distinct-characters-no-characters-adjacent/)

给定 `N` 和 `K`，打印一个有 `N` 个字符的字符串。字符串应该正好有 `k` 个不同的字符，并且没有相邻的位置相同。
