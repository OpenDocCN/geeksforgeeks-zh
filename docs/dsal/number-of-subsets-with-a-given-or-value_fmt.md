...
> 原文: [https://www.geeksforgeeks.org/number-of-subsets-with-a-given-or-value/](https://www.geeksforgeeks.org/number-of-subsets-with-a-given-or-value/)
...
上述递归关系可以定义为子数组 `arr[i…N-1]` 的子集的数量，使得用 `curr_or` 对它们进行 `or` 运算将产生所需的 `OR` 值。

递归关系是合理的，因为只有两条路径：要么取当前元素，用 `curr_or` 进行 `OR` 运算，要么忽略，继续前进。
