...
> 原文: [https://www.geeksforgeeks.org/check-given-array-of-size-n-can-represent-bst-of-n-levels-or-not/](https://www.geeksforgeeks.org/check-given-array-of-size-n-can-represent-bst-of-n-levels-or-not/)
...
3. 对于每个元素，检查
    ...
    b. 否则如果 `(arr[i] < arr[i-1] && arr[i] > min && arr[i] < max)`，更新 `max = arr[i]`
    c. 如果以上两个条件都不成立，则元素不会插入
