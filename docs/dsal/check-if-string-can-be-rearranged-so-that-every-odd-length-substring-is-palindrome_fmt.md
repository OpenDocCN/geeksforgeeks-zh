...
> 原文: [https://www.geeksforgeeks.org/check-if-string-can-be-rearranged-so-that-every-odd-length-substring-is-palindrome/](https://www.geeksforgeeks.org/check-if-string-can-be-rearranged-so-that-every-odd-length-substring-is-palindrome/)

给定一个字符串 `S`。任务是检查是否有可能重新排列字符串，使得每个奇数长度的子字符串都是回文。
