...
> 原文: [https://www.geeksforgeeks.org/compute-ncr-p-set-3-using-fermat-little-theorem/](https://www.geeksforgeeks.org/compute-ncr-p-set-3-using-fermat-little-theorem/)
...
[计算 <sup>n</sup>C<sub>r</sub> % p | 集合 2 (卢卡斯定理)](https://www.geeksforgeeks.org/compute-ncr-p-set-2-lucas-theorem/)
本文讨论基于费马定理的解法。

**背景: 费马小定理与模逆**

费马小定理说明如果 `p` 是素数，那么对于任意整数 `a`，数 `a^p – a` 是 `p` 的整数倍，在模算术的记数法中，这表示为:
`a^p ≡ a (mod p)`

如果 `a` 不能被 `p` 整除，费马小定理相当于陈述 `a^(p–1) – 1` 是 `p` 的整数倍，即
`a^(p-1) ≡ 1 (mod p)`
如果我们把两边乘以 `a^(-1)` ，就得到。
`a^(p-2) ≡ a^(-1) (mod p)`
所以我们可以找到[模逆](https://www.geeksforgeeks.org/multiplicative-inverse-under-modulo-m/)为 `p-2`。
