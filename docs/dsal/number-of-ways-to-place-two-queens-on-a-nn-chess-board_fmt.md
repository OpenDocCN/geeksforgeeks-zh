...
> 原文: [https://www.geeksforgeeks.org/number-of-ways-to-place-two-queens-on-a-nn-chess-board/](https://www.geeksforgeeks.org/number-of-ways-to-place-two-queens-on-a-nn-chess-board/)
...
如果我们不把皇后放在最后一行和最后一列，那么答案将只是放在`(N-1)*(N-1)`棋盘上的位置数，而如果我们放在最后一列和最后一行，那么皇后的可能位置将是 `2 * N-1`，并在 `3 *(N-1)`个位置进行攻击。因此，另一个皇后的可能位置将是 `N^2–3 *(N-1)–1`。最后，有`(N-1)*(N-2)`种组合，其中两个皇后都在最后一行和最后一列。因此，重复关系将是–

```
Q(N) = Q(N-1) + (N^2-3*(N-1)-1)-(N-1)*(N-2)
