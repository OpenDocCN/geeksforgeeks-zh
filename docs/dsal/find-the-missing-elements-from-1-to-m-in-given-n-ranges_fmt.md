...
> 原文：[https://www.geeksforgeeks.org/find-the-missing-elements-from-1-to-m-in-given-n-ranges/](https://www.geeksforgeeks.org/find-the-missing-elements-from-1-to-m-in-given-n-ranges/)
...
```php
<?php
// PHP program to find missing elements
// from given Ranges
...
// Store ranges in vector of pair
$ranges = array();
array_push($ranges, array(1, 2));
array_push($ranges, array(4, 5));

findMissingNumber($ranges, $M);
?>
```

**时间复杂度**：`O(n * log(n))`，其中 `n` 为向量的长度。
