...
> 原文: [https://www.geeksforgeeks.org/position-of-robot-after-given-movements/](https://www.geeksforgeeks.org/position-of-robot-after-given-movements/)
...
> **输入:** `Move = "uddlruuuduuruduulldrrrr"`
> **输出:** `(2, 3)`

**来源:** [高盛面试经验|第 36 集](https://www.geeksforgeeks.org/goldman-sachs-interview-experience-set-26-experienced/)。

**进场:**
分别计算向上运动(`U`)、向下运动(`D`)、向左运动(`L`)和向右运动(`R`)的次数为 `countUp`、`countDown`、`countLeft` 和 `countRight`。最终 `x` 坐标为 `(countRight - countLeft)`，`y` 坐标为 `(countUp - countDown)`。
