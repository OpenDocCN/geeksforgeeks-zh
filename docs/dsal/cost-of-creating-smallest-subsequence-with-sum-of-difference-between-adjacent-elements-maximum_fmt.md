...
> 原文: [https://www.geeksforgeeks.org/cost-of-creating-smallest-subsequence-with-sum-of-difference-between-adjacent-elements-maximum/](https://www.geeksforgeeks.org/cost-of-creating-smallest-subsequence-with-sum-of-difference-between-adjacent-elements-maximum/)
...
2.  然后
> `| b–a | + | c–b | = | a–c | = 10`
3.  这里我们可以去掉中间的元素来减少原始序列的大小。
4.  这样，通过从序列中移除中间元素来减小数组的大小不会影响总和，并且还会减小序列的长度。
5.  存储集合中所有移除的元素。添加移除元素的成本。最后，通过排除移除的元素来计算总和序列。
6.  然后打印后续要素和发生成本的总和。
