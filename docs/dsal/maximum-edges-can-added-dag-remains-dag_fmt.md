...
给我们一个 DAG，我们需要找到可以添加到该 DAG 的最大边数，此后新图仍然是 DAG，这意味着经过重整的图应具有最大边数，即使添加单个边也将创建一个在图中循环。
...
怎么说呢，不可能再增加边了？原因是我们从左到右添加了所有可能的边，如果要添加更多的边，我们需要从右到左做边，但是从右到左添加边我们肯定会创建一个循环，因为它的对应部分是从左到右边已经被添加到图形中，创建循环不是我们所需要的。
