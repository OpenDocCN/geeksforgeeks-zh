...
> 原文: [https://www.geeksforgeeks.org/implementation-of-array-class-in-javascript/](https://www.geeksforgeeks.org/implementation-of-array-class-in-javascript/)
...
在上例中，创建一个类`Array`，它包含两个属性，即`length`和`data`，其中`length`将存储数组的长度，而`data`是一个用于存储元素的对象。
...
在上例中，`item`变量将存储来自`data`对象的最后一个元素，并执行最后一个元素的删除，然后，它将长度减少 1 并返回该对象。
...
这个函数接受两个参数`item`和`index`。`index`号表示要插入数据的位置，`item`是要在索引处插入的值。

### `deleteAt(index)`

该函数用于删除数据对象中给定索引或属性处的元素。
...
在上述功能中，使用循环到达索引处直至结束，并在`index`处复制下一个元素，循环结束时存在最后一个元素的两个副本，通过`delete`操作符删除最后一个元素。
