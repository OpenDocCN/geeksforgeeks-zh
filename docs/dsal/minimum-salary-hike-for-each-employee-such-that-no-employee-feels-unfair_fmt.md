...
> 原文：[https://www.geeksforgeeks.org/minimum-salary-hike-for-each-employee-such-that-no-employee-feels-unfair/](https://www.geeksforgeeks.org/minimum-salary-hike-for-each-employee-such-that-no-employee-feels-unfair/)

一个公司有 `N` 个员工，每个员工都有一些评分。员工会根据他们的评分获得加薪，即评分较高的员工将获得更高的加薪。员工只知道其邻居的提价和评级，即员工的左侧和右侧。
给定一个表示 `N` 员工评分的正整数数组 `arr`，任务是找到每个员工应该提高的最低加薪幅度，这样就不会有员工觉得不公平。
...
这个问题可以用贪婪算法解决。由于员工只知道他们邻居的 hike 和 rating，因此以下条件之一将适用于给定的 rating：

1.  **类型 1：** `H_{i-1} >= H_i` 且 `H_i <= H_{i+1}`
2.  **类型 2：** `H_{i-1} < H_i` 且 `H_i <= H_{i+1}`
3.  **类型 3：** `H_{i-1} >= H_i` 且 `H_i > H_{i+1}`
4.  **类型 4：** `H_{i-1} < H_i` 且 `H_i > H_{i+1}`

对于每个员工，基于上述条件，将每个员工的加薪设置为：

*   **类型 1：** 设置 hike 至 `1`。
*   **类型 2：** 通过 `H_{i-1} + 1` 提高他的步幅。
*   **类型 3：** 通过 `H_{i+1} + 1` 提高他的步幅。
*   **类型 4：** 提高 `max(H_{i-1}, H_{i+1}) + 1`。
