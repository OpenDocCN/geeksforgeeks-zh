# Python 中反转字符串的 5 种不同方式

> 原文: [https://www.geeksforgeeks.org/reverse-string-python-5-different-ways/](https://www.geeksforgeeks.org/reverse-string-python-5-different-ways/)
...
**说明:** 在上面的代码中，我们调用了一个函数来反转一个字符串，这个函数迭代到每一个元素，并智能地在开头连接每个字符，从而得到反转后的字符串。
...
**解释:** 创建一个空栈。一个接一个的字符串被推入堆栈。堆栈中的所有字符被逐个弹出，并放回字符串。
...
本文由 `【曼吉特·辛格】` 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 `contribute.geeksforgeeks.org` 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
