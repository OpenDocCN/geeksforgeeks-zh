...
> 原文: [https://www.geeksforgeeks.org/find-the-number-of-rectangles-of-size-21-which-can-be-placed-inside-a-rectangle-of-size-nm/](https://www.geeksforgeeks.org/find-the-number-of-rectangles-of-size-21-which-can-be-placed-inside-a-rectangle-of-size-nm/)

给定两个整数 `n`、`m`。找出可以放在 `n*m` 大小的矩形内的 `2*1` 大小的矩形数量。
...
3.  如果两者都是奇数，那么用小矩形覆盖 `N-1` 行板，并将 `floor(M/2)` 小矩形放在最后一行。在最坏的情况下(`N` 和 `M` 是奇数)，一个单元仍然未被覆盖。
