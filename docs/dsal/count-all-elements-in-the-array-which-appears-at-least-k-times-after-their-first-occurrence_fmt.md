...
> 原文: [https://www.geeksforgeeks.org/count-all-elements-in-the-array-which-appears-at-least-k-times-after-their-first-occurrence/](https://www.geeksforgeeks.org/count-all-elements-in-the-array-which-appears-at-least-k-times-after-their-first-occurrence/)

给定一个由 `N` 个整数元素和一个整数 `K` 组成的数组。任务是计算所有不同的 `arr[i]`，使得 `arr[i]` 在索引范围 `i + 1` 到 `n–1` 中至少出现 `K` 次。
...
从 `i = 0` 到 `n-1` 开始，计算 `arr[i]` 在 `i+1` 到 `n-1` 范围内的出现次数。如果计数大于或等于 `K`，将结果增加 1。创建一个散列数组以避免重复。
...
声明另一个哈希映射来存储所有元素的出现，并从 `n–1` 到 `0` 开始。如果 `出现[arr[i]] ≥ k`，则将计数增加 1，否则将 `arr[i]` 的出现增加 1。
