...
从左到右遍历给定的数组`arr`。 遍历时，维护数组中非零元素的数量。 让计数为`count`。 对于每个非零元素`arr[i]`，将其放在`arr[count]`处，并递增`count`。 完全遍历之后，所有非零元素都已移至前端，并且`count`被设置为第一个0的索引。现在我们要做的就是运行一个循环，使所有元素从`count`到结束都为零。
