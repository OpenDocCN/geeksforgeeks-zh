...
> 原文：[https://www.geeksforgeeks.org/program-to-find-count-of-numbers-having-odd-number-of-divisors-in-given-range/](https://www.geeksforgeeks.org/program-to-find-count-of-numbers-having-odd-number-of-divisors-in-given-range/)
...
**更好的方法:**
一个数可以用它的素因子的乘积，加上适当的幂来表示。这些幂可以用来得到一个整数的因子数。如果数字是 `num`，可以表示为 `(a^P1)*(b^p2)*(c^P3)`，那么 `num` 的因子个数是 `(p1 + 1) * (p2 + 1) * (p3 + 1)`。
