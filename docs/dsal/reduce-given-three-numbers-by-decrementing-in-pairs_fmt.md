...
> 原文: [https://www.geeksforgeeks.org/reduce-given-three-numbers-by-decrementing-in-pairs/](https://www.geeksforgeeks.org/reduce-given-three-numbers-by-decrementing-in-pairs/)
...
思路是在以下条件的基础上对给定的数进行降序排列，求最大运算数:
...
*   首先，使 `A` 和 `B` 相等。为此，选择 `A` 和 `C` 执行 `(A - B)` 操作。将更新后的值命名为 `A1`、`B1`、`C1`。`A1`、`B1`、`C1` 的数值为:
    > `A1 = A - (A - B)`
    > `B1 = B`
    > `C1 = C - (A - B)`
*   执行的操作数 = `(A - B)`。
*   `A1` 和 `B1` 相等。所以，选择 `A1` 和 `B1` 对 `(A1 - C1)` 进行操作。
*   让 `A2`、`B2`、`C2` 成为上述操作后 `A`、`B`、`C` 的更新值。`A2`、`B2`、`C2` 的值将相同，即:
...
*   当 `A2 = B2 = C2` 时，会出现两种情况:
    *   **`A2`、`B2`、`C2` 为偶数:** 每对 `(A2, B2)`、`(B2, C2)` 和 `(C2, A2)` 进行 3 次运算，则 `A2`、`B2`、`C2` 的计数减少 2。
        让 `A2 = B2 = C2 = 4`。设可执行的运算为 `X`，那么，`X = (4 + 4 + 4) / 2 = 6`。因此，`X` 的值可以概括为:
        > `X = (A2 + B2 + C2) / 2 = (3 * (C - A + B)) / 2`
    *   **`A2`、`B2`、`C2` 为奇数:** 对 `(A2, B2)`、`(B2, C2)` 和 `(C2, A2)` 的每组 3 次运算，`A2`、`B2`、`C2` 的计数减少 2，最终值 `A2`、`B2` 和 `C2` 分别达到 1、1 和 1。这里可以执行一个附加操作。
        让 `A2 = B2 = C2 = 5`。执行 6 次运算后，`A2 = B2 = C2 = 1`。这里可以再进行一次操作。因此，可以执行的操作总数为 7 (6+1)。让可执行的运算为 `Y`，那么，`Y = floor((5 + 5 + 5) / 2) = 7`。因此，`Y` 的值可以概括为:
        > `Y = (A2 + B2 + C2) / 2 = (3 * (C - A + B)) / 2`
