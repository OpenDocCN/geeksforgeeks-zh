...
> 原文: [https://www.geeksforgeeks.org/avl-trees-containing-a-parent-node-pointer/](https://www.geeksforgeeks.org/avl-trees-containing-a-parent-node-pointer/)

`AVL` 树是自平衡的 `二叉查找树(BST)`，其中左右子树的高度差对于所有节点不能超过一。`AVL` 树中的插入和删除已经在[上一篇文章](https://www.geeksforgeeks.org/avl-tree-set-1-insertion/)中讨论过。在本文中，插入、搜索和删除操作在 `AVL` 树上进行讨论，这些树的结构中也有一个父指针。
