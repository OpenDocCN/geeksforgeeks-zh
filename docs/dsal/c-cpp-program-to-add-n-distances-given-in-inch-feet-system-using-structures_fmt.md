...
> 原文: [https://www.geeksforgeeks.org/c-cpp-program-to-add-n-distances-given-in-inch-feet-system-using-structures/](https://www.geeksforgeeks.org/c-cpp-program-to-add-n-distances-given-in-inch-feet-system-using-structures/)

给定一个包含英寸-英尺系统的 `N` 距离的数组 `arr[]`，使得数组的每个元素以 `{英寸，英尺}` 的形式表示一个距离。任务是使用[结构](https://www.geeksforgeeks.org/structures-c/)添加所有 `N` 英寸英尺距离。
...
2.  如果所有英寸的总和（比如 `inch_sum`）大于 12，则需要将 `inch_sum` 转换为英尺，因为

```
1 feet = 12 inches
```

因此将 `inch_sum` 更新为 `inch_sum % 12`。然后求 `N` 距离的所有英尺（比如 `feet_sum`）的总和，并将 `inch_sum / 12` 加到这个总和上。

3.  分别打印 `feet_sum` 和 `inch_sum`。
