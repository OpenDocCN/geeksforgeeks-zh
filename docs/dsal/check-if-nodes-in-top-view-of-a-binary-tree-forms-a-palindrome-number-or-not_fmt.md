...
> 原文: [https://www.geeksforgeeks.org/check-if-nodes-in-top-view-of-a-binary-tree-forms-a-palindrome-number-or-not/](https://www.geeksforgeeks.org/check-if-nodes-in-top-view-of-a-binary-tree-forms-a-palindrome-number-or-not/)
...
要解决给定的问题，思路是使用本文[中讨论的方法](https://www.geeksforgeeks.org/print-nodes-top-view-binary-tree/)找到给定的 `二叉树` 的俯视图，并将其存储在数组中，比如 `arr[]`。现在，[检查数组 `arr[]` 是否回文](https://www.geeksforgeeks.org/program-to-check-if-an-array-is-palindrome-or-not/)。如果发现为真，则打印 `是` 否则打印 `否`。
