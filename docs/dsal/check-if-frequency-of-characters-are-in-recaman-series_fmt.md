...
> 原文: [https://www.geeksforgeeks.org/check-if-frequency-of-characters-are-in-recaman-series/](https://www.geeksforgeeks.org/check-if-frequency-of-characters-are-in-recaman-series/)

给定一串小写字母。任务是检查在以任何可能的方式排列后，这个字符串中字母的频率是否形成了雷卡曼序列（不包括第一个术语）。
按顺序打印“是”，否则输出“否”。
[雷卡曼序列](https://www.geeksforgeeks.org/recamans-sequence/)的几个起始术语是:

> 0, 1, 3, 6, 2, 7, 13, 20, 12, 21, 11, 22, 10, 23, 9, 24, 8 ….

**注:** 因为是零，所以不考虑雷卡曼序列的第一项。
**例:**
...
*   现在，制作一个数组，并在其中插入雷卡曼序列的前 `N` 个元素。
*   现在，遍历数组并检查数组的元素是否作为键出现在地图中（不包括对零的检查）。
