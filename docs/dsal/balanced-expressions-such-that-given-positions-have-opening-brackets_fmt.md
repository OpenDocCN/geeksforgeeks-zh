...
> 原文: [https://www.geeksforgeeks.org/balanced-expressions-such-that-given-positions-have-opening-brackets/](https://www.geeksforgeeks.org/balanced-expressions-such-that-given-positions-have-opening-brackets/)

给定整数 `n` 和位置数组 `position[]` (1<= `position[i]` <= 2n)，求长度为 `2n` 的合法括号表达式的数量，使得在给定位置上是左括号。
...
这个问题可以通过[**动态编程**](https://www.geeksforgeeks.org/dynamic-programming/)解决。

设 `DP[i, j]` 为填充前 `i` 个位置的有效方式数，使得类型为“[”的括号比类型为“]”的括号多 `j` 个。有效的方式意味着它是匹配括号表达式的前缀，并且强制“[”括号被强制的位置已经被满足。很容易看出 `DP[2N, 0]` 就是最终答案。

DP 的基本情况是，`DP[0, 0] = 1`。我们需要在第一个位置填充一个“[”括号，只有一种方法可以做到。

如果位置 `i` 有一个可以用散列数组标记的左括号序列，那么循环出现如下:
