...
> 原文: [https://www.geeksforgeeks.org/check-given-two-number-friendly-pair-not/](https://www.geeksforgeeks.org/check-given-two-number-friendly-pair-not/)

给定两个正整数 `N`、`M`。任务是检查 `N` 和 `M` 是否是[友好的一对](https://en.wikipedia.org/wiki/Friendly_number)。

> 在数论中，友好对是两个具有共同丰度指数的数，一个数的除数之和与数本身的比值，即 `？(n)/n`。所以，两个数字 `n` 和 `m` 是友好数字如果 `？(n)/n =？(m)/m`。哪里 `？(n)` 是 `n` 的除数之和。
...
想法是找到 `n` 和 `m` 的除数的[和](https://www.geeksforgeeks.org/sum-factors-number/)，并检查 `n` 和 `m` 的丰度指数，我们将找到 `n` 和 `m` 的[最大公约数](https://www.geeksforgeeks.org/c-program-find-gcd-hcf-two-numbers/)及其除数之和。并通过检查分子和分母是否相等来检查 `n` 和 `m` 的丰度指数的简化形式是否相等。为了找到简化形式，我们将分子和分母除以 `GCD`。
