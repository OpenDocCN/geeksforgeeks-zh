...
在这段代码中，当访问每个节点时，该节点的级别设置为其父节点级别的增量，即 `level[子] = 级别[父] + 1`。每个节点的级别就是这样确定的。根节点位于树中的零级。
...
`等级[3] = 等级[1] + 1`，即 `等级[3] = 1 + 1 = 2`。
`等级[4] = 等级[1] + 1`，即 `等级[4] = 1 + 1 = 2`。
`等级[5] = 等级[1] + 1`，即 `等级[5] = 1 + 1 = 2`。
`等级[6] = 等级[2] + 1`，即 `等级[6] = 1 + 1 = 2`。
