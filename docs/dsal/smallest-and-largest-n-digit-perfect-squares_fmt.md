...
**方式:**从 `N = 1` 开始增加 `N` 的数值，系列会像 9、81、961、9801、…..
为最大的 `N` 位数完美平方，其第 `N` 项为 `pow(ceil(sqrt(pow(10, N))) - 1, 2)`。
和 1，16，100，1024，…..为最小的 `N` 位数完美平方，其第 `N` 项为 `pow(ceil(sqrt(pow(10, N-1))), 2)`。
以下是上述方法的实施:
