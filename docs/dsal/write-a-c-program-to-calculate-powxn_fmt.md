...
> 原文: [https://www.geeksforgeeks.org/write-a-c-program-to-calculate-powxn/](https://www.geeksforgeeks.org/write-a-c-program-to-calculate-powxn/)

给定两个整数 `x` 和 `n`，写一个函数计算 `x^n`。我们可以假设 `x` 和 `n` 很小，溢出不会发生。
...
**算法范式:** 分而治之。
上述函数只需计算一次 `power(x, y/2)` 并存储即可优化为 O(logn)。
...
对于新手来说，这是一个非常高效的方法，因为新手觉得很难理解迭代递归调用。这个方法也需要 `T(n)= O(log(n))` 的复杂度。
...
[为 `pow(x，y)` 写一个迭代的 O(Log y)函数](https://www.geeksforgeeks.org/write-an-iterative-olog-y-function-for-powx-y/)
[模运算中的幂](https://www.geeksforgeeks.org/modular-exponentiation-power-in-modular-arithmetic/)
