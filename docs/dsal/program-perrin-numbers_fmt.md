...
佩兰数是以下整数序列中的数字：3，0，2，3，2，5，5，7，10，12，17，22，29，39……

在数学术语中，佩兰数的序列 `p(n)` 由递推关系定义：
```
...
我们可以类似于斐波那契数的[方法 5 的实现](https://www.geeksforgeeks.org/program-for-nth-fibonacci-number/)来实现这个方法。由于我们可以在 `O(Log n)` 中计算一个常数矩阵的 n 次幂，所以该方法的时间复杂度为 `O(Log n)`。

## 应用

n 顶点循环图中不同的[极大独立集](https://en.wikipedia.org/wiki/Maximal_independent_set)的个数按 n 的第 n 个 Perrin 数计数（n > 1）。
