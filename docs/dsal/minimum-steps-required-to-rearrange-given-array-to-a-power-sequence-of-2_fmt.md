...
> 原文: [https://www.geeksforgeeks.org/minimum-steps-required-to-rearrange-given-array-to-a-power-sequence-of-2/](https://www.geeksforgeeks.org/minimum-steps-required-to-rearrange-given-array-to-a-power-sequence-of-2/)

给定一个由 `N` 个正整数组成的数组 `arr[]`，任务是通过以下操作找到将给定的整数数组变成 2 的幂序列所需的最小步骤：

*   给定数组重新排序。不算一步。
*   对于每一步，从数组中选择任意索引 `i`，并将 `arr[i]` 更改为 `arr[i] - 1` 或 `arr[i] + 1`。

一个序列被称为 2 的幂序列，如果对于每个 `i` 索引 (`0 ≤ i ≤ N-1`)，`arr[i] = 2^i`，其中 `N` 是给定数组的长度。
...
为了解决给定的问题，思路是按照升序对数组进行排序，对于排序后的数组的每个 `i` 索引，计算 `arr[i]` 和 `2^i` 之间的绝对差值。绝对差异的总和给了我们所需的答案。
