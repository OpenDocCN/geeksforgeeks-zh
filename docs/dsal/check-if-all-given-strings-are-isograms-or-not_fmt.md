...
> 原文: [https://www.geeksforgeeks.org/check-if-all-given-strings-are-isograms-or-not/](https://www.geeksforgeeks.org/check-if-all-given-strings-are-isograms-or-not/)

给定一个包含 `N` 个字符串的数组 `arr`，任务是检查所有字符串是否都是[等值线图](https://www.geeksforgeeks.org/check-string-isogram-not/)。如果是，打印 `是`，否则打印 `否`。
