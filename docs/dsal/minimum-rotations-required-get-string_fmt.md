...
> 原文：[https://www.geeksforgeeks.org/minimum-rotations-required-get-string/](https://www.geeksforgeeks.org/minimum-rotations-required-get-string/)
...
**第一步：** 初始化 `result = 0`（这里的 `result` 是旋转计数）。
...
// string.
    ...
    function findRotations( str) {
        ...
        for (var i = 1; i <= n; i++) {
            ...
            var substring = tmp.substring(i , i + str.length);
