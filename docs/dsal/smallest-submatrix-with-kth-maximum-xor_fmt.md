...
给定尺寸为`N×M`的矩阵`m[][]`和整数`K`，对于矩阵的每个索引计算`XOR(i, j)`，它等于从索引`(1, 1)`到`(i, j)`的子矩阵所有元素的按位`XOR`。任务是找到具有第`K`大`XOR(i, j)`值的子矩阵。如果存在多个此类子矩阵，则打印最小的子矩阵。
...
**方法**：想法是使用动态规划查找`XOR(i, j)`。

*   根据`xor[i][j] = xor[i-1][j] ^ xor[i][j-1] ^ xor[i-1][j-1] ^ m[i][j]`，计算按位`XOR(i, j)`。
*   将针对各个索引`(i, j)`获得的`XOR(i, j)`值存储在映射中。
*   使用大小为`K`的最小堆，找到所有`XOR(i, j)`值的第`K`个最大值。
*   查找最小索引`(i, j)`，对于该索引，`XOR(i, j)`等于第`K`个最大值。以上步骤使用映射。
