...
> 原文: [https://www.geeksforgeeks.org/program-for-nth-fibonacci-number/](https://www.geeksforgeeks.org/program-for-nth-fibonacci-number/)
...
***时间复杂度:*** `T(n) = T(n-1) + T(n-2)`，这接近于 `2^n`。

如果原始递归树被实现，那么这将是指数级时间复杂度，但是现在 `n` 次调用递归函数。
