...
> 原文：[https://www.geeksforgeeks.org/trapping-rain-water-in-a-matrix/](https://www.geeksforgeeks.org/trapping-rain-water-in-a-matrix/)

给定一个由正整数组成的维度为 **M*N** 的[矩阵](https://www.geeksforgeeks.org/matrix/) `arr[][]`，其中 `arr[i][j]` 代表每个单元格的高度，任务是找出雨后困在矩阵中的水的总体积。
...
> **解释：**
> 雨水可以通过以下方式被困：
> 1.  单元格 `{(0, 0), (0, 1), (0, 2), (1, 0), (1, 2), (2, 0), (2, 1), (2, 2)}` 捕捉 0 单位体积的雨水，因为所有的水都从矩阵中流出，因为单元格在边界上。
> 2.  单元格 `(2, 2)` 在单元格 `{(0, 1), (1, 0), (1, 2) 和 (2, 1)}` 之间截留 1 单位体积的雨水。
>
> 因此，总共有 1 单位体积的雨水被截留在矩阵内。
>
...
*   初始化两个变量，比如 `ans` 为 `0` 和 `maxHeight` 为 `0`，分别存储 `PQ` 中所有单元格的总体积和最大高度。
*   重复[直到 `PQ` 不为空](https://www.geeksforgeeks.org/priority_queueempty-priority_queuesize-c-stl/)并执行以下步骤：
    *   [将 `PQ`](https://www.geeksforgeeks.org/priority_queuetop-c-stl/) 的顶部节点存储在一个变量中，比如 `front`，[擦除 `PQ` 的顶部元素](https://www.geeksforgeeks.org/priority_queuepush-priority_queuepop-c-stl/)。
    *   将 `maxHeight` 的值更新为 `maxHeight` 和 `front.height` 的最大值。
    *   现在，[遍历当前单元格的所有相邻节点](https://www.geeksforgeeks.org/find-all-reachable-nodes-from-every-node-present-in-a-given-set/)(`front.x`, `front.y`) 并执行以下操作：
        *   如果相邻单元格有效，即该单元格未超出界限且尚未被访问，则将相邻单元格的值推入 `PQ`。
        *   如果相邻单元格的高度小于 `maxHeight`，则按 `maxHeight` 与相邻单元格的高度之差增加 `ans`。
*   最后，完成上述步骤后，打印 `ans` 的值，作为雨后积水的结果。
