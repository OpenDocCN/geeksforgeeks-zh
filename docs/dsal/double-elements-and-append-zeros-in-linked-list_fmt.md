...
public class GfG {
    // Recursive function to double one of two elements
    // and make next one as 0, which are equal before 0
    public static void changeTwoBefore0(Node head) {
        // there should be atleast three elements to perform required operation
        if (head == null || head.next == null || head.next.next == null)
            return;

        // when two continuous elements are same
        if ((head.data == head.next.data) && (head.next.next.data == 0)) {
            int temp = head.data;
            head.data = 2 * temp;
            head.next.data = 0;
        ...
        // recursive call to changeTwoBefore0 for next element
        changeTwoBefore0(head);
    }
```
...
```java
    ...
    public static Node appendZero(Node head) {
        ...
        // We check until original tail
        while (curr != origTail) {
            // If current data is 0, append after tail and update tail.
            if (curr.data == 0) {
                tail.next = curr;
                tail = curr;
                prev.next = curr.next;
        ...
        // Finally making sure that linked list is null terminated.
        tail.next = null;
