...
是考虑每个元素，并检查所有行中是否存在该元素。 如果存在，请打印。

### 更好的解决方案

是对矩阵中的所有行进行排序，并使用与[在这里](https://www.geeksforgeeks.org/find-common-element-rows-row-wise-sorted-matrix/)类似的方法。 排序将花费`O(mnlogn)`时间，而查找公用元素将花费`O(mn)`时间。 因此，此解决方案的总体时间复杂度为`O(mnlogn)`。
