# 在数组 D 上执行给定操作后可到达的数组元素数量

> 原文: [https://www.geeksforgeeks.org/number-of-elements-from-the-array-which-are-reachable-after-performing-given-operations-on-d/](https://www.geeksforgeeks.org/number-of-elements-from-the-array-which-are-reachable-after-performing-given-operations-on-d/)
...
**方法:** 这个问题可以用[丢番图方程](https://www.geeksforgeeks.org/linear-diophantine-equations/)的一个性质来解决，让我们想从数组中得到的整数为 `x`。如果我们从 `D` 开始，可以对其进行任意次数的加减 `A` 或 `B`，那就意味着我们需要找出下面的方程是否有整数解。
