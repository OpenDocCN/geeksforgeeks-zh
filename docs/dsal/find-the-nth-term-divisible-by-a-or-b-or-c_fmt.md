...
给定四个整数 `a`、`b`、`c` 和 `N`。任务是找到可被 `a`、`b` 或 `c` 整除的 `N` 第项。
...
一个简单的方法是遍历所有术语，从 `1` 开始，直到我们找到所需的 `N` th 术语，该术语可被 `a`、`b` 或 `c` 整除。该解决方案的时间复杂度为 `O(N)`。

## 高效途径:

思路是用[二分搜索法](https://www.geeksforgeeks.org/binary-search/)。在这里，我们可以通过使用以下公式来计算从 `1` 到 `num` 有多少个数可以被 `a`、`b` 或 `c` 整除：`(num/a)+(num/b)+(num/c)–(num/LCM(a，b))–(num/LCM(b，c))–(num/LCM(a，c)) + (num / lcm(a, lcm(b, c)))`。
