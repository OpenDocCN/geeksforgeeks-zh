...
为了解决给定的问题，[计算给定图形](https://www.geeksforgeeks.org/program-to-count-number-of-connected-components-in-an-undirected-graph/)中存在的连通组件的数量。令计数为`C`。观察到，如果`C`大于`K`，那么由于连接零件的数量只会增加，因此不可能去除边会产生`K`连接的零件。否则，答案将始终存在。
...
class Graph:
    ...
    def DFS(self, s, visited):
        ...
        while (len(stack)):
            ...
            s = stack[-1]
            stack.pop()
            # Traverse adjacent vertices of the popped vertex s
            for node in self.adj[s]:
                if (not visited[node]):
                    # If adjacent is unvisited, push it to the stack
                    visited[node] = True
                    stack.append(node)

# Function to return the count edges removed
