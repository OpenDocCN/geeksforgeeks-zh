...
> 原文：[https://www.geeksforgeeks.org/split-an-array-containing-n-elements-into-k-sets-of-distinct-elements/](https://www.geeksforgeeks.org/split-an-array-containing-n-elements-into-k-sets-of-distinct-elements/)
...
为了解决这个问题，我们使用 `map` 来存储每个元素的频率。如果任何元素的频率超过 `K`，打印 `-1`。维护另一个 `map` 来存储每个频率的集合。如果没有任何元素的频率大于 `K`，则打印所有相应频率的集合作为所需集合。
