...
> 原文：[https://www.geeksforgeeks.org/check-if-array-can-be-sorted-by-swapping-pairs-with-gcd-of-set-bits-count-equal-to-that-of-the-smallest-array-element/](https://www.geeksforgeeks.org/check-if-array-can-be-sorted-by-swapping-pairs-with-gcd-of-set-bits-count-equal-to-that-of-the-smallest-array-element/)
...
3.  否则，如果在`arr[]`中需要交换第 `i` 个和第 `j` 个位置元素，则计算`arr[i]`的设置位与`arr[j]`的设置位的 `GCD`，并检查其是否等于数组最小元素中的设置位的计数。
