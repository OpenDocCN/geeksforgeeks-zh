...
> 原文：[https://www.geeksforgeeks.org/check-if-a-triplet-with-given-sum-exists-in-bst/](https://www.geeksforgeeks.org/check-if-a-triplet-with-given-sum-exists-in-bst/)
...
众所周知，`BST` 的[有序遍历中的元素是按递增顺序排序的](https://www.geeksforgeeks.org/tree-traversals-inorder-preorder-and-postorder/)。因此，我们的想法是在给定的 `BST` 上按顺序遍历，并将元素存储在向量或数组中。现在任务简化为[在排序的数组中检查给定总和的三元组](https://www.geeksforgeeks.org/find-a-triplet-that-sum-to-a-given-value/)。
