...
> 原文: [https://www.geeksforgeeks.org/check-whether-all-the-bits-are-unset-in-the-given-range-or-not/](https://www.geeksforgeeks.org/check-whether-all-the-bits-are-unset-in-the-given-range-or-not/)
...
2.  计算 `new_num = n & num`。
3.  如果 `new_num == 0`，则返回 "Yes" (在给定范围内，所有位都不设置)。
4.  否则返回 "No" (在给定的范围内，所有的位不会被取消设置)。
