...
给定一个由 `N` 个顶点和 `M` 条边组成的**无向图**，以及类型为 `{X, Y}` 的查询 `Q[][]`，任务是检查顶点 `X` 和 `Y` 是否在图的同一个连通分量中。
...
思路是用[不相交集合-并集](https://www.geeksforgeeks.org/union-find/)来解决问题。使用的分离集联合数据结构的基本接口如下:
...
*   `union_set(a, b)`: 合并两个指定的集合（元素 `a` 所在的集合，元素 `b` 所在的集合）。两个连接的顶点合并成一个集合（连接的组件）。
