...
> 原文: [https://www.geeksforgeeks.org/pair-formation-maximum-pair-sum-minimized/](https://www.geeksforgeeks.org/pair-formation-maximum-pair-sum-minimized/)
...
**方法:** 想法是首先对给定的数组进行排序，然后遍历循环以形成对 `(i, j)`，其中 `i` 将从 `0` 开始，`j` 将相应地从数组的末尾开始。递增 `i` 和递减 `j` 形成下一对，以此类推。
