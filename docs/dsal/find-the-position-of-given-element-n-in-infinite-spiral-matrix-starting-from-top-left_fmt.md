...
> 原文：[https://www.geeksforgeeks.org/find-the-position-of-given-element-n-in-infinite-spiral-matrix-starting-from-top-left/](https://www.geeksforgeeks.org/find-the-position-of-given-element-n-in-infinite-spiral-matrix-starting-from-top-left/)
...
*   要找到数字所在的“倒 L”，找到数字的[平方根](https://www.geeksforgeeks.org/square-root-of-an-integer/)的[上限](https://www.geeksforgeeks.org/ceil-floor-functions-cpp/)。
...
*   如果该差值 `d` 小于 `l`，则 `n` 的行 `r` 和列 `c` 由下式给出：

> `r = l`
> `c = d + l`

*   如果该差值 `d` 大于或等于 `l`，则 `n` 的行 `r` 和列 `c` 由下式给出：
