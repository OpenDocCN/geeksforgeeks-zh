...
> 原文: [https://www.geeksforgeeks.org/minimum-replacements-required-to-have-at-most-k-distinct-elements-in-the-array/](https://www.geeksforgeeks.org/minimum-replacements-required-to-have-at-most-k-distinct-elements-in-the-array/)

给定一个由 `N` 个正整数和一个整数 `K` 组成的数组 `arr[]`，任务是找到需要被其他数组元素替换的最小数组元素数，使得该数组最多包含 `K` 个不同的元素。
...
*   使用变量 `i` 迭代范围 `[K, len]`。对于每一个 `i` 值，更新 `cntMin += Freq[i]`。
*   最后，打印 `cntMin` 的值。
