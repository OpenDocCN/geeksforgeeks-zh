...
> 原文: [https://www.geeksforgeeks.org/find-the-number-of-squares-inside-the-given-square-grid/](https://www.geeksforgeeks.org/find-the-number-of-squares-inside-the-given-square-grid/)
...
举几个例子，可以观察到对于一个大小为 `N * N` 的格子，里面的方块数会是 `1^2 + 2^2 + 3^2 + … + N^2`。
...
通过使用直接公式。然而，和 `\sum_{i=1}^{n}k^{2}` 具有封闭形式（直接公式） `\frac{n\left(n+1\right)\left(2n+1\right)}{6}`。因此，我们可以用它来计算 `O(1)` 时间内的总和。
