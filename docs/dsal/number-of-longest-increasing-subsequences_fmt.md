...
> 原文: [https://www.geeksforgeeks.org/number-of-longest-increasing-subsequences/](https://www.geeksforgeeks.org/number-of-longest-increasing-subsequences/)

给定一个大小为 `N` 的数组 `arr[]`，任务是计算给定数组中存在的最长递增子序列的数量。
...
最简单的方法是生成给定数组中存在的所有可能的子序列 `arr[]` 并计算最大长度的递增子序列。检查所有子序列后打印计数。
...
优化上述方法，思路是使用动态规划，因为上述问题有重叠子问题需要计算多次，减少计算使用制表或记忆。按照以下步骤解决问题:

*   初始化两个数组 `dp_l[]` 和 `dp_c[]`，分别存储每个索引处最长递增子序列的长度和最长递增子序列的计数。
*   使用变量 `i` 迭代范围 `[1，N–1]`:
    *   使用变量 `j` 迭代范围 `[0，i–1]`:
        *   如果 `arr[i] > arr[j]` 则检查以下情况:
            *   如果 `(dp_l[j] + 1)` 大于 `dp_l[i]`，则更新 `dp_l[i]` 为 `dp_l[j] + 1`，`dp_c[i]` 为 `dp_c[j]`
*   找到数组中的最大元素 `dp_l[]`，并将其存储在变量 `max_length` 中，该变量将给出 LIS 的长度。
*   用 0 初始化一个变量 `count`，以存储最长递增子序列的数量。
*   遍历数组 `dp_l[]`，如果在任何索引 `idx`、`dp_l[idx]` 与 `max_length` 相同，那么将 `count` 增加 `dp_c[idx]`。
*   完成上述步骤后，打印 `count` 的值，即给定数组中最长递增子序列的数量。
