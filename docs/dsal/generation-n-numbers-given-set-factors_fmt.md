...
> 原文: [https://www.geeksforgeeks.org/generation-n-numbers-given-set-factors/](https://www.geeksforgeeks.org/generation-n-numbers-given-set-factors/)

给定一个由 k 个数字组成的数组`factor[]`，任务是打印前 n 个数字（按升序排列），其因子来自给定的数组。
...
这个问题主要是[丑数问题](https://www.geeksforgeeks.org/ugly-numbers/)的延伸。我们创建一个辅助数组`next[]`，其大小与`factor[]`相同，以跟踪因子的下一个倍数。在每次迭代中，我们从`next`输出最小元素，然后用相应的因子递增它。如果最小值等于前一个输出值，则递增它（以避免重复）。否则我们打印最小值。
