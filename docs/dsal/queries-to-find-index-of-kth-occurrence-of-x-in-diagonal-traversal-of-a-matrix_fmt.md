...
> 原文：[https://www.geeksforgeeks.org/queries-to-find-index-of-kth-occurrence-of-x-in-diagonal-traversal-of-a-matrix/](https://www.geeksforgeeks.org/queries-to-find-index-of-kth-occurrence-of-x-in-diagonal-traversal-of-a-matrix/)

给定一个大小为 `N*M` 的矩阵 `A[][]` 和一个由形式为 `{X, K}` 的查询组成的 2D 数组 `Q[][]`，每个查询的任务是在执行从左到右的对角线遍历时，找到元素 `X` 在矩阵中的第 `K` 次出现的位置。如果矩阵中元素 `X` 的频率小于 `K`，则打印 `-1`。
...
**解释：**
`A[][]` 的对角线遍历是 `{1, 2, 4, 5}`。
`4` 的第 1 次出现在第 3 个位置，因此输出为 `3`。
`5` 的第 1 次出现在第 4 个位置，因此输出为 `4`。
矩阵中不存在 `10`，因此输出为 `-1`。
...
### 天真方法
解决给定问题的最简单方法是对角遍历每个查询的矩阵，找到元素 `Q[i][0]` 的第 `Q[i][1]` 次出现。如果第 `Q[i][1]` 次出现不存在，则打印 `-1`。否则，打印该位置。
**时间复杂度：** `O(Q * N * M)`
**辅助空间：** `O(1)`

### 高效方法
为了优化上述方法，其思想是将给定矩阵对角遍历的每个元素的索引存储在 `HashMap` 中，然后为每个查询找到相应的索引。按照以下步骤解决问题：

1.  初始化一个 `HashMap` `M` 来存储矩阵对角遍历中每个元素的位置。
2.  对角遍历矩阵并将遍历中每个元素的索引存储在 `HashMap` `M` 中。
3.  现在，遍历查询 `Q[][]`，对于每个查询 `{X, K}` 执行以下步骤：
    *   如果 `X` 不在 `M` 中或者 `X` 的出现次数小于 `K`，则打印 `-1`。
    *   否则，从散列表 `M` 中打印元素 `X` 的第 `K` 次出现的位置。
