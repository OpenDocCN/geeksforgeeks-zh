...
> 原文: [https://www.geeksforgeeks.org/print-the-longest-palindromic-prefix-of-a-given-string/](https://www.geeksforgeeks.org/print-the-longest-palindromic-prefix-of-a-given-string/)
...
**朴素方法:** 思路是[从起始索引生成给定字符串](https://www.geeksforgeeks.org/program-print-substrings-given-string/)的所有子串，检查子串是否回文。最大长度的回文字符串就是结果字符串。
...
*   创建一个字符串长度大小的数组(比如 `lps[]`)，该数组将存储最长的回文前缀，该前缀也是字符串 `str2` 的后缀。
*   使用 [KMP 搜索算法](https://www.geeksforgeeks.org/kmp-algorithm-for-pattern-searching/)的预处理算法更新 `lps[]`。
*   `LPS[length(str2) - 1]` 将给出给定字符串 `str` 的最长回文前缀字符串的长度。
