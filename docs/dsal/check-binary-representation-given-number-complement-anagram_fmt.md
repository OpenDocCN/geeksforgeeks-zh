...
> 原文: [https://www.geeksforgeeks.org/check-binary-representation-given-number-complement-anagram/](https://www.geeksforgeeks.org/check-binary-representation-given-number-complement-anagram/)
...
只需计算给定数字的位表示中存在的 `1` 的数量。如果存在的 `1` 的个数是 `32`，那么它的补码在它的位表示中也将有 `32` 个 `1`，它们将是彼此的字谜。
