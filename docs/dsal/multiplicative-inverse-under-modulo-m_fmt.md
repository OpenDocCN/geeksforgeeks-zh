...
> 原文: [https://www.geeksforgeeks.org/multiplicative-inverse-under-modulo-m/](https://www.geeksforgeeks.org/multiplicative-inverse-under-modulo-m/)
...
`x`的值应该在{1, 2, …, m-1}内，即在整数模`m`的范围内（注意`x`不能为0，因为`0 mod m`永远不会为1）。
当且仅当`a`和`m`互质（即`gcd(a, m) = 1`）时，“`a`模`m`”的乘法逆存在。
...
天真的方法是尝试从1到`m`的所有数字，对于每个数字`x`，检查`(a * x) % m`是否为1。
...
想法是使用[扩展欧几里得算法](https://www.geeksforgeeks.org/basic-and-extended-euclidean-algorithms/)，该算法采用两个整数`a`和`b`，找到它们的`gcd`，还找到`x`和`y`，这样

```
ax + by = gcd(a, b)
```

为了在模`m`下找到`a`的乘法逆，我们在上面的公式中放入`b = m`。既然我们知道`a`和`m`是互质的，我们可以把`gcd`的值设为1。
...
我们可以去掉左边的第二项，因为`my (mod m)`对于整数`y`总是0。

```
ax ≅ 1 (mod m)
```

所以我们可以用[扩展欧几里得算法](https://www.geeksforgeeks.org/basic-and-extended-euclidean-algorithms/)找到的`x`就是`a`的乘法逆。
