...
给定一个容量囊 `W`，和两个[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `A[]` 和 `B[]` 的长度 `N`，其中 `A[i]` 代表一个第 `i` 块黄金的重量，而 `B[i]` 代表通过取第 `i` 块获得的利润。
...
**方法：** 给定的问题可以使用称为[分数背包](https://www.geeksforgeeks.org/fractional-knapsack-problem/)的[贪心算法](https://www.geeksforgeeks.org/greedy-algorithms/)来解决。按照以下步骤解决问题：

*   初始化一个[对](https://www.geeksforgeeks.org/pair-in-cpp-stl/)的[向量](https://www.geeksforgeeks.org/vector-in-cpp-stl/) `V` 来存储以数组 `B` 中的数组元素为第一个元素，以数组 `A` 中的数组元素为第二个元素组成的对，并出现在同一索引中。
*   [迭代范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[0, N]`，如果 `A[i]` 不是[完美平方](https://www.geeksforgeeks.org/check-if-given-number-is-perfect-square-in-cpp/)，则推对 `{B[i], A[i]}` 进入向量 `V`。
...
*   将一个变量 `profit` 初始化为 `0` 存储最大利润。
*   [遍历向量](https://www.geeksforgeeks.org/vector-in-cpp-stl/) `V`，在每次迭代中执行以下步骤：
    *   如果当前对的第二个元素小于或等于 `W`，则以对的第一个元素增加 `profit`，即取整块黄金，然后以当前对的第二个元素减少 `W`。
    *   否则，取一个等于 `W` 与当前对第二个元素之比的量，存入变量 `P`。然后将 `profit` 增加 `P * 当前对的第一个元素`，然后中断。
*   最后，完成以上步骤后，打印 `profit` 中得到的值作为答案。
                          ...
                          long long W)
{
    ...
    for (int i = 0; i < V.size(); i++) {
        // If V[i].second is less than W
        if (V[i].second <= W) {
            // Increment profit by V[i].first
            profit += V[i].first;

            // Decrement V[i].second from W
            W -= V[i].second;
        }
        // Otherwise
        else {
