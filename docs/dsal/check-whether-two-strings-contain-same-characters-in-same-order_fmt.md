...
> 原文: [https://www.geeksforgeeks.org/check-whether-two-strings-contain-same-characters-in-same-order/](https://www.geeksforgeeks.org/check-whether-two-strings-contain-same-characters-in-same-order/)

给定两个字符串 `s1` 和 `s2`，任务是找出这两个字符串是否包含以相同顺序出现的相同字符。例如：字符串 `"Geeks"` 和字符串 `"Geks"` 包含相同顺序的相同字符。
