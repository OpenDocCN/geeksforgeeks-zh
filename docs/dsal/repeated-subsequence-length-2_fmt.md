...
> 原文: [https://www.geeksforgeeks.org/repeated-subsequence-length-2/](https://www.geeksforgeeks.org/repeated-subsequence-length-2/)
...
问题是[最长公共子序列](https://www.geeksforgeeks.org/longest-repeating-subsequence/)问题的经典变异。我们已经在这里讨论了动态编程解决方案。动态规划解占用 `O(n^2)` 时间和空间。本文讨论了 `O(n)` 时空方法。
