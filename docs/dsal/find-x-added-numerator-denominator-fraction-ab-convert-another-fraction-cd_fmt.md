...
> 原文: [https://www.geeksforgeeks.org/find-x-added-numerator-denominator-fraction-ab-convert-another-fraction-cd/](https://www.geeksforgeeks.org/find-x-added-numerator-denominator-fraction-ab-convert-another-fraction-cd/)

给定 `a/b` 形式的分数，其中 `a` & `b` 是正整数。求 `δX`，当它被加到给定分数的分子和分母上时，它将产生一个新的不可约分数 `c/d`。
