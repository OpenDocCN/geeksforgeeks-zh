...
> 原文：[https://www.geeksforgeeks.org/generate-passwords-given-character-set/](https://www.geeksforgeeks.org/generate-passwords-given-character-set/)
...
如果我们想查看字数，我们可以取消代码中有 `cnt` 变量的行的注释。我们可以观察到它出来是 $n^1 + n^2 +..+ n^n$，其中 `n = len`。因此程序的时间复杂度也是 $O(n^n)$，因此是指数型的。我们也可以在生成时检查特定的密码，如果找到就中断循环返回。我们还可以包含其他要生成的符号，如果需要，通过使用哈希表预处理输入来移除重复的符号。
