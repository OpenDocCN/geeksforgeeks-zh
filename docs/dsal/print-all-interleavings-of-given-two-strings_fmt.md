...
> 原文：[https://www.geeksforgeeks.org/print-all-interleavings-of-given-two-strings/](https://www.geeksforgeeks.org/print-all-interleavings-of-given-two-strings/)
...
给定两个字符串的交错字符串保持了单个字符串中字符的顺序。例如，在上面第一个例子的所有交错中，“A”在“B”之前，“C”在“D”之前。

## 算法解释

设 `str1` 的长度为 `m`，`str2` 的长度为 `n`，假设 `str1` 和 `str2` 中的所有字符都不同。让 `count(m, n)` 为这些串中所有交错串的计数。`count(m, n)` 的值可以写成如下形式。
...
要打印所有交错，我们可以首先修复 `str1[0]` 到输出字符串中的 `m-1]`，并递归调用 `str1[1..m-1]` 和 `str2[0..n-1]`。然后我们可以修复 `str2[0]` 的第一个字符到 `n-1]` 并递归调用 `str1[0..m-1]` 和 `str2[1..n-1]`。感谢 `akash01` 提供以下 C 实现。
