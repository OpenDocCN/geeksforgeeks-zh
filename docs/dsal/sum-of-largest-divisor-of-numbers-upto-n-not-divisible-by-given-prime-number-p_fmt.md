...
> 原文: [https://www.geeksforgeeks.org/sum-of-largest-divisor-of-numbers-upto-n-not-divisible-by-given-prime-number-p/](https://www.geeksforgeeks.org/sum-of-largest-divisor-of-numbers-upto-n-not-divisible-by-given-prime-number-p/)
...
天真的想法是[找到范围 `[1, N]` 中每个数](https://www.geeksforgeeks.org/find-divisors-natural-number-set-1/)的除数，找到最大的不能被 `P` 整除的除数。打印所有最大除数的总和。
...
其思想是观察一个数 `N` 的最大除数不能被 `P` 整除，如果 `N` 不能被 `P` 整除，则为 `N` 本身，否则所需除数将与 `N/P` 相同。以下是步骤：
