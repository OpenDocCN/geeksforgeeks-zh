...
> 原文：[https://www.geeksforgeeks.org/newton-forward-backward-interpolation/](https://www.geeksforgeeks.org/newton-forward-backward-interpolation/)
...
这个公式对于在给定值集开始附近插值 `f(x)` 的值特别有用。`h` 称为差的区间，且 `u = (x–a)/h`，这里 `a` 是第一项。
