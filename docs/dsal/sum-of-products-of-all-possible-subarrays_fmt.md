...
> 原文: [https://www.geeksforgeeks.org/sum-of-products-of-all-possible-subarrays/](https://www.geeksforgeeks.org/sum-of-products-of-all-possible-subarrays/)
...
> 再次，`(b + bc + bcd) + (c + cd) = b * (1 + c + cd) + (c + cd)`
>
> 同样，表达式 `(c + cd)` 可以使用两次。
> 后半部分同上。
