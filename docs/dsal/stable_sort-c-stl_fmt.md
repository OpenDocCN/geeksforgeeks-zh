...
`stable_sort()` 用于对`[first, last)`范围内的元素进行升序排序。类似于 `std::sort`，但是 `stable_sort()` 保持等价元素的相对顺序。它位于 `<algorithm>` 头文件下。
...
*   `first`: 迭代器，指向范围中的第一个元素。
*   `last`: 迭代器，指向范围中过去的最后一个元素。
...
| **时间复杂度** | 是 **O(n*log(n))**。 | 是 **O(n*log^2(n))**。如果与长度成线性比例的额外内存不可用。如果可用，则为 **O(n * log(n))**。 |
