...
> 原文: [https://www.geeksforgeeks.org/reduce-a-string-to-a-valid-email-address-of-minimum-length-by-replacing-specified-substrings/](https://www.geeksforgeeks.org/reduce-a-string-to-a-valid-email-address-of-minimum-length-by-replacing-specified-substrings/)

给定代表长度为 `N` 的电子邮件地址的字符串 `S`，任务是通过将其中的 `"点"` 替换为 `"."` 和 `"at"` 替换为 `"@"` 来找到字符串的最小可能长度，这样该字符串就代表了一个有效的电子邮件地址。

> 一个电子邮件地址只能有一个 `"@"` 并且可能有零个或多个点 `(".")`。不能有 `'@'` 或 `'.'` 在电子邮件地址的开头和结尾。

**示例:**

> **输入:** `S = "geeksforgeeksatgmaildotcom"`
...
> **输出:** `at@.dotcom`
> **说明:**
> 可以多次替换，但我们不能从电子邮件地址的开头替换 `"at"` 或从结尾替换 `"dot"`，因为它不会形成有效的电子邮件地址。
