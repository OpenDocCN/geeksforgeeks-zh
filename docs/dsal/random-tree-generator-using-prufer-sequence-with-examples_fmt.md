...
> 原文：[https://www.geeksforgeeks.org/random-tree-generator-using-prufer-sequence-with-examples/](https://www.geeksforgeeks.org/random-tree-generator-using-prufer-sequence-with-examples/)

给定一个整数 `N`，任务是生成一个带有 `(N–1)` 条边的 `N` 个节点的随机标记树，不形成循环。
...
这种方法使用[普吕弗序列](https://www.geeksforgeeks.org/prufer-code-tree-creation/)来生成随机树。
...
如果节点数为 `N`，则普吕弗序列的长度为 `(N–2)`，每个位置可以有 `N` 个可能的值。所以有 `N` 个节点的可能标记树的数量为 `N^(N–2)`。
...
*   生成随机序列
    ...
    S = {s1, s2, s3.....sn-2}
    ```
*   其中序列的每个元素 `s_i ∈ { 1, 2, 3, …, N }`，允许元素重复。
*   从生成的普吕弗序列**生成树**：
    1.  用值 `{1, 2, 3, …, N}` 创建 `N` 个节点。
    2.  找到最小元素 `x`，使得 `x ∈ { 1, 2, 3, …, N }` 且 `x ∉ S`。
    3.  将值为 `X` 的节点连接到值为 `s_1` 的节点。
    4.  从 `S` 中删除 `s_1`。
    5.  重复步骤 2 中的相同过程，直到普吕弗序列为空。

**例如：**

*   节点数 = `3`
*   那么普吕弗序列的长度将是 `(N–2)`，因为在这种情况下，它将是 1，并且可能的值可以是 `{1, 2, 3}`。
*   可能的随机序列是 `{1}`、`{2}`、`{3}`。
