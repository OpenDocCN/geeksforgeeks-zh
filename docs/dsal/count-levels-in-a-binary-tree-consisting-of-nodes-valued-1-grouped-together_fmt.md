...
> 原文: [https://www.geeksforgeeks.org/count-levels-in-a-binary-tree-consisting-of-nodes-valued-1-grouped-together/](https://www.geeksforgeeks.org/count-levels-in-a-binary-tree-consisting-of-nodes-valued-1-grouped-together/)

给定一个仅由 `0` 和 `1` 组成的[二叉树](https://www.geeksforgeeks.org/binary-tree-data-structure/)，任务是打印[二叉树](https://www.geeksforgeeks.org/binary-tree-data-structure/)中的级别计数，其中所有 `1` 连续放置在一个组中。
...
*   遍历[二叉树](https://www.geeksforgeeks.org/binary-tree-data-structure/)的每一级，考虑以下三个变量:
    1.  `flag1`: 第一次出现值为 `1` 的节点后，设置为 `1`。
    2.  `flag0`: 第一次出现值为 `0` 的节点后设置为 `1`，第二次出现值为 1 的节点后设置为。
    3.  `flag2`: 在第一次出现值为 `1` 的节点后设置，此时`flag0`和`flag1`都设置为 `1`。
*   遍历每个级别后，检查`flag1`是否设置为 1，`flag2`是否为 0。如果发现是真的，将该级别包括在计数中。
*   最后，打印获得的计数。
