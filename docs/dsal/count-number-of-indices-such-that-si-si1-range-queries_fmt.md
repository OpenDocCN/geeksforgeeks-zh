...
> 原文: [https://www.geeksforgeeks.org/count-number-of-indices-such-that-si-si1-range-queries/](https://www.geeksforgeeks.org/count-number-of-indices-such-that-si-si1-range-queries/)

给定一个字符串 `str`。现在，对于由两个整数 `L` 和 `R` 组成的每个查询，任务是找到索引的数量，使得 `str[i] = str[i+1]` 和 `L ≤ i，i+1 ≤ R`。
