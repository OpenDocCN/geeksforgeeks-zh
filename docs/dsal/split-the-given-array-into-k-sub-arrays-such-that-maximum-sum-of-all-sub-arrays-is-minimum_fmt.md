...
> 原文: [https://www.geeksforgeeks.org/split-the-given-array-into-k-sub-arrays-such-that-maximum-sum-of-all-sub-arrays-is-minimum/](https://www.geeksforgeeks.org/split-the-given-array-into-k-sub-arrays-such-that-maximum-sum-of-all-sub-arrays-is-minimum/)

给定一个由 `N` 个元素组成的数组 `arr[]` 和一个数字 `k` (`1 <= K <= N`)。将给定的数组分成 `K` 个子数组（它们必须覆盖所有元素）。在形成的 `K` 个子数组中，可达到的最大子数组和必须是最小可能的。找出那个可能的子数组和。
...
**时间复杂度**: `O((n-1)C(k-1))` (**注:** `C` 在这里表示组合(即 `(n-1)! / ((n-k)! * (k-1)!)` )
...
### 时间复杂度
O(N*log(Sum))，其中 N 是数组的元素个数，Sum 是数组所有元素的和。