...
> 原文: [https://www.geeksforgeeks.org/check-whether-bishop-can-take-down-pawn-or-not/](https://www.geeksforgeeks.org/check-whether-bishop-can-take-down-pawn-or-not/)

给定一个主教和一个棋子在 `8 * 8` 棋盘上的位置，任务是检查主教是否能一步拿下棋子。主教和棋子的位置用棋盘的行列号来表示。

**举例:**

> **输入:** `bishopX = 5`，`bishopY = 5`，`pawnX = 1`，`pawnY = 1`
> **输出:** 是
> **输入:** `bishopX = 5`，`bishopY = 5`，`pawnX = 1`，`pawnY = 3`
> **输出:** 否
