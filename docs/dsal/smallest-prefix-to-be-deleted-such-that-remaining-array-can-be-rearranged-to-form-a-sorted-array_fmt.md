...
> 原文: [https://www.geeksforgeeks.org/smallest-prefix-to-be-deleted-such-that-remaining-array-can-be-rearranged-to-form-a-sorted-array/](https://www.geeksforgeeks.org/smallest-prefix-to-be-deleted-such-that-remaining-array-can-be-rearranged-to-form-a-sorted-array/)

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是找到需要删除的前缀的最小长度，以便通过逐个选择第一个或最后一个元素来重复重新排列剩余的数组元素，从而形成一个排序数组。
...
### 暴力方法
最简单的方法是从给定的数组中删除前缀的所有可能长度，对于每个前缀，检查是否可以通过删除这些前缀从剩余的数组元素中形成排序数组。如果发现为真，则打印删除的前缀的最小长度。

**时间复杂度:** `O(N^2)`
**辅助空间:** `O(N)`

### 优化方法
上述方法可以通过观察得到的后缀必须是 `arr[1] <= arr[2] <= ... >= arr[N-2] >= arr[N-1]` 的形式来优化，其中 `N` 是剩余数组的长度，后缀是最大长度。请遵循以下步骤:

1.  将变量 `index` 初始化为 `N-1`。
2.  从终点开始遍历数组，并在 `arr[index-1] <= arr[index]` 处停止。
3.  随着迭代的进行，我们不断减少 `index`。
4.  完成以上所有步骤后，打印 `index` 的值，这是必须删除的前缀的最小长度。
