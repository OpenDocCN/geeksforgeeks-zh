...
> 原文: [https://www.geeksforgeeks.org/find-n-distinct-numbers-whose-bitwise-xor-is-equal-to-k/](https://www.geeksforgeeks.org/find-n-distinct-numbers-whose-bitwise-xor-is-equal-to-k/)
...
*   现在，计算 `(N–3)` 元素的按位异或，并将其存储在一个变量中，比如 `值`。可以根据以下条件选择其余三个元素:
    ...
    *   **情况 2:** 如果 `值` 不等于 `K`，执行以下步骤:
        ...
        *   通过考虑最后 3 个元素等于 `0`，`P`，`P 异或 K 异或 值`，这三个元素的按位异或等于 `K 异或 值`。
