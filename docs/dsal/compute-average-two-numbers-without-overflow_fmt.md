...
> 原文: [https://www.geeksforgeeks.org/compute-average-two-numbers-without-overflow/](https://www.geeksforgeeks.org/compute-average-two-numbers-without-overflow/)

给定两个数字，`a` 和 `b`。计算这两个数字的平均值。
熟知的公式 `(a + b) / 2` 在以下情况下可能失效:
如果，`a = b = (2^31) – 1`；即 `INT_MAX`。
