...
> 原文: [https://www.geeksforgeeks.org/print-all-possible-paths-from-the-first-row-to-the-last-row-in-a-2d-array/](https://www.geeksforgeeks.org/print-all-possible-paths-from-the-first-row-to-the-last-row-in-a-2d-array/)

给定一个具有 `M` 行和 `N` 列的 2D 字符数组。任务是打印从顶部（第一行）到底部（最后一行）的所有可能路径。

**示例：**

> **输入:** `arr[]` = {‘a’, ‘b’, ‘c’, ‘d’, ‘e’, ‘f’, ‘g’, ‘h’, ‘i’}
> **输出:**
> adg adi aeg aei afg
