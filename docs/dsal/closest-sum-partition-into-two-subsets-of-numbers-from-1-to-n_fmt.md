...
> 原文：[https://www.geeksforgeeks.org/closest-sum-partition-into-two-subsets-of-numbers-from-1-to-n/](https://www.geeksforgeeks.org/closest-sum-partition-into-two-subsets-of-numbers-from-1-to-n/)

给定一个整数序列 `1, 2, 3, 4, …, n`。任务是将它分成两组 `A` 和 `B`，每个元素恰好属于一组，使得 `| sum(A) – sum(B) |` 是最小可能。打印 `| sum(A) – sum(B) |` 的值。

**举例：**

> **输入：** 3
> **输出：** 0
> `A = {1, 2}` 和 `B = {3}`，`| sum(A) – sum(B) | = | 3 – 3 | = 0`。
> **输入：** 6
> **输出：** 0
> `A = {1, 3, 4}` 和 `B = {2, 5}`，`| sum(A) – sum(B) | = | 8 – 7 | = 1`。
> **输入：** 5
> **输出：** 1
