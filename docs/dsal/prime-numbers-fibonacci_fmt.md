...
一个**有效的解决方案**是使用[筛来生成 `n`](https://www.geeksforgeeks.org/sieve-of-eratosthenes/) 以内的所有素数。在我们生成素数之后，如果一个数的形式是 `5i^2 + 4` 或者形式是 `5i^2–4`，我们可以通过使用一个数是斐波那契数的属性来快速检查这个数是否是斐波那契数。详见[本](https://www.geeksforgeeks.org/check-number-fibonacci-number/)。
