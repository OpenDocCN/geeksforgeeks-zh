...
> 原文: [https://www.geeksforgeeks.org/check-binary-tree-subtree-another-binary-tree-set-2/](https://www.geeksforgeeks.org/check-binary-tree-subtree-another-binary-tree-set-2/)
...
对于这个问题，我们已经讨论了[一个 O(n)<sup>2</sup>的解决方案](https://www.geeksforgeeks.org/check-if-a-binary-tree-is-subtree-of-another-binary-tree/)。在这篇文章中，讨论了一个 O(n)的解决方案。这个想法是基于这样一个事实，即[序和前序/后序唯一地标识了一个二叉树](https://www.geeksforgeeks.org/if-you-are-given-two-traversal-sequences-can-you-construct-the-binary-tree/)。树 `S` 是 `T` 的一个子树，如果 `S` 的有序和前序遍历分别是 `T` 的有序和前序遍历的子串。
以下是详细步骤。
...
**时间复杂度:** 二叉树的有序和前序遍历花费 `O(n)` 个时间。函数 `strstr()` 也可以使用 [KMP 字符串匹配算法](https://www.geeksforgeeks.org/searching-for-patterns-set-2-kmp-algorithm/) 在 `O(n)` 时间内实现。
