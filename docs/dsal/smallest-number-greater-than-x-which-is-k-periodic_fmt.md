...
> 原文: [https://www.geeksforgeeks.org/smallest-number-greater-than-x-which-is-k-periodic/](https://www.geeksforgeeks.org/smallest-number-greater-than-x-which-is-k-periodic/)

给定一个由 `N` 个数字组成的字符串整数 `X` 和一个整数 `K`，任务是找出大于或等于 `X` 的最小整数，它是 [K 周期](https://www.geeksforgeeks.org/check-if-the-given-string-is-k-periodic/) 的 (`K <= N`)。
...
**做法:**
要解决问题，思路是让 `X[i] = X[i-K]` 为所有 `i > K`。然后按照以下步骤解决问题:

*   检查 `X` 是否大于或等于初始整数。如果是，则打印当前字符串作为答案。
*   否则，从右向左遍历，找到不等于 `'9'` 的第一个数字。然后将该数字增加 `1`，并用一个名为 `POS` 的变量标记该位置。
*   然后将 `POS` 后的所有数字设置为 `'0'`。再次使 `X[i] = X[i-K]` 为所有 `i > K`。
