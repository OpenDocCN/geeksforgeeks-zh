...
> 原文：[https://www.geeksforgeeks.org/kth-number-exceeding-n-whose-sum-of-its-digits-divisible-by-m/](https://www.geeksforgeeks.org/kth-number-exceeding-n-whose-sum-of-its-digits-divisible-by-m/)

给定整数 `N`、`K` 和 `M`，任务是找到大于 `N` 的第 `K` 个数，其[位数之和](https://www.geeksforgeeks.org/program-for-sum-of-the-digits-of-a-given-number/)可被 `M` 整除。
...
> **输出：** `32`
> **说明：**
> 大于 `N = 6` 且其位数之和可被 `5` 整除的数为 `{14，19，23，28，32，…}`。第 `K` 个号，即第 `5` 个号为 `32`。
>
> **输入：** `N = 40`，`K = 4`，`M = 5`
> **输出：** `55`
> **说明：**
> 大于 `N = 40` 且其位数之和可被 `5` 整除的数为 `{41，46，50，55，…}`。第 `K` 个号，即第 `4` 个号为 `55`。

## 简单方法

最简单的方法是检查每个大于 `N` 的数字，如果其数字的[和](https://www.geeksforgeeks.org/program-for-sum-of-the-digits-of-a-given-number/)可被 `M` 整除，则将计数器增加 `1`。继续检查数字，直到计数器等于 `K`。
...
思路是使用[**动态规划**](https://www.geeksforgeeks.org/dynamic-programming/)和[**二分搜索法**](https://www.geeksforgeeks.org/binary-search/)。按照以下步骤解决问题：

*   首先创建一个[递归记忆函数](https://www.geeksforgeeks.org/memoization-1d-2d-and-3d/)，求小于等于 `S` 的整数总数，其位数之和可被 `M` 整除，如下图：

> `dp` 过渡将是：
>
> `dp(S, index, sum, tight, M) = dp(index + 1, (sum + i) % M, tight, M)` 其中，
>
> *   `S` 给出了一个极限，对于这个极限，需要找到所有小于或等于 `S` 且位数之和可被 `M` 整除的数。
> *   `index` 是当前需要放置一个数字的索引。
> *   `sum` 取 `0` 到 `4` 之间的值。
> *   基本条件是，如果 `index` 变得大于 `S` 的长度，并且总和可被 `M` 整除，则返回 `1`。
> *   `M` 为除数。
>
...
>
> *   `tight` 会变成 `1`，表示这个数字仍然没有变得小于 `S`。
> *   `i` 将从 `0` 迭代到数字 `S[index]`。

*   现在使用[二分搜索法](https://www.geeksforgeeks.org/binary-search/)，其中下限为 `N + 1`，上限为某个大整数。
*   初始化一个变量 `left`，用于存储小于或等于 `N` 的总数，其位数之和可被 `M` 整除。
*   求下限和上限的中点，然后用上述 `dp` 函数求小于或等于中点且位数之和可被 `M` 整除的数。顺其自然 `right`。
*   如果 `left + K` 等于 `right`，则将答案更新为中间，并将上限设置为 `mid - 1`。
*   否则，如果 `left + K` 小于 `right`，则将上限设置为 `mid - 1`。
*   如果 `left + K` 大于 `right`，则将下限设置为 `mid + 1`。
*   重复上述步骤，同时下限小于或等于上限。
