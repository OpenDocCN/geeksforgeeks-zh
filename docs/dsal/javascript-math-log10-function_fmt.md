...
> 原文: [https://www.geeksforgeeks.org/javascript-math-log10-function/](https://www.geeksforgeeks.org/javascript-math-log10-function/)

`Math.log10()`是 JavaScript 中的一个内置函数，它给出任何数字的以 10 为底的对数值。
...
该函数接受单个参数`p`，该参数是要计算的以 10 为底的对数的任何数字。
...
**说明:**
这里数字 5 的以 10 为底的对数值是 0.6989700043360189，如输出所示。
...
这个函数的参数应该总是一个数字，否则它会返回`NaN`，也就是说，当它的参数作为字符串时，它不是一个数字。
...
每当我们需要任何数字的以 10 为底的对数值时，我们就借助这个函数。它的价值在数学题中需要很多次。
...
JavaScript `Math.log10()`函数支持的浏览器如下:
