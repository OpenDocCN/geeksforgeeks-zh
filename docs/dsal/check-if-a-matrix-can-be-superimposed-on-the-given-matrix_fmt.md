...
> 原文: [https://www.geeksforgeeks.org/check-if-a-matrix-can-be-superimposed-on-the-given-matrix/](https://www.geeksforgeeks.org/check-if-a-matrix-can-be-superimposed-on-the-given-matrix/)

给定一个大小为 `N * M` 的矩阵 `字母[][]`，由 `'#'` 和 `'*'` 组成，另一个大小为 `X * Y` 的矩阵 `印章[][]` 只包含 `'{ content }'`。任务是找出较大的一个的所有 `'*'` 是否可以被 `'{content}'` 代替；通过在字母矩阵上叠加印章矩阵。

**注意:** 在叠加操作中，只有所有字符都为 `'*'` 或 `'{ content }'` 的区域可以考虑。
