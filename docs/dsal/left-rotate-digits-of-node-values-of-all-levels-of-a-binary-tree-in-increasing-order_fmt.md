...
> 原文: [https://www.geeksforgeeks.org/left-rotate-digits-of-node-values-of-all-levels-of-a-binary-tree-in-increasing-order/](https://www.geeksforgeeks.org/left-rotate-digits-of-node-values-of-all-levels-of-a-binary-tree-in-increasing-order/)
...
> ```
> **解释:**
> **2 级:** 节点值为 241 和 123。向左旋转 241 两次和 123 一次将节点值修改为 124 和 231。
> **三级:** 节点值为 324、235、161。324 一次、235 一次和 161 一次的左旋转数字分别将节点值修改为{243、352、611}。
>
> **输入:**
> ```
> 12
...
给定的问题可以通过执行[级别顺序遍历](https://www.geeksforgeeks.org/level-order-tree-traversal/)并向左旋转节点值的数字以使值以递增的顺序进入每个级别来解决。按照以下步骤解决问题:
...
*   迭代直到[队列不为空](https://www.geeksforgeeks.org/queueempty-queuesize-c-stl/)并执行以下步骤:
    ...
    *   将 `prev` 的值更新为 `temp` 的当前值。
    *   如果 `temp` 有左或右子，则将其推入队列。
*   完成上述步骤后，如果当前节点集没有按递增顺序排序，则打印`-1`。否则，检查下一次迭代。
