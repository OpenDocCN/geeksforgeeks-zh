...
> 原文：[https://www.geeksforgeeks.org/count-elements-in-the-given-range-which-have-maximum-number-of-divisors/](https://www.geeksforgeeks.org/count-elements-in-the-given-range-which-have-maximum-number-of-divisors/)
...
*   第一个可整除是第一个可被 `i` 整除的数(外环)，大于或等于 `x`

这里先用[求最接近 n 且可被 m 除尽的数](https://www.geeksforgeeks.org/find-number-closest-n-divisible-m/)法计算 `first_divisible`。然后[找到数字](http://Find the number of divisors of each element.)的约数。
