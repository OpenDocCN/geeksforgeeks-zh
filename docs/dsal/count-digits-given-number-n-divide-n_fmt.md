...
> 原文: [https://www.geeksforgeeks.org/count-digits-given-number-n-divide-n/](https://www.geeksforgeeks.org/count-digits-given-number-n-divide-n/)
...
这个问题的一个**简单解决方案**是读取字符串形式的数字，并逐个检查 `N` 中出现的每个数字的可除性。这种方法的时间复杂度为 `O(N^2)`。

这个问题的有效解决方案是使用一个额外的大小为 10 的数组 `divide[]`。因为我们只有 10 个数字，所以运行一个从 1 到 9 的循环，并检查从 1 到 9 的每个数字的可除性。如果任一数字除 `N`，则在数字处的 `divide[]` 数组中标记为真作为索引。如果当前数字 `i` 的 `divide[i]` 为真，现在遍历数字串并增加结果。
...
## 服务器端编程语言（PHP - Professional Hypertext Preprocessor 的缩写）
