...
> 原文：[https://www.geeksforgeeks.org/program-to-find-the-kth-character-after-decrypting-a-string/](https://www.geeksforgeeks.org/program-to-find-the-kth-character-after-decrypting-a-string/)

给定一个由字符和数字组成的字符串和一个整数 `k`，任务是解密该字符串并返回解密字符串中的第 `k` 个字符。
...
*   当 `i` 小于输入字符串的长度时循环，检查当前字符是否是字母。如果是，则将 `total_len` 增加 `1` 并检查总长度是否小于或等于 `k`，如果是，则返回字符串，否则递增 `i`。
*   初始化 `n = 0`，再次循环，当 `i` 小于输入字符串的长度，并且 `i` 不是字母表时，解析该数字并递增 `i`，找到 `next_total_len = total_len * n`。
    *   如果 `k <= total_len`，那么通过初始化 `pos = k % total_len` 获得字符的位置。
    *   如果找不到位置，则更新 `pos = total_len`，最后在第 `k` 个位置返回字符。如果没有找到，则返回 `-1`。
