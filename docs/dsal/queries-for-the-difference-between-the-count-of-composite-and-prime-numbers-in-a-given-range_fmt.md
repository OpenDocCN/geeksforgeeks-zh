...
> 原文：[https://www.geeksforgeeks.org/queries-for-the-difference-between-the-count-of-composite-and-prime-numbers-in-a-given-range/](https://www.geeksforgeeks.org/queries-for-the-difference-between-the-count-of-composite-and-prime-numbers-in-a-given-range/)

给定 `Q` 查询，其中每个查询由两个正整数 `L` 和 `R` 组成，任务是在范围 `[L, R]` 内找到素数计数和复合数计数之间的绝对差。
...
*   使用厄拉多塞的[筛](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)，生成一个数组 `prime[]`，使得 `prime[i] = 1` 如果 `i` 是素数，否则为 `0`。
*   现在更新 `prime[]` 数组，使得 `prime[i]` 存储素数的计数，即 `≤ i`。
*   对于每一个查询，范围 `[L, R]` 内素数的计数可以通过 `prime[R] - prime[L-1]` 求出，复合数的计数将是从元素总数中减去素数的计数。
*   打印素数计数和上一步中找到的复合数之间的绝对差值。
