...
> 原文: [https://www.geeksforgeeks.org/finite-automata-algorithm-for-pattern-searching/](https://www.geeksforgeeks.org/finite-automata-algorithm-for-pattern-searching/)

给定一个文本 `txt[0..n-1]` 和模式 `pat[0..m-1]`，编写一个函数 `search(char pat[], char txt[])`，打印 `txt[]` 中所有出现的 `pat[]`。你可以假设 `n > m`。
...
FA 中的状态数将为 `M+1`，其中 `M` 是模式的长度。构造 FA 最主要的是为每个可能的字符从当前状态获取下一个状态。给定字符 `x` 和状态 `k`，我们可以通过考虑字符串 `pat[0..k-1]x`，它基本上是模式字符 `pat[0]`、`pat[1]` … `pat[k-1]` 和字符 `x` 的串联。其思想是获得给定模式的最长前缀的长度，使得前缀也是 `pat[0..k-1]x`。长度的值给出了下一个状态。例如，让我们看看如何从当前状态 5 和上图中的字符 `C` 获得下一个状态。我们需要考虑字符串 `pat[0..4]C`，即 `ACACAC`。模式的最长前缀的长度是 4 (`ACAC`)，使得前缀是 `ACACAC` 的后缀。因此，字符 `C` 的下一个状态(来自状态 5)是 4。

在下面的代码中，`computeTF()` 构造了 FA。`computeTF()` 的时间复杂度是 `O(m^3*NO_OF_CHARS)`，其中 `m` 是模式的长度，`NO_OF_CHARS` 是字母表的大小(模式和文本中可能的字符总数)。该实现尝试所有可能的前缀，从可能是后缀 `pat[0..k-1]x`。在 `O(m*NO_OF_CHARS)` 中构造 FA 有更好的实现(提示:我们可以在 KMP 算法中使用类似 [lps 数组构造](https://www.geeksforgeeks.org/kmp-algorithm-for-pattern-searching/)的东西)。我们已经在下一篇关于模式搜索的文章中介绍了更好的实现方式：[Pattern Searching Set 5 (Efficient Construction of Finite Automata)](https://www.geeksforgeeks.org/pattern-searching-set-5-efficient-constructtion-of-finite-automata/)。
