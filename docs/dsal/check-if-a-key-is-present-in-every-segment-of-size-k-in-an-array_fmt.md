...
> 原文: [https://www.geeksforgeeks.org/check-if-a-key-is-present-in-every-segment-of-size-k-in-an-array/](https://www.geeksforgeeks.org/check-if-a-key-is-present-in-every-segment-of-size-k-in-an-array/)

给定一个数组 `arr[]`，数组的大小是 `n`，另一个是键 `x`，给你一个段大小 `k`。任务是在 `arr[]` 中，找到大小为 `k` 的每个段中存在的键 `x`。
...
> `k = 5`
> **输出:** 是
> 有三段，最后一段未满 `{ 21, 23, 56, 65, 34}`，`{54, 76, 32, 23, 45}` 和 `{21, 23, 25}`。
> `23` 是呈现所有窗口。
