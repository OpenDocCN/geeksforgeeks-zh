...
给定一个由 `V` 个顶点和 `E` 条边组成的加权有向图。任务是打印权重之和为负的[循环路径](https://www.geeksforgeeks.org/detect-cycle-in-a-graph/)。如果不存在这样的路径，则打印 **`-1`**。
...
*   执行贝尔曼-福特算法的 `N-1` 次迭代，放松每条边 `(u, v)`。跟踪每个顶点的父节点，并将其存储在一个数组中 `parent[]`。
