...
> 原文: [https://www.geeksforgeeks.org/total-pairs-in-an-array-such-that-the-bitwise-and-bitwise-or-and-bitwise-xor-of-lsb-is-1/](https://www.geeksforgeeks.org/total-pairs-in-an-array-such-that-the-bitwise-and-bitwise-or-and-bitwise-xor-of-lsb-is-1/)

给定一个大小为 `N` 的数组 `arr[]`。任务是找到配对的数量 `(arr[i], arr[j])`，即 `cntAND`、`cntOR` 和 `cntXOR`，这样：
...
3.  因此，所需对的总数：
    *   **对于 XOR:** `cntXOR = cntOdd * cntEven`
    *   **对于 AND:** `cntAND = cntOdd * (cntOdd - 1) / 2`
    *   **对于 OR:** `cntOR = (cntOdd * cntEven) + cntOdd * (cntOdd - 1) / 2`
