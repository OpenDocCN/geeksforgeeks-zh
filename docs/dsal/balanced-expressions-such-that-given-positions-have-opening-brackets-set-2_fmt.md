...
> 原文: [https://www.geeksforgeeks.org/balanced-expressions-such-that-given-positions-have-opening-brackets-set-2/](https://www.geeksforgeeks.org/balanced-expressions-such-that-given-positions-have-opening-brackets-set-2/)

给定一个整数 `n` 和一个位置数组 `position[]` (1 <= length(position[]) <= 2n)，找出长度为 2n 的适当括号表达式的数量，以便给定的位置有左括号。

**注意:** `position[]` 数组是以(基于 1 的索引)[0，1，1，0]的形式给出的。这里 1 表示开放式支架应该放置的位置。在值为 0 的位置，可以放置左括号或右括号。

**示例**:

> **输入** : n = 3，`position[]` = [0，1，0，0，0]
> **输出** : 3
> 长度 6 和位置 2 的开括号的合适的括号顺序是:
> `[[]][][]`
> `[[]]`
> `[[][][]]`
>
> **输入** : n = 2，`position[]` = [1，0，1，0]
> **输出** : 1
> 唯一的可能是:
> `[]`
