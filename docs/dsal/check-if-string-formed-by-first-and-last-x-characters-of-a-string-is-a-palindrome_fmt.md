...
> 原文: [https://www.geeksforgeeks.org/check-if-string-formed-by-first-and-last-x-characters-of-a-string-is-a-palindrome/](https://www.geeksforgeeks.org/check-if-string-formed-by-first-and-last-x-characters-of-a-string-is-a-palindrome/)

给定一个[字符串](https://www.geeksforgeeks.org/category/data-structures/c-strings/) `字符串`和一个整数 `X`。任务是查找[字符串](https://www.geeksforgeeks.org/category/data-structures/c-strings/) `字符串`和[反转](https://www.geeksforgeeks.org/reverse-a-string-in-java/)后的[字符串](https://www.geeksforgeeks.org/category/data-structures/c-strings/) `字符串`的前 `X` 个字符是否相同。如果相等，则打印`真`，否则打印`假`。
...
*   [迭代](https://www.geeksforgeeks.org/java-while-loop-with-examples/)而 `i` 小于 `n` 和 `x` :
    *   如果从开始的`和`最后的字符和最后的`和`不相等，则打印`假`和`返回`。
*   完成以上步骤后，打印`真`作为答案。
