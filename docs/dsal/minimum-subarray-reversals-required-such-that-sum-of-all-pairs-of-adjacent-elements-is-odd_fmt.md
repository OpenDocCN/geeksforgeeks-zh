...
> 原文: [https://www.geeksforgeeks.org/minimum-subarray-reversals-required-such-that-sum-of-all-pairs-of-adjacent-elements-is-odd/](https://www.geeksforgeeks.org/minimum-subarray-reversals-required-such-that-sum-of-all-pairs-of-adjacent-elements-is-odd/)

给定大小为 `N` 的数组 `arr[]`，具有相等数量的偶数和奇数，任务是找到使相邻元素对之和为奇数所需的最小子数组反转次数。
...
> **输出:** 3
> **解释:**
> **步骤 1:** 反转子数组 `[2, 4]` 将数组修改为 `{13, 2, 3, 8, 6, 5, 7, 10, 14, 15}`
> **步骤 2:** 反转子数组 `[2, 4]` 将数组修改为 `{13, 2, 3, 8, 5, 6, 7, 10, 14, 15}`
> **步骤 3:** 反转子数组 `[8, 9]` 将数组修改为 `{13, 2, 3, 8, 5, 6, 7, 10, 15, 14}`
> 上述反转后，所有相邻元素对的和为奇数。
> 因此，所需的最小反转为 3 次。
>
...
要使相邻元素之和为奇数，其思想是以奇偶或偶奇的方式排列元素。要最小化反转操作的计数，请观察给定数组的以下属性:

*   如果有任何长度为 `M` 的子数组具有相同奇偶性的所有元素，那么也存在长度为 `M` 的子数组具有相反奇偶性的所有元素，因为数组中偶数和奇数元素的计数相同。
*   因此，使长度为 `M` 的子数组交替奇偶校验所需的反转操作计数为 `(M–1)`。

按照以下步骤解决问题:

*   遍历给定的数组，找到数组中每个连续的相同奇数和偶数元素所需的子数组反转计数。
*   让连续奇数和偶数元素的反转计数分别为 `countOdd` 和 `countEven`。
*   反转的最小计数由 `max(countOdd, countEven)` 给出，因为任务是移除所有连续的相同元素，而需要移除的最大连续块数决定了操作数。
