...
> 原文：[https://www.geeksforgeeks.org/find-whether-only-two-parallel-lines-contain-all-coordinates-points-or-not/](https://www.geeksforgeeks.org/find-whether-only-two-parallel-lines-contain-all-coordinates-points-or-not/)
...
**逼近：** 由点`(x1, y1)`和`(x2, y2)`构成的直线的斜率为`(y2-y1)/(x2-x1)`。因为给定的数组由点的坐标组成，如`(i, arr[i])`。所以，`(arr[2]-arr[1]) / (2-1)`是由`(1, arr[1])`和`(2, arr[2])`构成的直线的斜率。仅考虑三个点，例如`P0(0, arr[0])`、`P1(1, arr[1])`和`P2(2, arr[2])`，因为要求只有两条平行线，所以这三个点中的两点必须位于同一条线上。因此，三种可能的情况是：
