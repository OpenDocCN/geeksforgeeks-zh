...
> 原文: [https://www.geeksforgeeks.org/kth-number-from-the-set-of-multiples-of-numbers-a-b-and-c/](https://www.geeksforgeeks.org/kth-number-from-the-set-of-multiples-of-numbers-a-b-and-c/)

给定四个整数 `A`、`B`、`C` 和 `K`。假设 `A`、`B`、`C` 的所有倍数按照排序顺序存储在一个集合中，没有重复，现在的任务是从那个集合中找到第 `K` 个元素。
...
**进场:** [二分搜索法](https://www.geeksforgeeks.org/binary-search/)可以在 `K` 这里使用，找到套内的第 `K` 号。如果是 `A` 的倍数，我们来找第 `K` 号。
将二分搜索法涂抹在 `A` 的倍数上，从 `1` 到 `K` (因为可能会有最大 `K` 的 `A` 的倍数，在所需的集合中找到第 `K` 号)。
现在，对于一个值 `mid`，所需的 `A` 的倍数将是 `A * mid` (说 `X`)。现在的任务是找出这是否是第 `K` 号所需要的套号。可以找到如下所示:

> 求 `A` 的倍数小于 `X` 说 `A1`
> 求 `B` 小于 `X` 说 `B1`
> 求 `C` 小于 `X` 说 `C1`
> 求 `lcm(A， B)` (可被 A 和 B 整除)小于 `X` 说 `AB1`
> 求 `lcm(B，C)` 的倍数小于 `X` 说 `BC1`
> 求 `lcm(C，A)` 的倍数小于 `X` 说 `CA1`
...
对 `B` 执行相同的步骤(如果第 `K` 号不是 `A` 的倍数)，然后对 `C` 执行相同的步骤。
既然第 `K` 号必然是 `A`、`B` 或者 `C` 的倍数，这三个二分搜索法肯定会传回结果。
以下是上述办法的实施:
