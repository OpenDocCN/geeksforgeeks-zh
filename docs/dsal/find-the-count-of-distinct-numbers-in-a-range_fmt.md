...
> 原文：[https://www.geeksforgeeks.org/find-the-count-of-distinct-numbers-in-a-range/](https://www.geeksforgeeks.org/find-the-count-of-distinct-numbers-in-a-range/)

## 问题描述

给定一个大小为 `N` 的数组 `ar`，其中只包含从 0 到 63 的数字，然后你会被问及关于它的问题。
查询如下：

*   `1 X Y`，即将索引 `X` 处的元素更改为 `Y`
*   `2 L R`，即打印介于左 `L` 和右 `R` 之间的不同元素的计数
...
# Building the Segment Tree
def build(left, right, node, ar, seg):
