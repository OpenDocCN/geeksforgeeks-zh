...
> 原文：[https://www.geeksforgeeks.org/a-permutation-where-each-element-indicates-either-number-of-elements-before-or-after-it/](https://www.geeksforgeeks.org/a-permutation-where-each-element-indicates-either-number-of-elements-before-or-after-it/)

给定一组 `n` 个元素。任务是检查给定数组的置换是否存在，使得每个元素指示它之前或之后存在的元素的数量。如果存在，则打印 `Yes`，否则打印 `No`。
