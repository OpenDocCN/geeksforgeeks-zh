...
> 原文: [https://www.geeksforgeeks.org/strings-formed-from-given-characters-without-any-consecutive-repeating-characters/](https://www.geeksforgeeks.org/strings-formed-from-given-characters-without-any-consecutive-repeating-characters/)

给定一个字符串数组 `arr[]` 和一个字符串 `str`，任务是打印数组 `arr` 中符合以下条件的所有字符串:

*   结果字符串不应包含任何连续的重复字符。
*   结果字符串应该只使用字符串 `str` 中的字符。
...
**方法:** 思路是遍历数组，对于每个字符串，检查它是否包含任何连续的重复字符以及字符串 `str` 中提到的字符之外的任何字符。如果上述任一条件通过，则继续检查下一个字符串。否则，打印字符串。
