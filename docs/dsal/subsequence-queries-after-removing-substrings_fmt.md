...
> 原文：[https://www.geeksforgeeks.org/subsequence-queries-after-removing-substrings/](https://www.geeksforgeeks.org/subsequence-queries-after-removing-substrings/)

给定两个字符串 `A` 和 `B`，问题是如果我们去掉子字符串 `A[i]..A[j]`，那么字符串 `B` 是否会是字符串 `A` 的子序列。假设有 `Q` 个查询给出了索引 `i` 和 `j`，并且每个查询都是相互独立的。
...
更有效的方法`是对字符串进行预处理，因为我们必须遇到多个查询。我们可以将字符串 B 中匹配的字符数存储在两个独立的数组中，直到字符串 A 的前向和后向的每个索引。最后我们可以说答案是肯定的，如果下面的等式成立，否则就是否定的: 向前[i-1] +向后[j+1] >= 长度(B)。这很有效，因为我们正在移除 A[i]..A[j] 来自 A，想知道 A 中匹配的 B 的字符数之和来自 A[1]..A[i-1] 和 A[j+1]..A[len]，如果这个和至少是字符串 b 的长度，它就是一个子序列。下面是上述方法的实现:`
