...
> 原文: [https://www.geeksforgeeks.org/inserting-m-into-n-such-that-m-starts-at-bit-j-and-ends-at-bit-i-set-2/](https://www.geeksforgeeks.org/inserting-m-into-n-such-that-m-starts-at-bit-j-and-ends-at-bit-i-set-2/)
...
现在用这个掩码与 `N` 做与运算，`i-1` 到 0 位的掩码将保持不变，而其余位变为 0。因此，我们只剩下 `i-1` 到 0 位。
...
现在，因为 `N` 已清除了从 `j` 到 0 的位，所以只需将 `M` 放入 `N` 中，并将 `M` 左移 `i` 位置，将 `M` 的 MSB 与 `N` 中的 `j` 位置对齐。
