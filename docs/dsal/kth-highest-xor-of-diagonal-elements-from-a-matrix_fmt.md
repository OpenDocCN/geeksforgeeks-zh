...
> 原文：[https://www.geeksforgeeks.org/kth-highest-xor-of-diagonal-elements-from-a-matrix/](https://www.geeksforgeeks.org/kth-highest-xor-of-diagonal-elements-from-a-matrix/)

给定一个大小为 `N * N` 的正方形[矩阵](https://www.geeksforgeeks.org/matrix/) `mat[][]`，任务是计算每一个对角元素的异或值，并求出得到的 `K`<sup>th</sup> 最大异或值。
**注：** 矩阵中有 `2 * N - 1` 条对角线。第 `i` 条对角线的起点为 `(min(N, i), (1 + max(i - N, 0))`。
...
*   对于每一条第 `i`<sup>th</sup> [对角线](https://www.geeksforgeeks.org/matrix/)，起点为 `(min(N, i), (1 + max(i - N, 0))`。
*   将每个对角线的异或存储在一个向量中。
