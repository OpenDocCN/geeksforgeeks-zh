...
> 原文：[https://www.geeksforgeeks.org/vertex-cover-problem-set-2-dynamic-programming-solution-tree/](https://www.geeksforgeeks.org/vertex-cover-problem-set-2-dynamic-programming-solution-tree/)

无向图的[顶点覆盖](https://www.geeksforgeeks.org/vertex-cover-problem-set-1-introduction-approximate-algorithm-2/)是其顶点的子集，使得对于图的每条边`(u, v)`，要么`u`要么`v`处于顶点覆盖中。虽然名称是顶点覆盖，但是集合覆盖了给定图形的所有边。求图的最小尺寸顶点覆盖的问题是[NP 完全](https://www.geeksforgeeks.org/np-completeness-set-1/)的。但是对于树来说，它可以在多项式时间内求解。这篇文章讨论了二叉树的一个解决方案。相同的解决方案可以扩展到 n 元树。

例如，考虑下面的二叉树。最小的顶点覆盖是`{20, 50, 30}`，顶点覆盖的大小是 3。

![LargestIndependentSet1](img/134684d5897d732bccbabc56c30a8f36.png)

这个想法是考虑以下两种可能性：根节点是顶点覆盖的一部分，以及根节点不是顶点覆盖的一部分的递归。

### 1) 根是顶点覆盖的一部分
在这种情况下根覆盖所有子边。我们递归地计算左右子树的顶点覆盖的大小，并将结果加 1（对于根节点）。

### 2) 根不是顶点覆盖的一部分
在这种情况下，根的两个子节点必须包含在顶点覆盖中，以覆盖所有根到子节点的边。我们递归地计算所有子节点的顶点覆盖的大小和结果的子节点数（对于根的两个子节点）。
...
1.  方法将与所讨论的动态编程方法相同。
2.  对于每个节点，如果我们从顶点覆盖中排除这个节点，那么我们必须包括它的相邻节点，如果我们把这个节点包括在顶点覆盖中，那么我们将取取它的邻域的两种可能性中的最小值顶点覆盖中的节点，以获得最小顶点覆盖。
