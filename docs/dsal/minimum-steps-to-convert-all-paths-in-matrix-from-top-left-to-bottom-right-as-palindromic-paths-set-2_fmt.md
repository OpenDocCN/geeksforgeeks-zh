...
## 天真方法:
天真方法请参考[这篇](https://www.geeksforgeeks.org/minimum-steps-to-convert-all-paths-in-matrix-from-top-left-to-bottom-right-as-palindromic-paths/)文章。
...
1.  创建一个 2D 数组`frequency_diagonal`，存储每个选定对角线上所有数字的频率。
2.  每个对角线可以唯一地表示为`(i，j)`的和。
3.  初始化一个`count`变量，该变量存储需要替换数值的单元格总数的计数。
4.  迭代`mat[][]`并增加当前元素在其所属对角线上的频率`((i + j)`值)。
5.  将变量`elements`初始化为 `1`，该变量存储当前选择的每对对角线中的元素数量。
6.  初始化 `start = 0` 和`end = M+N–2`，重复以下步骤直到 `start < end` :
    ...
    *   将`start`增加 `1`，将`end`减少 `1`，如果当前对角线上的元素数量小于矩阵任意对角线上的最大可能元素数量，则将`elements`增加 `1`。
7.  完成上述步骤后，打印总计数的值。
