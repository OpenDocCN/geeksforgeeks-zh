...
我们知道，在任何有向图中，如果图的所有顶点都是某个循环的一部分，则称其处于**强连通分支(SCCs)** 中。当且仅当[图中包含任何桥](https://www.geeksforgeeks.org/bridge-in-a-graph/)时，给定的无向图不形成 SCC。以下是步骤:

*   我们将使用数组 `mark[]` 来存储 DFS 遍历期间的访问节点，`order[]` 来存储访问节点的索引号，`bridge_detect[]` 来存储给定图中存在的任何桥。
