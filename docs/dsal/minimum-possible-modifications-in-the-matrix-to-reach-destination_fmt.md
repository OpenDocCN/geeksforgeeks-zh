...
> **输入:** `mat[][] = {{2, 2, 4}, {1, 1, 1}, {3, 3, 2}};`
> **输出:** `1`
> 将 `mat[1][2]` 的值改为 `4`。所以移动的顺序是 `(0, 0) -> (0, 1) -> (0, 2) -> (1, 2) -> (2, 2)`
>
> **输入:** `mat[][] = {{2, 2, 1}, {4, 2, 3}, {4, 3, 2}}`
> **输出:** `2`

**先决条件:**
1. [迪克斯特拉算法](https://www.geeksforgeeks.org/dijkstras-shortest-path-algorithm-greedy-algo-7/)
...
*   让我们将 2D 矩阵的每个单元视为加权图的一个节点，并且每个节点最多可以有四个相连的节点(可能有四个方向)。每条边的权重如下:
    *   `weight(U, V) = 0`，如果节点 `U` 的移动方向指向 `V`，否则
    *   `weight(U, V) = 1`
