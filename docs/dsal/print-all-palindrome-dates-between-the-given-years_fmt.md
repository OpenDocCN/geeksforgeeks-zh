...
> 原文: [https://www.geeksforgeeks.org/print-all-palindrome-dates-between-the-given-years/](https://www.geeksforgeeks.org/print-all-palindrome-dates-between-the-given-years/)

给定两年 `Y1` 和 `Y2`，其中 `10^3 ≤ Y1 ≤ Y2 ≤ 10^6`，任务是查找并打印给定年份之间回文的所有日期。
...
由于前四位数字（即日和月）必须与后四位数字（年）相反，日期才能成为回文。所以，每年最多会有一个回文日期。为了找到日期，对于每一年，反转年数并检查形成的日期是否有效。如果它是有效的，那么打印它。
