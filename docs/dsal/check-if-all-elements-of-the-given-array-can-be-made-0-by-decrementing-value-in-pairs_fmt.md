...
> 原文: [https://www.geeksforgeeks.org/check-if-all-elements-of-the-given-array-can-be-made-0-by-decrementing-value-in-pairs/](https://www.geeksforgeeks.org/check-if-all-elements-of-the-given-array-can-be-made-0-by-decrementing-value-in-pairs/)

给定一个由正整数组成的数组 `arr[]`，任务是通过执行以下操作检查给定数组的所有元素是否都可以为 0:
*   选择两个索引 `i` 和 `j`，这样 `i != j` 并从 `arr[i]` 和 `arr[j]` 中减去 1
*   上述操作可以执行任何次数
