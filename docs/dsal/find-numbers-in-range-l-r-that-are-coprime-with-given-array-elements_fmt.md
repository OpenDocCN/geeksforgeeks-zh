...
> 原文: [https://www.geeksforgeeks.org/find-numbers-in-range-l-r-that-are-coprime-with-given-array-elements/](https://www.geeksforgeeks.org/find-numbers-in-range-l-r-that-are-coprime-with-given-array-elements/)

给定一个由 `N` 个不同正整数和一个范围`【L，R】`组成的数组`arr[]`，任务是找到给定范围`【L，R】`中与所有数组元素互素的元素。
...
给定的问题可以通过将每个数组元素的所有因子存储在哈希集中来解决。让另一个 `HashSet` 出现，比如说 `S` 由`【L，R】`范围内的数字组成，现在的任务是去掉从这个 `HashSet` `S` 计算出的除数的倍数，得到结果数。按照以下步骤解决问题:
