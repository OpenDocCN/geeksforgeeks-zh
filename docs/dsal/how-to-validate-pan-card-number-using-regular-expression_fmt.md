...
> 原文: [https://www.geeksforgeeks.org/how-to-validate-pan-card-number-using-regular-expression/](https://www.geeksforgeeks.org/how-to-validate-pan-card-number-using-regular-expression/)

给定由字母数字字符组成的字符串 `str`，任务是使用[正则表达式](https://www.geeksforgeeks.org/write-regular-expressions/)检查该字符串是否为有效的 `PAN`(永久账号)卡号。
有效的 PAN 卡号必须满足以下条件:
...
*   用正则表达式匹配给定的字符串，在 Java 中，这可以使用 `Pattern.matcher()` 来完成。
