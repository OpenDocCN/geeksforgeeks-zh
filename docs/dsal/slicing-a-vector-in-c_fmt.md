...
给定向量[中的 N 个整数](https://www.geeksforgeeks.org/vector-in-cpp-stl/) `arr` 和正数 `X` 和 `Y`，任务是将给定向量从索引 `X` 切分到 `Y`。
...
3.  使用向量中的 [`copy()`](https://www.geeksforgeeks.org/ways-copy-vector-c/) 函数在这些迭代器之间复制这些范围内的元素。
...
**方法三:**我们还可以使用 C++ STL 中的内置函数 [`slice()`](https://www.geeksforgeeks.org/stdslice-valarray-slice-selector/) 对给定的向量进行切片。以下是上述方法的实施:
