...
> 原文: [https://www.geeksforgeeks.org/split-a-binary-string-such-that-count-of-0s-and-1s-in-left-and-right-substrings-is-maximum/](https://www.geeksforgeeks.org/split-a-binary-string-such-that-count-of-0s-and-1s-in-left-and-right-substrings-is-maximum/)

给定长度为 `N` 的二进制字符串 `str`，任务是通过将二进制字符串拆分为两个非空子字符串，找到左侧子字符串的 0 计数和右侧子字符串的 1 计数的最大和。
...
3.  遍历二进制字符串，对于每个字符，检查是否是 `'1'`。如果发现为真，则将 `cntOne` 的值增加 `1`。
4.  初始化两个变量，说 `zero` 和 `one`，存储 `0` s 的计数和 `1` s 的计数，直到 `i` 索引。
5.  遍历二进制字符串并更新 `res = max(res, cntOne - one + zero)` 的值。
6.  最后，打印 `res` 的值。
...
# Function to find the maximum sum of count of
# 0s in the left sub and count of 1s in
# the right sub by splitting the string
