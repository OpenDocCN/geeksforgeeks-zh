...
> 原文: [https://www.geeksforgeeks.org/recursively-remove-adjacent-duplicates-given-string/](https://www.geeksforgeeks.org/recursively-remove-adjacent-duplicates-given-string/)
...
解的时间复杂度可以写成 `T(n) = T(n-k) + O(k)`，其中 `n` 是输入字符串的长度，`k` 是相同的第一个字符的个数。递归的解是 `O(n)`。
