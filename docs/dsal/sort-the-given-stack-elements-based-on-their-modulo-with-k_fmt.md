...
> 原文: [https://www.geeksforgeeks.org/sort-the-given-stack-elements-based-on-their-modulo-with-k/](https://www.geeksforgeeks.org/sort-the-given-stack-elements-based-on-their-modulo-with-k/)
...
> **输出:** `12 2 6 10 3`
> `{12, 2, 6, 10, 3}` 是所需的排序顺序，因为 `K = 4` 的这些元素的模是 `{2, 3, 2, 2, 0}`。

> **输入:** `stack = {3, 4, 5, 10, 11, 1}`，`K = 3`
> **输出:** `3 1 4 10 5 11`

**方法:** 使用另一个临时堆栈对堆栈的元素进行[排序的方法](https://www.geeksforgeeks.org/sort-stack-using-temporary-stack/)已经在这篇文章中讨论过了，这里可以使用相同的方法根据元素与 `K` 的模对元素进行排序，唯一的区别是当被比较的元素给出相同的模值时，它们将根据它们的值进行比较。
