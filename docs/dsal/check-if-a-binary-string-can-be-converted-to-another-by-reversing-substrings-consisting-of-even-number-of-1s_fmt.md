...
> 原文: [https://www.geeksforgeeks.org/check-if-a-binary-string-can-be-converted-to-another-by-reversing-substrings-consisting-of-even-number-of-1s/](https://www.geeksforgeeks.org/check-if-a-binary-string-can-be-converted-to-another-by-reversing-substrings-consisting-of-even-number-of-1s/)

给定两个长度为 `N` 的[二进制字符串](https://www.geeksforgeeks.org/tag/binary-string/) `A` 和 `B`，任务是通过反转包含偶数个 `1` 的 `A` 的[子字符串](https://www.geeksforgeeks.org/substring-in-cpp/)来检查字符串 `A` 是否可以转换为 `B`。
...
*   如果字符串 `A` 可以转换成字符串 `B`，那么反过来也成立，因为转换 `A` 到 `B` 的操作可以反过来转换 `B` 到 `A`。
*   `A` 只能等于 `B` 当:
    *   `长度(A) = 长度(B)` 且 `A` 和 `B` 中 `1` 的数量相同
    *   `CNT_A = CNT_B`，其中 `cnt_S` 是 `i` 的位置号，其中 `1 ≤ i ≤ 长度(S)` 和 `(∑_{j=1}^{i} S_j) mod 2 = 1`

按照以下步骤解决问题:

1.  [遍历字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-c/) `A` 和 `B`，并在变量中存储 `1` 的频率，分别记为 `count1A` 和 `count1B`。
2.  初始化一个变量，比如 `temp`，来存储 `1` 的临时计数。
3.  使用变量 `i` 遍历字符串 `A`，并执行以下步骤:
    *   如果当前字符是 `1`，那么将 `temp` 增加 `1`。
    *   否则，如果 `temp` 的值为奇数，则将变量 `odd1A` 增加 `1`。否则，将变量 `even1A` 增加 `1`。
4.  重复上述步骤 `2` 至 `3` 进行字符串 `B` 也。
