...
2. 查询`[l, r]` – 计算范围在`l`和`r`之间的所有数字的总和。
...
第一个查询是计算从`A[1]`到`A[7]`的数字总和，即`6 + 4 + 1 + 10 + 3 + 2 + 4 = 30`。类似，第二个查询结果为 13。对于第三个查询，是更新操作，因此`A[3]`将保持为 1，`A[4]`将变为 4。`A[5]`将变为 2。第四个查询将得出`A[4] = 4`。
