...
> 原文: [https://www.geeksforgeeks.org/number-digits-product-two-numbers/](https://www.geeksforgeeks.org/number-digits-product-two-numbers/)
...
将两个数字相乘，然后通过使用循环构造找到乘积中的位数。取乘积的绝对值求位数。
...
要计算两个数乘积的位数，我们可以使用下面给出的公式:

```
count = floor(log10(abs(a)) + log10(abs(b))) + 1
```
...
本文由**阿育什·乔哈里**供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用`contribute.geeksforgeeks.org`写一篇文章或者把你的文章邮寄到`contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
