...
如果 `n = 0`，则回答为‘否’。否则执行这两个操作，直到 `n` 变为 0。
...
class GFG {
    ...
    // are set or not in the binary representation of 'n'
...
如果 `n = 0`，则回答为‘否’。否则在 `n` 上加 1。让它成为 `num = n + 1`。如果 `num & (num - 1) == 0`，则所有位都被置位，否则所有位都不被置位。
...
我们可以简单地对数字的二进制表示中存在的总集合位进行计数，基于此，我们可以检查该数字是否等于`pow(2, __builtin_popcount(n))`。如果恰好相等，那么我们返回 1，否则返回 0；
