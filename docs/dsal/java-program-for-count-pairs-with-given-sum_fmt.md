...
> 原文：[https://www.geeksforgeeks.org/java-program-for-count-pairs-with-given-sum/](https://www.geeksforgeeks.org/java-program-for-count-pairs-with-given-sum/)

## 问题描述

给定一个整数数组和一个数`sum`，求数组中和等于`sum`的整数对的个数。
...
一个简单的解决方案是遍历每个元素，检查数组中是否有另一个数字可以添加到其中以给出总和。
...
更多详情请参考[给定和计数对](https://www.geeksforgeeks.org/count-pairs-with-given-sum/)的完整文章！