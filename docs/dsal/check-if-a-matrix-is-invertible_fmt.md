...
> 原文: [https://www.geeksforgeeks.org/check-if-a-matrix-is-invertible/](https://www.geeksforgeeks.org/check-if-a-matrix-is-invertible/)
...
其中 `I_n` 表示 `n` 乘 `n` 的单位矩阵。矩阵 `B` 称为 `A` 的逆矩阵。
方阵可逆当且仅当其行列式非零。
...
我们找到矩阵的[行列式](https://www.geeksforgeeks.org/determinant-of-a-matrix/)。然后我们检查行列式值是否为 `0`。如果值为 `0`，那么我们输出，不可逆。
