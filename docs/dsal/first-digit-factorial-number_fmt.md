...
一个简单的解决方案是计算数的阶乘，然后找到其中的第一个数字。
以上解决方案导致很快溢出。更好的解决方案是利用[阶乘包含尾随 0](https://www.geeksforgeeks.org/count-trailing-zeroes-factorial-number/)的事实，移除尾随 0 不会改变第一个数字。例如，对于 `x > 0` 和 `y > 0`，`x * y` 的第一位数字与 `x * y * 100` 相同。
