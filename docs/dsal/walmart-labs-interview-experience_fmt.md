...
> 原文：[https://www.geeksforgeeks.org/walmart-labs-interview-experience/](https://www.geeksforgeeks.org/walmart-labs-interview-experience/)

有 1 轮编码和 4 次 F2F 面试。这是针对软件开发人员职位的（注：-不是针对沃尔玛实验室的研究部门）

## 编码轮次

本轮给出 2 道编码题。所使用的平台是`HackerRank`，我们必须在 1 小时内在他们的办公室完成这两个问题。严禁使用互联网。

1.  找出数组中的最大和，这样就不会有 2 个元素相邻。在这种情况下，还有一个条件，即第一个和最后一个元素也不应该放在一起。这个问题是对此的[修改](https://practice.geeksforgeeks.org/problems/stickler-theif/0)。
...
1.  [Intersection of Two Linked Lists](https://practice.geeksforgeeks.org/problems/intersection-of-two-linked-list/1) 和 [Intersection Point in Y Shapped Linked Lists](https://practice.geeksforgeeks.org/problems/intersection-point-in-y-shapped-linked-lists/1)。
...
1.  设计一个完整的铁路预订系统。代码中检查了以下内容：
    ...
    b. 使用的设计模式（如果有）
    c. 用例是否优化？
2.  关于 [LRU Cache](https://practice.geeksforgeeks.org/problems/lru-cache/1) 实现的问题。
