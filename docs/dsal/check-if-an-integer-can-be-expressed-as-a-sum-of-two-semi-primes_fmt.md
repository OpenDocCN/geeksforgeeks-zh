...
> 原文: [https://www.geeksforgeeks.org/check-if-an-integer-can-be-expressed-as-a-sum-of-two-semi-primes/](https://www.geeksforgeeks.org/check-if-an-integer-can-be-expressed-as-a-sum-of-two-semi-primes/)
...
一个**简单的解决方案**是遍历 `i=1` 到 `N`，检查 `i` 和 `N-i` 是否是半素数。如果是，打印 `i` 和 `n-i`。
