...
*   可以观察到，要移动 `N` 第盘，`(N–1)` 第盘需要移动。因此，要移动`(N–1)` 第个磁盘，`(N–2)` 第个磁盘需要移动。这个过程递归地进行。
...
*   [迭代范围](https://www.geeksforgeeks.org/range-based-loop-c/)`[0, 2^N–1]`，[设置最右侧的未设置位](https://www.geeksforgeeks.org/set-rightmost-unset-bit-2/)，并将其右侧的所有位取消设置。
...
**高效方法:** 上述方法可基于以下观察进行优化: **M** 在范围 `[1, 2^N - 1]` 内，源棒等于 `(M & (M - 1)) % 3`，目的棒等于 `((M | (M - 1)) + 1) % 3`。所以思路是迭代范围 `[1, 2^N - 1]`，打印 `(I & (I - 1)) % 3` 为源棒，`((I | (I - 1)) + 1) % 3` 为目的棒。
