...
> 原文：[https://www.geeksforgeeks.org/check-possible-sort-array-conditional-swapping-adjacent-allowed/](https://www.geeksforgeeks.org/check-possible-sort-array-conditional-swapping-adjacent-allowed/)
...
虽然这些问题乍一看很复杂，但有一个简单的解决方案。如果我们从左到右遍历数组，并确保在到达 `i` 之前对索引 `i` 之前的元素进行排序，那么我们必须具有最大 `arr[0..i-1]`。这个最大值必须小于 `arr[i]` 或者只比 `arr[i]` 大一个。在第一种情况下，我们只是继续前进。在第二种情况下，我们交换并继续前进。
