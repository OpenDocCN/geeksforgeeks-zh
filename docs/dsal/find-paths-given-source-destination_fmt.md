...
> 原文：`https://www.geeksforgeeks.org/find-paths-given-source-destination/`
...
2.  从源开始 DFS 遍历。
3.  继续将访问过的顶点存储在数组或哈希映射中，比如 `path[]`。
4.  如果到达目标顶点，打印 `path[]` 的内容。
5.  重要的是将 `path[]` 中的当前顶点也标记为已访问，这样遍历就不会循环进行。
