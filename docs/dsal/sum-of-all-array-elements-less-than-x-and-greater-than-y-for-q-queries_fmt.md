...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-array-elements-less-than-x-and-greater-than-y-for-q-queries/](https://www.geeksforgeeks.org/sum-of-all-array-elements-less-than-x-and-greater-than-y-for-q-queries/)
...
1.  构建一个[前缀和数组](https://www.geeksforgeeks.org/prefix-sum-array-implementation-applications-competitive-programming/)，其中 `prefix_sum` 表示 `arr[0] + arr[1] + … arr[i]` 的和。
2.  找到最后一个索引 `i`，其值小于 `X`，并提取前缀 sum 到 `i` 索引。在 Python 和 C++中分别使用 [`bisect_left()`](https://www.geeksforgeeks.org/bisect-algorithm-functions-in-python/) 或 [`lower_bound()`](https://www.geeksforgeeks.org/lower_bound-in-cpp/) 可以在 *O(logN)* 复杂度中获取索引。
3.  同样，找到数组中第一个索引 `i`，其值大于 `Y`，并计算总和 `prefix_sum[n-1]–prefix_sum[i-1]`。Python 和 C++中的内置函数[分别 `bisect()`](https://www.geeksforgeeks.org/binary-search-bisect-in-python/) 和 [`upper_bound()`](https://www.geeksforgeeks.org/stdupper_bound-in-cpp/)，在 *O(logN)* 中执行所需操作。
...
def sumLessThanX(ar, x, prefix_sum):
