...
假设有 `n` 个代理和 `n` 个任务。任何代理都可以被分配来执行任何任务，所产生的成本可能会因代理任务的分配而异。要求执行所有任务时，每个任务只分配一个代理，每个代理只分配一个任务，这样可以使分配的总成本最小化。
...
**匈牙利算法，也称为 Munkres 赋值算法**，利用以下多项式运行时复杂度定理（**最坏情况 `O(n^3)`**）和保证最优性：
