...
> 原文：[https://www.geeksforgeeks.org/java-program-for-rotating-a-linked-list/](https://www.geeksforgeeks.org/java-program-for-rotating-a-linked-list/)
...
从头遍历列表，在第 `kth` 个节点停止。存储指向第 `k` 个节点的指针。接下来我们可以使用 `kthNode->next` 获得第 `(k+1)` 个节点。一直遍历到最后，并存储一个指向最后一个节点的指针。最后，如上所述更改指针。
