...
> 原文: [https://www.geeksforgeeks.org/sum-array-elements-first-continuously-increasing-decreasing/](https://www.geeksforgeeks.org/sum-array-elements-first-continuously-increasing-decreasing/)
...
How does it work?
If we take a closer look, we can notice that the sum can be written as.

```
(arr[0] - 1)*n + (1 + 2 + .. x + (x -1) + (x-2) + ..1)
```

Let us understand above result with example `{10, 11, 12, 13, 12, 11, 10}`. If we subtract 9 (`arr[0]-1`) from this array, we get `{1, 2, 3, 2, 1}`.
