...
> 原文: [https://www.geeksforgeeks.org/pairs-with-gcd-equal-to-one-in-the-given-range/](https://www.geeksforgeeks.org/pairs-with-gcd-equal-to-one-in-the-given-range/)
...
因为范围 (`L`, `R`) 中的每个数字在每对中必须恰好包含一次。因此，如果 `L-R` 是一个偶数，那么它是不可能的。如果 `L-R` 是奇数，则打印所有相邻对，因为相邻对的 GCD 始终为 1。
