...
> 原文: [https://www.geeksforgeeks.org/find-the-nth-natural-number-which-is-not-divisible-by-a/](https://www.geeksforgeeks.org/find-the-nth-natural-number-which-is-not-divisible-by-a/)
...
为了解决上面提到的问题，我们必须观察到每`(A–1)`个整数后都有一个间隙，因为跳过了`A`的倍数。为了发现数字`N`位于哪个集合中，我们将`N`除以`(A–1)`，并将其存储在一个变量中，让我们说`商`。现在将`A`与那个变量相乘，并将余数相加，我们将得到结果。
