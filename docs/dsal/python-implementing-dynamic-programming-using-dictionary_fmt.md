...
class Fibonacci(dict):
    ...
    # __missing__ function in python
    def __missing__(self, n):
        # Base case
        if n <= 1:
            # Storing the value in the dictionary before returning
            self[n] = n
            return n
        # Storing the value in the dictionary before returning the value
...
class memoise(dict):
    ...
    # Fibonacci numbers in a Dictionary
...
# number using the above defined decorator
@memoise
def Fibonacci(n):
    # Corner case
    if n < 0:
