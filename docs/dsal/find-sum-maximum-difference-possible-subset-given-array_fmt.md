...
> 原文：[`https://www.geeksforgeeks.org/find-sum-maximum-difference-possible-subset-given-array/`](https://www.geeksforgeeks.org/find-sum-maximum-difference-possible-subset-given-array/)

给我们一个 `n` 个非负整数的数组 `arr[]`（允许重复元素），从给定数组的所有子集中找出最大可能差的和。
假设最大值代表任何子集的最大值，而最小值代表集合的最小值。我们需要找到所有可能子集的 `max(s)-min(s)` 之和。
