...
> 原文:[https://www.geeksforgeeks.org/rearrange-array-elements-to-maximize-the-sum-of-mex-of-all-prefix-arrays/](https://www.geeksforgeeks.org/rearrange-array-elements-to-maximize-the-sum-of-mex-of-all-prefix-arrays/)
...
> `arr[]={0，1，0}`，`Mex(0) + Mex(0，0) + Mex(0，0，1) = 1 + 1 + 2 = 4`
> 因此，排列的最大值为 5，`arr[]={0，1，0}`。

## 天真方法

最简单的方法是[生成给定数组的所有可能排列](https://www.geeksforgeeks.org/heaps-algorithm-for-generating-permutations/) `arr[]`，然后对于每个排列，找到所有前缀数组的 [MEX](https://www.geeksforgeeks.org/construct-mex-array-from-the-given-array/) 的值，同时跟踪整体最大值。迭代所有可能的排列后，打印具有最大值的排列。
...
最优思想是基于这样的观察:当所有不同的元素以递增的顺序排列并且重复出现在数组的末尾时，前缀数组的 MEX 之和将最大。
按照以下步骤解决问题:
...
*   [遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]`，将所有[不同元素](https://www.geeksforgeeks.org/print-distinct-elements-given-integer-array/)推入数组 `ans[]`。
*   再次[遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]`，并将剩余的[重复元素](https://www.geeksforgeeks.org/find-duplicates-in-on-time-and-constant-extra-space/)全部推入数组 `ans[]`。
*   完成以上步骤后，打印数组 `ans[]`。
