...
> 原文：[https://www.geeksforgeeks.org/generate-an-array-having-bitwise-and-of-the-previous-and-the-next-element/](https://www.geeksforgeeks.org/generate-an-array-having-bitwise-and-of-the-previous-and-the-next-element/)

给定一个整数数组`arr[]`，包含`N`个元素，任务是生成另一个数组，该数组具有上一个和下一个元素的（按位）与，但有以下例外。
...
新数组的第一个和第二个元素可以分别计算为`arr[0] & arr[1]`和`arr[N-1] & arr[N-2]`。其余元素可计算为`arr[i-1] & arr[i+1]`。
