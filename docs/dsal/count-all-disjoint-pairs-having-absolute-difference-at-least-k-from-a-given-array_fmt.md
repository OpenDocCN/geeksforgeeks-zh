...
> 原文：[https://www.geeksforgeeks.org/count-all-disjoint-pairs-having-absolute-difference-at-least-k-from-a-given-array/](https://www.geeksforgeeks.org/count-all-disjoint-pairs-having-absolute-difference-at-least-k-from-a-given-array/)

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是计算所有绝对差至少为 `K` 的不相交对。
**注：** 对 `(arr[i], arr[j])` 和 `(arr[j], arr[i])` 视为同一对。
...
**天真方法：** 最简单的方法是生成给定数组的所有可能的对，并对那些绝对差值至少为 `K` 的对进行计数，并跟踪已经成对的元素，使用辅助数组 `visited[]` 来标记成对的元素。
...
高效的思路是利用[二分搜索法](https://www.geeksforgeeks.org/binary-search/)找到第一个至少有 K 个差异的出现。以下是步骤:
