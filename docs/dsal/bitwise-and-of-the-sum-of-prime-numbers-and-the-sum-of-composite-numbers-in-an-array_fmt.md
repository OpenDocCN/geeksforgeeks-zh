...
> 原文: [https://www.geeksforgeeks.org/bitwise-and-of-the-sum-of-prime-numbers-and-the-sum-of-composite-numbers-in-an-array/](https://www.geeksforgeeks.org/bitwise-and-of-the-sum-of-prime-numbers-and-the-sum-of-composite-numbers-in-an-array/)
...
一个简单的解决方案是遍历数组，并不断检查每个元素是否是质数。如果这个数是质数，那么把它加到存储数组中质数总和的 `S1`，或者把它加到存储非质数总和的 `S2`。最后，打印 `S1 & S2`。

**时间复杂度:** `O(N * sqrt(N))`

## 高效方法

使用厄拉多塞的[筛](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)生成数组最大元素之前的所有素数，并将其存储在哈希中。现在，遍历数组，检查数字是否是质数。最后，计算并打印素数之和与复合数之和的按位“与”。
