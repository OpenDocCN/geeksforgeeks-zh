...
> 原文: [https://www.geeksforgeeks.org/find-the-maximum-value-of-y-for-a-given-x-from-given-set-of-lines/](https://www.geeksforgeeks.org/find-the-maximum-value-of-y-for-a-given-x-from-given-set-of-lines/)

给定一组由二维数组表示的线，数组分别由斜率 (`m`) 和截距 (`c`) 定义，以及 `Q` 个查询，使得每个查询包含一个值 `x`。任务是从所有给定的一组线中为 `x` 的每个值找到 `y` 的最大值。
...
天真法是在每一行中代入 `x` 的值，计算所有行的最大值。对于每个查询，将花费 `O(N)` 时间，因此解决方案的复杂性变为 `O(Q * N)`，其中 `N` 是行数，`Q` 是查询数。

## 高效方法

思路是使用[凸包绝招](https://www.geeksforgeeks.org/convex-hull-set-1-jarviss-algorithm-or-wrapping/)：
