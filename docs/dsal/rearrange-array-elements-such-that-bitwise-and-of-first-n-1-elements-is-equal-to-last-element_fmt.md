...
> 原文: [https://www.geeksforgeeks.org/rearrange-array-elements-such-that-bitwise-and-of-first-n-1-elements-is-equal-to-last-element/](https://www.geeksforgeeks.org/rearrange-array-elements-such-that-bitwise-and-of-first-n-1-elements-is-equal-to-last-element/)

给定一个正整数数组 `arr[]` 和 `N`，任务是找到一个排列，使得第一个 `N–1` 元素的按位“与”等于最后一个元素。如果没有这样的安排，那么输出将是 `-1`。
...
*   因此，很明显，将最小的元素放在最后一个索引处，然后检查最后一个元素是否等于第一个 `N–1` 元素的按位“与”。如果是，则打印所需的排列，否则打印 `-1`。
