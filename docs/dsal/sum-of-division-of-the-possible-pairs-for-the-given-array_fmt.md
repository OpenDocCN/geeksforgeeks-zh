...
> 原文: [https://www.geeksforgeeks.org/sum-of-division-of-the-possible-pairs-for-the-given-array/](https://www.geeksforgeeks.org/sum-of-division-of-the-possible-pairs-for-the-given-array/)

给定一个`N`个正整数的数组`arr[]`。对于所有可能的对`(x，y)`，任务是找到`x/y`的总和。
**注:** 如果`(x/y)`的小数部分为`>= 0.5`，则加上`(x/y)`的`ceil`，否则加上`(x/y)`的`floor`。
...
**天真方法:** 想法是生成给定数组中所有可能的对，并为每个对`(x，y)`找到`(x/y)`的总和。
**时间复杂度:** `O(N^2)`
