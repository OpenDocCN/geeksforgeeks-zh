...
> 原文: [https://www.geeksforgeeks.org/minimum-time-required-to-transport-all-the-boxes-from-source-to-the-destination-under-the-given-constraints/](https://www.geeksforgeeks.org/minimum-time-required-to-transport-all-the-boxes-from-source-to-the-destination-under-the-given-constraints/)

给定两个数组，`box[]`和`truck[]`，其中`box[i]`代表第`i`个箱子，`truck[i]`代表第`i`辆卡车可以承载的最大负载。现在每辆卡车从源到目的地需要**1小时**运输一箱，从目的地再回到源需要**1小时**。现在，假设所有的箱子都保存在源头，任务是找到将所有箱子从源头运输到目的地所需的最短时间。

**注意**：总会有一段时间可以运输箱子，在任何时候都只能用卡车运输一个箱子。

**示例:**

> **输入:** `box[] = {7, 6, 5, 4, 3}`，`truck[] = {10, 3}`
> **输出:** 7
> 第1小时: `truck[0]`运载`box[0]`，`truck[1]`运载`box[4]`
> 第2小时: 两辆卡车都回到了源位置。
> 现在，`truck[1]`不能再运送箱子了，因为所有剩余的箱子的重量都超过了`truck[1]`的容量。
> 所以，`truck[0]`将在总共四个小时内搬运`box[1]`和`box[2]`。(源-目的地，然后目的地-源)
> 最后，`box[3]`将需要一个小时才能到达目的地。
> 所以，总耗时= 2 + 4 + 1 = 7
>
...
思路是用二分搜索法对两个数组进行排序。这里的下限将是`0`，上限将是`2 * size of box[]`，因为在最坏的情况下，运输所有箱子所需的时间将是`2 * size of box[]`。现在计算中间值，对于每个中间值，检查装载机是否能及时运输所有箱子=`mid`。如果是，则将上限更新为`mid–1`，如果不是，则将下限更新为`mid + 1`。
...
以下是一个使用 PHP 和 JavaScript 实现的算法示例，用于计算运输箱子所需的最短时间。
