...
> 原文: [https://www.geeksforgeeks.org/check-if-a-number-starts-with-another-number-or-not/](https://www.geeksforgeeks.org/check-if-a-number-starts-with-another-number-or-not/)

给定两个数字 `A` 和 `B`，其中 `A > B`，任务是检查 `B` 是否是 `A` 的前缀。打印“是”如果是前缀，否则打印“否”。
...
使用内置函数 `std::boost::algorithm::starts_with()`，可以检查任意字符串是否包含另一字符串的前缀。
