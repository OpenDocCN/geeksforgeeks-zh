...
> 原文:[https://www.geeksforgeeks.org/secretary-problem-optimal-stopping-problem/](https://www.geeksforgeeks.org/secretary-problem-optimal-stopping-problem/)
...
在数学上，最优停止或提前停止理论关注的是选择采取某一特定行动的时间的问题，目的是获得最大的预期报酬或最小的预期成本。
...
该策略不选择样本量为 `n/e`，而是选择 `n` 的平方根作为最优样本量(即 `sqrt(n)` )。

本文由 `舒巴姆拉纳` 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 `contribute.geeksforgeeks.org` 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
