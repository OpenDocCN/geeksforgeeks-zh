...
> 原文: [https://www.geeksforgeeks.org/kth-smallest-or-largest-element-in-unsorted-array-set-4/](https://www.geeksforgeeks.org/kth-smallest-or-largest-element-in-unsorted-array-set-4/)
...
思路是使用[计数排序](https://www.geeksforgeeks.org/counting-sort/)的概念。以下是步骤:
...
## 时间复杂度
`O(N+MaxElement)` 其中 `N` 是给定数组中的元素个数，`MaxElement` 是数组中的最大个数。伪线性时间复杂度。

## 辅助空间
`O(M)` 其中 `M` 是给定数组中的最大元素。