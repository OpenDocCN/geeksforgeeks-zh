...
> 原文: [https://www.geeksforgeeks.org/sum-first-k-even-length-palindrome-numbers/](https://www.geeksforgeeks.org/sum-first-k-even-length-palindrome-numbers/)
...
一个天真的方法将是检查每一个偶数长度的数字，如果它是一个回文数字，那么我们总结它。我们对前 `K` 个偶数长度的回文数字重复同样的过程，并将它们相加得到总和。
...
所以第 `n` 个数字会是 `int(string(n)+rev(string(n)))`。
整数到字符串、字符串到整数的转换见此处。
以下是上述方法的实施:
