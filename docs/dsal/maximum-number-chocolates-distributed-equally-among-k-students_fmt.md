...
考虑所有子数组的总和。选择最大和。使其为`maxSum`。返回`maxSum / k`。时间复杂度为`O(N ^ 2)`。

## 方法 2（有效方法）

创建数组`sum[]`，其中`sum[i]`存储`sum(arr[0] + .. arr[i])`。创建一个具有作为`ele, idx`的元组的哈希表，其中`ele`表示`sum[i] % k`的元素，而`idx`表示从左到右遍历数组`sum[]`时元素的第一次出现的索引。现在，从 0 到`n`遍历`sum[i]`，然后执行以下步骤。
