...
> 原文: [https://www.geeksforgeeks.org/queries-to-calculate-sum-of-squares-of-ascii-values-of-characters-of-a-substring-with-updates/](https://www.geeksforgeeks.org/queries-to-calculate-sum-of-squares-of-ascii-values-of-characters-of-a-substring-with-updates/)

给定长度为 `N` 的字符串 `S` 和由 `M` 个查询组成的二维数组 `Q[][]`，查询类型如下：

*   **查询(`"U"`, `"I"`, `"X"`):** 用字符 `X` 更新索引 `I` 处的字符。
*   **查询(`"S"`, `"L"`, `"R"`):** 打印子串 `{S[L], ..., S[R]}` 中字符位置的平方和，按英文字母排序。

## 示例

**输入:** `S = "geeksforgeeks"`, `Q[][] = { {"S", "0", "2"}, {"S", "1", "2"}, {"U", "1", "a"}, {"S", "0", "2"}, {"S", "4", "5"} }`
**输出:**
```
99
50
...
```
**解释:**
对于查询 `("S", "0", "2")`，子串 `"gek"` 中字符位置的平方和 = 7*7 + 5*5 + 11*11 = 99。
对于查询 `("S", "1", "2")`，子串 `"ee"` 中字符位置的平方和 = 5*5 + 5*5 = 50。
对于查询 `("U", "1", "a")`: 将 `S[1]` 替换为 `"a"` 将 `S` 修改为 `"gaeksforgeeks"`。
对于查询 `("S", "0", "2")`，子串 `"gae"` 中字符位置的平方和 = 7*7 + 1*1 + 5*5 = 75。
对于查询 `("S", "4", "5")`，子串 `"ks"` 中字符位置的平方和 = 11*11 + 19*19 = 397。

**输入:** `S = "geeksforgeeks"`, `Q[][] = { {"S", "1", "2"} }`
**输出:** 50

## 天真方法

解决问题最简单的方法是遍历数组 `Q[][]`。对于每个类型为 `'S'` 的查询，只需遍历子串 `{S[L], ..., S[R]}` 并打印英文字母中字符位置的平方和。在每次执行类型为 `'U'` 的查询时，只需将 `X` 分配给 `S[I]`。
...
上述方法可以通过使用线段树数据结构进行优化。按照以下步骤解决问题:

*   初始化一个线段树，比如 `tree[]`，其中每个节点存储其子树中英文字母字符位置的平方和。
*   对于查询 `("U", "I", "X")`，定义一个更新函数，类似于求和范围查询的更新函数。更新 `S[i] = X`，然后调用 `update()` 函数更新线段树。
*   对于查询 `("S", "L", "R")`，定义一个函数 `query()`，类似于求和范围查询，然后对子串 `{S[L], ..., S[R]}` 打印调用 `query()` 函数得到的和。
