...
> 原文：[https://www.geeksforgeeks.org/sum-of-similarities-of-string-with-all-of-its-suffixes/](https://www.geeksforgeeks.org/sum-of-similarities-of-string-with-all-of-its-suffixes/)
...
> **输入：** `str = "ababa"`
> **输出：** `9`
> `str` 的后缀是“ababa”、“baba”、“aba”、“ba”和“a”。这些字符串与原字符串“ababa”的相似之处分别是 5、0、3、0 & 1。
> 于是，答案是 5 + 0 + 3 + 0 + 1 = 9。
> **输入：** `str = "aaabab"`
> **输出：** `13`

**方法：** 使用 [Z 算法](https://www.geeksforgeeks.org/z-algorithm-linear-time-pattern-searching-algorithm/)计算 Z 数组——对于字符串 `[0..n-1]`，Z 数组与字符串长度相同。Z 数组的元素 `Z[i]` 存储从字符串 `[i]` 开始的最长子串的长度，该子串也是字符串 `[0..n-1]` 的前缀。Z 数组的第一个条目是字符串的长度。
