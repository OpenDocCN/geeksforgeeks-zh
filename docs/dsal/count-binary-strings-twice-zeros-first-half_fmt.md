...
> 原文：[https://www.geeksforgeeks.org/count-binary-strings-twice-zeros-first-half/](https://www.geeksforgeeks.org/count-binary-strings-twice-zeros-first-half/)
...
我们可以生成所有长度为 `N` 的二进制字符串，然后逐一检查所选字符串是否符合给定的场景。但是这种方法的时间复杂度是指数的，并且是 `O(N * 2^N)`。
...
1.  前半串包含两个“0”，后半串包含一个“0”的串数等于 `nC2 * nC1`。
2.  前半弦包含四个“0”后半弦包含两个“0”的弦的数量等于 `nC4 * nC2`。
3.  前半串包含 6 个“0”，后半串包含 3 个“0”的串数等于 `nC6 * nC3`。

我们重复上面的过程，直到前半部分的“0”变成 `n`，如果 `n` 是偶数，或者 `(n-1)` 如果 `n` 是奇数。
那么，我们最终的答案是 `Σ(nC(2 * i) * nCi)`，对于 `2 <= 2*i <= n`。
...
注意：可以使用动态规划技术来预先计算系数 `(n, i)`，即 `nCi`。
