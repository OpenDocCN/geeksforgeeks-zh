...
> 原文: [https://www.geeksforgeeks.org/program-dot-product-cross-product-two-vector/](https://www.geeksforgeeks.org/program-dot-product-cross-product-two-vector/)

有两个向量 `A` 和 `B`，我们要求两个向量数组的点积和叉积。点积也称为标量积，叉积也称为向量积。

## 点积–

让我们给出两个向量 `A` = a1 * i + a2 * j + a3 * k 和 `B` = b1 * i + b2 * j + b3 * k，其中 i、j 和 k 是沿 x、y 和 z 方向的单位向量。那么点积计算为 `dot product = a1 * b1 + a2 * b2 + a3 * b3`
...
假设我们已经给出了两个向量 `A` = a1 * i + a2 * j + a3 * k 和 `B` = b1 * i + b2 * j + b3 * k，那么叉积计算为 `cross product = (a2 * b3 - a3 * b2) * i + (a3 * b1 - a1 * b3) * j + (a1 * b2 - a2 * b1) * k`，其中 `[(a2 * b3 - a3 * b2), (a3 * b1 - a1 * b3), (a1 * b2 - a2 * b1)]` 是沿 i、j 和 k 方向的单位向量的系数。
...
## PHP（Professional Hypertext Preprocessor 的缩写）
