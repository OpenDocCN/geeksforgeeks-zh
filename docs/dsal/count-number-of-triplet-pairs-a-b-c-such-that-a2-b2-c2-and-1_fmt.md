...
> 原文: [https://www.geeksforgeeks.org/count-number-of-triplet-pairs-a-b-c-such-that-a2-b2-c2-and-1/](https://www.geeksforgeeks.org/count-number-of-triplet-pairs-a-b-c-such-that-a2-b2-c2-and-1/)

给定一个整数 `N`，任务是统计三胞胎 `(a, b, c)` 的数量，使得 `a^2 + b^2 = c^2` 且 `1 <= a <= b <= c <= N`。
...
*   找到直到 `n^2` 的所有完美方块，并将其保存到数组列表中。
*   现在，对于从 `1` 到 `n` 的每一个 `a`，执行以下操作:
    *   从之前计算的完美方块列表中选择 `c^2`。
    *   那么 `b^2` 可以计算为 `b^2 = c^2 - a^2`。
    *   现在检查上一步计算的 `a <= b <= c` 和 `b^2` 是否一定是一个完美的正方形。
    *   如果满足上述条件，则增加计数。
*   最后打印计数。
