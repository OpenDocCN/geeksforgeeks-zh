...
> 原文: [https://www.geeksforgeeks.org/python-sort-tuples-increasing-order-key/](https://www.geeksforgeeks.org/python-sort-tuples-increasing-order-key/)
...
给定元组，我们需要根据任何给定的关键字对它们进行排序。这可以使用 `sorted()` 函数来完成，在该函数中，我们使用 `key=last` 对它们进行排序，并将 `last` 存储为键索引，根据该索引我们必须对给定的元组进行排序。
