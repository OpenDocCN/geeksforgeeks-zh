...
> 原文:[https://www.geeksforgeeks.org/find-number-x-whose-sum-digits-equal-n/](https://www.geeksforgeeks.org/find-number-x-whose-sum-digits-equal-n/)

给定一个正数 `n`。我们需要找到一个数，这样这些数的位数之和等于 `n`。如果没有这样的数，打印 `-1`。这里 `N ∈ [1, 1000000000]`。
...
**方法一:(幼稚进场)**
我们这里已经讨论了进场。这种方法可能不适用于像 `10^9` 这样大的 `N`。
...
这种方法的最大复杂度可以是`O(100*len)`，其中 `len` 是数字中的位数 `max(len) = 9`。由此可见复杂性几乎可以说是`O(len)`。