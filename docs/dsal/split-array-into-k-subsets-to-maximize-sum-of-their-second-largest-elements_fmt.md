...
> 原文：[https://www.geeksforgeeks.org/split-array-into-k-subsets-to-maximize-sum-of-their-second-largest-elements/](https://www.geeksforgeeks.org/split-array-into-k-subsets-to-maximize-sum-of-their-second-largest-elements/)

给定一个由 `N` 个整数和一个整数 `K` 组成的数组 `arr[]`，任务是将数组拆分为 `K` 个子集（`N % K = 0`），使得所有子集的第二大元素之和最大化。
...
思路是对数组进行排序，在反向遍历数组时，每遇到一个第二元素，就不断增加，从数组中第二个最大的元素开始，精确到 `K` 次。按照以下步骤解决问题：
