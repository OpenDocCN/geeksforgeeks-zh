...
> 原文: [https://www.geeksforgeeks.org/best-time-to-buy-and-sell-stock/](https://www.geeksforgeeks.org/best-time-to-buy-and-sell-stock/)
...
> **输出:** `5`
> **说明:**
> 该股最低价格在第 2 日，即价格= 1。从第 2 天开始，股票的最高价格见证在第 5 天，即价格= 6。
> 因此，最大可能利润= 6–1 = 5。
>
> **输入:** `价格[] = {7，6，4，3，1}`
> **输出:** `0`
...
**方法:** 给定的问题可以基于寻找在较大数目之前出现的较小数目的两个阵列元素之间的[最大差值](https://www.geeksforgeeks.org/maximum-difference-between-two-elements/)的思想来解决。因此，这个问题可以简化为为每对指数 `i` 和 `j`，找到 `max⁡(prices[j]−prices[i])`，使得 `j > i`。
...
给定一个长度为 `N` 的数组 `价格[]`，代表不同日期股票的价格，任务是使用允许任意数量交易的交易找到不同日期买卖股票的最大可能利润。
...
**方法:** 想法是保持一个布尔值，表示当前是否有任何正在进行的购买。如果是，那么在目前的状态下，股票可以出售以实现利润最大化，或者在不出售股票的情况下转移到下一个价格。否则，如果没有交易发生，可以不买而买当前股票或移到下一个价格。
