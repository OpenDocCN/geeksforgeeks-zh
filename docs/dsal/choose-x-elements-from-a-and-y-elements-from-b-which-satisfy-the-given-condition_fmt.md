...
> 原文: [https://www.geeksforgeeks.org/choose-x-elements-from-a-and-y-elements-from-b-which-satisfy-the-given-condition/](https://www.geeksforgeeks.org/choose-x-elements-from-a-and-y-elements-from-b-which-satisfy-the-given-condition/)

## 问题描述

给定两个数组 `A[]` 和 `B[]` 以及两个整数 `X` 和 `Y`，任务是从 `A[]` 中选择 `X` 元素，从 `B[]` 中选择 `Y` 元素，使得从 `A[]` 中选择的所有元素小于从 `B[]` 中选择的所有元素。
...
**方法:** 为了满足给定的条件，最小的 `X` 元素必须从 `A[]` 中选择，最大的 `Y` 元素必须从 `B[]` 中选择。这可以通过对两个数组进行排序，然后从 `A[]` 中选择第 `X` 小的元素（记为 `xSmall`）和从 `B[]` 中选择第 `Y` 大的元素（记为 `yLarge`）来完成。
