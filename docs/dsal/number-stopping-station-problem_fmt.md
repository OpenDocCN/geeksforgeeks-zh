...
> 原文: [https://www.geeksforgeeks.org/number-stopping-station-problem/](https://www.geeksforgeeks.org/number-stopping-station-problem/)
...
如果你知道关于不可区分的球组合成可区分的盒子，那么你可以简单的使用 `^{(n-p+1)}C_p`。在这个问题中，`n` 是站点的数量，`p` 是您想要停止的站点的数量。在这里，停靠站就像不可区分的球，不停靠站就像可区分的盒子。
因此，`^{(12-8+1)}C_4` = `^{9}C_4` = `126`。
