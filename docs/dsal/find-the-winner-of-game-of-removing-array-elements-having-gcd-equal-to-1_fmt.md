...
> 原文: [https://www.geeksforgeeks.org/find-the-winner-of-game-of-removing-array-elements-having-gcd-equal-to-1/](https://www.geeksforgeeks.org/find-the-winner-of-game-of-removing-array-elements-having-gcd-equal-to-1/)

给定一个大小为 `N` 的数组 `arr[]`，任务是当两个玩家按照以下规则进行最佳游戏时，找到游戏的赢家：
...
玩家 1 和玩家 2 的最佳方式是总是移除在大多数数组元素中至少有一个公共[素因子](https://www.geeksforgeeks.org/print-all-prime-factors-of-a-given-number/)的数组元素。按照以下步骤解决问题:
...
*   [遍历地图](https://www.geeksforgeeks.org/traversing-a-map-or-unordered_map-in-cpp-stl/)找到 `map` 中存在的最大值，说 `maxCnt`。
*   如果 `N` 为奇数且 `maxCnt` 等于 `N`，或者如果 `N` 为偶数且 `maxCnt >= N–1`，则 **玩家 1** 获胜。
*   否则 **玩家 2** 获胜。
