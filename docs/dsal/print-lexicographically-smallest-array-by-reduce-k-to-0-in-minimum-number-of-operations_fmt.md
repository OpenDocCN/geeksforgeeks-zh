...
> 原文: [https://www.geeksforgeeks.org/print-lexicographically-smallest-array-by-reduce-k-to-0-in-minimum-number-of-operations/](https://www.geeksforgeeks.org/print-lexicographically-smallest-array-by-reduce-k-to-0-in-minimum-number-of-operations/)

给定一个[数组](https://www.geeksforgeeks.org/array-data-structure/) `arr[]` 和一个整数 `K`，任务是通过执行以下操作将 `K` 的值减少到 `0`。一个操作定义为选择 `2` 个索引 `i, j` 并从 `arr[i]`（即 `arr[i] = arr[i] - X`）中减去 `arr[i]` 和 `K` 的最小值（即 `X = min(arr[i], K)`），并将最小值加到 `arr[j]`（`arr[j] = arr[j] + X`）。请注意[数组](https://www.geeksforgeeks.org/array-data-structure/) `arr[]` 的元素不能为负。
...
> **输出:** `0 0 6`
> **解释:**
> 操作 1: 选择指数 `0` 和 `2`，然后减去 `arr[0]`（`=1`）和 `K`（`=15`）的最小值，即 `arr[0]`（`=1`）。现在修改后的数组是 `{0, 2, 4}`。
> 操作 2: 选择指数 `1` 和 `2`，然后从 `arr[1]` 中减去 `arr[1]`（`=2`）和 `K`（`=15`）的最小值，即 `arr[1]`（`=2`），并将其加到 `arr[2]`（`=4`）。现在修改后的数组是 `{0, 0, 6}`。
> 现在，对修改后的数组进行排序并打印。
...
*   [排序](https://www.geeksforgeeks.org/sort-c-stl/)数组 `arr`。
