...
> 原文：[https://www.geeksforgeeks.org/find-the-lexicographical-next-balanced-bracket-sequence/](https://www.geeksforgeeks.org/find-the-lexicographical-next-balanced-bracket-sequence/)

给定一个平衡括号序列作为包含字符`(`或`)`的字符串`str`，任务是找到下一个字典顺序平衡序列（如果可能的话），否则打印`-1`。
