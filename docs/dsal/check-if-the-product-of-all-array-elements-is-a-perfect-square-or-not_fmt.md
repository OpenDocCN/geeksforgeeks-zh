...
> 原文: [https://www.geeksforgeeks.org/check-if-the-product-of-all-array-elements-is-a-perfect-square-or-not/](https://www.geeksforgeeks.org/check-if-the-product-of-all-array-elements-is-a-perfect-square-or-not/)

给定一个由正整数 `N` 组成的数组 `arr[]`，任务是检查给定数组 `arr[]`的所有元素的乘积是否为完美平方。如果发现是真的，则打印“是”。否则，打印否。
...
找出数组所有元素的乘积，试着找出这是否是一个完美的正方形。但是这种方法的问题是，产品可能太大，我们无法储存，因此这种方法会失败。
...
这种方法基于数学观察。如果一个数的所有质因数都是偶数的幂，那么这个数就是一个完美的正方形。我们将使用这个概念来发现产品是否是一个完美的正方形。遵循以下步骤:

- 创建一个频率数组来存储质因数的幂。
- 开始遍历数组。
- 对于每个元素 `arr[i]`，使用厄拉多塞的筛找出 `arr[i]` 的素因子的次幂，并在频率数组中添加。
- 遍历完数组后，开始遍历频率数组。
- 如果任意元素(1 除外)有奇频则返回假，否则返回真。
