...
> 原文: [https://www.geeksforgeeks.org/check-if-an-array-can-be-split-into-k-non-overlapping-subarrays-whose-bitwise-and-values-are-equal/](https://www.geeksforgeeks.org/check-if-an-array-can-be-split-into-k-non-overlapping-subarrays-whose-bitwise-and-values-are-equal/)

给定一个大小为 `N` 的数组 `arr[]` 和一个正整数 `K`，任务是检查该数组是否可以拆分为 `K` 个非重叠和非空的子数组，使得所有子数组的位与相等。如果发现为真，则打印 `YES`。否则，打印 `NO`。
...
解决这个问题最简单的方法就是将数组以各种可能的方式拆分成 `K` 个子数组，检查所有 `K` 个子数组的 Bitwise AND 是否相等。如果发现任何分割都是真的，则打印 `YES`。否则，打印 `NO`。
...
为了优化上述方法，想法是利用这样一个事实：如果子数组的至少一个元素的 `i-th` 位是 `0`，那么该子数组的位与的 `i-th` 位也是 `0`。按照以下步骤解决问题:

*   初始化一个变量，说 `flag`，检查数组是否可以被拆分为 `K` 个子数组，使得所有子数组的位与相等。
*   初始化一个 2D 数组，比如说 `pref[][]`，其中 `pref[i][j]` 存储直到 `i-th` 索引的连续数组元素的计数，该索引的 `j-th` 位被设置。
*   迭代范围 `[0, N–K]`。对于第 `i-th` 元素的每个 `j-th` 位，检查以下情况:
    *   如果设置了直到第 `i-th` 索引的所有数组元素的第 `j-th` 位，并且在第 `i-th` 索引之后的数组的至少一个元素的第 `j-th` 位未设置，则更新 `flag = false`。
    *   如果直到第 `i-th` 索引的至少一个数组元素的第 `j-th` 位没有被设置，并且在第 `i-th` 索引之后的所有数组元素的第 `j-th` 位被设置，则更新 `flag = false`。
*   最后，检查 `flag` 是否等于 `true`。如果发现为真，则打印 `YES`。
*   否则，打印 `NO`。
