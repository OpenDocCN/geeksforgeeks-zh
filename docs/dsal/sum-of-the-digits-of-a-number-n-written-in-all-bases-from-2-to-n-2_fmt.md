...
> 原文: [https://www.geeksforgeeks.org/sum-of-the-digits-of-a-number-n-written-in-all-bases-from-2-to-n-2/](https://www.geeksforgeeks.org/sum-of-the-digits-of-a-number-n-written-in-all-bases-from-2-to-n-2/)
...
*   对于从 `2` 到 `(n / 2)` 的每个基数，用以下公式计算特定基数中 `n` 的位数:
    *   将 `n` 除以 `base` 计算 `remainder`，余数为该基数中 `n` 的一位数。
    *   将数字加到 `sum` 上，更新 n 为 `(n = n / base)`。
    *   重复上述步骤，同时 `n > 0`
*   打印上一步计算的 `sum`。
