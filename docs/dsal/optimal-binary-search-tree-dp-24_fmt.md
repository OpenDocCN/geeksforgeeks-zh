...
> 原文：[https://www.geeksforgeeks.org/optimal-binary-search-tree-dp-24/](https://www.geeksforgeeks.org/optimal-binary-search-tree-dp-24/)

给定一个排序的数组 `keys[0..n-1]` 个搜索关键字和一个数组 `freq[0..n-1]`，其中 `freq[i]` 是搜索 `keys[i]` 的次数。构建所有关键字的二叉查找树，以便所有搜索的总成本尽可能小。
...
上面公式的思路很简单，我们一个个尝试把所有节点都作为根（第二项 `r` 从 `i` 到 `j` 不等）。当我们将 `r` 节点作为根时，我们递归地计算从 `i` 到 `r-1` 和从 `r+1` 到 `j` 的最优成本。我们将从 `i` 到 `j` 的频率之和相加（见上述公式中的第一项）。

**将 `i` 到 `j` 的频率之和相加的原因：**

这可以分为两部分，一部分是 `freq[r]` 加上从 `i` 到 `j` 除 `r` 之外的所有元素的频率之和。添加 `freq[r]` 项是因为它将是根，这意味着级别为 1，因此 `freq[r]*1=freq[r]`。现在实际部分来了，我们将剩余元素的频率相加，因为当我们以 `r` 为根时，除了 `r` 之外的所有元素都比子问题中计算的低 1 级。让我更清楚地说，为了计算 `optCost(i, j)`，我们假设 `r` 作为根，并计算所有 `i <= r <= j`。这里对于每个子问题我们都是选择一个节点作为根。但实际上，子问题的根及其所有后代节点的级别将比父问题根的级别大 1。因此，除了 `r` 之外的所有节点的频率都应该被加上，这解释了它们相对于在子问题中假设的级别的下降。

## 2) 重叠子问题

`min` 的 `opt(i, r-1) + opt(r+1, j)` 值。以下是简单遵循上述递归结构的递归实现。
