...
# 分割二叉搜索树（BST）的Python实现

```python
# splitting index of the array
def getSplittingIndex(inOrder, index, k):
    for i in range(index):
        if (inOrder[i] >= k):
            return i - 1
    return index - 1

# Function to create the Balanced Binary search tree
...
# Function to split the BST into two Balanced BST
def splitBST(root, k):
    global index
    ...
    # Take auxiliary array for storing The inorder traversal of BST1
    inOrder = [0 for i in range(numNode + 1)]
    index = 0

    # Function call for storing inorder traversal of BST1
    storeInorder(root, inOrder)

    # Function call for getting splitting index
    splitIndex = getSplittingIndex(inOrder, index, k)
