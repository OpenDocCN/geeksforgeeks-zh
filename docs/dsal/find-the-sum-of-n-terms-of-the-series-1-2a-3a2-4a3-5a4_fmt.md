# 求数列 1、2a、3a²、4a³、5a⁴、… 的 n 项之和

> 原文: [https://www.geeksforgeeks.org/find-the-sum-of-n-terms-of-the-series-1-2a-3a2-4a3-5a4/](https://www.geeksforgeeks.org/find-the-sum-of-n-terms-of-the-series-1-2a-3a2-4a3-5a4/)

给定一系列 `1, 2a, 3a², 4a³, 5a⁴,.....` 和 `a` 的值，求该系列前 `n` 项的 `和`。
...
> `a = 3` 和 `n = 4`
>
> 循环将被执行 `n` 次，在这种情况下为 4 次。
>
> **循环 1:** 最初 `a = 1`，`n = 0`，`sum = 0` 的值
>
> 1.  `a^n = 3^0 = 1`
> 2.  `a^n * (n+1) = 3^0 * (0+1) = 1 * 1 = 1`
> 3.  `sum = sum + a^n * (n+1) = 0 + 1 = 1`
> 4.  将 `n` 增加 1。
>
> **循环 2:** `a = 3`，`n = 1`，`sum = 1` 的值
>
> 1.  `a^n = 3^1 = 3`
> 2.  `a^n * (n+1) = 3^1 * (1+1) = 3 * 2 = 6`
> 3.  `sum = sum + a^n * (n+1) = 1 + 6 = 7`
> 4.  `n` 的值增加 1。
>
> **循环 3:** `a = 3`，`n = 2`，`sum = 7` 的值
>
> 1.  `a^n = 3^2 = 9`
> 2.  `a^n * (n+1) = 3^2 * (2+1) = 9 * 3 = 27`
> 3.  `sum = sum + a^n * (n+1) = 7 + 27 = 34`
> 4.  将 `n` 增加 1。
>
> **循环 4:** `a = 3`，`n = 3`，`sum = 34` 的值
>
> 1.  `a^n = 3^3 = 27`
> 2.  `a^n * (n+1) = 3^3 * (3+1) = 27 * 4 = 108`
> 3.  `sum = sum + a^n * (n+1) = 34 + 108 = 142`
> 4.  将 `n` 的值增加 1。
...
在这种方法中，使用几何级数的概念提出了一种有效的解决方案。`几何级数(g.p)`中`n`项与`第一项 a` 和`公比 r` 的级数之和如下:
