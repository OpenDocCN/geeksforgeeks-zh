...
> 原文: [https://www.geeksforgeeks.org/program-for-newton-raphson-method/](https://www.geeksforgeeks.org/program-for-newton-raphson-method/)
...
3.  牛顿拉夫森法要求导数。有些功能可能很难无法区分。
4.  对于许多问题，牛顿-拉夫森方法比上述两种方法收敛得更快。
5.  此外，它可以识别重复的根，因为它没有明确地寻找 `f(x)` 符号的变化。

**公式:**
从初猜 `x1` 开始，牛顿拉夫森法用下面的公式求 `x` 的下一个值，即 `xn+1` 从前一个值 `xn` 开始。
...
**算法:**
输入: 初始 `x`，`func(x)`，`derivfunc(x)`
输出: `func()` 的根

1.  计算给定初始 `x` 的 `func(x)` 和 `derivfunc(x)` 的值
...
想法是在点 `x1` 处画一条与 `f(x1)` 相切的线。切线与 `x` 轴相交的点应该是比 `x1` 更好的根估计。称此点为 `x2`。计算 `f(x2)`，在 `x2` 处画一条切线。

![newtonRaphsonMethod](img/7f8283121a2142b3c91067b7225b11e6.png)

我们知道从 `(x1, f(x1))` 到 `(x2, 0)` 的直线斜率是 `f'(x1)`，其中 `f'` 代表 `f` 的导数。
...
[http://www.cae.tntech.edu/Members/renfro/me2000/lectures/2004-09-07_handouts.pdf/at_download/file](http://www.cae.tntech.edu/Members/renfro/me2000/lectures/2004-09-07_handouts.pdf/at_download/file)
