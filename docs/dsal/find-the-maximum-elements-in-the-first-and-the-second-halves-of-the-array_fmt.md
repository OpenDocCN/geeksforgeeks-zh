...
> 原文：[https://www.geeksforgeeks.org/find-the-maximum-elements-in-the-first-and-the-second-halves-of-the-array/](https://www.geeksforgeeks.org/find-the-maximum-elements-in-the-first-and-the-second-halves-of-the-array/)
...
计算数组的中间索引为 `mid = N / 2`。现在，如果 `N` 为偶数，第一半元素将出现在子数组 `arr[0…mid-1]` 和 `arr[mid…N-1]` 中。
如果 `N` 为奇数，则两半为 `arr[0…mid]` 和 `arr[mid…N-1]`。
