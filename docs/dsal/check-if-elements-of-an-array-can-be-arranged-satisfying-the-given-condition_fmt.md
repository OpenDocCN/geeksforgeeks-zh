...
> 原文: [https://www.geeksforgeeks.org/check-if-elements-of-an-array-can-be-arranged-satisfying-the-given-condition/](https://www.geeksforgeeks.org/check-if-elements-of-an-array-can-be-arranged-satisfying-the-given-condition/)
...
我们按照升序检查元素。当我们检查一个元素 `k` 而它没有被使用时，它必须与 `2 * k` 配对。我们将尝试先安排 `k` 后安排 `2 * k` 但是如果不能，那么答案就是 `False`。最后，如果所有操作都成功，则打印 `True`。
