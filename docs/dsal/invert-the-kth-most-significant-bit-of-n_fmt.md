...
> 原文: [https://www.geeksforgeeks.org/invert-the-kth-most-significant-bit-of-n/](https://www.geeksforgeeks.org/invert-the-kth-most-significant-bit-of-n/)

给定两个非负整数 `N` 和 `K`，任务是将 `N` 的第 `K` 个最高有效位反相，并打印反相后得到的数字。
...
> **输出:** 2
> `10` 的二进制表示为 `1010`。
> 第一位反相后变为 `0010`，其十进制等价为 `2`。
> **输入:** `N = 56`，`K = 2`
> **输出:** 40

**方法:** 找出 `N` 中的位数，如果位数小于 `K`，那么 `N` 本身就是需要的答案，否则翻转 `N` 的第 `K` 个最高有效位，打印翻转后得到的数。
