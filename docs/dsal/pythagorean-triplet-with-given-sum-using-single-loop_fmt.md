...
> 原文: [https://www.geeksforgeeks.org/pythagorean-triplet-with-given-sum-using-single-loop/](https://www.geeksforgeeks.org/pythagorean-triplet-with-given-sum-using-single-loop/)

一个[毕达哥拉斯三元组](https://www.geeksforgeeks.org/find-pythagorean-triplet-in-an-unsorted-array/)是一组自然数，使得 `a < b < c`，并且满足：

![a^2 + b^2 = c^2](img/96f42c08912cec6ae4b5279130fac550.png "Rendered by QuickLaTeX.com")

给定一个数 `N`，求一个和为给定 `N` 的毕达哥拉斯三元组，或者返回 `-1`。
...
思路是根据 `a` 求 `b` 和 `c` 的值，从 `1` 到 `n` 迭代 `a`。要根据 `a` 求 `b` 和 `c` 的值，我们要做以下工作：

我们有两个方程：

![a^2 + b^2 = c^2](img/548c3ba100c95898b4dd5d0c889fb21b.png "Rendered by QuickLaTeX.com")
`a + b + c = N`
...
![b = (N * N - 2 * N * a) / (2 * N - 2 * a)](img/d879c31987ec7500bf966beb01b64a57.png "Rendered by QuickLaTeX.com")
`c = N - b - a`

现在，从 `1` 到 `N` 迭代 `a`，分别计算 `b` 和 `c` 的值。然后，检查是否满足：
