...
> 原文: [https://www.geeksforgeeks.org/find-the-length-of-largest-subarray-in-which-all-elements-are-autobiographical-numbers/](https://www.geeksforgeeks.org/find-the-length-of-largest-subarray-in-which-all-elements-are-autobiographical-numbers/)

给定整数的`arr[]`数组，我们的任务是找到最大子数组的长度，使得子数组的所有元素都是[自传体数。](https://www.geeksforgeeks.org/self-descriptive-number/)
...
*   在每一步，将最大长度指定为`最大长度=max(当前长度，最大长度)`。`max_length` 的最终值将存储所需的结果。
