...
> 原文: [https://www.geeksforgeeks.org/java-program-to-merge-two-sorted-linked-lists-in-new-list/](https://www.geeksforgeeks.org/java-program-to-merge-two-sorted-linked-lists-in-new-list/)
...
这种方法背后的思想是，我们将在新列表中增加一个节点，即列表的头节点。我们将取类型列表中的一个变量，它总是在列表的最后一个节点，这样添加新节点就变得更容易了。我们将循环检查两个列表中较小的元素，并将该节点追加到结果列表中。如果我们到达任何列表的末尾，那么我们将简单地追加第二个列表中的剩余节点。
...
人们可以用递归方法解决这个问题。该函数将两个排序列表作为参数。如果任何列表为空，那么它只返回另一个列表中的剩余元素。否则，它将从两个列表中检查较小的元素，将较小的节点追加到结果列表中，并递归调用该列表和另一个列表的下一个节点的函数。
