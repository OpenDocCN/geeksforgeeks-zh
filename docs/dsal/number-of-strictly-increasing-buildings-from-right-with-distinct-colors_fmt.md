...
> 原文：[https://www.geeksforgeeks.org/number-of-strictly-increasing-buildings-from-right-with-distinct-colors/](https://www.geeksforgeeks.org/number-of-strictly-increasing-buildings-from-right-with-distinct-colors/)

给定一个整数``K``和两个大小为``K``的整数数组``H[]``和``C[]``，其中``H[]``存储连续建筑物的高度，``C[]``存储这些建筑物的颜色代码。
