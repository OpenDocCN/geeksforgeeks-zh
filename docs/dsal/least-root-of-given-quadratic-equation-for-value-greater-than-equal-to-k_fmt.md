...
> 原文: [https://www.geeksforgeeks.org/least-root-of-given-quadratic-equation-for-value-greater-than-equal-to-k/](https://www.geeksforgeeks.org/least-root-of-given-quadratic-equation-for-value-greater-than-equal-to-k/)

给定二次方程 `F(x) = Ax^2 + Bx + C`，系数 `A`、`B`、`C` 和一个整数 `K`，任务是求根 `x` 的最小值，使得 `F(x) ≥ K` 和 `x > 0`。如果不存在这样的值，则打印 `-1`。已知 `F(x)` 是单调递增函数。
...
> **输出:** `1`
> **说明:**
> 对于给定值 `F(x)= 3x^2+4x+5`，`x` 的最小值为 `1`，`F(x) = 12`，大于给定值 `K`。
>
> **输入:** `A = 3`，`B = 4`，`C = 5`，`K = 150`
> **输出:** `7`
> **说明:**
> 对于给定值 `F(x)= 3x^2+4x+5`，`x` 的最小值为 `7`，`F(x) = 180`，大于给定值 `K`。
...
*   计算 `F(mid)`，其中 `mid` 是范围 `[1, sqrt(K)]` 的中间值。以下三种情况是可能的:
    *   如果 `F(mid) ≥ K && F(mid-1) < K`：这意味着当前的 `mid` 是要求的答案。
