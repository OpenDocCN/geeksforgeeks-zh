...
> 原文: [https://www.geeksforgeeks.org/check-if-quantities-of-3-distinct-colors-can-be-converted-to-a-single-color-by-given-merge-pair-operations/](https://www.geeksforgeeks.org/check-if-quantities-of-3-distinct-colors-can-be-converted-to-a-single-color-by-given-merge-pair-operations/)
...
将所有颜色改为相同颜色意味着任务是达到最终状态，如 `T = (0, 0, R + G + B)` 或其其他两种排列中的任何一种。最初状态是 `I = (R, G, B)`。每次操作后，最初两种颜色的值各减少一，第三种颜色的值增加两。这个操作可以写成基于所选颜色的 `(-1, -1, +2)` 的排列。因此，形成以下等式:
