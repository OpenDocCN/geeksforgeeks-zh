# 约瑟夫斯问题在 O(N logN) 中的去除顺序

> 原文: [https://www.geeksforgeeks.org/order-of-removal-in-josephus-problem-in-on-logn/](https://www.geeksforgeeks.org/order-of-removal-in-josephus-problem-in-on-logn/)

给定 `N` 个站成一圈等待执行的孩子，以及一个数字 `K`，表示顺时针方向跳过 `K-1` 个孩子，第 `K` 个孩子在圈里被杀，然后开始执行第 `K+1` 个孩子，任务是打印将要在圈中被杀的孩子的顺序。
...
> 4.  从 2 开始数，第 `K` 个孩子是 2。所以第二个孩子被杀了。之后剩下要执行的孩子是 4，然后开始执行孩子 4。
...
最简单的想法就是用一个向量来存储剩余孩子的位置。然后当向量的大小大于 `1` 时迭代，并且在每次迭代中从向量中擦除期望的位置。
...
上述方法可以使用有序集进行优化。按照以下步骤解决问题:

*   初始化一个有序集，比如 `V`，然后将 `[1, N]` 范围内的元素插入 `V` 中。
*   初始化一个变量，将 `pos` 设为 `0`，以存储被移除元素的索引。
*   迭代至集合 `V` 的尺寸大于 `1`，执行以下步骤:
    *   将集合的大小存储在一个变量中，比如 `X`。
    *   将 `pos` 的值更新为 `(pos + K) % X`。
    *   在 `V` 中打印 `pos` 指向的元素，然后删除即可。
    *   将 `pos` 更新为 `pos % V.size()`。
*   最后，完成上述步骤后，打印集合 `V` 开头存储的最后一个元素。
