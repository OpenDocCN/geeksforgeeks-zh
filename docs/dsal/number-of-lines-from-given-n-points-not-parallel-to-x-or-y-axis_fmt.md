...
> 原文: [https://www.geeksforgeeks.org/number-of-lines-from-given-n-points-not-parallel-to-x-or-y-axis/](https://www.geeksforgeeks.org/number-of-lines-from-given-n-points-not-parallel-to-x-or-y-axis/)

给定 `2D` 平面上的 `N` 个不同整数点。任务是统计由给定的 `N` 点形成的不平行于 `X` 或 `Y` 轴的线的数量。

## 示例

> **输入:** `points[][] = {{1, 2}, {1, 5}, {1, 15}, {2, 10}}`
> **输出:** 3
> 选择的配对为 `{(1, 2), (2, 10)}`，`{(1, 5), (2, 10)}`，`{(1, 15), (2, 10)}`。
>
> **输入:** `points[][] = {{1, 2}, {2, 5}, {3, 15}}`
> **输出:** 3
> 选择任意一对点。
...
2.  可由 `N` 个点形成的线段总数 = `C(N, 2) = N*(N-1)/2`。
3.  现在我们将排除那些平行于 `X` 轴或 `Y` 轴的线段。
