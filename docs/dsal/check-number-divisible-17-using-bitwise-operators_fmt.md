...
> 原文: [https://www.geeksforgeeks.org/check-number-divisible-17-using-bitwise-operators/](https://www.geeksforgeeks.org/check-number-divisible-17-using-bitwise-operators/)
...
这意味着如果 `(floor(n/16)-n%16)` 能被 17 整除，则 `n` 能被 17 整除。
`(floor(n/16)-n%16)` 可以按位写成 `(int)(n>>4)–(int)(n&15)`，其中 `n>>4` 表示 `n/16`，`n&15` 表示 `n%16`。
