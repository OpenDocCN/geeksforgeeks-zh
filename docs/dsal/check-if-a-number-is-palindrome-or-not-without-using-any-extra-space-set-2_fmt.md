# 检查一个数字是否是回文，不使用任何额外的空间|设置 2

> 原文: [https://www.geeksforgeeks.org/check-if-a-number-is-palindrome-or-not-without-using-any-extra-space-set-2/](https://www.geeksforgeeks.org/check-if-a-number-is-palindrome-or-not-without-using-any-extra-space-set-2/)
...
*   [使用变量`i`迭代范围](https://www.geeksforgeeks.org/range-based-loop-c/)`[0, K/2)`，并执行以下任务:
    *   将`n%10`的值放入变量`ans`中，并用`10`除`n`。
*   如果`K%2`等于`1`，则将`n%10`的值放入变量`ans`中。
*   执行上述步骤后，如果`ans`等于`n`，则为苍白色，否则为非苍白色。
