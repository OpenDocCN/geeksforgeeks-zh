...
> 原文：[https://www.geeksforgeeks.org/least-number-to-be-added-to-or-subtracted-from-n-to-make-it-a-perfect-cube/](https://www.geeksforgeeks.org/least-number-to-be-added-to-or-subtracted-from-n-to-make-it-a-perfect-cube/)
...
1.  拿到号码。
2.  求数字的立方根，将结果转换为整数。
3.  将双精度值转换为整数后，这将包含 `N` 之前的完美立方体的根，即 `floor(cuberoot(N))`。
4.  然后求这个数的立方，这将是 `n` 之前的完美立方。
5.  求 `N` 后的完美立方根，即 `ceiling(cuberoot(N))`。
6.  然后求这个数的立方，这将是 `n` 之后的完美立方。
7.  检查 `floor` 值的立方是最接近 `N` 还是 `ceiling` 值。
8.  如果 `floor` 值的立方最接近 `N`，用`-`号打印差值。否则打印 `ceiling` 值的立方和带`+`号的 `N` 之间的差值。
