...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-differences-between-maximum-and-minimum-of-increasing-subarrays/](https://www.geeksforgeeks.org/sum-of-all-differences-between-maximum-and-minimum-of-increasing-subarrays/)
...
> 如果对于子阵列中的每一个 `i` 第索引，除了最后一个索引，`arr[i+1] > arr[i]` 之外，子阵列被认为是严格递增的
...
*   设 `i` 为当前子阵的起始元素，`j` 为当前子阵严格递增的指标。该子阵列的最大值和最小值分别为 `arr[j]` 和 `arr[i]`。因此，将 `(arr[j] – arr[i])` 加到 `sum` 中。
*   从 `(j+1)` 第索引继续迭代下一个子阵列。
*   完成数组遍历后，打印 `sum` 的最终值。
