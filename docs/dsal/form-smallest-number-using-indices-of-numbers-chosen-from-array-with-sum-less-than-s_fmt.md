...
> 原文：[https://www.geeksforgeeks.org/form-smallest-number-using-indices-of-numbers-chosen-from-array-with-sum-less-than-s/](https://www.geeksforgeeks.org/form-smallest-number-using-indices-of-numbers-chosen-from-array-with-sum-less-than-s/)

给定一个数组 `arr[]` 和一个整数 `S`，任务是从数组中选择最大数量的数字，使得数字的总和小于 `S`，并使用它们的索引形成最小可能的数字。注：任何元素都可以被选择任意次。
