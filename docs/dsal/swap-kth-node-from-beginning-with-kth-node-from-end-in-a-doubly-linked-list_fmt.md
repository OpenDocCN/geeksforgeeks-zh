...
`void swapNode(Node headReference, Node tailReference, int k)`

此函数用于交换双向链表中从开头数的第K个节点与从末尾数的第K个节点。

```java
{
    // 如果K为1，则需要将双向链表中的第一个节点与最后一个节点交换
    if (k == 1) {
        swapFirstAndLast(headReference, tailReference);
        return;
    }

    // 如果k为N，则需要将双向链表中的最后一个节点与第一个节点交换
    int nodeCount = getCount(headReference);
    if (k == nodeCount) {
        swapFirstAndLast(headReference, tailReference);
        return;
...
`private void swapFirstAndLast(Node headReference, Node tailReference)`

此函数用于交换双向链表中的第一个节点和最后一个节点。
...
```java
{
    if (headReference == null) {
        ...
        while (headReference != null) {
            Console.Write(headReference.getData() + " ");
            headReference = headReference.getNext();
        }
    }
}
...
如果您喜欢GeeksforGeeks并希望做出贡献，还可以使用[contribute.geeksforgeeks.org](https://contribute.geeksforgeeks.org/)撰写文章，或将您的文章邮寄至contribute@geeksforgeeks.org。查看您的文章出现在GeeksforGeeks主页上，并帮助其他Geeks。
