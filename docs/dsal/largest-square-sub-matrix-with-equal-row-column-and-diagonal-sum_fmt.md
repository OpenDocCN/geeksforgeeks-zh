...
给定尺寸为 `N*M` 的矩阵 `mat[][]`，任务是找到最大正方形子矩阵的大小，使得该子矩阵中所有行、列、对角线的和相等。
...
*   维护两个前缀和数组 `prefixSumRow[]` 和 `prefixSumColumn[]`，分别存储给定矩阵的行和列的前缀和。
*   执行以下步骤，检查从尺寸为 `K` 的单元格 `(i，j)` 开始的任何方形矩阵是否满足给定标准:
    1.  求子矩阵 `mat[i][j]` 到 `mat[i + K][j + K]` 的主对角线的元素之和并存储在变量中，比如 `sum`。
    2.  如果 `sum` 的值与下面提到的值相同，则返回 `true`。否则，返回 `false`。
        *   所有行的前缀和，即 `k` 在当时范围 `[I，I+K]` 内的所有值的 `prefixSumRow[K][j+K]–prefixSumRow[K][j]` 的值。
        *   所有列的前缀和，即 `k` 的所有值在当时范围 `[j，j+K]` 内的 `prefixSumColumn[I+K][j]–prefixSumColumn[I][K]` 值。
        *   反对角元素的前缀和。
