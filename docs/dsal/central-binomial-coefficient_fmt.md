...
给定一个整数 `N`，任务是找到第 `N` 个[中心二项式系数](https://en.wikipedia.org/wiki/Central_binomial_coefficient)。
...
> **输出:** 20
> **解释:**
> 第 `N` 个中心二项式系数 = `\binom{2N}{N}` = `\binom{2*3}{3}` = `\frac{6*5*4}{3*2*1}` = 20
> **输入:** `N = 2`
> **输出:** 6

## 方法

中心二项式系数是形式为 `\binom{2N}{N}` 的二项式系数。对于给定值 `N`，使用[动态编程](https://www.geeksforgeeks.org/dynamic-programming/)，可以使用此[方法](https://www.geeksforgeeks.org/binomial-coefficient-dp-9/)计算二项式系数 `\binom{2N}{N}`。
