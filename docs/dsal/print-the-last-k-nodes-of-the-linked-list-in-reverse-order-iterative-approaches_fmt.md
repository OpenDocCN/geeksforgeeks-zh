...
*   计算`链表`中的`节点数`。
*   声明一个`数组`，并以`节点数`作为其大小。
*   从`数组`末尾开始存储`链表`的`节点值`，即反向存储。
*   从`数组`开始打印`k`个值。

## 方法 3

想法是首先迭代地反向链表，如以下文章所述：[反转链表](https://www.geeksforgeeks.org/reverse-a-linked-list/)。 反转后，打印反转列表的前`k`个节点。 打印后，通过再次反转列表来恢复列表。
