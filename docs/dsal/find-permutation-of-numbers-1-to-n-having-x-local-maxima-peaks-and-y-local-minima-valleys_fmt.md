...
> 原文: [https://www.geeksforgeeks.org/find-permutation-of-numbers-1-to-n-having-x-local-maxima-peaks-and-y-local-minima-valleys/](https://www.geeksforgeeks.org/find-permutation-of-numbers-1-to-n-having-x-local-maxima-peaks-and-y-local-minima-valleys/)

给定三个整数 `N`、`A` 和 `B`，任务是找到从 `1` 到 `N` 的成对不同数字的排列，该排列恰好具有 `A` 个局部最小值和 `B` 个局部最大值。
...
*   遍历每个排列，如果下面的排列正好有 `A` 个局部最小值和 `B` 个局部最大值，打印排列。
*   如果不存在这样的排列，则打印 `-1`。
...
*   完成两个角例后，创建两个变量，`minValue` 存储排列的最小值，`maxValue` 存储最大值。现在把问题分成三种不同的情况，分别是 `(A > B)`、`(A < B)` 和 `(A=B)`。现在分别解决每个案例:
    *   如果 `(A > B):` 将 `minValue` 初始化为 `1`，并从索引 `2` 开始用 `minValue` 填充数组 `A` 次。每次插入后，将 `minValue` 增加 `1`，因为这些值应该是不同的。填充时，确保每次插入后保留一个索引为空，因为两个最小值不可能连续存在。在创建 `A` 个极小值之后，以递增的顺序填充数组的其余部分，这样就不会创建新的极小值。
    *   如果 `(B > A):` 将 `maxValue` 初始化为 `N`，并从索引 `2` 开始为 `B` 次填充数组 `maxValue`。每次插入后，将 `maxValue` 减少 `1`，因为这些值应该是不同的。填充时，确保每次插入后保留一个索引为空，因为两个最大值不可能连续存在。创建 `B` 个最大值后，按降序填充数组的其余部分，这样就不会创建新的最大值。
    *   如果 `(A = B):` 则初始化两个值 `minValue` 为 `1`，`maxValue` 为 `N`。用 `minValue` 填充数组的第一个元素，并将 `minValue` 增加 `1`。然后用 `maxValue` 填充偶数索引，将 `maxValue` 减少 `1`，用 `minValue` 填充奇数索引，将 `minValue` 增加 `1`，重复 `A` 次。现在，按照递增的顺序填充其余的位置。
