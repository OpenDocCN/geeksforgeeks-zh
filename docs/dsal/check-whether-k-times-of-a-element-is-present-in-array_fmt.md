...
一个简单的解决方案是运行两个嵌套的[循环](https://www.geeksforgeeks.org/loops-in-c-and-cpp/)，并检查每个元素在数组中是否也存在该元素的`K`倍。
...
想法是将所有元素存储在[哈希映射](https://www.geeksforgeeks.org/hashing-data-structure/)中，并针对每个元素检查该哈希映射中是否存在该元素的`K`倍。 如果它存在于哈希映射中，则返回`True`，否则返回`False`。
