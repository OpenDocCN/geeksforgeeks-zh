...
> 原文：[https://www.geeksforgeeks.org/count-elements-such-that-there-are-exactly-x-elements-with-values-greater-than-or-equal-to-x/](https://www.geeksforgeeks.org/count-elements-such-that-there-are-exactly-x-elements-with-values-greater-than-or-equal-to-x/)

给定一个由 `N` 个整数组成的数组 `arr`，任务是找出满足以下条件的元素个数：如果元素是 `X`，那么数组中必须有恰好 `X` 个大于或等于 `X` 的元素个数（不包括 `X` 的个数）。
...
该问题涉及有效搜索每个 `arr[i]`，元素 `arr[j]`（`i != j`）大于或等于 `arr[i]` 的数量。
