...
> 原文: [https://www.geeksforgeeks.org/given-a-set-find-xor-of-the-xors-of-all-subsets/](https://www.geeksforgeeks.org/given-a-set-find-xor-of-the-xors-of-all-subsets/)
...
逻辑很简单。让我们考虑第 `n` 个元素，它可以包含在剩余 `(n-1)` 个元素的所有子集中。`(n-1)` 个元素的子集数等于 `2^(n-1)`，`n > 1` 时总是偶数。因此，在异或结果中，每个元素被包括偶数次，并且任何数量的偶数出现的异或为 `0`。
