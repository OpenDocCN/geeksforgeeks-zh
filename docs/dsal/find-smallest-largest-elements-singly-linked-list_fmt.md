...
这个想法是在不等于`NULL`的情况下遍历链表，并将`max`和`min`变量分别初始化为`INT_MIN`和`INT_MAX`。

之后检查一个条件，如果头部值大于`max`，则将头部值分配给`max`，如果头部值小于`min`，则将头部值分配给`min`，然后头部值指向下一个节点。继续此过程，直到`head`不等于`NULL`。
