...
> 原文: [https://www.geeksforgeeks.org/construct-a-sequence-from-given-frequencies-of-n-consecutive-integers-with-unit-adjacent-difference/](https://www.geeksforgeeks.org/construct-a-sequence-from-given-frequencies-of-n-consecutive-integers-with-unit-adjacent-difference/)

给定一个[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `freq[]`，该数组存储从 `0` 到 `N–1` 的 `N` 个整数的频率。任务是构造一个序列，其中数字 `i` 出现 `freq[i]` 次 (`0≤i≤N–1`)，使得两个相邻数字之间的绝对差为 `1`。如果无法生成任何序列，则打印 `-1`。
...
1.  创建一个[地图](https://www.geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/) `M` 来存储数字的频率。另外，在一个变量中找到频率的总和，比如说 `total`。
2.  迭代地图，并对地图中的每个元素执行以下操作:
    *   创建地图副本 `M`。
    *   创建一个[向量](https://www.geeksforgeeks.org/vector-in-cpp-stl/) `sequence`，存储可能的答案。如果当前元素的频率为非零，则降低其频率并将其推入 `sequence`，并尝试按以下方式形成序列的其余 `total–1` 个元素:
        1.  让我们将插入到 `sequence` 中的最后一个元素称为 `last`。如果 `last-1` 的频率不为零，则降低其频率并将其推入 `sequence`。更新最后一个元素。
        2.  否则，如果 `last+1` 的频率为非零，则递减其频率并将其推入 `sequence`。更新最后一个元素。
        3.  否则，从内部循环中断开。
    *   如果 `sequence` 的大小等于 `total`，则将其作为答案返回。
