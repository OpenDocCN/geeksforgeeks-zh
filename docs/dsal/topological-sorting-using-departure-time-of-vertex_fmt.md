...
给定一个有向无环图，求该图的拓扑排序。
有向无环图(`DAG`)的拓扑排序是顶点的线性排序，使得对于每个有向边 `uv`，顶点 `u` 在排序中位于 `v` 之前。如果图形不是 `DAG`，则不可能对图形进行拓扑排序。
...
我们已经讨论了一个基于深度优先搜索(`DFS`)的算法，使用堆栈和[卡恩算法](https://www.geeksforgeeks.org/topological-sorting-indegree-based-solution/)进行拓扑排序。我们还讨论了如何打印所有拓扑类型的 `DAG` [在这里](https://www.geeksforgeeks.org/all-topological-sorts-of-a-directed-acyclic-graph/)。本文讨论了另一种基于 `DFS` 的方法，通过在 `DFS` 中引入顶点到达和离开时间的概念来寻找图的拓扑排序。

## 什么是 DFS 中顶点的到达时间与离开时间？
