...
> 原文: [https://www.geeksforgeeks.org/sort-even-and-odd-placed-elements-in-increasing-order/](https://www.geeksforgeeks.org/sort-even-and-odd-placed-elements-in-increasing-order/)
...
初始化两个列表来存储奇数和偶数索引数字。
遍历所有数字，将奇数索引数字存储在 `odd_indexes` 列表中，将偶数索引数字存储在 `even_indexes` 列表中。
按排序顺序打印 `odd_indexes` 列表中的元素。
按排序顺序打印 `even_indexes` 列表中的元素。
