...
> 原文：`https://www.geeksforgeeks.org/infix-to-prefix-conversion-using-two-stacks/`

## 中缀
如果运算符出现在表达式中的操作数之间，则该表达式称为中缀表达式。简单的形式（操作符 1 操作符 2）。
**例：** `(A+B) * (C-D)`

## 前缀
如果运算子出现在运算对象之前的表达式中，则该表达式称为前缀表达式。简单的形式（操作符 1 操作符 2）。
**示例：** `*+AB-CD`（中缀：`(A+B) * (C-D)`）
