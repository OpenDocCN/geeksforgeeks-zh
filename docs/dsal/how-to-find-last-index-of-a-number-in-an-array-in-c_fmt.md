...
> 原文：[https://www.geeksforgeeks.org/how-to-find-last-index-of-a-number-in-an-array-in-c/](https://www.geeksforgeeks.org/how-to-find-last-index-of-a-number-in-an-array-in-c/)

给定一个由 `N` 个整数和数字 `K` 组成的数组 `arr[]`，任务是在 `arr[]` 中找到最后一个出现的 `K`。如果元素不存在，则返回 `-1`。
