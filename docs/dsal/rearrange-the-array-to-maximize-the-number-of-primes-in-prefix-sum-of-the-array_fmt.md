...
> 原文: [https://www.geeksforgeeks.org/rearrange-the-array-to-maximize-the-number-of-primes-in-prefix-sum-of-the-array/](https://www.geeksforgeeks.org/rearrange-the-array-to-maximize-the-number-of-primes-in-prefix-sum-of-the-array/)

给定一个由 `1` 和 `2` 组成的数组 `arr[]`，任务是重新排列数组，使得重新排列的数组的前缀和具有最大的素数。**注意**可以有多个答案。

**举例:**

> **输入:** `arr[] = {1, 2, 1, 2, 1}`
> **输出:** `2 1 1 1 2`
> 前缀和数组为 `{2, 3, 4, 5, 7}`，其中 `{2, 3, 5, 7}` 为素数，为最大可能。
> **输入:** `arr[] = {1, 1, 2, 1, 1, 1, 2, 1, 1}`
> **输出:** `2 1 1 1 1 1 1 2`

**做法:** 这个问题可以用两个观察来解决，一个是第一个素数是 `2`，之后的所有其他素数都是奇数（所有偶数都不是素数）。因此，只要在第一个位置填充 `2`（如果有的话），然后填充奇数个 `1`，然后填充剩余的 `2`。在末尾插入剩下的 `1`（如果最初的 `1` 是偶数）。
