...
> 原文: [https://www.geeksforgeeks.org/split-array-into-three-continuous-subarrays-with-negative-0-and-positive-product-respectively/](https://www.geeksforgeeks.org/split-array-into-three-continuous-subarrays-with-negative-0-and-positive-product-respectively/)
...
**方法:** 这个问题可以通过[贪婪地](https://www.geeksforgeeks.org/greedy-algorithms/)选择直到第一个 `-1` 没有 `0` 的元素作为第一个子阵列，然后从最后一个子阵列中选择元素，直到产品变成 `1` 作为第三个子阵列，剩余的元素作为第二个子阵列来解决。按照以下步骤解决问题:

*   初始化两个变量，说 `l` 和 `r` 为 `-1`，存储第一子阵列最后一个元素和第三子阵列第一个元素的索引。
*   [遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]`，如果 `l` 为 `-1` 且 `arr[i]` 为 `0`，则打印“不可能”。否则，如果 `arr[i]` 为 `-1`，则将 `i` 分配给 `l` 和[跳出循环](https://www.geeksforgeeks.org/break-statement-cc/)。
