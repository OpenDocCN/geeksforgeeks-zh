...
> 原文: [https://www.geeksforgeeks.org/binary-search-tree-set-2-delete/](https://www.geeksforgeeks.org/binary-search-tree-set-2-delete/)
...
3) 要删除的节点有两个子节点：查找该节点的后继节点。将后继节点的值复制到当前节点，并删除后继节点。请注意，也可以使用前驱节点。
...
需要注意的重要一点是，只有当合适的子节点非空时，才需要寻找后继者。在这种特殊情况下，可以通过在节点的右子树中找到最小值来获得后继节点。
...
删除操作的最坏情况时间复杂度为 `O(h)`，其中 `h` 为二叉查找树的高度。在最坏的情况下，我们可能不得不从根走到最深的叶节点。倾斜树的高度可能变成 `n`，删除操作的时间复杂度可能变成 `O(n)`。
...
*   [BST](https://www.geeksforgeeks.org/binary-search-tree/)上的所有文章
