...
> 原文：[https://www.geeksforgeeks.org/round-off-number-given-number-significant-digits/](https://www.geeksforgeeks.org/round-off-number-given-number-significant-digits/)
...
1.  丢弃第 `n` 个有效数字右侧的所有数字。
2.  如果这个被丢弃的号码是：
    *   第 `n` 位不足半个单位，保持第 `n` 位数字不变。
    *   在第 `n` 位大于半个单位，将第 `n` 位数增加 1。
    *   在第 `n` 位置正好半个单位，如果其奇数，将第 `n` 位数增加 1，否则保持不变。
