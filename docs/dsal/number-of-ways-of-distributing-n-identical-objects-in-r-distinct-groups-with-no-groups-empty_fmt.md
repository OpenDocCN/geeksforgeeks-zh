...
> 原文: [https://www.geeksforgeeks.org/number-of-ways-of-distributing-n-identical-objects-in-r-distinct-groups-with-no-groups-empty/](https://www.geeksforgeeks.org/number-of-ways-of-distributing-n-identical-objects-in-r-distinct-groups-with-no-groups-empty/)

给定两个整数 `N` 和 `R`，任务是计算将 `N` 个相同的对象分配到 `R` 个不同的组中的方法数量，使得没有组是空的。
...
思路是用多项式定理。假设 `x<sub>1</sub>` 个物体放在第一组，`x<sub>2</sub>` 个物体放在第二组，`x<sub>R</sub>` 个物体放在 `R<sup>th</sup>` 组。给出的是，
`x<sub>1</sub> + x<sub>2</sub> + x<sub>3</sub> + … + x<sub>R</sub> = N` 对于所有 `x<sub>i</sub> ≥ 1` 对于 `1 ≤ i ≤ R`。
现在将每一个 `x<sub>i</sub>` 替换为 `y<sub>i</sub> = x<sub>i</sub> - 1`。现在所有的 `y` 变量都大于等于零。
等式变为:
`y<sub>1</sub> + y<sub>2</sub> + y<sub>3</sub> + … + y<sub>R</sub> + R = N` 对于所有 `y<sub>i</sub> ≥ 0` 对于 `1 ≤ i ≤ R`
即 `y<sub>1</sub> + y<sub>2</sub> + … + y<sub>R</sub> = N - R`。
其解为 `<sup>(N–R)+R–1</sup>C<sub>R–1</sub>`。
这个方程的解由 `<sup>N–1</sup>C<sub>R–1</sub>` 给出。
