# 使用 Z 算法从给定字符串中删除所有出现的单词

> 原文: [https://www.geeksforgeeks.org/remove-all-occurrences-of-a-word-from-a-given-string-using-z-algorithm/](https://www.geeksforgeeks.org/remove-all-occurrences-of-a-word-from-a-given-string-using-z-algorithm/)

给定两个长度为 `N` 的字符串和长度为 `M` 的单词，任务是从字符串 `str` 中移除字符串 `word` 的所有出现。
...
解决这个问题最简单的方法就是迭代字符串 `str` 的字符。对于每个索引，检查是否可以找到起始索引等于当前索引的子串，并且子串等于字符串 `word`。如果发现为真，则移除子字符串。最后，打印字符串。
...
使用 `replace()` 方法从字符串 `str` 中删除所有出现的字符串 `word`。
...
上述方法可以使用 Z 算法进行优化。按照以下步骤解决问题:

*   初始化一个字符串，说 `res` ，通过从给定字符串 `str` 中删除单词来存储该字符串。
*   初始化一个数组，比如 `Z[]` ，来存储字符串的 `Z` 值。
*   使用 Z 算法查找给定字符串 `str` 中所有出现的字符串 `word`。
*   最后，遍历数组 `Z[]` ，检查 `z[i + length(word) + 1]` 是否等于 `length(word)`。如果发现为真，则更新 `i += length(word) – 1`。
