...
> 原文: [https://www.geeksforgeeks.org/print-numbers-with-digits-0-and-1-only-such-that-their-sum-is-n/](https://www.geeksforgeeks.org/print-numbers-with-digits-0-and-1-only-such-that-their-sum-is-n/)
...
3.  循环 `N` 并检查 `N` 是否是 10 的倍数。如果是，获取小数并通过乘以 10 来更新 `p`，并将该值存储在向量中，并将 `N` 减少 `m`。对每个小数执行此操作，并打印向量的总大小。
4.  最后遍历向量并打印元素。
