...
> 原文：[https://www.geeksforgeeks.org/python-program-for-quicksort-on-doubly-linked-list/](https://www.geeksforgeeks.org/python-program-for-quicksort-on-doubly-linked-list/)
...
上述实现的时间复杂度与 `quickSort()` 对于数组的时间复杂度相同。最坏的情况下需要 O(n^2) 时间，一般情况下和最好的情况下都需要 O(n log n)。最坏的情况发生在链表已经排序的时候。
