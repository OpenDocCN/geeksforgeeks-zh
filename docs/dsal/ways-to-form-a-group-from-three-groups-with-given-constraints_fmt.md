...
> 原文: [https://www.geeksforgeeks.org/ways-to-form-a-group-from-three-groups-with-given-constraints/](https://www.geeksforgeeks.org/ways-to-form-a-group-from-three-groups-with-given-constraints/)
...
> `Y` 有人(`d`，`e`)
> `Z` 有人(`f`)
> 那么这 9 种方式就是{`a`，`b`，`d`，`f`}，{`a`，`b`，`e`，`f`}，{`a`，`c`，`d`，`f`}，{`a`，`c`，`e`，`f`}，{`b`，`c`，`d`，`f`}，{`b`，`c`，`e`，`f`}，{`a`，`b`，`c`，`d`，`f`}，{`a`，`b`，`c`，`e`，`f`}，{`a`，`b`，`c`，`d`，`e`，`f`}
> **输入:** `x = 4`，`y = 2`，`z = 1`
> **输出:** `27`

这个问题可以用[组合数学](https://www.geeksforgeeks.org/combinatorics-gq/)来解决。有三个职位(就不同群体的人而言)需要填补。第一个位置必须用比第二个位置大一个或多个的数字来填充。第三个可以用任何数字填充。我们知道，如果我们需要用 `N` 个人来填补 `k` 个职位，那么方法的数量就是`{N \choose K}`。因此，可以遵循以下步骤来解决上述问题。
...
*   填充位置的方式有`{y \choose i}` * `{x \choose j}`。
*   在计算了填充这两个位置的所有方法后，我们可以简单地将`{z \choose 1}` + `{z \choose 2}` + … `{z \choose z}`相乘，因为这是两者的公共部分。

`{N \choose K}`可以使用动态规划来预先计算，以降低时间复杂度。方法在这里讨论。
