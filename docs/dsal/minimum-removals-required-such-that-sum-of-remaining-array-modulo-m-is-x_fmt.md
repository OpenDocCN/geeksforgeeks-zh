...
给定一个由 `N` 个正整数和整数 `X` 和 `M` 组成的数组 `arr[]`，其中 `0 <= X < M`，任务是找到需要移除的元素的最小数量，使得剩余数组的和模 `M` 等于 `X`。如果不可能，打印 `-1`。
...
最简单的方法是生成给定数组的所有可能子集，对于每个子集，检查去除子集后数组的和模 `M` 是否等于 `X`。如果发现是真的，存储它的大小。打印获得的所有此类子集的最小尺寸。

**时间复杂度:** `O(2^N)`，其中 `N` 是给定数组的长度。
**辅助空间:** `O(N)`

## 高效方法

优化上述方法，思路是基于以下观察使用动态规划:
...
*   初始化一个 `dp[]` 表、`table[N+1][X+1]`，其中 `table[i][j]` 表示要移除的元素的最小数量，其索引在 `[0, i]` 范围内，使得它们的和为 `j`，其中 `X` 为要移除的和。
