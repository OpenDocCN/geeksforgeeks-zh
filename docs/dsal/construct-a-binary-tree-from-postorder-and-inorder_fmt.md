...
2. 我们在`in[]`中搜索`1`以找到根的左右子树。`in[]`中`1`左侧的所有内容都在左侧子树中，右侧的所有内容都在右侧子树中。
...
3. 以下两个步骤重复以上过程。
    1. 重复`in[] = {6, 3, 7}`和`post[] = {6, 7, 3}`。使创建的树成为根的右子节点。
    2. 重复`in[] = {4, 8, 2, 5}`和`post[] = {8, 4, 5, 2}`。使创建的树成为根的左子节点。
...
我们可以使用哈希（C++ 中的`unordered_map`或 Java 中的`HashMap`）优化上述解决方案。 我们将有序遍历的索引存储在哈希表中。 这样就可以完成`O(1)`搜索。
