...
> 原文：[https://www.geeksforgeeks.org/number-of-ways-to-represent-a-number-as-sum-of-k-fibonacci-numbers/](https://www.geeksforgeeks.org/number-of-ways-to-represent-a-number-as-sum-of-k-fibonacci-numbers/)
...
斐波那契数列为 `f(0)=1`，`f(1)=2`，`f(i)=f(i-1)+f(i-2)` 为 `i > 1`。让我们假设 `F(x, k, n)` 是利用来自 `f(0), f(1), …f(n-1)` 的恰好 `k` 个数形成和 `x` 的方法数。要找到 `F(x, k, n)` 的一个递归，注意有两种情况：`f(n-1)` 是否在和中。
