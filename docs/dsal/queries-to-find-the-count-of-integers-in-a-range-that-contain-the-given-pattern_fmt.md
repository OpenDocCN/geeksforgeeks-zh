...
> 原文：[https://www.geeksforgeeks.org/queries-to-find-the-count-of-integers-in-a-range-that-contain-the-given-pattern/](https://www.geeksforgeeks.org/queries-to-find-the-count-of-integers-in-a-range-that-contain-the-given-pattern/)

给定二进制模式`pattern`和`Q`查询，其中每个查询由范围`[L, R]`组成，对于每个查询，任务是从给定的范围中找到整数的计数，使得它们在其二进制表示中包含给定的模式。
