...
> 原文：`https://www.geeksforgeeks.org/find-the-larger-exponential-among-two-exponentials/`

给定四个整数 `A`、`B`、`C` 和 `D`。任务是找出 `A^B` 和 `C^D` 哪个更大。
...
计算 `A^B` 和 `C^D` 的值，然后进行比较。当数值较大时，如 `562145^321457`，该方法将失败。

## 高效途径

使用 `log`，我们可以将术语写成 `log(A^B)` 和 `log(C^D)`，也可以写成 `B * log(A)` 和 `D * log(C)`。这些值比原始值更容易计算和比较。
