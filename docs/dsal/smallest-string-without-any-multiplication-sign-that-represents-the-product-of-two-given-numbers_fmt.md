...
> 原文: [https://www.geeksforgeeks.org/smallest-string-without-any-multiplication-sign-that-represents-the-product-of-two-given-numbers/](https://www.geeksforgeeks.org/smallest-string-without-any-multiplication-sign-that-represents-the-product-of-two-given-numbers/)

给定两个数字 `A` 和 `B`，任务是打印最小可能长度的字符串，该字符串计算为两个给定数字的乘积，即 `A*B`，而不使用乘法符号。

> 2 的任何完美幂都可以用左移运算符的形式表示。
> **例如:**
> `N = 2 = 1<<1 = "2<<1"`
> `N = 4 = 1<<2 = "4<<2"`
...
> 让 `B = 2^{k_1} + 2^{k_2} + ... + 2^{k_n}`，其中 `k_1 > k_2 > ... > k_n`

*   因此，`A` 和 `B` 的乘积可以写成

> `A * B = A * (2^{k_1} + 2^{k_2} + ... + 2^{k_n})`

*   使用 `<<` (左移位运算符)将一个数乘以二的任意次方。
*   因此 `A x B = A<<k_1 + A<<k_2 + ... + A<<k_n`
*   要找到 `k_i`，我们使用 `log()` 函数并用余数 `B–2^{k_i}` 继续该过程，直到余数变为 `0` 或余数的 `log` 变为零。
*   同样，通过将 `A` 表示为 2 的幂，来表示 `A * B = B<<k_1 + B<<k_2 + ... + B<<k_n`。
*   比较两种表示法，并打印长度较小的字符串。
