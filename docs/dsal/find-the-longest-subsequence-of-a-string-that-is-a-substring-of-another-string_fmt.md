...
> 原文: [https://www.geeksforgeeks.org/find-the-longest-subsequence-of-a-string-that-is-a-substring-of-another-string/](https://www.geeksforgeeks.org/find-the-longest-subsequence-of-a-string-that-is-a-substring-of-another-string/)

给定由 `N` 和 `M` 字符组成的两个字符串 `X` 和 `Y`，任务是找到字符串 `X` 的最长子序列，这是字符串 `Y` 的子串。
...
解决给定问题最简单的方法是找到给定字符串 `X` 的所有子序列，并在所有生成的子序列中打印该子序列，该子序列长度最大，是 `Y` 的子串。
...
上述方法也可以通过动态规划进行优化。其思想是创建维度为 `(N + 1)*(M + 1)` 的 2D 数组 `dp[][]`，状态 `dp[i][j]` 是 `X[0, i]` 的子序列的最大长度，该子序列是 `Y[0, j]` 的子串。按照以下步骤解决问题:
