...
> 原文：[https://www.geeksforgeeks.org/squared-triangular-number-sum-cubes/](https://www.geeksforgeeks.org/squared-triangular-number-sum-cubes/)

给定一个数字 `s`（`1 <= s <= 1000000000`）。如果 `s` 是[前 `n` 个自然数](https://www.geeksforgeeks.org/sum-cubes-even-odd-natural-numbers/)的立方之和，然后打印 `n`，否则打印 `-1`。
...
一个**有效解**是基于公式 `n(n+1)/2` 的平方的前 `n` 个立方体的和。我们可以看到所有的数字都是正方形。

1.  [检查给定的数字是否是完美的正方形。](https://www.geeksforgeeks.org/check-if-given-number-is-perfect-square-in-cpp/)
2.  检查平方根是否为[三角形数](https://www.geeksforgeeks.org/triangular-numbers/)（详见三角形数的方法 2）。
