...
> 原文：[https://www.geeksforgeeks.org/comparisons-involved-modified-quicksort-using-merge-sort-tree/](https://www.geeksforgeeks.org/comparisons-involved-modified-quicksort-using-merge-sort-tree/)
...
**但是**，我们在每个分区中都有一个从 `L` 到 `R` 的数字排列，所以我们可以找到 `ceiling((R–L+1)/2)`<sup>这个范围内的最小数字，因为我们知道当我们对这个分区进行排序时，总是这个元素会作为中间元素结束，结果也是枢轴。现在，小于 `pivot` 的元素转到左边的子树，大于 `pivot` 的元素转到右边的子树，并保持它们的顺序。</sup>

我们对所有分区 `P` 重复这个过程，并找出每个分区中涉及的比较。因为在当前分区中，从 `L` 到 `R` 的所有元素都与枢轴进行比较，所以我们在当前分区中进行 `(R–L+1)` 比较。我们还需要通过递归计算来考虑左右子树的总比较。由此我们得出结论，
