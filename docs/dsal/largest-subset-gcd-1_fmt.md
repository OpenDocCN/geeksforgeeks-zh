...
我们找到所有可能子集的 [GCD](https://www.geeksforgeeks.org/c-program-find-gcd-hcf-two-numbers/) ，找到 GCD 为 1 的最大子集。花费的总时间将等于评估所有可能子集的 GCD 所花费的时间。总的可能子集是 `2^n` 。在最坏的情况下，子集内有 `n` 个元素，计算其 GCD 所需的时间为 `n * log(n)`。保存当前子集所需的额外空间为 `O(n)`。
