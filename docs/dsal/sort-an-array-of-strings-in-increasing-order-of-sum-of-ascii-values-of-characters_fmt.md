...
> 原文: [https://www.geeksforgeeks.org/sort-an-array-of-strings-in-increasing-order-of-sum-of-ascii-values-of-characters/](https://www.geeksforgeeks.org/sort-an-array-of-strings-in-increasing-order-of-sum-of-ascii-values-of-characters/)

给定一个由 `N` 个字符串组成的数组 `arr[]`，任务是按照字符的 ASCII 值之和的递增顺序对字符串进行排序。

## 示例

**输入:** `arr[] = {"for", "geeks", "app", "best"}`
**输出:** `app for best geeks`
**解释:**
...
**输出:** `a for geeks portal science computer geeks forgeeks`
**说明:**
各字符串字符的 ASCII 值之和为：`{1381, 97, 879, 730, 658, 327, 527}`。
因此，排序顺序为 `{"a", "for", "geeks", "portal", "science", "computer", "geeks forgeeks"}`。

## 方法

想法是使用辅助的数组来存储字符串对及其各自的字符 ASCII 值之和。然后，根据对中的第一个值对数组进行排序，然后打印排序后的字符串。按照以下步骤解决问题：

*   初始化一个存储 `pair<int, string>` 的向量 `V`，将字符串的 ASCII 值之和与字符串本身存储为一对。
*   使用变量 `i` 迭代范围 `[0, N-1]`：
    *   找到 `arr[i]` 字符的 ASCII 值之和，并将其存储在一个变量中，比如 `val`。
    *   在 `V` 中追加一对 `{val, arr[i]}`。
*   根据对中的第一个值对向量 `V` 进行排序。
*   使用变量 `i` 在向量 `V` 上的范围 `[0, n-1]` 中迭代：
    *   打印字符串，即 `v[i].second`。
...
**时间复杂度:** `O(N*log(N) + N*M)`，其中 `M` 是数组 `arr[]` 中最长字符串的长度。
**辅助空间:** `O(N)`