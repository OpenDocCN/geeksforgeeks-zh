...
> 原文：[https://www.geeksforgeeks.org/find-the-element-whose-multiplication-with-1-makes-array-sum-0/](https://www.geeksforgeeks.org/find-the-element-whose-multiplication-with-1-makes-array-sum-0/)
...
简单的解决方法是取每个元素，乘以 `-1`，检查新的和是否为 `0`。该算法在 `O(N^2)` 中工作。

## 有效方法

如果我们将 `S` 作为数组的初始和，并将当前元素 `A_i` 乘以 `-1`，那么新的和将变成 `S–2 * A_i`，这应该等于 `0`。因此，当第一次 `S = 2*A_i` 时，当前索引是我们必需的，如果没有元素满足条件，那么我们的答案将是 `-1`。该算法的时间复杂度为 `O(N)`。
