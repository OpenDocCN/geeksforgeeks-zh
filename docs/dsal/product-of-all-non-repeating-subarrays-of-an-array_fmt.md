...
> 原文: [https://www.geeksforgeeks.org/product-of-all-non-repeating-subarrays-of-an-array/](https://www.geeksforgeeks.org/product-of-all-non-repeating-subarrays-of-an-array/)
...
> **输出:** 1944810000
> **说明:**
> 给定阵列的可能子阵列为 `{10}`、`{10, 3}`、`{10, 7}`、`{10, 3, 7}`、`{3}`、`{7}`、`{3, 7}`。
> 产品分别为 10、30、70、210、3、7、21。因此，所有子阵列的总乘积= 1944810000。

## 天真方法

这个问题的天真方法是[生成给定数组的所有子数组](https://www.geeksforgeeks.org/generating-subarrays-using-recursion/)并计算它们的乘积。这种方法的时间复杂度是指数级的。

## 高效进场

思路是做一个观察。如果我们观察不重复的子阵列，我们可以观察到子阵列中单个元素的出现遵循与阵列长度的关系。
