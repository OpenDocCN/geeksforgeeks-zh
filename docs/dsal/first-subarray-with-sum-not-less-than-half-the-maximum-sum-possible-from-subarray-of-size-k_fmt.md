...
> 原文：[https://www.geeksforgeeks.org/first-subarray-with-sum-not-less-than-half-the-maximum-sum-possible-from-subarray-of-size-k/](https://www.geeksforgeeks.org/first-subarray-with-sum-not-less-than-half-the-maximum-sum-possible-from-subarray-of-size-k/)

## 问题陈述

给定一个数组 `arr[]` 和一个整数 `K`，任务是从任何大小的子阵列 `K` 中找到第一个子阵列，其和大于或等于最大可能和的一半。
...
这个问题可以使用滑动窗口技术来解决，因为要考虑子阵列。按照以下步骤解决此问题：
