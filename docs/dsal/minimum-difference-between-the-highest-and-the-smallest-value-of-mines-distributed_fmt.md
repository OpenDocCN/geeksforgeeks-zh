...
给定 `n` 个公司和 `m` 个油矿具有价值，任务是公平地在 `n` 个公司之间分配这些矿山。 那就是获得最高矿产价值的公司与获得最低矿产价值的公司之间的差异应该是最小的。 计算最小差异​​。 请注意，分配给每个公司的油矿应彼此相邻。 另外，`m >= n`

## 示例

> **输入**：`n = 2`，`m = 4`，地雷值= `[6, 10, 13, 2]`
