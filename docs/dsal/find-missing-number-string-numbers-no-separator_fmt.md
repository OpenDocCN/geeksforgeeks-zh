...
> 原文：[https://www.geeksforgeeks.org/find-missing-number-string-numbers-no-separator/](https://www.geeksforgeeks.org/find-missing-number-string-numbers-no-separator/)
...
function getValue(str, i, m) {
    ...
    let value = 0;
    for(let j = 0; j < m; j++) {
        let c = str[i + j].charCodeAt(0) - '0'.charCodeAt(0);
        if (c < 0 || c > 9) {
            return -1;
        }
        value = value * 10 + c;
...
function findMissingNumber(str) {
    ...
    for(let m = 1; m <= MAX_DIGITS; ++m) {
        ...
        for(let i = m; i != str.length; i += 1 + Math.floor(Math.log10(n))) {
            // If we haven't yet found the missing number
            // for current length. Next number is n+2\. Note
            // that we use Log10 as (n+2) may have more
            // length than n.
