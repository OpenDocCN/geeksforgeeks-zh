...
最简单的方法是[生成给定数组的所有子数组](https://www.geeksforgeeks.org/generating-subarrays-using-recursion/)，并对每个数组元素`arr[i]`计数最小元素是它的子数组数。
...
为了优化上述方法，其思想是找到每个元素的边界索引，直到它是最小的元素。 对于每个元素，令`L`和`R`分别为左侧和右侧的边界索引，直到`arr[i]`最小。 因此，可以通过以下方式计算所有子数组的数量：
