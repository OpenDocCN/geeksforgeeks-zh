...
> 原文：`https://www.geeksforgeeks.org/code-optimization-technique-logical-and-and-logical-or/`
...
> 在`实现 1`中，我们避免检查偶数是否是素数，因为素数测试需要比检查偶数/奇数更多的计算。
> 一个数变得奇数的概率大于它是质数的概率，这就是为什么我们首先检查这个数是否是奇数，然后再检查它是否是质数。

> 另一方面在`实现 2`中，我们在检查一个数是否为素数之前，先检查这个数是否为奇数，这使得不必要的计算，因为除了`2`之外的所有偶数都不是素数，但是实现仍然检查它们是否为素数。
