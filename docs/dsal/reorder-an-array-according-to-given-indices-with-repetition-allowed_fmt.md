...
> 原文：[https://www.geeksforgeeks.org/reorder-an-array-according-to-given-indices-with-repetition-allowed/](https://www.geeksforgeeks.org/reorder-an-array-according-to-given-indices-with-repetition-allowed/)

给定两个大小为 `N` 的整数数组 `arr` 和 `index`，任务是通过在 `index` 数组给定的索引处插入 `arr` 数组给定的元素来创建一个新数组。如果特定位置出现多次，则右移右侧数组元素，然后在给定索引处插入元素。

**注：** 给出了索引数组中所有的索引都在 `[0, N]` 的范围内。

**示例：**

> **输入：** `arr = [0, 1, 2, 3, 4]`, `index = [0, 1, 2, 2, 1]`
> **输出：** `[0, 4, 1, 3, 2]`
> **解释：**
> 首先我们在第 0，1，2 个索引处插入，这样数组就会变成 `[0, 1, 2]`
> 然后我们在第 2 个位置再次插入，右移所有右侧元素，这样数组就会变成 `[0, 1, 3, 2]`
>
> **输入：** `arr = [1, 2, 3, 4, 0]`, `index = [0, 1, 2, 3, 0]`
> **输出：** `[0, 1, 2, 3, 4]`
...
*   创建一个大小为 `N` 的新数组 `finalArr`，以存储结果输出。
*   对于给定 `arr` 数组中的每个元素，将其插入到由 `index` 数组给出的相应给定索引处，只需使用：
    ```
    finalArr[index[i]] = arr[i]
    where i is the current position during iteration
    ```
...
2.  对于给定 `arr` 数组中的每个元素，只需使用向量的 `insert()` 函数，将其插入到 `index` 数组给出的相应给定索引处。这将在特定位置插入，并自动处理重复位置。
