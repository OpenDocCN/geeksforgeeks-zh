...
> 原文: [https://www.geeksforgeeks.org/generate-an-array-with-product-of-all-subarrays-of-length-exceeding-one-divisible-by-k/](https://www.geeksforgeeks.org/generate-an-array-with-product-of-all-subarrays-of-length-exceeding-one-divisible-by-k/)

## 问题描述

给定两个正整数 `N` 和 `K`，任务是生成一个长度为 `N` 的数组，使得长度大于 `1` 的每个[子数组](https://www.geeksforgeeks.org/product-of-all-subarrays-of-an-array/)的乘积必须能被 `K` 整除，且数组的[最大元素](https://www.geeksforgeeks.org/c-program-find-largest-element-array/)必须小于 `K`。如果没有这样的数组，则打印 `-1`。
...
所以想法是取 `K` 的两个除数，设为 `d1` 和 `d2`，使得 `d1 * d2 = K`，并在数组中交替放置。按照以下步骤解决问题:
