...
给定大小为`N`的非负整数的数组`arr[]`，任务是找到一个整数`K`，以便该数组中至少有`K`个整数大于或等于`K`。
...
使用[散列](https://www.geeksforgeeks.org/counting-frequencies-of-array-elements/)。
...
{ 
    ArrayList<Integer> arr = new ArrayList<>(Arrays.asList(3, 0, 6, 1, 5)); 
