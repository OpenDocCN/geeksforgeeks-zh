...
> 原文：[https://www.geeksforgeeks.org/numbers-formed-by-flipping-common-set-bits-in-two-given-integers/](https://www.geeksforgeeks.org/numbers-formed-by-flipping-common-set-bits-in-two-given-integers/)
...
**天真方法：** 解决这个问题最简单的方法是[检查 `A` 和 `B` 的第 `i` 位是否都是一组](https://www.geeksforgeeks.org/check-whether-k-th-bit-set-not/)。如果发现为真，则翻转 `A` 和 `B` 的第 `i` 位。最后，打印 `A` 和 `B` 的更新值。
...
> 使用 `A = (A ^ (A & B))` 清除在 `A` 和 `B` 中均为设定位的 `A` 位。
> 使用 `B = (B ^ (A & B))` 清除在 `A` 和 `B` 中均为设定位的 `B` 位。
