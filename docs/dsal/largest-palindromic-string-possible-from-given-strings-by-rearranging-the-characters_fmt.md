# 通过重新排列字符，从给定字符串中选出最大可能的回文字符串

> 原文: [https://www.geeksforgeeks.org/largest-palindromic-string-possible-from-given-strings-by-rearranging-the-characters/](https://www.geeksforgeeks.org/largest-palindromic-string-possible-from-given-strings-by-rearranging-the-characters/)
...
2.  从子串 `A` 和 `B` 中获取唯一元素的可能情况是:
    *   如果在 `A` 和 `B` 中都存在一个独特的元素，那么取两者，因为会使 `T` 的长度增加 2。
