...
> 原文：[https://www.geeksforgeeks.org/find-maximum-value-of-x-such-that-n-kx-0/](https://www.geeksforgeeks.org/find-maximum-value-of-x-such-that-n-kx-0/)

给定两个整数 `n` 和 `k`。任务是找到 `x` 的最大值，使得 `n! % (k^x) = 0`。
...
1.  首先取 `k` 的平方根，存储在一个变量中，比如 `m`。
2.  从 `i=2` 到 `m` 循环。
3.  如果 `i = m`，则将 `k` 复制到 `i`。
4.  如果 `k` 能被 `i` 整除，那就用 `i` 整除 `k`。
5.  对 `n` 运行一个循环，将商加到一个变量上，比如 `u`。
6.  每次循环后存储 `r` 的最小值。
