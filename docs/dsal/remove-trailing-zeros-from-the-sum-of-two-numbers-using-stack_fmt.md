...
> 原文: [https://www.geeksforgeeks.org/remove-trailing-zeros-from-the-sum-of-two-numbers-using-stack/](https://www.geeksforgeeks.org/remove-trailing-zeros-from-the-sum-of-two-numbers-using-stack/)
...
*   计算 `A + B` 并存储在变量中，比如 `N`。
*   初始化一个 `栈< char >`，说 `S`，存储 `N` 的数字。
*   [将整数 `N` 转换为字符串](https://www.geeksforgeeks.org/converting-string-to-number-and-vice-versa-in-c/)，然后将字符推入堆栈 `S`。
*   当 `S` 不为空(`!S.empty()`)时迭代，如果堆栈的[顶部元素为“0”](https://www.geeksforgeeks.org/stack-top-c-stl/)[则将其弹出堆栈](https://www.geeksforgeeks.org/stack-pop-method-in-java/)。否则，[断开](https://www.geeksforgeeks.org/break-statement-cc/)。
*   初始化一个[字符串](https://www.geeksforgeeks.org/string-data-structure/)，比如说 `res`，来存储结果字符串。
*   在 [`S` 不为空的情况下迭代(`!S.empty()`)](https://www.geeksforgeeks.org/stack-empty-and-stack-size-in-c-stl/)，推送 `res` 中的所有字符，然后弹出顶部元素。
