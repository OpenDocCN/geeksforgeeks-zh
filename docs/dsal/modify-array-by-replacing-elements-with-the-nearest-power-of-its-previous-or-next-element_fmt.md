...
> 原文：[https://www.geeksforgeeks.org/modify-array-by-replacing-elements-with-the-nearest-power-of-its-previous-or-next-element/](https://www.geeksforgeeks.org/modify-array-by-replacing-elements-with-the-nearest-power-of-its-previous-or-next-element/)
...
*   [遍历阵列](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]` 并执行以下步骤：
    *   找出 `X^K` 最接近 `y` 的 `K` 的值。
    *   计算 `K` 取[地板值](https://www.geeksforgeeks.org/ceil-floor-functions-cpp/) `log_x(Y)`。
    *   因此 `K` 和 `K + 1` 将是最接近幂的两个整数。
    *   计算 `Y^K` 和 `Y^(K+1)` 并检查哪个最接近 `X` 并用最接近的值更新数组元素 `arr[i]`。
*   完成以上步骤后，[打印数组 `arr[]` 作为修改后的数组。](https://www.geeksforgeeks.org/c-program-to-print-an-array-using-recursion/)
