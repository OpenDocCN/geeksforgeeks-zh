...
> 原文: [https://www.geeksforgeeks.org/program-calculate-area-perimeter-equilateral-triangle/](https://www.geeksforgeeks.org/program-calculate-area-perimeter-equilateral-triangle/)
...
*   如果我们知道等边三角形的高和边长，那么我们可以使用下面提到的公式来计算它的面积。
...
我们来看看下图。我们知道三角形的 1/2 *底*高。`h`的值为`sqrt(a^2–(a/2)^2)= sqrt(3)* a/2`。所以面积变成了`1/2 * a *(sqrt(3)* a/2)=(sqrt(3)/4)* a * a`
