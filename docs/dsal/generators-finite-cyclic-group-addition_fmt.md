...
> 原文: [https://www.geeksforgeeks.org/generators-finite-cyclic-group-addition/](https://www.geeksforgeeks.org/generators-finite-cyclic-group-addition/)
...
一个**简单的解决方案**是运行一个从 `1` 到 `n-1` 的循环，对于每个元素检查它是否是生成器。为了检查生成器，我们不断添加元素，并检查是否可以生成所有数字，直到余数开始重复。
...
一个数 `n` 的生成器个数等于 `φ(n)`，其中 `φ` 是[欧拉全能性函数](https://www.geeksforgeeks.org/eulers-totient-function/)。
