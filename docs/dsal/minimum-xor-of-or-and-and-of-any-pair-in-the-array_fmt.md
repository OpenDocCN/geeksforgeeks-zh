...
> 原文: [https://www.geeksforgeeks.org/minimum-xor-of-or-and-and-of-any-pair-in-the-array/](https://www.geeksforgeeks.org/minimum-xor-of-or-and-and-of-any-pair-in-the-array/)

给定一个正整数的数组`arr[]`，大小为`N`。任务是找到给定数组中任意一对的按位“或”与“与”的按位“异或”的最小值。
...
> **解释:**
> 对于元素 2 和 3:
> 表达式`(2 & 3) xor (2 | 3)`的值为 1，这是给定数组中所有对的最小值。
> **输入:** `arr[] = {3, 6, 8, 4, 5}`
> **输出:** 1
> **解释:**
> 对于元素 4 和 5:
...
对于任意一对元素，如 `X` 和 `Y`，表达式 `(X & Y) xor (X | Y)` 的值可以写成:
...
根据以上计算，对于给定数组中的任意对 `(X, Y)`，表达式 `(X & Y) xor (X | Y)` 简化为 `X xor Y`。因此，给定数组中任意对的按位“或”与“与”的按位“异或”的最小值就是最小的对的“异或”，可以使用本文中讨论的方法来计算。
