...
> 原文：[https://www.geeksforgeeks.org/area-of-circumcircle-and-incircle-of-a-right-kite/](https://www.geeksforgeeks.org/area-of-circumcircle-and-incircle-of-a-right-kite/)
...
有一些观察来解决这个问题。按照以下步骤解决此问题：

*   这里 `a = AB = AD`，`b = BC = CD`
*   在风筝 `ABCD` 中，以相反的角度 `B` 和 `D` 作为 `90`，这样相反的角度可以计算为 `tan (A/2) = b/a` 和 `tan(C/2) = a/b`
*   让 `p` 作为对角线的长度 `AC`，`q` 作为对角线的长度 `BD`。
*   对角线 `AC` 可以使用[勾股定理](https://www.geeksforgeeks.org/pythagoras-theorem-and-its-converse-triangles-class-10-maths/)轻松计算。因此 `p = (a^2 + b^2)`
*   由于对角线等于风筝外接圆的直径，外接圆的半径计算为 `R = (a^2 + b^2)^(1/2) / 2`
*   因此，外接圆的面积将是 `π * R * R`
