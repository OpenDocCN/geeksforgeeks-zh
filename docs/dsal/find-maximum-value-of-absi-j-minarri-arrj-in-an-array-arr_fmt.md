# 求数组 `arr[]` 中 `ABS(I–j)* min(arr[I]，arr[j])` 的最大值

> 原文: [https://www.geeksforgeeks.org/find-maximum-value-of-absi-j-minarri-arrj-in-an-array-arr/](https://www.geeksforgeeks.org/find-maximum-value-of-absi-j-minarri-arrj-in-an-array-arr/)
...
一个**有效的解决方案**以线性时间复杂度解决了这个问题。我们取两个迭代器 `left = 0` 和 `right = n-1`，比较元素 `arr[left]` 和 `arr[right]`。
...
**这是如何工作的？**
重要的是要表明我们没有错过上面线性算法中的任何潜在对，也就是说，我们需要表明做 `left++` 或 `right--` 不会导致我们会得到更高 `maxProduct` 值的情况。
请注意，我们总是与 `(right - left)` 相乘。

1) 如果当前 `arr[left] < arr[right]`, then smaller values of `right` 没有用，因为它们不能产生更高的 `maxProduct` 值(因为我们用 `arr[left]` 乘以 `(right–left)`)。如果 `arr[left]` 大于其左侧的任何元素会怎样。在这种情况下，必须找到具有当前 `right` 的更好的元素对。因此，我们可以安全地增加 `left`，而不会错过任何更好的 `left` 电流对。

2) 当 `arr[right] < arr[left]` 时，类似的参数也适用。
