...
> 原文：[https://www.geeksforgeeks.org/number-of-ways-of-cutting-a-matrix-such-that-atleast-one-cell-is-filled-in-each-part/](https://www.geeksforgeeks.org/number-of-ways-of-cutting-a-matrix-such-that-atleast-one-cell-is-filled-in-each-part/)
...
>
> **输入：** `mat[][] = {{1, 0, 0}, {1, 1, 0}, {0, 0, 0}}`，`K = 3`
> **输出：** 1

**方法：** 想法是使用[动态编程](https://www.geeksforgeeks.org/dynamic-programming/)来计算切割具有至少一个填充单元的矩阵的方法的数量。

*   计算矩阵的[前缀和](https://www.geeksforgeeks.org/prefix-sum-array-implementation-applications-competitive-programming/)，这样我们就可以在 `O(1)` 时间内计算出矩阵的特定部分是否包含一个填充的单元格。
*   定义一个 `dp` 表，存储将矩阵切割成 `K` 个部分的方法数量，其中 `dp[k][r][c]` 表示从左上方到第 `r` 行和第 `c` 列将矩阵切割成 `k` 个部分的方法数量。
*   最后，对每一个可能的切割位置进行迭代，检查矩阵是否可以被分成两部分，直到该索引，并且这两部分是否都有效。
...
int ways(vector<vector<int>> &arr, int K)
{
  ...
  for (int r = R - 1; r >= 0; r--)
  {
    for (int c = C - 1; c >= 0; c--)
    {
      ...
      if (r + 1 < R) preSum[r][c] += preSum[r + 1][c];

      if (c + 1 < C) preSum[r][c] += preSum[r][c + 1];

      if (r + 1 < R && c + 1 < C) preSum[r][c] -= preSum[r + 1][c + 1];
    }
  }

  // dp(r, c, 1) = 1
  // if preSum[r][c] else 0
  int dp[K + 1][R][C];
      ...
      for (int c = C - 1; c >= 0; c--)
      {
        if (k == 1)
        {
          ...
          for (int r1 = r + 1; r1 < R; r1++)
          {
            ...
            // matrix (r, c) -> r1, C-1
            if (preSum[r][c] - preSum[r1][c] > 0)
              dp[k][r][c] += dp[k - 1][r1][c];
          }
          for (int c1 = c + 1; c1 < C; c1++)
          {
            ...
            // matrix (r, c) -> R-1, c1
            if (preSum[r][c] - preSum[r][c1] > 0)
              dp[k][r][c] += dp[k - 1][r][c1];
          }
        }
      }
    }
