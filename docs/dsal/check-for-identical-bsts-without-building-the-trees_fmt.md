...
> 原文：[https://www.geeksforgeeks.org/check-for-identical-bsts-without-building-the-trees/](https://www.geeksforgeeks.org/check-for-identical-bsts-without-building-the-trees/)
...
根据 BST 性质，左子树的元素必须小于，右子树的元素必须大于根。如果对于每个元素 `x`，`x` 的左右子树中的元素在两个数组中都出现在它之后，则两个数组表示相同的 BST。左右子树的根也是如此。
