...
## 先决条件
: [全为 1 的二进制矩阵的矩形子矩阵数](https://www.geeksforgeeks.org/number-of-submatrices-with-all-1s/)。

## 朴素解法
一个简单的解法是生成所有的子矩阵，并为每个子矩阵找到所需的`and`。这种方法的时间复杂度为`0(N^6)`。

## 高效方法
为了更好的理解，我们假设一个元素的任意一位用变量`I`表示，变量`sum`用来存储最终的和。
这里的想法是，我们将尝试使用第`i`位集来查找`AND`值的数量(具有逐位`and(&)`的子矩阵)。让我们假设，设置了第`i`位的子矩阵有`S_i`个。对于`i`位，该和可以更新为`和+=(2^I * S_I)`。
对于每个位`I`，创建一个布尔矩阵`set_bit`，如果设置了`arr[R][C]`的`I`位，则在索引`(R，C)`处存储`1`。否则，它存储`0`。然后，对于这个布尔数组，我们尝试寻找全为 1 的矩形子矩阵的个数(`S_i`)。对于第`i`位，最终总和将更新为:
