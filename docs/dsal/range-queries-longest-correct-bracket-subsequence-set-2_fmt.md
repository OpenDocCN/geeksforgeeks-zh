...
> 原文：[https://www.geeksforgeeks.org/range-queries-longest-correct-bracket-subsequence-set-2/](https://www.geeksforgeeks.org/range-queries-longest-correct-bracket-subsequence-set-2/)
...
方法：在上一篇帖子（[SET 1](https://www.geeksforgeeks.org/range-queries-longest-correct-bracket-subsequence/)）中我们讨论了一个对每个查询都适用于 `O(log n)` 的解决方案，现在是这篇帖子我们将去看一个对每个查询都适用于 `O(1)` 的解决方案。
这个想法基于最长有效平衡子串的帖子（[长度](https://www.geeksforgeeks.org/length-of-the-longest-valid-substring/)）。如果我们在一个临时数组中标记所有平衡括号/括号的索引（这里我们将其命名为 `BCP[]`，`BOP[]`），那么我们在 `O(1)` 时间内回答每个查询。
