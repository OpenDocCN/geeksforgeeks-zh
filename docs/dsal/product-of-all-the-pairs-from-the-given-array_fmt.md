...
> 原文: [https://www.geeksforgeeks.org/product-of-all-the-pairs-from-the-given-array/](https://www.geeksforgeeks.org/product-of-all-the-pairs-from-the-given-array/)
...
解决上述问题的天真方法是找到所有可能的对，并计算每对元素的乘积。
...
我们可以观察到，每个元素作为一对 `(X, Y)` 的元素之一，恰好出现 `(2 * N)` 次。正好 `N` 倍为 `X`，正好 `N` 倍为 `Y`。
