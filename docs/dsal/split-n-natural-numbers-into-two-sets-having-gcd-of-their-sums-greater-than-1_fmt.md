...
> 原文：[https://www.geeksforgeeks.org/split-n-natural-numbers-into-two-sets-having-gcd-of-their-sums-greater-than-1/](https://www.geeksforgeeks.org/split-n-natural-numbers-into-two-sets-having-gcd-of-their-sums-greater-than-1/)

给定一个整数 `N`，任务是创建从 `1` 到 `N` 的两组不同元素，使得它们各自和的 [gcd](https://www.geeksforgeeks.org/basic-and-extended-euclidean-algorithms/) 大于 1。打印相应的集合。如果无法进行这种拆分，请打印 `-1`。
...
一种简单的方法是将 `N` 以内的所有偶数拆分为一个集合，将所有奇数拆分为另一个集合。
...
1.  **如果 `N` 为奇数:**
    ...
    *   所以我们只需要将 `(N+1)/2` 放入第一个集合中，剩余的数字放入第二个集合。
    *   这将自动使其总和的 GCD 大于 1。
2.  **如果 `N` 为偶数:**
    *   在这种情况下，`N` 个自然数的和可以被 `N/2` 整除。
    *   因此我们只需要将 `N/2` 放入第一个集合中，剩余的数字放入第二个集合。
    *   这将自动使其总和的 GCD 大于 1。
