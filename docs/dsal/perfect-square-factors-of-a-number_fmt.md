...
> 原文: [https://www.geeksforgeeks.org/perfect-square-factors-of-a-number/](https://www.geeksforgeeks.org/perfect-square-factors-of-a-number/)
...
解决这个问题最简单的方法就是[找到给定数 `N` 的所有可能因子](https://www.geeksforgeeks.org/find-divisors-natural-number-set-1/)，对于每个因子，检查[因子是否为完美平方](https://www.geeksforgeeks.org/check-if-given-number-is-perfect-square-in-cpp/)。对于发现的每一个因素，增加 `计数`。打印最终 `计数`。
...
> `N = (1+a₁)*(1+a₂)*(1+a₃)*...*(1 + aₙ)`
> 其中 `a₁`，`a₂`，`a₃`，…，`aₙ` 是 `n` 的不同素因子的计数。

在一个完美的正方形中，不同质因数的计数必须能被 2 整除。因此，完美平方因子的计数由下式给出:

> 完美平方的 `N` 的因子 = `(1+a₁/2)*(1+a₂/2)*……*(1+aₙ/2)` 其中 `a₁`，`a₂`，`a₃`，…，`aₙ` 是 `N` 的不同素因子的计数。
