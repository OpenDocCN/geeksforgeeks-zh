...
> 原文: [https://www.geeksforgeeks.org/angle-between-a-pair-of-lines-in-3d/](https://www.geeksforgeeks.org/angle-between-a-pair-of-lines-in-3d/)
...
> **输出:** `54.6065`
> **输入:** `x1 = 10, y1 = 10, z1 = 10; x2 = 0, y2 = 0, z2 = 0; x3 = 15, y3 = 10, z3 = 15`
> **输出:** `56.4496`
...
2.  用 `cosθ` 的公式求 `AB` 线和 `BC` 线的两个方向比，求 `AB` 线和 `BC` 线夹角的余弦为:
    > ```
...
3.  假设有两个方向比:
    ...
    然后
    > 点积 `(A · B) = A * x + B * y + c * z`
    > `A` 的量值 = `|A| = \sqrt{a^{2} + b^{2} + c^{2}}`
    > `B` 的量值 = `|B| = \sqrt{x^{2} + y^{2} + z^{2}}`

4.  计算角度的余弦值给出了弧度的余弦值。要找到角度，将余弦值乘以 `(180 / π)`。
