...
> 原文: [https://www.geeksforgeeks.org/solving-homogeneous-recurrence-equations-using-polynomial-reduction/](https://www.geeksforgeeks.org/solving-homogeneous-recurrence-equations-using-polynomial-reduction/)
...
```
T(n) - a1*T(n-1) - a2*T(n-2) = 0
```

设 `T(n) = x^n`。
现在我们可以说 `T(n-1) = x^(n-1)` 和 `T(n-2) = x^(n-2)`。
现在等式将为：
...
将整个方程除以 `x^(n-2)`（由于 `x` 不等于 `0`），我们得到：

```
x^2 - a1*x - a2 = 0
```

我们得到的特征方程为 `x^2 + a1*x + a2 = 0`。
现在，求这个方程的根。
...
如果 `r1, r2, r3, ……, rk` 是特征方程的根，则递推方程的通解为：

```
tn = c1*r1^n + c2*r2^n + c3*r3^n +...........+ ck*rk^n
```
...
如果 `r1` 和 `r2` 是特征方程的两个根，并且它们彼此共轭，则可以表示为：
