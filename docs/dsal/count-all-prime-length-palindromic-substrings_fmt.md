...
> 原文: [https://www.geeksforgeeks.org/count-all-prime-length-palindromic-substrings/](https://www.geeksforgeeks.org/count-all-prime-length-palindromic-substrings/)
...
使用厄拉多塞的[筛](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)，找到所有的素数直到 `str` 的长度，因为那是 `str` 的 `子串` 所能拥有的最大长度。现在从最小素数开始，即 `j = 2` 到 `j ≤ len(str)`。如果 `j` 为质数，则统计 `str` 中 `长度= j` 的所有回文子串。打印最后的总计数。
