...
我们已经在[中讨论了 O(V³K) 方法，计算从一个源到一个目的地的所有可能的步行，正好有 K 条边](https://www.geeksforgeeks.org/count-possible-paths-source-destination-exactly-k-edges/)。本文讨论了一种 O(V³LogK) 方法。

## 方法

想法是计算结果矩阵，其中 `result = (graph)^k`。长度为 `k` 的从源到目的地的路径总数将简单地为 `result[src][dest]`。我们使用[这个](https://www.geeksforgeeks.org/write-a-c-program-to-calculate-powxn/)技术来计算给定图的邻接矩阵的幂。

这里指数=7 所用的 `pow()` 函数递归树如下：
