...
> 原文: [https://www.geeksforgeeks.org/program-to-find-nth-odd-fibonacci-number/](https://www.geeksforgeeks.org/program-to-find-nth-odd-fibonacci-number/)

给定一个整数 `N`，任务是找到第 `N` 个奇数斐波那契数。
奇数斐波那契数列为:
...
**进场:**
仔细观察可以推断，每三个斐波那契数中就有一个是偶数，所以第 `N` 个的奇数斐波那契数就是一般斐波那契数列中的 **{(3*N+1)/2} 第个**项。
以下是上述方法的实施:
