...
> 原文: [https://www.geeksforgeeks.org/print-all-array-elements-occurring-at-least-m-times/](https://www.geeksforgeeks.org/print-all-array-elements-occurring-at-least-m-times/)
...
*   从左到右遍历数组
*   检查一个元素是否已经出现在前面的遍历中。如果出现，检查下一个元素。否则再次从第 `i` 个位置到第 `(n–1)` 个位置遍历数组。
*   如果频率是 `>= M`，打印元素。
...
给定的问题可以通过[将数组元素](https://www.geeksforgeeks.org/counting-frequencies-of-array-elements/)的频率存储在 [HashMap](https://www.geeksforgeeks.org/java-util-hashmap-in-java-with-examples/) 中，比如说 `freq`，将频率 `>= M` 的地图中所有元素至少打印 `M` 来解决。
