...
> 原文：[https://www.geeksforgeeks.org/amazon-interview-experience-219-on-campus/](https://www.geeksforgeeks.org/amazon-interview-experience-219-on-campus/)
...
2) 给定 `n` 个非负整数，代表每个条形宽度为 1 的建筑物，计算下雨后能截留多少水，例如给定 `[0, 1, 0, 2, 1, 0, 1, 3, 2, 1, 2, 1]`，返回 `6`。
...
4. 在节点 `A` 和 `B` 的一个二叉查找树中找到最低的共同祖先，他问我是否已经看到这个问题了，我回答是肯定的，所以他让我快速编码，我就编码了。但是他问如果节点 `A` 或 `B` 或者两者都不存在怎么办，我说这个代码不起作用，但是我们可以首先检查它，节点 `A` 和 `B` 都存在，然后我们可以使用这个代码，他确信并移动到下一个代码。
5. 找出最多有一个弯曲的二叉树任意两个节点之间的最大距离。我们在这个问题上讨论了很多，然后他要求对我们讨论的方法进行编码，当编码时，我发现它是错误的，我告诉他这种方法行不通。最后他说把这个问题移到下一个。(只有我解决不了的问题)
6. 在 `LL` 中查找循环。
7. 找出一段话中的词频。我起初建议用散列法，但他对答案很满意。
8. 再次对项目进行简短的讨论。
...
3. 给定一组类和它们的依赖项，如果这个给定的集合可以在有或没有错误的情况下进行 `BUILD`(编译)，则返回。
   ...
   G 类: A
   ```
   返回错误，因为 `A` 类、`C` 类、`E` 类相互依赖。我说的问题和他同意并要求编码的图中的 `find loop` 一样。编码。
...
6. 什么是互斥？
7. 给定外来语言的字典顺序的单词，找到该语言中字符的顺序。我说我见过这个问题，是拓扑排序的问题，他接着下一个问题。
8. 多生产者单一消费者问题。
