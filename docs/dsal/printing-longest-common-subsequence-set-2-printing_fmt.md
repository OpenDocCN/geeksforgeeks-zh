...
> 原文: [https://www.geeksforgeeks.org/printing-longest-common-subsequence-set-2-printing/](https://www.geeksforgeeks.org/printing-longest-common-subsequence-set-2-printing/)
...
a) 如果 `X` 和 `Y` 的最后一个字符相同(即 `X[i-1] == Y[j-1]`)，则该字符必须出现在子串 `X[0…i-1]` 和 `Y[0…j-1]` 的所有 LCS 中。我们简单地在矩阵中递归 `L[i-1][j-1]`，并将当前字符附加到子串 `X[0…i-2]` 和 `Y[0…j-2]` 的所有 LCS 可能值上。
