...
> 原文: [https://www.geeksforgeeks.org/check-if-concatenation-of-two-strings-is-balanced-or-not/](https://www.geeksforgeeks.org/check-if-concatenation-of-two-strings-is-balanced-or-not/)
...
2.  现在遍历表达式字符串 `exp`。
    ...
    *   如果当前字符是右括号(`')'` 或 `'}'` 或 `']'`)，则从堆栈中弹出，如果弹出的字符是匹配的起始括号，则继续；否则括号不均衡。
...
一个有效的解决方案是检查给定的序列是否可以在不使用堆栈的情况下产生平衡的括号序列，即在恒定的额外空间中。
