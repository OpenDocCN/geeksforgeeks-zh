...
其思想是首先对给定的数组进行排序，然后使用类似于厄拉多塞的[筛的概念](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)。首先取一个大尺寸的数组(它是 `x` 的最大尺寸)。最初在所有索引中保持零。在零索引处做 `1`(无论数组是什么，我们都可以得到零)。现在，遍历整个数组，将所有可能的值设为 `1`。
