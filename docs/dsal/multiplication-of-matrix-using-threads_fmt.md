...
> 原文：[https://www.geeksforgeeks.org/multiplication-of-matrix-using-threads/](https://www.geeksforgeeks.org/multiplication-of-matrix-using-threads/)

矩阵的乘法确实需要时间。矩阵乘法的时间复杂度是 `O(n^3)`，使用标准矩阵乘法。而 Strassen 算法对其进行了改进，其时间复杂度为 `O(n^(2.8074))`。
但是，有没有办法用正规的方法来提高矩阵乘法的性能。
可以做多线程来改善。在多线程中，我们不是利用处理器的单个内核，而是利用所有或更多的内核来解决问题。
