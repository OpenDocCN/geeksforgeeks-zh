...
> 原文：[https://www.geeksforgeeks.org/problem-solving-minimum-spanning-trees-kruskals-prims/](https://www.geeksforgeeks.org/problem-solving-minimum-spanning-trees-kruskals-prims/)
...
如何求最小生成树的权重？这是基于 `MST` 的最简单类型的问题。为了用克鲁斯卡尔算法解决这个问题，
...
```
添加前三条边 `(v4，v5)`，`(v3，v5)`，`(v2，v4)`，不会创建循环。此外，我们可以使用边 `(v1，v2)` 将 `v1` 连接到 `v2`。总重量是这 `4` 条边的重量之和，即 `10`。
...
添加到 `MST` 的边序列？要解决这类问题，请尝试找出 `Kruskal` 可以产生的边序列。不匹配的序列就是答案。
