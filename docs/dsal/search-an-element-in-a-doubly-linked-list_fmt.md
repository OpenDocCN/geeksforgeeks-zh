...
> 原文：[https://www.geeksforgeeks.org/search-an-element-in-a-doubly-linked-list/](https://www.geeksforgeeks.org/search-an-element-in-a-doubly-linked-list/)

给定一个包含 `N` 节点和一个整数 `X` 的[双向链表(DLL)](https://www.geeksforgeeks.org/doubly-linked-list/)，任务是找到整数 `X` 在双向链表中的位置。如果没有找到这样的位置，则打印 `-1`。
...
> **输入：** `15<=>16<=>8<=>7<=>13`，`X = 8`
> **输出：** `3`
> **说明：** `X` (= 8)出现在双向链表的第 3 个节点。因此，要求的输出为 `3`。
>
> **输入：** `5 <=> 3 <=> 4 <=> 2 <=> 9`，`X = 0`
> **输出：** `-1`
> **说明：** `X` (= 0)不在双向链表中。因此，要求的输出为 `-1`。

**方法：** 按照以下步骤解决问题：

*   初始化一个变量，比如说 `pos`，来存储包含数据值 `X` 的节点在[双向链表](https://www.geeksforgeeks.org/doubly-linked-list/)中的位置。
*   初始化一个指针，比如 `temp`，存储[双向链表](https://www.geeksforgeeks.org/doubly-linked-list/)的**头节点**。
