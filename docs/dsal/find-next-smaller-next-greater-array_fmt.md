...
> 原文：[https://www.geeksforgeeks.org/find-next-smaller-next-greater-array/](https://www.geeksforgeeks.org/find-next-smaller-next-greater-array/)
...
一个**简单的解决方案**就是遍历所有元素。对于每个元素，找到当前元素的下一个较大的元素，然后为当前的下一个较大的元素找到正确的较小的元素。此解决方案花费的时间为`O(n^2)`。
**高效解决方案**耗时`O(n)`。注意，它是数组中[下一个较大元素](https://www.geeksforgeeks.org/next-greater-element/) & [下一个较小元素](https://www.geeksforgeeks.org/find-maximum-difference-between-nearest-left-and-right-smaller-elements/)的组合。
