...
给定两个分别由 `N` 和 `M` 个整数组成的数组 `H[]` 和 `B[]`，分别表示孔和球的直径。`M` 个球在带有 `N` 个孔的斜面上从 `A` 滚动到 `B`，各孔深度不同，如下图所示:
...
*   一个洞 `H[i]` 会变满，如果 `i` 号球落入其中。
*   如果一个洞满了，就不会再有球掉进去了。
*   一个球将从 `A` 到达 `B`，如果并且仅当它没有落入任何一个孔中。
*   如果一个球在洞 `P[i]`，那么它的位置就是 `i`。如果一个球到达了底点 `B`，那么把它的位置作为 `0`。

**示例:**

> **输入:** `H[] = {21, 3, 6}`，`B[] = {20, 15, 5, 7, 10, 4, 2, 1, 3, 6, 8}`
> **输出:** `1 0 3 0 3 3 2 0 0`
> **解释:** 直径 `20` 的球将落入孔 `H[1]` 中，孔 `H[1]` 将变满。
> 直径为 `15`、`7`、`10` 的球将到达底部，因为孔 `H[1]` 已满，孔 `H[2]` 和 `H[3]` 的直径小于球的直径。
> 直径为 `5`、`4`、`2` 的球会掉入 `H[3]` 孔。
> 直径为 `1` 的球会掉入孔 `H[2]` 中，因为孔 `H[3]` 已经满了。
> 直径为 `3` 的球会落入 `H[2]` 孔。
> 直径为 `6`、`8` 的球将到达最低点 `B`。
> 球 `20` 的位置为 `1`，因为它在孔 `H[1]` 中。
> 球 `15`、`7`、`10`、`3`、`6`、`8` 的位置为 `0`，因为它们到达了底点 `B`。
> 因此，直径为 `5`、`4`、`2` 的球在第 `3` 孔 `H[3]` 中，直径为 `1`、`3` 的球在第 `2` 孔 `H[2]` 中。

> **输入:** `H[] = {20, 15, 10, 5, 25}`，`B[] = {5, 10, 15, 20, 25, 30, 4, 9, 14, 19}`
> **输出:** `5 5 5 5 0 4 3 2 1`

**方法:** 按照以下步骤解决问题:

*   初始化一个大小为 `N` 的数组 `位置[]` 存储每个球的最终位置，初始化一个大小为 `N` 的数组 `深度[]` 存储每个洞的容量。
*   使用变量 `i` 迭代范围 `[1, N]`，并将 `孔[i]` 的初始 `深度[i]` 设置为 `i+1`。
*   [使用变量 `i` 遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `球[]`，并执行以下操作:
    *   使用变量 `j` 以相反的顺序迭代数组 `孔[]`。
        *   检查孔的直径是否大于或等于球的直径，即 `孔[j] >= 球[i]`，如果该孔未满，即 `深度[j] > 0`，则通过将 `j + 1` 附加在 `位置[]` 数组中，并将孔的深度递减 `1` 和[以断开回路](https://www.geeksforgeeks.org/break-statement-cc/)的方式将球放入该孔中。
