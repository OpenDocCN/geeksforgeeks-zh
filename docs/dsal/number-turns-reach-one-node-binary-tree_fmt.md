...
> 原文: [https://www.geeksforgeeks.org/number-turns-reach-one-node-binary-tree/](https://www.geeksforgeeks.org/number-turns-reach-one-node-binary-tree/)
...
我们必须遵循这个步骤。
1. 找到给定两个节点的`LCA`。
2. 给定节点出现在`LCA`的左侧或右侧或等于`LCA`。
3. 根据上述条件，我们的程序属于两种情况之一:
...
4. **`CountTurn`函数的工作原理**：
   如果我们移动到左子树，我们传递`true`；如果我们移动到右子树，我们传递`false`。
