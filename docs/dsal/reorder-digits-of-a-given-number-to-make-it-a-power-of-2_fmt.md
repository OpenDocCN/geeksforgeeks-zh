...
> 原文: [https://www.geeksforgeeks.org/reorder-digits-of-a-given-number-to-make-it-a-power-of-2/](https://www.geeksforgeeks.org/reorder-digits-of-a-given-number-to-make-it-a-power-of-2/)

给定一个正整数 `N`，任务是重新排列给定整数的位数，使整数成为 `2` 的幂。如果存在多个解，则打印最小可能的整数，而不前导 `0`。否则，打印 `-1`。
...
想法是生成给定整数的数字的所有排列。对于每个排列，检查整数是否是 `2` 的幂，或者不是。如果发现为真，则打印整数。否则，打印 `-1`。按照以下步骤解决问题:

*   将给定的整数转换成字符串，比如说 `字符串`。
*   按升序排列字符串。
*   生成字符串的所有可能排列。对于每个排列，检查字符串的等价整数值是否是 `2` 的幂，或者不是。如果发现是真的，那就打印号码。
