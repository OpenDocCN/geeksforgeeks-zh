...
> 原文: [https://www.geeksforgeeks.org/segregate-1s-and-0s-in-separate-halves-of-a-binary-string/](https://www.geeksforgeeks.org/segregate-1s-and-0s-in-separate-halves-of-a-binary-string/)

给定一个偶数长度的[二进制字符串](https://www.geeksforgeeks.org/tag/binary-string/) `字符串`，由相等数量的 `0` 和 `1` 组成，任务是[通过重复](https://www.geeksforgeeks.org/segregate-0s-and-1s-in-an-array-by-traversing-array-once/)[反转子字符串](https://www.geeksforgeeks.org/reverse-the-substrings-of-the-given-string-according-to-the-given-array-of-indices/)将所有的 1 和 0 分成两半。打印所需的最小反转次数。
...
*   现在，反转任何子串后，计数减少 `2`。
*   如果 `ans` 的值为奇数，则所需答案为 `(ans–1)/2`，因为最终的字符串将在字符串的中心包含一对这样的字符串。
*   否则，所需答案为 `ans/2`。
