...
使用 [`Eratosthenes 筛子`](http://www.geeksforgeeks.org/sieve-of-eratosthenes/)生成一个布尔向量，该布尔向量达到数组中最大元素的大小，该布尔向量可用于检查数字是否为质数。
...
创建布尔向量 `prime[0..n]`。最终，`prime[i]` 的值如果为 `false`，则表示 `i` 不是质数；否则为 `true`。
```python
...
计算数组中 k 个最小和 k 个最大质数的异或值的函数。
```python
def kMaxXOR(arr: list, n: int, k: int):
    # 在数组中找到最大值
    max_val = max(arr)

    # 使用筛法找到所有小于等于 max_val 的质数
    prime = SieveOfEratosthenes(max_val)

    # 使用最小堆来存储最大的 K 个质数和合数
    minHeapPrime, minHeapNonPrime = [], []
    ...
    for i in range(n):
        ...
        if prime[arr[i]]:
            ...
            # 如果最小堆的大小为 K 且堆顶元素小于当前元素，
            # 则需要用当前元素替换它，因为只需要最大的 k 个元素
