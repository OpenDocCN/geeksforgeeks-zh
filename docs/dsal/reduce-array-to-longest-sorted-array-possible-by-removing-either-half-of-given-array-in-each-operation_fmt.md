...
> 原文：[https://www.geeksforgeeks.org/reduce-array-to-longest-sorted-array-possible-by-removing-either-half-of-given-array-in-each-operation/](https://www.geeksforgeeks.org/reduce-array-to-longest-sorted-array-possible-by-removing-either-half-of-given-array-in-each-operation/)

给定一个大小为 `N`（始终为 2 的幂）的数组 `arr[]`，任务是找到最长的排序数组的长度，通过在每次操作时移除数组的任意一半，给定数组可以被缩减到该长度。
...
> **输出：** 2
> **解释：**
> 移除 `arr[]` 的前半部分将 `arr[]` 修改为 `{13, 14, 3, 4}`。
...
*   递归地将数组分成相等的两半，对于每一半，检查数组的分区是否排序。如果发现是真的，那么更新 `MaxLength` 到分区的长度。
*   最后，打印 `MaxLength` 的值。
