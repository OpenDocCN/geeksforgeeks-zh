...
> 原文：[https://www.geeksforgeeks.org/size-array-repeated-deletion-lis/](https://www.geeksforgeeks.org/size-array-repeated-deletion-lis/)

给定数组 `arr[0..n-1]` 包含正元素。任务是在重复删除 LIS（大小大于 1）后打印 `arr[]` 的剩余元素。如果有多个长度相同的 LIS，我们需要选择最先结束的 LIS。
