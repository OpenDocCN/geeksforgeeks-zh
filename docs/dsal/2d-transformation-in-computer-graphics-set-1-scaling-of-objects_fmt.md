...
> 原文：[https://www.geeksforgeeks.org/2d-transformation-in-computer-graphics-set-1-scaling-of-objects/](https://www.geeksforgeeks.org/2d-transformation-in-computer-graphics-set-1-scaling-of-objects/)

## 缩放变换

缩放变换会改变对象的大小。在缩放过程中，我们要么压缩，要么扩展对象的维度。
缩放操作可以通过将多边形的每个顶点坐标 `(x, y)` 乘以缩放因子 `sx` 和 `sy` 来实现，以产生变换后的坐标 `(x', y')`。
所以，`x' = x * sx` 和 `y' = y * sy`。
缩放因子 `sx`，`sy` 分别在 X 和 Y 方向缩放对象。所以，上面的等式可以用矩阵形式表示：
