...
> 原文：[https://www.geeksforgeeks.org/find-the-missing-element-in-an-array-of-integers-represented-in-binary-format/](https://www.geeksforgeeks.org/find-the-missing-element-in-an-array-of-integers-represented-in-binary-format/)
...
*   在给定的 `N` 个整数中，可以观察到数字的最低有效位（`LSB`）中 `1` 和 `0` 的不平衡。因为缺少一个数字，所以 `LSB` 中缺少 `0` 或 `1`。如果丢失的数字的最低有效位=`0`，则计数(`1`)将大于等于计数(`0`)。如果缺失数字的最低有效位为 `1`，则计数(`1`)小于计数(`0`)。
