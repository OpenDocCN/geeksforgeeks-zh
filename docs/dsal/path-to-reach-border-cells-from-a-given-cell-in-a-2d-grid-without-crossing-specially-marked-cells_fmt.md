...
> 原文：[https://www.geeksforgeeks.org/path-to-reach-border-cells-from-a-given-cell-in-a-2d-grid-without-crossing-specially-marked-cells/](https://www.geeksforgeeks.org/path-to-reach-border-cells-from-a-given-cell-in-a-2d-grid-without-crossing-specially-marked-cells/)

给定尺寸为 `N*M` 的[矩阵](https://www.geeksforgeeks.org/matrix/)，该矩阵由字符 `'M'` 和只有一个实例的 `'A'` 组成。任务是根据以下规则打印从值为 `'A'` 的单元格到矩阵任何边界单元格的任意一条路径：

*   每秒钟，从单元 `'A'` 开始的路径可以在具有字符 `'.'` 的所有四个相邻单元中移动。字符 `L`、`R`、`U`、`D` 分别代表从单元格 `(X, Y)` 移动到方向 `(X-1, Y)`、`(X+1, Y)`、`(X, Y-1)` 和 `(X, Y+1)`。
*   具有字符 `'#'` 和 `'M'` 的单元格是块单元格。
...
给定的问题可以这样解决：首先在网格上为所有的块单元 `'M'` 模拟[多源 BFS](https://www.geeksforgeeks.org/multi-source-shortest-path-in-unweighted-graph/)，然后从单元 `'A'` 执行 BFS，检查是否可以到达任何边界单元。按照以下步骤解决问题：

1.  初始化一个矩阵，比如说 `G[][]`，它存储达到具有值的单元的最小值来自所有具有值 `'M'` 的细胞。
2.  [对所有具有值 `'M'` 的单元格](https://www.geeksforgeeks.org/multi-source-shortest-path-in-unweighted-graph/)执行多源 BFS，以找到从其最近的具有 `'M'` 的单元格到达每个单元格的最短时间，并将其存储在矩阵 `G[][]` 中。
3.  初始化一个矩阵，比如说 `parent[][]` 存储每个单元格的父，说 `(X, Y)` 当对单元格进行任何移动时 `(X, Y)`。
4.  从 `'A'` 出现的位置开始在网格上执行 [BFS 遍历](https://www.geeksforgeeks.org/breadth-first-traversal-bfs-on-a-2d-array/)，比如说 `(SX, SY)`，使用以下步骤：
...
5.  如果没有到达矩阵的边界，则打印 `-1`。否则，使用以下步骤打印路径：
...
// iterate to all the positions from the given position i.e. (x+1, y), (x-1, y), (x, y+1), (x, y-1)
for (auto i : { 0, 1, 2, 3 }) {
...
// in the end if there are some places on grid that were blocked and BFS couldn't reach there
// then just manually iterate over them and change them to largest +1 i.e. treat them as '#'
for (int i = 1; i <= n; i++) {
for (int j = 1; j <= m; j++) {
    if (g[i][j] == -1) {
        g[i][j] = largest;
...
// A's BFS. It will return the time when A reaches boundary
...
parent.resize(n + 1, vector<pair<pair<int, int>, int> >(m + 1));

for (int i = 1; i <= n; i++) {
for (int j = 1; j <= m; j++) {

    // initialize that no one is currently the parent of (i, j)
    ...
    char x = Mat[i - 1][j - 1];
    if (x == 'M') {
        // if the input character is 'M', push the coordinates in M and in the grid take 0 as input
        M.push_back({ i, j });
        g[i][j] = 0;
    }
...
// if (end x) index is -1 and boundary has not been
if (ex == -1) {
    cout << ex;
}
else {
