...
> 原文: [https://www.geeksforgeeks.org/check-if-given-string-can-be-split-only-into-subsequences-abc/](https://www.geeksforgeeks.org/check-if-given-string-can-be-split-only-into-subsequences-abc/)

给定一个长度为 `N` 的[字符串](https://www.geeksforgeeks.org/c-string-class-and-its-applications/) `S`，其中字符串的每个字符或者是 `'A'`、`'B'` 或者是 `'C'`。任务是找出是否有可能将字符串拆分为子序列 `"ABC"`。如果可以拆分，则打印 `"Yes"`。否则，打印 `"No"`。
...
*   Similarly, every `b` must have at least one `a` on the left and one `c` on the right.

按照以下步骤解决问题:
1.  初始化 3 个整数的[deque](https://www.geeksforgeeks.org/deque-cpp-stl/)表示 `A`、`B` 和 `C`，分别存储字符 `'A'`、`'B'` 和 `'C'` 的索引。
2.  [遍历字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-c/)`S` 的字符，在每次迭代中，如果当前字符是 `'A'`，则将索引 `i` 推入 `A`。否则，如果当前字符是 `'B'`，则将索引 `i` 推入 `B`。否则，将索引 `i` 推入 `C`。
3.  如果 `N` 不是 `3` 的倍数，并且字符数 `'A'`、`'B'` 和 `'C'` 不相等，则打印 `"No"`。
4.  否则，[使用变量 `i` 迭代deque](https://www.geeksforgeeks.org/deque-cpp-stl/) `B`，在每次迭代中，如果 `B[i]` 大于deque `A` 的[前元素](https://www.geeksforgeeks.org/dequefront-dequeback-c-stl/)，则[弹出deque](https://www.geeksforgeeks.org/dequepop_front-dequepop_back-c-stl/) `A` 的前元素。否则打印 `"No"` 返回。
5.  现在再次[使用变量 `i` 反过来迭代deque](https://www.geeksforgeeks.org/deque-cpp-stl/) `B`，如果 `B[i]` 小于deque `C` 的[最后一个元素](https://www.geeksforgeeks.org/dequefront-dequeback-c-stl/)，则[弹出deque](https://www.geeksforgeeks.org/dequepop_front-dequepop_back-c-stl/) `C` 的最后一个元素。否则打印 `"No"` 返回。
6.  最后，如果以上情况都不满足，则打印 `"Yes"` 作为答案。
