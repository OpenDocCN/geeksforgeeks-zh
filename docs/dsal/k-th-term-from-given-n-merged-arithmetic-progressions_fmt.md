...
> 原文: [https://www.geeksforgeeks.org/k-th-term-from-given-n-merged-arithmetic-progressions/](https://www.geeksforgeeks.org/k-th-term-from-given-n-merged-arithmetic-progressions/)

给定一个整数 `K` 和一个包含 `N` 个整数的数组 `arr[]`，每个整数都是一个算术级数的第一项和公差，任务是找到集合 `S` 的第 `K` 个元素，集合 `S` 由 `N` 个算术级数合并而成。
...
*   检查是否可以通过合并 `N` 个系列获得 `K` 个元素。如果条款数量超过 `K`，则减少 `R` 至 `mid`。否则，将 `L` 更新为 `mid`。现在，继续前进，直到我们找到一个 `mid`，对于这个 `mid` 的 `K` 项可以在系列中精确地获得。
