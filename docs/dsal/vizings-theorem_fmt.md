...
在图论中，[Vizing 定理](https://en.wikipedia.org/wiki/Vizing%27s_theorem)指出，每个简单的无向图都可以使用最多比图的最大度数`d`大一个的颜色数进行边着色。简单来说，这个定理说明简单图的色指数可以是`d`或`d+1`。图的边着色所需的最小颜色数称为色指数。
