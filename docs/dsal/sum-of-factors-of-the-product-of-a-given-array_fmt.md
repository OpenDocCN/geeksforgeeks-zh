...
> 原文: [https://www.geeksforgeeks.org/sum-of-factors-of-the-product-of-a-given-array/](https://www.geeksforgeeks.org/sum-of-factors-of-the-product-of-a-given-array/)
...
>
> 那么 `P` 的因子之和 = ![\frac{2^{a+1}-1}{2-1} \times \frac{3^{b+1}-1}{3-1} \times \frac{5^{c+1}-1}{5-1}\times ... \times \frac{p^{k+1}-1}{p-1}](img/719df684cf9d04b5ae00906260448baf.png "Rendered by QuickLaTeX.com")

按照以下步骤解决问题:

1.  初始化一个整数，比如 `sum`，来存储数组乘积的所有因子的和。
2.  初始化一个整数数组，比如说 `count[]`，其中 `count[i]` 以数组元素乘积的形式存储质因数 `i` 的频率。
3.  [遍历数组](https://www.geeksforgeeks.org/iterating-arrays-java/) `count[]`，检查 `count[i]` 是否大于 `零`。如果发现为真，则将 `sum` 乘以 `(i^(count[i]+1))–1` 和 `(i - 1)` 的乘法倒数。
4.  最后，打印在 `sum` 中获得的结果。
