...
> 原文：[https://www.geeksforgeeks.org/find-xor-sum-of-bitwise-and-of-all-pairs-from-given-two-arrays/](https://www.geeksforgeeks.org/find-xor-sum-of-bitwise-and-of-all-pairs-from-given-two-arrays/)
...
> **输出:** `0`
> **解释:** 答案是 `(3&2)^(3&3)^(5&2)^(5&3)=1^3^0^2=0`。
>
...
天真方法是使用[嵌套循环](https://www.geeksforgeeks.org/nested-loops-in-c-with-examples/)计算所有对的[位与](https://www.geeksforgeeks.org/bitwise-operators-in-c-cpp/)，然后找到它们的[异或](https://www.geeksforgeeks.org/bitwise-operators-in-c-cpp/)和。按照以下步骤解决问题：

1.  将变量 `ans` 初始化为 `-1`，这将存储最终答案。
2.  遍历数组 `A`，并执行以下操作：
    1.  对于每个当前元素，遍历数组 `B`，并执行以下操作：
        1.  如果 `ans` 等于 `-1`，则将 `A[i] & B[j]` 的[位](https://www.geeksforgeeks.org/bitwise-operators-in-c-cpp/)存储起来。
        2.  否则，将 `ans` 的[位异或](https://www.geeksforgeeks.org/bitwise-operators-in-c-cpp/)和 `A[i] & B[j]` 的[位与](https://www.geeksforgeeks.org/bitwise-operators-in-c-cpp/)存储起来。
3.  返回 `ans`。
