...
给定一个从 `0` 到 `N` 的数值为 `N` 的数组 `arr[]`，任务是计算索引数组中的组件数量。

> **索引数组**表示如果我们在索引 `i`，那么它会导致 `arr[i]`。
...
> **解释：**
> 下面是 `2` 个组件的遍历：
> 组件 1：从 `0` 开始遍历，然后遍历的路径由 `1 -> 2 (arr[1]) -> 3 (arr[2]) -> 5 (arr[3])` 给出。
...
> **解释：**
> 下面是 `2` 个组件的遍历：
> 组件 1：从 `0` 开始遍历，然后遍历的路径由下式给出：`1 -> 2 (arr[1]) -> 0 (arr[2]) -> 1 (arr[0])`
> 组件 2：只有 `3` 是不变的。
> 所以，总成分 = `2`。

## 方法

思路是使用 [DFS 遍历](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/)的概念。以下是步骤：
