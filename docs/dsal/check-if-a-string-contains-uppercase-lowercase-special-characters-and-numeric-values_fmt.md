...
> 原文：[https://www.geeksforgeeks.org/check-if-a-string-contains-uppercase-lowercase-special-characters-and-numeric-values/](https://www.geeksforgeeks.org/check-if-a-string-contains-uppercase-lowercase-special-characters-and-numeric-values/)

给定长度为`N`的字符串`字符串`，任务是检查给定的字符串是否包含大写字母、小写字母、特殊字符和数值。如果字符串包含全部，则打印`“是”`。否则，打印`“否”`。
...
最简单的方法是迭代字符串，检查给定的字符串是否包含大写、小写、数字和特殊字符。以下是步骤：
...
思路是以一个正则表达式的概念来解决这个问题。以下是步骤：

*   创建一个正则表达式来检查给定的字符串是否包含大写、小写、特殊字符和数值，如下所述：
...
*   使用 `Pattern.matcher()` 将给定字符串与正则表达式匹配。
