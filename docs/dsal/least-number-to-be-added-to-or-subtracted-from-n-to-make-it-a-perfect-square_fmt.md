...
> 原文: [https://www.geeksforgeeks.org/least-number-to-be-added-to-or-subtracted-from-n-to-make-it-a-perfect-square/](https://www.geeksforgeeks.org/least-number-to-be-added-to-or-subtracted-from-n-to-make-it-a-perfect-square/)
...
3.  将双精度值转换为整数后，将包含 `N` 以上完美平方的根，即 `floor(sqrt(N))`。
4.  然后求这个数的平方，这将是 `n` 之前的完美平方。
5.  求 `N` 后完美平方的根，即 `ceil(sqrt(N))`。
6.  然后求这个数的平方，这将是 `n` 之后的完美平方。
7.  检查 `floor` 值的平方是最接近 `N` 还是 `ceil` 值。
8.  如果 `floor` 值的平方最接近 `N`，用`-`号打印差值。否则，用`+`号打印 `ceil` 值的平方和 `N` 之间的差值。
