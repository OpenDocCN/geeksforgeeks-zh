...
> 原文: [https://www.geeksforgeeks.org/program-to-calculate-area-of-a-parallelogram/](https://www.geeksforgeeks.org/program-to-calculate-area-of-a-parallelogram/)

给定表示平行四边形边长的整数 `A` 和 `B` 以及角度 `gamma`（即平行四边形的边与对角线 `D1` 和 `D2` 之间的夹角）以及对角线交点处的夹角 `theta`，任务是从提供的信息中找出平行四边形的面积。

> **平行四边形**是一种对边相等且平行的四边形，它们之间的角度不是直角。

## 示例

> **输入:** `A = 6`，`B = 10`，`theta = 30`
> **输出:** 18.48
> **说明:**
...
平行四边形的面积可以通过以下三个公式计算:

*   从给定的边 `A` 和 `B` 以及对角线之间的角度，平行四边形的面积可以通过以下公式计算:

> **边的平行四边形面积**和**对角线之间的角度** = `((A^2 – B^2) * tan(theta)) / 2`

![](img/6a89b6d6e1fc2b3ea0e3f4267560e395.png)

*   从给定的边 `A` 和 `B` 以及边之间的角度，平行四边形的面积可以通过以下公式计算:

> **边的平行四边形面积**和**边之间的角度** = `A * B * sin(gamma)`

![](img/29e3255745d4905012982cb05fb38640.png)

*   根据给定的对角线长度 `D1` 和 `D2` 以及它们之间的角度，平行四边形的面积可以通过以下公式计算:

> **对角线**和**对角线之间的角度**的平行四边形面积 = `(D1 * D2 * sin(theta)) / 2`
