...
> 原文：[https://www.geeksforgeeks.org/sort-elements-by-modulo-with-k/](https://www.geeksforgeeks.org/sort-elements-by-modulo-with-k/)
...
> **输出:** `12 2 6 10 3`
> `{12, 2, 6, 10, 3}`是所需的排序顺序，因为 `K = 4` 的这些元素的模是 `{0, 2, 2, 2, 3}`。
...
*   创建 `K` 个空[向量](https://www.geeksforgeeks.org/vector-in-cpp-stl/)。
*   从左到右遍历数组并更新向量，使得第 `i` 个向量包含当除以 `K` 时给出 `i` 作为余数的元素。
*   [单独排序](https://www.geeksforgeeks.org/merge-sort/)所有向量，因为所有与 `K` 取模值相同的元素必须按升序排序。
*   现在，从第一个向量开始到最后一个向量，在向量中从左到右将元素放入数组，即可给出所需排序顺序。
