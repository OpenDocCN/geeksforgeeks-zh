...
> 原文: [https://www.geeksforgeeks.org/activity-selection-problem-greedy-algo-1/](https://www.geeksforgeeks.org/activity-selection-problem-greedy-algo-1/)
...
**4)** [**霍夫曼编码**](https://www.geeksforgeeks.org/greedy-algorithms-set-3-huffman-coding/)：霍夫曼编码是一种无损压缩技术。它将可变长度的位代码分配给不同的字符。贪婪选择是给最频繁的字符分配最少的位长代码。
...
如果输入活动可能没有排序，需要 `O(n log n)` 时间。当给定输入活动总是被排序时，需要花费 `O(n)` 个时间。
