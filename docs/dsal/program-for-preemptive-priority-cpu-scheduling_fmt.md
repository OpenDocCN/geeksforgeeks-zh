...
> 原文: [https://www.geeksforgeeks.org/program-for-preemptive-priority-cpu-scheduling/](https://www.geeksforgeeks.org/program-for-preemptive-priority-cpu-scheduling/)
...
`void scheduling(进程堆[], 进程数组[], int n, int *heapsize, int * current time)` – 该函数负责执行从堆[]中提取的最高优先级。
