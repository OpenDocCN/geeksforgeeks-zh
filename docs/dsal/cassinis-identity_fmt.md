...
给定一个数字 `N`，任务是评估下面的表达式。预期时间复杂度为 `O(1)`。

```
f(n-1)*f(n+1) - f(n)*f(n)
```

其中 `f(n)` 是第 `n` 个[斐波那契数](https://www.geeksforgeeks.org/program-for-nth-fibonacci-number/)，`n >= 1`。前几个斐波那契数是 `0，1，1，2，3，5，8，13，…..`（将 `0` 视为第 `0` 个斐波那契数）。
...
**参考：**
[https://en.wikipedia.org/wiki/Cassini_and_Catalan_identities](https://en.wikipedia.org/wiki/Cassini_and_Catalan_identities)
