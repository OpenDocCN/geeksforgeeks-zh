...
最简单的方法是[生成长度为`K`的所有可能的子数组](https://www.geeksforgeeks.org/generating-subarrays-using-recursion/)，并遍历每个子数组并计算不同的[质因数](http://www.geeksforgeeks.org/print-all-prime-factors-of-a-given-number/) 它的元素。 最后，打印为任何子数组获得的独特质数的最大数量。
...
想法是使用[滑动窗口技术](https://www.geeksforgeeks.org/window-sliding-technique/)解决此问题。 请按照以下步骤操作：
...
def maximumDPF(arr, n, k):
    ...
    for i in range(0, k):
        ...
        num = arr[i]
        while num != 1:
            maps[spf[num]] = maps.get(spf[num], 0) + 1
            num = int(num / spf[num])
    ...
    for i in range(k, n):
        ...
        num = arr[i]
        while num != 1:
            maps[spf[num]] = int(maps.get(spf[num], 0)) + 1
            num = int(num / spf[num])
    ...
    public static int maximumDPF(int[] arr, int n, int k)
    {
        ...
        for (int i = 0; i < k; i++) {
            ...
            while (num != 1) {
                ...
                // prime factors
                maps.TryGetValue(spf[num], out currentCount);
