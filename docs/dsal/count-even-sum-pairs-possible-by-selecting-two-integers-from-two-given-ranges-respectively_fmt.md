...
> 原文: [https://www.geeksforgeeks.org/count-even-sum-pairs-possible-by-selecting-two-integers-from-two-given-ranges-respectively/](https://www.geeksforgeeks.org/count-even-sum-pairs-possible-by-selecting-two-integers-from-two-given-ranges-respectively/)
...
*   初始化一个变量，比如 `cntYEvenNums`，通过将 `Y` 除以 `2` 来存储 `1` 到 `Y` 之间的偶数计数。
*   初始化一个变量，比如 `cntYOddNums`，通过将 `(Y + 1)` 除以 `2` 来存储 `1` 到 `Y` 之间的奇数计数。
*   初始化一个变量，比如说 `cntPairs`，通过将 `cntXEvenNums` 乘以 `cntYEvenNums` 并将 `cntXOddNums` 乘以 `cntYOddNums` 来存储偶和对的计数，并求两者的和。
