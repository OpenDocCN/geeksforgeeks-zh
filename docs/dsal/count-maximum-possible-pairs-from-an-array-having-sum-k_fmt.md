...
> 原文: [https://www.geeksforgeeks.org/count-maximum-possible-pairs-from-an-array-having-sum-k/](https://www.geeksforgeeks.org/count-maximum-possible-pairs-from-an-array-having-sum-k/)

给定一个由 `N` 个整数和一个整数 `K` 组成的数组 `arr[]`，任务是从给定的数组中找到最大数量的具有总和 `K` 的对。
...
思路是使用[两点法](https://www.geeksforgeeks.org/two-pointers-technique/)。按照以下步骤解决问题：

*   将变量 `ans` 初始化为 `0`，以存储最大对数和 `K`。
...
要优化上述方法，思路是使用[哈希](https://www.geeksforgeeks.org/hashing-data-structure/)。按照以下步骤解决问题：

*   初始化一个变量，比如说 `ans`，用总和 `K` 存储最大的对数。
