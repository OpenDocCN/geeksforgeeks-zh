...
> 原文: [https://www.geeksforgeeks.org/compare-two-floating-point-numbers-given-in-scientific-notations/](https://www.geeksforgeeks.org/compare-two-floating-point-numbers-given-in-scientific-notations/)

给定两根弦 `N` 和 `M` 以 `a * 10^b` 的形式。任务是比较给定的两个浮点数，并打印出**较小的数字**，如果两个数字相等，则打印相等。
...
**天真方法:** 我们将计算从字符串 `N` 和 `M` 中提取的数字的值，然后比较哪一个更好。[`BigInteger`](https://www.geeksforgeeks.org/biginteger-class-in-java/) 类将用于存储和计算 [Java](https://www.geeksforgeeks.org/java/) 中的 `N` 和 `M` 的值。对于较大的测试用例，这种方法会产生超过时间限制的错误。

**最佳方法:**

*   从字符串 `N` 和 `M` 中提取尾数和指数
*   找到 `*` 的索引 (假设 `mulInd`)，那么 `mulInd` 之前的子串就是尾数 (`a1`)。
