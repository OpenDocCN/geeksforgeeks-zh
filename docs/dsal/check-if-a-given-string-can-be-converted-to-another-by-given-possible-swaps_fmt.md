...
> 原文: [https://www.geeksforgeeks.org/check-if-a-given-string-can-be-converted-to-another-by-given-possible-swaps/](https://www.geeksforgeeks.org/check-if-a-given-string-can-be-converted-to-another-by-given-possible-swaps/)
...
> 由于 `str1` 和 `str2` 相等，因此，所需的输出为 True。

> **输入:** `str1 = "xyyzzvl"`, `str2 = "yllzzvac"`
> **输出:** False
...
*   初始化两个[数组](https://www.geeksforgeeks.org/arrays-in-c-cpp/)，比如 `hash1[256]` 和 `hash2[256]` 分别存储 `str1` 和 `str2` 每个不同元素的频率。
*   初始化两个[集合](https://www.geeksforgeeks.org/set-in-cpp-stl/)，比如 `st1` 和 `st2` 来存储 `str1` 和 `str2` 的不同字符。
*   遍历字符串并存储 `str1` 和 `str2` 每个不同字符的频率。
*   [排序](https://www.geeksforgeeks.org/sort-c-stl/) `hash1[]` 和 `hash2[]` 数组。
*   如果 `st1` 和 `st2` 不相等，则打印 False。
*   使用变量 `i` 遍历 `hash1[]` 和 `hash2[]` 数组，检查 `hash1[i] != hash2[i]` 的值是否为真。如果发现是真的，则打印 False。
*   否则，打印 True。
