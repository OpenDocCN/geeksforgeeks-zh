...
> 原文: [https://www.geeksforgeeks.org/largest-substring-with-same-characters/](https://www.geeksforgeeks.org/largest-substring-with-same-characters/)

给定一个大小为 `N` 的字符串 `s`。任务是找到由相同字符组成的最大子串。
...
**方法:**
从左到右遍历字符串。取两个变量 `ans` 和 `temp`。如果当前元素与前一个元素相同，则增加 `temp`。如果当前元素不等于前一个元素，则使 `temp` 为 `1` 并更新 `ans`。
