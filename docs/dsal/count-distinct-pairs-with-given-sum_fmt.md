...
> 原文: [https://www.geeksforgeeks.org/count-distinct-pairs-with-given-sum/](https://www.geeksforgeeks.org/count-distinct-pairs-with-given-sum/)

## 问题描述

给定一个大小为 `N` 的数组 `arr[]` 和一个整数 `K`，任务是找出数组中不同对的计数，其和等于 `K`。
...
**输出:** 2
**解释:**
和 `K = 10` 不同的对是 `{ (arr[0], arr[4]), (arr[2], arr[5]) }`。
因此，要求的输出为 2。

## 天真方法

解决这个问题最简单的方法就是使用双指针技术。想法是排序数组并从给定数组中移除所有连续的重复元素。最后，统计给定数组中和等于 `K` 的对。按照以下步骤解决问题:

*   初始化一个变量，比如说 `cntPairs`，用和 `K` 存储数组中不同对的计数。
*   按递增顺序排列数组。
*   初始化两个变量，比如说 `i = 0`，`j = N - 1` 作为左右指针遍历数组的索引。
*   遍历数组并检查以下条件:
    *   如果 `arr[i] + arr[j] == K`: 删除连续的重复数组元素，并将 `cntPairs` 增加 1。更新 `i = i + 1` 和 `j = j - 1`。
