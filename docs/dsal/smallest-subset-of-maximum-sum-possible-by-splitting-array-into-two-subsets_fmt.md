...
> 原文: [https://www.geeksforgeeks.org/smallest-subset-of-maximum-sum-possible-by-splitting-array-into-two-subsets/](https://www.geeksforgeeks.org/smallest-subset-of-maximum-sum-possible-by-splitting-array-into-two-subsets/)
...
给定的问题可以通过[哈希](https://www.geeksforgeeks.org/hashing-data-structure/)和[排序](https://www.geeksforgeeks.org/sorting-algorithms/)来解决。
按照以下步骤解决问题:
...
*   初始化两个变量，比如 `S` 和 `flag`，分别存储第一个子集的和以及是否存在答案。
*   [按升序排列数组 `arr[]`](https://www.geeksforgeeks.org/c-program-to-sort-an-array-in-ascending-order/)。
*   初始化一个[数组列表](https://www.geeksforgeeks.org/arraylist-in-java/)，比如 `ans`，来存储结果子集的元素。
*   [按照](https://www.geeksforgeeks.org/iterating-arrays-java/)[的相反顺序](https://www.geeksforgeeks.org/collections-reverseorder-java-examples/)遍历数组 `arr[]`，并执行以下步骤:
    *   将当前字符的频率存储在一个变量中，比如 `F`。
    *   如果 `(F + ans.size())` 小于 `(N - (F + ans.size()))`，则在数组列表 `ans` 中追加元素 `arr[i]` 的次数。
    *   将 `i` 的值减 `F`。
    *   如果 `S` 的值大于[数组元素之和](https://www.geeksforgeeks.org/program-find-sum-elements-given-array/)，则将 `flag` 标记为 `真`，然后[破](https://www.geeksforgeeks.org/break-statement-in-java/)。
*   完成上述步骤后，如果 `flag` 的值为 `真`，则打印数组列表 `ans` 作为结果子集。否则，打印 `-1`。
