...
> 原文: [https://www.geeksforgeeks.org/find-the-last-positive-element-remaining-after-repeated-subtractions-of-smallest-positive-element-from-all-array-elements/](https://www.geeksforgeeks.org/find-the-last-positive-element-remaining-after-repeated-subtractions-of-smallest-positive-element-from-all-array-elements/)

给定一个由 `N` 个正整数组成的数组 `arr[]`，任务是从所有数组元素中反复减去最小的正数组元素后，找出剩余的最后一个正数组元素。
...
从数组中减去最小正元素，即 `1`，新数组为 `arr[] = {0, 0, 0, 2}`
...
解决给定问题最简单的方法是遍历给定数组 `arr[]` 找到数组中最小的正元素，从所有元素中减去。执行此操作，直到数组中只剩下一个正元素。完成上述步骤后，打印剩余的正数组元素。
...
通过观察最后一个正元素将是最大阵元和第二大阵元的差值，可以优化上述方法。按照以下步骤解决问题:
