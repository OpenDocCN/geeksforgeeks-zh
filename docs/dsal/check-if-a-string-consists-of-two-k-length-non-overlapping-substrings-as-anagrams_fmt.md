...
> 原文: [https://www.geeksforgeeks.org/check-if-a-string-consists-of-two-k-length-non-overlapping-substrings-as-anagrams/](https://www.geeksforgeeks.org/check-if-a-string-consists-of-two-k-length-non-overlapping-substrings-as-anagrams/)
...
解决这个问题的思路是[遍历给定的字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-python/)，使用[集合](https://www.geeksforgeeks.org/set-in-cpp-stl/)存储长度为 `K` 的子字符串，搜索给定字符串中存在的两个不重叠的子字符串。请遵循以下步骤:

*   初始化一个[无序集](https://www.geeksforgeeks.org/unorderd_set-stl-uses/) `集合`来存储`长度为 K`的子串。
...
*   如果在`字符串`中存在以索引 `(i–1)` 结束的长度为 `K` 的字符串，则 [将排序后的长度为 `K` 的字符串插入`集合`](https://www.geeksforgeeks.org/set-insert-function-in-c-stl/)。
