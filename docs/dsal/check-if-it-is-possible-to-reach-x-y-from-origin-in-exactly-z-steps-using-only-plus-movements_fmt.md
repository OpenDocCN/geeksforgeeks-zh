...
> 原文: [https://www.geeksforgeeks.org/check-if-it-is-possible-to-reach-x-y-from-origin-in-exactly-z-steps-using-only-plus-movements/](https://www.geeksforgeeks.org/check-if-it-is-possible-to-reach-x-y-from-origin-in-exactly-z-steps-using-only-plus-movements/)

给定一个点 `(x, y)`，任务是检查从原点到 `(x, y)` 是否有可能准确到达 `Z` 步。从给定点 `(x, y)` 开始，我们只能向四个方向移动：左 `(x-1, y)`、右 `(x+1, y)`、上 `(x, y+1)` 和下 `(x, y-1)`。
