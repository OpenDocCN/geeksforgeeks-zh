...
> 原文: [https://www.geeksforgeeks.org/writing-power-function-for-large-numbers/](https://www.geeksforgeeks.org/writing-power-function-for-large-numbers/)

我们给出了两个数字 `x` 和 `n`，它们分别是基数和指数。写一个计算 `x^n` 的函数，其中 `1 <= x, n <= 10000` 且可能发生溢出。
...
1.  将进位初始化为 0。
2.  对 `i=0` 到 `res_size-1` 执行以下操作
    *   a. 找到 `prod = res[i] * x + carry`。
    *   b. 将 `prod` 的最后一位数字存储在 `res[i]` 中，其余数字存储在 `carry` 中。
