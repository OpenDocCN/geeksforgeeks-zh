...
> 原文：[https://www.geeksforgeeks.org/smallest-number-never-becomes-negative-processed-array-elements/](https://www.geeksforgeeks.org/smallest-number-never-becomes-negative-processed-array-elements/)
...
简单的方法是找到数组中最大的元素，并测试从 1 开始到最大元素的每个数字，它是否以 0 值穿过整个数组。
...
解决这个问题的有效方法是利用这样一个事实，当你到达最后一个数组元素时，我们开始的值至少可以是 0，这意味着假设最后一个数组元素是 `a[n-1]`，那么 `a[n-2]` 处的值必须大于或等于 `a[n-1]/2`。
