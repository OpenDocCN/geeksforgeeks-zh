...
> 原文：[https://www.geeksforgeeks.org/understanding-threads-on-producer-consumer-problem-java/](https://www.geeksforgeeks.org/understanding-threads-on-producer-consumer-problem-java/)
...
1.  通过为 `Thread` 类创建一个对象。
2.  通过使用 `Runnable` 接口。

### 通过扩展 Thread 类来创建线程

我们创建了一个扩展 `java.lang.Thread` 类的类。此类覆盖 `Thread` 类中可用的 `run()` 方法。一个线程在 `run()` 方法中开始它的生命。我们创建一个 `Thread` 类的对象，调用 `start()` 方法开始执行一个线程。`start()` 调用线程对象上的 `run()` 方法。让我们来看一个使用线程寻找阶乘的例子：
