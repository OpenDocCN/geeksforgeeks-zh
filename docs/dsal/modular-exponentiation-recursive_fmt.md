...
> 原文: [https://www.geeksforgeeks.org/modular-exponentiation-recursive/](https://www.geeksforgeeks.org/modular-exponentiation-recursive/)

给定三个数字 `a`、`b` 和 `c`，我们需要找到 `(a^b) % c`。
现在为什么求幂后做“% c”，因为 `a^b` 即使对于相对较小的 `a`、`b` 值也会非常大，这是一个问题，因为我们试图编码问题的语言的数据类型很可能不会让我们存储这么大的数字。
