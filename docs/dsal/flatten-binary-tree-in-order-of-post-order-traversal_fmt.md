...
> 原文：[https://www.geeksforgeeks.org/flatten-binary-tree-in-order-of-post-order-traversal/](https://www.geeksforgeeks.org/flatten-binary-tree-in-order-of-post-order-traversal/)
...
一个简单的方法是从二叉树的后序遍历中重建二叉树。这将占用 `O(N)` 个额外空间，`N` 是 BST 中的节点数。
...
这将在最坏的情况下将空间复杂度提高到 `O(H)`，因为后序遍历需要 `O(H)` 个额外空间。
