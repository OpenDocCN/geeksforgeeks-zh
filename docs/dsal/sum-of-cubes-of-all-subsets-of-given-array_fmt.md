...
> 原文：`https://www.geeksforgeeks.org/sum-of-cubes-of-all-subsets-of-given-array/`
...
一个简单的方法是[找到所有的子集](https://www.geeksforgeeks.org/find-distinct-subsets-given-set/)，然后对该子集中的每个元素进行立方，并将其添加到结果中。这种方法的时间复杂度将是 `O(2^N)`。

## 高效方法

*   可以观察到，原始数组的每个元素在所有子集中出现 `2^(N–1)` 次。
*   因此最终答案中任何元素 `arr[i]` 的贡献都将是 `arr[i] * 2^(N – 1)`。
*   所以，所有子集的立方之和将是 `[arr[0]^3 + arr[1]^3 + arr[2]^3 + … + arr[N-1]^3] * 2^(N – 1)`。
