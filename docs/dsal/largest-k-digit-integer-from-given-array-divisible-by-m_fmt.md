...
> 原文: [https://www.geeksforgeeks.org/largest-k-digit-integer-from-given-array-divisible-by-m/](https://www.geeksforgeeks.org/largest-k-digit-integer-from-given-array-divisible-by-m/)

给定一个大小为 `N` 的数组 `arr[]` 和 2 个整数 `K` 和 `M`，任务是从数组 `arr[]` 中找到最大的整数 `K` 位，该整数可被 `M` 整除。如果找不到这样的整数，打印 `-1`。

> **注意:** `K` 的值是这样的，找到的整数总是小于或等于 `INT_MAX`。
...
这可以通过找到大小为 `K` 的所有子序列然后检查条件找到最大的一个来完成。
**时间复杂度:** `O(2^N)`
**辅助空间:** `O(1)`

## 高效方法

想法是检查数组中 `M` 的倍数中的每个数字，看是否有可能形成那个数字。如果有可能，那就更新答案。按照以下步骤解决问题:

*   如果 `K` 大于 `N`，则打印 `-1` 返回。
*   用值 `0` 初始化大小为 `10` 的向量 `freq[]`，以存储数组所有元素 `arr[]` 的频率。
*   迭代范围 `[0, N)` 并存储数组中每个元素的频率 `freq[]`。
*   按升序排列数组 `arr[]`。
*   初始化变量 `X` 并将其值设置为数组 `arr[]` 中最大的 `K` 位数。
*   如果 `X` 小于 `M`，则打印 `-1` 返回。
*   将变量 `answer` 初始化为 `-1` 保存答案。
*   使用变量 `i` 迭代范围 `[M, X]`，并执行以下步骤:
    *   将变量 `y` 初始化为 `i`。
    *   用值 `0` 初始化大小为 `10` 的向量 `v[]`，以存储数字 `y` 的所有数字的频率。
    *   遍历向量 `freq[]` 和 `v[]`，如果所有位置 `freq[j]` 都大于等于 `v[j]`，则更新 `answer` 作为 `answer` 或 `y` 的最大值。
*   执行上述步骤后，打印变量 `answer` 作为答案。
