...
> 原文: [https://www.geeksforgeeks.org/cost-required-to-empty-a-given-array-by-repeated-removal-of-maximum-obtained-by-given-operations/](https://www.geeksforgeeks.org/cost-required-to-empty-a-given-array-by-repeated-removal-of-maximum-obtained-by-given-operations/)

## 问题描述

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是在按指定顺序执行下列操作任意多次后，找出移除所有数组元素的代价：
...
第三步: 当前最大值=`3`。移除 `3` 并将每个元素减少 `1` 会将数组修改为 `{1, 1}`。成本=`12 + 3 = 15`。
第四步: 获得的最终成本=`15 + 1 = 16`。
...
*   从给定数组中找到最大值。
*   从数组中移除后，将其添加到成本 `cost` 中。
*   移除最大元素后，将所有剩余的数组元素减少 `1`。移除所有减少到 `0` 的元素。
*   重复以上步骤，直到移除所有数组元素。
*   最后打印 `cost` 的值。

**时间复杂度:** `O(N^2)`，其中 `N` 为给定数组的大小。
**辅助空间:** `O(1)`

## 高效解法

为了优化上述方法，我们的想法是观察数组中每个第 `i` 个最大元素被添加 `arr[i] - i` 次到 `cost` 中，其中 `i` 是每个元素的索引。按照以下步骤解决上述问题:

1.  按降序排列给定的数组。
2.  遍历数组并将每个数组元素的值 `arr[i] - i` 加到 `cost` 上，如果它大于 `0`。
3.  完成以上步骤后，打印 `cost` 的值。
