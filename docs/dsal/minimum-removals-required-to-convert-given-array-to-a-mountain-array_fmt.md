...
> 原文: [https://www.geeksforgeeks.org/minimum-removals-required-to-convert-given-array-to-a-mountain-array/](https://www.geeksforgeeks.org/minimum-removals-required-to-convert-given-array-to-a-mountain-array/)

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是找到使给定数组成为山形数组所需移除的最小数组元素数。
...
**方法:** 思路是用自下而上的动态规划方法解决这个问题。按照以下步骤解决问题:

1.  如果给定数组的长度小于 3，则该数组不能转换为山形数组。
2.  否则，遍历数组，对于每个 `i` 第个元素 `(0 < i < N)`，找到子数组 `{arr[0]，…，arr[i–1]}` 中递增子序列的长度并将其存储在数组中，比如 `leftIncreasing[]`。
3.  同样，求子阵 `{arr[i+1]，…，arr[N-1]}` 中递增子序列的长度对于每一个 `i` 元素 `(0 < i < N)`，并将其存储在数组中，说 `rightIncreasing[]`。
4.  找到满足以下条件的指数 `i (0 < i < N)`:
    1.  第一个强制条件是峰值条件，即 `leftIncreasing[i] > 0` 和 `rightIncreasing[i] > 0`。
    2.  在所有指标中，如果 `leftIncreasing[i] + rightIncreasing[i]` 为最大，则该指标为山阵峰，如 `X`。
5.  返回结果为 `N – (X + 1)`，加 1 使数组索引达到长度。

插图:

> 考虑数组 `arr[] = {4，3，6，4，5}`
> 因此，`leftIncreasing[] = { 0，0，1，1，2 }` & `rightIncreasing[] = { 2，1，1，0，0}`。
> 只有一个索引 `i = 2` (基于 0 的索引)，对于该索引，`leftIncreasing[2] > 0` 和 `rightIncreasing[2] > 0`。
> 因此，`X = leftIncreasing[2] + rightIncreasing[2] = 2`。
> 因此，所需答案 = `N – (X + 1) = 5 – (2 + 1) = 2`。
