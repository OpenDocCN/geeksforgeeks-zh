...
这是一种非参数检验，替代了具有重复测量的单向方差分析。它试图确定受试者是否在不同的场合/条件下发生了显著变化。例如：早上、下午、晚上一组人解决问题的能力相同或不同。当因变量为序数时，用于测试组间差异。当样本量很小时，该测试特别有用。

## 弗里德曼试验的要素

*   在三个或更多区块上测量的一组测量超时/实验条件。
*   一个因变量，可以是序数、区间或比率。
...
![F_{R}=\frac{12}{n k(k+1)} \sum R_{i}^{2}-3 n(k+1)](img/bc1f7707c68fa219d44f1e3e10d8ff6f.png "Rendered by QuickLaTeX.com")
...
1.  **计算值对表值：** 如果 `F_R` 大于临界值，则拒绝零假设。否则，接受零假设。
2.  **P 值法：** 将 P 值与 `α`（显著性水平）进行比较。如果 `p` 值小于或等于 `α`，则拒绝零假设。
...
使用此表找出 `α= 0.05` 和 `DF = 2` 的[临界卡方值](https://people.richland.edu/james/lecture/m170/tbl-chi.html)。
...
![F_{R}=\frac{12}{(7)(3)(4)}\left(9^{2}+13^{2}+20^{2}\right)-3(7)(4)](img/8a773cf23bbc480f7ccab48b2ef1358d.png "Rendered by QuickLaTeX.com")
