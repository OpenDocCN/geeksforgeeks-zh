...
**途径:**本文已经存在[这里](https://www.geeksforgeeks.org/number-of-subsequences-with-even-and-odd-sum/)有`O(N)`的时间复杂度 `N` 就是阵的大小。在继续前进之前，请访问这里的。
...
> `Oseq = 2^(奇数–1)`
> 将以上结果相乘即可计算出 `OddSumSeq = 2^偶数 * 2^(奇数–1)`
> `偶数 sumseq = 2^N–1–Odd sumseq`
