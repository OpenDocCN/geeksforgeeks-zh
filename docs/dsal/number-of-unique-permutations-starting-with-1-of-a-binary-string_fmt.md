...
> 原文: [https://www.geeksforgeeks.org/number-of-unique-permutations-starting-with-1-of-a-binary-string/](https://www.geeksforgeeks.org/number-of-unique-permutations-starting-with-1-of-a-binary-string/)
...
其思想是首先在给定的字符串中找到 1 的计数和 0 的计数。现在让我们考虑一下字符串的长度 `L`，字符串至少由一个 1 组成。设 1 的个数为 `n`，0 的个数为 `m`。在 1 的 `n` 个数中，我们必须在字符串的开头放置一个 1，这样我们就剩下 `n-1` 个 1 和 `m` 个 0 了，我们必须排列字符串的这些 `(n-1)` 个 1 和 `m` 个 0 的长度 `(L-1)`。
因此，排列数将为:
