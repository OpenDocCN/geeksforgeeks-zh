# 给定一个数组和两个整数 l 和 r，找到范围【l，r】中第 k 个最大的元素
...
给定一个由 `n` 个整数和一个整数 `k` 组成的未排序数组 `arr[]` ，任务是在给定的索引范围`【l】【r】`中找到最大的元素 `kth` 示例:
...
**方法:**一个简单的解决方案是对范围内的元素进行排序，得到 `kth` 最大的元素，对于每个查询，该解决方案的时间复杂度为 `nlog(n)` 。我们可以使用前缀数组和二分搜索法来解决 `log(n)` 中的每个查询。我们所要做的就是维护一个 2d 前缀数组，其中第行和第行包含的元素数量小于或等于第 `i` 行，且与给定数组的元素数量在同一范围内。前缀数组完成后，我们需要做的就是对前缀数组进行简单的二分搜索法运算。因此时间复杂度大大降低。
以下是上述方法的实施:
