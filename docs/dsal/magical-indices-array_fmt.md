...
问题是要计算图中显示的所有循环中的节点数。 每个索引代表图的单个节点。 如问题陈述中所述，每个节点都有一个有向边。 此图具有特殊的属性：从任何顶点开始遍历时，始终会检测到一个循环。 此属性将有助于减少解决方案的时间复杂度。

阅读有关如何在有向图中检测周期的信息：[在有向图中检测周期](https://www.geeksforgeeks.org/detect-cycle-in-a-graph/)
