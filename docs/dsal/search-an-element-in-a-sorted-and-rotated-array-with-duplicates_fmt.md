...
> 原文：[https://www.geeksforgeeks.org/search-an-element-in-a-sorted-and-rotated-array-with-duplicates/](https://www.geeksforgeeks.org/search-an-element-in-a-sorted-and-rotated-array-with-duplicates/)
...
> **输入：** `arr[] = {3, 3, 3, 1, 2, 3}`，`key = 3`
> **输出：** 0
> `arr[0] = 3`
>
> **输入：** `arr[] = {3, 3, 3, 1, 2, 3}`，`key = 11`
> **输出：** -1
> 11 不在给定数组中。
...
在这种情况下，保证 `arr[high]` 也等于 `arr[mid]`，所以可以在原逻辑之前检查条件 `arr[mid] == arr[low] == arr[high]`，如果是，则通过 `1` 向中间左右移动并重复。
