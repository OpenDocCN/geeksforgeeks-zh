...
> 原文: [https://www.geeksforgeeks.org/practice-set-recurrence-relations/](https://www.geeksforgeeks.org/practice-set-recurrence-relations/)
...
由主定理第一种情况得出 `T(n) = Ω(n^2.8)` 意味着 `O(n^2.8)` 以及 `O(n^3)`。
因此，选项(a)、(b)和(c)是正确的选项。

## Que-2.
按照渐近(大 O)复杂度的递减顺序对以下函数进行排序:
...
`f4 > f3` 因为我们可以写 `f4(n) = n^10 * (√2)^n = n^10 * (1.414)^n`，清楚显示 `f4 > f3`
`f3 > f1`:
`f1(n)= n^√n` 取两边对数 `log f1 = √n log n`
...
**解释–**
`f(n)= 2^(2n)= 2^n * 2^n`
选项(a)表示 `f(n) <= c * 2^n`，不成立。
选项(c)表示 `c1 * 2^n <= f(n) <= c2 * 2^n`，满足下界但不满足上界。
选项(b)表示 `c * 2^n <= f(n)` 这个条件满足，因此选项(b)是正确的。
...
## Que-5.
`t(n)= 3t(n/2+47)+2n^2+10*n–1/2`。`T(n)`会是
(a) `O(n^2)`
(b) `O(n^(3/2))`
(c) `O(n log n)`
(d) 这些都不是
...
**解释–**
对于更高值的 `n`，`n/2 >> 47`，所以我们可以忽略 `47`，现在 `T(n)` 将是
