...
> 原文: [https://www.geeksforgeeks.org/sum-numbers-formed-root-leaf-paths/](https://www.geeksforgeeks.org/sum-numbers-formed-root-leaf-paths/)
...
这个想法是对树进行一次前序遍历。在前序遍历中，跟踪计算值直到当前节点，让这个值为 `val`。对于每个节点，我们将 `val` 更新为 `val*10` 加上节点的数据。
...
上面的代码是一个简单的前序遍历代码，每隔恰好一次访问。因此，时间复杂度为 `O(n)`，其中 `n` 是给定二叉树中的节点数。
