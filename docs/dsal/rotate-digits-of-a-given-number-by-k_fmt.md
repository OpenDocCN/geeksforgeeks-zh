...
> 原文: [https://www.geeksforgeeks.org/rotate-digits-of-a-given-number-by-k/](https://www.geeksforgeeks.org/rotate-digits-of-a-given-number-by-k/)

给定两个整数 `N` 和 `K`，任务是将 `N` 的位数旋转 `K`。如果 `K` 是正整数，则向左旋转其位数；否则，向右旋转它的数字。
