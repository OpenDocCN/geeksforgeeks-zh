...
回文自拍号满足自拍乘法规则，使得存在另一个编号 `y`，编号 `y` 的 `x * reverse_digits_of(x) = y * reverse_digits_of(y)`，条件是编号 `y` 是通过对 `x` 中的数字进行某种排序得到的，即 `x` 和 `y` 应该具有相同的数字，但顺序不同。
...
```
 LHS = current_number * palindrome(current_number) 
```
...
public class palindrome_selfie {
    ...
    public int swap(int num, int i, int j) {
        ...
        charArray[j] = temp;
        // Convert char array back to int and return
        return Integer.valueOf(new String(charArray));
    }

    // Driver code
    public static void main(String args[]) {
        palindrome_selfie obj = new palindrome_selfie(1224);
        obj.palin_selfie();
    }
}
```
