...
要将矩阵**转置**，我们可以简单地将每一列的值改为行的值，反之亦然，然而，在这种情况下，结果矩阵不会按照我们的要求进行排序。因此，我们最初确定的元素数量少于当前元素要插入的列，以便获得当前元素应该放置的结果矩阵的精确索引。这是通过维护数组`index[]`来实现的，该索引的`i`th值表示矩阵中元素的数量少于列`i`。
...
```csharp
// C# code to perform add, multiply and transpose on sparse matrices
public class sparse_matrix {
    // Maximum number of elements in matrix
    static int MAX = 100;
