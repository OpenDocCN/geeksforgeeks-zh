...
> 原文：[https://www.geeksforgeeks.org/range-queries-for-count-of-armstrong-numbers-in-subarray-using-mos-algorithm/](https://www.geeksforgeeks.org/range-queries-for-count-of-armstrong-numbers-in-subarray-using-mos-algorithm/)

给定一个由表示范围的 `L` 和 `R` 表示的由 `N` 个元素和 `Q` 个查询组成的数组 `arr[]`，任务是打印子数组 `[L, R]` 中的[阿姆斯特朗数](https://www.geeksforgeeks.org/program-for-armstrong-numbers/)的个数。
...
> 查询 2: 子阵列 `[3…5]` 具有 **2** 个阿姆斯特朗编号，即 {9, 5}
...
*   如果当前元素是阿姆斯特朗数，则递增 `count_Armstrong`。

### 移除元素

*   如果当前元素是阿姆斯特朗数，则递减 `count_Armstrong`。
