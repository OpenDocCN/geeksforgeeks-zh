...
在堆栈机中，数据默认位于栈顶。堆栈充当源和目的地，`PUSH`和`POP`指令用于从堆栈中访问指令和数据。不需要传递源地址和目的地址，因为默认地址在堆栈顶部。在堆栈机器中，不需要在指令中传递显式地址。因此[指令格式仅由操作码（opcode）字段组成](https://www.geeksforgeeks.org/introduction-of-single-accumulator-based-cpu-organization/)。这种指令格式被称为零地址指令。
