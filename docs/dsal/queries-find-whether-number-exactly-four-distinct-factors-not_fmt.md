...
> 原文: [https://www.geeksforgeeks.org/queries-find-whether-number-exactly-four-distinct-factors-not/](https://www.geeksforgeeks.org/queries-find-whether-number-exactly-four-distinct-factors-not/)

## 问题描述

给定一个正整数`q`和`n`。对于每个查询`q`，找出一个数`n`是否正好有四个不同的除数。如果数字正好有四个除数，则打印`Yes`或`No`。`1 <= q, n <= 10^6`
...
我们可以使用厄拉多塞的筛选，这样我们将预先计算从`1`到`10^6`的所有质因数。现在我们将通过使用两个`for loops`来标记所有是两个质数的乘积的数字，即`mark[p * q] = true`。同时，我们还将通过取数字的立方来标记所有数字（立方根），即，标记`p * p * p = true`。
