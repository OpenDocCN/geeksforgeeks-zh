...
## 先决条件
[图形的优势集](https://www.geeksforgeeks.org/dominant-set-of-a-graph/) [NP-Complete](https://www.geeksforgeeks.org/np-completeness-set-1/)

> 图中的支配集 `G = (V, E)` 是顶点的子集 `V'`，条件是不属于 `V'` 的顶点与 `V'` 中的某个顶点相邻。

## 问题
给定一个图 `G(V, E)` 和一个整数 `k`，问题是确定该图是否有大小为 `k` 的支配集。
...
为了证明 Dominating Set 是 NP-Hard，我们需要将一个已知的 NP-Hard 问题归约到这个问题。我们将从 Vertex Cover 问题归约到 Dominating Set 问题。
Vertex Cover 问题的每个实例由一个图 `G = (V, E)` 和一个整数 `k` 组成，作为输入的顶点子集可以转换为一个由图 `G’ = (V’, E’)` 组成的 Dominating Set 问题。我们将按以下方式构造图 `G’`:
