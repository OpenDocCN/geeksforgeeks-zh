...
> 原文：[https://www.geeksforgeeks.org/find-single-movement-in-a-matrix/](https://www.geeksforgeeks.org/find-single-movement-in-a-matrix/)

给定四个整数 `x1`，`y1` 和 `x2`，`y2`，它们代表无限 2D 矩阵中的两个位置，任务是找出是否有可能在一次移动中从 `(x1，y1)` 移动到 `(x2，y2)`，移动方向是*向左，向右，向上或向下*。请注意，移动将重复进行，直到到达目的地。如果无法达到 `(x2，y2)` 输出 `-1`。
