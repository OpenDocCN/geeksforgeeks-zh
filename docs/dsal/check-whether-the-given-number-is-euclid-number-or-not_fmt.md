...
> 原文: [https://www.geeksforgeeks.org/check-whether-the-given-number-is-euclid-number-or-not/](https://www.geeksforgeeks.org/check-whether-the-given-number-is-euclid-number-or-not/)

给定一个正整数 `n`，任务是检查它是否是欧几里德数。如果给定的数字是欧几里德数，则打印“是”，否则打印“否”。

`欧几里德数:` 在数学中，欧几里德数是形式为 $E_{n} = p_{n}\# + 1$ 的整数，其中 $p_{n}\#$ 是前 `n` 个素数的乘积。
前几个欧几里德数是-
