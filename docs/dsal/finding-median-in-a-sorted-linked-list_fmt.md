...
3.  如果`count`是偶数，则再次遍历链表并找到：（第`n / 2`个元素`+`第`n / 2 + 1`个元素）`/ 2`

**注意**：上述解决方案两次遍历链表。

## 有效方法

一种有效方法是使用两个指针遍历列表以查找元素数。[参见此帖子的方法 2](https://www.geeksforgeeks.org/write-a-c-function-to-print-the-middle-of-the-linked-list/)。
