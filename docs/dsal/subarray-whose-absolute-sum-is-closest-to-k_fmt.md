...
如果数组仅由`非负整数`组成，则使用[滑动窗口技术](https://www.geeksforgeeks.org/window-sliding-technique/)来提高每次迭代中求和的计算时间。滑动窗口技术通过使用先前的子阵列和计算新的子阵列和来降低复杂性。增加右边的指数，直到差值`(K-sum)`大于零。考虑第一个子阵列为负`(K-sum)`，下一个子阵列为左索引= `i+1`（其中 `i` 为当前右索引）。
