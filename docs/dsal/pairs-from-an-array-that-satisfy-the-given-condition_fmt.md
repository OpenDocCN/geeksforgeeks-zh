...
> 原文: [https://www.geeksforgeeks.org/pairs-from-an-array-that-satisfy-the-given-condition/](https://www.geeksforgeeks.org/pairs-from-an-array-that-satisfy-the-given-condition/)

给定一个数组 `arr[]`，任务是计算数组中所有有效的对。如果 `func(arr[i]) + func(arr[j]) = func(XOR(arr[i], arr[j]))`，其中 `func(x)` 返回 `x` 中设置位的个数，则一对 `(arr[i], arr[j])` 被认为是有效的。
