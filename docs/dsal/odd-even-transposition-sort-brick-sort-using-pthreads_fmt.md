...
> 原文：[https://www.geeksforgeeks.org/odd-even-transposition-sort-brick-sort-using-pthreads/](https://www.geeksforgeeks.org/odd-even-transposition-sort-brick-sort-using-pthreads/)
...
由于使用线程进行并行计算，时间复杂度降低到 `O(N)`。

## 工作复杂度

这个程序的工作复杂度是 `O(N)`，因为 `N/2` 的线程（资源）被用来对数组进行排序。所以，这个项目的工作时间复杂度是 `O(N^2)`。