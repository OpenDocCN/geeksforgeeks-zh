...
> 原文: [https://www.geeksforgeeks.org/number-of-moves-required-to-guess-a-permutation/](https://www.geeksforgeeks.org/number-of-moves-required-to-guess-a-permutation/)
...
**进场：** 要想猜对第 `i` 个位置，需要 `(n-i)` 的猜测。对于每一个猜测，你需要合计 `i` 个移动（`(i-1)` 个移动来输入你已经知道的正确前缀，而 `1` 个移动来猜测当前的前缀）。在最后一步，你需要更多的移动来进入正确的排列。
