...
一个天真的解决方案是先计算 `n!`，然后计算 `n! % p`。当 `n!` 的值很小时，这没有问题。但当 `n` 的值较大时，`n!` 无法放入变量，会导致溢出。所以计算 `n!` 然后使用模运算符并不是一个好主意，因为即使 `n` 和 `p` 的值稍微大一点也会有溢出现象。
...
这个想法是使用[埃拉托斯特尼筛法](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)寻找所有小于 `n` 的素数。对于每一个质数 `p_i`，找到它整除 `n!` 的最大幂次。让最大的幂次成为 `k_i`。使用[模幂运算](https://www.geeksforgeeks.org/modular-exponentiation-power-in-modular-arithmetic/)计算 `p_i^k_i % p`。将其与模 `p` 下的最终结果相乘。
...
注意 `n!` (`n >= p`) 时 `% p` 为 `0`，该方法主要用于 `p` 接近输入数 `n` 时，例如(`25! % 29`)。从威尔逊定理，我们知道 `28!` 是 `-1`。所以我们基本上需要求 `[ (-1) *逆(28，29) *逆(27，29) *逆(26) ] % 29`。反函数 `逆(x，p)` 返回模 `p` 下 `x` 的逆(详见[本](https://www.geeksforgeeks.org/multiplicative-inverse-under-modulo-m/))。
...
请注意，上述算法的时间复杂度步骤 2 取决于所使用的素性测试算法和小于 n 的最大素值。以 [AKS](https://en.wikipedia.org/wiki/AKS_primality_test) 算法为例，取 `O(Log^10.5 n)` 时间。
