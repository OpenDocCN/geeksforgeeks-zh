...
> 原文：[https://www.geeksforgeeks.org/split-array-into-minimum-number-of-subarrays-having-gcd-of-its-first-and-last-element-exceeding-1/](https://www.geeksforgeeks.org/split-array-into-minimum-number-of-subarrays-having-gcd-of-its-first-and-last-element-exceeding-1/)
...
1.  将一个 `right` 指针固定到原始数组的最后一个元素 `arr[]`，并找到原始数组中最左边的元素，使得 `GCD(left, right) > 1`。如果没有找到这样的元素，就没有有效的答案。
2.  如果找到这样的元素，那就意味着我们找到了一个有效的子数组。然后将 `right` 指针改为 `(left - 1)`，再次开始搜索有效的子阵列。
3.  重复上述步骤，直到 `right` 大于 0 为止，并继续增加找到的子阵数量，直到现在。
4.  完成上述所有步骤后，打印计数值。
