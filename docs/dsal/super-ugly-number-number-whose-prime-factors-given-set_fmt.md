...
> 原文：[https://www.geeksforgeeks.org/super-ugly-number-number-whose-prime-factors-given-set/](https://www.geeksforgeeks.org/super-ugly-number-number-whose-prime-factors-given-set/)

超级丑数是正数，其所有质因数都在给定的质数列表中。给定一个数字 `n`，任务是找到第 `n` 个超级丑数。
可以假设给定的一组素数被排序。另外，按照惯例，第一个超级丑数是 `1`。
...
在我们之前的帖子中，我们讨论了[丑数](https://www.geeksforgeeks.org/ugly-numbers/)。这个问题基本上是**丑数**的延伸。
这个问题的一个**简单的解决方法**就是从 `1` 开始一个一个的挑选每个数字，检查其所有质因数，如果所有质因数都在给定的质数集中，那就说明这个数字是超级丑数。重复这个过程，直到我们得到第 `n` 个超级丑数。

这个问题的一个**高效解决方案**类似于[丑数](https://www.geeksforgeeks.org/ugly-numbers/)的**方法-2**。下面是算法：

1.  设 `k` 为给定素数数组的大小。
2.  声明一个集合来存储超级丑数。
3.  将第一个丑数（总是 `1`）插入集合中。
4.  用 `0` 初始化大小为 `k` 的 `multiple_Of[]` 数组。这个数组的每个元素都是 `primes[]` 数组中相应素数的迭代器。
5.  用 `primes[]` 初始化 `nextMultiple[]` 数组。这个数组的行为类似于给定 `primes[]` 数组中每个素数的下一个倍数变量，即：`nextMultiple[i] = primes[i] * ugly[++multiple_of[i]]`。
6.  现在循环，直到集合中有 `n` 个元素。
    a) 在 `nextMultiple[]` 数组中找到当前素数倍数中的最小值，并将其插入丑数集合中。
    b) 然后求出这个当前最小值是哪个素数的倍数。
...
// primes[]       --> given list of primes of size k
...
本文由 [**Shashank Mishra (Gullu)**](https://www.facebook.com/shashank.mishra.92167) 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 [write.geeksforgeeks.org](https://write.geeksforgeeks.org) 写一篇文章或者把你的文章邮寄到 review-team@geeksforgeeks.org。看到你的文章出现在极客博客主页上，帮助其他极客。

## 另一种方法（使用 `priority_queue`）

这里我们使用最小堆 `priority_queue`。
想法是将第一个丑数 `1` 推入 `priority_queue`，并在每次迭代中获取 `priority_queue` 的顶部，并将该顶部的所有倍数推入 `priority_queue`。
假设 `primes[] = {2, 3, 5}`，
那么在第一次迭代中 `1` 是顶部的，所以弹出 `1`，推 `1 * 2`，`1 * 3`，`1 * 5`。
第二次迭代最小值为 `2`，所以弹出，推 `2 * 2`，`2 * 3`，`2 * 5` 等等。
