...
> 原文: [https://www.geeksforgeeks.org/k-th-lexicographical-string-of-given-length/](https://www.geeksforgeeks.org/k-th-lexicographical-string-of-given-length/)

给定两个整数 `N` 和 `K`，任务是找到字典序上长度为 `N` 的第 `K` 个字符串。如果长度为 `N` 的可能字符串数小于 `K`，则打印 `-1`。
