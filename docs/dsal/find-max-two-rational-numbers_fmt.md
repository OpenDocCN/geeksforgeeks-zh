...
> 原文：[https://www.geeksforgeeks.org/find-max-two-rational-numbers/](https://www.geeksforgeeks.org/find-max-two-rational-numbers/)
...
设 `first` = 3/2，`second` = 3/4

*   首先取(4，2)的一个 [LCM](https://www.geeksforgeeks.org/c-program-find-lcm-two-numbers/) ，它是有理数的分母。所以这个的 `LCM` 是 4，然后用分母和倍数分别除以 `first` 和 `second` 分子，所以分母值是 `first` 分子= 6，`second` 分子= 3。
*   然后找出这两者之间的最大值。所以这里 `first` 分子是 `max`，然后打印 `first` 有理数。
