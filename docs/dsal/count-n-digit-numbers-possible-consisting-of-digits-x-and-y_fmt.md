...
> 原文: [https://www.geeksforgeeks.org/count-n-digit-numbers-possible-consisting-of-digits-x-and-y/](https://www.geeksforgeeks.org/count-n-digit-numbers-possible-consisting-of-digits-x-and-y/)
...
*   使用数字 `{ 0–9 }` 可能得到的总数 `N` 位数是 `10^N`。
*   可以使用数字 `{ 0–9 } – { X }` 形成的总计 `N` 位数是 `9^N`。
*   总计 `N` 位数可以使用数字 `{ 0–9 } – { X，Y}` 形成的数字是 `8^N`。
*   总计 `N` 位数包含数字 `X` 和 `Y` 的数字是所有可能的数字与不包含数字 `X` 或 `Y` 的数字之间的差值，后跟包含除 `X` 和 `Y` 之外的所有数字的数字总和。因此，答案是 `10^N – 2 * 9^N + 8^N`。
