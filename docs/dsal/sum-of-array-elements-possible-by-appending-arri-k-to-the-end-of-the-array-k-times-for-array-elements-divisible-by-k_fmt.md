...
> 原文: [https://www.geeksforgeeks.org/sum-of-array-elements-possible-by-appending-arri-k-to-the-end-of-the-array-k-times-for-array-elements-divisible-by-k/](https://www.geeksforgeeks.org/sum-of-array-elements-possible-by-appending-arri-k-to-the-end-of-the-array-k-times-for-array-elements-divisible-by-k/)

给定一个由 `N` 个整数和一个整数 `K` 组成的数组 `arr[]`，如果 `arr[i]` 可被 `K` 整除，则任务是通过遍历数组并在数组末尾添加 `arr[i] / K`、`K` 次来找到数组元素的和。否则，停止遍历。
...
**朴素法:** 解决给定问题最简单的方法是遍历给定数组并在数组末尾多次添加值 `arr[i] / K`、`K` 次。完成上述步骤后，打印数组元素的和。
...
时间复杂度:`O(N * K * log N)`
辅助空间:`O(M)`，M 是[数组的最大元素](https://www.geeksforgeeks.org/c-program-find-largest-element-array/)。
...
*   初始化一个变量，比如说`flag`标记为 `0` ，该变量存储元素是否要添加到数组的末尾。
*   [遍历阵](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)`A[]`并执行以下步骤:
    *   如果`flag`的值为 `0` 、`A[i]`可被 `K` 整除，则在 `V` 末尾推`A[i]`。
    *   否则，将`flag`的值更新为 `1` 。
    *   将`sum`的值增加`v[i % N]`。
*   完成上述步骤后，打印`sum`的值作为合成总和。
