...
想法是从行或列的侧面`x`朝`x'`（与`x`相对）遍历每一行或每一列（取决于给定的方向）。例如，如果给定的`x`值为`'l'`（左），则开始从左侧到右侧扫描每一行。遍历时，通过跳过具有值 0 的元素和连续元素之和（如果它们具有相等的值），将行或列元素存储在临时一维数组中（例如`temp[]`）。之后，开始将临时数组`temp[0..k]`从（行或列的）`x`侧复制到当前行或列的`x'`（与`x`相对），然后用 0 填充元素的重置。
