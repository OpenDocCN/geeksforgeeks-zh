...
> 原文：[https://www.geeksforgeeks.org/print-all-leaf-nodes-of-an-n-ary-tree-using-dfs/](https://www.geeksforgeeks.org/print-all-leaf-nodes-of-an-n-ary-tree-using-dfs/)

给定一个数组`` `edge[][]` ``，其中`` `(edge[i][0], edge[i][1])` ``定义了 n 元树中的一条边，任务是使用 DFS 打印给定树的所有叶节点。
...
[DFS](https://www.geeksforgeeks.org/depth-first-traversal-for-a-graph/) 可以用来遍历完整的树。我们将在遍历时跟踪父节点，以避免访问节点数组。最初，我们可以为每个节点设置一个标志，如果该节点至少有一个子节点（即非叶节点），那么我们将重置该标志。没有子节点的节点是叶节点。
