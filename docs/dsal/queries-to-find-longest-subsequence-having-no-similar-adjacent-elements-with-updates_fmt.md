...
> 原文: [https://www.geeksforgeeks.org/queries-to-find-longest-subsequence-having-no-similar-adjacent-elements-with-updates/](https://www.geeksforgeeks.org/queries-to-find-longest-subsequence-having-no-similar-adjacent-elements-with-updates/)

给定一个由 `N` 个整数组成的数组 `arr[]` 和一个查询数组 `Queries[][]`，每个查询的形式为 `{x，y}`。对于每个查询，任务是将索引 `x` (基于 1 的索引)处的元素替换为 `y`，并找到没有相似相邻元素的最长子序列的长度。

## 示例

**输入:** `arr[] = {1，1，2，5，2}`，`Q = 2`，`Queries[][]= { {1，3}，{4，2}}`
**输出:** `5 3`
**解释:**
最初数组是 `{1，1，2，5，2}`。
...
**输入:** `arr[] = {1，1，2，5，2}`，`Q = 1`，`Queries[][]= { { 2，2}}`
**输出:** `4`
**解释:**
最初数组是 `{1，1，2，5，2}`。
...
3. 通过变量 `count` 跟踪在任意点找到的最长子序列。
4. 遍历数组，每当序列的当前元素不等于序列的前一个元素时，将 `count` 变量增加 1。
5. 迭代一次序列后，`count` 包含最长所需子序列的长度。
...
为了优化上述方法，观察发现所需子序列中元素的存在仅取决于其先前的元素，如下所示:
* 在最终答案中，替换索引 `x` 处的元素只会影响索引 `x` 和 `x + 1` 处元素的贡献。
* 因此，从变量 `count` 中减去这两个索引的贡献，重新计算这两个索引的贡献，将在恒定时间内给出所需的答案。

按照以下步骤解决问题:
* 初始化一个变量，比如 `count`，来存储子序列的计数。
* 遍历给定的数组并计算那些不同于数组前一个元素的元素，并将其存储在变量 `count` 中。
* 现在遍历每个查询 `{x，y}`，并执行以下操作:
    * 如果索引 `(x–1)` 处的元素与索引 `x` 处的元素不同，则按 1 递减 `count`。
    * 如果索引 `(x–1)` 处的元素不同于元素 `y`，则按 1 递增 `count`。
    * 同样，如果索引 `(x + 1)` 处的元素与索引 `x` 处的元素不同，则以 1 递减 `count`。
    * 如果索引 `(x + 1)` 处的元素与元素 `y` 不同，则按 1 递增 `count`。
    * 打印每次查询后 `count` 的值作为结果。
