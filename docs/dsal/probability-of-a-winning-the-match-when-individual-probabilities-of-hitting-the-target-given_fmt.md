...
> 原文: [https://www.geeksforgeeks.org/probability-of-a-winning-the-match-when-individual-probabilities-of-hitting-the-target-given/](https://www.geeksforgeeks.org/probability-of-a-winning-the-match-when-individual-probabilities-of-hitting-the-target-given/)

给定四个整数 `a`、`b`、`c` 和 `d`。玩家 `A` 和 `B` 尝试点球得分。A 射击目标的概率为 `a / b`，B 射击目标的概率为 `c / d`。先罚点球的球员获胜。任务是找出 A 赢得比赛的概率。
...
如果我们考虑变量 `K = a / b` 为 A 击中目标的概率，`R = (1 - (a / b)) * (1 - (c / d))` 为 A 和 B 都没有击中目标的概率。
因此，解形成几何级数 `K * R^0 + K * R^1 + K * R^2 + …..` 其和为 `(K / (1 - R))`。将 `K` 和 `R` 的值放在一起后，我们得到公式为 `K * (1 / (1 - (1 - R) * (1 - K)))`。
以下是上述方法的实施：
