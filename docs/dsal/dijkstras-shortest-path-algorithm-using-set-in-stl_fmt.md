...
以上操作可以通过 C++ STL 的 [`set` 数据结构轻松实现](http://geeksquiz.com/set-associative-containers-the-c-standard-template-library-stl/)，`set` 将其所有键按排序顺序保留，因此最小距离顶点将始终在开始处，我们可以从那里提取它，这是 `ExtractMin` 操作。 如果任何顶点的距离变小，则更新相应的其他相邻顶点，然后删除其先前的条目并插入新的更新条目，即 `DecreaseKey` 操作。
...
时间复杂度：C++中的 `set` 通常使用自平衡二进制搜索树来实现。 因此，`set` 操作（如插入，删除）的时间复杂度为对数，而上述解决方案的时间复杂度为 `O（ELogV）`。
