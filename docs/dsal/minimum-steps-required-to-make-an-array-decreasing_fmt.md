...
> 原文: [https://www.geeksforgeeks.org/minimum-steps-required-to-make-an-array-decreasing/](https://www.geeksforgeeks.org/minimum-steps-required-to-make-an-array-decreasing/)
...
**天真方法:** 迭代数组并计算大于其左边的元素，如果元素不大于其左边，则将元素推入另一个数组(比如 `arr1`)，在数组完全迭代后，将 `arr1` 的所有元素复制到初始数组，并重复相同的过程，直到在一个步骤中移除的元素计数为 0。这种方法在最坏的情况下需要 `O(N^2)` 的时间和 `O(N)` 的空间。

**高效方法:** 想法是使用堆栈，只有当元素大于其前一个元素时，才将其推入堆栈，否则计算扫描和弹出的次数。我们只关心较小的元素。
