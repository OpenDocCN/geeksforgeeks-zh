...
> 原文：[https://www.geeksforgeeks.org/find-the-nth-number-in-the-merged-and-sorted-lists-of-given-ranges/](https://www.geeksforgeeks.org/find-the-nth-number-in-the-merged-and-sorted-lists-of-given-ranges/)
...
*   要计算位置，迭代所有范围，使变量 `t = 0` 来存储 `位置`。如果 `L[i] >= mid`，检查以下两个条件
    *   如果 `mid <= R[i]`，更新 `t += mid - L[i] + 1`。
    *   否则 `t += R[i] - L[i] + 1`。
*   二分搜索法范围可以更新为
    *   if(`t > n`) `max = mid - 1`。
    *   else `min = mid + 1`。
