...
> 原文：[https://www.geeksforgeeks.org/find-the-player-to-last-modify-a-string-such-that-even-number-of-consonants-and-no-vowels-are-left-in-the-string/](https://www.geeksforgeeks.org/find-the-player-to-last-modify-a-string-such-that-even-number-of-consonants-and-no-vowels-are-left-in-the-string/)
...
> **说明：**
> 玩家 A 可以通过执行以下招式获胜：
> 招式 1：A 将 `A` 变为 `f`，因此 `S = "fbcd"`
> 招式 2：B 移除 `f`，因此 `S = "bcd"`。
> 移 3：A 移除 `b`，因此，`S = "cd"`。
> 移动 4：B 移除 `c`，因此，`S = "d"`。
> 移 5：A 移除 `d`，故 `S = ""`。
