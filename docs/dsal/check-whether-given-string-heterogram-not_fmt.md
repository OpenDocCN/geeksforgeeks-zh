...
> 原文: [https://www.geeksforgeeks.org/check-whether-given-string-heterogram-not/](https://www.geeksforgeeks.org/check-whether-given-string-heterogram-not/)

给定一个字符串 `S`。任务是检查给定的字符串是否是异形词。一个[异形词](https://en.wikipedia.org/wiki/Heterogram_(literature))是一个单词、短语或句子，其中字母表中没有一个字母出现超过一次。
...
想法是创建一个大小为 26 的散列数组 `hash`，初始化为 0。遍历给定字符串的每个字母表，并在相应的哈希数组位置标记 1。如果第一次遇到该字母表，则返回 `false`。
