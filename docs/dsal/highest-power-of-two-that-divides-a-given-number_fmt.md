...
一个简单的解决方法就是从 1 开始逐个尝试 2 的所有次方，然后是 2，然后是 4，以此类推。
一个高效解决方案基于[位魔法](https://www.geeksforgeeks.org/category/algorithm/bit-magic/)。如果我们仔细看看，我们可以注意到，我们基本上需要找到最右边的位设置在与 `n` 相同的位置，所有其他位都为 0 的数字。比如 `n = 5 (10 **1** )`，我们的输出是 `00 **1**`。对于 `n = 48 (1 **1** 0000)`，我们的输出是 `0 **1** 0000`。
...
这也是一种有效的方法，可以使用 C 语言中的预定义函数来处理位，从而找到数字 `n` 的二次幂的最大除数。也就是 `__builtin_ctz(n)`，这个功能帮助你找到数字的尾随零，然后你就可以看到比特-魔法。
