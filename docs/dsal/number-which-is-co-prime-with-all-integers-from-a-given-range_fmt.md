...
> 原文: [https://www.geeksforgeeks.org/number-which-is-co-prime-with-all-integers-from-a-given-range/](https://www.geeksforgeeks.org/number-which-is-co-prime-with-all-integers-from-a-given-range/)

给定两个正整数 `L` 和 `R`，任务是找到一个大于 `1` 的整数 `X`，使得 `X` 与范围 `[L, R]` 内的所有整数同素。

**示例:**

> **输入:** `L = 16`，`R = 17`
> **输出:** `19`
> **说明:** 只有与 `[16, 17]` 范围内所有整数同素的数才是 19。
>
> **输入:** `L = 973360`，`R = 973432`
> **输出:** `973439`

**方法:** 解决给定问题最简单的方法就是找一个大于 `R` 的素数，因为这个整数不除 `[L, R]` 范围内的任何整数。所以思路是从 `(R + 1)` 这个值开始迭代，如果有一个整数是素数，那么打印这个整数，跳出循环。
