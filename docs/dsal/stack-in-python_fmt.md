...
Python 堆栈可以使用 `collections` 模块中的 `deque` 类来实现。在我们需要从容器两端进行更快的追加和弹出操作的情况下，`deque` 优于 `list`，因为 `deque` 为追加和弹出操作提供了 O(1) 的时间复杂度，而 `list` 提供了 O(n) 的时间复杂度。
使用列表中所示的相同方法，`append()` 和 `pop()`。
...
*   `full()` – 如果队列中有 `maxsize` 个项目，返回真。如果队列初始化时 `maxsize=0` (默认值)，则 `full()` 永远不会返回 True。
