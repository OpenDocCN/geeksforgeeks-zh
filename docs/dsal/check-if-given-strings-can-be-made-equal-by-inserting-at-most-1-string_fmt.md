...
> 原文: [https://www.geeksforgeeks.org/check-if-given-strings-can-be-made-equal-by-inserting-at-most-1-string/](https://www.geeksforgeeks.org/check-if-given-strings-can-be-made-equal-by-inserting-at-most-1-string/)

给定两个句子 `S1` 和 `S2`，任务是通过在这两个句子的任何一个中最多插入一个句子（可能是空的）来检查句子是否可以相等。

**示例:**

> **输入:** `S1 = "Start practicing on GeeksforGeeks"`, `S2 = "Start GeeksforGeeks"`
> **输出:**
> True
> **说明:** `"practicing on"` 可以插在 `S2` 中的 `"Start"` 和 `"GeeksforGeeks"` 之间，使其等于 `S1`。
>
> **输入:** `S1 = "New Delhi is capital of India"`, `S2 = "is capital"`
> **输出:**
> False
...
*   这两个句子的最长共同前缀和最长共同后缀去掉后，如果其中至少有一个是空的，则表示可以使它们相等。

借助 `deque` 可以解决问题。按照以下步骤解决问题:

*   检查 `S1` 和 `S2` 的大小是否相等。如果它们相等，请执行以下操作:
    *   如果 `S1` 等于 `S2`，则返回 `True`。
    *   否则，返回 `False`。
*   初始化两个 `deque` `X` 和 `Y`。
*   将 `S1` 的所有文字推入 `X`。
*   将 `S2` 的所有文字推入 `Y`。
*   虽然 `X` 和 `Y` 的 `front()` 相同，但是从 `X` 和 `Y` 的 `front()` 弹出。
*   当 `X` 和 `Y` 的 `back()` 相同时，从 `X` 和 `Y` 的 `back()` 弹出。
*   检查 `X` 或 `Y` 是否为空。如果有空的，返回 `True`。
*   否则，返回 `False`。
