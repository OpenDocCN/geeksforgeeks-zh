...
*   如果 `K` 大于 `1`，则从 `K` 和[中减去 `1`，将 `front` 的值与 `1` 进行异或。](http://www.geeksforgeeks.org/calculate-xor-1-n/)
*   [使用变量 `i` 迭代范围](https://www.geeksforgeeks.org/range-based-loop-c/)`[2，N]`并检查:
    *   如果 `front` 是 `1`，那么[弹出德格](https://www.geeksforgeeks.org/dequepop_front-dequepop_back-c-stl/) `dq[]`的前面，推入矢量 `ans[]`否则[弹出德格](https://www.geeksforgeeks.org/dequepop_front-dequepop_back-c-stl/)的后面 `dq[]`推入矢量 `ans[]`。另外，如果 `K` 大于，则从 `K` 和[中减去 `1`，将 `front` 的值与 `1` 异或。](http://www.geeksforgeeks.org/calculate-xor-1-n/)
