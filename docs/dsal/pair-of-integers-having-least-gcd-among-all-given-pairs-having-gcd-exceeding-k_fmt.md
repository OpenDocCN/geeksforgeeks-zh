...
> 原文: [https://www.geeksforgeeks.org/pair-of-integers-having-least-gcd-among-all-given-pairs-having-gcd-exceeding-k/](https://www.geeksforgeeks.org/pair-of-integers-having-least-gcd-among-all-given-pairs-having-gcd-exceeding-k/)
...
最简单的方法是迭代给定数组的所有对，并检查每个对，如果其 GCD 超过 `K`。从所有这些对中，打印具有最少 `GCD` 的对。
...
想法是观察数组元素按其对的 GCD 值的递增顺序排序，因此使用[二分搜索法](https://www.geeksforgeeks.org/binary-search/)。按照以下步骤解决问题:

*   计算搜索空间的中间值，检查 `GCD(arr[mid]) > K`。
*   如果超过 `K`，则更新答案，将搜索空间的上限值降低至 `mid–1`。
*   如果 `GCD(arr[mid]) ≤ K`，则将搜索空间的下限值增加到 `mid + 1`。
