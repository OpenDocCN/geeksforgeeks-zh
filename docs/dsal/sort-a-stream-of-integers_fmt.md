...
最简单的方法是[遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-a-array/)，对于每个数组元素，[线性扫描数组](https://www.geeksforgeeks.org/linear-search/)找到该元素的正确位置，并将其放入数组中。
...
对上述方法进行优化，思路是用一个[二分搜索法](https://www.geeksforgeeks.org/binary-search/)找到每个元素在排序列表中的正确位置。按照以下步骤解决问题:

*   初始化一个数组 `ans` 来存储最终排序的数字流。
*   [使用变量 `i` 遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]` 作为一个数字流，并执行以下步骤:
    *   如果数组 `ans` 为空，将 `arr[i]` 推至 `ans`。
    *   否则，使用 [`lower_bound()`](https://www.geeksforgeeks.org/lower_bound-in-cpp/) 在已经排序的数组 `ans[]` 中找到 `arr[i]` 的正确位置，并将 `arr[i]` 推到其正确位置。
*   完成以上步骤后，打印数组 `ans[]`。
