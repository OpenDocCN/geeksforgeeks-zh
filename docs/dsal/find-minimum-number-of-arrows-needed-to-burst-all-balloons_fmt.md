...
> 原文：[https://www.geeksforgeeks.org/find-minimum-number-of-arrows-needed-to-burst-all-balloons/](https://www.geeksforgeeks.org/find-minimum-number-of-arrows-needed-to-burst-all-balloons/)

给定大小为 `N` 的[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `points[][]`，其中 `points[i]` 代表从 `points[i][0]` 到 `points[i][1]` 的 X 坐标区域上的气球。Y 坐标不重要。所有的气球都需要爆裂。要爆裂气球，可在 `(x, 0)` 点发射一箭，箭垂直向上行进，将满足条件 `points[i][0] <= x <= points[i][1]` 的所有气球爆裂。任务是找到使所有气球爆炸所需的最小箭数。

**示例：**

> **输入：** `N = 4`，`points = {{10, 16}, {2, 8}, {1, 6}, {7, 12}}`
> **输出：** `2`
...
*   [使用比较器/λ表达式根据气球的结束位置对数组进行排序。`sort(points, (a, b) -> Integer.compare(a[1], b[1]))`。](https://www.geeksforgeeks.org/sorting-2d-array-according-values-given-column-java/)
*   制作一个变量 `arrows` 并用 `1` 初始化它（至少需要一个箭头来炸开气球）。
*   制作一个变量 `end` 并用 `points[0][1]` 初始化它，这将存储第一个气球的结束。
*   [使用变量 `i` 迭代范围](https://www.geeksforgeeks.org/loops-in-java/) `[1, N)`，并执行以下步骤：
    *   检查下一个气球 `points[i][0]` 的起点是否小于终点变量 `end`。
    *   如果是，那么继续迭代。
    *   否则，通过 `1` 增加 `arrows` 的计数，并将 `end` 的值设置为 `points[i][1]`。
*   完成上述步骤后，打印 `arrows` 的值，作为所需箭头的合成计数。
