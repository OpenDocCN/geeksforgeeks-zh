...
> 原文: [https://www.geeksforgeeks.org/balanced-ternary-number-system/](https://www.geeksforgeeks.org/balanced-ternary-number-system/)
...
一个**平衡三进制数字系统**是由数字 `-1`、`0` 和 `1` 组成的数字系统。由于不方便将 `-1` 写成数字，为此我们将进一步使用字母 `Z`。
...
**注：**
该系统还允许表示负数，不需要在数字前加负号。平衡三元系统中的所有负数都以 `Z` 开头，即：
`1`<sub>10</sub> = `Z`<sub>3</sub>、
`2`<sub>10</sub> = `Z1`<sub>3</sub>、
`3`<sub>10</sub> = `Z0`<sub>3</sub>、...
