...
给定一个整数 `N` 和一组 `N` 的所有非负幂作为 `S = {N^0 ，N^1 ，N^2 ，N^3 ，… }` ，按照子集和的递增顺序排列 `S` 的所有非空子集。任务是从该排序中找到 `K^第` 子集的最大和最小元素的和。
...
> 所以 `K^第` (`3^第`)子集的元素是`{1，4}`。所以这个子集的最大和最小元素之和= `(4 + 1) = 5`。
>
> **输入:** `N = 3`，`K = 4`
> **输出:** `18`
> **解释:**
...
> 所以子集 `4^第`位置的元素是`{9}`。所以这个子集的最大和最小元素之和= `(9 + 9) = 18`。

## 方法

该方法基于[动力装置](https://www.geeksforgeeks.org/power-set/)的概念。按照以下步骤解决问题:

1.  逆序生成整数 `K` 的对应二进制表达式(即子集的位置)，以保持子集内元素的和不断增加。
2.  然后根据连续位置的 `lst[]` 列表中出现的位，计算在相应位置的子集中是否会有元素。
3.  迭代 `lst[]` ，如果 `lst[i]` 为 `0` ，则忽略该位，否则 `(N^i) * lst[i]` 将为 `K^第` 子集 `num[]` )。
