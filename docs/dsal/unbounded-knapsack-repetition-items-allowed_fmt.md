...
> 原文：[https://www.geeksforgeeks.org/unbounded-knapsack-repetition-items-allowed/](https://www.geeksforgeeks.org/unbounded-knapsack-repetition-items-allowed/)

给定一个背包重量 `W` 和一套具有一定价值 `val[i]` 和重量 `wt[i]` 的 `n` 物品，我们需要精确计算出能够补足这个数量的最大价值。这与[经典背包问题](https://www.geeksforgeeks.org/dynamic-programming-set-10-0-1-knapsack-problem/)不同，这里允许我们使用一个物品的无限个实例。
...
本文使用来自[Shubham Joshi](https://www.facebook.com/Shubh1307)和[Ashish Kumar](https://write.geeksforgeeks.org)的输入进行编辑。如果你喜欢 GeeksforGeeks 并想投稿，你也可以用 [write.geeksforgeeks.org](https://write.geeksforgeeks.org) 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
