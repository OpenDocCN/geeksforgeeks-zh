...
> 原文：[https://www.geeksforgeeks.org/sorting-array-elements-with-set-bits-equal-to-k/](https://www.geeksforgeeks.org/sorting-array-elements-with-set-bits-equal-to-k/)

给定一个整数数组和一个数字`K`。任务是只对数组中总设置位等于`k`的那些元素进行排序。排序必须只在它们的相对位置进行，而不影响任何其他元素。
...
*   初始化两个空向量。
*   从左到右遍历数组，检查每个元素的设置位。
*   这里，C++内置函数`__builtin_popcount()`来计数设置位。
