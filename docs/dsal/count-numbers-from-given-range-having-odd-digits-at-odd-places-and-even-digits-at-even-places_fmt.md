...
> 原文: [https://www.geeksforgeeks.org/count-numbers-from-given-range-having-odd-digits-at-odd-places-and-even-digits-at-even-places/](https://www.geeksforgeeks.org/count-numbers-from-given-range-having-odd-digits-at-odd-places-and-even-digits-at-even-places/)
...
可以观察到，每个**偶数位置**都有 5 种选择 `{0, 2, 4, 6, 8}`，每个**奇数位置**也有 5 种选择 `{1, 3, 5, 7, 9}`。因此，每个职位都有 5 种可能。考虑到 `d` 是满足条件的任意数字中的位数，`D` 是 `N` 中的位数，可以进行以下观察:

*   **情况 1:** 如果 `d < D`，则满足条件的由 `d` 个数字组成的总数的计数为 `5^D`，因为每个数字都有 5 个选择，做出的数字都将小于 `N`。
*   **情况 2:** 如果 `D = d`，那么满足长度 `d` 条件的总数计数为 `5^D`。但是超过 `N` 的数字需要丢弃，由以下决定:
    *   偶数的地方，如果 `p^th` 地点的数字是 `x`，那么 `(5 - (x/2 + 1)) * 5^(D - p)` 的数字大于 `N`。
...
*   初始化一个整数变量 `count` 和一个向量 `digits` 来存储整数 `N` 的位数。
*   遍历给定数字的所有数字，即从 1 到 `D`，并执行以下操作:
    *   将 `5^I` 存储在变量中，比如 `res`。
    *   从 `p = 1` 遍历到 `p = D` 并执行以下操作:
        *   初始化一个变量 `x`，并在其中存储 `x = digits[p]`。
        *   如果 `p` 为偶数，则从 `res` 中减去 `(5 - (x/2 + 1)) * 5^(D - p)`。
        *   否则，从 `res` 中减去 `(5 - (x + 1)/2) * 5^(D - p)`。
*   将 `res` 添加到 `count` 中。
*   返回 `count`。
*   打印 `countNumberutill(R) - countNumberutill(L - 1)` 作为所需答案。
