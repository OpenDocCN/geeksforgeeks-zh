...
> 原文: [https://www.geeksforgeeks.org/generating-numbers-that-are-divisor-of-their-right-rotations/](https://www.geeksforgeeks.org/generating-numbers-that-are-divisor-of-their-right-rotations/)
...
最简单的方法是遍历所有大于等于 `10^(m-1)` 小于 `10^m` 的数字，检查是否满足要求的条件。我们可以用常数时间来检查，所以整个过程的时间复杂度为 `O(10^m)`，只对 `m` 的小值可行。
