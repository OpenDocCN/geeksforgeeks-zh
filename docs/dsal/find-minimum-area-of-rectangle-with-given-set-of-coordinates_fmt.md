...
> 原文: [https://www.geeksforgeeks.org/find-minimum-area-of-rectangle-with-given-set-of-coordinates/](https://www.geeksforgeeks.org/find-minimum-area-of-rectangle-with-given-set-of-coordinates/)

给定一组位于 **X-Y 平面**的点的数组 `arr`。任务是找到由这些点形成的矩形的最小面积。矩形的边应该平行于 X 轴和 Y 轴。如果给定点不能形成矩形，则打印 `0`。

**例:**

> **输入:** `arr[][] = [[1, 1], [1, 3], [3, 1], [3, 3], [2, 2]]`
> **输出:** `4`
> 唯一可能的矩形将由点 `(1, 1)`、`(1, 3)`、`(3, 1)` 和 `(3, 3)` 构成。
>
> **输入:** `arr[][] = [[1, 1], [1, 3], [3, 1], [3, 3]]`

**方法:** 用 `X` 坐标将点分组，这样垂直直线上的点就分组在一起了。然后，对于组中的每一对点，例如坐标 `(X, Y1)` 和 `(X, Y2)`，我们检查最小的矩形，这一对点作为要形成的矩形的最右边。我们可以通过跟踪我们以前访问过的所有其他点对来做到这一点。最后返回得到的矩形的最小可能面积。
