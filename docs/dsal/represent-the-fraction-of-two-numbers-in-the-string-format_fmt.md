...
> 原文: [https://www.geeksforgeeks.org/represent-the-fraction-of-two-numbers-in-the-string-format/](https://www.geeksforgeeks.org/represent-the-fraction-of-two-numbers-in-the-string-format/)
...
思路是先算积分商(小数点前的绝对部分)，再算小数部分。要检查小数部分是否重复，请在映射中插入余数(`num` % `den`)，键作为余数，值作为余数出现的索引位置。如果在任何时间点，余数变为零，则不存在重复分数，否则如果余数已经在地图中找到，则存在重复分数。
