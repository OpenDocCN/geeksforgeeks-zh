...
> 原文: [https://www.geeksforgeeks.org/sum-distinct-elements-elements-range-1-n/](https://www.geeksforgeeks.org/sum-distinct-elements-elements-range-1-n/)
...
我们检查 `a[abs(a[i])-1]` 是否为 `>=0`，如果是，则将 `a[abs(a[i])-1]` 标记为负。即 `a[0] = 1 >= 0`，我们将 `a[1-1]` 标记为 `a[0] = -1`。接下来，`a[1]`，检查 `abs(a[1])-1` 是否为正。如果为负，则表示 `a[1]` 以前已经出现过，否则为该元素的第一次出现。参考以下代码。
