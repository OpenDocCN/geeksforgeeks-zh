...
> 原文: [https://www.geeksforgeeks.org/print-all-permutations-with-repetition-of-characters/](https://www.geeksforgeeks.org/print-all-permutations-with-repetition-of-characters/)
...
在上面的实现中，假设输入字符串的所有字符都不同。实现可以很容易地修改，以处理重复的字符。我们必须添加一个预处理步骤来查找唯一的字符(在调用`allLexicographicRecur()`之前)。
