...
> 原文: [https://www.geeksforgeeks.org/check-whether-an-array-can-be-made-strictly-increasing-by-removing-at-most-one-element/](https://www.geeksforgeeks.org/check-whether-an-array-can-be-made-strictly-increasing-by-removing-at-most-one-element/)
...
**方法:** 给定问题可以通过[遍历数组 `arr[]` 并计算满足条件 `arr[i-1] ≥ arr[i]` 的指标来解决](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)。按照以下步骤解决问题:

*   初始化两个变量，比如说 `count` 为 `0` 和 `index` 为 `-1`，分别存储需要删除的元素数和被删除元素的索引。
*   在范围 `[1，N–1]` 内遍历给定数组，如果 `arr[i–1]` 的值至少为 `arr[i]`，则将 `count` 的值增加 `1`，并将 `index` 的值更新为 `i`。
*   如果 `count` 的值大于 `1`，则打印 `“否”`，返回。
*   否则，检查以下情况:
    *   如果 `count` 的值等于 `0`，则打印 `“是”` 并返回。
    *   如果 `index` 等于 `(N–1)` 或等于 `0`，则打印 `“是”` 并返回。
    *   检查移除 `index` 处的元素是否使 `arr[index–1] < arr[index+1]`，然后打印 `“是”` 并返回。
    *   检查移除 `index–1` 处的元素是否会使 `arr[index–2] < arr[index]`，其中 `index–2 >= 0`，然后打印 `“是”` 并返回。
    *   检查 `index < 0`，然后打印 `“是”` 并返回。
