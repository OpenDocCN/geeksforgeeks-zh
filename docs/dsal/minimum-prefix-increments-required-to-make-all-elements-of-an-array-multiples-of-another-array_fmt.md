...
1.  [遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `A` 从结束到这个开始。
2.  求的最近倍数与 `B[]` 的对应元素之间的最小差 `K`。
3.  由于 `K` 等于在 `i` 元素处运算的次数，因此所有元素的值从 `0` 索引到 `(i-1)` 索引将增加 `K`。
4.  现在维护一个变量`carry`，它将存储累积增量，因此如果 `i` 元素递增 `K` 次，那么将 `K` 加到`carry`上。
5.  `carry`变量的值将被用来找到`(i-1)`的新值 `A[]`的元素。
