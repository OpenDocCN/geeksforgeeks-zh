...
**注意:** 可以假设列表中存在 `target` 字，所有字的长度都相同。如果出现多个序列，打印所有序列。

**示例:**

> **输入:** `str[] = {"poon", "plee", "same", "poie", "plie", "poin"}`，`start = "toon"`，`target = "plee"`
> **输出:** `[["toon", "poon", "poin", "poie", "plee"]]`
> **解释:** toon → poon → poin → poie → plee
>
...
使用 [BFS](https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/) 可以解决问题。这里棘手的部分是做 [BFS](https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/) 的路径，而不是文字。按照以下步骤解决问题:

1.  初始化一个变量，比如 `res`，来存储所有可能的最短路径。
2.  创建一个 [set](https://www.geeksforgeeks.org/set-in-cpp-stl/) 来存储当前路径中所有访问过的单词，一旦当前路径完成，擦除所有访问过的单词。
