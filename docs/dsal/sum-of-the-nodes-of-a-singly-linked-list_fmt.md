...
通过传递`head`和存储总和的`sum`变量来调用函数。然后通过传递当前节点的下一个和`sum`变量来递归调用该函数。将当前节点的值加到总和上。
...
2.  使用循环开始遍历链表，直到遍历所有节点。将当前节点的值添加到总和，即`sum += ptr->data`。递增指向链表的下一个节点的指针，即`ptr = ptr->next`。
3.  返回`sum`。
