...
**时间复杂度:** `O(n)`
**辅助空间:** `O(n)`
[超丑数(素数因子在给定集合中的数)](https://www.geeksforgeeks.org/super-ugly-number-number-whose-prime-factors-given-set/)
...
1.  如果你有 n 的最大值，这个方法是合适的。否的形式是 `x = pow(2, p) * pow(3, q) * pow(5, r)`。
2.  从 `low = 1` 到 `high = 2147483647` 搜索。我们预计这个范围内会有第 n 个丑陋的数字。
