...
> 原文: [https://www.geeksforgeeks.org/find-permutation-array-from-the-cumulative-sum-array/](https://www.geeksforgeeks.org/find-permutation-array-from-the-cumulative-sum-array/)

## 问题描述

给定一个由 `N` 个元素组成的数组 `arr[]`，其中每个 `arr[i]` 是另一个数组 `P[]` 的子数组 `P[0…i]` 的累积和，其中 `P` 是从 `1` 到 `N` 的整数排列。任务是找到数组 `P[]`，如果没有这样的 `P` 则打印 `-1`。
...
*   累积数组的第一个元素必须是置换数组的第一个元素，位于 `i` 位置的元素将是 `arr[i] – arr[i–1]`，因为 `arr[]` 是置换数组的累积和数组。
