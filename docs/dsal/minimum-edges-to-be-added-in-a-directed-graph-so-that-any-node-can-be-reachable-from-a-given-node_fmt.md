...
首先，使用简单的 [DFS](http://www.geeksforgeeks.org/depth-first-traversal-for-a-graph/) 将`X`可以到达的所有顶点标记为良好。 然后，对于每个坏顶点（无法从`X`到达的顶点）`v`，计算从`v`可以到达的坏顶点的数量（这也可以通过简单的`DFS`完成）。 将此数字设为`cnt_v`。 现在，以`cnt_v`的非递增顺序遍历所有坏顶点。 对于当前的不良顶点`v`，如果仍未将其标记为好，请从其运行`DFS`，将所有可到达的顶点标记为好，并将答案增加1（实际上，我们隐含地添加了边`(s, v)`）。 可以证明该解决方案给出了最佳答案。
