...
> 原文：[https://www.geeksforgeeks.org/generate-first-k-multiples-of-n-using-bitwise-operators/](https://www.geeksforgeeks.org/generate-first-k-multiples-of-n-using-bitwise-operators/)
...
*   迭代到 `K`。
*   每次迭代，打印 `N` 的当前值。
*   然后，计算 `N` 的每一个 `i` 第位的 `2^i` 之和。将此总和加到 `N` 上，重复上述步骤。
