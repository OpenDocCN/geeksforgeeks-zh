...
给定一个整数 `n`，生成[阿伦森序列](https://en.wikipedia.org/wiki/Aronson%27s_sequence)的第一个 `n` 项。
...
一个简单的想法是存储字符串“T 是 the”来获得序列的前两个项。对于这些术语中的每一个，将其转换为序数形式的单词并附加到字符串中，然后计算下一个更高术语的值。对生成 n-2 次的每个后续较高项重复该过程，以生成阿伦森序列的前 n 个项。
