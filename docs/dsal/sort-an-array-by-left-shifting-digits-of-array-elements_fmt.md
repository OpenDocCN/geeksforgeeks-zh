...
> 原文: [https://www.geeksforgeeks.org/sort-an-array-by-left-shifting-digits-of-array-elements/](https://www.geeksforgeeks.org/sort-an-array-by-left-shifting-digits-of-array-elements/)

给定一个由正整数 `N` 组成的数组 `arr[]`，任务是左移数组元素的数字，使数组修改为排序形式。如果存在多个解决方案，则打印其中任何一个。否则，打印 `-1`。
...
2.  遍历数组并以所有可能的方式移动数组元素的位数，选择最小但大于前一个数组元素的那个。如果找不到这样的元素，那么打印 `-1`。
3.  否则，在执行上述操作后打印数组元素。
