...
> 原文: [https://www.geeksforgeeks.org/find-the-power-of-k-nearest-to-n/](https://www.geeksforgeeks.org/find-the-power-of-k-nearest-to-n/)

给定两个整数 `N` 和 `K`。任务是为整数 `N` 寻找 `K` 的最近幂，如果有两个最近的幂，考虑较大的一个。
...
> **输入:** `N = 32`，`K = 7`
> **输出:** `49`
> **说明:** `7` 的幂是 `7`，`49`，`343`，... 在这些数字中，`49` 最接近 `32`。
>
> **输入:** `N = 6`，`K = 3`
> **输出:** `9`
...
2.  `K` 的较小幂将是 `log_K N` 的**地板值**（说 `X`）。所以数值会是 `pow(K, X)`。[P 的最低值=最接近 P 且 ≤ P 的整数]
3.  而 `K` 的更大幂将是 `log_K N` 的**上限值**（说 `Y`）。所以数值会是 `pow(K, Y)`。[P 的最大值=最接近 P 的整数 ≥ P]
