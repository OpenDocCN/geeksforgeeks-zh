...
> 原文: [https://www.geeksforgeeks.org/check-if-any-subarray-of-length-m-repeats-at-least-k-times-consecutively-or-not/](https://www.geeksforgeeks.org/check-if-any-subarray-of-length-m-repeats-at-least-k-times-consecutively-or-not/)
...
**天真方法:** 最简单的方法是[生成所有可能的长度为 `M` 的子阵列](https://www.geeksforgeeks.org/generating-subarrays-using-recursion/)，并检查每个子阵列，在给定的阵列中，是否以一个[子阵列的形式将它精确地连接 `K` 次](https://www.geeksforgeeks.org/check-whether-an-array-is-subarray-of-another-array/)。如果发现是真的，则打印 `"是"`。否则，打印 `"否"`。
...
上述方法可以通过使用[二指针技术](https://www.geeksforgeeks.org/two-pointers-technique/)进行优化。按照以下步骤解决问题：

*   初始化一个变量，比如 `count` 算作 `0`。
*   [使用变量遍历给定数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]` 在索引范围 `[0, N–M]` 内，比如 `i`，并执行以下步骤：
    *   如果 `arr[i]` 的值等于 `arr[i + M]`，那么将 `count` 增加 `1`，因为在子数组中有一个匹配。
    *   否则，将 `count` 更新为 `0` 作为相邻子数组中有一个断点。
    *   如果 `count` 的值为 `M * (K–1)`，则表示存在长度 `M` 的 `K` 连续相等的子数组。因此，打印 `"Yes"` 并[跳出循环](https://www.geeksforgeeks.org/break-statement-cc/)。
*   完成上述步骤后，如果 `count` 从未变为 `M * (K–1)`，则打印 `"No"`。
