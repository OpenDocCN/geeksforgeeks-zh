...
> 原文: [https://www.geeksforgeeks.org/minimum-sum-of-array-elements-based-on-given-criteria/](https://www.geeksforgeeks.org/minimum-sum-of-array-elements-based-on-given-criteria/)

给定一个大小为 `N` 的数组 `A[]`，其中一些条目是整数，一些条目是 `-1`。任务是用满足以下标准的数字替换 `-1`。

1.  要替换的数字的二进制表示应该只有奇数位的 `0`，并且数字必须是偶数。
2.  数组条目 `A[i]` 与替换 `-1` 的数字需满足 `A[i]>=A[i-1]`，并且对于给定数组 `A[0]!=-1`。
...
> **输出:** `153`
> 指标 2: 用 `8` 代替 `-1` 作为其二进制表示为 `1000`，其奇数位为 `0`，`8` 为偶数，`8 >= 5`
> 指标 4: 用 `32` 代替 `-1` 作为其二进制表示为 `100000`，其奇数位为 `0`，`32` 为偶数，`32 >= 25`
