...
> 原文:[https://www.geeksforgeeks.org/find-minimum-speed-to-finish-all-jobs/](https://www.geeksforgeeks.org/find-minimum-speed-to-finish-all-jobs/)

给定一个数组 `A` 和一个整数 `H`，其中 `1 <= A[i] <= 10^9` 和 `len(A) <= H <= 10^9`。每个元素 `A[i]` 代表剩余的待完成的未完成工作，`H` 代表完成所有工作的剩余小时数。任务是找到每小时工作的最小速度，在此速度下，人需要在 `H` 小时内完成所有工作。

**注意:** 如果 `A[i]` 要做的工作比人的速度少，那么他就完成了 `A[i]` 的所有工作，在这一小时内不会去下一个元素。
...
如果我们让 `isPossible(K)` 为 `true` 当且仅当此人能以 `K` 的工作速度完成工作，那么就有一些 `X` 使得 `isPossible(K) = true` 当且仅当 `K >= X`。
比如 `A = [3, 6, 7, 11]` 和 `H = 8` 有一些 `X = 4` 这样 `isPossible(1) = isPossible(2) = isPossible(3) = false`，`isPossible(4) = isPossible(5) = ... = true`。
我们可以做一个[二分搜索法](https://www.geeksforgeeks.org/binary-search/)关于 `isPossible(K)` 的值找到第一个 `X` 这样 `isPossible(X)` 就是 `true`，这将是我们的答案。
现在，由于即使作业完成也不允许在当前小时内从一个元素移动到另一个元素，因此 `K` 的最大可能值可以是数组 `A[]` 中的最大元素。所以，要求 `isPossible(K)` 的值(即作业速度为 `K` 的人能否在 `H` 小时内完成所有作业)，做范围内的二分搜索法 `(1, max_element_of_array)`。
