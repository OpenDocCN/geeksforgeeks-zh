...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-odd-length-palindromic-numbers-within-the-range-l-r/](https://www.geeksforgeeks.org/sum-of-all-odd-length-palindromic-numbers-within-the-range-l-r/)

给定两个整数 `L` 和 `R`，任务是找出 `[L，R]` 范围内所有回文数的和，这些回文数的长度为奇数。
...
从 `L` 到 `R` 迭代，对于每个数字检查是否是回文且长度为奇数。如果是，那么把它加到总数中。最后打印总和的值。
