...
> 原文: [https://www.geeksforgeeks.org/find-the-sum-of-the-series-xxy-x2x2y2-x3x3y3-xnxnyn/](https://www.geeksforgeeks.org/find-the-sum-of-the-series-xxy-x2x2y2-x3x3y3-xnxnyn/)

给定一系列 `x(x+y)+x^{2}(x^{2}+y^{2})+...+x^{n}(x^{n}+y^{n})`，其中 `x`、`y` 和 `n` 取整数值。任务是找到给定系列的第 `n` 个项的和。
