...
> 原文：[https://www.geeksforgeeks.org/queries-on-insertion-of-an-element-in-a-bitonic-sequence/](https://www.geeksforgeeks.org/queries-on-insertion-of-an-element-in-a-bitonic-sequence/)

给定一个[双音素序列](https://www.geeksforgeeks.org/find-bitonic-point-given-bitonic-sequence/)和`Q`的查询数量。每个查询包含一个整数`x_i`，`1 <= i <= Q`，任务是在为每个查询插入整数后打印出二进制序列的长度。此外，在所有查询后打印双音素序列。
...
> ```
> **解释：**
> - 对于第一个查询，我们需要插入`x_1 = 5`，但是由于`5`是最大元素，并且`S`中最大元素只能出现一次，因此我们不会在`S`中插入`5`。因此，`size = 4`。
> - 对于第二个查询，我们需要插入`x_2 = 1`，所以我们将其插入递减部分，因为递增部分已经有`1`了。因此，`大小 = 5`。
> - 对于第三个查询，我们在递增边插入`x_3 = 3`，因此`大小`变为`6`。
> - 对于第四个查询，我们不能插入`x_2 = 2`，因为`2`同时在递增和递减侧。
...
**做法：** 思路是做两套，一套增序，一套减序。
1. 现在，对于每个查询，首先检查`x_i`是否大于双音素序列中的最大元素。
