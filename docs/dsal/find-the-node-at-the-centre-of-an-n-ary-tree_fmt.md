...
> 原文: [https://www.geeksforgeeks.org/find-the-node-at-the-centre-of-an-n-ary-tree/](https://www.geeksforgeeks.org/find-the-node-at-the-centre-of-an-n-ary-tree/)
...
**方法:** 可以观察到，最大偏心的路径是树的直径。因此，树直径的中心也将是树的中心。

**证明:**

> *   例如，考虑最长路径由奇数个顶点组成的情况。设最长路径为 `x - o - y`，其中 `x` 和 `y` 是路径的两个端点，`o` 是中间顶点。
> *   反证，如果树的中心不是 `o` 而是其他某个顶点 `O'`，那么以下两个陈述中至少有一个必须为真。
>     1.  路径 `XO'` 严格长于路径 `XO`
>     2.  路径 `YO'` 严格长于路径 `YO`
> *   这意味着 `O'` 将不满足最小偏心率的条件。因此，通过反证，我们证明了树的中心实际上是直径路径的中心。
