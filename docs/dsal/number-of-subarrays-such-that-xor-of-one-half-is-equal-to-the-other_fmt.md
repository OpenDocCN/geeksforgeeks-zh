...
> 原文：[https://www.geeksforgeeks.org/number-of-subarrays-such-that-xor-of-one-half-is-equal-to-the-other/](https://www.geeksforgeeks.org/number-of-subarrays-such-that-xor-of-one-half-is-equal-to-the-other/)
...
如果一个数组被分成相等的两半，其中一半的 `XOR` 等于另一半，这意味着整个数组的 `XOR` 应该是 `0`，因为 `A^A = 0`。现在，为了解决上述问题，从左边开始找出给定数组中所有元素的前缀异或。

假设一个子阵从 `l` 开始，到 `r` 结束，那么 `(r-l+1)` 应该是`偶数`。同样，为了找到给定范围的异或 `(l, r)` 用前缀异或减去前缀异或 `(l-1)`。
