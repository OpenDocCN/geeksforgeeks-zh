...
> 原文: [https://www.geeksforgeeks.org/sum-of-frequencies-of-characters-of-a-string-present-in-another-string/](https://www.geeksforgeeks.org/sum-of-frequencies-of-characters-of-a-string-present-in-another-string/)
...
**输入:** `S1 = "geEksFOR"`, `S2 = "GeEksforgeEKS"`
**输出:** `7`

## 朴素方法
最简单的方法是[迭代字符串的每个字符](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-c/) `S1`，计算其在字符串 `S2` 中的出现频率。

**时间复杂度:** `O(N^2)`
**辅助空间:** `O(1)`

## 高效方法
上述方法可以通过使用[哈希](https://www.geeksforgeeks.org/hashing-set-1-introduction/)进行优化。按照以下步骤解决问题:
- 初始化一个整数 `count` 来存储所需的总和。
- [将字符串 `S1` 的所有字符插入一个集合](https://www.geeksforgeeks.org/set-insert-function-in-c-stl/)中，比如 `bset`。
- [遍历字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-python/) `S2` 的字符，检查当前字符是否存在于集合 `bset` 中。如果发现为真，则增加 `count` 1。
