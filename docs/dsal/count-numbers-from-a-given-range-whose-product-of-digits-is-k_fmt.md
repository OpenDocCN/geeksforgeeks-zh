...
> 原文: [https://www.geeksforgeeks.org/count-numbers-from-a-given-range-whose-product-of-digits-is-k/](https://www.geeksforgeeks.org/count-numbers-from-a-given-range-whose-product-of-digits-is-k/)
...
>
> `cntNum(N, K, st, tight)`: 返回数字乘积为 `K` 的 `[0, X]` 范围内的数字计数
...
1.  初始化一个[三维数组](https://www.geeksforgeeks.org/multidimensional-arrays-c-cpp/) `DP[N][K][tight]` 计算并存储上述递归关系的所有子问题的值。
2.  最后，返回 `DP[N][sum][tight]` 的值。
