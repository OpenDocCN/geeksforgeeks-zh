...
> 原文：[https://www.geeksforgeeks.org/print-all-possible-permutations-of-an-array-with-duplicates-using-backtracking/](https://www.geeksforgeeks.org/print-all-possible-permutations-of-an-array-with-duplicates-using-backtracking/)
...
3.  在重复元素中设置选择顺序。
4.  **如果 `i > 0 && nums[i] == nums[i-1]`：** 在当前排列中添加 `nums[i]` 只有在排列中添加了 `nums[i-1]` 的情况下，即 `visited[i-1]` 为 `真` 时才进行。
