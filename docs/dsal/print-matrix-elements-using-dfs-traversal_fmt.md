...
> 原文: [https://www.geeksforgeeks.org/print-matrix-elements-using-dfs-traversal/](https://www.geeksforgeeks.org/print-matrix-elements-using-dfs-traversal/)
...
> **解释:** 矩阵元素的深度优先搜索遍历顺序为 `1 2 3 4 8 12 16 15 11 7 6 10 14 13 9 5`
>
...
*   定义一个函数，比如说 `isValid(i, j)`，检查位置 `(i, j)` 是否有效，也就是说 `(i, j)` 应该在矩阵内部而不是被访问。
...
// Utility function to print matrix
// elements using DFS Traversal
function DFSUtil(row, int col, grid,
            vis, M, N)
{
...
思路是使用[迭代深度优先搜索](https://www.geeksforgeeks.org/iterative-depth-first-traversal/)遍历矩阵，打印矩阵元素。按照以下步骤解决问题:

*   定义一个函数，比如`isValid(i, j)`，检查位置 `(i, j)` 是否有效，即 `(i, j)` 是否位于矩阵内部且未被访问。
...
*   当[栈](https://www.geeksforgeeks.org/stack-data-structure/)不为空时迭代:
