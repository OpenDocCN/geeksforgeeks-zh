...
> 原文: [https://www.geeksforgeeks.org/rearrange-a-string-in-sorted-order-followed-by-the-integer-sum/](https://www.geeksforgeeks.org/rearrange-a-string-in-sorted-order-followed-by-the-integer-sum/)
...
实现步骤如下：
1.  开始遍历给定的字符串。
    a) 如果遇到字母，则在哈希表中增加其出现次数计数。
    b) 如果遇到整数，则将其单独存储，每次累加求和。
2.  使用哈希表，先将所有字符按顺序追加到一个字符串中，最后在末尾追加整数的和。
3.  返回结果字符串。
