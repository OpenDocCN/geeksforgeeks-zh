...
> 原文: [https://www.geeksforgeeks.org/find-n-distinct-numbers-whose-bitwise-or-is-equal-to-k/](https://www.geeksforgeeks.org/find-n-distinct-numbers-whose-bitwise-or-is-equal-to-k/)
...
2.  所以，我们只有这些位置可以改变位在 `K` 中的 `1` 的位置。说那算 `Bit_K`。
3.  现在，我们可以用 `Bit_K` 组成 `2^Bit_K` 个不同的数。因此，如果我们将一个数字设置为 `K` 本身，那么剩下的 `N–1` 数字可以通过将每个数字中的所有位设置为 0 来构建，这些位在 `K` 中为 `0`，对于其他位位置，`Bit_K` 位而不是数字 `K`。
4.  如果 `2^Bit_K < N` 那么我们找不到任何可能的答案。
