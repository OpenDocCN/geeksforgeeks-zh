...
> 原文：[https://www.geeksforgeeks.org/reduce-n-to-1-in-minimum-moves-by-either-multiplying-by-2-or-dividing-by-6/](https://www.geeksforgeeks.org/reduce-n-to-1-in-minimum-moves-by-either-multiplying-by-2-or-dividing-by-6/)
...
*   否则，让 `count2` 为 `n` 的因式中的 2 的个数，让 `count3` 为 `n` 的因式中的 3 的个数。
    *   如果 `count2 > count3`，那么答案就是 `-1`，因为我们不能把两个都去掉。
