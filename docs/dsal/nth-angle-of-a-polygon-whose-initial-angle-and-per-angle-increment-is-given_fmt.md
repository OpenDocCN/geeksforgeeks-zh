...
> 原文: [https://www.geeksforgeeks.org/nth-angle-of-a-polygon-whose-initial-angle-and-per-angle-increment-is-given/](https://www.geeksforgeeks.org/nth-angle-of-a-polygon-whose-initial-angle-and-per-angle-increment-is-given/)

给定四个整数 `N`，`A`，`K`，`n`，其中 `N` 代表多边形的边数，`A` 代表多边形的初始角度，`K` 代表每增加一个角度，任务是找到具有 `N` 条边的多边形的第 `n` 个角度。如果不可能，则打印 `-1`。
...
*   求出 `N` 边多边形的角和以及给定多边形的角和。
*   检查两个值是否相等。如果是，那么第 `n` 个角度是可能的，因此找到第 `n` 个角度。
*   否则，打印 `-1`。
