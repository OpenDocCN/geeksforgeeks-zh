...
`n` 个数字的序列（`n < 3000`）被称为 **Jolly Jumper**，如果连续元素之间的差值的绝对值取从 `1` 到 `n-1` 的所有可能值。这个定义意味着单个整数的任何序列都是一个快乐的跳跃者。
...
**参考文献：**
[http://users.csc.calpoly.edu/~jdalbey/301/Labs/JollyJumpers.html](http://users.csc.calpoly.edu/~jdalbey/301/Labs/JollyJumpers.html)
