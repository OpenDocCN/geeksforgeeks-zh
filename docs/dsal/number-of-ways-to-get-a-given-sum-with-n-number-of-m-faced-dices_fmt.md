...
> 原文: [https://www.geeksforgeeks.org/number-of-ways-to-get-a-given-sum-with-n-number-of-m-faced-dices/](https://www.geeksforgeeks.org/number-of-ways-to-get-a-given-sum-with-n-number-of-m-faced-dices/)
...
*   基本案例:
    1.  如果 `sum == 0` 且 `noofthrowsleft == 0` 则返回 `1`。这意味着总和 `x` 已经达到。
    2.  如果 `sum < 0` 或 `noofthrowsleft == 0` 则返回 `0`。这意味着 `x` 总和并没有在所有的投掷中达到。
