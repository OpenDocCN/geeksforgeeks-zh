...
> 原文：[https://www.geeksforgeeks.org/queries-to-find-the-left-most-given-type-integer-in-a-binary-array/](https://www.geeksforgeeks.org/queries-to-find-the-left-most-given-type-integer-in-a-binary-array/)

给定一个二进制数组 `arr[]`，任务是设计一个支持 O(1) 中以下操作的数据结构。

*   **类型 1:** 从数组中移除并打印最左边的 `0`。
*   **类型 2:** 从数组中移除并打印最左边的 `1`。
*   **类型 3:** 移除并打印数组中最左边的元素。
...
**天真的方法:** 一个简单的方法是将所有元素插入到支持先进先出的队列中。这种方法在 O(1) 中可以找到数组中最左边的元素，但在 O(1) 中找不到最左边的 1 或 0。
