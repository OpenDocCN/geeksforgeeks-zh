...
> 原文：[https://www.geeksforgeeks.org/minimum-size-of-subset-of-pairs-whose-sum-is-at-least-the-remaining-array-elements/](https://www.geeksforgeeks.org/minimum-size-of-subset-of-pairs-whose-sum-is-at-least-the-remaining-array-elements/)

给定两个[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `A[]` 和 `B[]`，都由 `N` 个正整数组成，任务是找到元素对 `(A[i], B[i])` 的子集的最小大小，使得所有子集对的和至少是不包括在子集中的剩余数组元素 `A[]` 的和。
...
**高效方法：** 给定的问题可以通过使用[贪婪方法](https://www.geeksforgeeks.org/greedy-algorithms/)来解决，其思想是使用[排序](https://www.geeksforgeeks.org/sorting-algorithms/)，并且可以观察到选择第 `i` 对作为结果子集的一部分，两个子集之间的差减少了量 `(2*S[i] + U[i])`。按照以下步骤解决问题：
