...
> 原文: [https://www.geeksforgeeks.org/check-elements-array-divide-atleast-1-element-array/](https://www.geeksforgeeks.org/check-elements-array-divide-atleast-1-element-array/)
...
*   保持一个数组 `mark[]` 来标记 `A[]` 中数字的倍数。
*   标记 `A[]` 中所有元素的所有倍数，直到最大值为 `B[]`。
*   检查 `B[]` 中每个元素 `n` 的 `mark[B[i]]` 值是否不为 0，如果未标记，则打印。
