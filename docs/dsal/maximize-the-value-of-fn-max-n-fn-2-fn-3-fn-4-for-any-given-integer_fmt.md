...
最简单的方法是使用[递归](https://www.geeksforgeeks.org/recursion/)计算 `F(N)` 的值。在每个步骤中，分别对值 `N / 2`，`N / 3` 和 `N / 4` 分别调用三个递归调用，然后返回每个递归调用 `N` 的最大值与这些递归调用返回的值之和。在所有递归调用结束后，将值打印为结果。
