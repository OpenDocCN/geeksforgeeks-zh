...
### 解决方案
选项(A)有效，因为 `p` 是指向“string1”的字符的指针。使用 `++p`，`p` 将指向“string1”中的字符“t”。因此，`*++p` 将打印“t”。
...
### 解法
在给定的代码中，`p[20]`被声明为字符数组，`s` 是指向字符串的指针。`length` 将被初始化为 6。在 `for` 循环(`i = 0`)的第一次迭代中，
...
现在，`p + 4` 将指向 `'2'`，将打印从 `'2'` 开始直到字符串结束的字符串。