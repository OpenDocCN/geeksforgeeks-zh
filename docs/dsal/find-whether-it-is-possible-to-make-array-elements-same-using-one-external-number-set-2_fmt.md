...
> 原文: [https://www.geeksforgeeks.org/find-whether-it-is-possible-to-make-array-elements-same-using-one-external-number-set-2/](https://www.geeksforgeeks.org/find-whether-it-is-possible-to-make-array-elements-same-using-one-external-number-set-2/)
...
> **输出：** 是 1
> **解释：**
> 将 `X` 的值视为 1，将数组元素 `arr[0](= 2)` 和 `arr[4](= 2)` 递增 1，将 `arr[3](= 4)` 递减 1 将数组修改为 `{3, 3, 3, 3, 3}`。
> 因此，打印“是”，数值 `X` 为 1。
>
> **输入：** `arr[] = {4, 3, 2, 1}`
> **输出：** 否
...
*   初始化 `3` 变量，比如说，`X`，`Y` 和 `Z` 为 `-1`，以存储数组 `arr[]` 的所有 `3` 个不同整数。
