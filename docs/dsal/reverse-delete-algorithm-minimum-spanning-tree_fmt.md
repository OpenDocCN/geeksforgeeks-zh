...
1.  以上实现是反向删除算法的简单/幼稚实现，可以优化为`O(E log V (log log V)^3)`【来源: [Wiki](https://en.wikipedia.org/wiki/Reverse-delete_algorithm#Running_time)】。但是这种优化的时间复杂度仍然低于 MST 的 `Prim` 和 `Kruskal` 算法。
