...
像[斐波那契字](https://www.geeksforgeeks.org/fibonacci-word/)，一个 Tribonacci 字是特定的数字序列。Tribonacci 单词是通过重复连接形成的，就像 Fibonacci 单词是通过重复相加形成的一样。但与斐波那契字不同的是，Tribonacci 字是由后三项重复相加而成的，它的前三项互不相同。
