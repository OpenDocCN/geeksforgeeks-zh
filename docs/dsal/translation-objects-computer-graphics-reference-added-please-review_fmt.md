...
> 原文：[https://www.geeksforgeeks.org/translation-objects-computer-graphics-reference-added-please-review/](https://www.geeksforgeeks.org/translation-objects-computer-graphics-reference-added-please-review/)
...
这里我们只根据给定的平移因子 `dx` 和 `dy` 分别平移给定点的 x 和 y 坐标。
下面是平移一个点的 C++ 程序：
...
平移一条线的思路是，根据给定的平移因子 `(dx, dy)` 平移该线的两个端点，然后使用内置图形函数绘制一条新线。
下面是上述思路的 C++ 实现：
...
这里我们根据给定的平移因子 `dx` 和 `dy` 分别平移给定点 `A`(左上) 和 `B`(右下) 的 x 和 y 坐标，然后使用内置图形函数绘制一个矩形。
