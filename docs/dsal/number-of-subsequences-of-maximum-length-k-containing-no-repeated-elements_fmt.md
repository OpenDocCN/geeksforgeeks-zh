...
> 原文：[https://www.geeksforgeeks.org/number-of-subsequences-of-maximum-length-k-containing-no-repeated-elements/](https://www.geeksforgeeks.org/number-of-subsequences-of-maximum-length-k-containing-no-repeated-elements/)

给定一个由 `N` 个元素和正整数 `K` 组成的数组 `arr[]`，使得 `K ≤ N`。任务是找到最大长度 `K` 的子序列的数量，即长度为 0，1，2，…，K–1，K 的子序列都有不同的元素。

**示例：**

> **输入：** `arr[] = {2，2，3，3，5}`，`K = 2`
> **输出：** 14
> 所有有效子序列为 `{}`、`{2}`、`{2}`、`{3}`、`{5}`、`{2，3}`、`{2，3}`、`{2，3}`、`{2，3}`、`{2，5}`、`{2，5}`、`{2，5}`、`{3，5}` 和 `{3，5}`。
>
...
*   假设 `m` 是向量 `arr[]` 的长度。所以 `m` 将是不同元素的数量。可以有不重复的最大长度为 `m` 的子序列。如果 `m < k`，则没有长度 `k` 的子序列。所以，声明 `n = min(m，k)`。
*   现在应用动态编程。创建一个二维数组 `dp[n + 1][m + 1]`，这样 `dp[i][j]` 将存储长度为 `i` 的子序列的数量，其第一个元素从 `arr[]` 中的第 `j` 个元素之后开始。例如，`dp[1][1] = 3`，因为它表示长度为 1 的子序列的数量，其第一个元素在 `arr[]` 的第一个元素之后开始，第一个元素为 `{3}`、`{3}`、`{5}`。
    *   将第一行 `dp[][]` 初始化为 `1`。
