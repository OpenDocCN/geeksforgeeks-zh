...
> 原文：`https://www.geeksforgeeks.org/greatest-number-less-than-equal-to-b-that-can-be-formed-from-the-digits-of-a/`
...
> **输入：** `A = 123`，`B = 222`
> **输出：** `213`
> `123`，`132` 和 `213` 是唯一 `≤ 222` 的有效数字。其中以 `213` 为最大。
...
让我们从最左边开始一个数字一个数字地构造答案。我们需要建立一个字典上最大的答案，所以我们应该在每一步中选择最大的数字。
...
我们可以使用[多集](https://www.geeksforgeeks.org/multiset-in-cpp-stl/)来保持集合中一个数字的所有出现。我们也可以用 C++ 中的 [`upper_bound()`](https://www.geeksforgeeks.org/upper_bound-and-lower_bound-for-vector-in-cpp-stl/) 来做二分搜索法，快速找到要放置的数字。