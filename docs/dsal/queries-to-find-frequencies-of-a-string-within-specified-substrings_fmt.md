...
> 原文：[https://www.geeksforgeeks.org/queries-to-find-frequencies-of-a-string-within-specified-substrings/](https://www.geeksforgeeks.org/queries-to-find-frequencies-of-a-string-within-specified-substrings/)
...
**时间复杂度：** `O(|S| + Q)`，因为每个查询都是在 `O(1)` 中回答的。
**辅助空间：** `O(|S|)`