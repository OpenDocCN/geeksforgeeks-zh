...
> 原文：[https://www.geeksforgeeks.org/modify-string-by-replacing-characters-by-alphabets-whose-distance-from-that-character-is-equal-to-its-frequency/](https://www.geeksforgeeks.org/modify-string-by-replacing-characters-by-alphabets-whose-distance-from-that-character-is-equal-to-its-frequency/)
...
给定的问题可以通过维护[频率数组来解决，该数组存储字符串](https://www.geeksforgeeks.org/print-characters-frequencies-order-occurrence/)中每个字符的出现次数。按照以下步骤解决问题:
...
*   [遍历字符串](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `S` 使用变量 `i` 并执行以下步骤:
    *   将要添加到 `S[i]` 的值存储在变量 `add` 中，`add` 为 `(freq[i] % 26)`。
    *   如果将 `add` 的值加到 `S[i]` 后，`S[i]` 没有超过字符 `z`，则更新 `S[i]` 为 `S[i] + add`。
    *   否则，将 `add` 的值更新为 `(S[i] + add - 'z')`，然后将 `S[i]` 设置为 `('a' + add - 1)`。
