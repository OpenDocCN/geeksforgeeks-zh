...
> 原文: [https://www.geeksforgeeks.org/construct-array-b-as-last-element-left-of-every-suffix-array-obtained-by-performing-given-operations-on-every-suffix-of-given-array/](https://www.geeksforgeeks.org/construct-array-b-as-last-element-left-of-every-suffix-array-obtained-by-performing-given-operations-on-every-suffix-of-given-array/)
...
最简单的方法是遍历每个后缀数组，通过迭代后缀数组来执行上面给定的操作，然后打印得到的值。
...
> 1.  从[位运算属性](https://www.geeksforgeeks.org/bitwise-and-or-of-a-range/)：
>     *   `(x | y) - (x ^ y) = (x & y)`
> 2.  因此，对后缀数组执行给定操作后获得的最后一个值，是该后缀数组所有元素的[按位与](https://www.geeksforgeeks.org/bitwise-operators-in-c-cpp/)之和。

按照以下步骤解决问题：

*   [使用变量](https://www.geeksforgeeks.org/range-based-loop-c/) `i` 在范围 `[0, N-2]` 中以相反的顺序迭代，并且在每次迭代中将 `arr[i]` 更新为 `arr[i] & arr[i+1]`。
*   [在范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[0, N-1]` 中迭代，并使用变量 `i` 执行以下步骤：
    *   打印存储在 `arr[i]` 中的值，作为后缀数组在范围 `[i, N-1]` 内的答案。
