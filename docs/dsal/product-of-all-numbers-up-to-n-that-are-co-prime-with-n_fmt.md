...
> 原文: [https://www.geeksforgeeks.org/product-of-all-numbers-up-to-n-that-are-co-prime-with-n/](https://www.geeksforgeeks.org/product-of-all-numbers-up-to-n-that-are-co-prime-with-n/)

给定一个整数 `N`，任务是找到从范围 `[1, N]` 中与给定数 `N` [共质数](https://www.geeksforgeeks.org/check-two-numbers-co-prime-not/)的所有数的[乘积](https://www.geeksforgeeks.org/program-for-product-of-array/)。
...
1.  将 `product` 初始化为 `1`。
2.  迭代范围 `[1, N]`，如果 `i` 和 `N` 的 `GCD` 为 `1`，则将 `product` 乘以 `i`。
3.  完成上述步骤后，打印 `product` 的数值。
