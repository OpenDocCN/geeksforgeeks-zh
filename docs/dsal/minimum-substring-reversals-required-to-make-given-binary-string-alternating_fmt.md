...
> 原文：[https://www.geeksforgeeks.org/minimum-substring-reversals-required-to-make-given-binary-string-alternating/](https://www.geeksforgeeks.org/minimum-substring-reversals-required-to-make-given-binary-string-alternating/)
...
这个想法是基于这样的观察：当一个子串 `S[L，R]` 被反转时，那么不超过两对 `S[L–1]`、`S[L]` 和 `S[R]`、`S[R+1]` 被改变。而且，一对应该是连续的一对 `00`，另一对 `11`。因此，通过将 `00` 与 `11` 或与 `S` 的左/右边框配对，可以获得最小的运算次数。因此，所需的操作数是同一字符的连续对数的一半。按照以下步骤解决问题：

*   [遍历字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-c/) `S` 统计 `1s` 和 `0s` 的个数，分别存储在 `sum1` 和 `sum0` 中。
*   如果 `sum1` 和 `sum0` 的绝对差大于 `1`，则打印 `-1`。
