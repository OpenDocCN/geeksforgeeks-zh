...
> 原文: [https://www.geeksforgeeks.org/modulo-of-a-large-binary-string/](https://www.geeksforgeeks.org/modulo-of-a-large-binary-string/)
...
**方法:** 已知 `str` 为二进制字符串的 `(str % K)` 可以写成 `((str[n–1]* 2^0)+(str[n–2]* 2^1)+…+(str[0]* 2^n–1))% K` 又可以写成 `((str[n–1]* 2^0)% K +((str[n–2]* 2^1)% K)+…+((str[0]* 2^n–1)% K))% K`。这可以用来找到所需的答案，而无需实际将给定的二进制字符串转换为其十进制等价物。
