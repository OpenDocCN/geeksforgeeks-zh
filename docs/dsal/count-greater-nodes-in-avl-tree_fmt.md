...
> 原文: [https://www.geeksforgeeks.org/count-greater-nodes-in-avl-tree/](https://www.geeksforgeeks.org/count-greater-nodes-in-avl-tree/)
...
2.  为了计算大于给定值的节点数，我们简单地遍历树。当遍历时考虑三个案例：
...
`CountGreater`函数的时间复杂度是 `O(log(n))`，其中 `n` 是AVL树中的节点数，因为我们基本上是在AVL树中搜索给定的数，这需要 `O(log(n))` 时间。
