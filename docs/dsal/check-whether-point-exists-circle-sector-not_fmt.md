...
> 原文: [https://www.geeksforgeeks.org/check-whether-point-exists-circle-sector-not/](https://www.geeksforgeeks.org/check-whether-point-exists-circle-sector-not/)
...
1.  使用此角度 `= atan(y/x)` 将 `x`, `y` 转换为极坐标；`radius = sqrt(x * x + y * y)`；
