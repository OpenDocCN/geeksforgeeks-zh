...
> 原文：[https://www.geeksforgeeks.org/print-a-case-where-the-given-sorting-algorithm-fails/](https://www.geeksforgeeks.org/print-a-case-where-the-given-sorting-algorithm-fails/)
...
**逼近：** 在求解各种情况时，我们可以观察到，只有对于 `n <= 2`，给定的算法是**无效的**。任何高于 `N` 的值都将在给定的算法中失败。由 `N` 个数字（`1, 2, 3, ..., n`）组成的排序数组不能使用这个给定的算法进行反向排序。
