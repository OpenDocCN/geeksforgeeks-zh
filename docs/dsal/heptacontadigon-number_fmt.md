...
给定一个数字 `N`，任务是找到 `Nth` [七加龙数](https://en.wikipedia.org/wiki/List_of_polygons)。

> 一个**七加龙数**是一类图形数。它有一个 72 边的多边形，叫做七十二边形。第 `N` 个七加龙数是 72 个点的数量，所有其他点都被一个公共的共享角包围并形成一个图案。前几个七加龙数是 **1，72，213，424，…**
