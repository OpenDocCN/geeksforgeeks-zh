...
> 原文: [https://www.geeksforgeeks.org/sort-a-permutation-of-first-n-natural-numbers-by-swapping-pairs-satisfying-given-conditions/](https://www.geeksforgeeks.org/sort-a-permutation-of-first-n-natural-numbers-by-swapping-pairs-satisfying-given-conditions/)

给定大小为 `N` 的数组 `p[]` 表示第一个 `N` 自然数的排列，其中 `N` 是一个偶数，任务是通过取一对索引 `a, b` 对数组进行排序，并交换 `p[a]` 和 `p[b]`。打印在每个操作中交换的索引对。
...
*   创建一个数组 `posOfCurrNum[]`，该数组存储出现在 `p[]` 中的数字的位置。
*   使用变量 `i` 在范围 `[1, N]` 中迭代，并将 `posOfCurrNum[p[i]]` 设为 `i`。
*   声明一对 `ans` 的向量，以存储对给定数组 `p[]` 进行排序时要执行的交换。
*   使用变量 `i` 在范围 `[1, N]` 中迭代:
    *   如果 `p[i]` 等于 `i`，那么这意味着当前号码 `i` 已经出现在正确的位置。所以，继续到下一个迭代。
    *   用 `posOfCurrNum[i]` 初始化一个变量 `j` 来存储数字 `i` 的当前位置。
    *   现在，出现了 4 种情况:
        *   情况 1: 如果 `| i - j | * 2` 大于或等于 `n`，那么，`Swap(i, j)` 并将该对存储在 `ans` 中。
        *   情况 2: 如果 `n / 2` 小于或等于 `i - 1`，那么，`Swap(i, 1)` → `Swap(1, j)` → `Swap(i, 1)` 并将这些配对存储在 `ans` 中。
        *   情况 3: 如果 `n / 2` 小于或等于 `n - j`，那么，`Swap(i, n)` → `Swap(j, n)` → `Swap(i, n)` 并将这些对存储在 `ans` 中。
        *   情况 4: 如果 `n / 2` 小于 `j - 1` 且 `n / 2` 小于或等于 `n - i`，则 `Swap(i, n)` → `Swap(n, 1)` → `Swap(1, j)` → `Swap(1, n)` → `Swap(i, n)` 并将这些对存储在 `ans` 中。
*   打印向量 `ans` 的大小。
*   遍历对的向量 `ans`，打印每对。
