...
> 原文: `https://www.geeksforgeeks.org/predict-the-winner-of-the-game-on-the-basis-of-absolute-difference-of-sum-by-selecting-numbers/`

给定一组 `N` 个数字。两个玩家 `X` 和 `Y` 玩一个游戏，每走一步都有一个玩家选择一个号码。一个号码只能选择一次。在所有号码都被选中后，如果 `X` 和 `Y` 收集的号码总和之间的绝对差被 `4` 整除，则玩家 `X` 获胜，否则 `Y` 获胜。
...
*   初始化 `count0`、`count1`、`count2` 和 `count3` 至 `0`。
