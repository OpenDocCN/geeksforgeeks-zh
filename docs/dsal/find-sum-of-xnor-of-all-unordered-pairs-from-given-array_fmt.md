...
> 原文：[https://www.geeksforgeeks.org/find-sum-of-xnor-of-all-unordered-pairs-from-given-array/](https://www.geeksforgeeks.org/find-sum-of-xnor-of-all-unordered-pairs-from-given-array/)
...
> **输出**：`10`
> **解释**：这里，
> `2 XNOR 2 = 3`，`2 XNOR 2 = 3`，`2 XNOR 2 = 3`，`1 XNOR 1 = 1`，`2 XNOR 1 = 0`，`2 XNOR 1 = 0`，`2 XNOR 1 = 0`，`2 XNOR 1 = 0`，`2 XNOR 1 = 0`，`2 XNOR 1 = 0`，因此和为 `3+3+3+1+0+0+0+0+0+0 = 10`。
>
> **输入**：`N = 3`，`arr[] = {1，2，3}`
> **输出**：`3`
...
在 [XNOR](https://www.geeksforgeeks.org/xnor-two-numbers/) 操作后，一个位设置为 `1` 的唯一可能情况是两个位必须是相同的。按照以下步骤解决问题：

*   保持一个大小为 30 的位数组。左起第 0 个位置表示 `2^29` 包含在二进制表示中，左起第 29 个位置表示 `2^0` 包含在二进制表示中。
*   对于每个元素，如果第 `i` 位被设置为 `1`，则增加位阵列的第 `i` 位置。
*   遇到包含 `1` 的 `MSB` 后，计算“0”和“1”位。
*   直到遇到一个 `1` 位，所有的 `0` 位都将被浪费掉。分开存放。
*   最后，假设第一个位置有 `y` 个 1。因此，对于具有 `(1，1)` 位组合的 `i`th 位，存在 `y*(y-1)/2` 对，其给出结果 `1` 为 `XNOR`。同样，假设直到相同的位置有 `x` 个零。因此，对于具有 `(0，0)` 位组合的 `i`th 位，存在 `x*(x-1)/2` 对，其给出的结果 `1` 为 `XNOR`。
*   现在对于前导零的情况，将浪费的比特数乘以 `0` 比特数。对每个位位置做这个并计算答案。

为了更好地理解，考虑数组 `{35，42，27，69}`。这里是数组所有四个元素的二进制表示。

![img](img/d99114b01c9594d74217e6094febc8f5.png)
数组元素的二进制表示。

*   带箭头的一位是该元素的第一位，即 `1`。因此它是该元素的[最高有效位](https://www.geeksforgeeks.org/find-significant-set-bit-number/)。类似地，所有其他元素的 `MSB` 被着色为绿色。
*   出现在 `MSB` 前的零是浪费的位，用红色着色。这些二进制数组的大小各为 30。
*   在比特数组的前 23 个位置中，浪费的比特的数量为四，而 `1` 的数量为零，而 `0` 的数量为零，如图所示。
*   在第 24 个位置，浪费的比特是 `3`，并且 `1` 的数量将是 `1`，因为在元素 69 的第 24 个元素处遇到了 `MSB`。

> **第 23 位**：浪费位数：`4`、`1` 位：`0`、`0` 位：`0`
> **第 24 位**：浪费位数：`3`、`1` 位：`1`、`0` 位：`0`
> **第 25 位**：浪费位数：`1`、`1` 的数量：`0`、`0` 的数量：`1`（因为它发生在 `MSB` 之后）
> **第 26 位**：浪费位的数量：`0`、`1` 的数量：`1`、`0` 的数量：`3`
> **第 27 位**：浪费位的数量：`0`、`1` 的数量：`2`、`0` 的数量：`3`
> **第 29 位**：废位数量：`0`、`1` 号：`3`、`0` 号：`1`
> **第 30 位**：废位数量：`0`、`1` 号：`3`、`0` 号：`0`

*   现在在这些位中的每一个位中，对于任一对位中的 [XNOR](https://www.geeksforgeeks.org/xnor-two-numbers/) 值为 `1`，或者两者都应为 `1`，或者两者都应为 `0`。
...
*   如果两者都为 0，还应考虑浪费位，这发生在任何元素在浪费位的相同位置具有 0（非浪费）的情况下。让浪费的比特成为 `W[i]`。
*   所以总和 = `(n[i]*(n[i]-1)/2)*(2^(30-i-1)) + (m[i]*(m[i]-1)/2)*(2^(30-i-1)) + (w[i]*m[i]*(2^(30-i-1)))` 其中 `0 <= i <= 30`。
*   所以，对于上面的例子，直到第 23 位，一切都是 0。这里列出了从第 24 位开始。
*   `total = ((1*0)/2)*(2^6) + ((0*(-1))/2)*(2^6) + 3*0*(2^6) + ((2*1)/2)*(2^5) + ((1*0)/2)*(2^5) + 1*1*(2^5) + ((1*0)/2)*(2^4) + ((3*2)/2)*(2^4) + 0*3*(2^4) + ((2*1)/2)*(2^3) + ((2*1)/2)*(2^3) + 0*2*(2^3) + ((1*0)/2)*(2^2) + ((3*2)/2)*(2^2) + 0*3*(2^2) + ((3*2)/2)*(2^1)`
...
<script>
    ...
    const findSum = (n, r) => {
        ...
        for (let i = 0; i < 30; i++) {
            ...
            // y*(y-1)/2 pairs for ith bit has (1, 1)
            // bit combo which gives result 1 in XNOR
            let onePairs = Math.floor((y * (y - 1)) / 2);
            ...
            // x*(x-1)/2 pairs for ith bit has (0, 0)
            // bit combo which gives result 1 in XNOR
            let zeroPairs = Math.floor((x * (x - 1)) / 2);
...
```javascript
// `y*(y-1)/2` pairs for `i`th bit has `(1, 1)`
// bit combo which gives result `1` in XNOR
let onePairs = (y * (y - 1)) / 2;
// Adding value of `i`th
// bit number of `(1, 1)` pairs
// times to result
// (`2^(30-i-1)` => `2 ^ 29`, `2 ^ 28` etc.)
result += onePairs * Math.pow(2, 30 - i - 1);

// `x*(x-1)/2` pairs for `i`th bit has `(0, 0)`
// bit combo which gives result `1` in XNOR
let zeroPairs = (x * (x - 1)) / 2;

// Adding value of `i`th bit
// number of `(0, 0)` pairs
// times to result
// (`2^(30-i-1)` => `2 ^ 29`, `2 ^ 28` etc.)
result += zeroPairs * Math.pow(2, 30 - i - 1);
