...
> 让位于曲线上的点为 `(x1，y1)`，任何一个一般点都可以用 `(h，k)` 表示。
> 
> 所以，直线的斜率可以写成
> 
> 斜率=`(k-y1) / (h-x1)`
...
垂线距离原点的长度为`p`且该垂线与 x 轴的正方向成角度`α`的直线方程为:
...
> 假设直线 AB 是这样的，从原点 O 到直线的垂直 OQ 的长度是 `p`，并且 `< XOQ = α`。
> 
> 从图表中，使用截取形式，我们得到
> 
> AB 线的方程是
...
### 实施例 1。写出垂线距离原点的长度为 5 个单位的直线方程，该垂线与 x 轴的正方向成 45°。

### 解决方案

> 所以，基本上我们得到了 `p=5` 和 `α= 45°`的值
> 
> 把这些值放在上面的等式中，
> 
> `x /√2 + y√2 = 5`
...
### 例 2。写出直线方程，该直线的垂线距离原点的长度为 1 个单位，该垂线与 x 轴的正方向成 60°角。

### 解决方案

> 所以，基本上我们得到了 `p=1` 和 `α= 60°`的值
> 
> 把这些值放在上面的等式中，
> 
> `x * (1/2) + y * (√3/2) = 1`
...
![](img/34e03198b687383043523242c847325a.png)