...
> 原文：[https://www.geeksforgeeks.org/minimum-removal-make-palindrome-permutation/](https://www.geeksforgeeks.org/minimum-removal-make-palindrome-permutation/)
...
一种高效的方法是注意到我们不需要打印最少的字符，只需要最少的数字。所以，一个有效的想法是关键：回文可以有两种类型，偶数长度和奇数长度回文。我们可以推导出这样一个事实：一个偶数长度的回文必须让每个字符出现偶数次（即每个字符出现的频率是偶数）。同样，奇数回文必须让每个字符出现偶数次，只有一个字符出现奇数次。
