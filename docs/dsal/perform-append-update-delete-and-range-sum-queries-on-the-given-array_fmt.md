...
> 原文: [https://www.geeksforgeeks.org/perform-append-update-delete-and-range-sum-queries-on-the-given-array/](https://www.geeksforgeeks.org/perform-append-update-delete-and-range-sum-queries-on-the-given-array/)
...
> **输入：** `arr[] = {1, 2, 5, 3, 4}`，`Q[][] = {{4, 2, 4}, {3, 3, 0}, {1, 6, 0}, {4, 3, 5}}`
> **输出：**
> 10
> 13
...
> 第二次查询
>
> **输入：** `arr[] = {1, 2, 3, 4, 5}`，`Q[][] = {{4, 1, 5}, {3, 3, 0}, {1, 6, 0}, {4, 3, 5}, {2, 4, 10}, {4, 1, 5}}`
> **输出：**
> 15
> 15
> 23
...
解决这个问题的朴素方法是直接在给定的数组上执行查询，该方法的时间复杂度为 `O(Q * N)`。

## 高效方法

*   因为有一些数据结构，如[段树](https://www.geeksforgeeks.org/segment-tree-set-1-sum-of-given-range/)或[BIT（Fenwick树）](https://www.geeksforgeeks.org/binary-indexed-tree-or-fenwick-tree-2/)，为每个查询提供 `O(logN)` 中的点更新和范围总和。
*   帖子用一棵Fenwick树来做同样的事情，所以强烈建议在Fenwick树中进行[点更新](https://www.geeksforgeeks.org/binary-indexed-tree-or-fenwick-tree-2/)。
*   但主要问题是执行删除操作（类型-3 查询）。删除后需要换挡，最差情况下会再次导致 `O(Q * N)`。
*   可以使用一个技巧，在删除一个元素后，只需更新 `A[X] = 0` 处的值，并将 `X` 之后的索引映射到 `X + X` 之前删除的元素数量。
*   为了找到在 `X` 之前删除的元素数量，将使用另一个Fenwick树（在实现中为 `idx`），并在执行删除操作的索引处继续添加 `1`。
