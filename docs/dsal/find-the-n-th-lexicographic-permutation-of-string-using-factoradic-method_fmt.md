...
> 原文：[https://www.geeksforgeeks.org/find-the-n-th-lexicographic-permutation-of-string-using-factoradic-method/](https://www.geeksforgeeks.org/find-the-n-th-lexicographic-permutation-of-string-using-factoradic-method/)

给定带有唯一字符的字符串`str`和一个数字`N`，任务是使用因式分解法找到字符串的第`N`个字典排列。
...
2.  用`i`除以字符串的长度，每次将剩余部分存储在堆栈中，同时将`N`的值更新为`N/i`。
3.  每一步计算出的余数就是因式分解数。因此，在计算最终的`factoradic`表示后，开始在结果字符串中追加位置上的元素。
4.  每次迭代时从堆栈中移除元素。
5.  重复以上三个步骤，直到堆栈变空。

让我们用一个例子来理解这个方法。让字符串为`"abcde"`，`N`为`11`。然后：
...
**注：** 在[这篇文章](https://www.geeksforgeeks.org/lexicographically-n-th-permutation-string/)中讨论了用重复字符寻找第`N`个排列的方法。