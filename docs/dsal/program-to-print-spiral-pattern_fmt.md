...
> 原文: [https://www.geeksforgeeks.org/program-to-print-spiral-pattern/](https://www.geeksforgeeks.org/program-to-print-spiral-pattern/)
...
**进场:**
1.  创建一个 `n` 大小的 2D 数组。
2.  将数组的边界存储在边界变量 `boundary` 中。最初它将是 `n-1`，此后它在每次旋转后都会改变。
3.  将螺旋打印的左尺寸存储在可变 `sizeLeft` 中。最初它将是 `n-1`，此后每旋转 2 圈，它将减少 1。
4.  创建一个标志 `flag` 来确定 2 次旋转，因为每 2 次旋转后，`sizeLeft` 将减小。
