...
> 原文：[https://www.geeksforgeeks.org/string-after-processing-backspace-characters/](https://www.geeksforgeeks.org/string-after-processing-backspace-characters/)
...
**方法：** 用双端队列解决这个问题的简单方法如下：

*   遍历字符串 `S`。
*   如果找到除`#`外的任何字符，请将它们推入双端队列的后端。
*   如果找到字符`#`，则从双端队列的后端弹出一个字符。
*   最后，将所有元素从双端队列中弹出以构成新字符串。
