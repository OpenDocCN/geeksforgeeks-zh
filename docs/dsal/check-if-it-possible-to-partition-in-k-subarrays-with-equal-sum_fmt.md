...
2.  从第 0 <sup>th</sup> 索引开始，开始比较前缀和，一旦等于和，就意味着一个子数组的结束(假设在索引 `j` 处)。
3.  从 `(j + 1)` <sup>th</sup> 个索引中，找到另一个合适的 `i`，其和 (`prefix_sum[i] – prefix_sum[j]`) 等于所需和。该过程一直进行到找到所需数量的相邻子阵列，即 `K`。
4.  如果在任何索引处，任何子阵列总和大于所需总和，则从循环中断开，因为每个子阵列应包含相同的总和。
...
## 问题描述
给定一个数组，判断是否可以将其分割成K个连续的子数组，且每个子数组的和相等。
...
**辅助空间:** `O(N)`，其中 `N` 为数组的大小。

我们可以进一步将空间复杂度降低到 `O(1)`。
因为数组将被分成 `k` 个子数组，并且所有子数组都是连续的。所以思路是计算子数组的个数，其和等于整个数组的和除以 `k`。
如果个数 `== k` 打印 `Yes` 否则打印 `No`。
