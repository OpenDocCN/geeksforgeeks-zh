...
> 原文：[https://www.geeksforgeeks.org/sum-of-elements-in-given-range-from-array-formed-by-infinitely-concatenating-given-array/](https://www.geeksforgeeks.org/sum-of-elements-in-given-range-from-array-formed-by-infinitely-concatenating-given-array/)

给定一个由 `N` 个正整数和两个正整数 `L` 和 `R` 组成的数组 `arr[]`（基于 1 的索引），如果给定的数组 `arr[]` 连接到自身无限次，那么任务就是找出该范围内的数组元素之和 `[L，R]`。
...
**天真方法：** 解决给定问题的最简单方法是使用变量 `i` 迭代范围 `[L，R]`，并将 `arr[i % N]` 的值加到每个指标的 `sum` 上。迭代完成后，打印 `sum` 的值作为结果和。
...
*   初始化一个数组，说 `prefix[]` 大小为 `(N + 1)`，所有元素为 `0`。
*   使用变量 `i` 遍历数组 `arr[]`，并将 `prefix[i]` 更新为 `prefix[i–1]` 和 `arr[i–1]` 之和。
*   现在，范围 `[L，R]` 内的元素之和由下式给出：

> 范围内的元素之和 `[1，R]` – 范围内的元素之和 `[1，L–1]`。

*   初始化一个变量，说 `leftSum` 为 `((L–1)/N) * prefix[N] + prefix[(L-1)% N]` 存储范围 `[1，L-1]` 内的元素之和。
*   同样，将另一个变量 `rightSum` 初始化为 `(R/N) * prefix[N] + prefix[R % N]` 来存储范围 `[1，R]` 内的元素之和。
*   完成上述步骤后，打印 `(rightSum – leftSum)` 的值，作为给定范围内 `[L，R]` 元素的合成总和。
