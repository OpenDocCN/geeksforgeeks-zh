...
> 原文：[https://www.geeksforgeeks.org/number-of-cells-in-a-matrix-that-satisfy-the-given-condition/](https://www.geeksforgeeks.org/number-of-cells-in-a-matrix-that-satisfy-the-given-condition/)
...
**天真的做法：** 提到镜像只有在它的右边和下面的单元格都是空的情况下，才能放在空单元格中。最简单的方法是单独检查每个单元格是否满足条件。这种方式需要 `O(n^3)` 时间。

**高效方法：** 创建两个布尔数组 `row[][]` 和 `col[][]`，其中 `row[i][j]` 将存储如果第 `i` 行在第 `j` 列（包括它）之后的所有单元格都是 `1`，否则它将存储 `false`。`col[i][j]` 同理。现在，对于每个单元格 `mat[i][j]`，如果 `row[i][j]` 和 `col[i][j]` 都为真，则当前单元格有效，否则无效。计算所有这样的有效单元格，并在最后打印计数。
