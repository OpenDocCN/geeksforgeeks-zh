...
> 原文: [https://www.geeksforgeeks.org/bin-packing-problem-minimize-number-of-used-bins/](https://www.geeksforgeeks.org/bin-packing-problem-minimize-number-of-used-bins/)
...
`Next Fit` 是 2 个近似值，即该算法使用的箱数以最优的两倍为界。考虑任何两个相邻的箱子。这两个箱子里的物品总和必须是 `>c`；否则，`Next Fit` 会将第二个箱的所有项目放入第一个箱。所有其他垃圾箱也是如此。因此，最多浪费一半的空间，因此如果 `M` 是最优的，`Next Fit` 最多使用 `2M` 箱。
...
上述首次拟合的实现需要 `O(n^2)` 个时间，但是首次拟合可以使用自平衡二分搜索法树在 `O(n Log n)` 个时间内实现。
...
最差拟合也可以使用自平衡二分搜索树在 `O(n^2)` 时间内实现。
如果 `M` 是最佳箱数，那么最佳拟合使用的箱数不会超过 `2M-2`。因此，就箱数上限而言，最差拟合与首次拟合相同。

## 离线算法

在离线版本中，我们有所有的物品。不幸的是，离线版本也是 NP 完全的，但是我们有一个更好的近似算法。如果最佳值为 `M`，那么首次拟合递减最多使用 `(4M + 1)/3` 个箱子。

### 首次拟合递减

在线算法的一个问题是打包大物品很困难，尤其是如果它们出现在序列的后期。我们可以通过对输入序列进行排序，并将大物品放在第一位来避免这种情况。通过排序，我们得到首次拟合递减和最佳拟合递减，作为在线首次拟合和最佳拟合的离线模拟。
...
首次拟合递减为样本输入产生最佳结果，因为物品是先排序的。
首次拟合递减也可以使用自平衡二分搜索树在 `O(n Log n)` 时间内实现。
