...
> 原文: [https://www.geeksforgeeks.org/find-the-path-from-root-to-the-given-nodes-of-a-tree-for-multiple-queries/](https://www.geeksforgeeks.org/find-the-path-from-root-to-the-given-nodes-of-a-tree-for-multiple-queries/)

给定一棵树，其 `N` 个顶点的编号从 `0` 到 `N–1`（第 0 个节点是根节点）。同样，给定包含树中节点的 `q` 查询。任务是为多个查询找到从根节点到给定节点的路径。
...
从任意根顶点到任意顶点 `i` 的路径是从根顶点到其父顶点的路径，后跟父顶点本身。这可以通过修改树的[广度优先遍历](https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/)来实现。在路径列表中，对于每个未访问的顶点，将其父顶点的路径副本添加到列表中，然后将父顶点添加到列表中。
