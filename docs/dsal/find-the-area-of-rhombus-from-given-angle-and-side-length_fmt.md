...
> 原文: [https://www.geeksforgeeks.org/find-the-area-of-rhombus-from-given-angle-and-side-length/](https://www.geeksforgeeks.org/find-the-area-of-rhombus-from-given-angle-and-side-length/)

给定两个整数 `A` 和 `X`，分别表示一个[菱形的边长](https://en.wikipedia.org/wiki/Rhombus)和一个角度，任务是求菱形的面积。

> 一个**菱形**是四边等长的四边形，其中相对的两边平行，相对的角度相等。
...
**方法:** 对于边长为 `a` 且角度为 `x` 的菱形 `ABCD`，三角形 `ABD` 的面积可以使用三角形的[边-角-边特性](https://www.geeksforgeeks.org/area-of-triangle-using-side-angle-side-length-of-two-sides-and-the-included-angle/)通过以下公式计算:

> 三角形面积 `ABD = 1/2(a^2)sin x`
> 菱形面积 `ABCD` 将是 `ABD` 三角形面积的两倍。
>
> 因此，菱形面积 `ABCD = (a^2)sin x`
