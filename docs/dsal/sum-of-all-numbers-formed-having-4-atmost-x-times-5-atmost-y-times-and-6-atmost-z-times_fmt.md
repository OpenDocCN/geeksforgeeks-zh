...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-numbers-formed-having-4-atmost-x-times-5-atmost-y-times-and-6-atmost-z-times/](https://www.geeksforgeeks.org/sum-of-all-numbers-formed-having-4-atmost-x-times-5-atmost-y-times-and-6-atmost-z-times/)
...
*   需要所有 `i < x`，`j < y`，`k < z` 的精确 `i` 个 `4`，`j` 个 `5` 和 `k` 个 `6` 的数字来获得所需的总和。
*   因此，DP 数组 `exactnum[i][j][k]` 将存储具有精确的 `i` 个 `4`、`j` 个 `5` 和 `k` 个 `6` 的数字的精确计数。
*   如果 `exactnum[i-1][j][k]`、`exactnum[i][j-1][k]` 和 `exactnum[i][j][k-1]` 已经已知，那么可以观察到这些的和就是需要的答案，除非 `exactnum[i-1][j][k]`、`exactnum[i][j-1][k]` 或 `exactnum[i][j][k-1]` 为 0，既然如此，就跳过它吧。
*   `exactsum[i][j][k]` 以与 `exactnum` 相同的方式存储具有 `i` 个 `4`、`j` 个 `5` 和 `k` 个 `6` 的精确数字的和。
