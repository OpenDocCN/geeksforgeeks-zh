...
> 原文: [https://www.geeksforgeeks.org/ackermanns-function-using-dynamic-programming/](https://www.geeksforgeeks.org/ackermanns-function-using-dynamic-programming/)
...
*   `A(1, 0) = A(0, 1)` — (参考方程 (3))
    因为 `A(0, 1) = 2`，所以 **`A(1, 0) = 2`**

*   `A(1, 1) = A(0, A(1, 0))` — (参考方程 (1))
    `= A(0, 2)` — (参考方程 (4))
    `= 3` — (参考方程 (2))
    所以，**`A(1, 1) = 3`**

*   `A(1, 2) = A(0, A(1, 1))` — (参考方程 (1))
    `= A(0, 3)` — (参考方程 (5))
    `= 4` — (参考方程 (2))
    所以，**`A(1, 2) = 4`**
...
*   在下一步中，详细描述方法，并获得一个**通用**公式，在程序中使用时具有逻辑性。

先来解 **`A(2, 2)`**，前面有个理论

> **`A(2, 2)`** = `A(1, A(2, 1))` — (参考方程 (1))
> `A(2, 1) = 5`
> **`A(2, 2)`** = `A(1, 5)`
...
> `A(1, 4) = A(0, A(1, 3)) = A(0, 5) = 6` — (因为 `A(1, 3) = 5`)
