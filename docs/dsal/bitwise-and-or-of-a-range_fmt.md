...
给定两个非负长整数 `x` 和 `y`，且满足 `x <= y`，任务是找出从 `x` 到 `y` 所有整数的按位与（AND），即我们需要计算 `x & (x+1) & … & (y-1) & y` 的值。
...
一个**简单的解决方案**是遍历从 `x` 到 `y` 的所有数字，并对范围内的所有数字进行逐位与运算。

一个**高效的解决方案**是遵循以下步骤：
1.  找出两个数字中最高有效位（MSB）的位置。
2.  如果 MSB 的位置不同，则结果为 0。
3.  如果位置相同，令位置为 `msb_p`。
    a) 我们在结果上加 `2^msb_p`。
    b) 我们从 `x` 和 `y` 中减去 `2^msb_p`。
