...
这个想法是使用[哈希映射](https://www.geeksforgeeks.org/hashing-data-structure/)检查元素是否在`O(1)`时间出现在哈希映射中。 然后，遍历数组的元素，并针对每个元素检查数组中是否存在`arr[i] - X`或`arr[i] + X`。 如果是，则将此类元素的计数加 1。
