...
> 原文：[https://www.geeksforgeeks.org/smallest-element-greater-than-x-not-present-in-the-array/](https://www.geeksforgeeks.org/smallest-element-greater-than-x-not-present-in-the-array/)
...
**方法：** 高效的解决方案基于[二分搜索法](https://www.geeksforgeeks.org/binary-search/)。首先对数组进行排序。将 `low` 设为 `0`，`high` 为 `N-1`。并搜索元素 `X + 1`。如果 `mid` 的元素（即 `(low + high) / 2`）小于或等于搜索元素，则使 `low` 为 `mid + 1`。否则，将 `high` 设为 `mid - 1`。如果 `mid` 的元素等于搜索元素，那么将搜索元素增加 1，使 `high` 为 `N-1`。
