...
> 原文: [https://www.geeksforgeeks.org/count-number-of-triangles-cut-by-the-given-horizontal-and-vertical-line-segments/](https://www.geeksforgeeks.org/count-number-of-triangles-cut-by-the-given-horizontal-and-vertical-line-segments/)

给定由形式为 `{x1，y1，x2，y2，x3，y3}` 的 `N` 个三角形组成的三角形数组 `三角形[][]`，以及由形式为 `"X = X"` 或 `"Y = Y"` 的 `M` 条水平线和垂直线组成的表示线段方程式的数组 `切割[]`。任务是打印每个切口相交的三角形数量，使三角形的左右部分的面积大于零。
...
> **输出:** `1 1 0`
> **解释:**
> `X = 2` 处的切割将具有顶点 `(0，2)`、`(2，9)` 和 `(8，5)` 的第一个三角形分开
>
...
1.  创建一个结构来存储每个三角形的最大和最小 `X` 和 `Y` 坐标。
2.  遍历数组 `切割` 超范围 `[0，M–1]`。
3.  对于每次切割，用 `0` 初始化一个计数器 `计数`，以存储当前切割的答案，并开始从 `j = 0 到 N–1` 遍历 `三角形[][]` 数组。
4.  检查每个三角形，`切口【I】` 的形式为 `X=x` 即垂直切口，`x` 严格位于 `I<sup>th</sup>` 三角形的最大和最小 `X` 坐标之间，递增计数器 `计数` 否则继续检查每个切口的其他三角形。
...
class GFG{
    ...
    public static void Main(String[] args)
    {
        ...
        // Given coordinates of triangles
        int[][] Triangle = { new int[]{ 0, 2, 2, 9, 8, 5 },
                             new int[]{ 5, 0, 6, 3, 7, 0 } };
...
// Function to print the number of
// triangles cut by each line segment
`static void TriangleCuts(int[,] Triangle, String []Cuts, int N, int M, int COL)`
...
```
{
    ...
    for(int i = 0; i < M; i++)
    {
        ...
        // Extract value from the line
        // segment String
        int CutVal = StringtoInt(Cut.Substring(2, Cut.Length - 2));
