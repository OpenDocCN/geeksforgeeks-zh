...
*   创建一个[静态变量](https://www.geeksforgeeks.org/static-variables-in-c/) `input` 来维持字符串的状态。
*   检查是否第一次提取代币，然后用其初始化 `input`。
*   如果 `input` 为空，并且提取了所有令牌，则返回空。
*   在此步骤中，开始提取代币并将其存储在[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `result[]` 中。
*   现在，迭代一个循环，直到出现空值或分隔符，然后通过包含 `'\0'` 返回结果。
