...
> 原文: [https://www.geeksforgeeks.org/check-if-a-given-string-is-a-valid-hexadecimal-color-code-or-not/](https://www.geeksforgeeks.org/check-if-a-given-string-is-a-valid-hexadecimal-color-code-or-not/)
...
1.  检查字符串 `str` 是否存在以下情况:
    *   如果第一个字符不是 `'#'`，则返回 `false`。
    *   如果长度不是 `3` 或 `6`。否则，返回 `false`。
    *   现在，检查除第一个字符之外的所有字符，即 `0-9`、`A-F` 或 `a-f`。
2.  如果满足上述所有条件，则返回 `true`。
3.  根据以上观察打印答案。
