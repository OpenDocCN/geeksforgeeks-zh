...
> 原文：[https://www.geeksforgeeks.org/nth-term-of-given-recurrence-relation-having-each-term-equal-to-the-product-of-previous-k-terms/](https://www.geeksforgeeks.org/nth-term-of-given-recurrence-relation-having-each-term-equal-to-the-product-of-previous-k-terms/)

给定两个正整数 `N` 和 `K` 以及一个由 `K` 个正整数组成的数组 `F[]`。递归关系的第 `N` 项由下式给出：

> `F_N = F_{N-1} * F_{N-2} * F_{N-3} * …… * F_{N-K}`

任务是找到给定递归关系的第 `N` 项。由于第 `N` 项可能非常大，打印第 `N` 项模 `10^9 + 7`。
...
想法是使用递归关系生成给定序列的所有 `N` 项，并打印作为所需答案获得的第 `N` 项。
...
想法是使用双端队列（deque）数据结构，使用最后一个 `K` 术语查找下一个术语。以下是步骤：

*   初始化一个空的 deque，记为 `dq`。
*   计算第一个 `K` 项的乘积，由于它等于循环关系的第 `(K + 1)` 项，将其插入到 `dq` 的末尾。
*   迭代范围 `[K+2, N]`，并遵循以下步骤：
    *   让 `dq` 的最后一个元素为 `L`，`dq` 的前一个元素为 `F`。
    *   现在，使用公式 `第 i 项 = (L * L) / F` 计算第 `i` 项。
    *   因为 `L` 是从 `(i-1-K)` 到 `(i-2)` 元素的乘积。因此，要找到第 `i` 项，选择 `(i-K)` 到 `(i-1)` 的元素乘积，将第 `(i-1)` 项（即 `L`）乘以 `(i-1-K)` 到 `(i-2)` 的元素乘积，即可得到乘积。
    *   现在，将本乘积 `(L * L)` 除以第 `(i-1-K)` 项，在本例中为 `F`。
    *   现在，将第 `i` 项插入到 `dq` 的后面。
    *   从 `dq` 前面弹出一个元素。
*   完成以上步骤后，打印 `dq` 的最后一个元素。
