...
> 原文: [https://www.geeksforgeeks.org/smallest-possible-integer-k-such-that-ceil-of-each-array-element-when-divided-by-k-is-at-most-m/](https://www.geeksforgeeks.org/smallest-possible-integer-k-such-that-ceil-of-each-array-element-when-divided-by-k-is-at-most-m/)

给定一个由 `N` 个正整数和一个正整数 `M` 组成的数组 `arr[]`，任务是找到最小可能的整数 `K`，使得 `ceil(arr[0]/K) + ceil(arr[1]/K) + ... + ceil(arr[N-1]/K)` 最多为 `M`。
...
思路是用二分搜索法。从数组 `arr[]` 中设置 `low` 值为 `1` 和 `high` 值为最大值，应用二分搜索法求小于等于 `M` 的 `K` 值。按照以下步骤解决问题:

*   初始化变量，设 `low = 1` 和 `high` 作为最大阵元。
*   重复 `while` 循环直到 `high - low > 1` 并执行以下任务:
    *   通过 `mid = (low + high) / 2` 更新 `mid` 的值。
    *   遍历数组 `arr[]`，通过假设 `mid` 为 `K` 求 `ceil(arr[i]/K)` 之和。
    *   如果 `sum` 大于 `M`，则将 `high` 的值更新为 `high = mid`。否则，将 `low` 的值更新为 `low = mid + 1`。
*   完成以上步骤后，如果 `sum` 最多为 `M`，打印 `low` 值。否则，打印 `high` 值。
