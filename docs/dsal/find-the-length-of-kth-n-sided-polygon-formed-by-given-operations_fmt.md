...
> 原文: [https://www.geeksforgeeks.org/find-the-length-of-kth-n-sided-polygon-formed-by-given-operations/](https://www.geeksforgeeks.org/find-the-length-of-kth-n-sided-polygon-formed-by-given-operations/)

给定一个整数 `L`，代表一个 `N` 边正多边形的边长和一个整数 `K`，任务是通过连接边的中点找到 `(K–1)`<sup>第</sup>边正多边形内部形成的 `K`<sup>第</sup>`N` 边正多边形的边长。
...
*   假设，`θ` 代表 `N` 边多边形的内角，对于内部形成的所有多边形来说，该内角都是相同的，即 `θ = \frac{(N - 2)*180}{N}`。
*   通过连接边的中点在内部形成的第一个多边形的边的长度可以使用公式 `\frac{2*L*sin(\frac{\theta}{2})}{2}` 来计算。
*   在 `(K–1)`<sup>多边形</sup>内部形成的 `K`<sup>第</sup>多边形的边的长度以及连接 `(K–1)`<sup>第</sup>多边形的边的中点为 `L*Sin^{K - 1}(\frac{\theta}{2})`。

按照以下步骤解决问题:

*   求 `N` 边正多边形的[内角，并存储在一个变量中，比如以弧度表示的 `angle`。](https://www.geeksforgeeks.org/program-to-find-the-interior-and-exterior-angle-of-a-regular-polygon/)
*   用上面讨论的公式 `L*Sin^{K - 1}(\frac{\theta}{2})` 计算 `K`<sup>第</sup>`N` 边正多边形的边长后打印边长。
