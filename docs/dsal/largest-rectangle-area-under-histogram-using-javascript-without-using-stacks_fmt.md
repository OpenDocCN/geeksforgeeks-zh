...
> 原文：`https://www.geeksforgeeks.org/largest-rectangle-area-under-histogram-using-javascript-without-using-stacks/`
...
*   对于每个 `x` 条，我们用 `x` 作为矩形中最小的条来计算面积。我们将在 `x` 的左侧找到第一个较小（小于 `x`）条的索引，在 `x` 的右侧找到第一个较小条的索引。让我们分别称这些指数为 `left_index` 和 `right_index`。
