...
假设当某个节点被选为 `dfs` 的起始节点时，树中的反转总数为 `R`，则 **如果要从节点 `i` 到达其他每个节点，则需要将路径 `i` 的所有后边反转到起始节点，并且还需要将节点 `i` 以外的所有其他后边反转到起始节点路径。** 第一部分将是（节点 `i` 与起始节点的距离 – 节点 `i` 的后边数），因为我们要反转从节点 `i` 到起始节点的路径中的边，它将是总边（即距离）减去从节点到节点 `i` 的起始节点（即节点 `i` 的后边计数）。第二部分将是（树 `R` 的总边反转或总后边 – 节点 `i` 的后边计数）。在每个节点上计算了该值之后，我们将选择其中的最小值作为结果。
