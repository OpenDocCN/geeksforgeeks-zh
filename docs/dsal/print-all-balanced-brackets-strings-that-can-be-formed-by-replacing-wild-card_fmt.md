...
> 原文：[https://www.geeksforgeeks.org/print-all-balanced-brackets-strings-that-can-be-formed-by-replacing-wild-card/](https://www.geeksforgeeks.org/print-all-balanced-brackets-strings-that-can-be-formed-by-replacing-wild-card/)

给定包含字符`“？”`的[字符串](https://www.geeksforgeeks.org/string-class-in-java/)和`'('`、`')'`，任务是替换`'？'`字符为`"("`或`")"`，[打印所有包含](https://www.geeksforgeeks.org/write-a-c-program-to-print-all-permutations-of-a-given-string/)[平衡括号](https://www.geeksforgeeks.org/check-for-balanced-parentheses-in-an-expression/)的字符串。
...
给定的问题可以使用[递归](https://www.geeksforgeeks.org/recursion/)和[回溯](https://www.geeksforgeeks.org/backtracking-algorithms/)来解决。想法是用每一个`“？”`字符替换为`')'`然后递归调用下一个索引，回溯后改为`'('`然后递归调用下一个索引，回溯后改回`'？'`。按照以下步骤解决问题：
...
*   将字符数组 `ch` 和`索引` 0 作为参数传递给递归函数，并在每次递归调用时执行以下操作：
    *   如果索引等于字符数组的长度：
        ...
        *   如果上述条件成立，则打印字符串
    *   如果当前字符 `ch[index]`是`'('`或`')'`，则对下一个索引进行递归调用
    *   如果当前字符 `ch[index]`是`“？”`然后：
        *   将其替换为`"("`并对下一个索引进行递归调用
        *   替换为 `')'` ，并对下一个索引进行递归调用
        *   改回`'？'`从功能返回之前
