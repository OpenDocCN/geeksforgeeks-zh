...
> 原文: [https://www.geeksforgeeks.org/remove-the-first-and-last-occurrence-of-a-given-character-from-a-string/](https://www.geeksforgeeks.org/remove-the-first-and-last-occurrence-of-a-given-character-from-a-string/)

给定一个字符 `C` 和一个字符串 `S`，任务是从字符串 `S` 中移除字符 `C` 的第一个和最后一个出现。

**示例:**

> **输入:** `s = "geekforgeeks"`，`C = 'e'`
> **输出:** `geekforgaks`
> **解释:** 字符串中第一个和最后一个 `'e'` 被移除。
>
> **输入:** `S = "helloWorld"`，`C = 'l'`
> **输出:** `heloWord`
...
*   [`join()`](https://www.geeksforgeeks.org/join-function-python/) 列表并打印。
