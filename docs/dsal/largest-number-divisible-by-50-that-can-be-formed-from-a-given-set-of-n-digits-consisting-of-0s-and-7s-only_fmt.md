...
> 原文: [https://www.geeksforgeeks.org/largest-number-divisible-by-50-that-can-be-formed-from-a-given-set-of-n-digits-consisting-of-0s-and-7s-only/](https://www.geeksforgeeks.org/largest-number-divisible-by-50-that-can-be-formed-from-a-given-set-of-n-digits-consisting-of-0s-and-7s-only/)

给定一个由 `N` 个整数组成的数组 `arr[]`，这些整数或者是 `0` 或者是 `7`，任务是找到可以使用数组元素形成的最大数，这样它就可以被 `50` 整除。
...
2.  计算 5 的最近因子与阵列中存在的 `7` 的计数（因为 `35` 是最小因子 `5`，仅使用 7s 即可获得）。
3.  显示 `7` 的计算数。
4.  在上面的数字后面附加零。
