...
> 原文: [https://www.geeksforgeeks.org/find-the-count-of-smith-brothers-pairs-in-a-given-array/](https://www.geeksforgeeks.org/find-the-count-of-smith-brothers-pairs-in-a-given-array/)
...
天真的方法是使用嵌套循环迭代每个可能的对，并检查这些数字是否是史密斯兄弟对。按照以下步骤解决问题:

1.  初始化一个变量 `count` 到 `0`，该变量存储数组中史密斯兄弟对的数量。
2.  [从 `0` 到 `N-1` 遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `A`。对于每个当前指标 `i`，执行以下操作:
    1.  [从 `i+1` 到 `N-1` 遍历](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)数组。对于每个当前指标 `j`，执行以下操作:
        1.  检查 `A[i]` 和 `A[j]` 是否为[史密斯数](https://www.geeksforgeeks.org/smith-number/)。
        2.  如果它们是史密斯数，并且它们的差是 1，则递增 `count`。
3.  最后，返回 `count`。
...
由于只有连续的史密斯数才能形成史密斯兄弟对，最佳解决方案是对数组进行排序，然后，只检查连续的元素。
按照以下步骤解决问题。

1.  将变量`count`初始化为`0`。
2.  整理数组`A`。
3.  从`0`到`N-2`遍历`A`，对于每个当前索引`i`，执行以下操作:
    1.  检查`A[i]`和`A[i+1]`是否都是史密斯数。
    2.  如果都是史密斯数，递增`count`。
4.  最后，返回`count`。
