...
> 原文: [https://www.geeksforgeeks.org/find-x-such-that-elements-at-only-alternate-indices-in-given-array-are-divisible-by-x/](https://www.geeksforgeeks.org/find-x-such-that-elements-at-only-alternate-indices-in-given-array-are-divisible-by-x/)

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是找到一个整数 `X`，使得可被 `X` 整除的整数和不可被 `X` 整除的整数在数组中相互替换。如果没有这样的值，打印 `-1`。

## 示例

> **输入:** `arr[] = {6, 5, 9, 10, 12, 15}`
> **输出:** `5`
> **解释:** `x = 5` 除奇数索引处的所有元素（基于 0 的索引）但不除偶数索引处的元素，因此答案是 `5`。
>
> **输入:** `{10, 20, 40, 30}`
> **输出:** `-1`
...
该方法是基于 [GCD](http://www.geeksforgeeks.org/basic-and-extended-euclidean-algorithms/) 的，因为一组交替元素，无论是奇数索引还是偶数索引，都应该完全被一个整数整除，唯一能整除所有数字的数字就是这组元素的 GCD。一个集合的 GCD 不应该等于另一个集合的 GCD，那么只有那个 GCD 才是答案。按照以下步骤解决此问题:

*   遍历数组 `arr[]`，分别计算奇数索引 `(gcd1)` 处元素和偶数索引 `(gcd2)` 处元素的 GCD。
