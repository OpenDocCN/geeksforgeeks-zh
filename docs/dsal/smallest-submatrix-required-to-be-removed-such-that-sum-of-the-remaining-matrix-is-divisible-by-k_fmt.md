...
> 原文：[https://www.geeksforgeeks.org/smallest-submatrix-required-to-be-removed-such-that-sum-of-the-remaining-matrix-is-divisible-by-k/](https://www.geeksforgeeks.org/smallest-submatrix-required-to-be-removed-such-that-sum-of-the-remaining-matrix-is-divisible-by-k/)
...
*   初始化两个变量，说 `left` 和 `right` 分别存储每个子矩阵的**最左边一列**和**最右边一列**。
*   初始化一个[数组](https://www.geeksforgeeks.org/arrays-in-c-cpp/)，比如说 `prefixRowSum[N]`，其中 `prefixRowSum[i]` 存储子矩阵的所有元素的和，子矩阵的最左边的元素是 `(0, left)`，最右边的元素是 `(i, right)`。
*   迭代 `left` 和 `right` 的所有可能值，找到需要删除的最小子阵列的[长度，使 `PrefixRowSum[]` 剩余元素之和等于 `(S % K)`](https://www.geeksforgeeks.org/length-of-smallest-subarray-to-be-removed-to-make-sum-of-remaining-elements-divisible-by-k/) 并更新 `min_area`。
*   最后，打印 `min_area` 的值。
...
// required to be deleted to make the sum
