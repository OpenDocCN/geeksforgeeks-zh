...
> 原文: [https://www.geeksforgeeks.org/compute-abc-such-that-ac-bc-can-be-beyond-range/](https://www.geeksforgeeks.org/compute-abc-such-that-ac-bc-can-be-beyond-range/)

给定三个数字 `a`、`b` 和 `c`，使得 `a`、`b` 和 `c` 最多为 10<sup>16</sup>。任务是计算 `(a*b)%c`。
一个简单的做 `((a % c) * (b % c)) % c` 的解决方案在这里行不通。这里的问题是 `a` 和 `b` 可能很大，所以当我们计算 `(a % c) * (b % c)` 时，它超出了 `long long int` 可以容纳的范围，因此会发生溢出。例如，如果 `a = (10^12 + 7)`，`b = (10^13 + 5)`，`c = (10^15 + 3)`。
