...
> 原文：[https://www.geeksforgeeks.org/how-to-validate-ifsc-code-using-regular-expression/](https://www.geeksforgeeks.org/how-to-validate-ifsc-code-using-regular-expression/)

给定字符串 `str`，任务是使用[正则表达式](https://www.geeksforgeeks.org/write-regular-expressions/)检查给定字符串是否为有效的IFSC（印度金融系统）代码。
...
> **输出：** 真
> **说明：**
> 给定的字符串满足上述所有条件。因此，这是一个有效的IFSC（印度金融系统）代码。
> **输入：** `str = "SBIN0125"`；
> **输出：** 假
> **说明：**
> 给定字符串有8个字符。因此，它不是有效的IFSC（印度金融系统）代码。
> **输入：** `str = "1234sbin012"`；
> **输出：** 假
> **解释：**
> 给定字符串不以字母开头。因此，它不是有效的IFSC（印度金融系统）代码。

**做法：** 思路是用[正则表达式](https://www.geeksforgeeks.org/write-regular-expressions/)来解决这个问题。可以按照以下步骤计算答案。

*   获取字符串。
*   如下所述，创建一个正则表达式来检查有效的IFSC（印度金融系统）代码：
...
这个方法告诉这个字符串是否匹配给定的正则表达式。调用这种形式的方法 [`String.matches(regex)`](https://www.geeksforgeeks.org/java-lang-string-matches-java/) 会产生与表达式 `Pattern.matches(regex, string)` 完全相同的结果。`Pattern.compile(regex)` 编译该模式，这样当你执行 `matcher.matches()` 时，模式不会一次又一次地被重新编译。`Pattern.compile` 预编译它。但是，如果你使用 `String.matches`，它会在你每次执行该行时编译模式。所以，最好使用 `Pattern.compile()`。
