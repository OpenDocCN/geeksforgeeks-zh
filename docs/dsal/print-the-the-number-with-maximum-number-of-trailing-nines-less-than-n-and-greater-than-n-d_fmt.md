...
> 原文: [https://www.geeksforgeeks.org/print-the-the-number-with-maximum-number-of-trailing-nines-less-than-n-and-greater-than-n-d/](https://www.geeksforgeeks.org/print-the-the-number-with-maximum-number-of-trailing-nines-less-than-n-and-greater-than-n-d/)
...
一种**天真的方法**将从 `N` 迭代到 `N-D`，并找到尾随 9 数量最多的数字。

一个有效的方法可以通过一些关键的观察发现。这个问题的一个关键观察是，小于 `N` 的最大数字是其结尾至少是 (`K`9) 的数字：

> [n – (n MOD 10^k) – 1]

从 `N` 到 1 的位数总数开始遍历 `k` 的所有可能值，检查 `d > n % 10^k` 是否成立。如果没有得到这样的值，最终的答案将是 `N` 本身。否则，使用上面的观察来检查答案。
