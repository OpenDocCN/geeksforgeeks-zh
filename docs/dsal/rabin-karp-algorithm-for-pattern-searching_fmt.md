...
> 原文：[https://www.geeksforgeeks.org/rabin-karp-algorithm-for-pattern-searching/](https://www.geeksforgeeks.org/rabin-karp-algorithm-for-pattern-searching/)
...
下一次移位时的散列必须能够从当前散列值和文本中的下一个字符有效地计算出来，或者我们可以说 `hash(txt[s+1..s+m])` 必须能够从 `hash(txt[s..s+m-1])` 和 `txt[s+m]` 计算得出，即 `hash(txt[s+1..s+m]) = rehash(txt[s+m], hash(txt[s..s+m-1]))`，并且 `rehash` 必须是 `O(1)` 操作。
...
其中：
- `hash(txt[s..s+m-1])`：班次 `s` 的哈希值。
- `hash(txt[s+1..s+m])`：下一班次的哈希值（或班次 `s+1`）
- `d`：字母表中的字符数
- `q`：一个质数
- `h`：`d^(m-1)`
...
[`http://www . cs . Princeton . edu/courses/archive/fall 04/cos226/讲座/string . 4 up . pdf`](http://www.cs.Princeton.edu/courses/archive/fall04/cos226/lectures/string.4up.pdf)
[`http://en . Wikipedia . org/wiki/Rabin-Karp _ 等`](http://en.wikipedia.org/wiki/Rabin-Karp_algorithm)