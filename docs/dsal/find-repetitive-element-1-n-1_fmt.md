...
我们使用两个嵌套循环。外循环遍历所有元素，内循环检查外循环拾取的元素是否出现在其他任何地方。

**时间复杂度**：`O(n * n)`。

## 方法 2（使用求和公式）

我们知道[前 `n-1` 个自然数](https://www.geeksforgeeks.org/program-find-sum-first-n-natural-numbers/)的和为 `(n – 1) * n / 2`。我们计算数组元素的总和，并从中减去自然数总和，以找到唯一的缺失元素。
...
使用散列表存储已访问的元素。如果看到的元素再次出现，我们将其返回。
