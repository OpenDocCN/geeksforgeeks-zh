...
> 原文：`https://www.geeksforgeeks.org/find-minimum-adjustment-cost-of-an-array/`

给定一个正整数数组，替换数组中的每个元素，使数组中相邻元素之间的差小于或等于给定的目标。我们需要最小化调整成本，也就是新旧价值差异的总和。我们基本上需要最小化∑| `A[I]–A_new[I]` |其中 0 ≤ i ≤ n-1，n 为 `A[]` 的大小，`A_new[]` 为相邻差小于或等于目标的数组。
...
为了最小化调整成本∑| `A[I]–A_new[i]` |对于数组中的所有索引 I，| `A[I]–A_new[i]` |应该尽可能接近于零。另外，| `A[I]–A_new[I+1]` |≤目标。
这个问题可以通过`动态编程`解决。
让 `dp[i][j]` 定义将 `A[i]` 更改为 `j` 时的最小调整成本，然后 `dp` 关系由–定义
...
最后，对于所有 0 ≤ j ≤ M，数组的最小调整成本将是 `min { DP[n–1][j]}`。以下是上述想法的实现–
...
function `minAdjustmentCost( $A, $n, $target`)
{
    ...
    // Driver Code
    `$arr` = array(55, 77, 52, 61, 39, 6, 25, 60, 49, 47);
    `$n` = `count($arr)`;
    `$target` = 10;

    `echo "Minimum adjustment cost is ", minAdjustmentCost($arr, $n, $target)`;
...
<script>
    ...
    function `minAdjustmentCost(A, n, target`)
    {
        ...
        for (let `i` = 1; `i` < `n`; `i`++)
        {
            ...
            for (let `j` = 0; `j` <= `M`; `j`++)
            {
                // initialize minimal adjustment cost to `Number.MAX_VALUE`
    ...
    `document.write("Minimum adjustment cost is " + minAdjustmentCost(arr, n, target))`;
