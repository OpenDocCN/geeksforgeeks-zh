...
> 原文：[https://www.geeksforgeeks.org/find-the-platform-at-which-the-given-train-arrives/](https://www.geeksforgeeks.org/find-the-platform-at-which-the-given-train-arrives/)

给定一个由 `N` 列车信息组成的 2D 数组 `arr[][]`，其中 `arr[i][0]` 为车次号，`arr[i][1]` 为到达时间，`arr[i][2]` 为停运时长。给定另一个代表车次的整数 `F`，任务是按照以下规则找到车次为 `F` 的列车到达的站台号：
...
使用优先级队列可以解决给定的问题。按照以下步骤解决此问题：

*   根据列车到达时间对 `N` 列车的给定数组 `arr[]` 进行排序。
*   初始化一个优先级队列，比如说 `PQ`，对 `{PlatformNumber, time}` 按照最少出发时间执行分堆。在优先级队列中插入 `{平台号，时间}`，即 `{1, 0}`。
*   初始化一个 HashMap，比如说 `M`，存储任何一列火车到达的站台号。
*   遍历给定数组 `arr[]`，并执行以下步骤：
    *   弹出 `PQ` 的顶部平台，存放在 `free_platform[]`。
