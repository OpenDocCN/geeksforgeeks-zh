...
> 原文: [https://www.geeksforgeeks.org/minimum-possible-final-health-of-the-last-monster-in-a-game/](https://www.geeksforgeeks.org/minimum-possible-final-health-of-the-last-monster-in-a-game/)
...
从问题中可以观察到，一个人要找到怪物一定的生命值，比如说可以杀死包括自己在内的其他怪物的 `k`。一旦这个重要的观察结果出来，问题就变得容易了。假设我们有两个有生命值的怪物 `h1` 和 `h2`，假设 `h2 > h1`。我们可以看到，在随机选择中，最佳方式是选择一个生命值较低的怪物，并降低另一个怪物的生命值，直到它的生命值低于攻击怪物的生命值。之后我们会挑选第二个生命值小于 `h1` 的怪物，这个过程会一直持续到只剩下一个怪物。所以最后我们会得到最小值 `gcd(h1, h2)`。这个 `gcd` 方法可以扩展到所有的怪物。
...
**时间复杂度:** `O(N * log(MAX))`，其中 `N` 是数组的大小，`MAX` 是数组中的最大值。
