...
> 原文：[https://www.geeksforgeeks.org/reverse-level-order-traversal-in-spiral-form/](https://www.geeksforgeeks.org/reverse-level-order-traversal-in-spiral-form/)
...
想法是以[反向水平顺序](https://www.geeksforgeeks.org/reverse-level-order-traversal/)的方式遍历树，但稍加修改。我们将使用一个变量`标记`，并将其值设置为 1。当我们完成树的反向级别顺序遍历时，从左到右，我们将把`标志`的值设置为零，这样下次我们从右到左遍历树时，当我们完成遍历时，我们把它的值设置回 1。我们将重复这整个步骤，直到我们完全遍历了二叉树。
