# 给定一个字符串和一个整数 k，当所有子字符串按照给定条件排序后，找到第 kth 个子字符串

> 原文: [https://www.geeksforgeeks.org/given-a-string-and-an-integer-k-find-the-kth-sub-string-when-all-the-sub-strings-are-sorted-according-to-the-given-condition/](https://www.geeksforgeeks.org/given-a-string-and-an-integer-k-find-the-kth-sub-string-when-all-the-sub-strings-are-sorted-according-to-the-given-condition/)
...
**进场:** 思路是用[二分搜索法](https://www.geeksforgeeks.org/binary-search/)。一个数组 `substring` 将用于存储以第一个字符 + `substring[i–1]` 开始的子串的数量。现在在数组 `substring` 上使用二分搜索法，找到所需子串的 `起始索引`，然后找到相同子串的 `结束索引`，其中 `end = length_of_string – (substring[start] – k)`。
