...
*   首先，我们需要将单链表转换为双链表。在这里，我们给出了单链表结构节点`Node`，该节点仅具有`next`指针，而没有`prev`指针，因此，为了将单链表转换为双链表，我们使用[内存高效双链表（异或链表）](https://www.geeksforgeeks.org/xor-linked-list-a-memory-efficient-doubly-linked-list-set-2/)。
