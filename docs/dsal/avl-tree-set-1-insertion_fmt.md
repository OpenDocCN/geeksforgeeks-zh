...
### 时间复杂度
旋转操作（左右旋转）需要恒定的时间，因为只有几个指针在那里被改变。更新高度和获得平衡系数也需要恒定的时间。所以 AVL 插入的时间复杂度和 BST 插入一样，都是 `O(h)`，其中 `h` 是树的高度。由于 AVL 树是平衡的，所以高度为 `O(Logn)`。所以 AVL 插入的时间复杂度为 `O(Logn)`。
