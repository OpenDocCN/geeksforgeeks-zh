...
> 原文：[https://www.geeksforgeeks.org/number-of-ways-to-sum-up-a-total-of-n-from-limited-denominations/](https://www.geeksforgeeks.org/number-of-ways-to-sum-up-a-total-of-n-from-limited-denominations/)
...
> `(A * 1)+(B * 5)+(C * 10)+(D * 20) = N`
> 其中 `0 <= a <= A`，`0 <= b <= B`，`0 <= c <= C`，`0 <= d <= D` 和 `N` 为总金额。
...
1.  初始化数组 `ways[]` 存储预计算的 `A` 和 `B` 面额之和。
2.  迭代两个嵌套循环，以 `ways[]` 存储由 `A` 和 `B` 面额组成的值组合。
