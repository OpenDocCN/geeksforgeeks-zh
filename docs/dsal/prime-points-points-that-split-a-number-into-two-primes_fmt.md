...
> 原文：[https://www.geeksforgeeks.org/prime-points-points-that-split-a-number-into-two-primes/](https://www.geeksforgeeks.org/prime-points-points-that-split-a-number-into-two-primes/)

给定一个 `n` 位数。素数点是左右边数字为素数的数字的索引。打印数字的所有素数点。如果不存在素数点，打印 `-1`。
