...
> 原文: [https://www.geeksforgeeks.org/check-whether-given-string-can-be-generated-after-concatenating-given-strings/](https://www.geeksforgeeks.org/check-whether-given-string-can-be-generated-after-concatenating-given-strings/)

## 问题描述

给定三个字符串 `str`、`A` 和 `B`。任务是检查 `str = A + B` 还是 `str = B + A`，其中 `+` 表示串联。
...
1.  如果 `len(str) != len(A) + len(B)`，则不可能通过串联 `a + b` 或 `b + a` 来生成字符串 `str`。
2.  否则，检查字符串 `str` 是以 `a` 开头并以 `b` 结尾，还是以 `b` 开头并以 `a` 结尾。如果其中任何一项为真，则打印“是”；否则打印“否”。
