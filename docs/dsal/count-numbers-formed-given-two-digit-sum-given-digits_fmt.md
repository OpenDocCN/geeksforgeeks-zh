...
> 原文: [https://www.geeksforgeeks.org/count-numbers-formed-given-two-digit-sum-given-digits/](https://www.geeksforgeeks.org/count-numbers-formed-given-two-digit-sum-given-digits/)
...
**方法:** 由于 `N` 非常大，所以我们不能对所有的数字进行迭代来逐个检查。由于数字长度为 `N`，由两位数 `a` 和 `b` 组成，`a` 将占据 `i` 位置，`b` 将占据 `N–i` 位置。因此数字的总和将是 `(a*i + (n-i)*b)`。我们可以检查从 0 到 `n` 的所有 `i` 的数字总和是否由 `a` 和 `b` 组成。因此，形成的数字总数将是 `{n \choose i}`，这将对应于由 `a` 和 `b` 组成的所有数字组合，其数字总和也由 `a` 和 `b` 组成。

**在计算中实现模运算:**
为满足给定条件的从 0 到 `N` 的所有 `i` 计算 `{n \choose i}` % 1000000007。
...
由于计算反相需要 O(log N)，如果所有的阶乘都是预先计算的，那么总的时间复杂度将是 O(log N)。

一个**有效的解决方案**将是预先计算所有的阶乘直到 `N`。计算 `N` 的反方向！在 O(log N) 中，从 `(N–1)` 开始计算所有阶乘的方差！到 `1`！通过下面给出的公式。
