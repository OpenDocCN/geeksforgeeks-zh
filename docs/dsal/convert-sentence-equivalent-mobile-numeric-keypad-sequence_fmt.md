...
> 原文：[https://www.geeksforgeeks.org/convert-sentence-equivalent-mobile-numeric-keypad-sequence/](https://www.geeksforgeeks.org/convert-sentence-equivalent-mobile-numeric-keypad-sequence/)
...
*   对于每个字符，存储应该在数组中相应位置获得的序列，即对于 `Z`，存储 `9999`。`Y` 的话，`999`。对于 `K`，存储 `55` 等等。
*   对于每个字符，减去 `ASCII` 值 `“A”`，获得该字符在数组中指向的位置，并将存储在该数组中的序列添加到字符串中。
*   如果字符是空格，存储 `0`
*   打印整个序列。
...
```
4335577773366777743377773377777
```
