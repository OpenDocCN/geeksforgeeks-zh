...
*   `F = A \wedge \bar{B}`，是可满足的，因为 A =真，B =假使得 F =真。
*   `G = A \wedge \bar{A}`，不可满足，因为:
...
**例:** `F = (A_1 \vee B_1) \wedge (A_2 \vee B_2) \wedge (A_3 \vee B_3) \wedge ....... \wedge (A_m \vee B_m)`
因此，2-可满足性问题可以表述为:
...
要使 `CNF` 价值成为现实，每个条款的价值都应该是真实的。让其中一个从句成为`(A \vee B)`。
...
*   如果 `A = 0`，`B` 必须为 1，即`(\bar{A} \Rightarrow B)`
*   如果 `B = 0`，`A` 必须为 1，即`(\bar{B} \Rightarrow A)`
...
`(A \vee B)`在蕴涵图中表示为`edge(\bar{A} \rightarrow B) \ & edge(\bar{B} \rightarrow A)`
...
**注:** 言外之意(如果 `A` 那么 `B`)等同于它的对位(如果`\bar{B}`那么`\bar{A}`)。
现在，考虑以下情况:
...
**结论:** 如果任意两个变量`X`和`\bar{X}`处于一个周期，即`path(\bar{A} \rightarrow B) \& path({B} \rightarrow A)`都存在，那么 `CNF` 是不可满足的。否则，有一个可能的任务，`CNF` 是可满足的。
这里注意，我们使用路径是由于蕴涵的以下性质:
如果我们有`(A \Rightarrow B) \& (B \Rightarrow C), then A \Rightarrow C`
因此，如果我们在蕴涵图中有一条路径，那就相当于有一条直接边。
**结论从实现的角度来看:**
如果 `X` 和`\bar{X}`位于同一个 `SCC`(强连通分量)中，则 `CNF` 不可满足。
有向图强连通分支具有这样的节点，即每个节点都可以从 `SCC` 中的每个其他节点到达。
现在，如果 `X` 和`\bar{X}`位于同一个 `SCC` 上，我们肯定会有`path(\bar{A} \rightarrow B) \& path({B} \rightarrow A)`出现，从而得出结论。
