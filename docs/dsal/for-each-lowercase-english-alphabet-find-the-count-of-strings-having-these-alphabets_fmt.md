...
> 原文: [https://www.geeksforgeeks.org/for-each-lowercase-english-alphabet-find-the-count-of-strings-having-these-alphabets/](https://www.geeksforgeeks.org/for-each-lowercase-english-alphabet-find-the-count-of-strings-having-these-alphabets/)

给定一组小写英文字母字符串。任务是为每个字母 `[a-z]` 找到包含这些字母的字符串的数量。

## 例:

> **输入:** `str = {"geeks", "for", "code"}`
> **输出:** `{ 0 0 1 1 2 1 0 0 0 0 2 0 1 0 0 1 0 0 0 0 }`
> **解释:**
> 对于一个字母，说 `'e'`，它存在于 `{"geeks", "code"}`，因此它的计数是 2。
> 同样对于另一个字母的结果可以找到。

> **输入:** `str = {"i", "will", "practice", "everyday"}`
> **输出:** `2 0 1 2 0 0 3 0 0 0 0 1 0 0 1 0 1 0 1 1 0 1 0`
> **解释:**
> 对于一个字母，说 `'i'`，它存在于 `{"i", "will", "practice"}`，因此它的计数是 3。
> 同样对于另一个字母的结果可以找到。
...
*   此外，为了避免重复计数，我们创建了一个访问布尔数组来标记到目前为止遇到的字符。这种方法将复杂度降低到 `O(N)`。
