...
> 原文: [https://www.geeksforgeeks.org/check-if-a-given-array-is-sorted-in-spiral-manner-or-not/](https://www.geeksforgeeks.org/check-if-a-given-array-is-sorted-in-spiral-manner-or-not/)
...
思路是[遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)，对于每个数组元素，比如 `arr[i]`，检查 `arr[i]` 是否小于等于 `arr[N–i–1]` 和 `arr[N–i–1]` 是否小于等于 `arr[i+1]`。如果发现是假的，则打印`“否”`。否则，如果所有数组元素都满足条件，则打印`“是”`。按照以下步骤解决问题:
