...
> 原文: [https://www.geeksforgeeks.org/area-of-plot-remaining-at-the-end/](https://www.geeksforgeeks.org/area-of-plot-remaining-at-the-end/)

给定两个整数 `N` 和 `M` 表示矩形地块的长度和宽度，另一个整数 `K` 表示人数。每个人将把地块分成两部分，这样它将是地块中最大的可能的正方形，并将第二部分留给其他人，它一直持续到地块结束或每个人都得到地块。现在，任务是确定最后留下的地块面积。

## 举例

**输入:** `N = 5`，`M = 3`，`K = 2`
**输出:** `2`
第一个人将 `5×3` 的地块分为 2 部分，即 `3×3` 和 `2×3`，将得到尺寸为 `3×3` 的地块。另一个人再次将 `2×3` 的地块分成两部分，即 `2×2` 和 `1×2`，将得到尺寸为 `2×2` 的地块。现在地块剩下的部分是 `1×2` 的尺寸，面积为 2 个单位。
...
3.  对所有人重复上述步骤，同时地块的面积大于 0。
4.  打印最后剩余地块的面积，即 `长度 * 宽度`。
