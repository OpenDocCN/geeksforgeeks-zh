...
内循环最多运行`k`次。 要将每个元素移到正确的位置，最多需要移动`k`个元素。 因此总体复杂度将为`O(nk)`。
...
我们还可以使用平衡二叉搜索树而不是堆来存储`K + 1`个元素。 平衡 BST 上的插入和删除操作也需要`O(Logk)`时间。 因此，基于平衡 BST 的方法也将花费`O(nLogk)`时间，但是堆最小化方法似乎更有效，因为最小元素始终是根。 此外，堆不需要额外的空间来放置左右指针。