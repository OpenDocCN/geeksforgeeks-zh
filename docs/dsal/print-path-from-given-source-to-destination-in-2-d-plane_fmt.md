...
> 原文: [https://www.geeksforgeeks.org/print-path-from-given-source-to-destination-in-2-d-plane/](https://www.geeksforgeeks.org/print-path-from-given-source-to-destination-in-2-d-plane/)

给定源点 `(src_x, src_y)` 和目的点 `(dst_x, dst_y)` 的坐标，任务是确定从源点到达目的点的可能路径。从任何一点 `(x, y)` 来看，只有两种有效移动：`(2 * x + y, y)` 或 `(x, 2 * y + x)`。如果路径不存在，则打印 `-1`。

**例:**

> **输入:** `(src_x, src_y) = {5, 8}`, `(dst_x, dst_y) = {83, 21}`
...
如果当前坐标 `x` 或 `y` 超过目的地坐标，我们终止递归调用。将路径存储在堆栈中，到达目的地后打印堆栈中的元素。
