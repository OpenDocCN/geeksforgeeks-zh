...
> 原文: [https://www.geeksforgeeks.org/permute-two-arrays-sum-every-pair-greater-equal-k/](https://www.geeksforgeeks.org/permute-two-arrays-sum-every-pair-greater-equal-k/)

给定两个大小相等的数组 `a[]` 和 `b[]` 以及一个整数 `k`。任务是对两个数组进行置换，使得它们对应元素的和大于或等于 `k`，即 `a[i] + b[i] >= k`。如果存在这样的置换，任务是打印 `Yes`，否则打印 `No`。
...
其思想是按升序对一个数组进行排序，按降序对另一个数组进行排序，如果任何索引不满足条件 `a[i] + b[i] >= K`，则打印 `No`，否则打印 `Yes`。
如果排序数组的条件失败，则不存在满足不等式的数组置换。

### 证明

假设 `a_sorted[]` 按升序排序 `a[]`，而 `b_sorted[]` 按降序排序 `b[]`。
让新的置换 `b[]` 通过交换 `b_sorted[]` 的任意两个索引 `i`, `j` 来创建，

*   **情况 1:** `i < j`，`b[i]` 处的元素现在是 `b_sorted[j]`。
    现在，`a_sorted[i] + b_sorted[j] < K`，因为 `b_sorted[i] > b_sorted[j]` 作为 `b[]` 是按降序排序的，我们知道 `a_sorted[i] + b_sorted[i] < K`。
*   **情况 2:** `i > j`，`b[i]` 处的元素现在是 `b_sorted[j]`。
    现在，`a_sorted[j] + b_sorted[i] < k`，因为 `a_sorted[i] > a_sorted[j]` 作为一个[]按照递增顺序排序，我们知道 `a_sorted[i] + b_sorted[i] < k`。
