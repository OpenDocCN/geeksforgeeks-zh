...
> 原文: [https://www.geeksforgeeks.org/minimum-sub-array-such-that-number-of-1s-in-concatenation-of-binary-representation-of-its-elements-is-at-least-k/](https://www.geeksforgeeks.org/minimum-sub-array-such-that-number-of-1s-in-concatenation-of-binary-representation-of-its-elements-is-at-least-k/)
...
想法是使用两个变量 `j` 和 `i` 并分别将其初始化为 0 和 1，以及一个数组 `count_one`，该数组将存储数组中特定元素的二进制表示中存在的 1 的数量，以及一个变量 `sum` 存储直到索引的 1 的数量，以及 `ans` 存储最小长度。现在迭代数组，如果 `count_one` 的 `i`th 或 `j`th 元素的 1 的个数等于 `k`，那么将 `ans` 更新为 1，如果到 `i`th 元素的 1 的个数之和大于或等于 `k`，那么将 `ans` 更新为 `ans` 和 `(i-j)+1` 的最小值，否则如果小于 `k`，那么将 `j` 增加 1，以增加 `sum` 的值。
