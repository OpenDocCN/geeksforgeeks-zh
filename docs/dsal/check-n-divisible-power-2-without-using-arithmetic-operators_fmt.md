...
> 原文：[https://www.geeksforgeeks.org/check-n-divisible-power-2-without-using-arithmetic-operators/](https://www.geeksforgeeks.org/check-n-divisible-power-2-without-using-arithmetic-operators/)

给定两个正整数 `n` 和 `m`。问题是在不使用算术运算符的情况下，检查 `n` 是否可被 `2^m` 整除。
...
**方法：** 如果一个数可以被 2 整除，那么它的最低有效位（LSB）设置为 0，如果可以被 4 整除，那么两个 LSB 设置为 0，如果可以被 8 整除，那么三个 LSB 设置为 0，以此类推。记住这一点，如果 `(n & ((1 << m) - 1))` 等于 0，则数字 `n` 可被 `2^m` 整除。
