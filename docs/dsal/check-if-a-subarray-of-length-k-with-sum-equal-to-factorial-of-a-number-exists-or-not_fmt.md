# 检查长度为K且和等于一个数的阶乘的子阵列是否存在

> 原文：[https://www.geeksforgeeks.org/check-if-a-subarray-of-length-k-with-sum-equal-to-factorial-of-a-number-exists-or-not/](https://www.geeksforgeeks.org/check-if-a-subarray-of-length-k-with-sum-equal-to-factorial-of-a-number-exists-or-not/)

给定一个由 `N` 个整数和一个整数 `K` 组成的数组 `arr[]`，任务是找到一个长度为 `K` 的子数组，其元素之和等于任意数的阶乘。如果不存在这样的子阵列，打印 `-1`。
...
> **输出:** `2 4 6 9 3`
> **解释:**
> 子数组 `{2, 4, 6, 9, 3}` 的和为 `24`（等于 `4!`），满足要求的条件。
>
> **输入:** `arr[] = {23, 45, 2, 4, 6, 9, 3, 32}`，`K = 3`
> **输出:** `-1`
> **解释:**
> 不存在这样的长度为 `K`（等于 `3`）的子阵。

## 天真方法

解决问题最简单的方法是计算所有长度为 `K` 的子阵列的和，并检查这些和中是否有任何一个等于任意数的阶乘。如果发现任何子阵列为真，打印该子阵列。否则，打印 `-1`。
...
为了优化上述方法，思路是使用滑动窗口技术计算长度为 `K` 的所有子阵列的和，然后检查和是否为阶乘。以下是步骤：

1.  计算第一个 `K` 数组元素的和，并将和存储在一个变量中，比如 `sum`。
2.  然后遍历剩余数组并不断更新 `sum`，以得到当前大小为 `K` 的子阵的和，通过从前一个子阵中减去第一个元素再加上当前数组元素。
3.  要检查 `sum` 是否是一个数的阶乘，将 `sum` 除以 `2`、`3`，以此类推，直到不能再除为止。如果数字减为 `1`，总和就是数字的阶乘。
