...
> 长度为 `k` 的从 `0` 到 `2` 的路径数为 `2`（`{0->0->2}`、`{0->1->2}`）。
...
> 长度为 `k` 的从 `2` 到 `2` 的路径数为 `1`（`{2->1->0->2}`）。
...
我们假设有些 `k` 的答案是 `Mat_k`，那么 `k+1` 的答案是 `Mat_{k+1}`。

`Mat_{k+1}[i][j] = ∑_{p=1}^{N} Mat_k[i][p] * G[p][j]`

很容易看出，除了矩阵 `Mat_k` 和 `G` 的乘积之外，公式什么也不会计算，即 `Mat_{k+1} = Mat_k * G`。

因此，问题的解决方法可以表示为 `Mat_k = G * G * …… * G`（`k` 次）= `G^k`。
