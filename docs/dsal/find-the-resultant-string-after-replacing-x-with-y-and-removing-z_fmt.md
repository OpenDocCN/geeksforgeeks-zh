...
> 原文：[https://www.geeksforgeeks.org/find-the-resultant-string-after-replacing-x-with-y-and-removing-z/](https://www.geeksforgeeks.org/find-the-resultant-string-after-replacing-x-with-y-and-removing-z/)

给定一个字符串 `str`，任务是用给定的 `Y` 替换给定的 `X` 的所有出现，并移除给定的 `Z` 的任何出现，如果给定的字符串中没有多余的空间。
...
*   这个想法基于 **2 个指针**。
*   让双变量 `start` 和 `end` 指向字符串的开始和结束。
*   现在，如果 `start` 的字符是 `Z`，用另一个指针指向一个位置时没有 `Y` 的字符替换它。`start` 记住，如果找到，用 `Y` 替换字符 `X`。
