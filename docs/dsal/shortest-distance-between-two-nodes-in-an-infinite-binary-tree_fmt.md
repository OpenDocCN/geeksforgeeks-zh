...
> 原文: [https://www.geeksforgeeks.org/shortest-distance-between-two-nodes-in-an-infinite-binary-tree/](https://www.geeksforgeeks.org/shortest-distance-between-two-nodes-in-an-infinite-binary-tree/)
...
一种简单的方法是将两个节点的所有祖先存储在两个数据结构中（[`vector`](https://www.geeksforgeeks.org/vector-in-cpp-stl/)、[`array`](https://www.geeksforgeeks.org/array-data-structure/)等）并对 `vector1` 中的第一个元素（let index `i`）执行[二分搜索法](https://www.geeksforgeeks.org/binary-search/)，检查它是否存在于 `vector2` 中。如果有，返回 `vector2` 中元素的索引（let `x`）。答案是这样的
