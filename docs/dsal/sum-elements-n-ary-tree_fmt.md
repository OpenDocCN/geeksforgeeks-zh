...
**方法:**使用的方法类似于二叉树中的[级顺序遍历。首先推送队列中的根节点。对于每个节点，当它从队列中弹出时，在 `sum` 变量中添加该节点的值，并将弹出元素的子元素推入队列。在一般树的情况下，将子节点存储在向量中。因此，将向量的所有元素放入队列。](https://www.geeksforgeeks.org/level-order-tree-traversal/)
