...
假设你拥有的硬币和你想赚取的利润允许你连续最多损失`N`次**。连续输`(N + 1)`次的概率为`1/(2^(N + 1))`。这个[概率](https://www.geeksforgeeks.org/mathematics-probability/)随着`N 增加`而不断变小。例如，如果一个人可以承受连续输 9 次，那么赢得你想要的硬币的概率是 99.9990234%。这是一个基于实际应用的算法。

## 具体示例:

### 例 1:
在我们的实数坐标系的开放空间中的无偏随机游走是鞅的一个例子。

### 例 2:
让`Y_N= X_(N^2–N)`其中`X_N`是此人从上一次经历中获胜的运气。那么序列`{Y_N : N = 1，2，3，…}`就是鞅。这个序列可以用来显示这个人的总收益或损失大致在步骤数的平方根的正负之间变化。