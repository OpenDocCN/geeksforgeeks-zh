...
> 原文：[https://www.geeksforgeeks.org/count-clockwise-array-rotations-required-to-maximize-count-of-array-elements-present-at-indices-same-as-their-value/](https://www.geeksforgeeks.org/count-clockwise-array-rotations-required-to-maximize-count-of-array-elements-present-at-indices-same-as-their-value/)

给定一个由前 `N` 个自然数的[排列](https://www.geeksforgeeks.org/write-a-c-program-to-print-all-permutations-of-a-given-string/)组成的[数组](https://www.geeksforgeeks.org/array-data-structure/)`arr[]`，任务是找到数组中最少数量的[顺时针圆周旋转](https://www.geeksforgeeks.org/array-rotation/)，以最大化满足条件 `arr[i] = i`（*基于 1 的索引*）的元素数量，其中 `1 ≤ i ≤ N`。
...
*   将 `new_arr[]` 中各元素的[频率计数](https://www.geeksforgeeks.org/counting-frequencies-of-array-elements/)存储在数组 `freq[]` 中。
*   打印最大频率的元素作为所需答案。
