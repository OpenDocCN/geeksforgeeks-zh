...
**方法:** 我们将用[动态规划](https://www.geeksforgeeks.org/dynamic-programming/)来解决这个问题。让我们表示字符串 `str` 的长度为 `N`。现在，让 `dp[i][j]` 表示从 `0` 到 `i–1` 的回文子序列数和从 `j` 到 `N–1` 的回文子序列数。
让 `lens` 成为 `i` 和 `j` 之间的距离。对于每一个长度 `lens`，我们将固定我们的 `i` 和 `j`，并检查字符 `str[i]` 和 `str[j]` 是否相等。然后根据它，我们将进行 `dp` 过渡。

> 如果 `str[i] != str[j]` 那么 `dp[i][j] = dp[i–1][j] + dp[i][j+1] – dp[i–1][j+1]`
> 如果 `str[i] == str[j]` 那么 `dp[i][j] = dp[i–1][j] + dp[i][j+1]`
> 基本情况:
> 如果 `i == 0` 和 `j == n-1`...
