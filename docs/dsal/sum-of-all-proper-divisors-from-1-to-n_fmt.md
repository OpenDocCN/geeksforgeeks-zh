...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-proper-divisors-from-1-to-n/](https://www.geeksforgeeks.org/sum-of-all-proper-divisors-from-1-to-n/)

给定一个正整数 `N`，任务是找到 $\sum_{x=1}^{x=N} F(x)$ 的值，其中函数 `F(x)` 可以定义为 `x` 的所有适当因子之和。
...
> `F(4)= 1+2 = 3`
> `F(5)= 1`
> 总和= `F(1) + F(2) + F(3) + F(4) + F(5) = 0 + 1 + 1 + 3 + 1 = 6`

### 天真的做法

思路是分别求出 `[1, N]` 范围内每个数的适当除数之和，然后相加求出所需之和。
以下是上述方法的实现:
...
观察函数中的模式，可以看出“对于给定的数字 `N`，范围 `[1, N]` 内的每个数字 `x` 出现 `(N/x)` 次”。
