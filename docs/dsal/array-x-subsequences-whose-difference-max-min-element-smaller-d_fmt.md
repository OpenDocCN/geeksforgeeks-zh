...
> 原文：[https://www.geeksforgeeks.org/array-x-subsequences-whose-difference-max-min-element-smaller-d/](https://www.geeksforgeeks.org/array-x-subsequences-whose-difference-max-min-element-smaller-d/)
...
大小为 `N` 的数组中非空子序列的总数等于 `2^n - 1`。因此，我们只需要找到子序列总数小于或等于 `X` 的不相交集的长度，并输出该不相交集的元素。如果它不等于 `X`，那么必须在约化的 `X` 上进行相同的步骤，以找到下一个不相交的集合，直到 `X` 变成 `0`。为了使所有集合不相交，一种方法是将第一个数组元素取为 `1`，然后在第一个不相交集合中加上等于这个不相交集合长度的 `1`，然后在前一个不相交集合中的元素上加上 `d` 以形成另一个不相交集合，以此类推。

> 例如：`X = 25`，`d = 100`
> 第一个不相交的集合将有 4 个元素，因为它有 15 个子序列。所以第 1 个不相交集将是 `{ 1, 1, 1, 1 }`。
> 现在 `X` 变成 10，因此第二个不相交集将只包含 3 个元素。所以第 2 个不相交将是 `{ 101, 101, 101 }`（通过在 `1` 中添加 `100` 使其与第一个集合不相交）。现在 `X` 变成 3，因此最终的不相交集将包含两个元素，第 3 个不相交集将是 `{ 201, 201 }`（再次在 `101` 中添加 `100`，使其与前面的两个集不相交）。
