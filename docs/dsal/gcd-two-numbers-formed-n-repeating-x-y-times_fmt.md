# 计算由 n 重复 x 次和 y 次形成的两个数的 GCD

> 原文：[https://www.geeksforgeeks.org/gcd-two-numbers-formed-n-repeating-x-y-times/](https://www.geeksforgeeks.org/gcd-two-numbers-formed-n-repeating-x-y-times/)
...
设 `f(n, x)` 是一个给出 `n` 重复 `x` 次的数的函数。所以，我们需要求 `GCD(f(n, x), f(n, y))`。
设 `n = 123`，`x = 3`，`y = 2`。
...
所以，`A = 123123123`，`B'` 可以是 `123123000`。减去后 `A` 将变成 `123`，`B` 保持不变。
因此，`A = A - B' = f(n, x - y)`。
