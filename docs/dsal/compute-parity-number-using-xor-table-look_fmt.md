...
> 原文: [https://www.geeksforgeeks.org/compute-parity-number-using-xor-table-look/](https://www.geeksforgeeks.org/compute-parity-number-using-xor-table-look/)
...
如果我们把一个数 `S` 分成两部分 `S1` 和 `S2` 这样 `S = S1S2`。如果我们知道 `S1` 和 `S2` 的奇偶性，我们可以用以下事实计算 `S` 的奇偶性:

1.  如果 `S1` 和 `S2` 具有相同的奇偶校验，即它们都具有偶数比特或奇数比特，它们的并集 `S` 将具有偶数比特。
2.  因此 `S` 的奇偶性是 `S1` 和 `S2` 奇偶性的异或
...
由于 32 位或 64 位数字包含恒定的字节数，上述步骤需要 `O(1)` 时间。
