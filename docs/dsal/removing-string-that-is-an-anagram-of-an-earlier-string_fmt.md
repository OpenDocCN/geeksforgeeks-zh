...
> 原文：[https://www.geeksforgeeks.org/removing-string-that-is-an-anagram-of-an-earlier-string/](https://www.geeksforgeeks.org/removing-string-that-is-an-anagram-of-an-earlier-string/)

给定一个字符串数组 `arr`，任务是删除先前字符串的字谜，然后按排序顺序打印剩余的数组。

## 示例

> **输入:** `arr[] = {"geeks", "keegs", "code", "doce"}`，`N = 4`
> **输出:** `["code", "geeks"]`
> **解释:**
> `"geeks"` 和 `"keegs"` 都是字谜，所以我们去掉 `"keegs"`。
> 同样，`"code"` 和 `"doce"` 也是字谜，所以我们去掉了 `"doce"`。
>
> **输入:** `arr[] = {"tea", "ate", "anagram", "eat", "gramaan"}`，`N = 5`
> **输出:** `["anagram", "tea"]`
> **说明:** `"ate"` 和 `"eat"` 都是 `"tea"` 的字谜。
> `"gramaan"` 是 `"anagram"` 的字谜。因此，`array` 变成 `["anagram", "tea"]`。

## 方法

为了[检查给定的两个字符串是否是字谜而不是](https://www.geeksforgeeks.org/check-whether-two-strings-are-anagram-of-each-other/)，我们可以简单地对两个字符串进行排序并进行比较。此外，为了检查字符串是否出现，我们可以使用[散列表](https://www.geeksforgeeks.org/java-util-hashmap-in-java/)。
