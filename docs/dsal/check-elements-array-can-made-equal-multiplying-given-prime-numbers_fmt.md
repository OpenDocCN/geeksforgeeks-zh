...
> 原文: [https://www.geeksforgeeks.org/check-elements-array-can-made-equal-multiplying-given-prime-numbers/](https://www.geeksforgeeks.org/check-elements-array-can-made-equal-multiplying-given-prime-numbers/)
...
我们找到所有数组元素的 [LCM](https://www.geeksforgeeks.org/lcm-of-given-array-elements/)。只有当可以将所有数字转换为 `LCM` 时，才能使所有元素相等。所以我们找到每个元素的乘数，这样我们就可以通过乘以这个数，使这个元素等于 `LCM`。然后我们发现给定素数的数是否能形成给定的乘数。

**算法-**

**第一步:** 求数组 `O(n)` 中所有数的 `LCM`。
