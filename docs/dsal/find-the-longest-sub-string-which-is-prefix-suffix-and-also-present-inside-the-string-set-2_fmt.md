...
> 原文: [https://www.geeksforgeeks.org/find-the-longest-sub-string-which-is-prefix-suffix-and-also-present-inside-the-string-set-2/](https://www.geeksforgeeks.org/find-the-longest-sub-string-which-is-prefix-suffix-and-also-present-inside-the-string-set-2/)

给定字符串 `str`。任务是找到最长的子字符串，它是给定字符串 `str` 的前缀、后缀和子字符串。如果不存在这样的字符串，则打印 `-1`。
