...
给定`栈``S`，任务是[从上至下打印栈的元素](https://www.geeksforgeeks.org/print-stack-elements-from-bottom-to-top/)，以使元素仍然存在于栈中，而不改变它们的顺序。
...
1.  创建一个具有`栈`作为参数的递归函数。
2.  添加基本条件，如果`栈`为空，则从函数返回。
...
`单链表栈方法`：此方法讨论解决`单链表栈`表示法的问题的解决方案。步骤如下：
