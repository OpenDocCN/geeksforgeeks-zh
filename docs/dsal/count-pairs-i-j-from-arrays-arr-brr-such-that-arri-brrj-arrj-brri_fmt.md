...
给定两个[数组](https://www.geeksforgeeks.org/introduction-to-arrays/)`arr[]`和`brr[]`，它们由`N`个整数组成，其任务是计算对数`(i, j)`来自两个数组，使得`arr[i] – brr[j]`和`arr[j] – brr[i]`相等。
...
解决该问题的最简单方法是从两个给定的数组生成所有对，并检查所需条件。 对于发现条件为真的每对，增加此类对的**计数**。 最后，打印获得的**计数**。
...
想法是将给定的表达式`a[i] – b[j] = a[j] – b[i]`**转换为**`a[i] + b[i] = a[j] + b[j]`形式，然后计算满足条件的对。 步骤如下：
