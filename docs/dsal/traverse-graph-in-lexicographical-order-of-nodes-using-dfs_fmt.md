...
> 原文:[https://www.geeksforgeeks.org/traverse-graph-in-lexicographical-order-of-nodes-using-dfs/](https://www.geeksforgeeks.org/traverse-graph-in-lexicographical-order-of-nodes-using-dfs/)

给定由 `N` 节点、源 `S` 和类型 `{u，v}` 的数组 `Edges[][2]` 组成的图 `G`，表示在节点 `u` 和 `v` 之间有一条无向边，任务是使用 DFS 按照字典顺序遍历该图。
