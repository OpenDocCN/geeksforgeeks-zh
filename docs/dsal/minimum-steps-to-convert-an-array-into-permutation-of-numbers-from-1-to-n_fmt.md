...
> 原文: [https://www.geeksforgeeks.org/minimum-steps-to-convert-an-array-into-permutation-of-numbers-from-1-to-n/](https://www.geeksforgeeks.org/minimum-steps-to-convert-an-array-into-permutation-of-numbers-from-1-to-n/)

给定一个长度为 `N` 的数组 `arr`，任务是计算将给定序列转换为前 `N` 个自然数 `(1, 2, ..., N)` 的排列所需的最小操作数。在每个操作中，可以将一个元素递增或递减一。
