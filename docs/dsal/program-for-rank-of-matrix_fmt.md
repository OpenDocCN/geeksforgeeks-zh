...
class GFG {
    ...
    static int rankOfMatrix(int mat[][]) {
        ...
        for (int row = 0; row < rank; row++) {
            ...
            // 2) If all elements in current column below mat[r][row] are 0, then remvoe this column by swapping it with last column and reducing number of columns by 1.
            else { 
                boolean reduce = true;
            ...
            // Uncomment these lines to see intermediate results display(mat, R, C); 
            // printf("\n"); 
        }
