...
> 原文: [https://www.geeksforgeeks.org/queries-to-add-remove-and-return-the-difference-of-maximum-and-minimum/](https://www.geeksforgeeks.org/queries-to-add-remove-and-return-the-difference-of-maximum-and-minimum/)
...
*   将数字存储在向量数组中。
*   对于**查询类型-1**，向数组中添加一个元素。
*   对于**查询类型-2**，从向量或数组中移除元素。
*   对于**类型-3 的查询**，遍历数组，找到数组中的最小值和最大值，并返回它们的差值。

**时间复杂度:** `O(1)` 用于类型-1 的查询的插入，`O(N)` 用于最坏情况下的类型-2 的查询，`O(N)` 用于类型-3 的查询。
**辅助空间:** `O(N)`
...
*   使用 [`erase()`](https://www.geeksforgeeks.org/seterase-c-stl/) 功能从容器中删除元素。
