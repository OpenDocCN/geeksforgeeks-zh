...
> 原文：[https://www.geeksforgeeks.org/count-groupings-number-sum-digits-every-sub-group-less-equals-immediate-right-sub-group/](https://www.geeksforgeeks.org/count-groupings-number-sum-digits-every-sub-group-less-equals-immediate-right-sub-group/)
...
如果我们仔细看看上面的递归解，我们会注意到可能有重叠的子问题。例如，如果输入数字是 12345，那么对于`position`= 3 和`previous_sum`= 3，我们重复两次。类似地，对于`position` 4 和`previous_sum`= 7，我们重复两次。因此，可以使用**动态规划**来优化上述解决方案。下面是一个基于动态规划的解决方案。

1.  数字的最大总和可以是 9 *`length`，其中`length`是输入数字的长度。
2.  创建一个 2D 数组`int dp[MAX][9*MAX]`，其中`MAX`是输入数字的最大可能长度。值`dp[position][previous_sum]`将存储`position`和`previous_sum`的结果。
3.  如果当前子问题已经过评估，即:`dp[position][previous_sum] != -1`，然后使用这个结果，否则递归计算它的值。
4.  如果将当前位置数字包括在总和中，即:`sum = sum + num[position] - '0'`，`sum`变得大于等于前一个`sum`，然后递增结果，并调用`num`中下一个位置的问题。
