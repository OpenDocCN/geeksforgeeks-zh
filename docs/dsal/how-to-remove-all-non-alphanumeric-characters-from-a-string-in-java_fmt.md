...
> 原文: [https://www.geeksforgeeks.org/how-to-remove-all-non-alphanumeric-characters-from-a-string-in-java/](https://www.geeksforgeeks.org/how-to-remove-all-non-alphanumeric-characters-from-a-string-in-java/)
...
非字母数字字符包括除字母和数字之外的所有字符。可以是感叹号(`！`)，at 符号(`@`)，逗号(`，`)，问号(`？`)、冒号(`:`)、破折号(`-`) 等以及特殊字符如美元符号(`$`)、等号(`=`)、加号(`+`)、撇号(`'`)。
...
另一种方法涉及使用[正则表达式](https://www.geeksforgeeks.org/regular-expressions-in-java/)。可以使用正则表达式 `[^a-zA-Z0-9]` 轻松过滤字符串。