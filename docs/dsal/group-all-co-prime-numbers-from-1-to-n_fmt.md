...
> 原文: [https://www.geeksforgeeks.org/group-all-co-prime-numbers-from-1-to-n/](https://www.geeksforgeeks.org/group-all-co-prime-numbers-from-1-to-n/)
...
这个问题的关键观察是两个连续的数总是同素的。也就是 `GCD(a, a+1) = 1`。另一个重要的观察是偶数不能列在一组中。因为它们会导致最大公约数为 2。因此，每一个连续的偶数和奇数可以组成一个组，`1` 可以在任何组中，因为 `1` 的数字的最大公约数总是 1。
