...
> 原文: [https://www.geeksforgeeks.org/number-of-ways-to-make-binary-string-of-length-n-such-that-0s-always-occur-together-in-groups-of-size-k/](https://www.geeksforgeeks.org/number-of-ways-to-make-binary-string-of-length-n-such-that-0s-always-occur-together-in-groups-of-size-k/)
...
*   现在，如果我们考虑 `i > k`。如果我们决定第 `i` 个字符为“1”，那么 `dp[i] = dp[i-1]`，因为二进制字符串的数量不会改变。然而，如果我们决定第 `i` 个字符为“0”，那么我们要求先前的 `k-1` 个字符也应为“0”，因此 `dp[i] = dp[i-k]`。因此，`dp[i]` 将是这两个值的总和。
