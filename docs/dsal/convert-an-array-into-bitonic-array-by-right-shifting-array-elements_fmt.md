...
> 原文：[https://www.geeksforgeeks.org/convert-an-array-into-bitonic-array-by-right-shifting-array-elements/](https://www.geeksforgeeks.org/convert-an-array-into-bitonic-array-by-right-shifting-array-elements/)

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是对数组元素执行右移位操作，将给定数组转换成双音素数组。
...
> *   `5` → `00000101` → 4 次右移 → `01010000` → `80`
> *   `3` → `00000011` → 4 次右移 → `00110000` → `48`
...
*   使用右移操作，最大化数组的中间元素，即 `arr[N/2]`。
*   遍历给定的数组 `arr[]`，通过对数组元素执行右移操作（如果需要）以升序转换数组的前半部分。
*   遍历给定数组 `arr[]`，通过对数组元素的右移操作（如果需要）以降序转换数组的后半部分。
*   完成上述步骤后，检查数组是否为二进制。然后打印数组 `arr[]` 作为结果数组。
*   否则，打印 `-1`。
...
**时间复杂度：** `O(N * log(M))`，其中 `M` 为 `arr` 的最大元素。
**辅助空间：** `O(1)`