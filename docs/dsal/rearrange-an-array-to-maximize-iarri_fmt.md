# 重新排列一个数组，使 `i*arr[i]` 最大化

> 原文: [https://www.geeksforgeeks.org/rearrange-an-array-to-maximize-iarri/](https://www.geeksforgeeks.org/rearrange-an-array-to-maximize-iarri/)

给定一个 `N` 个整数的数组，你必须以任何顺序选择所有这些整数。对于您选择的每个整数，您得到的点数等于：**所选整数 * 当前整数之前所选整数的数量。** 您的任务就是将这些点最大化。
**注意：** 可以选择每一个整数正好 1 次。
