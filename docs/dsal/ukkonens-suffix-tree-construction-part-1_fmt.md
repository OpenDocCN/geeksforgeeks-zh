...
> 原文：[https://www.geeksforgeeks.org/ukkonens-suffix-tree-construction-part-1/](https://www.geeksforgeeks.org/ukkonens-suffix-tree-construction-part-1/)
...
**注**：当第 1 次或第 2 次阅读时，你可能会发现算法的某些部分很难理解，它非常好。只要多做一些尝试和思考，你就应该能够理解这些部分。
...
- 蓝色路径的字符串深度为 4，表示从位置 3 开始的后缀 `bxca`。
- 绿色路径的字符串深度为 2，表示从位置 5 开始的后缀 `ac`。
...
这里我们将有 5 个后缀：`xabxa`、`abxa`、`bxa`、`xa` 和 `a`。后缀 `xa` 和 `a` 的路径不以叶子结束。像上面这样的树（图 2）被称为隐式后缀树，因为一些后缀（`xa` 和 `a`）在树中看不到。
...
给定长度为 `m` 的字符串 `S`，输入后缀 `S` 的单个边 `[1..m]$`（整个字符串）到树中，然后依次输入后缀 `S[i..m]$` 到正在生长的树中，因为 `I` 从 2 增加到 `m`。让 `N_i` 表示编码从 1 到 `I` 的所有后缀的中间树。因此 `N_i+1` 由 `N_i` 构建如下：

- 从 `N_i` 的根开始。
- 找到从根开始匹配前缀 `S[I+1..m]$` 的最长路径。
- 匹配结束于节点（比如 `w`）或边的中间[比如 `(u，v)`]。
- 如果它在一条边 `(u，v)` 的中间，通过在边上与 `S[I+l]` 中的字符匹配的最后一个字符之后插入一个新的节点 `w`，将这条边 `(u，v)` 分成两条边。就在不匹配的第一个字符之前。新边 `(u，w)` 用 `(u，v)` 标签中与 `S[I+1]` 匹配的部分进行标记，新的边 `(w，v)` 用 `(u，v)` 标签的剩余部分来标记。
- 创建从 `w` 到标记为 `i+1` 的新叶的新边 `(w，i+1)`，并用后缀 `S[I+1]` 的不匹配部分标记新边。

这需要 `O(m^2)` 来为长度为 `m` 的字符串 `S` 构建后缀树。
以下是基于字符串 `xabxa$` 构建后缀树的几个步骤；基于以上算法：
...
Ukkonen 算法为每个前缀 `S[l..长度为米]` 构造一个隐式后缀树 `T_i`。
首先使用第 1 个字符构建 `T_1`，然后使用第 2 个字符构建 `T_2`，然后使用第 3 个字符构建 `T_3`，然后使用第 `m` 个字符构建…`T_m`。
隐式后缀树 `T_i+1` 建立在隐式后缀树 `T_i` 之上。
`S` 的真后缀树是由 `T_m` 通过加 `$`。
在任何时候，Ukkonen 的算法都会为目前看到的字符构建后缀树，因此它具有**在线**属性，在某些情况下可能会有用。
所用时间为 `O(m)`。

Ukkonen 的算法分为 `m` 个阶段（长度为 `m` 的字符串中每个字符一个阶段）。
在阶段 `i+1` 中，树 `T_i+1` 由树 `T_i` 构建而成。

每个 `i+1` 阶段进一步分为 `i+1` 扩展，每个扩展对应 `S[1..i+1]` 的 `i+1` 后缀。
在阶段 `i+1` 的扩展 `j` 中，算法首先从标有子串 `S[j..我]` 的根找到路径的末端。
然后，它通过将字符 `S(i+1)` 添加到它的末尾来扩展子串（如果它还不存在的话）。
在 `i+1` 期的延伸段 1 中，我们放入字符串 `S[1..我+1]` 在树上。这里是 `[1..我]` 由于之前的第一阶段，将已经出现在树中。我们只需要在树中添加第 `S[I+1]` 个字符（如果还没有的话）。
在 `i+1` 期的延伸 2 中，我们放入字符串 `S[2..我+1]` 在树上。这是 `[2..i]` 由于之前的第一阶段，将已经出现在树中。我们只需要在树中添加第 `S[i+1]` 个字符（如果还没有的话）。
在第 `I+1` 阶段的扩展 3 中，我们将字符串 `S[3..我+1]` 在树上。这是 `[3..我]` 由于之前的第一阶段，将已经出现在树中。我们只需要在树中添加第一个字符（如果还没有的话）。
...
...
在阶段 `i+1` 的扩展 `j` 中，算法找到 `S[j..i]` 的结束（由于之前的阶段 `I`，它已经在树中），然后它扩展了 `S[j..我]` 为了确定后缀 `S[j..i+1]` 在树上。
...
如果 `s[1..i]` 在非叶边缘内部（中间）结束，还将创建一个新的内部节点。
