...
> 原文: [https://www.geeksforgeeks.org/recursive-sum-digit-nx-n-x-large/](https://www.geeksforgeeks.org/recursive-sum-digit-nx-n-x-large/)
...
让 `b = x % 6`
然后
`SoD(n^x) = SoD(a^b)` 除了当 `a = 3` 或 `6`
`sod(n^x) = 9` for all `x>1`，当 `a = 3` 或 `6` 时
