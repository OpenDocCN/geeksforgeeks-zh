...
> 原文: [https://www.geeksforgeeks.org/optimal-strategy-for-the-divisor-game-using-dynamic-programming/](https://www.geeksforgeeks.org/optimal-strategy-for-the-divisor-game-using-dynamic-programming/)

给定一个整数 `N` 和两个玩家，A 和 B 在玩一个游戏。在每个玩家的回合中，该玩家通过从当前 `N` 中减去当前 `N` 的除数（小于 `N`），从而形成下一回合的新 `n`。没有任何除数可减的玩家输掉游戏。任务是告诉哪一个玩家赢了游戏，如果玩家 A 第一个回合，假设两个玩家都玩得最好。
