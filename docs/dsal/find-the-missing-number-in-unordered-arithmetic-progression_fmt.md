...
> 原文：[https://www.geeksforgeeks.org/find-the-missing-number-in-unordered-arithmetic-progression/](https://www.geeksforgeeks.org/find-the-missing-number-in-unordered-arithmetic-progression/)
...
想法是用[二分搜索法](https://www.geeksforgeeks.org/binary-search/)。对给定的数组进行排序，然后该数组将按照 AP 系列的排序顺序排列，我们可以应用二分搜索法（如本文中的）如下所述：

1.  找到中间元素，检查中间元素和中间元素的下一个元素之间的差是否等于公共差，如果不是，则缺少的元素位于 `中间` 和 `中间 + 1` 之间。
2.  如果中间元素等于给定算术级数中的 `(N/2)` 第 `th` 项，则缺失的元素位于中间元素的右侧。
3.  否则该元素位于中间元素的左侧。
...
为了优化上述方法我们将使用 XOR 的以下属性即 `a ^ a = 0`，因此，`(a ^ b ^ c) ^ (a ^ c) = b`。以下是步骤:
