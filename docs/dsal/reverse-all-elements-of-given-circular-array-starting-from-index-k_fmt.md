...
> 原文: [https://www.geeksforgeeks.org/reverse-all-elements-of-given-circular-array-starting-from-index-k/](https://www.geeksforgeeks.org/reverse-all-elements-of-given-circular-array-starting-from-index-k/)
...
解决给定问题，思路是用[两点逼近](https://www.geeksforgeeks.org/two-pointers-technique/)。按照以下步骤解决问题:

*   初始化三个变量 `start` 为 `K`，`end` 为 `(K–1)`，使用`两个指针`接近来跟踪边界，并且 `count` 为 `N / 2`。
*   迭代直到 `count` 的值为正，并执行以下步骤:
    *   [交换](https://www.geeksforgeeks.org/swap-two-numbers-without-using-temporary-variable/)元素 `arr[start % N]` 和 `arr[end % N]`。
    *   通过 `1` 增加 `start`，通过 `1` 减少 `end`。如果 `end` 等于 `-1`，则更新 `end` 为 `(N–1)`。
    *   将 `count` 减 1。
