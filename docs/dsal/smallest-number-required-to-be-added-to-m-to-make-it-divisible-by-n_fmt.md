...
> 原文: [https://www.geeksforgeeks.org/smallest-number-required-to-be-added-to-m-to-make-it-divisible-by-n/](https://www.geeksforgeeks.org/smallest-number-required-to-be-added-to-m-to-make-it-divisible-by-n/)
...
思路是找到大于等于 `M` 的最小数，即被 `N` 整除，然后从中减去 `M`。要得到 `N >= M` 的最小倍数，将 `M + N` 除以 `N`。如果余数为 `0`，则值为 `M`。否则，值为 `M + N - 余数`。
