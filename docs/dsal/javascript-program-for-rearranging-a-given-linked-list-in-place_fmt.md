...
> 原文: [https://www.geeksforgeeks.org/javascript-program-for-rearranging-a-given-linked-list-in-place/](https://www.geeksforgeeks.org/javascript-program-for-rearranging-a-given-linked-list-in-place/)

给定一个单链表 `L0->L1->……->Ln-1->Ln`。重新排列列表中的节点，使新形成的列表为: `L0->Ln->L1->Ln-1->L2->Ln-2`……
...
上述简单解法的时间复杂度为 `O(n^2)`，其中 `n` 为链表中的节点数。
...
1.  取两个指针 `prev` 和 `curr`，保存 `head` 和 `head->next` 的地址。
2.  比较他们的数据并交换。
3.  之后，形成新的链表。
