...
> 原文：[https://www.geeksforgeeks.org/walmart-labs-interview-experience-set-5-on-campus/](https://www.geeksforgeeks.org/walmart-labs-interview-experience-set-5-on-campus/)

最近，沃尔玛实验室来到我们的校园进行校园招聘。以下是我的面试经历：

## 第一轮（MCQ + 编码题）

这一轮是在`黑客地球`上进行的，包括 12 道来自 `DS`、`OOPS`、智能、网络等的 `MCQ` 题和 3 道编码题。

### Q1
给定 `n` 个由‘R’和‘B’组成的字符串。只有当第一个字符串的最后一个字符和第二个字符串的第一个字符相同时，才能组合两个字符串。给定 `n` 个字符串，您必须通过组合字符串来输出可能的最大长度。[（练习链接）](https://practice.geeksforgeeks.org/problems/combine-the-strings/0)
...
### Q2
找出素数范围内出现的最大位数。[（练习链接）](https://practice.geeksforgeeks.org/problems/find-the-highest-occurring-digit-in-prime-numbers-in-a-range/0)
例如：范围(21-40) -> 素数为 23，29，31，37，39 ( 2 出现 2 次，3 出现 4 次，1 出现 1 次和 9 出现 2 次 -> 答案为 3)
...
b. 这个问题被问到：[Count all possible paths from top left to bottom right](https://practice.geeksforgeeks.org/problems/count-all-possible-paths-from-top-left-to-bottom-right/0)。
...
- [Combine the strings](https://practice.geeksforgeeks.org/problems/combine-the-strings/0)