...
> 原文: [https://www.geeksforgeeks.org/segment-tree-set-1-range-minimum-query/](https://www.geeksforgeeks.org/segment-tree-set-1-range-minimum-query/)

我们在上一篇文章中用一个简单的例子介绍了[段树](https://www.geeksforgeeks.org/segment-tree-set-1-sum-of-given-range/)。在这篇文章中，[范围最小查询](http://en.wikipedia.org/wiki/Range_Minimum_Query)问题作为另一个可以使用段树的例子进行了讨论。以下是问题陈述：
...
一个简单的解决方案是运行一个从 `qs` 到 `qe` 的循环，找到给定范围内的最小元素。这种解决方案在最坏的情况下需要 `O(n)` 的时间。

### 预处理的解决方案

另一个解决方案是创建一个二维数组，其中一个条目 `[i, j]` 存储范围 `arr[i..j]` 中的最小值。现在可以用 `O(1)` 时间计算给定范围的最小值，但是预处理需要 `O(n^2)` 时间。此外，这种方法需要 `O(n^2)` 额外的空间，这对于大型输入数组来说可能会变得很大。

### 段树解决方案

[段树](https://www.geeksforgeeks.org/segment-tree-set-1-sum-of-given-range/) 可以在适中的时间内做预处理和查询。对于段树，预处理时间为 `O(n)`，范围最小查询的时间复杂度为 `O(log n)`。存储段树所需的额外空间为 `O(n)`。
...
我们从线段 `arr[0...n-1]` 开始。并且每次我们将当前段分成两半(如果它还没有变成长度为 1 的段)，然后在两半上调用相同的过程，并且对于每个这样的段，我们将最小值存储在段树节点中。
...
段树高度为 `⌈log₂n⌉`。由于树是用数组表示的，并且必须保持父索引和子索引之间的关系，因此分配给段树的内存大小将是 `2 * 2^⌈log₂n⌉ – 1`。
...
查询的时间复杂度为 `O(Logn)`。为了查询最小范围，我们在每个级别最多处理两个节点，级别数为 `O(Logn)`。
...
- [Range Minimum Query (RMQ)](http://community.topcoder.com/tc?module=Static&d1=tutorials&d2=lowestCommonAncestor#Range_Minimum_Query_(RMQ))
- [Range minimum query](http://wcipeg.com/wiki/Range_minimum_query)
