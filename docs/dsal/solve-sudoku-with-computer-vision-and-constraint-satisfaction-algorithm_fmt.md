...
> 原文: [https://www.geeksforgeeks.org/solve-sudoku-with-computer-vision-and-constraint-satisfaction-algorithm/](https://www.geeksforgeeks.org/solve-sudoku-with-computer-vision-and-constraint-satisfaction-algorithm/)
...
第一步，图像预处理:单独提取每个数独方块，依次保存为`photo#.png`（其中`#`从 0 到 80）。获得了 80×75 像素的图像。
...
If it is different from an empty box -> in empty boxes the algorithm marks zero because it does not find anything.
```python
    if (contours != 0): 
