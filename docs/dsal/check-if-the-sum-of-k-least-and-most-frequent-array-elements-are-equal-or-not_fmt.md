...
> 原文: [https://www.geeksforgeeks.org/check-if-the-sum-of-k-least-and-most-frequent-array-elements-are-equal-or-not/](https://www.geeksforgeeks.org/check-if-the-sum-of-k-least-and-most-frequent-array-elements-are-equal-or-not/)

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是检查 `K` 个最频繁数组元素的和与 `K` 个最不频繁数组元素的和在数组 `arr[]` 中是否相等。如果发现为真，则打印“是”。否则，打印“否”。
...
给定的问题可以通过用频率索引散列找到 `K` 个最频繁的元素来解决，根据频率数组找到 `K` 个最频繁的元素的和等于数组 `arr[]` 中 `K` 个最不频繁的元素的和。按照以下步骤解决问题:

*   初始化一个无序图，说 `M` 统计每个数组元素的频率。
*   迭代一个范围 `[0, N]` 并将每个数组元素的频率存储在无序映射 `M` 中。
*   初始化尺寸为 `N + 1` 的二维向量 `freq`，以在地图 `M` 中以给定频率存储元素。
*   迭代一个范围 `[0, N]` 并执行以下步骤:
    *   将变量 `f` 初始化为 `arr[i]` 的频率，即 `M[arr[i]]`。
    *   如果 `f` 不等于 `-1`，则将元素 `arr[i]` 推入向量 `freq` 为频率 `f`，并将 `M[arr[i]]` 的值设置为 `-1`。
*   将变量 `count` 初始化为 `0`，以跟踪 `K` 最频繁元素和 `K` 最不频繁数组元素。
*   将变量 `kleastfreqelem` 初始化为 `0`，存储 `K` 个最不频繁数组元素的和。
*   使用变量 `i` 迭代一个范围 `[0, N]`，并执行以下步骤:
    *   迭代一个范围 `[0, freq[i]]` 并执行以下步骤:
        *   将 `freq[i][j]` 的值添加到变量 `kleastfreqelem` 中，并将 `count` 的值增加 `1`。
        *   如果 `count` 等于 `K`，则脱离循环。
    *   如果 `count` 等于 `K`，则断开回路。
*   将 `count` 的值设置为 `0`。
*   将变量 `kmostfreqelem` 初始化为 `0`，以存储数组 `arr[]` 的 `K` 个最不频繁元素之和。
*   使用变量 `i` 迭代一个范围 `[N-1, 0]`，并执行以下步骤:
    *   迭代一个范围 `[0, freq[i]]` 并执行以下步骤:
        *   将 `freq[i][j]` 的值添加到变量 `kmostfreqelem` 中，并将 `count` 的值增加 `1`。
        *   如果 `count` 等于 `K`，则脱离循环。
    *   如果 `count` 等于 `K`，则脱离循环。
