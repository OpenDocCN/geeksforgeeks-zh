...
> 原文: [https://www.geeksforgeeks.org/order-of-teams-in-a-tournament-such-that-every-team-has-won-against-its-consecutive-team/](https://www.geeksforgeeks.org/order-of-teams-in-a-tournament-such-that-every-team-has-won-against-its-consecutive-team/)
...
> **输入:** `N = 5`
> `results[] = {{1, 4}, {4, 3}, {2, 3}, {1, 2}, {2, 1}, {3, 1}, {4, 2}, {2, 5}, {5, 3}, {4, 5}, {5, 1}}`
> **输出:** `4 2 5 3 1`

## 方法

想法是为每个队维护一个散列图，其中存储了该队在循环锦标赛中获胜的队。这可以通过对 `results` 的元素进行迭代来完成，对于每个获胜者，将它附加到该团队的哈希映射对象列表中。
    ...
    static void arrangeTeams(int[] teams, Result[] results) {
        ...
        for (int i = 0; i < results.length; i++) {
            ...
            if (map.containsKey(winner)) {
                map.get(winner).add(results[i].loser);
            } else {
