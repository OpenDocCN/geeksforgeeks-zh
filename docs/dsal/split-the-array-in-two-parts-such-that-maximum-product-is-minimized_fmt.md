...
> 原文: [https://www.geeksforgeeks.org/split-the-array-in-two-parts-such-that-maximum-product-is-minimized/](https://www.geeksforgeeks.org/split-the-array-in-two-parts-such-that-maximum-product-is-minimized/)

给定一个整数数组，大小为 `N(<= 16)` 的 `arr[]`，任务是将数组划分为 2 个部分，使得 2 个半部分的最大乘积最小。求一半的最小最大乘积。如果数组为空，打印 `-1`。
...
**输出:** `15`
**解释:** 可能的分区是–
- `{5, 7}, {3}` – 这里的乘积是 35 和 3，最大乘积是 35
- `{3, 7}, {5}` – 这里的乘积是 21 和 5，最大乘积是 21
- `{3, 5}, {7}` – 这里的乘积是 15 和 7，最大乘积是 15
- `{5, 7, 3}, {}` – 这里的乘积是 105 和 0，最大乘积是 105
...
由于 `N` 的值小于 16，这个问题可以通过使用位屏蔽来解决，即将位于设定位位置的所有数字相乘并将其放入一侧，类似地，将所有未设定位位置相乘并将其存储在另一半中，找到最大值并将其存储在集合中，最后返回集合的第一个元素。

按照以下步骤解决问题:

### 步骤

1.  初始化一个集合 `st[]` 来按顺序存储可能的答案。
2.  使用变量 `i` 迭代范围 `[0, 2^N)`，并执行以下任务:
    *   将变量 `product1` 和 `product2` 初始化为 `1`。
    *   使用变量 `j` 迭代范围 `[0, N)`，并执行以下任务:
        *   如果 `i & (1 << j)` 为真，则将值 `arr[j]` 乘以变量 `product1`，否则乘以变量 `product2`。
    *   将 `product1` 或 `product2` 的最大值插入 `st[]`。
3.  执行上述步骤后，打印集合的第一个值作为答案。
