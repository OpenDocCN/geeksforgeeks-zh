...
给定两个整数 `N` 和 `D`，任务是找到包含所有长度为 `N` 的排列的最小字符串，该排列可以使用前 `D` 个数字 `(0, 1, …, D-1)` 形成。
...
**解释:**
这里从数字 `{0, 1, 2, 3}` 开始的长度 2 的所有可能排列都是 `00 10 20 30 ...`。

## 方法

追加 `'0'` `N-1` 次，在当前状态下调用字符串上的 [DFS](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/)。逐个追加所有的 `D` 个字符。每次追加后，检查新字符串是否被访问。如果是，将其插入 [HashSet](https://www.geeksforgeeks.org/hashset-in-java/) 标记为已访问，并在答案中添加该字符。递归调用最后 `D` 个字符上的 `dfs` 函数。重复这个过程，直到从 `D` 位开始的所有可能的 `N` 长度子串都被附加到字符串中。打印生成的最终字符串。
...
class GeeksforGeeks {
    ...
    public static String reqString(int N, int D)
    {
        // Base case
        if (N == 1 && D == 1)
            return "0";
