...
**对象数组：** 这是一个[数组](https://www.geeksforgeeks.org/introduction-to-arrays/)，其元素属于[类](https://www.geeksforgeeks.org/c-classes-and-objects/)类型。它可以声明为任意[数据类型](https://www.geeksforgeeks.org/c-data-types/)的数组。

**语法：**

> `ClassName arrayName[size];`

下面是[C++](https://www.geeksforgeeks.org/c-plus-plus/)程序，通过计算3个学生的最高分来说明对象的数组：
...
*   在 [`main()`](https://www.geeksforgeeks.org/executing-main-in-c-behind-the-scene/) 中，创建了`Student`类的功能对象：
    ...
    *   在`for`循环中，已经进行了3组用户输入（即，用户将在此输入3组学生的姓名、姓名、总成绩）。
    *   然后通过`s1`对象调用`pos(Student obj[], int size)`成员函数，传递`s[3]`（包含学生详细信息集的对象数组）及其大小。
*   `pos(Student obj[], int size)`函数，返回学生集合总分最高的对象的位置，即`s[3]`对象数组的（0，1 或 2）索引位置，存储在`pos = s1.pos(s, 3)`中。
*   **显示部分：** 调用显示函数`s[pos].putdata()`。
    *   `putdata()`函数是显示`Student`类的对象详情。
    *   在这里，`pos`在`s`中存储（存储最高学生集合对象的索引位置），显示总分最高的学生详细信息。