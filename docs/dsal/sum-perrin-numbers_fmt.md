...
给定一个正数 `n`，求 `P0 + P1 + P2 + ... + Pn` 的值，其中 `pi` 表示第 `i` 个 Perrin 数。前几个佩兰数字是 3，0，2，3，2，5，5，7……。
...
在之前的帖子中，我们已经介绍了[佩兰号码](https://www.geeksforgeeks.org/program-perrin-numbers/)。在数学术语中，佩兰数的序列 `p(n)` 由递归关系定义：
...
P(n) + P(n-1) - P(1) - P(2) which is S(n-3).
Therefore,
...
本文由 **GeeksforGeeks** 署名作者 **Raza** 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 [contribute.geeksforgeeks.org](http://www.contribute.geeksforgeeks.org) 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
