...
> 原文：[https://www.geeksforgeeks.org/find-smallest-number-n-set-m-unset-bits/](https://www.geeksforgeeks.org/find-smallest-number-n-set-m-unset-bits/)
...
1.  计算 `num = (1 << (n + m)) - 1`。这将产生具有 `(n + m)` 比特数的数字 `num`，并且所有比特都被设置。
2.  现在，在 `num` 中从 `n` 到 `(n + m - 1)` 的范围内切换位，即从最右边的第 `n` 位切换位到最右边的第 `(n + m - 1)` 个位，然后返回切换后的编号。参考[这个](https://www.geeksforgeeks.org/toggle-bits-given-range/)岗位。
...
对于 `n` 和 `m` 的较大值，可以使用 `long int` 和 `long long int` 数据类型来生成所需的数字。
