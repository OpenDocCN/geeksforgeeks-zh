...
> 原文: [https://www.geeksforgeeks.org/remove-last-occurrence-of-a-word-from-a-given-sentence-string/](https://www.geeksforgeeks.org/remove-last-occurrence-of-a-word-from-a-given-sentence-string/)

给定两个大小分别为 `N` 和 `M` 的[字符串](https://www.geeksforgeeks.org/string-data-structure/) `S` 和 `W`，任务是从 `S` 中移除最后一个出现的 `W`。如果 `S` 中没有出现 `W`，则按原样打印 `S`。

**示例:**

> **输入:** `S = "This is GeeksForGeeks"`，`W = "Geeks"`
> **输出:** `This is GeeksFor`
> **解释:**
...
通过对字符串 `S` 的每个索引 `i` 进行迭代，检查是否有一个从索引 `i` 开始的子串，该子串等于字符串 `W`，可以解决这个问题。按照以下步骤解决问题:
