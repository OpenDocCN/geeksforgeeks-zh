# 根据给定的移除元素规则找到最小可能的数组大小

> 原文: [https://www.geeksforgeeks.org/find-minimum-possible-size-of-array-with-given-rules-for-removal/](https://www.geeksforgeeks.org/find-minimum-possible-size-of-array-with-given-rules-for-removal/)
...
2) 或元素被移除(如果它遵循移除规则)。当一个元素被移除时，还有两种可能。
    ...
    b) 存在 `x` 和 `y`，使得 `arr[x]–arr[i]= k`，`arr[y]–arr[x]= k`，且子阵列 `arr[i+1…x-1]` 和 `arr[x+1…y-1]` 可以被完全移除。
