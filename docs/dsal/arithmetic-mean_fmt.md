...
1.  `n` 个数 `x1`，`x2`，...，`xn` 的平均值为 `x`。如果每个观察值增加 `p`，则新观察值的平均值为 `(x + p)`。
2.  `n` 个数 `x1`，`x2`，...，`xn` 的平均值为 `x`。如果每个观察值减少 `p`，则新观察值的平均值为 `(x–p)`。
3.  `n` 个数 `x1`，`x2`，...，`xn` 的平均值为 `x`。如果每个观测值乘以一个非零数 `p`，新观测值的平均值为 `px`。
4.  `n` 个数 `x1`，`x2`，...，`xn` 的平均值为 `x`。如果每个观测值除以一个非零数 `p`，则新观测值的平均值为 `(x/p)`。
...
让 `A1`，`A2`，`A3`，`A4` ……`An` 为两个给定数字 `A` 和 `B` 之间的 `N` 个算术平均值，然后 `A`，`A1`，`A2` …..`An`，`B` 将处于算术级数。现在 `B` 是算术级数的第 `(N+2)` 项。所以:
...
我们有 `A` 的值和公差 `d` 的值，现在我们可以找到 `A` 和 `B` 之间所有的 `N` 个算术平均值。
