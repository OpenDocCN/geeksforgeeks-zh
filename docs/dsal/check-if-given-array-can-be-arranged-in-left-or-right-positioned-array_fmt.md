...
> 原文：[https://www.geeksforgeeks.org/check-if-given-array-can-be-arranged-in-left-or-right-positioned-array/](https://www.geeksforgeeks.org/check-if-given-array-can-be-arranged-in-left-or-right-positioned-array/)
...
针对这个问题的一个**高效解决方案**需要一点观察和纸笔工作。为了满足左或右定位数组条件，数组中的所有数字应该等于 `index`，`i` 或 `(n-1-i)` 和 `arr[i] < n`。因此，我们创建了一个大小为 `n` 的 `visited[]` 数组，并用 `0` 初始化它的元素。然后我们遍历数组并遵循给定的步骤：
...
*   现在遍历 `visited[]` 数组，如果 `visited[]` 数组的所有元素都变成 `1`，这意味着排列是可能的“是”或者“否”。
...
**时间复杂度:**`O(n)`
**辅助空间:** `O(n)`
本文由 [**沙莎克·米什拉(古鲁)**](https://practice.geeksforgeeks.org/user-profile.php?user=Shashank%20Mishra) 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用`contribute.geeksforgeeks.org`写一篇文章或者把你的文章邮寄到`contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
