# 程序找出任意两个不同数字的索引之间的最大差值

> 原文: [https://www.geeksforgeeks.org/program-to-find-the-maximum-difference-between-the-index-of-any-two-different-numbers/](https://www.geeksforgeeks.org/program-to-find-the-maximum-difference-between-the-index-of-any-two-different-numbers/)

给定一组 `N` 个整数。任务是找出任何两个不同数字的索引之间的最大差异。注意至少有两个不同的数字。
...
**方法:** 最初，从末尾开始检查与 `a[0]` 不同的第一个数字，将它们的索引之差存储为 `ind1`。此外，从一开始就检查第一个不同于 `a[n-1]` 的数字，将它们的索引之差存储为 `ind2`。答案将是 `max(ind1, ind2)`。
