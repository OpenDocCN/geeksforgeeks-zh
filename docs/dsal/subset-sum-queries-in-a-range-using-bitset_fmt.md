...
> 原文: [https://www.geeksforgeeks.org/subset-sum-queries-in-a-range-using-bitset/](https://www.geeksforgeeks.org/subset-sum-queries-in-a-range-using-bitset/)

给定一个由 `N` 个正整数和 `M` 个查询组成的数组 `[]`。每个查询由两个整数组成 `L` 和 `R`，用一个范围表示。对于每个查询，找出位于给定范围内的数字的计数，该计数可以表示为给定数组的任何子集的和。

**先决条件:** [使用位集的子集和查询](https://www.geeksforgeeks.org/subset-sum-queries-using-bitset/)
...
想法是使用一个 `bitset` 并迭代数组来表示所有可能的子集和。`bitset` 的当前状态是通过将 `bitset` 的先前状态左移 `X` 次来对其进行“或”来定义的，其中 `X` 是数组中当前处理的元素。为了在 `O(1)` 时间内回答查询，我们可以预计算到每个数字的数字计数，对于一个范围 `[L, R]`，答案将是 `pre[R]–pre[L–1]`，其中 `pre[]` 是预计算的数组。
...
每个查询都可以在 `O(1)` 时间内回答，预计算需要 `O(MAX)` 时间。