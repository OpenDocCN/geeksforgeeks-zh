...
> 原文: [https://www.geeksforgeeks.org/find-the-element-that-odd-number-of-times-in-olog-n-time/](https://www.geeksforgeeks.org/find-the-element-that-odd-number-of-times-in-olog-n-time/)
...
一个**简单解决方案**是对数组进行排序，然后从左到右遍历数组。由于数组是排序的，我们可以很容易地计算出所需的元素。这个解的时间复杂度是`O(n Log n)`。

一个**更好的解**是对所有元素进行异或，异或的结果会给出奇数出现的元素。这个解的时间复杂度是`O(n)`。详见[出现奇数次的元素](https://www.geeksforgeeks.org/find-the-number-occurring-odd-number-of-times/)中的基于异或的解决方案。
...
3) 如果`mid`是奇数，则比较`arr[mid]`和`arr[mid–1]`。如果两者相同，那么在`mid`之后或`mid`之前会出现一个奇怪的事件。
