...
> 原文: [https://www.geeksforgeeks.org/check-if-it-is-possible-to-assign-values-such-that-all-the-given-relations-are-satisfied/](https://www.geeksforgeeks.org/check-if-it-is-possible-to-assign-values-such-that-all-the-given-relations-are-satisfied/)

给定一组字符串 `arr[]`，其中每个 `arr[i]` 的形式为 `"i == j"` 或 `"i != j"`，其中 `i` 和 `j` 是表示它们之间关系的变量，任务是检查是否有可能给满足所有关系的变量赋值。如果发现是真的，则打印 `"Yes"`。否则，打印 `"No"`。
...
> **输出:** No
> **解释:**
> 第一个关系对于值 `i = 1` 和 `j = 1` 成立，但是第二个关系对于同一组值失败。因此，打印 No。
>
> **输入:** `arr[] = {"p == q", "q == r", "p == r"}`
> **输出:** Yes
> **说明:**
> `p`、`q`、`r` 中值 `1` 的赋值满足全部 3 个关系。因此，打印 "Yes"。

**方法:** 解决给定问题的方法是使用[并查集算法](https://www.geeksforgeeks.org/union-find-algorithm-set-2-union-by-rank/)。其思想是[遍历字符串数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)，遍历每一个等式关系，对两个变量进行并集运算。遍历之后，转到每个字符串中的每个不等关系，检查两个变量的父节点是否相同。按照以下步骤解决问题:

1.  使用 `0` 和变量将大小为 `26` 的[数组](https://www.geeksforgeeks.org/array-data-structure/) `parent[]` 初始化为 `true` 以存储所需结果。
2.  [使用变量 `i` 遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `parent[]`，将 `parent[i]` 设置为 `i`。
...
4.  再次，[遍历字符串数组中的每个字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-c/) `S`，并执行以下操作:
    1.  如果 `S[i][1]` 的值等于 `'!'`，将 `S[i][0 - 'a']` 和 `S[i][3 - 'a']` 的父节点分别存储在 `x` 和 `y` 中。
    2.  如果 `x` 的值等于 `y`，则将 `ans` 设置为 `false`。
5.  经过上述步骤后，如果 `ans` 的值为 `true`，则打印 `"Yes"`，否则打印 `"No"`。
