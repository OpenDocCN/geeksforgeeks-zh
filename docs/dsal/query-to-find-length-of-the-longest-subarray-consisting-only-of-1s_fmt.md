...
> 原文: [https://www.geeksforgeeks.org/query-to-find-length-of-the-longest-subarray-consisting-only-of-1s/](https://www.geeksforgeeks.org/query-to-find-length-of-the-longest-subarray-consisting-only-of-1s/)

给定大小为 `N` 的二进制数组 `arr[]` 和包含以下两种类型的 `K` 查询的二维数组 `Q[][]` :

*   **1 :** 只打印由 `1` s 组成的最长子阵列的长度。
*   **2 X :** 在 `X` 索引处翻转元素（*基于 1 的索引*），即如果当前元素为 `"0"`，则将元素更改为 `"1"`，反之亦然。
...
**天真方法:** 解决问题最简单的方法是遍历数组 `arr[]` 进行每个查询并执行给定的操作。
...
上述方法可以使用[段树](https://www.geeksforgeeks.org/segment-tree-set-1-sum-of-given-range/)数据结构进行优化。给定的问题可以基于以下观察来解决:
