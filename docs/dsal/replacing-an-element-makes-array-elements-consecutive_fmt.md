...
是要检查`arr[]`的每个元素，在替换后使[连续或不连续](https://www.geeksforgeeks.org/check-array-elements-consecutive-time-o1-space-handles-positive-negative-numbers/)。 此方法的时间复杂度`O(n^2)`。

## 更好的方法

基于以下重要观察结果：如果存在答案，则最小或最大元素将为答案。 如果存在答案，则有两种情况。
