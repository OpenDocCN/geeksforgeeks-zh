...
> 原文: [https://www.geeksforgeeks.org/compute-power-of-power-k-times-m/](https://www.geeksforgeeks.org/compute-power-of-power-k-times-m/)

给定 `x`，`k` 和 `m`。计算 `(x^x^x^...^x) % m`（`x` 为 `k` 次幂）。给定 `x` 总是素数，`m` 大于 `x`。
