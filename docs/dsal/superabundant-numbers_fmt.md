...
一个数是一个**过剩数**如果`σ(n)/n > σ(m)/m` 为所有 `m < n`，其中`σ`是 `n` 的除数之和

> 1, 2, 4, 6, 12, 24, 36, 48…

## 检查 N 是否是一个多余的数字

给定一个数字 `N` ，任务是检查 `N` 是否为**过剩数字**。如果 `N` 是多余的号码，则打印`“是”`否则打印`“否”`。
...
**方法:**对于从 1 到小于 N 的所有数字 `i`，检查`σ(N)/N > σ(m)/m` 是否为假，否则返回真。

**例如:**

> 如果 `N = 4`，则 `i = 1` 至 3 的`σ(4)/4 = 7/4 = 1.75`
>
> `σ(1)/1 = 1/1 = 1`
> `σ(2)/2 = 3/2 = 1.5`
> `σ(3)/3 = 4/3 = 1.333333`
