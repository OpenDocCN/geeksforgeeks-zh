...
> 原文：[https://www.geeksforgeeks.org/how-to-append-a-character-to-a-string-in-c/](https://www.geeksforgeeks.org/how-to-append-a-character-to-a-string-in-c/)
...
1.  获取字符串 `str` 和字符 `ch`。
2.  使用 `strncat()` 函数在字符串末尾追加字符 `ch`。`strncat()` 是用于字符串处理的预定义函数。`string.h` 是字符串函数需要的头文件。
