...
> 原文：[https://www.geeksforgeeks.org/subsequence-pair-from-given-array-having-all-unique-and-all-same-elements-respectively/](https://www.geeksforgeeks.org/subsequence-pair-from-given-array-having-all-unique-and-all-same-elements-respectively/)

给定一个由 **N** 个整数组成的数组`arr[]`，任务是选择两个长度相等的子序列，使得第一个子序列必须具有所有唯一的元素，第二个子序列必须具有所有相同的元素。打印子序列对的最大长度。
...
4.  子序列的最大值是基于以下两种情况给出的：
    *   具有不同元素的子序列必须具有最大频率的元素。因此，`min(d，f–1)`的最小值是具有给定性质的子序列的可能长度。
    *   如果具有不同元素的子序列的长度大于最大频率，那么`min(d–1，f)`的最小值就是具有给定性质的子序列的可能长度。
