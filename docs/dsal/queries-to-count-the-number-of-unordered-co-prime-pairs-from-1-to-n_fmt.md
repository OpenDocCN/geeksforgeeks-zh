...
> 原文: [https://www.geeksforgeeks.org/queries-to-count-the-number-of-unordered-co-prime-pairs-from-1-to-n/](https://www.geeksforgeeks.org/queries-to-count-the-number-of-unordered-co-prime-pairs-from-1-to-n/)
...
这里[欧拉全能性函数](https://www.geeksforgeeks.org/eulers-totient-function-for-all-numbers-smaller-than-or-equal-to-n/)会有帮助。表示为 `φ(N)` 的欧拉全能函数是一个算术函数，它计算小于或等于 `N` 的正整数，这些正整数相对于 `N` 是素数。
...
现在，使用[前缀求和法求出 1 到 `N` 之间所有 `i` 的所有 `φ(x)` 的和。](https://www.geeksforgeeks.org/prefix-sum-array-implementation-applications-competitive-programming/)利用这个，可以在 `O(1)` 时间内回答。
