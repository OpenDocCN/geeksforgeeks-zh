...
> 原文：[https://www.geeksforgeeks.org/count-maximum-elements-of-an-array-whose-absolute-difference-does-not-exceed-k/](https://www.geeksforgeeks.org/count-maximum-elements-of-an-array-whose-absolute-difference-does-not-exceed-k/)
...
> **输出：** 3
> 有最大 **3** 值，因此每对的绝对差值不超过 `K`（`K=5`）即 `{12, 15, 17}`。
> **输入：** `A[] = {1, 2, 5, 10, 8, 3}`，`K = 4`
> **输出：** 4
> 有最大 **4** 值，这样每对的绝对差值不超过 `K`（`K=4`）即 `{1, 2, 3, 5}`。

**进场：**

1.  给定数组按升序排序。
2.  从索引 `i = 0` 迭代到 `n`。
3.  对于每一个 `A[i]`，计算在 `A[i]` 到 `A[i] + K` 范围内的数值，即 `A[i] <= A[j] <= A[i]+K`。
4.  返回最大计数。
