...
给定一个带`V`顶点和`E`边且没有自环和多个边的有向图，任务是找到所需的最少颜色数目，以使相同颜色的边不形成循环并找到每个边的颜色。
...
`说明`：
在上面给出的图中，它仅形成一个循环，即顶点 1、2、3 形成一个循环。然后将连接 1->2 或 2->3 或 3->1 的边涂成一种颜色，并用不同的颜色着色另一条边，从而形成边周期没有相同的颜色。

`输入`：`V = {1,2,3,4,5}`，`E = {（1，2），（1，3），（2，3），（2，4），（3，4），（4，5），（5，3）}`
`输出`：2
边的颜色– 1 1 1 1 1 1 2
`说明`：
在上面给定的图中，它仅形成一个循环，是连接 3、4、5 的顶点，形成一个循环。然后连接 5->3 或 4->5 或 3->4 可以用不同的颜色着色，以使形成循环的边没有相同的颜色。
...
的想法是在图中找到循环，这可以借助 [DFS 完成](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/)，其中当已经访问过的节点再次访问时，一条新的边，然后用另一种颜色着色该边，否则，如果没有循环，则所有边只能用一种颜色着色。

## 算法

1.  用颜色 1 标记每个边，将每个顶点标记为未访问。
2.  使用 [DFS 遍历](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/)遍历该图并标记访问的节点。
3.  如果已经访问了一个节点，则连接顶点的边将标记为用颜色 2 着色。
4.  访问所有顶点时，打印边的颜色。
