...
> 原文：[https://www.geeksforgeeks.org/number-of-ways-to-convert-a-character-x-to-a-string-y/](https://www.geeksforgeeks.org/number-of-ways-to-convert-a-character-x-to-a-string-y/)
...
> 3.  右追加 y(`"ay"`)、左追加 xx(`"xxy"`)、右追加 y(`"xxay"`)。
>
> **输入：** `X = 'a'`，`Y = 'CD'`
> **输出：** 0
...
解决这个问题的一种方法是使用[动态规划](https://www.geeksforgeeks.org/dynamic-programming/)。
...
*   初始化变量 `ans = 0`，`mod = 1000000007`。
*   对于所有索引 `i`，如 `Y[i] = X`，更新答案为 `ans = (ans + F(i)) % mod`，其中 `F(i)=((N–1)!)/(i!*(N–i–1)!))% mod`。

原因上面的公式管用：试着找到问题的答案，找到(L 的 p 数)和(R 的 q 数)的排列数，其中 L 和 R 分别是左追加和右追加运算。
答案是 `(p + q)!/ (p! * q!)`。对于每个有效的 `i`，只需找到 `i` 个 Ls 和 `N–i–1` 个 Rs 的排列数量。
该方法的时间复杂度将为 `O(N)`。
