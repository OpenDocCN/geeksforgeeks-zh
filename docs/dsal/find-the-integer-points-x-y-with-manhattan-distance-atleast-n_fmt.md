...
> 原文: [https://www.geeksforgeeks.org/find-the-integer-points-x-y-with-manhattan-distance-atleast-n/](https://www.geeksforgeeks.org/find-the-integer-points-x-y-with-manhattan-distance-atleast-n/)

给定一个数字 `N`，任务是找到整数点 `(x, y)`，使得任意两点之间的 `0 <= x, y <= N` 且 [曼哈顿距离](https://www.geeksforgeeks.org/sum-manhattan-distances-pairs-points/) 至少为 `N`。
...
*   两点之间的曼哈顿距离 `(x1, y1)` 和 `(x2, y2)` 为: `|x1 – x2| + |y1 – y2|`。
*   这里，对于所有点对，该距离至少为 `n`。
