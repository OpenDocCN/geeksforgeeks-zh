...
3.  对于筛选列表中的每个条目编号 `n`，取模 `60` 余数 `r`:
    1.  如果 `r` 是 `1`、`13`、`17`、`29`、`37`、`41`、`49` 或 `53`，则将每个可能解的条目翻转为 `4x^2 + y^2 = n`。
    2.  如果 `r` 是 `7`、`19`、`31` 或 `43`，则将每个可能解的条目翻转到 `3x^2 + y^2 = n`。
    3.  如果 `r` 是 `11`、`23`、`47` 或 `59`，则将每个可能解的条目翻转到 `3x^2–y^2= n` 当 `x > y`。
    4.  如果 `r` 是别的东西，完全忽略它…
4.  从筛选列表中最低的数字开始。
...
5.  所有具有模 60 余数 7、19、31 或 43 的数都具有模 6 余数 1。当且仅当`3x^2 + y^2 = n`的解的数量为奇数且该数量为无平方时，这些数为素数。
6.  所有具有模 60 余数 11、23、47 或 59 的数都具有模 12 余数 11。当且仅当`3x^2–y^2= n`的解的数量为奇数且该数量为无平方时，这些数为素数。
