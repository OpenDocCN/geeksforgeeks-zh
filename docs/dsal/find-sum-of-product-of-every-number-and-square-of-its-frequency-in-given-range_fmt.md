...
> 原文：[https://www.geeksforgeeks.org/find-sum-of-product-of-every-number-and-square-of-its-frequency-in-given-range/](https://www.geeksforgeeks.org/find-sum-of-product-of-every-number-and-square-of-its-frequency-in-given-range/)

给定一个整数数组`arr`和一个查询数组`queries`，任务是在给定的范围`[L, R]`内找到每个数字与其频率的乘积之和，其中每个范围在查询数组中给定。

## 示例

> **输入：** `arr[] = [1, 2, 1]`, `queries: [{1, 2}, {1, 3}]`
> **输出：** `[3, 4]`
> **解释：**
> 对于查询`[1, 2]`，`freq[1] = 1`，`freq[2] = 1`，`ans = (1 * freq[1]) + (2 * freq[2])` => `ans = (1 * 1 + 2 * 1) = 3`
> 对于查询`[1, 3]`，`freq[1] = 2`，`freq[2] = 1`，`ans = (1 * freq[1]) + (2 * freq[2])` => `ans = (1 * 2) + (2 * 1) = 4`

> **输入：** `arr[] = [1, 1, 2, 2, 1, 3, 1, 1]`, `queries: [{2, 7}, {1, 6}]`
> **输出：** `[10, 10]`
> **解释：**
> 对于查询`(2, 7)`，`freq[1] = 3`，`freq[2] = 2`，`freq[3] = 1`；
> `ans = (1 * freq[1]) + (2 * freq[2]) + (3 * freq[3])`
> `ans = (1 * 3) + (2 * 2) + (3 * 1) = 10`

## 暴力方法

为了解决上面提到的问题，暴力方法是迭代查询中给出的子数组。为子数组中每个数字的频率维护一张[map](https://www.geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/)，并在map上迭代计算答案。
