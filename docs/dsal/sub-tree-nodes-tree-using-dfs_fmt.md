...
给定一棵树的 n 个节点及其连接，打印每个节点的子树节点。节点的**子树**被定义为节点的子树。这个名字强调的是，作为树节点后代的所有东西也是树，并且是更大的树的子集。
...
对每个节点进行 DFS 遍历，并打印从特定节点可到达的所有节点。
...
4.  现在，`dfs(1, 2)` 被调用，现在 `start[1] = 1`，`dfs_order.push_back(1)`，在 `adj[1]` 元素被遍历后 `visited[1] = 1`。
5.  现在 `adj[1]` 被遍历，它只包含节点 2。当 `adj[2]` 被遍历时，它不包含任何元素，它将断开并 `end[1]=2`。
