...
给定一个具有 `N` 个节点和一个整数 `K` 的二叉树，任务是打印二叉树的第 `K` 级的节点而不重复。
...
### 方法
想法是在队列的帮助下使用[级顺序遍历](https://www.geeksforgeeks.org/level-order-tree-traversal/)遍历二叉树，如果遍历的级别是 `K`，那么将该级别的所有节点存储在[集合](https://www.geeksforgeeks.org/set-in-cpp-stl/)中，使得在该级别没有重复的节点。
...
*   **空间复杂度:**在树的最底层最差的情况下，它可以有最大数量的节点，即 `2^(H-1)`，其中 `H` 是二叉树的高度，那么二叉树的空间复杂度将是 `O(2^(H-1))`