...
一种简单的解决方案是开始以螺旋形式遍历矩阵[打印螺旋矩阵](https://www.geeksforgeeks.org/print-a-given-matrix-in-spiral-form/)并启动计数器，即`count = 0`。只要`count`等于`K`，就打印该元素。
...
以螺旋顺序遍历数组时，使用一个循环遍历边。 因此，如果可以发现第`k`个元素位于给定的边，则可以在恒定时间内找到第`k`个元素。 这可以递归和迭代地完成。
