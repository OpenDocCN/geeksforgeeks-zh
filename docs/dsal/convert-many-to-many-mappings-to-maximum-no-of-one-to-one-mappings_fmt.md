...
> 原文：[https://www.geeksforgeeks.org/convert-many-to-many-mappings-to-maximum-no-of-one-to-one-mappings/](https://www.geeksforgeeks.org/convert-many-to-many-mappings-to-maximum-no-of-one-to-one-mappings/)

给定一个包含来自 `1..N` 的整数的数组 `arr1` 和另一个数组 `arr2` 包含来自 `1..M` 的整数。`arr1` 的每个元素映射到 `arr2` 中的多个元素。任务是最大化两个数组之间一对一映射的数量。
