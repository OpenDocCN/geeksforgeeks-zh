...
> 原文: [https://www.geeksforgeeks.org/check-if-it-is-possible-to-draw-a-straight-line-with-the-given-direction-cosines/](https://www.geeksforgeeks.org/check-if-it-is-possible-to-draw-a-straight-line-with-the-given-direction-cosines/)
...
**逼近:** 如果一条直线与正的 `X轴` 形成角度 `a`，与正的 `Y轴` 形成角度 `b`，与正的 `Z轴` 形成角度 `c`，则它的方向余弦为 `cos(a)`、`cos(b)` 和 `cos(c)`。
直线，`cos^2(a) + cos^2(b) + cos^2(c) = 1`。
以下是上述方法的实施:
