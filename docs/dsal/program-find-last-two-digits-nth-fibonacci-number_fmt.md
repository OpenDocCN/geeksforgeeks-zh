...
> 原文: [https://www.geeksforgeeks.org/program-find-last-two-digits-nth-fibonacci-number/](https://www.geeksforgeeks.org/program-find-last-two-digits-nth-fibonacci-number/)
...
一个简单的解决方法是找到第 `n` 个[斐波那契数](https://www.geeksforgeeks.org/program-for-nth-fibonacci-number/)并打印它的最后两位数字。但是 `N` 可能非常大，所以它不会工作。
一个更好的解决方案是利用第 300 个斐波那契数后的最后两位数开始重复的事实。
1) 求 `m = n % 300`。
2) 返回第 `m` 个斐波那契数。
