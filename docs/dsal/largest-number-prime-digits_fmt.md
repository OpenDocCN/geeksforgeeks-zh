...
给定一个巨大的整数值 `n`，求最大整数值 `x`，使得 `x <= n` 且 `x` 的所有数位都是素数。
...
我们知道素数是 `2`，`3`，`5` 和 `7`。另外，由于我们必须处理一个非常大的数字的每个数位，如果我们把它作为一个字符串来处理，会更容易。主要思路是先找到第一个非质数位，然后找到其左边第一个大于 `2` 的数字。现在我们可以用刚好小于它的质数替换找到的数字。如果数字是 `2`，我们必须删除它并用 `7` 替换下一个数字。之后，我们可以用 `7` 替换它右边的剩余数字。
