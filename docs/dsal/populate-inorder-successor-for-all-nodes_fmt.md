...
> 原文：[https://www.geeksforgeeks.org/populate-inorder-successor-for-all-nodes/](https://www.geeksforgeeks.org/populate-inorder-successor-for-all-nodes/)

给定一个二叉树，其中每个节点具有以下结构，编写一个函数来填充所有节点的`next`指针。每个节点的`next`指针应该设置为指向下一个后继节点。
...
最初，所有`next`指针都有空值。您的函数应该填充这些`next`指针，以便它们指向更高级的后继函数。

## 解决方案(使用反向有序遍历)

以反向有序遍历的方式遍历给定的树，并跟踪以前访问过的节点。当一个节点被访问时，分配一个先前访问过的节点作为`next`。
