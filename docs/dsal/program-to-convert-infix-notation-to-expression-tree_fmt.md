...
> 原文：[https://www.geeksforgeeks.org/program-to-convert-infix-notation-to-expression-tree/](https://www.geeksforgeeks.org/program-to-convert-infix-notation-to-expression-tree/)
...
空间复杂度为 `O(n)`（`char_stack` + `node_stack` <= n）。