...
> 原文：[https://www.geeksforgeeks.org/find-number-of-segments-covering-each-point-in-an-given-array/](https://www.geeksforgeeks.org/find-number-of-segments-covering-each-point-in-an-given-array/)
...
> **输入:** `segments = {{0, 3}, {1, 3}, {3, 8}}`, `points = {-1, 3, 8}`。
> **输出:** `{0, 3, 1}`
> **解释:**
> 
...
> *   第三段通过点 `8`
> 
> **输入:** `segments = {{1, 3}, {2, 4}, {5, 7}}`, `points = {0, 2, 5}`。
> **输出:** `{0, 2, 1}`
> **解释:**
> 
...
*   让我们用 `(l, r)` 来表示一个线段。形成一个对的向量，对于每个片段，在向量中推两个对，值为 `(l, +1)` 和 `(r + 1, -1)`。
...
`O(s*log(s) + p*log(p))`，其中 `s` 为线段数，`p` 为点数。
