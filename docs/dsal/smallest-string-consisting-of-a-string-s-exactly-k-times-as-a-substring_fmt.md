...
为了优化上述方法，找到[最长的固有前缀](https://www.geeksforgeeks.org/longest-prefix-also-suffix/)，它也是给定字符串 `S` 的后缀，然后生成一个排除最长公共前缀的 `S` 的子串，并将这个子串精确地添加到答案中`K–1`次到原始字符串。按照以下步骤解决问题:
