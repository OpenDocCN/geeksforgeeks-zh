...
> 原文：[https://www.geeksforgeeks.org/construct-a-matrix-with-no-element-exceeding-x-and-sum-of-two-adjacent-elements-not-exceeding-y/](https://www.geeksforgeeks.org/construct-a-matrix-with-no-element-exceeding-x-and-sum-of-two-adjacent-elements-not-exceeding-y/)
...
*   为了满足两个必要条件，矩阵只需要交替填充以下两个数字：
    ...
    > **插图：**
    > `N = 3`，`M = 3`，`X = 5`，`Y = 3`
    > 第一个数 = `min(X, Y) = min(5, 3) = 3`
    > 第二个数 = `min(2X, Y) - 3 = min(10, 3) - 3 = 3 - 3 = 0`
