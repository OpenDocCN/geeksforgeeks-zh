...
> 原文: [https://www.geeksforgeeks.org/check-if-a-number-can-be-represented-as-sum-of-two-positive-perfect-cubes/](https://www.geeksforgeeks.org/check-if-a-number-can-be-represented-as-sum-of-two-positive-perfect-cubes/)
...
想法是将从 `1` 到 `N` 的所有数字的完美立方存储在[地图](https://www.geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/)中，并检查 `N` 是否可以表示为地图中存在的两个数字的和。按照以下步骤解决问题:

*   初始化一个[有序图](https://www.geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/)，比如 `cubes`，按照排序顺序存储前 `N` 个自然数的完美立方体。
...
我们将 `lo` 声明为 `1`，`hi` 声明为 `n`（给定数字）的立方根，然后通过 `(lo<=hi)` 这个条件，如果当前值小于 `n`，我们将增加 `lo`，反之，如果大于，则减少 `hi`，其中当前值为 `(lo*lo*lo + hi*hi*hi)`。
