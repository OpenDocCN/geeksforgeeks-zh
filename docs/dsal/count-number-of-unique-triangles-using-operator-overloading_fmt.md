...
> 原文: [https://www.geeksforgeeks.org/count-number-of-unique-triangles-using-operator-overloading/](https://www.geeksforgeeks.org/count-number-of-unique-triangles-using-operator-overloading/)

给定 `N` 个三角形及其三条边的长度为 `a`、`b` 和 `c`。任务是计算这些给定三角形中唯一三角形的数量 `N`。如果两个三角形至少有一条边不同，那么它们就是不同的。
