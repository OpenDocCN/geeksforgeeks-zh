...
> 原文：[https://www.geeksforgeeks.org/remove-exactly-one-element-from-the-array-such-that-max-min-is-minimum/](https://www.geeksforgeeks.org/remove-exactly-one-element-from-the-array-such-that-max-min-is-minimum/)

给定一个由 `N` 个正整数组成的数组。任务是从该数组中删除恰好一个元素，以最小化 `max(a) - min(a)` 并打印最小可能的 `(max(a) - min(a))`。

**注：** `max(a)` 表示数组 `a` 中最大的数，`min(a)` 表示数组 `a` 中最小的数。数组中至少有 2 个元素。
