...
> 原文: [https://www.geeksforgeeks.org/probability-of-collision-between-two-trucks/](https://www.geeksforgeeks.org/probability-of-collision-between-two-trucks/)

给定两条线 `S` 和 `T`，其中 `S` 代表车辆从左向右移动的第一条车道，`T` 代表车辆从右向左移动的第二条车道。车辆可以是 `B(自行车)`、`C(汽车)`，也可以是 `T(卡车)`。任务是找出两辆卡车相撞的概率。
...
*   初始化一个变量 `answer` 等于 `0` 来存储碰撞计数。
*   计算字符串 `T` 中的卡车数量，并将其存储在变量 `count` 中。
*   同时迭代字符串 `S` 和 `T` 的字符:
    *   如果 `S[i]` 等于 `'T'`，将 `answer` 增加 `count`。
    *   如果 `T[i]` 等于 `'T'`，则按 `1` 递减 `count`。
