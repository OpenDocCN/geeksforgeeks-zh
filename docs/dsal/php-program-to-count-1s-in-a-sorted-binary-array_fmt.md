...
> 原文: [https://www.geeksforgeeks.org/php-program-to-count-1s-in-a-sorted-binary-array/](https://www.geeksforgeeks.org/php-program-to-count-1s-in-a-sorted-binary-array/)
...
一个简单的解决方案是线性遍历数组。简单解的时间复杂度为 `O(n)`。我们可以使用[二分搜索法](https://www.geeksforgeeks.org/binary-search/)找到以 `O(Logn)` 为单位的时间计数。想法是使用二分搜索法查找 1 的最后一次出现。一旦我们找到索引的最后一次出现，我们返回 `索引+1` 作为计数。
