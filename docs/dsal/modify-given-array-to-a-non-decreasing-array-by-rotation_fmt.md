...
> 原文: [https://www.geeksforgeeks.org/modify-given-array-to-a-non-decreasing-array-by-rotation/](https://www.geeksforgeeks.org/modify-given-array-to-a-non-decreasing-array-by-rotation/)

给定一个大小为 N（由副本组成）的数组 `arr[]`，任务是检查给定数组是否可以通过旋转将其转换为非递减数组。如果无法做到，则打印“否”。否则，打印“是”。
...
*   初始化一个向量，说 `v`，把原数组的所有元素都复制进去。
*   排序向量 `v`。
*   遍历原始数组并执行以下步骤:
    *   每次迭代旋转 1。
    *   如果数组等于向量 `v`，则打印“是”。否则，打印“否”。
