...
> 原文：[https://www.geeksforgeeks.org/get-the-original-matrix-when-largest-element-in-a-row-and-a-column-are-given/](https://www.geeksforgeeks.org/get-the-original-matrix-when-largest-element-in-a-row-and-a-column-are-given/)

分别给定 `N` 和 `M` 整数的两个数组 `A[]` 和 `B[]`。还给出了一个 `N X M` 二进制矩阵，其中 1 表示原矩阵中有一个正整数，0 表示原矩阵中该位置用 0 填充。任务是形成原始矩阵，使得 `A[i]` 表示第 `i` 行中的最大元素，`B[j]` 表示第 `j` 列中的最大元素。
...
**方法：** 对矩阵中的每个指标 `(i，j)` 进行迭代，如果 `mat[i][j] == 1` ，则用 `min(A[i]，B[j])` 填充该位置。这是因为当前元素是第 `i` 行和第 `j` 列的一部分，如果选择了 `max(A[i]，B[j])` ，则无法满足其中一个条件，即所选元素可能超过当前行或当前列所需的最大元素。
