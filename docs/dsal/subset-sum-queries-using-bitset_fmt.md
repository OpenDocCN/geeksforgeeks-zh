...
> 原文：[https://www.geeksforgeeks.org/subset-sum-queries-using-bitset/](https://www.geeksforgeeks.org/subset-sum-queries-using-bitset/)
...
我们基本上使用位数组 `bit[]` 来表示数组中元素的子集和。`bit[]` 的大小至少应为所有数组元素的总和加 1，以回答所有查询。如果 `x` 是给定数组的子集和，我们保留 `bit[x]` 为 1，否则为假。请注意，索引假定从 0 开始。
...
最后，我们将位数组设为 `1101111011`，因此，如果 `bit[x]` 为 1，则 `x` 的子集和存在，否则不存在。我们可以清楚地观察到，除了 2 和 7 之外，从 0 到 9 的所有数字的子集和都存在于数组中。
...
O(n) 为预计算，O(1) 为后续查询，其中 n 为数组中的元素个数。

参考 [http://stackoverflow.com/questions/12459563/what-is-the-size-of-bitset-in-c](http://stackoverflow.com/questions/12459563/what-is-the-size-of-bitset-in-c) 了解该方法的空间要求。

本文由 **Avinash Kumar Jain** 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 [contribute.geeksforgeeks.org](http://www.contribute.geeksforgeeks.org) 写一篇文章或者把你的文章邮寄到 `contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
