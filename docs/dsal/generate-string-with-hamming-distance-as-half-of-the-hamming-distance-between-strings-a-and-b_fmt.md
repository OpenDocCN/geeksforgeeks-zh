...
> 原文: [https://www.geeksforgeeks.org/generate-string-with-hamming-distance-as-half-of-the-hamming-distance-between-strings-a-and-b/](https://www.geeksforgeeks.org/generate-string-with-hamming-distance-as-half-of-the-hamming-distance-between-strings-a-and-b/)
...
1.  找出两个给定字符串 `A` 和 `B` 之间的[汉明距离](https://www.geeksforgeeks.org/hamming-distance-two-strings/)（`dist`）。如果 `dist` 是奇数，那么我们不能用字符串 `A` 和 `B` 生成另一个带有[汉明距离](https://www.geeksforgeeks.org/hamming-distance-two-strings/)（`dist/2`）的字符串。
2.  如果 `dist` 为偶数，则从字符串 `A` 中选择第一个（`dist/2` 个）不等于字符串 `B` 的字符，然后从字符串 `B` 中选择第二个（`dist/2` 个不等于字符串 `A` 的字符，组成结果字符串。
