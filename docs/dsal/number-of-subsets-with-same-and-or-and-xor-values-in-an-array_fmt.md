...
> 原文: [https://www.geeksforgeeks.org/number-of-subsets-with-same-and-or-and-xor-values-in-an-array/](https://www.geeksforgeeks.org/number-of-subsets-with-same-and-or-and-xor-values-in-an-array/)
...
**注意:** 既然答案可以很大，那就用 `1000000007` 来取模吧。
...
这个问题的朴素方法是[以迭代的方式遍历数组](https://www.geeksforgeeks.org/print-sums-subsets-given-set/)的所有子集，对于每个子集找到按位 `and`、`or` 和 `XOR` 值，并检查它们是否相等。最后，返回这样相等子集的计数。
...
- 如果某个元素的频率为 `K`，那么它可以形成的奇数大小子集的数量为 `2^(K–1)`，它可以形成的非空子集的总数为 `2^K–1`。
