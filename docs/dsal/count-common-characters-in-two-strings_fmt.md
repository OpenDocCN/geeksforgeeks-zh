...
原文: [https://www.geeksforgeeks.org/count-common-characters-in-two-strings/](https://www.geeksforgeeks.org/count-common-characters-in-two-strings/)
...
`(s1[0], s2[1])` 不包括，因为 `s1[0]` 已经与 `s2[0]` 配对。

输入: `s1 = "geeksforgeeks"`, `s2 = "platformforkeks"`
输出: 8

### 方法

统计两个字符串中所有字符的频率。现在，对于每个字符，如果该字符在字符串 `s1` 中的频率为 `freq1`，在字符串 `s2` 中的频率为 `freq2`，则该字符的有效对总数将为 `min(freq1, freq2)`。所有字符的该值之和即为所需答案。
