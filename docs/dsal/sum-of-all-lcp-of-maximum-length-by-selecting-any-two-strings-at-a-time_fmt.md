...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-lcp-of-maximum-length-by-selecting-any-two-strings-at-a-time/](https://www.geeksforgeeks.org/sum-of-all-lcp-of-maximum-length-by-selecting-any-two-strings-at-a-time/)

给定一个字符串列表，任务是通过一次选择任意两个字符串来找到最大长度的所有 [LCP(最长公共前缀)](https://www.geeksforgeeks.org/longest-common-prefix-using-linked-list/)的总和。

**例:**

> **输入:** `str[] = {"babab", "ababb", "abbab", "aaaaa", "babaa", "babbb"}`
> **输出:** 6
> **解释:**
...
> **输出:** 7
> **说明:**
> 选择第 3(`aaaaaa`)和第 4 串(`aaabaaa`) => LCP(`aaaa`)长度= 4，
...
**时间复杂度:** `O(M*N^2)`，其中 `M` =最大字符串长度，`N` =字符串个数。

**高效方法:**
使用 [Trie 数据结构](https://www.geeksforgeeks.org/trie-insert-and-search/)可以获得高效的解决方案。为了找到字符串之间共有的字符数，我们将使用变量 `visited` 来跟踪一个字符被访问的次数。
以下是步骤:
...
// Driver code
let T = new Trie();
let str = ["babab", "ababb", "abbab", "aaaaa", "babaa", "babbb"];
