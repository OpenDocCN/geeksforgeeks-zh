...
在先前的[文章](https://www.geeksforgeeks.org/flatten-a-binary-tree-into-linked-list/)中已经讨论了使用递归的方法。 这种方法暗示了使用栈对二叉树进行预遍历。 在此遍历中，每次将右子项推入栈时，都会使右子项等于左子项，并使左子项等于`NULL`。 如果节点的右子节点变为`NULL`，则会弹出栈，而右子节点将从栈中弹出。 重复上述步骤，直到栈的大小为零或根为`NULL`。
