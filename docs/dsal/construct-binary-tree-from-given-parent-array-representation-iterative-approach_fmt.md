...
> 原文: [https://www.geeksforgeeks.org/construct-binary-tree-from-given-parent-array-representation-iterative-approach/](https://www.geeksforgeeks.org/construct-binary-tree-from-given-parent-array-representation-iterative-approach/)
...
**方法:** 这个问题的递归方法在这里[讨论](https://www.geeksforgeeks.org/construct-a-binary-tree-from-parent-array-representation/)。以下是迭代方法:
...
即使节点没有按顺序排列，这种方法也是有效的。以下是上述方法的实现:
...
`Node` 类，`newNode()` 函数，`inorder()` 函数，`createTree()` 函数。
...
`Node` 类，`newNode()` 函数，`inorder()` 函数，`createTree()` 函数，`Dictionary<int, Node>`。
...
`Node` 类，`newNode()` 函数，`inorder()` 函数，`createTree()` 函数，`Map()`。
