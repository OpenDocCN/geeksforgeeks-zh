...
> 原文: [https://www.geeksforgeeks.org/count-all-pairs-of-an-array-which-differ-in-k-bits/](https://www.geeksforgeeks.org/count-all-pairs-of-an-array-which-differ-in-k-bits/)
...
**时间复杂度:** `O(N^2 * log MAX)`，其中 `MAX` 为输入数组中最大元素。
**辅助空间:** `O(1)`
...
### 时间复杂度
O(MAX <sup>2</sup> )，其中 `MAX` 为输入数组中的最大元素。
