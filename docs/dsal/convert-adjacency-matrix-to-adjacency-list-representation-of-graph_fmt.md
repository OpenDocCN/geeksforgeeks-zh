...
**邻接表:** 使用列表数组。数组的大小等于顶点的数量。让数组成为 `array[]`。条目 `array[i]` 表示与第 `i` 个顶点相邻的顶点列表。

将邻接矩阵转换为邻接表。创建列表数组并遍历邻接矩阵。如果对于矩阵中的任意单元格 `(i, j)` 满足 `mat[i][j] = 1`，则表示从 `i` 到 `j` 有一条边，因此在列表数组中的 `i-th` 位置插入列表中的 `j`。
