...
一个 [`五边形棱锥数`](https://en.wikipedia.org/wiki/Pentagonal_pyramidal_number) 属于具象数类。它是以五边形为底的金字塔中物体的数量。第 `n` 个五边形金字塔数等于前 `n` 个[五边形数](https://www.geeksforgeeks.org/nth-pentagonal-number/)之和。
...
这个方法很简单。它通过运行循环将所有的五边形数加起来直到 `n`，从而得到第 `n` 个五边形金字塔数。
...
在这种方法中，我们用公式在 O(1) 时间内得到第 `n` 个五边形金字塔数。

第 `n` 个五边形金字塔数 = `n^2 * (n + 1) / 2`
