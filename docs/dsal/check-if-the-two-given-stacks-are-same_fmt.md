...
> 原文: [https://www.geeksforgeeks.org/check-if-the-two-given-stacks-are-same/](https://www.geeksforgeeks.org/check-if-the-two-given-stacks-are-same/)
...
1.  取一个标志变量，初始设置为真，`flag = true`。该变量将指示堆栈是否相同。
2.  首先检查给定的`stack1`和`stack2`的尺寸是否相等。如果大小不相等，将`flag`设置为`false`并返回。
3.  如果大小相同，则比较两个给定堆栈的顶部元素。
4.  如果两个堆栈的顶部不是相同，将`flag`设置为`false`并返回，否则弹出两个堆栈的顶部元素。
5.  重复步骤3和4，直到所有元素都从两个堆栈中弹出。
6.  如果两个堆栈都为空，并且`flag`变量仍然为`true`，这意味着堆栈是相同的。
