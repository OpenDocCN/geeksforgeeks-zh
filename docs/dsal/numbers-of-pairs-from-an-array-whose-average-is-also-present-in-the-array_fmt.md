...
> 原文：[https://www.geeksforgeeks.org/numbers-of-pairs-from-an-array-whose-average-is-also-present-in-the-array/](https://www.geeksforgeeks.org/numbers-of-pairs-from-an-array-whose-average-is-also-present-in-the-array/)

给定由 `N` 个整数组成的数组 `arr[]`，任务是计算数组中不同对 `(arr[i], arr[j])` 的数量，使得对的平均值也存在于数组中。
...
1.  初始化一个变量，比如说 `count` 为 `0` 来存储数组中存在平均值的所有对的计数。
2.  将所有数组元素插入一个集合 `S`。
3.  遍历集合 `S`，对于集合 `S` 中的每个元素，生成给定数组的所有可能的对，如果任何对的总和与集合中的当前元素相同，则将 `count` 的值增加 `1`。
4.  完成上述步骤后，打印 `count` 的值作为成对计数的结果。
...
*   初始化一个变量，比如说 `count` 为 `0` 来存储数组中存在平均值的所有对的计数。
...
*   [遍历集合](https://www.geeksforgeeks.org/how-to-traverse-a-c-set-in-reverse-direction/) `S`，对于集合 `S` 中的每一个元素(比如说 `X` )，用值 `(M[X]/2)` 更新 `count` 的值。
*   完成上述步骤后，打印 `count` 的值作为成对计数的结果。
