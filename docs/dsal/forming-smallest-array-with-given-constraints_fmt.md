...
> 原文: [https://www.geeksforgeeks.org/forming-smallest-array-with-given-constraints/](https://www.geeksforgeeks.org/forming-smallest-array-with-given-constraints/)
...
> 输入: `x = 5`，`y = 7`，`z = 11`
> 输出: 7
> 最小的以 5 开始，有 7，以 11 结束，绝对差 1 为 `{ 5，6，7，8，9，10，11 }`。
...
因此，可以形成的最小数组的长度将是从 `X` 到 `z` 覆盖的点数，即 `1 + abs(x - y) + abs(y - z)` (1 is added to count point `x`)。
