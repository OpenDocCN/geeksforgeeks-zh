...
> 原文：[https://www.geeksforgeeks.org/how-to-swap-two-bits-in-a-given-integer/](https://www.geeksforgeeks.org/how-to-swap-two-bits-in-a-given-integer/)
...
思路是先找到位，然后用[基于异或的交换概念](https://www.geeksforgeeks.org/swap-two-numbers-without-using-temporary-variable/)，例如，要交换两个数字 `x` 和 `y`，我们要做 `x = x ^ y`，`y = y ^ x` 和 `x = x ^ y`。
