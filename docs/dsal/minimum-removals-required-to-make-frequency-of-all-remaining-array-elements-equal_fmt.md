...
> 原文: [https://www.geeksforgeeks.org/minimum-removals-required-to-make-frequency-of-all-remaining-array-elements-equal/](https://www.geeksforgeeks.org/minimum-removals-required-to-make-frequency-of-all-remaining-array-elements-equal/)

给定一个大小为 `N` 的数组 `arr[]`，任务是找到需要移除的最小数量的数组元素，使得剩余数组元素的频率相等。
...
*   初始化一个有序映射，比如 `freq`，来存储所有数组元素的频率。
*   遍历数组 `arr[]` 并存储数组元素各自的频率。
*   初始化一个向量，比如 `v`，来存储映射中存储的所有频率。
*   排序向量 `v`。
