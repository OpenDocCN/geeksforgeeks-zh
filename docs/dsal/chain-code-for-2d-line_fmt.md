...
链码可以通过对每个方向使用条件语句来生成，但是对于具有大量方向的系统（三维网格可以有多达 26 个方向）来描述变得非常乏味。相反，我们使用散列函数。计算并散列两个连续点的 X（`dx`）和 Y（`dy`）坐标之差，以生成这两个点之间的链码的密钥。

链码列表：`[5, 6, 7, 4, -1, 0, 3, 2, 1]`

哈希函数：`C(dx, dy) = 3dy + dx + 4`
