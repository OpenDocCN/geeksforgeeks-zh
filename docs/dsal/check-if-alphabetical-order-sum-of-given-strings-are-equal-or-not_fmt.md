...
> 原文: [https://www.geeksforgeeks.org/check-if-alphabetical-order-sum-of-given-strings-are-equal-or-not/](https://www.geeksforgeeks.org/check-if-alphabetical-order-sum-of-given-strings-are-equal-or-not/)

给定两个字符串`s1`和`s2`，任务是检查两个给定字符串的字符的**字母顺序和**是否相等或不是。

**示例:**

> **输入**: `s1="geek"`, `s2="abcdefg"`
> **输出**: 真
> **说明**: 两串字符的字母顺序和为:
> *   `s1= 7+5+5+11 = 28`
> *   `s2= 1+2+3+4+5+6+7 = 28`
...
**逼近**: 通过简单的在字符串上迭代找到字符的**字母顺序和**就可以解决任务。对于每个字符，在结果答案中加上当前字符+ 1的ASCII值。
