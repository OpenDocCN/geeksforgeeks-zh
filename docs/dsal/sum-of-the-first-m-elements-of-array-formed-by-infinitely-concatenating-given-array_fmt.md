...
> 原文: [https://www.geeksforgeeks.org/sum-of-the-first-m-elements-of-array-formed-by-infinitely-concatenating-given-array/](https://www.geeksforgeeks.org/sum-of-the-first-m-elements-of-array-formed-by-infinitely-concatenating-given-array/)

给定一个由 `N` 个整数和一个正整数 `M` 组成的数组 `arr[]`，任务是找出由给定数组 `arr[]` 无限级联而成的数组的前 `M` 个元素的和。
...
> **输出:** `9`
> **解释:**
> 给定数组 `arr[]` 无限级联形成的数组的形式为 `{1, 2, 3, 1, 2, 3, 1, 2, 3, ...}`。
> 数组前 `M(= 5)` 个元素之和为 `1 + 2 + 3 + 1 + 2 = 9`。

> **输入:** `arr[] = {1}`，`M = 7`
> **输出:** `7`

**方法:** 给定的问题可以通过使用模算符 (`%`) 求解，将给定的数组视为圆形数组，并据此求出第一个 `M` 元素的和。按照以下步骤解决此问题:

*   初始化一个变量，将 `sum` 设为 `0` 来存储新数组的第一个 `M` 元素的合成和。
*   使用变量 `i` 迭代范围 `[0, M–1]`，并将 `sum` 的值增加 `arr[i % N]`。
