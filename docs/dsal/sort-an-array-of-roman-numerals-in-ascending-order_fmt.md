...
> 原文: [https://www.geeksforgeeks.org/sort-an-array-of-roman-numerals-in-ascending-order/](https://www.geeksforgeeks.org/sort-an-array-of-roman-numerals-in-ascending-order/)

给定一个由罗马数字组成的数组`arr[]`和整数`N`，任务是按升序排列这些罗马数字。
...
解决这个问题的思路是将每个元素及其罗马到整数值存储在一个向量对中，然后根据存储的罗马到整数值对向量的所有元素进行排序。

*   迭代给定的罗马数字。
*   将罗马数字转换为十进制并将它们作为一对存储在向量中。
