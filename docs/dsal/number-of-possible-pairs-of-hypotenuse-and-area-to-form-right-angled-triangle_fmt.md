...
> 原文: [https://www.geeksforgeeks.org/number-of-possible-pairs-of-hypotenuse-and-area-to-form-right-angled-triangle/](https://www.geeksforgeeks.org/number-of-possible-pairs-of-hypotenuse-and-area-to-form-right-angled-triangle/)
...
在 `a^2` 中求解这个二次方程，放入判别式>= 0（`a` 存在的条件）。我们会得到，

```
H^2 >= 4*S
```

对于一个斜边为 `H`、面积为 `S` 的直角三角形要存在，必须满足 `H^2 >= 4*S`。

## 天真法

天真法是找到所有可能的 `(H, S)` 对，检查它们是否满足条件 `H^2 >= 4 * S`。计算满足此条件的对的数量，并打印计数。
...
一种有效的方法是以递增的顺序对可用的两个数组进行排序。然后，对于斜边的每一个可能的长度，应用二分搜索法来寻找满足必要条件的最大面积。
