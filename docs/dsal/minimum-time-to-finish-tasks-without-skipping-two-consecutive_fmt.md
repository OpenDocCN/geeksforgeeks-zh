...
> 原文：[https://www.geeksforgeeks.org/minimum-time-to-finish-tasks-without-skipping-two-consecutive/](https://www.geeksforgeeks.org/minimum-time-to-finish-tasks-without-skipping-two-consecutive/)
...
给定的问题具有以下递归性质。让 `minTime(i)` 成为完成前 i 个任务的最短时间。它可以写成至少两个值。

1.  `incl(i)`：如果第 i 个任务包含在列表中，让这个时间为 `incl(i)`
2.  `excl(i)`：如果第 i 个任务被排除在结果之外，让这个时间为 `excl(i)`
...
一个简单的解决方案是制作两个包含 `incl[]` 和不包含 `excl[]` 的表来存储任务的时间。最终返回 `incl[n-1]` 和 `excl[n-1]` 的最小值。这个解决方案需要 `O(n)` 个时间和 `O(n)` 个空间。
