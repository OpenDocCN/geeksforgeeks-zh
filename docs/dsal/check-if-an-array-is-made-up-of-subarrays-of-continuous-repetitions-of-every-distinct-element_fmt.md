...
> 原文: [https://www.geeksforgeeks.org/check-if-an-array-is-made-up-of-subarrays-of-continuous-repetitions-of-every-distinct-element/](https://www.geeksforgeeks.org/check-if-an-array-is-made-up-of-subarrays-of-continuous-repetitions-of-every-distinct-element/)
...
**输出:** 否
**解释:**
给定的数组由 3 个不同的元素 `{1，2，3}` 和子数组 `{1，1，1}`、`{2，2，2，2}`、`{1}`、`{3，3}` 组成。
...
*   如果在 `arr[i] != arr[i-1]` 处发现任何此类指数，检查 `curr` 是否大于 `1`。如果是，将 `curr` 复位至 `0` 并继续。否则，打印“否”，因为单个元素的子阵列不重复存在。
*   否则，增加 `curr`。
*   遍历数组后，检查 `curr` 是否大于 `1`。如果 `curr` 等于 `1`，这确保最后一个元素不同于第二个最后一个元素。因此，打印“否”。
*   否则，打印“是”。
