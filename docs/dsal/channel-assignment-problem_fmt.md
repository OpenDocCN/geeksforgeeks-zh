...
有 `M` 个发射机站和 `N` 个接收机站。给定一个跟踪要从给定发射机发送到接收机的数据包数量的矩阵。如果矩阵的第 `(I；j)` 个条目是 `k`，这意味着此时站 `I` 有 `k` 个分组要发送到站 `j`。
...
请注意，在任何时隙中可以传输的最大数据包数是 `min(M，N)`。
...
我们先定义输入输出形式。输入采用埃德蒙兹矩阵的形式，这是一个二维数组 `table[M][N]`，其中有 `M` 行（用于 `M` 个发送方）和 `N` 列（用于 `N` 个接收方）。值 `table[i][j]` 是必须从发送器 `I` 发送到接收器 `j` 的数据包数量。输出是在一个时隙内，在没有带宽干扰的情况下，可以传输的最大数据包数量。
...
在二分匹配问题的情况下，因为只有 `|V|` 条可能的边从源节点出来。所以总运行时间为 `O(m'n) = O((m + n)n)`。空间复杂度也从 `O((M+N)*(M+N))` 大幅降低至仅 `M` 大小的一维数组，从而存储 `M` 和 `N` 之间的映射。
