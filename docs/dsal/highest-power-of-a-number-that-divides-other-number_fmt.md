...
> 原文: [https://www.geeksforgeeks.org/highest-power-of-a-number-that-divides-other-number/](https://www.geeksforgeeks.org/highest-power-of-a-number-that-divides-other-number/)
...
**逼近**：首先对数字 `N` 和 `M` 进行质因数分解，并将 `N` 和 `M` 的质因数计数分别存储在 `freq1[]` 和 `freq2[]` 中。对于 `M` 的每个质因数，检查其 `freq2[num]` 是否大于 `freq1[num]`。如果是 `M` 的任意质因数，则最大功率为 `0`。否则，对于 `M` 的每个质因数，最大功率将是所有 `freq1[num] / freq2[num]` 的最小值。

对于一个数 `N = 24`，质因数将为 `2^3 * 3^1`。因此 `freq1[2] = 3`，`freq1[3] = 1`。
