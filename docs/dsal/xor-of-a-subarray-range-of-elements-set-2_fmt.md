...
> 原文: [https://www.geeksforgeeks.org/xor-of-a-subarray-range-of-elements-set-2/](https://www.geeksforgeeks.org/xor-of-a-subarray-range-of-elements-set-2/)

给定大小为 `N` 和 `Q` 个查询的数组整数 `arr[]`。每个查询的形式为 `(L, R)`，其中 `L` 和 `R` 是数组的索引。任务是找到子阵列 `arr[L…R]` 的异或值。

**示例:**

> **输入:** `arr[] = {2, 5, 1, 6, 1, 2, 5}`, `query[] = {{1, 4}}`
> **输出:** 3
> `arr[1…4]` 的异或值为 3。
>
> **输入:** `arr[] = {2, 5, 1, 6, 1, 2, 5}`, `query[] = {{0, 6}}`
> **输出:** 6
> `arr[0…6]` 的异或值为 6。
