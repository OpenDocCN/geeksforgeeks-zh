...
我们有一个格式为`{a0, a1, a2, ....., an, b0, b1, b2, …, bn}`的数组，我们的任务是使用`O(1)`空间重新排列成下面给出的格式，`{a0, b0, a1, b1, a2, b2, ………, an, bn}`。
...
如我们所见，我们必须转换数组，所以必须有一个偶数大小的数组。要重新排列数组，我们将从数组的中间开始，每次我们将后一半的1个元素向左移到所需位置。该算法将采用`O(n ^ 2)`。
