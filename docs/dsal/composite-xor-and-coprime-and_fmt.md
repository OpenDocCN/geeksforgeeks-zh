...
> 原文：[https://www.geeksforgeeks.org/composite-xor-and-coprime-and/](https://www.geeksforgeeks.org/composite-xor-and-coprime-and/)

给定一个数组`arr[]`，任务是计算无序索引对`(i, j)`的数量，使得`gcd(2, a[i]^a[j]) > 1`和`gcd(2, a[i]&a[j]) < 2`，其中`^`和`&`是按位异或和按位与运算符。
...
*   `gcd(2, a[i]^a[j]) > 1`只有在`a[i]`和`a[j]`均为偶数或均为奇数时才会为真。
*   从上一步开始缩小对，如果`a`和`b`都是偶数，一对`(a, b)`永远不会产生`gcd(2, a[i]&a[j]) < 2`。因此，只有`a`和`b`都是奇数的配对才能同时满足给定的两个条件。
*   计算给定数组中奇数元素的数量，并将其存储在`odd`中，结果将是`(odd * (odd - 1)) / 2`。
