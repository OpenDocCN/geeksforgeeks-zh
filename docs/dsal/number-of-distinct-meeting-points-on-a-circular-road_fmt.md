...
> 原文：[https://www.geeksforgeeks.org/number-of-distinct-meeting-points-on-a-circular-road/](https://www.geeksforgeeks.org/number-of-distinct-meeting-points-on-a-circular-road/)
...
让环形路的周长为 `d`。
让汽车所花费的时间 `A` 和 `B` 分别为 `t<sub>A</sub>` 和 `t<sub>B</sub>`。它们的相对速度是 `a–b`。
`A` 和 `B` 从起点出发，一段时间后，他们会在起点再次相遇。这个时间可以通过 `t<sub>a</sub>` 和 `t<sub>b</sub>` 的 `LCM` 来计算。在这段时间内，他们可能会在某个需要查明的点相遇。注意，在他们在起点相遇后，他们会继续在同一点相遇。
在起点再次相遇所需的时间将为：
