...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-subsets-whose-sum-is-a-perfect-number-from-a-given-array/](https://www.geeksforgeeks.org/sum-of-all-subsets-whose-sum-is-a-perfect-number-from-a-given-array/)
...
**时间复杂度:** `O(M * 2^N)`，其中 `M` 是数组 [`arr[]`](https://www.geeksforgeeks.org/program-find-sum-elements-given-array/) 元素的和。
**辅助空间:** `O(1)`

**迭代方法:** 由于从一个大小为 `N` 的数组中有 `2^N` 个可能的子集，其思想是[迭代一个从 `0` 到 `2^N–1`](https://www.geeksforgeeks.org/range-based-loop-c/) 的循环，对于每个数字，在当前数字的[二进制表示中选择对应于 `1`s 的所有数组元素](https://www.geeksforgeeks.org/binary-representation-of-a-given-number/)，然后按照以下步骤解决问题:
...
*   [使用变量 `i` 在范围](https://www.geeksforgeeks.org/range-based-loop-c/) `[0, 2^N–1]` 中迭代，并执行以下步骤:
    ...
    *   [使用变量 `j` 遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]`，并执行以下步骤:
        *   如果在 `i` 的[二进制表示](https://www.geeksforgeeks.org/binary-representation-of-a-given-number/)中设置了 `j^th` 位置，则将 `arr[j]` 的值加到 `S` 上。
    *   检查 `S` 是否为完全数，如果发现为真则打印 `S` 的值作为结果。
