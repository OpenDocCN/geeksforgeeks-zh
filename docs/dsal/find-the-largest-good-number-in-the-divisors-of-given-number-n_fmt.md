...
> 原文：[https://www.geeksforgeeks.org/find-the-largest-good-number-in-the-divisors-of-given-number-n/](https://www.geeksforgeeks.org/find-the-largest-good-number-in-the-divisors-of-given-number-n/)
...
**方法：** 求 `n` 的所有质因数，假设它们是 `p1`，`p2`，…，`pk`（在 `O(sqrt(n))` 时间复杂度中）。如果答案是 `a`，那么我们知道，对于每一个 `1 <= i <= k`，很明显，`a` 不能被 `pi^2`（以及 `pi` 的所有更大幂）整除。所以 `a <= p1 × p2 ×… × pk`。而且我们知道 `p1 × p2 × … × pk` 本身就是一个好数字。所以，答案是 `p1 × p2 ×…× pk`。
