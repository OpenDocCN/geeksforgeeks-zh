...
> 原文：[https://www.geeksforgeeks.org/range-query-for-count-of-set-bits/](https://www.geeksforgeeks.org/range-query-for-count-of-set-bits/)

给定一个正整数数组和一个包含两个整数的查询 `Q`，`L` & `R` 的任务是找到给定范围的集合位数。
...
是运行一个从 `L` 到 `R` 的循环，计算一个范围内的设置位数。该解决方案对每个查询取 `O(nlog s)`（其中 `s` 是位大小）。

## 高效的解决方案

是基于这样一个事实：如果我们在一个数组 `BitCount` 中存储数字的所有设置位的计数，那么我们可以在 `O(1)` 时间内回答每个查询。因此，开始遍历数组的元素，计算每个元素的设置位并存储在数组中。现在，求这个数组的累计和。这个数组将有助于回答查询。
