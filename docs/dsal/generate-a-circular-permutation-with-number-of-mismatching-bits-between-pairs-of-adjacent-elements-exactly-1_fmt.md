...
> 原文：[https://www.geeksforgeeks.org/generate-a-circular-permutation-with-number-of-mismatching-bits-between-pairs-of-adjacent-elements-exactly-1/](https://www.geeksforgeeks.org/generate-a-circular-permutation-with-number-of-mismatching-bits-between-pairs-of-adjacent-elements-exactly-1/)

给定两个整数 `N` 和 `S`，任务是从范围 `[0, 2^(N–1)]` 中找到数字的循环[排列](https://www.geeksforgeeks.org/permutation-and-combination/)，从 `S` 开始，使得任意一对相邻数字之间的不匹配位的[计数为 `1`](https://www.geeksforgeeks.org/number-of-mismatching-bits-in-the-binary-representation-of-two-integers/)。
...
*   一个简单的观察是，范围 `[2^I, 2^(I+1) – 1]` 中的数字可以通过在范围 `[0, 2^I – 1]` 中的每个数字前放置 `'1'` 来获得。
*   因此，可以通过在索引 `2^I – 1` 的每个数字前添加 `'1'` 来递归地解决这个问题，并且新数字在添加到它们的排列之前可以被反转。

按照以下步骤解决问题：

1.  初始化一个列表，比如 `res`，以存储所需的排列。
2.  初始化一个整数，比如说 `index`，来存储从 `0` 开始的排列中 `S` 的位置。
3.  以相反的顺序遍历范围 `[0, N–1]` 和[遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `res[]`，检查 `当前编号` 和 `2^i` 的和是否为 `S`。如果发现为真，则用 `res` 的当前索引更新 `index`，并将 `当前编号 + 2^i` 追加到列表 `res` 中。
4.  [通过 `index` 位置旋转列表 `res[]`](https://www.geeksforgeeks.org/array-rotation/)。
5.  完成以上步骤后，打印列表 `res[]` 作为答案。
