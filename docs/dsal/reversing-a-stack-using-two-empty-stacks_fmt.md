...
> 原文: [https://www.geeksforgeeks.org/reversing-a-stack-using-two-empty-stacks/](https://www.geeksforgeeks.org/reversing-a-stack-using-two-empty-stacks/)

给定一个[堆栈](https://www.geeksforgeeks.org/stack-data-structure/) `S`，任务是[使用两个额外堆栈反转堆栈](https://www.geeksforgeeks.org/reverse-a-stack-using-recursion/) `S`。
...
3.  调用 `transfer(S, A)` 将堆栈的所有元素 `S` 转移到 `A`。(*元素顺序颠倒*)。
4.  调用 `transfer(A, B)` 将堆栈的所有元素 `A` 转移到 `B`。(*元素的顺序与最初在 `S` 中的顺序相同*)
5.  调用 `transfer(B, S)` 将 `B` 的所有元素转移到 `S`。(*元素顺序颠倒*)
6.  最后显示堆栈 `S`。
