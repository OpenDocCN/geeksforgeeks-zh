...
> 原文：[https://www.geeksforgeeks.org/compare-two-linked-list-of-strings/](https://www.geeksforgeeks.org/compare-two-linked-list-of-strings/)

给定两个[链表](https://www.geeksforgeeks.org/data-structures/linked-list/) `L1` 和 `L2`，其中每个节点存储一个字符串。任务是检查组合所有节点的字符串是否相似。

**示例：**

> **输入：** `L1 = ["He", "llo", "wor", "LD"]`，`L2 = ["H", "e", "ll", "owo", "r", "LD"]`
> **输出：** `true`
> **说明：** 两个列表组成了“Helloworld”的字符串。
>
> **输入：** `L1 = ["w", "o", "l", "d"]`，`L2 = ["wo", "d", "rl"]`
> **输出：** `false`
> **解释：** `L1` 造“world”但 `L2` 造“wodrl”二者不同。
>
> **输入：** `L1 = ["w", "", "orl", "d"]`，`L2 = ["worl", "", "", "", "", "d"]`
> **输出：** `true`
> **说明：** 两个列表都组成了“world”的字符串。
...
*   遍历两个列表，同时为字符保留两个指针。
*   分别比较每个字符。如果不同，返回 `false`。否则，继续比较。
