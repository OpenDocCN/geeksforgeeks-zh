...
> 原文: [https://www.geeksforgeeks.org/check-whether-a-subsequence-exists-whose-sum-is-equal-to-k-if-arri-2arri-1/](https://www.geeksforgeeks.org/check-whether-a-subsequence-exists-whose-sum-is-equal-to-k-if-arri-2arri-1/)
...
> 我们找到了一个和为 18 的子序列。
>
> **输入:** `arr[] = {1, 3, 7, 15, 31}`，`k = 20`
> **输出:** False
> 没有子序列。

## 天真的解决方案

基本的解决方案是检查所有 `2^n` 可能的组合，并检查是否有任何和等于 `k` 的子序列。这个过程不适用于更高的 `n`，`n` 值 > 20。
时间复杂度: `O(2^N)`

## 高效解

给定 `arr[i] > 2*arr[i-1]`，所以我们可以说 `arr[i] > (arr[i-1] + arr[i-2] + … + arr[2] + arr[1] + arr[0])`。

我们假设 `arr[i] <= (arr[i-1] + arr[i-2] + … + arr[2] + arr[1] + arr[0])`，所以我们必须在集合中包含 `arr[i]`。因此，我们必须以降序遍历数组，当我们找到 `arr[i] <= k` 时，我们将在集合中包含 `arr[i]`，并从 `k` 中减去 `arr[i]`，并继续循环，直到 `k` 的值等于零。
