...
**第一轮次** – `MCQ` 轮次有 4 个部分的问题 – 语言交流、能力、编码、`OS` 和 `DBMS`，约 2 小时。
**第二轮次** – 编码轮次 - 2 个问题：
...
面试官让我先从自我介绍开始。他让我解释我喜欢的任何一个项目。然后他要求写一个将十进制转换成二进制的程序。后来他让我对一个 `BST` 做一个级序遍历。我做了一个递归解决方案，但他不满意，并要求我减少遍历的次数，所以我建议使用队列，他接受了。然后他问我一个问题——在一个由 `AuthorID`、书名、年份和成本组成的表格中，我如何检索一年内卖出 3 本以上书的作者数量。他还询问了一般的 `java` 概念，比如继承、多态性等等。然后他问了我一些问题，比如告诉我你的家庭，为什么是甲骨文等等。
...
[Minimum number of flipped bits](https://practice.geeksforgeeks.org/problems/minimum-number-of-flipped-bits/0)
[Change the string](https://practice.geeksforgeeks.org/problems/change-the-string/0)