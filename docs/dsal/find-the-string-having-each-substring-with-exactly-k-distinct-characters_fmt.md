...
> 原文：[https://www.geeksforgeeks.org/find-the-string-having-each-substring-with-exactly-k-distinct-characters/](https://www.geeksforgeeks.org/find-the-string-having-each-substring-with-exactly-k-distinct-characters/)

给定两个整数 `N` 和 `K`。任务是找到长度为 `N` 的字符串，这样长度大于等于 `K` 的每个子串都正好有 `K` 个不同的字符。
