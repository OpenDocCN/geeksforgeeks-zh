...
> 原文: [https://www.geeksforgeeks.org/construct-tree-from-given-inorder-and-preorder-traversal/](https://www.geeksforgeeks.org/construct-tree-from-given-inorder-and-preorder-traversal/)
...
3.  在 `Inorder` 中找到所选元素的索引。让索引在 `inIndex` 中。
4.  在 `inIndex` 之前为元素调用 `buildTree`，并使构建的树成为 `tNode` 的左子树。
5.  为索引后的元素调用 `buildTree`，并将构建的树作为节点的右子树。
6.  返回 `tNode`。
...
我们可以使用散列法(C++中的`unordered_map` 或 Java 中的`HashMap`)来优化上述解决方案。我们在散列表中存储有序遍历的索引。这样搜索可以进行 `O(1)`次。
...
1.  执行以下操作，直到到达最左边的节点。
    继续从 `PreOrder` 遍历创建节点。如果堆栈最顶端的元素不在集合中，将创建的节点链接到堆栈最顶端元素的左子元素(如果有)，而不弹出该元素。
...
[从后序和序](https://www.geeksforgeeks.org/construct-a-binary-tree-from-postorder-and-inorder/)构建二叉树