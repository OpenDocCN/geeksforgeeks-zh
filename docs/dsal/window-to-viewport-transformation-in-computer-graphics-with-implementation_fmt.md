...
> 原文: [https://www.geeksforgeeks.org/window-to-viewport-transformation-in-computer-graphics-with-implementation/](https://www.geeksforgeeks.org/window-to-viewport-transformation-in-computer-graphics-with-implementation/)
...
*   **世界坐标–**是我们定义图表的笛卡尔坐标 w.r.t，如 `X_wmin`、`X_wmax`、`Y_wmin`、`Y_wmax`
*   **设备坐标–**是要显示对象的屏幕坐标，如 `X_vmin`、`X_vmax`、`Y_vmin`、`Y_vmax`
*   **窗口–**是世界坐标上选择显示的区域。
*   **视口–**这是设备坐标上要显示图形的区域。
...
*   其中 `sx` 是 x 坐标的比例因子，`sy` 是 y 坐标的比例因子
...
*   对于窗口，`x_wmin= 20`，`x_wmax= 80`，`y_wmin= 40`，`y_wmax= 80`。
*   对于视口，`X_vmin = 30`，`X_vmax = 60`，`Y_vmin = 40`，`Y_vmax = 60`。
*   现在一个点 `(X_w, Y_w)` 在窗户上 `(30, 80)`。我们要在视口上计算那个点，即 `(X_v, Y_v)`。
*   首先利用上述公式计算出 x 坐标 `Sx` 的比例因子和 y 坐标 `Sy` 的比例因子。
...
*   所以，窗口上的点 `(X_w, Y_w) = (30, 80)` 将是视口上的 `(X_v, Y_v) = (35, 60)`。
