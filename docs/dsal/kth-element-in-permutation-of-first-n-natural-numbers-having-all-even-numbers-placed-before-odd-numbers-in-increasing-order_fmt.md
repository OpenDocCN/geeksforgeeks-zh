...
> 原文: [https://www.geeksforgeeks.org/kth-element-in-permutation-of-first-n-natural-numbers-having-all-even-numbers-placed-before-odd-numbers-in-increasing-order/](https://www.geeksforgeeks.org/kth-element-in-permutation-of-first-n-natural-numbers-having-all-even-numbers-placed-before-odd-numbers-in-increasing-order/)

给定两个整数 `N` 和 `K`，任务是在排列的第一个 `N` 自然数的排列中找到第 `K` 个元素，使得所有偶数以递增的顺序出现在奇数之前。
...
解决问题最简单的方法是首先生成所需的 `N` 个自然数的排列，然后遍历排列找到其中存在的第 `K` 个元素。
按照以下步骤解决问题:
...
对上述方法进行优化，思路是基于前半段的前 `N / 2` 个元素为偶数，第 `K` 个元素的值等于 `K * 2`。如果 `K > N/2`，第 `K` 个元素的值取决于 `N` 是奇数还是偶数。
按照以下步骤解决问题:

*   初始化一个变量，说 `ans`，来存储第 `K` 个元素。
