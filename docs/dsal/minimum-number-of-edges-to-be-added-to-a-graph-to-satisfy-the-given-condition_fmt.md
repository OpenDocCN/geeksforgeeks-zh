...
给定一个由 `N` 个节点（编号从 `0` 到 `N – 1`）和 `M` 条边（对 `{a, b}`）组成的图，任务是找到需要添加到图中的最小边数，使得如果存在从节点 `a` 到节点 `b` 的路径，那么也应该存在从节点 `a` 到节点 `a + 1, a + 2, a + 3, ..., b – 1` 的路径。
...
**输出**：`1`
**说明**：
存在从 `1` 到 `5` 的路径。因此，也应该存在从 `1` 到 `2`、`3` 和 `4` 的路径。
添加边 `{1, 2}` 将足以到达图的其他两个节点。

**输入**：`N = 8`, `M = 3`, `edges[] = {{1, 2}, {2, 3}, {3, 4}}`
**输出**：`0`
...
1.  为不交集联合创建结构。
2.  将 `answer` 初始化为 `0`，并遍历图中的所有节点以获取当前节点的组件。
3.  如果组件是未访问的，则将其标记为已访问。
4.  现在，从该组件的最小值到最大值进行迭代，并检查节点是否与当前节点位于同一组件中。如果不是，则将它们组合为一个组件，然后将 `answer` 增加 `1`。
5.  最后，打印 `answer`。
