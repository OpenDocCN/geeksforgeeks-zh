...
> 原文: [https://www.geeksforgeeks.org/queries-to-find-total-number-of-duplicate-character-in-range-l-to-r-in-the-string-s/](https://www.geeksforgeeks.org/queries-to-find-total-number-of-duplicate-character-in-range-l-to-r-in-the-string-s/)

给定由小写字母和代表 `S` 查询次数的整数 `Q` 组成的大小为 `N` 的字符串 `S`。我们的任务是打印所有查询 `Q` 的子串 `L` 到 `R` 中的重复字符数。
注: `1 ≤ N ≤ 10^6` 和 `1 ≤ Q ≤ 10^6`
