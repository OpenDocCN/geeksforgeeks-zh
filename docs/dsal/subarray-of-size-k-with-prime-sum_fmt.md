...
输出：`7 5 4 3`
说明：子阵列`{7，5，4，3}`的元素之和为 19。因此，需要的输出是 `7 5 4 3`。
...
解决问题的思路是利用[滑动窗口技术](https://www.geeksforgeeks.org/window-sliding-technique/)[找到 K 大小的所有子阵的和，检查是否是素数](https://www.geeksforgeeks.org/python-program-to-check-whether-a-number-is-prime-or-not/)。按照以下步骤解决问题。
...
4.  逐个迭代剩余的数组元素，通过添加第 `i` 元素并从数组中移除第 `(i–K)` 元素来更新 `currSum`。现在，检查 `currSum` 是否为质数。
