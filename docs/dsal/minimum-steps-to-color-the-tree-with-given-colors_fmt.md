...
给定一棵树，该树具有最初没有颜色的 `N` 个节点和大小为 `N` 的数组 `color[]`，该数组表示着色过程发生后每个节点的颜色。任务是使用尽可能少的步骤将树着色为给定的颜色。每一步都可以选择一个顶点 `v` 和一个颜色 `x`，然后用颜色 `x` 给 `v` 的子树中的所有顶点(包括 `v` 本身)上色。**注意**根是 1 号顶点。
