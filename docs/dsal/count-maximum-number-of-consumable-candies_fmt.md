...
> 原文: [https://www.geeksforgeeks.org/count-maximum-number-of-consumable-candies/](https://www.geeksforgeeks.org/count-maximum-number-of-consumable-candies/)
...
> *   如果对于每种糖果类型，`a[i]+m ≤ b[i]` 成立，那么消耗所有可用糖果是安全的。
> *   否则，对于所有 `0 ≤ i < N`，只能消耗 `min(A[i] + M，B[i])`。

按照以下步骤解决问题:

1.  初始化两个变量，比如 `ans` 和 `total`，以存储可安全消费的最大糖果数和糖果总数。
