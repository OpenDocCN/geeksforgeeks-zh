...
> 原文: [https://www.geeksforgeeks.org/check-if-a-number-s-can-be-made-divisible-by-d-by-repeatedly-adding-the-remainder-to-s/](https://www.geeksforgeeks.org/check-if-a-number-s-can-be-made-divisible-by-d-by-repeatedly-adding-the-remainder-to-s/)
...
> **输出:** 是
> **解释:**
> 假设第 i 个区间 mod D 上的值为 `V(i)`，即 `V(i) = (V(i-1) + V(i-1) % D) % D`，则
> `V(0) = S % D = 3`
> `V(1) = (3 + 3 % 6) % 6 = (3 + 3) % 6 = 0`
> 由于 `V(1) = 0`，因此，打印“是”。
...
给定的问题可以用[鸽子洞原理](https://www.geeksforgeeks.org/discrete-mathematics-the-pigeonhole-principle/)解决。按照以下步骤解决问题:
...
*   当余数值为 `0` 时，[退出循环](https://www.geeksforgeeks.org/break-statement-cc/)，存在一个边缘情况，在这种情况下，这也是一个可除的情况，但我们的逻辑将其视为一个循环。
