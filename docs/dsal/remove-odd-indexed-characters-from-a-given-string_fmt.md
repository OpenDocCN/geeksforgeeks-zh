...
> 原文: [https://www.geeksforgeeks.org/remove-odd-indexed-characters-from-a-given-string/](https://www.geeksforgeeks.org/remove-odd-indexed-characters-from-a-given-string/)

给定大小为 ``N`` 的字符串 ``str``，任务是删除给定字符串奇数索引（基于 0 的索引）中的字符。
...
*   初始化一个空字符串，比如 ``new_string``，存储结果。
*   遍历给定的字符串，对于每个索引，检查它是否是 ``偶数``。
