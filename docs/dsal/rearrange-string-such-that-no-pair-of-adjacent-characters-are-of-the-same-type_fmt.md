...
最简单的方法是[生成给定字符串](https://www.geeksforgeeks.org/write-a-c-program-to-print-all-permutations-of-a-given-string/)的所有可能排列，并针对每个排列检查是否满足给定条件。 如果对任何排列均正确，则打印该排列。 如果不存在这样的排列，则打印`-1`。
...
为了优化上述方法，其思想是将所有字母和数字分开存储，并通过将它们交替放置在结果字符串中来重新排列它们。 如果字母和数字的位数相差大于 1，则打印`-1`，因为可能的排列不可行。
