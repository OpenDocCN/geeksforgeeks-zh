...
> 原文: [https://www.geeksforgeeks.org/remove-array-end-element-maximize-sum-product/](https://www.geeksforgeeks.org/remove-array-end-element-maximize-sum-product/)
...
想法是使用[动态规划](https://www.geeksforgeeks.org/dynamic-programming/)。制作一个名为 `dp[][]` 的 2D 矩阵，用 0 初始化，其中 `dp[i][j]` 表示从数组的索引 `i` 到索引 `j` 的最大得分值。因此，我们的最终结果将存储在 `dp[0][n-1]` 中。
