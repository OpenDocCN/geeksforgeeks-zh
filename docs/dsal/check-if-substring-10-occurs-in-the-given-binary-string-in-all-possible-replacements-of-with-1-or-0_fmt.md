...
给定一个仅由`"0"`、`"1"`和`"?"`组成的字符串`S`，任务是检查在字符`"?"`的每一个可能的替换中是否存在子串`"10"`带 `1` 或 `0`。
...
给定的问题可以通过使用[贪婪方法](https://www.geeksforgeeks.org/greedy-algorithms/)来解决，该方法基于以下观察:如果字符串`S`包含许多连续的`"?"`，可以换成单个`'?'`最坏的情况下我们可以全部用 `1s` 或者 `0s` 代替。

因此，想法是通过替换连续的`"?"`从给定的字符串`S`创建一个新的字符串带单`"?"`然后检查是否存在`"10"`或`"1?0"`作为子串，那么在所有可能的替换后就有可能得到`"10"`作为子串，因此，打印`是`。否则，打印`否`。
