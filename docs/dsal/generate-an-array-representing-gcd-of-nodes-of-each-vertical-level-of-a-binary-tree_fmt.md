...
> 原文: [https://www.geeksforgeeks.org/generate-an-array-representing-gcd-of-nodes-of-each-vertical-level-of-a-binary-tree/](https://www.geeksforgeeks.org/generate-an-array-representing-gcd-of-nodes-of-each-vertical-level-of-a-binary-tree/)

给定一个[二叉树](https://www.geeksforgeeks.org/binary-tree-data-structure/)，任务是构造一个数组，使得数组的第 i 个索引包含存在于给定二叉树的第 i 个垂直层次的所有节点的[GCD](https://www.geeksforgeeks.org/basic-and-extended-euclidean-algorithms/)。
...
给定的问题可以通过对给定的树执行[垂直顺序遍历](https://www.geeksforgeeks.org/print-binary-tree-vertical-order/)来解决，并存储出现在相同垂直线中的每个节点的值，然后打印为每个级别存储的所有值的[GCD](https://www.geeksforgeeks.org/gcd-two-array-numbers/)。请按照以下步骤解决此问题:

1.  初始化一个[图](https://www.geeksforgeeks.org/python-map-function/) `M` 来存储执行遍历时每条垂直线的所有节点的 GCD。
2.  初始化一个变量，将 `hd` 设为 `0` 来记录水平距离。
3.  [递归遍历给定的树](https://www.geeksforgeeks.org/dfs-traversal-of-a-tree-using-recursion/)并执行以下步骤:
    - 将当前节点的值存储在地图中 `M` 作为**键**作为 `HD`，**值**作为**节点的值**。
    - 将变量 `hd` 递减 `1`，并递归调用左子树。
    - 将变量 `hd` 增加 `1`，并递归调用右子树。
4.  [遍历地图](https://www.geeksforgeeks.org/iterate-over-a-dictionary-in-python/)找到地图中存储的每个水平距离的所有节点的 GCD 作为关键，打印得到的 GCD 值。
