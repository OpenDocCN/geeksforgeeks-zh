...
> 原文：[https://www.geeksforgeeks.org/sort-array-converting-elements-squares/](https://www.geeksforgeeks.org/sort-array-converting-elements-squares/)
...
是先将每个数组元素转换成它的平方，然后应用任意`O(nlogn)`排序算法对数组元素进行排序。
...
### 算法–
*   初始化`左=0`和`右=n-1`
*   如果`abs(左)>= abs(右)`，则在结果数组的末尾存储`square(arr[左])`，并递增左指针
