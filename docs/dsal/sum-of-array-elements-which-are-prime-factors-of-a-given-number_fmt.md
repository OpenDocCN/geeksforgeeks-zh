...
> 原文: [https://www.geeksforgeeks.org/sum-of-array-elements-which-are-prime-factors-of-a-given-number/](https://www.geeksforgeeks.org/sum-of-array-elements-which-are-prime-factors-of-a-given-number/)

给定一个大小为 `N` 的数组 `arr[]` 和一个正整数 `K`，任务是找到所有数组元素的和，它们是 `K` 的素因子。
...
思路是遍历数组，对于每个数组元素，检查是否是 `K` 的素因子。将条件满足的那些元素加到总和中。按照以下步骤解决问题:

*   初始化一个变量，比如 `sum`，来存储需要的和。
*   遍历给定的数组，对于每个数组元素，执行以下操作:
    *   检查数组元素是否是 `K` 的质因数。
    *   如果发现为真，则将当前元素添加到 `sum` 中。
*   完成数组遍历后，打印 `sum` 的值作为结果。
