...
> 原文：[https://www.geeksforgeeks.org/find-maximum-xor-of-k-elements-in-an-array/](https://www.geeksforgeeks.org/find-maximum-xor-of-k-elements-in-an-array/)
...
问题可以使用[动态规划](https://www.geeksforgeeks.org/dynamic-programming/)解决。创建一个 `dp` 表 `dp[i][j][mask]`，该表在第 `i` 索引处（包括或不包括）存储可能的最大异或，并且 `j` 表示我们可以包含在 `K` 元素的子集中的剩余元素的数量。掩码是直到第 `i` 个索引为止所选所有元素的异或。
