...
> 原文: [https://www.geeksforgeeks.org/newtons-divided-difference-interpolation-formula/](https://www.geeksforgeeks.org/newtons-divided-difference-interpolation-formula/)
...
假设 `f(x_0)`、`f(x_1)`、`f(x_2)`……`f(x_n)` 是与自变量 `x=x_0`、`x_1`、`x_2`……`x_n` 对应的函数 `y=f(x)` 的 `(n+1)` 值，其中区间差不相同。
...
以此类推……划分的差异相对于自变量是对称的，即与自变量的顺序无关。
所以，`f[x_0, x_1] = f[x_1, x_0]`，`f[x_0, x_1, x_2] = f[x_2, x_1, x_0]`。

这些划分的差异形成了一个表，称为划分差异表。
...
```
f(x)=f(x_0)+(x-x_0)f[x_0, x_1]+(x-x_0)(x-x_1)f[x_0, x_1, x_2]+..........................+(x-x_0)(x-x_1)...(x-x_{k-1})f[x_0, x_1, x_2...x_k]
```
