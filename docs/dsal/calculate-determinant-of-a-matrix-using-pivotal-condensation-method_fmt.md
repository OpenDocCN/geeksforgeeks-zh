...
> 原文: [https://www.geeksforgeeks.org/calculate-determinant-of-a-matrix-using-pivotal-condensation-method/](https://www.geeksforgeeks.org/calculate-determinant-of-a-matrix-using-pivotal-condensation-method/)
...
> 执行 `R2 = R2–R1` 将矩阵 `mat[][]` 修改为 `{{1，2，3}，{1，1，1}，{1，1，1}}`。
> 现在，行 `R2` 和 `R3` 相等。
> 因此，矩阵的行列式的值等于零（利用矩阵的性质）。
>
...
思路是利用[枢纽凝聚法](https://en.wikipedia.org/wiki/Pivot_element)计算矩阵 `mat[][]` 的[行列式](https://www.geeksforgeeks.org/determinant-of-a-matrix/)。以下是建议方法的详细说明：

在这种计算维度为 `N × N` 的[方阵](https://www.geeksforgeeks.org/how-to-access-elements-of-a-square-matrix/)的行列式的方法中：

*   首先将维度为 `N*N` 的矩阵 `A[][]` 简化为维度为 `(N–1)*(N–1)` 的矩阵 `B[][]`：
...
*   现在进一步将矩阵化简为 `(N–2)*(N–2)`，计算矩阵 `B[][]` 的行列式。
*   重复上述过程，直到矩阵变成尺寸 `2*2`。
*   然后使用公式 `det(A) = ad-bc` 计算维度为 `2×2` 的矩阵的行列式，对于矩阵 `A[][]` 为 `{{a，b}，{c，d}}`。
...
*   当 `N` 大于 `2` 时迭代，检查以下内容：
    ...
    *   现在，将 `D` 乘以 `pow(1 / mat[0][0], N–2)`。
    *   使用公式 `B[i-1][j-1] = mat[0][0] * mat[i][j] - mat[i][0] * mat[0][j]`，计算维度为 `(N–1)x(N–1)` 的下一个矩阵，比如 `B[][]`。
    *   分配 `mat = B`。
*   将 `D` 乘以维度为 `2×2` 的矩阵 `mat[][]` 的行列式，即 `mat[0][0] * mat[1][1] – mat[0][1] * mat[1][0]`。
*   最后，打印存储在 `D` 中的值。
