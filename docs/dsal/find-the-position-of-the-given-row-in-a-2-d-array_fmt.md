...
> 原文: [https://www.geeksforgeeks.org/find-the-position-of-the-given-row-in-a-2-d-array/](https://www.geeksforgeeks.org/find-the-position-of-the-given-row-in-a-2-d-array/)
...
类似于一维数组上的线性搜索，对矩阵执行线性搜索，并将矩阵的每一行与给定的数组进行比较。如果某行与数组匹配，则打印其行号，否则打印-1。
以下是上述办法的实施情况:
...
由于矩阵是按行排序的，所以我们可以像在一维数组中一样使用二分搜索法。有必要以逐行方式对数组进行排序。下面是使用二分搜索法在矩阵中查找一行的步骤：

1.  将 `arr[]` 与中间一行进行比较。
2.  如果 `arr[]` 与中间行完全匹配，我们返回中间索引。
3.  否则如果 `arr[]` 大于中间行（至少存在一个 `j`，`0<=j<n`，使得 `arr[j] > mat[mid][j]`)，我们检查下半部分。
