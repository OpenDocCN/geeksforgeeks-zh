# 用给定的操作检查弦 S1 是否可以等于 S2

> 原文：[https://www.geeksforgeeks.org/check-whether-the-string-s1-can-be-made-equal-to-s2-with-the-given-operation/](https://www.geeksforgeeks.org/check-whether-the-string-s1-can-be-made-equal-to-s2-with-the-given-operation/)
...
*   根据来自 `S1` 的偶数索引处的字符创建一个字符串 `even_s1`。
*   同样，生成字符串 `even_s2`、`odd_s1` 和 `odd_s2`。
*   对前面步骤中的所有四个字符串进行排序。
*   如果 `even_s1 = even_s2` 和 `odd_s1 = odd_s2` 则打印 `是`。
*   否则打印 `否`，因为字符串不能相等。
...
class GFG
{
    ...
    // can be made equal to s2
    // with the given operation
    static bool canBeMadeEqual(string s1, string s2)
    {
        // Get the string formed by the
        // even indexed characters of s1
        char[] even1 = partEven(s1).ToCharArray();
        ...
        // If the strings can be made equal
        if (even_s1.Equals(even_s2) && odd_s1.Equals(odd_s2))
            return true;
        return false;
    }
