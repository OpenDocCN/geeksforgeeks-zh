...
> 原文：[https://www.geeksforgeeks.org/implementing-forward-iterator-in-bst/](https://www.geeksforgeeks.org/implementing-forward-iterator-in-bst/)
...
我们将创建一个名为 `q` 的栈来存储 BST 节点。创建一个变量 `curr` 并用指向根节点的指针初始化它。当 `curr` 不为空时：
*   将 `curr` 压入栈 `q`。
*   设置 `curr = curr->left`。
...
**时间复杂度：** 平均所有调用为 `O(1)`。在最坏的情况下，单次调用可以为 `O(h)`。

## `isEnd()`

如果栈 `q` 为空，则返回 `true`，否则返回 `false`。

**时间复杂度：** `O(1)`

迭代器的这个实现的最坏情况空间复杂度是 `O(h)`。需要注意的是迭代器指向栈的最顶层元素。
