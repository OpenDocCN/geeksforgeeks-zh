...
> 原文：[https://www.geeksforgeeks.org/program-to-subtract-two-integers-of-given-base/](https://www.geeksforgeeks.org/program-to-subtract-two-integers-of-given-base/)
...
*   初始化两个变量 `power = 1`、`carry = 0`，分别记录当前幂次和减法时产生的进位。
...
*   [循环](https://www.geeksforgeeks.org/range-based-loop-c/)直到 `X > 0` 并执行以下步骤：
    ...
    *   初始化 `temp = n1 - n2 + carry`。
    *   如果 `temp < 0`，则在 `temp` 上加上基数 `B`，即 `temp = temp + B`，设置 `carry = -1`，作为借位。否则，设置 `carry = 0`。
    *   将当前 `temp * power` 加到 `finalVal`，即 `finalVal = finalVal + temp * power`，设置 `power = power * 10`。
*   完成上述步骤后，打印 `finalVal` 作为结果。
