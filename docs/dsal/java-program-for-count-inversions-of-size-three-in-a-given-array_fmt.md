...
> 原文: [https://www.geeksforgeeks.org/java-program-for-count-inversions-of-size-three-in-a-given-array/](https://www.geeksforgeeks.org/java-program-for-count-inversions-of-size-three-in-a-given-array/)

给定大小为 `n` 的数组 `arr[]`。如果 `a[i] > a[j] > a[k]` 和 `i < j < k`，则三个元素 `arr[i]`、`arr[j]` 和 `arr[k]` 形成大小为 3 的反转。找出总的大小为 3 的反转数量。
