...
> 原文：[https://www.geeksforgeeks.org/modify-a-linked-list-to-contain-last-occurrences-of-every-duplicate-element/](https://www.geeksforgeeks.org/modify-a-linked-list-to-contain-last-occurrences-of-every-duplicate-element/)
...
*   初始化一个`dummy`节点，使其成为下一个指向`head`。
*   [反转给定的链表。](https://www.geeksforgeeks.org/reverse-a-linked-list/)
*   初始化一个[无序集](https://www.geeksforgeeks.org/unordered_set-in-cpp-stl/)，比如`visited`，存储已经访问过的节点。
*   初始化两个节点，说`currNode`，指向`dummy`节点，`nextNode`，存储`currNode`节点的`next`节点。
*   [遍历链表](https://www.geeksforgeeks.org/how-to-iterate-linkedlist-in-java/)，检查`currNode`节点的`next`节点的数据是否已经被访问过。
*   如果已经访问过，请执行以下步骤：
    *   初始化一个新节点，比如说`duplicate`，来存储`nextNode`，在这种情况下是一个复制节点。
    *   使`currNode`的`next`指向`nextNode`的`next`。
*   否则：
    *   将`nextNode`的数据插入到`visited`的集合中。
    *   使`nextNode`成为`currNode`。
*   最后，反转修改后的链表并返回。
