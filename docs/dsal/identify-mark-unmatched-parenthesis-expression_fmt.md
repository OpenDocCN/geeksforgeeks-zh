...
> 原文: [https://www.geeksforgeeks.org/identify-mark-unmatched-parenthesis-expression/](https://www.geeksforgeeks.org/identify-mark-unmatched-parenthesis-expression/)
...
这个想法基于一个[堆栈](https://www.geeksforgeeks.org/stack-data-structure/)。我们从字符串的开头到结尾运行一个循环，对于每个 `'('`，我们将它推入一个堆栈。如果堆栈是空的，并且我们遇到一个右括号 `")"`，我们在字符串的索引处替换 `-1`。否则，我们用 `0` 替换所有左括号 `"("`，用 `1` 替换右括号。然后从堆栈中弹出。
