...
三个圆 `ABC` 相交形成的形状为**勒乌三角**，相同顶点即 `ABC` 形成的三角形为边 `h` 的等边三角形。

> 现在，ACB 区面积， `A1 = (π * h^2) / 6`
> 同样，CBA 区面积，`A2 = (π * h^2) / 6`
> 和，BAC 区面积，`A3 = (π * h^2) / 6`
> 所以， `A1 + A2 + A3 = (π * h^2) / 2`
> 所以，勒鲁三角形的面积，`A = (π * h^2)/2 - 2 * (等边三角形的面积) = (π - √3) * h^2/2 = 0.70477 * h^2`
