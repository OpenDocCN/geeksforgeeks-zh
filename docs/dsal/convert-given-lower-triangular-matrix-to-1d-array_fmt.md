...
> 原文: [https://www.geeksforgeeks.org/convert-given-lower-triangular-matrix-to-1d-array/](https://www.geeksforgeeks.org/convert-given-lower-triangular-matrix-to-1d-array/)
...
> 位置 `(i，j)` 处矩阵元素的索引为 `((i *(i–1))/2+j–1)`，其中 `1 ≤ i，j ≤ N 且 i ≥ j`
...
> 位置 `(i，j)` 处矩阵元素的索引为 `(N *(j–1)–((j–2)*(j–1))/2)+(i–j)`，其中 `1 ≤ i，j ≤ N 且 i ≥ j`。
