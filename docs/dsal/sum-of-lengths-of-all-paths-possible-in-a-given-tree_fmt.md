...
> 原文: [https://www.geeksforgeeks.org/sum-of-lengths-of-all-paths-possible-in-a-given-tree/](https://www.geeksforgeeks.org/sum-of-lengths-of-all-paths-possible-in-a-given-tree/)
...
检查所有可能的路径，然后添加它们来计算最终结果。这种方法的复杂性将是 `O(2^N)`。

## 高效进场

可以注意到，一棵树的每一条边都是一座[桥](https://www.geeksforgeeks.org/bridge-in-a-graph/)。因此，该边将出现在该边所连接的两个子树之间的每条可能路径中。
例如，边 `(1–0)` 存在于 `{1, 3, 4}` 和 `{0, 2}` 之间的每条可能路径中，`(1–0)` 被使用了 `size({1, 3, 4}) * size({0, 2})` 次。因此，对于每条边，计算这条边将被考虑多少次，以得到通过它的路径。[DFS](https://www.geeksforgeeks.org/depth-first-traversal-for-a-graph/) 可以用来存储子树的大小，所有边的贡献可以用另一个 DFS 来计算。这种方法的复杂性将是 `O(n)`。
