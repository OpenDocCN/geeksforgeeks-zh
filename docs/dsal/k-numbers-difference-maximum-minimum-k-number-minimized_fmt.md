...
> 原文：`https://www.geeksforgeeks.org/k-numbers-difference-maximum-minimum-k-number-minimized/`

给定一组 `n` 个整数和一个正数 `k`。我们可以从给定的数组中取任意 `k` 个整数。任务是找到 `K` 个数的最大值和最小值之差的最小可能值。
