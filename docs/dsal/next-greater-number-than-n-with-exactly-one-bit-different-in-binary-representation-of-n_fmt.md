...
> 原文：[https://www.geeksforgeeks.org/next-greater-number-than-n-with-exactly-one-bit-different-in-binary-representation-of-n/](https://www.geeksforgeeks.org/next-greater-number-than-n-with-exactly-one-bit-different-in-binary-representation-of-n/)
...
我们举个例子，`1001` 是 9 的二进制形式，如果我们把 `1001` 的设定位转换成 `1000` 或 `0001`，那么我们发现数字是 8 和 1，它们都小于 `N`。而如果我们把 0 的位转换成 1，我们发现数字是 11 (`1011`) 或 13 (`1101`)，所以如果我们把最小的可能位转换成 1，那么我们得到大于 `N` 的最小可能数，只有 1 位不同，在这种情况下。
