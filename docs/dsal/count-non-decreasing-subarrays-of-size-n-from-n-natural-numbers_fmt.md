...
> 原文: [https://www.geeksforgeeks.org/count-non-decreasing-subarrays-of-size-n-from-n-natural-numbers/](https://www.geeksforgeeks.org/count-non-decreasing-subarrays-of-size-n-from-n-natural-numbers/)
...
> **解释:**
> 给定 `N` 个自然数的数组: `{1, 2, 3}`
> 可以构成的必需子数组: `[1, 1, 1]`, `[1, 1, 2]`, `[1, 2, 2]`, `[2, 2, 2]`, `[1, 1, 3]`, `[1, 3, 3]`, `[3, 3, 3]`

## 进场:

*   由于阵列的每个元素在 `1` 到 `N` 之间，并且子阵列可以具有非降序的重复元素，即 `a[0]≤a[1]≤……≤a[N–1]`。
*   从 `n` 个对象中选择替换 `r` 个对象的方式数量为 `C(n+r-1, r)` (使用[组合重复](https://www.geeksforgeeks.org/combinations-with-repetitions/))。
*   这里 `r = N` 和 `n = N`，因为我们可以从 `1` 到 `N` 中进行选择，所以所有长度为 `N` 且元素从 `1` 到 `N` 的排序数组的计数将是 `C(2*n-1, n)`。
