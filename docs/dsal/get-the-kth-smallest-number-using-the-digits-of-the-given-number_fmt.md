...
> 原文：[https://www.geeksforgeeks.org/get-the-kth-smallest-number-using-the-digits-of-the-given-number/](https://www.geeksforgeeks.org/get-the-kth-smallest-number-using-the-digits-of-the-given-number/)

给定一个非负数 `n` 和一个值 `k`。使用给定数字 `n` 的数字，找到能够形成的第 `k` 个最小数字。保证能形成第 `k` 个最小数。请注意，该数字可能非常大，甚至可能不适合 `long long int`。
