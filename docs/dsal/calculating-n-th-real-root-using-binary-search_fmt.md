...
> 原文: [https://www.geeksforgeeks.org/calculating-n-th-real-root-using-binary-search/](https://www.geeksforgeeks.org/calculating-n-th-real-root-using-binary-search/)
...
5.  运行循环，以便:
    ...
    *   如果我们的 `猜测` 的 `绝对误差` 小于 `ε`，那么退出循环。

**绝对误差:** 绝对误差可以计算为 `abs(猜测^n - x)`。
