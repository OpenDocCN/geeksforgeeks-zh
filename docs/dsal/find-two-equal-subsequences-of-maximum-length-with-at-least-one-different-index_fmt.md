...
> 原文：[https://www.geeksforgeeks.org/find-two-equal-subsequences-of-maximum-length-with-at-least-one-different-index/](https://www.geeksforgeeks.org/find-two-equal-subsequences-of-maximum-length-with-at-least-one-different-index/)

给定一个字符串 `str`，任务是找到最大长度 `K`，使得存在两个子序列 `A` 和 `B`，每个长度 `K`，满足 `A = B` 并且 `A` 和 `B` 之间的公共索引数量最多为 `K–1`。
