...
> 原文: [https://www.geeksforgeeks.org/smallest-and-largest-palindrome-with-n-digits/](https://www.geeksforgeeks.org/smallest-and-largest-palindrome-with-n-digits/)
...
仔细观察会发现，对于 `N = 1`，最小回文数为 `0`。对于 `N` 的任何其他值，最小的回文的第一个和最后一个数字都是 `1`，中间的所有数字都是 `0`。

*   情况 1: 如果 `N = 1`，那么答案将是 `0`。
*   案例二: 如果 `N != 1` 则答案为 `(10^(N-1)) + 1`。

## 最大 N 位回文数

和上面的做法类似，可以看到 `N` 次追加 `9` 可以得到 `N` 位可能的最大回文数。因此，最大的 `N` 位回文数字将是 `10^N - 1`。
