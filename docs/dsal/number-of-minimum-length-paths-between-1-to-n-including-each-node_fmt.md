...
> 原文: [https://www.geeksforgeeks.org/number-of-minimum-length-paths-between-1-to-n-including-each-node/](https://www.geeksforgeeks.org/number-of-minimum-length-paths-between-1-to-n-including-each-node/)
...
**方法:** 给定的问题可以通过执行两个 [BFS](https://www.geeksforgeeks.org/breadth-first-traversal-for-a-graph/) 来解决，一个从排除节点 `N` 的节点 `1` 开始，另一个从排除节点 `N` 的节点 `1` 开始，找到所有节点到 `1` 和 `N` 的最小距离，两个最小距离的乘积将是从 `1` 到的最小长度路径的总数。按照以下步骤解决问题:

*   初始化一个[队列](https://www.geeksforgeeks.org/queue-data-structure/)，比如说 `queue1` 从节点 `1` 执行 `BFS` ，以及一个队列 `queue2` 从节点 `N` 执行 `BFS`。
*   初始化[数组](https://www.geeksforgeeks.org/arrays-in-java/)，说 `dist[]` 存储最短距离，`ways[]` 统计到达该节点的路数。
*   执行两次 `BFS`，并在每种情况下执行以下步骤:
    *   [从队列](https://www.geeksforgeeks.org/queuepush-and-queuepop-in-cpp-stl/)弹出，将节点存储在 `x` 中，其距离存储在 `dis` 中。
    *   如果 `dist[x]` 小于 `dis`，则[继续](https://www.geeksforgeeks.org/continue-statement-cpp/)。
    *   遍历 `x` 的邻接表，对于每个孩子 `y` ，如果 `dist[y]` 大于 `dis + 1` ，则更新 `dist[y]` 等于 `dis+1`，`ways[y]` 等于 `ways[x]`。否则，如果 `dist[y]` 等于 `dis+1` ，则在 `ways[y]` 上增加 `ways[x]`。
*   最后，迭代范围 `N` ，对于每个节点，将**最小**长度路径的计数打印为 `ways1[i] * ways2[i]` 。
