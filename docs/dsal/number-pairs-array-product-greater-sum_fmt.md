...
> 原文: [https://www.geeksforgeeks.org/number-pairs-array-product-greater-sum/](https://www.geeksforgeeks.org/number-pairs-array-product-greater-sum/)

给定非负整数数组 `a[]`。计算数组中的对 `(i, j)` 的数量，使得 `a[i] + a[j] < a[i] * a[j]`。（对 `(i, j)` 和 `(j, i)` 被视为相同，且 `i` 不应等于 `j`）
...
当 `a[i] = 1` 时：`a[i] * a[j] = a[j]` 和 `a[i] + a[j] = 1 + a[j]`，因此当 `a[i] = 1` 时，找不到配对。
当 `a[i] = 2` 且 `a[j] = 2` 时：`a[i] * a[j] = 4`，`a[i] + a[j] = 4`，找不到配对。
要解决这个问题，统计数组中 `2`s 的个数，比如 `twoCount`。计算数组中大于 `2` 的数字，比如 `twoGreaterCount`。答案是 `twoCount * twoGreaterCount + twoGreaterCount * (twoGreaterCount - 1) / 2`。
...
本文由 **Ayush Jha** 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用`write.geeksforgeeks.org`写一篇文章或者把你的文章邮寄到`contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
