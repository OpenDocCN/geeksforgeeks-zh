# `std::string::append` vs `std::string::push_back()` vs `operator+=` 在 C++ 中

> 原文: [https://www.geeksforgeeks.org/stdstringappend-vs-stdstringpush_back-vs-operator-c/](https://www.geeksforgeeks.org/stdstringappend-vs-stdstringpush_back-vs-operator-c/)

要追加字符，可以使用运算符`+=`、`append()`和`push_back()`。所有这些都有助于添加字符，但在实现和应用上略有不同。
