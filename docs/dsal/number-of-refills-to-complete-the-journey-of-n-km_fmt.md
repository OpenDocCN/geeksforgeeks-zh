...
> 原文: [https://www.geeksforgeeks.org/number-of-refills-to-complete-the-journey-of-n-km/](https://www.geeksforgeeks.org/number-of-refills-to-complete-the-journey-of-n-km/)
...
由于总行程为 `N` 公里，所以在一个变量中记录到目前为止所覆盖的距离，比如 `distCovered`，该变量将被初始化为 0。将距离增加 `K` 公里，直到 `distCovered` 小于 `N`，因为 `K` 是自上次加注后车辆可以行驶的距离。此外，每次递增时，检查在 `distCovered` 和 `distCovered + K` 之间是否有强制停油泵，如果有，那么 `distCovered` 将是 `K` 加上最后一个在 `distCovered` 和 `distCovered + K` 之间停油泵的强制停油泵的位置。
以下是上述方法的实施：
