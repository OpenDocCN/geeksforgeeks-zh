...
*   借助[`ConcurrentHashMap`](https://www.geeksforgeeks.org/concurrenthashmap-in-java/)（类似于`Hashtable`，同步，但更快，因为使用了多个锁）
...
# Java 集合框架示例：LinkedHashSet 与 TreeSet
...
借助 `TreeSet`（实现了 `SortedSet` 接口，对象以升序存储）。
