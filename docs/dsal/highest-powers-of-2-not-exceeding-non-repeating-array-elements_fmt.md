...
> 原文: [https://www.geeksforgeeks.org/highest-powers-of-2-not-exceeding-non-repeating-array-elements/](https://www.geeksforgeeks.org/highest-powers-of-2-not-exceeding-non-repeating-array-elements/)

给定一个大小为 `N` 的阵 `arr[]`，任务是为每个非重复阵元寻找不超过该元的最高 2 次方。按升序打印 2 的次幂。如果数组不包含任何非重复元素，则打印 `0`。
...
解决这个问题最简单的方法是遍历数组，对于每个数组元素，检查它是否不重复。要使元素不重复，请将它们添加到另一个数组中。然后，对于新数组中的每个元素，找到不超过该元素的 2 的最高次方，并按升序打印。
**时间复杂度:** `O(N^2 * log arr[i])`，其中 `arr[i]` 为数组最大个数。
**辅助空间:** `O(N)`

## 高效方法

最佳思路是使用 `哈希`。按照以下步骤解决问题:

*   遍历数组 `arr[]` 并将每个数组元素的频率存储在 `映射` 中。
*   现在，遍历地图检查任一元素的频率是否为 `1`。
*   对于所有这些元素，找到不超过该元素的 2 的最高次幂，并将其存储在 `向量` 中。
*   按升序排列向量并打印向量中存在的元素。
