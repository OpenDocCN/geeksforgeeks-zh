...
> 原文: [https://www.geeksforgeeks.org/print-array-elements-in-alternatively-increasing-and-decreasing-order/](https://www.geeksforgeeks.org/print-array-elements-in-alternatively-increasing-and-decreasing-order/)
...
思路是使用[双指针技巧](https://www.geeksforgeeks.org/two-pointers-technique/)。首先按递增顺序对数组进行排序，并保持两个指针 `l` 和 `r`，其中 `l` 是按递增顺序打印数组，`r` 是按递减顺序打印数组。保留一个变量 `K` 以指定迭代中要打印的元素数量，并保留一个变量 `flag` 以交替地在升序和降序打印之间切换。
