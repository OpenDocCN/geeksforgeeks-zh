...
> 原文: [https://www.geeksforgeeks.org/queries-to-update-a-given-index-and-find-gcd-in-range/](https://www.geeksforgeeks.org/queries-to-update-a-given-index-and-find-gcd-in-range/)
...
使用[线段树](https://www.geeksforgeeks.org/segment-tree-set-1-sum-of-given-range/)可以解决以下问题。线段树可用于中等时间的预处理和查询。对于线段树，预处理时间为 `O(n)`，GCD 查询的时间为 `O(log n)`。存储线段树所需的额外空间为 `O(n)`。
...
**树的数组表示用于表示线段树，即对于索引 `i` 处的每个节点**

*   左边的孩子在索引 `2*i+1`
*   右边的孩子在 `2*i+2`，父母在 `floor((i-1)/2)`。

### 从给定的数组构建线段树

*   以段 `arr[0...n-1]` 开始并继续分成两半。每次我们将当前段分成两半（如果它还没有变成长度为 1 的段），然后在两半上调用相同的过程，对于每个这样的段，我们将 GCD 值存储在线段树节点中。
*   构建的线段树的所有层都将被完全填充，除了最后一层。此外，该树将是一个完整的二叉树（每个节点有 0 或两个子节点），因为我们总是在每个级别将段分成两半。
*   由于构造的树总是有 `n` 片叶子的全二叉树，因此会有 `n-1` 个内部节点。因此，节点总数将为 `2 * n - 1`。

像树构造和查询操作一样，更新也可以递归完成。我们得到了一个需要更新的索引。让 `diff` 成为要添加的值。我们从线段树的根开始，将 `diff` 添加到在其范围内给定索引的所有节点。如果一个节点在其范围内没有给定的索引，我们不会对该节点进行任何更改。
