...
> 原文: [https://www.geeksforgeeks.org/find-nth-number-in-a-sequence-which-is-not-a-multiple-of-a-given-number/](https://www.geeksforgeeks.org/find-nth-number-in-a-sequence-which-is-not-a-multiple-of-a-given-number/)
...
天真法是循环迭代 `[L, R]` 的范围，找到第 `N` 个数。步骤如下：

1.  将非倍数数和当前数的计数初始化为 0。
2.  迭代范围 `[L, R]`，直到非倍数计数不等于 `N`。
3.  如果当前数字不能被 `A` 整除，则将非倍数的计数增加 1。
...
关键观察是 `[1, A–1]` 范围内有 `A–1` 个数不能被 `A` 整除。同样，在 `[A+1, 2 * A–1]`、`[2 * A + 1, 3 * A–1]` 等范围内，也有 `A–1` 个不能被 `A` 整除的数字。
借助这个观察，不可被 `A` 整除的第 `N` 个数将是：
