...
> 原文：[https://www.geeksforgeeks.org/count-inversions-in-a-sequence-generated-by-appending-given-array-k-times/](https://www.geeksforgeeks.org/count-inversions-in-a-sequence-generated-by-appending-given-array-k-times/)

## 问题描述

给定一个数组`arr[]`，任务是将给定数组精确追加`K–1`次到它的末尾，并在结果数组中打印出反转总数。
...
最简单的方法是将给定数组的`K`个副本存储在一个向量中，然后，计算得到的向量的逆序计数。
...
解决这个问题的思路是先找到给定数组中反转总数，比如`inv`。然后，计算单个副本中不同元素对的数量，比如`X`。现在，通过以下等式计算追加数组的`K`个副本后的求逆总数：
