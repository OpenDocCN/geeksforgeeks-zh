...
> 原文：[https://www.geeksforgeeks.org/number-of-shortest-paths-to-reach-every-cell-from-bottom-left-cell-in-the-grid/](https://www.geeksforgeeks.org/number-of-shortest-paths-to-reach-every-cell-from-bottom-left-cell-in-the-grid/)

给定两个数字 `N` 和 `M`。任务是当移动从左下角开始时，在大小为 `N × M` 的网格中找到到达单元格 `(I, j)` 的最短路径数。注：单元格 `(I, j)` 代表网格中的第 `i` 行和 `j` 列。下图显示了到达 `4 × 4` 网格中的单元格 `(1, 4)` 的一些最短路径。
...
*   到达单元格 `(n, i)` 的最短路径数为 `1`，其中 `1 <= i <= M`。
*   到达单元格 `(i, 1)` 的最短路径数是 `1`，其中 `1 <= i <= N`。
*   到达单元格 `(i, j)` 的最短路径数是单元格 `(i-1, j)` 和 `(i, j+1)` 的最短路径数之和，其中 `1 <= j <= M` 和 `1 <= i <= N`。
