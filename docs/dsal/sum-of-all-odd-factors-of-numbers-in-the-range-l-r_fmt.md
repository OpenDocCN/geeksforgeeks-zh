...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-odd-factors-of-numbers-in-the-range-l-r/](https://www.geeksforgeeks.org/sum-of-all-odd-factors-of-numbers-in-the-range-l-r/)
...
> 因子(`7`) = `1，7`，奇数因子(`7`) = `1，7`，`sum_Odd_Factors(7) = 1 + 7 = 8`
> 因子(`8`) = `1，2，4，8`，奇数因子(`8`) = `1`，`sum_Odd_Factors(8) = 1`

**方法:** 我们可以修改厄拉多塞的筛法，将一个数的所有奇数因子的和存储在其对应的索引处。然后我们将创建一个前缀数组来存储该索引的总和。现在每个查询都可以用`prefix[r] – prefix[l–1]`在`O(1)`中回答。
