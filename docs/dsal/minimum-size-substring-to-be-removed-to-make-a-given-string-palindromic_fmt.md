...
> 原文: [https://www.geeksforgeeks.org/minimum-size-substring-to-be-removed-to-make-a-given-string-palindromic/](https://www.geeksforgeeks.org/minimum-size-substring-to-be-removed-to-make-a-given-string-palindromic/)

给定一个字符串 `S`，任务是在删除最小尺寸[子字符串](https://www.geeksforgeeks.org/substring-in-java/)后打印该字符串，使得 `S` 成为[回文](https://www.geeksforgeeks.org/c-program-check-given-string-palindrome/)。
...
想法是包含给定字符串 `S` 的最大大小[前缀和后缀](https://www.geeksforgeeks.org/string-from-prefix-and-suffix-of-given-two-strings/)，它们的连接形成一个[回文](https://www.geeksforgeeks.org/tag/palindrome/)。然后，从剩余的本身就是回文的字符串中选择最大长度的前缀或后缀。下面是借助图像的方法说明:
