...
> 原文: [https://www.geeksforgeeks.org/rearrange-an-array-such-that-product-of-every-two-consecutive-elements-is-a-multiple-of-4/](https://www.geeksforgeeks.org/rearrange-an-array-such-that-product-of-every-two-consecutive-elements-is-a-multiple-of-4/)
...
*   初始化以下三个变量:
    *   `odd` = 奇数元素的数量。
    *   `four` = 可被 4 整除的元素数量。
    *   `non_four` = 不能被 4 整除的偶数元素数。
*   考虑以下两种情况:
    *   **情况 1: `non_four = 0`**
        在这种情况下，不存在不能被 4 整除的偶元素。
        *   最优的方式是先将 `odd` 元素放在数组中的交替位置。
        *   用偶数元素填补空缺。
...
> **图解:**
> `arr[] = {2, 7, 1, 8, 2, 8}`
> 第一步: `four = 2`, `non_four = 2`, `odd = 2`
> 第二步: `{_ 8 _ 8}`
> 第三步: `{1 8 7 8}`
> 第四步: `{1 8 7 8 2}`

*   从数学上来说，这是可能的，`four >= odd`。
