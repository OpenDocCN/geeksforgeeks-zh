...
> 原文：[https://www.geeksforgeeks.org/move-spaces-front-string-single-traversal/](https://www.geeksforgeeks.org/move-spaces-front-string-single-traversal/)
...
思路是从头到尾维护两个索引 `i` 和 `j`。如果当前索引包含空格，将索引 `i` 中的字符与索引 `j` 交换。这将使所有空格都位于数组的开头。
