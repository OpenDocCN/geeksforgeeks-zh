...
> 原文: [https://www.geeksforgeeks.org/sum-of-bitwise-or-of-every-array-element-paired-with-all-other-array-elements/](https://www.geeksforgeeks.org/sum-of-bitwise-or-of-every-array-element-paired-with-all-other-array-elements/)
...
> 对于 `i = 1` 所需的和将是 `(2 | 1) + (2 | 2) + (2 | 3) + (2 | 4) = 14`
...
**高效方法:** 最佳思路是使用[位操作](https://www.geeksforgeeks.org/bits-manipulation-important-tactics/)，假设整数使用 **32** 位表示。按照以下步骤解决问题:
...
5.  如果设置了 `arr[i]` 的第 `k` 位，则意味着每一个可能的 `(arr[i] | arr[j])` 的第 `k` 位也将被设置。所以在这种情况下，将 `(1 << k) * N` 加到所需的**和**上。
6.  否则，如果未设置 `arr[i]` 的第 `k` 位，则意味着当且仅当 `arr[j]` 的第 `k` 位设置时，将设置 `(arr[i] | arr[j])` 的第 `k` 位。因此在这种情况下，将 `(1 << k) * freq[k]` 添加到所需的和中，该和先前计算为 `freq[k]` 元素数量设置了第 `k` 位。
