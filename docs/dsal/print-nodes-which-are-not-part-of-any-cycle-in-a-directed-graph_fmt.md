...
给定由值为 `[0, N-1]` 的节点组成的有向图 `G`，包含 `N` 个节点和 `E` 条边，以及表示顶点 `u` 和 `v` 之间有向边的类型为 `{u, v}` 的 2D 数组 `Edges[][2]`。任务是在给定的图 `G` 中找到不属于任何循环的节点。

## 示例

**输入:**
`N = 4`，`E = 4`，`Edges[][2] = { {0, 2}, {0, 1}, {2, 3}, {3, 0} }`
**输出:**
`1`
**解释:**

![img](img/05763e3c10b44e507101abddf59104bf.png)
...
**输入:**
`N = 6`，`E = 7`，`Edges[][2] = { {0, 1}, {0, 2}, {1, 3}, {2, 1}, {2, 5}, {3, 0}, {4, 5} }`
**输出:**
`4 5`
**解释:**

![img](img/ab490d759b83e56607641da5d3338540.png)
...
最简单的方法是为给定图中的每个节点检测有向图中的一个循环，并且只打印那些不属于给定图中任何循环的节点。
**时间复杂度:** `O(V * (V + E))`，其中 `V` 为顶点数，`E` 为边数。
**辅助空间:** `O(V)`

## 高效方法

为了优化上述方法，其思想是每当给定图中有任何循环时，将中间节点存储为“循环节点”。要实现这一部分，请使用辅助数组 `cyclePart[]`，该数组将在执行 DFS 遍历时存储中间循环节点。以下是步骤:

*   初始化一个大小为 `N` 的辅助数组 `cyclePart[]`，这样如果 `cyclePart[i] = 0`，那么第 `i` 个节点在任何循环中都不存在。
*   初始化一个大小为 `N` 的辅助数组 `recStack[]`，这样它将通过将被访问节点标记为 `true` 来将该节点存储在递归堆栈中。
*   对每个未访问的节点在给定的图上执行 DFS 遍历，并执行以下操作:
    *   现在在给定的图中找到一个循环，每当找到一个循环时，将循环中的节点标记为 `true`，因为该节点是循环的一部分。
    *   如果任何节点在递归调用中被访问并且 `recStack[节点]` 也为真，则该节点是循环的一部分，然后将该节点标记为 `true`。
*   执行 `DFS` 遍历后，遍历数组 `cyclePart[]` 并打印所有标记为 `false` 的节点，因为这些节点不是任何循环的一部分。
...
```cpp
// Stores the nodes that are part of any cycle
...
for (int i = 0; i < V; i++) {
    ...
    if (!visited[i]) {
        ...
        if (printNodesNotInCycleUtil(i, visited, recStack, cyclePart)) {
            // Mark as cycle node if it return true
            cyclePart[i] = 1;
        }
    }
}
...
for (int i = 0; i < V; i++) {
    // If node i is not a part of any cycle
    if (cyclePart[i] == 0) {
        cout << i << " ";
    }
}
...
```cpp
// Function that print the nodes for the given directed graph that are not present in any cycle
void solve(int N, int E, int Edges[][2]) {
...
class GFG {
    ...
    // Function to perform DFS Traversal and return true if current node v formes cycle
    static boolean printNodesNotInCycleUtil(int v, boolean visited[], boolean[] recStack, boolean[] cyclePart) {
        // If node v is unvisited
        if (visited[v] == false) {
            ...
            for (Integer child : adj.get(v)) {
                ...
                if (!visited[child] && printNodesNotInCycleUtil(child, visited, recStack, cyclePart)) {
                    // If child node is a part of cycle node
    ...
    // Function that print the nodes for the given directed graph that are not present in any cycle
    static void solve(int N, int E, int Edges[][]) {
        adj = new ArrayList<>();
        for (int i = 0; i < N; i++)
...
class Graph:
    ...
    # Function that adds directed edges between node v with node w
    ...
    def printNodesNotInCycle(self):
        ...
        for i in range(self.V):
            ...
            if (not visited[i]):
                # Perform DFS Traversal
                if (self.printNodesNotInCycleUtil(i, visited, recStack, cyclePart)):
                    # Mark as cycle node if it return True
                    cyclePart[i] = 1
