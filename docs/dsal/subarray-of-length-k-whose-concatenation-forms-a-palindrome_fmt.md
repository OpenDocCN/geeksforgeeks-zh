...
> 原文: [https://www.geeksforgeeks.org/subarray-of-length-k-whose-concatenation-forms-a-palindrome/](https://www.geeksforgeeks.org/subarray-of-length-k-whose-concatenation-forms-a-palindrome/)

给定一个由 `[0, 9]` 范围内的 `N` 个整数组成的数组 `arr[]`，任务是找到一个长度为 `K` 的子数组，从中我们可以生成一个数字回文数。如果没有这样的子阵列，打印 `-1`。
...
解决问题最简单的方法是生成所有长度为 `K` 的子阵列，对于每个子阵列，将子阵列中的所有元素串联起来，检查形成的数字是否为回文数字。
