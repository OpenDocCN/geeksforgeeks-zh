...
> 原文：[https://www.geeksforgeeks.org/first-element-appears-even-number-times-array/](https://www.geeksforgeeks.org/first-element-appears-even-number-times-array/)
...
一个**简单的解决方案**就是把每一个元素都一一考虑。对于每个元素，开始计数频率，计数为偶数的第一个元素给出结果。该解决方案的时间复杂度为 `O(n^2)`。

一个**高效的解决方案**可以使用具有 `O(n)` 个时间和 `O(n)` 个额外空间的**哈希映射**来解决这个问题，如：

*   如果哈希映射中不存在 `arr[i]`，将其作为对插入哈希映射 `(arr[i], false)`。
*   如果哈希映射中存在 `arr[i]`，并且值为真，则将它切换为假。
*   如果散列图中有一个 `arr[i]`，并且值为 `false`，则将它切换为 `true`。
