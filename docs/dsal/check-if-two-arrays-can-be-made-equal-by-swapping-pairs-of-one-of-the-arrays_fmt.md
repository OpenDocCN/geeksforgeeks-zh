...
> 原文: [https://www.geeksforgeeks.org/check-if-two-arrays-can-be-made-equal-by-swapping-pairs-of-one-of-the-arrays/](https://www.geeksforgeeks.org/check-if-two-arrays-can-be-made-equal-by-swapping-pairs-of-one-of-the-arrays/)

给定两个大小相同的二进制[数组](https://www.geeksforgeeks.org/array-data-structure/) `arr1[]` 和 `arr2[]`，任务是仅当 `arr1[i] = 0` 和 `arr1[j] = 1` (`0 ≤ i < j < N`) 时，通过交换成对的 `arr1[]` 来使两个数组相等。如果可以使两个数组相等，打印 `"是"`。否则，打印 `"否"`。
...
*   初始化两个变量，说 `count` 和 `flag` (`= true`)。
*   [遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)并对每个数组元素执行以下操作:
    *   **如果 `arr1[i] != arr2[i]`:**
        *   如果 `arr1[i] == 0`，将 `count` 增加 `1`。
        *   否则，用 `1` 递减 `count`，如果 `count < 0`，则更新 `flag = false`。
*   如果 `flag` 等于 `true`，则打印 `"是"`。否则，打印 `"否"`。
