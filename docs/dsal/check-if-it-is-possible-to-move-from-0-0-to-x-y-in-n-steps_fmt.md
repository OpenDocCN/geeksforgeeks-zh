# 检查是否可以从(0,0)分N步移动到(x,y)

> 原文: [https://www.geeksforgeeks.org/check-if-it-is-possible-to-move-from-0-0-to-x-y-in-n-steps/](https://www.geeksforgeeks.org/check-if-it-is-possible-to-move-from-0-0-to-x-y-in-n-steps/)

给定点`(x, y)`。找出是否有可能从`(0, 0)`到`(x, y)`精确`n`步移动。4种类型的步骤是有效的，您可以从一个点`(a, b)`移动到`(a, b+1)`，`(a, b-1)`，`(a+1, b)`，`(a-1, b)`。
