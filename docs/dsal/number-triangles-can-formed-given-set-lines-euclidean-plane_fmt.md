...
> 原文：[https://www.geeksforgeeks.org/number-triangles-can-formed-given-set-lines-euclidean-plane/](https://www.geeksforgeeks.org/number-triangles-can-formed-given-set-lines-euclidean-plane/)

给定欧几里得平面上的一组 `L = {l1, l2, ……, ln}` 条不同的直线。第 `i` 行由公式 `a_i*x + b_i*y = c_i` 给出。找出用集合 `L` 中的线可以构成的三角形的数量。注意，没有两对线在同一点相交。
...
**时间复杂度：** 有 `nC3` 三胞胎的线路。对于每个三元组，我们必须进行 3 次比较，以检查任何 2 条线是否平行，这意味着检查可以在 `O(1)` 时间内完成。这使得朴素算法 `O(n^3)`。
...
对于大于 3 的子集数，如果我们有 `k` 个子集，我们的任务是找到一次取 3 的子集的元素数之和。这可以通过维护计数数组来实现。我们制作一个计数数组，其中 `count_i` 表示平行线第 `i` 个子集的计数。
