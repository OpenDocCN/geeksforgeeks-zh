...
> 原文：[https://www.geeksforgeeks.org/program-quotient-remainder-big-number/](https://www.geeksforgeeks.org/program-quotient-remainder-big-number/)

给定一串数字并给定另一个数字（比如 `m`）（`0 <= m <= 10^18`）。任务是计算给定数字的模。
...
我们也可以用这个技巧来求大数的商和余数。以下是上述方法的实现：
...
本文由`沙钦·毕斯特`供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用`contribute.geeksforgeeks.org`写一篇文章或者把你的文章邮寄到`contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
