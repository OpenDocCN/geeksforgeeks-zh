...
> 原文: [https://www.geeksforgeeks.org/find-subfactorial-of-a-number/](https://www.geeksforgeeks.org/find-subfactorial-of-a-number/)

给定一个整数 `N`，任务是找到表示为 `!N` 的数字的**子工厂**。数字 `N` 的子因子使用编号 `N` 的递归关系定义:
...
*   使用 `i` 迭代从 `1` 到 `N` 的范围，并执行以下操作:
    *   将 `fact` 更新为 `fact*i`.
    *   如果 `count` 为**偶数**，则将 `res` 更新为 `res = res–(1/fact)`.
    *   如果 `count` 为**奇数**，则将 `res` 更新为 `res = res + (1 /fact)`.
    *   将 `count` 值增加 `1`.
*   最后，返回 `fact*(1 + res)` .
