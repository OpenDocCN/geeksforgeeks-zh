...
> 原文：[https://www.geeksforgeeks.org/find-subarray-of-length-k-with-maximum-peak/](https://www.geeksforgeeks.org/find-subarray-of-length-k-with-maximum-peak/)

给定一个长度为 `n` 的数组 `arr[]` 和一个正整数 `K`，我们必须找到一个长度为 `K` 的子数组，其中包含最大峰值。
...
> `Peak = 2`
> **说明：**
> 有两个长度为 7 的子阵列，即 `[1, 7]` 和 `[2, 8]`。两个子阵列都有 2 个峰值，即 3 和 6 指数是两个子阵列的峰值。我们必须返回具有最小 `l` 和最大峰值的子阵列，即 `l = 1` 和 `峰值 = 2`。
> **输入：**
> `arr = {3, 2, 3, 2, 1}`，`k = 3`
> **输出：**
> `Left = 2`
