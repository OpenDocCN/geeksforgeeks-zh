...
> 原文: [https://www.geeksforgeeks.org/find-the-number-of-elements-x-such-that-x-k-also-exists-in-the-array/](https://www.geeksforgeeks.org/find-the-number-of-elements-x-such-that-x-k-also-exists-in-the-array/)

给定一个`数组 a[]`和一个`整数 k`，求该数组中元素 `x` 的个数，使得 `x` 和 `k` 的和也存在于该数组中。
