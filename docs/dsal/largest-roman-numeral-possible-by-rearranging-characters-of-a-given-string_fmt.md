...
> 原文: [https://www.geeksforgeeks.org/largest-roman-numeral-possible-by-rearranging-characters-of-a-given-string/](https://www.geeksforgeeks.org/largest-roman-numeral-possible-by-rearranging-characters-of-a-given-string/)

给定一个长度为 `N` 的[字符串](https://www.geeksforgeeks.org/string-data-structure/) `S`，任务是通过重新排列给定字符串的字符来打印可能的最高罗马数字，前提是给定字符串是有效的[罗马数字](https://en.wikipedia.org/wiki/Roman_numerals)。如果发现不可能，则打印 `"无效"`。
...
**思路:** 思路是使用[按第二元素排序](https://www.geeksforgeeks.org/sorting-vector-of-pairs-in-c-set-1-sort-by-first-and-second/)。按照以下步骤解决问题:

*   [迭代字符串的字符](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-c/)。
*   检查字符串是否包含除 `{ 'I', 'V', 'X', 'L', 'C', 'D', 'M' }` 以外的任何字符。如发现属实，则打印 `"无效"`。
