...
> 原文：[https://www.geeksforgeeks.org/nearest-smaller-character-to-a-character-k-from-a-sorted-array/](https://www.geeksforgeeks.org/nearest-smaller-character-to-a-character-k-from-a-sorted-array/)
...
- 如果 `arr[mid]` 比 `K` 小，则将其存储在某个变量中，比如 `ch`。将 `start` 设置为 `mid + 1` 以搜索更靠近 `K` 的较小字符。
- 否则，将 `end` 设置为 `mid - 1`。
