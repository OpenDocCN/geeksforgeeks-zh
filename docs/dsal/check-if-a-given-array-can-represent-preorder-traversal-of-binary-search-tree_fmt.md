...
> 原文: [https://www.geeksforgeeks.org/check-if-a-given-array-can-represent-preorder-traversal-of-binary-search-tree/](https://www.geeksforgeeks.org/check-if-a-given-array-can-represent-preorder-traversal-of-binary-search-tree/)

给定一个数字数组，如果给定数组可以表示二叉查找树的前序遍历，则返回 `true`，否则返回 `false`。预期时间复杂度为 `O(n)`。
...
一个高效解可以在 `O(n)` 时间内解决这个问题。想法是使用堆栈。这个问题类似于[下一个(或最接近的)大元素问题](https://www.geeksforgeeks.org/next-greater-element/)。这里我们找到下一个更大的元素，在找到下一个更大的元素之后，如果我们找到一个更小的元素，那么返回 `false`。
