...
> 原文：[https://www.geeksforgeeks.org/program-decimal-hexadecimal-conversion/](https://www.geeksforgeeks.org/program-decimal-hexadecimal-conversion/)

给定一个十进制数作为输入，我们需要编写一个程序，将给定的十进制数转换成等价的十六进制数。即将基值为 10 的数字转换为基值 16。

十六进制数字使用 16 个值来表示一个数字。从 0 开始的数字 0-9 用数字 0-9 表示，10-15 用从 A–F 开始的字符表示。
...
**第一步**：2545 除以 16 等于 1 时计算余数。因此，`temp = 1`。因为 `temp` 低于 10。所以，`arr[0] = 48 + 1 = 49 = '1'`。
**第二步**：2545 除以 16。新号码是 `2545/16 = 159`。
**第三步**：159 除以 16 等于 15 时计算余数。因此，`temp = 15`。因为 `temp` 大于 10。所以，`arr[1] = 55 + 15 = 70 = 'F'`。
**第四步**：159 除以 16。新数字是 `159/16 = 9`。
**第五步**：9 除以 16 等于 9 时计算余数。因此，`temp = 9`。因为 `temp` 低于 10。所以，`arr[2] = 48 + 9 = 57 = '9'`。
