...
> 原文: [`https://www.geeksforgeeks.org/previous-number-1s-complement/`](https://www.geeksforgeeks.org/previous-number-1s-complement/)
...
**时间复杂度:** `O(log n)`，因为数字的二进制表示正在被计算。
**辅助空间:** `O(1)`，虽然辅助空间是 `O(1)`，但仍有一些内存空间正在得到用来存储数字的二进制表示。
