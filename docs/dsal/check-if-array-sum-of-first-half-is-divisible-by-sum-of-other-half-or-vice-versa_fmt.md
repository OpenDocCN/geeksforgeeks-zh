...
> 原文: [https://www.geeksforgeeks.org/check-if-array-sum-of-first-half-is-divisible-by-sum-of-other-half-or-vice-versa/](https://www.geeksforgeeks.org/check-if-array-sum-of-first-half-is-divisible-by-sum-of-other-half-or-vice-versa/)

给定一个大小为`N`的数组`arr[]`，任务是检查左子数组的和是否能被右子数组的和整除，反之亦然。如果是，打印`是`，否则打印`否`。这里，左子数组将包含从`0`到`mid=(N-1)/2`的元素，右子数组将包含从`mid+1`到`N-1`的元素。
...
1.  创建两个变量`sumL`和`sumR`分别存储左子数组和右子数组的总和。用`0`初始化它们。
2.  现在，运行一个从`0`到`N-1`的循环，对于`0`到`(N-1)/2`范围内的迭代，将元素添加到`sumL`中。对于`(N-1)/2`之后的迭代，将元素添加到`sumR`中。
