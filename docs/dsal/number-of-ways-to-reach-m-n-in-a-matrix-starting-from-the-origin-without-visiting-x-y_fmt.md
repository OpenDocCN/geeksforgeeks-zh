...
> 原文: [https://www.geeksforgeeks.org/number-of-ways-to-reach-m-n-in-a-matrix-starting-from-the-origin-without-visiting-x-y/](https://www.geeksforgeeks.org/number-of-ways-to-reach-m-n-in-a-matrix-starting-from-the-origin-without-visiting-x-y/)
...
> `(0, 0) -> (0, 1) -> (0, 2) -> (1, 2) -> (2, 2)`
> `(0, 0) -> (0, 1) -> (1, 1) -> (2, 1) -> (2, 2)` (此路径访问了 `(1, 1)`，因此无效)
> **输入:** `M = 5`，`N = 4`，`X = 3`，`Y = 2`
> **输出:** `66`
> **说明:**
...
**方法:**
解决上述问题的思路是从 `(0, 0)` 到 `(M, N)` 的总路径数中减去那些经过了 `(X, Y)` 的路径数。因此，

1.  从 `(0, 0)` 到达 `(M, N)` 的总途径数由下式给出：
    ![\text{Total ways from (0, 0) to (M, N)} = \binom{M + N}{M}](img/709d0476b0a5eec6b3a020ea348dafba.png "Rendered by QuickLaTeX.com")

2.  仅通过访问 `(X, Y)` 到达 `(M, N)` 的途径数是从 `(0, 0)` 到达 `(X, Y)`，然后从 `(X, Y)` 到达 `(M, N)` 的途径数的乘积，由下式给出：
