...
> 原文: [https://www.geeksforgeeks.org/validation-of-equation-given-as-string/](https://www.geeksforgeeks.org/validation-of-equation-given-as-string/)
...
*   遍历字符串，将所有操作数存储在一个数组 `操作数[]` 中，所有操作符存储在一个数组 `操作符[]` 中。
*   现在对 `操作数[0]` 和 `操作数[1]` 执行存储在 `运算符[0]` 中的算术运算，并将其存储在 `和` 中。
*   然后在 `和` 和 `运算符[2]` 上执行第二次算术运算，即 `运算符[1]`，等等。
