...
> 原文: [https://www.geeksforgeeks.org/queries-to-answer-the-x-th-smallest-sub-string-lexicographically/](https://www.geeksforgeeks.org/queries-to-answer-the-x-th-smallest-sub-string-lexicographically/)

## 问题描述

给定一个字符串 `str` 和 `Q` 个查询。每个查询由一个数字 `X` 组成，任务是打印给定字符串 `str` 的第 `X` 个字典最小子字符串。
...
> k
> ```
> “e”, “e”, “ee”, “eek”, “ek”, “g”, “ge”, “gee”, “geek”, “geek”, “geek” 和 “k” 是所有可能的子串，按字典顺序排序。
...
生成所有的子字符串，并将它们存储在任何数据结构中，并按照字典顺序对该数据结构进行排序。在下面的解决方案中，我们使用了一个 `vector` 来存储所有的子字符串，并且内置的 `sort()` 函数按照给定的顺序对它们进行排序。现在，对于每个查询打印 `vec[X–1]`，这将是第 `X` 个最小的子串。
