...
> 原文: [https://www.geeksforgeeks.org/closest-palindrome-number-whose-absolute-difference-min/](https://www.geeksforgeeks.org/closest-palindrome-number-whose-absolute-difference-min/)
...
**例 2 a :** 一种可能的获取最近回文的方法是复制前半部分，如果是，在末尾添加镜像。**左半部分:** 例如 `123456` 的左侧为 `123`，而 `12345` 的左半部分为 `12`。要转换成回文，我们可以取其左半部分的镜像，也可以取其右半部分的镜像。然而，如果我们取右半部分的镜像，那么这样形成的回文不一定是最接近的回文。所以，我们必须把左边的镜子复制到右边。
