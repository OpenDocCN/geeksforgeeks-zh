...
> 原文: [https://www.geeksforgeeks.org/queries-for-bitwise-or-in-the-index-range-l-r-of-the-given-array/](https://www.geeksforgeeks.org/queries-for-bitwise-or-in-the-index-range-l-r-of-the-given-array/)
...
遍历该范围，找到该范围内所有数字的逐位或。对于每个查询，这将花费`O(n)`个时间。

## 高效方法

如果我们将整数视为二进制数，我们可以很容易地看到，我们要设置的答案的第`i-th`位的条件是，应该设置范围`[L, R]`内任何整数的第`i-th`位。

因此，我们将计算每个位的前缀计数。我们将使用它来查找设置了`i-th`位的范围内的整数数量。如果其大于`0`，那么我们的答案的`i-th`位也将被设置。
