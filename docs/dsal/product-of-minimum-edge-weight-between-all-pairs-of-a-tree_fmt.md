...
给定一棵有 `N` 个顶点和 `N-1` 条边的树。让我们定义一个函数 `F(a, b)`，它等于节点 `a` 和 `b` 之间路径中的最小边权重。任务是计算所有这样的 `F(a, b)` 的乘积。这里 `(a, b)` 是无序对且 `a != b`。
所以，基本上，我们需要找到:
$$ \prod_{0 \le i < j \le n-1} F(i, j) $$
的值。

在输入中，我们将被赋予 `N` 的值，然后 `N-1` 行跟随。每一行包含 3 个整数 `u`，`v`，`w`，表示节点 `u` 和 `v` 之间的边以及它的权重 `w`。因为乘积会很大，所以以 `10^9+7` 取模输出它。
...
`F(1, 3) = 3`         `F(2, 4) = 1`
`F(1, 4) = 2`         `F(3, 4) = 2`
所有 `F(i, j)` 的乘积 = `1*3*2*1*1*2 = 12` mod `(10^9 +7) = 12`
...
如果我们仔细观察，那么我们将会看到，如果有一组节点的最小边权重为 `W`，并且如果我们在这个集合中添加一个节点，该节点通过权重为 `W` 的边与整个集合相连，这样 `W < w`，那么，这里最近添加的节点到集合中所有现有节点之间形成的路径的最小权重将是 `W`。

那么，这里我们可以应用[不相交集合并](https://www.geeksforgeeks.org/disjoint-set-data-structures/)的概念来解决这个问题。
首先，按照权重递减对边进行排序。最初将所有节点指定为一个集合。现在，当我们合并两个集合时，请执行以下操作:
```
