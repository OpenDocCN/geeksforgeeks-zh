...
> 原文: [https://www.geeksforgeeks.org/javascript-math-log10e-property/](https://www.geeksforgeeks.org/javascript-math-log10e-property/)

以下是 `Math.LOG10E` 属性的示例。
...
`Math.LOG10E` 是 JavaScript 中的一个属性，简单来说就是用来求 `e` 的以 10 为底的对数的值，其中 `e` 是一个约等于 2.718 的无理数和超越数。
...
Here, the logarithmic value of `e` with the base of 10 is simply displayed as the output.
...
`e` 的以 10 为底的对数的值可以如下所示的函数形式打印出来。
...
这里我们考虑 `Math.LOG10E` 作为一个函数，但实际上它是一个属性。
...
每当我们需要求 `e` 的基数 10 的对数时，我们就借助这个性质。
