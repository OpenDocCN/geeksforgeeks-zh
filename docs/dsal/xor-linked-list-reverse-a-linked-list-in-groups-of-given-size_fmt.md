...
> 原文：[https://www.geeksforgeeks.org/xor-linked-list-reverse-a-linked-list-in-groups-of-given-size/](https://www.geeksforgeeks.org/xor-linked-list-reverse-a-linked-list-in-groups-of-given-size/)

给定一个[异或链表](https://www.geeksforgeeks.org/xor-linked-list-a-memory-efficient-doubly-linked-list-set-1/)和一个整数 `K`，任务是反转给定异或链表中的每个 `K` 节点。
...
思想是递归反转一组中[异或链表](https://www.geeksforgeeks.org/xor-linked-list-a-memory-efficient-doubly-linked-list-set-2/)的每个 `K` 节点，并将每组 `K` 节点的第一个节点连接到其前一组节点的最后一个节点。递归函数如下：
...
*   反转异或链表的第一个 `K` 节点，递归反转一组大小 `K` 中的剩余节点。如果剩余节点数小于 `K`，则只需反转剩余节点即可。
