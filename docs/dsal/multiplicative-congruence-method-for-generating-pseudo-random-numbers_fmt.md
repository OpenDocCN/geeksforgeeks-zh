...
> 原文：[https://www.geeksforgeeks.org/multiplicative-congruence-method-for-generating-pseudo-random-numbers/](https://www.geeksforgeeks.org/multiplicative-congruence-method-for-generating-pseudo-random-numbers/)
...
> `X0`【0，m)，序列初始值–称为**种子**
>
> 应适当选择 `m`、`a` 和 `X0`，以获得几乎等于 `m` 的周期。

## 进场

*   选择种子值（`X0`）、模量参数（`m`）和乘数项（`a`）。
*   初始化需要生成的随机数数量（比如，一个整数变量 `nofrandomnums`）。
*   定义存储以保持生成的随机数（这里考虑的是向量的大小 `无随机数`）。
*   用种子值初始化向量的第 0 个索引。
