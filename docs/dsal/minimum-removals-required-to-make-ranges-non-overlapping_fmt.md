...
> 原文: [https://www.geeksforgeeks.org/minimum-removals-required-to-make-ranges-non-overlapping/](https://www.geeksforgeeks.org/minimum-removals-required-to-make-ranges-non-overlapping/)

给定一个带有起始值和结束值的范围列表，任务是找到需要移除的最小范围数，以使剩余范围不重叠。

## 示例:

> **输入:** `input = {{1, 2}, {4, 7}, {3, 8}}`
> **输出:** 1
> **解释:** 移除`{3, 8}`使`{{1, 2}, {4, 7}}`不重叠。
>
> **输入:** `input = {{10, 20}, {10, 20}, {10, 20}}`
> **输出:** 2
> **说明:** 去掉`[10, 20]`使剩余范围不重叠。
>
> **输入:** `input = {{1, 2}, {5, 10}, {18, 35}, {40, 45}}`
> **输出:** 0
> **说明:** 所有范围已经不重叠。
...
*   按起始值对范围进行排序。
*   遍历范围，检查是否有任何范围的起点小于上一个范围的终点（即存在重叠）。
*   移除结束点更大的范围。
