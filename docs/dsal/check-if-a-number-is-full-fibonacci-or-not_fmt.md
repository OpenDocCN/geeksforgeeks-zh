...
> 原文: [https://www.geeksforgeeks.org/check-if-a-number-is-full-fibonacci-or-not/](https://www.geeksforgeeks.org/check-if-a-number-is-full-fibonacci-or-not/)
...
首先检查 `N` 的所有数字是否都是[斐波那契](https://www.geeksforgeeks.org/check-number-fibonacci-number/)。如果是，同样地，如果且仅当 `(5*N^2 + 4)` 或 `(5 * N^2 - 4)` 中的一个或两个是一个完美的正方形，则根据一个数是斐波那契数的原理检查 `N` 是否是斐波那契数。
