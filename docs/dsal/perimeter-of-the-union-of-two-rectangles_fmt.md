...
> 原文：[https://www.geeksforgeeks.org/perimeter-of-the-union-of-two-rectangles/](https://www.geeksforgeeks.org/perimeter-of-the-union-of-two-rectangles/)

给定两个[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `X[]` 和 `Y[]`，每个长度为 **4**，其中 `(X[0], Y[0])` 和 `(X[1], Y[1])` 代表一个矩形的左下角和右上角，`(X[2], Y[2])` 和 `(X[3], Y[3])` 代表另一个矩形的左下角和右上角。
...
*   否则，分别打印 `X` 和 `Y` 坐标最大差值之和的两倍，即 `2 * (max(X[]) – min(X[]) + max(Y[]) – min(Y[]))`。
