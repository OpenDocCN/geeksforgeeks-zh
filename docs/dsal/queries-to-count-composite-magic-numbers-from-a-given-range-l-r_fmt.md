...
> 原文: [https://www.geeksforgeeks.org/queries-to-count-composite-magic-numbers-from-a-given-range-l-r/](https://www.geeksforgeeks.org/queries-to-count-composite-magic-numbers-from-a-given-range-l-r/)

给定两个大小为 `Q` 的[数组](https://www.geeksforgeeks.org/array-data-structure/) `L[]` 和 `R[]`，任务是从范围`[L[I], R[I]]` (0≤I<Q)中找出复合幻数，即既是[复合数](https://www.geeksforgeeks.org/composite-number/)又是[幻数](https://www.geeksforgeeks.org/check-number-magic-recursive-sum-digits-1/)的数。

**示例:**

> **输入:** `Q = 1`, `L[] = {10}`, `R[] = {100}`
> **输出:** 8
> **说明:** 范围`[L[0], R[0]]`内的数字中，既是复合的也是幻数的是{10, 28, 46, 55, 64, 82, 91, 100}。
>
...
**高效方法:** 为了优化上述方法，预先计算并存储一定范围内数组中复合幻数的计数。这优化了对每个查询的计算复杂性为 `O(1)`。按照以下步骤解决问题:

1.  初始化一个整数[数组](https://www.geeksforgeeks.org/array-data-structure/) `dp[]`，这样 `dp[i]` 存储到 `i` 的**复合幻数**的计数。
