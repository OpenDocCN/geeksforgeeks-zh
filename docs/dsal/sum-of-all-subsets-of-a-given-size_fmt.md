...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-subsets-of-a-given-size/](https://www.geeksforgeeks.org/sum-of-all-subsets-of-a-given-size/)

给定一个由 `N` 个整数和一个正整数 `K` 组成的数组 `arr[]`，任务是找出大小为 `K` 的所有子集的和。
...
> **输出:** 36
> **解释:**
> 大小为 `K(= 2)` 的子集为 `{1, 2}`、`{1, 4}`、`{1, 5}`、`{2, 4}`、`{2, 5}`、`{4, 5}`。所有子集总和总和 = 3 + 5 + 6 + 6 + 7 + 9 = 36。
>
> **输入:** `arr[] = {2, 4, 5, 6, 8}`，`K = 3`
> **输出:** 150

**天真方法:** 解决给定问题的最简单方法是生成给定数组的所有可能子集，并找到那些大小为 `K` 的子集的元素之和。找到所有 `K` 个大小子集的总和后，打印得到的所有总和的总和作为结果。
...
*   初始化变量，将 `sum` 设为 `0` 来存储所有数组元素的和。
*   执行上述步骤后，打印 `sum * freq` 的值作为结果总和。
...
```
Sum of all subsets of size =  2  is => 36
```
