...
> 原文: [https://www.geeksforgeeks.org/solve-the-logical-expression-given-by-string/](https://www.geeksforgeeks.org/solve-the-logical-expression-given-by-string/)

给定字符串 `str` 表示由运算符 `|`（或）、`&`（与）、`!`（非）、`0`、`1` 和 `[`、`]` 组成的逻辑表达式（即字符间无空格）。任务是打印逻辑表达式的结果。
