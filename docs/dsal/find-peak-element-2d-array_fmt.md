...
3.  如果`最大值` >= `A[索引][中间 1]` & `最大值` >= `A[索引][选择+1]`，则`最大值`为峰值，返回`最大值`。
...
## 时间复杂度
O(行*日志(列))。我们重复了一半的列数。在每次递归调用中，我们线性搜索当前中间列中的最大值。
...
本文由 **Rohit Thapliyal** 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用`write.geeksforgeeks.org`写一篇文章或者把你的文章邮寄到`contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
