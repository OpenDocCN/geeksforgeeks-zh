...
> 原文：[https://www.geeksforgeeks.org/pick-maximum-sum-m-elements-such-that-contiguous-repetitions-do-not-exceed-k/](https://www.geeksforgeeks.org/pick-maximum-sum-m-elements-such-that-contiguous-repetitions-do-not-exceed-k/)
...
**方法：** 如果我们想要最大和，我们必须从数组中获取最大值，但是我们最多可以重复这个最大值`K`次，所以我们必须用第二个最大值将其分开一次，然后我们再次获取第一个最大值，直到`K`次，这个循环一直持续到我们获取总的`M`值。
