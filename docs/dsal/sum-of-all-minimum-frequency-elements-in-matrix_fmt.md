...
> 原文: [https://www.geeksforgeeks.org/sum-of-all-minimum-frequency-elements-in-matrix/](https://www.geeksforgeeks.org/sum-of-all-minimum-frequency-elements-in-matrix/)
...
*   遍历矩阵，使用 `C++` 中的 `map` 来存储矩阵元素的频率，使得 `map` 的关键是矩阵元素，值是它在矩阵中的频率。
*   然后遍历 `map` 找到最小频率。
*   最后，遍历 `map`，找出元素的频率，并检查它是否与上一步中获得的最小频率相匹配，如果匹配，则将该元素的频率乘以总和。
