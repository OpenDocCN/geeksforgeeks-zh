...
> 原文: [https://www.geeksforgeeks.org/check-if-the-given-chessboard-is-valid-or-not/](https://www.geeksforgeeks.org/check-if-the-given-chessboard-is-valid-or-not/)
...
请注意，在棋盘上，每一对相邻的格子都涂上了不同的颜色。因此，对于每个单元格 `(i, j)`，检查相邻单元格即 `(i + 1, j)`，`(i - 1, j)`，`(i, j + 1)`，`(i, j - 1)` 是否涂有与 `(i, j)` 不同的颜色。
...
以下展示了使用不同编程语言实现检查二维数组是否为棋盘格模式的代码。
