...
我们可以对 `i` 运行从 `1` 到 `n` 的循环，如果 `n` 可被 `i` 整除，则乘以数字。该解决方案的时间复杂度为 `O(n)`。但是这种方法对于大的 `n` 值是不够的。

### 方法 2 (更好的方法)

更好的方法是运行 `i` 从 `1` 到 `sqrt(n)` 的循环。如果数能被 `i` 整除，用 `i` 和 `n/i` 相乘，这个解的时间复杂度是 `O(sqrt(n))`。
...
所以我们可以观察到因子的乘积将是 `n^(number of factors/2)`。但是当因子数是奇数时(这意味着该数是完美平方)，在这种情况下，乘积将是 `n^(number of factors/2) * sqrt(n)`。我们可以使用类似上述方法计算因子数量。我们可以使用[模幂运算](https://www.geeksforgeeks.org/modular-exponentiation-power-in-modular-arithmetic/)高效地计算幂。
...
本文由核素投稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用`write.geeksforgeeks.org`写一篇文章或者把你的文章邮寄到`contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
