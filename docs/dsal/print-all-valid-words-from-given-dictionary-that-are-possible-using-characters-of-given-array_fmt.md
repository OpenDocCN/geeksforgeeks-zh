...
> 原文：[https://www.geeksforgeeks.org/print-all-valid-words-from-given-dictionary-that-are-possible-using-characters-of-given-array/](https://www.geeksforgeeks.org/print-all-valid-words-from-given-dictionary-that-are-possible-using-characters-of-given-array/)

给定一个字符串字典 `dict[]` 和一个字符数组 `arr[]`。使用给定字符数组中的字符打印字典中所有可能的有效单词。

**例：**

> **输入：** `dict[] = {"go", "bat", "me", "eat", "goal", "boy", "run"}`，`arr[] = {'e', 'o', 'b', 'a', 'm', 'g', 'l'}`
> **输出：** `"go"`, `"me"`, `"goal"`。
> **说明：** 字典中只有这三个字符串的所有字符。
>
> **输入：** `Dict[] = {"goa", "bait", "wolves", "ate", "goal", "girl", "rise"}`，`arr[] = {'s', 'o', 't', 'a', 'e', 'g', 'l', 'i', 'r'}`
> **输出：** `"goa"`, `"ate"`, `"goal"`, `"girl"`
...
> **图解：** 考虑下面的例子。
>
> `dict[] = ["go", "bat", "eat", "me", "goal", "boy", "run"]`、
> `arr[] = ['e', 'o', 'b', 'a', 'm', 'g', 'l']`
>
> *   串联字符串 = `eobamgl`
> *   如果我们遍历 `"go"`
