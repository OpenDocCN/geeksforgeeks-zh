...
这个问题可以使用标准的最小生成树算法来解决，比如 [krushkal](https://www.geeksforgeeks.org/greedy-algorithms-set-2-kruskals-minimum-spanning-tree-mst/) 和 [prim](https://www.geeksforgeeks.org/greedy-algorithms-set-5-prims-minimum-spanning-tree-mst-2/) 的算法，但是我们需要修改我们的图来使用这些算法。最小生成树算法试图最小化权重的总和，这里我们需要最小化权重的乘积总和。我们可以利用[对数](https://en.wikipedia.org/wiki/Logarithm)的性质来克服这个问题。众所周知，
...
我们可以用图的对数值来代替图的每个权重，然后我们应用任何最小生成树算法，该算法将尝试最小化对数和(`wi`), 进而最小化权重乘积。例如图表，步骤如下图所示，
