...
> 原文: [https://www.geeksforgeeks.org/sum-of-the-updated-array-after-performing-the-given-operation/](https://www.geeksforgeeks.org/sum-of-the-updated-array-after-performing-the-given-operation/)
...
**有效方法:** 一种有效的解决方案是从末端遍历数组，使得到目前为止子数组的和（即 `sum(arr[i+1…n-1])`）可以用来计算当前子数组的和 `arr[i…n-1]`，即 `sum(arr[i…n-1]) = arr[i] + sum(arr[i+1…n-1])`。
