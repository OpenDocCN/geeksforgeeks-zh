...
左视图包含所有节点，这些节点是其级别中的第一个节点。简单的解决方法就是`级序遍历`并打印每一级中的第一个节点。
...
使用队列和空指针来标记每个级别的第一个元素。我们在第一个元素中插入一个空指针，当到达该空指针时，我们将 `bool` 标记为 `true`，并将下一个元素作为左视图元素。
