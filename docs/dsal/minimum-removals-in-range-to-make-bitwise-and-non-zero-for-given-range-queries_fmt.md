...
> 原文：[https://www.geeksforgeeks.org/minimum-removals-in-range-to-make-bitwise-and-non-zero-for-given-range-queries/](https://www.geeksforgeeks.org/minimum-removals-in-range-to-make-bitwise-and-non-zero-for-given-range-queries/)

给定一个`Q`范围查询的数组`queries[][]`，任务是从范围`[l, r]`中找到最小删除量，使得范围的位与为非零值。

**示例：**

> **输入：** `queries[][] = { {1, 5}, {3, 4}, {5, 10}, {10, 15} }`
> **输出：** `2 1 3 0`
> **解释：**
> 查询-1：`l = 1`，`r = 5`，集合为`{1, 2, 3, 4, 5}`。`(2, 4)`应该被移除，以使数组的AND非零，因此最小移除量为`2`。
> 查询-2：`l = 3`，`r = 4`，集合为`{3, 4}`。应该移除`3`或`4`，以使数组的“与”非零，因此最小移除量为`1`。
> 查询-3：`l = 5`，`r = 10`，集合为`{5, 6, 7, 8, 9, 10}`。`(5, 6, 7)`或`(8, 9, 10)`应被删除，以使范围的“与”不为零。最小移除量为`3`。
> 查询-4：`l = 10`，`r = 15`，集合为`{10, 11, 12, 13, 14, 15}`。数组的AND最初为非零，因此`0`个删除。
>
> **输入：** `queries[][] = { {100, 115}, {30, 40}, {101, 190} }`
> **输出：** `0 2 27`

**简单方法：** 这可以通过遍历每个范围并检查具有公共设置位的范围内元素的最大计数，并从元素的总计数中删除该计数，即`(r - l + 1)`。
...
**高效方法：** 该方法基于动态规划和前缀求和技术。这可以通过在31位的每个位置使用`dp[]`数组存储总设置位的计数直到该范围来实现。这里`dp[i][j]`的状态是指从`[1, i]`的`j`th位位置的总设定位。按照以下步骤解决上述问题：

*   使用前缀和调用`count_set_bits()`函数预先计算`dp[][]`数组。
*   现在遍历查询并分配`l = q[i][0]`，`r = q[i][1]`。
    *   初始化`max_set = INT_MIN`以存储设置了第`j`位的范围内元素的最大计数。
    *   迭代`[0, 30]`的位。
    *   计算由`total_set = (dp[r][j] - dp[l - 1][j])`设置的第`j`位的元素数量。
    *   通过取`max(max_set, total_set)`，存储第`j`位设置的元素的最大计数。
