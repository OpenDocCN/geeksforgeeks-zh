...
使用三个嵌套循环。 检查第一行的元素是否存在于所有后续行中。 `` `O(n ^ 3)` ``的时间复杂度。 可能需要额外的空间来处理重复的元素。

## 方法 2

按升序分别对矩阵的所有行进行排序。 然后对[在 3 个排序的数组中查找公共元素](https://www.geeksforgeeks.org/find-common-elements-three-sorted-arrays/)的问题应用改进的方法。 下面给出了相同的实现。
...
`O(n ^ 2 log n)`，大小为`n`的每一行都需要`O(n logn)`进行排序，总共有`n`行。
