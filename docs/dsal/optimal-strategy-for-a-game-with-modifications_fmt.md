...
> 原文: [https://www.geeksforgeeks.org/optimal-strategy-for-a-game-with-modifications/](https://www.geeksforgeeks.org/optimal-strategy-for-a-game-with-modifications/)

## 问题陈述

考虑一排价值 `v1` 到 `vn` 的 `n` 枚硬币，其中 `n` 是偶数。我们轮流和对手比赛。在每一回合中，玩家执行以下操作 `K` 次：玩家从该行中选择第一枚或最后一枚硬币，将其从该行中永久移除，并获得硬币的价值。确定如果用户先出手，用户肯定能赢的最大可能金额。

**注:** 对手和用户一样聪明。

**举例:**

> **输入:** `array = {10, 15, 20, 9, 2, 5}`, `k = 2`
> **输出:** `32`
> **解释:**
> 假设用户最初选择了 `10` 和 `15`。用户拥有的硬币值为 `25`，`{20, 9, 2, 5}` 剩余在数组中。
> 第二轮，对手挑 `20` 和 `9`，使其值 `29`。
> 第三轮，用户选择 `2` 和 `5`，使他的总值为 `32`。

> **输入:** `array = {10, 15, 20, 9, 2}`, `k = 1`
> **输出:** `32`
...
以 `arr = {10, 15, 20, 9, 2, 5}`, `k = 2` 为例得出递归解：
...
所选元素之和 **`Sc = (sum(l, r) – sum(l+i, l+i+n-k-1))`**。
现在对手将执行下一回合，因此下一步所选元素之和等于当前数组从 `l` 到 `r` 的总和减去下一步对手所选元素之和，即：
