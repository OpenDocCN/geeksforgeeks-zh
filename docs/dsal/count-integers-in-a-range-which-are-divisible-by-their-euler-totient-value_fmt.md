...
> 原文: [https://www.geeksforgeeks.org/count-integers-in-a-range-which-are-divisible-by-their-euler-totient-value/](https://www.geeksforgeeks.org/count-integers-in-a-range-which-are-divisible-by-their-euler-totient-value/)
...
**= 0**。这是因为对于素数 `p1 = 2` 和 `p2 = 3`，`p1–1 = 1` 和 `p2–1 = 2`。因此，只有表格 `2^p 3^q` 中 `p >= 1` 和 `q >= 0` 的数字需要在范围 `[L, R]` 内进行计数。
