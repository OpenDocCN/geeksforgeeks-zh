...
> 原文: [https://www.geeksforgeeks.org/count-numbers-in-a-range-that-are-divisible-by-all-array-elements/](https://www.geeksforgeeks.org/count-numbers-in-a-range-that-are-divisible-by-all-array-elements/)
...
一种简单的方法是从 `L` 迭代到 `R`，并计算可被所有数组元素整除的数字。
**时间复杂度:** `O((R-L) * N)`

一种高效的方法是找到 `N` 个数的 [LCM](https://www.geeksforgeeks.org/lcm-of-given-array-elements/)，然后统计在 `[L，R]` 范围内可被 `LCM` 整除的个数。直到 `R` 都可以被 `LCM` 整除的数是 `R/LCM`。所以利用排除原理，计数将是 `(R/LCM)–((L–1)/LCM)`。
