...
> 原文: [https://www.geeksforgeeks.org/construct-distinct-elements-array-given-size-sum-element-upper-bound/](https://www.geeksforgeeks.org/construct-distinct-elements-array-given-size-sum-element-upper-bound/)
...
2.  找到数组中最大的元素，但是由于数组是排序的，所以 `array[N]` 将是最大的。
    *   如果最大元素小于 `K`，我们用 `K` 替换它，并检查数组的新和。
        *   如果它小于给定的 `SUM`，我们移动到数组中的 `N-1` 位置，因为 `array[N]` 不能进一步递增，为了保持唯一性，我们将 `K` 减少 `1`。
        *   如果它大于给定的和，我们替换一个元素，这样和将被给定和，并且将脱离循环。
    *   如果最大的元素等于 `K`，我们移动到数组中的 `N-1` 位置，因为 `array[N]` 不能进一步递增，为了保持唯一性，我们将 `K` 减少 `1`。
3.  打印数组。
