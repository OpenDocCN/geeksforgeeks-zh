...
> 原文：[https://www.geeksforgeeks.org/smallest-odd-number-with-even-sum-of-digits-from-the-given-number-n/](https://www.geeksforgeeks.org/smallest-odd-number-with-even-sum-of-digits-from-the-given-number-n/)

以字符串的形式给出一个大数。任务是通过从给定字符串 `str` 中删除零个或多个字符来找到数字总和为偶数的最小奇数，在这里可以重新排列数字。
