...
> 原文: [https://www.geeksforgeeks.org/smallest-subarray-which-upon-repetition-gives-the-original-array/](https://www.geeksforgeeks.org/smallest-subarray-which-upon-repetition-gives-the-original-array/)

给定一个由 `N` 个整数组成的[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `arr[]`，任务是找到大小至少为 2 的最小子数组 `brr[]`，从而通过对数组 `brr[]` 执行重复运算，得到原始数组 `arr[]`。如果找不到这样的子数组，打印 `-1`。
...
> **输出:** `{1, 2, 3, 3}`
> **解释:**
> `{1, 2, 3, 3}` 是最小的子数组，重复 2 次就得到原始数组 `{1, 2, 3, 3, 1, 2, 3, 3}`。
>
> **输入:** `arr[] = {1, 1, 6, 1, 1, 7}`
> **输出:** `-1`
> **说明:**
...
想法是[生成所有可能的长度至少为 2 的子数组](https://www.geeksforgeeks.org/generating-subarrays-using-recursion/)，并检查重复这些子数组是否给出原始数组。
...
上述方法可以通过观察结果子数组 `brr[]` 必须从原始数组的第一个索引开始，以在重复时生成 `arr[]` 来优化。因此，只生成那些从第一个索引开始且长度至少为 2 的子数组，并检查重复这些子数组是否给出原始数组。以下是步骤:

1.  创建一个辅助数组 `brr[]`，并将原始数组的前两个元素插入其中，因为结果数组的大小必须至少为两个。
2.  遍历子数组长度 `[2, N/2+1]` 的可能值，并检查重复时长度为 `i` 的数组 `brr[]` 是否给出原始数组 `arr[]`。
3.  如果是，那么打印这个子数组，并打破循环。
