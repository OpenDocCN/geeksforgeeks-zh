# 通过删除0个或更多字符将一个字符串转换为另一个字符串的方式

> 原文：[https://www.geeksforgeeks.org/ways-transforming-one-string-removing-0-characters/](https://www.geeksforgeeks.org/ways-transforming-one-string-removing-0-characters/)
...
解决这个问题的想法是使用动态规划。构造一个 m*n 大小的 2D DP 矩阵，其中 m 是字符串 B 的大小，n 是字符串 A 的大小。
...
*   **情况 2**：当 `i > 0` 时，`dp[i][j]` 可由两种情况导出：
    *   **情况 2.a**：如果 `B[i] != A[j]`，那么解决方法就是忽略字符 `A[j]` 并对齐子串 `B[0..i]` 用 `A[0..(j-1)]`。因此，`dp[i][j] = dp[i][j-1]`。
