...
给定一个由 `N` 行和 `M` 列组成的布尔矩阵 `mat[][]`，最初用 `0`（空单元格）填充，一个整数 `K` 并查询 `{X, Y}` 类型的 `Q[][]`，任务是替换 `mat[X][Y] = 1`（非空单元格），并计算给定矩阵中连接的非空单元格的数量。
...
> 查询 2：`mat[][] = {{1, 0, 0}, {1, 1, 0}, {0, 0, 0}}`，`Count = 2`
> 查询 3：`mat[][] = {{1, 0, 0}, {1, 1, 0}, {0, 0, 0}}`，`Count = 1`
> 查询 4：`mat[][] = {{1, 0, 0}, {1, 1, 1}, {0, 0, 0}}`，`Count = 1`
...
*   因为最初矩阵中没有 `1`，所以 `count = 0`。
*   通过执行线性映射 `index = X * M + Y`，将二维问题转化为经典的[联合查找](https://www.geeksforgeeks.org/union-find-algorithm-set-2-union-by-rank/)，其中 `M` 为列长。
*   在每个查询中设置一个 `index` 后，增加 `count`。
*   如果 4 个相邻单元格中有一个非空单元格：
    *   对当前 `index` 和 `相邻单元格` 执行并集操作（连接两套）。
    *   递减 `count` 为两台机组连接。
