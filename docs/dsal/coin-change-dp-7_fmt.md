...
基本上，如果我们在 `s[m-1]`，我们可以取该硬币(无界包含)的尽可能多的实例，即`count(S，m，n–S[m-1])`；然后我们转到 `s[m-2]`。移动到 `s[m-2]`后，我们不能后退，不能为 `s[m-1]`做选择，即`count(S，m-1，n )`。

最后，由于我们要找到路的总数，所以我们将这 `2` 个可能的选择相加，即`count(S，m，n–S[m-1])+count(S，m-1，n)；`这将是我们需要的答案。
...
由于相同的子问题被再次调用，这个问题具有重叠子问题的性质。所以硬币兑换问题同时具有动态规划问题的两个性质(见[这个](https://www.geeksforgeeks.org/archives/12635)和[这个](https://www.geeksforgeeks.org/archives/12819))。像其他典型的[动态规划(DP)问题](https://www.geeksforgeeks.org/archives/tag/dynamic-programming)一样，相同子问题的重新计算可以通过以自下而上的方式构建临时数组 `table[][]` 来避免。
