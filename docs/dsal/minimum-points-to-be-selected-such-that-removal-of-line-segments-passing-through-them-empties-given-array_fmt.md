...
> 原文:[https://www.geeksforgeeks.org/minimum-points-to-be-selected-such-that-removal-of-line-segments-passing-through-them-empties-given-array/](https://www.geeksforgeeks.org/minimum-points-to-be-selected-such-that-removal-of-line-segments-passing-through-them-empties-given-array/)
...
*   初始化一个变量，说`` `point = arr[0][1]` ``来存储`` `X` ``轴的点。
*   [遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)，检查`` `arr[i][0]` ``的值是否大于`` `point` ``。如果发现为真，则将值`` `cntSteps` ``增加`` `1` ``并更新`` `point = arr[i][1]` ``的值。
*   最后，打印`` `cntSteps` ``的值。
