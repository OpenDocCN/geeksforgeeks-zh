...
> 原文: [https://www.geeksforgeeks.org/find-max-length-odd-parity-substring/](https://www.geeksforgeeks.org/find-max-length-odd-parity-substring/)

给定一个二进制字符串 `str`，任务是找到具有奇偶性的 `str` 的子字符串的最大长度。如果一个二进制字符串包含奇数个 `1`，则称其为奇数奇偶校验。
...
3.  如果 `cnt` 是奇数，那么结果将是完整的字符串。
4.  现在对于 `cnt` 为偶数和 `0` 的情况，所需的子串将从索引 `0` 开始，在最后一次出现 `1` 之前结束，或者在第一次出现 `1` 之后开始，在给定串的末尾结束。
