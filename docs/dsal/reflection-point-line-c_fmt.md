...
现在，使用坐标轴的平移和旋转方法，我们将找出一个点关于一般直线的反射。翻译的想法在之前的帖子里有描述。这里我们描述旋转的概念。
...
### 2. 旋转(将 B<sub>t</sub> A<sub>t</sub> 移动到 X 轴)

将所有点除以 B<sub>t</sub> (分割是指顺时针方向旋转，这是这里引入 X 轴的要求)。

```
Pr = Pt/Bt 
```

### 3. P<sub>r</sub> 关于 B<sub>r</sub> A<sub>r</sub> 的反射(只不过是 X 轴)
