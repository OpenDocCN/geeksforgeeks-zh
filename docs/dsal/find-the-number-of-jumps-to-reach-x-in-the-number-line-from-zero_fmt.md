...
> 原文：[https://www.geeksforgeeks.org/find-the-number-of-jumps-to-reach-x-in-the-number-line-from-zero/](https://www.geeksforgeeks.org/find-the-number-of-jumps-to-reach-x-in-the-number-line-from-zero/)
...
*   如果你总是朝着正确的方向跳，那么在 `n` 跳之后，你将会到达点 `p = 1+2+3+4+…+n`。
*   在这 `n` 次跳跃中的任何一次，如果你在第 `k` 次跳跃中没有向右跳跃，而是向左跳跃(`k<=n`)，你会位于点 `p–2k`。
