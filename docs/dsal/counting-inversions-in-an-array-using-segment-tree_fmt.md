...
> 原文：[https://www.geeksforgeeks.org/counting-inversions-in-an-array-using-segment-tree/](https://www.geeksforgeeks.org/counting-inversions-in-an-array-using-segment-tree/)
...
*   遍历数组，让索引 `i` 处的数字为 `a[i]`。我们将在 `[a[i]+1, max]` 范围内找到段树中有多少个数字，其中 `max` 是数组的最大元素，并将其添加到答案变量中。
