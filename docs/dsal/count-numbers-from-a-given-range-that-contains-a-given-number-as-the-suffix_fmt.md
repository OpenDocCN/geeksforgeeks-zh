...
> 原文：[https://www.geeksforgeeks.org/count-numbers-from-a-given-range-that-contains-a-given-number-as-the-suffix/](https://www.geeksforgeeks.org/count-numbers-from-a-given-range-that-contains-a-given-number-as-the-suffix/)

给定三个整数 `A`、`L` 和 `R`，任务是从范围 `L` 到 `R` 对包含 `A` 作为后缀的数字进行计数。
...
解决问题最简单的方法是遍历 `L` 到 `R` 范围内的数字，检查数字是否以 `A` 结尾。对于所有被发现为真的数字，将这些数字的计数增加 1。最后，打印最终计数。
...
*   计数并存储 `A` 的位数，并将其存储在一个变量中，比如说 `count`。
*   将 10 提升到 `A` 的位数的幂，即 `10^count`。
*   检查每 `10^count` 是否在 `[L, R]` 范围内。如果发现为真，则将计数增加 1。
*   打印 `count` 的最终值。
