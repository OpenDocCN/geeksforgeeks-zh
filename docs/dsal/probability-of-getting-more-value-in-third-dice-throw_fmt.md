...
> 原文：[https://www.geeksforgeeks.org/probability-of-getting-more-value-in-third-dice-throw/](https://www.geeksforgeeks.org/probability-of-getting-more-value-in-third-dice-throw/)
...
思路是找出 `A` 和 `B` 的最大值，然后 `6 - max(A, B)` 给我们剩下的 `C` 应该拿到的数字来赢得比赛。所以，用这两者的 [GCD](https://write.geeksforgeeks.org/geek/find-any-pair-with-given-gcd-and-lcm-3/) 可以找到 `6 - max(A, B)` 和 `6` 除的答案。
