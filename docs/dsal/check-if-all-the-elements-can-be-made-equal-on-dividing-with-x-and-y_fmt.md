...
> 原文: [https://www.geeksforgeeks.org/check-if-all-the-elements-can-be-made-equal-on-dividing-with-x-and-y/](https://www.geeksforgeeks.org/check-if-all-the-elements-can-be-made-equal-on-dividing-with-x-and-y/)

给定一个数组 `arr[]` 和两个整数 `X` 和 `Y`。任务是检查是否可以用 `X` 和 `Y` 对所有元素进行任意次（包括 0 次）除法运算，使所有元素相等。
...
**方法:** 从给定的数组中找到所有元素的 [gcd](https://www.geeksforgeeks.org/basic-and-extended-euclidean-algorithms/)，因为这个 gcd 是我们可以通过用一些任意常数（比如 `gcd = arr[0] / k1` 或 `arr[1] / k2` 或…或 `arr[n-1] / kn`）除所有元素而得到的值。现在的任务是找出这些常数 `k1, k2, k3, …, kn` 是否为 `X * X * X *……* Y * Y *……` 的形式。如果是，那么有可能使所有元素与给定的操作相等，否则就不相等。
