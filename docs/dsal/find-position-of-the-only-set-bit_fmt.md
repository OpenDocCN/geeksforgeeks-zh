...
> 原文: [https://www.geeksforgeeks.org/find-position-of-the-only-set-bit/](https://www.geeksforgeeks.org/find-position-of-the-only-set-bit/)
...
**(3)** 在循环内部，对 `i` 和数字 `N` 进行按位“与”。如果该操作的值为真，则设置 `pos` 位，因此中断循环并返回位置。否则，将 `pos` 增加 1，将左移位 `i` 增加 1，并重复该过程。
...
**我们也可以用 `log base 2` 找位置**。感谢[阿伦库马](https://www.facebook.com/arunkumar.somalinga)提出这个解决方案。
...
本文由纳伦德拉·康拉尔卡尔整理。如果你发现任何不正确的地方，或者你想分享更多关于上面讨论的话题的信息，请写评论。