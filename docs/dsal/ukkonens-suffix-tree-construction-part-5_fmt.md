...
> 原文: [https://www.geeksforgeeks.org/ukkonens-suffix-tree-construction-part-5/](https://www.geeksforgeeks.org/ukkonens-suffix-tree-construction-part-5/)
...
### 扩展 7
*   检查活动边缘(c)是否有边缘从活动节点(节点 A)伸出。如果没有，创建一个叶片边缘。如果有，走下去。在我们的例子中，边“c”出现在活动节点(节点 A)之外。
*   如有必要，走下去(技巧 1–跳过/计数)。在当前的扩展 7 中，不需要向下走，因为活动长度
*   检查字符串 S 的当前字符(也就是“d”)是否已经出现在活动点之后。如果没有，则规则 2 将适用。在我们的例子中，没有以“d”开头的路径从活动点伸出，所以我们创建一个标签为“d”的叶片边缘。由于活动点结束在一条边的中间，我们将在活动点之后创建一个新的内部节点(规则 2)

![Phase 10](img/5ec57f634b39d2179df876aae54638f5.png)

*在当前扩展 7 中新创建的内部节点 c(上图)，将在下一个扩展 8 中设置其后缀链接(见下图 38)。*

*   在树中添加后缀“abcd”时，将 `remainingSuffixCount` 减 1(从 4 到 3)。
*   现在 `activePoint` 将为下一个扩展 8 而改变。当前 `activeNode` 是一个内部节点(节点 A)，所以那里一定有一个后缀链接，我们将跟随它来获得新的 `activeNode`，那将是“节点 B”。`activeEdge` 和 `activeLength` 没有变化(这是 `APCFER2C2`)。所以新的 `activePoint` 是(节点 B，c，1)。

### 扩展 8
*   现在在扩展 8 中(这里我们将添加后缀“bcd”)，在当前活动点之后添加字符“d”时，将应用与前一个扩展 7 完全相同的逻辑。在前一个扩展 7 中，我们在活动点(节点 A，c，1)添加了字符“d”，而在当前扩展 8 中，我们将在活动点(节点 B，c，1)添加相同的字符“d”。所以逻辑将是相同的，这里我们创建一个标签为“d”的新叶片边缘，并将创建一个新的内部节点。前一个扩展的新内部节点(C)将通过后缀链接指向当前扩展的新节点(D)。

![Extension 7](img/d7a8098d63db9ae6c9ce1090797ba205.png)

*请注意，前一个扩展中的节点 C(见上图 37)在这里设置了其后缀链接，而在当前扩展中创建的节点 D 将在下一个扩展中设置其后缀链接。如果在接下来的扩展中没有创建新节点会发生什么？我们之前在阶段 6([第 4 部分](https://www.geeksforgeeks.org/ukkonens-suffix-tree-construction-part-4/ "Ukkonen’s Suffix Tree Construction – Part 4"))中见过这种情况，并将在本阶段 10 的最后一个扩展中再次看到。敬请关注。*

*   在树中添加后缀“bcd”时，将 `remainingSuffixCount` 减 1(从 3 到 2)。
*   现在 `activePoint` 将为下一个扩展 9 而改变。当前 `activeNode` 是一个内部节点(节点 B)，所以那里一定有一个后缀链接，我们将跟随它来获得新的 `activeNode`，那就是“根节点”。`activeEdge` 和 `activeLength` 没有变化(这是 `APCFER2C2`)。所以新的 `activePoint` 是(根，c，1)。

### 扩展 9
*   现在在扩展 9 中(这里我们将添加后缀“cd”)，在当前活动点之后添加字符“d”时，将应用与前一个扩展 7 和 8 完全相同的逻辑。请注意，前一个扩展 8 中创建的内部节点 D，现在通过后缀链接指向当前扩展中创建的内部节点 E。

![extension 8](img/0fcc8e01929a39166df7f40bed5e3d30.png)

*   随着树中后缀“cd”的增加，`remainingSuffixCount` 递减 1(从 2 到 1)。
*   现在 `activePoint` 将为下一个扩展 10 而改变。当前 `activeNode` 是根且 `activeLength` 为 1，基于 `APCFER2C1`，`activeNode` 将保持为“根”，`activeLength` 将减少 1(从 1 到零)，`activeEdge` 将为“d”。所以新的 `activePoint` 是(根，d，0)。

### 扩展 10

## 第 11 期

我们从字符串 S 中读出第 11 个字符(`$`)。

*   将 `END` 设置为 11（这将进行扩展 1 到 10）——因为到上一阶段 10 结束时为止，我们已经有 10 个叶边缘。

![extension 10](img/122dcae8c3eaf3347b493de74ebebeb5.png)

*   将 `remainingSuffixCount` 递增 1（从 0 到 1），即只有一个后缀“`$`”要添加到树中。
    *   由于 `activeLength` 为零，`activeEdge` 将更改为当前字符“`$`”；正在处理的字符串的数量（`APCFALZ`）。
    *   没有边缘从活动节点根出去，因此标签为“`$`”的叶边缘将被创建（规则 2）。

![phase 11](img/d96bbfa9d5b3219f93039a8540d26ccc.png)

*   将 `remainingSuffixCount` 减 1（从 1 到 0），作为后缀“`$`”在树中添加。这意味着没有更多的后缀需要添加，因此阶段 11 到此结束。请注意，该树是一个显式树，因为所有后缀都是显式添加到树中的（为什么？因为字符 `$` 以前在字符串 S 中没有出现过）。

现在我们已经将字符串 `'abcabxabcd$'` 的所有后缀添加到后缀树中。这棵树有 11 个叶端，从根到叶端的路径上的标签代表一个后缀。现在剩下的唯一一件事就是给每个叶子末端分配一个数字（后缀索引），这个数字就是字符串 S 中的后缀起始位置。在进行 `DFS` 遍历时，跟踪标签长度，当找到叶子末端时，将后缀索引设置为“`stringSize – labelSize + 1`”。索引后缀树如下所示：
