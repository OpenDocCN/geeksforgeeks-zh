...
> 原文: [https://www.geeksforgeeks.org/program-to-find-the-last-two-digits-of-xy/](https://www.geeksforgeeks.org/program-to-find-the-last-two-digits-of-xy/)

任务是找到 `x^y` 的最后两位数字。

由于可以结束的数字是 0-9，因此这个问题可以分为 5 种情况：
...
*   `3^4` 给 1，所以我们拿 `23^4`
*   `((23)^4)^8 * (23)^2`
*   `23^4` 的最后两位是 41，所以我们取 `(41)^8` 并根据给定的图表求解。
...
*   `7^4` 给 1，所以我们拿 `37^4`
*   `((37)^4)^11 * (37)^1`
*   `37^4` 的最后两位是 61，所以我们取 `(61)^11` 并根据图表求解。
...
*   `9^2` 给 1，所以我们拿 `59^2`
*   `((59)^2)^11`
*   `59^2` 的最后两位是 81，所以我们取 `(81)^11` 并根据图表求解。
...
因为找到一个数字的最后一个数字以 2，4，6，8 结尾；我们使用数字 76，这是一种魔数，因为它的平方、立方等包含最后 2 位数字，即 76。
...
*   **第一步: 转换** `4^972 = (2^2)^972 = 2^1944 = (2^10)^194 * 2^4`
*   **第二步: 偶次方所以，我们取 76**
    `= 76 * 16`
    `= 1216`
...
*   **第 1 步:** 转换
    `6^600 = (2)^600 * (3)^600 = (2^10)^60 * ((3)^4)^150` {适用于 `(3)^600`}
