...
> 原文: [https://www.geeksforgeeks.org/number-full-binary-trees-node-product-children/](https://www.geeksforgeeks.org/number-full-binary-trees-node-product-children/)

给定一组 `n` 个整数，每个整数都大于 1。任务是从给定的整数中找到[全二叉树](https://www.geeksforgeeks.org/binary-tree-set-3-types-of-binary-tree/)的个数，使得每个非叶节点值都是其子节点值的乘积。考虑到这一点，每个整数都可以在一个完整的二叉树中多次使用。
