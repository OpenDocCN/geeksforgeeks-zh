...
> 原文：[https://www.geeksforgeeks.org/find-the-array-element-having-maximum-frequency-of-the-digit-k/](https://www.geeksforgeeks.org/find-the-array-element-having-maximum-frequency-of-the-digit-k/)

给定一个大小为 `N` 的数组 `arr[]` 和一个整数 `K`，任务是找到一个包含数字 `K` 最大次数的数组元素。如果存在多个解决方案，则打印其中任何一个。否则，打印 `-1`。
...
**方法：** 想法是遍历数组，对于每个单独的数组元素，计算数字 `K` 在其中的出现次数。按照以下步骤解决问题：
