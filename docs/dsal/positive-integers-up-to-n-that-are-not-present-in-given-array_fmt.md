...
> 原文: [https://www.geeksforgeeks.org/positive-integers-up-to-n-that-are-not-present-in-given-array/](https://www.geeksforgeeks.org/positive-integers-up-to-n-that-are-not-present-in-given-array/)
...
1.  初始化一个[位组](https://www.geeksforgeeks.org/c-bitset-and-its-application/)变量，`bitset`，长度为 `N`。
2.  对于每个数组元素，使用 `bset.set(arr[i]-1, 0)` 将其位设置为 false，其中它将位置`arr[i]–1`处的位设置为 `0`。
3.  现在，从 `bset._Find_first()` 到 `bset.size()–1` 使用变量，说 `i`。
