...
> 原文: [https://www.geeksforgeeks.org/check-if-a-number-is-an-achilles-number-or-not/](https://www.geeksforgeeks.org/check-if-a-number-is-an-achilles-number-or-not/)

给定一个正整数 `N`，任务是检查 `N` 是否是阿基里斯数。如果 `N` 是阿基里斯数，则打印“是”，否则打印“否”。

`阿喀琉斯数:` 在数学中，阿喀琉斯数是一个强大的数（一个数 `n` 被称为强大的数，如果它的每一个质因数 `p`，`p^2` 也将其除）但不是[完美幂](https://www.geeksforgeeks.org/perfect-power-1-4-8-9-16-25-27/)。
阿喀琉斯号的前几名是——
...
3.  如果 `n` 很强大但不完美，那么 `n` 是一个阿基里斯数，否则不是。
