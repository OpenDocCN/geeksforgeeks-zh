...
> 原文：[https://www.geeksforgeeks.org/program-for-area-of-square-after-n-th-fold/](https://www.geeksforgeeks.org/program-for-area-of-square-after-n-th-fold/)

正方形是一个平面上的平面形状，由四个角上的四个点定义。正方形有四条等长的边和四个直角（90度角）。正方形是一种长方形。
给定正方形的边`N`和折叠数`F`。任务是找到`F-th`褶皱后的正方形面积。
方块的折叠如下：
