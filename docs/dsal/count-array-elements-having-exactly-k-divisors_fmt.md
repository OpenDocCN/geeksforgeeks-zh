...
给定一个由 `N` 个整数和一个整数 `K` 组成的数组 `arr[]`，任务是计算正好具有 `K` 个除数的数组元素的数量。
...
解决这个问题的思路是计算每个数组元素的除数总数，并将其存储在**地图**中。然后，遍历数组，对于每个数组元素，检查是否正好有 `K` 除数。打印此类数字的计数。按照以下步骤解决问题：

*   找到数组中存在的最大元素。
*   初始化一个数组 `prime[]`。
*   使用厄拉多塞的筛将范围 `[2]` 内存在的所有**质数**存储在数组 `prime` 中（数组大小为最大元素）。
*   遍历数组并使用给定的公式对每个数组元素进行素因子分解：
...
*   初始化一个映射来存储每个数组元素的除数总数。
*   遍历数组并初始化一个变量，比如说 `count`，到计数正好有 `K` 除数总数的元素数量。
*   打印精确具有 `K` 除数的元素数量。
