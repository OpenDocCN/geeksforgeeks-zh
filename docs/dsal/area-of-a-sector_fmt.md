...
在此图中，绿色阴影部分是一个扇形，`r`是半径，`θ`是角度，如图所示。这里，我们可以说阴影部分是次要部分，另一部分是主要部分。`L`是扇形的弧线。更多信息，请访问[扇区](https://en.wikipedia.org/wiki/Circular_sector)。
...
扇形的面积类似于圆的[面积计算](https://www.geeksforgeeks.org/c-program-find-area-circle/)，只是将圆的面积乘以扇形的角度。
