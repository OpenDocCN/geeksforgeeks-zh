...
> 原文: [https://www.geeksforgeeks.org/kth-smallest-prime-number-in-range-l-to-r-for-q-queries/](https://www.geeksforgeeks.org/kth-smallest-prime-number-in-range-l-to-r-for-q-queries/)

给定三个变量 `L`，`R` 和 `Q`，表示范围【L，R】和查询总数。每个查询都有一个变量 `K`。任务是在【`L`，`R`】范围内找到 `K` 第 `K` 个最小素数。如果 `K` 大于【`L`，`R`】范围内的素数个数，则返回 `-1`。
...
**说明:**
给定范围内的素数为 5，7，11，13，17，19 和第 4 和第 3 个最小素数为 13 和 11。
...
*   运行厄拉多塞的筛，寻找范围【`L`，`R`】内的数字
*   对于每个查询，从计算出的素数中找出第 `K` 个最小素数。
*   如果 `K` 超过该范围内的素数，返回 `-1`。
