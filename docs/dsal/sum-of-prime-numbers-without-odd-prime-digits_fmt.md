...
> 原文: [https://www.geeksforgeeks.org/sum-of-prime-numbers-without-odd-prime-digits/](https://www.geeksforgeeks.org/sum-of-prime-numbers-without-odd-prime-digits/)

给定一个整数 `N`。任务是找到前 `N` 个不包含任何奇素数位数的素数的和。
...
*   接下来检查每个质数是否包含奇数位质数。
*   如果不包含这样的数字，那么我们将把这个质数包括到我们需要的答案中。
