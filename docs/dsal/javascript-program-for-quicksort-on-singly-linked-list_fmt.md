...
> 原文：[https://www.geeksforgeeks.org/javascript-program-for-quicksort-on-singly-linked-list/](https://www.geeksforgeeks.org/javascript-program-for-quicksort-on-singly-linked-list/)
...
在`partition()`中，我们将最后一个元素视为轴心。我们遍历当前列表，如果一个节点的值大于 `pivot`，我们将它移到 `tail` 之后。如果节点的值较小，我们会将其保持在当前位置。

在`quickSortRecursive()`中，我们首先调用 `partition()` 将 `pivot` 放在正确的位置并返回 `pivot`。在枢轴被放置在正确的位置之后，我们找到左侧（枢轴之前的列表）的尾部节点，并为左侧列表重现。最后，我们重现正确的列表。
