...
> 原文：[https://www.geeksforgeeks.org/twin-pythagorean-triplets-in-an-array/](https://www.geeksforgeeks.org/twin-pythagorean-triplets-in-an-array/)

给定一个整数数组 `arr[]`，任务是检查数组中是否有**双勾股三元组**。

> 一个**孪生毕达哥拉斯三元组**是一个毕达哥拉斯三元组 `(a，b，c)`，其中两个值是连续的整数。前几个双胞胎三联体是 `(3，4，5)`，`(5，12，13)`，`(7，24，25)`，`(20，21，29)`，`(9，40，41)`，`(11，60，61)`，`(13，84，85)`，`(15，112，113)`，…。
...
*   求输入数组中每个元素的平方。这一步需要 `O(n)` 时间。
*   按递增顺序对方形数组进行排序。这一步需要 `O(nLogn)` 时间。
*   要找到一个三元组 `(a，b，c)`，使得 `a^2 = b^2 + c^2` 和至少一对是连续的，请执行以下操作。
    *   将 `a` 固定为排序数组的最后一个元素。
