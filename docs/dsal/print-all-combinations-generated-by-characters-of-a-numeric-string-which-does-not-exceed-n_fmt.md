...
> 原文: [https://www.geeksforgeeks.org/print-all-combinations-generated-by-characters-of-a-numeric-string-which-does-not-exceed-n/](https://www.geeksforgeeks.org/print-all-combinations-generated-by-characters-of-a-numeric-string-which-does-not-exceed-n/)

给定一个长度为 `M` 的数字[字符串](https://www.geeksforgeeks.org/string-data-structure/) `S` 和一个整数 `N`，任务是找出值最多为 `N` 的 `S`（允许重复）的所有不同组合。
...
## 方法
想法是使用[回溯](https://www.geeksforgeeks.org/backtracking-introduction/)生成所有可能的数字，然后打印那些不超过 `N` 的数字。按照以下步骤解决问题:

*   初始化一个[字符串集合](https://www.geeksforgeeks.org/set-in-cpp-stl/)，比如 `combinations[]`，来存储所有值不超过 `N` 的 `S` 的不同组合。
*   初始化一个字符串 `ans` 来存储当前可能来自 `S` 的数字组合。
*   声明一个函数 `generateCombinations()` 来生成所有值小于给定值 `N` 的必需组合，该函数定义为:
    *   使用变量 `i` 在范围 `[0, M]` 内遍历字符串 `S`，并执行以下操作:
        *   将当前字符 `S[i]` 推至 `ans`，并将当前字符串 `ans` 转换为数字并存储在 `x` 中。
        *   如果 `x` 小于或等于 `N`，则将字符串 `ans` 推入 `combinations[]` 并递归调用函数 `generateCombinations()`。
    *   通过从 `ans` 中移除第 `i` 个字符，返回到其先前的状态。
*   完成上述步骤后，打印存储在 `combinations[]` 中的所有字符串的集合。
