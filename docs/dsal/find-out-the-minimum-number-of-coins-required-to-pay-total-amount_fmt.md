...
> 原文: [https://www.geeksforgeeks.org/find-out-the-minimum-number-of-coins-required-to-pay-total-amount/](https://www.geeksforgeeks.org/find-out-the-minimum-number-of-coins-required-to-pay-total-amount/)

给定总金额 `N` 和无限数量的价值 `1`、`10` 和 `25` 的硬币。找出准确支付金额 `N` 所需的最小硬币数量。
...
1.  如果 `N` 的值小于 `10`，那么价值为 `1` 的硬币只能用于支付。
2.  当 `N` 大于 `9` 且小于 `25` 时，那么价值为 `1` 和 `10` 的硬币将用于支付。在这里，为了尽量减少硬币的使用数量，价值 `10` 的硬币将是首选。
3.  当 `N` 大于 `24` 时。那么所有价值 `1`、`10` 和 `25` 的硬币都将用于支付。为了最大限度地减少硬币数量，首要目标是尽可能多地首先使用价值 `25` 的硬币，然后使用价值 `10` 的硬币，然后使用价值 `1` 的硬币。
