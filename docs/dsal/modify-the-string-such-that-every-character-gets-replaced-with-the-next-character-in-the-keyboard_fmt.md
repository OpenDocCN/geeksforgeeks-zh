...
> 原文：[https://www.geeksforgeeks.org/modify-the-string-such-that-every-character-gets-replaced-with-the-next-character-in-the-keyboard/](https://www.geeksforgeeks.org/modify-the-string-such-that-every-character-gets-replaced-with-the-next-character-in-the-keyboard/)

给定一个由小写英文字母组成的字符串。任务是用键盘上的下一个字母（以圆形方式）键改变字符串的每个字符。例如，`'a'` 替换为 `'s'`，`'b'` 替换为 `'n'`，…，`'l'` 替换为 `'z'`，…，`'m'` 替换为 `'q'`。
...
**方法：** 对于英语字母表中的每个小写字符，在 [`unordered_map`](https://www.geeksforgeeks.org/unordered_map-in-stl-and-its-applications/) 中将其旁边的字符插入键盘。现在逐个字符遍历给定的字符串，并用之前创建的映射更新每个字符。
