...
> **说明**
>
> 在查询 1 中，子数组`[0 … 4]`具有`4`个斐波那契数：`{1, 2, 3, 8}`。
>
> 在查询 2 中，索引 3 的值更新为 5，数组`arr`现在为`{1, 2, 3, 5, 8, 9}`。
>
> 在查询 3 中，子数组`[0 … 4]`具有`5`个斐波那契数：`{1, 2, 3, 5, 8}`。

## 方法

为了处理点更新和范围查询，为此目的，[段树](http://www.geeksforgeeks.org/segment-tree-set-1-sum-of-given-range/)是最佳的。
...
每个查询和更新的时间复杂度为`O(log n)`，构建段树的时间复杂度为`O(n)`。
