...
给定一个链表和两个整数`p`和`q`，任务是按`p:q`的比例划分链表，即第一个列表包含来自原始列表的前`p`个节点，第二个列表包含其余`q`个节点。 如果无法按照给定的比例分割原始列表，请打印`-1`。
...
首先找到链表的长度。 如果总和超过实际长度，则无法分割列表，因此打印`-1`。 如果可以划分列表，则只需遍历列表，直到长度为`p`，然后将其分解为`p:q`的比率即可。 下一个节点将成为第二个列表的头，然后打印两个列表。
