...
给定一个有`N`个顶点（值从`0`到`N – 1`）和`N – 1`条边的有向图，任务是计算必须反转的最小边数，使得从每个节点到`节点 0`总是存在一条路径。
...
这个想法是对图使用 [BFS 遍历](https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/)。步骤如下：

1.  创建一个有向图，使给定图的边方向相反。
2.  创建[队列](http://www.geeksforgeeks.org/queue-data-structure/)，并将`节点 0`推送到队列中。
3.  在图上进行 BFS 遍历期间，请执行以下操作：
    ...
    *   在原图中遍历当前节点的邻接表，对于每个未访问的邻居节点，将其压入队列并计数。
