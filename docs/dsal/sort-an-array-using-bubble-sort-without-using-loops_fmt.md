...
> 原文：[https://www.geeksforgeeks.org/sort-an-array-using-bubble-sort-without-using-loops/](https://www.geeksforgeeks.org/sort-an-array-using-bubble-sort-without-using-loops/)

给定一个由 `N` 个整数组成的数组 `arr[]`，任务是使用冒泡排序对给定数组进行排序，而不使用循环。
...
**方法：** 在不使用循环的情况下实现冒泡排序的想法基于以下观察：

*   冒泡排序的排序算法执行如下步骤：
    *   外环遍历给定数组 `(N–1)` 次。
    *   内环遍历数组，如果 `arr[i] > arr[i+1]`，对于范围 `[0, N–1]` 内的每一个 `i`，交换两个相邻的元素。
*   可以观察到，在每一次 `N–1` 迭代中，范围 `[0, N–1–i]` 上的最大元素在范围 `[0, N–1]` 上每一次 `i` 移动到位置 `(N–1–i)`。
*   因此，想法是使用递归来避免循环。
