...
> 原文: [https://www.geeksforgeeks.org/print-all-triplets-with-sum-s-in-given-sorted-linked-list/](https://www.geeksforgeeks.org/print-all-triplets-with-sum-s-in-given-sorted-linked-list/)
...
**解释:** 这是仅有的总和等于 `S` 即 15 的不同三元组。请注意，`(2, 4, 9)` 和 `(9, 4, 2)` 两个三元组的总和都是 15，但它们并不明显，因为三元组的所有元素都是相同的。
...
**天真方法:** 使用三个嵌套循环。生成所有三元组，找到总和等于 `S` 的不同三元组。
**时间复杂度:** `O(N^3)`
**辅助空间:** `O(N^3)`
