...
> 原文: [https://www.geeksforgeeks.org/program-to-check-similarity-of-given-two-triangles/](https://www.geeksforgeeks.org/program-to-check-similarity-of-given-two-triangles/)
...
在三角形 `ABC` 和 `PQR` 中，如果 `AB/PQ = BC/QR = CA/RP`，则三角形相似。
...
在三角形 `ABC` 和 `PQR` 中，如果 `AB/PQ = BC/QR` 和 `∠ABC = ∠PQR`，则三角形相似。

![](img/f3526df4329f9359fab3d934b13fcf5c.png)

## 3. 角度-角度-角度(AAA)相似性标准

如果一个三角形的所有角度都等于另一个三角形的相应角度，那么通过*角度-角度-角度* (AAA)的性质，可以说这些三角形是相似的。
在三角形 `ABC` 和 `PQR` 中，如果 `∠ABC = ∠PQR`、`∠BCA = ∠QRP` 和 `∠CAB = ∠RPQ`，那么三角形是相似的。
