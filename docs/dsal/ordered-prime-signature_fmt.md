...
任何正整数，`n` 都可以用它的素数因子的形式表示。如果 `n` 有 `p_1`，`p_2`，等等。作为它的素因子，那么 `n` 可以表示为：
...
for (int i = 0; i < r.Count; i++)
{
    if (n % r[i] == 0)
    {
        ...
        // Storing the prime factor and its exponent in map
        factor.Add(r[i], k);

        // Storing the exponent in a List
        sort_exp.Add(k);
    }
}
...
static void divisors(int n)
{
    ...
    // Adding one to each element present in ordered prime signature
    for (int i = 0; i < l; i++)
    {
        div[i] += 1;
        // Multiplying the elements
