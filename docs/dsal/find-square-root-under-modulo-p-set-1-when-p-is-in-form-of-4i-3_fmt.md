# 求模 p 下的平方根（当 p 是 4*i + 3 的形式时）

> 原文：[https://www.geeksforgeeks.org/find-square-root-under-modulo-p-set-1-when-p-is-in-form-of-4i-3/](https://www.geeksforgeeks.org/find-square-root-under-modulo-p-set-1-when-p-is-in-form-of-4i-3/)

给定一个数 `n` 和一个素数 `p`，求模 `p` 下 `n` 的平方根，如果它存在的话。可以给出 `p` 是 `4*i + 3`（或 `p % 4 = 3`）的形式，其中 `i` 是整数。这类素数的例子有 7、11、19、23、31、…等。
