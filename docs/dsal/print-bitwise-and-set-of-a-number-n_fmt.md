...
> 原文: [https://www.geeksforgeeks.org/print-bitwise-and-set-of-a-number-n/](https://www.geeksforgeeks.org/print-bitwise-and-set-of-a-number-n/)
...
一个天真的方法是从 `0` 到 `N` 迭代所有的数字，检查是否 `(N & i == i)`。打印满足指定条件的数字。
...
高效的解决方案是使用按位运算符来查找子集。我们可以只迭代按位子集，而不是迭代每个 `i`。向后迭代 `i=(i-1) & n` 给我们每个按位子集，其中 `i` 从 `n` 开始，以 `1` 结束。
