...
> 原文：[https://www.geeksforgeeks.org/find-the-winner-by-adding-pairwise-difference-of-elements-in-the-array-until-possible/](https://www.geeksforgeeks.org/find-the-winner-by-adding-pairwise-difference-of-elements-in-the-array-until-possible/)
...
移动次数可以发现为，`(max(arr)/gcd)–n` 其中 `gcd` 是原始数组元素的 `gcd`，`max(arr) / gcd` 给出结果数组中元素的总数。从结果数组的元素计数中减去元素的原始计数将得到移动的次数。
以下是上述方法的实施：
