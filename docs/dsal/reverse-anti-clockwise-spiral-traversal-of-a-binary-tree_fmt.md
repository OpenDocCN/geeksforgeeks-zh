...
> 原文：[https://www.geeksforgeeks.org/reverse-anti-clockwise-spiral-traversal-of-a-binary-tree/](https://www.geeksforgeeks.org/reverse-anti-clockwise-spiral-traversal-of-a-binary-tree/)
...
想法是用两个变量 `i` 初始化为 1，`j` 初始化为树高，运行一个 `while` 循环，直到 `i` 变得大于 `j` 才会中断。我们将使用另一个变量 `flag` 并将其初始化为 1。现在在 `while` 循环中，我们将检查一个条件，如果 `flag` 等于 1，我们将从左到右遍历树并将 `flag` 标记为 0，以便下次我们从右到左遍历树，然后递减 `j` 的值，以便下次我们访问刚好在当前级别之上的级别。同样，当我们从顶部遍历级别时，我们将把 `flag` 标记为 1，以便下次我们从左向右遍历树，然后增加 `i` 的值，以便下次我们访问当前级别之下的级别。重复整个过程，直到二叉树被完全遍历。
