...
*   现在将你拥有的第一个 IP 传递给两个哈希函数，它们会生成一些随机数，如下所示
...
*   类似地将第三个 IP 传递给两个哈希函数，假设你得到以下哈希函数输出
...
*   **测试输入 #1**
    假设我们想检查 IP `75.245.10.1`。将此 IP 与我们用于添加上述输入的相同两个哈希函数一起传递。
...
1.  当布隆过滤器说某个元素不存在时，它肯定是不存在。它 100%保证给定的元素在集合中不可用，因为散列函数给出的索引的任一位都将被设置为 0。
