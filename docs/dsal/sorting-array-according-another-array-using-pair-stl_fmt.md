# 在STL中使用pair根据另一个数组对一个数组进行排序

> 原文：[https://www.geeksforgeeks.org/sorting-array-according-another-array-using-pair-stl/](https://www.geeksforgeeks.org/sorting-array-according-another-array-using-pair-stl/)
...
我们在下面的帖子中讨论了不同的方法。[根据另一个数组定义的顺序对一个数组进行排序](https://www.geeksforgeeks.org/sort-array-according-order-defined-another-array/)

在这篇文章中，我们将重点放在使用 C++ 的 STL 中的 [`pair`](https://www.geeksforgeeks.org/pair-in-cpp-stl/) 容器上。

为了完成我们的任务，我们将从这两个数组中分别创建元素对。然后简单地使用 `sort` 函数。需要注意的重要一点是，`pair` 中的第一个元素应该来自要执行排序的数组。
