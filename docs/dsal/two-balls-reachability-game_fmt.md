...
给定白色的`A`号和黑色的`B`号。你需要有`X`数量的白球和`Y`数量的黑球（`A <= X`，`B <= Y`）通过做一些（零个或更多）的操作来赢得比赛。
...
3.  该属性与问题中提到的操作相同。因此，从这里，我们得到，如果最终状态的`gcd`与初始状态的`gcd`相同，那么它总是有可能达到目标，否则没有。
