...
> 原文: [https://www.geeksforgeeks.org/minimum-steps-to-make-all-the-elements-of-the-array-divisible-by-4/](https://www.geeksforgeeks.org/minimum-steps-to-make-all-the-elements-of-the-array-divisible-by-4/)
...
>
> 如图所示，
> 将 `array[0]` 和 `array[2]` 组合成 4。`array[1]` 和 `array[4]` 以及 `array[3]` 和 `array[5]` 也是如此。`array[6]` 已经可以被 4 整除。所以通过做 3 步，数组中的所有元素都可以被 4 整除。

> **输入:** `array = {12, 31, 47, 32, 93, 24, 61, 29, 21, 34}`
> **输出:** 4

## 方法

这里的思想是将数组中的所有元素转换为模 4。首先，数组中所有元素的和应该能被 4 整除。如果没有，这个任务是不可能的。

- 初始化一个大小为 4 到 0 的数组 `modulus`。
- 初始化计数器 `count` 到 0，以记录完成的步数。
- 遍历输入数组，取每个元素的模 4。
- 将 `modulus` 数组中 mod 4 值的值增加 1。
- `modulus[0]` 是已经被 4 整除的元素个数。因此不需要将它们与任何其他元素配对。
- `modulus[1]` 和 `modulus[3]` 元素可以组合得到一个可被 4 整除的数。因此，将 `count` 增加到两者的最小值。
- `modulus[2]` 的每 2 个元素可以组合得到一个可整除为 4 的元素。
- 对于其余元素，将 `modulus[2]` 增加 `modulus[1]` 和 `modulus[3]` 的一半。
- 现在，将 `count` 增加一半 `modulus[2]`。我们拿一半是因为每一个**两个**元素合二为一。
- `count` 的最终值是将输入数组的所有元素转换为可被 4 整除所需的步数。
