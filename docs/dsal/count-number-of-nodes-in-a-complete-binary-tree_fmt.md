...
> 原文: [https://www.geeksforgeeks.org/count-number-of-nodes-in-a-complete-binary-tree/](https://www.geeksforgeeks.org/count-number-of-nodes-in-a-complete-binary-tree/)

给定由 `N` 个节点组成的[完全二叉树](https://www.geeksforgeeks.org/binary-tree-set-3-types-of-binary-tree/)的根，任务是找到给定[二叉树](https://www.geeksforgeeks.org/binary-tree-data-structure/)中的节点总数。
...
求解给定树的简单方法是对给定树执行 DFS 遍历，并统计其中的节点数。遍历后，打印获得的节点总数。
...
*   定义一个[函数](https://www.geeksforgeeks.org/functions-in-c/) `left_height(root)`，通过沿根的左侧方向遍历找到给定树的左侧高度，并将其存储在一个变量中，比如 `leftHeight`。
*   定义一个函数 `right_height(root)`，通过沿根的正确方向遍历找到给定树的正确高度，并将其存储在一个变量中，比如 `rightHeight`。
*   为当前根值找到给定树的左右高度，如果相等，则返回 `(2^height – 1)` 的值作为节点的合成计数。
