...
> 原文：[https://www.geeksforgeeks.org/find-the-date-after-next-half-year-from-a-given-date/](https://www.geeksforgeeks.org/find-the-date-after-next-half-year-from-a-given-date/)
...
> **输入：** `D = 15`，`M = "January"`
> **输出：** `7 月 16 日`
> **说明：** `1 月 15 日`至下半年的日期为 `7 月 16 日`。
>
> **输入：** `D = 10`，`M = "October"`
> **输出：** `4 月 10 日`

**方法：** 由于闰年包含 `366` 天，给定的问题可以通过将当前日期增加 `183` 天后找到数据来解决。按照以下步骤解决问题：

*   存储该数组中每个月的天数，比如 `days[]`。
*   初始化一个变量，说 `curMonth` 为 `M`，存储当月的指数。
*   初始化一个变量，说 `curDate` 压缩为 `D`，以存储当前日期。
*   初始化一个变量，说 `count` 为 `183`，代表要递增的天数。
*   迭代直到 `count` 的值为正，并执行以下步骤：
    *   如果 `count` 的值为正，`curDate` 最多为 `month` 的天数，则 `count` 的值递减 `1`，将 `curDate` 的值递增 `1`。
    *   如果 `count` 的值为 `0`，则[退出循环](https://www.geeksforgeeks.org/break-statement-cc/)。
    *   通过 `(curMonth + 1)%12` 更新 `curMonth` 的值。
*   完成以上步骤后，打印对应的日期，打印结果为 `curDate` 和 `curMonth`。
