...
> 原文: [https://www.geeksforgeeks.org/number-positions-address-row-major-column-major-order/](https://www.geeksforgeeks.org/number-positions-address-row-major-column-major-order/)
...
**时间复杂度**:通过用`j`(等式)建立`i`的关系，可以将`O(M)`复杂度降低到`O(min(M，N))`。2)在`N < M`的情况下迭代所有可能的`j`，并且通过根据`i`(等式)建立关系`j`。1)迭代所有可能的`i`否则。