...
> 原文: [https://www.geeksforgeeks.org/check-whether-a-given-number-is-power-isolated-or-not/](https://www.geeksforgeeks.org/check-whether-a-given-number-is-power-isolated-or-not/)

给定一个整数 `N`，用质因数 `n1^p1 * n2^p2 * ……` 表示。任务是检查整数 `N` 是否电源隔离。
...
> *   找到质因数及其幂，并将它们存储在键值对中。
> *   然后计算所有因子及其幂的乘积。
> *   如果乘积等于整数，则打印 `true`，否则打印 `false`。
