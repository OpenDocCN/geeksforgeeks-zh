...
> 原文: [https://www.geeksforgeeks.org/print-updated-levels-of-each-node-of-a-complete-binary-tree-based-on-difference-in-weights-of-subtrees/](https://www.geeksforgeeks.org/print-updated-levels-of-each-node-of-a-complete-binary-tree-based-on-difference-in-weights-of-subtrees/)

给定一个完整的二叉树，其 `N` `级`编号为`[0, (N–1)]`从根到最低级别按递减顺序排列，其`权重`编号为`[1, 2^N–1]`从根到最后一个叶节点按递增顺序排列，每个节点的任务是根据以下条件调整其左右子树中存在的所有节点的级别值:
...
struct node {
    int weight; // stores the weight of node
    int level;  // stores the level of node
    struct node* left;
    struct node* right;
...
struct node* insert(struct node* root,
    ...
    struct node* n = new node(n_weight, n_level);
...
int main() {
    ...
    struct node* root = createTree(weights, levels);
...
## JavaScript 实现：基于子树权重差异更新完全二叉树节点层级
