...
> 原文: [https://www.geeksforgeeks.org/number-of-triplets-such-that-each-value-is-less-than-n-and-each-pair-sum-is-a-multiple-of-k/](https://www.geeksforgeeks.org/number-of-triplets-such-that-each-value-is-less-than-n-and-each-pair-sum-is-a-multiple-of-k/)
...
> **输出:** 9
> 可能的三元组有:
> `{(1, 1, 1), (1, 1, 3), (1, 3, 1), (1, 3, 3), (2, 2, 2), (3, 1, 1), (3, 1, 3), (3, 3, 3)}`
>
> **输入:** `N = 5`，`K = 3`
> **输出:** 1
> 唯一可能的三元组是 `(3, 3, 3)`
...
鉴于 `(a + b)`、`(b + c)` 和 `(c + a)` 是 `K` 的倍数。因此，我们可以说 `(a + b) % K = 0`、`(b + c) % K = 0`、`(c + a) % K = 0`。
如果 `a` 属于 `K` 的 `x` 模类，那么 `b` 应该在使用第一个条件的 `(K - x)` 模类中。
从第二个条件可以看出 `c` 属于 `K` 的 `x` 模类。现在由于 `a` 和 `c` 属于同一个模类，它们必须满足第三个关系，即 `(a + c) % K = 0`。只有 `x = 0` 或 `x = K / 2` 才有可能。
当 `K` 为奇数时，`x = K / 2` 无效。

因此要解决这个问题，在 `0` 模类和 `K` 的 `(K / 2)` 模类中，计算从 `0` 到 `N` 的元素个数。

*   如果 `K` 为**奇数**则结果为 `cnt[0]^3`
*   如果 `K` 是**偶数**，那么结果就是 `cnt[0]^3 + cnt[K/2]^3`。
