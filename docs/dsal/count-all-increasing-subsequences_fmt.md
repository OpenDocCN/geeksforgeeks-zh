...
> 原文: [https://www.geeksforgeeks.org/count-all-increasing-subsequences/](https://www.geeksforgeeks.org/count-all-increasing-subsequences/)
...
一个简单的解决方法是使用最长增加子序列（`LIS`）问题的[动态规划解](https://www.geeksforgeeks.org/dynamic-programming-set-3-longest-increasing-subsequence/)。像 `LIS` 问题一样，我们首先计算在每个索引处结束的递增子序列的计数。最后，我们返回所有值的和（在 `LCS` 问题中，我们返回所有值的最大值）。
...
`O(n)`注意，内部循环最多运行 10 次。

### 辅助空间

`O(1)`注意`count[]`最多有 10 个元素。
