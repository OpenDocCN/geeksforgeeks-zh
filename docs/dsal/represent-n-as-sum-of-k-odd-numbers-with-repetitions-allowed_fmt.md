...
> 原文: [https://www.geeksforgeeks.org/represent-n-as-sum-of-k-odd-numbers-with-repetitions-allowed/](https://www.geeksforgeeks.org/represent-n-as-sum-of-k-odd-numbers-with-repetitions-allowed/)

给定两个整数 `N` 和 `K`，任务是将 `N` 表示为 `K` 个奇数的和。如果无法创建总和，则输出 `-1`。
**注：** 表示可能包含重复的奇数。
...
void sumOddNumbers(int N, int K)
{
    ...
    // N must be greater than equal to K
    // and check must be odd
