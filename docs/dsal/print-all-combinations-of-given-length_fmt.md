...
> 原文：[https://www.geeksforgeeks.org/print-all-combinations-of-given-length/](https://www.geeksforgeeks.org/print-all-combinations-of-given-length/)
...
对于给定的一组大小为 `n` 的字符串，将会有长度为 `k` 的 `n^k` 可能的字符串，其思想是从一个空的输出字符串开始（我们在下面的代码中称之为 `prefix`）。逐个将所有字符添加到 `prefix` 中。对于添加的每个字符，通过递归调用等于 `k-1` 的 `k` 来打印当前前缀的所有可能的字符串。
