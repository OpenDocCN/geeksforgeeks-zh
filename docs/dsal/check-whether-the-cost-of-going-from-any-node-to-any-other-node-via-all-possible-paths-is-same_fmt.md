...
**方法:** 想法是维持两个阵列 `dis[]` 维持行进路径的距离，以及 `vis[]` 维持访问顶点。使用成对的 [2D 矢量](https://www.geeksforgeeks.org/2d-vector-in-cpp-with-user-defined-size/)存储图形。该对的第一个值是目的节点，第二个值是与之相关的成本。现在， [DFS](https://www.geeksforgeeks.org/depth-first-traversal-for-a-graph/) 在图上运行。每个顶点都有以下两种情况:
