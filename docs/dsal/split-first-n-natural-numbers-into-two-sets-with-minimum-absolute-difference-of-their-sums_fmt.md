...
> 原文：[https://www.geeksforgeeks.org/split-first-n-natural-numbers-into-two-sets-with-minimum-absolute-difference-of-their-sums/](https://www.geeksforgeeks.org/split-first-n-natural-numbers-into-two-sets-with-minimum-absolute-difference-of-their-sums/)
...
这个问题可以用[贪婪手法](https://www.geeksforgeeks.org/greedy-algorithms/)解决。按照以下步骤解决问题:

*   初始化两个变量，比如 `sumSet1` 和 `sumSet2` 来存储两个集合的元素之和。
*   先遍历 `N` 个自然数，从 `N` 到 `1`。对于每个数字，检查 set1 中元素的当前总和是否小于或等于 set2 中元素的总和。如果发现为真，将当前遍历的号码加入 `集合1` 并更新 `集合1`。
*   否则，将当前自然数的值加到 `集合2` 并更新 `集合2`。
*   最后，打印 `abs(sumSet1 - sumSet2)` 作为所需答案。
...
>
> 数学证明:
> 考虑 4 个连续整数 `{a1, a2, a3, a4}`
> `a4 = a3 + 1`
> `a1 = a2 - 1`
> => `a4 + a1 = a3 + 1 + a2 - 1 = a3 + a2`
