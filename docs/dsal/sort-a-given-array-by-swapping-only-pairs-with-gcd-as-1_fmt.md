...
> 原文: [https://www.geeksforgeeks.org/sort-a-given-array-by-swapping-only-pairs-with-gcd-as-1/](https://www.geeksforgeeks.org/sort-a-given-array-by-swapping-only-pairs-with-gcd-as-1/)
    ...
    function isPossible(arr) {
        ...
        for (let i = 0; i < arr.length; i++) {
            for (let j = i + 1; j < arr.length; j++) {
                ...
                // GCD of both elements is 1,
                // GCD of both the indices
                if (arr[i] > arr[j] && __gcd(arr[i], arr[j]) == 1) {
                    let temp = arr[i];
                    arr[i] = arr[j];
                    arr[j] = temp;
