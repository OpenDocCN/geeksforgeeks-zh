...
> 原文: [https://www.geeksforgeeks.org/check-if-given-string-satisfies-the-following-conditions/](https://www.geeksforgeeks.org/check-if-given-string-satisfies-the-following-conditions/)

给定长度为 `L` 的[字符串](https://www.geeksforgeeks.org/string-data-structure/) `S`，其中 `L` 为[完全平方数](https://www.geeksforgeeks.org/check-if-given-number-is-perfect-square-in-cpp/)，任务是检查给定字符串是否满足以下条件：

*   将字符串的字符按行插入到尺寸为 `√L x √L` 的正方形[矩阵](https://www.geeksforgeeks.org/category/data-structures/matrix/) `A[][]` 中。
*   用 `0` 初始化另一个矩阵 `M[][]`，用 `1` 填充左对角线。现在，对于出现在左对角线的 `1`，用 `1` 填充其对应的右对角线。
...
思路是[遍历矩阵](https://www.geeksforgeeks.org/traverse-a-given-matrix-using-recursion/) `A[][]`，其中其在矩阵 `M[][]` 中对应的字符为 `1`。按照以下步骤解决问题：
