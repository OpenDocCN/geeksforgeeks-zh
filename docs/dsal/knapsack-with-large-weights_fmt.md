...
给定一个容量为 `C` 的背包和两个代表 `N` 个不同物品的重量和价值的数组 `w[]` 和 `val[]`，任务是找到你能放入背包的最大值。物品不可分割，重量为 `X` 的物品占背包 `X` 的容量。
...
**方法:** 传统著名的 [0-1 背包](https://www.geeksforgeeks.org/0-1-knapsack-problem-dp-10/)问题可以在 `O(N*C)` 时间内解决，但是如果背包容量巨大，那么二维 `N*C` 数组就无法创建。幸运的是，可以通过重新定义 `dp` 的状态来解决这个问题。
我们先来看看 `dp` 的状态。
`dp[V][i]` 代表获得至少 `V` 值所需的子数组 `arr[i…N-1]` 的最小权重子集。循环关系为:

> `dp[V][i] = min(dp[V][i+1], w[i] + dp[V – val[i]][i + 1])`

因此，对于从 `0` 到 `V` 可能的最大值的每个 `V`，尝试找出给定的 `V` 是否可以用给定的数组来表示。能代表的最大的这样的 `V` 就成了答案。
