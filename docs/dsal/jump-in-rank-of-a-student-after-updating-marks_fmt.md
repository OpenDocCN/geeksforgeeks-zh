...
> 原文: [https://www.geeksforgeeks.org/jump-in-rank-of-a-student-after-updating-marks/](https://www.geeksforgeeks.org/jump-in-rank-of-a-student-after-updating-marks/)
...
> `updates[] = {-10, 5, 9, 39}`
> **输出:** `Name: sonu, Jump: 3`
> 更新后的分数为 `{89, 80, 79, 99}`，很明显 `sonu` 的最高分是跳 3
>
> **输入:** `名称[] = {"Sam", "ram", "geek"}`，
> `标记[] = {80, 79, 75}`，
> `更新[] = {0, 5, -9}`
...
现在，根据`更新的内容`更新分数`标记[]`，然后用数组的单次遍历，找到更新后分数最高的学生。
