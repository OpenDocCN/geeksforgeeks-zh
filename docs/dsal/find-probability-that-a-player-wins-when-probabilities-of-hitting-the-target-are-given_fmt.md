...
> 原文: [https://www.geeksforgeeks.org/find-probability-that-a-player-wins-when-probabilities-of-hitting-the-target-are-given/](https://www.geeksforgeeks.org/find-probability-that-a-player-wins-when-probabilities-of-hitting-the-target-are-given/)
...
**进场:** 第一个玩家命中目标的概率为 `p / q`，未命中的概率为 `1–p/q`。第二个玩家命中目标的概率为 `r / s`，错过的概率为 `1–r/s`。让第一个玩家为 `x`，第二个玩家为 `y`。所以总概率会是 `x 赢了 + (x 输了 * y 输了 * x 赢了) + (x 输了 * y 输了 * x 输了 * y 输了 * x 赢了) + ……如此类推`。因为 `x` 任何回合都能赢，所以是无限序列。让 `t = (1–p/q)*(1–r/s)`。这里 `t < 1` 作为 `p / q` 和 `r / s` 始终是 `< 1`。所以级数会变成，`p/q + (p/q)* t + (p/q)* t^2 + ……`。这是一个无穷的 GP 级数，公比小于 1，其和会是 `(p/q)/(1–t)`。
