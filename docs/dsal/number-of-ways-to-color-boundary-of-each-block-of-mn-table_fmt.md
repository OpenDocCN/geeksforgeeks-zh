...
> 原文: [https://www.geeksforgeeks.org/number-of-ways-to-color-boundary-of-each-block-of-mn-table/](https://www.geeksforgeeks.org/number-of-ways-to-color-boundary-of-each-block-of-mn-table/)
...
*   找出填充矩形右上侧的方法。上侧有 `M` 个单位，右侧有 `N` 个单位。所以，给矩形的上侧和右侧上色的方法数量是 `pow(3, M + N)` ，因为每个方法都有 3 个选项。
*   现在，为了填充每个方块的下方和左侧，每个方块有 2 个选项，那么路的数量是 `pow(2, M * N)` 。
*   最终结果是:
