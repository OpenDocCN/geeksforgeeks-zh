...
> 原文: [https://www.geeksforgeeks.org/reconstruct-the-array-by-replacing-arri-with-arri-11-m/](https://www.geeksforgeeks.org/reconstruct-the-array-by-replacing-arri-with-arri-11-m/)

给定一个由 `N` 个元素和一个整数 `m` 组成的数组。现在，通过用 `-1` 替换一些数组元素来修改数组。任务是打印原始数组。
在原始数组中的元素是相关的，对于每个索引 `i`， `a[i] = (a[i-1]+1)% M` 。
保证数组中有一个非零值。
