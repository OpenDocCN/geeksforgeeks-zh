...
给定一个数字 `n`，任务是找出这个数字是否是史密斯数。史密斯数是一个复合数，其位数之和等于其素因数分解中的位数之和。
...
这个想法是首先使用 Sundaram 的筛法找到一个极限以下的所有质数（这在我们要为多个数检查史密斯数时特别有用）。现在，对于每一个要检查是否为史密斯数的输入，我们检查它的所有质因数，并找到每个质因数的位数总和。我们还可以找到给定数字的位数总和。最后我们比较两个和。如果它们是一样的，那么它就是史密斯数。
