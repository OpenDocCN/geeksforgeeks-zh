...
> 原文: [https://www.geeksforgeeks.org/print-all-palindromic-levels-of-a-binary-tree/](https://www.geeksforgeeks.org/print-all-palindromic-levels-of-a-binary-tree/)

给定一棵`二叉树`，任务是打印这棵树的所有回文层级。

`回文层级`是二叉树的任何层级，如果从左向右遍历，结果与从右向左遍历该层级相同，则称为回文层级。
...
**方法:** 为了检查每个级别是否是回文级别，我们需要首先对二叉树进行级别顺序遍历，得到每个级别的值。然后对于每一级，检查它是否是回文。这里一个`队列数据结构`用于存储树的级别，同时进行级别顺序遍历。
