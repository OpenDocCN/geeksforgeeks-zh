...
> 原文: [https://www.geeksforgeeks.org/sudo-placement-special-subsequences/](https://www.geeksforgeeks.org/sudo-placement-special-subsequences/)
...
[字符串的子序列](https://www.geeksforgeeks.org/print-subsequences-string/)

## 算法思路

这是一个细微的变化，对于经典问题，打印一个字符串的子序列。假设我们有一个递归函数 `generateSubsequences(input, i)`，它打印输入字符串的所有特殊子序列直到第 `i` 个位置。假设当前位置是输入字符串中的 `i`，那么有三种可能：

1.  按原样将 `input[i]` 包含在输出字符串中，并调用 `generateSubsequences(input, i + 1)`
2.  在输出字符串中排除 `input[i]`，并调用 `generateSubsequences(str, i + 1)`，或者
3.  包括 `input[i]` 的大写形式和调用 `generateSubsequences(str, i + 1)`。请务必首先从输出字符串中删除当前添加的字符。
