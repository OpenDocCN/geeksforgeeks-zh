...
> 原文: [https://www.geeksforgeeks.org/print-matrix-after-multiplying-matrix-elements-n-times/](https://www.geeksforgeeks.org/print-matrix-after-multiplying-matrix-elements-n-times/)

给定一个正方形[矩阵](https://www.geeksforgeeks.org/matrix/) `mat[][]` 和一个整数 `N`，任务是将矩阵 `mat` [乘以](https://www.geeksforgeeks.org/matrix/)自身 `N` 次后[打印矩阵](https://www.geeksforgeeks.org/print-2-d-array-matrix-java/)。
...
**思路:** 思路是使用[矩阵乘法](https://www.geeksforgeeks.org/strassens-matrix-multiplication/)和[单位矩阵](https://en.wikipedia.org/wiki/Identity_matrix)。即 `A = I * A` 或 `A = A * I`，其中 `A` 为 `N * M` 阶的矩阵，`I` 为尺寸 `M * N` 的单位矩阵，其中 `N` 为总行数，`M` 为矩阵中的总列数。

其思路是迭代范围 `[1, N]`，用 `A * I` 更新单位矩阵，这样在计算出 `A^2` 的值后（`A^2 = A * A`），`A^3` 就可以计算为 `A * A^2`，等等，直到 `A^N`。
