...
> 原文：[https://www.geeksforgeeks.org/find-the-point-where-a-function-becomes-negative/](https://www.geeksforgeeks.org/find-the-point-where-a-function-becomes-negative/)

给定一个函数 `int f(无符号 int x)`，该函数将一个`非负整数`x作为输入，并返回一个`整数`作为输出。该函数相对于 x 的值单调递增，即对于每个输入 x，`f(x+1)`的值大于`f(x)`。找到值`n`，其中`f()`首次变为正。因为`f()`是单调递增的，所以`f(n+1)`、`f(n+2)`、…的值必须为正，而`f(n-2)`、`f(n-3)`、…的值必须为负。
...
令 `high` 为 `f()` 首次变为正时 i 的值。
在找到`high`之后，我们可以应用二分搜索法来找到 n 吗？我们现在可以应用二分搜索法，我们可以用`high/2`作为低，用`high`作为二分搜索法的高指数。结果 n 必须介于`high/2`和`high`之间。
寻找`high`的步数为 O(Logn)。所以我们可以在 O(Logn) 时间里找到`high`。二分搜索法在 `high/2` 和 `high` 之间花了多少时间？`high`的值必须小于 2*n。`high/2`和`high`之间的元素数量必须为 O(n)。因此，二分搜索法的时间复杂度为 O(Logn)，总时间复杂度为 2*O(Logn)，也就是 O(Logn)。
