...
[广度优先搜索(BFS)](https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/) 方法讨论如下:
...
[深度优先搜索法(DFS)](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/) 方法讨论如下:

1.  初始化一个数组 `visited[]` 将访问过的顶点标记为真。
...
6.  如果任何递归调用返回真，这意味着我们有可能到达具有值 `K` 的索引，并且我们打印 `"Yes"` 否则打印 `"No"`。
...
4.  在基本情况下，检查 `start` 是否在数组长度范围内，并检查之前是否访问过。
