...
> 原文：[https://www.geeksforgeeks.org/top-three-elements-binary-tree/](https://www.geeksforgeeks.org/top-three-elements-binary-tree/)
...
我们可以简单的取三个变量`first`、`second`、`third`分别存储第一大、第二大、第三大，进行前序遍历，每次都会相应的更新元素。这种方法只需要`O(n)`时间。
