...
> 原文：[https://www.geeksforgeeks.org/check-if-the-sum-of-distinct-digits-of-two-integers-are-equal/](https://www.geeksforgeeks.org/check-if-the-sum-of-distinct-digits-of-two-integers-are-equal/)

给定两个整数 `m` 和 `n`，任务是找出两个数字的不同数字之和并打印 `YES` 如果两个和相等，则打印 `NO`。
...
**逼近：** 求 `m` 和 `n` 的唯一位数之和，分别存入 `sumM` 和 `sumN` 中。如果 `sumM == sumN` 则打印 `YES` 否则打印 `NO`。
