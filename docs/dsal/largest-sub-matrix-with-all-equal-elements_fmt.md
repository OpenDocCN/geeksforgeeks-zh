...
> 原文: [https://www.geeksforgeeks.org/largest-sub-matrix-with-all-equal-elements/](https://www.geeksforgeeks.org/largest-sub-matrix-with-all-equal-elements/)
...
2.  维护一个矩阵 `dp[N][M]`，其中 `dp[i][j]` 代表从 `i` 第 `T13】行开始到最后一行的 `j` 第 `列中出现的连续 `1s` 的数量。通过从下到上遍历每一列，可以很容易地填充这个矩阵。
