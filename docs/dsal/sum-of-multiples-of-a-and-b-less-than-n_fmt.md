...
> 原文: [https://www.geeksforgeeks.org/sum-of-multiples-of-a-and-b-less-than-n/](https://www.geeksforgeeks.org/sum-of-multiples-of-a-and-b-less-than-n/)
...
一种天真的方法是通过 1 到 `N` 进行迭代，找到 `A` 和 `B` 的倍数，并将其相加求和。循环结束时显示总和。

高效进场：由于 `A` 的倍数会形成 AP 系列 `A`、`2a`、`3a`……。和 `B` 形成 AP 系列 `B`，`2b`，`3b`……
将这两个系列的和相加，我们将得到这两个数字的倍数之和，但可能有一些共同的倍数，因此通过减去 `lcm(A，B)` 的倍数，从这两个系列的和中去除重复项。所以，减去 `lcm(A，B)` 的级数。
所以 `A` 和 `B` 小于 `N` 的倍数之和为 `sum(A) + sum(B) - sum(lcm(A，B))`。
