...
> 原文: [https://www.geeksforgeeks.org/partition-a-linked-list-into-k-continuous-groups-with-difference-in-their-sizes-at-most-1/](https://www.geeksforgeeks.org/partition-a-linked-list-into-k-continuous-groups-with-difference-in-their-sizes-at-most-1/)
...
*   初始化[链表](https://www.geeksforgeeks.org/data-structures/linked-list/)的[向量](https://www.geeksforgeeks.org/sorting-vector-of-pairs-in-c-set-1-sort-by-first-and-second/)，表示 `ans`，存储 `K` 组。
...
*   [遍历给定的链表](https://www.geeksforgeeks.org/linked-list-set-1-introduction/)并执行以下步骤:
    ...
    *   如果 `R` 的值为正，则将 `head` 的值更新为 `head->next`。
    *   [迭代一个循环，直到 `X` 非零](https://www.geeksforgeeks.org/range-based-loop-c/)并执行以下步骤:
        *   如果 `last` 节点与 `head` 节点相同，则将 `head` 节点移动到下一个节点。
        *   否则，加入 `last` 节点和 `head` 节点之间的链接，将 `last` 更新为 `head`，并将 `head` 节点移动到下一个节点。
        *   将当前链表作为向量 `ans` 中的 `currHead` 推送。
