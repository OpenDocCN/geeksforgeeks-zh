...
> 原文: [https://www.geeksforgeeks.org/modify-array-by-right-shifting-digits-of-array-elements-to-values-exceeding-k/](https://www.geeksforgeeks.org/modify-array-by-right-shifting-digits-of-array-elements-to-values-exceeding-k/)

给定一个大小为 `N` 的数组 `arr[]` 和一个整数 `K`，任务是通过对数组元素执行右移位操作来使所有数组元素 `> K`。
...
1.  遍历数组。
2.  对每个数组元素 `arr[i]` 的数字执行右移操作。
3.  如果 `arr[i] > k`，更新 `arr[i]` 并继续。
