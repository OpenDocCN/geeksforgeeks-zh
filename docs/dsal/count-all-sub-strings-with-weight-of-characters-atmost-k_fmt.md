...
> 原文: [https://www.geeksforgeeks.org/count-all-sub-strings-with-weight-of-characters-atmost-k/](https://www.geeksforgeeks.org/count-all-sub-strings-with-weight-of-characters-atmost-k/)

给定一个由小英文字母组成的字符串 `P` 和一个由英文字母表中所有字符的权重组成的字符串 `Q`，使得对于所有的 `i`，`0 ≤ Q[i] ≤ 9`。任务是在 `K` 内找到唯一子串的总数和权重。
