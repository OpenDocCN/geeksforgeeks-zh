...
> 原文：[https://www.geeksforgeeks.org/find-the-largest-alphabetic-character-present-in-the-string/](https://www.geeksforgeeks.org/find-the-largest-alphabetic-character-present-in-the-string/)
...
为了解决上述问题，幼稚方法是检查字符串中每个字符是否存在大写或小写字符，也就是说**对于字母 `A`，字符串中应该存在`“A”`和`“a”`。如果存在这样的字母，那么将跟踪该字符，并且仅当当前字符大于先前选择的字符时才更新。这种方法在最坏的情况下需要 `O(n^2)` 时间，可以进一步优化。**

## 高效方法

为了优化上述方法，我们使用了英语字母表中有 26 个字符的事实，我们可以通过维护大小为 26 的数组来跟踪小写和大写字符。当遍历给定字符串时，如果当前字符同时以大写和小写字母出现，我们将在两个数组中标记`为真`。标记了各个数组中的所有字符后，我们将运行一个从 `25` 到 `0` 的循环，并检查两个数组中是否都标记了`“真”`。如果是，那么我们打印该字符的大写字母，否则返回`-1`。
...
```python
# Python3 program to Find the Largest Alphabetic
# Character present in the string of both
# uppercase and lowercase English characters
    ...
    function largestCharacter(str)
    {
        ...
        for (let c = 0; c < arr.length; c++) {
            ...
            if (arr[c] == arr[c].toLowerCase())
                lowercase[arr[c].charCodeAt() - 97] = true;

            if (arr[c] == arr[c].toUpperCase())
                uppercase[arr[c].charCodeAt() - 65] = true;
        }
