...
假设给定图有唯一的最小割，且最小割中有 `C` 条边，边为 `{e1, e2, e3, ..ec}`。当且仅当集合 `{e1, e2, e3, ..ec}` 中没有边时，卡尔格算法才会产生此最小割，即在上述算法的主 `while` 循环中迭代删除。
...
`P[S2' | S1']` is conditional probability that is, a min cut is not chosen in second iteration given that it is not chosen in first iteration
...
`>= [1 - 2/n] * [1 - 2/(n-1)] * [1 - 2/(n-2)] * [1 - 2/(n-3)] *... ... * [1 - 2/(n - (n-4)] * [1 - 2/(n - (n-3)]`
