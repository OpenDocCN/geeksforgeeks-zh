...
> 原文: [https://www.geeksforgeeks.org/how-to-validate-mastercard-number-using-regular-expression/](https://www.geeksforgeeks.org/how-to-validate-mastercard-number-using-regular-expression/)

给定字符串`str`，任务是使用[正则表达式](https://www.geeksforgeeks.org/write-regular-expressions/)检查给定字符串是否为有效的主卡号。
有效的主卡号必须满足以下条件。
...
*   其中:
    ...
    *   `$` 代表字符串的结尾。
*   将给定的字符串与正则表达式匹配。
    在 Java 中，这可以通过使用 `Pattern.matcher()` 来实现；在 C++中，这可以通过使用 `regex_match()` 来实现。
