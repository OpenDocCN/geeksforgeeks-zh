...
> 原文: [https://www.geeksforgeeks.org/number-of-balanced-bracket-expressions-that-can-be-formed-from-a-string/](https://www.geeksforgeeks.org/number-of-balanced-bracket-expressions-that-can-be-formed-from-a-string/)

给定由字符 `(`、`)`、`{`、`}`、`[`、`]` 和 `?` 组成的字符串 `str`。任务是找到当 `?` 可以用任何括号字符替换时形成的平衡括号表达式总数。
下面是一些平衡括号表达式的例子: `{([])}`，`{()}[{}]` 等。
和，不平衡括号表达式: `{[}`，`{()]`，`{()}[)` 等。
...
1.  如果 `n` 为奇数，则结果始终为 `0`，因为不可能有平衡的表达式。
2.  如果 `n` 是偶数，则创建一个 `dp` 数组来存储预计算。
3.  使用以下操作调用递归函数:
    *   如果 `start > end` 那么你返回 `1`。
    *   如果已经计算出 `dp[start][end]`，则返回 `dp[start][end]`。
    *   从 `start + 1` 直到 `end` 运行一个循环，递增 `2` 找到其配对括号或 `?`。
    *   然后将字符串分成两半，通过调用递归函数检查从 `start + 1` 到 `i - 1` 和 `i + 1` 到 `end` 的正确后续括号表达式。
    *   如果 `st[start] = '?'` 和 `st[i] = '?'` 则总共可以形成括号对的 `3` 种组合，从而将递归函数的结果乘以 `3`。
4.  返回 `dp[start][end]`。
