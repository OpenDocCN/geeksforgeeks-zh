...
> 原文: [https://www.geeksforgeeks.org/new-algorithm-to-generate-prime-numbers-from-1-to-nth-number/](https://www.geeksforgeeks.org/new-algorithm-to-generate-prime-numbers-from-1-to-nth-number/)

除了厄拉多塞法生成素数的[筛](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)外，我们还可以实现一个从 `1` 到 `N` 生成素数的新算法。

知道**所有质数≥ 5** 都可以从一个模式中追溯出来可能会很神奇: 让我们试着理解这个系列:
...
> 我们假设两个相同大小的布尔型数组。 先说第一个是 `数组 1` ，它的每个元素都初始化为 `0` 。 第二个是 `数组 2` ，它的每个元素都被初始化为 `1` 。 现在， 在 `数组 1` 中，我们用 `1` 初始化指定的索引，使用公式 `y = (6 * x) - 1` 计算索引值。 并且，在 `数组 2` 中，我们用 `0` 初始化指定的索引，使用上述 4 个等式计算索引值。
>
> 现在，运行从 `0` 到 `N` 的循环，打印数组的索引值， 如果 `数组 1[i] = 1` 和 `数组 2[i] = 1` (`i` 为素数)。
