# 通过用 `arr[j]+|j-i|` 的最小可能值替换每个数组元素来修改数组

> 原文: [https://www.geeksforgeeks.org/modify-array-by-replacing-every-array-element-with-minimum-possible-value-of-arrj-j-i/](https://www.geeksforgeeks.org/modify-array-by-replacing-every-array-element-with-minimum-possible-value-of-arrj-j-i/)

给定大小为 `N` 的数组 `arr[]`，任务是为每个索引找到一个值，使得索引 `i` 处的值为 `arr[j]+|j-i|`，其中 `1 ≤ j ≤ N`，任务是为从 `1` 到 `N` 的每个索引找到最小值。
...
**天真方法:** 想法是使用两个嵌套的 `for` 循环来遍历数组和每个 `i` 索引，找到并打印 `arr[j]+|i-j|` 的最小值。
...
1.  取两个辅助数组 `dp1[]` 和 `dp2[]`，其中 `dp1[]` 存储从左遍历的答案，`dp2[]` 存储从右遍历的答案。
