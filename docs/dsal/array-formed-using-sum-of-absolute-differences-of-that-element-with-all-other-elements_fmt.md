...
> 原文: [https://www.geeksforgeeks.org/array-formed-using-sum-of-absolute-differences-of-that-element-with-all-other-elements/](https://www.geeksforgeeks.org/array-formed-using-sum-of-absolute-differences-of-that-element-with-all-other-elements/)

给定排序后的 `N` 个不同正整数的数组 `arr[]`。任务是生成一个数组，使得新数组中每个索引处的元素是对应元素与给定数组中所有其他元素的绝对差之和。
...
**天真方法:** 想法是为数组 `arr[]` 中的每个元素生成所有可能的对，并将新数组中每个元素的对的绝对差之和相加。在上述步骤之后，打印所有元素的数组。
