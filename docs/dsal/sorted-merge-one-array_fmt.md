...
给定两个排序的数组 `A` 和 `B`，其中 `A` 在末尾有足够大的缓冲区来保存 `B`。按排序顺序将 `B` 合并到 `A` 中。
...
一种方法是通过将较小的元素插入到 `A` 的前面来合并两个数组，但是这种方法的问题是我们必须在每次插入后将每个元素向右移动。因此，我们可以通过比较哪个元素更小来比较哪个元素更大，然后将该元素插入到 `a` 的末尾。下面是上述方法的解决方案。
