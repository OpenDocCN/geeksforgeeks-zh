...
> 原文: [https://www.geeksforgeeks.org/check-if-all-objects-of-type-a-and-b-can-be-placed-on-n-shelves/](https://www.geeksforgeeks.org/check-if-all-objects-of-type-a-and-b-can-be-placed-on-n-shelves/)
...
*   计算放置 `A` 型物品所需的最小物品数量，比如 `needa`。由于，`K` 个 A 型物品最多只能放在一个货架上，因此出现了以下两种情况:
    1.  如果 `A` 可以被 `K` 整除，那么所有的 A 型物品都可以放在 `A / K` 个货架上。
    2.  否则 `A % K` 个物品需要放在 1 个货架上，其余放在 `A / K` 个货架上。因此这种情况下需要 `A / K + 1` 个货架。
*   同样，计算放置 B 型物品所需的最小货架数量，比如 `needb`。
*   如果 `needa + needb` 超过 `N`，则无法分配。否则，是有可能的。
