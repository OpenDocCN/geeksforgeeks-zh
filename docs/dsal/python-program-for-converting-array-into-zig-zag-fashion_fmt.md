...
> 原文：[https://www.geeksforgeeks.org/python-program-for-converting-array-into-zig-zag-fashion/](https://www.geeksforgeeks.org/python-program-for-converting-array-into-zig-zag-fashion/)
...
假设我们当前正在处理 `B` 和 `C`，当前关系是 `‘<’`，因为当前关系是 `‘>’`，即 `A` 必须大于 `B`，所以关系是 `A > B` and `B > C`，我们可以推导出 `A > C`，所以如果我们交换 `B` 和 `C`，那么关系是 `A > C` 和 `C < B`。最后我们得到期望的顺序 `A C B`。
