...
> 原文: [https://www.geeksforgeeks.org/find-m-such-that-gcd-of-m-and-given-number-n-is-maximum/](https://www.geeksforgeeks.org/find-m-such-that-gcd-of-m-and-given-number-n-is-maximum/)
...
最简单的方法是循环遍历 `[1, N-1]` 范围内的所有号码，用 `N` 找到每个号码的 [GCD](https://www.geeksforgeeks.org/basic-and-extended-euclidean-algorithms/)。给定最大 GCD 和 `N` 的数字是要求的结果。
...
为了优化上述途径，我们观察到两个数的 GCD 肯定会是其在 `[1, N-1]` 范围内的除数之一。并且，如果除数最大，GCD 将最大。
因此，想法是找到 `N` 的所有[因子](https://www.geeksforgeeks.org/find-divisors-natural-number-set-1/)，并存储这些因子的最大值，这是所需的结果。
