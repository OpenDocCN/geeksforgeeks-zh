...
> 原文：[https://www.geeksforgeeks.org/count-distinct-substrings-that-contain-some-characters-at-most-k-times/](https://www.geeksforgeeks.org/count-distinct-substrings-that-contain-some-characters-at-most-k-times/)

给定一个整数 `k` 和一个字符串 `str`，任务是计算`不同的`子字符串的数量，使得每个子字符串包含的特定字符不超过 `k` 次。特定字符作为另一个字符串给出。
