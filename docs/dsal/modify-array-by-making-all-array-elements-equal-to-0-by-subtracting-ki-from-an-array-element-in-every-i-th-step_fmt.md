...
给定一个大小为`N`的`arr[]`，任务是在`i`步骤中，通过从数组元素中减去`K^i`来检查是否有可能将所有数组元素转换为`0`。如果可以，则打印“是”。否则，打印“否”。
...
> **解释:**
> 一种可能的操作顺序如下:
> 1.  从`arr[2]( = 3)`中减去`2^0`。此后，数组修改为，`arr[] = {8，0，2，4，32}`。
> 2.  从`arr[2]( = 2)`中减去`2^1`。此后，数组修改为，`arr[] = {8，0，0，4，32}`。
> 3.  从`arr[3]( = 4)`中减去`2^2`。此后，数组修改为，`arr[] = {8，0，0，0，32}`。
> 4.  从`arr[1]( = 8)`中减去`2^3`。此后，数组修改为，`arr[] = {0，0，0，0，32}`。
> 5.  不要从任何数组元素中减去`2^4`。
> 6.  从`arr[4]( = 32)`中减去`2^5`。此后，数组修改为，`arr[] = {0，0，0，0，0}`。
...
*   另外，初始化一个`map<int, int>`，比如`MP`，来存储是否使用了`K`的电源。
*   初始化一个变量，说`X`为`1`，存储`K`的幂的计数。
*   迭代直到`X`小于`INT_MAX`并执行以下步骤:
    *   将`X`的值推入向量。
    *   将`X`乘以`K`。
...
时间复杂度: `O(N * log<sub>K</sub>(INT_MAX))`
辅助空间: `O(log<sub>K</sub>(INT_MAX))`