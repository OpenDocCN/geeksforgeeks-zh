...
> 原文: [https://www.geeksforgeeks.org/how-to-implement-generic-linkedlist-in-java/](https://www.geeksforgeeks.org/how-to-implement-generic-linkedlist-in-java/)
...
*   `add(data)`: 它在链表的末尾添加了一个元素
*   `add(位置, 数据)`: 它将一个元素添加到链表中的任何有效位置
*   `remove(键)`: 它从链表中移除包含键的节点
*   `clear()`: 它清除整个链表
*   `empty()`: 它检查链表是否为空
*   `length()`: 返回链表的长度
