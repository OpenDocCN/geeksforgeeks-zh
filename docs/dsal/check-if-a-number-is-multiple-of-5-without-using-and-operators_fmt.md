...
> 原文：[https://www.geeksforgeeks.org/check-if-a-number-is-multiple-of-5-without-using-and-operators/](https://www.geeksforgeeks.org/check-if-a-number-is-multiple-of-5-without-using-and-operators/)

给定一个正数 `n`，编写一个函数 `isMultipleof5(int n)`，如果 `n` 是 5 的倍数，则返回 `true`，否则返回 `false`。**不允许使用 `%` 和 `/` 运算符**。
