...
> 原文：[https://www.geeksforgeeks.org/sum-manhattan-distances-pairs-points/](https://www.geeksforgeeks.org/sum-manhattan-distances-pairs-points/)

给定 `n` 个整数坐标。任务是找出所有坐标对之间的曼哈顿距离之和。
两个点之间的曼哈顿距离`(x1, y1)`和`(x2, y2)`为：`|x1–x2|+|y1–y2|`
...
2.  假设我们已经计算了任意两点之间的距离之和，直到某一点`x_{i-1}`。`x`小于`x_{i-1}`的所有值，让这个和为 `res`，现在我们必须计算任意两点之间的距离，包括`x_i`，其中 `x_i`大于`x_{i-1}`。为了计算每个点与下一个更大的点 `x_i` 的距离，我们可以将现有的差之和 `res` 与所有小于 `x_i` 的点 `x_k` 的距离相加。因此，任何两点之间的总和现在将等于`res` + `∑(x_i – x_k)`，其中`x_i`是测量差异的当前点，而`x_k`是所有小于`x_i`的点。
3.  因为每次计算`x_i`保持不变，我们可以将求和简化为：
    > `res = res + (x_i – x_1) + (x_i – x_2) + ... + (x_i – x_{i-1})`
    > `res = res + (x_i) * i – (x_1 + x_2 + ... + x_{i-1})`，因为在排序数组中，有 `i` 个元素小于当前索引 `i`。
    > `res = res + (x_i) * i – S_{i-1}`，其中 `S_{i-1}` 是所有先前点的总和，直到索引`i–1`。
4.  对于新的指数 `i`，我们需要加上当前指数`x_i`与之前所有指数`x_k`（`<x_i`）的差值。
5.  如果我们按非递减顺序对所有点进行排序，我们可以很容易地在 O(N) 时间内计算出每对坐标之间沿一个轴的期望距离之和，从左到右处理点，并使用上述方法。
    同样，我们不必担心两个点是否是相等的坐标，在按非递减顺序对点进行排序后，我们说一个点 `x_{i-1}` 小于 `x_i` 如果且仅当它在排序后的数组中出现得更早。
