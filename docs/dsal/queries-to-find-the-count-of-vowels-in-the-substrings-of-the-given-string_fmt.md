...
> 原文: [https://www.geeksforgeeks.org/queries-to-find-the-count-of-vowels-in-the-substrings-of-the-given-string/](https://www.geeksforgeeks.org/queries-to-find-the-count-of-vowels-in-the-substrings-of-the-given-string/)

给定长度为 `N` 和 `Q` 的字符串 `str`，其中每个查询由两个整数 `L` 和 `R` 组成。对于每个查询，任务是找到子串 `str[L…R]` 中元音的计数。
...
对于每个查询，遍历从 `L` 第字符到 `R` 第字符的字符串，并找到元音的计数。
...
创建前缀数组 `pre[]`，其中 `pre[i]` 将存储子串 `str[0…i]` 中的计数元音。现在 `[L, R]` 范围内的元音数可以很容易地在 `O(1)` 中计算为 `pre[R] – pre[L–1]`。
