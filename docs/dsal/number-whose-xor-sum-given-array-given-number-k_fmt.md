...
> 原文: [https://www.geeksforgeeks.org/number-whose-xor-sum-given-array-given-number-k/](https://www.geeksforgeeks.org/number-whose-xor-sum-given-array-given-number-k/)
...
基本思想是使用简单的 XOR 属性，即如果 `X ^ Y = Z`，那么 `X ^ Z = Y`。让我们假设要插入数组中的数字是 `x`，使得 `(a[0]^ a[1]^…^ a[n–1])^ x = k`。因此，要找到 `x`，我们可以使用关系 `(a[0]^ a[1]^…a[n–1])^ k = x`。下面是上述方法的实现。
