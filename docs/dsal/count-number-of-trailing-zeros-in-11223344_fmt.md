...
> 原文: [https://www.geeksforgeeks.org/count-number-of-trailing-zeros-in-11223344/](https://www.geeksforgeeks.org/count-number-of-trailing-zeros-in-11223344/)

给定一个整数`n`，任务是在函数`f(n)=\prod\limits_{i = 1}^{n} i^{i}`中找到尾随零的个数，即`f(n)= 1^1 * 2^2 * 3^3 *……* n^n`。
...
我们知道`5 * 2 = 10`，即一个尾随零是单个`5`和单个`2`相乘的结果。所以，如果我们有`x`数量的`5`和`y`数量的`2`，那么尾随零的数量将是`min(x, y)`。

现在，对于系列中的每一个数字`i`，我们需要统计其因子中的`2`和`5`，表示`x`和`y`。但是`2s`和`5s`的数量将分别为`x * i`和`y * i`，因为在系列中，数字`i`的幂是`i`。统计完整系列中`2s`和`5s`的数量，并打印出其中最少的一个，即所需答案。
