...
> 原文：[https://www.geeksforgeeks.org/random-number-generator-in-arbitrary-probability-distribution-fashion/](https://www.geeksforgeeks.org/random-number-generator-in-arbitrary-probability-distribution-fashion/)
...
`1。`取一个大小为 `n` 的辅助阵（比如 `prefix[]`）。
`2。`用前缀和填充它，使得 `prefix[i]` 表示从 `0` 到 `i` 的数字的和。
`3。`生成 `1` 到 `Sum`（包括两者）之间的随机数（比如 `r`），其中 `Sum` 表示输入频率阵列的总和。
`4。`在 `prefix` 数组中找到步骤#3 生成的随机数 `Ceil` 的索引。让指数为 `indexc`。
...
`freq[]`: `{2, 3, 1}`
`prefix[]`: `{2, 5, 6}`
由于 `prefix` 中的最后一个条目是 `6`，所以 `r` 的所有可能值都是 `[1, 2, 3, 4, 5, 6]`
...
**这是如何工作的？**
任何数字 `input[i]` 的生成次数与其出现频率相同，因为范围内存在整数计数（`prefix[i–1]`, `prefix[i]`）为 `input[i]`。像上面的例子一样，`3` 被生成三次，因为存在 `3` 个整数 `3`、`4` 和 `5`，它们的上限是 `5`。
