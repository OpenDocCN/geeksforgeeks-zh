...
> 原文: [https://www.geeksforgeeks.org/geometry-using-complex-numbers-c-set-2/](https://www.geeksforgeeks.org/geometry-using-complex-numbers-c-set-2/)

在浏览了[之前的](https://www.geeksforgeeks.org/geometry-using-complex-numbers-stdcomplex-in-c/)帖子后，我们知道了什么是复数，以及如何使用它们来模拟笛卡尔平面中的点。现在，我们将了解如何在 C++ 中使用来自 STL 的 `complex` 类。

要使用来自 STL 的 `complex` 类，我们使用 `#include <complex>`。

## 定义点类

我们可以通过 `typedef complex<double> point;` 在节目开始的时候来定义我们的点类。点的 X 和 Y 坐标分别是复数的实部和虚部。要访问我们的 X 和 Y 坐标，我们可以使用 `#define` 来宏 `real()` 和 `imag()` 函数：
...
4.  **OP 与 X 轴的夹角，其中 O 是原点:** `arg(P)`
5.  **P 绕原点的旋转:** `P * polar(r, θ)`
