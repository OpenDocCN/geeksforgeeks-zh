...
> 原文: [https://www.geeksforgeeks.org/print-all-unique-strings-present-in-a-given-array/](https://www.geeksforgeeks.org/print-all-unique-strings-present-in-a-given-array/)

给定一个字符串数组 `arr[]`，任务是打印给定数组中存在的所有唯一字符串。

**示例:**

> **输入:** `arr[] = {"geeks", "for", "geek", "ab", "geek", "for", "code", "karega"}`
> **输出:** `geeks ab code karega`
> **解释:**
> 字符串 `"geeks"` 的出现频率为 1。
> 字符串 `"for"` 的频率为 2。
> 字符串 `"ab"` 的频率为 1。
> 字符串 `"code"` 的频率为 1。
> 字符串 `"karega"` 的频率为 1。
> 因此，需要的输出是 `"geeks ab code karega"`
...
**时间复杂度:** `O(N^2 * M)`，其中 `M` 为字符串的最大长度。
**辅助空间:** `O(1)`

**高效方法:** 上述方法可以使用[字典树（Trie）](https://www.geeksforgeeks.org/trie-insert-and-search/)进行优化。其思想是[遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)，对于数组中的每一个第 `i` 个字符串，检查 `arr[i]` 是否存在于 `Trie` 中。如果发现为真，则将 `arr[i]` 标记为重复数组元素。否则，将 `arr[i]` 标记为唯一数组元素，并将 `arr[i]` 插入到字典树中。按照以下步骤解决问题:
...
*   创建一个具有根节点的 `Trie`，比如 `root`，来存储给定数组中每个字符串的字符。
*   [使用变量 `i` 遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)，对于每个第 `i` 个元素，检查 `arr[i]` 是否存在于 `Trie` 中。如果发现为真，则将 `isUniq[i]` 更新为 `false`。
*   否则，将 `isUniq[i]` 更新为 `true`，并将 `arr[i]` 插入 `Trie`。
*   最后，[使用变量 `i` 遍历 `isUniq[]` 数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)，对于每个第 `i` 个元素，检查 `isUniq[i]` 是否为 `true`。如果发现为真，则打印 `arr[i]`。
