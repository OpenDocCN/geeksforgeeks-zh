...
> 原文：[https://www.geeksforgeeks.org/sum-of-even-values-and-update-queries-on-an-array/](https://www.geeksforgeeks.org/sum-of-even-values-and-update-queries-on-an-array/)

给定整数数组 `arr[]` 和查询数组 `q[]`。
对于第 `i` 次查询，`索引 = q[i][0]` 和 `值 = q[i][1]`。任务是对于每个查询，更新 `arr[index] = arr[index] + 值`，然后返回数组中所有偶数元素的总和。
