...
> 原文: [https://www.geeksforgeeks.org/check-if-a-binary-string-can-be-split-into-disjoint-subsequences-which-are-equal-to-010/](https://www.geeksforgeeks.org/check-if-a-binary-string-can-be-split-into-disjoint-subsequences-which-are-equal-to-010/)
...
这个想法是基于这样的观察：如果下列任何一个条件成立，给定的二进制字符串将不满足要求的条件：
...
1.  初始化一个布尔变量 `res` 设为 `true` 检查字符串 `S` 是否满足给定条件。
...
3.  [使用变量 `i` 在范围 `[0, N-1]` 内遍历字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-c/) `S`
    ...
    2.  否则，用 `1` 递增 `count0` 的值。
    3.  检查 `count1 > count0` 的值，然后将 `res` 更新为 `false`。
4.  检查 `count0` 的值是否不等于 `2 * count1`，然后将 `res` 更新为 `false`。
5.  将 `count0` 和 `count1` 的值重置为 `0`。
