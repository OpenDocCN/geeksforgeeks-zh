...
> 原文：[https://www.geeksforgeeks.org/find-the-maximum-number-of-composite-summands-of-a-number/](https://www.geeksforgeeks.org/find-the-maximum-number-of-composite-summands-of-a-number/)
...
**逼近**：注意最小合成数等于 `4`。所以在一个大数字的分裂中会有很多 `4` 是很符合逻辑的。让我们写出小数字（`1 <= M <= N`）`dp_N` 是 `N` 的分裂中复合和的个数。
...
如果我们的查询 `n` 是小数字，那么我们打印 `dp_n`。否则，让我们找到最小数 `k`，这样 `n–4k` 就是一个小数。然后打印 `k + dp_{n–4k}`。
