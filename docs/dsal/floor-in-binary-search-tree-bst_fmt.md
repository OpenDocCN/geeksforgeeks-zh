...
> 原文：[https://www.geeksforgeeks.org/floor-in-binary-search-tree-bst/](https://www.geeksforgeeks.org/floor-in-binary-search-tree-bst/)
...
一个简单的解决方案是使用（有序或前序或后序）遍历树，并跟踪最近的较小或相同的元素。这个解决方案的时间复杂度是 `O(n)`，其中 `n` 是 BST 中的节点总数。

我们可以高效地在 `O(h)` 时间内找到最近的更小或相同的元素，其中 `h` 是 BST 的高度。
