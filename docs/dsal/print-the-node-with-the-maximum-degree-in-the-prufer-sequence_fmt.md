...
**高效方法：**创建一个大小为 2 的 `degree[]` 数组，大于普鲁弗序列的长度，因为如果 `N` 是节点数，普鲁弗序列的长度是 `N–2`。首先，用 `1` 填充度数数组。迭代普鲁弗序列，增加每个元素在度表中的出现频率。这种方法之所以有效，是因为普鲁弗序列中一个节点的频率比树中的度数少一个。现在迭代度数组，找到最大频率的节点，这就是我们的答案。
