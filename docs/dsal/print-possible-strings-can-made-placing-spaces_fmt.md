# 打印所有可能的字符串，这些字符串可以通过放置空格来制作

> 原文: [https://www.geeksforgeeks.org/print-possible-strings-can-made-placing-spaces/](https://www.geeksforgeeks.org/print-possible-strings-can-made-placing-spaces/)
...
因为间隙的数量是 `n-1`，所以存在总 `2^(n-1)` 图案，每个图案的长度在 `n` 到 `2n-1` 的范围内。因此，整体的复杂性将是 `O(n*(2^n))`.
...
本文由高拉夫·夏尔马供稿。如果你发现任何不正确的地方，或者你想分享更多关于上面讨论的话题的信息，请写评论。