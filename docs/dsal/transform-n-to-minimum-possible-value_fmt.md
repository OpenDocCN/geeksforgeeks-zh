...
> 原文: [https://www.geeksforgeeks.org/transform-n-to-minimum-possible-value/](https://www.geeksforgeeks.org/transform-n-to-minimum-possible-value/)

给定两个数字 `N` 和 `D`。将以下两种操作中的任何一种应用于 `N`：

1.  将 `D` 加到 `N` 上。
2.  将 `N` 改为 `digits(N)`，其中 `digits(N)` 为 `N` 的各位数字之和。

任务是将 `N` 转换为最小可能值。打印 `N` 的最小可能值和给定操作的应用次数（任意顺序）。操作数量必须最少。
...
**重要观察:** 最小值始终是最小值 over：`Dr(N + k*D)` 对于某些非负整数 `k`。

```
Dr(N + k*D) = Dr(Dr(N) + Dr(k*D))          (1)
```

现在，`Dr(k*D) = Dr(Dr(k) * Dr(D))`。`Dr(k)` 的可能值是 `0, 1, 2…9`，由数字 `k=0, 1, 2…9` 给出。

```
Dr(x) = Dr(Sum-of-digits(x))             (2)
```

*   `N` 的最小值等于其数字总和的最小值。如果我们把这个答案简化一次（执行数字和操作），然后加上 `D`，可以得到的最小值不会改变。因此，如果需要先执行一个 reduce 操作，然后执行一个 add 操作，那么我们可以先执行 add 操作，然后执行 reduce 操作，而不会影响我们可以到达的可能根。这从公式 (1) 和 (2) 的组合中显而易见。
*   所以，我们可以先做所有的加法运算，再做所有的数字和运算，达到任何一组运算都可能达到的任意个数。利用上述权利要求，我们可以证明最小可能值是 `Dr(N + k*D)` 的最小值，其中 `0 <= k <= 9`。

*   要找到最小步数，请注意加法和数字和运算的相对顺序确实会影响答案。此外，请注意，数字和函数的下降速度非常快。

*   任意数字 `<= 10^10` 到某个数字 `<= 90`，任意数字 `<= 90` 到某个数字 `<= 18`，以此类推。简而言之，任何数字都可以在 `<= 5` 步内减少到它的数字根。

*   通过这个，我们可以证明最小步长的值永远不能大于 15。这是一个宽松的上限，不是确切的上限。

*   使用暴力递归算法，在每一步分支 2 个不同的方向，一个是 `x = sum-of-digits(x)`，另一个是 `x = x+D`，但只是直到递归深度为 15。就这样，我们在探索了 `2^15` 种不同的方式后停下来。
