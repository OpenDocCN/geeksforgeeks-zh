...
> 原文：[https://www.geeksforgeeks.org/segment-tree-set-3-xor-given-range/](https://www.geeksforgeeks.org/segment-tree-set-3-xor-given-range/)

我们有一个数组 `arr[0...n-1]`。有两种类型的查询：
...
一个简单的解决方案是运行一个从 `l` 到 `r` 的循环，并计算给定范围内元素的异或。要更新一个值，只需执行 `arr[i] = x`。第一个操作需要 `O(n)` 时间，第二个操作需要 `O(1)` 时间。最坏情况下，`q` 个查询的时间复杂度为 `O(n * q)`，这在 `n ~ 10^5` 和 `q ~ 10^5` 时将花费大量时间。因此，该解决方案将超过时间限制。

## 解决方案 2

另一个解决方案是在所有可能的范围内存储异或，但是存在 `O(n^2)` 个可能的范围，因此对于 `n ~ 10^5`，它将超过空间复杂度。因此，在不考虑时间复杂度的情况下，我们可以声明这个解决方案将不起作用。

## 解决方案 3（线段树）

**先决条件**：[线段树](https://www.geeksforgeeks.org/segment-tree-set-1-sum-of-given-range/)

我们构建给定数组的线段树，使得数组元素位于叶子节点，内部节点存储覆盖在它们下面的叶子的异或值。
