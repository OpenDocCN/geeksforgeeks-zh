...
> 原文：[Sum of Sum of All Subsets of a Set Formed by First N Natural Numbers](https://www.geeksforgeeks.org/sum-of-sum-of-all-subsets-of-a-set-formed-by-first-n-natural-numbers/)

给定 `N`，`ff(N) = f(1) + f(2) + …… + f(N)`，其中 `f(k)` 是由前 `k` 个自然数组成的集合的所有子集的[和](https://www.geeksforgeeks.org/sum-subsets-set-formed-first-n-natural-numbers/)。任务是求 `ff(N)` 模 `1000000007`。
...
找到将要形成的序列模式。`f(1)`、`f(2)`、`f(3)` 的值分别为 1、6 和 24。让我们找到 `f(4)`。
