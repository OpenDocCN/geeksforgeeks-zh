...
> 原文: [https://www.geeksforgeeks.org/find-two-integers-a-and-b-such-that-a-n-a-n-and-b-n-b-n/](https://www.geeksforgeeks.org/find-two-integers-a-and-b-such-that-a-n-a-n-and-b-n-b-n/)

给定一个非负整数 `N`，任务是找出两个整数 `a`（小于 `n` 的最大整数）和 `b`（大于 `n` 的最小整数），使得 `A + N = A ^ N` 和 `B + N = B ^ N`。
