...
> 原文: [https://www.geeksforgeeks.org/largest-non-repeating-element/](https://www.geeksforgeeks.org/largest-non-repeating-element/)
...
*   [遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/)，对于每个 `i` 元素，检查数组中 `arr[i]` 的频率是否等于 `1`。如果发现为真，则更新 `LNRElem = max(LNRElem，arr[i])`。
*   最后，打印 `LNRElem` 的值。
