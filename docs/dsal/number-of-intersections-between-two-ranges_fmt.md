...
> 原文: [https://www.geeksforgeeks.org/number-of-intersections-between-two-ranges/](https://www.geeksforgeeks.org/number-of-intersections-between-two-ranges/)

给定类型 1 范围的 `N` 范围和类型 2 的 `M` 范围。任务是找出所有可能的类型 1 和类型 2 范围对之间的交集总数。给出了类型 1 和类型 2 范围的所有起点和终点。
...
> `type2[ ] = { { 2，8}，{9，12 } }`
> **输出:** `3`
> 范围 `{ 2，8 }` 与类型 1 范围 `{ 1，3 }` 相交，而 `{5，9}` 范围 `{ 9，12 }` 仅与 `{ 5，9 }` 相交。
> 所以路口总数为 `3` 个。
> **输入:** `N = 3`，`M = 1`
...
*   思路是用包含-排除法确定路口总数。
*   可能的交叉总数为 `n * m` 。现在减去那些与第一个类型 2 范围不相交的类型 1 范围的计数。
*   那些类型 1 范围将不与第 `i` 个类型 2 范围相交，该范围在第 `i` 个类型 2 范围开始之前结束，在第 `i` 个类型 2 范围结束之后开始。
*   这个计数可以通过使用二分搜索法来确定。C++内置函数 `upper_bound` 可以直接使用。
