...
> 原文: [https://www.geeksforgeeks.org/binary-indexed-tree-range-updates-point-queries/](https://www.geeksforgeeks.org/binary-indexed-tree-range-updates-point-queries/)

给定数组 `arr[0..n-1]`。需要执行以下操作。

1.  **更新(`l`, `r`, `val`)**: 从 `l` 到 `r` 向数组中的所有元素添加 `val`。
2.  **`getElement(i)`**: 查找数组中索引为 `i` 的元素。
...
1.  **更新(`l`, `r`, `val`)**: 从 `l` 到 `r` 迭代子数组，用 `val` 增加所有元素。
...
1.  **更新(`l`, `r`, `val`)**: 将 `val` 添加到 `l`<sup>第</sup>个元素中，并从 (`r+1`)<sup>第</sup>个元素中减去 `val`，对所有更新查询执行此操作。
...
让我们分析一下更新查询。**为什么要向 `l`<sup>第</sup>个索引添加 `val`？** 向 `l`<sup>第</sup>个索引添加 `val` 意味着 `l` 之后的所有元素都增加了 `val`，因为我们将为每个元素计算前缀和。**为什么要从 (`r+1`)<sup>第</sup>个索引减去 `val`？** 需要进行范围 `[l, r]` 的更新，但我们实际更新的是 `[l, n-1]`，因此我们需要从 `r` 之后的所有元素中移除 `val`，即从 (`r+1`)<sup>第</sup>个索引减去 `val`。这样 `val` 就被添加到了范围 `[l, r]`。下面是上述方法的实现。
...
# Update and Point Queries Without using BIT 
