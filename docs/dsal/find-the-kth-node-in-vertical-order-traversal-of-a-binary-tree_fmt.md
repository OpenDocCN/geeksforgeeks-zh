...
> 原文：[https://www.geeksforgeeks.org/find-the-kth-node-in-vertical-order-traversal-of-a-binary-tree/](https://www.geeksforgeeks.org/find-the-kth-node-in-vertical-order-traversal-of-a-binary-tree/)

给定一棵二叉树和一个整数 `k`，任务是打印二叉树的[垂直顺序遍历](https://www.geeksforgeeks.org/print-binary-tree-vertical-order/)中的第 `k` 个节点。如果不存在这样的节点，则打印 `-1`。
二叉树的垂直顺序遍历是指垂直打印。
...
思路是执行[垂直顺序遍历](https://www.geeksforgeeks.org/print-a-binary-tree-in-vertical-order-set-3-using-level-order-traversal/)并检查当前节点是否为第 `k` 个节点然后打印其值，如果树中的节点数小于 `K` 则打印 `-1`。
