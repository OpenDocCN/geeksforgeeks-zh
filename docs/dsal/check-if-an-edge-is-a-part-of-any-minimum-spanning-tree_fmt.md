...
> 原文: [https://www.geeksforgeeks.org/check-if-an-edge-is-a-part-of-any-minimum-spanning-tree/](https://www.geeksforgeeks.org/check-if-an-edge-is-a-part-of-any-minimum-spanning-tree/)

给定一个连接的无向加权图，其形式为 2D 数组，其中每一行都是描述一条边的类型【开始节点，结束节点，权重】，以及两个整数 `(A，B)`。如果 `(A，B)` 之间形成的边是图的任何[最小生成树](https://www.geeksforgeeks.org/properties-of-minimum-spanning-tree-mst/) (MST)的一部分，则返回。
...
> **输入:** `graph = [[0，1，20]，[0，2，5]，[0，3，10 ]，[ 2，3，10]]`，`A = 2`，`B = 3`
> **输出**: `True`
...
[Kruskal 算法](https://www.geeksforgeeks.org/kruskals-minimum-spanning-tree-algorithm-greedy-algo-2/)和[Prim 算法](https://www.geeksforgeeks.org/prims-minimum-spanning-tree-mst-greedy-algo-5/)是两个最常用的算法，可以用来寻找任何图的 MST。在本文中，解决方案基于 Kruskal 算法。按照下面提到的步骤使用这种方法解决问题:

1.  使用 Kruskal 算法，求整个图的最小生成树代价。
2.  由于正在检查 MST 中的边 `(A，B)` 的包含情况，首先将该边包含在最小生成树中，然后包含其他边。
3.  最后检查包括边 `(A，B)` 的生成树和计算出的最小二乘权重的成本是否相同。
4.  如果成本相同，那么边 `(A，B)` 是图的某个 MST 的一部分，否则不是。
