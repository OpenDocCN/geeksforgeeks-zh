...
> 原文: [https://www.geeksforgeeks.org/partition-a-linked-list-into-3-parts-such-that-the-maximum-difference-between-their-sizes-is-minimum/](https://www.geeksforgeeks.org/partition-a-linked-list-into-3-parts-such-that-the-maximum-difference-between-their-sizes-is-minimum/)
...
*   如果给定链表的 `size` 小于 `3`，则创建只有一个节点的 `size` 次[链表](https://www.geeksforgeeks.org/data-structures/linked-list/)和有 `NULL` 节点的 `3–size` 链表，并将其添加到 `ans` 向量中，`return`。
*   初始化一个变量，将 `minSize` 设为 `size / 3`，这将是要划分的[链表](https://www.geeksforgeeks.org/data-structures/linked-list/)的最小大小，将 `rem` 设为 `size % 3`。
*   [迭代链表](https://www.geeksforgeeks.org/iterate-over-a-list-in-python/)直到 `size` 变为 `0` 并执行以下步骤:
    *   在每次迭代中，如果 `rem` 等于 `0`，那么[再次迭代](https://www.geeksforgeeks.org/iterate-over-a-list-in-python/) `minSize` 次到链表中，并将该[链表](https://www.geeksforgeeks.org/data-structures/linked-list/)添加到 `ans` 中，并将 `rem` 递减 `1`。
    *   否则，将 `(minSize + 1)` 次迭代到链表中，并将该链表添加到 `ans` 中。
*   完成以上步骤后，打印向量 `ans[]` 中存储的所有链表。
