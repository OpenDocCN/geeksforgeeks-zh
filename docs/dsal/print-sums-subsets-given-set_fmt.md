...
我们可以递归解决这个问题。总共有 2<sup>n</sup> 个子集。对于每个元素，我们考虑两个选择，我们将它包含在一个子集中，而不将其包含在一个子集中。以下是基于这一思想的递归解决方案。
...
如上所述，总共有 2<sup>n</sup> 个子集。想法是生成从 0 到 2<sup>n</sup>–1 的循环。对于每个数字，选择当前数字的二进制表示中对应于 1 的所有数组元素。
