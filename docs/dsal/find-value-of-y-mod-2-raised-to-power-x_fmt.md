...
给定两个正整数 `x` 和 `y`，我们必须找到 `y mod 2^x` 的值。这是 `y` 除以 `2^x` 的余数。
...
为了解决这个问题，我们可以使用 `pow()` 和模运算符，并且可以很容易地找到余数。
但有几点我们要关心:

*   对于较高的 `x` 值，如 `2^x` 大于 `long long int` 范围，我们不能得到合适的结果。
*   每当 `y < 2^x` 时，余数总是 `y`。因此，在这种情况下，我们可以限制自己计算值 `2^x` 为:
...
*   我们可以在一个变量中存储 `2^x` 的 `2^x` 的最大值是 `2^63`。这意味着对于 `x > 63`，`y` 总是小于 `2^x`，在这种情况下，`y mod 2^x` 的余数就是 `y` 本身。
...
### 注意:
由于 `python` 是极限自由的，我们可以直接使用 `mod` 和 `pow()`函数
