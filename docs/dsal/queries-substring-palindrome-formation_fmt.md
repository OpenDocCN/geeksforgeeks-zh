...
> 原文: [https://www.geeksforgeeks.org/queries-substring-palindrome-formation/](https://www.geeksforgeeks.org/queries-substring-palindrome-formation/)
...
定义一个函数 `getFrequency(i，u)`，返回 `i` 第 `u` 个前缀中‘u’的频率。范围 `L…R` 中字符' u '的频率可以通过 `getFrequency(R，u)–getFrequency(L-1，u)` 找到。
每当更新(查询 1)将字符“u”更改为“v”时。`BIT[u]` 在索引 `I` 处用-1 更新，`BIT[v]` 在索引 `I` 处用+1 更新。
下面是该方法的实现:
