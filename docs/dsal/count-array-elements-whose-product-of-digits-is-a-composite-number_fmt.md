...
> 原文: [https://www.geeksforgeeks.org/count-array-elements-whose-product-of-digits-is-a-composite-number/](https://www.geeksforgeeks.org/count-array-elements-whose-product-of-digits-is-a-composite-number/)

给定一个由 `N` 个非负整数组成的数组 `arr[]`，任务是计算数组元素的数量，这些元素的个位数的乘积是一个复合数。
...
*   使用厄拉多塞的筛预计算并存储所有质数。
*   创建一个集合来存储所有不同的数组元素，它们的数字乘积是一个复合数字。
*   遍历给定的数组，对于每个数组元素，检查其所有数字的乘积是否为素数。如果发现为真，将当前元素添加到集合中。
*   否则，继续下一个数组元素。
*   遍历完毕后，打印集合的尺寸作为所需答案。
