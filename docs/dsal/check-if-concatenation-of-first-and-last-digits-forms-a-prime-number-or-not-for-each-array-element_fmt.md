...
> 原文: [https://www.geeksforgeeks.org/check-if-concatenation-of-first-and-last-digits-forms-a-prime-number-or-not-for-each-array-element/](https://www.geeksforgeeks.org/check-if-concatenation-of-first-and-last-digits-forms-a-prime-number-or-not-for-each-array-element/)

给定一个由 `N` 个整数组成的数组 `Q[]`，数组 `Q[]` 每个元素的任务是检查通过连接 `Q[i]` 的第一个和最后一个数字形成的任何数字是否是质数。
...
使用厄拉多塞的筛子可以高效解决问题。按照以下步骤解决给定问题:

1.  由于通过组合一对数字可能得到的最高数字是 99，因此使用厄拉多塞的筛预先计算并存储所有质数到 99，并将其存储在布尔数组中，比如 `prime[]`，其中 `prime[i] = 0` (非质数) 和 `1` (质数)。
2.  遍历数组 `Q[]`，并执行以下步骤:
    *   通过执行 `Q[i] % 10` 提取 `Q[i]` 的最后一位数字并存储在变量中，比如说 `last`。
    *   通过将 `Q[i]` 连续除以 `10` 提取 `Q[i]` 的第一个数字，直到 `Q[i]` 减少到小于 `10` 并将其存储在一个变量中，比如说 `first`。
    *   现在，生成两种可能的组合:
        *   `first * 10 + last`。
        *   `last * 10 + first`。
    *   对于以上两种组合中的每一种，检查它们中是否有质数。
