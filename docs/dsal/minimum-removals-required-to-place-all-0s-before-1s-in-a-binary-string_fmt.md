...
> 原文: [https://www.geeksforgeeks.org/minimum-removals-required-to-place-all-0s-before-1s-in-a-binary-string/](https://www.geeksforgeeks.org/minimum-removals-required-to-place-all-0s-before-1s-in-a-binary-string/)

给定一个[二进制字符串](https://www.geeksforgeeks.org/tag/binary-string/) `S`，任务是找到需要从 `S` 中删除的最小字符数，这样所有的 `0` 都被放在 `1` 之前。
...
解决问题的方法是：从右边找出最少需要删除的 `'0'` 字符数，记为 `right_0`，和最少需要删除的 `'1'` 字符数，记为 `left_1`，以得到需要的字符串。任一指标得到的 `right_0` 和 `left_1` 的最小和即为最终结果。

按照以下步骤解决给定的问题：

1.  初始化两个计数器变量，比如说 `right_0` 和 `left_1`。
2.  将字符串 `S` 中 `'0'` 的计数存储在 `right_0` 中，并将 `left_1` 设置为等于 `0`。
3.  初始化一个变量，记为 `res`，来存储需要的答案。
4.  [遍历字符串](https://www.geeksforgeeks.org/iterate-over-characters-of-a-string-in-c/) `S` 的字符，对于每个字符：
    *   检查 `S[i]` 是否等于 `'0'`。
        *   如果发现为真，则用 `1` 减少 `right_0`。
        *   否则，用 `1` 增加 `left_1`。
    *   检查 `right_0` 和 `left_1` 之和是否小于 `res`。如果发现为真，则更新 `res` 至最小 `res` 和 `right_0 + left_1`。
