...
> 原文：[https://www.geeksforgeeks.org/range-queries-for-finding-the-sum-of-all-even-parity-numbers/](https://www.geeksforgeeks.org/range-queries-for-finding-the-sum-of-all-even-parity-numbers/)

给定 `Q` 个查询，其中每个查询包含两个数字 `L` 和 `R`，表示一个范围 `[L, R]`。任务是找出位于给定范围 `[L, R]` 内的所有[偶校验数](https://www.geeksforgeeks.org/finding-the-parity-of-a-number-efficiently/)的和。
...
3.  对于 `Q` 个查询，范围 `[L, R]` 的所有偶数奇偶数之和可以计算如下：
