...
> 原文: [https://www.geeksforgeeks.org/smallest-integer-greater-than-n-such-that-it-consists-of-digit-m-exactly-k-times/](https://www.geeksforgeeks.org/smallest-integer-greater-than-n-such-that-it-consists-of-digit-m-exactly-k-times/)

给定三个整数 `n`、`m` 和 `k`，任务是找到最小的整数 `> n`，使得数字 `m` 在其中恰好出现 `k` 次。
...
**方法:** 从 `n + 1` 开始迭代，对于每个整数 `i` 检查它是否由数字 `m` 精确地构成 `k` 次。这样就可以找到最小整数 `> n` 与数字 `m` 正好出现 `k` 次。
