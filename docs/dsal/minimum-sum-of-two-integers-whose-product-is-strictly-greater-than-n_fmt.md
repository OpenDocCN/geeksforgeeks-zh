...
> 原文：[https://www.geeksforgeeks.org/minimum-sum-of-two-integers-whose-product-is-strictly-greater-than-n/](https://www.geeksforgeeks.org/minimum-sum-of-two-integers-whose-product-is-strictly-greater-than-n/)
...
让要求的数字为 `A` 和 `B`。这个想法是基于这样的观察：为了最小化它们的和，`A` 应该是大于 `√N` 的最小数。一旦找到 `A`，则 `B` 将等于满足 `A×B > N` 的最小数，可以线性地找到。
...
以上解决方案可以通过[二分搜索法](https://www.geeksforgeeks.org/binary-search/)找到 `A` 和 `B` 进行优化。按照以下步骤解决问题：

1.  初始化两个变量 `low = 0` 和 `high = 10^9`。
2.  迭代直到 `(high – low)` 大于 1，并执行以下操作：
    - 求中档 `mid` 的值为 `(low + high) / 2`。
    - 现在将 `√N` 与中间元素 `mid` 进行比较，如果 `√N` 小于等于 `mid`，则 `high` 为 `mid`。
    - 否则，将 `low` 更新为 `mid`。
3.  完成上述所有步骤后，设置 `A = high`。
4.  重复同样的程序找到 `B`，这样 `A×B > N`。
5.  完成上述步骤后，打印 `A` 和 `B` 的和作为结果。
