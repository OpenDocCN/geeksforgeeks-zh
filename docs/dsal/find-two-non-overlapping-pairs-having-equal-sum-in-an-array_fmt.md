...
> 原文：[https://www.geeksforgeeks.org/find-two-non-overlapping-pairs-having-equal-sum-in-an-array/](https://www.geeksforgeeks.org/find-two-non-overlapping-pairs-having-equal-sum-in-an-array/)

给定一个未排序的整数数组。任务是找到任意两个和相等的非重叠对。
如果两对的所有元素都在不同的索引处，则称这两对是不重叠的。即配对 `(A_i, A_j)` 和配对 `(A_m, A_n)` 如果 `i ≠ j ≠ m ≠ n` 则称不重叠。
