...
给定一个无向图、连通图和加权图，使用克鲁斯卡尔算法找到图的最小生成树（MST）。
...
我们已经在下面讨论了Kruskal的MST实现。

[贪婪算法|集合2(克鲁斯卡尔最小生成树算法)](https://www.geeksforgeeks.org/greedy-algorithms-set-2-kruskals-minimum-spanning-tree-mst/)

以下是使用克鲁斯卡尔算法寻找最小生成树的步骤：
...
这里有一些关键点，将有助于我们使用STL实现克鲁斯卡尔算法。

### 使用边的向量

使用一个包含图中所有边的向量，向量的每个元素将包含3个参数：源点、目的地以及源点和目的地之间边的代价。

```cpp
vector<pair<int, pair<int, int> > > edges;
```

这里，在外部对（即`pair<int,>`）中，第一个元素对应于边的成本，而第二个元素本身是一对，并且它包含边的两个顶点。

### 使用标准排序

使用内置的`std::sort`以非递减顺序对边进行排序；默认情况下，排序函数按非递减顺序排序。

### 使用并查集算法

我们使用[并查集算法](https://www.geeksforgeeks.org/union-find-algorithm-set-2-union-by-rank/)来检查当前边是否形成一个循环，如果它被添加到当前的MST中。如果是，丢弃它，否则包括它（联合）。
...
上述代码可以进行优化，当选择的边数变为`V-1`时，停止Kruskal的主循环。我们知道MST有`V-1`条边，选择`V-1`条边后没有必要迭代。我们没有添加这种优化来保持代码简单。
