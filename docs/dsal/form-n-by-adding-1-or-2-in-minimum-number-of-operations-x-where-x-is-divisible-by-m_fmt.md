...
> 原文: [https://www.geeksforgeeks.org/form-n-by-adding-1-or-2-in-minimum-number-of-operations-x-where-x-is-divisible-by-m/](https://www.geeksforgeeks.org/form-n-by-adding-1-or-2-in-minimum-number-of-operations-x-where-x-is-divisible-by-m/)
...
*   所以我们必须数 `n/2` 步，才能得到最少的步数。如果 `n` 是偶数，那么最小步数将是 `n/2`，但是如果是奇数，那么它将是 `n/2+1`，而不考虑步数可被 `m` 整除。为了使最小步数为 `m` 的倍数，我们可以做 `floor((minimum_steps+m–1)/m)* m`。
