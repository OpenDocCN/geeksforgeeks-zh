...
> 原文: [https://www.geeksforgeeks.org/ropes-data-structure-fast-string-concatenation/](https://www.geeksforgeeks.org/ropes-data-structure-fast-string-concatenation/)

对字符串最常见的操作之一是追加或连接。当字符串以传统方式(即字符数组)存储时，追加到字符串末尾将花费最少 `O(n)` 的时间(其中 `n` 是原始字符串的长度)。
我们可以通过使用绳索数据结构来减少花费的时间。
...
我们创建一个字符串 `c[]` 来存储串联的字符串。我们首先遍历 `a[]` 并将 `a[]` 的所有字符复制到 `c[]`。然后我们将 `b[]` 的所有字符复制到 `c[]`。
...
考虑下图(图片来源: [https://en.wikipedia.org/wiki/Rope_(data_structure)](https://en.wikipedia.org/wiki/Rope_(data_structure)))
...
本文由**阿基尔·戈尔**供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用`write.geeksforgeeks.org`写一篇文章或者把你的文章邮寄到`review-team@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
