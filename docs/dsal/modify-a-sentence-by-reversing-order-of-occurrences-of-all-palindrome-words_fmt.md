...
> 原文：[https://www.geeksforgeeks.org/modify-a-sentence-by-reversing-order-of-occurrences-of-all-palindrome-words/](https://www.geeksforgeeks.org/modify-a-sentence-by-reversing-order-of-occurrences-of-all-palindrome-words/)
...
*   迭代字符串 `S` 的字符，并使用 `split()` 从句子中拆分空格分隔的单词，并将它们存储在列表中，例如 `lis`。
*   使用 `append()` 功能，将所有回文单词追加到新列表中，比如说 `newlist`。
*   使用 `reverse()` 功能反向此新列表。
*   遍历列表 `lis`，继续用 `newlist[i]`（初始，`i=0`）替换回文词，并将 `i` 增加 `1`。
*   打印修改后的句子。
