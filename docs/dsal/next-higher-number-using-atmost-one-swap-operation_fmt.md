...
> 原文: [https://www.geeksforgeeks.org/next-higher-number-using-atmost-one-swap-operation/](https://www.geeksforgeeks.org/next-higher-number-using-atmost-one-swap-operation/)

给定一个非负数 `num`。问题是通过在 `num` 中任意两位数字之间的交换操作上执行 atmost 一次，找到大于 `num` 的最小数字。如果无法形成更大的数字，则打印“不可能”。
...
**逼近:**
首先找到最右边的数字的索引，这个数字比它左边的数字大，并且在它的右边存在比它更大的数字。让它的索引为 `ind`。现在，在索引 `ind` 右边找到比该数字大的最小数字的索引，并交换它们。让它的索引为 `greatSmallDgt`。最后交换索引 `ind` 和 `greatSmallDgt` 处的数字。如果 `num` 的数字是按降序排列的，则打印“不可能”。
