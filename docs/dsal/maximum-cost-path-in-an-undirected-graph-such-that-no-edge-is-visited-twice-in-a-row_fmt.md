...
给定一个具有`N`个顶点和`M`条边的无向图，并且每个顶点与一个成本相关联，并给出源顶点`S`。 任务是从源顶点`S`找到最大成本路径，以使没有边连续 2 次或更多次被访问。
...
*   `best`：存储最大成本叶子节点及其子节点（如果存在）的成本。
*   `check`：布尔变量用作在图形中查找循环的标志，找到循环后其值将更改为`0`。

步骤如下：

1.  执行 [DFS 遍历](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/)，并将标志变量`check`设置为`'1'`，最初表示未找到循环。
2.  同时为每个节点构建 `dp[]` ，并更新最大成本，直到遍历该节点为止。
3.  如果发现相邻节点已经访问过并且不是父节点，则找到循环并将 `check` 的值设置为`0`。
4.  将循环的所有节点的成本添加到 `canTake` 中。
5.  遍历遍历遍历节点的相邻节点后，未找到任何回路，则它表示从回路到叶顶点的路径成本，并且如果`dp[i]` 大于`best`。
6.  遍历图形后，将`canTake`和`best`的和打印出来。
