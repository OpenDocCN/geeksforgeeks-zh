# 检查是否可以从原点到达给定圆周上的任意点

> 原文: [https://www.geeksforgeeks.org/check-if-it-is-possible-to-reach-any-point-on-the-circumference-of-a-given-circle-from-origin/](https://www.geeksforgeeks.org/check-if-it-is-possible-to-reach-any-point-on-the-circumference-of-a-given-circle-from-origin/)

给定一个代表一系列动作的字符串 `S`（`L`、`R`、`U` 和 `D`）和一个代表圆心为原点 `(0,0)` 的圆的半径的整数 `R`，任务是检查是否有可能到达给定圆周上的任何一点。如果有可能到达圆周上的一个点，则打印 `"Yes"`。否则，打印 `"No"`。

每个方向需要执行的操作如下：

*   如果当前字符为 `L`，则 `x` 坐标减量 `1`。
*   如果当前字符为 `R`，则 `x` 坐标增量 `1`。
*   如果当前字符为 `U`，则 `y` 坐标增量 `1`。
*   如果当前字符为 `D`，则 `y` 坐标减量 `1`。
...
**简单方法:** 最简单的方法是生成给定字符串 `S` 的所有可能的子序列，如果存在任何导致从原点 `(0,0)` 到达圆的圆周的子序列，则打印 `"Yes"`。否则，打印 `"No"`。
...
*   如果存在任意两个值 `X` 和 `Y` 使得 `X` 和 `Y` 的平方和为 `R^2`，则从原点到圆的圆周存在三角形路径。

**按照以下步骤解决给定的问题:**

*   将给定字符串 `S` 中的字符数 `L`、`R`、`U` 和 `D` 存储在一个变量中，分别表示 `cntL`、`cntR`、`cntU` 和 `cntD`。
*   如果 `cntL`、`cntR`、`cntU`、`cntD` 的最大值至少为 `R`，则存在从原点到圆周的直线路径。因此，打印 `"Yes"`。
*   如果 `cntL` 和 `cntR` 的最大值与 `cntU` 和 `cntD` 的最大值的平方至少为 `R^2`，则打印 `"Yes"`，因为从原点到圆周存在三角形路径。
*   如果没有出现上述情况，则打印 `"No"`。
