...
> 原文: [https://www.geeksforgeeks.org/queries-to-update-array-by-adding-or-multiplying-array-elements-and-print-the-element-present-at-specified-index/](https://www.geeksforgeeks.org/queries-to-update-array-by-adding-or-multiplying-array-elements-and-print-the-element-present-at-specified-index/)
...
*   假设 `Mul` 是类型 `1` 查询中所有整数的乘积，`Add` 是类型 `0` 和类型 `1` 的所有查询的值，按照给定的顺序执行，从 `0` 开始。然后从上面可以观察到任何数组元素 `arr[i]` 修改为 `(arr[i] * Mul + Add)`。

按照以下步骤解决问题:
*   初始化两个变量，比如说 `Mul` 为 `1` 和 `Add` 为 `0`，存储类型 `1` 查询中所有整数的乘积，并在一个数字 `0` 上按照给定的顺序存储执行类型 `0` 和类型 `1` 查询后得到的值。
*   [遍历给定的查询数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `Q[][2]`，并执行以下步骤:
    *   如果 `Q[i][0]` 是 `0`，那么将 `Add` 的值增加到 `Q[i][1]`。
    *   否则，如果 `Q[i][0]` 的值为 `1`，则将 `Mul` 和 `Add` 乘以 `Q[i][1]`。
    *   否则，打印值 `(arr[Q[i][1]] * Mul + Add)` 作为类型 `2` 查询的结果。
