...
*   使用 [DFS](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/) 来[遍历图形，找到图形中的所有路径](https://www.geeksforgeeks.org/find-paths-given-source-destination/)。
...
*   执行 [DFS](https://www.geeksforgeeks.org/depth-first-traversal-for-a-graph/) 时，对于每个顶点 `i`
    ...
    *   将位集 `b[i]` 中的主导顶点集存储为其父顶点集的[交集](https://www.geeksforgeeks.org/intersection-function-python/)。将 `b[i]` 更新为 `b[i] & b[parent]`。
