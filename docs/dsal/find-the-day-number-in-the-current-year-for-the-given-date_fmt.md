...
> 原文: [https://www.geeksforgeeks.org/find-the-day-number-in-the-current-year-for-the-given-date/](https://www.geeksforgeeks.org/find-the-day-number-in-the-current-year-for-the-given-date/)

给定一个表示日期格式为 `YYYY-MM-DD` 的字符串 `str`，任务是查找当前年份的日号。比如 `1st` 1 月是一年中的 `1st` 日，`2nd` 1 月是一年中的 `2nd` 日，`1st` 2 月是一年中的 `32nd` 日等等。
...
*   从给定日期中提取年、月和日，并将其存储在变量 `year`、`month` 和 `day` 中。
*   创建一个数组 `days[]`，其中 `days[i]` 将存储 `i` 月的天数。
*   更新 `count = days[0] + days[1] + … + days[month–1]` 获得前几个月所有过去天数的计数。
*   如果给定的年份是闰年，则将该计数增加 `1`，以便计算 `29th` 2 月。
*   最后，将 `day` 加到当前月的日数上，打印出最终的 `dayNumber`。
