...
> 原文: [https://www.geeksforgeeks.org/queries-for-greatest-pair-sum-in-the-given-index-range-using-segment-tree/](https://www.geeksforgeeks.org/queries-for-greatest-pair-sum-in-the-given-index-range-using-segment-tree/)

给定一个包含 `N` 个整数的数组 `arr[]` 和一个代表范围 `[L，R]` 的数组 `Q[]`，任务是找到范围 `[L，R]` 中最大的对和值，其中 `0≤L≤R≤N–1`。
...
## 天真法:
这个问题的天真法是从 `[L，R]` 开始运行一个循环，找出给定范围内最大的两个元素。它们的和总是给定指数范围内最大的对和。这种方法的时间复杂度是每个查询的 `O(N)`。

## 高效方法:
想法是使用 [**段树**](https://www.geeksforgeeks.org/segment-tree-set-1-sum-of-given-range/ "Segment Tree") 来执行一些预处理，并在对数时间内找到每个查询的值。
...
>
> maximum value -> maximum value (left child maximum value, right child maximum value)
