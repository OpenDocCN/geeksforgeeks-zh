...
> 原文: [https://www.geeksforgeeks.org/queries-for-the-minimum-element-in-an-array-excluding-the-given-index-range/](https://www.geeksforgeeks.org/queries-for-the-minimum-element-in-an-array-excluding-the-given-index-range/)

给定一个由 `N` 个整数和 `Q` 个查询组成的数组 `arr[]`，其中每个查询由一个索引范围 `[L, R]` 组成。对于每个查询，任务是找到数组中的最小元素，不包括给定索引范围内的元素。
...
在多个查询的情况下，可以构建[段树](https://www.geeksforgeeks.org/segment-tree-set-1-range-minimum-query/)来找到任何索引范围内的最小元素。现在，对于每个查询 `[L, R]`，不包括该范围的最小元素将是 `min(min(arr[0…L-1]), min(arr[R+1…N-1]))`。
