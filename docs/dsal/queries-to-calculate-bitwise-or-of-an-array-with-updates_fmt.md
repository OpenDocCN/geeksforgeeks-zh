...
> 原文: [https://www.geeksforgeeks.org/queries-to-calculate-bitwise-or-of-an-array-with-updates/](https://www.geeksforgeeks.org/queries-to-calculate-bitwise-or-of-an-array-with-updates/)

给定一个由 `N` 个正整数组成的数组 `arr[]` 和一个由形式为 `{i, val}` 的查询组成的二维数组 `Q[][]`，每个查询的任务是用 `val` 替换 `arr[i]`，并计算修改后数组的按位或。
...
解决问题最简单的方法是通过 `Q[i][1]` 更新 `arr[Q[i][0]]` 后，针对每一个 `i` 查询遍历数组 `arr[]` 来计算数组的按位或。
...
1.  初始化大小为 32 的数组 `result[]`。将其所有元素设置为 0。
2.  遍历数组 `arr[]`。
    *   迭代每个数组元素的位。
    *   对于在当前数组元素中找到的每个第 `j` 个未设置位，递增 `result[j]`。

    ![](img/48e1880d43b8fb5bc4e556ef8046d695.png)

3.  现在，遍历数组 `Q[][]` 并执行以下操作:
    *   通过从各自的位置移除 `Q[i][0]` 的设定位来修改 `result[]`。
    *   通过从各自的位置添加 `Q[i][0]` 的设置位来更新 `result[]`。
    *   将 `result[]` 数组转换为其等价的十进制值并打印出来。
