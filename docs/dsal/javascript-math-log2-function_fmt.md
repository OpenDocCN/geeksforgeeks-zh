...
> 原文：[https://www.geeksforgeeks.org/javascript-math-log2-function/](https://www.geeksforgeeks.org/javascript-math-log2-function/)
...
该函数接受单个参数 `p`，该参数是要计算的以 2 为底的对数的任何数字。
...
该函数的参数应该始终是一个数字，否则它会返回 `NaN`，即当其参数被视为字符串时，它不是一个数字。
...
每当我们需要任何数的基数为 2 的对数的值时，我们就借助这个函数。它的价值在数学题中需要很多次。
...
JavaScript `Math.log2()`函数支持的浏览器如下：
