...
> 原文：[https://www.geeksforgeeks.org/sum-absolute-differences-pairs-given-array/](https://www.geeksforgeeks.org/sum-absolute-differences-pairs-given-array/)
...
一个`这个问题的简单解决方法`就是一个一个的找每一对，取其差，一起总结。这种方法的时间复杂度为`O(n^2)`。
...
每个元素的广义表达式为`sum = sum + (i * arr[i]) - (n-1-i) * arr[i]`。
...
**如果数组没有排序怎么办？**
对于数组未排序的情况，高效的解决方案也更好。我们可以先在`O(n Log n)`时间内对数组进行排序，然后在`O(n)`中找到需要的值。所以整体时间复杂度为`O(n Log n)`仍然优于`O(n^2)`。
