...
> 原文：[https://www.geeksforgeeks.org/number-of-ways-to-color-n-k-blocks-using-given-operation/](https://www.geeksforgeeks.org/number-of-ways-to-color-n-k-blocks-using-given-operation/)
...
// Recursive function to count the ways
function countWays(colored, count, n) {
    ...
    for (let i = 1; i < n + 1; i++) {
        ...
        if (colored[i] == 0) {
            ...
            // Check if adjacent blocks
            // are colored or not
            if (colored[i - 1] == 1 || colored[i + 1] == 1) {
                ...
                // recursively iterate for
                // next uncolored block
                answer = (answer + countWays(colored, count + 1, n)) % mod;
...
2.  给每组未着色的块着色是相互独立的。假设一段有 `x` 块，另一段有 `y` 块。当两个部分合并时，求总组合由下式给出:
    > 总组合 = `C(n, x) * 2^{x-1} * 2^{y-1}`
