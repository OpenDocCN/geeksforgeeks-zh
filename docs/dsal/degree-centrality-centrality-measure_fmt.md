...
在图论中，图的顶点的度（或价）是入射到该顶点的边数，循环数为 2。[1] 顶点的度`v`表示为`deg(v)`或`deg v`。 以`Δ(G)`表示的图 G 的最大度和以`δ(G)`表示的图的最小度是其顶点的最大和最小度。 在右边的图中，最大次数是 5，最小次数是 0。在规则图中，所有次数都是相同的，因此我们可以说图的次数。
...
对于具有`|V|`顶点和`|E|`边的给定图`G:=(V,E)`，顶点`v`的度中心定义为

`C_{D}(v)=\deg(v)`

对于图中所有节点的度中心度，在图的密集邻接矩阵表示中采用`Θ(V^2)`，对于边，在稀疏矩阵表示中采用`Θ(E)`。

节点级别的中心性定义可以扩展到整个图，在这种情况下，我们所说的是图中心化。 令`v*`为`G`中具有最高度中心度的节点。 令`X:=(Y,Z)`为使以下数量最大化的`|Y|`节点连接图（其中`y*`为`X`中度中心度最高的节点）：
...
当图形`X`包含一个所有其他节点都连接到的中心节点时（星形图），`H`的值将最大化。
