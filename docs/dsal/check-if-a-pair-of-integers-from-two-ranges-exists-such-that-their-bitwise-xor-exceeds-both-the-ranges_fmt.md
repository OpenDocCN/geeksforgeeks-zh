...
> 原文：[https://www.geeksforgeeks.org/check-if-a-pair-of-integers-from-two-ranges-exists-such-that-their-bitwise-xor-exceeds-both-the-ranges/](https://www.geeksforgeeks.org/check-if-a-pair-of-integers-from-two-ranges-exists-such-that-their-bitwise-xor-exceeds-both-the-ranges/)

给定两个整数 `A` 和 `B`，任务是检查在范围 `[1, A]` 和 `[1, B]` 内是否分别存在两个整数 `P` 和 `Q`，使得它们的按位异或大于 `A` 和 `B` 的按位异或。如果发现是真的，那么打印“是”。否则，打印“否”。
...
**天真方法：** 解决给定问题的最简单方法是通过遍历从 `1` 到 `X` 和 `1` 到 `Y` 的所有整数来生成 `(P, Q)` 的所有可能的对，并检查是否存在这样的对：它们的按位异或大于 `X` 和 `Y` 的按位异或。如果发现为真，则打印“是”。否则，打印“否”。
...
*   对于任意两个整数 `P` 和 `Q`，最大按位异或值为 `(P + Q)`，只有当 `P` 和 `Q` 在其二进制表示中没有公共位时才能找到。
*   有两种情况：
