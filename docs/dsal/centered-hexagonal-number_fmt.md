...
给定一个数字 `N`，任务是找到第 `N` 个居中的六边形数字。另外，找到居中的六边形系列。
...
正如中心六边形数字加起来的第 `N` 项将是第 `N^3`。那就是 –
> `1^3 + 2^3 + 3^3 + 4^3 + 5^3 + 6^3...` 最多 `N` 个术语 = `N^3`
> 然后，第 `N` 个术语将为 –
> => `N^3 - (N - 1)^3`
> => `3 * N * (N - 1) + 1`

**方法:** 使用公式 – `3 * N * (N - 1) + 1` 求中心六边形数的第 `N` 项。
以下是上述方法的实现:
...
*   **时间复杂度:** 在上面给出的方法中，我们找到了花费恒定时间的中心六边形数的第 `N` 项。因此，复杂性将是 `O(1)`。
...
**方法:**
使用循环变量 (比如说 `i`) 迭代循环，并使用公式 – `3 * i * (i - 1) + 1` 找到中心六边形数字的每个第 `i` 项。下面是上述方法的实现:
