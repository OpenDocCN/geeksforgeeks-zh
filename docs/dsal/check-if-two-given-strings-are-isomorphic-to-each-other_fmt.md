...
> 原文: [https://www.geeksforgeeks.org/check-if-two-given-strings-are-isomorphic-to-each-other/](https://www.geeksforgeeks.org/check-if-two-given-strings-are-isomorphic-to-each-other/)
...
一个简单的解决方案是考虑 `str1` 的每个字符，并检查它的所有出现是否映射到 `str2` 中的相同字符。这个解的时间复杂度是 `O(n*n)`。

一个高效解可以在 `O(n)` 时间内解决这个问题。其思想是创建一个数组来存储已处理字符的映射。
