...
> 原文：`https://www.geeksforgeeks.org/generate-a-string-whose-all-k-size-substrings-can-be-concatenated-to-form-the-given-string/`
...
2.  因此，遍历字符串并将每个子字符串的第一个字符附加到 `ans` 上，然后忽略下一个 `K-1` 字符。
3.  对除最后一个子字符串以外的所有子字符串重复此过程。
4.  将最后一个子串的所有字符追加到 `ans` 中。
5.  返回 `ans` 作为所需的解码字符串。
...
function decode_String(str, K)
{
    ...
    // Consider all characters
    // from the last substring
    for(let i = str.length - (K - 1); i < str.length; i++)
        ans += str[i];
