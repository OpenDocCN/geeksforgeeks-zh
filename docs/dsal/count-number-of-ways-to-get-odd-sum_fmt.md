...
> 原文: [https://www.geeksforgeeks.org/count-number-of-ways-to-get-odd-sum/](https://www.geeksforgeeks.org/count-number-of-ways-to-get-odd-sum/)
...
*   我们将此应用于存储 `DP` 数组中的计数。
*   获得第 `i` 对的偶数和的方法是 `dp[i-1][0] * cnt[i][0] + dp[i-1][1] * cnt[i][1]`。
*   获得第 `i` 对奇数和的方法是 `dp[i-1][1] * cnt[i][0] + dp[i-1][0] * cnt[i][1]`。
