...
给定一个大小为 `N + 1` 的只读[数组](https://www.geeksforgeeks.org/introduction-to-arrays/) `arr[]`，请找到该数组中多个重复元素之一，其中该数组仅包含介于 `1` 和 `N` 的整数。
...
在[先前的文章](https://www.geeksforgeeks.org/find-one-multiple-repeating-elements-read-array/)中，我们讨论了同一篇文章，但篇幅复杂度为 `O(N)` 和 `O(sqrt(N))`。

**方法**：此方法基于 [Floyd 的龟兔赛跑算法](//www.geeksforgeeks.org/detect-loop-in-a-linked-list/)（**循环检测算法**）。
...
4.  循环出现是因为 `arr[]` 包含重复元素（至少一个）。重复值是循环的入口。下面是一个示例来说明循环如何存在：
    **例如：** 设数组 `arr[] = {2, 6, 4, 1, 3, 1, 5}`
...
5.  算法由两部分组成，并使用两个指针，通常称为**乌龟**和**野兔**。
    ...
    *   在某些时候，乌龟也会进入循环，因为它移动的速度较慢，所以野兔在某个交叉点将乌龟赶上了。
    *   注意，在一般情况下，交点不是循环的入口，而是在循环中间的某个地方相交。
    *   将乌龟移动到序列的起点，野兔保持在循环内，并且以相同的速度移动，即**乌龟** `= arr[乌龟]` 和**野兔** `= arr[野兔]`。现在它们在重复元素处相交。
