# 查询在范围[L, R]内具有奇数频率的字符数

> 原文: [https://www.geeksforgeeks.org/queries-to-count-characters-having-odd-frequency-in-a-range-l-r/](https://www.geeksforgeeks.org/queries-to-count-characters-having-odd-frequency-in-a-range-l-r/)

给定长度为 `N` 的字符串 `S`，由小写字母组成，以及形式为 `[L, R]` 的 `Q[][]` 查询，任务是计算在范围 `[L, R]` 内出现奇数次的字符数。

## 示例

**输入:**
`S = "geeksforgeeks"`，`Q[][] = {{2, 4}, {0, 3}, {0, 12}}`
**输出:**
`3 2 3`
**解释:**
- 在 `[2, 4]` 中出现奇数次的字符: `{'e', 'k', 's'}`。
- 在 `[0, 3]` 中出现奇数次的字符: `{'g', 'k'}`。
...
`3`
**说明:**
在 `[0, 4]` 中出现奇数次的字符: `{'h', 'e', 'o'}`。

## 方法

按照以下步骤解决问题:
- 每个字符可以用唯一的 2 的幂来表示(按升序)。例如 `2^0` 为 `'a'`，`2^1` 为 `'b'`，以此类推，直至 `2^25` 为 `'z'`。
- 初始化一个大小为 `N` 的数组 `arr[]`，其中 `arr[i]` 是 `S[i]` 的相应整数值。
- 构建大小为 `N` 的前缀数组 `prefix[]`，其中 `prefix[i]` 是对从 `arr[0]` 到 `arr[i]` 的所有数字执行的异或运算的值。
- ` {arr[L], arr[L + 1], …, arr[R–1], arr[R]}` 的异或值中的置位数给出了给定范围 `[L, R]` 所需的答案。
