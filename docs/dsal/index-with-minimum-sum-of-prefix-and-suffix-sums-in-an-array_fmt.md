...
> 原文: [https://www.geeksforgeeks.org/index-with-minimum-sum-of-prefix-and-suffix-sums-in-an-array/](https://www.geeksforgeeks.org/index-with-minimum-sum-of-prefix-and-suffix-sums-in-an-array/)

给定一个整数数组。任务是在数组中找到索引 `i`，在该索引处 `前缀(i) + 后缀(i)` 的值最小。
...
假设我们需要最小化 `前缀[i] + 后缀[i]` 的值。这是第一个 `i` 元素和第三个 `N-i+1` 元素的总和。
