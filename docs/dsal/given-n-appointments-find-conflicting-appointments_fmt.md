...
> 原文: [https://www.geeksforgeeks.org/given-n-appointments-find-conflicting-appointments/](https://www.geeksforgeeks.org/given-n-appointments-find-conflicting-appointments/)
...
一个简单的解决方法就是从第二次约会到最后一次约会逐一处理所有约会。对于每个约会 `i`，检查它是否与 `i-1`、`i-2`、… `0` 冲突。该方法的时间复杂度为 `O(n^2)`。
...
以下是一个用Java编写的程序，用于打印给定预约集合中的所有冲突预约。
...
以下是一个用Python 3编写的程序，用于打印给定预约集合中的所有冲突预约。
