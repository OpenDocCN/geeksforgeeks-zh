...
这个想法是从第 1 章开始，做一个 DFS 来寻找边数为“k”的 sink。继续将访问过的顶点存储在一个数组中，比如“`path[]`”。如果我们到达目的顶点，并且路径和小于最优路径，则更新最优分配`optimal_path[]`。注意，该图是 DAG，因此在 DFS 期间不需要考虑周期。
...
def assignChapters(u, path, path_len, Sum, k):
    global CHAPTERS, DAYS, NOLINK, optimal_path, DAG, Min
