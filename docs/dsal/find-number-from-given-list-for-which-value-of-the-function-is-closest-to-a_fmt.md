...
> 原文: [https://www.geeksforgeeks.org/find-number-from-given-list-for-which-value-of-the-function-is-closest-to-a/](https://www.geeksforgeeks.org/find-number-from-given-list-for-which-value-of-the-function-is-closest-to-a/)

给定一个函数 `F(n)= P–(0.006 * n)`，其中 `P` 是给定的。给定一列整数和一个数字 `A`。任务是从给定列表中找到函数值最接近 `A` 的数字。
...
迭代给定列表中的每个值，并为每个值找到 `F(n)`。现在，比较 `F(n)` 和 `A` 的每个值与 `n` 的值的绝对差，对于后者，绝对差最小就是答案。
