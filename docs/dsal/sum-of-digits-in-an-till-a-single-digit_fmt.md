...
> 原文：[https://www.geeksforgeeks.org/sum-of-digits-in-an-till-a-single-digit/](https://www.geeksforgeeks.org/sum-of-digits-in-an-till-a-single-digit/)

给定两个数字 `a` 和 `n`，任务是找出 `a^n`（`pow(a, n)`）的位数之和。在一位数求和中，我们一直做位数求和，直到剩下一位数。
...
一种天真的方法是首先找到 `a^n`，然后使用这里讨论的方法找到 `a^n` 的数字总和。以上做法可能造成溢出。更好的解决方案是基于以下观察。
...
我们可以写一个类似于快速模幂运算的函数来计算 `digSum(a^n)`，它以对数（`n`）步来计算。以下是上述方法的实施：
