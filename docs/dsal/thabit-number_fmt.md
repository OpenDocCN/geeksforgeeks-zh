...
给定一个正整数 `n`，任务是检查它是否是一个 Thabit 数。如果给定的数字是塔比特数，则打印“是”，否则打印“否”。

[塔比特数](https://en.wikipedia.org/wiki/Thabit_number)：在数学中，塔比特数是 `3 * 2^n - 1` 形式的正整数，其中 `n` 是非负整数。

前几个 Thabit 数字是：2、5、11、23、47、95、191、383、767、1535、3071、6143、12287、24575、49151、98303、196607、393215。

**例：**

> 输入：47
> 输出：是
> 说明：对于 `n=4`，47 可以用 `3 * 2^n - 1` 表示为 `3 * 2^4 - 1`。
>
> 输入：65
> 输出：否
> 不存在可以用 `3 * 2^n - 1` 形式表示 65 的 `n` 值。

**进场：**

1.  给定的数字加 1，现在数字必须是 `3 * 2^n` 的形式。
2.  用 3 除这个数，现在这个数必须是 `2^n` 的形式。
