...
> 原文: [https://www.geeksforgeeks.org/k-th-lexicographically-smallest-binary-string-with-a-0s-and-b-1s/](https://www.geeksforgeeks.org/k-th-lexicographically-smallest-binary-string-with-a-0s-and-b-1s/)

给定三个正整数 `A`、`B` 和 `K`，任务是找到第 `K` 个按字典顺序最小的[二进制字符串](https://www.geeksforgeeks.org/tag/binary-string/)，该字符串恰好包含 `A` 个 `0` 和 `B` 个 `1`。
...
*   初始化一个[二维数组](https://www.geeksforgeeks.org/multidimensional-arrays-c-cpp/) `dp[][]`，使得 `dp[i][j]` 表示用 `i` 个 `0` 和 `j` 个 `1` 能组成的二进制字符串的总数。
...
*   填充此 `dp` 表后，可使用[递归函数](https://www.geeksforgeeks.org/recursive-functions/)计算第 `K` 个字典序最小二进制字符串。
*   因此，定义一个函数 `KthString`，该函数具有参数 `A`、`B`、`K` 和 `dp`。
*   现在，在这个递归函数的每次调用中:
    *   如果 `dp[A][B]` 的值至少为 `K`，则在第 `K` 个字典序最小二进制字符串中的该位置只能出现 `0`，然后递归调用状态 `(A-1, B)` 的函数。
