...
> 原文：[https://www.geeksforgeeks.org/check-if-diagonal-elements-of-a-matrix-are-prime-or-not/](https://www.geeksforgeeks.org/check-if-diagonal-elements-of-a-matrix-are-prime-or-not/)
...
**方法：** 思路是用厄拉多塞的[筛法](https://www.geeksforgeeks.org/sieve-of-eratosthenes/)[检查一个数是否是质数](https://www.geeksforgeeks.org/c-program-to-check-whether-a-number-is-prime-or-not/)。按照以下步骤解决问题：

*   使用厄拉多塞的筛预计算并存储质数。
*   使用变量 `i` 在范围`[0，N–1]`内迭代循环，并执行以下操作：
    *   检查 `M[i][i]`，即主对角线上的元素，是否是素数。
    *   检查 `M[i][N–1–i]`，即交叉对角线上的元素，是否是素数。
    *   如果不满足以上两个条件中的任何一个，打印`“否”`并[断开](https://www.geeksforgeeks.org/break-statement-cc/)循环。
*   否则，打印`“是”`。
