...
> 原文: [https://www.geeksforgeeks.org/largest-possible-square-submatrix-with-maximum-and-value/](https://www.geeksforgeeks.org/largest-possible-square-submatrix-with-maximum-and-value/)

给定一个整数矩阵 `mat[][]`，任务是从给定的矩阵中找到最大可能的正方形矩阵，该矩阵具有最大的[按位与](https://www.geeksforgeeks.org/bitwise-and-of-all-the-elements-of-array/)值。
...
> **解释:**
> 给定的正方形子矩阵的 AND 值为 2。
> 大小为 4 的子矩阵 `{{3, 3}, {3, 3}}` 的最大“与”值为 3。所有其他大小为 4 的正方形子矩阵的“与”值为 2。

> **输入:** `mat[][] = {{9, 9, 9, 8}, {9, 9, 9, 6}, {9, 9, 9, 3}, {2, 2, 2, 2}}`
> **输出:** 9
> **解释:**
> 大小为 9 的子矩阵 `{{9, 9, 9}, {9, 9, 9}, {9, 9, 9}}` 的最大“与”值为 9。

**天真方法:**
从给定矩阵生成所有正方形子矩阵。初始化一个变量 `answer` 以存储子矩阵的最大 & 值，另一个变量 `count` 以存储子矩阵中的元素数量。打印从所有正方形子矩阵获得的最大与值 `answer` 对应的 `count` 的最大值。
