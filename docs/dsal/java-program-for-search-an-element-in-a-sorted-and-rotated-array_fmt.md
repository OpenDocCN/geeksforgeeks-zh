...
> 原文: [https://www.geeksforgeeks.org/java-program-for-search-an-element-in-a-sorted-and-rotated-array/](https://www.geeksforgeeks.org/java-program-for-search-an-element-in-a-sorted-and-rotated-array/)
...
2.  查找枢轴的主要思想是–对于排序(按递增顺序)和旋转数组，枢轴元素是其下一个元素小于它的唯一元素。
3.  利用上面的说法和二分搜索法的枢轴可以找到。
4.  找到轴心后，将数组分成两个子数组。
...
**方法:** 代替二分搜索法的两次或两次以上传球，结果可以在二分搜索法的一次传球中找到。需要修改二分搜索法号来执行搜索。其思想是创建一个递归函数，将 `l` 和 `r` 作为输入和键的范围。
