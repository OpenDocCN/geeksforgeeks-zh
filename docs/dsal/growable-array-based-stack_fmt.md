...
我们都知道[栈](https://www.geeksforgeeks.org/stack-data-structure-introduction-program/)也称为`后进先出(LIFO)`结构。堆栈主要有两个主要操作，即`push`和`pop`，`push`在顶部插入一个元素，`pop`从堆栈顶部移除一个元素。
...
可生长堆栈上有两个操作:
1. 常规`push`操作: 在栈顶增加一个元素
2. 特殊`push`操作: 创建一个比旧堆栈大的新堆栈(根据上面的策略之一)，从旧堆栈中复制所有元素，然后将新元素推送到新堆栈中。
