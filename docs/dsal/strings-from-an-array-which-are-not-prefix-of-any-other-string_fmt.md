...
> 原文: [https://www.geeksforgeeks.org/strings-from-an-array-which-are-not-prefix-of-any-other-string/](https://www.geeksforgeeks.org/strings-from-an-array-which-are-not-prefix-of-any-other-string/)
...
那边
```
这里“app”是“苹果”的前缀，因此不打印，“the”是“那边”的前缀。
...
## 天真方法
对于数组的每个字符串，我们检查它是否是任何其他字符串的前缀。如果是，那么不要显示它。

## 高效方法
我们从数组中一个接一个地挑选字符串，并将其插入 `Trie`。那么字符串的插入有两种情况:
...
def display(root, Str, level):
  ...
  # If node is leaf node, it indicates end
  # of a String, so a null character is added
  # and String is displayed
