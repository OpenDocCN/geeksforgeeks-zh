...
> 原文: [https://www.geeksforgeeks.org/product-of-values-of-all-possible-non-empty-subsets-of-given-array/](https://www.geeksforgeeks.org/product-of-values-of-all-possible-non-empty-subsets-of-given-array/)
...
**方法:** 仔细观察，可以推断出所有子集内每个元素的出现次数为 `2^(N-1)`。因此，在最终的乘积中，数组中的每个元素都将乘以 `2^(N-1)` 次。

```
Product = a[0]^(2^(N-1)) * a[1]^(2^(N-1)) * ... * a[N-1]^(2^(N-1))
```
