...
> 原文：[https://www.geeksforgeeks.org/how-to-validate-indian-passport-number-using-regular-expression/](https://www.geeksforgeeks.org/how-to-validate-indian-passport-number-using-regular-expression/)

给定一个由字母数字字符组成的字符串 `str`，任务是使用[正则表达式](https://www.geeksforgeeks.org/write-regular-expressions/)检查给定的字符串是否有效的`护照号码`。
...
3.  其中：
    ...
    *   `$` 代表字符串的结尾。
4.  将给定的字符串与正则表达式匹配。在 Java 中，这可以通过使用 [`Pattern.matcher()`](https://www.geeksforgeeks.org/matcher-pattern-method-in-java-with-examples/) 来完成。
