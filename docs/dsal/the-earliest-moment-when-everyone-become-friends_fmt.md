...
> 原文：[https://www.geeksforgeeks.org/the-earliest-moment-when-everyone-become-friends/](https://www.geeksforgeeks.org/the-earliest-moment-when-everyone-become-friends/)

给定一组 `N` 人，每个人具有从 `0` 到 `(N–1)` 的唯一 ID 值，以及形式 `{U, V, time}` 的 `M` 元素的数组 `arr[]`，表示人 `U` 将在给定的 `time` 认识人 `V`。假设人 `U` 与人 `V` 相识，如果 `U` 与 `V` 是朋友，或者 `U` 是与 `V` 相识的人的朋友。任务是找到每个人互相认识的最早时间。
...
> *   在 `time = 2` 时，`{0}` 和 `{1}` 成为了朋友。因此，认识的人组成了 `{0, 1}`、`{2}`、`{3}`。
> *   在 `time = 3` 时，`{1}` 和 `{2}` 成为了朋友。因此，认识的人变成了 `{0, 1, 2}`、`{3}`。
> *   在 `time = 4` 时，`{2}` 和 `{3}` 成为了朋友。因此，认识的人变成了 `{0, 1, 2, 3}`。
>
> 因此，在 `time = 4` 时，每个人都互相认识了。
>
...
给定的问题可以使用[不相交集合并数据结构](https://www.geeksforgeeks.org/disjoint-set-data-structures/)来解决。这个想法是按照时间价值递增的顺序执行人与人之间的联合操作。要求的答案将是所有人属于同一个集合的时间。按照以下步骤解决给定的问题：

*   根据这里讨论的算法，用 `union` 和 `findSet` 功能实现[不相交集联合数据结构](https://www.geeksforgeeks.org/disjoint-set-data-structures/)。
*   初始化一个变量 `time`，存储 DSU 当前时间戳的值。
*   [按照时间的递增顺序对给定的数组](https://www.geeksforgeeks.org/sort-c-stl/) `arr[]` 进行排序。
*   [使用变量 `i` 遍历给定数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[]`，在 `(U_i, V_i)` 之间执行并集操作，如果 `U_i` 和 `V_i` 所属，则将当前时间戳更新为 `time_i`。
*   如果完全遍历数组 `arr[]` 后的集合总数为 `1`，则返回变量 `time` 的值，否则返回 `-1`。
