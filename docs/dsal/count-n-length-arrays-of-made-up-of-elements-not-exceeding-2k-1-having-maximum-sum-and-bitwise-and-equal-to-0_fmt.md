...
> 原文: [https://www.geeksforgeeks.org/count-n-length-arrays-of-made-up-of-elements-not-exceeding-2k-1-having-maximum-sum-and-bitwise-and-equal-to-0/](https://www.geeksforgeeks.org/count-n-length-arrays-of-made-up-of-elements-not-exceeding-2k-1-having-maximum-sum-and-bitwise-and-equal-to-0/)

给定两个整数 `N` 和 `K`，任务是找到满足以下条件的 `N`-长度[数组](https://www.geeksforgeeks.org/array-data-structure/)的个数:

*   数组元素的总和是最大可能值。
*   对于 `i` ( `1 ≤ i ≤ N` )的每个可能值，第 `i` 个元素应位于 `0` 和 `2^K–1` 之间。
...
*   定义一个[功能](https://www.geeksforgeeks.org/functions-in-c/) `power(long long x, long long y, int p)` 并执行以下任务:
    ...
    *   [循环迭代](https://www.geeksforgeeks.org/c-c-while-loop-with-examples/)直到 `y` 大于 `0`，执行以下任务。
        ...
        *   将 `x` 的值设置为 `(x*x)%p`。
*   将变量 `mod` 初始化为 `1e9+7`。
*   将变量 `ans` 初始化为[函数](https://www.geeksforgeeks.org/functions-in-c/) `power(N, K, mod)` 返回的值。
