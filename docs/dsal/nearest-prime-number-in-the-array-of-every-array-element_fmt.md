...
> 原文：[https://www.geeksforgeeks.org/nearest-prime-number-in-the-array-of-every-array-element/](https://www.geeksforgeeks.org/nearest-prime-number-in-the-array-of-every-array-element/)
...
*   将 `curr` 指向第一个由素数组成的索引。
*   对于到 `curr` 的每个索引，打印 `arr[primes[curr]]` 作为最近的素数。
*   对于超过 `curr` 的指数，将距离与 `primes[curr]` 和 `primes[curr + 1]` 进行比较。如果 `primes[curr]` 更近，打印 `arr[primes[curr]]`。否则，增加 `curr` 并打印 `arr[primes[curr]]`。
*   如果 `curr` 是数组中的最后一个质数，则打印 `arr[primes[curr]]` 用于所有索引。
