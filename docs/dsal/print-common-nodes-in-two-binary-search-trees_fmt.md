...
> 原文: [https://www.geeksforgeeks.org/print-common-nodes-in-two-binary-search-trees/](https://www.geeksforgeeks.org/print-common-nodes-in-two-binary-search-trees/)
...
一种简单的方法是在第二棵树中逐个搜索第一棵树的每个节点。该方案的时间复杂度为 `O(m * h)`，其中 `m` 是第一棵树的节点数，`h` 是第二棵树的高度。
...
*   **算法–**
    1.  对第一棵树进行有序遍历，并将遍历结果存储在辅助数组 `ar1[]` 中。参见此处的[合并两个平衡二叉搜索树](https://www.geeksforgeeks.org/merge-two-balanced-binary-search-trees/)。
    2.  对第二棵树进行有序遍历，并将遍历存储在辅助数组 `ar2[]` 中。
    3.  找到 `ar1[]` 和 `ar2[]` 的交集。详见[两个有序数组的并集与交集](https://www.geeksforgeeks.org/union-and-intersection-of-two-sorted-arrays-2/)。
*   **Complexity Analysis:**
    *   **时间复杂度:** `O(m+n)`。
