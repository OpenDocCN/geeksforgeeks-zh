...
> 原文: [https://www.geeksforgeeks.org/queries-to-count-numbers-from-given-range-which-are-divisible-by-all-its-digits/](https://www.geeksforgeeks.org/queries-to-count-numbers-from-given-range-which-are-divisible-by-all-its-digits/)

## 问题描述

给定一个 [2D 数组](https://www.geeksforgeeks.org/multidimensional-arrays-c-cpp/) `arr[][]`，每一行的查询形式为 `{ L, R }`，任务是对范围 `[L, R]` 内的数字进行计数，使得该数字可被其所有非零数字整除。
...
解决这个问题最简单的方法就是[遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr[][]`，并且对于数组的每一行，迭代范围 `[L, R]`。对于范围中的每个元素，检查该数字是否能被其所有非零数字整除。如果发现为真，则增加计数。最后，打印获得的计数。
...
上述方法可以通过找到 `arr[i][1]` 的最大可能值并使用[前缀求和技术](https://www.geeksforgeeks.org/prefix-sum-array-implementation-applications-competitive-programming/)预计算可被其非零数字整除的数字计数来优化。按照以下步骤解决问题:

1.  初始化一个变量，比如 `Max`，以存储 `arr[i][1]` 的最大可能值。
2.  初始化一个数组，比如说 `prefCntDiv[]`，其中 `prefCntDiv[i]` 存储范围 `[1, i]` 中可被非零数字整除的数字计数。
3.  迭代范围 `[1, Max]`。对于每一次 `i` 迭代，检查 `i` 是否能被其所有非零数字整除。如果发现为真，则更新 `prefCntDiv[i] = prefCntDiv[i - 1] + 1`。
4.  否则，更新 `prefCntDiv[i] = prefCntDiv[i - 1]`。
5.  [遍历数组](https://www.geeksforgeeks.org/c-program-to-traverse-an-array/) `arr`。对于数组的第 `i` 行，打印 `prefCntDiv[arr[i][1]] - prefCntDiv[arr[i][0] - 1]`。
