...
> 原文: [https://www.geeksforgeeks.org/count-number-of-substrings-of-a-string-consisting-of-same-characters/](https://www.geeksforgeeks.org/count-number-of-substrings-of-a-string-consisting-of-same-characters/)
...
将 `result` 初始化为 0。遍历字符串，找到相同字符的连续元素数(比如 `count`)。每当我们找到另一个字符时，将 `result` 增加 `count*(count+1)/2`，将 `count` 设置为 1，并从该索引开始，重复上述过程。
