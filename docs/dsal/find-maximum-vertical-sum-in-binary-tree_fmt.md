...
> 原文：[https://www.geeksforgeeks.org/find-maximum-vertical-sum-in-binary-tree/](https://www.geeksforgeeks.org/find-maximum-vertical-sum-in-binary-tree/)
...
一个简单的解法是先求从最小垂直标高到最大垂直标高开始的各标高的垂直标高之和。求一个垂直水平的和需要 `O(n)` 个时间。在最坏的情况下，这个解决方案的时间复杂度是 `O(n^2)`。

一种高效的解决方案是对给定的二叉树进行级别顺序遍历，并在遍历的同时更新每一级的垂直级别和。在找到每个水平的垂直和之后，从这些值中找到最大垂直和。
...
function maxVerticalSum(root) {
    ...
    while (q.length > 0) {
        ...
        if (verSum.has(currLev)) {
            verSum.set(currLev, verSum.get(currLev) + root.data);
        } else {
