...
> 原文: [https://www.geeksforgeeks.org/queries-for-the-product-of-first-n-factorials/](https://www.geeksforgeeks.org/queries-for-the-product-of-first-n-factorials/)

给定 `Q[]` 查询，其中每个查询由整数 `N` 组成，任务是为每个查询找到第一个 `N` 阶乘的乘积。因为结果可能很大，所以以 `10^9 + 7` 为模进行计算。
...
创建两个数组 `result[]` 和 `fact[]`，其中 `fact[i]` 将存储 `i` 的阶乘，`result[i]` 将存储第一个 `i` 阶乘的乘积。
初始化 `fact[0] = 1` 和 `result[0] = 1`。现在对于其余的值，循环关系将是:

> `fact[i] = fact[i - 1] * i`
> `result[i] = result[i - 1] * fact[i]`

现在，可以使用生成的 `result[]` 数组来回答每个查询。
以下是上述办法的实施情况:
