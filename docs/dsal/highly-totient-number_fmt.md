...
高度全能数 `k` 是对方程 `φ(x) = k` 有更多解的整数，其中 `φ` 是[欧拉全能函数](https://www.geeksforgeeks.org/eulers-totient-function/)。
...
一种有效的方法是将 `φ(x)` 直到 `10^5` 的所有值连同它们的频率一起存储在地图中，然后从 `1` 运行一个循环，直到高全能数的计数小于 `N`。对于每个 `i` 我们将检查 `i` 的频率是否大于前一个元素，如果是，则打印数字并增加计数，否则增加数字。
