...
> 原文: [https://www.geeksforgeeks.org/number-subarrays-m-odd-numbers/](https://www.geeksforgeeks.org/number-subarrays-m-odd-numbers/)
...
**有效方法:** 一种有效方法是在遍历时，计算 `prefix[]` 数组。`prefix[i]` 存储其中有 `i` 个奇数的前缀数。如果数组元素是奇数，我们增加奇数的计数。当奇数的计数超过或等于 `m` 时，将带有 `(odd - m)` 数字的前缀数加到答案中。在每一步 `odd >= m` 处，我们借助前缀数组计算形成特定索引的子数组的数量。`prefix[odd - m]` 为我们提供了具有 `odd - m` 个奇数的前缀数量，将其添加到计数中以获得直到索引的子阵列数量。
以下是上述方法的实施:
