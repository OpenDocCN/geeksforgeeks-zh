...
> 原文：`https://www.geeksforgeeks.org/given-a-sequence-of-words-print-all-anagrams-together-set-2/`
...
`Trie`数据结构可以用于更高效的解决方案。在`trie`中插入每个单词的排序顺序。因为所有的字谜都会在同一个叶节点结束。我们可以在叶节点处开始一个链表，其中每个节点代表原始单词数组的索引。最后，遍历`Trie`。遍历时，一次遍历一行每个链表。以下是详细步骤。
**1)** 创建一个空的`Trie`
