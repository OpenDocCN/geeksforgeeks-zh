...
> 原文: [https://www.geeksforgeeks.org/print-a-sorted-list-of-words-represented-by-the-expression-under-the-given-grammar/](https://www.geeksforgeeks.org/print-a-sorted-list-of-words-represented-by-the-expression-under-the-given-grammar/)

给定长度为 `n` 的[字符串](https://www.geeksforgeeks.org/string-data-structure/) `R(x)`，表示在给定语法下具有一组单词的表达式:
*   对于每个小写字母 `x`，`R(x) = {x}`
*   对于表达式 `e_1, e_2, …, e_k` 带有 `k≥2`，`R({e_1, e_2, …, e_k}) = R(e_1) ∪ R(e_2) ∪ … ∪ R(e_k)`。
