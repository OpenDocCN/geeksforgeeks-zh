...
现在来寻找 `n` 中任意非质数 `k` 的幂！，我们首先[找到数字 `k` 的所有质因数](https://www.geeksforgeeks.org/print-all-prime-factors-of-a-given-number/)以及它们出现的次数。然后，对于每个质因数，我们使用 [Legendre 公式](https://www.geeksforgeeks.org/given-p-and-n-find-the-largest-x-such-that-px-divides-n-2/)来计算出现次数，该公式指出 `n` 中质数 `p` 的最大可能幂是`⌊n/p⌋+⌊n/(p^2)⌋+⌊n/(p^3)⌋+……`
