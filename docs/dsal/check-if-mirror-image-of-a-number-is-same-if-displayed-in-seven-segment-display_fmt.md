...
> 原文: [https://www.geeksforgeeks.org/check-if-mirror-image-of-a-number-is-same-if-displayed-in-seven-segment-display/](https://www.geeksforgeeks.org/check-if-mirror-image-of-a-number-is-same-if-displayed-in-seven-segment-display/)

给定一个正数 `n`。任务是检查在[七段显示器](https://en.wikipedia.org/wiki/Seven-segment_display)上显示的数字镜像是否等于给定的数字。一个数的[镜像](https://en.wikipedia.org/wiki/Mirror_image)是该数的反射复制，看起来几乎相同，但在垂直于镜面的方向上是相反的。
