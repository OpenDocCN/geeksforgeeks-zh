...
> 原文: [https://www.geeksforgeeks.org/find-nth-smallest-number-having-exactly-4-divisors/](https://www.geeksforgeeks.org/find-nth-smallest-number-having-exactly-4-divisors/)

给定一个正整数 `N`，任务是找到序列中第 `N` 个最小的数，这个数正好有 `4` 个除数。
...
**逼近:** 这个问题可以通过观察 `i` 对 `P1<sup>a1</sup>* p2<sup>a2</sup>* P3<sup>a3</sup>…PK<sup>AK</sup>` 那么 `i` 的除数为 `(a1+1)(a2+1)…(ak+1)`。所以对于 `i` 来说，正好有 `4 个除数`，应该等于两个截然不同的素数的积，或者是某个素数的幂 `3`。按照以下步骤解决问题:

*   初始化数组 `divs[]` 和 `vis[]` 以存储任意数的除数，并分别检查给定数是否被考虑。
*   将变量 `cnt` 初始化为 `0`，以存储正好具有 `4` 除数的元素数。
*   现在，使用厄拉多塞算法的筛。
*   迭代当 `i` 小于 `n` 时，使用变量 `i` 从 `2` 开始:
    *   如果 `i` 是质数:
        *   使用变量 `j` 在范围 `[2 * i，1000000]` 中迭代，增量为 `i`:
            *   如果已经考虑了数字 `j`，那么 `continue`。
            *   将 `vis[j]` 更新为 `true`，将变量 `currNum` 初始化为 `j` 并将 `count` 计为 `0`。
            *   将 `currNum` 除以 `i` 而 `currNum % i` 等于 `0`，递增 `divs[j]` 和 `count` 除以 `1`。
            *   如果 `currNum` 等于 `1`，`count` 等于 `3`，`divs[j]` 等于 `3`，则以 `1` 递增 `cnt`。
            *   否则，如果 `currNum` 不等于 `1`，`count` 等于 `1`，`divs[j]` 等于 `1`，`divs[currNum]` 等于 `0`，然后将 `cnt` 增加 `1`。
