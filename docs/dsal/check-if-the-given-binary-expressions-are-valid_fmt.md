...
给定类型 `x = y` 和 `x != y` 的表达式，其中 `1 ≤ x, y ≤ n`，任务是检查从 `1` 到 `n` 的整数是否可以赋给 `x` 和 `y`，使得所有方程都满足。
...
思路是使用 union-find。对于每个语句，检查是否存在“=”符号，然后找到两个变量的父值，并使用 rank 方法将它们联合起来。现在，一旦所有变量之间都有“=”运算符的联合完成，就开始检查“!=”运算符，如果该运算符存在于父变量相同的任意两个变量之间，则表达式无效，否则它们有效。
