...
> 原文: [https://www.geeksforgeeks.org/bitwise-xor-of-all-unordered-pairs-from-a-given-array/](https://www.geeksforgeeks.org/bitwise-xor-of-all-unordered-pairs-from-a-given-array/)

给定一个大小为 `N` 的数组 `arr[]`，任务是找到给定数组中所有可能的无序对的位异或。
...
想法是遍历数组和生成给定数组的所有可能的对。最后，打印给定数组的这些对中存在的每个元素的位异或。按照以下步骤解决问题:

*   初始化一个变量，比如说 `totalXOR`，来存储这些对中每个元素的位 XOR。
*   遍历给定数组和从给定数组生成所有可能的对 `(arr[i]，arr[j])`。
*   对于每对 `(arr[i]，arr[j])`，更新 `totalXOR = (totalXOR ^ arr[i] ^ arr[j])` 的值。
*   最后，打印 `totalXOR` 的值。
