...
> 原文：[https://www.geeksforgeeks.org/rearrange-array-such-that-difference-of-adjacent-elements-is-in-descending-order/](https://www.geeksforgeeks.org/rearrange-array-such-that-difference-of-adjacent-elements-is-in-descending-order/)

给定一个具有 `n` 个整数的数组 `arr[]`，任务是重新排列数组的元素，使相邻元素的差按降序排列。
...
> `a[n]`，`a[1]`，`a[n-1]`，`a[2]` …..`a[(n/2)]`，`a[(n/2)+1]`
