...
> 原文：[https://www.geeksforgeeks.org/find-next-greater-number-set-digits/](https://www.geeksforgeeks.org/find-next-greater-number-set-digits/)
...
1.  从最右边的数字开始遍历给定的数字，继续遍历，直到找到一个比之前遍历的数字小的数字。例如，如果输入的数字是 `"534976"`，我们会在 `4` 处停止，因为 `4` 比下一个数字 `9` 小。如果我们找不到这样的数字，那么输出就是“不可能”。
2.  现在在上面找到的数字 `d` 的右侧搜索大于 `d` 的最小数字。对于 `"534976"`，`4` 的右侧包含 `"976"`。大于 `4` 的最小数字是 `6`。
3.  互换上面找到的两位数，我们得到上面例子中的 `536974`。
4.  现在将所有数字从 `d` 旁边的位置排序到数字的末尾。排序后得到的数字就是输出。例如，我们用粗体 `536974` 对数字进行排序。我们得到 `"536479"`，这是输入 `534976` 的下一个更大的数字。
