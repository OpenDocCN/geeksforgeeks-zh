...
> 原文: [https://www.geeksforgeeks.org/python-program-for-longest-increasing-subsequence/](https://www.geeksforgeeks.org/python-program-for-longest-increasing-subsequence/)

最长递增子序列 (LIS) 问题是求给定序列的最长子序列的长度，使子序列的所有元素按递增顺序排序。例如，`{10, 22, 9, 33, 21, 50, 41, 60, 80}` 的 LIS 长度为 6，LIS 为 `{10, 22, 33, 50, 60, 80}`。
...
def _lis(arr, n):
    ...
    """Recursively get all LIS ending with arr[0], arr[1]..arr[n-2]
       IF arr[n-1] is greater than arr[n-1], and max ending with
...
我们可以看到有很多子问题被一次又一次地解决。因此，该问题具有重叠子结构性质，用记忆法或制表法都可以避免同一子问题的重新计算。下面是 LIS 问题的一个动态规划实现。
