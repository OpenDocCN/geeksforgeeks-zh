...
2.  `DFS/BFS` 的应用
    给定一个有 `n` 个节点的有向图。给定的图是连通的。如果存在从 `u` 到 `v` 的边，则 `u` 依赖于 `v`。我们的任务是[找出每个节点的依赖总和](https://practice.geeksforgeeks.org/problems/sum-of-dependencies-in-a-graph/0)。
...
1.  给定两个非常大的数字 `a` 和 `b`（字符串格式）。你需要对以下操作产生结果：
    `0`: `Add a & b`
    `1`: `Subtract a & b`
    `2`: `Multiply a & b`
...
1.  [给定手机数字键盘。你只能按下当前按钮上、左、右或下的按钮。不允许按下底行角落按钮](https://practice.geeksforgeeks.org/problems/mobile-numeric-keypad/0) (即 `*` 和 `#` )。
