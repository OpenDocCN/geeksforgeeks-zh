...
**输出**：2
变换 `'a'->'d'`, `'b'->'a'` 和 `'b'->'a'->'d'`。
我们不能将 `(a, d)`, `(b, a)`, `(b, d)` 成对使用，因为 `(b, d)` 可以通过以下变换 `'b'->'a'->'d'` 实现。
...
可以通过使用图或不连续集来解决此问题。建立一个空图 `G` 并遍历字符串。仅当满足以下条件之一时，才在图形 `G` 中添加边：

*   `s1[i]` 和 `s2[i]` 都不在 `G` 中。
*   `s1[i]` 在 `G` 中，但 `s2[i]` 不在 `G` 中。
*   `s2[i]` 在 `G` 中，但 `s1[i]` 不在 `G` 中。
*   从 `s1[i]` 到 `s2[i]` 没有路径。
