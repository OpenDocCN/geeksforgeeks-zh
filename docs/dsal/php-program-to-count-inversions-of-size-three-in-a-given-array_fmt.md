...
> 原文: [https://www.geeksforgeeks.org/php-program-to-count-inversions-of-size-three-in-a-given-array/](https://www.geeksforgeeks.org/php-program-to-count-inversions-of-size-three-in-a-given-array/)

给定大小为 `n` 的数组 `arr[]`。如果 `a[i] > a[j] > a[k]` 和 `i < j < k`，则三个元素 `arr[i]`、`arr[j]` 和 `arr[k]` 形成大小为 3 的反转。Find total number of inversions of size 3.
...
[使用 BIT 计算大小二的倒位](https://www.geeksforgeeks.org/count-inversions-array-set-3-using-bit/)

思路与上述方法类似。我们计算所有元素中较大元素和较小元素的数量，然后将 `larger[]` 乘以 `smaller[]` 并将其添加到结果中。
