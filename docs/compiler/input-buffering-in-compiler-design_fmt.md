...
> 原文: [https://www.geeksforgeeks.org/input-buffering-in-compiler-design/](https://www.geeksforgeeks.org/input-buffering-in-compiler-design/)
...
在此方案中，仅使用一个缓冲区来存储输入字符串，但此方案的问题是，如果 `lexeme` 非常长，它会跨越缓冲区边界，为了扫描 `lexeme` 的其余部分，必须重新填充缓冲区，这会导致覆盖 `lexeme` 的开头部分。

![](img/38964b8b3591739e87d23b9c75ff95ab.png)

## 双缓冲区方案

为了克服单缓冲区方案的问题，在此方法中使用两个缓冲区来存储输入字符串。第一个缓冲区和第二个缓冲区被交替扫描。当到达当前缓冲区的末尾时，另一个缓冲区被填充。此方法的唯一问题是，如果 `lexeme` 的长度长于缓冲区的长度，则无法完全扫描输入。
