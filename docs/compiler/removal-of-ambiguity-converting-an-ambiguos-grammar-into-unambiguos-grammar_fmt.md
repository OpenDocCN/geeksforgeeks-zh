...
```
E -> P ^ E | P  // 右递归，因为^是右联想的。
P -> id
```

## 示例 2
考虑下面显示的语法，它有两个不同的运算符：
```
E -> E + E | E * E | id
```
