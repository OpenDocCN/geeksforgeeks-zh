...
> 原文：`https://www.geeksforgeeks.org/operator-grammar-and-precedence-parser-in-toc/`
...
`id` 和 `id` 之间没有任何关系，因为 `id` 不会被比较，两个变量也不会同时出现。这个表还有一个缺点——如果我们有 n 个操作符，那么表的大小将是 `n*n`，复杂度将是 `O(n^2)`。为了减小表格的尺寸，我们使用了**操作员功能表**。
