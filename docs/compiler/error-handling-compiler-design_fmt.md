...
> 原文：[https://www.geeksforgeeks.org/error-handling-compiler-design/](https://www.geeksforgeeks.org/error-handling-compiler-design/)
...
这是最简单的错误恢复方式，同时，它防止解析器在恢复错误时开发无限循环。解析器一次丢弃一个输入符号，直到找到一组指定的（如 `end`、分号）同步标记（通常是语句或表达式终止符）。当同一语句中很少出现多个错误时，这就足够了。

示例：考虑错误的表达式 `(1 + + 2) + 3`。紧急模式恢复：跳到下一个整数，然后继续。Bison：使用特殊终端`error`描述跳过多少输入。
