...
函数 `yylex()` 是运行规则部分和扩展()的主要 flex 函数。l)是用于保存程序的扩展名。
...
**第一步:**一个描述将要生成的词法分析器 `lex.l` 的输入文件是用 lex 语言编写的。lex 编译器将 `lex.l` 转换成 C 程序，在一个总是被命名为 `lex.yy.c.` 的文件中。

**步骤 2:**C 编译器将 `lex.yy.c` 文件编译成一个被称为 `a.out.` 的可执行文件。

**步骤 3:** 输出文件 `a.out` 获取一个输入字符流并产生一个标记流。
...
定义部分包含变量、常规定义、清单常量的声明。在定义部分，文本包含在 **“% { % }”** 括号中。括号内的内容直接复制到文件 `lex.yy.c`。
...
规则部分包含一系列规则，其形式为:*模式动作*和模式必须是非预期的，并且动作在{}括号中的同一行开始。规则部分包含在 **“% % %”** 中。
...
该部分包含 C 语句和附加功能。我们也可以单独编译这些函数，并用词法分析器加载。
...
**第一步:** `lex filename.l` 或 `lex filename.lex` 根据扩展名文件与一起保存。
