...
> 原文：[https://www.geeksforgeeks.org/language-processing-system-in-compiler-design/](https://www.geeksforgeeks.org/language-processing-system-in-compiler-design/)
...
### 预处理器
预处理器包括所有头文件，还评估一个宏（一个宏是否是一段被赋予名称的代码。每当使用该名称时，解释器或编译器都会将其替换为宏的内容。宏的目的要么是自动化序列使用的频率，要么是实现更强大的抽象）。它以源代码为输入，以修改后的源代码为输出。预处理器也称为宏赋值器，处理是可选的也就是说如果任何不支持 `#` 的语言包括和宏处理是不需要的。
