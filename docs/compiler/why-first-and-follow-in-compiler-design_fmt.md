...
> 原文: [https://www.geeksforgeeks.org/why-first-and-follow-in-compiler-design/](https://www.geeksforgeeks.org/why-first-and-follow-in-compiler-design/)
...
因此，在上面的例子中，如果它知道在读取输入字符串中的字符“c”并应用`S->cAd`后，输入字符串中的下一个字符是“A”，那么它将忽略产生规则`A->bc`（因为“b”是由该产生规则产生的字符串的第一个字符，而不是“A”），并直接使用产生规则`A->a`（因为“A”是由该产生规则产生的字符串的第一个字符，并且与输入字符串的当前字符相同，该当前字符也是“A”）。

因此验证了如果编译器/解析器知道通过应用产生式规则可以获得的字符串的第一个字符，那么它可以明智地应用正确的产生式规则来获得给定输入字符串的正确语法树。
