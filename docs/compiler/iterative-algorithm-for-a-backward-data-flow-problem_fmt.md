...
> 原文：[https://www.geeksforgeeks.org/iterative-algorithm-for-a-backward-data-flow-problem/](https://www.geeksforgeeks.org/iterative-algorithm-for-a-backward-data-flow-problem/)

## 简介
写这篇文章的原因是为了告诉你一套近似迭代的规则，用于后向统计浮点问题。在开始之前，您必须了解一些与统计浮动分析相关的术语。

## 数据流分析
[数据流分析](https://www.geeksforgeeks.org/data-flow-analysis-compiler/)是一种收集关于在计算机程序中不同点计算的可能值集的信息的技术。
是对事物流图顶部统计漂移的评价，即确定程序中关于统计定义和使用的记录的评价。在这个评估的帮助下，可以进行优化。一般来说，它的方式，其中的价值已经计算使用统计流量评估。带有漂移属性的统计数据表示可用于优化的记录。

## 控制流程图
[控制流程图](https://www.geeksforgeeks.org/software-engineering-control-flow-graph-cfg/)用于决定分配给变量的特定费用可能传播到的软件元素。
