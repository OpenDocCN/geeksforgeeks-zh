...
> 原文: [https://www.geeksforgeeks.org/slr-clr-and-lalr-parsers-set-3/](https://www.geeksforgeeks.org/slr-clr-and-lalr-parsers-set-3/)
...
1.  构造 `C = { I0, I1, …… In }`，为 `G'` 的 `LR(0)` 项集合。
2.  状态 `I` 由 `Ii` 构成。状态 `I` 的解析动作确定如下:
    *   如果 `[ A -> α。aβ ]` 在 `Ii` 和 `GOTO(Ii, a) = Ij`，然后将 `ACTION[i, a]` 设置为 “shift j”。这里 `a` 一定是终结符。
    *   如果 `[A -> α。]` 是在 `Ii` 中，然后设置 `ACTION[i, A]` 为 “reduce A -> α” 对于 `FOLLOW` 中的所有 `A`；这里的 `A` 可能不是 `S`。
    *   如果 `[S' -> S.]` is in `Ii`，则设置 `ACTION[i, $]` 为 “accept”。如果以上规则产生了任何冲突的动作，我们就说语法不是单反。
3.  使用以下规则为所有非终结符 `A` 构建状态 `I` 的 `GOTO` 转换:
...
*   状态 `I0` – 扩大 `LR(1)` 项目闭包。
*   使用 `I0` 在 `DFA` 的帮助下找到 `LR(1)` 项集合的所有集合。
*   将 `DFA` 转换为 `LR(1)` 解析表。
...
1.  构造 `C = { I0, I1, …… In }`，为 `G'` 的 `LR(1)` 项集合。
2.  状态 `I` 由 `Ii` 构成。状态 `I` 的解析动作确定如下:
    i) 如果 `[ A -> α。aβ, b ]` 在 `Ii` 和 `GOTO(Ii, a) = Ij`，然后将 `ACTION[i, a]` 设置为 “shift j”。这里 `a` 一定是终结符。
    ii) 如果 `[A -> α。, a]` 是在 `Ii`，`A ≠ S'`，然后将 `ACTION[i, a]` 设置为 “reduce A -> α”。
    iii) 如果 `[S' -> S ., $]` is in `Ii`，然后将 `ACTION[i, $]` 设置为 “accept”。
    如果以上规则产生了任何冲突动作，我们说语法是 不是 CLR。
3.  使用以下规则为所有非终结符 `A` 构建状态 `I` 的 `GOTO` 转换: 如果 `GOTO(Ii, A) = Ij`，则 `GOTO[i, A] = j`。
4.  规则 2 和 3 未定义的所有条目都会出错。
...
**注意** – 如果一个状态有两个约简，并且两个都有相同的前瞻，那么它将在解析表中有多个条目，从而产生冲突。如果一个状态有一个移位和一个约简，并且移位的符号在约简的产生式的 `FOLLOW` 集中，那么它将导致解析表中的多个条目，从而导致冲突。
...
2.  如果状态数 `LR(0) = n1`，状态数 `SLR = n2`，状态数 `LALR = n3`，状态数 `CLR = n4`，那么，`n1 = n2 = n3 <= n4`。
