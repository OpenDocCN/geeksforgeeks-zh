...
> 原文：[https://www.geeksforgeeks.org/removing-direct-and-indirect-left-recursion-in-a-grammar/](https://www.geeksforgeeks.org/removing-direct-and-indirect-left-recursion-in-a-grammar/)
...
3.  将新产生的非终结符写在左侧（LHS），在右侧（RHS）它可以产生 epsilon（`?`），或者产生新的产生式，其中跟在先前左侧非终结符后面的终结符或非终结符将在最后被新的非终结符替换。
...
在这里，从 `A` 开始，我们可以通过将 `C` 代入 `B`，再将 `B` 代入 `A`，再次推导出 `A`。
...
3.  现在新的产生式已转换为直接左递归的形式，通过直接左递归方法求解。
    消除上述中的直接左递归，
