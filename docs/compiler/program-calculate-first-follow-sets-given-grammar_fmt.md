...
> 原文: [https://www.geeksforgeeks.org/program-calculate-first-follow-sets-given-grammar/](https://www.geeksforgeeks.org/program-calculate-first-follow-sets-given-grammar/)
...
函数 `follow` 和 `followfirst` 都涉及给定非终端的 Follow 集的计算。开始符号的以下集合将始终包含“$”。现在，Follow 的计算分为三大类:
...
7.  不要使用“!”或“$”；因为它们被保留用于特殊目的。

### 解释

将语法存储在 2D 字符数组 `production` 上。 `findfirst` 功能用于计算任意非终端的第一个。`calc_first` 的计算首先属于两大类情况:
