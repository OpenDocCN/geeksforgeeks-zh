...
更多参考，敬请访问 [https://www.geeksforgeeks.org/lr-parser/](https://www.geeksforgeeks.org/lr-parser/)
...
- 在图中，`I0` 由扩充语法组成。
- `木卫一`什么时候去 `I1` 第 `0` 次生产的产品被转移到 `S(S'->S)` 的右侧。这个状态就是接受状态。编译器会看到 `s`。因为 `I1` 是第 `0` 部作品的一部分，所以前瞻是一样的
...
#### 步骤 3- 定义 `2` 个功能: 转到解析表中的【终端列表】和动作【非终端列表】。下面是 `CLR` 解析表
