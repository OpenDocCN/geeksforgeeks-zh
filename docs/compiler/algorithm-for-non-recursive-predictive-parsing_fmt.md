...
> 原文: [https://www.geeksforgeeks.org/algorithm-for-non-recursive-predictive-parsing/](https://www.geeksforgeeks.org/algorithm-for-non-recursive-predictive-parsing/)
...
2.  如果 `FIRST(A)` 在 [`FOLLOW(A)`](https://www.geeksforgeeks.org/follow-set-in-syntax-analysis/) 中包含每个终端 `'b'` 的空生产，则将该生产 (`A-> ε`) 添加到解析表中的 `M[A, b]` 中。
...
push Yk, ..., Y2, Y1 onto the stack, with Y1 on top;
...
解析器因此结束，因为只剩下 `$` 在其堆栈和输入流上。在这种情况下，解析器报告它已经接受了输入字符串，并将以下规则列表写入输出流:
