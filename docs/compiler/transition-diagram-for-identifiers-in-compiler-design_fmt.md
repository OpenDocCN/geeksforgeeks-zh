...
> 原文: [https://www.geeksforgeeks.org/transition-diagram-for-identifiers-in-compiler-design/](https://www.geeksforgeeks.org/transition-diagram-for-identifiers-in-compiler-design/)
...
`LETTER(C)` 是当且仅当 `C` 是字母时返回真值的过程。
`FAIL(C)` 是一个收回前瞻指针并启动下一个转换图的例程，否则调用错误例程。
`DIGIT(C)` 是当且仅当 `C` 是数字时返回真值的程序。
`DELIMITER(C)` 是返回真值的过程，当且仅当 `C` 是可以跟在标识符后面的字符，例如空白符号、算术、逻辑运算符、左括号、右括号、`+`、`:`、`;`等等。
