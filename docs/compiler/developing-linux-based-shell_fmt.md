...
下图显示了简化的执行过程，其中 shell 接收输入，将其传递给词法分析器(将详细讨论)，词法分析器将创建标记，然后词法分析器的输出将被传递给解析器，解析器检查其语法错误并执行分配的语义动作(这将构建命令表)，最后当解析器到达某个点时，该表将被执行。
...
IO 标记由 `#` 字符或 `">"` 组成，前面可以是数字 `1`(最多一次)，或者 `">>"`，前面可以是数字 `1`(最多一次)，最后是 `"> &"`，这是一个 IOR，前面和/或后面可以是一个或两个。

管道符号和 `&` 符号分别在 `"|"` 和 `"&"` 处形成，选项符号在连字符前面加空格，后面加任何字母或数字时形成。
`OPTION2` 标记由两个连字符组成，前面是空格，后面是任何字母字符。
...
> 2. `q0: newline { return 1; } | pipe q00 { clrcnt(); };`
> 3. `q1: option q2 | option2 q2 | arg_list q3 | io_modifier q4 | background q5 | io_descr q3 | /* empty */ { insert_node(); clrcnt(); };`
> 4. `q2: arg_list q3 | io_modifier q4 | io_descr q3 | q5 | /* empty */ { insert_node(); clrcnt(); };`
> 5. `q3: io_modifier q4 | io_descr q3 | q5 | /* empty */ { insert_node(); clrcnt(); };`
> 6. `q4: q3 file;`
> 7. `cmd: word { cmad.cmd = yylval.str; };`
> 8. `arg_list: arg | arg_list arg;`
> 9. `arg: word { insertargnode(yylval.str); };`
> 10. `file: word { io_red(yylval.str); };`
> 11. `io_modifier: io { cmad.op = yylval.str; };`
> 12. `io_descr: up { cmad.op = yylval.str; };`
> 13. `option: option { cmad.opt = yylval.str; } | option2 { cmad.opt2 = yylval.str; };`
> 14. `background: background { bg = '1'; };`
> 15. `q5: /* empty */ { insert_node(); clrcnt(); };`
...
我们选择了一个表作为我们的数据结构，我们需要存储关于每个命令的以下信息，`command`、`options`、`option2`、`arguments`、`standard-in`、`standard-out`、`standard-error`。
