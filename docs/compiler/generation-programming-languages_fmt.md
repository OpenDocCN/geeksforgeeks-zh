...
> 原文：[https://www.geeksforgeeks.org/generation-programming-languages/](https://www.geeksforgeeks.org/generation-programming-languages/)
...
第一代语言也被称为机器语言或 `1G` 语言。这种语言依赖于机器。机器语言语句是用二进制代码（`0/1` 形式）编写的，因为计算机只能理解二进制语言。
...
第二代语言也被称为汇编语言或 `2G` 语言。汇编语言包含人类可读的符号，可以使用汇编程序进一步转换为机器语言。
...
第三代也叫过程语言或 `3 GL`。它包括使用一系列人类容易理解的类似英语的单词来写指令。它也被称为高级编程语言。为了执行，需要使用编译器/解释器将这种语言的程序翻译成机器语言。这类语言的例子有 `C`、`PASCAL`、`FORTRAN`、`COBOL` 等。
...
第四代语言也被称为非过程语言或 `4GL`。它使用户能够访问数据库。例子：`SQL`、`Foxpro`、`Focus` 等。
