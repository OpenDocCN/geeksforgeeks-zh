...
如果给定输入字符串存在多个派生树，即存在多个**Leftmost Derivation Tree (LMDT)**或**Rightmost Derivation Tree**，则认为 CFG 是歧义的。

## 定义:

`G = (V，T，P，S)`是一个 CFG 据说是模棱两可的当且仅当 `T*`中存在一个有多个解析树的字符串。
其中 `V` 是一组有限的变量。
`T` 是一组有限的端子。
`P` 是 `A -> α`形式的有限乘积集，其中 `A` 是变量，`α∈(V∪T)*`。`S` 是称为起始符号的指定变量。
...
语法歧义是不可判定的，即没有消除语法歧义的特定算法，但我们可以通过:
