...
p5.js 中的 `endShape()` 函数在 `beginShape()` 函数之后使用，完成形状的绘制。当调用该函数时，在前面的 `beginShape()` 函数之后定义的所有图像数据被写入图像缓冲区，以用作形状。
...
*   **mode**: 是一个常数，可以用来改变功能的模式。它可以有一个值 `CLOSE`。这是用来关闭形状的，也就是连接形状的开始到结束。这是一个可选参数。
...
[https://p5js.org/reference/#/p5/endShape](https://p5js.org/reference/#/p5/endShape)