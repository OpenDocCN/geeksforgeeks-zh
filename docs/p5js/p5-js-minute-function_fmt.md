...
p5.js 中的 `minute()` 功能用于返回系统时钟中的当前分钟。`minute()` 函数的值介于 0 到 59 之间。
...
function draw() {
    ...
    // Initialize the parameter with current minute
    let m = minute();
...
## 参考:
[https://p5js.org/reference/#/p5/minute](https://p5js.org/reference/#/p5/minute)