...
p5.js 中的 `angleMode()` 功能用于设置角度解释的模式。它可以设置为角度或弧度。所有使用角度作为参数的函数都将遵循该函数设置的角度模式。
...
*   `mode`: 这是一个常数，可用于设置角度解释的模式。它可以有 `DEGREES` 或 `RADIANS` 值。

以下示例说明了 p5.js 中的 `angleMode()` 函数:
