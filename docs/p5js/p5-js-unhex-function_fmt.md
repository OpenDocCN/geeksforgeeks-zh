...
> 原文：`https://www.geeksforgeeks.org/p5-js-unhex-function/`

p5.js 中的 `unhex()` 函数用于将任意输入十六进制数的字符串表示转换为其等效的整数值。
...
这个函数接受一个参数 `String`，它是一个十六进制数，要转换成它的整数等值。此参数也可能是十六进制数字的字符串数组。

### 返回值

返回转换后的整数表示。

下面的程序说明了 p5.js 中的 `unhex()` 功能。
