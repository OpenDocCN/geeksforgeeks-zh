...
`imageMode()` 功能用于设置图像的图像模式。图像模式通过改变对 `image()` 功能给出的参数的解释方式来定义图像在画布中的位置。
...
该功能接受单个参数 `mode`，定义要使用的模式。它可以具有以下值:

*   `CORNER`: 该模式将 `image()` 的第二个和第三个参数解释为图像的左上角。
*   `CORNERS`: 该模式将赋予 `image()` 的第二个和第三个参数解释为图像的左上角，第四个和第五个参数解释为图像的右下角。
*   `CENTER`: 该模式将赋予 `image()` 的第二个和第三个参数解释为图像的中心点。此外，如果指定了第四个和第五个参数，它们将用作图像的宽度和高度。
...
[https://p5js.org/reference/#/p5/imagemode](https://p5js.org/reference/#/p5/imagemode)