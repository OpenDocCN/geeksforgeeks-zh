# p5.js TypedDict `print()` 方法

> 原文: [https://www.geeksforgeeks.org/p5-js-typeddict-print-method/](https://www.geeksforgeeks.org/p5-js-typeddict-print-method/)

p5 的 `print()` 方法。p5.js 中的 `TypedDict` 用于将字典中当前存在的所有键值对打印到控制台。键值对是相互映射的两个值的集合。这些值可以通过使用对的关键字部分查询字典来访问。类型化字典可以存储多个键值对，可以使用字典的方法访问这些键值对。
...
function setup() {
  ...
  let stringDict = createStringDict("Tokyo", "37.26 million");
