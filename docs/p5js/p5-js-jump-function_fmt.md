...
`jump()`函数是 p5.js 库中的一个内置函数。该功能用于在播放音频的某个特定点跳转，只需要该音频的时间位置即可。在网上播放这段时间。
...
*   `duration`: 此参数保存一个整数，以秒为单位定义播放持续时间。
...
过了中间位置后可能再按一次按钮，会从中间位置播放另一首时间歌曲。
