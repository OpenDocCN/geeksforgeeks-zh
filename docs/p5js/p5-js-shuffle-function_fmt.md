...
p5.js 中的 `shuffle()` 函数用于对给定数组元素的顺序进行洗牌。
...
## 注意:
在上面的代码中，`draw()` 函数没有被使用，因为如果我们使用 `draw` 函数，那么它不会给出一个明确的输出，即它会不断地改变数组字符串的顺序，并且不会给出任何停滞的结果。

## 参考:
[https://p5js.org/reference/#/p5/shuffle](https://p5js.org/reference/#/p5/shuffle)