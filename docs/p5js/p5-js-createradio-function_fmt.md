...
`createRadio()`函数用于在DOM（文档对象模型）中创建单选按钮元素。`option()`方法用于设置收音机的选项。
此功能需要p5.dom库。因此，在`index.html`文件的头部添加下面一行。
...
**参数:** 该函数接受单个参数`divIde`，分别保存创建的div和输入字段的id和名称。
...
**参考:** [https://p5js.org/reference/#/p5/createRadio](https://p5js.org/reference/#/p5/createRadio)