...
p5.js 中的 `scale()` 函数用于通过扩展或收缩形状或模型的顶点来增加或减小其大小。比例值被指定为十进制百分比，也就是说，比例值 `2.0` 会将形状的尺寸增加 200%。类似地，负值 `-2.0` 会将形状的尺寸减小 200%。
...
*   `scales`: 是 p5.指定按轴百分比缩放对象的向量或数字数组。

## 示例

下面的程序说明了 p5.js 中的 `scale()` 功能。
...
[https://p5js.org/reference/#/p5/scale](https://p5js.org/reference/#/p5/scale)