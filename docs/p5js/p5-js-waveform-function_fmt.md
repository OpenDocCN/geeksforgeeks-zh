...
`waveform()` 函数是 p5.js 库中的一个内置函数。该函数用于返回振幅值，这将是单个缓冲器中振幅读数的快照。这可以用来绘制声音的波形。
...
所有与声音相关的功能只有当声音库包含在 `index.html` 文件的头段中时才起作用。

## 参数

该函数接受两个参数，如上所述，如下所述:

*   `bins`: 该参数用于保存一个整数，该整数将像 2<sup>n</sup> 一样表示。范围是 16 到 1024，默认值是 1024，是可选参数。
*   `precision`: 该参数用于返回 Float32 数组，如果前一个参数保持任何值，该数组比常规数组更精确。它也是一个可选的。
