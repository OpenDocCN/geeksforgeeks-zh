...
`loadXML()`函数用于读取文件或 URL 的内容，并将其作为 XML 对象返回。文件必须存在于草图目录中才能访问。这种方法可用于支持高达 64MB 的文件大小。
...
*   `filename`: 这是一个字符串，表示必须从中加载 XML 数据的文件路径或网址。
...
### 参考:
[https://p5js.org/reference/#/p5/loadXML](https://p5js.org/reference/#/p5/loadXML)