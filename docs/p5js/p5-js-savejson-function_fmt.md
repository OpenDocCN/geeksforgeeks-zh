...
`saveJSON()`函数用于将一个对象或对象数组作为 JSON 对象写入`.json`文件。文件的保存将因网络浏览器而异。
...
*   `optimize`: 这是一个布尔值，它指定在写入 JSON 对象之前是否会删除换行符和空格。这是一个可选参数。
...
[https://p5js.org/reference/#/p5/saveJSON](https://p5js.org/reference/#/p5/saveJSON)