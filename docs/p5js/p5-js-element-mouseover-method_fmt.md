...
> 原文: [https://www.geeksforgeeks.org/p5-js-element-mouseover-method/](https://www.geeksforgeeks.org/p5-js-element-mouseover-method/)

`mouseOver()` 是 p5.js 中的一个方法。每当用户在元素上移动鼠标指针时，都会调用该方法。它可用于将事件侦听器附加到元素。
...
*   `fxn`: 这是一个每当鼠标移动到元素上时都会触发的函数。值为 `false` 也可以传递给它，以防止之前的功能触发。
