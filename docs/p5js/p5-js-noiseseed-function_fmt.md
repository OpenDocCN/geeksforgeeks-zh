...
`noiseSeed()`功能用于设置`noise()`功能的种子值。`noise()`函数默认返回一个半随机的数字，这意味着只有在程序运行之前，一个坐标的噪波值才是相同的。下次运行该程序将产生不同的值。
...
## 参考:
[https://p5js.org/reference/#/p5/noiseSeed](https://p5js.org/reference/#/p5/noiseSeed)