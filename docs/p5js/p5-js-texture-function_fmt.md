...
p5.js 中的 `纹理()` 函数用于为几何对象提供纹理。这个纹理可以是 `p5.Image`，`p5.MediaElement` 或 `p5.Graphics` 对象。
...
*   `tex`: 是 `p5.Image`，`p5.MediaElement` 或 `p5.Graphics` 对象，指定必须在模型上使用的 2D 纹理。
...
[https://p5js.org/reference/#/p5/texture](https://p5js.org/reference/#/p5/texture)