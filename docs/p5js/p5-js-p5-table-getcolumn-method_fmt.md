...
> 原文: [https://www.geeksforgeeks.org/p5-js-p5-table-getcolumn-method/](https://www.geeksforgeeks.org/p5-js-p5-table-getcolumn-method/)

`getColumn()`方法用于检索`p5.Table`中给定列的所有值，并以数组的形式返回它们。要返回的列可以以其标题或标识的形式给出。
...
*   `column`: 是一个字符串或数字，表示要返回的列的标题或编号。

**返回值:** 返回`column`参数指定的列值数组。

以下示例说明了 p5.js 中的 `getColumn()`函数:
