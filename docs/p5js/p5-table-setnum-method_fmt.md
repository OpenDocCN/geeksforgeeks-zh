...
p5.js 中的 `Table` 对象用于存储数据。`setNum()` 方法用于将给定的浮点数（Float）值存储到表的指定行和列中。行可以由行标识（index）指定，列可以由其列标识（index）或列名（name）指定。
...
*   `row`: 表示行标识（index）的数字。
*   `column`: 表示列名（name）或列标识（index）的字符串或数字。
*   `value`: 指定必须存储的数值的数字。
...
[https://p5js.org/reference/#/p5.Table/setNum](https://p5js.org/reference/#/p5.Table/setNum)