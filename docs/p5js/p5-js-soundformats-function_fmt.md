...
`soundFormats()`函数是 p5.js 库中的一个内置函数。该函数用于存储声音文件格式扩展的数组。比如`.mp3`、`.ogg`、`.wav`等。您必须上传服务器中的所有格式。客户端浏览器支持的格式将从此数组中选择并播放。
...
*   `formats`:该参数保存像`'mp3'`这样的文件格式的字符串，是可选参数。

下面的例子说明了 JavaScript 中的 `p5.js soundFormats()`函数:
