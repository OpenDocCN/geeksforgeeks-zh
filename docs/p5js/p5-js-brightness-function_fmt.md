...
p5.js 中的 `brightness()` 函数用于从颜色或像素阵列中提取 HSB 亮度值。
...
下面的程序说明了 p5.js 中的 `brightness()` 函数:

### 示例 1

本示例使用 `brightness()` 函数从颜色或像素阵列中提取 HSB 亮度值。
...
本例使用 `brightness()` 函数提取 HSB 亮度值，填充为灰度整数值。
...
**参考:** [https://p5js.org/reference/#/p5/brightness](https://p5js.org/reference/#/p5/brightness)