...
`noLoop()`功能用于执行`draw()`功能后停止程序。`loop()`函数一次又一次地运行`draw()`函数。如果在`setup()`函数中使用了`noLoop()`函数，那么它应该是块内的最后一行。如果使用`noLoop()`函数，则无法在事件处理函数(如`mousePressed()`或`keyPressed()`中更改或访问屏幕。
