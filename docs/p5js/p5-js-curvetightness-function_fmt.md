...
> 原文: [https://www.geeksforgeeks.org/p5-js-curveTightness-function/](https://www.geeksforgeeks.org/p5-js-curveTightness-function/)

p5.js 中的 `curveTightness()` 函数用于修改使用 `curveVertex()` 和 `curveVertex()` 函数创建的曲线的质量。紧密度参数用于定义曲线如何拟合其顶点。曲线的松紧度默认值为 `0.0`，值 `1.0` 用直线连接所有的点。
...
*   `amount`: 是一个数字，指定曲线从其原始顶点变形的量。该值的范围从 `-5.0` 到 `5.0`。

下面的例子说明了 p5.js 中的 `curveTightness()` 函数:
...
[https://p5js.org/reference/#/p5/curveTightness](https://p5js.org/reference/#/p5/curveTightness)