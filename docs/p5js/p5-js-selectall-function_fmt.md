...
`selectAll()` 函数用于搜索具有给定 id、类或标签名的元素，并将其作为 p5 元素数组返回。它的语法类似于 CSS 选择器。有一个可选参数可用于在给定元素中进行搜索。此方法返回页面上存在的所有元素，并匹配选择器。
...
*   `name`: 这是一个字符串，表示要搜索的元素的 id、类或标签名。
*   `container`: 这是一个可选参数，表示要在其中搜索的元素。

## 返回:

返回 p5.Element 数组，包含所有匹配元素。
...
[https://p5js.org/reference/#/p5/selectall](https://p5js.org/reference/#/p5/selectall)