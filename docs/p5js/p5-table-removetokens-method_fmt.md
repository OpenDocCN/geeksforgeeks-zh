...
`p5.Table` 的 `removeTokens()` 方法用于从表中的值中移除所有指定的字符。可以指定特定的列，以便仅从该列中移除令牌。但是，如果没有指定列，则处理表中所有列和行的值。
...
*   `chars`：它是一个字符串，指定必须删除的所有字符。
*   `column`：它是一个字符串或整数，指定要修剪的列的列名或列标识。这是一个可选参数。

下面的例子说明了 p5.js 中的 `removeTokens()` 方法。
...
**参考:** [https://p5js.org/reference/#/p5.Table/removeTokens](https://p5js.org/reference/#/p5.Table/removeTokens)