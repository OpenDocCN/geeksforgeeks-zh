...
`resizeCanvas()` 函数用于将画布调整到参数给定的高度和宽度。除非可选的 `noRedraw` 参数设置为真，否则在调整大小后画布会立即重绘。
...
*   `w`: 是表示新画布宽度的数字。
*   `h`: 是表示新画布高度的数字。
*   `noRedraw`: 这是一个布尔值，指定画布在调整大小时是否立即重绘。
...
## 参考
[https://p5js.org/reference/#/p5/resizeCanvas](https://p5js.org/reference/#/p5/resizeCanvas)