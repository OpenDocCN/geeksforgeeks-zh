...
`loadStrings()`函数用于读取文件的内容，并用文件的每一行创建一个字符串数组。如果使用文件名，要读取的文件必须位于草图目录，否则，可以指定文件的 URL。
...
## 参数:
该函数接受三个参数，如上所述，如下所述:
*   `filename`: 这是一个字符串，表示文件的名称或加载文件的网址。
*   `callback`: 这是函数成功执行后调用的函数。这个函数的第一个参数是字符串数组。
...
### 参考:
[https://p5js.org/reference/#/p5/loadStrings](https://p5js.org/reference/#/p5/loadStrings)