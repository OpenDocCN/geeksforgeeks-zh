...
> 原文: [https://www.geeksforgeeks.org/p5-js-mediaelement-addcue-method/](https://www.geeksforgeeks.org/p5-js-mediaelement-addcue-method/)

`p5.js`库的`MediaElement`的`addCue()`方法用于调度某个事件，每当媒体元素到达指定的提示点时触发。它接受一个回调函数，该函数可用于指定事件期间应该发生什么操作。还可以使用此方法将可选参数传递给回调函数。
...
*   `time`: 是一个数字，它指定了相对于媒体元素的时间(以秒为单位)，在该时间之后将调用给定的回调函数。
*   `callback`: 它是一个会在给定时间被调用的函数。
*   `value`: 是作为参数传递给回调函数的对象。这是一个可选参数。

## 返回值

这个方法返回一个数字，表示添加的提示的`id`。这可用于以后访问或移除提示。

## 示例 1

以下示例说明了`p5.js`库的`addCue()`方法。
