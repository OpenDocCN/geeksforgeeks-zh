...
*   `name`: 是一个字符串，表示需要搜索的元素的 id、类或标签名。
*   `container`: 这是一个可选参数，表示要搜索的元素。

## 返回值

当给定元素被成功找到时，它返回一个包含该节点的 `p5.element`。否则，它返回 `null`。
...
function mouseClicked() {
  // Select the first canvas element
  selectedCanvas = select("canvas");
  // Get the DOM node using .elt and
  // change background color to green
  selectedCanvas.elt.style.backgroundColor = "green";
}
```
