...
`pan()`函数是 p5.js 库中的一个内置函数。该功能用于控制网络上播放音频的平移。该函数的范围在(-1)和(1)之间，前者表示左侧，后者表示右侧。这种平移也可以通过滑块在不同范围内进行划分来控制。
...
在本例中，音频将在 4 秒钟后在您的左侧播放，然后在 4 秒钟后，在其余时间内将在右侧播放。
...
在本例中，您可以通过滑块左右控制平移效果，反之亦然。首发将是 0，这意味着双方都将被打。
