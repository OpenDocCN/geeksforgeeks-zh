...
每当按下一个键时，就会调用 `keyPressed()` 函数。最近键入的 ASCII 键存储在 `key` 变量中，但是它不区分大写和小写字符。非 ASCII 字符代码可以用它们各自的名称在 `keyCode` 变量中访问。
...
下面的例子说明了 p5.js 中的 `keyPressed()` 函数：
...
function keyPressed() {
  ...
  if (key == "ArrowLeft" && opac > 0)
    opac -= 20;
  // Increase opacity if the right arrow is pressed
