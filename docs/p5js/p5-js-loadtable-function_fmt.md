...
`loadTable()`函数用于读取文件或网址的内容，并创建一个 p5.Table 对象。`options` 参数可用于定义数据预期读取的格式类型。所有加载和保存的文件都采用 UTF 8 编码。
...
*   `filename`: 这是一个字符串，表示必须从其中加载数据的文件路径或网址。
*   `options`: 它是一个字符串，表示要加载的文件的格式。它既可以是使用逗号分隔值加载表的 `"csv"`，也可以是使用制表符分隔值加载表的 `"tsv"`。还可以指定值 `"header"` 来表示该表是否有标题值。可以通过将多个命令作为单独的参数传递来使用它们。这是一个可选参数。
*   `callback`: 这是一个函数，当这个函数执行成功时被调用。这个函数的第一个参数是从文件中加载的 XML 数据。这是一个可选参数。
...
## 参考
[T2]https://p5js.org/reference/#/p5/loadTable