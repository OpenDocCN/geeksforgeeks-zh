...
*   `column`: 是表示列名或列标识的字符串或数字。这是一个可选参数。

**返回值:**这个方法返回一个`TableRow`对象的数组，这些对象匹配给定的正则表达式。

下面的例子说明了p5.js中的`matchRows()`方法:
...
function getMatchedResults() {
  ...
  let matchQuery = matchQueryInput.value();

  if (matchQuery != "") {

    // Match the query in the column of 'name' 
    matchResults = table.matchRows(new RegExp(matchQuery), 'name');
