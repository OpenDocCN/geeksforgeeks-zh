...
> 原文：`https://www.geeksforgeeks.org/p5-js-max-function/`

p5.js 中的 `max()` 函数用于获取数列或两个给定数中的最大值。`max()` 函数接受任意数量的参数或数组。
...
`max(a, b)` 函数接受两个不同个数的参数并进行比较，得到其中的最大值。`max(arr)` 函数接受一个数字数组并返回最大值。
