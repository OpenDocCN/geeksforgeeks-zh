...
p5.js 中的 `Image` 对象的 `resize()` 方法用于将图像调整到给定的宽度和高度。通过使用 `0` 作为宽度或高度值之一，可以按比例缩放图像。
...
*   `width`: 是一个数字，指定调整大小后的图像的宽度。
*   `height`: 是一个数字，指定调整大小后图像的高度。
...
function setup() {
    ...
    heightSlider = createSlider(0, 500, 200);
    heightSlider.position(30, 300);

    widthSlider = createSlider(0, 500, 400);
...
function draw() {
    ...
    text("Move the sliders to resize the image", 20, 20);
...
[https://p5js.org/reference/#/p5.Image/resize](https://p5js.org/reference/#/p5.Image/resize)