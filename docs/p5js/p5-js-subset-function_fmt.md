...
p5.js 中的 `subset()` 函数用于获取给定数组元素的子集。该函数从现有数组中提取元素。
...
## 参数:
该功能接受三个参数，如上所述，描述如下:
*   `Array`: 此参数保存要对其执行 `subset()` 操作以获取其子集元素的数组元素。
*   `Start`: 这是要从哪里提取的数字。它的值从 0 开始。
*   `Count`: 这是要提取的元素数量。
...
## 参考:
[`https://p5js.org/reference/#/p5/subset`](https://p5js.org/reference/#/p5/subset)