...
p5.js 中的 `textLeading()` 函数用于设置文本行之间的间距，单位为像素。该函数用于对 `text()` 函数的所有后续调用。
...
## 参数:
该函数接受单个参数 `leading`，该参数以像素为单位存储行间距的大小。

下面的程序说明了 p5.js 中的 `textLeading()` 函数:

## 示例 1:
本示例使用 `textLeading()` 函数设置文本行之间的间距(以像素为单位)。
...
## 示例 2:
本示例使用 `textLeading()` 函数返回文本行之间的间距，以像素为单位。
...
**参考:** [https://p5js.org/reference/#/p5/textLeading](https://p5js.org/reference/#/p5/textLeading)