...
p5.js 中的 `str()`函数用于将给定的布尔值、字符串和数值转换为其字符串表示形式。
...
## 参数:
该函数接受单个参数 `value`，该值将被转换为其字符串表示。该值可以是整数、浮点、字符串、布尔值、负值或正值以及值数组。
...
## 示例 1:
本示例使用 `str()`函数将给定的输入值转换为其对应的字符串表示形式。
...
## 参考:
[https://p5js.org/reference/#/p5/str](https://p5js.org/reference/#/p5/str)