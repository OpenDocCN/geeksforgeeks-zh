...
`tint()`功能用于设置图像的填充值。它可用于用指定的颜色给图像着色，或通过使用 alpha 值使其透明。可以使用几个参数来指定色调。
...
*   `alpha`: 这是一个定义色调的 alpha 值的数字。
*   `value`: 它是一个字符串，用于定义色调的颜色。
*   `gray`: 是定义色调灰度值的数字。
*   `values`: 它是一组数字，定义了色调颜色的红色、绿色、蓝色和 alpha 分量。
*   `color`: 是 p5.Color 对象，定义色调颜色。

以下示例说明了 p5.js 中的`tint()`功能:
...
**参考:** [https://p5js.org/reference/#/p5/tint](https://p5js.org/reference/#/p5/tint)