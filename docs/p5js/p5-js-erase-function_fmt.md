...
p5.js 中的 `erase()` 功能用于减去使用该功能后将要完成的所有绘图。减去的绘图区域将显示画布下方的网页。使用 `noErase()` 函数可以取消该函数的效果。
...
*   `strengthFill`: 它是 0 到 255 范围内的一个数字，表示擦除形状的填充强度。它是一个可选值。默认值为 255，表示形状填充的全部强度。
*   `strengthStroke`: 是 0 到 255 范围内的数字，表示擦除形状笔画的强度。它是一个可选值。默认值为 255，表示形状笔划的全部强度。

以下示例说明了 p5.js 中的 `erase()` 功能:
...
## 参考:
[https://p5js.org/reference/#/p5/erase](https://p5js.org/reference/#/p5/erase)