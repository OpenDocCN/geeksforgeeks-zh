...
p5.js 中的 `nfs()`函数用于将输入数字（整数或浮点数）格式化为字符串，并在正输入数字前添加“ ”（空格），在负输入数字前添加负号（-）。此功能在对齐时使用。
...
*   `Left`: 这是正数，表示位数应该在小数点左边。
*   `Right`: 这是正数，表示位数应该在小数点右边。
...
本示例使用 `nfs()`函数格式化输入数字，并在正数前添加单个空格以设置对齐。
...
本示例使用 `nfs()`函数格式化输入数字，并在正数前添加空格以设置对齐。
...
[https://p5js.org/reference/#/p5/nfs](https://p5js.org/reference/#/p5/nfs)