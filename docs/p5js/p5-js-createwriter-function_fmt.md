...
p5.js 中的 `createWriter()`函数用于创建 `p5.PrintWriter` 对象，可用于写入或打印各种可用的流。
...
*   `name`: 是表示要创建的文件名称的字符串。
*   `extension`: 是指定文件扩展名的字符串。这是一个可选参数。

## 返回值:
返回 `p5.PrintWriter` 对象，表示编写器。
...
**参考:**[https://p5js.org/reference/#/p5/createWriter](https://p5js.org/reference/#/p5/createWriter)