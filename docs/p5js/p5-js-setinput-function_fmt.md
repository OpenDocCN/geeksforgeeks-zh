...
`setInput()`函数是 p5.js 库中的一个内置函数。该函数用于连接默认情况下作为主输出的 p5sound 实例。通过使用这个函数，您还可以传入一个特定的源。
...
*   `snd`: 此参数用于设置声源，可选。
*   `smoothing`: 此参数用于设置平滑幅度读数，范围在 0.0 到 1.0 之间，此参数也是可选的。
