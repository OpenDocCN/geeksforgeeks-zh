...
`processPeak()`函数是 p5.js 库中的一个内置函数。该功能用于跟踪音频节拍。该函数在脱机音频环境中执行声音文件，并将该数据发送给回调函数。
...
### `callback`
此参数是音频到达缓冲区末尾时将调用的函数的名称。

### `initThreshold`
该参数保存初始阈值。初始阈值默认为 0.9，是可选参数。

### `minThreshold`
该参数保持最小阈值。最小阈值默认为 0.22，它是可选参数。

### `minPeaks`
此参数保存最小峰值。最小峰值默认为 200，这是可选参数。
