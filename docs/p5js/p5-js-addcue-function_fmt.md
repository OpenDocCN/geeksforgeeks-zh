...
`addCue()`函数是 p5.js 库中的一个内置函数。该功能用于在每个特定时间，在正在播放的加载音频的特定点触发特定任务。
...
*   `time`: 该参数保存该时间将触发的事件的时间。
*   `callback`: 此参数是音频到达指定时间时将调用的函数的名称。
*   `value`: 该参数作为回调作为第二个参数传递，可选。
