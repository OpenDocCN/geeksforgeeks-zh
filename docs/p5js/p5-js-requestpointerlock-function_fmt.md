...
> 原文: [https://www.geeksforgeeks.org/p5-js-requestpointerlock-function/](https://www.geeksforgeeks.org/p5-js-requestpointerlock-function/)

函数`requestPointerLock()`用于将指针锁定到当前位置，然后使其不可见。所有浏览器都不支持此方法。它请求将指针锁定到一个 DOM 元素目标。

我们可以使用`movedX`和`movedY`来获得自上次调用`draw`以来鼠标移动的差异。它使我们能够创建不依赖于鼠标移动的场景。
