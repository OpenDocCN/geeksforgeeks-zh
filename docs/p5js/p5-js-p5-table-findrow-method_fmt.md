# p5.js Table.findRow()方法

> 原文: [https://www.geeksforgeeks.org/p5-js-p5-table-findrow-method/](https://www.geeksforgeeks.org/p5-js-p5-table-findrow-method/)

`p5.js` 中 `Table` 对象的 `findRow()` 方法用于查找包含给定值的第一行，并向该行返回一个值。方法用来搜索行的列可以指定为参数。即使存在多个匹配项，该方法也只返回可能匹配项的第一行。
...
function setup() {
  ...
  getColBtn = createButton("Get the matching row");
...
function getFindResults() {
  ...
  // Get the row values using findRow()
  if (findQuery != "") {

    // Find the result in the column of 'name'
    findResults = table.findRow(findQuery, 'name');
...
function showTable() {
  ...
  for (let r = 0; r < table.getRowCount(); r++)
    for (let c = 0; c < table.getColumnCount(); c++)
      text(table.getString(r, c), 20 + c * 100, 140 + r * 20);
...
function getFindResults() {
  ...
  if (findQuery != "") {
    ...
    // Find the result in the column with ID of '1'
    findResults = table.findRow(findQuery, 1);
...
## 参考:
[https://p5js.org/reference/#/p5.Table/findRow](https://p5js.org/reference/#/p5.Table/findRow)