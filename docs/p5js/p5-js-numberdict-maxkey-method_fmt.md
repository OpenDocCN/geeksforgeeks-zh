...
> 原文: [https://www.geeksforgeeks.org/p5-js-numberdict-maxkey-method/](https://www.geeksforgeeks.org/p5-js-numberdict-maxkey-method/)

p5 的 `maxKey()` 方法。p5.js 中的 `NumberDict` 用于查找数字字典中最高的键值。键值对是相互映射的两个值的集合。这些值可以通过使用对的关键字部分查询字典来访问。数字字典可以存储多个键值对，可以使用字典的方法访问这些键值对。
...
function setup() {
  ...
  setBtn = createButton("Create random dictionary");
