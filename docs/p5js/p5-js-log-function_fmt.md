...
p5.js 中的 `log()`函数用于获取 `log()`函数参数输入的任意数字的自然对数(以“e”为底)。
...
该函数接受单个参数 `x`，该参数为大于零(0)的任意数字，作为将要计算其自然对数的输入。
...
如果我们将输入作为负值，取零，那么它将输出分别作为“NaN”和-Infinity 返回。

## 参考

[https://p5js.org/reference/#/p5/log](https://p5js.org/reference/#/p5/log)