...
> 原文：[https://www.geeksforgeeks.org/how-to-draw-simple-animation-using-p5-js/](https://www.geeksforgeeks.org/how-to-draw-simple-animation-using-p5-js/)
...
*   设置函数 `setup()`，其中输出窗口的大小、颜色和背景，将 `iter` 和 `counter` 的值初始化为 1，并初始化顶点列表。
*   设置添加笔画、笔画粗细的函数。
*   设置一个 `if` 条件，在范围内检查 `iter`，如果是，将 `counter` 增加 0.05，如果没有出口，则将 `counter` 的上限值设为 `iter`。
*   函数添加房屋的顶点，将直线的起点和终点作为 `addVertices()`。
*   现在制作绘制房屋各部分的函数：
*   使函数在房屋中绘制垂直线和水平线。
*   制作绘制方形窗口的函数。
*   使函数绘制门。
