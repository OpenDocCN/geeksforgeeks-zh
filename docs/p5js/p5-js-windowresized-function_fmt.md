...
p5.js 中的 `windowResized()` 函数在每次浏览器窗口调整大小时被调用一次。只要窗口的大小增加，它就会自动调整高度和宽度。一旦调整了窗口的大小，就会自动调用这个函数，然后创建一个与之对应的新画布。
...
下面的程序说明了 p5.js 中的 `windowResized()` 函数。
...
**参考:** [https://p5js.org/reference/#/p5/windowResized](https://p5js.org/reference/#/p5/windowResized)