...
函数 `loadShader()` 从文件中加载着色器，并将它们作为变量中的 p5 着色器对象返回。文件扩展名可以是 `.glsl` 或 `.vert` 或 `.frag`。

应该从 `preload()` 中调用它 `loadShader()`。
...
### 参数:
*   `vertFilename` : 字符串类型，包含顶点着色器源代码的文件路径。
*   `fragFilename` : 字符串类型，包含片段着色器源代码的文件路径。
*   `callback` : 是 `loadShader` 完成后执行的函数。成功后，着色器对象作为第一个参数传递。它是可选的。
*   `errorCallback` : 是 `loadShader` 内部出现错误时执行的函数。当出现错误时，错误作为第一个参数传递。它是可选的。
