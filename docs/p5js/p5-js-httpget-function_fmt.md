...
p5.js 中的 `httpGet()`函数用于执行一个 httpGet 请求。p5 会根据 URL 自动猜测返回的数据类型，如果没有指定的话。
...
返回一个承诺，当操作成功完成时可以用数据解决，当出现错误时可以拒绝。

以下示例说明了 p5.js 中的 `httpGet()`函数。
...
[https://p5js.org/reference/#/p5/httpGet](https://p5js.org/reference/#/p5/httpGet)