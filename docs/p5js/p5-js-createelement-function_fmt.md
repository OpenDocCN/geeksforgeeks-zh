...
`createElement()`函数用于在 DOM（文档对象模型）中创建一个元素。`position()`函数用于设置元素的位置。
...
**参数:** 该函数接受两个参数，如上所述，描述如下:

*   `tag`: 该参数保存新元素的标签。
*   `content`: 此参数保存要插入元素的 HTML 内容。
...
**参考:** [https://p5js.org/reference/#/p5/createElement](https://p5js.org/reference/#/p5/createElement)