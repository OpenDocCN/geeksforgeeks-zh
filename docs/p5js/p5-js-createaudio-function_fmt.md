...
*   `callback`: 这是一个回调函数，当‘can playthrough’事件触发时会被触发。当音频完成加载且不需要任何额外缓冲时，将触发此事件。这是一个可选参数。

## 返回值:
返回一个指向`p5.MediaElement`的指针。
...
**参考:**[https://p5js.org/reference/#/p5/createAudio](https://p5js.org/reference/#/p5/createAudio)