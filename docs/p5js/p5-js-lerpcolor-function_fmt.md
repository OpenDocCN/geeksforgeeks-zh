...
*   `c1`：是`p5.Color`，代表最终颜色将被插值的第一种颜色。
*   `c2`：是`p5.Color`，表示最终颜色将被插值到的第二种颜色的颜色。
*   `amt`：它是一个介于 0 和 1 之间的数字，决定了哪种颜色将更多地用于插值。接近 0.1 的值更喜欢第一种颜色，接近 0.9 的值更喜欢第二种颜色用于插值。

**返回值：** 返回`p5.Color`，具有插值颜色的颜色元素。
...
**参考：** [https://p5js.org/reference/#/p5/lerpColor](https://p5js.org/reference/#/p5/lerpColor)