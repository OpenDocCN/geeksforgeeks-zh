...
`noise()`函数用于返回给定坐标下由柏林噪声产生的数字。该值是半随机的，这意味着在程序的生命周期内，坐标的值是固定的。

柏林噪声值不同于`random()`函数返回的值，因为与标准噪声相比，这种噪声具有更自然和更和谐的连续性。
...
function draw() {
    ...
    // Increase the x coordinate for plotting
    plot_x++;
}
```
...
function draw() {
    if (x_coordinate < 5) {
        ...
        // Output the noise along with its corresponding coordinate
        ...
        // Increase the y coordinate for plotting
        plot_y = plot_y + 15;
    }
    else
        x_coordinate = 0;
...
**参考：** [https://p5js.org/reference/#/p5/noise](https://p5js.org/reference/#/p5/noise)