...
> 原文：`https://www.geeksforgeeks.org/p5-js-specularcolor-function/`

p5.js 中的 `specularColor()` 函数用于设置镜面高光在与镜面材质和高光一起使用时的颜色。

该方法通常与 `specularMaterial()` 和 `shininess()` 函数一起使用来定义镜面高光。如果没有 `specularMaterial()` 函数，这个函数就没有效果。如果不使用此功能，默认的高光颜色是白色。
...
*   `value`：是定义高光颜色的字符串。
*   `gray`：是定义镜面高光灰度值的数字。
*   `values`：它是一个数字数组，定义了高光颜色的红色、绿色、蓝色和 alpha 分量。
*   `color`：是 p5.Color 对象，定义高光颜色。

下面的例子说明了 p5.js 中的 `specularColor()` 函数：
