...
p5.js 中的 `saturation()` 函数用于从颜色或像素阵列中提取 HSL 和 HSB 饱和度值。
...
该功能接受存储 p5 的单参数 `c`。颜色对象、颜色组件或 CSS 颜色。

下面的程序说明了 p5.js 中的 `saturation()` 函数:

### 示例 1:

本示例使用 `saturation()` 函数从像素阵列中提取 HSL 和 HSB 饱和度值。
...
本示例使用 `saturation()` 函数从像素阵列中提取 HSL 和 HSB 饱和度值，并将其填充为灰度值。
...
[https://p5js.org/reference/#/p5/saturation](https://p5js.org/reference/#/p5/saturation)