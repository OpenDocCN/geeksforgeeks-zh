...
*   `基本材质`: 用给定的颜色填充几何体，但不受光线影响。用 `fill()` 方法定义。
*   `正常材质`: 不取任何参数，自动映射到 RGB 颜色空间。使用 `normalMaterial()` 方法定义。
*   `环境材料`: 只有当有特定颜色的光时，它才会反射。用 `ambientMaterial()` 方法定义。
*   `镜面材质`: 是四种材质中最逼真的。镜面反射是一种技术方法，用来描述一种向单一方向反射光线的材料。它是用 `specularMaterial()` 方法定义的。
...
## 示例 3: 在没有光线的情况下使用 `ambientMaterial()`
