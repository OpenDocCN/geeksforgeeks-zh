...
p5.js 中的 `unchar()`函数用于将单个字符串转换为其对应的整数表示。这个函数与 `char()`函数正好相反。
...
## 参数:
该函数接受单个参数 `value`，该值将被转换为其对应的整数表示。该值可以是任何单字符字符串或单字符数组。
...
### 示例 1:
本示例使用 `unchar()`函数将单个字符串转换为其对应的整数表示形式。
...
参考: [https://p5js.org/reference/#/p5/unchar](https://p5js.org/reference/#/p5/unchar)