...
p5.js 中的系统变量 `rotationZ` 负责移动设备(智能手机和平板电脑)始终沿着 z 轴的旋转。它可以在 `draw()` 函数中使用，以连续获取当前沿 z 轴的旋转。如果图形 `angleMode()` 设置为度数，则该值将在 0 到 360 的范围内。当设置为 `RADIANS` 时，该值将为 0 到 `2*PI`。该变量仅适用于内置指南针的设备。
