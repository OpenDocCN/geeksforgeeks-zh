...
p5.js 中的 `p5.Table` 的 `findRows()` 方法用于查找包含给定值的所有行，并返回对这些行的引用。该方法搜索的列可以指定为参数。
...
function setup() {
  ...
  getColBtn = createButton("Get the matching rows");
...
function getFindResults() {
  ...
  if (findQuery != "") {
    ...
    findResults = table.findRows(findQuery, 'fruit');

    if (findResults.length > 0) {
      text("The rows that match the query are", 20, 120);

      // Display the matched rows
      for (let i = 0; i < findResults.length; i++) {
        text(findResults[i].arr[0], 20, 140 + i * 20);
        text(findResults[i].arr[1], 120, 140 + i * 20);
      }
    } else text("No Results Found", 20, 120);
  } else text("The query string is empty", 20, 120);

  text("Enter a string to find it" + " in the table", 20, 20);
}
...
function getFindResults() {
  ...
  if (findQuery != "") {
    ...
    findResults = table.findRows(findQuery, 'price');
...
**参考:** [https://p5js.org/reference/#/p5.Table/findRows](https://p5js.org/reference/#/p5.Table/findRows)