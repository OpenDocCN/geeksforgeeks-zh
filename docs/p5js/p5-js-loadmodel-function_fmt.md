...
`loadModel()` 函数用于从文件中加载三维模型，并将其作为 p5.Geometry 对象返回。该模型可以加载格式 `.obj` 或 `.stl`。

OBJ 和 STL 加载的文件没有任何缩放感，因此可能需要使用 `normalize` 参数，以便模型自动缩放到正确的大小。加载模型后，可以使用 `scale()` 函数进一步更改模型的大小。

这种方法本质上是异步的，因此在使用模型之前可能不会完成。因此，建议在 `preload()` 函数中加载模型。
...
[https://p5js.org/reference/#/p5/loadModel](https://p5js.org/reference/#/p5/loadModel)