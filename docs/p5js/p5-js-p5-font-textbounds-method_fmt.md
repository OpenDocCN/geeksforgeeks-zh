...
> 原文: [https://www.geeksforgeeks.org/p5-js-p5-font-textbounds-method/](https://www.geeksforgeeks.org/p5-js-p5-font-textbounds-method/)

`p5.js` 中的 `textBounds()` 方法为使用给定字符串所应用的字体返回一个紧密的边界框。此方法仅支持单行。
...
*   `x`: 是表示 x 位置的数字。
*   `y`: 是表示 y 位置的数字。
*   `fontSize`: 这是一个数字，表示要使用的字体大小。默认值为 `12`。这是一个可选参数。
*   `options`: 是一个可以用来指定 opentype 选项的对象。Opentype 字体包含对齐和基线选项等选项。默认值为 `"left"` 和 `"alphabetic"`。这是一个可选参数。
