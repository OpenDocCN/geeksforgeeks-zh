...
`saveStrings()`函数用于向文件中写入一个字符串数组，每个字符串一行。文件的保存将因网络浏览器而异。
...
*   `extension`: 指定用作保存文件扩展名的字符串。这是一个可选参数。
...
[T2】https://p5js.org/reference/#/p5/saveStrings](https://p5js.org/reference/#/p5/saveStrings)