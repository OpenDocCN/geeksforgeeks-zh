...
p5.js 中的 `randomSeed()` 函数用于每次运行程序时返回一个随机数。`random()` 和 `randomSeed()` 函数的区别在于，`random()` 函数在每次运行程序时都会产生不同的值，但是当使用 `randomSeed()` 函数时，它会在每次运行程序时给出一个恒定的随机数。
...
本示例使用 `randomSeed()` 函数在每次运行程序时返回一个随机数。
...
**参考:** [https://p5js.org/reference/#/p5/randomSeed](https://p5js.org/reference/#/p5/randomSeed)