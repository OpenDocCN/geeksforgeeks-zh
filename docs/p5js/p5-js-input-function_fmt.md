...
只要在元素上检测到用户输入，就会调用`input()`功能。它可以用来检测击键或滑块值的变化。它还可以用于将事件侦听器附加到元素。
...
*   `fxn`: 这是每当检测到输入时都会调用的回调函数。它可以被传递为`false`，这将阻止先前的触发功能停止触发。

下面的例子说明了 p5.js 中的`input()`功能:
...
**参考:** [https://p5js.org/reference/#/p5/input](https://p5js.org/reference/#/p5/input)