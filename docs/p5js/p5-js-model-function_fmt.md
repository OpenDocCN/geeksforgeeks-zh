...
`model()`功能用于将 3D 模型渲染到屏幕上。必须首先使用`loadModel()`函数加载要渲染的模型。
...
*   `model`: 是一个 p5.Geometry 对象，指定必须渲染到屏幕上的模型。

## 示例:

下面的程序说明了 p5.js 中的`model()`功能:
...
[https://p5js.org/reference/#/p5/model](https://p5js.org/reference/#/p5/model)