...
> 原文: [https://www.geeksforgeeks.org/p5-js-mediaelement-duration-method/](https://www.geeksforgeeks.org/p5-js-mediaelement-duration-method/)

`p5.js` 库中的 `MediaElement` 的 `duration()` 方法用于返回调用它的媒体元素的持续时间。返回的持续时间以秒为单位显示。
...
下面的示例说明了 `p5.js` 库中的 `duration()` 方法。
...
function setup() {
    ...
    example_media = createVideo("sample-video.mp4");
    ...
    let durationBtn = createButton("Get Duration");
...
function getDuration() {
    ...
    let media_duration = example_media.duration();
