...
`rate()`函数是 p5.js 库中的一个内置函数。该功能用于控制网络上播放音频的速度。这个速度也可以通过滑块在不同的范围内划分来控制。
...
*   `playbackRate`:此参数用于保存播放速度速率的值。

下面的例子说明了 JavaScript 中的 `p5.rate()`函数:

### Example 1

In this example the audio will play 2x time faster than normal rate.
...
在本例中，音频将以正常速度播放，但您可以更改。通过滑动滑块，滑块的每一步都会加快 0.2 倍。范围是 1 到 2，意味着正常速度要快 2 倍。
