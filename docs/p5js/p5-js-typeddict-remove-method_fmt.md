...
> 原文: [https://www.geeksforgeeks.org/p5-js-typeddict-remove-method/](https://www.geeksforgeeks.org/p5-js-typedict-remove-method/)

`remove()`方法用于从p5.js的`TypedDict`中移除给定的键值对。键值对是相互映射的两个值的集合。这些值可以通过使用对的关键字部分查询字典来访问。类型化字典可以存储多个键值对，可以使用字典的方法访问这些键值对。
...
function setup() {
  ...
  let stringDict = createStringDict(tmpObj);
  ...
  let existOne = stringDict.hasKey("Statue of Unity");
  text("Dictionary has key " +
       "'Statue of Unity': " +
       existOne, 20, 80);

  let existTwo = stringDict.hasKey("Spring Temple Buddha");
  text("Dictionary has key " +
  ...
  text("One key removed from the dictionary", 20, 140);
  ...
  existTwo = stringDict.hasKey("Spring Temple Buddha");
  text("Dictionary has key " +
       "'Spring Temple Buddha': " +
       existTwo, 20, 200);

  // Checking the other keys
  let existThree = stringDict.hasKey("Ushiku Daibutsu");
  text("Dictionary has key " +
       "'Ushiku Daibutsu': " +
       existThree, 20, 220);
}
