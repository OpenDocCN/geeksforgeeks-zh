...
每当鼠标或触摸板滚动导致垂直鼠标滚轮事件时，就会调用`mouseWheel()`功能。可以访问该事件来确定滚动的属性。`delta`属性返回发生的滚动量。该值可以是正值或负值，具体取决于滚动方向。
...
*   `event`: 这是一个可选的`WheelEvent`回调参数，可以用来访问滚动细节。
...
## 参考
[https://p5js.org/reference/#/p5/mouseWheel](https://p5js.org/reference/#/p5/mouseWheel)