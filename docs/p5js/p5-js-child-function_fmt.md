...
`child()`函数用于将元素作为子元素添加到父元素中。这个函数接受字符串标识、DOM 节点或 `p5.Element`。如果这个函数不包含任何参数，那么它返回一个子 DOM 节点数组。
...
该函数接受单个参数 `child`，它保存字符串或 `p5.Element` 的 ID、DOM 节点或 `p5.Element`，用于作为当前元素添加。
