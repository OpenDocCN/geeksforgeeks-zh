...
另外，Processing 是一种强调可行性的语言，程序员可以非常快速地创建软件原型，尝试一个新的想法，或者看看某件事是否可行。因此，处理（和 `p5.js`）程序通常被称为“草稿”。

## 首选编辑器

`p5.js` 的官方文档建议使用括号或崇高，然后包含 `JavaScript` 文件，最终引导我们像任何其他编程语言一样工作。但是在线 `p5.js` 网络编辑器是最好的选择。它基于一个基于网络的编程环境。

## P5.js 和 JavaScript 的区别？

`JavaScript` 是一种核心语言，它提供了在浏览器中构建任何功能的所有特性。它可以使用循环、函数、条件、DOM 操作、事件、画布等。因此，通过使用它来开发和设计任何框架。

`p5.js` 是一个 `JavaScript` 库。`p5.js` 运行在纯 `JavaScript` 上提供了一些功能，使得 `JavaScript` 用户的生活在 `web` 中很容易画出来。
