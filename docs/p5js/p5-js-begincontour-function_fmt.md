...
p5.js 中的 `beginContour()` 函数用于在其他形状中创建负形状，也就是说，它可以用于移除具有给定顶点的形状的一部分。此功能开始记录必须移除的形状。它与停止记录顶点的 `endContour()` 功能一起使用。

内部形状的顶点必须定义在与外部形状相反的方向上。如果外部形状的顶点是按顺时针顺序定义的，那么内部形状必须按逆时针方向定义。

该功能只能在 `beginShape()` 或 `endShape()` 功能中使用。像 `translate()`、`rotate()` 和 `scale()` 这样的变换不适用于形状和轮廓。
...
下面的程序说明了 p5.js 中的 `beginContour()` 函数:
...
**参考:** [https://p5js.org/reference/#/p5/beginContour](https://p5js.org/reference/#/p5/beginContour)