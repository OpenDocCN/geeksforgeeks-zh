...
p5.js 中的系统变量 `rotationX` 负责移动设备(智能手机和平板电脑)始终沿着 x 轴的旋转。它可以在 `draw()` 函数中使用，以连续获取当前沿 x 轴的旋转。如果图形 `angleMode()` 设置为度数，则该值将在-180 到 180 的范围内。当它被设置为 `RADIANS` 时，该值将是-`PI` 到 `PI`。
