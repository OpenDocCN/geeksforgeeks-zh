...
> 原文: [https://www.geeksforgeeks.org/linestyles-in-matplotlib-python/](https://www.geeksforgeeks.org/linestyles-in-matplotlib-python/)
...
绘制数据时的默认线型是 `matplotlib` 中的实体线型。我们可以使用 `plot()` 方法的 `linestyle` 或 `ls` 参数来更改此线型。
...
*   导入模块。
*   创建数据。
*   通常通过设置 `plot()` 方法的 `linestyle` 或 `ls` 参数来绘制数据。
*   显示图。
...
使用 `linestyle` 或 `ls` 参数可以可视化上述所有类型的线型。可视化时，`None` 或 `' '` 将生成空白图。