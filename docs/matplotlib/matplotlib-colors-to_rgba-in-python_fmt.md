...
> 原文: [https://www.geeksforgeeks.org/matplotlib-colors-to_rgba-in-python/](https://www.geeksforgeeks.org/matplotlib-colors-to_rgba-in-python/)
...
*   `alpha`: 它是一个接受标量的可选参数。如果 `alpha` 不是 `None`，则强制使用 `alpha` 值。但是如果 `c` 是 `"none"`（区分大小写），它就会映射到 `(0, 0, 0, 0)`。
