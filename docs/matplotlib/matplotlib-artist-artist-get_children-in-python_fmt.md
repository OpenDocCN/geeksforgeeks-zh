# matplotlib.artist.Artist.get_children()方法

> 原文：[https://www.geeksforgeeks.org/matplotlib-artist-artist-get_children-in-python/](https://www.geeksforgeeks.org/matplotlib-artist-artist-get_children-in-python/)

[`matplotlib`](https://www.geeksforgeeks.org/python-introduction-matplotlib/)是 Python 中的一个库，它是 `NumPy` 库的数字-数学扩展。`Artist`类包含将渲染到图形画布中的对象的抽象基类。图形中所有可见的元素都是`Artist`的子类。

`matplotlib`库的`artist`模块中的`get_children()`方法用于返回该艺术家的子艺术家列表。

> **句法：** `Artist.get_children(self)`
>
> **参数：** 此方法不接受任何参数。
>
> **返回：** 该方法返回该艺术家的子艺术家列表。

下面的例子说明了`matplotlib.artist.Artist.get_children()`函数在`matplotlib`中的作用：
...
> Ellipse(xy=(6.39084444414, width=0.3251963397808111, angle=131.180626549346)
> Ellipse(xy=(6.719146468170946, 3.636051606867032), width=0.75019420338975