...
`语法:` `Cursor(ax, horizOn=True, vertOn=True, useblit=False, **lineprops)`

`参数:`
*   `ax:` 光标要附着的轴。
*   `可选参数:`
    *   `horizOn:` 画水平线(默认:`True`)。
    *   `vertOn:` 绘制垂直线(默认:`True`)。
    *   `useblit:` 如果后端支持，使用块传送以加快绘图速度(默认:`False`)。
    *   `**lineprops:` 控制线条外观(线宽、颜色)的线条属性。
