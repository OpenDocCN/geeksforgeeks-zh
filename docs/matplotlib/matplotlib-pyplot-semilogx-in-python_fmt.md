...
此功能用于以 x 轴转换为日志格式的方式可视化数据。当其中一个参数非常大，因此最初以紧凑的方式存储时，此功能特别有用。它支持 [`plot()`](https://www.geeksforgeeks.org/matplotlib-pyplot-plot-function-in-python/) 和 [`matplotlib.axes.Axes.set_xscale()`](https://www.geeksforgeeks.org/matplotlib-axes-axes-set_xscale-in-python/) 的所有关键字参数。附加参数为 `basex`、`subsx` 和 `nonposx`。
...
*   `subsx`: (可选) 次轴的位置；`None` 默认为自动选择，这取决于图中的十年数。
*   `nonposx`: (可选) x 中的非正值可以被屏蔽为无效，或者被裁剪为非常小的正数。
*   `marker`: (可选) 将点显示为上述符号。
...
### 示例 3: 如果使用 `'g^'` 符号，则简单地移除负值或零值，仅绘制正值
