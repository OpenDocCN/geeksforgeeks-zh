...
> 原文：[https://www.geeksforgeeks.org/how-to-plot-two-dotted-lines-and-set-marker-using-matplotlib/](https://www.geeksforgeeks.org/how-to-plot-two-dotted-lines-and-set-marker-using-matplotlib/)

在本文中，我们将使用 python 编程语言中 `matplotlib` 包的各种函数绘制两条虚线并设置标记。

我们可以使用 [`pyplot.plot`](https://www.geeksforgeeks.org/matplotlib-pyplot-plot-function-in-python/) 和 `linestyle` 参数函数来绘制虚线。
...
在本例中，我们创建了四个列表（数据点），然后使用一个 [`sin`](https://www.geeksforgeeks.org/numpy-sin-python/) 函数绘制第一条线，通过提及标签使用虚线样式的 `NumPy` 模块，然后使用 [`cos`](https://www.geeksforgeeks.org/numpy-cos-python/) 函数绘制第二条线，通过提及标签使用两个数据点使用虚线样式的 `NumPy` 模块。
