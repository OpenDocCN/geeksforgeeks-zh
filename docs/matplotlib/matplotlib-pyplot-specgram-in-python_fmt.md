...
matplotlib 库 pyplot 模块中的 `specgram()` 函数用于绘制谱图。
...
> *   `x`: 这个参数是一个数据序列。
> *   `Fs`: 此参数为标量。它的默认值是 2。
> *   `window`: 该参数以一个数据段为自变量，返回该段的窗口版本。它的默认值是 `window_hanning()`。
...
> *   `mode`: 这个参数就是用什么样的频谱 `{ 'default', 'psd', 'magnitude', 'angle', 'phase' }`。
> *   `noverlap`: 此参数是块之间重叠的点数。
> *   `scale`: 此参数包含规格 `{ 'default', 'linear', 'dB' }` 中值的比例。
> *   `Fc`: 该参数为 `x` 的中心频率。
