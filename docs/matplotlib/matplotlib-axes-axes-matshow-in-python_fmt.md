## matplotlib.axes.Axes.matshow()函数详解
...
`matplotlib`库的`Axes`模块中的`Axes.matshow()`函数也用于将 2D 矩阵或数组的值绘制为彩色编码图像。
...
下面的例子说明了`matplotlib.axes.Axes.matshow()`函数在`matplotlib.axes`中的作用:
