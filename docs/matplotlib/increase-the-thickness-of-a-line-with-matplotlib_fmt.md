# 用 Matplotlib 增加线条的粗细

> 原文:[https://www.geeksforgeeks.org/increase-the-thickness-of-a-line-with-matplotlib/](https://www.geeksforgeeks.org/increase-the-thickness-of-a-line-with-matplotlib/)
...
> *   `fontdict`: 用于覆盖标签的默认字体属性。其默认值为“无”，并且是可选的。
> *   `labelpad`: 默认值为无。它用于指定标签与轴的间距。这是可选的。
> *   `**kwargs`: 用于指定可用于修改标签外观的其他属性。

*   `ylabel()` - 此功能用于设置 y 轴的标签

> **语法:** `plt.ylabel(ylabel, fontdict=None, labelpad=None, **kwargs)`
>
> **参数:**
>
...
*   `plot()` - 它用来制作点 x，y 的 2D 六边形宁滨图

> **语法:** `plt.plot(x, y, data=None, **kwargs)`
>
> **参数**
>
> *   `x, y`: 用于指定沿 x 轴和 y 轴绘制的数据。
> *   `data`: 默认值为无。它是一个带有标签数据的对象，可以代替 x，y 值传递。如果数据对象被传递，那么应该指定 x 和 y 标签。
> *   `**kwargs`: 用于指定线条属性，如线宽、颜色、抗锯齿、标记、标记颜色等。

*   `legend()` - 图例是描述图形元素的区域。在 `matplotlib` 库中，有一个名为 `legend()` 的函数，用于在轴上放置一个图例。

> **语法:** `plt.legend(**options)`
>
> **参数**
>
> *   `**options`: 用于指定图例的属性、大小、位置、边缘颜色、面颜色等
