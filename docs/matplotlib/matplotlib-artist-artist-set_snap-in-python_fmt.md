# Python 中的 `matplotlib.artist.Artist.set_snap()`

> 原文: [https://www.geeksforgeeks.org/matplotlib-artist-artist-set_snap-in-python/](https://www.geeksforgeeks.org/matplotlib-artist-artist-set_snap-in-python/)
...
`matplotlib` 库的 `artist` 模块中的 `set_snap()` 方法用于设置捕捉行为。
...
以下示例说明了 `matplotlib` 中的 `matplotlib.artist.Artist.set_snap()` 函数:
