# Python中的matplotlib.axes.Axes.get_autoscalex_on()

> 原文：[https://www.geeksforgeeks.org/matplotlib-axes-axes-get_autoscalex_on-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-get_autoscalex_on-in-python/)
...
## matplotlib.axes.Axes.get_autoscalex_on()函数

`matplotlib`库的`Axes`模块中的`Axes.get_autoscalex_on()`函数用于获取x轴的自动缩放是否应用于绘图命令。
...
下面的例子说明了`matplotlib.axes.Axes.get_autoscalex_on()`函数在`matplotlib.axes`中的作用：
