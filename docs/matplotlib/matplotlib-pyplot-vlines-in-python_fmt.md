...
在上面的示例中，`vlines()` 函数开头的第一个参数显示了要生成垂直线的轴点，下一个参数指的是垂直线总长度的下限，而第三个参数指的是垂直线总长度的上限。在这三个基本参数之后，我们可以使用的下一个参数是 `linestyles`，它决定了要绘制的线的类型(例如虚线、实线、虚线、虚线等)。)，最后一个使用的参数是 `colors`，可选，基本上是数组状的颜色。
