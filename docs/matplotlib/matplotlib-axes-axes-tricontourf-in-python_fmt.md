# Matplotlib 中的 axes.Axes.tricontourf()
...
## axes.Axes.tricontourf() 函数

`matplotlib`库的`Axes`模块中的`axes.tricontourf()`函数也用于在非结构化的三角形网格上绘制轮廓。`tricontour`和`tricontourf`分别绘制等高线和填充等高线。
...
*   `triangulation`: 该参数是一个`Triangulation`对象。
*   `Z`: 此参数是要进行轮廓的值的数组，三角测量中每个点一个。
*   `**kwargs`: 此参数是用于控制标签外观的`Text`属性。
    所有剩余的参数和`kwargs`与`matplotlib.pyplot.plot()`相同。

**注意:** 仅`tricontourf`关键字参数:
`antialiased`: 此参数是一个`bool`，使能抗锯齿，用于非结构化三角形网格上的轮廓。

**返回:** 返回包含以下内容的 2 行 2D 列表:
*   为三角形边缘绘制的线。
*   为三角形节点绘制的标记。

下面的例子说明了`matplotlib.axes.Axes.tricontourf()`函数在`matplotlib.axes`中的作用:
