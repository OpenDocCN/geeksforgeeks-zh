...
> **x，y:** 箭基的 `x` 和 `y` 坐标。
> **dx、dy:** 箭头沿 `x`、`y` 方向的长度。
> **`**kwargs`:** 有助于为箭头添加属性的可选参数，如为箭头添加颜色，更改箭头宽度。
...
# Adding an arrow to graph starting from the base (2, 4) and with the length of 2 units from both x and y
# And setting the width of arrow for better visualization
...
# changing the edge color to green
plt.arrow(2, 4, 2, 2,
          head_width=0.2,
          width=0.05,
          ec='green')
