# Python 中的 `matplotlib.artist.Artist.get_clip_box()`

> 原文: [https://www.geeksforgeeks.org/matplotlib-artist-artist-get_clip_box-in-python/](https://www.geeksforgeeks.org/matplotlib-artist-artist-get_clip_box-in-python/)
...
## `matplotlib.artist.Artist.get_clip_box()` 方法

`matplotlib` 库的 `artist` 模块中的 `get_clip_box()` 方法用于获取剪贴盒。
...
以下示例说明了 `matplotlib` 中的 `matplotlib.artist.Artist.get_clip_box()` 函数:
