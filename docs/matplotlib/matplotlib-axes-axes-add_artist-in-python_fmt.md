# Python 中的 `matplotlib.axes.axes.add_artist()`

> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-add_artist-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-add_artist-in-python/)
...
## `matplotlib.axes.axes.add_artist()` 函数

`matplotlib` 库的 `Axes` 模块中的 `Axes.add_artist()` 函数用于给轴添加一个 `artist`，并返回 `Artist`。

> **语法:** `Axes.add_artist(self, a)`
>
> **参数:** 该方法接受以下参数。
>
> *   `a`: 该参数为**图例**，由 `legend()` 函数产生。
>
> **返回值:** 此方法返回艺术家。
