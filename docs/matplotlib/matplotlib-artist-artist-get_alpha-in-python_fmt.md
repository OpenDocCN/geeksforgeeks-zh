# Python 中的 `matplotlib.artist.Artist.get_alpha()`

> 原文: [https://www.geeksforgeeks.org/matplotlib-artist-artist-get_alpha-in-python/](https://www.geeksforgeeks.org/matplotlib-artist-artist-get_alpha-in-python/)
...
## `matplotlib.artist.Artist.get_alpha()` 方法

`matplotlib` 库的 `artist` 模块中的 `get_alpha()` 方法用于获取用于混合的 alpha 值。
...
下面的例子说明了 `matplotlib` 中的 `matplotlib.artist.Artist.get_alpha()` 函数:
