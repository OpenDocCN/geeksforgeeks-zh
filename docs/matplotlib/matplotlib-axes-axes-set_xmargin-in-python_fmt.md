## matplotlib.axes.Axes.set_xmargin() in Python

> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-set_xmargin-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-set_xmargin-in-python/)
...
`matplotlib` 库的 `Axes` 模块中的 `Axes.set_xmargin()` 函数用于在自动缩放之前设置 X 数据限制的填充。

**语法:**
`Axes.set_xmargin(self, m)`
...
下面的例子说明了 `matplotlib.axes.Axes.set_xmargin()` 函数在 `matplotlib.axes` 中的作用：
