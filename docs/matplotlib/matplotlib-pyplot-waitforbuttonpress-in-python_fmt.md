# matplotlib.pyplot.waitforbuttonpress() 用法详解

> 原文链接: [https://www.geeksforgeeks.org/matplotlib-pyplot-waitforbuttonpress-in-python/](https://www.geeksforgeeks.org/matplotlib-pyplot-waitforbuttonpress-in-python/)
...
`matplotlib` 库 `pyplot` 模块中的 `waitforbuttonpress()` 方法用于阻止调用直到与图形交互。

> **语法:** `matplotlib.pyplot.waitforbuttonpress(timeout=-1)`
>
> **参数:** 该方法接受以下参数:
>
> *   `timeout`: 该参数为超时值。
...
下面的例子说明了 `matplotlib.pyplot` 中的 `matplotlib.pyplot.waitforbuttonpress()` 函数:
