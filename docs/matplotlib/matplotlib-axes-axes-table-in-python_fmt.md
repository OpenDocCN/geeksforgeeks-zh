...
`matplotlib` 库的 `axes` 模块中的 `Axes.table()` 函数也用于向 `Axes` 添加表格。
...
>
> 参数: 该方法接受以下描述的参数:
> *   `cellText`: 此参数包含要放入表格单元格的文本。
> *   `cellColours`: 该参数是单元格的背景颜色。
...
> *   `colLoc`: 此参数是列标题单元格的文本对齐方式。
> *   `loc`: 该参数是表格相对于 `ax` 的位置。
> *   `bbox`: 这个参数是绘制表格的边界框。
> *   `edges`: 此参数是要用线绘制的单元格边。
>
> 返回: 这将返回以下内容:
> *   `table`: 此方法返回创建的表。
