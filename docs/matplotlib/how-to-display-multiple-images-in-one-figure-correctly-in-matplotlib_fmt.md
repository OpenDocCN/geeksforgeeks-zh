...
在一幅图中显示多幅图像最简单的方法是使用 Matplotlib 的`figure()`、`add_subplot()`和`imshow()`方法。使用的方法是首先通过调用`fig = plt.figure()`启动`fig`对象，然后通过调用`add_subplot()`方法向`fig`中添加一个`axes`对象。然后将使用`imshow()`方法显示图像。
