...
`matplotlib` 库 `pyplot` 模块中的 `triplot()` 函数用于绘制一个非结构化的三角形网格作为线条和/或标记。
...
*   `**kwargs`: 此参数是用于控制标签外观的 `Text` 属性。所有剩余的参数和 `kwargs` 与 `matplotlib.pyplot.plot()` 相同。
