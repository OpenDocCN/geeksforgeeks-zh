...
> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-get_ymajorticklabels-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-get_ymajorticklabels-in-python/)

`Matplotlib` 是 Python 中的一个库，是 `NumPy` 库的数值-数学扩展。`Axes` 类包含了大部分的图形元素：轴、刻度、线、文本、多边形等，并设置坐标系。`Axes` 的实例通过回调属性支持回调。

## matplotlib.axes.Axes.get_ymajorticklabels() 函数

`matplotlib` 库的 `Axes` 模块中的 `get_ymajorticklabels()` 函数用于返回主 y 刻度标签。

> **语法:** `Axes.get_ymajorticklabels(self)`
>
> **参数:** 该方法不接受任何参数。
>
> **返回值:** 该方法返回文本实例列表。

下面的例子说明了 `matplotlib.axes.Axes.get_ymajorticklabels()` 函数在 `matplotlib.axes` 中的作用：
