# matplotlib.axes.Axes.stackplot()在Python中的使用
...
`matplotlib` 库的 `Axes` 模块中的 `axes.stackplot()` 函数用于创建堆叠区域图。
...
> *   `x`: 这个参数是 x 坐标的序列。
> *   `y`: 这个参数是 y 坐标的序列。
> *   `baseline`: 此参数为基线{'zero', 'sym', 'wiggle', 'weighted_wiggle'}。
> *   `colors`: 该参数是颜色的列表或元组。
> *   `labels`: 此参数是分配给每个数据系列的标签。
>
> **返回:** 这将返回以下内容:
>
