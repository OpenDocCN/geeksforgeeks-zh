# matplotlib.axes.Axes.magnitude_spectrum()函数详解

> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-magnitude_spectrum-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-magnitude_spectrum-in-python/)

`matplotlib`是Python中的一个库，是`NumPy`库的数值-数学扩展。`Axes`类包含了大部分的图形元素：轴、刻度、`Line2D`、文本、多边形等，并设置坐标系。`Axes`的实例通过回调属性支持回调。

`matplotlib.axes.Axes.magnitude_spectrum()`函数用于绘制震级谱。

> **语法:** `axes.magnitude_spectrum(self, x, Fs=None, Fc=None, window=None, pad_to=None, sides=None, *, data=None, **kwargs)`
>
> **参数:** 该方法接受以下描述的参数:
> *   `x`: 这个参数是一个数据序列。
> *   `Fs`: 此参数为标量。它的默认值是2。
...
下面的例子说明了`matplotlib.axes.Axes.magnitude_spectrum()`函数在`matplotlib.axes`中的作用:
