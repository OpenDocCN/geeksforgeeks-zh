...
> 原文: [https://www.geeksforgeeks.org/matplotlib-figure-figure-draw_artist-in-python/](https://www.geeksforgeeks.org/matplotlib-figure-figure-draw_artist-in-python/)
...
`matplotlib` 库的 `Figure` 模块的 `draw_artist()` 方法仅用于绘制 `matplotlib.artist.Artist` 实例 `a`。
...
下面的例子说明了 `matplotlib.figure.Figure.draw_artist()` 函数在 `matplotlib.figure` 中的作用:
