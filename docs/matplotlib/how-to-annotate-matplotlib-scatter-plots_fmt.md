...
散点图使用点来表示两个不同数值变量的值。在 Python 中，我们有一个 `matplotlib` 库，其中有一个名为散点图的函数，可以帮助我们创建散点图。在这里，我们将使用 `matplotlib.pyplot.scatter()` 方法来绘制。
...
> *   `marker`: 标记样式，默认: `rcParams["scatter.marker"]` (默认: `'o'`)
> *   `cmap`: `cmapstr` 或 `Colormap`，默认: `rcParams["image.cmap"]` (默认: `'viridis'`)
> *   `linewidth`: 浮点或类似数组，默认值: `rcParams["lines.linewidth"]` (默认值: 1.5)
> *   `alpha`: 浮动，默认: `无`→表示透明度
...
> **参数:**
>
> *   `x`, `y`: 标量—放置文本的位置。默认情况下，这是在数据坐标中。可以使用 `transform` 参数更改坐标系。
> *   `字符串`—文本。
...
如果我们想要注释散点图中的所有点，那么 `matplotlib.pyplot` 有一个内置的函数 `annotate`，它获取点的文本、`x` 和 `y` 坐标。

> **语法：** `matplotlib.pyplot.annotate(text, xy)`
>
> **参数:**
>
> *   `text`: 字符串—注释的文本。`s` 是此参数的弃用同义词。
