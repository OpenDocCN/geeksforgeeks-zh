...
> 原文：[https://www.geeksforgeeks.org/matplotlib-pyplot-suptitle-function-in-python/](https://www.geeksforgeeks.org/matplotlib-pyplot-suptitle-function-in-python/)
...
> *   `水平对齐(ha)`：`{'居中', '左', '右'}`，文本的水平对齐是相对于 `(x, y)` 的。它的默认值是 `'center'`。
> *   `垂直对齐(va)`：`{'顶部', '中心', '底部', '基线'}`，文本的垂直对齐是相对于 `(x, y)` 的。它的默认值是 `'top'`。
> *   `字体大小，大小`：`{大小以磅为单位, 'xx-小', 'x-小', '小', '中', '大', 'x-大', 'xx-大'}`，文字的字体大小。它的默认值是 `'large'`。
> *   `字体粗细，粗细`：`{0-1000 范围内的数值, '超轻', '轻', '正常', '常规', '书本', '中', '罗马', '半粗体', '半粗体', '中', '粗体', '重', '加粗', '黑色'}`，文本的字体粗细。它的默认值是 `'normal'`。
> *   `字体属性`：`无或字典`，字体属性字典。如果给定字体属性，字体大小和粗细的默认值取自字体属性默认值。在这种情况下，`rcParams["figure.title_size"] = 'large'` 和 `rcParams["figure.title_weight"] = 'normal'` 被忽略。
