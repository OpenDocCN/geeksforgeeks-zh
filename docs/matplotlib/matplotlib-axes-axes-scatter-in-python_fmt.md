# Python 中的 Matplotlib.axes.Axes.scatter()

> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-scatter-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-scatter-in-python/)

[Matplotlib](https://www.geeksforgeeks.org/python-introduction-matplotlib/) 是 Python 中的一个库，是 NumPy 库的数值-数学扩展。`Axes` 类包含了大部分的图形元素：轴、刻度、线二维、文本、多边形等，并设置坐标系。`Axes` 的实例通过回调属性支持回调。

## matplotlib.axes.Axes.scatter() 函数

`matplotlib` 库的 `Axes` 模块中的 `Axes.scatter()` 函数用于绘制不同标记大小和/或颜色的 y 对 x 散点图。

> **语法:** `Axes.scatter(self, x, y, s=None, c=None, marker=None, cmap=None, norm=None, vmin=None, vmax=None, alpha=None, linewidth=None, verts=None, edgecolors=None, *, plotnonfinite=False, data=None, **kwargs)`
>
> **参数:** 该方法接受以下描述的参数：
>
...
> *   `c`: 该参数为可选参数，包含颜色的顺序。
> *   `marker`: 该参数也是可选参数。它包含标记样式。
> *   `cmap`: 该参数也是一个可选参数，包含注册的颜色映射名称。其默认值为 `None`。
> *   `norm`: 该参数也是可选参数。它用于将亮度数据缩放到 0，1。其默认值为 `None`。
> *   `vmin`, `vmax`: 这些参数与 `norm` 一起使用，以默认值 `None` 归一化亮度数据。
> *   `alpha`: 该参数也是可选参数。它们在 0（透明）和 1（不透明）之间混合值。
> *   `linewidth`: 该参数也是可选参数。这是标记边缘的线宽。其默认值为 `None`。
> *   `edgecolors`: 该参数也是可选参数。它是颜色或 `{‘face’, ‘none’, None}` 的顺序。
> *   `plotnonfinite`: 此参数也是可选参数。这是标记边缘的线宽。其默认值为 `None`。
>
> **返回:** 返回容器，包括以下内容：
>
> *   `PathCollection`: 这将返回 x，y 绘图标记和/或线的 `Line2D` 实例。
> *   `caplines`: 这将返回误差线 caps 的 `Line2D` 实例的元组。
> *   `barlinecols`: 这将返回 `LineCollection` 的元组，包含水平和垂直误差范围。

下面的例子说明了 `matplotlib.axes.Axes.scatter()` 函数在 `matplotlib.axes` 中的作用：
