...
> 原文: [https://www.geeksforgeeks.org/introduction-to-3d-plotting-with-matplotlib/](https://www.geeksforgeeks.org/introduction-to-3d-plotting-with-matplotlib/)
...
*   在上面的例子中，首先，我们从 python 库中导入包，以便在我们的空画布中有一个 3D 绘图。为此，我们正在导入 `numpy`、`matplotlib.pyplot` 和 `mpl_toolkits`。
...
*   使用 `meshgrid`，我们从坐标向量返回坐标矩阵。`meshgrid` 制作二维坐标阵列，用于对二维网格上的三维标量场/矢量场进行矢量化评估，其中有一个一维阵列。现在，我们分别在 z 和 z1 的帮助下创建正弦和余弦曲线。
*   之后，我们将创建一个空的图形，在那里我们将绘制出三维图。使用 `fig.gca`，我们正在定义我们将要制作的图将是使用 `projection='3d'` 的 3D 投影。使用 `plot_wireframe` 我们正在用我们创建的上述点创建一个三维正弦和余弦曲线（不同颜色）。最后，我们展示上面的情节。
...
*   在上面的例子中，我们将使用 2 个三维图形。为此，我们将在一个画布或图形中创建两个子图。在代码中，我们像往常一样初始化了 python 中的包 `matplotlib.pyplot` 和 `NumPy`。
*   然后，我们将创建一个空图形，其中将绘制两个三维图。现在我们正在创建第一个子图。
*   之后，我们取包含一系列点的 X，Y，用它们创建一个 `meshgrid`，这个网格由两个 1D 数组组成，用于矩阵索引，创建一个 nd 数组。
*   然后，使用 X 和 Y 的值，我们通过形成一个表达式来创建 Z。使用 `plot_wireframe` 我们在三维轴上绘制点。
...
这里我们将看看如何创建一个表面图和一个三表面图。在这里，我们将这两个情节整合成一个单独的图形，这样我们就可以理解子图的概念以及要绘制的大量点的情节的概念。因此，让我们开始实施这一概念。与线框图不同，这些图的表面将填充颜色。
...
*   然后，使用网格，我们为`x1`和`y1`创建一个 nD 数组。
...
*   第一步是导入绘制上述图所需的所有包。除了`matplotlib.pyplot`和`NumPy`之外，我们正在导入另一个包，即`mpl_toolkits.mplot3d`。
*   之后，我们将创建一个空的图形，在那里我们将有我们的 2 个 3D 支线剧情。
*   之后，我们添加大小为 1×2 的第一个子图，并定义第一个子图。
*   之后，我们使用 3 个变量，在`get_test_data`的帮助下，我们正在创建一个自动化的网格，它将帮助我们绘制出图中的点。使用`ax.contour(X, Y, Z)`我们正在创建具有以下值范围的等高线图。
*   之后，我们定义相同大小的第二个图。填充等高线图不需要网格。它可以用`X1`、`Y1`、`Z1`中的简单值范围来创建。`Z1`借助于使用`X1`和`Y1`创建的表达式形成。使用`ax.contourf(X1, Y1, Z1)`我们正在绘制填充轮廓图。
*   最后，我们用 2 个支线剧情展示了上图。
...
*   之后，我们将创建一个空的图形，我们将绘制上面的图形。使用`np.arange()`和一系列任意值，我们在`x`和`z`中创建了一个值列表。现在我们是循环到`zs`，其中对于`zs`的每个值，我们得到了`y`的随机数字列表，其长度与`xs`的长度相同。
*   现在，对于每个`z`，我们沿着直线绘制`x`对`y`。使用`PolyCollection`，我们将所有的图用每个图中相同的颜色放在一起。在`add_collection3d`的帮助下，我们确定该图将在 x-z 轴上绘制，并定义`zs=zs`。
*   最后，我们展示上面的情节。
...
*   在上面的代码中，我们正在顶部导入所有必要的包(`matplotlib.pyplot`，`numpy`，`mpl_toolkits.mplot3d`)。
*   然后我们借助`plt.figure()`创建一个空图。借助`fig.gca(projection='3d')`，我们将投影指定为 3d，即 3D 绘图。
*   使用`np.meshgrid()`我们正在创建一个具有`X`，`Y`和`Z`的 N-D 阵列。在颤图中，绘制点不是唯一的任务。我们需要指定向量方向的分量。为此，我们取`u`，`v`，`w`，它们是通过使用`X`，`Y`和`Z`创建的不同表达式形成的。
*   使用`ax.quiver()`我们绘制箭袋图，其中我们将每个箭袋的长度指定为 0.2，并且为了使箭头具有相同的长度，我们将`normalize`定义为`True`。
...
*   在这个例子中，我们在 Matplotlib 的三维图上绘制 2D 数据。为此，我们需要 `NumPy` 和 `matplotlib.pyplot` 来创建值集并在 3D 投影中绘制它们。导入所有必要的包后，我们将在 `X`、`Y` 和 `Z` 中创建一系列值。
