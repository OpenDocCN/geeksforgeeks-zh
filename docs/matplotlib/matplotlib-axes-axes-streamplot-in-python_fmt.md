# matplotlib.axes.streamplot() 用法

> 原文：[https://www.geeksforgeeks.org/matplotlib-axes-streamplot-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-streamplot-in-python/)
...
*   `color`：该参数为流线颜色。
*   `cmap`：此参数用于绘制流线和箭头。
*   `norm`：此参数用于将用于缩放亮度数据的对象归一化为 0，1。
*   `arrowsize`：该参数是箭头尺寸的比例因子。
...
下面的例子说明了 `matplotlib.axes.Axes.streamplot()` 函数在 matplotlib.axes 中的作用：
