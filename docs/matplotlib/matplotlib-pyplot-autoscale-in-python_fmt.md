# Matplotlib.pyplot.autoscale() 在Python中的使用
...
>
> **参数:**
> `enable` 是一个布尔值参数，如果设置为真，则自动缩放打开，否则自动缩放关闭。这是一个可选参数。未指定时，它将默认值设为“真”。
> `axis` 是另一个可选参数，表示要操作的轴。通常可以是 'both'，'x' 或 'y'。未指定时，它将默认值作为“两者”。
> `tight` 取布尔参数，如果为真，首先将边距设置为零。然后，该参数被转发到 `autoscale_view` (无论其值如何)。这也是 `autoscale` 方法的可选参数。
