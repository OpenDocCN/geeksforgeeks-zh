...
> 原文：[https://www.geeksforgeeks.org/overlapping-histograms-with-matplotlib-in-python/](https://www.geeksforgeeks.org/overlapping-histograms-with-matplotlib-in-python/)
...
### 步骤 3：绘制 `sepal_length` 和 `petal_length` 的直方图
...
在这里，我们可以看到 `petal_length` 直方图的某些部分已经隐藏在 `sepal_length` 直方图的后面。为了正确地可视化两个直方图，我们需要将透明度参数 `alpha` 设置为合适的值。因此，让我们检查 `alpha` 的各种值，找出合适的值。

### 步骤 4：为 `sepal_length` 和 `petal_length` 设置 `alpha=0.5`
...
### 步骤 5：为 `sepal_length` 设置 `alpha=0.1`，为 `petal_length` 设置 `alpha=0.9`
...
通过以上两个步骤，我们可以推断，为了更好地可视化两个直方图，`alpha=0.5` 将是透明度参数的最合适选项。

现在，要在需要自定义颜色的地方绘制两个以上的重叠直方图，让我们遵循步骤 6。

### 步骤 6：用定制的颜色创建 2 个以上的重叠直方图
