# Python中的matplotlib.patches矩形

> 原文:[https://www.geeksforgeeks.org/matplotlib-patches-rectangle-in-python/](https://www.geeksforgeeks.org/matplotlib-patches-rectangle-in-python/)
...
`matplotlib.patches.Rectangle`类用于以指定的宽度、高度和旋转角度在xy = (x，y)处对左下方的地块进行矩形补片。

> `class matplotlib.patches.rectangle(xy, 宽度, 高度, 角度=0.0, **kwargs)`
>
> **参数:**
>
> *   `xy`: 左下角开始矩形绘图
...
| 财产 | 描述 |
| --- | --- |
| `agg_筛选器` | 接受(m，n，3)浮点数组和返回(m，n，3)数组的dpi值的筛选函数 |
| `希腊字母的第一个字母` | 浮动或无 |
| `愉快的` | 弯曲件 |
| `抗锯齿或aa` | 未知的 |
| `帽式` | { '对接'，'圆形'，'突出' } |
...
| `颜色` | rgba元组的颜色或序列 |
| `包含` | 请求即付的 |
| `edgecolor或ec或edgecolors` | 颜色或无或“自动” |
