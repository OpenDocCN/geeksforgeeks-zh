...
> 原文: [https://www.geeksforgeeks.org/matplotlib-ticker-loglocator-class-in-python/](https://www.geeksforgeeks.org/matplotlib-ticker-loglocator-class-in-python/)
...
> 参数:
>
> *   `subs`: 它是一个可选参数，可以是 `None`，也可以是字符串或一系列浮点数。默认为 `(1.0,)`。它提供用于放置刻度的基数的整数次幂。默认情况下，只有在基数的整数次幂时才会放置刻度。`'auto'` 和 `'all'` 是这里唯一接受的字符串值。刻度正好位于整数次幂之间，带有 `'auto'`，而带有 `'all'` 的整数次幂被接受。这里 `None` 值相当于 `'auto'`。

### 该类方法

*   `base(self, base)`: 此方法用于设置对数刻度的基数。
*   `nonsingular(self, vmin, vmax)`: 用于根据需要扩大范围，避免奇异。
*   `set_params(self, base=None, subs=None, numdecs=None, numticks=None)`: 用于设置刻度内的参数。
*   `tick_values(self, vmin, vmax)`: 此方法返回位于 `vmin` 和 `vmax` 范围之间的刻度值。
*   `subs(self, subs)`: 它用于设置每个 `base**i*subs[j]` 的日志缩放的次要刻度。
*   `view_limits(self, vmin, vmax)`: 这些方法在智能选择视图限制时非常有用。
