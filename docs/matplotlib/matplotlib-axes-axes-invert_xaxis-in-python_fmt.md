...
> 哎哎哎:# t0]https://www.geeksforgeeks.org/matplotlib-axes-reverse_xaxis-in-python/

## matplotlib.axes.Axes.invert_xaxis()函数

`matplotlib` 库的 `Axes` 模块中的 `Axes.invert_xaxis()` 函数用于 x 轴的反求。
...
下面的例子说明了 `matplotlib.axes.Axes.invert_xaxis()` 函数在 `matplotlib.axes` 中的作用：
