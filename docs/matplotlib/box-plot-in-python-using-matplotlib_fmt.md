...
> 原文：[https://www.geeksforgeeks.org/box-plot-in-python-using-matplotlib/](https://www.geeksforgeeks.org/box-plot-in-python-using-matplotlib/)
...
| `data` | 要绘制的数组或数组序列 |
| `notch` | 可选参数接受布尔值 |
| `vert` | 对于水平和垂直绘图，可选参数分别接受布尔值 `False` 和 `True` |
| `bootstrap` | 可选参数接受 `int` 指定凹口方框图周围的间隔 |
| `usermedians` | 可选参数接受与数据兼容的数组或数组维数序列 |
| `positions` | 可选参数接受数组并设置框的位置 |
| `widths` | 可选参数接受数组并设置框的宽度 |
| `patch_artist` | 具有布尔值的可选参数 |
| `labels` | 字符串序列为每个数据集设置标签 |
| `meanline` | 可选具有布尔值尝试将均值线呈现为框的全宽 |
| `order` | 可选参数设置箱线图的顺序 |
