...
> 原文: [https://www.geeksforgeeks.org/calculate-the-area-of-an-image-using-matplotlib/](https://www.geeksforgeeks.org/calculate-the-area-of-an-image-using-matplotlib/) 计算图像面积
...
2.  使用 `imread()` 方法导入图像。
3.  使用图像的 `shape` 属性获取图像的高度和宽度。它获取图像中的通道数。
4.  面积计算为，面积 = 高度 * 宽度。
5.  显示区域。
