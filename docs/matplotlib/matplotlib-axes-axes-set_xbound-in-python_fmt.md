...
> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-set_xbound-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-set_xbound-in-python/)
...
## matplotlib.axes.Axes.set_xbound() 函数

`matplotlib`库的`Axes`模块中的`Axes.set_xbound()`函数用于设置 x 轴的数值上下限。

> **语法:** `Axes.set_xbound(lower=None, upper=None)`
>
> **参数:** 该方法接受以下参数。
>
> *   `lower`, `upper`: 这些参数是下限和上限。如果为`None`，则不修改相应的轴边界。
>
> **返回:** 该方法返回以下内容
>
> *   `lower`, `upper`: 这将返回新的 x 轴下限和上限。

**注意:** 该功能可以在各种条件下代替`set_xlim`使用。

下面的例子说明了`matplotlib.axes.Axes.set_xbound()`函数在`matplotlib.axes`中的作用:
