# Python 中的 `matplotlib.artist.Artist.set_clip_on()`

> 原文: [https://www.geeksforgeeks.org/matplotlib-artist-artist-set_clip_on-in-python/](https://www.geeksforgeeks.org/matplotlib-artist-artist-set_clip_on-in-python/)
...
## `matplotlib.artist.Artist.set_clip_on()` 方法

`matplotlib` 库的 `artist` 模块中的 `set_clip_on()` 方法用于设置 artist 是否使用裁剪。
...
以下示例说明了 `matplotlib` 中的 `matplotlib.artist.Artist.set_clip_on()` 函数:
