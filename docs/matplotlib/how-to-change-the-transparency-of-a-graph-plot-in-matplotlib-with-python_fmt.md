...
[`Matplotlib`](https://www.geeksforgeeks.org/python-introduction-matplotlib/) 是 Python 中的一个库，它是 `NumPy` 库的数值-数学扩展。 [`Pyplot`](https://www.geeksforgeeks.org/pyplot-in-matplotlib/) 是一个基于状态的 `matplotlib` 模块的接口，提供了一个类似 MATLAB 的接口。 `Pyplot` 中可以使用的各种图有线图、等高线图、直方图、散点图、三维图等。

为了在 `matplotlib` 中改变图形图的透明度，我们将使用 `matplotlib.pyplot.plot()` 函数。 `matplotlib` 库中`plot()` 功能用于制作 2D 插图。
...
> *   `fmt`: 该参数为可选参数，包含字符串值。
> *   `data`: 该参数为可选参数，是一个带有标签数据的对象。
> 
> **返回:**这将返回以下内容:
> 
> *   `Line2D`: 这将返回表示打印数据的线 2D 对象列表。
...
在上面的程序中，线性图是用透明度描述的，即`alpha= 0.25`。
