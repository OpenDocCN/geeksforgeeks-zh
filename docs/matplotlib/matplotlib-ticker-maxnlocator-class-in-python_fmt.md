...
> 原文: [https://www.geeksforgeeks.org/matplotlib-ticker-maxnlocator-class-in-python/](https://www.geeksforgeeks.org/matplotlib-ticker-maxnlocator-class-in-python/)
...
> *   `steps`: 这是一个可选参数，表示从 1 开始到 10 结束的一个很好的数字序列。
> *   `integer`: 可选布尔值。如果设置为真，刻度仅接受整数值，前提是至少 `min_n_ticks` 整数在视图限制内。
> *   `symmetric`: 为可选值。如果设置为真，自动缩放将产生一个关于零对称的范围。
> *   `trim`: 它是一个可选参数，接受以下四个值中的任何一个: `{ 'lower', 'upper', 'both', None }`。默认情况下，它是 `'None'`。
