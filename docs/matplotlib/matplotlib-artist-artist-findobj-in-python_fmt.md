## matplotlib.artist.Artist.findobj() 方法详解

> 原文: [https://www.geeksforgeeks.org/matplotlib-artist-artist-findobj-in-python/](https://www.geeksforgeeks.org/matplotlib-artist-artist-findobj-in-python/)
...
`matplotlib`库的`artist`模块中的`findobj()`方法用于返回找到的艺术家对象。

> **语法:** `Artist.findobj(self, match=None, include_self=True)`
>
> **参数:** 该方法接受以下参数。
> - `match`: 该参数是匹配的过滤标准。默认值为`None`。
> - `include_self`: 该参数在待查列表中包含`self`进行匹配。
>
> **返回:** 此方法返回`Artist`列表。

以下示例说明了`matplotlib.artist.Artist.findobj()`在`matplotlib`中的功能:
