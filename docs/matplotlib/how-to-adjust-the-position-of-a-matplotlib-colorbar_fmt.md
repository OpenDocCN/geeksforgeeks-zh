...
> 原文: [https://www.geeksforgeeks.org/how-to-adjust-the-position-of-a-matplotlib-colorbar/](https://www.geeksforgeeks.org/how-to-adjust-the-position-of-a-matplotlib-colorbar/)
...
| `shrink` | 使用`set_label`将标签设置到颜色条。 |
| `pad` | 彩色条和新图像轴之间原始轴的相对子图间隙或分数。 |
| `norm` | 将数字参数或颜色转换为`RGBA`。 |
| `figsize` | 二元浮点数组。图尺寸(宽度、高度)单位为英寸。 |
| `add_subplot` | 将轴添加到图形中，作为子图排列的一部分。 |
| `add_axes` | 呈现在`matplotlib`库的图形模块中，用于向图形添加轴。 |
| `origin` | 图像处理中使用的惯例:原点在左上角。 |
...
![Matplotlib Colorbar 安装](img/b0faa274d489cc71abfdaef96766cbf9.png)
...
![输出](img/9cf4fc8bb73d10311389d7bbbb143b61.png)
...
![输出](img/ab99106e01ee33ffe3939484a68b94f7.png)

### 示例 2: 左侧 Matplotlib 颜色条的位置

生成`Matplotlib`图表，其中颜色条位于图表的左侧。这里，`ax`位置是手动设置的，颜色条使用关键字`location`链接到现有的绘图轴。`ax`参数用于颜色条引用列表中的多个轴，如果您将一个轴放在列表中，则此处可以使用该参数。
...
![输出](img/ea3ce5dd8187df8f336f98d80890996f.png)

这是一种生成彩条并确保它位于自己的轴上的简单方法。然后使用`cax`参数指定彩条的位置，其中为要绘制的彩条给出了轴。
...
要定位`Matplotlib`颜色条在图表下方，可以执行以下命令:
...
![输出](img/fb8b2deeef8c3edf7e37e2f41862fa10.png)

`pad`参数在图表的`x`轴和颜色条之间创建填充。`pad`值越高，颜色条远离`x`轴。要相对于子图移动颜色条，使用`pad`参数。
...
![输出](img/c0e05d373b46cbc3aedd24bd86dc3255.png)

使用`make_axes_locatable`实例来划分轴并创建与图像图对齐的新轴。`pad`参数将允许在两个轴之间设置空间:
...
![输出](img/19b4ba26390c94147516d01ca7bebd99.png)

### 示例 4: 图表上方颜色条的位置

要定位`Matplotlib`颜色条在图表上方，可以执行以下命令:
...
![输出](img/ff6a4aae553d8cbcc5b424e515e2f649.png)