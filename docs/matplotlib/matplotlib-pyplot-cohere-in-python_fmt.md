...
`matplotlib` 库 `pyplot` 模块中的 `cohere()` 函数用于绘制 `x` 和 `y` 之间的相干性，相干性是归一化的交叉谱密度。
...
> *   `sides`: 此参数指定要返回光谱的哪些边。这可以有以下值: `'default'`, `'onesided'`, `'twosided'`。
...
以下示例说明 `matplotlib.pyplot.cohere()` 函数在 `matplotlib.axis` 中的使用。
