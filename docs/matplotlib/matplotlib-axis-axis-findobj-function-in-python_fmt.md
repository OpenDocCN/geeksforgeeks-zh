...
> 原文: [https://www.geeksforgeeks.org/matplotlib-axis-axis-findobj-function-in-python/](https://www.geeksforgeeks.org/matplotlib-axis-axis-findobj-function-in-python/)

`Matplotlib` 是 Python 中的一个库，是 NumPy 库的数值-数学扩展。这是一个神奇的 Python 可视化库，用于 2D 数组图，并用于处理更广泛的 SciPy 堆栈。

## Matplotlib.axis.Axis.findobj() 函数

`matplotlib` 库的 `Axis` 模块中的 `Axis.findobj()` 函数用于查找艺术家对象。
