# Matplotlib.pyplot.contourf() 用法详解
...
`matplotlib` 库 `pyplot` 模块中的 `contourf()` 函数用于绘制等高线。但是 `contourf` 绘制填充轮廓，而 `contour` 绘制轮廓线。

> **语法:** `matplotlib.pyplot.contourf(*args, data=None, **kwargs)`
>
> **参数:** 该方法接受以下描述的参数:
>
> *   `X, Y`: 这些参数是 `Z` 中数值的坐标。
