# matplotlib.dates.drange() 用法详解

> 原文链接：[https://www.geeksforgeeks.org/matplotlib-dates-in-python/](https://www.geeksforgeeks.org/matplotlib-dates-in-python/)
...
1.  `dstart`: 日期范围的起点，是 Python 的 `datetime`。
2.  `dend`: 日期范围的结束点，是 Python 的 `datetime`。
3.  `delta`: 表示每个日期之间的间距，属于 Python 的 `datetime.timedelta`。

**返回:**
它返回一个 `numpy` 数组，这是一个表示 Matplotlib 日期的浮点数列表。
