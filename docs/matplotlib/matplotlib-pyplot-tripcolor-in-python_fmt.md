# Matplotlib.pyplot.tripcolor() 用法详解

> 原文链接: https://www.geeksforgeeks.org/matplotlib-pyplot-tripcolor-in-python/
...
`matplotlib` 库中 `pyplot` 模块中的 `tripcolor()` 函数用于创建非结构化三角形网格的伪彩色图。
...
所有剩余的 `args` 和 `kwargs` 与 `triplot()` 相同。
