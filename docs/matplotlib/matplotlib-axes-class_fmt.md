...
`axes()` 函数使用参数创建 `axes` 对象，其中参数是 4 个元素的列表 `[左, 底, 宽, 高]`。现在让我们简单了解一下 `axes()` 的功能。
...
其中 `labels` 指的是一系列字符串，`handles`，一系列 `Line2D` 或 `Patch` 实例，`loc` 可以是指定图例位置的字符串或整数。
...
`axes` 类的 `plot()` 函数将一个数组的值与另一个数组的值绘制成线或标记。
