# Python 中的 `matplotlib.axes.Axes.margins()`
...
`matplotlib` 库的 `Axes` 模块中的 `Axes.margins()` 函数用于设置或检索自动缩放边距。

> **语法:** `axes.margins(self, *margins, x=None, y=None, tight=True)`
> **参数:** 此方法接受以下参数。
> *   `*margins`: 此参数用于指定 x 轴和 y 轴限制的边距。
> *   `x`, `y`: 这些参数分别用于 x 轴和 y 轴的特定边距值。
> *   `tight`: 此参数传递给 `autoscale_view()`，该参数在边距更改后执行。
>
> **返回值:** 此方法返回以下值。
> *   边缘
> *   边缘

以下示例说明 `matplotlib.axes.Axes.margins()` 函数在 `matplotlib.axes` 中的使用：
