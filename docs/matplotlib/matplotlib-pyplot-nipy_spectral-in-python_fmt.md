# matplotlib.pyplot.nipy_spectral() 用法详解

## Matplotlib 与 Pyplot 简介

**Matplotlib** 是 Python 中的一个库，是 NumPy 库的数值-数学扩展。**Pyplot** 是一个基于状态的接口到 **Matplotlib** 模块，它提供了一个类似于 MATLAB 的接口。

## `matplotlib.pyplot.nipy_spectral()` 函数

`matplotlib` 库 `pyplot` 模块中的 `nipy_spectral()` 函数用于将 colormap 设置为 “nipy_spectral”。
...
下面的例子说明了 `matplotlib.pyplot.nipy_spectral()` 函数在 `matplotlib.pyplot` 中的作用:
