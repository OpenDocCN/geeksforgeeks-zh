# Python中的matplotlib.axes.Axes.get_adjustable()

> 原文：[https://www.geeksforgeeks.org/matplotlib-axes-axes-get_adjustable-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-get_adjustable-in-python/)
...
## matplotlib.axes.Axes.get_adjustable()函数

matplotlib库的axes模块中的`Axes.get_adjustable()`函数用于获取`Axes`给定的哪个参数来实现给定的方面。

> **语法：**`axes.get_adjustable(self)`
>
> **参数：**该方法不接受任何参数。
>
> **返回值：**此方法返回`adjustable`值。

以下示例说明了`matplotlib.axes.Axes.get_adjustable()`函数在`matplotlib.axes`中的作用：
