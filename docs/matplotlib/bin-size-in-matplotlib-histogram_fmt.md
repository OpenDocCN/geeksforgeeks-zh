...
**先决条件:** [Matplotlib](https://www.geeksforgeeks.org/python-matplotlib/)
...
这里，`bins= 5`，即要创建的箱的数量是 5。将面元设置为整数会创建大小或宽度相等的面元。随着料箱尺寸的改变，料箱宽度也会相应地改变，如下所示:
...
我们还可以在参数 `bins` 中传递一系列 `int` 或 `float`。其中序列的元素是仓的边/边界。在这种方法中，每个料箱的料箱宽度可能不同。

假设一个序列`[1，2，3，4，5]`被分配给 `bins`，那么所产生的仓的数量将是 4，即第一个仓将是`[1，2]`(包括 1，但不包括 2)第二个仓将是`[2，3]`(包括 2，但不包括 3)第三个仓将是`[3，4]`(包括 3，但不包括 4)。然而，在最后一个箱`[4，5]`中，包括 4 和 5。

因此，所有的箱子都是半开的`[a，b]`，但是最后一个箱子是关闭的`[a，b]`。在这种情况下，每个箱子的宽度是相等的。

如果分配给 `bins` 的序列的每个元素之间的差异不相等，那么每个仓的宽度是不同的，因此仓宽度取决于序列。
...
为了在面元参数中传递一个序列，我们还可以对均匀分布的面元使用 `range()` 函数。在 `range()` 内，起点是数据的最小值，终点是数据的最大值+前面提到的面元宽度，如在 `range()` 内，不包括终点，步长是面元宽度。

由于步长在 `range()` 内是固定的，因此我们在直方图中得到大小相等的面元。
