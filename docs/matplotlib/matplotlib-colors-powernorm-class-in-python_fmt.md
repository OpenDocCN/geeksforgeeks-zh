...
> 原文: [https://www.geeksforgeeks.org/matplotlib-colors-powernorm-class-in-python/](https://www.geeksforgeeks.org/matplotlib-colors-powernorm-class-in-python/)
...
`matplotlib.colors.PowerNorm` 类属于 `matplotlib.colors` 模块。`matplotlib.colors` 模块用于将颜色或数字参数转换为 RGBA 或 RGB。该模块用于将数字映射到颜色，或者在一维颜色数组(也称为颜色映射)中进行颜色规格转换。

`matplotlib.colors.PowerNorm` 类用于将值线性映射到 [0, 1] 的范围，然后在该范围内应用幂律归一化。它的基类是 `matplotlib.colors.Normalize`。

**类的方法:**

*   `inverse(self, values)`: 此方法返回色彩映射的反转值。
