...
*   导入模块
*   使用 `pydicom.data.data_manager.get_files()` 方法读取 DICOM 文件
...
> *   `pattern`: 默认为“*”。这是一个字符串模式，用于过滤文件。

*   提供两个参数: `base` 和 `pattern`
*   将数据显示为图像，即在 2D 常规栅格上。
*   显示图像

**注意:** 在变量名 `base` 中输入除文件名以外的 dcm 文件的位置，在 `pass_dicom` 变量中输入文件名。在这种情况下，文件存储在一个名为 `dicom_image` 的目录中，如图所示:
