...
> 原文：[https://www.geeksforgeeks.org/rotation-of-colorbar-tick-labels-in-matplotlib/](https://www.geeksforgeeks.org/rotation-of-colorbar-tick-labels-in-matplotlib/)

**颜色条**是一个轴，表示数据值到绘图中使用的颜色的映射。`matplotlib`的`pyplot`模块中的`colorbar()`函数将一个颜色条添加到绘图中，以指示色标。

![](img/7b058a840385d151644ec3c5df7c7a01.png)

典型颜色条

有时，为了更好的可视化和理解，需要旋转标签。要更改颜色条刻度标签的旋转，所需的旋转角度在以下方法中提供：
*   `cbar.ax.set_xticklabels`，如果颜色条方向为水平
*   `cbar.ax.set_yticklabels`，如果颜色条方向垂直
