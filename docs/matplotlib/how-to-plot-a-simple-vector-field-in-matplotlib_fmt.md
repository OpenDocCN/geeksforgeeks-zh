...
> 原文: [https://www.geeksforgeeks.org/how-to-plot-a-simple-vector-field-in-matplotlib/](https://www.geeksforgeeks.org/how-to-plot-a-simple-vector-field-in-matplotlib/)
...
在本文中，我们将讨论如何在 python 中绘制矢量场。为了执行此任务，我们将使用 `matplotlib` 模块中的 `quiver()` 方法和 `streamplot()` 方法。
...
其中 `X`，`Y` 是均匀间隔的网格[1D 阵列]，`U` 和 `V` 代表网格上每个点的流速。`density` 是地块每一面积的矢量数。`linewidth` 代表流线的厚度。
...
使用 `matplotlib` 模块中的 `quiver()` 方法绘制单个矢量。
...
使用 `matplotlib` 模块中的 `streamplot()` 方法绘制多个矢量。
