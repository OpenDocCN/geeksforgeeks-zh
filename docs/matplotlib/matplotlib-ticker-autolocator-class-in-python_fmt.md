...
> 原文: [https://www.geeksforgeeks.org/matplotlib-ticker-autolocator-class-in-python/](https://www.geeksforgeeks.org/matplotlib-ticker-autolocator-class-in-python/)
...
> **语法:** `class matplotlib.ticker.AutoLocator`
> **参数:**
> - `nbins`: 不是整数就是 `'auto'`，其中整数值代表最大间隔数；比最大刻度数少一个。根据轴的长度自动确定箱的数量。它是一个可选参数，默认值为 10。
> - `steps`: 这是一个可选参数，代表一个从 1 开始到 10 结束的漂亮的数字序列。
> - `integer`: 可选布尔值。如果设置为真，刻度仅接受整数值，前提是至少 `min_n_ticks` 整数在视图限制内。
> - `symmetric`: 是可选值。如果设置为真，自动缩放将产生一个关于零对称的范围。
> - `trim`: 这是一个可选参数，接受以下四个值中的任意一个: `{ 'lower', 'upper', 'both', None }`。默认情况下，它是 `None`。
