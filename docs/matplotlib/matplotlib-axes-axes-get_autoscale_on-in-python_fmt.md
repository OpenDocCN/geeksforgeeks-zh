# Python中的matplotlib.axes.Axes.get_autoscale_on()

> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-get_autoscale_on-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-get_autoscale_on-in-python/)
...
## matplotlib.axes.Axes.get_autoscale_on()函数

`matplotlib`库的`Axes`模块中的`Axes.get_autoscale_on()`函数用于获取绘图命令中是否对两个轴都应用了自动缩放。
...
下面的例子说明了`matplotlib.axes.Axes.get_autoscale_on()`函数在`matplotlib.axes`中的作用：
