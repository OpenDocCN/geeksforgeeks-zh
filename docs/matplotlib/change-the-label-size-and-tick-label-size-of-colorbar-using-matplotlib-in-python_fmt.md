...
> 原文：[https://www.geeksforgeeks.org/change-the-label-size-and-tick-label-size-of-colorbar-using-matplotlib-in-python/](https://www.geeksforgeeks.org/change-the-label-size-and-tick-label-size-of-colorbar-using-matplotlib-in-python/)
...
`im.figure.axes[1].tick_params(axis="x", labelsize=21)`
...
在本例中，我们借助方法 `tick_params(axis="both", labelsize=21)` 更改标签大小，通过传递参数 `axis` 值作为 `"both"`，标签大小作为 `21`。
...
在本例中，我们借助方法 `tick_params(axis="x", labelsize=18)` 更改颜色条的刻度标签大小，通过传递参数 `axis` 值为 `"x"`，标签大小为 `18`。
...
在本例中，我们借助方法 `tick_params(axis="y", labelsize=21)` 更改标签大小和颜色条的刻度标签大小，通过传递参数 `axis` 值为 `"y"`，标签尺寸为 `21`。
