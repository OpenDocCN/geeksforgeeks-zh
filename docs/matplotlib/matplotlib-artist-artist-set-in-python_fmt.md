...
> 原文: [https://www.geeksforgeeks.org/matplotlib-artist-artist-set-in-python/](https://www.geeksforgeeks.org/matplotlib-artist-artist-set-in-python/)
...
`matplotlib`库的`artist`模块中的`set()`方法是一个属性批量设置器。传递`kwargs`以设置属性。

### 语法
`Artist.set(self, **kwargs)`

### 参数
此方法不接受除`**kwargs`以外的任何参数。
