...
> 原文：[https://www.geeksforgeeks.org/how-to-rotate-x-axis-tick-label-text-in-matplotlib/](https://www.geeksforgeeks.org/how-to-rotate-x-axis-tick-label-text-in-matplotlib/)
...
在本例中，我们将使用 `plt.xticks()` 在图形级别上旋转 X 轴标签。

> **语法：** `matplotlib.pyplot.xticks(ticks=None, labels=None, **kwargs)`
>
> **参数：** 该方法接受以下描述的参数：
>
> *   `ticks`：此参数是 xtick 位置的列表。和可选参数。如果一个空列表作为参数被传递，那么它将移除所有的文本信号
...
在本例中，我们将使用 `tick.set_rotation()` 在轴级别上旋转 X 轴标签。

> **语法：** `axes.get_xticks(self, minor=False)`
>
> **参数：** 该方法接受以下参数。
>
> *   `minor`：该参数用于设置主要刻度还是设置次要刻度
>
> **返回值：** 这个方法返回一个文本值列表。
...
在本例中，我们将使用内置的 `ax.set_xticklabels()` 功能来单独旋转 X 轴标签。
...
在本例中，我们将使用内置的 `ax.tick_params()` 功能来单独旋转 X 轴标签。

> **语法：** `matplotlib.pyplot.tick_params(axis='both', **kwargs)`
...
# Setting title
plt.title('Rotating using ax.tick_params()')
