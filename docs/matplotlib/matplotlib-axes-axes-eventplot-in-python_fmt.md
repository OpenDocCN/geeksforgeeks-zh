# Matplotlib Axes Eventplot() 中的 Python
...
`matplotlib` 库的 `Axes` 模块中的 `Axes.eventplot()` 函数用于在给定位置绘制相同的平行线。

> **语法:** `Axes.eventplot(self, positions, orientation='horizontal', lineoffsets=1, linelengths=1, linewidths=None, colors=None, linestyles='solid', *, data=None, **kwargs)`
>
> **参数:** 该方法接受以下描述的参数:
>
> *   `positions`: 这个参数是对象的序列，每个值都是一个事件。
> *   `orientation`: 此参数用于控制事件集合的方向{ `'horizontal'`, `'vertical'` }。
> *   `lineoffsets`: 此参数是线中心在与方向正交的方向上从原点的偏移。
> *   `linelengths`: 该参数是线路的总高度。
> *   `linewidths`: 该参数是事件线的线宽，以磅为单位。
>
> **返回:** 这将返回以下内容:
>
> *   `list`: 返回 `EventCollection` 对象列表。
