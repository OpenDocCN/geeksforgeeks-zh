...
> 哎哎哎: [https://www.geeksforgeeks.org/matplotlib-lasso-selector-widget/](https://www.geeksforgeeks.org/matplotlib-lasso-selector-widget/)
...
在上面的代码中，我们从 `matplotlib.widgets` 模块中导入 `matplotlib` 包和 `LassoSelector` 工具到我们的 `python` 项目中。导入包后，我们创建一个图形（即一个空画布）并手动添加轴。然后，我们定义了一个函数 `onSelect()`，只要在图中按下鼠标就会被触发。然后，我们正在创建定义线条属性的 `line`，然后出现 `LassoSelector`，帮助我们在地块内部进行绘制。现在在 `LassoSelector` 里面有四个参数，第一个定义了我们已经创建的轴，第二个定义了 `onSelect()` 函数，第三个参数定义了线（`line`）的属性，最后一个参数定义了鼠标的哪一次点击将用于绘制（`左，右，中`）。

## 方法二

这里不用手动添加轴，我们也可以使用 `plt.subplots()` 自动创建轴来完成。
...
同样的一系列事件正在发生，就像我们之前的方法一样。唯一不同的是，这里我们是在 `plt.subplots()` 的帮助下自动创建轴。如果你穿过终端，你可以看到大量的坐标，这些坐标是我们在图形中绘制的点。