# Python 中的 matplotlib.axes.axes.semilogy()

> 原文:[https://www.geeksforgeeks.org/matplotlib-axes-axes-semilogy-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-semilogy-in-python/)
...
`matplotlib`库的`Axes`模块中的`axes.semilogy()`函数用于在 y 轴上进行对数缩放的绘图。
...
>
> **返回:**这将返回以下内容:
> *   `线`:这将返回表示打印数据的 Line2D 对象列表。

下面的例子说明了`matplotlib.axes.axes.semilogy()`函数在`matplotlib.axes`中的作用:
