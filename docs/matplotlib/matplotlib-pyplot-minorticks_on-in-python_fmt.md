# matplotlib.pyplot.minorticks_on() 在 Python 中的使用

> 原文: [https://www.geeksforgeeks.org/matplotlib-pyplot-minorticks_on-in-python/](https://www.geeksforgeeks.org/matplotlib-pyplot-minorticks_on-in-python/)
...
`matplotlib` 库 `pyplot` 模块中的 `minorticks_on()` 函数用于显示轴上的次要刻度。
...
下面的例子说明了 `matplotlib.pyplot.minorticks_on()` 函数在 `matplotlib.pyplot` 中的作用:
