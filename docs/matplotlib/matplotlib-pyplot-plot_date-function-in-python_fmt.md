...
> 原文：[https://www.geeksforgeeks.org/matplotlib-pyplot-plot_date-function-in-python/](https://www.geeksforgeeks.org/matplotlib-pyplot-plot_date-function-in-python/)
...
| No. | parameter/independent variable | Description |
| :-- | :-- | :-- |
| 1. | `x`, `y` | `x` 和 `y` 都是数据的坐标，即 `x` 轴水平，`y` 轴垂直。 |
| 2. | `fmt` | 它是一个可选的字符串参数，包含相应的绘图细节，如颜色、样式等。 |
| 3. | `tz` | `tz` 代表用于标注日期的时区，默认为 `(UTC)`。 |
| 4. | `xdate` | `xdate` 参数包含布尔值。如果 `xdate` 为真，则 `x` 轴在 `matplotlib` 中解释为日期。默认情况下，`xdate` 为真。 |
| 5. | `ydate` | 如果 `ydate` 为真，则 `y` 轴在 `matplotlib` 中解释为日期。默认情况下，`ydate` 为 `false`。 |
| 6. | `data` | 将用于绘图的数据。 |

最后一个参数 `**kwargs` 是控制 `Line2D` 属性的关键字参数，如动画、`dash_joinstyle`、颜色、线宽、线型、标记等。
...
日期格式更改为日-月-年。要了解更多关于 [`DateFormatter`](https://www.geeksforgeeks.org/matplotlib-dates-dateformatter-class-in-python/) 和 [`gca()`](https://www.geeksforgeeks.org/matplotlib-pyplot-gca-in-python/) 的信息，请点击此处。