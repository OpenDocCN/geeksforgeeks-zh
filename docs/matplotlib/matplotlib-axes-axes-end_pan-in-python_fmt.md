...
> 原文: [`https://www.geeksforgeeks.org/matplotlib-axes-axes-end_pan-in-python/`](https://www.geeksforgeeks.org/matplotlib-axes-axes-end_pan-in-python/)

[`Matplotlib`](https://www.geeksforgeeks.org/python-introduction-matplotlib/) 是 Python 中的一个库，是 [`NumPy`](https://www.geeksforgeeks.org/python-introduction-matplotlib/) 库的数值-数学扩展。`Axes` 类包含了大部分的图形元素:轴、刻度、`Line2D`、文本、多边形等，并设置坐标系。`Axes` 的实例通过回调属性支持回调。
