...
> 2.  `fignum`: 它接受三个值，即 `None`、`False` 或一个整数值。如果该值设置为 `None`，将创建一个自动编号的新图形窗口。如果该值是非零整数，那么它将被绘制到相应于给定数字的图形中，或者如果它不存在，则创建它。如果 `0` 被设置为该参数的值，则它使用当前轴，如果不存在，则创建一个轴。
> 
> **返回:** 返回 `AxesImage` 类的图像。
> **其他参数:** 它也接受 `imshow` 显示图像的参数。
