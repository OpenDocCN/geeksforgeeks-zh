...
> 原文：[https://www.geeksforgeeks.org/matplotlib-axes-axes-get_picker-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-get_picker-in-python/)
...
## `matplotlib.axes.Axes.get_picker()`函数

`matplotlib`库的`Axes`模块中的`Axes.get_picker()`函数用于返回艺术家的拾取行为。
...
下面的例子说明了`matplotlib.axes.Axes.get_picker()`函数在`matplotlib.axes`中的作用：
