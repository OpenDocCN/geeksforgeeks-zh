...
> 原文: [https://www.geeksforgeeks.org/formatting-axes-in-python-matplotlib/](https://www.geeksforgeeks.org/formatting-axes-in-python-matplotlib/)
...
这就是你认为的‘图’。它是包含数据空间的图像区域。轴包含两个或三个处理数据限制的轴（如果是三维）对象。下图显示了包含图形的图形的不同部分。
...
>
> *   **bottom** 和 **top** – 浮动，可选
>     数据坐标中的底部 ylim（起点）和顶部 ylim（终点）。不通过会使限制保持不变。
> *   **auto** – 布尔或无，可选
...
# are multiples of 10 and Label major
