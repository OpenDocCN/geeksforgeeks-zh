...
> **语法:** `Axes.xcorr(self, x, y, normed=True, detrend=mlab.detrend_none, usevlines=True, maxlags=10, *, data=None, **kwargs)`
> 
> **参数:**该方法接受以下描述的参数:
> 
> *   `x, y` : 这些参数是标量的序列。
> *   `detrend`:此参数为可选参数。其默认值为 `mlab.detrend_none`
> *   `normed`:该参数也是可选参数，包含 bool 值。其默认值为 `True`
> *   `usevlines`:该参数也是可选参数，包含 bool 值。其默认值为 `True`
> *   `maxlags`: 此参数也是可选参数，包含整数值。其默认值为 `10`
> *   `linestyle`:该参数也是可选参数，用于绘制数据点，仅当 `usevlines` 为 `False` 时使用。
> *   `marker`:该参数也是可选参数，包含字符串。其默认值为
> 
> **返回:**该方法返回以下内容:
> 
> *   `lags`:该方法返回滞后向量
> *   `c`: 该方法返回自相关向量。
> *   `line`: 加`LineCollection`如果 `usevlines` 为 `True`，否则加`Lines2D`。
