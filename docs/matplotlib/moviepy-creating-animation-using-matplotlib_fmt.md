...
> 原文: [https://www.geeksforgeeks.org/moviepy-creating-animation-using-matplotlib/](https://www.geeksforgeeks.org/moviepy-creating-animation-using-matplotlib/)
...
> 2. 导入 `moviepy` 模块
> 3. 创建 `numpy` 数组
> 4. 使用 `matplotlib` 创建一个 `subplot`
> 5. 通过调用 `make_frame` 方法创建视频剪辑文件
> 6. 制作框架的方法
> 7. 根据帧时间，使用三角学方法清除地块并创建新地块
> 8. 将绘图转换为 `numpy` 图像后返回。
