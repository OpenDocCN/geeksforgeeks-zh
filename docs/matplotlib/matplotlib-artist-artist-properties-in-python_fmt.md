...
[`Matplotlib`](https://www.geeksforgeeks.org/python-introduction-matplotlib/) 是 Python 中的一个库，它是 NumPy 库的数值-数学扩展。`Artist` 类包含用于渲染到 `FigureCanvas` 的对象的抽象基类。图形中的所有可见元素都是 `Artist` 的子类。

## matplotlib.artist.Artist.properties() 方法

`matplotlib` 库 `artist` 模块中的 `properties()` 方法用于获取 `artist` 所有属性的字典。
...
下面的例子说明了 `matplotlib` 中的 `matplotlib.artist.Artist.properties()` 函数：
