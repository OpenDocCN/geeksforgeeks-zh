...
> 原文：[https://www.geeksforgeeks.org/change-font-size-in-matplotlib/](https://www.geeksforgeeks.org/change-font-size-in-matplotlib/)
...
`rcParams` 是 `matplotlib` 库的一个实例，用于处理默认 `matplotlib` 值，因此要更改默认字体大小，我们只需将值传递给关键 `font.size`。
...
## 方法 2：`matplotlib.rc(group, **kwargs)`

该功能设置当前 `rc` 参数。`group` 是像 `line`、`font`、`axes` 等 `rc` 的分组，`kwargs` 是字典属性名称和值对。
