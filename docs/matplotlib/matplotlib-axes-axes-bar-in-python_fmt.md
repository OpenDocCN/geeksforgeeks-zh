# Matplotlib.axes.Axes.bar() 用法详解

> 原文链接：[https://www.geeksforgeeks.org/matplotlib-axes-axes-bar-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-bar-in-python/)
...
`matplotlib` 库的 `Axes` 模块中的 `Axes.bar()` 函数用于制作条形图。
...
* `x`：该参数是条的水平坐标序列。
* `height`：该参数是条的高度。
* `width`：此参数为可选参数。默认值为 `0.8` 的条宽。
* `bottom`：该参数也是可选参数。默认值为 `0`，是条基座的 y 坐标。
