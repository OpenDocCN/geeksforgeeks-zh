# Python 中的 `matplotlib.artist.Artist.set_zorder()`

> 原文: [https://www.geeksforgeeks.org/matplotlib-artist-artist-set_zorder-in-python/](https://www.geeksforgeeks.org/matplotlib-artist-artist-set_zorder-in-python/)
...
## `matplotlib.artist.Artist.set_zorder()` 方法

`matplotlib` 库的 `artist` 模块中的 `set_zorder()` 方法用于为艺术家设置 zorder。

> **语法:** `Artist.set_zorder(self, level)`
>
> **参数:** 该方法接受以下参数。
>
> *   `level`: 该参数包含浮点值。
...
以下示例说明了 `matplotlib` 中的 `matplotlib.artist.Artist.set_zorder()` 函数:
