...
> 原文: `https://www.geeksforgeeks.org/matplotlib-axes-axes-text-in-python/`
...
`matplotlib` 库的 `Axes` 模块中的 `Axes.text()` 函数也用于将文本 `s` 添加到数据坐标中 `x`，`y` 位置的轴上。
...
> *   `withdash`: 该参数也是可选参数，创建带灰的文本实例，而不是文本实例。
>
> **返回:** 该方法返回 `Text`，这是一个创建的文本实例。
