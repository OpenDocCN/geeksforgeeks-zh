# matplotlib.axes.get_xlim() 用法详解

> 原文链接: https://www.geeksforgeeks.org/matplotlib-axes-get_xlim-in-python/

**Matplotlib** 是 Python 中的一个库，是 NumPy 库的数值-数学扩展。**Axes** 类包含了大部分的图形元素：轴、刻度、线条、文本、多边形等，并设置坐标系。`Axes` 的实例通过回调属性支持回调。

## matplotlib.axes.Axes.get_xlim() 函数

`matplotlib` 库的 `Axes` 模块中的 `Axes.get_xlim()` 函数用于获取 x 轴视图限制。
