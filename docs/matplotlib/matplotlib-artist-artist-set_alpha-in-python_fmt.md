# Python 中的 `matplotlib.artist.Artist.set_alpha()`

> 原文: [https://www.geeksforgeeks.org/matplotlib-artist-artist-set_alpha-in-python/](https://www.geeksforgeeks.org/matplotlib-artist-artist-set_alpha-in-python/)
...
## `matplotlib.artist.Artist.set_alpha()` 方法

`matplotlib`库的`artist`模块中的`set_alpha()`方法用于设置用于混合的 alpha 值。
...
以下示例说明了`matplotlib`中的`matplotlib.artist.Artist.set_alpha()`函数:
