# matplotlib.axes.contourf() 用法详解

> 原文链接: [https://www.geeksforgeeks.org/matplotlib-axes-contourf-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-contourf-in-python/)
...
`matplotlib` 库的 `Axes` 模块中的 `Axes.contourf()` 函数也用于绘制等高线。但是，`contourf()` 绘制填充轮廓，而 `contour()` 绘制轮廓线。
...
*   `X`, `Y`: 这些参数是 `Z` 中数值的坐标。
*   `Z`: 该参数是绘制轮廓的高度值。
*   `levels`: 此参数用于确定轮廓线/区域的数量和位置。

**返回:** 这将返回以下内容:
*   `c`: 这将返回一个 `QuadContourSet` 对象。
