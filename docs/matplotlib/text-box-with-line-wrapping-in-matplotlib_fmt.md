...
> 原文：[https://www.geeksforgeeks.org/text-box-with-line-wrapping-in-matplotlib/](https://www.geeksforgeeks.org/text-box-with-line-wrapping-in-matplotlib/)
...
*   `figtext()`：`figtext()` 可以作为 `text()` 方法的替代。`figtext()` 有助于将文本放置在图形上的任何位置。我们也可以将文本放在图的坐标轴之外。
...
*   `annotate()`：借助这些方法，我们可以通过添加箭头来注释地块上的一个点。
