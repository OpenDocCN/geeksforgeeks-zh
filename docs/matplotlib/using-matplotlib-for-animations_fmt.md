...
> 原文: [https://www.geeksforgeeks.org/using-matplotlib-for-animations/](https://www.geeksforgeeks.org/using-matplotlib-for-animations/)
...
需要注意的是，我们必须始终保持对动画对象的引用，否则动画将停止。这是因为 `Animation` 类保存了对动画对象的单个指针引用，并且随着时间的推移运行动画，必须保留该指针引用，否则它将作为垃圾值被收集。
