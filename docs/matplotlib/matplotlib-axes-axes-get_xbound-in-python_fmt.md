# Python 中的 matplotlib.axes.Axes.get_xbound()

> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-get_xbound-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-get_xbound-in-python/)
...
`matplotlib` 库的 `Axes` 模块中的 `Axes.get_xbound()` 函数用于以递增的顺序返回 x 轴的数值上下限。
...
下面的例子说明了 `matplotlib.axes.Axes.get_xbound()` 函数在 `matplotlib.axes` 中的作用：
