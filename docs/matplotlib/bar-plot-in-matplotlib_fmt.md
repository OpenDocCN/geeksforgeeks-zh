...
在这里，``plt.bar(courses, values, color='maroon')`` 用于指定将 ``courses`` 列作为 X 轴，``values`` 作为 Y 轴绘制条形图。``color`` 属性用于设置条形的颜色(本例中为栗色)。``plt.xlabel("Courses offered")`` 和 ``plt.ylabel("No. of students enrolled")`` 用于标记相应的轴。``plt.title()`` 用于制作图表的标题。``plt.show()`` 用于使用前面的命令将图表显示为输出。
