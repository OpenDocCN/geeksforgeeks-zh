# 在 Python-Matplotlib 中更改三维曲面图中的网格线厚度

> 原文: [https://www.geeksforgeeks.org/change-grid-line-thickness-in-3d-surface-plot-in-python-matplotlib/](https://www.geeksforgeeks.org/change-grid-line-thickness-in-3d-surface-plot-in-python-matplotlib/)

**先决条件:** [Matplotlib](https://www.geeksforgeeks.org/three-dimensional-plotting-in-python-using-matplotlib/)
...
*   导入必要的库。
*   创建用于打印的三维数据。
*   使用 `ax = matplotlib.pyplot.gca('projection=3d')` 创建三维坐标系，这里 GCA 代表获取当前网格，通过参数 `projection = 3d`，它将生成三维坐标系。
*   现在为了改变网格线的厚度，我们必须通过使用 `ax.xaxis.update({"linewidth":3})` 或任何你想要设置的网格宽度来更新各个轴的轴信息，这是针对 x 轴的，就像你可以通过分别写 Y axis，Z axis 代替 `xaxis` 来设置 Y 轴和 Z 轴一样。
*   如果想改变各个轴的颜色，只需在字典 `ax.xaxis.update({"linewidth":3, "color":"red"})` 中传递 `"color":"red"` 或您想要设置的任何颜色。
...
### 示例 2: 使用 Matplotlib 更改三维曲面图中 Y 轴网格线的厚度。
...
### 示例 3: 使用 Matplotlib 改变 3D 曲面图中 Z 轴网格线的粗细。
