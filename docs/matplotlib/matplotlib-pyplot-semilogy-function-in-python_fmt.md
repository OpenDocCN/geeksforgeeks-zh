# Python 中的 `matplotlib.pyplot.semilogy()` 函数

> 原文: [https://www.geeksforgeeks.org/matplotlib-pyplot-semilogy-function-in-python/](https://www.geeksforgeeks.org/matplotlib-pyplot-semilogy-function-in-python/)
...
`matplotlib` 库 `pyplot` 模块中的 `matplotlib.pyplot.semilogy()` 函数用于在 y 轴上进行对数缩放的绘图。
...
> *   `nonposy`: 此参数是 y 中的非正值，可被屏蔽为无效，或被裁剪为非常小的正数。
>
> **返回:** 这将返回以下内容:
>
> *   `lines`: 这将返回表示打印数据的线 2D 对象列表。

以下示例说明了 `matplotlib.pyplot.semilogy()` 函数在 `matplotlib.pyplot` 中的用法:
