# matplotlib.axes.set_aspect() 用 Python

> 哎哎哎: [# t0]https://www.geeksforgeeks.org/matplotlib-axes-set_aspect-in-python/

[`Matplotlib`](https://www.geeksforgeeks.org/python-introduction-matplotlib/) 是 Python 中的一个库，是 [`NumPy`](https://www.geeksforgeeks.org/python-introduction-matplotlib/) 库的数值-数学扩展。

[`Axes`](https://www.geeksforgeeks.org/python-introduction-matplotlib/) 类包含大部分图形元素：轴、刻度、线 2D、文本、多边形等，并设置坐标系。`Axes` 的实例通过回调属性支持回调。
...
[`matplotlib`](https://www.geeksforgeeks.org/python-introduction-matplotlib/) 库的 [`Axes`](https://www.geeksforgeeks.org/python-introduction-matplotlib/) 模块中的 `Axes.set_aspect()` 函数用于设置轴缩放的纵横比，即 y 单位与 x 单位的比值。
...
下面的例子说明了 `matplotlib.axes.Axes.set_aspect()` 函数在 `matplotlib.axes` 中的作用：
