...
> 原文: [https://www.geeksforgeeks.org/how-to-change-matplotlib-color-bar-size-in-python/](https://www.geeksforgeeks.org/how-to-change-matplotlib-color-bar-size-in-python/)
...
使用 `colorbar()` 函数的 `shrink` 属性，我们可以缩放 colorbar 的大小。

> 语法: `matplotlib.pyplot.colorbar(mappable=None, shrink=缩放)`
...
使用此类，您可以更改颜色条轴的大小，但高度将与当前轴相同。这里我们使用 `make_axes_locatable` 函数，该函数返回显示图像的当前轴的 `AxisDivider` 对象。
