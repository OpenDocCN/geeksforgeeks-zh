...
> 原文: [https://www.geeksforgeeks.org/create-a-pseudocolor-plot-of-an-unstructured-triangular-grid-in-python-using-matplotlib/](https://www.geeksforgeeks.org/create-a-pseudocolor-plot-of-an-unstructured-triangular-grid-in-python-using-matplotlib/)

`Matplotlib` 是一个可以进行数据可视化的 Python 模块；并以饼图、直方图、条形图等形式对数据进行图形化分析。它还具有生成类似 MATLAB 的框架的能力。

## 非结构化三角形网格

非结构化三角形网格包含 **n 点**点和 **n 点**三角形，这些三角形可以由用户指定，也可以使用**德劳奈三角剖分**自动生成。

> **语法:** `matplotlib.tri.Triangulation(x, y, triangles=None, mask=None)`
>
> **参数:**
>
> *   `x, y`: 指定网格点的坐标。
> *   `triangles`: *[可选]* 形状的整数数组 `(n_tri, 3)`
> *   `mask`: *[可选]* 它指定哪些三角形被遮罩掉。
...
> **参数:**
>
> 1.  `cmap`: 可以是 `None`，也可以是 `matplotlib` colormap。`matplotlib` 有很多内置的 colormaps 可以通过 `matplotlib.cm.get_cmap` 访问。
...
>     *   颜色序列将设置颜色。
> 4.  `facecolors`: 提到字体。
> 5.  `shading`: 可以是 `'flat'` 也可以是 `'gouraud'`。
> 6.  `norm`: 如果其为 `None` 默认为 `Normalize()`。
> 7.  `vmax`: 可以是 `None`，也可以是标量值。
> 8.  `vmin`: 可以是 `None`，也可以是标量值。( *`vmax` 和 `vmin` 与归一化数据*结合使用)
