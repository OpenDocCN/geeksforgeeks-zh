# matplotlib.ticker.IndexFormatter 类

> 原文：[https://www.geeksforgeeks.org/matplotlib-ticker-indexformatter-class-in-python/](https://www.geeksforgeeks.org/matplotlib-ticker-indexformatter-class-in-python/)
...
`matplotlib.ticker.IndexFormatter` 类是 `matplotlib.ticker` 类的子类，用于格式化位置 `x`，该位置是 `i = int(x + 0.5)` 的最近的第 `i` 个标签。带有 `i >= len(list)` 的位置有 0 个刻度标签。

> **语法：** `matplotlib.ticker.IndexFormatter(labels)`
>
> **参数：**
>
> *   **labels：** 是标签列表。
