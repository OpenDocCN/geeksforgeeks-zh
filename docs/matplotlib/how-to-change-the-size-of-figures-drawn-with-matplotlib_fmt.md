...
这可以通过一个名为`figsize`的属性在`matplotlib`中来实现。`figsize`属性允许我们以单位英寸来指定图形的宽度和高度。

> **语法:**
>
> 将 matplotlib.pyplot 输入为 plt
>
> `fig_name = plt.figure(figsize=(宽度, 高度))`
...
*   我们创建一个变量`plt_1`，并将其设置为`plt.figure(figsize=(6, 3))`。
...
如果您使用的是除 jupyter 笔记本之外的 python IDE，那么这种方法是可行的。如果您正在使用 jupyter 笔记本，那你就不会用了，`plt.show()`。相反，您可以在导入`matplotlib`后使用`%matplotlib`。
