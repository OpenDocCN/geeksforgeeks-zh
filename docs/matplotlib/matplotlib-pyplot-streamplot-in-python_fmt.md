# matplotlib.pyplot.streamplot() 用法详解

> 哎哎哎: # t0] https://www.geeksforgeeks.org/matplotlib-pyplot-streamplot-in-python/
...
这里 `x_grid` 和 `y_grid` 是 x 和 y 点的数组。`x_vec` 和 `y_vec` 代表网格上每个点的流速。属性 `density` 指定将流线绘制在一起的距离。
...
这里，`x` 和 `y` 是均匀间隔网格上的 1D 阵列，`u` 和 `v` 是速度为 x 和 y 的 2D 阵列，其中行数应与 `y` 的长度相匹配，而列数应与 `x` 相匹配，`density` 是控制流线紧密度的浮点值。
