# matplotlib.artist.Artist.get_clip_on()方法

> 原文: [https://www.geeksforgeeks.org/matplotlib-artist-artist-get_clip_on-in-python/](https://www.geeksforgeeks.org/matplotlib-artist-artist-get_clip_on-in-python/)
...
`matplotlib`库的`artist`模块中的`get_clip_on()`方法用于获取艺术家是否使用剪辑。
...
以下示例说明了`matplotlib`中的`matplotlib.artist.Artist.get_clip_on()`函数:
