...
> 原文: [https://www.geeksforgeeks.org/create-lollipop-charts-with-pandas-and-matplotlib/](https://www.geeksforgeeks.org/create-lollipop-charts-with-pandas-and-matplotlib/)
...
首先，我们将通过 `plt.subplots()` 创建一个空图表，并使用接受(x，y)形式的参数的 `stem()` 图，这两个参数都可以是类似数组的值，我们将这些值绘制到图表中。`use_line_collection=True` 提高了茎图的性能，将茎线绘制为线条集合，而不是单个线条。`basefmt=' '` 从茎线上移除基线。
...
*   导入必要的库 (`matplotlib.pyplot` 和 `pandas`)
*   打开 CSV 文件并读取数据
*   使用 `plt.stem()`
