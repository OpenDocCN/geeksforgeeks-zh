...
## matplotlib.pyplot.ylabel() function

`matplotlib` 库 `pyplot` 模块中的 `ylabel()` 函数用于设置 y 轴的标签。

> **语法:** `matplotlib.pyplot.ylabel(ylabel, fontdict=None, labelpad=None, **kwargs)`
>
> **参数:** 该方法接受以下描述的参数:
>
> *   **`ylabel`:** 该参数为标签文本。并包含字符串值。
> *   **`labelpad`:** 此参数用于从轴边界框(包括刻度和刻度标签)以点为单位的间距，其默认值为 `None`。
> *   **`**kwargs`:** 该参数是 `Text` 属性，用于控制标签的外观。
...
```py
# Implementation of matplotlib.pyplot.ylabel() function
