...
## matplotlib.pyplot.yscale()函数详解

`matplotlib`库中`pyplot`模块的`matplotlib.pyplot.yscale()`函数用于设置 y 轴比例。

**语法：**
`matplotlib.pyplot.yscale(value, **kwargs)`

**参数:**
- `value` = {`"linear"`, `"log"`, `"symlog"`, `"logit"`... }，这些是各种适用的轴比例。
- `**kwargs` = 根据比例尺（`matplotlib.scale.LinearScale`、`LogScale`、`SymmetricalLogScale`、`LogitScale`）不同，接受不同的关键字参数。

### 示例1：使用yscale设置线性与对数坐标轴
...
plt.subplots_adjust(top=0.80, bottom=0.03, left=0.15, right=0.92, hspace=0.34, wspace=0.45)
