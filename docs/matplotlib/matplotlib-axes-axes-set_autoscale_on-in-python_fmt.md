...
> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-set_autoscale_on-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-set_autoscale_on-in-python/)
...
## matplotlib.axes.Axes.set_autoscale_on() 函数

`matplotlib` 库的 `Axes` 模块中的 `Axes.set_autoscale_on()` 函数用于设置绘图命令是否应用自动缩放。
...
下面的例子说明了 `matplotlib.axes.Axes.set_autoscale_on()` 函数在 `matplotlib.axes` 中的作用：
