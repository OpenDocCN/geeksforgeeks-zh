...
| `data` | 两个包含标签数据的标签名的表。 |
| `where` | 决定垂直线的位置。(值：`pre` | `post` | `mid`) |

**注意：** 参数 `fmt`、`data`、`where` 为可选。
...
查看突出显示的部分，查看值 `pre` 和 `post` 之间的差异。
...
`mid`：垂直线在一个值 (`x[i-1]` 和 `x[i+1]`)/2 处向上射出。下面给出了上面绘制的同一个步骤的曲线图。在下面的代码中，`'cs'` 表示线条颜色为红色，标记应为正方形。
