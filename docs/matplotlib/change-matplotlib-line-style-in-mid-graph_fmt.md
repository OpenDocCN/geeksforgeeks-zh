# 更改中间图中 matplotlib 线条样式

> 原文：`https://www.geeksforgeeks.org/change-matplotlib-line-style-in-mid-graph/`

## 先决条件

在本文中，我们将学习如何使用 Python 中的 `matplotlib` 来更改中间图形中的线条样式。

*   `Matplotlib`：这是一个巨大的 Python 可视化库，用于数组的 2D 图。`Matplotlib` 可能是一个多平台数据可视化库，构建在 `NumPy` 数组上，旨在与更广泛的 `SciPy` 堆栈一起工作。它是由约翰·亨特在 2002 年推出的。
