...
> 原文: [https://www.geeksforgeeks.org/covid-19-data-visualization-using-matplotlib-in-python/](https://www.geeksforgeeks.org/covid-19-data-visualization-using-matplotlib-in-python/)
...
**链接到数据集(CSV):** [点击此处](https://www.geeksforgeeks.org/covid-19-data-visualization-using-matplotlib-in-python/)
...
`case_time_series.csv` 数据集有 7 列。我们将收集每日确认、每日恢复和每日死亡的变量作为数组。
...
`grid` 功能允许您在图形上创建网格线。网格线的宽度可以通过简单地传递参数 `linewidth` 来调整，通过传递 `color` 参数来改变其颜色。
...
`set_facecolor` 允许您设置图形的背景颜色，这里是黑色。`set_xlabel` 和 `set_ylabel` 允许您沿两个轴设置标签，标签的大小和颜色可以更改。
...
现在我们通过调用绘图函数再次绘制图表，其中 `X` 为日期，`Y` 为每日确认。可以通过传递 `color`、`linewidth` 来更改颜色并调整绘图线和标记的宽度，`marker` 来创建标记（在这种情况下是圆形），`markersize` 来调整标记的大小并定义 `markeredgecolor` 标记的边缘颜色。
...
`xticks` 和 `yticks` 允许您更改日期和每日确认字体。为了使日期不相互重叠，我们将通过传递 `rotation='vertical'` 来垂直表示它们。为了使刻度易于阅读，我们将字体颜色改为白色，大小改为 20。
...
`annotate` 允许您在图表上进行注释。在这里，我们编写了一个代码，通过运行一个 for 循环来注释绘制点，该循环在绘制点绘制。`str(j)` 包含 `Y` 变量，即每日确认。传递的任何字符串都将被绘制出来。`xy` 是应该绘制字符串的坐标。最后可以定义颜色和大小。请注意，我们在 `xy` 坐标中为 `j` 添加了 `+100`，这样字符串就不会与标记重叠，并且它在 Y 轴上的距离为 100 个单位 — 轴。
...
要注释一个指向图中某个位置的箭头，并且它的尾部包含字符串，我们可以定义 `arrowprops` 参数以及由 `xytext` 定义的尾部坐标。请注意，`arrowprops` 的修改可以使用字典来完成。
...
因此，我们已经创建了列表 `slices`，基于该切片我们的饼图将被划分，并且相应的 `activities` 是有价值的。
...
绘制一个我们称之为“饼图”的图表。`pie` 函数，它接受 x 个值，这里是 `slices`，在此基础上，pie 被分割，后面是 `labels`，标签有对应的字符串和它所代表的值。`textprops` 可以更改这些字符串值。要改变饼图的半径或大小，我们称之为 `radius`。对于美学，我们称 `shadow` 为真，`startangle` 为 90°。我们可以通过传递相应颜色的列表来定义要分配的 `colors`。为了分隔出每一块派，我们可以传递相应的值列表来 `explode`。`autopct` 定义了允许显示的位置数量。在这种情况下，`autopct` 允许小数点前后有 2 个位置。
...
画一个我们称之为`bar`的条形图。并传递给它x轴和y轴值。在这里，我们为所有三种情况(即已恢复、已确认、已死亡)调用了三次绘图函数，并且所有三个值都是相对于y轴和x轴绘制的，这三个轴都是地区名称。如您所见，我们通过迭代已确认病例值，仅对已确认的数字进行注释。此外，我们已经提到了。`legend`表示图形的图例。
