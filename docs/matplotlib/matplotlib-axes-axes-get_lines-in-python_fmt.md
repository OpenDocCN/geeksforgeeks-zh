...
> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-get_lines-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-get_lines-in-python/)
...
`matplotlib` 库的 `axes` 模块中的 `Axes.get_lines()` 函数用于返回 `Axes` 包含的行列表。

> **语法:** `axes.get_lines(self)`
>
> **参数:** 此方法不接受任何参数。
>
> **返回:** 该方法返回坐标轴包含的行列表。
