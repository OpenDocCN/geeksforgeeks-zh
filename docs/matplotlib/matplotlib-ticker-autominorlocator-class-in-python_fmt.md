...
> 原文: [https://www.geeksforgeeks.org/matplotlib-ticker-autominorlocator-class-in-python/](https://www.geeksforgeeks.org/matplotlib-ticker-autominorlocator-class-in-python/)
...
> **语法:** `class matplotlib.ticker.AutoMinorLocator(n=None)`
>
> **参数:**
>
> *   `n`: 表示主要刻度间隔的细分数。如果省略 `n` 或为 `None`，它会自动设置为 5 或 4。

### 该类方法

*   `tick_values(self, vmin, vmax)`: 给定 `vmin` 和 `vmax`，它返回找到的刻度的值。
