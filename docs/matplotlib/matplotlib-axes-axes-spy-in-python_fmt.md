...
> 原文: [`https://www.geeksforgeeks.org/matplotlib-axes-axes-spy-in-python/`](https://www.geeksforgeeks.org/matplotlib-axes-axes-spy-in-python/)
...
`matplotlib` 库中 `Axes` 模块中的 `Axes.spy()` 函数也用于绘制 2D 阵列的稀疏模式。它还用于可视化数组的非零值。
...
>
> *   `Z`: 此参数是要绘制的数组。
> *   `precision`: 此参数用于确定是否要绘制任何非零值。
> *   `origin`: 该参数将数组的 `[0, 0]` 索引放在轴的左上角或左下角。
> *   `aspect`: 该参数为可选参数，用于控制轴的纵横比。
>
> **返回:** 这将返回以下内容:
>
> *   `ret`: 这将返回 `AxesImage` 或 `Line2D`。这取决于绘图风格。
