# matplotlib.pyplot.plot_date() 用法

> 哎哎哎: [https://www.geeksforgeeks.org/matplotlib-pyplot-plot_date-in-python/](https://www.geeksforgeeks.org/matplotlib-pyplot-plot_date-in-python/)

**[Matplotlib](https://www.geeksforgeeks.org/python-introduction-matplotlib/)** 是 Python 中的一个库，是 NumPy 库的数值-数学扩展。**[Pyplot](https://www.geeksforgeeks.org/pyplot-in-matplotlib/)** 是一个基于状态的接口到 **Matplotlib** 模块，它提供了一个类似于 MATLAB 的接口。

## `matplotlib.pyplot.plot_date()` 函数

`matplotlib` 库 `pyplot` 模块中的 `plot_date()` 函数用于绘制包含日期的数据。
...
*   `xdate`: 该参数也是可选参数。并且它包含带有默认值 `True` 的布尔值。如果为真，x 轴将被解释为 Matplotlib 日期。
*   `ydate`: 该参数也是可选参数。并且它包含带有默认值 `True` 的布尔值。如果为真，y 轴将被解释为 Matplotlib 日期。
