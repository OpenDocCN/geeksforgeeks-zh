...
> 原文: [https://www.geeksforgeeks.org/3d-visualisation-of-quick-sort-using-matplotlib-in-python/](https://www.geeksforgeeks.org/3d-visualisation-of-quick-sort-using-matplotlib-in-python/)
...
1.  我们将生成一个包含随机元素的数组。
2.  算法将被调用于该数组，并且为了可视化目的，将使用 `yield` 语句代替 `return` 语句。
3.  我们将获得比较和交换后的数组当前状态。因此，算法将返回一个生成器对象。
4.  将使用 `Matplotlib` 动画来可视化数组的比较和交换。
5.  然后我们将绘制一个图表，该图表将返回一个 `Poly3dCollection` 对象，该对象将用于进一步的动画。
