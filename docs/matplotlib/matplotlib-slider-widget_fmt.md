...
[Matplotlib](https://www.geeksforgeeks.org/python-introduction-matplotlib/) 提供了几个小部件来制作交互图。在这些小部件中，这里讨论了滑块小部件。滑块提供对绘图视觉属性的控制。`Slider()` 用于将表示绘图中浮点范围的滑块放置在提供的轴上。
...
1.  `ax`: 放置滑块的 `matplotlib.axes.Axes` 实例
2.  `label`: 滑块文本标签
3.  `valmin`: 滑块的最小值
4.  `valmax`: 滑块的最大值
5.  `valinit`: 滑块的初始值。默认值为 0.5。
6.  `valfmt`: 滑块值格式字符串(%)-格式。默认值为 `None`。如果没有，则使用 `ScalarFormatter`。
7.  `closedmin`: 滑块间隔是否在底部关闭。
8.  `closedmax`: 滑块间隔是否在顶部关闭。
9.  `slidermin`: 禁止当前滑块的值小于给定滑块的当前值。默认值为 `None`。
10. `slidermax`: 禁止当前滑块的值大于给定滑块的当前值。默认值为 `None`。
11. `dragging`: 滑块可以用鼠标拖动，也可以不用鼠标拖动。默认值为 `True`（可以用鼠标拖动滑块）。
12. `valstep`: 滑块将以 `valstep` 值的倍数滑动。默认值为 `None`。
13. `orientation`: 滑块方向，`'vertical'` 或 `'horizontal'`。默认值为 `'horizontal'`。
14. `**kwargs`: 与绘制滑块旋钮的矩形相关。有效属性，如 `facecolor`、`edgecolor`、`alpha` 等。这里可以使用矩形。

## 方法

*   `disconnect(self, cid)`: 移除连接 id 为 `cid` 的观察器。
*   `on_changed(self, func)`: 连接到滑块事件。当滑块值改变时，调用相应的功能 `func`。`func` 取一个新的滑块值作为参数，返回连接 id。
*   `reset(self)`: 滑块值设置为初始值。
*   `set_val(self, val)`: 将滑块值设置为 `val`。

## 例 1

以下示例演示了使用红色、绿色和蓝色值滑块改变条形图的颜色。
