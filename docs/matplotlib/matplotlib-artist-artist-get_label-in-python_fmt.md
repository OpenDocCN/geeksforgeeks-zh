# Python 中的 `matplotlib.artist.Artist.get_label()`

> 原文: [https://www.geeksforgeeks.org/matplotlib-artist-artist-get_label-in-python/](https://www.geeksforgeeks.org/matplotlib-artist-artist-get_label-in-python/)
...
## `matplotlib.artist.Artist.get_label()` 方法

`matplotlib` 库的 `artist` 模块中的 `set_label()` 方法用于设置将在图例中显示的标签。

> 语法: `Artist.get_label(self)`
>
> 参数: 此方法不接受任何参数。
>
> 返回: 该方法返回传说中该艺术家使用的标签。

下面的例子说明了 `matplotlib` 中的 `matplotlib.artist.Artist.get_label()` 函数:
