# matplotlib.axes.Axes.barbs() 函数详解

> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-barbs-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-barbs-in-python/)
...
`matplotlib` 库的 `Axes` 模块中的 `axes.barbs()` 函数也用于绘制倒刺的 2D 场。

> **语法:** `axes.barbs(self, *args, data=None, **kw)`
>
> **参数:** 该方法接受以下描述的参数:
>
...
> *   `length`: 该参数是倒钩的长度，以点为单位。
> *   `pivot`: 此参数是箭头锚定到 `X, Y` 网格的部分。
> *   `barbcolor`: 该参数类似于多边形的 `edgecolor` 参数，可以替代使用。
> *   `flagcolor`: 该参数类似于多边形的 `facecolor` 参数。
> *   `sizes`: 此参数是系数字典，指定给定特征与倒钩长度的比率。
> *   `fill_empty`: 此参数用于用标志色填充空倒刺。
> *   `barb_increments`: 该参数是增量字典，指定与倒钩不同部分相关联的值。
