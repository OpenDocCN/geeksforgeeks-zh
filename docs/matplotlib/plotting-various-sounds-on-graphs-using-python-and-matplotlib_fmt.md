...
> 原文: [https://www.geeksforgeeks.org/plotting-various-sounds-on-graphs-using-python-and-matplotlib/](https://www.geeksforgeeks.org/plotting-various-sounds-on-graphs-using-python-and-matplotlib/)

在本文中，我们将探索使用 Python 和 Matplotlib 可视化声波的方法。

## 需要的模块

`Matplotlib`: 使用以下命令安装 Matplotlib:

```py
pip install matplotlib
```

`Numpy`: Numpy 会随 Matplotlib 自动安装。但是，如果您遇到任何导入错误，请使用下面的命令安装 Numpy:
...
*   使用 `readframes()` 功能读取打开的声波的所有帧。
*   使用 `getframerate()` 功能将帧速率存储在变量中。
*   最后，使用帧速率以秒为单位绘制 x 轴。
