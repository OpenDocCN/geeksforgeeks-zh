...
> **语法:**
> `class matplotlib.widgets.RectangleSelector(ax, onselect, drawtype='box', minspanx=0, minspany=0, useblit=False, lineprops=None, rectprops=None, spancoords='data', button=None, maxdist=10, marker_props=None, interactive=False, state_modifier_keys=None)`

### 参数

*   `ax`: 放置小部件的 `matplotlib.axes.Axes` 实例。
*   `onselect`: 连接到选择事件的功能。选择完成后，调用相应的函数。`onselect` 函数以鼠标点击和鼠标释放事件作为参数。
*   `drawtype`: 指定如何显示选择。如果是 `'box'`，则画出完整的矩形方框。如果是 `'line'`，则画矩形线。如果 `'none'`，则不绘制任何内容。默认值为 `'box'`。
*   `button`: 提供可以触发矩形选择的鼠标按钮列表。默认情况下，所有按钮都是允许的。
*   `maxdist`: 是交互工具手柄可以激活的像素距离。默认值为 `10`。
*   `interactive`: 是一个布尔值，指定是否绘制一组手柄与小部件交互。

### 属性

*   `center`: 提供绘制矩形的中心。
*   `corners`: 提供矩形的角，从左下角开始，顺时针移动。
*   `edge_centers`: 提供从左开始顺时针移动的矩形边缘的中点。
*   `extents`: 返回 `(xmin, xmax, ymin, ymax)`。
*   `geometry`: 返回一个包含矩形四个角的 x 和 y 坐标的数组（从左上角开始和结束）。数组形状为 `(2, 5)`。所有 x 坐标都可以使用 `RectangleSelector.geometry[1, :]` 获得，y 坐标可以使用 `RectangleSelector.geometry[0, :]` 获得。
