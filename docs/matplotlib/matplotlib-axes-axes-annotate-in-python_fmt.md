# Matplotlib Axes Annotate() 用法详解

> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-annotate-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-annotate-in-python/)
...
`matplotlib` 库的 `Axes` 模块中的 `annotate()` 函数也用于给点 `xy` 标注文字文本。换句话说，我过去常常把文本放在 `xy`。
...
> *   `arrowprops`: 该参数也是可选参数，包含 `dict` 类型。其默认值为 `None`。
> *   `annotation_clip`: 此参数也是可选参数，包含布尔值。其默认值为 `None`，表现为 `True`。
>
> **返回:** 该方法返回 `Annotation`。

下面的例子说明了 `matplotlib.axes.Axes.annotate()` 函数在 `matplotlib.axes` 中的作用:
