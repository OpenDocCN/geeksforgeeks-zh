# matplotlib.axes.get_tightbbox() 用法详解

> 原文链接: [https://www.geeksforgeeks.org/matplotlib-axes-get_tightbbox-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-get_tightbbox-in-python/)
...
## matplotlib.axes.Axes.get_tightbbox() 函数

`matplotlib` 库的 `Axes` 模块中的 `get_tightbbox()` 函数用于返回轴的紧密包围盒，包括轴及其装饰器。

> **语法:** `get_tightbbox(self, renderer, call_axes_locator=True, bbox_extra_artists=None)`
>
> **参数:** 该方法接受以下参数。
>
> *   `renderer`: 该参数是将用于绘制图形的渲染器实例。
> *   `bbox_extra_artists`: 此参数是要包含在紧密包围盒中的艺术家列表。
> *   `call_axes_locator`: 此参数是要包含在紧密包围盒中的艺术家列表。
>
> **返回:** 该方法返回图形像素坐标中的包围盒。

下面的例子说明了 `matplotlib.axes.Axes.get_tightbbox()` 函数在 `matplotlib.axes` 中的作用：
