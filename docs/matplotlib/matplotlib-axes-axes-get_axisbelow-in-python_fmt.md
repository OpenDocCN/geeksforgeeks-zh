...
> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-get_axisbelow-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-get_axisbelow-in-python/)
...
## matplotlib.axes.Axes.get_axisbelow() 函数

`matplotlib` 库的 `Axes` 模块中的 `Axes.get_axisbelow()` 函数用来获取轴刻度和网格线是在大多数美工的上方还是下方。
...
下面的例子说明了 `matplotlib.axes.Axes.get_axisbelow()` 函数在 `matplotlib.axes` 中的作用：
