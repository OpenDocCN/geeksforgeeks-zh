# Python 中的 `matplotlib.artist.Artist.is_transform_set()`

> 原文: [https://www.geeksforgeeks.org/matplotlib-artist-artist-is_transform_set-in-python/](https://www.geeksforgeeks.org/matplotlib-artist-artist-is_transform_set-in-python/)
...
`matplotlib` 库的 `artist` 模块中的 `is_transform_set()` 方法用于获取 `Artist` 是否有显式设置的变换。
...
以下示例说明了 `matplotlib` 中的 `matplotlib.artist.Artist.is_transform_set()` 函数:
