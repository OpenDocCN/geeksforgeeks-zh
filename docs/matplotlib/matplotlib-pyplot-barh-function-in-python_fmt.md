...
> 原文: [https://www.geeksforgeeks.org/matplotlib-pyplot-barh-function-in-python/](https://www.geeksforgeeks.org/matplotlib-pyplot-barh-function-in-python/)
...
| :--- | :--- |
| `y` | `y` 条的坐标。 |
| `width` | 像标量或数组一样，表示条的宽度。 |
| `height` | 像标量或数组一样，表示条的高度（默认值为 0.8）。 |
| `left` | 标量或标量序列，表示条左侧的 X 坐标（默认值为 0）。 |
| `align` | `{'center', 'edge'}` 对齐 `y` 坐标的底部（默认值为 `center`）。 |
| `color` | 像标量或数组一样，表示条的颜色。 |
| `edgecolor` | 像标量或数组一样，表示条的边缘颜色。 |
| `linewidth` | 像标量或数组一样，表示条边的宽度。 |
| `tick_label` | 像标量或数组一样，表示条形的刻度标签（默认值为 `None`）。 |
...
这里，`plt.barh(courses, values, color='maroon')` 用于指定通过使用 `courses` 列作为 Y 轴，`values` 作为 X 轴来绘制条形图。`color` 属性用于设置条形的颜色（本例中为栗色）。`plt.xlabel("Courses offered")` 和 `plt.ylabel("No. of students enrolled")` 用于标记相应的轴。`plt.title()` 用于制作图表的标题。`plt.show()` 用于使用前面的命令将图表显示为输出。
