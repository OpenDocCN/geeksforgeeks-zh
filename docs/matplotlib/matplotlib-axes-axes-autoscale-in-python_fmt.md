# matplotlib.axes.Axes.autoscale() 用法详解

> 原文链接: [https://www.geeksforgeeks.org/matplotlib-axes-auto-scale-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-auto-scale-in-python/)

**[Matplotlib](https://www.geeksforgeeks.org/python-introduction-matplotlib/)** 是 Python 中的一个库，是 NumPy 库的数值-数学扩展。**轴类**包含了大部分的图形元素：轴、刻度、线二维、文本、多边形等，并设置坐标系。`Axes` 的实例通过回调属性支持回调。

## matplotlib.axes.Axes.autoscale() 函数

`matplotlib` 库的 `Axes` 模块中的 `Axes.autoscale()` 函数用于将轴视图自动缩放到数据（切换）。

> **语法:** `Axes.autoscale(self, enable=True, axis='both', tight=None)`
>
> **参数:** 该方法接受以下参数。
>
> *   `enable`: 如果该参数为真（默认），则自动缩放打开，否则关闭。
> *   `axis`: 该参数用于操作哪个轴。`{"both", "x", "y"}`
> *   `tight`: 该参数被转发到 `autoscale_view`。
>
> **返回值:** 此方法不返回值。
