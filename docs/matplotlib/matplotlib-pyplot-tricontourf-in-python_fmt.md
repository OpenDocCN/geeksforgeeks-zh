# matplotlib.pyplot.tricontourf()详解

> 原文链接: https://www.geeksforgeeks.org/matplotlib-pyplot-tricontourf-in-python/

[Matplotlib](https://www.geeksforgeeks.org/python-introduction-matplotlib/) 是 Python 中的一个库，是 NumPy 库的数值-数学扩展。[Pyplot](https://www.geeksforgeeks.org/pyplot-in-matplotlib/) 是一个基于状态的接口到 `Matplotlib` 模块，它提供了一个类似于 MATLAB 的接口。

## 函数说明

`matplotlib` 库中 `pyplot` 模块中的 `tricontourf()` 函数用于在非结构化的三角形网格上绘制填充等高线。
...
*   `triangulation`: 该参数是一个 `matplotlib.tri.Triangulation` 对象。
*   `z`: 此参数是要进行轮廓化的值的数组，三角测量中每个点一个。
*   `**kwargs`: 此参数是文本属性，用于控制标签的外观。

所有剩余的参数和 `kwargs` 与 `matplotlib.pyplot.plot()` 相同。

**返回:**
这将返回包含以下内容的 **2 行 2D** 列表:
*   为三角形边缘绘制的线。
*   为三角形节点绘制的标记。

**注意:** 仅 `tricontourf` 的关键字参数是 `antialiased`，这是一种用于非结构化三角形网格上的轮廓的布尔值，用于启用抗锯齿。

## 示例

下面的例子说明了 `matplotlib.pyplot.tricontourf()` 函数的用法。
