...
> 原文: [https://www.geeksforgeeks.org/create-an-animated-gif-using-python-matplotlib/](https://www.geeksforgeeks.org/create-an-animated-gif-using-python-matplotlib/)
...
*   这个想法是首先创建一个简单的任何函数的图(这里我们取了例如余弦)，然后 `FuncAnimation` 函数将继续调用 `动画函数`，在给定的时间间隔后作为参数连续传递给它。
*   我们只需要给出这样一个实现，它将导致我们已经通过的 `动画函数` 中的绘图位置发生变化。由于间隔太小(以毫秒为单位)，所以我们觉得它是动画。
...
*   现在让我们创建我们的动画功能，它将根据参数帧连续改变绘图的 x 和 y 坐标。原因是 `FuncAnimation` 会根据帧连续调用这个函数。因为动画仅仅意味着一帧接一帧地创建视频。
...
*   现在是时候调用我们的 `FuncAnimation` 函数了，它将根据帧数连续调用上面定义的函数。我们给出的时间间隔是 25 毫秒。
