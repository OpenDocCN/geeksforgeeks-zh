# matplotlib.axes.loglog() 中的 Python
...
`matplotlib` 库的 `Axes` 模块中的 `Axes.loglog()` 函数用于在 x 轴和 y 轴上进行对数缩放的绘图。
...
> *   `subsx`, `subsy`: 这些参数是次要 x/y 刻度的位置序列，是可选的。
> *   `nonposx`, `nonpsy`: 这些参数是 x 或 y 中的非正值，可以被屏蔽为无效，或者被裁剪为非常小的正数。
>
> **返回:** 这将返回以下内容:
> *   `lines`: 这将返回表示打印数据的 `Line2D` 对象列表。
