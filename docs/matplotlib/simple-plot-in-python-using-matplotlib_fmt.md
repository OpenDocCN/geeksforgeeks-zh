...
> 原文: [https://www.geeksforgeeks.org/simple-plot-in-python-using-matplotlib/](https://www.geeksforgeeks.org/simple-plot-in-python-using-matplotlib/)
...
*   将 x 轴和相应的 y 轴值定义为列表。
*   使用`plot()`函数在画布上绘制它们。
*   使用`xlabel()`和`ylabel()`函数给 x 轴和 y 轴命名。
*   使用`title()`函数给你的情节一个标题。
...
| 方法 | 描述 |
| --- | --- |
| `figure()` | 它在计算机的背景下创造情节，而不是显示它。我们还可以添加一个标签，作为我们将称之为这个图的名称的论据——在`legend()`中使用 |
| `show()` | 它显示创建的地块 |
| `xlabel()` | 它标记 x 轴 |
| `ylabel()` | 它标记 y 轴 |
| `title()` | 它给出了图表的标题 |
| `gca()` | 它有助于访问图的所有四个轴 |
| `gca().spines['right/left/top/bottom'].set_visible(True/False)` | 它可以接近单个脊椎或单个边界，并帮助改变它们的可见性 |
| `xticks()` | 它决定如何在 x 轴上做标记 |
| `yticks()` | 它决定如何在 y 轴上做标记 |
| `gca().legend()` | 传递一个列表作为所有绘制图的参数，如果未明确指定标签，则按照绘制图的相同顺序添加列表中的值 |
| `annotate()` | 它用于在图表的指定位置写注释 |
| `figure(figsize = (x, y))` | 每当我们希望结果显示在一个单独的窗口中时，我们就使用这个命令，`figsize`参数决定运行后显示的窗口的初始大小 |
| `subplot(r, c, I)` | 它用于在同一图中创建多个图，其中`r`表示图中的行数，`c`表示图中的列数，`I`表示特定图的位置 |
| `set_xticks` | 它用于设置子图中 x 轴上标记的范围和步长 |
