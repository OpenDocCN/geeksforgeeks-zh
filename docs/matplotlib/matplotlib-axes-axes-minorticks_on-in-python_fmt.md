...
> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-minorticks_on-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-minorticks_on-in-python/)
...
`matplotlib` 库的 `Axes` 模块中的 `minorticks_on()` 函数用于显示轴上的次要刻度。
...
下面的例子说明了 `matplotlib.axes.Axes.minorticks_on()` 函数在 `matplotlib.axes` 中的作用:
