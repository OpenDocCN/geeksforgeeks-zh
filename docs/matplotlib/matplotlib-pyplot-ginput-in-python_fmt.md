# Matplotlib.pyplot.ginput() 用法详解

> 参考链接：https://www.geeksforgeeks.org/matplotlib-pyplot-ginput-in-python/
...
`matplotlib` 库的 `ginput()` 方法（属于 `pyplot` 模块）用于等待用户与图形进行交互（例如点击）。
