...
> 原文：[https://www.geeksforgeeks.org/matplotlib-axis-axis-get_gridlines-function-in-python/](https://www.geeksforgeeks.org/matplotlib-axis-axis-get_gridlines-function-in-python/)
...
`matplotlib`库的`Axis`模块中的`Axis.get_gridlines()`函数用来获取网格线作为`Line2D`实例的列表。

`语法:` `Axis.get_gridlines(self)`

`参数:` 该方法不接受任何参数。

`Return value:` This method returns the grid lines as a list of Line2D instance.

以下示例说明`matplotlib.axis.Axis.get_gridlines()`函数在`matplotlib.axis`中的用法：
