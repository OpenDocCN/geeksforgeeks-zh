# Python 中的 `matplotlib.figure.Figure.get_constrained_layout_pads()`

> 原文: [https://www.geeksforgeeks.org/matplotlib-figure-figure-get_constrained_layout_pads-in-python/](https://www.geeksforgeeks.org/matplotlib-figure-figure-get_constrained_layout_pads-in-python/)
...
## `matplotlib.figure.Figure.get_constrained_layout_pads()` 方法

`matplotlib` 库图形模块的 `get_constrained_layout_pads()` 方法用于获取 `constrained_layout` 的填充。

> **语法:** `get_constrained_layout_pads(self, relative=False)`
>
> **参数:** 此方法不接受任何参数。
>
> **返回:** 该方法返回 `w_pad`、`h_pad` (以英寸为单位) 以及 `wspace` 和 `hspace` (作为子图的一部分) 的列表。

下面的例子说明了 `matplotlib.figure.Figure.get_constrained_layout_pads()` 函数在 `matplotlib.figure` 中的作用:
