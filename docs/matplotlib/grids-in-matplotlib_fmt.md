...
> **语法:** `matplotlib.pyplot.grid(b=None, where='major', axis='both', **kwargs)`
>
> **参数:**
> `b`: bool 值指定是否显示网格线。默认值为 `True`。
> `where`: 应用更改的网格线。值: `{ 'major', 'minor', 'both' }`
> `axis`: 应用更改的轴。值: `{'both', 'x', 'y'}`
> `**kwargs`: 可选行属性
>
> **返回:** 这个函数不返回任何东西。

`grid()` 通过指定布尔值（`True`/`False`）来设置网格的可见性。我们还可以选择显示次要或主要刻度，或者两者都显示。此外，颜色、线宽和线型可以作为附加参数进行更改。让我们看一些例子:
