...
在本文中，我们将看到 matplotlib 的 `textbox` 小部件。Matplotlib 是 Python 编程语言的绘图库。在本文中，我们将尝试绘制不同幂次（如 `t^2`、`t^3`、`t^9` 等）的图表，使用 `textbox` 小部件。

文本框是一个接受用户输入的小部件。输入也可以是公式，这样我们就可以根据这些公式生成图表。要使用这个小部件，我们使用 `TextBox()` 函数。

它接受两个参数：
*   `fig`，即要使用的图表。
*   与文本框一起显示的文本，即标签 `txtBox`。
...
在上面的程序中，我们为 `t^2` 创建了一个默认图形。在标记为 `txtBox` 的文本框中，我们可以输入想要查看图表的公式。