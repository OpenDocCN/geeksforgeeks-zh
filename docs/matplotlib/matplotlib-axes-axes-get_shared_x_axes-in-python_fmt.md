...
> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-get_shared_x_axes-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-get_shared_x_axes-in-python/)
...
`matplotlib`库的`Axes`模块中的`Axes.get_shared_x_axes()`函数用于返回 x 轴的共享轴 Grouper 对象的引用。
...
下面的例子说明了`matplotlib.axes.Axes.get_shared_x_axes()`函数在`matplotlib.axes`中的作用：
