...
> 原文: [https://www.geeksforgeeks.org/matplotlib-axes-axes-set_title-in-python/](https://www.geeksforgeeks.org/matplotlib-axes-axes-set_title-in-python/)
...
`matplotlib` 库的 `Axes` 模块中的 `Axes.set_title()` 函数用于设置轴的标题。
...
> *   `loc`: 此参数用于设置标题的位置 {`'center'`, `'left'`, `'right'`}。
