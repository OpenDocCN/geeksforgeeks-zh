...
> 原文：[https://www.geeksforgeeks.org/different-plotting-using-pandas-and-matplotlib/](https://www.geeksforgeeks.org/different-plotting-using-pandas-and-matplotlib/)

`matplotlib` 库中有不同类型的图，可以帮助我们根据您的需要制作合适的图形。根据给定的数据，我们可以制作很多图表，在 `pandas` 的帮助下，我们可以在绘制数据之前创建一个数据框架。让我们用 `pandas` 来讨论 `matplotlib` 中不同类型的情节。
...
### 基本绘图
在这个基本绘图中，我们可以使用随机生成的数据，使用数列和 `matplotlib` 绘制图形。
