# Python中的matplotlib.axis.Axis.have_units()函数

> 原文：[https://www.geeksforgeeks.org/matplotlib-axis-axis-have_units-function-in-python/](https://www.geeksforgeeks.org/matplotlib-axis-axis-have_units-function-in-python/)

`Matplotlib`是Python中的一个库，是`NumPy`库的数值-数学扩展。这是一个神奇的Python可视化库，用于2D数组图，并用于处理更广泛的`SciPy`堆栈。

## matplotlib.axis.Axis.have_units()函数

`matplotlib`库的轴模块中的`Axis.have_units()`函数用于检查单位是否设置在任意轴上。
...
下面的例子说明了`matplotlib.axis.Axis.have_units()`函数在`matplotlib.axis`中的作用。
