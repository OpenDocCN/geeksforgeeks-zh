# matplotlib.figure.get_tightbbox() 中的 Python
...
**利用 `matplotlib` 库的 `get_tightbbox()` 方法**图形模块获取以英寸为单位的图形的(紧)包围盒。

> **语法:** `get_tightbbox(self, renderer, bbox_extra_artists=None)`
>
> **参数:** 该方法接受以下参数。
>
> *   **`renderer`:** 该参数是将用于绘制图形的渲染器渲染实例渲染器。
> *   **`bbox_extra_artists`:** 此参数是要包含在紧密包围盒中的艺术家列表。
>
> **返回:** 该方法返回包含包围盒的 **`BboxBase`** 。

下面的例子说明了 `matplotlib.figure.Figure.get_tightbbox()` 函数在 `matplotlib.figure` 中的作用:
