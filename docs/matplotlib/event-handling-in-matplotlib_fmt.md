...
> `figure.canvas.mpl_connect( Event_name，回调函数或方法)`
...
| `draw_event` | `DrawEvent` | 出现画布绘制 |
| `key_press_event` | `KeyEvent` | 一个键被按下 |
| `key_release_event` | `KeyEvent` | 钥匙被释放 |
| `motion_notify_event` | `MouseEvent` | 鼠标的运动 |
| `pick_event` | `PickEvent` | 画布中的对象被选中 |
| `resize_event` | `ResizeEvent` | 图形画布会调整大小 |
| `scroll_event` | `MouseEvent` | 鼠标的滚轮滚动 |
| `figure_enter_event` | `LocationEvent` | 鼠标进入一个图形 |
| `axes_enter_event` | `LocationEvent` | 鼠标进入轴对象 |
| `axes_leave_event` | `LocationEvent` | 鼠标离开一个轴对象 |
| `figure_leave_event` | `LocationEvent` | 老鼠留下了一个身影 |

**注意**: 类是在 `matplotlib.backend_bases` 中定义的

## 鼠标事件

*   **`button_press_event`:** 该事件涉及鼠标按钮按压
