...
> **语法:** `matplotlib.pyplot.findobj(o=None, match=None, include_self=True)`
>
> **参数:**
>
> 1.  **match:** 此参数用于创建过滤器，以匹配搜索到的艺术家对象。这可以是三件事之一；
>     *   **None:** 这将返回艺术家中的所有对象。
>     *   **一个函数:** 一个带有 `def match(artist: Artist) -> bool` 签名的函数。此函数的结果包含函数返回真的艺术家。
>     *   **一个类实例:** 这个结果包含同一个类的艺术家或它的一个子类(`isinstance` 检查)，例如，`Line2D`。
