# matplotlib.artist.Artist.set_rasterized() 方法

> 原文: [https://www.geeksforgeeks.org/matplotlib-artist-artist-set_rasterized-in-python/](https://www.geeksforgeeks.org/matplotlib-artist-artist-set_rasterized-in-python/)
...
`matplotlib`库的`artist`模块中的`set_rasterized()`方法用于在矢量后端输出中强制栅格化(位图)绘制。

### 语法
`artist.set_rasterized(self, rasterized)`
...
以下示例说明了`matplotlib`中的`matplotlib.artist.Artist.set_rasterized()`函数:
