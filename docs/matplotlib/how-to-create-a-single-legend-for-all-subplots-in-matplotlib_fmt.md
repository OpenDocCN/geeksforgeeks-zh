# 如何在 Matplotlib 中为所有子图创建一个单独的图例？

> 原文：[https://www.geeksforgeeks.org/how-to-create-a-single-legend-for-all-subplots-in-matplotlib/](https://www.geeksforgeeks.org/how-to-create-a-single-legend-for-all-subplots-in-matplotlib/)

`matplotlib`中的`subplot()`函数有助于在单个图形中创建一个子图网格。在图中，子图从左上角开始逐行创建和排序。`Matplotlib`库中的一个图例基本上描述了图形元素。`legend()`可以通过将其放置在各种位置来定制和调整图形内部或外部的任何位置。有时需要为所有子图创建一个单独的图例。以下是显示所有子图的单一图例的示例。
...
例如，`subplot(2, 1, 1)`是用2行1列表示第一个子图的图形，第一个子图位于第一行。
...
如果`legend()`命令中的参数长度（即`args`）为0，则它会通过调用`get_legend_handles_labels()`方法，从标签属性中自动生成图例。
