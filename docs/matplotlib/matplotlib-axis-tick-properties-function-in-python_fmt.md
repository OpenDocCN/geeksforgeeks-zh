# Python中的matplotlib.axis.tick.properties()函数
...
`matplotlib`库的轴模块中的 `Tick.properties()`函数用来获取艺术家所有属性的字典。
...
以下示例说明 `matplotlib.axis.tick.properties()`函数在 `matplotlib.axis` 中的使用：
