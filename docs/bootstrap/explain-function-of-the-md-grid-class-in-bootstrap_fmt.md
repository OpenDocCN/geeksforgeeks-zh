# 解释 Bootstrap 的 md-grid 类的功能

> 原文: [https://www.geeksforgeeks.org/explain-function-of-the-md-grid-class-in-bootstrap/](https://www.geeksforgeeks.org/explain-function-of-the-md-grid-class-in-bootstrap/)
...
*   **行:** 行必须放置在**容器**或**容器-流体**内，以便正确对齐和填充。行用于创建水平的列组。
*   **列:** 网格列是通过指定您希望跨越的十二个可用列的数量来创建的。例如，三个相等的列将使用三个 `col-lg-4`。
...
在此示例中，`col-md-6` 表示中型屏幕设备的整个页面宽度的 50%，因为它将从 12 个网格列中获取 6 个网格列。
...
在此示例中，`col-md-3` 表示整个页面宽度的 25%，因为它将从 12 个网格列中获取 3 个网格列。而 `col-md-9` 表示整个页面宽度的 75%，因为它将获得中等尺寸屏幕设备的 12 个网格列中的 9 个网格列。
...
输出页面显示分别在 `col-md-3` 和 `col-md-9` 的帮助下获得的 25%和 75%的宽度。