...
`.nav`、`.nav-item` 和 `.nav-link` 类用于创建导航菜单。`.nav`、`.nav-item` 和 `.nav-link` 类分别与 `<ul>`、`<li>` 和链接元素一起使用。
...
`.justify-content-center` 和 `.justify-content-end` 类用于将导航菜单的对齐方式分别设置为居中和结束。
...
`.nav-tabs` 类用于创建标签导航菜单。`.active` 类用于激活链接。
...
**带下拉导航菜单的药丸/标签:**
`nav-pills`/`nav-tabs`类用于创建导航标签/标签菜单和`dropdown`类用于创建下拉导航菜单。
...
**可移动/动态药丸导航菜单:**
`data-toggle="pill"`属性用于使药丸可移动。然后加上一个`tab-pane`类，每个标签都有一个唯一的标识，并将它们包装在带有类`tab-content`的`<div>`元素中。
...
`data-toggle="pill"`属性用于使选项卡可切换。然后加上一个`tab-pane`类，每个标签都有一个唯一的标识，并将它们包装在带有类`tab-content`的`<div>`元素中。
