...
**水平方向:**
`.flex-row` 类用于水平显示 flex 项目。默认情况下，flexbox 水平放置。`.flex-row-reverse` 类用于反转 flexbox 的对齐方式(右对齐 flexbox)。
...
**垂直方向:**
`.flex-column` 类用于垂直显示 flexbox(相互堆叠)。`.flex-column-reverse` 类用于反向垂直放置 flexbox。
...
**内容对齐:**
`.justify-content-*` 类用于更改 flexbox 的对齐方式。flexbox 的对齐方式可以是起点、终点、中心、中间和周围。
...
`.flex-grow-1` 类在 flexbox 上使用，占据剩余空间。
...
`.order` 类用于更改 flexbox 的订单。有效的订单类别是从 0 到 12。
...
`.mr-auto` 和 `.ml-auto` 类用于为项目添加边距。`.mr-auto` 类将项目推向右侧并 `.ml-auto` 类向左推送项目。
...
**包装:** `.flex-nowrap` (默认)，`.flex-wrap` 或 `.flex-wrap-reverse` 类用于控制 flex 项目的包装。
...
**对齐内容:** `.align-content-*` 类用于设置伸缩项的垂直对齐方式。所有班级的名单是：`.align-content-start` (默认)、`.align-content-end`、`.align-content-center`、`.align-content-between`、`.align-content-around` 和 `.align-content-stretch`。
...
**对齐项目:** `.align-items-**` 类用于控制伸缩项的垂直对齐。所有班级的名单是: `.align-items-start`、`.align-items-end`、`.align-items-center`、`.align-items-baseline` 和 `.align-items-stretch` (默认)。
...
**调整自我:** `.align-self-**` 类用于控制伸缩项的垂直对齐。所有班级的名单是: `.align-self-start`、`.align-self-end`、`.align-self-center`、`.align-self-baseline` 和 `.align-self-stretch` (默认)。
