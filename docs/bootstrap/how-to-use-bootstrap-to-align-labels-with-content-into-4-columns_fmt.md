...
> 原文：[https://www.geeksforgeeks.org/how-to-use-bootstrap-to-align-labels-with-content-into-4-columns/](https://www.geeksforgeeks.org/how-to-use-bootstrap-to-align-labels-with-content-into-4-columns/)

本文的目的是将内容排列成四列，其中前两列表示标签及其内容，后两列表示标签及其内容。`row`类和`col`类用于创建一个网格，该网格可以由许多行和列来表示。`row`类创建一行，内容将被放入该行中，`col`类在该行中创建分区，为要放在该行中的 4 个标签或 4 个项目腾出空间。
