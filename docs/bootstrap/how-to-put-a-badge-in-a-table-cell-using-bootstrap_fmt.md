...
响应徽章包含在最新的引导版本中。徽章为任何内容添加了额外的信息，如计数或标签。徽章的样式为其目的提供了明显的暗示。这些徽章可以出现在句子、链接或按钮的顶端。下面的示例演示了引导徽章。
...
只需将包含徽章的分区包含在一个表格单元格中，徽章就可以放置在表格单元格中。这可以用来突出显示表格中的一些文本。以下示例演示了徽章在表格单元格中的使用。
