...
*   **基本卡:** 一张卡的基本积木是`card-body`，以`card`类为父类，我们可以创建一张基本卡。
    **例:**
...
*   **页眉和页脚:** `card-header`为卡片提供页眉，`card-footer`为卡片提供页脚，如下所示。
    **例:**
...
*   **标题和链接:** `card-title`用于给卡片一个标题，`card-link`用于在卡片需要时提供卡片链接。
    **例:**
...
*   **图片:** 卡片上的图片插入`card-img-top` 和`card-img-bottom`在这两者的帮助下，使用`img`标签来添加图片。
    **示例:**
...
*   **图像叠加:**在这种情况下，我们必须在您用于卡片图像的同一行代码中使用一个名为`card-img-overlay`的额外类。这会将图像变成卡片背景，并覆盖卡片的文本。
    **例:**
...
### 厨房水槽
它是由包含所有东西的卡片类型命名的，它是多种内容的混合和匹配，以制作您想要的卡片
示例:
