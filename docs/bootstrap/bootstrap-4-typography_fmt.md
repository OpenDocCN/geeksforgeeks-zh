...
*   **显示标题:** 用于创建更好的标题(更大的字体大小和更轻的字体粗细)。显示标题中有四个类别。`.display-1`，`.display-2`，`.display-3` 和 `.display-4`。
*   **`.lead`:** 用于使一个段落突出，即视觉效果更好。
*   **`<mark>`:** 用于突出文本内容。
*   **`<small>`:** 用于以略小的文本大小渲染缩写。
*   **`<blockquote>`:** 用于引用文本内容。
*   **`.blockquote-footer`:** 用于添加页脚详细信息，用于标识报价来源。
*   **`.text-center`:** 用于将文本对齐居中。
*   **`.list-inline`:** 用于使列表的元素内联。
*   **`.text-truncate`:** 用于通过用省略号截断来缩短较长的文本。
*   **`.text-muted`:** 用于文本颜色，文本静音淡化文本，即文本变灰。
*   **`.text-lowercase`:** 用于将文字转换为小写。
*   **`.text-uppercase`:** 用于将文字转换为大写。
*   **`.text-capitalize`:** 用于将文本转换为每个单词的第一个字母大写，其他字母小写。
*   **`.font-weight-bold`:** 设置字体粗细为加粗。它用于显示文本的重要性。
*   **`.font-weight-bolder`:** 设置字体粗细为深加粗。它用于显示文本的重要性。
*   **`.font-italic`:** 将字体样式设置为斜体。
*   **`.font-weight-light`:** 它将字体粗细设置为轻。
*   **`.font-weight-lighter`:** 设置较轻的字体重量。
*   **`.font-weight-normal`:** 设置正常字体大小和粗细。
*   **`.lead`:** 它让段落脱颖而出。
*   **`.small`:** 用于创建二级副标题。
