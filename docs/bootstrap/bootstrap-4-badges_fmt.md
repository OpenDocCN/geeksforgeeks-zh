...
`.badge`类用于给内容添加附加信息。例如，一些网站将一些通知与链接相关联。当登录到一个特定的网站时，会看到通知号码，该网站通过单击它来告知要查看的新闻或通知的号码。
...
**上下文徽章:**上下文类(`.badge-*`)用于改变徽章的颜色。徽章可以作为链接或按钮的一部分来提供一个计数器。根据他们的使用方式，徽章一定会让用户感到困惑。为此，使用不同颜色的变体，以便用户不会混淆。
