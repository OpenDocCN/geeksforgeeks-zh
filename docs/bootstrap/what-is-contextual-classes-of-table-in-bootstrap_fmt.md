...
> 原文: [https://www.geeksforgeeks.org/what-is-contextual-classes-of-table-in-bootstrap/](https://www.geeksforgeeks.org/what-is-contextual-classes-of-table-in-bootstrap/)

Bootstrap 提供了一系列类，可用于对表格应用各种样式，如更改标题外观、剥离行、添加或删除边框、使行可悬停等。Bootstrap 还提供了使表响应的类。

**简表:** `table` 类用于创建一个简单的 Bootstrap 表。这个类名与 `<table>` 标签一起使用来创建一个表。
