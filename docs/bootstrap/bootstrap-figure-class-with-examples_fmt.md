...
当需要显示一段内容时，使用`figure`，通常是带有可选标题的图像。Bootstrap 中的`figure`类用于为默认图形元素添加样式。

*   基础`.figure`类用于表示一个图元素。
*   `.figure-img`用于指示图元素中使用的图像。
*   `.figure-caption`可用于在图形下方显示标题。
...
**示例**:使用`.figure-caption`类在图像下方显示标题。此类与`<figcaption>`标签一起使用。
