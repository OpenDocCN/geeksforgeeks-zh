...
> 原文：[https://www.geeksforgeeks.org/containers-in-bootstrap-with-examples/](https://www.geeksforgeeks.org/containers-in-bootstrap-with-examples/)

在引导程序中，**容器**用于设置内容的边距。它包含行元素，行元素是列的容器。这就是所谓的网格系统。Bootstrap 中有两个容器类：
...
*   `.container`：`.container` 类提供了一个响应性的固定宽度容器。在下面的例子中，类为 `container` 的 `div` 将有一个固定的左右边距，并且不会占据其父级或视口的全部宽度。
...
*   `.container-fluid`：`.container-fluid` 类提供了一个全宽的容器，它跨越了视口的整个宽度。在下面的例子中，类别为 `container-fluid` 的 `div` 将占据视口的整个宽度，并且在视口调整大小时将会扩展或收缩。
