...
**基本导航栏：** `.navbar` 类用于创建导航栏。通过使用响应式创建 `navbar`。`.navbar-expand-xl|lg|md|sm` 类。响应式 `navbar` 垂直堆叠在小屏幕中。`<nav class="nav-item">` 元素后跟 `<a class="nav-link">` 用于创建 `nav link`。
...
**中央导航条：** `.justify-content-center` 类用于将导航栏的对齐方式设置为居中。导航栏在中、大、超大屏幕上居中显示。在小屏幕的情况下，导航栏垂直显示并左对齐。
...
**彩色导航条：** `.bg-color` 类用于更改导航栏的背景颜色。`.bg-color` 类是：`.bg-primary`，`.bg-success`，`.bg-info`，`.bg-warning`，`.bg-danger`，`.bg-secondary`，`.bg-dark` 和 `.bg-light`。`.navbar-dark` 类用于将链接文本颜色设置为白色和 `.navbar-light` 类用于将链接文本颜色设置为黑色。
    ...
    <div class="container">
        ...
        <nav class="navbar navbar-expand-sm bg-primary navbar-dark">
            ...
            <ul class="navbar-nav">
                ...
                <li class="nav-item"><a class="nav-link" href="#">Algo</a></li>
                <li class="nav-item"><a class="nav-link" href="#">DS</a></li>
...
折叠导航条用于小屏幕。它隐藏了导航链接，并替换为一个按钮。`class="navbar-toggler"`、`data-toggle="collapse"`和`data-target="#thetarget"`用于创建折叠导航栏。
            ...
            <div class="collapse navbar-collapse" id="collapse_Navbar">
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" id="navbardrop" data-toggle="dropdown">Languages</a>
...
`<form class="form-inline">`元素用于并排分组输入和按钮。`.input-group-prepend`或`.input-group-append`类用于附加输入文本字段的图标。
...
`.fixed-top` 类用于将导航栏固定在顶部位置，`.fixed-bottom` 类用于将导航栏固定在底部位置。
...
**粘性导航条:**
`sticky-top`类用于创建粘性导航栏。滚动页面时，粘性导航栏固定在页面顶部。
