...
> 原文: [https://www.geeksforgeeks.org/how-to-make-bootstrap-popover-appear-disappear-on-hover-instead-of-click/](https://www.geeksforgeeks.org/how-to-make-bootstrap-popover-appear-disappear-on-hover-instead-of-click/)
...
## 方法 1
这里，我们将只使用 jQuery 初始化在‘悬停’中指定 popover 触发器。我们可以在 `popover()` 函数本身中添加标题细节和触发标准作为参数。
...
## 方法 2
这里我们将使用`数据-触发器`属性。我们也可以通过将`【头衔】`和`【数据触发】`作为属性来实现，而无需在 `popover()` 方法中将它们指定为参数。
