...
> 原文: [https://www.geeksforgeeks.org/how-to-close-sidebar-by-default-using-bootstrap/](https://www.geeksforgeeks.org/how-to-close-sidebar-by-default-using-bootstrap/)
...
网页包含一个带 `id=main` 的分部，这个分部包含网页的实际内容。一个标记为 **“菜单”** 的按钮被创建，它触发了 `openNav()` 功能。功能 `openNav()` 使用 `document.getElementById()` 获取侧边栏分区，并设置侧边栏的宽度和 **“main”** 分区。工具条显示后，用户点击工具条右上角的关闭按钮，触发 `closeNav()` 功能。功能 `closeNav()` 使用 `document.getElementById()` 获取主分区，将侧边栏宽度设置为 0，对网页隐藏，主分区左边距设置为 0，使其占据整个屏幕宽度。因为这两个函数都是 javascript 函数，所以它们被放在脚本标签中。样式标签中指定的 CSS 支持边栏和主要部分的正确定位和转换。
...
该方法使用具有类型“复选框”和角色“按钮”的输入字段的切换功能来显示侧栏。当用户点击按钮时，复选框被选中，它将“内容”部分的左边距设置为 0，这意味着它占据了整个屏幕。当用户再次点击按钮时，复选框被取消选中，并且“内容”部分的左边距被设置为 200px，侧边栏被显示。这种转换发生在 CSS 中，不像前面的例子，整个操作是由 javascript 函数执行的。
