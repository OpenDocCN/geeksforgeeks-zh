...
*   `d-flex` 和 `d-inline-flex` 可以用于所有断点（sm、md、lg、xl）等。`d-sm-flex`，`d-sm-inline-flex` 等。
    **语法：**
...
*   使用 `flex-column-reverse` 从对面开始垂直方向。
    **语法：**
...
要改变伸缩项的对齐方式，可以使用 `justify-content-*` 类。
*可以是其中任意一个* `start`（默认）、`end`、`center`、`between` 或 `around`。
类似的 `justify-content` 可以用于所有断点（sm、md、lg、xl）等。`justify-content-sm-start`，`justify-content-sm-end` 等。
...
使用`.flex-fill`可以将伸缩项强制成相等的宽度。`.flex-fill`类可以用于所有断点(`sm`、`md`、`lg`、`xl`)。`.flex-sm-fill`等。
...
`.order`类用于更改弹性项的可视化顺序。订购数量从 `0` 到 `12`。`0` 优先级最高。`.order`可以用于所有断点(`sm`、`md`、`lg`、`xl`)等。`.order-sm-0`至`.order-sm-12`等。
...
`.flex-nowrap`(默认)，`.flex-wrap`和`.flex-wrap-reverse`用于将柔性物品包装在柔性容器中。`.flex-wrap`可以用于所有断点(`sm`、`md`、`lg`、`xl`)。`.flex-sm-nowrap`，`.flex-wrap`等。
...
基本上，它用于伸缩项的垂直对齐。`align-content`可以通过多种方式实现。
...
使用`align-items`类来更改伸缩项目在横轴上的对齐。`align-items`可以通过多种方式实现。
