...
当我们必须引用网页上的一些内容时，我们可以使用面板。我们在内容收件箱周围放置一些填充。Bootstrap 面板用 `panel` 类表示。
...
在 Bootstrap 面板中有一些部分，如 [Bootstrap 卡片](https://www.geeksforgeeks.org/bootstrap-cards/)。Bootstrap 面板的所有主体部分描述如下:

*   `panel-body` 用于定义面板的主体。
*   `panel-heading`: 用于给面板赋予标题。
*   `panel-footer`: 用于给面板赋予页脚类。
*   `panel-group`: 用于将不同的面板集合在一起成为一个组。
