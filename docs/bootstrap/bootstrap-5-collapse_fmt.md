...
Bootstrap 5 是 Bootstrap 的最新主要版本，在该版本中，他们修改了用户界面并进行各种更改。“折叠”用于切换项目中内容的可见性，包括几个类和 Bootstrap 5 附带的 JavaScript 插件。折叠组件的工作基于应用的类更改。例如，类 `collapse` 隐藏内容。在过渡期间应用类 `collapsing`，而类 `show` 显示内容。
...
该示例使用 Bootstrap 5 中的按钮和链接显示可折叠 div 的工作方式。
...
此示例用于显示 Bootstrap 5 中可折叠卡片的工作方式。
...
此示例用于显示 Bootstrap 5 中多个可折叠凹坑的工作情况。
