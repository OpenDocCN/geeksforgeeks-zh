...
该示例用于显示 Bootstrap 5 中一个模态的工作情况。
...
我们可以通过嵌套在一个模态框中使用 Bootstrap 网格系统。在 `.modal-body` 内使用 `.container-fluid`，然后像在其他地方一样使用普通的网格系统类。
...
我们可以用下面给出的不同数据触发相同的模态。

示例：

## 可选尺寸

引导模式的默认最大宽度为 500 像素。Bootstrap 提供了通过使用如下所述的特定类来自定义模式大小的选项：
...
输出为点击`发送邮件至@geeksforgeeks`：
![](img/4947daf42b264e85c0fe93dd45fea3ae.png)

输出为点击`发送邮件至作者@gurrrung`：
![](img/75f7a549e4e3904e6daa6926d78a7c9f.png)
