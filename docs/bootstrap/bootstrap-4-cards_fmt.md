...
Bootstrap 卡片是一个灵活的盒子，包含一些内容周围的填充。它包括页眉和页脚选项、颜色、内容和强大的显示选项。它取代了面板、井和缩略图的使用。它可以在一个名为 `card` 的容器中使用。

![](img/1208b2efdf08b16fac156c39e768dce3.png)

## 基础卡

`.card` 和 `.card-body` 类用于创建基础卡。`.card-body` 类用于 `.card` 类内部。
...
`.card-header` 类为卡片提供标题。`.card-footer` 类为卡片提供页脚。
...
`.card-title` 类用于设置卡片的标题。`.card-link` 类用于在需要时设置卡片的链接。
...
可以通过使用卡片的颜色来设置卡片样式，以便用户可以容易地理解特定卡片代表什么。它由警报中使用的颜色组成。
...
`.card-img-top` 或 `.card-img-bottom` 类用于将图像放置在卡片内部的顶部或底部。
...
<body>
    ...
    <div class="container">
        <div class="card" style="width: 18rem;">
            <img class="card-img-bottom" src="https://media.geeksforgeeks.org/wp-content/uploads/20190506125816/avt.png" alt="Card image cap">
...
`.card-deck` 类用于创建等高等宽的卡片网格，去除卡片之间的左右边距。
...
`.card-group` 类用于创建等高等宽的卡片网格，去除卡片之间的左右边距。
...
`.list-group` 和 `.list-group-flush` 类用于创建卡片中的内容列表。
...
它是由里面的所有东西组成的卡片类型的名称。它混合并匹配多种内容来制作所需的卡片。
...
<body>
    ...
    <div class="container">
        <div class="card" style="width:12rem;">
            <img class="card-img-top" src="https://media.geeksforgeeks.org/wp-content/uploads/20190506125816/avt.png">
