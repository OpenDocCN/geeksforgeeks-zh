...
> 原文：[https://www.geeksforgeeks.org/how-do-we-create-large-button-groups-in-bootstrap/](https://www.geeksforgeeks.org/how-do-we-create-large-button-groups-in-bootstrap/)
...
按钮组是 Bootstrap 5 提供的一个组件，基本上可以让你制作一组串联的按钮。按钮组也支持所有类型的按钮类。

## 类型

以下是 Bootstrap 5 中提供的 9 种按钮类型：
...
*   在每个 `div` 类中，添加一个 HTML `<button>` 元素，这些元素想要用预定义的引导按钮类名显示在屏幕上。

### 示例 1

为了创建按钮组，需要在容器标签中定义一个 `btn-group` 类，如 `<div>`。
...
我们还通过将类 `btn-group-*` 包括在内来创建相同大小的按钮组。`btn-group` 父类，而不是在每个按钮中包含大小调整类。
...
此示例演示了在 Bootstrap 中使用不同的按钮组大小调整组。
...
要使用 bootstrap 组件类，我们需要下载 Bootstrap 包或者我们可以直接使用 Bootstrap CDN 链接，从[主页面](https://getbootstrap.com/docs/5.0/getting-started/download/)，在 `<head>` 标签里面，如下所示。

> `<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">`

### 步骤 2

将类声明为容器，值为 `btn-group-lg` 在外 `<div>`。
...
现在，给内部 HTML `<button>` 标签添加不同的预定义按钮类。比如 `btn btn-success` 会将按钮的样式设置为 bootstrap success 类，也就是绿色按钮，像这样，你也可以使用不同的类。
...
`btn-group-lg` 类连同父类 `btn-group` 类用于创建水平排列的大按钮组。

### 示例 3

在本例中，我们在 Bootstrap 中创建了大按钮组的水平系列。
...
`btn-group-lg` 类连同父类 `btn-group-vertical` 类用于创建水平排列的大按钮组。

### 示例 4

在本例中，我们在 Bootstrap 中创建了一个垂直系列的大按钮组。
