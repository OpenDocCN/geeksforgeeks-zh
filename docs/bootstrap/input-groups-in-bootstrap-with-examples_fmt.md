...
> 原文: [https://www.geeksforgeeks.org/input-groups-in-bootstrap-with-examples/](https://www.geeksforgeeks.org/input-groups-in-bootstrap-with-examples/)
...
*   `.input-group-append` 类用于将其添加到输入后面。
*   `.input-group-text` 类用于设置组内显示的文本的样式。
...
*   `.input-group-sm` 类用于较小的尺寸。
*   `.input-group-lg` 类用于更大的尺寸。
*   `.input-group` 类是默认大小。
...
多个插件可以与其他类型堆叠或混合在一起，包括复选框和单选按钮。
