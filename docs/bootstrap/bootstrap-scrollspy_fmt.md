...
1.  `data-spy`：它是一个要添加元素的属性，该元素用作可滚动区域（在上面的示例中，我们将其作为主体）。`data-spy`的值是`scroll`。
2.  `data-target`：该属性用于连接导航栏和可滚动区。取值为导航栏 ID 或类的名称。
3.  `data-offset`：该属性指定计算滚动位置时从顶部偏移的像素数。