...
> 原文: [https://www.geeksforgeeks.org/responsive-images-in-bootstrap-with-examples/](https://www.geeksforgeeks.org/responsive-images-in-bootstrap-with-examples/)
...
*   `.img-responsive` 类: Bootstrap 中的响应镜像是通过添加`.img-responsive`类到`img`标签创建的。`.img-responsive`等级适用于: `max-width: 100%` | `height: auto` | `display: block`。
...
*   `.img-fluid` 类: 添加`.img-fluid`类到标记。`.img-fluid`等级适用于: `max-width: 100%` | `height: auto`。
...
*   `.img-rounded` 类: 图像的圆角由`.img-rounded`类实现。(IE8 不支持圆角。)
...
*   `.img-circle` 类: 图像的形状由`.img-circle`类做成圆形。(IE8 不支持圆角。)
...
*   `.img-thumbnail` 类: 缩略图的形状由`.img-thumbnail`类完成。
