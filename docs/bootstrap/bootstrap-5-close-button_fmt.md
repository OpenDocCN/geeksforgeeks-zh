...
Bootstrap 5 提供了一个新的组件，它是一个关闭按钮，可以用来关闭像模式和警报这样的内容。可以使用 `bootstrap.js` 或 `bootstrap.min.js` 将其包含在网页中。该组件可以与模型、警报和弹出窗口相结合。禁用的关闭按钮有 `pointer-events: none;` 应用于，防止悬停和活动状态触发。
