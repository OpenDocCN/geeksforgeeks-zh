...
`响应映像:` 在`<img>`标签中使用`img-fluid`类来创建响应图像。响应图像用于将图像自动调整到指定的框中。
...
`圆角图像:` `rounded`类用于创建圆角图像。该类与`<img>`标签一起使用。
...
`缩略图:` `img-thumbnail`类用于创建缩略图(带边框)图像。
...
`对齐图像:` `float-left`和`float-right`类用于设置图像的左右对齐。
