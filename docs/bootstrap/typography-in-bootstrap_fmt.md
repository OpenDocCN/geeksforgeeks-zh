...
*   `display`: 用于创建更好的标题。
*   `lead`: 用于使一个段落突出，即视觉上更好。
*   `mark`: 用于突出显示文字。
*   `small`: 用于创建二级副标题。
*   `initialism`: 用于以略小的文字大小渲染缩写。
*   `blockquote`: 用于引用内容。
*   `blockquote-footer`: 是标识报价来源的页脚明细。
*   `text-center`: 用于文本居中对齐。
*   `list-inline`: 用于使列表的元素内联。
*   `text-truncate`: 用于用省略号截断来缩短较长的文本。
*   `text-lowercase`: 用于将文字转换为小写。
*   `text-uppercase`: 用于将文字转换为大写。
*   `text-capitalize`: 用于将文本转换为每个单词的第一个字母大写，其他字母小写。

## 示例 1: 说明 `text-muted` 类
...
## 例 7: 举例说明 `blockquote` 标签和 `blockquote-footer` 类
