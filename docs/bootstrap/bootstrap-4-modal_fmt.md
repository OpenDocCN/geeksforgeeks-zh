...
*   模态是用 HTML、CSS 和 JavaScript 构建的。它们位于文档中所有其他内容的上方，并从中移除滚动，以便模态内容改为滚动。
*   点击模态“背景”将自动关闭模态。
*   该模式一次只支持一个模式窗口。
*   Modals 使用 `position: fixed`，这有时会对它的呈现有点特别。如果可能的话，把你的模态 HTML 放在顶层位置，以避免其他元素的潜在干扰。
*   由于位置:固定，在移动设备上使用 modals 几乎没有什么警告。

## Bootstrap Modal Contents

### 使用网格

通过将 `<b-container fluid>` 嵌套在模态体内来利用模态内的 Bootstrap 网格系统，否则您可以像在任何其他地方一样使用普通网格系统 `<b-row>` (或 `<b-form-row>`) 和 `<b-col>`。
