...
`form-inline`类与`<form>`元素一起使用来创建内联表单。内联表单包含左对齐和内联的元素。当视口宽度至少为 576px 时，内联属性适用。如果屏幕尺寸小于 576px，则表单元素将垂直堆叠。
...
Bootstrap 间距工具类用于添加空间，以使内联表单看起来更好。`mr-sm-2`类用于在所有设备(小型及以上)和上为每个输入添加右边距。`mb-2`类在底部增加了一个边距。
...
`was-validated`或`needs-validation`类中的`<form>`元素用于在提交表单之前或之后提供验证。带有绿色边框的输入字段表示有效，红色边框表示无效的表单内容。`valid-feedback`或`invalid-feedback`用于在提交表单前向用户显示通知消息。
