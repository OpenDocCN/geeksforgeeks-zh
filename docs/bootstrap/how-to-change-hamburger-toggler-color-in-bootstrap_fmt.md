...
> 原文：[https://www.geeksforgeeks.org/how-to-change-hamburger-toggler-color-in-bootstrap/](https://www.geeksforgeeks.org/how-to-change-hamburger-toggler-color-in-bootstrap/)
...
*   `.navbar-dark`：该类用于将导航栏内容的颜色设置为浅色。它会将 toggler 图标更改为白线。

**注**：命名好像有点倒退。不应该吗？`.navbar-dark` 使内容更暗。`.navbar-light`，让它更轻？深色和浅色代表背景的颜色，而不是导航栏上的内容。
...
Bootstrap 使用一个 SVG 图像来表示 toggler。可以在图标内的线条上用修改后的颜色创建新的图像数据。图像数据中的 `stroke` 属性用于表示 RGB 值中的颜色。该值被修改为所需的颜色。

该新图标用于 `.navbar-toggler-icon` 类具有 `background-image`，因此内置图标被这个新的 toggler 图标替换。
...
通过指定具有所需颜色的 `border-color` 属性来设置网格线的边框颜色。
