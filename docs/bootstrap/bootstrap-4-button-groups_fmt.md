...
Bootstrap 提供的类允许沿着同一条线，水平或垂直分组按钮。要分组的按钮嵌套在类为`.btn-group`的`<div>`元素中。
...
`.btn-group-vertical`类在父`div`中用于创建垂直按钮组。
...
通过将类别`btn-group-*`（可以是`sm`、`md`或`lg`）包含在`.btn-group`父元素中，可以给整个按钮组相同的尺寸，而不是在每个按钮中包含尺寸类。
...
但是，当按钮嵌套在`.btn-group-vertical`类下时，拆分按钮下拉菜单不会像预期的那样工作。
