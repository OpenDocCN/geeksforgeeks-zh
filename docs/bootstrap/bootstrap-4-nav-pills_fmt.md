...
- 向导航标签或导航元素添加类`nav-fill`或`nav-justified`。
- `nav-fill`和`nav-justified`的区别在于类`nav-fill`根据名称长度为 Nav Pill 项目提供了不相等的空间。但是`nav-justified`使 Nav Pill 空间彼此相等。

### 示例 1

下面的示例说明了如何使用 Bootstrap 4 使用类`nav-fill`或`nav-justified`来调整导航药丸。
...
**参考:** [https://getbootstrap.com/docs/4.0/components/navs/](https://getbootstrap.com/docs/4.0/components/navs/)