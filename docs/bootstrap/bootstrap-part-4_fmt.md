...
首先，我们使用`<form>`标签初始化表单。我们将把所有标签和表单控件包装在`<div>`标签中。这对于模板组件之间的最佳间距是必需的。
现在，我们可以根据需要在这些标签中设计表单。我们可以使用`<label>`标签来定义输入元素的标签。记住`<label>`的`for`属性应该等于相关元素的`id`属性才能将它们绑定在一起。在标签标签之后，可以输入`<input>`标签，该标签指定了用户可以输入数据的输入字段。要在输入标记中使用的类必须是`class="form-control"`。还可以使用`placeholder`属性，该属性指定描述输入字段预期值的简短提示。
...
水平形式与垂直形式的区别不仅在于标记的数量，还在于形式的呈现方式。要使表单水平，请在`<form>`元素中添加`class="form-horizontal"`。如果您正在使用`<label>`元素，那么您必须使用`class="control-label"`。另外，请记住，您可以使用 Bootstrap 的预定义网格类在水平布局中对齐标签和表单控件组。

您还可以向标签和输入字段添加特定的样式。添加任何`has-warning`、`has-success`、`has-error`等的类。在`<div>`标签中包含一个类，该类有一个`form-group`，当被选中时可以给它不同的效果。
...
你也可以在表单里面添加图案。为此，您必须添加`class="has-feedback"`并在`<input>`标记后添加`span`标记，在该标记中出现了字形。记住添加`form-control-feedback`作为`span`标记的类。
