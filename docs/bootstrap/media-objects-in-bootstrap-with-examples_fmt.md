...
*   要将介质与顶部、中部或底部对齐，请使用以下类别`media-top`、`media-middle`或`media-bottom`类别
...
Use the `media-top` class to top-align a media object. Use the `media-middle` class to middle-align a media object. Use the `media-bottom` class to bottom-align a media object.

## 嵌套格式的媒体对象

如果您想在第二个媒体下放置一个媒体，您可以使用媒体对象中的嵌套。嵌套的媒体可以放在父媒体对象的 `media-body` 类中。
