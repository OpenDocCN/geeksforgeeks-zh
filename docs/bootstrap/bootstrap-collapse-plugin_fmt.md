...
Bootstrap 提供一些 JavaScript 函数作为属性，折叠 JavaScript 插件附带一个名为 `data-**` 的属性。

要切换任何数据：

*   在切换按钮中`data-toggle`属性被设置为`collapse`。
*   `data-target`被设置为您想要切换的 div 的类或 id。
*   将 div 类设置为`data-toggle=collapse`您想要切换的，以便数据最初不可见。
...
在手风琴中，您可能需要另一个名为`data-parent`的属性，该属性确保如果显示其中一个项目，给定父项下的所有可折叠元素都将关闭。在这种情况下，将`data-parent`属性设置为容器 `div` 的 `id` 或`类`。
