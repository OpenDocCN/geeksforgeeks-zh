...
> 原文：[https://www.geeksforgeeks.org/what-are-button-groups-in-bootstrap/](https://www.geeksforgeeks.org/what-are-button-groups-in-bootstrap/)
...
Bootstrap 提供 4 种按钮大小，您可以通过在`btn-group`类中添加一个额外的`btn-group-*`类来直接添加到按钮中。不需要为每个按钮添加单独的类。
...
Bootstrap 还支持垂直堆叠而非水平堆叠的垂直按钮组。使用类别`btn-group-vertical`创建垂直按钮组：
...
Bootstrap 允许您通过嵌套在按钮下创建下拉菜单。在主`btn-group`类中添加一个`btn-group`类，以便在按钮中有一个下拉菜单：
