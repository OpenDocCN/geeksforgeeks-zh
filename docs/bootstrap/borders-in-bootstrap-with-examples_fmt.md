...
**边框:** 边框通常用于显示框或表格单元格或任何其他 HTML 元素周围的轮廓。在 Bootstrap 中，有不同的类可用于添加或删除边框。用于添加边框的类称为**加法类**，用于移除边框的类称为**减法类**。

**附加边框类**:

*   `.border`: 这个类在元素周围添加一个边框。
*   `.border-top`: 这个类在元素的上边缘添加一个边框。
*   `.border-left`: 这个类在元素的左边缘添加一个边框。
*   `.border-right`: 这个类在元素的右边添加一个边框。
*   `.border-bottom`: 这个类在元素的底部添加一个边框。

**减法边界类**:

*   `.border-0`: 如果元素存在，这个类从元素周围移除边框。
*   `.border-top-0`: 如果存在，这个类从元素的顶端移除边框。
*   `.border-left-0`: 如果存在，这个类从元素的左边缘移除边框。
*   `.border-right-0`: 如果存在，这个类从元素的右边移除边框。
*   `.border-bottom-0`: 如果存在，这个类从元素底部移除边框。
...
任何颜色都可以通过使用以下**边框颜色类**添加到边框中，这些颜色在 Bootstrap 中可用。如果您想要任何其他的裁剪颜色，那么您可以使用 CSS 属性手动设置它。
