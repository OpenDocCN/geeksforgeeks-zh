...
该示例使用 Bootstrap 5 中的按钮显示下拉菜单的工作方式。
...
`.dropdown-divider` 类用于通过使用细水平线来划分下拉菜单列表。此示例显示了 Bootstrap 5 中可折叠卡的工作方式。
...
**下拉位置:** `dropright` 和 `dropleft` 类用于设置下拉列表在左右两侧的位置。
...
**下降:** 使用 `dropup` 类代替 `dropdown` 类以向上展开菜单列表。
...
`.dropdown-item-text` 类用于在下拉菜单列表中添加纯文本。
...
`.btn-group` 类用于创建一组按钮，`.dropdown-menu` 类用于创建下拉列表。
...
`.dropdown-toggle-split` 类用于拆分下拉按钮。
...
以下是关于 Bootstrap 中垂直按钮组下拉列表的文档。

### 垂直按钮组下拉列表

`.btn-group-vertical` 类用于创建带有下拉列表的垂直按钮组。
