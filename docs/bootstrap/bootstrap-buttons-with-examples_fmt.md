...
> 原文：[`https://www.geeksforgeeks.org/bootstrap-buttons-with-examples/`](https://www.geeksforgeeks.org/bootstrap-buttons-with-examples/)

Bootstrap 为我们提供了不同的类，可以使用不同的标签，如 `<按钮>`、`< a >`、`<输入>` 和 `<标签>` 来应用自定义按钮样式。Bootstrap 还提供了可用于更改按钮状态和大小的类，还提供了应用切换、复选框和单选按钮等效果的类。

**实心按钮**：Bootstrap 为我们提供了一系列对应不同实心按钮样式的类。这些类如下所列：

<figure class="table">

| `BTN-light` | `BTN-小学` | `BTN 中学` | `BTN-成功` |
| `BTN-危险` | `BTN 警告` | `BTN 信息` | `BTN-暗` | `BTN` |
...
Bootstrap为我们提供了“激活”和“禁用”类来改变按钮的状态。

### 激活

`active`类用来让按钮和链接在激活状态下出现，即有深色背景，深色边框，有内嵌阴影。
...
*   **注意:**当使用`<a>`标签时，我们应该在标签内插入`role="button"`属性值对，如果按钮是活动的，那么我们在`<a>`标签内使用`aria-pressed="true"`属性值对，并将`class`值设置为`active`(`class="active"`)。
...
`disabled`类用于使按钮和链接在禁用状态下出现，即背景颜色较浅，外观较亮。
...
*   **注意:**当使用`<a>`标记时，我们应该在标记中插入`role="button"`属性值对。当使用带有`<a>`标签的`disabled`类时，我们应该在`<a>`标签中插入`aria-disabled="true"`和`tabindex="-1"`属性值对。此外，我们可以通过在`<button>`标签中添加`disabled`属性来禁用`<button>`，而无需使用Bootstrap的`disabled`类。
...
我们可以通过在`<button>`标签中添加`data-Toggle="button"`属性值对来使按钮切换其状态。我们还可以通过向`<button>`标签添加`active`类和`aria-prested="true"`属性值对来将按钮的切换状态预设为活动。
...
要获得复选框样式按钮，我们需要使用`<input>`标签，其`type`属性值为`"checkbox"`，该标签由`class`值设置为`"btn"`的`<label>`标签和上述实体或轮廓按钮类中的一个类包围。`<label>`标签依次被`<div>`标签包围，`class`值设置为`"btn-group-toggle"` (`class="btn-group-toggle"`)，在`<div>`标签内还需要`data-toggle="buttons"`属性对。以下示例将清除程序:
...
注意: 要使按钮默认勾选，我们需要在输入控件的`<label>`标签内添加`"active"`类，同时在`<input>`标签内放置`"checked"`属性。
输出:
...
要获得单选按钮，我们需要像往常一样使用`<input>`标签，其`type`属性值为`"radio"`，并设置其他必要的属性值组合来操作单选按钮。`<input>`标签又被`<label>`标签包围，该标签的`class`值设置为`"btn"`，并且是上述实体或轮廓按钮类中的一个类。`<label>`标签依次被`<div>`标签包围，`class`值设置为`"btn-group btn-group-toggle"` (`class="btn-group btn-group-toggle"`)，在`<div>`标签内还需要`data-toggle="buttons"`属性对。以下示例将清除程序:
