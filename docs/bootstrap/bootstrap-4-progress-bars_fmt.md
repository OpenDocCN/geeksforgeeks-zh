...
使用 CSS 属性更改进度条高度。默认进度高度为 `16px`。`progress` 和 `progress-bar` 容器的高度必须相同。

### 语法

```html
<div class="progress" style="height:30px;">
    <div class="progress-bar" style="width:x%;height:30px;"></div>
<div>
```
...
`progress-bar-striped` 类用于给进度条添加条纹。使用 `progress` 和 `progress-bar-striped` 类创建条纹进度条。使用 Bootstrap 4 上下文背景类来设置进度条的颜色。
...
<body style="text-align:center;">
    ...
    <div class="container">
        ...
        <div class="progress">
            <div class="progress-bar bg-danger progress-bar-striped" style="width:10%;">10%</div>
        </div><br>
        <div class="progress">
            <div class="progress-bar bg-info progress-bar-striped" style="width:70%;">70%</div>
        </div><br>
    </div>
</body>
</html>
...
`progress-bar-animated` 类用于创建动画进度条。使用 `progress`、`progress-bar-striped` 和 `progress-bar-animated` 创建一个动画进度条。
