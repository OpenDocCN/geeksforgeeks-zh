...
> 原文: [https://www.geeksforgeeks.org/how-to-design-bootstrap-fullscreen-select-feature-for-mobiles/](https://www.geeksforgeeks.org/how-to-design-bootstrap-fullscreen-select-feature-for-mobiles/)

在本文中，我们将学习如何为移动设备设计`bootstrap-fullscreen-select`功能。

`bootstrap-fullscreen-select`插件的关键功能如下。
...
以下代码是`bootstrap-fullscreen-select`插件的基本示例。在接下来的示例中，我们将在这个基本代码中设置属性。
...
在下面的示例代码中，我们将添加 Bootstrap 多选功能，并将禁用第二个下拉组。在代码的 JavaScript 部分，我们将学习添加`animation`和`padding`选项。各种其他选项也是可用的，开发者可以根据自己的需要进行尝试。
...
下面的代码演示了插件的`onOpen`、`onOpen`和`onClose`事件触发器。打开动画开始时，`onOpen`被触发，关闭动画执行后，`onClose`被触发。
