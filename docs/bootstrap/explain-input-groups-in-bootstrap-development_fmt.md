...
> 原文：[https://www.geeksforgeeks.org/explain-input-groups-in-bootstrap-development/](https://www.geeksforgeeks.org/explain-input-groups-in-bootstrap-development/)
...
将输入组元素添加到表单控件的基本步骤如下。

*   将子组件包裹在`.input-group`类内。
*   在输入的一侧或两侧应用一个按钮或附加按钮。
*   将按钮或附加按钮包装在`.input-group-prepend`类内（如果放在输入之前），或使用`.input-group-append`类（如果要放在最后）。

## 第一步

在 HTML [`<head>`](https://www.geeksforgeeks.org/html-head-tag/) 部分包含 Bootstrap CSS 来加载样式表。
...
我们可以直接复制[官方引导文档](https://getbootstrap.com/docs/4.0/getting-started/introduction/)中给出的引导启动器模板。
...
增加`.input-group`类内的 HTML [`<body>`](https://www.geeksforgeeks.org/html-body-tag/) 中的代码。在这个 HTML [`<div>`](https://www.geeksforgeeks.org/div-tag-html/) 部分，我们可以根据自己的选择添加插件和输入字段。
...
**注意：** 添加`<body>`标签内的所有输入组内容。

## 示例 1

下面的示例展示了我们如何在输入的两侧添加一个基本的附加按钮。它演示了`.input-group-prepend` 和 `.input-group-append` 类在 `.input-group` 类内的用法。
...
我们可以将 bootstrap 的相对表单大小类添加到`.input-group`类，如`.input-group-lg`、`.input-group-sm`等，即可自动调整内容大小。`.input-group`类返回默认大小。
...
下面的代码显示了带有复选框和单选按钮的输入组。除了文本，我们还可以在 T3 的插件中添加单选按钮或复选框。输入组 T5 类只需通过使用输入类型作为 `'checkbox'` 或 `'radio'`。
