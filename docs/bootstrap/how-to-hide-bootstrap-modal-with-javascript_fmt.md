...
> 原文：[https://www.geeksforgeeks.org/how-to-hide-bootstrap-modal-with-javascript/](https://www.geeksforgeeks.org/how-to-hide-bootstrap-modal-with-javascript/)

## 介绍

本文将告诉我们当引导模态（模态窗口）需要关闭时如何执行。在某个时间点，模态窗口——无论何时它被打开（和类模态一起），它都会被关闭。

一旦模态完成，在对用户隐藏后，事件将被激发。只要模态窗口被隐藏，函数就会被执行，下面的语法也会被触发。顺便说一句，它会呼叫呼叫者/用户，然后立即消失。此外，这完全不由用户管理。引导库已经内置，它将为您完成最大的工作。

## 语法

当引导模式即将被**隐藏**时，将使用以下语法。
