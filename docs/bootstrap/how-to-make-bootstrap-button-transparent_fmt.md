...
> 原文：[https://www.geeksforgeeks.org/how-to-make-bootstrap-button-transparent/](https://www.geeksforgeeks.org/how-to-make-bootstrap-button-transparent/)
...
*   `<button>`标签用于在 HTML 中指定按钮元素，按下后执行。
*   一般来说，引导的属性必须在课堂上提到。示例：`class=" "`
*   在 `class=" "` 内部：
    *   `btn` – 在提到 `btn` 属性时，指定它是一个引导按钮。
    *   `bg` – 指定按钮的背景颜色。
    *   `transparent` – 使按钮透明。
