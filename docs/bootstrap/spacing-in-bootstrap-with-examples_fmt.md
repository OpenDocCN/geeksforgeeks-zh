...
Bootstrap 有许多工具类可以轻松地为 HTML 中的元素设置样式。它包括用于修改元素外观的各种响应式填充和边距类。间隔实用程序没有断点符号，但可以应用于断点。
各种类别中使用以下语法来增加间距：

*   对于 `xs`：`(属性)(边)-(大小)`
*   对于 `sm`、`md`、`lg` 和 `xl`：`(属性)(边)-(断点)-(大小)`
...
断点是网站内容可以根据设备进行调整并允许向用户展示最佳布局的点。
...
| --- | --- |
| 出现在所有屏幕上 | `ml-3` |
| 仅出现在 `xs` | `ml-3`。`ml-sm-0` |
| 仅出现在 `sm` | `ml-sm-3`。`ml-md-0` |
| 仅出现在 `md` | `ml-md-3`。`ml-lg-0` |
| 仅出现在 `lg` | `ml-lg-3`。`d-xl-0` |
| 仅出现在 `xl` | `ml-xl-3` |
