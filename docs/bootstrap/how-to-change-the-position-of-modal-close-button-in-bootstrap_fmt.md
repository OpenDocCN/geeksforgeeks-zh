...
> 原文: [https://www.geeksforgeeks.org/how-to-change-the-position-of-modal-close-button-in-bootstrap/](https://www.geeksforgeeks.org/how-to-change-the-position-of-modal-close-button-in-bootstrap/)
...
`进场:` 在上图中，可以观察到模态表头内有一个 `class="close"` 的按钮。该按钮用于关闭模态元素。`data-dismiss` 属性用于切换模态元素的显示样式。
...
*   `&times;` 给出十字图标。
*   `data-dismiss` 将模态元素的显示属性从“块”切换到“无”。
...
`方法 2:` 使用 `data-dismiss` 属性定义您自己的关闭按钮。
...
**输出:** 按钮出现在模态元素的页脚部分，因为它被移动到页脚部分。

![](img/6d7d6b1d6c7d6c7d6c7d6c7d6c7d6c7d.png)

**注意:** 在方法 2 中，我们使用了 `data-dismiss` 属性来定义自己的关闭按钮。该按钮可以放置在模态元素内的任何位置。
