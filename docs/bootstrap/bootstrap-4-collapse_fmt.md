...
`.collapse` 类表示一个可折叠的元素，即点击按钮将显示或隐藏的内容。要控制(显示/隐藏)可折叠内容，请向锚点或按钮元素添加 `data-toggle="collapse"` 属性。然后添加 `data-target="#collapse_example"` 属性，将按钮与可折叠内容连接起来。
...
按钮或锚点标签可以通过在其 `href` 或 `data-target` 属性中使用 JQuery 选择器引用多个元素来显示或隐藏它们。如果多个按钮或定位标记可以用它们的 `href` 或 `data-target` 属性引用一个元素，那么它们可以显示和隐藏该元素。
...
以下示例通过扩展面板组件展示了一个简单的手风琴。使用 `data-parent` 属性确保当显示一个可折叠项目时，指定父项下的所有可折叠元素都将关闭。
