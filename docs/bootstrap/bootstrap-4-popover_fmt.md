...
`data-placement`属性用于设置弹出窗口元素的定位。popover 元素的位置可以设置为元素的顶部、底部、左侧或右侧。
...
默认情况下，当鼠标双击时，弹出窗口元素关闭。`data-trigger="focus"`属性用于在元素外部单击鼠标时关闭 popover 元素。
...
`data-trigger="hover"`属性用于创建 Popover 悬停效果。在这种情况下，当鼠标移动到上方时，将显示弹出窗口。
