...
我们可以在 `div` 类中使用`progress`类给我们的网页添加进度条。
在你的 HTML 代码中使用这个代码来添加一个默认的进度条。

### 创建进度条

1.  在 `div` 类中使用类`progress`。
2.  在已经制作好的 `div` 类中，添加另一个带有类`progress-bar`的 `div` 标签。
3.  使用宽度作为百分比，在样式属性下提及栏的进度。例如：`style="width:50%"`。
...
1.  在一个 `div` 元素中，使用一个`jumbotron`类。
2.  在这个 `div` 标签之后，您可以添加任何您想要的文本或信息。
3.  使用带有 `jumbotron` 类的 `div` 元素来关闭。
