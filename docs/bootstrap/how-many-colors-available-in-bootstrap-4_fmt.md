...
> 原文：[https://www.geeksforgeeks.org/how-many-colors-available-in-bootstrap-4/](https://www.geeksforgeeks.org/how-many-colors-available-in-bootstrap-4/)

Bootstrap 4 [支持 10 种不同的颜色实用程序类](https://www.geeksforgeeks.org/bootstrap-tutorials/)，可用于文本和背景颜色。每种颜色都有一个名称，可以用来描述页面上颜色的使用。

## 颜色实用程序文本类

这些类用于更改文本的颜色。

*   `text-primary`：它赋予文字天蓝色。
*   `text-secondary`：它赋予文本灰色。
*   `text-warning`：它给文本一个黄色的阴影。
*   `text-success`：它给文本一种绿色。
*   `text-danger`：它给文本一种红色。
*   `text-muted`：为文本赋予灰色。
*   `text-dark`：给文字一种深黑色。
*   `text-light`：给文字一个非常浅的灰白色。
*   `text-info`：它给文本一种蓝色的阴影。
*   `text-white`：它赋予文本白色。
...
这些类用于更改背景的颜色。当试图使用颜色实用程序类作为背景时，“文本”被替换为“背景”的“bg”。
