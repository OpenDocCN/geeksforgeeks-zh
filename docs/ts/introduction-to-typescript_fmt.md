...
**TypeScript === > > > JavaScript + 类型 + 一些新增功能**
...
*“通常情况下，TypeScript 最终被采用的方式——在企业、初创企业和个人开发者中——是你在一个项目上尝试它，然后你说‘哇，这太棒了！’然后你开始传播福音，它在你的势力范围内本地生长。”* — 安德斯·海尔斯伯格（核心开发者 TypeScript）。
...
保存上述文件后，我们需要将 TypeScript 代码转换成 JavaScript 文件。

在终端中，键入以下命令：

```bash
tsc types.ts (syntax : tsc filename).
```

成功编译后，一个与 `types.ts` 同名的 `.js` 扩展文件，即 `types.js` 将被创建在相同目录中，包含转译后的代码。现在运行 `index.html`，可以看到下面的输出。如上所述，TypeScript 代码被转换成标准的 JavaScript 代码。
