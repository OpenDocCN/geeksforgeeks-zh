...
> 原文: [https://www.geeksforgeeks.org/how-to-compile-a-typescript-file/](https://www.geeksforgeeks.org/how-to-compile-a-typescript-file/)
...
我们需要[下载](https://nodejs.org/en/download/) &安装 `node.js` & [`节点包管理器`](https://www.geeksforgeeks.org/node-js-npm-node-package-manager/) (`npm`)来设置打字稿工作的环境。为了检查为节点& `npm` 安装的版本，在 `cmd` 中键入以下命令:
...
在这个阶段，我们已经成功地完成了 Typescript 的环境设置，可以开始对它进行工作。请注意，我们必须在安装 typescript 的同一目录中创建并保存 typescript 文件。保存扩展名为 **.ts** 的打字稿文件 &在你的 `ts` 文件中添加下面的代码。
...
在本例中，您将看到我们遵循的安装、配置目录、编译&执行 `ts` 代码的完整步骤。
...
现在，typescript 文件被编译，我们的目录中可以有 `index.js` 文件，所以我们将通过键入下面的命令来检查输出。
...
从上图中，我们可以看到 javascript 文件正在成功运行，并显示了各自的输出。
