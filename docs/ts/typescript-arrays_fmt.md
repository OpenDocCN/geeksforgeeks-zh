...
```
let array_name: Array<datatype> = [val1, val2, valn..]
```
...
// or
var values: Array<string | number> = ['Apple', 2, 'Orange', 3, 4, 'Banana'];
```
...
```
let arr_name:datatype[initial_array_index][referenced_array_index] = [ [val1, val2, val3], [v1, v2, v3]];
```
...
我们可以通过使用或初始化数组对象来创建数组。`Array` 构造函数用于传递以下参数来创建数组:
