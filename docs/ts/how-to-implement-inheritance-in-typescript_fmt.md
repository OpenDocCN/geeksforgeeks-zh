...
在单一继承中，基类的属性和行为最多可以继承到一个派生类中。它用于向已经实现的类添加新功能。

### 示例

在本例中，我们创建了一个 `Person` 作为基类，并使用单一继承实现了 `Teacher` 作为派生类。
...
在本例中，我们创建了一个作为基类的 `Vehicle` 和一个作为子类的 `Car`，该子类派生了名为 `carModel` 的新类，`carModel` 类获取了 `Vehicle` 和 `Car` 类的所有特性。
