...
> 原文: [`https://www.geeksforgeeks.org/typescript-string-indexof-method/`](https://www.geeksforgeeks.org/typescript-string-indexof-method/)
...
*   `searchValue`: 此参数是一个字符串，表示要搜索的值。
*   `fromIndex`: 此参数是调用字符串中开始搜索的位置。

## 返回值

该方法返回找到的匹配项的索引，否则，如果没有找到，返回 `-1`。

以下示例说明了 TypeScriptJS 中的 `String indexOf()` 方法:
