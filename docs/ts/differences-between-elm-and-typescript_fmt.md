...
**TypeScript:** 它是一种基于 JavaScript 构建的开源编程语言，也被称为 JavaScript 超集。所有有效的 JavaScript 代码也是 TypeScript 代码。您可能会遇到一些错误，但是该程序仍然在 TypeScript 中运行。TypeScript 有自己的编译器，称为 TypeScript 编译器或 Babel，将 TypeScript 代码转换为 JavaScript 代码。因此，Typescript 是建立在 JavaScript 之上的。TypeScript 保护程序免受小错误的影响，因为编译器会自动捕获它们并节省时间。TypeScript 的打字系统比 JavaScript 好。TypeScript 提供了一种更好的记录方式，并验证代码是否正常工作。
