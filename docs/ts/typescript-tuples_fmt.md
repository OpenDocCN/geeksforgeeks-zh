...
为了表示多维坐标系，在抽象数学中使用的术语是元组。在 JavaScript 中我们没有元组作为数据类型，但在 TypeScript 中元组设施是可用的。
...
元组中允许这种类型的声明，因为我们正在向元组中添加数字和字符串值，它们对 `employee` 元组有效。
...
console.log("Items: " + empTuple); // here we print tuple elements
empTuple.pop(); // removed value from the tuple
console.log("Length of Tuple Items after pop: " + empTuple.length); // After popping elements from tuples calculate length of tuples.
console.log("Items: " + empTuple);
```
