...
在 TypeScript 中，有两种受支持的方法 `getter` 和 `setter` 来访问和设置类成员。这提供了对如何在每个对象上访问成员的更精细控制。
...
用于提取变量的值。`getter` 访问器属性是常规方法。它由对象文字中的 `get` 关键字表示。
...
`getter` 可以是公共的、受保护的、私有的。让某物表现得像一个属性或函数只是人为的。所以，`get square()` 和 `new MyClass().square` 与 `square()` 和 `new MyClass().square()` 相同。
...
*   TypeScript 访问器需要将编译器设置为输出 ECMAScript 5 或更高版本。它不支持低于 ECMAScript 5 的版本。
*   带有 `get` 和没有 `set` 属性的访问器被自动假定为只读的，不需要手动操作。当我们从代码中生成一个 `.d.ts` 文件时，这很有帮助。