...
> 原文：[https://www.geeksforgeeks.org/fabric-js-easeinoutelastic-method/](https://www.geeksforgeeks.org/fabric-js-easeinoutelastic-method/)

在游戏或动画中，有许多移动对象可以将它们从 A 点线性移动到 B 点，但是在应用了缓动或放松功能后，可以使其看起来更加自然。一个缓动功能描述了动画如何推进。这样，直线运动可以呈现出有趣的形状。

**放松功能**指定参数随时间的变化率。正是它的方程式使某物在开始时缓慢移动，在接近结束时加速或减速。最常见的一组缓动方程式来自罗伯特·彭纳的书和网页。

`easeInOutElastic()` 方法用于缓和其所用对象的进出效果。
