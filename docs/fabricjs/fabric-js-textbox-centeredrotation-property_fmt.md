...
> 原文：[https://www.geeksforgeeks.org/fabric-js-textbox-centeredrotation-property/](https://www.geeksforgeeks.org/fabric-js-textbox-centeredrotation-property/)
...
*   在此之后，我们将初始化由 FabricJS 提供的 `canvas` 和 `Textbox` 的实例，并使用 `centeredRotation` 属性启用/禁用画布 `Textbox` 的居中旋转，并在 `Canvas` 上呈现 `Textbox`，如下例所示。
...
该示例使用 FabricJS 禁用画布状文本框的居中旋转，如下所示。
