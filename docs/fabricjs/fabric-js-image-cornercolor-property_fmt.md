# Fabric.js 图像 cornerColor 属性

> 原文: [https://www.geeksforgeeks.org/fabric-js-image-cornercolor-property/](https://www.geeksforgeeks.org/fabric-js-image-cornercolor-property/)

`Fabric.js` 是一个用来处理画布的 JavaScript 库。`Image` 是 `fabric.js` 的一个类，用于创建图像实例。图像的 `cornerColor` 属性用于更改图像的控制角的颜色。

## 方法

首先导入 `fabric.js` 库。导入库后，在 `body` 标签中创建一个包含图像的画布块。之后，初始化一个由 `Fabric` 提供的 `Canvas` 和 `image` 类的实例。JS 并使用图像对象的 `cornerColor` 属性设置图像控制器的 `cornerColor`。
...
*   `image`: 该参数取图像。
*   `cornerColor`: 设置图像的控制角颜色。

## 示例 1

本示例使用 `FabricJS` 设置画布图像的控制角颜色，如下例所示。
