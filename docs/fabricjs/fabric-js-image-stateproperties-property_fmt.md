...
> 原文: [https://www.geeksforgeeks.org/fabric-js-image-stateproperties-property/](https://www.geeksforgeeks.org/fabric-js-image-stateproperties-property/)
...
首先导入 fabric.js 库。导入库后，在主体标签中创建一个包含图像的画布块。之后，初始化一个由 Fabric 提供的 `Canvas` 和 `image` 类的实例。JS 并使用 `stateProperties` 属性来获取画布图像的对象笔画的属性。之后，在画布上渲染图像。
...
本示例使用 FabricJS 设置画布图像的 `stateProperties` 属性，如下例所示:
