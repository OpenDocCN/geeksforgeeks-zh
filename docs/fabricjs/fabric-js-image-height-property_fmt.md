...
> 原文: [`https://www.geeksforgeeks.org/fabric-js-image-height-property/`](https://www.geeksforgeeks.org/fabric-js-image-height-property/)

**Fabric.js** 是一个用来处理画布的 JavaScript 库。画布图像是用于创建图像实例的 `fabric.js` 类之一。画布图像意味着图像是可移动的，可以根据需要拉伸。图像的 `height` 属性用于设置图像的高度。
