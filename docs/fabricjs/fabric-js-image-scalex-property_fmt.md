...
> 原文: [https://www.geeksforgeeks.org/fabric-js-image-scalex-property/](https://www.geeksforgeeks.org/fabric-js-image-scalex-property/)
...
首先导入 Fabric.js 库。导入库后，在包含图像的主体标签中创建一个画布块。之后，初始化 Fabric.js 提供的 `Canvas` 和 `Image` 类的一个实例，使用 `scaleX` 属性设置 `Image` 的默认水平比例，并在 `Canvas` 上渲染这个 `Image`。
...
本示例使用 Fabric.js 设置画布 `Image` 的 `scaleX` 属性，如下例所示:
