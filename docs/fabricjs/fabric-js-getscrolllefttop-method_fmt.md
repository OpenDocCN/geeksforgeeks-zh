...
> 原文: [https://www.geeksforgeeks.org/fabric-js-getscrolllefttop-method/](https://www.geeksforgeeks.org/fabric-js-getscrolllefttop-method/)

Fabric.js 中的 `getScrollLeftTop()` 方法用于查找元素的滚动偏移量。此方法返回的对象包含“顶部”和“左侧”属性，分别表示“顶部”和“左侧”偏移。必须首先选择页面中的 HTML 元素才能传递此方法。
...
*   `element`: 该参数用于指定需要找到滚动偏移量的 HTML 元素。

## 返回值

这个方法返回一个包含“顶部”和“左侧”滚动偏移量作为属性的对象。

下面的例子演示了 Fabric.js 的 `getScrollLeftTop()` 方法:
