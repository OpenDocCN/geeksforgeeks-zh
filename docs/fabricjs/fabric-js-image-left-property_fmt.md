# Fabric.js 图像 left 属性

> 原文: [https://www.geeksforgeeks.org/fabric-js-image-left-property/](https://www.geeksforgeeks.org/fabric-js-image-left-property/)
...
首先导入 `fabric.js` 库。导入库后，在主体标签中创建一个包含图像的画布块。之后，初始化一个由 `Fabric` 提供的 `Canvas` 和 `image` 类的实例。JS，并将 `left` 属性的数字作为参数来设置图像对象的左边位置。
...
*   `image`: 该参数取图像。
*   `left`: 该参数取一个数字来设置对象的左侧位置。

## 示例

本示例使用 `FabricJS` 设置画布图像沿 x 轴的左侧位置，如下例所示。
