...
> 原文: `https://www.geeksforgeeks.org/fabric-js-textbox-opacity-property/`
...
*   之后，我们将初始化由 FabricJS 提供的 `Canvas` 和 `Textbox` 的实例，并使用 `opacity` 属性设置 `Textbox` 的不透明度，并在 `Canvas` 上呈现 `Textbox`，如下例所示。
