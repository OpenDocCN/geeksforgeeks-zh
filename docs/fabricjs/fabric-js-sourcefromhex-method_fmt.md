...
> 原文: [https://www.geeksforgeeks.org/fabric-js-sourcefromhex-method/](https://www.geeksforgeeks.org/fabric-js-sourcefromhex-method/)

`sourceFromHex()`方法用于以十六进制(十六进制)格式返回指定颜色值的颜色数组表示。
...
*   `color`: 该参数保存 HEX 格式的指定颜色值。

## 返回值

这个方法返回一个 RGBA(红绿蓝和阿尔法)格式的值的数组表示，该值是十六进制格式的指定颜色值的颜色。
