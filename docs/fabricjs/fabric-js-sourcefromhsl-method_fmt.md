...
> 原文: [https://www.geeksforgeeks.org/fabric-js-sourcefromhsl-method/](https://www.geeksforgeeks.org/fabric-js-sourcefromhsl-method/)

该方法用于返回 `HSL`（色调-饱和度-亮度）或 `HSLA`（色调-饱和度-亮度-α）格式的指定颜色值的颜色数组表示。
...
*   `color`: 此参数保存 `HSL` 或 `HSLA` 格式的指定颜色值。
