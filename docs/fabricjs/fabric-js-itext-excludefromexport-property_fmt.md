# Fabric.js Itext excludeFromExport 属性
...
## 方法
首先导入 `fabric.js` 库。导入库后，在主体标签中创建一个包含 `Itext` 的画布块。之后，初始化一个由 Fabric 提供的 `Canvas` 和 `Itext` 类的实例。JS 并使用 `excludeFromExport` 属性。
...
## 示例
本示例使用 FabricJS 设置画布 `Itext` 的 `excludeFromExport` 属性，如下例所示:
