...
> 原文: `https://www.geeksforgeeks.org/fabric-js-itext-touchcornersize-property/`
...
首先导入 `fabric.js` 库。导入库后，在主体标签中创建一个包含 `Itext` 的画布块。之后，初始化画布实例和 `fabric` 提供的 `Itext` 类并使用 `touchCornerSize` 属性设置拐角尺寸。
