...
> 原文: [https://www.geeksforgeeks.org/fabric-js-textbox-movecursor-property/](https://www.geeksforgeeks.org/fabric-js-textbox-movecursor-property/)
...
*   导入库后，我们将在 `body` 标签中创建一个包含文本框的画布块。
*   之后，我们将初始化由 FabricJS 提供的 `canvas` 和 `Textbox` 的实例，并使用 `moveCursor` 属性设置 canvas Textbox 的移动光标值，并在 Canvas 上呈现 Textbox，如下例所示。
...
本示例使用 FabricJS 设置对象移动时将起作用的 `moveCursor` 值。
