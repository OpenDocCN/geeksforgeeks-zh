...
> 原文: [https://www.geeksforgeeks.org/fabric-js-polygon-noscalecache-property/](https://www.geeksforgeeks.org/fabric-js-polygon-noscalecache-property/)
...
*   **noScaleCache:** 当为 `true` 时，对象被缓存在一个额外的画布上。当为 `false` 时，除非必要（例如 `clipPath`），否则不缓存对象。默认为 `true`。

下面的例子说明了 Fabric.js 中的多边形 `noScaleCache` 属性:
