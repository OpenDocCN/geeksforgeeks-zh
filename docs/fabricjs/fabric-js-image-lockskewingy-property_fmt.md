...
> 原文: [https://www.geeksforgeeks.org/fabric-js-image-lockskewingy-property/](https://www.geeksforgeeks.org/fabric-js-image-lockskewingy-property/)
...
首先导入 `fabric.js` 库。导入后，在主体标签中创建一个包含图像的画布块。之后，初始化一个由 Fabric.js 提供的 `Canvas` 和 `image` 类的实例，并使用 `lockSkewingY` 属性来锁定垂直倾斜。最后，在画布上渲染图像。
...
本示例使用 FabricJS 设置画布图像的 `lockSkewingY` 属性，如下例所示:
