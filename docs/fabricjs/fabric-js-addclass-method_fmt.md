...
Fabric.js 中的 `addClass()`方法用于将指定的类添加到指定的 HTML 元素中。必须首先选择页面中的 HTML 元素才能传递此方法。
...
### 参数:
该方法接受两个参数，如上所述，如下所述:
*   `element`: 此参数用于指定要添加类的 HTML 元素。
*   `className`: 此参数用于指定必须添加到元素中的类。
...
<body>
    ...
    <script>
        // Select the element that has to be applied the classes
        let elem = document.querySelector('#box1');
        let elem2 = document.querySelector('#box2');

        // Add the required classes to the element
