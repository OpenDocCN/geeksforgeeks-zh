...
> 原文：[https://www.geeksforgeeks.org/fabric-js-easeoutelastic-method/](https://www.geeksforgeeks.org/fabric-js-easeoutelastic-method/)
...
*   `t`：此参数保存动画开始的指定时间。例如，如果`t`的值为0，则表示动画刚刚开始。
*   `b`：此参数保存对象在x轴上的指定起始位置。例如，如果`b`的值为10，则表示对象在x坐标上的起始位置为10。
*   `c`：此参数保存对象的指定值变化。例如，如果`c`的值是30，这意味着对象必须向右移动30°，以40°结束。
*   `d`：此参数保存整个过程的指定持续时间。例如，如果`d`的值为2，则意味着对象有2秒的时间来执行从10到40的运动。

## 返回值

该方法返回物体的缓和位置，即物体在特定时间的位置。
