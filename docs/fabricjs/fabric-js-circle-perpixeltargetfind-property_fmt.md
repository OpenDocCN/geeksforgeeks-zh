...
> 原文: [https://www.geeksforgeeks.org/fabric-js-circle-perpixeltargetfind-property/](https://www.geeksforgeeks.org/fabric-js-circle-perpixeltargetfind-property/)
...
*   之后，我们将初始化 FabricJS 提供的 `Canvas` 和 `circle` 的实例，并使用 `perPixelTargetFind` 属性设置 `circle` 的 `perPixelTargetFind` 属性值，并在 `Canvas` 上渲染 `circle`，如下例所示。
...
本示例使用 FabricJS 设置画布圆的 `perPixelTargetFind` 属性。
