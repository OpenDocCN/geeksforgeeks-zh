...
> 原文: [https://www.geeksforgeeks.org/fabric-js-triangle-strokewidth-property/](https://www.geeksforgeeks.org/fabric-js-triangle-strokewidth-property/)
...
*   在此之后，我们将初始化由 FabricJS 提供的 `canvas` 和 `Triangle` 的实例，并使用 `strokeWidth` 属性设置 `canvas` `Triangle` 的笔画宽度，并在 Canvas 上呈现 `Triangle`，如下例所示。
...
本示例使用 FabricJS 设置画布三角形的描边 `strokeWidth`。
