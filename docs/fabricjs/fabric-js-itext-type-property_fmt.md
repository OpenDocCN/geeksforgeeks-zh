...
> 原文: [https://www.geeksforgeeks.org/fabric-js-itext-type-property/](https://www.geeksforgeeks.org/fabric-js-itext-type-property/)
...
首先导入 `fabric.js` 库。导入后，在主体标签中创建一个包含 `Itext` 的画布块。之后，初始化画布实例和由 `Fabric.js` 提供的 `Itext` 类，并使用 `类型` 属性获取对象的类型。
...
本示例使用 `FabricJS` 设置画布 `Itext` 的 `类型` 属性，如下例所示。
