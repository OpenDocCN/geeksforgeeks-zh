...
> 原文: [https://www.geeksforgeeks.org/fabric-js-image-cropx-property/](https://www.geeksforgeeks.org/fabric-js-image-cropx-property/)
...
首先导入 `fabric.js` 库。导入库后，在主体标签中创建一个包含图像的画布块。之后，初始化一个由 `Fabric` 提供的 `Canvas` 和 `image` 类的实例。JS，并使用图像对象的 `cropX` 属性给出画布图像的裁剪宽度(以像素为单位)。
...
本示例使用 `FabricJS` 沿着 x 轴裁剪画布图像的部分，如下例所示。
