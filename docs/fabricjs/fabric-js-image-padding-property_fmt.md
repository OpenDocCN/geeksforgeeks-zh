...
首先导入 Fabric.js 库。导入库后，在包含图像的主体标签中创建一个画布块。之后，初始化一个由 Fabric.js 提供的 `Canvas` 和 `Image` 类的实例，使用 `padding` 属性设置 `padding`，并在 `Canvas` 上呈现这个 `Image`。
...
*   `padding`: 指定对象和控制边框之间的填充。

## 示例

本示例使用 Fabric.js 设置画布 `Image` 的填充属性，如下例所示:
