...
> 原文: [https://www.geeksforgeeks.org/fabric-js-itext-isediting-property/](https://www.geeksforgeeks.org/fabric-js-itext-isediting-property/)
...
首先导入 `fabric.js` 库。导入库后，在主体标签中创建一个包含 `IText` 的画布块。之后，初始化一个由 Fabric 提供的 `Canvas` 和 `IText` 类的实例。JS 并使用 `isEditing` 属性。
...
本示例使用 FabricJS 设置画布 `IText` 的 `isEditing` 属性，如下例所示:
