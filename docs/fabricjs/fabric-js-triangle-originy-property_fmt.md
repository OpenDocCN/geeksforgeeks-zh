...
为了实现这一点，我们将使用一个名为`FabricJS`的 JavaScript 库。导入库后，我们将在主体标签中创建一个包含三角形的画布块。之后，我们将初始化由 FabricJS 提供的`Canvas`和三角形的实例，并使用`originY`属性设置三角形的垂直变换，并在 Canvas 上渲染三角形，如下例所示。
