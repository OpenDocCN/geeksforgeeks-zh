# fabric.js `resolveNamespace()`方法

> 原文：[https://www.geeksforgeeks.org/fabric-js-resolvenamespace-method/](https://www.geeksforgeeks.org/fabric-js-resolvenamespace-method/)

`resolveNamespace()`方法用于返回指定命名空间的对象。
...
*   `namespace`：此参数保存指定的命名空间字符串，例如`"fabric.Image.filter"`、`"fabric"`等。
