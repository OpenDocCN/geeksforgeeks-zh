...
在 Vue.js 中，任何数据值都可以用括号输出。然而，这可能会进行小的计算，因为它仅限于单个 JavaScript 表达式，而且模板应该只关心向用户显示数据，而不执行任何逻辑计算。为了避免单个表达式的这种限制，并有更多的声明性模板，我们使用 Vue.js 的计算属性，计算值遵循相同的旧插值，但计算逻辑是在 JavaScript 中定义的。
