...
> 原文：[https://www.geeksforgeeks.org/building-a-shopping-list-with-vuejs/](https://www.geeksforgeeks.org/building-a-shopping-list-with-vuejs/)
            ...
            <div class="lists">
                <ul>
                    <li v-for="(list, index) in lists" :key="index">
                        {{ list }}
                        <i class="fa fa-trash" aria-hidden="true" v-on:click="removeList(index)">
                        </i>
                    </li>
                </ul>
            </div>
...
*   返回需要在视图中处理的数据。这些数据必须在 `data` 选项中指定。
*   运行数据属性中的项目列表。
*   我们可以使用 `v-for` 指令基于一个数组来呈现我们的水果项目列表。
*   现在，让我们通过使用 `v-model` 基于输入类型更新元素来创建双向数据绑定。
*   创建一个功能，使您能够删除我们列出的每个项目。

**结论：** 这是如何使用 CDN 通过静态 HTML 建立一个 `Vue` 项目，以及如何在 `Vue` 中创建一个简单的购物清单。我们了解了 `Vue` 实例、`data`、`methods`、条件、事件。