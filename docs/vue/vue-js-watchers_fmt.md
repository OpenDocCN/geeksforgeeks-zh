...
**注意:** 观察者一次只能改变一个属性。如果必须更改多个组件值，可以使用计算属性。
...
我们将首先创建一个简单的 Vue.js 应用，不使用任何 `Watcher`。该程序的功能是通过点击按钮改变组件的值(输入值乘以 2)。
...
在上例中，`result` 的值仅在按钮点击时发生变化。我们现在将使用 `watch` 来注意输入组件 `value1` 的变化，并自动更新 `result` 值。我们将定义 `watch`，并在其中写入所需的功能，如下所示:
...
观察者现在寻找输入值 `value1` 的变化。每当输入值发生变化时，里面的函数自动执行(即输入值乘以 2)，`result` 的值自动变化。我们不必专门分配任何事件并等待值的改变。我们将在前面的代码中添加上述更改，并移除按钮及其功能。
