...
`v-on:click` 指令是一个 [Vue.js](https://www.geeksforgeeks.org/vue-js-introduction-installation/) 指令，用于向元素添加 click 事件监听器。首先，我们将创建一个 id 为 `app` 的 div 元素，让我们将 `v-on:click` 指令应用于一个元素。此外，我们甚至可以在点击发生时执行一个功能。
...
本示例使用 VueJS 通过 `v-on:click` 切换元素的可见性。
