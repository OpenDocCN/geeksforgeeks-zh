...
> 原文：`https://www.geeksforgeeks.org/arithmetic-logic-shift-unit-in-computer-architecture/`
...
输出端的`4×1`多路复用器在`Ei`的相关算术输出和`Hi`的逻辑输出之间进行选择。多路复用器中的数据通过输入和选择，多路复用器的另外两个数据输入获得输入`Ai–1`用于`shr`操作，`Ai + 1`用于`shl`操作。
...
1.  前`8`个是算术运算，由`S3·S2 = 00`选择
2.  接下来的`4`个是由`S3·S2 = 01`选择的逻辑运算
3.  最后两个是移位操作，由`S3·S2 = 10` & `11`选择
