...
> 原文：[https://www.geeksforgeeks.org/timing-diagram-of-mov-instruction-in-microprocessor/](https://www.geeksforgeeks.org/timing-diagram-of-mov-instruction-in-microprocessor/)
...
`B` 是目的寄存器，`C` 是源寄存器，其内容需要传输到目的寄存器。
...
该指令只需要操作码提取，因此时序图需要 4 个 `T` 状态。对于操作码提取，`IO/M`(低电平有效)= 0，`S1` = 1，`S0` = 1。
...
6.  **`IO/M`(低电平有效)–**信号始终为 1，因为操作正在内存上执行。
