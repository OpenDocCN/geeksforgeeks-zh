...
> 原文:[https://www.geeksforgeeks.org/error-detection-codes-parity-bit-method/](https://www.geeksforgeeks.org/error-detection-codes-parity-bit-method/)
...
## 要记住的点:
*   在 `1 的补码`中，有符号数+0 和-0 有两种不同的表示。
*   8 位数字的`符号幅度表示`范围，其中 1 位用作符号位，如下所示-2<sup>7</sup> 到+2<sup>7</sup>。
*   [如果尾数的最高有效位为 1，则称浮点数](https://www.geeksforgeeks.org/floating-point-representation-basics/)为`规格化`。例如，6 位二进制数 001101 由于两个前导 0 而被归一化。
*   [`布斯算法`](https://www.geeksforgeeks.org/computer-organization-booths-algorithm/)使用两个 n 比特数进行乘法运算，给出 2n 比特的结果。
*   `布斯算法`使用数字的`2 的补码表示`，对正数和负数都有效。
