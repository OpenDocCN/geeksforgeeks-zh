...
**符号幅度法表示的数字范围** = `-(2^(n-1) -1)` 至 `+(2^(n-1) -1)` (对于 `n` 位数字)
...
*   因为总是 1，所以不需要存储
*   科学记数法，其中数字被标准化，在小数点前给出一位数，如在十进制中，例如 `3.123 x 10^3`
...
小数点后的数字= `1101`
扩展到 23 位= `11010000000000000000000`
