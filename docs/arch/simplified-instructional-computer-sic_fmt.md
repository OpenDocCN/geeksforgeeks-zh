...
> 原文: [https://www.geeksforgeeks.org/simplified-instructional-computer-sic/](https://www.geeksforgeeks.org/simplified-instructional-computer-sic/)

`SIC` 是一种假设的计算机，具有在真实机器中经常发现的硬件特征。这台机器有两个版本:

1.  标准模型
2.  `SIC/XE` (额外设备或昂贵)

`SIC` 的目标程序可以在 `SIC/XE` 上正确执行，这就是所谓的向上兼容。
...
I. `A` (累加器-`0`): 用于数学运算。
II. `X` (索引寄存器-`1`): 用于寻址。
III. `L` (链接寄存器-`2`): 在子程序的情况下，它存储指令的返回地址。
IV. 程序计数器 `8`: 保存下一条要执行的指令的地址。
V. `SW` (状态字-`9`): 包含多种信息
...
*   测试设备 (`TD`) 测试设备是否准备好。状态字寄存器中的条件代码用于此目的。如果 `cc` 不等于 `0`，则设备未准备好。
*   读取数据 (`RD`) 从器件读取一个字节，并存储在寄存器 `A` 中。
*   写数据 (`WD`) 将寄存器 `A` 中的一个字节写入器件。

**参考文献:**

Leland L. Beck: 《系统编程导论》，第三版，爱迪生-卫斯理，1997。