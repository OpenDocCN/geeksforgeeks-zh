...
> 原文: [https://www.geeksforgeeks.org/8086-program-sort-integer-array-ascending/](https://www.geeksforgeeks.org/8086-program-sort-integer-array-ascending/)

## 问题–
在 8086 微处理器中编写一个程序，对 n 个数字的数组中的数字进行升序排序，其中大小“n”存储在内存地址 `2000 : 500`，数字从内存地址 `2000 : 501` 开始存储。
...
17. `JNZ 40F`: 如果零标志复位，跳转到地址 `40F`。
18. `DEC CL`: 寄存器 `CL` 的值减 1。
19. `JNZ 407`: 如果零标志复位，跳转到地址 `407`。
20. `HLT`: 停止。