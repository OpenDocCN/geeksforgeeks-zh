...
> 原文：[https://www.geeksforgeeks.org/universal-shift-register-in-digital-logic/](https://www.geeksforgeeks.org/universal-shift-register-in-digital-logic/)

通用移位寄存器是同时具有右移位和左移位以及并行加载能力的寄存器。通用移位寄存器在计算机中用作存储元件。单向移位寄存器只能向一个方向移位。双向移位寄存器能够双向移位。通用移位寄存器是双向移位寄存器和单向移位寄存器的组合设计，具有并行负载提供。
