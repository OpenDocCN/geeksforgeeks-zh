...
| **名称** | **汇编语法** | **寻址功能** |
| --- | --- | --- |
| [即刻](https://www.geeksforgeeks.org/difference-between-direct-and-immediate-addressing-modes/) | `#Value` | `Operand = Value` |
| 绝对空头 | `Value` | `EA = SignExtW(Value)` |
| 绝对长 | `Value` | `EA = Value` |
| [注册](https://www.geeksforgeeks.org/difference-between-register-mode-and-register-indirect-mode/) | `Rn` | `EA = Rn; Operand = (Rn)` |
| 间接注册 | `(An)` | `EA = (An); Operand = ((An))` |
| 自动增量 | `(An)+` | `EA = (An); Operand = ((An)); Increment An` |
| 自动减量 | `-(An)` | `Decrement An; EA = (An); Operand = ((An))` |
| 索引基本 | `WValue(An)` | `EA = WValue + (An)` |
| 索引完整 | `BValue(An, Rk, S)` | `EA = BValue + (An) + (Rk)` |
| 相对基础 | `WValue(PC)` 或 `Label` | `EA = WValue + (PC)` |
| 相对满 | `BValue(PC, Rk, S)` 或 `Label` | `EA = BValue + (PC) + (Rk)` |
...
我们做一个快速的示例来了解一下。考虑指令`ADD 100(PC, A1), D0`。当以机器形式编码时，该指令由两个字组成。操作码字指定这是一条加法指令，目的寄存器是数据寄存器`D0`，源操作数使用完全相对寻址模式。第二个字，也称为`扩展字`，指定寄存器`A1`用作索引寄存器，它包含 8 位内的偏移值 100。
