...
> 原文: [https://www.geeksforgeeks.org/difference-between-8051-and-avr/](https://www.geeksforgeeks.org/difference-between-8051-and-avr/)

先决条件 – [微控制器(C)和微处理器(P)](https://www.geeksforgeeks.org/whats-difference-between-microcontoller-%C2%B5c-and-microprocessor-%C2%B5p/)

一个`微控制器`是一个单一的集成电路(`IC`)，相当于一个小的单机，设计用来执行`嵌入式系统`的特定任务。`8051`和`AVR`属于微控制器家族。`8051 微控制器`和`AVR 微控制器`在不同的架构和不同的指令集、速度、成本、内存、功耗、总线宽度等方面各不相同。现在让我们详细了解它们之间的区别。
...
| 02. | 微控制器来自英特尔公司。 | 微控制器来自`Atmel`公司。 |
