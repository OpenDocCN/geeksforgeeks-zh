...
> 原文：`https://www.geeksforgeeks.org/types-of-cache-misses/`

当数据在`高速缓冲存储器`中不可用时，发生`高速缓冲存储器未命中`。当中央处理器检测到未命中时，它通过从主存储器获取请求的数据来处理未命中。
...
一般来说，我们使用随机块替换、 `LRU` 或`先进先出页面替换`技术来从主存储器带来高速缓存中的未命中页面。