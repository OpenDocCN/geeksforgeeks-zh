...
> 原文: [https://www.geeksforgeeks.org/computer-organization-and-architecture-pipelining-set-1-execution-stages-and-throughput/](https://www.geeksforgeeks.org/computer-organization-and-architecture-pipelining-set-1-execution-stages-and-throughput/)

为了提高中央处理器的性能，我们有两个选择:
1. 通过引入更快的电路来改善硬件。
2. 安排硬件，以便可以同时执行多个操作。

既然硬件的速度有限制，更快电路的成本也相当高，我们就不得不采用第 2 个选项。

`流水线化`是对 CPU 的硬件元件进行排列，使其整体性能提高的过程。在流水线处理器中同时执行一条以上的指令。
...
来源: [goo.gl/J9KVNt](http://goo.gl/J9KVNt) [https://en.wikipedia.org/wiki/Hazard_(computer_architecture)](https://en.wikipedia.org/wiki/Hazard_(computer_architecture)) [https://en.wikipedia.org/wiki/Data_dependency](https://en.wikipedia.org/wiki/Data_dependency)
