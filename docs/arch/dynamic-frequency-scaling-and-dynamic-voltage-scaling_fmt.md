...
> 原文：[https://www.geeksforgeeks.org/dynamic-frequency-scaling-and-dynamic-voltage-scaling/](https://www.geeksforgeeks.org/dynamic-frequency-scaling-and-dynamic-voltage-scaling/)

“频率”在本文中是指时钟频率或中央处理器的操作频率。因此术语`动态频率缩放`是指运行时中央处理器时钟频率的变化。

现在，上面的定义会立刻引发一个问题，为什么我们需要这么做？

这个问题的答案在于性能和功耗之间的权衡。我们知道处理器的`性能取决于`2个指标，它们是：
...
现在，`动态频率缩放`是一种平衡性能和功耗的技术。它指的是时钟频率的持续变化，以优化中央处理器的性能和功耗。
