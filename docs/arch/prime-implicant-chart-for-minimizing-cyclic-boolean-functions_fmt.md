...
> 原文: [https://www.geeksforgeeks.org/prime-implicant-chart-for-minimizing-cyclic-boolean-functions/](https://www.geeksforgeeks.org/prime-implicant-chart-for-minimizing-cyclic-boolean-functions/)

先决条件 – [`K-Map`(`卡诺图`)](https://www.geeksforgeeks.org/k-mapkarnaugh-map/)、[`K-Map`中的`隐含项`](https://www.geeksforgeeks.org/digital-logic-implicants-k-map/)、
如果一个函数在其各自的`K-Map`中没有`本质`素隐含项，则称该函数为`循环布尔函数`。

## 循环函数的性质

*   每一个质数隐式都是相同的大小。
*   每个最小项至少被两个素隐含项覆盖（这意味着没有`本质`的素隐含项）。
*   没有`本质`的质因数意味着对于这些函数存在不止一个最小化的解/表达式，这将进一步使用数字电路来实现。
...
正如我们在上面的`K-Map`中看到的，不存在`本质`的素蕴涵。这里我们可以用质数隐含图来轻松求解。

## 使用素数隐含图求解上述函数的步骤

### Step-1
绘制质数隐含图如下。水平条目表示给定的最小项，它们与所有质数隐含项（垂直）进行映射。当一个质数隐含项在`K-Map`中覆盖某个特定最小项时，方框会被划叉（'x'）。
...
### Step-2
任意选择一个质数隐含项；勾选（`✓`）该质数隐含项及其对应覆盖的最小项。然后删除该质数隐含项的行及其最小项对应的列。

在我们的例子中，选择了覆盖最小项 2 和 10 的素数隐式 `A`。因此，删除 `A` 行以及 2 和 10 列。任意选择的素数隐含（在我们的例子 `A` 中）必须出现在最终的极小表达式中。

![](img/e91074c466b6c941495c1ef9bcf6e6b2.png)

![](img/e702c72cafd58b0e4c702b8a51d76e23.png)

### Step-3
找出所有被其他质数隐含项完全覆盖的质数隐含项，并删除它们对应的行（因为这些是非本质质数隐含项）。
...
1.  找出仅被一个质数隐含项覆盖的最小项。
2.  勾选（`✓`）该最小项、其对应的质数隐含项以及被该对应质数隐含项覆盖的所有其他最小项。
3.  如果所有的最小项都被勾选（`✓`），则停止程序，否则转到子步骤-1。
...
### 第二步
`A` 任意选择。现在删除了 `A` 行和相应的最小项（0 和 2）的列。
