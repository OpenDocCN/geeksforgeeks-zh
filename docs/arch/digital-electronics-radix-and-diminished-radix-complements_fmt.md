...
> 原文: [https://www.geeksforgeeks.org/digital-electronics-radix-and-diminished-radix-complements/](https://www.geeksforgeeks.org/digital-electronics-radix-and-diminished-radix-complements/)
...
*   对于二进制数，`r = 2`
*   所以 `2^n` 被表示为 `100…00`（1 后跟 `n` 个 `0`）
*   这意味着 `2^n -1` 被表示为 `1…11`（`n` 个 `1`）
...
当用数字硬件实现减法时，**借用**的方法比使用补码的方法效率低。（需要更多数量的电路）。做减法之前要记住的一件事是确保两者的位数相同。
...
将 `N` 的 10 的补码加到 `M` 上：`98765 + 95679 = 194444`，我们得到进位 1，这意味着 `M-N` 是正的。所以只要去掉进位，如果有的话，结果就是答案，所以答案是 `94444`。
