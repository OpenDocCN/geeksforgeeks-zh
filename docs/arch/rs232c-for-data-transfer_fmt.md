...
### 2. 电缆和电线:
`RS232 c` 的最大电缆长度等于`15.24 米`或等于`2500pF`的电容。电线阻抗的限值范围从`3 欧姆`到`7 欧姆`。
...
`RS232C` 需要`25 针`连接器来连接 `DTE` 和 `DCE`。下面是 `RS232C` 的引脚和信号列表，以及 `DTE` 和 `DCE` 之间使用驱动器和接收器的连接。
...
*   最初，当数据从计算机传输到调制解调器时，计算机激活发送给调制解调器的 `RTS` 信号。
*   调制解调器依次激活 `DCD`，然后 `CTS` 被激活。
*   然后计算机发送 `TXD` 的数据。数据传输完成后，计算机将停用 `RTS`，这将导致调制解调器停用 `CTS`。
