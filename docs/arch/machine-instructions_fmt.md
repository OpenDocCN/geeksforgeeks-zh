...
- `MOV`：移动字节或字到寄存器或存储器。
- `IN`，`OUT`：从端口输入字节或字，从输出字到端口。
- `LEA`：加载有效地址。
- `LDS`，`LES`：使用数据段、额外段加载指针。
- `PUSH`，`POP`：将单词推到堆栈上，将单词弹出堆栈。
- `XCHG`：交换字节或字。
- `XLAT`：使用查找表翻译字节。
...
- `AAA`，`AAS`，`AAM`，`AAD`：ASCII 调整为添加，子，多，div。
...
- `XOR`：字节或字的逻辑异或。
- `TEST`：测试字节或字（与，不存储）。
- `SHL`，`SHR`：逻辑移位旋转指令左移，右移字节还是字？由 `1` 或 `CL`。
- `SAL`，`SAR`：算术左移，右移字节还是单词？被 `1` 或 `CL`。
- `ROL`，`ROR`：向左，向右旋转字节还是单词？由 `1` 或 `CL` 表示。
- `RCL`，`RCR`：通过进位字节或字向左、向右旋转？由 `1` 或 `CL` 表示。
...
- `STOS`，`LODS`：加载、存储字节或字符串到 `AL`/`AX`。

### 5. 控制转移指令
有条件，无条件，调用子程序并从子程序返回。
