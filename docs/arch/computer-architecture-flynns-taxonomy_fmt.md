...
> 原文：[https://www.geeksforgeeks.org/computer-architecture-flynns-taxonomy/](https://www.geeksforgeeks.org/computer-architecture-flynns-taxonomy/)
...
### 1. 单指令单数据流系统
一个 `SISD` 计算系统是一个单处理器机器，能够执行一条指令，操作一个数据流。在 `SISD` 中，机器指令以顺序方式处理，采用这种模型的计算机通常称为顺序计算机。大多数传统计算机都具有 `SISD` 架构。所有待处理的指令和数据都必须存储在主存储器中。
...
### 2. 单指令多数据流系统
一个 `SIMD` 系统是一个多处理器机器，能够在所有 `CPU` 上执行相同的指令，但操作不同的数据流。基于 `SIMD` 模型的机器非常适合科学计算，因为它们涉及大量的向量和矩阵操作。为了将信息传递给所有处理单元（`PEs`），向量的有组织数据元素可以被划分为多个集合（`N` 个 `PE` 系统对应 `N` 组），每个 `PE` 可以处理一个数据集。
...
### 3. 多指令单数据流系统
一个 `MISD` 计算系统是一个多处理器机器，能够在不同的 `PEs` 上执行不同的指令，但它们都操作在同一个数据集上。

![](img/55d072b17930c4686f59409ddb17da7e.png)

例如 `Z = sin(x)+cos(x)+tan(x)`，系统对同一数据集执行不同的操作。使用 `MISD` 模型制造的机器在大多数应用中都没有用，虽然制造了一些机器，但是没有一个是商用的。

### 4. 多指令多数据流系统
一个 `MIMD` 系统是一个多处理器机器，能够在多个数据集上执行多条指令。`MIMD` 模型中的每个 `PE` 都有独立的指令流和数据流；因此，使用该模型构建的机器能够处理任何类型的应用程序。与 `SIMD` 和 `MISD` 机器不同，`MIMD` 机器中的 `PEs` 异步工作。
