...
> 原文: [https://www.geeksforgeeks.org/8085-program-count-number-given-8-bit-number/](https://www.geeksforgeeks.org/8085-program-count-number-given-8-bit-number/)

## 问题
编写程序对给定 8 位数中的 1 进行计数，使用寄存器 `B` 显示 1 的计数，其中起始地址为 `2000`，该数存储在 `3000` 存储地址，并将结果存储到 `3001` 存储地址。
...
| `2007` | `RAR` |  | 用进位向右旋转 `A` |
| `2008` | `JNC` | `200C` | 如果没有进位就跳 |
| `200B` | `INR` | `B` | `[B] <- [B] + 1` |
| `200C` | `DCR` | `C` | `[C] <- [C] - 1` |
| `200D` | `JNZ` | `2007` | 如果不为零，则跳转 |
| `2010` | `MOV` | `A, B` | `[A] <- [B]` |
| `2011` | `STA` | `3001` | 1 的数量 |
| `2014` | `HLT` |  | 停止 |
