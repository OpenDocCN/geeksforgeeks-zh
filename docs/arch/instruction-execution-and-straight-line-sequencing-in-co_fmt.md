...
> 原文: [https://www.geeksforgeeks.org/instruction-execution-and-straight-line-sequencing-in-co/](https://www.geeksforgeeks.org/instruction-execution-and-straight-line-sequencing-in-co/)

**简介:**
由于指令是存储在内存中的程序的一部分，所以每当处理器需要执行一条指令时，为此处理器首先从内存中取出指令，然后解码该指令，然后执行该指令。整个过程被称为`指令周期`。
...
*   处理器的`程序计数器`寄存器给出了需要从存储器中取出的指令的地址。
...
*   在解码或执行过程中，由于指令和存储段是4字节的，所以并行地，`PC`递增4(即它包含下一条指令的地址)。所以地址`I`的指令被执行。
*   所以每次，`PC`都会加4。因此，程序是以顺序方式执行的。这个过程叫做直线排序。
...
*   之后，寄存器`R0`被清零。
*   地址`LOOP`被一次又一次地重新加载，直到`R1`变成0(这意味着所有的数字都被加上)。每次添加一个数字，`R1`值就会递减。
*   当`R1`为0时，我们退出循环，存储在`R0`的结果被复制到内存位置`SUM`。
