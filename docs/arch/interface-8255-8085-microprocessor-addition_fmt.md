...
> 原文: [https://www.geeksforgeeks.org/interface-8255-8085-microprocessor-addition/](https://www.geeksforgeeks.org/interface-8255-8085-microprocessor-addition/)

## 问题
将 `8255` 与 `8085` 微处理器接口，编写一个确定 `A` 口和 `B` 口内容相加的汇编程序，并将结果存入 `c` 口。
...
1.  `MVI A, 92` 表示控制寄存器的值为 `92`。
    ...
    D1=1 as Port B is taking the input.
    ```
2.  `OUT 83` 将 `A` 的值放入端口控制寄存器的端口号 `83H`。
3.  `IN 80` 从端口 `a` 的端口号 `80H` 取输入。
4.  `MOV B, A` 将 `A` 寄存器的内容复制到 `B` 寄存器。
5.  `IN 81` 输入端口 `b` 的端口号 `feom 81H`。
6.  `ADD B` 添加 `A` 寄存器和 `B` 寄存器的内容。
