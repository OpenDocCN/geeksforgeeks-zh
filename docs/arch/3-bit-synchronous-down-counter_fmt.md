...
> 原文：[https://www.geeksforgeeks.org/3-bit-synchronous-down-counter/](https://www.geeksforgeeks.org/3-bit-synchronous-down-counter/)
...
**这里 `T = 1`，则有输出状态（下一个状态从前一个状态改变）改变，即 `Q` 从 0 变为 1 或 1 变为 0。`T= 0` 则没有状态输出状态改变，即 `Q` 保持不变。**
...
时钟在同一时刻提供给每个触发器。根据 K 图的简化方程，将触发（`T`）输入提供给每个触发器。
...
*   最初 `Q3 = 0`，`Q2 = 0`，`Q1 = 0`。
*   在 K 图的简化方程中，我们得到 `T1 = 1`，因此触发器 1 输出 `Q1` 在每个负沿被触发（因为时钟是负沿触发的）。触发器（`FF`）2 触发输入（`T2`）连接到 `Q'1`。因此，触发器 2 输出状态 `Q2` 只有在有时钟下降沿（即负沿触发）且 `Q’1= 1` 时才被触发。
*   类似地，触发器 3 的触发输入（`T`）连接到 `Q’2` 和 `Q’1`。因此，当存在时钟下降沿且 `Q’2 = 1` 且 `Q’1 = 1` 时，触发器 3 的输出被触发。（从时序图中可以看出）
*   因此，在第 8 个负边沿触发时钟后，我们得到输出（随着 `Q3(MSB) Q2 Q1(LSB)` 递减计数），三个触发器的输出再次变为 `Q3 = 0`，`Q2 = 0`，`Q1 =0`。
*   我们在每隔一个边沿时钟脉冲后获得输出（状态变化）。
*   通过 3 个触发器，我们得到输出为 2<sup>3</sup> - 1 = 7 到 0。