...
> 原文:[https://www.geeksforgeeks.org/8086-program-transfer-block-bytes-using-string-instruction/](https://www.geeksforgeeks.org/8086-program-transfer-block-bytes-using-string-instruction/)

## 问题–
编写一个汇编语言程序，通过使用字符串指令将一个字节块从一个存储位置传输到另一个存储位置。
...
## 假设–
*   指示要传输的字节数的计数器值存储在偏移量`500`处。
*   从`501`开始，必须传输`8`位数据被存储在连续的存储单元中。
*   数据从`600`开始被传送到连续的存储位置。
*   `DS`和`ES`的值取等于`0000`。
...
| 0412 | `INC SI` | SI |
| 0413 | `CLD` | 清除方向标志 |
| 0414 | `REP` | 重复直到 CX 不等于零并且 CX=CX-1 |
| 0415 | `MOVSB` | 将数据从源存储位置传输到目标存储位置 |
| 0416 | `HLT` | 结束 |
