...
> 原文:[https://www . geeksforgeks . org/同步和异步计数器之间的差异/](https://www.geeksforgeeks.org/differences-between-synchronous-and-asynchronous-counter/)

[`计数器`](https://www.geeksforgeeks.org/counters-in-digital-logic/)根据所施加的时钟脉冲有两种类型。这些计数器是:`异步计数器`和`同步计数器`。
在`异步计数器`也叫[波纹计数器](https://www.geeksforgeeks.org/digital-logic-ripple-counter/)中，不同的触发器用不同的时钟触发，而不是同时触发。而在`同步计数器`中，所有触发器同时用相同的时钟触发，`同步计数器`比`异步计数器`运行更快。
