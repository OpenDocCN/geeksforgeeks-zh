...
> 原文: [https://www.geeksforgeeks.org/difference-between-memory-mapped-io-and-io-mapped-io-with-reference-to-8085-microprocessor/](https://www.geeksforgeeks.org/difference-between-memory-mapped-io-and-io-mapped-io-with-reference-to-8085-microprocessor/)
...
| 使用的说明 | 使用的指令有 `LDA` 和 `STA` 等。 | 使用的指令有 `输入` 和 `输出`。 |
...
| 输入输出信号 | 在内存映射输入输出的情况下，在写入或读取周期期间(`IO/`M`= 0`)。 | 在输入输出映射输入输出的情况下，在写或读周期期间(`IO/`M`= 1`)。 |
