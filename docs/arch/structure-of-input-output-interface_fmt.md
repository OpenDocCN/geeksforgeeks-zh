...
该模块为整个设备操作产生必要的控制信号。来自中央处理器的所有命令都通过这个块被接受。它还允许接口单元的状态通过这个块被传送到数据总线上，接受来自系统总线的 `CS`、读写控制信号和来自系统地址总线的 `S₀`，`S₁`。读取和写入信号用于定义数据总线上的数据传输方向。
...
地址总线用于选择接口单元。地址总线的两条最低有效线(`A₀`、`A₁`)连接到选择线 `S₀`、`S₁`。这两条选择输入线用于选择接口单元中四个寄存器中的任何一个。接口单元的选择依据以下标准:
...
*   如果 `S₀`，`S₁` = `0 1`，则选择端口 `B` 数据寄存器用于 `CPU` 和 `I/O` 设备之间的数据传输。
*   如果 `S₀`，`S₁` = `1 0`，则选择控制寄存器，存储中央处理器发送的控制信息。