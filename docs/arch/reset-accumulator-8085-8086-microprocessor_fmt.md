...
2.  `AND AX, 0000`: 操作逐位执行，将源操作数(`0000`)与目标操作数进行 `AND` 运算，并将结果存储在 `AX` 中。
3.  `XOR AX, AX`: 对源寄存器和目的寄存器的值进行异或运算，并将结果存储在 `AX` 中。源操作数和目标操作数是相同的。因此，`AX = 0`。
