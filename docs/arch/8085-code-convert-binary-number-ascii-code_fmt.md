...
> 原文: [https://www.geeksforgeeks.org/8085-code-convert-binary-number-ascii-code/](https://www.geeksforgeeks.org/8085-code-convert-binary-number-ascii-code/)

## 问题
8085 中的汇编级程序，将二进制数转换为 ASCII 数。

## 示例
![](img/267b12c1fa615ddc665bc2ab0bb09bef.png)

## 假设
必须转换为 ASCII 值的二进制数存储在存储位置 `2050`，输出将显示在存储位置 `3050` 和 `3051`。
...
9.  `CALL 2500`: 该指令将在它之后停止执行主例程指令，并且将移动到用于执行子任务的子例程地址 `2500`，并且在执行子例程指令之后，它将回到主例程并且在 `CALL 2500` 之后执行指令。
10. `STA 3051`: 该指令将累加器的结果(在子程序中执行)存储到地址 `3051`。
11. `HLT`: 该指令将终止程序。
...
6.  `ADI 37`: 将累加器的内容加 `37`，再次将结果存储回累加器。
