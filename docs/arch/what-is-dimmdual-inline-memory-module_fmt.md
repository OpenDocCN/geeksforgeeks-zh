...
在 `SIMM` 的情况下，连接器只出现在模块的一侧，并且短接在一起。`SIMM`s 总是以匹配对的形式使用。`SIMM` 提供的最大数据存储量为 32 位/周期，电压消耗为 5 伏。
...
*   **基于内存类型的内存分类:**
    ...
    *   **`DDR`(双倍数据速率)`DIMM`:** 每个时钟周期访问两次数据包。`DDR DIMM`s 还在每侧使用两个槽口，以支持与低轮廓和高轮廓闩锁插座的兼容性。
