...
> 原文：[https://www.geeksforgeeks.org/call-instructions-and-stack-in-avr-microcontroller/](https://www.geeksforgeeks.org/call-instructions-and-stack-in-avr-microcontroller/)
...
在 I/O 内存空间中，有 2 个寄存器分别为 `SPL` (`SP` 的低字节)和 `SPH` (即 `SP` 的高字节)。`SP` 由这两个寄存器实现。
