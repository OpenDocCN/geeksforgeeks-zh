...
> 原文：[https://www.geeksforgeeks.org/applications-of-microprogrammed-control-unit/](https://www.geeksforgeeks.org/applications-of-microprogrammed-control-unit/)
...
*   这意味着每条指令都需要一组微指令。
*   一组微指令被称为微程序。
*   所有指令的微程序都存储在一个叫做控制存储器的小存储器中。控制存储器位于处理器内部。
...
## 微程序控制单元的应用
微程序具有灵活性、简单性、成本效益等优点。因此，它在以下应用中具有重大贡献：
...
### 4. 仿真
仿真是指让一个处理器(比如说 A)仿真另一个处理器(比如说 B)或表现得像另一个处理器。要做到这一点，A 必须能够执行 B 的指令。如果我们对 A 的控制存储器重新编程，与 B 的相同，那么 A 将能够对每条指令模拟 B 的行为。这只有在微程序控制单元中才有可能。通常在主处理器必须模拟数学协处理器的行为时使用。
...
### 7. 专用处理器的开发
所有的处理器都不是通用的。许多应用需要专用处理器，如用于通信的`DSP`(数字信号处理器)或[`GPU`](https://www.geeksforgeeks.org/what-is-a-graphics-card/)(图形处理器单元)用于图像处理。它们指令集复杂，也需要不断升级。微程序控制单元是他们的最佳选择。