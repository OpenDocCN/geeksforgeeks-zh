...
> 原文:[https://www.geeksforgeeks.org/design-of-512x8-ram-using-128x8-ram/](https://www.geeksforgeeks.org/design-of-512x8-ram-using-128x8-ram/)
...
如果我们有 `n` 位地址和 `m` 位字，那么我们的内存大小将是 `2^n x m`
...
4.  另外，我们可以把 128×8 的 `RAM` 芯片写成 `2^7×8`，每个 `RAM` 芯片都需要一个 7 位地址。我们将把剩下的 7 位地址线连接到每个内存。
