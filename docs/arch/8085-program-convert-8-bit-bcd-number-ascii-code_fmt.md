...
> 原文: [https://www.geeksforgeeks.org/8085-program-convert-8-bit-bcd-number-ascii-code/](https://www.geeksforgeeks.org/8085-program-convert-8-bit-bcd-number-ascii-code/)

## 问题
编写一个汇编级语言程序，将 8 位 BCD 号转换为其各自的 ACSII 代码。
...
1. 在累加器中输入 `2050` 的内容
2. 将累加器的内容移到寄存器 `B`
3. 使用 `ANI` 和 `0F` 将最低有效数字分开，并将 `30` 加到累加器上
4. 将累加器的内容存储到存储器位置 `3050`
5. 将寄存器 `B` 的内容移至累加器
6. 用 `ANI` 和 `F0` 分隔最高有效数字
7. 将累加器的内容旋转 4 次
8. 向累加器添加 `30`
