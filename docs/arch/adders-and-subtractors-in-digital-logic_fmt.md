...
> 原文：[https://www.geeksforgeeks.org/adders-and-subtractors-in-digital-logic/](https://www.geeksforgeeks.org/adders-and-subtractors-in-digital-logic/)
...
1+1=10
```
`1+1` 的结果是 `10`，其中“1”是进位输出(`Cout`)，“0”是和输出(正常输出)。
...
*   它包含三个输入(在中为 `A`、`B`、`Cin`)并产生两个输出(在中为 Sum 和 `Cout`)。
*   其中，`Cin` -> 运入，`Cout` -> 运出
...
*   它包含三个输入(在中为 `A`、`B`、`Bin`，产生两个输出(在中为 `D`、`Bout`)。
