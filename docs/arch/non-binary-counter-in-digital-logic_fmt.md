...
> 原文: [https://www.geeksforgeeks.org/non-binary-counter-in-digital-logic/](https://www.geeksforgeeks.org/non-binary-counter-in-digital-logic/)
...
为了设计一个非二进制计数器，需要一个逻辑门来检测 `M` 级。在 `mod-10 counter` 中，从 `0000` 到 `1001` 检测到 10 级，一旦 `1010` 出现，它就会清除所有触发器。
