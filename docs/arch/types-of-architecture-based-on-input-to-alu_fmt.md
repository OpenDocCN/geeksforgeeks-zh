...
> 原文：[https://www.geeksforgeeks.org/types-of-architecture-based-on-input-to-alu/](https://www.geeksforgeeks.org/types-of-architecture-based-on-input-to-alu/)
...
### 1. 基于累加器的体系结构
在这种体系结构中，算术逻辑单元的一个输入操作数来自累加器，另一个输入操作数来自寄存器、存储器等。获得输入操作数后，算术逻辑单元对操作数进行运算，结果将存储在累加器中。
...
我们还根据输入操作数的最大大小划分体系结构。例如，基于处理器字长的 32 位架构、64 位架构。