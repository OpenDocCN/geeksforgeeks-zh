...
> 原文: [https://www.geeksforgeeks.org/size-of-rom-for-n-bit-adder-subtractor/](https://www.geeksforgeeks.org/size-of-rom-for-n-bit-adder-subtractor/)
...
如图所示，具有 `n` 位地址和 `m` 位数据的存储单元阵列将具有 $2^n$ 行和 `m` 列。每行数据被称为一个字。因此，该数组包含一个 $2^n$ 字 × `m` 位数组。

在图的情况下，具有 2 位地址和 3 位数据的存储单元阵列将具有 $2^2$ 行和 3 列，因此该阵列包含 $2^n$ × `m` = $2^2$ × 3 = 4 字 × 3 位阵列。
...
因此，这将需要一个 $2^{(2n+1)} \times (n+1)$ 位 ROM。

这里，`n(A)` 表示 A 的输入数，`n(B)` 表示 B 的输入数。

## 示例-1

可用于为带有 `Cin` 和 `Cout` 的 16 位加法器/减法器编程的只读存储器的大小是 ____？
...
可用于为带有 `Cin` 和 `Cout` 的 8 位加法器/减法器编程的只读存储器的大小是 ____？
