...
首先，我们将采取一个小的汇编语言程序来理解它们各自通行证的工作。汇编语言语句格式：
...
1.  **START：** 该指令从位置 200 开始执行程序，带有 `START` 的标签为程序提供名称。（JOHN 是项目名称）
...
**步骤 1：`START 200`** （这里没有找到符号或文字，所以两个表都是空的）
...
现在，由 Pass-1 生成的表连同它们的 LC 值将进入汇编程序的 Pass-2，以进一步处理伪操作码和机器操作码。
