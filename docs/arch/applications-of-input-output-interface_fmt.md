...
## I/O 接口的各种应用:
I/O 的应用就是我们可以说接口有权限打开任何文件而没有任何一种关于文件的信息即即使文件的基本信息也是未知的。它还有一个特点，即使不会对操作系统造成任何中断，也可以用来给计算机系统增加新的设备。它还可以通过识别一般类型来抽象输入/输出设备的差异。对每种通用类型的访问都是通过一组标准化的功能进行的，称为接口。
