...
> 原文:[https://www.geeksforgeeks.org/c-program-to-implement-full-subtractor/](https://www.geeksforgeeks.org/c-program-to-implement-full-subtractor/)

## 先决条件
[全减法器](https://www.geeksforgeeks.org/full-subtractor-in-digital-logic/)
给定全减法器 `A`、`B`、`Bin` 的三个输入。任务是实现全减法器电路并打印三个输入的输出状态差(`D`)和布特(`Bout`)。
...
> B- Out = `āBin+āb+ B Bin` 即 `1 AND 1 + 1 AND 0 + 0 AND 1 = 1`。
...
*   我们将接受三个输入 `A`、`B`、`Bin`。
*   通过应用异或(`XOR`)给出差值 `D`。
*   通过应用 `āBin+āb+ B Bin` 给出了 `B-Out` 的值。
