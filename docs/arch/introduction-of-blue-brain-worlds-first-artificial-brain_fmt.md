...
> 原文: [https://www.geeksforgeeks.org/introduction-of-blue-brain-worlds-first-artificial-brain/](https://www.geeksforgeeks.org/introduction-of-blue-brain-worlds-first-artificial-brain/)
...
计算机之间的连接只通过使用纳米机器人作为接口形成，使用的软件被称为`BBP-SDK`，这是一个用`C++`库编写的软件开发工具包，用`Java`和`Python`包装。`BBP-SDK`从纳米机器人收集数据，收集数据的可视化是使用一个名为“实时神经元”的数据可视化软件完成的，该软件是用`C++`编写的，专门为神经元模拟的三维可视化而创建的。

最终数据存储在数据库中，并将用于人脑功能的进一步建模。如果这个实验成功了，那么大脑的知识就可以被储存和检查。

蓝色大脑项目是开源的，在`GitHub`上对所有人开放。
