...
> 原文：[https://www.geeksforgeeks.org/stack-machine-in-computer-organisation/](https://www.geeksforgeeks.org/stack-machine-in-computer-organisation/)
...
在堆垛机中，数据默认位于栈顶。堆栈充当源和目的地，`push`和`pop`指令用于从堆栈中访问指令和数据。不需要传递源地址和目的地址，因为默认地址在堆栈顶部。在堆栈机器中，不需要在指令中传递显式地址。因此[指令格式仅由操作码（opcode）字段组成](https://www.geeksforgeeks.org/introduction-of-single-accumulator-based-cpu-organization/)。这种指令格式被称为零地址指令。

堆栈的两个操作是插入（`push`）和删除（`pop`）堆栈中的项目。但是，在计算机堆栈中不会推送或弹出任何内容。
