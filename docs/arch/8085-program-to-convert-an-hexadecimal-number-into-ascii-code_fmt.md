...
> 原文:[https://www . geeksforgeeks . org/8085-程序-将一个十六进制数转换为 ascii 码/](https://www.geeksforgeeks.org/8085-program-to-convert-an-hexadecimal-number-into-ascii-code/)

## 问题
编写汇编级语言程序，将 HEX 代码转换为各自的 ASCII 代码。
...
| Two thousand and three | `MOV B, A` | 将累加器的内容移到 B |
| Two thousand and four | `ANI 0FH` | 屏蔽上半字节 |
| Two thousand and six | `CALL SUB1` | 获取高位半字节的 ascii 码 |
| Two thousand and nine | `STA 2051H` |  |
| Two thousand and twelve | `MOV A, B` | 将内容移动到累加器 |
| Two thousand and thirteen | `ANI F0H` | 屏蔽下半字节 |
| Two thousand and fifteen | `RLC` |  |
| Two thousand and sixteen | `RLC` |  |
| Two thousand and seventeen | `RLC` |  |
| Two thousand and eighteen | `RLC` |  |
| Two thousand and nineteen | `CALL SUB1` | 获取低位半字节的 ascii 码 |
| Two thousand and twenty-two | `STA 2052H` |  |
...
3.  `ANI 0FH`: 在累加器和 `0FH` 值之间执行“与”运算。
4.  `CALL SUB1`: 调用内存位置 `SUB1` 的子程序。
