...
> 原文：[https://www.geeksforgeeks.org/computer-organization-asynchronous-input-output-synchronization/](https://www.geeksforgeeks.org/computer-organization-asynchronous-input-output-synchronization/)
...
1.  **Source initiated Strobe –** 当源启动数据传输过程时。选通只是一个信号。

![](img/e08166ad3a7e6cd552ec158759b557de.png)

(i) 首先，源将数据放在数据总线上，并开启选通信号。
(ii) 目的地在看到选通脉冲的接通信号时，从数据总线读取数据。
(iii) 目的地从数据总线读取数据后，选通关闭。
...
1.  **Source initiated Handshaking –** 当源启动数据传输过程时。它由以下信号组成：
    `DATA VALID`：如果为 ON，表示数据总线上的数据有效，否则无效。
    `DATA ACCEPTED`：如果为 ON，表示数据已被接受，否则未被接受。
...
2.  **Destination initiated Handshaking –** 当目的地启动数据传输过程时。
    `REQUEST FOR DATA`：如果为 ON，请求将数据放到数据总线上。
    `DATA VALID`：如果为 ON，表示数据总线上的数据有效，否则数据无效。
