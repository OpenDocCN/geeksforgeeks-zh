...
*   `ICW1` 和 `ICW2` 命令是初始化的强制命令。
*   `ICW3` 命令在级联配置期间给出。
*   如果需要 `ICW4` ，则在 `ICW1` 中指定。
*   发出 `ICW` 命令的顺序是固定的，即先发出 `ICW1` ，然后发出 `ICW2` ，再发出 `ICW3` 。
...
*   当 `A0` = 0，`D4` = 1 时，控制字识别为 `ICW1` 。
*   它具有边缘和电平触发模式、单/级联模式、呼叫地址间隔以及是否需要 `ICW4` 的控制位。
*   地址线 `A7` 至 `A5` 用于中断向量地址。
...
*   `IR7` 被指定为优先级 7。
*   从机模式地址分配为 7。
*   当 `D0` = 0 时，表示不需要 `ICW4` 命令。因此，`ICW4` 中使用的功能被重置。
*   特殊屏蔽模式被重置，状态读取被分配给内部收益率。

### `ICW2` 命令

*   当 `A0` = 1 时，控制字识别为 `ICW2` 。
*   它存储关于中断向量地址的信息。
*   在基于 8085 的系统中，控制字的 `A15` 至 `A8` 位用于中断向量地址。
*   在基于 8086 的系统中，插入 `T6` 至 `T3` 位代替 `A15` 至 `A8` 和 `A10` 至 `A8` 用于选择中断级别，即 `000` 用于 `IR0` 和 `111` 用于 `IR7` 。
...
`ICW3` 命令字在系统中存在多个 8259 时使用，即当 `ICW` 的 `SNGL` 位为 0 时，将加载 8 位从寄存器。
...
用于设置和复位 `IMR`(中断屏蔽寄存器)中的屏蔽位。`M7`–`M0`描述 8 个掩码位

![](img/bb4ca59708d0465875ae41b9650e3d3d.png)

### `OCW2`

用于选择 8259 的工作模式。这里 `L2` 到 `L0` 用来描述需要执行动作的中断级别。
