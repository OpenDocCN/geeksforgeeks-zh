...
> 原文: [https://www.geeksforgeeks.org/8085-program-swap-two-8-bit-numbers/](https://www.geeksforgeeks.org/8085-program-swap-two-8-bit-numbers/)

## 问题
编写汇编语言程序，交换 8085 微处理器中存储的两个 8 位数字。

## 假设
假设有两个 8 位数字。一个 8 位数字存储在位置 `2500` 存储器地址，另一个存储在位置 `2501` 存储器地址。`05` 存储在位置 `2500`，`06` 存储在位置 `2501`（不一定，可以是任意两个 8 位数字）。
...
| 地址 | 记忆术 | 评论 |
| --- | --- | --- |
| `2000` | `LDA 2500` | `A <- M[2500]` |
| `2003` | `MOV B, A` | `B <- A` |
| `2004` | `LDA 2501` | `A <- M[2501]` |
| `2007` | `STA 2500` | `M[2500] <- A` |
| `200A` | `MOV A, B` | `A <- B` |
| `200B` | `STA 2501` | `M[2501] <- A` |
| `200E` | `HLT` | 终止程序 |
