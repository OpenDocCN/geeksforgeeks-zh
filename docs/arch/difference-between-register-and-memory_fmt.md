...
> 原文:[https://www . geeksforgeks . org/register-and-memory 之差/](https://www.geeksforgeeks.org/difference-between-register-and-memory/)
...
## 2. 内存
内存是用于存储计算机程序、指令和数据的硬件设备。处理器内部的内存是主内存(`RAM`)，处理器外部的内存是`辅助内存(硬盘)`。存储器也可以根据易失性和非易失性存储器来分类。易失性存储器是当计算机或硬件设备断电时丢失其内容的存储器。`随机存取存储器`是易失性存储器的一个例子。非易失性存储器是即使断电也能保存其内容的存储器。`EPROM`是非易失性存储器的一个例子。
