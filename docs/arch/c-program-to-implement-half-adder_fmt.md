...
> 原文: [https://www.geeksforgeeks.org/c-program-to-implement-half-adder/](https://www.geeksforgeeks.org/c-program-to-implement-half-adder/)
...
说明: 这里从逻辑表达式 `Sum = A XOR B` 即 `0 XOR 0 = 0`，`Carry = A AND B` 即 `0 AND 0 = 0`。

输入: `A=1`，`B=0`

输出: `Sum=1`，`Carry=0`

说明: 这里从逻辑表达式 `Sum = A XOR B` 即 `1 XOR 0 = 1`，`Carry = A AND B` 即 `1 AND 0 = 0`。
