# 分时总线、交叉开关与多端口存储器 – 互连结构之间的差异

> 原文：`https://www.geeksforgeeks.org/difference-between-time-shared-bus-crossbar-switch-multiport-memory-interconnection-structure/`

## 前提
详细研究[时间共享总线](https://write.geeksforgeeks.org/post/2699988)、[纵横制交换机](https://www.geeksforgeeks.org/crossbar-switch/)与[多端口存储器](https://www.geeksforgeeks.org/multiport-memory-multiprocessor-system/)互联结构。

## 简介
互联结构可以决定多处理器环境下系统的整体性能。处理器必须能够在多处理器系统中共享一组主内存模块与输入/输出设备。这种共享能力可以通过互连结构来提供。常用的互连结构可以给出如下：
...
## 多端口存储器
在多端口存储器系统中，控制、交换与优先级仲裁逻辑分布在交叉开关矩阵中，交叉开关矩阵分布在存储器模块的接口处。

![](img/9ba53477b0abf679c00c92584d14b362.png)

多端口存储器

## 分时总线、纵横制交换机与多端口存储器之间的差异
...
| 6. | 最低效率且仅适用于较小的系统。 | 功能单元是最简单和最便宜的。 | 功能单元允许低成本的单处理器。 |