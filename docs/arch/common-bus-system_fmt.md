...
4 个寄存器`DR`、`AC`、`IR`和`TR`为 16 位，2 个寄存器`AR`和`PC`为 12 位。`INPR`和`OUTR`各有 8 位。`INPR`从输入设备接收字符并将其传送到`AC`，而`OUTR`从`AC`接收字符并将其传送到输出设备。5 个寄存器有 3 个控制输入`LD`(负载)、`INR`(增量)和`CLR`(清零)。这些类型的寄存器类似于二进制计数器。
...
加法器和逻辑电路提供`AC`的 16 路输入。该电路有 3 组输入。一组来自实现寄存器微操作的`AC`输出。另一组来自用于执行算术和逻辑微操作的数据寄存器`DR`。这些操作的结果被发送到`AC`，而进位的结束存储在`E`中，如图所示。第三组输入来自`INPR`。
