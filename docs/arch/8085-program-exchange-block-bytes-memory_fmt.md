...
> 原文：[https://www.geeksforgeeks.org/8085-program-exchange-block-bytes-memory/](https://www.geeksforgeeks.org/8085-program-exchange-block-bytes-memory/)

## 问题
在 8085 微处理器中编写汇编级程序，用从地址 `3001` 开始的数据交换从地址 `2001` 开始的 4 字节块。
...
| 250F     | DCR C        | C <= C–1                 |
| 2510     | JNZ 2508     | 如果 C 不等于 0，跳到 2508 |
| 2513     | HLT          | 停止程序                 |
```
...
2.  `MVI C 04` – 分配即时数据，例如-此处 `C = 04`
    `MVI A 45` – 分配 45 的 `A`(累加器)，`A=45`
...
6.  `STAX D` – 将 `A`(累加器)的内容存储在由寄存器对 `D-E` 构成的地址中
7.  `INX H` – 递增寄存器对 `H-L` 的内容
8.  `INX D` – 递增寄存器对 `D-E` 的内容
9.  `DCR C` – 递减寄存器 `C` 的内容
