...
## 问题–
在 8086 微处理器中编写程序，生成 n 个八位数字的算术级数(AP)。
...
6.  `INC SI`: `SI` 值增加 `1`。
7.  `MOV DI, 600`: 将 `DI` 的值设置为 `600`。
8.  `MOV [DI], AL`: 在偏移 `DI` 存储寄存器 `AL` 的值。
9.  `INC DI`: `DI` 值增加 `1`。
10. `DEC CL`: 将寄存器 `CL` 的值减少 `1`。
11. `MOV BL, [SI]`: 从偏移 `SI` 加载值到寄存器 `BL`。
12. `ADD AL, BL`: 将寄存器 `AL` 的值加 `BL`。
13. `MOV [DI], AL`: 在偏移 `DI` 存储寄存器 `AL` 的值。
14. `INC DI`: `DI` 值增加 `1`。
