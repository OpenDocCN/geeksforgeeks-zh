...
> 原文：[https://www.geeksforgeeks.org/register-organization-in-risc-cpu/](https://www.geeksforgeeks.org/register-organization-in-risc-cpu/)

## RISC 处理器

它们是使用小指令集和简单寻址模式的处理器，因此它们的指令可以在 CPU 内更快地执行，对内存的引用更少。这种类型的处理器被归类为精简指令集计算机（RISC）。

要详细了解 RISC 处理器及其特性，请参考[本文](https://www.geeksforgeeks.org/computer-organization-risc-and-cisc/)。
...
| 指令 | 评论 |
| :--- | :--- |
| `R1 LOAD A` | 处理器寄存器 `R1` 加载有存储单元 `A` 的内容 |
| `R2 LOAD B` | 处理器寄存器 `R2` 加载了存储单元 `B` 的内容 |
| `R3 LOAD C` | 处理器寄存器 `R3` 加载有存储单元 `C` 的内容 |
| `R4 LOAD D` | 处理器寄存器 `R4` 加载了存储单元 `D` 的内容 |
| `MUL R1, R1, R2` | `R1` 和 `R2` 寄存器的内容相乘，结果存储在 `R1` 寄存器中（即执行 `A*B`） |
| `MUL R3, R3, R4` | `R3` 和 `R4` 寄存器的内容相乘，结果存储在 `R1` 寄存器中（即执行 `C*D`） |
| `ADD R1, R1, R3` | 将寄存器 `R1` 和 `R3` 的内容相加，结果存储在 `R1` 寄存器中（即 `(A*B) + (C*D)`） |
| `STORE X, R1` | `R1` 寄存器的内容存储在存储单元 `X` 中 |
...
![RISC CPU 的重叠寄存器窗口](img/69279afaf073e5947528f6dda730e7cb.png)
