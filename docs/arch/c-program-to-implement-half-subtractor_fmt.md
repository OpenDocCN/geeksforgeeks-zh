...
> 原文: [https://www.geeksforgeeks.org/c-program-to-implement-half-subtractor/](https://www.geeksforgeeks.org/c-program-to-implement-half-subtractor/)
...
差值由 `A` 和 `B` 的异或运算计算，借位由 `Ā` 和 `B` 的与运算计算。这里 `Ā` 是给出逆输出的 `A` 的非运算。
...
*   首先，我们将接受两个输入
*   通过应用异或我们得到差值。
*   通过应用 `NOT A`，我们得到了 `Ā` 的值。
*   通过应用与我们得到了 `Borrow` 的值。
