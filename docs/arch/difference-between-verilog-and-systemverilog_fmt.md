...
> 原文: [https://www.geeksforgeeks.org/difference-between-verilog-and-systemverilog/](https://www.geeksforgeeks.org/difference-between-verilog-and-systemverilog/)
...
## 2. SystemVerilog
`SystemVerilog` 是硬件描述语言（`HDL`）和硬件验证语言（`HVL`）的组合，组合起来称为 `HDVL`。意味着它描述电子电路的结构和行为，并验证用硬件描述语言编写的电子电路。`SystemVerilog` 作为 `Verilog` 的超集，在 2005 年对 `Verilog` 语言进行了大量扩展，成为 `IEEE` 标准 `1800`，并在 2012 年再次更新为 `IEEE 1800-2012` 标准。`SystemVerilog` 基于类级测试平台，本质上更具动态性。

## Verilog 和 SystemVerilog 的区别

| 编号 | Verilog | SystemVerilog |
| :--- | :--- | :--- |
| 01. | `Verilog` 是一种硬件描述语言。 | `SystemVerilog` 是硬件描述语言和硬件验证语言（`HVL`）的结合。 |
