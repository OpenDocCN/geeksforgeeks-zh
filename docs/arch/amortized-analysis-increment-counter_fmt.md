...
> 原文：[https://www.geeksforgeeks.org/amortized-analysis-increment-counter/](https://www.geeksforgeeks.org/amortized-analysis-increment-counter/)
...
让变量 `i` 包含值 0，我们执行 `i++` 多次。因为在硬件上，每个操作都是以二进制形式执行的。让二进制数存储在 8 位。所以，值是 `00000000`。让我们增加很多次。所以，我们找到的模式是：
