...
`8086`内部没有内存或只读存储器。但是，它有内部寄存器，用于存储中间和最终结果，并通过系统总线与位于其外部的存储器接口。

在`8086`的情况下，它是40针双列直插式封装集成电路中的16位整数处理器。

内部寄存器(存在于芯片内)的大小指示处理器一次可以处理多少信息(在本例中为16位寄存器)以及它如何在芯片内部移动数据，有时也称为内部数据总线。

`8086`为程序员提供了14个内部寄存器，每个寄存器16位或2字节宽。

## 记忆分割

*   为了提高执行速度和获取速度，`8086`对内存进行分段。
*   它的20位地址总线可以寻址1MB的内存，它把它分成16个64kB的段。
*   `8086`仅适用于整个1MB内存中的四个64KB数据段。
...
`BIU`主要包含4段寄存器、指令指针、预取队列和地址生成电路。

### 指令指针(`IP`)

*   它是一个16位寄存器。它保存代码段中下一个指令的偏移量。
*   提取每个指令字节后，`IP`递增。
*   每当分支指令发生时，`IP`都会获取一个新值。
*   `CS`乘以`10H`得到代码段的20位物理地址。
...
`欧盟`的主要组成部分是通用寄存器、算术逻辑单元、专用寄存器、指令寄存器和指令解码器以及标志/状态寄存器。
...
`8086`有四个16位通用寄存器`AX`、`BX`、`CX`和`DX`。执行期间存储中间值。每个都有两个8位部分(较高和较低)。
