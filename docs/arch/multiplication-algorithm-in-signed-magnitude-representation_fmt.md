...
> 原文：[https://www.geeksforgeeks.org/multiplication-algorithm-in-signed-magnitude-representation/](https://www.geeksforgeeks.org/multiplication-algorithm-in-signed-magnitude-representation/)
...
2.  寄存器 `B` (`Bs`) 和 `Q` (`Qs`) 的符号使用 **XOR** 功能进行比较（即，如果两个符号相同，XOR 操作的输出为 0，否则为 1），输出存储在 `As`（`A` 寄存器的符号）中。
