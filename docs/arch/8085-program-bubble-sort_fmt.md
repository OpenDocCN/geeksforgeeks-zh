...
## 问题–
在 8085 微处理器中编写一个汇编语言程序，使用冒泡排序对给定的 n 个数进行排序。

## 示例–
![](img/b978531aa6b1a79a2809e8fc38017c20.png)

## 假设–
列表大小存储在`2040`小时，列表编号从`2041`小时开始。
...
| `2006H` |   | `DCR C` | 减量`C` |
| `2007H` |   | `INX H` | 增加内存以访问列表 |
| `2008H` | `支票` | `莫夫 a，m` | 在累加器中检索列表元素 |
| `2009H` |   | `INX H` | 增加内存以访问下一个元素 |
| `200AH` |   | `CMP M` | 比较累加器和下一个元素 |
| `200BH` |   | `jc nextbyte` | 如果累加器较少，则跳转到`NEXTBYTE` |
...
| `2018H` | `NEXTBYTE` | `DCR C` | 为下一次迭代递减`C` |
| `2019H` |   | `JNZ 检查` | 如果`C>0`，跳转到`检查` |
| `201CH` |   | `莫夫 a，d` | 将数据内容转移到累加器 |
| `201DH` |   | `消费物价指数 01H` | 将累加器内容与`01H`进行比较 |
| `201FH` |   | `JZ 开始` | 如果`D=01H`，跳到`开始` |
| `2022H` |   | `HLT` | 停止 |
