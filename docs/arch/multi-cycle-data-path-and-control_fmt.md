...
> 原文: [https://www.geeksforgeeks.org/multi-cycle-data-path-and-control/](https://www.geeksforgeeks.org/multi-cycle-data-path-and-control/)

## 概述

多周期数据路径将指令分解成单独的步骤。它减少了平均指令时间。每个步骤需要一个时钟周期。每个功能单元在一条指令中可以使用不止一次，只要它在不同的时钟周期中使用。它减少了所需的硬件数量。
...
**Q1。** 查找添加 R1、R2、R3 的数据路径和控制阶段 (执行顺序)；表示 `R3<–R1+R2`

解决方案: 给出指令 `–添加 R3、R1、R2；`

*   阶段 1: 获取指令并增加程序计数器。
*   阶段 2: 解码以确定它是加法指令，并读取寄存器 R1 和 R2。
*   阶段 3: 将阶段 2 中检索到的两个值相加。
*   第四阶段: 空闲 (没有东西可以写入内存)
*   阶段 5: 将阶段 3 的结果写入寄存器 R3。

**Q2。** 查找 `R2 MOV 30(R1)` 的数据路径和控制阶段 (执行顺序)；表示 `【30+R1】<–R2`

解决方案: 给定指令 `R2 MOV 30(R1)；`

*   阶段 1 (提取) : 提取指令并增加程序计数器。
*   阶段 2 (解码) : 解码以找到相应的指令是 MOV，并读取寄存器 R1 和 R2
*   阶段 3 (执行) : 在相应的 R1 寄存器中添加 30。
*   第 4 阶段 (内存访问) : 将寄存器 R2 中的值写入第 3 阶段计算的内存地址。
*   阶段 5 (寄存器写) : 该步骤是空闲的 (寄存器中没有要写的内容)