...
> 原文: [https://www.geeksforgeeks.org/time-shared-bus-interconnection-structure-in-multiprocessor-system/](https://www.geeksforgeeks.org/time-shared-bus-interconnection-structure-in-multiprocessor-system/)
...
如果有冲突，我们可以使用 `总线控制器` 来解决。(`总线控制器` 可以设置不同功能单元的优先级)
...
*   **吞吐量下降** — 一次只有一个处理器可以与任何其他功能单元通信。
*   **增加了仲裁逻辑** — 随着处理器 & 内存单元数量的增加，总线争用问题也随之增加。
...
这是一个二进制 `n` 立方体架构。这里我们可以连接 `2^n` 个处理器，这里的每个处理器形成立方体的一个节点。一个节点可以是内存模块，也可以是输入输出接口，不一定是处理器。节点处的处理器具有直接通向 `n` 个其他节点 (总共 `2^n` 个节点) 的通信路径。总共有 `2^n` 个不同的 `n` 位二进制地址。
