...
> 原文: [https://www.geeksforgeeks.org/8085-program-find-nth-power-number/](https://www.geeksforgeeks.org/8085-program-find-nth-power-number/)

## 问题
用 8085 微处理器编写一个计算一个数的 n 次方的汇编语言代码。
...
| `2002H` | `RESULT` |  | 阶乘结果 |
|  |  |  |  |
| `2003H` |  | `LXI H, 2000H` | 从内存中加载数据 |
| `2006H` |  | `MOV B, M` | 将基数加载到 `B` 寄存器 |
| `2007H` |  | `INX H` | 增量内存 |
| `2008H` |  | `MOV C, M` | 将指数载入 `C` 寄存器 |
| `2009H` |  | `MVI D, 01H` | 将 `D` 寄存器设为 1 |
| `200BH` | `POWER_LOOP` | `CALL MULTIPLY` | 乘法子程序调用 |
| `200EH` |  | `DCR C` | 减量 `C` |
| `200FH` |  | `JNZ POWER_LOOP` | 调用 `POWER_LOOP`，直到 `C` 变为 0 |
| `2012H` |  | `INX H` | 增量内存 |
| `2013H` |  | `MOV M, D` | 将结果存储在内存中 |
| `2014H` |  | `HLT` | 停止 |
|  |  |  |  |
| `2100H` | `MULTIPLY` | `MOV E, B` | 将 `B` 的内容转移到 `E` |
| `2101H` |  | `MVI A, 00H` | 清除累加器以存储结果 |
| `2103H` | `MULTIPLY_LOOP` | `ADD D` | 将 `D` 的内容添加到 `A` 中 |
| `2104H` |  | `DCR E` | 减量 `E` |
| `2105H` |  | `JNZ MULTIPLY_LOOP` | 叠加 |
