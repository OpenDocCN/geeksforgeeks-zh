...
> 原文：[https://www.geeksforgeeks.org/conditional-branch-instructions-in-avr-microcontroller/](https://www.geeksforgeeks.org/conditional-branch-instructions-in-avr-microcontroller/)
...
1.  **状态寄存器** (`SReg`) :
    *   它是 AVR 微控制器中的标志寄存器。
    *   它是一个 8 位寄存器。这 8 位中只有 6 位称为条件标志。它们是 `C`、`N`、`V`、`S`、`H`。
    *   这些位的值表示指令执行后产生的一些条件。
2.  **`C`** – 进位标志:
    *   每当 `D7` 位有输出时，该标志置 1。
    *   该标志位在 8 位加法或减法后受到影响。
...
| --- | --- | --- |
| `BREQ` | 相等时分支 | `Z = 1` 时的分支 |
| `BRNE` | 不相等时分支 | `Z = 0` 时的分支 |
| `BRSH` | 相同或更高的分支 | `C = 0` 时的分支 |
| `BRLO` | 较低的分支 | `C = 1` 时的分支 |
| `BRLT` | 小于(签名)的分支 | `S = 1` 时的分支 |
| `BRGE` | 大于或等于(有符号)的分支 | `S = 0` 时的分支 |
