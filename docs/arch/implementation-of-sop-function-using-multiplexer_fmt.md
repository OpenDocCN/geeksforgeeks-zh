...
> 原文：[https://www.geeksforgeeks.org/implementation-of-sop-function-using-multiplexer/](https://www.geeksforgeeks.org/implementation-of-sop-function-using-multiplexer/)
...
1.  为给定数量的变量函数绘制真值表。
2.  考虑一个变量作为输入，其余变量作为选择线。
3.  形成矩阵，其中多路复用器的输入行是列，输入变量及其补码是行。
4.  根据真值表找出两行之间的 AND。
5.  因此，无论发现什么都被视为多路复用器的输入。
