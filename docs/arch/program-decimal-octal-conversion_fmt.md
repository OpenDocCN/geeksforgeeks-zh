...
> 原文: [https://www.geeksforgeeks.org/program-decimal-octal-conversion/](https://www.geeksforgeeks.org/program-decimal-octal-conversion/)
...
这个问题也可以在不使用数组的情况下使用以下算法来解决:

*   将八进制数初始化为 0，将 `countVal` 初始化为 1，将十进制数初始化为 `n`。
*   当十进制数除以 8 时求余数。
*   用 `八进制 + (余数 * countval)` 更新八进制数。
