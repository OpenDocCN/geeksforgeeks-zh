...
> 原文：`https://www.geeksforgeeks.org/array-multiplier-in-digital-logic/`

`阵列乘法器`是一种数字`组合电路`，用于通过使用全加器和半加器阵列来相乘两个二进制数。该数组用于几乎同时添加所涉及的各种产品术语。为了形成各种乘积项，在加法器阵列之前使用与门阵列。
...
具有更多位的组合电路二进制乘法器可以以类似的方式构造。乘法器的一位与被乘数的每一位进行“与”运算，级数与乘法器的位数相同。每一级与门的二进制输出与前一级的部分积并行相加，形成一个新的部分积。最后一级生产产品。对于`j`个乘数位和`k`个被乘数，我们需要
