...
> 原文：[https://www.geeksforgeeks.org/control-signals-generation-using-counter/](https://www.geeksforgeeks.org/control-signals-generation-using-counter/)
...
生成一个控制信号，该信号可以传送以下脉冲序列。脉冲序列将在 7 个脉冲后重复。

![](img/ec7a4c1bcd61ce083d43a79b46a4b008.png)

## 控制信号

这里我们需要产生一个控制信号（比如 `S`），它可以产生 `0111001` 的周期脉冲序列，然后重复。脉冲序列在 7 个脉冲后重复。因此，需要 `mod-7` 计数器。`mod-7` 计数器的输出将连接到解码器电路。为此，将设计一个 `mod-7` 计数器。需要三次 `T` 触发器（因为我们需要从 `000` 数到 `110`，所以需要 3 位）。
