...
> 原文: [https://www.geeksforgeeks.org/log-structured-file-system-lfs/](https://www.geeksforgeeks.org/log-structured-file-system-lfs/)
...
`D_{j, 0}, D_{j, 1}, D_{j, 2}` 和 `D_{j, 3}` 是对同一文件 j 的 4 次更新，一次写入磁盘。这是缓冲到 LFS 的一组更新之一。`D_{k, 0}` 是文件 k 的更新，在下一次循环中写入磁盘。

## 读写进程

**读进程简单来说:**
在找到文件的索引节点(保存在索引节点映射中)后，LFS 读取进程与 Unix 文件系统中的相同。
