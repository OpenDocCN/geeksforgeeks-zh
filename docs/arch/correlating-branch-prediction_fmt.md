...
程序中存在的条件分支会显著影响系统的性能。因此，我们需要提出有效的分支预测机制，以获得高精度的分支目标地址，从而最大限度地减少与控制风险相关的停顿。如果未能正确预测目标地址，将会在刷新流水线和将处理器恢复到执行分支指令时的状态方面出现损失。
...
在静态分支预测技术的情况下，底层硬件假设要么分支不总是被采用，要么分支总是被采用。让我们用一个示例代码来理解分支预测：
...
在动态分支预测技术中，底层硬件的预测不是固定的，而是动态变化的。该技术比静态技术具有更高的精度。一些动态分支预测技术有：
1.  1 位分支预测技术
2.  2 位分支预测技术
3.  相关分支预测技术
...
#### 2.1 1-bit Branch Prediction Technique

在这种技术中，硬件在一次错误假设后立即改变其假设。例如，如果硬件假设分支被采用但实际上分支未被采用，那么在下一步硬件会假设分支未被采用，反之亦然。
...
#### 2.2 2-bit Branch Predictor

在这种技术中，底层硬件不会在一次错误假设后立即改变其假设，而是在连续两次错误假设后才改变其假设，反之亦然。
...
#### 2.3 Correlating Branch Prediction

由于与其他分支的干扰，我们无法从 2 位分支预测器中获得显著的准确性。因此，相关分支预测应运而生，它也被称为两级分支预测器，其预测准确性更高，因为它还考虑了其他分支的近期行为。
...
**讲解循序渐进–**
**1.** 最小 `k` (这里 `k=3`) 有效位是 `100`，即 `4`，因此这指向本地历史表中的位置 `4`。在位置 `4`，它包含 `000`，即 `0`，因此它指向本地预测表中的位置 `0`。它包含状态 `00`，即分支未被采用，但实际上分支 (`0%2`) 被采用。所以现在两个表都更新了。完整流程如图：

![](img/304364c3d26fc58c668f0e3f2131ded0.png)

**2.** 在位置 `4`，现在它在本地历史表中包含 `100`，所以它指向本地预测表中的位置 `4`。它包含状态 `00`，即分支未被采用，实际上分支未被采用。因此相应地，表被更新。完整流程如图：

![](img/2cccd2bdec99d84d5a2f47798a504759.png)

**3.** 在位置 `4` 现在它在本地历史表中包含 `010`，所以它指向本地预测表中的位置 `2`。它包含状态 `00`，即未采取分支，实际上采取了分支。因此相应地，表被更新。完整流程如图：

![](img/b2dfa210b0d3d42a4afe2564deb90954.png)

**4.** 位置 `4`，现在在本地历史表中包含 `101`，所以指向本地预测表中的位置 `5`。它包含状态 `00`，即未采取分支，实际上未采取分支。因此相应的表格得到更新。完整的流程如图所示：

![](img/4a4e26fe556feb375b9177f509da40f8.png)

**5.** 现在位置 `4` 在本地历史表中包含 `010`，因此它指向本地预测表中的位置 `2`。它包含状态 `01`，即未采取分支，实际上采取了分支。因此相应的表格得到更新。完整的流程如图所示：
