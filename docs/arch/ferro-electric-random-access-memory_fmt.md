...
> 原文：[https://www.geeksforgeeks.org/ferro-electric-random-access-memory/](https://www.geeksforgeeks.org/ferro-electric-random-access-memory/)

`FRAM`是一种[随机存取存储器](https://www.geeksforgeeks.org/random-access-memory-ram-and-read-only-memory-rom/)，它使用铁电电容器来实现其非易失性（断电时内容不会丢失）。铁电材料的双电常数通常很高。最广泛使用的铁电材料是`PZT(锆钛酸铅)`。由于该层是铁电的，看起来它可能含铁，但实际上它不含铁。
