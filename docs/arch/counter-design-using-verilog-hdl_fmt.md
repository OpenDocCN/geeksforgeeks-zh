...
> 原文：[https://www.geeksforgeeks.org/counter-design-using-verilog-hdl/](https://www.geeksforgeeks.org/counter-design-using-verilog-hdl/)
...
*   当 `reset` 输入为逻辑高时，计数器将设置为零。
*   当 `load` 信号为逻辑高时，计数器将加载 `data` 输入。否则，它将向上或向下计数。
