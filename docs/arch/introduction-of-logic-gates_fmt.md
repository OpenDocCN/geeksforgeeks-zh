...
*   **XOR gate(`⊕`)–** 异或门给出 1 的输出，如果两个输入不同，则给出 0。

如果上述门的输出被否定，则获得另外三个逻辑门。

*   **NAND gate(`↑`)-** 与非门(否定的“与”)给出 0 的输出，如果两个输入都是 1，则给出 1。
*   **NOR gate(`↓`)-** 如果两个输入都为 0，或非门(求反或)给出 1 的输出，否则给出 0。
*   **XNOR gate(`⊙`)-** XNOR 门(求反异或)给出 1 的输出，如果两个输入相同，则为 0。
