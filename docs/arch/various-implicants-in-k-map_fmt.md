...
## 例-1:
给定 `F = ∑(1，5，6，7，11，12，13，15)`，求隐含数，`PI`，`EPI`，`RPI`，`SPI` 的个数。
...
## 例-2:
给定 `F =∞(0，1，5，8，12，13)`，求隐含数，`PI`，`EPI`，`RPI`，`SPI` 的个数。
...
## 例-3:
给定 `F = ∑(0，1，5，7，15，14，10)`，求隐含数，`PI`，`EPI`，`RPI`，`SPI` 的个数。
...
## 示例-4:
[GATE IT 2006 |问题 35](https://www.geeksforgeeks.org/gate-gate-it-2006-question-35/)