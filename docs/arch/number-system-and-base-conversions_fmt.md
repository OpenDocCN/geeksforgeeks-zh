...
> 原文: [https://www.geeksforgeeks.org/number-system-and-base-conversions/](https://www.geeksforgeeks.org/number-system-and-base-conversions/)
...
```
上图中，`d_{n-1}` 到 `d_{0}` 为整数部分，然后跟随一个基点，然后 `d_{-1}` 到 `d_{-m}` 为小数部分。
