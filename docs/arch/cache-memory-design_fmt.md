...
当一个替换块的数据被扫描进缓存时，映射功能决定了该块将占据的缓存位置。两个约束影响着映射功能的设计。首先，当一个块被扫描进入时，另一个块可能被替换。
...
替换算法根据映射功能的约束，选择在将一个新块加载到缓存中且缓存的所有槽都已被其他块占满时，要替换哪个块。我们希望替换那个在不久的将来最不可能被再次需要的块。虽然不可能准确找出这样的块，但一个相当有效的策略是替换那个在缓存中存在时间最长且未被引用的块。

这一政策是因为最近最少使用的（LRU）算法规则。发现最近最少使用的块所需的硬件机制平方度量。

## 写入策略

如果缓存中一个块的内容被改变，那么在替换它之前有必要将其写回主内存。写入策略规定了内存写操作何时发生。在一个极端情况下，每当块被更新时就会发生写入。
