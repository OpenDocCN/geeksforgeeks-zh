...
> 原文: [https://www.geeksforgeeks.org/combinational-and-sequential-circuits/](https://www.geeksforgeeks.org/combinational-and-sequential-circuits/)
...
通常使用两种类型的存储元件：`锁存器`和`触发器`。以信号电平（而不是信号转换）操作的存储元件被称为`锁存器`；由时钟转换控制的是`触发器`。