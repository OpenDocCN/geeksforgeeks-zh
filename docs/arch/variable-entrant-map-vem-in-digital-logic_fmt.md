...
> 原文: [https://www.geeksforgeeks.org/variable-entrant-map-vem-in-digital-logic/](https://www.geeksforgeeks.org/variable-entrant-map-vem-in-digital-logic/)
...
现在，让我们看看如果给定一个 VEM，如何找到 SOP 表达式。
...
3.  对 k 图中的所有变量重复步骤 2。
4.  VEM 的 SOP 通过将所有获得的 SOP 表达式进行或运算得到。
...
将所有变量写成 0 (`D` 和 `D’` 被认为是两个不同的变量)，留下 minterms，`0’` 不要管它是什么，得到 SOP 表达式。
...
VEM 的标准操作程序通过或运算得到所有的标准操作程序表达式。因此，给定 VEM 的标准操作程序表达式为:
