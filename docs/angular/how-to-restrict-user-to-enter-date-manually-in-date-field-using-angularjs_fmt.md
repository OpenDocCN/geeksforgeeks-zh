...
> 原文: [https://www.geeksforgeeks.org/how-to-restrict-user-to-enter-date-manually-in-date-field-using-angularjs/](https://www.geeksforgeeks.org/how-to-restrict-user-to-enter-date-manually-in-date-field-using-angularjs/)
...
*   在 `onkeydown` 事件中，我们需要返回 `false`，这样我们就可以限制用户手动输入日期。
*   为了实现上述目标，我们需要在 ts 文件中编写函数并返回 `false`。
