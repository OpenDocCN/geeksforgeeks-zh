...
> 原文：[https://www.geeksforgeeks.org/how-to-use-plunker-in-angular/](https://www.geeksforgeeks.org/how-to-use-plunker-in-angular/)
...
*   `` `app.component.ts` ``：这是主文件，我们将在其中转储属于我们项目的所有 `Angular` 代码。
*   `` `system.config.js` ``：该文件用于处理模块加载和将 `TypeScript` 编译成 `JavaScript` 的 `SystemJS` 的配置。
*   `` `tsconfig.json` ``：`TypeScript` 编译器的配置。

## 运行步骤

*   不需要任何额外的设置，一切你可以直接在网上编码。
*   这是 Plunker 官方的链接：[https://plnkr.co/](https://plnkr.co/)，去这个网站。
*   点击显示的新按钮：这将带你到一个弹出窗口。

![](img/68a5e4876a2487da4bcf9b78cdfaa9eb.png)

*   现在，您将会看到一个弹出窗口，询问您要对哪种编程语言进行编码，选择 **Angular Starter Template**。
