...
> 原文: [https://www.geeksforgeeks.org/how-to-create-todo-list-in-angular-7/](https://www.geeksforgeeks.org/how-to-create-todo-list-in-angular-7/)
...
## 2. 运行应用程序
通过 `cd` 进入应用程序并运行。之后打开本地主机，检查应用程序是否正常工作。
```ts
cd my-todo-list
ng serve
```
...
## 4. 编辑 HTML 模板
打开 `src/app` 文件夹，开始编辑 `app.component.html`:
```ts
...
## 5. 编写组件逻辑
打开 `app.component.ts` 文件，编写任务增删功能。
```ts
...
## 6. 导入表单模块
为了处理接受输入的表单，我们必须在 `app.module` 文件中导入表单模块。
```ts
