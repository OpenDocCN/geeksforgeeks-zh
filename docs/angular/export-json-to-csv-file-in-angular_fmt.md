# 在 Angular 中将 JSON 导出到 CSV 文件

> 原文: [https://www.geeksforgeeks.org/export-json-to-csv-file-in-angular/](https://www.geeksforgeeks.org/export-json-to-csv-file-in-angular/)

在本文中，我们将学习如何使用 `angular 8` 中的 `Blob` 将 `JSON 数组`转换为 `CSV` 以及如何将 `CSV` 导出为可下载文件。
...
第一个参数是对象数组，第二个参数是标题列表一般是 Json 的键。
...
下载方法接受两个参数，JSONdata 中的第一个参数和第二个参数是文件名。这里的默认文件名是 `data`。在下载文件方法中，我们调用 `ConvertToCSV` 方法将 JSON 转换为 CSV。

一个 [Blob](https://developer.mozilla.org/en-US/docs/Web/API/Blob) 对象代表一个不可变的原始数据的类似文件的对象。Blobs 代表不一定是 JavaScript 原生格式的数据。`File` 接口基于 blob，继承 Blob 功能并扩展它以支持用户系统上的文件。

## app.service.ts

创建一个名为 `app.service.ts` 的新服务文件。
