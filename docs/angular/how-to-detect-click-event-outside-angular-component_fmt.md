...
> 原文：[https://www.geeksforgeeks.org/how-to-detect-click-event-outside-angular-component/](https://www.geeksforgeeks.org/how-to-detect-click-event-outside-angular-component/)
...
这里的方法是使用 `@HostListener` 装饰器。在 angular 中，它是一个装饰器，有助于捕获 DOM 中发生的任何类型的事件，并为开发人员提供基于该事件执行任何操作的灵活性。这里，在简单的点击事件上，处理程序将把点击事件引用到组件上，对于 DOM 上的点击，它将使用 `document:click`。
...
3.  **handler_name**：下面是触发事件时将调用的方法定义。它由 `HostListener` 自动调用。

## 示例：在组件内通过点击进行绑定

为了在组件内绑定点击，将进入主机侦听器的事件名称将只是“click”。在这种情况下，上述代码将写成：
...
要检测组件外部的点击，需要查看另一个事件。在这里，`click` 将不起作用，因为它检测到组件内的点击。这里的关键是在 DOM 中寻找点击，而不仅仅是组件，因此 `document:click` 将是正确的选择，同时我们需要过滤掉组件中的事件，这是由布尔变量 `inside` 完成的。因此，在下面给出的代码中，将有另一个组件添加到其中，它将充当外部上下文，但是单击它将导致当前组件上的 `click` 事件。
...
在本例中，如果点击文本“Outside Component”，则显示的文本将是“Event Triggered Outside Component”。这显示了如何在 `GeeksComponent` 中捕捉组件外部的点击。