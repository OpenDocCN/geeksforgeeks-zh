...
> 原文: [https://www.geeksforgeeks.org/angularjs-ng-mousedown-directive/](https://www.geeksforgeeks.org/angularjs-ng-mousedown-directive/)

`ng-mousedown` 指令用于在特定的 HTML 元素上发生 `mousedown` 事件时应用自定义行为。当按下鼠标按钮时，它可以用来显示弹出警告。所有 HTML 元素都支持它。
...
本示例使用 `ng-mousedown` 指令在单击鼠标后设置元素的样式。
...
本示例使用 `ng-mousedown` 指令在点击输入区域后显示提醒消息。
