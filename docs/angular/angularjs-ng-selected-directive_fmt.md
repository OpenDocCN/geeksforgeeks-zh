...
> 原文: [https://www.geeksforgeeks.org/angularjs-ng-selected-directive/](https://www.geeksforgeeks.org/angularjs-ng-selected-directive/)

AngularJS 中的 `ng-selected` 指令用于指定 HTML 元素的选定属性。它可以用来选择在 HTML 元素上指定的默认值。如果 `ng-selected` 指令中的表达式返回 `true`，则显示选定的选项值，否则不显示。
