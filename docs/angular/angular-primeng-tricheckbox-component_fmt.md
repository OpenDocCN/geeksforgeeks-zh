...
> 原文: [https://www.geeksforgeeks.org/angular-primeng-tricheckbox-component/](https://www.geeksforgeeks.org/angular-primeng-tricheckbox-component/)
...
*   `name`: 用于给出元素的名称。它是字符串数据类型，默认值为 null。
*   `label`: 用于给出元素的标签。它是字符串数据类型，默认值为 null。
...
*   `style`: 用于给出组件的 Inline 样式。它属于对象数据类型，默认值为空。
*   `styleClass`: 是组件的 Style 类。它是字符串数据类型，默认值为 null。
*   `readonly`: 指定组件不可编辑。它是布尔数据类型，默认值为 false。
*   `checkboxTrueIcon`: 用于设置复选框真值的指定图标。它是字符串数据类型，默认值是 `pi pi-check`。
*   `checkboxFalseIcon`: 用于设置复选框假值的指定图标。它是字符串数据类型，默认值是 `pi pi-check`。
...
在本例中，我们将了解如何在 `triCheckbox` 组件中使用 `readonly` 和 `style` 属性。
...
**参考:** [https://primefaces.org/primeng/showcase/#/tristatecheckbox](https://primefaces.org/primeng/showcase/#/tristatecheckbox)