...
> 原文: [https://www.geeksforgeeks.org/angular-primeng-fieldset-component/](https://www.geeksforgeeks.org/angular-primeng-fieldset-component/)
...
*   `legend`: 是字段集的表头文本。它是字符串数据类型，默认值为 `null`。
*   `toggleable`: 指定点击图例是否可以切换内容。它是布尔数据类型，默认值为 `false`。
*   `collapsed`: 定义内容的默认可见性状态。它是布尔数据类型，默认值为 `false`。
*   `style`: 是 `fieldset` 的内联样式。它是字符串数据类型，默认值为 `null`。
...
在本例中，我们将了解如何在 `Fieldset` 组件中使用 `toggleable` 属性。
...
**参考:** [https://primefaces.org/primeng/showcase/#/fieldset](https://primefaces.org/primeng/showcase/#/fieldset)