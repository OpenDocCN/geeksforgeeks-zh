...
AngularJS 中的 `ng-show` 指令用于显示或隐藏指定的 HTML 元素。如果在 `ng-show` 属性中给定的表达式为真，那么 HTML 元素将会显示，否则它会隐藏 HTML 元素。所有 HTML 元素都支持它。
...
本示例使用 `ng-show` 指令在选中复选框后显示 HTML 元素。
...
本示例使用 `ng-show` 指令显示输入的数字是否为 5 的倍数。
