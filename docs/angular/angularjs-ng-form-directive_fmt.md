...
AngularJS 中的 `ng-form` 指令用于创建嵌套表单，即一个表单位于另一个表单的内部。它指定从 HTML 表单继承控件。它在表单指令中创建控件组，该指令可用于确定控件子组的有效性。
...
本示例使用 `ng-form` Directive 隐藏输入文本字段并显示其内容。
...
本示例使用 `ng-form` Directive 验证电子邮件并保存。
