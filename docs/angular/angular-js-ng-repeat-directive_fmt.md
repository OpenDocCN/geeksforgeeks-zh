...
> 原文: [https://www.geeksforgeeks.org/angular-js-ng-repeat-directive/](https://www.geeksforgeeks.org/angular-js-ng-repeat-directive/)
...
**注意-** `track by $index` 在这里使用，因为我们的列表中有重复的条目，即 `Geeks`。不允许重复的键，因为 AngularJS 使用键将 DOM 节点与项目相关联。`track by $index` 将导致项目由它们在数组中的位置而不是它们的值来键入。
