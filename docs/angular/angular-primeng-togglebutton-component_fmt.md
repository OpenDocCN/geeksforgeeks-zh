...
> 原文: [https://www.geeksforgeeks.org/angular-primeng-togglebutton-component/](https://www.geeksforgeeks.org/angular-primeng-togglebutton-component/)
...
`ToggleButton` 组件用于制作一个按钮，用户点击即可进行切换。

**属性:**

*   `onLabel`: 用于设置 on 状态的标签。它是字符串数据类型，默认值为 `null`。
*   `offLabel`: 用于设置标签为关闭状态。它是字符串数据类型，默认值为 `null`。
*   `onIcon`: 用于设置开启状态的图标。它是字符串数据类型，默认值为 `null`。
*   `offIcon`: 用于设置图标为关闭状态。它是字符串数据类型，默认值为 `null`。
*   `iconPos`: 用于设置图标的位置，有效值为“左”和“右”。它是字符串数据类型，默认值为 `left`。
...
在本例中，我们将了解如何在 `toggleButton` 组件中使用 `iconPos` 属性。
...
**参考:** [https://primefaces.org/primeng/showcase/#/togglebutton](https://primefaces.org/primeng/showcase/#/togglebutton)