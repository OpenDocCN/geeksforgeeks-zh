# 在Angular 11中建立兄弟组件之间的通信

> 原文: [https://www.geeksforgeeks.org/establish-communication-between-sibling-components-in-angular-11/](https://www.geeksforgeeks.org/establish-communication-between-sibling-components-in-angular-11/)

在本文中，我们将看到如何在客户端机器上的兄弟组件之间传递数据。

在Angular中，我们将网页分成多个组件，这些组件之间的关系形成了一个树状结构。一个组件可能有一个父组件和多个子组件。也就是说，它也可以有兄弟姐妹。在某些情况下，我们需要在这些独立的组件之间传递数据。传递数据很容易，而且在处理大型项目时，它会使我们的代码更加清晰易读。
...
在Angular中，我们可以利用其内置特性来实现这一点:

*   `@Output`装饰器有助于通过`EventEmitter`对象发出数据。我们将通过这个例子看到它的工作原理。
*   父组件将把数据捕获为`$event`变量。它可以在一种方法中使用它，也可以通过其他方式传输它。
...
*   安装angular cli。以管理员身份运行或在出现权限错误时使用sudo。

```tshtml
npm i -g @angular/cli
```

*   安装angular CLI后，使用以下命令启动新项目:
...
*   您将在应用程序文件夹中看到两个目录“search”和“table”。两者都将各有4个新文件。
