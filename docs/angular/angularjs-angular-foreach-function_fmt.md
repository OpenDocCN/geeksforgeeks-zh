...
> 原文：`https://www.geeksforgeeks.org/angularjs-angular-foreach-function/`

AngularJS 中的 `angular.forEach()` 函数用于迭代数组或对象中的每一项。它的工作方式类似于 for 循环，该循环包含对象键值对中对象的所有属性。
...
*   `object`：指要迭代的对象。
*   `iterator`：指迭代器函数（`value`, `key`, `obj`）。
*   `context`：这是可选的。它指的是成为迭代器函数上下文的对象。
