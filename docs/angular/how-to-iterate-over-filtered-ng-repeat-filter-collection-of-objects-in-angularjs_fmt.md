...
> 原文: [https://www.geeksforgeeks.org/how-to-iterate-over-filtered-ng-repeat-filter-collection-of-objects-in-angularjs/](https://www.geeksforgeeks.org/how-to-iterate-over-filtered-ng-repeat-filter-collection-of-objects-in-angularjs/)
...
在本例中，我们正在显示学生的详细信息，并且有一个搜索框可以按学生的姓名进行搜索。现在，我们将搜索框的输入绑定到 AngularJS `searchName` 变量中。`searchName` 变量用于过滤，我们为它创建一个别名，它就是这里的 `result`，然后当我们点击 change marks 按钮 `changemarks()` 函数被调用，`result` 别名变量作为参数传递。现在，在 `changemarks()` 函数中，我们将每个被过滤的学生的分数增加 10。因此，通过点击“更改标记”按钮，我们只更改了那些已经被过滤的学生的标记属性。
