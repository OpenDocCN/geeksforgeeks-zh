# Angular PrimeNG MegaMenu组件使用指南

> 原文：[https://www.geeksforgeeks.org/angular-primeng-megamenu-component/](https://www.geeksforgeeks.org/angular-primeng-megamenu-component/)
...
## MegaMenu 组件
是一个导航组件，用于制作菜单的多个编号的组件。

## 属性
*   `model`：是一组菜单。它接受数组数据类型作为输入&默认值为空。
*   `orientation`：用于定义方位。它是字符串数据类型&默认值是水平。
...
@NgModule({
  imports: [BrowserModule, BrowserAnimationsModule, MegaMenuModule],
...
## 示例 2
在本例中，我们将了解如何在 `MegaMenu` 组件中使用 `orientation` 属性。
...
## 参考
[https://primefaces.org/primeng/showcase/#/megamenu](https://primefaces.org/primeng/showcase/#/megamenu)