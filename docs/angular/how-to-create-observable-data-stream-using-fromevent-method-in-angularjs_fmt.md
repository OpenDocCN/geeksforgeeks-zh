...
> 原文：[https://www.geeksforgeeks.org/how-to-create-observable-data-stream-using-fromevent-method-in-angularjs/](https://www.geeksforgeeks.org/how-to-create-observable-data-stream-using-fromevent-method-in-angularjs/)
...
创建可观察对象最常见和最简单的方法是使用 RxJs 库提供的方法。这是一个第三方库，在您创建新的角度项目时会自动安装。其中一个方法是 `fromEvent`。稍后我们将讨论其他方法，如 `interval` 和 `timer`。

### FromEvent

`FromEvent` 是 RxJs 提供的一种创建可观察对象的方法。最好的是，我们可以直接从 DOM 事件中创建可观察的对象。DOM 事件是指点击事件、按键事件、滚动事件等。这是一个简单的鼠标点击，我们可以创建一个数据流，即一个可观察的。`FromEvent` 会帮助我们解决这个问题。
...
**示例**: `click`、`mousedown`、`mouseup`、`click`、`dblclick`、`mousemove`、`mouseenter`、`mousewheel`、`mouseleave`、`touchstart`、`touchmove`、`touchend`、`touchcancel`、`keydown`、`keypress`、`keyup` 等。

### 方法

我们将在 UI 上创建一个简单的按钮“添加图书”，每次点击这个按钮，我们将创建一个数据流。为了简单起见，我们的数据流将只包含一个硬编码的字符串，即“Book”。每次点击，我们都会继续生成第一本书、第二本书、第三本书等等。为了在用户界面上显示该流，我们将创建一个空的 `<ul>`，并在运行时继续将每本书作为一个列表项添加到 `<ul>` 中，即 `<li>`。

### 准备

要从 DOM 元素触发的任何事件中创建可观察的对象，首先，我们需要获取对该 DOM 元素的引用。这可以使用 `ViewChild` 和 `ElementRef` 来完成。因此，这方面的代码可以写成：
...
*   **TypeScript:**
    ...
    这里，`this.addBtn.nativeElement` 返回原生的 DOM 元素。这将是我们的目标（准确地说是事件目标），这是 `fromEvent` 方法的第一个参数。第二个参数是 `'click'`，因为我们使用的是按钮点击。所以，代码变成了：
...
*   **步骤 2: 创建一个名为 `FromEventComponent` 的组件**
    ...
    应创建 4 个新文件，并更新 `app.module.ts`

    ![](img/24888527853f0892ff9709f1d3414f35.png)

    所有必需的文件和更改

*   **步骤 3: 创建服务 `PrintService`**
...
```ts
<!-- To display the data/stream on UI we will keep on adding each stream as a list item in ul below -->
...
我们采用了一个计数变量来区分流。它将继续用“Book”添加自动递增的整数。这里，在 `this._printService.printStream('elementContainer', 'Book ' + count++)` 这一行，我们在告诉在哪里追加以及追加什么。
...
您可以看到每次点击鼠标都会处理流。在接下来的文章中，我们将了解 `interval` 和 `timer`。这只不过是另一种通过添加设施来创建可观察对象的方式。希望这有所帮助。
