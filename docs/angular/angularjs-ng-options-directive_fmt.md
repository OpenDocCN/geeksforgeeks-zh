...
> 原文: [https://www.geeksforgeeks.org/angularjs-ng-options-directive/](https://www.geeksforgeeks.org/angularjs-ng-options-directive/)

`ng-options` 指令用于构建带有选项的 HTML 元素并将其绑定到模型属性。用于在 `<select>` 列表中指定 `<option>`。它是专门为填充下拉列表的项目而设计的。由 `<select>` 元素支撑。
