...
> 原文：`https://www.geeksforgeeks.org/angular-primeng-progressbar-component/`
...
*   `value`: 指定进度的当前值。它接受数字数据类型，默认值为 `null`。
*   `showValue`: 指定显示还是隐藏进度条值。它属于布尔数据类型，默认值为 `true`。
*   `unit`: 指定值后附加的单位符号。它是字符串数据类型，默认值为 `%`。
*   `mode`: 指定进度的模式，有效值为 `"determinate"` 和 `"indeterminate"`。它是字符串数据类型，默认值是 `"determinate"`。

## 样式

*   `.p-progressbar`: 是容器元素。
*   `.p-progressbar-determinate`: 它是确定的 progress bar 的容器元素。
*   `.p-progressbar-indeterminate`: 它是不定 progress bar 的容器元素。
...
在本例中，我们将使动态进度条组件随机更新，并显示或不显示进度状态值。
...
`https://primefaces.org/primeng/showcase/#/progressbar`