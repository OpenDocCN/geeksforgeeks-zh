...
> 原文: [https://www.geeksforgeeks.org/difference-between-directive-and-component-in-angularjs/](https://www.geeksforgeeks.org/difference-between-directive-and-component-in-angularjs/)
...
5.  组件永远不应该修改超出自己范围的任何数据或 DOM。指令具有独立的作用域，默认情况下，子级从其父级继承作用域。
6.  每个 DOM 元素只能有一个组件。一个 DOM 元素中可以有多个指令。
7.  To register component we use `@Component` meta-data annotation. For directive, we use `@Directive` meta-data annotation. Two simple examples are shown in the point ‘Example’ above.
