...
> 原文: [https://www.geeksforgeeks.org/angular-primeng-steps-component/](https://www.geeksforgeeks.org/angular-primeng-steps-component/)
...
*   `model`: 是一个菜单项的数组。它是数据的数组类型&默认值为空。
*   `activeIndex`: 是活动项的索引。它接受数字作为输入数据类型&默认值为 0。
*   `readonly`: 指定项目是否可点击。它属于布尔数据类型&默认值为真。
*   `style`: 设置组件的内嵌样式。它属于字符串数据类型&，默认值为空。
...
[https://primefaces.org/primeng/showcase/#/steps/confirmation](https://primefaces.org/primeng/showcase/#/steps/confirmation)