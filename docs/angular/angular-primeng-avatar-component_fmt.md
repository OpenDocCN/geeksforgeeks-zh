...
> 原文: [https://www.geeksforgeeks.org/angular-primeng-avatar-component/](https://www.geeksforgeeks.org/angular-primeng-avatar-component/)
...
*   `label`: 用于定义要显示的文本。它是字符串数据类型，默认值为 null。
*   `icon`: 用于定义要显示的图标。它是字符串数据类型，默认值为 null。
*   `image`: 用于定义要显示的图像。它是字符串数据类型，默认值为 null。
*   `size`: 用于设置元素的大小，有效选项有“大”和“xlarge”。它是字符串数据类型，默认值为 null。
*   `shape`: 用于设置元素的形状，有效选项有“方形”和“圆形”。它是字符串数据类型，默认值为方形。
*   `style`: 用于设置组件的内嵌样式。它属于对象数据类型，默认值为空。
*   `styleClass`: 用于定义组件的样式类。它是字符串数据类型，默认值为 null。

## AvatarGroup 的属性

*   `style`: 这里，在本例中也是，用于设置组件的内嵌样式。它属于对象数据类型，默认值为空。
*   `styleClass`: 这里，在这种情况下也是，用来定义组件的样式类。它是字符串数据类型，默认值为 null。

## 头像造型

*   `p-avatar`: 是容器元素。
*   `p-avatar-image`: 是图像模式下的容器元素。
*   `p-avatar-circle`: 是圆形的容器元素。
*   `p-avatar-text`: 是头像的文字元素。
*   `p-avatar-icon`: 是头像的风格图标。
*   `p-avatar-lg`: 是大尺寸的容器元素。
*   `p-avatar-xl`: 是一个 xlarge 大小的容器元素。

## avatar group 的造型

*   `p-avatar-group`: 用于元素的容器。
...
**参考:** [https://primefaces.org/primeng/showcase/#/avatar](https://primefaces.org/primeng/showcase/#/avatar)