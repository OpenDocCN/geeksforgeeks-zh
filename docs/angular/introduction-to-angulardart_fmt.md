...
### 如何在线运行 AngularDart 代码

如果您是初学者，想要在线测试您的 `AngularDart` 代码，请按照以下步骤操作：
...
2.  在主方法中，我们将变量声明为 `"header"`。
3.  变量使用 `querySelector` 获取值。
4.  我们给标题变量分配了一个文本来在 `HTML` 中使用。
5.  在 `HTML` 文件中，我们使用 `id` 作为参数调用了 `header` 值。
...
| 在 `AngularDart` 中，我们使用 `apply` 函数。 | 在 `AngularJs` 中，使用 `link`/`compile` 函数。 |
