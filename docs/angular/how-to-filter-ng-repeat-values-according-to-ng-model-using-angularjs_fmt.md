...
> 原文:[https://www.geeksforgeeks.org/how-to-filter-ng-repeat-values-according-to-ng-model-using-angularjs/](https://www.geeksforgeeks.org/how-to-filter-ng-repeat-values-according-to-ng-model-using-angularjs/)

通过使用输入字段的值作为过滤器中的表达式，可以根据 AngularJS 中的 `ng-model` 过滤 `ng-repeat` 值。我们可以在输入字段上设置 `ng-model` 指令来过滤 `ng-repeat` 值。

以下示例说明了该方法:

**示例 1:** 根据 `ng-model` 过滤输入文本 `ng-repeat` 值。该过滤器将只显示匹配城市的名称。
