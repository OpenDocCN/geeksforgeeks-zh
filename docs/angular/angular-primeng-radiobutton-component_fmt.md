# Angular PrimeNG RadioButton 组件

> 原文: [https://www.geeksforgeeks.org/angular-primeng-radiobutton-component/](https://www.geeksforgeeks.org/angular-primeng-radiobutton-component/)
...
*   **tabindex:** 用于按 tab binding 顺序设置元素的索引。它是数字数据类型，默认值为 `null`。
...
*   **style:** 用于设置元素的内联样式。它属于对象数据类型，默认值为 `null`。
*   **styleClass:** 用于设置元素的样式类。它是字符串数据类型，默认值为 `null`。
*   **ariaLabel:** 用于定义标注输入元素的字符串。为字符串数据类型，默认值为 `null`。
*   **labelStyleClass:** 用于设置标签的样式类。它是字符串数据类型，默认值为 `null`。

**事件:**

*   **onClick:** 是单选按钮点击时触发的回调。
*   **onFocus:** 是单选按钮收到焦点时触发的回调。
*   **onBlur:** 是单选按钮失去焦点时触发的回调。
...
**参考:** [https://primefaces.org/primeng/showcase/#/radiobutton](https://primefaces.org/primeng/showcase/#/radiobutton)