...
> 原文：[https://www.geeksforgeeks.org/what-is-the-role-of-routeprovider-in-angularjs/](https://www.geeksforgeeks.org/what-is-the-role-of-routeprovider-in-angularjs/)
...
`$routeProvider` 是一个简单的应用编程接口，它接受 `when()` 或 `otherwise()` 方法。
...
<div ng-app="mainApp">
         ...
         <p><a href="#viewStudents">Display Student</a></p>
         <div ng-view></div>
...
*   `$routeProvider` 是 `config` (`mainApp` 模块)下的一个函数，使用键作为“`$routeProvider`”。
...
`$routeProvider` 是在调用 `when()` 和 `otherwise()` 函数的帮助下配置的。

`when()` 函数以路由路径和一个 JavaScript 对象为参数。

`otherwise()` 取一个 JavaScript 对象作为参数。
