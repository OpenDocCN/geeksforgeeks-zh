...
> 原文: [https://www.geeksforgeeks.org/angular-primeng-chip-component/](https://www.geeksforgeeks.org/angular-primeng-chip-component/)
...
*   `removable`: 用于定义是否显示移除图标。它是布尔数据类型，默认值为 false。
*   `style`: 用于设置组件的内嵌样式。它属于对象数据类型，默认值为空。
*   `styleClass`: 用于定义组件的样式类。它是字符串数据类型，默认值为 null。
*   `removeIcon`: 是移除元素的图标。它是字符串数据类型，默认值是 pi pi-times-circle。
...
### 示例 2
在本例中，我们将了解如何在 Chip 组件中使用可移动和图标属性。
...
### 参考
[https://primefaces.org/primeng/showcase/#/chip](https://primefaces.org/primeng/showcase/#/chip)