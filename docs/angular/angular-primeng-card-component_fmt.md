...
> 原文: [https://www.geeksforgeeks.org/angular-primeng-card-component/](https://www.geeksforgeeks.org/angular-primeng-card-component/)
...
*   `header`: 指定卡片标题。它是字符串数据类型&默认值为空。
*   `subheader`: 指定卡片的二级标题。它是字符串数据类型&默认值为空。
...
*   `p-card`: 是容器元素。
*   `p-card-header`: 是标题元素。
*   `p-card-subtitle`: 是副标题元素。
*   `p-card-content`: 是卡的内容。
*   `p-card-footer`: 是卡片的页脚。
...
在本例中，我们将了解如何在 Card 组件中使用 `subheader` 属性。
...
**参考:** [https://primefaces.org/primeng/showcase/#/card](https://primefaces.org/primeng/showcase/#/card)