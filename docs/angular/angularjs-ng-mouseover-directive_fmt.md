...
> 原文: [https://www.geeksforgeeks.org/angularjs-ng-mouseover-directive/](https://www.geeksforgeeks.org/angularjs-ng-mouseover-directive/)

AngularJS 中的 `ng-mouseover` 指令用于在特定的 HTML 元素上发生鼠标悬停事件时应用自定义行为。当鼠标在特定元素上移动时，它可以用来显示弹出警告。所有 HTML 元素都支持它。
...
本示例使用 `ng-mouseover` 指令在鼠标移动到元素上时显示内容。
...
本示例使用 `ng-mouseover` 指令在鼠标移动到元素上时显示警告消息。
