...
> 原文: [https://www.geeksforgeeks.org/angularjs-ng-change-directive/](https://www.geeksforgeeks.org/angularjs-ng-change-directive/)

每当输入元素的值发生变化时，都会使用 AngularJS 中的 `ng-change` 指令。只要输入值发生变化，就会立即计算表达式。它需要一个 `ng-model` 指令。只要输入有任何单一变化，就会触发。它可以与输入元素一起使用，如 `<input>`、`<textarea>`、`<checkbox>` 和 `<select>`。
...
其中 `expression` 是指每当输入值发生变化时要执行的表达式。

## 示例 1

本示例使用 `ng-change` 指令使用复选框显示/隐藏一些内容。
