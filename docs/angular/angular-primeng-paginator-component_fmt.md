...
> 原文: [https://www.geeksforgeeks.org/angular-primeng-paginator-component/](https://www.geeksforgeeks.org/angular-primeng-paginator-component/)
...
- `rows`: 是每页显示的数据计数。它是数字数据类型，默认值为 0。
- `first`: 是要显示的第一行的相对零号。它是数字数据类型，默认值为 0。
- `pageLinkSize`: 设置要显示的页面链接数量。它是数字数据类型，默认值为 5。
...
- `showFirstLastIcon`: 显示分页器上显示的图标是第一页和最后一页。它是布尔数据类型，默认值为真。
- `templateLeft`: 是注入分页器左侧的模板实例。它属于 `TemplateRef` 数据类型，默认值为 null。
...
- `dropdownScrollHeight`: 是视口的下拉高度，以像素为单位，如果列表的高度超过这个值，就定义一个滚动条。它是字符串数据类型，默认值为 200px。
- `currentPageReportTemplate`: 是当前页面报表元素的模板。它是字符串数据类型，默认值为 `({currentPage} of {totalPages})`。
- `showCurrentPageReport`: 指定是否显示当前页面报告。它是布尔数据类型，默认值为 false。
- `showJumpToPageDropdown`: 指定是否显示下拉菜单导航到任意页面。它是布尔数据类型，默认值为 false。
- `showPageLinks`: 指定是否显示页面链接。它是布尔数据类型，默认值为真。
...
- `p-paginator-rpp-options`: 是每页的行数下拉列表。
- `p-paginator-page-options`: 是跳转到每页下拉列表。
...
这是说明如何使用 Paginator 组件的基本示例。我们已经使用了 `rows` 和 `totalRecords` 属性，该属性将给出每页要呈现的数据计数信息和记录总数。
...
在本例中，我们将使用分页器组件中的 `pageLinkSize` 属性和 `rowsPerPageOptions` 下拉列表。
...
[https://primefaces.org/primeng/showcase/#/paginator](https://primefaces.org/primeng/showcase/#/paginator)