...
> 原文：[https://www.geeksforgeeks.org/angular-primeng-chips-component/](https://www.geeksforgeeks.org/angular-primeng-chips-component/)
...
*   `field`：用于设置要在芯片上显示的属性的名称。它是字符串数据类型，默认值为 null。
*   `max`：用于设置允许的最大条目数。它是数字数据类型，默认值为空。
*   `disabled`：用于禁用复选框。它属于布尔数据类型，默认值为 false。
*   `style`：用于给出组件的内嵌样式。它是字符串数据类型，默认值为 null。
...
*   `allowDuplicate`：用于设置是否允许重复值。它属于布尔数据类型，默认值为真。
*   `inputStyle`：用于设置输入字段的内联样式。它是字符串数据类型，默认值为 null。
*   `inputStyleClass`：用于设置输入字段的样式类。它是字符串数据类型，默认值为 null。
*   `addOnTab`：用于设置是否在 Tab 键上添加项目。它属于布尔数据类型，默认值为 false。
*   `addOnBlur`：用于设置输入失焦时是否添加项目。它属于布尔数据类型，默认值为 false。
*   `separator`：用于设置除了回车键之外，按下时添加项目的分隔符。它是字符串数据类型，默认值为 null。
...
**参考：** [https://primefaces.org/primeng/showcase/#/chips](https://primefaces.org/primeng/showcase/#/chips)