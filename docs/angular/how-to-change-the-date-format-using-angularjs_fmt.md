...
> 原文：[https://www.geeksforgeeks.org/how-to-change-the-date-format-using-angularjs/](https://www.geeksforgeeks.org/how-to-change-the-date-format-using-angularjs/)
...
在这个方法中，我们使用了管道（`|`）运算符。该管道操作符有助于根据所需格式（包括角度标准格式和用户定义格式）转换日期对象、数字。在 angular 中，可以使用此运算符基于任何格式、区域设置和时区修改日期对象。它只以可读的格式格式化日期。
...
| 现场 | 区域设置的默认值是–“未定义”。例如，我们可以将其设置为–‘en_US’。**可选参数** |
...
**示例：** 这里，我们使用角度控制器来更改日期格式。日期作为字符串并通过使用 `$filter` 过滤器传递，该筛选器用于筛选对象元素和数组。它以指定的格式为您提供原始数组的子集。
