...
> 原文: [https://www.geeksforgeeks.org/angular-primeng-tag-component/](https://www.geeksforgeeks.org/angular-primeng-tag-component/)
...
*   `value`: 指定标签内部显示的值。它是字符串数据类型，默认值为 `null`。
*   `severity`: 指定标签的严重性类型。它是字符串数据类型，默认值为 `null`。
*   `rounded`: 指定标签边角是否圆角。它属于布尔数据类型，默认值为 `false`。
*   `icon`: 设置标签的图标显示在数值旁边。它是字符串数据类型，默认值为 `null`。
*   `style`: 指定组件的内嵌样式。它属于对象数据类型，默认值为空。
*   `styleClass`: 指定组件的样式类。它是字符串数据类型，默认值为 `null`。
...
**参考:** [https://primefaces.org/primeng/showcase/#/tag](https://primefaces.org/primeng/showcase/#/tag)