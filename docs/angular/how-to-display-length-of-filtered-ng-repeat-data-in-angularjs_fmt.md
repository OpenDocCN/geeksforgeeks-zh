...
> 原文：[https://www.geeksforgeeks.org/how-to-display-length-of-filtered-ng-repeat-data-in-angularjs/](https://www.geeksforgeeks.org/how-to-display-length-of-filtered-ng-repeat-data-in-angularjs/)

任务是显示过滤后的重复数据的长度。这里我们要用别名表达式来解决这个问题。

## 方法
为了显示过滤后的 `ng-repeat` 数据的长度，我们使用了一个别名表达式。我们为用于过滤 `ng-repeat` 数据的变量创建一个别名，并显示每个时刻别名的长度。
