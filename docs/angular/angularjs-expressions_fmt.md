...
AngularJS 中的表达式用于将应用程序数据绑定到 HTML。表达式由 Angular 解析，结果返回到编写表达式的位置。AngularJS 中的表达式是用双大括号写的：`{{ expression }}`。它们的行为类似于 `ng-bind` 指令：`ng-bind="expression"`。
