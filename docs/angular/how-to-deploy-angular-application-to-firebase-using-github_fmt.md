# 如何使用 GitHub 将 Angular 应用部署到 Firebase？

> 原文：[https://www.geeksforgeeks.org/how-to-deploy-angular-application-to-firebase-using-github/](https://www.geeksforgeeks.org/how-to-deploy-angular-application-to-firebase-using-github/)
...
默认情况下，所有 Angular 项目都被设置为开发，所以让我们构建我们的项目并生成我们的 `dist` 文件。
...
*   Firebase 提供各种服务，如数据库、Firestore、函数、托管、存储。向下滚动，按空格键选择`托管`，然后按回车键继续。
...
*   输入您之前生成的包含 Angular 构建的 `dist` 文件夹的路径。在我的例子中，它是`dist/项目名称`。

![](img/3cb7cc8d4a7b454f48d48b942f174a1e.png)

*   现在选择`是`选项`设置自动构建并使用 GitHub 部署？`
*   以 `用户名/存储库` 格式输入您的 GitHub 用户名和存储库名称，选项为`您希望为哪个 GitHub 存储库设置 GitHub 工作流？`
