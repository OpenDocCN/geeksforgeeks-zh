...
> 原文：[https://www.geeksforgeeks.org/angularjs-ng-bind-html-directive/](https://www.geeksforgeeks.org/angularjs-ng-bind-html-directive/)

AngularJS 中的 `ng-bind-html` 指令用于将 HTML 元素的 `innerHTML` 绑定到应用程序数据，并从 HTML 字符串中移除危险代码。`ng-bind-html` 指令必须有 `$sanitize` 服务。所有 HTML 元素都支持它。
