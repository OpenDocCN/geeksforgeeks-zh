...
> 原文: [https://www.geeksforgeeks.org/angular-primeng-menu-component/](https://www.geeksforgeeks.org/angular-primeng-menu-component/)
...
菜单组件用于制作包含一些信息的组件&支持静态或动态定位。

## 属性

*   `model`: 是一组菜单。它接受数组数据类型作为输入&默认值为空。
*   `popup`: 用于定义菜单是否显示为弹出菜单。它是布尔数据类型&默认值为假。
*   `style`: 用于设置组件的内嵌样式。它是字符串数据类型&默认值为空。
...
*   `showTransitionOptions`: 用于显示表演动画的过渡选项。它是字符串数据类型&默认值是. 12s 三次贝塞尔曲线(0，0，0.2，1)。
...
<h2>GeeksforGeeks</h2>
<h5>PrimeNG Menu Component</h5>
<button type="button" pButton pRipple label="Click Here" (click)="menu.toggle($event)"></button>
...
参考: [https://primefaces.org/primeng/showcase/#/menu](https://primefaces.org/primeng/showcase/#/menu)