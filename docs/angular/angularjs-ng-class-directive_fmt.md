...
AngularJS 中的 `ng-class` 指令用于指定 HTML 元素上的 CSS 类。它用于动态绑定 HTML 元素上的类。如果 `ng-class` 指令中的表达式返回 `true`，则只添加该类，否则不添加。所有 HTML 元素都支持它。
