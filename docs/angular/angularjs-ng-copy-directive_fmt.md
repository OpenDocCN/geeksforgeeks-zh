...
AngularJS 中的 `ng-copy` 指令用于指定在输入文本字段中复制文本期间的自定义行为功能。当我们想要调用一个函数时，它可以被使用，当文本从输入域复制时，这个函数将被触发。所有输入元素都支持它。
...
其中 `expression` 告诉复制元素时要做什么。

## 示例

本示例使用 `ng-copy` 指令在元素将复制时显示一条消息。
