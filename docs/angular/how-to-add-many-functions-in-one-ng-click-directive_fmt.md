...
原文: [https://www.geeksforgeeks.org/how-to-add-many-functions-in-one-ng-click-directive/](https://www.geeksforgeeks.org/how-to-add-many-functions-in-one-ng-click-directive/)

AngularJS 中的 `ng-click` 指令用于在单击元素时应用自定义行为。它可以用来显示/隐藏某些元素，也可以在点击按钮时弹出警告。`ng-click` 指令是 AngularJS 中使用的一个非常方便的工具。当点击一个 HTML 元素时，`ng-click` 指令告诉 AngularJS 脚本该做什么。
...
关键是要加一个分号（`;`）或逗号（`,`）。所有的函数必须用一个（`;`）或一个（`,`）分隔。HTML 中的所有元素都支持这种语法。它基本上是一个表达式，当被点击时，就会被执行。

## 示例

本示例展示了如何在一次 `ng-click` 中添加多个功能。
