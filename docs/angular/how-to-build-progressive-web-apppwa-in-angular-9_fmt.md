...
> 原文: [https://www.geeksforgeeks.org/how-to-build-progressive-web-apppwa-in-angular-9/](https://www.geeksforgeeks.org/how-to-build-progressive-web-apppwa-in-angular-9/)
...
2.  **安全:** 使用 `HTTPS` 向我们的 PWA 提供数据确保了安全。
...
## 第一步: 初始化新的 Angular 项目

现在，我们先从创建 `angular` app 开始。在这里，我们将创建一个简单的天气应用程序。为此，首先，创建一个新的 `Angular App`，并使用以下命令在项目目录中导航。
...
我们将在开发前端时使用引导进行造型。在项目的 `index.html` 文件中添加以下链接。

```html
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta3/dist/js/bootstrap.bundle.min.js" integrity="sha384-Jew9zMc8r+ph31jmwh6wwp0wintqrmb4s7zodaudhntxwong2vi5dk3qm9ekf" crossorigin="anonymous"></script>
```

## 第三步: 获取天气数据的 OpenWeatherMap API

获取天气实况数据，我们将使用 [OpenWeatherMap](http://openweathermap.org/api) `API`。通过创建您的帐户获得一个应用编程接口密钥。

## 第 4 步: 开发天气应用程序的用户界面

通过运行以下命令创建名为 `weather` 的角度组件和名为 `API` 的角度服务:
...
## 第五步: 将 angular app 转换为 PWA

使用 `Angular CLI` 很容易将您的 `Angular app` 转换为 PWA。导航到项目的文件夹。现在，运行以下命令添加 PWA 功能。
...
3.  `assets/icons` 目录中有许多大小的默认图标(这些图标可以在以后更改)
...
你可以在这里找到这个天气应用的代码：`https://github.com/girishgr8/Angular-PWA`。

### 安装、启动与卸载应用

一旦你在浏览器中启动了 angular app，URL 栏右侧会出现一个下载图标，如下所示:
