...
> 原文：[https://www.geeksforgeeks.org/how-to-display-loading-screen-when-navigating-between-routes-using-angular/](https://www.geeksforgeeks.org/how-to-display-loading-screen-when-navigating-between-routes-using-angular/)
...
我们将创建一个简单的应用程序，它将在导航时模拟一些延迟，并在导航路线时显示一个装载微调器。让我们快速设置环境：
...
我们将从 `JSON` 文件而不是实际的服务器加载数据。在 `src/assets/` 中创建新文件 `data.json` 并添加以下数据。
...
要读取该数据，我们将使用 Angular 的 `HttpClient` 模块。我们需要在 `app.module.ts` 文件中注册它。
...
它将为每个组件生成 4 个文件。我们将理解其中一个的代码。另一个会有类似的工作。在 `batsman.component.ts` 中，添加以下代码。
...
首先，导入 `HttpClient` 类。我们在构造函数中创建了一个 `HttpClient` 对象作为依赖注入。我们还初始化了一个空的 `batsman` 数组。`HttpClient` 的 `get()` 方法将返回一个 `Observable`，该方法返回数据作为其 `subscribe(result_callback，error_callback)` 方法中的第一个参数。在回调中，我们模拟了 2000 毫秒的延迟，并推送了值为 0 的名称。这意味着一旦页面被加载，将有 2 秒的延迟，`batsman` 数组将被填充。现在将以下代码添加到 `batsman.component.html`。
...
有两个 `div` 标签。当 `batsman` 阵列为空时，显示第一个。另一个将在填充数组时显示。因此，在没有收到数据之前，我们将查看包含加载动画的第一个 `div`。`spinner-border` 类存在于引导程序中。所以我们需要在我们的项目中添加 bootstrap。编辑 `index.html` 文件如下。
...
现在分别给 `umpires.component.ts` 和 `umpires.component.html` 添加以下代码。
...
在 `app-routing.module.ts` 中为这些组件创建路由，如下所示：
...
在 `path` 键中添加路径，在 `component` 键中添加相应的组件。导入必要的组件。

### 步骤 7

现在在 `app.component.html` 为用户创建链接，编码部分完成：
...
上面的代码创建了两个导航到各自组件的链接。 `<router-outlet>` 标签显示导航组件。
