...
> 原文：[https://www.geeksforgeeks.org/template-reference-variable-in-angularjs/](https://www.geeksforgeeks.org/template-reference-variable-in-angularjs/)

**Angular 中的模板引用变量**用于访问 DOM 内部任意元素的所有属性。它也可以是对 Angular 组件或指令或 Web 组件的引用。
...
在上面的 HTML 模板中，我们用模板引用变量 `#geek` 创建了一个输入元素，并创建了一个按钮。单击按钮，我们访问完整的输入元素及其所有属性。
...
在模板驱动的表单中，我们实际上是使用模板引用变量来访问表单数据。
...
因此，在这里，我们使用表单元素上的模板引用变量 `#itemForm` 来访问表单数据，在表单提交上，我们显示通过表单字段提供的输入电子邮件。
