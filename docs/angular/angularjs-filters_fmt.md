...
*   `currency`：数字被格式化为货币格式。
*   `date`：日期被指定为特定的格式。
*   `filter`：根据提供的标准过滤阵列。
*   `limitTo`：数组或字符串被限制为指定数量的元素/字符。
*   `number`：如果格式化为字符串，则为数字。
*   `orderBy`：数组由表达式排序。
*   `lowercase`：该过滤器将字符串转换为小写字母。
*   `uppercase`：该过滤器将字符串转换为大写字母。
...
<p>This example displays only the names containing the letter "e".</p>
...
<p>Filter applied from first element to the fifth element.</p>
...
上面提到的代码要求用户输入。用户在输入框中输入一个术语后，该术语被存储在`ng-model="string"`中。现在，AngularJS将解析表达式，并返回表达式的确切位置。AngularJS表达式可以写在双大括号里面，比如: `{{ expression }}`。

**输出:**
![](img/a5530de6ccdac35f866a22b41afc56c9.png)

在这段代码中，输出`{{string}}`显示在输入框的正下方。但是，要将输入字符串更改为小写，`| lowercase`必须添加到表达式的名称中。

因此，`{{ string | lowercase }}`将以小写格式返回输入字符串。
...
这里，`spacing`指定每个缩进要使用的空格数。默认值为 2，但是该值是可选的。
...
<p>Click the button to get every element in the array that has a value of 38 or more.</p>
...
<script>
    ...
    function myFunction() {
        document.getElementById("demo").innerHTML = num.filter(checkNum);
    }
</script>
...
点击`Try it`按钮，数组中值为 38 或更大的所有元素都会打印到屏幕上。
