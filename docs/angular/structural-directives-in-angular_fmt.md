...
> 原文：[https://www.geeksforgeeks.org/structural-directives-in-angular/](https://www.geeksforgeeks.org/structural-directives-in-angular/)
...
在上面的语法中，`boolean`代表真值或假值。因此，它产生了如下两种有效语法：
...
在上面的语法中，`boolean`代表真值或假值。如果`boolean`为真，则在 DOM 上呈现 If 中的元素，否则在 DOM 上呈现另一个元素。
...
`*ngFor`用于循环遍历 DOM 中的动态列表。简单地说，它用于在 HTML DOM 中构建数据表示列表和表格。
...
在上面的语法中，用每个案例检查`expression`，然后在 DOM 上呈现与表达式匹配的案例，否则在 DOM 上呈现默认案例。
