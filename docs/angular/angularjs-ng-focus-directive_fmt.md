...
AngularJS 中的 `ng-focus` 指令用于在元素被聚焦时应用自定义行为。它可以用来显示/隐藏某个元素，或者在元素被聚焦时弹出警告。由 `<a>`、`<input>`、`<select>` 和 `<textarea>` 元素支持。
...
其中 `expression` 告诉当输入变得集中时该做什么。

### 示例 1

本示例使用 `ng-focus` 指令显示文本区域字段。
