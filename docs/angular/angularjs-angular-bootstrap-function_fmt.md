...
> 原文: [https://www.geeksforgeeks.org/angularjs-angular-bootstrap-function/](https://www.geeksforgeeks.org/angularjs-angular-bootstrap-function/)
...
*   `element`: `element`是一个 DOM 元素(例如文档)，它是 Angular 应用程序的根。
*   `modules`: (可选)`modules`是要加载的模块阵列。
*   `config`: (可选)`config`是用于配置选项的对象。
