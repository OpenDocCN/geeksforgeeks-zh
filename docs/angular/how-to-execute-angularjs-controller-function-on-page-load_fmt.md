...
> 原文：[https://www.geeksforgeeks.org/how-to-execute-angularjs-controller-function-on-page-load/](https://www.geeksforgeeks.org/how-to-execute-angularjs-controller-function-on-page-load/)
...
<!-- calling the firstFunction to initialize gfg variable -->
...
// We need $scope argument as we are altering the variables defined in the $scope
...
页面加载时调用函数，变量 `gfg` 的值设置为 `GeeksForGeeks`。

## 示例 2

在本例中，我们将为变量 `gfg` 分配一个对象并使用它。
...
<!-- initializing the gfg variable to 'GeeksForGeeks' -->
...
在页面加载时，变量 `gfg` 被赋予值 `GeeksForGeeks`。