...
> 原文: [https://www.geeksforgeeks.org/angularjs-ng-pluralize-directive/](https://www.geeksforgeeks.org/angularjs-ng-pluralize-directive/)

AngularJS 中的 `ng-pluralize` 指令用于根据 en-us 本地化规则指定要显示的消息。en-us 本地化规则与 AngularJS 捆绑在一起。AngularJS 中默认的复数类别是“一”和“其他”。
...
*   `count`: 指 Angular 表达式使用的计数属性值。
*   `when`: 用于指定实际字符串和复数类别之间的映射。属性值必须是 JSON 对象样式。
*   `offset`: 指定从总数中扣除的偏移量属性。

## 示例 1

本示例使用 `ng-pluralize` 指令显示内容。
...
本示例使用 `ng-pluralize` 指令显示输入的文本内容。
