...
> 原文：[https://www.geeksforgeeks.org/angularjs-ng-required-directive/](https://www.geeksforgeeks.org/angularjs-ng-required-directive/)

AngularJS 中的 `ng-required` 指令用于指定 HTML 元素所需的属性。只有当 `ng-required` 指令中的表达式返回 `true` 时，表单中的输入字段才是必需的。通过 `<input>`、`<select>` 和 `<textarea>` 进行支持。
...
本示例使用 `ng-required` 指令将表单标签的输入文本字段设置为必填。
...
选中复选框后，本示例使用 `ng-required` 指令创建必填字段。
