...
> 原文: [https://www.geeksforgeeks.org/angularjs-ng-readonly-directive/](https://www.geeksforgeeks.org/angularjs-ng-readonly-directive/)

AngularJS 中的 `ng-readonly` 指令用于指定一个 HTML 元素的 `readonly` 属性。只有当 `ng-readonly` 指令中的表达式返回 `true` 时，HTML 元素才是只读的。
...
本示例使用 `ng-readonly` 指令来启用 `readonly` 属性。
...
本示例使用 `ng-readonly` 指令来启用选项列表中的 `readonly` 属性。
