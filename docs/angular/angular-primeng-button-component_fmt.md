...
> 原文: [https://www.geeksforgeeks.org/angular-primeng-button-component/](https://www.geeksforgeeks.org/angular-primeng-button-component/)
...
它用于制作一个标准按钮，该按钮将指示可能的用户动作。

## 按钮的属性

*   `label`: 是按钮的文字。它是字符串数据类型&默认值为空。
*   `icon`: 是图标的名称。它是字符串数据类型&默认值为空。
*   `iconPos`: 指定图标的位置，有效值为“左”和“右”。它是字符串类型&默认值为左。
*   `loading`: 指定按钮是否处于加载状态。它是布尔数据类型&默认值为假。
*   `loadingIcon`: 是加载状态下显示的图标。它是字符串类型&默认值是 `pi pi-spinner pi-spin`。
*   `type`: 指定按钮的类型。它是字符串类型&默认值为空。
*   `badge`: 指定徽章值。它是字符串类型&默认值为空。
*   `badgeClass`: 指定徽章样式类。它是字符串类型&默认值为空。
*   `disabled`: 指定组件应禁用。它是布尔类型的&默认值为假。
*   `style`: 指定元素的内嵌样式。它是字符串类型&默认值为空。
*   `styleClass`: 指定元素的样式类。它是字符串类型&默认值为空。
*   `onClick`: 用于按钮被点击时回调执行。它属于事件类型&，默认值为空。
*   `onFocus`: 用于按钮对焦时回调执行。它属于事件类型&，默认值为空。
*   `onBlur`: 用于按钮失焦时回调执行。它属于事件类型&，默认值为空。
