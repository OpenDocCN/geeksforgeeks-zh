...
AngularJS 中的 `ng-mousemove` 指令用于在特定的 HTML 元素上发生 `mousemove` 事件时应用自定义行为。当鼠标移动到特定的 HTML 元素上时，它可以用来显示弹出警告。所有 HTML 元素都支持它。
...
本示例使用 `ng-mousemove` 指令更改 CSS 样式。
