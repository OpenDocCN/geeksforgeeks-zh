...
AngularJS 中的 `ng-cut` 指令用于指定输入字段中的文本被剪切时的自定义行为功能。当我们想要调用一个函数时，它可以被使用，当文本从输入域中被剪切时，这个函数将被触发。所有输入元素都支持它。
...
其中 `expression` 告诉输入被切时该做什么。

## 示例

本示例使用 `ng-cut` Directive 在剪切输入文本元素时显示消息。
