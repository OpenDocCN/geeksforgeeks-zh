...
> 原文: [https://www.geeksforgeeks.org/angular-primeng-colorpicker-component/](https://www.geeksforgeeks.org/angular-primeng-colorpicker-component/)
...
*   `inline`: 用于指定是否显示为叠加。它是布尔数据类型&默认值为假。
*   `format`: 用于设置值绑定中使用的格式，支持的格式有十六进制、RGB & HSB。为字符串数据类型&，默认值为十六进制。
*   `appendTo`: 用于设置附着叠加的目标元素。它是字符串数据类型&默认值为空。
*   `tabindex`: 用于按 tab binding 顺序设置元素的索引。它是数字数据类型&默认值为空。
*   `disabled`: 用于指定组件应禁用。它属于布尔数据类型&默认值为假。
...
*   `showTransitionOptions`: 用于设置显示动画的过渡选项。它是字符串数据类型，默认值是. 12s 三次贝塞尔曲线(0，0，0.2，1)。
*   `hideTransitionOptions`: 用于设置隐藏动画的过渡选项。它是字符串数据类型，默认值为. 1s 线性。
...
*   `p-colorpicker-panel`: 它是拾色器的造型面板元素。
*   `p-colorpicker-content`: 它是一个包含颜色和色调部分的包装器。
*   `p-colorpicker-color-selector`: 它是一个颜色选择器。
*   `p-colorpicker-color`: 它是一个造型颜色元素。
*   `p-colorpicker-color-handle`: 是颜色元素的句柄。
*   `p-colorpicker-hue`: 是色相滑块。
*   `p-colorpicker-hue-handle`: 是色相滑块的手柄。
...
有两个属性可以使用颜色选择器，第一个属性使用 `inline`&第二个属性使用覆盖。

## 示例 1

在这个示例中，我们将使用一个 `inline` 属性来选择颜色&这是展示如何使用 `colorPicker` 组件的基本示例。
...
在本例中，我们将了解如何使用 `overlay` 属性在 `colorPicker` 组件中选择颜色。
...
**参考:** [https://primefaces.org/primeng/showcase/#/colorpicker](https://primefaces.org/primeng/showcase/#/colorpicker)