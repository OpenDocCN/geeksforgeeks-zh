...
> 原文：[https://www.geeksforgeeks.org/how-to-generate-qr-codes-with-angular-10/](https://www.geeksforgeeks.org/how-to-generate-qr-codes-with-angular-10/)
...
在您的本地开发机器中，`Node.js` 10+ 和 `npm` 6+ 必须已经安装。

*   `Node.js` 可从 [https://nodejs.org/en/download/](https://nodejs.org/en/download/) 安装。
*   一旦安装完毕，我们可以通过使用以下命令检查版本：
    ```ts
    node --version
    ```
...
## 第二步：创建一个新的 Angular 应用

我们可以使用 Visual Studio Code 编辑器甚至从命令行提示符，我们可以创建一个生成二维码的示例项目：
```ts
ng new <projectname>
```
...
将安装必要的软件包。现在 `angular 10qrcode generation` 的项目结构为：
...
```
在 `@NgModule` 的 `imports` 数组中给出：
```ts
@NgModule({
  imports: [ NgxQRCodeModule ]
})
```
我们需要额外导入 `FormsModule`。所以，我们的 `src/app/app.module.ts` 片段如下：
...
现在库已经导入，我们可以在 Angular 应用中使用 `ngx-qrcode` 组件。
...
构建项目的步骤（在 `package.json` 文件所在的位置执行）：
```ts
ng build
```
...
运行项目的步骤（在 `package.json` 文件所在的位置执行）：
```ts
ng serve
```
由于代码被设置为在端口 4200 上运行，当访问 `http://localhost:4200/` 时，我们可以看到如下输出：

![](img/77ea647f58a81fe722839653091e5f68.png)

我们可以指定任何有效的网站网址，我们可以在 Angular 10 中成功生成二维码。在 CSS 中，我们可以美化阴影显示。让我们看看谷歌网站的输出：

![](img/c8fd970e95d4952708a74fb9cdab5d4c.png)

## 结论

`npm` Angular 中有很多像二维码生成器这样的重要库。我们可以根据需要有效地使用它们。二维码是任何应用/移动应用等的重要组成部分。