...
同样，Angular 框架也提供了各种选项，以在运行时实现更高的性能。其中一些是 `AOT` 编译，捆绑，缩小等。在 `AOT` 编译中，Angular 编译器在应用程序部署之前运行，因此减少了应用程序的整体大小。但是默认情况下，Angular 框架不会发生这种情况。

现在，让我们来谈谈 Angular 框架采用的默认流程，它帮助开发人员实现更高的性能，即捆绑和缩小。让我们在下面详细讨论它们:

`捆绑`和`缩小`是 Angular 采用的两个重要过程，目的是缩小应用程序的大小，以便在浏览器中快速下载。
