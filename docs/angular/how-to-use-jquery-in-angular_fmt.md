...
要使用 NPM 方法安装 jQuery，我们需要在 VS Code 终端运行命令来创建一个新的 angular 应用程序。
...
浏览 **https://jquery.com/download/** 时，你可以轻松找到 jQuery CDN 并下载它。
始终建议坚持使用官方 CDN 的最新版本，因为它支持子资源完整性 (SRI)。现在要使用 jQuery CDN，你需要直接从 jQuery CDN 域在 script 标签中引用该文件。带有子资源完整性属性的代码将如下所示。这里使用的是 jQuery 3.4.1。
...
该步骤包括浏览到位于 Angular CLI 项目文件夹根的文件 **“Angular-CLI . JSON”**，并找到 `scripts:[]` 属性，并按照给定的路径包含 jQuery 文件夹。
...
现在确认这个路径，浏览到 `node_modules->jquery->dist->jquery.min.js`。
...
现在，为了跟进本教程，我们需要在 `app.component.html` 中包含 Html 代码。
...
我们需要在 `app.component.ts` 中包含以下代码，以使按钮执行一个动作。
...
在本例中，我们使用 angular 中的 jquery 在 Html 中制作一个字段的动画。我们在 `app.controller.ts` 写 Html 代码，在 `app.controller.ts` 写 angular Code/jquery。
