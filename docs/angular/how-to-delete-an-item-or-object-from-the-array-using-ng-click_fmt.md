...
> 原文：[https://www.geeksforgeeks.org/how-to-delete-an-item-or-object-from-the-array-using-ng-click/](https://www.geeksforgeeks.org/how-to-delete-an-item-or-object-from-the-array-using-ng-click/)
...
*   **`indexno`**：这是需要的数量。定义是指定在什么位置添加/删除项目的整数。如果是负数表示从数组末尾指定位置。
*   **`nofitems(n)`**：这是可选数量。这表示要删除的项目数量。如果设置为 `0`，则不会删除任何项目。
*   **`item-1`、…`item-n`**：这也是可选数量。这表示要添加到阵列中的新项目。

## 示例

让我们更加关注示例。这里我们将通过例子来证明删除操作。这里给出了在极客网站上有账号的学生的名字。我们将尝试从学生姓名数组中删除其中一个姓名。
...
<div ng-app="studentNames" ng-init="names= ['Madhavi', 'Shivay', 'Priya']">
    <ul>
...
<p>Click the small x given in front of name to remove an item from the name list.</p>
