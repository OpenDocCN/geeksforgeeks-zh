...
在这里，我们将解释在 Angular 中可以与键盘输入一起使用的键盘事件和选项。有各种各样的选项可以用于按键事件，但首先让我向您解释什么是按键事件。

按键事件是一个 Angular 事件绑定，响应任何 DOM 事件。这是一个同步事件，在用户与基于文本的输入控件交互时触发。
当用户按下并释放一个键时，发生 `keyup` 事件。用于基于文本的输入控件时，通常用于在每次按键后获取值。
...
*   创建一个将接受输入的 `input` 元素。
*   在该输入元素中，通过传递 `$event` 调用 `keyup` 事件上的用户定义函数。
*   将 `$event` 转换为 TypeScript 文件中的用户定义函数，并通过 `event.target.value` 从事件变量中获取值，并将其添加到局部变量中，以查看击键时的同步变化。

## 示例

在这个实现中，`$event` 被传递给 TypeScript 文件中的用户定义函数 `onKeyUp()`。该功能将每次击键后输入的每个值添加到定义的 `text` 变量中，然后显示 `text` 变量。
...
每次空格键被触发，`text` 变量的值就会被更新。

### `keyup.enter`

`keyup.enter` 事件用于在按下回车键时生成事件。让我们以上面的例子为例，将 `keyup` 改为 `keyup.enter`。
...
每次触发回车时，`text` 变量的值都会更新。

### `keyup.a` (anyCustomCharacter)

当您按下您在键盘事件中提到的任何字符时，此事件被触发。字符可以是 (a-z, A-Z, 0-9 或任何其他特殊字符)。例如 `keyup.z`：这里的变化将显示在 z 字符触发。
...
每次触发 esc 键时，`text` 变量的值都会更新。
与上面类似，还有许多其他选项可以与快捷键一起使用。
...
```ts
<!--after shift is pressed event is passed to function -->
...
```ts
<!--after control is pressed event is passed to function -->
...
```ts
<!--after alt is pressed event is passed to function -->
...
`keyup.backspace` 事件用于在按下 backspace 键时生成事件。
...
```ts
<!--after backspace is pressed event is passed to function -->
