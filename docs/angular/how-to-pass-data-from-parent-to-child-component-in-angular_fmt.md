...
> 原文：[https://www.geeksforgeeks.org/how-to-pass-data-from-parent-to-child-component-in-angular/](https://www.geeksforgeeks.org/how-to-pass-data-from-parent-to-child-component-in-angular/)
...
`app.component.ts` 有两个数组。一个用于 `list_prog` – 语言列表，`prog_details` – 语言详细信息。
...
这里我们已经在父组件中添加了我们的子组件，并传递了两个数据，一个是 `prog_title`，另一个是 `curr_info`。但是请确保您已经在子组件中声明了变量 `details` 和 `title`。
...
### `cdetail.component.ts`

这里我们在子组件中使用了 `@Input()` 指令。现在在子组件中，可以使用父组件传递的数据。
