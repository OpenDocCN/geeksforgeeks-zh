...
*   在角度项目的`app`文件夹下新建一个文件夹。为此，请在您觉得合适的任何 `IDE` 中打开您的项目。我们将在这里使用 `VS 代码`集成开发环境。现在右键单击“app”，然后单击新文件夹，或者单击上面的图标来创建文件夹，或者从文件资源管理器手动创建文件夹。

![](img/3d689e70d2fae31bfaac964fa3a68a03.png)

创建新文件夹

*   现在创建一个 `typescript` 文件。就当是`customer.component.ts`吧。现在我们给这个文件添加内容，比如模板、选择器、样式等。
...
*   最后也是最后一步，转到`app.component.html`文件，添加您在自定义组件`customer.component.ts`中添加的选择器。
