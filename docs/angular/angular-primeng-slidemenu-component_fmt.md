...
> 原文: [https://www.geeksforgeeks.org/angular-primeng-slidemenu-component/](https://www.geeksforgeeks.org/angular-primeng-slidemenu-component/)
...
*   `model`: 是一个菜单项的数组。它接受数组数据类型&默认值为空。
*   `popup`: 定义菜单是否显示为弹出菜单。它属于布尔数据类型&默认值为假。
*   `style`: 是组件的内嵌样式。它接受字符串数据类型&默认值为空。
*   `styleClass`: 是组件的样式类。它接受字符串数据类型&默认值为空。
*   `transitionOptions`: 是用来滑动的动画。它接受字符串数据类型&默认值是缓和。
*   `effectDuration`: 是滑动动画的持续时间，单位为毫秒。它接受任何数据类型作为输入&默认值为 250。
*   `backLabel`: 是一个元素要后退导航的标签。它接受字符串数据类型&默认值回来了。
...
*   `showTransitionOptions`: 这些是显示动画的过渡选项。它接受字符串数据类型&默认值为. 12s 三次贝塞尔曲线(0，0，0.2，1)。
...
### 示例 2
在本例中，我们将使用弹出菜单制作一个 `slidemenu` 组件。
...
**参考:** [https://primefaces.org/primeng/showcase/#/slidemenu](https://primefaces.org/primeng/showcase/#/slidemenu)