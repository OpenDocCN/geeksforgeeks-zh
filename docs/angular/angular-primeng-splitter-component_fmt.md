...
*   `panelSize`: 用于指定的尺寸。它是数字数据类型&默认值为空。
*   `minSize`: 用于指定元素的最小大小。它是数字数据类型&默认值为空。
*   `layout`: 用于设置面板的方向。它是字符串数据类型&默认值是水平。
*   `gutterSize`: 用于以像素为单位指定分割线的大小。它是数字数据类型&默认值为 4。
*   `stateKey`: 用于指定有状态拆分器的存储标识符。它是字符串数据类型&默认值为空。
*   `stateStorage`: 用于定义有状态拆分器保持其状态的位置。它是字符串数据类型&默认值是会话。
*   `style`: 用于指定组件的内嵌样式。它属于对象数据类型&，默认值为空。
...
*   `onResizeStart`: 这是一个回调，在开始调整大小时触发。
*   `onResizeEnd`: 是调整大小结束时触发的回调。

## 样式类

*   `p-splitter`: 是容器元素。
*   `p-splitter-resizing`: 是调整大小时的容器元素。
*   `p-splitter-horizontal`: 是水平布局的容器元素。
*   `p-splitter-vertical`: 是垂直布局的容器元素。
*   `p-splitter-panel`: 是分路器面板元件。
*   `p-splitter-gutter`: 这是调整面板大小时要使用的檐槽元素。
*   `p-splitter-gutter-handle`: 是天沟的手柄元件。
...
在本例中，我们将了解如何在拆分器组件中使用`layout`属性。
...
**参考:**[https://primefaces.org/primeng/showcase/#/splitter](https://primefaces.org/primeng/showcase/#/splitter)