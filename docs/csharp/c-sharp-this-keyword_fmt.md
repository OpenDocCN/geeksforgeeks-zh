...
`this` 关键字是用来引用类的当前实例的。它用于从构造函数、实例方法和实例访问器中访问成员。`this`关键字也用于跟踪实例，该实例被调用来执行与该实例相关的一些计算或进一步处理。以下是在 C# 中使用“this”关键字的不同方式:
