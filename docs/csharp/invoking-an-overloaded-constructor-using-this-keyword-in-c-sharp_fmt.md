...
> 原文：[https://www.geeksforgeeks.org/invoking-an-overloaded-constructor-using-this-keyword-in-c-sharp/](https://www.geeksforgeeks.org/invoking-an-overloaded-constructor-using-this-keyword-in-c-sharp/)

## 先决条件
[C# 中的构造函数](https://www.geeksforgeeks.org/c-sharp-constructors/)
...
**说明：**在上例中，`Geek`类包含两个构造函数，即`Geek(int a, double b, string c)`和`Geek(int a, int b)`，两者都是参数化构造函数。现在我们用`this(50, 2.9, "Hello")`关键字在`Geek(int a, int b)`中调用`Geek(int a, double b, string c)`构造函数。这里`this(50, 2.9, "Hello")`关键字包含的参数数量和类型与`Geek(int a, double b, string c)`构造函数中的相同。