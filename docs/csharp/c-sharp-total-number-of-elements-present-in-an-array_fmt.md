...
> 原文：[https://www.geeksforgeeks.org/c-sharp-total-number-of-elements-present-in-an-array/](https://www.geeksforgeeks.org/c-sharp-total-number-of-elements-present-in-an-array/)

## 语法
`Array.GetLength(Int32)`方法用于查找数组指定维度中存在的元素总数。
```cs
public int GetLength (int dimension);
```
这里`dimension`是需要确定长度的数组的从零开始的维度。

## 返回值
该方法的返回类型为`System.Int32`。此方法返回一个32位整数，表示指定维度中的元素数量。

## 异常
如果`dimension`的值小于零或者`dimension`的值等于或大于`Rank`，该方法将给出`IndexOutOfRangeException`。
...
## 参考
[https://docs.microsoft.com/en-us/dotnet/api/system.array.getlength?view=netcore-2.1](https://docs.microsoft.com/en-us/dotnet/api/system.array.getlength?view=netcore-2.1)