# 如何在 C# 中设置 GroupBox 中内容的字体？

> 原文：[https://www.geeksforgeeks.org/how-to-set-the-font-of-content-present-in-the-groupbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-font-of-content-present-in-the-groupbox-in-c-sharp/)

在 Windows 窗体中，`GroupBox` 是一个容器，其中包含多个控件，这些控件相互关联。或者换句话说，`GroupBox` 是一组控件周围的框架显示，带有合适的可选标题。使用一个 `GroupBox` 可以对一个组中的相关控件进行分类。在分组框中，可以使用 `Font` 属性设置表单中分组框中文本的字体。您可以通过两种不同的方式设置此属性：

## 1. 设计时

设置 `GroupBox` 字体最简单的方法，如下图步骤所示：

*   **第一步：** 创建如下图所示的窗口表单：
    **Visual Studio -> File -> New -> Project -> Windows Forms App**
    ![](img/de9202f1f4646167e60ea580d67273d9.png)
*   **第二步：** 接下来，将 `GroupBox` 控件从工具箱拖放到窗体上，如下图所示：
    ![](img/d39194ec60f7e2cec314f73cca224221.png)
*   **第三步：** 拖放完成后，转到 `GroupBox` 的属性并设置其字体，如下图所示：
...
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法以编程方式设置 `GroupBox` 控件的字体：

```cs
public virtual System.Drawing.Font Font { get; set; }
```

这里，`Font` 代表分组框中内容的字体。以下步骤显示了如何动态设置 `GroupBox` 的字体：
...
*   **步骤 2：** 创建完 `GroupBox` 后，设置 `GroupBox` 类提供的 `Font` 属性。
