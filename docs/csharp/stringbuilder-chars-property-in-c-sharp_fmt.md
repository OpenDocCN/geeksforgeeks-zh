# StringBuilder 的 Chars[Int32] 属性 (C#)
...
`StringBuilder.Chars[Int32]` 属性用于获取或设置该实例中指定字符位置的字符。
...
> 这里的 `index` 是字符的位置。
>
> **属性值:** 该属性返回位置 `index` 处的 Unicode 字符。

## 异常

*   `ArgumentOutOfRangeException`: 如果在设置字符时索引超出了此实例的界限。
*   `IndexOutOfRangeException`: 如果获取字符时索引超出此实例的界限。
...
class GFG {
    ...
    public static void Main(String[] args)
    {
        ...
        // print the result
        Console.WriteLine("StringBuilder Object" + " contains = " + str);
        Console.WriteLine("Character at Position 1" + " in StringBuilder = " + ch);
    }
}
```
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.text.stringbuilder.chars?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.text.stringbuilder.chars?view=netframework-4.7.2)