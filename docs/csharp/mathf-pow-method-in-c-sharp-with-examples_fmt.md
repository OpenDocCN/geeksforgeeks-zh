# MathF.Pow() 方法在 C# 中的使用及示例

> 原文: [https://www.geeksforgeeks.org/mathf-pow-method-in-c-sharp-with-examples/](https://www.geeksforgeeks.org/mathf-pow-method-in-c-sharp-with-examples/)

在 C# 中，`MathF.Pow(Single, Single)` 是一个 `MathF` 类方法。这种方法用来计算一个数与另一个数的幂的乘积。

> **语法:** `public static float Pow (float x, float y);`
>
> **参数:**
> **x:** 是一个要加幂的单精度浮点数，这个参数的类型是 `System.Single`。
> **y:** 是单精度浮点数，指定幂或指数，该参数的类型为 `System.Single`。

**返回类型:** 函数返回升到幂的基数。这种方法的返回类型是 `System.Single`。
