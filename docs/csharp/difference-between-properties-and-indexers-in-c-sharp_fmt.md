...
> 原文: [https://www.geeksforgeeks.org/difference-between-properties-and-indexers-in-c-sharp/](https://www.geeksforgeeks.org/difference-between-properties-and-indexers-in-c-sharp/)
...
| 5. | 属性在创建时不需要 `this` 关键字。 | 索引器在其声明中需要 `this` 关键字。 |
| 6. | 属性的 `get` 访问器没有任何参数。 | 索引器的 `get` 访问器包含与索引器相同的参数列表。 |