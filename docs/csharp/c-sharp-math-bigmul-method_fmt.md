...
在 C# 中，`BigMul()` 是 `Math` 类的一个方法。此方法用于计算两个 32 位数字的全积。
...
> `a`：是第一个相乘的数字，这个参数的类型是 `System.Int32`。
> `b`：是第二个要相乘的数字，这个参数的类型是 `System.Int32`。

## 返回类型

该方法返回一个包含指定数字乘积的数字，该方法的返回类型为 `System.Int64`。
