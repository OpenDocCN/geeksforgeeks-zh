...
在 C# 中，`break` 语句用于终止循环（`for`、`if`、`while` 等）或某个条件下的开关语句。在终止控制后，控制将传递给中断语句后出现的语句（如果可用）。如果 `break` 语句存在于嵌套循环中，那么它将只终止那些包含 `break` 语句的循环。
...
在上面的例子中，内部循环被编程为执行 4 次迭代，但是当 `y` 的值大于 2 时，内部循环停止执行，因为我们使用 `break` 语句将内部循环的迭代次数限制为 2 次。然而，外环不受影响。
...
在上述示例中，循环终止所基于的循环条件始终为真。所以，这个循环将执行无限次，或者我们可以说永不终止。因此，这里我们使用 `break` 语句在 `x` 的值等于 7 时终止循环。
