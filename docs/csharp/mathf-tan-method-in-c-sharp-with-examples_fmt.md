...
## 语法
> 公静浮檀(浮 x)；
> 这里，x 是切线返回的角度(以弧度为单位)，该参数的类型为`System.单身`。

## 返回值
该方法将返回`System`类型的 *x* 的切线。单**。如果 *x* 等于 `NegativeInfinity`、`PositiveInfinity` 或 `NaN`，则该方法返回 `NaN`。
...
### 例 2:
展示 `MathF` 的工作原理。谭(单)法当论证是 `NaN` 还是无穷大。
