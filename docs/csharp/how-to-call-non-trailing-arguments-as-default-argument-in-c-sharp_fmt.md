...
> 原文：[https://www.geeksforgeeks.org/how-to-call-non-trailing-arguments-as-default-argument-in-c-sharp/](https://www.geeksforgeeks.org/how-to-call-non-trailing-arguments-as-default-argument-in-c-sharp/)
...
// 我们希望将 b 值赋为默认值，并将 a、c 作为参数传递给 defArgs 方法
defArgs(100, 200);
```

这里，我们想给 `a` 分配 100，给 `c` 分配 200，给 `b` 留默认值，但实际发生的情况是，给 `a` 分配 100，给 `b` 分配 200，给 `c` 留默认值。在调用 `defArgs()` 方法时，我们可以通过对 `b` 后面的参数使用 `parameter_name` 和冒号来调用 `b`（即非尾随参数）作为默认参数。
...
*   现在，从主方法调用 `defArgs()` 方法的非尾随参数作为默认参数。例如 `defArgs(100, c:200)`，这里我们想给 `a` 赋值 100，给 `c` 赋值 200，给 `b` 赋值默认值。
...
*   在 `defArgs()` 方法中，使用 `Console.WriteLine()` 方法打印参数的值。
