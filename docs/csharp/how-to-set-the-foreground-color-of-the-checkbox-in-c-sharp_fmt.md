...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-foreground-color-of-the-checkbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-foreground-color-of-the-checkbox-in-c-sharp/)

`CheckBox` 控件是 Windows 窗体的一部分，用于接受用户的输入。或者换句话说，`CheckBox` 控件允许我们从给定的列表中选择单个或多个元素。在 `CheckBox` 中，您可以使用 `CheckBox` 的 `ForeColor` 属性来设置 `CheckBox` 的前景色。这会让你的 `CheckBox` 更有吸引力。
这个属性是一个环境属性意味着如果我们不设置这个属性的值，那么它将从父控件中检索。在 Windows 窗体中，可以通过两种不同的方式设置该属性：

## 设计时

使用以下步骤设置 `CheckBox` 的 `ForeColor` 属性是最简单的方法：
...
比上面的方法稍微复杂一点。在此方法中，您可以使用以下语法以编程方式设置 `CheckBox` 的 `ForeColor` 属性：

```csharp
public virtual System.Drawing.Color ForeColor { get; set; }
```

这里，`Color` 用于表示复选框的前景色。以下步骤用于设置 `CheckBox` 的 `ForeColor` 属性：
...
*   **第 3 步：** 最后使用 `Add()` 方法将该 `CheckBox` 控件添加到窗体中。
