...
> 原文：[https://www.geeksforgeeks.org/c-sharp-set-all-bits-in-the-bitarray-to-the-specified-value/](https://www.geeksforgeeks.org/c-sharp-set-all-bits-in-the-bitarray-to-the-specified-value/)

`BitArray` 类管理一个紧凑的位值数组，这些位值被表示为布尔值，其中 `true` 表示该位在上是**，即 `1`，`false` 表示该位在**下是**，即 `0`。这个类包含在 `System.Collections` 命名空间。

`BitArray.SetAll(bool)` 方法用于将 `BitArray` 中的所有位设置为指定值。
...
这里， `value` 是分配给所有位的布尔值。

**注：** 此方法为 O(n) 运算，其中 n 为 `Count`。

下面的程序说明了 `BitArray.SetAll(bool)` 方法的使用：
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.collections.bitarray.setall?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.bitarray.setall?view=netframework-4.7.2)