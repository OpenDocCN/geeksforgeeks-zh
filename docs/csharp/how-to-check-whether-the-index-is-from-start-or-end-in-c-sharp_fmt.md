...
> 原文：[https://www.geeksforgeeks.org/how-to-check-whether-the-index-is-from-start-or-end-in-c-sharp/](https://www.geeksforgeeks.org/how-to-check-whether-the-index-is-from-start-or-end-in-c-sharp/)

在 C# 8.0 中引入了[索引结构](https://www.geeksforgeeks.org/index-struct-in-c-sharp-8-0/)。它表示可用于索引集合或序列的类型，并且可以从开始或结束开始。您可以在索引结构提供的`索引属性`的帮助下，检查给定的索引是从给定集合或序列的开始还是结束。如果`IsFromEnd`属性返回`false`，则表示索引从开始，或者如果`IsFromEnd`属性返回`true`，则表示索引从结束。
