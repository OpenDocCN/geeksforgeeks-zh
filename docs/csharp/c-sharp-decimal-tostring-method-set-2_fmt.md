# C# | Decimal.ToString 方法 | Set -2

> 原文: [https://www.geeksforgeeks.org/c-sharp-decimal-tostring-method-set-2/](https://www.geeksforgeeks.org/c-sharp-decimal-tostring-method-set-2/)

`Decimal.ToString()` 方法用于将当前实例的数值转换为其等价的字符串表示形式。该方法重载列表中有 4 种方法如下：
...
**返回值：** 该方法以 `provider` 参数指定的格式返回当前对象值的字符串表示形式。
