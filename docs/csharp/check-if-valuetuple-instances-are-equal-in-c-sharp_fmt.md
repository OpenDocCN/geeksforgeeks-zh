...
> 原文: [https://www.geeksforgeeks.org/check-if-valuetuple-instances-are-equal-in-c-sharp/](https://www.geeksforgeeks.org/check-if-valuetuple-instances-are-equal-in-c-sharp/)
...
在值元组中，可以使用 `ValueTuple.Equals` 方法检查两个值元组是否相同。此方法将返回一个值，该值指示给定的 `ValueTuple` 实例是否等于指定的对象。如果给定的元组相等，它将返回 `true`，否则返回 `false`。该方法可以通过两种不同的方式重载:

1.  `Equals(ValueTuple)` 方法
2.  `Equals(Object)` 方法

## `Equals(ValueTuple)` 方法

`Equals(ValueTuple)` 方法用于检查两个 `ValueTuple` 实例是否相等。它总是返回 `true`。该方法的返回类型为 `System.Boolean`。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.valuetuple.equals?view=netframework-4.8](https://docs.microsoft.com/en-us/dotnet/api/system.valuetuple.equals?view=netframework-4.8)