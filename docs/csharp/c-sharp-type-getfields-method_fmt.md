...
> 原文: [https://www.geeksforgeeks.org/c-sharp-type-getfields-method/](https://www.geeksforgeeks.org/c-sharp-type-getfields-method/)

`Type.GetFields()`方法用于获取当前类型的字段。此方法的重载列表中有 2 种方法，如下所示:
...
> **语法:** `public FieldInfo[] GetFields();`
>
> **返回值:** 该方法返回一个表示为当前类型定义的所有公共字段的`FieldInfo`对象的数组。或者，如果没有为当前类型定义公共字段，则为`FieldInfo`类型的空数组。

以下程序说明了`Type.GetFields()`方法的使用:
...
> **语法:** `public FieldInfo[] GetFields(BindingFlags bindingAttr);`
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.type.getfields?view=netframework-4.8](https://docs.microsoft.com/en-us/dotnet/api/system.type.getfields?view=netframework-4.8)