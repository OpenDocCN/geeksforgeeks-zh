...
> 原文：[https://www.geeksforgeeks.org/c-sharp-adding-elements-to-the-end-of-the-arraylist/](https://www.geeksforgeeks.org/c-sharp-adding-elements-to-the-end-of-the-arraylist/)

`AddRange(ICollection)`方法用于将`ICollection`的元素添加到数组列表的末尾。或者换句话说，这个方法用于将其他集合中的多个元素添加到数组列表中。在这里，元素被定义为基元或非基元类型。
...
这里，`col`是一个`ICollection`，它的元素应该被添加到数组列表的末尾。集合本身不能为`null`，但它可以包含`null`元素。

## 异常

*   如果`col`的值为`null`，则该方法将给出`ArgumentNullException`。
