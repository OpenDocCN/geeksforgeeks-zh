...
> 原文: [https://www.geeksforgeeks.org/preprocessor-directives-in-c-sharp/](https://www.geeksforgeeks.org/preprocessor-directives-in-c-sharp/)
...
| `#else` | 如果 `#if` 的符号值计算为 false，则执行 `#else` 指令语句 |
| `#elif` | 创建复合条件指令，如果符号值为真，则执行该指令 |
| `#error` | 创建用户定义的错误 |
| `#warning` | 创建用户定义的警告 |
| `#line` | 修改编译器的默认行号 |
| `#region` | 指定了可以展开或折叠的代码块 |
| `#endregion` | 指定区域的结尾 |
| `#pragma` | 为编译文件提供编译器信息 |
| `#pragma warning` | 用于启用或禁用警告 |
| `#pragma checksum` | 为源文件创建校验和 |

## 示例 1: 使用 `#define`、`#if`、`#else` 和 `#endif`

让我们用几个例子来理解这个概念。在下面给出的代码中，我们使用 `#define` 来定义一个名为 `shape` 的符号。所以这意味着 `shape` 评估为真。在主菜单中，我们使用 `#if` 检查 `shape` 是否存在。因为它确实存在，并且编译器事先知道它，所以部分 `#else` 将永远不会被执行，并且被编译器视为注释。`#endif` 用于指示 if 终止。
...
## 示例 2: 使用 `#warning` 和 `#undef`

再考虑一个例子。在下面给出的代码中，我们特意删除了符号 `shape` 和 `shape_` 的定义。因为编译器找不到这些，所以它执行 `#else` 指令。在这里，我们生成用户定义的警告和错误。
...
## 示例 3: 使用 `#region` 和 `#endregion`

`#region` 将一组指令定义为一个代码块，编译器会一次性编译该代码块。`#endregion` 标志着区块的结束。下面的程序描述了同样的情况。
...
## 示例 4: 使用 `#pragma warning` 和 `#pragma checksum`

在下面的代码中，我们使用 `#pragma warning disable` 来禁用所有警告。在 main 内部，我们生成一个用户定义的警告来检查它是否已经被禁用。并且 `#pragma checksum` 用于帮助文件的调试。
