...
> 原文：[https://www.geeksforgeeks.org/c-sharp-sorteddictionary-item-property/](https://www.geeksforgeeks.org/c-sharp-sorteddictionary-item-property/)
...
与指定的 `key` 相关联的值。如果没有找到指定的 `key`，一个 get 操作抛出一个 `KeyNotFoundException`，一个 set 操作用指定的键创建一个新元素。

## 异常

*   `ArgumentNullException`：如果 `key` 为空。
*   `KeyNotFoundException`：如果检索到属性并且集合中不存在 `key`。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.sorteddictionary-2.item?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.sorteddictionary-2.item?view=netframework-4.7.2)