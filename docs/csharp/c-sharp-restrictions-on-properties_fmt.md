...
> 原文: [https://www.geeksforgeeks.org/c-sharp-restrictions-on-properties/](https://www.geeksforgeeks.org/c-sharp-restrictions-on-properties/)
...
我们在编写属性时必须遵循一些规则或限制，如下所示:

*   **属性不能通过 `ref` 或 `out` 参数传递给方法:** 属性不能通过 `out` 或 `ref` 传递，因为`属性实际上是方法`。在编译时，`ref`和`out`都不被认为是方法签名的一部分。因此，它会给出一个编译时错误。除了从程序中完全删除 `ref` 或 `out` 之外，没有解决这个限制的办法。
