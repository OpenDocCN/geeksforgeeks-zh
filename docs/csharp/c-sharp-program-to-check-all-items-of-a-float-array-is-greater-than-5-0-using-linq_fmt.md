...
> 原文: [https://www.geeksforgeeks.org/c-sharp-program-to-check-all-items-of-a-float-array-is-greater-than-5-0-using-linq/](https://www.geeksforgeeks.org/c-sharp-program-to-check-all-items-of-a-float-array-is-greater-than-5-0-using-linq/)
...
这里，`result` 是存储最终结果的布尔类型变量，`nums` 是 `float` 的数组，`All()` 方法包含检查给定数组中的每个元素是否满足条件的条件。这里，如果数组的所有元素都满足给定的条件，那么这个语句 `nums.All(element => element > 5.0F)` 将返回 `true`。如果任何元素不满足给定的条件，那么它将返回 `false`。
