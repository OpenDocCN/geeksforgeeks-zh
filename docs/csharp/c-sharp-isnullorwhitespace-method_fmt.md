...
> 原文: [https://www.geeksforgeeks.org/c-sharp-isnullorwhitespace-method/](https://www.geeksforgeeks.org/c-sharp-isnullorwhitespace-method/)

在 C# 中，`IsNullOrWhiteSpace()` 是一个字符串方法。用于检查指定的字符串是**空**还是只包含**空格**字符。如果一个字符串没有被赋值或者被显式赋值为 `null`，那么它将为 `null`。
...
该方法将采用类型为 `System.String` 的参数，并且这个方法将返回一个布尔值。如果方法的参数列表为 `null` 或 `String.Empty`，或者只包含空白字符，则返回 `True`，否则返回 `False`。
...
`IsNullOrWhiteSpace()` 方法有一个备选代码如下:
