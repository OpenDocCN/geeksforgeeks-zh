...
`Last` 运算符用于返回集合或序列的最后一个元素。或者返回指定给定条件的最后一个元素。该方法可以通过两种不同的方式重载：

*   `Last<TSource>(IEnumerable<TSource>)`：此方法返回给定集合或序列的最后一个元素。
*   `Last<TSource>(IEnumerable<TSource>, Func<TSource, Boolean>)`：该方法根据给定条件返回最后一个元素。
...
*   如果序列不包含指定给定条件的元素，或者序列为空，则抛出 `InvalidOperationException`。
