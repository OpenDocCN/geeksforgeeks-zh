...
在 Windows 窗体中，`FlowLayoutPanel` 控件用于在水平或垂直流动方向上排列其子控件。或者换句话说，`FlowLayoutPanel` 是一个容器，用于在其中水平或垂直组织不同或相同类型的控件。在 `FlowLayoutPanel` 控件中，可以使用 `BackColor` 属性设置控件的背景色。此属性是环境属性。您可以通过两种不同的方式设置此属性：

## 1. 设计时
设置 `FlowLayoutPanel` 的背景颜色最简单的方法，如下步骤所示：
...
## 2. 运行时
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法以编程方式设置 `FlowLayoutPanel` 控件的背景色：
