...
C# 中的泛型集合是在 `System.Collections.Generic` 命名空间中定义的。它提供了标准数据结构的一般实现，如链表、栈、队列和字典。这些集合是类型安全的，因为它们是泛型，这意味着只有那些与集合类型兼容的项才能存储在泛型集合中，这消除了意外的类型不匹配。泛型集合由接口和类的集合定义。下表包含了 `System.Collections.Generic` 命名空间的常用类：

| 类别名 | 描述 |
| --- | --- |
| `Dictionary<TKey, TValue>` | 它存储键/值对，并提供类似于非泛型哈希表类中的功能。 |
| `List<T>` | 它是一个动态数组，提供类似于非泛型数组列表类中的功能。 |
| `Queue<T>` | 先进先出列表，提供与非通用队列类类似的功能。 |
| `SortedList<TKey, TValue>` | 它是一个键/值对的排序列表，提供了类似于非泛型 sorted list 类中的功能。 |
| `Stack<T>` | 它是一个先进先出的列表，提供了与非泛型 Stack 类类似的功能。 |
| `HashSet<T>` | 它是独特元素的无序集合。它防止在集合中插入重复项。 |
| `LinkedList<T>` | 它允许快速插入和移除元素。它实现了一个经典的链表。 |
...
| 类别名 | 描述 |
| --- | --- |
| `ArrayList` | 它是一个动态数组，意味着数组的大小不是固定的，它可以在运行时增加和减少。 |
| `Hashtable` | 它表示基于键的哈希代码组织的键和值对的集合。 |
| `Queue` | 它表示先进先出的对象集合。当您需要先进先出访问项目时，可以使用它。 |
| `Stack` | 它是一种线性数据结构。输入/输出遵循后进先出模式。 |
...
| 类别名 | 描述 |
| --- | --- |
| `BlockingCollection<T>` | 它为实现 `IProducerConsumerCollection<T>` 的线程安全集合提供了阻塞和绑定功能。 |
| `ConcurrentBag<T>` | 它代表一个线程安全的、无序的对象集合。 |
| `ConcurrentDictionary<TKey, TValue>` | 它表示一个线程安全的键/值对集合，可以由多个线程同时访问。 |
| `ConcurrentQueue<T>` | 它表示线程安全的先进先出(FIFO)集合。 |
| `ConcurrentStack<T>` | 它表示线程安全的后进先出(LIFO)集合。 |
| `OrderablePartitioner<TSource>` | 它代表了一种将可排序数据源分成多个分区的特殊方式。 |
| `Partitioner` | 它为数组、列表和枚举提供了通用的分区策略。 |
| `Partitioner<TSource>` | 它代表了将数据源拆分成多个分区的一种特殊方式。 |