...
> 原文：[https://www.geeksforgeeks.org/c-sharp-check-if-two-arraylist-objects-are-equal/](https://www.geeksforgeeks.org/c-sharp-check-if-two-arraylist-objects-are-equal/)

`Equals(object)`方法继承自`Object`类，用于检查指定的数组列表对象是否等于另一个数组列表对象。
...
`Equals(object)`方法只检查两个数组列表引用是否引用同一个对象。如果两个对象不同，即使它们具有相同的值，它也会返回`false`。
