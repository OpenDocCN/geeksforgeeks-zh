# 队列（Queue）中的 `CopyTo()` 方法

> 原文：[https://www.geeksforgeeks.org/queue-copyto-method-in-c-sharp/](https://www.geeksforgeeks.org/queue-copyto-method-in-c-sharp/)
...
## 参数：
- `array`：一维[数组](https://www.geeksforgeeks.org/c-sharp-arrays/)，是从 `Queue` 复制的元素的目的地。数组必须具有从零开始的索引。
- `index`：数组中从零开始复制的索引。

## 异常：
- `ArgumentNullException`：如果 `array` 为 `null`。
- `ArgumentOutOfRangeException`：如果 `index` 小于零。
- `ArgumentException`：如果数组是多维的，或者源队列中的元素数量大于目标数组可以包含的元素数量。
...
## 参考：
- [https://docs.microsoft.com/en-us/dotnet/api/system.collections.queue.copyto?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.queue.copyto?view=netframework-4.7.2)