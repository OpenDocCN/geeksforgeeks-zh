...
> 原文：[https://www.geeksforgeeks.org/uri-isloopback-property-in-c-sharp-with-example/](https://www.geeksforgeeks.org/uri-isloopback-property-in-c-sharp-with-example/)

`Uri.IsLoopback` 属性是 `Uri` 类的属性，用于检查指定的 `Uri` 是否引用了本地主机。
...
**返回值：** 这个属性的返回类型是 `bool`，如果这个 `Uri` 引用了本地主机，则返回 `true`，否则返回 `false`。

**异常：** 如果此实例表示相对 `Uri`，则此属性将给出 `InvalidOperationException`，因为此属性仅适用于绝对 URI。
