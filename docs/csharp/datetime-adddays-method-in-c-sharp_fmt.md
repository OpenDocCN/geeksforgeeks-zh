# DateTime.AddDays() 方法在 C# 中

> 原文：[DateTime.AddDays() Method in C#](https://www.geeksforgeeks.org/datetime-adddays-method-in-c-sharp/)

此方法用于返回一个新的 `DateTime`，该对象的值是将指定的天数添加到此实例的值中。
...
这里 `value` 是整数和小数的天数。`value` 参数可以是负数或正数。

## 返回值

该方法返回一个对象，该对象的值是该实例表示的日期和时间与 `value` 表示的天数之和。

## 异常

如果生成的日期时间小于 `MinValue` 或大于 `MaxValue`，此方法将给出 `ArgumentOutOfRangeException`。

## 示例

以下程序说明了 `DateTime.AddDays(Double)` 方法的使用：
...
*   [DateTime.AddDays 方法 (System)](https://docs.microsoft.com/en-us/dotnet/api/system.datetime.adddays?view=netframework-4.7.2)