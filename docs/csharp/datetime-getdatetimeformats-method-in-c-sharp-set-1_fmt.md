...
> 原文: [https://www.geeksforgeeks.org/datetime-getdatetimeformats-method-in-c-sharp-set-1/](https://www.geeksforgeeks.org/datetime-getdatetimeformats-method-in-c-sharp-set-1/)
...
> **语法:** `public string[] GetDateTimeFormats()`
>
...
> **语法:** `public string[] GetDateTimeFormats(char format);`
> 这里需要一个标准的日期和时间格式字符串。
>
> **返回值:** 这个方法返回一个字符串数组，其中每个元素都是用格式标准日期和时间格式说明符格式化的这个实例的值的表示。
>
> **异常:** 如果 `format` 不是有效的标准日期和时间格式说明符，此方法将给出 `FormatException`。

以下程序说明了 `GetDateTimeFormats(char)` 方法的使用:
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.datetime.getdatetimeformats?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.datetime.getdatetimeformats?view=netframework-4.7.2)