# File.SetLastWriteTime 方法 (C#)

> 原文: [https://www.geeksforgeeks.org/file-setlastwritetime-method-in-csharp-with-examples/](https://www.geeksforgeeks.org/file-setlastwritetime-method-in-csharp-with-examples/)

`File.SetLastWriteTime(String)` 是一个内置的 `File` 类方法，用于设置指定文件上次写入的日期和时间。
...
*   `Path`: 这是要为其设置日期和时间信息的指定文件。
*   `Last write time`: 这是路径的指定本地最后写入日期和时间。
...
下面是说明 `File.SetLastWriteTime()` 方法的程序。
