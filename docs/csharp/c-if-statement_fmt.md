...
- `then`语句包含单个或多个语句，这些语句用大括号(`{}`)括起来。对于单个语句，大括号可以是可选的，如果花括号`{}`不与`if`语句一起使用，则紧挨着它的语句仅被认为与`if`语句相关联。
- `then`语句可以是任何类型的，就像它可以包含另一个`if-else`语句。
...
在上面的例子中，我们有两个字符串，即`x`和`y`，现在在第一个`if`语句中，我们将字符串`x`与字符串`y`进行比较，比较的结果为真。因此，`then`块执行并打印“两个字符串相等..！! "。现在在第二个`if`语句中，我们检查两个字符串是否相等，但是字符串是相等的，所以这个`if`语句的`then`块不会执行。
