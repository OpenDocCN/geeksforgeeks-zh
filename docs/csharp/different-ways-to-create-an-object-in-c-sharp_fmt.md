...
> 原文：[https://www.geeksforgeeks.org/different-ways-to-create-an-object-in-c-sharp/](https://www.geeksforgeeks.org/different-ways-to-create-an-object-in-c-sharp/)
...
类是引用类型，在运行时，引用类型的任何对象都被赋予空值，除非它是使用新运算符声明的。新运算符仅在运行时将内存中的空间分配给对象，这意味着分配是动态的。
...
引用只能用类名和引用名声明。引用不能独立存在。它必须被分配给同一个类的一个已经存在的对象。引用中所做的任何更改都将保存到它所引用的对象中。有点像化名。
...
如果需要多个相同类别的对象，可以创建一个对象数组。这将要求您首先声明数组，然后初始化每个元素{在这种情况下是对象}。您可以使用 for 循环进行初始化。
