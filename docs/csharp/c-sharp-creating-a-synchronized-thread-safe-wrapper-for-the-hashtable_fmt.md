...
> 原文：[https://www.geeksforgeeks.org/c-sharp-creating-a-synchronized-thread-safe-wrapper-for-the-hashtable/](https://www.geeksforgeeks.org/c-sharp-creating-a-synchronized-thread-safe-wrapper-for-the-hashtable/)

`Hashtable.Synchronized(Hashtable table)` 方法用于为[哈希表](https://www.geeksforgeeks.org/c-hashtable-class/)返回一个同步(线程安全)包装器。
...
### 返回值：
这个方法为哈希表返回一个同步的(线程安全的)包装。

### 异常：
如果 `table` 为 `null`，该方法将抛出 `ArgumentNullException`。
...
### 参考：
*   [https://docs.microsoft.com/en-us/dotnet/api/system.collections.hashtable.synchronized?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.hashtable.synchronized?view=netframework-4.7.2)