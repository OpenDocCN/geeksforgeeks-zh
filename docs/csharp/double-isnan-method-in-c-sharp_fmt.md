# Double.IsNaN() 方法在 C# 中

> 原文：[https://www.geeksforgeeks.org/double-isnan-method-in-c-sharp/](https://www.geeksforgeeks.org/double-isnan-method-in-c-sharp/)

在 C# 中，`Double.IsNaN()` 是 `Double` 结构的一种方法。此方法用于检查指定的值是否不是数字（`NaN`）。
...
## 返回类型
该函数返回布尔值，即如果指定值不是数字（`NaN`），则返回 `true`，否则返回 `false`。
