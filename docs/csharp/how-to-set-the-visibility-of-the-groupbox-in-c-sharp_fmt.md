...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-visibility-of-the-groupbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-visibility-of-the-groupbox-in-c-sharp/)

在 Windows 窗体中，`GroupBox` 是一个容器，其中包含多个控件，这些控件相互关联。或者换句话说，`GroupBox` 是一组控件周围的框架显示，带有合适的可选标题。或者使用一个组框对一个组中的相关控件进行分类。在组框中，可以使用 `Visible` 属性设置表单中组框的可见性。
如果该属性的值设置为 `true`，则该组框在屏幕上可见，如果该属性的值设置为 `false`，则该组框在屏幕上不可见。此属性的默认值为 `true`。您可以通过两种不同的方式设置此属性：

## 1. 设计时
设置组框可见性最简单的方法，如下步骤所示：

*   **第一步：** 创建如下图所示的窗口表单：
    `Visual Studio->File->New->Project->Windows Forms App`
    ![](img/de9202f1f4646167e60ea580d67273d9.png)
*   **第二步：** 接下来，从工具箱中拖放 `GroupBox` 控件到窗体上，如下图所示：
    ![](img/d39194ec60f7e2cec314f73cca224221.png)
*   **第三步：** 拖放完成后，转到 `GroupBox` 的属性窗口并设置其可见性，如下图所示：
...
## 2. 运行时
比上面的方法稍微复杂一点。在此方法中，您可以在给定语法的帮助下以编程方式设置 `GroupBox` 控件的可见性：
...
*   **步骤 2：** 创建完 `GroupBox` 后，设置 `GroupBox` 类提供的 `Visible` 属性。
