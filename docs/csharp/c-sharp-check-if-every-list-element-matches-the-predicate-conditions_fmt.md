...
> 原文：[https://www.geeksforgeeks.org/c-sharp-check-if-every-list-element-matches-the-predicate-conditions/](https://www.geeksforgeeks.org/c-sharp-check-if-every-list-element-matches-the-predicate-conditions/)

`List<T>.TrueForAll(Predicate<T>)` 用于检查 `List<T>` 中的每个元素是否符合指定谓词定义的条件。
...
## 参数：
> `match`：是 `Predicate<T>` 委托定义了检查元素的条件。

## 返回值：
如果 `List<T>` 中的每个元素都符合指定谓词定义的条件，则该方法返回 `true`，否则返回 `false`。如果列表没有元素，返回值为 `true`。

## 异常：
如果 `match` 为空，该方法将给出 `ArgumentNullException`。

以下程序说明了 `List<T>.TrueForAll(Predicate<T>)` 方法的使用：
...
## 参考：
*   [https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.list-1.trueforall?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.list-1.trueforall?view=netframework-4.7.2)