# File.OpenWrite(String) 方法详解与示例

> 原文: [https://www.geeksforgeeks.org/file-openwrite-method-in-csharp-with-examples/](https://www.geeksforgeeks.org/file-openwrite-method-in-csharp-with-examples/)

`File.OpenWrite(String)` 是一个内置的 `File` 类方法，用于打开现有文件或创建新文件进行写入。
...
*   `path`: 这是将要打开进行写入的指定文本文件。

**例外:**

*   `UnauthorizedAccessException`: 调用方没有所需的权限。或者 `path` 指定了只读文件或目录。
...
以下是说明 `File.OpenWrite(String)` 方法的程序。
