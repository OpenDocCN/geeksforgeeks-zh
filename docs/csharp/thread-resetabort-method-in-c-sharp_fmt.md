...
一个[线程类](https://www.geeksforgeeks.org/thread-class-in-c/)负责在多线程编程中创建和管理一个线程。它提供了一种称为`ResetAbort`的方法，负责取消当前线程的中止请求。它防止`ThreadAbortException`终止线程。
...
*   `ThreadStateException`: 如果当前线程没有调用中止。
*   `SecurityException`: 如果调用者没有当前线程所需的安全权限。
