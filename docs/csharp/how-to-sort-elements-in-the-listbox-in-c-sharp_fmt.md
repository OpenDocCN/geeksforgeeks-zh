...
> 原文：[https://www.geeksforgeeks.org/how-to-sort-elements-in-the-listbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-sort-elements-in-the-listbox-in-c-sharp/)

在 Windows 窗体中，`ListBox` 控件用于显示列表中的多个元素，用户可以从中选择一个或多个元素，这些元素通常显示在多个列中。在列表框中，您可以使用列表框的 `Sorted` 属性对列表框中的元素进行排序。此属性总是按字母顺序对字符串类型元素进行排序，按升序对整数类型元素进行排序。此属性的默认值为 `false`。您可以通过两种不同的方式设置此属性：

## 1. 设计时

对列表框中的元素进行排序是最简单的方法，如以下步骤所示：
...
*   **步骤 3：** 拖放完成后，您将进入 `ListBox` 控件的属性窗口以对元素进行排序。
    ![](img/28d908b024ea7aacdb9e7f8b077bc0cb.png)
...
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法，以编程方式对 `ListBox` 控件中的元素进行排序：

```csharp
public bool Sorted { get; set; }
```

这里，该属性的值为 `System.Boolean` 类型。如果要对列表框中的元素进行排序，请将该属性的值设置为 `true`。否则，为 `false`。以下步骤显示了如何动态设置列表框的 `Sorted` 属性：
