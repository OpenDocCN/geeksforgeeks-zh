...
> 原文: [https://www.geeksforgeeks.org/c-sharp-program-for-counting-lines-in-a-string/](https://www.geeksforgeeks.org/c-sharp-program-for-counting-lines-in-a-string/)
...
我们还可以使用 `Regex.Matches()` 方法计算给定字符串中的行数。这用于在指定的输入字符串中搜索给定正则表达式的所有匹配项。

**语法:**

> `Regex.Matches(string input, string matchpattern)`

在这里，这个方法使用两个名为 `inputstr` 的参数来搜索匹配的输入字符串，并使用 `matchpattern` 来表示与 `inputstr` 匹配的正则表达式模式。

**示例:**

在这个例子中，我们将在 `Matches()` 方法中传递一个正则表达式 `"\n"`，并找到它的出现次数，通过添加一个 1，我们将得到字符串中的行数。
