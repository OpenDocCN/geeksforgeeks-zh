...
`BitArray.CopyTo(Array, Int32)`方法用于将整个`BitArray`复制到兼容的一维数组中，从目标数组的指定索引处开始。
...
*   `ArgumentException`: 如果`arr`是多维的**或**源数组中的元素数量大于从`index`到目标数组末尾的可用空间。
...
*   该方法使用`Array.Copy`复制元素。
*   这个方法是一个 O(n)运算，其中 n 是`Count`。

## 参考:

*   [https://docs.microsoft.com/en-us/dotnet/api/system.collections.bitarray.copyto?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.bitarray.copyto?view=netframework-4.7.2)