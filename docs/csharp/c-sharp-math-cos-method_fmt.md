...
`Math.Cos()` 是一个内置的数学类方法，它返回给定双值参数(指定角度)的余弦值。
...
## 参数:
> **num:** 是返回余弦的角度(以弧度为单位)，该参数的类型为 `System.Double`。

## 返回值:
返回类型为 `System.Double` 的值。如果 `num` 等于 `NegativeInfinity`、`PositiveInfinity` 或 `NaN`，则该方法返回 `NaN`。

下面是说明 `Math.Cos()` 方法的程序。
...
### 程序 2:
展示当参数是 `NaN` 或无穷大时，`Math.Cos()` 方法的工作原理。
