...
> 原文：[https://www.geeksforgeeks.org/datetime-addseconds-method-in-c-sharp/](https://www.geeksforgeeks.org/datetime-addseconds-method-in-c-sharp/)
...
这里，`value` 是整数和小数的秒数。`value` 参数可以是负数或正数。

## 返回值

该方法返回一个对象，该对象的值是该实例表示的日期和时间与 `value` 表示的秒数之和。

## 异常

如果结果日期时间小于 `MinValue` 或大于 `MaxValue`，此方法将抛出 `ArgumentOutOfRangeException`。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.datetime.addseconds?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.datetime.addseconds?view=netframework-4.7.2)