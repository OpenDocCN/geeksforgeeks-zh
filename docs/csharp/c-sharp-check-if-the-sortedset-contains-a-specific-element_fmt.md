...
> 原文：`https://www.geeksforgeeks.org/c-sharp-check-if-the-sortedset-contains-a-specific-element/`

`SortedSet` 类表示按排序顺序排列的对象集合。这个类隶属于 `System.Collections.Generic` 命名空间。`SortedSet<T>.Contains(T)` 方法用于检查排序集是否包含特定元素。
...
**返回值：** 如果集合包含指定项目，则返回 `true`，否则返回 `false`。
