...
上例中，`My_Family` 为基类，`My_Member` 为派生类。在这两个类中，我们有相同名称的方法，即 `member()` 方法。但是在派生类中，`member()` 方法是用新关键字声明的。调用此方法时，它会打印家庭成员的姓名和年龄，而不是家庭成员的总数。这意味着当我们借助派生类对象调用 `member()` 方法时，由于新关键字的存在，它隐藏了基类中存在的同名方法。
...
> prog.cs(9,14): (location of previous symbol)
