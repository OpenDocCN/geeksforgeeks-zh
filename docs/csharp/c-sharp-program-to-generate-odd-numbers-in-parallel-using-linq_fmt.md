...
> 原文：[https://www.geeksforgeeks.org/c-sharp-program-to-generate-odd-numbers-in-parallel-using-linq/](https://www.geeksforgeeks.org/c-sharp-program-to-generate-odd-numbers-in-parallel-using-linq/)

LINQ 被称为语言集成查询，它是在 .NET 3.5 中引入的。它赋予了 .NET 语言生成查询以从数据源检索数据。它消除了编程语言和数据库之间的不匹配，并且无论使用哪种类型的数据源，用于创建查询的语法都是相同的。在本文中，我们将使用 LINQ 并行生成奇数。因此，我们将使用 `ParallelQuery{TSource}` 并行生成奇数。与此同时，我们必须使用 `where` 和 `select` 子句来获取奇数。
...
1.  Implement the `ParallelQuery{TSource}` and mention the range (i.e. 10 to 11).
...
4.  Use `foreach` loop to iterate the odd numbers.
