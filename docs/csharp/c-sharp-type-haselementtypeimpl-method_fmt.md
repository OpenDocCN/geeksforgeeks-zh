...
> 原文：[https://www.geeksforgeeks.org/c-sharp-type-haselementtypeimpl-method/](https://www.geeksforgeeks.org/c-sharp-type-haselementtypeimpl-method/)

`Type.HasElementTypeImpl()` 方法在派生类中被重写时使用，用于实现 `HasElementType` 属性并确定当前类型是包含还是引用另一个类型。这意味着此方法检查当前类型是数组、指针还是通过引用传递的。
...
如果类型是数组、指针或通过引用传递，则该方法返回 `true`，否则返回 `false`。

以下程序说明了 `Type.HasElementTypeImpl()` 方法的使用：
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.type.haselementtypeimpl?view=netframework-4.8](https://docs.microsoft.com/en-us/dotnet/api/system.type.haselementtypeimpl?view=netframework-4.8)