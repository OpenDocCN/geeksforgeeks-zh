...
> 原文: [https://www.geeksforgeeks.org/linq-generation-operator-defaultifempty/](https://www.geeksforgeeks.org/linq-generation-operator-defaultifempty/)
...
*   该运算符以两种不同的方式重载:
    *   `DefaultIfEmpty<TSource>(IEnumerable<TSource>, TSource)`: 如果序列为空，此方法用于返回单例集合中指定序列或指定值的元素。
    *   `DefaultIfEmpty<TSource>(IEnumerable<TSource>)`: 如果序列为空，此方法用于返回单例集合中指定序列的元素或类型参数的默认值。
*   它不支持 C# 和 VB.Net 语言中的查询语法。
*   它支持 C# 和 VB.Net 语言中方法语法。
*   它同时出现在 `Queryable` 类和 `Enumerable` 类中。
*   它通过使用延迟执行来实现。
*   如果给定的源为空，`DefaultIfEmpty<TSource>(IEnumerable<TSource>)` 将返回 `ArgumentNullException`。
*   引用类型和可空类型的默认值为 `null`。
