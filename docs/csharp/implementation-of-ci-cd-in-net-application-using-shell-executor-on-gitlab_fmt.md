# 在 .NET 应用程序中实施 CI/CD：使用 GitLab Shell Executor

> 原文：[https://www.geeksforgeeks.org/implementation-of-ci-cd-in-net-application-using-shell-executor-on-gitlab/](https://www.geeksforgeeks.org/implementation-of-ci-cd-in-net-application-using-shell-executor-on-gitlab/)
...
| 软件 | 描述 |
| --- | --- |
| Git | 这是在 GitLab 上提交变更的第一个要求。这是一个版本控制软件，可以跟踪文件的变化。 |
| GitLab Runner | GitLab Runner 的工作是获取并执行作业（当新的提交发生时）。 |
...
| `Nuget.exe` | 下载 `Nuget.exe` 并保存在 `MSBuild` 可用的目录中。 |

## 路径配置

| 软件/文件 | 路径描述 |
| --- | --- |
| Git | 在机器上成功安装 Git 后。需要设置以下路径来与 GitLab 存储库通信。转到系统环境变量，用用户变量路径添加这两个变量：<br>• `C:\Program Files\Git`<br>• `C:\Program Files\Git\bin` |
| `MSBuild.exe` | `MSBuild.exe` 路径将需要在 Yml 文件，而配置与 GitLab。例如，`C:\Program Files (x86)\Microsoft Visual Studio\2019\Enterprise\MSBuild\Current\Bin\MSBuild.exe` 或您的 `MSBuild` 可用的位置。 |
| `Nuget.exe` | `Nuget.exe` 路径也将需要在 Yml 文件，而配置与 GitLab。例如，`C:\Tools\Nuget\nuget.exe` 或您的 Nuget 可用的地方。 |
| `mstest.exe` | 这对于运行的测试用例非常有用。Net，在 Visual Studio 2019 中，它在 `C:\Program Files (x86)\Microsoft Visual Studio\2019\Enterprise\Common7\IDE\mstest.exe` 中可用。 |
| `.gitlab-ci.yml` | 这个文件应该在包含所有配置项/光盘配置（包括软件和脚本路径）的项目根目录中。在这里，您可以提到这个存储库应该如何运行。在将此文件添加到根目录之前，应该检查它是否是有效的 yml 文件。 |
...
### 2. 下载成功后，保存在你的目录下（例如：`C:\Tools\gitlab-Runner`），用以下命令重命名
...
### 8. 验证 GitLab Runner 已经注册了相应的存储库，并且 Runner 已经启动

转到 GitLab 账号→选择要注册跑步者的库→设置→ CI/CD →展开跑步者，会有一个绿色圆圈可用，显示信息为“**为该项目激活的跑步者。**”
...
| 命令 | 描述 |
| --- | --- |
| `gitlab-runner.exe register` | 向 GitLab Runner 注册该项目 |
| `gitlab-runner.exe start` | 启动跑步者 |
| `gitlab-runner.exe stop` | 拦住跑者 |
| `gitlab-runner.exe status` | 了解 gitlab-runner 的状态 |
| `gitlab-runner unregister --name test-runner` | 注销一个项目的 runner，用你的 runner 名字替换 `test-runner`，这个名字可以在 `config.toml` 文件（你的 `gitlab-runner.exe` 所在的地方）中找到。 |
| `gitlab-runner unregister --url http://gitlab.example.com/ --token t0k3n` | 通过网址和令牌删除跑步者 |
| `gitlab-runner unregister --all-runners` | 注销所有跑步者 |
| `gitlab-runner restart` | 该命令停止，然后启动 GitLab Runner 服务 |
| `gitlab-runner uninstall` | 该命令停止并卸载作为服务运行的 GitLab Runner |
| `gitlab-runner exec` | 要查看可用执行者的列表，请运行 |
| `gitlab-runner --help` | 通过执行以下命令来检查最近的命令列表 |
| `gitlab-runner run --help` | 可以看到环境变量的名称 |
| `gitlab-runner debug` | 要在调试模式下运行命令 |
| `gitlab-runner exec shell` | 要查看 shell 执行器所有可用选项的列表，请运行 |

## `.gitlab-ci.yml` 示例

这是 `.gitlab-ci.yml` 文件，并根据需求进行更改。
...
配置成功后，进行提交，查看 GitLab 上的作业状态。转到项目存储库→选择配置项/光盘菜单。