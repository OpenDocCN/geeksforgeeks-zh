...
视窗演示基金会（WPF）是一个用于创建桌面应用程序的开发框架。它是[.NET 框架](https://www.geeksforgeeks.org/introduction-to-net-framework/)的一部分。WPF 有一个独立于分辨率和基于矢量的渲染引擎，有助于处理现代图形硬件。WPF 的最新版本是 `4.6`。在这个框架中，应用程序的用户界面用 `XAML` 语言设计，应用程序逻辑用 [C#](https://www.geeksforgeeks.org/introduction-to-c-sharp/) 编程语言编写。
...
**步骤 2：** **配置新项目**对话框打开。输入项目名称 **简介**，然后选择**创建**。
...
**步骤 5：** 要运行程序，可以使用 Visual Studio 工具栏中的 `F5` 键或播放按钮。要停止程序，您可以使用工具栏中的暂停按钮。也可以通过菜单栏中的**调试 -> 开始调试**菜单来运行程序。
