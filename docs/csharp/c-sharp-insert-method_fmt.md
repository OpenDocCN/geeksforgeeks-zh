...
*   `Indexvalue`：是当前字符串的索引位置，新值将被插入于此。此参数的类型是 `System.Int32`。
*   `value`：要插入的字符串参数值，类型是 `System.String`。

## 异常:

*   `ArgumentNullException`：如果字符串值为 `null`。
*   `ArgumentOutOfRangeException`：唉呀 `Indexvalue`(索引值)阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云见阿云。

## 注意:

这个方法总是返回一个新的字符串，该字符串被修改后的值被插入到指定的位置。`Insert()`方法的返回值类型是 `System.String`。如果 `Indexvalue` 等于当前实例的长度，则该值将附加到此实例的末尾。
