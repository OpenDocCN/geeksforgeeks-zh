...
对象类是 [.NET Framework](https://www.geeksforgeeks.org/c-net-framework-basic-architecture-component-stack/) 中所有类的基类。它存在于 `System` 命名空间中。在 C# 中，.NET 基类库 (BCL) 有一个特定于语言的别名，它是 `Object` 类，完全限定名为 `System.Object`。C# 中的每个类都是直接或间接从 `Object` 类派生的。如果一个类不扩展任何其他类，那么它是 `Object` 类的直接子类，如果扩展其他类，那么它是间接派生的。因此，`Object` 类方法对所有 C# 类都可用。因此，在任何 C# 程序中，`Object` 类都是继承层次的根。`Object` 类的主要目的是为派生类提供低级服务。

C# 中有两种类型，即 [引用类型和值类型](https://www.geeksforgeeks.org/c-data-types-2/)。通过使用 `System.ValueType` 类，值类型隐式继承 `Object` 类。`System.ValueType` 类用更适合值类型的实现重写 `Object` 类中的虚拟方法。在其他编程语言中，内置类型如 `int`、`double`、`float` 等没有任何面向对象的属性。要模拟内置类型的面向对象行为，必须将它们显式包装到对象中。但是在 C# 中，由于从 `System` 继承的值类型的存在，我们不需要这样的包装。值类型进一步继承自 `System.Object`。因此在 C# 中，值类型的工作方式也类似于引用类型。引用类型通过使用其他引用类型直接或间接继承 `Object` 类。

[![](img/ef57a8ed2ca7fbb08e08587495992e1f.png)](https://media.geeksforgeeks.org/wp-content/uploads/Object_Class-1.jpg)

**上图说明:** 这里可以看到类型层次结构顶部的 `Object` 类。*1 级*和*2 级*为参照类型。`Class 1` 直接继承 `Object` 类，而 `Class 2` 通过使用 `Class 1` 间接继承。`Struct 1` 是通过 `System.ValueType` 隐式继承 `Object` 类的值类型。
...
| `ReferenceEquals(Object, Object)` | 确定指定的对象实例是否为同一实例。 |
| `ToString()` | 返回表示当前对象的字符串。 |

**要点:**

*   C# 类不需要声明 `Object` 类的继承，因为继承是隐式的。
*   `Object` 类中定义的每个方法在 `System` 中的所有对象中都是可用的，就像 .NET Framework 中的所有类一样是从 `Object` 类派生出来的。
