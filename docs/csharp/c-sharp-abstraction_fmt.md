...
> 原文：`https://www.geeksforgeeks.org/c-sharp-abstraction/`
...
*   借助`abstract`关键字声明一个抽象类。
*   在 C# 中，你是`不允许`创建抽象类的对象。或者换句话说，您不能将抽象类直接与`new`运算符一起使用。
*   包含`abstract`关键字及其某些方法（并非所有抽象方法）的类称为`抽象基类`。
*   包含`abstract`关键字及其所有方法的类称为纯抽象基类。
*   不允许在抽象类之外声明抽象方法。
*   不允许将抽象类声明为`密封类`。
