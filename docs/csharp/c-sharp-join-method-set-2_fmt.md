...
*   [`String.Join(String, Obj[])`](https://www.geeksforgeeks.org/c-join-method-set-1/)
*   [`String.Join(String, String[])`](https://www.geeksforgeeks.org/c-join-method-set-1/)
*   [`String.Join(String, String[], Int32, Int32)`](https://www.geeksforgeeks.org/c-join-method-set-1/)
...
```cs
public static string Join(string separator, IEnumerable<string> L1)
```

### 参数

> **separator**：是用作分隔符的字符串，只有当值有多个元素且类型为 `System.String` 时，分隔符才会包含在返回的字符串中。
>
> **L1**：这是一个包含要连接的字符串的集合，类型是 `System.Collections.Generic.IEnumerable<String>`。

### 返回类型

该方法返回类型为 `System.String` 的字符串，由分隔符字符串分隔的值的成员组成。如果值没有成员，该方法返回 `String.Empty`。

### 异常

如果 `L1` 为 `null`，这个方法可以抛出 `ArgumentNullException`。

### 示例

在下面的程序中，使用内置的列表集合创建了一个**单词列表**。因此，**同一个列表集合**的对象在 `Join()` 方法中与分隔符一起传递，结果，用户得到结果字符串。
...
```cs
public static string Join<T>(string separator, IEnumerable<T> T1)
```
...
> **separator**：是用作分隔符的字符串，只有当值有多个元素且类型为 `System.String` 时，分隔符才会包含在返回的字符串中。
>
> **T1**：它是一个集合，包含要连接的对象，类型是 `System.Collections.Generic.IEnumerable<T>`。
...
如果 `T1` 为 `null`，这个方法可以抛出 `ArgumentNullException`。

### 示例

在下面的代码中，首先创建了一个用户定义的**项类**，结果各种项名**被添加到构造器**中。此外，**列表将保存类“items”类型的对象。** 结果，在主方法中，包含类 `items` 类型的对象的列表与**分隔符“--”**一起传递，以获得输出字符串值。
