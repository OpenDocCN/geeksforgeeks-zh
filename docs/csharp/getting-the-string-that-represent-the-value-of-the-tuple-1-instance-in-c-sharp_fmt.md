...
> 原文：[https://www.geeksforgeeks.org/getting-the-string-that-represent-the-value-of-the-tuple-1-instance-in-c-sharp/](https://www.geeksforgeeks.org/getting-the-string-that-represent-the-value-of-the-tuple-1-instance-in-c-sharp/)

一个[元组](https://www.geeksforgeeks.org/c-sharp-tuple/)是一个数据结构，它给你最简单的方法来表示一个数据集。您也可以通过使用`字符串方法`来获取表示元组对象的字符串。该方法返回一个表示`元组<T1>`对象的字符串。此方法表示的字符串采用`(Item1)`的形式，这里 `Item1` 表示 `Item1` 属性值。它将代表一个字符串。如果任何属性包含空值，则为空字符串。
...
该方法的返回类型为`System.String`。因此，它将返回一个表示`Tuple<T1>`对象的字符串。
