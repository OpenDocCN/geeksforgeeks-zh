...
> 原文: [https://www.geeksforgeeks.org/converting-a-string-to-its-equivalent-byte-array-in-c-sharp/](https://www.geeksforgeeks.org/converting-a-string-to-its-equivalent-byte-array-in-c-sharp/)
...
## 方法 1: 使用 `Convert.ToByte()` 方法

此方法为 `Convert` 类方法。它用于将其他基本数据类型转换为字节数据类型。
...
> **步骤 3:** 遍历字符串，使用 `ToByte()` 方法将每个字符转换为字节，并将所有字节存储到字节数组中。
> **步骤 4:** 返回或对字节数组执行操作。
...
## 方法二: 使用 `Encoding.ASCII.GetBytes()` 方法

将编码为 `Encoding.ASCII.GetBytes()` 方法用于接受一个字符串作为参数，得到字节数组。
...
> **第一步:** 获取字符串。
> **步骤 2:** 创建一个空字节数组。
> **第三步:** 使用 `GetBytes()` 方法将字符串转换为 `byte[]` 并将所有转换字符串存储到字节数组中。
> **步骤 4:** 返回或对字节数组执行操作。
