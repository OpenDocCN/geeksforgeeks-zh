...
*   `Math.Sign(Decimal)`: 返回指定十进制数符号的整数。
*   `Math.Sign(Double)`: 返回指定双精度浮点数符号的整数。
*   `Math.Sign(Int16)`: 返回指定 16 位有符号整数符号的整数。这里 `Int16` 是 `short` 数据类型的简称。
*   `Math.Sign(Int32)`: 返回指定 32 位有符号整数符号的整数。这里 `Int32` 是 `int` 数据类型。
*   `Math.Sign(Int64)`: 返回指定 64 位有符号整数符号的整数。这里 `Int64` 是 `long` 的数据类型。
*   `Math.Sign(SByte)`: 返回指定 8 位有符号整数符号的整数。
*   `Math.Sign(Single)`: 返回指定单精度浮点数符号的整数。这里 `Single` 是 `float` 数据类型。
...
该方法采用单个参数，形式为 `byte`、`int`、`double`、`sbyte` 等。

## 返回类型

该方法返回类型 `System.Int32`，符合下列条件：
