...
> 原文：[https://www.geeksforgeeks.org/c-sharp-remove-the-specified-element-from-a-hashset/](https://www.geeksforgeeks.org/c-sharp-remove-the-specified-element-from-a-hashset/)

一个 `HashSet` 是一个独特元素的无序集合。它属于 `System.Collections.Generic` 命名空间。它用于我们希望防止在集合中插入重复项的情况。就性能而言，与列表相比更好。`HashSet<T>.Remove(T)` 方法用于从 `HashSet<T>` 对象中移除指定元素。
...
**返回值：** 如果成功找到并移除元素，则该方法返回 `true`，如果在 `HashSet<T>` 对象中未找到项目，则返回 `false`。

下面的例子说明了 `HashSet<T>.Remove(T)` 方法的使用：
...
*   [`https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.hashset-1.remove?view=netframework-4.7.2`](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.hashset-1.remove?view=netframework-4.7.2)