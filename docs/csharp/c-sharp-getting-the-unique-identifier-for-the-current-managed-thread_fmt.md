...
> 原文：[https://www.geeksforgeeks.org/c-sharp-getting-the-unique-identifier-for-the-current-managed-thread/](https://www.geeksforgeeks.org/c-sharp-getting-the-unique-identifier-for-the-current-managed-thread/)

`Thread`类负责在多线程编程中创建和管理线程。它提供了一个名为`ManagedThreadId`的属性来检查当前托管线程的唯一标识符。或者换句话说，线程的`ManagedThreadId`属性的值在其进程中唯一地定义了该线程。`ManagedThreadId`属性的值不会随时间变化。
...
此属性返回一个值，该值指示此托管线程的唯一标识符。该属性的返回类型为`System.Int32`。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.threading.thread.managedthreadid?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.threading.thread.managedthreadid?view=netframework-4.7.2)