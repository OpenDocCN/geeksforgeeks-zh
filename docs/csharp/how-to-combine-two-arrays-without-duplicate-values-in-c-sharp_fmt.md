...
> 原文: [https://www.geeksforgeeks.org/how-to-combine-two-arrays-without-duplicate-values-in-c-sharp/](https://www.geeksforgeeks.org/how-to-combine-two-arrays-without-duplicate-values-in-c-sharp/)
...
1.  声明任意类型（整数、字符串等）的两个数组。
2.  应用 `Union()` 函数，并使用 `ToArray()` 函数转换为数组。
    ```cs
    final = array1.Union(array2).ToArray();
    ```
3.  现在使用 `Array.ForEach()` 函数迭代最终数组中的元素。
    ```cs
