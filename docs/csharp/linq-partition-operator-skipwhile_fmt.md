...
> 原文: [https://www.geeksforgeeks.org/linq-partition-operator-skipwhile/](https://www.geeksforgeeks.org/linq-partition-operator-skipwhile/)
...
*   `SkipWhile<TSource>(IEnumerable<TSource>, Func<TSource, Boolean>)`: 只要指定条件为真，该方法用于绕过给定序列中的元素，然后返回剩余的元素。
*   `SkipWhile<TSource>(IEnumerable<TSource>, Func<TSource, Int32, Boolean>)`: 该方法用于在指定条件为真的情况下绕过给定序列中的元素，然后返回剩余的元素。元素的索引用于谓词函数的逻辑中。
