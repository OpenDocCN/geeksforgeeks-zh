...
`Math.Sinh()` 是内置的数学类方法，它返回给定的双曲正弦值参数(指定角度)。
...
> **num:** 是要返回双曲正弦的数字，该参数的类型是 `System.Double`。

## 返回值:

该方法返回类型为 `System.Double` 的数字的双曲正弦值。如果 `num` 等于 `NegativeInfinity`、`PositiveInfinity` 或 `NaN`，此方法将返回一个等于 `num` 的 `Double`。
