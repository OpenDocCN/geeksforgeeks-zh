...
> 原文: [https://www.geeksforgeeks.org/c-sharp-program-to-check-given-directory-exists-or-not/](https://www.geeksforgeeks.org/c-sharp-program-to-check-given-directory-exists-or-not/)
...
其中，`myPath`是`Exists()`方法的参数，类型为`string`。它表示指定目录的位置或路径。现在，如果给定路径引用现有目录，`Exists`方法将返回`true`，否则将返回`false`。

## 返回类型

这个方法的返回类型是一个布尔值，可以是`true`，也可以是`false`。如果给定的`myPath`引用现有目录，此方法将返回`true`，否则将返回`false`。
