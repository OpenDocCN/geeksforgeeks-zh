...
> `array`: 它是一维的、从零开始的数组，用来检查条件。
> `match`: 是谓词定义了检查元素的条件。

## 返回值:

如果 `array` 中的每个元素都符合指定谓词定义的条件，则该方法返回 `true`，否则返回 `false`。如果数组中没有元素，返回值为 `true`。

## 异常:

如果 `array` 为 `null` **或者** `match` 为 `null`，则该方法抛出 `ArgumentNullException`。

下面的程序说明了 `Array.TrueForAll()` 的使用。
