...
`Math.Acos()` 是一个内置的 `Math` 类方法，它返回余弦值为双精度参数的角度。如果参数是 `NaN`，那么结果就是 `NaN`。
...
**num**：表示余弦的数字，该参数的类型为 `System.Double`。必须是大于等于 `-1` 但小于等于 `1` 的值。

## 返回类型

返回角度 θ，以弧度为单位，类型为 `System.Double`。这里的角度总是以弧度来度量的，例如 0 ≤ θ ≤ π。

## 注意

如果 `num` 的值大于 1 或小于 -1 或等于 `NaN`，则该方法始终返回 `NaN` 作为结果。要将弧度（返回值）转换为度数，将其乘以 `180 / Math.PI`。
...
[https://msdn.microsoft.com/en-us/library/system.math.Acos](https://msdn.microsoft.com/en-us/library/system.math.Acos)