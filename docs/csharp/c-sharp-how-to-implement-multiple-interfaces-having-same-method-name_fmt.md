...
> 原文: [https://www.geeksforgeeks.org/c-sharp-how-to-implement-multiple-interfaces-having-same-method-name/](https://www.geeksforgeeks.org/c-sharp-how-to-implement-multiple-interfaces-having-same-method-name/)

## 问题描述

像一个类，接口可以有方法、属性、事件以及索引器作为其成员。但是接口将只包含成员的声明。接口成员的实现将由类给出，它们隐式或显式地实现接口。
...
> prog.cs(22,7): error CS0737: `Geeks` does not implement interface member `G1.mymethod()`. `Geeks.mymethod()` is not public
> prog.cs(11,7): (Location of symbol related to previous error)
> prog.cs(28,7): (Location of symbol related to previous error)
