...
> 原文：[https://www.geeksforgeeks.org/basics-of-file-handling-in-c-sharp/](https://www.geeksforgeeks.org/basics-of-file-handling-in-c-sharp/)

通常，文件用于存储数据。术语“文件处理”指的是各种操作，如创建文件、读取文件、写入文件、追加文件等。文件处理中最常用的两个基本操作是文件的读取和写入。当我们打开文件进行读写时，文件就变成了流。流是用于通信的字节序列。文件可以形成两个流，一个是用于读取文件的输入流，另一个是用于写入文件的输出流。在 C# 中，`System.IO` 命名空间包含处理输入和输出流的类，并提供关于文件和目录结构的信息。
...
| 方法 | 描述 |
| --- | --- |
| `Close()` | 关闭当前 `StreamWriter` 对象和与之关联的流。 |
| `Flush()` | 清除缓冲区中的所有数据，并将其写入与之关联的流中。 |
| `Write()` | 向流中写入数据。对于要写入流的不同数据类型，它有不同的重载。 |
...
| 方法 | 描述 |
| --- | --- |
| `Close()` | 关闭当前的 `StreamReader` 对象和与之关联的流。 |
| `Peek()` | 返回下一个可用字符，但不使用它。 |
| `Read()` | 读取输入流中的下一个字符，并将字符在流中的位置增加 1 |
| `ReadLine()` | 从输入流中读取一行，并以字符串形式返回数据 |
| `Seek()` | 它用于在文件的特定位置读/写 |
