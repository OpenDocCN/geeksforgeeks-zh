# 获取代表 C# 中 `ValueTuple<T1>` 实例的值的字符串

> 原文: [https://www.geeksforgeeks.org/getting-the-string-that-represent-the-value-of-valuetuple-2-instance-in-c-sharp/](https://www.geeksforgeeks.org/getting-the-string-that-represent-the-value-of-valuetuple-2-instance-in-c-sharp/)
...
此方法返回一个字符串，该字符串将代表 `T2` 的值元组 `<T1>` 对象的值。这个方法表示的字符串是 `(Item1，Item2)` 的形式，这里 `Item1` 和 `Item2` 表示 `Item1` 和 `Item2` 属性的值，它将表示一个*字符串。如果任何属性包含空值，则为空*。
...
该方法的返回类型为 `System.String`。因此，它将返回一个表示 `ValueTuple<T1, T2>` 对象的字符串。
