...
**示例:**
首先我们在 `Program1.cs` 文件中创建一个名为 `Geek` 的类。包含`M1()`、`M2()`、`M3()`三种方法。
...
现在我们在 `Program2.cs` 文件中创建一个名为 `NewMethodClass` 的静态类。包含 `M4()` 和 `M5()` 两种方法。现在我们想在 `Geek` 类中添加这些方法，所以我们使用`绑定参数`将这些方法与 `Geek` 类绑定。之后，我们创建了另一个名为 `GFG` 的类，其中 `Geek` 类访问所有五种方法。
...
*   这里，`绑定参数`是用于将新方法与现有类或结构绑定的参数。当您调用扩展方法时，它们不接受任何值，因为它们仅用于绑定，不用于任何其他用途。在扩展方法的参数列表中，绑定参数始终位于第一位。如果您将绑定参数写在第二、第三或任何其他位置而不是第一位，编译器将报错。绑定参数使用 `this` 关键字创建，后跟您要添加新方法的类名和参数名。例如：

```cs
    this Geek g
```

这里，`this`关键字是用来绑定的，`Geek`是你要绑定的类名，`g`是参数名。
