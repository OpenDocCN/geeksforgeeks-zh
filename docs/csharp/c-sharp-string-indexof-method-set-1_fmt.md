# C# 字符串 IndexOf() 方法（设置 - 1）

> 原文：[https://www.geeksforgeeks.org/c-sharp-string-indexof-method-set-1/](https://www.geeksforgeeks.org/c-sharp-string-indexof-method-set-1/)

在 C# 中，`IndexOf()` 方法是一种 `String` 方法。此方法用于在字符串的当前实例中查找指定字符或字符串的第一个匹配项的从零开始的索引。如果找不到字符或字符串，该方法返回 -1。这个方法可以通过传递不同的参数来重载。
...
*   `String.IndexOf(string s1, StringComparison cttype)`
...
*   **参数：** 该方法取 `System` 类型的参数 `char x`。指定要搜索的字符的字符。
*   **返回类型：** 该方法的返回类型为 `System.Int32`。
...
*   **参数：** 该方法取两个参数，即 `System` 类型的 `char x`。指定要搜索的字符的字符和 `start1` 类型的 `System.Int32` 以整数值的形式指定开始搜索的位置。
*   **返回类型：** 该方法的返回类型为 `System.Int32`。
*   **异常：** 如果 `start1` 小于 0（零）或大于字符串长度，此方法可以给出 `ArgumentOutOfRangeException`。

**示例：** 在下面的代码中，用户想要知道指定字符串 "HelloGeeks" 中字符 "H" 的索引，因此，该方法返回字符 "H" 的相应索引。然而，如果 `start1` 大于 1，那么很明显返回 -1。
...
此方法返回字符串中指定字符的第一个匹配项的从零开始的索引。但是，对该字符的搜索将从指定位置 `start1` 直到指定位置即 `start2` 为止，如果没有找到，则返回 -1。
...
*   **参数：** 该方法取三个参数，即 `System` 类型的 `char x`。指定要搜索的字符的字符，`start1` 类型的 `System.Int32` 以整数值的形式指定开始搜索的位置，以及 `start2` 类型的 `System.Int32` 指定停止搜索的结束位置。
*   **返回类型：** 该方法的返回类型为 `System.Int32`。
*   **异常：** 如果 `start1` 或 `start2` 为负或 `start1` 大于当前字符串的长度或 `start2` 大于当前字符串的长度减去 `start1`，此方法可给出 `ArgumentOutOfRangeException`。

**示例：** 在下面的代码中，用户想知道指定字符串 "My Life My Rules" 中字符 "R" 的索引，因此，该方法返回字符 "R" 的索引值。同样，对于 `start1` > 1 和 `start2` < 8 的情况，它再次返回 -1，因为它没有找到任何字符。
...
*   **参数：** 该方法取 `System` 类型的参数 `s1`。字符串指定要搜索的子字符串。
*   **返回类型：** 该方法的返回类型为 `System.Int32`。如果找到该字符串，则为 `s1` 的从零开始的索引位置，否则为 -1。如果 `s1` 是 `String.Empty`，返回值为 0。
*   **异常：** 如果 `s1` 为 `null`，这个方法可以给出 `ArgumentNullException`。
