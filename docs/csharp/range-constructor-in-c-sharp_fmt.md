...
范围（索引、指标）构造器是[范围结构](https://www.geeksforgeeks.org/range-structure-in-c-sharp-8-0/)的一部分。此构造函数用于创建 `Range` 的新实例以及指定的开始和结束索引。当您使用范围运算符或构造函数创建范围时，它不会添加最后一个元素或结束索引元素。
...
这里，`start` 表示范围的起始索引，`end` 表示范围的最后一个索引。
