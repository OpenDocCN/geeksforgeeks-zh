...
> 原文：[https://www.geeksforgeeks.org/total-number-of-elements-in-a-specified-dimension-of-an-array-in-c-sharp/](https://www.geeksforgeeks.org/total-number-of-elements-in-a-specified-dimension-of-an-array-in-c-sharp/)

## `Array.GetLongLength(Int32)` 方法

`Array.GetLongLength(Int32)` 方法用于获取一个 64 位整数，代表数组指定维度的元素个数。
...
这里，`dimension` 是要计算长度的数组的从零开始的维度。

### 返回值

返回一个 `64-bit` 整数，代表指定维度的元素个数。

### 异常

如果 `dimension` 小于零或大于等于 `Rank`，该方法抛出 `IndexOutOfRangeException`。
...
在上面的程序中，`GetLength` 方法返回类型为 `System.Int32`，但是 `GetLongLength` 方法返回类型为 `System.Int64`。

### 参考

*   [https://docs.microsoft.com/en-us/dotnet/api/system.array.getlonglength?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.array.getlonglength?view=netframework-4.7.2)