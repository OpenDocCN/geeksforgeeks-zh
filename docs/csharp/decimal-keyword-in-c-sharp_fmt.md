...
[decimal](https://www.geeksforgeeks.org/c-sharp-data-types/#:~:text=Decimal%20Types%20%3A%20The%20decimal%20type,decimal%20x%20%3D%20300.5m%3B.) 是一个关键字，用于声明一个变量，该变量可以存储 `1.0×10^-28` 到 `7.9228×10^28` 范围内的浮动类型值。它是 `System.Decimal` 在内存中占据 16 个字节（128 位）。
...
我们必须用 `'M'` 或 `'m'` 作为字面量的后缀，来表示一个十进制值。
