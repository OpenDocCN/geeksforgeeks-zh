...
> 原文: [https://www.geeksforgeeks.org/mathf-exp-method-in-c-sharp-with-examples/](https://www.geeksforgeeks.org/mathf-exp-method-in-c-sharp-with-examples/)

在 C# 中，`Exp(Single)` 是一个 `MathF` 类方法，用于将 `e` 提升到指定的幂。这里 `e` 是一个数学常数，其值约为 `2.71828`。

> **语法:** `public static float Exp(float x);`
> 此处，`x` 为 `System.Single` 类型，指定一个幂。
>
> **返回类型:** 返回一个数字 `e` 升到 `x` 幂后的值，类型为 `System.Single`。

**注:**

*   如果 `x` 等于 `NaN`，则返回值为 `NaN`。
*   如果 `x` 等于 `PositiveInfinity`，则返回值为 `Infinity`。
*   如果 `x` 等于 `NegativeInfinity`，则返回值为 `0`。
