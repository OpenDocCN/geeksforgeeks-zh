...
*   **逻辑非:** `!` 如果考虑的条件不满足，运算符返回 true。否则返回假。比如 `!a` 如果 a 为假，即当 a=0 时，返回真。
...
*   `"<<="` (左移分配): 该运算符是`"<<"`和`"="`运算符的组合。该运算符首先将左边变量的当前值左移右边的值，然后将结果赋给左边的变量。
    例:
...
*   `">>="` (右移赋值): 该运算符是`">>"`和`"="`运算符的组合。该运算符首先将左边变量的当前值右移右边的值，然后将结果赋给左边的变量。
    例:
...
*   `"^="` (按位异或赋值): 这个运算符是`'^'`和`'='`运算符的组合。该运算符首先用右边的值“按位异或”左边变量的当前值，然后将结果赋给左边的变量。
    例:
...
*   `"|="` (按位包含或赋值): 该运算符是`"|"`和`"="`运算符的组合。这个运算符首先用右边的值“按位或”左边变量的当前值，然后将结果赋给左边的变量。
    例:
...
说明:
条件: 必须评估为真或假。
如果条件为真，第一个表达式被求值并成为结果。
如果条件为假，则`second_expression`被求值并成为结果。
