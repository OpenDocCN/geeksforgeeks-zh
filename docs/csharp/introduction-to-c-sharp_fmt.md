...
了解更多[.Net 框架](https://en.wikipedia.org/wiki/.NET_Framework)
...
网上有各种各样的 `IDE`，比如 [GeeksforGeeks IDE](https://ide.geeksforgeeks.org/)、[CodeChef IDE](https://www.codechef.com/ide) 等。它可以用来运行 C# 程序而无需安装。
...
1.  **注释：** 注释用于解释代码，其使用方式类似于 `Java` 或 `C` 或 `C++`。编译器忽略注释条目，并且不执行它们。注释可以是单行或多行。
    ...
    **语法：**
    ...
    /* Multi line comments*/
    ```
2.  **using System：** `using` 关键字用于在程序中包含系统命名空间。
3.  **命名空间声明：** 命名空间是类的集合。`HelloGeeksApp` 命名空间包含类 `HelloGeeks`。
4.  **类：** 类包含程序中要使用的数据和方法。方法定义类的行为。类 `HelloGeeks` 只有一个类似于 `JAVA` 的 `Main` 方法。
5.  **static void Main()：** `static` 关键字告诉我们，这个方法不需要实例化类就可以访问。`void` 关键字告诉我们这个方法不会返回任何东西。`Main()` 方法是我们应用的切入点。在我们的程序中，`Main()` 方法用 `Console` 语句指定其行为。`WriteLine("Hello Geeks");`。
6.  **Console.WriteLine()：** `WriteLine()` 是在 `System` 命名空间中定义的 `Console` 类的方法。
7.  **Console.ReadKey()：** 这是给 `VS.NET` 用户的。这使得程序等待按键，并阻止屏幕快速运行和关闭。

**注意：** C# 区分大小写，所有语句和表达式必须以分号（`;`）结束。
...
*   C# 代码被编译成中间语言（`.Net` 中间语言），它是一种标准语言，与目标操作系统和体系结构无关。
