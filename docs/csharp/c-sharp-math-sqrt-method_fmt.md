...
在 C# 中，`Math.Sqrt()` 是 `Math` 类的一个方法，用于计算指定数字的平方根。
...
`d`：要计算平方根的数字，该参数类型为 `System.Double`。

## 返回类型

该方法返回 `d` 的平方根，如果 `d` 等于 `NaN`、`NegativeInfinity` 或 `PositiveInfinity`，则返回该值。这种方法的返回类型是 `System.Double`。
...
下面 C# 程序说明了 `Math.Sqrt()` 的工作原理。

### 程序 1
当参数为正 `double` 值时，该方法返回给定值的平方根。
...
### 程序 3
当参数是带小数位的 `double` 值时，那么这个方法将返回给定值的平方根。
...
**注意：** 如果值太大，那么其给出的编译时错误为 `CS1021: Integral constant is too large`。

**参考：** [https://msdn.microsoft.com/en-us/library/system.math.sqrt](https://msdn.microsoft.com/en-us/library/system.math.sqrt)