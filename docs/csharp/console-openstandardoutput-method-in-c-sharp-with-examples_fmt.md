# Console.OpenStandardOutput() 方法（C#）及示例

> 原文：[https://www.geeksforgeeks.org/console-openstandardoutput-method-in-c-sharp-with-examples/](https://www.geeksforgeeks.org/console-openstandardoutput-method-in-c-sharp-with-examples/)

`Console.OpenStandardOutput` 方法用于获取标准输出流。C# 中有两种 `OpenStandardOutput` 方法重载，如下所示：
...
> **语法：** `public static System.IO.Stream OpenStandardOutput();`
> **参数：** 此方法不接受任何参数。
> **返回值：** 此方法返回标准输出流。
...
> **语法：** `public static System.IO.Stream OpenStandardOutput(int bufferSize);`
> **参数：** 该方法接受以下参数。
> 
> *   `bufferSize`：该参数是内部流缓冲区大小。
> 
> **返回值：** 此方法返回标准输出流。
