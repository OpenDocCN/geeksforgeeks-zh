...
> 原文: [https://www.geeksforgeeks.org/c-sharp-string-concat-with-examples-set-1/](https://www.geeksforgeeks.org/c-sharp-string-concat-with-examples-set-1/)

`String.Concat` 方法用于连接一个或多个 `String` 实例或一个或多个对象实例的值的 `String` 表示。它总是返回串联字符串。
...
> `strC`: 要连接的第三个字符串。
> `strD`: 要连接的第四个字符串。
> 所有这些参数的类型都是 `System.String`。

**返回值：** 该方法的返回类型为 `System.String`。该方法返回一个由四个字符串串联而成的字符串，即 `strA`、`strB`、`strC` 和 `strD`。
...
**注意：** 如果该方法中存在空参数，则 `String.Empty` 用于代替空参数。
...
**返回值：** 该方法的返回类型为 `System.String`。它返回串联字符串，该字符串表示参数列表中的每个值。
...
此方法用于连接三个指定对象的字符串表示形式。如果该方法中存在空参数，则 `String.Empty` 用于代替空参数。
...
**返回值：** 该方法的返回类型为 `System.String`。此方法返回一个串联字符串，它是参数列表中每个值的表示形式。
