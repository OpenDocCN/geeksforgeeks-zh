...
> 原文: [https://www.geeksforgeeks.org/c-sharp-type-getmembers-method/](https://www.geeksforgeeks.org/c-sharp-type-getmembers-method/)

`Type.GetMembers()` 方法用于获取当前类型的成员（属性、方法、字段、事件等）。此方法的重载列表中有 2 种方法，如下所示：
...
> 这里，需要一个由一个或多个 `BindingFlags` 组成的位掩码来指定如何进行搜索，或者 `Zero`（默认值）来返回一个空数组。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.type.getmembers?view=netframework-4.8](https://docs.microsoft.com/en-us/dotnet/api/system.type.getmembers?view=netframework-4.8)