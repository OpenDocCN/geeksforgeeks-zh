...
> 原文：[https://www.geeksforgeeks.org/c-sharp-dynamic-coding-attributes-in-depth/](https://www.geeksforgeeks.org/c-sharp-dynamic-coding-attributes-in-depth/)

.NET 公共语言通过引入属性提供了运行时动态编码实践的实现，这些属性允许将编译时生成并嵌入到带程序元素的 .NET 程序集。此外，通过使用 .NET 机制称为反射。因此，本文对 .NET 属性。

## 属性

[属性](https://www.geeksforgeeks.org/attributes-in-c-sharp/)来源于 `System.Attribute` 类。属性可以被定位在任何启示上，然而，一个特定的特征可能会限制它所基于的呈现的种类。在 .NET 代码中，我们可以通过将包含在方形部分(`[]`)中的属性的名称放在它所应用的元素的声明上来确定属性。此外，我们可以将额外的元数据整合到一个利用质量的聚会中。更重要的是，属性类似于描述性的词语，用于元数据解释，比如可以应用于给定种类的组件、集合、模块、技术等等。以下是中属性的语法。NET 编程如下；

```cs
[type: attributeName(parameter1, parameter2, ………n)]
```

.NET 构造为属性的使用指定了两种类型的属性：预定义属性和自定义属性。属性通常可以有零个或多个参数。因此，下面的 C# 代码提出了属性的执行，其中使用过时的 as 将方法声明为不推荐使用；
...
属性用于信息定义、反射、web 服务、序列化、设置类蓝图以及在运行时指定第三方库。总的来说，从文档的角度来看，在元数据表中做一个条目是很有帮助的。此外，编译器会在调试期间自动发现属性条目的存在。.NET 框架在源代码中规定了客户和预定义属性。下一节将描述代码中一些预定义属性的实现。
...
下面的代码示例说明了 `[DllImport]` 属性的功能，该属性调用非托管 dll `user32.dll` 来填充消息框，如下所示；
...
到目前为止，我们已经体验了预定义的属性，在本系列中，我们还可以进一步开发自己的自定义属性，并在其他代码中利用它。为此，该类必须以属性为前缀，并派生为 `System.Attribute` 类如下；
