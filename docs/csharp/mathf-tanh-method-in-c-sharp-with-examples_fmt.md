...
`MathF.Tanh(Single)`方法是内置的`MathF`类方法，它返回给定单值参数的双曲正切值。

> **语法:** `public static float Tanh(float x);`
> 这里，`x`是要返回双曲正切的数字，这个参数的类型是`System.Single`。

**返回值:** 此方法返回类型为`System.Single`的`x`的双曲正切值。如果`x`等于否定有限性或肯定有限性，则返回肯定有限性。如果`x`等于`NaN`，则返回`NaN`。
