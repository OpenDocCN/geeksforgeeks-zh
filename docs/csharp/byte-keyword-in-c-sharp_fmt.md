...
**[关键词](https://www.geeksforgeeks.org/c-sharp-keywords/)** 是一种语言中用于某种内部过程或代表某种预定义动作的词语。`byte` 是一个关键字，用于声明一个变量，该变量可以存储 0 到 255 范围内的无符号值。是 `System.Byte` 的别名。

`byte` 关键字占用内存 1 个字节（8 位）。
