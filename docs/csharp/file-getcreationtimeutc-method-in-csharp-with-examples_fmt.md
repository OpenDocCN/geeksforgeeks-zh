# File.GetCreationTimeUtc() 方法在 C# 中的示例

> 原文：[https://www.geeksforgeeks.org/file-getcreationtimeutc-method-in-csharp-with-examples/](https://www.geeksforgeeks.org/file-getcreationtimeutc-method-in-csharp-with-examples/)

**`File.GetCreationTimeUtc(String)`** 是一个内置的 `File` 类方法，用于返回指定文件或目录的创建日期和时间，以协调世界时（UTC）表示。
...
## 返回值：
以协调世界时（UTC）返回指定文件或目录的创建日期和时间。

下面是说明 `File.GetCreationTimeUtc()` 方法的程序。
