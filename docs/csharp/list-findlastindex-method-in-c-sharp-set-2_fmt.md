...
此方法用于搜索与指定谓词定义的条件相匹配的元素，并返回列表中最后一次出现的从零开始的索引或其一部分。该方法的重载列表中有 3 种方法:

*   [`FindLastIndex(Predicate<T>)` 方法](https://www.geeksforgeeks.org/list-findlastindex-method-in-c-sharp-set-1/)
*   [`FindLastIndex(Int32, Predicate<T>)` 方法](#2nd)
*   [`FindLastIndex(Int32, Int32, Predicate<T>)` 方法](#3rd)

在这里，我们将只讨论最后两种方法。

## FindLastIndex(Int32, Predicate<T>) 方法

此方法搜索与指定谓词定义的条件匹配的元素，并返回从第一个元素延伸到指定索引的 [`List<T>`](https://www.geeksforgeeks.org/c-list-class/) 中元素范围内最后一次出现的从零开始的索引。

> **语法:** `public int FindLastIndex(int start, Predicate<T> match);`
>
> **参数:**
> **start**: 是搜索开始的起始索引。
> **match**: 是谓词委托定义了被搜索元素的条件。

**返回值:** 如果找到该元素，则返回与参数 `match` 指定条件匹配的元素最后一次出现的类型为 `Int32` 的从零开始的索引。如果没有找到，则返回 `-1`。

**异常:**

*   **ArgumentNullException:** 如果 `match` 为 `null`。
*   **ArgumentOutOfRangeException:** 如果 `start` 在列表 `List<T>` 的有效索引范围之外。
...
在本例中，我们使用一个 XML 文件，从起始索引中搜索一个项目，并打印该项目的索引，如果找不到该项目，则打印“-1”，如果找到，则打印索引。物品是“GeeksForGeeks”。但是这里我们没有 XML 文件，这里编译器给出了一个异常。
...
> **语法:** `public int FindLastIndex(int startIndex, int count, Predicate<T> match);`
>
> **参数:**
...
*   **ArgumentNullException:** 如果 `match` 为 `null`。
*   **ArgumentOutOfRangeException:** 如果 `startIndex` 在范围之外，或者 `count` 小于 0(零)，或者 `startIndex` 和 `count` 不在列表中指定有效的部分。
...
*   `List<T>` 从 `startIndex` 开始搜索并在第一个元素结束。
*   `Predicate<T>` 是一个方法的委托，如果传递给它的对象与委托中定义的条件相匹配，则该委托返回真。当前 `List<T>` 的元素被单独传递给 `Predicate<T>` 委托。
*   此方法执行线性搜索；因此，这个方法是一个 O(n)运算，其中 n 是从 `List<T>` 开始到 `startIndex` 的元素个数。