...
> 原文：[https://www.geeksforgeeks.org/linq-quantifier-operator-contains/](https://www.geeksforgeeks.org/linq-quantifier-operator-contains/)

在 LINQ 中，量词运算符用于返回一个布尔值，该值表明某些或所有元素是否满足给定条件。标准查询运算符支持 3 种不同类型的量词运算符：
...
*   `Contains<源>(IEnumerable<源>, 源)`：此方法用于通过使用默认的相等比较器来检查序列是否包含指定的元素。
*   `Contains<源>(IEnumerable<源>, 源, IEqualityComparer<源>)`：此方法用于使用指定的 `IEqualityComparer` 检查序列是否包含指定的元素。
...
*   该运算符的返回类型为 `System.Boolean`。
*   您可以使用自定义类来检查给定集合中的对象，该类将 `IEqualityComparer` 派生为 `Contains`。
