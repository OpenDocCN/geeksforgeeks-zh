...
> 原文:[https://www.geeksforgeeks.org/c-sharp-check-if-the-arraylist-is-read-only/](https://www.geeksforgeeks.org/c-sharp-check-if-the-arraylist-is-read-only/)

`ArrayList`表示可以单独索引的对象的有序集合。它基本上是数组的替代。它还允许动态内存分配、添加、搜索和排序列表中的项目。`ArrayList.IsReadOnly`属性用于检查数组列表是否为只读。
...
**返回值:**如果数组列表是只读的，这个方法返回`true`，否则返回`false`。默认值为`false`。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.collections.arraylist.isreadonly?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.arraylist.isreadonly?view=netframework-4.7.2)