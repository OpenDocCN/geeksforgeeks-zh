...
> 原文: [https://www.geeksforgeeks.org/how-to-set-minimum-date-in-the-datetimepicker-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-minimum-date-in-the-datetimepicker-in-c-sharp/)

在 Windows 窗体中，`DateTimePicker` 控件用于在窗体中选择和显示具有特定格式的日期/时间。在 `DateTimePicker` 控件中，您可以使用 `MinDate` 属性设置在 `DateTimePicker` 中可以选择的最小日期和时间。此属性的默认值为 `1/1/1753 00:00:00`。您可以通过两种不同的方式设置此属性：
...
*   **第三步：** 拖放完成后，转到 `DateTimePicker` 的属性窗口，并在其中设置最小日期和时间，如下图所示：
...
如果该属性的值不小于最大日期值，它将引发 `ArgumentException`，如果该属性的值小于最小时间值，它还将引发 `SystemException`。以下步骤显示了如何动态设置 `DateTimePicker` 中的最小日期和时间：
...
*   **步骤 2：** 创建日期选择器后，设置由 `DateTimePicker` 类提供的 `MinDate` 属性。
...
*   **步骤 3：** 最后使用下面的语句将这个 `DateTimePicker` 控件添加到表单中：
