...
在 [C#](https://www.geeksforgeeks.org/introduction-to-c-sharp/) 中，`ToUpper()` 是一个字符串方法。它将每个字符都转换成大写（如果有大写版本的话）。如果一个字符没有等同的大写字母，它将保持不变。例如，特殊符号保持不变。这个方法可以通过传递不同类型的参数来重载。
...
**返回类型:** 它的返回字符串值，它是给定字符串的大写等价物。这种方法的返回类型是 `System.String`。
...
> **`culture`**: 是提供特定文化大小写规则的必需对象。

**返回类型:** 该方法返回类型为 `System.String` 的当前字符串的大写等价物。

**异常:** 如果 `culture` 的值为 `null`，这个方法可以抛出 `ArgumentNullException`。
...
**参考:** [https://docs.microsoft.com/en-us/dotnet/api/system.string.toupper?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.string.toupper?view=netframework-4.7.2)