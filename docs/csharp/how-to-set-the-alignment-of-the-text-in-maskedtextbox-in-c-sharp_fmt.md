...
> 原文:[https://www.geeksforgeeks.org/how-to-set-the-alignment-of-the-text-in-maskedtextbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-alignment-of-the-text-in-maskedtextbox-in-c-sharp/)

在 C# 中，`MaskedTextBox` 控件为表单上的用户输入(如日期、电话号码等)提供了一个验证过程。或者换句话说，它被用来提供区分正确和不正确用户输入的屏蔽。在掩码文本框控件中，您可以使用 `TextAlign` 属性设置掩码文本框中文本的对齐方式，该属性将显示给用户。此属性有三个在 `HorizontalAlignment` 枚举下定义的值，这些值是:

*   `Center` 值用于对齐控件中心的文本。
*   `Left` 值用于对齐控件左侧的文本。
*   `Right` 值用于对齐控件右侧的文本。

该属性的默认值为 `Left`。您可以通过两种不同的方式设置此属性:
...
这里，`HorizontalAlignment` 表示该属性的值。如果这个属性的值不属于 `HorizontalAlignment` 枚举，那么它将抛出一个 `InvalidEnumArgumentException`。以下步骤显示了如何动态设置掩码文本框中文本的对齐方式:
...
*   **步骤 2:** 创建 `MaskedTextBox` 后，设置 `MaskedTextBox` 类提供的 `MaskedTextBox` 的 `TextAlign` 属性。
