...
> 原文：[https://www.geeksforgeeks.org/how-to-add-items-in-listbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-add-items-in-listbox-in-c-sharp/)

在 Windows 窗体中，`ListBox` 控件用于显示列表中的多个元素，用户可以从中选择一个或多个元素，这些元素通常显示在多个列中。在列表框中，可以使用 `Items` 属性在列表框中添加项。此属性允许您获取对当前存储在列表框中的元素列表的引用。在此引用的帮助下，您可以添加、移除和获取集合中元素的计数。您可以通过两种不同的方式设置此属性：
...
3.  **步骤 3：** 拖放完成后，转到 `ListBox` 控件的属性以在 `ListBox` 中添加元素。
    ![](img/a45cb13a0e0a4c71ab4d2f731db0f811.png)
...
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法以编程方式添加 `ListBox` 控件中的元素：

```cs
public System.Windows.Forms.ListBox.ObjectCollection Items { get; }
```

这里，`ListBox.ObjectCollection` 表示列表框中的元素。以下步骤显示了如何在列表框中动态添加元素：
...
2.  **步骤 2：** 创建 `ListBox` 后，设置 `ListBox` 类提供的 `ListBox` 的 `Items` 属性。
...
3.  **步骤 3：** 最后使用 `Add()` 方法将此 `ListBox` 控件添加到窗体。
