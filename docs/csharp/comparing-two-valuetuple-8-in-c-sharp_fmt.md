...
> 原文：[https://www.geeksforgeeks.org/comparing-two-valuetuple-8-in-c-sharp/](https://www.geeksforgeeks.org/comparing-two-valuetuple-8-in-c-sharp/)
...
您也可以使用 `CompareTo(ValueTuple<T1, T2, T3, T4, T5, T6, T7, TRest>)` 方法来比较两个值元组的实例。或者换句话说，借助 `CompareTo` 方法，您可以将当前的 [`ValueTuple<T1, T2, T3, T4, T5, T6, T7, TRest>`](https://www.geeksforgeeks.org/c-sharp-valuetuple-8-struct/) 实例与指定的 `ValueTuple<T1, T2, T3, T4, T5, T6, T7, TRest>` 实例进行比较。此方法还将嵌套值元组相互比较。
...
该方法的返回类型为 `System.Int32`。并且它总是返回一个有符号整数，该整数指示此实例和其他实例在排序顺序中的相对位置，如下列表所示：
