...
> 原文: [https://www.geeksforgeeks.org/c-sharp-program-to-join-multiple-data-sources-using-linq/](https://www.geeksforgeeks.org/c-sharp-program-to-join-multiple-data-sources-using-linq/)

`LINQ` 被称为语言集成查询，它是在 .NET 3.5 中引入的。它提供了 .NET 语言创建查询以从数据源检索数据。在本文中，我们将讨论如何使用 `LINQ` 连接多个数据源。这里的数据源是指列表。因此，我们使用列表集合创建三个具有学生详细信息的数据源，然后基于 `id` 连接数据，这在使用 `join` 关键字的所有列表中都很常见。
...
其中 `data` 是数据源列表，`iterator` 用于从特定数据源获取数据

## 返回:

将根据比较的 `column_names` 返回匹配的行。
...
**1.** 通过声明变量，使用名为 `students`、`departments` 和 `addresses` 的列表创建三个数据源。

**2.** 给这些列表添加值。

**3.** 根据 `student_id`、`dept_id`、`add_id` 进行加入。
