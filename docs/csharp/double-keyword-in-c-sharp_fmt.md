...
[关键词](https://www.geeksforgeeks.org/c-sharp-keywords/) 是一种语言中用于某种内部过程或代表某种预定义动作的词语。`double` 是一个关键字，用于声明一个变量，该变量可以存储从 **5.0 x 10 <sup>-324</sup> 到 1.7 x 10 <sup>308</sup>** 范围内的浮点值。它是 `System.Double` 的别名。基本上，它是一个 64 位双精度浮点数，精度为 14 到 15 位。

`double` 关键字占用内存中 8 字节(64 位)的空间。
