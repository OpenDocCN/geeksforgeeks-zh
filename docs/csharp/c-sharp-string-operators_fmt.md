...
[字符串](https://www.geeksforgeeks.org/c-string/)是一个字符数组。`String`类将文本表示为一系列 Unicode 字符，并在 .NET 基类库中定义。`String`类的主要用途是提供属性、运算符和方法，这样使用字符串就变得容易了。
...
> `string x`: 字符串 `x` 是要比较的第一个字符串。
> `string y`: 字符串 `y` 是第二个要比较的字符串。

**返回值：** 该运算符的返回类型为 `System.Boolean`。如果字符串 `x` 等于字符串 `y`，则返回 `true`，否则返回 `false`。
...
**返回值：** 该运算符的返回类型为 `System.Boolean`。如果字符串 `x` 不等于字符串 `y`，则返回 `true`，否则返回 `false`。
...
**参考：** [https://docs.microsoft.com/en-us/dotnet/api/system.string?view=netframework-4.7.2#operators](https://docs.microsoft.com/en-us/dotnet/api/system.string?view=netframework-4.7.2#operators)