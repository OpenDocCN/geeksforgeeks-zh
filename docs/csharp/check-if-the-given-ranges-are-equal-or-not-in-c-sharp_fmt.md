...
> 原文：[https://www.geeksforgeeks.org/check-if-the-given-ranges-are-equal-or-not-in-c-sharp/](https://www.geeksforgeeks.org/check-if-the-given-ranges-are-equal-or-not-in-c-sharp/)
...
这个方法是`Object`类方法，它返回一个值，显示给定的范围对象等于另一个范围对象。如果返回 `true`，则当前 `range` 对象等于另一个 `range` 对象；如果返回 `false`，则当前 `range` 对象不等于另一个 `range` 对象。
...
这个方法返回一个值，显示给定的对象等于另一个对象。如果返回 `true`，则当前对象等于另一个 `range` 对象；如果返回 `false`，则当前对象不等于另一个 `range` 对象。
