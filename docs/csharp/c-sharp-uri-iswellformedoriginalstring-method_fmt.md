...
> 原文: [https://www.geeksforgeeks.org/c-sharp-uri-iswellformedoriginalstring-method/](https://www.geeksforgeeks.org/c-sharp-uri-iswellformedoriginalstring-method/)

`Uri.IsWellFormedOriginalString()` 方法用于显示用于构造此 `Uri` 的字符串是否格式良好，不需要进一步转义。
...
如果字符串格式良好，该方法返回 `true`，否则返回 `false`。

下面的程序说明了 `Uri.IsWellFormedOriginalString()` 方法的使用:
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.uri.iswellformedoriginalstring?view=netstandard-2.1](https://docs.microsoft.com/en-us/dotnet/api/system.uri.iswellformedoriginalstring?view=netstandard-2.1)