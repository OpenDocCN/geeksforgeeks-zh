# C# 检查两个 SortedSet<T> 对象是否相等

> 原文：[https://www.geeksforgeeks.org/c-sharp-check-if-two-sortedsett-objects-are-equal/](https://www.geeksforgeeks.org/c-sharp-check-if-two-sortedsett-objects-are-equal/)

从 `Object` 类继承的 `Equals(Object)` 方法用于检查指定的 [`SortedSet<T>`](https://www.geeksforgeeks.org/c-sortedset-class/) 对象是否等于另一个 `SortedSet<T>` 对象。
...
**返回值：** 如果指定对象等于当前对象，则该方法返回 `true`，否则返回 `false`。
...
**注意：** 如果当前实例是引用类型，`Equals(Object)` 方法检查引用是否相等。