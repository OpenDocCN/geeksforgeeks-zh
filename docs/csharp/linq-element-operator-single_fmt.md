...
*   `Single<TSource>(IEnumerable<TSource>, Func<TSource, Boolean>)`: 此方法返回集合或序列中指定给定条件的唯一元素，如果存在多个指定给定条件的元素，将引发异常。
*   `Single<TSource>(IEnumerable<TSource>)`: 此方法返回给定序列或集合中的唯一元素，如果序列或集合中没有恰好一个元素，则抛出异常。
