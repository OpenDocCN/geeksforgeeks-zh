...
> 原文：[https://www.geeksforgeeks.org/getting-the-string-that-represent-the-value-of-the-tuple-8-instance-in-c-sharp/](https://www.geeksforgeeks.org/getting-the-string-that-represent-the-value-of-the-tuple-8-instance-in-c-sharp/)

一个[元组](https://www.geeksforgeeks.org/c-sharp-tuple/)是一个数据结构，它给你最简单的方法来表示一个数据集。您也可以通过使用`字符串方法`来获取表示元组对象的字符串。该方法返回一个字符串，该字符串将表示 `Tuple<T1, T2, T3, T4, T5, T6, T7, TRest>` 对象。此方法表示的字符串的形式为 `(Item1, Item2, Item3, Item4, Item5, Item6, Item7, Item8, ...)`。这里，`Item1`、`Item2`、`Item3`、`Item4`、`Item5`、`Item6`、`Item7` 表示 `Item1`、`Item2`、`Item3`、`Item4`、`Item5`、`Item6`、`Item7` 的属性值，`Item8` 表示元组 `<T1, T2, T3, T4, T5, T6, T7, TRest>` 对象的 `Rest` 的值。`Item1` 属性和任何附加嵌套组件的值后跟 `Item8`。它将代表一个字符串。如果任何属性包含空值，则为空字符串。
...
**返回类型：** 该方法的返回类型为 `System.String`。因此，它将返回一个表示 `Tuple<T1, T2, T3, T4, T5, T6, T7, TRest>` 对象的字符串。
