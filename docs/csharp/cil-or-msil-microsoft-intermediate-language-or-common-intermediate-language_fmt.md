...
> 原文: [https://www.geeksforgeeks.org/cil-or-msil-microsoft-intermediate-language-or-common-intermediate-language/](https://www.geeksforgeeks.org/cil-or-msil-microsoft-intermediate-language-or-common-intermediate-language/)
...
在上面的 `MSIL` 中，有一个或两个字节长的操作码。继承所有其他类的基类声明包含在 `mscorlib.dll` 中。在 `Main()` 方法中，指令 `ldstr` 将字符串“ *GeeksforGeeks* ”加载到堆栈上。然后是静态 `System.Console.WriteLine` 函数，从堆栈中弹出字符串。最后，`ret` 指令发出结束函数调用的信号。
然后是 `.ctor()` 语句暗示了一个默认的构造函数，没有类 `Demo` 的参数。该构造函数由编译器为非静态类 `Demo` 自动创建。`call` 指令传递基对象构造函数，`ret` 指令发出函数调用结束的信号。