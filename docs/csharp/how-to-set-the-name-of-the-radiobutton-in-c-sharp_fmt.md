...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-name-of-the-radiobutton-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-name-of-the-radiobutton-in-c-sharp/)

在 Windows 窗体中，单选按钮控件用于从选项组中选择一个选项。例如，从给定的列表中选择您的性别，因此您将在三个选项中仅选择一个选项，如男性或女性或变性者。在 Windows 窗体中，您可以使用单选按钮控件的 `Name` 属性为您的单选按钮命名。您可以通过两种不同的方式设置此属性：

## 1. 设计时
最简单的方法是将名称设置为 `RadioButton`，如下图截图所示：
...
### 第三步
拖放后，转到 `RadioButton` 控件的属性窗口以设置名称。
![](img/2fee17d9d8756f5a72bf7d6f90ee227b.png)
...
## 2. 运行时
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法以编程方式将名称设置为单选按钮：

```cs
public string Name { get; set; }
```

该属性的值为 `System.String` 类型，该字符串代表单选按钮的名称。以下步骤显示了如何动态设置单选按钮的名称：
...
### 第二步
创建单选按钮后，设置 `RadioButton` 类提供的单选按钮的 `Name` 属性。
