# Console.ReadKey() 方法在 C# 中

> 原文：[https://www.geeksforgeeks.org/console-readkey-method-in-c-sharp/](https://www.geeksforgeeks.org/console-readkey-method-in-c-sharp/)

`Console.ReadKey()` 方法使程序等待按键，并且它会阻塞屏幕直到按键被按下。简而言之，它获取用户按下的下一个字符或任何键。按下的键会显示在控制台窗口中（如果发生任何输入过程）。此方法的重载列表中有两种方法，如下所示：
...
> **语法：** `public static ConsoleKeyInfo ReadKey();`
>
> **返回值：** 此方法返回一个描述 `ConsoleKey` 常量和 Unicode 字符（如果有的话）的对象，它对应于被按下的键。
>
> **异常：** 如果 `Console` 的 `In` 属性属于控制台以外的某个流，此方法将给出 `InvalidOperationException`。`In` 属性用于获取标准输入流。
...
这种方法与前面的方法更相似，也就是说，它还获取用户按下的下一个字符或任意键。唯一的区别是，按下的键可选地显示在控制台窗口中。

> **语法：** `public static ConsoleKeyInfo ReadKey(bool key);`
> 这里，`key` 用来决定是否在控制台窗口显示按下的键。如果为 `true`，则按下的键不会显示在输出窗口中。如果为 `false`，则按下的键将显示在输出窗口中。
>
> **返回值：** 此方法返回一个描述 `ConsoleKey` 常量和 Unicode 字符（如果有的话）的对象，它对应于被按下的键。
>
> **异常：** 当 `Console` 的 `In` 属性属于控制台以外的某个流时，此方法将给出 `InvalidOperationException`。`In` 属性用于获取标准输入流。
