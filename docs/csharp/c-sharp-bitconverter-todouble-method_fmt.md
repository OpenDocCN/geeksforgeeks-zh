...
> 原文：[https://www.geeksforgeeks.org/c-sharp-bitconverter-todouble-method/](https://www.geeksforgeeks.org/c-sharp-bitconverter-todouble-method/)

`BitConverter.ToDouble()` 方法用于返回字节数组中指定位置的八个字节转换而来的双精度浮点数。
...
这个方法返回一个从 `startIndex` 开始的 8 个字节组成的双精度浮点数。
...
如果 `startIndex` 小于零或大于 `value` 的长度减 1。

### ArgumentException

如果 `startIndex` 大于或等于 `value` 的长度减 7，并且小于或等于 `value` 的长度减 1。

以下程序说明了 `BitConverter.ToDouble(Byte[], Int32)` 方法的使用：
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.bitconverter.todouble?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.bitconverter.todouble?view=netframework-4.7.2)