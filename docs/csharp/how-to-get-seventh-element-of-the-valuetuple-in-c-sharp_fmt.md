...
> 原文：`https://www.geeksforgeeks.org/how-to-get-seventh-element-of-the-valuetuple-in-c-sharp/`

[`ValueTuple`](https://www.geeksforgeeks.org/valuetuple-in-c-sharp/) 是 C# 7.0 中引入的一个结构，代表值类型 Tuple。它允许您存储包含多个值的数据集，这些值可能彼此相关，也可能彼此不相关。`Item7` 属性用于获取给定值元组的第七个未命名元素。它适用于每个值元组，如 7 值元组和 8 值元组。
...
这里，`T7` 是 `ValueTuple<>` 结构的字段值。该 `ValueTuple<>` 可以是 7 值元组，也可以是 8 值元组。

## 示例 1：

在下面的代码中，可以看到我们正在访问每个值元组的第七个元素。
