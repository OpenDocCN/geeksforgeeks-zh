...
> 原文：[https://www.geeksforgeeks.org/c-sharp-program-to-estimate-the-size-of-file-using-linq/](https://www.geeksforgeeks.org/c-sharp-program-to-estimate-the-size-of-file-using-linq/)
...
`GetFiles`：它将返回特定目录或子目录中存在的文件的名称。
...
其中`path`是要搜索的目录。该字符串不区分大小写。

`Select`方法：该方法用于将序列的每个元素投影到一个新的形式中。或者换句话说，当我们想要从指定的集合或序列中获取单个值时，使用这个方法。
...
`FileInfo`类：该类提供了不同类型的属性和实例方法，用于文件的复制、创建、删除、移动和打开，并有助于文件流对象的创建。
...
`Math.Round`方法：此方法用于将一个值舍入到最接近的整数或指定的小数位数。
...
2.  使用`File`类选择文件，使用`Average()`函数计算平均值。

```cs
average = list.Select(file =>new FileInfo(file).Length).Average();
```

3.  使用`Math.Round`函数将尺寸四舍五入到小数点后 1 位。
