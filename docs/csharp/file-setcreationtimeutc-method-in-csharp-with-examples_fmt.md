# File.SetCreationTimeUtc() 方法（C# 中）及示例
...
`File.SetCreationTimeUtc(string, DateTime)` 是一个内置的 `File` 类方法，用于设置文件创建的日期和时间，以协调世界时（UTC）表示。
...
*   `path`：为其设置创建日期和时间信息的指定文件。
*   `creationTimeUtc`：包含为 `path` 创建设置的值的协调世界时日期和时间。
...
*   `ArgumentOutOfRangeException`：`creationTimeUtc` 指定一个超出此操作允许的日期、时间或两者范围的值。
*   `UnauthorizedAccessException`：调用方没有所需的权限。
*   `NotSupportedException`：`path` 的格式无效。

下面是说明 `File.SetCreationTimeUtc()` 方法的程序。
