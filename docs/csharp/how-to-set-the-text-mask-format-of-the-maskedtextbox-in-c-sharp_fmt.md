...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-text-mask-format-of-the-maskedtextbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-text-mask-format-of-the-maskedtextbox-in-c-sharp/)

在 C# 中，`MaskedTextBox` 控件为表单上的用户输入（如日期、电话号码等）提供了一个验证过程。或者换句话说，它被用来提供区分正确和不正确用户输入的屏蔽。在掩码文本框控件中，使用掩码文本框控件提供的 `TextMaskFormat` 属性，设置一个值，该值指示文本和提示字符包含在掩码文本框中的格式化字符串中。这个属性有四个值，这些值是在 `MaskFormat` 枚举下定义的，这些值是：
...
最简单的方法是设置 `MaskedTextBox` 控件的 `TextMaskFormat` 属性的值，如下步骤所示：
...
比上面的方法稍微复杂一点。在此方法中，您可以在给定语法的帮助下，以编程方式设置 `MaskedTextBox` 控件的 `TextMaskFormat` 属性的值：

```cs
public System.Windows.Forms.MaskFormat TextMaskFormat { get; set; }
```

这里，`MaskFormat` 保存 `TextMaskFormat` 属性的值。如果这个属性的值不属于 `MaskFormat` 枚举，那么它将抛出一个 `InvalidEnumArgumentException`。以下步骤显示如何动态设置掩码文本框控件的 `TextMaskFormat` 属性的值：
...
*   **第 2 步:** 创建 `MaskedTextBox` 后，设置 `MaskedTextBox` 类提供的 `MaskedTextBox` 的 `TextMaskFormat` 属性。
