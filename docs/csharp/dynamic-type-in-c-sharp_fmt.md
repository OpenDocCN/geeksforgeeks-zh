...
在 C# 4.0 中，引入了一种新的类型，称为动态类型。它用于避免编译时类型检查。编译器不会在编译时检查动态类型变量的类型，而是在运行时获取类型。动态类型变量是使用 `dynamic` 关键字创建的。
...
*   在大多数情况下，动态类型的行为类似于 `object` 类型。
*   您可以在运行时使用 `GetType()` 方法获取动态变量的实际类型。动态类型会根据右侧的值在运行时改变其类型。如下例所示。
...
*   您还可以在方法中传递动态类型参数，以便该方法可以在运行时接受任何类型的参数。如下例所示。
...
*   它不支持 Visual Studio 中的 `IntelliSense`。
