...
> 原文: [https://www.geeksforgeeks.org/c-program-to-convert-a-binary-string-to-an-integer/](https://www.geeksforgeeks.org/c-program-to-convert-a-binary-string-to-an-integer/)

给定一个二进制字符串作为输入，我们需要编写一个程序，将二进制字符串转换成等价的整数。要将二进制字符串转换为整数，我们必须使用 `Convert.ToInt32` 函数。二进制数的基数是 2。
...
class GFG {
    static void Main(string[] args)
    {
        ...
        // to print the value
        Console.WriteLine("Number value of binary \"{0}\" is = {1}", bin_strng, number);
    }
}
```
