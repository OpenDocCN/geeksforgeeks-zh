...
[`Tuple`](https://www.geeksforgeeks.org/c-sharp-tuple/) 是一种数据结构，它为您提供了最简单的方法来表示具有多个值的数据集，这些值可能彼此相关，也可能彼此不相关。 `Item2` 属性用于获取给定元组的第二个元素。它不适用于 1 元组，也不适用于所有其他剩余元组。

### 语法:
```cs
public T2 Item2 { get; }
```
这里， `T2` 是当前 `Tuple < >`对象的第二个分量的值。这个 `元组< >` 可以是 2 元组，或 3 元组，或 4 元组，或 5 元组，或 6 元组，或 7 元组，或 8 元组。

### 示例:
在下面的代码中，可以看到我们正在访问每个元组的第二个元素。
```cs
