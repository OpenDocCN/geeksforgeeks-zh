...
[`ValueTuple`](https://www.geeksforgeeks.org/valuetuple-in-c-sharp/) 是 C# 7.0 中引入的一个结构，代表值类型 Tuple。它允许您存储包含多个值的数据集，这些值可能彼此相关，也可能彼此不相关。 `Item3` 属性用于获取给定值元组的第三个未命名元素。它适用于每个值元组，如 3 值元组、4 值元组等。
