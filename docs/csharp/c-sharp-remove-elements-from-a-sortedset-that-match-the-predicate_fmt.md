...
> 原文：[https://www.geeksforgeeks.org/c-sharp-remove-elements-from-a-sortedset-that-match-the-predicate/](https://www.geeksforgeeks.org/c-sharp-remove-elements-from-a-sortedset-that-match-the-predicate/)

`SortedSet` 类表示按排序顺序排列的对象集合。这个类属于 `System.Collections.Generic` 命名空间。`SortedSet<T>.RemoveWhere(Predicate<T>)` 方法用于从 `SortedSet<T>` 中移除所有与指定谓词定义的条件相匹配的元素。
...
这个方法返回从 `SortedSet<T>` 集合中移除的元素数量。

## 异常

如果匹配为空，该方法将给出 `ArgumentNullException`。

## 注意

调用这个方法是一个 O(n) 运算，其中 n 是 `Count`，即 `SortedSet` 中包含的元素个数。

下面是举例说明 `SortedSet<T>.RemoveWhere(Predicate<T>)` 方法。
...
- [https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.sortedset-1.removewhere?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.sortedset-1.removewhere?view=netframework-4.7.2)