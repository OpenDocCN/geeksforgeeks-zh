# Array.LastIndexOf 方法 (C#)
## 集合 - 1

> 原文: [https://www.geeksforgeeks.org/array-lastindexof-method-in-c-sharp-set-1/](https://www.geeksforgeeks.org/array-lastindexof-method-in-c-sharp-set-1/)

`Array.LastIndexOf` 方法用于查找一维数组中的最后一个匹配元素。它从数组的最后一个元素开始搜索，返回包含指定值的元素的索引。此方法的重载列表中有 6 种方法，如下所示：
...
> **语法:** `public static int LastIndexOf(Array arr, object value);`
>
> **参数:**
> `arr` : 搜索的一维数组。
> `value` : 数组中要定位的对象。

**返回值:** 如果找到了 `value` 的索引，则该方法返回该 `value` 在整个数组中最后一次出现的索引。如果没有找到，那么返回数组的下限减 1。

**异常:**

*   `ArgumentNullException` : 如果数组 `arr` 为空。
...
> **语法:** `public static int LastIndexOf(Array arr, object value, int start);`
>
> **参数:**
> `arr` : 搜索的一维数组。
> `value` : 数组中要定位的对象。
...
**返回值:** 如果找到了 `value` 的索引，则该方法返回从第一个元素开始延伸的数组 `arr` 中元素范围内该 `value` 最后一次出现的索引。如果没有找到，那么返回数组的下限减 1。

**异常:**

*   `ArgumentNullException` : 如果数组 `arr` 为空。
*   `RankException` : 如果数组 `arr` 是多维的。
*   `ArgumentOutOfRangeException` : 如果 `start` 索引在数组的有效索引范围之外。
...
**语法:**
`public static int LastIndexOf (Array arr, object value, int start, int count);`
...
**返回值:**
如果找到了 `value` 的索引，则该方法返回数组 `arr` 中包含 `count` 中指定元素数量的元素范围内该 `value` 最后一次出现的索引，并在 `start` 索引处结束。如果没有找到，那么数组的下限减 1。

**异常:**

- `ArgumentNullException` : 如果数组 `arr` 为空。
- `RankException` : 如果数组 `arr` 是多维的。
- `ArgumentOutOfRangeException` : 如果 `start` 索引在数组的有效索引范围之外，或者 `count` 小于零，或者 `start` 索引和计数没有在数组中指定有效的部分。
