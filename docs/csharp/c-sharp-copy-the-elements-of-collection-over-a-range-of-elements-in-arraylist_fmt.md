# C# 在数组列表中的一系列元素上复制集合的元素

> 原文：[https://www.geeksforgeeks.org/c-sharp-copy-the-elements-of-collection-over-a-range-of-elements-in-arraylist/](https://www.geeksforgeeks.org/c-sharp-copy-the-elements-of-collection-over-a-range-of-elements-in-arraylist/)

## `ArrayList.SetRange(Int32, ICollection)` 方法

`ArrayList.SetRange(Int32, ICollection)` 方法用于将集合的元素复制到数组列表中的一系列元素上。
...
- `index`：是从零开始的数组列表索引，从这里开始复制 `c` 的元素。该参数的类型为 `System.Int32`。
...
- `ArgumentNullException`：如果 `c` 的值为空。
- `NotSupportedException`：如果数组列表是只读的。
- `ArgumentOutOfRangeException`：如果 `index` *小于零* 或 *`index` + `c` 中元素个数* > `Count`。
...
- [https://docs.microsoft.com/en-us/dotnet/api/system.collections.arraylist.setrange?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.arraylist.setrange?view=netframework-4.7.2)