...
在 C# 中，`Char.Equals()` 是一个 `System.Char` 结构方法，用于通过检查当前实例是否等于指定对象或 `Char` 值来返回值。这个方法可以通过传递不同类型的参数来重载。
...
**返回类型:** 如果给定的 `ob` 参数等于当前实例的值，则返回 `true`，否则返回 `false`。该方法的返回类型为 `System.Boolean`。
...
**返回类型:** 如果给定的 `ob` 参数是 `Char` 的一个实例，并且等于当前实例的值，则返回 `true`，否则返回 `false`。该方法的返回类型为 `System.Boolean`。
...
**参考:** [https://docs.microsoft.com/en-us/dotnet/api/system.char.equals?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.char.equals?view=netframework-4.7.2)