...
> 原文：[https://www.geeksforgeeks.org/what-is-just-in-time-jit-compiler-in-dot-net/](https://www.geeksforgeeks.org/what-is-just-in-time-jit-compiler-in-dot-net/)

准时制编译器是`T4`公共语言运行时的一部分，负责管理`.NET`程序的执行，不考虑任何`.NET`编程语言。特定于语言的编译器将源代码转换为中间语言。这种中间语言然后被实时编译器转换成机器代码。这种机器代码特定于运行`JIT`编译器的计算机环境。

## JIT 编译器的工作方式

`JIT`编译器要求加快代码执行速度，提供多平台支持。其工作原理如下：
