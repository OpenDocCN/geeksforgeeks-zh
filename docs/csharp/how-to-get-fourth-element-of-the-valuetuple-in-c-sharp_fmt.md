...
> 原文：[https://www.geeksforgeeks.org/how-to-get-fourth-element-of-the-valuetuple-in-c-sharp/](https://www.geeksforgeeks.org/how-to-get-fourth-element-of-the-valuetuple-in-c-sharp/)

[`ValueTuple`](https://www.geeksforgeeks.org/valuetuple-in-c-sharp/) 是 C# 7.0 中引入的一个结构，代表值类型 Tuple。它允许您存储包含多个值的数据集，这些值可能彼此相关，也可能彼此不相关。`Item4` 属性用于获取给定值元组的第四个未命名元素。它适用于每个值元组，如 4 值元组、5 值元组等。
...
这里，`T4` 是 `ValueTuple<>` 结构的字段值。该 `ValueTuple<>` 可以是 4 值元组、5 值元组、6 值元组、7 值元组或 8 值元组。

## 示例

### 示例 1

在下面的代码中，可以看到我们正在访问每个值元组的第四个元素。
