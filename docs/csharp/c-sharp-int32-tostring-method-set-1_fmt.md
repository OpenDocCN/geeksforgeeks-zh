# C# | Int32.ToString 方法 | Set-1

> 原文: [https://www.geeksforgeeks.org/c-sharp-int32-tostring-method-set-1/](https://www.geeksforgeeks.org/c-sharp-int32-tostring-method-set-1/)

`Int32.ToString` 方法用于将当前 `Int32` 实例的数值转换为其等价的字符串表示形式。这个方法的重载列表中有 4 个方法如下:
这里，我们将讨论前两个方法。
...
**返回值:** 该方法以 `provider` 参数指定的格式返回当前实例值的字符串表示形式。
**示例:**
