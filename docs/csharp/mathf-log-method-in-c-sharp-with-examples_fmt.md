...
> 原文: [https://www.geeksforgeeks.org/mathf-log-method-in-c-sharp-with-examples/](https://www.geeksforgeeks.org/mathf-log-method-in-c-sharp-with-examples/)

在 C# 中，`MathF.Log()` 是一个 `MathF` 类的方法。它用于返回指定数字的对数。通过更改传递的参数数量，可以重载此方法。`MathF` 的重载列表中总共有 2 种方法。`Log()` 方法如下：
...
> **语法:** `public static float Log(float x);`
> 此处，`x` 为待计算自然对数（以 e 为底）的指定数，类型为 `System.Single`。

**返回值:** 返回 `x` 的自然对数，类型为 `System.Single`。

**注意:** 参数 `x` 始终指定为 10 进制数。返回值取决于传递的参数。以下是一些情况：

*   如果参数为正，则方法将返回自然对数或 **对数<sub>e</sub>(val)**。
*   如果参数是零，那么结果就是负无穷大。
...
> **语法:** `public static float Log(float x, float y);`
>
> **参数:**
> **x:** 是要计算对数的指定数字，类型为 `System.Single`。
> **y:** 是对数的底数，类型为 `System.Single`。

**返回值:** 返回 `x` 的对数，类型为 `System.Single`。
