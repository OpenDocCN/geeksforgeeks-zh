...
> 原文：[`https://www.geeksforgeeks.org/c-sharp-program-to-demonstrate-the-use-of-the-method-as-a-condition-in-the-linq-where-method-of-list-collection/`](https://www.geeksforgeeks.org/c-sharp-program-to-demonstrate-the-use-of-the-method-as-a-condition-in-the-linq-where-method-of-list-collection/)
...
> 1.  创建一个名为`checkstring`的函数，检查字符串长度是否少于 4 个字符，即`str.Length < 4`。
> 2.  创建一个包含员工姓名的列表(例如`XEmployee`)。
> 3.  将员工姓名添加到列表中。
> 4.  现在使用`data`查找长度小于 4 个字符的员工姓名。`Where(employee => checkstring(employee))`。
> 5.  显示员工姓名。
