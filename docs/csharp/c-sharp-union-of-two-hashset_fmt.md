...
一个 `HashSet` 是一个独特元素的无序集合。它属于 `System.Collections.Generic` 命名空间。它用于我们希望防止在集合中插入重复项的情况。就性能而言，与列表相比更好。对于两个 `HashSet` 的并集，使用 `HashSet<T>.UnionWith(IEnumerable<T>)` 方法。
...
如果 `set` 为 `null`，则此方法给出 `ArgumentNullException`。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.hashset-1.unionwith?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.hashset-1.unionwith?view=netframework-4.7.2)