...
> 原文: [https://www.geeksforgeeks.org/c-sharp-explicit-interface-implementation/](https://www.geeksforgeeks.org/c-sharp-explicit-interface-implementation/)
...
这个程序展示了显式接口实现的使用。这里我们有两个接口 `I1` 和 `I2`，它们具有相同的名为 `printMethod` 的方法签名，返回类型为 `void`。类 `C` 实现了这两个接口，因此我们使用显式接口实现来区分方法。
...
这里，我们有一个接口 `Pyramid`，方法是 `绘制金字塔`。类 `Display` 继承了这个方法，并提供了一个在屏幕上打印“***”金字塔的实现。我们使用显式实现来覆盖 `drawPyramid` 方法。
...
我们可以对属性和接口的基本上任何其他成员使用显式接口实现。这里我们在接口 `I1` 和 `I2` 中分别有属性 `X` 和方法 `X`，名称相同，返回类型相同。我们只在方法 `X` 上使用显式接口实现。这样，编译器将使用属性 `X`，除非另有说明。
