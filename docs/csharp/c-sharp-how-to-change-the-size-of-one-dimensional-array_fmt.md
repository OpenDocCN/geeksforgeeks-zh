...
> 原文：[https://www.geeksforgeeks.org/c-sharp-how-to-change-the-size-of-one-dimensional-array/](https://www.geeksforgeeks.org/c-sharp-how-to-change-the-size-of-one-dimensional-array/)

`Array.Resize(T[], Int32)` 方法用于调整数组中元素数量的大小。或者换句话说，此方法用于将一维数组的元素数量更改为指定的新大小。它只调整一维数组的大小，不调整多维数组的大小。
...
- `array`：它是一维的，从零开始调整大小的数组，或者为 `null` 创建一个指定大小的新数组。
- `newSize`：正是新数组的规模。

## 异常

如果 `newSize` 的值小于零，那么这个方法会给出 `ArgumentOutOfRangeException`。

## 注

此方法为 O(n) 运算，其中 n 为新大小。
