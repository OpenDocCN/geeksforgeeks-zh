...
在 C# 中，使用`struct`关键字定义结构。使用`struct`关键字可以定义包含不同数据类型的结构。一个结构也可以包含构造函数、常数、字段、方法、属性、索引器和事件等。
...
在上面的代码中，用数据成员`Name`、`Age`和`Weight`创建了一个名为`Person`的结构。在主方法中，创建结构类型`Person`的`P1`。现在，`P1`可以借助`.`（点）操作符访问其数据成员。

## 复制结构

在 C# 中，用户可以使用`=`（赋值）运算符将一个结构对象复制到另一个结构对象中。
...
`struct Person`的数据成员在`P1`的帮助下初始化，数据成员的值可以由`P1`使用`=`（赋值运算符）复制到`P2`。

## 结构的嵌套

C# 允许将一个结构声明为另一个结构，这个概念被称为结构的嵌套。
