...
> 原文：[https://www.geeksforgeeks.org/c-sharp-program-to-get-file-time-using-file-class/](https://www.geeksforgeeks.org/c-sharp-program-to-get-file-time-using-file-class/)

给定一个文件，现在我们的任务是使用 `File` 类获取文件时间。所以我们使用 `File` 类的 `GetCreationTime()` 方法。此方法用于查找给定文件或目录的创建日期和时间。此方法将只接受一个参数，即文件的路径，如果此路径参数不存在，则它将返回公元 1601 年 1 月 1 日午夜 12:00 协调世界时(UTC)，调整为当地时间。
...
*   `UnauthorizedAccessException`：当调用者没有所需的权限时，就会出现此异常。
...
*   `NotSupportedException`：当文件路径的格式无效时，将出现此异常。
