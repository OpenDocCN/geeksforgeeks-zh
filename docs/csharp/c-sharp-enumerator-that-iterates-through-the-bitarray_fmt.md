...
> 原文：[https://www.geeksforgeeks.org/c-sharp-enumerator-that-iterates-through-the-bitarray/](https://www.geeksforgeeks.org/c-sharp-enumerator-that-iterates-through-the-bitarray/)

`BitArray` 类管理一个紧凑的位值数组，这些位值被表示为布尔值，其中 `true` 表示该位在上是**，即 `1`，`false` 表示该位在**下是**，即 `0`。这个类包含在 `System.Collections` 命名空间。
`BitArray.GetEnumerator` 方法用于**获取一个遍历 `BitArray` 的枚举器。**
...
这个方法为整个数组返回一个 `IEnumerator`。

下面的程序说明了 `BitArray.GetEnumerator` 方法的使用：
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.collections.bitarray.getenumerator?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.bitarray.getenumerator?view=netframework-4.7.2)