...
在 C# 中，所有的异常都是从基类 `Exception` 派生出来的，基类又进一步分为两个分支 `ApplicationException` 和另一个分支 `SystemException`。`SystemException` 是所有 CLR 或程序代码生成错误的基类。`ApplicationException` 是所有应用相关异常的基类。所有异常类都是直接或间接从 `Exception` 类派生的。在 `ApplicationException` 的情况下，用户可以创建自己的异常类型和类。但是 `SystemException` 包含所有已知的异常类型，如 `DivideByZeroException` 或 `NullReferenceException` 等。
...
*   **`NullReferenceException`**：当用户尝试引用一个 `NULL` 类型的对象时出现。

…..还有更多。

**异常类的属性：** 异常类有很多属性，可以帮助用户在异常期间获取关于异常的信息。

*   **`Data`**：该属性有助于获取该属性在键值对中保存的任意数据的信息。
*   **`TargetSite`**：这个属性有助于获取异常将抛出的方法的名称。
*   **`Message`**：该属性有助于提供异常发生的主要原因的详细信息。
*   **`HelpLink`**：该属性有助于保存特定异常的网址。
