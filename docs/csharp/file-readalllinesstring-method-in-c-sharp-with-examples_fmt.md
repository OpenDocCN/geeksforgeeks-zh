# 文件。C# 中的 `ReadAllLines(String)` 方法及示例

> 原文：[https://www.geeksforgeeks.org/file-readalllinesstring-method-in-c-sharp-with-examples/](https://www.geeksforgeeks.org/file-readalllinesstring-method-in-c-sharp-with-examples/)

`File.ReadAllLines(String)` 是一个内置的 `File` 类方法，用于打开一个文本文件，然后将文件的所有行读入一个字符串数组，然后关闭文件。
...
以下是说明 `File.ReadAllLines(String)` 方法的程序。
