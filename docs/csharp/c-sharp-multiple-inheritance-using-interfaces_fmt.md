...
但是 C# 不支持多类继承。为了克服这个问题，我们使用接口来实现多类继承。借助[接口](https://www.geeksforgeeks.org/c-sharp-interface/)，`C` 类（如上图所示）可以得到 `A` 类和 `B` 类的特征。

## `例 1:`
首先我们尝试将 `Geeks1` 和 `Geeks2` 类的特性继承成 `GeeksforGeeks` 类，那么编译器会给出一个错误，因为 C# 直接不支持多类继承。
...
### `运行时错误:`
> `prog.cs(61,30): error CS1721: Class 'GeeksforGeeks' cannot have multiple base classes: 'Geeks1' and 'Geeks2'`
> `prog.cs(35,7): (Location of the symbol related to previous error)`

但是我们可以使用接口将 `Geeks1` 和 `Geeks2` 类的特性间接继承到 `GeeksforGeeks` 类中。如下图所示。

![](img/216c073594d92019bd8c0db754a14197.png)

## `例 2:`
两个 `GFG1` 和 `GFG2` 接口都是由 `Geeks1` 和 `Geeks2` 类实现的。现在 `Geeks1` 和 `Geeks2` 类定义了 `languages()` 和 `courses()` 方法。当 `GeeksforGeeks` 类继承 `GFG1` 和 `GFG2` 接口时，您不需要重新定义 `languages()` 和 `courses()` 方法，只需简单地创建 `Geeks1` 和 `Geeks2` 类的对象，并使用 `GeeksforGeeks` 类中的这些对象访问 `languages()` 和 `courses()` 方法。
