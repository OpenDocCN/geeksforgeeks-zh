...
在 `LINQ`，交叉连接是一个过程，其中两个序列的元素相互组合，意味着第一个序列或集合的元素与另一个序列或集合的元素组合，而没有任何键选择或任何过滤条件。结果序列中存在的元素数量等于两个源序列或集合中元素的乘积。或者换句话说，我们可以说交叉连接会产生笛卡尔乘积。交叉连接也称为完全连接。在交叉连接中，我们不要求在指定连接条件的关键字上有 `*`。
