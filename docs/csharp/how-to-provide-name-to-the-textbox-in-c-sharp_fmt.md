...
> 原文：[https://www.geeksforgeeks.org/how-to-provide-name-to-the-textbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-provide-name-to-the-textbox-in-c-sharp/)

在 Windows 窗体中，文本框扮演着重要的角色。在文本框的帮助下，用户可以在应用程序中输入数据，可以是单行的，也可以是多行的。在文本框中，您可以借助文本框的`Name`属性为文本框控件指定一个名称。此属性的默认值为空字符串。在 Windows 窗体中，可以通过两种不同的方式设置此属性：

## 设计时

设置文本框的`Name`属性是最简单的方法，如以下步骤所示：
...
*   **步骤 3：** 拖放完成后，转到`TextBox`控件的属性窗口，设置`TextBox`的`Name`属性。如下图所示：
...
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法以编程方式设置文本框的`Name`属性：

```cs
public string Name { get; set; }
```

这里，该属性的值为`System.String`类型。以下步骤用于设置文本框的`Name`属性：
...
*   **步骤 2：** 创建文本框后，设置`TextBox`类提供的`Name`属性。
