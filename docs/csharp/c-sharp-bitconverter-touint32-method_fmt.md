...
> 原文:[https://www . geesforgeks . org/c-sharp-bit converter-touint 32-method/](https://www.geeksforgeeks.org/c-sharp-bitconverter-touint32-method/)
...
### 返回值:
这个方法返回一个 32 位无符号整数，由`startIndex`开始的四个字节组成。

### 例外:
*   **参数异常:**如果`startIndex`大于或等于`value`的长度减 3，并且小于或等于`value`的长度减 1。
*   **参数空异常:**如果`value`为空。
*   **argumentoutofrangerexception:**如果`startIndex`小于零或大于`value`的长度减 1。

以下程序说明了`BitConverter.ToUInt32()`方法的使用。
...
### 例 3: 为`ArgumentOutOfRangeException`为

# C# 示例

## 示例 1: 使用 BitConverter.ToUInt16
...
*   `https://docs.microsoft.com/en-us/dotnet/api/system.bitconverter.touint32?view=netframework-4.7.2`