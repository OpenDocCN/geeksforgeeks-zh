...
以上程序只有一个线程，称为主线程。主线程和其他线程一样工作，但是它是自动启动的，你不需要任何`Start()`方法来启动主线程的执行。`thrA`和`thrB`都是主线程的子线程。第一个主线程在那个子线程开始工作后开始工作。
...
语句`Thread.currentThread().join()`，会告诉主线程等待这个线程（即等待自身）死亡。因此，主线程等待自己死亡，这只是一个死锁。