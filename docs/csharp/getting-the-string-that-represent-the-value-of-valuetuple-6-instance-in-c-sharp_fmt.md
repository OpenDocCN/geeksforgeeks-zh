# 获取代表 C# 中 ValueTuple 实例的值的字符串

> 原文：[https://www.geeksforgeeks.org/getting-the-string-that-represent-the-value-of-valuetuple-6-instance-in-c-sharp/](https://www.geeksforgeeks.org/getting-the-string-that-represent-the-value-of-valuetuple-6-instance-in-c-sharp/)

## ValueTuple 简介

[`ValueTuple`](https://www.geeksforgeeks.org/valuetuple-in-c-sharp/) 是 C# 7.0 中引入的一个结构，代表值类型 Tuple。它允许您存储包含多个值的数据集，这些值可能彼此相关，也可能彼此不相关。您也可以在 [`ToString()`](https://www.geeksforgeeks.org/valuetuple-in-c-sharp/) 方法的帮助下获得一个代表 `ValueTuple` 对象的值的字符串。

## ToString 方法

此方法返回一个字符串，该字符串将表示 `ValueTuple<T1, T2, T3, T4, T5, T6>` 对象的值。该方法表示的字符串形式为 `(Item1, Item2, Item3, Item4, Item5, Item6)`，这里 `Item1`、`Item2`、`Item3`、`Item4`、`Item5`、`Item6` 表示 `Item1`、`Item2`、`Item3`、`Item4`、`Item5`、`Item6` 属性的值，它将表示一个字符串。如果任何属性包含空值，则为空。
...
该方法的返回类型为 `System.String`。因此，它将返回一个表示 `ValueTuple<T1, T2, T3, T4, T5, T6>` 对象的字符串。
