...
*   `First<TSource>(IEnumerable<TSource>)`: 此方法返回给定序列或集合的第一个元素，没有任何条件。
*   `First<TSource>(IEnumerable<TSource>, Func<TSource, Boolean>)`: 该方法返回指定给定条件的第一个元素。
...
*   如果给定的集合或序列中不包含任何元素，那么这个方法将抛出一个 `InvalidOperationException`。
