...
> 原文：[https://www.geeksforgeeks.org/c-sharp-set-the-bit-at-a-specific-position-in-the-bitarray-to-the-specified-value/](https://www.geeksforgeeks.org/c-sharp-set-the-bit-at-a-specific-position-in-the-bitarray-to-the-specified-value/)

`BitArray` 类管理一个紧凑的位值数组，这些位值被表示为布尔值，其中 `true` 表示该位在上是**，即 `1`，`false` 表示该位在**下是**，即 `0`。这个类包含在 `System.Collections` 命名空间。
`BitArray.Set(Int32, Boolean)` 方法用于将 `BitArray` 中特定位置**的位设置为指定值。
...
如果 `index` 小于零**或**`index` 大于或等于数组中的元素数，此方法将给出 `ArgumentOutOfRangeException` 异常。

## 注

此方法为 O(1) 运算。

下面的程序说明了 `BitArray.Set(Int32, Boolean)` 方法的使用：
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.collections.bitarray.set?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.bitarray.set?view=netframework-4.7.2)