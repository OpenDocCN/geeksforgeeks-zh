...
> 原文：[https://www.geeksforgeeks.org/c-sharp-program-to-display-the-student-details-using-select-clause-linq/](https://www.geeksforgeeks.org/c-sharp-program-to-display-the-student-details-using-select-clause-linq/)

`LINQ` 被称为语言集成查询，它是在 .NET 3.5 中引入的。它赋予了 .NET 语言生成查询以从数据源检索数据。它消除了编程语言和数据库之间的不匹配，并且无论使用哪种类型的数据源，用于创建查询的语法都是相同的。在本文中，我们将学习如何使用 `LINQ` 的 [`select`](https://www.geeksforgeeks.org/linq-projection-operator-select/) 子句打印学生名单及其详细信息。当我们想从给定的集合中选择一些特定的值时，使用 `Select` 子句。
...
1.  创建一个包含四个变量（`stu_id`、`stu_name`、`stu_dept` 和 `stu_semester`）的学生列表。
