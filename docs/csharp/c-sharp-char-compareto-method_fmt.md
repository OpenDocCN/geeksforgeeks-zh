...
> 原文: [https://www.geeksforgeeks.org/c-sharp-char-compareto-method/](https://www.geeksforgeeks.org/c-sharp-char-compareto-method/)

在 C# 中，`Char.CompareTo()` 是一个 `System.Char` struct 方法，用于比较指定对象或值类型的此实例，并检查给定实例是否在指定对象或值类型之前、之后或出现在排序顺序中的相同位置。可以通过向该方法传递不同类型的参数来重载该方法。
...
**返回类型:** 它返回一个带符号的数字，表示实例在排序顺序中相对于 `ch` 参数的位置。这种方法的返回类型是 `System.Int32`。下表显示了返回值的不同情况:

| 返回值 | 描述 |
| --- | --- |
| 小于零 | 此实例在 `ch` 之前。 |
| 零 | 此实例在排序中的位置与 `ch` 中的位置相同。 |
| 大于零 | 此实例跟在 `ch` 后面。 |
...
> **obj** : 需要与该实例进行比较的对象，否则为 `null`。

**返回类型:** 它返回一个带符号的数字，表示实例在排序顺序中相对于 `obj` 参数的位置。这种方法的返回类型是 `System.Int32`。下表显示了返回值的不同情况:

| 返回值 | 描述 |
| --- | --- |
| 小于零 | 此实例在 `obj` 之前。 |
| 零 | 此实例在排序中的位置与 `obj` 中的位置相同。 |
| 大于零 | 此实例跟随着 `obj` 或者 `obj` 是 `null`。 |

**异常:** 如果对象不是 `Char` 对象，那么这个方法将给出 `ArgumentException`。
...
**参考:** [https://docs.microsoft.com/en-us/dotnet/api/system.char.compareto?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.char.compareto?view=netframework-4.7.2)