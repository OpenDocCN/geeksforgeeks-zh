...
> 原文: [https://www.geeksforgeeks.org/how-to-schedule-a-thread-for-execution-in-c-sharp/](https://www.geeksforgeeks.org/how-to-schedule-a-thread-for-execution-in-c-sharp/)

`Thread.Start` 方法负责调度一个线程执行。这个方法可以通过传递不同的参数来重载。
...
这里，`parameter` 是一个包含线程执行的方法要使用的数据的对象。

**异常:**

*   `ThreadStateException`: 如果线程已经启动。
*   `OutOfMemoryException`: 如果内存不足，无法启动线程。
*   `InvalidOperationException`: 如果线程是使用 `ThreadStart` 委托而不是 `ParameterizedThreadStart` 委托创建的。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.threading.thread.start?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.threading.thread.start?view=netframework-4.7.2)