...
在 C# 中，`Math.DivRem()` 是一个数学类方法，它将两个数字相除并返回余数。通过使用除法运算符，我们不能在单独的变量中得到余数，但是使用 `DivRem()` 方法，我们可以得到两者。
...
> **dividend**：是类型为 `Int32` 的输入被除数。
> **divisor**：是类型为 `Int32` 的输入除数。
> **result**：是 `Int32` 类型的输出余数。

### 返回类型

该函数返回类型 `Int32` 的指定数字的商。

### 异常

如果 `divisor` 的值为零，则该方法将给出 `DivideByZeroException`。
...
此方法用于计算两个 64 位有符号整数（long）的商，并在输出参数中返回余数。

### 语法

```cs
public static long DivRem(long dividend, long divisor, out long result);
```

### 参数

> **dividend**：是类型为 `Int64` 的输入被除数。
> **divisor**：是 `Int64` 类型的输入除数。
> **result**：是 `Int64` 类型的输出余数。

### 返回类型

该函数返回类型 `Int64` 的指定数字的商。
...
**参考文献：** [https://docs.microsoft.com/en-us/dotnet/api/system.math.divrem?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.math.divrem?view=netframework-4.7.2)