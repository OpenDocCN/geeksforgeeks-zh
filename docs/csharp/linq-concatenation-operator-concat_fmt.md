...
> 原文：[https://www.geeksforgeeks.org/linq-concatenation-operator-concat/](https://www.geeksforgeeks.org/linq-concatenation-operator-concat/)

串联是将一个序列附加到另一个序列中的过程。在 LINQ 中，连接操作只包含一个称为 `Concat` 的运算符。它用于追加两个相同类型的序列或集合，并返回一个新的序列或集合。
