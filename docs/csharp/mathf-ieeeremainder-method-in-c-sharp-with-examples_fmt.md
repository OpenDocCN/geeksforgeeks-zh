# MathF.IEEERemainder() 方法及示例

> 原文: [https://www.geeksforgeeks.org/mathf-ieeeremainder-method-in-c-sharp-with-examples/](https://www.geeksforgeeks.org/mathf-ieeeremainder-method-in-c-sharp-with-examples/)

在 C# 中，`IEEERemainder(Single)` 是一个 `MathF` 类方法，用于返回一个指定数除以另一个指定数所得的余数。
...
- `x`: 是 `System.Single` 类型的被除数。
- `y`: 是 `System.Single` 类型的除数。

## 返回类型

此方法返回一个等于 `x - (y * Q)` 的数字，其中 `Q` 是 `x / y` 的商，四舍五入到 `System.Single` 类型的最接近整数。
...
class Geeks {
    ...
    private static void DisplayRemainder(float x, float y)
    {
        var calculation = $"{x} / {y} = ";
