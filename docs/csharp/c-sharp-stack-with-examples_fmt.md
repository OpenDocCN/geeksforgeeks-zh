...
一个[栈](https://www.geeksforgeeks.org/stack-data-structure/)代表一个后进先出的对象集合。当您需要后进先出访问项目时，可以使用它。它既是泛型集合，也是非泛型集合。通用堆栈在`System.Collections.Generic`命名空间中定义，而非通用堆栈在`System.Collections`命名空间中定义，这里我们将讨论非泛型类型栈。堆栈用于创建动态集合，该集合根据程序的需要而增长。在堆栈中，您可以存储相同类型或不同类型的元素。
...
*   堆栈类实现了`IEnumerable`、`ICollection`和`IList`接口。
...
**步骤 1:** 包含`System.Collections`命名空间在你的程序中借助`using`关键字。
...
*   [`Clear`](https://www.geeksforgeeks.org/stack-clear-method-in-c-sharp/): 此方法用于从堆栈中移除所有对象。
...
| 通用堆栈 | 非通用堆栈 |
| --- | --- |
| 通用堆栈在`System.Collections.Generic`命名空间下定义。 | 非通用堆栈在`System.Collections`命名空间下定义。 |
