...
> 原文：[https://www.geeksforgeeks.org/sbyte-equals-method-in-c-sharp-with-examples/](https://www.geeksforgeeks.org/sbyte-equals-method-in-c-sharp-with-examples/)

`SByte.Equals` 方法用于获取一个值，该值指示当前实例是否等于指定的对象或字节。此方法的重载列表中有 2 种方法，如下所示：
*   `Equals(sbyte)` 方法
*   `Equals(Object)` 方法
...
下面的程序说明了 `SByte.Equals(sbyte)` 方法的使用：
...
> 这里，需要一个对象与当前实例进行比较。
>
> **返回值：** 如果 `obj` 是 `SByte` 的实例，并且等于该实例的值，则该方法返回 `true`，否则返回 `false`。
