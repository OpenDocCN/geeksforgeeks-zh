...
> 原文：[https://www.geeksforgeeks.org/checking-if-two-valuetuple-1-are-equal-or-not-in-c-sharp/](https://www.geeksforgeeks.org/checking-if-two-valuetuple-1-are-equal-or-not-in-c-sharp/)
...
1.  [`Equals(ValueTuple<T1>)` 方法](#equalsvaluetuplet1-方法)
2.  [`Equals(Object)` 方法](#equalsobject-方法)

### 1. `Equals(ValueTuple<T1>)` 方法

`Equals(ValueTuple<T1>)` 方法用于检查两个 `ValueTuple<T1>` 实例是否相等。它总是返回 `true`。这种方法的返回类型是 `System.Boolean`。在 `Equals(ValueTuple<T1>)` 方法中，在以下条件下，另一个参数被认为等于当前实例：
...
在 `ValueTuple<T1>` 中，`Equals(Object)` 方法用于返回一个值，该值表明当前实例等于指定的对象。在 `Equals(Object)` 方法中，在以下条件下，`obj` 被认为等于当前实例：
