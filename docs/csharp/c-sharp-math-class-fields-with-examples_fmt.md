...
> 原文：[https://www.geeksforgeeks.org/c-sharp-math-class-fields-with-examples/](https://www.geeksforgeeks.org/c-sharp-math-class-fields-with-examples/)

在 C# 中，`Math`类为对数函数、三角函数和其他数学函数提供常数和静态方法。`Math`类有如下两个字段：
...
**程序 2：** 我们来看一个比较`Math.E`用幂级数计算的值的例子。
...
**程序 1：** 说明`Math.PI`字段在`Math`类中的应用。
...
**程序 2：** 用`Math.PI`常数值演示不同数据类型的乘法运算。
