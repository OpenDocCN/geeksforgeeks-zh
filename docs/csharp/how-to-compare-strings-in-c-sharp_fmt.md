...
> 原文:[https://www.geeksforgeeks.org/how-to-compare-strings-in-c-sharp/](https://www.geeksforgeeks.org/how-to-compare-strings-in-c-sharp/)

字符串是字符的集合，用于存储纯文本。与 C 或 C++不同，C# 中的字符串不以空字符结尾。字符串对象的最大大小取决于系统的内部架构。后面跟`string`的变量实际上是`string`类的对象。

## 如何实例化一个字符串对象？

我们可以通过使用变量名称后跟`string`关键字来创建`string`类的对象。
...
## 方法 1:使用`String.Equals()`方法

`String`类在`.NET`基类库中指定。换句话说，字符串对象是系统的顺序集合，代表字符串的字符对象。`System.String`类是不可变的，也就是说，一旦创建了它的状态，我们就不能对它进行更改。`String.Equals()`方法是`String`类的一种方法。此方法将两个字符串作为参数进行比较。它返回一个逻辑值，`true`或`false`，借助它我们可以确定给定的字符串是否相同。
...
**返回类型:**该方法的返回类型为布尔型。如果两个字符串相等，则为`true`；如果两个字符串不同，则为`false`
...
这是在`String`类下定义的实例方法，它直接应用于字符串或字符串对象。它根据要比较的字符串返回一个数值。这个方法提供了字符串比较的详细信息，这就是为什么它比`String.Equals()`方法更有优势。
...
1.  声明一个静态方法`Compare()`在主方法之外。将此方法的返回类型设置为 `int`。
2.  将变量 `len` 初始化为两个字符串长度的最小值。
3.  使用 `for` 循环迭代`index = 0` 到`index = len–1`。每次迭代时，比较字符串的相应字符。
4.  如果第一个字符串在`index`处的第一个不匹配字符小于第二个字符串在`index`处的字符，那么我们将返回-1。
5.  如果第一个字符串在`index`处的第一个不匹配字符小于第二个字符串在`index`处的字符，那么我们将返回 1。
6.  在 `for` 循环结束后，如果两个字符串的长度相等，则返回 0。如果第一个字符串的长度小于第二个字符串，则返回-1，否则返回 1。
7.  通过传递要比较的字符串作为参数，从主函数调用 `Compare()` 函数。如果`Compare()`函数返回 0，则打印第一个字符串与第二个字符串相同。如果`Compare()`函数返回-1，则打印第一个字符串小于第二个字符串，否则打印第一个字符串大于第二个字符串。
