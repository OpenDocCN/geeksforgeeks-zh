...
[关键词](https://www.geeksforgeeks.org/c-sharp-keywords/) 是一种语言中用于某种内部过程或代表某种预定义动作的词语。`float` 是一个用于声明变量的关键字，该变量可以存储从 `1.5 x 10 <sup>-45</sup>` 到 `3.4 x 10 <sup>38</sup>` 范围内的浮点值。是 `System.Single` 的别名。
...
`float` 关键字占用内存中 4 个字节(32 位)的空间。我们使用后缀 `f` 或 `F` 的值来表示浮点值。
