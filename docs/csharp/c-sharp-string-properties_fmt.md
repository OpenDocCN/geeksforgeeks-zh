...
该字符串是一个字符数组。`String`类将文本表示为一系列 Unicode 字符，并在 .NET 基础类库中定义。`String`类的主要用途是提供属性和方法，这样处理字符串就变得容易了。

## 字符串类的两个属性

1.  `Chars[Int32]`：用于获取当前字符串对象中指定位置的 `Char` 对象。在 C# 中，`Chars` 属性是一个索引器。
2.  `Length`：用于获取当前 `String` 对象中的字符数。
...
*   **参数：** 该方法只取一个参数，即当前字符串中的位置索引，类型为 `System.Int32`。索引参数从零开始。
*   **返回值：** 该方法在索引参数指定的位置返回 `Char` 对象，其属性值类型为 `System.Char`。
