...
属性在 C# 中用于传达关于各种代码元素(如方法、程序集、属性、类型等)的声明性信息或元数据。通过使用声明性标记将属性添加到代码中，该标记使用方括号([ ])放置在所需代码元素的顶部。.NET 框架提供了两种类型的属性实现:
...
*   属性一般来源于`System.Attribute`类。

## 预定义的属性

预定义属性是属于 .NET 框架类库，并且由 C# 编译器支持用于特定目的。从`System.Attribute`基类导出的一些预定义属性给出如下:

| 属性 | 描述 |
| :--- | :--- |
| `AttributeUsageAttribute` | 此属性指定不同属性的用法。 |
| `CLSCompliantAttribute` | 此属性显示特定代码元素是否符合公共语言规范。 |
| `ContextStaticAttribute` | 此属性指示静态字段值对于指定的上下文是否唯一。 |
| `FlagsAttribute` | 此属性指示枚举可以用作一组标志。 |
| `LoaderOptimizationAttribute` | 此属性设置主方法中默认加载程序的优化策略。 |
| `NonSerializedAttribute` | 此属性表示不应序列化可序列化类的字段。 |
| `ObsoleteAttribute` | 该属性标记过时的代码元素，即不再使用的代码元素。 |
| `SerializableAttribute` | 此属性表示可序列化类的字段可以序列化。 |
| `ThreadStaticAttribute` | 该属性表示每个线程都有一个唯一的静态字段值。 |
...
> prog.cs(9,14): warning CS3003: `GFG.z' is not CLS-compliant

### FlagsAttribute

`FlagsAttribute`指定枚举可以用作一组标志。这最常用于按位运算符。
...
*   定义从`System.Attribute`类派生的自定义属性类。
*   自定义属性类名应该有后缀`Attribute`。
*   使用`AttributeUsage`属性指定创建的自定义属性类的用法。
*   创建自定义属性类的构造函数和可访问属性。
