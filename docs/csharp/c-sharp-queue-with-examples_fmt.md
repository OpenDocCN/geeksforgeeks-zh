...
*   队列类实现了`IEnumerable`、`ICollection`和`ICloneable`接口。
*   当您在列表中添加一个项目时，它被称为`入队`。
*   当你移除一个物品时，它被称为`出列`。
...
队列类有四个构造函数，用于创建队列，如下所示：

*   `Queue()`：此构造函数用于创建`Queue`类的一个实例，该实例为空，具有默认的初始容量，并使用默认的增长因子。
*   `Queue(ICollection)`：此构造函数用于创建`Queue`类的实例，该实例包含从指定集合复制的元素，具有与复制的元素数量相同的初始容量，并使用默认的增长因子。
...
**第一步：** 包含`System.Collections`命名空间在你的程序中借助`using`关键字。
...
**步骤 2：** 使用`Queue`类创建一个队列，如下所示：

```cs
Queue queue_name = new Queue();
```

**第三步：** 如果想在队列中添加元素，那么使用`Enqueue()`方法在队列中添加元素。如下例所示。
