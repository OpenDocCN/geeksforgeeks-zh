...
> 原文: [https://www.geeksforgeeks.org/linq-equality-operator-sequenceequal/](https://www.geeksforgeeks.org/linq-equality-operator-sequenceequal/)

相等运算符用于检查给定的两个序列是否相等。在 LINQ 中，等式运算只包含一个称为 `SequenceEqual` 的运算符。它用于检查序列或集合中的给定元素是否相等。如果给定的序列或集合相等，则返回 `true`，否则返回 `false`。
