...
`Math.Atan()` 是一个内置的 `Math` 类方法，它返回其切线作为双值参数给出的角度。如果参数是 `NaN`，那么结果就是 `NaN`。
...
**num:** 是表示切线的数字，该参数的类型为 `System.Double`。
...
[https://msdn.microsoft.com/en-us/library/system.math.atan](https://msdn.microsoft.com/en-us/library/system.math.atan)