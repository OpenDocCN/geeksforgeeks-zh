...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-alignment-of-text-in-checkbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-alignment-of-text-in-checkbox-in-c-sharp/)

`CheckBox` 控件是 Windows 窗体的一部分，用于接受用户的输入。或者换句话说，`CheckBox` 控件允许我们从给定的列表中选择单个或多个元素。在复选框中，您可以使用复选框的 `TextAlign` 属性设置复选框中内容的对齐方式。该属性的默认值为中左。在 Windows 窗体中，可以通过两种不同的方式设置此属性：

## 设计时

使用以下步骤设置复选框的 `TextAlign` 属性是最简单的方法：
...
比上面的方法稍微复杂一点。在此方法中，您可以使用以下语法以编程方式设置 `CheckBox` 的 `TextAlign` 属性：

```cs
public override System.Drawing.ContentAlignment TextAlign { get; set; }
```

这里，`ContentAlignment` 用于表示内容对齐值。以下步骤用于设置复选框的 `TextAlign` 属性：
