...
> 原文：[https://www.geeksforgeeks.org/how-to-set-foreground-color-of-the-groupbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-foreground-color-of-the-groupbox-in-c-sharp/)

在 Windows 窗体中，`GroupBox` 是一个容器，其中包含多个控件，这些控件相互关联。或者换句话说，`GroupBox` 是一组控件周围的框架显示，带有合适的可选标题。或者使用一个组框对一个组中的相关控件进行分类。在分组框中，可以使用 `ForeColor` 属性设置表单中分组框的前景色。您可以通过两种不同的方式设置此属性：

## 1. 设计时

最简单的方法是设置 `GroupBox` 的前景色，如下图步骤所示：

*   **第一步：** 创建如下图所示的窗口表单：
    `Visual Studio->File->New->Project->Windows Forms App`
    ![](img/de9202f1f4646167e60ea580d67273d9.png)
*   **第二步：** 接下来，将 `GroupBox` 从工具箱拖放到窗体上，如下图所示：
    ![](img/3e0d6750b498ddd294dc81fffa68f124.png)
*   **第三步：** 拖放完成后，转到 `GroupBox` 的属性窗口并设置其前景色，如下图所示：
...
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法以编程方式设置 `GroupBox` 的前景色：

```cs
public virtual System.Drawing.Color ForeColor { get; set; }
```

这里，`Color` 表示组框的前景色。以下步骤显示了如何动态设置组框的前景色：
...
*   **步骤 2：** 创建完 `GroupBox` 后，设置 `GroupBox` 类提供的 `ForeColor` 属性。
