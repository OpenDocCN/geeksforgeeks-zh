...
`MemberwiseClone` 方法用于创建当前 `对象` 的浅拷贝或进行克隆。浅拷贝是对象的逐位拷贝。在这种情况下，会创建一个新对象，并且该对象具有现有对象的精确副本。基本上，此方法将当前对象的非静态字段复制到新对象。

*   如果字段是引用类型，则仅复制引用，而非引用的对象。因此，克隆对象和原始对象将引用同一个对象。
*   如果字段是值类型，则将执行该字段的逐位拷贝。
