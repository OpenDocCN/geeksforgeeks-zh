...
> 原文：[https://www.geeksforgeeks.org/c-sharp-check-if-an-array-object-is-equal-to-another-array-object/](https://www.geeksforgeeks.org/c-sharp-check-if-an-array-object-is-equal-to-another-array-object/)

一个[数组](https://www.geeksforgeeks.org/c-sharp-arrays/)是一组相似类型的变量，它们被一个共同的名字所引用。每个数据项被称为数组的一个元素。`Equals(Object)`方法由[`Array`](https://www.geeksforgeeks.org/c-array-class/)类从[`Object`](https://www.geeksforgeeks.org/c-object-class/)类继承而来，用于检查一个数组是否等于另一个数组。
...
如果指定对象等于当前对象，则该方法返回`true`，否则返回`false`。
...
如果当前实例是引用类型，`Equals(Object)`方法检查引用是否相等。