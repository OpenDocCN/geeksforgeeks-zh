...
> 原文：[https://www.geeksforgeeks.org/c-sharp-remove-the-first-occurrence-from-the-stringcollection/](https://www.geeksforgeeks.org/c-sharp-remove-the-first-occurrence-from-the-stringcollection/)

`StringCollection` 类是 .NET Framework 类库中表示字符串集合的类。`StringCollection` 类在 `System.Collections.Specialized` 命名空间中定义。
`StringCollection.Remove(String)` 方法用于从 `StringCollection` 中删除特定字符串的第一个匹配项。
...
这里，`value` 是要从字符串集合中删除的字符串。该值可以为 `null`。

## 注意：

*   `StringCollection` 中允许重复字符串。
*   仅删除第一个匹配项。要删除所有出现的指定字符串，请在 `IndexOf` 不返回 `-1` 时重复使用 `RemoveAt(IndexOf(value))`。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.collections.specialized.stringcollection.remove?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.specialized.stringcollection.remove?view=netframework-4.7.2)