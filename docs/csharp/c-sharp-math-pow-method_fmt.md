...
在 C# 中，`Math.Pow()`是一种数学类方法。这种方法用来计算一个数与另一个数的幂的乘积。
...
- **base**: 是一个要加幂的双精度浮点数，这个参数的类型是`System.Double`。
- **power**: 是一个双精度浮点数，指定一个幂或指数，这个参数的类型是`System.Double`。

## 返回类型:

函数返回升到幂的基数。这种方法的类型是`System.Double`。
