...
> 原文：[https://www.geeksforgeeks.org/difference-between-hashtable-and-dictionary-in-c-sharp/](https://www.geeksforgeeks.org/difference-between-hashtable-and-dictionary-in-c-sharp/)

在 C# 中，字典是一个通用集合，通常用于存储键/值对。字典在 `System.Collections.Generic` 命名空间下定义。它本质上是动态的，意味着字典的大小会根据需要而增长。
...
哈希表是基于键的哈希代码排列的键/值对的集合。或者换句话说，哈希表用于创建使用哈希表进行存储的集合。它是 `System.Collections` 命名空间下定义的非泛型集合类型。在哈希表中，键对象必须是不可变的，只要它们在哈希表中用作键。
...
| 哈希表在 `System.Collections` 命名空间下定义。 | 字典在 `System.Collections.Generic` 命名空间下定义。 |
