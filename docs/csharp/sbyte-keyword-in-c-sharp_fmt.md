...
[关键词](https://www.geeksforgeeks.org/c-sharp-keywords/) 是一种语言中用于某种内部过程或代表某种预定义动作的词语。`SByte` 是一个关键字，用于声明一个变量，该变量可以存储-128 到+127 范围内的有符号值。是 `System.SByte` 的别名。

`SByte` 关键字占用内存 1 字节(8 位)。
