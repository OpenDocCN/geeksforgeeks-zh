...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-visibility-of-listbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-visibility-of-listbox-in-c-sharp/)

在 Windows 窗体中，`ListBox` 控件用于显示列表中的多个元素，用户可以从中选择一个或多个元素，这些元素通常显示在多个列中。在列表框中，可以使用 `ListBox` 的 `Visible` 属性设置列表框的可见性。
如果该属性的值设置为 `true`，则 `ListBox` 控件及其子控件在屏幕上可见，如果该属性的值设置为 `false`，则 `ListBox` 控件及其子控件在屏幕上不可见。此属性的默认值为 `true`。您可以通过两种不同的方式设置此属性：

## 1. 设计时
设置列表框的可见性最简单的方法，如以下步骤所示：
...
*   **步骤 3：** 拖放完成后，转到 `ListBox` 控件的属性窗口以设置其可见性。
    ![](img/1365fdb6bb9d5194570bb89b2efb2242.png)
...
## 2. 运行时
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法以编程方式设置 `ListBox` 控件的可见性：

```cs
public bool Visible { get; set; }
```

这里，该属性的值为 `System.Boolean` 类型。以下步骤显示了如何动态设置列表框的可见性：
...
*   **步骤 2：** 创建 `ListBox` 后，设置 `ListBox` 类提供的 `Visible` 属性。
