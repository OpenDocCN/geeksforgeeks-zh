...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-background-color-of-the-checkbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-background-color-of-the-checkbox-in-c-sharp/)

`CheckBox` 控件是 Windows 窗体的一部分，用于接受用户的输入。或者换句话说，`CheckBox` 控件允许我们从给定的列表中选择单个或多个元素。在 `CheckBox` 中，您可以使用 `CheckBox` 的 `BackColor` 属性来设置 `CheckBox` 的背景颜色。这会让您的 `CheckBox` 更有吸引力。在 Windows 窗体中，可以通过两种不同的方式设置该属性：

## 1. 设计时

使用以下步骤设置 `CheckBox` 的 `BackColor` 属性是最简单的方法：
...
比上面的方法稍微复杂一点。在此方法中，您可以使用以下语法以编程方式设置 `CheckBox` 的 `BackColor` 属性：

```cs
public override System.Drawing.Color BackColor { get; set; }
```

这里，`Color` 用于表示复选框的背景颜色。以下步骤用于设置复选框的 `BackColor` 属性：
...
*   **第 3 步**：最后使用 `Controls.Add()` 方法将该复选框控件添加到表单中。
