...
像类一样，接口可以有方法、属性、事件和索引器作为其成员。但是接口将只包含成员的声明。接口成员的实现将由隐式或显式实现接口的类给出。

- 接口指定类必须做什么，而不是如何做。
- 接口不能有私有成员。
- 默认情况下，接口的所有成员都是公共的和抽象的。
- 该界面将始终借助关键字 `interface` 进行定义。
...
要声明接口，使用 `interface` 关键字。它用于提供全面的抽象。这意味着接口中的所有成员都是用空体声明的，默认情况下是公共的和抽象的。实现接口的类必须实现接口中声明的所有方法。
