...
> 原文: [https://www.geeksforgeeks.org/c-sharp-program-to-print-the-employees-whose-id-is-greater-than-101-using-linq/](https://www.geeksforgeeks.org/c-sharp-program-to-print-the-employees-whose-id-is-greater-than-101-using-linq/)

LINQ 被称为语言集成查询，它是在 .NET 3.5 中引入的。它赋予 .NET 语言生成查询，从数据源中检索数据。它消除了编程语言和数据库之间的不匹配，并且无论使用哪种类型的数据源，用于创建查询的语法都是相同的。在本文中，我们将使用 LINQ 显示 ID 大于 101 的员工详细信息。
...
1.  创建包含三个变量(`emp_id`、`emp_name`和`emp_age`)的员工列表
