...
> 原文: [https://www.geeksforgeeks.org/c-sharp-dictionary-with-examples/](https://www.geeksforgeeks.org/c-sharp-dictionary-with-examples/)
...
字典类有`7`个构造函数用来创建字典，这里我们只使用`Dictionary<TKey, TValue>()`构造函数，如果想了解更多关于构造函数的知识，那就参考**C# | Dictionary Class**。

**`Dictionary<TKey, TValue>()`:** 此构造函数用于创建`Dictionary<TKey, TValue>`类的实例，该类为空，具有默认的初始容量，并使用默认的相等比较器作为键类型，如下所示:

**步骤 1:** 包含`System.Collections.Generic`命名空间在程序中使用关键字的帮助。
...
**步骤 2:** 使用`Dictionary<TKey, TValue>`类创建字典，如下所示:
...
在字典中，您可以从字典中移除元素。`Dictionary<TKey, TValue>`类提供了两种不同的方法来移除元素，这两种方法是:

*   **[`Clear`](https://www.geeksforgeeks.org/c-sharp-dictionary-clear-method/):** 此方法从字典`< TKey, TValue >`中移除所有键和值。
*   **[`Remove`](https://www.geeksforgeeks.org/c-sharp-dictionary-remove-method/):** 此方法从字典`< TKey, TValue >`中移除具有指定键的值。
...
在字典中，您可以检查给定的键或值是否存在于指定的字典中。`Dictionary<TKey, TValue>`类提供了两种不同的检查方法，这两种方法是:

*   **[`ContainsKey`](https://www.geeksforgeeks.org/c-sharp-dictionary-containskey-method/):** 此方法用于检查字典`< TKey, TValue >`是否包含指定的 Key。
*   **[`ContainsValue`](https://www.geeksforgeeks.org/c-sharp-dictionary-containsvalue-method/):** 此方法用于检查字典`< TKey, TValue >`是否包含特定值。
