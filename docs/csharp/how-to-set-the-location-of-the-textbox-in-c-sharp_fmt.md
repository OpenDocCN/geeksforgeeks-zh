...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-location-of-the-textbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-location-of-the-textbox-in-c-sharp/)
...
最简单的方法是设置文本框的 `Location` 属性，如下步骤所示：
...
*   **步骤 3：** 拖放完成后，转到 `TextBox` 控件的属性窗口，设置 `TextBox` 的 `Location` 属性。如下图所示：
...
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法以编程方式设置文本框的 `Location` 属性：

```cs
public System.Drawing.Point Location { get; set; }
```

这里，`Point` 用于表示控件相对于其容器左上角的左上角。以下步骤用于设置文本框的 `Location` 属性：
...
*   **步骤 2：** 创建文本框后，设置 `TextBox` 类提供的文本框的 `Location` 属性。
