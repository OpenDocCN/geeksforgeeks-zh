...
> 原文: [https://www.geeksforgeeks.org/c-sharp-type-getnestedtypes-method/](https://www.geeksforgeeks.org/c-sharp-type-getnestedtypes-method/)

`Type.GetNestedTypes()` 方法用于获取嵌套在当前类型中的类型。此方法的重载列表中有 2 种方法，如下所示:

## `GetNestedTypes()` 方法

此方法用于返回嵌套在当前类型中的公共类型。

> **语法:** `public Type[] GetNestedTypes();`
...
> **语法:** `public abstract Type[] GetNestedTypes(System.Reflection.BindingFlags bindingAttr);`
> 这里，需要一个由一个或多个 `BindingFlags` 组成的位掩码来指定如何进行搜索，或者使用 `Zero` 来返回 `null`。
> **返回值:** 该方法返回一个 `Type` 对象数组，表示嵌套在当前类型中符合指定绑定约束的所有类型(搜索不是递归的)，如果没有找到符合绑定约束的嵌套类型，则返回 `Type` 类型的空数组。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.type.getnestedtypes?view=netframework-4.8](https://docs.microsoft.com/en-us/dotnet/api/system.type.getnestedtypes?view=netframework-4.8)