...
`ArrayList`表示可以单独索引的对象的有序集合。它基本上是数组的替代。它还允许动态内存分配、添加、搜索和排序列表中的项目。`ArrayList.IsFixedSize`属性用于检查数组列表是否有固定大小。
...
如果数组列表有固定的大小，这个方法返回`true`，否则返回`false`。默认值为`false`。

下面的程序说明了`ArrayList.IsFixedSize`属性的使用：
