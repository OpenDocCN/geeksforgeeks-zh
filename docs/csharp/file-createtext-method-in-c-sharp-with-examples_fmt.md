...
> 原文：[https://www.geeksforgeeks.org/file-createtext-method-in-c-sharp-with-examples/](https://www.geeksforgeeks.org/file-createtext-method-in-c-sharp-with-examples/)

`File.CreateText()` 是一个内置的 `File` 类方法，用于用给定的 UTF-8 编码文本覆盖现有文件的内容，如果文件尚未创建，此函数将创建一个具有指定内容的新文件。
...
> *   `Path`：这是将被覆盖 UTF-8 编码文本的文件。如果文件不存在，则创建它。

## 异常：

*   `UnauthorizedAccessException`：调用方没有所需的权限。或者路径指定了一个只读文件。或者路径指定了一个隐藏的文件。
*   `ArgumentException`：`path` 是零长度字符串，仅包含空格或一个或多个无效字符。
...
返回使用 UTF-8 编码写入指定文件的 `StreamWriter`。

以下是说明 `File.CreateText()` 方法的程序。
...
最初，没有创建文件，因此在代码本身下面创建一个名为 `file.txt` 的文件。
