# C# | StringComparer.Compare 方法

> 原文：[https://www.geeksforgeeks.org/c-sharp-stringcomparer-compare-method/](https://www.geeksforgeeks.org/c-sharp-stringcomparer-compare-method/)

`StringComparer.Compare` 方法用于比较两个对象或字符串，并返回它们相对排序顺序的指示。该方法的重载列表中有 2 种方法：
...
**异常：** 如果 `a` 和 `b` 都不是 `String` 对象，且 `a` 和 `b` 都没有实现 `IComparable` 接口，这个方法会抛出 `ArgumentException`。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.stringcomparer.compare?view=netframework-4.8](https://docs.microsoft.com/en-us/dotnet/api/system.stringcomparer.compare?view=netframework-4.8)