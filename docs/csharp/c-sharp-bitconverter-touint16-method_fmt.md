...
> 原文：[https://www.geeksforgeeks.org/c-sharp-bitconverter-touint16-method/](https://www.geeksforgeeks.org/c-sharp-bitconverter-touint16-method/)
...
**返回值：** 该方法返回一个 16 位无符号整数，由从 `startIndex` 开始的两个字节组成。

**异常：**

*   `ArgumentNullException`：如果 `value` 为空。
*   `ArgumentOutOfRangeException`：如果 `startIndex` 小于零或大于 `value` 长度减 1。
*   `ArgumentException`：如果 `startIndex` 等于 `value` 长度减 1。

以下程序说明了 `BitConverter.ToUInt16` 方法的使用：
...
**例 3：** 为 `ArgumentOutOfRangeException` 为例

# C# BitConverter.ToUInt16 方法示例
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.bitconverter.touint16?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.bitconverter.touint16?view=netframework-4.7.2)