...
[关键词](https://www.geeksforgeeks.org/c-sharp-keywords/) 是一种语言中用于某种内部过程或代表某种预定义动作的词语。`bool` 是一个关键字，用于声明一个可以存储布尔值 `true` 或 `false` 的变量。是 `System.Boolean` 的别名。

`bool` 关键字占用内存 1 字节(8 位)。`bool` 只有两种可能的值，即 `true` 或 `false`。
