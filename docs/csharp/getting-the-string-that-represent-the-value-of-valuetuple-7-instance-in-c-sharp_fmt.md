# 获取代表C#中ValueTuple<T1>实例的值的字符串

> 原文：[https://www.geeksforgeeks.org/getting-the-string-that-represent-the-value-of-valuetuple-7-instance-in-c-sharp/](https://www.geeksforgeeks.org/getting-the-string-that-represent-the-value-of-valuetuple-7-instance-in-c-sharp/)

[`ValueTuple`](https://www.geeksforgeeks.org/valuetuple-in-c-sharp/)是C# 7.0中引入的一个结构，代表值类型Tuple。它允许您存储包含多个值的数据集，这些值可能彼此相关，也可能彼此不相关。您也可以在`ToString()`方法的帮助下获得一个代表`ValueTuple`对象的值的字符串。

此方法返回一个字符串，该字符串将表示`ValueTuple<T1, T2, T3, T4, T5, T6, T7>`对象的值。此方法表示的字符串的形式为`(Item1, Item2, Item3, Item4, Item5, Item6, Item7)`，这里`Item1`、`Item2`、`Item3`、`Item4`、`Item5`、`Item6`、`Item7`表示`Item1`、`Item2`、`Item3`、`Item4`、`Item5`、`Item6`、`Item7`的属性值，它将表示一个字符串。如果任何属性包含空值，则为空。
...
该方法的返回类型为`System.String`。因此，它将返回一个表示`ValueTuple<T1, T2, T3, T4, T5, T6, T7>`对象的字符串。
