...
> 原文：[https://www.geeksforgeeks.org/c-sharp-adding-the-elements-to-the-end-of-the-arraylist/](https://www.geeksforgeeks.org/c-sharp-adding-the-elements-to-the-end-of-the-arraylist/)

`ArrayList.AddRange(ICollection)` 方法用于将 `ICollection` 的元素添加到 `ArrayList` 的末尾。
...
这里，`c` 是应该将元素添加到数组列表末尾的 `ICollection`。集合本身不能是 `null`，但是它可以包含 `null` 的元素。

## 异常

*   `ArgumentNullException`：如果 `c` 为 `null`。
*   `NotSupportedException`：如果数组列表是只读的或者数组列表有固定的大小。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.collections.arraylist.addrange?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.arraylist.addrange?view=netframework-4.7.2)