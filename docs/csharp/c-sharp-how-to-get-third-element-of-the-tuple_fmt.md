...
`Tuple`是一种数据结构，它为您提供了最简单的方法来表示具有多个值的数据集，这些值可能彼此相关，也可能彼此不相关。`Item3`属性用于获取给定元组的第三个元素。它不适用于1元组、2元组，但适用于所有其他剩余元组。
...
这里，`T3`是当前`Tuple<>`对象的第三个分量的值。这个`Tuple<>`可以是3元组、4元组、5元组、6元组、7元组或8元组。

## 示例:

在下面的代码中，可以看到我们正在访问每个元组的第三个元素。
