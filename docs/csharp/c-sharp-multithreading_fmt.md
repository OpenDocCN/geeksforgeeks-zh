...
例如，我们有一个名为 `Geek` 的类，这个类包含两种不同的方法，即 `method1()`、`method2()`。现在主线程负责执行所有这些方法，所以主线程逐个执行所有这些方法。
...
**说明：** 这里，首先 `method1()` 执行。在 `method1()` 的 `for` 循环中，当 `I` 的值等于 5 时，从 0 开始，然后该方法进入睡眠 6 秒钟，6 秒钟后，它恢复其过程并打印剩余值。直到 `method2()` 处于等待状态。`method2()` 在 `method1()` 完成分配的任务时开始工作。因此为了克服单线程模型的缺点 `multithreading` 被引入。
