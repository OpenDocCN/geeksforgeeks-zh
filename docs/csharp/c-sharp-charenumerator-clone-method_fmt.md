...
> 原文：[https://www.geeksforgeeks.org/c-sharp-charenumerator-clone-method/](https://www.geeksforgeeks.org/c-sharp-charenumerator-clone-method/)

`CharEnumerator.Clone()` 方法用于创建当前 `CharEnumerator` 对象的副本。这对于在遍历字符串对象时保存当前状态非常有用。
...
## 返回值
该方法返回一个对象，该对象是当前 `CharEnumerator` 对象和当前 `CharEnumerator` 对象的副本。

下面是说明使用 `CharEnumerator.Clone()` 方法的程序：
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.charenumerator.clone?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.charenumerator.clone?view=netframework-4.7.2)