...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-size-of-the-groupbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-size-of-the-groupbox-in-c-sharp/)

在 Windows 窗体中，`GroupBox` 是一个容器，其中包含多个控件，并且这些控件相互关联。或者换句话说，`GroupBox` 是一组控件周围的框架显示，带有合适的可选标题。或者使用一个组框对一个组中的相关控件进行分类。在组框中，可以使用 `Size` 属性设置表单中组框的大小。

该属性以像素为单位表示组框的高度和宽度。您可以通过两种不同的方式设置此属性：

## 1. 设计时

最简单的方法是按照以下步骤设置组框的大小：

*   **Step 1:** 创建一个 Windows 窗体，如下图所示：
    **Visual Studio->File->New->Project->Windows Forms App**
    ![](img/de9202f1f4646167e60ea580d67273d9.png)

*   **Step 2:** 接下来，从工具箱中拖放 `GroupBox` 到窗体上，如下图所示：
    ![](img/3e0d6750b498ddd294dc81fffa68f124.png)

*   **Step 3:** 拖放完成后，转到 `GroupBox` 的属性窗口并设置其 `Size`，如下图所示：
...
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法以编程方式设置 `GroupBox` 的大小：

```cs
public System.Drawing.Size Size { get; set; }
```

这里，`Size` 以像素为单位表示组框的高度和宽度。以下步骤显示了如何动态设置组框的大小：
...
*   **第二步:** 创建完 `GroupBox` 后，设置 `GroupBox` 类提供的 `Size` 属性。
