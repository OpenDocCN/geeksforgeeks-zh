...
命名空间用于组织类。它有助于在更大的 .NET 编程项目中控制方法和类的范围。简单来说，你可以说它提供了一种方法来保持一组名称（比如类名）不同于其他组名称。使用名称空间的最大优点是，在一个名称空间中声明的类名不会与在另一个名称空间中声明的类名冲突。它也被称为**命名的具有共同特征的类组**。名称空间的成员可以是**名称空间、接口、结构和委托**。

## 定义命名空间

要在 C# 中定义命名空间，我们将使用 `namespace` 关键字，后跟命名空间的名称和包含命名空间主体的花括号，如下所示:
...
命名空间的成员通过使用点 (`.`) 运算符访问。C# 中的类通过其各自的命名空间而完全为人所知。
...
*   在 C# 中，类的全名从其命名空间名称开始，后跟点 (`.`) 运算符和类名，后者被称为类的完全限定名。
...
*   在 `System.Console.WriteLine()` 中，`System` 是一个命名空间，在这个命名空间中我们有一个名为 `Console` 的类，它的方法是 `WriteLine()`。
*   没有必要将 C# 中的每个类都保留在 Namespace 中，但是我们这样做是为了很好地组织我们的代码。
*   这里 `.` 是用于分隔类名和命名空间以及函数名和类名的分隔符。

## using 关键字

每次通过使用函数或类的完全限定名来调用函数或类（或者您可以说是命名空间的成员）实际上是不切实际的。在上例中，`System.Console.WriteLine("Hello Geeks!");` 和 `first.Geeks_1.display();` 是全限定名。所以 C# 提供了一个使用的关键字 `using`，帮助用户避免一次又一次的写全限定名。用户只需在程序开始时提到名称空间名称，然后就可以轻松避免使用完全限定的名称。
...
例如，`Generic` 是作为 `System.Collections` 命名空间中的嵌套命名空间。`System.Collections.Generic`
