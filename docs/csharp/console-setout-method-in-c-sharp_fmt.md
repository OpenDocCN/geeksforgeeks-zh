# Console.SetOut() 方法在 C# 中

> 原文: [https://www.geeksforgeeks.org/console-setout-method-in-c-sharp/](https://www.geeksforgeeks.org/console-setout-method-in-c-sharp/)

`Console.SetOut(TextWriter)` 方法在 C# 中用于重定向标准输出流。在这个方法的帮助下，用户可以指定一个 `StreamWriter` 作为输出对象。`Console.SetOut` 方法将接收一个 `TextWriter` 类型的对象。`StreamWriter` 可以传递到 `Console.SetOut` 并隐式转换为 `TextWriter` 类型。它只是将 `Out` 标准输出流属性设置为它获取的指定 `TextWriter` 对象。
...
## 返回值:
将 `StreamWriter` 返回给指定的 `TextWriter` 对象。

## 异常:

*   当 `newOut` 为 `null` 时，**抛出 `ArgumentNullException`**，它不接受它作为有效的参数。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.console.setout?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.console.setout?view=netframework-4.7.2)