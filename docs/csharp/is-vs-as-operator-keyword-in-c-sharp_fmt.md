...
> 原文: [https://www.geeksforgeeks.org/is-vs-as-operator-keyword-in-c-sharp/](https://www.geeksforgeeks.org/is-vs-as-operator-keyword-in-c-sharp/)

[`is`](https://www.geeksforgeeks.org/c-is-operator-keyword/) 和 [`as`](https://www.geeksforgeeks.org/c-as-operator-keyword/) 运算符的区别如下：

*   `is` 运算符用于检查对象的运行时类型是否与给定类型兼容，而 `as` 运算符用于执行兼容引用类型或可空类型之间的转换。
*   `is` 运算符是布尔类型，而 `as` 运算符不是布尔类型。
*   如果给定对象是同一类型的，则 `is` 运算符返回 `true`，而当与给定类型兼容时，`as` 运算符返回该对象。
*   如果给定的对象不是同一类型的，则 `is` 运算符返回 `false`，而如果无法进行转换，则 `as` 运算符返回 `null`。
*   `is` 运算符仅用于引用、装箱和取消装箱转换，而 `as` 运算符仅用于可空、引用和装箱转换。
