...
> 原文：[https://www.geeksforgeeks.org/c-sharp-nesting-of-try-and-catch-blocks/](https://www.geeksforgeeks.org/c-sharp-nesting-of-try-and-catch-blocks/)
...
**示例 1：** 在该程序中，`DivideByZeroException` 在内部 `try` 块中生成，该内部 `try` 块被与内部 `try` 块相关联的 `catch` 块捕获，并继续程序的流程。当 `IndexOutOfRangeException` 在内部 `try` 块内生成且未被内部 `catch` 块捕获时，内部 `try` 块将该异常转移到外部 `try` 块。之后，与外部 `try` 块关联的 `catch` 块捕获导致程序终止的异常。这里对于 **17/0** 和 **24/0** 内部 `try` 块正在执行，但是对于编号 **25** 外部 `try` 块正在执行。
...
**示例 2：** 在下面的示例中，在内部 `try` 块内生成异常，该异常被与内部 `try` 块相关联的 `catch` 块捕获。
