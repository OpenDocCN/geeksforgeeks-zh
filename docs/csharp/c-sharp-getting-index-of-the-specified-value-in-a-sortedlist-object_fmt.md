...
> 原文：[https://www.geeksforgeeks.org/c-sharp-getting-index-of-the-specified-value-in-a-sortedlist-object/](https://www.geeksforgeeks.org/c-sharp-getting-index-of-the-specified-value-in-a-sortedlist-object/)

## `SortedList.IndexOfValue(Object)` 方法

`SortedList.IndexOfValue(Object)` 方法用于获取 `SortedList` 对象中指定值第一次出现的从零开始的索引。
...
这里，`value` 是位于排序列表对象中的值。该值可以为 `null`。

### 返回值

如果在 `SortedList` 对象中找到了 `value`，则该方法返回第一个出现的 `value` 参数的从零开始的索引，否则返回 `-1`。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.collections.sortedlist.indexofvalue?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.sortedlist.indexofvalue?view=netframework-4.7.2)