...
> 原文: [https://www.geeksforgeeks.org/c-sharp-create-hashset-from-another-collection/](https://www.geeksforgeeks.org/c-sharp-create-hashset-from-another-collection/)

## 介绍

一个 `HashSet` 是一个独特元素的无序集合。它属于 `System.Collections.Generic` 命名空间。它用于我们希望防止在集合中插入重复项的情况。就性能而言，与列表相比更好。您可以从另一个集合创建一个 `HashSet`，方法是在创建 `HashSet` 的对象时将该集合作为参数传递。
