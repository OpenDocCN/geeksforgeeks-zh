...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-size-of-the-listbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-size-of-the-listbox-in-c-sharp/)

在 Windows 窗体中，`ListBox` 控件用于显示列表中的多个元素，用户可以从中选择一个或多个元素，这些元素通常显示在多个列中。在列表框中，您可以使用列表框的 `Size` 属性以像素为单位设置列表框的高度和宽度，这使得您的列表框更有吸引力。您可以通过两种不同的方式设置此属性：

## 1. 设计时

最简单的方法是设置列表框的大小，如以下步骤所示：
...
*   **步骤 3：** 拖放完成后，转到 `ListBox` 控件的属性窗口以设置列表框的大小。
    ![](img/eb4ad29f4849cc2cd22bfc7ed4dd7d55.png)
...
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法以编程方式设置 `ListBox` 控件的大小：

```cs
public System.Drawing.Size Size { get; set; }
```

这里，`Size` 以像素为单位表示列表框的高度和宽度。以下步骤显示了如何动态设置列表框的大小：
...
*   **步骤 2：** 创建 `ListBox` 后，设置 `ListBox` 类提供的 `Size` 属性。
