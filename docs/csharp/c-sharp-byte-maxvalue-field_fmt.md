...
`Byte`结构的`MaxValue`字段用于表示字节数据类型的最大值。该字段的值为常量意味着用户不能更改该字段的值。该字段的值为 255。其十六进制值为`0xFF`。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.byte.maxvalue?view=netstandard-2.1](https://docs.microsoft.com/en-us/dotnet/api/system.byte.maxvalue?view=netstandard-2.1)