...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-format-of-the-datetimepicker-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-format-of-the-datetimepicker-in-c-sharp/)

在 Windows 窗体中，`DateTimePicker` 控件用于在窗体中选择和显示具有特定格式的日期/时间。在日期选择器控件中，您可以通过 `Format` 属性设置日期选择器中显示的日期和时间的格式。该属性的值在 `DateTimePickerFormat` 枚举下定义，其值为：

*   `Custom`: 用于以自定义格式显示日期和时间。
*   `Long`: 用于以操作系统设置的长日期格式显示日期和时间。或者换句话说，它同时显示日期和时间。
*   `Short`: 用于以操作系统设定的短日期格式显示日期和时间。或者换句话说，它只显示日期。
*   `Time`: 显示操作系统设定的时间。

此属性的默认值为 `Long`。您可以通过两种不同的方式设置此属性：

## 设计时

设置日期时间选择器的格式是最简单的方法，如以下步骤所示：
...
*   **Step 3:** 拖放完成后，转到 `DateTimePicker` 的属性窗口并设置其格式，如下图所示：
...
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法以编程方式设置 `DateTimePicker` 控件的格式：
...
*   **步骤 2:** 创建日期选择器后，设置由日期选择器类提供的日期选择器的 `Format` 属性。
