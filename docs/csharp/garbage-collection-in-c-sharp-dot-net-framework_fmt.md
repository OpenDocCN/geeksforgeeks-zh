...
> 原文：[https://www.geeksforgeeks.org/garbage-collection-in-c-sharp-dot-net-framework/](https://www.geeksforgeeks.org/garbage-collection-in-c-sharp-dot-net-framework/)
...
*   如果调用`GC.Collect()`方法，然后发生垃圾收集。但是，该方法仅在异常情况下调用，因为通常垃圾收集器会自动运行。
...
堆内存被组织成 3 代，以便在垃圾收集期间可以适当地处理具有不同生存期的各种对象。根据项目规模，每一代的内存将由公共语言运行库提供。在内部，优化引擎将调用`ChooseGeneration()`方法来选择哪些对象将进入第 1 代或第 2 代。
...
该方法返回目标对象的生成号。它需要一个参数，即需要生成号的目标对象。

演示`GC.GetGeneration()`方法的程序如下：
...
### `GC.GetTotalMemory()`方法

此方法返回系统中分配的字节数。它需要一个布尔参数，其中`true`表示方法在返回之前等待垃圾收集的发生，`false`表示相反。

演示`GC.GetTotalMemory()`方法的程序给出如下：
...
可以使用`GC.Collect()`方法在系统中强制进行垃圾收集。此方法需要一个参数，即发生垃圾收集的最早的一代的数量。

演示`GC.Collect()`方法的程序如下：
