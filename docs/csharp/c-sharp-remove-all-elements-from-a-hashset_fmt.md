...
一个 `HashSet` 是一个独特元素的无序集合。它属于 `System.Collections.Generic` 命名空间。它用于我们希望防止在集合中插入重复项的情况。就性能而言，与列表相比更好。`HashSet<T>.Clear()` 方法用于从 `HashSet` 对象中移除所有元素。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.hashset-1.clear?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.hashset-1.clear?view=netframework-4.7.2)