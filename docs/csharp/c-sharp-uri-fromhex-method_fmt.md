...
`Uri.FromHex(Char)`方法用于获取一个十六进制数字的十进制值。
...
## 返回值
该方法返回一个`Int32`值，该值包含一个从 0 到 15 的数字，对应于指定的十六进制数字。

## 异常
如果数字不是有效的十六进制数字(0-9，a-f，A-F)，此方法抛出`ArgumentException`。

下面的程序说明了`Uri.FromHex(Char)`方法的使用:
...
## 参考
*   [https://docs.microsoft.com/en-us/dotnet/api/system.uri.fromhex?view=netstandard-2.1](https://docs.microsoft.com/en-us/dotnet/api/system.uri.fromhex?view=netstandard-2.1)