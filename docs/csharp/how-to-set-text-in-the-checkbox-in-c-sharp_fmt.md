...
> 原文：[https://www.geeksforgeeks.org/how-to-set-text-in-the-checkbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-text-in-the-checkbox-in-c-sharp/)

`CheckBox` 控件是 Windows 窗体的一部分，用于接受用户的输入。或者换句话说，`CheckBox` 控件允许我们从给定的列表中选择单个或多个元素。在复选框中，您可以使用 `CheckBox` 的 `Text` 属性设置复选框中的文本。这会让你的 `CheckBox` 更有吸引力。在 Windows 窗体中，可以通过两种不同的方式设置此属性：

## 1. 设计时

使用以下步骤设置复选框的 `Text` 属性是最简单的方法：
...
比上面的方法稍微复杂一点。在此方法中，您可以使用以下语法以编程方式设置复选框的 `Text` 属性：

`public override string Text { get; set; }`

这里，`CheckBox` 的文本是 `string` 类型的。以下步骤用于设置复选框的 `Text` 属性：

*   **步骤 1：** 使用 `CheckBox` 类提供的 `CheckBox()` 构造函数创建一个复选框。
