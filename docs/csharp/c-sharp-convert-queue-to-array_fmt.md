...
> 原文：[https://www.geeksforgeeks.org/c-sharp-convert-queue-to-array/](https://www.geeksforgeeks.org/c-sharp-convert-queue-to-array/)

`Queue` 代表一个 `先进先出` 集合的对象。当您需要先进先出访问项目时，可以使用它。当您在列表中添加一个项目时，它被称为 `入队`，当您移除一个项目时，它被称为 `出队`。`Queue<T>.ToArray` 方法用于将 `Queue` 元素复制到新数组中。
