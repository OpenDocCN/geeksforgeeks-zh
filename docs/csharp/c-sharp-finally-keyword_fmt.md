...
当 `try`/`catch` 块离开执行时，`finally` 块将执行，不管是什么条件导致的。无论 `try` 块正常终止还是由于异常而终止，它总是执行。`finally` 块的主要目的是释放系统资源。`finally` 块跟随 `try`/`catch` 块。
...
*   `finally` 块不包含任何 `return`、`continue`、`break` 语句，因为它不允许控件离开 `finally` 块。
*   您也可以使用 `finally` 块，只使用 `try` 块，这意味着不使用 `catch` 块，但是在这种情况下，不会处理任何异常。
...
public class GFG {
    ...
    static public void Main() {
        ...
        // finally block
        // This block release the resources of the system
...
在上面的示例中，在 `try` 块内生成了一个异常(`DivideByZeroException`)，该异常被与 `try` 块关联的 `catch` 块捕获。现在，在 `try`-`catch` 块离开它们的执行之后，`finally` 块执行并释放 `try`-`catch` 块使用的系统资源。

## 示例 3：使用带有未处理异常的 `finally` 块
...
public class GFG {
    ...
    static public void Main() {
        ...
        // Here this exception is unhandled
...
在本例中，在 `try` 块内生成了一个异常(`DivideByZeroException`)，但由于与 `try` 块相关联的 `catch` 块不存在，因此不会处理。在这种情况下，当 `try` 块退出执行时，`finally` 块执行并释放系统资源。这意味着 `finally` 块不关心异常是否被处理。