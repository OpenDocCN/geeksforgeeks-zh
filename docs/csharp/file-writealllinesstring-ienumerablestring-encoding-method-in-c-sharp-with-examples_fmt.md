# File.WriteAllLines(String, IEnumerable<String>, Encoding) 方法示例

> 原文: [https://www.geeksforgeeks.org/file-writealllinesstring-ienumerablestring-encoding-method-in-c-sharp-with-examples/](https://www.geeksforgeeks.org/file-writealllinesstring-ienumerablestring-encoding-method-in-c-sharp-with-examples/)

`File.WriteAllLines(String, IEnumerable<String>, Encoding)` 是一个内置的 `File` 类方法，用于使用指定的编码创建一个新文件，将一组字符串写入文件，然后关闭文件。
...
*   `UnauthorizedAccessException`: `path` 指定了一个只读文件。或者 `path` 指定了一个隐藏的文件。或者当前平台不支持此操作。或者 `path` 是一个目录。或者调用者没有所需的权限。

下面是说明 `File.WriteAllLines(String, IEnumerable<String>, Encoding)` 方法的程序。
