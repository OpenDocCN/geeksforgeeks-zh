...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-location-of-the-groupbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-location-of-the-groupbox-in-c-sharp/)

在 Windows 窗体中，`GroupBox` 是一个容器，其中包含多个控件，这些控件相互关联。或者换句话说，`GroupBox` 是一组控件周围的框架显示，带有合适的可选标题。或者使用一个组框对一个组中的相关控件进行分类。在组框中，可以使用 `Location` 属性在表单中设置组框的位置。
...
*   **Step 3:** 拖放完成后，转到 `GroupBox` 的属性窗口并设置其 `Location`，如下图所示：
...
## 运行时
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法以编程方式设置 `GroupBox` 的位置：

```cs
public System.Drawing.Point Location { get; set; }
```

这里，`Point` 代表组框的坐标。以下步骤显示了如何动态设置组框的位置：
