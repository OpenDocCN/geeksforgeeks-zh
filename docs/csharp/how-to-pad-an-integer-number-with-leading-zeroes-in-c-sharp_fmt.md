...
> 原文：[https://www.geeksforgeeks.org/how-to-pad-an-integer-number-with-leading-zeroes-in-c-sharp/](https://www.geeksforgeeks.org/how-to-pad-an-integer-number-with-leading-zeroes-in-c-sharp/)

给定一个数字 `N`，任务是用 C# 中的 `P` 个前导零来填充这个数字。
...
`Format()` 方法用于将指定字符串中的一个或多个格式项替换为指定对象的字符串表示形式。

> **第一步：** 得到数字 `N` 和前导零个数 `P`。
>
> **第二步：** 将数字转换为字符串，填充字符串，使用 `string.Format()` 方法与格式化字符串参数 `"{0:0000}"`（当 `P=4` 时）。
>
> ```cs
...
使用 `ToString()` 方法将当前实例的数值转换为其等价的字符串表示。

> **第一步：** 得到数字 `N` 和前导零个数 `P`。
>
> **第二步：** 使用 `ToString()` 方法将数字转换为字符串，并使用带格式的字符串参数 `"0000"` 填充字符串（表示 `P=4`）。
>
> ```cs
> val = N.ToString("0000");
> ```
...
`PadLeft()` 方法用于通过填充字符串中的字符来向右对齐字符串中的字符。

> **第一步：** 得到数字 `N` 和前导零个数 `P`。
>
> **第二步：** 使用 `ToString()` 方法将数字转换为字符串。
>
> ```cs
> val = N.ToString();
> ```
>
> **第三步：** 然后用 `PadLeft()` 方法填充字符串。
>
> ```cs
