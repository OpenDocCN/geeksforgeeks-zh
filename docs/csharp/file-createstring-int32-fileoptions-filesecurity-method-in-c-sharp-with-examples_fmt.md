...
`File.Create(String, Int32, FileOptions, FileSecurity)` 是一个内置的 `File` 类方法，用于覆盖现有文件，指定缓冲区大小和描述如何创建或覆盖文件的选项，以及决定文件访问控制和审核安全性的值。如果指定的文件不存在，此函数本身会创建一个新文件。
...
返回具有指定缓冲区大小、文件选项和文件安全性的新文件。

以下是说明 `File.Create(String, Int32, FileOptions, FileSecurity)` 方法的程序。
