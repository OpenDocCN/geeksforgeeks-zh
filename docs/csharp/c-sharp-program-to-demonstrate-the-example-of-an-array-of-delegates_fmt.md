...
> 原文：[https://www.geeksforgeeks.org/c-sharp-program-to-demonstrate-the-example-of-an-array-of-delegates/](https://www.geeksforgeeks.org/c-sharp-program-to-demonstrate-the-example-of-an-array-of-delegates/)
...
2. 创建三种常规方法，命名为 `Area`、`Perimeter` 和 `Diagonal`。
...
在上面的例子中，我们创建了一个名为 `Myrect(double len, double width)` 的委托，它需要两个参数。现在我们创建一个名为 `GFG` 的类，它包含三个名为 `Area`、`Perimeter` 和 `Diagonal` 的方法，这些方法采用两个名为 `len`（即矩形的长度）和 `width`（即矩形的宽度）的参数。在主方法中，我们创建两个变量，并用矩形的长度和宽度（即 4 和 5）初始化它们。现在我们创建一个名为 `del` 的大小为 3 的委托数组，并将这些方法传递给委托的每个数组索引。即 `del[0] = Area`；`del[1] = Perimeter`；`del[2] = Diagonal`；。通过调用委托（即 `del[i](len, width)`）迭代 `for` 循环内部的数组来显示数据（这里 `for` 循环迭代三次）。