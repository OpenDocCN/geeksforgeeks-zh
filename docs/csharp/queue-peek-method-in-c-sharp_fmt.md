...
此方法返回队列开头的对象，而不移除它。此方法类似于 `Dequeue` 方法，但 `Peek` 不修改队列，是一个 O(1) 操作。该方法属于 `System.Collections` 命名空间。
...
## 异常：
如果 `Queue` 为空，此方法将给出 `InvalidOperationException`。
...
## 参考：
*   [https://docs.microsoft.com/en-us/dotnet/api/system.collections.queue.peek?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.collections.queue.peek?view=netframework-4.7.2)