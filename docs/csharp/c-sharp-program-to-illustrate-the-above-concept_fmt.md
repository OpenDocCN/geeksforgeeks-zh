...
> 原文：[https://www.geeksforgeeks.org/c-sharp-program-to-illustrate-the-above-concept/](https://www.geeksforgeeks.org/c-sharp-program-to-illustrate-the-above-concept/)
...
在下面的代码中，我们创建了三个类:个人、教师和医生。在这个例子中，`Person` 类被 `Teacher` 类和 `Doctor` 类继承。每个类中的构造函数都用于初始化数据成员。然后我们创建了 `Teacher` 和 `Doctor` 对象，并使用 `TeacherDetails()` 和 `DoctorDetails()` 分别为教师和医生生成信息。
