...
> 原文: [https://www.geeksforgeeks.org/getting-the-string-that-represent-the-value-of-the-tuple-2-instance-in-c-csharp/](https://www.geeksforgeeks.org/getting-the-string-that-represent-the-value-of-the-tuple-2-instance-in-c-csharp/)

一个[元组](https://www.geeksforgeeks.org/c-sharp-tuple/)是一个数据结构，它给你最简单的方法来表示一个数据集。您也可以通过使用`字符串方法`来获取表示元组对象的字符串。该方法返回一个字符串，该字符串将表示`Tuple<T1, T2>`对象。这个方法表示的字符串是`(Item1, Item2)`的形式，这里 `Item1` 和 `Item2` 表示 `Item1` 和 `Item2` 的属性值。它将代表一个字符串。如果任何属性包含空值，则为空。
...
**返回类型:** 该方法的返回类型为`System.String`。因此，它将返回一个表示`Tuple<T1, T2>`对象的字符串。
