...
> 原文：[https://www.geeksforgeeks.org/c-sharp-program-to-reverse-the-list-of-cities-using-linq/](https://www.geeksforgeeks.org/c-sharp-program-to-reverse-the-list-of-cities-using-linq/)

给定一个城市名称列表，我们需要反转城市列表，这可以使用 LINQ 的 `Reverse()` 方法来完成。`Reverse()` 方法反转指定列表中元素的顺序。它在 `Queryable` 类和 `Enumerable` 类中都可用。如果给定的源为空，它将返回 `ArgumentNullException`。
