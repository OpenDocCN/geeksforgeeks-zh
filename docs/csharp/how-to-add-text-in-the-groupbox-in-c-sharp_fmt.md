...
> 原文:[https://www.geeksforgeeks.org/how-to-add-text-in-the-groupbox-in-c-sharp/](https://www.geeksforgeeks.org/how-to-add-text-in-the-groupbox-in-c-sharp/)

在 Windows 窗体中，`GroupBox` 是一个容器，其中包含多个控件，这些控件相互关联。或者换句话说，`GroupBox` 是一组控件周围的框架显示，带有合适的可选标题。或者使用一个组框对一个组中的相关控件进行分类。在组框中，可以使用`Text`属性在表单的组框中添加文本。您可以通过两种不同的方式设置此属性：

## 设计时
在组框中添加文本是最简单的方法，如以下步骤所示：
...
*   **第三步：** 拖放后，转到 `GroupBox` 的属性并添加文本，如下图所示：
...
## 运行时
比上面的方法稍微复杂一点。在此方法中，您可以在给定语法的帮助下，以编程方式在 `GroupBox` 控件中添加文本：
...
*   **第一步：** 使用 `GroupBox` 类提供的 `GroupBox()` 构造函数创建一个 `GroupBox`。
...
*   **第三步：** 最后，将此 `GroupBox` 控件添加到窗体，并使用以下语句将其他控件添加到 `GroupBox`：
