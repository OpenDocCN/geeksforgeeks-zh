...
> 原文：[https://www.geeksforgeeks.org/c-sharp-getting-the-type-of-the-tuples-element/](https://www.geeksforgeeks.org/c-sharp-getting-the-type-of-the-tuples-element/)

元组是一种数据结构，它为您提供了表示数据集的最简单方法。您可以使用 `GetType()` 方法获取元组对象的类型。这个方法将返回元组对象的类型。该方法继承自 [`Object` 类](https://www.geeksforgeeks.org/c-sharp-object-class/)。
...
此方法的返回类型为 `Type`。意味着它返回 `Tuple<>` 对象的类型，其中 `Tuple<>` 可以是 1 元组、2 元组、3 元组、4 元组、5 元组、6 元组、7 元组或 8 元组。
...
```cs
// C# program to illustrate the use of GetType method
using System;
