# UInt32.ToString() 方法在 C# 中的使用（含示例）| Set–1

> 原文：[https://www.geeksforgeeks.org/uint32-tostring-method-in-c-sharp-with-examples-set-1/](https://www.geeksforgeeks.org/uint32-tostring-method-in-c-sharp-with-examples-set-1/)
...
### `UInt32.ToString(String, IFormatProvider)` 方法
...
> **`format`：** 是数字格式字符串。
> **`provider`：** 它是一个提供关于当前实例的区域性特定格式信息的对象。

**返回值：** 该方法返回由 `format` 和 `provider` 参数指定的当前实例的字符串表示形式。
**异常：** 如果 `format` 参数无效，该方法将给出 `FormatException`。
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.uint32.tostring?view=netstandard-2.1](https://docs.microsoft.com/en-us/dotnet/api/system.uint32.tostring?view=netstandard-2.1)