# MathF.Cosh() 方法在 C# 中的应用与示例

> 原文：[https://www.geeksforgeeks.org/mathf-cosh-method-in-c-sharp-with-examples/](https://www.geeksforgeeks.org/mathf-cosh-method-in-c-sharp-with-examples/)

`MathF.Cosh(Single)` 方法是内置的 `MathF` 类方法，它返回给定单精度浮点值参数的双曲余弦值。

**语法：**
```cs
public static float Cosh(float x);
```
这里，`x` 是要返回双曲余弦的数字，这个参数的类型是 `System.Single`。

**返回值：**
此方法返回类型为 `System.Single` 的 `x` 的双曲余弦。如果 `x` 等于负无穷大或正无穷大，则返回正无穷大。如果 `x` 等于 `NaN`，则返回 `NaN`。
