...
> 原文：[https://www.geeksforgeeks.org/checking-if-two-valuetuple-4-are-equal-or-not-in-c-sharp/](https://www.geeksforgeeks.org/checking-if-two-valuetuple-4-are-equal-or-not-in-c-sharp/)
...
1.  [`Equals(ValueTuple<T1, T2, T3, T4>)` 方法](#equalsvaluetuplet1-t2-t3-t4-方法)
2.  [`Equals(Object)` 方法](#equalsobject-方法)

## 1. `Equals(ValueTuple<T1, T2, T3, T4>)` 方法

`Equals(ValueTuple<T1, T2, T3, T4>)` 方法用于检查两个 `ValueTuple<T1, T2, T3, T4>` 实例是否相等。它总是返回 `true`。这种方法的返回类型是 `System.Boolean`。在 `Equals(ValueTuple<T1, T2, T3, T4>)` 方法中，在以下条件下，其他参数被认为等于当前实例：
...
在 `ValueTuple<T1, T2, T3, T4>` 中，`Equals(Object)` 方法用于返回一个值，该值表明当前实例等于指定的对象。在 `Equals(Object)` 方法中，在以下条件下，`obj` 被认为等于当前实例：
