...
> 原文：`https://www.geeksforgeeks.org/index-constructor-in-c-sharp/`
...
这里，`值`是索引值，它是系统值。`Int32` 类型。索引的值应该等于或大于零。`FromEnd` 是布尔值，表示索引是从集合或序列的开始还是从结束开始。在 C# 8.0 或更高版本上执行的以下代码：
