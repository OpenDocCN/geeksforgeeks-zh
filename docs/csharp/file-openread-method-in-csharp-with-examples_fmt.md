# File.OpenRead(String) 方法详解及示例

> 原文: [https://www.geeksforgeeks.org/file-openread-method-in-csharp-with-examples/](https://www.geeksforgeeks.org/file-openread-method-in-csharp-with-examples/)

`File.OpenRead(String)` 是一个内置的 `File` 类方法，用于打开一个现有文件进行读取。
...
* `UnauthorizedAccessException`: `path` 指定了一个目录。或者呼叫者没有所需的权限。
...
以下是说明 `File.OpenRead(String)` 方法的程序。
