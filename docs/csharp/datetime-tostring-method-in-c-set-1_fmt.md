...
> **语法:** `public string ToString(string format, IFormatProvider provider);`
>
> **参数:**
> `format`: 一个标准或自定义的日期和时间格式字符串。
> `provider`: 提供区域性特定格式信息的对象。
>
> **返回值:** 此方法返回由 `format` 和 `provider` 指定的当前 `DateTime` 对象的值的字符串表示形式。

**异常:**

*   `FormatException`: 如果 `format` 长度为 1，并且不是为 `DateTimeFormatInfo` 定义的格式说明符字符之一，则该格式不包含有效的自定义格式模式。
*   `ArgumentOutOfRangeException`: 如果日期和时间超出 `provider` 使用的日历支持的日期范围。

以下程序说明了 `DateTime.ToString(String, IFormatProvider)` 方法的使用:
...
>
> **返回值:** 这个方法返回一个字符串，表示当前日期时间对象的值，由 `format` 指定。

**异常:**

*   `FormatException`: 如果 `format` 长度为 1，并且不是为 `DateTimeFormatInfo` 定义的格式说明符字符之一，或者 `format` 不包含有效的自定义格式模式。
*   `ArgumentOutOfRangeException`: 如果日期和时间在当前区域性使用的日历支持的日期范围之外。

下面的程序举例说明了 `ToString(String)` 方法的使用:
...
*   [https://docs.microsoft.com/en-us/dotnet/api/system.datetime.tostring?view=netframework-4.7.2](https://docs.microsoft.com/en-us/dotnet/api/system.datetime.tostring?view=netframework-4.7.2)