...
> 原文：[https://www.geeksforgeeks.org/c-program-for-converting-hexadecimal-string-to-integer/](https://www.geeksforgeeks.org/c-program-for-converting-hexadecimal-string-to-integer/)

给定一个十六进制数作为输入，我们需要编写一个程序将给定的十六进制数转换成等价的整数。要将十六进制字符串转换为整数，我们必须使用 `Convert.ToInt32` 函数来转换这些值。
...
如果我们为例如 `672g` 输入了错误的值，它会显示错误：`输入一个十六进制数:System.FormatException: 字符串末尾有其他不可解析的字符。`
