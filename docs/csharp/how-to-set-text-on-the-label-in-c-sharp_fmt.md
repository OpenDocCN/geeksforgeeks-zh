...
在 Windows 窗体中，`Label`控件用于在窗体上显示文本，它不参与用户输入或鼠标或键盘事件。您可以使用`Text`属性在标签控件中设置文本。这让你的标签更有吸引力。您可以使用两种不同的方法设置此属性：

## 1. 设计时

使用以下步骤设置标签控件的文本属性是最简单的方法：
...
### 步骤 3
拖放之后，转到`Label`控件的属性以设置`Label`的`Text`属性。
...
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法以编程方式设置`Label`控件中的文本：

```cs
public override string Text { get; set; }
```

这里，该属性的值为`System.String`，表示标签中的内容。以下步骤用于设置标签的文本属性：
