# MathF.Round() 方法在 C# 中的应用（带示例 | 第2部分）

> 原文：[https://www.geeksforgeeks.org/mathf-round-method-in-c-sharp-with-examples-set-2/](https://www.geeksforgeeks.org/mathf-round-method-in-c-sharp-with-examples-set-2/)

在 C# 中，`MathF.Round()` 是一个 `MathF` 类方法，用于将一个值舍入到最接近的整数或指定的小数位数。它返回数字的最近值，其精度等于传递的第二个参数。如果要舍入的值正好介于两个数字（例如，偶数和奇数）的中间，则返回偶数。`MathF.Round()` 适用于 IEEE 标准 754 第 4 节。通过更改传递的参数的数量和类型，可以重载此方法。`MathF.Round()` 方法在重载列表中有 4 种方法，如下所示：
...
> **参数：**
>
> **x**：需要四舍五入的单精度浮点数，此参数类型为 `System.Single`。
>
> **digits**：是返回值中的小数位数，该参数类型为 `System.Int32`。
>
> **mode**：如果值在另外两个数字中间，作为 `MidpointRounding` 的话，如何舍入 **x** 的规范。

**返回类型：** 该方法返回最接近 `x` 的数字，其小数位数等于 `digits`。如果 `x` 的小数位数比 `digits` 少，则 `x` 不变地返回。该方法的返回类型为 `System.Single`。
...
**返回类型：** 该方法返回最接近的整数值。如果 `x` 在两个整数中间，其中一个是偶数，另一个是奇数，那么模式决定返回两者中的哪一个。这种方法的返回类型是 `System.Single`。

**异常：** 如果 `mode` 不是 `MidpointRounding` 的有效值，此方法给出 `ArgumentException`。
