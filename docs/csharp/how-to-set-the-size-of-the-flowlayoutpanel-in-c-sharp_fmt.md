...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-size-of-the-flowlayoutpanel-in-c-sharp/](https://www.geeksforgeeks.org/how-to-set-the-size-of-the-flowlayoutpanel-in-c-sharp/)

在 Windows 窗体中，`FlowLayoutPanel` 控件用于在水平或垂直流动方向上排列其子控件。或者换句话说，`FlowLayoutPanel` 是一个容器，用于在其中水平或垂直组织不同或相同的控件。在 `FlowLayoutPanel` 控件中，可以使用 `Size` 属性设置表单中控件的大小。借助此属性，您可以以像素为单位设置控件的高度和宽度。您可以通过两种不同的方式设置此属性：

## 设计时

设置 `FlowLayoutPanel` 的大小是最简单的方法，如以下步骤所示：

1.  **第一步**：创建如下图所示的窗口表单：
    `Visual Studio->File->New->Project->Windows Forms App`
    ![](img/de9202f1f4646167e60ea580d67273d9.png)
2.  **第二步**：接下来，从工具箱中拖放 `FlowLayoutPanel` 控件到窗体上，如下图所示：
    ![](img/7c56f90e5038e57f1451c130e878cac2.png)
3.  **第三步**：拖放完成后，进入 `FlowLayoutPanel` 的属性窗口并设置其 `Size`，如下图所示：
...
比上面的方法稍微复杂一点。在此方法中，您可以借助给定的语法以编程方式设置 `FlowLayoutPanel` 控件的大小：
...
2.  **步骤 2**：创建完 `FlowLayoutPanel` 后，设置 `FlowLayoutPanel` 类提供的 `Size` 属性。
