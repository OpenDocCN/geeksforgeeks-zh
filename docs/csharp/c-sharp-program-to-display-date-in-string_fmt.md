...
> 原文：[https://www.geeksforgeeks.org/c-sharp-program-to-display-date-in-string/](https://www.geeksforgeeks.org/c-sharp-program-to-display-date-in-string/)

C# 语言中的 `System` 命名空间和 `mscorlib.dll` 程序集提供了多种类和结构。它还提供了 `DateTime` 结构，我们可以使用它来初始化日期和时间对象。使用这个结构，我们还可以确定年、月、工作日等。

![](img/0441c2320feabeba6204d0a2f366427e.png)

**1. DateTime 构造函数：** `DateTime` 构造函数用于初始化 `DateTime` 结构的实例。构造函数方法接受年、月、日、时间作为参数。
...
**2. DateTime.Now() 方法：** 获取当前日期和时间 `DateTime.Now()` 方法使用的是 `DateTime` 结构。它返回一个 `DateTime` 结构的对象，该对象可以用 `ToString()` 方法表示为字符串，如下所述。
...
**3. ToString() 方法：** 每当我们想要以字符串格式打印对象时，这都非常有用。此方法用于将当前 `DateTime` 对象值转换为使用指定格式的字符串。
...
**1.** 该方法用于创建包含“月/日/年”格式的当前数据的 `DateTime` 结构对象。因为它返回了 `DateTime` 结构的一个实例，所以我们可以通过使用 `ToString()` 方法将它用作字符串。比如 2021 年 4 月 12 日。
...
**2.** 该方法用于创建一个 `DateTime` 结构的对象，该对象包含“月 日”格式的当前数据。因为它返回了 `DateTime` 结构的一个实例，所以我们可以使用 `ToString()` 方法将它作为一个字符串使用。比如 04 年 12 月
...
**3.** 该方法用于创建包含当前格式数据的 `DateTime` 结构的对象。因为它返回了 `DateTime` 结构的一个实例，所以我们可以使用 `ToString()` 方法将它作为一个字符串使用。例如，2021 年 12 月 03 日星期六凌晨 04:10
...
**4.** 该方法用于创建包含当前格式数据的 `DateTime` 结构的对象。因为它返回了 `DateTime` 结构的一个实例，所以我们可以使用 `ToString()` 方法将它作为一个字符串使用。例如，2021 年 12 月 04 日星期六 06:40:59
...
**5.** 该方法用于创建包含当前格式数据的 `DateTime` 结构的对象。因为它返回了 `DateTime` 结构的一个实例，所以我们可以使用 `ToString()` 方法将它作为一个字符串使用。例如，12/04/2021 06:41
...
**6.** 该方法用于创建包含当前格式数据的 `DateTime` 结构的对象。因为它返回了 `DateTime` 结构的一个实例，所以我们可以使用 `ToString()` 方法将它作为一个字符串使用。例如，2021 年 4 月 12 日上午 06:44
...
**7.** 该方法用于创建包含当前格式数据的 `DateTime` 结构的对象。因为它返回了 `DateTime` 结构的一个实例，所以我们可以使用 `ToString()` 方法将它作为一个字符串使用。例如，2021 年 4 月 12 日 6:44
...
**8.** 该方法用于创建包含当前格式数据的 `DateTime` 结构的对象。因为它返回了 `DateTime` 结构的一个实例，所以我们可以使用 `ToString()` 方法将它作为一个字符串使用。例如，2021-12-04T07:33:15.9841280+00:00
...
**9.** 该方法用于创建包含当前格式数据的 `DateTime` 结构的对象。因为它返回了 `DateTime` 结构的一个实例，所以我们可以使用 `ToString()` 方法将它作为一个字符串使用。例如，Sat, 04 Dec 2021 07:34:42 GMT
...
**10.** 该方法用于创建包含当前格式数据的 `DateTime` 结构的对象。因为它返回了 `DateTime` 结构的一个实例，所以我们可以使用 `ToString()` 方法将它作为一个字符串使用。例如，2021-12-04T07:41:12
...
class GFG{
    ...
    static public void Main()
    {
        ...
        // Getting the string form of the current date in 
        // a format, 2021-12-04T19:12:29.6715830+00:00           
        ...
        // Getting the string form of the current date in
        // a format, Sat, 04 Dec 2021 19:12:29 'GMT'           
...
currentDate9 : Tue, 07 Dec 2021 09:00:37 'GMT'
