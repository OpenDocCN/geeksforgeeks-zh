...
在 C# 中，字符串是一个 Unicode 字符序列或字符数组。Unicode 字符的范围是从 `U+0000` 到 `U+FFFF`。字符数组也被称为*文本*。所以字符串是文本的表示。字符串是一个重要的概念，人们会混淆字符串是关键字还是对象或类。所以让我们理清这个概念。

一个字符串用类 `System.String` 表示。`string` 关键字是 `System.String` 类的别名，而不是编写 `System.String`。我们可以使用 `string`，这是 `System.String` 类的简写。所以我们可以说 `string` 和 `String` 都可以作为 `System.String` 类的别名。所以 `string` 是 `System.String` 的对象。

**例:**

```csharp
string s1 = "GeeksforGeeks"; // 使用 string 关键字
String S2 = "GFG"; // 使用 String 类
System.String s3 = "Pro Geek"; // 使用 System.String 类创建字符串
```

字符串类在 [.NET](https://www.geeksforgeeks.org/c-net-framework-basic-architecture-component-stack/) 基础类库中定义。换句话说，字符串对象是 `System.String` 类的连续集合的对象。内存中字符串对象的最大大小为 2GB，即大约 10 亿个字符。`System.String` 类是不可变的，即一旦创建，其状态就不能改变。

**程序:** 说明如何声明字符串，初始化字符串。另外，下面的程序显示了一行字符串的声明和初始化。
...
**区别 `string` 和 `System.String`:**

`string` 是 `System.String` 的别名，`string` 和 `System.String` 的意思是相同的，它不会影响应用程序的性能。`string` 是 C# 中的关键字。所以主要的区别来自于上下文，如何使用这些:

*   `string` 用于声明，而不是 `System.String` 用于访问静态字符串方法。
*   `string` 用于声明字段、属性等。它将使用预定义的类型 `System.String`。这是最简单的使用方法。
*   `string` 必须使用 `System.String` 类方法，如 `string.Substring`，`string.IndexOf` 等。`string` 只是 `System.String` 的别名。

**注:** 在 .NET 中，文本被存储为 `Char` 对象的顺序集合，因此在 C# 字符串的末尾没有空终止字符。因此，C# 字符串可以包含任意数量的嵌入空字符(`'\0'`)。
...
**使用 [构造函数](https://www.geeksforgeeks.org/c-sharp-constructors/) 创建字符串:** `String` 类有几个重载的构造函数，它们接受一个字符或字节数组。一些构造函数包含指向字符数组或有符号字节数组的指针作为参数。
...
**使用属性或方法创建字符串:** 检索属性或调用总是返回字符串的方法。例如，使用 `String` 类的方法从更大的字符串中提取子字符串。
...
**使用格式创建字符串:** 使用 `Format` 方法将值或对象转换为其字符串表示形式。`String.Format` 方法返回一个字符串。
