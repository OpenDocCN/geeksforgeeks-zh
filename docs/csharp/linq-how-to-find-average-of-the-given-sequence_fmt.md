...
> 原文：[https://www.geeksforgeeks.org/linq-how-to-find-average-of-the-given-sequence/](https://www.geeksforgeeks.org/linq-how-to-find-average-of-the-given-sequence/)

在 LINQ 中，你可以使用 `Average` 方法找到给定序列的平均值。此方法返回给定序列或集合中元素的平均值。它返回可空或不可空的 `decimal`、`double`、`float`、`int` 等类型的值。它不支持 C# 中的查询语法，但在 VB.NET 中支持。它在 C# 的 `Enumerable` 和 `Queryable` 类中都可用。
