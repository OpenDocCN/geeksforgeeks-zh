...
> 原文：[https://www.geeksforgeeks.org/basics-operations-of-file-and-directory-in-c-sharp/](https://www.geeksforgeeks.org/basics-operations-of-file-and-directory-in-c-sharp/)

在本文中，我们将介绍如何创建、删除和重命名目录，以及如何删除和重命名文件。

## 创建目录

我们可以使用 `Directory` 类中的 `CreateDirectory()` 方法创建目录。
...
您将在特定位置找到更新后的目录名称。没有所谓的 `Rename()` 方法，所以我们使用 `Move()` 方法来重命名目录。移动和重命名是 C# 中相同的操作。
