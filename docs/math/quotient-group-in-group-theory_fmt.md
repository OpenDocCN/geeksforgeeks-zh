...
> 原文: [https://www.geeksforgeeks.org/quotient-group-in-group-theory/](https://www.geeksforgeeks.org/quotient-group-in-group-theory/)

## 先决条件:
[群体知识](https://www.geeksforgeeks.org/groups-discrete-mathematics/)，[陪集](https://www.geeksforgeeks.org/cosets-in-mathematics/)。
...
**例–** a. `(N, +)` 和 b. `(R, +, ·)`，其中 `N` 是一组自然数 & `R` 是一组实数。给你。`(·)` 指定乘法运算。
...
3.  **逆的存在–** 对于每个元素 `a ∈ G`；存在逆 `(a⁻¹) ∈ G`，使得：`aoa⁻¹ = a⁻¹oa = e`
...
`= { {0, 3}, {1, 4}, {2, 5} }`（因为 `{3, 0}` 与 `{0, 3}` 相同，等等）
