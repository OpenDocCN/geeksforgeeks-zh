...
> 原文: [https://www.geeksforgeeks.org/hypergraph-its-representation-discrete-mathematics/](https://www.geeksforgeeks.org/hypergraph-its-representation-discrete-mathematics/)

## 简介
超图是超边（广义边）可以连接到顶点/节点的子集而不是两个顶点/节点的图。
超图的边（也称为超边）是任意的非空顶点集。一个 `k`-超图的所有超边恰好连接 `k` 个顶点；因此，普通图是 `2`-超图（因为一条边连接 `2` 个顶点）。

## 超图表示
无向超图 `H` 定义为一对 `H = (V, E)`，其中 `V` 是一组称为节点或顶点的项，`E` 是 `V` 的一组非空子集，称为超边或边（在无向超图中）。
...
## 超图到二部图
因为用二部图来表示超图总是可能的（尽管不总是方便的），所以超图很少被使用。二部图中的顶点集可分为 `P` 和 `Q` 两个子集，每条边将 `P` 中的一个顶点连接到 `Q` 中的一个顶点。
我们简单地将 `H` 的顶点表示为 `Q` 中的顶点，将 `H` 的超边表示为 `P` 中的顶点，并且每当 `s` 是 `H` 中的超边 `t` 的成员时插入一条边 `(P, Q)`。

![](img/299595ba5f7b38584841766c1a89deac.png)

超图有两种描述方式。左边的五个顶点由三个超边连接。在右侧，相同的五个顶点通过普通边连接到表示超边的新顶点（三个）。
...
*   **`d`-正则**
    每个顶点都有一个 `d` 度，这意味着它包含在精确的 `d` 个超边中。
...
*   **非简单**
    有环（单个顶点的超边）或重复边（两个或多个顶点集合相同的边）。
    例：
    在下图中，我们可以看到 `2` 个环：`e1` & `e2`，所以是一个非简单超图。
...
*   **`k` 均匀**
    每个超边都是由正好 `k` 个顶点组成的。
    示例：
    在下面的超图中，我们可以看到每个超边（`e1`，`e2`，`e3`，`e4`）由 `2` 个顶点组成，因此它是一个 `2`-均匀超图。
...
*   **`k`-划分**
    每个超边恰好包含每种类型的一个顶点，这些顶点被分成 `k` 个部分。
    例如：
