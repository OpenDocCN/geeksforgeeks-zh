...
> 原文: [https://www.geeksforgeeks.org/peano-axioms-number-system-discrete-mathematics/](https://www.geeksforgeeks.org/peano-axioms-number-system-discrete-mathematics/)

## 引言
自然数的集合在下面被公理化地定义。意大利数学家 G. 佩亚诺和德国数学家 J. W. R. 戴德金被认为是这些公理的发明者。这些公理的目的是证明一个自然数的存在，然后定义一个函数来创建剩余的自然数，称为后继函数。

## 皮亚诺公理
进一步推理和论证的前提或起点是公理、公设或假设，即被假定为真的陈述。佩亚诺提出的公理是：
...
8.  **`∀ a, b∈N；如果 S(a) = S(b), 则 a = b`。**
    ...
    所以，如果我们说这两个后继者是相同的，那么就意味着它们是相同数量的后继者。
9.  **`如果 V 是感应集；即：0 ∈ V 和每一个自然数 n ∈ V, 则 S(n)∈V, 且 N ⊂ V`。**
