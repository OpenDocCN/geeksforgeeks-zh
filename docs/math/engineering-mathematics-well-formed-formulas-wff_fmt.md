...
> 原文：`https://www.geeksforgeeks.org/engineering-mathematics-well-formed-formulas-wff/`
...
3.  如果 `P` 和 `Q` 是 WFF，那么 `(P ∨ Q)`，`(P ∧ Q)`，`(P → Q)`，`(P ⇔ Q)` 等也是 WFF。

### **格式良好的公式示例:**

| WFF | 说明 |
| --- | --- |
| `P` | 通过**规则 1** 每个语句本身就是一个 WFF，`P` 是一个 WFF，让 `P = Q`，那么 `Q` 也将是一个 WFF。 |
| `(P ⇒ Q) ⇒ Q` | 通过**规则 3** 连接 `(P ⇒ Q)` 和带有连接符号的 `Q`。 |
| `((Q ∧ P))` | 通过**规则 3** 将 `Q` 和 `P` 与连接符号 `∧` 连接起来。 |
| `((P ∨ Q) ∧ Q)` | 通过**规则 3** 用连接符号 `∧` 连接 `(P ∨ Q)` 和 `Q`。 |
| `(¬(P ∨ Q) ∧ Q)` | 通过**规则 3** 用连接符号 `∧` 连接 `(P ∨ Q)` 和 `Q`，然后使用规则 2。 |
