...
> 原文：[https://www.geeksforgeeks.org/elementary-matrices-discrete-mathematics/](https://www.geeksforgeeks.org/elementary-matrices-discrete-mathematics/)
...
**解释–**
我们知道，3 * 3 : `I_3` 的身份矩阵给出为–

![](img/c0a1ad76aa725e1ccf7d150552788fef.png)
矩阵 `I_3`

通过应用 `C1`-交换 `I_3` 中的列 1 和列 3，我们可以得到矩阵 `M`，所以 `M` 是一个初等矩阵。
...
*   初等矩阵总是方阵。
*   如果基本矩阵 `E` 是通过对 `I_m` 执行特定的行运算得到的，`A` 是 `m*n` 矩阵，则乘积 `EA` 是通过对 `A` 执行相同的行运算得到的矩阵。
...
如果我们在 `I_3` 上“执行操作”：`R1 = R1 * 6`，我们会得到–
