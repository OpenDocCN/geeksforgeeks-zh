...
> 原文：[https://www.geeksforgeeks.org/four-color-theorem-and-kuratowskis-theorem-in-discrete-mathematics/](https://www.geeksforgeeks.org/four-color-theorem-and-kuratowskis-theorem-in-discrete-mathematics/)
...
## 如何上色？
取任意一张地图，将其划分为一组相连的区域：`R1`，`R2`……`Rn`，边界连续。
必须有某种方式来分配集合 `{R，G，B，Y}` 中的每个区域 `Ri`，使得如果两个区域 `Ri` 和 `Rj` 是“接触的”（即它们之间共享一些非零长度的边界），它们必须接收不同的颜色。

### 示例 1
四色地图如下所示：

![平面地图](img/f7b801365cf603076596c2bb6a69b54c.png)

在这里，正如你所看到的，每个接触到另一个区域的区域都有不同于接触到的区域的颜色。我们总共需要最多四种颜色来给这张地图上色——红色、绿色、蓝色和黄色。

### 示例 2
未着色地图 `G` 到彩色地图的转换如下所示：

![地图 G](img/bdec01a4c41ed21458dcbfd746631b60.png)

在这里，您可以看到每个接触到另一个区域的区域都有不同于接触到的区域的颜色。我们总共需要最多四种颜色来给这张地图上色——红色、绿色、蓝色和黄色。

### 示例 3
未着色地图 `H` 到彩色地图的转换如下所示：

![地图 H](img/b52e7a81be5d322e62a63d6b8592325b.png)
...
为了证明这个定理，我们将通过一些定义，并确保 `K3,3` 和 `K5` 都是非平面的。来看看 `K3,3`。
...
**证明：**
现在，我们用矛盾来证明。
相反，说 `K3,3` 是平面的。然后是 `K3,3` 的平面嵌入满足：
然后，通过欧拉公式：`v - e + f = 2`，其中 `v` = 顶点总数，`e` = 边数，`f` = 面总数。

![K3,3 和 K5 图](img/eebbc71c07c5c50ab32ba6d55611ff4b.png)

在图 (a) 中，双向图：`v= 6`，`e= 9`。
由于 `K3,3` 是二分的，里面没有 3-圈（里面可以有奇数圈）。
因此，嵌入的每个面必须由至少 4 条从 `K3,3` 开始的边界定。
此外，每个边在面的边界中被计数两次。
所以必须有：`f ≤ 2 * e / 4`
`f ≤ e / 2`
`f ≤ 4.5`。
