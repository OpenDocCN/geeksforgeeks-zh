...
> 原文：[https://www.geeksforgeeks.org/proof-why-probability-of-complement-of-a-equals-to-one-minus-probability-of-a-pa-1-pa/](https://www.geeksforgeeks.org/proof-why-probability-of-complement-of-a-equals-to-one-minus-probability-of-a-pa-1-pa/)
...
*   [**补语**](https://www.geeksforgeeks.org/set-operations/#:~:text=Complement,set%20A%20is%20U%20%E2%80%93%20A.) **–**
    在集合中，集合的补语，`A` 是 `A` 中不存在的所有元素的集合，用 `A'` 或 `A^c` 表示。例如，在一个滚动骰子的实验中，如果 `A` 是所有偶数结果的集合，那么，`A'` 就是所有奇数结果的集合。
...
> 3.  If `a1`, `a2`, `a3`, `a4` ... `aN` are mutually exclusive events, then the probability of the combination of these mutually exclusive events will be equal to the sum of the probabilities of these mutually exclusive events. That is, `P(a1 ∪ a2 ∪ ... ∪ aN) = P(a1) + P(a2) + ... + P(aN)`.
...
1.  考虑一个事件，因为一个实验的样本空间包含了一个实验的所有可能的结果，而 `A` 和 `A'` 的[并集包含了这个实验的所有可能的结果](https://www.geeksforgeeks.org/union-and-intersection-of-sets/)。所以，样本空间可以写成：
...
>
> 即 `S = A ∪ A'`, 由于 `A ∩ A' = ∅`，我们可以说 `A` 和 `A'` 是互斥事件，这意味着：
>
> `P(S) = P(A ∪ A')`
>
> `P(S) = P(A) + P(A')`
