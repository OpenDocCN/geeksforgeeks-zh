...
> 原文:[https://www.geeksforgeeks.org/types-of-proofs-predicate-logic-discrete-mathematics/](https://www.geeksforgeeks.org/types-of-proofs-predicate-logic-discrete-mathematics/)
...
### 5. 逆正证明
我们可以通过证明 `¬Q ⇒ ¬P` 来间接证明 `P ⇒ Q`，假设 `¬Q`，然后利用推理规则、公理、定义和逻辑等价证明 `¬P`。
...
假设 `n` 为奇数。
那么:`n = 2p+1`；对于某些整数 `p`.
则 `n^2=(2p+1)^2= 4p^2+4p+1 = 2(2p^2+2p)+1`，；也就是 `2*(某个整数)+ 1`。
因此，我们可以说 `n^2` 是奇数。于是 `Q` 真。

**2. `Q ⇒ P`:** 这里我们使用的是反正证明方式。
`¬Q` : `n^2` 为偶数，`¬P` : `n` 为偶数。
我们需要证明:`¬Q ⇒ ¬P`(`P ⇒ Q` 表示 `¬Q ⇒ ¬P`)
假设 `n` 为偶数，
则 `n = 2p`；对于某个整数 `p`.
那么 `n^2 =(2p)^2= 4p^2= 2(2p^2)`，这是一个偶数，因为它可以被 2 整除。

从(1.)`P ⇒ Q` & from(2)`¬Q ⇒ ¬P`，`n` 可以是奇数当且仅当 `n^2` 是奇数。

### 问题2: 如果一个数能被 4 整除，那么它也能被 2 整除。
**解:**
使用直接证明:
假设:`x` 可被 4 整除
