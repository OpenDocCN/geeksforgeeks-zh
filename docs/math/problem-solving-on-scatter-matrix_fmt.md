...
> 原文: [https://www.geeksforgeeks.org/problem-solving-on-scatter-matrix/](https://www.geeksforgeeks.org/problem-solving-on-scatter-matrix/)

**先决条件:** [散点图矩阵](https://www.geeksforgeeks.org/scatter-plot-matrix/)

我们计算可用数据点的 `S_w` (类散布矩阵内)和 `S_B` (类散布矩阵间)。

`S_W` : 为了最小化类内的可变性，类内分散。
`S_B` : 增加阶级之间的可变性，阶级之间的分散性。
