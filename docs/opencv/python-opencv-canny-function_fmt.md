...
> **语法:** `cv2.Canny(image, T_lower, T_upper, aperture_size, L2Gradient)`
>
> **其中:**
>
> *   `image`: 将应用 Canny 滤波器的输入图像
> *   `T_lower`: 滞后阈值中的下阈值
> *   `T_upper`: 滞后阈值中的上限阈值
...
*   使用索贝尔滤波器计算图像梯度。
*   应用非最大值抑制（NMS）来细化边缘。
*   最后，应用滞后阈值，该阈值在 `Canny()`函数中使用两个阈值 `T_upper` 和 `T_lower`。
...
这是一个布尔参数，指定是要计算通常的梯度方程还是 L2Gradient 算法。同样，这是一个可选参数。`L2gradient` 算法计算 `sqrt(gradient_x^2 + gradient_y^2)`，而 `L1gradient` 只是 `abs(gradient_x) + abs(gradient_y)`。
