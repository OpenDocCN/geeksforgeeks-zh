...
> 原文: [https://www.geeksforgeeks.org/python-opencv-cv2-puttext-method/](https://www.geeksforgeeks.org/python-opencv-cv2-puttext-method/)

`OpenCV-Python`是一个Python绑定库，旨在解决计算机视觉问题。`cv2.putText()`方法用于在任意图像上绘制文本字符串。
...
> `字体`: 表示字体类型。字体类型有`FONT_HERSHEY_SIMPLY`、`FONT_HERSHEY_PLAIN`等。
