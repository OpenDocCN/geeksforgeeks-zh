...
> 原文:[https://www.geeksforgeeks.org/python-opencv-pose-estimation/](https://www.geeksforgeeks.org/python-opencv-pose-estimation/)
...
要了解更多关于 OpenCV 的信息，请访问[https://opencv.org/about/](https://opencv.org/about/)

## 可以使用的数据集

在数据集选择中，COCO 和 MPII 是最近案例中的默认选择。特别是，COCO 是一个著名的数据集，因为它具有非常宽的人体姿态和大量的图像。LSP 和 FLIC 数据集也用在 COCO 和 MPII 旁边。

*   [http://cocodataset.org/#keypoints-2018](http://cocodataset.org/#keypoints-2018)
*   [http://human-pose.mpi-inf.mpg.de/](http://human-pose.mpi-inf.mpg.de/)
*   [http://www.robots.ox.ac.uk/~vgg/data/pose_evaluation/](http://www.robots.ox.ac.uk/~vgg/data/pose_evaluation/)
...
检查文件夹以确保模型二进制文件（`.caffemodel` 文件）已下载。如果你不能运行以上脚本，那么你可以点击[这里](http://posefs1.perception.cs.cmu.edu/OpenPose/models/pose/mpi/pose_iter_160000.caffemodel)下载 MPII 模型，点击[这里](http://posefs1.perception.cs.cmu.edu/OpenPose/models/pose/coco/pose_iter_440000.caffemodel)下载 COCO 模型。
...
我们使用 OpenCV 读取的输入帧应该被转换成一个输入斑点（像 Caffe），以便它可以被馈送到网络。这是使用 `blobFromImage` 函数完成的，该函数将图像从 OpenCV 格式转换为 Caffe blob 格式。
