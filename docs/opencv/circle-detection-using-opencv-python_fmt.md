...
> 原文: [https://www.geeksforgeeks.org/circle-detection-using-opencv-python/](https://www.geeksforgeeks.org/circle-detection-using-opencv-python/)
...
为了检测圆，我们可以固定一个点`(x, y)`。现在，我们需要找到 3 个参数：`a`、`b` 和 `r`。因此，问题是在三维搜索空间中。为了找到可能的圆，该算法使用一个称为“累加器矩阵”的三维矩阵来存储潜在的 `a`、`b` 和 `r` 值。`a`（中心的 x 坐标）的值可以在 1 到 `rows` 的范围内，`b`（中心的 y 坐标）可以在 1 到 `cols` 的范围内，`r` 可以在 1 到 `maxRadius = ![\sqrt{rows^2 + cols^2}](img/dc5f4aa61f83f3d738a0883f674468ff.png "Rendered by QuickLaTeX.com")`的范围内。

下面是算法的步骤。

*   **初始化累加器矩阵：** 用零初始化维度 `rows * cols * maxRadius` 的矩阵。
...
> **param1：** Canny 边缘检测需要两个参数——`minVal` 和 `maxVal`。`param1` 是两者中较高的阈值。第二个参数设置为 `param1/2`。
> **param2：** 这是候选检测圆的累加器阈值。通过增加该阈值，我们可以确保只返回对应于较大累加器值的最佳圆。
> **minRadius：** 最小圆半径。
> **maxRadius：** 最大圆半径。
