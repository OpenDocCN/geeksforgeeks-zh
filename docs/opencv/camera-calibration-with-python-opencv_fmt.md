...
> 原文: [https://www.geeksforgeeks.org/camera-calibration-with-python-opencv/](https://www.geeksforgeeks.org/camera-calibration-with-python-opencv/)
...
*   **第二步:** 捕捉棋盘图像的不同视点。
*   **第三步:** `findChessboardCorners()` 是 `OpenCV` 中的一种方法，用于为不同图像中的每个 3D 点寻找像素坐标 `(u, v)`。
*   **第四步:** 然后用 `calibrateCamera()` 方法寻找摄像机参数。

它将把我们计算出的 `(threedpoints, twodpoints, grayColor.shape[::-1], None, None)` 作为参数，并返回一个列表，该列表包含的元素有 `camera matrix`、`distortion coefficients`、`rotation vectors` 和 `translation vectors`。

`camera matrix` 帮助将 3D 对象点转换为 2D 图像点，`distortion coefficients` 返回相机在世界上的位置，值为 `rotation` 和 `translation` 矢量。
