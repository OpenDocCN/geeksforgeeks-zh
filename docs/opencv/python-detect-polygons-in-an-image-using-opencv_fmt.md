...
> 原文: [https://www.geeksforgeeks.org/python-detect-polygons-in-an-image-using-opencv/](https://www.geeksforgeeks.org/python-detect-polygons-in-an-image-using-opencv/)

## 方法
我们将用于检测给定多边形形状的方法将基于根据其具有的边数对检测到的形状进行分类。例如，如果检测到的多项式有 3 条边，那么它可以被认为是三角形，如果多项式有 4 条边，那么它可以被分类为正方形或矩形。
...
## 实现
在下面的代码中，我们将从图像 `'arrow.jpg'` 中检测一个箭头形状的物体。形状将根据它的边数来检测。
