...
> 原文: [`https://www.geeksforgeeks.org/python-opencv-settrackbarpos-function/`](https://www.geeksforgeeks.org/python-opencv-settrackbarpos-function/)

`setTrackbarPos()` 函数用于设置指定窗口中指定跟踪条的位置。它不返回任何东西。`setTrackbarPos()` 接受三个参数：第一个是跟踪条名称，第二个是窗口名称（它是跟踪条的父窗口），第三个是要设置到跟踪条的位置的新值。它返回 `None`。
