...
> 原文: [https://www.geeksforgeeks.org/white-and-black-dot-detection-using-opencv-python/](https://www.geeksforgeeks.org/white-and-black-dot-detection-using-opencv-python/)
...
首先我们需要导入 OpenCV 库。所有关于图像处理的功能都存在于这个库中。为了存储图像的路径，我们将在变量 `path` 中处理。
...
轮廓可以简单地解释为连接所有连续点（沿边界）的曲线，具有相同的颜色或强度。轮廓是用于形状分析和物体检测和识别的有用工具。轮廓为使用二值图像提供了更好的准确性。`cv2.findContours()` 函数中有三个自变量，第一个是源图像，第二个是轮廓检索方式，第三个是轮廓逼近法。它输出轮廓和层次。`cnts` 是图像中所有轮廓的 Python 列表。每个单独的轮廓是对象边界点的 (x, y) 坐标的 Numpy 数组。
...
现在为了计算白点，我们需要稍微改变一下阈值。我们必须使用 `cv2.THRESH_BINARY` 而不是 `cv2.THRESH_BINARY_INV`，因为我们在黑色表面上计算白色值。其他过程都是一样的。我们可以更改 `s1` 和 `s2` 的值来检查最佳结果。
...
[https://docs.opencv.org/3.1.0/d4/d73/tutorial_py_contours_begin.html](https://docs.opencv.org/3.1.0/d4/d73/tutorial_py_contours_begin.html)