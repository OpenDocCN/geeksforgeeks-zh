...
> 原文:[https://www.geeksforgeeks.org/python-opencv-destroyallwindows-function/](https://www.geeksforgeeks.org/python-opencv-destroyallwindows-function/)

`Python Opencv destroyAllWindows()`功能允许用户随时破坏所有窗口。它不接受任何参数，也不返回任何内容。它类似于`destroyWindow()`。`destroyWindow()`只销毁一个特定的窗口，但在`destroyAllWindows()`的情况下，它会销毁所有窗口。
