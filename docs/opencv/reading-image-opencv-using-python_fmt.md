...
> 原文：[https://www.geeksforgeeks.org/reading-image-opencv-using-python/](https://www.geeksforgeeks.org/reading-image-opencv-using-python/)
...
> **参数：**
> **`path`：** 代表待读图像路径的字符串。
> **`flag`：** 它指定了应该以何种方式读取图像。它的默认值是 `cv2.IMREAD_COLOR`。
>
> **返回值：** 该方法返回从指定文件加载的图像。
...
> **`cv2.IMREAD_GRAYSCALE`：** 指定以灰度模式加载图像。或者，我们可以为该标志传递整数值 `0`。
