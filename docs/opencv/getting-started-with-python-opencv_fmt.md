...
> 原文：[https://www.geeksforgeeks.org/getting-started-with-python-opencv/](https://www.geeksforgeeks.org/getting-started-with-python-opencv/)
...
要安装 OpenCV，必须在系统上预装 Python 和 PIP。如果 Python 不存在，请通过[如何在 Linux 上安装 Python？](https://www.geeksforgeeks.org/how-to-install-python-on-linux/)并按照提供的说明操作。如果 PIP 不存在，通过[如何在 Linux 上安装 PIP？](https://www.geeksforgeeks.org/how-to-install-pip-in-linux/)并按照提供的说明操作。
...
就像除此之外，我们还可以减去两幅图像中的像素值，并借助 `cv2.subtract()` 进行合并。图像的大小和深度应该相等。
...
平移指的是物体的直线移动，即图像从一个位置移动到另一个位置。如果我们知道水平和垂直方向的位移量，比如(`tx`，`ty`)，那么我们就可以做一个变换矩阵。现在，我们可以使用 `cv2.warpAffine()` 函数来实现翻译。这个函数需要一个 2×3 的数组。`numpy` 数组应该是浮点型的。
...
*   `cv2.THRESH_TRUNC`: 如果像素强度值大于阈值，则截断为阈值。像素值被设置为与阈值相同。所有其他值保持不变。
...
在[大津阈值](https://www.geeksforgeeks.org/python-thresholding-techniques-using-opencv-set-3-otsu-thresholding/)中，阈值的值不是选择的，而是自动确定的。考虑双峰图像(两个不同的图像值)。生成的直方图包含两个峰值。因此，一般条件是选择一个位于两个直方图峰值中间的阈值。我们使用传统的`cv2.threshold`函数和`cv2.THRESH_OTSU`作为额外的标志。
...
*   `sigmaSpace`: 坐标空间中![\sigma](img/40ed8b248d47e8faf99eceed78e15a81.png "Rendered by QuickLaTeX.com")的值。它的值越大，混合在一起的像素就越多，因为它们的颜色在`sigmaColor`范围内。
...
[轮廓](https://www.geeksforgeeks.org/find-and-draw-contours-using-opencv-python/)被定义为连接图像边界上具有相同强度的所有点的线。轮廓在形状分析、寻找感兴趣对象的大小和对象检测中很方便。OpenCV 具有`findContours()`功能，有助于从图像中提取轮廓。它最适合二值图像，所以我们应该首先应用阈值技术，索贝尔边缘等。
...
为了演示上述功能的使用，我们需要一个尺寸为 400 X 400 的图像，用纯色填充(本例中为黑色)。为了做到这一点，我们可以利用 `numpy.zeros` 函数来创建所需的图像。
