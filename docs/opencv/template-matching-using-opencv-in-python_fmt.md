...
> 原文：[https://www.geeksforgeeks.org/template-matching-using-opencv-in-python/](https://www.geeksforgeeks.org/template-matching-using-opencv-in-python/)

**模板匹配**是一种寻找图像中与面片（模板）相似的区域的技术。斑块是具有一定特征的小图像。模板匹配的目标是在图像中找到补丁/模板。要找到它，用户必须给出两个输入图像：**源图像**——要在其中找到模板的图像和**模板图像（`T`）**——要在源图像中找到的图像。
