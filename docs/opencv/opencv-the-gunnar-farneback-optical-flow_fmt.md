...
> 原文: [https://www.geeksforgeeks.org/opencv-the-gunnar-farneback-optical-flow/](https://www.geeksforgeeks.org/opencv-the-gunnar-farneback-optical-flow/)
...
既然我们只有一个方程来求两个未知数，我们就用不同的方法来求解：
...
*   `next`: 与 `prev` 相同类型和大小的第二个输入图像。
*   `pyr_scale`: 参数，指定为每个图像构建金字塔的图像比例(比例< 1)。一个经典的金字塔一般是 0.5 的比例，每增加一层，它就会减少一半。
*   `levels`: `levels=1` 表示，没有多余的图层(只有初始图像)。它是包含第一个图像的金字塔层数。
*   `winsize`: 是平均窗口大小，大小越大，算法对噪声的鲁棒性越强，提供快速的运动检测，虽然给出的是模糊的运动场。
*   `iterations`: 在每个金字塔级别要执行的迭代次数。
*   `poly_n`: 一般是 5 或者 7，是像素邻域的大小，用来寻找像素之间的多项式展开。
*   `poly_sigma`: 高斯的标准差，即导数要平滑作为多项式展开的基础。`poly_n=5` 可以是 1.1，`poly_n=7` 可以是 1.5。
*   `flow`: 计算出的流量图像，其大小与 `prev` 相似，类型为 `CV_32FC2`。
*   `flags`: 可以是：
    *   `OPTFLOW_USE_INITIAL_FLOW`： 使用输入流作为初始近似的组合。
    *   `OPTFLOW_FARNEBACK_GAUSSIAN`： 使用高斯 `winsize` x `winsize` 滤镜。
