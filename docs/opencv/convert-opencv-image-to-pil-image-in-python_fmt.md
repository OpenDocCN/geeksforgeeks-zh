...
> 原文：[`https://www.geeksforgeeks.org/convert-opencv-image-to-pil-image-in-python/`](https://www.geeksforgeeks.org/convert-opencv-image-to-pil-image-in-python/)
...
1.  `path`：表示要读取的图像的路径的字符串。
2.  `flag`：指定应该如何读取图像。它的默认值是 `cv2.IMREAD_COLOR`。
...
4.  `dstCn`：是目的图像中的通道数。如果参数为 0，则通道数自动从 `src` 和 `code` 中导出。这是一个可选参数。
