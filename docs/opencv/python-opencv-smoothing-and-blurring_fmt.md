...
> 原文: [https://www.geeksforgeeks.org/python-opencv-smoothing-and-blurring/](https://www.geeksforgeeks.org/python-opencv-smoothing-and-blurring/)
...
> **语法:** `cv2.blur(image, shapeOfTheKernel)`

*   `image` – 需要平滑的图像
*   `kernel shape` – 矩阵状 3 乘 3 / 5 乘 5 的形状
...
> **语法:** `cv2.GaussianBlur(image, shapeOfTheKernel, sigmaX)`
...
> **语法:** `cv2.medianBlur(image, kernelSize)`

*   `image` – 我们需要应用平滑的图像
*   `kernelSize` – 内核的大小，因为它总是取平方矩阵，该值必须是大于 2 的正整数。
...
> **语法:** `cv2.bilateralFilter(image, diameter, sigmaColor, sigmaSpace)`
