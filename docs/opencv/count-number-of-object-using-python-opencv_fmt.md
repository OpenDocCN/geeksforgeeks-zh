...
> 原文：[https://www.geeksforgeeks.org/count-number-of-object-using-python-opencv/](https://www.geeksforgeeks.org/count-number-of-object-using-python-opencv/)
...
我们将使用 `cv2.imread(image-name)` 命令读取图像，然后使用 `cv2.cvtColor(image-name, cv2.COLOR_BGR2GRAY)` 命令将该图像转换为灰度图像。
...
为了计数，我们必须检测边缘，但是在检测边缘之前，我们必须使图像模糊以避免噪声。使用 `cv2.GaussianBlur(image-name, kernel-size, standard-deviation)`。
...
现在我们将使用 canny 算法检测边缘，`cv2.canny()` 函数中的第二个和第三个参数是阈值。30 和 150 之间值被认为是该图像的边缘。
...
我们可以看到边没有连接。我们需要连接边缘，必须让更多的 thiker 和可见。
...
现在我们要计算图像中的轮廓，然后把图像从 BGR 转换成 RGB，然后画出轮廓。
