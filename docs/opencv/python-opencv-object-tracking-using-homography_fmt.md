...
> 原文：[https://www.geeksforgeeks.org/python-opencv-object-tracking-using-homography/](https://www.geeksforgeeks.org/python-opencv-object-tracking-using-homography/)
...
特征匹配是指根据搜索距离从两个相似的数据集中找到对应的特征。现在将使用`SIFT`算法和`FLANN`类型特征匹配。
...
要检测物体的单应性，我们必须获得矩阵，并使用函数`cv2.findHomography()`获得物体的单应性。
