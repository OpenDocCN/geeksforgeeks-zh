...
> 原文：[https://www.geeksforgeeks.org/contour-detection-with-custom-seeds-using-python-opencv/](https://www.geeksforgeeks.org/contour-detection-with-custom-seeds-using-python-opencv/)

本文讨论了如何通过单击图像来动态检测轮廓，并为图像的不同部分分配不同的颜色。轮廓是形状分析和目标检测与识别的非常有用的工具。该程序使用 `OpenCV`、`Numpy` 和 `Matplotlib` 库。它还使用内置的 `OpenCV` 分水岭算法来检测轮廓。
