...
> 原文: [https://www.geeksforgeeks.org/multiple-color-detection-in-real-time-using-python-opencv/](https://www.geeksforgeeks.org/multiple-color-detection-in-real-time-using-python-opencv/)
...
这是一个使用 Python 编程语言实时检测多种颜色(这里只考虑了红色和蓝色颜色)的实现。
...
1.  **Input:** 通过网络摄像头捕获视频。
2.  以图像帧的形式读取视频流。
3.  将图像帧从 BGR（RGB 颜色空间表示为三个整数值从 0 到 255 的红、绿、蓝矩阵）转换为 HSV（色调-饱和度-明度）颜色空间。*色调* 描述颜色，*饱和度* 表示该颜色中的灰色量，*明度* 描述颜色的亮度或强度。这可以分别表示为范围在 0-179、0-255 和 0-255 的三个矩阵。
4.  定义每种颜色的范围并创建相应的掩膜。
5.  形态学变换：膨胀，以去除图像中的噪声。
6.  在图像帧和掩膜之间执行 `bitwise_and` 操作，以专门检测该特定颜色并丢弃其他颜色。
7.  为各个颜色创建轮廓，以区分显示检测到的彩色区域。
8.  **Output:** 实时检测颜色。

![img](img/9de8ad5d8e8ee2add3556f3c93595d91.png)
