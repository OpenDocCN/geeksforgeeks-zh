# 使用 Python–OpenCV 更改图像的对比度和亮度

> 原文：[https://www.geeksforgeeks.org/changing-the-contrast-and-brightness-of-an-image-using-python-opencv/](https://www.geeksforgeeks.org/changing-the-contrast-and-brightness-of-an-image-using-python-opencv/)
...
> **语法：** `imread(filename)`：`filename`（图像文件的名称）。
> `namedWindow(winname)`：`winname`（窗口的名称）。
...
### 第二步
通过调用控制器功能，返回编辑后的图像，之后 `imshow()` 功能将显示受影响的图像。
...
### 第三步
控制器功能会根据轨迹球位置控制图像的亮度和对比度，并返回编辑后的图像。

> **语法：** `addWeighted(src1, alpha, src2, beta, gamma)`
> **参数：**
> `src1`：第一个输入数组。
> `alpha`：第一个阵元素的权重。
...
> `color`：文字颜色。
> `thickness`：用于绘制文字的线条粗细。
> `lineType`：线型。请参见 `#LineTypes`。
> `bottomLeftOrigin`：为真时，图像数据原点在左下角。否则，它在左上角。
