...
## 使用 OpenCV 对数据进行预处理
在输入模型之前对图像进行预处理会给出非常准确的结果，因为它有助于提取图像的复杂特征。OpenCV 有一些内置的功能，比如 `cvtColor()` 和 `equalizeHist()` 来完成这个任务。按照以下步骤完成此任务–

*   首先，使用 `cvtColor()` 函数将图像转换为灰度图像，以减少计算量。
*   `equalizeHist()` 功能通过将像素与其附近的像素归一化来均衡像素的强度，从而增加图像的对比度。
...
## 建立模型
由于数据集中有 43 类图像，我们将 `num_classes` 设置为 43。该模型包含两个 `Conv2D` 层，后跟一个 `MaxPooling2D` 层。为了有效提取要素，这要做两次，然后是密集层。添加了 0.5 的丢失层，以避免数据过拟合。
