# Express.js 中的 Handlebars 模板

> 原文：[https://www.geeksforgeeks.org/handlebars-templating-in-expressjs/](https://www.geeksforgeeks.org/handlebars-templating-in-expressjs/)
...
要在 Express 中使用 Handlebars，我们需要将 HTML 代码存储到源目录的 `views` 文件夹中，并使用 `.hbs` 扩展名，因为 `hbs` 会在 `views` 文件夹中查找页面。

## 基本设置

我们在 `index.js` 文件中需要做的第一件事就是引入 `hbs` 模块。
...
在这里，我们将 `demo` 对象作为 `demo` 发送到我们的 `hbs` 页面。我们可以检索视图文件夹中的 `dynamic.hbs` 中的信息。
