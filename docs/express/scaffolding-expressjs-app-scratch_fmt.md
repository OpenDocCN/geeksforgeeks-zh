...
> 原文：[https://www.geeksforgeeks.org/scaffolding-expressjs-app-scratch/](https://www.geeksforgeeks.org/scaffolding-expressjs-app-scratch/)
...
使用 `express-generator` 包安装 `express` 命令行工具。`express-generator` 用于创建应用程序的结构。
...
`bin` 里面的文件叫 `www`，是我们 `app` 的主要配置文件。

### `public`

`public` 文件夹包含要公开使用的文件，如 `JavaScript` 文件、`CSS` 文件、图像等。

### `routes`

`routes` 文件夹包含文件，这些文件包含帮助导航到地图不同区域的方法。它包含各种 `js` 文件。

### `views`

`views` 文件夹包含构成应用程序视图部分的各种文件。
**示例：** 主页、注册页面等。

**注：** 撰写本文时文件的扩展名为 `.jade`。将这些文件扩展名更改为 `.pug`，因为 `jade` 项目已经改成了 `pug`。
...
`app.js` 文件是主文件，是所有其他文件的头部。这里安装的各种软件包必须是 `require`。除此之外，它还有许多其他用途，如处理路由器、中间件等。

### `package.json`

[`package.json`](https://www.geeksforgeeks.org/node-js-package-json/) 文件是任意 `Node.js` 项目和 `express.js` 应用的清单文件。
包含项目的元数据，如 `app` 中使用的包及其版本(称为依赖项)、`start` 和 `test`(从终端作为 `npm start` 运行)等各种脚本、`app` 的名称、`app` 的描述、`app` 的版本等。

## 运行脚手架应用程序

使用以下命令安装运行该应用程序所需的 [`package.json`](https://www.geeksforgeeks.org/node-js-package-json/) 文件中提到的所有依赖项：
