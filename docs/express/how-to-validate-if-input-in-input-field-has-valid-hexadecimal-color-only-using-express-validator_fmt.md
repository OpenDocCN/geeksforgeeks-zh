...
> 原文: [https://www.geeksforgeeks.org/how-to-validate-if-input-in-input-field-has-valid-hexadecimal-color-only-using-express-validator/](https://www.geeksforgeeks.org/how-to-validate-if-input-in-input-field-has-valid-hexadecimal-color-only-using-express-validator/)
...
*   通过验证输入验证输入: `check('inputFieldName')` 和带有“.”的验证链。
...
此示例说明了如何验证输入字段，以仅允许有效的十六进制颜色代码。
...
该文件包含所有验证逻辑 (验证输入字段的逻辑，只允许有效的十六进制颜色代码)。
