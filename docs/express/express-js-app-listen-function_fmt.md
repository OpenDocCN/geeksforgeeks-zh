...
> 原文: [https://www.geeksforgeeks.org/express-js-app-listen-function/](https://www.geeksforgeeks.org/express-js-app-listen-function/)

`app.listen()`功能用于绑定和监听指定主机和端口上的连接。这个方法和 Node 的 `http.Server.listen()`方法是一样的。
...
这就是如何使用 `express` 的 `app.listen()` 函数绑定并监听指定主机和端口上的连接。