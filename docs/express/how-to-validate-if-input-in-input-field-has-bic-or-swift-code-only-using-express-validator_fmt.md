...
> 原文: [https://www.geeksforgeeks.org/how-to-validate-if-input-in-input-field-has-bic-or-swift-code-only-using-express-validator/](https://www.geeksforgeeks.org/how-to-validate-if-input-in-input-field-has-bic-or-swift-code-only-using-express-validator/)
...
*   通过 `validateInputField` 验证输入: `check(输入字段名)` 和带有“.”的验证 `isBIC()` 上的链。
...
该文件包含所有的验证逻辑(验证输入字段只接受有效 BIC 的逻辑)。
