...
中间件可以从一个链接到另一个，因此创建了按顺序执行的功能链。最后一个函数将响应发送回浏览器。因此，在将响应发送回浏览器之前，不同的中间件会处理请求。
...
转到您的项目目录，输入以下命令创建 NodeJs 项目。确保 NodeJs 安装在您的机器上。
...
在 `package.json` 文件的 `scripts` 部分，添加以下代码行。
...
在目录中创建一个 `index.js` 文件。确保它不在您正在使用的目录的任何子目录中。
