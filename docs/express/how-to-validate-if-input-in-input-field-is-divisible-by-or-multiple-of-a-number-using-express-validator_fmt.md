...
> 原文: [https://www.geeksforgeeks.org/how-to-validate-if-input-in-input-field-is-divisible-by-or-multiple-of-a-number-using-express-validator/](https://www.geeksforgeeks.org/how-to-validate-if-input-in-input-field-is-divisible-by-or-multiple-of-a-number-using-express-validator/)
...
*   通过 `validateInputField` 验证输入: `check(输入字段名)` 和带有“.”的验证链。
...
此示例说明了如何验证输入字段，以仅允许可被某个特定数字整除的数字。
...
**文件名–`validator.js`:** 该文件包含所有验证逻辑(验证输入字段只允许大写字母的逻辑)。
