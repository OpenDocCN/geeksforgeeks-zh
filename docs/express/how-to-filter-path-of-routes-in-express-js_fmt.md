# 如何在 Express.js 中过滤路径？

> 原文: [https://www.geeksforgeeks.org/how-to-filter-path-of-routes-in-express-js/](https://www.geeksforgeeks.org/how-to-filter-path-of-routes-in-express-js/)
...
*   `path`: 是调用中间件函数的路径。它可以是表示路径或路径模式的字符串，也可以是匹配路径的正则表达式模式。
*   `callback`: 是一个中间件函数或一系列/数组中间件函数。
...
**注意:** `Home.js` 和 `login.js` 文件存在于 `routes` 文件夹中。
