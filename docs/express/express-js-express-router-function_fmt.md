...
`express.Router()`功能用于创建新的路由器对象。当您想在程序中创建一个新的路由器对象来处理请求时，可以使用这个函数。
...
*   `mergeParams`: 它保留来自父路由器的`req.params`值。
*   `strict`: 这样可以实现严格的路由。
...
现在向`http://localhost:3000/user`、`http://localhost:3000/admin`和`http://localhost:3000/student`发出`GET`请求，然后您将在控制台上看到以下输出:
...
[官方文件](https://expressjs.com/en/4x/api.html#express.router)