...
> 原文：[https://www.geeksforgeeks.org/how-to-validate-if-input-in-input-field-must-contains-a-seed-word-using-express-validator/](https://www.geeksforgeeks.org/how-to-validate-if-input-in-input-field-must-contains-a-seed-word-using-express-validator/)
...
*   通过验证输入验证输入：`check(输入字段名)` 和验证链是否包含带“.”的 `contains(种子)`。
*   如果要忽略区分大小写，请添加 `options` 对象参数 `{ignoreCase:true}`。默认情况下，`ignoreCase` 设置为 `false`。
*   在路由中使用验证名称（`validatePlayerName`）作为一个中间件，作为一个验证数组。
...
该文件包含所有验证逻辑（验证输入字段的逻辑，只允许包含种子字的值）。
