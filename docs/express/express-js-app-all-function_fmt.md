...
`app.all()` 功能用于路由所有类型的 HTTP 请求。例如，如果我们有 POST、GET、PUT、DELETE 等，对任何特定路由的请求，比如说`/user`，那么我们可以定义单个 API `app.all('/user')`，它将接受所有类型的 HTTP 请求，而不是定义不同的 API，比如 `app.post("/user")`、`app.get("/user")`等。
...
1.  `path`: 是调用中间件函数的路径。
2.  `callback`: 可以是中间件功能，也可以是一系列/数组功能。
