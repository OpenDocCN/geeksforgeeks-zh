...
`req.ip` 属性包含请求的远程 IP 地址。当用户想要获取向应用程序发出的传入请求的 IP 地址时，它非常有用。
...
现在打开浏览器，向 `http://localhost:3000/` 发出 GET 请求，头部设置为 `x-forwarded-for:203.0.113.195`，现在您可以在控制台上看到以下输出：
...
[https://expressjs.com/en/4x/api.html#req.ip](https://expressjs.com/en/4x/api.html#req.ip)