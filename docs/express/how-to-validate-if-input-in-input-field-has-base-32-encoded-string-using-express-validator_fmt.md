...
> 原文：[https://www.geeksforgeeks.org/how-to-validate-if-input-in-input-field-has-base-32-encoded-string-using-express-validator/](https://www.geeksforgeeks.org/how-to-validate-if-input-in-input-field-has-base-32-encoded-string-using-express-validator/)
...
*   Validate the input by `validateInputField`: `check('enter the field name')` and chain on the validation `.isBase32()`.
...
**文件名–form.js:** 该文件包含显示 HTML 表单的逻辑。
...
**文件名–validator.js:** 该文件包含所有的验证逻辑(验证输入字段只接受基本编码 32 字符串的逻辑)。
