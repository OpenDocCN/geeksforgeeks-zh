...
> 原文：[https://www.geeksforgeeks.org/how-to-implement-jwt-authentication-in-express-js-app/](https://www.geeksforgeeks.org/how-to-implement-jwt-authentication-in-express-js-app/)
...
JWT 非常流行通过 HTTP 进行认证和授权。这些令牌可用作授予访问服务器资源权限的凭据。

## 为什么认证中需要 JWT

由于 HTTP 请求是无状态的，知道当前请求与过去的请求相关是一项具有挑战性的任务。例如，登录用户拥有更改其数据的某些权限，但在下一次请求时，服务器将如何识别是同一用户较早请求登录。为了解决这个挑战，我们使用 JWT。
...
*   在登录路径中，我们从请求正文中提取了 `email` 和 `password`，然后在数据库中搜索用户。如果找到，我们将继续检查提供的密码是否与实际密码匹配。
*   在注册路线中，我们提取了 `name`、`email` 和 `password` 等详细信息，使用 `mongoose` 提供的 `save()` 方法在数据库中注册用户。

最后，我们通过提供有效载荷作为 `userId` 和 `email` 创建了一个 1 小时到期的**令牌**，因为只有这样才足以提取用户信息。`sign()` 方法接受有效负载、秘密 jwt 密钥和到期时间，然后生成一个令牌。
...
*   令牌通过请求头发送，我们在这里从 `authorization` 头提取令牌，我们使用 `split()` 函数，因为令牌仍然是“`Bearer TOKEN`”的形式，我们只想提取令牌，这就是为什么提供索引 `1`。
*   `verify()` 方法接受令牌和 jwt 密钥，并提供令牌的解码。在此之后，我们可以获得用户的信息。
...
至此我们已经成功实现了从创建到解码的令牌。这个“`accessResource`”路由的例子足以说明工作原理，您可以在内部以您想要的方式使用您的令牌，并且可以相应地允许用户使用。这意味着我们能够成功地保存无状态 HTTP 请求的信息。