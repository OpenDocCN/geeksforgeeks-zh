...
`Express.js`是[`node.js`](https://www.geeksforgeeks.org/working-of-express-js-middleware-and-its-benefits/)最强大的框架。`Express.js`是一个路由和中间件框架，用于处理网页的不同路由，它在请求和响应周期之间工作。`Express.js`使用不同种类的中间件功能以便完成发出的不同请求客户端对于例如客户端可以发出`get`、`put`、`post`、和`delete`请求这些请求可以通过这些中间件功能轻松处理
