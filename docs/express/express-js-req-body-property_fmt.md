...
`req.body` 属性包含在请求正文中提交的数据的键值对。默认情况下，它是未定义的，当您使用名为 body-parsing 的中间件（如 `express.urlencoded()` 或 `express.json()`）时才会被填充。
...
4.  现在向 `http://localhost:3000/profile` 发出 POST 请求，请求正文如下:
...
现在向 `http://localhost:3000/signup` 发出 POST 请求，请求正文如下:
