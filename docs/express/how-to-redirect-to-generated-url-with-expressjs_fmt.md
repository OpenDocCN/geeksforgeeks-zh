...
> 原文: [https://www.geeksforgeeks.org/how-to-redirect-to-generated-url-with-expressjs/](https://www.geeksforgeeks.org/how-to-redirect-to-generated-url-with-expressjs/)
...
> **客户端:** 在 `/` 的网址上进行 **GET 请求** 登录页面。
> **服务器:** 渲染登录页面
> **客户端:** 接下来，用户填写表单数据，并在网址 `/login` 上发出 **POST 请求**。
> **服务器:** 如果用户数据匹配，则重定向至 `/dashboard/[USER EMAIL]`。
> **客户端:** 用户在 `/dashboard/[USER EMAIL]` 上发出 **GET 请求**。
> **服务器:** 渲染仪表盘页面。
...
客户端文件，EJS 的默认行为是查看 `views` 文件夹中的模板进行渲染。因此，让我们在主节点项目文件夹中创建一个 `views` 文件夹，并创建两个名为 `login.ejs` 和 `dashboard.ejs` 的文件。

`login.ejs` 负责用户登录请求，如果登录成功，用户将重定向到 `dashboard.ejs`。
...
在根文件夹上创建一个文件名 `server.js`。这个文件是有一些中间件的，它响应用户的请求。一般来说，用户登录信息是从数据库中获取的，但对我们来说，从一个大数据库中获取信息是公平的。

在这里，如果用户数据匹配，则登录处理程序路由被重定向到 `/dashboard/[USER EMAIL]`，否则返回 HTTP 状态码 401。
