...
> 原文：[https://www.geeksforgeeks.org/change-data-type-of-given-numpy-array/](https://www.geeksforgeeks.org/change-data-type-of-given-numpy-array/)

在这篇文章中，我们将看到改变给定 numpy 数组的数据类型的方法。为了改变给定数组对象的数据类型，我们将使用 `numpy.astype()` 函数。该函数接受一个作为目标数据类型的参数。该函数支持所有数据的泛型类型和内置类型。

## 问题 1

给定一个 numpy 数组，其底层数据为 `'int32'` 类型。将给定对象的数据类型更改为 `'float64'`。
...
给定一个 numpy 数组，其底层数据为 `'int32'` 类型。将给定对象的数据类型更改为 `'complex128'`。
