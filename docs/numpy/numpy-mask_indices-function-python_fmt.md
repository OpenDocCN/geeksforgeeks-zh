...
> 原文: [https://www.geeksforgeeks.org/numpy-mask_indices-function-python/](https://www.geeksforgeeks.org/numpy-mask_indices-function-python/)

`numpy.mask_indices()` 函数返回索引以访问 (n, n) 数组，给定一个屏蔽函数。
...
## 返回值
【数组元组】对应于 `mask_func(np.ones((n, n)), k)` 为真的位置的 n 个数组索引。
