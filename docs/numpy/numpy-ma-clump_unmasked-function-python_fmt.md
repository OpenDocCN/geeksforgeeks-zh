# numpy.ma.clump_unmasked()函数 | Python

> 原文: [https://www.geeksforgeeks.org/numpy-ma-clump_unmasked-function-python/](https://www.geeksforgeeks.org/numpy-ma-clump_unmasked-function-python/)

`numpy.ma.clump_unmasked()` 函数返回与一维数组中未屏蔽的块相对应的切片列表。

## 语法
`numpy.ma.clump_unmasked(arr)`

## 参数
- `arr`: 【`ndarray`】一维蒙面阵。

## 返回
【切片列表】切片列表，`a` 中每个未屏蔽元素的连续区域一个。
