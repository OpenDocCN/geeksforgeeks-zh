...
记录数组允许使用 `arr.a` 和 `arr.b` 作为数组成员访问字段。`numpy.recarray.clip()` 函数返回一个值被限制在 `[min, max]` 的数组。必须给出最大值或最小值之一。
...
- **min**: 最小值。
  - 如果为 `None`，则不在下间隔边缘进行剪裁。最小值和最大值中最多只能有一个为 `None`。
- **max**: 最大值。
  - 如果为 `None`，则不在上区间边缘进行裁剪。最小值和最大值中最多只能有一个为 `None`。
  - 如果 `a_min` 或 `a_max` 是 `array_like`，那么这三个数组将被广播以匹配它们的形状。
- **out**: 结果会放在这个数组中。它可能是就地剪辑的输入数组。`out` 的形状必须正确，才能容纳输出。它的类型得以保留。

## 返回值

**【clipped_array, ndarray】** 小于最小值的值用最小值替换，大于最大值的值用最大值替换的数组。
