...
> 原文：[https://www.geeksforgeeks.org/numpy-gradient-descent-optimizer-of-neural-networks/](https://www.geeksforgeeks.org/numpy-gradient-descent-optimizer-of-neural-networks/)
...
*   `Gradient` 是一个函数，或者可以是一个 Python 可调用对象，它使用一个向量 `&` 来返回我们试图最小化的函数的梯度。
*   `Start` 是我们分配给函数的任意起始点。它是一个独立的变量。它也可以是一个列表或多变量的 Numpy 数组。
*   `learn_rate` 控制向量更新的幅度。
*   `n_iter` 是操作应该运行的迭代次数。
*   `Tol` 是容差水平，指定了每次迭代中的最小移动量。
