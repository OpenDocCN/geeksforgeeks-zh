# Numpy recarray.argsort()函数

> 哎哎哎:# t0]https://www . geeksforgeeks . org/num py-reckler-argsort-function-python/

在numpy中，数组可能具有包含字段的数据类型，类似于电子表格中的列。一个例子是`[(a, int), (b, float)]`，数组中的每个条目都是一对(int，float)。通常，这些属性是使用字典查找来访问的，例如`arr['a']`和`arr['b']`。

记录数组允许使用`arr.a`和`arr.b`作为数组成员访问字段。`numpy.recarray.argsort()`函数返回对该数组进行排序的索引。

## 语法

`numpy.recarray.argsort(arr, axis=-1, kind='quicksort', order=None)`

## 参数

- **arr :** 输入记录数组。
- **axis:**【int或None】要排序的轴。如果为`None`，则数组在排序前被展平。默认值为-1，沿最后一个轴排序。
- **kind:** ['quicksort'，'mergesort'，'heapsort']选择算法。默认值为'quicksort'。
- **order:**【字符串或字符串列表】当`arr`是定义了字段的数组时，此参数指定首先比较哪些字段，其次比较哪些字段等。

## 返回

【index_Array，ndarray】沿指定轴排序的索引数组。
