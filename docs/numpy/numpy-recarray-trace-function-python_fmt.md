...
记录数组允许使用`arr.a`和`arr.b`作为数组成员访问字段。`numpy.recarray.trace()`函数用于返回沿数组对角线的和。
...
**参数:**
- `offset`:【int，可选】对角线与主对角线的偏移。可以是积极的也可以是消极的。默认为 0。
- `axis1`:【int，可选】用作二维子阵列的第一轴和第二轴的轴，对角线应取自该轴。
- `axis2`:【int，可选】用作二维子阵列的第一轴和第二轴的轴，对角线应取自该轴。
- `dtype`:【可选】确定返回数组的数据类型。
- `out`:【n 数组，可选】存储结果的位置。
    - 如果提供，它必须具有输入广播到的形状。
    - 如果未提供或无，则返回新分配的阵列。
