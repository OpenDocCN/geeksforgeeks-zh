...
记录数组允许使用`arr.a`和`arr.b`作为数组成员访问字段。`numpy.recarray.all()`如果记录数组中的所有元素都评估为真，则函数返回真。
