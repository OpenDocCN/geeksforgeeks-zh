...
> 原文：`https://www.geeksforgeeks.org/numpy-around-python/`

`numpy.around(arr, decimals=0, out=None)`：这个数学函数帮助用户将数组元素均匀地舍入到给定的小数位数。

## 参数

- `arr`：输入数组。
- `decimals`：要四舍五入的小数位数。默认值为 `0`。如果是负数，它指定小数点左边的位置。
- `out`：可选参数，输出结果数组。

## 返回

返回一个数组，其中所有数组元素都被四舍五入，类型与输入相同。
