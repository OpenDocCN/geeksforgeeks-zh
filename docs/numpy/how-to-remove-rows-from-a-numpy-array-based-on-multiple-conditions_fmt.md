...
> 原文：[https://www.geeksforgeeks.org/how-to-remove-rows-from-a-numpy-array-based-on-multiple-conditions/](https://www.geeksforgeeks.org/how-to-remove-rows-from-a-numpy-array-based-on-multiple-conditions/)

在本文中，我们将学习如何基于多个条件从 NumPy 数组中移除行。为了完成我们的任务，我们需要 NumPy 模块提供的一些内置方法，如下所示：

*   [`np.delete(ndarray, index, axis)`](https://www.geeksforgeeks.org/numpy-delete-python/)：基于给定的索引条件和指定的轴从 NumPy 数组中删除行或列的项目，参数 `ndarray` 是将发生操作的数组，`index` 是基于要删除的条件的特定行，在我们的情况下，`axis=0` 用于删除行。
...
这里 `np.delete(nparray, np.where((nparray >= 5) & (nparray <= 20))[0], axis=0)` 表示它将删除至少有一个或多个元素大于或等于 5 且小于或等于 20 的行。因此，根据给定的条件，第 2、第 3、第 4 和第 5 行都有元素，因此会被删除或移除。
...
这里 `np.delete(nparray, np.where((nparray[:, 0] >= 25) & (nparray[:, 0] <= 35))[0], axis=0)` 表示它将删除其中至少有一个或多个元素的第一个元素大于或等于 25 且小于或等于 35 的行。指向每行第一个元素的 `nparray[:, 0]`。所以，根据给定的条件，第 6、7、8 行有元素，所以它被删除或移除。
...
这里 `np.delete(nparray, np.where((nparray[:, 2] % 2 == 0) | (nparray[:, 4] % 3 == 0) | (nparray[:, 3] % 3 == 0))[0], axis=0)` 表示它将删除至少有一个或多个元素的第三列项可被 2 整除的行，或者删除至少有一个或多个元素的第五列项和第四列项可被 3 整除的行，`axis=0` 用于删除这些行。`nparray[:, 2]`，`nparray[:, 4]`，`nparray[:, 3]` 分别指向每行的第三、第五和第四项。所以，根据给定的条件，第 6、第 7、第 8 行有元素，所以它被删除或移除。
