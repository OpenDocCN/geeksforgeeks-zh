...
> 原文: [https://www.geeksforgeeks.org/calculate-expx-1-for-all-elements-in-a-given-numpy-array/](https://www.geeksforgeeks.org/calculate-expx-1-for-all-elements-in-a-given-numpy-array/)
...
> **参数:**
> `arr :` 输入数组。
> `out :` 存储结果的位置。如果提供，它必须具有输入广播到的形状。如果未提供或 `None`，则返回新分配的数组。形状必须与输入数组相同。
> `where :` 布尔值。真值表示计算该位置的通用函数(`ufunc`)，假值表示将值单独留在输出中。
...
如果输入是数组，则按元素应用函数。`np.exp([1, 2, 3])` 相当于 `[np.exp(1), np.exp(2), np.exp(3)]`。
