# numpy.vsplit()函数 | Python

> 原文: [https://www.geeksforgeeks.org/numpy-vsplit-function-python/](https://www.geeksforgeeks.org/numpy-vsplit-function-python/)

`numpy.vsplit()` 函数将一个数组垂直拆分为多个子数组（逐行）。`vsplit` 相当于 `axis=0` 时的 `split`（默认），数组总是沿着第一个轴拆分，与数组维度无关。

> **语法:** `numpy.vsplit(arr, indices_or_sections)`
> **参数:**
> **arr:** [`ndarray`] 要分成子数组的数组。
> **indices_or_sections:** [`int` 或 1-D 数组] 如果 `indices_or_sections` 为整数 `N`，数组将沿轴分成 `N` 个相等的数组。
> 如果 `indices_or_sections` 是排序整数的一维数组，则条目指示数组沿轴拆分的位置。
> **返回:** [`list` of `ndarray`] 子数组列表。
