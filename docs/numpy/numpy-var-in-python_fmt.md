...
> **`arr`：**【array_like】输入数组。
> **`axis`：**【int 或 int 的元组】轴，我们要沿着该轴计算方差。否则会认为`arr`被展平（在所有轴上工作）。`axis = 0`表示沿列的方差，`axis = 1`表示沿行的方差。
> **`out`：**【ndarray，可选】我们想要放置结果的不同数组。数组必须具有与预期输出相同的维度。
> **`dtype`：**【data-type，可选】计算方差时我们想要的类型。
>
