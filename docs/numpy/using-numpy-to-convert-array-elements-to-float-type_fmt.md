...
> 原文: [https://www.geeksforgeeks.org/using-numpy-to-convert-array-elements-to-float-type/](https://www.geeksforgeeks.org/using-numpy-to-convert-array-elements-to-float-type/)
...
这里，我们可以利用 NumPy 提供的 `astype()` 功能。这个函数用指定的数据类型创建初始数组的另一个副本，在这种情况下是 `float`，然后我们可以将这个副本分配给一个特定的标识符，这个标识符就是 `convertedArray`。注意数据类型是用 NumPy 来指定的，主要是因为 NumPy `astype()` 函数的约束，它只会把 NumPy 类型作为参数。
