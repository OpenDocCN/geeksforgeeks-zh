...
> 原文: [https://www.geeksforgeeks.org/numpy-maskedarray-argsort-function-python/](https://www.geeksforgeeks.org/numpy-maskedarray-argsort-function-python/)
...
>
> **参数:**
> - `axis`: 【无，整数】要排序的轴。如果默认值为“无”，则使用展平数组。
> - `kind`: ['quicksort'，'mergesort'，'heapsort']排序算法。默认值为“快速排序”。
> - `order`: 【列表，可选】当`a`是定义了字段的数组时，此参数指定首先比较哪些字段，其次比较哪些字段等。
> - `endwith`: 【True，False，可选】缺失值(如果有)应被视为最大值(`True`)还是最小值(`False`)。当数组在数据类型的相同极端包含未屏蔽值时，这些值和屏蔽值的顺序是未定义的。
> - `fill_value`: 【var，可选】用于填充屏蔽值的值。如果无，最小填充值的输出(自`._data`)来代替。
>
