...
`numpy.nanprod(arr, axis=None, dtype=None, out=None, keepdims='class numpy。_ 全局。_NoValue ')`

## 参数

- `arr`:【Array _ like】包含需要求和的数字的数组。如果 `arr` 不是数组，则尝试转换。
- `axis`:计算产品的轴。默认情况下，计算展平数组的乘积。
- `dtype`: 返回的数组的类型，以及对元素求和的累加器的类型。默认情况下，使用 `arr` 的*数据类型*。
- `out`:【n 数组，可选】存储结果的位置。
  - 如果提供，它必须具有输入广播到的形状。
  - 如果未提供或无，则返回新分配的阵列。
- `keepdims`:如果设置为真，减少的轴将作为尺寸为 1 的尺寸留在结果中。使用此选项，结果将根据原始 `arr` 正确广播。
