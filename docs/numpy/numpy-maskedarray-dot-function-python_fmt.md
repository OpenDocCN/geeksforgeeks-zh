...
> 原文：[https://www.geeksforgeeks.org/numpy-maskedarray-dot-function-python/](https://www.geeksforgeeks.org/numpy-maskedarray-dot-function-python/)

`numpy.MaskedArray.dot()` 函数用于计算两个掩膜数组的点积。
...
> - `strict`：【bool，可选】屏蔽数据是传播（`True`）还是设置为 0（`False`）进行计算。默认值为 `False`。
>
> **返回：**【ndarray】`arr1` 和 `arr2` 的点积。
