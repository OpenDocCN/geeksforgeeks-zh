...
> 原文: [https://www.geeksforgeeks.org/numpy-matrix-operations-zeros-function/](https://www.geeksforgeeks.org/numpy-matrix-operations-zeros-function/)
...
- `shape`: 【int, int】输出矩阵的行数和列数。如果形状的长度为 1（即，N），或者是标量 N，则 out 成为形状 (1, N) 的单行矩阵。
- `dtype`: 【可选】所需输出数据类型。
- `order`: 在内存中是按行-主（C 风格）还是列-主（Fortran 风格）顺序存储多维数据。
