# numpy.random.random_sample() 函数中的随机采样

> 原文: [https://www.geeksforgeeks.org/random-sampling-in-numpy-random_sample-function/](https://www.geeksforgeeks.org/random-sampling-in-numpy-random_sample-function/)

`numpy.random.random_sample()` 是 numpy 中做随机抽样的功能之一。它返回一个指定形状的数组，并在半开间隔 `[0.0, 1.0)` 内用随机浮动值填充它。
...
**返回:** 间隔 `[0.0, 1.0)` 内的随机浮动数组或单个此类随机浮动值（如果未提供 `size`）。
