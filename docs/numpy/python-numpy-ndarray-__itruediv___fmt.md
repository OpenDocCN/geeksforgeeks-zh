...
借助`ndarray.__itruediv__()`，我们可以除一个特定的值，该值在`ndarray.__itruediv__()`方法中作为参数提供。数值将被分配给numpy数组中的每个元素。

> **语法:** `ndarray.__itruediv__(self, value, /)`
>
> **返回:** 自身 /= 值

## 示例 1

在这个示例中，我们可以看到数组中的每个元素都用方法`ndarray.__itruediv__()`中作为参数给出的值进行了除。此方法对于数组的正、负和浮点值都很有效。
