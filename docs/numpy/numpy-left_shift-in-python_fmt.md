...
通过在 `arr1` 的右侧附加 `arr2` 个 0，这些位被向左移动。由于数字的内部表示是二进制格式，因此此操作相当于将 `arr1` 乘以 `2**arr2`。例如，如果数字是 5，我们想左移 2 位，那么左移 2 位后，结果将是 `5*(2^2) = 20`。
...
- `arr1` : 整数类型的 array_like。
- `arr2` : 整数类型的 array_like。要追加到 `arr1` 的零的个数。`arr2` 的值应为正整数。
...
整型数组。返回 `arr1`，位左移 `arr2` 次。如果 `arr1` 和 `arr2` 都是标量，这就是标量。
