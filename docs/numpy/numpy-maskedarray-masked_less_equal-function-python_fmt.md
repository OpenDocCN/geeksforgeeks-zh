...
> 原文：[https://www.geeksforgeeks.org/numpy-maskedarray-masked_less_equal-function-python/](https://www.geeksforgeeks.org/numpy-maskedarray-masked_less_equal-function-python/)
...
**语法：** `numpy.ma.masked_less_equal(arr, value, copy=True)`

**参数：**
- `arr`：【ndarray】输入我们要屏蔽的数组。
- `value`：【int】用于屏蔽 <= `value` 的数组元素。
- `copy`：【bool】如果为真（默认），在结果中复制一份`arr`。如果为假，就地修改`arr`并返回视图。

**返回：**【MaskedArray】屏蔽后的结果数组。
