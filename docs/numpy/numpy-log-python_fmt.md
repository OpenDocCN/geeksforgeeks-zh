...
`numpy.log()` 是一个数学函数，帮助用户计算 x 的自然对数，其中 x 属于所有输入数组元素。
自然对数是 `exp()` 的倒数，所以 `log(exp(x)) = x`。自然对数以 e 为底数。
...
[https://docs.scipy.org/doc/numpy-dev/reference/generated/numpy.log.html#numpy.log](https://docs.scipy.org/doc/numpy-dev/reference/generated/numpy.log.html#numpy.log)