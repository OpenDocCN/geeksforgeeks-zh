...
> 原文: [https://www.geeksforgeeks.org/python-numpy-np-laggrid3d-method/](https://www.geeksforgeeks.org/python-numpy-np-laggrid3d-method/)
...
## 参数
- `x, y, z`: 【array_like】三维数列在 `x`，`y`，`z` 的笛卡儿积中的点处求值。如果 `x` 或 `y` 或 `z` 是列表或元组，则首先将其转换为数组，否则保持不变，如果不是数组，则将其视为标量。
