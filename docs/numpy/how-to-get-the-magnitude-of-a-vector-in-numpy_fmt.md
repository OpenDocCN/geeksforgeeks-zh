...
> 原文：`https://www.geeksforgeeks.org/how-to-get-the-magnitude-of-a-vector-in-numpy/`
...
通过定义一个显式函数，基于以下数学公式计算给定向量的大小：

如果向量 `V` 满足 `V = (a, b, c)`，那么 `||V|| = √(a*a + b*b + c*c)`。
...
# function defination to compute magnitude of the vector
...
## 2. 使用 NumPy 的 `norm()` 方法

通过使用 `NumPy` 库的 `linalg` 模块中的 `norm()` 方法。`NumPy` 的线性代数模块提供了多种方法来对任何 `NumPy` 数组应用线性代数运算。下面是一些使用 `numpy.linalg.norm()` 计算向量大小的程序：
