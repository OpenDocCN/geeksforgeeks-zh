# Python 中的 `numpy.random.exponential()`

> 原文: [https://www.geeksforgeeks.org/numpy-random-exponential-in-python/](https://www.geeksforgeeks.org/numpy-random-exponential-in-python/)

借助 `numpy.random.exponential()` 方法，我们可以从指数分布中获取随机样本，并利用该方法返回随机样本的 `numpy` 数组。
...
在这个例子中我们可以看到，通过使用 `numpy.random.exponential()` 方法，我们能够得到指数分布的随机样本，并返回 `numpy` 数组的样本。
