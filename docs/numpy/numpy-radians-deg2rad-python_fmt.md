# Python 中的 `numpy.radians()` 和 `deg2rad()`

> 哎哎哎:# t0]https://www . geeksforgeeks . org/num py-radians-deg2rad-python/

`numpy.radians()` 是一个数学函数，帮助用户将角度从度数转换为弧度。

> **语法:** `numpy.radians(x[, out]) = ufunc 'radians')`
> **参数:**
> - `x`: *【array_like】* 元素以度为单位。
> - `out`: *【ndarray, 可选】* 输出数组的形状与 `x` 相同。
>
> 2pi 弧度 = 360 度
>
> **返回:** 用弧度值代替度数值的数组。
...
此数学函数可帮助用户将角度从度数转换为弧度。

> **参数:**
> - `x`: *【array_like】* 元素以弧度为单位。
> - `out`: *【ndarray, 可选】* 输出数组的形状与 `x` 相同。
>
> 2pi 弧度 = 360 度
>
