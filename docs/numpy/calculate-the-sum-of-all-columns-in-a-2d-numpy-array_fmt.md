...
> 原文: [https://www.geeksforgeeks.org/calculate-the-sum-of-all-columns-in-a-2d-numpy-array/](https://www.geeksforgeeks.org/calculate-the-sum-of-all-columns-in-a-2d-numpy-array/)

让我们看看如何计算 2D NumPy 数组中所有列的总和。

## 方法 1
使用嵌套循环按列访问数组元素，然后将它们的和存储在变量中，然后打印出来。
...
## 方法 2
使用 NumPy，`numpy.sum()`函数中的`sum()`函数返回指定轴上数组元素的和。要计算所有列的总和，`sum()`函数中的`axis`参数应为 0。
