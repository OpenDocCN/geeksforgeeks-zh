...
- **`axis`**: 【int 或 None】要沿其排序的轴。如果为 `None`，则数组在排序前被展平。默认值为 `-1`，沿最后一个轴排序。
- **`kind`**: 选择算法。默认值为 `'introselect'`。
- **`order`**: 【string 或 string list】当 `arr` 是定义了字段的数组时，此参数指定首先比较哪些字段，其次比较哪些字段等。

## 返回值

【ndarray】与 `arr` 类型和形状相同的分区数组。
