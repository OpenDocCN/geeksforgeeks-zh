...
记录数组允许使用`arr.a`和`arr.b`作为数组成员访问字段。`numpy.recarray.var()`函数返回数组元素沿给定轴的方差。
...
**参数：**
- `axis`：【int或int的元组】轴，我们要沿着该轴计算方差。否则，它将考虑将arr展平（在所有轴上工作）。`axis = 0`表示沿列的方差，`axis = 1`表示沿行的方差。
- `dtype`：【数据类型，可选】计算方差时我们想要的类型。
- `out`：【ndarray，可选】存储结果的位置。
    - 如果提供，它必须具有输入广播到的形状。
    - 如果未提供或`None`，则返回新分配的数组。
- `ddof`：【int，可选】“Delta自由度”：计算中使用的除数为`N - ddof`，其中`N`代表元素个数。默认情况下，`ddof`为零。
- `keepdims`：【布尔，可选】如果设置为`True`，减少的轴将作为尺寸为1的维度留在结果中。使用此选项，结果将根据输入数组正确广播。
