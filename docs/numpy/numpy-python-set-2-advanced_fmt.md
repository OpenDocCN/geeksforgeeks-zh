...
## 3. Broadcasting
术语 broadcasting 描述了 NumPy 在算术运算期间如何处理具有不同形状的数组。在特定约束下，较小的数组被“广播”到较大的数组上，以便它们具有兼容的形状。
...
**广播规则:** 为了广播，操作中两个数组的尾轴大小必须相同，或者其中一个必须是 **1**。
...
NumPy 具有原生支持日期时间功能的核心数组数据类型。该数据类型称为 `datetime64`，如此命名是因为 `datetime` 已被 Python 中包含的 `datetime` 库占用。
考虑下面的例子：
...
NumPy 的线性代数模块提供了多种方法，可在任何 numpy 数组上应用线性代数。
你可以找到：
*   秩、行列式、迹等。数组的。
*   矩阵的特征值
...
线性回归线的形式是 `w1 * x + w2 = y`，并且它是最小化从每个数据点到直线的距离的平方和的线。因此，给定 `n` 对数据 `(xi, yi)`，我们要寻找的参数是 `w1` 和 `w2`，它们可以将误差降至最低:
