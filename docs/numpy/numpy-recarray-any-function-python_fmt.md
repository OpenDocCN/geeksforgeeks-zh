...
记录数组允许使用`arr.a`和`arr.b`作为数组成员访问字段。`numpy.recarray.any()`函数如果记录数组中的任何元素评估为真，则返回真。
...
- **axis**: 【无或整数或整数元组，可选】执行逻辑或运算的一个或多个轴。
- **out**: 【ndarray，可选】存储结果的位置。
    - 如果提供，它必须具有输入广播到的形状。
    - 如果未提供或无，则返回新分配的数组。
- **keepdims**: 【布尔，可选】如果设置为真，则缩小的轴作为尺寸为 1 的维度留在结果中。使用此选项，结果将根据输入数组正确广播。
    如果传递了默认值，那么`keepdims`不会传递给`ndarray`子类的任何方法，但是任何非默认值都会传递。如果子类`sum`方法不实现`keepdims`，将引发任何异常。

### 返回值

- 【ndarray，bool】如果任何元素的计算结果为真，则返回真。
...
如果我们将`numpy.recarray.any()`应用于整个记录数组，那么它将给出`TypeError`，因为数组是灵活的或混合类型的。
