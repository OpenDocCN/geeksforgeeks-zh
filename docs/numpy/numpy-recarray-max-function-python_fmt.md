...
**参数:**
- `axis`: 【无或 int 或 int 元组，可选】要沿其操作的一个或多个轴。默认情况下，使用展平输入。
- `out`: 【n 数组，可选】存储结果的位置。
    - 如果提供，它必须具有输入广播到的形状。
    - 如果未提供或无，则返回新分配的阵列。
- `keepdims`: 【布尔，可选】如果设置为真，则缩小的轴作为尺寸为 1 的尺寸留在结果中。使用此选项，结果将根据输入数组正确广播。

**返回:** 【数组或标量】记录数组的最大值。如果`axis`为“无”，则结果为标量值。如果给定轴，结果是一个`arr.ndim`–1 维的数组。
