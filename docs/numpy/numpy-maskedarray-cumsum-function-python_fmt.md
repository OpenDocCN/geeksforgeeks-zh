...
> 原文：[https://www.geeksforgeeks.org/numpy-maskedarray-cumsum-function-python/](https://www.geeksforgeeks.org/numpy-maskedarray-cumsum-function-python/)
...
- `axis`：【int，可选】计算累计和的轴。默认值（`None`）是计算展平数组的总和。
...
【cum_sum_along_axis, ndarray】除非指定 `out`，否则将返回一个保存结果的新数组，在这种情况下，将返回对 `out` 的引用。
