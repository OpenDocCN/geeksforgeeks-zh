...
> 原文: [https://www.geeksforgeeks.org/numpy-maskedarray-any-function-python/](https://www.geeksforgeeks.org/numpy-maskedarray-any-function-python/)
...
`numpy.MaskedArray.any(axis=None, out=None, keepdims)`

## 参数

- `axis`: 【无或整数或整数元组，可选】执行逻辑与归约的一个或多个轴。
- `out`: 【n 数组，可选】存储结果的位置。
    - 如果提供，它必须具有输入广播到的形状。
    - 如果未提供或无，则返回新分配的阵列。
...
【bool 或 ndarray】除非指定`out`，否则将返回新的 boolean 或 ndarray，在这种情况下，将返回对`out`的引用。
