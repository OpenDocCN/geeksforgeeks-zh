...
> 原文: [https://www.geeksforgeeks.org/numpy-frombuffer-function-python/](https://www.geeksforgeeks.org/numpy-frombuffer-function-python/)
...
> **参数:**
>
> **`buffer`:** [buffer-like] 公开缓冲区接口的对象。
>
> **`dtype`:** [data-type, 可选] 返回数组的数据类型，默认数据类型为 `float`。
>
> **`count`:** [int, 可选] 要读取的项目数。
>
> **`offset`:** [int, 可选] 从此偏移量开始读取缓冲区，默认值为 0。
>
> **返回:** 这个函数将缓冲区解释为一维数组。
