...
> 原文: [https://www.geeksforgeeks.org/numpy-string-operations-find-function/](https://www.geeksforgeeks.org/numpy-string-operations-find-function/)

`numpy.core.defchararray.find(arr, sub, start=0, end=None)` 是 numpy 中做字符串运算的另一个函数。它返回字符串中最低的索引，在该索引中，`arr` 中的每个元素在从 `start` 到 `end` 的范围内都可以找到子字符串 `sub`。否则返回 `-1`。
