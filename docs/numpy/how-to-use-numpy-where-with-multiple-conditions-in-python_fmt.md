...
> 原文：[https://www.geeksforgeeks.org/how-to-use-numpy-where-with-multiple-conditions-in-python/](https://www.geeksforgeeks.org/how-to-use-numpy-where-with-multiple-conditions-in-python/)
...
如果给出了唯一的条件，则返回元组 `条件.nonzero()`，即条件为真的索引。在上面的语法中，我们可以看到 `where()` 函数可以接受两个参数，其中一个是强制的，另一个是可选的。如果条件的值为真，将根据索引创建一个数组。
...
np_arr1 = np.array([23, 11, 45, 43, 60, 18, 33, 71, 52, 38])
