...
`hsplit()` 函数沿 `axis` 参数 = 1 的位置拆分数组。`numpy.hsplit` 等同于 `split` 且 `axis` 参数 = 1，无论数组维度如何，数组总是沿第二个轴拆分。此函数将数组水平（按列）拆分为多个子数组。
...
# Horizontal splitting of array 'a' using np.hsplit().
print("The array {} gets splitted \
horizontally to form {} ".format(a, np.hsplit(a, 3)))

# Horizontal splitting of array 'a' using 'split' with axis parameter = 1.
print("The array {} gets splitted \
...
`vsplit()` 函数沿 `axis` 参数 = 0 的位置拆分数组。`numpy.vsplit` 等同于 `split` 且 `axis` 参数 = 0。此函数将数组垂直（按行）拆分为多个子数组。
...
# Vertical splitting of array 'a' using np.vsplit().
print("The array {} gets splitted \
vertically to form {} ".format(a, np.vsplit(a, 3)))

# Vertical splitting of array 'a' using 'split' with axis parameter = 0.
print("The array {} gets splitted \
...
# Depth-wise splitting of array 'b' using np.dsplit().
print("The array {} gets splitted \
