...
通过使用数组作为索引，可以在 `numpy` 中进行索引。在切片的情况下，返回数组的视图或浅层副本，但在索引数组中，返回原始数组的副本。`Numpy` 数组可以用除元组之外的其他数组或任何其他序列进行索引。最后一个元素的索引是 `-1`，倒数第二个是 `-2`，依此类推。
...
考虑语法 `x[obj]`，其中 `x` 是数组，`obj` 是索引。切片对象是基本切片情况下的索引。当 `obj` 为:

1.  具有 `start:stop:step` 形式的切片对象
2.  整数
3.  或者切片对象和整数的元组
...
使用整数进行索引时。第一维度的每个元素与第二维度的元素配对。所以在这种情况下，元素的索引是 `(0, 0)`，`(1, 0)`，`(2, 1)`，并选择相应的元素。
