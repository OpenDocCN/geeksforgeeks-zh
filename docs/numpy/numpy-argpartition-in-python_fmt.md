...
`numpy.argpartition()` 函数用于创建输入数组的间接分区副本，其元素以这样的方式重新排列，即第 `kth` 个位置的元素的值位于排序数组中的位置。所有小于第 `kth` 个元素的元素都移动到这个元素之前，所有等于或大于这个元素的元素都移动到它的后面。两个分区中元素的顺序未定义。它返回一个与 `arr` 形状相同的索引数组，即 `arr[index_array]` 产生 `arr` 的一个分区。
...
- **arr** : 【array_like】输入数组。
- **kth** : 【int or sequence of ints】要划分的元素索引。
- **axis** : 【int or None】要沿其排序的轴。如果为 `None`，则数组在排序前被展平。默认值为 `-1`，沿最后一个轴排序。
- **kind** : 选择算法。默认值为 `'introselect'`。
- **order** : 【str or list of str】当 `arr` 是定义了字段的数组时，此参数指定首先比较哪些字段，其次比较哪些字段等。

## 返回值

【index_array, ndarray】沿指定轴划分数组的索引数组。
