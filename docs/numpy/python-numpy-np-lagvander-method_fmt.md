...
> 原文: [https://www.geeksforgeeks.org/python-numpy-np-lagvander-method/](https://www.geeksforgeeks.org/python-numpy-np-lagvander-method/)
...
## 参数
- `arr`: 【Array-like】点的数组。数据类型被转换为 `float64` 或 `complex128`，这取决于是否有任何元素是复杂的。如果 `x` 是标量，它被转换成一维数组。
- `degree`: 【int】所得矩阵的度数。

## 返回
返回具有大小即 `array.size + (degree + 1)` 的矩阵。
