...
> 原文: [https://www.geeksforgeeks.org/numpy-maskedarray-all-function-python/](https://www.geeksforgeeks.org/numpy-maskedarray-all-function-python/)
...
## 参数
- **axis**: 【int 或 None】轴或执行逻辑与归约的轴。
- **out**: 【ndarray，可选】存储结果的位置。
    - 如果提供，它必须具有输入广播到的形状。
    - 如果未提供或无，则返回新分配的数组。
- **keepdims**: 【bool，可选】如果设置为真，则缩小的轴作为尺寸为 1 的尺寸留在结果中。使用此选项，结果将根据输入数组正确广播。

## 返回值
【ndarray，bool】除非指定 `out`，否则将返回新的布尔值或数组，在这种情况下，将返回对 `out` 的引用。
