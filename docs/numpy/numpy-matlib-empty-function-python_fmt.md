...
> 原文: [https://www.geeksforgeeks.org/numpy-matlib-empty-function-python/](https://www.geeksforgeeks.org/numpy-matlib-empty-function-python/)
...
- `dtype`: 【数据类型，可选】所需输出数据类型。
- `order`: 【{'C', 'F'}, 可选】内存中是按行-主（C 风格）还是列-主（Fortran 风格）顺序存储多维数据。

## 返回
【矩阵】给定形状和类型的新矩阵，不初始化条目。
