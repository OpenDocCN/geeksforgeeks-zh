...
## 无复制
正常分配不会复制数组对象。相反，它使用与原始数组完全相同的 `id` 来访问它。此外，任何一方的变化都会反映到另一方身上。
...
## 查看
这也叫浅抄。视图只是原始数组的视图，视图不拥有数据。当我们更改视图时，它会影响原始数组，当我们更改原始数组时，它会影响视图。
...
## 副本
这也称为深度副本。拷贝是一个全新的阵列，拷贝拥有数据。当我们对副本进行更改时，它不会影响原始阵列，而当对原始阵列进行更改时，它不会影响副本。
