...
`numpy.packbits()`是numpy中做二进制运算的另一个函数。它用于将二进制值数组的元素打包成`uint8`数组中的位。通过在末尾插入零位，将结果填充为完整字节。

> **语法：** `numpy.packbits(arr, axis=None)`
>
> **参数：**
> `arr`：【array_like】整数或布尔值的数组，其元素应该被打包成位。
> `axis`：【int，可选】完成位填充的维度。如果没有，则打包成扁平数组。
>
> **返回：**【打包数组】`uint8`类型的数组，其元素表示对应于输入元素的逻辑（0或非零）值的位。
