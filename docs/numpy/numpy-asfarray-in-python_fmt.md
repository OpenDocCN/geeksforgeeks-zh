...
`numpy.asfarray(arr, dtype='numpy.float64')`

## 参数

- **`arr`**: 【array_like】输入数据，以任何可以转换为浮点型数组的形式。这包括标量、列表、元组列表、元组、元组元组元组、列表元组和数组。
- **`dtype`**: 强制输入数组 `arr` 的浮点型代码。如果数据类型是“int”数据类型之一，它将被 `float64` 替换。
