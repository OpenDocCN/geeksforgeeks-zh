...
## 参数
- `arr` : 【阵 _ 象】输入阵。
- `max_line_width`:【int，可选】字符串应跨越的最大列数。换行符在数组元素后适当地拆分字符串。
- `precision`:【int，可选】浮点精度。默认为当前打印精度(一般为 8)。
- `suppress_small`:【bool，可选】它将非常小的数字表示为零，默认为 False。非常小的数字由精度定义，如果精度为 8，则小于 5e-9 的数字表示为零。
