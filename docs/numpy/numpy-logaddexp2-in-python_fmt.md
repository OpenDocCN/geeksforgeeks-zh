...
```py
numpy.logaddexp2(arr1, arr2, /, out=None, *, where=True, casting='same_kind', order='K', dtype=None, ufunc 'logaddexp')
```
...
【ndarray 或标量】返回 `2**x1 + 2**x2` 的以 2 为底的对数。如果 `arr1` 和 `arr2` 都是标量，这就是标量。
