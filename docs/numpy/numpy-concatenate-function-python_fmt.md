...
> 原文: `https://www.geeksforgeeks.org/numpy-concatenate-function-python/`
...
- **arr1, arr2, ...**: 【array_like 的序列】数组必须具有相同的形状，但与 `axis` 对应的维度除外。
- **axis**: 【int，可选】阵列将沿其连接的轴。如果 `axis` 为 `None`，数组在使用前会被展平。默认值为 0。
- **out**: 【n 数组，可选】如果提供，目的地放置结果。形状必须正确，与未指定 `out` 参数时 `concatenate` 返回的形状相匹配。
