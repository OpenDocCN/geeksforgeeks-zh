...
借助 `numpy.ndarray.__imul__()` 方法，我们可以乘以一个特定的值，该值在 ndarray 中作为参数提供。`__imul__()`方法。数值将被乘以到 numpy 数组中的每个元素。

> **语法:** `ndarray.__imul__(self, value, /)`
> **返回:** `self*=value`

**示例#1 :**
在这个示例中，我们可以看到数组中的每个元素都是乘以的值，该值在方法 ndarray 中作为参数给出。`__imul__()`。请记住，此方法适用于所有类型的数值。
