...
| `arcsin`，`arccos`，`arctan` | 计算反正弦、余弦和正切值 |
| `hypot` | 计算给定直角三角形的斜边 |
...
这些函数用于计算数组元素的平均值、中值、方差和最小值。它包括像这样的功能

| 功能 | 描述 |
| --- | --- |
| `amin`，`amax` | 返回数组或沿轴的最小值或最大值 |
| `ptp` | 返回数组或沿轴的值范围(最大值-最小值) |
| `percentile(a，p，axis)` | 计算数组或沿指定轴的 pth 百分比 |
| `median` | 计算沿指定轴的数据中值 |
| `mean` | 沿指定轴计算数据平均值 |
| `std` | 计算沿指定轴的数据标准偏差 |
| `var` | 计算沿指定轴的数据方差 |
| `average` | 计算沿指定轴的数据平均值 |
...
这些函数接受整数值作为输入参数，并对这些整数的二进制表示执行按位运算。它包括类似于的功能

| 功能 | 描述 |
| --- | --- |
| `bitwise_and` | 对两个数组元素执行按位“与”运算 |
| `bitwise_or` | 对两个数组元素执行按位“或”运算 |
| `bitwise_xor` | 对两个数组元素执行按位异或运算 |
| `invert` | 对数组元素执行按位反转 |
| `left_shift` | 向左移动元素的位 |
| `right_shift` | 向右移动元素的位 |
