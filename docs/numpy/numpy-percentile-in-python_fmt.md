...
`numpy.percentile()`函数用于计算给定数据（数组元素）沿指定轴的第 n 个百分位数。
...
- `arr`: 输入数组。
- `n`: 百分位值。
- `axis`: 轴，我们要沿着该轴计算百分位值。否则，它将考虑将`arr`展平（在所有轴上工作）。`axis = 0`表示沿列工作，`axis = 1`表示沿行工作。
- `out`: 我们想要放置结果的不同数组。数组必须具有与预期输出相同的维度。

**返回:** 数组的第 n 个百分位（如果`axis`为`None`，则为标量值）或沿指定轴有百分位值的数组。
