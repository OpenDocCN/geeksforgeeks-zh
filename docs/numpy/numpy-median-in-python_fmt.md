...
*   `arr`：【array_like】输入数组。
*   `axis`：【int 或 int 的元组】轴，我们要沿着该轴计算中位数。否则，它将考虑将 `arr` 展平（在所有轴上工作）。`axis = 0` 表示沿列工作，`axis = 1` 表示沿行工作。
*   `out`：【n 数组，可选】我们想要放置结果的不同数组。数组必须具有与预期输出相同的维度。
*   `dtype`：【数据类型，可选】计算中位数时我们想要的类型。

## 结果

数组的中值（如果轴为无，则为标量值）或沿指定轴具有中值的数组。
