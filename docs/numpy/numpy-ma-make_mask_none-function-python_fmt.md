# numpy.ma.make_mask_none() 函数

> 原文：[https://www.geeksforgeeks.org/numpy-ma-make_mask_none-function-python/](https://www.geeksforgeeks.org/numpy-ma-make_mask_none-function-python/)
...
- **newshape**：`tuple`
  表示掩码形状的元组。
- **dtype**：`{None, dtype}`，可选
  默认情况下，数据类型为 `None`。否则，使用一个新的数据类型，其字段与转换为布尔类型的数据类型相同。

## 返回

`ndarray`
适当形状和数据类型的数组，用 `False` 填充。
