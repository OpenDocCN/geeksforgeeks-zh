...
> 原文: [https://www.geeksforgeeks.org/numpy-string-operations-equal-function/](https://www.geeksforgeeks.org/numpy-string-operations-equal-function/)

`numpy.core.defchararray.equal(arr1, arr2)` 是 numpy 中做字符串运算的另一个函数。它逐个检查两个相同形状数组的元素，如果它们相等，则返回 `True`。否则，返回 `False`。
