...
`numpy.nansum()` 函数用于计算给定轴上的数组元素之和，将非数字（NaN）视为零。

## 语法

`numpy.nansum(arr, axis=None, dtype=None, out=None, keepdims='无值')`

## 参数

- **arr**: 【Array-like】包含需要求和的数字的数组。如果 `arr` 不是数组，则尝试转换。
- **axis**: 轴或计算总和的轴。默认值是计算展平数组的总和。
- **dtype**: 返回的数组的类型，以及对元素求和的累加器的类型。默认情况下，使用 `arr` 的数据类型。
- **out**: 【ndarray，可选】存储结果的位置。
    - 如果提供，它必须具有输入广播到的形状。
    - 如果未提供或为 `None`，则返回新分配的数组。
- **keepdims**: 布尔值，可选。
    - 如果设置为 `True`，减少的轴作为尺寸为 1 的维度留在结果中。使用此选项，结果将根据原始 `arr` 正确广播。
    - 如果该值不是默认值，则 `keepdims` 将传递给 `ndarray` 子类的均值或求和方法。
    - 如果子类方法不实现 `keepdims`，将引发任何异常。

## 返回值

除非指定 `out`，否则将返回一个保存结果的新数组，在 `out` 中返回结果。结果的大小与 `arr` 相同，形状与 `arr` 相同，如果 `axis` 不是 `None` 或 `arr`，则为一维数组。
...
如果正无穷大和负无穷大都存在，总和将不是一个数（NaN）。如果正无穷大和负无穷大中的一个存在，总和将是正无穷大或负无穷大，这是存在的。
