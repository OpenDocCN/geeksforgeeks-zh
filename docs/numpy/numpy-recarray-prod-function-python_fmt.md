...
记录数组允许使用`arr.a`和`arr.b`作为数组成员访问字段。`numpy.recarray.prod()`函数返回给定轴上数组元素的乘积。
...
**参数:**
- **axis**: 【无或整数或整数元组，可选】执行乘积的一个或多个轴。默认值`axis=None`将计算输入数组中所有元素的乘积。如果轴为负，则从最后一个轴到第一个轴计数。
- **dtype**: 【数据类型，可选】返回数组的类型。
- **out**: 【n数组，可选】存储结果的位置。
    - 如果提供，它必须具有输入广播到的形状。
    - 如果未提供或`None`，则返回新分配的数组。
- **keepdims**: 【布尔，可选】如果设置为`True`，则缩小的轴作为尺寸为1的维度留在结果中。使用此选项，结果将根据输入数组正确广播。
