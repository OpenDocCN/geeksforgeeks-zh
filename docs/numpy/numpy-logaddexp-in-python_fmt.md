...
`numpy.logaddexp(arr1, arr2, /, out=None, *, where=True, casting='same_kind', order='K', dtype=None, ufunc='logaddexp')`
...
【数组或标量】返回 `exp(arr1) + exp(arr2)` 的对数。如果 `arr1` 和 `arr2` 都是标量，这就是标量。
