# numpy 字符串操作 | upper()函数

> 原文: [`https://www.geeksforgeeks.org/numpy-string-operations-upper-function/`](https://www.geeksforgeeks.org/numpy-string-operations-upper-function/)

`numpy.core.defchararray.upper(arr)` 函数用于返回元素被转换为大写的数组。

**参数:**
`arr`: 【array_like】输入可能是字符串或 unicode 的数组。

**返回:**
【ndarray】输出字符串或 unicode 的高位数组，具体取决于输入类型。
