...
`numpy.logical_and(arr1, arr2, out=None, where=True, casting='same_kind', order='K', dtype=None, ufunc='logical_and')`：这是一个逻辑函数，它帮助用户逐元素找出 `arr1` 和 `arr2` 的真值。两个数组必须具有相同的形状。
...
一个包含 `arr1` 和 `arr2` 逐元素逻辑与运算布尔结果的数组（形状相同）。
...
[https://docs.scipy.org/doc/numpy-1.13.0/reference/generated/numpy.logical_and.html#numpy.logical_and](https://docs.scipy.org/doc/numpy-1.13.0/reference/generated/numpy.logical_and.html#numpy.logical_and)