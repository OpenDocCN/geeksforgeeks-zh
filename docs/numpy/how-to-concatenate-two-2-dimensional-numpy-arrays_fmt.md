...
> 原文：[https://www.geeksforgeeks.org/how-to-concatenate-two-2-dimensional-numpy-arrays/](https://www.geeksforgeeks.org/how-to-concatenate-two-2-dimensional-numpy-arrays/)
...
然后必须创建两个 2D 阵列来执行操作，通过使用 [`arange()`](https://www.geeksforgeeks.org/numpy-arange-python/) 和 [`reshape()`](https://www.geeksforgeeks.org/numpy-reshape-python/) 功能。使用 NumPy，我们可以以各种方式和方法执行多个 2D 阵列的拼接。

## 方法一：使用 `concatenate()` 功能

我们可以使用 [`concatenate()`](https://www.geeksforgeeks.org/numpy-concatenate-function-python/) 功能来执行连接操作。使用这个函数，数组可以按行或按列连接，假设它们分别有相等的行或列。列级连接可以通过将轴等于 1 作为函数中的一个参数来实现。
...
[`stack()`](https://www.geeksforgeeks.org/numpy-stack-in-python/) 功能的使用方法与 `concatenate()` 功能相同，其中轴等于一。通过使用这个，阵列一个堆叠在另一个之上。
...
[`hstack()`](https://www.geeksforgeeks.org/numpy-hstack-in-python/) 函数将数组水平堆叠，即沿一列堆叠。
