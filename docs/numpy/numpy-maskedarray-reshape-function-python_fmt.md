...
> 原文: [https://www.geeksforgeeks.org/numpy-maskedarray-reshape-function-python/](https://www.geeksforgeeks.org/numpy-maskedarray-reshape-function-python/)

`numpy.MaskedArray.reshape()` 函数用于在不改变其数据的情况下，给被屏蔽的数组赋予一个新的形状。它返回一个包含相同数据但具有新形状的屏蔽数组。结果是原始数组的视图；如果这不可能，则会引发 `ValueError`。
...
- `shape`: 【int 或 int 的元组】新形状应该与原始形状兼容。
