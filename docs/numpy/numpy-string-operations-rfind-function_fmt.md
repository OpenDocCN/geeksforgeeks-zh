...
> 原文：[https://www.geeksforgeeks.org/numpy-string-operations-rfind-function/](https://www.geeksforgeeks.org/numpy-string-operations-rfind-function/)

## 函数介绍

`numpy.core.defchararray.find(arr, sub, start=0, end=None)` 是 numpy 中做字符串运算的另一个函数。它返回字符串中最高的索引，在该索引中为 `arr` 中的每个元素找到子字符串 `sub`。如果 `sub` 不包含在 `[start, end]` 中，则返回 `-1`。
