...
> 原文：[https://www.geeksforgeeks.org/python-numpy-nanmedian-function/](https://www.geeksforgeeks.org/python-numpy-nanmedian-function/)

**`numpy.nanmedian()`** 函数可以用来计算忽略 `NaN` 值的数组的中值。如果数组有 `NaN` 值，我们可以在不影响 `NaN` 值的情况下求出中值。让我们看看关于 `numpy.nanmedian()` 方法的不同类型的例子。

> **语法：** `numpy.nanmedian(a, axis=None, out=None, overwrite_input=False, keepdims=)`
> **参数：**
> `a`：【array_like】输入数组
> `axis`：我们可以用 `axis=1` 表示行方向，`axis=0` 表示列方向。
> `out`：输出数组
> `overwrite_input`：如果为真，则允许使用输入数组 `a` 的内存进行计算。对中值的调用将修改输入数组。
> `keepdims`：如果设置为真，减少的轴将作为尺寸为 1 的尺寸留在结果中。使用此选项，结果将正确地广播到原始 `a`。
> **返回：** 返回 n 数组中的中间值。
...
print("Median of array with axis = 1:",
...
Median of array with axis = 1: [ 12.  nan  nan]
