# Python | Numpy MaskedArray.——《洪水》

> 哎哎哎:# t0]https://www . geeksforgeeks . org/python-num py-masked array-_ _ flood rdiv _/

`numpy.ma.MaskedArray class`是`ndarray`的一个子类，设计用于处理缺少数据的数值数组。借助`Numpy MaskedArray.__floordiv__`运算符，我们可以对作为参数提供给该函数的特定值进行除法运算。
...
## 示例#1
在这个示例中，我们可以看到在应用`MaskedArray.__floordiv__()`之后，我们得到了数组中被划分的每个元素的 floor 值。此方法对于数组的正、负和浮点值都很有效。
