...
>
> **参数:**
> `arr`: 【array_like】输入数据，以任何可以转换为数组的形式。这包括列表、元组列表、元组、元组元组元组、列表元组和数组。
> `dtype`: 【数据类型，可选】默认情况下，数据类型是从输入数据中推断出来的。
> `order`: 是使用行主（C 风格）还是列主（Fortran 风格）内存表示。默认为 `'C'`。
>
> **返回:** 【ndarray】`arr` 的数组解释。如果输入已经是具有匹配数据类型和顺序的数组，则不执行复制。如果 `arr` 是 `ndarray` 的子类，则返回基类 `ndarray`。
