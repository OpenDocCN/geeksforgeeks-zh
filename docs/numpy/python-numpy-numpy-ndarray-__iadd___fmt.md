...
借助`numpy.ndarray.__iadd__()`方法，我们可以添加一个特定的值，该值在`ndarray.__iadd__()`方法中作为参数提供。数值将被添加到 numpy 数组的每个元素中。
...
**示例#1 :**
在这个示例中，我们可以看到数组中的每个元素都被添加了，该值在方法`ndarray.__iadd__()`中作为参数给出。请记住，此方法适用于所有类型的数值。
