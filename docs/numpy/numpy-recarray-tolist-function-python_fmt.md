...
> 原文: [https://www.geeksforgeeks.org/numpy-recarray-tolist-function-python/](https://www.geeksforgeeks.org/numpy-recarray-tolist-function-python/)

在 numpy 中，数组可能具有包含字段的数据类型，类似于电子表格中的列。一个例子是`[(a, int), (b, float)]`，数组中的每个条目都是一对(int，float)。通常，这些属性是使用字典查找来访问的，例如`arr['a']`和`arr['b']`。

记录数组允许使用`arr.a`和`arr.b`作为数组成员访问字段。`numpy.recarray.tolist()`函数用于使数组成为一个(可能是嵌套的)列表。它以(嵌套的)Python 列表的形式返回数组数据的副本。数据项被转换为最近的兼容 Python 类型。
