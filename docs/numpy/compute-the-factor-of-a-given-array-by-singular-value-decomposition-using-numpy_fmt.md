...
> 原文: [https://www.geeksforgeeks.org/compute-the-factor-of-a-given-array-by-singular-value-decomposition-using-numpy/](https://www.geeksforgeeks.org/compute-the-factor-of-a-given-array-by-singular-value-decomposition-using-numpy/) 通过奇异值分解计算给定数组的因子
...
> *   `full_matrix(bool, 可选)`: 如果为真(默认)，`u` 和 `vh` 分别具有形状(…，M，M)和(…，N，N)。否则，形状分别为(…，M，K)和(…，K，N)，其中 `K = min(M, N)`。
> *   `compute_uv(bool, 可选)`: 除 `s` 外是否计算 `u` 和 `vh`，默认值为 `True`。
> *   `hermitian(bool, 可选)`: 如果为 `True`，则假设 `a` 为 `hermitian`(如果实值，则为对称)，这为寻找奇异值提供了更有效的方法。其默认值为 `False`。
