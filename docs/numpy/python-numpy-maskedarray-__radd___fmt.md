...
`numpy.ma.MaskedArray class`是`ndarray`的一个子类，设计用于处理缺少数据的数值数组。借助`MaskedArray.__radd__`我们可以添加一个特定的值，该值在`MaskedArray`中作为参数提供。`__radd__()`方法。数值将被添加到`numpy`数组中的每个元素。

> **语法:**`numpy.ma.MaskedArray.__radd__(other)`
