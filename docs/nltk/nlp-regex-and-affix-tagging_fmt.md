...
*   `RegexpTagger`是`SequentialBackoffTagger`的一个子类。它可以位于`DefaultTagger`类之前，以便标记n-gram标记器遗漏的单词，因此可以成为退避链的有用部分。
...
指定三个字符的后缀。单词长度必须至少为5个字符，如果单词少于5个字符，则返回`None`作为标记。
