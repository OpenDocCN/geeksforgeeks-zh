...
> 原文: [https://www.geeksforgeeks.org/nlp-filtering-insignificant-words/](https://www.geeksforgeeks.org/nlp-filtering-insignificant-words/)
...
## 代码#1: `filter_insignificant()` 函数过滤掉不重要的词
...
`filter_insignificant()` 通过迭代组块中的标记单词，检查该标记是否以 `tag_suffix` 结束(对于每个标记)。如果标记以任何一个 `tag_suffixes` 结尾，则跳过标记的单词。否则，如果标记正常，标记的单词将被追加到返回的新的好的块中。

## 代码#2: 在短语上使用 `filter_insignificant()`
