...
> 原文: [https://www.geeksforgeeks.org/nlp-splitting-and-merging-chunks/](https://www.geeksforgeeks.org/nlp-splitting-and-merging-chunks/)

**`SplitRule` 类:** 它基于为此目的指定的分割模式来分割块。它被指定为`<NN.*>}{<.*>`即两边被图案包围的两个相对的花括号。

**`MergeRule` 类:** 它基于第一个组块的结束和第二个组块的开始将两个组块合并在一起。它被指定为`<NN.*>{}<.*>`即花括号相向。
