...
> 原文：[https://www.geeksforgeeks.org/nlp-storing-an-ordered-dictionary-in-redis/](https://www.geeksforgeeks.org/nlp-storing-an-ordered-dictionary-in-redis/)

有序字典就像普通字典，但是关键字是由一个排序函数排序的。就 Redis 而言，它支持关键字为字符串、值为浮点分数的有序字典。在必须计算信息增益并且必须存储所有单词和分数以备后用的情况下，这种结构可以派上用场。

`redis_collections.py` 中的 `RedisOrderedDict` 类扩展了 `collections.MutableMapping` 来免费获得一些 `dict` 兼容的方法。然后，它实现了所有需要 Redis 有序集（也称为 `Zset`）命令的关键方法。
...
class RedisOrderedDict(collections.MutableMapping):
    ...
    def keys(self, start = 0, end = -1):
        ...
        # by high value instead of by lowest
...
大部分代码可能看起来类似于 `RedisHashMap`，这是意料之中的，因为它们都扩展了 `collections.MutableMapping`。这里的主要区别在于 `RedisOrderedDict` 通过浮点值对键进行排序，因此它不适合像 `RedisHashMap` 那样的任意键值存储。
...
*   `keys()`: 这使用 `zrevrange` 命令获取有序集中的所有键，按最高分排序。它需要两个可选的关键字参数 `start` 和 `end`，以更有效地获得有序键的一部分。