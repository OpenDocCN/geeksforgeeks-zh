...
> 原文: [https://www.geeksforgeeks.org/nlp-extracting-named-entities/](https://www.geeksforgeeks.org/nlp-extracting-named-entities/)

## 介绍

识别命名实体是一种特定类型的块提取，它使用实体标签和块标签。常见的实体标签包括 `PERSON`、`LOCATION` 和 `ORGANIZATION`。POS 标记的句子被解析成具有正常组块的组块树，但是树标签可以是代替组块短语标签的实体标签。NLTK 已经有一个预先训练好的命名实体组块器，可以在 `nltk.chunk` 模块中使用 `ne_chunk()` 方法。这个方法把一个句子分成一棵树。

## 代码示例

### 代码#1: 在树库_组块语料库的标记句子上使用 `ne_chunk()`
