...
> 原文: [https://www.geeksforgeeks.org/nlp-classifier-based-chunking-set-2/](https://www.geeksforgeeks.org/nlp-classifier-based-chunking-set-2/)
...
单词可以通过 `tagger` 传递到我们的特征检测器函数中，通过创建嵌套的 2-元组的形式 `((word，pos)，iob)`，`chunk_trees2train_chunks()` 方法产生这些嵌套的 2-元组。提取以下特征：
