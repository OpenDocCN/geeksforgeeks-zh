...
> 原文: [https://www.geeksforgeeks.org/nlp-storing-frequency-distribution-in-redis/](https://www.geeksforgeeks.org/nlp-storing-frequency-distribution-in-redis/)

`nltk` 中的许多类都使用了 `FreqDist` 类来存储和管理频率分布。它非常有用，但是都在内存中，没有提供持久化数据的方法。多个进程也无法访问单个 `FreqDist`。所有这些都可以通过在 Redis 上建立一个 `FreqDist` 来改变。
...
*   `FreqDist` 类扩展了标准库 `collections.Counter` 类，它使一个 `FreqDist` 成为一个带有一些额外方法的小包装器，比如 `N()`。
*   `N()` 方法返回样本结果的数量，这是频率分布中所有值的总和。
*   通过扩展一个 `RedisHashMap` 然后实现 `N()` 方法，在 Redis 之上创建了一个与 API 兼容的类。
...
大部分工作是在 `RedisHashMap` 类中完成的，该类扩展了 `collections.MutableMapping` 然后覆盖所有需要 Redis 特定命令的方法。概述每个使用特定 Redis 命令的方法：

*   `__len__()` : 这使用 `hlen` 命令来获取哈希映射中的元素数量。
*   `__contains__()`: 这使用 `hexists` 命令来检查哈希映射中是否存在元素。
*   `__getitem__()`: 这使用 `hget` 命令从哈希映射中获取一个值。
*   `__setitem__()`: 这使用 `hset` 命令在哈希映射中设置一个值。
*   `__delitem__()`: 这使用 `hdel` 命令从哈希映射中移除一个值。
*   `keys()`: 这使用 `hkeys` 命令获取哈希映射中的所有密钥。
