...
> 原文: [https://www.geeksforgeeks.org/nlp-training-tagger-based-chunker-set-1/](https://www.geeksforgeeks.org/nlp-training-tagger-based-chunker-set-1/)

训练组块器是手动指定正则表达式（正则表达式）组块模式的替代方法。但是手动训练来指定表达式是一项乏味的任务，因为它遵循命中和尝试的方法来获得准确的正确模式。因此，现有的语料库数据可以用来训练组块。

## 介绍

在下面的代码中，我们使用 `treebank_chunk` 语料库以树的形式生成分块句子。
- 要训练基于 tagger 的 chunker – `chunked_sents()` 方法由 `TagChunker` 类使用。
