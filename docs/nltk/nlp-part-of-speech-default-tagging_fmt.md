...
> 原文：[https://www.geeksforgeeks.org/nlp-part-of-speech-default-tagging/](https://www.geeksforgeeks.org/nlp-part-of-speech-default-tagging/)

## 什么是词性标注？
这是一个将句子转换成形式的过程——单词列表，元组列表(其中每个元组都有一个形式*(单词，标签)*)。的标记是词性标记，表示该词是否是名词、形容词、动词等。
![](img/d725f7ba82d7a11dfd457488c391d824.png)

**默认标注**是词性标注的基本步骤。它是使用`DefaultTagger`类执行的。`DefaultTagger`类将`'标签'`作为单个参数。`'NN'`是单数名词的标签。`DefaultTagger`在使用最常见的词性标签时最有用。这就是为什么推荐使用名词标签的原因。
...
**注意:**标记句子列表中的每个标记(在上面的代码中)都是`'NN'`，就像我们使用的`DefaultTagger`类一样。
