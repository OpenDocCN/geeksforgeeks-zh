...
*   假设段落被空行分割
*   使用 `para_block_reader` 功能完成，即 `nltk.corpus.reader.util.read_blankline_block`
*   许多其他块读取器存在于 `nltk.corpus.reader.util` 中，其目的是从流中读取文本块。
...
### 标签集:
是一个或多个语料库使用的 POS 标签列表。

### 通用标记集:
它是一个简化和精简的标记集，仅由 12 个词性标记组成
