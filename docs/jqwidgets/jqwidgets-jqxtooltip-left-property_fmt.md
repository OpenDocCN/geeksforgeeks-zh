...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtooltip-left-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtooltip-left-property/)
...
`left` 属性用于根据位置属性设置或返回 `jqxTooltip` 的水平偏移量。它接受数字/字符串类型的值，默认值为 `0`。
...
## 示例:
以下示例说明了 `jQWidgets` `jqxTooltip` `left` 属性。
