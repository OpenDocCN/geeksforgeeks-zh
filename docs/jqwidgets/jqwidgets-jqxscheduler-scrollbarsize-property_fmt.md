# jQWidgets jqxScheduler scrollBarSize 属性
...
`scrollBarSize` 属性用于设置或返回显示的 `jqxScheduler` 中滚动条的大小。它属于数字类型，默认值为 15。
...
```html
var scrollBarSize = $('Selector').jqxScheduler('scrollBarSize');
```
...
以下示例说明了 `jQWidgets` 中的 `jqxScheduler` `scrollBarSize` 属性。
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxScheduler scrollBarSize property</h3>
