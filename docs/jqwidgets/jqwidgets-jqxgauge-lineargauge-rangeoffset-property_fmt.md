...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxgauge-lineargauge-rangeoffset-property/](https://www.geeksforgeeks.org/jqwidgets-jqxgauge-lineargauge-rangeoffset-property/)
...
`rangeOffset` 属性用于设置或返回 `rangeOffset` 属性。即它用于设置或返回偏离刻度的范围。它接受数字类型值，默认值为 0。
...
```html
var rangesOffset = $('Selector').jqxLinearGauge('rangesOffset');
```
...
下面的示例说明了 jQWidgets 中的 `jqxLinearGauge` `rangeOffset` 属性。
