...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxslider-theme-property/](https://www.geeksforgeeks.org/jqwidgets-jqxslider-theme-property/)
...
**主题属性**用于设置或返回 `jqxSlider` 小部件的主题。它接受字符串类型值，默认值为空(`""`)。要使用这个属性，首先，我们需要在标题部分包含主题样式表(`jqx.energyblue.css`)。主题文件包括在 `jqx.base.css` 文件之后。
...
```javascript
$('selector').jqxSlider({ theme: String });
```

返回 `theme` 属性。

```javascript
var theme = $('selector').jqxSlider('theme');
```
...
下面的例子说明了 jQWidgets `jqxSlider` `theme` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxSlider theme Property
        </h3>
...
![](img/4a41bcdb35fa575d4bbec00011a59d83.png)
