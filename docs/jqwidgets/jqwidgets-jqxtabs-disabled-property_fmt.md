# jQWidgets jqxTabs 禁用属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtabs-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtabs-disabled-property/)
...
`disabled` 属性用于启用或禁用 `jqxTabs` 小部件。它接受布尔类型值，默认值为 `false`。
...
```javascript
$('selector').jqxTabs({ disabled: Boolean });
```

返回 `disabled` 属性。

```javascript
var disabled = $('selector').jqxTabs('disabled');
```
...
**示例:** 以下示例说明了 `jQWidgets` `jqxTabs` `disabled` 属性。
...
![](img/edbdba11e070e916d5f5d57e7bcdbcfc.png)
