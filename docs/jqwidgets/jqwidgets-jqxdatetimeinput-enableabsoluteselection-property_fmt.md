# jQWidgets jqxDateTimeInput enableAbsoluteSelection 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdatetimeinput-enableabsoluteselection-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdatetimeinput-enableabsoluteselection-property/)
...
`enableAbsoluteSelection` 属性用于使用户在键入或选择文本输入字段时一次只能选择一个符号。它接受布尔类型值，默认值为 `false`。

## 语法

设置 `enableAbsoluteSelection` 属性。

```javascript
$('selector').jqxDateTimeInput({ enableAbsoluteSelection: Boolean });
```

返回 `enableAbsoluteSelection` 属性。
...
下面的例子说明了 jQWidgets 中的 `jqxDateTimeInput` `enableAbsoluteSelection` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxDateTimeInput enableAbsoluteSelection Property
        </h3>
...
![](img/8219938c967d671586053ec5ef2fb286.png)
