...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxslider-layout-property/](https://www.geeksforgeeks.org/jqwidgets-jqxslider-layout-property/)
...
`layout` 属性用于设置或返回滑块小部件的布局。它接受字符串类型的值，默认值为 `"normal"`。
...
下面的例子说明了 jQWidgets `jqxSlider` 的 `layout` 属性。
