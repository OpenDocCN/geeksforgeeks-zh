...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxpopover-ismodal-property/](https://www.geeksforgeeks.org/jqwidgets-jqxpopover-ismodal-property/)
...
`isModal` 属性用于设置或获取指定的 `jqxPopover` 是否显示为模态对话框。当该属性值设置为 `true` 时，弹出窗口会阻止用户与底层用户界面的交互。
...
以下示例说明了 jQWidgets jqxPopover `isModal` 属性。在下面的例子中，`isModal` 属性的值被设置为真。
