# jQWidgets jqxMenu 禁用属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxmenu-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxmenu-disabled-property/)
...
`disabled` 属性用于启用或禁用 `jqxMenu` 小部件。它接受布尔类型值，默认值为 `false`。
...
```javascript
$('selector').jqxMenu({ disabled: Boolean });
```

返回 `disabled` 的属性。

```javascript
var disabled = $('selector').jqxMenu('disabled');
```
...
下面的例子说明了 jQWidgets `jqxMenu` `disabled` 属性。
...
<body>
    ...
    <h3>
        jQWidgets jqxMenu disabled Property
    </h3>
...
![](img/b50ee8c3b7635f5dd667788bb94282b9.png)
