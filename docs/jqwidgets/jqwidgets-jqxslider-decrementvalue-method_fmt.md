...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxslider-decrementvalue-method/](https://www.geeksforgeeks.org/jqwidgets-jqxslider-decrementvalue-method/)
...
`decrementValue()` 方法用于用 `step` 属性的值减少 `jqxSlider` 小部件的值。它不接受任何参数，也不返回值。
...
下面的例子说明了 jQWidgets `jqxSlider` `decrementValue()` 方法。
