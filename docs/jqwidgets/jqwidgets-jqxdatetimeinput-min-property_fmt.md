# jq 构件 jqxDateTimeInput min 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdatetimeinput-min-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdatetimeinput-min-property/)
...
`min` 属性用于设置或返回 `jqxDateTimeInput` 小部件的最小日期。它接受日期类型值，默认值为 `Date(1900, 1, 1)`。
...
```html
$('selector').jqxDateTimeInput({ min: Date });
```

返回最小属性。

```html
var min = $('selector').jqxDateTimeInput('min');
```
...
下面的例子说明了 jQWidgets 中的 `jqxDateTimeInput` `min` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxDateTimeInput min Property
        </h3>
...
![](img/10482d4e0bf852a3fd1c313b139d806a.png)
