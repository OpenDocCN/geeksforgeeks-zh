...
搜索模式属性用于设置或返回项目的增量搜索模式。当用户输入一个有焦点的*列表框*时，这个属性很有用。其中， `jqxListBox` 小部件试图借助输入的文本和选择的搜索模式来确定搜索的项目。它接受字符串类型值，默认值为`“startswith”`。
...
以下示例说明了 jQWidgets 中的 `jqxListBox` 搜索模式属性。
