...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxscrollview-movethreshold-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscrollview-movethreshold-property/)
...
`moveThreshold` 属性指定用户应该拖动当前元素多少才能导航到下一个/上一个元素。它接受数字类型的值，默认值为 0.5。
...
```html
var moveThreshold = $('selector').jqxScrollView('moveThreshold');
```
