# jQWidgets jqxMenu 模式属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxmenu-mode-property/](https://www.geeksforgeeks.org/jqwidgets-jqxmenu-mode-property/)
...
**模式属性**用于设置或返回 `jqxMenu` 小部件的显示模式。它接受字符串类型值，默认值为“水平”。
...
设置 `mode` 属性。
```html
$('selector').jqxMenu({ mode: Boolean });
```

返回 `mode` 属性。
```html
var mode = $('selector').jqxMenu('mode');
```
...
下面的例子说明了 jQWidgets `jqxMenu` 模式属性。
...
<body>
    ...
    <h3>
        jQWidgets jqxMenu mode Property
    </h3>
...
![](img/c80079121b3fa7af38846ab72018a48b.png)
