...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxprogressbar-template-property/](https://www.geeksforgeeks.org/jqwidgets-jqxprogressbar-template-property/)
...
*   `primary`：设置深蓝色风格，增加视觉重量。
*   `success`：它为成功或积极的行动设置绿色风格。
*   `warning`：设置代表警告的橙色样式。
*   `danger`：设置代表危险或消极动作的红色样式。
*   `info`：设置蓝色按钮，不绑定语义动作或使用。
...
## 示例
以下示例说明了 jQWidgets 中的 `jqxProgressBar` `模板`属性。
