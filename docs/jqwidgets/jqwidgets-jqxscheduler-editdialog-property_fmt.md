...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-editdialog-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-editdialog-property/)
...
`editDialog` 属性用于设置或返回显示的 `jqxScheduler` 的编辑对话框是否启用。它是布尔类型，默认值为*真*。
...
以下示例说明了 jQWidgets 中的 `jqxScheduler` `editDialog` 属性。
