# jQWidgets jqxListMenu filterCallback 属性
...
`filterCallback` 属性用于使用过滤器输入设置或返回过滤后的元素。它接受函数类型值，默认值是 `function(text, searchValue) { return text.toString().toLowerCase().indexOf(searchValue.toLowerCase()) >= 0; };`。
...
```javascript
var filterCallback = $('selector').jqxListMenu('filterCallback');
```
