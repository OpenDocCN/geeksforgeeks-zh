# jQWidgets jqxNumberInput 十进制数字属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-decimaldigits-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-decimaldigits-property/)
...
`十进制数字属性`用于设置或返回数值中使用的小数位数。它接受数字类型值，默认值为 2。
...
```html
$('selector').jqxNumberInput({ decimalDigits: Boolean });
```

返回十进制数字属性。

```html
var decimalDigits = $('selector').jqxNumberInput('decimalDigits');
```
...
下面的例子说明了 jQWidgets jqxNumberInput `decimalDigits` 属性。
...
![](img/62636bcb0fc4e27511c15f3cadeb096c.png)
