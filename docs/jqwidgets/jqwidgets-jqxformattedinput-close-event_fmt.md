# jqwidget jqxfformattitinput close event

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-close-event/](https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-close-event/)
...
`关闭事件`用于在 `jqxFormattedInput` 小部件的弹出窗口关闭时触发。
...
下面的例子说明了 jQWidgets `jqxFormattedInput` 关闭事件。
...
<body>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#jqxFI").on("close", function(event) {
                alert("Closing DropDown List");
            });
        });
    </script>
</body>
...
![](img/09d7dee79964220726db86cc02bef253.png)
