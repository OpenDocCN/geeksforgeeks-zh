...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxcomplexinput-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxcomplexinput-disabled-property/)
...
`disabled` 属性用于启用或禁用 `jqxComplexInput` 小部件。它接受布尔类型值，默认值为 `false`。
...
以下示例说明了 jQWidgets `jqxComplexInput` `disabled` 属性。
