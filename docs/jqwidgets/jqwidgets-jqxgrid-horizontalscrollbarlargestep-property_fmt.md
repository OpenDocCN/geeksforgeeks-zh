# jQWidgets jqxGrid horizontalscrollbarlargestep 属性
...
`horizontalscrollbarlargestep` 属性用于设置或返回滚动条的大步长。此外，它定义了当用户点击显示的 `jqxGrid` 的拇指上方或下方的部分时，水平滚动条的值被改变的步骤。它是数字类型，默认值为 50。
...
```html
var horizontalscrollbarlargestep = $('#Selector').jqxGrid('horizontalscrollbarlargestep');
```
...
下面的例子说明了 jQWidgets 中的 `jqxGrid` `horizontalscrollbarlargestep` 属性。
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxGrid horizontalscrollbarlargestep property
        </h3>
        <br />
