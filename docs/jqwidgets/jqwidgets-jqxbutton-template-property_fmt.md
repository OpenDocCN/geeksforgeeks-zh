...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxbutton-template-property/](https://www.geeksforgeeks.org/jqwidgets-jqxbutton-template-property/)
...
`template` 属性用于指定显示按钮的模板，作为所用默认样式的选项。它的类型为字符串，默认值为“默认值”。
...
以下示例说明了 `jQWidgets` 中的 `jqxButton` `template` 属性。
