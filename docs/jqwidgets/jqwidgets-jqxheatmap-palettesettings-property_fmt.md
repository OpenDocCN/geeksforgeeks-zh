...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxheatmap-palettesettings-property/](https://www.geeksforgeeks.org/jqwidgets-jqxheatmap-palettesettings-property/)

## jQWidgets
`jQWidgets` 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大和优化的框架，独立于平台，并得到广泛支持。

## jqxHeatMap
`jqxHeatMap` 代表一个 jQuery 小部件，它显示了使用颜色编码来表示不同值的数据的图形表示。

## paletteSettings 属性
`paletteSettings` 属性用于设置或返回参数设置属性。即该属性用于设置或返回热图调色板的设置。它接受对象类型值，默认值为 `null`。
...
```html
var paletteSettings = $('Selector').jqxHeatMap('paletteSettings');
```

## 属性
*   `palette`: 用于设置热图单元格的颜色集合。它包含以下值:
    *   `value`: 调色板颜色值。
    *   `color`: 调色板。
    *   `label`: 调色板颜色的标签。此属性是可选的。
*   `type`: 用于设置颜色样式。可能的选项有 `'gradient'` (用于显示具有线性渐变颜色的单元格)、`'fixed'` (用于显示具有固定颜色的单元格)。默认值为 `'gradient'`。
*   `emptyPointColor`: 用于设置空点的颜色。默认值为 `'#eee'`。
...
## 示例
以下示例说明了 `jQWidgets` 中的 `jqxHeatMap` `paletteSettings` 属性。
