# jQWidgets jqxPasswordInput showPasswordIcon 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxpasswordinput-showpasswordicon-property/](https://www.geeksforgeeks.org/jqwidgets-jqxpasswordinput-showpasswordicon-property/)
...
`showPasswordIcon` 属性用于设置或返回是否显示/隐藏密码图标。它接受布尔类型值，默认值为 `true`。

## 语法

设置 `showPasswordIcon` 属性。

```javascript
$('selector').jqxPasswordInput({ showPasswordIcon: Boolean });
```

返回 `showPasswordIcon` 属性。
