...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdocking-orientation-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdocking-orientation-property/)
...
`orientation` 属性用于设置或获取指定 `jqxDocking` 的方向。该属性检查面板是并排还是在彼此下方。
...
```javascript
var vertical = $('#jqxDocking').jqxDocking('orientation');
```
...
以下示例说明了 `jQWidgets` 的 `orientation` 属性。在以下示例中，`orientation` 属性的值已设置为 `'horizontal'`。
