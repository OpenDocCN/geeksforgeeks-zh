...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxcomplexinput-spinbuttons-property/](https://www.geeksforgeeks.org/jqwidgets-jqxcomplexinput-spinbuttons-property/)
...
`spinButtons` 属性用于设置或返回旋转按钮。它接受布尔类型值，默认值为 `false`。要添加旋转按钮，在 `jqxComplexInput` 的初始化 div 中需要一个额外的空 div 元素。
...
下面的例子说明了 jQWidgets `jqxComplexInput` `spinButtons` 属性。
