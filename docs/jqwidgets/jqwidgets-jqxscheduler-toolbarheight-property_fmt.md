# jQWidgets jqxScheduler 工具栏高度属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-toolbarheight-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-toolbarheight-property/)
...
`toolbarHeight` 属性用于在显示的 `jqxScheduler` 中设置或返回工具栏的高度。它属于数字类型，默认值为 54。
...
```html
$("#Selector").jqxScheduler({ toolbarHeight: 40 });
```

*   返回 `toolbarHeight` 属性。

```html
var toolbarHeight = $('#Selector').jqxScheduler('toolbarHeight');
```
...
下面的示例说明了 jQWidgets 中的 `jqxScheduler` 的 `toolbarHeight` 属性。
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxScheduler toolbarHeight property </h3>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#jqxBtn").on("click", function () {
                var th = $('#jqxs').jqxScheduler('toolbarHeight');
                $('#log').text("Toolbar height: " + th);
            });
        });
    </script>
</body>
...
![](img/51245b96267e8be0befc26086e775c0c.png)
