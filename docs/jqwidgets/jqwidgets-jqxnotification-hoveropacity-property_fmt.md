...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxnotification-hoveropacity-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnotification-hoveropacity-property/)
...
`jqxNotification` 代表一个 jQuery 小部件，可以用来向用户显示一些通知内容。`jqxNotification` 小部件内容可以根据用户需求进行修改。
...
以下示例说明了 jQWidgets 中的 `jqxNotification` 悬停不透明度属性：
