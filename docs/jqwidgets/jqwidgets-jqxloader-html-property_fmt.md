# jQWidgets jqxLoader html 属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxloader-html-property/](https://www.geeksforgeeks.org/jqwidgets-jqxloader-html-property/)
...
`html` 属性用于设置或返回加载器元素的内容。它接受字符串类型值，默认值为空。
...
下面的例子说明了 `jQWidgets` `jqxLoader` 的 `html` 属性。
