...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-incrementalsearch-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-incrementalsearch-property/)
...
`incrementalSearch` 属性用于设置或返回增量搜索属性。只要您键入搜索字符串的第一个字符，增量搜索就开始搜索。它接受布尔类型值，默认值为 `true`。

## 语法
设置 `incrementalSearch` 属性。

```html
$('selector').jqxDropDownList({ incrementalSearch: Number });
```

返回 `incrementalSearch` 属性。

```html
var incrementalSearch = $('selector').jqxDropDownList('incrementalSearch');
```
...
## 示例
下面的示例说明了 jQWidgets 中的 `jqxDropDownList` `incrementalSearch` 属性。
...
## 输出
![](img/545eb29884aaa54be81e9c32d379c10f.png)
