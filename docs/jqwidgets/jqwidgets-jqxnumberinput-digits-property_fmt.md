# jQWidgets jqxNumberInput digits 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-digits-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-digits-property/)
...
`digits` 属性用于设置或返回输入字段允许的位数。它接受数字类型值，默认值为 8。
...
```javascript
$('selector').jqxNumberInput({ digits: Number });
```

返回 `digits` 属性。

```javascript
var digits = $('selector').jqxNumberInput('digits');
```
...
下面的例子说明了 jQWidgets `jqxNumberInput` `digits` 属性。
...
![](img/e714637dd1462d029d0322c039f3ed13.png)
