# jQWidgets jqxTabs 启用切换属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtabs-enabledhover-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtabs-enabledhover-property/)
...
**启用悬停属性** `enabledHover` 用于启用或禁用标签悬停效果。它接受布尔类型值，默认值为 `true`。
...
```javascript
$('selector').jqxTabs({ enabledHover: Boolean });
```

返回 `enabledHover` 属性。

```javascript
var enabledHover = $('selector').jqxTabs('enabledHover');
```
...
下面的例子说明了 jQWidgets `jqxTabs` `enabledHover` 属性。
...
![](img/c5b722d2897e695b6fbc0078e0981e35.png)
