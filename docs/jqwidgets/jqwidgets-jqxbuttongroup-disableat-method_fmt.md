...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxbuttongroup-disableat-method/](https://www.geeksforgeeks.org/jqwidgets-jqxbuttongroup-disableat-method/)
...
`disableAt()` 方法用于在特定索引处禁用显示的 `jqxButtonGroup`。它有一个参数，一个数字类型的索引。它不返回任何东西。
...
下面的示例说明了 jQWidgets 中的 `jqxButtonGroup` `disableAt()` 方法。
...
<html lang="en">
  ...
  <body>
    <center>
      ...
      <div id="jqxBG">
        <button style="padding: 6px 36px" id="l">ON</button>
        <button style="padding: 6px 36px" id="c">Radio_Button</button>
        <button style="padding: 6px 36px" id="r">OFF</button>
      </div>
      <div>
        <input
          type="button"
