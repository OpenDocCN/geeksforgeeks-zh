...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtooltip-width-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtooltip-width-property/)
...
`width` 属性用于设置或返回 `jqxTooltip` 小部件的宽度。它接受数字/字符串类型的值，默认值为“自动”。
...
下面的例子说明了 jQWidgets `jqxTooltip` `width` 属性。
