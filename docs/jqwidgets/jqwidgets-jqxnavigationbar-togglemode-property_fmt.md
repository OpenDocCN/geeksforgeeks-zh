...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxnavigationbar-togglemode-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnavigationbar-togglemode-property/)
...
`toggleMode` 属性用于设置或获取用于展开或折叠指定 `jqxNavigationBar` 内容的用户交互。此属性接受三个可能的值，即 `"click"`、`"dblclick"` 和 `"none"`。
...
下面的示例说明了 `jQWidgets` `jqxNavigationBar` `toggleMode` 属性。在下面的例子中，`toggleMode` 属性的值被设置为 `"dblclick"`。
