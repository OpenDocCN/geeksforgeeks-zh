# jQWidgets jqxGrid showgroupsheader 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxgrid-showgroupsheader-property/](https://www.geeksforgeeks.org/jqwidgets-jqxgrid-showgroupsheader-property/)

**jQWidgets** 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大、优化、独立于平台并且得到广泛支持的框架。**jqxGrid** 用于说明以表格形式显示数据的 jQuery 小部件。此外，它完全支持与数据的连接，以及分页、分组、排序、过滤和编辑。

`showgroupsheader` 属性用于在显示的 `jqxGrid` 中显示或隐藏组的标题。它属于布尔类型，默认值为 `true`。

### 语法

*   设置 `showgroupsheader` 属性。

```html
$('#Selector').jqxGrid({ showgroupsheader: false});
```

*   返回 `showgroupsheader` 属性。

```html
var showgroupsheader = $('#Selector').jqxGrid('showgroupsheader');
```
...
以下示例说明了 jQWidgets 中的 jqxGrid `showgroupsheader` 属性。
