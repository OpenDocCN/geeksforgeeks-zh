# jQWidgets jqxScheduler 高度属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-height-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-height-property/)
...
`高度属性`用于设置或返回显示的 `jqxScheduler` 的高度。它是数字或字符串类型，默认值为 600。
...
```html
$("#Selector").jqxScheduler({ height: 800 });
```

*   返回高度属性。

```html
var height = $('#Selector').jqxScheduler('height');
```
...
**示例：** 下面的示例说明了 `jqxScheduler` 在 jQWidgets 中的高度属性。
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxScheduler height property
        </h3>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#jqxBtn").on("click", function () {
                var h = $('#jqxs').jqxScheduler('height');
                $('#log').text("Height: " + h);
            });
        });
    </script>
...
![](img/2e6dec2a3c6ac81c767239a60a6a8c45.png)
