# jQWidgets jqxMenu 缓动属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxmenu-easing-property/](https://www.geeksforgeeks.org/jqwidgets-jqxmenu-easing-property/)
...
缓动属性用于将动画的缓动设置或返回到 jQuery 支持的缓动之一。它接受字符串类型的值，默认值为 `"easeInOutSine"`。
...
```javascript
$('selector').jqxMenu({ easing: String });
```

获取缓动属性。

```javascript
var easing = $('selector').jqxMenu('easing');
```
...
下面的例子说明了 jQWidgets `jqxMenu` 缓动属性。
...
<body>
    ...
    <h3>
        jQWidgets jqxMenu easing Property
    </h3>
...
![](img/994f8c10787c14d93b3ca8d54352ed39.png)
