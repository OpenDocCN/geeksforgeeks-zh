# jQWidgets jqxBarGauge formatFunction 属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxbargauge-formatfunction-property/](https://www.geeksforgeeks.org/jqwidgets-jqxbargauge-formatfunction-property/)

jQWidgets 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大和优化的框架，独立于平台，并得到广泛支持。`jqxBarGauge` 表示一个 jQuery 条形图小部件，它为给定的值绘制一个条形图。

`formatFunction` 属性设置或返回条形图的 `formatFunction`。它用于对特定的段进行更改。

**语法：**

```javascript
$('.selector').jqxBarGauge({
  values: [array],
  formatFunction: {
  }
});
```

**链接文件：** 从 [https://www.jqwidgets.com/download/](https://www.jqwidgets.com/download/) 链接下载 jQWidgets。在 HTML 文件中，找到下载文件夹中的脚本文件：

```html
<link rel="stylesheet" href="jqwidgets/styles/jqx.base.css" type="text/css">
<script type="text/javascript" src="scripts/jquery-1.11.1.min.js"></script>
<script type="text/javascript" src="jqwidgets/jqxcore.js"></script>
<script type="text/javascript" src="jqwidgets/jqxdraw.js"></script>
```

以下示例说明了 jQWidgets 中的 `jqxBarGauge` `formatFunction` 属性：

## 示例

### HTML

```html
<!DOCTYPE html>
<html lang="en">

<head>
    <link rel="stylesheet" href=
        "jqwidgets/styles/jqx.base.css" type="text/css" />
    <script type="text/javascript" src="scripts/jquery-1.11.1.min.js">
    </script>
    <script type="text/javascript" src="jqwidgets/jqxcore.js">
    </script>
    <script type="text/javascript" src="jqwidgets/jqxdraw.js">
    </script>
    <script type="text/javascript" src="jqwidgets/jqxbargauge.js">
    </script>
</head>

<body>
    <center>
        <h1 style="color: green;">
            GeeksforGeeks
        </h1>

        <h3>
            jQWidgets jqxBarGauge formatFunction Property
        </h3>

        <div id="gfg"></div>
    </center>

    <script type="text/javascript">
        $(document).ready(function () {
            $('#gfg').jqxBarGauge({
                values: [100, 150, 200],
                max: 200,
                labels: {
                    connectorColor: 'green',
                    font: {
                        color: 'green',
                        size: 10,
                    },
                    formatFunction: function (value, index) {
                        return value;
                    },
                }
            });
        });
    </script>
</body>

</html>
```

**输出：**

![](img/0e9f94bc89cb6f07c8f82d6ff3129a93.png)

**参考：** [https://www.jqwidgets.com/jquery-widgets-documentation/documentation/jqxbargauge/jquery-bar-gauge-api.htm](https://www.jqwidgets.com/jquery-widgets-documentation/documentation/jqxbargauge/jquery-bar-gauge-api.htm)