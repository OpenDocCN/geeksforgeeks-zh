...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdragdrop-ondragstart-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdragdrop-ondragstart-property/)
...
`onDragStart` 属性用于设置或返回拖动开始时正在执行的回调。接受函数类型值，默认值为 `null`。
...
以下示例说明了 jQWidgets 中的 `jqxDragDrop` `onDragStart` 属性。
