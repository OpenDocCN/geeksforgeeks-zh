# jQWidgets jqxExpander width 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxexpander-width-property/](https://www.geeksforgeeks.org/jqwidgets-jqxexpander-width-property/)
...
`width` 属性用于设置或返回扩展器小部件的宽度。它接受数字/字符串类型的值，默认值为 `"auto"`。
...
以下示例说明了 jQWidgets `jqxExpander` `width` 属性。
