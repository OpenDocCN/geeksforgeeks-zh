...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxtoolbar-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtoolbar-disabled-property/)
...
`disabled` 属性用于启用或禁用显示的 `jqxToolBar`，包括每个工具。它是布尔类型，默认值是 `false`。
...
下面的例子说明了 `jQWidgets` 中的 `jqxToolBar` `disabled` 属性。
