...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtimepicker-readonly-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtimepicker-readonly-property/)
...
`readonly` 属性用于禁用用户与元素的交互。它接受布尔类型值，默认值为 `false`。
...
下面的例子说明了 jQWidgets `jqxTimePicker` `readonly` 属性。
