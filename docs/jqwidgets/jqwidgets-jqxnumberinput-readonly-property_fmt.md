# jQWidgets jqxNumberInput readOnly 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-readonly-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-readonly-property/)
...
`readOnly` 属性用于设置或返回输入小部件的只读状态。它接受布尔类型值，默认值为 `false`。
...
下面的例子说明了 jQWidgets jqxNumberInput `readOnly` 属性。
