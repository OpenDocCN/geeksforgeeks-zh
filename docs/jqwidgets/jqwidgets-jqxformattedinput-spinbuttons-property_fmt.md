# jQWidgets jqxFormattedInput spinButtons 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-spinbuttons-property/](https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-spinbuttons-property/)
...
`spinButtons` 属性用于显示或隐藏旋转按钮。要添加旋转按钮，我们需要在初始化中清空 `<div>` 元素。它接受布尔类型值，默认值为 `true`。
...
下面的例子说明了 jQWidgets `jqxFormattedInput` `spinButtons` 属性。
