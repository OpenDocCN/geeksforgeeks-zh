# jQWidgets jqxSlider 禁用属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxslider-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxslider-disabled-property/)
...
`disabled` 属性用于设置或返回滑块选项是否被禁用。它接受布尔类型值，默认值为 `false`。
...
```javascript
$('selector').jqxSlider({ disabled: Boolean });
```

返回 `disabled` 属性。

```javascript
var disabled = $('selector').jqxSlider('disabled');
```
...
下面的例子说明了 jQWidgets `jqxSlider` 的 `disabled` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxSlider disabled Property
        </h3>
        <div id='content'>
...
![](img/9c37d5471a96eeb21ad4675fcc90e7b3.png)
