...
`bounceEnabled` 属性用于设置或返回页面改变时是否启用弹跳效果。它接受布尔类型值，默认值为 `true`。
...
```javascript
var bounceEnabled = $('selector').jqxScrollView('bounceEnabled');
```
...
**示例：** 以下示例说明了 jQWidgets 中的 `jqxScrollView` `bounceEnabled` 属性。
