# jQWidgets jqxSlider 工具提示属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxslider-tooltip-property/](https://www.geeksforgeeks.org/jqwidgets-jqxslider-tooltip-property/)
...
`tooltip` 属性用于设置或返回滑块工具提示是否显示。它接受布尔类型值，默认值为 `false`。
...
```html
$('selector').jqxSlider({ tooltip: Boolean });
```

返回 `tooltip` 属性。

```javascript
var tooltip = $('selector').jqxSlider('tooltip');
```
...
下面的例子说明了 jQWidgets `jqxSlider` `tooltip` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxSlider tooltip Property
        </h3>
        <div id='content'>
...
![](img/cee25d3ba1f5138f956c1e7af80941b9.png)
