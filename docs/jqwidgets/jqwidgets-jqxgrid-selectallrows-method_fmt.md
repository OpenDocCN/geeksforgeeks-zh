...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxgrid-selectallrows-method/](https://www.geeksforgeeks.org/jqwidgets-jqxgrid-selectallrows-method/)

`jQWidgets` 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大、优化、独立于平台并且得到广泛支持的框架。`jqxGrid` 用于说明以表格形式显示数据的 jQuery 小部件。此外，它完全支持与数据的连接，以及分页、分组、排序、过滤和编辑。

`selectallrows()`方法用于选择显示的 `jqxGrid` 的每一行。其中，选择模式是多行或多行扩展。它没有参数，也不返回任何内容。

## 语法

```javascript
$('#Selector').jqxGrid('selectallrows');
```
...
下面的例子说明了 `jQWidgets` 中的 `jqxGrid` `selectallrows()`方法。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxGrid selectallrows() method
        </h3>
        <br />
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#jqxBtn").on("click", function () {
                $('#jqxg').jqxGrid('selectallrows');
                $("#log").text("Every single row is selected!");
            });
        });
    </script>
</body>
...
![](img/0f48d1e7b7cfae8833f695e8eb340fcb.png)
