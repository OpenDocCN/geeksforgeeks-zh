...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxknob-dragstartangle-property/](https://www.geeksforgeeks.org/jqwidgets-jqxknob-dragstartangle-property/)
...
`dragStartAngle` 属性用于设置或获取指针拖动开始的指定 `jqxKnob` 的度数。

**语法:**

*   用于设置 `dragStartAngle` 属性。

```html
$('#jqxKnob').jqxKnob({dragStartAngle: 60 });
```

*   获取 `dragStartAngle` 属性。

```html
var allowValueChangeOnClick = $('#jqxKnob').jqxKnob('dragStartAngle');
```
...
**示例:** 下面的示例说明了 jQWidgets `jqxKnob` `dragStartAngle` 属性。在以下示例中，`dragStartAngle` 属性的值已设置为 60。
