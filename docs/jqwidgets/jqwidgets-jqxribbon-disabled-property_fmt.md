# jQWidgets jqxRibbon 禁用属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxribbon-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxribbon-disabled-property/)
...
`disabled` 属性用于启用或禁用功能区。它接受布尔类型值，默认值为 `false`。
...
```javascript
$('selector').jqxRibbon({ disabled: Boolean });
```

返回 `disabled` 属性:

```javascript
var disabled = $('selector').jqxRibbon('disabled');
```
...
以下示例说明了 jQWidgets jqxRibbon `disabled` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxRibbon disabled Property
        </h3>
    </center>
...
![](img/259b3cde5564867e1674a35b46fcdc0d.png)
