...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-setcontent-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-setcontent-method/)
...
`setContent()` 方法用于设置下拉列表的内容。它接受字符串类型的单参数 `content`，不返回值。

## 语法

```html
$("Selector").jqxDropDownList('setContent', 'content');
```
...
下面的例子说明了 jQWidgets 中的 `jqxDropDownList` `setContent()` 方法。
...
![](img/5e61fdb82177090a9fe5dcfd423fafbb.png)
