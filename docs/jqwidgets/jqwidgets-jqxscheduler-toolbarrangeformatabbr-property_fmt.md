...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-toolbarrangeformatabbr-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-toolbarrangeformatabbr-property/)
...
每当显示的 `jqxScheduler` 的宽度不足以显示完整的格式时，`toolBarRangeFormatAbbr` 属性用于设置或返回工具栏的日期格式。类型为字符串，默认值为“*日/月/年*”。

## 语法

设置 `toolBarRangeFormatAbbr` 属性。

```html
$("Selector").jqxScheduler({ toolBarRangeFormatAbbr: 'dd MM yyyy' });
```

返回 `toolBarRangeFormatAbbr` 属性。
...
下面的示例说明了 jQWidgets 中的 `jqxScheduler` `toolBarRangeFormatAbbr` 属性。
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxScheduler toolBarRangeFormatAbbr property</h3>
...
![](img/3c492d8538f35b85fbe88f6e76cb550c.png)
