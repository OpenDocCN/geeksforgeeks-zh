# jQWidgets `onError` 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxvalidator-onerror-property/](https://www.geeksforgeeks.org/jqwidgets-jqxvalidator-onerror-property/)
...
`onError` 属性用于设置或获取回调函数，该函数将在验证任何错误时调用。
...
下面的示例说明了 jQWidgets `jqxValidator` 的 `onError` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxValidator onError Property
        </h3>
        ...
        <input type="button" style="margin:28px;" 
               id="button_for_onError" 
               value="Value of the onError property"/>
        <div id="log"></div>
...
$(document).ready(function () {
    $('#Employee_Form').jqxValidator({
        onError: function () {
            $("#log").html('The form has been filled incorrectly.');
        },
        Rules: [
            {
                input: '#Employee_Name',
    ...
    $("#button_for_onError").jqxButton({
        width: 300
    });
    $("#button_for_onError").click(
        function () {
...
![](img/0fb574601e03b3fffb28771cb1280312.png)
