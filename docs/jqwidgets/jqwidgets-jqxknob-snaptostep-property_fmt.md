# jQWidgets jqxKnob snapToStep 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxknob-snaptostep-property/](https://www.geeksforgeeks.org/jqwidgets-jqxknob-snaptostep-property/)
...
`snapToStep` 属性用于设置或获取指定 `jqxKnob` 的旋钮值是否捕捉最近的步长。其可能值为 `true` 或 `false`。
...
```javascript
var allowValueChangeOnClick = $('#jqxKnob').jqxKnob('snapToStep');
```
...
下面的示例说明了 jQWidgets `jqxKnob` 的 `snapToStep` 属性。在以下示例中， `snapToStep` 属性的值已设置为 `true`。
