...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxscrollview-slideshow-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscrollview-slideshow-property/)
...
`slideShow` 属性用于设置或返回幻灯片模式是否启用。该模式用于在一个时间间隔内自动改变页面。它接受布尔类型值，默认值为真。
...
下面的例子说明了 jQWidgets 中的 `jqxScrollView` `slideShow` 属性。
