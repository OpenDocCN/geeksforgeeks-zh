# jQWidgets jqxRangeSelector 高度属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxrangeselector-height-property/](https://www.geeksforgeeks.org/jqwidgets-jqxrangeselector-height-property/)
...
`height` 属性用于设置或返回范围选择器的高度。它接受数字/字符串类型的值，默认值为 `100`。
...
```javascript
$('selector').jqxRangeSelector({ height: Number/String });
```

返回 `height` 属性。

```javascript
var height = $('selector').jqxRangeSelector('height');
```
...
下面的例子说明了 jQWidgets `jqxRangeSelector` `height` 属性。
...
<body>
    ...
    <h3>
        jQWidgets jqxRangeSelector height Property
    </h3>
...
![](img/4ec19853b0c66ceed07f130bce4b0524.png)
