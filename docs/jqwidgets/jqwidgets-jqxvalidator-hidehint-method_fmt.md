...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxvalidator-hidehint-method/](https://www.geeksforgeeks.org/jqwidgets-jqxvalidator-hidehint-method/)
...
*   `input`: 这是隐藏提示的特定输入的 id 值。它是字符串类型。
...
下面的示例说明了 jQWidgets `jqxValidator` `hideHint()` 方法。在下面的例子中，最后两个提示被隐藏。
