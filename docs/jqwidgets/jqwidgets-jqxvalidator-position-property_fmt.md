# jQWidgets jqxValidator position 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxvalidator-position-property/](https://www.geeksforgeeks.org/jqwidgets-jqxvalidator-position-property/)
...
`position` 属性用于设置或获取指定 `jqxValidator` 提示的默认位置。
...
以下示例说明了 `jQWidgets` 的 `position` 属性。在下面的例子中，`position` 属性的值被设置为 `'left'`。
