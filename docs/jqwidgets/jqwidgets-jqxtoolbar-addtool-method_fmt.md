# jqxToolBar addTool() 方法

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxtoolbar-addtool-method/](https://www.geeksforgeeks.org/jqwidgets-jqxtoolbar-addtool-method/)
...
`addTool()` 方法用于向显示的 `jqxToolBar` 添加工具。它不返回任何东西。
...
*   **type**：是新工具的规定类型。其中，可能的值可以是 `button`、`dropdownlist`、`toggleButton`、`combobox`、`input` 以及 `custom`。它属于字符串类型。
*   **position**：是规定的工具插入位置。这里可能的值是 `first` 和 `last`。它属于字符串类型。
*   **separator**：表示新工具后是否包含分隔符。这里可能的值有 `true` 和 `false`。它属于布尔类型。
*   **initCallback**：是新工具初始化后要调用的规定回调函数。它属于函数类型。这里可以传递三个参数，如下所示：
    1.  **type**：是所述工具的类型。
    2.  **tool**：它是一个表示工具的声明的 jQuery 对象。
    3.  **menuToolIninitialization**：它说明是为工具栏工具调用回调函数，即 `false`，还是为弹出菜单工具调用回调函数，即 `true`。它属于布尔类型。
...
[https://www.jqwidgets.com/jquery-widgets-documentation/documentation/jqxtoolbar/jquery-toolbar-api.htm?search=](https://www.jqwidgets.com/jquery-widgets-documentation/documentation/jqxtoolbar/jquery-toolbar-api.htm?search=)