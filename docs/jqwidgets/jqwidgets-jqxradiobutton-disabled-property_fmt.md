...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxradiobutton-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxradiobutton-disabled-property/)
...
`disabled` 属性用于设置或返回单选按钮是否禁用。它接受布尔类型值，默认值为 `false`。
...
以下示例说明了 jQWidgets `jqxRadioButton` `disabled` 属性。
