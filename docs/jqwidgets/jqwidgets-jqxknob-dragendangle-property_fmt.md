...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxknob-dragendangle-property/](https://www.geeksforgeeks.org/jqwidgets-jqxknob-dragendangle-property/)
...
`dragEndAngle` 属性用于设置或获取指定的 `jqxKnob` 的角度，指针的拖动将在该角度结束。

## 语法

*   用于设置 `dragEndAngle` 属性。
...
```javascript
var allowValueChangeOnClick = $('#jqxKnob').jqxKnob('dragEndAngle');
```
...
以下示例说明了 jQWidgets `jqxKnob` 的 `dragEndAngle` 属性。在下面的示例中，`dragEndAngle` 属性的值已设置为 300。
