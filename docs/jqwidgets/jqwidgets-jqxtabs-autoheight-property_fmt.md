# jQWidgets jqxTabs 自动高度属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtabs-autoheight-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtabs-autoheight-property/)
...
**自动高度属性**用于设置或返回 `jqxTabs` 标题的高度，该高度将等于具有最大高度的项目。它接受字符串类型值，默认值为 `true`。
...
```javascript
$('selector').jqxTabs({ autoHeight: Boolean });
```

返回自动高度属性。

```javascript
var theme = $('selector').jqxTabs('autoHeight');
```
...
下面的例子说明了 jQWidgets `jqxTabs` `autoHeight` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxTabs autoHeight Property
        </h3>
...
![](img/742b0b242f0529421cb4248add478e28.png)
