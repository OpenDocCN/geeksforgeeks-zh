...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxgrid-iscolumngroupable-method/](https://www.geeksforgeeks.org/jqwidgets-jqxgrid-iscolumngroupable-method/)

**jQWidgets** 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大、优化、独立于平台并且得到广泛支持的框架。`jqxGrid` 用于说明以表格形式显示数据的 jQuery 小部件。此外，它完全支持与数据的连接，以及分页、分组、排序、过滤和编辑。

`isColumnGroupable()` 方法用于检查用户是否可以通过列进行分组。它有一个名为 `dataField` 的参数，类型为字符串。它返回一个布尔值。

## 语法

```javascript
var groupable = $('#Selector').jqxGrid('iscolumngroupable', 'dataField');
```
...
下面的例子说明了 jQWidgets 中的 `jqxGrid` `isColumnGroupable()` 方法。
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxGrid `isColumnGroupable()` method
        </h3>
        <br />
        <div id="jqxg"></div>
        <div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#jqxBtn").on("click", function () {
                var icg = $('#jqxg').jqxGrid('iscolumngroupable', 'subjectnames');
                $('#log').html("Column groupable: " + icg);
            });
        });
    </script>
</body>
...
![](img/33bced4bcd5bb97f03a8f215756034ca.png)
