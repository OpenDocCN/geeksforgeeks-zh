...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxchart-tooltipmoveduration-property/](https://www.geeksforgeeks.org/jqwidgets-jqxchart-tooltipmoveduration-property/)
...
*   设置 `toolTipMoveDuration` 属性。

```javascript
$('Selector').jqxChart({ toolTipMoveDuration : number});
```

*   返回 `toolTipMoveDuration` 属性。

```javascript
var toolTipMoveDuration = $('Selector').jqxChart('toolTipMoveDuration');
```
...
以下示例说明了 jQWidgets 中的 `jqxChart` `toolTipMoveDuration` 属性。
