# jQWidgets jqxTabs 动画类型属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtabs-animationtype-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtabs-animationtype-property/)
...
动画类型属性用于设置或返回切换选项卡的动画类型。它接受字符串类型值，默认值为“无”。
...
```javascript
$('selector').jqxTabs({ animationType: String });
```

返回动画类型属性。

```javascript
var animationType = $('selector').jqxTabs('animationType');
```
...
下面的例子说明了 jQWidgets `jqxTabs` `animationType` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxTabs animationType Property
        </h3>
...
![](img/f9297da4c8ab51c3e2e9513ab67fd76b.png)
