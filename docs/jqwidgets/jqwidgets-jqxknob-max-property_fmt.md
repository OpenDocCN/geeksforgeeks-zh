# jQWidgets jqxKnob max 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxknob-max-property/](https://www.geeksforgeeks.org/jqwidgets-jqxknob-max-property/)
...
`max` 属性用于设置或获取指定 `jqxKnob` 的最大值属性。
...
```html
$('#jqxKnob').jqxKnob({max: value });
```

*   获取 `max` 属性:

```html
var allowValueChangeOnClick = $('#jqxKnob').jqxKnob('max');
```
...
以下示例说明了 `jQWidgets` `jqxKnob` `max` 属性。在以下示例中，`max` 属性的值已设置为 50。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxKnob max Property
        </h3>
        <div id='jqx_Knob'>
        </div>
...
![](img/9ca74d5ac032b2c9a2c9cd75cb0abf48.png)
