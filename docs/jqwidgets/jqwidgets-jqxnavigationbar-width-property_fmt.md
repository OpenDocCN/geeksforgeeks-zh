...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxnavigationbar-width-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnavigationbar-width-property/)
...
`width` 属性用于设置或获取指定 `jqxNavigationBar` 的宽度。这个属性接受一些“自动”的值，或者像“Npx”这样的字符串，其中 N 是任何数字或者以像素为单位的数值。
...
下面的示例说明了 jQWidgets `jqxNavigationBar` `width` 属性。在以下示例中，`width` 属性的值已设置为 250。
