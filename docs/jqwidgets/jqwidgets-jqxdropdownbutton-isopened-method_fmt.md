...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownbutton-isopened-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownbutton-isopened-method/)
...
## isOpened() 方法
`isOpened()` 方法用于检查显示的下拉按钮是否打开。如果已经打开，则此方法返回 `true`，否则返回 `false`。它没有参数。它返回布尔值。
...
## 示例
下面的示例说明了 jQWidgets 中的 `jqxDropDownButton` `isOpened()` 方法。
