...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxlistbox-indeterminateindex-method/](https://www.geeksforgeeks.org/jqwidgets-jqxlistbox-indeterminateindex-method/)
...
每当 `checkboxes` 属性值设置为真时，`indeterminateIndex()` 方法用于确定所述列表中的项目。此外，这里使用的索引是基于零的，换句话说，为了不确定列表的第一项，必须与索引 `零` 一起调用 `indeterminateIndex` 方法。它不返回任何东西。
...
下面的示例说明了 `jQWidgets` 中的 `jqxListBox` `indeterminateIndex()` 方法。
