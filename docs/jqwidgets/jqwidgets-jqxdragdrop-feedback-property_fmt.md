...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdragdrop-feedback-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdragdrop-feedback-property/)
...
`feedback` 属性用于设置或返回反馈。它是用户实际拖动的对象。它接受字符串类型值，默认值为 `"clone"`。
...
以下示例说明了 jQWidgets 中的 `jqxDragDrop` `feedback` 属性。
