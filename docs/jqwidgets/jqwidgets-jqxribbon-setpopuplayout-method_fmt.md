...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxribbon-setpopuplayout-method/](https://www.geeksforgeeks.org/jqwidgets-jqxribbon-setpopuplayout-method/)
...
如果模式设置为“弹出”，则使用 `setPopupLayout()` 方法来设置项目内容的布局。

## 语法

```
$('#jqxRibbon').jqxRibbon('setPopupLayout', index, layout, width, height);
```
...
*   `index`: 表示索引编号。
*   `layout`: 描述内容的布局。它接受字符串类型的值，其可能的值是–“默认”、“近”、“远”、“中”。
*   `width`: 描述元素的宽度。
*   `height`: 描述元素的高度。
...
下面的例子说明了 `jQWidgets` `jqxRibbon` `setPopupLayout()` 方法。
