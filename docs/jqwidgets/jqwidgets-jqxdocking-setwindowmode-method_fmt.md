...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdocking-setwindowmode-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdocking-setwindowmode-method/)
...
`setWindowMode()` 方法用于将模式设置到指定的 `jqxDocking` 窗口。
...
*   `windowId`: 这是正在设置模式的窗口的标识。
*   `mode`: 这是指定模式。它的可能值是 `'default'` 或 `'floating'`。
...
下面的示例说明了 `jQWidgets` `jqxDocking` `setWindowMode()` 方法。在下面的例子中，窗口的模式是 `'floating'`，后来在调用 `setWindowMode()` 方法后，窗口的模式被更改为 `'default'`。
