# jqxKnob 小部件的 allowValueChangeOnDrag 属性

> 哎哎哎: https://www.geeksforgeeks.org/jqwidgets-jqxknob-allowvaluechangeondrag-property/
...
`allowValueChangeOnDrag` 属性用于设置或获取是否可以通过拖动指针来更改指定的 `jqxKnob` 的值。
...
以下示例说明了 jQWidgets `jqxKnob` 的 `allowValueChangeOnDrag` 属性。
