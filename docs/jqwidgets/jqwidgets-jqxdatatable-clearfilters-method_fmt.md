...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdatatable-clearfilters-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdatatable-clearfilters-method/)
...
`clearFilters()` 方法用于清除过滤器。此函数不接受任何参数。
...
下面的示例说明了 `jQWidgets` `clearFilters()` 方法。在本例中，已经有一个应用的过滤器将使用 `clearFilters()` 方法移除。
