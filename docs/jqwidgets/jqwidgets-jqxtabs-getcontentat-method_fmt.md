...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxtabs-getcontentat-method/](https://www.geeksforgeeks.org/jqwidgets-jqxtabs-getcontentat-method/)
...
`getContentAt()` 方法用于获取一个 Tab 的内容。它接受数字类型的单参数 `index`。索引从 0 开始。它返回字符串类型值（选项卡内容）。

## 语法

```javascript
$('Selector').jqxTabs('getContentAt', index);
```
...
下面的例子说明了 jQWidgets `jqxTabs` `getContentAt()` 方法。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxTabs getContentAt() Method
        </h3>
        ...
        <input id="jqxBtn" type="button" 
            value="Get Content of First Tab" 
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#jqxBtn').click(function () {
                var Content = $('#jqxTabs').jqxTabs('getContentAt', 0);
                alert("The Content of First Tab: " + Content.innerHTML);
            });
        });
    </script>
</body>
...
![](img/c4518502b0abed3e2067dc3fd667e6d1.png)
