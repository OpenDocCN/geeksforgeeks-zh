...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxvalidator-closeonclick-property/](https://www.geeksforgeeks.org/jqwidgets-jqxvalidator-closeonclick-property/)
...
`closeOnClick` 属性用于设置或获取指定 `jqxValidator` 的提示点击后是否关闭。该方法接受两个可能的值，即 `true` 和 `false`。

## 语法

用于设置 `closeOnClick` 属性。
...
从给定链接下载 [jQWidgets](https://www.jqwidgets.com/download/)。在 HTML 文件中，找到下载文件中的脚本文件。
...
下面的示例说明了 jQWidgets `closeOnClick` 属性。在下面的例子中，`closeOnClick` 属性的值被设置为 `true`。
...
<body>
    <center>
        ...
        <input type="button" style="margin:28px;" id="button_for_closeOnClick" value="Value of the closeOnClick property" />
        <div id="log"></div>
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                $("#button_for_closeOnClick").click(
                    function () {
                        var Value_of_closeOnClick = $('#Employee_Form').jqxValidator('closeOnClick');
                        $("#log").html(JSON.stringify(Value_of_closeOnClick));
                    });
            });
        </script>
    </center>
