# jQWidgets jqxRangeSelector 宽度属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxrangeselector-width-property/](https://www.geeksforgeeks.org/jqwidgets-jqxrangeselector-width-property/)
...
`宽度属性`用于设置或返回 `jqxRangeSelector` 小部件的宽度。它接受数字/字符串类型的值，默认值为 `400`。
...
```html
$('selector').jqxRangeSelector({ width: Number/String });
```

返回 `width` 属性。

```html
var width = $('selector').jqxRangeSelector('width');
```
...
下面的例子说明了 jQWidgets `jqxRangeSelector` `宽度属性`。
...
![](img/07d8a43b7150addbb38988b808a21ed3.png)
