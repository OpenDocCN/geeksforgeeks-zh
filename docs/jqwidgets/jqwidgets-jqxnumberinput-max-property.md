# jqwidgets jqxnombinput max property

> 原文:[https://www . geesforgeks . org/jqwidgets-jqxnumberinput-max-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-max-property/)
...
**max 属性**用于设置或返回 jqxNumberInput 小部件的最大值。它接受数字类型值，默认值为 99999999。
...
```html
$('selector').jqxNumberInput({ max: Number });
```

返回 max 属性。

```html
var max = $('selector').jqxNumberInput('max');
```
...
下面的例子说明了 jQWidgets jqxNumberInput max 属性。
...
![](img/00f62e2e28d42638c3915a0f6a06af0a.png)
