...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-source-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-source-property/)
...
`source` 属性用于设置或返回项目的来源。它接受数组类型值，默认值为空。
...
以下示例说明了 `jQWidgets` 中的 `jqxDropDownList` `source` 属性。
