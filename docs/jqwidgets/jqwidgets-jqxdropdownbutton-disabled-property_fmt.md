# jQWidgets jqxDropDownButton disabled 属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxdropdownbutton-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownbutton-disabled-property/)
...
`disabled` 属性用于启用或禁用显示的下拉按钮。它是布尔类型，默认值为 `false`。
...
**示例：** 下面的示例说明了 jQWidgets 中的 `jqxDropDownButton` `disabled` 属性。
