...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-selectfirst-method/](https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-selectfirst-method/)
...
`selectFirst()` 方法用于在输入字段的数字开头添加插入符号。它不接受任何参数，也不返回值。
...
下面的例子说明了 `jQWidgets` `jqxFormattedInput` `selectFirst()` 方法。
