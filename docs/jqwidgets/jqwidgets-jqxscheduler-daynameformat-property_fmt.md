# jQWidgets jqxScheduler dayNameFormat 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-daynameformat-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-daynameformat-property/)
...
`dayNameFormat` 属性用于设置或返回显示的 `jqxScheduler` 的列的日格式是否被禁用。类型为字符串，默认值为 `full`。
...
```html
var dayNameFormat = $('#Selector').jqxScheduler('dayNameFormat');
```
