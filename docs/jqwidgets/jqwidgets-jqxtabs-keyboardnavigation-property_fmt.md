# jQWidgets jqxTabs 键盘导航属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxtabs-keyboardnavigation-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtabs-keyboardnavigation-property/)
...
`keyboardNavigation` 属性用于启用或禁用键盘导航。它接受布尔类型值，默认值为 `true`。
...
```javascript
$('selector').jqxTabs({ keyboardNavigation: Boolean });
```

返回键盘导航属性。

```javascript
var keybNav = $('selector').jqxTabs('keyboardNavigation');
```
...
下面的例子说明了 jQWidgets `jqxTabs` `keyboardNavigation` 属性。
...
![](img/a49e60370ed43355c9335f2bfa13b10d.png)
