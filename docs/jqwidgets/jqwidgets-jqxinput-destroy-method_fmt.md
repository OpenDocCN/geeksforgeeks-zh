# jQWidgets jqxInput 销毁()方法

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxinput-destroy-method/](https://www.geeksforgeeks.org/jqwidgets-jqxinput-destroy-method/)

jQWidgets 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大、优化、独立于平台并且得到广泛支持的框架。`jqxInput` 用于表示包含自动完成功能的 jQuery 输入小部件。

## 销毁()方法

`销毁()`方法用于销毁 `jqxInput` 小部件。它不接受任何参数，也不返回值。

## 语法

```html
$('selector').jqxInput('destroy'); 
```
...
下面的例子说明了 jQWidgets 中的 `jqxInput` `destroy()`方法。
...
<body class='default'>
    <center>
        ...
        <h3>
            jQWidgets jqxInput destroy() Method
        </h3>
        <br>
...
![](img/12ba04e79ed62376f6250a692c24e4b6.png)
