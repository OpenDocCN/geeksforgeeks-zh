...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxchart-padding-property/](https://www.geeksforgeeks.org/jqwidgets-jqxchart-padding-property/)
...
`padding` 属性用于设置或返回填充属性。即该属性用于设置图表的左、上、右&底部填充。它接受填充类型值，默认值为 `{ left: 5, top: 5, right: 5, bottom: 5 }`。
...
以下示例说明了 jqxChart 在 jQWidgets 中的 `padding` 属性。
