...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxnavbar-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnavbar-disabled-property/)
...
`disabled` 属性用于启用/禁用 navbar 小部件。它接受布尔类型值，默认值为 `false`。
...
下面的例子说明了 jQWidgets `jqxNavBar` `disabled` 属性。
