...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-checkitem-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-checkitem-method/)
...
当“复选框”属性值设置为 `true` 时，`checkItem()` 方法用于检查列表项。它接受字符串/对象类型的单个参数 `item`，并且不返回值。
...
下面的示例说明了 `jQWidgets` 中的 `jqxDropDownList` `checkItem()` 方法。
