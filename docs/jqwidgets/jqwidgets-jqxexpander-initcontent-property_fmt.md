...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxexpander-initcontent-property/](https://www.geeksforgeeks.org/jqwidgets-jqxexpander-initcontent-property/)
...
`initContent` 属性用于设置或返回需要初始化项目内容时正在调用的回调函数。接受函数类型值，默认值为 `null`。
...
以下示例说明了 jQWidgets `jqxExpander` 的 `initContent` 属性。
