...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxribbon-disableat-method/](https://www.geeksforgeeks.org/jqwidgets-jqxribbon-disableat-method/)
...
`disableAt()`方法用于禁用给定索引的项目。它接受数字类型的单参数`index`，不返回任何值。
...
下面的示例说明了 jQWidgets jqxRibbon `disableAt()`方法。
