...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxsplitter-splitbarsize-property/](https://www.geeksforgeeks.org/jqwidgets-jqxsplitter-splitbarsize-property/)
...
`splitBarSize` 属性用于设置或获取指定 `jqxSplitter` 的分割条大小。
...
下面的示例说明了 `jqxSplitter` 的 `splitBarSize` 属性。在下面的例子中，`splitBarSize` 属性的值被设置为 20。
