...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxgauge-lineargauge-height-property/](https://www.geeksforgeeks.org/jqwidgets-jqxgauge-lineargauge-height-property/)
...
`height` 属性用于设置或返回高度属性，即用于设置或返回 `jqxLinearGauge` 的高度。该属性只接受 `像素` 和 `百分比` 的大小。它接受数字类型值，默认值为 `300`。
...
以下示例说明了 `jQWidgets` 中的 `jqxlineargage` **高度**属性。
