# jQWidgets jqxFormattedInput 模板属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-template-property/](https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-template-property/)
...
`template` 属性用于设置或返回模板，作为默认样式的替代。它接受字符串类型值，其默认值为“默认值”。
...
下面的例子说明了 jQWidgets `jqxFormattedInput` `template` 属性。
