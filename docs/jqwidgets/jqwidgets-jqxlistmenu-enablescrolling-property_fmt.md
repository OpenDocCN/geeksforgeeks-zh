...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxlistmenu-enablescrolling-property/](https://www.geeksforgeeks.org/jqwidgets-jqxlistmenu-enablescrolling-property/)
...
`enableScrolling` 属性用于设置或返回项目溢出可见区域时是否自动出现垂直滚动条。为了使用这个属性，我们需要包含 `jqxpanel.js` 和 `jqxscrollbar.js` 文件。它接受布尔类型值，默认值为真。

## 语法

设置 `enableScrolling` 属性。

```html
$('selector').jqxListMenu({ enableScrolling: Boolean });
```

返回 `enableScrolling` 属性。
