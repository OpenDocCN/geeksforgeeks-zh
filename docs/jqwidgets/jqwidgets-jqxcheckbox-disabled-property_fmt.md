...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxcheckbox-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxcheckbox-disabled-property/)
...
`disabled` 属性用于设置或返回禁用的复选框。它接受布尔类型值，默认值为 `false`。
...
以下示例说明了 jQWidgets 中 `jqxCheckBox` 的 `disabled` 属性。
