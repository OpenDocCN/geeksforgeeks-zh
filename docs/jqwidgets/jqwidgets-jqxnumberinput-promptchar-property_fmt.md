# jQWidgets jqxNumberInput promptChar 属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-promptchar-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-promptchar-property/)
...
`promptChar` 属性用于设置或返回可编辑字符为空时显示的提示字符。它接受字符类型值，默认值为“_”。
...
```javascript
$('selector').jqxNumberInput({ promptChar: Char });
```

返回 `promptChar` 属性。

```javascript
var promptChar = $('selector').jqxNumberInput('promptChar');
```
...
下面的例子说明了 jQWidgets jqxNumberInput `promptChar` 属性。
...
![](img/cefeb7a6ff0211eab4bd98755fcd5e64.png)
