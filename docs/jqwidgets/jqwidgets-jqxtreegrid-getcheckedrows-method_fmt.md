...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtreegrid-getcheckedrows-method/](https://www.geeksforgeeks.org/jqwidgets-jqxtreegrid-getcheckedrows-method/)
...
*   `checked`: 这是布尔值。这里，返回该行的选中状态。
*   `expanded`: 这是布尔值。这里，返回行的展开状态。
*   `icon`: 这是字符串值。这里，返回该行的图标网址。
*   `leaf`: 这是布尔值。这里，返回层次结构中行的叶的存在。
*   `level`: 这是整数值。这里，返回行的层次级别。
*   `parent`: 这是对象。它为根行返回 `null`，否则返回父行的对象。
*   `records`: 这是一个数组。这里，返回该行的子行集合。
*   `selected`: 这是布尔值。这里，返回行的选定状态。
...
下面的示例说明了 `jQWidgets` `jqxTreeGrid` `getCheckedRows()` 方法。在下面的示例中，返回选中的第一行和第三行。
