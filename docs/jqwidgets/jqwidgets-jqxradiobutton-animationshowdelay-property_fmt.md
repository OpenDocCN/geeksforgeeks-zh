...
## animationShowDelay 属性

`animationShowDelay` 属性用于设置或返回将要检查 `jqxRadioButton` 时动画的时间延迟。它接受数字类型值，默认值为 250。
...
下面的例子说明了 jQWidgets `jqxRadioButton` `animationShowDelay` 属性。
