...
`addItem()`方法用于向所述列表添加新项目。此外，如果附加了新项目，则返回`真`，否则返回`假`，如果没有附加项目，则返回。它有一个名为`item`的参数，类型为`object`或`string`，并且不返回任何内容。
...
下面的示例说明了 jQWidgets 中的 `jqxListBox` `addItem()` 方法。
...
[https://www . jqwidgets . com/jquery-widgets-documentation/documentation/jqxlistbox/jquery-listbox-API . htm？搜索=](https://www.jqwidgets.com/jquery-widgets-documentation/documentation/jqxlistbox/jquery-listbox-api.htm?search=)