# jQWidgets jqxLoader 文本属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxloader-text-property/](https://www.geeksforgeeks.org/jqwidgets-jqxloader-text-property/)
...
`text` 属性用于设置或返回加载器元素的文本。它接受字符串类型的值，默认值是“加载...”。
...
```javascript
$('selector').jqxLoader({ text: String });
```

返回 `text` 属性。

```javascript
var text = $('selector').jqxLoader('text');
```
...
下面的例子说明了 jQWidgets `jqxLoader` `text` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxLoader text Property
        </h3>
...
![](img/e96ecaea2fe24f55ab823ddd4d799831.png)
