...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxsplitter-destroy-method/](https://www.geeksforgeeks.org/jqwidgets-jqxsplitter-destroy-method/)
...
`destroy()` 方法用于销毁指定的 `jqxSplitter`。该方法将从 DOM 中移除 `jqxSplitter`，还将移除所有内部事件处理程序和样式。
...
下面的示例说明了 jQWidgets jqxSplitter `destroy()` 的方法。
