...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-template-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-template-property/)
...
`template` 属性用于设置或返回模板作为默认样式的替代。它接受字符串类型值，其默认值为“默认值”。

它的可能值如下。

*   `'default'`: 是默认模板，其样式取决于 `theme` 属性值。
...
以下示例说明了 jQWidgets 中的 `jqxDropDownList` `template` 属性。
