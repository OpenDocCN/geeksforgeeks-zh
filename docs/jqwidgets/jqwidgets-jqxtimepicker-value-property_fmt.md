...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtimepicker-value-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtimepicker-value-property/)
...
`value` 属性用于设置或返回 `jqxTimePicker` 小部件的值。它接受日期类型值，默认值为 `new Date()`。
...
## 示例
以下示例说明了 `jQWidgets` `jqxTimePicker` `value` 属性。
