...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-render-method/](https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-render-method/)
...
`render()`方法用于渲染 `jqxFormattedInput` 小部件。它不接受任何参数，也不返回值。
...
下面的例子说明了 jQWidgets `jqxFormattedInput` `render()`方法。
