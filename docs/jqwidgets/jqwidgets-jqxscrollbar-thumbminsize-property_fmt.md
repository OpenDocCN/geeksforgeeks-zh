...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxscrollbar-thumbminsize-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscrollbar-thumbminsize-property/)
...
`thumbMinSize` 属性用于为指定的 `jqxScrollBar` 设置或获取滚动条缩略图的最小尺寸。
...
下面的示例说明了 jQWidgets `jqxScrollBar` 的 `thumbMinSize` 属性。在下面的例子中，`thumbMinSize` 属性的值被设置为 10。
