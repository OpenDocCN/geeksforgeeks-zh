...
> 原文:[https://www . geeksforgeeks . org/jqwidgets-jqxmenu-animationshowdelay-property/](https://www.geeksforgeeks.org/jqwidgets-jqxmenu-animationshowdelay-property/)
...
**animationShowDelay 属性**用于设置或返回动画开始前的延迟。它接受数字类型值，默认值为 200。
...
```html
$('selector').jqxMenu({ animationShowDelay: Number });
```

返回 animationShowDelay 属性。

```html
var delay = $('selector').jqxMenu('animationShowDelay');
```
...
下面的例子说明了 jQWidgets jqxMenu animationshoulday 属性。
...
    <h3>
        jQWidgets jqxMenu animationShowDelay Property
    </h3>
...
![](img/43eef81c8030bad2876fd7cc1ddb8609.png)
