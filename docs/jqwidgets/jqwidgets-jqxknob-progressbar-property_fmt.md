...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxknob-progressbar-property/](https://www.geeksforgeeks.org/jqwidgets-jqxknob-progressbar-property/)
...
*   `offset`: 用于指定进度条的偏移量。
*   `style`: 用于指定进度条的样式。
*   `background`: 用于指定进度条的背景样式。
*   `size`: 用于指定进度条的大小。
*   `range`: 这是一个指定进度条范围的数组。
...
**示例:** 以下示例说明了 jQWidgets `jqxKnob` `progressBar` 属性。
