...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxfileupload-refresh-method/](https://www.geeksforgeeks.org/jqwidgets-jqxfileupload-refresh-method/)
...
`refresh()`方法用于刷新小部件。即，它用于设置明确使用另一个按钮来刷新文件的按钮。它不接受任何参数，也不返回值。
...
## 示例:
下面的示例说明了 `jQWidgets` 中的 `jqxFileUpload` `refresh()`方法。
