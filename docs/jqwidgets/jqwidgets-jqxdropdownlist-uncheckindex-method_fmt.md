...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-uncheckindex-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-uncheckindex-method/)
...
当 `checkboxes` 属性值设置为 `true` 时，`uncheckIndex()` 方法用于取消选中列表项。它接受数字类型的单参数 `index`，不返回值。
