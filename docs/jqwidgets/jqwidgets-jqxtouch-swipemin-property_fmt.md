# jQWidgets jqxttouch swipeMin 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtouch-swipemin-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtouch-swipemin-property/)
...
`swipeMin` 属性用于表示指定 `jqxTouch` 小部件的插件所需的最小滑动距离。
...
## 示例
下面的示例说明了 `jQWidgets` `jqxttouch` 的 `swipeMin` 属性。在下面的示例中，`swipeMin` 属性的值被设置为 45。
