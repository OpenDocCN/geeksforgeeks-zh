# jQWidgets jqxDropDownList 宽度属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-width-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-width-property/)
...
`width` 属性用于设置或返回 `jqxDropDownList` 的宽度。它接受数字/字符串类型的值，默认值为空。

## 语法
设置 `width` 属性。

```html
$('selector').jqxDropDownList({ width: Number/String });
```

返回 `width` 属性。

```html
var width = $('selector').jqxDropDownList('width');
```
...
## 示例
以下示例说明了 jQWidgets 中的 `jqxDropDownList` `width` 属性。
...
![](img/6a726a7fdbe6e4ca732506e523f669fa.png)
