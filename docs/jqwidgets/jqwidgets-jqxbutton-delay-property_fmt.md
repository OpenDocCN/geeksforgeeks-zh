# jQWidgets jqxButton delay 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxbutton-delay-property/](https://www.geeksforgeeks.org/jqwidgets-jqxbutton-delay-property/)
...
`delay` 属性用于指定两个 `click` 事件之间的暂停。此外，它只能在 `jqxRepeatButton` 中实现。其中，无论何时点击显示的按钮，`jqxRepeatButton` 都会使 `continuous click` 事件。它属于数字类型，默认值为 `50`。
...
以下示例说明了 jQWidgets 中的 `jqxButton` `delay` 属性。
