# jqwidgets jqxscheduler editdateformat string property

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-editdialogdateformatstring-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-editdialogdateformatstring-property/)
...
`editDialogDateFormatString` 属性用于在显示的 `jqxScheduler` 的编辑对话框中设置或返回日期输入的格式字符串，只要约会没有设置为全天。它属于字符串类型，其默认值为 `dd/MM/yyyy`。

## 语法

*   设置 `editDialogDateFormatString` 属性。
...
以下示例说明了 jQWidgets 中的 `jqxScheduler` `editDialogDateFormatString` 属性。
