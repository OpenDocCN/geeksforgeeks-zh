# jQWidgets jqxSlider ticksPosition 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxslider-ticksposition-property/](https://www.geeksforgeeks.org/jqwidgets-jqxslider-ticksposition-property/)
...
`ticksPosition` 属性用于设置或返回滑块部件的刻度位置。它接受字符串类型的值，默认值为 `'both'`。
...
```javascript
$('selector').jqxSlider({ ticksPosition: 'top' });
```
...
下面的例子说明了 jQWidgets `jqxSlider` 的 `ticksPosition` 属性。
