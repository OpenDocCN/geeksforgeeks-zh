# jQWidgets jqxGrid everpresentrowheight 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxgrid-everpresentrowheight-property/](https://www.geeksforgeeks.org/jqwidgets-jqxgrid-everpresentrowheight-property/)

`jQWidgets` 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大、优化、独立于平台并且得到广泛支持的框架。`jqxGrid` 用于说明以表格形式显示数据的 jQuery 小部件。此外，它完全支持与数据的连接，以及分页、分组、排序、过滤和编辑。

`everpresentrowheight` 属性用于设置或返回显示的 `jqxGrid` 中一直存在的行的高度。它是数字类型，缺省值是 30。

## 语法

设置 `everpresentrowheight` 属性:

```javascript
$('#Selector').jqxGrid({ everpresentrowheight: 35});
```

返回 `everpresentrowheight` 属性:

```javascript
var everpresentrowheight = $('#Selector').jqxGrid('everpresentrowheight');
```
...
下面的示例说明了 `jQWidgets` 中的 `jqxGrid` `everpresentrowheight` 属性。
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxGrid everpresentrowheight property</h3>
        <br />
        <div id="jqxg"></div>
        <div>
...
![](img/e30431ff1c35359560bd27c5a8726856.png)
