...
*   `top`: 这是指定的垂直滚动条值，滚动到该值。
*   `left`: 这是指定的水平滚动条值，滚动到该值。

## 返回值

该方法返回使用的 `top` 和 `left` 参数的对象，即 `object.left` 和 `object.top`。
...
下面的示例说明了 `jQWidgets` `jqxTreeGrid` 的 `scrollOffset()` 方法。在下面的示例中，水平滚动被执行到其值 30。
