# jQWidgets jqxMenu 键盘导航属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxmenu-keyboardnavigation-property/](https://www.geeksforgeeks.org/jqwidgets-jqxmenu-keyboardnavigation-property/)
...
`键盘导航属性`用于设置或返回 `jqxMenu` 的键盘导航。它接受布尔类型值，默认值为 `false`。
...
```javascript
$('selector').jqxMenu({ keyboardNavigation: Boolean });
```

返回键盘导航属性。

```javascript
var keyboardNavigation = $('selector').jqxMenu('keyboardNavigation');
```
...
下面的例子说明了 jQWidgets `jqxMenu` `keyboardNavigation` 属性。
...
<body>
    ...
    <h3>
        jQWidgets jqxMenu keyboardNavigation Property
    </h3>
...
![](img/22509689a4231bd1583bb58c776dcd4a.png)
