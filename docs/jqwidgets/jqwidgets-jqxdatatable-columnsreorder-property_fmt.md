...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdatatable-columnsreorder-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdatatable-columnsreorder-property/)

`jQWidgets` 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大、优化、独立于平台并且得到广泛支持的框架。`jqxDataTable` 用于读取和显示 HTML 表格中的数据。这也用于显示来自各种数据源的数据，如 XML、JSON、Array、CSV 或 TSV。

`columnsReorder` 属性用于启用或禁用指定 `jqxDataTable` 的列重新排序过程。此属性接受“真”和“假”两个值。如果此属性接受“真”值，则可以通过拖放来更改任何列的位置。

## 语法

设置 `columnsReorder` 属性：

```javascript
$('#dataTable').jqxDataTable({columnsReorder: true });
```

获取 `columnsReorder` 属性：
...
下面的示例说明了 `jQWidgets` 的 `columnsReorder` 属性。在下面的示例中，`columnsReorder` 属性的值已设置为“真”。
