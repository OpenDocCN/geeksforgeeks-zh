...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxswitchbutton-enable-method/](https://www.geeksforgeeks.org/jqwidgets-jqxswitchbutton-enable-method/)
...
`enable()`方法用于启用显示的开关按钮。它没有参数，也不返回任何内容。
...
下面的示例说明了`jQWidgets`中的`jqxSwitchButton` `enable()`方法。
