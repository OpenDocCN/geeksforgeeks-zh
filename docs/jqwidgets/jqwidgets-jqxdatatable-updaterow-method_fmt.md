...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdatatable-updaterow-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdatatable-updaterow-method/)

`jQWidgets` 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大、优化、独立于平台并且得到广泛支持的框架。`jqxDataTable` 用于读取和显示 HTML 表格中的数据。这也用于显示来自各种数据源的数据，如 XML、JSON、Array、CSV 或 TSV。

`updateRow()` 方法用于更新行的数据。关于与服务器的同步，也请查看 `jqxDataAdapter` 插件的帮助文档。

## 语法

```html
$("#dataTable").jqxDataTable('updateRow', 0, rowData);
```
...
*   **行索引**: 这是将要执行数据更新的指定行索引。它的值从 0 开始。
*   **rowData**: 这是将要更新的数据。
...
下面的例子说明了 jQWidgets `updateRow()` 方法。
...
<body>
    <center>
        ...
        <h3> 
          jQWidgets jqxDataTable updateRow() Method 
        </h3>
        <div id="table"></div>
        <input type="button" style="margin: 20px;" 
               id="jqxbutton" 
               value="Update the second row" />
    </center>
</body>
...
![](img/6a8b285c572226ba1069b8d85eba7e39.png)
