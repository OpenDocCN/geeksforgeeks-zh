...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxcalendar-culture-property/](https://www.geeksforgeeks.org/jqwidgets-jqxcalendar-culture-property/)
...
`culture` 属性用于设置或返回 `jqxCalendar` 的区域性。如果我们使用区域性设置，那么我们需要添加一个包含语言代码的文件，比如德语的 `jquery.glob.de-DE.js`。它接受字符串类型值，其默认值为 `"default"`。
...
以下示例说明了 jQWidgets 中的 `jqxCalendar` 的 `culture` 属性。
