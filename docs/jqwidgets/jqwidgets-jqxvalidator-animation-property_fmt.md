...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxvalidator-animation-property/](https://www.geeksforgeeks.org/jqwidgets-jqxvalidator-animation-property/)
...
`animation` 属性用于设置或获取隐藏和显示指定 `jqxValidator` 提示的动画。该方法接受两个可能的值，即 `"fade"` 和 `"none"`。
...
以下示例说明了 jQWidgets `animation` 属性。在下面的例子中，`animation` 属性的值被设置为 `"fade"`。
