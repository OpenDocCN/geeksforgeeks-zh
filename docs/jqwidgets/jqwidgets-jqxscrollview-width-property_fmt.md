...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxscrollview-width-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscrollview-width-property/)
...
`width` 属性用于设置或返回 `jqxScrollView` 小部件的宽度。它接受数字/字符串类型的值，默认值为 320。
...
以下示例说明了 jQWidgets 中的 `jqxScrollView` `width` 属性。
