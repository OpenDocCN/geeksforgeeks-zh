# jQWidgets jqxNumberInput symbol 属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-symbol-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-symbol-property/)
...
`symbol` 属性用于设置或返回用作货币或百分比符号的字符串。它接受字符串类型值，默认值为 `""`。
...
下面的例子说明了 jQWidgets `jqxNumberInput` `symbol` 属性。
