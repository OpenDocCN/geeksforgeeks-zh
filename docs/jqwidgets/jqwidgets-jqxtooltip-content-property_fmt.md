...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxtooltip-content-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtooltip-content-property/)
...
`content` 属性用于设置或返回 `jqxTooltip` 小部件的内容。它可以是纯文本或 HTML 代码。它接受字符串类型值，默认值为空。
...
下面的示例说明了 jQWidgets `jqxTooltip` `content` 属性。
