...
`footerTemplate` 属性用于设置或返回页脚模板。`footerTemplate` 属性的值可以是 `HTMLTemplateElement` 的 id，也可以是 `HTMLTemplateElement` 本身的 id。它接受 `String`/`HTMLTemplateElement` 类型值。
...
**示例：** 以下示例说明了 `jQWidgets` `jqxTimePicker` 的 `footerTemplate` 属性。
