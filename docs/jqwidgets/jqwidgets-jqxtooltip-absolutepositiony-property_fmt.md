# jQWidgets jqxTooltip absolutePositionY 属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxtooltip-absolutepositiony-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtooltip-absolutepositiony-property/)
...
如果 `position` 属性设置为 `'absolute'`，则 `absolutePositionY` 属性用于设置或返回工具提示的垂直位置。它接受数字/字符串类型的值，默认值为 `0`。

## 语法

设置 `absolutePositionY` 属性。

```javascript
$('Selector').jqxTooltip({ absolutePositionY: Number });
```

返回 `absolutePositionY` 属性。

```javascript
var absPosY = $('Selector').jqxTooltip('absolutePositionY');
```
...
下面的例子说明了 jQWidgets `jqxTooltip` 的 `absolutePositionY` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxTooltip absolutePositionY Property
        </h3>
...
![](img/4fdafef1954acb07825e364a463804b4.png)
