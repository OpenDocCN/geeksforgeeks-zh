...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxbuttongroup-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxbuttongroup-disabled-property/)
...
`禁用`属性用于启用或禁用显示的 `jqxButtonGroup`。它是布尔类型，默认值为 `false`。
...
以下示例说明了 `jqxButtonGroup` 在 `jQWidgets` 中 `禁用` 了属性。
