# jQWidgets jqxDropDownButton enableBrowserBoundsDetection 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownbutton-enablebrowserboundsdetection-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownbutton-enablebrowserboundsdetection-property/)
...
`enableBrowserBoundsDetection` 属性用于打开显示的下拉按钮上方的弹出窗口，以防下拉按钮下方空间不足。它是布尔类型，默认值为 `false`。

## 语法

要设置 `enableBrowserBoundsDetection` 属性。
...
获取 `enableBrowserBoundsDetection` 属性。
...
下面的示例说明了 `jQWidgets` 中的 `jqxDropDownButton` `enableBrowserBoundsDetection` 属性。
...
<body>
    <center>
      ...
      <h3>jQWidgets jqxDropDownButton 
          enableBrowserBoundsDetection property</h3>
      <br />
      <div>
        <input
          type="button"
...
<script type="text/javascript">
      $(document).ready(function () {
        ...
        $("#jqxBtn").on("click", function () {
          var ebbd = $("#jqxDdB").jqxDropDownButton(
            "enableBrowserBoundsDetection"
          );
          $("#log").html("Boolean: " + ebbd);
        });
      });
    </script>
  </body>
...
![](img/8e9e77dbe9090dc8b299b428698e5677.png)
