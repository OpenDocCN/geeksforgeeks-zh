...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtouch-orientationchangeenabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtouch-orientationchangeenabled-property/)
...
`orientationchangenabled`属性用于指定的 `jqxTouch` 小部件是否启用了 `orientonchange` 事件。此属性接受两个可能的值，即真和假。

## 语法

*   设置 `orientationChangeEnabled` 属性:
    ```javascript
    $('#jqxTouch').jqxTouch({orientationChangeEnabled: true});
    ```

*   要获取 `orientationChangeEnabled` 属性:
    ```javascript
...
下面的示例说明了 jQWidgets `jqxTouch` 的 `orientationChangeEnabled` 属性。在下面的例子中，`orientationChangeEnabled` 属性的值被设置为 true。
...
[https://www.jqwidgets.com/jquery-widgets-documentation/documentation/jqxtouch/jquery-touch-api.htm](https://www.jqwidgets.com/jquery-widgets-documentation/documentation/jqxtouch/jquery-touch-api.htm)