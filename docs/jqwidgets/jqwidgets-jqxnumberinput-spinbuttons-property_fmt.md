# jQWidgets jqxNumberInput spinButtons 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-spinbuttons-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-spinbuttons-property/)
...
`spinButtons` 属性用于显示或隐藏旋转按钮。它接受布尔类型值，默认值为 `false`。
...
```javascript
$('selector').jqxNumberInput({ spinButtons: Boolean });
```

返回 `spinButtons` 属性。

```javascript
var spinButtons = $('selector').jqxNumberInput('spinButtons');
```
...
下面的例子说明了 jQWidgets `jqxNumberInput` `spinButtons` 属性。
...
![](img/adcdf74b8d65a14e8941a817a1aaf4bd.png)
