# jQWidgets jqxnumber input allowNull 属性

> 原文:[https://www . geesforgeks . org/jqwidgets-jqxnumberinput-allownlul-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-allownull-property/)
...
**allowNull 属性**用于设置或返回 jqxNumberInput 小部件值是否可以为空。它接受布尔类型值，默认值为真。
...
```html
$('selector').jqxNumberInput({ allowNull: Boolean });
```

返回 allowNull 属性。

```html
var allowNull = $('selector').jqxNumberInput('allowNull');
```
...
下面的例子说明了 jQWidgets 的 jqxNumberInput allowNull 属性。
...
![](img/b47a93c77fc08a3ab186184659650b3f.png)
