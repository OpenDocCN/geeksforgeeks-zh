...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxgrid-columnmenuclosing-property/](https://www.geeksforgeeks.org/jqwidgets-jqxgrid-columnmenuclosing-property/)

jQWidgets 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大、优化、独立于平台并且得到广泛支持的框架。`jqxGrid` 用于说明以表格形式显示数据的 jQuery 小部件。此外，它完全支持与数据的连接，以及分页、分组、排序、过滤和编辑。

`colummenuclosing` 属性是一个回调函数，每当该列的菜单关闭时都会调用该函数。此外，这里可以传递三个参数，即菜单、数据字段以及菜单的高度。如果函数返回 `false`，则关闭将停止。它属于函数类型，其默认值为空。
...
```html
var columnmenuclosing = $('#Selector').jqxGrid('columnmenuclosing');
```
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxGrid columnmenuclosing property
        </h3>
        <br />
