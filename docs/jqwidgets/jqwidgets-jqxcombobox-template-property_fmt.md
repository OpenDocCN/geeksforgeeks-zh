# jQWidgets jqxComboBox 模板属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxcombobox-template-property/](https://www.geeksforgeeks.org/jqwidgets-jqxcombobox-template-property/)
...
`template` 属性用于设置或返回按钮的模板，作为默认样式的替代。它接受字符串类型值，其默认值为“default”。
...
```html
$('selector').jqxComboBox({ template: String });
```

返回 `template` 属性。

```html
var template = $('selector').jqxComboBox('template');
```
...
以下示例说明了 jQWidgets 中的 `jqxComboBox` `template` 属性:
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxComboBox template Property
        </h3>
...
![](img/e7e392daf55ae29a65c1b62403af62ca.png)
