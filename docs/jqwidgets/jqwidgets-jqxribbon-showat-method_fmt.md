...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxribbon-showat-method/](https://www.geeksforgeeks.org/jqwidgets-jqxribbon-showat-method/)
...
`showAt()` 方法用于显示给定索引的项目。它接受数字类型的单参数 `index`，不返回值。
...
下面的例子说明了 `jQWidgets` `jqxRibbon` `showAt()` 方法。
