# jQWidgets jqxTooltip absolutePositionX 属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxtooltip-absolutepositionx-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtooltip-absolutepositionx-property/)
...
如果 `position` 属性设置为 `'absolute'`，则 `absolutePositionX` 属性用于设置或返回工具提示的水平位置。它接受数字/字符串类型的值，默认值为 `0`。

## 语法

设置 `absolutePositionX` 属性。

```javascript
$('Selector').jqxTooltip({ absolutePositionX: Number });
```

返回 `absolutePositionX` 属性。
