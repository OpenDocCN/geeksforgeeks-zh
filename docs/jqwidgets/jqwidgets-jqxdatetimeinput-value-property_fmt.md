...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdatetimeinput-value-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdatetimeinput-value-property/)
...
`value` 属性用于设置或返回 `jqxDateTimeInput` 小部件的值。它接受日期类型值，默认值为“今天的日期”。
...
以下示例说明了 `jQWidgets` 中的 `jqxDateTimeInput` `value` 属性。
