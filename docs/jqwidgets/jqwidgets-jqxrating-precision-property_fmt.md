...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxrating-precision-property/](https://www.geeksforgeeks.org/jqwidgets-jqxrating-precision-property/)
...
`precision` 属性用于设置或返回 `jqxRating` 小部件的投票精度。它接受数字类型值，默认值为 1。
...
以下示例说明了 jQWidgets `jqxRating` 的 `precision` 属性。
