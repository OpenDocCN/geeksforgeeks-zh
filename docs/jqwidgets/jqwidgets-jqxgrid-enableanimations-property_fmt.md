...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxgrid-enableanimations-property/](https://www.geeksforgeeks.org/jqwidgets-jqxgrid-enableanimations-property/)

jQWidgets 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大、优化、独立于平台并且得到广泛支持的框架。`jqxGrid` 用于说明以表格形式显示数据的 jQuery 小部件。此外，它完全支持与数据的连接，以及分页、分组、排序、过滤和编辑。

`enableanimations` 属性用于启用或禁用显示的 `jqxGrid` 的动画。它属于布尔类型，其缺省值为真。

## 语法

设置 `enableanimations` 属性。

```html
$('#Selector').jqxGrid({ enableanimations: false});
```

返回 `enableanimations` 属性。

```html
var enableanimations = $('#Selector').jqxGrid('enableanimations');
```
...
<script type="text/javascript" src="jqwidgets/jj"></script>
```
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxGrid enableanimations property</h3>
        <br />
        <div id="jqxg"></div>
        <div>
