...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-legendheight-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-legendheight-property/)
...
`legendHeight` 属性用于设置或返回显示的 `jqxScheduler` 的图例高度。它属于数字类型，默认值为 34。
...
```html
var legendHeight = $('#Selector').jqxScheduler('legendHeight');
```
...
以下示例说明了 jQWidgets 中的 `jqxScheduler` `legendHeight` 属性。
