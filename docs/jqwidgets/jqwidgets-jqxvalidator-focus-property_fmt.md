# jQWidgets jqxValidator 焦点属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxvalidator-focus-property/](https://www.geeksforgeeks.org/jqwidgets-jqxvalidator-focus-property/)
...
`focus` 属性用于设置或获取指定的 `jqxValidator` 是否聚焦第一个无效输入。该方法接受两个可能的值，即 `true` 和 `false`。
...
## 示例:
以下示例说明了 `jQWidgets` `focus` 属性。在下面的示例中， `focus` 属性的值被设置为 `false`。
