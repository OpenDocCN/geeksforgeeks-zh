# jQWidgets jqxLoader 宽度属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxloader-width-property/](https://www.geeksforgeeks.org/jqwidgets-jqxloader-width-property/)
...
`width` 属性用于设置或返回加载器小部件的宽度。它接受数字/字符串类型的值，默认值为 150。
...
下面的例子说明了 jQWidgets `jqxLoader` 的 `width` 属性。
