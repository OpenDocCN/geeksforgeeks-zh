# jQWidgets jqxFormattedInput dropDownWidth 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-dropdownwidth-property/](https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-dropdownwidth-property/)
...
`dropDownWidth` 属性用于设置或返回下拉小部件的宽度。它接受数字/字符串类型的值，默认值为空。
...
```javascript
$('selector').jqxFormattedInput({ dropDownWidth: Number/String });
```

返回 `dropDownWidth` 属性。

```javascript
var dropDownWidth = $('selector').jqxFormattedInput('dropDownWidth');
```
...
下面的例子说明了 jQWidgets `jqxFormattedInput` `dropDownWidth` 属性。
...
![](img/deeba6ff5ecd9a66c4bf9e2d43a6b9e6.png)
