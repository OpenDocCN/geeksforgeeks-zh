...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtooltip-autohidedelay-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtooltip-autohidedelay-property/)
...
**自动隐藏延迟属性**用于设置或返回工具提示自动隐藏的持续时间。仅当 `autoHide` 属性设置为真时，此属性才有效。如果我们将 `autoHideDelay` 属性值设置为 0，那么工具提示只会在鼠标离开父元素后隐藏。
...
下面的例子说明了 jQWidgets `jqxTooltip` `autoHideDelay` 属性。
