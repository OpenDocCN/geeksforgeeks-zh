# jQWidgets jqxDropDownList autoOpen 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-autoopen-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-autoopen-property/)
...
`autoOpen` 属性用于设置或返回当鼠标光标在小部件上移动时是否自动打开下拉选项。它接受布尔类型值，默认值为 `false`。
...
**示例:** 以下示例说明了 `jQWidgets` 中的 `jqxDropDownList` `autoOpen` 属性。
