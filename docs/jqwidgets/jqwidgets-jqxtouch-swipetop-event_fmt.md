...
> 哎哎哎:# t0]https://www . geeksforgeeks . org/jqwidgets-jqxtouch-swipetop-event/
...
当用户向上轻击指定的 `jqxTouch` 小部件时，触发 `swipetop` 事件。
...
**示例:** 以下示例说明了 `jQWidgets` `jqxTouch` 的 `swipetop` 事件。
