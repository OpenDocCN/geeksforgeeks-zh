...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdatatable-selectionmode-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdatatable-selectionmode-property/)
...
`selectionMode` 属性用于设置或获取指定的选择模式。
...
在 `"custom"` 模式下，只能通过 API 选择/取消选择行。
...
以下示例说明了 jQWidgets `selectionMode` 属性。在以下示例中，`selectionMode` 属性的值已设置为 `"multipleRows"`。
