# jQWidgets jqxToolBar 高度属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxtoolbar-height-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtoolbar-height-property/)
...
`高度属性`用于设置或返回显示的 `jqxToolBar` 的高度。它的类型为数字或字符串，默认值为 35。
...
```html
$('#Selector').jqxToolBar({ height: 50});
```

*   返回高度属性。

```html
var height = $('#Selector').jqxToolBar('height');
```
...
下面的例子说明了 jQWidgets 中的 `jqxToolBar` `高度属性`。
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxToolBar height property</h3>
...
![](img/be699b32d5192341c4d69846c99f3b12.png)
