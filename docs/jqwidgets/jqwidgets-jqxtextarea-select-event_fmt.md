# jQWidgets jqxTextArea 选择事件

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtextarea-select-event/](https://www.geeksforgeeks.org/jqwidgets-jqxtextarea-select-event/)
...
当从文本区自动建议弹出列表中选择一个项目时，触发 `jqxTextArea` 的 `select` 事件。
...
**示例:** 以下示例说明了 jQWidgets jqxTextArea 的 `select` 事件。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxTextArea select Event
        </h3>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#jqxTA').on('select', function (event) {
                if (event.args) {
                    alert("An item has been selected");
                }
            });
        });
    </script>
...
![](img/fb7ae1ea0834f861055eed5cc6a3c730.png)
