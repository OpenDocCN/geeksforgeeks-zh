...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdocking-hidecollapsebutton-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdocking-hidecollapsebutton-method/)
...
`hideCollapseButton()` 方法用于隐藏指定 `jqxDocking` 的特定窗口的折叠按钮。
...
### 示例:
下面的示例说明了 `jQWidgets` `jqxDocking` `hideCollapseButton()` 方法。在下面的示例中，第二个窗口的折叠按钮被隐藏。
