...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxnavigationbar-add-method/](https://www.geeksforgeeks.org/jqwidgets-jqxnavigationbar-add-method/)
...
`add()` 方法用于在指定的 `jqxNavigationBar` 底部插入一个新项目。
...
下面的示例说明了 `jQWidgets` `jqxNavigationBar` `add()` 方法。
