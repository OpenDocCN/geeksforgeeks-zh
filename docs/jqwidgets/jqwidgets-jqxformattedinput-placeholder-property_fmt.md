# jQWidgets jqxFormattedInput 占位符属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-placeholder-property/](https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-placeholder-property/)
...
`placeHolder` 属性用于设置或返回 `jqxFormattedInput` 小部件的占位符。它接受字符串类型值，默认值为空。
...
```javascript
$('selector').jqxFormattedInput({ placeHolder: String });
```

返回 `placeHolder` 属性。

```javascript
var placeHolder = $('selector').jqxFormattedInput('placeHolder');
```
...
下面的例子说明了 jQWidgets `jqxFormattedInput` `placeHolder` 属性。
...
![](img/0c384a83611f8f34283fffcb3be922d7.png)
