...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxresponsivepanel-collapsebreakpoint-property/](https://www.geeksforgeeks.org/jqwidgets-jqxresponsivepanel-collapsebreakpoint-property/)
...
`collapseBreakpoint` 属性用于设置或返回改变浏览器宽度后面板是否显示/隐藏。它接受数字类型的值，默认值为 1000。

## 语法

*   设置 `collapseBreakpoint` 属性。
...
下面的例子说明了 jQWidgets 中的 `jqxResponsivePanel` `collapseBreakpoint` 属性。
