...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdocking-addwindow-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdocking-addwindow-method/)
...
*   `mode`: 这是新窗口的模式。它的可能值是“默认”、“停靠”和“浮动”。这是一个可选参数。
*   `panel`: 这是面板的索引。这是一个可选参数。
*   `position`: 这是窗口进入面板的位置，即索引。这是一个可选参数。
...
下面的示例说明了 jQWidgets jqxDocking `addWindow()` 方法。在下面的示例中，添加了一个 id 为“Second_Window”的新窗口。
