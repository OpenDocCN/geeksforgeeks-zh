# jQWidgets jqxNumberInput disabled 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-disabled-property/)
...
`disabled` 属性用于设置或返回 `jqxNumberInput` 是否禁用。它接受布尔类型值，默认值为 `false`。
...
下面的例子说明了 jQWidgets `jqxNumberInput` `disabled` 属性。
