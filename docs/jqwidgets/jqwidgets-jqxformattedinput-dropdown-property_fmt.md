# jQWidgets jqxFormattedInput 下拉属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-dropdown-property/](https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-dropdown-property/)
...
`dropDown` 属性用于设置或返回下拉选项是否启用。此选项用于更改给定数字的基数。它接受布尔类型值，默认值为 `false`。
...
```javascript
$('selector').jqxFormattedInput({ dropDown: Boolean });
```

返回 `dropDown` 属性。

```javascript
var dropDown = $('selector').jqxFormattedInput('dropDown');
```
...
下面的例子说明了 jQWidgets `jqxFormattedInput` `dropDown` 属性。
...
![](img/963ef0b55100558e8873e00ce8033e0e.png)
