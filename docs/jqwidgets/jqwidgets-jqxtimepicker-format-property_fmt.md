# jQWidgets jqxTimePicker 格式属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtimepicker-format-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtimepicker-format-property/)
...
`format` 属性用于设置或返回使用 12 小时或 24 小时时钟惯例的时钟格式。它接受字符串类型值，默认值为“12 小时”。
...
```javascript
$('selector').jqxTimePicker({ format: false });
```

返回 `format` 属性。

```javascript
var format = $('selector').jqxTimePicker('format');
```
...
下面的例子说明了 jQWidgets `jqxTimePicker` `format` 属性。
...
![](img/03beeef0defd72e259b52237ed5c1d96.png)
