...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxdatetimeinput-placeholder-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdatetimeinput-placeholder-property/)
...
`placeHolder` 属性用于设置或返回当小部件值为 `null` 时显示的小部件占位符值。它接受字符串类型的值，默认值为 `""`。
...
以下示例说明了 jQWidgets 中的 `jqxDateTimeInput` `placeHolder` 属性。
