...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxbutton-uncheck-method/](https://www.geeksforgeeks.org/jqwidgets-jqxbutton-uncheck-method/)
...
`unCheck()` 方法用于取消选中显示的按钮。它可以在 `jqxToggleButton` 中访问。它没有参数，也不返回任何内容。
...
下面的示例说明了 `jQWidgets` 中的 `jqxButton` `unCheck()` 方法。
