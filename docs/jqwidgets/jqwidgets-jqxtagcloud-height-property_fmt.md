...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtagcloud-height-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtagcloud-height-property/)
...
`height` 属性用于设置或获取指定 `jqxTagCloud` 的高度。它既可以接受数字也可以接受字符串，其默认值为空。
...
以下示例说明了 jQWidgets `jqxTagCloud` 的 `height` 属性。在下面的例子中，`height` 属性的值被设置为 25。
