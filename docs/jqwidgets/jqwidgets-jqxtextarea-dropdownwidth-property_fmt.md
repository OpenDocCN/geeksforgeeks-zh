# jQWidgets jqxTextArea dropdown width 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtextarea-dropdownwidth-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtextarea-dropdownwidth-property/)
...
`dropDownWidth` 属性用于设置或返回 `jqxTextArea` 下拉(弹出)宽度。它接受数字/字符串类型的值，默认值为空。
...
```html
$('selector').jqxTextArea({ dropDownWidth: Number/String });
```

返回 `dropDownWidth` 属性。

```html
var DDW = $('selector').jqxTextArea('dropDownWidth');
```
...
下面的例子说明了 jQWidgets `jqxTextArea` `dropDownWidth` 属性。
...
![](img/d1b251cf73ffa23110daa9b364f70d79.png)
