# jQWidgets jqxTreeMap showLegend 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtreemap-showlegend-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtreemap-showlegend-property/)
...
`showLegend` 属性用于检查指定 `jqxTreeMap` 的图例是否可见。当 `colorMode` 属性的值设置为“自动颜色”或“范围颜色”时，该属性将生效。
...
下面的示例说明了 `jQWidgets` `jqxTreeMap` `showLegend` 属性。在以下示例中，`showLegend` 属性的值已设置为 `true`。
