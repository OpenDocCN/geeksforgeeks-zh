...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-touchscrollbarsize-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-touchscrollbarsize-property/)
...
`touchScrollBarSize` 属性用于设置或返回触摸设备上显示的 `jqxScheduler` 中滚动条的大小。它属于数字类型，其默认值为 18。

## 语法

*   设置 `touchScrollBarSize` 属性。

```html
$("#Selector").jqxScheduler({ touchScrollBarSize: 20 });
```

*   返回 `touchScrollBarSize` 属性。
...
以下示例说明了 jQWidgets 中的 `jqxScheduler` `touchScrollBarSize` 属性。
