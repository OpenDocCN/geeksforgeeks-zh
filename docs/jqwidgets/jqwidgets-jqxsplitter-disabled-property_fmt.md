...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxsplitter-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxsplitter-disabled-property/)
...
`disabled` 属性用于设置或获取禁用属性，该属性用于禁用小部件。
...
以下示例说明了 jQWidgets `jqxSplitter` `disabled` 属性。在下面的例子中，`disabled` 属性的值被设置为 `true`。
