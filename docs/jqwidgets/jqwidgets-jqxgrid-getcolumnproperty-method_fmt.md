...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxgrid-getcolumnproperty-method/](https://www.geeksforgeeks.org/jqwidgets-jqxgrid-getcolumnproperty-method/)
...
*   `text`: 是列的规定名称。
*   `hidden`: 表示指定列是否隐藏。
*   `hideable`: 表示指定的列是否可以隐藏。
*   `renderer`: 渲染指定的列。
*   `cellsrenderer`: 渲染细胞。
*   `align`: 定义列的对齐方式。
*   `cellsalign`: 它定义了细胞的排列。
*   `cellsformat`: 定义单元格的格式。
*   `pinned`: 表示一列是否被固定。
*   `contenttype`: 定义指定列的内容类型。
*   `resizable`: 表示列是否可调整大小。
*   `filterable`: 表示该列是否可过滤。
*   `editable`: 表示该列是否可编辑。
...
下面的示例说明了 jQWidgets 中的 jqxGrid `getcolumnproperty()`方法。
