...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxcalendar-height-property/](https://www.geeksforgeeks.org/jqwidgets-jqxcalendar-height-property/)
...
`height` 属性用于设置或返回日历的高度。它接受数字/字符串类型值，默认值为 `null`。
...
以下示例说明了 jQWidgets 中的 `jqxCalendar` `height` 属性。
