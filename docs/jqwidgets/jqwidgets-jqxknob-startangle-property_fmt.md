# jQWidgets jqxKnob startAngle 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxknob-startangle-property/](https://www.geeksforgeeks.org/jqwidgets-jqxknob-startangle-property/)
...
`startAngle` 属性用于设置或获取指定 `jqxKnob` 的 0 位置的度数偏移。
...
```javascript
var allowValueChangeOnClick = $('#jqxKnob').jqxKnob('startAngle');
```
...
以下示例说明了 `jQWidgets` `jqxKnob` `startAngle` 属性。在下面的示例中， `startAngle` 属性的值被设置为 0。
