...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdocking-pinwindow-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdocking-pinwindow-method/)
...
### 示例:
下面的示例说明了 `jQWidgets` `jqxDocking` `pinWindow()` 方法。在下面的例子中，调用 `pinWindow()` 方法后，窗口将被固定，不会被拖动到任何地方。
