# jQWidgets jqxGauge linear gauge 配色方案属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxgauge-lineargauge-colorscheme-property/](https://www.geeksforgeeks.org/jqwidgets-jqxgauge-lineargauge-colorscheme-property/)
...
`colorScheme` 属性用于设置或返回 `colorScheme` 属性，即用于设置线性标尺的标尺颜色托盘。它接受字符串类型值，默认值为 `"scheme01"`。
...
以下示例说明了 jQWidgets 中的 `jqxLinearGauge` `colorScheme` 属性。
