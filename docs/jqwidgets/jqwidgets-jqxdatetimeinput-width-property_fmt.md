# jq 构件 jqxDateTimeInput width 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdatetimeinput-width-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdatetimeinput-width-property/)
...
`width` 属性用于设置或返回 `jqxDateTimeInput` 小部件的宽度。它接受数字/字符串类型的值，默认值为空。
...
```javascript
$('selector').jqxDateTimeInput({ width: Number/String });
```

返回 `width` 属性。

```javascript
var width = $('selector').jqxDateTimeInput('width');
```
...
以下示例说明了 jQWidgets 中的 `jqxDateTimeInput` `width` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxDateTimeInput width Property
        </h3>
...
![](img/e5f97b2087412df25f45240f4f8c6a76.png)
