# jQWidgets jqxDropDownButton 宽度属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownbutton-width-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownbutton-width-property/)
...
`width` 属性用于设置或获取显示的下拉按钮的宽度。类型为数字或字符串，默认值为 `null`。
...
下面的示例说明了 jQWidgets 中的 `jqxDropDownButton` `width` 属性。
