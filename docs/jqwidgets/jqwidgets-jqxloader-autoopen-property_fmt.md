...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxloader-autoopen-property/](https://www.geeksforgeeks.org/jqwidgets-jqxloader-autoopen-property/)
...
`autoOpen` 属性用于设置或返回创建后的加载程序。它接受布尔类型值，默认值为 `false`。
...
下面的例子说明了 `jQWidgets` `jqxLoader` `autoOpen` 属性。
