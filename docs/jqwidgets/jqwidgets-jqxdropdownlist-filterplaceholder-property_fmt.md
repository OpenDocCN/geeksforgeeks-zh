...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-filterplaceholder-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-filterplaceholder-property/)
...
`filterPlaceHolder` 属性用于设置或返回过滤器输入的占位符。它接受字符串类型值，默认值为“查找”。

## 语法

设置 `filterPlaceHolder` 属性。

```javascript
$('selector').jqxDropDownList({ filterPlaceHolder: 'String' });
```

返回 `filterPlaceHolder` 属性。
...
以下示例说明了 `jQWidgets` 中的 `jqxDropDownList` `filterPlaceHolder` 属性。
...
![](img/5ea6d37ed613459e1c34df4d318799bd.png)
