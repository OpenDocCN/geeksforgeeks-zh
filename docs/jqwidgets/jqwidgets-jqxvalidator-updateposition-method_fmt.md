...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxvalidator-updateposition-method/](https://www.geeksforgeeks.org/jqwidgets-jqxvalidator-updateposition-method/)
...
下面的示例说明了 jQWidgets `updatePosition()` 方法。在下面的示例中，运行代码后，当前窗口的大小已经调整，这就是为什么提示的位置已经改变。所以调用 `updatePosition()` 方法后，所有提示的位置都会更新到默认位置。
