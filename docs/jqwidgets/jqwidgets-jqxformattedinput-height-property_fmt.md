# jQWidgets jqxFormattedInput height 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-height-property/](https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-height-property/)
...
`height` 属性用于设置或返回 `jqxFormattedInput` 小部件的高度。它接受数字/字符串类型的值，默认值为空。
...
下面的例子说明了 jQWidgets `jqxFormattedInput` `height` 属性。
