# jQWidgets jqxTagCloud 字体大小单位属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtagcloud-fontsizeunit-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtagcloud-fontsizeunit-property/)
...
`fontSizeUnit` 属性用于设置或获取指定 `jqxTagCloud` 的字体大小单位。它的可能值是 `"px"`、`"em"` 和 `"%"`。

## 语法

*   用于设置 `fontSizeUnit` 属性。

```html
$('#jqxTagCloud').jqxTagCloud({ fontSizeUnit: 'px' });
```

*   用于获得 `fontSizeUnit` 属性。
...
下面的示例说明了 `jQWidgets` `jqxTagCloud` `fontSizeUnit` 属性。在下面的例子中，`fontSizeUnit` 属性的值被设置为 `"px"`。
