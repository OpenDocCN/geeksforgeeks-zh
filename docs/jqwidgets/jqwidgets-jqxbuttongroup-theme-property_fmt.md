...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxbuttongroup-theme-property/](https://www.geeksforgeeks.org/jqwidgets-jqxbuttongroup-theme-property/)
...
`theme` 属性用于设置或返回 `jqxButtonGroup` 小部件的主题。它接受字符串类型值，默认值为空(`''`)。为了使用这个属性，我们需要将 `theme` 样式表（即 `jqx.energyblue.css`）合并到标题部分。主题文件包含在 `jqx.base.css` 文件之后。
...
以下示例说明了 `jQWidgets` 中的 `jqxButtonGroup` `theme` 属性。
