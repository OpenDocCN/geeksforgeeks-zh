# jQWidgets jqxScheduler 最小属性

> 原文:[https://www . geesforgeks . org/jqwidgets-jqxscheduler-min-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-min-property/)
...
***min*** 属性用于设置或返回显示的 **jqxScheduler** 的最小航空日期。类型为 **jqxDate** ，默认值为“new $.jqx.date(0)”
...
*   设置 *min* 属性。
    ...
    ```html
    $("#Selector").jqxScheduler({ min: new $.jqx.date(0) });
    ```

*   返回 *min* 属性。

    ```html
    var min = $('#Selector').jqxScheduler('min');
    ```
...
**示例:**以下示例说明了 jQWidgets 中的 **jqxScheduler** *min* 属性。
...
        <h3>jQWidgets jqxScheduler min property   </h3>
...
                var m = $('#jqxs').jqxScheduler('min');
                $('#log').text("Minimum navigation date: "
                    + m.toString());
            });
        });
    </script>
...
![](img/f6acfeb5aaee9737906a376706a038aa.png)
