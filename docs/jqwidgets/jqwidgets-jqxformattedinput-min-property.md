# jqwidget jqxfformattitinput min property

> 原文:[https://www . geesforgeks . org/jqwidgets-jqxformatedinput-min-property/](https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-min-property/)
...
**min 属性**用于设置或返回小部件的最小值。它接受字符串类型的值，默认值是十进制的“-9223372036854775808”(-2⁶)。
...
```html
$('selector').jqxFormattedInput({ min: String });
```

返回最小属性。

```html
var min = $('selector').jqxFormattedInput('min');
```
...
下面的例子说明了 jQWidgets jqxFormattedInput min 属性。
...
![](img/89a3acad0c6e3a163e0af033bd7e5caa.png)
