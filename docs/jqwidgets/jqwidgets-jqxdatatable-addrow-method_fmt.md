...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxdatatable-addrow-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdatatable-addrow-method/)
...
`addRow()` 方法用于在现有数据表中添加新行。关于与服务器的同步，也请查看 `jqxDataAdapter` 插件的帮助文档。
...
*   `rowPosition`：这是给定的行位置。它属于字符串类型，其值为 `"last"` 和 `"first"`。默认情况下，使用 `"last"`。
