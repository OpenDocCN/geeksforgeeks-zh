...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtreemap-colormode-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtreemap-colormode-property/)
...
`colorMode` 属性用于设置或获取指定 `jqxTreeMap` 的扇区渲染行为。此属性接受三个可能的值，如下所示:
...
下面的示例说明了 jQWidgets `jqxTreeMap` `colorMode` 属性。在下面的例子中， `colorMode` 属性的值被设置为 `'autoColors'`。
