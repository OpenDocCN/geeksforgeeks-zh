# jQWidgets jqxRibbon selectedIndex 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxribbon-selectedindex-property/](https://www.geeksforgeeks.org/jqwidgets-jqxribbon-selectedindex-property/)
...
`selectedIndex` 属性用于设置或返回选择的索引标签。它接受数字类型值，默认值为 0。索引从 0 开始。
...
```html
$('selector').jqxRibbon({ selectedIndex: Number });
```

返回 `selectedIndex` 属性。

```javascript
var selectedIndex = $('selector').jqxRibbon('selectedIndex');
```
...
以下示例说明了 jQWidgets jqxRibbon `selectedIndex` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxRibbon selectedIndex Property
        </h3>
    </center>
...
![](img/3975e3274bb5c35d9c0f1fba3b285be2.png)
