...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdatatable-scrolloffset-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdatatable-scrolloffset-method/)

`jQWidgets` 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大、优化、独立于平台并且得到广泛支持的框架。`jqxDataTable` 用于读取和显示 HTML 表格中的数据。这也用于显示来自各种数据源的数据，如 XML、JSON、Array、CSV 或 TSV。

`scrollOffset()` 方法用于将指定的数据表滚动到特定位置。
...
*   `top`: 这是指定的顶部位置，滚动到该位置。
*   `left`: 这是指定的左侧位置，滚动到该位置。
...
下面的例子说明了 `jQWidgets` 的 `scrollOffset()` 方法。
...
![](img/4b61e1fffacae80e07348c32328a6543.png)
