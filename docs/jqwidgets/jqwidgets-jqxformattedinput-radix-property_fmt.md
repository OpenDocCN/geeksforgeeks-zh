...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-radix-property/](https://www.geeksforgeeks.org/jqwidgets-jqxformattedinput-radix-property/)
...
`radix` 属性用于设置或返回 `jqxFormattedInput` 小部件的基数。基数指定显示部件值的数字系统。它接受数字/字符串类型的值，默认值为 10。
...
下面的例子说明了 jQWidgets `jqxFormattedInput` `radix` 属性。
