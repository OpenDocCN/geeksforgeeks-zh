...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdragdrop-opacity-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdragdrop-opacity-property/)
...
`opacity` 属性用于设置或返回可拖动对象拖动时的不透明度。它接受数字类型值，默认值为 0.6。
...
以下示例说明了 jQWidgets 中的 `jqxDragDrop` `opacity` 属性。
