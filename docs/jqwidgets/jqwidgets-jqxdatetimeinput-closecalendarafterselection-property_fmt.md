# jQWidgets jqxdatetime input closecalendaraftereselection 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdatetimeinput-closecalendarafterselection-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdatetimeinput-closecalendarafterselection-property/)
...
**关闭日历选择属性**用于设置或返回选择后弹出日历是否关闭。它接受布尔类型值，默认值为真。

**语法:**

设置 `closeCalendarAfterSelection` 属性。

```javascript
$('selector').jqxDateTimeInput({ closeCalendarAfterSelection: String });
```

返回 `closeCalendarAfterSelection` 属性。
...
下面的例子说明了 jQWidgets 中的 `jqxDateTimeInput` `closecalendarafterselect` 属性。
...
![](img/741ed530521c68c0c132386535547e1b.png)
