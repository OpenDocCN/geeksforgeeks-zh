# jQWidgets jqxNavigationBar collapseAnimationDuration 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxnavigationbar-collapseanimationduration-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnavigationbar-collapseanimationduration-property/)

`jQWidgets` 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大、优化、独立于平台并且得到广泛支持的框架。`jqxNavigationBar` 用于表示具有标题和内容部分的 jQuery 小部件。单击标题，内容将相应地展开或折叠。

`collapseAnimationDuration` 属性用于设置或获取指定 `jqxNavigationBar` 的折叠动画持续时间。此属性接受数值。

## 语法

用于设置 `collapseAnimationDuration` 属性:
...
下面的示例说明了 jQWidgets `jqxNavigationBar` 的 `collapseAnimationDuration` 属性。在下面的例子中，`collapseAnimationDuration` 属性的值被设置为 2000。
