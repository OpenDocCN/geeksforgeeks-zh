# jQWidgets jqxSlider 最小属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxslider-min-property/](https://www.geeksforgeeks.org/jqwidgets-jqxslider-min-property/)
...
`min` 属性用于设置或返回滑块小部件的最小值。它接受数字类型值，默认值为 0。
...
```html
$('selector').jqxSlider({ min: Number });
```

返回最小属性。

```html
var min = $('selector').jqxSlider('min');
```
...
下面的例子说明了 jQWidgets `jqxSlider` `min` 属性。
...
![](img/398b0cbcd628dffc49dbbe6507ab3c46.png)
