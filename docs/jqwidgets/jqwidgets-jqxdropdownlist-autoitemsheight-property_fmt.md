...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-autoitemsheight-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-autoitemsheight-property/)
...
`autoItemsHeight` 属性用于设置或返回当项目达到下拉菜单的宽度时是否换行。它接受布尔类型值，默认值为 `false`。
...
**示例:** 下面的示例说明了 jQWidgets 中的 `jqxDropDownList` `autoItemsHeight` 属性。
