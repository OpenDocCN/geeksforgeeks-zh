...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-getappointmentproperty-method/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-getappointmentproperty-method/)
...
`getAppointmentProperty()` 方法用于从显示的 `jqxScheduler` 中返回约会属性的值。它不返回任何东西。
...
*   `appointmentId`：是指定的约会 Id。它属于字符串类型。
*   `name`：为声明的物业名称。它属于字符串类型。
...
下面的示例说明了 jQWidgets 中的 `jqxScheduler` `getAppointmentProperty()` 方法。
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxScheduler getAppointmentProperty() method</h3>
