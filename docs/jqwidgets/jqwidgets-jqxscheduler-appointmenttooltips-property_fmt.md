# jQWidgets jqxScheduler appointmentTooltips 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-appointmenttooltips-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-appointmenttooltips-property/)
...
`appointmentTooltips` 属性用于设置或返回显示的 `jqxScheduler` 的工具提示是否启用。它属于布尔类型，默认值为真。

## 语法

*   设置 `appointmentTooltips` 属性。

```html
$("Selector").jqxScheduler({ appointmentTooltips: false });
```

*   返回 `appointmentTooltips` 属性。
...
以下示例说明了 `jQWidgets` 中的 `jqxScheduler` `appointmentTooltips` 属性。
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxScheduler appointmentTooltips property</h3>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#jqxBtn").on("click", function () {
                var att = $('#jqxs').jqxScheduler('appointmentTooltips');
                $('#log').text("Appointment tooltips visible: " + att);
            });
        });
    </script>
</body>
...
![](img/b197b2b058c0bd1e2fa51c6a1d33080a.png)
