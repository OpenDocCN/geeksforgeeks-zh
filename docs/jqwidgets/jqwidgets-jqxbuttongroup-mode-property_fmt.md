...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxbuttongroup-mode-property/](https://www.geeksforgeeks.org/jqwidgets-jqxbuttongroup-mode-property/)
...
`模式属性`用于设置或获取显示的 `jqxButtonGroup` 的模式。它是字符串类型，默认值为 `default`。
...
以下示例说明了 `jQWidgets` 中的 `jqxButtonGroup` `模式` 属性。
