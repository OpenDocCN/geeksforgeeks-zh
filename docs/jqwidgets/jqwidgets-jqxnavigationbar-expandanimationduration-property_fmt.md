# jQWidgets jqxNavigationBar expandAnimationDuration 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxnavigationbar-expandanimationduration-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnavigationbar-expandanimationduration-property/)
...
`expandAnimationDuration` 属性用于设置或获取指定 `jqxNavigationBar` 的扩展动画时长。此属性接受数值。
...
### 获取 `expandAnimationDuration` 属性

```javascript
var expandAnimationDuration = $('Selector').jqxNavigationBar('expandAnimationDuration');
```
...
下面的示例说明了 jQWidgets `jqxNavigationBar` 的 `expandAnimationDuration` 属性。在下面的示例中，`expandAnimationDuration` 属性的值已设置为 2000。
