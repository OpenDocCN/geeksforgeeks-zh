...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtabs-addat-method/](https://www.geeksforgeeks.org/jqwidgets-jqxtabs-addat-method/)
...
`addAt()` 方法用于在给定位置添加制表符。添加选项卡包含选项卡的内容。该方法接受下面给出的三个参数:

*   `索引`: 接受添加新标签的索引号。索引从 0 开始。
*   `标签标题`: 接受新增标签的标题。
*   `标签内容`: 接受新增标签的内容。
...
```html
$('Selector').jqxTabs('addAt', index, tabTitle, tabContent);
```
...
下面的例子说明了 jQWidgets `jqxTabs` `addAt()` 方法。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxTabs addAt() Method
        </h3>
...
![](img/f7ab304b860c9d7265c52d87f324bbe7.png)
