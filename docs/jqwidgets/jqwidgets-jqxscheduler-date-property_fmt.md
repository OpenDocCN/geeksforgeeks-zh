# jQWidgets jqxScheduler 日期属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-date-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-date-property/)
...
`date` 属性用于设置或返回显示的 `jqxScheduler` 的日期。它的类型是 `jqxDate`，缺省值是 `new $.jqx.date('todayDate ')`。
...
以下示例说明了 `jQWidgets` 中的 `jqxScheduler` `date` 属性。
