...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdockpanel-width-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdockpanel-width-property/)
...
`width` 属性用于设置或获取指定 `jqxDockPanel` 的宽度。
...
下面的示例说明了 `jQWidgets` `jqxDockPanel` `width` 属性。在以下示例中，`width` 属性的值已设置为 300。
