# jqxKnob 小部件的 allowValueChangeOnMouseWheel 属性
...
`allowValueChangeOnMouseWheel` 属性用于设置或获取鼠标滚轮上指定的 `jqxKnob` 的值是否可以更改。

## 语法

### 用于设置 `allowValueChangeOnMouseWheel` 属性

```javascript
$('#jqxKnob').jqxKnob({allowValueChangeOnMouseWheel: true });
```

### 获取 `allowValueChangeOnMouseWheel` 属性
...
以下示例说明了 jQWidgets `jqxKnob` 的 `allowValueChangeOnMouseWheel` 属性。
