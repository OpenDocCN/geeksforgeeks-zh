# jQWidgets jqxTabs 精选特性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtabs-selecteditem-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtabs-selecteditem-property/)
...
`selectedItem` 属性用于设置或返回所选标签。它接受数字类型值，默认值为 0。
...
```html
$('selector').jqxTabs({ selectedItem: Number });
```

返回 `selectedItem` 属性。

```html
var selectedItem = $('selector').jqxTabs('selectedItem');
```
...
下面的例子说明了 jQWidgets `jqxTabs` `selectedItem` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxTabs selectedItem Property
        </h3>
...
![](img/4ae118405c29a564ed3c525e2ec5bec3.png)
