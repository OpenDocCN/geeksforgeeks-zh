...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-indeterminateindex-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-indeterminateindex-method/)
...
当 `checkboxes` 属性值设置为 `true` 时，`indeterminateIndex()` 方法用于设置列表项的不确定状态。它接受数字类型的单个参数 `index`，并且不返回任何值。
...
下面的示例说明了 jQWidgets 中的 `jqxDropDownList` `indeterminateIndex()` 方法。
