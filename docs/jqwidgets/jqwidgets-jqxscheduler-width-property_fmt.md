# jQWidgets jqxScheduler 宽度属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-width-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-width-property/)
...
`宽度属性`用于设置或返回显示的 `jqxScheduler` 的宽度。它是数字或字符串类型，默认值为 `800`。
...
```html
$("#Selector").jqxScheduler({ width: 800 });
```

*   返回 `width` 属性。

```html
var width = $('#Selector').jqxScheduler('width');
```
...
下面的示例说明了 jQWidgets 中的 `jqxScheduler` 宽度属性。
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxScheduler width property</h3>
...
![](img/cc02f3549379e1f653f1b6cb40863d1d.png)
