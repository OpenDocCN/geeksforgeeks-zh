# jQWidgets jqxSlider 显示按钮属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxslider-showbuttons-property/](https://www.geeksforgeeks.org/jqwidgets-jqxslider-showbuttons-property/)
...
`showButtons` 属性用于设置或返回是否显示滚动按钮。它接受布尔类型值，默认值为 `true`。
...
```javascript
$('selector').jqxSlider({ showButtons: Boolean });
```

返回 `showButtons` 属性。

```javascript
var showButtons = $('selector').jqxSlider('showButtons');
```
...
下面的例子说明了 jQWidgets `jqxSlider` `showButtons` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxSlider showButtons Property
        </h3>
...
![](img/0a2ff2ddbac6c92a59993742d05fea92.png)
