...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxribbon-removeat-method/](https://www.geeksforgeeks.org/jqwidgets-jqxribbon-removeat-method/)
...
`removeAt()` 方法用于使用给定的索引号从功能区小部件中移除项目。它接受数字类型的单个参数 `index`，并且不返回值。
...
### 示例:
下面的示例说明了 jQWidgets `jqxRibbon` `removeAt()` 方法。
