...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxdragdrop-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdragdrop-disabled-property/)
...
`disabled` 属性用于设置或返回拖动小部件被禁用。它接受布尔类型值，默认值为 `false`。
...
以下示例说明了 jQWidgets 中的 `jqxDragDrop` `disabled` 属性。
