# jQWidgets jqxDropDownList filterDelay 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-filterdelay-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-filterdelay-property/)
...
*filterDelay* 属性用于设置或返回滤波器延迟。它接受数字类型的值，默认值为 100。
...
```html
$('selector').jqxDropDownList({ filterDelay: Number });
```

返回 *filterDelay* 属性。

```html
var filterDelay = $('selector').jqxDropDownList('filterDelay');
```
...
以下示例说明了 `jQWidgets` 中的 `jqxDropDownList` *filterDelay* 属性。
...
![](img/a9a6b41b2f51af171cf02db6cc58a80a.png)
