...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-rowsheight-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-rowsheight-property/)
...
`rowsHeight` 属性用于设置或返回显示的 `jqxScheduler` 行的高度。它属于数字类型，默认值为 27。
...
```html
$("#Selector").jqxScheduler({ rowsHeight: 30 });
```

*   返回 `rowsHeight` 属性。

```html
var rowsHeight = $('#Selector').jqxScheduler('rowsHeight');
```
...
下面的示例说明了 jQWidgets 中的 `jqxScheduler` `rowsHeight` 属性。
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxScheduler rowsHeight property </h3>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#jqxBtn").on("click", function () {
                var rh = $('#jqxs').jqxScheduler('rowsHeight');
                $('#log').text("Rows height: " + rh);
            });
        });
    </script>
</body>
...
![](img/865215da503b3de31eb58c284a26bcf9.png)
