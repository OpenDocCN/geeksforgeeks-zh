...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxinput-source-property/](https://www.geeksforgeeks.org/jqwidgets-jqxinput-source-property/)

## 简介
jQWidgets 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大、优化、独立于平台并且得到广泛支持的框架。`jqxInput` 用于表示包含自动完成功能的 jQuery 输入小部件。

## 源属性
`source` 属性用于设置小部件的数据源。`source` 函数接受两个参数，输入文本值和一个回调函数。它接受数组或函数类型值，默认值为 `[]`。
...
下面的例子说明了 jQWidgets 中的 `jqxInput` `source` 属性。
