...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-enablehover-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-enablehover-property/)
...
`enableHover` 属性用于启用/禁用悬停状态。它接受布尔类型值，默认值为 `true`。
...
```javascript
$('selector').jqxDropDownList({ enableHover: Boolean });
```

返回 `enableHover` 属性。

```javascript
var enableHover = $('selector').jqxDropDownList('enableHover');
```
...
下面的例子说明了 jQWidgets 中的 `jqxDropDownList` `enableHover` 属性。
...
![](img/518267bfb77e220d6c9b63b3953b86bd.png)
