# jQWidgets jqxSlider 宽度属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxslider-width-property/](https://www.geeksforgeeks.org/jqwidgets-jqxslider-width-property/)
...
`宽度属性`用于设置或返回 `jqxSlider` 小部件的宽度。它接受数字/字符串类型的值，默认值为 300。
...
```html
$('selector').jqxSlider({ width: Number/String });
```

返回 `width` 属性。

```html
var width = $('selector').jqxSlider('width');
```
...
下面的例子说明了 jQWidgets `jqxSlider` `宽度属性`。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxSlider width Property
        </h3>
...
![](img/330d713f17d577198bcab33ea8db43e9.png)
