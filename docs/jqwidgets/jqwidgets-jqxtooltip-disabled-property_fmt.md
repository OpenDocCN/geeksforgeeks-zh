# jQWidgets jqxTooltip 禁用属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxtooltip-disabled-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtooltip-disabled-property/)
...
`禁用属性`用于设置或返回工具提示是否禁用。它接受布尔值，默认值为 `false`。
...
```html
$('Selector').jqxTooltip({ disabled: Boolean });
```

返回 `disabled` 的属性。

```html
var disabled = $('Selector').jqxTooltip('disabled');
```
...
下面的例子说明了 jQWidgets `jqxTooltip` `disabled` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxTooltip disabled Property
        </h3>
        <br><br>
...
![](img/4582e4ee1221cb1f142dcf6bf5cdef79.png)
