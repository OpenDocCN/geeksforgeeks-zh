# jQWidgets jqxNumberInput textAlign 属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-textalign-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-textalign-property/)
...
`textAlign` 属性用于设置或返回 `jqxNumberInput` 小部件的对齐方式。它接受字符串类型的值，默认值为 `"right"`。
...
```javascript
$('selector').jqxNumberInput({ textAlign: String });
```

返回文本对齐属性。

```javascript
var textAlign = $('selector').jqxNumberInput('textAlign');
```
...
下面的例子说明了 jQWidgets jqxNumberInput `textAlign` 属性。
...
![](img/5855f4ad1c9bd9982d3afa96d66fd501.png)
