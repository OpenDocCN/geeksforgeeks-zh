# jQWidgets jqxNumberInput spinButtonsStep 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-spinbuttonsstep-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-spinbuttonsstep-property/)
...
`spinButtonsStep` 属性用于设置或返回增加/减少步长。它接受数字类型值，默认值为 1。
...
```html
$('selector').jqxNumberInput({ spinButtonsStep: Number });
```

返回 `spinButtonsStep` 属性。

```html
var spinButtonsStep = $('selector').jqxNumberInput('spinButtonsStep');
```
...
下面的例子说明了 jQWidgets `jqxNumberInput` `spinButtonsStep` 属性。
...
![](img/b9966aa4c69f478a17d502bc3ac1e8a6.png)
