...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxgrid-refreshdata-method/](https://www.geeksforgeeks.org/jqwidgets-jqxgrid-refreshdata-method/)

jQWidgets 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大、优化、独立于平台并且得到广泛支持的框架。`jqxGrid` 用于说明以表格形式显示数据的 jQuery 小部件。此外，它完全支持与数据的连接，以及分页、分组、排序、过滤和编辑。

## refreshdata()方法

`refreshdata()`方法用于刷新数据。它没有参数，也不返回任何内容。

## 语法

```javascript
$('#Selector').jqxGrid('refreshdata');
```
...
下面的例子说明了 jQWidgets 中的 `jqxGrid` `refreshdata()`方法。
...
<body>
    <center>
        ...
        <h3>jQWidgets jqxGrid refreshdata()
            method
        </h3>
        <br />
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#jqxBtn").on("click", function () {
                $('#jqxg').jqxGrid('refreshdata');
                $("#log").html("Data Refreshed!");
            });
        });
    </script>
</body>
...
![](img/c72e02b5b6e4faa5b66bed40e24335ef.png)
