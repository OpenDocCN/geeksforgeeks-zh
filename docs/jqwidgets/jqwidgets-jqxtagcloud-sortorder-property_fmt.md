...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtagcloud-sortorder-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtagcloud-sortorder-property/)
...
`sortOrder` 属性用于指定指定 `jqxTagCloud` 的标签的排序顺序。
...
下面的示例说明了 `jQWidgets` `jqxTagCloud` `sortOrder` 属性。在下面的例子中，`sortOrder` 属性的值被设置为 `'descending'`。
