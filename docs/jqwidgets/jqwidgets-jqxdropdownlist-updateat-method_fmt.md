...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-updateat-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-updateat-method/)
...
`updateAt()` 方法用于将一个项目更新到下拉列表中。它接受两个参数，第一个是对象类型的 `item`，第二个是数字类型的 `index`。它不返回值。
...
```html
$("Selector").jqxDropDownList('updateAt', 'item', index);
```
...
下面的例子说明了 jQWidgets 中的 `jqxDropDownList` `updateAt()` 方法。
...
<body>
    <center>
        ...
        <input id="jqxBtn" type="button" 
            value="Update Item at Given Index" 
...
![](img/7e8b2ef618750bc0c59f2ed2c3611206.png)
