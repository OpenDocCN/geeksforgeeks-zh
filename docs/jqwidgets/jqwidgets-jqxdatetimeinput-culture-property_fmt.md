# jQWidgets jqxDateTimeInput 区域性属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxdatetimeinput-culture-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdatetimeinput-culture-property/)
...
`区域性属性`用于设置或返回 `jqxDateTimeInput` 小部件的区域性。要设置这个属性，首先我们需要包括
...
```javascript
$('selector').jqxDateTimeInput({ culture: String });
```

返回区域性属性。

```javascript
var culture = $('selector').jqxDateTimeInput('culture');
```
...
> `<script type="text/javascript" src="jqwidgets/jqxc`

下面的示例说明了 jQWidgets 中的 `jqxDateTimeInput` 区域性属性。
...
<h3>
            jQWidgets jqxDateTimeInput culture Property
        </h3>
...
![](img/2901dada4a501a3dd780aea0c725425c.png)
