# jQWidgets jqxNumberInput negativeSymbol 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-negativesymbol-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-negativesymbol-property/)
...
`negativeSymbol` 属性用于设置或返回用作否定符号的字符串。它接受字符串类型的值，默认值为“-”。

## 语法

设置 `negativeSymbol` 属性。

```javascript
$('selector').jqxNumberInput({ negativeSymbol: String });
```

返回 `negativeSymbol` 属性。
...
下面的例子说明了 jQWidgets `jqxNumberInput` `negativeSymbol` 属性。
...
![](img/050a433dbca6c919a0746e73805fd8a4.png)
