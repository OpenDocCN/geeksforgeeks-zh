...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-isopened-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-isopened-method/)
...
`isOpened()` 方法用于检查弹出窗口是否打开。不接受任何参数，返回 `true`，如果弹出窗口打开，否则返回 `false`。
...
下面的示例说明了 jQWidgets 中的 `jqxDropDownList` `isOpened()` 方法。
