# jQWidgets jqxDropDownList itemHeight 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-itemheight-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-itemheight-property/)
...
`itemHeight` 属性用于设置或返回 `jqxDropDownList` 项目的高度。如果 `itemHeight` 等于 `-1`，则项目高度被设置为所需高度。它接受数字类型值，默认值为 `-1`。
...
**示例:** 下面的示例说明了 `jQWidgets` 中的 `jqxDropDownList` `itemHeight` 属性。
