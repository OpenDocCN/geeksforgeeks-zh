# jQWidgets jqxNumberInput groupSeparator 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-groupseparator-property/](https://www.geeksforgeeks.org/jqwidgets-jqxnumberinput-groupseparator-property/)
...
`groupSeparator` 属性用于设置或返回分隔输入字段数字组的字符串。它将数值中小数点左边的数字分开。它接受字符类型值，默认值为','。
...
下面的例子说明了 jQWidgets jqxNumberInput `groupSeparator` 属性。
