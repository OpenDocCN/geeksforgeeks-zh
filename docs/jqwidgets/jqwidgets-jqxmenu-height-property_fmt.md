# jQWidgets jqxMenu 高度属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxmenu-height-property/](https://www.geeksforgeeks.org/jqwidgets-jqxmenu-height-property/)
...
`height` 属性用于设置或返回 `jqxMenu` 小部件的高度。它接受数字/字符串类型的值，默认值为空。
...
```javascript
$('selector').jqxMenu({ height: Number/String });
```

返回 `height` 属性。

```javascript
var height = $('selector').jqxMenu('height');
```
...
下面的例子说明了 jQWidgets `jqxMenu` `height` 属性。
...
<body>
    ...
    <h3>
        jQWidgets jqxMenu height Property
    </h3>
...
![](img/43de55c5fd509bfe1986b780ffc8138e.png)
