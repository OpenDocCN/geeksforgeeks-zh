...
## appointmentsMinHeight 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-appointmentsminheight-property/](https://www.geeksforgeeks.org/jqwidgets-jqxscheduler-appointmentsminheight-property/)
...
`appointmentsMinHeight` 属性用于设置或返回 `jqxScheduler` 中显示的约会的最小高度。它属于数字类型，其默认值为 18。

### 语法

*   设置 `appointmentsMinHeight` 属性。

```html
$("#Selector").jqxScheduler({ appointmentsMinHeight: 20 });
```

*   返回 `appointmentsMinHeight` 属性。

```javascript
var appointmentsMinHeight = $('#Selector').jqxScheduler('appointmentsMinHeight');
```
...
下面的示例说明了 `jQWidgets` 中的 `jqxScheduler` `appointmentsMinHeight` 属性。
