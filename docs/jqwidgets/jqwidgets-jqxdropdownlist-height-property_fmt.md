# jQWidgets jqxDropDownList 高度属性

> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-height-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-height-property/)
...
`height` 属性用于设置或返回 `jqxDropDownList` 小部件的高度。它接受数字/字符串类型的值，默认值为空。
...
```javascript
$('selector').jqxDropDownList({ height: Number });
```

返回 `height` 属性。

```javascript
var height = $('selector').jqxDropDownList('height');
```
...
以下示例说明了 jQWidgets 中的 `jqxDropDownList` `height` 属性。
...
![](img/7ed9867c54b6f7b23b3c849827aa5b23.png)
