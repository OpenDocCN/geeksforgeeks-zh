...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-checkindex-method/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-checkindex-method/)
...
当“复选框”属性值设置为真时，`checkIndex()`方法用于检查列表项。它接受数字类型的单个参数`index`，并且不返回任何值。索引从 0 开始。

## 语法
```html
$("Selector").jqxDropDownList('checkIndex', index );
```
...
## 示例
下面的示例说明了`jQWidgets`中的`jqxDropDownList`的`checkIndex()`方法。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxDropDownList checkIndex() Method
        </h3>

        <div id='jqxDDL'></div>

        <input id="jqxBtn" type="button" value="checkIndex() method"
...
## 输出
![](img/04d804ed12d3bee6e097a01a3d3736b6.png)
