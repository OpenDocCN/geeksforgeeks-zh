# jqwidgets jqxtagcloud takeTopWeightedItems 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtagcloud-taketopweighteditems-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtagcloud-taketopweighteditems-property/)
...
`takeTopWeightedItems` 属性用于指示 `displayLimit` 是否为指定的 `jqxTagCloud` 优先考虑最高值成员。

## 语法

要设置 `takeTopWeightedItems` 属性:

```javascript
$('#jqxTagCloud').jqxTagCloud({ takeTopWeightedItems: true });
```

要获取 `takeTopWeightedItems` 属性:
...
下面的示例说明了 `jQWidgets` `jqxTagCloud` `takeTopWeightedItems` 属性。在下面的例子中，`takeTopWeightedItems` 属性的值被设置为 `true`。
