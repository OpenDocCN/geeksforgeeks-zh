...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-filterable-property/](https://www.geeksforgeeks.org/jqwidgets-jqxdropdownlist-filterable-property/)
...
`filterable` 属性用于设置或返回过滤是否启用。它接受布尔类型值，默认值为 `false`。
...
下面的例子说明了 jQWidgets 中的 `jqxDropDownList` `filterable` 属性。
