...
`keyboardNavigation` 属性用于启用或禁用指定 `jqxDocking` 的键盘导航。该属性接受布尔值。

## 语法

*   用于设置 `keyboardNavigation` 属性:
...
下面的示例说明了 jQWidgets `jqxDocking` 的 `keyboardNavigation` 属性。在下面的例子中，`keyboardNavigation` 属性的值被设置为 `true`。
