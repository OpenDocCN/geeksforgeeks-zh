...
> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxtreegrid-columnsreorder-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtreegrid-columnsreorder-property/)

`jQWidgets` 是一个 JavaScript 框架，用于为 PC 和移动设备制作基于 web 的应用程序。它是一个非常强大、优化、独立于平台并且得到广泛支持的框架。`jqxTreeGrid` 用于以树状结构表示数据。这个小部件对于显示分层数据的多列、数据分页、排序和筛选、数据编辑、列大小调整、固定列、条件格式、聚合和行选择非常有用。这些小部件还读取和显示来自任何类型数据源的数据，如 XML、JSON、Array、CSV 或 TSV。

`columnsReorder` 属性用于设置或获取指定 `jqxTreeGrid` 的列的重新排序。此属性接受布尔值。

## 语法

用于设置 `columnsReorder` 属性。

```javascript
$('#treeGrid').jqxTreeGrid({columnsReorder: true});
```

为获得 `columnsReorder` 属性。
...
下面的示例说明了 jQWidgets `jqxTreeGrid` 的 `columnsReorder` 属性。在下面的示例中，`columnsReorder` 的值已被设置为 `true`。
