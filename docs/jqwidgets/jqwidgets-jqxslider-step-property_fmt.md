# jQWidgets jqxSlider `step` 属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxslider-step-property/](https://www.geeksforgeeks.org/jqwidgets-jqxslider-step-property/)
...
`step` 属性用于用户使用键盘箭头时设置或返回滑块的步长。它接受数字类型值，默认值为 1。
...
```javascript
$('selector').jqxSlider({ step: Number });
```

返回 `step` 属性。

```javascript
var step = $('selector').jqxSlider('step');
```
...
下面的例子说明了 jQWidgets jqxSlider `step` 属性。
...
<body>
    <center>
        ...
        <h3>
            jQWidgets jqxSlider step Property
        </h3>
...
![](img/f22fb8435532ac07755a92d542b363bd.png)
