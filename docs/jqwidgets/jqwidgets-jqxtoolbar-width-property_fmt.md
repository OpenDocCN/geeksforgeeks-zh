...
> 原文：[https://www.geeksforgeeks.org/jqwidgets-jqxtoolbar-width-property/](https://www.geeksforgeeks.org/jqwidgets-jqxtoolbar-width-property/)
...
`width` 属性用于设置或返回显示的 `jqxToolBar` 的宽度。它属于数字或字符串类型，默认值为 `null`。
...
以下示例说明了 `jQWidgets` 中的 `jqxToolBar` `width` 属性。
