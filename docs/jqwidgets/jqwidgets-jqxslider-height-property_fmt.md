# jQWidgets jqxSlider 高度属性

> 原文: [https://www.geeksforgeeks.org/jqwidgets-jqxslider-height-property/](https://www.geeksforgeeks.org/jqwidgets-jqxslider-height-property/)
...
`height` 属性用于设置或返回滑块部件的高度。它接受数字/字符串类型的值，默认值为 35。
...
```javascript
$('selector').jqxSlider({ height: Number/String });
```

返回 `height` 属性。

```javascript
var height = $('selector').jqxSlider('height');
```
...
下面的例子说明了 jQWidgets jqxSlider `height` 属性。
...
![](img/4d019985f0181ee6127ce8e960f93c8b.png)
