...
> 原文: [https://www.geeksforgeeks.org/scipy-stats-tvar-function-python/](https://www.geeksforgeeks.org/scipy-stats-tvar-function-python/)
...
- `array`: 输入具有计算修剪方差的元素的数组或对象。
- `limits`: 数组的下限和上限要考虑，小于下限或大于上限的值会被忽略。如果限制为 `None` [默认值]，则使用所有值。
- `including`: 决定是包含等于下限还是上限的值，还是在计算时将其排除。

## 返回值

根据设置的参数，对数组元素的方差进行修剪。
