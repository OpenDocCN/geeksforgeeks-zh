...
> 原文: [https://www.geeksforgeeks.org/python-scipy-integrate-romberg-method/](https://www.geeksforgeeks.org/python-scipy-integrate-romberg-method/)

借助 `scipy.integrate.romberg()` 方法，我们可以得到一个可调用函数从极限 `a` 到极限 `b` 的龙贝格积分。
...
在这个示例中，我们可以看到，通过使用 `scipy.integrate.romberg()` 方法，我们能够获得可调用函数从极限 `a` 到极限 `b` 的龙贝格积分。
