...
*   `lambertw` – 计算任意复数 `z` 的 `W(z) * exp(W(z))`，其中 `W` 是朗伯图函数
*   `logsumexp` – 给出给定数字的指数和的对数
*   `perm` – 给出元素的排列
...
> **语法:** `scipy.special.exprel(input_data)`
...
> **语法:** `scipy.special.gamma(input_data)`
>
> 其中，`input_data` 为输入数字。
...
> **语法:** `scipy.special.lambertw(input_data)`
