...
> 原文：[https://www.geeksforgeeks.org/scipy-stats-kurtosistest-function-python/](https://www.geeksforgeeks.org/scipy-stats-kurtosistest-function-python/)
...
- `array`：输入数组或有元素的对象。
- `axis`：轴，将沿该轴计算 `kurtosistest`。默认情况下，`axis=0`。

## 返回

正态分布数据集的 Z 值（统计值）和 P 值。
