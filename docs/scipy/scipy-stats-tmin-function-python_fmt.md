...
> 原文: [https://www.geeksforgeeks.org/scipy-stats-tmin-function-python/](https://www.geeksforgeeks.org/scipy-stats-tmin-function-python/)

`scipy.stats.tmin(array, lowerlimit=None, axis=0, inclusive=True)` 函数沿指定轴计算数组元素的修剪最小值，同时忽略指定限制之外的值。

## 参数

- `array`: 输入数组或对象中具有计算最小值的元素。
- `axis`: 【默认值为零】输入包含要计算最小值的元素的数组或对象。
- `lowerlimit`: 数组的下限和上限要考虑，小于下限或大于上限的值将被忽略。如果限制为无[默认值]，则使用所有值。
- `inclusive`: 决定是包含等于下限还是上限的值，还是在计算时将其排除。
