...
> 原文: [https://www.geeksforgeeks.org/scipy-cluster-hierarchy-dendrogram/](https://www.geeksforgeeks.org/scipy-cluster-hierarchy-dendrogram/)
...
*   分级和聚集聚类的功能由 `hierarchy` 模块提供。
*   为了执行分级聚类，使用 `scipy.cluster.hierarchy.linkage` 函数。该功能的参数有:

> **语法:** `scipy.cluster.hierarchy.linkage(y, method='single', metric='euclidean', optimal_ordering=False)`

*   为了将分级聚类绘制为树形图，使用了 `scipy.cluster.hierarchy.dendrogram` 功能。

> **语法:** `scipy.cluster.hierarchy.dendrogram(Z, p=30, truncate_mode=None, color_threshold='default', get_leaves=True, orientation='top', labels=None, count_sort=False, distance_sort=False, show_leaf_counts=True, no_plot=False, no_labels=False, leaf_font_size=None, leaf_rotation=None, leaf_label_func=None, show_contracted=False, link_color_func=None, ax=None, above_threshold_color='C1')`
...
`scipy.cluster.hierarchy.dendrogram` 的参数 `orientation` 已设置为 `'right'`。它在右边绘制根，在左边绘制后代链接。