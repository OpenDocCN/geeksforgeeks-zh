...
> 原文：[https://www.geeksforgeeks.org/cutting-hierarchical-dendrogram-into-clusters-using-scipy-in-python/](https://www.geeksforgeeks.org/cutting-hierarchical-dendrogram-into-clusters-using-scipy-in-python/)
...
使用连锁矩阵可以很容易地绘制树形图。链接矩阵是通过`linkage()`函数创建的。该矩阵包含分层聚类的编码，以呈现为树形图。
...
>
> *   `y`：输入 1D/ 2D 阵列的输入向量
> *   `method`：计算新形成的簇与其他点之间距离的方法。`method` = 'single'，'complete'，'average'，'centroid'
> *   `metric`：在输入是观察向量集合的情况下使用的距离度量
> *   `optimal_ordering`：如果为真，将对连锁矩阵进行重新排序，使连续叶片之间的距离最小
