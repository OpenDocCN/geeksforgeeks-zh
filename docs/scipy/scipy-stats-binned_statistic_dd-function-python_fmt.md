...
> 原文: [https://www.geeksforgeeks.org/scipy-stats-binned_statistic_dd-function-python/](https://www.geeksforgeeks.org/scipy-stats-binned_statistic_dd-function-python/)
...
- **arr :** 【数组类】数据以(N，D)数组的形式传递给直方图。
- **values:** 【数组类】用于计算统计数据。
- **statistic:** 统计计算{均值、计数、中值、和、函数}。默认是卑鄙的。
- **bins:** 【int 或 scalars】如果 `bins` 为 int，则定义给定范围内等宽 bin 的数量(默认为 10)。如果面元是一个序列，它定义面元的边。
- **range:** (浮动，浮动)箱柜的上下范围，如果没有提供，范围从 `x.max()` 到 `x.min()`。
