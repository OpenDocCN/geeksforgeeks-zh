...
> 原文: [https://www.geeksforgeeks.org/scipy-stats-kurtosis-function-python/](https://www.geeksforgeeks.org/scipy-stats-kurtosis-function-python/)
...
- `array`: 输入数组或有元素的对象。
- `axis`: 测量峰度值的轴。默认情况下，`axis=0`。
- `fisher`: Bool；如果为真，则使用 Fisher 定义（正常 0.0）；否则，如果设置为 `False`，将使用 Pearson 的定义（normal 3.0）。
- `bias`: Bool；如果设置为 `False`，计算将针对统计偏差进行校正。
