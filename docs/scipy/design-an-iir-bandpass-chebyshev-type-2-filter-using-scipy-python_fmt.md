...
> 原文: [https://www.geeksforgeeks.org/design-an-iir-bandpass-chebyshev-type-2-filter-using-scipy-python/](https://www.geeksforgeeks.org/design-an-iir-bandpass-chebyshev-type-2-filter-using-scipy-python/)
...
切比雪夫类型 2 通过在阻带中加入相等的纹波，将整个阻带上理想和实际频率响应之间的绝对差异降至最小。
...
**步骤 2:** 定义用户自定义函数 `mfreqz()` 和 `impz()`。`mfreqz` 是幅值和相位图的函数，`impz` 是脉冲和阶跃响应的函数。
