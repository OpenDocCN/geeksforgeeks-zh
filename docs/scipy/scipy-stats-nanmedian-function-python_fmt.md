...
> 原文：[`https://www.geeksforgeeks.org/scipy-stats-nanmedian-function-python/`](https://www.geeksforgeeks.org/scipy-stats-nanmedian-function-python/)

`` `scipy.stats.nanmedian(array, axis=0)` `` 函数通过忽略沿数组指定轴的数组元素的 NaN（不是数字）值来计算中位数。

## 参数

*   **array**：输入数组或对象，包含用于计算中位数的元素，其中可能包括 NaN 值。
*   **axis**：计算中间值的轴。默认情况下，`axis=0`。
