# scipy.stats.scoreatpercentile() 函数

> 原文：[https://www.geeksforgeeks.org/scipy-stats-scoreatpercentile-function-python/](https://www.geeksforgeeks.org/scipy-stats-scoreatpercentile-function-python/)
...
> **参数：**
> `arr`：【array_like】输入数组。
> `per`：【array_like】我们需要分数的百分位数。
> `limit`：【tuple】计算百分位数的下限和上限。
> `axis`：【int】轴，我们需要沿着这个轴计算分数。
>
> **结果：** 相对于数组元素的百分位得分。
