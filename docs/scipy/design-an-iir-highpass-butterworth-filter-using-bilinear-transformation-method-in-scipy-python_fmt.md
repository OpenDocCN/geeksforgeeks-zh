...
> 原文: [https://www.geeksforgeeks.org/design-an-iir-highpass-butterworth-filter-using-bilinear-transformation-method-in-scipy-python/](https://www.geeksforgeeks.org/design-an-iir-highpass-butterworth-filter-using-bilinear-transformation-method-in-scipy-python/)
...
### 步骤 2: 定义用户自定义函数 `mfreqz()` 和 `impz()`

`mfreqz` 是幅度和相位图的函数，`impz` 是脉冲和阶跃响应的函数。
...
### 步骤 7: 通过 `signal.butter()` 功能，使用 `N` 和 `wc` 设计模拟巴特沃斯滤波器
