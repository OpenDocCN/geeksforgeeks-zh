...
为了创建基于固定数据点的函数，使用 `scipy.interpolate.interp1d`。它接受数据点 `x` 和 `y`，并返回一个可以用新的 `x` 调用的函数，然后返回相应的 `y` 点。

> **语法:** `scipy.interpolate.interp1d(x, y, kind, axis, copy, bounds_error, fill_value, assume_sorted)`
...
*   为了找到一维曲线的 B 样条表示，使用 `scipy.interpolate.splrep`。

> **语法:** `scipy.interpolate.splrep(x, y, w, xb, xe, k, task, s, t, full_output, per, quiet)`

*   为了计算 B 样条或其导数，使用 `scipy.interpolate.splev`。

> **语法:** `scipy.interpolate.splev(x, tck, der, ext)`
...
它是一个适合一组给定数据点的一维平滑样条。`scipy.interpolate.UnivariateSpline` 用于将 k 度的样条 `y = spl(x)` 拟合到所提供的 `x`，`y` 数据。`s` 通过指定平滑条件来指定结的数量。使用给定的平滑因子 `s` 和在最后一次调用中找到的节点进行的样条计算。

> **语法:** `scipy.interpolate.UnivariateSpline(x, y, w, bbox, k, s, ext)`
...
`scipy.interpolate.Rbf` 用于对 n 维的散乱数据进行插值。径向基函数被定义为对应于固定的参考数据点。`scipy.interpolate.Rbf` 是一类径向基函数插值，用于将函数从二维散乱数据插值到三维区域。

> **语法:** `scipy.interpolate.Rbf(*args)`
