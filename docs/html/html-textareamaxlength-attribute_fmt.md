...
The **HTML `<Textarea>` `maxlength` attribute** is used to specify the maximum number of characters that can be entered into the `<Textarea>` element.
...
**Supported Browsers:** The browsers supported by the HTML `<Textarea>` `maxlength` attribute are listed below:
