...
Youtube Player 提供播放、暂停、音量等控制，可以使用`controls`参数禁用或启用。
有两个可用的参数:
...
**注意:** 现在不推荐使用`object`和`embed`标签，所以建议使用`iframe`标签。