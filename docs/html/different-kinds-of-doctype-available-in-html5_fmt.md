...
> 原文：[https://www.geeksforgeeks.org/different-kinds-of-doctype-available-in-html5/](https://www.geeksforgeeks.org/different-kinds-of-doctype-available-in-html5/)

在本文中，我们将了解 `Doctype` 及其在 `HTML5` 中的类型。在学习 `HTML5` 中不同种类的 `Doctype` 之前，首先要熟悉术语“`Doctype`”。当你查看任何一个网站的时候，你可能已经注意到在 `<html>` 标签开始之前，`<!DOCTYPE html>` 编写完成。
...
**用法：** 在 `HTML` 的早期版本中，`Doctype` 的主要用法是创建对 `DTD`（文档类型定义）的引用，并且强制引用 `DTD`，但是在 `HTML5` 中，它不需要引用 `DTD`。文档类型定义（`DTD`）主要用于定义具有一系列有效属性和元素的结构。
