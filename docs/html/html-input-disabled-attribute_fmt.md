...
> 原文: [https://www.geeksforgeeks.org/html-input-disabled-attribute/](https://www.geeksforgeeks.org/html-input-disabled-attribute/)

HTML 中 `<input>` 元素的 **禁用** 属性用于指定输入字段被禁用。禁用的输入不可点击且不可用。它是一个布尔属性。禁用的 `<input>` 元素不在表单中提交。
