...
HTML 中的 `<button>` 标签用来定义可点击的按钮。该标签用于提交内容。图像和文本内容可以在 `<button>` 标签内使用。不同的浏览器对 `<button>` 使用不同的默认类型。按钮可以使用 CSS 来设置样式。
...
*   [表单](https://www.geeksforgeeks.org/html-form-tag/):用于创建用户输入的表单。在 `<form>` 标签中使用了许多元素。
...
*   [类型](https://www.geeksforgeeks.org/html-type-attribute/):用于指定按钮元素的按钮类型。它也用于 `<input>` 元素，以指定要显示的输入类型。
...
![输出示例](img/429f2b4384ba2f36bbedd44cde5e27d2.png)
