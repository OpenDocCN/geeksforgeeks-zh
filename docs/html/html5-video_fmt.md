...
**示例:** 这个示例说明了 `<video>` 标签的使用，其中我们使用了 `preload` 属性，该属性的值设置为 `auto`，指定了当页面加载时浏览器应该加载整个视频。
...
**示例:** 这个简单的示例说明了 `<video>` 标签在 HTML 中的使用。这里，`控件`属性用于添加播放、暂停、音量等控件，`<source>` 元素用于指定浏览器将选择播放的视频。
...
**示例:** 该示例说明了 HTML `<video>` 标签中自动播放属性的使用。
