...
`HTML` 代表超文本标记语言。超文本定义了在线页面之间的链接。它用于定义标签内的文本文档，标签定义了网站的结构。这种语言用于注释文本，以便机器能够知道它并相应地操作文本。大多数标记语言(如超文本标记语言)是人类可读的。这种语言使用标签来定义需要对文本进行什么操作。
...
**设置图像的宽度和高度:** [`width`](https://www.geeksforgeeks.org/html-input-width-attribute/) 和 [`height`](https://www.geeksforgeeks.org/html-img-height-attribute/) 属性用于指定图像的宽度和高度。默认情况下，属性值以像素为单位指定。
...
*   **像素:** `像素` (1px = 1 英寸的 1/96)
*   **英寸:** `英寸` (1 英寸= 96px = 2.54cm 厘米)
*   **pt:** `分` (1pt = 1 英寸的 1/72)

**示例 1:** 高度为 600 像素、宽度为 500 像素的图像。`width` 属性指定图片的宽度，以磅为单位。
...
**示例 2:** 另一个示例展示 [`width`](https://www.geeksforgeeks.org/html-img-width-attribute/) 和 [`height`](https://www.geeksforgeeks.org/html-img-height-attribute/) 属性的使用。我们提到宽度为 450 像素，高度为 220 像素。
...
**示例 3:** 另一个 HTML 程序展示了使用 [`<img>`](https://www.geeksforgeeks.org/html-img-tag/) 标签的 `width` 和 `height` 属性。在这段代码中，我们将 `height` 和 `width` 的值取为 250 像素。
