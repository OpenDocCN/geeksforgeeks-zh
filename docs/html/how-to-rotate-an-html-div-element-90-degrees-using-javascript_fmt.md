...
> 原文：[https://www.geeksforgeeks.org/how-to-rotate-an-html-div-element-90-degrees-using-javascript/](https://www.geeksforgeeks.org/how-to-rotate-an-html-div-element-90-degrees-using-javascript/)

使用`transform`属性可以将元素旋转 90 度。此属性用于移动、旋转、缩放和其他对元素执行各种变换。
可以使用`rotate()`变换函数作为旋转元素的值。它采用一个定义旋转角度的参数。旋转角度由两部分组成，旋转值后跟旋转单位。单位可以用度数(`deg`)、坡度(`grad`)、弧度(`rad`)和转角(`turn`)来定义。
