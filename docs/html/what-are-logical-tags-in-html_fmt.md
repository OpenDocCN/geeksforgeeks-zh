...
> 原文: [https://www.geeksforgeeks.org/什么是 html 中的逻辑标签/](https://www.geeksforgeeks.org/what-are-logical-tags-in-html/)
...
| `<acronym>` | 定义缩略词。 |
| `<address>` | 个人或组织的联系信息。 |
| `<cite>` | 定义引用。它以斜体格式显示文本。 |
| `<code>` | 定义一段计算机代码。 |
| `<blockquote>` | 定义长引号。 |
| `<del>` | 定义已删除的文本，用于标记已从文档中删除的部分文本。 |
| `<dfn>` | 定义定义元素，用于在 HTML 中表示定义实例。 |
| `<ins>` | 定义插入的文本。 |
| `<kbd>` | 定义键盘输入文本。 |
| `<pre>` | 定义预格式化文本块，该文本块保留网页浏览器忽略的文本空格、换行符、制表符和其他格式字符。 |
| `<q>` | 定义短引号。 |
| `<samp>` | 定义计算机程序的示例输出文本。 |
| `<strong>` | 定义强文本，即显示文本的重要性。 |
| `<var>` | 定义数学方程或计算机程序中的变量。 |
...
**[**3. `<ins>`标签:**](https://www.geeksforgeeks.org/html-ins-tag/)**`<ins>`标签用于显示文档中插入了一些文字。浏览器通常会显示带下划线的插入标签。**
...
**[**6. `<kbd>`标签:**](https://www.geeksforgeeks.org/html-kbd-tag/)**`<kbd>`标签用于表示显示的文本是从键盘输入的。里面的内容以等间距字体显示。**
