...
> 原文: [https://www.geeksforgeeks.org/html-aria-labelledby-attribute/](https://www.geeksforgeeks.org/html-aria-labelledby-attribute/)

**aria-labelledby** 属性是 HTML 中的一个内置属性，用于创建对象及其标签之间的关系。当任何元素同时包含属性 **aria-labelledby** 和 **aria-label** 时，浏览器的高优先级毫无疑问将是 **aria-labelledby**。这个 aria-labelledby 属性可以用于任何典型的 HTML 表单元素；它不局限于元素而是 **aria-label** 属性。我们在使用 aria-label 的时候要小心，因为它并不适用于所有的 HTML 元素。

**语法:**

```html
<element aria-labelledby="label1 label2 ..."> Content </element>
```
