...
HTML 中的 `<legend>` 对齐属性用于指定 `<fieldset>` 元素中标题的对齐方式。除 Opera 12 及更早版本外，主要浏览器都支持 `<legend>` 元素的左右对齐。任何浏览器都不支持底部对齐。
**语法:**
...
**注意:** HTML 5 不支持 `<legend>` 对齐属性。
以下示例说明了 `align` 属性在 `<legend>` 元素中的使用:
**示例:**
...
![Output Image](img/52b4fbd4028efc050e4e02601ed46b25.png)

**支持的浏览器:**以下是 `<legend>` 对齐属性支持的浏览器:
