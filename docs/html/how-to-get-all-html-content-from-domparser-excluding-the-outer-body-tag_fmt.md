...
> 原文：[https://www.geeksforgeeks.org/how-to-get-all-html-content-from-domparser-excluding-the-outer-body-tag/](https://www.geeksforgeeks.org/how-to-get-all-html-content-from-domparser-excluding-the-outer-body-tag/)
...
2.  使用 `.parseFromString()` 函数解析文档。它需要两个参数，待解析的字符串和文档类型。
    **语法：**
    ```html
    const parsedDocument = parser.parseFromString(htmlInput, "text/html");
    ```

3.  使用 `doc.all` 元素访问整个 HTML 页面，现在获取其存储在索引 `0` 处的根元素。我们也可以使用 `getElementById()` 来获取特定元素的内容。
    **语法：**
    ```html
...
<body>
    <h2>
        DomParser to get all HTML content
    </h2>
    <p>
        Click on the button Below to parse the HTML document
    </p>
    <!-- Paragraph element to show the output -->
    <p id="output"> </p>
    <!-- Button to call the parsing function -->
...
也可以使用 `getElementsByClassName('className')` 和 `getElementById('IDName')` 检索单个组件的文本内容。
...
function parse(htmlInput) {
    ...
    // and storing the returned HTML object in a variable
    const parsedDocument = parser.parseFromString(htmlInput, "text/html");
    // Retrieve all text content from DOM object
    var allText = parsedDocument.all[0].textContent;
    // Printing the output to webpage and console
    console.log(parsedDocument.all[0].textContent);
}
```