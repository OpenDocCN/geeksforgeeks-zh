...
SVG(可缩放矢量图形)模式是使用 HTML 在图形中定义模式的元素。要使用创建 SVG 图案，您必须在 SVG 内部定义一个 `<pattern>`，之后，您可以定义该 `<pattern>` 内部的形状。现在必须通过指定颜色、边框等来定义形状区域。
...
*   **视口:**给出特定部分的图像。
*   **patternUnits:**定义 x、y、高度、宽度的坐标系。默认值为 ObjectBoundingBox。
*   **patternContentUnits:**它定义了里面的内容。默认为用户空间用户。
