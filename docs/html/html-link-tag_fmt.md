...
**示例:**在本例中，我们使用了`<link>`标签在 HTML 中声明了标签内部的`rel`属性和`type`属性。
...
链接标签用于定义文档和外部资源之间的链接。链接标签主要用于链接到外部样式表。这个元素可以出现多次，但它只出现在头部。链接元素是空的，它只包含属性。`link`元素中的值表示被链接到项与包含文档的关系。
...
*   **charset**:用于指定 HTML 链接文档的字符编码。
*   **crossOrigin**:指定链接文档的 CORS 设置。
*   **disabled**:用于指定链接单据禁用。
*   **href**:用于指定链接文档的网址。
*   **hreflang**:用于指定链接文档的语言。
*   **media**:用于指定目标资源优化的媒体/设备。
*   **rel**:用于指定当前与链接单据的关系。
*   **rev**:将链接单据的反向关系赋给当前单据。
*   **sizes**:用于指定视觉媒体的图标尺寸，仅在 rel =“icon”时有效。
*   **target**:用于指定加载链接文档的窗口或框架。
