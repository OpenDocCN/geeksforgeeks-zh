...
> 原文：[https://www.geeksforgeeks.org/how-to-write-address-in-an-html-document/](https://www.geeksforgeeks.org/how-to-write-address-in-an-html-document/)

HTML 中的 `<address>` 标签用来写一个人或者一个组织的联系方式。如果在 `<body>` 标签内使用 `<address>` 标签，则它代表文档的联系信息，如果在 `<article>` 标签内使用 `<address>` 标签，则它代表物品的联系信息。

`<address>` 标签内的文本将以斜体格式显示。一些浏览器在地址元素前后添加了换行符。
