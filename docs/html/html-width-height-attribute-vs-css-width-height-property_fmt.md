...
> 原文: [https://www.geeksforgeeks.org/html-width-height-attribute-vs-CSS-width-height-property/](https://www.geeksforgeeks.org/html-width-height-attribute-vs-CSS-width-height-property/)

![](img/06576a56e626c7bc08ba5e3d5fcf9ed4.png)

在 HTML 5 中，很少有元素遵循宽度和高度属性，最大元素不遵循该属性。像 `img`、`iframe`、`canvas`，以及 `svg` 一类的元素遵循宽度和高度属性，但是 `div`、`span`、`article` 和 `section` 等元素不遵循该属性。

宽度和高度属性在 `img`、`svg` 标签中受到影响，那些是弱类型的样式，可以被 CSS 覆盖。如果您为 `img` 标签分配了 `width="500"` 和 `height="200"`，并为 `img` 标签加载 CSS 宽度和高度属性，如 `width: 400px`，那么 500 会暗淡，400 会影响图像。
