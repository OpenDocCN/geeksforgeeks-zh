...
> 原文：[https://www.geeksforgeeks.org/how-to-merge-table-cells-in-html/](https://www.geeksforgeeks.org/how-to-merge-table-cells-in-html/)

本文的目的是合并 HTML 中的表格单元格。可以使用 HTML 中的 `rowspan` 和 `colspan` 属性来完成。`rowspan` 用于合并或合并一行中的单元格数，而 `colspan` 用于合并表格中的列单元格。
