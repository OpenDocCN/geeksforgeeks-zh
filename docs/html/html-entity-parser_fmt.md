        ...
        for (int i = 0; i < input.size(); i++) {
            ...
            if (input[i] == '&') {
                ...
                while (i < input.size()) {
                    ...
                    if (input[i] == ';' && m.find(buffer) != m.end()) {
                        // Append the parsed
                        // character
                        output = output + m[buffer];
...
string parseInputString(string input) {
    ...
    for (const auto& pair : m) {
        // Replace the entity with its mapped value
        input = regex_replace(input, regex(pair.first), pair.second);
    }
