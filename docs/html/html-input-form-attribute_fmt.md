...
**HTML `<input>` 表单属性**用于指定 `<input>` 元素可以包含一个或多个表单。
...
**属性值:** 包含单个值 `form_id`，该值包含指定按钮元素所属的一个或多个的 `form_id`。该属性的值应该是 `<form>` 元素的 id。

**示例:** 本示例说明了在 `<input>` 元素的 `form` 属性的使用。
...
**支持的浏览器:** 支持的浏览器 **HTML `<input>` 表单属性**如下:
