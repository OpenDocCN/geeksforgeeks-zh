...
此示例显示了简单的 `onclick` 事件，单击按钮以 HTML 格式显示日期&时间。
...
该属性包含一个单值脚本，当鼠标点击元素时可以工作。

## 支持的标签

除了 `<base>`, `<bdo>`, `<br>`, `<head>`, `<html>`, `<iframe>`, `<meta>`, `<param>`, `<script>` 和 `<style>` 以及 `<title>` 外，所有 HTML 标签都支持该属性。

## 示例 1

在本例中，当点击“点击此处”时，将显示文本“欢迎来到极客论坛”。
