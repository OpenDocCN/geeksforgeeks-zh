...
在这篇文章中，我们将学习如何用 HTML 邮件链接发送邮件。`mailto`链接是消费者想要沟通或者想要反馈时默认的发送邮件方式，然后点击`mailto`链接会打开默认的发送邮件窗口。所以我们可以使用`mailto`，一旦表单提交，它会指引我们到邮件。使用`mailto`我们可以设置表单的`action`字段，在这种情况下，web 浏览器调用电子邮件客户端将表单提交发送到指定的电子邮件地址。此外，我们还可以附上抄送、密件抄送、主题和正文内容。
...
*   `bcc:` 此参数保存另一封将接收邮件的盲复件的邮件，可选。
*   `subject:` 此参数保存邮件的主题，可选。
*   `body:` 此参数保存邮件内容，可选。
*   `?`: 此参数为第一个参数分隔符，可选。
*   `&`: 这个保存其他参数分隔符，可选。
...
为了将邮件发送到指定的邮件地址，我们将使用`mailto:`设置表单的`action`字段，网络浏览器将调用电子邮件客户端将表单提交发送到指定的电子邮件地址。点击邮件链接，它将打开一个默认的发送邮件窗口，而不必复制它并输入到电子邮件客户端。

下面的例子说明了 HTML 中`mailto`链接的使用。

### 示例 1

在本例中，我们使用了 HTML 表单作为姓名、联系电话&文本区提交建议。
...
这个示例说明了如何使用`mailto`链接通过 HTML 中的邮件提交反馈表单。
