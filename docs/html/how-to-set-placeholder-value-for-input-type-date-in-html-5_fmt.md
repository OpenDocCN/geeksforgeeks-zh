...
> 原文：[https://www.geeksforgeeks.org/how-to-set-placeholder-value-for-input-type-date-in-html-5/](https://www.geeksforgeeks.org/how-to-set-placeholder-value-for-input-type-date-in-html-5/)

`placeholder` 属性不适用于输入类型 `date`，因此无法在 `input type="date"` 中直接放置任何值作为占位符。您可以在输入字段中使用 `onfocus` 事件。因为您需要为输入类型 `date` 设置一个自定义占位符值，并且您有一个下拉日历，用户可以从中选择日期。
...
**注意：** 点击左侧字段后，得到 `dd/mm/yyyy` 框，再次点击后得到日历下拉菜单。这是因为输入框实际上是一个 `text` 输入，但是点击它（即聚焦于它），我们得到 `date` 类型输入。但是右边的字段是普通的文本输入字段。
...
*   失去焦点后，由于我们介绍的 `onblur` 事件，它会变回文本字段：
