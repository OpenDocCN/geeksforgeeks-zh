...
> 原文: [https://www.geeksforgeeks.org/how-to-select-multiple-options-at-once-in-dropdown-list-in-html5/](https://www.geeksforgeeks.org/how-to-select-multiple-options-at-once-in-dropdown-list-in-html5/)

下拉列表是 HTML 中最灵活的元素之一。它类似于无线电输入，即默认情况下只能从一组项目中选择一个项目。但是，当`multiple`属性与`<select>`元素一起使用时，我们可以从列表中选择多个选项。`multiple`属性是一个布尔属性，用于指定是否可以一次选择多个选项。

选择多个选项的过程因不同的操作系统和浏览器而异，如下所述:
*   **窗口**: 我们需要按住`CTRL`按钮选择多个选项。
*   **Mac**: 我们需要按住`Command`按钮选择多个选项。

请注意，由于处理这一问题的方式不同，并且必须通知用户有多个选项可用，因此使用复选框会更方便用户。

下面的例子演示了`multiple`属性的使用:
