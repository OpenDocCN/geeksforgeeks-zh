...
HTML [`<map>`](https://www.geeksforgeeks.org/html-map-tag/) 标签用于在 HTML 中定义图像地图。图像内可点击的区域使用 [`<area>`](https://www.geeksforgeeks.org/html-area-tag/) 标签定义。
...
*   `name`: 用于将地图与其使用的图像关联起来。

## 注意:

地图中的 `name` 属性必须与图像的 [`usemap`](https://www.geeksforgeeks.org/html-area-tag/) 属性具有相同的值。

## 示例:

在本例中，突出显示的区域是映射的区域，点击它将指向网站。
