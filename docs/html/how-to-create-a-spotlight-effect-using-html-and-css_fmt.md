...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-spotlight-effect-using-html-and-css/](https://www.geeksforgeeks.org/how-to-create-a-spotlight-effect-using-html-and-css/)
...
**CSS 版块：** 在这个版块中，`CSS` 被用来给我们的 `HTML` 页面赋予不同类型的动画和效果，让它在用户看来具有交互性。首先重置浏览器效果，然后设置图像和鼠标指针的位置和大小。[`filter`](https://www.geeksforgeeks.org/css-filter-property/) 属性用于给元素赋予视觉效果。[`clip-path`](https://www.geeksforgeeks.org/css-clip-path-property/) 属性用于将元素转换为不同类型的形状。
...
**JavaScript 部分：** 这个部分处理网页的交互部分。它使用 [`offsetX`](https://www.geeksforgeeks.org/html-dom-mouseevent-offsetx-property/) 和 [`offsetY`](https://www.geeksforgeeks.org/html-dom-mouseevent-offsetx-property/) 属性来获取 `X` 和 `Y` 坐标，从而检测图像上的鼠标移动。然后使用 `clip-path` 属性为聚光灯效果创建一个圆。
