...
> 原文：[https://www.geeksforgeeks.org/apply-glowing-effect-to-the-image-using-html-and-css/](https://www.geeksforgeeks.org/apply-glowing-effect-to-the-image-using-html-and-css/)

在浏览大多数网站时，你会看到一些特殊的效果，当你将光标放在不同的图片上时，可以看到这些效果。因此，在本文中，我们将实现相同的。我们可以用这样的图片作为我们网站的卡片。

在本文中，您将学习如何将发光效果应用于特定的图像，当您将光标放在图像上时，您将看到该效果。我们将用 **HTML** 和 **CSS** 来实现。提供了一个示例视频，以便更清楚地了解您将在本文中开发什么。
...
现在，我们将应用一些 CSS 属性来装饰图像，并修复它在网页上的位置。关键是当用户将光标放在图像上时，使用 CSS 的 `box-shadow` 属性使图像发光。下面是内部 CSS 代码给你的帮助。
...
本示例使用 `box-shadow` 属性对图像应用发光效果。你可以根据自己的需要改变背景的发光颜色。
