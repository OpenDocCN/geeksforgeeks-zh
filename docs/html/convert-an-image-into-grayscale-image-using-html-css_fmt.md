...
> 原文：`https://www.geeksforgeeks.org/convert-an-image-into-grayscale-image-using-html-css/`

给定一个彩色图像，任务是使用 CSS 属性将图像转换为灰度图像。在 CSS 中，`filter`属性用于将图像转换为灰度图像。`Filter`属性主要用于设置图像的视觉效果。
...
在本例中，使用`filter: grayscale(100%)`将图像转换为灰度。
