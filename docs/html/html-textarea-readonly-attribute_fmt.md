...
The `<textarea>` `readonly` attribute in HTML is used to specify that the textarea element is read-only. If the textarea is readonly, then its content cannot be changed but can be copied and highlighted. It is a boolean attribute.
**Syntax:**
...
**Supported Browsers:** The browsers supported by the `<textarea>` `readonly` attribute are listed below:
