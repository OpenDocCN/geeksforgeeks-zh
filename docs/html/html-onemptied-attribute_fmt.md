...
HTML `onemptied` 属性用于在媒体遇到致命错误或媒体文件不可用或媒体播放列表为空时触发事件。
...
**属性值:** 该属性包含单值脚本，当媒体为空时，使用 `onemptied` 属性。该视频或音频文件受 `<音频>` 和 `<视频>` 标签支持。
...
**支持的浏览器:** HTML `onemptied` 属性支持的浏览器如下:
