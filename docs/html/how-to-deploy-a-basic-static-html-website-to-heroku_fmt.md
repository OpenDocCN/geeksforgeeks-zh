...
Heroku 是一个简单的一站式解决方案，可以托管任何网站或服务器。本文围绕如何在 Heroku 上托管自己的静态 HTML 网页展开。为了演示这一点，我们将构建一个简单的网页并托管它。
...
使用 Heroku 仪表板右上角的**新**按钮创建您的 [Heroku](http://heroku.com) 应用程序。我们将把这个应用命名为`“your-app-name-123”`。
...
现在我们已经创建了我们的应用程序，我们只需要将本地 git 存储库连接到 Heroku 应用程序。为此，我们将 Heroku 应用程序的遥控器添加到 git 存储库中。(别忘了用自己的应用名称替换`“your-app-name-123”`)
...
您的应用程序现已在 Heroku 上线。要查看您的应用程序，请点击上一步结束时出现的链接，该链接看起来像`https://your-app-name-123.herokuapp.com/`。要提交并保存对您的`home.html`文件的所有更改，从您的`“folder”`目录执行以下命令。
