...
> 原文: [https://www.geeksforgeeks.org/html-ondragover-event-attribute/](https://www.geeksforgeeks.org/html-ondragover-event-attribute/)

当将可拖动的元素或文本拖动到有效的放置目标时，将触发 ondragover 事件属性。要拖动元素，请将该元素扭曲并拖动到拖动点。这里我们将使用全局 HTML 5 可拖动属性。不能删除数据和元素。若要允许删除，必须调用 `event.preventDefault()` 方法。它在 HTML 5 中是新的，不支持以下版本的 HTML。
...
*   **掉落目标上的事件触发器:**
    ...
    *   `ondragleave`: 当被拖动的元素离开放置目标时，这将触发。
...
*   **拖动元素前:**
    ![Drag element before](img/4e682e6f1d0dcabe05c5a209628711a0.png)
*   **掉落元素后:**
    ![Drop element after](img/5ca5126efcc878ca6eb4ffd96198c907.png)
