...
> 原文: [https://www.geeksforgeeks.org/什么是媒体元素标签-由-html5-引入/](https://www.geeksforgeeks.org/what-are-the-media-element-tags-introduced-by-html5/)

HTML5 引入了 5 个最受欢迎的媒体元素标签，即 `<音频>`, `<视频>`, `<来源>`, `<嵌入>`, `<曲目>`。这些媒体元素标签用 HTML 改变了整个开发。

在本文中，您将简要了解这五个媒体元素标签。

## 媒体标签:

*   `<音频>`: 它是一个内嵌元素，用于将声音文件嵌入到网页中。
*   `<视频>`: 用于将视频文件嵌入网页。
*   `<来源>`: 用于附加音频、视频、图片等多媒体文件。
*   `<嵌入>`: 用于将一般为音频或视频等多媒体内容的外部应用嵌入到 HTML 文档中。
*   `<轨道>`: 为媒体组件音视频指定文字轨道。
...
如果你想在网页中添加歌曲或任何声音文件等音频，这是一个很有用的标签。
