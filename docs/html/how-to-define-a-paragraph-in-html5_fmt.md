...
> 原文：`https://www.geeksforgeeks.org/how-to-define-a-paragraph-in-html5/`

在本文中，我们使用文档中的一个 [`<p>` 元素](https://www.geeksforgeeks.org/html-paragraph/) 来定义一个段落。HTML 中的 `<p>` 标签定义了段落。这些都有开始和结束标签。所以任何写在 `<p>` 和 `</p>` 内的内容都被视为段落内容。大多数浏览器将一行作为段落阅读，即使我们没有使用结束标记，即 `</p>`，但这可能会产生意想不到的结果。所以，这是一个很好的约定，我们必须使用结束标记。
