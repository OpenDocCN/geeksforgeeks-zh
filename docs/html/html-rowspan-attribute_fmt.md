...
**用法:** 可与 [HTML 表格](https://www.geeksforgeeks.org/html-tables/) 中的 `<td>` 和 `<th>` 元素配合使用。

**属性值:** 它包含一个值，即**数**，该值指定表格单元格应跨越的行数。

*   **<td>:** `rowspan` 属性与 `<td>` 标签一起使用时，决定了它应该跨越的标准单元格的数量。
    **语法:**
...
*   **<th>:** `rowspan` 属性与 `<th>` 标记一起使用时，决定了它应该跨越的标题单元格的数量。
    **语法:**
