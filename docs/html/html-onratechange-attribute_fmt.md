...
该属性包含单值脚本，该脚本在对已更改的事件属性调用时执行。该属性由 `<audio>` 和 `<video>` 标签支持。
...
<script> 
            // Get the video element with id="vidID" 
