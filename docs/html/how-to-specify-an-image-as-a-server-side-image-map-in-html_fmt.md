...
> 原文: [https://www.geeksforgeeks.org/how-to-specify-an-image-as-a-server-side-image-map-in-html/](https://www.geeksforgeeks.org/how-to-specify-an-image-as-a-server-side-image-map-in-html/)

在图像映射中，用图像内的某组坐标来指定图像，这些坐标充当到不同目的地的超链接区域。它不同于图像链接，因为在图像链接中，图像可以用于服务单个链接或目的地，而在映射图像中，图像的不同坐标可以服务不同的链接或目的地。

`<map>` 元素用于定义用户界面上可点击的地图图像。`<map>` 需要标签来帮助图像和地图之间的关系。`<map>` 元素包含多个元素，用于定义图像地图中的可点击区域。
