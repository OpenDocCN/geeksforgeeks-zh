...
> 原文：[https://www.geeksforgeeks.org/what-is-the-battery-status-api-in-html5/](https://www.geeksforgeeks.org/what-is-the-battery-status-api-in-html5/)

## 简介

电池状态 API 有时也称为电池 API，提供电脑电池的相关信息。它提供电池的剩余电量和充电状态。如果电池正在充电或放电，它还会给出对电池完全充电或放电的剩余时间的估计。

电池状态应用编程接口不是外部应用编程接口。它在内置的 `Navigator` 对象的 `navigator.getBattery()` 方法中可用。

## 获取电池对象

*   使用 `navigator.getBattery()` 方法获取系统的电池信息。
*   将一个 JavaScript `then()` 方法添加到 `navigator.getBattery()` 对象中。之所以需要 `then()` 方法，是因为用作参数的函数只有在我们成功从系统中获取电池信息时才会执行。
*   用作 `then()` 方法的参数的函数将电池对象作为参数。该电池对象携带系统电池的所有信息。
...
*   **返回值：** 如果电池正在充电，则返回布尔值 `true`，如果电池没有充电，则返回 `false`。
*   **`chargingchange` 事件：** 我们可以通过给电池对象添加事件监听器来检测电池充电状态的变化。如果发生 `chargingchange` 事件，将触发事件监听器功能。
...
该方法将电池电量作为一个数字返回，假设满电池电量为 1。我们必须将其乘以 100，才能得到电池电量的百分比。

*   **`levelchange` 事件：** 此事件检测电池电平的变化。只要电池电量发生变化，我们就可以通过在 `levelchange` 事件中添加事件监听器来触发条件。
...
该方法以秒为单位返回电池完全充电的预计剩余时间。如果电池没有连接充电器，它会返回 `Infinity`。

*   **`chargingtimechange` 事件：** 该事件检测该估计时间的变化。我们可以给这个事件添加一个监听器并更新它。
...
### 4. `dischargingTime` 方法

该方法以秒为单位返回电池完全放电的预计剩余时间。如果电池连接到充电器，它会返回 `Infinity`。

*   **`dischargingtimechange` 事件：** 该事件检测该估计时间的变化。我们可以给这个事件添加一个监听器并更新它。
...
在本例中，我们已经使用电池状态 API 获取了电池状态，并在我们的网页上显示了数据。
