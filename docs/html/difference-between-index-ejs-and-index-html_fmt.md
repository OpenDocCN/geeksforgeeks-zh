...
> 原文: [https://www.geeksforgeeks.org/difference-between-index-ejs-and-index-html/](https://www.geeksforgeeks.org/difference-between-index-ejs-and-index-html/)
...
假设你有一个显示员工工资的网页。如果你创建了一个静态的 HTML 文件，那么你需要在每次新员工加入或者工资变化的时候重写它。但是，在 EJS 中，向其中插入动态数据变得太容易了。
...
**第三步:** 创建一个 `index.ejs` 文件，并将其放入 `views` 文件夹中，代码如下。
...
**输出:** 现在打开浏览器，转到 `http://localhost:3000/employee/salary`，会看到如下输出:
