...
**咏叹调标签**有助于定义字符串，并为使用辅助技术的用户提供关于文档结构的附加信息。在大多数情况下，`aria-label` 用于用更精确的信息替换现有标签。然而，我们在使用 `aria-label` 时应该小心，因为它并不适用于所有的 HTML 元素。

`aria-label` 属性可以与以下 HTML 元素一起使用：
