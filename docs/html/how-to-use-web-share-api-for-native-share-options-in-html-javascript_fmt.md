...
> 原文：[https://www.geeksforgeeks.org/how-to-use-web-share-api-for-native-share-options-in-html-javascript/](https://www.geeksforgeeks.org/how-to-use-web-share-api-for-native-share-options-in-html-javascript/)

`Web Share API` 是一个 `JavaScript` API，允许网站通过与 `操作系统` 集成，通过设备的 `本地共享对话框` 共享 `文本/网址/文件`。它只适用于 `移动设备` 和有限的浏览器。它最初是在 `Chrome 61` 中为安卓推出的。
...
**注意：** 桌面和非 `HTTPS` 协议不支持 `Web Share API`。因此，为了使用它，有必要在 `HTTPS` 提供网页服务。
