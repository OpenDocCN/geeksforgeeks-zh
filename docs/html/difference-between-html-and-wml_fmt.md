...
> 原文: [https://www.geeksforgeeks.org/difference-between-html-and-wml/](https://www.geeksforgeeks.org/difference-between-html-and-wml/)

## 1. 超文本标记语言 (HTML)
超文本标记语言 (`HTML`) 是指创建网页的标准标记语言。它由一系列元素/标签组成，这些元素/标签用于定义内容的结构，即设计网页的结构。它是为向台式计算机提供内容而创建的。`HTML` 可以通过像级联样式表 (`CSS`) 这样的技术和像 `JavaScript` 这样的脚本语言来辅助。
...
## 2. 无线标记语言 (WML)
基于 `XML` 的无线标记语言 (`WML`) 是指第一种用于实现无线应用协议 (`WAP`) 规范手段的设备的标记语言，应用于无线设备，即蜂窝电话、掌上电脑。它以前被称为手持设备标记语言 (`HDML`)。它能做的事情非常有限，甚至与 `HTML` 相比，它的数字标签更少。
