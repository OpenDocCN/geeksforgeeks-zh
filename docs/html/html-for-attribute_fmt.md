...
属性的 **HTML |在 **<label>** 和 **<output>** 元素中使用。
**对于<label>标签元素中的属性:**当出现在<label>标签元素中时，用于指定标签绑定到的表单元素的类型。
...
**示例-1:** 本示例说明了<label>标签元素中 for 属性的使用。
...
**对于<output>元素中的属性:**当出现在<output>元素中时，它指定结果和计算之间的关系。
...
**示例-2:** 本示例说明了<output>元素中的 for 属性的使用。
