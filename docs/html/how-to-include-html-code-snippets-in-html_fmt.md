...
> 原文：[https://www.geeksforgeeks.org/how-to-include-html-code-snippets-in-html/](https://www.geeksforgeeks.org/how-to-include-html-code-snippets-in-html/)
...
*   调用主 `index.html` 文件中的函数，包含 `gfg.html` 的片段。

## 第一步

创建名为 `index.html` 的主 HTML 文件。该文件将显示标题文本“这是 index.html”。
...
创建另一个你选择的 HTML 文件，包含在 `index.html` 文件中，并将该文件保存在存在 `index.html` 的同一目录中。在本文中，创建一个名为 `gfg.html` 的 HTML 文件，其中显示了“极客的计算机科学门户”。
...
我们将把 `gfg.html` 片段包含在 `index.html` 文件中。包含 HTML 是通过使用 `GFG-include-html-snippet` 属性来完成的。
