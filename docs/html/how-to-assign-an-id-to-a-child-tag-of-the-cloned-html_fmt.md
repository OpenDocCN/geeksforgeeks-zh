...
> 原文：[https://www.geeksforgeeks.org/how-to-assign-an-id-to-a-child-tag-of-the-cloned-html/](https://www.geeksforgeeks.org/how-to-assign-an-id-to-a-child-tag-of-the-cloned-html/)
...
在这里，我们将克隆引导卡，该卡具有名为 `cloneme` 的 id，并具有子标签，如 `<b>`、`<button>`、`<p>` 标签。

**步骤：**

1.  当用户按下网页用户界面部分的“点击我”按钮时，它将在点击监听器回调时运行。
2.  在这个回调函数中，我们简单地复制了一个 id 为 `cloneme` 的 HTML 标记。也可以带属性类的 HTML 标签。
3.  在克隆了 Html 代码之后，我们将通过使用标签的属性名称（如 id、class）查找标签来更改 `<b>` 和 `<button>` 标签的子标签的 ID。
