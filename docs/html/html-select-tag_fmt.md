...
*   [**自动对焦:**](https://www.geeksforgeeks.org/html-select-autofocus-attribute/)HTML `<select>` 自动对焦属性用于指定页面加载时下拉菜单应自动对焦。这是一种布尔属性。
*   [**禁用:**](https://www.geeksforgeeks.org/html-select-disabled-attribute/)<select>禁用属性用于指定选择元素被禁用。禁用的下拉列表不可点击且不可用。它是一个布尔属性。
*   [**表单:**](https://www.geeksforgeeks.org/html-select-form-attribute/)<select>表单属性用于指定< select >元素所属的一个或多个表单。
*   [**多重:**](https://www.geeksforgeeks.org/html-select-multiple-attribute/) 选择>`多重属性的 HTML 是布尔属性。它规定允许用户选择在<选择>元素中出现的多个值。
*   [**名称:**](https://www.geeksforgeeks.org/html-select-name-attribute/)<select>名称属性用于为下拉列表指定名称。它用于在提交表单后引用表单数据，或者在 JavaScript 中引用元素。
*   [**必选:**](https://www.geeksforgeeks.org/html-select-required-attribute/)<select>必选属性是一个布尔属性，用于指定用户在提交表单前应选择的值。
*   [**大小:**](https://www.geeksforgeeks.org/html-select-size-attribute/)<select> size 属性用于指定下拉列表中可见选项的数量。
...
在本例中，我们使用的是带有 select 标记的 optgroup 标记。`optgroup` 标记用于在下拉列表中显示相关选项。
