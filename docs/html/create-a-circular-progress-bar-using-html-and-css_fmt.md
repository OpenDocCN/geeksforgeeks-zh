...
> 原文: [https://www.geeksforgeeks.org/create-a-circular-progress-bar-using-html-and-css/](https://www.geeksforgeeks.org/create-a-circular-progress-bar-using-html-and-css/)
...
首先，我们将使用 HTML 设计一个基本的进度条结构。首先，我们将创建一个包含两个进度条的容器 `div`。之后，我们将在容器 `div` 中创建另一个 `div`，创建循环 `div` 元素。这里我们增加了一些评论，有助于读者轻松理解。
...
接下来，我们将使用一些 CSS 属性来设计进度条。`container` 类设置 `div` 元素的位置。其他 CSS 类用于创建圆形进度条和添加 CSS 样式。
...
在本节中，我们将结合以上两个部分，使用 HTML 和 CSS 创建一个圆形进度条。
