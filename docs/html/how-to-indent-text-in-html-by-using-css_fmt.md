...
> 原文：[https://www.geeksforgeeks.org/how-to-indent-text-in-html-by-using-css/](https://www.geeksforgeeks.org/how-to-indent-text-in-html-by-using-css/)

在本文中，我们将讨论在 HTML 文档中缩进文本的各种方法。文本缩进用于设置块中文本行前的空白长度。它通常标志着一个段落的开始。

有两种常见的文本缩进方法：

## 方法 1：使用 `margin-left` 属性

该属性用于在元素左侧添加边距。它可以通过以适当的长度单位或百分比指定所需的空间来添加所需的缩进。

**示例：** 本示例使用 `margin-left` 属性缩进整个文本块。
...
该属性用于设置段落中第一行文本前的空间量。可以根据需要以长度单位或百分比指定。

**示例：** 在本例中，使用 `text-indent` 属性缩进段落的第一行。
