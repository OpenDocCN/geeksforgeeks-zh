...
在 HTML 中，表格由 `<table>` 标签定义，表格的表头由 `<thead>` 标签定义，默认情况下表格的表头居中加粗，表格的行由 `<tr>` 定义，行的数据或信息由 `<td>` 标签定义。
...
**insertRow()** 方法将一个空的 `<tr>` 放入表中，在该定义的表中创建新行。**deleteRow()** 方法用于从定义的表中删除行。
...
下拉菜单允许用户选择预定义的选项。它基本上是一个可折叠的菜单。为了创建一个下拉菜单，我们需要基本的 HTML、CSS 和 JavaScript。

**注意:** 为了获得这个效果的完整结果，下面三个部分必须结合起来。

### HTML 部分

按钮 `<button>` 和 `<a>` 标签用于创建下拉菜单。
**例:**
...
`.dropbtn` 是可切换菜单的按钮，`background-color` 设置按钮颜色，`.dropbtn:hover, .dropbtn:focus` 在按钮上设置悬停效果，`position: relative;` 将在下拉按钮下出现下拉菜单，使用 `.dropdown-content` 维护内容。
**例:**
...
点击下拉按钮后-
![](img/67386e17d71f519c4f2edf4a5c722e24.png)
