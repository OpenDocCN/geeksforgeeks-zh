...
# HTML5 | MathML `<mtext>` 标签

> 原文: [https://www.geeksforgeeks.org/html5-mathml-mtext-tag/](https://www.geeksforgeeks.org/html5-mathml-mtext-tag/)

HTML5 中的 **MathML `<mtext>` 标签**用于打印任意表达式前后的任意文本。您可以使用此标签来表示表达式，也可以为该表达式命名。用于打印此标签所使用的任何表达式的注释。
**语法:**
...
*   **direction:** 该属性保存方向值。两种类型的方向值是可接受的：从左到右的 `ltr` 和从右到左的 `rtl`。
...
**支持的浏览器:** HTML5 MathML `<mtext>` 标签支持的浏览器如下:
