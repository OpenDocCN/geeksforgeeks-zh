...
CSS `position` 属性用于设置文本在图像上的位置。这可以通过将图像和文本包含在一个 HTML `div` 中来实现。然后将 `div` 的位置设为 `relative`，将文本的位置设为 `absolute`。绝对元素相对于其父元素(`div`)定位。这些元素的 `top`、`right`、`bottom` 和 `left` 属性指定它们相对于父元素的位置。
