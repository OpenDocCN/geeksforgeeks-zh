...
canvas 的 `beginPath()` 方法用于创建从当前点到起点的路径。调用 `closePath()` 方法后，我们可以使用 `stroke()` 方法绘制路径，`fill()` 方法以黑色作为默认填充路径，`fillStyle` 属性以另一种颜色或渐变或图案进行填充。
...
![示例 1 输出](img/4c03c709be09e8a656d52f6288fe6803.png)
...
![示例 2 输出](img/5b1e96bea0fc9907cf47db3c0ba98dec.png)

## 支持的浏览器

`canvas beginPath()` 方法支持的浏览器如下:
