...
HTML5 中的 **MathML `<ms>`** 标记用于表示字符串，该字符串将通过编程语言和计算机代数系统来表示数学表达式。默认情况下，字符串表达式用引号括起来。表达式的左侧用 `l` 引号括起来，表达式的右侧用 `rquotes` 括起来。`<ms>` 标记的元素不是 ASCII 字符，而是 `<mglyph>` 和 `<mtext>` 标记的序列字符。
...
*   **dir:** 此属性保存方向两种类型的方向值：从左到右（`ltr`）和从右到左（`rtl`）。
...
![Output](img/31a9f270e4f060a5af596d566f55cd36.png)
