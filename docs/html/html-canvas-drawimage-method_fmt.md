...
在 [HTML5](https://www.geeksforgeeks.org/html5-introduction/) 中，画布 `drawImage()` 功能用于在画布上显示图像或视频。该功能可用于显示整个图像或图像的一小部分。但是，必须先加载图像，才能将其进一步加载到画布上。
...
*   **x:** 表示图像必须放置的 x 坐标。
*   **y:** 表示需要放置图像的 y 坐标。
*   **swidth:** 为可选参数，表示裁剪图像的宽度。
*   **sheight:** 为可选参数，表示裁剪图像的高度。
*   **sx:** 为可选参数，表示开始剪裁的 x 坐标。
*   **sy:** 可选参数，表示 y 坐标从哪里开始裁剪。
*   **width:** 为可选参数，表示要使用的图像宽度。
*   **height:** 为可选参数，表示要使用的图像高度。
...
![输出示例](img/0de05b311406909c420cb7bb9b1d4c5e.png)
