...
> 原文：[https://www.geeksforgeeks.org/how-to-draw-with-mouse-in-html-5-canvas/](https://www.geeksforgeeks.org/how-to-draw-with-mouse-in-html-5-canvas/)
...
默认情况下，画布元素有一些属性，如填充等（可以更改样式）。因此，属性`offsetTop`和`offsetLeft`用于检索画布相对于其`偏移父元素`（DOM 中画布的最近祖先元素）的位置。通过从`event.clientX`和`event.clientY`中减去这些值，我们可以将`绘图的起点重新定位到光标`的顶端。

在函数`sketch()`中，我们使用以下内置方法来添加功能：

*   `beginPath()`：开始新路径，每次点击鼠标左键。
*   `lineWidth`：设置将要绘制的线的宽度。
*   `strokeStyle`：在这方面，我们用它来设置线条的颜色为黑色。可以更改该属性以产生不同颜色的线条。
*   `moveTo()`：路径的起始位置移动到画布上的指定坐标。
*   `lineTo()`：创建从所述位置到指定坐标的直线。
*   `stroke()`：为创建的线条添加描边。没有这个，线就看不见了。
...
功能`sketch()`只有在`标志的值为真`时才会执行。在`beginPath()`之后更新存储在对象`coord`中的坐标很重要，因此调用`getPosition(event)`。将 JavaScript 文件链接到 HTML 文件后，将获得以下代码。
