...
*   `charoff`: It is used to set the number of characters that will be aligned from the character specified by the `char` attribute. The value of these attributes are in numeric form.
...
*   `scope`: It is used to specify the scope of header content.
...
**Supported Browsers:** The browsers supported by the `<td>` tag are listed below
