...
> 原文：[https://www.geeksforgeeks.org/design-dragons-world-game-using-html-css-and-javascript/](https://www.geeksforgeeks.org/design-dragons-world-game-using-html-css-and-javascript/)
...
*   **定位游戏名称：**游戏名称由 `CSS` 的 `absolute` 属性定位。
*   **背景图样式：**在 `container` 类中，我们已经将游戏的背景图与 `background-size` 设置为 `cover`。
*   **记分卡样式：**我们已经将记分卡放置在页面的右上角，并为其提供了合适的背景颜色，以使其具有吸引力。其中的文本将显示在 `white` 中。
...
**2. 更新分数：**只有满足给定条件时，分数才被满足。我们将计算障碍和龙的左值和底值，然后根据适当的值增加分数，该值表明龙已经从障碍中拯救了自己。为此，我们采用了 `cross` 变量，并为其指定了 `true`。当龙安全通过障碍物时，我们将该值设置为 `false`。大约 1 秒后，我们将 `cross` 的值更改为 `true`。我们还使障碍在每次穿越后跑得更快，从而增加了难度。
...
setInterval(() => {
    ...
    if (offsetx < 120 && offsety <= 125 144) {
...
[龙游](https://github.com/SAEb-ai/Dragon-s-World-Game)
