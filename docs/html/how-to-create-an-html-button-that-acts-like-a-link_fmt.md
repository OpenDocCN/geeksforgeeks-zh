...
> 原文: [https://www.geeksforgeeks.org/how-to-create-an-html-button-that-acts-like-a-link/](https://www.geeksforgeeks.org/how-to-create-an-html-button-that-acts-like-a-link/)
...
当用户点击按钮时，`onclick` 事件属性起作用。当鼠标点击按钮时，按钮就像一个链接，将页面重定向到给定的位置。
...
这个方法在锚点标签内创建一个按钮。锚标签将网页重定向到给定的位置。
...
这个方法创建一个简单的锚点标签链接，然后应用一些 CSS 属性使它像一个按钮。
...
该方法使用表单标签和按钮标签。单击按钮后，将调用表单 `action` 属性，并将网页重定向到给定位置。
