...
**HTML `<input>` 表单更新属性**用于指定提交表单时不验证输入元素。它是一个布尔属性，该属性可用于覆盖 `<表单>` 元素的 **`newdata`** 属性，并且只能与 `<input type="submit">` 一起使用。
...
**示例:** 本示例说明使用 `<input>` `formnovalidate` 属性。
...
**支持的浏览器:** 支持的浏览器 *HTML `<input>` `formnovalidate` 属性*如下:
