...
> 原文：[https://www.geeksforgeeks.org/how-to-include-social-media-icons-in-html/](https://www.geeksforgeeks.org/how-to-include-social-media-icons-in-html/)
...
**注意：** 可以添加社交媒体图标为图片（SVG、PNG）或[字体图标](https://www.geeksforgeeks.org/css-icons/)（`字体牛逼`）。在本文中，探索这两种方式。

## 方法一：使用 Font Awesome

要使用 `字体牛逼` 图标，在 `<head>` 部分添加以下 CDN 链接。
...
## 方法二：使用 Google Material Icons

使用 `谷歌图标`，在 `<head>` 部分添加以下链接。
