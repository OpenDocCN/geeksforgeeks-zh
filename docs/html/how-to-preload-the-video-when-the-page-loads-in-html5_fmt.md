...
> 原文：[https://www.geeksforgeeks.org/how-to-preload-the-video-when-the-page-loads-in-html5/](https://www.geeksforgeeks.org/how-to-preload-the-video-when-the-page-loads-in-html5/)

HTML [`preload`](https://www.geeksforgeeks.org/html-video-preload-attribute/) 属性用于指定页面加载时开发者认为应该加载视频的方式。
...
`preload` 属性有 `auto`、`metadata` 和 `none` 3 个值。

**注意：** 空字符串导致默认 `auto` 属性值。

**示例 1：** 以下示例使用 `preload` 属性的 `none` 属性值来演示 `video` 元素。
...
**示例 2：** 以下示例使用 `auto` 属性值演示了 `video` 元素。
