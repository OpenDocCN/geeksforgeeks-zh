...
您可以使用 [CSS `transform`](https://www.geeksforgeeks.org/css-transform-property/) 属性轻松旋转 HTML 中的图像。该属性用于移动、旋转、缩放和执行元素的各种变换。

**方法：** 可以使用 [`rotate()`](https://www.geeksforgeeks.org/css-rotate-function/) 函数，该函数被定义为一种变换，在不使元素变形的情况下，围绕 2D 平面上的固定点旋转元素。它采用一个定义旋转角度的参数。旋转角度由两部分组成，旋转值后跟旋转单位。单位可以用度数(`deg`)、坡度(`grad`)、弧度(`rad`)和转角(`turn`)来定义。方法 `rotate()` 的值将顺时针旋转元素。
