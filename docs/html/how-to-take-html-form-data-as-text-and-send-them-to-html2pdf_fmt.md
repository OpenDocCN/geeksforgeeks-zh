...
> 原文：[https://www.geeksforgeeks.org/how-to-take-html-form-data-as-text-and-send-them-to-html2pdf/](https://www.geeksforgeeks.org/how-to-take-html-form-data-as-text-and-send-them-to-html2pdf/)

为了将 HTML 表单数据转换为 pdf，主要的方法是使用 html2pdf 库中的 `html2pdf()` 函数。

## 方法
首先，创建一个以加密类型为文本的表单，并添加一些输入字段。之后我们需要使用 html2pdf 库中的 `html2pdf(element)` 函数。在表单的提交按钮上提供 `onclick()` 功能。`html2pdf(element)` 函数接受一个输入，该输入是需要转换为 pdf 格式的标签或表单的 id。
