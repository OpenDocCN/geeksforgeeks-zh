...
# HTML5 | MathML `<mtable>` 标签

> 原文: [https://www.geeksforgeeks.org/html5-mathml-mtable-tag/](https://www.geeksforgeeks.org/html5-mathml-mtable-tag/)

**MathML `<mtable>` 标签**用于在 HTML5 中创建表格或矩阵。这个标签类似于普通的 HTML `<table>` 标签。`<table>` 有 `<tr>`、`<td>` 和 `<td>` 标签，同样它也有 `<mtr>`、`<mtd>` 和 `<mtd>` 标签，它们的作用相同。但是我们不使用 `<table>` 和其他子标签来创建矩阵。这里 `<mtable>` 对创建矩阵很有用。因为 MathML 是关于 HTML 中的数学表达式表示的。
...
![输出示例](img/6877166facff8b3d0ff83cc1d8edfe6f.png)
