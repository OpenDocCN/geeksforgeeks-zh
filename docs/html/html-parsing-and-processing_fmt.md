...
**解析模型概述**
![解析模型概述](img/58d4ac628c69aa8928756279a055b69b.png)
...
*   **输入字节流:**
    输入字节流中的字节必须转换为字符，以便在标记器作为其输入流时使用，绕过输入字节流和字符编码进行解码。
...
*   **Tokenization:** 实现预期将使用以下状态机来标记 HTML。状态机预期从数据状态开始。大多数状态接受单个字符，该字符要么将状态机切换到新状态以重新消费当前输入字符，要么切换到新状态以消费下一个字符。一些状态具有更复杂的行为，可以在切换到另一个状态之前接受多个字符。在某些情况下，标记化器状态还受树构建阶段的影响，甚至允许向流中插入额外的字符。
