...
> 原文：[https://www.geeksforgeeks.org/how-to-disable-autocomplete-of-an-html-input-field/](https://www.geeksforgeeks.org/how-to-disable-autocomplete-of-an-html-input-field/)
...
*   使用带有 `autocomplete` 属性的 `<input>` 标签。
*   将 `autocomplete` 属性设置为值 `"off"`。

**示例：** 下面的代码演示了一个表单有两个输入字段，一个字段的自动完成状态为 `on`，另一个字段的自动完成功能为 `off`。
