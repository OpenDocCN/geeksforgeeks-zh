...
> 原文：[https://www.geeksforgeeks.org/how-to-define-the-url-of-the-video-file-in-html5/](https://www.geeksforgeeks.org/how-to-define-the-url-of-the-video-file-in-html5/)

在本文中，我们将学习如何在 HTML 中定义视频的 URL。`<video>` 标签用于在网页上嵌入视频文件。这将根据浏览器显示视频播放器和播放器控件。视频源可以是单一的，也可以支持多种格式的视频，以便多个浏览器支持视频播放。

**方法：** `<video>` 标签有一个属性 `src`，用来指定视频文件的加载路径。当要指定多个源时，`src` 属性也可以与每个 `<source>` 标签一起使用，以定义该源视频文件的路径。我们将使用这个属性来定义视频文件的 URL。
...
**示例 1：** 在本例中，属性与 `<video>` 标签一起使用。它说明了 `src` 属性来定义视频文件的 URL。
...
**示例 2：** 在本例中，该属性与 `<source>` 标签一起使用来定义多个视频源。
