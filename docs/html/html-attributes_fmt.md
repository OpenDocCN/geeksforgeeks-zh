...
2.  **alt 属性:**顾名思义，这是一个备用标签，用于在主要属性(即 `<img>` 标签)无法显示分配给它的值时显示或显示某些内容。这也可以用来向实际上坐在编码端的开发人员描述图像。
...
5.  **title 属性:**标题属性用于解释鼠标悬停在其上的元素。不同元素的行为不同，但通常在加载或鼠标指针悬停在值上时显示该值。
...
6.  **href 属性:**该属性用于指定任何地址的链接。该属性与 `<a>` 和 `<>` 标签一起使用。放在 `href` 属性中的链接链接到 `<a>` 标签中显示的文本。点击文本后，我们将被重定向到该链接。默认情况下，链接在同一个标签中打开，但是通过使用**目标**属性并将其值设置为**“_ blank”**，我们将根据浏览器配置被重定向到另一个选项卡或另一个窗口。
...
7.  **style 属性:**该属性用于为 HTML 元素提供各种 CSS(层叠样式表)效果，如增加字体大小、改变字体系列、着色等。有关详细解释，请参考 HTML |样式标签。下面的程序显示了几个符合样式属性的
...
8.  **lang 属性:**语言是用 `lang` 属性声明的。对于辅助功能应用程序和搜索引擎来说，声明语言是很重要的。
