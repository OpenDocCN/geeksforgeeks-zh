...
> 原文：[https://www.geeksforgeeks.org/what-is-formmethod-attribute-in-html-form/](https://www.geeksforgeeks.org/what-is-formmethod-attribute-in-html-form/)

## 什么是 formmethod 属性？

HTML 中的 `formmethod` 属性用于定义一种 HTTP 技术，该技术指定如何向后端服务器发送表单数据。该属性适用于`<button>`、`<input type="submit">`和`<input type="image">`。它覆盖了`<form>`元素的`method`属性的特征。
