...
> 原文: [https://www.geeksforgeeks.org/how-to-create-breadcrumbs-using-html-and-css/](https://www.geeksforgeeks.org/how-to-create-breadcrumbs-using-html-and-css/)
...
这里我们将使用 `<li>` 标记来创建一个普通结构。这将创建一个简单的界面，您可以通过运行以下代码来检查它：
...
这里最难的任务是在面包屑的右侧创建箭头形状。为了创建箭头形状，我们将在选择器之后使用 `:after`。使用 `z-index` 属性将一个列表放在另一个列表上。对于一个 CSS 开发人员来说，其他事情都很容易。
...
在本节中，我们将结合 HTML 和 CSS 代码来创建面包屑。
