...
HTML 中的 `<i>` 标签用于以斜体显示内容。该标签通常用于显示技术术语、短语、不同语言中的重要单词。`<i>` 标签是包含开始标签、内容和结束标签的容器标签。
...
*   出于可读性的目的，使用 `<i>` 标记表示您希望以不同于普通短语的方式显示的单词
*   像 `<i>` 和 `<b>` 这样的标签现在定义了语义，而不是印刷外观。因此，要以斜体显示文本，用户可以使用 CSS 字体样式属性。
*   仅当未标记以下元素时，才使用 `<i>` 标签:
    *   `<em>` 中
    *   `<strong>` 中
    *   `<mark>` 中
