...
HTML 中的 `<address>` 标签表示一个人或者一个组织的联系方式。如果在 `<body>` 标签内使用 `<address>` 标签，则表示文档的联系信息；如果在 `<article>` 标签内使用 `<address>` 标签，则表示文章的联系信息。`<address>` 标签内的文本将以斜体格式显示。一些浏览器在 `address` 元素前后添加了换行符。
