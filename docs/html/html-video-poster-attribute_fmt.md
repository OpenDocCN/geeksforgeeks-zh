...
The **HTML `<video>` poster Attribute** is used to display the image while video downloading or when the user clicks the play button. If this image is not set, then it will take the first frame of the video as a poster image.
...
**Attribute Values:** It contains a single value URL which specifies the link of the source image. There are two types of URL links which are listed below:
...
**Supported Browsers:** The browsers supported by the HTML `<video>` poster Attribute are listed below:
