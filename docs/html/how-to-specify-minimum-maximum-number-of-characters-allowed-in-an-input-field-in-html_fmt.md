...
> 原文：[https://www.geeksforgeeks.org/how-to-specify-minimum-maximum-number-of-characters-allowed-in-an-input-field-in-html/](https://www.geeksforgeeks.org/how-to-specify-minimum-maximum-number-of-characters-allowed-in-an-input-field-in-html/)

给定一个输入字段，任务是设置 HTML 中输入字段允许的最小/最大字符数。

*   要设置输入字段中的最大字符限制，我们使用`<input>` `maxlength`属性。该属性用于指定进入`<input>`元素的最大字符数。
*   要设置输入字段中的最小字符限制，我们使用`<input>` `minlength`属性。该属性用于指定进入`<input>`元素的最小字符数。
...
*   **number1:** 包含允许`<input>`元素中最大字符数的单值数字。它的默认值是 524288。
