...
HTML 标题标签用于定义页面的标题。HTML 定义了六个级别的标题。这六个标题元素是 H1、H2、H3、H4、H5 和 H6；H1 是最高级别，H6 是最低级别。
...
代表水平规则的 `<hr>` 标记用于定义 HTML 页面中的主题分隔符。`<hr>` 标签为空标签，不需要结束标签。基本用于内容分离。详情请参考 [HTML `<hr>` 标签](https://www.geeksforgeeks.org/html-hr-tag/)文章。
...
HTML 中的 `<head>` 标签用于定义包含文档相关信息的文档头部。HTML 头部元素充当元数据的容器。元数据是关于未显示的 HTML 文档的数据。`<head>` 元素与标题无关。`<head>` 元素包含其他头部元素，如 `<title>`、`<meta>`、`<link>`、`<style>`、`<script>` 等。头部元素放置在 “`<html>`” 标签和 `<body>` 标签之间。详见 [HTML `<title>` 标签](https://www.geeksforgeeks.org/html-head-tag/)文章。
