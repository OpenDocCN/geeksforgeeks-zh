...
让任何 HTML5 元素包括图像都是可拖动的非常容易。我们借助[`draggable`](https://www.geeksforgeeks.org/html-draggable-attribute/)属性。它以`true`、`false`或`auto`为参数。默认值为`auto`。`draggable`属性取决于浏览器。如果我们设置值为`true`，那么图像是可拖动的。如果我们设置值为`false`，图像不可拖动。
