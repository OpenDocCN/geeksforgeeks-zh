...
> 原文: [https://www.geeksforgeeks.org/什么是 html 中的物理标签/](https://www.geeksforgeeks.org/what-are-physical-tags-in-html/)

物理标签用于指示如何使用 HTML 标签格式化或指示特定字符。任何物理样式标签都可以包含文本中允许的任何项目，包括常规文本、图像、换行符等。虽然每个物理标签都有定义的样式，但是您可以通过为每个标签定义自己的外观来覆盖该样式。所有物理标签都需要结束标签。
...
| 标签 | 表示 | 目的 |
| --- | --- | --- |
| `<b>` | 大胆的 | 粗体增加了文本的重要性，因为粗体标签将文本转换为粗体。 |
| `<i>` | 意大利语族的 | 斜体标签用于定义具有特殊含义的文本。 |
| `<u>` | 强调 | 它用于给文本加下划线。 |
| `<big>` | 大的 | 大标签将字体大小增加 1(注意:在 HTML 5 中不能使用大标签) |
| `<small>` | 小的 | 一个小标签定义了小文本，在写版权的时候使用。 |
| `<sub>` | 下标 | 下标用于替代基线。 |
| `<sup>` | 上标 | 上标通常用于显示基线以上的元素 |
| `<strike>` | 穿透 | 这是一个编辑标记，告诉读者忽略文本段落。 |
| `<tt>` | 电传文本 | 电传文本给出了默认的等间距字体系列。 |
...
**4. `<big>` :** `<big>`标签用于使文本变大一号，即小到中、中到大、大到 x-大，但是`<big>`标签不会使字体大小大于浏览器的最大字体大小。
...
**5. `<small>` :** `<small>`标签用于使文本小一号，即 x 大到大。小但是`<small>`标签不会使字体大小小于浏览器的最小字体大小。
...
**6. `<sub>` :** 子标签定义下标。下标出现在正常线的一半以下，字体较小。这些标签大多在写化学式时使用。
