...
HTML 中的**字符集属性**用于定义字符编码。字符集属性与 `<meta>` 和 `<script>` 元素一起使用。
**<meta> 标签中的字符集属性:** 字符集属性存在于 `<meta>` 元素中。它指定了 HTML 文档的字符编码。
...
**示例:** 这个示例说明了在 `<meta>` 元素中使用 charset 属性。
...
<meta charset="UTF-8" name="keywords" content="Meta Tags, Metadata" />
    </head>
...
当字符集属性出现在脚本元素中时，它指定外部脚本中使用的字符编码。
