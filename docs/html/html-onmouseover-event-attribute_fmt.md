...
> 原文: [https://www.geeksforgeeks.org/html-onmouseover-event-attribute/](https://www.geeksforgeeks.org/html-onmouseover-event-attribute/)
...
该属性包含单值 `*脚本*`，当鼠标在元素上移动时生效。

## 支持的标签

除了 `<基础>`, `< bdo>`, `< br>`, `< head>`, `< html>`, `< iframe>`, `< meta>`, `< param>`, `<脚本>`, `<样式>` 和 `<标题>` 外，几乎所有 HTML 元素都支持该属性。
