...
> 原文: [https://www.geeksforgeeks.org/html-course-building-main-content-section-2/](https://www.geeksforgeeks.org/html-course-building-main-content-section-2/)
...
1.  用名为 `row` 的类声明父 `div`。
2.  在父 `row` `div` 中声明三个 `div` 以包含三列，并分别将它们的 `id` 指定为 `column1`、`column2` 和 `column3`。
3.  **对于每一列**:
    *   用 `class="column-title"` 声明一个 `div`。列的标题。
    *   声明一个段落 `<p>` 元素来描述内容。
    *   声明一个锚点标签 `<a>` 来添加一个外部链接，该链接稍后将被设计为一个按钮。

以下是主要内容第 2 节的完整 HTML 代码
...
<p>
                A practice portal to test your knowledge
