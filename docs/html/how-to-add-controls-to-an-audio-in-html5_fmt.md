...
> 原文：[https://www.geeksforgeeks.org/how-to-add-controls-to-an-audio-in-html5/](https://www.geeksforgeeks.org/how-to-add-controls-to-an-audio-in-html5/)

`HTML <audio> controls` 属性用于指定播放音频的控件，这意味着它允许将带有音频控件的 HTML5 音乐播放器直接嵌入网页。
...
从上面的语法，`controls` 属性添加音频控件，如播放、暂停和音量，`<source>` 元素允许您指定替代音频文件。

音频标签主要支持 5 个属性，如下所示：

1.  `autoplay`：使音频自动开始播放，无需等待整个音频文件下载完成。
2.  `loop`：通过循环可以反复播放音频。
3.  `muted`：使播放器默认静音。
4.  `preload`：这可以设置为以下值。
    *   `auto`：这意味着文件是否应该在页面加载后立即加载。
    *   `metadata`：这意味着是否只有元数据、曲目标题等。应该是加载的。
    *   `none`：这意味着当页面加载时，浏览器不应该加载文件。
