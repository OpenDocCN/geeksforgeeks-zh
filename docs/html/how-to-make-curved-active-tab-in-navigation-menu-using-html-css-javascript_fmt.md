...
> 原文: [https://www.geeksforgeeks.org/how-to-make-curved-active-tab-in-navigation-menu-using-html-css-javascript/](https://www.geeksforgeeks.org/how-to-make-curved-active-tab-in-navigation-menu-using-html-css-javascript/)
...
*   为我们的默认活动项目选择一个列表项目。向此项添加 `active` 类。
*   添加自定义 `CSS`，背景和字体，使我们的导航好看。

## 添加 JavaScript–点击使活动标签可移动
...
*   在列表元素内的 `<b>` 标签上添加一类“左曲线”，在第二个 `<b>` 标签上添加“底曲线”。
*   使用类别为“左曲线”的 `<b>` 标记在左侧创建一个矩形。
*   使用选择器 `::before` 的 `css` 向该类的元素添加边框半径。
*   将左曲线显示为 `display: “无”`。
*   当链接处于活动状态时，显示为 `display: “阻止”`。
*   对“右曲线”(第二个 `<b>` 标记)重复相同的过程。
...
<head>
    ...
    <!-- The Pseudo elements : Style for the outside curve-->
    <style>
