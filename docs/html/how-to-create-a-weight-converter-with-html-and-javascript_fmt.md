...
> 原文: [https://www.geeksforgeeks.org/how-to-create-a-weight-converter-with-html-and-javascript/](https://www.geeksforgeeks.org/how-to-create-a-weight-converter-with-html-and-javascript/)
...
`<input>` 标记用作输入字段，用户可以在其中输入他们的数据，`placeholder` 是指定简短提示的属性，该提示描述了用户要输入的 `<input>` 元素的期望值。

`oninput` 是当元素获得用户输入时发生的事件，即当 `<input>` 或 `<textarea>` 元素的值改变时。在上面的代码中，该属性包含值 `kiloweightConvert(this.value)`，当 `oninput` 事件被触发时，该属性工作。

当元素的值被改变时，发生 `onchange` 事件。

这两个事件是相似的，但是不同的是 `oninput` 事件在元素的值改变后被激活，而 `onchange` 事件发生在元素失去焦点时，在内容改变后。`onchange` 事件也适用于 `<select>` 元素。
...
| :--- | :--- |
| lb to kg | ounces to pounds |
| Pounds to ounces | G = ounce /0.035274 |
| **pounds to stones** | Stone = lb *0.071429 |
| **ounces to stones** | Stone = ounce * 0.0044643 |
| Kg = g /1000 | **Stone to kilogram** |
| Kg = ST/0.15747 | **grams to ounces** |
| Oz = g *0.035274 | **Stones to ounces** |

## 示例

下面给出了如何将一个值从千克转换为其他测量值：
