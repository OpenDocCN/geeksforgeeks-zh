...
> 原文：`https://www.geeksforgeeks.org/how-to-specify-how-much-work-the-task-requires-in-total-in-html5/`

[`<progress>`](https://www.geeksforgeeks.org/html-5-progress-tag/) 标签显示一个任务完成了多少。

**属性：**

*   `max`：任务所需的总工作量。1 是默认值。
*   `value`：表示任务已经完成的比例。

**备注：**

要显示任务的进度，根据 JavaScript 使用 `<progress>` 标记。 `<progress>` 标签不用于反映规格（例如，磁盘空间使用情况或查询结果的相关性）。相反，使用 [`<meter>`](https://www.geeksforgeeks.org/html5-meter-tag/) 标签来显示仪表。要获得最佳的可访问性实践，请使用 [`<label>`](https://www.geeksforgeeks.org/html-label-tag/) 标签。

**示例 1：** 以下示例演示了如何使用`<progress>`标签来显示进度条。
...
`<progress>` 标签也支持 HTML 全局属性。 `<progress>` 标签也支持 HTML 事件属性。