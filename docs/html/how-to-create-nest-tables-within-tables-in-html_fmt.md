...
> 原文：[https://www.geeksforgeeks.org/how-to-create-nest-tables-within-tables-in-html/](https://www.geeksforgeeks.org/how-to-create-nest-tables-within-tables-in-html/)

[HTML 表格](https://www.geeksforgeeks.org/html-tables/)对于以行和列的形式组织内容非常有帮助。但是有时需要在表中添加一个表。HTML 支持这种功能，被称为表的嵌套。表可以嵌套在一起，在表中创建一个表。

要创建嵌套表，我们需要使用`<table>`标记创建一个表。这张桌子被称为外桌。将成为嵌套表的第二个表称为内部表。该表也是使用`<table>`标签创建的，但是有一件特别的事情必须记住。
