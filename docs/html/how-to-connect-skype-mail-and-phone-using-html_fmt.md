...
> 原文：[https://www.geeksforgeeks.org/how-to-connect-skype-mail-and-phone-using-html/](https://www.geeksforgeeks.org/how-to-connect-skype-mail-and-phone-using-html/)

**连接邮件：** `mailto` 链接是当消费者想要沟通或者想要给出反馈时发送邮件的默认方式，那么通过点击 `mailto` 链接会打开一个默认的发送邮件窗口。因此，我们可以使用 `mailto`，一旦提交表单，邮件就会直接发送到该邮件。使用 `mailto` 我们可以设置表单的 `action` 字段，在这种情况下，web 浏览器调用电子邮件客户端将表单提交发送到指定的电子邮件地址。此外，我们还可以附上抄送、密件抄送、主题和正文内容。
...
**连接电话：** 随着手机上网的兴起，直接从网页打电话的好处变得更加现实。使用超文本标记语言很容易建立一个可调用的链接。HTML 为浏览器提供了协议，如 `tel`，用于添加可点击的电话号码。每个浏览器对这些协议的响应都不一样。有些人用显示屏上的号码启动手机应用，等你打电话，而有些人直接打电话。

**方法：**

*   在锚标签的 `href` 属性中添加 `tel:(一个号码)`。
