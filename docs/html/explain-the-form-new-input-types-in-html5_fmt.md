...
> 原文：[https://www.geeksforgeeks.org/explain-the-form-new-input-types-in-html5/](https://www.geeksforgeeks.org/explain-the-form-new-input-types-in-html5/)

在本文中，我们将讨论 HTML5 提供的较新的表单输入类型的功能。有时，在填写登记表或任何在线表格时，需要遵循适当的格式来填写特定的数据。现在很容易使用网络表单填写常见数据，如`日期`、`电子邮件`、`url`等。HTML5 表单中几乎引入了 13 种新的输入类型。我们将看到所有的输入类型并一一理解它们。

**输入类型属性：**

1.  `颜色`：该输入类型允许用户从颜色选择器中选择颜色。
2.  `日期`：此输入类型允许用户从下拉日历中选择日期。
3.  `时间`：此输入类型允许用户输入时间。
4.  `日期时间`：该输入类型允许用户选择日期和时间以及时区。
5.  `日期时间-本地`：该输入类型允许用户选择本地日期和时间。
6.  `周`：该输入类型允许用户从下拉日历中选择周和年。
7.  `电子邮件`：此输入类型允许用户输入电子邮件地址。
8.  `月`：该输入类型允许用户从下拉日历中选择月份和年份。
9.  `数字`：该输入类型允许用户输入数值。
10. `范围`：该输入类型允许用户在指定范围内输入数值。
...
在本例中，您将了解`日期时间`、`日期时间-本地`和`周`输入类型。
...
**注意：** 火狐、Safari 和 Internet Explorer 浏览器不支持`日期时间-本地`和`周`。
...
在本例中，您将了解`电子邮件`、`月份`、`号码`和`范围`输入类型。
