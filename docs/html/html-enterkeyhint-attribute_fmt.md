...
在本文中，我们将了解 HTML 中的 `enterkeyhint` 属性。`enterkeyhint` 属性是一个枚举属性，它指定在虚拟键盘上为 enter 键呈现什么操作标签。这是全局属性，允许作者自定义回车键的显示，以便对用户更有帮助。
...
本示例通过指定不同的值来描述 `enterkeyhint` 属性的用法。
