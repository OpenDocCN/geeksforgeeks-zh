...
> 原文：[https://www.geeksforgeeks.org/how-to-specify-the-type-of-files-that-server-accepts-in-html5/](https://www.geeksforgeeks.org/how-to-specify-the-type-of-files-that-server-accepts-in-html5/)

在本文中，我们将学习如何指定服务器在 HTML5 中接受的文件类型。 [`<input>`接受属性](https://www.geeksforgeeks.org/html-input-accept-attribute/)用于指定服务器接受的文件类型。该属性只能用于`<input type="file">`元素。此属性不用于验证工具，因为文件上载应该在服务器上进行验证。

我们将逐一添加每个特定的文件。

### 使用 MSOffice 文件
首先，我们将从使用 MSOffice 文件开始。我们将使用`accept`属性来指定文件类型。
