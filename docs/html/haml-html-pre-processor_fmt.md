...
Haml 代表汉普顿·卡特林创建的 `HTML 抽象标记语言`，创建它背后唯一的目的就是让标记变得美观。它基本上是一个基于 Ruby 的预处理器，需要 `Ruby` 安装在你的本地机器上，对于苹果操作系统，Ruby 是预先安装的，而对于 windows 用户，可以从[这里](https://rubyinstaller.org/)安装。要安装 Haml，请遵循以下命令。
