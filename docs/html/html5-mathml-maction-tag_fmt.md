...
下面的例子将说明 `<html 5 MathML <maction>>` tag 的概念示例:
...
**支持的浏览器:** 支持的浏览器有 **HTML5 MathML `<maction>` 标签** 如下:
