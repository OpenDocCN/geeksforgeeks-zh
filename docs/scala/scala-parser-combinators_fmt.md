...
3.  在语法中，顺序组合是隐式的，但在程序中，它是由显式运算符 `~` 来表示的。所以我们需要在一个作品的每两个连续符号之间插入一个 `~`。
4.  重复用 `rep( … )` 代替 `{ … }`。
5.  句号 `.` 在每个产品的末尾被省略，但是可以使用分号 `;`。
...
`ParseExpr` 对象定义了一个主方法，用于解析传递给它的第一个命令行参数。解析由表达式完成：`parseAll(expr, input)`。
