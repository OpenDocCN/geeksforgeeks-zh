...
不可变映射通过使用基于列表的数据结构来实现。[Scala 列表](https://www.geeksforgeeks.org/scala-lists/)类保存一个有序的线性项目列表。我们必须为 `ListMap` 导入 `scala.collection.mutable.ListMap`。`ListMap` 集合仅用于少量元素。
