...
**正则表达式**解释了一种用于匹配一系列输入数据的通用模式，因此它在众多编程语言的模式匹配中很有帮助。在 Scala 中，正则表达式通常被称为 Scala 正则表达式。

`Regex` 是从包 `scala.util.matching.Regex` 导入的类，广泛应用于搜索和文本解析。为了将字符串重铸为正则表达式，我们需要对所述字符串使用 `r()` 方法。
...
在这里，我们调用了所述字符串上的方法 `r()` 来获得 `Regex` 类的实例，以便创建一个模式。上面的代码中使用了 `findFirstIn()` 方法来查找正则表达式的第一个匹配项。为了找到表达式中所有匹配的单词，使用 `findAllIn()` 方法。
