...
**Groovy** 是一种面向对象的高级编程语言，与 `Java` 语法兼容。它被用作 Java 平台的编程语言和脚本语言。2004 年，Groovy 语言由鲍勃·麦克沃特和詹姆斯·斯特拉坎开发。Groovy 的源代码被编译成字节码，因此它可以在任何安装了 `JRE (Java 运行时环境)`的系统中运行。
...
**Scala** 是一种通用的高级编程语言，支持面向对象编程和函数编程的概念。这是一种可扩展的语言，用于为多个平台编写软件。2003 年，Scala 语言由马丁·奥德斯基先生和他的研究团队发明。Scala 源代码首先经过 Scala 编译器生成字节码，然后字节码传到 `JVM (Java 虚拟机)`生成输出。
