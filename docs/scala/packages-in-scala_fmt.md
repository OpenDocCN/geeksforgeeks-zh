...
包将数据绑定在一个文件中，或者作为数据封装，当保存一个文件时，它位于默认包下或文件顶部指定的包名下。包名和目录结构密切相关。例如，如果包名是 `college.student.cse`，那么将有 3 个目录，`college`、`student` 和 `cse`。使得 `cse` 存在于 `student` 中，`student` 存在于 `college` 中。
...
域名的包命名约定与 `org.geeksforgeeks.practice`、`org.geeksforgeeks.contribute` 的顺序相反。
