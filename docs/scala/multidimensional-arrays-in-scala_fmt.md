...
> 原文：[https://www.geeksforgeeks.org/multidimensional-arrays-in-scala/](https://www.geeksforgeeks.org/multidimensional-arrays-in-scala/)

**多维数组**基本上是以*矩阵格式*存储数据的数组。对于矩阵和表格，我们需要多维数组。我们可以用`Array`和`Array.ofDim`的方法创建多维数组。

## 语法

> `val arrayName = Array.ofDim[data_type](numberOfRows, numberOfColumns)`
> 或
...
这给了过程更多的控制，并让我们创建“参差不齐”的数组（其中每个包含的数组可能大小不同）。我们也可以使用`Array`来创建多维数组。在下面的例子中，我们使用数组的数组创建了一个多维数组。
