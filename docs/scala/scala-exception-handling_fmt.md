...
Scala 中的异常处理以不同的方式实现，但是它的行为与 Java 完全一样，并且与现有的 Java 库无缝协作。在 Scala 中，所有异常都是`未检查`的。没有检查异常的概念，Scala 在选择是否捕获异常的能力方面提供了很大的灵活性。
...
`throw`关键字用于抛出异常。它看起来和 Java 中的一样。我们创建一个异常对象，然后使用`throw`关键字抛出它。
