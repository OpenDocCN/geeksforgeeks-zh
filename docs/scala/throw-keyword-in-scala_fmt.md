...
val x =
  if(n % 10 == 0)
    ...
    throw new RuntimeException("n 必须是 10 的倍数")
```

## 解释

如果 `n` 是 10 的倍数，那么 5 被赋给 `x`，但是如果 `n` 不是 10 的倍数，那么在 `x` 可以被初始化为任何东西之前抛出一个异常。正因为如此，我们可以说 `throw` 具有任何一种价值。在 scala 中，抛出异常和 Java 是一样的。我们创建一个异常对象，然后用 `throw` 关键字抛出它。
