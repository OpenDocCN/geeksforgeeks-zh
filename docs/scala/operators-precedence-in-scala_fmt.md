...
> 原文: [https://www.geeksforgeeks.org/operators-precedence-in-scala/](https://www.geeksforgeeks.org/operators-precedence-in-scala/)
...
在上例中 `e = a + b * c / d`；这里，`e` 被赋值为 `50`，而不是 `120`，因为运算符 `*` 的优先级高于 `/` 和 `+`，所以它首先与 `10 * 15` 相乘，然后除以 `5`，然后相加为 `20`。