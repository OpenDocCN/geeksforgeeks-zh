...
> 原文：[https://www.geeksforgeeks.org/difference-between-haskell-and-scala/](https://www.geeksforgeeks.org/difference-between-haskell-and-scala/)
...
5.  **在 JVM 上运行 & 可以执行 Java 代码**：Java 和 Scala 有一个共同的运行时环境。因此用户可以很容易地从 Java 转移到 Scala。Scala 编译器将程序编译成`.class`文件，包含可以由`JVM`执行的字节码。
