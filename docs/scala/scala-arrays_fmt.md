...
在这里，`datatype` 指定要分配的数据类型，`size` 指定数组中的元素数量，`var` 是链接到数组的数组变量的名称。
...
多维数组包含多行来存储值。Scala 有一个方法 `Array.ofDim` 在 Scala 中创建多维数组。在像矩阵和表格这样的结构中，可以使用多维数组。
