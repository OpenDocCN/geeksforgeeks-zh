...
> 原文: [https://www.geeksforgeeks.org/scala-immutable-treeset-find-method/](https://www.geeksforgeeks.org/scala-immutable-treeset-find-method/)

在 Scala 不可变的`TreeSet`类中，`find()`方法用于返回满足树集中给定谓词的元素。

> **方法定义:** `def find(p: (A) => Boolean): Option[A]`
>
> **返回类型:** 返回满足给定谓词的第一个元素（如果存在），否则返回`None`。
