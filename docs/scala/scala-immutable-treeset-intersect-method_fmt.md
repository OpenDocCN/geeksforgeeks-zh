...
> 原文: [https://www.geeksforgeeks.org/scala-immutable-treeset-intersect-method/](https://www.geeksforgeeks.org/scala-immutable-treeset-intersect-method/)

在 Scala 不可变 `TreeSet` 类中，`intersect()` 方法用于返回一个新的树集，该树集由给定树集中的元素组成。

> **方法定义:** `def intersect[B >: A](that: collection.Seq[B]): TreeSet[A]`
>
