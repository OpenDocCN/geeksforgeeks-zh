...
> 原文: [https://www.geeksforgeeks.org/scala-sortedset-take-method-with-example/](https://www.geeksforgeeks.org/scala-sortedset-take-method-with-example/)

`take()` 方法用于返回由 `SortedSet` 的前 `n` 个元素组成的 `SortedSet`。

> **方法定义:** `def take(n: Int): SortedSet[A]`
> 其中 `n` 指定要选择的元素数量。
>
