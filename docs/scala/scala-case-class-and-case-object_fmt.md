...
## 案例类解释
一个`案例类`就像一个常规类，有一个对不变数据建模的特点。它在模式匹配中也是有建设性的。已经用修饰符`case`定义了，由于这个`case`关键字，我们可以得到一些好处来阻止自己做一段代码，这些代码必须包含在许多地方，很少或没有改变。正如我们在下面看到的，最小案例类需要关键字`案例类`、一个标识符和一个可能为空的参数列表。
...
## 注意:
`Case`类有一个默认的`apply()`方法，管理对象的构造。

## 格对象的解释
一个`格对象`也像一个对象，比普通对象有更多的属性。它是案例类和对象的混合。案例对象比常规对象有更多的特征。
下面两个是格对象的重要特征:
...
### 示例 : To create a duplicate of same instance without altering the parameters.
```scala
