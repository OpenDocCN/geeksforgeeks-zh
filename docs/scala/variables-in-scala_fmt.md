...
这里，`name` 是变量的名称，`String` 是变量的数据类型，`geekforgeeks` 是存储在内存中的值。
...
这里，`name` 是变量的名称，`String` 是变量的数据类型，`geeksforgeeks` 是存储在内存中的值。
...
Scala 支持变量类型推断。在变量类型推断中，值直接赋给变量，而不定义其数据类型，Scala 编译器自动解析哪个值属于哪个数据类型。
