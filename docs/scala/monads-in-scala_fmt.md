...
因此，当`flatMap`应用于上述序列时，将返回一个列表，其中内部分组被移除，并生成一个序列。

**注：** 支持`map`以及`flatMap`的集合称为`单子`。现在，让我们来看一些 Scala 中`单子`的例子。

支持`map`以及`flatMap`的收藏示例。
...
这里，`列表(x) = (1, 2, 3)` 和 `列表(y) = (1, 3, 5, 7)`。现在，让我们看看输出是如何计算的。
