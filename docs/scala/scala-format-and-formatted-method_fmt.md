...
> 原文：[https://www.geeksforgeeks.org/scala-format-and-formatted-method/](https://www.geeksforgeeks.org/scala-format-and-formatted-method/)
...
**注：**
*   在 Scala 中，字符串的格式化可以通过两种方法完成，即 `format()` 方法和 `formatted()` 方法。
*   这些方法都是从名为 `StringLike` 的特质着手的。

## Format method

`format()` 方法可用于格式化字符串，您甚至可以向其传递参数，其中 `%d` 用于整数，`%f` 用于浮点或双精度。

**示例：**
```scala
// Scala program of format method
...
**示例：**
```scala
// Scala program of format for strings and characters.
...
**例：**
```scala
// Scala program for formatted method
