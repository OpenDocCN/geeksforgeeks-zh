...
`reduce()` 方法是一个高阶函数，它获取集合（数组、列表等）中的所有元素，并使用二进制运算将其组合以产生一个值。有必要确保操作是可交换的和关联的。匿名函数作为参数传递给 `reduce` 函数。
...
通过 `reduce` 方法选择数字进行操作的顺序是随机的。这就是为什么不喜欢非交换和非关联操作的原因。
...
在上面的程序中，`reduce` 方法选择随机对，并找出特定对中的最大值。这些值再次相互比较，直到获得单个最大值元素。在 Spark 中处理弹性分布式数据集时，我们通常使用 `reduce()` 方法和 `map()` 方法。`map()` 方法帮助我们将一个集合转换为另一个集合，而 `reduce()` 方法允许我们执行一些操作。

**使用 `map()` 和 `reduce()` 查找平均值：**
...
在上面的程序中，集合的所有元素都被转换成带有两个元素的元组。元组的第一个元素是数字本身，第二个元素是计数器。最初，所有计数器都设置为 1。输出本身是一个包含两个元素的元组：第一个值是总和，第二个值是元素的数量。
