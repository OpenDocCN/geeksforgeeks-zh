...
> 原文: [https://www.geeksforgeeks.org/scala-immutable-treeset-forall-method/](https://www.geeksforgeeks.org/scala-immutable-treeset-forall-method/)

在 Scala 不可变 `TreeSet` 类中，`forall()` 方法用于检查谓词是否对树集合的所有元素都成立。

## 方法定义

定义为 `all(p: (A) => Boolean): Boolean`
