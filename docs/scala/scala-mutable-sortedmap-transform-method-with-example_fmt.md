...
> 原文: [https://www.geeksforgeeks.org/scala-mutable-sortedmap-transform-method-with-example/](https://www.geeksforgeeks.org/scala-mutable-sortedmap-transform-method-with-example/)

`transform()` 方法用于将 `SortedMap` 的元素转换为可变的 `SortedMap`。

> **方法定义:** `def transform(f: (K, V) => V): SortedMap.this.type`
>
