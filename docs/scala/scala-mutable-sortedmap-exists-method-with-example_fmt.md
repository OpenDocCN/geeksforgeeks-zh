...
> 原文: [https://www.geeksforgeeks.org/scala-mutable-sortedmap-exists-method-with-example/](https://www.geeksforgeeks.org/scala-mutable-sortedmap-exists-method-with-example/)

`exists()` 方法用于检查给定的谓词是否满足 `SortedMap` 的元素。

> **方法定义:** `def exists(p:(A, B) => Boolean): Boolean`
>
