# Scala TreeSet dropWhile()方法示例

> 原文: `https://www.geeksforgeeks.org/scala-treeset-dropwhile-method-with-example/`

在 Scala `TreeSet` 类中，`dropWhile()` 方法用于从前面删除满足树集中给定谓词的最长前缀。

## 方法定义与返回类型

**方法定义:** `def dropWhile(p: (A) => Boolean): TreeSet[A]`
