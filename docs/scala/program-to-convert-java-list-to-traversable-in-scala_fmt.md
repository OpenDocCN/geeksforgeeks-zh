...
> 原文：[`https://www.geeksforgeeks.org/program-to-convert-java-list-to-traversable-in-scala/`](https://www.geeksforgeeks.org/program-to-convert-java-list-to-traversable-in-scala/)
...
因此，返回一个整数缓冲区。这里，首先创建一个列表，使用 `add` 方法将 `int` 元素添加到列表中。之后，使用 `toTraversable` 方法将所述列表转换为可遍历集合。
