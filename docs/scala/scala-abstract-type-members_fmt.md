...
因此，在上面的类 `Sample` 中，我们已经声明了三个方法：`contents`、`width` 和 `height`。后两种方法的实现已经定义，而在第一种方法 `contents` 中，没有提到任何类型的实现。因此，该方法是 `Sample` 类的抽象成员。需要注意的是，具有抽象成员的类本身必须声明为抽象的。类前面的这个 `abstract` 关键字表示该类肯定会有一个没有实现的抽象成员。
...
在上面的例子中，抽象类被声明，它定义了一个抽象类型 `T`、一个抽象方法 `transform`、一个抽象初始值 `initial` 和一个抽象当前值 `current`。
...
在上面的例子中，特征 `vehicle` 有一个抽象的 `val category` 以及一个简单的名为 `car_type` 的具体方法和一个 `toString` 方法的覆盖。然后 `car`、`bike`、`bus`、`truck` 类扩展 `vehicle` 类，为字段 `category` 提供值。

我们在上面的代码中看到了抽象成员的未定义实现是如何被用来赋值和改变不同类型的每个对象的赋值的。就像在这个例子中，我们为不同类型的车辆存储了多个 `category` 值。
