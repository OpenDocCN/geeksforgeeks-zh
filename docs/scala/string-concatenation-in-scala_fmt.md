...
> 原文: [https://www.geeksforgeeks.org/string-concatenation-in-scala/](https://www.geeksforgeeks.org/string-concatenation-in-scala/)
...
在上面的例子中，我们使用 `concat()` 函数将第二个字符串连接到第一个字符串的末尾。`str1` 为 `Welcome! GeeksforGeeks`，`str2` 是 `to Portal`。在连接两个字符串后，我们得到新的字符串 `Welcome! GeeksforGeeks to Portal`。

## 使用 `+` 运算符

我们可以通过使用 `+` 运算符来添加两个字符串。
...
在上例中，字符串 `str1` 是 `Welcome to`，字符串 `str2` 是 `GeeksforGeeks`。通过使用 `+` 运算符连接两个字符串，我们可以在连接两个字符串后获得输出。