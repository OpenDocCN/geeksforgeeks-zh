...
> 原文: [https://www.geeksforgeeks.org/scala-tutorial-learn-scala-with-step-by-step-guide/](https://www.geeksforgeeks.org/scala-tutorial-learn-scala-with-step-by-step-guide/)
...
*   `函数式`: 它也是一种函数式编程语言，因为每个函数都是一个值，每个值都是一个对象。它支持高阶函数、嵌套函数、匿名函数等。
...
欲了解更多关于决策制定的信息，请参考 Scala [`决策制定`](https://www.geeksforgeeks.org/scala-decision-making-if-if-else-nested-if-else-if-else-if/)

## 循环

编程语言中的循环是一种功能，当某些条件评估为真时，它有助于重复执行一组指令/函数。循环使程序员的任务更简单。Scala 提供了不同类型的循环来处理程序中基于条件的情况。Scala 中的循环是:

*   [`for` 循环](https://www.geeksforgeeks.org/for-loop-in-scala/)
