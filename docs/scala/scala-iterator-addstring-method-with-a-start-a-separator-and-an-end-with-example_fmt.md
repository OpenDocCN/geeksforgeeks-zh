# Scala 迭代器 addString()方法

## 开头、分隔符和结尾

> 原文: `https://www.geeksforgeeks.org/scala-iterator-addstring-method-with-a-start-a-separator-and-an-end-with-example/`

用于将 Scala 迭代器的元素附加到字符串生成器的方法。该方法与 `addString()` 方法相同，但这里还包括一个开始、一个分隔符和一个结束。

> **方法定义:** `def addString(b: StringBuilder, start: String, sep: String, end: String): StringBuilder`
>
> 其中，`sep` 是所述的分隔符。
>
