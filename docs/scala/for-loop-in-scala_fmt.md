...
在 for 循环中，我们可以使用 `` `until` `` 我们想要打印从 0 到 n-1 的值。或者换句话说，直到 for 循环排除了如下程序所示的结束值，它只打印 0 到 9，而不打印 0 到 10。
...
在上面的例子中，for 循环使用两个过滤器来过滤给定的集合。这些过滤器消除了小于 7 和大于 2 的等级。
...
在上面的例子中，输出是一个变量，其中 `` `rank` `` 的所有值都以集合的形式存储。for 循环只显示排名大于 4 且不等于排名 8 的作者的排名。