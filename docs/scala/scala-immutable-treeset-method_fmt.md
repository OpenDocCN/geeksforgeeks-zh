...
> 原文: [https://www.geeksforgeeks.org/scala-immutable-treeset-method/](https://www.geeksforgeeks.org/scala-immutable-treeset-method/)

在 Scala 不可变的`TreeSet`类中，`&()`方法用于返回一个新的树集，该树集包含两个给定树集中存在的元素。

### 方法定义
`def &(that: TreeSet[A]): TreeSet[A]`

### 返回类型
它返回一个新的树集，该树集包含两个给定树集中存在的元素。
