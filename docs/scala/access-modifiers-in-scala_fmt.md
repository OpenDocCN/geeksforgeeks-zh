...
`scala` 中的访问修饰符用于定义 `Scala` 中包、类或对象成员的访问字段。要使用访问修饰符，必须在包、类或对象的成员定义中包含它的关键字。这些修饰符将把对成员的访问限制在代码的特定区域。
`Scala` 中有三种类型的访问修饰符:
...
当一个成员被声明为私有时，我们只能在定义类内部或者通过它的一个对象来使用它。
...
这里我们声明了一个变量 `a` 为 `private`，现在只能在它的定义类中或者通过 `class` 对象来访问它。
...
`Scala` 中没有 `public` 关键字。默认访问级别(未指定修饰符时)对应于 `Java` 的公共访问级别。我们可以在任何地方找到这些。
