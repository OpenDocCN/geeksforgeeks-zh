...
当我们想要在调用方法时在方法内部传递一个值时，我们使用这些变量。如果有从方法外部对对象的引用，则可以在方法内部和方法外部访问它们。这些变量`总是可变的`，与`val`关键字一起使用。
...
上例显示 `rect` 类是通过 `mult` 方法接受两个方法参数变量 `s1` 和 `s2` 而创建的。创建 `Area` 对象，并通过将值传递给变量 `s1` 和 `s2` 来调用 `rect` 方法。因此，下面是获得的输出。

## 本地变量

这些类型的变量在方法内部声明，并且只能在方法内部访问。使用`var`和`val`关键词，这些变量既可以是`可变的`也可以是不可变的。
...
上面的例子显示了类 `Area` 在方法 `mult` 中有局部变量 `s1`，`s2` 和 `s`。这些变量在方法之外是不可访问的。因此获得的输出如下。