...
*   逻辑或(`||`)运算符在所考虑的一个(或两个)条件满足时返回真。否则返回假。使用“*或*是`||`运算符的替代。例如，如果 `a` 或 `b` 中的一个为真(即非零)，则`a || b`返回真。当然，当 `a` 和 `b` 都为真时，它返回真。
*   逻辑非(`!`)如果所考虑的条件不满足，运算符返回 true。否则返回假。使用“*而不是*是一种替代`!`操作员。比如`!true`返回假。
...
*   加法与赋值(`+=`)运算符用于将左操作数和右操作数相加，然后赋值给左边的变量。
...
*   按位异或和赋值(`^=`)运算符用于对左操作数和右操作数执行按位异或运算，并将其分配给左边的变量。
