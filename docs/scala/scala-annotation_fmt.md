...
*   `@scala.beans.BeanProperty`: 这个注释，当前缀到某个变量 `X` 的定义时，会导致 Java bean 样式的 getter 和 setter 方法 `getX`、`setX` 被添加到包含该变量的类中。使用 `get` 或 `set` 后，变量的第一个字母看起来是大写的。
*   `@scala.beans.BooleanBeanProperty`: 这个注释相当于 `scala.reflect.BeanProperty`，但是生成的 getter 方法的名字是 `isX` 而不是 `getX`。
