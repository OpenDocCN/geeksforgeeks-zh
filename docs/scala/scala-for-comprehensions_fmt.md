...
例如 `b <- books`，在这个表达式中，值 `b` 迭代包含在 `books` 中的所有元素。
...
例如 `n = b.name`，变量 `n` 被绑定到值 `b.name`。该语句的结果类似于为了理解而在 `for` 之外编写这段代码: `val n = b.name`。
...
object Geeks 
{
    ...
    { 
        ...
        val MoreThanTwenty = for (language <- LanguageBase
                                  ...
                                  yield language.name // i.e. add this to a list
...
在上面的例子中，与 `yield` 语句一起使用的 `for` 循环实际上创建了一个列表。因为我们说 `yield language.name`，结果是一个 `List[String]`。`language` 是我们的发电机，`if (language.article >=20 && language.article < 30)` 是一个守卫，过滤掉那些文章数量不在 20 到 30 之间的元素。

### 例 2: 没有 `yield`

我们可以在理解中省略 `yield`。在这种情况下，理解将返回 `Unit`。这可能是有帮助的，以防我们想要执行副作用。这里有一个类似上面的程序，不使用 `yield`。
