...
在 Scala 中，`flatMap()` 方法与 [`map()`](https://www.geeksforgeeks.org/scala-map-method/) 方法相同，唯一不同的是在 `flatMap` 中，删除了一个项目的内部分组，生成了一个序列。可以定义为 `map` 方法和 `flatten` 方法的混合。运行 `map` 方法后运行 `flatten` 方法得到的输出与 `flatMap()` 得到的输出相同。因此，我们可以说 `flatMap` 首先运行 `map` 方法，然后运行 `flatten` 方法来生成期望的结果。

**注:**

*   它有一个内置的 `Option` 类，作为一个选项，可以表示为一个最多有一个元素的序列，即要么是空的，要么只有一个元素。
*   `flatten()` 方法用于分解 Scala 集合的元素，以便用相似类型的元素构建一个集合。

让我们看一个例子来说明 `flatMap` 是如何工作的。
...
所以，我们在这里可以看到两种情况下得到的输出是一样的。因此，我们可以说 `flatMap` 是 `map` 和 `flatten` 方法的组合。
...
*   Utilizing `flatMap` on a sequence of Strings.
    ...
    **Output:**
    ...
    这里，`flatMap` 被应用于所述序列，因此，产生字符序列的列表。
...
*   Applying `flatMap` with another functions.
    ...
    **Output:**
    ...
    这里，`flatMap` 应用于程序中定义的另一个函数，因此生成了一个数字序列列表。让我们看看输出是如何计算的。
    ...
    因此，第一步的工作就像在所述的另一个函数上应用 `map` 方法。

    ```scala
    List(1, 2, 3, 2, 3, 4, 3, 4, 5)
    ```

    第二步的工作方式类似于将 `flatten` 应用于第一步通过 map 方法获得的输出。
    ...
    **Example:**
    ...
    object GfG {
        ...
        def main(args: Array[String]) {
            ...
            val seq = Seq(4, 5, 6, 7)
            // Applying flatMap on another function
    ...
    **Output:**
    ...
    这里，也获得了类似于具有另一功能的 `flatMap` 的第一示例的输出。
