...
Scala 中的 [`Option`](https://www.geeksforgeeks.org/scala-option/) 是指某一特定类型的单一元素或无元素的载体。当一个方法返回一个甚至可以为空的值时，就使用了 `Option`，即定义的方法返回一个 `Option` 的实例，而不是返回单个对象或空值。
有几个方法我们可以调用 Scala `Option`。
