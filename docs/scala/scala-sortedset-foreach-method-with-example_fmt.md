...
> 原文: [https://www.geeksforgeeks.org/scala-sortedset-foreach-method-with-example/](https://www.geeksforgeeks.org/scala-sortedset-foreach-method-with-example/)

`foreach()` 方法用于将给定函数应用于排序集合的所有元素。

## 方法定义

> **方法定义:** `def foreach(f: (A) => Unit): Unit`
>
