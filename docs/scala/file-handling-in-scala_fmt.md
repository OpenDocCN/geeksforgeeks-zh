...
> 2.  `File(字符串路径名)` 将参数字符串转换为抽象路径名，创建一个新的文件实例。
>
> **写入文件**
...
Scala 不提供写文件的类，但是它提供读文件的类。这是 `Source` 类。我们使用它的伴随对象来读取文件。为了读取这个文件的内容，我们调用类 `Source` 的 `fromFile()` 方法来读取包含文件名作为参数的文件内容。

> *   `scala.io.Source` 包括源文件的可迭代表示方法。
> *   `Source.fromFile` 从输入文件创建一个源文件。
