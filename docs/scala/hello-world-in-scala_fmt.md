...
*   **`println(“你好世界！”)`**: `println` 是 Scala 中的一种方法，用于在控制台上显示输出。
...
第一步: 使用 `scalac Hello.scala` 编译上述文件。编译后会生成一个 `Geeks.class` 文件，类文件名和对象名相同(这里对象名是 `Geeks`)。
第二步: 现在打开命令 `scala Geeks`。它会给出结果。
