...
`Product2` 是 Scala 中的一个[特性](https://www.geeksforgeeks.org/scala-traits/)，是两个元素的笛卡尔乘积。在内置类中，它可以被认为是两个元素的元组。这里的线性超类型是`Product`、`Equals`、`Any`，这里的子类是`Tuple2`。`Product2` 扩展了如下产品：
