...
`String` 是一个字符序列。在 Scala 中，`String` 的对象是不可变的，这意味着一个常量，一旦创建就不能更改。
...
| `String replace(char ch1, char ch2)` | 这个函数返回一个新的字符串，其中 `ch1` 的元素被 `ch2` 替换。 |
| `String[] split(String reg)` | 这个函数在给定正则表达式的匹配项周围拆分字符串。 |
| `String substring(int i)` | 此函数返回一个新字符串，它是给定字符串的子字符串。 |
| `String trim()` | 这个函数返回一个字符串的副本，去掉开头和结尾的空白。 |
| `boolean startsWith(String prefix)` | 该函数用于检查给定的字符串是否以指定的前缀开头。 |