...
Scala 中的 Currying 简单来说就是一种转换函数的技术或者过程。该函数将多个参数转换为一个接受单个参数的函数。它广泛应用于多种功能语言中。
...
假设，我们必须将这个 `add` 函数转换为 Curried 函数，也就是将接受两个(多个)参数的函数转换为接受一个(单个)参数的函数。
...
## 使用部分应用的 curry Function

我们有另一种方法来使用这个 Curried function，那就是部分应用的 function 。所以，让我们举一个简单的例子来理解。我们在主函数中定义了一个变量和
