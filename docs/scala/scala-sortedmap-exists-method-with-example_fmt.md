...
> 原文: [https://www.geeksforgeeks.org/scala-sortedmap-exists-method-with-example/](https://www.geeksforgeeks.org/scala-sortedmap-exists-method-with-example/)

`exists()` 方法用于检查给定的谓词是否满足 `SortedMap` 的元素。

> **方法定义:** `def exists(p:(A，B) =>布尔型):布尔型`
>
