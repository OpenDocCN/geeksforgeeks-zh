...
这里定义了两种应用分部函数的方法，其中`isDefinedAt`状态条件，如果满足给定条件，`apply`执行操作。

## 定义分部函数的方法:
分部函数有一些定义方法，包括`case语句`、`collect方法`、`orElse`和`andThen`。
...
这里，使用 `case` 语句创建分部函数，所以这里不需要`apply`和`isDefinedAt`。
...
这里，`andThen`将把部分函数的输出附加给另一个给定的函数，然后返回那个值。