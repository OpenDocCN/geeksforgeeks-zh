...
*   使用 `Stream.from()`，创建一个正在生成连续数字的流。这个数字从参数开始。
*   一个数字流被赋予 `sieve_of_Eratosthenes` 方法。这个方法通过过滤掉元素，缓慢地生成连续的元素。
...
对厄拉多塞方法的 `sieve_of_Eratosthenes` 进行了一些修改，以便使用流创建 `abc()` 方法。元素从递归流中取出并打印出来。
