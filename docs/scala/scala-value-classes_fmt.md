...
正如我们可以看到的，在上面的例子中，一个被创建和表示的价值类是 `int` 。上述代码由值类 `Vclass` 中的 `def` 组成。这里 `Vclass` 是一个用户定义的值类，它包装了 `Int` 参数并封装了一个 `square` 方法。要调用 `square` 方法，请创建 `Vclass` 类的对象，如下所示: `val v = new Vclass(5)`
...
*   值类不能有惰性 `val`、`var` 或 `val` 作为成员。它只能有 `def` 作为成员。
*   没有其他类可以扩展值类。