...
> 原文：[https://www.geeksforgeeks.org/scala-sequence-comprehensions/](https://www.geeksforgeeks.org/scala-sequence-comprehensions/)

理解具有 `( enumerators ) * yield e` 的结构，其中枚举器是指以分号分隔的枚举数列表。[枚举器](https://www.geeksforgeeks.org/enumeration-in-scala/)要么是引入新变量的生成器，要么是过滤器。理解为枚举器生成的每个绑定评估主体 `e`，并返回这些值的序列。在本文中，Scala 提供了一个轻量级的符号来表达**序列理解**。在测序中，可以在没有平面图的情况下进行多次提取。这可以通过两种方法来实现，一种是使笛卡儿积重复，另一种是对对象列表进行排序。不使用平面图的另一种方法是为 Scala 中的验证计划一种特殊的自动生成语法。
...
在本例中，一个新的整数变量 `i` 被绑定到列表 `List.range(from, to)` 的所有值。如果 `i % 4 == 0` 它将从列表中移除所有奇数，并给出 0，20 之间完全可被 4 整除的数字的输出。
