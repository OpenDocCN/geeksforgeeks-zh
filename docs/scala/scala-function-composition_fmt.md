...
在上面的语法中，函数 2 首先使用传递的参数，然后返回一个要传递给函数 1 的值。
...
在上面的例子中，首先调用 `mul` 函数，得到 4(2 * 2)，然后调用 `add` 函数，得到 5(4 + 1)。类似地 `(add compose mul compose sub)(2)` 将打印 3(步骤 1:2–1 = 1，步骤 2 : 1 * 2 = 2，步骤 3 : 2 + 1 = 3)。

## andThen

`andThen` 方法也与 `val` 函数协同工作。
**语法：**
...
在上面的语法中，函数 1 首先使用传递的参数，然后返回一个要传递给函数 2 的值。
或与下图相同:
...
在上面的例子中，首先 `add` 一个被调用的函数，我们得到 3(2 + 1)，然后再 `mul` 一个被调用的函数，我们得到 6(3 * 2)。类似地，`(add andThen mul andThen sub)(2)` 将打印 5(步骤 1 : 2 + 1 = 3，步骤 2 : 3 * 2 = 6，步骤 3:6–1 = 5)。
