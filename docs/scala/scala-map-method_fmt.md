...
在上面的例子中，我们将用户定义的函数 `square` 作为参数传递给 `map()` 方法。将创建一个新的集合，其中包含原始集合的元素方块。源集合不受影响。
...
在上面的例子中，一个[匿名函数](https://www.geeksforgeeks.org/anonymous-functions-in-scala/)作为参数传递给了 `map()` 方法，而不是定义一个整函数进行平方运算。这种方法减少了代码大小。