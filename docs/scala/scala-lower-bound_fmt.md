...
在本节中，我们将更加关注下限。**类型下限**用于声明另一个类型的超类型。下限用 `>:` 表示。任何下限语句都可以写成 `[T >: S]`。这里 `T` 是类型参数，`S` 是类型。上述下界声明意味着类型参数 `T` 必须是类型 `S` 或 `S` 的超类型。

让我们理解超类型，假设我们写 `S >: T`，那么这个下界语句总是意味着 `T` 等于 `S` 或是 `S` 的子类型，这意味着在这种情况下 `T` 是有界的。所以我们可以说 `T` 是 `S` 的超类型。
...
在上面的例子中，我们定义了从类型参数 `T` 到类型 `Demo` 的下限。并且 `T` 必须是 `Demo` 或 `Demo` 类型的超类型。
...
现在所有的类和子类都已经用上面的语句声明了。所以如果我们考虑语句 `position[A]`，这里 `position` 是泛型，`A` 是抽象类型。这可以进一步简化，`position[A]` 表示传递到 `position` 所需的值必须是类型 `A`。这意味着如果我给出一个命令 `position[fruits]`，那么它将使用 `fruits` 的实例来创建。为了检验这一点，我们将看到一个例子：
...
在上面给出的例子中，我们将注意到第一条语句将被编译，但是第二条语句将显示错误，因为 `fruits` 和 `drinks` 不是同一类型的。另一方面，第三个语句将被编译，因为 `vegetables` 是 `eatables` 的子类型。现在在上面给出的语句中使用一些下限命令：

```scala
class position[A >: fruits](val place=A)
```

通过这种说法，我们可以说，我们可以定位任何 `fruits` 的超类型，这意味着 `eatables` 和 `Food`。因此，通过这一点，我们可以明确地看到，如果对任何真实世界的例子使用一些逻辑，Scala 的下限都是非常有用和适用的。因此，我们可以将所有现实生活中的限制和约束加到我们在编码语言中使用的变量上。
...
这里，下界是在类 `ComputerSciencePortal` 中定义的，`GeeksforGeeks` 是 `Author` 的超类。它在下限被接受。
...
这里我们定义了类 `School` 中的下界，即 `[T >: Teacher]`，这意味着 `display` 方法接受 `Principal` 类对象或其子类（即 `Teacher`）或 `Teacher` 类子类（即 `Student`），这意味着将接受 `Teacher` 的超类。