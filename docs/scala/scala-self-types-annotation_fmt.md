...
一个[特质](https://www.geeksforgeeks.org/scala-traits/)的**自身类型注释**是 `this` 的假定类型，在一个特质内，被使用的接收者。任何混入特质的具体类都必须确保其自身类型符合被混入的特质。这意味着使用自身类型不会将局部变量和方法暴露给它的子类和子特征。把一个大类分成几个特征是自我类型最常见的用法。自我类型是一种声明一个特征必须混合到另一个特征或类中的方式，即使它没有直接扩展它，这使得那些混合在特征中的依赖成员可以不导入。

要在特质中使用自我类型，在它旁边写下标识符名称，在它后面写下要混合的特质的类型，并跟随 `=>` 符号；如下所示。
...
在上面的例子中，我们创建了 `with_powers`、`without_powers` 和 `person` 特征。这里类 `extraordinary_person` 扩展特质 `person`。同样类 `ordinary_person` 也扩展特质 `person`。自我类型标注发生在两个类中，通过使用 `this: 特质名 =>`。`val mohan= new ordinary_person() with with_powers;` 显示一个不符合 `ordinary_person` 自我类型的错误。
...
这里，在上面的例子中，`C2` 是一个抽象类型，`C2` 的自我类型与 `B` 是一致的。`(new C2 with B)` 符合 `C2` 的自我类型与 `B` (`this: B =>`)。所以，自我类型让你指定什么类型的特征被允许混合。
...
在上面的例子中 (`this: Being =>`)，它基本上意味着 `Being` 的变量 `name`、`age`、`gender` 现在都在 `greetings` 和 `info` 方法的范围内，但是没有将它们暴露给扩展它的任何其他子类或特征。所以自我类型指定了任何具体类的需求，这种类型的特征应该被混合在其中。