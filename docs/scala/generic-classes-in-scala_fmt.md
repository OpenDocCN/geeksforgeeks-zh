...
在 Scala 中，形成**泛型类**与在 Java 中形成泛型类极其相似。像参数一样接受类型的类被称为 Scala 中的*泛型类*。这个类在方括号内取一个类似参数的类型，即`[ ]`。在 Scala 中，这些类被明确用于收集类的进度。泛型类型的子类型是不变的，即如果我们有两个列表类型，`A`和`B`，那么`List[A]`是`List[B]`的子类型，当且仅当`B`类型相当于`A`类型。

## 需要记住的几点

*   用于简单类型的类型参数的符号是`A`，类似于`List[A]`。
*   用于第二、第三、第四等类型参数的符号，泛型类中的类型分别为`B`、`C`、`D`等。
*   在 Scala `Map`中，用于键的符号是`A`，用于值的符号是`B`。
*   用于数值的符号是`N`。
...
这里，抽象类`Divide`有一个类型参数`z`，它存在于方括号中，所以这个类是泛型类型，这个类型参数`z`可以占用每一个数据类型。我们定义了一个方法`divide`在泛型类内部，泛型类有两个变量，即`u`和`v`，这些变量的数据类型为`z`。如上所述的类`intDivide`采用整数类型，如上所述的类`doubleDivide`采用双类型。因此，类型参数`z`被`Int`和`Double`数据类型所取代。通过这种方式，子类型在泛型类中是可能的。
...
object GfG
{
    ...
    def main(args: Array[String]) 
    {
        ...
        def addition[N](a: N, b: N)(implicit num: Numeric[N]): N = num.plus(a, b)
