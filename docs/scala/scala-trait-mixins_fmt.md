...
我们可以用一个类或一个抽象类来扩展几个Scala特征，这个抽象类被称为特征融合子。值得知道的是，只有特质或特质与类的交融或特质与抽象类的交融才能被我们延展。这里甚至必须保持特征序列`with`混合，否则编译器会抛出一个错误。
...
在这里，Mixins 的正确顺序是，我们需要先扩展任意类或抽象类，然后用关键字`with`扩展任意性状。
...
object GfG
{
    ...
    def main(args:Array[String])
    {
        // Creating object of 'Myclass' with trait 'Text'
