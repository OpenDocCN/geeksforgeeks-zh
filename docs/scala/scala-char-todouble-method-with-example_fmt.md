...
> 原文: [https://www.geeksforgeeks.org/scala-char-todouble-method-with-example/](https://www.geeksforgeeks.org/scala-char-todouble-method-with-example/)

`toDouble()` 方法用于将指定字符转换为 `Double` 或其类型为 `Double` 的 ASCII 值。

> **方法定义:** `def toDouble: Double`
>
