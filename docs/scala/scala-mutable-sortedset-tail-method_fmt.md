...
> 原文: [https://www.geeksforgeeks.org/scala-mutable-sortedset-tail-method/](https://www.geeksforgeeks.org/scala-mutable-sortedset-tail-method/)

在 Scala 可变集合中，`tail()` 方法用于返回一个 `SortedSet`，它由除 `SortedSet` 的第一个元素之外的所有元素组成。

> **方法定义:** `def tail: SortedSet[A]`
>
