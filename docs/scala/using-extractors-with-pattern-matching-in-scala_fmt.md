...
> 原文: [https://www.geeksforgeeks.org/using-extractors-with-pattern-matching-in-scala/](https://www.geeksforgeeks.org/using-extractors-with-pattern-matching-in-scala/)

Scala Extractor 被定义为一个对象，它的一部分是一个名为 `unapply` 的方法。[提取器](https://www.geeksforgeeks.org/scala-extractors/)可用于[模式匹配](https://www.geeksforgeeks.org/scala-pattern-matching/)。在模式匹配中比较提取器的对象时，`unapply` 方法将自动执行。
...
在上面的例子中，对象名是 `GFG`，我们也在使用 `unapply` 方法和 `match` 表达式。
...
一个 `Case` 类中已经有了提取器，所以可以通过模式匹配自发地使用。