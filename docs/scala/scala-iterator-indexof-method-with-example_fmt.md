...
`indexOf()` 方法属于 `AbstractIterator` 类的具体值成员。它有助于搜索值，然后指出它们在指定迭代器中的位置。
...
这里，`indexOf()` 方法中的值 `9` 出现在迭代器的第三个位置，因此它返回三。
...
这里，迭代器中不存在 `indexOf()` 方法中声明的值，因此它返回 `-1`。

**注意：** 如果在所述迭代器中不存在 `indexOf()` 方法中给出的值，则该方法将返回 `-1`。