...
在 Scala 中，我们使用 `break` 语句来中断程序中循环的执行。Scala 编程语言不包含任何 `break` 语句的概念（在 2.8 版本以上），它提供了一个 `break` 方法代替 `break` 语句，用于中断程序或循环的执行。通过导入 `scala.util.control.breaks._` 包来使用此功能。
...
## 说明
在上例中，`num1` 和 `num2` 的初始值均为 0。现在，首先外部 `for` 循环开始并打印 `x` 列表中的 5，然后内部 `for` 循环开始工作并打印 `y` 列表中的 20，25，当控件进入 `num2 == 25` 条件时，内部循环中断。10 和 15 也是如此。