...
多个线程同时执行的过程，称为**多线程**。它允许您独立执行多个任务。

## Scala 中的线程是什么？

线程是轻量级的子进程，占用内存较少。一个多线程程序包含两个或多个可以并发运行的线程，并且每个线程可以同时处理不同的任务，特别是当您的系统（计算机）有多个 CPU 时，可以最佳利用可用资源。多线程在 Scala 中用于开发并发应用程序。
...
我们创建了一个扩展 `Thread` 类的类。此类重写 `Thread` 类中可用的 `run()` 方法。线程在 `run()` 方法中开始它的生命。我们创建一个新类的对象，并调用 `start()` 方法来启动线程的执行。`start()` 调用 `Thread` 对象上的 `run()` 方法。
...
### 通过扩展 Runnable 接口创建线程

我们创建了一个新的类，它扩展了 `Runnable` 接口并覆盖了 `run()` 方法。然后我们实例化一个 `Thread` 对象，将创建的类传递给构造函数。然后我们在这个对象上调用 `start()` 方法。
