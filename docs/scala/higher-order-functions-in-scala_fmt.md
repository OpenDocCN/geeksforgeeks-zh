...
> 原文：[https://www.geeksforgeeks.org/higher-order-functions-in-scala/](https://www.geeksforgeeks.org/higher-order-functions-in-scala/)
...
这里，`apply`函数包含另一个函数`x`，其值为`y`，并将函数`x`应用于`y`。
...
这里，`map`是以另一个函数即`(y => multiplyValue(y))`为参数的函数，所以是高阶函数。