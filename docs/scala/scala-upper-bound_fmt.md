...
其中，`T` 是类型参数，`S` 是类型。`[T <: S]` 意味着这个类型参数 `T` 必须与 `S` 相同或是 `S` 的子类型。
...
这里，*上界*是在 `ComputerSciencePortal` 类中定义的，`GeeksforGeeks` 是 `Author` 的超级类，所以不接受。
...
这里我们已经定义了*上界*在类 `School` 即 `[T <: Teacher]`，这意味着 `display` 方法接受 `Teacher` 类对象或其子类（即 `Student`），这意味着 `Teacher` 的超类将不被接受。所以，如果老师的超级班级，即 `Principal` 没有被评论，那么就会显示*类型不匹配*错误。