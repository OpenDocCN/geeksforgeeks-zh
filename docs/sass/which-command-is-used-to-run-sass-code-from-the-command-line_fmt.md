...
我们可以使用以下命令从 `npm` 安装最新版本的 SASS 包。在我们运行以下命令之前，必须在系统上安装 `Node.js`。详细安装流程请参考 [Node.js NPM(节点包管理器)](https://www.geeksforgeeks.org/node-js-npm-node-package-manager/)。现在，运行下面的命令来全局安装 SASS 编译器。
...
如果我们在当前的工作目录中有一个文件 `style.scss`，我们想把它编译成一个 css 文件，作为同一个目录中的 `style.css`。我们使用以下命令:
...
如果我们在另一个目录中有一个文件 `style.scss`(在这种情况下，它在“D:\Geeksforgeeks”中)，而不是在当前目录中，并且我们希望在同一个目录中将它编译成一个作为 `style.css` 的 css 文件，我们可以使用以下命令:
