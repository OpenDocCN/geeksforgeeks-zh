...
就像任何其他编程语言一样，SASS 提供了许多内置模块，这些模块包含各种有用的功能和一些混合，使其易于使用。模块将使用 `@use` 规则加载，就像任何其他用户定义的样式表一样，类似地，它的函数可以像任何其他模块成员一样被调用。所有内置模块都以 `sass:` 开头，以表明它们不同于其他模块，是 sass 本身的一部分。

## 兼容性

目前只有 Dart Sass 支持使用内置模块。像 LibSass 或 Ruby Sass 这样的其他实现需要使用它们的全局名称来调用函数，而不是使用 `@use` 规则。
...
*   `sass:selector` 模块提供对 sass 选择器引擎的访问。
