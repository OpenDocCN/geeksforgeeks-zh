...
> 原文: [https://www.geeksforgeeks.org/how-to-see-the-changes-in-whole-directory-folder-containing-many-sass-files/](https://www.geeksforgeeks.org/how-to-see-the-changes-in-whole-directory-folder-containing-many-sass-files/)
...
安装最新版本的 Node.js 后，按照以下步骤编译您的 Sass 文件。
...
`node-sass` 是一个将 SCSS 本机编译成 CSS 文件的库。此命令将安装 `node-sass` 作为开发依赖项。

### 3. 修改 scripts 字段

在 `package.json` 文件中，您需要对 `scripts` 字段进行更改。
