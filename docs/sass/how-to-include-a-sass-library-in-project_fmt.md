...
> 原文：[https://www.geeksforgeeks.org/how-to-include-a-sass-library-in-project/](https://www.geeksforgeeks.org/how-to-include-a-sass-library-in-project/)

## 本文的先决条件

本文的先决条件包括一些关于 [SASS](https://www.geeksforgeeks.org/sass-introduction/) 及其在不同领域的应用的知识。
...
### 方法 4：使用 Windows 包管理器或 Mac 包管理器

如果你是 `windows` 或 `mac` 用户，你可能不知道巧克力（`Chocolatey`）和 `Homebrew`。这些是软件包管理器，它使任何用户都可以轻松地下载、更新或卸载他们系统中的任何应用程序。此外，它还自动设置环境变量。因此，我们也可以使用这样的管理器来安装 `sass`。对于 `windows`，如果我们想这样做，我们必须编写下面给定的代码。
