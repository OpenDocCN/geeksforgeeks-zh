...
> 原文: [https://www.geeksforgeeks.org/what-are-the-data-types-that-sass-supports/](https://www.geeksforgeeks.org/what-are-the-data-types-that-sass-supports/)
...
Sass 将 Number 视为整数和实数，在使用时，它确实会识别正在使用哪些值。有一些标准单位被用作 `px`(像素单位)或 `em`(短暂单位)。为了对数字进行运算，我们必须确保使用相同的单位，否则会产生一些错误。
...
Sass 也有布尔数据类型，布尔有两个值，可以是真也可以是假。为了使用布尔运算，它提供了三个基本操作符，它们被称为 `and`，`or` 和 `not`。请注意，我们使用小写字母作为运算符，因为大写字母在 Sass 中将被视为字符串。
...
*   `or`: 如果两个值中有一个为真，则返回真值；如果两个值都为假，则返回假值。
...
Sass 支持 List 数据类型，基本上是用来表示一系列用空格或逗号分隔的值。请注意，如果列表中使用了单个值，它将被视为有效列表。
...
Sass 中的地图基本都是键值对。这些对必须用逗号分隔，并且所有对都必须用括号括起来。
...
Null 数据类型只包含一个基本上不知道或未知的值，它表示为小写的 `null`。正如我们看到的，有些数据类型有一些函数和运算符，但是对于 null，在 Sass 中没有任何运算符和函数可用。
...
字符串是有序的字符序列，可以用双引号或单引号表示，也可以不用引号。这里有一些例子，将明确我们对字符串的看法。
...
Sass 支持颜色作为数据类型，因为它用于定义颜色值。在这里，我们可以看到在常规 CSS 中使用的相同的颜色表达式。
