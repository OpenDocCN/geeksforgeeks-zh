...
这种类型的 mixin 是可以使用的，当我们不需要改变属性的值即我们想一次又一次地使用一组具有相同属性值的属性时。

**语法:**

*   定义 mixin: `@mixin name-of-mixin {…}`
*   在当前块中使用 mixin: `@include name-of-mixin;`。
...
这种类型的 mixin 可以使用，当我们想要一次又一次地使用一组具有不同值的属性时，我们可以像函数一样使用参数传递不同的值。这里，参数默认值的概念与任何其他编程语言相同。

**语法:**

*   定义 mixin: `@mixin name-of-mixin(参数…){…}`
*   使用 mixin: `@include name-of-mixin(parameter …)` 在当前块中。
