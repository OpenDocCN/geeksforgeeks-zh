...
> 原文: [https://www.geeksforgeeks.org/sass-shadowing-and-flow-control/](https://www.geeksforgeeks.org/sass-shadowing-and-flow-control/)

阴影和流量控制是改变[变量](https://www.geeksforgeeks.org/sass-variables/)值的方法。当我们使用 SASS 库时，这两种方法非常有用。我们将逐一讨论。

## 1. Shadowing

局部变量可以被声明为与全局变量同名。如果我们这样做，那么该变量在局部作用域中的值将是该局部作用域中的变量值，而不是全局值。请看下面的例子。
...
现在，如果您想在条件执行的某个时刻或出于任何原因更改全局变量的值，该怎么办？SASS 允许我们使用 `!global`。局部变量声明后的 `!global`。这个概念之所以被称为“阴影化”，是因为全局变量值的变化使用了 `!global` 会反映在整个文档中，而不仅仅是在 `!global` 表达式所在的作用域。
...
您可以使用流程控制编译来更改变量的值。在这里，变量的值将在流程控制块之后立即改变。它之前的变量值将保持不变。请看下面的例子。
