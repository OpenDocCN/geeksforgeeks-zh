...
SCSS 语法使用`.scss`文件扩展名。和 CSS 很像。你甚至可以说 SCSS 是 CSS 的超集，这意味着所有有效的 CSS 也是有效的 SCSS。由于它与 CSS 的相似性，它是最简单和流行的 SASS 语法。
...
这个语法是 SASS 的原始语法，它使用`.sass`作为文件扩展名，正因为如此，它有时被简称为 sass。这种语法具有与 SCSS 相同的所有特征，唯一的区别是 SASS 使用缩进而不是 SCSS 的花括号和分号。
