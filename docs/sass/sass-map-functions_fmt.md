...
## 1. `map-has-key($map, $key)` 函数

该函数返回一个布尔值，检查给定的地图是否包含给定的键。
...
## 2. `map-merge($map1, $map2)` 函数

该函数返回一个包含 `$map2` 的地图，该地图连接到 `$map1` 的末尾。
