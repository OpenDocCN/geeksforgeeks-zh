...
> 原文: `https://www.geeksforgeeks.org/sass-extend-rule/`
...
`@extend` 规则更新包含扩展选择器的样式规则，使其包含扩展选择器。当扩展选择器时，Sass 通过从不生成像 `#main#footer` 这样不可能匹配任何元素的选择器来实现智能统一。它还确保了复杂的选择器是交错的，这样无论 HTML 元素以何种顺序嵌套，它们都可以工作。
