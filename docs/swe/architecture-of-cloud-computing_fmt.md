...
> 原文：[https://www.geeksforgeeks.org/architecture-of-cloud-computing/](https://www.geeksforgeeks.org/architecture-of-cloud-computing/)
...
*   `客户端基础设施` – `客户端基础设施` 是指前端组件。它包含访问云平台所需的应用程序和用户界面。
...
1.  `应用` –
    后端的 `应用` 是指客户端访问的软件或平台。意味着它根据客户需求在后端提供服务。
2.  `服务` –
    后端的 `服务` 指的是三大类基于云的服务，如 [`SaaS`、`PaaS` 和 `IaaS`](https://www.geeksforgeeks.org/cloud-based-services/) 。还管理用户访问的服务类型。
3.  `云运行时` –
    后端的 `云运行时` 是指向虚拟机提供执行和运行时平台/环境。
4.  `存储` –
    后端 `存储` 是指对存储的数据提供灵活、可扩展的存储服务和管理。
5.  `基础设施` –
    后端的云 `基础设施` 是指云的硬件和软件组件，像它包括服务器、存储、网络设备、虚拟化软件等。
6.  `管理` –
    后端 `管理` 是指对应用、服务、云运行时、存储、基础设施和其他安全机制等后端组件的管理。
7.  `安全性` –
    后端 `安全性` 是指在后端实施不同的安全机制，为最终用户保护云资源、系统、文件和基础设施。
8.  `互联网` –
    `互联网` 连接充当前端和后端之间的媒介或桥梁，建立前端和后端之间的交互和通信。
