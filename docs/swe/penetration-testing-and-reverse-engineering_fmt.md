...
> 原文：[https://www.geeksforgeeks.org/penetration-testing-and-reverse-engineering/](https://www.geeksforgeeks.org/penetration-testing-and-reverse-engineering/)
...
*   `hexdump` – 允许用户查看特定输入数据的十六进制视图，能够将文件内容提取为多种格式，如十进制、八进制和 ASCII。
*   `Objdump` – 显示关于目标文件的信息，并用于反汇编可执行文件。
*   `strings` – 提取嵌入在二进制文件和其他可执行文件中的字符串。
...
*   `固件修改套件` – 允许对嵌入式设备中的各种固件进行解压缩。它支持各种文件系统和版本，如 SquashFS 2.0、3.0 和 4.0。