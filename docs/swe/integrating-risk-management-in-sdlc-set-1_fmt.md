...
> 原文：[https://www.geeksforgeeks.org/integrating-risk-management-in-sdlc-set-1/](https://www.geeksforgeeks.org/integrating-risk-management-in-sdlc-set-1/)
...
这是第一个也是最重要的阶段。通常这个阶段在大型项目中作为一个独立的阶段进行，而不是作为需求定义阶段下的一个子阶段。这个阶段允许团队获得关于主要风险因素、给定项目的成本和时间的估计。你可能想知道为什么这如此重要？可行性研究帮助我们了解构建系统是否值得。它有助于识别主要的风险因素。
...
它从应用领域分析开始。这个阶段需要不同利益相关者的参与，以确保高效、正确和完整地收集系统服务、其性能和约束。然后审查并阐述此数据集，使其为下一阶段做好准备。
...
在此步骤中，通过采访用户或头脑风暴或其他方式收集的需求将：首先被分析，然后被分类和组织，例如功能性和非功能性需求组，然后对这些需求进行优先级排序，以便更好地了解哪些需求是高优先级并且必须存在于系统中。完成所有这些步骤后，需求将被协商。
...
这涉及验证迄今为止收集和分析的需求，以检查它们是否真正定义了用户希望从系统中获得什么。
...
此步骤涉及通过使用正式语言写下所有已达成一致的需求来创建需求文档（`RD`）。`RD` 充当不同利益相关者之间的沟通手段。
