...
3.  **标记 `X` 轴和 `Y` 轴:**
    应在图表的 `Y` 轴上按从高到低的降序列出项目，并在 `X` 轴上列出频率、时间或成本等数字。通过按发生频率或原因及其特征的降序排序来管理正在收集的数据。
4.  **画条:**
    保持每个条的宽度相同，应该为对应于其特定计数的各种类别画条。根据正在排序的数据绘制条形图。`x` 轴代表缺陷的主要原因或特征，而 `Y` 轴代表缺陷出现的频率或次数。也计算累计百分比。
5.  **Identification :**
