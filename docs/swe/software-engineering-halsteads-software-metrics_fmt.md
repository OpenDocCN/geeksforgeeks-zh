...
> 原文：[https://www.geeksforgeeks.org/software-engineering-halsteads-software-metrics/](https://www.geeksforgeeks.org/software-engineering-halsteads-software-metrics/)
...
*   **Halstead Program Length –** 运算符出现的总数和操作数出现的总次数。
    `N = N1 + N2`

    估计程序长度为，`N^ = n1 log2(n1) + N2 log2(N2)`
...
*   **霍尔斯特德词汇表–** 唯一运算符和唯一操作数出现的总数。
    `n = n1 + n2`
*   **Program Volume –** 与程序大小成正比，表示存储程序所需空间的大小（以比特为单位）。此参数取决于特定的算法实现。属性 `V`、`N` 和代码行数被证明是线性相关的，并且对于测量相对程序大小同样有效。
    `V = 大小 * (log2(词汇)) = N * log2(n)`

    体积的计量单位是尺寸“位”的通用单位。如果对词汇表使用统一的二进制编码，它就是程序的实际大小。并且误差 = 体积 / 3000

*   **Potential Minimum Volume –** 潜在最小体积 `V*` 被定义为可以对问题进行编码的最简洁程序的体积。
    `V* = (2 + n2*) * log2(2 + n2*)`

    这里，`n2*` 是唯一输入和输出参数的计数。

*   **Program Level –** 为了对编程语言进行排名，考虑了编程语言提供的抽象级别，即程序级别 (`L`)。语言的级别越高，使用该语言开发程序所需的努力就越少。
    `L = V* / V`

    `L` 的值在 0 和 1 之间，`L=1` 表示以最高可能的级别（即最小大小）编写的程序。
    估计节目级别为 `L^ = 2 * (n2) / (n1 * N2)`
...
*   **Language Level –** 显示算法实现程序语言级别。相同的算法如果用低级程序语言编写，则需要额外的努力。例如，在 Pascal 中编程比在汇编语言中更容易。
    `L' = V / D / D`
    `lambda = L * V* = L^2 * V`
*   **智能内容–** 确定程序中呈现（陈述）的智能量。此参数提供了程序复杂性的度量，与实现它的程序语言无关。
    `I = V / D`
*   **Programming Time –** 显示将现有算法转化为指定程序语言实现所需的时间（以分钟为单位）。
    `T = E / (f * S)`
...
13. 在数组中，`"数组名[索引]"` 中 `"数组名"` 和 `"索引"` 等变量被视为操作数，而 `[ ]` 被视为运算符。
