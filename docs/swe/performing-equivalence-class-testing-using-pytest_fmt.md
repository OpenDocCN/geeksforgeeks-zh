...
> 原文: [https://www.geeksforgeeks.org/performing-equivalence-class-testing-using-pytest/](https://www.geeksforgeeks.org/performing-equivalence-class-testing-using-pytest/)
...
现在，我们需要使用 `Pytest` 库为上述程序编写测试用例。每个测试用例都写在一个单独的函数中，我们使用 `pytest.raises` 函数来检查给定的输入是有效还是无效。对于上面的程序，我们创建了 7 个 *无效类* 和 1 个 *有效类*。7 个无效类为:
...
要执行上述测试用例，在一个文件夹中创建两个单独的文件 `triangletype.py` 和 `test_triangletype_eq.py`。要执行，写入以下命令:
