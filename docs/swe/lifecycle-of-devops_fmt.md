...
`DevOps` 生命周期是一组阶段，包括在 `DevOps` 中，参与开发和运营小组的职责，以更快地交付软件程序。`DevOps` 遵循积极的技术，包括编码、构建、测试、发布、部署、操作、展示和计划。

`DevOps` 生命周期遵循一系列阶段，如不停的开发、不停的集成、不停的测试、不停的监控和不停的反馈。`DevOps` 生命周期的每个部分都与一些设备和应用科学相关，以获得流程。一些经常使用的设备是开源的，并且主要基于商业企业的需求。`DevOps` 的生命周期易于操作，有助于令人满意的交付。
