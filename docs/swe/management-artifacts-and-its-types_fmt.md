...
> 原文：[https://www.geeksforgeeks.org/management-artifacts-and-its-types/](https://www.geeksforgeeks.org/management-artifacts-and-its-types/)
...
### 1. Business Case –
`Business Case` 通常为启动项目、任务、计划或组合提供理由。这个理由简单地基于对开发和实施的预估成本与风险和问题的对比，以及对将获得的业务收益和节省的评估。
...
`Software Development Plan` 旨在制定开发、修改和升级软件系统所需的全部计划。它是软件开发经理的现成解决方案。它为获取者提供了洞察力和工具，用于检查软件开发必须遵循的流程。

它只表明两件事：定期更新以及经理和从业者的理解和认可。

### 3. Work Breakdown Structure (WBS) –
[Work Breakdown Structure (WBS)](https://www.geeksforgeeks.org/software-engineering-work-breakdown-structure/) 是将项目分解为小型组件的、以交付物为导向的结构。创建和开发 `WBS` 是为了建立对项目范围的共同理解。
...
`Release Specification` 通常指在原材料、中间产品和最终产品使用或发布之前，对其进行精确测量的测试和限制。
