...
> 原文：[https://www.geeksforgeeks.org/software-engineering-white-box-testing/](https://www.geeksforgeeks.org/software-engineering-white-box-testing/)
...
在这种技术中，目标是遍历所有语句至少一次。因此，每一行代码都经过测试。在流程图的情况下，每个节点必须至少遍历一次。由于涵盖了所有代码行，有助于指出错误代码。
...
在这种技术中，测试用例被设计为来自所有决策点的每个分支至少被遍历一次。在流程图中，所有边必须至少遍历一次。
...
在这种技术中，至少测试一次条件所有可能结果的所有可能组合。让我们考虑以下示例：
1.  读取 `X`，`Y`
2.  `IF(X == 0 || Y == 0)`
3.  打印“0”
    *   #TC1: `X = 0`，`Y = 0`
...
因此，两个单独的条件需要四个测试用例。
同样，如果有 `n` 个条件，那么将需要 `2^n` 个测试用例。

### 基本路径测试

在这种技术中，从代码或流程图制作控制流图，然后计算圈复杂度，它定义了独立路径的数量，以便可以为每个独立路径设计最少数量的测试用例。
...
循环被广泛使用，这些是许多算法的基础，因此，它们的测试非常重要。错误经常发生在循环的开始和结束。
