...
> 原文：[https://www.geeksforgeeks.org/combinatorial-testing-tools-in-software-testing/](https://www.geeksforgeeks.org/combinatorial-testing-tools-in-software-testing/)
...
1.  [点击这里](https://matris.sba-research.org/tools/cagen/#/workspaces)打开 Kagan 工具。
2.  创建一个新的工作区并为其指定一个相关名称。
3.  现在点击创建的工作区，您将能够在屏幕上看到输入参数模型。
4.  在此模型中输入因素、它们的水平和基数（水平数量）。
5.  在输入参数模型中添加所有因素及其水平后，点击左侧面板上的“生成按钮”以生成测试配置。
...
1.  该工具附带一个可视化工具，显示添加到测试套件中的每个测试用例的覆盖范围。
2.  另一个惊人的功能是测试脚本的生成。如果您想对您的应用程序执行组合测试，您可以简单地使用这些测试脚本进行详细评估。可以使用工具中的许多选项根据我们的需要修改测试脚本。
3.  在组合测试（Pairwiser 测试）的情况下，pairwiser 是一个非常流行的工具，因为它们在市场上提供了更快的 pairwiser 算法。

**点击尝试：** [配对工具](https://inductive.no/pairwiser/)

### 4. testcover.com - 测试封面
