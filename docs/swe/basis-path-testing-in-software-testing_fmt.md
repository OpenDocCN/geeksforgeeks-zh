...
> 原文：[https://www.geeksforgeeks.org/basis-path-testing-in-software-testing/](https://www.geeksforgeeks.org/basis-path-testing-in-software-testing/)
...
1.  对于一个函数（例如 `Main()` 或 `阶乘()`），只构造一个流图。如果在一个程序中有多个函数，那么为每个函数构造一个单独的流程图。此外，在圈复杂度公式中，“`p`”的值是根据总共存在的图的数量来设置的。
2.  如果一个决策节点恰好有两个箭头离开它，则它被计为一个决策节点。然而，如果有超过 2 个箭头离开一个决策节点，则使用以下公式计算：
    ```
    d = k - 1
    ```
    这里，`k` 是离开决策节点的箭头数量。
