...
> 原文: [https://www.geeksforgeeks.org/boundary-value-analysis-triangle-problem/](https://www.geeksforgeeks.org/boundary-value-analysis-triangle-problem/)
...
变量的标称值可以是 `(l，r)` 范围内的任何值。在大多数 BVA 实现中，它被视为范围 `(r+l)/2` 的中间值。右图显示了一个双变量问题的分析边界值的标称值和极值。

在单一故障假设下，在 BVA，一个有 `n` 个输入的问题的测试用例总数是 `4n+1`。`4n` 个案例对应测试案例，每个变量的四个极值保持另 `n-1` 个变量在标称值。另一种情况是所有变量都保持在一个标称值。
