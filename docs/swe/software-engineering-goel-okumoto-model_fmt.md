...
> 原文：[https://www.geeksforgeeks.org/software-engineering-goel-okumoto-model/](https://www.geeksforgeeks.org/software-engineering-goel-okumoto-model/)

## 模型概述

`Goel-Okumoto` 模型（也称为指数 `NHP` 模型）基于以下假设：
...
```
\frac{y_nt_ne^{-bt_n}}{1-e^{-bt_n}} = \sum_{k=1}^n  \frac{(y_k-y_{k-1})(t_ke^{-bt_k}-t_{k-1}e^{-bt_{k-1}})}{(e^{-bt_{k-1}}-e^{-bt_k})}
```

### 第二类数据

类似地，对于第二类数据，使用最大似然估计方法对参数 `a` 和 `b` 的估计可以通过求解以下方程获得：
...
```
\frac{n}{b} = \sum_{k=1}^n S_i + \frac{nS_ne^{(-bS_n)}}{1-e^{-bS_n})}
```

## 可靠性函数

设 `\hat{a}` 和 `\hat{b}` 分别为参数 `a` 和 `b` 的 `MLE`。然后，我们可以获得平均值函数和可靠性函数的最大似然估计，如下所示：
...
其中 `z_a` 为标准正态分布的 `100(1+\alpha)/2` 百分位，即 `N(0，1)`。