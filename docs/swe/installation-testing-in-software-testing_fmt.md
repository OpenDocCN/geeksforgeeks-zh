...
> 原文：[https://www.geeksforgeeks.org/installation-testing-in-software-testing/](https://www.geeksforgeeks.org/installation-testing-in-software-testing/)
...
*   第一个最大的优势是在测试性能的基本层面验证应用程序和软件的设计。
*   这是 `STLC` 中非常关键的一部分，有助于维护标准。
*   检查软件版本是一种非常快速便捷的方法。
*   测试的较大输出将帮助开发人员改进应用程序或软件。

## 安装测试的缺点

*   有时由于一些外部因素和代码中的错误而失败，这是一个耗时且令人疲惫的过程。
*   测试用例的执行非常耗时，尤其是在安装测试期间。
*   或者结果完全取决于测试用例驱动。
