...
> 原文：[https://www.geeksforgeeks.org/module-coupling-slice-based-test-case-prioritization/](https://www.geeksforgeeks.org/module-coupling-slice-based-test-case-prioritization/)
...
1.  首先，我们创建一个**模块耦合矩阵** `C`，表示程序中所有模块之间的耦合。构造一个 `m` X `m` 模块耦合矩阵，其中 `m` 为程序中的模块数。使用模块耦合值填充矩阵，即 `Cij` 代表模块 `i` 和模块 `j` 之间的耦合。模块耦合矩阵本质上是对称的，即 `Cij = Cji`，所有对角元素即 `Cii` 对所有 `i` 等于 1。
...
![Call Graph](img/2b67ee42be8814b2eb39b061733c9a18.png)
![Coupling and Cohesion Values](img/144c46e2a7924f388ac6664984583cb8.png)
![Coupling and Cohesion Types](img/8ffa1c05aa297b15b6c20c95d8e520b8.png)
