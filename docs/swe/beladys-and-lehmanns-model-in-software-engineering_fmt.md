...
> 原文：[https://www.geeksforgeeks.org/beladys-and-lehmanns-model-in-software-engineering/](https://www.geeksforgeeks.org/beladys-and-lehmanns-model-in-software-engineering/)
...
- `M` = 系统扩展的总维护工作量。
- `P` = 代表所有生产性工作，例如分析、评估、设计、编码和测试。
- `C` = 由于缺乏结构化设计和文档而导致的复杂度。
- `D` = 维护团队对软件的熟悉程度。
- `K` = 通过比较该模型与实际项目工作量之间的关系而确定的常数；它被称为经验常数，因为其值取决于环境。
...
一个软件项目的开发工作量是 300 人-月。经验确定的常数(`K`)为 0.3。代码的复杂度等于 8(相当高的值)。在以下情况下，计算总工作量(`M`)
