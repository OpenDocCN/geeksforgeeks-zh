...
> 原文: [https://www.geeksforgeeks.org/advantages-disadvantages-of-cocomo-model/](https://www.geeksforgeeks.org/advantages-disadvantages-of-cocomo-model/)

## 概述:
`COCOMO`代表建设性成本模型，是基于`LOC`即代码行数的回归模型。`COCOMO`模型是Boehm在1981年提出的，它是一个软件项目的程序性成本估算模型，经常被用作可靠地预测与制定项目相关的各种参数(如规模、工作量、成本、时间和质量)的过程。`COCOMO`模型是世界上最常用的模型之一。
