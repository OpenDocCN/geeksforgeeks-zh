...
> 原文: [https://www.geeksforgeeks.org/software-engineering-function-oriented-design/](https://www.geeksforgeeks.org/software-engineering-function-oriented-design/)
...
数据字典只是存储`DFD`s中定义的所有数据项信息的存储库。在需求阶段，数据字典包含数据项。数据字典包括项目名称、别名（项目的其他名称）、描述/用途、相关数据项、值范围、数据结构定义/形式。

### 3. Structure Charts

`Structure Charts`是系统的层次化表示，它将系统划分为黑盒（功能对用户已知但内部细节未知）。组件从上到下、从左到右读取。当一个模块调用另一个模块时，它将被调用的模块视为黑盒，传递所需的参数并接收结果。
