...
可以考虑开发不同的容错工具。在驱动器控制器级别，廉价磁盘冗余阵列(`RAID`)是一种常见的容错策略，可以在开发时实施。有各种设施级容错形式，包括`冷`、`热`、`暖`和`复制站点`。容错计算在灾难恢复和中断期间发挥着巨大的作用。为此，容错策略还包括一些不间断电源(`UPS`)，如发电机，即使在电网发生故障后，它也能独立运行。拜占庭容错(`BFT`)是未来容错架构的另一个问题。`BFT`系统对于区块链、核电和航天工业是必要的，因为即使系统中的某些节点出现故障或受到恶意行为者的驱动，这些系统也能防止停机。
