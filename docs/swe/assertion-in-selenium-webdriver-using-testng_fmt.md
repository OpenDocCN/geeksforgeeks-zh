...
> 原文: [https://www.geeksforgeeks.org/assertion-in-selenium-webdriver-using-testng/](https://www.geeksforgeeks.org/assertion-in-selenium-webdriver-using-testng/)
...
这与 `assertEquals` 正好相反。每当预期值和实际值不同时，断言都会毫无例外地通过。但是，如果实际值和预期值相同，那么 assert 会异常失败，测试会被标记为失败。
