...
真实环境通过服务器交换他们的数据，其中用户终端服务由不同的服务器处理，管理服务由另一个服务器处理。开发软件时面临困难的开发人员。另一方面，当软件依赖于外部依赖时，测试人员不能进行有效的单元测试。为了提供有效的测试，需要模拟服务器。模拟服务器消除了对真实服务器的依赖，允许测试人员独立进行测试。图给出了模拟服务器的虚拟表示。
