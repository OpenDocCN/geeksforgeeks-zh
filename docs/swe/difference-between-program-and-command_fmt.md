...
**1。程序:**
程序，顾名思义，就是程序员用`编程语言`开发的一组指令，由编译后的代码组成，直接从计算机操作系统中运行。
...
| 程序由编写计算机程序的程序员来处理。 | 命令由操作系统的一部分处理，即`命令解释器`。 |
| `翻译器`将整个程序从一种语言转换成另一种语言，`解释器`在每一步按顺序执行程序，`调试器`执行程序，允许程序员检查程序的操作。 | 这些是通过在命令行中键入它们，然后按 enter 键发出的。 |
