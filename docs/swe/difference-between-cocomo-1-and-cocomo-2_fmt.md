...
| 可可米一世 | 科莫二世(神圣罗马帝国) |
| --- | --- |
| `COCOMO I` 在软件开发周期的 [`瀑布模型`](https://www.geeksforgeeks.org/software-engineering-cocomo-model/) 中很有用。 | `COCOMO II` 在软件的 [`非顺序、快速开发和重用模型`](https://www.geeksforgeeks.org/software-engineering-cocomo-ii-model/) 中非常有用。 |
| 它提供了工作量和进度的估计。 | 它提供的估计值代表最有可能的估计值的一个标准差。 |
| 该模型基于 [`线性重用公式`](https://www.geeksforgeeks.org/software-engineering-cocomo-ii-model/)。 | 该模型基于 [`非线性重用公式`](https://www.geeksforgeeks.org/software-engineering-cocomo-ii-model/)。 |
| 这个模型也是基于合理稳定需求的假设。 | 这个模型也是基于重用模型的，它着眼于理解和评估所需的努力。 |
| 努力方程的指数由三种发展模式决定。 | 努力方程的指数由 5 个标度因子决定。 |
| 开发从分配给软件的需求开始。 | 它遵循 [`螺旋式的发展`](https://www.geeksforgeeks.org/software-engineering-cocomo-ii-model/)。 |
