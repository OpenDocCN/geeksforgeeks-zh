...
if 构造用于根据给定条件的结果，控制两个或多个路径之一的执行流程。
...
当执行流可能沿两条或多条路径向下流动时，使用选择构造。当有许多条件且值是离散的时，使用选择构造。选择结构中有两个或多个条件。每个条件语句都是一个入口点，除非使用 `break` 语句，否则执行将从该点继续。`break` 语句使程序从选择构造的末尾继续。这些条件只能使用离散值。
...
当需要连续执行一个代码块直到满足一个条件时，使用重复构造。这种类型的循环用于执行代码块，条件应该为真。
