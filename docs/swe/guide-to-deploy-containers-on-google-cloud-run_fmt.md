...
谷歌云运行是安全 HTTPS 连接上 API 部署的快速和可扩展解决方案之一，可以被认为是每个开发人员的首选。Firebase 是静态网站，对吗？如果我们也可以在 Firebase 上部署动态网站呢？你只需要**云运行**。在本教程中，我们将向您展示如何在云上部署一个简单的网站。我们将使用 [Flask](https://palletsprojects.com/p/flask/) 作为网站。首先用谷歌云 SDK 设置你的系统。如果你不知道怎么做，我建议你阅读他们的官方文档 [这里](https://cloud.google.com/sdk)。
...
*   首先创建一个项目文件夹。就我而言，我将使用名称 `cloud-run-tutorial`。
...
*   然后键入 `gcloud run deploy --image gcr.io/${project_id}/helloworld:v1 --platform managed`
    *   系统会提示您输入服务名称：按 `Enter` 接受默认名称 `helloworld`。
    *   系统将提示您输入地区：选择您选择的地区。
...
恭喜你！您刚刚将打包在容器镜像中的应用程序部署到云运行。Cloud Run 会自动水平扩展您的容器镜像以处理接收到的请求，然后在需求减少时进行缩减。