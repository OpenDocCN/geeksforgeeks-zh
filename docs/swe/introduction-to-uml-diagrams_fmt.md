...
该视图捕获了系统的各个重要组件及其相互依赖关系。例如实现视图可能会将`图形用户界面`部分、`中间件`和`数据库`部分显示为不同的部分，并且还会捕获它们的相互依赖关系。

### 5. 环境视图

该视图描述了众多添加剂在硬件特定部分的实现。对于一个平滑的系统，利用`案例模型`、`类图`和`交互图`中的一个也是足够的。对于对象经历许多状态变化的系统，状态图也是必要的。对于在大量硬件组件上实现的系统，部署图也是必要的。因此，要构建的模型种类取决于手头的事情。