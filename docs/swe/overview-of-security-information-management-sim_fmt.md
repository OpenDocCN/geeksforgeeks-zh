...
> 原文: [https://www.geeksforgeeks.org/overview-of-security-information-management-sim/](https://www.geeksforgeeks.org/overview-of-security-information-management-sim/)
...
*   然后，它们将从许多资源收集的事件数据转换为通用且简化的格式。通常，数据被转换为 `XML` 文件。
*   `SIM` 系统收集并协调来自各种资源的数据，以帮助管理员识别系统上的真实威胁和 `false positive`。`false positive` 指的是一个看起来像是重大威胁，但实际上并非威胁的事件。
*   一旦发生可疑活动，`SIM` 工具将通过向组织的管理员发送警报并生成图形化表示（如报告和图表）来响应这些事件。

## SIM 系统生成的报告通常用于

1.  检测未经授权的访问、文件修改和数据泄露。
2.  识别业务组织可能用于促进其发展的数据趋势。
3.  它们也用于识别网络行为和评估性能。
