...
> 原文: [https://www.geeksforgeeks.org/points-to-remember-about-arduino-uno-or-mega-in-iot/](https://www.geeksforgeeks.org/points-to-remember-about-arduino-uno-or-mega-in-iot/)
...
1.  它有 `ATMega328` 微控制器。
2.  它的尺寸比 `Mega` 小，因为 `Uno` 的尺寸是 `2.7` 英寸 * `2.1` 英寸。
...
`Arduino Mega 2560` 是 `Arduino` 的一款微控制器板，基于 `ATMega2560`。它也是需要更多输入/输出线、更多草图内存和更多内存的项目的流行板。因为它提供了更多的输入/输出引脚，比 `Uno` 更多的内存，所以在更高的要求，`Arduino Mega` 是首选。它提供了像 `Uno` 这样的所有功能。
...
10. `Mega` 有 `54` 个数字输入/输出引脚。
11. `Mega` 有 `15` 个带脉宽调制引脚的数字输入/输出。
12. `Uno` 中有 `16` 个模拟引脚。
