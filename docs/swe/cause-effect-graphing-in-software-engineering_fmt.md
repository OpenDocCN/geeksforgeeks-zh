...
> 原文: [https://www.geeksforgeeks.org/cause-effect-graphing-in-software-engineering/](https://www.geeksforgeeks.org/cause-effect-graphing-in-software-engineering/)
...
2.  **因果识别:**
    这包括识别规范中的原因(不同的输入条件)和结果(输出条件)。
3.  **将规格转换为因果图:**
    使用布尔表达式将因果链接在一起，以获得因果图。如果可能的话，还会在原因和结果之间添加约束。
4.  **转换成决策表:**
    然后将因果图转换成限定条目决策表。如果您不知道决策表的概念，请查看[这个](https://www.geeksforgeeks.org/software-engineering-decision-table/)链接。
