...
越早发现 bug 越好。它允许测试和开发团队之间更好的沟通。它允许两个团队逐个解决每个 bug。

对比一下极右的方法。在这里，所有的错误都被一次解决。许多人可能会被遗忘或忽视（由于时间或预算的限制）。

### 2. 多种测试类型

当代码在每个设计阶段进行测试时，可以应用多种测试类型。
...
采用左移方法，测试在软件开发的每个关键阶段进行。因此，测试团队最终（必然）参与到项目的规划中。他们也了解其业务目标。
...
左移方法利用频繁的“自动化”。它让他们执行连续测试以减少时间。但是，这仍然留下了设计这些测试的问题。而这通常留给测试团队来解决。
