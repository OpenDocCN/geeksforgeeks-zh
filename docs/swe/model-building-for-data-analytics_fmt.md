...
> 原文: [https://www.geeksforgeeks.org/model-building-for-data-analytics/](https://www.geeksforgeeks.org/model-building-for-data-analytics/)
...
### Python :
是为机器学习和分析提供工具包的编程语言，如 `scikit-learn`、`NumPy`、`scipy`、`Pandas`，以及使用 `matplotlib` 的相关数据可视化。

### SQL :
数据库实现中的 `SQL`，比如 `MADlib`，提供了一个替代内存桌面的分析工具。

### MADlib :
它为 `PostgreSQL` 或 `Greenplum` 提供了一个可以在数据库中执行的开源机器学习算法库。
