...
原文: [https://www.geeksforgeeks.org/difference-between-devops-and-itil/](https://www.geeksforgeeks.org/difference-between-devops-and-itil/)
...
关于`DevOps`和`ITIL`有很多不同的观点。在信息技术领域，你必须在两者之间做出选择。`DevOps`和`ITIL`有他们自己的好处，但是他们把大部分的好处混在了一起。凭借精益和敏捷，`DevOps`的工作非常顺利。`DevOps`创建自己的反馈，以便在处理新项目或组件时可以纠正缺陷。
...
| 05. | `DevOps`专注于概念。它拥有动态的知识体系。 | `ITIL`专注于发展。它有一个静态的知识体系。 |
```