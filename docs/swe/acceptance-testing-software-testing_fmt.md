...
> 原文: [https://www.geeksforgeeks.org/acceptance-testing-software-testing/](https://www.geeksforgeeks.org/acceptance-testing-software-testing/)
...
1.  **用户验收测试 (UAT):**
    用户验收测试用于确定产品是否为用户正常工作。客户经常使用的特定需求主要是为了测试目的而挑选的。这也被称为最终用户测试。
2.  **业务验收测试 (BAT):**
    BAT 用于确定产品是否符合业务目标和目的。最佳可得技术主要侧重于业务利润，由于市场条件和新技术的不断变化，这些利润极具挑战性，因此当前的实施可能不得不改变，从而导致额外的预算。
3.  **合同验收测试 (CAT):**
    CAT 是一份合同，规定产品上线后，必须在预定期间内执行验收测试，并且应通过所有验收用例。
    这里有一个称为服务级别协议 (SLA) 的合同，其中包含条款，即只有产品服务符合所有要求时才会付款，这意味着合同已履行。
    有时，这个合同发生在产品上线之前。在测试周期、测试领域、后期遇到的问题的条件、付款等方面，应该有一个明确的合同。
4.  **法规验收测试 (RAT):**
    RAT 用于确定产品是否违反了其发布所在国家/地区的政府定义的规则和法规。这可能是无意的，但会对业务产生负面影响。
...
5.  **运行验收测试 (OAT):**
    ...
    OAT 保证产品在投放生产前的稳定性。
6.  **Alpha 测试:**
    Alpha 测试是由一个专门的测试人员团队（通常称为 alpha testers）在开发测试环境中用来确定产品。
7.  **Beta 测试:**
