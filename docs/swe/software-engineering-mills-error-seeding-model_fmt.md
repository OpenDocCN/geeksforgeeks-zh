...
> 原文: [https://www.geeksforgeeks.org/software-engineering-mills-error-seeding-model/](https://www.geeksforgeeks.org/software-engineering-mills-error-seeding-model/)
...
是基于两个独立的程序员组，使用独立的测试用例集测试程序的错误。假设在总共 `N` 个初始错误中，第一个程序员检测到 `n1` 个错误(并且根本没有删除它们)，第二个程序员独立地从同一个程序中检测到 `r` 个错误。
