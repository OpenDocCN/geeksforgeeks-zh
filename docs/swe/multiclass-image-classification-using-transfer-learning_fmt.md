...
图像分类是有监督的机器学习问题之一，旨在将数据集的图像分类到它们各自的类别或标签中。各种狗品种的图像分类是一个经典的图像分类问题，在本文中，我们将通过使用预训练的模型`InceptionResNetV2`并对其进行定制来做同样的事情。
...
*   使用的数据集总共包括 120 种狗。
*   每张图片都有一个文件名，这是它的唯一标识符。
*   训练数据集 (`train.zip`)：包含 10,222 张用于训练我们模型的图像。
*   测试数据集 (`test.zip`)：有 10,357 张图像，我们必须将它们分类到各自的类别或标签中。
*   标签文件 (`labels.csv`)：包含图片 ID 对应的品种名称。
*   提交示例文件 (`sample_submission.csv`)：包含要提交的正确格式示例。

上述所有文件都可以从[这里](https://drive.google.com/drive/folders/1z31bsh7gNrUiwameOEWhqtWNZuKEdKQ7?usp=sharing)下载。
