...
> 原文: [https://www.geeksforgeeks.org/design-video-sharing-system-like-youtube/](https://www.geeksforgeeks.org/design-video-sharing-system-like-youtube/)
...
此外，缓存是返回数据的重要系统。如果您正在构建大型可扩展系统，您必须考虑不同的缓存策略。您可以将 `CDN` 用作视频内容缓存，将 `Redis`/`Memcache` 用作元数据缓存。缓存最大的问题是伸缩(全局缓存机制)和缓存回收策略。如果您在 AWS 上构建您的服务，您可以使用 `Cloudfront` 作为 `CDN`(内容缓存)，也可以使用 AWS 上的 `Elasticache` 服务作为元数据缓存。请注意，`Cloudfront` 是高度可扩展的 AWS 服务，您不需要使用维护/可伸缩性。AWS 将自行维护和扩展 `Cloudfront` 服务。

## API 设计

在当今世界，许多系统支持移动平台，因此 `API` 是能够区分开发人员并支持移动支持的最佳选择。我们可以用 `REST` 或者 `SOAP`。很多大公司根据自己的系统，更喜欢 `REST` 或者 `SOAP`。我们将在下面提到三个主要的应用编程接口:

*1- 上传视频(`apiKey`, `title`, `description`, `categoryID`, `language`)*
...
– `LikeCount` (`int`)
– `DislikeCount` (`int`)
– `ViewCount` (`int`) – 我们可以使用大整数来保持显示的数字
– `UploadDate` (`datetime`)
...
```java
// Java code for Youtube architecture
// ... (具体代码内容)
```

**参考: [http://highscalability.com/youtube-architecture](http://highscalability.com/youtube-architecture)**
