...
> 原文: [https://www.geeksforgeeks.org/design-an-online-book-reader-system/](https://www.geeksforgeeks.org/design-an-online-book-reader-system/)
...
`OnlineReaderSystem` 类代表了我们程序的主体。我们可以实现类，这样它就可以存储所有涉及用户管理的书籍的信息，并刷新显示，但这将使这个类相当庞大。相反，我们选择将这些组件分离成 `Library`、`UserManager` 和 `Display` 类。
...
此类代表显示，负责显示书籍、其页面和内容。它也显示当前用户。它提供了`turnPageForward`、`turnPageBackward`、`refreshPage`等方法。
...
此类代表书籍，是一个简单的POJO。`User`和`Book`类仅保存数据并提供很少的功能。
