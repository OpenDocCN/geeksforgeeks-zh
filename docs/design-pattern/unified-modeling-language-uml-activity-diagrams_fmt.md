...
> 原文: [https://www.geeksforgeeks.org/unified-modeling-language-uml-activity-diagrams/](https://www.geeksforgeeks.org/unified-modeling-language-uml-activity-diagrams/)
...
2.  **动作或活动状态 –** 一个活动表示在对象上或由对象执行的动作。我们使用带圆角的矩形表示一个活动。基本上，发生的任何动作或事件都使用活动来表示。
...
3.  **动作流或控制流 –** 动作流或控制流也被称为路径和边。它们用于显示从一个活动状态到另一个活动状态的转换。
...
4.  **决策节点和分支 –** 当我们需要在决定控制流之前做出决策时，我们使用决策节点。
...
5.  **守卫 –** 守卫指的是有时写在决策节点旁边箭头上的语句，通常在方括号内。

![UML-Activity-Diagram](img/5dd724b81b4556c7933ef165f5f1a757.png)
**图–** 决策节点旁边使用的守卫

该语句必须为真，控件才能沿特定方向移动。守卫帮助我们了解决定流程的约束和条件。
...
当我们使用一个分叉节点时，两个活动同时执行，也就是说，在将活动分成两部分之前不做任何决定。这两部分都需要在分叉语句的情况下执行。
我们使用一个圆形的实心矩形条来表示分叉符号，从父活动状态传入的箭头和指向新创建的活动的传出箭头。
例如:在下面的例子中，煮咖啡的活动可以分成两个并发的活动，因此我们使用了分叉符号。

![UML-Activity-Diagram](img/f0a189bd0aa39c0a19f015a2fe0e475f.png)
**图–** 一个使用分叉的图表

7.  **汇合 –** 汇合节点用于支持并发活动汇合为一个。对于汇合符号，我们有两个或多个传入边和一个传出边。
...
8.  **合并或合并事件 –** 当不是并发执行的活动需要合并时，就会出现这种情况。我们使用合并符号来处理此类场景。如果控制流继续到下一个活动，而不管选择了哪条路径，我们可以将两个或多个活动合并为一个。
...
9.  **泳道 –** 我们使用泳道将相关活动分组到一列中。泳道将相关活动分组到一列或一行中。泳道可以是垂直的和水平的。泳道用于为活动图增加模块化。使用泳道不是强制性的。它们通常能为活动图提供更清晰的视图。这类似于在程序中创建一个函数。这样做不是强制性的，但这是一个推荐的做法。
...
11. **最终状态或结束状态 –** 当特定过程或活动结束时系统达到的状态称为最终状态或结束状态。我们使用一个填充圆圈内的圆圈符号来表示状态机图中的最终状态。一个系统或一个过程可以有多个最终状态。
