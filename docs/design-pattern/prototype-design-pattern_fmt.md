...
原型模式是一种创造性的设计模式。当创建对象既费时又费钱时，就需要原型模式，所以我们用现有的对象本身来创建对象。从现有对象创建对象的最佳方法之一是 `clone()` 方法。克隆是实现原型模式的最简单方法。但是，根据您的业务模型决定如何复制现有对象是您的责任。
...
当一个系统应该独立于它的产品是如何被创建、组成和表示的时候，以及当要实例化的类在运行时被指定的时候。
...
*   它还对客户端隐藏了具体的产品类。
*   `Prototype` 的每个子类都必须实现 `clone()` 操作，当考虑中的类已经存在时，这可能很困难。当 `clone()` 的内部包含不支持复制或具有循环引用的对象时，实现 `clone()` 也会很困难。

**进一步阅读：**[Python 中的原型法](https://www.geeksforgeeks.org/prototype-method-python-design-patterns/)

本文由 **Saket Kumar** 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用 [write.geeksforgeeks.org](https://write.geeksforgeeks.org) 写一篇文章或者把你的文章邮寄到 review-team@geeksforgeeks.org。看到你的文章出现在极客博客主页上，帮助其他极客。
