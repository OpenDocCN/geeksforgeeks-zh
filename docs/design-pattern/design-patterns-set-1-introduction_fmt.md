...
> 原文:[https://www.geeksforgeeks.org/design-patterns-set-1-introduction/](https://www.geeksforgeeks.org/design-patterns-set-1-introduction/)
...
这些设计模式都是关于类实例化或对象创建的。这些模式可以进一步分为类创建模式和对象创建模式。类创建模式在实例化过程中有效地使用继承，而对象创建模式则有效地使用委托来完成工作。

创造性设计模式是`工厂方法`、`抽象工厂`、`构建器`、`单例`、`对象池`和`原型`。
...
这些设计模式是关于组织不同的类和对象以形成更大的结构并提供新功能。

结构设计模式有`适配器`、`桥`、`复合`、`装饰`、`外观`、`Flyweight`、`私有类数据`和`代理`。
