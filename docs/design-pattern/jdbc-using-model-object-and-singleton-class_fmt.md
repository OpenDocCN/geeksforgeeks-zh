...
3.  **数据库连接:** 在 `com.jdbc.util` 包中创建单例连接类 `DatabaseConnection`。在变量 `user` 上使用您的 MySQL 用户名和密码，然后通过。仔细看“url”变量的最后一部分。必须保持名称与数据库名称相同。我的数据库的名称是“org”，这就是为什么我把“org”放在 URL 变量中。
