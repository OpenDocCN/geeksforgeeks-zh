...
*   **抽象表达式** (`Expression`): 声明 AST 中所有节点(终端和非终端)覆盖的 `interpreter()` 操作。
*   **终端表达式** (`TerminalExpression`): 实现终端表达式的 `interpreter()` 操作。
*   **非终结表达式** (`OrExpression`、`AndExpression`): 对所有非终结表达式执行 `interpreter()` 操作。
*   **上下文** (`String`): 包含解释器全局的信息。必须解释和解析的是这个带有后缀符号的字符串表达式。
*   **客户端** (`InterpreterPattern`): 构建(或提供)由终端表达式和非终端表达式组装的 AST。客户端调用 `interpreter()` 操作。
...
在上面的代码中，我们正在创建一个接口 `Expression` 和具体的类来实现表达式接口。定义了一个类 `TerminalExpression` 作为主解释器，其他类 `OrExpression`、`AndExpression` 用于创建组合表达式。
