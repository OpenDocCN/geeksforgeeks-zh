...
`int idx = timestamp % 300;` => `hits[idx]` 保持在这一秒钟内发生的点击次数

但是在我们将 `idx` 的命中次数增加 1 之前，时间戳实际上属于 `[idx]` 正在跟踪的第二次命中。
`timestamp[i]` 存储上次计数命中的时间戳。
如果 `timestamp[i]` > 为 `timestamp`，则应该丢弃该命中，因为它不是在过去 5 分钟内发生的。
如果 `timestamp[i]` == `timestamp`，那么 `hits[i]` 增加 1。
如果 `timestamp[i]` < `timestamp` - 300，则该命中已过期，应被忽略。
