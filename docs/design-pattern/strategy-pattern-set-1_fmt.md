...
一如既往，我们将通过定义一个问题并使用策略模式来解决它来学习这个模式。假设我们正在构建一个游戏“街头霸王”。为简单起见，假设一个角色可能有四个动作，即踢、打、滚和跳。每个角色都有拳打脚踢的动作，但是翻滚和跳跃是可选的。你会如何模拟你的类？假设最初你使用继承并抽象出一个`战斗机`类中的共同特征，让其他角色子类化`战斗机`类。
...
看看下面的设计:
![fighter2](img/a115b94eda4909a3eed3a79eaaeb1feb.png)
...
*这里策略模式来搭救我们。我们将学习什么是策略模式，然后应用它来解决我们的问题。*

## 定义

维基百科将策略模式定义为:

*“在计算机编程中，**策略模式**(也称为**策略模式**)是一种软件设计模式，能够在运行时选择算法的行为。策略模式*
...
![Untitled](img/37e390dbf1881f4034d8501957c5f8b4.png)
...
*   [Head First设计模式](http://www.amazon.com/Head-First-Design-Patterns/dp/0596007124)
