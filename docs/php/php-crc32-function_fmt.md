...
`crc32()`函数帮助我们计算字符串的32位[CRC](https://en.wikipedia.org/wiki/Cyclic_redundancy_check)或[循环冗余校验和多项式](https://en.wikipedia.org/wiki/Cyclic_redundancy_check)。此函数使用[CRC32算法](https://en.wikipedia.org/wiki/Cyclic_redundancy_check#CRC-32_algorithm)，可用于验证数据完整性。
...
`crc32()`函数以整数形式返回给定字符串的`crc32`校验和。
...
此程序帮助我们计算字符串“Hello World”的32位CRC，包括带`%u`和不带`%u`。
...
此程序帮助我们计算字符串“GeeksforGeeks.”的32位CRC，包括带`%u`和不带`%u`。
...
此程序帮助我们计算字符串“Computer Science.”的32位CRC，包括带`%u`和不带`%u`。
