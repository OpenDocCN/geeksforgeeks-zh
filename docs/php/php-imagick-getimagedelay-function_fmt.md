...
`Imagick::getImageDelay()` 函数是 PHP 中的一个内置函数，用于获取图像延迟。延迟实际上是在 gif 动画中从一个图像过渡到另一个图像所花费的时间。
...
此函数返回一个整数值，其中包含以厘米为单位的图像延迟（100 centi = 1 秒）。
...
[https://www.php.net/manual/en/imagick.getimagedelay.php](https://www.php.net/manual/en/imagick.getimagedelay.php)