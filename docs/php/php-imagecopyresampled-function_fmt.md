...
`imagecopyresampled()` 函数是 PHP 中的一个内置函数，用于将一个图像的矩形部分复制到另一个图像，平滑地内插像素值，以便特别是减小图像大小时仍能保持很大的清晰度。
...
下面给出的程序说明了 PHP 中的 `imagecopyresampled()` 函数：
...
[https://www.php.net/manual/en/function.imagecopyresampled.php](https://www.php.net/manual/en/function.imagecopyresampled.php)