...
> 原文:[https://www.geeksforgeeks.org/how-to-prevent-sql-injection-in-php/](https://www.geeksforgeeks.org/how-to-prevent-sql-injection-in-php/)
...
现在为了避免这种类型的 SQL 注入，我们需要使用 `mysqli_real_escape_string()` 函数来净化密码输入和用户名输入。

`mysqli_real_escape_string()` 函数将特殊字符作为用户的输入，而不将其视为查询用法。
