...
## 简介
`PHPSpreadsheet` 是一个用 PHP 编写的库，它借助一组给定的类来帮助读取和写入不同类型的电子表格文件格式。支持电子表格各种格式有 Excel(.xlsx)、Open Document Format(.ods)、SpreadsheetML(.xml)、CSV 等。
...
## 安装
`PHPSpreadsheet` 可以在 [Composer](https://getcomposer.org) 的帮助下安装。

在终端上运行以下命令以安装 `PHPSpreadsheet`：
```php
...
## 引用
[https://phpspreadsheet.readthedocs.io/en/develop/](https://phpspreadsheet.readthedocs.io/en/develop/)