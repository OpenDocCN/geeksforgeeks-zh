...
此函数接受四个参数，如上面的语法所示。 前两个参数是必需的，必须提供，而其余两个参数是可选的。 所有这些参数如下所述：

*   `$string`：此必选参数指定要搜索的字符串
*   `$charlist`：此强制参数指定要在给定 `$string` 中搜索的字符列表。
...
此程序显示带有 `$start` 参数的 `strcspn()` 函数的用法。
...
此程序说明了带负 `$length` 参数的 `strcspn()` 函数的用法。
...
此程序显示带有负 `$start` 参数的 `strcspn()` 函数的用法。
