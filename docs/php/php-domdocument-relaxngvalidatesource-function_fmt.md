...
`DOMDocument::relaxNGValidateSource()` 函数是 PHP 中的一个内置函数，用于使用字符串作为 RNG 模式对文档执行 RELAX NG 验证。`relaxNGValidate()` 和 `relaxNGValidateSource()` 之间的区别在于前者接受 RNG 模式文件名，而后者也可以接受字符串形式的 RNG 模式。
...
此函数接受保存 RNG 模式的单个参数 `$source`。

## 返回值

此函数成功时返回 `TRUE`，失败时返回 `FALSE`。

下面给出的程序演示了 PHP 中的 `DOMDocument::relaxNGValidateSource()` 函数：
