...
`Imagick::roundCorners()`函数是 PHP 中的一个内置函数，用于对图像角点进行圆角处理。
...
*   `$y_rounding`：它指定 y 舍入量。
*   `$stroke_width`（可选）：它指定笔划宽度。该参数的默认值为 10。
*   `$displace`（可选）：它指定图像位移。该参数的默认值为 5。
*   `$size_correction`（可选）：它指定大小校正。该参数的默认值为 -6。
...
下面给出的程序演示了 PHP 中的`Imagick::roundCorners()`函数：
...
[https://www.php.net/manual/en/imagick.roundcorners.php](https://www.php.net/manual/en/imagick.roundcorners.php)