...
函数的作用是：`imageistruecolor()` 函数是 PHP 中的一个内置函数，用于判断图像是否为真彩色图像。真彩色图像是其中每个像素由三个值（即 RGB）指定的图像。
...
此函数接受保存图像的单个参数 `$image`。

## 返回值

如果图像为真彩色，则此函数返回 `TRUE`，否则返回 `FALSE`。

下面给出的程序演示了 PHP 中的 `imageistruecolor()` 函数：
...
[https://www.php.net/manual/en/function.imageistruecolor.php](https://www.php.net/manual/en/function.imageistruecolor.php)