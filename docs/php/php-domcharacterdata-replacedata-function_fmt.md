# DOMCharacterData::replaceData() 函数
...
`DOMCharacterData::replaceData()` 函数是 PHP 中的内置函数，用于替换 `DOMCharacterData` 节点中的子字符串。
...
如果 `$offset` 为负或大于数据中的 16 位单元数，或者如果 `$count` 为负，则会引发 `DOM_INDEX_SIZE_ERR`。
...
[https://www.php.net/manual/en/domcharacterdata.replacedata.php](https://www.php.net/manual/en/domcharacterdata.replacedata.php)