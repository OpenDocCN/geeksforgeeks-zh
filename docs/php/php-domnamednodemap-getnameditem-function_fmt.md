...
`DOMNamedNodeMap::getNamedItem()` 函数是 PHP 中的内置函数，用于检索由名称指定的节点。这用于获取属性项，并进一步获取有关属性的信息。
...
此函数接受单个参数 `$name`，该参数保存要检索的节点的 `nodeName`。

## 返回值

成功时，此函数返回指定名称的 `DOMNode`。
...
在此示例中，我们将通过更改属性的值来检查函数是否获取最新属性值。
