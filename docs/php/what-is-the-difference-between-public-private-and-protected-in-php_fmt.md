...
> 原文：[https://www.geeksforgeeks.org/what-is-the-difference-between-public-private-and-protected-in-php/](https://www.geeksforgeeks.org/what-is-the-difference-between-public-private-and-protected-in-php/)

公共、私有和受保护的称为访问修饰符。就像 C++一样，PHP 也有 `public`、`private` 和 `protected` 三种访问修饰符。属性、方法或常数的可见性可以通过在声明前加上这些关键字来定义。

*   如果类成员声明为 `public` 的，那么它可以在任何地方被访问。
*   如果类成员声明为 `protected` 的，那么它只能在类本身内通过继承子类来访问。
*   如果类成员声明为 `private`，那么它只能由定义该成员的类访问。
