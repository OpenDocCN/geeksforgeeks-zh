...
> 原文：[https://www.geeksforgeeks.org/how-to-get-the-number-of-lines-in-a-file-using-php/](https://www.geeksforgeeks.org/how-to-get-the-number-of-lines-in-a-file-using-php/)

给定一个文件引用，用 PHP 找到这个文件的行数。共有 3 种方法可以解决这个问题。

`test.txt` 文件用于测试以下所有 PHP 代码：
...
将整个文件加载到内存中，然后使用 `count()` 函数返回该文件的行数。
...
这里我们一次只加载一行文件，比第一种方法好。PHP `fopen()` 函数用来打开一个文件或者 URL。PHP `fgets()` 函数用于从打开的文件中返回一行。`fclose()` 函数用于关闭文件指针。
...
我们只将特定大小的数据加载到内存中。我们不关心装载时的断线。我们将迭代加载的数据，计算其中的换行符数量。
