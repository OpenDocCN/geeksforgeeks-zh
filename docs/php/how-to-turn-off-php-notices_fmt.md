...
## 方法 1
关闭通知是最简单方便的方法。可以通过设置`php.ini`文件来禁用通知。在当前文件中，搜索代码行`错误报告`。会出现一行`默认值:E_ALL`，如下图:
...
## 方法 2
要关闭通知，可以在 PHP 文件的代码开头添加一行代码。例如`gfg.php`下面的代码看起来像这样
...
## 方法 3
可以给任意操作符添加“@”使 PHP notifications 在执行当前操作时静音:
`@ $mid = $_POST['mid'];`