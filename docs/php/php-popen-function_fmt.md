...
函数的作用是：打开通向用户使用命令参数指定的程序的管道。它返回的文件指针与`fopen()`返回的文件指针相同，但本质上是单向的，即它只能用于读取或写入。`popen()`指针可以与`fgets()`、`fgetss()`和`fwrite()`一起使用。由`popen()`函数启动的文件指针必须用`pclose()`关闭。
...
1.  由`popen()`函数启动的文件指针必须用`pclose()`关闭。
...
## 相关文章：
[PHP|pclose()函数](https://www.geeksforgeeks.org/php-pclose-function/)
