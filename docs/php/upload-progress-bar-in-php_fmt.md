...
`XAMPP` 是一个由 Apache 开发的免费开源的跨平台 web 服务器解决方案堆栈包，允许 web 应用程序在本地 web 服务器上轻松测试。但是当使用 PHP 将文件上传到服务器时，这是一个令人头疼的问题。在大多数情况下，PHP 服务器不允许上传大于 30 兆字节(30MB)的文件。
...
如果你的托管提供商不允许你访问 `php.ini` 文件怎么办？在这种情况下，建议使用 JavaScript 将你的大文件以小 blobs 的形式上传到服务器。JavaScript 会将您的本地文件分成多个文件，并尝试将这些文件上传到服务器。这里你必须记住，你的 blob 大小不能超过你的 PHP 主机的上传大小限制。
...
*   打开浏览器标签页，在地址栏中输入 `localhost/geeks/index.php` 或 `localhost/geeks`。它将显示输出。
