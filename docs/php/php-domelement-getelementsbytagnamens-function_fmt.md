# DOMElement::getElementsByTagNameNS() 函数

> 原文：[https://www.geeksforgeeks.org/php-domelement-getelementsbytagnamens-function/](https://www.geeksforgeeks.org/php-domelement-getelementsbytagnamens-function/)

`DOMElement::getElementsByTagNameNS()` 函数是 PHP 中的一个内置函数，用于获取具有给定 `localName` 和 `namespaceURI` 的所有后代元素。
...
此函数返回一个 `DOMNodeList` 值，其中包含所有匹配的元素，顺序与它们在此元素树的预序遍历中遇到的顺序相同。
