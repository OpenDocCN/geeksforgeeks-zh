...
[PHP|EXPLODE()](https://www.geeksforgeeks.org/php-explode-function/) 函数是 PHP 中的内置函数，用于将字符串拆分成不同的字符串。 函数的作用是：根据字符串分隔符拆分字符串，即在出现分隔符的地方拆分字符串。 此函数返回一个数组，其中包含通过拆分原始字符串形成的字符串。
...
[PHP|preg_plit()](https://www.geeksforgeeks.org/php-preg-split-function/) 函数是 PHP 中的一个内置函数，用于通过正则表达式将给定的字符串转换为数组。 如果匹配失败，将返回一个包含输入字符串的单个元素的数组。
