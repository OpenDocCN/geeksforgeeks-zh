...
`ImagickDraw::composite()` 函数是 PHP 中的一个内置函数，用于使用指定的合成操作符、指定的位置和指定的大小将图像合成到当前图像中。
...
*   `$compose`：它指定对应于[复合常量](https://www.php.net/manual/en/imagick.constants.php/#imagick.constants.composite-default)的复合运算符。
*   `$x`：它指定左上角的 y 坐标。
*   `$y`：它指定左上角的 x 坐标。
*   `$width`：它指定合成图像的宽度。
*   `$Height`：它指定合成图像的高度。
...
下面的程序演示了 PHP 中的 `ImagickDraw::composite()` 函数：
...
[https://www.php.net/manual/en/imagickdraw.composite.php](https://www.php.net/manual/en/imagickdraw.composite.php)