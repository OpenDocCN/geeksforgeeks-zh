...
*   `$node`: 此参数保存 DOM 元素节点。
*   `$class_name`: 保存类名的可选参数。如果使用此参数，`simplexml_import_dom()` 函数将返回指定类的对象。此类应扩展 `SimpleXMLElement` 类。

## 返回值

此函数成功时返回 `SimpleXMLElement`，失败时返回 `False`。
...
[https://www.php.net/manual/en/function.simplexml-import-dom.php](https://www.php.net/manual/en/function.simplexml-import-dom.php)