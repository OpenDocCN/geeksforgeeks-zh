...
此函数接受 GMP 编号 `$num` 作为强制参数，如上面的语法所示，我们要查找其下一个素数。在 PHP 5.6 版和更高版本中，该参数可以是 GMP 对象，也可以传递数字字符串，前提是可以将该字符串转换为数字。

## 返回值

此函数返回一个 GMP 号，它是刚刚大于作为参数传递给它的 GMP 号的质数。
...
当将数字字符串作为 GMP 编号作为参数传递时，计算刚好大于 GMP 编号的质数的程序。
...
当 GMP 编号作为参数传递时，计算大于 GMP 编号的质数的程序。
