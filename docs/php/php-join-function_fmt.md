...
`Join()`函数接受两个参数，其中一个是可选的，另一个是必需的。

*   `$separator`：这是一个可选参数，属于字符串类型。数组的值将连接成一个字符串，并将由这里提供的`$separator`参数分隔。这是可选的，如果未提供，则默认为`""`（即空字符串）。
*   `$array`：要联接其值以形成字符串的数组。

## 返回类型

`Join()`函数的返回类型为 String。它将返回由`$array`的元素组成的连接字符串。
