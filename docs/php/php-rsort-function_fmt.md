...
`rsort()`是 PHP 中的一个内置函数，用于按降序（即从大到小）对数组进行排序。它对实际数组进行排序，因此更改会反映在数组本身中。该函数为我们提供了 6 种排序类型，可以根据这些类型对数组进行排序。
...
1.  `$array`：该参数指定要排序的数组。
2.  `SORT_TYPE`：可选参数。此参数指定将在输入数组上执行排序操作的模式。有 6 种分类类型，如下所述：
    ...
    *   `SORT_NATURAL` - 当我们在`SORT_TYPE`参数中传递`4`或`SORT_NATURAL`时，数组中的项将使用自然排序作为字符串进行比较。
...
返回布尔值，成功返回`TRUE`，失败返回`FALSE`。它按降序对原始数组进行排序，该数组将作为参数传递给它。
...
演示如何使用`rsort()`函数以大小写敏感的降序对字符串进行排序的程序。
...
演示如何使用`rsort()`函数以不区分大小写的降序对字符串进行排序的程序。
