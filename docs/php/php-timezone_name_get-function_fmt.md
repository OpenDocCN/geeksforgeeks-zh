...
`timezone_name_get()`函数是 PHP 中的一个内置函数，用于返回时区的名称。`DateTime`对象作为参数发送给`timezone_name_get()`函数，如果成功则返回时区名称，如果失败则返回`False`。
...
此函数成功时返回时区名称，失败时返回`False`。

## 异常：

`timezone_name_get()`函数是`DateTimeZone::getName()`函数的别名。
...
*   [PHP timezone_name_from_abbr()函数](https://www.geeksforgeeks.org/php-timezone_name_from_abbr-function/)
*   [PHP timezone_version_get()函数](https://www.geeksforgeeks.org/php-timezone_version_get-function/)
