...
> 原文：[https://www.geeksforgeeks.org/how-to-add-days-to-date-in-php/](https://www.geeksforgeeks.org/how-to-add-days-to-date-in-php/)

向日期添加天数可以通过多种方式完成。在 PHP 中使用像 `strtotime()`、`date_add()` 这样的内置函数是非常容易做到的。
...
**程序：** 用 `strtotime()` 函数在 PHP 中给日期添加天数的 PHP 程序。
...
*   `object`：指定 `date_create()` 函数返回的 `DateTime` 对象。
*   `interval`：指定 `DateInterval` 对象。

**程序：** PHP 程序使用 `date_add()` 函数在 PHP 中给日期添加天数。
