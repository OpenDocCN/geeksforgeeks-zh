...
`IntlCalendar::setTime()` 函数是 PHP 中的一个内置函数，用于设置日历时间对象，单位为从纪元开始的毫秒。日历时间瞬间由浮点数表示，其值应为自纪元(1970 年 1 月 1 日 00：00：00.000 UTC)以来的整数毫秒数。它忽略了闰秒。
...
*   `$date`: 此参数保存由自纪元以来的毫秒数表示的瞬间。它忽略闰秒。
