# PHP Imagick::inverseFourierTransformImage() 函数
...
`Imagick::inverseFourierTransformImage()` 函数是 PHP 中的一个内置函数，用于实现图像的离散傅立叶逆变换（DFT），可以是幅值/相位图像对，也可以是实像/虚像图像对。
...
*   `$complement`：它包含一个布尔值，如果为真，则组合为幅度/相位对，否则为实/虚图像对。
...
下面给出的程序演示了 PHP 中的 `Imagick::inverseFourierTransformImage()` 函数：
...
[https://www.php.net/manual/en/imagick.inversefouriertransformimage.php](https://www.php.net/manual/en/imagick.inversefouriertransformimage.php)