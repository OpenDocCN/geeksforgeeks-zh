...
## 参数：
此函数接受单个参数，如以上语法和所述，如下所述。

*   `$STRING`: it must contain a valid PHP code to calculate, but it should not contain the opening and closing PHP tags.

## 注意：
所有语句必须使用分号正确结束。 例如，将字符串初始化为‘ECHO“Geek for Geek”’将导致解析错误。 为了进行纠正，需要将其初始化为‘Echo’Geek for Geek‘；’。

## 返回值：
返回 `NULL`，除非在包含 PHP 代码的输入字符串中调用了 `RETURN` 语句。 然后返回值。 如果输入字符串出现解析错误，该函数将返回 `FALSE`。
