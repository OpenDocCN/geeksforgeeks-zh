...
`Reflection::getModifierNames()`函数是 PHP 中的内置函数，用于返回指定修饰符名称的数组。
...
此函数接受单个参数`$modifiers`，它是修饰符的位域。这里，位字段是由多个相邻的计算机存储器位置组成的数据结构。

## 返回值

此函数返回指定修饰符名称的数组。

下面的程序说明 PHP 中的`Reflection::getModifierNames()`函数：
