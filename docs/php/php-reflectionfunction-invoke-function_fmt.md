...
## ReflectionFunction::invoke()函数

`ReflectionFunction::invoke()`函数是 PHP 中的一个内置函数，用于返回调用的函数调用的结果。
...
此函数接受参数`$args`，该参数保存传递给被调用函数的参数列表。
...
[https://www.php.net/manual/en/reflectionfunction.invoke.php](https://www.php.net/manual/en/reflectionfunction.invoke.php)