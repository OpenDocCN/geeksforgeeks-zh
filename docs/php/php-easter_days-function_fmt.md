...
`easter_days()` 函数是 PHP 中的一个内置函数，它返回 3 月 21 日之后的天数，即复活节在给定的年份。如果未指定年份，则将当前年份作为默认值。
...
1.  `$year` 此参数指定年份。如果未传递参数，则将当前年份作为默认值。
2.  `$method` 此参数允许您基于其他日历来计算复活节日期。如果 `$method` 设置为 `CAL_EASTER_ROMAN`，则使用 1582 年至 1752 年之间的格里高利历。

## 返回值

此函数返回 3 月 21 日之后的天数，即复活节在给定年份。如果未将 `$year` 作为参数传递，则将当前年份作为默认年份，并返回当前年份 3 月 21 日之后的天数。
...
下面的程序解释了未传递参数时 `easter_days()` 函数的工作原理。
...
下面的程序解释了传递 `$year` 参数时 `easter_days()` 函数的工作原理。
...
下面的程序解释了同时传递两个参数时 `easter_days()` 函数的工作原理。
