...
`date_create_immutable_from_format()` 函数是 PHP 中的内置函数，用于根据指定的格式解析时间字符串。
...
```php
DateTimeImmutable DateTime::createFromFormat( string $format,
...
此函数返回一个新的 `DateTimeImmutable` 对象，该对象表示由时间字符串指定的日期和时间，该时间字符串的格式为给定的格式。
