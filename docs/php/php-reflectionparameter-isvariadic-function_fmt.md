...
`ReflectionParameter::isVariadic()` 函数是 PHP 中的一个内置函数，用于在指定参数是可变参数时返回 `true`，否则返回 `false`。
...
如果指定的参数是可变参数，则此函数返回 `TRUE`，否则返回 `FALSE`。

下面的程序说明 PHP 中的 `ReflectionParameter::isVariadic()` 函数：
...
[https://www.php.net/manual/en/reflectionparameter.isvariadic.php](https://www.php.net/manual/en/reflectionparameter.isvariadic.php)