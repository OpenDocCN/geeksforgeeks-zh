...
`Imagick::setSamplingFactors()` 函数是 PHP 中的一个内置函数，用于设置图像采样因子。
...
此函数接受单个参数 `$factors`，该参数保存包含采样因子的关联数组。
...
下面给出的程序演示了 PHP 中的 `Imagick::setSamplingFactors()` 函数：
