...
```php
public Ds\Vector::sorted( $comparator )
```

## 参数

此函数接受保存排序函数的单个参数`$comparator`。
...
// Use sorted() function to sort
// the copy of vector elements
$res = $vect->sorted(function($element1, $element2) {
...
[http://php.net/manual/en/ds-vector.sorted.php](http://php.net/manual/en/ds-vector.sorted.php)