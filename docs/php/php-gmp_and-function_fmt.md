...
`gmp_and()` 是 PHP 中的一个内置函数，用于计算两个 GMP 数字的位与（[GNU Multiple Precision：对于大数](https://en.wikipedia.org/wiki/GNU_Multiple_Precision_Arithmetic_Library)）。
...
此函数返回按位传递的 GMP 编号以及作为参数传递给它的 GMP 编号。
...
当作为 GMP 编号的数字字符串作为参数传递时，计算 GMP 编号的位与运算的程序。
...
当 GMP 编号作为参数传递时，计算 GMP 编号的位与运算的程序。
