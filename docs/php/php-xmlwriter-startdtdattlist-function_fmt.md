...
`XMLWriter::startDtdAttlist()` 函数是 PHP 中的内置函数，用于启动 DTD AttList。DTD 是 Document Type Definition 的缩写，它定义了 XML 文档的结构、合法元素和属性。在 DTD 中，属性是用 ATTLIST 声明声明的。
