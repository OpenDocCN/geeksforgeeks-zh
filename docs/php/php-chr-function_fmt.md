...
ASCII 值表可以参考这里的[链接](https://www.eecis.udel.edu/~amer/CISC651/ASCII-Conversion-Chart.pdf)。
...
该函数接受单个参数 `$asciiVal`。此参数包含有效的 ASCII 值。函数的作用是：返回我们传递给它的 ASCII 值的对应字符，作为参数 `$asciiVal`。
...
当传递不同的 ASCII 码但它们的等价字符相同时，演示 `chr()`函数的程序。
