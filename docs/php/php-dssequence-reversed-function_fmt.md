...
`Ds\Sequence::reversed()` 函数是 PHP 中的一个内置函数，用于返回 Sequence 元素的反向副本。

## 语法

```php
public abstract Ds\Sequence::reversed(): Ds\Sequence
```
...
此函数返回 Sequence 元素的反转副本。

以下程序说明了 PHP 中的 `Ds\Sequence::reversed()` 函数：
