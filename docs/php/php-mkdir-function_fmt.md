...
`mkdir()` 创建一个具有指定路径名的新目录。路径和模式作为参数发送给 `mkdir()` 函数，成功时返回 `TRUE`，失败时返回 `FALSE`。在 Windows 平台上忽略 `mkdir()` 函数中的 `mode` 参数。
...
3.  **`recursive`：** 可选参数，用于设置递归模式。
4.  **`context`：** 可选参数，指定流的行为。
...
1.  `mkdir()` 函数中的 `mode` 参数必须以八进制表示形式指定，使其以零开头。
