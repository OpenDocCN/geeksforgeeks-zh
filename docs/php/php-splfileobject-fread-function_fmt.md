...
此函数接受单个参数 `$length`，该参数用于指定要从文件中读取的长度（以字节为单位）。

## 返回值：

此函数成功时返回从文件读取的字符串，失败时返回 `False`。

## 注意：

它确保名为 `gfg.txt` 的以下程序中使用的文件具有读取权限。
...
[http://php.net/manual/en/splfileobject.fread.php](http://php.net/manual/en/splfileobject.fread.php)