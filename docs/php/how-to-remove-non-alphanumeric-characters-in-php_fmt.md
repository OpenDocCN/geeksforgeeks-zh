...
> 原文: [https://www.geeksforgeeks.org/how-to-remove-non-alphanumeric-characters-in-php/](https://www.geeksforgeeks.org/how-to-remove-non-alphanumeric-characters-in-php/)
...
```php
int preg_replace( $pattern, $replacement_string, $original_string )
```
...
正则表达式 `'/[\W]/'` 匹配所有非字母数字字符，并用 `''` (空字符串)替换。
