...
在 PHP 中设置 Cookie，使用 `setcookie()` 函数。`setcookie()` 函数需要在脚本生成任何输出之前调用，否则不会设置 cookie。
...
创建一个名为 `Auction_Item` 的 cookie，并为其赋值“豪华车”。cookie 将在 2 天后过期(2 天* 24 小时* 60 分钟* 60 秒)。
...
在访问 Cookie 的值之前，最好检查是否设置了 Cookie。因此，为了检查是否设置了 cookie，使用了 PHP `isset()` 函数。要检查是否设置了 cookie“拍卖项目”，`isset()` 函数的执行如下:
...
对于访问 Cookie 值，使用 PHP `$_COOKIE` 超全局变量。它是一个关联数组，包含浏览器在当前请求中发送的所有 cookies 值的记录。记录存储为一个列表，其中 cookie 名称用作密钥。要访问名为“拍卖项目”的 cookie，可以执行以下代码。
...
`setcookie()` 函数可以用来删除一个 cookie。对于删除 cookie，通过传递 cookie 名称和其他参数或空字符串来调用 `setcookie()` 函数，但是这一次，到期日期需要设置在过去。要删除名为“拍卖项目”的 cookie，可以执行以下代码。
