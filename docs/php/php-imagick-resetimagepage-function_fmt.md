...
`Imagick::setImagePage()` 函数是 PHP 中的一个内置函数，用于重置图像页面。
...
此函数接受单个参数 `$page`，该参数以 `wxh+x+y` 的形式保存页面定义，其中 `W` 表示宽度，`H` 表示高度，`x` 表示 x 坐标，`y` 表示 y 坐标。
...
[https://www.php.net/manual/en/imagick.resetimagepage.php](https://www.php.net/manual/en/imagick.resetimagepage.php)