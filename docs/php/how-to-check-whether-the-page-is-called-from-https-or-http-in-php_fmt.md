...
> 原文：[https://www.geeksforgeeks.org/how-to-check-whether-the-page-is-called-from-https-or-http-in-php/](https://www.geeksforgeeks.org/how-to-check-whether-the-page-is-called-from-https-or-http-in-php/)

本文的目的是检查页面是从‘HTTPS’还是‘HTTP’调用的，我们可以使用以下两种方法。

## 方法 1

检查连接是否使用 [SSL](https://www.geeksforgeeks.org/secure-socket-layer-ssl/) ，如果设置了 `$_SERVER['HTTPS']` 的值，那么我们可以说连接是安全的，是从' HTTPS '调用的。如果该值为空，这意味着该值被设置为“0”或“关”，那么我们可以说连接不安全，页面是从“HTTP”调用的。
...
早期方法的一个问题是，在某些服务器中， `$_SERVER['HTTPS']` 未定义，这可能会导致在检查页面是从“HTTPS”还是从“HTTP”调用时出现错误消息。为了克服这个问题，我们还必须检查服务器端口号，如果使用的端口号是 443，则连接是通过“HTTPS”建立的。
