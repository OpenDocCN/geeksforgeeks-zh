...
> 原文: [https://www.geeksforgeeks.org/how-to-get-first-day-of-week-in-php/](https://www.geeksforgeeks.org/how-to-get-first-day-of-week-in-php/)

使用 `strtotime()` 函数通过 PHP 获取一周的第一天。该函数返回默认的时间变量 `timestamp`，然后使用 `date()` 函数将时间戳日期转换为可理解的日期。
...
*   `timestamp`: 是生成日期的默认时间变量。

**注意:** 在 PHP 中，一周从星期一开始，所以如果时间字符串被给定为 `"this week"`，输出将是星期一的时间戳，通过 `date()` 函数可以使其可读。

## 程序 1
当时间字符串为 `"this week"` 时，获取默认的一周第一天。
...
PHP 支持时间字符串中的 `-ve` 索引。因此，为了获得前一周，它可以使用时间字符串作为 `"-1 week"`，为了获得一天，它还必须在时间字符串中包含一天的名称。
