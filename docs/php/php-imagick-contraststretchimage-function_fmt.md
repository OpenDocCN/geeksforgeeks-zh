# PHP Imagick contrastStretchImage() 函数
...
`Imagick::contrastStretchImage()` 函数是 PHP 中的一个内置函数，用于增强图像的对比度。此功能通过调整像素颜色以覆盖整个可用颜色范围来增强彩色图像的对比度。
...
*   `$black_point`：此参数保存黑点。
*   `$white_point`：此参数保存白点。
*   `$channel`：此参数保存 `Imagick` 通道常量，这些常量提供对通道模式有效的任何通道常量。可以使用按位运算符组合多个通道。通道常量的默认值为 `Imagick::CHANNEL_DEFAULT`。
...
下面的程序说明了 PHP 中的 `Imagick::contrastStretchImage()` 函数：
...
$imagick = new Imagick('https://media.geeksforgeeks.org/wp-content/uploads/geeksforgeeks-9.png');
...
[https://www.php.net/manual/en/imagick.contraststretchimage.php](https://www.php.net/manual/en/imagick.contraststretchimage.php)