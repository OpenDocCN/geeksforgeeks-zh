...
`ReflectionParameter::isDefaultValueConstant()` 函数是 PHP 中的一个内置函数，用于在默认值为常量时返回 `TRUE`，否则返回 `FALSE`。
...
如果默认值为常量，则此函数返回 `TRUE`，否则返回 `FALSE`。

以下程序说明 PHP 中的 `ReflectionParameter::isDefaultValueConstant()` 函数：
