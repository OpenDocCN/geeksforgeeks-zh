...
`GmagickDraw::setfillcolor()` 函数是 PHP 中的一个内置函数，用于设置绘制时使用的填充颜色。
...
此函数接受单个参数 `$color`，该参数用于保存像素颜色值。

## 返回值

此函数成功时返回 `GmagickDraw` 对象。
...
下面给出的程序演示了 PHP 中的 `GmagickDraw::setfillcolor()` 函数：
...
[https://www.php.net/manual/en/gmagickdraw.setfillcolor.php](https://www.php.net/manual/en/gmagickdraw.setfillcolor.php)