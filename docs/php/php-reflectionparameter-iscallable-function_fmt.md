# ReflectionParameter::isCallable() 函数
...
`ReflectionParameter::isCallable()` 函数是 PHP 中的一个内置函数，用于在指定参数可调用时返回 `true`，否则返回 `false`。
...
如果指定的参数可调用，则此函数返回 `TRUE`，否则返回 `FALSE`。

以下程序说明 PHP 中的 `ReflectionParameter::isCallable()` 函数：
...
class Department3
{
    function Marketing( string $Parameter4, 
...
[https://www.php.net/manual/en/reflectionparameter.iscallable.php](https://www.php.net/manual/en/reflectionparameter.iscallable.php)