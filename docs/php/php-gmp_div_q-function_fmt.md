...
此函数接受 GMP 编号、`$num1`和`$num2`作为必选参数，如上面的语法所示。 这些参数可以是 PHP 5.6 版和更高版本中的 GMP 对象，也可以传递数字字符串，以便可以将这些字符串转换为数字。

## 返回值

此函数返回一个 GMP 编号，它是`$num1`除以`$num2`时的商。 该函数根据`$num1`和`$num2`的值返回正、负或零。
