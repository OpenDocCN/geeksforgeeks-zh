...
> 原文：[https://www.geeksforgeeks.org/php-xmlreader-xml-function/](https://www.geeksforgeeks.org/php-xmlreader-xml-function/)

`XMLReader::xml()` 函数是 PHP 中的一个内置函数，用于设置包含要解析的 XML 的数据。`xml()` 函数与 `open()` 函数的用途相同，但唯一的区别是前者接受 XML 作为字符串，而后者接受它作为单独的 `.xml` 文件。
...
*   `$source`: 指定包含要解析的 XML 的字符串。
*   `$encoding` (可选): 指定文档编码或 NULL。
*   `$options` (可选): 指定一个可选的位掩码。
