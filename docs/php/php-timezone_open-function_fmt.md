...
`timezone_open()` 函数是 PHP 中的一个内置函数，用于创建新的 `DateTimeZone` 对象。该函数接受一个时区字符串作为参数，成功时返回 `DateTimeZone` 对象，失败时返回 `False`。
...
此函数接受单个必需参数 `$timezone`，用于指定要创建的新 `DateTimeZone` 对象的时区。
...
作为参数传递的时区必须是 PHP 支持的时区，否则可能会导致错误的结果。
...
`timezone_open()` 函数在传递的时区不是支持的或有效的时区时会发出警告。

## 引用

[http://php.net/manual/en/function.timezone-open.php](http://php.net/manual/en/function.timezone-open.php)