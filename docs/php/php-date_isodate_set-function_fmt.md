...
*   `$object`：此参数仅在过程化风格中使用。此参数由 `date_create()` 函数创建。此函数会修改此对象。
*   `$year`：此参数用于设置日期的年份。
*   `$week`：此参数设置日期的周。
*   `$day`：此参数设置从一周第一天开始的偏移量。

## 返回值

此函数成功时返回方法链接的 `DateTime` 对象，失败时返回 `False`。
...
[http://php.net/manual/en/datetime.setisodate.php](http://php.net/manual/en/datetime.setisodate.php)