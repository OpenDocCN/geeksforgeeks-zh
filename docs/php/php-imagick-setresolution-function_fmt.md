...
`Imagick::setResolution()`函数是 PHP 中的内置函数，用于设置图像的分辨率。 此函数不会更改图像的实际分辨率，只是在读取或创建图像之前在 Imagick 对象中设置它，要更改图像分辨率，请使用[`setImageResolution()`](https://www.geeksforgeeks.org/php-imagick-setimageresolution-function/)函数。 在读取或创建图像之前需要调用此函数。
