...
`DOMDocument::createElementNS()` 函数是 PHP 中的内置函数，用于创建具有关联名称空间的新元素节点。
...
*   `$qualifiedName`: 此参数保存元素的限定名称，例如前缀:标签名。
*   `$value`: 此参数保存元素的值。此参数的默认值为空或无，表示已创建一个空元素。

## 返回值：

此函数成功时返回新的 `DOMElement`，失败时返回 `False`。
...
[https://www.php.net/manual/en/domdocument.createelementns.php](https://www.php.net/manual/en/domdocument.createelementns.php)