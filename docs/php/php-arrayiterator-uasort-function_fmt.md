...
`ArrayIterator::uasort()` 函数是 PHP 中的一个内置函数，用于使用用户定义的比较函数对元素进行排序，并维护它们的索引关联。
...
该函数接受单个参数 `$cmp_function`，这是用户自定义的比较函数。此比较函数接受两个参数，即 `ArrayIterator` 的值，如果第一个参数小于、等于或大于零，则分别返回小于、等于或大于零的值。
...
[https://www.php.net/manual/en/arrayiterator.uasort.php](https://www.php.net/manual/en/arrayiterator.uasort.php)