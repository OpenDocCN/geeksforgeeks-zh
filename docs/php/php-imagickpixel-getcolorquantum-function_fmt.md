# ImagickPixel::getColorQuantum() 函数
...
`ImagickPixel::getColorQuantum()` 函数是 PHP 中的一个内置函数，用于获取数组中像素的颜色作为量子值。此函数返回一个具有键 `r`、`g`、`b` 的数组，每个键分别代表红色、绿色、蓝色和 Alpha/不透明度。量子值的范围从 0 到 65535，其中 0 是最低值，65535 是最高值，即不透明度为 0 表示透明，65535 表示不透明。
...
[https://www.php.net/manual/en/imagickpixel.getcolorquantum.php](https://www.php.net/manual/en/imagickpixel.getcolorquantum.php)