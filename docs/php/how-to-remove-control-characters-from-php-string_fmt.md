...
> 原文：[https://www.geeksforgeeks.org/how-to-remove-control-characters-from-php-string/](https://www.geeksforgeeks.org/how-to-remove-control-characters-from-php-string/)
...
有许多正则表达式可以用来删除 ASCII 值低于 32 的所有字符。这里，我们将使用 `preg_replace()` 方法。

**注意：**`ereg_replace()` 方法是从 PHP >= 7 中移除的，所以这里我们将使用 `preg_replace()` 方法。
