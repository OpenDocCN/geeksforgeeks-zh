...
此函数接受四个参数，如上面的语法所示，其中前两个参数是强制提供的，其余两个参数是可选的。 所有这些参数如下所述：
...
在此程序中，通过将最后一个参数设置为`STR_PAD_BOTH`，我们将填充到输入字符串的两侧。 如果填充长度不是偶数，则右侧获得额外的填充。
...
在此程序中，通过将最后一个参数设置为`STR_PAD_LEFT`，我们将填充到输入字符串的左侧。
...
在此程序中，通过将最后一个参数设置为`STR_PAD_RIGHT`，我们将填充到输入字符串的右侧。
