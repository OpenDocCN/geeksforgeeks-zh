...
此函数接受单个参数 `$codepoint`，该参数是必需的。 输入参数 `$codepoint` 是一个字符或整数值，编码为 UTF-8 字符串。

## 返回值

如果 `$codepoint` 是 Java 标识符字符，则返回 True，否则返回 False。

以下程序说明 PHP 中的 `IntlChar::isJavaIDPart()` 函数：
...
[http://php.net/manual/en/intlchar.isjavaidpart.php](http://php.net/manual/en/intlchar.isjavaidpart.php)