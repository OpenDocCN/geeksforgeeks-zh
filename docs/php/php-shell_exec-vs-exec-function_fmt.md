...
> 原文：[https://www.geeksforgeeks.org/php-shell_exec-vs-exec-function/](https://www.geeksforgeeks.org/php-shell_exec-vs-exec-function/)
...
此函数接受单个参数 `$cmd`，该参数用于保存将要执行的命令。

### 返回值

此函数返回执行的命令，如果出现错误，则返回 `NULL`。
...
*   `$OUTPUT`：此参数用于指定一个数组，该数组将被命令的每一行输出填充。
*   `$RETURN_var`：`$RETURN_var` 参数与 `output` 参数一起出现，然后它返回已执行命令的状态，该状态将被写入此变量。

### 返回值

此函数返回执行的命令，请务必设置并使用 `OUTPUT` 参数。
