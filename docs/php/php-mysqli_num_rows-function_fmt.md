...
`mysqli_num_rows()` 函数是 PHP 中的一个内置函数，用于返回结果集中的行数。它通常用于检查数据库中是否存在数据。要使用此功能，必须先建立与 MySQL 数据库的连接。
...
假设在名为 `Geek` 的 MySQL 数据库中有一个名为 `Geek` 的表。下面是对表 `geek` 的描述。
...
下面的程序演示了 PHP 中的 `mysqli_num_rows()` 函数。
...
<?php
    ...
    // query to fetch Username and Password from the table geek
