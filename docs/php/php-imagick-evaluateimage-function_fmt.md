...
`Imagick::evaluateImage()` 函数是 PHP 中的内置函数，用于将表达式应用于图像。它可以将任何算术、逻辑或关系表达式应用于图像并评估其结果。该操作符有许多用途，从改变图像的亮度（即使图像变亮或变暗），或改变图像的对比度，或产生图像的负片。
...
如果函数执行成功，则返回布尔值 `TRUE`，否则返回 `FALSE`。

下面的程序演示了 PHP 中的 `Imagick::evaluateImage()` 函数：
