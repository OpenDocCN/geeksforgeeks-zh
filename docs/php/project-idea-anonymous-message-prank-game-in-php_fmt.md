...
首先，让我们从 Assets 文件夹开始。 因此，我们创建此文件夹是因为我们在项目中使用的资产，如图像、视频、全局 CSS 和附加到项目中所有文件的 js 文件。 我们还为所有文件夹创建了一个 `index.php` 文件，因为如果有人试图访问此文件夹，则他只能看到索引页，而不是我们的目录。
...
将文件名设置为 `ProtectXss.php`。此文件主要用于保护我们的 Web 应用程序免受跨站点脚本攻击。
