# 使用简单 HTML DOM 解析器的 PHP 网页抓取

> 原文：[https://www.geeksforgeeks.org/web-scrapping-in-php-using-simple-html-dom-parser/](https://www.geeksforgeeks.org/web-scrapping-in-php-using-simple-html-dom-parser/)
...
## 示例 1
下面给出的示例显示了这个应用编程接口的使用，在本地主机上显示谷歌搜索。
...
// Extracting DOM
$html = file_get_html('http://www.google.com/search?q='.$_POST["search"]);
...
## 示例 2
这里我们将尝试访问 google 第一个搜索结果上的文本。为此，我们首先获取 DOM 组件，该组件具有向谷歌请求的查询的第一个结果。在这里，我们从 DOM 中获取具有类“kCrYT”的 span 标记，其中有所有搜索到的详细信息的列表，但是我们只需要第一个，所以循环只迭代一次。

### PHP 代码
如果你已经在谷歌搜索引擎上搜索过任何东西，这个代码可以工作。
```php
<?php
include('simple_html_dom.php');
