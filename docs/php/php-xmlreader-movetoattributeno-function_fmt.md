...
`XMLReader::moveToAttributeNo()`函数是 PHP 中的一个内置函数，用于按索引将光标移动到属性。当节点具有多个属性，但我们只需要特定属性时，此函数非常有用。
