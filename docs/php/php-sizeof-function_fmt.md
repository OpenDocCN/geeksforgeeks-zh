...
> 原文：[`https://www.geeksforgeeks.org/php-sizeof-function/`](https://www.geeksforgeeks.org/php-sizeof-function/)
...
*   `array`：此参数表示要计数的元素数组。
*   `mode`：这是一个可选参数，用于指定函数的模式。可以取两个不同的值，如下所示：
    *   `0`：默认值，不计算多维数组的所有元素。
    *   `1`：递归计数数组（计算多维数组的所有元素）。

## 返回值

此函数返回语法中所示的整数值，表示数组中存在的元素数。
