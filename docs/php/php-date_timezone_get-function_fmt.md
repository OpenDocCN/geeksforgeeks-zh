...
此函数接受单个参数 `$Object`，这在过程式风格中是必需的。 它用于指定 `date_create()` 函数返回的 `DateTime` 对象。 面向对象的风格不需要任何参数。

## 返回值

此函数成功时返回 `DateTimeZone` 对象，失败时返回 `False`。
...
*   [PHP `gmdate()`](https://www.geeksforgeeks.org/php-gmdate-function/)
*   [PHP `date_date_set()`](https://www.geeksforgeeks.org/php-date_date_set-function/)

## 引用

[http://php.net/manual/en/datetime.gettimezone.php](http://php.net/manual/en/datetime.gettimezone.php)