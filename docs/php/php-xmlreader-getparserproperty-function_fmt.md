# XMLReader::getParserProperty() 函数
...
`XMLReader::getParserProperty()` 函数是 PHP 中的内置函数，用于检查是否设置了指定的属性。
...
此函数接受单个参数 `$property`，该参数保存与解析器选项常量之一相对应的整数。解析器选项常量列表如下：
...
[https://www.php.net/manual/en/xmlreader.getparserproperty.php](https://www.php.net/manual/en/xmlreader.getparserproperty.php)