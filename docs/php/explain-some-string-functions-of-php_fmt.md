...
> 原文: [https://www.geeksforgeeks.org/explain-some-string-functions-of-php/](https://www.geeksforgeeks.org/explain-some-string-functions-of-php/)
...
## `trim()`、`ltrim()`、`rtrim()` 以及 `chop()` 函数

它从字符串中删除空格或其他字符。它们有两个参数:一个字符串和另一个 `charList`，后者是需要省略的字符列表。
