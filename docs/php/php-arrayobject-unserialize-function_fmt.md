...
`ArrayObject::unserialize()`函数是 PHP 中的一个内置函数，用于对序列化后的 `ArrayObject` 进行非序列化。
...
该函数接受保存序列化数组对象的单个参数 `$serialized`。
...
下面的程序说明了 PHP 中的 `ArrayObject::unserialize()`函数:
...
[https://www.php.net/manual/en/arrayobject.unserialize.php](https://www.php.net/manual/en/arrayobject.unserialize.php)