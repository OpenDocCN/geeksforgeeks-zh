...
**注意：** `(IS_FINITED(value))` 等同于 `(!IS_INFINITE(value) && !IS_NAN(value))`，即一个数字只能是 `FINDITED`、`INFINITE` 和 `NAN(不是数字)` 中的一个。您不需要同时检查 `IS_INFINITE()` 和 `IS_NaN()` 来查看数字是否无效或超出范围。

## 12. `power()`
此函数将 `base` 和 `exponent` 作为参数，并返回 `base` 的 `exponent` 幂。
...
## 15. `log()`
此函数接受任何数字和底数作为参数，并返回数字的自然对数或以 `base` 为底的数字的对数。

**语法：**
```php
log(number, base);
```
其中，`number` 指定要计算对数的值。`base(可选)` 指定要使用的对数底（默认值为 `e`）。
...
**示例：**
这里，`min` 指定要返回的最小数字（默认值为 0）。`max` 指定要返回的最大数字（默认值为 `getrandmax()`）。
```php
<?php
echo(rand() . "\n");
echo(rand(12,100));
