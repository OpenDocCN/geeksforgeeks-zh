# DOMDocument::normalizeDocument() 函数
...
`DOMDocument::normalizeDocument()` 函数是 PHP 中的一个内置函数，用于规范化文档。如果您保存并加载了文档，则此函数用于将文档转换为标准格式。
...
下面的程序说明了 PHP 中的 `DOMDocument::normalizeDocument()` 函数：
