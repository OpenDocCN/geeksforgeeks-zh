...
`str_ireplace()`是 PHP 中的一个内置函数，用于将搜索字符串或搜索字符串数组中出现的所有内容分别替换为给定字符串或数组中的替换字符串或替换字符串数组。 此函数以不区分大小写的方式执行搜索。 此函数类似于[`str_replace()`](https://www.geeksforgeeks.org/php-str_replace-function/)函数。 不同之处在于`str_replace()`函数区分大小写，而`str_ireplace()`不区分大小写。
...
该函数接受 4 个参数，其中 3 个是必选的，1 个是可选的。 所有这些参数如下所述：
...
此函数返回一个字符串或基于`$subjectVal`参数的数组，其中包含替换的值。
...
下面的程序演示了 PHP 中的`str_ireplace()`函数：

## 程序 1

此程序显示`str_ireplace()`函数不区分大小写。
