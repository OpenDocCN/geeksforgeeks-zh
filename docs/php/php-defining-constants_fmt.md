...
在生产级代码中，将信息作为变量或常量保存，而不是显式使用它们，这一点非常重要。PHP 常量只是一个简单值的标识符，它往往不会随着时间的推移而改变（例如网站的域名，例如 `www.geeksforgeeks.org`）。理想的做法是将所有常量保存在单个 PHP 脚本中，这样维护起来更容易。有效的常量名称必须以字母或下划线开头，并且不需要 `$` 符号。需要注意的是，常量与其作用域无关，即常量自动为全局作用域。
...
*   `identifier`：指定要分配给常量的名称。
*   `value`：指定要分配给常量的值。
*   `case-insensitivity`（可选）：指定常量标识符是否应该不区分大小写。默认情况下，它设置为 `FALSE`，即区分大小写。
