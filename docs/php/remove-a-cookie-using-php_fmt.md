...
Cookie 是服务器为了给用户保存状态信息而发送的小文件。它存储在客户端计算机上，并在用户每次请求同一页面时发送到服务器。

要创建 cookie，您可以使用 PHP 的 `setcookie()` 函数来设置 cookie。
...
如前所述，我们可以使用 `setcookie()` 函数设置 Cookie。
...
PHP 中没有专门提供删除 Cookie 的函数。我们所要做的就是通过使用 `setcookie()` 函数将其设置为过去的时间来更新 cookie 的**过期时间**值。一个非常简单的方法是从当前时间中扣除几秒钟。
