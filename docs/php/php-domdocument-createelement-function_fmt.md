...
`DOMDocument::createElement()` 函数是 PHP 中的内置函数，用于创建 `DOMElement` 类的新实例。
...
*   `$name`: 此参数保存元素的标签名称。
*   `$value`: 此参数保存元素的值。此函数的默认值会创建一个空元素。您可以稍后使用 `DOMElement::$nodeValue` 来设置元素的值。

## 返回值

此函数成功时返回 `DOMElement` 类的新实例，失败时返回 `False`。
...
[https://www.php.net/manual/en/domdocument.createelement.php](https://www.php.net/manual/en/domdocument.createelement.php)