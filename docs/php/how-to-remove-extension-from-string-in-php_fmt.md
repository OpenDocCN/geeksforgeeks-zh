...
> 原文: [https://www.geeksforgeeks.org/how-to-remove-extension-from-string-in-php/](https://www.geeksforgeeks.org/how-to-remove-extension-from-string-in-php/)
...
`pathinfo()` 函数返回一个包含目录名、基名、扩展名和文件名的数组。
...
或者，如果只有一个 `PATHINFO_` 常量作为参数传递，它只返回完整文件名的那部分。
...
`basename()` 函数用于以字符串形式返回路径的尾随名称组件。`basename()` 对输入字符串进行天真的操作，不知道实际的文件系统或路径组件，如“..”。
...
从文件名中删除扩展名的另一种方法是使用字符串函数 `substr()` 和 `strrpos()`。`substr()` 函数返回字符串的一部分，而 `strrpos()` 函数查找字符串中最后一个子字符串的位置。
