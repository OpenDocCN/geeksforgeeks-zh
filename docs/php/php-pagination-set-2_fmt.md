...
在`connection.php`中创建到数据库的连接后，我们只需使用`REQUIRE_ONCE`关键字将文件导入。 接下来，我们将显式定义在单个页面中显示的最大记录数。 随后，我们检查 URL 中是否有任何`page`属性，即我们在`$_GET`方法中查找`page`属性，如果找到，则将其设置为要显示的页码，否则为第一页或值为 1。之后，我们将计算索引以开始提取，它是使用前面推导的公式计算的，但由于 SQL 使用 0 索引，所以我们不会添加额外的 1。最后，我们只提取从索引`$start_from`开始的记录的每个字段，并将其限制为`$limit`。
