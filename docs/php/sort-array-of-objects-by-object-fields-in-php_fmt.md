...
> 原文：[https://www.geeksforgeeks.org/sort-array-of-objects-by-object-fields-in-php/](https://www.geeksforgeeks.org/sort-array-of-objects-by-object-fields-in-php/)

给定一个对象数组，任务是按给定的字段按对象对数组进行排序。

## 方法

`usort()`函数是 PHP 中的一个内置函数，用于使用给定的比较器函数有条件地对元素数组进行排序。`usort()`函数也可用于按对象字段对对象数组进行排序。调用`usort()`函数，将第一个参数作为对象数组，将第二个参数作为比较器函数，在此基础上对两个数组对象进行比较。
