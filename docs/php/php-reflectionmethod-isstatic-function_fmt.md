...
`ReflectionMethod::isStatic()` 函数是 PHP 中的一个内置函数，用于在指定的方法是静态的时返回 `true`，否则返回 `false`。
...
如果指定的方法是静态的，则此函数返回 `TRUE`，否则返回 `FALSE`。

## 程序 1：`ReflectionMethod::isStatic()` 函数
...
[https://www.php.net/manual/en/reflectionmethod.isstatic.php](https://www.php.net/manual/en/reflectionmethod.isstatic.php)