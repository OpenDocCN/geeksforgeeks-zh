...
`Gmagick::CompositeImage()` 函数是 PHP 中的一个内置函数，用于在指定的偏移量将一个图像合成到另一个图像上。偏移实际上是从开始合成第二幅图像的位置开始的距离。
...
此程序使用 `Gmagick::CompositeImage()` 函数合成两幅没有偏移的图像。
...
此程序使用 `Gmagick::CompositeImage()` 函数合成两幅带有偏移量的图像。
...
[https://www.php.net/manual/en/gmagick.compositeimage.php](https://www.php.net/manual/en/gmagick.compositeimage.php)