...
此函数返回在仅包含`charlist`参数中的字符的字符串中找到的字符数。
...
此程序说明如何使用带有`$start`和`$length`参数的`strspn()`函数。
...
此程序说明了带负`$length`参数的`strspn()`函数的用法。
...
此程序说明了带负`$start`参数的`strspn()`函数的用法。
