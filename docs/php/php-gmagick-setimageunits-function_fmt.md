...
`Gmagick::setimageunits()` 函数是 PHP 中的一个内置函数，用于设置特定图像的分辨率单位。此函数对图像没有视觉影响，但只更改分辨率单位，可以是未定义分辨率、`PixelsPerInchResolution` 或 `PixelsPerCentimeterResolution` 之一。
...
此函数接受单个参数 `$resolution`，该参数保存与[分辨率常量](https://www.php.net/manual/en/gmagick.constants.php#gmagick.constants.resolution-undefined)之一相对应的整数。
...
下面给出的程序演示了 PHP 中的 `Gmagick::setimageunits()` 函数：
...
[https://www.php.net/manual/en/gmagick.setimageunits.php](https://www.php.net/manual/en/gmagick.setimageunits.php)