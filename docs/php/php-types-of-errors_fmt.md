...
PHP 中有各种类型的错误，但它包含基本上四种主要类型的错误。

## 1. 解析错误或语法错误

这是程序员在程序源代码中犯的错误类型。语法错误由编译器捕获。修复语法错误后，编译器编译代码并执行它。解析错误可能由未闭合的引号、缺失或多余的圆括号、未闭合的大括号、缺少分号等引起。
...
这是一种错误类型，PHP 编译器可以理解 PHP 代码，但它可以识别未声明的函数。这意味着在没有定义函数的情况下调用函数。
...
**说明：**第 12 行调用了函数`diff()`，但函数定义不可用。所以它会产生误差。

## 3. 警告错误

警告错误的主要原因是包含一个缺失的文件。这意味着 PHP 函数调用了缺失的文件。
...
**解释：**此程序使用未声明变量`$geeks`，因此它会给出错误消息。
