...
> Original: [https://www.geeksforgeeks.org/php-show-source-function/](https://www.geeksforgeeks.org/php-show-source-function/)
...
如果设置为 `true`，则以字符串形式返回突出显示的代码。它将在成功时返回 `TRUE`，或者在失败时返回 `FALSE`。
...
下面的程序使用文件名 `show_source.php` 保存文件。
...
下面的程序使用文件名 `source_code.php` 保存文件。
