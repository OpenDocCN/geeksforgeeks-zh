...
函数的作用是：`imagealphablending()` 是 PHP 的内置函数，用于设置图像的混合模式。此功能允许使用两种不同的模式（混合模式和非混合模式）来绘制真彩色图像。绘制使用过的调色板图像时，混合模式不可用。
...
下面的程序说明了 PHP 中的 `imagealphablending()` 函数：
...
[http://php.net/manual/en/function.imagealphablending.php](http://php.net/manual/en/function.imagealphablending.php)