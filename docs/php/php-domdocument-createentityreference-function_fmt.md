...
`DOMDocument::createEntityReference()` 函数是 PHP 中的内置函数，用于创建 `DOMEntityReference` 类的新实例。
...
此函数接受保存实体引用内容的单个参数 `$name`。实体引用不包含前导 `&` 和尾随 `;` 字符。

## 返回值

此函数成功时返回新的 `DOMEntityReference` 对象，失败时返回 `False`。
