...
`XMLWriter::endDtd()` 函数是 PHP 中的一个内置函数，用于结束使用 `XMLWriter::startDtd()` 函数启动的当前 DTD。DTD 是 Document Type Definition 的缩写，它定义了 XML 文档的结构、合法元素和属性。DTD 在网页中不可见，因为它们的行为类似于注释。
