...
此函数接受两个 GMP 编号 `$num1` 和 `$num2` 作为必选参数，如上面的语法所示。 这些参数可以是 PHP 5.6 版和更高版本中的 GMP 对象，也可以将数字字符串传递给函数，前提是可以将这些字符串转换为数字。

## 返回值

此函数返回一个 GMP 编号，它是除法的余数。
...
当作为 GMP 编号的数字字符串作为参数传递时，执行 GMP 编号除法的程序。
...
http://php.net/manual/en/function.gmp-div-r.php