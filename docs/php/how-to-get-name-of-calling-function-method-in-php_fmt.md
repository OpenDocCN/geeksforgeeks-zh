...
> 原文：[https://www.geeksforgeeks.org/how-to-get-name-of-calling-function-method-in-php/](https://www.geeksforgeeks.org/how-to-get-name-of-calling-function-method-in-php/)

## 为什么需要获取调用函数的名称？
代码由执行不同任务但彼此直接或间接相关联的多个函数组成，并且突然在某些函数中显示错误，则有必要找到发生错误的函数的名称。

## 进场:
*   声明一个用户定义的函数`CallingFunctionName()`，它将作为调试器函数（它跟踪脚本中的另一个函数）。
*   使用“new”关键字创建异常类的引用。
...
## 解释:
`getTrace()`函数将扫描整个方法调用堆栈，并以关联数组的形式存储所有信息。关联数组包含以下信息。
*   `【文件】:`当前 PHP 文件的绝对路径。
...
## 说明:
输出包含函数`firstCall()`的所有信息，因为下标`trace`的值是 2。
*   `trace[0]:`位置 0 将是函数本身（这里称为`CallingFunctionName()`）。
*   `trace[1]:`位置 1 将是函数`secondCall`，因为这是调用堆栈中最上面的函数。
