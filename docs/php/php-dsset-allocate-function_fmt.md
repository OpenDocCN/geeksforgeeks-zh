...
`Ds\Set::allocate()` 函数是 PHP 中的一个内置函数，用于为所需的容量分配内存。
...
此函数接受单个参数 `$capacity`，该参数保存要分配的容量的值。容量总是以 2 的幂四舍五入。
...
下面的程序说明了 PHP 中的 `Ds\Set::allocate()` 函数：
...
[http://php.net/manual/en/ds-set.allocate.php](http://php.net/manual/en/ds-set.allocate.php)