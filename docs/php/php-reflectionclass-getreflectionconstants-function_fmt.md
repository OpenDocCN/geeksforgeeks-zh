...
`ReflectionClass::getReflectionConstants()` 函数是 PHP 中的内置函数，用于返回 `ReflectionClassConstant` 对象的数组。
...
此函数返回 `ReflectionClassConstant` 对象的数组。

## 程序 1

以下程序说明 PHP 中的 `ReflectionClass::getReflectionConstants()` 函数：
