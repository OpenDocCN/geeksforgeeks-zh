...
`XMLReader::expand()` 函数是 PHP 中的一个内置函数，用于复制当前节点并返回适当的 DOM 对象。
...
此函数接受单个参数 `$basenode`，该参数保存定义所创建 DOM 对象的目标 DOMDocument 的 `DOMNode`。

## 返回值

此函数成功时返回 `DOMNode`，失败时返回 `False`。
...
[https://www.php.net/manual/en/xmlreader.expand.php](https://www.php.net/manual/en/xmlreader.expand.php)