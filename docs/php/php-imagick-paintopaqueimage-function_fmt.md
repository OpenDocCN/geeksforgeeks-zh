# PHP Imagick paintOpaqueImage() 函数
...
`Imagick::paintOpaqueImage()` 函数是 PHP 中的一个内置函数，用于更改与特定颜色匹配的任何像素。
...
*   `$channel`：此参数保存 Imagick 通道常量，这些常量提供对通道模式有效的任何通道常量。 可以使用按位运算符组合多个通道。 默认值为 `Imagick::CHANNEL_DEFAULT`。
...
下面的程序演示了 PHP 中的 `Imagick::paintOpaqueImage()` 函数：
...
[https://www.php.net/manual/en/imagick.paintopaqueimage.php](https://www.php.net/manual/en/imagick.paintopaqueimage.php)