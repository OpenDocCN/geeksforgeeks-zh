...
`highlight_file()` 函数是 PHP 中的一个内置函数，用于突出显示文件的语法。语法通过使用 HTML 标记突出显示。
...
成功返回 `TRUE`，失败返回 `FALSE`。如果 `$return` 设置为 `true`，它将以字符串形式返回突出显示的代码。
...
[https://php.net/manual/en/function.highlight-file.php](https://php.net/manual/en/function.highlight-file.php)