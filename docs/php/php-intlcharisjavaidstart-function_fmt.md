...
`IntlChar::isJavaIDStart()` 函数是 PHP 中的一个内置函数，用于检查输入字符代码点是否允许作为 Java 标识符的第一个字符。对于一般类别为“SC”(货币符号)和“Pc”(连接标点符号)的字符返回 True。
...
如果 `$codepoint` 可以作为 Java 标识符字符的开头，则返回 True，否则返回 False。
...
[http://php.net/manual/en/intlchar.isjavaidstart.php](http://php.net/manual/en/intlchar.isjavaidstart.php)