...
`DirectoryIterator::isLink()` 函数是 PHP 中的内置函数，用于检查当前 `DirectoryIterator` 项是否为符号链接。
...
如果项目是符号链接，则此函数返回 `TRUE`，否则返回 `FALSE`。
...
[https://www.php.net/manual/en/directoryiterator.islink.php](https://www.php.net/manual/en/directoryiterator.islink.php)