...
`Ds\Deque::reversed()` 函数是 PHP 中的一个内置函数，用于返回包含颠倒顺序的元素的 Deque 副本。
...
此函数返回一个 `Deque`，它是包含逆序元素的原始 `Deque` 的副本。

下面的程序说明了 PHP 中的 `Ds\Deque::reversed()` 函数：
...
[http://php.net/manual/en/ds-deque.reversed.php](http://php.net/manual/en/ds-deque.reversed.php)