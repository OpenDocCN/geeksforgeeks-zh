...
| `-` | 它查找给定项目范围内的字符范围，例如，`[a-z]`表示从 a 到 z。 |
| `&#124;` | 它是逻辑或，例如 `x&#124;y` 表示 x 或 y。 |
...
让我们看看 PHP 中正则表达式的预定义函数速查表。PHP 为程序员提供了许多使用正则表达式的有用函数。
...
| [`ereg_replace()`](https://www.geeksforgeeks.org/php-ereg_replace-function/) | 此函数搜索特定的字符串模式，并用替换字符串替换原始字符串(如果找到)。 |
| `eregi_replace()` | 如果搜索模式不区分大小写，则该函数的行为类似于 `ereg_replace()`。 |
| [`preg_replace()`](https://www.geeksforgeeks.org/php-preg_replace-function/) | 如果正则表达式可以在模式和替换字符串中使用，则该函数的行为类似于 `ereg_replace()`函数。 |
| `preg_split()` | 该函数的行为类似于 PHP `Split()`函数。它通过正则表达式作为其参数拆分字符串。 |
...
| `ereg()` | 此函数用于搜索由模式指定的字符串，如果找到则返回 TRUE，否则返回 FALSE。 |
| `eregi()` | 如果搜索不区分大小写，则此函数的行为类似于 `ereg()`函数。 |
...
正则表达式中使用的字符有两种：正则字符和元字符。常规字符是那些具有*‘字面’*含义的字符，元字符是那些在正则表达式中具有*‘特殊’*含义的字符。

| 元字符 | 描述 / 描写 / 形容 / 类别 | 例证 / 范例 / 榜样 / 例子 |
| :--- | :--- | :--- |
| `.` | 它可以匹配除换行符以外的任何单个字符。 | `/./`匹配只有一个字符的字符串。 |
| `^` | 它与字符串的开头匹配。 | `/^geek/`匹配任何以 geek 开头的字符串。 |
| `$` | 它与字符串末尾的字符串模式匹配。 | `/com$/`匹配以 com 结尾的字符串，例如 google.com 等。 |
...
PHP 中的一些正则表达式类似于算术表达式，称为 POSIX 正则表达式。有时，复杂表达式是通过在正则表达式中组合各种元素或运算符来创建的。最基本的正则表达式是匹配单个字符的正则表达式。
...
| `[:digit:]` | 它匹配从 0 到 9 的所有数字。 |
| `[[:digit:]]` | 它匹配包含 0 到 9 之间数字的任何字符串。 |
| `[:xdigit:]` | 它匹配所有的十六进制数字。 |
| `[:punct:]` | 它匹配所有的标点符号。 |
| `[:space:]` | 它匹配空格和制表符等空白字符。 |
| `[:blank:]` | 它匹配所有空格字符，如换行符。 |
| `[[:space:]]` | 它匹配任何包含空格的字符串。 |
| `[:cntrl:]` | 它匹配所有控制字符。 |
| `[:graph:]` | 它匹配除空格和控制字符之外的所有可见或打印字符。 |
| `[:print:]` | 它匹配除控制字符之外的所有打印字符和空格。 |
| `[:word:]` | 它匹配所有单词字符，如数字、字母和下划线。 |

## 正则表达式中的量词

量词是特殊字符，它告诉带括号的字符或字符组的数量、频率或实例数或出现的次数。它们也被称为贪婪和懒惰的表情。让我们来看看量词的一些概念和例子。

| 量词 | 意义 / 隐含意思 / 内涵 / 目 |
| :--- | :--- |
| `a+` | 它与至少包含一个 a 的字符串匹配。 |
| `a*` | 它匹配包含零个或多个 a 的字符串。 |
| `a?` | 它匹配任何包含零个或一个 a 的字符串。 |
...
| `a.*a` | 它匹配任何包含 a 的字符串模式，然后是任何字符，然后是另一个 a。 |
| `^.{3}$` | 它匹配恰好包含三个字符的任何字符串。 |
...
正则表达式是描述特定模式中的某些字符串文本的模式，或者它被定义为用一行表示的模式匹配算法。正则表达式在编程世界中对于验证检查和识别特定模板非常有用。PHP 提供了许多支持正则表达式的内置函数。元字符有助于创建复杂的模式。