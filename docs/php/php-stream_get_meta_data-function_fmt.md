...
该函数接受单个参数 `$stream`，该参数指定要检索的元数据，该元数据由任何函数 `fopen()`、`fsockopen()` 和 `pfsockopen()` 创建。

## 返回值

此函数返回包含以下项的数组：

*   `TIMED_OUT`：它是布尔类型的项，如果流超时，则为 `true`。
*   `blocked`：它是布尔类型的项，如果流处于阻塞 IO 模式，则为 `True`。
...
以下程序说明 PHP 中的 `stream_get_meta_data()` 函数：
