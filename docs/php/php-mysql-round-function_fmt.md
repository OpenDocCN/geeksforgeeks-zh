...
在本文中，我们将了解 MySQL `round()`函数在 PHP 中的工作方式。 [MySQL `round()`函数](https://www.geeksforgeeks.org/round-function-in-mysql/)用于将一个数字舍入到指定的小数位。 如果没有为四舍五入提供指定的小数位，则会将数字四舍五入到最接近的整数。
