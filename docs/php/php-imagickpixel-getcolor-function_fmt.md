...
`ImagickPixel::getColor()` 函数是 PHP 中的一个内置函数，用于获取 `ImagickPixel` 对象以数组形式描述的颜色。如果颜色设置了不透明度通道，则该值将作为列表中的第四个值提供。数组的关键点是 `r` 为（红色）、`b` 为（蓝色）、`g` 为（绿色）和 `a` 为（Alpha/不透明度）。
...
此函数接受单个参数 `$normalized`，该参数指示是否规格化值。
...
[https://www.php.net/manual/en/imagickpixel.getcolor.php](https://www.php.net/manual/en/imagickpixel.getcolor.php)