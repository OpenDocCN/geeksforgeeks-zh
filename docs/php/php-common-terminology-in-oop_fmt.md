...
> 原文: [https://www.geeksforgeeks.org/php-common-terminology-in-oop/](https://www.geeksforgeeks.org/php-common-terminology-in-oop/)
...
在上面的例子中，创建了一个电视类的对象`$tv_one`，并实现了显示该对象行为的函数。最初`$tv_one->volume`为 1。调用其函数后，音量已增大并显示更新结果。`$this`是指特定的或当前的对象。您可以创建多个对象并显示其行为。实现这些代码将会有代码重用的好处，并且代码在将来会很容易管理。
