...
函数`memcached::addServer()`是 PHP 中 `Memcached` 类的内置函数，用于将服务器添加到服务器池。它会将指定的服务器添加到服务器池中。目前还没有建立到服务器的连接，但是如果您使用一致密钥分发选项(通过 `Memcached::DISTRIBUTION_CONSISTENT` 或 `Memcached::OPT_LIBKETAMA_COMPATIBLE`)，则必须更新一些内部数据结构。因此，如果需要添加多个服务器，最好使用 `Memcached::addServers()`，因为更新只发生一次。
...
*   `port`：运行 memcache 的端口。通常，这是 `11211`。
*   `weight`：服务器相对于池中所有服务器的总权重的权重。用于负载均衡。
