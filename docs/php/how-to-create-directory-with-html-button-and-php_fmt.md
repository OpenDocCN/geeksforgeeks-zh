...
> 原文：[https://www.geeksforgeeks.org/how-to-create-directory-with-html-button-and-php/](https://www.geeksforgeeks.org/how-to-create-directory-with-html-button-and-php/)

`mkdir()` 函数用于在 PHP 中创建目录。这是 PHP 中的一个内置函数。`mkdir()` 函数用指定的路径名创建一个新目录。路径和模式作为参数发送给 `mkdir()` 函数，成功时返回真，失败时返回假。
