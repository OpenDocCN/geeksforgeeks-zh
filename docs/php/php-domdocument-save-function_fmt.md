...
`DOMDocument::save()` 函数是 PHP 中的一个内置函数，用于从 DOM 表示创建 XML 文档。此函数在从头创建新 DOM 文档后使用。
...
*   `$filename`: 此参数保存 XML 文档的保存路径。
*   `$options`: 此参数保存附加选项。目前，此参数仅支持 `LIBXML_NOEMPTYTAG`。

## 返回值

此函数返回成功时写入的字节数，失败时返回 `False`。
...
[https://www.php.net/manual/en/domdocument.save.php](https://www.php.net/manual/en/domdocument.save.php)