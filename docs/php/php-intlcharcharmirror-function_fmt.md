...
此函数返回可用作镜像替代的另一个 Unicode 代码点，或者如果没有这样的映射或代码点没有 `BIDI_MIRRORED` 属性，则返回代码点本身。 该函数返回一个整数，除非在 `UTF-8` 字符串中传递了代码点，否则将返回一个字符串。
...
[http://php.net/manual/en/intlchar.charmirror.php](http://php.net/manual/en/intlchar.charmirror.php)