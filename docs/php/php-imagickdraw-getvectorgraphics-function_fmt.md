...
`ImagickDraw::getVectorGraphics()`函数是 PHP 中的内置函数，用于获取包含矢量图形的字符串。简而言之，它包含字符串形式的所有绘制命令。它还用于从`ImagickDraw`对象提取注释。它返回一个包含如此多不需要的数据的大字符串，可以使用 PHP `substr()`函数对其进行修剪。
...
[https://www.php.net/manual/en/imagickdraw.getvectorgraphics.php](https://www.php.net/manual/en/imagickdraw.getvectorgraphics.php)