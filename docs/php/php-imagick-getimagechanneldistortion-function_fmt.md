...
`Imagick::getImageChannelDistortion()` 函数是 PHP 中的内置函数，用于将图像的图像通道与重建图像进行比较，并返回指定的失真度量。
...
*   `reference`: 指定要与之比较的 `Imagick` 对象。
*   `channel`: 指定对您的通道模式有效的通道常量。要应用多个通道，请使用位运算符组合通道类型常量。
*   `indicator`: 指定一个指示器类型常量。测量常量列表如下：
