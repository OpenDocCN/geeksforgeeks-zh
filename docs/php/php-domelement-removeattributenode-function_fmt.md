...
`DOMElement::removeAttributeNode()`函数是 PHP 中的一个内置函数，用于从元素中删除属性。
...
如果节点是只读的，则此函数抛出 `DOM_NO_MODIFICATION_ALLOWED_ERR`，如果`$oldnode`不是元素的属性，则抛出 `DOM_NOT_FOUND_ERROR`。
