...
`IntlCalendar::set()` 函数是 PHP 中的一个内置函数，用于一次设置时间字段或几个公共字段。字段值的范围取决于日历。如果只有四个参数，则不能调用此函数。
...
*   `$hour`：此参数保存 `IntlCalendar::FIELD_HOUR_OF_DAY` 字段的新值。
*   `$minute`：此参数保存 `IntlCalendar::FIELD_MINUTE` 字段的新值。
...
[https://www.php.net/manual/en/intlcalendar.set.php](https://www.php.net/manual/en/intlcalendar.set.php)