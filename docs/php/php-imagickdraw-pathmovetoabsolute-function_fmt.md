# ImagickDraw::pathMoveToAbsolute() 函数
...
`ImagickDraw::pathMoveToAbsolute()` 函数是 PHP 中的一个内置函数，用于使用绝对坐标在给定坐标处启动新的子路径。然后，当前点将成为指定的坐标。此函数用于在开始绘制任何内容之前设置初始坐标。
...
下面给出的程序说明了 PHP 中的 `ImagickDraw::pathMoveToAbsolute()` 函数：
...
[https://www.php.net/manual/en/imagickdraw.pathmovetoabsolute.php](https://www.php.net/manual/en/imagickdraw.pathmovetoabsolute.php)