...
`IntlChar::foldCase()`函数是 PHP 中的一个内置函数，用于在代码点上执行案例折叠。大小写折叠意味着给定的字符被映射到其等效的小写字符。
...
此函数返回码点的`SIMPLE_CASE_FOLDING`。如果码点没有大小写折叠等效项，则返回码点本身。

下面的程序说明了 PHP 中的`IntlChar::foldCase()`函数：
