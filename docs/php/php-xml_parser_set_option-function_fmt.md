...
`xml_parser_set_option()` 函数是 PHP 中的内置函数，用于设置 XML 解析器中的选项。
...
*   `$specified_option`：它是必需的参数，指定要为指定解析器设置的选项。此参数的可能值为：
...
*   `$option_value`：它是必需参数，指定要为指定选项设置的新值。
...
## 引用
[https://www.php.net/manual/en/function.xml-parser-set-option.php](https://www.php.net/manual/en/function.xml-parser-set-option.php)