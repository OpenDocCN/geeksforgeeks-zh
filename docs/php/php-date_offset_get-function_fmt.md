...
此函数接受单个参数`$Object`，这在过程样式中是必需的。 `date_create()`函数返回的 `DateTime` 对象。 但在面向对象样式的情况下，不需要参数。

## 返回值：

此函数成功时返回 UTC(协调世界时)的时区偏移量(以秒为单位)，失败时返回 False。
...
[http://php.net/manual/en/datetime.getoffset.php](http://php.net/manual/en/datetime.getoffset.php)