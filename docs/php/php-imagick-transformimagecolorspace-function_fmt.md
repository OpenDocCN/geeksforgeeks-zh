# PHP Imagick transformImageColorspace() 函数
...
`Imagick::transformImageColorspace()` 函数是 PHP 中的一个内置函数，用于将图像转换为新的色彩空间。
...
此函数接受用于转换彩色图像的单个参数 `$colorspace`。

## 返回值

成功时此函数返回 `True`。

下面的程序演示了 PHP 中的 `Imagick::transformImageColorspace()` 函数：
...
[http://php.net/manual/en/imagick.transformimagecolorspace.php](http://php.net/manual/en/imagick.transformimagecolorspace.php)