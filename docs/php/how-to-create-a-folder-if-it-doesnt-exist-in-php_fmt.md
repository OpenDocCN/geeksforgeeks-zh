...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-folder-if-it-doesnt-exist-in-php/](https://www.geeksforgeeks.org/how-to-create-a-folder-if-it-doesnt-exist-in-php/)

我们可以很容易地在 PHP 中创建一个文件夹，但是在此之前，您必须检查该文件夹或目录是否已经存在。因此，在本文中，您将学习在 PHP 中检查和创建文件夹或目录。

**方法：**

1.  [`file_exists()`](https://www.geeksforgeeks.org/php-file_exists-function/)：是一个内置功能，用于检查文件或目录是否存在。
...
**返回值：** 如果文件是目录，则返回 `true`，否则返回 `false`。
