...
> 原文：[https://www.geeksforgeeks.org/how-to-loop-through-an-associative-array-and-get-the-key-in-php/](https://www.geeksforgeeks.org/how-to-loop-through-an-associative-array-and-get-the-key-in-php/)

`关联数组`用于存储键值对。例如，要将学生不同科目的分数存储在数组中，数字索引数组不是最佳选择。相反，我们可以使用各自主题的名称作为关联数组中的键，值将是它们各自获得的分数。在关联数组中，键值对与 `=>` 符号相关联。

## 方法 1：使用 `foreach` 循环

在该方法中，使用 `foreach` 循环遍历整个关联数组并显示关键元素。
...
`array_keys()` 是 PHP 中的一个内置函数，用于返回数组的所有键或键的子集。
