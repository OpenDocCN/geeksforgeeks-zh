...
`imagecreatefromwbmp()` 函数是 PHP 中的一个内置函数，用于从 WBMP 文件或 URL 创建新图像。WBMP（无线应用协议位图格式）是一种针对移动计算设备优化的单色图形文件格式。这个加载的图像可以在程序中进一步处理。当您想要在从 WBMP 文件加载图像后对其进行编辑时，通常使用此函数。可以使用 `imagewbmp()` 函数将图像转换为 WBMP。
...
下面的示例说明了 PHP 中的 `imagecreatefromwbmp()` 函数：
...
[https://www.php.net/manual/en/function.imagecreatefromwbmp.php](https://www.php.net/manual/en/function.imagecreatefromwbmp.php)