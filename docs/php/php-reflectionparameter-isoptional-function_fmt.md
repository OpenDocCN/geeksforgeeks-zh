...
`ReflectionParameter::isOptional()` 函数是 PHP 中的一个内置函数，用于在指定的参数是可选的情况下返回 `true`，否则返回 `false`。
...
如果指定的参数是可选的，则此函数返回 `TRUE`，否则返回 `FALSE`。

## 程序 1：`ReflectionParameter::isOptional()` 函数
...
[https://www.php.net/manual/en/reflectionparameter.isoptional.php](https://www.php.net/manual/en/reflectionparameter.isoptional.php)