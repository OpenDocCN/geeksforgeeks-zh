...
> 原文：[https://www.geeksforgeeks.org/saving-an-image-from-url-in-php/](https://www.geeksforgeeks.org/saving-an-image-from-url-in-php/)
...
这是完成任务最基本、最简单的方法。就像任何其他文件一样，从创建一个空文件开始，并以“写”模式打开它。之后，从源网址获取内容并粘贴到这个文件中。这听起来很简单。
...
严格来说，cURL 不仅仅是一个 HTTP 库。它还有其他几种数据传输协议。由于我们的图像在一个 HTTP 服务器上，我们将自己限制在这个库的这个小部分。
...
*   In this function, we have used the `curl _ init` function to initialize an instance of cURL so as to use it to obtain data.
...
$data = file_get_contents_curl('https://media.geeksforgeeks.org/wp-content/uploads/geeksforgeeks-6-1.png');
