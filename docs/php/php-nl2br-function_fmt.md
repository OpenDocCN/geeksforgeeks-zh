...
此函数迭代输入字符串，并在每个换行符之前插入`br`标记，然后返回更改后的字符串。
...
*   Different operating systems prefer to use different character sequences as newline characters, for example, Windows uses `\r\n`, while Linux uses `\n`.
