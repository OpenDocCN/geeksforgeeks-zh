...
> 原文: [https://www.geeksforgeeks.org/how-to-check-foreach-loop-key-value-in-php/](https://www.geeksforgeeks.org/how-to-check-foreach-loop-key-value-in-php/)

在 PHP 中，`foreach` 循环可用于循环元素数组。它可以通过多种方式使用，例如：
...
在上面的例子中，名为 `$names` 的数组的每个元素被迭代并被分配给变量 `$value`。
