...
`IntlDateFormatter::getErrorCode()` 函数是 PHP 中的内置函数，用于返回上次操作的错误代码。
...
此函数使用单个参数 `$fmt`，该参数保存格式化程序的资源。

## 返回值

此函数返回错误代码，`UErrorCode` 值之一。 初始值为 `U_ZERO_ERROR`。
...
[https://www.php.net/manual/en/intldateformatter.geterrorcode.php](https://www.php.net/manual/en/intldateformatter.geterrorcode.php)