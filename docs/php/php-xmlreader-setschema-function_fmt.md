...
`XMLReader::setSchema()` 函数是 PHP 中的一个内置函数，用于使用 XSD 模式验证文档。这个 XSD 模式只是一个定义 XML 结构规则的文件，应该是 `.xsd` 文件的形式。
...
如果在没有架构支持的情况下编译 libxml 扩展，则此函数引发 `E_WARNING`。
