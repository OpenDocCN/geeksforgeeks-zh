...
`DOMDocument::saveHTMLFile()` 函数是 PHP 中的内置函数，用于从 DOM 表示创建 HTML 文档。此函数在创建 DOM 文档后使用。
...
此函数接受单个参数 `$filename`，该参数保存 HTML 文档的路径。

## 返回值

此函数成功时返回字节数，失败时返回 `False`。
...
[https://www.php.net/manual/en/domdocument.savehtmlfile.php](https://www.php.net/manual/en/domdocument.savehtmlfile.php)