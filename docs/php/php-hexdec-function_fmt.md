...
PHP 中的 `hexdec()` 函数将十六进制数转换为十进制数。
函数的作用是：转换太大而不适合整数类型的数字，在这种情况下，较大的值将作为浮点数返回。 如果 `hexdec()` 遇到任何非十六进制字符，它会忽略它们。
...
`hexdec()` 函数接受单个参数 `$value`。 它是要计算其十进制等价物的十六进制数。

## 返回值

PHP 中的 `hexdec()` 函数返回相当于十六进制数的十进制。
