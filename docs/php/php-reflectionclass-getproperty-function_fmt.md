...
`ReflectionClass::getProperty()`函数是 PHP 中的内置函数，用于返回指定类的 `ReflectionProperty` 数组。
...
此函数接受参数`name`，它是属性的名称。

## 返回值

此函数返回指定类的 `ReflectionProperty` 数组。

## 示例

以下程序说明 PHP 中的 `ReflectionClass::getProperty()`函数：
...
[https://www.php.net/manual/en/reflectionclass.getproperties.php](https://www.php.net/manual/en/reflectionclass.getproperties.php)