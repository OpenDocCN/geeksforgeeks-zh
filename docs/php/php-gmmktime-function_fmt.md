...
`gmmktime()` 函数是 PHP 中的一个内置函数，用于返回 GMT 日期的 Unix 时间戳。`$hour`、`$minute`、`$second`、`$month`、`$day`、`$year` 和 `$is_dst` 作为参数发送到 `gmmktime()` 函数，如果成功则返回整数 Unix 时间戳，如果出错则返回 `FALSE`。
...
此函数成功时返回整数 Unix 时间戳，错误时返回 `False`。

## 异常

如果使用 `$is_dst` 参数，则 PHP 5.3.0 版本会抛出 `E_DEPRECATED` 错误。
...
[http://php.net/manual/en/function.gmmktime.php](http://php.net/manual/en/function.gmmktime.php)