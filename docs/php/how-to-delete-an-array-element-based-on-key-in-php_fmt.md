...
> 原文: [https://www.geeksforgeeks.org/how-to-delete-an-array-element-based-on-key-in-php/](https://www.geeksforgeeks.org/how-to-delete-an-array-element-based-on-key-in-php/)
...
**使用 `unset()` 函数：** `unset()` 函数用于从数组中移除元素。`unset` 函数用于销毁任何其他变量，并以同样的方式删除数组中的任何元素。这个 `unset` 命令将数组键作为输入，并从数组中移除该元素。移除后，关联的键和值不会改变。
...
**参数：** 本功能接受单参数 `$variable`。它是必需的参数，用于取消设置元素。
