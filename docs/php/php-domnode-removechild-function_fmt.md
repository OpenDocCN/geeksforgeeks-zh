...
`DOMNode::removeChild()` 函数是 PHP 中的一个内置函数，用于从子级列表中删除子级。
...
如果节点是只读的，抛出 `DOM_NO_MODIFICATION_ALLOWED_ERR`；如果 `$oldnode` 不是此节点的子节点，则抛出 `DOM_NOT_FOUND`。
