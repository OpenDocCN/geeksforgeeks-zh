...
`ctype_punct()` 是 PHP 中的一个内置函数，用于检查非空格或字母数字字符的可打印字符。字符串中的每个字符都是可打印的，但字母数字、数字或空白都不能返回 `True`，否则将返回 `False`。
...
此函数接受单个参数 `$text`。它是指定字符串的必选参数。

## 返回值

如果字符串不包含任何字母数字、数字或空白字符，则返回 `True`；如果失败，则返回 `False`。
...
`ctype_punct()` 函数的代码接受包含整数和特殊符号的输入字符串数组。
...
[http://php.net/manual/en/function.ctype-punct.php](http://php.net/manual/en/function.ctype-punct.php)