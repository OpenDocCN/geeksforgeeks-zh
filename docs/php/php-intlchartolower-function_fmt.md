...
此函数接受单个参数`$codepoint`，这是必需的。 输入参数是一个字符，编码为 UTF-8 字符串。

## 返回值：

此函数返回给定字符的小写映射字符。 如果字符没有小写映射字符，则返回自身。 返回类型将为 INTEGER，除非代码点作为 UTF-8 字符串传递，在这种情况下将返回字符串。
...
[http://php.net/manual/en/intlchar.tolower.php](http://php.net/manual/en/intlchar.tolower.php)