...
`XMLWriter::setIndent()` 函数是 PHP 中的一个内置函数，用于打开/关闭 XML 文档中的缩进，默认情况下该函数处于关闭状态。
...
此函数接受单个参数 `$indent`，该参数包含一个布尔值，声明 `true`（启用缩进）或 `false`（禁用缩进）。
...
[https://www.php.net/manual/en/function.xmlwriter-set-indent.php](https://www.php.net/manual/en/function.xmlwriter-set-indent.php)