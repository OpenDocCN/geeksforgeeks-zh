...
`gmstrftime()` 函数是 PHP 中的内置函数，用于根据本地设置格式化 GMT/UTC 时间/日期。PHP 中的 `gmstrftime()` 函数与 `strftime()` 的行为方式相同，只是 `gmstrftime()` 函数返回的时间是格林威治标准时间(GMT)。`$format` 和 `$timestamp` 作为参数发送给 `gmstrftime()` 函数，并返回根据使用给定时间戳指定的格式格式化的字符串。
...
*   `$format`: 必需参数，用于指定结果的格式。
*   `$timestamp`: 可选参数，指定表示要格式化的日期和/或时间的 UNIX 时间戳。

## 返回值

此函数根据给定时间戳指定的格式返回格式化字符串。
...
[http://php.net/manual/en/function.gmstrftime.php](http://php.net/manual/en/function.gmstrftime.php)