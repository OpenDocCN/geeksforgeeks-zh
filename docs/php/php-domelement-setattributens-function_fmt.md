...
`DOMElement::setAttributeNS()` 函数是 PHP 中的一个内置函数，用于将具有给定名称空间和名称的属性设置为给定值。如果该属性不存在，则会创建该属性。
...
如果节点为只读或 `DOM_NAMESPACE_ERR`，如果 `$qualifiedName` 是错误的限定名称，或者如果 `$qualifiedName` 具有前缀且名称空间 URI 为 `NULL`，则此函数引发 `DOM_NO_MODIFICATION_ALLOWED_ERR`。
