...
`Imagick::pingImageBlob()` 函数是 PHP 中的一个内置函数，用于查询图像属性，而无需将整个图像加载到内存中。可以查询图片的宽度、高度、大小、格式。它以整个图像流为参数。
...
下面的程序演示了 PHP 中的 `Imagick::pingImageBlob()` 函数。此程序将显示图像的高度和宽度，而不会将其实际加载到屏幕上。
