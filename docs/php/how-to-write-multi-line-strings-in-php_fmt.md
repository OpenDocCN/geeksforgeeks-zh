...
> 原文：`https://www.geeksforgeeks.org/how-to-write-multi-line-strings-in-php/`
...
我们可以使用串联赋值运算符`.=`连接两个字符串，用`PHP_EOL`标记行尾。
...
我们可以使用 PHP Heredoc 或 PHP Nowdoc 语法直接编写多行字符串变量。heredoc 和 nowdoc 的区别在于，heredoc 使用双引号字符串。对转义序列等的解析是在 heredoc 内部完成的，而 nowdoc 使用单引号字符串，因此不执行解析。
