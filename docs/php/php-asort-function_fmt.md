...
*   `$sorting_type`: 这是可选参数。下面讨论了不同的排序类型:
    *   `SORT_REGULAR`: `$sorting_type`的值为`SORT_REGULAR`，则项目比较正常。
    *   `SORT_NUMERIC`: `$sorting_type`的值为`SORT_NUMERIC`，然后对项目进行数值比较。
    *   `SORT_STRING`: `$sorting_type`的值为`SORT_STRING`，则项目作为字符串进行比较。
    *   `SORT_LOCALE_STRING`: `$sorting_type`的值是`SORT_LOCALE_STRING`，然后根据当前区域设置将项目作为字符串进行比较。

## 返回值:

该函数成功返回`true`，失败返回`false`。
