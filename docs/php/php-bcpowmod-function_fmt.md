...
该函数接受四个参数，如上面的语法所示，解释如下:

*   `$base` : 此参数为字符串类型，表示左操作数或数字，该数字是乘幂的基数。此参数是必需的。
*   `$exponent` : 该参数为字符串类型，代表右操作数或代表指数的数字之一。此参数是必需的。
*   `$mod` : 该参数为字符串类型，接受代表 modulos 的操作数或数字。此参数是必需的。
*   `$scaleVal` : 此参数为 int 类型，可选。此参数告诉 (`$base`^`$exponent`)%`$mod` 的结果中小数点后出现的位数。它的默认值为零。

## 返回值:

该函数返回字符串形式的 (`$base`^`$exponent`) % `$mod` 结果。
...
[http://php.net/manual/en/function.bcpowmod.php](http://php.net/manual/en/function.bcpowmod.php)