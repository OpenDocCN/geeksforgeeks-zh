...
`ImagickPixel::getHSL()`函数是 PHP 中的一个内置函数，用于获取 `ImagickPixel` 对象描述的规格化 HSL 颜色，每个对象都是 0 到 1 之间的浮点数。HSL 代表色调、饱和度和亮度。 一般来说，色调决定像素的颜色，而饱和度决定颜色的强度，而亮度决定颜色是暗淡的还是明亮的。
...
下面给出的程序说明了 PHP 中的 `ImagickPixel::getHSL()`函数：
