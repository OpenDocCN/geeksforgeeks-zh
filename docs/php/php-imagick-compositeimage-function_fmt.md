...
`Imagick::compositeImage()` 函数是 PHP 中的一个内置函数，用于将一个图像合成到另一个图像中，并生成合成图像。
...
*   `$composite`：它是一个复合运算符常量，如 `Imagick::COMPOSITE_DEFAULT`、`Imagick::COMPOSITE_MATHEMATICS` 等。
