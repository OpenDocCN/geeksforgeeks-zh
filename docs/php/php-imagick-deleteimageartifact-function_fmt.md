...
`Imagick::deleteImageArtifact()`函数是 PHP 中的一个内置函数，用于删除图像工件。图像属性和图像工件之间的主要区别在于属性是公共的，而工件是私有的。
...
下面的程序演示了 PHP 中的`Imagick::deleteImageArtifact()`函数：
