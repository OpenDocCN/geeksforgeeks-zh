...
> 原文：[https://www.geeksforgeeks.org/php-imageconvolution-function/](https://www.geeksforgeeks.org/php-imageconvolution-function/)

`imageconvolution()` 函数是 PHP 中的一个内置函数，用于修改图像内容。它使用给定的系数和偏移量在图像中应用 3x3 卷积矩阵。此函数成功时返回 `TRUE`，失败时返回 `FALSE`。
...
下面的程序说明了 PHP 中的 `imageconvolution()` 函数：
...
[http://php.net/manual/en/function.imageconvolution.php](http://php.net/manual/en/function.imageconvolution.php)