...
> 原文：[https://www.geeksforgeeks.org/php-disk_total_space-function/](https://www.geeksforgeeks.org/php-disk_total_space-function/)

PHP 中的 `disk_total_space()` 函数是一个内置函数，用于返回指定目录的总空间。函数的作用是以字节为单位表示总空间，它返回文件系统或磁盘分区上的总空间。

该函数返回指定目录的相应文件系统或磁盘分区上的总字节数。
...
PHP 中的 `disk_total_space()` 函数只接受一个参数 `$directory`，即要检查的目录路径。
