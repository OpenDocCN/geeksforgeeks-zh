...
函数的作用是：`getprotobynumber()` 函数是 PHP 中的一个内置函数，它返回指定协议号的协议名。
...
此函数接受必需的单个参数 `$protocol_number`。它指定协议号，如 6 表示 TCP，17 表示 UDP 等。

## 返回值

此函数成功时返回协议名，失败时返回 `False`。
...
[https://www.php.net/manual/en/function.getprotobynumber.php](https://www.php.net/manual/en/function.getprotobynumber.php)