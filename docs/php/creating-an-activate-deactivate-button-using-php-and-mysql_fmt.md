...
> 原文：[https://www.geeksforgeeks.org/creating-an-activate-deactivate-button-using-php-and-mysql/](https://www.geeksforgeeks.org/creating-an-activate-deactivate-button-using-php-and-mysql/)
...
为了达到预期的结果，让我们创建一个名为“`courses`”的数据库，并使用下面的 SQL 代码设置几门课程。
...
**网页创建：** 在 `htdocs` 中创建一个名为 `courses` 的文件夹，并将以下文件存储为“`course-page.php`”。
...
我们实际上没有使用按钮，而是使用链接到 PHP 文件的超链接，这些文件执行表“`courses`”的状态变量的切换。所以我们也需要创建这些文件。
