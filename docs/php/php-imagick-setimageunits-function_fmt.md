...
此函数接受单个参数 `$units`，该参数用于指定要为图像设置的单位。

## 分辨率常量

`Imagick::setImageUnits()` 函数设置图像单位，如下所述：
...
下面的程序说明 PHP 中的 `Imagick::setImageUnits()` 函数。
...
[http://php.net/manual/en/imagick.setimageunits.php](http://php.net/manual/en/imagick.setimageunits.php)