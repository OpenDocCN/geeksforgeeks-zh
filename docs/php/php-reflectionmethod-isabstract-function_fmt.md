...
`ReflectionMethod::isAbstract()`函数是 PHP 中的一个内置函数，用于在指定的方法是抽象的时返回 `true`，否则返回 `false`。
...
如果指定的方法是抽象的，则此函数返回 `TRUE`，否则返回 `FALSE`。

### Program 1

以下程序说明 PHP 中的 `ReflectionMethod::isAbstract()`函数：
