# ArrayIterator::offsetExists() 函数

> 原文：[https://www.geeksforgeeks.org/php-arrayiterator-offsetexists-function/](https://www.geeksforgeeks.org/php-arrayiterator-offsetexists-function/)

`ArrayIterator::offsetExists()` 函数是 PHP 中的一个内置函数，用于检查给定索引处是否存在偏移量。
...
该函数接受单个参数 `$index`，保存索引值，检查偏移的存在。

## 返回值

如果存在偏移，该函数返回 `true`，否则返回 `false`。
...
[https://www.php.net/manual/en/arrayiterator.offsetexists.php](https://www.php.net/manual/en/arrayiterator.offsetexists.php)