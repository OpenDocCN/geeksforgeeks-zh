...
`intl_is_failure()` 函数是 PHP 中的一个内置函数，用于检查给定的错误代码是否指示失败。
...
此函数接受单个参数 `$error_code`，该参数是函数 `intl_get_error_code()`、`COLLATOR_GET_ERROR_CODE()` 返回的值。

## 返回值

如果代码指示失败，则返回 True，如果成功或出现警告，则返回 False。

下面的程序演示了 PHP 中的 `intl_is_failure()` 函数：
