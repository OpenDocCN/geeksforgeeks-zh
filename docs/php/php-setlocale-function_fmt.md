...
它返回新的当前区域设置，如果您的平台上未实现区域设置功能、指定的区域设置不存在或类别名称无效，则返回 `False`。

## 参数

此函数接受上述两个参数，如下所述：

*   **category**：它是一个名为常量的整数，指定受区域设置影响的函数类别：
    *   `LC_ALL` - 适用于以下所有内容
    *   `LC_COLLATE` - 用于字符串比较
    *   `LC_CTYPE` - 用于字符分类和转换
    ...
    *   `LC_Messages` - 用于系统响应
*   **locale**：通常是指定地区所需的区域设置数组。
    *   如果 `locale` 为 `NULL` 或空字符串 - 区域设置名称将从与上述类别同名的环境变量的值或从“lang”设置。
    *   如果 `locale` 是 `"0"` - 区域设置不受影响，则只返回当前设置。
    *   如果 `locale` 是数组 - 区域设置不受影响，则只返回当前设置。
...
**引用：** [https://www.php.net/manual/en/function.setlocale.php](https://www.php.net/manual/en/function.setlocale.php)