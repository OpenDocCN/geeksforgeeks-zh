# PHP Imagick matteFloodfillImage() 函数
...
`Imagick::matteFloodfillImage()` 函数是 PHP 中的一个内置函数，用于更改颜色的透明度值。此函数用于更改与目标匹配且是其直接邻居的任何像素的透明度值。
...
下面的程序演示了 PHP 中的 `Imagick::matteFloodfillImage()` 函数：
