...
> 原文: [https://www.geeksforgeeks.org/deleting-all-files-from-a-folder-using-php/](https://www.geeksforgeeks.org/deleting-all-files-from-a-folder-using-php/)

在 PHP 中，可以使用各种方法和内置方法删除文件夹中的文件，例如 `unlink()`、`DirectoryIterator` 和 `RecursiveDirectoryIterator`。下面解释其中一些方法：
...
*   使用 `RecursiveDirectoryIterator` 和 `RecursiveIteratorIterator` 生成给定文件夹目录内所有目录层次结构的列表。
*   `FilesystemIterator::SKIP_DOTS` 用于生成文件列表时忽略点。
*   `RecursiveIteratorIterator::CHILD_FIRST` 用于选择根目录下存在的文件。
