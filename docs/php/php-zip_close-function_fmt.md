...
函数的作用是：关闭由 `zip_open()` 函数打开的 zip 存档文件。`zip_close()` 会导致流关闭，并断开与相应 Zip 存档的连接。由 `zip_open()` 函数打开的 zip 资源作为参数发送给 `zip_close()` 函数，并且不返回值。
...
`zip_close()` 函数接受单个参数 `$zip_file`。这是一个必选参数，指定要关闭的 zip 文件资源。
...
*   必须首先使用 PHP `zip_open()` 函数打开要关闭的 zip 归档文件，否则 PHP `zip_close()` 函数会生成 PHP 警告。
*   如果压缩档案无效，则返回 `ER_OPEN` 错误。
*   如果压缩存档为空，则返回 `ER_NOZIP` 错误。

## 示例

下面的程序演示了 PHP 中的 `zip_close()` 函数：
