# PHP ImagickDraw::steganoImage() 函数
...
`ImagickDraw::steganoImage()` 函数是 PHP 中的一个内置函数，用于在图像中隐藏数字水印。水印也可以是简单的文本或图像。通常，彩色图像会因此而失真。隐藏的水印可以稍后恢复以证明图像的真实性。偏移定义图像中隐藏水印的开始位置。在恢复图像时需要偏移量。
