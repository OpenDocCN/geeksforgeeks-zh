...
此函数接受两个 GMP 编号 `$num1` 和 `$num2` 作为上述语法中显示的强制参数。这些参数可以是 PHP 5.6 版和更高版本中的 GMP 对象，也可以传递数字字符串，前提是可以将该字符串转换为数字。

## 返回值

此函数返回一个正 GMP 数，它是 `$num1` 和 `$num2` 的 [XOR](http://php.net/manual/en/class.gmp.php)。
...
当作为 GMP 编号的数字字符串作为参数传递时，该程序计算两个数字的异或。
