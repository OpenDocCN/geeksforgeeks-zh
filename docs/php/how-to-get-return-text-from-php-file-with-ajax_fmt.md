...
> 原文：[https://www.geeksforgeeks.org/how-to-get-return-text-from-php-file-with-ajax/](https://www.geeksforgeeks.org/how-to-get-return-text-from-php-file-with-ajax/)
...
接下来，我们检查了状态代码是否等于 200，这意味着请求成功了。最后，我们获取存储在 `XMLHttpRequest` 对象的 `responseText` 属性中的响应。设置侦听器后，我们通过调用 `XMLHttpRequest` 对象的 `open` 方法来发起请求。然后调用 `XMLHttpRequest` 对象的 `send` 方法，该方法实际上是将请求发送到服务器。当服务器响应时，您可以看到显示 PHP 文件响应的文本。
