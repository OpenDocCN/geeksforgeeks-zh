...
`xml_get_current_byte_index()` 函数是 PHP 中的内置函数，用于返回 XML 解析器的字节索引。
...
## 参数
此函数接受必需的单个参数 `$parser`。此参数指定要使用的 XML 解析器。

## 返回值
此函数成功时返回指定解析器的当前字节索引，失败时返回 `False`。
...
## 参考
[https://www.php.net/manual/en/function.xml-get-current-byte-index.php](https://www.php.net/manual/en/function.xml-get-current-byte-index.php)