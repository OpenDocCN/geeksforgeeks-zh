...
> 原文：`https://www.geeksforgeeks.org/php-bcpow-function/`
...
```php
string bcpow ( string $base , string $exponent , int $scaleVal )
```

## 参数

该函数接受三个参数，如上面的语法所示，解释如下：

*   `$base`：此参数为字符串类型，代表将提升功率的基数。此参数是必需的。
*   `$exponent`：该参数为字符串类型，代表指数。此参数是必需的。
*   `$scaleVal`：此参数为 int 类型，可选。该参数表示基数`$base`的`$exponent`次方结果中小数点后出现的位数。它的默认值为零。

## 返回值

该函数将`$base`的`$exponent`次方结果作为字符串返回。
...
T2`http://php.net/manual/en/function.bcpow.php`