...
`DOMDocumentFragment::appendXML()`函数是 PHP 中的一个内置函数，用于将原始 XML 数据附加到 `DOMDocumentFragment`。
...
此函数接受单个参数`$data`，该参数保存要追加的 XML。

## 返回值

此函数成功时返回 `TRUE`，失败时返回 `FALSE`。

下面给出的程序演示了 PHP 中的`DOMDocumentFragment::appendXML()`函数：
