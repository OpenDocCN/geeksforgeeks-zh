...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-a-pdf-document-to-a-preview-image-in-php/](https://www.geeksforgeeks.org/how-to-convert-a-pdf-document-to-a-preview-image-in-php/)
...
*   `-sDEVICE`：设置图像的输出文件格式。
*   `-dTextAlphaBits`，`-dGraphicsAlphaBits`：设置合成图像的抗锯齿。允许的值是 1、2 和 4。
*   `-r{NUM}`：设置图像的分辨率（以 dpi 为单位）。
*   `-dFirstPage`，`-dLastPage`：设置需要渲染的文档的第一页和最后一页。
