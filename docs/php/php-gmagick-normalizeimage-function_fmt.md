...
`Gmagick::normalizeimage()` 函数是 PHP 中的一个内置函数，用于通过调整像素的颜色以跨越整个颜色范围来增强彩色图像的对比度。
...
以下程序说明了 PHP 中的 `Gmagick::normalizeimage()` 函数：
...
[http://php.net/manual/en/gmagick.normalizeimage.php](http://php.net/manual/en/gmagick.normalizeimage.php)