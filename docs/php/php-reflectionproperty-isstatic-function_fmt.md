...
`ReflectionProperty::isStatic()` 函数是 PHP 中的一个内置函数，用于在指定的属性为静态属性时返回 `True`，否则返回 `False`。
...
如果指定的属性是静态的，则此函数返回 `TRUE`，否则返回 `FALSE`。
...
[https://secure.php.net/manual/en/reflectionproperty.isstatic.php](https://secure.php.net/manual/en/reflectionproperty.isstatic.php)