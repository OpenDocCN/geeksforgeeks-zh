...
`addcslashes()` 函数是 PHP 中的内置函数。`addcslashes()` 函数用于在给定字符串中的某些指定字符前添加反斜杠。
...
该函数返回一个转义字符串，即输入字符串 `$string`，在 `$characters` 之前添加反斜杠。
...
下面的程序说明了 PHP 中的 `addcslashes()` 函数:
...
[http://php.net/manual/en/function.addcslashes.php](http://php.net/manual/en/function.addcslashes.php)