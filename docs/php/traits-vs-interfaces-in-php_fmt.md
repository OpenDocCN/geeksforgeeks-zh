...
`trait` 根本不是界面。特性可以定义静态成员和静态方法。它帮助开发人员在不同类层次的几个独立类中自由重用方法。特性降低了复杂性，并避免了与多重继承和混合相关的问题。注意 PHP 不允许多重继承。因此，通过允许我们在多个类中重用相同的功能，`Traits` 被用来填补这个空白。
...
它指定了类必须实现的所有此类方法的列表。使用关键字 `interface` 实现与类相同的接口。它可以使用 `extends` 运算符扩展接口。`Interface` 中的所有方法都是抽象方法，可以有自己的常量。有一个具体的类概念，它是一个实现接口的类，该接口必须实现所有具有相同名称和签名的方法。
界面中的所有方法必须具有公共访问级别。
