...
与静态、父等类上下文中使用的其他保留关键字不同，`$this` 不需要用美元符号（`$`）声明。这是因为在 PHP 中，`$this` 被当作伪变量。

在 PHP 中，`$this` 被声明为一个变量声明（带有 `$` 符号），即使它是保留关键字。更具体地说，`$this` 是一个特殊的只读变量，它没有在代码的任何地方声明，它代表一个根据程序执行的上下文而变化的值。

## 示例 2

使用 `$this` 关键字更新特定对象的变量值的程序。
...
在本例中，`$this` 关键字在静态方法的上下文中调用非静态方法时变为“未定义”。
