...
此函数将返回从`min`到`getrandmax()`的随机整数值，包括最大值。

## 注意：

如果未指定最小值和最大值，则默认值分别为 0 和`getrandmax()`。

## 示例：

在本例中，我们将使用`rand()`函数，该函数将返回一个介于 0 和`getrandmax()`之间的随机整数。`rand(15,35)`将返回范围[15，35]内的随机整数。
