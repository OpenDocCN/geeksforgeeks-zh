...
`Gmagick::labelimage()` 函数是 PHP 中的一个内置函数，用于标记图像。标签只是贴在图像上的字符串，稍后可以从图像中提取回来。此标签在图像本身上不可见，但您可以使用 `Annotate` 函数执行此操作。
...
此函数接受保存图像标签的单个参数 `$label`。

## 返回值

此函数返回带有标签的 `Gmagick` 对象。
...
[https://www.php.net/manual/en/gmagick.labelimage.php](https://www.php.net/manual/en/gmagick.labelimage.php)