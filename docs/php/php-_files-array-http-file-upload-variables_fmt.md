...
> 原文: [https://www.geeksforgeeks.org/php-_files-array-http-file-upload-variables/](https://www.geeksforgeeks.org/php-_files-array-http-file-upload-variables/)
...
添加 HTML 代码后跟 PHP 脚本不同的文件。让我们制作一个上传文件的 HTML 表单 `index.html`。
...
现在，是时候编写一个能够处理文件上传系统的 PHP 脚本了。`file-upload-manager.php`
...
在上面的脚本中，一旦我们提交了表单，稍后我们就可以通过一个 PHP 超全局关联数组 `$_FILES` 访问信息。除了使用 `$_FILES` 数组之外，许多内置函数也发挥了重要作用。在我们完成上传文件后，在脚本中我们将检查服务器的请求方法，如果是 `POST`，那么它将继续，否则系统将抛出一个错误。稍后，我们访问了 `$_FILES` 数组来获取文件名、文件大小和文件类型。一旦我们获得了这些信息，使用 [`echo`](https://www.geeksforgeeks.org/php-echo-print/) 打印文件的信息。
