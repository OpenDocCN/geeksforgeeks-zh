...
> 原文：[https://www.geeksforgeeks.org/how-to-get-the-position-of-character-in-a-string-in-php/](https://www.geeksforgeeks.org/how-to-get-the-position-of-character-in-a-string-in-php/)

在本文中，我们将获得 PHP 中给定字符串中字符的位置。字符串是一组字符。我们将通过使用 [`strpos()` 函数](https://www.geeksforgeeks.org/php-strpos-stripos-functions/) 来获取字符串中字符的位置。
...
*   `string`（必选）：这个参数是指我们需要在其中搜索所需字符串出现的原始字符串。
*   `character`（必选）：这个参数是指我们需要搜索的字符串。
