...
> 原文: [https://www.geeksforgeeks.org/how-to-get-numeric-index-of-associative-array-in-php/](https://www.geeksforgeeks.org/how-to-get-numeric-index-of-associative-array-in-php/)

在 PHP 中，我们可以使用 `=>` 符号将名称/标签与每个数组元素相关联。这非常有帮助，因为很容易记住元素，因为每个元素都由标签而不是索引值来表示。

## 使用 `array_keys()` 函数

`array_keys()` 函数是 PHP 中的一个内置函数，用于返回数组的所有键或键的子集。
...
使用 `array_keys()` 函数获取关联数组的数值索引的程序。
