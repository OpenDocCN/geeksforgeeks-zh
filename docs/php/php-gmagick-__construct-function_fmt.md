...
`Gmagick::__construct()` 函数是 PHP 中的内置函数，用于创建 `Gmagick` 对象。可以使用这个新创建的 `Gmagick` 对象进行进一步的 Gmagick 操作。
...
此函数接受保存文件名的单个参数 `$filename`。

## 返回值

如果成功，此函数将返回一个新的 `Gmagick` 对象。
...
[https://www.php.net/manual/en/gmagick.construct.php](https://www.php.net/manual/en/gmagick.construct.php)