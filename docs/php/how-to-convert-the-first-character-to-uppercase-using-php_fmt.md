...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-the-first-character-to-uppercase-using-php/](https://www.geeksforgeeks.org/how-to-convert-the-first-character-to-uppercase-using-php/) 将第一个字符转换为大写
...
**第二步：** 从字符串中提取的字符可以转换为大写，使用 `ord()` 函数，然后从其 ASCII 值中减去 32。然后使用 `chr()` 方法将大写字符的结果 ASCII 值转换为字符。

**第三步：** `substr()` 方法用于从索引的开始到结束返回原始字符串的子字符串。
