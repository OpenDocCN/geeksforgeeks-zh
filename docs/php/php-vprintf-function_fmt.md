...
PHP 中 `vprintf()` 函数是一个内置函数，用于将数组值显示为格式化字符串。根据格式将数组值显示为格式化字符串。它的工作方式类似于 [`printf()`](https://www.geeksforgeeks.org/php-format-specifiers/)，但接受参数数组，而不是变量数参数。成功时返回输出字符串的长度。
...
本程序将使用 `vprintf` 函数显示 `%b`、`%d`、`%u`、`%x`、`%X`、`%f`、`%F`、`%o` 格式的用法。
...
本程序将使用 `vprintf` 函数显示 `%c` 和 `%s` 格式的用法。
...
本程序将使用 `vprintf` 函数显示 `%e`、`%g`、`%E`、`%G` 格式的用法。
...
在本程序中，所有四个变量将使用 `vprintf` 函数分别打印 10、20、30、40 空格。
