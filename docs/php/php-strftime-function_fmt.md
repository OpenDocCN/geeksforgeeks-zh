...
*   `$timestamp`：可选的 `$timestamp` 参数是一个整数 Unix 时间戳，如果未提供时间戳，则默认为当前本地时间。 换句话说，它默认为 `time()` 的值。

## 返回值

它使用给定的 `$timeStamp` 返回格式化为 `$format` 的字符串(如果明确提到，否则需要默认时间)。 月份和工作日名称以及其他与语言相关的字符串与使用 `setlocale()` 设置的当前区域设置相关。
