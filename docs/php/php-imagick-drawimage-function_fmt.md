...
`Imagick::drawImage()` 函数是 PHP 中的一个内置函数，用于在 `Imagick` 对象上呈现 `ImagickDraw` 对象。我们借助 `ImagickDraw` 方法设置图像矩阵、参数和绘制图像的边界，然后使用 `Imagick::drawImage()` 函数进行渲染。
...
此函数接受单个参数 `$draw`，该参数保存 `ImagickDraw` 实例以获取有关要在屏幕上呈现的图像的详细信息。
...
[https://www.php.net/manual/en/imagick.drawimage.php](https://www.php.net/manual/en/imagick.drawimage.php)