...
`ImagickDraw::getFontStyle()`函数是 PHP 中的一个内置函数，用于获取使用文本进行批注时使用的字体样式。
...
此函数返回一个整数值，该值对应于[样式常量](https://www.php.net/manual/en/imagick.constants.php/#imagick.constants.style-normal)之一，或者在未设置样式时返回 0。
样式常量列表如下：
