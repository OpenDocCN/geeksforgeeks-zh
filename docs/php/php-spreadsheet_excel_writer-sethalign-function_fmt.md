...
`setHAlign()` 函数是 `Spreadsheet_Excel_Writer` 中的内置函数，用于设置电子表格的单元格对齐方式。
...
此函数接受作为 `$location` 的单个参数，该参数接受文本放置位置为 'Left'、'Center'、'Right'、'Fill'、'Justify'、'Merge'、'EQUAL_SPACE'。

## 返回值

此函数成功时返回 `TRUE`，失败时返回 `PEAR_ERROR`。
...
[https://pear.php.net/manual/en/package.fileformats.spreadsheet-excel-writer.spreadsheet-excel-writer-format.sethalign.php](https://pear.php.net/manual/en/package.fileformats.spreadsheet-excel-writer.spreadsheet-excel-writer-format.sethalign.php)