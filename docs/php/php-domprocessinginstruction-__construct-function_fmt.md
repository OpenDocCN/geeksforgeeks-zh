# DOMProcessingInstruction::__construct() 函数
...
`DOMProcessingInstruction::__construct()` 函数是 PHP 中的一个内置函数，它使用一个新的只读 `DOMProcessingInstruction` 对象。要创建可写节点，请使用 `DOMDocument::createProcessingInstruction()`。
