...
`ReflectionMethod::export()` 函数是 PHP 中的一个内置函数，用于在返回参数设置为 `true` 时以字符串形式返回导出，否则返回 `NULL`。
...
*   `class`：这是初始化的类的名称。
*   `name`：这是方法的名称。
*   `return`：其值为 `TRUE` 或 `FALSE`。 使用 `TRUE` 将返回导出，而使用 `FALSE` 则相反。

## 返回值

如果返回参数已设置为 `true`，则此函数以字符串形式返回导出，否则返回 `NULL`。

以下程序说明 PHP 中的 `ReflectionMethod::export()` 函数：
...
[https://www.php.net/manual/en/reflectionmethod.export.php](https://www.php.net/manual/en/reflectionmethod.export.php)