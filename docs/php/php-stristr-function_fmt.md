...
`stristr()` 函数是 PHP 中的内置函数。它搜索一个字符串在另一个字符串中的第一个匹配项，并从前者在后者中的第一个匹配项开始显示后者的一部分（如果指定，则在此之前）。此函数是不区分大小写的。
...
*   `$before`：为可选参数。它指定一个布尔值，其默认值为 `False`。如果设置为 `true`，则返回第一次出现搜索参数之前的字符串部分。
