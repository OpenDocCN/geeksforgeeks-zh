...
```php
public Ds\Set::filter( $callback )
```

## 参数：
此函数接受单个参数`$callback`，该参数是可选的，如果应该包含该值，则返回 True，否则返回 False。

## 返回值：
此函数返回一个新的集合，其中包含回调返回 True 的所有值，如果未提供回调，则包含转换为 True 的所有值。
...
## 引用：
[https://www.php.net/manual/en/ds-set.filter.php](https://www.php.net/manual/en/ds-set.filter.php)