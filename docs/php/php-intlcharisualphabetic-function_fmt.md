...
此函数接受单个参数 `$codepoint`，该参数是必需的。 输入参数是整数或字符，编码为 `UTF-8` 字符串。

## 返回值

如果 `$codepoint` 是字母 Unicode 字符，则返回 `True`，否则返回 `False`。
...
[http://php.net/manual/en/intlchar.isualphabetic.php](http://php.net/manual/en/intlchar.isualphabetic.php)