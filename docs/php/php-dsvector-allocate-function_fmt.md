...
`Ds\Vector::allocate()`函数是 PHP 中的一个内置函数，用于为所需的容量分配足够的内存。它提供用于分配空间的向量的自定义大小。
...
此函数接受单个参数`$capacity`，该参数保存要分配的空间。
...
下面的程序说明了 PHP 中的`Ds\Vector::allocate()`函数：
...
[http://php.net/manual/en/ds-vector.allocate.php](http://php.net/manual/en/ds-vector.allocate.php)