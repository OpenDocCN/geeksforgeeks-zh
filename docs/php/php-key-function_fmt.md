...
> 原文：[https://www.geeksforgeeks.org/php-key-function/](https://www.geeksforgeeks.org/php-key-function/)
...
此函数接受单个参数`$array`。它是我们要为其查找由内部指针指向的当前元素的数组。

## 返回值

返回给定数组当前元素的索引。如果输入数组为空，则`key()`函数将返回`NULL`。
