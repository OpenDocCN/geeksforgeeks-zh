...
`Gmagick::getimagecompose()` 函数是 PHP 中的一个内置函数，用于获取与图像相关联的复合运算符。合成运算符决定用于图像合成的方法。复合运算符可以是给定的[复合运算符常数](https://www.php.net/manual/en/gmagick.constants.php#gmagick.constants.composite-default)中的任何一个。
...
此函数返回包含 Compose 运算符的整数值。

## 异常

此函数在出错时引发 `GmagickException`。

下面给出的程序演示了 PHP 中的 `Gmagick::getimagecompose()` 函数：
