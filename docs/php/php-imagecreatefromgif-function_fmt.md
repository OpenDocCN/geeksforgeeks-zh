...
`imagecreatefromgif()`函数是 PHP 中的一个内置函数，用于从 GIF 文件或 URL 的给定部分创建新图像。此外，这个图像可以在程序中处理。此函数仅加载动画的第一帧。
...
下面的示例说明了 PHP 中的`imagecreatefromgif()`函数：
...
$im = imagecreatefromgif('https://media.geeksforgeeks.org/wp-content/uploads/20191117145951/g4gnaimation1.gif');
...
?>
```
这将把第一帧保存为同一文件夹中的`firstframe.jpg`。
