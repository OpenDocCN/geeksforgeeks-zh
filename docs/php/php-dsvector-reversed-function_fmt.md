...
`Ds\Vector::reversed()` 函数是 PHP 中的一个内置函数，用于在将原始向量的元素复制到副本后反转向量的元素。
...
此函数以相反的顺序返回原始向量的副本。此外，原始向量也不会有任何效果。

下面的程序演示了 PHP 中的 `Ds\Vector::reversed()` 函数：
...
[http://php.net/manual/en/ds-vector.reversed.php](http://php.net/manual/en/ds-vector.reversed.php)