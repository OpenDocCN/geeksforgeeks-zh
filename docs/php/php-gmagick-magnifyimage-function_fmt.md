...
`Gmagick::magnifyimage()` 函数是 PHP 中的一个内置函数，用于将图像按比例缩放到 2 倍。此函数用于将图像缩放为其原始大小的两倍。
...
此函数返回放大后的 `Gmagick` 对象。

## 错误/异常

此函数在出错时引发 `GmagickException`。

下面的程序说明了 PHP 中的 `Gmagick::magnifyimage()` 函数。
