...
`Reflection::getShortName()` 函数是 PHP 中的一个内置函数，用于返回指定类的短名称，即没有命名空间的部分。
...
此函数返回指定类的短名称，即没有命名空间的部分。

下面的程序说明了 PHP 中的 `Reflection::getShortName()` 函数：
...
[https://www.php.net/manual/en/reflectionclass.getshortname.php](https://www.php.net/manual/en/reflectionclass.getshortname.php)