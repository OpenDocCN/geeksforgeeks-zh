...
`array_multisort()`是 PHP 中的一个内置函数，用于一次对多个数组进行排序，或者对每个维度都有一个多维数组进行排序。
...
数组一般取一个参数，就是需要排序的数组。但是除此之外，这个函数还可以带两个可选参数 `sorting_order` 和 `sorting_type`。

1.  `$array1`：此参数指定我们要排序的数组。
2.  `sorting_order`：此参数指定使用的顺序，即升序或降序。该参数的默认值为 `SORT_ASC`。也就是按升序排序。为了按降序排序，我们必须将此参数设置为 `SORT_DESC`。
3.  `sorting_type`：此参数指定数组的排序选项，具体如下：
    *   `SORT_REGULAR`：定期比较元素（标准 ASCII）。
    *   `SORT_NUMERIC`：将元素作为数值进行比较。
    *   `SORT_STRING`：将元素作为字符串值进行比较。
    *   `SORT_LOCALE_STRING`：基于当前区域设置，将元素作为字符串进行比较。
...
`array_multisort()`函数返回一个布尔值。也就是说，它将在成功时返回 `true`，在失败时返回 `false`。

## 注意

如果比较时两个成员相等，那么它们在排序数组中的相对顺序是未定义的。
