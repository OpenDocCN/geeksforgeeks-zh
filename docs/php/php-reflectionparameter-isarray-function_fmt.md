# ReflectionParameter::isArray() 函数
...
`ReflectionParameter::isArray()` 函数是 PHP 中的内置函数，用于在指定参数为数组时返回 `TRUE`，否则返回 `FALSE`。
...
如果指定的参数是数组，则此函数返回 `TRUE`，否则返回 `FALSE`。

## 程序 1

以下程序说明 PHP 中的 `ReflectionParameter::isArray()` 函数：
...
[https://www.php.net/manual/en/reflectionparameter.isarray.php](https://www.php.net/manual/en/reflectionparameter.isarray.php)