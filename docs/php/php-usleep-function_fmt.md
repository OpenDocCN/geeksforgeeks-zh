...
PHP 中的 `usleep()` 函数接受一个参数 `microseconds`。这是一个必选参数，用于指定脚本执行的延迟时间。
...
1.  如果指定的微秒数为负数，则抛出错误。
2.  `usleep()` 函数调用消耗 CPU 周期，只有在必要时才能使用。`sleep()` 函数是一个更好的选择，因为它不占用 CPU 周期。
...
[http://php.net/manual/en/function.usleep.php](http://php.net/manual/en/function.usleep.php)