...
> Original: [https://www.geeksforgeeks.org/php-gmp_perfect_square-function/](https://www.geeksforgeeks.org/php-gmp_perfect-square-function/)
...
该函数接受一个 GMP 编号 `$num`。在 PHP 5.6 版和更高版本中，该参数可以是 GMP 对象，也可以传递数字字符串，前提是可以将该字符串转换为数字。

## 返回值

如果给定的数字 `$num` 是完美平方，则函数返回 `true`，否则返回 `false`。
...
下面的程序演示了当 GMP 编号作为参数传递时，`gmp_perfect_square()` 函数的工作原理。
...
下面的程序演示了将数字字符串作为参数传递时 `gmp_perfect_square()` 的工作原理。
