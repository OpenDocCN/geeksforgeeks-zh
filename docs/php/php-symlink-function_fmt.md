...
PHP 中的 `symlink()` 函数是一个内置函数，用于为已经存在的目标创建符号链接。它有助于为目标创建特定的名称链接。目标名称和链接名称作为参数发送给 `symlink()` 函数，成功时返回 `True`，失败时返回 `False`。`symlink()` 函数不提供 HTML 链接，而是文件系统中的链接。
...
1.  `target`：这是一个必选参数，用于指定必须创建其链接的目标。
2.  `link`：必选参数，指定链接名称。
...
下面的程序演示了 `symlink()` 函数。假设有一个名为 `gfg.txt` 的文件。
...
// creating a link using symlink() function
symlink($target_pointer, $link_name);
?>
```
