...
`ImagickPixel::getColorCount()` 函数是 PHP 中的一个内置函数，用于获取与像素颜色相关联的颜色计数。颜色计数是图像中与此 `ImagickPixel` 具有相同颜色的像素数。`getColorCount()` 似乎只适用于通过 `getImageHistogram()` 创建的 `ImagickPixel` 对象。
...
[https://www.php.net/manual/en/imagickpixel.getcolorcount.php](https://www.php.net/manual/en/imagickpixel.getcolorcount.php)