# DOMImplementation::createDocumentType()函数
...
`DOMImplementation::createDocumentType()`函数是 PHP 中的内置函数，用于创建空的 `DOMDocumentType` 对象。 实体声明和符号不可用。
...
*   `$publicId` (可选)：它指定外部子集公共标识符的限定名称。
*   `$systemId` (可选)：它指定外部子集系统标识符。

## 返回值

此函数返回 `ownerDocument` 设置为空的 `DOMDocumentType` 节点。

## 异常

如果命名空间有错误，则此函数抛出 `DOM_NAMESPACE_ERR`，由 `$qualifiedName` 确定。

## 示例程序

下面给出的程序说明了 PHP 中的 `DOMImplementation::createDocumentType()` 函数：
