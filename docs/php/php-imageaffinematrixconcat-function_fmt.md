...
图像仿射变换函数是 PHP 中内置的 GD 函数，用于连接两个仿射变换矩阵。使用 GD 的 `imageaffinematrixget()` 函数和 `img_affine_*` 常量得到仿射变换数组。
...
它返回串联的仿射变换矩阵，该矩阵有一个键为 0 到 5 的数组，成功时为浮点值，失败时为 `False`。

下面的程序演示了 PHP 中的 `imageaffinematrixconcat()` 函数：
