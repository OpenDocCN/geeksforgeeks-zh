...
> 原文：[https://www.geeksforgeeks.org/how-to-get-file-name-from-a-path-in-php/](https://www.geeksforgeeks.org/how-to-get-file-name-from-a-path-in-php/)
...
`basename()` 函数是一个内置函数，如果文件的路径是作为 `basename()` 函数的参数提供的，它将返回文件的基本名称。
...
`path` 是指定要检查的路径的必填字段。`suffix` 是指定文件扩展名的可选字段。如果文件名有这个文件扩展名，文件扩展名将不会显示。
