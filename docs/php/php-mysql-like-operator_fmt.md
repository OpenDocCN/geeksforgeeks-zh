...
在本文中，我们将在 Xampp 服务器中使用 `LIKE` 运算符和 SQL 来显示数据。这里我们将考虑学生地址数据库作为一个例子。
...
PHP 代表超文本预处理器。它被用作服务器端脚本语言，可以用 `xampp` 工具连接 `MySQL` 服务器。`MySQL` 是一种用于管理数据库的查询语言。

## LIKE 运算符

SQL 中的 `LIKE` 运算符在 `WHERE` 子句中用于搜索列中的指定模式。有两个通配符可以与 `LIKE` 运算符结合使用：
...
1.  创建数据库（命名数据库）并创建名为 `student_address` 的表。
2.  使用 `PHP` 将数据插入表中。
3.  编写 `PHP` 代码来执行类似的操作。
4.  观察结果。
...
访问 `localhost/form.php`

![](img/266b2209dcfc7decf81a2902d0d965d0.png)

### 步骤 4: 演示 LIKE 运算符 (form1.php)
