...
该函数接受单个参数 `$num`，该参数是一个 GMP 编号并返回其整数值。此参数可以是 PHP 5.6 版和更高版本中的 GMP 对象，也可以传递数字字符串，前提是可以将该字符串转换为数字。

## 返回值

此函数返回给定 GMP 编号的整数值 `$num`。
...
如果数字字符串作为整数传递，则返回相同的整数（高于 PHP 整数限制除外）。但如果传递 GMP 数字，它将返回 GMP 数字的整数值。下面的程序说明了 `gmp_intval()` 函数的用法。

### 程序 1

下面的程序演示了将数字字符串作为参数传递时 `gmp_intval()` 函数的工作方式。
...
下面的程序演示了将 GMP 编号作为参数传递时 `gmp_intval()` 的工作方式。
