...
*   `$index`: 此参数保存要插入元素的索引值。其值介于 `0 <= $index <= count` 之间。其中 `count` 表示序列中的元素数量。
*   `$values`: 此参数保存要插入的值。
...
[http://php.net/manual/en/ds-sequence.insert.php](http://php.net/manual/en/ds-sequence.insert.php)