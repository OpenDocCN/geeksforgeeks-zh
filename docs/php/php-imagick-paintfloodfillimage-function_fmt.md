# PHP Imagick paintFloodFillImage() 函数
...
`paintFloodFillImage()` 函数是 PHP 中的一个内置函数，用于更改与目标匹配且是其直接邻居的任何像素的颜色值。
...
**注意：** 此函数将替换为 [`Imagick::floodFillPaintImage()` 函数](https://www.geeksforgeeks.org/php-imagick-floodfillpaintimage-function/)。
...
下面的程序演示了 PHP 中的 `Imagick::paintFloodFillImage()` 函数：
...
$img->paintFloodFillImage('cyan', 1, 'white', 1, 1, false);
