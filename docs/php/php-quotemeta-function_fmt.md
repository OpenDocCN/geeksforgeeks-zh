...
`quotemeta()` 函数是 PHP 中的一个内置函数，它接受一个字符串作为参数，并返回一个字符串，该字符串在字符串中的一些预定义字符前面添加了反斜杠。
...
此函数只接受一个必选参数 `$string`。此参数指定要在上面提到的预定义字符前面添加反斜杠的字符串。

## 返回值

它通过在 `$string` 参数中的预定义字符前面添加反斜杠来返回字符串。
