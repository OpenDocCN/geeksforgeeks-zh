...
> 原文: [https://www.geeksforgeeks.org/how-to-remove-all-leading-zeros-in-a-string-in-php/](https://www.geeksforgeeks.org/how-to-remove-all-leading-zeros-in-a-string-in-php/)
...
`ltrim()` 函数用于删除字符串左侧的空格或其他字符（如果指定）。
...
首先将给定的字符串转换为数字，将字符串类型转换为 `int`，`int` 将自动删除所有前导零，然后再次将其类型转换为 `string`，使其再次成为 `string`。
