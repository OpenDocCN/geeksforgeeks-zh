...
`ImagickPixel::__construct()` 函数是 PHP 中的内置函数，用于构造 `ImagickPixel` 对象。如果指定了颜色，则构造对象，然后使用该颜色进行初始化。
...
此函数接受单个参数 `$color`，该参数是可选的，包含颜色。

## 返回值

成功时此函数返回 `ImagickPixel` 对象。
...
### 示例 3：结合 Imagick 和 ImagickDraw 使用
