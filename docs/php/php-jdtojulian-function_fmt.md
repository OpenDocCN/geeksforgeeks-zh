...
`jdtojulian()` 函数是 PHP 中的一个内置函数，用于将儒略日计数转换为儒略历日期。它将儒略日计数转换为包含月/日/年格式的儒略历日期的字符串。
...
**参数：**
此函数接受单个参数 `$julianday`，这是必需的。它包含以整数形式表示的朱利安日期。

**返回值：**
此函数以月/日/年格式的字符串形式返回儒略日期。
...
**引用：**
[http://php.net/manual/en/function.jdtojulian.php](http://php.net/manual/en/function.jdtojulian.php)