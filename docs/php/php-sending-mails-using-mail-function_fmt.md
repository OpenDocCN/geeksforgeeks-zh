...
*   `subject`：指定邮件的主题。
*   `message`：指定要发送的消息。
*   `additional_headers`（可选）：这是一个可选参数，可以创建多个标题元素，例如 `From`（指定发件人）、`CC`（指定抄送/抄送收件人）、`Bcc`（指定密件抄送/密送收件人）。**注意：**要添加多个标题参数，必须使用 `'\r\n'`。
*   `additional_parameters`（可选）：这是另一个可选参数，可以作为附加标头的扩展传递。这可以指定一组用作 `Sendmail_PATH` 配置设置的标志。
