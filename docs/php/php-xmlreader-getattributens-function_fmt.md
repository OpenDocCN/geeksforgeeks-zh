...
`XMLReader::getAttributeNs()`函数是 PHP 中的一个内置函数，用于根据名称和命名空间 URI 获取属性的值，如果属性不存在或不在元素节点上，则获取空字符串。
...
while ($XMLReader->read()) {
    if ($XMLReader->nodeType == XMLREADER::ELEMENT) {
        ...
        $value = $XMLReader->getAttributeNs("attrib", "my_namespace");
...
[https://www.php.net/manual/en/xmlreader.getattributens.php](https://www.php.net/manual/en/xmlreader.getattributens.php)