...
> 原文：[https://www.geeksforgeeks.org/load-csv-data-into-mysql-server-using-php/](https://www.geeksforgeeks.org/load-csv-data-into-mysql-server-using-php/)

在本文中，我们将使用 XAMPP 服务器中的 PHP 将 CSV 文件中的数据存储到 MySQL 数据库中，并在 web 浏览器中显示这些数据。

`逗号分隔值(CSV)` 是包含数据内容的文本文件。它有助于在类似表的结构中存储数据。CSV 文件以 `.csv` 扩展名存储。可以使用任何文本编辑器创建 CSV 文件，如 `记事本`、`Notepad++` 等。将内容添加到记事本中的文本文件后，使用 `.csv` 扩展名将其存储为 csv 文件。
...
?>
```
这里，`servername` 是 `localhost`，`username` 是你的用户名，默认是 `root`，`password` 默认为空。`databasename` 是你的数据库名称。
...
```
这里：
`filename` 用于指定要打开的文件或 URL。
`mode` 用于指定你对该文件/流所需的访问类型。

### 3. 获取 CSV 文件

我们可以使用 `fgetcsv()` 函数获取 CSV 文件。
