...
> 原文：[https://www.geeksforgeeks.org/php-functions/](https://www.geeksforgeeks.org/php-functions/)
...
*   **用户定义函数**：除了内置函数，PHP 允许我们创建我们自己的自定义函数，称为用户定义函数。使用它，我们可以创建我们自己的代码包，并在需要的地方通过简单地调用它来使用它。
...
PHP 允许我们为函数参数设置默认参数值。如果我们没有为具有默认值的参数传递任何实参，则 PHP 将在函数调用中使用此参数的默认设置值。
