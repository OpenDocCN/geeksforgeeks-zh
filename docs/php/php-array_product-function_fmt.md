...
函数只取一个参数 `$array`，指的是我们希望得到其元素乘积的输入数组。

## 返回值

`array_product()`函数根据数组元素的性质返回一个整数或浮点值。
...
当传递给 `array_product()` 函数的数组只包含整数值时，`array_product()` 函数返回一个整数值，等于传递给它的数组所有元素的乘积。
...
当传递给 `array_product()` 函数的数组同时包含整数和浮点值时，`array_product()` 函数返回一个浮点值，等于传递给它的数组所有元素的乘积。
...
[http://php.net/manual/en/function.array-product.php](http://php.net/manual/en/function.array-product.php)