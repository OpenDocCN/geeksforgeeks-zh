# ReflectionExtension::isPersistent() 函数
...
`ReflectionExtension::isPersistent()` 函数是 PHP 中的内置函数，用于在指定的扩展是持久的时返回 `TRUE`，否则返回 `FALSE`。
...
如果指定的扩展是持久的，则此函数返回 `TRUE`，否则返回 `FALSE`。
...
[https://www.php.net/manual/en/reflectionextension.ispersistent.php](https://www.php.net/manual/en/reflectionextension.ispersistent.php)