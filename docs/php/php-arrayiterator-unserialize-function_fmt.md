...
`ArrayIterator::unserialize()` 函数是 PHP 中的一个内置函数，用于对序列化对象进行非序列化。
...
这个函数接受单个参数 `$serialized`，它保存序列化数组迭代器对象。

## 返回值

该函数返回未序列化对象的序列化数组运算符对象。
...
[https://www.php.net/manual/en/arrayiterator.unserialize.php](https://www.php.net/manual/en/arrayiterator.unserialize.php)