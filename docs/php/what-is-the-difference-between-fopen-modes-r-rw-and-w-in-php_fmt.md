...
> 原文：[https://www.geeksforgeeks.org/what-is-the-difference-between-fopen-modes-r-rw-and-w-in-php/](https://www.geeksforgeeks.org/what-is-the-difference-between-fopen-modes-r-rw-and-w-in-php/)
...
```php
$variable_name = fopen($file_source_path, $access_mode)
```
...
*   它检查参数的第一个字符，即 `mode[0]`。根据它是 `r`、`w`、`a` 还是 `x`，识别相应的模式。
*   它会在模式参数中查找“+”的存在。如果存在，它会设置适当的标志。

因此 **`r+`** 和 **`w+`** 打开文件和放置文件指针的方式是有区别的。**`r+`** 和 **`rw+`** 相同。PHP 只关心字符串以 **`r`** 开头，有一个 **`+`**。**`w`** 在 **`rw+`** 中被忽略。因此，它们的工作原理是一样的。
