...
> 原文：[https://www.geeksforgeeks.org/how-to-parse-a-json-file-in-php/](https://www.geeksforgeeks.org/how-to-parse-a-json-file-in-php/)
...
创建一个 JSON 文件，保存为`my_data.json`，我们在文件中取了*学生*的数据。内容如下。
...
使用`file_get_contents()`函数将 JSON 文件读入 PHP。该函数用于将文件读入 PHP 代码。
...
*   使用`json_decode()`函数解码到 json 文件成数组显示出来。
