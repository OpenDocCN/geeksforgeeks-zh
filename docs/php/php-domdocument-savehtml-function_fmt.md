# DOMDocument::saveHTML() 函数

> 原文：[https://www.geeksforgeeks.org/php-domdocument-savehtml-function/](https://www.geeksforgeeks.org/php-domdocument-savehtml-function/)

`DOMDocument::saveHTML()` 函数是 PHP 中的一个内置函数，用于从 DOM 表示创建 HTML 文档。通常在从头构建 DOM 文档后使用。
...
此函数接受单个可选参数 `$node`，用于输出文档的子集。

## 返回值

此函数成功时返回 HTML 文档，失败时返回 `False`。
