...
`Ds\Deque::pop()` 函数是 PHP 中的一个内置函数，用于从 Deque 中删除最后一个元素（如果 Deque 不为空）并返回它。如果 Deque 为空，则抛出异常。
...
此函数返回从 Deque 中删除的最后一个元素。

## 程序示例

以下程序说明了 PHP 中的 `Ds\Deque::pop()` 函数：
...
[http://php.net/manual/en/ds-deque.pop.php](http://php.net/manual/en/ds-deque.pop.php)