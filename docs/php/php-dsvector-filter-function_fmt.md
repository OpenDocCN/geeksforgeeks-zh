...
> 原文：[https://www.geeksforgeeks.org/php-dsvector-filter-function/](https://www.geeksforgeeks.org/php-dsvector-filter-function/)
...
```php
public Ds\Vector::filter( $callback )
```

## 参数

此函数接受单个参数 `$callback`，如果要包括向量中的元素，则返回 `TRUE`，否则返回 `FALSE`。

## 返回值

此函数返回包含根据可调用函数过滤的所有元素的向量。
...
[http://php.net/manual/en/ds-vector.filter.php](http://php.net/manual/en/ds-vector.filter.php)