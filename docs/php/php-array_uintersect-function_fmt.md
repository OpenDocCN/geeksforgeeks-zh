...
> 原文：`https://www.geeksforgeeks.org/php-array_uintersect-function/`

`array_uintersect()` 是 PHP 中的一个内置函数，用于根据值计算两个或多个数组的交集。在用户定义函数的帮助下，将第一个数组值与所有其他数组进行比较，并返回匹配项。
...
该函数接受两种类型的参数。一个是数组列表，另一个是用户定义的函数。
...
该函数返回另一个数组，该数组包含第一个数组中作为参数传递的所有其他数组中存在的所有元素。如果没有元素匹配，则返回空数组。
...
演示 `array_uintersect()` 函数工作原理的 PHP 程序。
...
用三个数组演示 `array_uintersect()` 函数工作的 PHP 程序。
