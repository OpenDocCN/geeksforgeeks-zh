...
`Imagick::setImageFormat()` 函数是 PHP 中的内置函数，用于设置序列中特定图像的格式。
...
此函数接受单个参数 `$format`，该参数保存图像格式（如 PNG、SVG 等）的字符串值。格式支持完全取决于 Imagick 的安装。
...
[https://www.php.net/manual/en/imagick.setimageformat.php](https://www.php.net/manual/en/imagick.setimageformat.php)