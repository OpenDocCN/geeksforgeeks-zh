...
> 原文: [https://www.geeksforgeeks.org/how-to-push-a-value-based-on-matched-value-in-php/](https://www.geeksforgeeks.org/how-to-push-a-value-based-on-matched-value-in-php/)

在 PHP 中，为了在匹配时推进数组中的值，我们需要一个带有键和值对的数组。包含键和值对的数组称为[关联数组](https://www.geeksforgeeks.org/associative-arrays-in-php/)。

## 方法

创建两个不同的关联数组，命名为 `$array1` 和 `$array2`。然后将 `$array1` 的值与 `$array2` 的键进行比较，如果我们得到匹配，那么我们将按如下方式推送静态键和值对。
...
上述程序会将 `$array2` 中的值推送到 `$array3` 中，`$array3` 为空。
...
在上面的程序中，`$student` 数组中没有 `dept4` 的匹配项，因此它不会显示在输出部分。