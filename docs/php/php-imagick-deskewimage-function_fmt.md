...
`Imagick::deskewImage()`函数是 PHP 中的一个内置函数，用于对图像进行去倾斜或从图像中移除倾斜。如果 `Imagick` 针对 Imagick 版本 6.4.5 或更高版本进行编译，则此功能可用。当相机未对齐或纸张未放置在平面上时，会出现`倾斜`。
...
此函数接受单个参数`$threshold`，该参数保存去歪斜阈值值。
...
[https://www.php.net/manual/en/imagick.deskewimage.php](https://www.php.net/manual/en/imagick.deskewimage.php)