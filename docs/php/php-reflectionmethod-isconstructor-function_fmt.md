...
`ReflectionMethod::isConstructor()` 函数是 PHP 中的内置函数，用于在指定的方法是构造函数时返回 `TRUE`，否则返回 `FALSE`。
...
如果指定的方法是构造函数，则此函数返回 `TRUE`，否则返回 `FALSE`。
...
[https://www.php.net/manual/en/reflectionmethod.isconstructor.php](https://www.php.net/manual/en/reflectionmethod.isconstructor.php)