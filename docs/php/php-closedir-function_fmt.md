...
PHP 中的 `closedir()` 函数只接受一个参数，如下所述。

*   `$dir_handle`：这是一个可选参数，指定之前由 `opendir()` 打开的目录句柄资源。如果未指定此参数，将假定并关闭由 `opendir()` 打开的最后一个链接。
...
2.  如果未指定 `$dir_handle` 参数，`opendir()` 打开的最后一个链接将被 `closedir()` 函数假定并关闭。
...
[http://php.net/manual/en/function.closedir.php](http://php.net/manual/en/function.closedir.php)