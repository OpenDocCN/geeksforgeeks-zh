...
`imagesavealpha()` 函数是 PHP 中的一个内置函数，用于设置保存 PNG 图像时是否保留完整的 Alpha 通道信息。Alpha 通道指示图像是完全透明的还是不透明的。必须使用 `imagealphablending($im, false)` 禁用 Alpha 混合，才能首先保留 Alpha 通道。
...
*   `$image`: 指定要处理的图像资源。
*   `$save_flag`: 指定是否保存 Alpha 通道。

## 返回值

此函数成功时返回 `TRUE`，失败时返回 `FALSE`。

下面的示例说明了 PHP 中的 `imagesavealpha()` 函数：

## 示例 1：启用保存 Alpha 信息

在此示例中，我们将启用保存 Alpha 信息。
...
[https://www.php.net/manual/en/function.imagesavealpha.php](https://www.php.net/manual/en/function.imagesavealpha.php)