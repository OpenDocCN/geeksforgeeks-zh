...
`Imagick::getImageProperties()` 函数是 PHP 中的一个内置函数，用于获取图像属性。
...
*   `$pattern`: 指定属性名称的模式。默认值是 `*`，返回所有属性。
*   `$includes_values`: 指定是否仅返回属性名称。其默认值为 `true`。如果为 `false`，则仅返回属性名称。

## 返回值

此函数返回包含图像属性或仅包含属性名称的数组。
