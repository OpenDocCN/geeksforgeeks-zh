...
PHP 中的 `fputs()` 函数是一个内置函数，用于写入打开的文件。
`fputs()` 函数在文件末尾或达到作为参数传递的指定长度时（以先到者为准）停止。
必须写入的文件、字符串和长度作为参数发送给 `fputs()` 函数，它在成功时返回写入的字节数，在失败时返回 `False`。
...
PHP 中的 `fputs()` 函数接受三个参数。

1.  `file`：必选参数，指定文件。
2.  `string`：必选参数，指定要写入的字符串。
3.  `length`：可选参数，指定要写入的最大字节数。
...
1.  由于 `fputs()` 是二进制安全的，因此可以使用该函数写入二进制数据，如图像和字符数据。
2.  不带 `LENGTH` 参数的 `fputs()` 函数将所有数据写到末尾，但它不包括遇到的第一个第 0 个字节。
