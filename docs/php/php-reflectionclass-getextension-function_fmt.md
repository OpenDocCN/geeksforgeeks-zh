...
`ReflectionClass::getExtension()` 函数是 PHP 中的一个内置函数，用于返回 `ReflectionExtension` 对象，该对象表示已定义类的扩展，或者对于用户定义的类为 `NULL`。
...
此函数返回 `ReflectionExtension` 对象，该对象表示已定义类的扩展，或者对于用户定义的类返回 `NULL`。
...
[https://www.php.net/manual/en/reflectionclass.getextension.php](https://www.php.net/manual/en/reflectionclass.getextension.php)