...
`Imagick::chopImage()` 函数是 PHP 中的一个内置函数，用于删除图像区域并对其进行修剪。此函数接受图像的尺寸，并对要裁剪图像的区域和尺寸进行裁切。
...
下面的程序演示了 PHP 中的 `Imagick::chopImage()` 函数：
...
[http://php.net/manual/en/imagick.chopimage.php](//php.net/manual/en/imagick.chopimage.php)