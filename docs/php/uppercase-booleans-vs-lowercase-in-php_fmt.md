...
> 原文:[https://www . geeksforgeks . org/大写-booleans-vs-小写-in-php/](https://www.geeksforgeeks.org/uppercase-booleans-vs-lowercase-in-php/)

布尔值表示真值。布尔值表示两种可能的值:真或假。可以给真值 1，给假值 0。

若要指定布尔文字，请使用常量`TRUE`或`FALSE`。两者都是*不区分大小写*。意思是`TRUE`等于`true`，`FALSE`等于`false`。所以它可以写成
