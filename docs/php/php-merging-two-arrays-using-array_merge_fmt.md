...
如果参数传递给 `array_merge()`函数，并且此数组参数的键是整数，则输出数组中的键将从 0 开始重新编号，并为下一个元素递增 1。下面的程序说明了这一点：
