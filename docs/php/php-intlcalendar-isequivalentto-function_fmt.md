...
`IntlCalendar::isEquivalentTo()`函数是 PHP 中的一个内置函数，用于检查给定的日历是否相等，但时间不同。
...
*   `$other`: 此参数保存另一个用于执行比较的日历。

## 返回值：

如果日历相同（可能只是设置的时间不同），则此函数返回 `TRUE`，如果未设置参数值，则返回错误。
...
[https://www.php.net/manual/en/intlcalendar.isequivalentto.php](https://www.php.net/manual/en/intlcalendar.isequivalentto.php)