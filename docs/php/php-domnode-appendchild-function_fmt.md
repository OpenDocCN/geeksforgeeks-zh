...
`DOMNode::appendChild()` 函数是 PHP 中的一个内置函数，用于将子级附加到现有的子级列表或创建新的子级列表。可以使用 `DOMDocument::createElement()`、`DOMDocument::createTextNode()` 或使用任何其他节点创建子对象。
...
如果节点是只读的，或者要插入的节点的上一个父代是只读的，或者是 `DOM_HIERARCHY_REQUEST_ERR`，如果节点的类型不允许 `$newnode` 节点类型的子节点，或者如果要追加的节点是此节点的祖先之一、此节点本身或 `DOM_HIERARCHY_REQUEST_ERR`，则此函数抛出 `DOM_NO_MODIFICATION_ALLOWED_ERR`，或者如果要追加的节点是此节点的祖先之一、此节点本身或。如果 `$newnode` 是从与创建此节点的文档不同的文档创建的。

下面给出的程序说明了 PHP 中的 `DOMNode::appendChild()` 函数：
