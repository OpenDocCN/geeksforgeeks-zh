...
`Imagick::distortImage()`函数是 PHP 中的一个内置函数，用于使用各种失真方法对图像进行失真。
...
*   `$bestfit`：此参数存储尝试调整目标大小以适应扭曲的源的方法类型的值。
...
下面的程序演示了 PHP 中的`Imagick::distortImage()`函数：
...
[http://php.net/manual/en/imagick.distortimage.php](//php.net/manual/en/imagick.distortimage.php)