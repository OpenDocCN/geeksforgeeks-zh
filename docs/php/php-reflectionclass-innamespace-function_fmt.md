# ReflectionClass::inNamespace() 函数

`ReflectionClass::inNamespace()` 函数是 PHP 中的一个内置函数，用于检查类是否存在于命名空间中。成功时返回 `TRUE`，失败时返回 `FALSE`。
...
[https://www.php.net/manual/en/reflectionclass.innamespace.php](https://www.php.net/manual/en/reflectionclass.innamespace.php)