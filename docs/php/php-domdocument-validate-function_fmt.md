...
## DOMDocument::validate() 函数

`DOMDocument::validate()` 函数是 PHP 中的一个内置函数，用于根据文档的 DTD(文档类型定义)验证文档。 DTD 定义 XML 文件要遵循的规则或结构，如果 XML 文档不遵循此格式，则此函数将返回 `False`。
...
如果文档遵循 DTD，则此函数返回 `TRUE` 或 `FALSE`。

下面的示例说明了 PHP 中的 `DOMDocument::validate()` 函数：
...
[https://www.php.net/manual/en/domdocument.validate.php](https://www.php.net/manual/en/domdocument.validate.php)