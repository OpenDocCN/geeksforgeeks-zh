...
> 原文: [https://www.geeksforgeeks.org/why-use-guzzle-instead-of-curl-in-php/](https://www.geeksforgeeks.org/why-use-guzzle-instead-of-curl-in-php/)

## 什么是 cURL？

cURL 是 PHP 中的一个模块，用它我们可以使用 `libcurl`。`libcurl` 是一个在 PHP 中使用的库，用于创建连接并与各种不同的服务器通信，这些服务器可能具有不同类型的协议。cURL 为我们提供了各种预构建函数，比如 `curl_init()`、`curl_setopt()`、`curl_exec()`、`curl_close()`。
