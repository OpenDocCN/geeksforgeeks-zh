...
Soundex 键是一个四字符长的字母数字字符串（以字母开头），表示给定字符串的英语发音。
...
此函数接受单个参数 `$str`，该参数表示我们要为其查找 Soundex 键的字符串。
...
在下面的程序中，`Soundex()` 函数生成字符串“geeksforgeek”的 Soundex 键。
...
在下面的程序中，我们将看到两个发音相同的单词产生相似的发音键。
