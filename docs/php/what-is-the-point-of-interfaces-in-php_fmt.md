...
> 原文：`https://www.geeksforgeeks.org/what-is-the-point-of-interfaces-in-php/`

接口允许用户创建指定类必须实现的公共方法的程序，而不涉及如何实现特定方法的复杂性和细节。它通常被称为下一个抽象层次。它类似于抽象方法，类似于抽象类。接口的定义就像类的定义一样，但是`class`关键字被`interface`关键字和函数原型所代替。该接口不包含数据变量。接口在某种程度上是有帮助的，因为它确保为程序员的所有方法维护一种元数据。
...
如上图所示，我们在所有三个类中都有相同的函数`draw()`并且在类`Circle`中还有一个函数作为`setRadius()`。现在我们已经创建了三个不同的接口，并在我们的类中实现了它们，如下所示：
