...
`IntlChar::isDefined()` 函数是 PHP 中的内置函数，用于检查是否定义了代码点。如果该字符被分配了字符，则该字符被认为是被确定的。对于 CN（其他，未分配）以外的一般类别为 True。
...
此函数接受单个参数 `$codepoint`，该参数是必需的。`$codepoint` 值是整数值或字符，编码为 `UTF-8` 字符串。

## 返回值

如果 `$codepoint` 是定义的字符，则此函数返回 `True`，否则返回 `False`。

下面的程序演示了 PHP 中的 `IntlChar::isDefined()` 函数：
...
[http://php.net/manual/en/intlchar.isdefined.php](http://php.net/manual/en/intlchar.isdefined.php)