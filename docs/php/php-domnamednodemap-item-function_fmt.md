...
`DOMNamedNodeMap::item()` 函数是 PHP 中的内置函数，用于检索 `index` 指定的节点。此函数用于从元素中获取特定属性，进而可以根据需要获取该属性的名称或值。
...
此函数接受保存索引的单个参数 `$index`。

## 返回值

此函数返回地图中索引位置处的节点。

下面给定的程序说明了 PHP 中的 `DOMNamedNodeMap::item()` 函数。

## 程序 1

在此示例中，我们将使用 `item()` 获取属性的名称。
...
[https://www.php.net/manual/en/domnamednodemap.item.php](https://www.php.net/manual/en/domnamednodemap.item.php)