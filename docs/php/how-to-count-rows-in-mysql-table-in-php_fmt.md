...
通过使用 PHP 和 MySQL，可以执行数据库操作。我们可以通过使用 MySQL `mysqli_num_rows()` 函数得到一个表中的总行数。

### 语法

`mysqli_num_rows(result);`

`result` 就是指定 `mysqli_query()` 函数返回的结果集标识符。
...
*   创建名为 `database` 的数据库，并在数据库内创建名为 `building` 的表格。
*   将记录插入其中
...
在 localhost 中运行上述 PHP 文件后，得到如下结果。
...
在下面的示例中，我们使用 MySQL `count()` 函数对表行进行计数。这是一个用来计算行数的聚合函数。
