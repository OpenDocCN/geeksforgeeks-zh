...
`Imagick::averageImages()` 函数是 PHP 中的一个内置函数，用于在图像处理后创建平均两个或多个图像。它在 PECL Imagick 2.0.0 版本中有很好的定义。此函数在 Imagick 的后续版本中已不再使用，因此在 PECL Imagick 2.1.0 版本中将其替换为 `Imagick::mergeImageLayers()`。
