...
> 原文：[https://www.geeksforgeeks.org/how-to-include-all-php-files-from-a-directory/](https://www.geeksforgeeks.org/how-to-include-all-php-files-from-a-directory/)

给定一个目录中的PHP文件列表，任务是包含一个目录中的所有文件。为了同时包含目录中的所有PHP文件，需要一个`foreach`循环。

## 示例

这个示例在一个目录中包含四个PHP文件（`file1.php`，`file2.php`，`file3.php`，`file4.php`）。创建一个名为`main.php`的文件，其中包含PHP代码来包含文件。
