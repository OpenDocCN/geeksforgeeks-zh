...
> 原文：[https://www.geeksforgeeks.org/how-to-add-api-function-to-a-simple-php-page/](https://www.geeksforgeeks.org/how-to-add-api-function-to-a-simple-php-page/)

## 介绍

应用程序编程接口是一个包含一组规则、协议或工具的系统，这些规则、协议或工具有助于通过标准数据访问在两个应用程序或软件之间提供交互。它非常类似于用于开发网页或移动应用程序的网络服务。一个应用程序可以调用其他程序的应用编程接口来利用一些功能。应用编程接口获取请求并在程序员的软件系统中返回结果。
...
一个非常常见的例子，使用支付流程 API 而不是开发我们自己的支付流程。Web 服务 API 是通过网络连接访问的独立于平台的方法。

1.  **SOAP：** 简单对象访问协议，使用 web 服务定义语言或 XML 进行数据传输。它非常健壮。这些用于集成应用编程接口。
2.  **JSON-RPC：** 数据传输使用 JSON。
3.  **REST：** 一套规则，包含用于数据交换的一些标准架构原则。为了发出请求，它使用 HTTP 方法 `GET`、`PUT`、`POST`、`PATCH`、`DELETE` 来执行所有 CRUD 操作。它消耗的带宽较少，并且可以舒适地访问云服务。
...
```php
$returnData = callAPI('GET', 'https://api.geeksforgeeks.org/url_for_get/'.$user['user']['buyer_id'], false);
...
$apiCall = apiCall('POST', 'https://api.geeksforgeeks.org/url_for_post/', json_encode($dataArray));
...
```php
$dataArray = array(
    "amount" => "required_installment"
);
$updateData = APIcall('PUT', 'https://api.geeksforgeeks.org/url_for_put/'.$putParameter, json_encode($dataArray));
...
```php
APIcall('DELETE', 'https://api.geeksforgeeks.org/url_for_delete/'.$required_id, false);
```
