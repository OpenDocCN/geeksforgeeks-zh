...
*   `$passphrase`: this parameter is used to control access to computer systems, programs, or data.
*   `$configargs`: "configargs" parameter is used to initialize the request. Developers can also mention alternative OpenSSL configuration files by setting the configuration key value to the file path that will be used.
