...
`jdtogregorian()` 函数是一个内置函数，用于将[儒略日整数](https://en.wikipedia.org/wiki/Julian_calendar)转换为[公历日期](https://en.wikipedia.org/wiki/Gregorian_calendar)。该函数接受儒略日整数，并以`$月/$日/$年`返回转换后的公历日期。
