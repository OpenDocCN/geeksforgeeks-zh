...
`DOMText::splitText()`函数是 PHP 中的一个内置函数，用于在指定偏移量处将一个节点拆分为两个节点。
...
此函数接受单个参数`$offset`，该参数保存要拆分的偏移量。

## 返回值

此函数返回相同类型的新节点，该节点包含偏移量及其之后的所有内容。

下面给出的程序演示了 PHP 中的`DOMText::splitText()`函数：
