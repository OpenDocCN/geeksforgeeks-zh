...
`assertInfinite()` 函数是 `PHPUnit` 中的内置函数，用于断言实际获得的值是否为 `INF`。如果实际值为 `INF`，则此断言将返回 `TRUE`，否则将返回 `FALSE`。如果为 `true`，则断言的测试用例通过，否则测试用例失败。
...
*   `$ActualVariable`: 此参数可以是任何类型的变量，代表实际值。
