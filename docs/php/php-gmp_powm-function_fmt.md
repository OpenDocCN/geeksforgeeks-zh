...
`gmp_powm()` 是 PHP 中的一个内置函数，用于计算两个 GMP 数的幂与另一个 GMP 数的模数。([GNU Multiple Precision](https://en.wikipedia.org/wiki/GNU_Multiple_Precision_Arithmetic_Library)：用于大数)
...
此函数返回一个正的 [`gmp`](http://php.net/manual/en/class.gmp.php) 数字，等于 `($base ^ $exp) % $mod`。
...
下面的程序演示了当 GMP 编号作为参数传递时，`gmp_powm()` 函数的工作原理。
...
下面的程序演示了将数字字符串作为参数传递时 `gmp_powm()` 的工作方式。
