# SimpleXMLElement::getDocNamespaces() 函数
...
*   `$recursive`: 一个可选的布尔参数。其默认值为 `False`。如果传递 `True`，它将递归地返回父节点和子节点中的名称空间。如果设置为 `False`，则只返回父节点的名称空间。
*   `$from_root`: 一个可选的布尔参数。默认值为 `True`。如果设置为 `True`，它将从 XML 文档的根开始检查名称空间。如果设置为 `false`，它将在子节点下检查名称空间。

## 返回值

此函数返回一个包含名称空间名称及其关联 URI 的数组。
