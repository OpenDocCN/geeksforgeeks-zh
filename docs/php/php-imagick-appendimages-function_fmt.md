...
## 参数：
此函数接受单个参数`$STACK`，这是必需的。 如果堆栈值为 False，则图像从左到右堆叠，如果堆栈值为 True，则图像从上到下堆叠。 堆栈的默认值为 False。

## 返回值：
此函数在成功时返回 Imagick 实例。
...
## 引用：
[http://php.net/manual/en/imagick.appendimages.php](http://php.net/manual/en/imagick.appendimages.php)