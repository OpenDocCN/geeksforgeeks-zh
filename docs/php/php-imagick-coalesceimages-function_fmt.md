...
`Imagick::coalesceImages()` 函数是 PHP 中的一个内置函数，用于将一组图像组合成单个图像。它根据任何页面偏移量和处理方法合成一组图像。动画序列 GIF、MIFF 和 MNG 通常从图像背景开始，并且每个后续图像在大小和偏移量上各不相同。
...
下面的程序演示了 PHP 中的 `Imagick::coalesceImages()` 函数：
