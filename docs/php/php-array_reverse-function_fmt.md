...
2.  `$key_preserve` (可选): 这是一个可选参数，可以设置为 `TRUE` 或 `FALSE`，它指的是数组的键的保存。默认情况下，该参数的值为 `FALSE`。

## 返回值

该函数返回传入参数的数组，元素的顺序相反。
...
1.  This program reverses an array taking the `$key_preserve` as `FALSE` by default. This don’t preserve the keys.
