...
## 错误
错误是一个意外的程序结果，程序本身无法处理。这可以通过手动使用代码中的问题来解决。错误可能是程序本身无法处理的无限循环，因此您必须手动修复该问题。有一个处理错误的简单程序，即使用`die()`功能。
...
## 异常
异常也是程序的意外结果，但异常可以由程序自己通过抛出另一个异常来处理。异常应该只用于错误条件，其中错误是不可删除的。使用`try`和`catch`的方法有一个克服异常的简单方法。
...
| 这可以使用 `PHP die()` 函数来完成。 | 使用 `throw new Exception()` 的基本异常处理在提前异常处理的情况下，您必须使用`try`和`catch`方法。 |
| 错误主要是由程序运行的环境引起的。 | 程序本身负责导致异常。 |

## 注意
PHP 默认是一种异常轻语言，但是在使用面向对象的代码时，可以将错误变成异常。