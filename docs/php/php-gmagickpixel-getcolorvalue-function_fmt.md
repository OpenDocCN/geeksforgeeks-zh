...
`GmagickPixel::getColorvalue()` 函数是 PHP 中的一个内置函数，用于获取给定 `GmagickPixel` 颜色的所提供颜色通道的归一化值。规格化值是介于 0 和 1 之间的浮点数。
...
此函数接受单个参数 `$color`，该参数保存与[颜色常量](https://www.php.net/manual/en/gmagick.constants.php#gmagick.constants.color-black)之一相对应的整数值。
所有颜色常量列表如下：
