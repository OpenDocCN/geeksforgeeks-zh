...
`stripslashes()`函数是 PHP 中的内置函数。此函数用于删除字符串中的反斜杠。
...
此函数只接受一个参数，如上面的语法所示。具体说明如下：

*   `string`：这是指定函数将在其上操作的字符串的唯一必需参数。
...
下面的程序演示了 PHP 中的 `stripslashes()`函数：
...
在此程序中，我们将看到 `stripslashes()`函数的数组实现。`stripslashes()`不是递归的。为了将此函数应用于数组，需要使用递归函数。
