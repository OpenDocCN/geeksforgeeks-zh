...
`Imagick::getImage()` 函数是 PHP 中的一个内置函数，用于获取 `Imagick` 对象的当前图像序列。此函数用于将一个 `Imagick` 对象的内容复制到新变量中。
...
如果成功，此函数将返回一个新的 `Imagick` 对象。

下面的程序演示了 PHP 中的 `Imagick::getImage()` 函数：
...
[https://www.php.net/manual/en/imagick.getimage.php](https://www.php.net/manual/en/imagick.getimage.php)