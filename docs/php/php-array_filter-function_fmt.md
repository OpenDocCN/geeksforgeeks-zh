...
函数取三个参数，其中一个是必选的，另外两个是可选的。

1.  `$array` (必选): 指要过滤的输入数组。
2.  `$callback_function` (可选): 指用户定义的函数。如果没有提供函数，数组中所有等于 `FALSE` 的条目将被删除。
3.  `$flag` (可选): 指传递给回调函数的参数。
    *   `ARRAY_FILTER_USE_KEY` - 将键作为唯一参数传递给回调函数，而不是数组的值。
    *   `ARRAY_FILTER_USE_BOTH` - 将值和键作为参数传递给回调函数，而不仅仅是值。
