...
`DateTime::modify()` 函数是 PHP 中的一个内置函数，用于修改或可以更改 `DateTime` 对象的时间戳。
...
*   `$object`: 它指定由 `date_create()` 函数返回的 `DateTime` 对象。此对象由 `DateTime::modify()` 函数修改。
*   `$modify`: 它指定日期/时间字符串。通过增加或减少此字符串来修改 `DateTime` 对象。

## 返回值

此函数成功时返回修改后的 `DateTime` 对象，失败时返回 `False`。
...
[https://www.php.net/manual/en/datetime.modify.php](https://www.php.net/manual/en/datetime.modify.php)