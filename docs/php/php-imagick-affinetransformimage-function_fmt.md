...
`Imagick::affineTransformImage()` 函数是 PHP 中的一个内置函数，用于根据仿射矩阵转换图像。
...
此函数接受单个参数 `$matrix`，该参数保存仿射矩阵的值，可以基于旋转、SHEER、SCALE 等。
...
此程序使用 `Imagick::affineTransformImage()` 函数通过仿射给定的仿射矩阵来变换图像。
...
// Use affineTransformImage() function
...
[https://www.php.net/manual/en/imagick.affinetransformimage.php](https://www.php.net/manual/en/imagick.affinetransformimage.php)