...
此函数接受单个参数 `$image`，这是必需的。此 `$image` 变量可以存储由 `imagecreatetruecolor()` 图像创建函数创建的图像。

## 返回值

此函数返回图像的宽度，如果出错则返回 `False`。
...
[http://php.net/manual/en/function.imagesx.php](http://php.net/manual/en/function.imagesx.php)