...
`Ds\Map::ksort()` 函数是 PHP 中的一个内置函数，用于按键对地图元素进行就地排序。
...
此函数接受单个参数 `$comparator`，该参数包含对映射副本进行排序时将根据其比较值的函数。比较器应该根据作为参数传递给它的两个值的比较返回下列值：
...
下面的程序演示了 PHP 中的 `Ds\Map::ksort()` 函数：
...
[https://www.php.net/manual/en/ds-map.ksort.php](https://www.php.net/manual/en/ds-map.ksort.php)