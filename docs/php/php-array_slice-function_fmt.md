...
3.  `$slicing_range` (可选): 该参数是指需要进行切片的范围或极限点。负值将指示从字符串末尾开始的计数。现在，这个也可以留空。如果留空，该函数将从起点到终点对所有值进行切分。
4.  `preserve` (可选): 该参数只能取两个布尔参数，即 `true` 或 `false`。这将告诉该功能是保留按键还是重置按键。`true` 表示保留密钥，`false` 表示重置密钥。默认值为 `false`。

## 返回值

如前所述，该函数将返回数组的选定部分或切片部分。

下面的程序说明了 PHP 中的 `array_slice()` 函数:

*   在这个程序中，我们将传递所有的正参数以及 `true` 值来保存密钥。
