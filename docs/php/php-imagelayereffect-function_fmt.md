...
函数的作用是：`imagelayereffect()` 是 PHP 的内置函数，用于设置 Alpha 混合标志以使用分层效果。 此函数成功时返回 `True`，失败时返回 `False`。
...
*   `$effect`：该参数设置 Effect 常量的值。 效果常量的值如下所示：
    *   `IMG_EFFECT_REPLACE`：设置像素替换。 它等效于将 `True` 传递给 `imagelayereffect()` 函数。
    *   `IMG_EFFECT_ALPHABLEND`：设置正常像素混合。 它等效于将 `false` 传递给 `imagelayereffect()` 函数。
