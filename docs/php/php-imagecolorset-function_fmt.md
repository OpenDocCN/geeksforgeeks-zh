...
`imagecolorset()` 函数是 PHP 中的内置函数，用于设置指定调色板索引的颜色。它用于将调色板中的某个索引设置为指定的颜色。在调色板图像中高效地创建类似填充的效果时，此函数非常有用。
...
*   `$image`：由一个图像创建函数返回，例如 `imagecreatetruecolor()`。它用于创建图像的大小。
...
[http://php.net/manual/en/function.imagecolorset.php](http://php.net/manual/en/function.imagecolorset.php)