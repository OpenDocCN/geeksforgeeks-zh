...
*   `[6]` 或 `[rdev]` - 信息节点设备类型
*   `[7]` 或 `[size]` - 以字节为单位的大小
*   `[8]` 或 `[atime]` - 上次访问时间 (作为 Unix 时间戳)
*   `[9]` 或 `[mtime]` - 上次修改时间 (作为 Unix 时间戳)
*   `[10]` 或 `[ctime]` - 上次更改信息节点时间 (作为 Unix 时间戳)
*   `[11]` 或 `[blksize]` - 文件系统 IO 的块大小 (如果支持)
*   `[12]` 或 `[blocks]` - 分配的块数

## 注意

> 此函数类似于 `stat()`，不同之处在于如果文件参数是符号链接，则符号链接的状态是返回而不是符号链接指向的文件的状态。
...
**参考：** [http://php.net/manual/en/function.lstat.php](http://php.net/manual/en/function.lstat.php)