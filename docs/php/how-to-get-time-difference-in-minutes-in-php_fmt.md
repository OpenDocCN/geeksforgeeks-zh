...
> 原文: [https://www.geeksforgeeks.org/how-to-get-time-difference-in-minutes-in-php/](https://www.geeksforgeeks.org/how-to-get-time-difference-in-minutes-in-php/)

在本文中，我们将学习如何使用 PHP 获取以分钟为单位的时差。

我们将使用内置函数 [`date_diff()`](https://www.geeksforgeeks.org/php-date_diff-function/) 来获取以分钟为单位的时差。为此，我们需要一个开始日期和结束日期来使用 `date_diff()` 函数计算它们的时差（以分钟为单位）。
...
该函数返回第一个日期时间对象和第二个日期时间对象之间的差值，否则在失败时返回 `false`。

## 例 1

下面的程序说明了 `date_diff()` 函数获取分钟的时间差。
