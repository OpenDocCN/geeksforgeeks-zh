...
`DOMDocument::getElementById()` 函数是 PHP 中的一个内置函数，用于搜索具有特定 id 的元素。
...
此函数接受单个参数 `$elementId`，该参数保存要搜索的 id。

## 返回值

此函数返回 `DOMElement`，如果未找到元素，则返回 `NULL`。

下面给出的程序演示了 PHP 中的 `DOMDocument::getElementById()` 函数：

## 程序 1

在本程序中，我们将获得具有特定 id 的元素的标记名。
...
[https://www.php.net/manual/en/domdocument.getelementbyid.php](https://www.php.net/manual/en/domdocument.getelementbyid.php)