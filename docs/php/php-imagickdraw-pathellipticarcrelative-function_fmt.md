...
`ImagickDraw::pathEllipticArcRelative()` 函数是 PHP 中的一个内置函数，用于使用相对坐标绘制从当前点到 (x, y) 的椭圆弧。
...
// Draw elliptic arc
$draw->pathStart();
$draw->pathEllipticArcRelative(120, 9250, 210, false, true, 300, 400);
$draw->pathClose();
$draw->pathFinish();
...
// Draw elliptic arc
$draw->pathStart();
$draw->pathEllipticArcRelative(120, 50, 10, true, true, 300, 400);
$draw->pathFinish();
