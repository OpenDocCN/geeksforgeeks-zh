...
`Collator::create()`函数是 PHP 中的一个内置函数，用于创建一个新的收集器。
...
该函数接受单个参数 `$locale`，该参数保存所需的排序规则。如果为区域设置传递空值，将使用默认的区域设置排序规则。如果在区域设置中传递空字符串("")或“根”，则将使用 UCA 规则。

## 返回值

该函数成功返回排序器对象的新实例，错误返回空值。
...
**参考:**[https://www.php.net/manual/en/collator.create.php](https://www.php.net/manual/en/collator.create.php)