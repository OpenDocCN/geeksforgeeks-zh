...
> 原文: [https://www.geeksforgeeks.org/when-to-use-self-over-this-in-php/](https://www.geeksforgeeks.org/when-to-use-self-over-this-in-php/)

`self` 和 `$this` 是两个不同的运算符，分别用来表示当前类和当前对象。`self` 用于访问静态或类变量或方法，而 `$this` 用于访问非静态或对象变量或方法。
...
**`$this` 运算符:** `$this`，为‘this’号；暗号暗示，是一个物体。`$this` 表示类的当前对象。它用于访问类的非静态成员。
...
**示例 2:** 这个示例是 php 中使用 `$this` 的多态行为的演示。
