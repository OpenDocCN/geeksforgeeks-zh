...
`file_get_contents()` 函数接受一个强制参数和四个可选参数。
...
成功时返回读取数据，失败时返回 `False`。

## 方法

为了将文件放入字符串，我们将使用 `file_get_contents()` 函数。对于第一个示例，我们将 URL 链接指定为将重定向到给定站点的参数。对于第二个示例，生成包含数据的文件名 `gfg.txt`。此函数将文件读入字符串，相应地呈现内容。
