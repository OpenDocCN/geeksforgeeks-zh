...
> 原文: [https://www.geeksforgeeks.org/how-to-send-an-email-using-phpmailer/](https://www.geeksforgeeks.org/how-to-send-an-email-using-phpmailer/)
...
安装 PHPMailer 最好的方法是使用 Composer。继续之前，确保安装 [Composer](https://getcomposer.org/)。

1.  打开命令提示符，转到要使用 PHPMailer 的项目目录。
2.  运行以下命令：
    ```php
    composer require phpmailer/phpmailer
    ```
3.  等待安装完成。它会将所有必要的类下载到您的项目文件夹中。
...
*   `SMTPDebug`：用于显示有关连接问题和发送电子邮件的消息。它具有以下值：
    ...
    *   `3`：作为 `2`，加上有关初始连接的更多信息–此级别可帮助诊断 `STARTTLS` 故障。
    *   `4`：作为 `3`，加上显示更低级别的信息。
...
*   `Subject`：设置邮件的主题。
*   `Body`：设置邮件的内容。
*   `AltBody`：备选正文，以防电子邮件客户端不支持 HTML。
