...
`Imagick::queryFormats()` 函数是 PHP 中的一个内置函数，用于获取 Imagick 支持的格式。
...
此函数接受单个参数 `$pattern`，该参数保存要与格式匹配的模式。

## 返回值

此函数返回包含 Imagick 支持的格式的数组。
...
[`https://www.php.net/manual/en/imagick.queryformats.php`](https://www.php.net/manual/en/imagick.queryformats.php)