...
`Imagick::getImageInterlaceScheme()`函数是 PHP 中的一个内置函数，用于获取图像隔行扫描方案。
...
此函数返回一个整数值，其中包含与[隔行扫描常数](https://www.php.net/manual/en/imagick.constants.php#imagick.constants.interlace-undefined)之一相对应的隔行扫描方案。
...
下面给定的程序说明了 PHP 中的 `Imagick::getImageInterlaceScheme()`函数：
