...
第一个数字的值存储在临时变量中。 然后，该值将被第二个数字的值替换。 然后将临时变量的值赋给第二个数字。 执行此操作所需的时间复杂度为 `O(1)`。 所需空间也大致相当于 `O(1)`。
...
XOR 运算符可用于在 PHP 中交换数字。 但是，此技术仅适用于整数的情况。 XOR 运算符应用程序需要恒定的时间复杂度和空间复杂度。
...
使用参数作为交换后的数字创建数组对象，即第一个参数是第二个数字，第二个参数是第一个数字。
