...
`DateTimeImmutable::setTime()` 函数是 PHP 中的一个内置函数，用于通过重置创建的 `DateTimeImmutable` 对象的当前时间来设置所需的时间。
...
**返回值：** 此函数返回新的 `DateTimeImmutable` 对象。

以下程序说明 PHP 中 `DateTimeImmutable::setTime()` 函数的用法：
