...
`XMLReader::moveToElement()` 函数是 PHP 中的一个内置函数，用于将光标移动到当前属性的父元素。通过将此函数与 `XMLReader::moveToAttribute()` 函数结合使用，可以使用此函数获取具有特定属性的元素。
...
调用此方法时，如果函数成功，则返回 `TRUE`；如果失败或未定位在属性上，则返回 `FALSE`。

下面给出的程序演示了 PHP 中的 `XMLReader::moveToElement()` 函数：

### 程序 1

在本程序中，我们将获得属性为 `attrib` 的元素的名称。
...
在此程序中，我们将获得属性为 `attrib` 的所有元素的名称。
