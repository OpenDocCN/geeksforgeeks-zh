...
`ReflectionProperty::isPrivate()` 函数是 PHP 中的一个内置函数，用于在指定属性为私有时返回 `true`，否则返回 `false`。
...
如果指定的属性是私有的，则此函数返回 `TRUE`，否则返回 `FALSE`。

## 程序 1

以下程序说明 PHP 中的 `ReflectionProperty::isPrivate()` 函数：
