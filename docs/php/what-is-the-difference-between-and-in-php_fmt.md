...
> 原文：[https://www.geeksforgeeks.org/what-is-the-difference-between-and-in-php/](https://www.geeksforgeeks.org/what-is-the-difference-between-and-in-php/)

在本文中，我们将讨论 PHP 中的 `==` 和 `===` 运算符之间的区别。两者都是比较运算符，用于比较两个或多个值。

## ==运算符

该运算符用于检查给定值是否相等。如果是，则返回 `true`，否则返回 `false`。
...
该运算符用于检查给定值及其数据类型是否相等。如果是，则返回 `true`，否则返回 `false`。
