# `DOMDocument::createAttributeNS()` 函数
...
`DOMDocument::createAttributeNS()` 函数是 PHP 中的内置函数，用于创建具有关联名称空间的新属性节点。
...
*   `$qualifiedName` 此参数保存属性的标签名称和前缀。例如：`prefix: tag name`。

## 返回值

此函数成功时返回新的 `DOMAttr` 对象，失败时返回 `False`。
...
[https://www.php.net/manual/en/domdocument.createattributens.php](https://www.php.net/manual/en/domdocument.createattributens.php)