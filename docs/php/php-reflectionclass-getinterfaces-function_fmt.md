...
`ReflectionClass::getInterfaces()` 函数是 PHP 中的内置函数，用于返回接口的关联数组。这些返回的数组包含作为接口名称的键和作为 `ReflectionClass` 对象的数组值。
...
此函数返回接口的关联数组。这些返回的数组包含作为接口名称的键和作为 `ReflectionClass` 对象的数组值。
