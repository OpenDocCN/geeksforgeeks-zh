...
`Imagick::morphology()`函数是 PHP 中的一个内置函数，用于根据给定的形态学方法将用户提供的内核应用于图像。
...
*   `$morphologyMethod`：此参数保存要使用的形态学方法。
...
下面的程序说明了 PHP 中的`Imagick::morphology()`函数：
