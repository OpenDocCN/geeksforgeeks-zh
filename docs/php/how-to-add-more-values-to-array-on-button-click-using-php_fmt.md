...
> 原文：[https://www.geeksforgeeks.org/how-to-add-more-values-to-array-on-button-click-using-php/](https://www.geeksforgeeks.org/how-to-add-more-values-to-array-on-button-click-using-php/)
...
我们正在处理请求以及发送到 `data.php` 的数据。每当发送请求时，它都会打开 JSON 文件并从中读取前一个数组。如果没有先前的数组，即 `array.json` 文件为空，则它创建一个数组。然后，它会向其中追加数据。

## 输出

## 执行步骤

我们将首先确认我们的 `array.json` 文件为空。
...
![](img/bb62770c451fd5ba3efb76a7c1af3b17.png)

用户输入屏幕

在输入框中键入你想添加到数组的内容，然后点击 `Add` 按钮。你会注意到出现一个对话框，确认数据已添加，它甚至显示了数组。

![](img/294a5bdd1b9298e230cb32514d320a2d.png)
...
![](img/1ca02760ee05b2547bb15db53f87592b.png)