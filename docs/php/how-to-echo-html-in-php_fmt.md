...
PHP `echo` 或 `print` 可用于显示 HTML 标记、JavaScript、文本或变量。
...
## 2. 使用 `echo` 简写或分离 HTML

PHP `echo` 简写可用于显示任何表达式的结果、任何变量的值或 HTML 标记。
...
我们可以使用 `<<<` `heredoc` 来打印 HTML。`<<<` 后面必须跟一个标识符和换行符。使用相同的标识符来关闭 `heredoc` 的主体。
