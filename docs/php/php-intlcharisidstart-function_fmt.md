...
`IntlChar::isIDStart()` 函数是 PHP 中的一个内置函数，用于检查给定的输入字符代码点是否允许作为标识符的第一个字符。一般类别为“L”（字母）和“NL”（字母和数字）的字符返回真。
...
此函数接受单个必需参数 `$codepoint`。输入参数是字符或整数值，编码为 `UTF-8` 字符串。

## 返回值

如果 `$codepoint` 以标识符字符开头，则返回 `True`，否则返回 `False`。
...
[http://php.net/manual/en/intlchar.isidstart.php](http://php.net/manual/en/intlchar.isidstart.php)