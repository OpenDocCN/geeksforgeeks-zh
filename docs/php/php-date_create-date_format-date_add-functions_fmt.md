...
1.  `time`：(可选)指定日期/时间字符串。 `NULL` 或默认值表示当前日期/时间。 您可以参考[此链接](http://php.net/manual/en/datetime.formats.php)了解 PHP 中支持的日期和时间格式。
2.  `timezone`：(可选)`time`的时区。

### 返回值

此函数返回指定日期的新 `DateTime` 对象。
...
此函数接受两个参数，这两个参数都是必须提供的。

1.  `object`：指定由 `date_create()`返回的 `DateTime` 对象
2.  `format`：指定日期的格式。 它接受 PHP 中 `date()`函数支持的格式。 示例-`H`(24 小时格式)、`h`(12 小时格式)、`i`(分钟：00 到 59)、`s`(秒：00 到 59)等。

### 返回值

`date_format()`函数返回一个字符串，该字符串表示成功格式化时按照指定格式格式化的日期，否则在失败时返回 `False`。
...
此函数接受三个参数，所有这些参数都是必须提供的。

1.  `object`：指定由 `date_create()`返回的 `DateTime` 对象。 此函数返回一个新的 `DateTime` 对象。
