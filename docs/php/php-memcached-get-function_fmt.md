...
`memcached::get()`函数是 PHP 中 `Memcached` 类的内置函数，用于获取存储在 memcache 服务器上的给定键下的值。
...
*   `$flags`：控制返回结果的标志。 当给定 `memcached::GET_EXTENDED` 时，该函数还将返回 CAS 令牌。

## 返回值

返回缓存中存储的值，否则返回 `False`。 如果将标志设置为 `memcached::GET_EXTENDED`，则返回包含该值和 CAS 标记的数组，而不是仅返回值。 如果键不存在，`memcached::getResultCode()` 将返回 `memcached::RES_NOTFOUND`。
