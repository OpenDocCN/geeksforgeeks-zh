...
我们已经在 [PHP|变量和数据类型](https://www.geeksforgeeks.org/php-variables-data-types/) 中讨论了 PHP 中的变量和全局变量。在本文中，我们将了解 PHP 中的*超全局变量*。
...
现在让我们来详细了解一下其中一些超全局变量：

## `$GLOBALS`

它是一个超全局变量，用于从 PHP 脚本中的任何位置访问全局变量。PHP 将所有全局变量存储在数组 `$GLOBALS[]` 中，其中 index 保存可以访问的全局变量名。

下面的程序说明了 `$GLOBALS` 在 PHP 中的用法：
...
在上面的代码中，两个全局变量被声明为 `$x` 和 `$y`，它们被赋予了一定的值。然后定义函数 `multiplication()`，将 `$x` 和 `$y` 的值相乘，并存储在全局数组中定义的另一个变量 `$z` 中。

## `$_SERVER`

它是一个 PHP 超级全局变量，存储有关头文件、路径和脚本位置的信息。其中一些元素用于从超级全局变量 `$_SERVER` 获取信息。
...
它是一个超全局变量，用于在提交 HTML 表单后收集数据。`$_REQUEST` 不是最常用的，因为 `$_POST` 和 `$_GET` 执行相同的任务，并且使用广泛。
...
它是一个超级全局变量，用于在提交 HTML 表单后收集数据。当表单使用方法 `POST` 传输数据时，数据在查询字符串中不可见，因此在此方法中会保持安全级别。
...
`$_GET` 是一个超级全局变量，用于在提交 HTML 表单后收集数据。当表单使用 `GET` 方法传输数据时，数据在查询字符串中可见，因此值不会隐藏。`$_GET` 超级全局数组变量存储 URL 中的值。
