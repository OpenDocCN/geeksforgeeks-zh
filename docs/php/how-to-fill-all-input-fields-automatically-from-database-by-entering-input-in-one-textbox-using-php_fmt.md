...
> 原文: [https://www.geeksforgeeks.org/how-to-fill-all-input-fields-automatically-from-database-by-entering-input-in-one-textbox-using-php/](https://www.geeksforgeeks.org/how-to-fill-all-input-fields-automatically-from-database-by-entering-input-in-one-textbox-using-php/) 通过在一个文本框中输入输入来自动填充所有输入字段
...
**示例:** 假设在相应的输入字段中输入 `user id`。如果数据库中存在 `user id`，则 `XMLHttpRequest` 对象从数据库中获取与该 `user id` 对应的 `名` 和 `姓` 的值。它会用您在响应中获得的数据替换其他输入文本框的 `innerHtml`。
