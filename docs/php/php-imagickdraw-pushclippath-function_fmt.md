...
`ImagickDraw::pushClipPath()` 函数是 PHP 中的一个内置函数，用于启动剪辑路径定义。剪辑路径用于创建剪辑区域，该区域决定应显示图像的哪一部分。显示区域内的零件，而隐藏区域外部的零件。
...
此函数接受保存剪辑路径名称的单个参数 `$clip_mask_id`。

## 返回值

如果成功，此函数返回 `TRUE`。

下面的程序演示了 PHP 中的 `ImagickDraw::pushClipPath()` 函数：
