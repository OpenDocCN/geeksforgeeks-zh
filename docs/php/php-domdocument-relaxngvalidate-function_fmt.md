...
`DOMDocument::relaxNGValidate()` 函数是 PHP 中的一个内置函数，用于对文档执行 RELAX NG 验证。RELAX NG 是 DTD 的替代方法，它定义了 XML 文档需要遵循的结构。
...
此函数接受保存 RNG 文件的单个参数 `$filename`。

## 返回值

此函数成功时返回 `TRUE`，失败时返回 `FALSE`。

下面给出的程序说明了 PHP 中的 `DOMDocument::relaxNGValidate()` 函数：
