...
2.  `$mode` - 这是指定返回值类型的强制参数。该模式可以在 0-5 的范围内（包括 0-5）。
    ...
    *   `5` - 返回月份名称（Vendemiaire、Brumaire、Frimaire 等）。在法国共和党中，当模式被传递为 5 时。

## 返回值

该函数根据传递的模式返回月份名称。如果将除 0-5 以外的任何值作为模式传递，则该模式被视为 0。
...
下面的程序演示了当 mode 作为 0 传递时的 `jdmonthname()` 函数。
...
下面的程序演示了模式作为 1 传递时的 `jdmonthname()` 函数。
...
下面的程序演示了模式作为 2 传递时的 `jdmonthname()` 函数。
...
下面的程序演示了模式作为 3 传递时的 `jdmonthname()` 函数。
...
下面的程序演示了模式作为 4 传递时的 `jdmonthname()` 函数。
...
下面的程序演示了当模式传递超出范围时的 `jdmonthname()` 函数。
