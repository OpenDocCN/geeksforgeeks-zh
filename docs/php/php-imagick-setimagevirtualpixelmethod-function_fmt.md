...
`Imagick::setImageVirtualPixelMethod()` 函数是 PHP 中的内置函数，用于设置图像虚拟像素方法。
...
**参数：** 此函数接受单个参数 `$method`，该参数包含与 `VIRTUALPIXELMETHOD` 常量之一相对应的整数值。 我们也可以像 `setImageVirtualPixelMethod(imagick::VIRTUALPIXELMETHOD_BLACK);` 那样直接传递常量。
...
*   `Imagick::VIRTUALPIXELMETHOD_CONSTANT(2)`
*   `Imagick::VIRTUALPIXELMETHOD_TRACE(8)`
