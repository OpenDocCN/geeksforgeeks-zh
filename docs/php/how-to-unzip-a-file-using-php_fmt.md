...
> 原文：[https://www.geeksforgeeks.org/how-to-unzip-a-file-using-php/](https://www.geeksforgeeks.org/how-to-unzip-a-file-using-php/)
...
*   **Destination:** 您可以使用 `$destination` 参数来指定文件的解压位置。
*   **Entry:** 您可以使用 `$entries` 参数来指定要解压的单个文件名，或者传递一个文件数组。
...
创建一个 `ZipArchive` 类的对象，并使用 `$zip->open()` 方法打开一个给定的 zip 文件。如果返回真，则使用 `extractTo()` 方法将文件提取到指定的路径，方法是在其中传递路径地址作为参数。
...
使用文件元素，可以选择想要提取的 zip 文件。如果所选文件有效，则传递到 `open()` 方法，并使用 `extractTo()` 方法将其提取到指定路径。