...
> 原文：[https://www.geeksforgeeks.org/multidimensional-associative-array-in-php/](https://www.geeksforgeeks.org/multidimensional-associative-array-in-php/)
...
## 说明
在上面的程序中，父索引是 `Python` 和 `PHP`。父键与一组具有常数值的键相关联。最后一个键，即每个父键的 `description` 已经与该组键和常数值的另一个数组相关联。这里，`Python` 和 `PHP` 是 `first_release`、`latest_release`、`designed_by` 和 `description` 的父键，而 `description` 是 `extension`、`typing_discipline` 和 `license` 的父键。

## 检索值
我们可以使用以下方法检索多维数组的值：

### 使用键
我们可以使用关联数组的键来直接检索数据值。
...
### 使用 foreach 循环
我们可以使用 `foreach` 循环来检索多维关联数组中关联的每个键的值。
