...
PHP 中的 `fgetss()` 函数是一个内置函数，用于在从打开的文件中删除 HTML 和 PHP 标记后，从打开的文件中返回一行。

`fgetss()` 函数在指定长度、文件末尾（EOF）或新行（以先出现者为准）停止返回。要读取的文件和要读取的字节数作为参数发送给 `fgetss()` 函数，并从用户指定的文件返回长度为 `length - 1` 字节的字符串。它在失败时返回 `FALSE`。
...
2.  `length`：它指定 `fgetss()` 函数要读取的字节数。默认值为 `1024` 字节。
3.  `tags`：可选参数，用于指定不应条带化的标签。

## 返回值：

它在删除所有 HTML 和 PHP 标记后，从用户指向的文件返回一个长度为 `length - 1` 字节的字符串。
