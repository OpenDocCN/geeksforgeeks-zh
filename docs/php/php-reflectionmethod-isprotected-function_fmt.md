# `ReflectionMethod::isProtected()` 函数

> 原文：[https://www.geeksforgeeks.org/php-reflectionmethod-isprotected-function/](https://www.geeksforgeeks.org/php-reflectionmethod-isprotected-function/)

`ReflectionMethod::isProtected()` 函数是 PHP 中的一个内置函数，用于在指定的方法受保护时返回 `TRUE`，否则返回 `FALSE`。
...
如果指定的方法受保护，则此函数返回 `TRUE`，否则返回 `FALSE`。
