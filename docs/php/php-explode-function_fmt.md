...
`explode()` 是 PHP 中的一个内置函数，用于将一个字符串拆分成不同的字符串。函数的作用是：根据字符串分隔符拆分字符串，即在出现分隔符的任何位置拆分字符串。此函数返回一个数组，其中包含通过拆分原始字符串形成的字符串。
...
`explode()` 函数接受三个参数，其中两个是必选的，一个是可选的。下面描述了所有这三个参数：
