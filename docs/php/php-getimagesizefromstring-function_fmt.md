...
`getimagesizefromstring()`函数是 PHP 中的一个内置函数，用于从字符串中获取图像的大小。 此函数接受图像数据作为字符串参数，并确定图像大小并返回尺寸以及图像的文件类型和高度/宽度。
...
此函数返回尺寸以及文件类型和高度/宽度文本字符串。

下面的程序演示了 PHP 中的`getimagesizefromstring()`函数：
