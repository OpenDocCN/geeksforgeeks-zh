...
> 原文：[https://www.geeksforgeeks.org/how-to-get-the-last-character-of-a-string-in-php/](https://www.geeksforgeeks.org/how-to-get-the-last-character-of-a-string-in-php/)
...
在这个方法中，我们会找到字符串的长度，然后打印 `(length-1)` 的值。例如，如果字符串是 “Akshit”，它的长度是 6，在零索引中，`(length-1)` 的值是 5，这是字符 “t”。长度可以使用 PHP `strlen()` 方法找到。
...
`substr()` 是 PHP 中的内置函数，用于提取字符串的一部分。
...
从最后看，“s” 排在第 1 位。我们可以显示从 `-1` 开始的字符串。
