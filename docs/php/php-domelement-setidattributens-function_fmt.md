...
`DOMElement::setIdAttributeNS()` 函数是 PHP 中的一个内置函数，用于声明给定本地名称和命名空间 URI 指定的属性为 ID 类型。
...
如果节点为只读，则此函数抛出 `DOM_NO_MODIFICATION_ALLOWED_ERR`；如果名称不是此元素的属性，则抛出 `DOM_NOT_FOUND`。
