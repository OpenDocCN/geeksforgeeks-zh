...
`ReflectionMethod::isDestructor()` 函数是 PHP 中的内置函数，用于在指定的方法是析构函数时返回 `true`，否则返回 `false`。
...
如果指定的方法是析构函数，则此函数返回 `TRUE`，否则返回 `FALSE`。
...
[https://www.php.net/manual/en/reflectionmethod.isdestructor.php](https://www.php.net/manual/en/reflectionmethod.isdestructor.php)