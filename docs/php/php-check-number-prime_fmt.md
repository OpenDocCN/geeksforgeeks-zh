...
一个简单的解决方法是迭代从 2 到 `$number/2` 的所有数字，对于每个数字检查它是否整除 `$number`，如果我们找到任何能整除的数字，我们返回 `0`（假），否则我们将返回 `1`（真）。
...
我们可以通过观察来优化上述方法，我们可以检查直到 `sqrt($number)`，而不是检查直到 `$number/2`，因为 `$number` 的较大因子必须是已经检查过的较小因子的倍数。
