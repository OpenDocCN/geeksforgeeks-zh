...
函数`imagecreatefromwebp()`是 PHP 中的一个内置函数，用于从 WebP 文件或 URL 创建新图像。WebP 是一种同时采用有损和无损压缩的图像格式。这个图像可以在程序中进一步处理。当您想要在从 WebP 文件加载图像后对其进行编辑时，通常使用此功能。可以使用`imagewebp()`函数将图像转换为 WebP。
...
下面给出的程序演示了 PHP 中的`imagecreatefromwebp()`函数：
...
[https://www.php.net/manual/en/function.imagecreatefromwebp.php](https://www.php.net/manual/en/function.imagecreatefromwebp.php)