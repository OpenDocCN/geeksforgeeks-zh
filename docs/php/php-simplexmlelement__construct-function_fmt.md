...
`__construct()` 函数是 PHP 中的一个内置函数，用于为 XML 创建一个新的 `SimpleXMLElement` 对象。
...
它返回一个表示 XML 结构化数据的 `SimpleXMLElement` 对象。
...
*   `LIBXML_PARSEHUGE` - 设置 `XML_PARSE_HUGE` 标志，该标志从解析器中移除任何硬编码限制，这将再次影响文本节点大小和文档最大深度等限制
