...
这个程序是关于 `current()` 方法的简单功能。它将创建一个具有指向同一实例的新名称的变量，并在新实例的帮助下显示旧变量的内容。
...
它使用第二个变量对图像执行模糊操作，更改将反映在第一个变量上，因为这两个变量都指向同一实例。
...
[https://www.php.net/manual/en/imagick.current.php](https://www.php.net/manual/en/imagick.current.php)