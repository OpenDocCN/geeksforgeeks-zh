...
`ReflectionFunction::export()` 函数是 PHP 中的一个内置函数，用于在返回参数设置为 `true` 时以字符串形式返回导出，否则返回 `NULL`。
...
*   `$name`: 要导出的指定函数。
*   `$return`: 布尔值 `TRUE` 或 `FALSE`。如果其值设置为 `True`，则将被导出；如果其值设置为 `False`，则将返回 `NULL`。默认为 `False`。

## 返回值

如果返回参数设置为 `true`，则此函数以字符串形式返回导出，否则返回 `NULL`。
...
[https://www.php.net/manual/en/reflectionfunction.export.php](https://www.php.net/manual/en/reflectionfunction.export.php)