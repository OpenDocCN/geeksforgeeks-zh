...
> 原文: [https://www.geeksforgeeks.org/tailwind-css-transition-property/](https://www.geeksforgeeks.org/tailwind-css-transition-property/)
...
*   `transition-none`: 该值用于指定没有类会获得过渡效果。
*   `transition-all`: 所有的职业都会获得一个过渡效果。这也是该类的默认值。
*   `transition`: 我们可以指定将要应用过渡效果的 CSS 属性的名称。我们还可以通过用逗号分隔来指定多个属性。
*   `transition-colors`: 该值用于指定颜色类将获得过渡效果。
*   `transition-opacity`: 该值用于指定不透明度类将获得过渡效果。
*   `transition-shadow`: 此值用于指定阴影类将获得过渡效果。
*   `transition-transform`: 该值用于指定变换成已定义的形状。
