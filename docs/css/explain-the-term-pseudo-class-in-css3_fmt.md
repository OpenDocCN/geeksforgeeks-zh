...
> 原文: [https://www.geeksforgeeks.org/explain-the-term-pseudo-class-in-css3/](https://www.geeksforgeeks.org/explain-the-term-pseudo-class-in-css3/)
...
*   **选择器:** 选择器告知要设置样式的 `HTML` 元素。在这种情况下，选择器是 `h1` 元素。
*   **声明:** 每个声明包括一个 `CSS` 属性名和值，用分号分隔。在这种情况下，它的 `font-size` 和 `background-color`。

**示例:** 在本例中，所有 `p` 元素内部的文本将居中对齐，它将具有 `25px` 的[字体大小](https://www.geeksforgeeks.org/css-font-size-property/)和 `green` 颜色。
...
1.  **语言伪类:** 可以根据脚本方向或语言选择元素。例子有 `:dir`、`:lang`。
2.  **用户动作伪类:** 这些是特定于用户交互的，以便应用一种风格，例如鼠标悬停。例如 `:hover`、`:focus`。
3.  **资源状态伪类:** 应用于能够处于可以描述为暂停、播放等状态的媒体元素。
4.  **结构伪类:** 与文档树中某个元素的位置有关。例如 `:last-child`、`:nth-child`。
5.  **输入伪类:** 主要与表单等输入元素相关，支持基于 `HTML` 属性选择元素，说明字段在交互前后。例子有 `:checked`、`:valid`。
6.  **时间维伪类:** 与轨迹等有时序的东西交互时应用。例子有 `:future`、`:current`。
7.  **位置伪类:** 它以当前文档中与链接相关的元素为目标。例子有 `:scope`、`:visited`。

**示例 1:** 以下两个示例演示了伪类。`:hover` 伪类属于用户动作伪类范畴。当用户使用指向设备与元素交互时，它适用，而不必激活它。当鼠标悬停在 `h1` 元素上时，文本会带有下划线。
...
**示例 2:** 点击链接时页面加载颜色发生变化。`:visited` 伪类属于位置伪类。它应用于用户已经访问过的链接。由于隐私的原因，它所应用的样式非常有限。一旦我们访问[“极客网站”](https://www.geeksforgeeks.org/)网站，链接的颜色就会变成 `green`。
