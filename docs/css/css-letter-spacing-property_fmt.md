...
CSS 中的`letter-spacing`属性用于设置文本字符之间的间距行为，即增加或减少文本中字符之间的间距。
...
### 正常
当前字体的正常字母间距，即字符之间没有多余的间距。这是默认值。
...
### length
This mode specifies extra inter-character space in addition to the default space between characters. Negative values are also allowed.
