...
*   `none`：用于将 `grid-column-gap` 属性设置为默认值。`grid-column-gap` 的默认值为 `0`。
*   `length`：列间间隙的大小以长度给出。长度值可以是 `px`、`em` 等形式。该值必须非负。
*   `initial`：用于将 `grid-column-gap` 属性设置为默认值。
*   `inherit`：此属性从其父级继承。
...
CSS `grid-column-gap` 属性支持的浏览器如下：
