...
<body>
    <h1>GeeksforGeeks</h1>
    <img src="https://media.geeksforgeeks.org/wp-content/uploads/geek.png" width="400" height="150">
...
在`示例-1`中，`z-index`设置为`-1`，因此图像出现在文本后面，但是在`示例-2`中，当`z-index`设置为`+1`时，图像隐藏了文本。

## 支持的浏览器:

由 `z-index` 属性支持的浏览器如下:
