...
这个类在 [顺风 CSS](https://www.geeksforgeeks.org/css-tailwind-introduction/) 中接受大量的值，其中所有的属性都包含在类形式中。此类用于设置表格中单元格的边框，并告知这些单元格是否共享一个公共边框。在 CSS 中，我们可以通过使用 [CSS 边框折叠属性](https://www.geeksforgeeks.org/css-border-collapse-property/)来实现。
