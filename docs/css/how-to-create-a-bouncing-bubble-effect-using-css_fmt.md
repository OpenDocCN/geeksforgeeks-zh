...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-bouncing-bubble-effect-using-css/](https://www.geeksforgeeks.org/how-to-create-a-bouncing-bubble-effect-using-css/)

让一个网站变得更加**动态**和**吸引人**的一个简单方法是给它添加一些**动画**。一个这样的动画是**弹跳气泡效果**。

## 方法

基本思想是使用 `<span>` 元素创建一个截面，给它一个圆形，然后通过使用 CSS 动画属性 `translateY`，气泡可以沿着 Y 轴上下移动。可以遵循以下步骤来获得期望的结果。
...
1.  使用 `span` 元素创建几个容器，如下所示：
    ...
    ```
2.  要使容器看起来是球形的，边界半径必须更改为：`border-radius: 50%;`
3.  为了给球体一个泡泡般的效果，不透明度和背景颜色可以改变为：`background-color: #DF0101; opacity: 0.8;`
