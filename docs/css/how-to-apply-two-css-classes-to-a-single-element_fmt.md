...
> 原文：[https://www.geeksforgeeks.org/how-to-apply-two-css-classes-to-a-single-element/](https://www.geeksforgeeks.org/how-to-apply-two-css-classes-to-a-single-element/)
...
然后，可以使用`.class_1`和`.class_2`单独设置类的样式。或者该元素可以只包含使用`.class_1.class_2`的两个类。
...
在上例中，类`.para`的样式应用于两段，而类`.second_para`的样式仅应用于第二段。

## 包含两个类的样式元素

仅包含两个类的元素将被样式化。包含这两个类中的一个或零个的所有其他元素都不会被设置样式。
...
我们也可以使用 JavaScript 添加和移除类。我们将使用标签的`classList`属性，该属性将类名作为 DOMTokenList 对象返回。我们将使用`add()`方法动态地给一个元素添加多个类。

### `add(class_1, class_2, …)`

用于将一个类或多个类赋给 HTML 内部的一个元素。
