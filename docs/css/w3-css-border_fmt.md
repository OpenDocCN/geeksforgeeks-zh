...
> 原文：`https://www.geeksforgeeks.org/w3-css-border/`
...
| 4。 | `w3-border-right` | 它是用来给目标元素添加右边界的。 |
| 5。 | `w3-border-left` | 用于给目标元素添加左边框。 |
...
可以使用 `W3.CSS` 中提供的以下`边框颜色类`将任何颜色添加到边框中。如果您想要任何其他自定义颜色，可以使用 CSS 属性手动设置。
...
您可以使用 `W3.CSS` 边框类向目标元素的任何一侧添加一个粗边框。

<figure class="table">

| 序号 | 类名 | 说明 |
| --- | --- | --- |
| 1。 | `w3-bottombar` | 用于在目标元素的底部添加粗边框。 |
| 2。 | `w3-leftbar` | 用于在目标元素的左侧添加粗边框。 |
| 3。 | `w3-rightbar` | 用于在目标元素的右侧添加粗边框。 |
| 4。 | `w3-topbar` | 用于在目标元素的顶部添加粗边框。 |
