...
> 原文: [https://www.geeksforgeeks.org/what-is-at-rules-and-why-use-of-at-in-css/](https://www.geeksforgeeks.org/what-is-at-rules-and-why-use-of-at-in-css/)

在本教程中，我们将学习 At 规则及其在 CSS 中的用法。At-rules 只是一些指示 CSS 在特定条件下如何行为的 CSS 语句。ie。At-规则允许开发人员根据特定条件更改网站的设计或布局。每个 At-rule 命令都以``@``开头，后跟一个`identifier`，以分号`;`结尾或下一个 CSS 区块。
...
定义网站要使用的字符集。如果 CSS 文件包含非 ASCII 字符，我们可以很容易地使用它。可以使用的字符集有 `UTF-8`、`iso-8859-15`、`iso-8859-1` 等。您可以查看任何特定的字符集，但最常见和最受欢迎的字符集是网站上的 `UTF-8`。
...
用于从位于本地系统或互联网的其他 CSS 文件中导入 CSS 样式或规则。通常在指示包含外部样式表的样式表顶部提到它。下面的例子说明了我们通常使用的各种`@import`。
...
用来告诉 CSS 引擎所有的 XML 命名空间都要用在 CSS 样式表中。
...
用于基于媒体查询结果的 HTML 代码的一部分或一个块。例如，使用媒体屏幕宽度制作响应导航栏。
...
**说明:** 在这里，当屏幕宽度小于 450 px 时，标题的颜色将为红色，否则为绿色。`@media` 与 `screen` 一起使用，以获得我们想要检查屏幕宽度的正确上下文，条件是“最小宽度:450px”。您也可以使用更多的查询。
...
用于了解浏览器的支持能力，即。如果条件满足，那么它将为这些元素应用样式。例如，如果浏览器支持网格。
...
在这种情况下，如果浏览器不支持`display: grid`属性，那么可以使用下面的代码:
...
用于修改一些 CSS 属性&在打印文档时将样式应用到单个页面。
...
指定显示文本的自定义字体。字体可以从服务器或系统的任何地方加载。首先，我们必须定义字体名称，然后提供要加载的字体的网址。
...
语法非常清楚。首先，我们已经将容器与动画链接起来。接下来，我们使用了与`#page` 中同名的`@keyframes` 来制作动画。
...
它允许您定义不属于预定义样式集的计数器样式。一个 `@counter-style` 规则定义了如何将一个计数器值转换成另一个字符串。
