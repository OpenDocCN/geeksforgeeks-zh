...
## 示例 1：使用 `margin: auto` 居中对齐

`margin: auto` 属性用于将块元素对齐到中心。

**注意:** 在 IE8 中使用 `margin: auto` 无效，除非声明了 `!DOCTYPE`。

本示例使用 `margin: auto` 属性描述 CSS 对齐。
...
## 示例 2：使用 `position: absolute` 对齐

我们可以使用 `position: absolute` 属性来对齐项目。

本示例使用 `position: absolute` 属性描述 CSS 对齐。
...
## 示例 3：使用 `text-align: center` 对齐文本

我们可以将任何用 HTML 编写的文本对齐在中间。我们可以在各种标签中使用 `text-align: center` 属性。

本示例使用 `text-align: center` 属性描述 CSS 对齐。
...
## 示例 4：使用 `padding` 垂直对齐

要垂直对齐项目，我们可以使用 `padding`。

本示例使用 `padding` 属性描述 CSS 对齐。
...
## 示例 5：使用 `padding` 和 `text-align` 组合对齐

使用 `padding` 和 `text-align` 的组合来垂直和水平对齐文本。

本示例使用 `padding` 和 `text-align` 属性描述 CSS 对齐。
