...
> 原文: [https://www.geeksforgeeks.org/different-ways-to-hide-elements-using-css/](https://www.geeksforgeeks.org/different-ways-to-hide-elements-using-css/)
...
当我们在 CSS 中对一个元素使用 `position: absolute` 属性时，它仅仅意味着该元素的位置固定到它的父容器，如果没有可用的容器，那么文档体就被用作它的父容器。现在我们可以使用 `top`、`bottom`、`left`、`right` 属性来更改该元素的位置。我们将使用这些属性之一来隐藏文档主体中的元素。

**示例:** 在本例中，我们将通过将文本元素移出屏幕来隐藏它。我们将把 `h1` 标签的位置设置为绝对的，点击一个按钮，我们将添加一个类，其中我们将使用 `left` 属性，并将值设置为 `-999px`。通过这样做，文本将从显示屏中移出。我们还可以使用其他 `top`、`bottom` 和 `right` 属性来隐藏文档中的元素。
...
颜色属性也可以通过使元素透明来使其不可见，我们将使用 alpha 通道应用颜色。我们可以用下面的方法设置 alpha 通道。

1.  `color: transparent`
2.  `color: hsla(色相、饱和度、明度、alpha);`
3.  `color: rgba(红、绿、蓝、alpha);`
4.  `color: #RRGGBBAA`
5.  `color: #RBGA`

**注意:** 我们也可以使用颜色以外的这些值，例如 `background-color`、`border-color`、`background` 等。
...
该属性用于根据给定的形状创建剪辑原因，我们将使用该属性隐藏元素。剪辑路径属性有各种值，其中之一是 `circle()`。当我们使用 `clip-path: circle(0)` 时，特定元素将被完全隐藏。
...
CSS 的 `display` 属性也可以用来隐藏 DOM 中的元素。我们将使用它的一个被称为 `display: none` 的值，这可能是最常用的属性来隐藏网页中的 HTML 元素。

**示例:** 隐藏显示元素: `display: none`。
...
CSS 中的 `filter` 属性用于对 HTML 元素应用一些图形失真。`Filter` 属性有几个值，但是为了隐藏元素，我们将使用它的一个属性，称为 `opacity()`。顾名思义，我们将使用最低的不透明度值来使元素透明，这样就可以隐藏任何特定的元素。

**示例:** 使用滤镜隐藏元素: `filter: opacity()` 属性。
...
这也可能是隐藏任何元素的最简单方法之一。我们将借助 `height`、`width`、`font-size` 等属性来缩小元素的尺寸。请注意，当我们使用维度来减少或完全隐藏元素时，不要忘记使用 `overflow: hidden` 属性，以便元素的完整内容被隐藏。
...
我们可以直接使用 CSS 中的 `opacity` 属性，给元素应用一个透明效果。我们将使用不透明度值从 `0` 到 `1` 或百分比从 `0%` 到 `100%`。但是在这种情况下，我们将使用 `opacity(0)` 或 `opacity(0%)` 来使元素透明或完全隐藏元素。
...
**9。可见性:** 该属性只有两个值 `hidden` 和 `visible`。对于隐藏页面中的元素，我们将简单地使用 `visibility: hidden`。这是对 DOM 隐藏 HTML 元素最简单的方法之一。

**示例:** 隐藏可见性元素: `visibility: hidden` 属性。

# 使用CSS visibility属性隐藏元素
