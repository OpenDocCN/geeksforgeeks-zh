...
`mask-composite` 属性定义了多个复合操作来组合多个遮罩层。第一个网址层在所有层的顶部，最后一个网址层在底部。
...
*   **关键字值:** 该属性值是指用 `add`、`subtract`、`intersect`、`exclude` 等单位定义的值。
*   **全局值:** 该属性值是指用 `inherit`、`initial`、`unset` 等单位定义的值。

**示例 1:** 以下示例使用 `add` 来说明 `mask-composite` 属性:
...
**示例 2:** 以下示例使用 `subtract` 来说明 `mask-composite` 属性:
