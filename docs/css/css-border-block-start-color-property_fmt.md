...
> 原文：[https://www.geeksforgeeks.org/css-border-block-start-color-property/](https://www.geeksforgeeks.org/css-border-block-start-color-property/)

CSS 中的 `border-block-start-color` 属性用于在样式表的一个地方定义单个逻辑块开始边框属性值。此属性设置定义元素边框顶部的边框颜色。
...
下面的例子说明了 CSS 中的 `border-block-start-color` 属性：
...
`border-block-start-color` 属性支持的浏览器如下：
