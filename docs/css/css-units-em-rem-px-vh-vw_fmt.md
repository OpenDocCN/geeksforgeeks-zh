...
**例 2:** 另一种类型的相对宽度称为视图宽度(`vw`)和视图高度(`vh`)。`1vw`等于整个屏幕尺寸的 1 %,因此`100vw`将占据整个宽度，而`50vw`显然将占据一半宽度，但是关于`vw`与百分比的重要一点是，视口单位基于整个屏幕尺寸，而百分比是相对于其父级的。
...
**例 3:** `rem`单位和`em`单位都是相对的，但不是相对于它们周围的东西，比如它们父元素的宽度或高度，而是实际上相对于字体大小。
