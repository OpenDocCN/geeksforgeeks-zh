...
## 步骤 1:
*   创建一个半径为 `50%`，宽度和高度为 `180px` 的黑色圆。
*   为了保持圆圈在中间，在它的左边加上 `350px`。
*   同样，为了使圆圈可见，添加一个背景色作为 `currentColor`。
...
*   将 `before` 元素添加到 `div` 中，并添加 `position`、`top`、`width`、`height` 和 `border`。
*   也添加 `border-radius`，直到弧线需要形成小胡子的地方。
*   固定 `transform-origin`，使左髭末端准确到来，然后以 `-40deg` 角旋转。
...
*   在 `before` 元素中添加 `position`、`top`、`width`、`height` 和 `border`。
*   也添加 `border-radius`，直到弧线需要形成小胡子的地方。
*   固定 `transform-origin`，使右髭的末端也准确到来，然后以 `40deg` 角旋转。
