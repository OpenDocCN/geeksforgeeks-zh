...
`scroll-margin`属性用于一次设置一个元素的所有滚动边距。为滚动边距指定的值决定了主要在支持范围之外的页面应该保持可见的程度。

因此，`scroll-margin`值代表定义滚动捕捉区域的外接，该区域用于将该框捕捉到支架。
...
*   `length`: 该属性是指用长度单位 exp: `px`、`em`、`vh` 等定义的值。
...
在本例中，您可以通过滚动到示例内容的两个“界面”中间的某个点来查看滚动边距的效果。
...
**输出:** 当滚动经过中间的子元素时，滚动将捕捉到偶数子`div`左边缘外的`2rem`，以及奇数子`div`左边缘外的`5rem`。
