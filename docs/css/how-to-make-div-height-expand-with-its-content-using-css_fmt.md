...
> 原文：[https://www.geeksforgeeks.org/how-to-make-div-height-expand-with-its-content-using-css/](https://www.geeksforgeeks.org/how-to-make-div-height-expand-with-its-content-using-css/)
...
*   `height: auto;` 用于将高度属性设置为默认值。如果高度属性设置为自动，浏览器将计算元素的高度。
*   `height: length;` 用于以 `px`、`cm` 等形式设置元素的高度。长度不能为负。
*   `height: initial;` 用于将高度属性设置为默认值。
*   `height: inherit;` 用于从其父元素设置高度属性。

## 例 1：

本例使用 `height: auto` 属性来显示内容。
