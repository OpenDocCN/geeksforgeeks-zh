...
> 原文：[https://www.geeksforgeeks.org/css-inset-inline-start-property/](https://www.geeksforgeeks.org/css-inset-inline-start-property/)

CSS 中的 `inset-inline-start` 属性用于定义逻辑内嵌开始偏移量，而不是块偏移量或逻辑块。此属性可以应用于任何写入模式属性。
...
*   `percentage`: 与窗口大小百分比设置的长度相同。
*   `auto`: 当希望浏览器确定块大小时使用。
*   `initial`: 用于将 `inset-inline-start` 属性的值设置为默认值。
*   `inherit`: 当需要元素从其父元素继承 `inset-inline-start` 属性时使用。
*   `unset`: 用于取消设置默认内嵌开始。

下面的例子说明了 CSS 中的 `inset-inline-start` 属性。
