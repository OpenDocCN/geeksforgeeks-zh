...
这个类在[顺风 CSS](https://www.geeksforgeeks.org/css-tailwind-introduction/)中接受大量的值，其中所有的属性都以类的形式被覆盖。它是[CSS 列表样式类型属性](https://www.geeksforgeeks.org/css-list-style-type-property/)的替代。如果`list-style-image`的值为`none`，则此类指定列表项标记的外观(如光盘、字符或自定义计数器样式)。

## 列出样式类型类别

*   `list-none`: 此模式下不显示标记。
*   `list-disc`: 这是默认值。标记是一个实心圆。
*   `list-decimal`: 标记是一个小数，以 1 开头。
