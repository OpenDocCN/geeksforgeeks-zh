...
> 原文：[https://www.geeksforgeeks.org/how-to-set-multiple-background-images-using-css/](https://www.geeksforgeeks.org/how-to-set-multiple-background-images-using-css/)

一个元素的多个背景图像可以用 CSS 放在 HTML 页面中。使用 CSS 背景属性为任何模式中的元素添加多个背景图像，并使用其他 CSS 属性设置图像的高度和宽度。

## 下面列出了使用的背景属性：

*   `background-image: url(), url(), ...;` 此属性用于为元素设置一个或多个背景图像，用逗号分隔。
*   `background-position: right bottom, left top;` 此属性用于设置页面中不同图像的位置。它为每个背景图像设置初始位置。
*   `background-repeat: no-repeat, repeat;` 该属性用于设置背景图像的重复。背景图像可以沿着水平轴和垂直轴重复。
*   `background-size: cover | contain | 30% | 200px 100px;` 该属性用于设置元素背景图像的大小。
