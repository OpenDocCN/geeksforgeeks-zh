...
**堆叠形态**:将类`pure-form-stacked`添加到任意 [`<form>`](https://www.geeksforgeeks.org/html-forms/) 元素中，创建堆叠形态。

**注意:**通过在`<form>`类中添加`pure-form`来创建默认表单，然后您可以创建任何特定类型的表单，如堆叠、对齐等。

**手动添加链接:**现在已经在机器上下载了 `pure.css` zip 文件。提取它，并把它放在你的网站目录。您可以使用 [`<link>`](https://www.geeksforgeeks.org/html-link-tag/) 标签将其附加到您的主 HTML 文档中。
