...
> 原文：[https://www.geeksforgeeks.org/css-border-block-end-color-property/](https://www.geeksforgeeks.org/css-border-block-end-color-property/)

CSS 中的 `border-block-end-color` 属性用于在样式表的一个地方定义单个逻辑块结束边框属性值。此属性设置定义元素边框底部的边框颜色。
...
以下示例说明了 CSS 中的 `border-block-end-color` 属性：
...
`border-block-end-color` 属性支持的浏览器如下：
