...
> 原文: [https://www.geeksforgeeks.org/css-page-break-before-property/](https://www.geeksforgeeks.org/css-page-break-before-property/)

CSS 中的分页符属性用于在指定元素之前添加分页符。

此属性有助于定义文档打印时的行为。类似地，`page-break-before`、`page-break-after` 和 `page-break-inside` 这三个属性都有助于确定并定义打印结果文档的方式。
