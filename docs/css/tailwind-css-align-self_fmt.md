...
这个类接受 [Tailwind CSS](https://www.geeksforgeeks.org/css-tailwind-introduction/) 中的很多值。它是 [CSS `align-self`](https://www.geeksforgeeks.org/css-align-self-property/) 属性的替代物。此类用于控制单个 flex 或 grid 项如何沿其容器的横轴定位。
...
该类用于继承其父容器 `align-items` 属性，如果没有父容器，则用于拉伸。这是一个默认值。
