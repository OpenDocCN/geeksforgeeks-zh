...
这个类在`tailwind` CSS中接受多个值。所有的属性都包含在类的形式中。它是 [CSS clear属性](https://www.geeksforgeeks.org/css-clear-property/#:~:text=The%20clear%20property%20is%20used,which%20is%20floated%2C%20it%20will.)的替代物。此类用于指定浮动元素的哪一侧不允许浮动。它设置或返回元素相对于浮动对象的位置。如果该元素可以水平放置在另一个浮动元素旁边的空间中，它将会。基本上，它用于控制元素周围内容的包装。
