...
> 原文: [https://www.geeksforgeeks.org/parallax-scrolling-effect-using-css/](https://www.geeksforgeeks.org/parallax-scrolling-effect-using-css/)
...
首先，让我们了解给定示例中正在发生的事情。创建效果是因为背景中的图像保持固定，没有移动，但其他图像在移动。这种简单的技术让这个效果看起来很棒。
...
该属性用于确定背景图像是 `fixed` 还是 `scroll`（随页面滚动）。
...
*   `cover` – 将背景图像缩放到尽可能大，以便背景区域完全被背景图像覆盖。
