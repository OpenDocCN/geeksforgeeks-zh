...
`:nth-child()` CSS [`伪类`](https://www.geeksforgeeks.org/css-pseudo-classes/)选择器用于根据元素在一组兄弟中的位置来匹配元素。它匹配第 n 个子元素的每个元素，不管其父元素的类型如何。
...
其中 `number` 是代表匹配元素模式的单个参数。它可以是 `odd`、`even`，也可以是函数式符号。

*   **`odd`:** 表示一系列中位置为奇数的元素: 1、3、5 等。
*   **`even`:** 表示位置在 2、4、6 等数列中偶的元素。
...
**支持的浏览器:** 以下是 `:nth-child()` 选择器支持的浏览器:
