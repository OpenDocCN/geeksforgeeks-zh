...
> 原文：[https://www.geeksforgeeks.org/how-to-detect-touch-screen-device-using-css/](https://www.geeksforgeeks.org/how-to-detect-touch-screen-device-using-css/)
...
仅 HTML 无法检测触摸屏设备。除了 HTML，我们还需要 CSS 媒体查询。在 CSS 媒体查询中，我们有一个名为 `pointer` 的功能，用于检测用户使用的指点设备的指点精度。它有以下 3 个值。

*   `pointer: none`：当设备的输入机制没有指向设备时触发。
*   `pointer: coarse`：当指点设备的输入机制精度较低时触发。例如，触摸屏。
*   `pointer: fine`：当指点设备的输入机制精度较高时触发。例如，鼠标、触控板、手写笔等。
