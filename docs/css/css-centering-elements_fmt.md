...
有时，我们会遇到在网页中居中元素的问题。真的这么难吗？元素居中并不太难。有这么多不同的方法。有一点我们需要知道的是，哪种技术是为了哪种目的。一旦你理解了这个问题，学会最好的技巧就会容易得多。那么让我们看看一些情况，讨论一下实现目标的最佳方法。
...
如果我们有两个或两个以上的块级元素需要水平居中排成一行，那么让它们成为不同的显示类型 `display: inline-block` 会更好：
