...
> 原文：[https://www.geeksforgeeks.org/explain-nesting-and-grouping-in-css/](https://www.geeksforgeeks.org/explain-nesting-and-grouping-in-css/)
...
**输出：** 我们使用嵌套获得了绿色的 `<a>` 标签和米色的 `<th>` 标签。
...
*   在 `<style>` 标签内添加 `<head>`。
*   在 `<head>` 标签里面添加各种 `<style>` 标签，里面有内容。
*   在 `<body>` 标签内添加内容。
...
| 2. | 这可能有助于简化并一次管理不同元素的属性，但是如果包含相同属性的嵌套元素的数量增加，这可能会变得复杂。管理这样的嵌套属性可能很困难。 | 一次将属性应用于不同元素的数量很简单并可以轻松管理。 |
