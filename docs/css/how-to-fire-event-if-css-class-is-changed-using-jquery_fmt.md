...
> 原文：[https://www.geeksforgeeks.org/how-to-fire-event-if-css-class-is-changed-using-jquery/](https://www.geeksforgeeks.org/how-to-fire-event-if-css-class-is-changed-using-jquery/)

给定一个带有标题和按钮的 HTML 文档，当按钮的 CSS 类改变时，jQuery 应该触发一个事件。

## 方法

在 jQuery 中，没有关于类变化触发事件的规定。另一种方法是，当您使用 [`trigger()` 函数](https://www.geeksforgeeks.org/jquery-trigger-method/)以编程方式更改类时，手动引发事件。每当按钮的等级改变时，`trigger()` 功能将引发一个事件。
