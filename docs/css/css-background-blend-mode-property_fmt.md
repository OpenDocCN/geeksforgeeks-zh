...
> 原文：[https://www.geeksforgeeks.org/css-background-blend-mode-property/](https://www.geeksforgeeks.org/css-background-blend-mode-property/)

`background-blend-mode` 属性定义元素的背景图像应该如何相互混合以及如何与元素的背景颜色混合。
...
### screen
设置混合模式为屏幕。在这种模式下，图像和颜色都被反转、相乘然后反转。
...
### darken
将混合模式设置为变暗。在这种模式下，如果背景图像比背景颜色暗，则图像被替换，否则，图像保持原样。
...
### lighten
设置混合模式变亮。在这种模式下，如果背景图像比背景颜色浅，则图像被替换，否则，图像保持原样。
...
### color-dodge
设置混合模式为颜色减淡。在这种模式下，背景色被背景图像的反转所分割。这与屏幕混合模式非常相似。
...
### saturation
设置混合模式为饱和度。该模式保持背景图像的饱和度，同时混合背景颜色的色调和亮度。
...
### difference
设置混合模式为差异。该模式是从最亮的背景图像中减去较暗的背景图像颜色和背景颜色的结果。通常图像会有很高的对比度。
...
### luminosity
将混合模式设置为亮度。在这种模式下，顶部颜色的亮度被保留，同时使用背景颜色的饱和度和色调。
...
### overlay
设置混合模式为叠加。在这种模式下，背景色与背景图像混合，以反映背景的亮度或暗度。
...
## 支持的浏览器
`background-blend-mode` 属性支持的浏览器如下：
