...
`CSS place-content` 属性是 [`align-content`](https://www.geeksforgeeks.org/css-align-content-property/) 和 [`justify-content`](https://www.geeksforgeeks.org/css-justify-content-property/) 属性的简写。CSS 中的简写属性意味着您可以在单个属性中设置多个属性值。这里，`place-content` 属性可以保存 `align-content` 和 `justify-content` 属性值。
...
*   `space-around`: 该属性在挠曲线周围平均分配空间。
*   `space-between`: 该属性以相等的间距分布挠曲线空间。
*   `space-evenly`: 该属性定义了它们之间间距相等但与角的间距不同的位置。
*   `stretch`: 该属性定义拉伸的线，以占据伸缩容器的剩余空间。这是默认值。

以下示例说明了 CSS 位置内容属性:

## 示例 1

在本例中，我们将使用以下 `place-content` 属性值: `flex-start center`。
...
这里我们将使用 `place-content:flex-start` 属性值。
...
这里我们将使用 `place-content:flex-end` 属性值。
