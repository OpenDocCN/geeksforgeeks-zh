...
**输出:** 可以看到，在应用 `display: flex` 属性后，项目已经水平对齐，因为伸缩框的默认伸缩方向是行。

![](img/79fb17dc6b5da7be4906d60c78118e3f.png)

`display: flex`

*   **[flex-direction](https://www.geeksforgeeks.org/css-flex-direction-property/):** 它设置伸缩容器主轴的方向，并指定项目将如何放置在容器内。
...
*   `row`: 柔性项沿一行水平显示。
*   `column`: 柔性项沿列垂直显示。
*   `row-reverse`: 弹性项沿一行水平显示，但顺序相反。
*   `column-reverse`: 弹性项沿列垂直显示，但顺序相反。
...
*   **[flex-wrap](https://www.geeksforgeeks.org/css-flex-wrap-property/):** 指定伸缩容器是单行还是多行。
...
*   `wrap`: 指定如果需要，伸缩项会换行，并且会多行布局。
*   `wrap-reverse`: 与 `wrap` 相同，但在这种情况下，伸缩项将以相反的顺序缠绕。
*   `initial`: 表示指定为属性初始值的值。
...
*   [**flex-flow property**](https://www.geeksforgeeks.org/css-flex-flow-property/): 是伸缩方向和伸缩包裹的简写。默认情况下，折曲方向为行，折曲方向为无折曲。
...
*   [**justify-content property**](https://www.geeksforgeeks.org/css-justify-content-property/): 它定义了项目如何沿着当前行的主/主轴定位。
...
*   `center`: 柔性物品位于容器的中心。
*   `space-between`: Flex 项目以均匀的间距分布，第一个项目将位于容器的开头，最后一个项目将位于容器的结尾。
*   `space-around`: Flex 项目以均匀的间隔分布，第一个项目的开始和最后一个项目的结束都有一半的空间。
*   `space-evenly`: 伸缩项在对齐容器内沿主轴均匀分布。
*   `initial`: 表示指定为属性初始值的值。
...
*   [**align-content property**](https://www.geeksforgeeks.org/css-align-content-property/): 当横轴有多余的空间时，对齐内容会对齐容器内的多行。它类似于对齐内容，即在主轴内对齐各个项目。
...
*   `flex-start`: 线条向容器的开始对齐。
*   `flex-end`: 线朝向容器的末端对齐。
*   `center`: 线条向容器中心对齐。
*   `space-between`: 线条均匀分布，第一项在容器的开始，最后一项在容器的结束。
*   `space-around`: 线条均匀分布，第一项开始和最后一项结束各占一半空间。
*   `stretch`(默认): 线条拉伸占用剩余空间。
...
*   [`flex-grow`](https://www.geeksforgeeks.org/css-flex-grow-property/)：它指定了与该容器内的其他项目相比，该项目将增长多少。
*   [`flex-shrink`](https://www.geeksforgeeks.org/css-flex-shrink-property/)：它指定了该物品相对于该容器内其他物品的收缩程度。
*   [`flex-basis`](https://www.geeksforgeeks.org/css-flex-basis-property/)：它指定了灵活项目的初始大小。
...
*   **flex-grow:** 它设置伸缩项的 flex-grow 属性，并定义其增长能力。默认弹性增长值为 0。
...
*   **flex-shrink:** 设置伸缩项的 flex-shrink 属性，定义伸缩项收缩的能力。默认弹性增长值为 1。
...
*   [`align-self`](https://www.geeksforgeeks.org/css-align-self-property/)：它定义了单个伸缩项如何沿横轴对齐。
...
*   **auto(默认):** 项继承父容器的 `align-items` 属性。
*   **initial:** 设置为默认值。
*   **inherit:** 项从其父元素继承 `align-self` 属性。
...
<head>
    ...
    <link rel="stylesheet" href="style_align-self.css" />
</head>
