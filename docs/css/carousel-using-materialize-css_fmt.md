...
> 原文: [https://www.geeksforgeeks.org/carousel-using-materialize-css/](https://www.geeksforgeeks.org/carousel-using-materialize-css/)
...
使用上面的代码片段，我们可以通过给出图像的来源和这些图像的超链接(如果需要)来轻松创建我们的转盘。让我们看看下面的例子。
...
同样，我们也可以通过将 jQuery `fullWidth` 选项设置为 `true` 来创建全幅滑块。我们还可以在滑块底部显示指示器。这个滑块也是触摸兼容的。

为了创建这种类型的滑块，我们有一个类作为 `carousel-slider`，在这个 `div` 中，我们放置用于创建滑块的图像，如下所示。
