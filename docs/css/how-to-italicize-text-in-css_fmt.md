...
在本文中，我们将看到如何使用 CSS 使文本倾斜。使用 CSS [`font-style`](https://www.geeksforgeeks.org/css-font-style-property/) 属性对任何网页中的文本进行斜体处理。我们有多种选项可以将其设置为斜体文本。

*   **正常:**是正常的字体样式。它与 400 相同，400 是粗体的默认数值。
*   **加粗:**它就是加粗的字体风格。和 700 一样。
*   **斜体:**为草书体。

**方法:**CSS 中 [`font-style`](https://www.geeksforgeeks.org/css-font-style-property/) 属性有三个值，分别是`普通`、`斜体`和`粗体`。CSS 值`斜体`用于文本的斜体。
...
当你将使用一个外部的 CSS 文件时，总是为你的网页使用一些默认的 CSS，这个默认的 CSS 写在`* { }`选择器中。这将从您的网页中删除默认的 CSS。

### 输出

现在，正如你在输出中看到的，我们已经使用 CSS 在我们的网页中创建了一个斜体文本，这将吸引读者阅读网页上的内容。
