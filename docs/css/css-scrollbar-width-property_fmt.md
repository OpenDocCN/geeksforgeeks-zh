...
当显示时，`scrollbar-width`属性用于设置元素滚动条的宽度或厚度。这个属性可以用在用户界面要求元素显示得更突出的页面上，缩小滚动条宽度可以给元素更多的空间。
