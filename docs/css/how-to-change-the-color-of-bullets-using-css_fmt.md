...
> 原文：[https://www.geeksforgeeks.org/how-to-change-the-color-of-bullets-using-css/](https://www.geeksforgeeks.org/how-to-change-the-color-of-bullets-using-css/)
...
*   使用额外的标记。
*   使用 CSS 样式 [`::before`](https://www.geeksforgeeks.org/css-before-selector/) 选择器。
...
添加额外标记后，列表文本和项目符号可以有不同的颜色。在下面的示例中，我们将在一个 `<span>` 范围内包含列表文本，然后定义显示结果的样式。您可以用任何 HTML 标记来括起，如 `<strong>`、`<p>` 等。根据你的需要。
...
第一步是通过 CSS 样式移除默认样式。然后为您想要的项目符号内容添加相应的样式。您可以根据需要的设计和间距编辑样式。使用自己的缩进和颜色样式创建自己的自定义项目符号。与上一步相比，没有必要添加额外的标记。旧浏览器不支持 `::before`，在这种情况下可能会影响你网站的外观。
...
![](img/c6afcaffd2d32339de0ec0f5792de75b.png)
