...
`inset()` 函数是 CSS 中的一个内置函数，与 `filter` 属性一起用于更改图像的嵌入。
...
该功能接受单个参数 `length`，用于保存应用于图像的对比度值。参数可以是 `percentage` 值或 `length`。它是强制参数。
...
<body>
    ...
    <div>
        <img src="https://media.geeksforgeeks.org/wp-content/uploads/20190807114330/GFG115.png"
             alt="Sample image">
    </div>
