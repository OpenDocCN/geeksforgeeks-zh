...
**方法:**为了使用图标，我们需要在`<head>`部分内添加所需的 CDN 链接。在 HTML 中添加预定义类的以下步骤:

*   将图标类的名称添加到任何内联 HTML 元素中。
*   `<i>`和`<span>`元素被广泛用于添加图标。
...
我们可以使用`font-size`属性使其成为大尺寸图标进行显示。下面的例子说明了`font-size`属性的使用。

### 示例 1

在本例中，我们对每个图标类&使用了`font-size`属性，其值设置为 32px。
...
在本例中，我们使用了`fa-heart`类，该类具有`font-size`属性&颜色属性，其值分别设置为 28px &红色、蓝色。
...
在本例中，我们使用了带有所需图标的`material-icons`类。
...
在本例中，我们使用了云形状的`material-icons`类。
...
我们可以使用`font-size`属性使其成为大尺寸图标进行显示。下面的例子说明了预定义类的`font-size`属性的使用。

### 示例 1

在本例中，我们对每个字形类&使用了`font-size`属性，所有的值都设置为不同的数字。
...
在本例中，我们使用了`thumbs-up`类的大拇指向上形状&将所有的值设置为不同的数字。
