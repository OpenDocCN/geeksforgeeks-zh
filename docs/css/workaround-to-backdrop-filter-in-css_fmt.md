...
> 原文: [https://www.geeksforgeeks.org/workaround-to-backdrop-filter-in-css/](https://www.geeksforgeeks.org/workaround-to-backdrop-filter-in-css/)
...
*   **`brightness()`** – 用于使图像变亮或变暗（`0%` – 更暗，`100%` – 更亮）。
...
*   **`contrast()`** – 用于设置图像的对比度（`100%` – 原图，`0%` – 更暗）。
...
*   **`grayscale()`** – 用于将图像的颜色转换为黑白。值 `0%` 表示原图，`100%` 表示完全黑白图像。
...
`opacity()` 用于设置图像的透明度。默认值为 `0%`，表示图像完全透明。
...
`sepia()` 用于将图像转换为棕褐色调，使其外观更温暖。`0%` 值代表原始图像，`100%` 代表完全棕褐色的图像。
