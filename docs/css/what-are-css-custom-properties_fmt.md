...
> 原文: [https://www.geeksforgeeks.org/what-are-css-custom-properties/](https://www.geeksforgeeks.org/what-are-css-custom-properties/)

在本文中，我们将了解 CSS 自定义属性及其实现。CSS 自定义属性也称为 CSS 变量或级联变量。这些实体包含将在整个文档中重用的特定值，可以使用 `var()` 函数访问（例如，`color: var(--primary-color);`）。前缀为 `--` 的属性名描述了自定义属性，它们的值将在使用 `var()` 函数的其他声明中使用。
...
*   `--custom-name`：接受自定义属性名称的必选参数。
*   `value`：为可选参数。它接受当自定义属性无效时使用的回退值。
