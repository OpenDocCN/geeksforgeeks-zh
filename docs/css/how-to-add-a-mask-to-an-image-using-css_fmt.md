...
> 原文：[https://www.geeksforgeeks.org/how-to-add-a-mask-to-an-image-using-css/](https://www.geeksforgeeks.org/how-to-add-a-mask-to-an-image-using-css/)

CSS 中的 `mask-image` 属性用于设置图像或文本的蒙版。它用于形成特定元素的掩膜层。您可以使用 CSS 中的 [`mask-image`](https://www.geeksforgeeks.org/css-mask-image-property/) 属性为图像添加遮罩。在本文中，您将了解蒙版图像属性的不同属性值及其不同用途。
...
*   `<image>`：使用线性渐变作为蒙版图像。
*   `inherit`：继承父级的掩码属性。
*   `initial`：应用属性即匹配源的默认设置。
*   `unset`：放弃元素的当前掩码属性。

### 例 1：使用 `<make-source>` 属性值。
