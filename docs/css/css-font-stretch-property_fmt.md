...
*   `normal`: 为默认字体脸。它不会拉伸字体。
*   `ultra-condensed`: 用于指定比正常更浓缩的字体。
*   `extra-condensed`: 用于设置文字窄于正常，不窄于超浓缩。
*   `condensed`: 用于设置文字窄于半浓缩，不窄于超浓缩。
*   `semi-condensed`: 用于设置文字窄于正常，不窄于半浓缩。
*   `semi-expanded`: 用于设置文字比正常宽，但不宽则展开。
*   `expanded`: 用于设置文本比半展开更宽，但比超展开不宽。
*   `extra-expanded`: 用于设置文字比展开的宽，但不是比超展开的宽。
*   `ultra-expanded`: 用于设置文字比超展开更宽。
...
**支持的浏览器:** `font-stretch` 属性支持的浏览器如下:
