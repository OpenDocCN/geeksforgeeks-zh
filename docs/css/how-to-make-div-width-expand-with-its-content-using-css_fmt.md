...
> 原文：[https://www.geeksforgeeks.org/how-to-make-div-width-expand-with-its-content-using-css/](https://www.geeksforgeeks.org/how-to-make-div-width-expand-with-its-content-using-css/)
...
*   `width: auto;` 用于将宽度属性设置为默认值。如果 `width` 属性设置为 `auto`，浏览器将计算元素的宽度。
*   `width: length;` 用于以 `px`、`cm` 等形式设置元素宽度。长度不能为负。
*   `width: initial;` 用于将宽度属性设置为默认值。
*   `width: inherit;` 用于从其父元素设置宽度属性。

## 例 1：

本例使用 `width: auto` 属性来显示内容。
