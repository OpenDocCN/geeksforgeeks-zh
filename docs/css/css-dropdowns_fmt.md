...
## 右对齐下拉菜单:
右对齐下拉菜单是一个浮动值向右的下拉菜单，用于在右侧屏幕上显示下拉内容。将浮动右添加到包含内容的 `div`。
...
## 图像下拉菜单:
不是下拉菜单，而是放大你悬停的图像。需要基本的 CSS 和一个图像来使它工作。
...
## 点击下拉:
这需要对 JavaScript 有基本的了解，因为它是用来运行一些函数来使点击的下拉工作的。
