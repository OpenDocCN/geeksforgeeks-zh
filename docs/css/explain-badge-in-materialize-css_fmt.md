# 在 Materialize CSS 中解释徽章？

> 原文：[https://www.geeksforgeeks.org/explain-badge-in-materialize-css/](https://www.geeksforgeeks.org/explain-badge-in-materialize-css/)

**徽章**是 Materialize CSS 非常有用的组件之一。它就像一个图标，帮助我们显示未读消息、通知、电子邮件和所有其他类似的东西。在本文中，我们将讨论如何在 Materialize CSS 中使用徽章。徽章可以通过在 `span` 元素中添加 `badge` 类来定义。我们可以使用 `new` 类为徽章添加背景。
...
以下示例显示了在 Materialize CSS 中添加徽章的多种方法。
