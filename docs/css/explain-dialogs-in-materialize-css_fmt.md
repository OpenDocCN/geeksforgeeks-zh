...
> 原文: [https://www.geeksforgeeks.org/explain-dialogs-in-materialize-css/](https://www.geeksforgeeks.org/explain-dialogs-in-materialize-css/)
...
*   `string`: 是需要在对话框中显示的字符串。
*   `time`: 显示消息字符串的时间。
*   `styleClass`: 这是添加各种风格。
*   `callback`: 这个回调是吐司一出就要调用的方法。

### 链接文件

包括物化和 jQuery CDN 到 [`<head>` 标签](https://www.geeksforgeeks.org/html-head-tag/)之前的所有其他样式表加载我们的 CSS 和 JavaScript。
