...
> 原文: [https://www.geeksforgeeks.org/how-to-use-the-position-property-in-css-to-align-elements/](https://www.geeksforgeeks.org/how-to-use-the-position-property-in-css-to-align-elements/)

在本文中，我们将学习如何使用 CSS 中的 `position` 属性来对齐元素。我们可以使用 CSS 的 `position` 属性和它的一些辅助属性来对齐元素。

**方法:** `position` 属性用于设置元素的位置。我们可以使用 `position` 属性和一些辅助属性 `left`、`right`、`top`、`bottom` 来对齐元素，辅助属性用于设置元素的坐标。要使用 `position` 属性对齐元素，我们设置元素的位置，然后使用 helper 属性设置元素的坐标。
...
**例 2:** `absolute` 位置根据页面取位置，因为绝对位置以页面为父。要根据父类来制作，我们必须在父类中添加 `relative` 。
