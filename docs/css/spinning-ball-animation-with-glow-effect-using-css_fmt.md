...
> 原文: [https://www.geeksforgeeks.org/spinning-ball-animation-with-glow-effect-using-css/](https://www.geeksforgeeks.org/spinning-ball-animation-with-glow-effect-using-css/)
...
*   给这个类名添加样式。
*   使用`@keyframes`规则创建名为`spinBall`的动画。使用`transform`属性旋转球。
*   在`box-shadow`属性中应用 2 色嵌入组合来获得发光旋转效果。
*   可以通过更改动画持续时间来调整旋转速度。
