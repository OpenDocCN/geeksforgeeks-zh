...
*   点击 `tr` (表格行)元素为 CSS。
*   使用 [`nth-child()`](https://www.geeksforgeeks.org/css-nth-child-selector/) 选择器，将您选择的 `background-color` 添加到所有奇数(或偶数)表格行。
...
*   点击 CSS 中的 `td` (表格数据)元素。
*   使用 [`nth-child()`](https://www.geeksforgeeks.org/css-nth-child-selector/) 选择器，向所有奇数(或偶数)表格数据单元格添加您选择的 `background-color`。
