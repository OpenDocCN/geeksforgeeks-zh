...
在此分页中，通过使用 `active` 类，当前页面将被突出显示。当鼠标在页面链接上移动时，悬停将改变页面链接的颜色。
...
对于圆形活动和可悬停按钮，只需添加 `border-radius` 属性。
...
在这种类型的分页中，会在分页中添加边框。使用 `border` 属性向分页添加边框。
...
## 分页之间的间隔:
如果不想对页面链接进行分组，可以对它们进行间隔。使用 `margin` 属性在链接之间提供空间。
