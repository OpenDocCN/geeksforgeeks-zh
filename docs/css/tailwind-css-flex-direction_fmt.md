...
CSS [flexbox](https://www.geeksforgeeks.org/introduction-to-css-flexbox/) 是开发前端的一个重要特性，CSS 中有四个方向可用，所以在 [Tailwind CSS](https://www.geeksforgeeks.org/css-tailwind-introduction/) 中，所有属性都以类的形式覆盖。它是 [CSS flex-direction 属性](https://www.geeksforgeeks.org/css-flex-direction-property/)的替代品，用于前端的快速开发。

## 注意

要激活弹性方向，您必须在弹性方向类之前将 `flex` 类包含在元素中。
...
它按照与文本方向相同的方式排列行。弹性方向的默认值是 `row`。它用于指定该项具有正常的文本方向。它使项目在行中遵循正常的文本方向。
...
这个类用来跟随相反的文本方向。正如我们在输出中看到的，它使弹性项的顺序与文本方向完全相反。
...
它将行排列为与文本方向相同但从上到下的列。它用于指定项目具有正常的从上到下方向。正如我们在输出中看到的，它使项目遵循正常的从上到下的方向。
...
它将该行排列为与 `row-reverse` 相同但从下到上的列。它用于指定项目具有从底部到顶部的正常方向。正如我们在输出中看到的，它使项目遵循正常的从下到上的方向。
