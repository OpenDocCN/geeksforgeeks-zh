...
CSS `stroke-color` 属性设置 SVG 元素边框的描边颜色。

**注意:** 由于没有浏览器支持 `stroke-color` 属性，因此，我们使用 `stroke` 作为速记属性，来定义笔画的颜色。
...
*   `color-name` : 该属性值是指颜色的名称，例如: `red`, `green`, `blue` 等。
*   `rgb_value` : 该属性值是指颜色的 rgb 值，例如: `rgb(255, 0, 0)`, `rgb(255, 350, 1)` 等。
*   `hexadecimal-value` : 这个属性值是指颜色的十六进制值，例如: `#fff`, `#000` 等。

## 示例 1

以下是使用 `currentColor` 属性值演示 `stroke-color` 属性的使用的示例。
...
下面是使用颜色名称属性值演示 `stroke-color` 属性的使用的示例。
