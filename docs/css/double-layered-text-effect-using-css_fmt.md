...
> 原文: [https://www.geeksforgeeks.org/double-layered-text-effect-using-css/](https://www.geeksforgeeks.org/double-layered-text-effect-using-css/)

在以动画网站及其用户为受众的文本动画世界中，双层文本效果功能是非常新的。该功能基本上每个单词有两层，上层可以通过`悬停`等各种事件进行操作。上层的旋转是以一定的角度进行的，以产生一种铰链的效果。看起来上层从一个点铰接到下层。

**方法:** 方法是首先使用`::before`选择器创建两个图层，然后使用`悬停`选择器在鼠标悬停时旋转它。

## HTML 代码

在这一节中，我们将每个字母表包装在一个`跨度`中，`数据标题`属性的值与字母表的值相同。
...
*   **步骤 2:** 现在使用`::before`选择器，内容设置为`跨度`标签中使用的`数据标题`。这将创建文本的第二层。确保提供与第一层不同的颜色。
*   **第三步:** 现在使用一些`过渡`来给出平滑的动画。
