...
这个类接受[顺风 CSS](https://www.geeksforgeeks.org/css-tailwind-introduction/) 中的很多值。它是 [CSS Place Content 属性](https://www.geeksforgeeks.org/css-place-content-property/)的替代物。此类用于控制内容如何同时对齐和对齐。在一个类中设置多个属性值。这里`place-content`类可以保存`align-content`和`justify-content`类的值。
...
这个类定义了被拉伸以占据 flex 容器剩余空间的线。这是默认值。
