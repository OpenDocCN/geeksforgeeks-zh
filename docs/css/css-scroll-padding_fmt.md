...
`scroll-padding` 属性是滚动捕捉模块中的一个内置属性。此属性设置滚动填充长字符。此属性充当滑动元素顶部的磁铁，它会粘在视口顶部，并在该位置停止滚动(强制)。

`scroll-padding` 属性是可选的，当 [`scroll-snap-type`](https://www.geeksforgeeks.org/css-scroll-snap-type/) 属性设置为 `none` 时使用该属性。
