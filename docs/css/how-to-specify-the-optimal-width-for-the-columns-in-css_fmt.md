...
> 原文：[`https://www.geeksforgeeks.org/how-to-specify-the-optimal-width-for-the-columns-in-css/`](https://www.geeksforgeeks.org/how-to-specify-the-optimal-width-for-the-columns-in-css/)

在本文中，我们将学习为 CSS 中的列指定最佳宽度。CSS 的 `column-width` 属性用于指定列的最佳宽度。

**方法：** 我们可以使用 [`column-width`](https://www.geeksforgeeks.org/css-column-width-property/) 属性为 CSS 中的列指定最佳宽度。我们为列宽设置一个值，所有列宽都不小于指定值。
...
**示例 1：** 后面的示例演示了设置 `column-width` 属性。输出结果为四列内容。
...
**示例 2：** 以下示例演示了为文本内容设置属性，如 `column-count` 和 `column-width`。
