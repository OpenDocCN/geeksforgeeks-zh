...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-responsive-scrollbox-in-css/](https://www.geeksforgeeks.org/how-to-create-a-responsive-scrollbox-in-css/)
...
要创建响应滚动框，添加一个 `<div>` 标签，然后继续创建滚动框。你所需要做的就是选择滚动框的 `height` 和 `width`（确保你的框的高度足够短，这样你就有一个溢出的文本，允许框向下滚动）。添加 `overflow: auto` 创建滚动效果。

现在，您已经格式化了文本框，可以添加内容了。格式化文本框的内容，就像在 HTML 页面上一样。添加完所有内容后，请关闭您的 `<div>` 标签。您已经创建了一个滚动框，您的页面将立即看起来更加系统。
