...
> 原文：[https://www.geeksforgeeks.org/how-to-prevent-text-in-a-table-cell-from-wrapping-using-css/](https://www.geeksforgeeks.org/how-to-prevent-text-in-a-table-cell-from-wrapping-using-css/)

给定一个包含表头和表体部分的表。任务是防止表格单元格中的文本使用 CSS 换行。为了实现这一点，我们使用 CSS 的 `white-space` 属性。该属性强制将 `th` 的内容显示在一行中。`white-space` 有许多属性值。
...
## 示例 1
本示例使用 `white-space` 属性来防止使用 CSS 进行单元格换行。
```html
<!DOCTYPE html>
<html>
   <head>
