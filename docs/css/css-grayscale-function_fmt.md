...
CSS 中的 `grayscale()` 函数是一个内置函数，用于对图像应用滤镜来设置图像的灰度。
...
该功能接受保存灰度值的单个参数 `amount`。灰度值是根据数字和百分比设置的。值 `0%` 表示原始图像，而 `0%` 到 `100%` 之间的值表示效果的线性乘数。

## 示例

下面的示例说明了 CSS 中的 `grayscale()` 函数。
