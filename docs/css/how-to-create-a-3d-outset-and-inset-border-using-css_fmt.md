...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-3d-outset-and-inset-border-using-css/](https://www.geeksforgeeks.org/how-to-create-a-3d-outset-and-inset-border-using-css/)

在这篇文章中，我们将使用 CSS 创建一个三维开始和插入边框。Inset border 属性使内容看起来是嵌入的（在表面内部），另一方面，begin 属性使内容看起来是浮雕的（在表面外部）。您可以通过使用用于装饰边框的边框样式属性来完成此任务。

## 三维嵌入边框

用于定义三维嵌入边框，其效果取决于边框颜色值。
