...
**网格检查器**是一个强大的工具，可以使用浏览器验证网格系统。在 Mozilla Firefox 中，有一个内置的网格检查器，但是在谷歌 Chrome 中，您必须使用扩展。网格检查器非常有用，因为网格系统并不复杂，或者跨越许多轨迹。很难想象将物品固定在适当位置的实际网格。网格检查器允许您查看网格线。
...
*   **第二步:** 确保在带有源代码的窗格中选择了带有`display: grid`的元素，在另一个窗格中选择了**规则**选项卡，点击`display: grid`之间的网格图标(网格检查器截图)。

![](img/28035b4b79cc7f373e46631d463e6fbc.png)

## 在 Chrome 上使用网格检查器的步骤
...
*   **步骤 3:** `mouseover` 网格元素，将显示行间距和列间距。