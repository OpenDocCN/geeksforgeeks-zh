...
*   `mix-blend-multiply`: 这将元素的颜色与背景相乘。产生的颜色总是和背景一样暗。
*   `mix-blend-screen`: 这将元素的颜色与背景相乘，然后补充结果。生成的颜色总是像混合层一样明亮。
*   `mix-blend-exclusion`: 这将从元素的最亮颜色中减去两种颜色中的较暗颜色。结果类似于“差异”，但对比度较低。
*   `mix-blend-overlay`: 这将对元素中较亮的颜色应用“倍增”，对较暗的颜色应用“屏蔽”。这种效果实际上与“强光”相反。
*   `mix-blend-darken`: 这将背景替换为元素颜色较暗的部分。
*   `mix-blend-lighten`: 这将背景替换为元素颜色较亮的部分。
*   `mix-blend-color-dodge`: 这将使背景颜色变浅，以反映元素的颜色。
*   `mix-blend-color-burn`: 这会使背景颜色变暗，以反映图像的自然颜色。结果增加了元素和背景之间的对比度。
*   `mix-blend-hard-light`: 这将对较亮的颜色应用“倍增”，对较暗的元素应用“屏蔽”。这种效果实际上与“叠加”相反。
*   `mix-blend-soft-light`: 这将对较亮的颜色应用“倍增”，对元素中较暗的颜色应用“屏蔽”。产生的效果比“叠加”更柔和。
*   `mix-blend-difference`: 这将减去背景颜色和元素颜色之间的绝对差异。
*   `mix-blend-hue`: 这将元素的色调应用于背景的饱和度和亮度。
*   `mix-blend-saturation`: 这将元素的饱和度应用于背景的色调和亮度。
*   `mix-blend-color`: 这将元素的色调和饱和度应用于背景的亮度。
*   `mix-blend-luminosity`: 这将元素的亮度应用于背景的色调和饱和度。
...
`mix-blend-mode` CSS 属性设置元素的内容应该如何与元素的父元素和元素背景的内容混合。

## 示例 1

以下代码使用类 `mix-blend-normal`、`mix-blend-multiply`、`mix-blend-overlay`。
...
以下代码使用 `mix-blend-exclusion`、`mix-blend-color-burn` 和 `mix-blend-hard-light`。
