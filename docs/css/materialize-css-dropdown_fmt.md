...
物化 CSS 提供了一个下拉工具，允许用户从列表中的一组给定值中选择一个值。要向任何按钮添加下拉列表，必须确保`data-target`属性与`<ul>`标签中的`id`匹配。

下拉菜单中使用的主要类别和属性为:

1.  `dropdown-content`类用于标识哪个`<ul>`标签应该成为物化下拉组件。
2.  `data-activates`属性用于指定下拉`<ul>`元素的`id`。
...
*   使用`divider`类添加除法器。它可以添加到空的`<li>`标签中，以显示一个分隔线。
*   通过使用`<i>`标签使用`material-icons`类添加图标。可以指定要使用的图标，它将显示在列表项的文本旁边。
