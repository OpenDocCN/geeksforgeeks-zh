...
CSS 按钮用于通过对按钮应用各种样式属性来装饰网页。按钮用于事件处理和与用户交互。从提交表单到查看一些信息，我们必须点击按钮。`button`标签用于在 HTML 中创建按钮。

**示例:** 这个简单的示例描述了`button`标签。
...
此属性用于设置按钮中文本的颜色。颜色值可以根据颜色名称、颜色十六进制代码等进行设置。
