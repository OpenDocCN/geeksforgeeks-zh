...
`rgba()`函数是 CSS 中的一个内置函数，用于使用红-绿-蓝-阿尔法(rgba)模型定义颜色。它是 [`rgb()`](https://www.geeksforgeeks.org/css-rgb-function/) 颜色值的扩展，包含指定颜色透明度的 alpha 通道。
...
本示例说明了 `rgba()` 函数中不同的颜色强度(即。，红色，蓝色&绿色)，显示每种颜色的特定值，以及用于设置颜色不透明度的 `alpha` 值。
