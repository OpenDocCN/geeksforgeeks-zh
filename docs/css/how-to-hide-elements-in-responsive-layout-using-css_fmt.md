...
> 原文: [https://www.geeksforgeeks.org/how-to-hide-elements-in-responsive-layout-using-css/](https://www.geeksforgeeks.org/how-to-hide-elements-in-responsive-layout-using-css/)
...
这里使用的媒体类型是 `screen`，它与 `only` 关键字一起使用，因此媒体查询只影响选定的屏幕。媒体功能可以根据宽度进行更改。例如，它可以与宽度媒体功能一起使用。当宽度设置为特定值（如使用 `min-width` 和 `max-width`）时，可以对其进行修改。其他受支持的媒体功能可用于检查有助于响应网站的值。
...
在本例中，通过将 `display` 属性设置为 `none` 来隐藏元素。媒体查询包含将 `display` 属性设置为 `block`。
