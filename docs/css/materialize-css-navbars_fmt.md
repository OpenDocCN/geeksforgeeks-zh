...
为了右对齐导航条链接，`right` `类`被添加到 `< ul >` 元素中。
...
为使标志居中对齐，在 `< a 类=“品牌标志”>` 中增加了 `center` `类`。尽管徽标会在中下方屏幕上居中显示。使用时，请确保链接不重叠。
...
用于表示当前活动页面，为此 `active` `类`添加到 `li` 标签中。
...
用于扩展导航栏的组件。为此，`nav-wrapper` `类`被添加到外部 `导航` 标签中。这将允许标签组件内的 `导航包装` 和 `高度` 是可变的。
    ...
    <div class="nav-wrapper green">
      <div class="container">
        ...
        <ul class="right hide-on-med-and-down">
          ...
          <ul id='dropdown1' class='dropdown-content green-text'>
