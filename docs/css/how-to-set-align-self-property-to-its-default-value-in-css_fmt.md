...
> 原文：[https://www.geeksforgeeks.org/how-to-set-align-self-property-to-its-default-value-in-css/](https://www.geeksforgeeks.org/how-to-set-align-self-property-to-its-default-value-in-css/)

在本文中，我们将学习如何将 `align-self` 属性设置为默认值。`align-self` 属性用于指定父元素中选定项的对齐方式。

## 方法

`align-self` 属性中的 `auto` 值用于设置 CSS 中的默认值。该值使该属性直接继承其父元素的行为。在这种情况下，这是不可能的，元素像使用 `stretch` 属性一样拉伸。
