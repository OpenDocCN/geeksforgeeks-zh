...
`:invalid` 选择器用于选择每个不根据元素验证的表单元素。
此选择器仅适用于某些有限制的表单元素，如具有 `min` 和 `max` 属性的 `input` 元素、具有合法电子邮件的电子邮件字段或具有数值的数字字段等。与使用 `:invalid` 选择器相反，可以使用 `:valid` 选择器来选择根据元素进行验证的表单元素。
