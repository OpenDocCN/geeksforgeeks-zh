...
> 原文：[https://www.geeksforgeeks.org/css-tooltip-fade-in-animation/](https://www.geeksforgeeks.org/css-tooltip-fade-in-animation/)

当鼠标悬停在某个元素上时，工具提示用于显示该元素的相关信息。默认情况下，工具提示显示没有延迟（动画）。

## 如何给 Tooltip 添加动画？

我们可以在工具提示文本变得可见之前为其添加淡入效果。这可以通过使用 CSS 中的`transition`属性在给定的秒数内将`opacity`从 0 更改为全。
...
最初，工具提示文本的`opacity`设置为 0，然后过渡应用于延迟为 3s 的`opacity`。
当我们悬停在“GeeksforGeeks”文本上时，工具提示在 3s 内 100%可见。这会导致淡入效果/动画。