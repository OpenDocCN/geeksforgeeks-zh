...
> 原文：[https://www.geeksforgeeks.org/how-to-load-css-and-js-files-dynamically/](https://www.geeksforgeeks.org/how-to-load-css-and-js-files-dynamically/)
...
**第一步：** 创建 `index.html` 文件和 `app.js` 文件。这将是我们的 HTML 文件，通过它我们将演示 JS 和 CSS 文件的动态加载。`app.js` 文件将包含调用文件动态加载的功能。我们会静态地将它添加到我们的 HTML 文件中。
...
*   我们使用 [`document.createElement(<elementName>)`](https://www.geeksforgeeks.org/html-dom-createelement-method/) 创建所需元素。
*   然后我们像 [`script.src`](https://www.geeksforgeeks.org/html-dom-script-src-property/) 和 [`script.type`](https://www.geeksforgeeks.org/html-dom-script-type-property/) 一样定义/分配属性。
*   我们使用 [`document.getElementsByTagName('head')[0].append(<elementVariable>)`](https://www.geeksforgeeks.org/html-dom-getelementsbytagname-method/) 将元素追加到头部。
...
**步骤 2：** 现在创建一个 `styles.css` 文件，该文件将被动态加载。该文件包含使用两个 [HTML div](https://www.geeksforgeeks.org/div-tag-html/) 的 id 分别向两个 [HTML div](https://www.geeksforgeeks.org/div-tag-html/) 提供[边框](https://www.geeksforgeeks.org/css-border-property/)、[边距、填充](https://www.geeksforgeeks.org/css-margins-padding/)和[背景色](https://www.geeksforgeeks.org/css-background-color-property/)的代码。
...
**第三步：** 现在我们将创建一个 `script.js` 文件，通过点击**查看消息**按钮编辑 [h3](https://www.geeksforgeeks.org/how-to-define-an-html-heading-in-html5/) 元素，并消失下方的 div，或者将其[显示](https://www.geeksforgeeks.org/css-display-property/)属性更改为 `none` 来显示消息。这个 JS 文件将被动态加载。
