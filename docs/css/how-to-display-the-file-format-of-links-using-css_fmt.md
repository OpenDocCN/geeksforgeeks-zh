...
> 原文: [https://www.geeksforgeeks.org/how-to-display-the-file-format-of-links-using-css/](https://www.geeksforgeeks.org/how-to-display-the-file-format-of-links-using-css/)
...
因此，为了解决这个问题，我们可以只使用一些 `CSS` 样式来公开我们网页中链接的文件格式。这是通过定位页面中链接的文件类型，并在选择器后使用 `::after` 添加图标图像，并使用图标路径指定 `content` 属性来完成的。它会自动在页面上找到的每个与文件扩展名匹配的合适链接上插入图标图像。
