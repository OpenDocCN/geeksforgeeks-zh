...
> 原文：[https://www.geeksforgeeks.org/how-to-create-shape-using-css-clipping/](https://www.geeksforgeeks.org/how-to-create-shape-using-css-clipping/)

你可以使用 CSS 中的 [`clip-path`](https://www.geeksforgeeks.org/css-clip-path-property/) 属性轻松创建形状。实际上 `clip-path` 属性允许您将元素剪辑为基本形状。像圆、椭圆、多边形这样的基本形状可以使用裁剪路径的 `<basic-shape>` 属性值来实现，这将在本文中进一步讨论。

## 属性值

*   `<basic-shape>`：它包括一些像圆形、矩形、椭圆形等形状来剪辑给定的图像。
...
`clip-path` 允许剪辑 SVG 元素、图像或任何 HTML 元素的可见部分。
