...
基本上，动画是以下属性的简写属性:`动画名称`、`动画持续时间`、`动画计时功能`、`动画延迟`、`动画迭代次数`、`动画方向`、`动画填充模式`和`动画播放状态`。对于本文，我们将使用`动画-名称`、`动画-持续时间`、`动画-迭代-计数`、`动画-计时-函数`。

*   `动画-名称`:`floating`(这是指下面定义的`@关键帧`)。
...
在每个断点，我们都有属性转换。允许用户旋转、倾斜、缩放或平移给定元素的`变换`属性。在这种情况下，我们将使用`translate`属性。基本上翻译帮助我们垂直和水平放置东西。
