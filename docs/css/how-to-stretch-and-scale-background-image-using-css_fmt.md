...
> 原文：[https://www.geeksforgeeks.org/how-to-stretch-and-scale-background-image-using-css/](https://www.geeksforgeeks.org/how-to-stretch-and-scale-background-image-using-css/)

## 背景尺寸属性

`background-size` 属性用于拉伸和缩放背景图像。此属性设置背景图像的大小。这里我们将看到 `background-size` 和 `background-position` 属性的所有可能的例子。
...
- **`length`**：该属性值用于缩放背景图像。它会改变背景图像的大小。长度值可以是 `px`、`em`、`%` 等形式。
