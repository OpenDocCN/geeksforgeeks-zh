...
> 原文：[https://www.geeksforgeeks.org/how-to-format-a-number-as-currency-using-css/](https://www.geeksforgeeks.org/how-to-format-a-number-as-currency-using-css/)

给定一个数字，任务是使用 CSS 和一点点 JavaScript 将 HTML 元素的数字格式化为货币。单纯用 CSS 是不可能完成任务的。解析数字时需要一点 JavaScript 来添加逗号。

## 方法

在代码中，CSS 类`currSign`添加了货币符号（比如“{ content }”）号前。JavaScript 函数`toLocaleString()`返回一个字符串，该字符串带有数字的语言敏感表示。
...
<body>
    ...
    <!-- Javascript code to format the number as per the locale -->
    <script>
