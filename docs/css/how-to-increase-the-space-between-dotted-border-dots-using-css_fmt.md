...
> 原文：[https://www.geeksforgeeks.org/how-to-increase-the-space-between-dotted-border-dots-using-css/](https://www.geeksforgeeks.org/how-to-increase-the-space-between-dotted-border-dots-using-css/) 增加点边框点间距

任务是增加点状边框点之间的间距。您可以使用`background-size`属性调整大小，使用`background-image`属性调整比例，使用`linear-gradient`百分比调整比例。因此，您可以使用多种背景拥有多个虚线边框。在这个例子中，我们有一条由 3px 点和 7px 间距组成的虚线。
...
下面给出的文件包含 CSS 属性。此文件用`.css`扩展保存。例如：
...
下面是利用创建的外部样式表的 HTML 文件。`<link>`标签用于链接外部样式表和 HTML 网页。
