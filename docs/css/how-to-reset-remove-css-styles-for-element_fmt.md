...
> 原文：[https://www.geeksforgeeks.org/how-to-reset-remove-css-styles-for-element/](https://www.geeksforgeeks.org/how-to-reset-remove-css-styles-for-element/)
...
大多数情况下，网络浏览器提供的默认按钮非常平淡无奇，没有风格化。为了使它更加风格化，并使它适合网页的主题，它可以使用 CSS 手动风格化。但是在移除默认样式之前，不能应用手动样式。因此，我们应用下面的代码来移除按钮上的默认样式。

## 示例 1

这里你会看到如何通过 HTML 和 CSS 来使用 `unset` 属性。本示例仅移除默认浏览器样式。
...
这里你会看到如何通过 HTML 触发 `unset` 属性，CSS 一个 jQuery。
