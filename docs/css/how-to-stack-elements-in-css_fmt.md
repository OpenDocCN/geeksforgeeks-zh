...
属性 `position: absolute;` 用于将任意元素定位在绝对位置，该属性可用于将元素堆叠在彼此之上。使用这种方法，任何元素都可以放置在任何地方，而不管其他元素的位置如何。
...
堆叠元素的另一种方式是使用 CSS 网格。网格可用于根据需要放置元素。使用下面的代码将一个元素简单地堆叠在另一个之上。
