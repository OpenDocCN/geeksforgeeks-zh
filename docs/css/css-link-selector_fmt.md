...
`:link`选择器用于定位未访问的链接，而不是已经访问过的链接。对于访问时的样式使用`[:visited](https://www.geeksforgeeks.org/css-visited-selector/)`，对于点击时的样式使用`[:active](https://www.geeksforgeeks.org/css-active-selector/)`，对于链接时的样式移动鼠标使用`:hover`。
