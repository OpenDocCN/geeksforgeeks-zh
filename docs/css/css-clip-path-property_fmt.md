...
CSS 的`clip-path`属性用于剪辑图像的特定部分，使得部分内的图像被显示，而部分外的图像不被显示。
...
它包括一些形状，如圆形、矩形、椭圆形等，用于剪辑给定的图像。

### 示例 1:

本示例显示了`clip-path`属性的基本用法，用于插入剪辑给定图像的特定形状。
...
本示例显示了`clip-path`属性的基本用法，其中该值设置为`none`。
...
在这种情况下，剪辑部分取自另一个包含图像或元素的 HTML 元素。元素 id 用于引用元素。

### 示例 3:

本示例显示了`clip-path`属性的基本用法，其中属性值被设置为`url()`。
