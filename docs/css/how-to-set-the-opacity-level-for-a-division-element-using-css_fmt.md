...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-opacity-level-for-a-division-element-using-css/](https://www.geeksforgeeks.org/how-to-set-the-opacity-level-for-a-division-element-using-css/)

在本文中，我们将使用 CSS 设置元素的不透明度。不透明度是描述元素透明度的元素属性。

不透明度属性用于描述元素的透明度。不透明度的值介于 `0.0` 到 `1.0` 之间，其中低值表示高透明度，高值表示低透明度。不透明度百分比计算为 `不透明度% = 不透明度 * 100`。
