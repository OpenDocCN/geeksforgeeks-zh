...
宝丽来图片是一个图片框，下方有一个标题。在本文中，您将学习使用 HTML 和 CSS 创建宝丽来图像。这可以通过使用 HTML `<div>` 或 `<figure>` 标签来完成。

## 1. 使用 `div`

通过使用 HTML `div` 标签并添加 CSS 属性来创建偏光片图像，如下所示。将图像宽度设为 100%，使其与父图像一样宽。
...
<body>
    <div class="card">
        <img src="gfg.png" alt="gfg img" width="100%">
        <div class="card-txt">
            <p>GeeksforGeeks</p>
        </div>
    </div>
...
在以下示例中，使用 HTML5 `figure` 标签创建了宝丽来图像。`figure` 标签有一些默认的边距。一个 `box-shadow` 被添加到它，这样它就可以将自己与背景隔离开来。
...
用户可以根据需要添加任意多的偏光片，创建图库。

**示例:** `display: flex` 属性使 `figure` 排成一行，这通常会在单独的行上。默认情况下，画廊 `div` 覆盖整个宽度。因此宽度被改变并且添加了 `margin: auto` 属性以使图库居中。`justify-content: space-between` 用于对齐图形并在它们之间创建相等的空间。`flex-wrap: wrap` 用于使图形折绕，并在排满时将项目移到下一排。
