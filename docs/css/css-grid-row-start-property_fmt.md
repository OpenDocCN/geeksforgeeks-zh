...
*   `auto`：将网格行开始属性设置为默认值一行。
*   `span`：它指定项目将跨越的行数。
*   整数（`row-line`）：指定项目开始的行。
*   `initial`：将网格行开始属性设置为默认值。
*   `inherit`：网格行开始属性是从其父级继承的。

**注意：** 不要显式初始化容器的物品高度和宽度。如果初始化，它的跨度效应就无法观察到。

## 示例 1

该示例描述了没有 `grid-row-start` 属性的容器项。
...
本示例将 `grid-row-start` 属性描述为 `auto`。
...
本示例将 `grid-row-start` 属性描述为 `row-line`。
...
本示例将 `grid-row-start` 属性描述为 `initial`。
