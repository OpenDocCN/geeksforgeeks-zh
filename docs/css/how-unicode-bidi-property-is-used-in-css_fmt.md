...
> 原文：[https://www.geeksforgeeks.org/how-unicode-bidi-property-is-used-in-css/](https://www.geeksforgeeks.org/how-unicode-bidi-property-is-used-in-css/)

`unicode-bidi` 属性在 CSS 中使用，该属性与 `direction` 属性一起应用于 HTML 中，以确定如何在文档中处理双向文本。
...
*   `normal`：为默认值。
*   `embed`：该值用于开启额外的嵌入级别。
*   `bidi-override`：该值为内联元素创建覆盖，如果是块元素，则为不在另一个块元素内的内联级后代创建覆盖。
