...
CSS 中的`块大小属性`用于定义元素块的水平或垂直大小。根据[书写模式 p](https://www.geeksforgeeks.org/css-writing-mode-property/) 属性的值，它与`宽度`或`高度`属性一致。它会在元素文本的左边留下空白。
...
*   **自动:** 当希望浏览器确定块大小时使用。
*   **初始值:** 用于将`块大小属性`的值设置为默认值。
*   **inherit:** 当希望元素继承其父元素的`块大小属性`作为自己的属性时使用。
*   **unset:** 用于取消设置默认块大小。
