...
> 原文: [https://www.geeksforgeeks.org/how-to-make-grid-items-auto-height-using-tailwind-css/](https://www.geeksforgeeks.org/how-to-make-grid-items-auto-height-using-tailwind-css/)

您可以使用顺风 CSS 中的 `grid-template-columns` 属性轻松地将 CSS 网格项目设置为自动高度。

顺风使用 `grid-cols` 和 `grid-rows` 属性，这是 CSS 中 `grid-template-columns` 属性的替代。CSS 中的 `grid-template-columns` 属性用于设置网格的列数和列的大小。
...
**注意:** 从上面的输出可以观察到，每一行都有不同的高度，这取决于特定行的网格所需的 `max-height`。
