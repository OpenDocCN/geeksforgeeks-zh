...
*   此属性主要用于将图像与其对应的文本对齐。
*   这个属性也可以用来对齐表格中的单元格。
*   `vertical-align` 属性不能用于像 `p`、`ol`、`ul`、`h1`、`div` 等块级元素的对齐。

## 属性值

*   `baseline`：它将元素基线与父元素的基线相对应。这可能因浏览器而异。这是默认值。
*   `sub`：它将元素基线与其父元素的下标基线对齐。
*   `super`：它将元素基线与其父元素的上标基线相对应。
*   `text-top`：它将元素顶部与父字体的顶部对齐。
*   `text-bottom`：将元素的底部与父字体的底部对齐。
*   `middle`：将元素的中间与父元素的中间对齐。
*   `top`：将元素的顶部与其所在线上最高元素的顶部对齐。
*   `bottom`：将元素的底部与其直线上最短元素的顶部对齐。
*   `length`：将元素的基线与其父元素基线上方的给定长度对齐。允许负值。
*   `百分比`：将元素的基线与其父元素基线上方的给定百分比相对应，该值为 `line-height` 属性的百分比。
*   `initial`：将该值初始化为默认值。
*   `inherit`：从父元素继承值。
