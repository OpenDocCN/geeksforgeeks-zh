...
**[animation-name](https://www.geeksforgeeks.org/css-animation-name-property/):** 用于指定描述动画的`@keyframes`的名称。

```html
animation-name: animation_name;
```

**[animation-duration](https://www.geeksforgeeks.org/css-animation-duration-property/):** 用于指定动画完成一个循环所需要的时长。

## 示例：使用animation-duration属性描述CSS动画属性
...
**[animation-timing-function](https://www.geeksforgeeks.org/css-animation-timing-function-property/):** 指定动画如何通过关键帧进行过渡。它可以有以下值:

*   `ease`: 动画开始慢，然后快，最后慢慢结束(这是默认)
*   `linear`: 动画从头到尾以相同的速度播放
*   `ease-in`: 动画缓慢开始播放
*   `ease-out`: 动画以缓慢的结尾播放
*   `ease-in-out`: 动画开始和结束都很慢。

## 示例：使用animation-timing-function属性描述CSS动画属性
...
**[animation-delay](https://www.geeksforgeeks.org/css-animation-delay-property/):** 用于指定动画开始时的延时。

## 示例：使用animation-delay属性描述CSS动画属性
...
**[animation-iteration-count](https://www.geeksforgeeks.org/css-animation-iteration-count-property/):** 用于指定动画重复的次数。它可以指定为`infinite`来无限重复动画。

## 示例：使用animation-iteration-count属性描述CSS动画属性
...
**[animation-direction](https://www.geeksforgeeks.org/css-animation-direction-property/):** 指定动画的方向。它可以具有以下值:

*   `normal`: 动画向前播放。这是默认值。
*   `reverse`: 动画播放方向相反，即向后。
*   `alternate`: 动画先向前播放，后向后播放。
*   `alternate-reverse`: 动画先向后播放，再向前播放。

## 示例：使用animation-direction属性描述CSS动画属性
...
`animation-fill-mode` 属性指定动画执行前后应用的值。

*   `none`: 动画在执行之前或之后不会对元素应用任何属性。这是默认值。
*   `forwards`: 动画完成后，元素将保留最后一个关键帧的相同动画属性。
*   `backwards`: 元素将获得动画开始前第一个关键帧的属性。
*   `both`: 动画将遵循 `forwards` 和 `backwards` 的规则，即它将在开始前获得为初始关键帧定义的属性，并将在动画完成后保留最后一个关键帧的值。

**示例:** 本示例使用 `animation-fill-mode` 属性描述了 CSS 动画属性。
...
`animation-play-state` 属性可以让你播放/暂停动画。

### 动画简写属性

`animation` 是一种简写方式，包含了多个动画属性，以获得更快的代码。属性应该按以下顺序排列:
...
**示例:** 本示例使用 `animation-play-state` 属性描述 CSS 动画属性，没有使用 `animation` 速记属性。
...
**示例:** 本示例使用 `animation-play-state` 属性和 `animation` 速记属性来描述 CSS 动画属性。
