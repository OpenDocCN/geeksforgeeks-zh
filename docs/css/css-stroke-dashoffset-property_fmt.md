...
> 原文：[https://www.geeksforgeeks.org/css-stroke-dashoffset-property/](https://www.geeksforgeeks.org/css-stroke-dashoffset-property/)

`stroke-dashoffset` 属性用于设置沿 SVG 路径的位置，笔划的虚线图案将从该位置开始。较高的值意味着破折号将从更远的位置开始。
...
*   `length`：用于以长度单位设置偏移量。该值根据元素的路径长度进行解析。
*   `percentage`：用于以百分比值设置偏移量。该值解析为当前视口的百分比。
