...
> 原文: [https://www.geeksforgeeks.org/css-padding-inline-end-property/](https://www.geeksforgeeks.org/css-padding-inline-end-property/)

CSS 中的 `padding-inline-end` 属性用于定义元素的逻辑块结束填充。此属性有助于根据元素的书写模式、方向和文本方向放置填充。
...
*   `percentage`: 用于根据元素宽度的百分比设置 `padding-inline-end` 值。
*   `auto`: 当希望浏览器确定块大小时使用。
*   `initial`: 用于将块大小属性的值设置为默认值。
*   `inherit`: 当希望元素继承其父元素的块大小属性作为自己的属性时使用。
*   `unset`: 用于取消设置默认块大小。

以下示例说明了 CSS 中的 `padding-inline-end` 属性:
...
**支持的浏览器:** `padding-inline-end` 属性支持的浏览器如下:
