...
**注意：** 过渡效果可以使用伪类定义为两种状态（`:hover` 和 `:active`），如 `:hover` 或 `:active`，或使用 JavaScript 动态设置的类。
...
*   `transition-property`：指定过渡效果应该应用到的 CSS 属性。
*   `transition-duration`：它指定过渡动画完成所需的时间长度。
*   `transition-timing-function`：指定过渡速度。
*   `transition-delay`：指定过渡开始时的过渡延迟或时间。
