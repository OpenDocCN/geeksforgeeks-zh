...
在 CSS 中设置图像的样式的工作方式与使用内容的填充、边框和边距的[框模型](https://www.geeksforgeeks.org/css-box-model/)设置任何元素的样式完全相同。下面列出了许多设置图像样式的方法:
...
边框半径属性用于设置边框图像的半径。该属性可以包含一个、两个、三个或四个值。它是四个属性的组合: `border-top-left-radius`、`border-top-right-radius`、`border-bottom-left-radius`、`border-bottom-right-radius`。

**示例:** 此示例说明了使用造型图像属性创建圆角图像。
...
不透明度属性用于设置图像透明。不透明度值介于 `0.0` 到 `1.0` 之间。

**示例:** 该示例说明了使用造型图像属性创建透明图像。
...
可以使用左边距和右边距属性将图像居中到框中。

**示例:** 该示例说明了使用造型图像属性将图像定位到中心。
