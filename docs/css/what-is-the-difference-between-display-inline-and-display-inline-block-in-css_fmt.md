...
> 原文：[https://www.geeksforgeeks.org/what-is-the-difference-between-display-inline-and-display-inline-block-in-css/](https://www.geeksforgeeks.org/what-is-the-difference-between-display-inline-and-display-inline-block-in-css/)

CSS 中的 `display` 属性是一个非常有用且常用的属性，它包含很多值。本文包含 `display: inline` 和 `display: inline-block` 属性。

## `display: inline` 属性

该属性用于将元素显示为内联元素（如 `<span>`）。`height` 和 `width` 属性在 `display: inline;` 时不受影响。它只允许左边界和右边界，不允许上边界和下边界。简单地说，它的相邻元素前后都没有换行符，并且允许它旁边有 HTML。
...
该属性用于将元素显示为内联级块容器。元素本身被格式化为内联元素，但是它可以应用 `height` 和 `width` 值。它作为内联元素放置（与相邻内容在同一行）。它看起来像一个内联元素，但它的行为就像一个块元素，不会强制换行。
