...
CSS [伪元素](https://www.geeksforgeeks.org/css-pseudo-elements/)中的`::placeholder`选择器用于通过更改文本颜色来设计占位符文本，并允许修改文本的样式。

`::placeholder`伪元素是 CSS 属性中唯一适用的子集，可以在其选择器中使用`::placeholder`在规则中应用。默认情况下，大多数浏览器的占位符文本外观为半透明或浅灰色。
...
我们将通过例子理解`::placeholder`选择器的概念及其实现。

**示例 1:** 本示例说明了用于更改文本颜色以及占位符文本的背景颜色的`::placeholder`选择器。
...
**示例 2:** 本示例说明了`::placeholder`选择器，用于设置占位符文本，其属性标识描述输入字段预期值的提示。
...
**支持的浏览器:** 以下列出了`::placeholder`选择器支持的浏览器:
