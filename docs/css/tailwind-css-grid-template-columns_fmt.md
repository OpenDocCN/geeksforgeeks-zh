...
> 原文: [https://www.geeksforgeeks.org/tailwind-css-grid-template-columns/](https://www.geeksforgeeks.org/tailwind-css-grid-template-columns/)

这个类在 tailwind CSS 中接受多个值，所有的属性都以类的形式被覆盖。是 CSS 中 [`grid-template-columns`](https://www.geeksforgeeks.org/css-grid-template-columns-property/) 属性的替代。它用于设置网格的列数和列的大小，这里我们也将这样做，但是为了前端的快速开发。列的数量由给这个类的值的数量来设置。
...
*   `grid-cols-7`: 每行包含七列。
*   `grid-cols-8`: 每行包含八列。
*   `grid-cols-9`: 每行包含 9 列。
*   `grid-cols-10`: 每行包含十列。
*   `grid-cols-11`: 每行包含 11 列。
*   `grid-cols-12`: 每行包含 12 列。
*   `grid-cols-none`: 不遵循网格-列属性。
