# 如何在 CSS 中应用 !important 属性？

> 原文：[https://www.geeksforgeeks.org/how-to-apply-important-in-css/](https://www.geeksforgeeks.org/how-to-apply-important-in-css/)

`!important` CSS 中重要属性用于提供比正常属性更多的权重(重要性)。在 CSS 中，`!important`意味着“这很重要”，忽略所有后续规则，并应用`!important`的规则。`!important`关键字必须放在行尾，紧接在分号之前。

*   换句话说，它增加了速记属性所代表的所有子属性的重要性。
*   在正常使用中，外部样式表中定义的规则被文档头中定义的样式否决，而文档头中定义的样式又被元素本身中的内联样式否决(假设选择器的特性相同)。
*   定义一个规则`!important`属性，它抛弃了关于后一个规则覆盖前一个规则的常规关注。
...
在上例中，`body`的背景色是绿色而不是黄色是因为`!important`保留在`body`标签内的绿色背景颜色之后。
