...
您可以使用 Tailwind CSS 中的 `flex` 属性轻松地在整个屏幕上垂直对齐 div。顺风使用`justify-center`和`items-center`属性，这是 CSS 中[伸缩属性](https://www.geeksforgeeks.org/advance-css-layout-with-flexbox/)的替代。
...
*   **`justify-center`:** 当伸缩项按行堆叠时，此属性将伸缩项在水平方向(主轴)的中心对齐。
*   **`items-center`:** 当伸缩项目按行堆叠时，此属性将伸缩项目在垂直方向(横轴)的中心对齐。
...
**重要概念:** 无论何时翻转弯曲的方向，都将翻转水平对齐(`justify-{alignment}`)和垂直对齐(`items-{alignment}`)。因此，如果 flex 是在行方向，则`justify-{alignment}`是水平方向。当弯曲方向为列方向时，对齐方向为垂直方向。
...
## 示例 2: 使用 `m-auto` 将元素居中

`m-auto` 用于将项目水平和垂直居中。以下示例将在整个屏幕上垂直和水平对齐 `div`。
