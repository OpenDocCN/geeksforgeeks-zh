...
### `flex-1`
指定项目相对于其余灵活项目增长多少的比率。它已被用于允许灵活项目根据需要增长和收缩，忽略其初始大小。
...
### `flex-initial`
这个类定义了该项目相对于其他灵活项目将增长多少。考虑到它的初始大小，它已经被用来允许伸缩项收缩但不增长。
...
### `flex-auto`
该类指定该项目相对于灵活项目的内容将增长多少。它已被用于允许灵活项目根据需要增长和收缩，忽略其初始大小。
...
### `flex-none`
如果有额外的可用空间或没有额外的可用空间，该类将限制设置为增加或缩小元素。这是为了防止 flex 项目增长或收缩，它只根据内容的大小占用空间。
