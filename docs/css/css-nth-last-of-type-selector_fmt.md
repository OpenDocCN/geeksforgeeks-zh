# CSS :nth-last-of-type() 选择器

> 原文: [https://www.geeksforgeeks.org/css-nth-last-of-type-selector/](https://www.geeksforgeeks.org/css-nth-last-of-type-selector/)

CSS 中的 `:nth-last-of-type()` 选择器用于样式化父元素的第 n 个子元素，从最后一个子元素开始计数。一个 **n** 可以是一个数字，一个关键词，或者一个公式。
...
其中 `number` 是表示从末尾开始计数的匹配元素模式的参数。它可以是 `odd`、`even` 或函数符号。

*   **奇数 (`odd`)**: 表示一系列中位置为奇数的元素：1、3、5 等，从头数到尾。
*   **偶数 (`even`)**: 表示位置在 2、4、6 等数列中偶数的元素，从头数到尾。
...
在该示例中，根据从末尾开始计数的公式 `2n` 选择每个偶数元素。
...
以下列出了 `:nth-last-of-type()` 选择器支持的浏览器：
