...
`@supports`条件式规则是检查浏览器对特定 CSS 属性的支持，并基于此应用样式。
**语法:**
...
`@media`条件规则是用于基于媒体查询应用样式的规则。它可用于检查设备宽度和/或高度，并应用基于此指定的样式。
**语法:**
...
`@document`条件规则用于对指定的网址应用样式，即样式仅应用于指定的网址。
**语法:**
...
它是实验性的，只在带有 `-moz-` 前缀即 `@-moz-document` 的 Firefox 上工作。
**例:**
