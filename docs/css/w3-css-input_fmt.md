...
**`w3-input` 类：** 这个类可以用于 `text`、`password`、`number`、`datetime`、`datetime-local`、`date`、`month`、`time`、`week`、`email`、`url`、`search`、`tel` 和 `color`，这意味着它支持所有的 HTML5 输入类型。它用于将输入元素的宽度设置为 100%。
...
**`w3-check` 类别：** 复选框用于从提供的列表中选择任何特定项目。此类仅在复选框中使用。
...
**`w3-radio` 类别：** 单选按钮用于用户从多个选项中选择唯一的选项。它类似于复选框，只是从输入类型中删除 `checkbox` 并放置 `radio`。此类仅用于单选按钮。
...
**`w3-select` 类：** 这里有两种类型的选择列表：从列表中选择单个元素，以及从列表中选择两个或多个元素（选择多个元素时，只需按住 shift 键并逐个选择您的项目）。
