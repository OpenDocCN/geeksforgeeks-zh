...
> 原文：[https://www.geeksforgeeks.org/transition-shorthand-with-multiple-properties-in-css/](https://www.geeksforgeeks.org/transition-shorthand-with-multiple-properties-in-css/)
...
**注意：** 如果没有指定 `duration` 部分，过渡将没有效果，因为它的默认值是 `0`。`transition` 属性主要指定两件事。第一个是添加效果的 CSS 属性，第二个是持续时间，除非过渡效果会更小。
