...
例如，我们已经声明了一个标题标签(`h1`)，并分配了一些`属性:值`对，用于设置标题标签的样式。这里，`h1`是选择器，`{颜色:绿色；字体系列:无衬线字体；}`是一个声明块，它可以包含一个或多个用分号分隔的声明，`颜色:绿色；`是一个`属性:值`对，应用于 HTML 元素以设置它们的样式。

```html
h1 { color: green; font-family: sans-serif;}
```

每个声明都有一个 CSS 属性名和值，用冒号分隔并用大括号括起来。对于声明多个 CSS 属性，可以用分号分隔。
...
一个 `CSS 选择器`用于根据 HTML 元素的元素名称、id、属性等选择 HTML 元素。它可以同时选择一个或多个元素。
...
我们将专注于简单选择器并通过示例了解它的实现。

简单选择器有三种分类方式:

### CSS 元素选择器

`CSS 元素选择器`用于选择需要样式化的 HTML 元素。在选择器声明中，有 HTML 元素的名称，应用于该元素的 CSS 属性写在括号`{}`内。
...
**示例:**该示例说明了如何使用元素选择器，通过元素名称选择 HTML 元素，并为其设置样式。
...
`CSS id 选择器`用于设置给定 `id` 的样式。`id` 属性是 HTML 文档中的唯一标识符。`id` 选择器与`#`字符一起使用。
...
**示例:**这个示例说明了如何使用 `id` 选择器来根据 HTML 元素的 `id` 选择它们并设置样式。
...
`CSS 类选择器`用于选择属于特定 `class` 属性的所有元素。要选择具有特定类的元素，请使用`.`字符，指定类名。类名主要用于给定类设置 CSS 属性。
...
**示例:**这个示例说明了如何使用类选择器通过类选择 HTML 元素并设置样式。
...
`CSS *选择器`用于选择一个 HTML 文档中的所有元素。它还会选择另一个元素下面的所有元素。它也被称为通用选择器。
...
**示例:**这个示例说明了通用选择器的使用，用于选择包含通用 CSS 属性的所有 HTML 元素，并为它们设置样式。
