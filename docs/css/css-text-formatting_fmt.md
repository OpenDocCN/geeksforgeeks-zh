...
文本颜色属性用于设置文本的颜色。
文本颜色可以通过使用名称“`red`”、十六进制值“`#ff0000`”或其 RGB 值“`rgb(255, 0, 0)`”来设置。
...
文本对齐属性用于设置文本的水平对齐。
文本可以设置为 `left`、`right`、`center` 和 `justify`。
在 `justify` 对齐方式中，线条被拉伸，使左右边距变直。
...
文本装饰用于在文本中添加或移除装饰。
文字装饰可以是 `underline`、`overline`、`line-through` 或 `none`。
...
文本转换可以是 `uppercase`、`lowercase` 或 `capitalize`。
`capitalize` 用于将每个单词的首字母改为大写。
