...
> 原文: [https://www.geeksforgeeks.org/css-border-inline-width-property/](https://www.geeksforgeeks.org/css-border-inline-width-property/)

CSS 中的 `border-inline-width` 属性用于在样式表中的一个位置设置单个逻辑内联边框宽度属性值。它设置定义边框元素的顶部和底部的边框宽度。
...
以下示例说明了 CSS 中的 `border-inline-width` 属性:
...
`border-inline-width` 属性支持的浏览器如下:
