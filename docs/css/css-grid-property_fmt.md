...
| 值 | 描述 |
| --- | --- |
| `none` | 它是默认值，没有提到行和列的具体大小。 |
| `grid-template-rows / grid-template-columns` | 它用于分隔行和列的大小。 |
| `grid-template-areas` | 它用于使用命名项指定网格布局。 |
| `grid-template-rows / grid-auto-columns` | 它用于指定自动尺寸(高度)和设置自动尺寸列。 |
| `grid-auto-rows / grid-template-columns` | 它用于指定自动大小并设置自动网格大小列。 |
| `grid-template-rows / [grid-auto-flow] grid-auto-columns` | 它用于指定如何放置项目以及自动调整行和列的大小。 |
| `[grid-auto-flow] grid-auto-rows / grid-template-columns` | 它用于指定如何放置项目以及自动调整行和网格模板列的大小。 |
...
*   `grid-template-areas`: 使用命名项目指定网格布局。
*   `grid-auto-rows`: 指定行的自动大小。
*   `grid-auto-columns`: 指定列的自动大小。
*   `grid-auto-flow`: 指定如何放置自动放置的项目，以及行的自动大小。

## 示例 2: 这是 `grid-template-rows` 和 `grid-template-columns` 的示例。
