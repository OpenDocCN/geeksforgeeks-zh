...
### `not` 关键字:
`not` 关键字可以用来检查条件指定的反面。
为了解释这个关键词，下面是一个例子，如果不支持 `display: flex`，应用样式。
...
在上面的示例中，因为浏览器支持 `display: flex`，所以没有应用样式。

### `and` 关键字:
使用 `and` 关键字检查两个条件，如果两个条件都为真，则执行样式块。
...
### `or` 关键字:
当我们想要执行样式块时，即使指定的多个条件中有一个为真，也要使用 `or` 关键字。
