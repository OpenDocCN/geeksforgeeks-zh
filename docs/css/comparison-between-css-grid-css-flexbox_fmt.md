...
> 原文：[https://www.geeksforgeeks.org/comparison-between-css-grid-css-flexbox/](https://www.geeksforgeeks.org/comparison-between-css-grid-css-flexbox/)
...
首先，您必须将容器元素定义为带有`display: grid`。使用`grid-template-columns`和`grid-template-rows`设置列和行的大小，然后将其子元素放入带有`grid-column`和`grid-row`的网格中。
...
要开始，您必须将容器元素定义为带有`display: flex`。
...
*   CSS 网格为网格流动性和`auto`关键字功能部署分数度量单位，以自动调整列或行。
