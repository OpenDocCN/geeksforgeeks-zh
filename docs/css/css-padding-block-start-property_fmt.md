...
> 原文：[https://www.geeksforgeeks.org/css-padding-block-start-property/](https://www.geeksforgeeks.org/css-padding-block-start-property/)

`padding-block-start` 属性用于定义元素的逻辑块开始填充。此属性有助于根据元素的书写模式、方向和文本方向放置填充。
...
*   `auto`：当希望浏览器确定填充块开始大小时使用。
*   `initial`：用于将 `padding-block-start` 属性的值设置为默认值。
*   `inherit`：当希望元素继承其父元素的 `padding-block-start` 属性作为自己的属性时使用。
*   `unset`：用于取消设置默认填充块开始。

下面的例子说明了 CSS 中的 `padding-block-start` 属性：
...
**支持的浏览器：**
`padding-block-start` 属性支持的浏览器如下：
