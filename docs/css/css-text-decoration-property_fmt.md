...
`text-decoration`属性用于“修饰”文本的内容。它本质上是用不同种类的线条装饰文本。是`text-decoration-line`（必选）、`text-decoration-style`、`text-decoration-color`的简写属性。文本装饰跨子文本元素绘制，这意味着如果父元素指定了文本装饰，则其子元素不能移除父元素指定的装饰。
...
`text-decoration-line`：用于设置各种文字装饰（如下划线、上划线等）。
...
**示例:** 该示例说明了`text-decoration-line`属性的使用，该属性的值可以设置为`underline`、`line-through`和`overline`。
...
`text-decoration-style`：用于设置元素的文字装饰（如点状、波浪形等）。
...
**示例:** 该示例说明了`text-decoration-style`属性的使用。
...
`text-decoration-color`：用于指定文本上装饰的颜色（上划线、下划线和贯穿线）。
...
**示例:** 该示例说明了`text-decoration-color`属性的使用。
...
**示例:** 本示例说明了`text-decoration`属性的使用，该属性的值设置为`initial`。
...
**示例:** 该示例说明了`text-decoration`属性的使用，该属性的值被设置为`inherit`。
