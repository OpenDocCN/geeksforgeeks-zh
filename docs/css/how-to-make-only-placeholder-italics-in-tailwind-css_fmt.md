...
> 原文: [https://www.geeksforgeeks.org/how-to-make-only-placeholder-italics-in-tailwind-css/](https://www.geeksforgeeks.org/how-to-make-only-placeholder-italics-in-tailwind-css/)
...
按照以下步骤将自己的实用程序类添加到顺风中。

### 步骤 1

运行下面的代码到你的文件夹终端。这将创建[T3 文件。](https://www.geeksforgeeks.org/node-js-package-json/)
...
将下面的代码复制粘贴到你文件夹的终端。这将为顺风创建所需的节点模块。
...
创建一个公共文件夹，在公共文件夹里面添加 `style.css` 和 `tailwind.css`。

![](img/8f8138aba7ec9685f665df8b5db7ab79.png)

### 第四步

在 `tailwind.css` 文件中添加代码。使用这个文件，你可以自定义你的顺风 CSS 以及默认样式。Tailwind 会在构建时将这些指令与它根据您配置的设计系统生成的所有样式进行交换。
...
打开 `package.json` 文件，在脚本部分下添加以下代码。
...
在终端运行以下代码。这将使用预定义的顺风 css 代码填充您的 `style.css` 文件。
...
最后，运行下面的代码。这将使用安装**顺风 CSS npm** 包时包含的顺风命令行工具为您的项目生成一个**顺风配置文件**。
...
[`::placeholder`](https://www.geeksforgeeks.org/css-placeholder-selector/) 是一个 CSS 伪元素，允许您通过更改文本颜色来设置表单元素的占位符文本的样式，并且允许修改文本的样式。
...
通过使用 `@layer` 指令，顺风会自动将这些样式移动到与 `@tailwind utilities` 相同的位置，以避免意外的[特异性问题](https://www.geeksforgeeks.org/css-specificity/)。现在您可以添加上面的 `placeholder-italic` 类以将 [*字体样式*](https://www.geeksforgeeks.org/css-font-style-property/) **斜体** 应用到占位符，如下例所示。
...
在下面的示例中，第一个输入的占位符是*斜体*，第二个输入是非斜体(默认)。
