...
如果要将表单控件标记为必填，请将`required`属性添加到`<input>`元素。假设您的表单需要电话号码，那么您可以使用此属性来显示需要电话号码。
...
如果您不想接受用户的任何输入，则可以使用禁用的输入。将`disabled`属性添加到`<input>`元素以禁用表单控件。
...
如果要使表单输入为只读，则可以使用只读输入。只读输入和禁用输入之间的主要区别在于禁用输入不是交互式的，而在只读输入中，您可以与输入交互并选择其文本。因此，它们仍然是可聚焦的。将`readonly`属性添加到`<input>`元素以创建只读输入。
...
向`<input>`元素添加类名`pure-input-rounded`，以显示具有圆角的表单控件。它可以用来在你的网页上创建一个搜索框。
...
要标准化和对齐复选框和单选输入，请将类`pure-checkbox`或`pure-radio`添加到`<label>`元素。
