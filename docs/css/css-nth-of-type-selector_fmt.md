...
CSS 选择器中的 `:nth-of-type()` 仅用于样式化其父元素的第 n 个数目的子元素。n 可以是数字、关键字或公式。
...
其中 `number` 是表示匹配元素模式的参数。它可以是 `odd`、`even` 或函数符号。

*   **奇数:** 表示一系列中位置为奇数的元素: 1、3、5 等，从头数到尾。
*   **偶数:** 表示位置在 2、4、6 等数列中偶数的元素，从头数到尾。
...
**示例-2:** 在本示例中，通过使用 `odd` 或 `even` 关键字作为参数来选择每个奇数和偶数元素。
...
**支持的浏览器:** 以下列出了 `:nth-of-type()` 选择器支持的浏览器:
