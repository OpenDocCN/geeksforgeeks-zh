...
> 原文：[https://www.geeksforgeeks.org/how-to-arrange-text-in-multi-columns-using-css3/](https://www.geeksforgeeks.org/how-to-arrange-text-in-multi-columns-using-css3/)
...
*   [`column-count`](https://www.geeksforgeeks.org/css-column-count-property/)：该属性用于统计文档中需要划分的列元素个数。
*   [`column-gap`](https://www.geeksforgeeks.org/css-column-gap-property/)：此属性用于定义列之间的间隙。
*   [`column-rule-style`](https://www.geeksforgeeks.org/css-column-rule-style-property/)：此属性用于指定列之间的样式。
*   [`column-rule-width`](https://www.geeksforgeeks.org/css-column-rule-width-property/)：此属性用于指定列间规则的宽度。
*   [`column-rule-color`](https://www.geeksforgeeks.org/css-column-rule-color-property/)：这个属性用来定义列之间的颜色。
*   [`column-rule`](https://www.geeksforgeeks.org/css-column-rule-property/)：此属性用于定义列间规则的样式、宽度、颜色。
*   [`column-span`](https://www.geeksforgeeks.org/css-column-span-property/)：这个属性用来定义一个元素应该跨越多少列。
*   [`column-width`](https://www.geeksforgeeks.org/css-column-width-property/)：此属性用于指定每列的宽度。
...
*   通过使用通用的 CSS 属性，如 `float`、`padding`、`text-align` 和 `width` 属性。

我们将使用以上两种方法并通过例子来理解它们。

## 示例 1

在本例中，我们将使用 `column-count` 属性，并将该属性设置为 4，因此列数变为 4。
...
在本例中，我们将使用通用 CSS 属性以类似的方式排列多行列，以实现相同的输出。
