...
CSS 中的`border`属性用于设置元素边框的样式。该属性是其他三个属性`border-width`、`border-style`和`border-color`的组合，可以用作这三个属性的简写符号。它设置或返回边框宽度、边框样式、边框颜色属性。
...
*   `width`：该值指定边框的粗细或宽度。
*   `style`：该值为边框指定一种样式，即边框是否为虚线、虚线、实线等。
*   `color`：该值指定边框的颜色。
