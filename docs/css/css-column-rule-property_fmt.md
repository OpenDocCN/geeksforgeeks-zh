...
CSS 中的 `column-rule` 属性用于指定列之间规则的宽度、样式和颜色。
...
*   `column-rule-width`: 该值用于设置列间规则的宽度。该属性值的默认值为 `medium`。
*   `column-rule-style`: 该值用于设置列间规则的样式。该属性值的默认值为 `none`。
*   `column-rule-color`: 该值用于设置列间规则的颜色。该属性值的默认值是元素的颜色。
*   `initial`: 该值用于设置 `column-rule` 属性的默认值。
*   `inherit`: 该值告诉 `column-rule` 属性从其父级继承属性。
