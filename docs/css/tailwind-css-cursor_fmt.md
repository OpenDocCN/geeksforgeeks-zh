...
*   `cursor-auto`: 这是浏览器设置光标的默认类。
*   `cursor-default`: 这是默认的光标类。
*   `cursor-pointer`: 在这个类中，光标是一个指针，表示一个链接。
*   `cursor-wait`: 在这个类中，光标表示程序正忙。
*   `cursor-text`: 在该类中，光标表示可以选择的文本。
*   `cursor-move`: 在这个类中，光标表示要移动的东西。
*   `cursor-help`:
*   `cursor-not-allowed`: 在这个类中，光标表示请求的动作不会被执行。
