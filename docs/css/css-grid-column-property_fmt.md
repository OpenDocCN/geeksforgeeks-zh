...
`grid-column-start` 和 `grid-column-end` 属性可以用三种方式描述：

*   **`auto`**: 元素将被放置在默认流程中。这是默认值。
*   **`span n`**: 它指定在 `grid-column-start` 和 `grid-column-end` 两种情况下项目将跨越的列数。
*   **列线**: 描述 `grid-column-start` 时在哪个列开始显示项目，`grid-column-end` 时在哪个列结束显示项目。

## 示例

### 示例 1：`grid-column: auto`

本示例描述了 `grid-column: auto` 属性。
...
### 示例 2：`grid-column: column line`

本示例描述了 `grid-column: column line` 属性。
...
### 示例 3：`grid-column: span n`

该示例描述了 `grid-column: span n` 属性。
