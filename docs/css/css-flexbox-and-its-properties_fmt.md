...
> 原文: [https://www.geeksforgeeks.org/css-flexbox-and-its-properties/](https://www.geeksforgeeks.org/css-flexbox-and-its-properties/)

## 预 Flexbox

总结 CSS 中可能与学习 Flexbox 相关的一些属性。

*   我们有不同的位置属性，例如 `absolute` 和 `relative`，这取决于我们想要如何在页面中放置元素。
*   浮动，网格，明确的修复，以帮助定位。
*   列的固定高度（创建弹性框的主要元素之一）。
...
*   `display` 定义一个伸缩容器——伸缩格式上下文。
*   `flex-direction` 定义容器内的主轴。
*   `flex-wrap` 允许柔性物品缠绕在多条线上。
*   `flex-flow` 是 `flex-direction` + `flex-wrap` 的简写。
*   `justify-content` 允许项目沿主轴对齐。
*   `align-content` 允许项目在一条直线上沿横轴对齐。
*   `align-items` 在横轴上对齐多行物品。

### 子属性

*   `order` 允许在不改变源顺序的情况下改变物品的顺序。
*   `flex-grow` 允许一个项目填满可用的空闲空间。
*   `flex-shrink` 如果没有足够的可用空间，允许物品收缩。
*   `flex-basis` 在空间分配之前定义项目的大小。
*   `flex` 是 `flex-grow` + `flex-shrink` + `flex-basis` 的简写。
*   `align-self` 能够在 flex 容器内对齐单个项目。
