...
*   `skew-x-{amount}`: 此类指定 x 轴对应的歪斜角度。
*   `-skew-x-{amount}`: 该类指定与反 x 轴对应的歪斜角度。
*   `skew-y-{amount}`: 该类指定 y 轴对应的歪斜角度。
*   `-skew-y-{amount}`: 此类指定与反 y 轴对应的歪斜角度。

## 注释

1.  首先使用`transform`实用程序启用变换，然后使用`skew-x-{amount}`和`skew-y-{amount}`实用程序指定倾斜角度，从而倾斜元素。
2.  `amount`完全取决于你的选择，你可以将其设置为百分比满，或者直接放入`rem`值。
