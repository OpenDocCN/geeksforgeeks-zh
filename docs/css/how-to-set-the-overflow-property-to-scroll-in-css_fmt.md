...
在本文中，我们将看到如何设置[溢出属性](https://www.geeksforgeeks.org/css-overflow/)在 CSS 中滚动。`overflow`属性用于控制大内容。它告诉我们当一个元素的内容太大而无法放入指定的区域时该怎么做。当`overflow`属性设置为`scroll`时，溢出将被剪切，但会添加一个滚动条来查看其余部分。
...
在本例中，我们必须使用 [`overflow-x` 属性](https://www.geeksforgeeks.org/css-overflow-x-property/)进行滚动。当内容在左右边缘溢出时，使用`overflow-x`属性。
...
在本例中，我们使用了`overflow-y`属性进行滚动。当内容在上下边缘溢出时，使用[溢出属性](https://www.geeksforgeeks.org/css-overflow-y-property/)
