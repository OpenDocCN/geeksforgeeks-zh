...
> 原文：[https://www.geeksforgeeks.org/making-a-div-vertically-scrollable-using-css/](https://www.geeksforgeeks.org/making-a-div-vertically-scrollable-using-css/)

通过使用 CSS `overflow` 属性，使一个 `div` 可以垂直滚动是很容易的。`overflow` 属性中有不同的值。例如：`overflow: auto;` 以及像 `overflow-x: hidden;` 和 `overflow-y: auto;` 这样的轴控制。它将使垂直和水平滚动条，而 `auto` 将只使垂直滚动条。

对于垂直可滚动栏，使用 x 轴和 y 轴。设置 `overflow-x: hidden;` 和 `overflow-y: auto;`，这将自动隐藏水平滚动条，只显示垂直滚动条。在这里，滚动 `div` 可以垂直滚动。
...
在此示例中，使用 `auto` 代替 `overflow-x: hidden;` 和 `overflow-y: auto;` 使 `div` 垂直滚动。
