...
> 原文：[https://www.geeksforgeeks.org/css-scroll-padding-inline-start-property/](https://www.geeksforgeeks.org/css-scroll-padding-inline-start-property/)

`scroll-padding-inline-start`属性用于一次性将所有滚动填充设置到内联维度中滚动元素或容器的开始边缘。起始侧分别是`horizontal-tb`书写模式的左侧和`vertical-lr`或`vertical-rl`书写模式的顶部或底部，其中`horizontal-tb`代表*水平从上到下*和`vertical-rl`或`vertical-lr`代表*垂直从右向左*和*垂直从左向右*。
...
*   **keyword_values:** 该属性是指用`auto`等单位定义的keyword_values。一般来说，这将被设置为`0px`默认情况下，但它可以是非零值以及如果非零值是更合适的。

## 示例

在本例中，您可以通过滚动到示例内容的两个“界面”中间的某个点来查看`scroll-padding-inline-start`的效果。
