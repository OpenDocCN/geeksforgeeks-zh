...
> 原文: [https://www.geeksforgeeks.org/how-to-set-a-background-color-to-full-page-using-tailwind-css/](https://www.geeksforgeeks.org/how-to-set-a-background-color-to-full-page-using-tailwind-css/)

在本文中，我们将学习使用 [Tailwind CSS](https://www.geeksforgeeks.org/css-tailwind-introduction/) 应用背景色。

## 进场

我们可以通过简单地改变一个 HTML 主体的屏幕高度来设置一个整页的背景颜色。在 Tailwind CSS 中，我们使用了另一种 CSS `background-color` 属性，表示为 [`background-color`](https://www.geeksforgeeks.org/tailwind-css-background-color/) (例如: `bg-blue-200` )，用于指定元素的背景色。背景覆盖有填充和边框但不包括边距的元素的总大小。
...
以下示例使用 3 个 `div` 设置 HTML 文档的全背景色。
...
如果像上面的代码一样避免使用工具来设置像 `h-screen` 这样的元素的高度，那么通过简单地在 HTML 主体上使用 `bg-blue-200`，您将得到相同的结果。
