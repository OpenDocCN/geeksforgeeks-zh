...
这个类在 tailwind CSS 中接受两个值。它是 [`CSS 可见性属性`](https://www.geeksforgeeks.org/css-visibility-property/#:~:t) 的替代品。此类用于指定一个元素在 web 文档中是否可见，但隐藏的元素会占用 web 文档中的空间。使用 `display` 属性从浏览器中移除或隐藏并删除元素。
