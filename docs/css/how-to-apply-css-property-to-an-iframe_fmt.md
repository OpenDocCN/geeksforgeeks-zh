...
> 原文：[https://www.geeksforgeeks.org/how-to-apply-css-property-to-an-iframe/](https://www.geeksforgeeks.org/how-to-apply-css-property-to-an-iframe/)

一个 `iframe` 是**内联框架**的简称。通过使用 `iframe` 标签，您可以将另一个网页内容以矩形结构显示到 HTML 页面文档中。网页内容可以是任何视频、另一个网站、任何图像等等。这个 `iframe` 也有滚动条和边框，让文档有良好的外观和感觉。
...
有几种技术可以将 CSS 属性应用于 iframe，如下所示。我们可以通过在 `iframe` 标签中使用 CSS 来为 `iframe` 使用**内嵌 CSS**。
...
在下面的示例中，`iframe` 的宽度和高度都是 `300px`，边框的厚度是 `3px` 并且是**点线**样式。
...
您可以使用**内部 CSS** 来标记 HTML 文件中的 `iframe`。
...
可以对 `iframe` 使用**外部 CSS**，也就是利用外部 CSS 文件。使用 `iframe` 标签前的 `link` 和 `href` 标签，为 CSS 代码创建一个不同的文件，并将其包含在 HTML 文件中。
...
在这种方法中，使用 JavaScript 代码在 HTML 文档的头部添加 CSS 文件链接。使用的方法有：
