...
CSS 数据类型整数代表 `<integer>`。它是一个整数，可以是正（`+`）或负（`-`）。整数包含 0-9 之间的一个或多个数字。不允许使用十进制值。

表示整数值不需要单位。很多 CSS 属性都取整数值，比如 `z-index`、`line-height`、`column-count`、`counter-increment`、`grid-column` 等等。
...
<html>
    ...
    <body>
        ...
        <div class="gfg">
            ...
            is allowed. No units are required to represent an integer value.
...
在这个例子中，`z-index` 接受一个整数作为输入。因为图像的 `z-index` 为 `-1`，所以它将被放在标题后面。