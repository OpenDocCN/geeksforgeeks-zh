...
CSS 中的 `:indeterminate` 选择器用于选择任何处于不确定状态的表单元素，即既没有选中也没有取消选中的状态。

该选择器的目标元素是:
*   `indeterminate` 属性被 JavaScript 设置为 `true` 的 `<input>` 元素
*   当表单中具有相同 `name` 值的所有单选按钮未被选中时的 `<input>` 元素
...
**支持的浏览器:** `:indeterminate` 选择器支持的浏览器如下:
*   苹果 Safari
*   谷歌 Chrome 39.0
*   Firefox 51.0
*   歌剧
