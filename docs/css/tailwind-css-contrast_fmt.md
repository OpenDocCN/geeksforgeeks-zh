...
对比度类用于设置图像的对比度。此类可以增加或减少对比度。在 CSS 中，我们通过使用 [CSS `contrast()` 函数](https://www.geeksforgeeks.org/css-contrast-function/)来实现。顺风 CSS 在 2.1 版本中新增了特色亮度。

## 对比度类

*   `contrast-0`：此类用于设置与 CSS `contrast(0)` 相当的对比度。
*   `contrast-50`：这个类用来设置相当于 CSS `contrast(0.5)` 的对比度。
*   `contrast-75`：这个类用来设置相当于 CSS `contrast(0.75)` 的对比度。
*   `contrast-100`：这个类用来设置相当于 CSS `contrast(1.0)` 的对比度。
*   `contrast-125`：这个类用来设置相当于 CSS `contrast(1.25)` 的对比度。
*   `contrast-150`：这个类用来设置相当于 CSS `contrast(1.50)` 的对比度。
*   `contrast-200`：这个类用来设置相当于 CSS `contrast(2.0)` 的对比度。
