...
> 原文: [https://www.geeksforgeeks.org/how-to-create-a-mega-menu-using-html-and-css/](https://www.geeksforgeeks.org/how-to-create-a-mega-menu-using-html-and-css/)

`HTML` 是一种用于创建网页的标记语言，`CSS` 是一种样式表语言，用于设计用 `HTML` 等标记语言编写的文档。在本文中，我们将了解如何仅借助于 `HTML` 和 `CSS` 在我们的网页上创建一个大型菜单。巨型菜单是一种可扩展菜单，其中许多选项显示在一行中。我们必须创建一个菜单，当用户将鼠标移动到导航栏内的一个元素上时，该菜单就会出现。

## 创建超级菜单的步骤

### 创建一个 HTML 文件

创建一个导航菜单，并在导航栏中使用一个按钮作为菜单，作为一个超级菜单工作，如下代码所示。
...
通过 `CSS` 设计你的导航栏和巨型菜单，并将该文件链接到你的 `HTML` 页面。以下是上述 `HTML` 文件中使用的文件 `menu.css` 的内容。
