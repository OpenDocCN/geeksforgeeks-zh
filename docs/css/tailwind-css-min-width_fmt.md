...
这个类在 [Tailwind CSS](https://www.geeksforgeeks.org/css-tailwind-introduction/) 中接受很多值，其中所有的属性都以类的形式被覆盖。它是 [CSS `min-width` 属性](https://www.geeksforgeeks.org/css-min-width-property/)的替代品。此类用于定义元素的最小宽度。宽度值不能小于 `min-width` 的值。如果元素中指定的内容较小，`min-width` 保持指定的最小宽度。

## 最小宽度等级

*   `min-w-0`: 此类用于设置 `min-width` 的长度。
*   `min-w-full`: 此类用于设置满负荷时 `min-width` 的长度。
*   `min-w-min`: 此类用于将 `min-width` 的长度设置为最小容量。
*   `min-w-max`: 该等级用于设置最大容量时 `min-width` 的长度。
