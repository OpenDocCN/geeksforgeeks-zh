...
> 原文: [https://www.geeksforgeeks.org/how-to-wrap-table-cell-td-content-using-css/](https://www.geeksforgeeks.org/how-to-wrap-table-cell-td-content-using-css/)
...
有两种方法使用 CSS 将表格单元格`<td>`内容换行，如下所示:
...
**注意:** 在上表中，我们定义了一个 600px 的`table`宽，并将`table-layout`应用为`fixed`。注意标记为红色的长单词是通过去除空格而形成的，例如目的。

**方法一:** 使用换行属性: `word-wrap: break-word;` 属性用于在适当的断点处断开长单词。
...
**方法二:** 使用断字属性: `word-break: break-all;` 属性用于在任何字符处断开单词。
