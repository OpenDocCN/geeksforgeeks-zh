...
这个类用于在一个容器中分配行，使得每个项目周围有相等的空间，但是也考虑了当使用*内容-周围*类*时，您通常会在每个项目之间看到的双倍空间。*
