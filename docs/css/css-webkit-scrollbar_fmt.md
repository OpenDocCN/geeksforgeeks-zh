...
`::-webkit-scrollbar` 是 CSS 中的一个伪元素，用于修改浏览器滚动条的外观。在我们开始讨论它如何工作以及如何实现之前，我们需要了解一些关于元素的事实。
...
*   `::-webkit-scrollbar-thumb`: 可以拖动的滚动手柄
*   `::-webkit-scrollbar-track`: 进度条
*   `::-webkit-scrollbar-track-piece`: 手柄未覆盖的区域
*   `::-webkit-scrollbar-corner`: 滚动条的底角，垂直滚动条和水平滚动条在此交汇
...
支持的浏览器 `CSS ::-webkit-scrollbar` 如下:
