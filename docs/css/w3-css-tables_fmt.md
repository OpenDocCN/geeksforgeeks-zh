...
**简单表:** 使用`.w3-table`类创建一个简单的W3.CSS表格。这个类名与`<table>`标签一起使用来创建一个表。
...
**剥离行:** 使用`.w3-striped`类来创建交替的暗行和亮行。使用`<table>`标签中的`table`、`w3-table`和`w3-striped`类的组合来创建一个剥离表。

**语法:**

```html
<table class="w3-table w3-striped"> Table Contents... <table>
```
...
**加边框表格:** 使用`.w3-border`在表格中添加边框。边框只出现在表格周围，而不出现在表格中。在`table`标记中使用`.w3-border`和`.w3-table`可以看到这种效果。
...
如果我们想要一个完全有边框的表，我们必须在表标签中使用`.w3-bordered`类以及`.w3-border`和`w3-table`。
...
**可悬停表格:** 使用`.w3-hoverable`类在表格行上添加悬停效果(鼠标移动时背景颜色变为灰色)。在`<table>`标签中使用`w3-table`和`w3-hoverable`类的组合来创建悬停行表。
...
`w3-center` 类用于将表的所有内容放在中心。在 `<table>` 标签中使用 `w3-table` 和 `w3-center` 类的组合来创建这种效果。
