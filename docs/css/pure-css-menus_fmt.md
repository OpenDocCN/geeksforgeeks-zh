...
Pure.CSS 提供的菜单默认为垂直菜单。 默认样式是最小的，因此开发人员可以轻松地对其进行自定义。 默认垂直菜单通常包含其容器的全宽，但开发人员可以指定它。 开发人员还可以相应地设置显示属性。
...
*   `pure-menu-heading` 是为菜单列表内部或外部的标题添加的类。 默认情况下，它将内部文本大写。
...
将类 `pure-menu-horizontal` 添加到默认垂直的 `pure-menu` 以使其成为水平菜单。 子元素，如 `pure-menu-list`、`pure-menu-item`、`pure-menu-link` 和 `pure-menu-heading` 保持与默认垂直菜单相同。
...
纯 CSS 提供了默认类，使我们能够将菜单项突出显示为禁用或选中。 在 `pure-menu-item` 之后添加 `pure-menu-disabled` 类名以禁用项目，并添加 `pure-menu-selected` 类名以使项目看起来像被选中的。
...
下拉项目只能添加到水平菜单。 下面讨论创建下拉菜单的步骤。

*   选择要添加下拉项的列表项，并向其中添加类 `pure-menu-has-children`。
*   在该元素内创建一个无序列表，其类为 `pure-menu-children`。
*   使用 `pure-menu-item` 类添加其中的列表项。
