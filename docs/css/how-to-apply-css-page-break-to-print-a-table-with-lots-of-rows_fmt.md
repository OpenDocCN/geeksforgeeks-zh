...
> 原文：[https://www.geeksforgeeks.org/how-to-apply-css-page-break-to-print-a-table-with-lots-of-rows/](https://www.geeksforgeeks.org/how-to-apply-css-page-break-to-print-a-table-with-lots-of-rows/)
...
1.  `page-break-before`：在元素之前添加分页符
2.  `page-break-after`：在元素后添加分页符
3.  `page-break-inside`：设置元素内部是否应该避免分页符。
...
这里，`name_of_the_element` 指的是需要分页的元素（例如，`table`）。`name_of_the_property` 是指需要应用于元素的属性。值规定了打印文档时给定属性的行为方式（例如，`always`）。
