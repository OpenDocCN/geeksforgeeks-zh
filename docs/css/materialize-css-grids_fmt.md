...
有 12 个标准列流体响应网格系统，帮助您以有序和简单的方式布局页面。使用行和列样式类分别定义行和列。`row` 用于指定用于响应列的无填充容器，`col` 用于指定具有子类的列。

有一个 `container` 类用于将页面内容居中。容器类设置为窗宽的~70%。要添加 `container`，只需将您的内容放入带有 `container` 类的 `<div>` 标签中。下面是语法：
...
标准网格有 12 列。无论浏览器的大小如何，这些列中的每一列都将始终具有相等的宽度。创建布局时，请记住所有列必须包含在一行中，并且您必须将列类添加到您的内部 `<div>` 中，以使它们成为列。您可以通过 **推拉** 轻松更改您的列顺序。只需将 `push-s2` 或 `pull-s2` 添加到类中，其中 `s` 是屏幕类前缀（`s` = 小，`m` = 中，`l` = 大），后面的数字是您想要推或拉的列数。
...
| 断点 | 移动设备 <= 600px | 平板设备 >600px | 桌面设备 >992px | 大桌面设备 >1200px |
| :--- | :--- | :--- | :--- | :--- |
| **Container width** | 90% | 85% | 70% | 70% |
| **Class prefix** | `.s` | `.m` | `.l` | `.xl` |
| **Number of columns** | Twelve | Twelve | Twelve | Twelve |

在前面的例子中，我们只使用 `col s12` 来定义小屏幕的尺寸。仅仅说 `s12`，我们实际上是在说 `s12 m12 l12` 类。但是通过明确定义大小，我们可以使我们的网站更具响应性。
...
<body>
    ...
    <!--Responsive layout-->
    <div class="row">
        <div class="grid-example col s12 light-green center">
            <span class="flow-text">
                Always full-width (col s12)
            </span>
        </div>
        <div class="grid-example col s8 m6 green center">
            <span class="flow-text">
