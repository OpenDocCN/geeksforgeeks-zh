...
> 原文：[https://www.geeksforgeeks.org/how-to-animate-rainbow-heart-from-a-square-using-css/](https://www.geeksforgeeks.org/how-to-animate-rainbow-heart-from-a-square-using-css/)
...
首先，我们将在主体中创建两个分区，分别是`square`和`container`。然后，在`style.css`文件中，我们会添加一些通用的主体和类样式，使所有内容居中。然后我们将使用`::before`和`::after`选择器创建两个圆，并使用`border-radius`属性使其成为一个圆。
...
从正方形变为心形时的心脏运动可以使用其中一个关键帧中的`transform`属性来添加。这将导致从正方形到心形的转变。
