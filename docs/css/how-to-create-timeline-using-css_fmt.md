...
> 原文: [https://www.geeksforgeeks.org/how-to-create-timeline-using-css/](https://www.geeksforgeeks.org/how-to-create-timeline-using-css/)

我们可以使用一些基本的 HTML 和 CSS 轻松地创建一个时间线。HTML 代码用于创建时间轴的基本结构，CSS 代码用于设置样式。

## HTML 代码

在这一节中，我们将创建一个时间线的结构。我们的结构将包括四个事件。

**步骤:**

*   用类`main-container`创建一个`div`元素。
*   在我们的主容器中，用类`text-wrapper`再创建四个`div`。
*   在每个包含类`text-wrapper`的`div`中包含另一个包含类`content`的`div`，并在其中包含一个`h3`和`p`标签。
...
我们将使用 CSS 给我们的时间线一些结构，并设置事件的位置。
