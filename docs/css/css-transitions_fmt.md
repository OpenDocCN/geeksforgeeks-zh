...
## `transition-property`
此属性允许你选择你想要在过渡（变化）期间制作动画的 CSS 属性。
...
*   我们可以指定单个 `property` 或一组逗号分隔的属性 `property1, property2, …, propertyN`。

## `transition-duration`
此属性允许你确定从一种 CSS 属性过渡到另一种属性需要多长时间。

**语法:**
```html
transition-duration: time;
```
这里，`time` 可以是秒(`s`)或者毫秒(`ms`)，你应该在数字后面用‘s’或者‘ms’(不带引号)。

## `transition-timing-function`
此属性允许你确定过渡期间变化的速度和方式。例如，变化应该在开始时快，结束时慢，等等。
...
## `transition-delay`
此属性允许你确定在过渡实际开始发生之前需要等待的时间量。

**语法:**
```html
transition-delay: time;
```
这里，同样，`time` 可以是秒(`s`)或毫秒(`ms`)，你应该在数字后使用‘s’或‘ms’(不带引号)。

## 速记属性
根据下面给出的语法，您可以将上面提到的所有四个过渡属性组合成一个速记属性。这样我们就不用写长代码，也不会弄得一团糟。注意属性的排序，它有意义。

**语法:**
```html
transition: (property name) | (duration) | (timing function) | (delay);
```
取值同上。此属性必须与初始状态的其他 CSS 属性(如果有)放在一起。你应该至少使用 `property name` 和 `duration` 来获得任何可动画化的效果。此外，价值观的排序也很重要。第一个值是属性名，第二个值是持续时间，依此类推，如上所列。因此，如果只提到一个数字，它将作为持续时间，而不是作为延迟。
