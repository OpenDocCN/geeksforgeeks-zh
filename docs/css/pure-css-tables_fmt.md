...
*   `pure-table`:该类用于为表格设置默认填充样式，并为带有强调标题的表格元素分配边框。
*   `pure-table-bordered`:该类用于在表格上垂直和水平绘制所有表格单元格的边框。
*   `pure-table-horizontal`:这个类用来绘制只有水平线的表格。
*   `pure-table-odd`:这个类用来创建一个斑马风格效果的条纹表，对于用户来说更加可见和吸引人。
*   `pure-table-striped`:基本上是用来显示分条表的。它在传递到`pure-table`类旁边的`<table>`元素后自动分条一个表。

**示例:**现在我们在这里学习如何在 HTML 中使用表类。

## 默认表格

`pure-table`类在`<table>`标签中使用，以一些基本的 CSS 样式创建一个基本表格，这些表格就像普通的 HTML 表格一样，带有一些填充和边框，添加到带有标题的表格元素中。
...
这里我们使用`pure-table-bordered`类来创建加边表格。这个类将为表格的所有单元格添加垂直和水平边框。
...
这里我们使用了`pure-table-horizontal`类来创建这种类型的表格。这将创建一个只有水平线的表格。
...
为了更好地可视化大尺寸表格，我们可以使用分条单元格。对于这里的分条表，我们在每一个`<tr>`元素标签中使用了`pure-table-odd`纯 CSS 类来改变奇数行的背景。这将创建一个具有斑马风格效果的分条表，对于用户的大量数据来说，这种效果更加明显和吸引人。
