...
> 原文: [https://www.geeksforgeeks.org/what-is-the-best-way-to-include-css-file-why-use-import/](https://www.geeksforgeeks.org/what-is-the-best-way-to-include-css-file-why-use-import/)
...
### 外部样式表(使用 HTML `<link>` 标签)

外部 CSS 包含单独的 CSS 文件，借助标签属性(例如类、id、标题等)仅包含样式属性。属性编写在一个单独的文件中，具有 `.css` 扩展名，应该使用 `<link>` 标签链接到 HTML 文档。这意味着对于每个元素，样式只能设置一次，并且将跨网页应用。`<link>` 标签用于链接外部样式表和 HTML 网页。

```html
<link rel="stylesheet" href="style.css">
```

### 外部样式表(使用 `@import` At-Rule)

At-rule 方法必须包含在 `<style>` 标签中，或者包含在样式表中。
...
这可以在单个 HTML 文档必须唯一设置样式时使用。CSS 规则集应该在标题部分的 HTML 文件中，即 CSS 嵌入在 HTML 文件中。
...
内联 CSS 包含的 CSS 属性在正文部分附带的元素被称为内联 CSS。这种样式是使用 `style` 属性在 HTML 标记中指定的。它用于为单个元素应用独特的样式。
...
外部样式表(使用 HTML `<link>` 标签)是用于链接元素的最佳方法。跨不同页面维护和重用 CSS 文件既简单又高效。`<link>` 标签放置在 HTML `<head>` 元素中。要为级联样式表指定媒体类型，请使用 `type="text/css"` 属性，该属性用于忽略浏览器中不支持的样式表类型。

### 例 1

下面给出的文件包含 CSS 属性。此文件与 HTML 一起保存，具有 `.css` 扩展名。例如: `geeks.css`
