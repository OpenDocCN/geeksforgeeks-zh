...
> 原文：[https://www.geeksforgeeks.org/css-border-image-slice-property/](https://www.geeksforgeeks.org/css-border-image-slice-property/)

在 CSS 中，`border-image-slice` 属性用于分割或切片由 [`border-image-source`](https://www.geeksforgeeks.org/css-border-images/) 属性指定的图像。

`border-image-slice` 属性将给定图像分为：
...
**注意：** 中间区域默认保持透明，`fill` 值用于使其不透明/半透明。
下图显示了上述区域：
...
| `fill` | Make the middle area appear as the background image. |
| `initial` | Initializes the property to the default value. |
| `inherit` | Inherit the value of the attribute from its parent element. |

下面的程序举例说明了 `border-image-slice` 属性：
