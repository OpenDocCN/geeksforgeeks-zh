...
*   [`边距`](https://www.geeksforgeeks.org/css-margins-padding/): 该属性用于在元素 ie 周围创建空间。，在边境地区周围。
...
为了设置元素的`宽度`和`高度`属性(为了在浏览器中正确呈现内容)，我们需要了解 `CSS Box 模型`的工作原理。

在用 CSS 设置元素的`宽度`和`高度`属性时，我们只设置了内容区域的宽度和高度。我们需要添加`填充`、`边框`和`边距`，以便计算元素的完整大小。考虑下面的例子。
...
该示例说明了如何使用 CSS Box 模型来对齐&并正确显示它。
