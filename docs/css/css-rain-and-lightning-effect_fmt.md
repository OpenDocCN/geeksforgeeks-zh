...
> 原文：`https://www.geeksforgeeks.org/css-rain-and-lightning-effect/`

为了生成雨和闪电效果，我们将使用允许动画 HTML 元素的 CSS 动画。

我们将使用`@关键帧`，允许动画在特定时间从当前样式逐渐变为新样式，然后我们将使用`filter: hue-rotate()`来产生闪电效果。

## 实现步骤

*   在区域中添加背景图像。
*   要生成雨水效果，请使用 CSS 的`::before`伪元素，将雨水的背景图像添加到同一部分。
*   我们将创建一个名为`color-change`、持续`10s`且无限循环的闪电效果动画。
*   为了创建颜色变化动画，我们将使用`@关键帧`，并使用属性`filter: hue-rotate(度)`给出效果。
*   为了再次创建一个下雨效果，我们将使用`@关键帧`属性，并在某个时间间隔内更改背景图像的位置。
