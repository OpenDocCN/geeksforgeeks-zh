...
> 原文：[https://www.geeksforgeeks.org/tailwind-css-grid-column-start-end/](https://www.geeksforgeeks.org/tailwind-css-grid-column-start-end/)
...
这个类会覆盖跨度面积，提到数字后这个类会保存一个跨度的面积，我们都知道有 12 个网格柱或者你可以说 12 个网格跨度。
...
## 起始线和结束线 (`col-start`|`end`)

此类用于使元素在第 n 个网格线开始或结束。这些还可以与 `col-span-number` 实用程序结合使用，以跨越特定数量的列。

**注意**：我们可以将这个类和上述的类（跨越列 `col-span`）合并，下面的例子会给你一个如何使用的思路。
