...
> 原文: [https://www.geeksforgeeks.org/how-to-specify-a-fixed-background-image-in-css/](https://www.geeksforgeeks.org/how-to-specify-a-fixed-background-image-in-css/)

在本文中，我们将看到如何在 CSS 中指定一个固定的背景图像。为了在 CSS 中保持背景固定、滚动或局部，我们必须使用 `background-attachment` 属性。

## 背景-附件属性

[`background-attachment`](https://www.geeksforgeeks.org/css-background-attachment-property/) 属性在 CSS 中用来设置背景图像为固定或滚动。此属性的默认值是 `scroll`。

`background-attachment` 属性的值:

*   `scroll`: 是 `background-attachment` 属性的默认值。它用于滚动带有背景页的图像。
*   `fixed`: 背景图像不会滚动。它与页面固定在一起。
*   `local`: 背景图像会随着内容滚动。

要保持背景图像固定，必须使用值为 `fixed` 的 `background-attachment` 属性。
