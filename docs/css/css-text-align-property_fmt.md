...
CSS 中的 `text-align` 属性用于指定元素 ie 中文本的水平对齐方式。，它用于在块元素或表格单元格框内水平设置内容的对齐方式。
...
方向为 `ltr` 时向左，方向为 `rtl` 时向右。

## 属性值

*   `left`: 用于将文本对齐设置为左侧。这是默认属性。
*   `right`: 用于将文本对齐设置为右侧。
*   `center`: 用于将文本对齐设置为居中。
*   [`justify`](https://www.geeksforgeeks.org/css-text-justify-property/): 用于将词语展开成一条完整的线，即通过拉伸一个元素的内容。
...
本示例说明了使用 `text-align` 属性，以便将其与指定值对齐。
