...
文本“旧文本”需要首先隐藏，新文本必须准确定位在旧文本所在的位置。为此，我们首先使用 CSS 将该文本的 `visibility` 更改为 `hidden`。
...
注意 `:after` 是这里使用的伪元素。我们再次使用 `visibility` 修改器来显示新文本。`content` 属性包含新文本。
...
## 方法 2: 使用带有 `<span>` 标签的伪元素和可见性修改器

在这个方法中，我们需要更多一点的标记，但是我们不再需要为我们的新文本指定任何绝对位置。我们使用 `<span>` 标签包装文本。

```html
<p class="toBeReplaced"><span>Old Text</span></p>
```

在这个方法中，我们使用一个子元素来包装文本，也就是说，文本现在在 `<span>` 和 `</span>` 标签内。所以我们可以使用 `display: none` CSS 属性来隐藏 `<span>` 元素中的文本。然后我们可以简单地替换文本，就像我们在前面的方法中所做的那样，而不需要指定任何定位。
