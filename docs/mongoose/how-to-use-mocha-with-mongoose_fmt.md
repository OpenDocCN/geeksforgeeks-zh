...
`.on()` 和 `.once()` 是事件处理程序。它们将事件作为第一个参数，将函数作为第二个参数，一旦事件发生，函数就会执行。
...
`--recursive` 表示它将在测试目录中递归测试，并且 `--exit` 表示在所有测试执行完毕后退出。使用以下命令使用 Mocha 运行测试。
...
通过在测试回调中向 `it()` 添加 `done()`，摩卡将知道这个测试已经完成。
