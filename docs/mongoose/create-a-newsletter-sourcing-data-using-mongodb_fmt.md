...
这是启动应用运行所需的重要文件。要调用 ndtv 站点，刮取数据，并将其存储在 MongoDB 数据库中。
...
要获取`ndtv.com`的新闻，我们需要点击**获取新文章**。这将在内部调用我们的`/刮擦`路径。一旦这个调用完成，在`MongoDB`中，在`ndtvnews`数据库下，名为`articles`的集合将填充如下所示的数据:

![](img/4d77c7f812c13be265faf1caefb6551c.png)

在这里，最初保存的`saved`属性将为假，`_id`在`MongoDB`中自动创建，这是集合中文档的唯一标识。该属性仅帮助查看文档、保存文档等。

![](img/5c192ed58877cea965e017aa7aa04e21.png)

点击**在NDTV上查看文章**，将导航到相应的文章。这之所以成为可能，只是因为`articles`集合中存在`_id`属性。所以，当我们点击NDTV上的**查看文章**时，作为一个超链接，它会直接内部拾取文档的`_id`值，显示`link`。点击**保存文章**时，`_id`值将是该文章的标识部分。
...
**结论:**
抓取任何一个新闻网站，单独显示标题内容连同后面的链接一起进行，都是比较简单容易的，我们可以很容易的保存文章，查看保存的文章。
