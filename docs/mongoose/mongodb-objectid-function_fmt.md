...
MongoDB 使用 `ObjectID()` 为数据库中的所有文档创建唯一的标识符。它不同于传统的自动递增整数标识，但它有自己的一套优势。

对象标识符是一个 GUID（全球唯一标识符）。GUID 通过算法随机生成，以确保唯一性。这些标识可以在服务器上生成，但是如上面的片段所示，它们也可以在客户端上生成。这意味着客户端可以为将要插入数据库的文档生成标识。
