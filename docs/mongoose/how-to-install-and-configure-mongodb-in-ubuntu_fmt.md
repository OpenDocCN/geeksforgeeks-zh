...
> 原文: [https://www.geeksforgeeks.org/how-to-install-and-configure-mongodb-in-ubuntu/](https://www.geeksforgeeks.org/how-to-install-and-configure-mongodb-in-ubuntu/)

## 简介

`MongoDB` 是一个面向开源文档的 `NoSQL` 数据库，用 `C++` 编写。`MongoDB` 是无模式数据库系统，因此很容易在其中添加新字段。这是一个分布式系统，因此数据恢复是即时和更可靠的。
...
首先需要更新和升级系统存储库，以便安装 `MongoDB`。在您的终端中键入以下命令，然后按回车键。
...
现在，使用 `apt` 安装 `MongoDB` 包。键入以下命令，然后按回车键。
...
阅读更多关于 `Unix` 中 [`systemctl` 命令](https://www.geeksforgeeks.org/systemctl-in-unix/)的信息。