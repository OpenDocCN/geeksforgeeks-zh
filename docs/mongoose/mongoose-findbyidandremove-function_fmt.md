...
> 原文: [https://www.geeksforgeeks.org/mongoose-findbyidandremove-function/](https://www.geeksforgeeks.org/mongoose-findbyidandremove-function/)

`findByIdAndRemove()`函数用于查找匹配的文档，将其移除，将找到的文档(如果有)传递给回调。
...
这就是如何使用 `mongoose` 的 `findByIdAndRemove()` 来查找匹配的文档，将其移除，并将找到的文档(如果有)传递给回调。