...
> 原文: [https://www.geeksforgeeks.org/c-look-disk-scheduling-algorithm/](https://www.geeksforgeeks.org/c-look-disk-scheduling-algorithm/)
...
1.  设 `Request` 数组表示一个数组，该数组存储已按到达时间升序请求的磁道索引，`head` 是磁盘磁头的位置。
...
C-LOOK 算法的实现如下。**注意** `distance` 变量用来存储磁头和当前磁道位置之间的绝对距离，`disk_size` 就是磁盘的大小。向量 `left` 和 `right` 分别在初始头部位置的左侧和右侧存储所有请求轨迹。
