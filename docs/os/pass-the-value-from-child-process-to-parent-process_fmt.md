...
> 原文: [https://www.geeksforgeeks.org/pass-the-value-from-child-process-to-parent-process/](https://www.geeksforgeeks.org/pass-the-value-from-child-process-to-parent-process/)

## 先决条件
[Pipe()和 Fork() Basic](https://www.geeksforgeeks.org/c-program-demonstrate-fork-and-pipe/)

编写一个 C 程序，子进程取一个输入数组，用 `Pipe()` 和 `Fork()` 发送给父进程，然后在父进程中打印出来。

## 示例
假设子进程中有一个数组 `a[]={1，2，3，4，5}`，那么输出应该是 `1 2 3 4 5`。
