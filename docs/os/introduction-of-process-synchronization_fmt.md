...
> 原文：[https://www.geeksforgeeks.org/introduction-of-process-synchronization/](https://www.geeksforgeeks.org/introduction-of-process-synchronization/)
...
*   布尔`flag[i]`：初始化为`FALSE`，最初没有人对进入临界区感兴趣
...
信号量使用两个原子操作，`wait()`和`signal()`来实现进程同步。
