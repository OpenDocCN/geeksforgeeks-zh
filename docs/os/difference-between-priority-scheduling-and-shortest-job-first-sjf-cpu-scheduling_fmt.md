...
> 原文: [https://www.geeksforgeeks.org/difference-between-priority-scheduling-and-shortest-job-first-sjf-cpu-scheduling/](https://www.geeksforgeeks.org/difference-between-priority-scheduling-and-shortest-job-first-sjf-cpu-scheduling/)
...
## 2. 最短作业优先(SJF)
最短作业优先(SJF)调度算法基于进程的突发时间。进程根据其突发时间被放入就绪队列。该算法首先处理突发时间最少的进程。仅比较那些在该时间之前存在或已经到达的进程的突发时间。它本质上也是非先发制人的。其抢先版称为最短剩余时间优先(SRTF)算法。
