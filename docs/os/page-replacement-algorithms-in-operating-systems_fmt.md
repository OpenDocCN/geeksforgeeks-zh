...
> 原文: [https://www.geeksforgeeks.org/page-replacement-algorithms-in-operating-systems/](https://www.geeksforgeeks.org/page-replacement-algorithms-in-operating-systems/)

在使用分页进行内存管理的操作系统中，需要一个页面替换算法来决定新页面进入时需要替换哪个页面。

## 页面错误

当正在运行的程序访问映射到虚拟地址空间但未加载到物理内存中的内存页面时，就会发生页面错误。
