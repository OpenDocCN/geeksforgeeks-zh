...
> 原文：[https://www.geeksforgeeks.org/mutex-lock-for-linux-thread-synchronization/](https://www.geeksforgeeks.org/mutex-lock-for-linux-thread-synchronization/)
...
**问题：**从最后两个日志中，可以看到日志“Job 2 has finished”重复了两次，而没有看到“Job 1 has finished”的日志。

## 为什么会发生？

通过仔细观察和可视化代码的执行，我们可以看到：

*   日志“Job 2 has started”是在“Job 1 has started”之后打印的，因此很容易得出结论，当线程 1 正在处理时，调度程序调度了线程 2。
*   如果上述假设成立，那么在作业 1 完成之前，`counter` 变量的值再次递增。
*   因此，当作业 1 实际完成时，计数器的错误值会产生日志“Job 2 has finished”，随后是实际作业 2 的“Job 2 has finished”，反之亦然，因为它取决于调度程序。
*   所以我们看到问题不在于重复的日志，而在于 `counter` 变量的错误值。
*   实际问题是当第一个线程正在使用或将要使用变量 `counter` 时，第二个线程使用了它。
*   换句话说，我们可以说线程之间在使用共享资源 `counter` 时缺乏同步导致了问题，或者用一个词来说，这个问题是由于两个线程之间的“同步问题”发生的。
...
1.  **`int pthread_mutex_init(pthread_mutex_t *restrict mutex, const pthread_mutexattr_t *restrict attr)`**：创建一个由 `mutex` 引用的互斥体，其属性由 `attr` 指定。如果 `attr` 为 `NULL`，则使用默认的互斥体属性（`NONRECURSIVE`）。

    **返回值**
    如果成功，`pthread_mutex_init()` 返回 0，`mutex` 的状态变为初始化并解锁。
    如果不成功，`pthread_mutex_init()` 返回 -1。

2.  **`int pthread_mutex_lock(pthread_mutex_t *mutex)`**：锁定一个标识互斥体的互斥体对象。如果互斥体已被另一个线程锁定，则该线程等待互斥体变为可用。锁定互斥体的线程成为其当前所有者，并保持所有者身份，直到同一线程将其解锁。当互斥体具有递归属性时，锁的使用可能不同。当这种互斥体被同一线程多次锁定时，计数会递增，并且没有等待线程被唤醒。拥有线程必须调用 `pthread_mutex_unlock()` 相同次数以将计数递减到零。
...
1.  **`int pthread_mutex_unlock(pthread_mutex_t *mutex)`**：释放一个互斥体对象。如果一个或多个线程正在等待锁定该互斥体，`pthread_mutex_unlock()` 会导致其中一个线程从 `pthread_mutex_lock()` 返回，并获取该互斥体对象。如果没有线程在等待该互斥体，则该互斥体在没有当前所有者的情况下解锁。当互斥体具有递归属性时，锁的使用可能不同。当这种互斥体被同一线程多次锁定时，解锁将递减计数，并且没有等待线程被唤醒以继续运行。如果计数递减到零，则互斥体被释放，如果有任何线程在等待它，则会被唤醒。

    **返回值**
    如果成功，`pthread_mutex_unlock()` 返回 0。
    如果不成功，`pthread_mutex_unlock()` 返回 -1。

2.  **`int pthread_mutex_destroy(pthread_mutex_t *mutex)`**：删除一个标识互斥体的互斥体对象。互斥体用于保护共享资源。`mutex` 被设置为无效值，但可以使用 `pthread_mutex_init()` 重新初始化。
...
*   互斥体在主函数的开始被初始化。
*   使用共享资源 `counter` 时，在 `trythis()` 函数中锁定了相同的互斥体。
