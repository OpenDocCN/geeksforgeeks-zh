...
> 原文: [https://www.geeksforgeeks.org/writing-cron-expressions-for-scheduling-tasks/](https://www.geeksforgeeks.org/writing-cron-expressions-for-scheduling-tasks/)
...
| `-` | 范围。表示连续的值范围。 | 在`<hour>`字段中使用“5-8”表示小时 5、6、7 和 8 |
| `,` | 多个值。分隔不同值的列表 | 在`<hour>`字段中使用“5，6，10”表示小时 5，6 和 10 |
...
1.  **每天上午 09:23:15 运行一次–**
    由于计划每天、每月和每年运行一次，因此这些字段将用`*`标记。对于星期几或月几，我们必须使用“?”，一个是“*”，另一个是“?”(两者都不能同时为“?”或“*”)。此外，小时字段将是 9，分钟字段将是 23，秒的字段将是 0。因此表达式是 `0 23 9 * * ? *`。
...
3.  **从每天下午 04:00 开始到下午 04:55 结束每 5 分钟运行一次–**
    要每 5 分钟运行一次计划，分钟字段中使用的增量为 5，如下所示 `0/5`。对于小时字段，由于计划将仅在下午 04:00 的小时内运行，因此要使用的值仅为 16。因此，表达式是: `0 0/5 16 * * ?`。

## 理解 Cron 表达式:
以下是 Cron 表达式的进一步例子及其含义:

| 表达式 | 意义 |
| :--- | :--- |
| `0 */5 * ? * *` | 每五分钟一次 |
| `0 20,30,45 * ? * *` | 每小时 20、30 和 45 分钟(因此每小时三次) |
| `0 30,45 14 ? 1-5 MON` | 在 1 月至 5 月(1-5 日)的每个星期一下午 2:30 和 2:45 |
| `0 0 9 ? * MON-FRI` | 从周一到周五每天早上 09:00 |
| `15 30 * ? * *` | 每小时 30 分钟的第 15 秒。例如 10:30:15，11:30:15，… |
| `25 30 10 * * ? 2021` | 2021 年每天上午 10:30:25 |
| `0 20 8 ? * TUE 2020-2022` | 2020 年、2021 年和 2022 年每个月的最后一个星期二上午 08:20 |
...
*   在同一个 cron 表达式中，不能用相同的值同时指定月日和周日字段。如果两个值中的一个用 `*` 来表示，那么另一个一定要用 `?` 来表示。
*   将计划时间设置在上午 12:00 到 1:00 之间时，与夏令时相关的更改可能会导致跳过或重复计划的激发。
*   如果需要，表达式可能已经分成 2 个。例如，要表示除了凌晨 03:00 和 06:00 之间每分钟触发一次的计划，需要以下两个表达式: `0 * 0-2 * * ?` 每分钟运行一次时间表，在上午 12:00 到 02:59 之间，另一个表达式 `0 * 6-23 * * ?` 每分钟运行一次，时间在早上 06:00 到晚上 11:59 之间。