...
> 原文：[https://www.geeksforgeeks.org/setuid-setgid-and-sticky-bits-in-linux-file-permissions/](https://www.geeksforgeeks.org/setuid-setgid-and-sticky-bits-in-linux-file-permissions/)
...
此外，文件的所有权还取决于创建者的`uid`（用户标识）和`gid`（组标识），如本文[所述](https://www.geeksforgeeks.org/real-effective-and-saved-userid-in-linux/)。同样，当我们启动一个流程时，它会与启动它的用户的`uid`和`gid`一起运行。
