...
> 原文:[https://www . geesforgeks . org/ISR 和函数调用之间的区别/](https://www.geeksforgeks.org/difference-between-isr-and-function-call/)
...
| 2. | *   `ISR` 地址被写入中断向量表。
*   **例如，** –
    在 `8086` 的情况下，内存的前 `1KB`，地址从 `00000 H` … `003FF H`，被保留给 `IVT`。
*   每个中断的 `ISR` 地址是固定的。 | 子程序的地址写在主程序代码中的指令中。 |
| 3. | `ISR` 的地址由硬件决定。 | 子程序的地址写在主程序内部。 |
| 4. | `ISR` 用于所有通用任务。
**例如–**
