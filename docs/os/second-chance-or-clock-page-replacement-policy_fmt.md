...
> 原文: [https://www.geeksforgeeks.org/second-chance-or-clock-page-replacement-policy/](https://www.geeksforgeeks.org/second-chance-or-clock-page-replacement-policy/)
...
*   **Pass-11:** `Frame={2, 4, 0}`，`second_chance= {0, 0, 0}`，`指针=0`，`pf=6` (2 和 4 获得第二次机会)
...
*   **Pass-14:** `Frame={1, 4, 0}`，`second_chance= {0, 1, 0}`，`指针=1`，`pf=7`(无变化)

*   **Pass-15:** `Frame={1, 4, 2}`，`second_chance= {0, 0, 0}`，`指针=0`，`pf=8` (4 由于第二次机会再次存活！)

*   **Pass-16:** `Frame={1, 4, 2}`，`second_chance= {0, 1, 0}`，`指针=0`，`pf=8`(第二次机会更新)

*   **Pass-17:** `Frame={3, 4, 2}`，`second_chance= {0, 1, 0}`，`指针=1`，`pf=9`(指针，pf 更新)

*   **Pass-18:** `Frame={3, 4, 2}`，`second_chance= {0, 1, 0}`，`指针=1`，`pf=9`(无变化)
...
创建一个数组 `frames` 以跟踪当前在内存中的页面，并创建另一个布尔数组 `second_chance` 以跟踪自上次替换以来该页面是否被访问过(即它是否值得第二次机会)以及一个变量 `pointer` 以跟踪替换目标。

1.  开始遍历数组 `arr` 。如果页面已经存在，只需将其在 `second_chance` 中对应的元素设置为 `true` 并返回。

2.  如果页面不存在，检查 `pointer` 指向的空间是否为空(表示缓存尚未满)–如果是，我们将把元素放在那里并返回，否则我们将逐个遍历数组 `arr` (循环使用 `pointer` 的值)，将所有对应的 `second_chance` 元素标记为 `false`，直到找到一个已经为 `false` 的元素。那是最适合替换的页面，所以我们这样做并返回。
