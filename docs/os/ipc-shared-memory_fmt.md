...
*   `ftok()`：用于生成唯一密钥。
*   `shmget()`：`int shmget(key_t key, size_t size, int shmflg)`；成功完成后，`shmget()`返回共享内存段的标识符。
*   `shmat()`：在使用共享内存段之前，您必须使用 `shmat()`将自己连接到它。`void *shmat(int shmid, void *shmaddr, int shmflg)`；`shmid`为共享内存 id。`shmaddr`指定了要使用的具体地址，但是我们应该将其设置为零，操作系统会自动选择地址。
*   `shmdt()`：当你使用完共享内存段后，你的程序应该使用 `shmdt()`从其中分离出来。`int shmdt(void *shmaddr)`；
*   `shmctl()`：脱离共享内存时，不会被破坏。所以，要消灭就要用到 `shmctl()`。`shmctl(shmid, IPC_RMID, NULL)`；
