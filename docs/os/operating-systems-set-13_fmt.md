...
回答 (C)
一旦分配完所有资源(分别是 `X`、`Y` 和 `Z` 的 5、4 和 3 个实例)，就剩下 `X`、`Y` 和 `Z` 的 0、1 和 2 个实例。只有 `P1` 的需求才能得到满足。所以 `P1` 可以先完成执行。一旦 `P1` 完成，它会分别释放 2、1 和 3 个单位的 `X`、`Y` 和 `Z`。在 `P0` 和 `P2` 之间，只能满足 `P0` 的需求。所以 `P0` 完成它的执行。最后，`P2` 完成了执行。
...
答案 (D)
上述同步结构不能防止死锁。当 `wants1` 和 `wants2` 都为 `true` 时，`P1` 和 `P2` 就永远停留在他们的循环中，等待对方完成。
