...
> 原文：[https://www.geeksforgeeks.org/multistep-processing-of-a-user-program/](https://www.geeksforgeeks.org/multistep-processing-of-a-user-program/)
...
源代码中的地址通常是符号化的（比如变量`count`）。编译器将这些地址绑定到可重定位的地址，这些地址由[链接器或加载器](https://www.geeksforgeeks.org/difference-between-loading-and-linking/)绑定到绝对地址（绑定——从一个地址空间到另一个地址空间的映射）。
...
2.  **[加载时间](https://www.geeksforgeeks.org/difference-between-compile-time-and-load-time-address-binding/)：**
    如果进程所在的内存地址在编译时未知，那么编译器必须生成可重定位的代码（没有运行时的静态内存地址）。如果起始地址改变，那么程序必须重新加载以合并该值。
...
![Multistep Processing of a User Program](img/29faddedf1cb78b30d63ebd561c36ee9.png)

<center>**Figure –** 用户程序的多步处理</center>

## 逻辑地址 vs 物理地址

[逻辑地址](https://www.geeksforgeeks.org/logical-and-physical-address-in-operating-system/)是中央处理器生成的地址，而物理地址是存储单元看到的地址（即加载到存储器的存储器地址寄存器中的地址）。用户程序处理逻辑地址，但永远看不到物理地址。
