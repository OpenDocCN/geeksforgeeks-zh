...
II。处于阻塞状态的进程 P2 可以进行转换 E，而另一个进程 P1 处于运行状态。
III。操作系统使用抢先调度。
IV。操作系统使用非抢先调度。
...
答案(C)
I 是假的。如果一个进程进行转换 D，将导致另一个进程进行转换 B，而不是 A。
II 为真。当输入/输出完成时，一个进程可以移动到就绪状态，而不管其他进程是否处于运行状态。
III 是真的，因为有一个从运行到就绪状态的过渡。
IV 为假，因为 OS 使用抢先调度。

## 2) 进程关键部分解决方案

实现过程关键部分的 `enter_CS()` 和 `leave_CS()` 功能使用测试和设置指令实现如下:
...
回答(A)
上面的解决方案是一个简单的`测试和设置`解决方案，确保不会发生死锁，但它不使用任何队列来避免饥饿或具有先进先出顺序。

## 3) 多级页表

与用于将虚拟地址转换为物理地址的单级页表相比，多级页表是优选的，因为
