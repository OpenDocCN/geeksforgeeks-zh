...
> 原文：[https://www.geeksforgeeks.org/difference-between-rpc-and-rmi/](https://www.geeksforgeeks.org/difference-between-rpc-and-rmi/)

## 介绍

`RPC` 代表**远程过程调用**，支持过程编程。在软件允许进程管理共享信息的情况下，它的机制几乎类似于 `IPC`。在完全不同的进程在不同的系统上执行，并且基本上需要基于消息的通信的情况下，与环境相关联。

![](img/7d0e82b532860722dd5384e045e30762.png)

上图显示了 `RPC` 的工作步骤。

`RMI` 代表**远程方法调用**，类似于 `RPC`，但它支持面向对象编程，这是 `Java` 的特性。允许线程决定外来对象的策略。在 `RMI` 中，对象是作为参数而不是普通数据传递的。
...
| 10. | `RPC` 中存在一个巨大的版本控制问题。 | 虽然有可能使用 `RMI` 版本控制。 |
