...
> 原文: [https://www.geeksforgeeks.org/what-happens-when-we-call-a-function/](https://www.geeksforgeeks.org/what-happens-when-we-call-a-function/)
...
*   `Program Stack`: `Program Stack`是保存所有函数调用的栈，底部元素是主函数。
*   `堆栈帧`: `堆栈帧`实际上是一个缓冲存储器，它是程序堆栈的一个元素，并且具有被调用函数的数据，包括：
    *   返回地址
    *   输入参数
    *   局部变量
    *   寄存器存储
*   `栈指针`: `栈指针`是指向程序栈顶的指针，即最近调用的函数。
...
1.  程序计数器指向下一个指令存储位置，即在执行一个函数之前的104，而100是调用函数的存储位置。
2.  为了恢复返回地址，程序计数器的内容被推入堆栈。现在存储在栈顶的地址是104。
3.  调用函数执行: 现在程序计数器指向2000，这是子程序的起始地址。在子程序中所有连续指令执行后，从堆栈中弹出地址。
4.  堆栈弹出是指移除堆栈顶部并分配给程序计数器。现在程序计数器持有返回地址，即104。
