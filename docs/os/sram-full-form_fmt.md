...
**静态随机存取存储器**代表**静态随机存取存储器**。它是半导体的一种形式。它广泛应用于微处理器、通用计算应用和电子设备。`SRAM`本质上是易失性的，这意味着一旦电源切断，存储在其中的数据就会全部被擦除。`SRAM`由触发器组成。它由 4-6 个晶体管组成，一旦触发器存储了该位，它就保持存储，直到相反的位存储在其中。
