...
> 原文: [https://www.geeksforgeeks.org/cache-memory-design/](https://www.geeksforgeeks.org/concept-of-cache-memory-design/)
...
为了与处理器速度匹配，缓存内存非常小，以便查找和获取数据所需的时间更少。它们通常根据架构分为多个层次。缓存的大小应能容纳块的大小，而块的大小又由处理器的架构决定。当块大小增加时，由于局部性原理，`hit ratio`最初会增加。

导致将更多数据带入高速缓存的块大小的进一步增加将降低命中率，因为在某个时间点之后，使用由新块带入的新数据的概率小于重新使用正被清空以给新块腾出空间的数据的概率。

### 2. Mapping function –
当从主存储器中读取一个数据块时，映射函数决定读入的主存储器块将占用缓存中的哪个位置。如果缓存已满，则需要用主存储器块替换缓存内存块，这会导致复杂性。应该替换哪个缓存块？
...
### 4. Write Policy –
内存缓存最重要的方面之一。从缓存中选择要被新读入的主存储器块替换的数据块应该首先被写回主存储器。这是为了防止数据丢失。需要决定何时将缓存内存块写回主存储器。
