...
操作系统中的进程从形成到完成从不同的状态传递。一个过程由程序数据及其相关数据和一个[过程控制块](https://www.geeksforgeeks.org/process-table-and-process-control-block-pcb/)(`PCB`)组成。进程可能会因为以下事件而改变其状态，如输入/输出请求、中断例程、[进程同步](https://www.geeksforgeeks.org/introduction-of-process-synchronization/)、[进程调度算法](https://www.geeksforgeeks.org/cpu-scheduling-in-operating-systems/)等。该过程可以运行，也可以不运行，如果正在运行，则必须由系统维护，以获得过程的适当进展。
