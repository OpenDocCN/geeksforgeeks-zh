...
> 原文: [https://www.geeksforgeeks.org/transforming-of-i-o-requests-to-hardware-operations/](https://www.geeksforgeeks.org/transforming-of-i-o-requests-to-hardware-operations/)
...
1.  **系统调用–**
    每当任何 I/O 请求到来时，进程都会对文件的先前打开的文件描述符发出`阻塞读取()`系统调用。基本上，系统调用代码的作用是检查内核中参数的正确性。如果我们以输入形式输入的数据已经在缓冲区缓存中可用，数据将被返回处理，在这种情况下，输入/输出请求就完成了。
2.  **输入不可用时的替代方法–**
...
6.  **中断处理程序的角色–**
    中断通过`中断向量表`发送到正确的中断处理程序。它存储任何必要数据，向设备驱动发出信号，并从中断返回。
7.  **完成输入/输出请求–**
