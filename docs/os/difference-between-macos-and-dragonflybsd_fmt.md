# macOS 和 DragonFly BSD 的区别

> 原文：[https://www.geeksforgeeks.org/difference-between-macos-and-dragonflybsd/](https://www.geeksforgeeks.org/difference-between-macos-and-dragonflybsd/)
...
## 2. DragonFlyBSD
`DragonFlyBSD` 是由马修·狄龙开发的免费开源操作系统。这是一个类似 `Unix` 的操作系统，其设计灵感来自 `FreeBSD` 操作系统。基本不用于个人电脑。它是专门为服务器、工作站、网络连接存储和嵌入式系统设计的。主要使用的[编程语言](https://www.geeksforgeeks.org/introduction-to-programming-languages/)是 [`C` 语言](https://www.geeksforgeeks.org/c-language-set-1-introduction/)。`DragonFly BSD` 的第一个版本于 2003 年推出。它有混合内核。

## macOS 和 DragonFly BSD 的区别

| 编号 | macOS | DragonFly BSD |
| :--- | :--- | :--- |
| 1. | 由`苹果公司`开发。 | 是`马修·狄龙`开发的。 |
...
| 11. | `macOS` 支持的文件系统有 `HFS+`、`APFS`、`HFS`、`UFS`、`AFP`、`ISO 9660`、`FAT`、`UDF`、`NFS`、`SMBFS`、`NTFS`、`FTP`、`WebDAV` 和 `ZFS`。 | `DragonFlyBSD` 支持的文件系统有 `UFS1`、`MFS`、`ext2`、`FAT`、`HAMMER` 和 `ISO 9660`。 |