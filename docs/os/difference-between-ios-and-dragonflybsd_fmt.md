# iOS 和 DragonFlyBSD 的区别

> 原文：[https://www.geeksforgeeks.org/difference-between-ios-and-dragonflybsd/](https://www.geeksforgeeks.org/difference-between-ios-and-dragonflybsd/)
...
## 2. DragonFlyBSD
`DragonFlyBSD` 是由马修·狄龙开发的免费开源操作系统。这是一个类似 `Unix` 的操作系统，其设计灵感来自 `FreeBSD` 操作系统。基本不用于个人电脑。它是专门为服务器、工作站、网络连接存储和嵌入式系统设计的。主要使用的[编程语言](https://www.geeksforgeeks.org/introduction-to-programming-languages/)是 [`C` 语言](https://www.geeksforgeeks.org/c-language-set-1-introduction/)。`DragonFlyBSD` 的第一个版本于 2003 年推出。它有混合内核。

## iOS 和 DragonFlyBSD 的区别

| 编号 | iOS | DragonFlyBSD |
| :--- | :--- | :--- |
| 1. | 它由 `Apple` 公司开发并拥有。 | 是 `Matthew Dillon` 开发的。 |
...
| 6. | 它的原生 `API` 是 `Cocoa` 和 `BSD-POSIX`。 | 它的原生 `API` 是 `BSD/POSIX`。 |
| 7. | 它的首选许可证是专有，`APSL` 和通用处理器图形处理器。 | 它有 `BSD` 的首选许可证。 |
