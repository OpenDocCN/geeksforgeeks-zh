...
> 原文：[https://www.geeksforgeeks.org/how-to-extract-and-disassemble-a-linux-kernel/](https://www.geeksforgeeks.org/how-to-extract-and-disassemble-a-linux-kernel/)
...
上面的命令将为我们的内核副本运行 `extract-vmlinux` 脚本，并将其输出到名为 `decomp-vmlinuz` 的文件中，该文件将位于我们当前的工作目录下。
...
分解后的内核代码现在可以在 `disassembled-vmlinuz.asm` 文件中找到。
...
现在，我们可以很容易地在 `System.map-x.x-xx-generic` 文件中对我们的符号名称进行 grep，以获得起始地址，然后我们可以在 `disassembled-vmlinuz.asm` 文件中查找该地址。
例如，我们可以查找 `register_keyboard_notifier`：
