...
*   一个人必须处理一个 [`passwd`文件](https://www.geeksforgeeks.org/passwd-command-in-linux-with-examples/)。
*   还必须处理一个`shadow`文件。
...
环境变量的目的是影响程序或软件的行为方式。例如，`$LANG` 变量将与用户理解特定语言的软件进行通信，因此它必须在接口级别提供相同的语言。这里我们将使用一个 `PATH` 变量来模拟系统查找 `passwd` 和影子文件的位置。我们将把它命名为 `PFILE`。
...
这里，定义一个[结构](https://www.geeksforgeeks.org/structures-c/)，它将保存所有必须更新的数据。任何其他方法都会引入额外的代码行，并使跟踪代码变得困难。此外，将您创建的`结构`重新命名为`Struct`也是一个很好的建议。当使用 `struct` 作为参数和返回类型时，不重命名将在编译期间导致大量引用和标识符错误。
