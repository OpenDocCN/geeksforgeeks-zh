...
> 原文：[https://www.geeksforgeeks.org/first-come-first-serve-cpu-scheduling-non-preemptive/](https://www.geeksforgeeks.org/first-come-first-serve-cpu-scheduling-non-preemptive/)

## 先决条件
[操作系统中的 CPU 调度](https://www.geeksforgeeks.org/cpu-scheduling-in-operating-systems/)

给定 **N** 个进程，它们的到达时间为 `at[]`，突发时间为 `bt[]`。任务是使用[先来先服务（FCFS）](https://www.geeksforgeeks.org/program-for-fcfs-cpu-scheduling-set-1/)算法找到每个进程的等待时间和平均等待时间。
...
2.  后续进程的等待时间可以通过下式计算：
    ...
    ```
    其中 `wt[i]` 是当前进程的等待时间，`at[i-1]` 是前一进程的到达时间，`bt[i-1]` 是前一进程的突发时间，`wt[i-1]` 是前一进程的等待时间，`at[i]` 是当前进程的到达时间。
3.  平均等待时间可以通过下式计算：
    ```
