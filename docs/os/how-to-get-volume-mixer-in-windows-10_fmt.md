...
> 原文：[https://www.geeksforgeeks.org/how-to-get-volume-mixer-in-windows-10/](https://www.geeksforgeeks.org/how-to-get-volume-mixer-in-windows-10/)
...
*   打开运行窗口。在那里输入`regedit`并按回车键打开注册表编辑器。
...
右键点击`CurrentVersion`，然后转到`新建 -> 键`并命名为 `MTCUVC`。
...
右键点击 `MTCUVC` ，然后点击`新建 -> DWORD(32 位)值`。将其命名为 `EnableMtcUvc`。
...
现在只需关闭注册表编辑器窗口，点击任务栏上的扬声器图标，就可以获得旧的音量控制。
...
另一种方法是使用微软商店上的开源 EarTrumpet 应用程序，它为相同的任务提供了更好的用户界面。
