...
> 原文: [`https://www.geeksforgeeks.org/lock-variable-synchronization-mechanism/`](https://www.geeksforgeeks.org/lock-variable-synchronization-mechanism/)

**先决条件–** [`进程同步`](https://www.geeksforgeeks.org/process-synchronization-set-1/)

锁变量为进程提供了最简单的同步机制。关于锁定变量，值得注意的几点是-

1.  它是一个在用户模式下实现的软件机制，即不需要操作系统的支持。
2.  这是一个繁忙的等待解决方案(即使在技术上处于等待状态时，也能保持 CPU 繁忙)。
3.  它可以用于两个以上的过程。

当`锁定= 0`时表示临界区是空的(初始值)，`锁定= 1`时表示临界区被占用。
伪代码看起来像这样–
