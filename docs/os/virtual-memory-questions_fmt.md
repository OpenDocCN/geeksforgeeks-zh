...
> 原文：`https://www.geeksforgeeks.org/virtual-memory-questions/`
...
为每个进程设置`页面错误率`的下限和上限。使用上述步骤，建立“可接受的”页面错误率。
...
**答案：(b)**
**解释：** 当系统上的进程需要更多内存时，系统会发生系统颠簸。如果进程没有“足够”的页面，`页面错误率`会非常高。这就导致：
```
– `CPU` 利用率低
– 操作系统大部分时间都花在`磁盘交换`上
```
