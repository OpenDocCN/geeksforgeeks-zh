...
> 原文：[https://www.geeksforgeeks.org/process-schedulers-in-operating-system/](https://www.geeksforgeeks.org/process-schedulers-in-operating-system/)
...
1.  **长期或作业调度程序：**
    它将新进程带入“就绪状态”。它控制 `多编程` 的程度，即在任何时间点处于就绪状态的进程数。重要的是，长期调度程序要仔细选择输入输出和中央处理器绑定的进程。`输入输出受限任务`是在输入输出操作中使用大量时间的任务，而`中央处理器受限进程`是在中央处理器上花费时间的任务。作业调度器通过保持两者之间的平衡来提高效率。
