...
> 原文：[https://www.geeksforgeeks.org/resource-allocation-graph-rag-in-operating-system/](https://www.geeksforgeeks.org/resource-allocation-graph-rag-in-operating-system/)
...
1.  **进程顶点**：每个进程将被表示为一个进程顶点。一般来说，这个进程会用一个圆圈来表示。
...
现在来到`RAG`的边。`RAG`中有两种类型的边：

1.  **分配边**：如果已经为进程分配了资源，则称之为分配边。
2.  **请求边**：这意味着在未来，进程可能需要一些资源来完成执行，这称为请求边。
...
如果资源分配图中有一个循环，并且循环中的每个资源只提供一个实例，那么进程将处于死锁状态。例如，如果进程`P1`持有资源`R1`，进程`P2`持有资源`R2`，进程`P1`等待`R2`，进程`P2`等待`R1`，那么进程`P1`和进程`P2`将处于死锁状态。
...
*   进程总数是三个：`P1`、`P2`和`P3`；资源总数是两个：`R1`和`R2`。

**分配矩阵**

*   要构建分配矩阵，只需转到资源，看看它被分配给哪个进程。
*   `R1`被分配给`P1`，因此在分配矩阵中写1；类似地，`R2`被分配给`P2`以及`P3`，对于剩余的元素写0。

**请求矩阵**

*   为了找出请求矩阵，您必须进入进程并查看输出边。
*   `P1`正在请求资源`R2`，所以在矩阵中写1；类似地，`P2`请求`R1`，对于剩余的元素写0。

所以现在可用的资源是 `(0, 0)`。

**检查死锁（安全与否）**

![](img/46bccb6725a3d13a153280a37bf90d73.png)
...
因此，可用资源是 `(0, 0)`，但需求是 `(0, 1)`、`(1, 0)`和`(1, 0)`。所以你不能满足任何一个需求。因此，它陷入了僵局。
