...
在本文中，我们将创建一个简单的 [Writer-Reader](https://www.geeksforgeeks.org/readers-writers-problem-set-1-introduction-and-readers-preference-solution/) 应用程序，它使用两个线程，一个用于写入文件，另一个用于读取文件。这里我们将讨论在 C/C++ 中使用 Win32 线程的方法。可以使用[`CreateThread()`](https://docs.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-createthread)方法创建窗口线程。
...
3.  使用[`CreateThread`](https://docs.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-createthread)功能创建两个线程，用于从文件中写入和读取数据
