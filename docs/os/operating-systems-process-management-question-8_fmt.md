...
## 原文链接
[https://www.geeksforgeeks.org/operating-systems-process-management-question-8/](https://www.geeksforgeeks.org/operating-systems-process-management-question-8/)
...
## 选项分析
选项 C 也可能导致死锁。想象一个情况，进程 `X` 获得了 `a`，进程 `Y` 获得了 `b`，进程 `Z` 获得了 `c` 和 `d`，现在有循环等待。

选项 C 也可能导致死锁。想象一个情况，进程 `X` 获得了 `b`，进程 `Y` 获得了 `c`，进程 `Z` 获得了 `a`，现在有循环等待。

选项 D 也可能导致死锁。想象一个情况，进程 `X` 获得了 `a` 和 `b`，进程 `Y` 获得了 `c`，`X` 和 `Y` 循环等待对方。

参考：[http://www.eee.metu.edu.tr/~halici/courses/442/Ch5%20Deadlocks.pdf](http://www.eee.metu.edu.tr/~halici/courses/442/Ch5%20Deadlocks.pdf)
