...
> 原文: [https://www.geeksforgeeks.org/difference-between-cli-and-gui/](https://www.geeksforgeeks.org/difference-between-cli-and-gui/)

`CLI` 是用于**命令行界面**的单词形式。命令行界面允许用户在终端或控制台窗口中大量输入与操作系统交互的命令。命令行界面是一个平台或媒介，用户可以通过编写命令来响应可见的提示，并从系统获得响应，因此用户必须被迫输入命令或命令序列来执行任务。命令行界面适用于价格昂贵的计算，只要输入正确是优先考虑的。
