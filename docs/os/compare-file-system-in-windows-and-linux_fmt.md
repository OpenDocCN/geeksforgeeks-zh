...
> 原文：[https://www.geeksforgeeks.org/compare-file-system-in-windows-and-linux/](https://www.geeksforgeeks.org/compare-file-system-in-windows-and-linux/)
...
Windows 2000 (`W2K`) 支持许多文件系统，包括在 Windows 95、`MS-DOS` 和 `OS/2` 上运行的文件分配表（`FAT`）。但是 `W2K` 的开发者也设计了一个新的文件系统，`W2K` 文件系统（`NTFS`），旨在满足工作站和服务器的高端需求。
...
`NTFS` 是一个灵活而强大的文件系统，建立在简单的文件系统模型之上。`NTFS` 最显著的特点包括：
