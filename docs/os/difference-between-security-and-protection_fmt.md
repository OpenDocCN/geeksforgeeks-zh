...
> 原文：[https://www.geeksforgeeks.org/difference-between-security-and-protection/](https://www.geeksforgeeks.org/difference-between-security-and-protection/)

## 安全性：
安全系统涵盖了其系统资源（保存的数据、内存、磁盘等）的安全性，包括恶性变更、非法访问以及差异或不一致性。安全性提供了一种机制（认证和加密）来分析用户是否允许使用系统。
...
保护的例子可以从安全方面给出，任何组织都会有许多部门，下面有几个工作人员操作。不同的部门会互相分享频繁的信息，但不是敏感的信息。因此，不同的员工对信息拥有不同的访问权限，因此他们将访问定义的数据。
