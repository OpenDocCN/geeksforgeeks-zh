...
> 原文: [https://www.geeksforgeeks.org/dining-philosophers-solution-using-monitors/](https://www.geeksforgeeks.org/dining-philosophers-solution-using-monitors/)
...
### 吃饭
当哲学家拿到了两把叉子，也就是说，他已经进入了这个部分。

哲学家我可以设置变量`state[i]` = 只有当她的两个邻居不吃饭时才吃 (`state[(i+4) % 5] != EATING`) 和 (`state[(i+1) % 5] != EATING`)。
...
这让哲学家 I 可以在她饿了却拿不到她需要的筷子时拖延自己。我们现在可以描述一下我们对哲学家就餐问题的解决方案。筷子的分配由班长“用餐哲学家”控制。每个哲学家，在开始吃饭之前，都必须调用`Pickup()`操作。这一行为可能导致哲学家进程的中止。手术成功完成后，哲学家可以吃饭了。接下来，哲学家调用`Putdown()`操作。因此，哲学家 I 必须按照以下顺序调用操作`pickup()`和`putdown()`:
