...
> 原文: [https://www.geeksforgeeks.org/beladys-anomaly-in-page-replacement-algorithms/](https://www.geeksforgeeks.org/beladys-anomaly-in-page-replacement-algorithms/)
...
一般来说，当增加进程虚拟内存的帧数时，页面错误越少，进程的执行速度就越快。有时会发生相反的情况，即当更多的帧被分配给一个进程时，会出现更多的页面错误。这个最意想不到的结果被称为贝拉蒂异常。

贝拉蒂异常是指对于给定的内存访问模式，增加页面帧数会导致页面错误数增加的现象。
...
另外两个常用的页面替换算法是 `Optimal` 和 `LRU`，但是对于任何引用字符串，贝拉蒂异常永远不会出现在这些算法中，因为它们属于一类基于堆栈的页面替换算法。

基于堆栈的算法可以显示出 *N* 帧的内存中的页面集合始终是具有 *N + 1* 帧的内存中的页面集合的子集。对于 `LRU` 替换，内存中的页面集将是最近引用的页面。如果帧数增加，那么这些 *n* 页仍将是最近引用的，因此仍将在内存中。而在 `FIFO` 中，如果名为 *b* 的页面在页面 *a* 之前进入物理内存，则 *b* 的替换优先级大于 *a* 的替换优先级，但这与页面帧数无关，因此 `FIFO` 不遵循堆栈页面替换策略，因此会出现贝拉蒂异常。

## 示例

考虑下图，了解基于堆栈的页面替换算法的行为。
...
**案例-1：** 如果系统有 3 帧，使用 `FIFO` 页面替换算法的给定参考字符串总共会产生 9 个页面错误。下图说明了示例中页面错误的模式。
...
从上面的例子可以看出，在使用 `FIFO` 页面替换算法的同时增加帧数时，页面错误的数量从 9 增加到 10。

**注意：** 并非每个字符串引用模式都会导致 `FIFO` 中的贝拉蒂异常，但在增加帧数时，某些类型的字符串引用会恶化 `FIFO` 性能。

## 为什么基于堆栈的算法不会出现异常

所有基于堆栈的算法都不会出现贝拉蒂异常，因为这些类型的算法会为页面分配一个独立于页面帧数的优先级（用于替换）。这种策略的例子有最优、`LRU` 和 `LFU`。此外，这些算法还具有很好的模拟特性，即可以通过参考串一次计算任意数量的页面帧的未命中（或命中）比率。
...
各种因素实质上影响页面错误的数量，例如引用字符串长度和可用的空闲页面框架数量。由于高速缓存大小较小以及高速缓存内容变化速度过快，也会出现异常。此外，即使在增加帧数之后，固定数量的页面错误的情况也可以被视为异常。像随机页面替换算法这样的算法通常也容易受到贝拉蒂异常的影响，因为它可能表现得像 `FIFO` 页面替换算法。但是基于堆栈的算法通常不受所有这些情况的影响，因为当帧增加时，它们保证给出更好的页面命中。
