...
4. 别名的第三种解决方案要求缓存更小，越小的缓存会导致更多的缓存未命中。解决方案 1、2 和 3 是基于硬件的。另一个被称为**页面颜色/缓存着色**的解决方案是由操作系统实现的软件解决方案，用于在不限制缓存大小的情况下解决混叠问题。在 64KB 高速缓存的情况下，需要 16 种颜色来确保不会发生混叠。此外，请阅读: `ARMv6` 上的页面着色