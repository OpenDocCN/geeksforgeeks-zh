...
> 原文：[https://www.geeksforgeeks.org/difference-between-aix-and-ibm-i/](https://www.geeksforgeeks.org/difference-between-aix-and-ibm-i/)
...
| 特性 | AIX | IBM i |
| :--- | :--- | :--- |
| 全称 | 高级交互执行程序（Advanced Interactive Executive） | IBM i |
| 开发者 | 它是由 IBM 开发并拥有的。 | 它是由 IBM 开发并拥有的。 |
| 推出时间 | 它于 1986 年推出。 | 它于 1988 年推出。 |
| 目标系统类型 | 它的目标系统类型是服务器、网络连接存储和工作站。 | 它的目标系统类型是小型机和服务器。 |
| 内核类型 | 内核类型与模块是一体的。 | 内核类型是微内核和虚拟机。 |
| 许可 | 首选许可是专有的。 | 首选许可是专有的。 |
| 用于个人电脑 | 它用于个人电脑。 | 它不用于个人电脑。 |
| 应用范围 | 它用于所有公司的计算机。 | 它主要用于 IBM 设备。 |
| 支持的文件系统 | 支持的文件系统有 `NTFS`、`FAT`、`ISO 9660`、`UDF`。 | 支持的文件系统有 `JFS`、`JFS2`、`ISO 9660`、`UDF`、`NFS`、`SMBFS` 和 `GPFS`。 |