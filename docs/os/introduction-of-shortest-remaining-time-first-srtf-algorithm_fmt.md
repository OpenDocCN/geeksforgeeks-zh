...
如上所述，对于同一组作业，SRT 算法的执行速度比 SJN 算法快。但是，这里忽略了开销费用，即上下文切换所需的时间。当一个作业被抢占时，它的所有处理信息必须保存在它的`印刷电路板`上，以便以后继续执行，操作系统切换到的另一个作业的`印刷电路板`上的内容被加载到内存中的寄存器中。这就是所谓的`上下文切换`。
