...
不同的用户访问任何文件的方法不同。一般的保护方式是将`身份相关访问`与所有文件和目录相关联，并创建一个名为`访问控制列表(ACL)`的列表，该列表指定了用户的名称以及与每个用户相关联的访问类型。访问列表的主要问题是它们的长度。如果我们想让每个人都能读取一个文件，我们必须列出所有具有读取权限的用户。这种技术有两个不良后果:
