...
> 原文: [https://www.geeksforgeeks.org/file-systems-in-operating-system/](https://www.geeksforgeeks.org/file-systems-in-operating-system/)

文件是记录在辅助存储器上的相关信息的集合。或者，文件是逻辑相关实体的集合。从用户的角度来看，文件是逻辑辅助存储的最小分配。
...
| 源代码 | `c`，`java`，`cpp`，`asm`，`a` | 各种语言的源代码 |
| 一批 | `bat`，`sh` | 给命令解释器的命令 |
| 文本 | `txt`，`doc` | 文本数据、文件 |
| 文字处理软件 | `wp`、`tex`、`rtf`、`doc` | 各种文字处理器格式 |
| 档案馆 | `arc`，`zip`，`tar` | 组合成一个压缩文件的相关文件 |
