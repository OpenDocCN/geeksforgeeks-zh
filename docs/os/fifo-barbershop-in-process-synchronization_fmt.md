...
> 原文: [https://www.geeksforgeeks.org/fifo-barbershop-in-process-synchronization/](https://www.geeksforgeeks.org/fifo-barbershop-in-process-synchronization/)
...
**解决方案–**
为了维护客户的秩序，我们应该实现一个数据结构，称为队列，它遵循先进先出的原则。将使用信号量列表，而不是为客户使用单个信号量。从现在开始，我们将客户视为线索。因此对于线程，将使用一个信号量列表，命名为 `queue`。这种方法背后的直觉是:

1.  当每个线程进入理发店时，它会创建一个线程并将其放入 `queue`。
2.  每个线程都等待自己的信号量，而不是等待理发师空闲。
3.  每当理发师空闲或醒来时，他就从 `queue` 中取出线，并发出信号让它走向椅子。

**注意–**
理发师必须获得 `mutex` 才能访问 `queue`。因此，`mutex` 将使用一些信号量来控制线程对 `queue` 的访问，和理发师处理 `queue`。
