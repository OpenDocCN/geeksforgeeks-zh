...
**FCFS 和 RR（CPU 量子为 2 个时间单位的循环调度）策略下 3 个进程的完成顺序为**
...
## 3. 考虑虚拟页面引用字符串
`1, 2, 3, 2, 4, 1, 3, 2, 4, 1`
在运行于计算机系统上的按需分页虚拟内存系统上，该系统的主内存大小为 3 页帧，最初为空。让 `LRU`、`FIFO` 和 `Optimal` 表示相应页面替换策略下的页面错误数量。那么
(A) `Optimal` < `LRU` < `FIFO`
(B) `Optimal` < `FIFO` < `LRU`
(C) `Optimal` = `LRU`
(D) `Optimal` = `FIFO`

答案：(B) `Optimal` 将是 5，`FIFO` 6 和 `LRU` 9。
