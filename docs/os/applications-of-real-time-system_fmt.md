...
> 原文: `https://www.geeksforgeeks.org/applications-of-real-time-system/`

**实时系统**是一个进行实时处理的系统，这意味着在指定的时间限制内获得响应或系统满足指定的截止日期。实时系统有两种类型——硬件和软件。两者都用于不同的情况。硬实时系统被用于甚至不允许延迟几纳米或几微米的地方。软实时系统在时间表达上提供了一些放松。
