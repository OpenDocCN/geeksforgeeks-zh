...
> 原文：[https://www.geeksforgeeks.org/difference-between-macos-and-ghostbsd/](https://www.geeksforgeeks.org/difference-between-macos-and-ghostbsd/)
...
## 2. GhostBSD
`GhostBSD` 是由 `Eric Turgeon` 和 `GhostBSD` 团队开发的一个自由开源的操作系统。这是一个类似 `Unix` 的操作系统，其设计灵感来自 `TrueOS` 操作系统。它有一个名为 `MATE` 的默认桌面环境。它专门用于台式机和工作站。早些时候，它使用了一个名为 `GNOME` 的默认桌面环境。`GhostBSD` 的第一个版本于 2009 年推出。它具有带模块的单片内核。
...
| 12. | `macOS` 支持的文件系统有 `HFS+`、`APFS`、`HFS`、`UFS`、`AFP`、`ISO 9660`、`FAT`、`UDF`、`NFS`、`SMBFS`、`NTFS`、`FTP`、`WebDAV` 和 `ZFS`。 | `GhostBSD` 支持的文件系统有 `UFS2`、`ext2`、`ext3`、`FAT`、`ISO 9660`、`UDF`、`NFS`、`ReiserFS`、`XFS` 和 `ZFS`。 |