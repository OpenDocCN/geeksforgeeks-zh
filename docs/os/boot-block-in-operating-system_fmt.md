...
在 `Windows 2000` 中启动是从运行系统只读存储器中的代码开始的。该代码指示系统直接从 `MBR` 读取代码。除此之外，引导代码还包含列出硬盘分区的表，以及指示从系统引导哪个分区的标志。一旦系统识别了引导分区，它就从存储器中读取第一个扇区，该扇区被称为`引导扇区`，并继续引导过程的剩余部分，包括加载各种系统服务。
