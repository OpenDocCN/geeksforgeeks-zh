...
> 原文: [https://www.geeksforgeeks.org/c-scan-disk-scheduling-algorithm/](https://www.geeksforgeeks.org/c-scan-disk-scheduling-algorithm/)
...
**注:**
`distance` 变量用于存储磁头与当前磁道位置之间的绝对距离。`disk_size` 是磁盘的大小。向量 `left` 和 `right` 分别在初始头部位置的左侧和右侧存储所有请求轨迹。
