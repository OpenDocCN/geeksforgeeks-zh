...
> 原文：[https://www.geeksforgeeks.org/introduction-of-secondary-memory/](https://www.geeksforgeeks.org/introduction-of-secondary-memory/)
...
*   **`Hard disk`：** 一个硬盘由一个或多个称为盘片的圆形磁盘组成，这些盘片安装在一个公共主轴上。每个盘片的表面都涂有磁性材料。每个磁盘的两个表面都能够存储数据，但最上面和最下面的磁盘除外，它们只使用内表面。信息通过磁性读/写头记录在旋转磁盘的表面上。这些头连接到一个称为访问臂的公共臂上。
