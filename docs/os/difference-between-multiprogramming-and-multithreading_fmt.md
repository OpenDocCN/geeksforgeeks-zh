...
> 原文：`https://www.geeksforgeeks.org/difference-between-multiprogramming-and-multithreading/`

## 多道程序设计
多道程序设计是一次运行多个进程，它通过组织作业(代码和数据)来提高 CPU 利用率，使 CPU 始终有一个要执行。动机是在主内存中保留多个作业。如果一个作业被输入/输出占用，可以将 CPU 分配给其他作业。
