...
> 原文：[https://www.geeksforgeeks.org/resource-reservation-protocol-in-real-time-systems/](https://www.geeksforgeeks.org/resource-reservation-protocol-in-real-time-systems/)
...
`Flowspec` 在 `RSVP` 中被用来确定不同的参数，如带宽、链路强度、拥塞等，以实现平滑和无碰撞的通信和数据传输。

## Filterspcec 在 RSVP 中用于过滤数据包

作为固定或共享接收器，`Filterspcec` 用于根据数据包的目的地类型路由数据包。
