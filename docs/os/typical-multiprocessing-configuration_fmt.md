...
> 原文：[https://www.geeksforgeeks.org/typical-multiprocessing-configuration/](https://www.geeksforgeeks.org/typical-multiprocessing-configuration/)
...
主/从配置是一个单处理器系统，其中有额外的从处理器在工作，由主处理器管理。这是一个不对称的系统。
...
这种配置的优势是不容易出现灾难性故障。如果一个处理器出现故障，其他处理器可以独立继续工作。

这种配置的缺点是很难检测到处理器是否出现故障。
