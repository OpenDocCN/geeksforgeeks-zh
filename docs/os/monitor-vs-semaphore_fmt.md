...
[监视器](https://www.geeksforgeeks.org/monitors-in-process-synchronization/)是一种高级同步构造。它是一种抽象数据类型。监视器类型包含共享变量和对共享变量进行操作的一组过程。
...
[信号量](https://www.geeksforgeeks.org/semaphores-in-process-synchronization/)是低级对象。信号量是非负整数变量。信号量的值表示系统中可用的共享资源的数量。信号量的值只能通过两个函数修改，即 `wait()` 和 `signal()` 操作（初始化除外）。
