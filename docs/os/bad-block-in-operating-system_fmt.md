...
> 原文：[https://www.geeksforgeeks.org/bad-block-in-operating-system/](https://www.geeksforgeeks.org/bad-block-in-operating-system/)
...
1.  **物理或硬坏块**：来源于存储介质损坏。
2.  **软或逻辑坏块**：当操作系统无法从扇区读取数据时，出现软或逻辑坏块。

**示例**：软坏块包括特定存储块的循环冗余校验(`CRC`)或纠错码(`ECC`)与磁盘读取的数据不匹配。
