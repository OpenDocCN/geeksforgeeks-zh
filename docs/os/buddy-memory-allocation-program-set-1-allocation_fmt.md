...
> 原文: [https://www.geeksforgeeks.org/buddy-memory-allocation-program-set-1-allocation/](https://www.geeksforgeeks.org/buddy-memory-allocation-program-set-1-allocation/)
...
当分配请求到来时，我们寻找比它大的最小块。如果在空闲列表中找到这样的块，分配就完成了(比如，请求是 27 KB，跟踪 32 KB 块的空闲列表中至少有一个元素)，否则我们向上遍历空闲列表，直到找到一个足够大的块。然后我们继续将其分成两个块-一个用于添加到下一个空闲列表(较小的大小)，一个用于遍历到树，直到我们到达目标并将请求的内存块返回给用户。如果这样的分配是不可能的，我们只需返回 `null`。
