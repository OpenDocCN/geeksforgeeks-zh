...
> 原文: [https://www.geeksforgeeks.org/difference-between-a-process-stack-and-a-cpu-stack/](https://www.geeksforgeeks.org/difference-between-a-process-stack-and-a-cpu-stack/)
...
| 进程堆栈 | 中央处理器堆栈 |
| --- | --- |
| 每个进程都有自己的[进程控制块](https://www.geeksforgeeks.org/process-table-and-process-control-block-pcb/)(`PCB`)，以将这些信息保存在上下文开关上，从而允许调度算法在基本进程标识上运行。 | 每个进程没有自己的[`PCB`](https://www.geeksforgeeks.org/process-table-and-process-control-block-pcb/)，因此进程的状态也包含在寄存器中，以供驱逐时存储。 |
| 当进程获得中央处理器时，与该标识相关联的`PCB`将被恢复。 | 与进程堆栈不同，与该标识关联的`PCB`不会恢复。 |
