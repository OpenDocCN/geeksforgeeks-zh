...
> 原文：`https://www.geeksforgeeks.org/first-in-never-out-fino-scheduling-in-operating-system/`
...
*   在只写内存 (`WOM`) 中——只写内存 (`WOM`) 是一种虚构的计算机内存，与只读内存 (`ROM`) 正好相反。从技术上讲，`WOM` 是一种可以写入但永远无法读取的存储设备。
...
*   `FINO` 队列也用于计算机网络中的黑洞。在计算机科学和网络中，黑洞指的是计算机网络中的虚拟位置，传入和/或传出的流量在不通知源数据未到达预期接收者的情况下被静默丢弃或丢弃，所有这些想法和实现都基于 `FINO`。