...
> 原文：[https://www.geeksforgeeks.org/5-operating-system-concepts-you-should-know-as-a-developer/](https://www.geeksforgeeks.org/5-operating-system-concepts-you-should-know-as-a-developer/)
...
这个过程基本上被定义为一个正在执行的程序。这个过程应该按顺序执行。当你在一个文本文件中编写一个计算机程序，当你执行这个程序时，它就变成了你系统中的一个进程。这个过程执行程序中提到的所有任务。一个进程主要分为四个部分：`Stack`、`Heap`、`Text`和`Data`。

*   `Stack`：`Stack` 负责存储函数/方法参数等临时数据。它返回地址和局部变量。
*   `Heap`：`Heap`在进程运行时动态地将内存分配给进程。
*   `Text`：`Text`包括由程序计数器的值和存储在处理器寄存器中的数据表示的当前活动。
*   `Data`：包含全局和静态变量。
...
`User-Level Threads`：在这个级别的线程中，线程管理内核并不知道线程的存在。线程库还维护创建和销毁线程的代码。它还包含用于在线程之间传递消息和数据的代码。还维护了用于调度线程执行和恢复线程上下文的代码。

`Kernel-Level Threads`：线程级管理由内核完成。在应用程序区域，您不会找到线程管理代码。操作系统直接支持它。
...
*   `Job Queue`：它负责保存系统中的所有进程
*   `Ready Queue`：主内存中所有进程都已就绪，等待执行。
*   `Device Queue`：该队列存储由于输入/输出设备不可用而被阻塞的进程
...
*   `Symbolic Address`：用于源代码。变量名、常量和指令标签是符号地址空间的基本元素。
*   `Relative Address`：编译时，编译器将符号地址转换为相对地址。
*   `Physical Address`：加载程序负责在程序加载到主内存时生成这些地址。
