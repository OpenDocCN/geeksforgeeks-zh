...
> 原文: [https://www.geeksforgeeks.org/difference-between-turn-around-time-tat-and-waiting-time-wt-in-cpu-scheduling/](https://www.geeksforgeeks.org/difference-between-turn-around-time-tat-and-waiting-time-wt-in-cpu-scheduling/)
...
1.  它是从提交流程到完成流程的时间间隔。
2.  差 b/w `完成时间`和`到达时间`称为周转时间。

`完成时间(CT)`是流程完成其执行的时间。
`到达时间(AT)`是流程到达就绪状态的时间。
