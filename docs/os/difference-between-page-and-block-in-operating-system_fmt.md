...
> 原文：[https://www.geeksforgeeks.org/difference-between-page-and-block-in-operating-system/](https://www.geeksforgeeks.org/difference-between-page-and-block-in-operating-system/)
...
*   大多数操作系统使用块来存储数据。
*   当我们将数据放入块中时，称为`分块`，当我们从块中提取数据时，称为`去块`。
...
*   一些操作系统使用页面而不是块。
*   页面是内存中最小的存储单元，与内存相当。
*   如果主存储器中不存在处理器所需的页面，则称之为`页面错误`，并由称为`页面错误例程`的特殊例程带入主存储器。
*   在主内存中获取所需页面的技术称为`页面要求`。
*   页面充当操作系统和硬盘之间的中间人。
