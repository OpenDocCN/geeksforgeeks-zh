...
> 原文：`https://www.geeksforgeeks.org/web-operating-system/`
...
`AJAX`（异步 Javascript 和 XML）是一组用于创建异步 web 应用程序的技术。这些网络应用程序可以在后台从服务器发送和检索数据，而不会干扰任何其他网页，以小信息的形式。

## 流程

网络操作系统按照指定的程序工作。

1.  用户发出运行特定程序的请求，连同指定的数据。
2.  该请求然后被传递到最近的教育引擎，它可能驻留在网络上的任何地方。
3.  引擎随后在其资源中执行搜索，以检查它是否请求了程序。
4.  然后检查本地系统是否能够满足服务质量参数。
5.  引擎可能拒绝请求或将其传递给其他教育引擎，直到其中一个最终接受请求。
...
在该策略中，请求机器只向列表中的一台机器发送包含列表中其余机器的请求，因此网络负载相对较小。如果请求服务的程序在这台机器上可用，它会发回一个肯定的消息，否则它会将请求传递给任何一台机器，并列出所有剩余的机器，以此类推。
