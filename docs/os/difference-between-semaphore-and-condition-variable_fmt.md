...
> 原文: [https://www.geeksforgeeks.org/difference-between-semaphore-and-condition-variable/](https://www.geeksforgeeks.org/difference-between-semaphore-and-condition-variable/)
...
| 它们很粘，因为它们有内存，即使没有人调用 `sem_wait()`，`sem_post()`也会增加信号量。 | 它们是非粘性的，因为如果没有人等待 `signal()`，就不会保存 `signal()`。 |
