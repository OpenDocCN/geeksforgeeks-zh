...
> 原文: [https://www.geeksforgeeks.org/difference-between-sstf-and-look-disk-scheduling-algorithm/](https://www.geeksforgeeks.org/difference-between-sstf-and-look-disk-scheduling-algorithm/)

## SSTF 磁盘调度算法
`SSTF` 代表最短寻道时间优先。如名称所指定的，该算法服务于最接近 `head` 或 `pointer` 当前位置的任务请求。这里，`head` 的方向在决定整个 `head` 运动中起着至关重要的作用。如果两个请求之间出现联系，那么负责人将按照正在进行的方向处理遇到的请求。
...
## LOOK 磁盘调度算法
`LOOK` 算法其实是 `SCAN` 算法的改进版。在这种算法中，`head` 在磁盘的一端开始第一个请求，并通过服务其间的所有请求向另一端移动。与扫描不同，在这种情况下，`head` 不是一直到最后一个磁道，而是一直到最后一个请求，然后改变方向。与 `SSTF` 不同，它不服务于最接近 `head` 或 `pointer` 当前位置的任务请求。
...
| 6. | `LOOK` 不会对任何请求造成饥饿。 | 在这里，远离 `head` 的请求将遭受饥饿。 |
