...
> 原文：[https://www.geeksforgeeks.org/creating-sparse-files-on-windows-linux/](https://www.geeksforgeeks.org/creating-sparse-files-on-windows-linux/)
...
Windows 提供名为 `fsutil`（文件系统实用程序）的命令行程序，允许创建/分配/查询稀疏文件。要在 Windows 上创建稀疏文件，请在命令解释器（`cmd`）上执行以下命令。
...
在 Linux 上，创建稀疏文件的过程真的很琐碎，因为存在几个创建稀疏文件的命令。我们将使用 `truncate` 命令来创建稀疏文件。命令如下。
