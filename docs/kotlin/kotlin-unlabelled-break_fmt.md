...
我们将学习如何在`while`、`do-while`和`for` loop 中使用未标记的`break`表达式。
...
在上面的程序中，我们借助`while`循环和`break`表达式计算了从 1 到 10 的整数之和。取一个变量`sum`，用 0 初始化。另一个变量`i`迭代循环，并用 1 初始化。
现在，迭代器从(`i` = 1)继续执行`sum`语句。当迭代器值`i`变为 11 时，它执行中断表达式并退出循环，而不检查测试表达式(`i` <= `Int.MAX_VALUE`)。然后，控制传递到`while`块的以下语句`print()`，它打印整数之和= 55

## 在 do-while 循环中使用无标记断点
...
在上面的程序中，我们遍历数组来打印行星的名称。首先，初始化一个数组`names`用行星名命名，而`i`是测试表达式的迭代器。我们使用`names.size`计算数组的大小。
`do`块首先打印数组的元素，并且每次将数组在任意索引处的值与“Jupiter”进行比较。如果匹配，则递增迭代器并再次执行。如果匹配，则执行中断表达式并退出`do-while`循环，而不检查测试表达式。
...
在下面的程序中，我们通过比较字符值遍历字符串到一个特定的位置。首先，用值“GeeksforGeeks”初始化一个数组名`name`。然后使用迭代器遍历`for`循环，打印字符值，并在每个位置与字符进行比较。如果匹配，则退出循环并将控件转移到下面的语句。
