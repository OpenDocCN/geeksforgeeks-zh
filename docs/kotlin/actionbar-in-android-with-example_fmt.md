...
该文件定义了活动的布局。在这个例子中，主要的焦点是`ActionBar`，因此活动将只包含一个简单的[文本视图](https://www.geeksforgeeks.org/textview-in-kotlin/)。下面是代码。
...
> **注:**为了解决所有与 `ActionBar` 相关的问题，谷歌在发布 **`API 21`** (安卓棒棒糖)的同时，推出了 **`Toolbar`**。