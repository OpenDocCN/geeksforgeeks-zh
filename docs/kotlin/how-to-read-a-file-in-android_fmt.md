...
> 原文：[https://www.geeksforgeeks.org/how-to-read-a-file-in-android/](https://www.geeksforgeeks.org/how-to-read-a-file-in-android/)

在安卓开发中，数据以文件、共享首选项和数据库的形式呈现。我们有不同格式的数据，根据我们的应用程序需要做的任务。在视频播放器或音乐播放器应用程序的情况下，我们在文件中有数据存储，**如何在本文学习的安卓**中读取这些文件。这里，首先我将在资产文件夹中存储一个文件（在我的例子中是 JSON 文件）。
...
*   **步骤 2：** 创建一个变量来存储文件的大小。
*   **步骤 3：** 创建一个文件大小的缓冲区。
*   **步骤 4：** 我们将把 `inputStream` 文件读入缓冲区。
*   **步骤 5：** 关闭 `inputStream` 文件。
*   **步骤 6：** 将缓冲文件转换为您需要的数据格式。
