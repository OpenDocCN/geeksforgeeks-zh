...
**说明:**
如果你熟悉 Java 或者其他任何面向对象的编程语言，你可能会在上面的程序中注意到，由于 `B` 类继承了 `A` 类，传递给 `display` 函数的参数是 `B` 类的一个实例。根据动态方法调度的概念，输出应该是 `25`，但是由于扩展函数是静态解析的，所以在类型 `A` 上调用 `operate` 函数，因此输出是 `10`。
