...
> 原文：[https://www.geeksforgeeks.org/how-to-build-a-grocery-android-app-using-mvvm-and-room-database/](https://www.geeksforgeeks.org/how-to-build-a-grocery-android-app-using-mvvm-and-room-database/)
...
在这个项目中，我们使用 [MVVM(模型视图视图模型)](https://www.geeksforgeeks.org/introduction-to-model-view-view-model-mvvm/)作为建筑模式，`Room` 作为数据库，[Coroutines](https://www.geeksforgeeks.org/kotlin-coroutines-on-android/) 和 [RecyclerView](https://www.geeksforgeeks.org/android-recyclerview/) 来显示项目列表。在跳到项目之前，让我们先了解一下这些术语。
...
Room 持久性库是一个数据库管理库，用于存储应用程序的数据，如杂货项目名称、杂货项目数量和杂货项目价格。Room 是 [SQLite](https://www.geeksforgeeks.org/introduction-to-sqlite/) 上的一个覆盖层，有助于轻松对数据库进行操作。
...
在进入编码部分之前，首先将这些库添加到你的 [gradle 文件](https://www.geeksforgeeks.org/android-build-gradle/)中，并将插件作为 `kotlin-kapt` 应用。要添加这些库，请转到 **Gradle Scripts > build.gradle (Module: app)**。
...
`entities` 类包含数据库中的所有列，应该用 `@Entity(tableName = "表的名称")` 进行注释。实体类是一个数据类。`@ColumnInfo` 注释用于输入列变量名和数据类型。我们还将添加主键用于自动增量。转到 **app > Java > com.example.application-name**。右键单击 **com.example.application-name** 转到新建并创建 Kotlin 文件/类，并将该文件命名为 `GroceryEntities`。看下面的代码来完全理解和实现。
