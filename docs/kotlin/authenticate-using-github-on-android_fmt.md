...
> 原文：[https://www.geeksforgeeks.org/authenticate-using-github-on-android/](https://www.geeksforgeeks.org/authenticate-using-github-on-android/)
...
转到您的 [`firebase` 控制台](http://console.firebase.google.com/)，转到您的应用程序，然后导航到 **项目设置**，并在连接的应用程序中添加类似指纹的 `SHA1`。要找到 `SHA1`，请前往 `Gradle`（`android studio` 窗口右侧）> 您的应用程序名称 > 任务 > `android` > `signingReport`（双击）。
...
现在，我们需要 **客户端 ID** 和 **客户端密钥**。为此[在 `GitHub` 上将你的应用](https://github.com/settings/applications/new)注册为开发者应用，并获取你的应用的 `OAuth 2.0` 客户端 `ID` 和客户端密码。要注册您的应用程序，请写下应用程序名称，输入应用程序的网站主页网址（这里我给出的网址与授权回调网址相同），并简要提供应用程序描述。
...
点击 **注册应用**。您将获得您的 **客户端标识** 和 **客户端密钥**，将其复制并粘贴到 `firebase` 控制台中（在登录方法下），然后单击保存按钮启用 `GitHub`。

![](img/8a7eec348a0aa142839866259f5e9473.png)

### 第四步

回到 `android studio`。通过使用 [`Firebase Android BoM`](https://firebase.google.com/docs/android/learn-more#bom)，在您的 `build.gradle`（模块：您的应用程序名称.app）中添加 `Firebase` 身份验证 `Android` 库的依赖项。
...
通过使用 `Firebase Android BoM`，您的应用将始终使用 `Firebase Android` 库的 **兼容版本**。

### 第五步：使用主活动文件

导航到 `app > res > 布局 > activity_main.xml` 并将下面的代码添加到该文件中。下面是 `activity_main.xml` 文件的代码。
