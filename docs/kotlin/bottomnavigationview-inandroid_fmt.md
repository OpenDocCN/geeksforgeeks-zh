...
*   现在创建一个新的安卓资源目录。右键点击 `res` 文件夹，选择安卓资源目录。确保选择资源类型作为 `menu`。现在创建 `bottom_menu.xml` 文件并添加以下代码。在这个文件中，我们为底部导航视图添加了标题、id 和菜单图标。下面是 `bottom_menu.xml` 文件的代码。
...
*   通过右键单击 java 包创建一个 `AlgorithmFragment`，选择新建 -> `Fragment (Blank)`。
*   按照上述步骤进行 `CourseFragment` 和 `ProfileFragment`。
*   现在在 `fragment_algorithm.xml` 文件中添加以下代码。此处布局中添加了一个[文本视图](https://www.geeksforgeeks.org/textview-widget-in-android-using-java-with-examples/)。
*   下面是 `fragment_algorithm.xml` 文件的代码。
...
*   现在在 `fragment_course.xml` 文件中添加以下代码。这里一个文本视图被添加到布局中。
*   下面是 `fragment_course.xml` 文件的代码。
...
*   下面是 `fragment_profile.xml` 文件的代码。
...
*   现在在 `activity_main.xml` 文件中添加以下代码。在这个文件中，我们将底部导航视图添加到布局中。
...
*   现在在 `MainActivity.java` 文件中添加以下代码。在这个文件中，我们在导航中添加了 `OnNavigationItemSelectedListener` 来帮助在片段之间导航。当用户点击图标时，它将切换片段。
