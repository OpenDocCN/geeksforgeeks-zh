...
> 原文：[https://www.geeksforgeeks.org/how-to-create-custom-loading-button-by-extending-viewclass-in-android/](https://www.geeksforgeeks.org/how-to-create-custom-loading-button-by-extending-viewclass-in-android/)
...
创建一个密封类 `ButtonState` 描述自定义按钮的状态（如点击，加载&完成）。下面是 `ButtonState.kt` 文件的代码。
...
创建另一个类 `LoadingButton`，其中与按钮相关的所有内容，如颜色、文本、动画等都将被定义。通过用 [`JvmOverloads`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.jvm/-jvm-overloads/) 注释来创建构造函数。下面是 `LoadingButton.kt` 文件的代码。
...
`loading_animation.xml` (在 `res -> animator` 下，在 res 下创建 animator 目录)
