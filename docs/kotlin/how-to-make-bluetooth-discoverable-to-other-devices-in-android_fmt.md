...
*   要使设备在其他设备列表中可被发现，必须使用`ACTION_REQUEST_DISCOVERABLE`意图，使用`startActivityForResult(Intent, int)`初始化新活动。
...
转到`AndroidManifest.xml`文件，添加蓝牙适配器所需的这些权限: `BLUETOOTH`，`BLUETOOTH_ADMIN`，和`ACCESS_COARSE_LOCATION`。
...
现在转到代表应用程序用户界面的`activity_main.xml`文件。创建三个`按钮`，一个用于打开设备的蓝牙，一个用于使设备的蓝牙在其他设备上可被发现，一个用于关闭蓝牙服务。下面是`activity_main.xml`文件的代码。代码中添加了注释，以更详细地理解代码。
