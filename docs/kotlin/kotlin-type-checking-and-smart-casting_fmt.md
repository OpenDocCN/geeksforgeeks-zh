...
**说明:**
这里我们初始化三个变量`name`、`age`和`salary`然后传入`List<Any>`。然后，借助`for`循环遍历列表，在每个 `if-else` 块中，我们使用`is`运算符检查元素的类型，并执行相应的 `print()` 语句。

使用 `when` 表达式–
我们可以很容易地将 `if-else` 块替换为`when`表达式。我们已经了解了何时在控制流文章中表达。更多细节可以参考科特林中的[当表达式](https://www.geeksforgeeks.org/kotlin-when-expression/)。
...
让我们以 Java 为例，首先，我们使用`instanceof`运算符检查变量的类型，然后将其转换为目标类型，如下所示–
