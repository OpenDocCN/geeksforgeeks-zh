...
假设`ZoomControls`是`ZoomControls`类的引用，该类用于调用`ZoomControls`类的不同方法。

> `zoomControls = (ZoomControls) findViewById(R.id.simpleZoomControls);`
...
*   `setOnZoomInClickListener(OnClickListener listener)`: 按下放大按钮时调用此方法。用于自定义按下放大按钮时显示的用户界面。
...
*   `setOnZoomOutClickListener(OnClickListener listener)`: 按下缩小时调用此方法。它的工作方式与`setOnZoomInClickListener()`方法相同，但它最小化了 ie 聚合用户界面的方式。
...
*   `setIsZoomOutEnabled(boolean isEnabled)`: 也是缩放控件的方法之一，用于启用或禁用缩小功能。
...
*   `setZoomSpeed(long speed)`: 这用于设置缩放控件正在进行的缩放的缩放速度。

## 缩放控件的重要属性

*   `id`: 该属性用于给缩放控件一个唯一的标识。
*   `background`: 这是用来给缩放控件赋予背景颜色的。

> `android:id="@+id/zoom_controls"`
> `android:background="#fffff"`

*   `padding`: 用于在缩放控件的侧面填充。
...
下面给出一个 GIF 示例，了解一下在这篇文章中我们要做什么。请注意，我们将使用 **Java** 和 **Kotlin** 语言来实现这个项目。
...
现在转到代表应用程序用户界面的**活动文件**。下面是`activity_main.xml`文件的代码。在代码内部添加注释，更详细地理解代码。
