...
> 原文：[https://www.geeksforgeeks.org/how-to-create-classes-in-android-studio/](https://www.geeksforgeeks.org/how-to-create-classes-in-android-studio/)

在安卓项目开发期间，大部分时间都需要项目中的类。例如，在 CRUD 操作的情况下，我们需要一个模型类来插入和检索数据。同样为了在我们的自定义视图中保存信息，我们需要创建一个 getter setter 类。所以基本上在安卓中有两种类型的类我们可以创建并且我们经常使用。
...
**步骤 3**：完成步骤 2 后，将出现如下弹出屏幕。在这里输入您的类名称，选择 `类`，点击 `进入` 按钮。

![](img/9ae548c637f249c312183fbb6a89d5ce.png)

成功完成上述步骤后，您可以在这里找到您的 `Java` 类。前往 `app > java > 您的包名 > GeeksforGeeks.java`。你可以在这里编写自己的 `Java` 代码。
...
成功完成以上步骤后，您可以在这里找到您的 `Kotlin` 类。去 `app > java > 你的包名 > GeeksforGeeks.kt`。你可以在这里自己写 `Kotlin` 代码。
