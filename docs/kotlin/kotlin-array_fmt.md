...
在 Kotlin 中，数组不是一种本机数据类型，而是由 `Array` 类表示的相似项的可变集合。
在 Kotlin 中有两种方式定义数组。
...
在上面的例子中，我们将数组的大小传递为 3 和一个[lambda 表达式](https://www.geeksforgeeks.org/kotlin-lambdas-expressions-and-anonymous-functions/)，该表达式将元素值从 0 初始化为 9。
...
除此之外，Kotlin 还有一些**内置的**工厂方法来创建原始数据类型的数组，比如 `byteArray`、`intArray`、`shortArray` 等。这些类不扩展 `Array` 类；但是，它们实现相同的方法和属性。
...
如你所知，Kotlin 的一个数组基本上就是一个类。因此，我们可以通过类对象的成员函数来访问它的数据。`get()` 和 `set()` 功能被称为成员函数。
...
**注：** 内部，索引运算符 `[]` 实际上是一个重载运算符（参见运算符重载），仅代表对 `get()` 和 `set()` 成员函数的调用。
...
{   // declare an array using arrayOf()
    ...
    num.set(1, 6)   // set the second element equal to 6
...
在遍历数组时，最简单也是最常用的习惯用法是使用 [for 循环](https://www.geeksforgeeks.org/kotlin-for-loop/)。
**语法：**
...
或者，我们可以使用范围来达到相同的效果。在 Kotlin 中，**范围**是两个值（开始和结束）之间的间隔，可以使用 `..` 操作符。然后可以使用 `for` 关键字中的 `in` 遍历该范围。
**范围语法：**
