...
> 原文: [https://www.geeksforgeeks.org/function-literals-with-receiver-in-kotlin/](https://www.geeksforgeeks.org/function-literals-with-receiver-in-kotlin/)
...
函数文字可以访问被调用的接收器，并且可以访问与该接收器关联的方法。我们也可以在普通函数中将接收器作为参数传递，其中第一个参数用于接收器。这在我们需要使用普通函数时非常有用。所以 `(String) -> Int` 类似于 `(String, String) -> Int` 兼容。查看以下示例:
...
fun main(args: Array<String>) {
  ...
  fun testIfEqual(op: (String, String) -> Int, a: String, b: String, c: Int) =
    assert(op(a, b) == c)
  testIfEqual(addS, "gonna do ", "nothing much.",
