...
> 原文: [https://www.geeksforgeeks.org/difference-between-var-and-val-in-kotlin/](https://www.geeksforgeeks.org/difference-between-var-and-val-in-kotlin/)
...
使用 `val` 存储的对象不能更改，不能重新分配，就像 java 中的 **final** 关键字一样。`val` 是**不可改变的**。一旦赋值，`val` 就变成只读的，但是 `val` 对象的属性可以改变，但是对象本身是只读的。
