...
> 原文：[https://www.geeksforgeeks.org/how-to-add-crossfade-animation-between-activities-in-android/](https://www.geeksforgeeks.org/how-to-add-crossfade-animation-between-activities-in-android/)

[安卓动画](https://www.geeksforgeeks.org/animation-in-android-with-example/)可以说是安卓应用中更好的用户界面和用户体验的最重要因素。在一系列关于 GeeksforGeeks 以创造更好的用户界面的文章中，应用程序的 UX 也将是其中之一。在本文中，我们将了解如何在安卓系统中的两个[活动](https://www.geeksforgeeks.org/introduction-to-activities-in-android/)之间添加交叉渐变动画。这里的意思是两个活动之间的平稳过渡。通常，它意味着在一个活动中淡出，而在另一个活动中淡出。它创造了一个平稳的过渡，在短时间内，用户将对这两种活动都有所感受。这是我们将在活动之间创建的同一类型动画的示例。注意，我们将使用 **Java 和 Kotlin** 语言来实现这个项目。
...
给它一个你选择的名字，就像我们给它的`SecondActivity`一样，然后点击完成。

### 第四步：使用布局文件

转到`res/layout/activity_second`，粘贴下面的代码。这里，我们使用带有按钮的约束布局来切换到另一个活动。

![](img/07ca1f4773358533bbeabbfbd92f1512.png)

下面是`activity_second.xml`文件的代码。
...
此外，导航至`res/layout/activity_main`，移除所有默认代码并粘贴以下代码。下面是`activity_main.xml`文件的代码。
...
动画文件夹和动画文件的创建请参考[如何在安卓工作室中创建动画文件夹&动画文件](https://www.geeksforgeeks.org/how-to-create-anim-folder-animation-file-in-android-studio/)。我们已经创建了两个动画文件，并将文件命名为`fade_in`和`fade_out`。现在，从`fade_in`中移除所有默认代码，并粘贴下面的代码。下面是`fade_in.xml`文件的代码。
...
只需在`onCreate()`方法后的`MainActivity`类中添加以下代码。这是前往`SecondActivity`的代码，我们正在设置两者之间的动画。
...
此外，在`onCreate()`方法后的`SecondActivity`中添加以下代码。
...
**GitHub 链接：**[https://github.com/shivamparashar165/crossfade_android_anim](https://github.com/shivamparashar165/crossfade_android_anim)