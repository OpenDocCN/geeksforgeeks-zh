...
| `android:checked` | 用于指定开关控制的当前状态。 |
| `android:thumb` | 用于设置可拉伸的，用作可以前后移动的拇指。 |
| `android:thumbTint` | 用于设置应用于拇指的色调。 |
| `android:text` | 用于设置开关的文本。 |
| `android:textOn` | 用于在切换按钮处于打开(选中)状态时设置文本。 |
| `android:textOff` | 用于在切换按钮处于关闭(未选中)状态时设置文本。 |
| `android:textStyle` | 用于设置文本的样式。例如，粗体、斜体、粗斜体等。 |
| `android:textColor` | 用于设置文本的颜色。 |
| `android:textSize` | 用于设置文本的大小。 |
| `android:background` | 用于设置切换按钮的背景色。 |
| `android:drawableBottom` | 用于将可绘制设置为文本的底部。 |
| `android:drawableLeft` | 用于设置文本左侧的可绘制。 |
| `android:drawableRight` | 用于将可绘制设置在文本的右侧。 |
| `android:padding` | 用于从左、右、上、下设置填充。 |
...
然后，将 `setOnCheckedChangeListener()` 设置为开关上的监听器，并使用 if 条件检查按钮的状态。
