...
转到代表应用程序用户界面的 `activity_main.xml` 文件。创建一个[编辑文本](https://www.geeksforgeeks.org/android-edittext-in-kotlin/)，在这里我们将提供要保存在剪贴板中的文本，以及一个**按钮**来执行保存操作。下面是 `activity_main.xml` 文件的代码。
