...
> 原文：[https://www.geeksforgeeks.org/how-to-use-singleton-pattern-for-room-database-in-android/](https://www.geeksforgeeks.org/how-to-use-singleton-pattern-for-room-database-in-android/)
...
First, let's define the data entity. This will be a data class. Let's name it `contact.kt`
...
Then we need to create knives. Knife is an interface, so there is no need to define methods in it. The room needs to pay attention to the implementation of these methods. Dao is used to access data objects in the database.
...
Now we will create the database class, which is the main access point for the application's persistent data. It is an abstract class that inherits `RoomDatabase`. We have achieved singleton pattern here.
...
When we create an instance of the `Database` class, we use a `companion object` to directly access the `getDatabase()` method. We use a synchronization block to prevent different threads from making multiple calls at the same time.
