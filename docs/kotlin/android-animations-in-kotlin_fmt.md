...
| XML ATTRIBUTES | 描述 |
| --- | --- |
| `android:duration` | 它用于指定动画运行的持续时间 |
| `android:fromAlpha` | 这是动画的起始 alpha 值，其中 1.0 表示完全不透明，0.0 表示完全透明 |
| `android:toAlpha` | 这是结束 alpha 值 |
| `android:id` | 设置视图的唯一 id |
| `android:fromYDelta` | 这是要在动画开始时应用的 Y 坐标变化 |
| `android:toYDelta` | 这是要在动画结束时应用的 Y 坐标变化 |
| `android:startOffset` | 到达开始时间后，动画运行时会出现延迟(以毫秒为单位)。 |
| `android:pivotX` | 它表示从起点开始缩放的 X 轴坐标。 |
| `android:pivotY` | 它表示从起点开始缩放的 Y 轴坐标。 |
| `android:fromXScale` | 起始 X 尺寸偏移， |
| `android:fromYScale` | 起始 Y 尺寸偏移， |
| `android:toXScale` | X 尺寸偏移的结束 |
| `android:toYScale` | Y 尺寸偏移的结束 |
| `android:fromDegrees` | 起始角度位置，单位为度。 |
| `android:toDegrees` | 结束角度位置，单位为度。 |
| `android:interpolator` | 插值器定义动画的变化率 |

首先，我们将创建一个新的安卓应用程序。然后，我们将创建一些动画。如果已经创建了项目，则忽略步骤 1。
...
如果您正确地遵循了上述过程，您将成功地获得一个新创建的项目。创建项目后，我们将修改 xml 文件。在 xml 文件中，我们将创建一个执行所有动画的 `TextView`，以及八个不同动画的八个 `Button`。
...
修改布局后，我们将为动画创建 xml 文件。所以我们将首先创建一个名为 `anim` 的文件夹。在这个文件夹中，我们将添加用于制作动画的 XML 文件。为此，请转到 `app/res` 右键单击，然后选择 `安卓资源目录` 并将其命名为 `anim`。
...
你可以在这里找到完整的代码:[https://github.com/missyadavmanisha/AnimationsInKotlin](https://github.com/missyadavmanisha/AnimationsInKotlin)