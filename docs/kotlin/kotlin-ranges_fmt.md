...
在[柯特林](https://www.geeksforgeeks.org/introduction-to-kotlin/)中，范围是由端点定义的有限值的集合。科特林的范围包括开始、停止和步骤。起始值和终止值包含在范围内，步长值默认为 1。该系列用于同类产品。
...
它与`rangeTo()`或`..`运算符不同。它以降序创建一个范围，即从较大的值到较小的值。下面我们以相反的顺序为整数和字符创建范围。
...
使用关键字`step`，可以提供值之间的步长。它主要用于在`rangeTo()`或`downTo()`或`in`中提供两个值之间的差距。`..`运算符。步长的默认值为 1，`step`函数值不能为 0。
...
它用于反转给定的范围类型。我们可以使用`reversed()`函数以降序打印范围，而不是`downTo()`。
...
柯特林范围内有一些预定义函数：`min()`、`max()`、`sum()`、`average()`。
