...
> 原文：[https://www.geeksforgeeks.org/dynamichorizontal-scrollview-in-kotlin/](https://www.geeksforgeeks.org/dynamichorizontal-scrollview-in-kotlin/)

安卓`ScrollView`允许滚动父视图组中的多个视图。安卓应用程序中的滚动可以通过两种方式完成：垂直或水平。

在本文中，我们将讨论如何通过编程在Kotlin中创建一个水平滚动视图。
...
在这里，我们将使用`RelativeLayout`从Kotlin文件中获取滚动视图。
...
```xml
<resources>
    <string name="app_name">Dynamic Horizontal ScrollView</string>
</resources>
```

## 添加图像

我们需要添加一些图像，可以用于滚动的目的。因此，我们必须将图像从本地计算机路径复制到`app/res/drawable`文件夹。

## 在主活动文件中创建水平滚动视图

打开`app/src/main/java/your package name/MainActivity.kt`。在这个文件中，我们声明了一个变量`horizontalScrollView`来创建`HorizontalScrollView`小部件，如下所示：
