...
> 原文：[https://www.geeksforgeeks.org/how-to-get-resource-name-from-resource-id-in-android/](https://www.geeksforgeeks.org/how-to-get-resource-name-from-resource-id-in-android/)
...
*   `<package_name>` 是存储资源的包的名称
*   `<resource_type>` 是资源类型（可绘制、字符串、颜色等）的 `R` 子类
*   `<resource_name>` 是资源的名称（如果在布局元素中声明的话，如 `android:name`）或者是没有扩展名的文件名
