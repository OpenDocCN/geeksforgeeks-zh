...
> 原文：[https://www.geeksforgeeks.org/destructuring-declarations-in-kotlin/](https://www.geeksforgeeks.org/destructuring-declarations-in-kotlin/)
...
析构声明基于`组件()`函数的概念。析构声明可以定义的变量的数量，类提供这些数量的组件函数，从`组件1()`，`组件2()`到`组件n()`。默认情况下，Kotlin 中的`数据类`实现组件功能。
...
fun main() {
    ...
    // Creating two variables using destructuring declaration
...
**注意：** 如果要在析构声明中使用，组件函数定义必须在`operator`关键字之前。
