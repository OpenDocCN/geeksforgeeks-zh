...
> 原文：[https://www.geeksforgeeks.org/kotlin-infix-function-notation/](https://www.geeksforgeeks.org/kotlin-infix-function-notation/)

在本文中，我们将学习 Kotlin 函数中使用的*中缀符号*。在 Kotlin 中，标有 `infix` 关键字的函数也可以使用中缀表示法调用，即不使用括号和点进行调用。
Kotlin 中有两种中缀函数符号：
...
**说明：**
在这里，我们已经使用中缀 (`a and b`) 调用了 `a.and(b)` 函数。
两者在标准输出中产生相同的结果。
...
如果我们将值移位 2 位，则 `2^3 = 8` 变为 `2^(3-2=1) = 2`。
...
1. 首先，我们把中缀定义为类 `math` 中的中缀符号函数，因为它必须是成员函数。
2. `infix` 关键字用于标记功能。
...
然后，我们为类 `math()` 创建一个对象，并使用中缀符号调用该函数：
