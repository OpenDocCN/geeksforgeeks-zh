...
> 原文：[https://www.geeksforgeeks.org/how-to-pass-data-to-destination-using-safe-args-in-android/](https://www.geeksforgeeks.org/how-to-pass-data-to-destination-using-safe-args-in-android/)

**安全参数**是一个 Gradle 插件，允许您将数据传递到目标 UI 组件。它生成简单的对象和生成器类，用于类型安全导航和访问任何关联的参数。[安全参数](https://developer.android.com/guide/navigation/navigation-pass-data)强烈推荐用于导航和传递数据，因为它确保类型安全。下面给出了一个示例视频，以了解我们将在本文中做什么。注意，我们将使用**Kotlin**语言来实现这个项目。
...
将会打开一个对话框。在片段名称中写 `Registration`，在片段布局名称中写 `fragment_registration`。以类似的方式创建另一个片段，片段名为 `Details`，片段布局名为 `fragment_details`。

![](img/a84e9d2fc51ebe630f23243c6c002f58.png)

### 第四步：创建两个片段的 XML 布局

转到 `Fragment_registration.xml` 文件，参考以下代码。下面是文件的代码。这是应用程序中使用的基本布局。
...
转到 `Fragment_detail.xml` 文件，参考以下代码。下面是文件的代码。
...
创建一个新的类 `User.kt`。我们将使用具有名称、电子邮件、密码的自定义通用 `User` 数据传递给另一个片段。
...
在对话框中，将文件名写为 `nav_graph`，选择资源类型为“导航”。然后单击确定。现在打开刚刚创建的 `nav_graph.xml` 文件，点击新的目标图标，选择两个片段。将 `Fragment_registration` 设为主页。创建从 `Fragment_registration` 到 `Fragment_details` 的动作，并在 `Fragment_details` 的参数中传递 `User` 参数。它的 XML 代码如下所示。
...
转到`Registration.kt`文件，参考以下代码。以下是`Registration.kt`文件的代码。代码中添加了注释，以更详细地理解代码。
...
### 步骤9：使用`Details.kt`文件

转到`Details.kt`文件，参考以下代码。以下是`Details.kt`文件的代码。代码中添加了注释，以更详细地理解代码。
