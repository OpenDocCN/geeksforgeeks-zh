...
> 原文：[https://www.geeksforgeeks.org/how-to-create-custom-camera-using-camerax-in-android/](https://www.geeksforgeeks.org/how-to-create-custom-camera-using-camerax-in-android/)

[`CameraX`](https://developer.android.com/training/camerax) 用于在 app 中创建自定义相机。`CameraX` 是一个 Jetpack 支持库，旨在帮助您简化相机应用程序开发。下面给出了一个示例视频，以了解我们将在本文中做什么。注意，我们将使用 `Kotlin` 语言来实现这个项目。
...
如果 `kotlin` 扩展缺失，如下图添加 `kotlin-android-extensions`，点击“立即同步”。
...
转到 `activity_main.xml` 文件，参考以下代码。下面是 `activity_main.xml` 文件的代码。请注意，有一个名为 `PreviewView` 的视图带有 id `viewFinder`，我们将把它用作相机的取景器。
...
转到 `res > drawable` 并创建一个新文件 `capture_button.xml`。下面是它的代码。
...
[CameraX 示例视频](https://media.geeksforgeeks.org/wp-content/uploads/20210217190833/camerax_gfg.mp4)