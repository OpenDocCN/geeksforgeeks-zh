...
> 原文：[https://www.geeksforgeeks.org/how-to-invoke-keyboard-programmatically-in-android/](https://www.geeksforgeeks.org/how-to-invoke-keyboard-programmatically-in-android/)
...
键盘通常由输入法调用，如`编辑文本`，然而我们可以在没有这些输入法的情况下调用它们。有两种`方法`我们可以完成这个任务，通过对以下方面进行相当大的改变：

1.  `AndroidManifest.xml` 文件（或）
...
转到 `AndroidManifest.xml` 文件或 `MainActivity.kt` 文件并参考以下代码。请注意，这两种方法都可以一次实现。
