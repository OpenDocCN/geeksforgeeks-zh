...
创建布局后，我们需要从活动`onCreate()`回调方法中加载 XML 布局资源，并使用`findViewById`从 XML 中访问 UI 元素。
...
| `android:layout_width` | 用于声明布局中视图和视图组元素的宽度。 |
| `android:layout_height` | 用于声明布局中视图和视图组元素的高度。 |
| `android:layout_marginLeft` | 用于声明视图和视图组元素左侧使用的额外空间。 |
| `android:layout_marginRight` | 用于声明视图和视图组元素右侧使用的额外空间。 |
| `android:layout_marginTop` | 用于声明视图和视图组元素顶部使用的额外空间。 |
| `android:layout_marginBottom` | 用于声明视图和视图组元素底部使用的额外空间。 |
| `android:layout_gravity` | 用于定义子视图在布局中的位置。 |
