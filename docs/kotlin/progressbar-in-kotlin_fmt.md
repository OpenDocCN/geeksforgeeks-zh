...
我们可以通过将 `indeterminate` 属性设置为 `true` 来使用如下的不定 `progressBar`。
...
| `android:min` | 用于设置最小值 |
| `android:max` | 用于设置最大值 |
| `android:progress` | 用于设置 0 到最大值之间的默认进度整数值。 |
| `android:minHeight` | 用于设置进度条的高度。 |
| `android:minWidth` | 用于设置进度条的宽度。 |
| `android:background` | 用于设置进度条的背景颜色 |
| `android:indeterminate` | 用于启用不确定进度模式。 |
| `android:padding` | 用于设置进度条左侧、右侧、顶部或底部的填充。 |
