...
> 原文: [https://www.geeksforgeeks.org/dynamic-textswitcher-in-kotlin/](https://www.geeksforgeeks.org/dynamic-textswitcher-in-kotlin/)

安卓`TextSwitcher`是一个用户界面小部件，包含多个文本视图，一次显示一个。`TextSwitcher`是`ViewSwitcher`的子类，用于动画显示一个文本并显示下一个文本。

这里，我们在Kotlin文件中以编程方式创建`TextSwitcher`。
...
然后，我们创建`TextSwitcher`并为文本视图设置属性，如文本颜色、文本大小。

```kt
val textSwitcher = findViewById<TextSwitcher>(R.id.textSwitcher)
```

使用此在布局中添加`TextSwitcher`。
...
![](img/85319d4bea74ff6d78adcacc86420215.png)
![](img/bc6cdd2f8ceb8c17e1b4c093197f1a2a.png)