...
> 原文：[https://www.geeksforgeeks.org/android-progress-notifications-in-kotlin/](https://www.geeksforgeeks.org/android-progress-notifications-in-kotlin/)
...
1.  **小图标** – **必选**，可通过 `setSmallIcon()` 进行设置。
2.  **应用名称** – **由系统提供。**
3.  **时间戳** – **由系统提供，但可以被覆盖。**
4.  **大图标** – **可选**，可通过 `setLargeIcon()` 进行设置。
5.  **标题** – **可选**，可通过 `setContentTitle()`。
6.  **文本** – **可选**，可通过 `setContentText()` 进行设置。
...
### 固定工期进度指标
（在 `MainActivity.kt` 第 67 行注释中添加代码后）
![\\*](img/2d50af4f577d660e5682ddbed0d4fb95.png "Rendered by QuickLaTeX.com")
