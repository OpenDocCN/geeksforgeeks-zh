...
在[安卓](https://www.geeksforgeeks.org/introduction-to-android-development/)中，`VideoView`用于加载视频文件。我们可以依靠任何外部资源、网址或本地数据来获取视频内容。在本文中，我们将讨论如何在 Kotlin 中动态创建视频视图。

> **注意:**如果我们在后台使用视频视图或者刚从一个当前的视频会话返回，旧的视频位置没有保存，也就是我们上次离开视频的旧状态没有保存。为了实现它，我们需要利用一些外部数据库来存储状态。

为了简化整个过程，`VideoView`类提供了以下方法:

| 方法 | 描述 |
| --- | --- |
| `public void start()` | 它用于启动视频视图 |
| `public void pause()` | 暂停视频 |
| `public void resume()` | 恢复视频 |
| `public void stopPlayback()` | 停止视频 |
| `public void setVideoURI(Uri)` | 用于设置需要访问的视频文件的路径。它可以是 url，也可以是本地路径 |
| `public void seekTo(int msec)` | 跳到特定的时间间隔 |
| `public void setMediaController(MediaController controller)` | 这用于设置视频视图的控制器(播放、暂停、快进，就像您在视频中看到的按钮一样) |
| `public void getDuration()` | 它用于获取视频的总时长 |
| `public void getCurrentPosition()` | 它用于获取视频的当前时间间隔或位置 |
| `public void isPlaying()` | 它根据视频是否正在播放返回一个布尔值 |
| `public void setOnPreparedListener(MediaPlayer.OnPreparedListener)` | 当视频刚刚准备好开始时，它是一个监听器 |
| `public void setOnErrorListener(MediaPlayer.OnErrorListener)` | 当播放视频时发生错误时，它是一个监听器 |
| `public void setOnCompletionListener(MediaPlayer.OnCompletionListener)` | 当视频结束时，它是一个听众 |
| `public void setAnchorView(View view)` | 它设置媒体控制器控件在屏幕上的位置 |
