...
> 原文：[https://www.geeksforgeeks.org/how-to-implement-swipe-down-to-refresh-in-android-using-android-studio/](https://www.geeksforgeeks.org/how-to-implement-swipe-down-to-refresh-in-android-using-android-studio/)
...
### 步骤 1：
在开始编写代码之前，有必要在 `build.gradle` 文件中添加一个 `SwipeRefreshLayout` 依赖项以启用滑动布局。这种依赖性是：
```gradle
implementation "androidx.swiperefreshlayout:swiperefreshlayout:1.1.0"
```

### 步骤 2：
重要的是从前端 `activity_main.xml` 开始。创建一个 `SwipeRefreshLayout` 并添加一个 `TextView` 在屏幕上显示字符串并为其提供一定的 id。
...
### 步骤 3：
来到 `MainActivity` 文件，下面提供了同样的预览。在这个文件中，通过使用 `findViewById()` 方法将 `swipeRefreshLayout` 和 `textView` 连接到它的 XML 文件。并且还调用 `setOnRefreshListener()` 在用户向下滑动屏幕后更改文本。用户也可以根据需要在这个方法中编写所需的代码。
...
当然，受益的不仅仅是用户。假设有一个应用程序，信息直接从云存储库中获取。对于每个回调请求（向云），拥有这样一个存储库的开发人员向服务支付最低金额，可能是 [Google Cloud Platform (GCP)](https://www.geeksforgeeks.org/what-is-google-cloud-platform-gcp/) 、[Amazon Web Services (AWS)](https://www.geeksforgeeks.org/introduction-to-amazon-web-services/) ，或任何其他东西。