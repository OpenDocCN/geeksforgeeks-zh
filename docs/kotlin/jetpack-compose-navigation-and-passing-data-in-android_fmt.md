# Jetpack Compose 中的导航与数据传递

> 原文：[https://www.geeksforgeeks.org/jetpack-compose-navigation-and-passing-data-in-android/](https://www.geeksforgeeks.org/jetpack-compose-navigation-and-passing-data-in-android/)

几乎每个应用都使用某种导航，允许用户从一个屏幕移动到另一个屏幕。在本文中，我们将学习使用 Compose 方式在 Jetpack Compose 中实现导航。我们将建立一个简单的应用程序，演示 Jetpack Compose 导航，它将有三个屏幕（主页、个人资料和设置）。它将导航到带有一些数据的设置屏幕和不带任何数据的个人资料屏幕。下面给出了一个示例视频，让我们了解一下在本文中要做什么。
...
1.  [Kotlin](https://www.geeksforgeeks.org/kotlin-programming-language/) 基础知识。
2.  [Jetpack Compose](https://www.geeksforgeeks.org/basics-of-jetpack-compose-in-android/) 基础知识。

## 分步实施

### 步骤 1：创建一个新项目（或者在现有的 Compose 项目中使用它）
...
打开 `build.gradle` (app) 并将此行添加到依赖项中。

> `implementation "androidx.navigation:navigation-compose:2.4.0-alpha04"`

### 步骤 3：创建路由

创建一个名为 `Routes` 的文件。添加以下代码。它将包含所有屏幕的路径名。
...
它将有三个屏幕，所以我们需要创建三个可组合项。创建一个名为 `screens` 的包，并创建三个文件（`Home.kt`、`Profile.kt`、`Settings.kt`）。
...
最后，从 `MainActivity` 的 `onCreate` 中的 `setContent` 调用这个可组合项。
...
此外，您可以在使用 Compose 动画 API 打开屏幕时添加动画。
