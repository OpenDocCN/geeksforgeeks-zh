...
> 原文：[https://www.geeksforgeeks.org/android-fade-in-out-in-kotlin/](https://www.geeksforgeeks.org/android-fade-in-out-in-kotlin/)

在**安卓**中，动画是为了使用户界面更具交互性、清晰和美观而添加的视觉效果。淡入和淡出动画用于在设定的时间间隔内修改任何视图的外观，以便用户可以被告知我们的应用程序中正在发生的变化。
在本文中，我们将讨论如何在Kotlin中创建淡入淡出动画。

## XML属性说明

| XML 属性 | 描述 |
| --- | --- |
| `android:duration` | 它用于指定动画的持续时间 |
...
第一步是在Android Studio创建一个新的项目。为此，请遵循以下步骤：

*   单击 **File**，然后单击 **New**，然后单击 **New Project**，并给出您喜欢的名称。
*   然后，选择 **Kotlin** 语言支持，点击 **Next** 按钮。
*   选择最小 SDK，无论您需要什么。
*   选择 **Empty Activity**，然后点击 **Finish**。

之后，我们需要设计我们的布局。为此，我们需要处理 XML 文件。进入 `app > res > layout`，粘贴如下代码：
...
在这个文件夹中，我们将添加用于制作动画的 XML 文件。为此，请转到 **app/res** 右键，然后选择 **Android Resource Directory**，并将其命名为 **anim**。
再次右键点击这个 `anim` 文件夹，选择 **Animation Resource File**，命名为 **fade_in**。同样，也创建 **fade_out.xml** 并粘贴以下代码。
...
打开 `app/src/main/java/your package name/MainActivity.kt`，进行如下更改：
