...
> 原文:[https://www.geeksforgeeks.org/how-to-create-interfaces-in-android-studio/](https://www.geeksforgeeks.org/how-to-create-interfaces-in-android-studio/)
...
public interface InteractiveConfFragmentsListener {
...
要声明一个接口，使用`interface`关键字。它用于提供全面的抽象。这意味着接口中的所有方法都是用空体声明的，并且是公共的，所有字段都是公共的，默认情况下是静态的、和 `final`。实现接口的类必须实现接口中声明的所有方法。要实现界面使用`implements`关键字。
