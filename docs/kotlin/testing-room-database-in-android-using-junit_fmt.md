...
> 原文: [https://www.geeksforgeeks.org/testing-room-database-in-android-using-junit/](https://www.geeksforgeeks.org/testing-room-database-in-android-using-junit/)
...
要创建 `LanguageDatabase` 的测试类，请右键单击 `LanguageDatabase`，然后单击生成，然后选择测试。将打开一个对话框，从对话框中选择测试库为 `JUnit4` 并保留类名为默认的 `LanguageDatabaseTest`，点击确定。之后，会打开另一个对话框选择目标目录，选择有 `..\app\src\androidTest\` 的目录。因为我们的测试类需要来自应用程序的上下文。下面是指导你创建测试类的截图。
...
### 第 7 步: 使用 `LanguageDatabaseTest.kt` 类

转到 `LanguageDatabaseTest.kt` 文件，编写以下代码。代码中添加了注释，以更详细地理解代码。
...
要运行测试用例，点击类名附近的小运行图标，然后选择运行 `LanguageDatabaseTest`。如果所有的测试用例都通过了，你会在运行控制台得到一个绿色的勾号。就我们而言，所有测试都通过了。
