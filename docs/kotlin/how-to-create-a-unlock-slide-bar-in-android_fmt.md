...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-unlock-slide-bar-in-android/](https://www.geeksforgeeks.org/how-to-create-a-unlock-slide-bar-in-android/)
...
`SeekBar` 与 `ProgressBar` 具有相同的属性。但唯一不同的是，用户通过在 `SeekBar` 中移动滑块(拇指)来确定进度。要将 `SeekBar` 添加到布局(XML)文件中，可以使用 `<seekbar>` 元素。下面是解锁滑动条的示例。
...
下面给出一个 GIF 示例，了解一下在这篇文章中我们要做什么。注意我们要用 `Kotlin` 语言来实现这个项目。
...
现在转到 `activity_main.xml` 文件，该文件表示应用程序的用户界面。如图所示，创建一个 `SeekBar` 和 `TextView`。下面是 `activity_main.xml` 文件的代码。
