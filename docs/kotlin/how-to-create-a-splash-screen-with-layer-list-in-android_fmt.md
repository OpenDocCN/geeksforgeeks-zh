...
> 原文:[https://www.geeksforgeeks.org/how-to-create-a-splash-screen-with-layer-list-in-android/](https://www.geeksforgeeks.org/how-to-create-a-splash-screen-with-layer-list-in-android/)
...
在要添加闪屏内容的位置添加新的可绘制资源。转到`res`目录->`drawable`->`your_splashScreenDrawable.xml`
...
在这个代码片段中，您将使用`layer-list`中的`item`标签。这些物品标签有特定的属性来满足你的需求。所以这个片段将成为启动屏幕，它包含一个带有背景色的位图。
...
而`styles.xml`中的这段代码将为闪屏增加额外的属性，比如全屏等等。在这个属性中，`android:windowBackground`将添加可绘制的资源作为屏幕背景。

**第三步:在 MainActivity.kt 文件中处理闪屏显示时间**
...
现在这个代码是主要部分，因为它处理屏幕显示时间。这里有一个扩展`Runnable`的`Launcher`类，它被`Handler`对象延迟，当延迟时间结束时，启动器进入另一个活动。现在，剩下的关键部分在应用程序的清单文件中。
