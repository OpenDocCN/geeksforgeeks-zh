...
> 原文：[https://www.geeksforgeeks.org/how-to-create-splash-screen-without-an-extra-activity-in-android/](https://www.geeksforgeeks.org/how-to-create-splash-screen-without-an-extra-activity-in-android/)
...
### 步骤 2：创建一个新的可绘制文件，并将其命名为 `splash_image.xml`

转到 `res->drawable->splash_image.xml` 和以下代码。添加注释。
...
### 第三步：在 `themes.xml` 文件中添加新样式

转到 `res->values->themes.xml` 添加一个新样式，并将其命名为 `splashScreenTheme`。这将保存我们想要在闪屏上显示的信息，如 `窗口背景` 和 `状态栏颜色`。
...
**注意：** `activity_main.xml` 文件只是有一个显示消息的 `TextView`。
