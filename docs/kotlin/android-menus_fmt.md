...
*   `<menu>` 是根元素，用于在 XML 文件中定义菜单，它可以包含多个元素。
*   `<item>` 用于创建一个菜单项。它也可以包含嵌套的 `<menu>` 元素来创建子菜单。
*   `<group>` 是 `<item>` 元素的一个可选且不可见的选项，用于对菜单项进行分类，以便它们可以共享属性，例如活动状态和可见性。

## activity_main.xml

如果我们想在菜单项中添加子菜单，那么我们需要添加一个 `<menu>` 元素作为一个 `<item>` 的子元素。
