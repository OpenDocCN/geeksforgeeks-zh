...
| 属性 | 描述 |
| --- | --- |
| `android:fillViewport` | 定义`scrollview`是否应拉伸其内容以填充视口。 |
...
| 属性 | 描述 |
| --- | --- |
| `android:measureChildren` | 确定在测量时是测量所有子级还是只测量那些处于可见或不可见状态的子级。默认为`false`。 |

### 从视图

| 属性 | 描述 |
| --- | --- |
| `android:alpha` | 视图的`alpha`属性，值介于`0`(完全透明)和`1`(完全不透明)之间。 |
| `android:background` | 用作背景的图画。 |
...
| `android:isScrollContainer` | 如果视图将用作滚动容器，则设置此选项，这意味着可以调整视图的大小以缩小整个窗口，从而为输入法留出空间。 |
| `android:minHeight` | 定义视图的最小高度。 |
| `android:minWidth` | 定义视图的最小宽度。 |
| `android:onClick` | 单击视图时要在该视图的上下文中调用的方法的名称。 |
| `android:padding` | 以像素为单位设置所有四条边的填充。 |
...
| 属性 | 描述 |
| --- | --- |
| `android:addStatesFromChildren` | 设置该视图组的可绘制状态是否也包括其子视图组的可绘制状态。 |
| `android:animateLayoutChanges` | 定义布局中的更改是否应导致布局转换运行。 |
| `android:clipChildren` | 定义子对象是否被限制在其边界内绘制。 |
| `android:clipToPadding` | 定义如果填充不为零，视图组是否将剪切其子视图并将任何边缘效果调整到其填充。 |
| `android:layoutAnimation` | 定义第一次布局视图组时要使用的布局动画。 |
| `android:layoutMode` | 定义此视图组的布局模式。 |
| `android:splitMotionEvents` | 设置在触摸事件分派期间，此视图组是否应该拆分运动事件以分离子视图。 |
