...
这里，`GeeksforGeeks` 的类型是 `String`，所以编译器认为我们说的是 `MyClass<String>`。
...
在下面的例子中，我们创建了一个 `Company` 类，其主构造函数只有一个参数。现在，我们尝试将 `Company` 类的对象中不同类型的数据作为字符串和整数传递。`Company` 类的主构造函数接受字符串类型 (`"GeeksforGeeks"`)，但在传递整数类型 (`12`) 时给出编译时错误。
...
为了解决上述问题，我们可以创建一个用户定义的泛型类型类，在单个类中接受不同类型的参数。类型为 `T` 的类 `Company` 是一个通用类型类，同时接受 `Int` 和 `String` 类型的参数。
...
与 Java 不同，Kotlin 默认情况下使数组不变。通过扩展，泛型类型在 Kotlin 中是不变的。这可以通过关键词中的 `out` 和 `in` 来管理。不变性是一种属性，通过这种属性，已经为特定数据类型定义的标准泛型函数/类不能接受或返回另一种数据类型。`Any` 都是所有其他数据类型的超类型。
方差为两种类型：

1.  声明地点差异 (使用 `in` 和 `out`)
2.  使用部位差异: 类型投影
...
协变意味着替换子类型是可接受的，但超类型是不可接受的，即泛型函数/类可以接受已经为其定义的数据类型的子类型，例如，为 `Number` 定义的泛型类可以接受 `Int`，但为 `Int` 定义的泛型类不能接受 `Number`。这可以在 Kotlin 中使用 `out` 关键字来实现，如下所示:
...
我们可以通过将 `out` 关键字附加到声明站点来直接允许协变。以下代码工作正常。
...
它用于替换子类型中的超类型值，即泛型函数/类可以接受已经为其定义的数据类型的超类型，例如，为 `Number` 定义的泛型类不能接受 `Int`，但为 `Int` 定义的泛型类可以接受 `Number`。使用关键字中的 `in` 在 Kotlin 中实现如下:
