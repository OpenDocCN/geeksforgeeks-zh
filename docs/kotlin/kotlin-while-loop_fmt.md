...
在下面的程序中，我们使用 while 循环打印数字。首先，用 1 初始化变量`number`。将表达式(`number <= 10`)放入 while 循环中，检查是否正确？。如果为真，则进入块并执行打印语句，并将`number`增加 1。这样重复，直到条件变为假。
...
在下面的程序中，我们创建一个数组(`names`)，用不同数量的字符串初始化，并将变量`index`初始化为 0。一个数组的大小可以通过`names.size`来计算。将条件(`index < names.size`)放入 while 循环中。
如果`index`值小于或等于数组大小，则它进入块并打印存储在相应索引处的名称，并且在每次迭代后增加`index`值。这样重复，直到条件变为假。
