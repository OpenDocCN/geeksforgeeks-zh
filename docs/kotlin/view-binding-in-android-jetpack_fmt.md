...
> 原文: [https://www.geeksforgeeks.org/view-binding-in-android-jetpack/](https://www.geeksforgeeks.org/view-binding-in-android-jetpack/)
...
*   此外，无论所有元素的标识是在布局 XML 文件中创建的，视图绑定都会将它们转换为 camel case。例如: `android:id = " button _ submit "->button submit。` 这在代码可读性方面非常有用。
...
*   文件的主布局包含一个编辑文本和一个按钮。要实现相同的用户界面，在 `activity_main.xml` 文件中调用以下代码。
