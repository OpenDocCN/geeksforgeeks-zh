...
> 原文：[https://www.geeksforgeeks.org/how-to-communicate-between-fragments-in-android/](https://www.geeksforgeeks.org/how-to-communicate-between-fragments-in-android/)
...
*   我们可以使用 **view model** 在片段中进行通信。
*   我们也可以使用 **interface** 在片段之间进行通信。
...
导航到 `MainActivity.kt` 所在的项目文件。右键单击该文件夹，然后单击新包，并将其命名为 `fragments`。现在在这个新创建的文件夹中创建两个空白片段，并将它们命名为 `Fragment1` 和 `Fragment2`。

### 第三步：处理 XML 文件

导航到 `app > res > layout > fragment1.xml` 并将下面的代码添加到该文件中。下面是 `fragment1.xml` 文件的代码。
...
导航到 `app > res > layout > fragment2.xml` 并将下面的代码添加到该文件中。下面是 `fragment2.xml` 文件的代码。
