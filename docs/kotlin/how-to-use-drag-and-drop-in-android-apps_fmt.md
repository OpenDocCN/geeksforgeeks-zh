...
> 原文：[https://www.geeksforgeeks.org/how-to-use-drag-and-drop-in-android-apps/](https://www.geeksforgeeks.org/how-to-use-drag-and-drop-in-android-apps/)
...
| `getAction()`值 | 这是什么意思？ |
| --- | --- |
| `ACTION_DRAG_ENTERED` | When the drag shadow enters the bounding box of the `View` object, the drag event listener receives the event action type. When the shadow is dragged into the bounding box, the listener receives the first event action type. If the listener wants to receive the drag event of this operation in the future, it must return a Boolean value of `true` to the system. |
| `ACTION_DRAG_LOCATION` | When the drag shadow is still in the bounding box of the view, the drag event listener of the view object will receive the `ACTION_DRAG_ENTERED` event when it gets the action dragentered event. |
| `ACTION_DRAG_STARTED` | The drag event listener of the `View` object receives this event operation type immediately after the application runs `startDrag()` and gets the drag shadow. |
| `ACTION_DRAG_EXITED` | When the drag event listener of the view object receives an `ACTION_DRAG_ENTERED` and at least one `ACTION_DRAG_LOCATION` event, and after the user has dragged the drag shadow out of the bounding box of the view, the event action type is received by it. |
| `ACTION_DROP` | 当用户在 `View` 对象上释放拖拽阴影时，`View` 对象上的拖拽事件监听器接收到该事件动作类型。只有当侦听器响应 `ACTION_DRAG_STARTED` drag 事件而响应 `true` 时，此操作类型才会传递给视图对象的侦听器。如果用户在未注册侦听器的视图上释放拖动阴影，或者如果用户在不属于当前布局的任何内容上释放拖动阴影，则不会传输此操作类型。如果丢弃被正确处理，监听器应该返回布尔值 `true`。如果不是，它应该返回 `false`。 |
| `ACTION_DRAG_ENDED` | Notifies the view that the drag-and-drop action is complete. |
...
在这里获取完整代码：[https://media.geeksforgeeks.org/wp-content/cdn-uploads/20210908003121/Geeks-for-Geeks-Android-Annotation-main1.zip](https://media.geeksforgeeks.org/wp-content/cdn-uploads/20210908003121/Geeks-for-Geeks-Android-Annotation-main1.zip)。
