...
在 Kotlin 中，`when` 替换了其他语言如 Java 的 `switch` 操作符。当满足某个条件时，需要执行某个代码块。`when` 表达式的参数与所有分支逐一比较，直到找到某种匹配。找到第一个匹配项后，它会到达 `when` 块的末尾，并执行 `when` 块旁边的代码。与 Java 或任何其他编程语言中的 `switch case` 不同，我们不需要在每个 `case` 的末尾使用 `break` 语句。

在 Kotlin 中，`when` 可以用两种方式：
*   `when` 作为语句
*   `when` 作为一种表达式

### 使用 `when` 作为语句（带 `else`）

`when` 可以作为有无 `else` 分支的语句。如果将其用作语句，则将所有单个分支的值与参数进行顺序比较，并在条件匹配的情况下执行相应的分支。如果没有一个分支满足条件，那么它将执行 `else` 分支。
...
如果用作表达式，满足条件的分支的值将是整个表达式的值。作为表达式，`when` 返回一个与自变量匹配的值时，我们可以将其存储在变量中或者直接打印出来。
...
**使用 `in` 或 `!in` 检查输入值是否在范围内**
使用 `in` 或 `!in` 运算符，我们可以检查在 `when` 块中传入的参数是否在某个范围内。Kotlin 中的 `in` 运算符用于检查某个范围内特定变量或属性的存在性。如果参数位于特定范围内，那么 `in` 运算符返回 `true`，如果参数不位于特定范围内，那么 `!in` 返回 `true`。
...
**检查给定变量是否为特定类型**
使用 `is` 或 `!is` 运算符我们可以在 `when` 块中检查作为参数传递的变量的类型。如果变量是整数类型，则为 `is Int` 返回真，否则返回假。
