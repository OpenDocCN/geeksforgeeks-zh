...
> 原文：[https://www.geeksforgeeks.org/types-of-observables-in-rxjava/](https://www.geeksforgeeks.org/types-of-observables-in-rxjava/)
...
![图一。RxJava 中的可观察性解释。](img/6214c2022552a27f7ce20168f41b84ab.png)
...
当“可观察”发出大量观察者无法吸收的值时，“可流动”开始发挥作用。在这种情况下，可观察对象必须根据策略跳过一些数据，否则，它将抛出异常。`Flowable` 观察点使用一种策略来处理异常。`BackpressureStrategy` 为策略，`MissingBackpressureException` 为异常。

> **提示：** 就像类型#1 一样，你可以使用 `Flowable.create()` 类似地创建 `Flowable`。
...
```kt
fun singleObserver(): SingleObserver<String> {
