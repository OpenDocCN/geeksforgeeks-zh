...
**委托**控制任何对象从一个实例到另一个实例的权力/权限分配。对于类和函数实现，委托可以用于它们之间的静态和可变关系。在委托技术的帮助下，类和函数中的继承实现可以被改变，面向对象编程语言天生支持它，而不需要任何样板代码。委托是在 `by` 关键字的帮助下在 Kotlin 中使用的。
...
我们知道，在 Kotlin 中，继承为我们提供了不可变的对象之间的永久静态关系，而委托是可变的，这一事实使得**委托**成为一个极其强大的选择。在本例中，使用 `Newfeature` 类，我们可以通过委托其所有公共成员（即 `mymessage` 和 `mymessageline`）来实现具有新功能的委托基类，并且我们正在 `by` 关键字的帮助下使用该实现。
