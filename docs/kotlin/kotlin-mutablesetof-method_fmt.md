...
[Kotlin](https://www.geeksforgeeks.org/introduction-to-kotlin/) `Set`接口是一个通用的无序元素集合，不包含重复的元素。Kotlin支持两种类型的集合：`可变`和`不可变`。
`setOf()`为`不可变`，表示只支持只读功能；`mutableSetOf()`为`可变`，表示支持读写两种功能。
