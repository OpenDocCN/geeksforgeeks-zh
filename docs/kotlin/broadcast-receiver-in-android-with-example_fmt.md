...
> 原文：[https://www.geeksforgeeks.org/broadcast-receiver-in-android-with-example/](https://www.geeksforgeeks.org/broadcast-receiver-in-android-with-example/)
...
由于从 API Level 26 开始，大部分广播只能被动态接收器捕获，因此我们在下面给出的示例项目中实现了动态接收器。`Intent`类中定义了一些静态字段，可以用来广播不同的事件。我们将飞机模式的改变视为广播事件，但是有许多事件可以使用广播寄存器。以下是一些重要的全系统生成意图：

| 意图 | 事件描述 |
| --- | --- |
| `android.intent.action.BATTERY_LOW` | 表示设备电池电量不足。 |
| `android.intent.action.BOOT_COMPLETED` | 这将在系统完成引导后广播一次 |
| `android.intent.action.CALL` | 给数据指定的人打电话 |
| `android.intent.action.DATE_CHANGED` | 指示日期已更改 |
| `android.intent.action.REBOOT` | 表示设备已经重新启动 |
| `android.net.conn.CONNECTION_CHANGE` | 移动网络或 wifi 连接已更改（或重置） |
| `android.intent.action.AIRPLANE_MODE_CHANGED` | 这表明飞机模式已经打开或关闭。 |
...
转到 `app > java > 您的包名（其中有 mainactivity） > 右键单击 > 新建 > Kotlin 文件/类`，并将文件命名为 `AirplaneModeChangeReceiver`。以下是 `AirplaneModeChangeReceiver` 文件的代码。代码中添加了注释，以更详细地理解代码。
