...
> 原文: [https://www.geeksforgeeks.org/how-to-create-static-shortcuts-in-android-app/](https://www.geeksforgeeks.org/how-to-create-static-shortcuts-in-android-app/)
...
### 第二步：在 `res/xml` 文件夹中创建安卓资源文件 (xml)

转到 **res -> 安卓资源文件**并创建一个如下所示的安卓资源文件。

![android resource file](img/910e35691cefe7984a4630e22db7e94f.png)

创建一个文件，我们可以存储所有的快捷方式。一个具有 XML 格式的安卓资源文件被创建，在那里我们编程所有的快捷方式。保存该文件的文件路径为 `res/xml/shortcuts.xml`。无需更改任何其他参数，点击**确定**。

![Android res file](img/e54b43bae9c9faa1d8323ab4b1567016.png)

一旦文件生成，快捷方式可以被声明，但是在此之前，在安卓清单中声明**元数据**来链接 `shortcuts.xml` 文件作为应用程序的资源。

### 第三步：将 `shortcuts.xml` 文件声明为应用程序快捷方式的资源
...
### 第五步：配置 `shortcuts.xml` 文件生成不同的快捷方式

因为这是一个 XML 文件，这个文件的结构将是分层元素的形式。`<shortcuts>` 是根元素，而 `<shortcut>` 元素定义了快捷键的上下文。一个快捷方式主要由 6 个参数和 1 个意图 3 个参数组成。我们现在简单地定义每一个。

**快捷元素的参数:**

| 元素 | 描述 |
| --- | --- |
| `android:shortcutId` | 字符串文字，表示快捷方式管理器对象对其执行操作。 |
| `android:enabled` | 确定用户是否可以与快捷方式交互从支持的发射器，或者 `true` 或者 `false`。 |
| `android:shortcutShortLabel` | 描述快捷方式的简洁短语，用途最多 10 个字符。 |
| `android:shortcutLongLabel` | 描述快捷方式的扩展短语，用途最多 25 个字符。 |
| `android:shortcutDisabledMessage` | 出现在支持的启动器中的消息，当用户试图启动禁用的快捷方式时。 |
| `android:icon` | 根据快捷方式显示图标。 |

通过右键单击图标并单击**图像/矢量**资源，可在 `res/drawable` 文件夹中创建一个图标。参考下图。

![vector asset](img/f01cc0d6fb0283332a0f110445024642.png)

选择矢量资产，选择剪贴画，给它命名 `icon`，如下图所示。
...
| 因素 | 描述 |
| --- | --- |
| `android:action` | 当用户选择快捷方式时，系统启动的操作。 |
| `android:targetPackage` | 应用程序的包。 |
| `android:targetClass` | 快捷方式希望用户重定向到的应用程序的类。 |

**快捷方式代码 `shortcuts.xml` 文件:**

只是为了可视化快捷方式，需要参数 `shortcutId`、`enabled`、`icon`、`shortcutShortLabel`，其余可以忽略。这些值是随机的，并在本文后面部分提供的 `res/values/strings.xml` 文件中定义。在这里，意图的行为方式与将用户从一个活动发送到另一个活动时的行为方式相同。当用户按住应用程序图标 2 秒钟时，会出现 4 个快捷方式，单击这些快捷方式会打开应用程序的不同活动。参考以下程序中的注释。
...
要检查快捷方式是否将用户带入不同的活动，请在项目中再添加 3 个活动：活动 1、活动 2 和活动 3。这些活动也在上面的代码中实现。以下是这些活动的前端代码。`Activity1.kt`，`Activity2.kt`，`Activity3.kt` 的代码不相同。
...
在 `RES/values/strings . XML` 下声明的这些字符串值(`compose_shortcut_short_label0`，`compose_shortcut_short_label1`，`compose_shortcut_short_label2`，`compose_shortcut_short_label3`)定义了当快捷方式列表扩展时显示为快捷方式标签的字符串。
