...
如您所见，当`a`的值为`null`时，`let`函数简单地避开代码块。因此，解决了程序员最大的噩梦-**NullPointerException。**
...
当`company`值为空时，`run`的主体将被忽略。当它为非空时，主体执行。
...
我们可以通过 lambda receiver 关键字–`this`来引用上下文对象。`this`关键字在`run`、`with`、`apply`功能中做对象引用。
...
> **注意:**我们可以排除`this`关键字来指代类的成员。

**2. `it`**

`let`和`also`函数将对象的上下文作为 **lambda 参数。**
