...
> 原文：[https://www.geeksforgeeks.org/android-rotate-animations-in-kotlin/](https://www.geeksforgeeks.org/android-rotate-animations-in-kotlin/)
...
| `android:pivotY` | 定义对象围绕其旋转的点的 Y 坐标 |
| `android:fromDegrees` | 物体的旋转从这个几何角度开始 |
| `android:toDegrees` | 物体的旋转以这个几何角度结束 |
| `android:duration` | 用于以毫秒为单位定义动画的持续时间 |
| `android:startOffset` | 用于以毫秒为单位延迟动画时间 |
...
在应用程序的 `res` 文件夹中创建新目录，并将其命名为 `anim`。在该目录下创建 2 个**动画资源文件**，即顺时针旋转和逆时针旋转。这两个文件是保存动画细节的 XML 文件。下面是这两个文件的代码。

**文件名：** `rotate_clockwise.xml`
