...
> 原文：[https://www.geeksforgeeks.org/unit-testing-in-android-using-junit/](https://www.geeksforgeks.org/unit-testing-in-android-using-junit/)

做单元测试是为了保证开发人员写出高质量、无错误的代码。建议在编写实际的应用程序之前编写单元测试，您将事先编写测试，并且实际的代码将必须遵守测试制定的设计指南。在本文中，我们使用 `JUnit` 来测试我们的代码。`JUnit` 是一个用于 Java 应用的“单元测试”框架，默认情况下已经包含在 android studio 中。它是单元测试和用户界面测试的自动化框架。包含 `@Test`、`@Before`、`@After` 等注释。这里我们将只使用 `@Test` 注释来保持文章的通俗易懂。注意，我们将使用 `Kotlin` 语言来实现这个项目。
...
创建一个新的 Kotlin 文件 `RegistrationUtil` 并选择其类型作为对象。由于这是单例，我们不需要在其他类中使用它时创建它的对象。它有一个名为 `validRegistrationInput` 的函数，需要三个参数 `userName`、`password` 和 `confirmPassword`。我们将使用以下测试用例，用不同的输入集测试这个函数。
...
要创建 `RegistrationUtil` 的测试类，右键单击 `RegistrationUtil` 上的，然后单击生成，然后选择测试。将会打开一个对话框，从对话框中选择测试库为 JUnit4，并将类名保留为默认名称，即 `RegistrationUtilTest`，然后单击确定。之后，会打开另一个对话框选择目的目录，选择有 `..app\src\test\` 的目录。因为我们的测试类不要求来自应用的任何上下文。下面是指导你创建测试类的截图。
...
转到 `RegistrationUtilTest.kt` 文件，并编写以下代码。代码中添加了注释，以更详细地理解代码。
...
要运行测试用例，点击类名附近的小运行图标，然后选择运行 `RegistrationUtilTest`。如果所有的测试用例都通过了，你将会在运行控制台得到一个绿色的勾号。就我们而言，所有测试都通过了。
