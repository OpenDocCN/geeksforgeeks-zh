...
安卓`DatePicker`是一个用户界面控件，用于在我们的安卓应用中按日、月、年选择日期。日期选择器用于确保用户选择有效的日期。

在具有`calendar`和`spinner`两种模式的安卓日期选择器中，第一种模式显示完整的日历，第二种模式在微调视图中显示日期。

我们可以通过两种方式创建日期选择器控件，要么在 XML 文件中手动创建，要么在活动文件中以编程方式创建。

首先，我们按照以下步骤创建一个新项目：

1.  点击`File`，然后`New` => `New Project`。
2.  之后包含 Kotlin 支持，点击`Next`。
