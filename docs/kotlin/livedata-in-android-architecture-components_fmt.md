...
> 原文：[https://www.geeksforgeeks.org/livedata-in-android-architecture-components/](https://www.geeksforgeeks.org/livedata-in-android-architecture-components/)

LiveData 是[安卓架构组件](https://www.geeksforgeeks.org/jetpack-architecture-components-in-android/)之一。LiveData 是一个**可观察数据持有者类**。这里的可观察是什么意思？可观察意味着 LiveData 可以被其他组件观察到，比如[活动](https://www.geeksforgeeks.org/introduction-to-activities-in-android/)和[片段](https://www.geeksforgeeks.org/introduction-fragments-android/)（UI 控制器）。LiveData 最重要的一点是，它了解观察者的生命周期，比如活动或片段。这意味着 LiveData 只更新处于活动生命周期状态的应用程序组件，如活动或片段。LiveData 通知处于“已启动”或“已恢复”生命周期状态的观察者（活动或片段）。注册观察 LiveData 对象的非活动观察者不会收到有关更改的通知。这里，非活动观察器是指不处于“已启动”或“已恢复”状态的观察器。我们可以注册一个观察者，该观察者与一个实现 `LifecycleOwner` 接口的对象配对，我们将在示例中看到这个接口。当相应的生命周期对象的状态变为“已销毁”时，这种关系允许观察者被移除。
...
需要通过 LiveData 实现 `onActive()` 和 `onInactive()` 方法：
...
为了观察一个 LiveData 组件，`observe(LifecycleOwner, Observer<T>)` 方法被调用：
...
**第三步：** 创建一个 Kotlin 类文件 `MainActivityViewModel.kt`。我们的 `MainActivity` 类文件扩展了 `ViewModel` 类。
...
> **注**：这里我们使用的是 `MutableLiveData`，没错。但问题是为什么？因为已经有 `LiveData` 可用，所以 `MutableLiveData` 扩展了 `LiveData` 类，两个函数 `setValue()` 和 `postValue()` 是公开可用的。
...
> **注意**：这里 `observe()` 里面的 `this` 是 `LifecycleOwner`，正如我们上面讨论的观察值，我们应该传递 `LifecycleOwner`。这里的意思是 `MainActivity`，也就是观察者。
