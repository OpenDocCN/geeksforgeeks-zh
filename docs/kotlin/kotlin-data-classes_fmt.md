...
我们经常创建类来保存一些数据。在这样的类中，一些`标准函数`通常可以从数据中推导出来。在科特林，这种类型的类被称为`数据类`，并被标记为`data`。
...
*   `equals()`如果两个对象具有相同的内容，并且其工作方式类似于`==`，但是对于`Float`和`Double`值的工作方式不同，则该方法返回 true。
...
*   如果两个对象按照`equals()`的方法相等，那么返回的哈希码也将相同
