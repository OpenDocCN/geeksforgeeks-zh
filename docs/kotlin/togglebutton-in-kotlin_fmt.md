...
> **注意：** `ToggleButton` 继承了安卓的[按钮类](https://www.geeksforgeeks.org/button-in-kotlin/)。因此，按钮的所有属性也适用于此。
...
| `android:textOff` | 未选中时按钮上显示的文本 |
| `android:textOn` | 选中按钮时按钮上显示的文本 |
...
布局中的切换按钮可以使用 `findViewById()` 功能访问。

```kotlin
val toggle: ToggleButton = findViewById(R.id.toggleButton)
```

访问后，使用 `setOnCheckedChangeListener()` 方法设置一个监听器根据切换状态执行操作。
