...
| `android:checked` | 将复选框的默认状态设置为选中或未选中 |
| `android:background` | 设置复选框的背景颜色 |
| `android:text` | 用于在复选框中存储文本 |
| `android:fontFamily` | 设置复选框文本的字体 |
| `android:textSize` | 要设置 `CheckBox` 文本大小 |
| `android:layout_width` | 要设置 `CheckBox` 宽度 |
| `android:layout_height` | 要设置 `CheckBox` 高度 |
| `android:gravity` | 用于调整 `CheckBox` 文本对齐方式 |
| `android:padding` | 用于调整复选框的左、右、上和下填充 |
