...
`Toast` 是安卓屏幕上短时间间隔显示的短提醒信息。安卓吐司是一个简短的弹出通知，当我们在应用程序中执行任何操作时，它用来显示信息。
...
## 第一步
在 Android studio 中创建一个活动为空的项目，并确保已经用 Kotlin 语言配置了该应用。

![Configure-Project](img/ab8b1acdb6a529c8d02d268c121ba02a.png)
项目配置窗口

## 第二步
在左侧的项目窗口中，导航至 `app > res > layout > activity_main.xml`。
...
## 第五步
让我们对齐编辑文本和按钮。为了使组件在安卓系统中链接，我们有一些限制。我们将在编辑文本的末尾保留按钮，在按钮的开头保留编辑文本。
...
## 第六步
现在，我们去 Kotlin 类文件，让祝酒发生。打开 `app > java > com.example.toastedemo > MainActivity.kt` 中的 Kotlin 类文件。
...
这里，`messageEditText` 是我们在步骤 3 的布局文件中创建的 `EditText` 的对象。为了获取编辑文本的内容，我们使用了 `text` 属性，并将其存储在变量 `message` 中。
...
> *   `上下文` – 要使用的上下文。通常是您的 `Application` 或 `Activity` 对象。
> *   `文本` – 要显示的文本。可以是格式化文本。
> *   `持续时间` – 显示消息的时间。`LENGTH_SHORT` 或 `LENGTH_LONG`

我们将吐司存储在变量 `toast` 中。为了使吐司可见，我们应该调用 `toast.show()`。

## 第七步
现在我们将在按钮上添加一个点击事件监听器，并使其在点击/点击按钮时调用我们的 `toastMessage()` 方法。
