...
Kotlin 反射可在 `kotlin.reflect` package 获得。
...
我们可以获得在柯特林中定义的每个命名函数的**函数引用**。这可以通过功能名称前的 **`::` 操作符**来完成。这些函数引用可以用作其他函数的参数。在重载函数的情况下，我们可以显式指定函数的类型，也可以根据内容隐式确定。
...
我们可以使用 **`::` 运算符**，以与函数类似的方式获取**属性引用**。如果属性属于某个类，则还应该使用 **`::` 运算符**指定类名。这些属性引用允许我们将属性视为对象，也就是说，我们可以使用 get 函数获取这些值，或者使用 set 函数对其进行修改。
