...
> 原文：[https://www.geeksforgeeks.org/build-an-android-app-to-check-a-number-is-armstrong-or-not/](https://www.geeksforgeeks.org/build-an-android-app-to-check-a-number-is-armstrong-or-not/)

## 阿姆斯特朗的号码是多少？

如果一个数的位数之和与该数的位数之和的幂相加，则称该数为阿姆斯特朗数。给定一个数 `x`，确定给定的数是否是阿姆斯特朗的数。**n 位数**的正整数称为**阶 n** 的阿姆斯壮数（阶是位数）if。
