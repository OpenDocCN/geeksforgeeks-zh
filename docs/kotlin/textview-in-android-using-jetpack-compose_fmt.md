...
> 原文:[https://www.geeksforgeeks.org/textview-in-android-using-jetpack-compose/](https://www.geeksforgeeks.org/textview-in-android-using-jetpack-compose/)

`Jetpack Compose` 是谷歌提供的全新工具包。这对于设计漂亮的用户界面设计非常有用。安卓 `TextView` 是安卓中的一个简单视图，用于在我们的应用程序中显示文本。在本文中，我们将看一下使用 Jetpack Compose 在 Android 中实现 `TextView`。

## 文本视图小部件的属性

| 属性 | 描述 |
| --- | --- |
| `text` | 来设置我们必须在文本视图中显示的文本。 |
| `style` | 来改变安卓系统中 `TextView` 的风格。 |
| `color` | 更改文本视图的颜色。 |
| `fontSize` | 来改变文本的字体大小。 |
| `fontWeight` | 设定字体 ie 的权重。特别大胆，黑暗，还有更多 |
| `fontStyle` | 将字体样式设置为斜体。 |
| `fontFamily` | 更改文本的字体系列。 |
| `letterSpacing` | 设置文本视图中字母之间的间距。 |
| `background` | 为我们的文本视图设置背景色。 |
| `shadow` | 为我们的文本视图添加阴影。 |
| `textAlign` | 更改文本视图的对齐方式。 |
| `modifier` | 修改器被用来给我们的文本视图添加填充。 |
...
### 第二步:在 `MainActivity.kt` 文件中添加 `TextView`
