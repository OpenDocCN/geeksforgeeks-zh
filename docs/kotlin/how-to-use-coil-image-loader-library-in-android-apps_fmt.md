...
> 原文：[https://www.geeksforgeeks.org/how-to-use-coil-image-loader-library-in-android-apps/](https://www.geeksforgeeks.org/how-to-use-coil-image-loader-library-in-android-apps/)

`COIL` 是 `Coroutine` 图像加载器的首字母缩写。`COIL` 是安卓中著名的从 `URL` 加载图像的库之一。这是一个用于从服务器加载图像的现代库。该库用于从服务器、资产文件夹以及安卓项目中的可绘制文件夹加载图像。这个库的重要特点是快速、轻量、易用。在本文中，我们将看到如何在安卓应用程序中使用这个图像加载器库。
...
### 第二步：在 build.gradle 文件中添加线圈图像加载库的依赖关系
