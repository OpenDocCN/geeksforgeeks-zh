# Kotlin 异常处理：尝试、捕获、抛出与 finally

> 原文：[https://www.geeksforgeeks.org/kotlin-exception-handling-try-catch-throw-and-finally/](https://www.geeksforgeeks.org/kotlin-exception-handling-try-catch-throw-and-finally/)
...
2.  **未检查异常** – 通常由于逻辑错误而在运行时检查的异常，例如 `NullPointerException`、`ArrayIndexOutOfBoundsException` 等。

## Kotlin 中的异常

在 Kotlin 中，我们只有未检查的异常，并且只能在运行时捕获。所有的异常类都是 `Throwable` 类的后代。

我们一般使用 `throw` 表达式来抛出一个异常对象：
...
*   `ArithmeticException`：对数字进行无效算术运算时抛出，例如被零除。
*   `SecurityException`：抛出表示安全违规。
*   `ArrayIndexOutOfBoundsException`：当我们试图访问数组的无效索引值时抛出。
...
在 Kotlin 中，我们在程序中使用 `try-catch` 块进行异常处理。`try` 块包含负责引发异常的代码，`catch` 块用于处理异常。该块必须在 `main` 或其他方法中写入。`try` 块后面应该跟 `catch` 块或 `finally` 块，或者两者都跟。
...
**说明：**
在上面的程序中，我们使用了 `try-catch` 块。可以抛出异常的 `num` 变量被包含在 `try` 块的大括号内，因为算术中没有定义除零。异常被 `catch` 块捕获并执行 `println()` 语句。
...
在上面的代码中，我们使用了 `try-catch` 作为表达式。在程序顶部声明一个函数 `test`，并使用 `try-catch` 块返回一个值。我们已经从 `main` 方法中调用了 `test` 函数，并传递了参数值 `(10, 2)`。`test` 函数评估参数并返回 `try` 块的值 `(10/2 = 5)`。但是在下一次调用中，我们传递了 `(b=0)`，这一次异常被捕获并返回 `catch` 块的表达式。

## Kotlin finally 块

在 Kotlin 中，`finally` 块总是被执行，而不管 `catch` 块是否处理了异常。所以它被用来执行重要的代码语句。
我们也可以使用 `finally` 块和 `try` 块，并从那里跳过 `catch` 块。

**带有 try 块的 finally 块的语法：**
...
**使用 finally 与 try-catch 块的 Kotlin 程序：**
