...
Kotlin 语言提供了一种简单的数据类型，可以在单个实例中存储两个值。这可以使用称为 `Pair` 的数据类来完成。这是一个简单的[泛型类](https://www.geeksforgeeks.org/kotlin-generics/)，可以存储相同或不同数据类型的两个值，两个值之间可以有也可以没有关系。两个 `Pair` 对象之间的比较是基于**值**进行的，即如果两个配对对象的值相等，则两个配对对象相等。
...
我们可以在单个变量中接收对的值，或者我们可以使用 `first` 和 `second` 属性来提取值。
- `first`: 该字段存储配对的第一个值。
- `second`: 该字段存储配对的第二个值。
