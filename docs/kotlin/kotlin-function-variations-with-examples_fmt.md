...
> 原文: [https://www.geeksforgeeks.org/kotlin-function-variations-with-examples/](https://www.geeksforgeeks.org/kotlin-function-variations-with-examples/)
...
在 Kotlin 中，我们在定义函数时，可以选择指定可见性修改器。如果在定义时没有提到它，默认情况下，它被认为是公共的。
...
我们用“=”符号代替大括号作为函数体。我们也可以省略单表达式函数中的返回类型。
...
在 Kotlin 中，定义函数时，我们需要声明函数中返回的数据类型。如果我们什么都不返回，那么我们可以选择指定`Unit`或者如果我们什么都不指定，Kotlin 会假设它是一个什么都不返回的函数。
...
当我们在一个函数中有很多参数时，很难记住每个参数的位置。因此，这里我们可以使用**命名参数**，这里我们需要指定名称以及函数的值。我们可以更改值的顺序，该值将被分配给一个与值同名的参数。
