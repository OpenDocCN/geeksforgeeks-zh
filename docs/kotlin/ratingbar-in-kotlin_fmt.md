...
在 `RatingBar` 中，我们可以使用 `android:stepSize` 设置步长，它总是会返回一个 `1.0`、`2.0`、`2.5` 等浮点数形式的评级值。通过使用 `android:numStars` 属性，我们可以在 `RatingBar` 中指定星星的数量。`RatingBar` 用于从用户或客户那里获得关于产品、电影或酒店体验等的评级。
...
| --- | --- |
| `android:id` | 用于唯一标识控件。 |
| `android:rating` | 用于设置评级栏的默认评级值。 |
| `android:numStars` | 用于设置要显示的星星数量。 |
| `android:background` | 用于设置评级栏的背景颜色。 |
| `android:padding` | 用于设置滚动条的左、右、上或下的填充。 |
| `android:stepSize` | 用于在评级栏上设置步长，如 `0.5` 或 `1`。 |

## 修改 `activity_main.xml` 文件

在这个文件中，我们在线性布局中添加了评级栏和按钮。还为两个小部件设置属性，如 `id`、`stepSize`、`background` 等。
...
然后，声明另一个变量 `button`，并使用其 id 访问该按钮。
