# Kotlin中的`arrayListOf()`函数

`arrayListOf()`是[Kotlin ArrayList](https://www.geeksforgeeks.org/kotlin-list-arraylist/)类的函数，用于创建新的`ArrayList`。数组列表是可变的，这意味着我们可以修改数组列表的内容。
...
Kotlin中的`ArrayList`有一个属性，即`size`。它返回数组列表中的元素数量。
...
该函数用于将指定的元素集合添加到所提供索引处的`ArrayList`中。
...
该函数用于检查数组列表中是否存在元素。如果找到的话，它返回`true`，否则返回`false`。
...
此函数用于检查数组列表中是否存在元素集合。如果找到的话，它返回`true`，否则返回`false`。
...
该函数返回数组列表中指定元素第一次出现的索引。如果数组列表中没有元素，则返回`-1`。
...
该函数返回数组列表中指定元素最后一次出现的索引。如果数组列表中没有元素，则返回`-1`。
...
此函数用于从数组列表中移除指定元素的单个实例。如果元素出现在数组列表中并被移除，则返回`true`，否则返回`false`。
...
此函数用于从数组列表中移除元素的集合。如果元素集合被移除，则返回`true`，否则返回`false`。
...
该函数用于通过元素在数组列表中的位置来移除元素。如果元素集合被移除，则返回`true`，否则返回`false`。
...
此函数用于将一个元素添加到`ArrayList`中的指定位置。
...
如果数组列表为空，则该函数返回`true`，否则返回`false`。
