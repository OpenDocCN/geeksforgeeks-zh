...
> 原文: [https://www.geeksforgeeks.org/how-to-build-material-and-dark-themes-apps-using-style-in-android/](https://www.geeksforgeeks.org/how-to-build-material-and-dark-themes-apps-using-style-in-android/)
...
| 属性 | 描述 | 缺省值 |
| --- | --- | --- |
| `colorPrimary` | 此颜色是应用程序的原色。此颜色是应用程序中使用最多的颜色。这也可以称为品牌颜色 | `#FF6200EE` |
| `colorPrimaryVariant` | 这是 `colorPrimary` 的变体。这可能是原色的较亮或较暗的变体。 | `#FF3700B3` |
| `colorOnPrimary` | 名称本身表明颜色应该分配给那些高于颜色原色的元素。 | `#FFFFFFFF` |

### 次要颜色变体

| 属性 | 描述 | 缺省值 |
| --- | --- | --- |
| `colorSecondary` | 在颜色原色之后使用最多的颜色。这种颜色是原色的补色。 | `#FF03DAC5` |
| `colorSecondaryVariant` | 这是 `colorSecondary` 的变体。这可能是 `colorSecondary` 的较亮或较暗的变体。 | `#FF018786` |
| `colorOnSecondary` | 该名称还表明颜色应该分配给那些高于颜色二级的元素。 | `#FF000000` |

### 表面颜色

| 属性 | 描述 | 缺省值 |
| --- | --- | --- |
| `colorSurface` | 影响表面组件(如卡片、图纸、菜单)的表面颜色。 | `#FFFFFF` |
| `colorOnSurface` | 分配给 `colorSurface` 上方的文本或图标的颜色 | `#000000` |

### 错误颜色

| 属性 | 描述 | 缺省值 |
| --- | --- | --- |
| `colorError` | 这表示分配给表示错误的元素的颜色。 | `#B00020` |
| `colorOnError` | 分配给 `colorError` 上方的文本或图标的颜色。 | `#FFFFFF` |
...
*   我们需要主要决定原色和次原色，并从中提取颜色代码。在这种情况下，原色是绿色，作为补充，`红色_200` 和 `红色_700` 是次要品牌颜色。
...
*   当应用程序处于灯光主题下时，将应用下列值 `theme.xml`。
