...
可以使用如下所示的 `<TableLayout>` 来定义 `TableLayout`：
...
在这个文件中，我们声明了 `TableLayout`，并开始在 `TableRow` 的帮助下添加表行。我们正在创建玩家排名表，其中我们定义了四列：排名、姓名、国家和积分。
...
创建布局后，我们需要从活动的 `onCreate()` 回调方法中加载 XML 布局资源，并使用 `findViewById` 从 XML 中访问 UI 元素。
