...
科特林区分*不可变*和*可变*地图。用 `mapOf()` 创建的不可变映射意味着这些是只读的，而用 `mutableMapOf()` 创建的可变映射意味着我们可以同时执行读写。
...
我们可以用两种方法来确定地图的大小。使用地图的 `size` 属性并使用 `count()` 方法。
...
**说明：**
这里键值 `1` 已经初始化了两个值：`geeks1` 和 `geeks2`，但是我们知道 `mapOf()` 对于一个关键项只能有一个值，因此最后一个值只由地图存储，`geeks1` 被淘汰。