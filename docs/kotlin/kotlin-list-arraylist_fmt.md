...
> 1) `ArrayList<E>()`: – 创建一个空的数组列表
> 2) `ArrayList(容量: Int)`: – 创建一个指定大小的数组列表。
> 3) `ArrayList(元素: Collection<E>)`: – 创建一个由集合元素填充的数组列表。

## 一些重要的方法

### `add(索引: Int, 元素: E): Boolean`
...
### `addAll(索引: Int, 元素: Collection): Boolean`
...
它用于从当前集合中移除特定元素的第一个匹配项(如果有)。类似地，为了移除索引 i 处的元素，我们使用 `remove(索引)`。
