...
首先检查你用的是不是 python 的最新版本。因为 PyGame 只支持 python 3.7.7 或更高版本，所以请确保您使用的是最新版本的 python。键入以下命令检查文章的版本。
...
检查你是否使用 pip 最新版本: pip 是一个 python 包安装程序，如果你想在你的 python 文件中使用任何外部包，你首先要在你的本地系统中安装它，所以使用 pip 工具。如果您已经在使用新的 pip 版本，请按照下面的步骤操作。如果没有，请参考关于如何在 Linux 系统中安装 pip 的文章。

`pip3 --version`

## 更新系统包

如果没有使用最新的 pip 版本，则成为 root 用户并更新 Linux 包: 打开终端，确保您是根用户。
...
要确保 PyTorch 安装在您的系统中，只需键入 `python3` 并运行它，然后键入 `import torch` 以使用 PyTorch 库，最后键入并运行 `print(torch.__version__)`。如果您的系统中安装了 PyTorch，它会显示您的系统中安装了哪个版本的 PyTorch。
