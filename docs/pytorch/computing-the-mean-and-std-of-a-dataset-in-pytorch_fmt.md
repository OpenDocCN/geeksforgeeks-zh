...
> 原文: [https://www.geeksforgeeks.org/computing-the-mean-and-std-of-a-dataset-in-pytorch/](https://www.geeksforgeeks.org/computing-the-mean-and-std-of-a-dataset-in-pytorch/)
...
现在我们有了数据，我们可以通过调用`mean()`和`std()`方法来找到均值和标准差。
...
在二维张量中，平均值与一维张量的平均值相同，除了这里我们可以通过一个`axis`参数来找到行和列的平均值和标准值。让我们从获取数据开始。
...
`mean()`和`std()`方法在按原样调用时将返回整个数据集的总标准差，但是如果我们传递一个轴参数，我们可以找到行和列的平均值和 std。对于`axis = 0`，我们得到一个张量，它具有每列的平均值或标准值。对于`axis = 1`，我们得到一个张量，它有每行的平均值或标准值。
