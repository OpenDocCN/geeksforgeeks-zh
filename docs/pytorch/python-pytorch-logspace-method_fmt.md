...
> 原文: [https://www.geeksforgeeks.org/python-pytorch-logspace-method/](https://www.geeksforgeeks.org/python-pytorch-logspace-method/)

PyTorch 是脸书开发的开源机器学习库。它用于深度神经网络和自然语言处理。
函数 `torch.logspace()` 返回步长点的一维张量，步长点以对数方式间隔开，以 `base^start` 和 `base^end` 为底。
输出张量是一维的大小 `steps`。

> **语法** : `torch.logspace(start, end, steps=100, base=10, out=None)`
> **参数** :
> **`start`**: 设定点的起始值。
> **`end`**: 该组点的结束值
> **`steps`**: 开始和结束之间的采样点数。默认值: 100。
> **`base`**: 对数函数的底数。默认: 10.0
> **`out`** (张量，可选): 输出张量
> **返回类型** : 一个张量
