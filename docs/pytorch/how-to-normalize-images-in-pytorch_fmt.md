...
> 原文: [https://www.geeksforgeeks.org/how-to-normalize-images-in-pytorch/](https://www.geeksforgeeks.org/how-to-normalize-images-in-pytorch/)

图像变换是将图像像素的原始值改变为一组新值的过程。我们对图像进行的一种转换是将图像转换成 PyTorch 张量。当图像被转换成 PyTorch 张量时，像素值在 0.0 和 1.0 之间缩放。在 PyTorch 中，这个转换可以使用 `torchvision.transforms.ToTensor()` 来完成。它将像素范围为 [0, 255] 的 PIL 图像转换为范围为 [0.0, 1.0] 的形状 (C, H, W) 的 PyTorch 浮点张量。
...
规范化有助于获得一定范围内的数据，并降低偏斜度，这有助于更快、更好地学习。规范化还可以解决梯度减少和爆炸的问题。

## 在 PyTorch 中规范化图像

PyTorch 中的规范化使用 `torchvision.transforms.Normalize()` 完成。这用平均值和标准偏差来归一化张量图像。

> **语法:** `torchvision.transforms.Normalize()`
> 
> **参数:**
> 
> *   `mean`: 每个通道的平均值序列。
...
```py
# python code to load and visualize an image
...
使用 `ToTensor()` 将 PIL 图像转换为 PyTorch 张量，并绘制该张量图像的像素值。我们定义了我们的变换函数，将 PIL 图像转换成 PyTorch 张量图像。

```py
# Python code for converting PIL Image to PyTorch Tensor image and plot pixel values
...
### 使用 `torchvision.transforms.Normalize()` 对图像进行归一化
...
```py
# Python code to calculate mean and std of normalized image
