...
> 原文：[https://www.geeksforgeeks.org/how-to-set-up-and-run-cuda-operations-in-pytorch/](https://www.geeksforgeeks.org/how-to-set-up-and-run-cuda-operations-in-pytorch/)
...
首先，您应该通过官方的 NVIDIA CUDA 兼容性列表检查他们系统的 GPU，以确保他们的 GPU 是否启用了 CUDA。PyTorch 通过提供一个很好的用户友好的界面，让你选择你的操作系统和其他要求，使 CUDA 的安装过程非常简单，如下图所示。根据我们的计算机器，我们将按照下图中给出的规格进行安装。
...
> 2.  `torch.cuda.is_available()`：如果系统支持 CUDA，则返回 `True`，否则返回 `False`
> 3.  `torch.cuda.current_device()`：返回当前设备的 ID
> 4.  `torch.cuda.get_device_name(device_ID)`：返回 ID = `device_ID` 的 CUDA 设备的名称
...
为了演示上述功能，我们将创建一个测试张量，并执行以下操作：检查张量的当前设备并应用张量运算（平方），将张量传输到 GPU 并应用相同的张量运算（平方），并比较两个设备的结果。
...
# Checking the equality of the two results
...
> **语法：** `Model.to(device_name)`
>
> **返回：** 由 `device_name` 指定的设备上的机器学习 `Model` 的新实例：`"cpu"` 代表 CPU，`"cuda"` 代表启用 CUDA 的 GPU

在本例中，我们从 `torchvision.models` 实用程序中导入预训练的 **ResNet-18** 模型，读者可以使用相同的步骤将模型传输到他们选择的设备。
