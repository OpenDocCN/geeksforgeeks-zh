...
> 原文: [https://www.geeksforgeeks.org/implement-deep-autoencoder-in-pytorch-for-image-reconstruction/](https://www.geeksforgeeks.org/implement-deep-autoencoder-in-pytorch-for-image-reconstruction/)
...
> `pip3 install torch torchvision torchio numpy matplotlib`

现在进入最有趣的部分，代码。本文假设您基本熟悉 `PyTorch` 工作流及其各种实用程序，如 `数据加载器`、`数据集` 和 `张量` 转换。为了快速复习这些概念，鼓励读者阅读以下文章:

*   [使用 PyTorch 通过验证来训练神经网络](https://www.geeksforgeeks.org/training-neural-networks-with-validation-using-pytorch/)
*   [PyTorch 入门](https://www.geeksforgeeks.org/pytorch-tutorial-for-beginners/)
...
*   **初始化变换:** 首先，我们初始化变换，该变换将应用于所获得的数据集中的每个条目。由于张量是 Pytorch 的内部函数，我们首先将每个项目转换成张量，并对它们进行归一化，以将像素值限制在 0 和 1 之间。这样做是为了使优化过程更容易和更快。
*   **下载数据集:** 然后，我们使用 `torchvision.datasets` 实用程序下载数据集，并将其存储在本地机器上的文件夹 `./MNIST/train` 和 `./MNIST/test` 中用于训练和测试设备。我们还将这些数据集转换为批处理大小等于 256 的数据加载器，以加快学习速度。鼓励读者玩弄这些价值观，期待一致的结果。
...
在这一步中，我们初始化我们的 `DeepAutoencoder` 类，它是 `torch.nn.Module` 的一个子类。这为我们抽象出了很多样板代码，现在我们可以专注于构建我们的模型架构，如下所示:
