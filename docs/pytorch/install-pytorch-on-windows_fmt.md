...
在命令提示符下输入以下命令，检查 `python` 是否已经安装。
...
输入以下任一命令（根据您的系统规格）安装最新稳定版本的 PyTorch。
...
在命令提示符下输入以下命令，检查 PyTorch 是否安装成功。
...
在 Anaconda 提示符下输入以下命令，检查是否安装了 `conda`。
...
在管理员模式下打开 Anaconda 提示符，输入以下任一命令（根据您的系统规格）即可安装最新稳定版本的 PyTorch。
...
在 Anaconda 提示符下输入以下命令，检查 PyTorch 是否安装成功。
