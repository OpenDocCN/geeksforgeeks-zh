...
> 原文: [https://www.geeksforgeeks.org/find-determinant-of-a-complex-matrix-in-pytorch/](https://www.geeksforgeeks.org/find-determinant-of-a-complex-matrix-in-pytorch/)

`torch.linalg` 模块提供函数 `torch.linalg.det()` 计算正方形（实数或复数）矩阵的行列式。该函数还计算一批中每个方阵的行列式。
...
此功能支持浮点型、双精度、`cfloat` 和 `cdouble` 数据类型。

我们先来看看如何在 PyTorch 中创建一个复杂的矩阵，然后计算矩阵的行列式。为了在 PyTorch 中创建一个复杂矩阵，我们使用 **2D 复张量**。我们还可以创建一批复杂的矩阵。支持的复杂数据类型有 `torch.cfloat` 和 `torch.cdouble`。
...
在 PyTorch 中创建复杂矩阵的另一种方法是使用 `torch.complex(real, imag)`。这创建了一个复张量，其实部和虚部分别为 `real` 和 `imag`。无论是 `real` 还是 `imag` 都必须是浮点型或双精度型，`imag` 必须与 `real` 同类型。在下面的程序中，我们创建一个大小为 2×2 的 2D 复张量，并计算其行列式。
