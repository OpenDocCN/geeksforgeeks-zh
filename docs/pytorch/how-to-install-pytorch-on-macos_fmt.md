...
> 原文: [https://www.geeksforgeeks.org/how-to-install-pytorch-on-macos/](https://www.geeksforgeeks.org/how-to-install-pytorch-on-macos/)

**PyTorch** 是基于 Torch 库的开源机器学习库，用于计算机视觉和自然语言处理等应用，主要由脸书 AI Research 实验室开发。它是在修改后的 BSD 许可下发布的免费开源软件。

## 先决条件

*   [Python](https://www.geeksforgeeks.org/how-to-download-and-install-python-latest-version-on-macos-mac-os-x/)
*   应安装 [pip](https://www.geeksforgeeks.org/how-to-install-pip-in-macos/) 或 [Anaconda](https://www.geeksforgeeks.org/how-to-install-anaconda-on-linux/)。
...
**步骤 4:** 在命令提示符下输入以下命令，检查 Pytorch 是否安装成功。
...
如果这个命令成功运行，并且我们能够得到一个 `torch` 版本，那么我们可以走了，否则重新安装它。

### 方法二：使用 conda 安装需要的依赖项

**步骤 1:** 如果 `conda` 被停用，激活 `conda` 提示。
...
**步骤 4:** 在 Anaconda 提示符下输入以下命令，检查 Pytorch 是否安装成功。
