...
> 原文: [https://www.geeksforgeeks.org/change-view-of-tensor-in-pytorch/](https://www.geeksforgeeks.org/change-view-of-tensor-in-pytorch/)

在本文中，我们将改变 PyTorch 中给定张量的视图。为此，我们将使用 `view()` 函数来更改二维格式（即行和列）中的张量。我们必须指定要查看的行数和列数。

> 语法: `tensor.view(行数，列数)`

## 示例 1

Python 程序创建一个包含 10 个元素的张量，视图包含 5 行 2 列，反之亦然。
