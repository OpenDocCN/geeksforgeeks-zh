...
> 原文: [https://www.geeksforgeeks.org/implementing-an-autoencoder-in-pytorch/](https://www.geeksforgeeks.org/implementing-an-autoencoder-in-pytorch/)
...
我们将使用 `torch` 包中的 `torch.optim` 和 `torch.nn` 模块以及 `torchvision` 包中的 `datasets` 和 `transforms`。在本文中，我们将使用流行的 MNIST 数据集，该数据集包含 0 到 9 之间的手写单个数字的灰度图像。
