...
> 原文：[https://www.geeksforgeeks.org/adjusting-learning-rate-of-a-neural-network-in-pytorch/](https://www.geeksforgeeks.org/adjusting-learning-rate-of-a-neural-network-in-pytorch/)
...
每 `step_size` 个时期将学习速率乘以 `gamma`。例如，如果 `lr` = 0.1、`γ`= 0.1、`step_size` = 10，则在 10 个纪元之后，`lr` 变为 `lr * gamma`，在这种情况下为 0.01，在另外 10 个纪元之后，它变为 0.001。
...
这是 `StepLR` 的一个更定制的版本，在这个版本中，`LR` 在到达它的某个时代后被改变。在这里，我们提供了里程碑，即我们想要更新我们的学习速度的时代。
...
这是 `LR` 中 `StepLR` 的一个激进版本，在每个纪元之后都会改变。你可以把它想象成 `step_size= 1` 的 `StepLR`。
...
当指标停止改善时，减少学习速率。一旦学习停滞，模型通常会受益于将学习速率降低 2-10 倍。这个调度器读取一个度量值，如果对于一个 `patience` 数量的时期没有看到任何改进，那么学习速率降低。
...
既然我们已经准备好了 `DataLoader`，我们现在可以开始创建我们的模型了。PyTorch 模型遵循以下格式：
...
现在我们有了我们的模型，我们可以指定我们的优化器，损失函数和我们的 `lr_scheduler`。我们将使用 `SGD` 优化器，`CrossEntropyLoss` 用于损失函数，`ReduceLROnPlateau` 作为调度器。
