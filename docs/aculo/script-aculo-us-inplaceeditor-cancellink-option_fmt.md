...
> 原文: [https://www.geeksforgeeks.org/script-aculo-us-inplaceeditor-cancellink-option/](https://www.geeksforgeeks.org/script-aculo-us-inplaceeditor-cancellink-option/)

`script.aculo.us` 库是一个跨浏览器库，旨在改进网站的用户界面。`Ajax.InPlaceEditor` 用于使元素可编辑，从而允许用户编辑页面上的内容并将更改提交给服务器。

`cancelLink` 选项用于指定是否显示取消编辑并关闭编辑器的链接。默认值设置为 `true`。
...
**布尔值:** 这是一个布尔值，指定是否显示取消编辑并关闭编辑器的链接。默认值设置为 `true`。
