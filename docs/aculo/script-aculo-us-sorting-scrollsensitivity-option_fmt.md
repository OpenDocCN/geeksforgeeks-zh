# `script.aculo.us` 排序 `scrollSensitivity` 选项

> 原文: [https://www.geeksforgeeks.org/script-aculo-us-sorting-scrollsensitivity-option/](https://www.geeksforgeeks.org/script-aculo-us-sorting-scrollsensitivity-option/)

`script.aculo.us` 库是一个跨浏览器库，旨在改进网站的用户界面。可排序模块可用于使任何列表可排序，允许用户根据所需的顺序拖动任何项目。

`scrollSensitivity` 选项用于指定从滚动容器底部开始的像素数量，之后元素将开始滚动。它以像素为单位。
