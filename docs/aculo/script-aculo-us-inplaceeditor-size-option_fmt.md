# script.aculo.us InPlaceEditor 大小选项

> 原文: [https://www.geeksforgeeks.org/script-aculo-us-inplaceeditor-size-option/](https://www.geeksforgeeks.org/script-aculo-us-inplaceeditor-size-option/)

`script.aculo.us` 是一个跨浏览器库，旨在改善网站的用户界面。`Ajax.InPlaceEditor` 用于使元素可编辑，从而允许用户编辑页面上的内容并将更改提交给服务器。

位置编辑器 `size` 选项用于定义可编辑字段的宽度。
