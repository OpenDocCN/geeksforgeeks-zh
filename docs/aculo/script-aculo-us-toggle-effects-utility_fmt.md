...
> 原文：[https://www.geeksforgeeks.org/script-aculo-us-toggle-effects-utility/](https://www.geeksforgeeks.org/script-aculo-us-toggle-effects-utility/)

切换效果实用程序用于切换带有附加动画的元素。支持三个动画道具 `Appear`、`Blind` 和 `Slide`。

*   **Appear:** 模拟 `Appear` 效果模块隐藏和显示元素。
*   **Slide:** 模拟 `SlideUp`、`SlideDown` 效果模块进行隐藏和展示元素。
*   **Blind:** 模拟 `BlindUp` 和 `BlindDown` 效果模块进行隐藏和元素展示。
