...
InPlaceEditor 中的 `loadTextURL` 选项用于指定当可编辑元素在上交互时将从其加载文本的服务器的网址。当服务器上的文本被格式化并要求加载到可编辑元素中时，这很有用。
...
*   `URL`: 这是一个字符串，指定用于加载文本的网址。默认值为 `null`。

以下示例说明了该选项的使用。

## 示例

该示例演示了 InPlaceEditor `loadTextURL` 选项。
