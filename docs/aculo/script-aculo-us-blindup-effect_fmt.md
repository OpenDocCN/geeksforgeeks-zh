...
> 原文: [https://www.geeksforgeeks.org/script-aculo-us-blindup-effect/](https://www.geeksforgeeks.org/script-aculo-us-blindup-effect/)
...
*   **scaleY:** 布尔值，默认为 `true`。
*   **scaleContent:** 布尔值，默认为 `true`。
*   **scaleFromCenter:** 布尔值，默认为 `false`。
*   **scaleFrom:** 整数值默认为 `100`。
*   **scaleMode:** 用字符串值设置缩放模式，默认为 `'box'`。
*   **scaleTo:** 整数值默认为 `100`。

### 注意:

要使用这个库，我们应该安装这个库，然后在我们的程序中使用它。而要做到这一点，你可以顺着这个链接 `http://script.aculo.us/downloads`。

### 示例 1:

为了演示这种效果的使用，我们编写了一小段代码。其中我们编写了一个名为 `ShowEffect` 方法的小 JavaScript 函数，使用了这个库的 `BlindUp` 方法。通过点击**点击我来屏蔽这条线！**，你会看清楚效果。
