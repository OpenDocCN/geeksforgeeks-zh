...
> 原文: [https://www.geeksforgeeks.org/script-aculo-us-autocompleter-local-partialsearch-option/](https://www.geeksforgeeks.org/script-aculo-us-autocompleter-local-partialsearch-option/)
...
*   `布尔值`: 这是一个布尔值，指定文本是否只在字符串的开头匹配。默认值为 `假`。

## 示例 1:

在本例中，我们将 `partialSearch` 选项设置为 `false`，因此它将仅从字符串的开头开始搜索。
...
在本例中，我们将 `partialSearch` 选项设置为 `true`，因此它将从字符串中的任何位置进行搜索。
