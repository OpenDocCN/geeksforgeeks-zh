...
> 原文: [https://www.geeksforgeeks.org/how-to-get-names-of-all-colorscales-in-plotly-python/](https://www.geeksforgeeks.org/how-to-get-names-of-all-colorscales-in-plotly-python/)
...
`Aggrnyl` 和 `Aggrnyl_r` 的区别在于，它会显示相反的刻度，即 `Aggrnyl`(从亮到暗)`Aggrnyl_r`(从暗到亮)。为了在下面的例子中更清楚地理解它。
...
在本例中，我们在 `Plotly Express` 中选择色阶作为 `colorscale="Agsunset"`，这将从内置的 `Plotly` 库中选择 `Agsunset` 色阶。
...
在本例中，我们在 `Plotly Express` 中选择色标为 `colorscale="Agsunset_r"` (**r** 代表反转)，这将从内置的 `Plotly` 库中选择**反转** `Agsunset_r` 色标。两者之间唯一的区别是它将显示相反的比例，即 `Agsunset_r`(从暗到亮)和 `Agsunset`(从亮到暗)。
