...
**文本框:** 用于向/从用户显示和获取文本。文本可以使用任何键盘输入进行编辑，它以字符串的形式进行输入。它显示标题、要显示的消息、更改给定文本的位置，以及一对用于确认文本的“确定”、“取消”按钮。它类似于输入文本，但用于显示大文本，并对给定文本进行自动换行。下面是输入框的外观。
...
如果按下“确定”，则返回修改后的文本；如果按下“取消”，则返回`None`。
