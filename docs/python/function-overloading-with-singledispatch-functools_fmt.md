...
> 原文: [https://www.geeksforgeeks.org/function-overloading-with-singledispatch-functools/](https://www.geeksforgeeks.org/function-overloading-with-singledispatch-functools/)

## 介绍

`singledispatch`泛型函数装饰器将一个函数转换为泛型函数，根据其第一个参数的类型，泛型函数可以有不同的行为。修饰函数充当默认实现。要向函数添加重载实现，请使用泛型函数的`register()`属性。它是一个装饰器，接受一个类型参数并装饰一个实现该类型操作的函数。
