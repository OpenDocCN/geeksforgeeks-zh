...
> 原文：[https://www.geeksforgeeks.org/how-to-add-timestamp-to-excel-file-in-python/](https://www.geeksforgeeks.org/how-to-add-timestamp-to-excel-file-in-python/)

在本文中，我们将讨论如何使用 Python 向 Excel 文件添加时间戳。

## 所需模块

*   `datetime`：这个模块帮助我们用 Python 处理日期和时间。

```py
pip install datetime
```

*   `openpyxl`：是一个用来读写 Excel 文件的 Python 库。
...
`strftime()` 函数用于将日期和时间对象转换为它们的字符串表示。它接受一个或多个格式化代码输入，并返回字符串表示形式。
...
if __name__ == '__main__':
    ...
    # Save the workbook with a filename and close the object
    wb.save('gfg.xlsx')
    wb.close()
```
