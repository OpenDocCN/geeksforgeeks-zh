...
*   使用`*`，我们与`*`相关联的变量变成了可迭代的，这意味着您可以对它进行迭代，运行一些更高阶的函数如`map`和`filter`等。
...
Python 中函数定义中的特殊语法`**kwargs`用于传递关键字化的可变长度参数列表。我们把`kwargs`这个名字和双星一起使用。原因是因为双星允许我们传递关键字参数（以及任意数量的参数）。
