...
> 原文: [https://www.geeksforgeeks.org/deep-neural-network-with-l-layers/](https://www.geeksforgeeks.org/deep-neural-network-with-l-layers/)
...
*   图层 `i` 的线性输出表示为 `Zi`，激活后的输出表示为 `Ai`。`Zi` 和 `Ai` 的维度是一样的。
...
*   `L_model_forward` [LINEAR -> tanh](L-1 次) -> LINEAR -> SIGMOID（整体模型）

线性正向模块（在所有示例中矢量化）计算以下等式：
`Zi = Wi * A(I–1) + bi`
`Ai = activation_func(Zi)`
...
*   线性向后（计算任何层的线性输出 `Z` 的梯度）
*   线性激活向后，其中激活将是 `tanh` 或 `Sigmoid`。
*   `L_model_backward` [LINEAR -> tanh](L-1 次) -> LINEAR -> SIGMOID（全模型反向传播）

对于 `i` 层，线性部分是：`Zi = Wi * A(I–1) + bi`
表示 `dZi = dJ/dZi`，我们可以得到 `dWi`、`dbi` 和 `dA(I–1)` 为：
`dWi = dJ/dWi = (1/m) * dZi * transpose(A(i - 1))`
`dbi = dJ/dbi = (1/m) * sum(dZi)`
`dA(i - 1) = dJ/dA(i - 1) = transpose(Wi) * dZi`
...
这里我们将计算 `sigmoid` 和 `tanh` 函数的导数。[激活函数的理解推导代码](https://medium.com/@omkar.nallagoni/activation-functions-with-derivative-and-python-code-sigmoid-vs-tanh-vs-relu-44d23915c1f4)
