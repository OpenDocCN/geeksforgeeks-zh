...
先决条件: [`sum()`函数](https://www.geeksforgeeks.org/sum-function-python/)、[`len()`函数](https://www.geeksforgeeks.org/list-methods-in-python-set-1-in-not-in-len-min-max/)、[`round()`函数](https://www.geeksforgeeks.org/round-function-python/)、`reduce()`、`lambda`、和 [`mean()`](https://www.geeksforgeeks.org/statistical-functions-python-set-1averages-measure-central-location/)。
