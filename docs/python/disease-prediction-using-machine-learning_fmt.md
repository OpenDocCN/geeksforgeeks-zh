...
> 原文: [https://www.geeksforgeeks.org/disease-prediction-using-machine-learning/](https://www.geeksforgeeks.org/disease-prediction-using-machine-learning/)
...
从上面的图中，我们可以观察到数据集是一个平衡的数据集，即每种疾病正好有 120 个样本，不需要进一步平衡。我们可以注意到，我们的目标列，即预测列是对象数据类型，这种格式不适合训练机器学习模型。因此，我们将使用 `LabelEncoder` 将预测列转换为数字数据类型。`LabelEncoder` 通过为标签指定唯一的索引，将标签转换为数字形式。如果标签的总数是 `n`，那么分配给每个标签的数字将在 0 到 `n-1` 之间。
