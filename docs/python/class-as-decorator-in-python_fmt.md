...
我们可以将装饰器定义为类。为了做到这一点，我们必须使用类的 `__call__` 方法。当用户需要创建一个充当函数的对象时，函数装饰器需要返回一个充当函数的对象，因此 `__call__` 可能很有用。例如
...
为了打印执行程序所需的时间，我们使用 `__call__` 函数并使用一个 `time` 模块，这样就可以得到一个程序的执行时间。
