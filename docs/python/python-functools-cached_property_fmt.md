...
> 原文: [https://www.geeksforgeeks.org/python-functools-cached_property/](https://www.geeksforgeeks.org/python-functools-cached_property/)
...
您可以清楚地看到，使用 `@property`，每次调用 `increase()` 方法时都会计算结果的值，这就是它的值增加 50 的原因。但是使用 `@cached_property`，结果的值只计算一次，然后存储在缓存中，这样每次调用 `increase()` 方法都可以访问，而无需重新计算结果的值，这就是为什么输出每次显示 100。
...
虽然输出是相同的，但它大大减少了执行时间。在创建实例时传递的列表总和只计算一次，并存储在缓存中，此后，每次调用 `find_sum` 属性时，它都使用相同的、已经计算过的总和值，从而避免了对总和进行昂贵的重新计算。因此，它减少了执行时间，使我们的程序更快。