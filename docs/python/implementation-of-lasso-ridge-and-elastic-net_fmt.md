...
> 原文：[https://www.geeksforgeeks.org/implementation-of-lasso-ridge-and-elastic-net/](https://www.geeksforgeeks.org/implementation-of-lasso-ridge-and-elastic-net/)
...
我们将使用波士顿房屋预测数据集。该数据集存在于 `sklearn` (`scikit-learn`) 库的 `datasets` 模块中。我们可以按如下方式导入该数据集。
...
现在，我们应用 `train_test_split` 将数据集分成两部分，一部分用于训练，另一部分用于测试。我们将使用 25% 的数据进行测试。
...
![RidgeRegcoefficient](img/17598595453fe8f9aa0ad041c0c38f1e.png)

上述数据的柱状图为：

[![ridgeatAlpha1](img/818bdd30d9afeb54c3b2b9572bcc1c18.png)](https://media.geeksforgeeks.org/wp-content/uploads/20200228165206/rigdgeatAlpha1.png)
...
我们来看看另一个带有 ![\alpha ](img/11c7b3b7c0597881c15e1cb95b4e74ce.png "Rendered by QuickLaTeX.com") = 10 的柱状图
...
套索回归类似于岭回归，只是这里我们用系数的平均绝对值代替了均方值。与岭回归不同，套索回归可以通过将其系数值降低到 0 来完全消除变量。我们给普通最小二乘（OLS）增加的新术语叫做 **L<sub>1</sub>** 正则化。
...
当我们增加 ![\alpha ](img/11c7b3b7c0597881c15e1cb95b4e74ce.png "Rendered by QuickLaTeX.com") 的值时，套索回归给出了与岭回归相同的结果。再来看看 ![\alpha ](img/11c7b3b7c0597881c15e1cb95b4e74ce.png "Rendered by QuickLaTeX.com") = 10 的另一个图。

![](img/69f5b6dcfa51badacf1053726b068954.png)

## 弹性网

在弹性网正则化中，我们加入了 L<sub>1</sub> 和 L<sub>2</sub> 两个项，得到最终的损失函数。这就导致我们减少了以下损失函数：
...
，其中 ![ \alpha ](img/11c7b3b7c0597881c15e1cb95b4e74ce.png "Rendered by QuickLaTeX.com") 在 0 和 1 之间。当 ![ \alpha ](img/11c7b3b7c0597881c15e1cb95b4e74ce.png "Rendered by QuickLaTeX.com") = 1 时，将罚项减为 L<sub>1</sub> 罚项，若 ![ \alpha ](img/11c7b3b7c0597881c15e1cb95b4e74ce.png "Rendered by QuickLaTeX.com") = 0，将罚项减为 L<sub>2</sub> 罚项。
...
# Calculate Mean Squared Error
mean_squared_error_en = np.mean((y_pred_en - y_test)**2)
...
elastic_coeff['Coefficient Estimate'] = pd.Series(elastic_net.coef_)
...
*   岭回归在损失函数中加入 `L2` 正则化惩罚项。这个项减少了系数，但没有使它们为 0，因此没有完全消除任何独立变量。它可以用来衡量不同自变量的影响。

*   Lasso 回归将 `L1` 正则化惩罚项加入损失函数。这一项减少了系数，使其为 0，从而有效地完全消除了相应的自变量。它可以用于特征选择等。

*   弹性网是上述两种正则化的组合。它包含 `L1` 和 `L2` 作为其惩罚项。在大多数测试用例中，它的性能优于岭回归和套索回归。