...
> 原文: [https://www.geeksforgeeks.org/data-science-apps-using-streamlit/](https://www.geeksforgeeks.org/data-science-apps-using-streamlit/)
...
*   `streamlit.header()/streamlit.subheader()`: 这些功能用于设置一个节的头/子头。这些功能也支持降价。
...
*   流线库中也有常见的 UI 小部件，如 `streamlit.button()`、`streamlit.checkbox()`、`streamlit.radio()` 等。
...
我们将使用流线缓存功能来缓存我们的数据，因此当我们再次重新运行时，我们不需要再次加载数据。之后，我们使用这些数据绘制不同的图和地图。以下是该文件的完整代码。
...
现在，将此代码保存为`app.py`，并将此命令运行为: `streamlit run app.py`。这将打开一个浏览器窗口，加载应用。让我们来看看这个应用的截图。
