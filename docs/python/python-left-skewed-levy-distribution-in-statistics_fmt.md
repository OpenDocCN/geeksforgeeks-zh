...
> 原文: [https://www.geeksforgeeks.org/python-left-skewed-levy-distribution-in-statistics/](https://www.geeksforgeeks.org/python-left-skewed-levy-distribution-in-statistics/)

`scipy.stats.levy_l()` 是左偏 levy 连续随机变量。它继承自泛型方法的，作为 `rv_continuous` 类的实例。它用特定于这个特定分布的细节来完成这些方法。
...
> `moments` : 【可选】由字母['mvsk']组成；'m'=均值，'v'=方差，'s'= Fisher 偏斜度，'k'= Fisher 峰度。(默认值= 'mv')。
>
> **结果:** 左偏 Levy 连续随机变量
