...
> 原文：[https://www.geeksforgeeks.org/how-to-create-an-empty-pygame-window/](https://www.geeksforgeeks.org/how-to-create-an-empty-pygame-window/)
...
**第二步：** 初始化一个窗口显示。我们用 `set_mode()` 函数创建一个窗口。我们将窗口的宽度和高度作为参数传递给 `set_mode()` 函数。

```py
pygame.display.set_mode((width_of_window,height_of_window))
```

**步骤 3：** 保持该窗口运行，直到用户按下退出按钮。我们使用的变量 `running` 是 `True`，除非用户按下退出按钮。为了保持游戏运行，我们使用 `while` 循环来检查变量是否为真。
