...
> 原文：[https://www.geeksforgeeks.org/how-to-not-get-caught-while-web-scraping/](https://www.geeksforgeeks.org/how-to-not-get-caught-while-web-scraping/)
...
*   创建一个代理服务器池，并循环或迭代它们。
*   导入 `requests` 模块，并与代理一起向 `https://www.geeksforgeeks.org/` 发送 `GET` 请求。
...
*   一些网站需要一个主要浏览器的用户代理，否则它将不允许查看内容，所以最好的方法是创建一个虚假用户代理列表并迭代它们，或者使用来自 `fake_useragent` 模块的 `UserAgent`，并在请求网站时将其作为标题传递。
...
*   如果你是从谷歌来的，在发送 GET 请求时，在 `referer` 头中提供 `referer_url`。
...
*   检测某个链接是否设置了 `display: none` 或 `visibility: hidden` CSS 属性，应避免跟随该链接，否则会将你识别为刮刀。蜜罐是智能站长检测爬虫并阻止用户所有请求的最简单方法之一。
