...
在 Python 中，日期和时间不是它们自己的数据类型，但是可以导入名为 `datetime` 的模块来处理日期和时间。`Python Datetime` 模块内置于 Python 中，无需外接安装。
...
*   [**日期时间**](https://www.geeksforgeeks.org/python-datetime-datetime-class/) – 它是日期和时间以及年、月、日、时、分、秒、微秒和 `tzinfo` 属性的组合。
...
**注意** – 如果参数不是整数，它将引发 `TypeError`，如果超出范围，将引发 `ValueError`。

### 示例 1：Python 中表示日期的日期对象
...
我们可以使用 `date` 类的 `year`、`month` 和 `day` 属性从 `date` 对象中获取年、月和日期属性。
...
我们可以使用 `fromtimestamp()` 方法从时间戳创建日期对象。时间戳是从世界协调时 1970 年 1 月 1 日到特定日期的秒数。
...
| --- | --- |
| `ctime()` | 返回表示日期的字符串 |
| `fromisocalendar()` | 返回与国际标准化组织日历对应的日期 |
| `fromisoformat()` | 从日期的字符串表示形式返回日期对象 |
| `fromordinal()` | 从公历序数返回日期对象，其中第 1 年的 1 月 1 日具有序数 1 |
| [`fromtimestamp()`](https://www.geeksforgeeks.org/fromtimestamp-function-of-datetime-date-class-in-python/) | 从 POSIX 时间戳返回一个日期对象 |
| `isocalendar()` | 返回一个元组年、周和工作日 |
| `isoformat()` | 返回日期的字符串表示形式 |
| `isoweekday()` | 以整数形式返回一周中的某一天，其中周一为 1，周日为 7 |
| `replace()` | 使用给定参数更改日期对象的值 |
| `strftime()` | 以给定格式返回日期的字符串表示形式 |
| `timetuple()` | 返回 `time.struct_time` 类型的对象 |
| `today()` | 返回当前本地日期 |
| `toordinal()` | 返回日期的公历序数，其中第 1 年的 1 月 1 日具有序数 1 |
| `weekday()` | 以整数形式返回一周中的某一天，其中周一为 0，周日为 6 |
...
| 函数名 | 描述 |
| --- | --- |
| `dst()` | 返回 `tzinfo.dst()`。`tzinfo` 不是 `None` |
| `fromisoformat()` | 从时间的字符串表示形式返回时间对象 |
| `isoformat()` | 从 `time` 对象返回时间的字符串表示形式 |
| `replace()` | 使用给定参数更改时间对象的值 |
| `strftime()` | 以给定格式返回时间的字符串表示形式 |
| `tzname()` | 返回 `tzinfo.tzname()`。`tzinfo` 不是 `None` |
| `utcoffset()` | 返回 `tzinfo.utcoffset()`。如果 `tzinfo` 不是 `None` |
...
`year`、`month` 和 `day` 参数是必需的。`tzinfo` 可以是 `None`，其余所有属性必须是以下范围内的整数：
...
我们可以在 Python 中使用 `datetime.strftime()` 和 `datetime.isoformat()` 方法将 `datetime` 转换为字符串。
...
| `combine()` | 组合日期和时间对象，并返回一个日期时间对象 |
| `ctime()` | 返回日期和时间的字符串表示形式 |
| `date()` | 返回日期类对象 |
| `fromisoformat()` | 从日期和时间的字符串表示形式返回 `datetime` 对象 |
| `fromordinal()` | 从公历序数返回日期对象，其中第 1 年的 1 月 1 日具有序数 1。小时、分钟、秒和微秒是 0 |
| `fromtimestamp()` | 从 POSIX 时间戳返回日期和时间 |
| `isocalendar()` | 返回一个元组年、周和工作日 |
| `isoformat()` | 返回日期和时间的字符串表示 |
| `isoweekday()` | 以整数形式返回一周中的某一天，其中周一为 1，周日为 7 |
| `now()` | 使用 `tz` 参数返回当前本地日期和时间 |
| `replace()` | 更改日期时间对象的特定属性 |
| `strftime()` | 以给定格式返回日期时间对象的字符串表示形式 |
| `strptime()` | 返回与日期字符串对应的日期时间对象 |
| `time()` | 返回时间类对象 |
| `timetuple()` | 返回 `time.struct_time` 类型的对象 |
| `timetz()` | 返回时间类对象 |
| `today()` | 返回本地日期时间，`tzinfo` 为无 |
| `toordinal()` | 返回日期的公历序数，其中第 1 年的 1 月 1 日具有序数 1 |
| `tzname()` | 返回时区的名称 |
...
Python `timedelta` 类用于计算日期差异，也可以用于 Python 中的日期操作。这是执行日期操作最简单的方法之一。
...
| `//` (地板除) | 用 `float` 或 `int` 除 `timedelta` 对象，并返回输出的整数值 |
| `%` (模) | 将两个时间增量对象相除，并返回余数 |
| `+` (一元加) | 返回相同的时间增量对象 |
| `-` (一元减) | 返回 `-1 * timedelta` 的结果 |
| `abs()` | 如果 `timedelta.days > 1=0`，则返回 `+(timedelta)`，否则返回 `-(timedelta)` |
| `str()` | 返回一个格式为 `(+/-) days, HH:MM:SS.uuuuuu` 的字符串 |
| `repr()` | 返回构造函数调用形式的字符串表示形式 |

## 格式化日期时间

格式化 `datetime` 可能是非常必要的，因为日期表示可能因地而异。像在一些国家，它可以是 `yyyy-mm-dd`，在其他国家，它可以是 `DD-mm-yyy`。要格式化 Python `datetime`，可以使用 `strftime()` 和 `strptime()` 函数。
...
**注:** 详见 [`strftime()` 方法](https://www.geeksforgeeks.org/python-strftime-function/)。
...
`datetime` 中的时区可用于希望根据特定区域的时区显示时间的情况。这可以使用 Python 的 [`pytz` 模块](https://www.geeksforgeeks.org/python-pytz/)来完成。该模块提供日期时间转换功能，并帮助用户服务于国际客户群。
