...
> 原文: [https://www.geeksforgeeks.org/python-assign-range-of-elements-to-list/](https://www.geeksforgeeks.org/python-assign-range-of-elements-to-list/)

给列表分配元素是一个常见的问题，在前面的文章中已经讨论了它的许多变种。这篇特别的文章讨论了列表中元素范围的插入。让我们讨论一下解决这个问题的某些方法。

## 方法#1: 使用`extend()`

这可以使用`extend()`函数来解决，在`extend()`函数中，可以使用`range()`函数在后端插入数字范围。
