...
> 原文：[https://www.geeksforgeeks.org/interesting-facts-about-strings-in-python-set-2/](https://www.geeksforgeeks.org/interesting-facts-about-strings-in-python-set-2/)
...
因此，我们可以使用语法获得所需的字符，`string_name[index_position]`：
...
*   `beginning` 表示字符串的起始索引。
*   `end` 表示字符串的结束索引（不包含该位置）。
*   `step` 表示两个字符之间的步长。

注意：我们也可以使用 `beginning` 和 `end` 或仅和 `step` 对字符串进行切片，这是可选的。
