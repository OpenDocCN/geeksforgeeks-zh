...
> 原文: [https://www.geeksforgeeks.org/how-to-flip-an-image-horizontally-or-vertically-in-python/](https://www.geeksforgeeks.org/how-to-flip-an-image-horizontally-or-vertically-in-python/)

**先决条件:** `PIL`

给定一个图像，这里的任务是生成一个 Python 脚本来水平和垂直翻转图像。这里用于任务的模块是 `PIL` 和 `transpose()` 函数的这个模块。
...
关键词 `FLIP_TOP_BOTTOM` 和 `FLIP_LEFT_RIGHT` 将传递给 `transpose` 方法进行翻转。
