...
> 原文: [https://www.geeksforgeeks.org/looping-through-buttons-in-tkinter/](https://www.geeksforgeeks.org/looping-through-buttons-in-tkinter/)
...
## 步骤 1
导入 Tkinter 包及其所有模块，并创建一个根窗口(`root = Tk()`)。
...
## 步骤 2
现在我们添加一个 `Entry()` 类，只要点击其中一个按钮，就会显示按钮名称。
...
## 步骤 3
现在我们创建一个空字典(`button_dict`)来保存所有的按钮对象和一个包含所有按钮名称的列表。现在循环列表中的每一项，创建一个[按钮](https://www.geeksforgeeks.org/python-creating-a-button-in-tkinter/)对象，并将其存储在字典中。对于按钮命令，创建一个名为“action”的函数，对于每个按钮，调用 `text_update()` 函数来更新前面创建的 `entry()` 对象中条目的更改。
