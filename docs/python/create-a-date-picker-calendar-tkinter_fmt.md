...
> 原文：[https://www.geeksforgeeks.org/create-a-date-picker-calendar-tkinter/](https://www.geeksforgeeks.org/create-a-date-picker-calendar-tkinter/)
...
*   首先，我们将导入所需的库
*   然后我们将创建一个 `Calendar` 对象并传递默认日期
*   从日历中选择年、月和日期
*   要获取所选日期值的值，请使用 `get()` 方法。

> **语法：** `Calendar(master=None, **kw)`
>
> **`year`：** `int`
>
> *   它最初显示年份，默认是当前年份。
