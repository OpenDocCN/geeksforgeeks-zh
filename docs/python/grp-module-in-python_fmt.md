...
Python 中的 `grp` 模块提供了对 Unix 组数据库的访问。 Unix 组数据库的每个条目被报告为一个类似元组的对象，其属性类似于组结构的成员，该组结构在 [<grp.h> 头](http://man7.org/linux/man-pages/man0/grp.h.0p.html)中定义。

以下是元组状对象的属性，表示存储在 Unix 组数据库中的条目:
...
**注意:** `grp` 模块是 UNIX 特有的服务。因此，本模块的所有方法仅在 UNIX 版本上可用。

Python 中的 `grp` 模块定义了以下方法:
...
Python 中的 `grp.getgrgid()` 方法用于获取存储在 UNIX 组数据库中的指定组 id 的条目。如果指定的组 id 无效或找不到与之关联的条目，则会引发 `KeyError` 异常。
...
>
> **返回类型:** 这个方法返回一个类 `grp.struct_group` 的类似元组的对象，它代表与指定组 id (`gid`)相关联的组数据库条目。
...
Python 中的 `grp.getgrnam()` 方法用于获取存储在 UNIX 组数据库中的指定组名的条目。如果指定的组名无效或找不到与之关联的条目，则会引发 `KeyError` 异常。

> **语法:** `grp.getgrnam(name)`
>
> **参数:**
> **name:** 表示需要组数据库条目的组名的字符串值。
>
> **返回类型:** 这个方法返回一个类 `grp.struct_group` 的类似元组的对象，它代表与指定组名相关联的组数据库条目。
...
Python 中的 `grp.getgrall()` 方法用于获取存储在 UNIX 组数据库中的所有可用条目。

> **语法:** `grp.getgrall()`
>
> **参数:** 不需要参数。
>
> **返回类型:** 这个方法返回一个类 `grp.struct_group` 的类似元组的对象列表，其元素代表组数据库条目。
...
```
`struct_group(gr_name=’bluetooth’, gr_passwd=’x’, gr_gid=116, gr_mem=[])`
