...
> 原文: [https://www.geeksforgeeks.org/guide-to-install-turicreate-in-python3-x/](https://www.geeksforgeeks.org/guide-to-install-turicreate-in-python3-x/)
...
或者，如果您使用的是 Anaconda ([https://www.anaconda.com/](https://www.anaconda.com/))，您可以使用它的虚拟环境:

```py
# creating a virtual environment using Anaconda.
conda create -n <virtual-environment-name> anaconda
```
...
在 Windows 中，不能直接安装这个包，但是可以使用 Windows Linux 子系统 (WSL) 在系统中安装 turicreate。
