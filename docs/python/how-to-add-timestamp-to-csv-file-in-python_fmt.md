...
> 原文: [https://www.geeksforgeeks.org/how-to-add-timestamp-to-csv-file-in-python/](https://www.geeksforgeeks.org/how-to-add-timestamp-to-csv-file-in-python/)
...
*   使用 `open()` 函数以读写模式（`'r+'`）打开 CSV 文件。
    *   `open()` 函数的作用是：打开一个文件，并将其作为文件对象返回。
    *   `newline = ' '` 控制通用换行符模式的工作方式。
    *   `csv.writer()` 返回一个 writer 对象，该对象负责将用户数据转换为分隔字符串。
*   使用 `datetime` 模块的 `datetime.now()` 函数获取当前日期和时间。
*   在 `for` 循环的帮助下迭代 `rows` 变量中的所有数据。
...
使用 `for` 循环将存储在第一个文件中的每个数据追加到 `rows` 变量中。使用 `csv` 模块的 `writer()` 函数创建第二个文件的 writer 对象。现在使用 `for` 循环遍历 `rows` 变量中的所有数据。将当前日期和时间存储在变量中，然后使用 `insert()` 函数将其插入第 0 个索引处的数据中。使用 `csv` 模块的 `writerow()` 函数将存储的数据写入文件 2。
