...
*   每个线程都有一个目标函数 `thread_task` ，其中`increment`函数被调用 100000 次。
*   `increment`函数将在每次调用中将全局变量 `x` 递增 1。

`x` 的预计最终值是 200000，但是我们在 `main_task` 函数的 10 次迭代中得到的是一些不同的值。

这是由于线程对共享变量 `x` 的并发访问造成的。 `x` 的不可预测性只不过是`比赛条件`。
...
`threading`模块提供了一个`Lock`类来处理比赛情况。使用操作系统提供的`信号量`对象来实现锁定。
...
*   `acquire([blocking])`:获取锁。锁可以是阻塞的，也可以是非阻塞的。
    *   当阻塞参数设置为`True`(默认值)时调用，线程执行被阻塞，直到锁被解锁，然后锁被设置为锁定并返回`True`。
    *   当阻塞参数设置为`False`时调用，线程执行不会被阻塞。如果锁未锁定，则设置为锁定并立即返回`True`否则返回`False`。
*   `release()`:释放锁定。
    *   当锁被锁定时，将其重置为解锁状态，然后返回。如果任何其他线程在等待锁解锁时被阻止，请允许其中一个线程继续。
    *   如果锁已经解锁，则会出现`ThreadError`。
...
*   首先，使用`lock = threading.Lock()`创建一个`Lock`对象。
*   然后，`Lock`作为目标函数参数传递:
    ```py
    t1 = threading.Thread(target=thread_task, args=(lock,))
    t2 = threading.Thread(target=thread_task, args=(lock,))
    ```
*   在目标函数的临界区中，我们使用`lock.acquire()`方法应用锁。一旦锁被获取，其他线程就无法访问临界区（这里是`increment`函数），直到使用`lock.release()`方法释放锁。
    ```py
    lock.acquire()
    increment()
    lock.release()
...
这就把我们带到了这个关于 Python 中`多线程`的教程系列的结尾。
