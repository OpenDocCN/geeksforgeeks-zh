...
> 原文: [https://www.geeksforgeeks.org/absolute-and-relative-imports-in-python/](https://www.geeksforgeeks.org/absolute-and-relative-imports-in-python/)

## Python 中 `import` 的工作原理
...
当一个模块被导入时，解释器首先在 `sys.modules` 中搜索它，`sys.modules` 是所有先前已经导入的模块的缓存。如果没有找到，那么它会搜索所有具有该名称的内置模块，如果找到了，那么解释器会运行所有代码并提供给文件。如果找不到该模块，它将在变量 `sys.path` 给出的目录列表中搜索同名文件。
...
这里有一个名为 project 的目录，下面有两个子目录即 `pkg1`、`pkg2`。`pkg1` 有两个模块，`module1` 和 `module2`。
`pkg2` 包含三个模块，`module3`、`module4`、`__init__.py` 和一个包含 `module5.py` 的子包名 `subpkg1`。
...
相对导入指定从其当前位置导入的对象或模块，即导入语句所在的位置。相对导入有两种类型:
