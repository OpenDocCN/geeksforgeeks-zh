...
> 原文: [https://www.geeksforgeeks.org/how-to-extract-image-information-from-youtube-playlist-using-python/](https://www.geeksforgeeks.org/how-to-extract-image-information-from-youtube-playlist-using-python/)

## 先决条件

[YouTube API](https://www.geeksforgeeks.org/youtube-data-api-set-1/)
...
*   在 `list()` 方法中，在 `part` 属性中传递 `snippet`，在 `playlistId` 中传递目标播放列表的 ID。
*   在 `maxResults` 和 `pageToken` 中传递 50 值，最初传递 `None` 值。
