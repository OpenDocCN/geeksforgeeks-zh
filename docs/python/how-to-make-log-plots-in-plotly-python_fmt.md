...
没有提供用于创建日志图的特定功能。但是，可以使用 `graph_objects` 类的 [`scatter()`](https://www.geeksforgeeks.org/scatter-plot-in-plotly-using-graph_objects-class/) 方法创建。我们将不得不改变 `xaxis_type` 和 `yaxis_type` 来记录。
