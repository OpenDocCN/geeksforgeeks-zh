...
> 原文: [https://www.geeksforgeeks.org/python-categorize-tuple-values-into-dictionary-value-list/](https://www.geeksforgeeks.org/python-categorize-tuple-values-into-dictionary-value-list/)

有时，在使用 Python 时，我们可能会面临这样一个问题，即我们有元组列表形式的数据，我们打算用元组值列表将它们分类到字典中。让我们讨论执行这项任务的某些方法。

## 方法一：使用 `setdefault()` + 循环

使用 `setdefault()` 可以轻松执行该任务。在这种情况下，我们只需通过迭代获取元组的键和值对，`setdefault()` 用于为字典的相应键赋值。
