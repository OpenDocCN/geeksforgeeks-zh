...
Python3 中的 `print()` 函数支持一个 `file` 参数，该参数指定函数应该将给定的对象写到哪里。如果没有明确说明，默认为 `sys.stdout`。
...
将 `file` 参数指定为 `sys.stderr`，而不是默认值。这在调试小程序时非常有用（在其他情况下最好使用调试器）。
...
用所需文件的名称指定 `file` 参数，而不是默认值。如果文件不存在，将创建一个同名的新文件并写入其中。
