...
> 原文：[https://www.geeksforgeeks.org/how-to-install-python-nmap-library-in-linux/](https://www.geeksforgeeks.org/how-to-install-python-nmap-library-in-linux/)
...
第一种方法是用 `python` 编写脚本，并使用以下命令在终端上运行。
...
第二种方法涉及通过命令行界面测试 `Nmap` 方法。要实现这一点，打开终端并运行命令 `python`，这样您就可以获得如下所示的界面。
