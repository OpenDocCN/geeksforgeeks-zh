...
> 原文: [https://www.geeksforgeeks.org/python-check-whether-two-lists-circularly-identical/](https://www.geeksforgeeks.org/python-check-whether-two-lists-circularly-identical/)
...
使用遍历，我们必须给定列表加倍。检查任意 `x` (0 <= n) 到任意 `x+n`，并与列表 2 进行比较，查看列表 1 和列表 2 是否相同，如果两者相同，则列表 2 循环相同。使用两个循环，检查这个属性。第一个循环将从 0 运行到 `len(list1)`，然后检查索引(`x` 到 `x+n`)是否与 `list2` 相同，如果是，则返回 `true`，否则返回 `false`。
...
使用 Python 的内置函数 `map()` 我们可以一步完成，在这里我们必须将 `list2` 映射到一个字符串中，然后查看它是否存在于 `list1` (2*`list1`)在另一个字符串中的两次映射中。
