...
> 原文: [https://www.geeksforgeeks.org/python-binary-group-tuple-list-elements/](https://www.geeksforgeeks.org/python-binary-group-tuple-list-elements/)

有时，在处理元组时，我们可能会遇到分组问题，无论是基于性别还是任何特定的二元类别。这在许多领域都有应用。让我们讨论一下实现这一点的某些方法。

## 方法#1: 使用生成器+循环+`zip()`

蛮力方法执行此任务，在此，我们借助`zip()`执行组合分组，迭代逻辑由生成器和循环处理。
