...
> 原文：[https://www.geeksforgeeks.org/draw-indian-flag-using-matlab/](https://www.geeksforgeeks.org/draw-indian-flag-using-matlab/)

在数字图像处理中，彩色图像以三维矩阵表示。图像可以用各种颜色模型来表示，例如 RGB（红、绿、蓝）模型、HSV（色调、饱和度、值）模型、YIQ（亮度-同相正交）模型、CMYK（青色、品红色、黄色、黑色）模型。通常，图像以 RGB 模型表示。矩阵的第一个通道是红色，第二个通道是绿色，第三个通道是蓝色。
