...
`Python SQLite3 模块`用于将 SQLite 数据库与 Python 集成。它是一个标准化的 Python DBI API 2.0，为与 SQLite 数据库交互提供了一个简单明了的界面。不需要单独安装这个模块，因为它是在 2.5x 版本之后与 Python 一起提供的。
...
*   [Python–`json.dump()`和`json.dumps()`的区别](https://www.geeksforgeeks.org/python-difference-between-json-dump-and-json-dumps/)
...
*   [`json.load()`和`json.loads()`的区别](https://www.geeksforgeeks.org/python-difference-between-json-load-and-json-loads/)
...
*   [使用 `PyMongo` 获取集合索引的所有信息](https://www.geeksforgeeks.org/get-all-the-information-of-a-collections-indexes-using-pymongo/)
