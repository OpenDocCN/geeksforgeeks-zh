...
*   **创建命令:**在解释器提示符下输入的一行文本的第一部分是命令。`identchars` 成员中包含的最长字符串是 Command。非重音字母、数字和下划线符号是默认识别字符。行尾是命令的参数。
...
*   **返回值:** 在最常见的情况下:命令不应返回值。当你想退出解释器循环时，任何返回真值的命令都会停止解释器，这是一个例外。
...
`cmdparser` 包包含两个用于编写文本命令解析器的模块。这个模块特别使用了内置的 Python `cmd` 模块。该软件包由两个模块组成:
...
*   **Subtree:** 它匹配整个命令子树，并将结果与指定的令牌一起存储在 `fields` 字典中。命令规范字符串应传递给构造函数，类型类将覆盖 `convert()` 方法并以某种方式解释命令（尽管这完全是可选的）。

装饰器用于从 `cmd.Cmd` 派生的命令处理程序，它允许从文档字符串帮助文本中自动提取命令字符串，并允许将命令解析和完成添加到类的命令处理方法中。实现了 `do_XXX()` 形式的各种方法来实现 `cmd.Cmd` 类。
...
`Datetimeparse` 模块添加了特定的令牌类型来解析人类可读的日期和时间规范。指定了绝对和相对两种类型的日期，并根据需要将其转换为其他实例。一些例子是
...
*   **时间树:**12 或 24 小时格式的时间由 `TimeSubtree` 解析。返回值如 `time.localtime()` 返回。
*   **RelativeTimeSubtree:** 返回值是 `cmdparser.DateDelta` 的一个实例，它是一个包含 `datetime.timedelta` 的包装类。它解析表示当前时间偏移的短语，例如 3 天和 2 小时前。
*   **DateTimeSubstree:** `datetime.datetime` 实例是返回值。分析日期和时间的规范，接受 `DateSubtree` 和 `TimeSubtree` 短语的组合，或者接受 `RelativeTimeSubtree` 短语；在后一种情况下，时间是相对于当前时间而言的。
