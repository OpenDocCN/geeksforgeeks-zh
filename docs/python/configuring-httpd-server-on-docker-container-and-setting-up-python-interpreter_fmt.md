...
> 原文：[https://www.geeksforgeeks.org/configuring-httpd-server-on-docker-container-and-setting-up-python-interpreter/](https://www.geeksforgeeks.org/configuring-httpd-server-on-docker-container-and-setting-up-python-interpreter/)
...
*   要进入已退出的 docker 容器，我们有一个命令 `docker attach task` 这里 `task` 是你在启动容器时给出的名称，你可以根据需要给出任何名称。在进入容器之前，您必须首先通过以下命令启动停止的容器:
...
*   命令: `apt-get install net-tools` 将帮助您运行 `ifconfig` 命令。
...
*   现在通过命令启动服务， `systemctl start apache2` 并检查状态，确保其处于活动状态。
