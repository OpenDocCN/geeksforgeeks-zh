# 如何用 Python 中的 Altair 制作简单的直方图？

> 原文: [https://www.geeksforgeeks.org/how-to-make-a-simple-histogram-with-altair-in-python/](https://www.geeksforgeeks.org/how-to-make-a-simple-histogram-with-altair-in-python/)

**先决条件:** [`altair`](https://www.geeksforgeeks.org/introduction-to-altair-in-python/)

简单直方图是用矩形表示频率分布，矩形的宽度代表类别间隔。直方图是将分组数据点组织到指定范围的图形表示。通过使用直方图，我们可以将大量数据及其频率可视化为一个连续的图。

## 安装

为了安装 `altair` 库和 `vega_datasets`，我们在命令提示符下运行以下命令。
...
在这篇文章中，我们将在 `altair` 程序库的帮助下，通过使用下面不同例子中的 `cars` 和 `iris` 数据集来绘制简单直方图。
...
*   创建或加载数据集。
*   从数据集中选择要制作直方图的列。
*   为了制作直方图，在 `altair` 库中我们必须给出三个重要的元素 (`alt.Chart()`, `mark_bar()`, `encode()`)。
*   把它放在变量中，让它被命名为 `hist`。
*   然后为了看到这个情节，我们必须写一行代码 `hist.show()`，我们就完成了。

> **语法:**
> `alt.Chart(dataset_name).mark_bar().encode(x="name_of_col1", y="name_of_col2")`
...
## 示例 3: 通过将 `bin` 设置为 `Acceleration`，使用 `cars` 数据集制作简单直方图
...
## 示例 4: 通过将 `bin` 设置为 `Horsepower`，使用 `cars` 数据集制作简单直方图
...
## 示例 5: 在 `sepalLength` 上使用 `iris` 数据集制作默认的简单直方图
...
加载 `iris` 数据集后，我们还可以在程序中使用 `print(iris_data)` 打印 `iris` 数据集。我们的 `iris` 数据集打印后是这样的。

![](img/b594ebe693e953e37f6a85ad12ebaa3e.png)

## 例 6: 通过设置 `bin` 和 `color` 制作简单直方图
...
## 示例 7: 通过设置 `bin` 和 `color`，在 `petalWidth` 上使用 `iris` 数据集制作简单直方图
