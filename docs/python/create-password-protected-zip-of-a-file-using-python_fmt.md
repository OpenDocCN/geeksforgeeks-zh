...
> 原文：[https://www.geeksforgeeks.org/create-password-protected-zip-of-a-file-using-python/](https://www.geeksforgeeks.org/create-password-protected-zip-of-a-file-using-python/)
...
>
> *   `src` 文件路径（字符串）
> *   `src` 文件前缀路径（字符串）或 `None`（添加到文件前的路径）
> *   `dst` 文件路径（字符串）
> *   `密码`（字符串）或 `None`（创建无密码 zip）
