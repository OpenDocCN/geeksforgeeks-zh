...
> 原文: [https://www.geeksforgeeks.org/python-decimal-to-binary-list-conversion/](https://www.geeksforgeeks.org/python-decimal-to-binary-list-conversion/)
...
## 方法一：使用列表理解+ `format()`
在该方法中，十进制到二进制的转换由`format()`函数处理。转换到列表的逻辑由列表理解功能完成。
...
## 方法二：使用`bin()` + 列表理解
内置的`bin()`函数执行转换为二进制的功能，列表理解处理将二进制数转换为列表的逻辑。
