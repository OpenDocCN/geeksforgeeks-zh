...
> 原文: [https://www.geeksforgeeks.org/how-to-concatenate-column-values-of-a-mysql-table-using-python/](https://www.geeksforgeeks.org/how-to-concatenate-column-values-of-a-mysql-table-using-python/)
...
> **语法:** `SELECT CONCAT(column_name1, column_name2) AS alias_name FROM table_name;`
...
*   使用 `connect()` 函数建立与数据库服务器的连接。传递主机、用户(根用户或您的用户名)、密码(如果存在)和数据库参数给 `connect()` 方法。
*   然后使用 `cursor()` 函数创建一个游标对象。
*   对 `Persons` 表执行上面不用的语法。

> **语法:**
> `cursor.execute("SELECT CONCAT(column_name1, column_name2) AS alias_name FROM table_name;")`
