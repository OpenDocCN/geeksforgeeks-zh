...
有时候，开发人员可能希望在程序的某个点接受用户输入。为此，Python 提供了 `input()` 函数。
...
Python 提供了 [`print()`](https://www.geeksforgeeks.org/python-output-using-print-function/) 函数，向标准输出设备显示输出。

> **语法:** `print(*values, sep=' ', end='\n', file=sys.stdout, flush=False)`
>
> **参数:**
> - `*values`: 任意值，可以有多个。打印前将转换为字符串。
> - `sep='分隔符'`: (可选) 指定如何分隔多个对象。默认: `' '`。
> - `end='结束符'`: (可选) 指定在末尾打印什么。默认: `'\n'`。
> - `file`: (可选) 具有写方法的对象。默认: `sys.stdout`。
> - `flush`: (可选) 布尔值，指定输出是刷新(`True`)还是缓冲(`False`)。默认: `False`。
>
> **返回:** 返回输出到屏幕。
...
### 示例: 带有自定义 sep 和 end 参数的 Python 打印输出
...
我们可以使用 [**格式化的字符串字面值 (f-string)**](https://www.geeksforgeeks.org/formatted-string-literals-f-strings-python/) ，在开引号或三引号之前用 `f` 或 `F` 开始一个字符串。在这个字符串中，我们可以在 `{` 和 `}` 之间编写 Python 表达式，这些表达式可以引用变量或任何文字值。

**示例: 使用 f-string 的 Python 字符串格式化**
