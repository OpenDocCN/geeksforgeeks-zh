...
> 原文: [https://www.geeksforgeeks.org/python-check-if-a-variable-is-string/](https://www.geeksforgeeks.org/python-check-if-a-variable-is-string/)
...
这个任务也可以使用 `type()` 函数来实现，在 `type()` 函数中，我们只需要传递变量，并将其等同于一个特定的类型。
