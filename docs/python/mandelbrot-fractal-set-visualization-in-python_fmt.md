...
> 原文: [https://www.geeksforgeeks.org/mandelbrot-fractal-set-visualization-in-python/](https://www.geeksforgeeks.org/mandelbrot-fractal-set-visualization-in-python/)
...
曼德勃罗集合是复数 `c` 的集合，对于该集合，函数 `f_c(z) = z^2 + c` 在从 `z = 0` 迭代时不发散，即对于该集合，序列 `f_c(0), f_c(f_c(0))` 等，保持绝对值有界。简单地说，曼德勃罗集是一组特殊的复数，当绘制时，它有一个高度复杂的分形边界。
