...
> 原文：[https://www.geeksforgeeks.org/detect-and-recognize-car-license-plate-from-a-video-in-real-time/](https://www.geeksforgeeks.org/detect-and-recognize-car-license-plate-from-a-video-in-real-time/)
...
1.  找到图像中的所有轮廓。
2.  找到每个轮廓的边框。
3.  用一个平均车牌比较和验证每个边框的边长比和面积。
4.  在经过验证的轮廓内的图像中应用图像分割以找到其中的字符。
5.  使用光学字符识别识别字符。
...
7.  现在找到验证区域中的轮廓，并验证该区域中最大轮廓的边框的边长比和面积。验证后，你会得到一个完美的车牌轮廓。现在从原始图像中提取轮廓。你会得到盘子的图像：
    ...
    此步骤由 `PlateFinder` 类的 `clean_plate` 和 `ratioCheck` 方法执行。
...
8.  为了准确识别车牌上的字符，必须进行图像分割。因为第一步是从车牌图像 HSV 格式中提取价值通道。
...
11. 构造一个遮罩来显示所有的角色组件，然后在遮罩中找到轮廓。提取轮廓后，取最大的一个，找到它的边界矩形，并验证边比。

12. 验证侧比后，找到轮廓的凸包，并将其绘制在字符候选掩码上。
...
首先，创建一个 `PlateFinder` 类，用于查找车牌并验证其大小比例和面积。
...
方法 `extract_contours` 返回预处理图像的所有外部轮廓。

方法 `find_possible_plates` 用 `preprocess` 方法对图像进行预处理，然后用 `extract_contours` 方法提取轮廓，然后检查所有提取轮廓的边比和面积，用 `check_plate` 和 `clean_plate` 方法清洁轮廓内的图像。用 `clean_plate` 方法清洗轮廓图像后，用 `find_characters_on_plate` 方法查找板上的所有字符。

`find_characters_on_plate` 方法使用 `segment_chars` 功能查找字符。它通过计算阈值图像轮廓的凸包并将其绘制在字符上以显示字符来找到字符。
...
它在`load_graph`和`load_label`函数中加载预处理后的OCR模型及其标签文件。`label_image_list`方法用`convert_tensor`方法将图像转换为张量，然后用`label_image_list`函数预测张量的标签并返回许可证号。
...
可以从我的 [GitHub](https://github.com/hritik7080/Car-License-Plate-Recognition) 下载带有OCR模型和测试视频的源代码。
