...
一个`Gtk.HeaderBar`与水平的`Gtk.Box`相同，它允许在开始或结束位置放置子组件，并显示标题。GTK+支持客户端装饰，因此我们可以使用`Gtk.HeaderBar`代替标题栏。标题将相对于框的宽度居中。一个`Gtk.HeaderBar`通常位于窗口的顶部，应该包含影响下面内容的常用控件。它们还提供对窗口控件的访问，包括关闭窗口按钮和窗口菜单。
