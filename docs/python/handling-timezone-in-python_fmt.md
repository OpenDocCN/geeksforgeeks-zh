...
上面的语法返回当前本地日期时间，没有任何时区信息。但是通过使用 `pytz` 包，我们可以在不同的时区处理这些日期时间信息——`now()` 为我们提供了在时区中传递的选项，因此如果您将时区留空，它还将返回当前的本地日期时间。`now()` 的产量取决于机器。主机的本地时间和时区设置将决定输出。
...
> `datetime.datetime.now(pytz.timezone('YOUR_TIMEZONE'))`
...
> `dtObj = datetimeObject_instance.astimezone(pytz.timezone('new_time_zone'))`
>
> 哪里，
>
