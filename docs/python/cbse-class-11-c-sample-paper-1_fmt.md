...
> 原文: [https://www.geeksforgeeks.org/cbse-class-11-c-sample-paper-1/](https://www.geeksforgeeks.org/cbse-class-11-c-sample-paper-1/)
...
参考: [逻辑错误和语法错误](https://www.geeksforgeeks.org/errors-in-cc/)。
...
3.  `s = 1/2mv²`
    表达式: `1/2 * m * v * v;` 或者 `s = 1/2 * m * pow(v, 2);`
4.  `x=-b+√(b² -4ac) /2a`
...
### [A] 画一个[流程图](https://www.geeksforgeeks.org/an-introduction-to-flowcharts/)，打印三个给定编号中最小的 (2)
...
void main()
{
    const int MAX = 0; // Error
    int a, b;
    cin << a >> b; // Error
    if (a > b)
        MAX = a;
    for (int x = 0; x < MAX; x++) // x undeclared error.
        cout << x;
}
```
...
### [C] 用 `C++` 写程序打印[斐波那契数列](https://www.geeksforgeeks.org/program-to-print-first-n-fibonacci-numbers/) : `0, 1, 1, 2, 3, 5, 8..` (3)
...
**(ii)** `(2C9)_16 = (?)_10`
= `(2C9)_16 = (001011001001)_2 = (713)_10`
