...
> 原文: [https://www.geeksforgeeks.org/python-extract-similar-key-values/](https://www.geeksforgeeks.org/python-extract-similar-key-values/)
...
res = [val for key, val in test_dict.items() if ''.join(list(sorted(key))) == tst_wrd]
