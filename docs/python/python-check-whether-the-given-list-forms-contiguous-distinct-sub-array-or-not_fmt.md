...
> 原文: [https://www.geeksforgeeks.org/python-check-whether-the-given-list-forms-contiguous-distinct-sub-array-or-not/](https://www.geeksforgeeks.org/python-check-whether-the-given-list-forms-contiguous-distinct-sub-array-or-not/)
...
> **输出:** `是`
> **解释:**
> 给定元素可以以 `[1, 1]`、`[3]`、`[6, 6, 6]` 的形式转换为连续相异集合数组。
> `1` 的数量 = 2
> `3` 的数量 = 1
> `6` 的数量 = 3
>
...
> **输出:** `否`
> **解释:**
> 给定元素不能转换为连续相异集合数组，因为子数组 `[3, 5, 2, 2, 2, 3]` 违反条件（元素需要是连续的），`3` 再次出现在 `5` 和 `2` 之后。
>
> **输入:** `[9, 9, 4, 4, 1, 6]`
> **输出:** `否`
> **解释:**
> 给定元素不能转换为连续的相异集合数组。
> 它的形式是 `[9, 9]`、`[4, 4, 4]`、`[1]`、`[6]`，元素是连续存在的。
...
def contig_distinct_setarr(l, n):
    ...
    if (flag != False):
        ...
        while k < len(a):
            ...
            # here we extract the contiguous
