...
Numpy 库利用 `corrcoef()` 函数返回一个 2×2 的矩阵。该矩阵由 `x` 与 `x` (0，0)、`x` 与 `y` (0，1)、`y` 与 `x` (1，0)和 `y` 与 `y` (1，1)的相关性组成。我们只关心 `x` 与 `y` 的相关性，即单元格(0，1)或(1，0)。下面是一个例子。

### 示例 1

假设一家冰淇淋店记录了当天冰淇淋的总销量与温度的关系。
...
假设给定男孩的葡萄糖水平与年龄相对应。找出年龄(`x`)和体内葡萄糖水平(`y`)之间的相关性。
