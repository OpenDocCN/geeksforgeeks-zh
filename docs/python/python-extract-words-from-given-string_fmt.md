...
> 原文：[https://www.geeksforgeeks.org/python-extract-words-from-given-string/](https://www.geeksforgeeks.org/python-extract-words-from-given-string/)
...
如上所述，使用 `split()` 在字符串中查找单词的传统方法在包含所有特殊字符和标点符号的情况下可能会失败，因此需要正则表达式来执行此任务。`findall()` 函数在过滤字符串并提取忽略标点符号的单词后返回列表。
...
## 方法三：使用 `re.sub()` + `string.punctuation`

该方法也使用了正则表达式，但是使用了获取所有标点符号的 `string.punctuation`，忽略所有标点符号，得到过滤后的结果字符串。
