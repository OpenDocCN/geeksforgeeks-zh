...
> 原文：[https://www.geeksforgeeks.org/download-instagram-reel-using-python/](https://www.geeksforgeeks.org/download-instagram-reel-using-python/)
...
从 PyPI 安装 `insta-scrape` 库。

> `pip install insta-scrape`

要从 Instagram 下载卷轴，首先导入所有需要的库。在我们的例子中，我们正在导入 `time` 模块，因为在保存文件时，我们将把时间和文件名连接起来，以避免命名冲突。现在添加会话标识和标题。会话标识在您注销之前有效。所以当你下次会的时候，你需要再次传递一个新的会话 id。(通过检查页面，您将在会话存储中获得您的会话标识)。由于 Instagram 的更新策略，直接抓取数据并不那么容易，所以我们必须将会话 id 传递到头中。使用 `Reel` 模块和提供下载位置，我们可以完成这项工作。
