...
> 原文：[https://www.geeksforgeeks.org/get-current-time-in-different-timezone-using-python/](https://www.geeksforgeeks.org/get-current-time-in-different-timezone-using-python/)
...
为了得到当地时间，我们可以使用`time`模块。`time`模块的一些重要功能

*   `localtime()` – 有助于获取当前的当地时间
*   `strftime("%H:%M:%S", t)` – 它有助于决定用于显示时间的时间格式
