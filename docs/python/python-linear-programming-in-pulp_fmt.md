...
> 原文：[https://www.geeksforgeeks.org/python-linear-programming-in-pulp/](https://www.geeksforgeeks.org/python-linear-programming-in-pulp/)
...
`pulp` 是 Python 生态系统中众多求解优化问题的库之一。您可以在 `Jupyter notebook` 中安装 `pulp`，如下所示：
...
*   **1-2 线：** 首先将库 `pulp` 作为 `p` 导入。
*   **第 4-5 行：** 通过给你的问题取一个合适的名字来定义问题，这里我给的名字是 `'Problem'`。此外，指定目标函数的目标是最大化还是最小化。
