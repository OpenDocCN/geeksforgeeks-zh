...
> 原文: [https://www.geeksforgeeks.org/introduction-to-social-networks-using-networkx-in-python/](https://www.geeksforgeeks.org/introduction-to-social-networks-using-networkx-in-python/)

**先决条件–** [Python 基础知识](https://www.geeksforgeeks.org/python-tutorial/)
...
社交网络中最重要的 python 库是 [`NetworkX`](https://www.geeksforgeeks.org/networkx-python-software-package-study-complex-networks/)。
...
*   `Graph`: 这种类型的图存储节点和边，边是无向的。它可以有自循环，但不能有平行边。
*   `DiGraph`: 这种类型的图是有向图的基类。它可以有节点和边，边本质上是有方向的。它可以有自循环，但在双向图中不允许有平行边。
*   `MultiGraph`: 这种类型的图是无向图类，可以存储多条或平行边。它也可以有自循环。多边是两个节点之间的多条边。
*   `MultiDiGraph`: 这种类型的图是可以存储多条边的有向图类。它也可以有自循环。多边是两个节点之间的多条边。
