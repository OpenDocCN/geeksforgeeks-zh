...
> 原文：[https://www.geeksforgeeks.org/add-comment-to-column-in-mysql-using-python/](https://www.geeksforgeeks.org/add-comment-to-column-in-mysql-using-python/)
...
在本文中，我们使用一个名为 `test` 的数据库，其中有一个名为 `geeks` 的表。请看下图。
...
```sql
ALTER TABLE table_name MODIFY column_name type_of_that_column COMMENT '在此输入注释';
```
