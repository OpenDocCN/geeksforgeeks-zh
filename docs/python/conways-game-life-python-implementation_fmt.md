...
> 原文: [https://www.geeksforgeeks.org/conways-game-life-python-implementation/](https://www.geeksforgeeks.org/conways-game-life-python-implementation/)
...
因为《生命游戏》是建立在 9 个方格的格子上，每个格子都有 8 个相邻的格子，如图所示。在 `grid[i][j]` 上访问模拟中的给定单元 `(i, j)`，其中 `i` 和 `j` 分别是行和列索引。给定单元在给定时刻的值取决于其邻居在前一时间步的状态。康威的人生游戏有四条规则。
