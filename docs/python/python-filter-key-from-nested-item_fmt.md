...
> 原文: [https://www.geeksforgeeks.org/python-filter-key-from-nested-item/](https://www.geeksforgeeks.org/python-filter-key-from-nested-item/)

有时，在使用 Python 字典时，我们会遇到一个问题，即我们需要提取具有特定键值对的键作为其嵌套项的一部分。这种问题在 web 开发领域很常见。让我们讨论执行这项任务的某些方法。

> `输入`: `test_dict = { 'gfg' : { 'best' : 10, 'good' : 5 } }`
> `输出`: `['gfg']`
>
> `输入`: `test_dict = { 'gfg' : { 'best' : 10, 'good' : 12 } }`
> `输出`: `[]`
...
> 原版词典: `{'CS': { '无价':6}, 'is': {'better': 6, 'educational': 4}, 'gfg': {'good': 5, 'best': 4}, 'Maths': {'good': 5} }`
> 匹配键: `['gfg', 'Maths']`
