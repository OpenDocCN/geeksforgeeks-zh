...
> 原文: [https://www.geeksforgeeks.org/python-converting-list-string-to-dictionary/](https://www.geeksforgeeks.org/python-converting-list-string-to-dictionary/)

关于数据类型之间相互转换的另一个问题是将列表字符串转换为键和值的字典。在机器学习领域，这个特殊的问题可能发生在我们需要将大量的字符串数据转换成字典进行预处理的地方。让我们讨论一下完成这项任务的某些方法。

## 方法#1: 使用字典理解 + `split()`
字典理解可用于字典的构造，`split()`功能可用于在列表中执行必要的拆分，以获得字典的有效键值对。
