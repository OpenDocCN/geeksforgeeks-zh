...
> 原文:[https://www.geeksforgeeks.org/how-to-handle-invalid-arguments-with-argparse-in-python/](https://www.geeksforgeeks.org/how-to-handle-invalid-arguments-with-argparse-in-python/)
...
*   **第一次运行:** 论据是 `geek12345678`。用户名应该至少有 5 个字符。
*   **第二次运行:** 争论的是 `geek12345678`。不满足密码的标准。
*   **第三次运行:** 论据是 `geek123BC@#`。这些通过了所有的约束，因此系统退出

**2. 使用 `ArgumentError()`**

该函数采用两个参数，即参数和要显示的消息。为此，应该设置一个引用参数的变量。请看下面的例子。程序从用户那里获取两个值，第一个应该是奇数，第二个应该是偶数。保存为 `奇数_偶数.py`。
