...
> 原文：[https://www.geeksforgeeks.org/class-factories-a-powerful-pattern-in-python/](https://www.geeksforgeeks.org/class-factories-a-powerful-pattern-in-python/)
...
使用 `type`，我们可以动态地创建类。但是这样做会将函数和类一起留在命名空间中。让我们看看代码，以便更好地理解它。
...
在上面的代码中，我们设计了一个名为 `Apple` 的类，它以颜色作为属性。除此之外，我们还使用装饰器 `@classmethod` 声明了一个名为 `classapple` 的类方法。`classapple` 方法的功能是返回苹果的颜色。但是，您可以注意到，即使将苹果的颜色设置为黄色和绿色，对象也会返回默认颜色红色。使用类工厂可以克服这个限制。
