...
> 原文: [https://www.geeksforgeeks.org/python-get-even-indexed-elements-in-tuple/](https://www.geeksforgeeks.org/python-get-even-indexed-elements-in-tuple/)
...
以上功能的组合可以用来解决这个问题。在本文中，我们使用生成器表达式执行迭代任务，使用`enumerate()`检查偶数索引，并使用`tuple()`将结果转换为元组。
