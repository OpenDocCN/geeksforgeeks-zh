...
> 原文: [https://www.geeksforgeeks.org/python-legendre-polynomials-using-recursion-relation/](https://www.geeksforgeeks.org/python-legendre-polynomials-using-recursion-relation/)

勒让德多项式是科学和工程中经常出现的一类正交多项式。因此，他们这一代对这些领域至关重要。计算勒让德多项式有不同的方法，使用生成函数、罗德里格斯公式、递推关系、格拉姆-施密特正交化等。最简单也是最准确的方法之一，就是使用递推关系。
...
我们将勒让德多项式定义为一个名为 `P(n, x)` 的函数，其中 `n` 称为多项式的阶，`x` 是求值点。基本情况是，如果 `n` 是 0，那么多项式的值总是 1，当阶是 1 时，它是 `x`。这些是循环关系所需的种子值。对于 `n` 的其他值，直接从 Bonnet 的递归中递归定义函数。因此，`P(n, x)` 通过递归方法返回勒让德多项式的值（用同一函数本身的其他基本情况有效定义的函数）。
