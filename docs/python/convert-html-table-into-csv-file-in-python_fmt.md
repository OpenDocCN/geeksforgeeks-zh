...
> 原文：[https://www.geeksforgeeks.org/convert-html-table-into-csv-file-in-python/](https://www.geeksforgeeks.org/convert-html-table-into-csv-file-in-python/)

`CSV` 文件是一个逗号分隔值文件，使用逗号分隔值。当我们谈论机器学习、数据处理和数据可视化时，`CSV` 文件在当今世界是一个有用的东西。在本文中，我们将讨论如何将一个 `HTML` 表转换成 `CSV` 文件。
