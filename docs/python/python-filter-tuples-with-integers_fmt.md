...
> 原文: [https://www.geeksforgeeks.org/python-filter-tuples-with-integers/](https://www.geeksforgeeks.org/python-filter-tuples-with-integers/)
...
在这种情况下，我们迭代每个元组，并使用 `isinstance()` 检查除 `int` 之外的数据类型，如果找到非整数，该元组被标记为无效并被省略。
...
## 方法 2: 使用 all() + 列表推导式 + isinstance()

在这种情况下，`all()` 用于配合 `isinstance()` 检查所有元素是否都是整数，如果检查通过，则将元组添加到结果中。
