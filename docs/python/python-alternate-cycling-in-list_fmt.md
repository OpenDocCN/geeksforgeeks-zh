...
> 原文: [https://www.geeksforgeeks.org/python-alternate-cycling-in-list/](https://www.geeksforgeeks.org/python-alternate-cycling-in-list/)
...
上述功能的组合可用于执行该任务。在这种情况下，`reversed()` 和 `iter()` 分别用于创建反向和正常序列列表的迭代器，`cycle()` 执行交替访问的任务。`islice()` 执行提取元素和构建新列表的任务。`next()` 执行访问元素的任务。
