...
> 原文: [https://www.geeksforgeeks.org/introduction-to-sanic-web-framework-python/](https://www.geeksforgeeks.org/introduction-to-sanic-web-framework-python/)
...
要在 Python 虚拟环境中安装 `sanic`，我们将执行以下命令 – `pip3 install sanic`
...
我们可以从 IDE 运行 `main.py`，或者通过执行以下命令从终端运行文件 – `python3 main.py`
...
Sanic app 对象的 `config` 属性用于配置参数。应用程序配置对象可以按如下方式分配键值对:
...
Sanic 支持 `@route` 装饰器将处理函数映射到 HTTP 请求。我们可以在 `@route` 装饰器中使用一个名为 `methods` 的可选参数来处理列表中的任何一个 HTTP 方法。
...
Sanic 路由可以提供 html 文件、json 内容、媒体文件等。为了提供像图像、pdf、静态 html 文件等静态内容，我们需要使用 `app.static()` 方法，该方法将静态文件的路径映射到由 `@route` 指定的端点。
...
运行 `main.py` 并访问 `http://0.0.0.0:8000/floral_image.jpg` 在浏览器上渲染图像。
...
Python 3.5 引入了 `asyncio`，这是*一个使用异步/等待语法*编写并发代码的库 (来源: [https://docs.python.org/3/library/asyncio.html](https://docs.python.org/3/library/asyncio.html))。`asyncio` 库提供了一个运行异步输入输出功能的事件循环。Sanic 提供对 `async/await` 语法的支持，从而使请求处理无阻塞且超快。将 `async` 关键字添加到请求处理程序函数中，可以使函数异步处理代码，从而利用 Sanic 的性能优势。
...
参考: [Sanic 文档](https://sanic.readthedocs.io/en/latest/)。