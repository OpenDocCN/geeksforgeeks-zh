...
请求库是 Python 向指定网址发出 HTTP 请求的重要方面之一。本文围绕如何使用`requests`向指定的网址发出 GET 请求展开。`GET()`方法。在检查 GET 方法之前，让我们弄清楚 GET 请求是什么–

## 获取 Http 方法

GET 方法用于使用给定的 URI 从给定的服务器检索信息。GET 方法发送附加到页面请求的编码用户信息。页面和编码信息用“？”分隔。
例如:
