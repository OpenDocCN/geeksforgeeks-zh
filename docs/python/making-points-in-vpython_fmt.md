...
几何中的一个点是一个位置。我们可以使用`points()`方法在`VPython`中生成点。
...
> **参数:**
>
> *   `pos` : 是点的位置。指定包含 3 个值的向量列表，例如 `pos = [vector(-1, 0, 0), vector(1, 0, 0)]`
> *   `color`: 是点的颜色。指定一个包含 3 个值的向量，例如 `color = vector(1, 1, 1)` 将给出白色
> *   `radius`: 是点的半径。指定一个浮动值，默认半径为 2.5，示例 `radius = 5`
