...
> 原文：[https://www.geeksforgeeks.org/finding-duplicate-files-with-python/](https://www.geeksforgeeks.org/finding-duplicate-files-with-python/)

在本文中，我们将编写一个 Python 脚本来查找文件系统或特定文件夹中的重复文件。

## 方法 1：使用 `filecmp`

Python 模块 `filecmp` 提供了比较目录和文件的功能。`cmp` 函数比较文件，如果它们看起来相同，则返回 `True`，否则返回 `False`。
...
*   `f1`：一个文件的名称
*   `f2`：要比较的另一个文件的名称
*   `shallow`：有了这个，我们设置要不要比较内容。

**注意：** 默认值为 `True`，保证只比较文件的签名，不比较内容。

**返回类型：** 布尔值（如果文件相同则为 `True`，否则为 `False`）
