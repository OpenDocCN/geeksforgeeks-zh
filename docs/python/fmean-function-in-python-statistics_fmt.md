...
> 原文: [https://www.geeksforgeeks.org/fmean-function-in-python-statistics/](https://www.geeksforgeeks.org/fmean-function-in-python-statistics/)

Python 3.8 的 `statistics` 模块提供了一个函数 `fmean()`，该函数将所有数据转换为浮点数据类型，然后计算以序列或可迭代形式提供的数据的算术平均值或平均值。这个函数的输出总是一个浮点数。
...
>   File "C:\Users\Admin\AppData\Local\Programs\Python\Python38-32\lib\statistics.py", line 345, in fmean
>     raise StatisticsError(_sum == 0, 'fmean requires at least one data point')
