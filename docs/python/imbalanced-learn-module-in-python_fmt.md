...
> 原文: [https://www.geeksforgeeks.org/imbalanced-learn-module-in-python/](https://www.geeksforgeeks.org/imbalanced-learn-module-in-python/)
...
**重采样器:** `fit_resample`方法将数据和目标重采样到字典中，字典中的键值对为`data_resample`和`targets_resample`。
...
我们将使用名为`make_classification`数据集的内置数据集，该数据集返回:
...
>
> **参数(可选):** `sampling_strategy='auto'`, `return_indices=False`, `random_state=None`, `ratio=None`
>
> **实现:**
...
>
> **参数(可选):** `sampling_strategy='auto'`, `return_indices=False`, `random_state=None`, `n_neighbors=3`, `kind_sel='all'`, `n_jobs=1`, `ratio=None`
>
> **实施:**
...
>
> **参数(可选):** `sampling_strategy='auto'`, `return_indices=False`, `random_state=None`, `replacement=False`, `ratio=None`
>
> **实现:**
