...
> 原文：[https://www.geeksforgeeks.org/python-filter-dictionary-values-in-heterogeneous-dictionary/](https://www.geeksforgeeks.org/python-filter-dictionary-values-in-heterogeneous-dictionary/)
...
以上函数的组合也可以用来解决这个问题。在这种情况下，我们执行类似于上面的任务，但不同的是，类型测试是由 `isinstance()` 而不是 `type()` 完成的。
