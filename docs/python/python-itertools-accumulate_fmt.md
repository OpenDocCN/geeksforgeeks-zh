...
让我们创建一个字符串列表，并将其命名为 `colors`。我们可以使用 `for` 循环来迭代列表，如下所示:
...
使用前必须导入 `itertools` 模块。我们还必须导入 `operator` 模块，因为我们想与操作符一起工作。
...
`itertools` 模块是函数的集合。我们将探索其中一个 `accumulate()` 函数。
...
> **语法**
> `itertools.accumulate(iterable[, func])` -> 累积对象
