...
> 原文: [https://www.geeksforgeeks.org/isoformat-method-of-datetime-class-in-python/](https://www.geeksforgeeks.org/isoformat-method-of-datetime-class-in-python/)
...
> **语法:** `isoformat(sep='T', timespec='auto')`
>
> **参数:** 该函数接受两个参数，如下图所示:
>
> *   `sep`: 是日期和时间字段之间要打印的分隔符。它是一个可选参数，默认值为 `"T"`。
> *   `timespec`: 是 timespec 的格式说明符。它也是一个可选参数，默认值为 `"auto"`。该参数还具有如下所示的一些值:
>     *   `auto`: 对于自动值，时间分量将以 `HH:MM:SS` 格式打印。如果微秒组件可用，它将被打印。否则，将省略微秒，而不是打印为零。
>     *   `hours`: 对于小时值，返回的时间组件将只有 `HH` 格式的小时。请注意，时区组件不同于时间组件。
>     *   `minutes`: 对于指定的分钟值，返回的时间组件将只有以 `HH:MM` 格式打印的小时和分钟。
>     *   `seconds`: 对于指定的秒值，返回的时间分量将具有 `HH:MM:SS` 格式。
>     *   `milliseconds`: 对于指定的毫秒值，返回的时间分量将具有 `HH:MM:SS:mmm` 格式，其中 `mmm` 为毫秒。微秒将被排除。
>     *   `microseconds`: 对于指定的微秒，返回的时间分量将具有 `HH:MM:SS:mmmmmm` 格式，其中 `mmmmmm` 是微秒。
>
> **返回值:** 这个函数以 ISO 8601 格式返回 Python `DateTime.date` 对象的日期值。

## 示例 1

在下面的示例中，`isoformat()` 函数在今天的日期被调用，它以 ISO 8601 格式返回相同的今天的日期字符串。
...
在下面的示例中，`isoformat()` 函数在今天的日期和时间被调用，它以 ISO 8601 格式返回相同的今天的日期和时间字符串。
...
在下面的示例中，`isoformat()` 函数采用了两个参数，一个是分隔符，如 `"#"`，另一个参数是特定于时间的格式说明符。但是，如果使用不同的时间说明符值，输出可以根据它进行格式化。这里使用了特定于时间的参数的不同值，这些值已经在上面的参数部分进行了说明。
