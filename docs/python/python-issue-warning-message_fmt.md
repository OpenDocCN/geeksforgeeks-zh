...
拥有一个可以发出警告消息的程序（例如，关于不推荐使用的功能或使用问题）。

## 代码示例 1：使用 `warnings.warn()` 功能
...
`warn()` 的参数是一个警告消息和一个警告类，它通常是以下之一：`UserWarning`、`DeprecationWarning`、`SyntaxWarning`、`RuntimeWarning`、`ResourceWarning` 或 `FutureWarning`。
...
*   或者，可以使用 `warnings.simplefilter()` 功能来控制输出。`'always'` 参数会显示所有警告消息，`'ignore'` 会忽略所有警告，`'error'` 会将警告转化为异常。
