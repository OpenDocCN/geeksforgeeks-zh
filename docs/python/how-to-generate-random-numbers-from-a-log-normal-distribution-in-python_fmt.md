...
> 原文: [https://www.geeksforgeeks.org/how-to-generate-random-numbers-from-a-log-normal-distribution-in-python/](https://www.geeksforgeeks.org/how-to-generate-random-numbers-from-a-log-normal-distribution-in-python/)
...
*   `mean`: 取基础正态分布的平均值。
*   `sigma`: 它只取基础正态分布的标准偏差的非负值
*   `size`: 取给定形状的整型或元组。如果传递一个值，它将返回一个整数作为结果。如果一个元组，那么它从对数正态分布返回一个 2D 值矩阵。

### 返回

从参数化对数正态分布(`ndarray` 或标量)中抽取样本。
