...
> 原文: [https://www.geeksforgeeks.org/python-filter-list-of-strings-based-on-the-substring-list/](https://www.geeksforgeeks.org/python-filter-list-of-strings-based-on-the-substring-list/)

给定两个字符串列表`strings`和`substrings`，编写一个 Python 程序过滤掉`strings`中包含`substrings`中字符串的所有字符串。
...
我们可以在运算符中使用列表理解和`any()`来检查`substr`中的字符串是否包含在`string`中。
