...
Python 中的 `os` 模块提供了与操作系统交互的功能。`os` 属于 Python 的标准实用程序模块，提供了一种使用操作系统相关功能的可移植方式。`os` 和 `os.path` 模块包括许多与文件系统交互的功能。在文件名和路径无效或不可访问的情况下，或者在其他具有正确类型但不被操作系统接受的参数的情况下，`os` 模块中的所有函数都会产生 `OSError`。

`os` 模块中有不同的方法可用于创建目录。这些是：

*   [`os.mkdir()`](#使用-osmkdir)
*   [`os.makedirs()`](#使用-osmakedirs)

## 使用 `os.mkdir()`

Python 中的 `os.mkdir()` 方法用于以指定的数值模式创建一个名为 `path` 的目录。如果要创建的目录已经存在，该方法将引发 `FileExistsError`。

> **语法:** `os.mkdir(path, mode=0o777, *, dir_fd=None)`
>
> **参数:**
> **`path`:** 表示文件系统路径的类路径对象。类似路径的对象是表示路径的字符串或字节对象。
> **`mode` (可选):** 表示待创建目录模式的整数值。如果省略该参数，则使用默认值 `0o777`。
> **`dir_fd` (可选):** 引用目录的文件描述符。此参数的默认值为 `None`。如果指定的 `path` 是绝对的，那么 `dir_fd` 被忽略。
>
> **注意:** 参数列表中的 “`*`” 表示以下所有参数（在我们的例子中为 `dir_fd`）都是仅包含关键字的参数，并且可以使用它们的名称来提供，而不是作为位置参数。
>
> **返回类型:** 此方法不返回值。
...
假设我们想要创建目录 “Nikhil”，但是目录 “GeeksForGeeks” 和 “Authors” 在路径中不可用。然后 `os.makedirs()` 方法会在指定路径中创建所有不可用/缺失的目录。首先创建 “GeeksForGeeks” 和 “Authors”，然后创建 “Nikhil” 目录。

> **语法:** `os.makedirs(path, mode=0o777, exist_ok=False)`
>
> **参数:**
> **`path`:** 表示文件系统路径的类路径对象。类似路径的对象是表示路径的字符串或字节对象。
> **`mode` (可选):** 表示新创建目录模式的整数值。如果省略该参数，则使用默认值 `0o777`。
