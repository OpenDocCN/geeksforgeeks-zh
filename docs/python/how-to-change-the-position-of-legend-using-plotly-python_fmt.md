...
> 原文: [https://www.geeksforgeeks.org/how-to-change-the-position-of-legend-using-plotly-python/](https://www.geeksforgeeks.org/how-to-change-the-position-of-legend-using-plotly-python/)

在本文中，我们将讨论如何使用 Python 在 Plotly 中更改图例的位置。

当一个对象的变化必须参照另一个对象来描述时，默认情况下会出现图例。图例使图形更容易阅读，因为它包含所用颜色代码或键的描述。默认情况下，图例出现在图的右上角，但是在图的外部，但是可以根据需要定位它们。让我们首先创建一个规则的图，以便区别可以很明显。
...
为了定位图例，我们使用 `update_layout` 函数，将 `legend` 设置为描述图例属性的字典。定位键设置位置，`y` 和 `x` 容纳相对于轴的边距。
