...
### `colorama` 模块
然后可以使用 `Colorama` 对 `ANSI` 转义序列的不断简写来完成彩色文本的跨平台打印。
...
### `termcolor` 模块
`termcolor` 是一个 `python` 模块，用于 `ANSII Color` 格式化，以便在终端中输出。
...
### 构建要调用的函数
我们可以构建函数来调用特定颜色命名的函数来执行相关的 `ANSI` 转义序列。
...
### 迭代函数
我们可以设计迭代 & 自生成 `ANSI Escape` 序列，函数。
