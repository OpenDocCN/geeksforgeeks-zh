...
> 原文: [https://www.geeksforgeeks.org/identifying-patterns-in-dataframes-using-data-pattern-module/](https://www.geeksforgeeks.org/identifying-patterns-in-dataframes-using-data-pattern-module/)

**先决条件:** [`pandas` 模块](https://www.geeksforgeeks.org/pandas-tutorial/)， [熊猫数据框](https://www.geeksforgeeks.org/python-pandas-dataframe/)

`pandas` 是建立在 `NumPy` 库之上的开源库。它是一个 Python 包，提供了各种数据结构和操作来操作数字数据和时间序列。它主要是流行的，因为导入和分析数据容易得多。`pandas` 速度快，为用户带来高性能&生产力。

**数据框**是二维可变大小、潜在异构的表格数据结构，在 `pandas` 中带有标记轴(行和列)。数据框是一种二维数据结构，即数据以表格形式排列成行和列。`pandas` 数据框由数据、行和列三个主要部分组成。

**数据模式**模块，为了在数据框中找到简单的数据模式，我们将使用 python 中的 `data-patterns` 模块，该模块用于在结构化数据集中生成和评估模式，并导出到 Excel 和 JSON，并将生成的模式转换为 Pandas 代码。
...
1.  导入所需模块。
2.  分配数据框。
3.  使用数据框作为构造函数参数创建模式混合器对象。
4.  调用模式混合器对象的 `find()` 方法来识别数据帧中的各种模式。
...
数据项 `Value4` 和 `Value5` 在 `9` 和 `1` 的支持下具有相等的模式。
