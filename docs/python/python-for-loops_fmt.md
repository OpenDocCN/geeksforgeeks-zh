...
[`pass` 语句](https://www.geeksforgeeks.org/python-pass-statement/)来写空循环。`Pass` 也用于空的控制语句、函数和类。

### 示例:带 pass 语句的 Python For 循环
...
*   **开始:**整数，从该整数开始返回整数序列
*   **停止:**整数，在此之前返回整数序列。整数的范围以 `停止-1` 结束。
*   **步骤:**确定序列中每个整数之间增量的整数值

### 示例:Python 用于带 range 函数的循环
