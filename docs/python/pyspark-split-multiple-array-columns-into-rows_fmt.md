...
要将多个数组列数据拆分为行，pyspark 提供了一个名为 `explode()` 的函数。使用 `explode`，我们将为数组中的每个元素获得一个新行。当一个数组传递给这个函数时，它会创建一个新的默认列，并且包含所有数组元素作为它的行，数组中的空值将被忽略。这是一个内置功能，可在 `pyspark.sql.functions` 模块中获得。

> **语法:**`pyspark.sql.functions.explode(col)`
> 
> **参数:**
> 
...
**1。`explode_outer()`:**`explode_outer` 函数为数组元素的每个元素将数组列拆分为一行，不管它是否包含空值。而简单 `explode()`忽略列中的空值。
...
**3。`posexplode_outer()`:**`posexplode_outer()`为数组中的每个元素将数组列拆分为行，并且还提供了元素在数组中的位置。它创建两列“pos”来携带数组元素的位置，创建“col”来携带特定的数组元素，无论它是否也包含空值。这意味着 `posexplode_outer()`具有 `explode_outer()`和 `posexplode()`函数的功能。让我们看看这个例子:
