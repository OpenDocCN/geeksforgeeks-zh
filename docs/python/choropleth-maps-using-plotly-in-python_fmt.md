...
> 原文: [https://www.geeksforgeeks.org/choropleth-maps-using-plotly-in-python/](https://www.geeksforgeeks.org/choropleth-maps-using-plotly-in-python/)
...
`plotly.express.choropleth(data_frame=None, lat=None, lon=None, locations=None, locationmode=None, geojson=None, color=None, scope=None, center=None, title=None, width=None, height=None)`
...
*   `locationmode`：国际标准化组织 3、美国各州或国家名称之一。这确定了用于将 `locations` 中的条目与地图上的区域相匹配的位置集。
*   `geojson`：包含一个多边形要素集合，其 `id` 是来自 `locations` 的引用。
*   `color`：用于给标记指定颜色。
*   `scope`：可能的值–“世界”、“美国”、“欧洲”、“亚洲”、“非洲”、“北美”或“南美”。默认为“世界”，除非 `projection` 设置为“albers usa”，强制为“usa”。
*   `center`：设置地图的中心点。
