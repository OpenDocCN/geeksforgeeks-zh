...
> 原文：[https://www.geeksforgeeks.org/making-automatic-module-installer-in-python/](https://www.geeksforgeeks.org/making-automatic-module-installer-in-python/)

**先决条件：**

*   [urllib](https://www.geeksforgeeks.org/python-urllib-module/)
*   [subprocess](https://www.geeksforgeeks.org/python-subprocess-module-to-execute-programs-written-in-different-languages/)

很多时候，安装内置 Python 中尚未提供的模块的任务看起来令人沮丧。本文的重点是删除打开命令行界面的任务，并键入 `pip install` 模块名称来下载一些 python 模块。在本文中，我们将尝试自动化这个过程。

### 进场：

*   使用 python 导入 `subprocess` 模块来模拟命令行。
...
1.  检查互联网是开着还是关着。
2.  使用 `urllib.request.urlopen(host)` 命令到达特定的网址。
    *   如果成功到达，返回 `True`。
    *   否则，如果没有达到，即互联网关闭，返回 `False`。
