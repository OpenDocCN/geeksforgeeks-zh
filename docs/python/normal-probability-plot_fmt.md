...
*   **Q-Q 图:** Q-Q(分位数到分位数)图用于比较两个分布的分位数。分位数可以定义为具有相等概率的连续区间，或将样本在  和之间以类似的方式划分 分布可以是理论分布或来自过程的样本分布、等。正态概率图是`q-q`图的一个例子。
...
其中 `U_i` 是均匀序中值统计量，`G` 是正态分布的百分比点函数。它是累积分布函数的倒数。即给定概率分布，我们需要累积分布函数。
...
在这个实现中，我们将使用 `statsmodels` python 库和 `seaborn` 库来可视化不同的图。这些库都是预装在 `colab` 中的，但是对于本地环境使用 `pip` 安装。
