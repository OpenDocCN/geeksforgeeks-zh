...
*   将下载名为 `credentials.json` 的文件。将此文件复制到 Python 的工作目录中。
...
> **语法:** `ezgmail.send(email, sub, body)`
>
> **参数:**
>
> *   `email`: 收件人的电子邮件地址
...
`EZGmail` 有 `GmailThread` 对象来表示对话线程，还有 `GmailMessage` 对象来表示单个电子邮件。`GmailThread` 对象的 `messages` 属性包含对话线程中每条消息的 `GmailMessage` 对象列表。

*   `unread()`: 方法返回未读线程的 `GmailThread` 对象列表。
*   `summary()`: 在线程对象列表上调用时，返回线程中消息的摘要。
*   `messages`: 返回 `GmailMessage` 对象列表的 `GmailThread` 对象的属性。每一个都有描述电子邮件的属性，如 `subject`、`body`、`timestamp`、`sender` 和 `recipients`。
*   `recent()`: 返回最近线程的 `GmailThread` 对象列表的函数。`maxResults` 参数用于设置要显示的最近邮件数量。默认值为 25。
...
我们还可以搜索特定的电子邮件，就像我们在 gmail 搜索框中输入查询一样。`search()` 函数返回与搜索参数匹配的 `GmailMessage` 对象列表。我们还可以执行特殊的搜索操作，例如:

*   `'from:sender@email.com'`: 从特定发件人处获取电子邮件
*   `'subject:Python'`: 特定主题的电子邮件
*   `'label:unread'`: 未读电子邮件
...
*   `GmailMessage` 对象的 `attachments` 属性返回电子邮件附带的文件名称列表。
*   `downloadAttachment()` 方法将这些文件名中的任何一个作为参数，并下载该文件。
*   使用 `downloadAllAttachments()` 方法一次下载所有文件。
...
# downloading the first file in the list
