...
> 原文: [https://www.geeksforgeeks.org/python-interleave-two-lists-of-different-length/](https://www.geeksforgeeks.org/python-interleave-two-lists-of-different-length/)
...
## 方法 2: 使用 `chain()` + `zip()` + `cycle()`

大多数操作与上述方法一样，唯一的区别是交织任务是使用 `chain()` 执行的。
