...
> 原文：[https://www.geeksforgeeks.org/moviepy-inserting-text-in-the-video/](https://www.geeksforgeeks.org/moviepy-inserting-text-in-the-video/)

在本文中，我们将了解如何在 MoviePy 中向视频剪辑插入文本。MoviePy 是一个用于视频编辑的 Python 模块，可用于视频和 GIF 的基本操作。视频是由帧组成的，帧的组合创建了一个视频，每一帧都是一个独立的图像。`VideoClip` 是 MoviePy 中所有其他视频剪辑的基类。为了做到这一点，我们需要安装 ImageMagick，否则它将无法工作。向视频添加文本类似于创建复合视频。
