...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-population-pyramid-using-plotly-in-python/](https://www.geeksforgeeks.org/how-to-create-a-population-pyramid-using-plotly-in-python/)
...
在本文中，我们将研究如何用 Python 创建人口金字塔。为了达到我们的目的，我们将使用两个额外的库 `pandas` 和 `plotly` 来绘制我们的图表。如果您没有安装这些库，您可以通过 `pip` 命令安装它们。
...
我们将创建一个已经导入的 `graph_objects` 模块的实例，并使用这个实例，我们将使用 `add_trace()` 方法将男性和女性数据逐个添加到图表中。
