...
包`zope.interface`为 Python 提供了一个“对象接口”的实现。它由 Zope 工具包项目维护。该包直接导出两个对象`Interface`和`Attribute`。它还导出了几个助手方法。它旨在提供比 Python 内置的`abc`模块更严格的语义和更好的错误消息。

## 声明接口

在 python 中，接口是使用 python`class`语句定义的，是`zope.interface.Interface`的子类。`Interface`是所有接口的父接口。
...
接口作为设计类的蓝图，所以接口是使用类上的`@implementer`装饰器来实现的。如果一个类实现了一个接口，那么这个类的实例就提供了这个接口。对象可以直接提供接口，除了它们的类实现什么。
...
*   `implementedBy(类)`–返回一个布尔值，如果类实现接口则为真，否则为假
*   `providedBy(对象)`–返回一个布尔值，如果对象提供接口，则返回 True，否则返回 False
*   `providedBy(类)`–返回 False，因为类不提供接口，但实现接口
*   `list(zope.interface.implementedBy(类))`–返回一个类实现的接口列表
*   `list(zope.interface.providedBy(对象))`–返回一个对象提供的接口列表。
*   `list(zope.interface.providedBy(类))`–返回空列表，因为类不提供接口，但是实现了。
...
*   `extends(接口)`–返回布尔值，一个接口是否扩展另一个接口。
*   `isOrExtends(接口)`–返回布尔值，无论接口是相同的还是一个扩展另一个。
*   `isEqualOrExtendedBy(接口)`–返回布尔值，无论接口是相同的还是一个被另一个扩展。
