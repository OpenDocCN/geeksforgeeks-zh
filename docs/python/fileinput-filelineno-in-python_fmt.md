# Python中的`fileinput.filelineno()`方法

> 原文: [https://www.geeksforgeeks.org/fileinput-filelineno-in-python/](https://www.geeksforgeeks.org/fileinput-filelineno-in-python/)

借助`fileinput.filelineno()`方法，我们可以通过使用该方法从输入文件中获取每个文件在线读取的每一行的行号。
...
**示例#1:**
在这个示例中，我们可以看到，通过使用`fileinput.filelineno()`方法，我们能够从给定文件中获取每个文件的每行读取的行号。
