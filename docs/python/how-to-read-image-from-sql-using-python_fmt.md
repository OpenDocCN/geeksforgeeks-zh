...
> 原文: [https://www.geeksforgeeks.org/how-to-read-image-from-sql-using-python/](https://www.geeksforgeeks.org/how-to-read-image-from-sql-using-python/)
...
1.  首先，我们创建一个名为 `convert_data` 的函数，并将两个参数作为 `data` 和 `file_name`。在这个函数中，我们用给定的文件名将二进制数据转换成人类可读或可理解的形式。在第一个参数中，数据存储二进制数据，在第二个参数中，文件名保存从数据库中检索的文件名。
...
5.  使用 for 循环，用于逐个检索记录。为了访问每一列，我们使用索引值，例如第 1 列的 `0`、第 2 列的 `1`、第 3 列的 `2` 等。
