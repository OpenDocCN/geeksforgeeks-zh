...
*   `locationtagger.find_locations(text)`：返回带有位置信息的实体。`text` 参数将文本作为输入。
...
*   `entity.country_cities`：提取文本中找到城市的国家。
*   `entity.other_countries`：提取文本中存在区域或城市的所有国家列表。
*   `entity.region_cities`：提取文本中有城市的区域。
