...
> 原文：[https://www.geeksforgeeks.org/py-facts-10-interesting-facts-about-python/](https://www.geeksforgeeks.org/py-facts-10-interesting-facts-about-python/)
...
实际上有一首蒂姆·皮特斯写的诗，名叫《Python 之禅》，只需在解释器中写下 `import this` 就可以读懂。
...
在 Python 中可以返回多个值。不相信？参见下面的代码片段：
...
可以在 Python 中使用带有 `for` 循环的 `else` 子句。这是一种特殊类型的语法，只有当 `for` 循环自然退出时才执行，没有任何 `break` 语句。
...
在 Python 中，一切都是通过引用来完成的。它不支持指针。

## 5. 函数参数解包

函数参数解包是 Python 的另一个令人敬畏的特性。可以分别使用 `*` 和 `**` 将列表或字典解包为函数参数。这通常被称为 Splat 运算符。此处示例：
...
想在 `for` 循环中找到索引吗？用 `enumerate` 包装一个 iterable，它将生成该项及其索引。请参见这段代码片段：
...
一个可以在 Python 中链式比较的运算符。`ans = 1 < x < 10` 在 Python 中是可执行的。这里有更多的例子：
...
我们不能定义无穷大对吗？但是等等！这不适合 Python。看到这个惊人的例子：
...
不用循环构建列表，可以用列表推导式更简洁地构建。请参阅此代码了解更多信息。
...
最后是 Python 的特殊切片运算符。这是一种从列表中获取项目以及更改它们的方法。请参见这段代码片段：
