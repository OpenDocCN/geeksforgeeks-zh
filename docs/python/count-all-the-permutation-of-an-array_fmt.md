...
> 原文: [https://www.geeksforgeeks.org/count-all-the-permutation-of-an-array/](https://www.geeksforgeeks.org/count-all-the-permutation-of-an-array/)
...
遍历所有排列的列表，从 `A[]` 中移除包含任何子阵列 `[i，i+1]` 的那些阵列。
...
以下是基于以下事实的递归解: 阵列的长度决定了没有子阵列的所有排列的数量 `[i，i+1]` 对于 `A[]` 中的每个 `i`。
