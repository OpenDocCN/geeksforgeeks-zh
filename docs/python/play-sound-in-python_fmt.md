...
*   还有一个可选的第二个参数 `blocking`，默认设置为 `True`。我们可以将其设置为 `False`，使功能异步运行。
*   它适用于 `WAV` 和 `MP3` 文件。
...
`tksnack` 模块依赖于一个名为 `tkinter` 的模块来激活 python 脚本中的一个 `tk` 对象。您必须为 Python 安装 `tkinker` 和 `tksnack` 包。运行以下命令来安装软件包:
...
`play()` 方法用于播放音频文件。`blocking` 参数声明声音将异步播放。
