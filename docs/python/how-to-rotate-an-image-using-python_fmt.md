...
> 原文: [https://www.geeksforgeeks.org/how-to-rotate-an-image-using-python/](https://www.geeksforgeeks.org/how-to-rotate-an-image-using-python/)

在本文中，让我们看看如何使用 Python 旋转图像。通过图像旋转，图像将围绕其中心旋转指定的度数。图像的旋转是一种几何变换，它可以通过正向变换（或反向变换）来完成。

这里使用的图像处理库 `Pillow` 采用逆变换。如果为图像旋转指定的度数不是 90 度的整数倍，则某些像素值会超出图像边界，即位于图像维度之外。这样的值不会显示在输出图像中。
...
Python 图像处理库 `Pillow` 的 `rotate()` 方法以度数为参数，在**逆时针方向**旋转图像到指定的度数。

## 方法 2：使用 OpenCV 在 Python 中将图像旋转一个角度

这很常见，因为每个人都知道 Python `OpenCV` 是一个模块，用于处理与计算机视觉相关的实时应用程序。`OpenCV` 使用图像处理库 `imutils` 来处理图像。`imutils.rotate()` 函数用于在 Python 中将图像旋转一个角度。
...
使用 OpenCV 旋转 45 度的图像。

![](img/cc256443ead9a3932471752964529053.png)

使用 OpenCV 旋转 90 度的图像。

同样，`OpenCV` 也会在**逆时针**方向上将图像旋转到指定的度数。