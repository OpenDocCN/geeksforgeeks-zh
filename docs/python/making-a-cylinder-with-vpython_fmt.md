...
> *   `axis`:是圆柱体的对中轴。指定包含 3 个值的向量，例如 `axis = vector(1, 2, 1)`
> *   `up` : 是气缸的方位。指定一个包含 3 个值的向量，例如 `up = vector(0, 1, 0)`
> *   `color`:是圆柱体的颜色。指定一个包含 3 个值的向量，例如 `color = vector(1, 1, 1)` 将给出白色
> *   `opacity`:是圆柱体的不透明度。分配一个浮动值，其中 1 是最不透明的，0 是最不透明的，例如 `opacity = 0.5`
> *   `shininess`:是圆柱体的光泽。指定一个浮动值，其中 1 是最闪亮的，0 是最不闪亮的，例如 `shininess = 0.6`
> *   `emissive`:是圆柱体的发射率。指定一个布尔值，其中“真”是发射性的，“假”不是发射性的，例如 `emissive = False`
> *   `texture`:是圆柱体的纹理。从纹理类中指定所需的纹理，例如 `texture = textures.stucco`
> *   `length`:是圆柱体的长度。分配一个浮点值，默认长度为 1，示例 `length = 10`
> *   `radius`:是圆柱体的半径。指定一个浮动值，默认高度为 1，例如 `radius = 5`
> *   `size`:是圆柱体的尺寸。指定一个包含 3 个值的向量，分别代表长度、高度和宽度，例如 `size = vector(1, 1, 1)`
>
> 所有参数都是可选的。
...
# import the module
from vpython import *
cylinder(texture = textures.stucco,
         axis = vector(-1, 4, 0),
         up = vector(1, 2, 2))
```
