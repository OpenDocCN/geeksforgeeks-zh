...
> 原文：[https://www.geeksforgeeks.org/get-live-weather-desktop-notifications-using-python/](https://www.geeksforgeeks.org/get-live-weather-desktop-notifications-using-python/)
...
*   `bs4`：`BeautifulSoup`（`bs4`）是一个从 HTML 和 XML 文件中拉出数据的 Python 库。要安装此模块，请在终端中键入以下命令。
...
*   `requests`：这个库可以让你极其轻松的发送 HTTP/1.1 请求。要安装此模块，请在终端中键入以下命令。
...
1.  从给定的网址中提取数据。
2.  借助 `requests` 和 `BeautifulSoup` 刮数据。
3.  将数据转换成 html 代码。
4.  找到需要的细节，过滤掉。
5.  将结果保存在字符串中。
6.  在通知对象中传递结果。
