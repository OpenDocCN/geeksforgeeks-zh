...
> 原文: [https://www.geeksforgeeks.org/how-to-add-drag-behavior-in-kivy-widget/](https://www.geeksforgeeks.org/how-to-add-drag-behavior-in-kivy-widget/)
...
实际上我们正在做的是，我们正在使用的只是使用继承的概念，我们正在通过结合 Kivy Label 小部件和 Kivy 的 `DragBehavior` 类的属性来制作一个新的小部件(`DraggableLabel`)。从代码中，您可以看到我们定义了三个属性：
...
*   `drag_distance`: 拖动拖动行为前要移动的距离，以像素为单位。一旦移动了距离，`DragBehavior` 就会开始拖动，并且不会向孩子发送任何触摸事件。
