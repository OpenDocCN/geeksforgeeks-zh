...
> 原文: [https://www.geeksforgeeks.org/python-increment-1s-in-list-based-on-pattern/](https://www.geeksforgeeks.org/python-increment-1s-in-list-based-on-pattern/)
...
## 方法#2：使用 `itertools` 模块的 `count`、`chain` 和 `groupby`

对于给定的问题，这是一种高效且更加精确的方法，我们使用 `itertools` 模块中的 `count`、`chain` 和 `groupby`。
