...
*   每当 `PyScaffold` 的新更新发布时，您可以使用此命令更新项目的结构/脚手架：

```py
putup --update my_project
```

*   一次更新只会更新用户不常使用或修改的文件，因此，要更新所有文件，使用强制更新：
...
*   使用 `git status` 检查未跟踪的文件，并用 `git add` 添加它们。
...
*   还要运行 `python setup.py build_sphinx` 和 `python setup.py test`，以检查 `Sphinx` 和 `pytest` 是否正确运行。