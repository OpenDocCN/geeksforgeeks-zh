...
> 原文：[https://www.geeksforgeeks.org/__subclasscheck__-and-__subclasshook__-in-python/](https://www.geeksforgeeks.org/__subclasscheck__-and-__subclasshook__-in-python/)
...
## Python中的`__subclasshook__`方法

抽象类可以覆盖`__subclasshook__()`方法自定义`issubclass()`。当发现一个类是ABC类的子类时，它返回`True`，如果不是，它返回`False`，如果子类检查以通常的机制继续，它返回“未实现”。这个方法是在ABC类中定义的，有一些条件。符合这些条件的类被认为是子类。
