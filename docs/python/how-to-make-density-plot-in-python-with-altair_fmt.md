...
> 原文：[`https://www.geeksforgeeks.org/how-to-make-density-plot-in-python-with-altair/`](https://www.geeksforgeeks.org/how-to-make-density-plot-in-python-with-altair/)
...
*   [`Altair`](https://altair-viz.github.io/) - 是基于 Vega 和 Vega-lite 的统计可视化库。
...
如您所见，数据集中有七列。我们将使用`charges`绘制密度图。为此，我们必须首先将数据转换为密度。这是通过使用`transform_density()`函数来完成的。参数是感兴趣的变量和表示转换变量的名称，转换变量写成`as_=['charges', 'density']`。把它放在一起-
