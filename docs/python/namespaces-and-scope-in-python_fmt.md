...
> 原文：[`https://www.geeksforgeeks.org/namespaces-and-scope-in-python/`](https://www.geeksforgeeks.org/namespaces-and-scope-in-python/)
...
但是在某些情况下，您可能只对更新或处理全局变量感兴趣，如下面的示例所示，您应该将其显式标记为全局变量，并且更新或处理。请注意，行 `count = count +1` 引用了全局变量，因此使用了全局变量，但将其与写为 `count = 1` 的同一行进行比较。那么根据范围规则，绝对需要行 `global count`。
