...
> 原文:[https://www.geeksforgeeks.org/lollipop-graph-in-python-using-networkx-module/](https://www.geeksforgeeks.org/lollipop-graph-in-python-using-networkx-module/)

棒棒糖图由两部分组成一个完整的称为团的图和一个路径图。更准确地说，`L_(m，n)` 是具有 `m` 节点完全图和 `n` 节点路径图的图。
...
我们将使用 `networkx` 模块来实现棒棒糖图。它带有一个内置函数 `networkx.lollipop_graph()`，可以使用 `networkx.draw()` 方法进行说明。Python 中的这个模块用于可视化和分析不同类型的图形。

> **语法:** `networkx.lollipop_graph(m，n)`
>
> **参数:**
>
> *   `m`:完全图(团)中的节点数
...
*   我们将导入所需的模块 `networkx`
*   然后我们将使用 `networkx.lollipop_graph(m，n)` 创建一个 graph 对象。
...
## 示例 3:不使用 `lollipop_graph()` 函数创建棒棒糖图。

### 进场:

*   我们将把所需的模块 `networkx` 导入为 `nx`。
*   然后我们将手动定义 `lollipop_graph` 函数。
