## Python：使用KNNimputer()进行插补

> 原文：[https://www.geeksforgeeks.org/python-imputation-using-the-knnimputer/](https://www.geeksforgeeks.org/python-imputation-using-the-knnimputer/)
...
> `metric`：用于搜索的距离度量。
> 值：`{nan_euclidean, callable}`，默认：`nan_euclidean`
> `weights`：确定应该在什么基础上处理相邻值。
> 值：`{uniform, distance, callable}`，默认：`uniform`。

### 代码：Python 代码说明 KNNImputer 类
