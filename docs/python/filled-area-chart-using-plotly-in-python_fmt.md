...
> 原文: [https://www.geeksforgeeks.org/filled-area-chart-using-plotly-in-python/](https://www.geeksforgeeks.org/filled-area-chart-using-plotly-in-python/)
...
> 参数:
>
> `data_frame`: 需要传递此参数才能使用列名（而不是关键字名）。类数组和字典在内部被转换成 pandas 数据帧。
>
> `x`, `y`: `data_frame` 中列的名称，或者 pandas Series 或 array_like 对象。此列或 array_like 中的值用于在笛卡尔坐标中沿 x 轴和 y 轴定位标记。
>
