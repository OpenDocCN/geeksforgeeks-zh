...
> 原文: [https://www.geeksforgeeks.org/how-to-make-a-subdomain-scanner-in-python/](https://www.geeksforgeeks.org/how-to-make-a-subdomain-scanner-in-python/)
...
*   为了避免程序在域名子域无效时崩溃，我们必须使用一个 `try-catch` 块来跳过那个无效的子域，并在 `catch` 块的帮助下传递它，并在 `catch` 块的帮助下扫描即将到来的子域，扫描应该在 `requests` 模块的帮助下完成，因为特定的网址获取请求应该根据服务器响应网址发送到服务器。
...
*   为列表中的每个子域运行 `for` 循环，并将子域与 URL 序列中的协议和域名连接起来，并将其存储在名为 `url` 的变量中。
*   现在我们使用 `requests.get()` 函数，在该过程中，通过使用给定的 URL 从给定的服务器中检索信息，如果我们能够从服务器中检索信息，这意味着该子域在该域名下是有效的，否则我们将传递它，因为我们在 `try` 块中使用了 `try` 和 `catch` 块。我们将传递 `requests.get()` 函数，扫描后，我们将打印该 URL，否则我们将在 `catch` 块中捕获并传递。
*   然后创建主功能，在取用户输入的域名。
*   然后以读取模式从存储器中打开子域文本文件列表，以扫描每个子域。
*   在读取模式下打开文件后，我们使用 `splitlines()` 函数将拆分的字符串存储在名为 `sub_dom` 的变量中。
*   现在调用我们创建的绕过域名和子 dom 扫描子域的函数。

**显示文本文件中存在的子域名称，并创建该子域的列表。**
