...
| `PUT` | `POST` |
| --- | --- |
| 向特定资源发出 PUT 请求。如果请求-URI 引用了一个已经存在的资源，将会发生更新操作，否则如果请求-URI 是一个有效的资源 URI(假设允许客户端确定资源标识符)，将会发生创建操作。<br>**示例–**<br>`PUT /article/{article-id}` | POST 方法用于请求源服务器接受包含在请求中的实体，将其作为请求行中由请求-URI 标识的资源的新从属。它本质上意味着 POST 请求-URI 应该是 URI 的集合。<br>**示例–**<br>`POST /articles` |
| PUT 方法是幂等的。因此，如果您多次发送重试请求，这应该相当于单个请求修改。 | POST 不是幂等的。因此，如果您重试请求 N 次，您将在服务器上创建 N 个具有 N 个不同 URIs 的资源。 |
| 当您想要修改已经是资源集合一部分的单个资源时，请使用 PUT。PUT 会覆盖整个资源。如果请求更新部分资源，请使用 `PATCH`。 | 当您想要在资源集合下添加子资源时，请使用 POST。 |
| 通常，在实践中，总是使用 PUT 进行更新操作。 | 始终使用 POST 进行创建操作。 |