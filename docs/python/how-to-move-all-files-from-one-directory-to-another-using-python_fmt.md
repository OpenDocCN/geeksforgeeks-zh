...
> 原文：[https://www.geeksforgeeks.org/how-to-move-all-files-from-one-directory-to-another-using-python/](https://www.geeksforgeeks.org/how-to-move-all-files-from-one-directory-to-another-using-python/)
...
## 方法 1：使用 `os` 模块中的 `rename()` 方法

`rename()` 方法接受两个参数，第一个是源路径，第二个是目标路径，`rename` 函数会将源路径的文件移动到提供的目标。

首先导入 `os` 模块，存储源目录的路径和目的目录的路径，使用 `os` 模块中的 `listdir()` 方法，将源目录中的所有文件做一个列表。现在使用 `rename()` 方法逐个移动列表中的所有文件。
