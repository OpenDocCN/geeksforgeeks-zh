...
> 原文: [https://www.geeksforgeeks.org/grant-mysql-table-and-column-permissions-using-python/](https://www.geeksforgeeks.org/grant-mysql-table-and-column-permissions-using-python/)
...
上述用户可以登录 MySQL 服务器，但不能进行任何操作，如查询数据和从表中选择数据库。在我们的案例中，用户名为 `geeksforgeeks`，密码为 `1234`。
...
**注意:** 要授予用户 `geeksforgeeks` 权限，您必须登录 `root` 账户。用户不能授予自己权限。

下面是 Python 程序给用户 `geeksforgeeks` 添加表和列权限:
...
我们可以看到，对 `CREATE` 和 `ALTER` MySQL 表的权限已经提供给了用户 `geeksforgeeks`。