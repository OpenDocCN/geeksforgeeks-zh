...
> 原文：[https://www.geeksforgeeks.org/environment-variables-in-python/](https://www.geeksforgeeks.org/environment-variables-in-python/)
...
该变量用于设置标准 Python 库的默认位置。默认情况下，Python 在 `prefix/lib/pythonX.Y` 和 `exec_prefix/lib/pythonX.Y` 中搜索其库，此处 `prefix` 和 `exec_prefix` 为安装相关目录，均默认为 `/usr/local`。

### `PYTHONSTARTUP`

每当解释器第一次初始化时，Python 都会在 Unix 中寻找一个名为 `.pythonrc.py` 的可读文件，并执行其中的命令。同一文件的路径存储在 `PYTHONSTARTUP` 变量中。这些文件负责设置 `PYTHONPATH`。

### `PYTHONINSPECT`

如果 `PYTHONINSPECT` 变量被设置（非空字符串），它会迫使 Python 进入交互式模式，并忽略 `PYTHONSTARTUP` 文件。还可以对 Python 代码进行更改，并在程序终止时强制其进入检查模式。这相当于使用 `-i` 命令行选项。

### `PYTHONCASEOK`

这个环境变量用于在调用 Python 解释器时忽略所有导入语句中的大小写。在 Windows 机器中，它用于在导入语句中查找第一个不区分大小写的匹配。
...
网上有很多 IDE，比如 VSCode、Sublime Text、PyCharm 等。这里我们将使用 Jupyter Notebook 演示 Python 的使用，这是一个来自 Anaconda 的 Python IDE。
