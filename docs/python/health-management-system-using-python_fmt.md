...
> 原文：[https://www.geeksforgeeks.org/health-management-system-using-python/](https://www.geeksforgeeks.org/health-management-system-using-python/)
...
### `selectname()`
该功能允许用户选择需要执行任务的人员。这个函数首先给出两个人之间的选择，并将输入作为一个整数。然后，它被返回进行进一步处理。
...
### `select_file_action()`
这个函数接受应该对文件做什么，即文件应该用于写入数据(日志)还是检索现有数据。这再次采用整数参数并返回它们。
...
### `select_task()`
此功能有助于指定与必须输入的任务相关的数据。这里给出的选择是在食物和运动之间。这再次接受整数输入，并将其返回进行进一步处理。
...
[output.mp4](https://media.geeksforgeeks.org/wp-content/uploads/20210401092452/output.mp4)