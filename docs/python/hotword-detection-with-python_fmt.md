...
> 原文: [https://www.geeksforgeeks.org/hotword-detection-with-python/](https://www.geeksforgeeks.org/hotword-detection-with-python/)
...
3.  从[这里](http://ftp5.gwdg.de/pub/linux/archlinux/community/os/x86_64//python-snowboy-1.3.0-1-x86_64.pkg.tar.xz)领取雪球包裹。该文件适用于所有基于 Linux 的操作系统。
    雪球也支持树莓 Pi 的所有版本(1、2、3 和零)。支持的操作系统是 Raspbian 8.0。
4.  当你用一个名为 `usr` 的文件夹解压时。现在导航到 `usr/lib/python3.7/site-packages`。
5.  使用文件管理器或 `cp -r` 命令将两个文件夹(`snowboy` 和 `snowboy-1.2.0b1-py3.7.egg-info`)复制到 `/usr/lib/python3.7/site-packages`。
