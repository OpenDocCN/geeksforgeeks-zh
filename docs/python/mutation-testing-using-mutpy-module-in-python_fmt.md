...
> 原文: [https://www.geeksforgeeks.org/mutation-testing-using-mutpy-module-in-python/](https://www.geeksforgeeks.org/mutation-testing-using-mutpy-module-in-python/)
...
现在，我们对一个程序执行突变测试，这个程序检查一个数是否是质数。
...
现在，我们需要使用 **Pytest** 或 **Unittest** 库为上述程序编写测试用例。测试用例应该用杀死所有突变的方法来编写，即测试用例应该足够有效以给出一个好的突变分数。测试用例是使用如下所示文件中的 `unittest` 库编写的。

测试用例是使用 `self.assertEqual()` 编写的，这是一个测试断言，通常用于确定测试用例是通过还是失败。我们在下面编写了三个测试函数来检查三种类型的输入:

1.  输入是质数。
2.  输入是非质数。
3.  无效输入。

**注意:** 函数名和测试文件名都要以 `test` 开头。
...
*   **Target:** 将运行测试用例的目标文件，在我们的例子中是 `isprime.py`。
*   **Unit test:** 包含要执行的单元测试的文件，即我们例子中的 `test_isprime.py`。
*   **Runner:** 用于执行测试的测试运行器，例如 `pytest` 或 `unittest`。
