...
> 原文：[https://www.geeksforgeeks.org/python-levy_stable-distribution-in-statistics/](https://www.geeksforgeeks.org/python-levy_stable-distribution-in-statistics/)

`scipy.stats.levy_stable()`是Levy稳定的连续随机变量。它继承自泛型方法，作为`rv_continuous`类的实例。它用特定于这个分布的细节来完成这些方法。
...
- `loc`：[可选] 位置参数。默认= 0
- `scale`：[可选] 比例参数。默认值= 1
- `size`：[整数元组，可选] 形状或随机变量。
- `moments`：[可选] 由字母`['mvsk']`组成；`'m'`=均值，`'v'`=方差，`'s'`=Fisher偏斜度，`'k'`=Fisher峰度。(默认值= `'mv'`)。

**结果：** Levy稳定连续随机变量

## 代码#1：创建Levy稳定的连续随机变量
