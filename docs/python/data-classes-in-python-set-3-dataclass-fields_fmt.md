...
> 原文：[https://www.geeksforgeeks.org/data-classes-in-python-set-3-dataclass-fields/](https://www.geeksforgeeks.org/data-classes-in-python-set-3-dataclass-fields/)
...
> `dataclass.field(*, default=MISSING, default_factory=MISSING, repr=True, hash=None, init=True, compare=True, metadata=None)`

`MISSING` 值是用于检测是否提供了 `default` 和 `default_factory` 参数的监控对象。使用此哨兵是因为 `None` 是 `default` 的有效值。没有代码应该直接使用 `MISSING` 值。

**默认值：** 如果在对象创建过程中没有提供值，该参数指定属性的默认值。与函数中的参数类似，具有默认值的字段必须位于任何没有默认值的字段之后。
...
**`default_factory`：** 如果提供，它必须是一个零参数可调用函数，当该字段需要默认值时将调用该函数。
可调用的返回值将被设置为对象创建中属性的默认值。
要么为默认工厂提供一个可调用的，要么为属性提供一个默认值。同时指定 `default` 和 `default_factory` 是错误的。
...
`init`、`repr` 和 `hash` 参数类似于[上一篇文章](https://www.geeksforgeeks.org/data-classes-in-python-set-2-decorator-parameters/)中讨论的 `dataclass` 函数中的参数。`compare` 参数可以与 `order` 相关，就像在 `dataclass` 函数中一样。不同之处在于它们只适用于特定的属性，而不是装饰器下的数据类中的所有属性。
