...
`shutil`模块提供了很多对文件和文件集合进行高级操作的功能。它属于 Python 的标准实用程序模块。该模块有助于文件和目录复制和删除过程的自动化。
...
> **`src`:** 代表源目录路径的字符串。
> **`dst`:** 代表目的地路径的字符串。
> **`symlinks`(可选):**此参数接受 `True` 或 `False`，具体取决于原始链接或链接链接的元数据将被复制到新的树中。
> **`ignore`(可选):**如果给定了 `ignore`，它必须是一个可调用的，它将接收 `copytree()`访问的目录作为参数，以及 `os.listdir()`返回的目录列表。
> **`copy_function`(可选):**此参数默认值为 `copy2`。对于这个参数，我们可以使用像 `copy()`这样的其他复制函数。
> **`ignore_dangling_symlinks`(可选):**此参数值设置为真时，用于在符号链接所指向的文件不存在时对引发的异常进行静默。
