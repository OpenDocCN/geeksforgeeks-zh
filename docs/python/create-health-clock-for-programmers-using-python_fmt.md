...
> 原文：[https://www.geeksforgeeks.org/create-health-clock-for-programmers-using-python/](https://www.geeksforgeeks.org/create-health-clock-for-programmers-using-python/)
...
*   我们将从 `pygame` 模块导入 `mixer` 播放我们的声音，从 `time` 模块导入 `time` 设置时间间隔，从 `time` 模块导入 `sleep`，将节目冻结一个相应的时间间隔。
...
*   在每一个 `if` 的条件下，我们都会做同样的事情，即调用 `musicloop()` 函数播放音乐，要求用户做相应的事情来停止音乐，并在文件中记录（运动完成的）时间，以便最终显示出来。
