...
在本文中，我们将看到如何在 `mahotas` 中进行高斯滤波。为此，我们将使用来自核分割基准的荧光显微图像。我们可以借助下面给出的命令获得图像：

```py
mahotas.demos.nuclear_image()
```

**高斯滤波器**是线性滤波器。它通常用于模糊图像或减少噪声。如果您使用其中的两个并减去，您可以将它们用于“反锐化掩模”（边缘检测）。高斯滤波器本身会模糊边缘并降低对比度。
下图是核图像：
...
> **语法：** `mahotas.gaussian_filter(nuclear, 20)`
