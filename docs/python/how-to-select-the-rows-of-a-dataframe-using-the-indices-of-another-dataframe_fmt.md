...
> 原文: [https://www.geeksforgeeks.org/how-to-select-the-rows-of-a-dataframe-using-the-indices-of-another-dataframe/](https://www.geeksforgeeks.org/how-to-select-the-rows-of-a-dataframe-using-the-indices-of-another-dataframe/)

**先决条件:**
*   [Pandas](https://www.geeksforgeeks.org/pandas-tutorial/)
*   [random](https://www.geeksforgeeks.org/random-numbers-in-python/)

使用 Pandas 模块，可以使用另一个数据框中的索引从一个数据框中选择行。本文对此进行了详细讨论。建议在 Jupyter Notebook 中实现所有代码，以便于操作。

## 方法:
*   导入模块。
*   创建第一个数据框。在下面给出的示例中，`choice()`、`randint()` 和 `random()` 都属于 `random` 模块，用于生成数据框。

**1) `choice()` –** `choice()` 是 Python 编程语言中的一个内置函数，它从列表、元组或字符串中返回一个随机项。

> **语法:** `random.choice(序列)`
>
> **参数:** `序列` 是一个强制参数，可以是列表、元组或字符串。
>
> **返回:** `choice()` 返回随机项目。
...
*   现在我们将使用 `dataframe.loc[]` 函数，使用第二个数据帧的索引来选择第一个数据帧的行值。Pandas `DataFrame.loc[]` 属性通过给定数据帧中的标签或布尔数组访问一组行和列。
