...
> 原文：[https://www.geeksforgeeks.org/multiple-exception-handling-in-python/](https://www.geeksforgeeks.org/multiple-exception-handling-in-python/)
...
除非语句可以按照下面给出的代码重写。这是可行的，因为`OSError`是一个基类，对于`FileNotFoundError`和`PermissionError`异常是通用的。
...
虽然根据`as`处理多个异常并不具体，但值得注意的是，可以使用它们作为关键字来处理抛出的异常，如下面给出的代码所示。
