...
*   导入模块
*   打开图像
*   使用 `Resize()` 功能调整图像大小。两个调整大小的图像应该具有相同的宽度和高度，以便它们的纵横比保持不变，并且可以粘贴到新的背景图像中。
*   为了创建一个新的图像，它有一个 `new()` 函数，该函数有 3 个参数(“模式”、“大小”、“颜色”)
*   使用 `paste()` 将图像粘贴到新图像
...
*   新图像的大小应为(宽度，2 *高度)。
*   创建新图像后，使用 `paste()` 粘贴第一个图像，并传递参数(`img_resize`，(位置))#position(0，0)
*   粘贴第一个图像后，使用 `paste` 粘贴第二个图像，并传递参数(`img1_reszie`，(位置))。在位置上，宽度将是相同的，但高度将是第一个图像高度的最后一个位置。。#位置=(0，180)
