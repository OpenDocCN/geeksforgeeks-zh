# 在 Python Arcade 中添加背景图像

> 原文: [https://www.geeksforgeeks.org/add-backgound-image-in-python-arcade/](https://www.geeksforgeeks.org/add-backgound-image-in-python-arcade/)
...
> 语法: `arcade.load_texture(name, x, y, width, height)`
>
> 参数:
>
> *   `name`: 保存纹理的文件的名称。
...
> 语法: `arcade.draw_texture_rectangle(x, y, width, height, texture, angle, alpha)`
>
> 参数:
>
> *   `x`: 矩形中心的 x 坐标。
...
> *   `width`: 纹理的宽度。
> *   `height`: 纹理的高度。
> *   `texture`: 从 `load_texture()` 调用返回的纹理的标识符。
> *   `angle`: 矩形的旋转。
> *   `alpha`: 图像的透明度。
