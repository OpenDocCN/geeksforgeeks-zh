...
> 原文: [https://www.geeksforgeeks.org/python-index-ranks-of-elements/](https://www.geeksforgeeks.org/python-index-ranks-of-elements/)
...
**例: 使用** `循环` + `过滤()` + `列表理解()` + `集合()` + `求和()` + `循环`

在这种情况下，使用`过滤器()`和`列表理解`来提取元素的索引。`set()`用于获取列表中存在的唯一数字。`sum()`用于计算所有指数的总和。
