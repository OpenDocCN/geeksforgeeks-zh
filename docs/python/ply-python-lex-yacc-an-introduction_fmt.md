...
> 原文: [https://www.geeksforgeeks.org/ply-python-lex-yacc-an-introduction/](https://www.geeksforgeeks.org/ply-python-lex-yacc-an-introduction/)
...
要在 python 代码中导入该模块，请使用 `import ply.lex as lex`。
...
您可以使用以下方法将 `yacc` 导入您的 python 代码 `import ply.yacc as yacc`。
...
`yacc.py` 也使用显式 `token()` 来收集令牌和语法规则，`yacc.py` 会根据用户需求不断调用该令牌。`yacc.py` 吐出抽象语法树 (`AST`) 作为输出。

## 优于 UNIX yacc

Python 实现 `yacc.py` 不涉及代码生成过程，而是使用 **反射** 来制作其 lexers 和 parsers，这节省了空间，因为它不需要任何额外的编译器构造步骤和代码文件生成。
要从您的 lex 文件中导入令牌，请使用 `from lex_file_name_here import tokens`，其中 `tokens` 是 lex 文件中指定的令牌列表。
...
**参考文献:** [https://www.dabeaz.com/ply/ply.html](https://www.dabeaz.com/ply/ply.html)