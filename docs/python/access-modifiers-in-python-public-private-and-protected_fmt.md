...
> 原文: [https://www.geeksforgeeks.org/access-modifiers-in-python-public-private-and-protected/](https://www.geeksforgeeks.org/access-modifiers-in-python-public-private-and-protected/)
...
在上面的程序中，`accessProtectedMembers()` 方法是类 `Sub` 的公共成员函数，访问类 `Super` 的受保护成员函数 `_displayProtectedMembers()` 方法，而 `accessPrivateMembers()` 方法是类 `Super` 的公共成员函数，访问类 `Super` 的私有成员函数 `__displayPrivateMembers()` 方法。