...
## register()
`register()`函数接受一个函数作为参数，该函数将在解释器终止时执行。如果传递了多个函数作为参数（例如 `fun1()`, `fun2()`...），那么它们的执行顺序将是逆序的（...`fun2()`, `fun1()`）。执行遵循后进先出（LIFO）的概念。
...
## unregister()
`unregister()` 函数从程序中定义的函数中移除指定的 `fun`。它确保在解释器终止时不会调用该 `fun`。
