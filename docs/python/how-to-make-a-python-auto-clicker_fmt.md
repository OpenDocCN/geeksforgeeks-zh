...
> 原文：[https://www.geeksforgeeks.org/how-to-make-a-python-auto-clicker/](https://www.geeksforgeeks.org/how-to-make-a-python-auto-clicker/)
...
在这个项目中，我们将使用跨平台模块 `pynput` 来控制鼠标，同时监控键盘，以创建简单的自动点击器。为了检查鼠标事件，我们将为此执行安装 `pynput` 模块（用于控制鼠标），在 `cmd` 中执行 `pip install pynput`。
...
验证 `pynput` 模块是否已成功安装到您的工作环境中。为此，请在系统上打开 IDLE，即 `cmd` 或 `Python Shell`。执行命令 `import pynput`，执行此命令后，输出应为零错误，这意味着您的模块已成功安装。
...
**第一步：** 导入时间和线程，然后从 `pynput.mouse` 模块导入 `Button` 和 `Controller`。从 `pynput.keyboard` 导入 `Listener` 和 `KeyCode`。
...
*   `delay`：每次点击之间的延迟（秒）
*   `button`：按钮是用来点击你想点击的任何方向。`Button.left` | `Button.middle` | `Button.right`
*   `start_stop_key`：运行自动点击器执行程序时，用于点击开始和停止的键。它应该来自一个 `Key` 类或使用 `KeyCode` 设置。
*   `stop_key`：用于终止正在执行的自动点击器的键。这应该来自 `Key` 类或使用 `KeyCode` 设置。
...
**步骤 3：** 创建一个扩展 `threading.Thread` 的类。将 `delay` 和 `button` 传递给类，该类有两个标志以检查程序是否执行。
...
**步骤 5：** 当线程开始时，创建一个方法 `run`，`program_running` 在循环上运行，直到该值为假，并在现有循环内创建另一个循环，检查 `running` 是否设置为真。在这种情况下，我们都在两个环路内，它会在设置按钮上 `click`，并休眠设置的 `delay`。
...
**步骤 6：** 为鼠标控制器创建一个实例，然后创建 `ClickMouse` 线程。启动实例，进入 `run` 方法内部的循环。
...
**第七步：** 创建一个方法 `on_press`，该方法以一个键为参数，设置一个键盘监听器。执行时 `start_stop_key` 与 `KeyCode(char='a')` 匹配。那么当线程中的 `running` 标志设置为真时，点击将被终止。如果执行 `stop_key` 并停止监听器，则在方法中调用 `exit` 方法。
