...
> 原文: [https://www.geeksforgeeks.org/python-decimal-copy_sign-method/](https://www.geeksforgeeks.org/python-decimal-copy_sign-method/)

`Decimal.copy_sign()` 是一个 `Decimal` 类方法，返回第一个 `Decimal` 值的副本，符号设置为与第二个 `Decimal` 值的符号相同。
...
返回第一个 `Decimal` 值的副本，其符号被设置为与第二个 `Decimal` 值的符号相同。

## 代码示例 1：copy_sign() 方法示例
