...
> 原文：[https://www.geeksforgeeks.org/check-the-version-of-the-python-interpreter/](https://www.geeksforgeeks.org/check-the-version-of-the-python-interpreter/)

在本文中，我们将了解系统中 `python` 解释器的当前版本。获得解释器的版本真的很重要，因为 `python` 解释器在计算机编程和源代码编码行业中被广泛使用。它接受一个交互命令并执行它。

因为解释器是获取代码并帮助执行的人。有时由于旧版本的解释器，`making` 命令无法正常工作，并且正如我们所知，`python` 语言经常更新版本，增加了一些新功能，因此用户必须用最新的发布版本更新解释器。
...
这个函数可以通过导入 `platform` 库来访问，并且总是返回一个运行用户的 `python` 版本的字符串。为了更好地理解，可以采取以下步骤：
