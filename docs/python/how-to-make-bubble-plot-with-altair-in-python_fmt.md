...
`Altair`是一个简单易用的 python 统计可视化库。它包含许多类型的内置地块和各种选项来修改属性和生成其他地块。**气泡图**对于第三个变量的数据的二元分析是非常有用的可视化。它不容易在阿尔泰库中找到，但可以通过对散点图进行一些简单的修改来实现。
...
*   **颜色**:通过设置 `mark_point()` 方法的 `color` 参数，可以改变气泡的默认颜色。
*   **不透明度**:通过设置 `mark_point()` 方法的 `opacity` 参数，可以改变气泡的默认不透明度。范围从 0 到 1。
*   **填充**:默认为假，但可以将 `filled` 参数改为真，从而用指定的颜色填充气泡。
