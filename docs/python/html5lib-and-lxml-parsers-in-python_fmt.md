...
> 原文: [https://www.geeksforgeeks.org/html5lib-and-lxml-parsers-in-python/](https://www.geeksforgeeks.org/html5lib-and-lxml-parsers-in-python/)
...
一个纯 Python 库，用于解析 HTML。它的设计符合 WHATWG HTML 规范，所有主要的网络浏览器都是这样实现的。

### lxml

一个针对 C 库 `libxml2` 和 `libxslt` 的成熟 Pythonic 绑定。它的独特之处在于，它将这些库的速度和 XML 特性的完整性与本机 Python 应用编程接口的简单性相结合，该接口大多与众所周知的 `ElementTree` 应用编程接口兼容，但优于后者。
...
*   极其宽大。
*   非常慢。为什么呢？因为它有大量 Python 代码做后盾。

### lxml

*   非常快。为什么呢？因为它得到了许多 C 代码的支持。
