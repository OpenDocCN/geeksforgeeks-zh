...
*   `Logger.warning(msg)`: 这将在该记录器上记录一条警告级别的消息。
*   `Logger.error(msg)` : 这将在该记录器上记录一条错误级别的消息。
*   `Logger.critical(msg)` : 这将在该日志记录程序上记录一条级别为 critical 的消息。
*   `Logger.log(lvl, msg)` : 这将在此 Logger 上记录一条具有整数级 lvl 的消息。
*   `Logger.exception(msg)`: 这将在该记录器上记录一条错误级别的消息。
*   `Logger.setLevel(lvl)` : 此功能将此 Logger 的阈值设置为 lvl。这意味着低于此级别的所有消息都将被忽略。
*   `Logger.addFilter(filter)`: 这将一个特定的筛选器 filter 添加到此 logger。
*   `Logger.removeFilter(filter)`: 这将特定的筛选器 filter 删除到该日志程序。
*   `Logger.filter(record)`: 此方法将记录器的过滤器应用于所提供的记录，如果要处理记录，则返回真。否则，它将返回 False。
*   `Logger.addHandler(hdlr)` : 这将一个特定的处理程序 hdlr 添加到该记录器的中。
*   `Logger.removeHandler(hdlr)`: 这将特定的处理程序 hdlr 移至该记录器。
...
3.  此外，可以根据分配给每个级别的数值设置记录器的级别，作为跟踪的阈值。
    有几个属性可以作为参数传递。
4.  所有这些参数的列表在 [Python Library](https://docs.python.org/3/library/logging.html#logrecord-attributes) 中给出。用户可以根据需要选择所需的属性。
