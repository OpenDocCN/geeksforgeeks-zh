...
> 原文：[https://www.geeksforgeeks.org/how-to-store-python-functions-in-a-sqlite-table/](https://www.geeksforgeeks.org/how-to-store-python-functions-in-a-sqlite-table/)
...
### 步骤 2
创建游标对象并调用其 `execute()` 方法来创建一个表。
```py
c = conn.cursor()
c.execute('''CREATE TABLE pyfuction
             (func_defination TEXT)''')
```
初始化数据库后，我们必须使用 `conn.cursor()` 创建游标对象。游标允许我们执行 SQL 命令。
