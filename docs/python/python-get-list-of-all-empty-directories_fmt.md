...
> 原文: [https://www.geeksforgeeks.org/python-get-list-of-all-empty-directories/](https://www.geeksforgeeks.org/python-get-list-of-all-empty-directories/)

Python 中的 [`os`](https://www.geeksforgeeks.org/os-module-python-examples/) 模块用于与操作系统交互。该模块附带 Python 的标准实用程序模块，因此无需从外部安装。在文件名和路径无效或不可访问的情况下，或者在其他具有正确类型但不被操作系统接受的参数的情况下，`os` 模块中的所有功能都会引发 `OSError`。
...
> **参数:**
> `top`: 起始目录为 `os.walk()`。
> `topdown`: 如果该可选参数为真，则从上而下扫描目录，否则从下而上扫描目录。默认情况下，这是真的。
> `onerror`: 它是一个处理可能出现的错误的函数。
> `followlinks`: 如果设置为真，这将访问符号链接所指向的目录。
>
...
以上代码可以使用 [列表理解](https://www.geeksforgeeks.org/python-list-comprehension-and-slicing/) 来缩短，这是一种比较 Pythonic 的方式。下面是实现。
