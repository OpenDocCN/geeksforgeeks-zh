...
`JSON`（JavaScript Object Notation）是一个主要用于存储和传输数据的文件，主要用于服务器和 web 应用程序之间。它通常用于表示结构化数据。在本文中，我们将讨论如何使用 Python 处理 `JSON` 数据。Python 提供了一个名为 `json` 的模块，该模块附带了 Python 的标准内置实用程序。
...
`json` 模块的 [`load()`](https://www.geeksforgeeks.org/json-load-in-python/) 和 [`loads()`](https://www.geeksforgeeks.org/json-loads-in-python/) 功能使得解析 `JSON` 对象变得更加容易。
