...
> 原文: [https://www.geeksforgeeks.org/build-an-application-to-extract-url-and-metadata-from-a-pdf-using-python/](https://www.geeksforgeeks.org/build-an-application-to-extract-url-and-metadata-from-a-pdf-using-python/)

PDF(可移植文档格式)是 Adobe 开发的最常用的独立于平台的文件格式，用于呈现文档。Python 有很多 PDF 相关的包，其中之一就是 `pdfx` 模块。`pdfx` 模块用于从给定的 PDF 或 PDF 网址中提取网址、元数据和纯文本。
...
*   Use `get_metadata()` method to get metadata.
*   用 `Get_references_as_dict()` 方法获取网址.
