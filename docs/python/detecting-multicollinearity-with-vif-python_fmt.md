...
> 原文：[https://www.geeksforgeeks.org/detecting-multicollinearity-with-vif-python/](https://www.geeksforgeeks.org/detecting-multicollinearity-with-vif-python/)
...
`statsmodels` 提供了一个名为 `variance_inflation_factor()` 的函数来计算 `VIF`。

> **语法：** `statsmodels.stats.outliers_influence.variance_inflation_factor(exog, exog_idx)`
>
> **参数：**
>
> *   `exog`：包含执行线性回归的特征的数组。
...
以下示例中使用的数据集包含 500 人的身高、体重、性别和体重指数。这里因变量是 `Index`。
...
*   每个特征 `Index` 被传递到 `variance_inflation_factor()` 以找到相应的 `VIF`。
*   这些值以 `pandas` 数据帧的形式存储。
...
我们可以看到，`Height` 和 `Weight` 都有很高的 `VIF` 值，说明这两个变量高度相关。这是意料之中的，因为一个人的身高确实会影响他们的体重。因此，将这两个特征放在一起考虑会得到一个具有高度多重共线性的模型。