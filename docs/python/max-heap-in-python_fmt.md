...
一个[Max-Heap](https://www.geeksforgeeks.org/binary-heap/)是一个完整的二叉树，其中每个内部节点中的值大于或等于该节点的子节点中的值。
...
2.  `extractMax()`:从最大堆中移除最大元素。该操作的时间复杂度为`O(Log n)`，因为该操作需要在移除根后维护堆属性(通过调用`heapify()`)。
3.  `insert()`:插入新钥匙需要`O(Log n)`时间。我们在树的末端添加一个新的键。如果新密钥比它的父密钥小，那么我们不需要做任何事情。否则，我们需要遍历以修复违反的堆属性。

**注意:**在下面的实现中，我们从索引1开始做索引，以简化实现。
