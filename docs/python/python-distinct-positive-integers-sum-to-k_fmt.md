...
> 原文: [https://www.geeksforgeeks.org/python-distinct-positive-integers-sum-to-k/](https://www.geeksforgeeks.org/python-distinct-positive-integers-sum-to-k/)
...
## 方法 2: 使用 `combinations()` + `sum()`

在这种情况下，我们使用 `combinations()` 获得元素，并使用 `sum()` 来检查求和，这不会以贪婪但随机的方式获得所需的求和。
