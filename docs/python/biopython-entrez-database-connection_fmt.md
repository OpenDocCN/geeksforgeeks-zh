...
> 原文: [https://www.geeksforgeeks.org/biopython-entrez-database-connection/](https://www.geeksforgeeks.org/biopython-entrez-database-connection/)
...
Biopython 有一个名为 `Bio.Entrez` 的 Entrez 专用模块。Entrez 模块从 Entrez 搜索系统返回的 XML 文件中提取信息，并将其显示为 Python 字典和列表。连接数据库的步骤如下:
...
*   阅读 `einfo()` 方法提供的信息。
*   这样获得的数据是 XML 格式的，所以要在 Python 对象中获得这些数据，需要使用 `read()` 方法。
*   现在记录是字典格式的，只有一个键。
