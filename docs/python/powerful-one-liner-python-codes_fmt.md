...
> 原文: [https://www.geeksforgeeks.org/powerful-one-liner-python-codes/](https://www.geeksforgeeks.org/powerful-one-liner-python-codes/)
...
我们知道这个事实，但有时我们在翻译其他语言时往往会忽略它。这是两个数字的交换。最天真的做法是:
...
然而，Python 也为此提供了一行代码。Pythonic 的方法是:
...
Lambda 函数是 Python 单行函数，在要计算表达式时经常使用。
...
列表推导式是创建列表的简洁方式。我们可以利用列表推导式来代替通常的方法。
...
使用 `if-else` 或 `while` 循环时，此技巧可能会有所帮助。与其这样做–
...
在 Python 中有各种方法可以反转列表。Pythonic 反转列表的方法:
...
求阶乘。寻找阶乘的正常方法是迭代到那个数，并在每次迭代中乘以这个数。
...
我们可以使用 `math.factorial(x)` –它返回 `x` 的阶乘。但是如果数字是负的或非整数，它会引起 `ValueError`。
...
还有一种简洁地求阶乘的方法是使用 `reduce()` 和 `lambda` 函数。
