...
正如我们所看到的，`innerFunction()`可以很容易地在`outerFunction`主体内部访问，但不能在它的主体外部访问。因此，在这里，`innerFunction()`被视为嵌套函数，它使用`text`作为非局部变量。
...
2.  函数`innerFunction`的范围仅在外部函数内。但是通过使用闭包，我们可以很容易地扩展它的范围来调用它范围之外的函数。
