...
> 原文：`https://www.geeksforgeeks.org/choose-elements-from-list-with-different-probability-in-python/`
...
在这种情况下对我们有帮助的函数是 `random.choices()`。该功能允许在 Python 中进行带替换的加权随机选择。

**语法：**

> `random.choices(population, weights=None, *, cum_weights=None, k=1)`

这里，`weights` 参数起着重要的作用。
...
在上面的例子中，我们给列表的每个元素分配权重。元素 `13` 的权重最高，为 55，因此其出现的概率最大。正如我们在输出中看到的，元素 13 出现了 3 次，19 出现了 2 次，以此类推。所以，现在从列表中选择一个元素的概率是不同的。
...
一个元素的累计权重是通过将它的前一个元素的权重和它自己的权重相加来确定的。
...
**注：** `k` 的值取决于用户，由于其相对权重，所以 `weights` 的总和可以超过 100。