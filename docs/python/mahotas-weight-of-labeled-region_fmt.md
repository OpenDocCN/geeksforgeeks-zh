...
> 原文: [https://www.geeksforgeeks.org/mahotas-weight-of-labeled-region/](https://www.geeksforgeeks.org/mahotas-weight-of-labeled-region/)
...
借助 `mahotas.label_sum` 方法，我们可以得到每个区域的大小，这个大小简单地用每个区域的像素数来衡量。相反，我们可以测量每个区域的总重量。

为了做到这一点，我们将使用 `mahotas.label_sum` 方法。

## 语法:
`mahotas.label_sum(数组，labeled_region)`

## 参数:
以两个数组对象为参数，即区域形状随机数组和标记区域。
...
sums = mh.label_sum(array, labeled)
