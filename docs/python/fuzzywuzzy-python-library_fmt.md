...
但是其中一个非常简单的方法是使用 `fuzzywuzzy` 库，在这里我们可以得到 100 分的分数，这表示两个字符串通过给出相似性指数是相等的。本文讨论我们如何开始使用 `fuzzywuzzy` 库。
...
`FuzzyWuzzy` 由 SeatGeek 开发和开源，SeatGeek 是一个寻找体育和音乐会门票的服务。他们最初的用例，在他们的[博客中讨论过。](http://chairnerd.seatgeek.com/fuzzywuzzy-fuzzy-string-matching-in-python/)
...
# Exclamation mark in second string, but still partially words are same so score comes 100

fuzz.partial_ratio("geeks for geeks", "geeks geeks")

# score is less because there is a extra token in the middle middle of the string.
```
...
现在假设我们有一个选项列表，我们想要找到最接近的匹配，我们可以使用 `process` 模块
...
```py
# Python code showing all the ratios together, make sure you have installed fuzzywuzzy module
