...
> 原文：[https://www.geeksforgeeks.org/how-to-check-if-something-is-a-rdd-or-a-dataframe-in-pyspark/](https://www.geeksforgeeks.org/how-to-check-if-something-is-a-rdd-or-a-dataframe-in-pyspark/)

在本文中，我们将使用 `isinstance()`、`type()` 和调度方法检查数据是 RDD 还是数据帧。

## 方法 1：使用 `isinstance()` 方法

它用于检查特定数据是 RDD 还是数据帧。它返回布尔值。

> **语法：** `isinstance(data, DataFrame/RDD)`
>
> 其中：
> - `data` 是我们的输入数据
...
> **语法：** `isinstance(data, RDD)`
>
> 其中：
> 1. `data` 是我们的输入数据
> 2. 从 `pyspark.sql` 模块中导入该方法
