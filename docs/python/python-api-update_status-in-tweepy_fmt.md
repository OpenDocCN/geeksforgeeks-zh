...
> 原文: [https://www.geeksforgeeks.org/python-api-update_status-in-tweepy/](https://www.geeksforgeeks.org/python-api-update_status-in-tweepy/)
...
> *   `media_id`: 要与推文关联的 `media_id` 列表。
> *   `possibly_sensitive`: 如果推文可能包含敏感数据，将其设置为真。
> *   `lat`: 推文的纬度。
> *   `long`: 推文的经度。
> *   `place_id`: 推文的地点名称。
