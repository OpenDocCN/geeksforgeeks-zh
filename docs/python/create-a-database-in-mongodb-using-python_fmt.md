...
> 原文: [https://www.geeksforgeeks.org/create-a-database-in-mongodb-using-python/](https://www.geeksforgeeks.org/create-a-database-in-mongodb-using-python/)
...
在上面的例子中，它清楚地显示了数据库是如何创建的。创建客户端时，本地主机及其端口号(此处为 `27017`)被传递给 `MongoClient`。然后，通过使用 `client`，创建一个名为 `'GFG'` 的新数据库。
