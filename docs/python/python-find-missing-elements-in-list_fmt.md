...
> 原文：[https://www.geeksforgeeks.org/python-find-missing-elements-in-list/](https://www.geeksforgeeks.org/python-find-missing-elements-in-list/)
...
我们也可以使用集合的差集操作来找出缺失的元素。这种方法在列表范围已知时非常高效。
...
# using set difference
# Finding missing elements in List
full_set = set(range(max(test_list) + 1))
missing_set = full_set - set(test_list)
res = list(missing_set)
