...
> 原文: [https://www.geeksforgeeks.org/python-concatenate-all-keys-which-have-similar-values/](https://www.geeksforgeeks.org/python-concatenate-all-keys-which-have-similar-values/)
...
>
> 串联键: `{'gfg-is': frozenset({3, 4, 5}), 'best-for': frozenset({1, 3, 4}), 'geeks': frozenset({1, 2, 3})}`

## 方法二: 使用 `groupby()` + `join()` + 字典理解
...
> 原始字典为: `{'gfg': {3, 4, 5}, 'is': {3, 4, 5}, 'best': {1, 3, 4}, 'for': {1, 3, 4}, 'geeks': {1, 2, 3}}`
>
> 串联键: `{'gfg-is': {3, 4, 5}, 'best-for': {1, 3, 4}, 'geeks': {1, 2, 3}}`