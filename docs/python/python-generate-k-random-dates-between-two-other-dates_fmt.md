...
> 原文: [https://www.geeksforgeeks.org/python-generate-k-random-dates-between-two-other-dates/](https://www.geeksforgeeks.org/python-generate-k-random-dates-between-two-other-dates/)

给定两个日期，任务是编写一个 Python 程序来随机获取 K 个日期。

> **输入:** `test_date1`，`test_date2` = `date(2015, 6, 3)`，`date(2015, 7, 1)`，`K` = 7
...
>
> **输入:** `test_date1`，`test_date2` = `date(2015, 6, 3)`，`date(2015, 8, 1)`，`K` = 6
...
**方法#1: 使用 `choices()` + 循环 + `timedelta()`**

在本例中，我们使用 1 天差的 loop 和 `timedelta()` 提取范围内的所有日期。其中，使用 `choices()` 选择 K 个随机日期。
...
**方法 2: 使用 `randrange()` + `timedelta()` + 循环**
