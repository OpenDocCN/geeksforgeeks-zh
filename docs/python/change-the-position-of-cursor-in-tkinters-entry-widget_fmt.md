# 在 Tkinter 的入口小部件中更改光标的位置

> 原文：[https://www.geeksforgeeks.org/change-the-position-of-cursor-in-tkinters-entry-widget/](https://www.geeksforgeeks.org/change-the-position-of-cursor-in-tkinters-entry-widget/)

`tkinter` 中的一个小部件，用于输入一行文本的显示，称为入口（`Entry`）小部件。您是否在 `tkinter` 中创建了一个包含大量信息的入口小部件？大量文本是否会导致您在其中的任何其他位置出现问题？别担心，我们有解决这个问题的办法。你需要做的只是设置一个按钮，当你点击这个按钮时，它会把你带到入口小部件中想要的位置。
