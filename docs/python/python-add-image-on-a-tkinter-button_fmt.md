...
可以借助`PhotoImage()`方法添加图像。这是一个 Tkinter 方法，意味着您不必导入任何其他模块来使用它。

**重要提示:**如果在[按钮](https://www.geeksforgeeks.org/python-creating-a-button-in-tkinter/)上同时给出图像和文本，则文本将占主导地位，按钮上将只出现图像。但是如果你想同时显示图像和文本，那么你必须在按钮选项中通过`compound`参数。
