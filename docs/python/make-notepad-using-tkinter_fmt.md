...
**注意:** `messagebox` 用于将消息写在名为记事本的白色框中，`filedialog` 用于当您从系统中的任何位置打开文件或将文件保存在特定位置或地方时，对话框出现。
...
要运行此代码，请通过扩展名`.py`保存它。然后打开`cmd`（命令提示符），移动到保存文件的位置，然后写下以下内容：

```py
python "filename".py
```

然后按回车键，它就会运行。或者只需双击`.py`扩展名文件即可直接运行。