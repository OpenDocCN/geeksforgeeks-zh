...
> 原文: [https://www.geeksforgeeks.org/parsing-pdfs-in-python-with-tika/](https://www.geeksforgeeks.org/parsing-pdfs-in-python-with-tika/)
...
为了从 PDF 文件中提取内容，我们将使用`parser`对象的`from_file()`方法。所以我们先来看看描述。

> **语法:** `parser.from_file(filename, additional)`
>
> **参数:**
>
> *   `filename`: This is the location of the file. It is opened in rb mode, that is, read binary mode.
> *   `additional`: param service: the service requested from tika server, with the default value of "all", which leads to recursive text content+metadata.
