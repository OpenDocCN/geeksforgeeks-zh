# Python datetime.toordinal()方法示例

> 原文: [https://www.geeksforgeeks.org/python-datetime-toordinal-method-with-example/](https://www.geeksforgeeks.org/python-datetime-toordinal-method-with-example/)
...
使用日期时间模块的 `date.today()` 类，使用 `datetime.toordinal()` 函数返回给定日期时间对象的公历序数。
...
**注意:** 日期时间类的属性应该在给定的范围内，否则会显示 `ValueError`。
...
Traceback (most recent call last):
  File "/home/2ecd5f27fbc894dc8eab3aa6559c7ab.py", line 5, in <module>
    dateIs = datetime(189, 0, 0)
...
使用 `datetime.toordinal()` 函数返回给定 `datetime` 对象的公历序数。
