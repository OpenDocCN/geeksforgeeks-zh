...
> 原文: [https://www.geeksforgeeks.org/how-to-change-a-dictionary-into-a-class/](https://www.geeksforgeeks.org/how-to-change-a-dictionary-into-a-class/)

使用[字典](https://www.geeksforgeeks.org/python-dictionary/)是一件好事，但是当我们有很多字典时，它就变得很难使用。那么让我们来了解一下如何将字典转换成类。

### 接近

让我们以一个简单的字典`my_dict`为例，它将名称、等级和主题作为我的关键字，并将它们对应的值作为 Geeks、1223、Python。我们在这里调用一个函数`Dict2Class`，这个函数将我们的字典作为输入，并将其转换为类。然后，我们使用`setattr()`函数将每个键作为属性添加到类中，从而循环遍历字典。
