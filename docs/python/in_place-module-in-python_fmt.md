...
*   `filename`: 要就地打开和编辑的文件的位置。
*   `mode`: 打开文件的模式。选项有字节(`b`)和文本(`t`)。如果它以字节为单位打开，则以字节为单位进行读写。在文本模式下，R & W 以字符串形式出现。默认为文本模式。
*   `backup`: 进程结束后保存原始文件的路径，以防保留原始文件的备份。
*   `backup_ext`: 编辑前已创建原始文件的备份，名称为 `filename+backup_ext`。
...
上述方法的替代方法是使用关键字 `backup_ext`。到这个上只需要传递额外的单词来区分现有的文件。
