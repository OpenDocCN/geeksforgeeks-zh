...
> 原文: [https://www.geeksforgeeks.org/how-to-download-all-images-from-a-web-page-in-python/](https://www.geeksforgeeks.org/how-to-download-all-images-from-a-web-page-in-python/)
...
*   `bs4`: 美丽汤 (`bs4`) 是一个从 HTML 和 XML 文件中拉出数据的 Python 库。这个模块没有内置 Python。
*   `requests`: `Requests` 可以让你极其轻松地发送 HTTP/1.1 请求。该模块也没有内置 Python。
*   `os`: Python 中的 `os` 模块提供了与操作系统交互的功能。`os` 属于 Python 的标准实用程序模块。该模块提供了一种使用操作系统相关功能的可移植方式。

## 方法

*   导入模块
*   获取 HTML 代码
*   在 BeautifulSoup 中使用 `findAll()` 方法从 HTML 代码中获取 `img` 标签列表。

```py
images = soup.findAll('img')
```

*   在 `os` 中使用 `mkdir()` 方法创建单独的文件夹下载图像。
