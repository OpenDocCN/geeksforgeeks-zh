...
> 原文：[https://www.geeksforgeeks.org/circle-of-squares-using-python/](https://www.geeksforgeeks.org/circle-of-squares-using-python/)

`Turtle`库使用户能够使用命令绘制图片或形状，为他们提供了一个虚拟画布。`Turtle`自带 Python 的标准库。它需要一个支持`Tk`的 Python 版本，因为它使用`Tkit`作为图形。在本文中，我们将使用 Python 的`Turtle`模块从正方形中生成一个圆形图案。
