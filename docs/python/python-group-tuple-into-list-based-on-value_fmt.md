...
> 原文：[https://www.geeksforgeeks.org/python-group-tuple-into-list-based-on-value/](https://www.geeksforgeeks.org/python-group-tuple-into-list-based-on-value/)

有时，在使用 Python 元组时，我们会遇到一个问题，即我们需要根据分配给它的值将元组元素分组到嵌套列表中。这在许多分组应用中非常有用。让我们讨论执行这项任务的某些方法。

## 方法一：使用 `itemgetter()` + 列表理解 + `groupby()`
