...
*   创建一个布尔变量，我们将使用它来运行 `while` 循环，然后创建一个无限循环来运行我们的游戏。将帧率设置为 3fps 只是为了正确查看结果。如果 `value` 变量的值大于子画面列表的长度，则在该 `value` 变量中设置 0。
*   将精灵图像存储在 `image` 变量中，然后创建一个变量来存储起始 `x` 和 `y` 坐标。根据我们的 `value` 变量中存储的值更改 `y` 坐标。
...
*   如果事件的类型是 `KEYUP`，即键盘按钮被释放，检查事件键，如果释放的按钮是左箭头键或右箭头键，将 `moving` 值设置为 `False`，将 `value` 变量的值设置为 0。
*   使用 `key.get_pressed()`方法将按下的键存储在新变量中。更改玩家的 `x` 坐标，并将 `moving` 变量设置为真。如果 `moving` 变量为真，则将 `value` 变量的值增加 1。如果 `value` 变量的值大于子画面列表的长度，则在该 `value` 变量中设置 0。将精灵图像存储在 `image` 变量中，然后缩放图像。