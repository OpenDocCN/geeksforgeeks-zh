...
> 原文：[https://www.geeksforgeeks.org/create-a-telegram-bot-using-python/](https://www.geeksforgeeks.org/create-a-telegram-bot-using-python/)
...
**第三步：** 给你的机器人取一个唯一的名字。命名后，僵尸父亲会询问它的用户名。然后给出一个唯一的名字，但是记住你的机器人的用户名必须以 `bot` 结束，比如 `my_bot`、`hellobot` 等等。

![](img/cada3fb22b51903f2d3b12a7c24690c2.png)

**第四步：** 在给出一个唯一的名字后，如果它被接受，你会收到一条类似如下的信息–
...
*   `Updater`：这将包含我们从 BotFather 获得的 API 密钥，以指定我们使用 python 代码在哪个机器人中添加功能。
*   `Update`：这将在机器人每次收到更新时调用，即消息或命令，并将向用户发送消息。
...
*   `Filters`：这将从发送的消息中过滤正常的文本、命令、图像等。
