...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-bytes-to-int-in-python/](https://www.geeksforgeeks.org/how-to-convert-bytes-to-int-in-python/)
...
>
> *   `bytes` – 一个字节对象
> *   `byteorder` – 确定整数值的表示顺序。`byteorder` 的值可以是 `"big"`，最高有效位存储在开头，最低有效位存储在结尾，也可以是 `"little"`，最高有效位存储在末尾，最低有效位存储在开头。大字节顺序计算基数为 256 的整数值。
> *   `signed` – 默认值为 `False`。指示是否表示数字的二进制补码。
>
> **返回：** 一个等于给定字节的整数
