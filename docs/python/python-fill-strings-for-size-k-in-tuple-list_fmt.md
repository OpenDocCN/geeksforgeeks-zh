...
> 原文: [https://www.geeksforgeeks.org/python-fill-strings-for-size-k-in-tuple-list/](https://www.geeksforgeeks.org/python-fill-strings-for-size-k-in-tuple-list/)

有时，在使用元组列表时，我们会遇到一个问题，即我们需要执行字符串填充来完成列表中的特定大小。这种类型的询问可能发生在数据域和数据预处理中。让我们讨论执行这项任务的某些方法。

> `输入` : `test_list = [('Gfg '，' is ')，(' best '，' for ')，(' CS '，' Geeks')]`，`K = 6`，`fill_char = ' # '`
> `输出` : `[('Gfg### '，' is#### ')，(' best## '，' for # # # # ')，(' CS### '，' Geeks# ')]`
>
