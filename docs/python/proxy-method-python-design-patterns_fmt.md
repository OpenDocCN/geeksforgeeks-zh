...
> 原文：`https://www.geeksforgeeks.org/proxy-method-python-design-patterns/`

代理方法是`结构设计模式`，它允许您为另一个对象提供替换。这里，我们使用不同的类来表示另一个类的功能。最重要的部分是，我们在这里创建了一个具有原始对象功能的对象，以提供给外部世界。
单词`Proxy`的意思是“代替”或“代表”，直接解释了`Proxy`方法。

代理也被称为代理、句柄和包装器。它们在结构上与`适配器`和`装饰器`密切相关，但目的不同。
