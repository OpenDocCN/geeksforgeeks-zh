...
> 原文: [https://www.geeksforgeeks.org/how-to-order-pyspark-dataframe-by-list-of-columns/](https://www.geeksforgeeks.org/how-to-order-pyspark-dataframe-by-list-of-columns/)
...
### 语法
```py
dataframe.orderBy(['column1', 'column2', 'column_n'], ascending=True).show()
```
其中，
*   `dataframe` 是使用 pyspark 从嵌套列表中创建的 dataframe 名称
*   其中 `column` 是列名
*   `ascending=True` 指定数据帧的升序，`ascending=False` 指定数据帧的降序
*   用于显示列的 `show()` 方法。
...
2.  其中 `column` 是列名
3.  `ascending=True` 指定数据帧的升序，`ascending=False` 指定数据帧的降序
4.  用于显示列的 `show()` 方法。
