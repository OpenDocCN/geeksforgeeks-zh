...
> 原文：[https://www.geeksforgeeks.org/on-off-toggle-button-switch-in-tkinter/](https://www.geeksforgeeks.org/on-off-toggle-button-switch-in-tkinter/)
...
*   制作一个名为 `is_on` 的全局变量；默认值为 `True`，表示开关打开。
*   制作两个 `PhotoImage` 对象；一个对象有“开”图像，另一个对象有“关”图像。
*   创建一个默认为“开”图像的按钮；将边框宽度设置为零。
*   使用 `config()` 方法，创建一个改变按钮图像的功能。
*   该功能将检查 `is_on` 值是 `True` 还是 `False`。
