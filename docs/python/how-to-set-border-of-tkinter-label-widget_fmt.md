...
> 原文：[https://www.geeksforgeeks.org/how-to-set-border-of-tkinter-label-widget/](https://www.geeksforgeeks.org/how-to-set-border-of-tkinter-label-widget/)

这里的任务是使用 `Tkinter` 模块起草一个 Python 程序来设置标签小部件的边框。Tkinter 标签小部件是显示文本或图像的区域。我们可以随时更新这篇文章。
...
语法：`Label(master, options, ...)`

参数：

*   `master`：代表父窗口。
*   `options`：标签有很多选项，像 `bg`、`fg`、`font`、`bd` 等。

现在要设置标签的边框，我们需要向标签属性添加两个选项：

*   `borderwidth`：它将代表标签周围边框的大小。默认情况下，`borderwidth` 为 2 像素。“`bd`”也可以作为 `borderwidth` 的简写。
*   `relief`：它将指定标签周围装饰边框的外观。默认情况下，它是平面的。除了平面，还有许多更容易接受的值，如 `raised`、`sunken`、`ridge` 等。
