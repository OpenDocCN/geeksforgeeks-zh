...
> 原文: [https://www.geeksforgeeks.org/map-function-dictionary-python-sum-ascii-values/](https://www.geeksforgeeks.org/map-function-dictionary-python-sum-ascii-values/)
...
此问题已有解决方案，请参考[句子中每个单词的 ASCII 值总和](https://www.geeksforgeeks.org/sums-of-ascii-values-of-each-word-in-a-sentence/)链接。我们将使用[`map()`](https://www.geeksforgeeks.org/sum-2d-array-python-using-map-function/)函数和[`dict`](https://www.youtube.com/watch?v=z7z_e5-l2yE&t=35s)数据结构在 Python 中快速解决这个问题。方法很简单，

### 步骤

1.  首先将句子中的所有单词用空格分开。
2.  创建一个空字典，其中包含单词作为关键字，字符的 ASCII 值之和作为值。
3.  现在遍历拆分单词列表，并为每个单词映射当前单词每个字符的[`ord()`](https://www.geeksforgeeks.org/ord-function-python/)函数，并计算当前单词每个字符的 ASCII 值之和。
