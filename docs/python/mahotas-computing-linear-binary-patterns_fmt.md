...
> 原文: [https://www.geeksforgeeks.org/mahotas-computing-linear-binary-patterns/](https://www.geeksforgeeks.org/mahotas-computing-linear-binary-patterns/)

在本文中，我们将看到如何在 mahotas 中获得图像的线性二进制模式。局部二值模式是计算机视觉中用于分类的一种视觉描述符。LBP 是 1990 年提出的纹理谱模型的特例。1994 年首次描述了 LBP。为此，我们将使用来自核分割基准的荧光显微图像。我们可以借助下面给出的命令获得图像。
...
> **语法:** `mahotas.features.lbp(image, radius, points)`
> **参数:** 它以图像对象和两个整数作为参数。
> **返回:** 它返回一维数字数组即直方图特征。

**注意:** 输入应该是过滤后的图像或者加载为灰色。
