...
> 原文：[https://www.geeksforgeeks.org/how-to-scrape-all-pdf-files-in-a-website/](https://www.geeksforgeeks.org/how-to-scrape-all-pdf-files-in-a-website/)

## 先决条件

[用美图实现 Python 网页抓取](https://www.geeksforgeeks.org/implementing-web-scraping-python-beautiful-soup/)

网页抓取是一种从网站中提取数据并将该数据用于其他用途的方法。Python 中有几个用于进行网页抓取的库和模块。在本文中，我们将学习如何借助 python 中最好的网页抓取模块之一 `BeautifulSoup`、以及针对 GET 请求的 `requests` 模块从网站上抓取 PDF 文件。另外，为了获得更多关于 PDF 文件的信息，我们使用 `PyPDF2` 模块。
...
*   抓取是通过 `https://www.geeksforgeeks.org/how-to-extract-pdf-tables-in-python/` 链接完成的
*   `requests` 模块用于发出获取请求
*   `read.content` 用于遍历所有的 HTML 代码。打印将输出网页的源代码。
*   `soup` 有 HTML 内容，用来解析 HTML
...
### 第四步：用 pypdf2 模块创建 `info` 函数，获取 pdf 所需的所有信息
...
*   `info` 函数负责在 PDF 中提供所有需要的输出。
*   `io.BytesIO(response.content)` – 之所以使用它，是因为 `response.content` 是一个二进制代码，`requests` 库级别很低，一般是编译的（不是解释的）。所以要处理字节，使用 `io.BytesIO`。
*   有几个 `PyPDF2` 函数可以访问 pdf 中的不同数据。
