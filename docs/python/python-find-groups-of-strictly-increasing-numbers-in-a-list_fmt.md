...
> 原文：[https://www.geeksforgeeks.org/python-find-groups-of-strictly-increasing-numbers-in-a-list/](https://www.geeksforgeeks.org/python-find-groups-of-strictly-increasing-numbers-in-a-list/)
...
这是上述天真方法的替代方案。这个方法相当简单直接。它构造了一个 `start_bound` 列表和一个 `end_bound` 列表，其中包含递增整数的开始和结束序列的位置。因此只需使用 `for` 循环返回边界。
...
Python `itertools` 提供了循环和 `groupby` 等操作，这些操作在该方法中使用。首先，我们使用循环形成另一个列表 `temp_list`。循环生成一系列无限重复的值。然后我们使用 `groupby` 操作对 `temp_list` 进行相应的分组，最终得到所需的输出。
