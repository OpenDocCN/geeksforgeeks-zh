...
> 原文: [https://www.geeksforgeeks.org/python-find-all-pairs-of-consecutive-odd-positive-integer-smaller-than-a-with-sum-greater-than-b/](https://www.geeksforgeeks.org/python-find-all-pairs-of-consecutive-odd-positive-integer-smaller-than-a-with-sum-greater-than-b/)
...
给出两个数字，然后检查它们是否为正整数，并检查第一个数字是否大于第二个数字的一半。然后我们将检查奇数正整数，并在变量 `a` 中赋值。在 `while` 语句中，找到并打印奇数连续整数对。
