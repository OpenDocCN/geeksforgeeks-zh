...
在世界的不同地区，使用不同类型的日期格式，因此，编程语言通常为开发人员提供多种日期格式来处理。在 Python 中，它是通过使用名为`datetime`的库来处理的。它由可用于处理数据和时间值的类和方法组成。
...
在处理从 csv 导入的数据集或从网站表单获取输入时，需要多次从字符串到日期的转换。为此，python 提供了一个名为`strptime()`的方法。
