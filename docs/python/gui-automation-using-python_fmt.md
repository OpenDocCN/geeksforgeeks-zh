...
现在，我们将尝试移动鼠标光标，点击特定的位置，并执行打开和关闭应用程序。为了移动鼠标指针，我们可以使用 `moveTo()` 并指定 x、y 值以及它将执行操作的持续时间。`click()` 函数的作用是：点击鼠标指针现在所在的位置。代码基本上将鼠标光标移动到 (519, 1060) (x, y) 值，然后使用 `click()` 单击光标现在所在的位置，然后我们再次使用 `moveTo()` 移动到位置 (1717, 352)，并再次模拟单击。
