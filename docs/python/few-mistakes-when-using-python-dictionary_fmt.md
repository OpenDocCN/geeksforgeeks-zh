...
> 原文：[https://www.geeksforgeeks.org/few-mistakes-when-using-python-dictionary/](https://www.geeksforgeeks.org/few-mistakes-when-using-python-dictionary/)
...
*   为了访问键值，我们通常使用 `dict_name[key_name]`，相反，我们应该使用 `get()` 方法来消除在整个代码中抛出的异常。
*   为了更新键的值，我们通常使用 `dict_name[key_name]='new_value'`，相反，我们应该使用 `update(key=value)` 方法来消除在整个代码中抛出的异常。
