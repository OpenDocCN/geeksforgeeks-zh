...
> 原文: [https://www.geeksforgeeks.org/gated-recurrent-unit-networks/](https://www.geeksforgeeks.org/gated-recurrent-unit-networks/)
...
3.  **电流记忆门(`\overline{h}_{t}`)**: 在典型的门控循环单位网络讨论中，经常被忽略。它被并入复位门，就像输入调制门是输入门的一个子部分一样，用于将一些非线性引入输入，并使输入为零均值。使其成为重置门的一个子部分的另一个原因是为了减少以前的信息对传递到未来的当前信息的影响。
...
*   计算电流记忆门的过程有点不同。首先，计算重置门和先前隐藏状态向量的 `Hadmard` 积。然后将该向量参数化，再与参数化的当前输入向量相加。
...
*   要计算当前隐藏状态，首先定义一个与输入维度相同的全1向量。该向量将被称为 `ones`，数学上表示为 `1`。首先，计算更新门和先前隐藏状态向量的 `hadmard` 积。然后通过从 `ones` 中减去更新门来生成一个新向量，再计算新生成的向量与当前记忆门的 `hadmard` 积。最后，将两个向量相加得到当前隐藏状态向量。
...
使用链式法则，并利用 `\overline{y}_{t}` 是 `h_{t}` 的函数并且确实是 `\overline{h}_{t}` 的函数的事实，出现以下表达式：
...
请注意，梯度方程包含一系列 `\partial {h}_{t}`，看起来类似于基本的递归神经网络，但由于 `h_{t}` 导数的内部工作原理，该方程的工作原理不同。

## 门控递归单元如何解决渐变消失的问题？

梯度的值由从 `\frac{\partial h_{t}}{\partial h_{t-1}}` 开始的导数链控制。回忆一下 `h_{t}` 的表达方式：
...
用上面的表达式计算 `\frac{\partial \overline{h_{t}}}{\partial h_{t-1}}` 的值：
...
在这种情况下，不考虑 `r` 的值，术语 `\frac{\partial \overline{h_{t}}}{\partial h_{t-1}}` 等于 `z`，而 `z` 又等于 1。

**2A 案例(`z=0`，`r=0`)**:

在这种情况下，术语 `\frac{\partial \overline{h_{t}}}{\partial h_{t-1}}` 等于 0。

**2B 案例(`z=0`，`r=1`)**:

在这种情况下，术语 `\frac{\partial \overline{h_{t}}}{\partial h_{t-1}}` 等于 `(1-\overline{h}_{t}^{2})(W)`。该值由可训练的权重矩阵控制，因此网络学习调整权重，使得术语 `\frac{\partial \overline{h_{t}}}{\partial h_{t-1}}` 更接近 1。
