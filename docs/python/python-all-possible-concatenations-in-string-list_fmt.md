...
> 原文: [https://www.geeksforgeeks.org/python-all-possible-concatenations-in-string-list/](https://www.geeksforgeeks.org/python-all-possible-concatenations-in-string-list/)
...
## 方法: 使用 `permutations()` + `join()` + 循环

以上函数的组合可以用来解决这个问题。在本文中，我们使用 `join()` 执行连接任务，并使用 `permutations()` 执行所有可能的组合提取。
