...
> 原文：[https://www.geeksforgeeks.org/creating-a-multiple-selection-using-tkinter/](https://www.geeksforgeeks.org/creating-a-multiple-selection-using-tkinter/)
...
Tkinter 是 python 中的 GUI 库，易于阅读和理解。在 Tkinter 中，可以使用列表框小部件进行多重选择。通常，列表框以列表的形式显示不同的项目。列表框小部件从列表中提供一个或多个项目选择。`Listbox`小部件中有许多选项可供用户选择多个选项。通过将选择模式选项指定为`multiple`，用户可以选择多个选项。如果选择模式选项是`single`，则用户只能选择一个选项。

列表框小部件的`selectmode`选项可以是`single`、`browse`、`multiple`或`extended`。

*   **single** – 选择一行文本。
*   **browse** – 这是一个默认选项，用户可以选择一行文本。
*   **multiple** – 选择多行文本，无需从选项的第一行拖到最后一行。
*   **extended** – 用户可以选择并拖动相邻的多行文本。
