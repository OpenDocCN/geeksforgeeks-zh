...
> 原文: [https://www.geeksforgeeks.org/python-custom-consecutive-character-pairing/](https://www.geeksforgeeks.org/python-custom-consecutive-character-pairing/)

有时，在使用 Python Strings 时，我们可能会遇到这样的问题，即需要使用 deliminator 执行连续字符串的配对。这在许多领域都有应用。让我们讨论执行这项任务的某些方法。

**方法一: 使用 `join()` + `列表理解`**
以上功能的组合可以用来执行此任务。在本文中，我们使用 `join()` 执行连接字符的任务，并使用 `列表理解` 执行编译。
