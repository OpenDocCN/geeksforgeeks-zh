...
> **参数:**
> *   `type`: 数据类型列表，每种类型返回对应的令牌。
> *   `amount`: 给定类型重复特定次数。
> *   `as_list`: 如果为真，令牌流作为列表返回，否则返回生成器。
> *   `file`: 文件实体流。

*   `read_many(*type, amount=1, file=_StdIn)`: 从输入流中读取令牌，除非输入了其他类型的元素。

**例 1:`read()`和 `read_many()`的工作方式**
...
`read()`和 `read_many()`函数提供了获取文件作为输入流的功能，以从文件中获取数据并在控制台上渲染，或者使用 `file`参数渲染任何文件。
...
与 `read_many()`类似，不同之处在于它一次读取整行，而不是在空格处移动到换行符。阅读整行。
