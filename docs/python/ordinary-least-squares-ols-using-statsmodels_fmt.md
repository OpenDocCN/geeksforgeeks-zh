...
> 原文: [https://www.geeksforgeeks.org/ordinary-least-squares-ols-using-statsmodels/](https://www.geeksforgeeks.org/ordinary-least-squares-ols-using-statsmodels/)
...
在 `OLS` 方法中，我们必须选择 `b_1` 和 `b_0` 的值，使得计算值与观测值 `y` 之间的差的平方和最小化。
...
`\hat{y_i}` = 第 `i` 次观测的预测值
`y_i` = 第 `i` 次观测的实际值
`\epsilon_i` = 第 `i` 次观测的误差/残差
`n` = 观测总数

为了得到使 `S` 最小化的 `b_0` 和 `b_1` 的值，我们可以对每个系数取一个偏导数，使其等于零。
...
*   首先我们定义变量 `x` 和 `y`。在下面的例子中，变量是使用 `pandas` 从 `csv` 文件中读取的。示例中使用的文件可以在[这里](https://drive.google.com/file/d/13AK0eheyKUvHEHts-CPnb67KphEz8YKv/view)下载。
*   接下来，我们需要使用 `add_constant()` 方法将常数 `b_0` 添加到等式中。
*   `statsmodels.api` 模块的 `OLS()` 功能用于执行 OLS 回归。它返回一个 OLS 对象。然后在这个对象上调用 `fit()` 方法来拟合数据的回归线。
