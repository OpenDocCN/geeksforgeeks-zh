...
| `C` 的语法比 `python` 难，正因为如此，程序员更喜欢用 `python` 而不是 `C` | `Python` 程序比 `c` 更容易学习、编写和阅读。 |
...
| `c` 中的基本 `if` 语句表示为: `if()` | `Python` 中的基本 `if` 语句表示为: `if:` |
| | `Python` 中的基本 `if-else` 语句表示为: `if ():` |
