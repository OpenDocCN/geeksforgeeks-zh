...
`input()` 函数用于获取用户输入。默认情况下，它以字符串的形式返回用户输入。
...
*   `prompt`：一个字符串，代表输入前的默认消息（通常是屏幕输出）。但是，提示信息是可选的。

**返回**：`input()` 返回一个字符串对象。即使输入的值是一个整数，它也会将其转换成一个字符串。

### 例1：以用户的姓名和年龄为输入并打印出来

默认情况下，`input()` 返回一个字符串。因此姓名和年龄将存储为字符串。
