...
> 原文: [https://www.geeksforgeeks.org/create-error-bars-in-plotly-python/](https://www.geeksforgeeks.org/create-error-bars-in-plotly-python/)
...
用于表示 2D 数据点的函数，如 `px.scatter`、`px.line`、`px.bar` 等，误差线作为列名给出，它是 `error_x`（对于 x 位置的误差）和 `error_y`（对于 y 位置的误差）的值。误差线是数据的图形表示交替，用在图表上表示报告容量的误差或不确定性。

### 示例 1

在本例中，我们将使用 `tips()` 数据集绘制一个简单的错误图。
...
在本例中，我们将看到非对称误差条，当结果存在非线性相关性时，就会出现非对称误差。
...
在本例中，我们将看到对称误差条，对称平均绝对百分比误差是基于百分比误差的精度度量。
...
在本例中，我们将看到如何使用错误栏的属性对其进行着色和样式设置。
