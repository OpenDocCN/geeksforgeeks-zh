...
> 原文：[https://www.geeksforgeeks.org/find-the-size-of-a-set-in-python/](https://www.geeksforgeeks.org/find-the-size-of-a-set-in-python/)

一个[集合](https://www.geeksforgeeks.org/sets-in-python/)是一个无序的集合数据类型，它是可迭代的，可变的，并且没有重复的元素。Python 的 `set` 类代表了集合的数学概念。集合的大小是指集合对象占用的内存量（以字节为单位）。在本文中，我们将学习获得 Python 集大小的各种方法。
