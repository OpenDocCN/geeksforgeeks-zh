...
> 原文: [https://www.geeksforgeeks.org/counters-in-python-set-2-accessing-counters/](https://www.geeksforgeeks.org/counters-in-python-set-2-accessing-counters/)

[Python 中的计数器 | 集合 1 (初始化和更新)](https://www.geeksforgeeks.org/counters-in-python-set-1/)

一旦初始化，`Counter` 就像字典一样被访问。此外，它不会引发 `KeyError`（如果键不存在），取而代之的是该键的计数显示为 `0`。
