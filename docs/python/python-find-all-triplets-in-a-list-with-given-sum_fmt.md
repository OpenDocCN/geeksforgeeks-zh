...
## 方法#1 : 朴素(使用集合)
在这种方法中，我们使用两个 `for` 循环。第一个循环设置第一个元素，另一个循环检查包括第一个元素的其他两个元素是否加起来为 `k`。这种方法需要`0(n^2)`的时间复杂度。
...
## 方法 2 : 使用 itertools
Python `itertools` 模块提供`combinations(iterable，r)`功能。该工具从输入表中返回 `r` 长度的元素子序列。每次我们将 3 个元素组合在一起，检查它们的总和是否为 `k`。
