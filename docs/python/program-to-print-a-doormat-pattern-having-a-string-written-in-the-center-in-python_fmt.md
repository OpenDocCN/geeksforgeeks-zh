...
给定行数 `R` 和列数 `C`，任务是打印门垫图案，如下例所示。垫子设计中的列数必须是设计中行数的 3 倍。门垫的中央应该印有 `GeeksforGeeks`。垫子的宽度等于图案中的列数。
...
1.  `ljust`：此方法返回长度为 `l` 的左对齐字符串。
2.  `居中`：该方法返回长度为 `l` 的居中对齐字符串。
3.  `rjust`：此方法返回长度为 `l` 的右对齐字符串。
