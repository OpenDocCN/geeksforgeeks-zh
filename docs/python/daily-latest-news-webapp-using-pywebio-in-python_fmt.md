...
> 原文: [https://www.geeksforgeeks.org/daily-latest-news-webapp-using-pywebio-in-python/](https://www.geeksforgeeks.org/daily-latest-news-webapp-using-pywebio-in-python/)
...
### 第一步
首先从 [获取自己的 API 密钥](https://newsapi.org/docs/client-libraries/python) 然后导入以下模块。
...
### 第二步
在一个变量中加载一个 API 键(你已经从网站中提取的)，并把国家的名称作为用户的输入。
...
### 步骤 3
现在，该功能将检查用户给出的国家名称是否有效。它通过将所有国家的**国际标准化组织代码**存储在国家字典中来进行检查，如果输入的国家不在字典中，它将返回一条错误消息。
...
### 第四步
这里我们得到用户的选择，并根据这个选择将数据保存在变量中，以便进一步显示。

*   使用单选功能创建一个选择框列表，以便用户可以选择他/她想要获取新闻的类别。
*   然后使用 `get_top_headlines` 函数并将类别传递给它，将所有最新的头条新闻存储在一个变量中。
...
### 第五步
现在我们将在特定时间间隔后以弹出窗口的形式循环打印新闻标题及其内容(您可以在下面的代码中根据自己的选择进行设置)。
