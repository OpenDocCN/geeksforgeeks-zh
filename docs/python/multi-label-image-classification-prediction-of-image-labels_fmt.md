...
这里，我们将使用 `NumPy` 和 `scikit-image` 的 `imread` 功能。由于我们有下载的数据，我们可以快速计算每个主题有多少张图片。例如，假设每个文件夹中有 100 个图像(`r`、`v` 和 `d`)，则可以设置一个值为 100 的变量 `limit`。下一步是为这些数据创建空数组，并用数据填充这些数组。我们将快速制作 3 个阵列来容纳系列图像的数据。我们使用下面的代码片段创建一个充满“无”值的数组:
...
如前所述，`熊猫`为我们的表格制作了一个类似电子表格软件的环境。让我们将数组转换为数据帧:
