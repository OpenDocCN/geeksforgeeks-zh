...
> 原文：[https://www.geeksforgeeks.org/python-flatten-dictionary-with-list/](https://www.geeksforgeeks.org/python-flatten-dictionary-with-list/)

给定一个列表和字典，在列表中键的可用元素的位置展平带有键和值的字典。

> **输入**：`test_list = ["Gfg", "is", "Best", "For", "Geeks"]`, `subs_dict = {"Gfg": 7}`
> **输出**：`['Gfg', 7, 'is', 'Best', 'For', 'Geeks']`
> **解释**：`"Gfg"`被替换，后面是它在字典中的值。
>
> **输入**：`test_list = ["Gfg", "is", "Best", "For", "Geeks"]`, `subs_dict = {"Gfg": 7, "Best": 8}`
> **输出**：`['Gfg', 7, 'is', 'Best', 8, 'For', 'Geeks']`
> **解释**：无替换。没有匹配的值。
...
## 方法二：使用 `chain.from_iterable()` + 列表理解
