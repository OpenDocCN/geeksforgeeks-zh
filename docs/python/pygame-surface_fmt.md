...
在 pygame 中创建表面非常容易。我们只需要用元组将高度和宽度传递给 `pygame.Surface()` 方法。我们可以使用各种方法来格式化我们想要的表面。比如我们可以使用 `pygame.draw()` 来绘制形状，我们可以使用 `surface.fill()` 方法在表面进行填充。现在，实现这些功能。让我们讨论语法和参数。
...
> **语法:** `pygame.Surface.fill(颜色, rect=无, special_flags=0)`
...
要显示一个表面，它需要在屏幕上显示。块传输可以被认为是将一个表面的像素复制到另一个表面。我们可以使用 `surface.blit()` 方法进行 blit，该方法将需要进行 blit 的曲面作为第一个参数，将坐标元组作为第二个坐标。

> **语法:** `pygame.Surface.blit(源, 目的地, 区域=无, 特殊_flags=0)`
...
既然我们已经讨论了一些表面函数。`blit()`，`.fill()`，等。让我们讨论 pygame 屏幕的一些更重要的功能。

*   **`pygame.Surface.convert`**: 制作一个像素格式改变的曲面副本。新的像素格式可以根据现有的表面或深度来确定，可以使用标志和遮罩参数。

> **语法** : `pygame.Surface.convert(曲面=无)`
...
*   **`pygame.Surface.convert_alpha`**: 它用所需的像素格式创建一个新的表面副本。新表面的格式将适合于使用每像素 alpha 快速传送到给定的格式。如果没有给定曲面，新曲面将被优化，以便与当前显示进行块传输。

> **语法:** `pygame.Surface.convert_alpha(曲面)`
...
*   **`pygame.Surface.copy`**: 它创建曲面的新副本。复制的曲面将具有与原始曲面相同的像素格式、调色板、透明度设置和类别。
...
*   **`pygame.Surface.set_colorkey`**: 设置表面的当前颜色键。将此表面块传送到目标时，任何与 colorkey 颜色相同的像素都将是透明的。
...
*   **`pygame.Surface.get_colorkey`**: 返回曲面的当前颜色键值。如果没有设置颜色键，则返回无。
...
*   **`pygame.Surface.set_alpha`**: 为整个曲面图像设置的α值。传递 0 表示不可见，传递 255 表示完全不透明。
