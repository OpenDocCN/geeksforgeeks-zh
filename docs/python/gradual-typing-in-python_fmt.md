...
**渐进式类型检查器**是一个静态类型检查器，用于检查渐进式程序静态类型部分的类型错误。类型检查器通过将变量和函数参数分配给一种称为`Any`的特殊类型来处理逐渐类型化程序的动态类型化部分。静态类型检查器会将每种类型视为与`Any`兼容，并将`Any`视为与每种类型兼容。这意味着可以对`Any`类型的值执行任何操作或方法调用，并将其分配给任何变量，静态类型检查器不会抱怨。
...
此命令使 mypy type 检查您的`program.py`文件并打印出它发现的任何错误。Mypy 将静态地键入检查您的代码：这意味着它将检查错误，而不会运行您的代码，就像一个过线器一样。
