...
> 原文：[https://www.geeksforgeeks.org/python-grouped-summation-of-tuple-list/](https://www.geeksforgeeks.org/python-grouped-summation-of-tuple-list/)

很多时候，我们会得到一个元组列表，我们需要对它的键进行分组，并在分组时执行某些操作。最常见的操作是加法。让我们讨论执行这项任务的某些方法。除了加法，其他操作也可以通过做一些小的改变来完成。

**方法：使用 `Counter()` + `+` 运算符**

此任务可以使用 `Counter()` 功能来执行，因为它在内部进行分组，加法运算符可用于指定分组结果的功能。
