...
> 原文: [https://www.geeksforgeeks.org/passive-aggressive-classifiers/](https://www.geeksforgeeks.org/passive-aggressive-classifiers/)
...
*   `C`: 这是正则化参数，表示模型对错误预测的惩罚。
*   `max_iter`: 模型对训练数据进行迭代的最大次数。
*   `tol`: 停止标准。如果设置为`None`，当(`loss > previous_loss - tol`)时，模型将停止。默认情况下，它设置为`1e-3`。
