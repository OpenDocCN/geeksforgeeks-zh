...
> 原文：[https://www.geeksforgeeks.org/extract-time-from-datetime-in-python/](https://www.geeksforgeeks.org/extract-time-from-datetime-in-python/)

在本文中，我们将看到如何在 Python 中从 `DateTime` 中提取时间。

在 Python 中，没有 `datetime` 这样的数据类型，首先，我们必须将数据创建为日期时间格式，然后将日期时间数据转换为时间。Python 模块用于将数据转换为日期时间格式，但是在本文中，我们将使用 `datetime` 模块来完成这项任务。
...
> *   `格式`: 这将是字符串，但默认为 `None`。解析时间，例如 `"%d/%m/%Y"`，请注意 `"%f"` 将一直解析到纳秒。
>
> 例如 –> `format = "%Y %b %d %H %M %S"`
>
> 例如，`datetime_obj = datetime.strptime("19022002101010", "%d%m%Y%H%M%S")` # 它将返回 `datetime` 对象。

## 从日期时间对象中提取时间

在本节中，我们将从 `DateTime` 对象中提取时间。

我们将字符串变成日期时间对象，现在我们通过调用 `time()` 方法从日期时间对象中提取时间。
...
在本节中，我们将从 `DateTime` 对象中提取的时间中提取小时，所有 3 个步骤与上一个示例相同。从 `DateTime` 对象中提取小时的 `hour` 方法。

我们必须提取时间，所以下一部分是通过调用 `.hour` 方法。
...
在本例中，我们将添加 `.minute` 和 `.second` 方法来从 `DateTime` 对象中提取分钟和秒。
...
如果日期不在这个范围内，那么代码会给我们一个 `ValueError`。为了确定出现此错误的时间，我们可以做的是使用异常语句，并告诉用户您输入了错误的日期，并且无法从该日期获取时间，因为该日期从未存在过，因此如何在该日期中存在时间。
