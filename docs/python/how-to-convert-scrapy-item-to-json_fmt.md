...
> 原文: [https://www.geeksforgeeks.org/how-to-convert-scrapy-item-to-json/](https://www.geeksforgeeks.org/how-to-convert-scrapy-item-to-json/)

**先决条件:**

*   [Scrapy](https://www.geeksforgeeks.org/implementing-web-scraping-python-scrapy/)
*   JSON

**Scrapy** 是一款用来收集网页数据的网页抓取工具，也可以用来修改和存储我们想要的任何形式的数据。每当数据被 scrapy 的蜘蛛抓取时，我们就把这些原始数据转换成 scrapy 的项目，然后我们会把这些项目传递给管道进行进一步的处理。在管道中，这些项目将被转换为 JSON 数据，我们可以打印它，也可以将其保存在另一个文件中。因此，我们可以从网络抓取的数据中检索 JSON 数据。

## 初始化目录和设置项目

让我们首先创建一个 Scrapy 的项目。为此，请确保系统中安装了 Python 和 PIP。然后一个接一个地运行下面给出的命令，创建一个类似于我们将在本文中使用的项目。
...
现在要创建一个 Scrapy 项目，使用下面给出的命令，并创建一个蜘蛛。

```bash
scrapy startproject ScrapyTutorial # 项目名称是 ScrapyTutorial
cd ScrapyTutorial
scrapy genspider spider_to_crawl https://quotes.toscrape.com/
# 上面的链接提到了我们要抓取蜘蛛的网站。
```

一旦您使用 pip 安装程序创建了一个 Scrapy 的项目，那么项目目录的输出看起来就像图像中给出的一样。(如果你想更多地了解一个 Scrapy 项目并熟悉它，请参考这个)。
...
上图中**项目名称**为**ScrapyTutorial**，里面有很多文件，如图所示。
...
请注意，我们对上面的默认文件做了一些更改，即注释掉了 `allowed_domains` 行，我们还对 `start_urls` 做了一些更改(删除了“**http://**”)。

## 将 Scrapy 项目转换为 JSON

管道是一种方法，我们可以通过它来转换、修改或存储抓取的数据项。因此，让我们先来谈谈它的一些组件。

默认的 `pipelines.py` 文件如下所示:
...
首先，我们必须在我们的 `settings.py` 文件中注册管道组件的名称。语法如下。

**语法:**

```python
ITEM_PIPELINES = {
    'myproject.pipelines.component': #优先级数字,
    # 许多其他组件
}
```
...
我们必须执行的另一件事是声明项目的格式，我们必须使用该格式将数据传递给管道。为此，我们将使用我们的 `items.py` 文件。
...
既然我们已经看到了如何在 `pipelines.py` 文件中实现组件，以及如何完成设置和声明项目。现在我们准备好了一个例子，在这个例子中，我们将把我们收集的数据项转换成 JSON 格式。为了转换 JSON 格式的数据，我们将使用 python 的 JSON 库及其 `dump` 属性。我们的想法是将抓取的数据放入 `pipelines.py` 文件中，然后打开一个文件，将所有的 JSON 数据写入其中。所以方法命名为:

*   `open_spider()`：当蜘蛛开始爬行时，会调用此方法打开文件( `result.json` )。
*   `close_spider()`：当 spider 关闭，刮除结束时，会调用此方法关闭文件。
...
在使用命令 `scrapy crawl spider_to_crawl` 后，下面给出的步骤将会发生。

*   由于创建了 `result.json` 文件，蜘蛛被爬网。现在蜘蛛抓取网页，收集 `Quotes_all` 变量中的数据。然后从变量中提取每个数据，并作为键值传递给文件中声明的项目，即 `Quote`。最后在 `yield` 中我们称 `pipelines.py` 文件为进一步处理。
*   我们在 `pipelines.py` 文件中从 spider 接收 `item` 变量，然后使用 `dumps` 方法将其转换为 JSON，然后将输出写入打开的文件中。
*   文件被关闭，我们可以看到输出。
