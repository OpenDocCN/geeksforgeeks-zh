...
> 原文: [https://www.geeksforgeeks.org/python-cmath-isclose-function/](https://www.geeksforgeeks.org/python-cmath-isclose-function/)

`cmath`模块包含许多用于复数数学运算的函数。`cmath.isclose()`函数用于检查两个复数值是否接近。该函数传递的值可以是`整数`、`浮点数`和`复数`。
