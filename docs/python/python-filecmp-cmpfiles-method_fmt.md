...
> 原文: [https://www.geeksforgeeks.org/python-filecmp-cmpfiles-method/](https://www.geeksforgeeks.org/python-filecmp-cmpfiles-method/)
...
- `dir1`：第一个目录的路径。它可以是字符串、字节或 `os.PathLike` 对象。
- `dir2`：第二个目录的路径。它可以是字符串、字节或 `os.PathLike` 对象。
- `common`：表示将在 `dir1` 和 `dir2` 中进行比较的文件名称的列表。
- `shallow`：布尔值 `True` 或 `False`。此参数的默认值为 `True`。如果其值为真，则仅比较文件的元数据。如果为假，则比较文件的内容。

## 返回类型

该方法返回一个三元组列表，代表匹配、不匹配和错误列表。

## 示例

`filecmp.cmpfiles(dir1, dir2, [file1, file2, file3])` 将比较 `dir1/file1` 与 `dir2/file1`、`dir1/file2` 与 `dir2/file2`、`dir1/file3` 与 `dir2/file3` 并将返回匹配、不匹配和错误列表。
