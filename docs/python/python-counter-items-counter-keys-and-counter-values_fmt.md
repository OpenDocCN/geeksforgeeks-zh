...
> 原文: `https://www.geeksforgeeks.org/python-counter-items-counter-keys-and-counter-values/`

`Counter` 类是 Python3 中 `collections` 模块提供的一种特殊类型的对象数据集。`collections` 模块为用户提供了专门的容器数据类型，从而为 Python 的通用内置功能（如 `dict`、`list` 和 `tuple`）提供了一种替代方案。`Counter` 是一个子类，用于计数 hashable 对象。它在被调用时隐式地创建一个 iterable 的哈希表。
