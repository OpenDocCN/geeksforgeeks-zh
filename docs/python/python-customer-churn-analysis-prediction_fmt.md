...
> 原文: [https://www.geeksforgeeks.org/python-customer-churn-analysis-prediction/](https://www.geeksforgeeks.org/python-customer-churn-analysis-prediction/)
...
### 代码: 通过`Churn`对数据进行分组，并计算`Customer service calls`的平均值
...
没错。也许不出所料，流失者似乎比非流失者打更多的客服电话。

### 代码: 找出一个`State`是否比另一个州有更多的流失者
...
虽然加州是美国人口最多的州，但我们的数据集中来自加州的客户却没有那么多。例如，亚利桑那州(AZ)有 64 名客户，其中 4 人最终流失。相比之下，加州流失的顾客数量(和比例)更高。这是对公司有用的信息。
...
### 代码: 可视化流失者和非流失者之间`Customer service calls`的差异
...
在`telcochurndata`数据中，`Churn`、`Voice mail plan`和`International plan`是二进制特征，可以很容易地转换成 0 和 1。
...
## 在训练集上训练一个`RandomForestClassifier`模型
