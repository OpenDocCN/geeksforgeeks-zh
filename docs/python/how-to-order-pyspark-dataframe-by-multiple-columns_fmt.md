...
> 原文: [https://www.geeksforgeeks.org/how-to-order-pyspark-dataframe-by-multiple-columns/](https://www.geeksforgeeks.org/how-to-order-pyspark-dataframe-by-multiple-columns/)

在本文中，我们将使用 PySpark DataFrame 中的 `orderBy()` 函数对多个列进行排序。对行进行排序意味着按升序或降序排列行，因此我们将使用嵌套列表创建数据框并获取不同的数据。

`orderBy()` 是对一列或多列进行排序的函数。默认情况下，它按升序排序。
...
Python 程序，通过使用 `orderBy()` 函数按降序基于两列对数据帧进行排序来显示数据帧。
...
Python 程序，通过使用 `orderBy()` 函数基于两列按升序对数据帧进行排序来显示数据帧。
