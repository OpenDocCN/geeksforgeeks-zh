...
> *   `axis`: 是箱体的对中轴。指定包含 3 个值的向量，例如 `axis = vector(1, 2, 1)`
> *   `up`: 是盒子的方位。指定一个包含 3 个值的向量，例如 `up = vector(0, 1, 0)`
> *   `color`: 是盒子的颜色。指定一个包含 3 个值的向量，例如 `color = vector(1, 1, 1)` 将给出白色
> *   `opacity`: 是盒子的不透明度。分配一个浮动值，其中 1 是最不透明的，0 是最不透明的，例如 `opacity = 0.5`
> *   `shininess`: 是盒子的光泽。指定一个浮动值，其中 1 是最闪亮的，0 是最不闪亮的，例如 `shininess = 0.6`
> *   `emissive`: 是盒子的发射率。指定一个布尔值，其中“真”是发射性的，“假”不是发射性的，例如 `emissive = False`
> *   `texture`: 是盒子的纹理。从纹理类中指定所需的纹理，例如 `texture = textures.stucco`
> *   `length`: 是简单情况下盒子在 x 方向的长度。分配一个浮点值，默认长度为 1，示例 `length = 10`
> *   `height`: 是简单情况下盒子在 y 方向的高度。指定一个浮动值，默认高度为 1，例如 `height = 10`
> *   `width`: 是简单情况下盒子在 z 方向的宽度。分配一个浮点值，默认宽度为 1，示例 `width = 10`
> *   `size`: 是盒子的大小。分配一个包含 3 个值的向量，分别表示长度、高度和宽度，默认大小为`(2, 2, 2)`，示例 `size = vector(1, 1, 1)`
>
> 所有参数都是可选的。
