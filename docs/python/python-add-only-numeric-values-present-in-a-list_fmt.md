...
> 原文：[https://www.geeksforgeeks.org/python-add-only-numeric-values-present-in-a-list/](https://www.geeksforgeeks.org/python-add-only-numeric-values-present-in-a-list/)

给定一个包含字符和数字的列表，任务是只添加列表中的数字。下面给出了一些完成给定任务的方法。

## 方法一：使用`filter()`和`lambda`
