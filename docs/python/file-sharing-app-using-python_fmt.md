...
> 原文：[https://www.geeksforgeeks.org/file-sharing-app-using-python/](https://www.geeksforgeeks.org/file-sharing-app-using-python/)

[**计算机网络**](https://www.geeksforgeeks.org/basics-computer-networking/)是一个重要的话题，要理解这些概念，就需要这些概念的实际应用。在这篇特别的文章中，我们将看到如何使用 Python 制作一个简单的文件共享应用程序。一个 [`HTTP Web Server`](https://www.geeksforgeeks.org/web-server-and-its-type/) 是理解 [`URL`](https://www.geeksforgeeks.org/url-full-form/) 和 [`HTTP`](https://www.geeksforgeeks.org/http-full-form/)（查看网页所使用的协议）的软件。Python 有几个包，是模块的集合。并且它有几个内置的服务器。本项目中使用的模块有：

*   [`http.server`](https://docs.python.org/3/library/http.server.html) 是一个 [`socketserver`](https://docs.python.org/3/library/socketserver.html) 服务器，它创建并监听 HTTP 套接字。
*   [`socketserver`](https://docs.python.org/3/library/socketserver.html) 模块简化了编写网络服务器的任务。
*   [`webbrowser`](https://docs.python.org/3/library/webbrowser.html) 模块为我们提供了一个高级界面，允许和显示基于 Web 的文档，只需调用 [`open()`](https://docs.python.org/3/library/webbrowser.html#webbrowser.open) 函数即可。
*   [`pyqrcode`](https://pypi.org/project/PyQRCode/) 模块用来生成二维码，只用两行代码。
*   [`os`](https://docs.python.org/3/library/os.html) 模块有助于与操作系统交互。用于在命令行上打开文件、操作路径和读取所有文件中的所有行。
*   [`pypng`](https://pypi.org/project/pypng/) 允许使用纯 Python 读写 PNG 图像文件。
...
*   代码通过 `os` 模块找到用户配置文件的名称。并更改目录以访问桌面上的文件。
