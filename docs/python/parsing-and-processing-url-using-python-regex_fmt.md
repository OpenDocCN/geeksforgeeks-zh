...
> 原文: [https://www.geeksforgeeks.org/parsing-and-processing-url-using-python-regex/](https://www.geeksforgeeks.org/parsing-and-processing-url-using-python-regex/)
...
如果 URL 是不同的类型，如 `file://localhost:4040/zip_file`，并带有端口号，那么为了提取端口号，由于它是可选的，我们将使用 `?` 符号。这里，端口号 `4040` 出现在 `:` 符号之后。因此，由于是数字 `(:\d+)` 被使用。由于并非所有 URL 都以主机号结尾，因此要将其设为可选，请使用以下语法 `(:\d+)?`。
