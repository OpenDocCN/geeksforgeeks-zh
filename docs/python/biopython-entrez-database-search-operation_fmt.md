...
> 原文: [https://www.geeksforgeeks.org/biopython-entrez-database-search-operation/](https://www.geeksforgeeks.org/biopython-entrez-database-search-operation/)
...
1.  导入所需的模块。
2.  设置您的电子邮件以识别谁与数据库连接。
3.  设置 Entrez 工具参数，默认为 Biopython。
4.  使用上面提供的任何具有适当参数的方法。
5.  返回的数据将是 XML 格式的，因此要在 python 对象中获取该数据，需要使用 `Entrez.read()` 方法来读取该对象。
6.  阅读提供的信息。
