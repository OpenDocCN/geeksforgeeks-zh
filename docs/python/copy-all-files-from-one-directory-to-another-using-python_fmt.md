...
> 原文:[https://www.geeksforgeeks.org/copy-all-files-from-one-directory-to-another-using-python/](https://www.geeksforgeeks.org/copy-all-files-from-one-directory-to-another-using-python/)
...
[`shutil.copytree()`](https://www.geeksforgeeks.org/python-shutil-copytree-method/) 方法递归地将一整棵以源(`src`)为根的目录树复制到目标目录。它用于递归地将文件从一个位置复制到另一个位置。目标不应是现有目录。它是在复制操作执行期间创建的。
...
> *   `src`: 源目录
> *   `dst`: 目的地控制器
> *   `copy_function` (可选): 默认值–`copy2()`。也可以使用 `copy()` 方法。
>
> **返回:** 新创建的目标目录名
