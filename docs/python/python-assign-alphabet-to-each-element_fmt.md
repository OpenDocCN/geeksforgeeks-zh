...
> 原文: [https://www.geeksforgeeks.org/python-assign-alphabet-to-each-element/](https://www.geeksforgeeks.org/python-assign-alphabet-to-each-element/)
...
## 方法一: 使用 `ascii_lowercase()` + 循环 + 列表理解

在本文中，我们使用 `ascii_lowercase()` 提取所有小写字母，并创建将相同元素映射到相似字符的字典，然后使用列表理解将该字典展平为适当的索引。
...
## 方法二: 使用 `defaultdict()` + `ascii_lowercase()` + `iter()`

在本文中，我们使用 `defaultdict()` 为相似的元素赋值，`ascii_lowercase()` 用于获取所有小写字母。
