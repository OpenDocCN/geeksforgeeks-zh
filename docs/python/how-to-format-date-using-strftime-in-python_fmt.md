...
> 原文: [https://www.geeksforgeeks.org/how-to-format-date-using-strftime-in-python/](https://www.geeksforgeeks.org/how-to-format-date-using-strftime-in-python/)

在本文中，我们将看到如何在 Python 中使用 [`strftime()`](https://www.geeksforgeeks.org/python-strftime-function/) 来格式化日期。 [`localtime()`](https://www.geeksforgeeks.org/python-time-localtime-method/) 和 [`gmtime()`](https://www.geeksforgeeks.org/python-time-gmtime-method/) 返回一个表示时间的元组，该元组被转换为一个字符串，该字符串由使用 python time 方法 `strftime()` 的 `format` 参数指定。
