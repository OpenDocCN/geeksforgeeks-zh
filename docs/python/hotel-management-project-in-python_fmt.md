...
> 原文：[https://www.geeksforgeeks.org/hotel-management-project-in-python/](https://www.geeksforgeeks.org/hotel-management-project-in-python/)
...
*   `date(str)` - 功能验证用户/客户输入的日期。
*   `Booking()` - 通过输入用户/客户详细信息在酒店预订房间的功能。
*   `Rooms_Info()` - 功能为用户/客户提供酒店房间信息（即关于房间设施的信息）。
*   `restaurant()` - 客房服务功能，为用户/顾客提供餐厅的菜单卡，以便在房间点餐。
*   `Payment()` - 结账时生成酒店房间和餐厅账单的付款功能。
*   `Record()` - 对入住酒店的顾客进行记录的功能。
