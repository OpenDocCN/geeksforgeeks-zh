...
要在我们的对象中添加随机移动，我们将使用[`random`](https://www.geeksforgeeks.org/python-random-module/)模块的[`randint()`](https://www.geeksforgeeks.org/python-randint-function/)方法。
...
要在多个对象中添加随机移动，我们将为这些对象创建单独的速度和方向变量。然后我们将借助`colliderect()`方法检查物体是否发生碰撞。
