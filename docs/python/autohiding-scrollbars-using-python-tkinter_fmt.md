...
> 原文：`https://www.geeksforgeeks.org/autohiding-scrollbars-using-python-tkinter/`
...
此外，`tkinter` 启用许多控件，如标签、文本框、列表框、按钮、滚动条等，这些控件在图形用户界面应用程序中使用。这些控件被称为[小部件](https://www.geeksforgeeks.org/python-gui-tkinter/)。

## tkinter 的几何管理方法

这些方法用于跨父小部件区域组织小部件。此外，所有 `tkinter` 小部件都可以访问这些方法。有三种[几何管理方式](https://www.geeksforgeeks.org/python-geometry-method-in-tkinter/)即 [`pack()`](https://www.geeksforgeeks.org/python-pack-method-in-tkinter/)、[`grid()`](https://www.geeksforgeeks.org/python-grid-method-in-tkinter/)、[`place()`](https://www.geeksforgeeks.org/python-place-method-in-tkinter/)。所有这些方法都有不同的作用。

现在，让我们讨论一下使用 Python-tkinter 自动隐藏滚动条的话题。
...
以下示例说明了使用 Python-tkinter 自动隐藏滚动条的用法：
