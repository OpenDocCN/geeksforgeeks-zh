...
**阅读序列比对：** Biopython 提供的 `AlignIO` 用于读写序列比对。生物信息学中有很多格式可以指定序列比对数据类似于序列数据。`AlignIO` 的用法类似于 `Bio.SeqIO`，唯一不同的是 `Bio.SeqIO` 处理序列数据，而 `AlignIO` 用于序列数据对齐。以下是下载样本序列比对文件的一些步骤：

*   首先打开浏览器，访问`http://pfam.xfam.org/family/browse`，可以按字母顺序看到所有 `Pfam` 家族。
...
> qqqltqdlrmpnwslrfvydrnnqdllkrplppgim...nrk B3 pft 7 _ celju/62-126 nrk B3 pft 7 _ celju/62-126 naftertefrtphnvfvldgfeidrelpspva...nrt k4 kem 7 _ simas/61-125 mqntpapaiskhdinwlldqrlervv
>
> Showing Alignment Sequence Record
> qqqltqdlrmpnwslrfvylvdnkrplppplgivlaprpydkvqdrnrk
> avnaterefterthtwarrnfvldgfeidrelpspdlmrdldrpfkkknrt
> mqntpapailieskhdinwlldqrgrleqrvpavanqlrsrafrhregpt
