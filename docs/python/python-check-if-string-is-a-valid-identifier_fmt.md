...
> 原文: [https://www.geeksforgeeks.org/python-check-if-string-is-a-valid-identifier/](https://www.geeksforgeeks.org/python-check-if-string-is-a-valid-identifier/)
...
`re.search()`：这个方法要么返回 `None`（如果模式不匹配），要么返回包含字符串匹配部分信息的 `MatchObject`。此方法在第一次匹配后停止，因此这比提取数据更适合测试正则表达式。
