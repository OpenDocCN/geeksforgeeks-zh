...
> 原文：[https://www.geeksforgeeks.org/introduction-to-pafy-module-in-python/](https://www.geeksforgeeks.org/introduction-to-pafy-module-in-python/)
...
> 3. 命令行工具(`ytdl`)用于直接从命令行下载
> 4. 检索网址，在 `vlc` 或 `mplayer` 等播放器中播放视频
> 5. 处理年龄限制视频和不可嵌入视频
> 6. 小型、独立、单一可导入模块文件
> 7. 选择最高质量的流进行下载或流式传输
> 8. 仅下载 `m4v` 或 `webm` 格式的视频(无音频)
> 9. 仅下载 `ogg` 或 `m4a` 格式的音频(无视频)
> 10. 检索播放列表和播放列表元数据
> 11. 适用于 Python 2.6+ 和 3.3+
