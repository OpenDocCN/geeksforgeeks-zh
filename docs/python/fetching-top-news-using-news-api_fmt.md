...
> 原文：[https://www.geeksforgeeks.org/fetching-top-news-using-news-api/](https://www.geeksforgeeks.org/fetching-top-news-using-news-api/)

新闻应用编程接口是一个简单的基于 JSON 的 REST 应用编程接口，用于从网络上搜索和检索新闻文章。利用这个，人们可以获取新闻网站上的头条新闻，或者搜索特定主题（或关键词）的头条新闻。

可以根据某些标准检索新闻。假设要搜索的主题（关键词）是“极客”（geek）或可能与特定渠道有关。一切都可以完成，但是需要 API 键才能开始。
...
首先在你的系统中安装 `newsapi` 和 `pycountry`，如果不在，使用下面的命令进行安装：
