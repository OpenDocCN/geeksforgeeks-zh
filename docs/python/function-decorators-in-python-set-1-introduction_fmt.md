...
> 原文: [https://www.geeksforgeeks.org/function-decorators-in-python-set-1-introduction/](https://www.geeksforgeeks.org/function-decorators-in-python-set-1-introduction/)
...
`add()`返回作为参数传递的`x`和`y`的和，但是它被装饰函数包装，调用`add(2, 3)`将简单地给出两个数字的和，但是当我们调用`add.data`时，然后将`add`函数作为参数传递给装饰函数`attach_data`，并且该函数返回带有属性`data`的`add`函数，该属性设置为3，因此打印它。
