...
> 原文: [https://www.geeksforgeeks.org/how-to-write-spark-udf-user-defined-functions-in-python/](https://www.geeksforgeeks.org/how-to-write-spark-udf-user-defined-functions-in-python/)
...
`udf` 的第二个参数 `floatingtype()` 将总是强制 UDF 函数只返回 `floating type` 中的结果。现在，我们将使用我们的 `udf` 函数，在我们的数据框中的 `RawScore` 列上标记 `UDF`，并将生成一个名为“ <lambda>RawScore”的新列，这将是该列的默认命名。这方面的代码如下所示–
