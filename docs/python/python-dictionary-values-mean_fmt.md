...
> 原文: [https://www.geeksforgeeks.org/python-dictionary-values-mean/](https://www.geeksforgeeks.org/python-dictionary-values-mean/)

给定一个字典，找出所有存在值的平均值。

> **输入**: `test_dict = {"Gfg":4, "is":4, "Best":4, "for":4, "Geeks":4}`
> **输出**: 4.0
> **解释**: (4 + 4 + 4 + 4 + 4) / 5 = 4.0，遂取均值。
>
> **输入**: `test_dict = {"Gfg":5, "is":10, "Best":15}`
> **输出**: 10.0
> **解释**: 这些的平均值为 10.0
...
上述功能的组合可以用来解决这个问题。在本例中，我们使用 `sum()` 和使用 `len()` 计算的总密钥大小 `values()` 来执行求和。
