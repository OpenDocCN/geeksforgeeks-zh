...
> 原文: [https://www.geeksforgeeks.org/how-to-print-dataframe-in-python-without-index/](https://www.geeksforgeeks.org/how-to-print-dataframe-in-python-without-index/)

## 先决条件

`pandas`

打印数据帧时，默认情况下，索引会随输出一起显示，但如果需要，可以将其删除。要打印不带索引的数据帧，必须将 `to_string()` 中的 `index` 参数设置为 `False`。
...
*   导入模块
*   创建一个简单的数据帧。
*   在 `to_string()` 中将 `index` 设置为 `False`
*   打印结果
