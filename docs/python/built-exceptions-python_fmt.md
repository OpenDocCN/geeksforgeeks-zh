...
Python 中有几个内置的异常，在出现错误时会引发。这些内置异常可以使用 `locals()` 内置函数查看，如下所示:
...
### 1. 异常 `BaseException`
这是所有内置异常的基类。它并不意味着由用户定义的类直接继承。对于用户定义的类，使用 `Exception`。此类负责使用 `str()` 和传递的参数创建异常的字符串表示形式。如果没有参数，则返回空字符串。
*   **参数:** 参数是给异常构造函数的参数元组。
...
### 6. `ImportError`
当 `import` 语句无法加载模块时，或者当 `from...import` 中的“来自列表”的名称找不到时，会引发 `ImportError`。
...
这是 `ImportError` 的子类，当找不到模块时，导入会引发该子类。当在系统模块中找不到时，也会引发此问题。
...
`OSError`异常在系统函数返回系统相关错误时引发，包括“文件未找到”或“磁盘已满”等I/O故障。
...
当算术运算的结果超出范围时，会引发`OverflowError`。整数会引发`MemoryError`而不是`OverflowError`。`OverflowError`有时会在整数超出所需范围时引发。由于C语言中浮点异常处理缺乏标准化，浮点运算不会被检查。
...
## 异常递归错误
递归错误源于运行时错误。当解释器检测到超过最大递归深度时，会引发此异常。
...
## 异常运行时间错误
当没有其他异常适用时，会引发运行时间错误。它返回一个字符串，精确地指示出了什么问题。

## `exception StopIteration`
内置函数`next()`和迭代器的`__next__()`方法会引发`StopIteration`错误，以信号表示迭代器已产生所有项。
...
当解析器遇到语法错误时，会引发`SyntaxError`。语法错误可能发生在`import`语句中，或在调用内置函数`exec()`或`eval()`时，或在读取初始脚本或标准输入时发生。
...
## 异常系统错误
当解释器发现内部错误时，会引发系统错误。相关联的值是一个字符串，指示出了什么问题。

## 异常系统退出
调用`sys.exit()`函数时，系统退出被引发。对`sys.exit()`的调用被转换为异常，以执行清理处理程序(`try`语句的`finally`子句)和调试脚本，而不会冒失去控制的风险。
...
`UnboundLocalError`是`NameError`的一个子类，当在函数或方法中引用局部变量但该变量尚未赋值时引发。
...
## 异常 `UnicodeError`
这个异常是`ValueError`的一个子类。发生与Unicode相关的编码或解码错误时，会引发`UnicodeError`。

## `exception ValueError`
当内置操作或函数接收到类型正确但值无效的参数时，会引发`ValueError`。
...
当除法或取模运算的第二个参数为零时，会引发`ZeroDivisionError`。此异常返回一个字符串，指示操作数的类型和操作。
...
本文由**阿迪提·古普塔**供稿。如果你喜欢GeeksforGeeks并想投稿，你也可以使用[contribute.geeksforgeeks.org](http://contribute.geeksforgeeks.org)写一篇文章或者把你的文章邮寄到`contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
