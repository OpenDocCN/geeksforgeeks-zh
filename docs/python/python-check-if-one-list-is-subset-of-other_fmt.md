...
> 原文：[https://www.geeksforgeeks.org/python-check-if-one-list-is-subset-of-other/](https://www.geeksforgeeks.org/python-check-if-one-list-is-subset-of-other/)

有时我们会遇到这样的问题：检查一个列表是否只是列表的扩展，即是否只是一个列表的超集。这类问题在竞争性编程中相当普遍。`all()` 函数有助于这一事业。让我们讨论实现这一特定任务的各种方法。
