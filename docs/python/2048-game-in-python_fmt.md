...
本文件包含实现 2048 游戏逻辑的所有函数。

## 初始化与状态

### `start_game()`

此函数用于初始化游戏网格。它创建一个 4x4 的二维列表 `mat`，所有元素初始为 0。随后打印操作指令，并调用 `add_new_2(mat)` 添加第一个数字，最后返回初始化的网格。
...
此函数在网格 `mat` 的随机空单元格（值为 0）中添加一个数字 2。它通过循环随机选择行索引 `r` 和列索引 `c`，直到找到一个空单元格。
...
此函数检查网格 `mat` 的当前状态并返回结果字符串：
- 如果任何单元格的值为 2048，返回 `'WON'`。
- 如果存在值为 0 的空单元格，返回 `'GAME NOT OVER'`。
- 如果在任意方向（左、右、上、下）上存在可合并的相邻相同单元格，返回 `'GAME NOT OVER'`。
- 否则，返回 `'LOST'`。
...
此函数将网格 `mat` 中的所有非零单元格向左压缩，移除行内的空隙。它返回压缩后的新网格 `new_mat` 和一个布尔值 `changed`，指示网格是否发生了变化。
...
此函数在压缩后的网格 `mat` 上，从左到右合并相邻的相同值单元格。合并后，左侧单元格值加倍，右侧单元格置 0。它返回合并后的网格和一个布尔值 `changed`。
...
此函数反转网格 `mat` 中每一行的元素顺序，用于实现向右移动的操作。它返回反转后的新网格。
...
此函数计算网格 `mat` 的转置，即交换行和列。用于实现向上和向下移动的操作。它返回转置后的新网格。
...
此函数实现向左移动网格 `grid` 的逻辑。它依次调用 `compress`、`merge` 和再次 `compress`，并返回结果网格和一个布尔值 `changed`。
...
此函数实现向右移动网格 `grid` 的逻辑。它通过反转网格、调用 `move_left`、再反转结果来实现。
...
此函数实现向上移动网格 `grid` 的逻辑。它通过转置网格、调用 `move_left`、再转置结果来实现。
...
此函数实现向下移动网格 `grid` 的逻辑。它通过转置网格、调用 `move_right`、再转置结果来实现。
