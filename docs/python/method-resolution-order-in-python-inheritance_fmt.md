...
> 原文：[https://www.geeksforgeeks.org/method-resolution-order-in-python-inheritance/](https://www.geeksforgeeks.org/method-resolution-order-in-python-inheritance/)
...
在 Python 的旧版本（2.1）中，我们必然会使用旧风格的类，但是在 Python（3.x）&2.2 中，我们必然只会使用新的类。新样式类的第一个父类继承自 Python 根 `object` 类。
...
C3 线性化算法是一种使用新型类的算法。它用于移除由 DLR 算法创建的不一致。它们有一定的局限性：
