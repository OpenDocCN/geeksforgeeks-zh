...
因此我们可以说`装饰者`是一个`可调用的`接受并返回一个可调用的。
...
`new_init`，它的主要职责是运行包装函数，并为包装函数添加额外的功能。`@functools.wraps(org_init)`更新包装函数，以反映包装函数的外观。查看[功能工具](https://www.geeksforgeeks.org/python-functools-wraps-function/)了解详情。
