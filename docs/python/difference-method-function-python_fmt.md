...
> 原文: [https://www.geeksforgeeks.org/difference-method-function-python/](https://www.geeksforgeeks.org/difference-method-function-python/)
...
了解更多 [Python `ceil()`和`floor()`方法](https://www.geeksforgeeks.org/floor-ceil-function-python/)。

## 函数

1.  函数是一个代码块，它的名字也叫做(独立)。
2.  函数可以有不同的参数，也可以没有任何参数。如果有任何数据(参数)通过，则为明确通过。
3.  它可以返回也可以不返回任何数据。
4.  函数不处理类及其实例概念。
...
2.  函数只能通过名称来调用，因为它是独立定义的。但是方法不能仅仅用它的名字来调用，我们需要通过引用定义它的类来调用这个类，也就是说，方法是在一个类中定义的，因此它们依赖于这个类。