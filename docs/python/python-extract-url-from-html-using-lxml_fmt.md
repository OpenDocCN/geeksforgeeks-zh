...
> 原文: [https://www.geeksforgeeks.org/python-extract-url-from-html-using-lxml/](https://www.geeksforgeeks.org/python-extract-url-from-html-using-lxml/)
...
`requests` 是一个 Python 库，用来报废网站。它使用 `get()` 方法请求网络服务器的网址，并以网址作为参数，作为回报，它给出响应对象。该对象将包括请求和响应的详细信息。阅读网页内容，使用 `response.text()` 方法。该内容由 web 服务器根据请求发回。
