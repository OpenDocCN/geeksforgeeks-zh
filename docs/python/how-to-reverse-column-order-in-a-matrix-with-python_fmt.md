...
> 原文：[https://www.geeksforgeeks.org/how-to-reverse-column-order-in-a-matrix-with-python/](https://www.geeksforgeeks.org/how-to-reverse-column-order-in-a-matrix-with-python/)
...
> **语法：** `numpy.fliplr(m)`
>
> **参数：** `m`（`array_like`）– 输入数组必须至少为 2d
>
> **返回值：** `ndarray` – 返回 `m` 的视图，列反转，该操作的时间复杂度为 O(1)。
