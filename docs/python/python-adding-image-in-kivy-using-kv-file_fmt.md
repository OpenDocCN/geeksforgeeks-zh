...
> 原文: [https://www.geeksforgeeks.org/python-add-image-in-kivy-using-kv-file/](https://www.geeksforgeeks.org/python-adding-image-in-kivy-using-kv-file/)
...
> `from kivy.uix.image import Image, AsyncImage` (使用时不需要 .kv 文件)

因为模块 `kivy.uix.image` 拥有与图像相关的所有功能。

图像可以通过两种类型加载到应用程序:

> **1) 同步加载:** 从系统加载图像(必须从 .py 和 .kv 文件所在的文件夹加载)
