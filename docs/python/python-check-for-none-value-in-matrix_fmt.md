...
Python 支持列表作为其列表元素，因此可以形成矩阵。有时我们可能会有一个实用程序，其中我们要求在列表(即矩阵)列表中执行`None`检查，这在所有编码领域都很常见，尤其是数据科学领域。让我们讨论一下实现这一点的某些方法。

**方法#1:使用`any()` +列表理解**
可以使用`any()`函数执行`if`条件的任务，并且可以使用`列表理解`计算嵌套列表中每个元素的检查。
