...
> 原文:[https://www . geeksforgeks . org/program-generate-可能-有效-IP-address-给定-string/](https://www.geeksforgeeks.org/program-generate-possible-valid-ip-addresses-given-string/)
...
用“.”分割字符串然后检查所有角落的箱子。在进入循环之前，检查字符串的大小。使用循环字符串生成所有可能的组合。如果发现 IP 有效，则返回 IP 地址，否则只返回空列表。
以下是上述办法的实施情况:
...
*   **时间复杂度:** `O(n^3)`，其中 `n` 是字符串的长度。需要字符串的三次嵌套遍历，其中 `n` 总是小于 12。
*   **辅助空间:** `O(n)`。只要需要额外的空间。

## 另一种有效的方法(动态规划)

对于这个问题存在一种 `dp` 方法，可以在时间复杂度 `O(n*4*3)=O(12n)=O(n)` 和空间复杂度 `O(4n)` 上求解。
