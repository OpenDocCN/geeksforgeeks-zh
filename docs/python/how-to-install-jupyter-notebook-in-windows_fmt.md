...
> 原文: [https://www.geeksforgeeks.org/how-to-install-jupyter-notebook-in-windows/](https://www.geeksforgeeks.org/how-to-install-jupyter-notebook-in-windows/)
...
*   `使用 Anaconda:`
    使用 `Anaconda` 发行版安装 `Python` 和 `Jupyter`，该发行版包括 `Python`、`Jupyter Notebook` 和其他用于科学计算和数据科学的常用软件包。安装 `Anaconda`，请参考[如何在 windows 上安装 Anaconda？](https://www.geeksforgeeks.org/how-to-install-anaconda-on-windows/)并按照提供的说明操作。
*   `使用 PIP:`
    使用 `PIP` 包管理器安装 `Jupyter`，用于安装和管理用 `Python` 编写的软件包/库。要安装 `pip`，请参考[如何在 Windows 上安装 PIP？](https://www.geeksforgeeks.org/how-to-install-pip-on-windows/)并遵循提供的说明。
