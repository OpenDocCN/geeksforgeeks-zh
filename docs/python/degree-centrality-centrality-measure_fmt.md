...
在图论中，图的一个顶点的度(或化合价)是指入射到该顶点的边的个数，循环数为两次。[1]顶点的度数 `v` 表示为 `\deg(v)` 或 `\deg v`。图 `G` 的最大度数用 `\Delta (G)` 表示，图的最小度数用 `\delta (G)` 表示，是其顶点的最大和最小度数。在右边的图中，最大度数为 5，最小度数为 0。在正则图中，所有的度都是相同的，所以我们可以说图的度。
...
对于具有 `|V|` 顶点和 `|E|` 边的给定图形 `G:=(V,E)`，顶点 `v` 的度中心度定义为

`C_{D}(v)=\deg(v)`

计算图中所有节点的度中心度在图的密集邻接矩阵表示中取 `\Theta(V^2)`，在稀疏矩阵表示中取 `\Theta(E)`。

节点级中心性的定义可以扩展到整个图，在这种情况下我们说的是图的中心性。设 `v*` 为 `G` 中中心度最高的节点。设 `X:=(Y,Z)` 为最大化以下量的 `|Y|` 节点连通图(其中 `y*` 为 `X` 中度中心度最高的节点):
...
当图形 `X` 包含一个所有其他节点都连接到的中心节点(星形图形)时，`H` 的值最大化，在这种情况下
...
你可以在 [https://en.wikipedia.org/wiki/Centrality#Degree_centrality](https://en.wikipedia.org/wiki/Centrality#Degree_centrality) 和 [http://networkx.readthedocs.io/en/networkx-1.10/index.html](http://networkx.readthedocs.io/en/networkx-1.10/index.html) 找到更多信息。
