...
`exec()` 函数用于 Python 程序的动态执行，可以是字符串，也可以是目标代码。如果它是一个字符串，则该字符串被解析为一组 Python 语句，然后执行这些语句，除非出现语法错误；如果它是一个目标代码，则简单地执行它。我们必须小心，返回语句不能在函数定义之外使用，甚至不能在传递给 `exec()` 函数的代码上下文中使用。它不返回任何值，因此返回 `None`。
...
现在让我们看看这个函数是如何工作的。在下面的代码中，我们使用了一个目标代码，并使用 `exec()` 函数执行了它。我们刚刚获取了 `object` 参数，省略了另外两个字段。
...
Python 允许我们通过使用可能不需要的 `globals` 和 `locals` 参数来限制各种变量和方法的使用。这些 `locals` 和 `globals` 参数主要用于局部或全局变量字典。如果缺少 `locals` 参数，`globals` 参数将取代，这意味着 `globals` 参数可用于两个字段。让我们看看代码如何只传递 `globals` 参数。
...
所以我们看到，在传递一个空字典作为 `globals` 参数时，只显示 `__builtins__` 而不显示其他数学函数，就像前面的例子一样。这意味着 `object` 参数将只支持 `__builtins__` 参数。这表明所有其他功能都被限制在 `object` 上。为了证明这一点，让我们试着用一个数学函数，看看会发生什么。
...
本示例应该打印 120 作为输出，但是它显示“无输出”并引发异常。虽然我们已经导入了数学模块，但是由于我们通过传递 `globals` 参数设置的限制，`factorial()` 方法无法工作。
我们也可以允许许多函数中的少数函数执行。假设我们想限制除 `factorial()` 函数之外的所有其他数学模块。
...
我们还可以更改这些预定义方法的名称，并在执行过程中为它们提供用户定义的名称。我们可以将函数的名称从 `factorial()` 改为 `fact()`，这是用户自定义的。
...
现在让我们看看在传递 `globals` 和 `locals` 参数时还能做什么。使用 `locals` 参数，我们可以根据需要实现这些功能。
