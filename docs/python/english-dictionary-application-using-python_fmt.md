...
> 原文: [https://www.geeksforgeeks.org/english-dictionary-application-using-python/](https://www.geeksforgeeks.org/english-dictionary-application-using-python/)

`Python` 中的字典是一个无序的数据值集合，用于像地图一样存储数据值，与其他只保存单个值作为元素的数据类型不同，字典保存 `key:value` 对。字典中提供了键值，以使其更加优化。字典中的每个键值对由冒号分隔，而每个键由“逗号”分隔。

`Python` 中的字典的工作方式类似于现实世界中的字典。字典的键必须是唯一的并且是不可变的数据类型，例如字符串、整数和元组，但是键值可以重复并且是任何类型。

注意 – 要了解更多关于词典的信息，请点击[此处](https://www.geeksforgeeks.org/python-dictionary/)。

## 所需模块

*   `json`: 内置 `python`，无需外接安装。要了解更多关于 `JSON` 的信息，请点击[此处](https://www.geeksforgeeks.org/javascript-json/)。
