...
> 原文：[https://www.geeksforgeeks.org/getpass-and-getuser-in-python-password-without-echo/](https://www.geeksforgeeks.org/getpass-and-getuser-in-python-password-without-echo/)
...
`getpass()` 函数用于使用字符串提示向用户进行提示，并将用户输入的内容读取为密码。输入读数默认为“密码:”并作为字符串返回给调用者。
让我们通过一些例子来了解它的实现。
