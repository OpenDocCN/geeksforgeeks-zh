...
> 原文：[https://www.geeksforgeeks.org/extract-text-from-pdf-file-using-python/](https://www.geeksforgeeks.org/extract-text-from-pdf-file-using-python/)
...
*   `pdfReader = PyPDF2.PdfFileReader(pdfFileObj)`
    在这里，我们创建一个`PyPDF2`模块的`PdfFileReader`类的对象，并通过 pdf 文件对象得到一个 pdf reader 对象。

*   `print(pdfReader.numPages)`
    `numPages`属性给出 pdf 文件的页数。例如，在我们的例子中，它是 20（见输出的第一行）。

*   `pageObj = pdfReader.getPage(0)`
    现在，我们创建一个`PyPDF2`模块的`PageObject`类的对象。pdf 阅读器对象有一个函数`getPage()`，它以页码（从索引 0 开始）为参数，返回 Page 对象。
