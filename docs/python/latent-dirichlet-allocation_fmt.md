...
- 每个主题在具有公共对称先验的单词上有一个 V 维多项式 `β_k`。
- 对于每个主题 `1…k`：
    - 在单词上画一个多项式 `\varphi \sim Dir(\beta)`。
- 对于每个文档 `1…d`：
    - 在题目上画一个多项式 `\theta \sim Dir(\alpha)`
    - 对于每个单词 `w_{N_d}`：
        - 用 `Z_{N_d} \epsilon [1..K]` 画一个题目 `Z_{N_d} \sim Mult(\theta_D)`
...
- 狄利克雷分布可以定义为向量值输入的概率密度，该向量值输入具有与我们的多项式参数 `\theta` 相同的特征。它具有非零值，例如：
...
- LDA 中的推理问题，计算给定文档和语料库参数 `α` 和 `β` 的隐藏变量的后验值。也就是计算 `P(`
