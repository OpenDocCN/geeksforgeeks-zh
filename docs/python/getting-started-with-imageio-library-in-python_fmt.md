...
> 原文: [https://www.geeksforgeeks.org/getting-started-with-imageio-library-in-python/](https://www.geeksforgeeks.org/getting-started-with-imageio-library-in-python/)
...
为了读取图像，我们必须使用 `imageio.imread()` 方法。
...
为了读取一个 `GIF` 文件，我们不得不使用 `imageio.get_reader()` 方法。
...
>     *   `'?'` 因为不在乎
>
> **返回:** `Reader` 对象，可用于从指定文件中读取数据和元数据。
...
为了创建图像文件，我们必须使用 `imageio.imwrite()` 方法。

> **语法:** `imageio.imwrite(filename, numpy_ndarray, format=None)`
>
> **参数:**
>
> *   **文件名:** 保存文件的路径/名称
