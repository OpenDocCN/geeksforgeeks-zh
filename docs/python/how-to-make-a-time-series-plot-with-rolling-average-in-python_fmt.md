...
> 原文: [https://www.geeksforgeeks.org/how-to-make-a-time-series-plot-with-rolling-average-in-python/](https://www.geeksforgeeks.org/how-to-make-a-time-series-plot-with-rolling-average-in-python/)
...
滚动平均值或移动平均值是最后 `n` 个值的简单平均值。它可以帮助我们发现本来很难发现的趋势。此外，它们可以用来确定长期趋势。你可以简单地计算滚动平均值，将之前的“n”个值相加，然后除以“n”本身。但就此而言，滚动平均值的第一个 `(n-1)` 值将是 `NaN`。

在本文中，我们将学习如何使用 `Pandas` 和 `Seaborn` 库在 Python 中制作滚动平均值的时间序列图。下面是使用 `pandas` 计算滚动平均值的语法。

> **语法:** `pandas.DataFrame.rolling(n).mean()`
...
## 步骤 4: 使用 `pandas.DataFrame.rolling.mean()` 计算滚动平均值
