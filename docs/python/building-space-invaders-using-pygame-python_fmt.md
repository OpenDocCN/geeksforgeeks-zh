...
> 原文: [https://www.geeksforgeeks.org/building-space-invaders-using-pygame-python/](https://www.geeksforgeeks.org/building-space-invaders-using-pygame-python/)
...
`isCollision()` 函数返回的值存储在 `collision` 变量中。根据函数内部为冲突设置的标准，函数返回的值为真或假。我们来看看 `isCollision()` 函数里面是什么:
...
返回 `True` 或 `False`，在此基础上代码决定游戏是否结束。为了理解 `game_over()` 函数，让我们来看看下面出现在游戏循环中的代码片段:
