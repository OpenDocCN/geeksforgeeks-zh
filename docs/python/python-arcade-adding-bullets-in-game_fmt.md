...
> 原文: [https://www.geeksforgeeks.org/python-arcade-adding-bullets-in-game/](https://www.geeksforgeeks.org/python-arcade-adding-bullets-in-game/)

在本文中，我们将学习如何使用 Python 在 Arcade 中为游戏添加子弹。

## 添加子弹

在本例中，我们将向屏幕添加子弹。为此，我们将使用一些函数：

`draw_text()`：此功能用于使用 Pyglet 的标签将文本绘制到屏幕。

> **语法：** `arcade.draw_text(text, x, y, color, size, width, align, font_name)`
>
> **参数：**
>
> *   `text`：我们要显示的文本
...
`PhysicsEnginePlatformer()`：在平台转换器中使用的简单物理引擎。

> **语法：** `arcade.PhysicsEnginePlatformer(player_sprite, platforms, gravity, ladders)`
>
> **参数：**
>
> *   `player_sprite`：玩家的精灵
...
class MainGame(arcade.Window):
    ...
    # Creating on_update function to update the x coordinate
    ...
    # Creating function to change the velocity when button is pressed
    def on_key_press(self, symbol, modifier):

        # Checking the button pressed and changing the value of velocity
    ...
    # Creating function to change the velocity when button is released
    def on_key_release(self, symbol, modifier):

        # Checking the button released and changing the value of velocity
        if symbol == arcade.key.LEFT:
            self.vel_x = 0
        elif symbol == arcade.key.RIGHT:
            self.vel_x = 0
