...
> 原文：[https://www.geeksforgeeks.org/create-and-import-modules-in-python/](https://www.geeksforgeeks.org/create-and-import-modules-in-python/)

在 Python 中，模块是一个包含 Python 语句和定义的独立的 Python 文件，就像一个名为`GFG.py`的文件，可以认为是一个名为`GFG`的模块，可以借助`import`语句导入。然而，人们可能会对模块和包之间的区别感到困惑。包是目录中模块的集合，为模块提供结构和层次。
