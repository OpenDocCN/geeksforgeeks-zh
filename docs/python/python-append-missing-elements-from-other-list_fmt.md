...
> 原文: [https://www.geeksforgeeks.org/python-append-missing-elements-from-other-list/](https://www.geeksforgeeks.org/python-append-missing-elements-from-other-list/)
...
## 方法二: 使用 `set()` + `-` 运算符 + `extend()`

在本例中，我们使用 `set()` 和 `-` 运算符检查列表 2 中缺少的列表 1 的元素，并使用 `extend()` 连接这两个列表以获得所需的结果。
