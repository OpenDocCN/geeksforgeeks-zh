...
*   仅在函数内部引用的变量是隐式全局变量。
*   我们使用`global`关键字在函数中使用全局变量。
*   不需要在函数外使用`global`关键字。

## 使用全局关键字:

要访问函数内部的全局变量，不需要使用`global`关键字。
...
在上例中，在进行更改之前和之后()，变量`x`取局部变量的值，即`x = 15`。在`add()`函数之外，变量`x`将取`change()`函数中定义的值，即`x = 20`。因为我们已经在`x`中使用了`global`关键字来创建`change()`函数(局部范围)中的全局变量。