...
> 原文：[https://www.geeksforgeeks.org/create-an-n-x-n-square-matrix-where-all-the-sub-matrix-have-the-sum-of-opposite-corner-elements-as-even/](https://www.geeksforgeeks.org/create-an-n-x-n-square-matrix-where-all-the-sub-matrix-have-the-sum-of-opposite-corner-elements-as-even/)

给定一个整数 `n`，任务是生成一个由 `(n×n)` 组成的`方阵`，其元素范围从 `1` 到 `n^2`，条件如下：

*   矩阵的元素应该是不同的，即只使用一次从 `1` 到 `n^2` 的数字。
...
> 16 15 14 13
> ```
> **说明:** 子矩阵 `[1 2; 4 5]`，`[2 3; 5 6]`，`[4 5; 7 8]`，`[5 6; 8 9]`，`[1 2 3; 4 5 6; 7 8 9]` 满足对角元素具有偶和的条件。
...
*   当 `n` 是奇数时，对角线已经在所有奇数或偶数元素中，因此我们不需要修改和生成简单的二维数组。
