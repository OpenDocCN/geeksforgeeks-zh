...
> 原文：[https://www.geeksforgeeks.org/mouse-keyboard-automation-using-python/](https://www.geeksforgeeks.org/mouse-keyboard-automation-using-python/)
...
用 `.py` 扩展名保存此文件，然后运行文件。这个 python 代码使用 `size()` 函数以 `x, y` 格式输出你的屏幕分辨率。
...
这段代码使用 `moveTo()` 函数，该函数接受 `x` 和 `y` 坐标，以及一个可选的 `duration` 参数。该函数将鼠标指针从其当前位置移动到 `x, y` 坐标，并按照 `duration` 参数的指定花费时间。保存并运行这个 python 脚本，看到鼠标指针神奇地从当前位置移动到坐标 `(100, 100)`，这个过程需要 1 秒钟。
...
`scroll` 函数以像素数为自变量，将屏幕向上滚动到给定的像素数。
...
您可以使用 `typewrite()` 功能自动输入字符串。只需传递您想要作为此函数的参数键入的字符串。
...
使用 `hotkey()` 功能按 `ctrl-c`、`ctrl-a` 等组合键。
