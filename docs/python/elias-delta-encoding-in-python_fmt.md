...
> 原文: [https://www.geeksforgeeks.org/elias-delta-encoding-in-python/](https://www.geeksforgeeks.org/elias-delta-encoding-in-python/)
...
*   从 `math` 库中导入 `log`、`floor` 函数来执行对数运算。
*   从用户获取输入 `k`，以在 Elias Gamma 中编码。
*   使用 `math` 模块中的 `floor` 和 `log` 函数，找到 `1 + floor(log2(X))` 并将其存储在变量 `N` 中。
...
*   使用 `"{0:b}".format(k)` 找到 `k` 的二进制等价物，并将其存储在名为 `binary` 的变量中。
    ...
    *   `b` 指定参数应转换为二进制形式。
*   返回字符串 `binary[1:]`，它是没有 MSB 的 `X` 的二进制表示。
...
*   使用 `math` 模块中的 `floor` 和 `log` 函数，找到 `1 + floor(log2(k))`。
*   将 `1 + floor(log2(k))` 的结果传递给 `EliasGammaEncode` 函数。
...
*   得到 `EliasGammaEncode` 的结果和无 MSB 的 `k` 的二进制表示
*   连接两个结果并在控制台上打印出来
