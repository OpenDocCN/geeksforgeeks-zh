...
> 原文：[https://www.geeksforgeeks.org/how-to-facet-a-scatter-plot-with-altair/](https://www.geeksforgeeks.org/how-to-facet-a-scatter-plot-with-altair/)
...
在这里，我们使用牛郎星库制作散点图。为此，我们使用阿尔泰中的 `Chart()` 函数加载数据，然后使用 `mark_point()` 函数进行散点图。然后我们使用美学 x 轴和 y 轴来 `encode()` 函数。完成散点图后，我们将使用分组的列值（如簇）对其进行分面。
