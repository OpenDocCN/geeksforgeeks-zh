...
> 原文: [https://www.geeksforgeeks.org/convert-python-dictionary-list-to-pyspark-dataframe/](https://www.geeksforgeeks.org/convert-python-dictionary-list-to-pyspark-dataframe/)
...
其中，`column` 是要在 pyspark dataframe 中获取的字典列的名称，`DataType` 是特定列的数据类型。
...
> **语法:** `spark.createDataFrame([Row(**iterator) for iterator in data])`
>
> **其中:**
>
> *   `createDataFrame()` 是创建数据帧的方法
> *   `Row(**iterator)` 来迭代字典列表。
> *   `data` 是字典列表
