...
> 原文：[https://www.geeksforgeeks.org/find-the-longest-common-prefix-between-two-strings-after-performing-swaps-on-second-string/](https://www.geeksforgeeks.org/find-the-longest-common-prefix-between-two-strings-after-performing-swaps-on-second-string/)

给定两个字符串 `a` 和 `b`。在对字符串 `b` 执行零个或多个操作后，找到它们之间最长的公共前缀。在每个操作中，您可以交换任意两个字母。
...
假设我们只允许在字符串 `b` 中执行交换，前缀的长度应该最大化。所以想法是遍历字符串 `a`，检查字符串 `a` 中当前字符的频率是否与字符串 `b` 中相同或更少。如果是，则在字符串中向前移动，否则会中断并打印字符串中与字符匹配的部分的长度 `b`。
