...
> 原文: [https://www.geeksforgeeks.org/difference-between-str-capitalize-vs-str-title/](https://www.geeksforgeeks.org/difference-between-str-capitalize-vs-str-title/)

`title()` 和 `capitalize()` 具有相似的首字母大写功能。让我们看看他们两个的区别。
...
在 Python 中，`capitalize()` 方法将字符串的第一个字符转换为大写字母。如果字符串的第一个字符是大写的，那么它将返回原始字符串。
...
### `title()` 和 `capitalize()` 之间的区别

它们之间的区别在于 Python string 方法 `title()` 返回一个字符串的副本，其中所有单词的第一个字符都是大写的，而 string 方法 `capitalize()` 返回一个字符串的副本，其中只有整个字符串的第一个单词是大写的。
