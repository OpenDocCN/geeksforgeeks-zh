...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-a-nested-ordereddict-to-dict/](https://www.geeksforgeeks.org/how-to-convert-a-nested-ordereddict-to-dict/)

在本文中，我们将讨论如何将嵌套的 `OrderedDict` 转换为 `dict`？在此之前，我们必须了解一些概念：
...
*   一个 [**OrderedDict**](https://www.geeksforgeeks.org/ordereddict-in-python/) 是一个字典子类，它记住了首次插入键的顺序。
*   `dict()` 和 `OrderedDict()` 唯一的区别是：`OrderedDict` **保留了插入按键的顺序**。一个常规字典不会跟踪插入顺序，迭代它会以任意顺序给出值。相比之下，插入项目的顺序由 `OrderedDict` 记忆。

为了定义嵌套的 `OrderedDict`，我们使用 Python 中的 `collections` 模块。
...
我们可以在一行内将 `OrderedDict` 转换为 `dict`，但这仅适用于简单的 `OrderedDict`，而不适用于嵌套 `OrderedDict`。
...
因此，要将嵌套的 `OrderedDict` 转换为 `dict`，我们使用 `json.loads()` 和 `json.dumps()` 方法。
...
*   `json.dumps()` 函数的作用是：将 Python 对象转换成 json 字符串。
