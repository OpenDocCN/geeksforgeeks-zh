...
> 原文: [https://www.geeksforgeeks.org/how-to-convert-a-list-to-a-dataframe-row-in-python/](https://www.geeksforgeeks.org/how-to-convert-a-list-to-a-dataframe-row-in-python/)
...
> 语法: `pd.DataFrame(list1, columns, dtype)`
> 其中
> - `list1` 是输入值的列表
> - `columns` 是值列表的列名
> - `dtype` 是列数据类型
...
在这里，我们将单独的列表作为输入，这样每个列表都将作为一列，因此列表的数量就是 dataframe 中的 n 列，并且使用 `zip` 函数来组合列表。

> 语法: `pd.DataFrame(list(zip(list1, list2, ..., listn)), columns)`
> 其中
> - `columns` 是列表值的列名
> - `list1, listn` 表示列的输入列表数
