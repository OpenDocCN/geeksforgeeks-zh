...
> 原文: [https://www.geeksforgeeks.org/missing-data-imputation-with-fancyimpute/](https://www.geeksforgeeks.org/missing-data-imputation-with-fancyimpute/)

在真实世界的数据集中，总会有一些数据丢失。这主要与数据是如何收集的有关。缺失数据在创建预测模型中起着重要的作用，因为有些算法在缺失数据集上表现不佳。

## fancyimpute

`fancyimpute` 是一个缺失数据插补算法库。`fancyimpute` 使用机器学习算法来估算缺失值。`fancyimpute` 使用所有列来估算缺失的值。有两种方法可以估算缺失的数据：
