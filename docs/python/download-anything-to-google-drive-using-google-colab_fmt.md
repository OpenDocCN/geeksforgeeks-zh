...
> 原文: [https://www.geeksforgeeks.org/download-anything-to-google-drive-using-google-colab/](https://www.geeksforgeeks.org/download-anything-to-google-drive-using-google-colab/)
...
**[谷歌 Colab](https://colab.research.google.com/notebooks/welcome.ipynb)** :

谷歌 Colab 是一款支持 GPU 的免费云服务。它就像 Jupyter 笔记本，使用起来非常简单。在本教程中，我们将使用谷歌 Colab 下载任何东西到我们的谷歌驱动器。
...
在运行代码时，会出现一个蓝色链接和一个文本框，我们需要提供一个权限文本。因此，点击链接，一个新的标签将会打开，在那里你将被要求允许访问谷歌驱动器。提供权限后，将显示一个文本，我们需要复制并粘贴在 colabs 的文本框。
...
运行此代码下载 gdrive 中的文件（用您的**文件的 url** 更改 `file_url`）。
...
我们可以使用 google colab 下载 google drive 上的任何文件。你可以看到一个文件夹 *Parrot*（3.7 GB 的 Parrot 操作系统），使用 Colab 下载到 gdrive。