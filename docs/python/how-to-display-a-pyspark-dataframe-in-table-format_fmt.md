...
> 原文: [https://www.geeksforgeeks.org/how-to-display-a-pyspark-dataframe-in-table-format/](https://www.geeksforgeeks.org/how-to-display-a-pyspark-dataframe-in-table-format/)

在本文中，我们将以表格格式显示 PySpark 数据帧的数据。我们将使用 `show()` 功能和 `toPandas()` 功能以所需的格式显示数据帧。
...
Pandas 代表一种面板数据结构，用于以像表格一样的二维格式表示数据。
...
使用不带参数的 `show()` 函数。它将产生我们所拥有的整个数据帧。
...
使用 `show()` 函数，`vertical=True` 作为参数。垂直显示数据框中的记录。

> **语法:** `dataframe.show(vertical)`
>
> `vertical` 可以是 `True` 也可以是 `False`。
...
使用 `show()` 函数，参数为 `truncate`。在所有列的每个值中显示第一个字母。
...
使用 `toPandas()` 方法，该方法将其转换为 Pandas 数据帧，该数据帧看起来非常像表格。
