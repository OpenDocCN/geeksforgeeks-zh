...
> 原文: [https://www.geeksforgeeks.org/a-simple-news-app-with-tkinter-and-newsapi/](https://www.geeksforgeeks.org/a-simple-news-app-with-tkinter-and-newsapi/)

`先决条件`： [Tkinter](https://www.geeksforgeeks.org/python-gui-tkinter/)

`Tkinter` 是一个非常著名的 `python` 库，用于为基于 `windows` 和 `UNIX` 的操作系统构建桌面应用程序。这里的想法是生成一个 `python` 代码，用 `Tkinter` 构建一个新闻应用程序。您可以很容易地从 `newsapi.org` 获得一个 `API` 密钥。
...
![最终应用](img/580e90014adc53c7be01fc10f0fda10a.png)