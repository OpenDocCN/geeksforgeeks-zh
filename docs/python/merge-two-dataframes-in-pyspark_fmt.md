...
`reduce(fun, seq)` 函数用于将参数中传递的特定函数应用于所传递序列中提到的所有列表元素。该功能在 `functools` 模块中定义。
