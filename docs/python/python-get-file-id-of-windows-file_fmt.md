...
> 原文: [https://www.geeksforgeeks.org/python-get-file-id-of-windows-file/](https://www.geeksforgeeks.org/python-get-file-id-of-windows-file/)
...
不需要为了调用命令行而仅使用`OS`库。也可以使用替代方法来调用命令行(例如。`subprocess.popen()`)也可以用于此目的)。在本文中，我们将使用文件的路径来查询驱动器上文件的文件标识。然后稍后将使用这个文件标识来获取文件的绝对路径。
