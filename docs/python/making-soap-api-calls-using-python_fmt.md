...
> 原文: [https://www.geeksforgeeks.org/making-soap-api-calls-using-python/](https://www.geeksforgeeks.org/making-soap-api-calls-using-python/)
...
首先，我们导入 [`requests`](https://www.geeksforgeeks.org/python-requests-tutorial/) 库，然后我们定义 SOAP URL。

下一步也是最重要的一步是根据 SOAP URL 中提供的结构格式化 XML 主体。要知道格式，只需访问 SOAP URL，点击 `CountryISOCode` 链接，并相应地格式化 XML。

然后，您只需准备标题并进行 POST 调用。
...
现在我们已经看到了如何用 `requests` 进行 SOAP 调用，我们将看到通过 `Zeep` 进行调用是多么容易。首先需要安装 `zeep`。
...
*   首先，设置 WSDL URL。您只需访问基本网址并点击**服务描述**，即可获得 WSDL URL。这将带你到 WSDL 的网址。基本网址将是*服务 URL*，并在基本网址后附加服务名称。
*   接下来，您需要创建一个标题元素。现在，您需要用 `method_url` 和 `service_url` 设置头元素。
*   现在，用 WSDL URL 初始化一个 `zeep` 客户端。
