...
> 原文: [https://www.geeksforgeeks.org/python-convert-matrix-to-sets-of-set/](https://www.geeksforgeeks.org/python-convert-matrix-to-sets-of-set/)
...
## 方法#1: 使用 `set()` + `frozenset()` + 生成器表达式

在这种情况下，我们在生成器表达式中执行迭代直到矩阵得到内部集合，并且使用 `set()` 将外部容器转换为集合。内部容器需要是 hashable，因此必须是 `frozenset()` 才能创建。
