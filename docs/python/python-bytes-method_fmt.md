...
`Python bytes()`函数将对象转换为给定大小和数据的不可变字节表示的对象。

> **语法:** `bytes(src, enc, err)`
>
> **参数:**
>
> *   `src`: 必须转换的源对象
...
当我们传递 nothing in `bytes()`函数时，它会创建一个大小为 0 的数组。
...
`bytes()`接受字符串作为参数，并需要一个编码方案来执行它。其中最重要的方面是在编码失败的情况下处理错误，定义的一些错误处理方案如下:
