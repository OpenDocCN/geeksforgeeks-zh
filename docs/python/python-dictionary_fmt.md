...
在 Python 字典中，元素的添加可以通过多种方式完成。一次一个值可以通过定义值和关键字添加到字典中，例如 `Dict[key] = "value"`。使用内置的 `update()` 方法可以更新字典中的现有值。嵌套键值也可以添加到现有字典中。
...
**注意:** `del Dict` 将删除整个字典，因此删除后打印它会产生错误。
...
| 方法 | 描述 |
| :--- | :--- |
| [`copy()`](https://www.geeksforgeeks.org/python-dictionary-copy/) | 方法返回字典的一个浅拷贝。 |
| [`clear()`](https://www.geeksforgeeks.org/python-dictionary-clear/) | `clear()`方法从字典中移除所有项目。 |
| [`pop()`](https://www.geeksforgeeks.org/python-dictionary-pop-method/) | 从字典中移除并返回具有给定键的元素。 |
| [`popitem()`](https://www.geeksforgeeks.org/python-dictionary-popitem-method/) | 从字典中移除任意键值对，并将其作为元组返回。 |
...
| `str()` | 生成字典的可打印字符串表示形式。 |
| [`update()`](https://www.geeksforgeeks.org/python-dictionary-update-method/) | 将字典2的键值对添加到字典中。 |
| [`setdefault()`](https://www.geeksforgeeks.org/python-dictionary-setdefault-method/) | 如果键不在`dict`中，则设置`dict[key]=default`。 |
| [`keys()`](https://www.geeksforgeeks.org/python-dictionary-keys-method/) | 返回字典关键字的列表。 |
| [`items()`](https://www.geeksforgeeks.org/python-dictionary-items-method/) | 返回`dict`的`(键, 值)`元组对的列表。 |
| [`has_key()`](https://www.geeksforgeeks.org/python-dictionary-has_key/) | 如果关键字在字典中，则返回`true`，否则返回`false`。 |
| [`fromkeys()`](https://www.geeksforgeeks.org/python-dictionary-fromkeys-method/) | 用`seq`中的键和设置为`value`的值创建新字典。 |
| [`type()`](https://www.geeksforgeeks.org/python-type-function/) | 返回传递的变量的类型。 |
| `cmp()` | 比较两个字典的元素。 |
...
*   [`get()`方法](https://www.geeksforgeeks.org/get-method-dictionaries-python/)
