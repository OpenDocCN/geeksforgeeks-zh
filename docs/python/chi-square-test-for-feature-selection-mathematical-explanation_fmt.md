...
> 原文：[https://www.geeksforgeeks.org/chi-square-test-for-feature-selection-mathematical-explanation/](https://www.geeksforgeeks.org/chi-square-test-for-feature-selection-mathematical-explanation/)
...
对于每个特征，创建一个包含 `m` 行和 `k` 列的`应急表`。每个单元格 `(i, j)` 表示属性特征为 `i`、类别标签为 `k` 的行数。因此，该表中的每个单元格表示观察到的频率。要计算每个像元的预期频率，首先计算要素值在总数据集中的比例，然后将其乘以当前类别标签的总数。
