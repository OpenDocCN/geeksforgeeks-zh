...
### (b) 命名以下功能所属的模块: 1
a. `uniform()` b. `sqrt()` a. [`random()`](https://www.geeksforgeeks.org/random-numbers-in-python/) b. `math()`
...
```
def __init__(self):
    self.mystr = _________ #接受一个字符串
    self.mylist = _________ #将 mystr 转换为 list
```
编写代码以在从堆栈中移除元素时显示。
```
def disp(self):
...
## d. 编写一个生成器函数 `generatesq()`，显示从 100 到 n 的数字的平方根，其中 n 作为参数传递。
```py
import math
def generatesq(n):
...
### 二、用在文件 `mydata` 中写入 `"作业成本"` 的语句填充空白 1。
`file.write("作业成本")`
