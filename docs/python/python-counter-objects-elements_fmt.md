# Python Counter对象的elements()方法

> 原文：[https://www.geeksforgeeks.org/python-counter-objects-elements/](https://www.geeksforgeeks.org/python-counter-objects-elements/)

`Counter`类是Python3中`collections`模块提供的一种特殊类型的对象数据集。`collections`模块为用户提供了专门的容器数据类型，从而为Python的通用内置功能（如字典、列表和元组）提供了一种替代方案。

`Counter`是一个子类，用于计数可散列的对象。它在被调用时隐式地创建一个可迭代对象的哈希表。

`elements()`是`Counter`类的函数之一，在`Counter`对象上调用时会返回`Counter`对象中所有已知元素的迭代器。

> **参数：** 不带任何参数
> **返回类型：** 返回`Counter`对象中所有计数为正的元素的迭代器
> **错误和异常：**
> - 直接打印时会打印垃圾值，因为它返回的是迭代器对象，而不是特定的数据容器。
> - 如果一个项目的计数已经在`Counter`对象中初始化，那么它将忽略零值和负值的项目。

## 代码示例1：在简单的数据容器上处理`elements()`
...
## 代码示例2：具有不同数据容器的各种`Counter`对象上的`elements()`
...
## 代码示例4：当`Counter`中的项目计数初始化为负值或零时
...
**注意：** 我们可以从输出中推断出值小于1的项被`elements()`省略。

## 应用

`Counter`对象及其功能共同用于处理海量数据。我们可以看到`Counter()`为使用它调用的数据容器创建了一个哈希映射，这比手工处理元素非常有用。它是一个非常高效的处理和功能工具之一，甚至可以处理大范围的数据。