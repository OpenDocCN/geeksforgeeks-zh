...
> 原文：[https://www.geeksforgeeks.org/python-common-row-elements-summation/](https://www.geeksforgeeks.org/python-common-row-elements-summation/)
...
结合上述功能，只需一行即可完成该特定任务。这个 `reduce()` 功能可以用来操作&的功能，对所有的列表进行操作。`set()` 函数可用于将列表转换为集合，以消除重复。执行求和的任务是使用 `sum()` 完成的。
