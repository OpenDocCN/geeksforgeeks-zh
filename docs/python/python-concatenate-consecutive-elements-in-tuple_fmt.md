...
> 原文: [https://www.geeksforgeeks.org/python-concatenate-consecutive-elements-in-tuple/](https://www.geeksforgeeks.org/python-concatenate-consecutive-elements-in-tuple/)

有时，在处理数据时，我们可能会遇到需要寻找累积结果的问题。这可以是任何类型、乘积或总和。这里我们将讨论相邻元素的连接。让我们讨论执行这项任务的某些方法。

方法#1: 使用 `zip()` + 生成器表达式 + `tuple()`
上述功能的组合可用于执行该任务。在本文中，我们使用生成器表达式来提供串联逻辑，同时的元素配对由 `zip()` 完成。使用 `tuple()` 将结果转换为元组形式。
