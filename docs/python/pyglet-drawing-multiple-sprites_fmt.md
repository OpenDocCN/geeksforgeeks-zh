...
> 原文: [https://www.geeksforgeeks.org/pyglet-drawing-multiple-sprites/](https://www.geeksforgeeks.org/pyglet-drawing-multiple-sprites/)

## 简介

在本文中，我们将看到如何在 python 的 PYGLET 模块中在窗口上绘制多个精灵。Pyglet 是一个易于使用但功能强大的库，用于开发视觉丰富的图形用户界面应用程序，如游戏、多媒体等。

窗口是占用操作系统资源的“重量级”对象。窗口可能显示为浮动区域，或者可以设置为充满整个屏幕（全屏）。精灵是屏幕上显示的图像的实例。多个精灵可以在屏幕的不同位置显示相同的图像。精灵也可以放大或缩小，以任何角度旋转，并以部分不透明度绘制。图像是借助 pyglet 的图像模块加载的。
