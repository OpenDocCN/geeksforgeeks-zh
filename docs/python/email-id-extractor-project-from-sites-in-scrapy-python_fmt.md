...
> 原文: [https://www.geeksforgeeks.org/email-id-extractor-project-from-sites-in-scrapy-python/](https://www.geeksforgeeks.org/email-id-extractor-project-from-sites-in-scrapy-python/)
...
**要从 geeksforgeeks 网站抓取的电子邮件 id–** [‘反馈@ geeksforgeks . org’，‘类@ geeksforgeks . org’，‘投诉@ geeksforgeks . org’，‘投稿@ geeksforgeks . org’]
...
### 4. 下载 chrome 驱动程序
现在为你的 chrome 下载 chrome 驱动程序，并把它放在 chrome `scrapy.cfg` 文件附近。下载 chrome 驱动请参考本网站–[下载 chrome 驱动](https://chromedriver.chromium.org/downloads)。
...
### 2. 创建 `start_requests` 功能从 selenium 打到站点
您可以添加自己的网址。
...
for link in Finallinks: 
    if ('Contact' in link or 'contact' in link or 'About' in link or 'about' in link or 
        'CONTACT' in link or 'ABOUT' in link): 
        links.append(link) 
```
...
通过 `response.text` 我们获得了请求的网址的所有源代码。正则表达式 `'\w+@\w+\.{1}\w+'` 这里使用的可以翻译为类似这样的内容:查找每一段以一个或多个字母开头，后跟一个 at 符号(“@”)的字符串，后跟一个或多个以点结尾的字母。
...
**`parsed` 函数的解释:**
上面的正则表达式也导致了像 `select@1.13` 这样的垃圾值在这个来自 geeksforgeeks 的刮擦邮件 id 中，我们知道 `select@1.13` 不是邮件 id。已解析的函数筛选器应用仅接受包含“`.com`”和“`.in`”的电子邮件的筛选器。
