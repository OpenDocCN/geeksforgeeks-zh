...
创建一个带有属性名称、持续时间、价格、评级的课程。该类的一个实例被创建，现在我们使用`delattr()`方法删除`rating`属性。最后，我们检查`rating`属性是否存在。`try`块用于处理密钥错误。
...
创建一个带有属性名称、持续时间、价格、评级的课程。创建了类的一个实例，现在我们使用`delattr()`方法删除`price`属性。最后，我们检查`price`属性是否存在。`try`块用于处理密钥错误。
