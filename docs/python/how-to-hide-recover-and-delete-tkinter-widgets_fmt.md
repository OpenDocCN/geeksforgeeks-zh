...
> 原文: [https://www.geeksforgeeks.org/how-to-hide-recover-and-delete-tkinter-widgets/](https://www.geeksforgeeks.org/how-to-hide-recover-and-delete-tkinter-widgets/)

`Tkinter` 是一个用于创建 GUI（图形用户界面）应用程序的 Python 包。`Tkinter` 为我们提供了各种常见的图形用户界面元素，我们可以使用它们来构建界面，例如按钮、标签、框架、消息、菜单、以及各种输入字段和显示区域。我们称这些元素为小部件。

`小部件` 是图形用户界面的一个元素，它说明信息，以便用户可以与操作系统交互。在 Tkinter 中，Widgets 是对象；表示按钮、框架等的类的实例。

在本文中，我们将演示如何通过使用小部件的各种元素（如按钮、标签、框架等）来隐藏、恢复和删除 Tkinter 小部件。

我们可以通过调用 `pack_forget()` 方法来隐藏 Tkinter 小部件，使小部件不可见。我们需要再次调用 `pack()` 方法来打包小部件，使其可见，或者恢复它。我们还可以通过调用 `destroy()` 方法来永久删除 Tkinter 小部件。

## 示例 1: 使用 `pack_forget()` 和 `pack()` 方法隐藏和恢复 Tkinter 中的按钮。
...
## 示例 2: 使用 `pack_forget()` 和 `pack()` 方法隐藏和恢复 Tkinter 中的标签。
...
## 示例 3: 通过调用 `destroy()` 方法永久删除 Tkinter 小部件。

我们可以在任何可用的小部件以及主 tkinter 窗口中使用这个方法。
