...
> 原文：[https://www.geeksforgeeks.org/create-find-and-replace-features-in-tkinter-text-widget/](https://www.geeksforgeeks.org/create-find-and-replace-features-in-tkinter-text-widget/)
...
*   `Label()`：它添加任何事物的陈述或名称或标签。
*   `Entry()`：增加对话框输入文本。
*   `Entry实例.pack()`：它在指定的位置包装入口箱。
*   `Button()`：将带有命令和标签的按钮放置在指定位置。
*   `Text()`：它放置一个文本框来编写和添加内容。
*   `Text实例.insert()`：它在指定的索引处添加文本（这里的索引是字符串类型的）。
*   `Text实例.tag_add()`：标签用于同时引用所有内容，例如，如果一个段落包含文本“this”五次，为了同时对它们进行更改，我们使用标签。`tag_add` 将所有这些文本添加到由我们提供名称的特定标签下。
*   `Text实例.tag_config()`：用于配置高亮、字体、背景色、前景色。
*   `Text实例.tag_remove()`：用于删除从起始索引到结束索引使用的所有文本。
*   `Entry实例.get()`：用于访问在输入对话框中输入的文本。
*   `Text实例.search()`：用于在整个编辑器中搜索特定文本，从开始索引到结束，参数为 `nocase`，如果设置为 1，则表示搜索时不考虑大小写。
*   `Button实例.config()`：用于配置按钮，即我们可以单独添加命令或按钮被按下时发生的变化。
*   `Tk实例.mainloop()`：用于确保文本小部件保持打开状态。
