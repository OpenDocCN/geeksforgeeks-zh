...
> 原文：[https://www.geeksforgeeks.org/python-check-if-a-list-is-contained-in-another-list/](https://www.geeksforgeeks.org/python-check-if-a-list-is-contained-in-another-list/)

给定两个列表`A`和`B`，编写一个Python程序来检查列表`A`是否包含在列表`B`中，而不会破坏列表`A`的顺序。
...
## 进场#3：使用`join`和`map`模块

这里我们使用`join`将两个列表连接到字符串，然后使用运算符`in`中的检查列表`A`是否包含在`B`中。
