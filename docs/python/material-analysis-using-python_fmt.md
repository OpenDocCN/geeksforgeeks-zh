...
> 原文：[https://www.geeksforgeeks.org/material-analysis-using-python/](https://www.geeksforgeeks.org/material-analysis-using-python/)
...
**例如：** 如果你有一个数据库，比如说，锰（Mn）化合物和它们的磁性行为。分析相同的数据可以使用机器学习方法预测未知磁性化合物的磁性（其性质仍然是新的）。

## 描述符

在字典中，它被描述为用来描述或识别某物的单词或表达方式。

描述符用于描述计算算法的复合。属性元素的许多表示可以转换成向量和矩阵的数学格式（像使用独热向量编码来描述元素的电子配置），以将它们作为输入传递给机器学习算法。
...
2.  各种输入/输出格式，如 CIF， Gaussian， XYZ， VASP。
3.  电子结构分析，如态密度和能带结构。
4.  强大的分析工具。
...
使用 Pymatgen 库的 `Element` 类获取元素的细节（如原子质量、熔点）。将元素符号作为参数传递给 `Element` 类。
...
Pymatgen 有许多库，根据它们所代表的属性进行分组/分离。这里，`pymatgen` 创建一个简单的立方点阵，然后获取它的结构。如果没有文件名，将返回一个字符串。否则，输出将被写入文件。
...
API 密钥生成参见此网站：[https://materialsproject.org/open](https://materialsproject.org/open)

这里，首先使用应用编程接口键创建对象，然后查询特定任务标识的属性数据（任务标识可以被认为是材料项目数据库中每个元素的唯一标识）。

注意——属性名称在 `properties` 参数下提及。如果不存在这样的特定属性数据，则接收该特定属性的空对象。
...
# fetching details of a compound related to TaskId=mp-1010
...
其次，获取铁（Fe）化合物的所有定义的性质数据（元素和化合物）。
