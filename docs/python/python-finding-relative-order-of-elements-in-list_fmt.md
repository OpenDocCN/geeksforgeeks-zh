...
> 原文：[https://www.geeksforgeeks.org/python-finding-relative-order-of-elements-in-list/](https://www.geeksforgeeks.org/python-finding-relative-order-of-elements-in-list/)
...
以上所有功能可以组合起来完成这个特定的任务。`sorted()` 函数返回排序顺序，索引由 `index()` 函数完成。列表理解的任务是对整个列表元素进行操作，并整合这两项任务。
