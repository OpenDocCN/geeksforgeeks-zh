...
参数:
`exception_type`: 表示异常类别。
`exception_value`: 表示异常类型。像被零除错误、浮点错误，这些都是算术异常的类型。
`exception_traceback`: traceback 是包含解决异常所需的所有信息的报告。
...
```py
# Python program to demonstrate __exit__ method
