...
> 原文：[https://www.geeksforgeeks.org/cnn-introduction-to-pooling-layer/](https://www.geeksforgeeks.org/cnn-introduction-to-pooling-layer/)

池化操作包括在特征图的每个通道上滑动二维过滤器，并汇总过滤器覆盖区域内的特征。
对于尺寸为 `n_h x n_w x n_c` 的特征图，池化层后得到的输出尺寸为：
...
*   池化层汇总了由卷积层生成的特征图区域中存在的特征。因此，对概括的特征而不是由卷积层生成的精确定位的特征执行进一步的操作。这使得该模型对输入图像中特征位置的变化更加稳健。

## 池化层类型

### 最大池化

1.  最大池化是一种池化操作，从过滤器覆盖的特征图区域中选择最大元素。因此，最大池化层后的输出将是包含先前特征图中最显著特征的特征图。

![](img/4ec540512a199d869faab34c66024d69.png)

2.  这可以使用 Keras 中的 `MaxPooling2D` 层来实现，如下所示：
...
1.  平均池化计算过滤器覆盖的特征图区域中存在的元素的平均值。因此，虽然最大池化提供了特征图中某个特定斑块的最显著特征，但平均池化提供了斑块中存在的特征的平均值。

![](img/c38e22c1e219aef5c76ce57b288c581b.png)

2.  **代码#2：使用 Keras 执行平均池化**
...
1.  全局池化将特征图中的每个通道减少到一个值。因此，一个 `n_h x n_w x n_c` 特征图被简化为 `1 x 1 x n_c` 特征图。这相当于使用尺寸 `n_h x n_w` 的过滤器，即特征图的尺寸。
