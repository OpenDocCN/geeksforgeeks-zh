...
> 原文:[https://www . geeksforgeks . org/input-and-sys-stdin-readline 之差/](https://www.geeksforgeeks.org/difference-between-input-and-sys-stdin-readline/)
...
`Stdin`代表标准输入，即程序从中读取输入数据的流。此方法与`input()`方法略有不同，因为它还读取用户输入的转义字符。此外，该方法还提供了`size`参数，即一次可以读取多少个字符。
...
| `input()` | `sys.stdin.readline()`方法 |
| --- | --- |
| `input()`接受用户的输入，但不读取转义字符。 | `readline()`也接受用户的输入，但也读取转义字符。 |
