...
> 原文：[https://www.geeksforgeeks.org/how-to-close-a-tkinter-window-with-a-button/](https://www.geeksforgeeks.org/how-to-close-a-tkinter-window-with-a-button/)

**先决条件：** `Tkinter`

Python 的 Tkinter 模块提供了 `Button` 函数，可以在 Tkinter 窗口中创建一个按钮，一旦点击该按钮，就可以执行任何任务。可以在 `Button()` 功能的 `command` 参数中分配任务。下面给出了实现这一点的各种方法。
...
*   创建一个名为 `root` 的主窗口。
*   添加一个按钮。
*   将 `root.destroy` 分配给该按钮的 `command` 属性。

### 示例：直接在 `command` 属性中使用 `destroy()`
...
*   创建一个名为 `root` 的主窗口。
*   添加一个按钮。
*   将 `root.quit` 分配给该按钮的 `command` 属性。
