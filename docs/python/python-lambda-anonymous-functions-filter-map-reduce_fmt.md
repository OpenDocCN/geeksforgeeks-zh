...
> 原文: [https://www.geeksforgeeks.org/python-lambda-anonymous-functions-filter-map-reduce/](https://www.geeksforgeeks.org/python-lambda-anonymous-functions-filter-map-reduce/)
...
Python 中的 `reduce()`函数接受一个函数和一个列表作为参数。使用 lambda 函数和 iterable 调用该函数，并返回新的简化结果。这在可迭代的对上执行重复操作。`reduce()`功能属于 `functools` 模块。
