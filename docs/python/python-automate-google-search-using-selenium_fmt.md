...
> 原文：[https://www.geeksforgeeks.org/python-automate-google-search-using-selenium/](https://www.geeksforgeeks.org/python-automate-google-search-using-selenium/)

只需 2 分钟，使用 Python 脚本即可实现谷歌搜索自动化。这可以使用 `selenium`（浏览器自动化工具）来完成。Selenium 是一个用于测试 web 应用程序的可移植框架。它可以自动执行您需要手动执行的相同交互，这是它的一个小例子。掌握 Selenium 将帮助您自动化日常任务，例如控制您的推文、Whatsapp 短信，甚至只是谷歌搜索，而无需实际打开浏览器，只需 15-30 行 python 代码。自动化的极限是无限的。
