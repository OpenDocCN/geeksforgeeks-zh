...
> 原文：[https://www.geeksforgeeks.org/how-to-close-a-window-in-tkinter/](https://www.geeksforgeeks.org/how-to-close-a-window-in-tkinter/)
...
要关闭一个 `tkinter` 窗口，我们可以使用 `destroy()` 方法。`destroy()` 是一个通用的小部件方法，也就是说，我们可以在任何可用的小部件以及主 `tkinter` 窗口中使用这个方法。
...
在上面的例子中，点击按钮 `destroy()` 方法被调用并且 `tkinter` 窗口被关闭。