...
> 原文: [https://www.geeksforgeeks.org/how-to-convert-signed-to-unsigned-integer-in-python/](https://www.geeksforgeeks.org/how-to-convert-signed-to-unsigned-integer-in-python/)
...
## 示例 1: 将 `2^32` (或 `1 << 32`) 添加到有符号整数中，将其转换为无符号整数
...
例如，`x << y` 将带有 `y` 整数的整数 `x` 左移 `y` 位。这与 `x` 乘以 `2` 再乘以 `y` 的幂(`2**y`)是一样的。
