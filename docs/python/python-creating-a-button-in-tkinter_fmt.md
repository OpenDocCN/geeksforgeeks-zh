...
> 原文: [https://www.geeksforgeeks.org/python-creating-a-button-in-tkinter/](https://www.geeksforgeeks.org/python-creating-a-button-in-tkinter/)

Tkinter 是 Python 的标准 GUI（图形用户界面）包。它是 Python 本身附带的图形用户界面应用程序最常用的包之一。让我们看看如何使用 Tkinter 创建一个按钮。

**按照以下步骤操作：**

1.  导入 Python 2.x 中的 `tkinter` module（注：大写 T）
2.  创建主窗口（`root = Tk()`）
3.  添加任意多的小部件。
...
**注意：** 请参见两个代码的输出，因为 `tkinter.ttk` 不支持边框，所以第二个输出中不存在边框。此外，当您将鼠标悬停在两个按钮上时，`ttk.Button` 将改变其颜色并变成浅蓝色（效果可能因操作系统而异），因为它支持现代图形，而对于简单的 `Button`，它不会改变颜色，因为它不支持现代图形。