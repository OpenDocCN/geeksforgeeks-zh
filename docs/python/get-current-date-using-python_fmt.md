...
> 原文: [https://www.geeksforgeeks.org/get-current-date-using-python/](https://www.geeksforgeeks.org/get-current-date-using-python/)
...
`date.today()`方法属于`datetime`模块下的`date`类，返回一个包含今日日期值的日期对象。
...
Python 库定义了一个函数，主要用于获取当前时间和日期。`now()`函数返回当前的本地日期和时间，该函数定义在`datetime`模块下。
