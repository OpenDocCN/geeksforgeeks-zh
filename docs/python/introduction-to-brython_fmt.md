...
`Brython` 代表浏览器的 Python。它是运行在浏览器中的 Python3 的实现。相信 `Brython` 的目标是取代 javascript 成为浏览器的脚本语言。大多数现代浏览器都支持 `Brython`，甚至智能手机也支持 `Brython`。

## 主要特性

`Brython` 支持 Python3 的大部分语法，包括理解、生成器、元类、导入等。以及 CPython 发行版的许多模块。
...
要使用 `Brython`，只需:
1.  加载 `brython.js` 脚本。
2.  在页面加载时运行 `brython()` 函数。
3.  在 `<script type="text/python">` 标签中编写 Python 代码。
...
我们可以从列表列表中构建一个表。在这个例子中，我们将只显示 Python 脚本，周围的 HTML 代码将保持与上一个例子相同。
