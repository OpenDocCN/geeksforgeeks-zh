...
> 原文: [https://www.geeksforgeeks.org/creating-files-in-hdfs-using-python-snakebite/](https://www.geeksforgeeks.org/creating-files-in-hdfs-using-python-snakebite/)
...
使用 `mkdir()` 方法使用蛇咬包在 HDFS 创建目录。

## 步骤 1

在您的本地目录中创建一个文件，名称为 `create_directory.py` 在所需的位置。
...
在 `create_directory.py` python 文件中编写下面的代码。
...
运行 `create_directory.py` 文件，观察结果。
