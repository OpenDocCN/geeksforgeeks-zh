...
> 原文: [https://www.geeksforgeeks.org/functional-programming-in-python/](https://www.geeksforgeeks.org/functional-programming-in-python/)
...
**注意:** 更多信息请参考 Python 中的 [第一类函数。](https://www.geeksforgeeks.org/first-class-functions-python/)

**内置高阶功能**

为了使列表和迭代器等可迭代对象的处理更加容易，Python 实现了一些常用的高阶函数。这些函数返回一个节省空间的迭代器。一些内置的高阶函数是:

*   **`map()`:** `map()` 函数在将给定函数应用于给定可迭代对象（列表、元组等）的每个项后，返回结果列表。

    > **语法:** `map(fun, iter)`
    >
    > **参数:**
    > `fun`: 这是一个函数，给定可迭代的每个元素都会传递到这个函数。
    > `iter`: 这是一个要映射的 `iter`。
    >
    > **返回类型:** 返回映射类的迭代器。
    ...
    **注意:** 更多信息请参考 [Python `map()` 函数](http://geeksforgeeks.org/python-map-function/)

*   **`filter()`:** `filter()` 方法使用一个函数来测试序列中的每个元素是否为真，从而过滤给定的序列。

    > **语法:** `filter(fun, sequence)`
    >
    > **参数:**
    > `fun`: 测试序列中每个元素是否为真的函数。
    > `sequence`: 需要过滤的序列，可以是集合、列表、元组，也可以是任意迭代器的容器。
    >
    > **返回类型:** 返回已经过滤的迭代器。
    ...
    **注意:** 更多信息请参考 Python 中的 [`filter()`](https://www.geeksforgeeks.org/filter-in-python/)

*   **Lambda 函数:** 在 Python 中，匿名函数意味着一个函数没有名称。正如我们已经知道的，`def` 关键字用于定义普通函数，而 `lambda` 关键字用于创建匿名函数。
...
Traceback (most recent call last):
  File "/home/ee8bf8d8f560b97c7ec0ef080a077879.py", line 10, in <module>
    immutable[1] = 'K'
