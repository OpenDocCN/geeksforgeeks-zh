...
*   **浮点**–该值由 `float` 类表示。它是一个用浮点表示的实数。它由小数点指定。可选地，字符 E 或 E 后跟一个正整数或负整数可以被附加以指定科学符号。
*   **复数**–复数用 `complex` 类表示。指定为*(实部)+(虚部)j* 。例如–2+3j
...
**注意**–要了解更多关于字符串的知识，请参考[Python 字符串](https://www.geeksforgeeks.org/python-strings/)。
