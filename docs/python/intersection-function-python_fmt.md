...
`Python` 交集()`intersection()`函数返回一个新的集合，该集合包含所有集合共有的元素
...
## Python 交集()`intersection()`语法:

> `set1.intersection(set2, set3, set4...)`
> 在参数中，可以给出任意数量的集合

## Python 交集()`intersection()`返回值:

> 函数的作用是:返回一个集合，集合中的所有集合(`set1`，`set2`，`set3`...)都与`set1`相交。只有在没有传递参数的情况下，它才会返回`set1`的副本。

## Python 交集()`intersection()`示例

### 示例 1:集合交集的工作()`intersection()`
