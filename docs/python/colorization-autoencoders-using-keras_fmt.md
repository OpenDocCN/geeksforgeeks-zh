...
> 原文：[https://www.geeksforgeeks.org/colorization-autoencoders-using-keras/](https://www.geeksforgeeks.org/colorization-autoencoders-using-keras/)
...
在自动编码器的设计过程中，正确选择潜在尺寸至关重要。如果潜在维度大于输入维度，自动编码器倾向于记忆输入。我们将使用 CNNs 实现编码器部分，并将使用 CNNs 转置用于自动编码器的解码器部分。

为了简单起见，我们将使用**CIFAR100 数据集**，它在 [Keras 数据集](https://keras.io/datasets/)中 readily available。该数据集包含形状为**32 * 32 * 3**的**50k 彩色**图像用于训练，以及相同形状的**10k 彩色**图像用于测试目的。
...
对于着色任务，输入是灰度图像。灰度图像只有一个通道，而彩色图像有三个通道，即红、绿、蓝。我们使用来自 Keras 库的 `Input` 来输入形状为 `(rows, cols, 1)` 的数据。
...
自动编码器的解码器部分尝试解压缩潜在矢量，以便输入。在我们的例子中，解码器的输入是一层形状为 `(None, 256)`。它遵循三个反卷积层的堆叠，每个层中的过滤器数量递减。我们确保最后一层，在这种情况下，应该是形状 `(None, 32, 32, 3)`。通道的数量应该是 3，以便在反向传播期间将图像的重建与地面真实情况进行比较。
...
我们现在训练自动编码器模型，将整个数据分割成批次（`batch_size`），用于 `30` 个时代。这里需要注意的重要一点是，如果我们检查 `fit` 函数，我们会发现，模型的输入是灰度图像数据集，相应的彩色图像用作标签。验证集也发生了类似的事情。
