...
> 原文: [https://www.geeksforgeeks.org/count-words-in-a-given-string/](https://www.geeksforgeeks.org/count-words-in-a-given-string/)

给定一个字符串，计算其中的字数。单词由以下字符分隔：空格（`' '`）或新行（`'\n'`）或制表符（`'\t'`）或这些字符的组合。

这个问题可以有很多解决办法。下面是一个简单有趣的解决方案。想法是维持两种状态：`IN` 和 `OUT`。状态 `OUT` 表示看到一个分隔符。状态 `IN` 表示看到一个单词字符。当前一个状态为 `OUT`，下一个字符为单词字符时，我们增加单词计数。
