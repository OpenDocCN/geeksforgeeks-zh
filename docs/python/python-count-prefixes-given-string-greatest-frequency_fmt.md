...
> 原文: [https://www.geeksforgeeks.org/python-count-prefixes-given-string-greatest-frequency/](https://www.geeksforgeeks.org/python-count-prefixes-given-string-greatest-frequency/)
...
取一个空字符串，存储所有形成的前缀的字符串值。然后检查字母的频率是否比第二个字母高。如果没有发现这样的情况，那么结果将是 `0` 前缀。
下面是实现:
