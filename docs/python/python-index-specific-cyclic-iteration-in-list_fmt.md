...
> 原文: [https://www.geeksforgeeks.org/python-index-specific-cyclic-iteration-in-list/](https://www.geeksforgeeks.org/python-index-specific-cyclic-iteration-in-list/)
...
`itertools` 库内置了一些函数，可以帮助解决这个特定的问题。`cycle()` 函数执行循环部分，`dropwhile()` 函数使循环从列表开始，`islice()` 函数指定循环大小。
