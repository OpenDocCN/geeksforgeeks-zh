...
在本文中，我们将学习如何使用 python 中的`altair`制作带有中线的直方图。这篇文章也是牛郎星图形语法的一个很好的例子。

`altair`是 python 中最新的交互式数据可视化库之一。《牛郎星》基于`vega`和`vegalite`——一种交互式图形语法。这里我们将使用`import`关键字使用`altair`库来使用它。
...
现在，我们将生成数据，用中线制作直方图。这里，我们将使用`Numpy`从正态分布生成随机数，并创建`pandas`数据帧。
...
基本上在这里我们倾向于用中线建立条形图，它可以产生两层双星图像对象，并将它们组合起来。利用牛郎星的`Chart`功能，我们创建了数据的数据帧的基本图。
...
使用`altair`库中的`mark_rule()`函数，我们再次使用带有数据的基础对象来创建中线。
...
在带有中线的基本直方图中，`altair`库使用默认参数绘制直方图。就像，牛郎星为我们选择了蓝色作为直方图和箱数。同样，牛郎星选择黑色作为中线。但是我们可以使用牛郎星轻松定制带有中线的直方图。

首先，我们将增加直方图中的箱数，然后将中线的颜色更改为红色。

我们将在坐标轴参数内使用`maxbins=100`参数，形成包含 100 个 bin 的直方图。然后我们可以使用`mark_rule()`功能内的`color='red'`将中线的颜色改为红色。最后，让我们结合直方图和中线对象。然后我们将在`altair`改进带有中线的直方图版本。
