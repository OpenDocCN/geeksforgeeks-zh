...
> 原文：[https://www.geeksforgeeks.org/convert-mp3-to-wav-using-python/](https://www.geeksforgeeks.org/convert-mp3-to-wav-using-python/)
...
所以首先让我们安装 `pydub`。这是一个音频处理模块。Python 提供了一个名为 `pydub` 的模块来处理音频文件。`pydub` 是一个只可以使用的 Python 库。wav 文件。
...
`pydub` 模块使用 `ffmpeg` 或 `avconf` 程序进行实际转换。所以你必须安装 `ffmpeg` 才能使这个工作。但是如果其他不需要 `pydub` 的话，可以直接使用内置的 `subprocess` 模块调用类似 `ffmpeg` 的转换器程序，如下图方法所示。

## 方法 2

将 mp3 文件转换为 wav 文件是一个简单的两行脚本或代码。

这里不需要 `pydub` 模块，可以使用内置的 `subprocess` 模块调用转换器程序 `ffmpeg`，如下图：
