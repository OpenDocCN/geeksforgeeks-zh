...
> 原文: [https://www.geeksforgeeks.org/complex-demodulation-phase-and-amplitude-plot/](https://www.geeksforgeeks.org/complex-demodulation-phase-and-amplitude-plot/)
...
其中，`α`为振幅，`φ`为相移，`ω`为主频。复杂解调图的目标是改善频率估计。
...
上述方程对良好的初始值敏感。频率`ω`的初始值可以通过频谱图获得。复杂的解调相位图用于评估该估计是否足够。如果估计不充分，那么是否应该增加或减少。
...
其中，`a_i`是用标准最小二乘法拟合的某种类型的线性模型。最常见的情况是线性拟合，即模型变成如下:
