...
> 原文：[https://www.geeksforgeeks.org/how-to-define-an-auto-increment-primary-key-in-postgresql-using-python/](https://www.geeksforgeeks.org/how-to-define-an-auto-increment-primary-key-in-postgresql-using-python/)
...
必须用 `SERIAL` 定义一列作为主键。这里 `SERIAL` 不是真正的数据类型，只是一个简写符号，告诉 Postgres 为指定的列创建一个自动递增的唯一标识符。通过简单地将一个列设置为 `SERIAL` 并附加 `PRIMARY KEY`，Postgres 将处理所有复杂的幕后工作，并为每个 `INSERT` 用唯一的主键值自动递增指定的列。
...
在雇员表中，名为 `EMPLOYEE_ID` 的列将被实现为自动递增的主键列。
...
![使用pgAdmin创建的EMPLOYEE表结构](img/8d938820af7f8a8a22bd4ac3091b1e52.png)

现在，需要插入数据来验证自动增量功能是否工作。这可以直接通过 pgadmin 或使用 Python 代码来完成。

**pgadmin 方式：**

下面是显示插入查询和结果集执行情况的屏幕截图。

![通过pgAdmin手动插入数据并查看结果](img/07759bff13752dc40263c3c5d213b553.png)
...
![通过Python代码插入数据后的EMPLOYEE表结果](img/4276bab81cc4a494362d0b933d1c91ce.png)