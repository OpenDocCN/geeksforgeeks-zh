...
> 原文: [https://www.geeksforgeeks.org/gmail-login-using-python-selenium/](https://www.geeksforgeeks.org/gmail-login-using-python-selenium/)
...
*   使用 `selenium` 模块提供的 `find_element_by_xpath()` 函数，我们可以找到所需的元素（用户名框、密码框、下一步按钮）。
*   使用 `selenium` 模块提供的 `send_keys()` 函数，向当前元素输入数据。
*   使用 `selenium` 模块提供的 `click()` 函数，点击当前元素。
*   `get()` 属性允许我们打开一个网页。
...
**注意：** 这里 `driver` 是你为 `webdriver.Chrome()` 选择的变量名称。
...
出于安全考虑，我们也可以输入密码而不在屏幕上显示。为此，我们必须增加一个名为 `getpass` 的模块。现在只需对密码的输入语句进行一次更改，我们就可以输入密码，而无需在屏幕上显示。
