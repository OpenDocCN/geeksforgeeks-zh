...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-lists-to-xml-in-python/](https://www.geeksforgeeks.org/how-to-convert-lists-to-xml-in-python/)

在本文中，任务是在 Python 中将给定的列表转换为 XML。但是首先，让我们讨论什么是 XML？

XML 也可以是定义一组规则的术语，用于在人类可读和机器可读的格式中编码文档。XML 的设计目标专注于简单性、通用性和在网络上的实用性。它是一种文本格式，通过 Unicode 为各种人类语言提供了强大的支持。

更多信息可以参考文章[使用 Python 创建 XML 文档](https://www.geeksforgeeks.org/create-xml-documents-using-python/)。
...
XML 文档有称为元素的部分，由开始和结束标记定义。标签是以 `<` 开头并以 `>` 结尾的标记结构。
