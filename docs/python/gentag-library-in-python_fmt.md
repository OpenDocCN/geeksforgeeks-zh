...
Python 中的 `gentag` 库提供了一种标记 Python 对象的有效方法。这些任意的 Python 对象一旦被分配了 Python 标签，就可以基于这些标签进一步挖掘。在本文中，重点是使用 Python 标签的赋值、操作和操作。
...
需要使用 `gentag` 库的 `Scope()` 类来分配标签对象。可以使用 `define()` 方法分配每个标签，该方法接受对象名称和要分配给它的标签列表作为参数。
...
可以使用 `evaluate()` 对标记执行简单的集合操作，如 `&`（and），`|`（or），`-`（差分）和对称 difference（`^`）。所有标签的交集可以使用 `'all'` 来执行。
...
可以使用 `define()` 将每个新标签初始化为一组标签操作的组合。只需将标记操作作为第二个参数传递。
