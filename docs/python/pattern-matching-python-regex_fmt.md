...
> 原文: [https://www.geeksforgeeks.org/pattern-matching-python-regex/](https://www.geeksforgeeks.org/pattern-matching-python-regex/)
...
*   正则表达式可以更加复杂。例如，在模式后添加花括号中的 3 (`{3}`) 就像是说“匹配此模式三次”。所以稍微短一些的正则表达式是：
...
假设您想将区号与电话号码的其余部分分开。添加括号将在正则表达式中创建分组: `(\d\d\d)-(\d\d\d-\d\d\d\d)`。然后您可以使用 `group()` 匹配对象方法仅从一个分组中抓取匹配的文本。
...
如果您想一次检索所有分组，请使用 `groups()` 方法——注意名称使用复数形式。
...
`mo.groups()` 将返回一个包含多个值的元组，您可以使用多重赋值技巧将每个值分配给一个单独的变量，如下面的 `areaCode, mainNumber = mo.groups()` 行。
...
括号在正则表达式中具有特殊含义，但如果您需要在文本中匹配括号该怎么办。例如，也许您尝试匹配的电话号码的区号设置在括号中。在这种情况下，您需要用反斜杠转义 `(` 和 `)` 字符。在交互式 shell 中输入以下内容:
...
这里，`(哈){3}`匹配“哈哈哈”但不匹配“哈”。由于它与“哈”不匹配，`search()`返回“无”。

## 可选匹配问号

有时有一种模式，你想匹配只是可选的。也就是说，正则表达式应该找到一个匹配，不管那一段文本是否存在。`?`字符将它前面的组标记为模式的可选部分。例如，在交互式外壳中输入以下内容：
...
正则表达式`Bat(wo)+man`不会匹配字符串“蝙蝠侠历险记”，因为加号至少需要一个`wo`。
