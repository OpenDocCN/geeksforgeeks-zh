...
元组 `res` 包含两个定义游戏分辨率的值。然后，我们需要定义另一个变量 `screen`，它实际上将充当我们的工作台。这可以通过使用 `pygame.display.set_mode((arg，arg))`来完成。元组 `(arg，arg)`可以存储到变量 `res` 中，以减少处理器负载。
...
def game(lead_y, lead_X, speed, count):
    while True:
        ...
        if keys[pygame.K_UP]:
            # if up key is pressed then the players y pos will decrement by 10
            lead_y -= 10 
        ...
        if e_p[0] > 0 and e_p[0] <= width:
            # if the enemy block's X coordinate is between 0 and the width of the screen the X value gets decremented by 10
            e_p[0] -= 10 
        else:
