...
> 原文: [https://www.geeksforgeeks.org/how-to-use-python-pexpect-to-automate-linux-commands/](https://www.geeksforgeeks.org/how-to-use-python-pexpect-to-automate-linux-commands/)
...
可以调用 `pexpect.run()` 方法来执行命令并返回其输出。这个功能可以用来替代 `os.system`。
...
`spawn` 类是启动一个新的子进程并控制它的主要接口。`spawn` 中的字符串可以被需要执行的 shell 命令替换。

> **语法:** `pexpect.spawn("rm ./dev")`

`pexpect.spawn` 类的重要方法是 `expect()`。

> **语法:** `expect(pattern, timeout=-1, searchwindowsize=-1, async_=False)`

此方法等待子进程返回给定的字符串。在 `expect` 方法中指定的模式将在整个字符串中匹配。`timeout` 用于引发 `pexpect.Timeout`。`searchwindowsize` 参数用于设置类的 `maxread` 属性。创建非阻塞应用程序时，设置 `async_ = True`。
...
*   导入 `pexpect`。
*   用 `pexpect.spawn('ftp ftp.us.debian.org')` 生成一个子进程。
*   FTP 客户端要求用户名。将模式与 `expect('Name .*: ')` 匹配。
