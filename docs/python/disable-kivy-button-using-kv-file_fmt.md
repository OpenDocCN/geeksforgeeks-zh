...
> 原文: [https://www.geeksforgeeks.org/disable-kivy-button-using-kv-file/](https://www.geeksforgeeks.org/disable-kivy-button-using-kv-file/)

在本文中，我们将学习如何在 Kivy 中使用 .kv 文件**禁用按钮**。有些地方我们需要禁用按钮，让我们看看怎么做。

> [Kivy 教程——用例子学习 Kivy。](https://www.geeksforgeeks.org/kivy-tutorial/)

**按钮**是一个带有相关动作的标签，当按钮被按下（或在点击/触摸后被释放）时会被触发。我们可以在按钮后面添加功能，并为按钮设置样式。但是要禁用该按钮，我们有一个属性名：`disabled`，它必须为 `True`。

该属性将有助于禁用按钮，即按钮将会存在，但没有用，因为它被禁用，按钮的任何功能都将不起作用。

在本文中，我们使用了 [`RelativeLayout`](https://www.geeksforgeeks.org/python-relative-layout-in-kivy-using-kv-file/) 来设置工作和禁用按钮的相对位置。

> **注意:** `disabled` 属性在 1.8.0 版本中引入。如果你想使用它，你需要更新你的框架。
