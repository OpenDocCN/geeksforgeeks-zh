...
在本文中，我们将看到如何在 `pafy` 中获取给定 YouTube 视频流的 URL。`Pafy` 是一个 Python 库，用于下载 YouTube 内容和检索元数据。`Pafy` 对象是包含给定视频所有信息的对象。流基本上是可用的分辨率的视频是可以在 YouTube 上获得的。URL 代表统一资源定位符，用于指定万维网上的地址。URL 是连接到网络的任何资源（例如，超文本页面、图像和声音文件）的基本网络标识。该协议规定了如何传输来自链路的信息。
