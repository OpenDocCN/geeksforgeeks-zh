...
> 原文: [https://www.geeksforgeeks.org/python-distance-between-collections-of-inputs/](https://www.geeksforgeeks.org/python-distance-between-collections-of-inputs/)

`scipy.stats.cdist(array，axis=0)` 函数计算两个输入集合的每对之间的距离。

### 参数:
`数组`: 输入数组或具有计算两个输入集合的每对之间的距离的元素的对象。
`轴`: 要沿其计算的轴。默认情况下，`axis= 0`
