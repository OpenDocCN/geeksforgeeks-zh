...
我们可以通过使用 `psycopg2` PostgreSQL 驱动程序返回类似字典的值，可以使用也可以不使用 `extras` 模块，本文将讨论这两种情况。`extras` 模块为 `Psycopg2` 提供了各种各样的好东西。它保存助手函数和类，直到找到更好的分布位置。

## 不使用 `psycopg2.extras` 模块
...
## 使用 `psycopg2.extras` 模块

`RealDictCursor` 是一种使用字典作为行的基本类型的游标。此游标不允许普通索引获取数据。只能借助表的列名键来获取数据。
