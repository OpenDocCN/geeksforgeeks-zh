...
我们也可以使用`QRCode`类来创建一个二维码并更改其详细信息。它采用以下参数:

*   **版本:**该参数为 1-40 的整数，控制二维码的大小(最小的版本 1 是 21×21 矩阵)。
*   **纠错:**该参数控制二维码使用的纠错。有以下四个常量可供使用:
    *   `qrcode.constants.ERROR_RESERVE_L`:大约 7%或更少的错误可以被纠正。
    *   `qrcode.constants.ERROR_CORRECT_M`(默认) **:** 大约 15%或更少的错误可以被纠正。
    *   `qrcode.constants.ERROR_CORRECT_Q`:大约 25%或更少的错误可以被纠正。
    *   `qrcode.constants.ERROR_RESERVE_H`:大约 30%或更少的错误可以被纠正。
...
*   `make_image()`: 此方法用于将二维码对象转换为图像文件。它采用 `fill_color` 和 `back_color` 可选参数来设置前景和背景颜色。
