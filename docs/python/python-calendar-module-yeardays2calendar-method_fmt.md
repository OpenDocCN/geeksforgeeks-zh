...
> 原文：[https://www.geeksforgeeks.org/python-calendar-module-yeardays2calendar-method/](https://www.geeksforgeeks.org/python-calendar-module-yeardays2calendar-method/)

日历模块允许像程序一样输出日历，并提供与日历相关的附加有用功能。日历模块中定义的函数和类使用理想化的日历，当前的公历在两个方向上无限延长。
`yeardays2calendar()` Python 中的方法用于获取指定年份的数据。周列表中的条目是天数和工作日数的元组。本月以外的天数为零。
