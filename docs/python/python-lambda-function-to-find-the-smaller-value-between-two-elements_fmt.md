...
> 原文: [https://www.geeksforgeeks.org/python-lambda-function-to-find-the-smaller-value-between-two-elements/](https://www.geeksforgeeks.org/python-lambda-function-to-find-the-smaller-value-between-two-elements/)
...
两个 `lambda` 函数将存储在一个元组中，这样第一个元素是 `b`，第二个元素是 `b`，如果 `[a<b]` 是 `true` 它返回 `1` 并打印索引为 `1` 的元素，否则如果 `[a<b]` 是 `false` 它返回 `0`，所以打印索引为 `0` 的元素。