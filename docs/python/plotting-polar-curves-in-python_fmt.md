...
> 原文: [https://www.geeksforgeeks.org/plotting-polar-curves-in-python/](https://www.geeksforgeeks.org/plotting-polar-curves-in-python/)
...
> **语法**: `matplotlib.pyplot.polar(theta, r, **kwargs)`
>
> **参数**:
>
> *   `θ`–角度
...
*   对于每个弧度值`θ`，根据每个曲线的特定公式计算相应的 `r` 值。

### 1. 圆

圆是由平面上距给定点(中心)给定距离(半径)的所有点组成的形状。因此，`r`是等于`半径`的`常数`值。
...
椭圆是一个点在一个平面内移动的轨迹，使得它与另外两个点(焦点)的距离之和是恒定的。这里，`r`定义为:
...
心形是一个圆围绕另一个相同的圆滚动时，该圆圆周上一点的轨迹。这里，r 定义为:
...
[![](img/7e51d08424517e1561800c64a35b4938.png)](https://media.geeksforgeeks.org/wp-content/uploads/20200612114623/Screenshot20200612at125317AM-300x274.png)

### 4. 阿基米德螺线

阿基米德螺线是一个点在一条直线上匀速运动的轨迹，该点本身也在围绕它的一个端点匀速转动。这里，r 定义为:
...
一条红藻或玫瑰曲线是在极坐标中绘制的玫瑰形正弦曲线。这里，r 定义为:
