...
> 原文: [https://www.geeksforgeeks.org/python-float-type-and-its-methods/](https://www.geeksforgeeks.org/python-float-type-and-its-methods/)

Python 中的 `float` 类型代表浮点数。浮点用于表示实数，用小数点来划分整数和小数部分。例如，`97.98`、`32.3+e18`、`-32.54e100` 都是浮点数。

Python 浮点值表示为 64 位双精度值。任何浮点数的最大值大约为 `1.8 x 10^308`。任何大于这个数的数字都会用 Python 中的字符串 `inf` 来表示。
...
`float.as_integer_ratio()`: 返回一对整数，其比率正好等于实际的具有正分母的 float。在无穷小的情况下，它会引发溢出错误和值错误。
...
`float.hex()`: 以十六进制字符串形式返回浮点数的表示形式。
