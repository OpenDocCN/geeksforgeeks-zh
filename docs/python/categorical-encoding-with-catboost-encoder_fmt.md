...
> 原文：[https://www.geeksforgeeks.org/categorical-encoding-with-catboost-encoder/](https://www.geeksforgeeks.org/categorical-encoding-with-catboost-encoder/)
...
![\frac{TargetSum+prior}{FeatureCount+1}](img/e99527f9ba9c4250628b64fd461e1d97.png "Rendered by QuickLaTeX.com")

**目标计数**：特定分类特征的目标值之和（直到当前特征）。

**prior**：是由（整个数据集中目标值之和）/（数据集中观测值（即行）总数）确定的常数值。
...
例如，如果我们有带值的分类要素列 `color=["红色"、"蓝色"、"蓝色"、"绿色"、"红色"、"红色"、"黑色"、"黑色"、"蓝色"、"绿色"]` 和带有值的目标列，`target=[1、2、3、2、3、1、4、4、2、3]`

那么，prior 将是 `25/10 = 2.5`
...
*   `cols`：待编码特征（列）列表。默认情况下，它是 `None`，表示具有 `object` 数据类型的所有列都将被编码。
*   `drop_invariant`：`True` 表示零方差的 drop 列（每行相同的值）。默认情况下为 `False`。
*   `return_df`：`True` 从转换中返回 pandas 数据帧，`False` 将返回 numpy 数组。默认情况下为 `True`。
*   `handle_missing`：处理缺失（未填充）值的方式。`error` 生成缺失值错误， `return_nan` 返回 `nan`，`value` 返回目标平均值。默认为 `value`。
*   `handle_unknown`：处理未知（未定义）值的方式。选项与 `handle_missing` 参数相同。
*   `sigma`：用于减少过拟合。训练数据加上正态分布噪声，而测试数据保持不变。Sigma 是正态分布的标准偏差。
...
编码器在 `category_encoders` 库中作为 `CatBoostEncoder` 提供。该编码器的工作原理类似于带 `fit_transform()`，`.fit()` 和 `.transform()` 方法的 scikit-learn 变换器。
...
![Catboost 编码后的数据集(train_cbe)](img/0d69c3a9adff0a153e59c959974bab5d.png)