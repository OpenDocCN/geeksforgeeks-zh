...
> 原文：[https://www.geeksforgeeks.org/how-to-read-numbers-in-csv-files-in-python/](https://www.geeksforgeeks.org/how-to-read-numbers-in-csv-files-in-python/)
...
2.  导入 `csv` 库。
3.  创建一个嵌套列表 `marks`，以表格形式存储学生的数学和 Python 卷号及其标记。
4.  在 `writer` 对象的 `'w'` 模式下打开一个新的 CSV 文件（或一个现有的 CSV 文件）和其他必要的参数（此处为分隔符和引号）。
5.  借助 `writerows` 方法将列表 `marks` 写入其中。
6.  为了读取行，使用 `reader` 对象，并将每一行（也是一个列表）存储在一个新的列表 `output` 中。
7.  打印列表 `output` 以验证代码。

## 读取 CSV 文件中不带引号的数字

为了写入我们的 CSV 文件 `'my_csv'`，我们使用 `writer` 对象的 `writerows()` 方法。但是要按原样读取数字，我们将使用 `writer` 对象的一个可选参数，即 `quoting`。`quoting` 参数告诉作者要引用哪个字符。

如果 `quoting` 设置为 `csv.QUOTE_NONNUMERIC`，那么 `writerow()` 将引用所有包含文本数据的字段，并将所有数值字段转换为浮点数据类型。
...
如果 `quoting` 设置为 `csv.QUOTE_ALL`，那么 `writerow()` 将引用所有字段，数字现在将存储在引号中。为了读取每一行的数字，我们使用 CSV 库中的 `reader` 对象，并将所有行存储在一个列表 `output` 中，之后我们还会打印该列表。
