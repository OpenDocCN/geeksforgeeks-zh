...
> 原文: [https://www.geeksforgeeks.org/python-check-order-specific-data-type-in-tuple/](https://www.geeksforgeeks.org/python-check-order-specific-data-type-in-tuple/)
...
该任务可以使用上述功能的组合来执行。在这种情况下，我们只需要使用 `isinstance()` 测试数据类型，并检查我们使用的链式 `if` 语句的每个元组元素。
