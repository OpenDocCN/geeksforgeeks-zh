...
> 原文: [https://www.geeksforgeeks.org/python-check-k-consecutive-1s-binary-number/](https://www.geeksforgeeks.org/python-check-k-consecutive-1s-binary-number/)
...
方法：用 `k` 个 1 创建一个新字符串。使用 `if` 条件检查 `s` 中是否有这个新字符串。在 Python 中，如果 `new` 在 `s` 中，检查 `new` 的存在，因此如果有，则返回 `true`，否则返回 `false`。
