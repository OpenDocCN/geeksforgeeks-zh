...
> 原文：[https://www.geeksforgeeks.org/how-to-plot-the-coherence-between-two-signals-in-python/](https://www.geeksforgeeks.org/how-to-plot-the-coherence-between-two-signals-in-python/)
...
> **1) x, y：** 是数据的序列。
> **2) Fs：** 为标量参数，默认值为 2。
> **3) window：** 该参数以数据段为自变量，返回该段的窗口版本。其默认值为 `window_hanning()`。
> **4) sides：** 此参数指定要返回光谱的哪些边。这可以有以下值：`'default'`、`'onesided'` 和 `'twosided'`。
> **5) pad_to：** 此参数包含数据段填充到的整数值。
> **6) Fc：** 此参数还包含偏移绘图 x 范围的整数值，以反映频率范围。其默认值为 0。
> **7) NFFT：** 该参数包含用于快速傅立叶变换的每个块中的数据点数。
> **8) detrend：** 此参数包含 fft 前应用于每个分段的函数，旨在移除均值或线性趋势 `{'none', 'mean', 'linear'}`。
> **9) scale_by_freq：** 该参数允许对返回的频率值进行积分。
> **10) noverlap：** 此参数为块间重叠的点数。
> **11) Fc：** 此参数为 x 的中心频率。
>
> **返回：** 这个方法返回如下：
> **1) Cxy：** 这个返回相干向量。
> **2) freqs：** 返回 `Cxy` 中元素的频率。
>
> **结果为 `(Cxy, freqs)`**
...
# a variable say 'cor' using the following code
