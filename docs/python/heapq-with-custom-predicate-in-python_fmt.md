...
> 原文: [https://www.geeksforgeeks.org/heapq-with-custom-predicate-in-python/](https://www.geeksforgeeks.org/heapq-with-custom-predicate-in-python/)

**先决条件:** [堆队列或 heapq 模块](https://www.geeksforgeeks.org/heap-queue-or-heapq-in-python/)
...
考虑一种情况，一个类的对象必须在最小堆中维护。例如，让我们考虑一个具有属性的类，如 `name`、`des`（职位）、`yos`（服务年限）、`salary`。该类的对象必须基于 `yos`（服务年限）在最小堆中维护。
