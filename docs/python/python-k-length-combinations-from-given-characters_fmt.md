...
> 原文: [https://www.geeksforgeeks.org/python-k-length-combinations-from-given-characters/](https://www.geeksforgeeks.org/python-k-length-combinations-from-given-characters/)
...
## 方法#1: 使用 `itertools.product()` + `join()` + `map()`

该任务可以由 `product()` 使用 `repeat` 参数来执行，但返回的结果是单个字符的元组列表。这些都可以使用 `join()` 和 `map()` 进行连接。
