...
文本小部件为用户提供多行文本区域。文本小部件实例是在 `Text` 类的帮助下创建的。它还用于显示文本行，并允许编辑文本。
...
该示例的上述输出允许用户多行输入文本。但是它没有显示用户输入的超出文本小部件高度的所有文本，即 `height=3`。由于它只显示 3 行文本，因此对这种多行文本使用滚动条可以解决这个问题。
