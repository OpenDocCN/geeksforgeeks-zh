...
`else` 与循环一起使用，可以配合 `while` 和 `for` 循环。`else` 块在循环结束时执行，意味着当给定的循环条件为假时，则执行 `else` 块。让我们看看下面 `while` 循环和 `for` 循环的例子。
...
*   `i+=1` 增量 `i`，因为我们不想无限次地执行 `while` 循环。
*   打印 `i` 的值
...
在下面的示例中，`else` 语句只有在数组中没有元素是偶数的情况下才会被执行，也就是说，如果 `if` 语句没有被执行任何迭代。因此，对于数组 `[1, 9, 8]`，`if` 在循环的第三次迭代中执行，因此 `for` 循环之后出现的 `else` 被忽略。在数组 `[1, 3, 5]` 的情况下，`if` 不在任何迭代中执行，因此在循环执行后执行 `else`。
