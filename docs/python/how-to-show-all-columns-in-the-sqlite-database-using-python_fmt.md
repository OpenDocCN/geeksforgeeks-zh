...
> 原文：`https://www.geeksforgeeks.org/how-to-show-all-columns-in-the-sqlite-database-using-python/`
...
*   使用 `connect()` 方法连接到数据库。
*   创建一个 `cursor` 对象，并使用创建的光标对象执行查询，以便创建一个表并将值插入其中。
*   使用光标对象的 `description` 关键字获取列名。`description` 关键字只指定二维元组中表的列，该元组不包含任何值，只包含列名。
