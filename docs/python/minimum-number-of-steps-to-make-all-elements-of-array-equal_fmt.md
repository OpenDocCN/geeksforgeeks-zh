...
> 原文: [https://www.geeksforgeeks.org/minimum-number-of-steps-to-make-all-elements-of-array-equal/](https://www.geeksforgeeks.org/minimum-number-of-steps-to-make-all-elements-of-array-equal/)
...
> **索引 2:** `10 – (1 * 5) = 5`，采取的步骤= `5`
> **索引 3:** `5` 不变，采取的步骤= `0`
> **索引 4:** `15 – (5 * 2) = 5`，采取的步骤= `2`
...
*   运行 `while` 循环，直到数组的最小值大于 `-1`。
    ...
    *   将步数增加 `1`。
    *   检查所有数组元素是否相等，如果是，则将标志标记为真，并中断 `while` 循环。
    *   否则，重复上述步骤来计算所需的总步骤数。
*   最后，检查标志是否为真，如果是，则数组是可转换的。

**举例说明:**
给定数组 `A[] = {5, 7, 10, 5, 15}`，`B[] = {2, 2, 1, 3, 5}`
...
*   **时间复杂度:** `O(N^2 logN)` 和上面的方法一样，有两个循环来转换数组。外环取 `O(N)`，内环取 `O(NlogN)`，就像我们在内环中使用 `set` 一样。
*   **辅助空间:** `O(N)`。我们正在使用一个占用 `O(N)` 个额外空间的 `set`。