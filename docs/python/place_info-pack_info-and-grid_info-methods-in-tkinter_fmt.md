# 将`_info()`、`pack_info()`和`grid_info()`方法放入Tinter

> 原文：[https://www.geeksforgeeks.org/place_info-pack_info-and-grid_info-methods-in-tkinter/](https://www.geeksforgeeks.org/place_info-pack_info-and-grid_info-methods-in-tkinter/)

要获取小部件几何管理选项的所有信息，可以使用`tkinter`中相应的`place_info()`、`pack_info()`和`grid_info()`方法。
...
该方法用于获取关于小部件的几何管理的信息，如行号、列号、行跨度、列跨度、填充值等。
