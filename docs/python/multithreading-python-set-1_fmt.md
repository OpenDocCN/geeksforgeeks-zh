...
在 Python 中，`threading` 模块为在一个程序中产生多个线程提供了一个非常简单直观的 API。

让我们考虑一个使用 `threading` 模块的简单例子:
...
*   要创建一个新线程，我们创建一个 `Thread` 类的对象。它接受以下参数:
    *   `target`: 线程要执行的功能
    *   `args`: 传递给目标函数的参数
...
*   要开始一个线程，我们使用 `Thread` 类的 `start()` 方法。
...
*   一旦线程开始，当前程序（你可以把它看作是一个主线程）也会继续执行。为了停止当前程序的执行直到一个线程完成，我们使用 `join()` 方法。
...
*   我们使用 `threading.main_thread()` 函数来获取主线程对象。在正常情况下，主线程是启动 Python 解释器的线程。`name` 线程对象的属性用于获取线程的名称。

```py
print("Main thread name: {}".format(threading.main_thread().name))
```

*   我们使用 `threading.current_thread()` 函数获取当前线程对象。
