...
然而，如果这些多行注释直接放在函数或类签名之后，那么这些注释就变成了文档字符串。Docstring 是 Python 的内置特性，用于将已经编写的文档与 Python 模块、函数、类和方法相关联。它被添加到函数、模块或类的正下方，以描述它们的功能。在 Python 中，文档字符串通过`__doc__`属性变得可用。
