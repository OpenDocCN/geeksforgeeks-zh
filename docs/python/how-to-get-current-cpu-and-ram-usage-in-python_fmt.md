...
> 原文: [https://www.geeksforgeeks.org/how-to-get-current-cpu-and-ram-usage-in-python/](https://www.geeksforgeeks.org/how-to-get-current-cpu-and-ram-usage-in-python/)
...
函数 `psutil.virtual_memory()` 返回一个关于系统内存使用情况的命名元组。元组中的第三个字段代表内存的使用百分比。按 `(total - available) / total * 100` 计算。

函数输出中的字段为:

*   **Total**: Total memory not including swap.
*   **available**: available memory of the process
*   **percentage**: percentage of used memory.
*   **used**: used memory
*   **Free memory**: unused memory available at any time.
