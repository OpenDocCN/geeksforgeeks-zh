...
> 原文：[https://www.geeksforgeeks.org/getting-saved-wifi-passwords-using-python/](https://www.geeksforgeeks.org/getting-saved-wifi-passwords-using-python/)

通常在连接 wifi 时，我们必须输入一些密码才能访问网络，但我们无法直接看到之前输入的密码，即保存的网络密码。在本文中，我们将看到如何使用 python 获取所有保存的 WiFi 名称和密码，为了做到这一点，我们将使用 Python 的 `subprocess` 模块。

`Wi-Fi` 是一种无线网络技术，允许计算机(笔记本电脑和台式机)、移动设备(智能手机和可穿戴设备)和其他设备(打印机和摄像机)等设备与互联网连接。

Python 中的 `subprocess` 模块(2.x 和 3.x)用于通过创建新流程，通过 Python 代码运行新的应用程序或程序。它还有助于获得输入/输出/错误管道以及各种命令的退出代码。

> **实施步骤：**
> 1. 导入 `subprocess` 模块
> 2. 借助 `check_output` 方法获取无线局域网(wifi)的元数据
> 3. 解码元数据，按照行拆分元数据
> 4. 从解码的元数据中获取保存的 wlan 网络的名称
> 5. 现在对于每个名称，根据名称再次获取 wlan 的元数据
> 6. 启动 `try` and `catch` block，在 `try` block 中，解码并分割该元数据，获得给定 wifi 名称的密码
> 7. 打印密码和 wifi 名称，如果没有密码则打印空白
> 8. 在 `except` 发生过程错误的块中，打印编码错误
