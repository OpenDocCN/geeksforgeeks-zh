...
> 原文: [https://www.geeksforgeeks.org/animated-data-visualization-using-plotly-express/](https://www.geeksforgeeks.org/animated-data-visualization-using-plotly-express/)
...
地图是一种地图，它使用公共属性的定义区域中的颜色差异，以便将数据可视化为一个区域(在本例中为一个国家)的汇总汇总。Plotly Express 可以轻松绘制合唱团。下面的代码展示了如何从 `gapminder` 获取信息。设置要给合唱团上色的参数。在这里，我们希望合唱团根据预期寿命来给各个地区着色。`hover_name` 显示悬停时的设定数据。`animation_frame` 是指动画应该在其上完成的参数(大部分情况下，这个参数是时间序列数据)。
