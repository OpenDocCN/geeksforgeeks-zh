...
> 原文：[https://www.geeksforgeeks.org/complement-naive-bayes-cnb-algorithm/](https://www.geeksforgeeks.org/complement-naive-bayes-cnb-algorithm/)
...
现在，让我们考虑一个例子：比方说，我们有两个类：`苹果`和`香蕉`，我们必须根据一定数量的单词出现的频率来分类给定的句子是与苹果相关还是与香蕉相关。下面是简单数据集的表格表示：
...
其中 `f_i` 是某个属性的出现频率。例如，某个单词在一个句子中出现的次数。
...
现在，从 6.302 < 85.333, the predicted class is `苹果`。
...
*   [scikit-learn文档](https://scikit-learn.org/stable/modules/naive_bayes.html#complement-naive-bayes)。