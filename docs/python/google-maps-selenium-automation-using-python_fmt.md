...
> 原文: [https://www.geeksforgeeks.org/google-maps-selenium-automation-using-python/](https://www.geeksforgeeks.org/google-maps-selenium-automation-using-python/)
...
## 步骤 3) 下一步声明一个功能在这个功能下你需要检查谷歌地图网站上的搜索栏。并将类名复制到变量 `Place`。您需要向特定的 web 元素发送密钥。给出你的目的地作为输入。在 `Submit` 变量中提供 `xpath` 值这用于按下搜索按钮
...
## 步骤 4) 声明一个叫做 `directions` 的函数。在此功能下，您需要点击方向按钮。从谷歌地图网站复制方向按钮的 X 路径值。并将该值粘贴到变量中。
