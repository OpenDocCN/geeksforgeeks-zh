...
> 原文: [https://www.geeksforgeeks.org/python-extract-string-till-all-occurrence-of-characters-from-other-string/](https://www.geeksforgeeks.org/python-extract-string-till-all-occurrence-of-characters-from-other-string/)

给定一个字符串，任务是编写一个 Python 程序来提取，直到找到其他字符串中的所有字符。

> **输入:** `test_str = "geeksforgeeks is best for all geeks"`, `check_str = "freak"`
> **输出:** geeksforgeeks is best for a
> **解释:** `a` 是 `freak` 中最后一个以字符串形式出现的字母。字符串会一直打印到第一次出现。
>
> **输入:** `test_str = "geeksforgeeks is best for all geeks"`, `check_str = "geeki"`
> **输出:** geeksforgeeks i
> **解释:** `i` 是 `geeki` 中最后一个以字符串形式出现的字母。字符串会一直打印到第一次出现 `i`。
