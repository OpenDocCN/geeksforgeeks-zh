...
如果这个命令成功运行，并且我们能够获得 `Python` 版本，那么我们就可以开始了。否则，我们必须在我们的系统中安装 `Python`，要做到这一点，请参考 [**如何在 Windows 上安装 Python？**](https://www.geeksforgeeks.org/how-to-install-python-on-windows/)

### 第二步:检查 PIP 安装

`PIP` 是一个用来安装 `python` 包的工具。`PIP` 是 `Python 2.7.9+` 和 `Python 3.4+` 自动安装的。打开命令提示符，输入如下所示的命令，检查是否安装了 `pip`。
