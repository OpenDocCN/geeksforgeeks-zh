...
> 原文: [https://www.geeksforgeeks.org/how-to-create-and-customize-venn-diagrams-in-python/](https://www.geeksforgeeks.org/how-to-create-and-customize-venn-diagrams-in-python/)
...
语句 `venn2(subsets= (30, 10, 5), set_labels=('A组', 'B组'))` 是指 `subsets` 的参数是 3 元素列表，其中数字 50，10，7 对应于 aB，AB，Ab。

> Ab = 包含在 A 组中，但不包含在 B 组中
> aB = 包含在 B 组，但不包含在 A 组
> AB = 包含在 A 组和 B 组中

`set_labels` 参数允许您在维恩图中标记您的两个组。`matplotlib` 库 `pyplot` 模块中的 `show()` 功能用于显示所有图形。
...
维恩图的默认颜色是红色和绿色。现在我们将使用 `set_colors` 参数自定义颜色橙色和蓝色。`alpha` 参数用于控制透明度。
