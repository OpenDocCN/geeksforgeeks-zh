...
> 原文: [https://www.geeksforgeeks.org/python-case-insensitive-string-replacement/](https://www.geeksforgeeks.org/python-case-insensitive-string-replacement/)

给定一串单词。任务是编写一个 Python 程序，用给定的字符串替换给定的单词，而不考虑大小写。

**示例:**

> **输入:** `test_str = "gfg is BeSt"`, `repl = "good"`, `subs = "best"`
> **输出:** `gfg is good`
> **解释:** `BeSt` 替换为 `"good"` 忽略大小写。
>
> **输入:** `test_str = "gfg is BeSt"`, `repl = "better"`, `subs = "best"`
> **输出:** `gfg is better`
> **解释:** `BeSt` 替换为 `"better"` 忽略大小写。

## 方法#1: 使用 `re.IGNORECASE` + `re.escape()` + `re.sub()`

在这种情况下，`re` 的 `sub()` 用于执行替换，`IGNORECASE` 标志用于忽略大小写并执行不区分大小写的替换。
...
使用特殊的忽略大小写正则表达式也可以解决这个问题。`lambda` 函数用于处理字符串中的转义字符。
