...
> 原文: [https://www.geeksforgeeks.org/permutation-of-first-n-positive-integers-such-that-prime-numbers-are-at-prime-indices/](https://www.geeksforgeeks.org/permutation-of-first-n-positive-integers-such-that-prime-numbers-are-at-prime-indices/)

## 问题描述

给定一个整数 `N`，任务是找到前 `N` 个正整数的排列数，使得素数位于素数索引处（对于基于 1 的索引）。

**注:** 既然排列数可能很大，返回答案模 `10^9 + 7`。
...
从 1 到 `N` 有 `K` 个素数，从索引 1 到 `N` 正好有 `K` 个素数索引，所以素数的排列数是 `K!`。同样，非质数的排列数是 `(N-K)!`。所以排列总数是 `K! * (N-K)!`。
