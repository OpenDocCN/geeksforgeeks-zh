...
> 原文：[https://www.geeksforgeeks.org/how-to-get-the-duration-of-audio-in-python/](https://www.geeksforgeeks.org/how-to-get-the-duration-of-audio-in-python/)

使用 Python 语言可以找到音频文件的持续时间，Python 语言的库使用丰富。使用一些库如 `mutagen`、`wave`、`audioread`、`scipy` 等。不仅仅限于提取音频文件的长度/持续时间，还提供了更多功能。
...
## 方法 1：使用 Python 库 `mutagen`

`mutagen` 是一个处理音频元数据的 Python 模块。它支持各种格式的音频文件，如 wavpack、mp3、Ogg 等。以下是使用它计算音频文件持续时间的步骤：

### 第一步：安装 `mutagen`

由于 `mutagen` 是一个外部 python 库，因此首先需要使用 `pip` 命令安装它，如下所示：
...
在我们的程序中，我们将使用 `mutagen` 库的一些内置功能，让我们稍微探索一下它们，以更好地理解源代码：
...
>
> **示例：** `voice_length = voice_info.length`
...
>
> **示例：** `with audioread.audio_open('sample.wav') as f:`
> # 声明 1... 声明 n
...
>
> **示例：** `variable1, variable2 = scipy.io.wavfile.read('Example.wav')`
