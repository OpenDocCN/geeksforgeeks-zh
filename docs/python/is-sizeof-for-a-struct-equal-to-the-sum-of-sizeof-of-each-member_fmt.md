...
> 原文：[https://www.geeksforgeeks.org/is-sizeof-for-a-struct-equal-to-the-sum-of-sizeof-of-each-member/](https://www.geeksforgeeks.org/is-sizeof-for-a-struct-equal-to-the-sum-of-sizeof-of-each-member/)

## 先决条件
[C 中的 sizeof 运算符](https://www.geeksforgeeks.org/sizeof-operator-c/)
...
在这种情况下，`y` (`short int`) 后跟 `x` (`int`)，因此在 `y` 后需要填充。在这种情况下，数据对齐不需要在末尾填充。

C 语言不允许编译器对结构成员重新排序以减少填充量。为了最小化填充量，结构成员必须按降序排序（类似于 Case 2）。