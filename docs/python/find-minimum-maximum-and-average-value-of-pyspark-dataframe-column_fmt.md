...
> 原文: [https://www.geeksforgeeks.org/find-minimum-maximum-and-average-value-of-pyspark-dataframe-column/](https://www.geeksforgeeks.org/find-minimum-maximum-and-average-value-of-pyspark-dataframe-column/)

在本文中，我们将在 PySpark 数据框中找到特定列的最大值、最小值和平均值。

为此，我们将使用 `agg()` 函数。此函数计算聚合并将结果作为数据帧返回。

> **语法:** `dataframe.agg({'column_name':'avg'/'max'/'min'})`
>
> 哪里，
>
> *   `dataframe` 是输入数据帧
