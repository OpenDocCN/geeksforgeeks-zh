...
> 原文：[https://www.geeksforgeeks.org/html-cleaning-and-entity-conversion-python/](https://www.geeksforgeeks.org/html-cleaning-and-entity-conversion-python/)

网页上最重要的也是最容易被忽视的任务就是文本的清理。每当人们想解析 HTML 时，总是避免嵌入 Javascript 和 CSS。用户只对网络服务器上的标签和文本感兴趣。

## `lxml` 安装

`lxml` 是 C 库 `libxslt` 和 `libxml2` 的 Python 绑定。所以维护一个 Python 库，它是非常快速的 HTML 解析和 XML 库。为了让它工作，还需要安装 C 库。链接 `http://lxml.de/installation.html` 将提供所有安装说明。
...
它是一个用于实体转换的 HTML 解析器库。它只是给定一个包含 HTML 实体的字符串，创建一个 `BeautifulSoup` 的实例。然后它检索字符串属性。
...
但是反过来是不可能的，也就是说，对于 `BeautifulSoup` 中的 `htmlentitydefs.name2codepoint` 字典中的对应值来替换它们。