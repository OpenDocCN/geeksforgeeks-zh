...
**16。`try`**: 这个关键字是用来异常处理的，是用来捕捉代码中使用关键字以外的错误。“尝试”块中的代码被检查，如果有任何类型的错误，除了块被执行。

**17。`except`**: 如上所述，这与“try”一起工作来捕捉异常。

**18。`raise`**: 也用于异常处理，明确引发异常。

**19。`finally`**: 不管“尝试”块的结果是什么，被称为“最后”的块总是被执行。详细文章–[Python 中的异常处理](https://www.geeksforgeeks.org/python-exception-handling/)

**20。`for`**: 这个关键字用于控制流量和循环。

**21。`while`**: 有一个类似“for”的工作方式，用于控制流程和 for 循环。

**22。`pass`**: 是 python 中的 null 语句。遇到这种情况时，什么也不会发生。这用于防止缩进错误，并用作占位符。详细文章–[传递](https://www.geeksforgeeks.org/loops-and-loop-control-statements-continue-break-and-pass-in-python/)

**23。`import`**: 此语句用于将特定模块包含到当前程序中。

**24。`from`**: 一般用于导入，from 用于从导入的模块导入特定功能。

**25。`as`**: 此关键字用于为导入的模块创建别名。即给导入的模块起一个新的名字。`import math as my_math`。
...
**29。`with`**: 这个关键字用来包装上下文管理器定义的方法内的代码块的执行。这个关键词在日常编程中使用不多。

**30。`in`**: 该关键字用于检查容器是否包含值。这个关键字也用于在容器中循环。
...
**32。`global`**: 这个关键字用来定义函数内部的一个变量是全局范围的。

**33。`nonlocal`**: 这个关键字的工作原理类似于全局，但不是全局，这个关键字声明一个变量指向外部封闭函数的变量，如果是嵌套函数。
