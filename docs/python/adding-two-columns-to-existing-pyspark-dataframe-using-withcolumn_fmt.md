...
> 原文: [https://www.geeksforgeeks.org/adding-two-columns-to-existing-pyspark-dataframe-using-withcolumn/](https://www.geeksforgeeks.org/adding-two-columns-to-existing-pyspark-dataframe-using-withcolumn/)

在本文中，我们将看到如何使用 `withColumn` 向现有的 PySpark 数据框添加两列。

`withColumn` 用于更改值、转换现有列的数据类型、创建新列等等。

> **语法:** `df.withColumn(colName, col)`
> **返回:** 一个新的 `DataFrame`，通过添加一列或替换同名的现有列。
...
这里我们将使用 [cricket_data_set_odi.csv 文件](https://media.geeksforgeeks.org/wp-content/cdn-uploads/20210628161151/Cricket_data_set_odi.csv)作为数据集，并从该文件创建数据框。
