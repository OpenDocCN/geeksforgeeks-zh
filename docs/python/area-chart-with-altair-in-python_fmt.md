...
> 原文: [https://www.geeksforgeeks.org/area-chart-with-altair-in-python/](https://www.geeksforgeeks.org/area-chart-with-altair-in-python/)
...
*   **区域颜色:** 通过设置 `mark_area()` 方法的 `color` 参数，可以改变区域颜色的默认颜色。
*   **不透明度:** 可以通过设置 `mark_area()` 方法的 `opacity` 参数来更改区域的默认不透明度。范围从 0 到 1。
*   **线条颜色:** 您也可以通过在 `mark_area()` 方法的 `line` 字典参数中指定 `color` 键的值来更改实际线条图的颜色。
