...
> 原文：[https://www.geeksforgeeks.org/mathematical-functions-in-python-set-4-special-functions-and-constants/](https://www.geeksforgeeks.org/mathematical-functions-in-python-set-4-special-functions-and-constants/)
...
## `pi`
这是一个内置常数，输出 pi（3.141592）的值。

## `e`
这是一个内置常数，输出 e（2.718281）的值。
...
## `inf`
这是一个正浮点无穷大常量。`-inf` 用于表示负浮点无穷大。这个常量在 python 3.5 及更高版本中定义。

## `isinf()`
该功能用于检查值是否为无穷大。

## `nan`
这个常量表示“不是数字”。这个常量在 python 3.5 及更高版本中定义。

## `isnan()`
如果数字为 `nan` 则此函数返回 `True`，否则返回 `False`。
