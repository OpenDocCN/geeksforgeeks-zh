...
> 原文：[https://www.geeksforgeeks.org/important-differences-between-python-2-x-and-python-3-x-with-examples/](https://www.geeksforgeeks.org/important-differences-between-python-2-x-and-python-3-x-with-examples/)
...
*   [外部参照](#xrange)
*   [错误处理](#error-handling)
*   [`__future__` 模块](#__future__-模块)

## 除法运算符

如果我们在 Python 2.x 中移植代码或执行 Python 3.x 代码，如果整数除法的更改没有被注意到（因为它不会引发任何错误），这将是危险的。在移植代码时，最好使用浮点值（如 `7.0/5` 或 `7/5.0`）来获得预期的结果。
...
Python 2.x 的 `xrange()` 在 Python 3.x 中不存在。在 Python 2.x 中，`range` 返回列表，即 `range(3)` 返回 `[0, 1, 2]`，而 `xrange` 返回 `xrange` 对象，即 `xrange(3)` 返回迭代器对象，其工作方式类似于 Java 迭代器，需要时会生成数字。
如果我们需要多次迭代同一个序列，我们更喜欢 `range()`，因为 `range` 提供了一个静态列表。`xrange()` 每次都会重构序列。`xrange()` 不支持切片和其他列表方法。`xrange()` 的优点是，当任务是在大范围内迭代时，它可以节省内存。
在 Python 3.x 中，`range` 函数现在执行的是 `xrange` 在 Python 2.x 中执行的操作，因此为了保持代码的可移植性，我们可能希望坚持使用 `range` 来代替。所以 Python 3.x 的 `range` 函数就是 Python 2.x 的 `xrange`。
...
如果我们计划在 2.x 代码中支持 Python 3.x，我们可以在代码中使用 `__future__` 导入。
...
关于 `__future__` 模块的更多详细信息，请参考本文中的[链接](https://docs.python.org/2/library/__future__.html)。
