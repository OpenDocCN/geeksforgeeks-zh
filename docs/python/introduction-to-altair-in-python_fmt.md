...
> 原文：[https://www.geeksforgeeks.org/introduction-to-altair-in-python/](https://www.geeksforgeeks.org/introduction-to-altair-in-python/)

`Altair`是 Python 中的统计可视化库。它本质上是声明性的，基于 `Vega` 和 `Vega-Lite` 可视化语法。它正迅速成为人们寻找快速有效的方法来可视化数据集的首选。如果你使用过像 `matplotlib` 这样的命令式可视化库，你将能够正确地理解 `Altair` 的能力。

它被正确地视为声明性可视化库，因为当可视化 `Altair` 中的任何数据集时，用户只需要指定数据列如何映射到编码通道，即声明数据列和编码通道（如 x 轴和 y 轴、行、列等）之间的链接。简单的框架，一个声明性的可视化库允许你专注于“什么”而不是“如何”部分，在没有用户帮助的情况下处理其他情节细节。
...
所有的 `Altair` 图都需要三个基本要素：数据、标记和编码。也可以通过仅指定数据和标记来制作有效的图表。

所有 `Altair` 图的基本格式是：

```py
alt.Chart(数据).mark_bar().encode(
    encoding1='column1',
    encoding2='column2',
)
```
...
数据集是传递给图表的第一个参数。`Altair` 中的数据是围绕 `pandas` 数据框架构建的，因此编码变得非常简单，它能够检测编码中所需的数据类型，但是您也可以使用以下数据：
...
`mark` 属性指定数据应如何在绘图上表示。在 `Altair` 中有许多类型的标记方法，其格式如下：

```py
mark_markname()
```

一些基本标记包括区域、条、点、文本、刻度和线条。`Altair` 还提供了一些复合标记，如方框图、误差带和误差条。这些标记方法也可以接受可选参数，如颜色和不透明度。

使用 `Altair` 的一个主要优点是，只需更改标记类型就可以更改图表类型。

### 编码

可视化中最重要的事情之一是将数据映射到图表的视觉属性。这种在 `Altair` 的映射称为编码，通过 `Chart.encode()` 方法实现。`Altair` 有各种类型的编码通道：位置通道、标记属性通道、超链接通道等。其中最常用的是位置通道的 `x`（x 轴值）和 `y`（y 轴值）以及标记属性通道的 `color` 和 `opacity`。
...
在本例中，我们将以散点图的形式可视化来自 `vega_datasets` 库的 `iris` 数据集。本例中散点图使用的标记方法是 `mark_point()`。对于这个双变量分析，我们将 `sepalLength` 列和 `petalLength` 列映射到 x 轴和 y 轴编码。此外，为了区分这些点，我们将 `shape` 编码映射到 `species` 列。
