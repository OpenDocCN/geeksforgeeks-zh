...
*   `version` : 是提到程序版本的可选参数。
*   `help` : 负责显示帮助信息。默认情况下为真。
...
以“<”开头，以“>”结尾的元素称为位置参数。它是位置敏感的。

### `--option` / `-o`

以“-”或“--”开头的元素称为长选项或短选项。可以称之为 `--option` 或 `-o`。
...
以“[”开头、以“]”结尾的元素称为可选参数。它被认为是可选的。

### `<argument>...`

当出现在左边的元素可以重复多次时，使用省略号“...”。

### `(required)`

以“(”开头、以“)”结尾的元素是必需元素。`(--option1 | --option2)` 表示需要 `--option1` 或 `--option2`。
