...
## 先决条件
使用 Python 中的 C 代码| [集 1](https://www.geeksforgeeks.org/using-c-codes-in-python-set-1/) 、[集 2](https://www.geeksforgeeks.org/using-c-codes-in-python-set-2/)

## 问题#1
如果使用 `ctypes` ，那么就会出现一个问题，原来的 C 代码可能会使用没有干净利落地映射到 Python 的语言。
...
## 第三期
在下面的代码中，创建了一个 `ctypes.c_int` 的实例，并作为指针对象传入。`c_int` 对象可以变异，不像正常的 Python 整数。要检索或更改该值，使用 `.value` 属性即可。
