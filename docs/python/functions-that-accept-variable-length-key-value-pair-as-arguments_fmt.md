...
> 原文: [https://www.geeksforgeeks.org/functions-that-accept-variable-length-key-value-pair-as-arguments/](https://www.geeksforgeeks.org/functions-that-accept-variable-length-key-value-pair-as-arguments/)

为了将变长键值对作为参数传递给函数，Python 提供了一个名为 ``**kwargs`` 的特性。`kwarg` 代表关键字参数。当人们想要处理函数中的命名参数时，它被证明是一个有效的解决方案。
...
**注意:** 在任何参数前添加 ``**`` 使其成为 `kwargs` 参数。它接受关键字作为参数。
