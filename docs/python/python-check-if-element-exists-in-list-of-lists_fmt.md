...
> 原文：[https://www.geeksforgeeks.org/python-check-if-element-exists-in-list-of-lists/](https://www.geeksforgeeks.org/python-check-if-element-exists-in-list-of-lists/)

给定一个列表列表，任务是确定给定元素是否存在于任何子列表中。下面给出了一些解决给定任务的方法。

## 方法#1：使用 `any()`

`any()` 方法在给定迭代器中出现特定元素时返回 `True`。
