...
> 原文: [https://www.geeksforgeeks.org/python-call-function-from-another-file/](https://www.geeksforgeeks.org/python-call-function-from-another-file/)
...
现在创建了另一个 Python 文件，它调用在 `test.py` 中定义的 `displayText()` 函数。
...
**示例 2:** 创建了一个 Python 文件 `calc.py`，其中包含 `addNumbers()`、`subtractNumbers()`、`multiplyNumbers()`、`divideNumbers()` 和 `modulusNumbers()`。
...
这两个文件都被导入到另一个名为 `file.py` 的 Python 文件中。
