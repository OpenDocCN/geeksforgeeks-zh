...
#### `Windows`
有很多解释器可以免费运行 Python 脚本，比如从 [http://python.org/](http://python.org/) 下载的 Python 软件附带的 `IDLE`（集成开发环境）。

#### `Linux`
Python 预装了流行的 Linux 发行版，比如 Ubuntu 和 Fedora。要检查您运行的是哪个版本的 python，请在终端模拟器中键入 `python`。解释器应该启动并打印版本号。

#### `macOS`
一般来说，`Python 2.7` 都是和 macOS 捆绑在一起的。您必须从 [http://python.org/](http://python.org/) 手动安装 `Python 3`。
...
**第 2 行：** `print("GeeksQuiz")` 要在控制台上打印东西，使用 `print()` 功能。这个函数还会在我们的消息打印出来后添加一个换行符（不像 C 语言）。请注意，在 Python 2 中，`print` 不是一个函数，而是一个关键字，因此可以不带括号使用。但是，在 Python 3 中，它是一个函数，必须用括号来调用。
