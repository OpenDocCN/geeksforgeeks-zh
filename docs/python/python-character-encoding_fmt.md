...
查找具有非标准字符编码的文本是文本处理中非常常见的步骤。理想情况下，所有文本都应该来自 `utf-8` 或 `ASCII` 编码，但情况可能并非总是如此。因此，在编码未知的情况下，必须检测这种未编码的文本，并将其转换为标准编码。因此，在进一步处理文本之前，这一步很重要。

**安装 `charade`:**
为了执行编码的检测和转换，需要 Python 库 `charade`。这个模块可以简单地使用 `sudo easy_install charade` 或 `pip install charade` 安装。

我们来看看 `charade` 模块周围的包装函数。

### 代码：`detect(string)`，检测编码
...
d1 = charade.detect('geek')
print ("d1 is encoded as : ", d1)

d2 = charade.detect('ascii')
...
`detect()`： 这是一个 `charade.detect()` 包装器。它对字符串进行编码，并处理 `UnicodeDecodeError` 异常。它需要一个 `bytes` 对象，因此在尝试检测编码之前对字符串进行编码。

`convert()`： 这是个包装器。它首先调用 `detect()` 来获取编码。然后，它返回一个解码的字符串。