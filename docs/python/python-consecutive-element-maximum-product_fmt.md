...
> 原文：[https://www.geeksforgeeks.org/python-consecutive-element-maximum-product/](https://www.geeksforgeeks.org/python-consecutive-element-maximum-product/)

有时，我们可能会遇到这样的问题，即我们需要从列表中获取 2 个数字的最大乘积，但有一个限制，即这些数字必须连续。这种类型的问题可能在竞争性编程时出现。让我们讨论一下解决这个问题的某些方法。

## 方法一：使用 `max()` + `zip()` + 列表理解

这个问题可以用上述三个函数的组合来解决，其中 `max` 函数可以用来获得最大值，`zip` 和列表理解完成了将逻辑扩展到整个列表的任务。
