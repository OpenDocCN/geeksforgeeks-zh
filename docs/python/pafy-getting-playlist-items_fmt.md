...
在本文中，我们将看到如何在 `pafy` 中获取 `youtube` 播放列表项目。`Pafy` 是一个 `python` 库，用于下载 `YouTube` 内容和检索元数据。`Pafy` 对象是包含给定视频所有信息的对象。`YouTube` 中的播放列表是按顺序播放的视频列表或组，一个视频接一个视频。当一个视频结束播放时，下一个视频会自动开始播放，因此您不必单击或搜索来开始播放新视频。播放列表项目是单个视频，总项目是播放列表中存在的总视频。
