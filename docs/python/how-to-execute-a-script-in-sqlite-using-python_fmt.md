...
> 原文: [https://www.geeksforgeeks.org/how-to-execute-a-script-in-sqlite-using-python/](https://www.geeksforgeeks.org/how-to-execute-a-script-in-sqlite-using-python/)
...
### 第一步:
首先我们需要导入 Python 中的 `sqlite3` 模块。

> `import sqlite3`

### 步骤 2:
通过创建数据库连接到数据库。我们可以通过简单地创建一个名为 `geeks_db.db` 的数据库来连接到数据库，或者我们可以通过使用 `:memory:` 在内存中简单地创建一个数据库。
