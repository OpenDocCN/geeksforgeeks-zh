...
在上面的代码中，我们首先导入之前安装的模块。然后我们将变量 `arr` 初始化为一个随机的 `numpy` 数组，范围直到 1000。因此，我们可以说这个数组由大量数据组成。接下来，我们以“只写”属性打开文件。这意味着如果没有任何名为 `test.hdf5` 的文件，它将创建一个，否则它将删除(覆盖)现有文件的内容。打开文件时，我们使用了 `h5py.File()` 而不是 `open()`，因为与 `open()` 方法相比，它占了上风。如果使用 `h5py.File()` 打开文件，我们不需要关闭文件。`create_dataset()` 将变量 `dset` 设置为之前创建的数组。
...
在这里，我们再次打开文件，但这一次我们通过“只读”属性打开它，因此不能对文件进行任何更改。我们将变量 `data` 设置为之前文件中存储的数据。我们来看看输出:

看起来好像没有什么新东西。它只是一个数组，我们打印出来的数字就像在一个数组中一样。但是，**变量 `data` 不是数组**。它实际上与数组非常不同。这是数据集。它不是将数据存储在内存中，而是将其保存在计算机的硬盘中，从而像目录一样维护一个层次结构:
...
我们创建了两个数据集，但整个过程与之前相同。使用“w”属性创建名为 `test_read.hdf5` 的文件，该文件包含两个随机数数据集(`array_1` 和 `array_2`)。现在假设我们只想读取 `array_2` 的选择部分。例如，我们想读取 `array_2` 中对应于 `array_1` 的值大于 `1` 的部分。如果我们使用传统的文本文件而不是 HDF5 文件，这几乎是不可能实现的。这正是我们看到的 HDF5 文件的力量:
