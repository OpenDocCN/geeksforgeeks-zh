...
*   所有系数 `B` 都设置为 0。
*   发现预测因子 `x_j` 与 `y` 最相关。
*   在与 `y` 最相关的方向增加系数 `B_j`，当你发现其他预测因子 `x_k` 的相关性等于或高于 `x_j` 时，停止。
*   向与 `x_j` 和 `x_k` 等角的方向延伸 (`B_j`, `B_k`)。
*   继续并重复，直到模型中包含所有预测因子。

## 在 Python3 中实现 LARS

对于本例，我们将使用波士顿住房数据集，该数据集具有波士顿马萨诸塞州地区的房屋中值。你可以在这里了解更多关于这个数据集[的信息](https://www.cs.toronto.edu/~delve/data/boston/bostonDetail.html#:~:text=The%20Boston%20Housing%20Dataset, the%20area%20of%20Boston%20Mass.)。
...
*   [scikit-learn 文档](https://scikit-learn.org/stable/modules/linear_model.html#least-angle-regression)
