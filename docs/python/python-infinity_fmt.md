...
由于无穷大可以是正的，也可以是负的，所以它们可以分别表示为 `float('inf')` 和 `float('-inf')`。下面的代码显示了上述内容的实现:
...
Python 的 `math` 模块也可以用来表示无限整数。下面的代码显示了它是如何实现的:
...
### 3. 使用 Python 的 `decimal` 模块

Python 的 `decimal` 模块也可以用来表示无限浮点值。
正值用 `Decimal('Infinity')`，负值用 `Decimal('-Infinity')`。
...
要检查一个给定的数是否是无限的，可以使用 `math` 库的 `isinf()` 方法，该方法返回一个布尔值。下面的代码显示了 `isinf()` 方法的使用:
