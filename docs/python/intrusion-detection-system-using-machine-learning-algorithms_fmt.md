...
[KDD 杯 1999 数据集](http://kdd.ics.uci.edu/databases/kddcup99/kddcup99.html)
...
| 功能名称 | 描述 | 类型 |
| --- | --- | --- |
| `duration` | 连接的长度(秒) | 连续的 |
| `protocol_type` | 协议的类型，例如 `tcp`、`udp` 等。 | 分离的 |
| `service` | 目的地的网络服务，例如 `http`、`telnet` 等。 | 分离的 |
| `src_bytes` | 从源到目标的数据字节数 | 连续的 |
| `dst_bytes` | 从目标到源的数据字节数 | 连续的 |
| `flag` | 连接的正常或错误状态 | 分离的 |
| `land` | 如果连接来自/去往同一主机/端口，则为 1；否则为 0 | 分离的 |
| `wrong_fragment` | “错误”片段的数量 | 连续的 |
| `urgent` | 紧急数据包数量 | 连续的 |
...
| --- | --- | --- |
| `hot` | “热门”指标的数量 | 连续的 |
| `num_failed_logins` | 失败的登录尝试次数 | 连续的 |
| `logged_in` | 1 如果成功登录；否则为 0 | 分离的 |
| `num_compromised` | “妥协”条件的数量 | 连续的 |
| `root_shell` | 如果获得根壳，则为 1；否则为 0 | 分离的 |
| `su_attempted` | 1 如果尝试了“su root”命令；否则为 0 | 分离的 |
| `num_root` | “根”访问次数 | 连续的 |
| `num_file_creations` | 文件创建操作的次数 | 连续的 |
| `num_shells` | 外壳提示的数量 | 连续的 |
| `num_access_files` | 对访问控制文件的操作数 | 连续的 |
| `num_outbound_cmds` | `ftp` 会话中出站命令的数量 | 连续的 |
| `is_host_login` | 1 如果登录属于“热门”列表；否则为 0 | 分离的 |
...
| 功能名称 | 描述 | 类型 |
| --- | --- | --- |
| `count` | 过去两秒内与当前连接到同一主机的连接数 | 连续的 |
|  | 注意:以下功能指的是这些同主机连接。 |  |
| `serror_rate` | 有“SYN”错误的连接百分比 | 连续的 |
| `rerror_rate` | 有“REJ”错误的连接百分比 | 连续的 |
| `same_srv_rate` | 同一服务的连接百分比 | 连续的 |
| `diff_srv_rate` | 不同服务的连接百分比 | 连续的 |
| `srv_count` | 过去两秒钟内与当前连接相同的服务的连接数 | 连续的 |
|  | 注意:以下功能指的是这些相同服务的连接。 |  |
| `srv_serror_rate` | 有“SYN”错误的连接百分比 | 连续的 |
| `srv_rerror_rate` | 有“REJ”错误的连接百分比 | 连续的 |
| `srv_diff_host_rate` | 不同主机的连接百分比 | 连续的 |

## 应用的各种算法

高斯朴素贝叶斯、决策树、随机森林、支持向量机、逻辑回归。

## 使用的方法

我在 KDD 数据集上应用了上面提到的各种分类算法，并比较这些结果来构建预测模型。
...
`logged_in`(如果成功登录，则为 1；否则为 0):我们注意到只有 70000 个数据包成功登录。
...
对特征应用特征映射，例如：`protocol_type` 和 `flag`。
...
以上对不同模型的分析表明，考虑到准确性和时间复杂性，决策树模型最适合我们的数据。

**实现链接:**
完整代码上传到我的 github 账号–[https://github.com/mudgalabhay/intrusion-detection-system](https://github.com/mudgalabhay/intrusion-detection-system)