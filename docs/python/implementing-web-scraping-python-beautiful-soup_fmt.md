...
> 原文：[https://www.geeksforgeeks.org/implementing-web-scraping-python-beautiful-soup/](https://www.geeksforgeeks.org/implementing-web-scraping-python-beautiful-soup/)
...
3.  现在，我们需要做的就是导航和搜索我们创建的解析树，即树遍历。对于这个任务，我们将使用另一个第三方 python 库，[Beautiful Soup](http://www.crummy.com/software/BeautifulSoup/)。它是一个 Python 库，用于从 HTML 和 XML 文件中提取数据。
...
*   首先导入 `requests` 库。
*   然后，指定你要刮的网页的网址。
*   将 HTTP 请求发送到指定的 URL，并将来自服务器的响应保存在名为 `r` 的响应对象中。
...
`BeautifulSoup` 库的一个非常好的地方是它建立在 HTML 解析库的基础上，比如 `html5lib`、`lxml`、`html.parser` 等等。所以 `BeautifulSoup` 对象和指定解析器库可以同时创建。
...
现在 `soup.prettify()` 已经打印出来了，给出了从原始 HTML 内容创建的解析树的可视化表示。
...
第一个参数是要搜索的 HTML 标记，第二个参数是字典类型元素，用于指定与该标记相关联的附加属性。`find()` 方法返回第一个匹配的元素。你可以试着打印 `table.prettify()` 来了解这段代码的作用。
...
我们创建了一个字典来保存报价的所有信息。嵌套结构可以使用点符号来访问。要访问 HTML 元素中的文本，我们使用 `.text`：
...
在这里，我们创建了一个名为 `inspirational_quotes.csv` 的 CSV 文件，并保存其中的所有报价以备将来使用。
