...
> 原文: [https://www.geeksforgeeks.org/calendar-functions-in-python-set-1-calendar-month-isleap/](https://www.geeksforgeeks.org/calendar-functions-in-python-set-1-calendar-month-isleap/)

Python 定义了一个内置的模块 `calendar`，处理与日历相关的操作。

## `calendar` 模块上的操作

**1. `calendar(year, w, l, c)`**：该功能显示年份、字符宽度、每周行数和分栏数。
...
**3. `isleap(year)`**：该函数检查参数中提到的年份是否为 `闰年`。
...
**5. `month(year, month, w, l)`**：该函数打印参数中提到的特定年份的 `月`。一周取 4 个参数，年、月、字宽、行数。
