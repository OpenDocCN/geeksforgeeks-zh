...
> 原文：[https://www.geeksforgeeks.org/how-to-make-a-bell-curve-in-python/](https://www.geeksforgeeks.org/how-to-make-a-bell-curve-in-python/)
...
**方法：** 我们将列出 x 轴上的点，并将这些点传递到我们的自定义 `pdf` 函数中，以生成概率分布函数，从而生成对应于 `x` 中每个点的 `y` 值。现在，我们使用 `matplotlib` 库中可用的 `plot()` 和 `scatter()` 方法绘制曲线。`plot()` 方法用于制作线图，而 `scatter()` 方法用于在图形内部创建点状点。
