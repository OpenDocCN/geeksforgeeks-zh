...
为了告诉 Python 一个特定的目录是一个包，我们创建了一个名为 `__init__.py` 的文件，然后它被认为是一个包，我们可以在其中创建其他模块和子包。这个 `__init__.py` 文件可以留空，也可以用包的初始化代码进行编码。
...
2.  **然后我们需要创建模块**。为此，我们需要创建一个名为 `Bmw.py` 的文件，并通过将以下代码放入其中来创建其内容。
...
3.  **最后我们创建 `__init__.py` 文件。** 这个文件将被放置在 `Cars` 目录内，可以留空，或者我们可以将这段初始化代码放入其中。
...
现在，让我们使用我们创建的包。为此，在 `Cars` 包所在的目录中创建一个 `example.py` 文件，并向其中添加以下代码:
...
1.  **包内导入**
    假设 `Cars` 和品牌目录都是包。要使它们成为一个包，它们都必须包含 `__init__.py` 文件，或者是空白的，或者带有一些初始化代码。让我们假设 `Cars` 的所有模型都是模块。使用包有助于单独或整体导入任何模块。
    假设我们想买宝马 i8。其语法是:
...
4.  **‘from…import’ in Packages**
    现在，每当我们需要使用这样的函数时，我们都需要在导入父包之后写下整个长行。为了以更简单的方式解决这个问题，我们使用 `from` 关键字。为此，我们首先需要使用 `from` 和 `import` 来导入模块:
...
5.  **‘from…import *’ in Packages**
    当使用 `from…import` 语法时，我们可以从子模块导入任何内容，到类、函数或变量，这些都在同一个模块中定义。如果导入部分中提到的属性在包中未定义，则编译器将抛出 `ImportError` 异常。
