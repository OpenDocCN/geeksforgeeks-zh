...
> 原文：[https://www.geeksforgeeks.org/python-add-space-between-potential-words/](https://www.geeksforgeeks.org/python-add-space-between-potential-words/)

给定字符串列表，任务是在以大写字母开头的序列前添加一个空格。

> **输入**：`test_list = ["gfg Best", "forGeeks", "and computersciencestudents"]`
> **输出**：`['gfg Best', 'for Geeks', 'and Computer Science Students']`
> **解释**：用大写字母分隔的单词。
>
> **输入**：`test_list = ["computersciencestudentslovefg"]`
> **输出**：`['计算机专业学生爱 Gfg']`
> **解释**：大写字母分隔的单词。

## 方法 1：使用循环 + `join()`

这是执行这项任务的方法之一。在这种情况下，我们执行的任务是迭代所有的字符串，然后是所有的字符，然后使用循环以暴力的方式添加空格。`isupper()` 用于检查大写字符。
...
## 方法 2：使用 `re.sub()` + 列表推导式

上述功能的组合也可以用来解决这个问题。在本文中，我们使用正则表达式代码来检查大写字母，并使用列表推导式来执行空格添加和连接。
