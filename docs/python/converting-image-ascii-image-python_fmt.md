...
> 原文：[https://www.geeksforgeeks.org/converting-image-ascii-image-python/](https://www.geeksforgeeks.org/converting-image-ascii-image-python/)
...
`gscale1` 处的值是 70 级灰度斜坡，`gscale2` 处的值是更简单的 10 级灰度斜坡。这两个值都存储为字符串，字符范围从最暗到最亮。
...
首先，图像块作为 `PIL` 图像对象传入。在第二步将图像转换为 `numpy` 数组，此时 `im` 成为每个像素的二维亮度数组。第三步，存储图像的尺寸（宽度和高度）。第四步，`numpy.average()` 计算图像中亮度值的平均值，方法是使用 `numpy.reshape()` 首先将二维数组的宽度和高度（`w`，`h`）转换为一维数组的长度是宽度乘以高度（`w*h`）的乘积。`numpy.average()` 调用然后对这些数组值求和并计算平均值。
...
**资源**
1.  [维基百科:ASCII_ART](https://en.wikipedia.org/wiki/ASCII_art)
2.  Python 游乐场:好奇程序员的极客项目。
