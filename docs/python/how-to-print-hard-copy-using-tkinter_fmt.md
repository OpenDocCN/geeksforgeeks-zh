...
> 原文：[https://www.geeksforgeeks.org/how-to-print-hard-copy-using-tkinter/](https://www.geeksforgeeks.org/how-to-print-hard-copy-using-tkinter/)

**先决条件：** `Tkinter`、[`win32api`](https://www.geeksforgeeks.org/design-a-keylogger-in-python/)
...
*   `askopenfilename`：此方法用于打开给定的文件。
    ...
    > `filedialog.askopenfilename(mode='r', filetypes=[('任何要显示的名称', '文件类型的扩展名')])`
...
*   `ShellExecute`：用于执行系统的 shell 命令。
    ...
    > `file`：要打开的文件的名称。
    >
    > `params`：要传递的参数，如果文件名包含 associate 可行。对于文档文件，应该是无。
    >
    > `dir`：应用程序的初始目录。
    >
    > `bShow`：指定设备打开后是否显示。如果 `file` 参数指定了文档文件，则该参数为零。
