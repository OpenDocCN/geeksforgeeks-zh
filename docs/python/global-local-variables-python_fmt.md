...
> 原文：[https://www.geeksforgeeks.org/global-local-variables-python/](https://www.geeksforgeeks.org/global-local-variables-python/)
...
如果我们想做赋值或者改变全局变量，我们只需要在函数中使用 `global` 关键字。打印和访问不需要全局。Python“假设”我们想要一个局部变量，因为在 `f()` 内部赋值给 `s`，所以第一条语句抛出错误消息。任何在函数内部更改或创建的变量，如果没有声明为全局变量，都是局部变量。要告诉 Python，我们想要使用全局变量，我们必须使用关键字 `global`，如下例所示：
