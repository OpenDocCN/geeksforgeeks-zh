...
> 原文：[https://www.geeksforgeeks.org/listing-directories-files-python/](https://www.geeksforgeeks.org/listing-directories-files-python/)
...
7.  `isfile()` – 它检查传递的参数是否表示文件的路径。如果是，则返回 `True`，否则返回 `False`。
8.  `isdir()` – 它检查传递的参数是否表示目录的路径。如果是，则返回 `True`，否则返回 `False`。
9.  `append()` – 用于追加列表中的项目。
...
`/Users/admin/projects/Python/PythonFiles` 里面有大量的 Python 文件和目录，我们将一一列出。
...
本文由 **Rishikesh Agrawani** 供稿。如果你喜欢 GeeksforGeeks 并想投稿，你也可以使用`contribute.geeksforgeeks.org`写一篇文章或者把你的文章邮寄到`contribute@geeksforgeeks.org`。看到你的文章出现在极客博客主页上，帮助其他极客。
