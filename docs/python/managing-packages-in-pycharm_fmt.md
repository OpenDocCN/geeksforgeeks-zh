...
`PyCharm` 支持 Python 包的安装、卸载和升级。默认情况下，`PyCharm` 同样使用 `pip` 包管理器。类似地，`conda` 包管理器用于处理 `conda` 环境。在本文中，我们将研究在 `PyCharm` 中管理 Python 包的过程。
...
2.  **选项**：如果选中，它可以用来运行文本字段中的 `pip` 命令（例如 `pip install Flask`）。
3.  **安装到用户的站点包目录<路径>**：如果勾选，可以指定你希望包安装的位置（即具体目录）。默认情况下，它在当前解释器中安装包。该选项**而不是**可用于 `conda` 环境。

### `第四步：` 在最后点击`安装包`。根据你的网速，需要一些时间。
