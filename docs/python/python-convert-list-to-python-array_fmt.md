...
> 原文: [https://www.geeksforgeeks.org/python-convert-list-to-python-array/](https://www.geeksforgeeks.org/python-convert-list-to-python-array/)

有时，在 Python 中工作时，我们可能会遇到这样的问题：我们需要将数据元素限制为只有一种类型。列表可以是异构的，可以有多个数据类型的数据，这有时是不可取的。需要将其转换为限制数据类型的数据结构。Python 语言附带了可用于此目的的数组数据结构。让我们讨论一种将列表转换为数组的方法。
