...
> 原文：[https://www.geeksforgeeks.org/authentication-using-python-requests/](https://www.geeksforgeeks.org/authentication-using-python-requests/)
...
**OAuth 1 认证**
几种 web APIs 的一种常见认证形式是 OAuth。`requests-oauthlib` 库允许请求用户轻松地发出 OAuth 1 验证请求:
...
有关如何操作 OAuth 流的更多信息，请参见 OAuth 官方网站。有关 `requests-oauthlib` 的示例和文档，请参见 GitHub 上的 `requests_oauthlib` 存储库

**OAuth 2 和 OpenID Connect 认证**
`requests-oauthlib` 库还处理 OAuth 2，这是支持 OpenID Connect 的认证机制。有关各种 OAuth2 凭据管理流程的详细信息，请参见 [`requests-oauthlib` OAuth2 文档](https://requests-oauthlib.readthedocs.io/en/latest/oauth2_workflow.html)：
...
**其他身份验证**
`Requests` 旨在允许轻松快速地插入其他形式的身份验证。开源社区的成员经常为更复杂或不常用的身份验证形式编写身份验证处理程序。一些最好的项目被归入“请求”组织，包括：
