...
> 原文：[https://www.geeksforgeeks.org/python-check-if-k-occurs-atleast-n-times-in-a-list/](https://www.geeksforgeeks.org/python-check-if-k-occurs-atleast-n-times-in-a-list/)
...
方法 2：使用 `next()` + `islice()`
这两个功能可以一起使用，以比上面更有效的方式执行这个特定的任务。`islice` 函数将处理求和部分，`next()` 函数有助于迭代逻辑。
