...
> 原文：[https://www.geeksforgeeks.org/inventory-management-with-json-in-python/](https://www.geeksforgeeks.org/inventory-management-with-json-in-python/)
...
print("Enter Your User ID if You are Old Customer else press '0' To New User ID :- ")
p = int(input())
...
if (user_id != -1):
    ...
    for i in range(n):
        print("Enter Product ID of Product " + str(i+1)+" that you want to buy")
        id = input()
        ...
        if id in data.keys():
            ...
            if (float(quantity) <= float(data[id]['quantity'])):
                ...
                print("The Quantity You Have Asked is Quite High Than That is Available in Stock")
                print("Did you Want To buy According to The Quantity Available in Stock then Enter '0' Else '1' to skip This Product")
                key = int(input())
