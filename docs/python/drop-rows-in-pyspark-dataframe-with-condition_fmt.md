# 用条件删除 PySpark 数据框中的行

> 原文: [https://www.geeksforgeeks.org/drop-rows-in-pyspark-dataframe-with-condition/](https://www.geeksforgeeks.org/drop-rows-in-pyspark-dataframe-with-condition/)
...
**`where()`:** 该功能用于检查条件并给出结果。这意味着它会根据条件删除行。
...
> **语法:** `dataframe.where(dataframe.column.isNotNull())`
...
> **语法:** `dataframe.dropDuplicates(['column_name'])`
...
## 使用 `distinct` 函数删除重复的行

我们可以通过使用 `distinct` 函数来删除重复的行。
