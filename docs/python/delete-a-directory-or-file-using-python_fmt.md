...
> 原文: [https://www.geeksforgeeks.org/delete-a-directory-or-file-using-python/](https://www.geeksforgeeks.org/delete-a-directory-or-file-using-python/)
...
Python 中的 `os.remove()` 方法用于移除或删除文件路径。此方法不能删除目录。如果指定的路径是目录，则方法将引发 `OSError`。

> **语法:** `os.remove(path, *, dir_fd=None)`
>
> **参数:**
> **`path`**: 表示文件路径的类路径对象。类似路径的对象是表示路径的字符串或字节对象。
> **`dir_fd` (可选):** 引用目录的文件描述符。此参数的默认值为 `None`。
...
Python 中的 `os.rmdir()` 方法用于移除或删除空目录。如果指定的路径不是空目录就会引发 `OSError`。
...
**示例 3:** 通过在 `onerror` 中传递一个错误，应该传递一个必须包含三个参数的函数。

*   **函数–** 引发异常的函数。
*   **路径–** 传递的路径名在移除时引发了异常
*   **exit info–** `sys.exc_info()` 引发的异常信息
