...
> 原文：[https://www.geeksforgeeks.org/facial-expression-detection-using-deepface-module-in-python/](https://www.geeksforgeeks.org/facial-expression-detection-using-deepface-module-in-python/)
...
**步骤 3:** 创建一个结果变量，用于存储结果。使用 `Deepface` 的 `analyze()` 方法，`Deepface` 的 `analyze()` 方法包含很强的面部属性分析特征，如年龄、性别、面部表情。面部表情包括愤怒、恐惧、中立、悲伤、厌恶、高兴和惊讶。打印结果。结果显示该人的面部表情百分比。
