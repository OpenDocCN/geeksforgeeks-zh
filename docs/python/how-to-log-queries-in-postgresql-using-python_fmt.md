...
> 原文：[https://www.geeksforgeeks.org/how-to-log-queries-in-postgresql-using-python/](https://www.geeksforgeeks.org/how-to-log-queries-in-postgresql-using-python/)
...
所有记录器都是 `根记录器` 的后代。`根记录器` 总是有一个显式的级别集，默认情况下是 `警告`。`根记录器` 可用于轻松打开和关闭所有库中的所有记录器。
...
使用了 `根记录器`，只写了三条消息。这是因为默认情况下，只写入级别 `警告` 和更高的消息。
