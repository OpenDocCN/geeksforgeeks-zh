...
`divmod()` 方法取两个参数 `x` 和 `y`，其中 `x` 作为分子，`y` 作为分母。该方法同时计算 `x // y` 和 `x % y`，并返回两个值。

*   如果 `x` 和 `y` 是整数，返回值为 `(x // y, x % y)`。
*   如果 `x` 或 `y` 是浮点数，结果是 `(q, x % y)`，其中 `q` 是商的整数部分。
...
1.  如果其中一个参数（比如 `x` 和 `y`）是浮点数，那么结果就是 `(q, x % y)`。这里，`q` 是商的整数部分。
2.  如果第二个参数为 `0`，则返回除零错误。
3.  如果第一个参数为 `0`，则返回 `(0, 0)`。

## 实际应用

使用 `divmod()` 函数检查一个数是否为素数。
