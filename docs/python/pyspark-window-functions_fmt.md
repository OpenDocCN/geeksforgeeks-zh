...
为了首先对一组行执行窗口函数操作，我们需要进行分区，即使用 `Window.partitionBy()` 函数定义数据行组，对于行号和秩函数，我们需要使用 `orderBy` 子句对分区数据进行额外的排序。

> Window.partition 语法:
> `window.partitionBy("column_name").orderBy("column_name")`
>
> **窗口功能语法:**
> `dataframe.withColumn("new_col_name", Window_function().over(windowPartition))`

让我们用例子来逐一理解和实现所有这些功能。

## 分析功能

分析函数是对数据或由 `SELECT` 子句或 `ORDER BY` 子句划分的有限行集进行操作后返回结果的函数。它返回的结果行数与输入行数相同。例如 `lead()`、`lag()`、`cume_dist()`。
...
在输出中，我们可以看到一个名为“cume_dist”的新列被添加到 `df` 中，它包含按 `Age` 列排序的 `Department` 列的累积分布。
...
在输出中，按照窗口分区中指定的“Subject”和“Marks”列，为每行提供了等级。
...
聚合函数或聚合函数是将多行的值分组以形成单个汇总值的函数。它们所操作的行组的定义是通过使用 SQL `GROUP BY` 子句完成的。例如 `avg()`、`sum()`、`min()`、`max()` 等。
