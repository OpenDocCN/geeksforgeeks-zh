...
> 原文: [https://www.geeksforgeeks.org/insert-python-list-into-postgresql-database/](https://www.geeksforgeeks.org/insert-python-list-into-postgresql-database/)
...
首先将所有需要的库导入工作空间，并建立数据库连接。将自动提交设置为 `false`，并创建一个 `cursor` 对象。现在，创建一个要插入到表中的数据列表。遍历列表并插入值。提交并关闭连接。
