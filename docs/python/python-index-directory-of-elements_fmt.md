...
> 原文: [https://www.geeksforgeeks.org/python-index-directory-of-elements/](https://www.geeksforgeeks.org/python-index-directory-of-elements/)
...
## 方法一：利用词典理解 + `enumerate()` + `set()`

在这种情况下，我们使用 `enumerate()` 来获取所有索引，并将其附加到索引列表中以进行值匹配。字典理解用于列表中所有元素的迭代。`set()` 用于不重复获取所有元素。
...
## 方法二：使用字典理解 + `groupby()` + `enumerate()` + `sorted()` + `itemgetter()`
