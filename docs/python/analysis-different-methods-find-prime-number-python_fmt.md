...
> 原文: [https://www.geeksforgeeks.org/analysis-different-methods-find-prime-number-python/](https://www.geeksforgeeks.org/analysis-different-methods-find-prime-number-python/)

如果你参加竞争性编程，你可能会熟悉这样一个事实，即与素数相关的问题是问题设置者的选择之一。在这里，我们将讨论如何优化你的函数，该函数在给定的一组范围内检查素数，并且还将计算执行它们的时间。
...
在上面的代码中，我们检查了从 1 到 100000 的所有数字，不管这些数字是否是质数。它有一个巨大的运行时间，如图所示。运行大约需要 1 分钟。这是一种简单的方法，但需要花费大量时间来运行。所以，它在竞争性编程中不是首选。
...
在这个函数中，我们计算一个整数，比如 `max_div`，它是数字的平方根，并使用 Python 的 `math` 库获得它的 `floor` 值。在最后一个例子中，我们从 2 迭代到 `n-1`。但是在这里，我们把除数减少了一半，如图所示。你需要导入 `math` 模块来获得 `floor` 和 `sqrt` 函数。
...
现在，我们将优化我们的代码到下一个级别，比以前的方法花费更少的时间。你可能已经注意到，在最后一个示例中，我们迭代了每个偶数，直到达到极限，这是一种浪费。需要注意的是，除了两个以外，所有的偶数都不能是质数。在这个方法中，我们踢出所有的偶数来优化我们的代码，并且只检查奇数除数。
