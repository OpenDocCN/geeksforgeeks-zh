...
> 原文: [https://www.geeksforgeeks.org/how-to-open-and-close-a-file-in-python/](https://www.geeksforgeeks.org/how-to-open-and-close-a-file-in-python/)
...
> *   `Access_Mode`: 访问模式控制打开文件中可能的操作类型。下表列出了 Python 中所有可用的访问模式:
...
| 直读式记录 | `r+` | 打开文件进行读写。 |
| 只写 | `w` | 打开文件进行写入。 |
| 写和读 | `w+` | 打开文件进行读写。与 `r+` 不同，如果文件不存在，不会引发 I/O 错误。 |
