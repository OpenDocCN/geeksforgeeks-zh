...
> 原文：[https://www.geeksforgeeks.org/python-easygui-module-introduction/](https://www.geeksforgeeks.org/python-easygui-module-introduction/)

`EasyGUI` 是一个用 Python 进行非常简单、非常容易的 GUI 编程的模块。`EasyGUI` 与其他 GUI 生成器的不同之处在于 `EasyGUI` 不是事件驱动的。相反，所有的图形用户界面交互都是通过简单的函数调用来调用的。与其他复杂的图形用户界面不同，`EasyGUI` 是迄今为止最简单的图形用户界面。
