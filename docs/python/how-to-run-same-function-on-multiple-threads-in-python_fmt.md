...
> 原文: [https://www.geeksforgeeks.org/how-to-run-same-function-on-multiple-threads-in-python/](https://www.geeksforgeeks.org/how-to-run-same-function-on-multiple-threads-in-python/)
...
在 Python 中，我们可以使用 [`threading`](https://www.geeksforgeeks.org/thread-based-parallelism-python/) 模块创建和运行线程。python 中的这个模块为线程提供了强大的高级支持。

## 逐步方法

*   导入库。我们将使用 `threading` 模块来创建和运行线程。为了观察输出，我们将使用 `time` 模块创建一些延迟。
...
*   现在使用 `threading.Thread` 类创建 2 个或更多线程。创建线程的语法如下:
...
*   现在我们需要开始执行线程。`Thread` 类有一个 `start()` 方法，在运行模式下启动线程。线程将一直运行，直到它们完成。
...
*   我们可以使用 `Thread` 类的 `join()` 方法，在所有线程完成之前阻止程序继续执行。
