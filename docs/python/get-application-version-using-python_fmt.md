...
> 原文: [https://www.geeksforgeeks.org/get-application-version-using-python/](https://www.geeksforgeeks.org/get-application-version-using-python/)
...
*   `LOWORD`: win32 API `LOWORD` 宏的接口。值是整数数据类型。

```py
LOWORD(val)
```

*   `HIWORD`: win32 API `HIWORD` 宏的接口。值是整数数据类型。
