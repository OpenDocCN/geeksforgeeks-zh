...
> 原文:[https://www.geeksforgeeks.org/colorize-images-using-deoldify/](https://www.geeksforgeeks.org/colorize-images-using-deoldify/)
...
*   `self-attention`: 作者使用 `U-Net` 架构作为生成器，他们还修改了架构，在模型中使用了光谱归一化和 `self-attention`。
*   `two-timescale update rule`: 是训练 `GAN` 架构的一种方式。这只是一个一对一的生成器/评论家架构，以及更高的评论家学习率。这是修改，以纳入一个临界批评损失，以确保批评是“赶上”之前，继续发电机培训。这对于 `NoGAN` 训练特别有用。
*   `No-GAN`: 这种 `GAN` 训练方法是由模型的作者开发的。该模型背后的主要思想是，您可以获得氮化镓培训的好处，同时花费最少的时间进行直接氮化镓培训。我们将更详细地讨论 `NoGAN`。
*   `generator loss`: 发电机中有两种类型的 `NoGAN` 学习:
    *   `perceptual loss`: 该损失在发电机中用于报告和最小化由于模型中的偏差而产生的损失。
    *   `critic loss`: 是鉴别者/批评家中使用的损失。
...
*   `artistic`: 该模型在图像着色、细节和活力方面取得了最佳效果。这些模型使用带有 `U-Net` 的 `ResNet 34` 主干架构，强调解码器侧的层深度。该模型存在一些缺点，例如该模型不能为自然场景和人像等常见任务提供稳定性，并且需要大量时间和参数调整才能获得最佳结果。
*   `stable`: 该模型在风景和人像中取得最佳效果。它为人脸提供了更好的颜色，而不是人脸的灰色。这些模型使用带有 `U-Net` 的 `ResNet 101` 主干架构，强调解码器侧的层深度。这种模型通常没有艺术模型那么怪异的杂色，但也没有那么丰富多彩。
*   `video`: 该模型针对平滑、一致和无闪烁的视频进行了优化。这将是所有三个模型中最没有色彩的。该模型类似于“稳定”的架构，但在训练上有所不同。
...
source_url = 'https://preview.redd.it/a702q2585j961.jpg?width=640&crop=smart&auto=webp&s=a5f2523513bb24648737760369d2864eb1f57118' #@param {type:"string"}
