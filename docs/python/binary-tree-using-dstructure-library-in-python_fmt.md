...
> 原文: [https://www.geeksforgeeks.org/binary-tree-using-dstructure-library-in-python/](https://www.geeksforgeeks.org/binary-tree-using-dstructure-library-in-python/)

`dstructure`包是一个处理数据结构和算法的 Python 库。在本文中，我们将讨论如何使用 Python 中的`dstructure`库实现二叉树并执行各种操作。
...
元素最多有两个子元素的树称为[二叉树](https://www.geeksforgeeks.org/binary-tree-set-1-introduction/)。由于二叉树中的每个元素只能有两个子元素，我们通常将它们命名为左右子元素。然而，`dstructure`库有助于直接实现二叉树。
