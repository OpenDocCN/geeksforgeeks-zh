...
> 原文：[https://www.geeksforgeeks.org/compare-two-files-line-by-line-in-python/](https://www.geeksforgeeks.org/compare-two-files-line-by-line-in-python/)
...
在 `difflib` 库中，有一个类可用于比较名为 `Differ` 的文件之间的差异。这个类用于比较文本行的序列，并产生人类可读的差异或增量。
...
### 方法 3：使用 `while` 循环和 `intersection()` 方法

**接近**

*   以读取模式打开这两个文件
*   存储字符串列表
*   开始借助公共字符串的 `intersection()` 方法来比较这两个文件
