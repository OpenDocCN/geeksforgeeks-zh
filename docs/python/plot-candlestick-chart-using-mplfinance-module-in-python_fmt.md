...
> 原文：[https://www.geeksforgeeks.org/plot-candlestick-chart-using-mplfinance-module-in-python/](https://www.geeksforgeeks.org/plot-candlestick-chart-using-mplfinance-module-in-python/)
...
> **语法：** `mplfinance.candlestick_ohlc(ax, quotes, width=0.2, colorup='k', colordown='r', alpha=1.0)`
> **参数：**
> *   `ax`：要绘制到的轴实例。
> *   `quotes`：序列（时间、开盘、高、低、收盘、…）序列。
> *   `width`：矩形宽度一天的分数。
> *   `colorup`：闭合处矩形的颜色 >= 打开。
> *   `colordown`：关闭的矩形的颜色 < 打开。
