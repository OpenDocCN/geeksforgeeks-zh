...
> 原文: [https://www.geeksforgeeks.org/how-to-find-available-wifi-networks-using-python/](https://www.geeksforgeeks.org/how-to-find-available-wifi-networks-using-python/)
...
对于子流程模块，我们需要使用 `check_output()` 方法。我们将传递一个关于 WiFi 网络我们需要知道的事情的列表。我们将需要 `netsh`，`wlan`，`show` 和 `network`。传递这些参数是为了将输出存储在其中，然后将其转换为字符串来显示输出。
