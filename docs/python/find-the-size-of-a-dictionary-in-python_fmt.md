...
> 原文: [https://www.geeksforgeeks.org/find-the-size-of-a-dictionary-in-python/](https://www.geeksforgeeks.org/find-the-size-of-a-dictionary-in-python/)

[Python 中的 Dictionary](https://www.geeksforgeeks.org/python-dictionary/) 是一个无序的数据值集合，用于像映射一样存储数据值。与其他只保存单个值作为元素的数据类型不同，Dictionary 保存 `key:value` 对。字典中提供了键值，以使其更加优化。字典的大小是指字典对象占用的内存量（以字节为单位）。在本文中，我们将学习获得 Python 字典大小的各种方法。
