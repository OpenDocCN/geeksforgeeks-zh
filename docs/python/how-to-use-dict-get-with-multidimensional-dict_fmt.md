...
> 原文: [https://www.geeksforgeeks.org/how-to-use-dict-get-with-multidimensional-dict/](https://www.geeksforgeeks.org/how-to-use-dict-get-with-multidimensional-dict/)

如果关键字在字典中，Python 中的 `get()` 方法会返回关键字的值。如果关键字不在字典中，则返回 `None`。它还接受另一个可选参数，即在字典中找不到关键字时将返回的值。

> **语法:** `dict.get(key, value)`

这里，`key` 是一个必选参数，它是一个我们想从字典中获取值的键。而 `value` 是一个可选字段，它是在字典中找不到指定键时返回的值。`value` 的默认值为 `None`。
...
现在，让我们看看如何在多维字典中使用 `dict.get()`。对于多维字典，我们在单个语句中多次使用 `get()`。
...
你可以看到它给出了正确的输出。让我们理解这个 `dict.get()` 的工作原理。首先 `dict.get('India')` 返回字典中关键字 `'India'` 的所有值。它返回 `{'captain':'Virat', 'Batsman':'Rohit', 'Bolwer':'Bumrah'}`。现在对于这个字典我们再次使用 `get()` 方法，确实找到了值。所以对于 `'Batsman'` 来说，它返回了 `'Rohit'`。
...
你可以看到 `'Fielder'` 不存在于由 `'India'` 返回的字典中。这就是为什么它给出 `'Not Found'` 作为输出。但是使用 `get()` 方法有一个问题。如果没有找到对应于第一个键的值，那么它将返回一个 `None` 字符串，第二个 `get()` 方法将应用于该字符串。这将给出一个错误，因为 `dict.get()` 是字典的方法，而不是字符串的方法。
...
这给出了 `'NoneType'` 对象错误，因为 `None` 数据类型没有名为 `get()` 的方法。因此，为了解决这个错误，我们将使用 `get()` 方法的第二个参数，如果在字典中找不到键，则该参数用于默认输出。如果字典中不存在关键字，我们将返回一个空字典。
