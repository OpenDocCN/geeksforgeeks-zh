...
> 原文: [https://www.geeksforgeeks.org/python-extract-item-with-maximum-tuple-value/](https://www.geeksforgeeks.org/python-extract-item-with-maximum-tuple-value/)
...
# Extract Item with Maximum Tuple Value
# Using max() + map() + itemgetter() + zip()
res = max(zip(test_dict.keys(), map(itemgetter(tup_idx), test_dict.values())), key = itemgetter(1))[0]
