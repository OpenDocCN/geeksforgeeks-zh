...
[超参数调整](https://www.geeksforgeeks.org/ml-hyperparameter-tuning/)是通过调整神经网络的参数来提高模型的效率。一些 scikit-learn API，如 `GridSearchCV` 和 `RandomizedSearchCV`，用于执行超参数调整。

在本文中，您将学习如何使用 `GridSearchCV` 来调整 Keras 神经网络超参数。

## 方法:

1.  我们将包装在 scikit-learn 中使用的 Keras 模型——使用 `KerasClassifier`，它是一个包装器。
2.  我们将使用 `KerasClassifier` 和 `GridSearchCV` 进行交叉验证。
...
数据集可以从[这里](https://www.geeksforgeeks.org/hyperparameter-tuning-using-gridsearchcv-and-kerasclassifier/)下载。导入数据集，使用该数据集我们可以预测客户是留下还是离开。
