...
`dict()`函数用于创建 [Python 字典](https://www.geeksforgeeks.org/python-dictionary/)。字典是键值对的集合。字典是可变的数据结构，即字典中的数据可以修改。在 python 3.6 和更低版本中，dictionary 是键值对的无序集合，但在 python 3.7 中，dictionary 是有序的数据结构。字典是一种索引数据结构，即字典的内容可以通过使用索引来访问，在字典中，关键字用作索引。
