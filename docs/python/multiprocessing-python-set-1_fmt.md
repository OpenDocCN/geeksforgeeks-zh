...
在 Python 中，`多处理`模块包括一个非常简单直观的 API，用于在多个进程之间划分工作。
让我们考虑一个使用`多处理`模块的简单例子:
...
*   要导入`多处理`模块，我们需要:
    ...
    import multiprocessing
    ```
*   为了创建一个进程，我们创建一个`Process`类的对象。它接受以下参数:
    *   `目标`:流程要执行的功能
    *   `参数`:传递给目标函数的参数

注:`Process`构造器还接受许多其他参数，这些参数将在后面讨论。在上面的例子中，我们用不同的目标函数创建了两个过程:
    ```py
    p1 = multiprocessing.Process(target=print_square, args=(10, ))
    p2 = multiprocessing.Process(target=print_cube, args=(10, ))
    ```
*   要开始一个流程，我们用`Process`类的`start()`方法。
    ```py
    p1.start()
    p2.start()
    ```
*   一旦进程开始，当前程序也会继续执行。为了停止当前程序的执行直到一个进程完成，我们使用`join()`方法。
    ```py
    p1.join()
    p2.join()
    ```
...
*   主 python 脚本有一个不同的进程 ID，`多处理`模块在我们创建`Process`对象`p1`和`p2`时生成具有不同进程 ID 的新进程。在上面的程序中，我们使用`os.getpid()`函数来获取运行当前目标函数的进程的 ID。
请注意，它与我们使用`Process`类的`pid`属性获得的`p1`和`p2`的流程标识相匹配。
*   每个进程独立运行，有自己的内存空间。
*   一旦目标函数的执行完成，进程就会终止。在上面的程序中，我们使用了`Process`类的`is_alive()`方法来检查一个进程是否仍然活动。
