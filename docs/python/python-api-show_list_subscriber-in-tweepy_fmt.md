# Python Tweepy中的API.show_list_subscriber()

> 原文：[https://www.geeksforgeeks.org/python-api-show_list_subscriber-in-tweepy/](https://www.geeksforgeeks.org/python-api-show_list_subscriber-in-tweepy/)
...
### 语法
`API.show_list_subscriber(参数)`

### 参数
*   `list_id`：列表的 id。
*   `slug`：列表中的 slug。
*   `screen_name`：被检查用户的屏幕名称。
*   `user_id`：被检查用户的用户 id。
*   `owner_id`：列表所有者的 id。
