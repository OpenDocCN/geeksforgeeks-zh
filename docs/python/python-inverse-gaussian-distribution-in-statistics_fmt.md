...
> 原文: [https://www.geeksforgeeks.org/python-inverse-gaussian-distribution-in-statistics/](https://www.geeksforgeeks.org/python-inverse-gaussian-distribution-in-statistics/)

`scipy.stats.invgauss()` 是一个反高斯连续随机变量。它继承自泛型方法的，作为 `rv_continuous` 类的实例。它用特定于这个特定分布的细节来完成这些方法。
