# 使用 Python 将 PNG 转换为 JPG

> 原文：[https://www.geeksforgeeks.org/convert-png-to-jpg-using-python/](https://www.geeksforgeeks.org/convert-png-to-jpg-using-python/)

PNG 和 JPG 格式用于图像插图。这两种格式都被用来为某些类型的图像提供良好的兼容性，比如 PNG 更适合线条画和图标图形，而 JPG 适合照片。然而，对于媒体和图片的使用和存储，两者都是可以相互转换的。Python 提供了各种选项来执行图像转换。
...
打开并识别图像文件。除非显式执行 `load()` 操作，否则它不会加载文件。它只是打开图像，而没有实际分析图像内容。
...
用指定的文件名保存图像。如果没有指定扩展名，则从指定的文件名分析扩展名。
...
> **参数:**
>
> `path` — 表示文件名的字符串，文件名必须包含扩展名图像格式，如 `.jpg`，`.png` 等。
>
> `image` — 要保存的图像。
>
> **返回类型:**
