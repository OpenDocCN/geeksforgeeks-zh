# Python Decimal compare_total_mag()方法

> 原文：[https://www.geeksforgeeks.org/python-decimal-compare_total_mag-method/](https://www.geeksforgeeks.org/python-decimal-compare_total_mag-method/)

## 方法概述

`Decimal.compare_total_mag()` 是 `Decimal` 类的一个方法，它通过使用两个 `Decimal` 值的抽象表示（即绝对值）而不是它们的实际值来进行比较，并且会忽略每个操作数的符号。
