...
> 原文: [https://www.geeksforgeeks.org/how-to-round-numbers-in-python/](https://www.geeksforgeeks.org/how-to-round-numbers-in-python/)
...
> 截断函数可以通过以下方式实现:
>
> *   将数字乘以 `10 ** p` (10 的 p 次方)将小数点向右移动 `p` 位。
> *   使用 `int()` 获取新数字的整数部分。
...
## 方法三: 使用 `math.ceil()` 和 `math.floor()` 功能
...
> 向上舍入功能可以通过以下方式实现:
>
> *   首先，通过将 `n` 乘以 `10 ** decimals`，将 `n` 中的小数点向右移动正确的位数。
> *   使用 `math.ceil()` 将新值向上舍入到最接近的整数。
> *   最后，通过除以 `10 ** decimals`，小数点被移回左边。

使用 `round_up()` 用户自定义函数实现向上舍入概念:
...
>
> *   首先，通过将 `n` 乘以 `10 ** decimals`，将 `n` 中的小数点向右移动正确的位数。
> *   使用 `math.floor()` 将新值向下舍入到最接近的整数。
> *   最后，通过除以 `10 ** decimals`，小数点被移回左边。

使用 `round_down()` 用户自定义函数实现向下舍入概念:
...
当我们对数据集中的值进行舍入时，有一种方法可以减轻舍入偏差。我们可以简单地将关系舍入到所需精度的最接近的偶数。半到偶数策略是 Python 内置 `round()` 使用的策略。`Decimal` 类支持快速正确舍入的十进制浮点运算。与浮点数据类型相比，这有几个优点。十进制模块中的默认舍入策略是 `ROUND_HALF_EVEN`。
