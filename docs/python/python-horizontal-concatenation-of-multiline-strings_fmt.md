...
> 原文: [https://www.geeksforgeeks.org/python-horizontal-concatenation-of-multiline-strings/](https://www.geeksforgeeks.org/python-horizontal-concatenation-of-multiline-strings/)
...
> **输出** :
> `geeksgeeks`
> **解释**:第一行与第二串第一行连接，“geeks” -> “geeks”。

## 方法一: 使用 `zip()` + `split()` + `join()` + 列表理解

在本例中，我们使用 `split()` 通过 `'\n'` 执行拆分任务，并使用 `zip()` 将它们配对在一起。下一步是使用 `'\n'` 连接水平方向的压缩字符串和 `join()`。
...
## 方法 2: 使用 `map()` + `operator.add` + `join()`

在本例中，我们使用 `map()` 来借助 `add()` 执行拼接，`split()` 用于通过 `'\n'` 执行初始拆分。
