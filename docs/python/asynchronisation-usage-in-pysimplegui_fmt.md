...
我们将这种设计模式用于实际上需要定期轮询或输出的项目。在这种情况下，我们表示希望在窗口中有一个`timeout=10`。此外，这将导致调用返回一个“超时键”，因为每 10 毫秒的事件已经过去，而没有一些图形用户界面的事情首先发生。超时键是`PySimpleGUI.TIMEOUT_KEY`，通常写成`sg.TIMEOUT_KEY`。

使用带超时的窗口时请小心。您很少需要使用`timeout=0`。零值是非阻塞调用，因此我们不需要试图滥用这种设计模式。
