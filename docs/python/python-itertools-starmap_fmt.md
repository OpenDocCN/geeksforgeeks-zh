...
[`itertools`](https://www.geeksforgeeks.org/python-itertools/) 是 Python 中的一个模块，它有一组用于处理迭代器的函数。它们使得遍历像列表和字符串这样的数据项变得非常容易。其中一个 itertools 功能是 `starmap()`。
...
这里，`map()`将列表中的每个元组视为单个参数，因此会引发错误。`starmap()`克服了这个问题。请看下面的代码片段:
