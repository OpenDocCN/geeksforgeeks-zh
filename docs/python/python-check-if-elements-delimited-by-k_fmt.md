...
> 原文：[`https://www.geeksforgeeks.org/python-check-if-elements-delimited-by-k/`](https://www.geeksforgeeks.org/python-check-if-elements-delimited-by-k/)
...
这是执行这项任务的方法。在本文中，我们使用 `isalpha()` 和 `isdigit()` 执行检查字母和数字段的任务。任何元素（不完全是数字或字母）的存在都被称为非 `K` 定界，并且在 `split()` 过程中仍未解决。
