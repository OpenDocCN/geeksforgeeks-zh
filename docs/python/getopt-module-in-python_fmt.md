...
`getopt` 模块是一个基于 Unix `getopt()`函数建立的惯例的命令行选项解析器。它通常用于解析参数序列，如 `sys.argv`。换句话说，这个模块帮助脚本解析 `sys.argv` 中的命令行参数。它的工作方式类似于解析命令行参数的 C `getopt()`函数。
...
> 参数:
> `args`: 要传递的参数列表。
> `options`: 脚本想要识别的一串选项字母。需要参数的选项后面应该跟一个冒号(`:`)。
> `long_options`: 长选项名称的字符串列表。需要参数的选项后面应该跟一个等号(`=`)。
> 
