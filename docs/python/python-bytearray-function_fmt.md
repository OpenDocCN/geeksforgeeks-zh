...
`source`参数可以用几种不同的方式初始化数组。让我们借助例子逐一讨论。

## 代码示例

### 代码#1
如果字符串必须提供`encoding`和`errors`参数，`bytearray()`使用`str.encode()`将字符串转换为字节。
...
### 代码#4
如果`Iterable`(范围`0 <= x < 256`)，用作数组的初始内容。
