...
> 原文：[https://www.geeksforgeeks.org/mathematical-functions-in-python-set-3-trigonometric-and-angular-functions/](https://www.geeksforgeeks.org/mathematical-functions-in-python-set-3-trigonometric-and-angular-functions/)
...
## 1. `sin()`
该函数返回作为参数传递的值的**正弦**。这个函数传递的值应该是**弧度**。

## 2. `cos()`
该函数返回作为参数传递的值的**余弦**。这个函数传递的值应该是**弧度**。
...
## 3. `tan()`
该函数返回作为参数传递的值的**正切值**。这个函数传递的值应该是**弧度**。

## 4. `hypot(a, b)`
返回参数中传递的值的**斜边**。数值上，它返回 `sqrt(a*a + b*b)` 的值。
