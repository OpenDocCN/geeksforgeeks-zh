...
> 原文: [https://www.geeksforgeeks.org/create-a-yes-no-message-box-in-python-using-tkinter/](https://www.geeksforgeeks.org/create-a-yes-no-message-box-in-python-using-tkinter/)

Python 提供了许多图形用户界面（GUI）框架，但 Tk 界面或 [`Tkinter`](https://www.geeksforgeeks.org/python-gui-tkinter/) 是使用最广泛的框架。它是跨平台的，允许相同的代码运行，而与操作系统平台（视窗、Linux 或苹果操作系统）无关。`Tkinter` 重量轻，速度快，使用简单。`Tkinter` 提供了多种小部件，可以使用标准属性和几何管理方法进行定制。`Tkinter` 消息框可用于向用户提问或显示消息。
...
*   `title`：用于给出一个名称，显示在对话框的标题中。
*   `message`：为用户提问。

**返回值**：点击是选项返回 `'yes'`，点击否选项返回 `'no'`。
...
用于创建父窗口。`Tk` 类被实例化，没有任何参数。通过更改类名参数的值，可以将父窗口的名称更改为所需的名称。这里的 `root` 是父窗口。
...
*   `master`：用于表示父窗口。这里的 `root` 就是主人。
*   `option`：用于指定边框、背景色、高度、宽度等。

**返回值**：方法返回一个字符串（`!canvas`）。
...
*   `master`：这里 `root` 是父窗口。
*   `options`：有许多受支持的选项。这种情况下使用的选项是 `text` 和 `command`。
    *   `text`：按钮文本
    *   `command`：按下按钮时要调用的动作或方法。

**返回值**：方法返回一个字符串（`!button`）。
...
*   `options`：支持多种选项，如锚点、高度、宽度、状态、标签、窗口。这里使用的选项是 `window`。
