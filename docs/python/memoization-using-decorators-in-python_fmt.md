...
> 原文：[https://www.geeksforgeeks.org/memoization-using-decorators-in-python/](https://www.geeksforgeeks.org/memoization-using-decorators-in-python/)
...
说明：
1. 已经定义了一个名为 `memoize_factorial` 的函数。它的主要目的是将中间结果存储在名为 `memory` 的变量中。
2. 第二个函数叫做 `facto` 是计算阶乘的函数。它已经被装饰器（函数 `memoize_factorial`）注释了。由于闭包的概念，`facto` 可以访问 `memory` 变量。注释相当于写作，

```py
facto = memoize_factorial(facto)
```

3. 当调用 `facto(5)` 时，除了存储中间结果之外，还会进行递归操作。每次需要进行计算时，都会检查结果是否存在于 `memory` 中。如果是，则使用，否则，计算值并存储在 `memory` 中。
