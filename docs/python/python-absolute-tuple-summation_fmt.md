...
> 原文: [https://www.geeksforgeeks.org/python-absolute-tuple-summation/](https://www.geeksforgeeks.org/python-absolute-tuple-summation/)
...
## 方法#1: 使用 `sum()` + 列表推导式 + `abs()`

以上功能的组合可以用来解决这个问题。在这种情况下，我们使用 `sum()`，`abs()` 为绝对值执行计算和的任务，并使用列表推导式来遍历列表和每个元组内部，使用生成器表达式进行迭代。
