...
> 原文: [https://www.geeksforgeeks.org/how-to-import-variables-from-another-file-in-python/](https://www.geeksforgeeks.org/how-to-import-variables-from-another-file-in-python/)

当代码行增加时，搜索所需的代码块是很麻烦的。根据工作情况区分代码行是一个很好的做法。这可以通过为不同的工作代码创建单独的文件来实现。我们知道，Python 中的各种库提供了各种方法和变量，我们使用简单的 `import <library_name>` 就可以访问这些方法和变量。比如数学库。如果我们想使用 `pi` 变量，我们使用 `import math`，然后是 `math.pi`。
...
*   `import <file_name>` 后使用 `<file_name>.<variable_name>` 访问变量
*   `from <file_name> import <variable_names>` 并使用变量
*   `from <file_name> import *` 然后直接使用变量。

**示例:**

假设我们有一个名为 `swaps.py` 的文件。我们必须将 `x` 和 `y` 变量从这个文件导入到另一个名为 `calval.py` 的文件中。
