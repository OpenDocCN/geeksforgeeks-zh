...
在 Jupyter 中，一切都在单元格中运行。它提供了将单元格类型更改为标记、文本、Python 控制台等的选项。在 Python IPython 控制台单元中，Jupyter 允许执行 Python 代码。
...
在 Jupyter 中，控制台命令可以由 `!` 在单元格内的命令前签名执行。例如，如果下面的代码写在 Jupyter 单元格中，它将作为命令在 CMD 中执行。
