...
布尔值只能有两种类型，即 `True` 或 `False`。输出 `<class 'bool'>` 指示变量是布尔数据类型。
...
布尔运算是真值和假值的简单算术。这些值可以通过使用布尔运算符进行操作，这些运算符包括 `or`、`and` 和 `not`。常见的布尔运算有：
...
在上面的例子中，我们使用了 `Python` `boolean` 和 `if` 语句以及 `OR` 运算符来检查 `a` 是否大于 `b` 或者 `b` 是否小于 `c`，如果任何条件为真（上面例子中的 `b < c`），则执行 `if` 块。
...
布尔 `not` 运算符只需要一个参数，并返回该参数的负数，即返回真表示假，返回假表示真。
...
这两个运算符都用来比较两个结果。`==` (等效运算符) 如果两个结果相等则返回 `True`。`!=` (非等价运算符) 如果两个结果不相同则返回 `True`。
...
[`is` 关键字](https://www.geeksforgeeks.org/is-keyword-in-python/) 用来测试两个变量是否属于同一个对象。如果两个对象相同，测试将返回真，否则即使两个对象 100% 相等，测试也将返回假。
...
[`in` 运算符检查成员资格](https://www.geeksforgeeks.org/python-in-keyword/)，即检查值是否存在于列表、元组、范围、字符串等中。
