...
> 原文: [https://www.geeksforgeeks.org/application-to-get-live-usd-inr-rate-using-python/](https://www.geeksforgeeks.org/application-to-get-live-usd-inr-rate-using-python/)

在本文中，我们将编写一个 python 脚本来获取美元/印度卢比汇率的实时信息，并将其与图形用户界面应用程序绑定。

## 所需模块

*   **bs4**: `BeautifulSoup` 是一个从 `HTML` 和 `XML` 文件中拉出数据的 Python 库。
...
*   **requests**: 这个模块可以让你非常轻松的发送 `HTTP/1.1` 请求。
...
*   借助 `requests` 和 `BeautifulSoup` 模块来抓取数据。
*   将数据转换成 `HTML` 代码。
*   找到所需的详细信息并进行筛选。
...
### 步骤 3: 现在将网址传入 `getdata()` 函数，并将该数据(货币明细)转换为 `HTML` 代码
