...
> 原文: [https://www.geeksforgeeks.org/given-two-arrays-find-nm-1-unique-sum-pairs/](https://www.geeksforgeeks.org/given-two-arrays-find-nm-1-unique-sum-pairs/)
...
我们可以在 `A` 和 `B` 两个数组上运行一个循环，找到前 `N+M-1` 个唯一的和对，并沿途打印它们的索引。为了检查某个特定的金额是否已经被使用，我们制作了一个 Python bool 字典。如果该和的字典值为假，则意味着它是一个新的和值。如果为真，则该总和已被使用。
...
**时间复杂度:** 由于我们使用嵌套循环遍历数组，时间复杂度变成了 `O(n^2)`。

## 高效方法

一旦我们对两个数组进行了排序，首先我们可以通过将数组 `A` 的所有元素与数组 `B` 的第一个元素配对，然后将数组 `B` 的其他 `M–1` 个元素与数组 `A` 的最后一个元素配对，从而形成 `N` 个唯一的和对。
