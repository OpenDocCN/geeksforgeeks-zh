...
Python `getopt` 模块类似于 c 的 [`getopt()`](https://www.geeksforgeeks.org/getopt-function-in-c-to-parse-command-line-arguments/) 函数，不同于 `sys` 模块，`getopt` 模块通过参数验证扩展了输入字符串的分离。它允许包括赋值在内的短选项和长选项。但是，该模块需要使用 `sys` 模块来正确处理输入数据。要使用 `getopt` 模块，需要从命令行参数列表中移除第一个元素。
