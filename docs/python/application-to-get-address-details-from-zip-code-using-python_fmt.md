...
> 原文：[https://www.geeksforgeeks.org/application-to-get-address-details-from-zip-code-using-python/](https://www.geeksforgeeks.org/application-to-get-address-details-from-zip-code-using-python/)
...
`tkinter` 模块是 Python 中的内置模块，但是我们需要安装 `geopy` 模块：
...
1.  导入 `geopy` 模块。
2.  使用 `Nominatim` API 访问一组坐标对应的，`Nominatim` 使用 OpenStreetMap 数据通过名称和地址（地理编码）查找地球上的位置。
3.  使用 `geocode()` 获取给定 `zipcode` 的位置并显示出来。
