...
> 原文: [https://www.geeksforgeeks.org/create-grouped-bar-chart-using-altair-in-python/](https://www.geeksforgeeks.org/create-grouped-bar-chart-using-altair-in-python/)
...
我们可以通过调用 `alt.Chart()` 方法来绘制分组图。我们希望我们的类别显示为列，因此我们可以在 `alt.Column()` 方法中传递 `Format`，我们的系列(玩家)将在 x 轴上表示，因此我们在 `alt.X()` 字段中通过 `Player`，我们的值(运行)应该显示在 y 轴上，因此我们通过了 `alt.Y()` 中的 `Highest Score` 字段。每个玩家在所有格式中都应该有相同的颜色，所以我们在 `alt.Color()` 方法中传递 `Player`。
