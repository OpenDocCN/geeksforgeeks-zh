...
> 原文：[https://www.geeksforgeeks.org/flipping-tiles-memory-game-using-python3/](https://www.geeksforgeeks.org/flipping-tiles-memory-game-using-python3/)
...
为了用 Python 模拟这个游戏，我们将使用`海龟`和`随机`模块。

## 实现步骤

1.  导入`海龟`和`随机`模块。Python 提供了可以生成随机数的`随机`模块，`海龟`模块被用于制作不同的对象。
