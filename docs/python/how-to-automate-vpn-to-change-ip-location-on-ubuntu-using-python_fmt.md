...
> 原文：[https://www.geeksforgeeks.org/how-to-automate-vpn-to-change-ip-location-on-ubuntu-using-python/](https://www.geeksforgeeks.org/how-to-automate-vpn-to-change-ip-location-on-ubuntu-using-python/)
...
### 第一步：
打开终端（`Ctrl+Alt+T`），通过在终端上键入以下命令，使用 `gedit` 创建文件。
...
### 步骤 3：
创建 windscribe (VPN) 提供的免费 VPN 服务器代码列表。
...
*   使用 `random` 模块从 `codeList` 中选择一个随机代码。

```py
choiceCode = random.choice(codeList)
```

*   创建 15 到 20 分钟的随机睡眠，之后使用 `time` 和 `random` 模块更改系统的 IP。
...
### 步骤 1：
使用下面给出的命令登录到您用来在系统上设置[虚拟专用网络的 Windscribe。](https://www.geeksforgeeks.org/how-to-setup-vpn-on-ubuntu-linux-system-for-ip-spoofing-using-windscribe/)
