...
> 原文：[https://www.geeksforgeeks.org/complex-numbers-python-set-2-important-functions-constants/](https://www.geeksforgeeks.org/complex-numbers-python-set-2-important-functions-constants/)
...
### 2. `log(x, b)`
这个函数返回 `x` 的**对数值，基数为 `b`**，两者都在它的参数中提到。如果未指定基数，则返回 `x` 的自然对数。
...
### 5. `isfinite()`
如果复数的实部和虚部都是**有限的**，则返回**真，否则返回假。**

### 6. `isinf()`
如果复数的实部或虚部为/为**无穷大**，则返回**真，否则返回假。**

### 7. `isnan()`
如果复数的**实部或虚部**为 **NaN**，则返回 true，否则返回 false。
