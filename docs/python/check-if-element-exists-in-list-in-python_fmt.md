...
> 原文: [https://www.geeksforgeeks.org/check-if-element-exists-in-list-in-python/](https://www.geeksforgeeks.org/check-if-element-exists-in-list-in-python/)
...
将列表转换为集合，然后在中使用 `in` 可能比只在列表中使用 `in` 更有效。但是有效率也有一定的负面影响。其中之一是列表的顺序没有被保留，如果你选择为它取一个新的列表，你将需要使用额外的空间。另一个缺点是 `set` 不允许重复，因此重复的元素将从原始列表中删除。

## 方法四: 使用 `sort()` + `bisect_left()`

测试元素存在的常规二分搜索法方法，因此列表必须首先排序，因此不保留元素顺序。`bisect_left()` 返回要查找的元素的第一次出现，其工作方式类似于 C++ STL 中的 `lower_bound()`。

> **注意:** 等分函数只会说明插入元素的位置，而不会说明元素是否存在的细节。

**代码#2 :** 演示如何使用 `set()` + `in` 和 `sort()` + `bisect_left()` 检查列表中元素的存在。
...
我们可以使用内置的 python List 方法 `count()`，来检查传递的元素是否存在于 List 中。如果传递的元素存在于列表中，`count()` 方法将显示它在整个列表中出现的次数。如果是非零正数，则表示列表中存在元素。
