...
Python 中的`pwd module`提供了对 Unix 用户帐户和密码数据库的访问。存储在 Unix 用户帐户和密码数据库中的每个条目被报告为一个类似元组的对象，其属性类似于`pwd.h`头文件中定义的`passwd`结构的成员。
...
Python 中的`pwd.getpwuid()`方法用于获取指定用户 id 的密码数据库条目。
...
>
> **返回类型:** 这个方法返回一个类`pwd.struct_passwd`的类似元组的对象，它代表指定用户 id 的密码数据库条目。
...
> **语法:** `pwd.getpwnam(name)`
>
> **参数:**
> **名称:** 表示需要输入密码数据库的用户名的字符串值。
>
> **返回类型:** 这个方法返回一个类`pwd.struct_passwd`的类似元组的对象，它代表指定名称的密码数据库条目。
...
Python 中的`pwd.getpwall()`方法用于获取密码数据库中存储的所有可用条目的列表。

> **语法:** `pwd.getpwall()`
>
> **参数:** 不需要参数。
>
> **返回类型:** 这个方法返回一个类`pwd.struct_passwd`的元组样对象的列表，其元素代表密码数据库中存储的所有可用条目。
...
pwd.struct_passwd(pw_name='saned', pw_passwd='x', pw_uid=109, pw_gid=115, pw_gecos='', pw_dir='/var/lib/saned', pw_shell='/usr/sbin/nologin')
pwd.struct_passwd(pw_name='pulse', pw_passwd='x', pw_uid=110, pw_gid=116, pw_gecos='PulseAudio daemon,,, ', pw_dir='/var/run/pulse', pw_shell='/usr/sbin/nologin')
pwd.struct_passwd(pw_name='avahi', pw_passwd='x', pw_uid=111, pw_gid=117, pw_gecos='Avahi mDNS daemon,,, ', pw_dir='/var/run/avahi-daemon', pw_shell='/usr/sbin/nologin')
pwd.struct_passwd(pw_name='colord', pw_passwd='x', pw_uid=112, pw_gid=118, pw_gecos='colord colour management daemon,,, ', pw_dir='/var/lib/colord', pw_shell='/usr/sbin/nologin')
pwd.struct_passwd(pw_name='hplip', pw_passwd='x', pw_uid=113, pw_gid=7, pw_gecos='HPLIP system user,,, ', pw_dir='/var/run/hplip', pw_shell='/bin/false')
pwd.struct_passwd(pw_name='geoclue', pw_passwd='x', pw_uid=114, pw_gid=119, pw_gecos='Geolocation daemon,,, ', pw_dir='/var/lib/geoclue', pw_shell='/usr/sbin/nologin')
pwd.struct_passwd(pw_name='gnome-initial-setup', pw_passwd='x', pw_uid=115, pw_gid=65534, pw_gecos='', pw_dir='/run/gnome-initial-setup', pw_shell='/bin/false')
pwd.struct_passwd(pw_name='gdm', pw_passwd='x', pw_uid=116, pw_gid=120, pw_gecos='Gnome Display Manager,,, ', pw_dir='/var/lib/gdm3', pw_shell='/bin/false')
pwd.struct_passwd(pw_name='johndoe', pw_passwd='x', pw_uid=1000, pw_gid=1000, pw_gecos='John Doe,,, ', pw_dir='/home/johndoe', pw_shell='/bin/bash')
```

## 系统用户信息示例

```
struct_passwd(pw_name=’avahi-autoipd’, pw_passwd=’x’, pw_uid=109, pw_gid=115, pw_gecos=’Avahi autoip daemon,,, ‘, pw_dir=’/var/lib/avahi-autoipd’, pw_shell=’/bin/false’)
