...
> 原文: [https://www.geeksforgeeks.org/convert-pdf-to-csv-using-python/](https://www.geeksforgeeks.org/convert-pdf-to-csv-using-python/)
...
这里将使用 `tabula-py` 模块将 PDF 文件转换为任何其他格式。`tabula-py` 是 `tabula-java` 的一个简单的 Python 包装器，可以读取 PDF 中的表格。你可以从 PDF 中读取表格，并将其转换成 `pandas` 数据帧。`tabula-py` 还使您能够将 PDF 文件转换为 CSV、TSV 或 JSON 文件。
...
*   然后我们将使用 `to_csv()` 方法将 PDF 文件转换成 CSV 文件。
