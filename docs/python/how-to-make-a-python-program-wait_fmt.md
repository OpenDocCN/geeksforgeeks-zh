...
> 原文: [https://www.geeksforgeeks.org/how-to-make-a-python-program-wait/](https://www.geeksforgeeks.org/how-to-make-a-python-program-wait/)
...
Python 有一个名为 `time` 的模块。这个模块给出了几个有用的功能来控制与时间相关的任务。`sleep()` 就是这样一个函数，它在给定的秒数内暂停调用线程的执行，并返回 void。该参数可以是一个浮点数，以指示更精确的睡眠时间。这是最常用的方法，因为它易于使用，并且独立于平台。实现如下:
