...
> 原文：[https://www.geeksforgeeks.org/creating-linear-kernel-svm-in-python/](https://www.geeksforgeeks.org/creating-linear-kernel-svm-in-python/)

## 先决条件

`SVM`

让我们使用 Python 的 `sklearn` 库和可以在 Python 的 `datasets` 库中找到的 `虹膜数据集` 来创建一个 `线性核 SVM`。
...
在上图中，有两组特征：“`蓝色`特征”和“`黄色`特征”。因为这些可以很容易地分离，或者换句话说，它们是线性可分离的，所以这里可以使用线性核。

## 使用线性核的优势

1.  用线性核训练 SVM 比用任何其他核都要快。
2.  当用线性核训练 SVM 时，只需要优化 `C` 正则化参数。另一方面，当使用其他内核进行训练时，需要优化 `γ` 参数，这意味着执行网格搜索通常需要更多时间。
