...
> 原文: [https://www.geeksforgeeks.org/how-to-add-a-header-to-a-csv-file-in-python/](https://www.geeksforgeeks.org/how-to-add-a-header-to-a-csv-file-in-python/)
...
使用 `DictWriter()` 的另一种方法可以用于向 `CSV` 文件的内容附加一个标题。`fieldnames` 属性可用于指定 `CSV` 文件的标题，`delimiter` 参数通过 `csv` 模块中给出的分隔符分隔值，需要添加标题。然后在 `csvwriter` 对象上调用 `writeheader()` 方法，不传递任何参数。
