...
> 原文: [https://www.geeksforgeeks.org/defining-dataframe-schema-with-structfield-and-structtype/](https://www.geeksforgeeks.org/defining-dataframe-schema-with-structfield-and-structtype/)

在本文中，我们将学习如何用结构字段和结构类型定义数据框架模式。

*   `StructType`和`StructField`用于定义数据框的架构或其部分。这定义了每个列的名称、数据类型和可空标志。
*   `StructType`对象是`StructField`对象的集合。它是一个内置数据类型，包含结构字段列表。
...
> *   `fields` – 结构字段列表。
> *   `name` – 列的名称。
> *   `dataType` – 数据类型，如`IntegerType`、`StringType`、`FloatType`等。
> *   `nullable` – 字段是否为空/None。
...
在上面的代码中，我们使`nullable`标志=`True`。使其为真的用途是，如果在创建数据框时任何字段值为空/None，则数据框也将以None值创建。
