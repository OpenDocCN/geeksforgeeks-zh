...
Katz centrality 通过测量直接邻居(一级节点)的数量以及网络中通过这些直接邻居连接到所考虑的节点的所有其他节点的数量来计算网络中节点的相对影响。然而，与远方邻居的连接会受到衰减因子 `alpha` 的影响。一对节点之间的每条路径或连接都被赋予一个由 `alpha` 确定的权重，节点之间的距离称为 `alpha^{d}`。

例如，在右图中，假设正在测量约翰的中心性，并且 `alpha = 0.5`。分配给连接约翰和他的近邻简和鲍勃的每个链接的权重将是 `(0.5)^{1} = 0.5`。由于何塞通过鲍勃间接连接到约翰，分配给这个连接(由两个链接组成)的权重将是 `(0.5)^{2} = 0.25`。同样，分配给阿格涅塔和约翰之间通过阿齐兹和简的连接的权重将是 `(0.5)^{3} = 0.125`，分配给阿格涅塔和约翰之间通过迭戈、何塞和鲍勃的连接的权重将是 `(0.5)^{4} = 0.0625`。
...
注意，上面的定义使用了邻接矩阵 `A` 的位置 `(i, j)` 处的元素上升到幂 `k`(即 `A^{k}`)反映了节点 `i` 和 `j` 之间的 `k` 度连接的总数。衰减因子 `alpha` 的值必须选择为小于邻接矩阵 `A` 的最大特征值的绝对值的倒数。在这种情况下，以下表达式可用于计算卡兹中心度:
