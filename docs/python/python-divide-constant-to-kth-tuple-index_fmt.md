...
> 原文：[https://www.geeksforgeeks.org/python-divide-constant-to-kth-tuple-index/](https://www.geeksforgeeks.org/python-divide-constant-to-kth-tuple-index/)

很多时候，在处理记录时，我们会遇到一个问题，那就是我们需要更改元组元素的值。这是使用元组时常见的问题。让我们讨论用列表中元组的 Kth 元素划分 N 的某些方法。

## 方法#1：使用循环

使用循环可以执行该任务。在这种情况下，我们只需迭代列表，通过代码中的预定义值 `N` 来更改 `K`th 元素。
