# 使用 difflib 模块比较 Python 中的序列

> 原文：[https://www.geeksforgeeks.org/compare-sequences-in-python-using-dfflib-module/](https://www.geeksforgeeks.org/compare-sequences-in-python-using-dfflib-module/)

`difflib` Python 模块包含各种评估序列比较的特性，可以用来比较文件，可以创建不同格式的文件变体信息，包括 HTML 和上下文以及统一的 diffs。
...
*   这个类的 `get_matching_blocks()` 方法返回描述匹配子序列的三元组列表。每个三元组都是 `(i, j, n)` 的形式，意味着 `a[i:i+n] == b[j:j+n]`。
...
*   `get_close_matches()` 方法：此方法返回最佳字符或字符组匹配列。`word` 是一个序列，其中需要紧密的相似性（通常是一个字符串），`possibilities` 是一组匹配术语的序列（大部分是字符串列表）。
...
此类用于匹配文本行形式的序列，并创建人类可读的变体或增量。`Differ` 增量的每一行都以两个字母的代码开始：
...
*   这个类中的 `compare()` 方法，比较两个序列的行，并生成 delta（一个序列的行）。
