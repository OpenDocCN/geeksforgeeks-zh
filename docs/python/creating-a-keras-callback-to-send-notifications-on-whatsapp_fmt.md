...
原文: [https://www.geeksforgeeks.org/creating-a-keras-callback-to-send-notifications-on-whatsapp/](https://www.geeksforgeeks.org/creating-a-keras-callback-to-send-notifications-on-whatsapp/)
...
**第四步。** 用 Python 设置 Twilio 客户端，发送第一条消息。打开一个代码编辑器，将下面的代码粘贴到 python 文件中。在下面的代码片段中，`account_sid`和`auth_token`是从控制台获得的令牌，如步骤 3 所示。`Your_whatsapp_number`是您想要接收文本通知的号码。最后，`From_number`是您在步骤 2 中为 WhatsApp 设置 Twilio 沙盒时遇到的电话号码。注意-在数字中包括国家代码。例如，如果您的电话号码是 112233445，国家代码是+91，您必须在`Your_whatsapp_number`字段中输入+91112233445。
...
TensorFlow 团队在回调类中命名方法方面做得很好，这使得理解每个函数打算做什么/能够做什么变得非常简单。使用的命名约定以`on_{train/test/batch/epoch}_{begin/end}`的形式命名每个函数。

对于我们的任务，我们想知道模型何时停止训练。该信息可以从函数`on_train_end()`获得。因此，我们可以创建一个类`WhatsappCallBack`，它扩展了 Keras 的回调类。在这个类中，我们将重载`on_train_end()`函数，以便了解所需的信息。
...
**第一步。** 我们使用 sklearn 库的[`make_classification`方法](https://scikit-learn.org/stable/modules/generated/sklearn.datasets.make_classification.html)生成 1000 个随机样本数据点和一个非常简单的 ANN。
...
**第二步。** 我们创建一个`WhatsappCallBack`类的实例，并在回调参数中传递这个实例，同时拟合我们的模型。
...
以下存储库中的整个工作代码–
[https://github.com/Raman1121/ML-Practice/blob/master/CustomCallback.ipynb](https://github.com/Raman1121/ML-Practice/blob/master/CustomCallback.ipynb)

参考文献:

1.  [https://keunwoochoi.wordpress.com/2016/07/16/keras-callbacks/](https://keunwoochoi.wordpress.com/2016/07/16/keras-callbacks/)
2.  [https://www.twilio.com/docs/whatsapp/tutorial/send-whatsapp-notification-messages-templates](https://www.twilio.com/docs/whatsapp/tutorial/send-whatsapp-notification-messages-templates)