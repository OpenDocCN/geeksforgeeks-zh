# Python 中的 `focus_set()` 和 `focus_get()` 方法

> 原文: [https://www.geeksforgeeks.org/python-focus_set-and-focus_get-method/](https://www.geeksforgeeks.org/python-focus_set-and-focus_get-method/)

Tkinter 有许多小部件可以在任何图形用户界面中提供功能。它还支持各种通用的小部件方法，这些方法可以应用于任何小部件。
`focus_get()` 和 `focus_set()` 方法也是通用的 widget 方法。它们也可以应用于 `Tk()` 类。
