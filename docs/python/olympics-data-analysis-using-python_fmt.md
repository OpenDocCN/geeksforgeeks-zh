...
> 原文: [https://www.geeksforgeeks.org/olympics-data-analysis-using-python/](https://www.geeksforgeeks.org/olympics-data-analysis-using-python/)
...
*   `Matplotlib`: 是数值数学扩展 `NumPy`
*   `seaborn`: 用于 Python 中可视化统计图形绘制
...
我们使用 `pandas.read_csv()` 方法读入数据框，并显示每个数据集的前 5 行。
...
这里我们将使用 Python 中的 `pandas.merge()` 合并两个数据帧。
...
现在可以使用 `pandas` 和 `matplotlib` 获得数据，让我们看看一些例子
...
在这里，我们将创建一个关于年龄的金牌数的图表。为此，我们将创建 `countplot` 用于图形表示，其中 X 轴表示运动员的年龄，Y 轴表示奖牌数量。
