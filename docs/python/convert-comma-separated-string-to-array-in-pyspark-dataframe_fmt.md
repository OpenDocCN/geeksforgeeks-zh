...
> 原文：[https://www.geeksforgeeks.org/convert-comma-separated-string-to-array-in-pyspark-dataframe/](https://www.geeksforgeeks.org/convert-comma-separated-string-to-array-in-pyspark-dataframe/)

在本文中，我们将学习如何在 pyspark dataframe 中将逗号分隔的字符串转换为数组。

在 pyspark SQL 中，`split()`函数将分隔符分隔的字符串转换为数组。这是通过基于分隔符（如空格、逗号）拆分字符串，并将它们堆叠成一个数组来实现的。此函数返回数组类型的`pyspark.sql.Column`。
