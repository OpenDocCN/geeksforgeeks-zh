...
> 原文：[https://www.geeksforgeeks.org/graph-plotting-in-python-set-1/](https://www.geeksforgeeks.org/graph-plotting-in-python-set-1/)
...
*   将 x 轴和相应的 y 轴值定义为列表。
*   使用 `plt.plot()` 功能在画布上绘制它们。
*   使用 `plt.xlabel()` 和 `plt.ylabel()` 功能给 x 轴和 y 轴命名。
*   使用 `plt.title()` 功能给你的剧情一个标题。
*   最后，要查看你的剧情，我们用 `plt.show()` 功能。
...
*   在这里，我们在同一张图上画两条直线。我们通过给它们一个名称（`label`）来区分它们，该名称作为 `plt.plot()` 函数的参数传递。
*   给出线条类型和颜色信息的小矩形框称为图例。我们可以使用 `plt.legend()` 功能为我们的剧情添加一个传说。
...
*   下一步是“`bin`”数值范围——即将整个数值范围划分为一系列区间——然后统计每个区间内有多少数值。这里我们定义了 `bins` = 10。所以，总共有 100/10 = 10 个区间。
...
*   首先，我们使用名为 `activities` 的列表来定义 `labels`。
*   然后，可以使用另一个名为 `slices` 的列表来定义每个标签的一部分。
*   每个标签的颜色使用名为 `colors` 的列表来定义。
*   `shadow=True` 将在饼图中的每个标签下显示一个阴影。
*   `startangle` 从 x 轴逆时针旋转饼图的起点给定角度。
*   `explode` 用于设置我们偏移每个楔形的半径分数。
*   `autopct` 用于格式化每个标签的值。这里，我们将它设置为只显示小数点后 1 位的百分比值。
