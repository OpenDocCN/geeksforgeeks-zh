...
> 原文：[https://www.geeksforgeeks.org/automated-software-testing-with-python/](https://www.geeksforgeeks.org/automated-software-testing-with-python/)
...
我们在 Python 模块（`.py`）中编写测试。为了运行我们的测试，我们只需使用任何 IDE 或终端执行测试模块。

现在，让我们使用`unittest`模块为上面讨论的小软件编写一些测试。

1.  在名为“tests”的文件夹中创建一个名为`tests.py`的文件。
2.  在`tests.py`导入`unittest`。
3.  创建一个名为`TestClass`的类，该类继承自`unittest.TestCase`类。
