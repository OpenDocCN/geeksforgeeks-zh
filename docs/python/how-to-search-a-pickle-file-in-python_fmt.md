...
> 原文: [https://www.geeksforgeeks.org/how-to-search-a-pickle-file-in-python/](https://www.geeksforgeeks.org/how-to-search-a-pickle-file-in-python/)

## 先决条件

[泡菜档](https://www.geeksforgeeks.org/understanding-python-pickling-example/)

Python `pickle` 模块用于序列化和反序列化 Python 对象结构。Python 中的任何对象都可以被腌制，以便保存在磁盘上。`pickle` 所做的是在将对象写入文件之前先“序列化”对象。酸洗是一种转换 python 对象（列表、字典等）转换成字符流的方法。想法是这个字符流包含在另一个 python 脚本中重建对象所需的所有信息。
