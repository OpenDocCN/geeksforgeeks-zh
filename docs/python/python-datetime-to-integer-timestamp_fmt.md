...
> 原文: [https://www.geeksforgeeks.org/python-datetime-to-integer-timestamp/](https://www.geeksforgeeks.org/python-datetime-to-integer-timestamp/)

在本文中，我们将看到如何将 Python `DateTime` 转换为整数时间戳。

`timestamp()` 函数返回自 1970 年 1 月 1 日以来经过的秒数。那个零时刻被称为纪元。首先，我们将获得当前时间，或者明确提到我们想要时间戳的所需日期和时间。有几种方法可以得到日期和时间。我们将在浏览示例时看到它们。然后我们使用 `timestamp()` 函数将日期时间转换为时间戳。

最后，我们将以秒和毫秒为单位舍入时间戳，并显式地将其类型转换为整数数据类型，我们的工作就完成了！

## 例 1: 当前日期时间的整数时间戳

在这里，我们导入 `datetime` 模块以使用其中的 `datetime` 函数。然后使用 `datetime.now()` 函数获取当前日期和时间。使用 `DateTime.timestamp()` 方法将 `DateTime` 对象转换为时间戳。我们将以秒为单位获取时间戳。然后舍入时间戳，并显式地将浮点数类型转换为整数，以秒为单位获得整数时间戳。
...
首先，我们在 `datetime.datetime()` 对象中输入 UTC 时间。然后我们将对象传递给 `d.timetuple()` 函数，该函数给出一个包含年、月、日等参数的 `tuple`，然后使用 `calendar` 函数将 `datetime` 转换为整数 UTC 时间戳。
