...
`numpy.nanmax(arr, axis=None, out=None, keepdims=<no value>)`

## 参数

- `arr`：输入数组。
- `axis`：我们想要最大值的轴。否则，将认为 `arr` 被展平（在所有轴上工作）。`axis=0` 表示沿列工作，`axis=1` 表示沿行工作。
- `out`：不同的数组中我们要放置的结果。数组必须具有与预期输出相同的维度。
- `keepdims`：如果设置为 `True`，减少的轴将作为尺寸为 1 的尺寸留在结果中。使用此选项，结果将根据原始 `a` 正确广播。

## 返回值

最大数组值（如果 `axis` 为 `None`，则为标量值）或沿指定轴具有最大值的数组。
