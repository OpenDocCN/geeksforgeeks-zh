...
> 原文：[https://www.geeksforgeeks.org/python-find-min-max-in-heterogeneous-list/](https://www.geeksforgeeks.org/python-find-min-max-in-heterogeneous-list/)
...
这个特殊的问题可以通过使用 `isinstance` 方法过滤我们对 `min`/`max` 的搜索来解决，我们可以过滤掉整数值，然后可以使用 `min`/`max` 函数来获得所需的 `min`/`max` 值。
...
上面的问题也可以通过使用 `lambda` 函数作为 `min()`/`max()` 中的键以及执行检查整数值任务的 `isinstance` 方法来解决。
