...
`网页抓取`是一种帮助从网站提取大量数据的机制，通过这种机制，数据被提取并保存到计算机的本地文件或表格(电子表格)格式的数据库中。
...
`BeautifulSoup` :它是一个不可思议的从网页中拉出信息的工具。您可以使用它来提取表格、列表、段落，还可以放置过滤器来从网页中提取信息。查看[BeautifulSoup文档页面](https://www.crummy.com/software/BeautifulSoup/bs4/doc/)。

`BeautifulSoup` 不为我们取网页。我们可以使用 `urllib2` 和 `BeautifulSoup` 库。
