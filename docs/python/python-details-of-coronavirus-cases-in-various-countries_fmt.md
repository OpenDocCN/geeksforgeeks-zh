...
> 原文: [https://www.geeksforgeeks.org/python-details-of-coronavirus-cases-in-various-countries/](https://www.geeksforgeeks.org/python-details-of-coronavirus-cases-in-various-countries/)
...
### 美人汤
`BeautifulSoup` 是一个库，可以方便的从网页上刮取信息。它位于 HTML 或 XML 解析器之上，为迭代、搜索和修改解析树提供了 Pythonic 习惯用法。
...
> 为了制作这个剧本我们要做以下几点:
> 1. 从国家名称创建网址
> 2. 借助 `requests` 和 `BeautifulSoup`
> 3. 刮取数据。将数据转换成 html 代码。
> 4. 找到所需的详细信息并进行筛选。
> 5. 将结果保存在字典中。
