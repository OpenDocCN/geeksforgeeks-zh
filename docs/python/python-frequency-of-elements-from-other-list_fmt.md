...
> 原文: [https://www.geeksforgeeks.org/python-frequency-of-elements-from-other-list/](https://www.geeksforgeeks.org/python-frequency-of-elements-from-other-list/)
...
## 方法二：使用 `Counter()` + `setdefault()` + 循环

这是执行该任务的另一种方式。在本文中，我们使用 `Counter()` 计算频率，并使用循环赋值。对于不存在的值，`setdefault()` 用于将计数器初始化为 0。
