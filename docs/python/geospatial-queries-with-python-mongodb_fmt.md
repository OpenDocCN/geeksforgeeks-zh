...
> 原文: [https://www.geeksforgeeks.org/geospatial-queries-with-python-mongodb/](https://www.geeksforgeeks.org/geospatial-queries-with-python-mongodb/)
...
*   一个名为 `coordinates` 的字段，用于指定对象的坐标。

**注意:** 如果指定经纬度坐标，先列出经度再列出纬度。

## 所需模块

*   **[pymongo](https://www.geeksforgeeks.org/mongodb-and-python/):** 该模块用于与 MongoDB 交互。要安装它，请在终端中键入以下命令。
...
*   **Basemap:** 该模块用于使用 Python 绘制地图。要安装此模块，请在终端中键入以下命令。
...
*   然后单击连接到应用程序按钮。
*   复制 `cluster_uri` 并将其粘贴到一个名为 `course_cluster_uri` 的变量中。
