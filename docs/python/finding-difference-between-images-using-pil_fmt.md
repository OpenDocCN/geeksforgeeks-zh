...
> 原文: [https://www.geeksforgeeks.org/finding-difference-between-images-using-pil/](https://www.geeksforgeeks.org/finding-difference-between-images-using-pil/)

Python 解释器本身并不具备处理图像并得出结论的能力。所以，`PIL` (Python 图像库) 给解释器增加了图像处理能力。`PIL` 是一个开源库，它为 Python 处理图像及其表示提供了外部文件支持和效率。基本上，`PIL` 被设计成以图像(像素)的形式访问数据，以使分析更快。
