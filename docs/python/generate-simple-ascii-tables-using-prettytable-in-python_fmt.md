...
> 原文: [https://www.geeksforgeeks.org/generate-simple-ascii-tables-using-prettytable-in-python/](https://www.geeksforgeeks.org/generate-simple-ascii-tables-using-prettytable-in-python/)
...
为了能够使用 `prettytable` 库，我们首先需要使用 `pip` 工具命令安装它：
...
为此，您可以首先使用 `field_names` 属性设置字段名，然后使用 `add_row` 方法一次添加一行。
...
为此，您可以使用 `add_column` 方法，该方法接受两个参数——一个字符串，它是您要添加的列对应的字段的名称，以及一个包含列数据的列表或元组。
