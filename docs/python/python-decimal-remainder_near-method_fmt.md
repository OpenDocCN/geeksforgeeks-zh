# Python | Decimal `remainder_near()` 方法

> 原文: [https://www.geeksforgeeks.org/python-decimal-remainder_near-method/](https://www.geeksforgeeks.org/python-decimal-remainder_near-method/)

`Decimal` 的 `remainder_near()` 方法是一个 `Decimal` 类方法，返回 `x - y * n`，其中 `n` 是最接近 `x / y` 精确值的整数。
...
## 返回
`x - y * n`，其中 `n` 是最接近 `x / y` 精确值的整数。

## 代码示例 1：`remainder_near()` 方法示例
