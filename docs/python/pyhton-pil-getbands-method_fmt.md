...
PIL is the Python Imaging Library which provides the python interpreter with image editing capabilities. `PIL.Image.getbands()` method returns a tuple containing the name of each band in the image. For example, `getbands` on an RGB image returns `("R", "G", "B")`.
