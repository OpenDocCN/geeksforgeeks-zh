...
> 原文: [https://www.geeksforgeeks.org/face-comparision-using-face-and-python/](https://www.geeksforgeeks.org/face-comparision-using-face-and-python/)
...
这个 app 主要用于一个类似“给你送一些机密文件，快递员小子先验证你的脸再送快递员”这样的人脸验证流程。

**注意:** 一个 `facepp` API 只允许比较两张照片的图片 URL 链接。

**使用 `postimages.org` 将图像转换为网址。**

在本网站中，通过点击“选择图片”按钮从本地驱动器中选择您的照片，然后本网站将在处理您的照片后创建不同的网址链接。(见下图)
