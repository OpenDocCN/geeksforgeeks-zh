...
> 原文: [https://www.geeksforgeeks.org/how-to-convert-string-to-integer-in-python/](https://www.geeksforgeeks.org/how-to-convert-string-to-integer-in-python/)
...
*   类似地，我们使用 `ord()` 函数。`ord('0')` 将返回 48，`ord('1')` 返回 49，以此类推。
*   这里逻辑是 `ord('1') - ord('0') = 1`，`ord('2') - ord('0') = 2`，依此类推，它给出了要从给定输入数字中提取的有效数字。
