...
> 原文：[https://www.geeksforgeeks.org/python-common-words-among-tuple-strings/](https://www.geeksforgeeks.org/python-common-words-among-tuple-strings/)

有时，在处理元组时，我们会遇到一个问题，即我们需要找到以字符串为元素的单元组内字符串中出现的单词的交集。让我们讨论一下解决这个问题的某些方法。

**方法#1：使用 `join()` + `set()` + `&` 运算符 + `split()`**
