...
> 原文：[https://www.geeksforgeeks.org/python-intersection-two-lists/](https://www.geeksforgeeks.org/python-intersection-two-lists/)
...
在这个方法中我们`set()`较大的列表，然后使用内置的函数`intersection()`计算相交列表。`intersection()`是集合的一级部分。
...
通过使用这个混合方法程序的复杂度下降到 O(n)。这是执行以下程序的有效方法。
...
这是在其他列表内部的子列表上执行交集的地方。这里我们使用了`filter()`的概念。
