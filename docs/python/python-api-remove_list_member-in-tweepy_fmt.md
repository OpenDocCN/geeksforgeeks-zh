...
> 原文: [https://www.geeksforgeeks.org/python-api-remove_list_member-in-tweepy/](https://www.geeksforgeeks.org/python-api-remove_list_member-in-tweepy/)
...
> *   `列表_id` : 列表的 id。
> *   `slug`: 列表中的 slug。
> *   `user_id`: 要从列表中删除的用户的标识。
> *   `screen_name`: 要从列表中删除的用户的屏幕名称。
> *   `owner_id`: 列表所有者的 id。
...
```py
Number of members before remove_list_member() is used : 1
