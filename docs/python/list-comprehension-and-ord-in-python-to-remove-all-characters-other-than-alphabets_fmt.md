# 在 Python 中使用列表理解和 ord() 删除除字母以外的所有字符

> 原文：[https://www.geeksforgeeks.org/list-comprehension-and-ord-in-python-to-remove-all-characters-other-than-alphabets/](https://www.geeksforgeeks.org/list-comprehension-and-ord-in-python-to-remove-all-characters-other-than-alphabets/)
...
此问题已有解决方案，请参考[从字符串中删除除字母以外的所有字符](https://www.geeksforgeeks.org/remove-characters-alphabets-string/)链接。
...
## `ord()` 和 `range()` 函数在 Python 中是如何工作的？

*   `ord()` 方法返回一个整数，代表给定 Unicode 字符的 Unicode 代码点。例如：

    ```py
    ord('5') = 53 and ord('A') = 65 and ord('$') = 36
    ```

*   `range(a, b, step)` 函数生成一个元素列表，其范围从 `a`（含）到 `b`（不含），给定步长递增/递减。
...
def removeAll(input):
    ...
    sepChars = [char for char in input if
                ord(char) in range(ord('a'), ord('z')+1, 1) or 
