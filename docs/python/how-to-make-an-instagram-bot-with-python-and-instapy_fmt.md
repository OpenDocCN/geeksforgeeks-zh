...
`Instabot` 库是 Instagram 的一个推广脚本和 API Python 包装器。
...
要取消关注，许多人会创建一个取消关注列表，然后使用 `unfollow_users` 功能。
...
在这里，我们将使用 `unfollow_everyone()` 函数取消每个人在我们帐户中的关注。
...
要像发送一样发送，请列出用户，然后使用 `send_like` 功能。机器人根据聊天中的列表向朋友发送喜欢的内容。
