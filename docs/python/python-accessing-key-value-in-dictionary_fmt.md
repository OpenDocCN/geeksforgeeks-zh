...
> 原文: [https://www.geeksforgeeks.org/python-accessing-key-value-in-dictionary/](https://www.geeksforgeeks.org/python-accessing-key-value-in-dictionary/)
...
`items()` 方法遍历字典中所有的键值对，帮助我们在循环中一个接一个地访问 `key-value` 对，也是一个很好的带值访问字典键的方法。
...
Python 还提供了 `enumerate()` 函数来帮助迭代各种容器，无论是字典还是列表。这个功能的力量也可以用来执行这个任务。它还有助于访问字典中该对位置的命名索引。
