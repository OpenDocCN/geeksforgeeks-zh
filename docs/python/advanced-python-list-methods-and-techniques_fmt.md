...
> 原文：[https://www.geeksforgeeks.org/advanced-python-list-methods-and-techniques/](https://www.geeksforgeeks.org/advanced-python-list-methods-and-techniques/)
...
要按升序或降序对列表进行排序，我们使用具有以下语法的 `sort()` 函数：
...
要反转列表的顺序，我们使用 `reverse()` 功能。它的语法是：
...
因为 python 中的字典不包含重复的键。我们使用 `dict.fromkeys()` 函数将我们的列表转换为以列表元素为关键字的字典。然后我们把字典转换回列表。
...
Python 列表可以借助 `filter()` 功能或者借助列表理解进行过滤。以下是语法：
...
我们甚至可以借助 `zip()` 函数将列表组合起来，从而得到一个元组列表。这里，来自列表 A 的每个项目以元组的形式与来自列表 B 的相应元素组合。以下是语法：
...
为了找到最频繁的元素，我们使用 `set()` 功能。`set()` 函数从列表中删除所有重复的元素，而 `max()` 函数返回最频繁的元素（在‘键’的帮助下找到）。该键是可选的单参数函数。以下是语法：
