...
在本文中，我们将看到如何使用 python 在 `Pycairo` 中获取 `SVG` 单元。`SVG` 单位用于描述 `SVG` 规范中坐标和长度的有效单位。
...
> 以下是开罗的 `SVG` 单位
> 1.  `user` 用户单位，当前坐标系中的一个值。如果在初始坐标系的根元素中使用，它对应于像素
> 2.  `em` 元素字体的大小
> 3.  `ex` 元素字体的 `x` 高度
> 4.  `px` 像素(1px = 1/96 <sup>第</sup>个，共 1 英寸)
> 5.  `in` 英寸(1 英寸= 2.54 厘米= 96 像素)
> 6.  `cm` 厘米(1cm = 96px/2.54)
> 7.  `mm` 毫米(1 毫米= 1/10 <sup>分之一</sup>厘米)
> 8.  `pt` 点(1pt = 1/72 <sup>第</sup>个，共 1 英寸)
> 9.  `pc` Picas (1pc = 1/6 <sup>第</sup>位，共 1 英寸)
> 10. `percentage` 百分比，是另一个参考值的分数。
