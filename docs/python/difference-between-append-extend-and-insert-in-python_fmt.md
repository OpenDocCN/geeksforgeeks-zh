...
> 原文：[https://www.geeksforgeeks.org/difference-between-append-extend-and-insert-in-python/](https://www.geeksforgeeks.org/difference-between-append-extend-and-insert-in-python/)

列表就像动态大小的数组，用其他语言声明（C++中的 `vector` 和 Java 中的 `ArrayList`）。列表不必总是相同的，这使得它成为 [Python](https://www.geeksforgeeks.org/python-programming-language/) 中最强大的工具。单个列表可能包含整数、字符串和对象等数据类型。
...
| 时间复杂度为 O(1) | 具有 O(n) 的线性复杂度。 | 时间复杂度为 O(k)，其中 k 是可迭代的长度。 |
