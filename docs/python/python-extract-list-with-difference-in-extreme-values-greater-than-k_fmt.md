...
> 原文: [https://www.geeksforgeeks.org/python-extract-list-with-difference-in-extreme-values-greater-than-k/](https://www.geeksforgeeks.org/python-extract-list-with-difference-in-extreme-values-greater-than-k/)
...
在这种情况下，我们使用列表理解执行迭代任务，使用比较运算符执行检查任务。使用`max()`和`min()`计算值。
...
## 方法二: 使用`filter()` + `lambda` + `min()` + `max()`

在本例中，我们使用`filter()`和`lambda`执行过滤任务，`min()`和`max()`用于获取极值差。
