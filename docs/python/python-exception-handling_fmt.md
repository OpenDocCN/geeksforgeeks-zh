...
在本文中，我们将讨论如何使用 `try` 在 Python 中处理异常。`抓住`，`最后`借助恰当的例子陈述。
...
**注意：**`异常`是 Python 中所有异常的基类。您可以在这里查看异常层次结构[。](https://docs.python.org/2/library/exceptions.html#exception-hierarchy)
...
Python 最后提供了一个关键字 `finally`，总是在 `try` 和 `except` 块之后执行。`最终`块总是在正常终止 `尝试`块之后执行，或者在 `尝试`块由于某种异常而终止之后执行。
...
[`raise` 语句](https://www.geeksforgeeks.org/python-raising-an-exception-to-another-exception/)允许程序员强制发生特定的异常。`raise` 中的唯一参数指示要引发的异常。这必须是异常实例或异常类（从 `异常`派生的类）。
