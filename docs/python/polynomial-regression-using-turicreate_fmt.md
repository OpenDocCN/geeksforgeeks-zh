...
> 原文: [https://www.geeksforgeeks.org/polynomial-regression-using-turicreate/](https://www.geeksforgeeks.org/polynomial-regression-using-turicreate/)

在本文中，我们将讨论使用[图创建](https://www.geeksforgeeks.org/turicreate-library-in-python/)实现[多项式回归](https://www.geeksforgeeks.org/python-implementation-of-polynomial-regression/)。

`多项式回归`：多项式回归是回归分析的一种形式，将因变量如`y`和自变量如`x`之间的关系建模为`n`次多项式。它表示为：
...
## 步骤 1：导入重要的库，并使用图创建中的`SArray`和`SFrame`生成一个非常小的数据集，我们将使用它来执行多项式回归。
...
## 步骤 3：创建一个包含输入、多项式次数和输出的`SFrame`，以便拟合我们的回归模型。
