...
> 原文：[https://www.geeksforgeeks.org/how-to-bind-multiple-commands-to-tkinter-button/](https://www.geeksforgeeks.org/how-to-bind-multiple-commands-to-tkinter-button/)

Tkinter 中的按钮小部件提供了一种与应用程序交互的方式。用户按下一个按钮来执行附加到该按钮的某些操作。一般来说，我们为一个按钮提供一个操作，但是如果用户想要为一个按钮附加多个操作，该怎么办。

在本文中，我们将看到如何将多个操作/命令绑定到一个按钮。

要在 Tkinter 中创建按钮，请遵循以下语法。

> **语法：**`Button(master, text="Button", command=function, options, ...)`
>
> **参数：**
>
> *   `master`：指放置按钮的顶层窗口
...
让我们看看下面的例子，我们实际上有参数传递到 `fun1` 和 `fun2`。
