...
> 原文：[https://www.geeksforgeeks.org/handling-nameerror-exception-in-python/](https://www.geeksforgeeks.org/handling-nameerror-exception-in-python/)

**先决条件：** [Python 异常处理](https://www.geeksforgeeks.org/python-exception-handling/)

Python 中有几个标准的例外，`NameError` 就是其中之一。名称错误在被访问的标识符未在本地或全局范围内定义时引发。引发名称错误的一般原因是：
...
在下面的示例程序中，变量 `geek` 是在赋值函数 `assign()` 的局部范围内定义的。因此，它不能被全局访问。这会引发名称错误。
