...
> 原文: [https://www.geeksforgeeks.org/introduction-to-chempy-in-python/](https://www.geeksforgeeks.org/introduction-to-chempy-in-python/)
...
可以通过在命令提示符/终端中运行以下脚本来安装 `ChemPy`:
...
### 示例 2：在 ChemPy 中表示化学反应

我们来看看如何在 `ChemPy` 中表示化学反应。考虑水的形成。在反应中，两个 `H2` 分子与一个 `O2` 分子结合形成两个 `H2O` 分子。在 `ChemPy` 中，将使用 `chempy.chemistry` 模块的 `Reaction()` 函数创建反应。
