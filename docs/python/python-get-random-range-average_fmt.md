...
> 原文: [https://www.geeksforgeeks.org/python-get-random-range-average/](https://www.geeksforgeeks.org/python-get-random-range-average/)
...
以上功能的组合可以解决这个问题。在本文中，我们使用 `sum()` 来计算总和，使用 `uniform()` 来执行提取数字的任务，并且使用生成器表达式将整个逻辑封装在一行中。
