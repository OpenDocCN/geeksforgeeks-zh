...
> 原文：[https://www.geeksforgeeks.org/ann-implementation-of-self-organizing-neural-network-sonn-from-scratch/](https://www.geeksforgeeks.org/ann-implementation-of-self-organizing-neural-network-sonn-from-scratch/)

## 先决条件
[ANN | 自组织神经网络 (SONN) 学习算法](https://www.geeksforgeeks.org/ann-self-organizing-neural-network-sonn-learning-algorithm)

为了实现 `SONN`，这里有一些基本的考虑：

1.  构建一个自组织神经网络 (`SONN`)，或将 **100 个神经元** 分成 **10 行** 和 **10 列**。
2.  `Kohonen` 网络排列在二维矩阵中，使用 **1500** 个在 **-1 和 +1** 之间的向量。
3.  二维输入向量训练网络在区间内随机生成，初始突触权重在相同区间内随机选择为 **-1 和 +1**。
4.  二维输入向量的学习率被分配，使得网络中的每个神经元只响应出现在其区域内的输入向量。
