...
> 原文: [https://www.geeksforgeeks.org/lambda-with-if-but-without-else-in-python/](https://www.geeksforgeeks.org/lambda-with-if-but-without-else-in-python/)
...
由于 `lambda` 函数必须对每个有效输入都有一个返回值，我们不能用 `if` 来定义它，但不能用 `else` 来定义，因为我们没有指定如果 `if` 条件为假，即其 `else` 部分，我们将返回什么。
...
为了纠正它，我们需要指定如果 `if` 条件为假将返回什么，即我们必须指定它的 `else` 部分。
...
第一个代码是 `if` 但是没有 `else`，那么第二个代码是 `if-else`。
...
第一个代码有 `if` 但没有 `else`，然后第二个代码有 `if-else`。
