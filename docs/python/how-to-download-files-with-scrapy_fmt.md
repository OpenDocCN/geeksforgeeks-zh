...
> 原文: [https://www.geeksforgeeks.org/how-to-download-files-with-scrapy/](https://www.geeksforgeeks.org/how-to-download-files-with-scrapy/)

Scrapy 是一个快速的高级网页抓取和网页抓取框架，用于抓取网站并从其页面中提取结构化数据。它可以用于广泛的目的，从数据挖掘到监控和自动化测试。在本教程中，我们将探索如何使用 Scrapy 的爬行蜘蛛下载文件。
...
```py
scrapy genspider -t crawl NIRsoft www.nirsoft.net # NIRsoft 是蜘蛛的名字，www.nirsoft.com 是我们将要爬行的网站 (域名)
```
...
`.downloadline` 显示所有的下载链接都是类名“downloadline”下的锚标签
...
因为我们的目标是下载实用程序的安装文件，所以最好将爬虫程序限制为只下载 `.zip`、`.exe` 文件，剩下的就不用了。这也将减少爬网时间，从而使脚本更加高效。
...
我们会找到所有的 `.zip`、`.exe` 文件下载到指定的目标文件夹。然而，我们还有一个问题:
