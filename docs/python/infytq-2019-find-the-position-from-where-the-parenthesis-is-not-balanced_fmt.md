...
> 原文: [https://www.geeksforgeeks.org/infytq-2019-find-the-position-from-where-the-parenthesis-is-not-balanced/](https://www.geeksforgeeks.org/infytq-2019-find-the-position-from-where-the-parenthesis-is-not-balanced/)

给定一个由 `"("`, `")"`, `"{"`, `"}"`, `"["`, `"]"` 的括号组成的字符串 `str`。
如果 `str` 完全平衡，返回 `0`，否则返回发现嵌套错误的索引（从 `1` 开始）。
...
1.  首先，我们创建 `lst1` 和 `lst2`，它们将分别存储左括号和右括号。
        ...
        for(int i = 0; i < String.size(); i++)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // them with their corresponding opening
