...
> 原文: [`https://www.geeksforgeeks.org/python-extract-monodigit-elements/`](https://www.geeksforgeeks.org/python-extract-monodigit-elements/)
...
> **解释**: 所有元素都有一个唯一的数字或字符。
>
> **输入**: `test_list = [463, "GFG", 8838, 43, 991]`
> **输出**: `[]`
> **解释**: 未发现仅有一位数的元素。
...
## 方法二: 使用 `filter()` + `lambda` + `all()`

在这种情况下，我们使用 `lambda` 功能执行过滤任务，`filter()`，再次使用 `all()` 检查所有数字的相等性。
