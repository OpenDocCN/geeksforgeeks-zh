...
> 原文：[https://www.geeksforgeeks.org/make-multiple-directories-based-on-a-list-using-python/](https://www.geeksforgeeks.org/make-multiple-directories-based-on-a-list-using-python/)
...
*   [`os.mkdir(path)`](https://www.geeksforgeeks.org/python-os-mkdir-method/)：用于在目录中一次创建单个文件夹（此处为 `path`）。
*   [`os.path.join(root_path, path)`](https://www.geeksforgeeks.org/python-os-path-join-method/)：这个方法将几个路径连接到一个目录。这里，参数 `root_path` 将与想要创建的路径连接在一起。
*   [`partial(function, argument1, argument2, ...)`](https://www.geeksforgeeks.org/partial-functions-python/)：这个方法允许一个人固定一定数量的参数并生成一个新的函数。
*   [`os.makedirs(path)`](https://www.geeksforgeeks.org/python-os-makedirs-method/)：这个方法帮助我们一次创建多个目录。这里的参数 `path` 表示我们想要创建的带有子文件夹的目录。
