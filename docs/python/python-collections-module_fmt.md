...
一个[计数器](https://www.geeksforgeeks.org/counters-in-python-set-1/)是字典的一个子类。它用于以无序字典的形式保存可迭代表中元素的计数，其中键表示可迭代表中的元素，值表示可迭代表中该元素的计数。
...
注: 更多信息请参考 Python 中的[计数器](https://www.geeksforgeeks.org/counters-in-python-set-1/)。

## OrderedDict

一个[OrderedDict](https://www.geeksforgeeks.org/ordereddict-in-python/)也是字典的一个子类，但是不像字典，它记住了键插入的顺序。
...
注: 更多信息请参考 Python 中的[OrderedDict](https://www.geeksforgeeks.org/ordereddict-in-python/)。

## DefaultDict

一个[DefaultDict](https://www.geeksforgeeks.org/defaultdict-in-python/)也是字典的一个子类。它用于为不存在且从不引发键错误的键提供一些默认值。
...
注: 更多信息请参考 Python 中的[defaultdict](https://www.geeksforgeeks.org/defaultdict-in-python/)。

## ChainMap

一个[ChainMap](https://www.geeksforgeeks.org/chainmap-in-python/)将许多词典封装成一个单元，并返回一个词典列表。
...
可以使用键名访问链映射中的值。也可以使用 `keys()` 和 `values()` 方法访问它们。
...
> 1
> values() view object of ChainMap containing {'a': 1, 'b': 2, 'c': 3, 'd': 4, 'e': 5, 'f': 6}
> keys() view object of ChainMap containing ['a', 'b', 'c', 'd', 'e', 'f']
...
注: 更多信息请参考 Python 中的[ChainMap](https://www.geeksforgeeks.org/chainmap-in-python/)。

## namedtuple

一个[namedtuple](https://www.geeksforgeeks.org/namedtuple-in-python/)返回一个元组对象，其中包含普通元组缺少的每个位置的名称。例如，考虑一个名为 `student` 的元组，其中第一个元素代表 `fname`，第二个元素代表 `lname`，第三个元素代表 `DOB`。假设为了调用 `fname` 而不是记住索引位置，您实际上可以通过使用 `fname` 参数来调用元素，那么访问元组元素将非常容易。该功能由 `namedtuple` 提供。
...
2.  `_asdict()`: 该函数返回一个 `OrderedDict`，该 `OrderedDict` 是根据 `namedtuple()` 的映射值构造的。
...
注: 更多信息请参考 Python 中的[namedtuple](https://www.geeksforgeeks.org/namedtuple-in-python/)。
...
`deque`中的元素可以从两端插入。使用`append()`方法从右追加插入元素，使用`appendleft()`方法从左追加插入元素。
...
**注:**更多信息请参考 Python 中的[deque。](https://www.geeksforgeeks.org/deque-in-python/)
