...
> 原文：`https://www.geeksforgeeks.org/python-catch-all-exceptions/`
...
[`try` 和 `except` 语句](https://www.geeksforgeeks.org/python-try-except/)用于捕捉和处理 Python 中的异常。可以引发异常的语句保存在 `try` 子句中，处理异常的语句写在 `except` 子句中。
