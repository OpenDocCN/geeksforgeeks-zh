...
> 原文: [https://www.geeksforgeeks.org/how-to-remove-lines-starting-with-any-prefix-using-python/](https://www.geeksforgeeks.org/how-to-remove-lines-starting-with-any-prefix-using-python/)

给定一个文本文件，逐行读取该文本文件的内容，并且只打印那些不以定义的前缀开头的行。同时将这些打印行存储在另一个文本文件中。有以下几种方法可以完成这项任务:

## 方法 1: 使用循环和 `startswith()`
...
# open GeeksforGeeks file in read mode
...
在上面的例子中，我们打开一个文件，一行行地读取它的内容。我们使用 `startswith()` 方法检查该行是否以给定的前缀开始。如果该行以 `"TextGenerator"` 开头，我们跳过该行，否则我们打印该行并将其存储在另一个文件中。这样，我们可以删除以指定前缀开头的行。
...
# defining object file1 to open GeeksforGeeks file in read mode
file1 = open('GeeksforGeeks.txt', 'r')
...
for line in file1.readlines():
    ...
    # reading all lines that begin with "TextGenerator"
...
在上面的例子中，我们打开一个文件，一行行地读取它的内容。我们使用常规表达式检查该行是否以 `"Geeks"` 开头。如果那一行以 `"Geeks"` 开头，我们跳过那一行，打印其余的行，并将这些行存储在另一个文件中。