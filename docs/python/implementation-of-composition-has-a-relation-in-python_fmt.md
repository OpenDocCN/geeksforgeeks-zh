...
> 原文：[https://www.geeksforgeeks.org/implementation-of-composition-has-a-relation-in-python/](https://www.geeksforgeeks.org/implementation-of-composition-has-a-relation-in-python/)
...
在上面的例子中，`Car`类有一个`Engine`类参考。在`Car`类中，我们也可以创建不同的变量和方法。使用车内`Engine`类的对象引用，我们可以很容易地访问车内`Engine`类的每个成员。
...
在上面的例子中，我们有 2 个类`Employee`和`Data`。在`Data`类构造器中，我们正在创建一个`Employee`类的对象，因此我们可以轻松访问`Employee`类的成员。在`Data`类`Employee`类对象成为`Data`类的实例变量。我们在构造函数中创建对象，所以每当我们调用类`Employee`的任何方法或变量时，我们必须使用 [`self` 关键字](https://www.geeksforgeeks.org/self-in-python-class/)。我们可以将`self.emp_obj`替换为`Employee`，但是通过使用类名`Employee`我们只能访问`Employee`类的静态方法或变量。
