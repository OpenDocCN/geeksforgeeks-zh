...
## 安装 ImageMagick
**ImageMagick**并不是严格要求的，只有用户想写文字的时候才需要。它也可以用作 gif 的后端，尽管用户可以在没有 ImageMagick 的情况下使用 MoviePy 制作 gif。以下是下载 ImageMagick 的链接
...
3.  如果用户同时使用多个视频、音频和图像源(> 100)，可能会出现内存问题