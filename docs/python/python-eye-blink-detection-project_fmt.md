...
> 原文: [https://www.geeksforgeeks.org/python-eye-blink-detection-project/](https://www.geeksforgeeks.org/python-eye-blink-detection-project/)

在本教程中，您将学习如何使用被称为`Haar级联`的特征映射器检测人类眨眼。在这个项目中，我们将使用 python 语言和 OpenCV 库分别进行算法执行和图像处理。我们将在项目中使用的 `Haar级联` 是预先准备好的，并与 OpenCV 库一起存储为 `haarcascade_frontalface_default.xml` 和 `haarcascade_eye_tree_eyeglasses.xml` 文件。该项目对驾驶员睡意检测、眨眼锁定、眼睛检测、面部检测等系统以及 `Haar级联` 使用 OpenCV 库有了基本的了解。
...
利用所有可能的分类器大小计算特征，但是计算特征所花费的计算量，一个 `24×24` 窗口产生超过 160000 个特征。同样，对于每个特征计算，也需要像素的总和。为了降低计算成本，`Haar级联` 的创建者引入了积分图像，这意味着无论图像有多大，它都会将给定像素的计算简化为只涉及四个像素的运算。
...
`haarcascade_frontalface_default.xml` 和 `haarcascade_eye_tree_eyeglasses.xml` 是存储在与 python 脚本相同目录中的 XML 文件。
