...
> 原文: [https://www.geeksforgeeks.org/man-in-the-middle-attack-in-diffie-hellman-key-exchange/](https://www.geeksforgeeks.org/man-in-the-middle-attack-in-diffie-hellman-key-exchange/)
...
马洛里截取爱丽丝的公共值（`gᵃ (mod p)`），阻止它到达鲍勃，而是发送给鲍勃她自己的公共值（`gᶜ (modp)`），马洛里截取鲍勃的公共值（`gᵇ (mod p)`），阻止它到达爱丽丝，而是发送给爱丽丝她自己的公共值（`gᵈ (modp)`）

![](img/76363b9d68abb5058c9ac9586a6a5876.png)

**步骤 4:** 计算密钥

爱丽丝将计算一个密钥 `S₁ =gᵈᵃ (mod p)`，鲍勃将计算一个不同的密钥 `S₂ =gᶜᵇ (mod p)`

![](img/6e4afc22eca6a9584ed09dc6bc8b541e.png)

**第五步:** 如果爱丽丝使用 `S₁` 作为密钥对稍后发送给鲍勃的消息进行加密，则马洛里可以对其进行解密，使用 `S₂` 对其进行重新加密，并将其发送给鲍勃。鲍勃和爱丽丝不会注意到任何问题，可能会假设他们的通信是加密的，但实际上，马洛里可以解密、读取、修改，然后重新加密他们所有的对话。
