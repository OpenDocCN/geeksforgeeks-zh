...
> 原文：[https://www.geeksforgeeks.org/finding-the-word-analogy-from-given-words-using-word2vec-embeddings/](https://www.geeksforgeeks.org/finding-the-word-analogy-from-given-words-using-word2vec-embeddings/)

在许多分班考试中，我们经常会遇到一个基本问题来寻找单词类比。在单词类比任务中，我们完成句子“`a` 是 to `b` 就像 `c` 是 to `_ _`”，常表示为`a : b :: c : d`，我们要找到单词`d`。一个示例问题可以是这样的：“男人对女人就像国王对 __”。

人脑可以识别出空白处必须填入单词“`queen`”。但是对于一台机器来说，要理解这种模式，并用最合适的词来填补空白，需要进行大量的训练。如果我们可以使用机器学习算法来自动完成寻找类比这个词的任务，会怎么样。在本教程中，我们将使用`Word2Vec`模型和一个名为“`GoogleNews-vectors-negative300.bin`”的预训练模型，该模型由 Google 在超过 500 亿个单词上进行训练。预训练数据集中的每个单词都被嵌入到 300 维空间中，并且上下文/含义相似的单词在空间中被放置得彼此更靠近。
...
我们需要安装一个额外的`gensim`库，要使用 word2vec 模型，要安装 gensim 使用命令`pip install gensim`在您的终端/命令提示符下。
