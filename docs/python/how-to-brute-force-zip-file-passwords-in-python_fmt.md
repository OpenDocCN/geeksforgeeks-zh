...
> 原文: [https://www.geeksforgeeks.org/how-to-brute-force-zip-file-passwords-in-python/](https://www.geeksforgeeks.org/how-to-brute-force-zip-file-passwords-in-python/)
...
*   调用 `crack_password` 函数，如果找到密码则返回真，否则返回假。将文本文件和 `ZipFile` 对象的名称作为参数传递。
*   `idx` 变量用于跟踪行号。
...
*   如果密码不正确，将生成一个异常。在 `except` 块中，继续循环检查文件中的其他单词。
