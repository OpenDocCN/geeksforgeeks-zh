...
> 原文：[https://www.geeksforgeeks.org/interactive-charts-using-pywedge-package/](https://www.geeksforgeeks.org/interactive-charts-using-pywedge-package/)
...
`Pywedge` 库有一个 `make_charts` 的方法，提供给我们制作 8 种不同类型的图表，命名如下：
...
执行时的 `make_charts` 方法会产生一个窗口，为我们提供 8 种不同类型的图表选项，我们可以选择所需的功能来绘制图表并可视化结果。
