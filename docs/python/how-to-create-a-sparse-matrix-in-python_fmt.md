...
> 原文: [https://www.geeksforgeeks.org/how-to-create-a-sparse-matrix-in-python/](https://www.geeksforgeeks.org/how-to-create-a-sparse-matrix-in-python/)

如果矩阵的大部分元素都有 **0 值**，那么就称之为稀疏矩阵。使用稀疏矩阵代替简单矩阵的两大好处是：
...
Python 的 `SciPy` 给出了使用多个数据结构创建稀疏矩阵的工具，以及将密集矩阵转换为稀疏矩阵的工具。函数 `csr_matrix()` 用于创建压缩稀疏行格式的稀疏矩阵，而 `csc_matrix()` 用于创建压缩稀疏列格式的稀疏矩阵。
