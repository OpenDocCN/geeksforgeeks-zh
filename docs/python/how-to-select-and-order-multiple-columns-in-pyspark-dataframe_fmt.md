...
> 原文: [https://www.geeksforgeeks.org/how-to-select-and-order-multiple-columns-in-pyspark-dataframe/](https://www.geeksforgeeks.org/how-to-select-and-order-multiple-columns-in-pyspark-dataframe/)
...
*   `select()`: 此方法用于选择数据框列的一部分，并返回新选择的数据框的副本。
    > 语法: `dataframe.select(['column1', 'column2', 'column n']).show()`

*   `sort()`: 此方法用于对数据帧的数据进行排序，并返回新排序的数据帧的副本。默认情况下，这将按升序对数据帧进行排序。
    > 语法: `dataframe.sort(['column1', 'column2', 'column n'], ascending=True).show()`

*   `orderBy()`: 该方法类似于 `sort`，也用于对数据帧进行排序。默认情况下，这将按升序对数据帧进行排序。
    > 语法: `dataframe.orderBy(['column1', 'column2', 'column n'], ascending=True).show()`
