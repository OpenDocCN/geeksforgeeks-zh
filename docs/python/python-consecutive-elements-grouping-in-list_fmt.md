...
> 原文: [https://www.geeksforgeeks.org/python-consecutive-elements-grouping-in-list/](https://www.geeksforgeeks.org/python-consecutive-elements-grouping-in-list/)
...
该任务可以使用上述函数的组合来执行。在本文中，我们创建了一个生成器函数，其中我们传递了使用 `enumerate()` 访问其索引元素的列表，并使用 `groupby()` 和 `lambda` 按连续元素分组。仅适用于 Python2。
