...
> 原文：[https://www.geeksforgeeks.org/python-consecutive-prefix-overlap-concatenation/](https://www.geeksforgeeks.org/python-consecutive-prefix-overlap-concatenation/)

有时，在使用 Python 字符串时，我们可能会有需要执行字符串列表中所有元素的连接的应用程序。这可能是棘手的情况下，我们需要重叠当前元素的后缀与前缀的下一个匹配的情况下。让我们讨论执行这项任务的某些方法。

## 方法#1：使用循环 + `endswith()` + `join()` + 列表推导式 + `zip()`

以上功能的组合可以用来执行这个任务。在本文中，我们使用循环作为使用 `endswith()` 的连接/不连接的逻辑。如果是联接，则使用 `join()` 执行。整个逻辑是在列表推导式中编译的。
