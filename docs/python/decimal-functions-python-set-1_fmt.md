...
> 原文：[https://www.geeksforgeeks.org/decimal-functions-python-set-1/](https://www.geeksforgeeks.org/decimal-functions-python-set-1/)
...
- `as_tuple()`：将十进制数作为包含 3 个参数、符号（0 代表 +，1 代表 -）、数字和指数值的元组返回。
- `fma(a, b)`：这个 “fma” 代表融合乘加。它根据参数中的数字计算 `(num * a) + b`。此功能中不发生 `(num * a)` 的舍入。
...
- `compare()`：此函数用于比较十进制数。如果第一个十进制参数大于第二个十进制参数，则返回 1；如果第一个十进制参数小于第二个十进制参数，则返回 -1；如果两者相等，则返回 0。
- `compare_total_mag()`：比较十进制数的总幅度。如果第一个十进制参数大于第二个（忽略符号），则返回 1，如果第一个十进制参数小于第二个（忽略符号），则返回 -1，如果两者相等（忽略符号），则返回 0。
...
### copy_abs(), copy_negate() 与 copy_sign()

- `copy_abs()`：该函数打印十进制参数的绝对值。
- `copy_negate()`：该函数打印十进制参数的否定。
- `copy_sign()`：该函数通过从第二个参数复制符号来打印第一个参数。
