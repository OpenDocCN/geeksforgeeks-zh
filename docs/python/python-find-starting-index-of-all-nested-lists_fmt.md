...
> 原文: [https://www.geeksforgeeks.org/python-find-starting-index-of-all-nested-lists/](https://www.geeksforgeeks.org/python-find-starting-index-of-all-nested-lists/)
...
> **输出:** `[0, 1, 5, 7, 13]`
> **说明:** 1+4 = 2 个初始列表的长度= 5，3，第 3 个列表从第 5 个索引[基于 0 的索引]开始，因此是 5。作为结果列表中的第三个元素。
...
## 方法 2: 使用 `accumulate()` + `map()` + `len()`

在本例中，我们使用 `accumulate()` 执行求和任务，`map()` 用于获取使用 `len()` 计算的所有子列表的长度。
