# Python 中 `dict.items()` 和 `dict.iteritems()` 的区别

> 原文：[https://www.geeksforgeeks.org/difference-between-dict-items-and-dict-iteritems-in-python/](https://www.geeksforgeeks.org/difference-between-dict-items-and-dict-iteritems-in-python/)

`dict.items()` 和 `dict.iteritems()` 几乎做的是同样的事情，但它们之间略有不同：

*   `dict.items()`: 以 (key, value) 元组对的形式返回字典列表的副本。它是 Python v3.x 版本的方法，在 Python v2.x 版本中也存在。
*   `dict.iteritems()`: 以 (key, value) 元组对的形式返回字典列表的迭代器。这是一个 Python v2.x 版本的方法，在 Python v3.x 版本中被移除。
