...
> 原文: [https://www.geeksforgeeks.org/python-k-length-concatenate-single-valued-tuple/](https://www.geeksforgeeks.org/python-k-length-concatenate-single-valued-tuple/)
...
以上函数的组合可以用来解决这个问题。在本文中，我们使用 `zip_longest()` 执行连接任务，并且使用 `chain.from_iterables()` 在连接之前将元组展平为列表元素。
