...
> 原文: [https://www.geeksforgeeks.org/data-classes-in-python-an-introduction/](https://www.geeksforgeeks.org/data-classes-in-python-an-introduction/)

`数据类模块`是 Python 3.7 中引入的一个实用工具，用来制作专门用于存储数据的结构化类。这些类持有特定的属性和函数，专门处理数据及其表示。

虽然该模块是在 Python3.7 中引入的，但也可以通过安装`数据类`库在 Python3.6 中使用。
