...
> 原文：[https://www.geeksforgeeks.org/get-psycopg2-count-number-of-results/](https://www.geeksforgeeks.org/get-psycopg2-count-number-of-results/)
...
>
> `SELECT count(*) FROM table_name WHERE condition;` #返回具有指定条件的行数
...
例如，在下面给出的代码中，我们编写第一个查询来返回表中存在的行数，编写第二个查询来返回价格名称为 1.99 的行数。最后，获取并显示行数。
