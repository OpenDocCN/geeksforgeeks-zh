...
> 原文：[https://www.geeksforgeeks.org/natural-language-processing-using-polyglot-introduction/](https://www.geeksforgeeks.org/natural-language-processing-using-polyglot-introduction/)

本文解释了一个名为 [`polyglot`](https://polyglot.readthedocs.io/en/latest/index.html) 的 Python NLP 包，该包支持各种多语言应用，并提供了广泛的分析和广泛的语言覆盖。由拉米艾尔弗开发。它由许多功能组成，如：
...
首先，让我们安装一些必需的包。使用 Google Colab 可以轻松流畅地安装。

```py
pip install polyglot pyicu Morfessor pycld2
```

下载一些必要的模型。使用 Google Colab 可以轻松安装模型。
...
sentences = u"""Suggest a platform for placement preparation?. GFG is a very good platform for placement preparation."""
text = Text(sentences)
