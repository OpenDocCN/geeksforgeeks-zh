...
> 原文：[https://www.geeksforgeeks.org/exploring-data-distribution-set-1/](https://www.geeksforgeeks.org/exploring-data-distribution-set-1/)

每当我们从事数据科学和机器学习时，我们处理数据并从中发现有用信息的方法都是基于数据的分布。分布意味着数据如何以不同的可能方式呈现，特定数据的百分比，识别异常值。所以，数据分发就是用图形化的方式来组织和显示有用的信息。
...
基于数据的百分位数，如下图所示。箱线图的顶部和底部是数据的第 75 个和第 25 个百分位数。延长线被称为胡须，包括其余数据的范围。
![](img/7bbce618940ef42b18897c9a69b358a8.png)
...
它是一种将数据分配到等间距的范围、区段中的工具，并告诉我们有多少个值落在每个区段中。
