...
> 原文：[https://www.geeksforgeeks.org/how-to-remove-brackets-from-text-file-in-python/](https://www.geeksforgeeks.org/how-to-remove-brackets-from-text-file-in-python/)

有时从文本文件中删除括号变得很困难，这对我们来说是不必要的。因此，python 可以为我们做到这一点。在 python 中，我们可以借助 [`re`](https://www.geeksforgeeks.org/regular-expression-python-examples-set-1/) 模块去掉括号。
...
> import `re`
> path=
> # `pattern` 是查找括号的特殊 RE 表达式。
> # `repl` 是一个字符串，用模式替换选中的东西
> # RE 在 `sentence` 字符串中搜索
...
在这里，我们使用了 regex 的 2 个语法字符来完成这个操作-
...
更多信息请参考 Python 中的[文件处理](https://www.geeksforgeeks.org/file-handling-python/)。
