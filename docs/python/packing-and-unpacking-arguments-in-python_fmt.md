...
> 原文: [https://www.geeksforgeeks.org/packing-and-unpacking-arguments-in-python/](https://www.geeksforgeeks.org/packing-and-unpacking-arguments-in-python/)
...
作为另一个例子，考虑内置的 `range()` 函数，它需要单独的 `start` 和 `stop` 参数。如果它们不能单独使用，用 `*` 运算符编写函数调用，从列表或元组中解包参数:
...
上面的函数 `mySum()` 进行“打包”，将此方法调用接收的所有参数打包到一个变量中。一旦我们有了这个“打包”的变量，我们就可以像使用普通元组一样使用它。`args[0]` 和 `args[1]` 将分别给出第一个和第二个参数。因为我们的元组是不可变的，所以您可以将 `args` 元组转换为列表，这样您也可以在其中修改、删除和重新排列项目。
...
这里 `**` 解包了与之一起使用的字典，并将字典中的项目作为关键字参数传递给函数。所以写 `fun(1, **d)` 就相当于写 `fun(1, b=4, c=10)`。
...
**参考:**
[http://hangar.runway7.net/python/packing-unpacking-arguments](http://hangar.runway7.net/python/packing-unpacking-arguments)
