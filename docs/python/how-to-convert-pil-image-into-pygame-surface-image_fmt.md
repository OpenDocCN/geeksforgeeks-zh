...
> 原文: [https://www.geeksforgeeks.org/how-to-convert-pil-image-into-pygame-surface-image/](https://www.geeksforgeeks.org/how-to-convert-pil-image-into-pygame-surface-image/)
...
### 第五步:
接下来，将你想要转换成 Pygame Surface Image 的 PIL 图像作为输入。
...
### 步骤 6:
计算将 PIL 图像转换为 pygame 表面图像时将使用的 `mode` 值。
...
### 步骤 7:
进一步，计算将 PIL 图像转换为 pygame 表面图像时将使用的 `size` 值。
...
### 步骤 8:
计算将 PIL 图像转换为 pygame 表面图像时将使用的 `data` 值。
...
### 第 12.4 步:
然后，让你的应用在运行状态下做你想让它做的任何事情。

### 第 12.5 步:
在做了所有你想做的事情之后，更新完成的更改。
