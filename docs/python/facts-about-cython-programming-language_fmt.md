...
> 原文: [https://www.geeksforgeeks.org/facts-about-cython-programming-language/](https://www.geeksforgeeks.org/facts-about-cython-programming-language/)

`Cython` 是一种编程语言。它可以在 Windows、macOS 和 Linux 操作系统上运行。它的版本从 2.6 到 3.8 不等。`Cython 3.0.0` 正在开发中。在 `Cython` 中，用 `Python` 编写的代码被转换成 `C` 语言。像 `Quora` 这样的高流量网站使用 `Cython` 编程语言。
