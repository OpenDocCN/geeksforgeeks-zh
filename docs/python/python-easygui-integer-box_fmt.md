...
## 整数框
用于获取用户输入的整数，输入应该是整数输入，而不是输入框中出现的刺痛。它显示标题、要显示的消息、输入整数的位置以及一对用于确认输入的“确定”、“取消”按钮。我们可以在用户输入文本的地方设置一些默认的整数值，也可以设置用户可以输入的上下限值，下图是
![](img/109bff6821506051c403a1a593d34c06.png)的输入框

为了做到这一点，我们将使用`integerbox`方法

## 语法
`integerbox(消息、标题、default_integer、下界、上界)`

## 参数
取 5 个参数，第一个字符串即要显示的消息/信息，第二个字符串即窗口标题，第三个是整数，是默认整数，第四个和第五个分别是代表下限和上限的整数
