...
> 原文: [https://www.geeksforgeeks.org/fun-fact-generator-web-app-in-python/](https://www.geeksforgeeks.org/fun-fact-generator-web-app-in-python/)

在本文中，我们将讨论如何使用 `PyWebio` 模块在 Python 中创建一个有趣的事实生成器网络应用程序。本质上，它将随机创建有趣的事实，并在网络界面上显示它们。该脚本将借助 `GET` 方法从 `uselessfacts.jsph.pl` 中检索数据，我们将使用 `requests` 和 `json` 模块分别获取和加载数据。数据加载后，只需在字典中传递文本，这样我们就只能得到笑话。为了在网络上创建一个简单的交互界面，我们将使用 `PyWebIO` 模块。
...
*   导入所有必需的模块
*   使用 `requests.request()` 方法发送 `GET` 请求，并使用 `json.loads()` 函数解析有效的 `JSON` 消息并将其转换为 Python 字典。由于 `requests` 功能会创建一个项目字典，我们只需要文本，所以我们会在 `json` 模块内部传递 `response.text`。
*   现在，因为我们需要文本，即事实，我们将在 `data` 列表中传递文本，并使用 `pywebio` 模块中定义的 `style` 功能打印事实。因为 `data` 是一个包含几个无用项目的随机笑话的字典，我们将只得到文本部分。
