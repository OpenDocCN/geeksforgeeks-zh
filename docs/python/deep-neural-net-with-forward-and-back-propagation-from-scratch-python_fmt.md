...
> 原文: [https://www.geeksforgeeks.org/deep-neural-net-with-forward-and-back-propagation-from-scratch-python/](https://www.geeksforgeeks.org/deep-neural-net-with-forward-and-back-propagation-from-scratch-python/)
...
<center>
![ \begin{array}{c} z^{[1]}=W^{[1]} x+b^{[1]} \\ a^{[1](i)}=\tanh \left(z^{[1]}\right) \\ z^{[2]}=W^{[2]} a^{[1]}+b^{[2]} \\ \hat{y}=a^{[2]}=\sigma\left(z^{[2]}\right) \\ y_{\text {prediction}}=\left\{\begin{array}{ll} 1 & \text { if } a^{[2]}>0.5 \\ 0 & \text { otherwise } \end{array}\right. \end{array}](img/1f9c7dce26cd096e1462f139d65b58c1.png "Rendered by QuickLaTeX.com")
</center>
...
```py
def back_propagate(W1, b1, W2, b2, cache, X, Y, learning_rate, m):
...
现在我们将使用上面定义的功能训练模型，可以根据处理单元的便利性和能力来放置时代。
...
for i in range(0, num_iterations):
    ...
    W1, W2, b1, b2 = backward_propagation(W1, b1, W2, b2, cache, X, Y, learning_rate, m)
