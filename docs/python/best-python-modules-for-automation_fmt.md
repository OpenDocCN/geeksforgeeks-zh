...
> 原文：[https://www.geeksforgeeks.org/best-python-modules-for-automation/](https://www.geeksforgeeks.org/best-python-modules-for-automation/)
...
*   `BeautifulSoup` 是从 HTML 和 XML 文件中提取数据的最佳库。
...
*   它还包括一堆测试库和其他工具。
*   `Robot Framework` 是基于 Python 的，但是您也可以使用 `Jython`（Java）或 `IronPython`（.NET）。
*   机器人还可以测试其他东西，例如：
    *   FTP
    *   MongoDB
    *   Selenium
...
*   `Lettuce` 是另一个简单易用的基于黄瓜和 Python 的行为驱动自动化工具。
*   `Lettuce` 的主要目标是专注于行为驱动开发的常见任务，使工作更简单、更有趣。
...
*   总的来说，`Pandas` 不仅仅局限于处理与数据相关的任务；它是创建更集中、更强大的数据工具的最佳起点。