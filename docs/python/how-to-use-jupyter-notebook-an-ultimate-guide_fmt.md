...
> 原文：[https://www.geeksforgeeks.org/how-to-use-jupyter-notebook-an-ultimate-guide/](https://www.geeksforgeeks.org/how-to-use-jupyter-notebook-an-ultimate-guide/)
...
> **目录**
> *   [安装](#安装)
> *   [启动 Jupyter 笔记本](#启动-jupyter-笔记本)
> *   [创建笔记本](#创建笔记本)
> *   [Jupyter 笔记本中的 Hello World](#jupyter-笔记本中的-hello-world)
> *   [Jupyter 笔记本中的单元格](#jupyter-笔记本中的单元格)
> *   [内核](#内核)
> *   [命名笔记本](#命名笔记本)
> *   [笔记本扩展](#笔记本扩展)
...
要创建新笔记本，请单击右上角的“新建”按钮。单击它打开一个下拉列表，然后如果您单击 `Python3`，它将打开一个新笔记本。
...
选择要执行的单元格后，点击“运行”按钮或按下 `⇧ + ⏎` 来运行单元格。将上述代码写入 `jupyter` 笔记本后，输出结果为：

![jupyter-notebook-hello-world](img/690e297ca7c3dcca4a0e1b9820b3da6e.png)

**注意：** 当一个单元格执行了，左边的标签，即 `In[]` 变为 `In[1]`。如果该单元仍在执行中，标签仍为 `In[*]`。

## Jupyter 笔记本中的单元格

单元格可以被认为是 `Jupyter` 的身体。在上面的截图中，带有绿色轮廓的框是一个单元格。有 3 种类型的单元格：
...
`Markdown` 是一种流行的标记语言，是 `HTML` 的超集。`Jupyter` 笔记本也支持 `Markdown`。可以使用单元格菜单将单元格类型更改为 `Markdown`。
...
在 `Jupyter` 笔记本中添加列表真的很简单。使用 `'*'` 符号可以添加列表。嵌套列表可以通过缩进来创建。
...
`LaTeX` 表达式可以通过 `'$'` 包围 `LaTeX` 代码来添加，为了在中间写表达式，通过 `'$'` 包围 `LaTeX` 代码。
...
提供原始单元直接写入输出。`Jupyter` 笔记本不评估此单元格。通过 `nbconvert` 后，原始单元格到达目标文件夹，无需任何修改。例如，一个人可以将完整的 `Python` 代码写入到原始单元格中，只有通过 `nbconvert` 转换后，才能由 `Python` 渲染。
...
创建笔记本时，`Jupyter Notebook` 会将笔记本命名为默认的“无标题”。但是，笔记本可以重命名。要重命名笔记本，只需单击“无标题”一词。这将提示一个名为“重命名笔记本”的对话框。在文本栏中输入笔记本的有效名称，然后单击“确定”。
