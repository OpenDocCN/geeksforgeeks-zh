...
> 原文：[https://www.geeksforgeeks.org/destroy-method-in-tkinter-python/](https://www.geeksforgeeks.org/destroy-method-in-tkinter-python/)
...
正如您可能观察到的，在上面的代码中，在按钮-2 中传递的命令是销毁按钮-1，所以只要您按下按钮-2，按钮-1 就会被销毁。
![destroy() method passed as command](img/d8f6245a4bee4e248bf42250e017de2b.png)

## 代码#2：结合 after() 方法使用 destroy() 方法
...
还有另一种方法 `quit()` 不破坏小部件，但退出 tcl/tk 解释器，即停止 `mainloop()`。