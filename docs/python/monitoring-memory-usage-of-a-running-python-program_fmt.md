...
> 原文：[https://www.geeksforgeeks.org/monitoring-memory-usage-of-a-running-python-program/](https://www.geeksforgeeks.org/monitoring-memory-usage-of-a-running-python-program/)
...
## 方法 3：使用经典的 `memory_profiler`

来自 PyPI 的 `memory_profiler` 是一个 Python 库模块，用于监控进程内存。它使用 `psutil` 代码来创建装饰器，然后使用它来获取内存分布。有了这个 PyPI 模块，通过导入可以节省行，直接调用装饰器。要安装，请使用以下工具：
...
输出显示代码中每一行消耗的内存。使用 `memory_profiler` 实现查找内存消耗非常容易，因为我们直接调用装饰器，而不是编写全新的代码。
