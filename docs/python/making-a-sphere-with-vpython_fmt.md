...
> *   `axis` : 是球体的对齐轴。指定包含 3 个值的向量，例如 `axis = vector(1, 2, 1)`
> *   `up` : 是球体的方位。指定一个包含 3 个值的向量，例如 `up = vector(0, 1, 0)`
> *   `color` : 是球体的颜色。指定一个包含 3 个值的向量，例如 `color = vector(1, 1, 1)` 将给出白色
> *   `opacity` : 是球体的不透明度。分配一个浮动值，其中 1 是最不透明的，0 是最不透明的，例如 `opacity = 0.5`
> *   `shininess` : 是球体的光亮。指定一个浮动值，其中 1 是最闪亮的，0 是最不闪亮的，例如 `shininess = 0.6`
> *   `emissive` : 是球体的发射率。指定一个布尔值，其中“真”是发射性的，“假”不是发射性的，例如 `emissive = False`
> *   `texture` : 是球体的纹理。从纹理类中指定所需的纹理，例如 `texture = textures.stucco`
> *   `radius` : 是球体的半径。指定一个浮动值，默认半径为 1，示例 `radius = 10`
> *   `size` : 是球体的大小。分配一个包含 3 个值的向量，分别表示长度、高度和重量，默认大小为(2，2，2)，示例 `size = vector(1, 1, 1)`
>
> 所有参数都是可选的。

### 例 1

一个没有参数的球体，所有参数都会有默认值。
...
# import the module
from vpython import *
sphere(texture = textures.stucco,
       axis = vector(1, 0, 0),
       up = vector(0, 1, 0))
```

**输出:**
![](img/bee955f9c5998535af4f0830e004a80d.png)