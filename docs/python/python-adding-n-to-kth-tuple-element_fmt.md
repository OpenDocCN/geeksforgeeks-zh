...
> 原文：[https://www.geeksforgeeks.org/python-adding-n-to-kth-tuple-element/](https://www.geeksforgeeks.org/python-adding-n-to-kth-tuple-element/)

很多时候，在处理记录时，我们会遇到一个问题，那就是我们需要更改元组元素的值。这是使用元组时常见的问题。让我们讨论一些方法，可以将 `N` 添加到列表中元组的第 `K` 个元素中。

## 方法#1：使用循环

使用循环可以执行该任务。在这种情况下，我们只需迭代列表，通过代码中的预定义值 `N` 来更改第 `K` 个元素。
