...
> 原文: [https://www.geeksforgeeks.org/python-count-occurrence-of-all-elements-of-list-in-a-tuple/](https://www.geeksforgeeks.org/python-count-occurrence-of-all-elements-of-list-in-a-tuple/)
...
第一种方法是朴素的方法。使用 `for` 循环遍历给定的列表，并计算列表中每个元组的出现次数。最后，返回计数。
...
## 方法 2：从 Python Collections 模块中使用 Counter

导入 `Counter` 来解决给定的问题。`Counter` 是一个容器，用于记录等值被添加的次数。将结果保存在 `counts` 中后，我们使用 `for` 循环，计算列表中的每个项目在 `counts` 中出现的次数，并对其求和以给出最终输出。
