...
> 原文: [https://www.geeksforgeeks.org/how-to-create-filename-containing-date-or-time-in-python/](https://www.geeksforgeeks.org/how-to-create-filename-containing-date-or-time-in-python/)

**先决条件:** [日期时间模块](https://www.geeksforgeeks.org/python-datetime-module/)

在本文中，我们将看到如何使用 Python 创建带有日期或时间的文件名。

为此，我们将使用 DateTime 模块。首先导入模块，然后用 `datetime.now()` 对象获取当前时间。现在将其转换为字符串，然后使用 `file` 对象创建一个文件，就像使用 python 中的文件处理概念创建常规文件一样。
