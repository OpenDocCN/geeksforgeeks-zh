...
| **文件扩展名** | `.pl` 文件扩展名用于保存 `Perl` 脚本。例如 `myDocument.pl` | `.py` 文件扩展名用于保存 `Python` 脚本。示例:`myFile.py` |
| **报表结束** | 在 `Perl` 中，所有语句都应该以分号结束。 | 在 `Python` 中，没有必要用分号结束语句，因为它处理空格。 |
| **意见和文件** | 对于内联注释，我们在 `Perl` 中使用 `#` 号。例如 `# Inline-Perl 中的注释` 而对于文档，我们使用 `=` 和 `=cut` 例如 `= Perl 中的文档` 从这里开始，到这里结束。`=cut` | `Python` 还使用 `#` 进行内联注释。例如 `# Inline-Python 中的注释` 但是对于文档，我们使用 `"""`，即三个引号 例如 `"""Python 中的文档` 从这里开始，到这里结束。`"""` |
