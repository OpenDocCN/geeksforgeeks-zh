...
> 原文: [https://www.geeksforgeeks.org/fetching-recently-sent-mails-details-sent-via-a-gmail-account-using-python/](https://www.geeksforgeeks.org/fetching-recently-sent-mails-details-sent-via-a-gmail-account-using-python/)
...
*   导入 `imaplib`、`email`、`webbrowser` 和 `os` 模块。
*   与 Gmail 帐户建立 `imap` 连接。
*   实例化 Gmail 帐户的 `username` 和 `password` 变量。
*   登录 Gmail 帐户。
*   选择发送的邮件。
*   确定要检索的已发送电子邮件的数量 `n`。
