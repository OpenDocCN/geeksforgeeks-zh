...
> 原文: [https://www.geeksforgeeks.org/python-difference-between-sorted-and-sort/](https://www.geeksforgeeks.org/python-difference-between-sorted-and-sort/)
...
> **语法:** `sorted(iterable, key, reverse=False)`
>
> **参数:**
> **iterable:** 序列（列表、元组、字符串）或集合（字典、集合、frozenset）或任何其他需要排序的迭代器。
> **key（可选）:** 用作键或排序比较基础的功能。
> **reverse（可选）:** 如果设置为 `True`，则 iterable 将按反向（降序）顺序排序，默认情况下设置为 `False`。
>
> **返回类型:** 返回排序列表。
...
这个可选的参数 `key` 接受一个函数作为它的值。这个键函数在排序之前转换每个元素，它接受值并返回 1 个值，然后在排序中使用该值而不是原始值。

**示例:** 假设我们想要根据字符串的长度对其进行排序。这可以通过将 `len()` 函数作为值传递给 `key` 参数来实现。下面是实现。
...
> **语法:** `List_name.sort(key, reverse=False)`
>
> **参数:**
> **key:** 用作排序比较键的函数。
> **reverse:** 如果为 `True`，列表按降序排序。
>
> **返回类型:** 无
