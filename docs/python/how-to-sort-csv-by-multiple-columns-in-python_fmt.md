...
> 原文: [https://www.geeksforgeeks.org/how-to-sort-csv-by-multiple-columns-in-python/](https://www.geeksforgeeks.org/how-to-sort-csv-by-multiple-columns-in-python/)
...
将 CSV 文件转换为数据框后，我们需要通过 `sort_values()` 方法中的参数将 CSV 文件的两个或多个列名添加为 `by`，`axis` 赋值为 0，如下所示:
...
这里，转换成数据帧后，CSV 文件按多列排序，`depth` 列先按升序排序，然后 `table` 列按每个 `depth` 的升序排序。
...
在下面的示例中，CSV 文件按 `depth` 降序排序，然后按每个 `depth` 的 `table` 升序排序。
