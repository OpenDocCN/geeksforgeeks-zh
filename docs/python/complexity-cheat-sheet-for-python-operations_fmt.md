...
> 原文: [https://www.geeksforgeeks.org/complexity-cheat-sheet-for-python-operations/](https://www.geeksforgeeks.org/complexity-cheat-sheet-for-python-operations/)
...
像 `list`、`set`、`dictionary` 这样的 Python 内置数据结构提供了大量的操作，使得编写简洁的代码变得更加容易，但是没有意识到它们的复杂性可能会导致您的 python 代码出现意想不到的缓慢行为。
...
|  | 平均案例 | 摊销最坏情况 |
| --- | --- | --- |
| 附加 | `l.append(item)` | `O(1)` | `O(1)` |
| 清楚的 | `l.clear()` | `O(1)` | `O(1)` |
| 包含 | `item in/not in l` | `O(N)` | `O(N)` |
| 复制 | `l.copy()` | `O(N)` | `O(N)` |
| 删除 | `del l[i]` | `O(N)` | `O(N)` |
| 扩展 | `l.extend(…)` | `O(N)` | `O(N)` |
| 平等 | `l1==l2`，`l1!=l2` | `O(N)` | `O(N)` |
| 索引 | `l[i]` | `O(1)` | `O(1)` |
| 循环 | `for item in l:` | `O(N)` | `O(N)` |
| 长度 | `len(l)` | `O(1)` | `O(1)` |
| 乘；成倍增加；（使）繁殖 | `k*l` | `O(k*N)` | `O(k*N)` |
| 最小值、最大值 | `min(l)`，`max(l)` | `O(N)` | `O(N)` |
| 从头到尾弹出 | `l.pop(-1)` | `O(1)` | `O(1)` |
| Pop 中间体 | `l.pop(item)` | `O(N)` | `O(N)` |
| 去除 | `l.remove(…)` | `O(N)` | `O(N)` |
| 反面的 | `l.reverse()` | `O(N)` | `O(N)` |
...
| 删除 | `del d[k]` | `O(1)` | `O(N)` |
| 得到 | `d.get()` | `O(1)` | `O(N)` |
| 迭代(键、值、项目) | `for item in d:` | `O(N)` | `O(N)` |
| 长度 | `len(d)` | `O(1)` | `O(1)` |
| 流行音乐 | `d.pop(item)` | `O(1)` | `O(N)` |
...
|  | 平均案例 | 摊销最坏情况 |
| --- | --- | --- |
| 增加 | `s.add(item)` | `O(1)` | `O(N)` |
| 清楚的 | `s.clear()` | `O(1)` | `O(1)` |
| 复制 | `s.copy()` | `O(N)` | `O(N)` |
| 包含 | `item in/not in` | `O(1)` | `O(N)` |
| 创造 | `set(…)` | `O(len)` | `O(len)` |
| 抛弃 | `s.discard(item)` | `O(1)` | `O(N)` |
| 差异 | `s1-s2` | `O(len(s1))` | `O(len(s1))` |
| 差异更新 | `s1.difference_update(s2)` | `O(len(s2))` | – |
| 平等 | `s1==s2`，`s1!=s2` | `O(min(len(s1), len(s2)))` | `O(min(len(s1), len(s2)))` |
| 交集 | `s1 and s2` | `O(min(len(s1), len(s2)))` | `O(min(len(s1), len(s2)))` |
| 循环 | `for item in:` | `O(N)` | `O(N)` |
| 是子集 | `s1<=s2` | `O(len(s1))` | `O(len(s1))` |
| 是超集 | `s1>=s2` | `O(len(s2))` | `O(len(s1))` |
| 流行音乐 | `s.pop()` | `O(1)` | `O(N)` |
| 联盟 | `s1|s2` | `O(len(s1)+len(s2))` | – |
| 对称差 | `s1^s2` | `len(s1)` | `O(len(s1)*len(s2))` |
