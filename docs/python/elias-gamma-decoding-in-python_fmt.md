...
> 原文: [https://www.geeksforgeeks.org/elias-gamma-decoding-in-python/](https://www.geeksforgeeks.org/elias-gamma-decoding-in-python/)

Elias gamma 码是一种通用码，用于对正整数序列进行编码。它是由彼得·埃利亚斯开发的。当不能事先确定整数的上限时，这是最有用的。

**公式:**

> Elias Gamma 编码 = 一元(1 + floor(log2(`x`))) + 不带 `MSB` 的 `x` 的二进制表示
