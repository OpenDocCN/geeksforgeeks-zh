...
> 原文: [https://www.geeksforgeeks.org/minimum-number-of-characters-to-be-replaced-to-make-a-given-string-palindrome/](https://www.geeksforgeeks.org/minimum-number-of-characters-to-be-replaced-to-make-a-given-string-palindrome/)
...
运行从 0 到 `(length)/2-1` 的循环，检查第 `i` 索引处的字符是否为 `s[i] != s[length-i-1]`，那么我们将用其中字母顺序较小的字符替换字母顺序较大的字符，并继续相同的过程，直到所有元素都被遍历。
