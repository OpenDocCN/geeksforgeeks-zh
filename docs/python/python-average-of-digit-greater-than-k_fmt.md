...
> 原文: [https://www.geeksforgeeks.org/python-average-of-digit-greater-than-k/](https://www.geeksforgeeks.org/python-average-of-digit-greater-than-k/)

给定元素列表，提取位数平均值大于 `k` 的元素。

> **输入**: `test_list = [633, 719, 8382, 119, 327]`, `K = 5`
> **输出**: `[719, 8382]`
> **解释**: `(7 + 1 + 9) / 3 = 5.6` 和 `(8 + 3 + 8 + 2) / 4 = 5.2`，均大于 5，因此返回。
>
> **输入**: `test_list = [633, 719, 8382, 96]`, `K = 5`
> **输出**: `[719, 8382, 96]`
...
## 方法二: 使用 `sum()` + `len()` + `filter()` + `lambda`

这里，使用 `filter()` 和 `lambda` 进行滤波操作，其余所有操作都按照上述方法处理。
