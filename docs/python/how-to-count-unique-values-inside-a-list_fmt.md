...
> 原文：[https://www.geeksforgeeks.org/how-to-count-unique-values-inside-a-list/](https://www.geeksforgeeks.org/how-to-count-unique-values-inside-a-list/)
...
在这个方法中，我们将使用一个函数名 `Counter`。模块 `collections` 有此功能。使用 `Counter` 功能，我们将创建一个字典。字典的 `keys` 将是唯一的项目，值将是列表中该键的编号。我们将使用键创建一个列表，列表的长度将是我们的答案。
...
在这个方法中，我们将把我们的列表转换成集合。由于 `set` 不包含任何重复的项目，那么打印集合的长度将给出唯一项目的总数。
