...
> 原文：[https://www.geeksforgeeks.org/finding-the-size-resolution-of-image-in-python/](https://www.geeksforgeeks.org/finding-the-size-resolution-of-image-in-python/)
...
我们将通过导入库 `cv2` 来导入 OpenCV。我们将使用 `cv2.imread()` 功能加载图像。之后，可以使用 `shape` 属性找到尺寸。`shape[0]` 会给我们高度，`shape[1]` 会给我们宽度。
