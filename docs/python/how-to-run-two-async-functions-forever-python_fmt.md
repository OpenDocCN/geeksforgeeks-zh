...
> 原文：[https://www.geeksforgeeks.org/how-to-run-two-async-functions-forever-python/](https://www.geeksforgeeks.org/how-to-run-two-async-functions-forever-python/)
...
**【协程】：** 协程是一种通用的控制结构，流量控制在两个不同的例程之间协作传递，而不返回。在 `` `asyncio` `` 中，可以通过在 `` `def` `` 之前使用 `` `async` `` 关键字来创建 Coroutine。
...
> RuntimeWarning:从未等待过 coroutine“speak_async”
> 
> speak_async()
...
为此，我们必须创建一个新的异步函数（`` `main` ``），并调用该新函数（`` `main` ``）中的所有异步函数（我们希望同时运行）。然后使用事件循环调用新的（主）函数…
...
这里可以看到 `` `function_async()` `` 和 `` `function_2()` `` 没有并发运行，先显示 `` `function_async()` `` 的输出，再显示 `` `function_2()` `` 的输出，也就是说 `` `function_async()` `` 执行完之后，`` `function_2()` `` 正在执行。
...
**方法 2：** 对两个函数使用 `` `while True` `` 循环，并使用 `` `asyncio.ensure_future()` `` 和 `` `loop.run_forever()` `` 调用它们

**注意：** `` `ensure_future` `` 让我们在后台执行一个 coroutine，不需要显式等待它完成。
