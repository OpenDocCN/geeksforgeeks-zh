...
> 原文：[https://www.geeksforgeeks.org/create-battery-notifier-for-laptop-using-python/](https://www.geeksforgeeks.org/create-battery-notifier-for-laptop-using-python/)
...
*   `plyer`: `plyer`模块用于访问硬件的特性。这个模块没有内置 Python。我们需要从外部安装它。要安装此模块，请在终端中键入以下命令。
...
> *   `title` (str) – 通知标题
> *   `message` (str) – 通知消息
> *   `app_name` (str) – 发布此通知的应用名称
> *   `app_icon` (str) – 与消息一起显示的图标
> *   `timeout` (int) – 显示消息的时间，默认为 10
> *   `ticker` (str) – 通知到达时状态栏上显示的文本
> *   `toast` (bool) – 简单安卓消息代替完整通知
