...
> 原文: [https://www.geeksforgeeks.org/how-to-use-threadpoolexecutor-in-python3/](https://www.geeksforgeeks.org/how-to-use-threadpoolexecutor-in-python3/)
...
> **语法:** `concurrent.futures.ThreadPoolExecutor(max_workers=None, thread_name_prefix='', initializer=None, initargs=())`
>
> **参数:**
>
> *   `max_workers` : 这是一个线程数，也就是池的大小。从 3.8 开始，默认值为 `min(32, os.cpu_count() + 4)`。在这 5 个线程中，有 4 个是为输入/输出绑定任务保留的。
> *   `thread_name_prefix`: `thread_name_prefix` 是从 python 3.6 开始添加的，为线程赋予名称，以便于调试。
> *   `initializer`: `initializer` 接受一个在每个工作线程开始时调用的可调用。
> *   `initargs`: 它是传递给 `initializer` 的一组参数。
...
2.  `map(fn, *iterables, timeout=None, chunksize=1)`:
    *   它立即将方法和 iterables 映射在一起，并将同时引发异常。`Future`。如果未能在超时限制内完成，则超时错误。
    *   如果 iterables 非常大，那么 `chunksize` 大于 1 可以提高使用 `ProcessPoolExecutor` 时的性能，但是对于 `ThreadPoolExecutor`，它没有这样的优势，即它可以保留默认值。
3.  `shutdown(wait=True, *, cancel_futures=False)`:
    *   它向执行器发出信号，要求在执行完 `Future` 时释放所有资源。
...
下面的代码是通过发出一个 HTTP 请求在互联网上获取图像，我使用的是 `requests` 库。代码的第一部分对 API 进行一对一的调用，即下载速度很慢，而代码的第二部分使用线程进行并行请求以获取 API。
