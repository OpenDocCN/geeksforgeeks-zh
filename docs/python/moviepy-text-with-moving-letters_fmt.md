...
> 原文：[https://www.geeksforgeeks.org/moviepy-text-with-moving-letters/](https://www.geeksforgeeks.org/moviepy-text-with-moving-letters/)

在本文中，我们将看到如何移动文本剪辑的字母。`MoviePy`是一个用于视频编辑的`Python`模块，可用于视频和`GIF`的基本操作。视频是由帧组成的，帧的组合创建了一个视频，每一帧都是一个独立的图像。`VideoClip`是`MoviePy`中所有其他视频剪辑的基类。带有移动字母的文字，就像字母在自我扩散和重新排列，创造出一种移动的效果。
