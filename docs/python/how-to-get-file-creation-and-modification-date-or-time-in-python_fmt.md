...
> 原文：[https://www.geeksforgeeks.org/how-to-get-file-creation-and-modification-date-or-time-in-python/](https://www.geeksforgeeks.org/how-to-get-file-creation-and-modification-date-or-time-in-python/)
...
我们将使用在 `os` 库中 `path` 模块内部找到的 `getctime()` 和 `getmtime()` 函数来获取文件的创建和修改时间。以上两个函数都返回自 *EPOCH*（1970 年 1 月 1 日世界协调时 00:00:00）以来的时间（时间为浮点数据类型）。因为这个数字不像一个可以理解的时间戳，我们必须转换时间，即它变得可以识别。为此，我们将使用 `time` 库中的 `ctime()` 函数。

> `ctime(秒)`
>
> 将自纪元以来的时间（秒）转换为本地时间字符串。
>
> 这相当于 `asctime(localtime(秒))`。当时间元组是
>
