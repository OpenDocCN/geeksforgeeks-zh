...
> 原文：[https://www.geeksforgeeks.org/how-to-add-moving-platforms-in-pygame/](https://www.geeksforgeeks.org/how-to-add-moving-platforms-in-pygame/)
...
我们可以使用 `draw()` 方法在 pygame 中轻松创建任何类型的平台。为此，我们将使用 `draw.rect()` 函数创建具有特定宽度和高度的矩形。

> **语法：** `pygame.draw.rect(表面, 颜色, 矩形, 宽度)`
>
> **参数：**
>
> *   **表面：** 这里我们可以通过要在其上绘制矩形的表面。
> *   **颜色：** 这里我们可以为我们的矩形传递颜色。
> *   **矩形：** 这里我们可以传递矩形、位置和尺寸。
...
现在我们要添加我们的播放器，以及我们的播放器和平台之间的碰撞。为此，我们使用了 `colliderect()` 方法。

> **语法：** `pygame.Rect.colliderect(直肠 1, 直肠 2)`
>
> **参数：** 将取两个矩形作为其参数。
>
