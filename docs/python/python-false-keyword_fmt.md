...
> 原文：`https://www.geeksforgeeks.org/python-false-keyword/`
...
在这个例子中，我们将布尔表达式`（5 > 10）`分配给变量`gfg_flag`，然后我们将打印该变量以显示分配给它的值，即`False`关键字。接下来，我们将尝试在`if`语句中使用这个变量，这将导致执行`else`块。
