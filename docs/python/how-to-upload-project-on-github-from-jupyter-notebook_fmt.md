...
> 原文: [https://www.geeksforgeeks.org/how-to-upload-project-on-github-from-jupyter-notebook/](https://www.geeksforgeeks.org/how-to-upload-project-on-github-from-jupyter-notebook/)

Jupyter 笔记本既不是简单的文本编辑器，也不是功能齐全的 IDE，而是一个包含编程代码和文本描述的电子文件。它还可以包含嵌入的图表、绘图、图像、视频和链接。它们在火狐或谷歌浏览器等网络浏览器中运行。

它们可以包含各种编程语言的代码，尽管主要是 Python 中流行的`.py`扩展，他们也支持带`.md`扩展的 Markdowns。一种认为 Jupyter 笔记本是 Python `REPL` 和 Python 模块`.py` 文件带减价`.md` 文件在代码段之间抛出。
...
一个`Jupyter`文件浏览器将在一个网页浏览器标签中打开，你会看到你的基本(主)目录。

在右上方，你可以点击新建(下拉)- >蟒 3

![](img/0df823deb4593fbd5674d3d375b29baa.png)

之后，Jupyter 笔记本的一个新标签会在你的浏览器中打开。

![](img/2293d611c8d8e5aeb1d9feaf733a1b16.png)

现在，点击无题- >重命名提示将打开- >重命名您的文件。

您也可以点击代码下拉菜单，选择`markdown`字段进行解释(段落)或选择`heading`字段进行标题。

![](img/43321ae038cdb28adc2e82263dfa30ec.png)

然后在笔记本上写一些代码。要运行代码，按下运行按钮，如上图所示。
...
1.  点击文件- >下载为- >笔记本(`.ipynb`)
2.  在 Github 中创建一个新存储库。
3.  单击添加文件- >创建新文件。
4.  浏览您的目录并上传您的文件(示例文件名: `GeeksForGeeks.ipynb` )并单击打开。
5.  提交变更。
