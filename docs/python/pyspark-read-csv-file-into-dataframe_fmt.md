...
> 原文: [https://www.geeksforgeeks.org/pyspark-read-csv-file-into-dataframe/](https://www.geeksforgeeks.org/pyspark-read-csv-file-into-dataframe/)
...
在这里，我们将使用 `spark.read.csv` 将单个 CSV 读入 dataframe，然后使用 `toPandas()` 创建包含该数据的 dataframe。
...
这里，我们传递了我们的 csv 文件 `authors.csv`。其次，我们传递了 CSV 文件中使用的分隔符。这里的分隔符是逗号 `','`。接下来，我们将 `inferSchema` 属性设置为 `True`，这将通过 CSV 文件并自动将其模式适配到 PySpark 数据帧中。然后，我们使用 `toPandas()` 方法将 PySpark 数据帧转换为 pandas 数据帧 `df`。

## 读取多个 CSV 文件

为了读取多个 CSV 文件，我们将把 CSV 文件的路径作为 Python 列表传递，列表元素为 `str` 类型。
...
要读取目录中的所有 CSV 文件，我们将使用 `*` 来匹配目录中的每个文件。
