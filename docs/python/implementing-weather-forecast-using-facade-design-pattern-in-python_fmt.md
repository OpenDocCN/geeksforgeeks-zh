...
> 原文：[https://www.geeksforgeeks.org/implementing-weather-forecast-using-facade-design-pattern-in-python/](https://www.geeksforgeeks.org/implementing-weather-forecast-using-facade-design-pattern-in-python/)
...
*   `WeatherRetriever`类向天气应用编程接口端点发出请求，并返回预测数据。
*   `Crawler`类抓取`WeatherRetriever`类返回的数据，以`JSON`格式解码，并获取温度。
*   `WeatherForecast`类负责存储温度数据。
*   最后，`Converter`类将温度从开尔文转换为摄氏度
