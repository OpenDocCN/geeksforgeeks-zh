...
`complex()`函数是 Python 中的内置函数。此方法在传递实数和虚数时返回一个[复数](https://www.geeksforgeeks.org/complex-numbers/)(实数+虚数)示例(5+2j)，或者它还将字符串转换为复数。
...
*   `real`:数字类型(包括`复数`)。默认为零。
*   `imag`:数字类型(包括`复数`)。默认为零。

**返回:**

*   (`real`+`imag`)形式的复数示例(5+2j)
*   类型:`complex`

**注意:**如果传递的第一个参数是字符串，那么第二个参数就不应该传递，否则会引发`TypeError`。字符串不得在`+`或`–`运算符周围包含空格，否则将引发`ValueError`。
