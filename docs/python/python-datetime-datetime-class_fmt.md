...
> 原文: [https://www.geeksforgeeks.org/python-datetime-datetime-class/](https://www.geeksforgeeks.org/python-datetime-datetime-class/)

日期时间模块的`datetime`类，顾名思义，既包含日期信息，也包含时间信息。像日期对象一样，`DateTime`假设当前公历在两个方向上扩展；像时间对象一样，`DateTime`假设每天正好有`3600*24`秒。但是与日期类不同，日期类的对象是潜在的感知对象，即它也包含关于时区的信息。
...
**注意:** 传递整数以外的参数将引发`TypeError`，传递范围以外的参数将引发`ValueError`。
...
| `min` | 可表示的最小日期时间 |
| `max` | 可表示的最大日期时间 |
| `resolution` | 日期时间对象之间的最小可能差异 |
| `year` | 年的范围必须在`MINYEAR`和`MAXYEAR`之间 |
| `month` | 月份范围必须介于 1 和 12 之间 |
| `day` | 日期范围必须介于 1 和给定年份的给定月份中的天数之间 |
...
| `isocalendar()` | 返回一个元组`year`、`week`和`weekday` |
