...
Python 中的 `pathlib` 模块提供了各种表示文件系统路径的类，其语义适合不同的操作系统。该模块属于 Python 的标准实用程序模块。

`Path classes` 在 `pathlib` 模块中分为 **纯路径** 和 **具体路径**。纯路径仅提供计算操作，但不提供输入/输出操作，而从纯路径继承的具体路径提供计算和输入/输出操作。
...
### 类 `pathlib.PurePath(*path segments)`

这是一个代表系统路径风格的通用类。在实例化这个类时，将创建 `pathlib.PurePosixPath` 或 `pathlib.PureWindowsPath`。
...
### 类 `pathlib.PurePosixPath(*path segments)`

这是 `PurePosixPath` 类的子类。它代表非 Windows 文件系统路径。
...
### 类 `pathlib.PureWindowsPath(*path segments)`

这也是 `pathlib.PurePath` 类的一个子类。它表示 Windows 文件系统路径。
...
这个方法是用来检查路径是否是绝对的。如果路径是绝对的，此方法返回 `True`，否则返回 `False`。
...
### 类 `pathlib.PosixPath(*path segments)`

这个类是 `pathlib.Path` 和 `pathlib.PurePosixPath` 类的子类。这个类表示具体的非 Windows 文件系统路径。

**注意:** 不能在 Windows 操作系统上实例化 `pathlib.PosixPath` 类。
...
### 类 `pathlib.WindowsPath(*path segments)`

这个类是 `pathlib.Path` 和 `pathlib.PureWindowsPath` 的子类。这个类表示具体的 Windows 文件系统路径。
