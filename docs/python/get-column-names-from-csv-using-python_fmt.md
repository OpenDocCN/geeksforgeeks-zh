...
> 原文：[https://www.geeksforgeeks.org/get-column-names-from-csv-using-python/](https://www.geeksforgeeks.org/get-column-names-from-csv-using-python/)
...
*   使用 `DictReader` 将 CSV 文件作为字典读取，然后打印出字典的关键字
...
在第二种方法下，我们使用 `csv` 库的 `DictReader` 函数将 CSV 文件作为字典读取。我们可以简单地使用 `keys()` 方法来获取列名。
...
在这种方法下，我们使用 Python 的 `pandas` 库将 CSV 文件作为数据框读取。然后，我们只需要调用数据框的 `.columns` 方法。
