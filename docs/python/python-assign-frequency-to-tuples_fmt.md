...
> 原文：`https://www.geeksforgeeks.org/python-assign-frequency-to-tuples/`
...
在这种情况下，我们使用 `Counter()` 提取频率，使用 `items()` 提取频率号，使用 `*运算符` 解包元素，并使用列表理解将其分配给元组列表中的所有元素。
...
## 方法 2：使用 `most_common()` + `Counter()` + `*运算符` + 列表理解
