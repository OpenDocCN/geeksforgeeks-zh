...
> 原文: [https://www.geeksforgeeks.org/collapsible-pane-in-tkinter-python/](https://www.geeksforgeeks.org/collapsible-pane-in-tkinter-python/)
...
> **参数:**
> `parent` = 小部件的父代。
> `expanded_text` = 窗格打开时按钮上显示的文本。
> `collapsed_text` = 窗格关闭时按钮上显示的文本。
> **功能:**
> `_activate()` = 检查变量的值并显示或隐藏框架。
> `toggle()` = 将标签框架切换到相反的状态。
> **小部件:**
> `self._button` = 切换框架的按钮。
> `frame` = 保存小部件的框架。
> `_separator` = 分离器。
