...
> 原文：[https://www.geeksforgeeks.org/probability-plot-correlation-coefficient/](https://www.geeksforgeeks.org/probability-plot-correlation-coefficient/)
...
具有形状参数`λ`的土耳其-λ PPCC 图对于对称分布特别有用。它表示一个分布是短尾分布还是长尾分布，还可以进一步表示几种常见的分布。具体来说：
...
如果土耳其-拉姆达 PPCC 图给出的最大值`λ = 0.14`，那么我们可以得出结论，正态分布是数据的良好近似。如果最大值`λ < 0.14`，则意味着一个长尾分布，如双指数分布或逻辑分布将是更好的选择。如果最大值`λ = -1`，则意味着一个非常长的尾部分布，如柯西分布。如果最大值`λ > 0.14`，则意味着非常短的分布，如贝塔分布或均匀分布。

## 实现

在这个实现中，我们将生成不同的分布，检查它们的土耳其-λ形状参数值，并绘制 PPCC 图。我正在使用谷歌合作实验室，它包含一些预安装的库如`scipy`，`numpy`，`statsmodel`，`seaborn`等。但是，可以在本地环境中使用`pip install`轻松安装这些库。
