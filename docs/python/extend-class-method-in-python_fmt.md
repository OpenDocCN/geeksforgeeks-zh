...
> 原文：[https://www.geeksforgeeks.org/extend-class-method-in-python/](https://www.geeksforgeeks.org/extend-class-method-in-python/)

在 Python 中，当一个子类定义一个已经存在于其超类中的函数，以便以自己的方式添加一些其他功能时，子类中的函数被称为扩展方法，这种机制被称为扩展。这是 Python 展示[多态性](https://www.geeksforgeeks.org/polymorphism-in-python/)的一种方式。这类似于在 Java 中重写和在 C++中重写虚拟方法。来自子类实例的调用执行子类版本的函数。调用超类的版本使用`super()`方法。
