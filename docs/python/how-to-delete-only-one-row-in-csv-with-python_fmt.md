...
> 原文: [https://www.geeksforgeeks.org/how-to-delete-only-one-row-in-csv-with-python/](https://www.geeksforgeeks.org/how-to-delete-only-one-row-in-csv-with-python/)

**先决条件:** `Pandas`

人们可以通过 `Pandas` 库在 Python 中打开和编辑 CSV 文件。编辑文件时，可能需要删除文件中的整行。以下是一些不同的方法来做到这一点:
...
此方法仅适用于从数据集中移除第一行或最后一行。下面两行代码虽然意思相同，但代表了。`Pandas` 的 `iloc[]` 方法。
