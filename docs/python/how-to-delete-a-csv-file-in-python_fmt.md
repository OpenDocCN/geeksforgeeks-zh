...
> 原文：[https://www.geeksforgeeks.org/how-to-delete-a-csv-file-in-python/](https://www.geeksforgeeks.org/how-to-delete-a-csv-file-in-python/)

在本文中，我们将删除 Python 中的一个 CSV 文件。CSV（逗号分隔值文件）是处理表格数据最常用的文件格式。数据值由逗号（`,`）分隔。第一行给出列的名称，下一行给出每列的值。
...
我们可以使用 [`os.walk()`](https://www.geeksforgeeks.org/os-walk-python/) 功能遍历一个目录，删除特定的文件。在下面的例子中，我们将删除给定目录中的所有 `*.csv` 文件。
