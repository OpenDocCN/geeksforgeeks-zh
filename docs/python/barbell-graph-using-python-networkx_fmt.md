# 使用 Python NetworkX 绘制杠铃图

> 原文：[https://www.geeksforgeeks.org/barbell-graph-using-python-networkx/](https://www.geeksforgeeks.org/barbell-graph-using-python-networkx/)

**先决条件：** [`网络`](https://www.geeksforgeeks.org/networkx-python-software-package-study-complex-networks/)

杠铃图有很多种定义。最常用的是 n-杠铃图，这是一个简单的图，通过连接一个完整的图的两个副本与 n 个节点获得。在本文中，我们将看到如何使用 Python 绘制杠铃图。
...
2.  总边数（在 n-杠铃图中）：
    ...
    ```
    Total number of edges = 2*number of edges in complete graph + 1
    =2*(n*(n-1)/2)+1 = n*(n-1) + 1
    ```
...
*   **Python 中的 `networkx` 库**，用于实现和分析 Python 中不同种类的图形（数据结构）。对于安装，请使用以下命令：
...
**`barbell_graph(n, m)`**：它返回一个杠铃图，其中有两个由 `n` 个节点组成的完全图，这两个完全图之间通过 `m` 个节点桥连接。
...
当我们将 `(4, 2)` 作为参数传递给 `nx.barbell_graph()` 函数时，它被分配了一个由 2 个节点的桥连接的 4 个节点簇的图。最后，我们使用 `draw_networkx(G)` 函数得到了作为图 `G` 的视图的输出。
