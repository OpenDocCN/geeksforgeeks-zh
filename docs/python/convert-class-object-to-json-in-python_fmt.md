...
> 原文: [https://www.geeksforgeeks.org/convert-class-object-to-json-in-python/](https://www.geeksforgeeks.org/convert-class-object-to-json-in-python/)

类对象到 JSON 的转换是使用 Python 中的 [json](https://www.geeksforgeeks.org/read-write-and-parse-json-using-python/) 包完成的。`json.dumps()` 将 Python 对象转换为 json 字符串。每个 Python 对象都有一个由 `__dict__` 表示的属性，它存储对象的属性。
