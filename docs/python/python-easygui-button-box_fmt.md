...
用于在 EasyGUI 中显示一个有多个按钮的窗口，在有条件的情况下可以在很多按钮中选择一个，例如电梯中的按钮一次用户只能选择一个选项，下图是普通按钮框的样子
...
`buttonbox(text, title, button_list)`

### 参数

需要 3 个参数，第一个字符串即要显示的文本，第二个字符串即窗口标题和第三个按钮列表(字符串)。
