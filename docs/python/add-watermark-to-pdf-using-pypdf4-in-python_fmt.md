...
> 原文:[https://www . geesforgeks . org/add-watermark-to-pdf-using-py pdf 4-in-python/](https://www.geeksforgeeks.org/add-watermark-to-pypdf4-in-python/)
...
1.  **阅读原始输入`pdf`的页面**(使用`PdfFileReader()`类)
2.  **阅读**水印**的第一页**(使用`PdfFileReader()`类)
3.  使用`PdfFileWriter()`类创建一个`pdf_writer`对象
4.  接下来，就是在`input_pdf`中的页面上重复。
5.  **呼叫**`mergePage()`**和**把`watermark_page`传给它。
6.  **使用**`addPage()`**方法将此**合并页面添加到`pdf_writer`对象。
