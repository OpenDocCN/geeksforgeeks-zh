# 使用 pyqrcode 模块生成 QR Code

> 原文：[https://www.geeksforgeeks.org/python-generate-qr-code-using-pyqrcode-module/](https://www.geeksforgeeks.org/python-generate-qr-code-using-pyqrcode-module/)

来看看如何使用`pyqrcode`模块在 Python 中生成二维码。

`pyqrcode`模块是一个二维码发生器。该模块自动化了创建二维码的大部分构建过程。本模块试图尽可能遵循二维码标准。`pyqrcode`中使用的术语和编码直接来自标准。
