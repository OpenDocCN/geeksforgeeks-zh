...
> 原文：[https://www.geeksforgeeks.org/how-to-plot-normal-distribution-over-histogram-in-python/](https://www.geeksforgeeks.org/how-to-plot-normal-distribution-over-histogram-in-python/)
...
*   `random.normal()` 求数据正态分布的方法。它有三个参数：
    *   `loc` –（平均）钟形件顶部的位置。
    *   `scale` –（标准差）您希望图表分布的均匀程度。
    *   `size` – 返回阵列的形状
*   `Matplotlib` 库的 `Pyplot` 模块中的功能 `hist()` 用于绘制直方图。它有如下参数：
    *   `data`：该参数为数据序列。
    *   `bins`：此参数可选，包含整数、序列或字符串。
...
*   借助 `mean()` 和 `stdev()` 方法，计算均值和标准差，初始化为 `mean` 和 `sd` 变量。
*   在 `plot()` 方法中，我们使用了一种方法 `pdf()` 来显示概率密度函数。这个 `pdf()` 方法呈现在 `scipy.stats.norm` 里面。
...
> **参数：** 该方法使用以下参数，如下所述：
>
> *   `left`：使用该参数将 xlim 设置为左侧。
> *   `right`：使用该参数在右侧设置 xlim。
> *   `**kwargs`：该参数是控制标签外观的文本属性。
>
> **返回值：**
>
> *   `left, right`：返回 x 轴新极限值的元组。
