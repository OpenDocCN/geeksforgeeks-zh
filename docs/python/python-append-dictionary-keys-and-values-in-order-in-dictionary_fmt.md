...
> 原文: [https://www.geeksforgeeks.org/python-append-dictionary-keys-and-values-in-order-in-dictionary/](https://www.geeksforgeeks.org/python-append-dictionary-keys-and-values-in-order-in-dictionary/)

给定一个字典，在列表中执行关键字后接值的追加。

> **输入**: `test_dict = {"Gfg": 1, "is": 2, "Best": 3}`
> **输出**: `['Gfg', 'is', 'Best', 1, 2, 3]`
> **解释**: 列表中所有值之前的所有键。
>
> **输入**: `test_dict = {"Gfg": 1, "Best": 3}`
> **输出**: `['Gfg', 'Best', 1, 3]`
> **解释**: 列表中所有值前的所有键。

## 方法#1: 使用 `list()` + `keys()` + `values()`

这是执行这项任务的方法之一。在本例中，我们使用 `keys()` 和 `values()` 提取键和值，然后使用 `list()` 转换为列表，并按顺序执行追加。
