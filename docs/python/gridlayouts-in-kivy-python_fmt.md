...
*   初始大小由 `col_default_width` 和 `row_default_height` 属性给出。我们可以通过设置 `col_force_default` 或 `row_force_default` 属性来强制默认大小。这将强制布局忽略子级的 `width` 和 `size_hint` 属性，并使用默认大小。
*   若要自定义单个列或行的大小，请使用 `cols_minimum` 或 `rows_minimum`。
...
**注:** 了解如何使用 `.kv` 文件，请[访问本文档](https://kivy.org/doc/stable/guide/lang.html)。
