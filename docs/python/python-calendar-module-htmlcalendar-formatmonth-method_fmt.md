...
> 原文：[https://www.geeksforgeeks.org/python-calendar-module-htmlcalendar-formatmonth-method/](https://www.geeksforgeeks.org/python-calendar-module-htmlcalendar-formatmonth-method/)

日历模块允许像程序一样输出日历，并提供与日历相关的附加有用功能。日历模块中定义的函数和类使用理想化的日历，当前的阳历在两个方向上无限延长。

## 类 `calendar.HTMLCalendar(firstweekday=0)`

`calendar.HTMLCalendar(firstweekday=0)` 可以用来生成 HTML 日历。`formatmonth()` 方法是 `HTMLCalendar` 实例的方法之一。

## `formatmonth()` 方法

`formatmonth()` 方法在 Python 中是用来获取月历作为 HTML 表的。

> **语法：** `formatmonth(year, month, withyear=True)`
>
> **参数：**
> - `year`：日历年
> - `month`：日历月
