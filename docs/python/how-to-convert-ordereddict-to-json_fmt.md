...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-ordereddict-to-json/](https://www.geeksforgeeks.org/how-to-convert-ordereddict-to-json/)
...
*   JSON 中的文本是通过带引号的字符串完成的，该字符串包含 `{}` 内键值映射中的一个值。它类似于 Python 中的字典。JSON 展示了一个类似于标准库 `marshal` 和 `pickle` 模块的用户的应用编程接口，Python 本地支持 JSON 特性。
...
为了定义 `OrderedDict`，我们使用了 Python 中的 `collections` 模块。
...
要将 `OrderedDict` 转换为 JSON，我们使用的是 `json.dumps()`。
