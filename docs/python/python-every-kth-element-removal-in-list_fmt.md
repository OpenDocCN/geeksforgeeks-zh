...
> 原文: [https://www.geeksforgeeks.org/python-every-kth-element-removal-in-list/](https://www.geeksforgeeks.org/python-every-kth-element-removal-in-list/)

我们通常希望对列表中的所有元素使用特定的函数。但是有时，根据要求，我们希望删除列表中的某些元素，基本上是列表中的每个 Kth 元素。让我们讨论一下实现这一点的某些方法。

## 方法一：使用列表理解 + `enumerate()`

获取列表每第 k 个数的功能可以借助列表理解来完成，`enumerate()`功能有助于整个列表的迭代。
