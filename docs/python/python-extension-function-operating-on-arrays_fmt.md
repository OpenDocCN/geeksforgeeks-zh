...
> 原文: [https://www.geeksforgeeks.org/python-extension-function-operating-on-arrays/](https://www.geeksforgeeks.org/python-extension-function-operating-on-arrays/)

让我们编写一个 C 扩展函数，它可以对连续的数据数组进行操作，这可能是由数组模块或像 NumPy 这样的库创建的，并且这个函数应该是通用的，而不是特定于任何一个数组库的。

代码应该使用缓冲协议以可 portable 的方式接收和处理数组。下面的代码是一个 C 扩展函数，它接收数组数据，并使用 Python 中的 C 代码调用本文中的 `avg(double *buf, int len)` 函数。
...
*   例如，`PyBUF_ANY_CONTIGUOUS` 指定需要一个连续的内存区域。