...
> 原文: [https://www.geeksforgeeks.org/manipulate-date-and-time-with-the-datetime-module-in-python/](https://www.geeksforgeeks.org/manipulate-date-and-time-with-the-datetime-module-in-python/)
...
为了获得当前日期和时间，使用了 `datetime.now()` 方法。它返回本地当前日期和时间。
...
在世界各个地区，使用不同类型的日期格式。`DateTime` 提供了 [`strftime()`](https://www.geeksforgeeks.org/python-strftime-function/) 的方法来处理这样的格式化。此函数用于将日期和时间对象转换为其字符串表示形式。它接受一个或多个格式化代码输入，并返回字符串表示形式。
...
不包含任何时区信息的日期时间对象被称为幼稚日期时间对象。对于初始日期时间对象，`datetime.tzinfo` 将为 `None`。感知日期时间对象包含嵌入其中的时区信息。
