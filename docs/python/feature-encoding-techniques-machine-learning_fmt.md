...
> 原文: [https://www.geeksforgeeks.org/feature-encoding-techniques-machine-learning/](https://www.geeksforgeeks.org/feature-encoding-techniques-machine-learning/)
...
标签编码算法相当简单，它考虑了编码的顺序，因此可以用来编码序数数据。
...
克服标签编码的缺点，因为它考虑了列中的一些层次结构，这些层次结构可能会误导数据中存在的标称特征。我们可以使用 One-Hot 编码策略。
...
我们可以使用 Scikit learn 类中提供的序数编码对序数特征进行编码。它确保变量的有序性得以保持。
...
哈希是应用哈希函数将字符串转换为唯一哈希值的过程。这个过程非常有用，因为它可以处理大量的分类数据和低内存使用。
...
目标编码很好，因为它拾取了可以解释目标的值。它被大多数高尔夫球手在比赛中使用。用目标变量的平均值替换分类值的基本思想。
