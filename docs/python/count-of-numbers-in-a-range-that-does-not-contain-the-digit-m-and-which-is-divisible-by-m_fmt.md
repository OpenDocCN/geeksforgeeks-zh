...
> 原文：[https://www.geeksforgeeks.org/count-of-numbers-in-a-range-that-does-not-contain-the-digit-m-and-which-is-divisible-by-m/](https://www.geeksforgeeks.org/count-of-numbers-in-a-range-that-does-not-contain-the-digit-m-and-which-is-divisible-by-m/)

给定三个整数，下范围 `L`，上范围 `U`，一个数字 `M`。任务是计算 `L` 和 `U` 之间的所有数字，使得这个数字可以被 `M` 整除，而且它不包含数字 `M`。
