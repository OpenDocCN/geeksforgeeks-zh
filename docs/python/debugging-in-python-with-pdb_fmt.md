...
1.  要导入，我们只需在代码中使用`import pdb`。
2.  对于调试，我们将使用`pdb.set_trace()`方法。现在，在 Python 3.7 中`breakpoint()`方法也可用于此。
...
要了解不同命令的更多信息，您可以键入`help`并获取所需信息。
...
类似地，我们可以使用`breakpoint()`方法(不需要导入 pdb)。
...
就像用`p`和`pp`打印发音一样，您可以使用命令`show [expression]`建议 pdb 在执行停止时显示发音的估计值，以避免发音发生变化。利用命令`undisplay [expression]`来明确展示清晰度。
...
在堆栈跟踪中将当前帧`count`(默认为 1)上移一级(移至较旧的帧)。

```py
d(own) [count]
```

在堆栈跟踪中将当前帧`count`(默认为 1)下移一级(至新帧)。
