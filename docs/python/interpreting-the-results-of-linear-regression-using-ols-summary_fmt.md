...
> 原文：[https://www.geeksforgeeks.org/interpreting-the-results-of-linear-regression-using-ols-summary/](https://www.geeksforgeeks.org/interpreting-the-results-of-linear-regression-using-ols-summary/)
...
## 因变量
因变量是将依赖于其他变量的变量。在这个回归分析中 `Y` 是我们的因变量，因为我们要分析 `X` 对 `Y` 的影响。

## 模型
由于其高效性，`普通最小二乘法(OLS)` 是应用最广泛的模型。该模型给出了真实人口回归线的最佳近似。OLS 的原则是误差平方最小化( `∑e_i^2` )。

## 观察次数
观察次数是我们样本的大小，即 N = 150。
...
## 常数项
常数项是回归线的截距。从回归线(方程式 1)来看，截距为-3.002。在回归中，我们省略了一些对因变量没有太大影响的自变量，截距表示这些省略变量的平均值和模型中存在的噪声。

## 系数项
系数项表示 `X` 单位变化的 `Y` 的变化，即如果 `X` 上升 1 个单位，则 `Y` 上升 0.7529。如果你熟悉导数，那么你可以把它联系为 `Y` 相对于 `X` 的变化率。

## 参数的标准误差
标准误差也叫标准差。标准误差显示了这些参数的采样可变性。标准误差计算如下–
...
这里，`σ^2` 是回归的 `标准误差(SER)`。和 `σ^2` 等于 RSS(平方和残差即 `∑e_i^2`)。

### t–统计
理论上，我们假设误差项遵循正态分布，因此参数 `b_1` 和 `b_2` 也具有正态分布，方差在上面部分中计算。

也就是说，

*   `b_1 ~ N(B_1, σ^2 / ∑(x_i - x̄)^2)`
*   `b_2 ~ N(B_2, σ^2 / ∑(x_i - x̄)^2)`

这里 `B_1` 和 `B_2` 是 `b1` 和 `b2` 的真实值。

t–通过假设以下假设计算统计数据–

*   `H_0 : B_2 = 0(变量 X 对 Y 无影响)`
*   `H_a: B_2 ≠ 0(X 对 Y 有重大影响)`

t 统计的计算:

`t = (b_1 – B_1) / s.e(b_1)`

从汇总表来看，`b_1 = -3.2002`，`se(b_1) = 0.257`，所以，

`t = (-3.2002 – 0) / 0.257 = -12.458`

同样，`b_2 = 0.7529`，`se(b_2) = 0.044`

`t = (0.7529 – 0) / 0.044 = 17.296`

### p–值
理论上，我们认为 p 值是获得至少与假设零假设为真所计算的 `H_0` 相矛盾的 t 统计量的概率。在汇总表中，我们可以看到两个参数的 P 值都等于 0。这并不完全是 0，但是由于我们有非常大的 t 统计(-12.458 和 17.296)，p 值将大约为 0。

如果你知道显著性水平，那么你可以看到我们可以在几乎每个显著性水平上拒绝零假设。

### 置信区间
检验假设的方法有很多，包括上面提到的 p 值方法。置信区间方法就是其中之一。5%是标准显著性水平(α)，在此水平上进行 C.I。

C.I 对于 `B_1` 是 `(b_1 – t_{α/2} * s.e(b_1), b_1 + t_{α/2} * s.e(b_1))`

由于 `α = 5%`，`b_1 = -3.2002`，`s.e(b_1) = 0.257`，由 t 表可知，`t_{0.025, 148} = 1.655`，

放入数值后，`B_1` 的 C.I 约为 ( -3.708，-2.693)。`b_2` 也可以这样做。
...
### r–平方值
`R^2` 是决定系数，告诉我们自变量可以解释多少百分比变化的自变量。这里 Y 的 66.9 %的变异可以用 x 来解释，`R^2` 的最大可能值可以是 1，意味着 `R^2` 值越大回归越好。
