...
> 原文: [https://www.geeksforgeeks.org/count-of-pairs-of-strings-which-differ-in-exactly-one-position/](https://www.geeksforgeeks.org/count-of-pairs-of-strings-which-differ-in-exactly-one-position/)
...
> 让 `str1 = "ABC"` 和 `str2 = "ADC"`
> 对于 `str1`，添加 `"#BC"`、`"a#c"` 和 `"ab#"` 到一个[集合](https://www.geeksforgeeks.org/set-in-cpp-stl/)。
> 现在对于 `str2`，以类似的方式生成字符串，如果任何生成的字符串已经存在于集合中，则两个字符串在精确的 1 个索引位置上不同。
> 例如，`"a#c"` 就是生成的字符串之一。
