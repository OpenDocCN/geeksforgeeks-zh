...
**标记图像**是整数图像，其中的值对应于不同的区域。也就是说，区域 1 是值为 1 的所有像素，区域 2 是值为 2 的像素，以此类推。为了做到这一点，我们将使用 `mahotas.label.filter_label` 方法。

> **语法:** `mahotas.label.filter_label(label_image, filter1, filter2)`
