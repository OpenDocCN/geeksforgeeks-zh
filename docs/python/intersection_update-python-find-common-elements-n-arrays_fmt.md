...
假设我们有两个集合 A 和 B，那么 `A.intersection_update(B)` 操作用集合 A 和 B 中的公共元素更新集合 A。比如 `A = set([1, 2, 3])` 和 `B = set([4, 2, 3])`，现在取 `A.intersection_update(B)` 后，集合 A 的值将为 `[2, 3]`。语法为 `anySet.intersection_update(iterable)`。
