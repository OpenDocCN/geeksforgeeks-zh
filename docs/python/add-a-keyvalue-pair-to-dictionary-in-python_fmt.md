...
> 原文：[https://www.geeksforgeeks.org/add-a-keyvalue-pair-to-dictionary-in-python/](https://www.geeksforgeeks.org/add-a-keyvalue-pair-to-dictionary-in-python/)

[Python 中的 Dictionary](https://www.geeksforgeeks.org/python-dictionary/) 是一个无序的数据值集合，用于像映射一样存储数据值，与其他只保存单个值作为元素的数据类型不同，`Dictionary` 保存 `key:value` 对。
