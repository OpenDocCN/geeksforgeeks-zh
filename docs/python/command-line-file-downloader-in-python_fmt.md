...
> 原文: [https://www.geeksforgeeks.org/command-line-file-downloader-in-python/](https://www.geeksforgeeks.org/command-line-file-downloader-in-python/)
...
我们将使用 `requests` 通过 HTTP 打开一个文件流，然后将数据分块保存到本地文件中。让我们看看代码的样子，然后把它们放在一起。
...
输出将包括一个带有下载细节的动态进度条。为此，我们使用 `sys.stdout.write()` 和 `sys.stdout.flush()` 方法。
