...
> 原文：[https://www.geeksforgeeks.org/create-custom-datatypes-using-pydantic-module-in-python/](https://www.geeksforgeeks.org/create-custom-datatypes-using-pydantic-module-in-python/)
...
运行后，第一个`print`语句将成功执行，但是在下一次初始化`User`模型时，它将抛出类型为`ValueError`的*验证错误*，因为个人详细信息和地址的`id`不匹配。