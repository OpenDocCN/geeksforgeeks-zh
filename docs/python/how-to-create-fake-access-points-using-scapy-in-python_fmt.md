...
> 原文：[https://www.geeksforgeeks.org/how-to-create-fake-access-points-using-scapy-in-python/](https://www.geeksforgeeks.org/how-to-create-fake-access-points-using-scapy-in-python/)
...
*   `scapy`
*   `Internet Protocol`（Internet Protocol 的缩写）
*   `airmon-ng`
*   `dnsmasq`（可选）
...
现在我们将使用 `addr2` 和 `addr3` 的相同 MAC 地址，因为发送方是接入点。然后我们将使用 `Dot11Elt` 创建信标帧，然后堆叠在一起，并使用 `scapy` 模块的 `sendp()` 方法发送它们。下面是实现：
...
当您到达脚本的最后一行并且 `loop=0` 时，您的系统将只发送 1 个数据包作为接入点。

![](img/de02ceffb089e40e5f93db0019b355ee.png)

当你到达最后一行代码和 `loop=1` 时，由于系统作为接入点连续发送数据包，将会产生如下输出。
