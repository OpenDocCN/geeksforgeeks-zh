...
>
> *   `cols`: 要排序的列列表
> *   `args`: 指定列中列出的列的排序顺序(即升序或降序)
>
> **返回类型:** 返回按指定列排序的新的`数据框`。

**数据框创建:** 创建一个名为`Spark`的新`SparkSession`对象，然后使用自定义数据创建一个`数据框`。
...
按照数据框中员工`Job`的降序和`Salary`的升序对数据框进行排序。当具有相同`Job`的两行之间发生冲突时，将通过按`Salary`的升序列出行来解决。
