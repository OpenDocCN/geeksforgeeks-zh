...
> 原文: [https://www.geeksforgeeks.org/exporting-variable-to-csv-file-in-python/](https://www.geeksforgeeks.org/exporting-variable-to-csv-file-in-python/)

CSV 文件或逗号分隔值文件用于跨平台存储和共享数据。列由逗号或其他相关分隔符分隔，使数据具有表格结构。

有时我们会遇到现成的 CSV 文件，而有时我们需要根据项目中的要求创建一个。Python 可以使用许多模块编写 CSV 文件。Python 标准库中的 [`csv`](https://www.geeksforgeeks.org/reading-and-writing-csv-files-in-python/) 模块提供了类和方法来[读取并将数据写入 csv 文件](https://www.geeksforgeeks.org/working-csv-files-python/)。
...
*   `csv.writerow()` – 该函数将项目写入一个可迭代对象（列表、元组或字符串）中，用分隔符将它们分开。
*   `csv.writerows()` – 该函数将可迭代对象列表作为参数，并将它们写入新行。
