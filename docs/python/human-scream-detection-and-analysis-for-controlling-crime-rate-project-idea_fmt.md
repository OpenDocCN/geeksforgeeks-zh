...
> 原文：[https://www.geeksforgeeks.org/human-scream-detection-and-analysis-for-controlling-crime-rate-project-idea/](https://www.geeksforgeeks.org/human-scream-detection-and-analysis-for-controlling-crime-rate-project-idea/)
...
4.  机器学习需要神经网络知识。
5.  必须熟悉库，如 `pandas`、`NumPy`、`scikit-learn`、`TensorFlow` 和 `librosa`。
...
现在，我们将对数据集中存在的声音训练 `MPN`（多层感知器模型），以获得模型的良好精度，当整个训练完成后，再像前面一样使用 `TensorFlow` 库保存它。`MPN` 的工作图如下。
...
该研究论文已在`第五届发明系统与控制国际会议【ICISC 2021】`上发表，主题为[通过三阶段监督学习和深度学习进行人类尖叫检测](https://link.springer.com/chapter/10.1007/978-981-16-1395-1_28)。
...
**Github 链接：** [https://github.com/themockingjester/Human_Scream_Detection_using_ml_and_deep_learning](https://github.com/themockingjester/Human_Scream_Detection_using_ml_and_deep_learning)