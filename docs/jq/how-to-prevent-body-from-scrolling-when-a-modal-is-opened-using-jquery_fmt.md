...
> 原文: [https://www.geeksforgeeks.org/how-to-prevent-body-from-scrolling-when-a-modal-is-opened-using-jquery/](https://www.geeksforgeeks.org/how-to-prevent-body-from-scrolling-when-a-modal-is-opened-using-jquery/)

给定一个带有模式的 HTML 文档，任务是在模式处于打开状态时防止主体元素滚动。使用 JavaScript 可以轻松完成这项任务。

## 方法

这个问题的一个简单解决方案是，每当打开模态时，将主体元素的 `overflow` 属性的值设置为 `hidden`，这将禁用所选元素上的滚动。一旦模态关闭，我们将主体元素的 `overflow` 属性设置为 `auto`，以便在主体元素上启用滚动功能。为了确定模态是否被打开，我们将使用 JavaScript 的 `classList.contains()` 方法检查它的类列表中是否有隐藏的 `hidden` CSS 类。这个 `hidden` 类负责按钮点击时模态的打开和关闭（改变显示属性）。为了更好地理解，请查看给定的示例。
