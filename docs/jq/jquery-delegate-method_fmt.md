...
jQuery 中的 `delegate()` 方法用于向指定的元素添加一个或多个事件处理程序，这些事件处理程序是所选元素的子元素，也用于指定每当事件发生时运行的函数。
...
*   `event`: 为必选项，用于指定附加到元素的一个或多个事件。多个事件值由空格分隔，并且必须是有效的事件。
*   `data`: 为可选参数，用于指定传递给函数的附加数据。
*   `function`: 为必选项，用于指定事件发生时要运行的功能。
