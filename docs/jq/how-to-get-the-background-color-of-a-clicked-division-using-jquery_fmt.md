...
> 原文：[https://www.geeksforgeeks.org/how-to-get-the-background-color-of-a-clicked-division-using-jquery/](https://www.geeksforgeeks.org/how-to-get-the-background-color-of-a-clicked-division-using-jquery/)

在本文中，我们将学习如何使用 jQuery 获取被点击的分部的背景色。

## 方法

首先使用公共选择器选择页面上的所有分区，并使用 `click()` 方法应用点击绑定来触发颜色检测。然后通过使用 `this` 作为选择器，可以找到当前点击的分区。

jQuery 中的 `css()` 方法用于获取和设置它所使用的元素的计算样式。它接受两个参数，其中第一个参数定义了我们需要获取或设置样式的样式，第二个参数定义了必须设置的值。我们可以使用这种方法，通过传递参数 `"background-color"` 来获取当前的背景色。然后，这可以显示为 RGB 值中的文本，或者分配给另一个元素。
