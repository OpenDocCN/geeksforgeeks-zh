# jQuery Mobile Collapsibleset Widget 的 collapsedIcon 选项
...
jQuery Mobile 是一种基于网络的技术，用于制作可在所有智能手机、平板电脑和台式机上访问的响应内容。在本文中，当内容处于折叠状态时，我们将使用 jQuery Mobile `collapsibleset` Widget 的 `collapsedIcon` 选项来设置可折叠容器标题的图标。它接受字符串类型值，默认值为 `"plus"`。
...
[https://api.jquerymobile.com/collapsibleset/#option-collapsedIcon](https://api.jquerymobile.com/collapsibleset/#option-collapsedIcon)