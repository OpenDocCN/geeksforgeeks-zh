...
> 原文：[https://www.geeksforgeeks.org/how-to-fire-an-event-on-file-select-using-jquery/](https://www.geeksforgeeks.org/how-to-fire-an-event-on-file-select-using-jquery/)

给定一个 HTML 文档和使用 jQuery 选择文件时触发事件的任务。`change()`方法用于在选择文件时触发事件。

## 使用 `change()` 方法

`change()`方法用于更改输入字段的值。这个方法适用于`input`、`textarea`和`select`元素。
