...
> 原文: [https://www.geeksforgeeks.org/jquery-dblclick-with-examples/](https://www.geeksforgeeks.org/jquery-dblclick-with-examples/)

`dblclick()` 是 jQuery 中的一个内置方法，用于触发双击事件发生。当双击选定的元素时，会出现此方法。
...
这里的 `selector` 是被选择的元素。

## 参数

它接受一个可选参数 `args`，指定双击后执行特定任务的函数。
