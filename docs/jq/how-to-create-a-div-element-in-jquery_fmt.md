...
> 原文: [https://www.geeksforgeeks.org/how-to-create-a-div-element-in-jquery/](https://www.geeksforgeeks.org/how-to-create-a-div-element-in-jquery/)

有多种方法可以使用 jQuery 在 HTML 文档中创建 HTML 元素。但最简单的是 `append()` 和 `prepend()` 的方法。
...
本示例使用 `appendTo()` 方法在选定元素的末尾创建 div 元素。
...
本示例使用 `appendTo()` 方法在 div 元素内创建 div 元素。
