...
> 原文: [https://www.geeksforgeeks.org/how-to-remove-parent-element-except-its-child-element-using-jquery/](https://www.geeksforgeeks.org/how-to-remove-parent-element-except-its-child-element-using-jquery/)

给定一个 HTML 文档，任务是移除除子元素之外的父元素。

## 方法 1

使用 `contents()` 方法选择所有直接子元素，包括所选元素的文本和注释节点。选定的元素存储在变量中。现在，使用 `replaceWith()` 方法将父元素的内容替换为存储在变量中的所有子元素。

### 示例

本示例使用 `contents()` 和 `replaceWith()` 方法移除父元素，但其子元素除外。
