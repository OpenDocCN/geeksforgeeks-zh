...
> 原文：[https://www.geeksforgeeks.org/how-to-get-relative-click-coordinates-on-the-target-element-using-jquery/](https://www.geeksforgeeks.org/how-to-get-relative-click-coordinates-on-the-target-element-using-jquery/)
...
*   将单击事件附加到元素。
*   事件发生时调用匿名函数。
*   通过减去`offset().left`计算相对于父元素的`X`。`pageX`属性中的`left`属性。
*   同样，通过减去`offset().top`计算相对于父元素的`Y`。`pageY`属性中的`top`属性。
