...
> 原文：[https://www.geeksforgeeks.org/jquery-callbacks-disable-method/](https://www.geeksforgeeks.org/jquery-callbacks-disable-method/)

jQuery 中的 `Callbacks.disable()` 方法用于禁止回调列表进一步执行任何其他操作。这个方法返回它所附着的回调对象（this）。
...
### 示例 2
这个示例提供了一个按钮，首先禁用回调，然后添加并激发方法来查看结果。
