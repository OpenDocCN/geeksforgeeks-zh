...
> 原文：[https://www.geeksforgeeks.org/why-error-is-not-defined-occured-in-jquery/](https://www.geeksforgeeks.org/why-error-is-not-defined-occured-in-jquery/)
...
在上面的例子中，我们看到 `num` 在声明之前已经被调用过了。这就是为什么引用错误 `num 没有定义` 被扔在第一行的原因。在第三行中，再次调用 `num`。但是，这次不会抛出错误，因为变量已经在脚本的第二行中定义了。
...
### 在 jQuery 脚本文件前嵌入 jQuery 插件

`$` 用于声明 jQuery 中的任何变量。插件基本上是预先编写的代码块。这些块使用预定义的 jQuery 函数和方法。因此，有必要在插件文件之前嵌入 jQuery 脚本文件。否则，应用程序将无法理解 jQuery 编码。
