...
包含单参数 `selector`，必选。它指定不选择的元素。这个参数接受所有类型的选择器。

## jQuery `not()` 方法

此方法返回与定义条件不匹配的元素。此方法指定一个条件。不匹配该条件的元素将被返回，而匹配的元素将被移除。此方法主要用于从一组选定元素中移除一个或多个元素。
...
*   `condition`: 此参数为可选。它指定了一个选择器表达式、一个 jQuery 对象或一个或多个要从一组选定元素中移除的元素。
*   `function(index)`: 此参数可选。它为组中的每个元素指定一个要运行的函数。如果返回 `true`，则移除该元素，否则保留该元素。
    *   `index`: 指定元素在集合中的索引位置。

## 示例 1

在本示例中，首先选择开始的所有等级 `GFG-` ，然后使用 `not()` 方法从选择中移除等级 `GFG-1`。
...
在本示例中，首先选择起始的所有等级 `GFG-` ，然后使用 `:not` 选择器从选择中移除等级 `GFG-1`。
