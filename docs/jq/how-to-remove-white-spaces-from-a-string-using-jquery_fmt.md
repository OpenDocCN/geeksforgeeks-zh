...
> 原文: [https://www.geeksforgeeks.org/how-to-remove-white-spaces-from-a-string-using-jquery/](https://www.geeksforgeeks.org/how-to-remove-white-spaces-from-a-string-using-jquery/) 从字符串中删除空格
...
在这种情况下，我们在 `<pre>` 标签中写入包含空格的字符串，然后应用 `trim()` 方法从开始和结束位置移除所有空格。删除空白后，我们使用 `html()` 方法显示修剪后的字符串。
