...
jQuery 中的 `escapeSelector()` 方法用于转义具有特殊有效字符或字符串的 CSS 选择器。它可以选择 id 的元素（`#ID1`，`#ID2`）和 class 的元素（`.class1`，`.class2`）。让我们借助几个例子来理解它。
...
该方法接受单参数 `selector`，该选择器保存包含要转义的选择器表达式的字符串（例如 `#ID1` 等）。

## 示例 1

本示例选择 class = '.list' 的元素。
