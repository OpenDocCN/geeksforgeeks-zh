...
[`jQuery`](https://www.geeksforgeeks.org/jquery-tutorials/) 提供前端日历 [`CLNDR`](https://github.com/kylestetz/CLNDR) 插件使用的 HTML 模板多用于单日或多日事件管理系统。它用于以日历格式指定各种事件的日期。
你必须在工作文件夹中下载需要的文件，这样程序员就可以把它包含在 HTML 结构页面的头部，如下面的程序中实现的那样。

## 注意

在头部包含【`下划线.js`】和【`矩.js`】库，在 [`clndr.js`](https://github.com/kylestetz/CLNDR) 之前作为 [`CLNDR`](https://github.com/kylestetz/CLNDR) 插件就靠这些库。[下载 [`CLNDR`](https://github.com/kylestetz/CLNDR) 插件的链接](https://github.com/kylestetz/CLNDR)。

## 示例 1：基本用法

以下示例演示了以日历格式显示当前月份的 [`CLNDR`](https://github.com/kylestetz/CLNDR) 插件的基本用法。像“显示相邻月份”和“相邻日期日历月”这样的选项被设置为 `true`，以便快速轻松地遍历前几个月和后几个月。
...
以下示例演示了使用 [`CLNDR`](https://github.com/kylestetz/CLNDR) 插件传递事件以显示信息。在 [`jQuery`](https://www.geeksforgeeks.org/jquery-tutorials/) 代码中，事件列表以数组的形式传递给 [`CLNDR`](https://github.com/kylestetz/CLNDR) `click` 事件。一旦用户点击事件日期，它就会以“红色”显示日期，并在底部显示事件标题。一周中的几天也以特定的格式设置。程序员可以根据应用需求设置其他选项。
...
**输出:**
[`CLNDR`](https://github.com/kylestetz/CLNDR) 插件还可以通过访问数组对象中事件的 `start` 和 `end` 日期，以类似于单日事件的方式处理多日事件。

![](img/2d0e7c25a276d5a1b0b17bcc4ac5257b.png)

## 示例 3：日期范围约束

以下示例演示了带有一些约束的 [`CLNDR`](https://github.com/kylestetz/CLNDR) 插件，例如在日历中选择特定范围内的日期。它有助于处理 web 应用程序中的大量用户和日期验证。
