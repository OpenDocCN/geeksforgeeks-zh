...
> 原文: [https://www.geeksforgeeks.org/how-to-check-whether-a-value-is-numeric-or-not-using-jquery/](https://www.geeksforgeeks.org/how-to-check-whether-a-value-is-numeric-or-not-using-jquery/) 检查值是否为数字

给定一个输入元素，任务是使用 jQuery 检查输入的值是否是数字。jQuery `$.isNumeric()` 方法用于检查输入的数字是否为数字。

## `$.isNumeric()` 方法
用于检查给定的参数是否为数值。如果是数字，则返回真，否则返回假。
...
## 示例 1
本示例使用 `jQuery.isNumeric()` 方法检查输入的元素是否为数字。
