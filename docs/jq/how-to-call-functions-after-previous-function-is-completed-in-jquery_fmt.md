...
> 原文：[https://www.geeksforgeeks.org/how-to-call-functions-after-previous-function-is-completed-in-jquery/](https://www.geeksforgeeks.org/how-to-call-functions-after-previous-function-is-completed-in-jquery/)
...
在这个示例中，我们将一个回调函数作为参数传递给函数一，这样它就可以在执行结束时调用这个新的回调函数。任务是先执行 `callbackSecond` 函数，然后执行 `callbackSecond` 函数。首先将 `callbackSecond` 作为参数传递给回调。
