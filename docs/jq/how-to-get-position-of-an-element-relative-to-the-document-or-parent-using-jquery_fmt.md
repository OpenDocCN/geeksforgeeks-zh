...
为了获得一个元素相对于文档的位置，使用了 jQuery [`offset()`方法](https://www.geeksforgeeks.org/jquery-offset-with-examples/)。[`offset()`方法](http://geeksforgeeks.org/jquery-offset-with-examples/) 是 jQuery 中的内置方法，用于设置或返回所选元素的偏移坐标。我们也可以使用 jQuery [`position()`方法](https://www.geeksforgeeks.org/jquery-position-with-examples/)。`position()`方法是 jQuery 中的一个内置方法，用于查找 DOM 树中第一个匹配元素相对于其父元素的位置。
