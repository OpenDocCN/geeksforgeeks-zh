...
> 原文：[https://www.geeksforgeeks.org/how-to-clone-a-block-using-jquery/](https://www.geeksforgeeks.org/how-to-clone-a-block-using-jquery/)
...
*   `true` 指定也应该复制事件处理程序。
*   `false`（默认）事件处理程序不应被复制。

## 示例 1

在本例中，我们没有向 `clone()` 方法传递任何值。
...
在本例中，我们将 `true` 值传递给 `clone()` 方法，该方法也复制事件处理程序。在这里，每当用户单击段落时，我们都会更改段落的背景颜色。正如您在输出中看到的，这将被复制。
