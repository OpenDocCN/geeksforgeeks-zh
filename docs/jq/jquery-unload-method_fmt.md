...
jQuery 中的 `unload()` 方法用于在用户尝试导航离开当前网页时执行卸载事件。当用户改变页面的动态状态时，例如用户点击链接离开页面，在地址栏中键入新的网址等，可以触发该事件。`unload` 方法应该只用于窗口对象。它指定卸载事件发生时会发生什么。
...
*   `function`：是必选参数，用于指定触发卸载事件时要运行的功能。
...
<body style="text-align:center;">
    ...
    <p>When you click <a href="https://ide.geeksforgeeks.org">GeeksForGeeks</a>,
...
*   卸载事件的工作依赖于浏览器。
*   `unload()` 方法在 3.0 版中被删除。
*   jQuery 1.8 版中不推荐使用 `unload()` 方法。