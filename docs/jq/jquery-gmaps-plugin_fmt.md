...
jQuery 提供了 `GMaps` 插件，帮助程序员以多种方式使用谷歌地图。您必须在工作文件夹中下载所需的文件，以便程序员可以在 HTML 结构页面的标题部分中包含以下程序中实现的内容。
...
**示例 3:** 以下程序演示了插件的事件处理功能。它在点击和拖动事件上显示消息。
...
**示例 4:** 以下示例演示了 `GMap` 插件的开放街道地图地图类型。
