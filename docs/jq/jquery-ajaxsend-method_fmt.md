...
jQuery 中的 `ajaxSend()` 方法用于指定当 AJAX 请求即将发送时要运行的函数。
...
该方法接受单参数函数，该函数是强制的。该函数接受三个参数，如上所述，如下所述:

*   `event`: 保存事件对象。
*   `xhr`: 它保存 `XMLHttpRequest` 对象。
*   `options`: 保存 AJAX 请求中使用的选项。
...
本示例通过从服务器获取数据来更改 `<p>` 元素的内容。当 AJAX 请求准备发送时，页面显示 "AJAX 请求即将发送"。
...
本示例通过从服务器获取数据来更改 `<h1>` 元素的内容。当 AJAX 请求准备发送时，页面显示 "AJAX 请求即将发送"。
