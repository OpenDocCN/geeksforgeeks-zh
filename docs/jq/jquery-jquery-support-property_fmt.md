...
> 原文: [https://www.geeksforgeeks.org/jquery-jquery-support-property/](https://www.geeksforgeeks.org/jquery-jquery-support-property/)
...
该属性包含单个参数 `属性值`，这是必需的。它用于指定要测试的函数。包括的测试有:
...
本示例使用 `jQuery.support` 属性向浏览器发送文本，以创建一个 `XMLHttpRequest` 对象。
...
此示例用 `cors` 参数值说明了 `jQuery.support` 属性。
