...
## 示例 1：jQuery 代码展示 `noConflict()` 方法的工作原理
...
**输出：** 在这里可以看到`$`标志的控制解除后，我们就不能再使用`$`标志了。这就是为什么当我们再次用`$`符号调用`length`属性时，我们收到了一个错误。所以基本上，调用`noConflict()`方法后，我们要用`jQuery`代替`$`符号。而`$`标志将被其他库使用。
...
`noConflict()`方法返回一个对 jQuery 的引用，我们可以将其存储在一个变量中以备将来使用。
...
Rohan 正在用 jQuery 做他的学校项目，他已经写了大约 100 行 jQuery 代码。他即将完成他的项目，但后来他想在他的项目中增加一些功能。所以他添加了一些同样使用`$` sign 的 JavaScript 库，所以他使用了`noConflict()`方法。但是当他添加了`noConflict()`方法时，他意识到他必须用 jQuery 替换所有先前的`$`符号。

**不……** 我们不需要用 jQuery 改变每个`$`符号。

## 在函数内部使用 `$` 符号

如果我们有一个使用`$`符号的函数，我们可以将`$`符号作为参数传递给该函数。这将允许我们在函数内部使用`$`符号，但是我们仍然必须在函数外部使用 jQuery。
