...
jQuery 中的 `outerWidth()` 方法用于返回包含边框和填充的元素的外宽度值。
...
该方法接受单个可选参数 `includeMargin`。它包含布尔值，用于指定是否包含边距。如果 `includeMargin` 设置为 `true`，则包含边距，否则不包含边距。默认情况下，`includeMargin` 设置为 `false`。
...
<head>
    ...
    <!-- Script to display outerWidth of box including margin -->
    <script>
        $(document).ready(function() {
            $("button").click(function(){
                alert("Outer width of div: " + $("div").outerWidth(true));
            });
        });
    </script>
</head>
