...
> 原文：[https://www.geeksforgeeks.org/how-to-disable-browser-autofill-on-input-fields-using-jquery/](https://www.geeksforgeeks.org/how-to-disable-browser-autofill-on-input-fields-using-jquery/)
...
- 从 `<script>` 标记中导入 jQuery CDN，以便在页面上使用 jQuery。
- 然后在 `<script>` 标记中编写 jQuery 代码，以禁用输入字段的自动填充。

## 为了实现这一点，我们使用 jQuery 的两种方法为字段设置属性值

- [`attr()` 方法](https://www.geeksforgeeks.org/jquery-attr-method/)
- [`prop()` 方法](https://www.geeksforgeeks.org/jquery-prop-with-examples/)
