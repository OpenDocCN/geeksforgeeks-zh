...
> 原文: [https://www.geeksforgeeks.org/how-to-check-an-element-is-visible-or-not-using-jquery/](https://www.geeksforgeeks.org/how-to-check-an-element-is-visible-or-not-using-jquery/)

给定一个 HTML 文档，任务是使用 jQuery `:visible` 选择器检查元素是**可见**还是**不可见**。`:visible` 选择器可与 `toggle()` 功能一起使用以切换元素的可见性。它将与元素可见性一起工作: `display: none;` 或 `opacity: 0;`。
...
本示例使用 `:visible` 选择器使用 jQuery 检查元素是否可见。
