...
借助 [`val()` 方法](https://www.geeksforgeeks.org/jquery-val-with-examples/)，我们可以得到 jQuery 中 `textarea` 的值。`val()` 方法用于从`textarea`等元素中获取值，输入并选择。此方法只是返回或设置所选元素的 value 属性，主要用于表单元素。当在空集合上调用时，或者当没有给出输入时，它返回未定义或空白的输出。
