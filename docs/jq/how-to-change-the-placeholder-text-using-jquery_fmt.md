...
> 原文: [https://www.geeksforgeeks.org/how-to-change-the-placeholder-text-using-jquery/](https://www.geeksforgeeks.org/how-to-change-the-placeholder-text-using-jquery/)
...
在这个方法中我们将使用 [`attr()`](https://www.geeksforgeeks.org/jquery-attr-method/) 方法。`attr()` 方法用于设置或返回所选元素的指定属性。它需要两个参数，第一个是要设置的属性，第二个是要更改为的属性值。

`placeholder` 属性控制将在输入区域显示为占位符的文本。该属性作为第一个参数传递给 `attr()` 方法。需要设置的文本作为第二个参数传递。这会将元素的占位符文本更改为新文本。
...
使用 `placeholder` 属性。`placeholder` 属性用于获取或设置输入文本的占位符。这可用于将占位符文本更改为新文本。首先使用 jQuery 选择器选择元素。然后，可以将新的占位符文本分配给元素的 `placeholder` 属性。这会将元素的占位符文本更改为新文本。
