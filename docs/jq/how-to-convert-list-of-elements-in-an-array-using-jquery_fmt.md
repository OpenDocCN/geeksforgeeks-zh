...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-list-of-elements-in-an-array-using-jquery/](https://www.geeksforgeeks.org/how-to-convert-list-of-elements-in-an-array-using-jquery/)
...
*   列一个清单（有序/无序）。
*   选择`<ul>`元素的父元素，并为父元素的每个子元素调用一个匿名函数`.each()`或`.map()`方法。
*   获取每个子级的文本值，并将其附加到数组中。

## 例 1：使用 `.each()` 方法

本例使用`.each()`方法来转换数组中的元素列表。
