...
> 原文：[https://www.geeksforgeeks.org/how-to-display-child-row-information-using-jquery-datatables-plugin/](https://www.geeksforgeeks.org/how-to-display-child-row-information-using-jquery-datatables-plugin/)
...
在下面的示例中，像 `employee_id`、`name`、`designation`、`salary` 和 `address` 这样的员工详细信息存在于一个 Ajax 文本文件 `nestedData.txt` 中。一个 HTML 表用于在行和列中存储所有的细节。
...
以下是上面代码中 `ajax` 选项使用的 `nestedData.txt` 文件的数据。
