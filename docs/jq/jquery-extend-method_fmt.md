...
`extend()`方法接受上面提到并在下面描述的四个参数:

*   **deep:** 这个参数是合并变递归。
*   **target:** 此参数是要扩展的对象。它将接收新的属性。
*   **object1:** 此参数是包含要合并的附加属性的对象。
*   **objectN:** 此参数是包含要合并的属性的附加对象。

## 返回值:

合并后返回对象。

下面的例子说明了`extend()`方法在 jQuery 中的使用:

## 示例 1:

在本例中， `extend()`方法将两个对象合并为一个。
