...
在 HTML 中，我们刚刚添加了一个正常结构的 `h1` 标签，来执行 jQuery 中的打字插件。
...
在 JS 中，我们有一个 `Typed` 的对象，我们在其中传递了 `h1` 标签的类。然后我们有一个具有四个属性的对象。让我们逐一了解它们。

*   `strings`：需要 N 个字符串才能应用效果。在我们的例子中，我们使用了两个字符串。
*   `typeSpeed`：用于设置文本的打字或出现速度。
*   `backSpeed`：用于设置文本退格或消失速度。
*   `loop`：是布尔属性。我们将其设置为 `true` 以循环效果。
