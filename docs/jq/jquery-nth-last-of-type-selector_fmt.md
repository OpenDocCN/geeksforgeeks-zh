...
> 原文: [https://www.geeksforgeeks.org/jquery-nth-last-of-type-selector/](https://www.geeksforgeeks.org/jquery-nth-last-of-type-selector/)

**:nth-last-of-type() 选择器**用于选择具有相同元素名称的父元素的所有子元素，从最后一个到第一个计数。它使用 1 索引计数。
...
*   `n`: 它从最后选择第 n 个元素。
*   `even`: 它从最后一个开始选择每个偶数元素。
*   `odd`: 它从最后选择每一个奇数元素。
*   `formula`: 保存生成元素分组的公式，从最后开始，例如: `3n+2` 取 `n`-0，1，2，…的值，选择该表达式输出的每个元素。

## 示例 1

本示例使用 `:nth-last-of-type()` 选择器从最后一个中选择奇数元素。
...
本示例使用 `:nth-last-of-type()` 选择器选择最后一个元素。
