...
`trigger()`方法是 jQuery 中的一种方法，用于在选定的元素上触发指定的事件处理程序。
...
在上例中，使用了一个`Increase(obj)`函数，该函数以一个 html 元素为对象，通过使用`parseInt()`函数将字符串转换为整数，将其中的数字文本的值增加一。
...
此外，jQuery 选择器用于选择按钮并向其附加点击事件方法，在其中我们调用`Increase(obj)`功能。
...
当点击`Increase #1`按钮时，相应 div 内的值增加 1。但是当点击`Increase #2`按钮时，两个 div 的值都增加一。因为我们是在`#btn2`的绑定点击方法内，借助`trigger()`方法事件触发“点击”。

## 示例 2:

本示例借助`trigger()`方法触发输入元素的焦点事件。
