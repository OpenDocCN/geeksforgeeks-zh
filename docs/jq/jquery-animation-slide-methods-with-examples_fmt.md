...
*   `speed` 参数指定应用效果的速度。他们只能接受这些值:“慢”、“快”或毫秒。
*   `callback` 参数指定在执行 `animate()` 方法后要执行的函数。
...
在本例中，我们展示了下滑效果。如果单击“向下滑动”面板，则会对 HTML 元素进行相应的更改。
...
This method makes the element to slide up.
...
在本例中，我们展示了上滑效果。如果单击“上滑”面板，则会对 HTML 元素进行相应的更改。
...
This method makes the element to slide up/down.
...
*   `speed` 参数指定应用效果的速度。他们只能接受这些值:“慢”、“快”或毫秒。
*   `callback` 参数指定在执行特定的 slide 方法后要执行的函数。
