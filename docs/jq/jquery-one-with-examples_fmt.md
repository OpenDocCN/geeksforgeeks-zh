...
*   `event`: 必选参数。它用于指定要附加到元素的一个或多个事件。如果给定多个事件，则使用空格将它们分开。
*   `data`: 可选参数。它用于指定传递给函数的附加数据。
*   `function`: 必选参数。它用于指定事件发生时要运行的函数。

## 返回值

这个方法用指定的事件处理程序返回选中的元素。
