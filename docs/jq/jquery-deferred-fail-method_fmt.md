...
> 原文: `https://www.geeksforgeeks.org/jquery-deferred-fail-method/`

jQuery 中的 `deferred.fail()` 方法用于添加当 deferred 对象被拒绝时要调用的处理程序。此方法接受一个或多个参数，这些参数可以是函数或函数数组。回调的执行顺序与它们被添加的顺序相同。当延迟对象被拒绝时，调用失败的回调。
...
*   `failedCallbacks`: 此参数指定一个函数或函数数组，当“延迟”被拒绝时将调用这些函数。
*   `failedCallbacks`: 此参数指定可选的附加函数，或函数数组，当延迟被拒绝时将调用这些函数。
