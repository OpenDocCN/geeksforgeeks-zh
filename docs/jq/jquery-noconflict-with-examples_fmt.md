...
`noConflict()` 是 jQuery 中的内置函数。jQuery 一般使用 `$` 作为快捷方式标识符。现在有很多 JavaScript 库，比如 Angular Js、Ember、Knockout 等。如果其他带有 jQuery 的 JavaScript 框架也开始使用 `$` 签名作为快捷方式，那么就会有冲突，其中一个框架可能会停止工作。因此，实现了 `noConflict` 方法。
...
在上面的例子中，有两个 JavaScript 库在同一个文件中使用，并且它们都使用相同的 `$` 签名选择元素。现在冲突出现了，可能其中一个库无法使用 `$` 签名访问元素。

`noConflict()` 方法释放对 `$` 的保留快捷方式标识符，以便其他脚本可以使用它。
...
<body>
    <p style="color:green">This will show how to save reference and use them.</p>
