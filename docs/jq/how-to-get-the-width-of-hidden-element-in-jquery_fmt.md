...
> 原文：[https://www.geeksforgeeks.org/how-to-get-the-width-of-hidden-element-in-jquery/](https://www.geeksforgeeks.org/how-to-get-the-width-of-hidden-element-in-jquery/)

借助 `hide()` jQuery 函数可以隐藏一个 HTML 元素，或者我们可以通过在 CSS 中设置 `visibility: hidden` 来轻松隐藏它。我们可以很容易地在 jQuery 中找到这个隐藏元素的宽度。

每个 HTML 元素定义了两种宽度，即元素的**内宽**和**外宽**：

*   **`innerWidth`**：当所选元素不考虑边框宽度时，会考虑该宽度。
*   **`outerWidth`**：当考虑所选元素的边框宽度时，会考虑该宽度。
