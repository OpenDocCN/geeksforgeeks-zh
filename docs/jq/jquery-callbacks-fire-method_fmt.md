...
jQuery `回调.fire()` 方法用于调用列表中给定参数的所有回调。这个方法返回它被附加到的回调对象(`this`)。
...
*   `arguments`: 该参数定义了要传递回回调列表的参数或参数列表。

## 返回值:

这个方法返回它所附着的回调对象(`this`)。

## 示例 1:

本示例将 `fun1()` 添加到回调中，然后调用 `fire()` 方法，然后再次添加使用不同参数调用回调的相同方法。
