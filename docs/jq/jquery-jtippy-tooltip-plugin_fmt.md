...
**示例 1:** 以下示例演示了 `jTippy` 工具提示插件的基本功能。
...
**示例 2:** 以下示例演示了各种选项设置和事件触发功能的使用。下面只使用了几个，开发人员可以根据应用程序的需要来探索和使用它们。当工具提示显示给用户时，触发 `on("jt-show")` 事件，该事件在 HTML `div` 中显示消息，其 id 为 `showDivID`。一旦 `on("jt-hide")` 事件被触发，消息 `div` 被隐藏。
