...
**:only-of-type** 选择器是 jQuery 中的一个内置选择器，用于选择其父元素的唯一子元素。
...
在上面的例子中，其父元素（这里是 `div` 标签）的所有唯一子元素（这里是 `p` 标签）都被格式化为绿色，即“第二个 div 的唯一 Geek。”和“第三个 div 的唯一 Geek。”。
...
在上面的例子中，它们各自父元素的所有唯一子元素（这里是 `p` 标签）都被格式化为绿色，即“`id` 的唯一 Geek”，“`class` 的唯一 Geek”和“`body` 的唯一 Geek”，因为每个段落标签被认为是每个父级的不同子级。