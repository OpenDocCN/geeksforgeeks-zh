...
jQuery 中的 `deferred.pipe()` 方法用于添加实用方法进行过滤，链式 deferred。
...
*   `failFilter`: 这是一个可选功能，当延迟被拒绝时调用。
*   `progressFilter`: 这是一个可选函数，当进度通知被发送到延迟对象时调用。
