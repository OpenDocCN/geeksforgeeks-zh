...
jQuery 中的 `merge()` 方法用于将两个数组的内容合并到第一个数组中。
...
`merge()` 方法只接受上面提到的两个参数，如下所述:

*   `first`: 这个参数是第一个要合并的类数组对象，第二个的元素将被添加到其中。
*   `second`: 这个参数是第二个要合并到第一个中的类数组对象，其内容保持不变。
...
function geek() {
    ...
    var fo = [ 3, 6, 9 ];
    var fif = [];
    var n = $.merge(th, $.merge($.merge(fif, $.merge(fi, se )), fo));
    document.getElementById("geeks").innerHTML = "Merged : [" + n + "]";
}
</script>
</body>
