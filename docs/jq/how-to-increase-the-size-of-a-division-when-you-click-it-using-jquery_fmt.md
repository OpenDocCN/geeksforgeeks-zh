...
## 方法 1: 使用 `height()` 和 `width()` 方法

首先使用公共选择器选择页面上的所有分区，并使用 `click()` 方法应用点击绑定来触发增加大小的功能。然后，通过使用 `this` 作为选择器，可以找到当前点击的分区。

使用 [`height()` 方法](https://www.geeksforgeeks.org/jquery-height-and-innerheight-with-examples/)可以找到元素的当前高度，使用 [`width()` 方法](https://www.geeksforgeeks.org/jquery-width/)可以找到当前宽度。这些临时存储在单独的变量中。然后使用相同的方法设置修改后的高度和宽度。新值可以通过将当前高度和宽度乘以乘数来计算。这个乘数可以指定为一个变量。这将同样增加点击部分的大小。
...
这类似于上述方法。首先使用公共选择器选择页面上的所有分区，并使用 `click()` 方法应用点击绑定来触发增加大小的功能。然后，通过使用 `this` 作为选择器，可以找到当前点击的分区。

使用 `css()` 方法，传递第一个参数为`"height"`或`"width"`，即可求出元素当前的高度和宽度。这将返回当前分割的高度和宽度。在使用 [`parseInt()` 方法](https://www.geeksforgeeks.org/javascript-parseint-function/)将它们解析为整数后，它们被临时存储在单独的变量中。再次使用 `css()` 方法分配新的高度和宽度，绕过新值作为第二个参数。与上述方法类似，可以通过将当前高度和宽度乘以乘数来计算新值。
