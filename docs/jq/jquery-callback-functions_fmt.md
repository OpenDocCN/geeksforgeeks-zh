...
## 方法:
我们将不使用回调函数并尝试切换 `div` 元素的代码。同时，我们将使用一个`alert`(Javascript 浏览器警报)来展示`回调()`对我们有多有用。

## 示例-1:
我们定义一个 `div`，并在 `div` 下面添加一个按钮。现在，我们使用一个简单的 JQuery 代码使按钮允许 `div` 隐藏。
