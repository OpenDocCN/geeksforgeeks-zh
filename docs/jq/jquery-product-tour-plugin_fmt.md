...
## 注意
请从 [`产品游`插件](https://codyhouse.co/gem/product-tour)下载以下库文件，并保存在您的工作文件夹中，以包含在您的代码中。程序员可以根据项目需求改变路径和 CSS 文件。

## 程序
下面的例子演示了使用 CSS 和 javascript 代码开发的`产品游`插件的基本工作原理。HTML 页面由带有列表项的无序列表组成。每个列表项包含产品指南的一个步骤以及插件类，插件类控制属性，如标题、描述、手机图像和任何步骤的工具提示的位置。
