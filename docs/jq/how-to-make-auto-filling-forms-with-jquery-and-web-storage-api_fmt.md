...
> 原文：[https://www.geeksforgeeks.org/how-to-make-auto-filling-forms-with-jquery-and-web-storage-api/](https://www.geeksforgeeks.org/how-to-make-auto-filling-forms-with-jquery-and-web-storage-api/)

网络存储应用编程接口提供了浏览器存储键/值对的机制，比使用 cookies 更直观。Web Storage 中的两种机制是 `sessionStorage` 和 `localStorage`，`sessionStorage` 只存储一个会话的数据，直到浏览器关闭，`localStorage` 也做同样的事情，但是即使浏览器关闭并重新打开也仍然存在，因此建议使用本地存储来自动填充表单。
...
`方法 1:` 使用 `window.localStorage.setItem(key，value)` 将数据本地存储在浏览器上，然后在脚本标签中提取数据并用于自动填写表单。

`示例:` 让我们假设输入字段为“国家”的情况，需要使用 `localStorage` 属性自动填充国家字段。下面的代码实现以给定的形式获取并自动完成国家字段。
