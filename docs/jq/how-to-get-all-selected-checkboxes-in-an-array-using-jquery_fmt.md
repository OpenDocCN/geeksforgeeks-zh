...
> 原文：[https://www.geeksforgeeks.org/how-to-get-all-selected-checkboxes-in-an-array-using-jquery/](https://www.geeksforgeeks.org/how-to-get-all-selected-checkboxes-in-an-array-using-jquery/) 获取阵列中所有选中的复选框
...
*   `event`：此参数为必填项。它指定一个或多个要添加到选定元素的事件或命名空间。如果有多个事件值，用空格隔开。事件必须是有效的。
...
此方法设置/返回所选元素的文本内容。如果使用此方法返回内容，则提供所有匹配元素的文本内容（HTML 标签将被移除）。如果用此方法设置内容，则替换所有匹配元素的内容。
...
该方法返回/设置匹配元素的值属性。如果是返回值，这个方法返回第一个选中元素的值属性的值。在设置值的情况下，该方法为所有选定的元素设置值属性的值。
...
*   `value`：此参数为必填项。它指定值属性的值。
*   `function(index, cValue)`：此参数可选。它指定一个返回要设置的值的函数。
    *   `index`：返回集合中元素的索引。
    *   `cValue`：返回所选元素的当前值属性。
...
*   `item1, item2, ..., itemN`：此参数为必填项。它指定要添加到数组中的项。
...
本示例通过选择器选择选中的复选框，然后调用 `.each()` 方法到每一个元素，最后把它们推入数组。
...
<body style="text-align:center;" id="body">
    ...
    <script>
        $('#GFG_UP').text('First check few elements then click on the button.');
...
该示例也执行相同的工作，但方法不同。本示例通过选择器（与前一个不同）选择选中的复选框，然后调用 `.each()` 方法到每一个元素，最后把它们推入数组。
