...
> 原文: [https://www.geeksforgeeks.org/jquery-event-isdefaultprevented-method/](https://www.geeksforgeeks.org/jquery-event-isdefaultprevented-method/)

`event.isDefaultPrevented()` 方法是 jQuery 中的一个内置方法，它检查是否为事件调用了 `preventDefault()` 方法。此方法返回一个布尔值。如果对事件调用了 `preventDefault()`，则返回 `true`，否则返回 `false`。
...
接受来自事件绑定功能的单参数 `event`。

## 返回值

如果事件上调用了 `preventDefault()` 函数，则返回 `true`，否则返回 `false`。

## 示例 1

本示例检查是否在事件上调用了 `isPreventDefault()` 方法。
...
**注:** 黑体文字 (`true`/`false`) 为 `isDefaultPrevented()` 方法的值。

## 示例 2

本示例检查 `isDefaultPrevented()` 方法是否阻止默认操作。
