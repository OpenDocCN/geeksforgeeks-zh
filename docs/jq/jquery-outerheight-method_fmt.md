...
> 原文: `https://www.geeksforgeeks.org/jquery-outerheight-method/`
...
该方法接受单参数 `includeMargin`。它包含布尔值，用于指定是否包含边距。如果 `includeMargin` 设置为 `true`，则包含边距，否则不包含边距。默认情况下，`includeMargin` 设置为 `false`。
