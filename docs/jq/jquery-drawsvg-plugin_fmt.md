...
[SVG](https://www.geeksforgeeks.org/html-svg-basics/)（可缩放矢量图形）是基于可扩展标记语言的图形，支持增强交互性的二维图像动画。SVG 的规范是由万维网联盟在 XML 文本文件中定义的开放标准。可以使用任何绘图软件或文本文件编辑器来更改或创建这些文件。
...
以下示例使用 jQuery `DrawSVG` 插件演示了带有圆形顶点的矩形的简单动画。`path` 元素是在 SVG 库中创建直线和圆弧等基本形状的最重要元素。它还可以结合许多基本形状来创建复杂的形状。`"d"` 是定义元素形状的参数。
...
以下示例演示了 `drawsvg()` 方法以及选项设置和使用回调方法。一旦动画完成，就执行回调函数。
...
<script>
        var $svg = $("svg").drawsvg({
            ...
            duration: 5000,
            callback: function() {
                $('#callbackDiv').html('<p><strong>Animation completed !</strong></p>');
            }
        }).drawsvg("animate");
    </script>
</body>
...
以下示例演示了带有页面滚动事件的 `drawsvg()` 方法。根据用户在上下方向滚动页面，路径以淡入和淡出的方式进行动画显示。当前页面需要是可滚动的，如下面的脚本所示。
...
<script>
        ...
        $winVar.scroll(function() {
            var scrollPercent = $winVar.scrollTop() / max;
            $svgVar.drawsvg('progress', scrollPercent);
        });
    </script>
</body>
