...
> 原文：`https://www.geeksforgeeks.org/jquery-callbacks-add-method/`

jQuery `callbacks.add()` 方法用于向回调列表添加回调或回调集合。此方法返回它所附加到的回调对象（`this`）。
...
*   `callbacks`：这个参数保存一个函数，或者函数数组，这些函数将被添加到回调列表中。

## 示例 1

这个方法在回调中添加一个方法 `fun1()` 并调用它。
...
本示例将方法 `fun1()` 和 `fun2()` 添加到回调中，然后调用它们。请注意，第二次调用 `fire()` 方法时，它使用相同的参数“GFG_2”调用这两个函数。
