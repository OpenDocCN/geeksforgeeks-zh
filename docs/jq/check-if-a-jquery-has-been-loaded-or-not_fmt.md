...
> 原文: [https://www.geeksforgeeks.org/check-if-a-jquery-has-been-loaded-or-not/](https://www.geeksforgeeks.org/check-if-a-jquery-has-been-loaded-or-not/)
...
*   首先检查它是否已加载。
*   如果没有加载，则动态加载它，创建一个脚本元素，并向其添加类似`(type, src)`的属性，然后最终将其追加到头部元素内部的末尾。

## 示例 1

在本例中，jQuery 已经加载，因此消息`“脚本已经加载！”`在屏幕上打印。
...
脚本元素的 `src` 属性改为 `sc`。在这个例子中，jQuery 没有被加载，所以消息`“脚本加载成功！”`加载成功后在屏幕上打印。
