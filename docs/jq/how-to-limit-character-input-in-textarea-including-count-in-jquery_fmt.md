...
> 原文：[https://www.geeksforgeeks.org/how-to-limit-character-input-in-textarea-including-count-in-jquery/](https://www.geeksforgeeks.org/how-to-limit-character-input-in-textarea-including-count-in-jquery/)

在本文中，我们将在 jQuery 中设置 textarea 中的字符输入限制，包括计数。要在输入文本区设置限制字符，我们使用 `length` 属性。

**进场：**

我们创建一个给定最大长度的输入文本区域，然后使用 jQuery 代码来限制字符。首先，我们设置最大限制，然后当我们点击按钮并在屏幕上显示计数时，使用 `keyup()` 方法将文本区域字符限制减少 1。
