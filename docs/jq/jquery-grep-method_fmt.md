...
*   `array`: 此参数保存用于搜索的类数组对象。
*   `function(element, index)`: 它是采用两个参数的过滤函数，`element`保存数组的元素，`index`保存该特定元素的索引。
*   `invert`: 如果为假或未通过，则函数返回一个数组，该数组包含“回调”返回真的所有元素。如果传递的是 `true`，则函数返回一个数组，该数组包含“回调”返回 `false` 的所有元素。
...
在本例中，`grep()`方法应用于数字数组，根据条件过滤一些数字。不影响原数组。
...
在本例中，`grep()`方法应用于 JavaScript 对象数组，根据条件过滤掉一些对象。这个方法不影响原始数组。
