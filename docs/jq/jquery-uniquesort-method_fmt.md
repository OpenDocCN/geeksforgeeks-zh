...
jQuery 中的 `uniqueSort()` 方法用于对 DOM 元素的数组进行排序，在适当的位置移除重复的元素。
...
`uniqueSort()`方法只接受上面提到的一个参数，下面将进行描述:

*   `array`: 此参数保存 DOM 元素的数组。

## 返回值:

移除重复项后返回 DOM 元素的排序数组。

### 示例 1:

在本例中，`uniqueSort()`方法从 `div` 数组中移除重复元素。
...
在本例中，`uniqueSort()`方法从 `p` 的数组中移除所有重复的元素。
