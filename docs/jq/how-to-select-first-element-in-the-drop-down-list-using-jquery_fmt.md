...
> 原文: [https://www.geeksforgeeks.org/how-to-select-first-element-in-the-drop-down-list-using-jquery/](https://www.geeksforgeeks.org/how-to-select-first-element-in-the-drop-down-list-using-jquery/)

给定一个包含`option`元素的`select`下拉列表，任务是使用 JQuery 获取`select`元素的第一个元素。

## 方法 1

*   使用 JQuery 选择器选择`<select>`元素的第一个`<option>`元素。
*   使用 [`.prop()` 属性方法](https://www.geeksforgeeks.org/jquery-prop-with-examples/) 获取该特定元素的属性。
...
*   使用 JQuery 选择器选择`<select>`元素。
*   该选择器更具体，使用 `option` 选择第一个元素：`:nth-child(1)`。
*   这将获得对以 `1` 开头的第一个 `option` 索引的访问权。
