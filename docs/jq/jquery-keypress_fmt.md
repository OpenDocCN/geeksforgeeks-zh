...
检测任何特定的按键事件。`event.keyCode` 和 `event.which` 将返回按下的按键的 ASCII 值，两者都是浏览器特定的。下面的代码用于检查在页面上按下了哪个键。
...
对于所有按键，如非打印字符(如 ALT、CTRL、SHIFT、ESC)，按键事件不会被触发。`keydown()` 方法可以用来检查这些键。