...
> 原文: [https://www.geeksforgeeks.org/how-to-detect-value-change-on-hidden-input-field-in-jquery/](https://www.geeksforgeeks.org/how-to-detect-value-change-on-hidden-input-field-in-jquery/)

在 `<input>` 元素加上 `type="hidden"`，jQuery 的 `change()` 方法不会自动触发。`change()` 方法只在用户自己更改字段值时自动触发，而不是开发者。当开发人员更改隐藏字段内的值时，他也必须**触发更改**。

有两种方法可以检测隐藏输入字段的值变化:

*   通过使用 [`bind()` 方法](https://www.geeksforgeeks.org/jquery-bind-with-examples/)
*   通过使用 [`change()` 方法](https://www.geeksforgeeks.org/jquery-change-with-examples/)

以下示例将说明检测隐藏字段中值变化的两种方法。

## 示例 1

以下示例使用 jQuery 的内置 `bind()` 方法。
