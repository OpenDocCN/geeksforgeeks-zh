...
> 原文: [https://www.geeksforgeeks.org/how-to-make-water-ripple-effect-using-jquery/](https://www.geeksforgeeks.org/how-to-make-water-ripple-effect-using-jquery/)

在这里，我们将使用 `jQuery` 在我们的网页上产生水波纹效果。为此，我们将使用 `jQuery ripple.js`。

### 安装

在进一步移动之前，首先我们要将水波纹 `CDN` 文件添加到我们的项目文件中:
...
*   `dropRadius` 定义通过在画布上单击或移动鼠标而产生的拖放大小(以像素为单位)。
*   `perturbance` 是由波纹引起的折射量。`0` 表示没有折射。
*   `resolution`、`width` 和 `height` 是要渲染到的 `WebGL` 纹理的分辨率、宽度和高度。该值越大，渲染越平滑，波纹传播越慢。
*   `interactive`，鼠标点击和鼠标移动这两个事件都可以触发效果。
