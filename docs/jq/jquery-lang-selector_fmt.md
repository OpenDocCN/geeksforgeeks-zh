...
`:lang()` 选择器用于选择所有具有指定 `lang` 属性值的元素。
该属性包含*单值语言代码*，用于指定内容的语言。一些语言的例子是英语的 `"en"`，西班牙语的 `"es"` 等等。它将整个单词作为值，如 `lang="en"` 或后跟连字符(-)如 `lang="en-us"`。
...
以下是 `jQuery :lang Selector` 支持的浏览器:
