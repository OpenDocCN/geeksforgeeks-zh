...
> 原文: [https://www.geeksforgeeks.org/jquery-selectors-and-event-methods/](https://www.geeksforgeeks.org/jquery-selectors-and-event-methods/)
...
### Id 选择器
Id 选择器根据元素的 Id 来选择元素。
**示例：**
在本例中，当用户双击按钮时，带有 `id = "gfg"` 的元素会被隐藏。
代码：
...
类选择器根据元素的类来选择元素。
**示例：**
在这个例子中，当用户点击按钮时，带有 `class = “GFG”` 的元素会被隐藏。
代码：
...
事件指的是网站访问者在与网站（或网页）交互时执行的操作。可以有各种类型的事件，例如：
...
| 方法名 | 描述 |
| :--- | :--- |
| `click()` | `click()` 方法包含一个用于事件处理的函数，当用户单击特定的 HTML 元素时会调用该函数。 |
| `dblclick()` | `dblclick()` 方法包含一个用于事件处理的函数，当用户双击特定的 HTML 元素时调用该函数。 |
| `mouseenter()` | `mouseenter()` 方法包含一个用于事件处理的函数，当鼠标指针进入特定的 HTML 元素时调用该函数。 |
| `mouseleave()` | `mouseleave()` 方法包含一个用于事件处理的函数，当鼠标指针从先前选择的特定 HTML 元素中移除时，该函数将被调用。 |
| `mousedown()` | `mousedown()` 方法包含一个用于事件处理的函数，当鼠标指针在 HTML 元素上按下鼠标左键、右键或中键时调用该函数。 |
| `mouseup()` | `mouseup()` 方法包含一个用于事件处理的函数，当鼠标指针在 HTML 元素上时，释放鼠标左键、右键或中键时会调用该函数。 |
| `hover()` | `hover()` 方法包含一个用于事件处理的函数，当鼠标指针进入和离开 HTML 元素时调用该函数。它是 `mouseenter()` 和 `mouseleave()` 方法的组合。 |

## 获取和设置方法

jQuery 有各种方法来获取属性的值并将属性设置为特定值。这些方法功能强大，足以改变网站内容及其样式。其中一些方法是：
