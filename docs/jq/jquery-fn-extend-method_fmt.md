...
`jQuery.fn.extend()`方法用于将对象的内容合并到 jQuery 原型上，以提供新的 jQuery 实例方法。
...
*   `object`: 此参数保存要合并到 jQuery 原型上的对象。
...
在本例中，`jQuery.fn.extend()`方法用于向 jQuery 原型对象添加两个方法，然后使用其中一个。
...
在本例中，`jQuery.fn.extend()`方法用于将更多的方法合并到 jQuery 原型对象中。
