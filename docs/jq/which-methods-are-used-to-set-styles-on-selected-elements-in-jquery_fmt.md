...
> 原文: [https://www.geeksforgeeks.org/which-methods-are-used-to-set-styles-on-selected-elements-in-jquery/](https://www.geeksforgeeks.org/which-methods-are-used-to-set-styles-on-selected-elements-in-jquery/)
...
`css()` 方法用于更改所选元素的样式属性。jQuery 中的 `css()` 可以有不同的用法。
...
使用 `addClass()` 方法将类添加到特定元素中。我们可以向添加的类中添加一些 CSS 属性。
