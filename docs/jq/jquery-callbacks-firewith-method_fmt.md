...
> 原文: [https://www.geeksforgeeks.org/jquery-callbacks-firewith-method/](https://www.geeksforgeeks.org/jquery-callbacks-firewith-method/)

jQuery 中的 `callbacks.fireWith()` 方法用于调用当前列表中所有具有给定上下文和参数的回调。
...
*   `context`: 该参数定义了对应该在其中触发列表中回调的上下文的引用。
*   `params`: 该参数是要传递给列表中回调的参数的数组或类似数组的对象。如果未使用或未定义，将不传递任何参数。
