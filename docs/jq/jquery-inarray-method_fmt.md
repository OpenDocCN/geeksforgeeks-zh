...
jQuery 中的这个 `inArray()`方法用于在数组中搜索一个特定的值并返回它的索引(如果没有找到则返回-1)。
...
在本例中， `inArray()`方法像前面的示例一样搜索数组中的元素“极客”。但是，index 属性也被传递来搜索索引 3 中的元素，并返回-1。
