...
> 原文: [https://www.geeksforgeeks.org/how-to-add-attributes-to-an-html-element-in-jquery/](https://www.geeksforgeeks.org/how-to-add-attributes-to-an-html-element-in-jquery/)

给定一个包含一些元素的 HTML 文档，任务是向 HTML 元素添加属性。

## 方法

给定一个包含 `<标签>`、`<输入>` 和 `<按钮>` 元素的 HTML 文档。`<输入>` 元素包含 `type` 和 `id` 属性。当用户点击按钮时，jQuery 函数被调用。我们使用 `$(selector).attr()` 方法添加属性。这里，我们将 `placeholder` 属性添加到输入字段中。
