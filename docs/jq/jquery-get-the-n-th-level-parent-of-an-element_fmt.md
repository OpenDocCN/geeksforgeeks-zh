...
> 原文: [https://www.geeksforgeeks.org/jquery-get-the-n-th-level-parent-of-an-element/](https://www.geeksforgeeks.org/jquery-get-the-n-th-level-parent-of-an-element/)
...
This method returns an element with a passed index number of the matched elements. The index start from 0.
...
*   **index**: 此参数为必选项。它指定元素的索引。接受正数或负数。

## 示例 1

本示例使用 `parents()` 和 `eq()` 方法来查找 `class='child'` 元素的第 0 级父元素，该元素是 `<div>` 元素。
...
本例使用 `parents()` 和 `eq()` 方法找到 `class='child'` 元素的第二级父元素，它是 `<body>` 元素。
