...
> 原文: [https://www.geeksforgeeks.org/jquery-deferred-progress-method/](https://www.geeksforgeeks.org/jquery-deferred-progress-method/)

jQuery 中的这个 `deferred.progress()` 方法用于添加当 deferred 对象生成进度通知时要调用的处理程序。
...
*   `progressCallbacks`: 该参数是一个函数或函数数组，当延迟对象生成进度通知时调用。
*   `progressCallbacks`: 它是一个可选参数，是一个函数或函数数组，当 deferred 生成进度通知时调用。
