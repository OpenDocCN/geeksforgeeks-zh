...
> 原文：[https://www.geeksforgeeks.org/whats-the-difference-between-toarray-and-makearray-in-jquery/](https://www.geeksforgeeks.org/whats-the-difference-between-toarray-and-makearray-in-jquery/)

在本文中，我们将学习 jQuery 中 `toArray()` 和 `makeArray()` 方法之间的区别。

## toArray() 方法

`toArray()` 方法用于 DOM（文档对象模型）元素，将它们转换为 JavaScript 数组。我们可以迭代这个数组，计算它的长度，并像普通数组一样使用它们的索引访问元素。但是，我们不能使用此方法从其他类似数组的对象创建数组。这是两种功能的主要区别。
...
`makeArray()` 方法用于类似数组的对象，将它们转换为数组。然后我们可以在这个数组上使用普通的数组函数。该方法支持 DOM 元素的转换，因为它们是类似数组的对象。
...
| **toArray()** | **makeArray()** |
| --- | --- |
| 此方法支持将 DOM 元素转换为数组。 | 此方法支持将所有类似数组的元素转换为数组。 |
| 仅支持 DOM 元素转换。其他类似数组的元素将抛出错误。 | 所有类型的元素都可以转换为包含 DOM 元素的数组。 |