...
jQuery 中的这个 `deferred.then()` 方法用于添加处理程序，当 deferred 对象被解析、拒绝或正在进行时，将调用这些处理程序。
...
*   `doneCallbacks`: 这是一个函数，或者函数数组，在解析 deferred 时调用。
*   `failCallbacks`: 这是一个函数，或者函数数组，当 deferred 被拒绝时调用。
*   `progressCallbacks`: 这是一个函数或函数数组，当进度通知被发送到 deferred 对象时调用。
...
在本例中，使用 `notify` 和 `resolve` 方法调用了 `then()` 方法。
...
在本例中，使用 `notify` 和 `reject` 方法调用了 `then()` 方法。
