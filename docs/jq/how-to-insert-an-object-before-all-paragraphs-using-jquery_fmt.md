...
可以使用调用的方法在所有段落前插入一个 `jQuery` 对象。`before()`。

*   **`before()` :** 此方法用于在匹配元素集中的每个元素之前插入参数指定的内容。它可以接受任何数量的附加参数，如–。`prepend()`和。`insertAfter()`之后。

*   **`insertBefore()`:** 它与上面的函数类似，但是内容在方法之前，并且插入到目标之前，而目标又作为。`insertBefore()`方法的参数。
