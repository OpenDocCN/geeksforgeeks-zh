...
jQuery 中的 `parseHTML()` 方法用来将一个字符串解析成一个 DOM 节点的数组。
...
`parseHTML()` 方法接受上面提到的和下面描述的三个参数:

*   `data`: 这个参数是需要解析的 HTML 字符串。
*   `context`: 此参数是用作将在其中创建 HTML 片段的上下文的文档元素。
...
在本例中，`parseHTML()` 方法将一个字符串解析成一个 DOM 节点数组。
...
在本例中，`parseHTML()` 方法使用 HTML 字符串创建一个 DOM 节点数组，并将其插入 div。
