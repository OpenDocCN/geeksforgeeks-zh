...
> 原文：[https://www.geeksforgeeks.org/how-to-hide-a-div-when-the-user-clicks-outside-of-it-using-jquery/](https://www.geeksforgeeks.org/how-to-hide-a-div-when-the-user-clicks-outside-of-it-using-jquery/)
...
1.  鼠标上移事件首先在文档上进行检查。
    ...
    ```
2.  在目标单击时调用`closest()`方法。此方法返回 DOM 树中选定元素的第一个祖先。然后在结果上使用`length`属性来找出祖先的数量。如果没有祖先，则意味着单击在元素之外。
    ```html
...
1.  鼠标上移事件首先在文档上进行检查。
    ...
    ```
2.  检查元素是否包含点击目标，通过传递`is()`方法和`has()`方法与点击目标来实现。
