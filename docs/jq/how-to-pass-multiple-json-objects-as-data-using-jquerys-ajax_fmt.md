# 如何使用 jQuery 的 `$()` 将多个 JSON 对象作为数据传递给 `ajax()`？

> 原文: [https://www.geeksforgeeks.org/how-to-pass-multiple-json-objects-as-data-using-jquerys-ajax/](https://www.geeksforgeeks.org/how-to-pass-multiple-json-objects-as-data-using-jquerys-ajax/)

本文的目的是在一个 HTML 文档中使用 jQuery [`$ajax()`](https://www.geeksforgeeks.org/jquery-ajax-method/) 方法将多个 JSON 对象作为数据传递。

`方法:` 在 HTML 文档中创建一个按钮，将 JSON 对象发送到 PHP 服务器。在 JavaScript 文件中，给按钮添加一个 [`click()`](https://www.geeksforgeeks.org/jquery-click-with-examples/) 事件监听器。点击按钮，使用 jQuery `$ajax()` 方法请求 PHP 文件，通过该方法多个 JSON 对象被传递到服务器。
