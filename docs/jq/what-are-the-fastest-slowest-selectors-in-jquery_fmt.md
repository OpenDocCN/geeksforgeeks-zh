...
> 原文: [https://www.geeksforgeeks.org/what-are-the-fastest-slowest-selectors-in-jquery/](https://www.geeksforgeeks.org/what-are-the-fastest-slowest-selectors-in-jquery/)
...
ID 选择器: jQuery `#ID` 选择器根据 HTML 标签的 [ID](https://www.geeksforgeeks.org/html-id-attributes/) 属性选择元素。
...
类选择器: jQuery `.class` 选择器选择类别匹配的元素或元素集。不同的 HTML 元素允许使用相同的类名。
...
注意: 如果我们考虑今天的现代浏览器，那么这些选择器同样快速，因为在现代浏览器中，类名在内部被散列，我们有 `getElementsByClassName()` 而在旧的浏览器中没有类似于 `getElementsByClassName()` 的功能。这就是原因 `.class` 名称在内部解析为 jQuery，然后遍历 DOM 的每个元素并检查类名。