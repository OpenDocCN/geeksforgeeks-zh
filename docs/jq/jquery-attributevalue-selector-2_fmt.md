...
> 原文：[https://www.geeksforgeeks.org/jquery-attributevalue-selector-2/](https://www.geeksforgeeks.org/jquery-attributevalue-selector-2/)

`$("[attribute*='value']")` 用于选择属性参数指定的属性包含值参数指定的词的所有元素。
...
*   **属性**：`attribute` 指定需要查找的属性。
*   **值**：`value` 是与具有指定属性的每个元素匹配的值的字符串。
