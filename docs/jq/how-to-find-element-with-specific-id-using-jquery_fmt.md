...
> 原文: [https://www.geeksforgeeks.org/how-to-find-element-with-specific-id-using-jquery/](https://www.geeksforgeeks.org/how-to-find-element-with-specific-id-using-jquery/)
...
`例 1：` 在本例中，`var name`包含要检查的 ID 名称。首先 JQuery 选择器通过`var name`检查 ID，然后使用`length`属性验证是否选择了某个东西。
...
`示例 2：` 在本例中，要检查的 ID 名称直接在 JQuery 选择器中传递。首先 JQuery 选择器检查 ID，然后`length`属性用于验证是否选择了某个东西。
