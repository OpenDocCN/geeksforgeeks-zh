# jQuery Page Piling 插件

> 原文: [https://www.geeksforgeeks.org/jquery-page-piling-plugin/](https://www.geeksforgeeks.org/jquery-page-piling-plugin/)

jQuery `pagePiling.js` 插件是一个丰富的特性，可供程序员堆积多个布局部分，一个在另一个之上，通过 URL 或鼠标滚动或侧边项目符号导航访问每个页面。此功能为目的地锚点链接提供所有类型的平滑垂直、水平和侧面导航。它还管理回调函数、视频背景和滚动速度。在下面的例子中，我们将只讨论其中的几个。

**注意:** 请从 [这个链接](https://github.com/alvarotrigo/pagePiling.js/) 下载以下库文件，并保存在您的工作文件夹中，以包含在您的代码中。请从这个[链接](https://github.com/alvarotrigo/pagePiling.js/tree/master/examples)下载 `examples.css` 文件。
...
在以下示例中，顶部和底部循环特征被启用。为此，`loopTop` 和 `loopBottom` 属性设置为“真”，如 jQuery 代码所示。
...
在以下示例中，通过设置选项`direction: 'horizontal'`来启用页面的水平滚动，如 jQuery 代码所示。
