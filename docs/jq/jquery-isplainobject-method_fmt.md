...
jQuery 中的这个`isPlainObject()`方法是用来检查一个对象是否是普通对象的。
...
以下示例说明了在 jQuery 中使用`isPlainObject()`方法:

### 示例 1:

在本例中，`isPlainObject()`方法检查一个对象，看它是否是普通对象。
...
在本例中，`isPlainObject()`方法也会检查一个对象，看它是否是普通对象。
