...
> 原文：[https://www.geeksforgeeks.org/how-to-count-number-of-rows-and-columns-in-a-table-using-jquery/](https://www.geeksforgeeks.org/how-to-count-number-of-rows-and-columns-in-a-table-using-jquery/)

给定一个包含表的 HTML 文档，任务是使用 JQuery 计算该表中的行数和列数。

## 方法

`length`属性用于使用 jQuery 计算 HTML 表格中的行数和列数。用于查找 HTML 表中行数和列数的选择器有：

*   要计算行数，使用`#Table_Id tr`选择器。它选择表中所有的`<tr>`元素。这包括包含表格标题的行。`length`属性用于所选元素以获取行数。
*   要计算列数，使用`#Table_Id tr th`选择器。它选择表格中嵌套在`<tr>`元素内的所有数量的`<th>`元素。`length`属性用于所选元素，以获取列数。
