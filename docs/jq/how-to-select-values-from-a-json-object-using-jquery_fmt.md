...
> 原文：[https://www.geeksforgeeks.org/how-to-select-values-from-a-json-object-using-jquery/](https://www.geeksforgeeks.org/how-to-select-values-from-a-json-object-using-jquery/)
...
首先我们在一个变量中存储一个包含（键，值）对的 JSON 对象。我们已经使用 `<button>` 元素创建了一个按钮，当我们单击该按钮时，jQuery 函数使用选择器和单击事件调用。点击按钮后，`attr()` 方法添加了名称属性 `<select>` 元素。使用 `append()` 和 `each()` 方法将 `<option>` 元素追加到每个元素中。
    ...
    <script>
        $(document).ready(function () {
            ...
            var json = [
                ...
                { "name": "GeeksforGeeks", "text": "GeeksforGeeks" }
            ];
