...
在本文中，我们将看到当 jQuery 中的输入字段发生变化时，如何在文本消息中追加一个元素。要在输入字段更改时用文本消息追加元素，可以使用 [`change()`](https://www.geeksforgeeks.org/jquery-change-with-examples/) 和 [`appendTo()`](https://www.geeksforgeeks.org/jquery-appendto-with-examples/) 方法。[`change()` 方法](https://www.geeksforgeeks.org/jquery-change-with-examples/)用于检测输入字段值的变化。该方法仅适用于 `<input>`、`<textarea>` 和 `<select>` 元素。[`appendTo()` 方法](https://www.geeksforgeeks.org/jquery-appendto-with-examples/)用于追加元素。
