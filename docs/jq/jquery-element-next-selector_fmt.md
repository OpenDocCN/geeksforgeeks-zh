...
`element + next` 选择器在 jQuery 中用于选择指定元素的紧邻的下一个元素。该选择器仅在“下一个”元素紧跟在指定元素之后时有效。

## 例如

*   如果语句是 `$("div + p")`，那么这将只选择第一个紧跟在 `div` 旁边的 `p` 元素，其他 `p` 元素被忽略。
...
这里，标题元素位于段落和 div 元素之间。所以 `p` 元素不会发生变化。
