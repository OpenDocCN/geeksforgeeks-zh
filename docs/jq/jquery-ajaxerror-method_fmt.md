...
jQuery 中的 `ajaxError()`方法用于指定 AJAX 请求失败时要运行的函数。
...
该方法接受单参数函数，该函数为必选项。该函数接受下列四个参数:
...
本示例通过从服务器获取数据来更改 `<p>` 元素的内容。当 AJAX 请求由于错误而失败时，页面显示 **AJAX 请求失败。**。
...
本示例通过从服务器获取数据来更改 `<h1>` 元素的内容。当 AJAX 请求由于错误而失败时，页面显示 **AJAX 请求失败。**。
