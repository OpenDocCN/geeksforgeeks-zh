...
## 先决条件
[K-Means 聚类](https://www.geeksforgeeks.org/k-means-clustering-introduction/)

## 谱聚类
**谱聚类**是一种不断增长的聚类算法，在很多情况下都比很多传统的聚类算法表现得更好。它将每个数据点视为一个图节点，从而将聚类问题转化为图划分问题。典型的实现包括三个基本步骤：

### 构建相似图
该步骤以邻接矩阵的形式构建相似图，该邻接矩阵由 `A` 表示。邻接矩阵可以通过以下方式构建：

*   **ε-邻域图:** 参数 `ε` 是预先固定的。然后，每个点都连接到它的 `ε` 半径内的所有点。如果任意两点之间的所有距离在比例上相似，则通常不存储边的权重，即两点之间的距离，因为它们不提供任何附加信息。因此，在这种情况下，构建的图是一个无向和未加权的图。
...
### 将数据投影到低维空间
这一步是为了考虑同一簇的成员在给定维度空间中可能相距很远的可能性。因此，维度空间被缩减，使得这些点在缩减的维度空间中更接近，从而可以通过传统的聚类算法将它们聚类在一起。这是通过计算**图拉普拉斯矩阵**来完成的。为了计算它，首先需要定义节点的度。第 `i` 个节点的度由下式给出：

<center>![d_{i} = \sum _{j=1|(i, j)\epsilon E}^{n} w_{ij}](img/d8d7a0fa551e8f8d0c0f619d7323cef0.png "Rendered by QuickLaTeX.com")</center>

注意 `w_{ij}` 是上面邻接矩阵中定义的节点 `i` 和 `j` 之间的边。
...
### 聚类数据
这个过程主要涉及使用任何传统的聚类技术对缩减后的数据进行聚类——通常是 K-Means 聚类。首先，每个节点被分配图拉普拉斯矩阵标准化后的一行。然后使用任何传统技术对这些数据进行聚类。为了转换聚类结果，节点标识符被保留。
...
## 步骤 4: 建立聚类模型并可视化聚类
在下面的步骤中，两个不同的光谱聚类模型具有不同的参数 `affinity` 值。您可以在这里阅读光谱聚类类[的文档。](https://scikit-learn.org/stable/modules/generated/sklearn.metrics.silhouette_score.html)
