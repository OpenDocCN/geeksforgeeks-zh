...
机器学习模型的学习曲线显示了机器学习模型的预测误差如何随着训练集的大小增加或减少而变化。
...
为了这个例子，我们将使用非常流行的`digits`数据集。关于这个数据集的更多信息，可以参考下面的链接:[https://scikit-learn.org/stable/auto_examples/datasets/plot_digits_last_image](https://scikit-learn.org/stable/auto_examples/datasets/plot_digits_last_image)
