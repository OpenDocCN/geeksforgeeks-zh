...
> 原文：[https://www.geeksforgeeks.org/sklearn-stratifiedshufflesplit-function-in-python/](https://www.geeksforgeeks.org/sklearn-stratifiedshufflesplit-function-in-python/)

在本文中，我们将从 `sklearn` 库中了解 `StratifiedShuffleSplit` 交叉验证器，该库提供训练测试索引以将数据拆分为训练测试集。

## 什么是 StratifiedShuffleSplit？

`StratifiedShuffleSplit` 是 `ShuffleSplit` 和 `StratifiedFold` 的组合。使用 `StratifiedShuffleSplit` 类标签的分布比例在训练和测试数据集之间几乎是均匀的。`StratifiedShuffleSplit` 和 `StratifiedFold`（`shuffle = True`）的主要区别在于在 `StratifiedFold` 中，数据集在开始时只被洗牌一次，然后分裂成指定数量的折叠。这就排除了训练测试集重叠的任何机会。
...
### 步骤 4) 创建 StratifiedShuffleSplit 类的对象
...
### 步骤 5) 调用实例，将数据帧拆分为训练样本和测试样本

`split()` 函数返回训练测试样本的索引。使用回归算法，比较每个预测值的准确性。
