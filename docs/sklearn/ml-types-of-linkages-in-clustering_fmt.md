...
> 原文: [https://www.geeksforgeeks.org/ml-types-of-linkages-in-clustering/](https://www.geeksforgeeks.org/ml-types-of-linkages-in-clustering/)
...
对于两个聚类 `R` 和 `S`，单链接返回两点 `i` 和 `j` 之间的最小距离，使得 `i` 属于 `R`，`j` 属于 `S`。
...
对于两个聚类 `R` 和 `S`，完全联动返回两点 `i` 和 `j` 之间的最大距离，使得 `i` 属于 `R`，`j` 属于 `S`。
...
对于两个聚类 `R` 和 `S`，首先计算 `R` 中任意数据点 `i` 和 `S` 中任意数据点 `j` 之间的距离，然后计算这些距离的算术平均值。平均链接返回算术平均值。
...
![n_{S}](img/73c2c1fbeb87943960b541cc26978a2a.png "Rendered by QuickLaTeX.com") – `S` 中的数据点数量
