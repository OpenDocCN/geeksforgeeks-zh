...
## 标准缩放器
`StandardScaler`遵循**标准正态分布**。因此，它使*均值= 0* 并将数据换算成单位方差。

## 最小最大缩放器
`MinMaxScaler`缩放范围*【0，1】*内的所有数据特征，如果数据集中有负值，则缩放范围*【-1，1】*内的所有数据特征。此缩放会压缩窄范围*【0，0.005】*内的所有内联。

## 异常值的影响
在存在异常值的情况下，由于在计算经验均值和标准差时异常值的影响，`StandardScaler`不能保证特征尺度的平衡。这导致特征值范围的缩小。

## 鲁棒缩放器
通过使用`RobustScaler()`，我们可以去除异常值，然后使用标准缩放器或最小最大缩放器对数据集进行预处理。
...
## 代码:StandardScaler、MinMaxScaler 和 RobustScaler 的比较
...
*   `with_centering`: 布尔值，默认为`True`。如果该值为真，则数据在缩放前居中。当它应用于稀疏矩阵时，转换将引发异常，因为对它们进行中心化需要构建一个密集的矩阵，该矩阵通常太大而不适合内存。
*   `with_scaling`: 布尔值，默认为`True`。它将数据缩放到四分位数范围。
*   `quantile_range`: 元组`(q_min, q_max)`，其中`0.0 < q_min < q_max < 100.0`。分位数范围用于计算比例。默认值: `(25.0, 75.0)` = (第一分位数, 第三分位数) = IQR。
*   `copy`: 布尔值，可选。默认为`True`。如果输入已经是`NumPy`数组或`scipy.sparse CSC`矩阵，并且如果`axis=1`，通过将该参数设置为`False`来避免复制，而是执行就地行规范化。
