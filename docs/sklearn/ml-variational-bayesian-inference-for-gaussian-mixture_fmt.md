...
> 原文: [https://www.geeksforgeeks.org/ml-variational-bayesian-inference-for-gaussian-mixture/](https://www.geeksforgeeks.org/ml-variational-bayesian-inference-for-gaussian-mixture/)

**先决条件:** [高斯混合](https://www.geeksforgeeks.org/gaussian-mixture-model/)

高斯混合模型假设数据被分成多个聚类，使得给定聚类中的每个数据点遵循特定的多变量高斯分布，并且每个聚类的多变量高斯分布彼此独立。为了在这种模型中对数据进行聚类，需要计算给定观测数据下数据点属于给定聚类的后验概率。为此目的的一种近似方法是贝叶斯法。但是对于大型数据集，边际概率的计算非常繁琐。由于只需要找到给定点的最可能的聚类，可以使用近似方法，因为它们减少了机械功。最好的近似方法之一是使用变分贝叶斯推断方法。该方法使用了 [KL 散度](https://en.wikipedia.org/wiki/Kullback%E2%80%93Leibler_divergence)和[平均场近似](https://en.wikipedia.org/wiki/Mean_field_theory)的概念。
...
贝叶斯高斯混合类的两个主要参数是 `n_components` 和 `covariance_type`。

1.  `n_components`: 它决定给定数据中的最大簇数。
2.  `covariance_type`: 描述要使用的协方差参数的类型。
...
在下面给出的步骤中，参数 `n_components` 将固定为 5，而参数 `covariance_type` 将针对所有可能的值而变化，以可视化该参数对聚类的影响。

## 步骤 5: 为不同的协方差类型值建立聚类模型并可视化结果

### a) `covariance_type = 'full'`
