...
> 原文：`https://www.geeksforgeeks.org/ml-gradient-boosting/`
...
整体由 *N* 棵树组成。使用特征矩阵 *X* 和标签 *y* 训练树 1。标记为 *y1(hat)* 的预测用于确定训练集残差 *r1*。然后使用树 1 的特征矩阵 *X* 和残差 *r1* 作为标签来训练树 2。预测结果 *r1(hat)* 然后用于确定残差 *r2*。重复该过程，直到形成集合的所有 *N* 棵树都被训练。
...
`scikit-learn` 中的梯度增强回归类是 **`GradientBoostingRegressor`**。一个类似的算法被用于被称为 **`GradientBoostingClassifier`** 的分类。
