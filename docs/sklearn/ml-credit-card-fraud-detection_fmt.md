...
> 原文: [https://www.geeksforgeeks.org/ml-credit-card-fraud-detection/](https://www.geeksforgeeks.org/ml-credit-card-fraud-detection/)
...
可以从[这个链接](https://www.kaggle.com/mlg-ulb/creditcardfraud/download)下载数据集，如果链接不工作请转到[这个](https://www.kaggle.com/mlg-ulb/creditcardfraud)链接登录 `kaggle` 下载数据集。
...
在热图中，我们可以清楚地看到，大多数特征与其他特征不相关，但有一些特征彼此之间存在正相关或负相关。例如，*V2* 和 *V5* 与被称为 `Amount` 的特征高度负相关。我们也看到了与 `V20` 和 `Amount` 的一些相关性。这让我们对可用数据有了更深入的了解。
