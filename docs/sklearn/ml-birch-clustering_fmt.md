...
*   **`n_clusters`**：整个 BIRCH 算法完成后返回的聚类数，即最终聚类步骤后的聚类数。如果设置为 `None`，则不执行最后的聚类步骤，并返回中间聚类。

**Python 中 BIRCH 的实现：**
为了这个例子，我们将使用 scikit-learn 的 `make_blobs()` 方法生成一个用于聚类的数据集。要了解 `make_blobs()` 的更多信息，您可以参考以下链接：[https://scikit-learn.org/stable/modules/generated/sklearn.datasets.make_blobs.html](https://scikit-learn.org/stable/modules/generated/sklearn.datasets.make_blobs.html)
...
![](img/e77b5c424fbf7eba0eaa08c337f1ce4c.png)