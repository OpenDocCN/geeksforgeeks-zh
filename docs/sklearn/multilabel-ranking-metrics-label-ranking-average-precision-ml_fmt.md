...
> 原文: [https://www.geeksforgeeks.org/multilabel-ranking-metrics-label-ranking-average-precision-ml/](https://www.geeksforgeeks.org/multilabel-ranking-metrics-label-ranking-average-precision-ml/)
...
与每个标签关联的分数由 ![\hat{f}](img/38285c36748a9ba2a423494fec557473.png "Rendered by QuickLaTeX.com") 表示，其中，
...
为了理解上面的例子，我们来看三类人（以 `[1, 0, 0]` 为代表），猫（以 `[0, 1, 0]` 为代表），狗（以 `[0, 0, 1]` 为代表）。我们收到了三个样品，如 `[1, 0, 0]`、`[1, 0, 1]`、`[1, 1, 0]`。这意味着我们总共有 **5** 个地面真实标签（人类 3 个，猫 1 个，狗 1 个）。

例如，在第一个样本中，只有真正的标签人类在预测标签中获得了第 2 的位置。所以，秩 = 2。接下来我们需要找出一路上有多少正确的标签。只有一个正确的人类标签，因此分子值为 1。因此分数变为 1/2 = 0.5。
...
在第二个样本中，排名第一的预测是人类，其次是猫和狗。人类的分数是 1/1 = 1，狗是 2/3 = 0.66（一路上正确标签的排名数量 / 狗类在预测标签中的排名）。
第二个样本的 `LRAP` 值为：
