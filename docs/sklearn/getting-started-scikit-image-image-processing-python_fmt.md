...
> 原文: [https://www.geeksforgeeks.org/getting-started-scikit-image-image-processing-python/](https://www.geeksforgeeks.org/getting-started-scikit-image-image-processing-python/)
...
`scikit-image` 的大部分功能都存在于子模块中。图像表示为数字阵列，例如灰度二维图像的二维阵列。
...
`skimage.data` 子模块提供了一组返回示例图像的函数。
