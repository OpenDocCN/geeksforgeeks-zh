# ML：线性回归
## sklearn.linear_model.LinearRegression 中的线性回归

> 原文：[https://www.geeksforgeeks.org/ml-linear_model-linear-regression-in-python/](https://www.geeksforgeeks.org/ml-linear_model-linear-regression-in-python/)

这是来自 `sklearn.linear_model` 的普通最小二乘线性回归。
...
> `fit_intercept`：布尔值，默认值为 `True`。是否为模型计算截距。
> `normalize`：布尔值，默认值为 `False`。回归前归一化。
> `copy_X`：布尔值，默认值为 `True`。如果为 `True`，则覆盖 `X` 的副本。
> `n_jobs`：整数，默认值为 `1`。如果为 `-1`，则使用所有 CPU。这将加快处理大型数据集的速度。
...
要下载数据集[点击这里](https://www.geeksforgeeks.org/ml-linear_model-linear-regression-in-python/)。
