...
> 原文: [https://www.geeksforgeeks.org/polynomial-regression-for-non-linear-data-ml/](https://www.geeksforgeeks.org/polynomial-regression-for-non-linear-data-ml/)
...
*   **自由落体运动方程**: 物体在重力作用下自由下落 `t` 秒的距离为 `g t^2`。
*   **匀速加速物体所经过的距离**: 该距离可以计算为 `ut + (1/2)at^2`，其中，
    > `g` = 重力加速度
    > `u` = 初始速度
    > `ho` = 初始高度
    > `a` = 加速度
...
我们将使用 Scikit-Learn 的 `PolynomialFeatures` 类来实现。
...
### 步骤 5: 根据均方误差、均方根误差和 r2 得分计算模型的性能。
...
### 步骤 6: 为了提高性能，我们需要把模型做得复杂一点。所以，让我们拟合一个 2 次多项式，然后进行线性回归。
...
**重要事实**: 多项式特征(`degree`= `d`)将包含 `n` 个特征的数组转换为包含 `(n + d)! / d!n!` 特征的数组！

## 结论

多项式回归是处理非线性数据的一种有效方法，因为它可以找到特征之间的关系，这是普通线性回归模型难以做到的。