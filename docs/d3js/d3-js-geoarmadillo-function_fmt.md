...
`d3.js` 中的 `geoArmadillo()` 函数用于绘制给定 `geojson` 数据的**犰狳投影**。这是一种用于世界地图的地图投影，可以用来显示地球的大部分，而不是透视的有限视图。它既不共形也不等面积。默认情况下，中心假定平行度为 20，如果使用不同的平行度，则可以更改中心。
...
<body>
    ...
    <script>
        ...
        d3.json("https://raw.githubusercontent.com/janasayantan/datageojson/master/geoasia.json",
            function (data) {
