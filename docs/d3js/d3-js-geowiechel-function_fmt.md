...
d3.js 中的 `geoWiechel()` 函数用于绘制 Wiechel 投影，这是一种方位角的等面积投影，也是威廉·h·Wiechel 在 1879 年提出的新奇地图。这也是一种改进的方位投影。方向、形状和距离的扭曲在边缘相当严重。
...
该方法根据给定的 JSON 数据创建并返回 Wiechel 投影。

## 示例 1：无旋转的世界Wiechel投影

以下示例对世界进行 Wiechel 投影，中心位于`(0, 0)`且旋转为 0。
...
## 示例 2：自定义中心和旋转的世界Wiechel投影

以下示例在自定义中心和旋转后，对世界进行维切尔投影。
