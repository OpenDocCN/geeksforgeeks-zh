# D3.js 中的 `time.interpolate()` 函数

> 原文: [https://www.geeksforgeeks.org/d3-js-time-interpolate-function/](https://www.geeksforgeeks.org/d3-js-time-interpolate-function/)

`D3.js` 中的 `time.interpolate()` 功能用于更改标尺的范围插值器工厂。如果未指定工厂，则返回当前插值器。
...
*   `interpolate`: 该参数取插值函数。

以下程序说明了 `D3.js` 中的 `time.interpolate()` 函数:
