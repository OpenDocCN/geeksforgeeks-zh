...
> 原文：[https://www.geeksforgeeks.org/d3-js-arearadial-curve-method/](https://www.geeksforgeeks.org/d3-js-arearadial-curve-method/)

使用`D3.js`中的`areaRadial.curve()`方法来指定给定面积径向曲线的类型。`D3.js`提供了几个曲线工厂，可以用来获取不同类型的曲线。
...
* `curve_factory`：这是用于面积径向的曲线类型。这是一个可选参数。

**返回值：** 此方法不返回值。

下面给出了`D3.js`中`areaRadial.curve()`方法的几个例子：
