# D3.js | `d3.bisectRight()`功能

> 原文: [https://www.geeksforgeeks.org/d3-js-d3-bisectRight-function/](https://www.geeksforgeeks.org/d3-js-d3-bisectRight-function/)

`bisectRight()`函数是 D3.js 中的内置函数，它接受一个值作为其参数之一，并返回索引以将元素插入作为另一个参数传递的数组中，从而在指定范围或整个数组中保持排序顺序。
...
以下程序说明了 `d3.bisectRight()`功能的使用:

## 示例 1

这个程序说明了 `d3.bisectRight()`只传递两个强制参数的用法。
...
这个程序演示了 `d3.bisectRight()`将所有四个参数传递给函数的用法。
...
本程序说明了 `d3.bisectRight()`和 `d3.bisectLeft()`功能之间的区别。
...
参考: [https://devdocs.io/d3~5/d3-array#bisectRight](https://devdocs.io/d3~5/d3-array#bisectRight)