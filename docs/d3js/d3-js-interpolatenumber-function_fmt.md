# D3.js `interpolateNumber()` 函数

> 原文: [https://www.geeksforgeeks.org/d3-js-interpolatenumber-function/](https://www.geeksforgeeks.org/d3-js-interpolatenumber-function/)

`d3.interpolateNumber()` 函数用于返回两个给定数字之间的插值。它与 `d3.interpolate()` 函数几乎相同，只是它只接受数字作为参数。
...
下面给出了 `d3.interpolateNumber()` 函数的几个例子。
