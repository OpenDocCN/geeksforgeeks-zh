...
D3.js 中的 `d3.range()` 函数用于返回一个数组，该数组包含从 `start` 参数开始的算术级数，并在一个称为 `step` 的均匀间隔数值序列上迭代，以 `stop` 参数结束。
...
*   `stop`: 是不加入输出数组的独占整数值。
*   `step`: 它是整数值，定期与起始值相加，并打印结果，直到停止值到达。
...
参考: [https://devdocs.io/d3~5/d3-array#range](https://devdocs.io/d3~5/d3-array#range)