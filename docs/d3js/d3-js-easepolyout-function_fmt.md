# D3.js easePolyOut 函数

> 原文: [https://www.geeksforgeeks.org/d3-js-easepolyout-function/](https://www.geeksforgeeks.org/d3-js-easepolyout-function/)

`d3.js` 中的 `D3.easePolyOut()` 函数用于对特定元素应用反向指数缓动过渡效果。如果没有指定指数，那么它默认为 3。

**方法：** `D3.js` 过渡函数将用于对特定元素应用不同的缓动效果。首先，创建一个 SVG 元素并将其附加到 HTML 页面的 `<body>` 中，然后创建一个矩形或任何其他形状并将其附加到 SVG 中。设置所应用形状的一些属性，使其具有良好的颜色和大小，并应用 `d3.transition()` 功能，然后应用 `.ease()` 功能，并将 `d3.easePolyOut` 作为参数来缓动该功能。
