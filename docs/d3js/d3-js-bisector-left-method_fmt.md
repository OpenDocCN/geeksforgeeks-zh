...
**示例 1:** 在这个示例中，我们可以看到，通过使用`bisector.left()`方法，我们能够获得要插入排序数组左侧的元素的索引。
```
