...
> 原文: [https://www.geeksforgeeks.org/d3-js-scalediverging-function/](https://www.geeksforgeeks.org/d3-js-scalediverging-function/)

`D3.js`中的`d3.scaleDiverging()`函数用于构造并返回一个指定域和固定范围的 diverging scale。发散音阶与连续音阶非常相似。唯一的区别是，该标尺的输出范围由插值器固定，因此该范围不可配置。
...
*   `domain`: 这是最大和最小输入值范围。这是一个可选参数。默认域是`[0, 1]`。
*   `interpolator`: 这需要插值器功能。这是一个可选参数。默认函数是标识函数。
