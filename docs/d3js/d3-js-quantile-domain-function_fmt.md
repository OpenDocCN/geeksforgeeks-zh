...
> 原文: [https://www.geeksforgeeks.org/d3-js-quantile-domain-function/](https://www.geeksforgeeks.org/d3-js-quantile-domain-function/)

`quantile.domain()`功能用于设置分位数标度的域。给定的域数组不能为空，并且必须包含至少一个数值。
...
*   `domain`: 该参数设置标度的域，即最小值和最大值。它接受离散数值的数组。数组必须包含至少一个数值。

## 返回值

该函数不返回值。

下面的例子说明了 D3.js 中的 `quantile.domain()`函数:
