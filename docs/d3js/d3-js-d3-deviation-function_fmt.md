...
这个函数接受一个参数`数组`，它是一个要计算标准差的元素数组。这里的元素应该是整数而不是字符串，否则会返回 `undefined`。

## 返回值:

返回给定数组元素的标准差。

下面的程序说明了 D3.js 中的 `d3.deviation()`功能。
...
在上面的输出中，如果参数的元素或字符串少于一个，则返回 `undefined`，如果参数的字符串包含一些整数值，则返回整数值的标准偏差。

## 参考:

[https://devdocs.io/d3~4/d3-array#deviation](https://devdocs.io/d3~4/d3-array#deviation)