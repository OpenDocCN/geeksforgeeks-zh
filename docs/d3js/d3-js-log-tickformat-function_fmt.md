...
> 原文: [https://www.geeksforgeeks.org/d3-js-log-tickformat-function/](https://www.geeksforgeeks.org/d3-js-log-tickformat-function/)

`log.tickFormat()` 函数用于格式化 `log.ticks()` 函数返回的 tick。这个函数类似于 `pow.tickFormat()`，唯一的区别是这个函数是为对数刻度定制的。如果刻度太多，格式化程序可能会为某些刻度返回空字符串，这样可以避免将计数设置为无穷大。
