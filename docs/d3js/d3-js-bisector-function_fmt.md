...
D3.js 中的`bisector()`函数用于使用指定的访问器或比较器函数返回新的平分线。这种方法可以用来平分对象数组，而不是局限于简单的基元数组。
...
该函数只接受一个参数，上面提到了，下面描述了:

*   `accessor`/`comparator`: 此参数是可以是存取器或比较器的功能。
...
该程序使用比较器函数参数说明了`d3.bisector()`的使用。
