...
> 原文: [https://www.geeksforgeeks.org/d3-js-interpolatesinebow-function/](https://www.geeksforgeeks.org/d3-js-interpolatesinebow-function/)

`d3.interpolateSinebow()` 函数是由吉姆·本加德纳和查理·洛德引入的正弦曲线配色方案的一部分。该函数用于返回对应于 Sinebow 配色方案的 `RGB` 颜色字符串。
...
<body>
    ...
    <script>
        // Creating different colors for different values of t
        // Values of t is 0.4
        let color1 = d3.interpolateSinebow(0.4);
        // Values of t is 0.6
        let color2 = d3.interpolateSinebow(0.6);
        // Values of t is 0.8
        let color3 = d3.interpolateSinebow(0.8);
        // Values of t is 0.9
        let color4 = d3.interpolateSinebow(0.9);
        // Values of t is 0
        let color5 = d3.interpolateSinebow(0);
