...
D3.js 中的 `d3.extent()` 函数用于使用自然顺序返回给定数组中的最小值和最大值。如果一个数组是空的，那么它返回 `undefined`，`undefined` 作为输出。
...
这个函数接受一个参数 `Array`，它是一个元素数组，数组中的最小值和最大值都要计算。这里的元素可以是整数或任何字符串。
...
在上述输出中，第一个元素为最小值，第二个元素为最大值。

## 参考

[https://devdocs.io/d3~4/d3-array#extent](https://devdocs.io/d3~4/d3-array#extent)