...
d3.js 中的 `geoBonne()`函数用于绘制 Bonne 伪球面等面积投影。邦尼投影保持了沿中央子午线和标准平行线区域的精确形状，但逐渐远离这些区域扭曲。它根据给定的 geo JSON 数据进行 Bonne 投影。
...
该方法返回 Bonne 伪圆锥等面积投影。

## 示例

下面的示例制作了世界的 Bonne 伪圆锥等面积投影。
