...
D3.js 中的 `d3.select()`函数用于选择与指定选择器字符串匹配的第一个元素。如果有任何元素不匹配，则返回空选择。如果选择器匹配了多个元素，那么将只选择第一个匹配的元素。
...
[https://devdocs.io/d3~5/d3-selection#select](https://devdocs.io/d3~5/d3-selection#select)