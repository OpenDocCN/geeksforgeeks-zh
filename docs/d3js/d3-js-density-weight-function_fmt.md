...
> 原文: [https://www.geeksforgeeks.org/d3-js-density-weight-function/](https://www.geeksforgeeks.org/d3-js-density-weight-function/)

`weight()` 函数用于设置密度轮廓估计器函数的权重取值器。如果给定了权重，则该函数为点权重设置取值器，否则将取值器设置为默认值 1。
...
*   `weight`: 取一个数字，用于设置点权重的取值器。

## 返回值

这个函数不返回任何东西。

下面给出了几个 `weight()` 函数的例子。
