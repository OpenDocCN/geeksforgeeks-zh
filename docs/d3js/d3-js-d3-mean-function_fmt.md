...
D3.js 中的 `d3.mean()` 函数用于返回给定数组元素的平均值。如果数组为空，则返回 `undefined`。
...
该函数接受一个参数 `Array`，它是一个要计算其平均值的元素数组。这里的元素应该是整数而不是字符串，否则会返回 `undefined`。
...
参考: [https://devdocs.io/d3~4/d3-array#mean](https://devdocs.io/d3~4/d3-array#mean)