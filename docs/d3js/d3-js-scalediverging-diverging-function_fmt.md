...
> 原文: [https://www.geeksforgeeks.org/d3-js-scalediverging-diverging-function/](https://www.geeksforgeeks.org/d3-js-scalediverging-diverging-function/)

`发散音阶`与连续音阶非常相似。唯一的区别是，该标尺的输出范围由插值器固定，因此该范围不可配置。

`d3.scaleDiverging()`的`diverging()`函数用于返回一个范围内的值，该范围对应于必须位于指定域内的给定输入值。
