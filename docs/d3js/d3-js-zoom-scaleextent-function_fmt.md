...
> 原文：[https://www.geeksforgeeks.org/d3-js-zoom-scaleextent-function/](https://www.geeksforgeeks.org/d3-js-zoom-scaleextent-function/)

`D3.js`中的`zoom.scaleExtent()`函数用于设置缩放范围到指定的数字数组`[k0, k1]`。`k0`是最小允许的比例因子，`k1`是最大允许的比例因子。
...
*   `extent`：该参数可以定义为返回这样一个数组的函数。

## 返回值

该函数返回缩放行为。

以下程序说明了`D3.js`中的`zoom.scaleExtent()`函数。
...
<body>
    <center>
        ...
        <script>
            ...
            svg.append("rect").attr({"x":100, "y":0, "height":100, "width":100, "fill":"purple"})
