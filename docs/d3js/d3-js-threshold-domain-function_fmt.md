...
> 原文: [https://www.geeksforgeeks.org/d3-js-threshold-domain-function/](https://www.geeksforgeeks.org/d3-js-threshold-domain-function/)

`threshold.domain()`函数用于设置阈值尺度的域。这里给出的值必须是非降序的，如果不是，那么标度的行为是未定义的。
...
*   `domain`: 此参数接受按排序顺序排列的元素数组，该数组设置刻度的定义域。
