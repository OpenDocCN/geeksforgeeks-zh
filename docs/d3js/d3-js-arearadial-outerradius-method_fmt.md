...
> 原文：[https://www.geeksforgeeks.org/d3-js-arearadial-outerradius-method/](https://www.geeksforgeeks.org/d3-js-arearadial-outerradius-method/)

`areaRadial.outerRadius()` 方法用于设置或返回区域径向的外半径取值器。它可以是一个数字，也可以是一个函数，返回一个代表我们区域外半径的数字。
...
<body>
    ...
    <script>
        ...
        var xScale = d3.scaleLinear().domain([0, 8]).range([25, 200]);
        var yScale = d3.scaleLinear().domain([0, 20]).range([200, 25]);
