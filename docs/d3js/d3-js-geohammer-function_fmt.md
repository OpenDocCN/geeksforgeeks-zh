...
D3.js 是一个 JavaScript 库，用于在 web 浏览器中产生动态的、交互式的数据可视化。它利用了可伸缩矢量图形（`SVG`）、`HTML5` 和级联样式表（`CSS`）标准。

`d3.js` 中的 `geoHammer()` 函数用于绘制 Hammer 投影，是 Ernst Hammer 描述的等面积地图投影。
