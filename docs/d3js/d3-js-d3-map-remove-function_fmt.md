...
`map.remove()`函数是 D3.js 中的一个内置函数。如果创建的映射包含给定的键字符串，那么它将删除条目并返回 `true`。如果键字符串不存在，则返回 `false`。
...
如果创建的映射具有指定键字符串的条目，则该函数返回`true`，否则返回`false`。
...
**参考:** [https://devdocs.io/d3~5/d3-collection#map_remove](https://devdocs.io/d3~5/d3-collection#map_remove)