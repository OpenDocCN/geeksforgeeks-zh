...
> 原文：[https://www.geeksforgeeks.org/d3-js-pie-startangle-function/](https://www.geeksforgeeks.org/d3-js-pie-startangle-function/)

`D3.js`中的`pie.startAngle()`函数用于设置饼图的开始角度。指定角度后，它会将起始角度设置为给定的角度或函数，并返回饼图生成器。如果未指定角度，它将返回当前起始角度访问器，默认为无起始角度。
...
*   `angle`：是以弧度为单位指定起始角度的数字或函数。这是一个可选参数。
