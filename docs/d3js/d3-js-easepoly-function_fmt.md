...
`d3.js` 中的 `d3.easePoly()` 函数用于给特定元素赋予对称多项式效果。该聚合缓和函数用作 `d3.ease()` 函数中的参数。

## 方法
D3.js 转换函数将用于对特定元素应用不同的缓和效果。首先，创建一个 SVG 元素并将其附加到 HTML 页面的正文中，然后创建一个圆并将其附加到 SVG 中。设置圆的一些属性，使其具有良好的颜色和大小，并应用 `d3.transition` 函数，后跟 `ease()` 函数，并将 `d3.easePoly` 作为参数应用于 `ease` 函数。
