...
D3.js 中的 `d3.median()` 函数用于返回给定数组元素的中值或中点。如果数组为空，则返回 `undefined`。
...
**参数:** 该函数接受一个参数 `Array`，它是一个元素数组，其中值将被计算。这里的元素应该是整数而不是字符串，否则会返回 `undefined`。

**返回值:** 返回给定数组元素的中值。

下面的程序说明了 D3.js 中的 `d3.median()` 函数。
...
**参考:** [https://devdocs.io/d3~4/d3-array#median](https://devdocs.io/d3~4/d3-array#median)