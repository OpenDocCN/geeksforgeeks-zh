# D3.js `diverging.domain()`函数

> 原文: [https://www.geeksforgeeks.org/d3-js-diverging-domain-function/](https://www.geeksforgeeks.org/d3-js-diverging-domain-function/)

`发散音阶`与连续音阶非常相似。唯一的区别是，该标尺的输出范围由插值器固定，因此该范围不可配置。

`diverging.domain()`功能用于设置标尺的域。域数组必须采用三个数值。
...
*   `domain`: 这是一个取三个数值的数组。刻度的默认范围是`[0, 0.5, 1]`。
