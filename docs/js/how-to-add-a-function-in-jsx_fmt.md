...
[React.js](https://www.geeksforgeeks.org/reactjs-tutorials/) 使用了 [JSX](https://www.geeksforgeeks.org/javascript-tutorial/) 扩展，为我们编写应用程序提供了更简单的方式。功能是任何网络应用程序最重要的部分之一，使用它们，我们可以对用户界面进行优化计算和渲染，最终为我们提供一个完整构建的网站。在本文中，我们将讨论如何在 JSX 中添加一个函数。
...
*   **步骤 2：** 创建项目文件夹（即 `foldername`）后，使用以下命令移动到该文件夹中：
    ...
    cd foldername
    ```
*   **步骤 3：** 在您的系统上设置 React 环境后，我们可以从创建一个 `App.js` 文件开始，并创建一个名为 `components` 的目录，我们将在其中编写我们想要的函数。
...
在本例中，我们将考虑一个示例来演示如何将函数添加到我们的应用程序中。现在在 `components` 目录中，我们将创建一个 `display.js` 文件，该文件将包含将要添加到应用程序中的显示功能。
...
现在，在这个示例中，我们将创建带有更改的 `display.js` 文件。这次我们可以从 `App.js` 文件中修改表头的内容。
...
从代码中，我们可以预测现在的输出。请注意，您可以通过更改标签中的 `name` 属性来渲染自定义输出。
...
*   [https://www.geeksforgeeks.org/reactjs-functional-components/](https://www.geeksforgeeks.org/reactjs-functional-components/)
