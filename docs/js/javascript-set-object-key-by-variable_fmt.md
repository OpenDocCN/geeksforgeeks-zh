...
> 原文: [https://www.geeksforgeeks.org/javascript-set-object-key-by-variable/](https://www.geeksforgeeks.org/javascript-set-object-key-by-variable/)
...
本示例通过变量`key`设置对象键，然后将`{"GFG_key":"GFG_Val"}`对象插入数组。
...
本示例通过变量`key`设置对象键，并通过变量`val`设置对象值，然后将`{"GFG_key":"GFG_N_Val"}`对象插入到数组中。
