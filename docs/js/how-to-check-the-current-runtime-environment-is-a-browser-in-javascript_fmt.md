...
> 原文：`https://www.geeksforgeeks.org/how-to-check-the-current-runtime-environment-is-a-browser-in-javascript/`
...
*   我们首先使用 `typeof` 操作符检查 `process` 是否属于“对象”类型，以及 `require` 的类型是否属于功能。当这两个条件都为真时，那么环境就是 Node.js，因此我们返回 `false`。
*   我们通过检查 `importScripts` 的类型是否是一个函数来类似地检查环境是否是一个服务工作者。如果条件匹配，我们再次返回 `false`。
*   最后，我们检查 `window` 的类型是否等于一个‘对象’。`true` 条件表示环境是浏览器，我们从函数返回 `true`。
