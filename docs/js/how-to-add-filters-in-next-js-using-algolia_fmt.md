...
> 原文：[https://www.geeksforgeeks.org/how-to-add-filters-in-next-js-using-algolia/](https://www.geeksforgeeks.org/how-to-add-filters-in-next-js-using-algolia/)
...
首先添加我们的过滤器，我们将在 algolia 创建一个帐户，使我们能够在几毫秒内搜索内容。之后，我们将获得我们稍后将在应用程序中使用的 API 密钥。然后我们将创建一个新的索引来上传我们的数据。在我们应用程序的主页上，我们将使用 API 键和 `algoliasearch` 模块从 algolia 获取带有过滤器小部件的数据。
...
1.  要获取 API 密钥，请转到设置 > API 密钥。

![](img/f47ca5cc432d1357112afc3f3b73c653.png)

2.  之后，创建一个索引并上传您想要过滤的数据。您可以以 JSON、CSV 格式或使用它们的 API 上传数据。
...
您可以使用 algolia 的 `RefinementList` 小部件轻松添加复选框过滤器。使用这个小部件，用户可以基于方面值过滤数据集。小部件只显示当前搜索上下文最相关的方面值。现在我们将在我们的 `index.js` 文件中添加下面的代码。在下面的代码中，首先，我们导入我们的 `RefinementList`，然后，我们用 `Title` 属性调用我们的 `RefinementList`。
...
可以使用 algolia 的 `Menu` 小部件轻松添加 Onclick 滤镜。使用这个小部件，用户可以基于方面值过滤数据集。小部件只显示当前搜索上下文最相关的方面值。现在我们将在我们的 `index.js` 文件中添加下面的代码。在下面的代码中，首先，我们导入我们的 `Menu`，然后，我们用 `Title` 属性调用我们的 `Menu`。
...
可以使用 algolia 的 `MenuSelect` 小部件轻松添加下拉过滤器。使用这个小部件，用户可以基于方面值过滤数据集。小部件只显示当前搜索上下文最相关的方面值。现在我们将在我们的 `index.js` 文件中添加下面的代码。在下面的代码中，首先，我们导入我们的 `MenuSelect`，然后，我们用 `Title` 属性调用我们的 `MenuSelect`。
