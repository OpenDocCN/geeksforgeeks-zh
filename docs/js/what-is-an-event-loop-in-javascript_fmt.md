...
> 原文：[https://www.geeksforgeeks.org/what-is-an-event-loop-in-javascript/](https://www.geeksforgeeks.org/what-is-an-event-loop-in-javascript/)
...
函数栈是一个跟踪运行时执行的所有其他函数的函数。当你在 JavaScript 中遇到一个错误时，你是否见过一个堆栈跟踪被打印出来。这只不过是错误发生时函数堆栈的快照。
...
事件循环是指每当函数堆栈变空时，将东西从队列中取出并放入函数执行堆栈的循环。
