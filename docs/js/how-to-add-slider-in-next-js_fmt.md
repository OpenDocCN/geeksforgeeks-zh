...
**方法：** 要添加我们的滑块，我们将使用 `react-range` 包。`react-range` 包帮助我们在应用程序的任何地方集成滑块。首先，我们将安装 `react-range` 软件包，然后我们将在主页上添加一个滑块。
...
安装 `react-range` 包后，我们可以很容易地在我们的应用程序中添加滑块。对于这个例子，我们将在我们的主页上添加一个滑块。
