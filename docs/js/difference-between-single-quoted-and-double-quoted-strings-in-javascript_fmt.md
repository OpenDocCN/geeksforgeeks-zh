...
> 原文: [https://www.geeksforgeeks.org/difference-between-single-quoted-and-double-quoted-strings-in-javascript/](https://www.geeksforgeeks.org/difference-between-single-quoted-and-double-quoted-strings-in-javascript/)
...
**使用单引号字符串:** 在使用单引号字符串定义字符串文字时，我们只需要转义字符串内部的单引号。而无需转义双引号，可以精确书写。
...
**输出:** 在这个例子中，`GeeksforGeeks`必须写在单引号内。因为在定义字符串文字时，我们使用的是单引号字符串，所以我们需要在字符串中使用反斜杠(`\`)来转义单引号(`'`)。`工程师`是用双引号引起来的。由于定义字符串文字时，我们使用的是单引号字符串，因此，我们不需要在字符串中转义双引号(`" "`)。
...
**输出:** 在本例中，`GeeksforGeeks`必须用双引号括起来。因为在定义字符串文字时，我们使用的是双引号字符串，所以我们需要在字符串中使用反斜杠(`\`)来转义双引号(`" "`)。`工程师`用单引号括起来。由于定义字符串文字时，我们使用的是双引号字符串，因此，我们不需要在字符串中转义单引号(`'`)。
...
*   **示例 2:** 使用**反引号**或**发音符号(`)**是使用单引号或双引号的替代方法。在这种情况下，您不需要转义任何单引号或双引号。所以，这是一个没有错误的选择。同样，用反引号括起来的字符串将严格且精确地等于用单引号或双引号括起来的字符串。
...
**输出:** 在这里，您可以看到，当我们在反引号(`)中封装字符串时，我们没有转义任何双引号或单引号，这为我们产生了相同的输出。
...
**要记住的几点:** 虽然反引号(`)可以在任何地方使用，并且错误更少，但是当从`JavaScript`中处理`JSON`文件时，`stringify()`和`parse()`函数必须只包含在双引号中，因为它已经知道了双引号。
...
*   虽然您可以在上述两种情况下使用反引号(`)，但您不需要逃避任何事情。