...
> 原文: [https://www.geeksforgeeks.org/7-javascript-shorthand-techniques-that-will-save-your-time/](https://www.geeksforgeeks.org/7-javascript-shorthand-techniques-that-will-save-your-time/)

在本文中，我们将讨论 JavaScript 中 7 个很酷的速记技巧，它们将节省开发人员的时间。

## 1. 箭头函数
在 ES6 中引入了 JavaScript 箭头函数。这个函数的主要优点是，它有更短的语法。
...
## 3. For 循环
要循环一个数组，我们通常使用传统的 `for` 循环。我们可以利用 `for...of` 循环来迭代数组。要访问每个值的索引，我们可以使用 `for...in` 循环。
...
## 4. 将字符串转换为数字
有内置的方法，如 `parseInt()` 和 `parseFloat()` 可用于将字符串转换为数字。也可以简单地在字符串值前面提供一元运算符 (`+`)。
...
## 5. 互换两个变量
为了交换两个变量，我们经常使用第三个变量。但是它可以很容易地用数组解构赋值来完成。
...
## 7. 多条件检查
对于多值匹配，我们可以将所有值放在数组中，并使用 `indexOf()` 或 `includes()` 方法。
