...
JSP API 是一组可以用来制作 JSP 页面的类和接口。这些类和接口包含在 `javax.servlet.jsp` 包中。`javax.servlet.jsp` 包中描述的一部分类有:
...
错误信息类表征错误页面的错误数据。您必须设置页面要求的估计值，`isErrorPage`以与证明页面是错误页面保持一致。`ErrorData`类扩展了`java.lang.Object`类。可以在 JSP 页面中使用的错误数据中描述的部分策略是:
...
在 JSP 页面中，为了组合活动和布局信息，我们可以利用`JspWriter`类。理解的变量给出了`JspWriter`类对象。`JspWriter`类扩展了`java.io.Writer`类。JSP 页面中可以使用的`JspWriter`类中的一部分技术是:

1.  `clear()`:清除摇篮的物质。如果支持现在被清除，`clear()`技术抛出一个`IOException`特例。
2.  `close()`:关闭并冲洗水流。
3.  `flush()`:冲洗摇篮流。`flush()`策略会刷新一系列写入器和输出流中的每个托架。它抛出`java.io.IOException`特例，以防在关闭流后调用`clear()`或`flush()`。
...
在 servlet 条件下利用 JSP 创新时，设置数据由`PageContext`类给出。`PageContext`类扩展了`JSPContext`类。页面上下文提供了对与 JSP 页面相关的名称空间的访问。`PageContext`类中描述的一些策略是:
...
8.  `include()`: Processes the current servlet demand and the reaction determined in the URL. The `include()` technique accepts two contentions, a URL way and the flush estimation of boolean sort.
