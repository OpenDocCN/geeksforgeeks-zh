...
`Number` 数据类型可以用来保存十进制值，也可以保存不带小数的值。
...
`Boolean` 数据类型只能接受两个值，即 `true` 和 `false`。
...
Javascript 中的对象是具有属性和方法的实体。 在 javascript 中，一切都是对象。
