...
`Intl()` 方法是在构造函数的帮助下，将字符串、数字、日期和时间以本地格式进行格式化。
...
*   **`locales` 参数:** `locales` 参数用于确定操作中使用的语言环境。地区可能是:
    *   未定义: 使用默认区域设置。
...
*   **`options` 参数:** `options` 的参数是一个对象，其属性随不同的构造函数和函数而变化。如果 `options` 的参数是未定义的，则使用所有属性的默认值。

### 例 1: `Intl.Collator()`

`Intl.Collator()` 构造函数创建 `Intl.Collator` 对象，帮助在任何集合的语言敏感排序中进行排序。由于我们用英语对数组进行排序，所以它是按字母顺序排序的。
...
### 例 2: `Intl.DateTimeFormat()`

`Intl.DateTimeFormat()` 构造函数创建一个 `Intl.DateTimeFormat` 对象，有助于区分语言的日期时间格式。这里的子标签是 `"en-US"`，所以日期变量是美国英语的格式。
...
### 例 3: `Intl.DisplayNames()`

`Intl.DisplayNames()` 构造函数创建一个 `Intl.DisplayNames` 对象，帮助构造器翻译任何语言、区域和脚本显示名称。这里 `Intl` 根据地区用英语翻译 `'UN'`。
...
`Intl.ListFormat()` 创建一个 `Intl.ListFormat` 对象，帮助设置列表格式。这里的子标签是 `"en"`，所以它是英文的，样式很长，类型是连词，所以格式化的列表有一些连接词。
...
`Intl.Locale()` 构造函数为区域、语言和脚本创建一个 `Locale` 标识符。这里的子标签是 `"en-US"`，因此它创建了美国英语地区语言区域标识符。
...
### 例 6: `Intl.NumberFormat()`

`Intl.NumberFormat()` 构造函数创建 `Intl.NumberFormat` 帮助格式化数字的对象。样式是十进制，所以数字是十进制格式的格式。
...
### 例 7: `Intl.PluralRules()`

`Intl.PluralRules()` 构造函数创建一个有助于复数敏感格式的对象。它返回一个字符串，该字符串指示用于数字的复数规则。
...
### 例 8: `Intl.RelativeTimeFormat()`

`Intl.RelativeTimeFormat()` 构造函数创建一个对象 `Intl.RelativeTimeFormat` 有助于相对时间格式。它根据提供给构造函数的子标记进行格式化。
