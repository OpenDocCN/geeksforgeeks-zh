...
> 原文：[https://www.geeksforgeeks.org/tips-for-writing-better-conditionals-in-javascript/](https://www.geeksforgeeks.org/tips-for-writing-better-conditionals-in-javascript/)
...
可以对多个条件使用 `array.includes()` 方法。`array.includes()` 方法用于确定数组中是否存在特定元素。它返回“真”或“假”。
...
function test() {
    ...
    const isAllblue = cars.every(c => c.color == 'blue');
...
### 3. Return Early or Early exit

JavaScript 中的“提前返回”是一种简单的方法，可以将函数体中的“else”语句数量减少到零。在代码中使用“提前返回”有很多理由：
1.  它减少了代码总量。
2.  它通过降低逻辑复杂度来减少行的长度。
3.  它有助于提高可读性。
...
**注意：** 始终以少筑巢、早归为目标，但不要糟蹋它。如果你的代码简短直接，如果嵌套的 `if` 更清晰。

### 4. Use Object Literal or Maps

对象文字基本上是一个键值对数组，用于代替 `switch` 语句。让我们看下面的例子。
