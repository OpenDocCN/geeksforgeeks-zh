...
> 原文：[https://www.geeksforgeeks.org/create-a-string-with-multiple-spaces-in-javascript/](https://www.geeksforgeeks.org/create-a-string-with-multiple-spaces-in-javascript/)
...
*   **`start`**：此参数为必填项。它指定开始提取的位置。第一个索引从 0 开始。
...
返回一个新字符串，该字符串包含文本的提取部分。如果 `length` 为 0 或负数，它将返回一个空字符串。
...
本示例通过 `\xa0`（它是一个无中断空格字符）在字符串中添加空格。
