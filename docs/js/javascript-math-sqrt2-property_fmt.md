...
`Math.SQRT2`是 JavaScript 中的一个属性，简单来说就是用来求 2 的平方根值，其值近似为 **1.4142**。
就是说，即 √2 ≈ 1.4142。
...
下面的例子说明了 JavaScript 中的`Math.SQRT2`属性。
...
**程序 2:** 这里我们考虑`Math.SQRT2`作为一个函数，但实际上它是一个属性，这就是为什么错误作为输出被显示。
