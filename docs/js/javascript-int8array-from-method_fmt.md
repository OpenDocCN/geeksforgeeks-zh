...
> 原文: [`https://www.geeksforgeeks.org/javascript-int8array-from-method/`](https://www.geeksforgeeks.org/javascript-int8array-from-method/)

`Int8Array` 数组表示 8 位有符号整数的二进制补码数组。默认情况下，`Int8Array` 的内容被初始化为 0。`from()` 函数用于从类似数组或可迭代对象创建新 `Int8Array`。因此，当您想要将 `arrayLike` 或 `iterable` 对象转换为 `Int8Array` 时，您可以使用此函数，方法是将对象作为参数传递给此函数，如果需要，还可以传递 `map` 函数和 `map` 函数使用的值。
...
1.  **`source`**: 这个参数是一个类似数组或者可迭代的对象，用来转换成 `Int8Array` 对象。
