...
**捕获阶段**是事件深入元素的时候。**目标阶段**是事件到达元素时，**冒泡阶段**是事件从元素冒泡时。
...
在上面的代码中，`body` 和 `button` 处于冒泡阶段(默认)，而 `div` 设置为捕获阶段。当点击 `button` 时，它再次从顶部开始。说到 `body` 元素，它不运行功能，因为我们还处于捕获阶段。但是当它到达 `div` 时，它运行该函数，因为 `addEventListener()` 的第三个参数为 `true`。所以它必须在捕获阶段运行。当它到达按钮时，它从捕获阶段切换到目标阶段，最后切换到冒泡阶段。它触发处于默认模式的 `addEventListener`。
