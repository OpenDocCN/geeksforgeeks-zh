...
### `CSS:` 该段落被指定为 `opacity` 的过渡，持续时间为一秒。此外，要添加到移除元素的 `removed` 类指定最终的 `opacity` 为零。这给移除一个褪色的外观。
...
### `JavaScript:` 首先，我们创建按钮和段落的 `DOM` 对象(带有合适的标签和 `id`)。此外，我们将事件监听器添加到按钮中，这个监听器将一个 `removed` 类添加到段落中，从而降低 `opacity` 并启动淡入淡出过程。最后，我们在段落中添加一个监听器，监听过渡的结束。一旦过渡完成，段落就被删除。
