...
> 原文：[https://www.geeksforgeeks.org/how-to-detect-when-cancel-is-clicked-on-file-input-using-javascript/](https://www.geeksforgeeks.org/how-to-detect-when-cancel-is-clicked-on-file-input-using-javascript/)
...
*   属性值 `id = "theFile"` 用于使用 `getElementById()` 方法将 JavaScript 代码链接到它。
...
*   变量 `theFile` 被创建，并且使用其 id 选择输入元素。
*   `document.body.onfocus = checkIt` 行定义了当一个元素获得焦点时，`onfocus` 事件被激发。

**`checkIt()` 功能说明：**

*   当用户上传任何文件时，使用 `theFile.value.length` 属性可以找到文件的长度。当该值变为非零值时，条件得到满足，并出现一个显示“文件已加载”的警告框。
*   当用户不上传文件时，文件的长度为零。因此，条件不会得到满足，并且会出现一个警告框，显示“已单击取消”。

## 完整代码

在本节中，我们将结合以上两段代码来检测在文件输入时何时点击取消。
