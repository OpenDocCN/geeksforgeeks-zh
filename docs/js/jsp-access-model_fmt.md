...
JSP Model 1 架构或 JSP 中心架构包含 `JavaBeans` 或 `EJB` 模型对象、视图 JSP 页面和动作 JSP 页面。

在 Model 1 架构中，来自 Web 浏览器的传入请求被直接发送到 JSP 页面，该 JSP 页面负责处理它并将其发送回客户端。所有数据访问都使用 `beans` 执行，因此仍然实现了表示与内容的分离。
...
2.  访问 JSP 页面主要是 java 代码，但是不能像 java 代码那样容易开发、编译和调试。
...
JSP Model 2 架构或以 servlet 为中心的架构包含 `Java beans` 或 `EJB` 模型对象、查看 JSP 页面和 `Servlet` 或命令类。
