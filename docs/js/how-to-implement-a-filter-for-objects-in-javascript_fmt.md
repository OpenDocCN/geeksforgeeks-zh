# 如何在 JavaScript 中实现对象的 filter()？

> 原文: [https://www.geeksforgeeks.org/how-to-implement-a-filter-for-objects-in-javascript/](https://www.geeksforgeeks.org/how-to-implement-a-filter-for-objects-in-javascript/)
...
*   `callback` 是一个谓词，用来测试数组的每个对象。返回“真”保留对象，否则返回“假”。它包含三个参数:
    *   `object`: 数组中正在处理的当前对象。
    *   `ind` (可选): 数组中正在处理的当前对象的索引。
    *   `array` (可选): 调用 `filter` 的数组。
    *   `Arg` (可选): 要使用的值 (`.this`) 时执行回调。
