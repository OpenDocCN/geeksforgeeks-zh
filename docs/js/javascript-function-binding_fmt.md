...
我们使用 `bind()` 方法调用一个带有 `this` 值的函数，`this` 关键字指的是当前选中的同一个对象。换句话说，`bind()` 方法允许我们在调用函数或方法时，轻松设置哪个对象将被 `this` 关键字绑定。

绑定的需求通常会出现，当我们在一个方法中使用 `this` 关键字，并且我们从一个接收者对象调用该方法时，那么有时候 `this` 并没有绑定到我们期望绑定的对象。这导致了我们程序中的错误。

## 默认绑定示例

现在，调用函数 `printFunc()` 时，一个简单的程序打印 `this` 关键字调用的名称。
...
这里访问名称 `"ABC"`，`this` 关键字将 `name` 变量绑定到函数是没有问题的。它被称为默认绑定。`this` 关键字指的是 `geeks` 对象。
...
这里我们做了一个新的变量函数 `printFunc2`，指的是对象 `geeks` 的函数 `printFunc()`。这里 `this` 的绑定丢失，所以不产生输出。

## 使用 bind() 方法

为了确保 `this` 的任何绑定都不会丢失，我们使用 `bind()` 方法。

通过使用 `bind()` 方法，我们可以将 `this` 的上下文设置为特定的对象。因此，我们也可以使用其他变量来调用绑定函数。
...
`bind()` 方法创建了一个新函数，其中 `this` 关键字引用了上例 `geeks` 括号中的参数。通过这种方式，`bind()` 方法能够调用具有指定的 `this` 值的函数。
