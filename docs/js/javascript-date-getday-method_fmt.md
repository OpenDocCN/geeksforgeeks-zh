...
> 原文: [https://www.geeksforgeeks.org/javascript-date-getday-method/](https://www.geeksforgeeks.org/javascript-date-getday-method/)
...
返回给定日期的星期几。一周中的某一天将以整数值的形式返回，从 0 到 6 表示周日为 0，周一为 1，以此类推，直到周六为 6。

上述方法的更多代码如下:

### 程序 1

如果没有给出月份的日期，默认情况下，函数会将其视为月份的第一个日期，因此会返回星期几。这是一个特例。
...
JavaScript `Date getDay()` 方法支持的浏览器如下:
