...
> 原文: [https://www.geeksforgeeks.org/how-to-slide-down-the-page-after-video-ends-using-javascript/](https://www.geeksforgeeks.org/how-to-slide-down-the-page-after-video-ends-using-javascript/)
...
编写 HTML 标记，将视频元素添加到您的 HTML 文件中。
...
在 CSS 文件中添加样式(或者根据您的选择添加任何其他样式)。
...
添加 JavaScript，只允许页面在视频结束后滑动或滚动。
...
*   我们终于添加了最后一段最重要的代码。我们使用了 JavaScript 的 `onended` 函数，去掉了溢出被隐藏的属性，使用了 JavaScript 中的 `scrollIntoView()` 函数。
...
<body>
    <`video` `id`="Vid" `width`="320" `height`="176" `controls`>
        <`source` `src`="https://media.geeksforgeeks.org/wp-content/uploads/20210219152742/video.mp4" `type`="video/mp4">
        Your browser does not support HTML5 video.
    </`video`>
