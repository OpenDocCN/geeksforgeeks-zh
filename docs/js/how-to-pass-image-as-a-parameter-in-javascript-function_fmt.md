...
> 原文：[https://www.geeksforgeeks.org/how-to-pass-image-as-a-parameter-in-javascript-function/](https://www.geeksforgeeks.org/how-to-pass-image-as-a-parameter-in-javascript-function/)
...
在本例中，我们将在 id 为“imgDiv”的 DIV 中显示一个图像。
...
4.  现在用 `document.createElement()` 创建一个 img 元素，并将其分配到变量 `imgElement` 中。
5.  然后通过 `imgElement.src = pic` 将图像的 URL 分配给它的 `src` 属性。
6.  现在通过 `append()` 方法将 img 元素追加到 div 元素中。
