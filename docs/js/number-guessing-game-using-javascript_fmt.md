...
> 原文：[https://www.geeksforgeeks.org/number-guessing-game-using-javascript/](https://www.geeksforgeeks.org/number-guessing-game-using-javascript/)
...
1.  `document.getElementById("给定 Id ")`：`document.getElementById()` 用于从具有用户提供（指定）的 id 的 HTML 页面中获取元素。`.value` 用于访问被访问的 HTML 元素的值。
