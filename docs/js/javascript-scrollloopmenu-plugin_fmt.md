...
在本文中，我们将学习如何使用基于 JavaScript 的滚动循环菜单插件来实现滚动循环菜单。这是另一种简单的交互式滚动菜单，用于开发网页，以获得良好的视觉效果。

## 注意

请下载工作文件夹中的 JavaScript `scrolloopmenu` 插件，并在你的 HTML 代码头部包含所需文件。
...
以下示例演示了使用 HTML 控件和基于 JavaScript 的滚动循环菜单插件的滚动循环菜单。插件的 `base.css` 文件的 `menu`、`menu _ item-inner`、`frame _ _ link` 等不同类被附加到 HTML 标记上，如下所示，以获得预期的输出。
