...
> 原文：[https://www.geeksforgeeks.org/how-to-detect-the-device-is-an-android-device-using-javascript/](https://www.geeksforgeeks.org/how-to-detect-the-device-is-an-android-device-using-javascript/)

任务是检测一个设备，这个设备是**安卓手机**还是没有使用 JavaScript。

## 方法 1

*   使用 `navigator.userAgent` 属性获取浏览器发送给服务器的用户代理头的值。
*   检查用户代理中“android”的索引。
...
*   获取变量名用户代理中的 `navigator.userAgent` 属性。
*   使用正则表达式在用户代理中检查“安卓”
*   如果 `android` 的值不是 0，那么就是安卓手机，否则不是安卓手机。
