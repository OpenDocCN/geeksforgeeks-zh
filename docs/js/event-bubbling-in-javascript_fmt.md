...
*   `type`: 用于指事件的类型。
*   `listener`: 当指定类型的事件发生时，我们要调用的函数。
*   `useCapture`: 布尔值。布尔值表示事件阶段。默认情况下，`useCapture` 为 `false`。这意味着它正处于冒泡阶段。

## 示例 1

这个示例展示了 JavaScript 中事件冒泡的工作方式。
...
从上面的例子中我们了解到，在冒泡中，首先处理最里面的元素的事件，然后处理最外面的：首先处理 `<button>` 元素的点击事件，然后处理 `<div>` 元素的点击事件。