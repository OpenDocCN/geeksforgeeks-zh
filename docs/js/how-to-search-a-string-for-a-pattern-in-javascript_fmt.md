...
> 原文：[https://www.geeksforgeeks.org/how-to-search-a-string-for-a-pattern-in-javascript/](https://www.geeksforgeeks.org/how-to-search-a-string-for-a-pattern-in-javascript/)

在本文中，我们将看到如何在 JavaScript 中搜索字符串模式。我们将使用以下方法来搜索字符串：

## 方法 1：使用 `search()`

在这里，我们将学习如何在 JavaScript 中搜索包含给定模式的字符串。`string.search()` 方法是 JavaScript 中用于此目的的内置方法。它搜索给定字符串中正则表达式之间的匹配。
...
返回给定字符串中第一个匹配正则表达式的索引值，否则返回 `-1`。它从索引 0 开始，如果匹配了任何字母表，它将返回其对应的索引，并且不再进一步检查。
...
**解释：** 我们可以观察到第一个 `G` 匹配出现在索引 0，而第一个 `c` 匹配出现在第 17 个索引，而字母 `z` 在字符串 "GeeksforGeeks is computer science portal" 中不存在，因此返回 `-1`。
...
我们还可以使用 JavaScript String `match()` 函数，只要找到与给定字符串匹配的字符串，该函数就会返回包含给定表达式的数组，否则返回 `null`。
...
*   **String name：** 我们想要搜索模式的字符串的名称作为参数。
*   **Expression：** 是我们要检查的模式/子串是否存在于上述字符串中。
