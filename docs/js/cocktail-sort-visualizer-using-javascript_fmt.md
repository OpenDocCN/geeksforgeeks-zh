...
> 原文: [https://www.geeksforgeeks.org/cocktail-sort-visualizer-using-javascript/](https://www.geeksforgeeks.org/cocktail-sort-visualizer-using-javascript/)
...
像`鸡尾酒排序`这样的算法可以通过可视化而不是长代码来轻松理解。在本文中，`鸡尾酒排序可视化工具`是使用`HTML`、`CSS`和`JavaScript`实现的。
...
*   按钮`生成新数组`使用`Math.random()`函数生成一个随机值数组，以及一个对应于该值的高度条。
