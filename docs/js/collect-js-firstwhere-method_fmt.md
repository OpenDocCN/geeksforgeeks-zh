...
`firstWhere()`方法用于返回具有给定键和值对的集合中的第一个元素。通过仅指定对象中的任何键-值对，可以使用它来查找数组中的任何元素。
...
`collect()`方法接受一个转换为集合的参数，然后对其应用`firstWhere()`方法。`firstWhere()`方法将要搜索的键和值对作为参数。
...
let first_Val = collection.firstWhere("name", "Rahul");
...
let first_Val = collection.firstWhere("dob", "25-10-96");
console.log(first_Val);
```
