...
## 方法 1:使用带有“平滑”行为的 `scrollIntoView()`:

使用 `scrollIntoView()` 方法将用户的视图滚动到它被调用的元素。它包含几个选项，可以定义这些选项来修改滚动行为。其中之一是“行为”属性。此属性的默认值使滚动立即跳转到其目的地，而不是平滑滚动。将此值设置为“平滑”会更改此行为，并使页面平滑滚动。

锚定链接的哈希部分首先使用 `hash` 属性提取，然后使用 `querySelector()` 方法选择。然后，在这个选定的元素上调用 `scrollIntoView()` 方法，将页面平滑地滚动到这个位置。
...
## 方法二:使用 jQuery `scrollTop()` 方法:

jQuery 中的 `scrollTop()` 方法用于滚动到页面的特定部分。使用可用的内置动画制作此方法的动画可以使滚动更加平滑。
