...
> 原文：[https://www.geeksforgeeks.org/how-to-describe-object-arguments-in-jsdoc/](https://www.geeksforgeeks.org/how-to-describe-object-arguments-in-jsdoc/)
...
对于只用作给定方法的参数而不需要进一步记录每个属性的对象，该语法是一个很好的实践。这个也可以用于 `@returns`。
...
这种语法对于只用作给定方法的参数的对象来说是一种很好的做法，它们需要每个属性的进一步文档。这个不能用于 `@returns`。
...
`@typedef` 在这种情况下很有用。一旦在源码中定义了类型，你就可以将其用作 JSDoc 标签（如 `@param` 或 `@returns`）的类型。
