...
> 原文: [https://www.geeksforgeeks.org/how-to-set-location-and-location-href-using-javascript/](https://www.geeksforgeeks.org/how-to-set-location-and-location-href-using-javascript/)

`location` 和 `location.href` 用于设置或返回当前页面的完整网址。它们返回一个字符串，该字符串包含协议的整个网址。
...
像 `location.split("#");` 这样的命令不能作为 `location` 是一个对象，但是 `location` 可以作为一个字符串。

## 示例

下面的代码演示了 DOM Location `href` 属性。
