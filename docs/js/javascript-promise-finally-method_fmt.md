...
> 原文: [https://www.geeksforgeeks.org/javascript-promise-finally-method/](https://www.geeksforgeeks.org/javascript-promise-finally-method/)

`Promise`对象的`finally()`方法用于在承诺结算时返回一个承诺，即要么兑现要么拒绝。承诺是一个JavaScript对象，它在异步函数成功执行后生成一个值。当它由于超时而没有成功执行时，就会产生一个错误。
