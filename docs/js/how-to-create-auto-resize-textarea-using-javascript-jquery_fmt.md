...
> 原文: [https://www.geeksforgeeks.org/how-to-create-auto-resize-textarea-using-javascript-jquery/](https://www.geeksforgeeks.org/how-to-create-auto-resize-textarea-using-javascript-jquery/)

创建一个文本区域，当我们在其中键入或粘贴内容时，任务是自动调整它的大小。可以通过使用 JavaScript 和 jQuery 来实现。

## 方法一: 使用 JavaScript

为了改变高度，创建了一个新的函数来改变 `textarea` 的 `style` 属性。文本区的高度首先设置为 `auto`。该值使浏览器自动设置元素的高度。这将使文本可滚动，因为文本区域的高度小于文本。紧接在下一行，高度再次设置为等于 `scrollHeight`。
...
与上面使用的方法类似。jQuery 中的 `on()` 方法用于将事件处理程序附加到任何元素。首先选择 `textarea`，此方法用于在所选元素上应用事件处理程序。
在回调中声明了一个新函数，该函数改变了 `textarea` 的 `style` 属性。文本区域的高度首先设置为 `auto`，然后立即在下一行，高度再次设置为等于 `scrollHeight`。
