...
> 原文：`https://www.geeksforgeeks.org/javascript-uint8array-of-method/`

`Uint8Array`数组表示8位无符号整数的数组。这些值由0初始化。数组中的元素可以由对象的方法引用，也可以使用标准语法（通过括号表示法）引用。

`Uint8Array.of()`方法根据可变数量的参数创建一个新的类型化数组。
...
在此示例中，传递的值是通过方法转换为`Uint8`的`int`值。-49999和799分别转换为177和31。
