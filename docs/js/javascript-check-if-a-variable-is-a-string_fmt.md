...
> 原文: [https://www.geeksforgeeks.org/javascript-check-if-a-variable-is-a-string/](https://www.geeksforgeeks.org/javascript-check-if-a-variable-is-a-string/)

使用 `typeof` 运算符可以检查变量的类型。它直接应用于变量名或变量。
...
本示例检查变量 `boolValue` 和 `numValue` 是否为字符串。
...
本示例检查变量 `strValue` 和 `objGFG` 是否为字符串。
