...
> 原文: [https://www.geeksforgeeks.org/javascript-check-if-a-key-exists-inside-a-json-object/](https://www.geeksforgeeks.org/javascript-check-if-a-key-exists-inside-a-json-object/)
...
*   `prop`: 此参数为必填项。它指定要检查的属性的字符串名称或符号。
...
本示例使用 `hasOwnProperty()` 方法检查 `obj` 对象的 `prop_1` 属性。
...
本示例通过简单的数组访问来检查 `obj` 对象的 `pro_1` 属性。
