...
> 原文: [https://www.geeksforgeeks.org/javascript-insert-a-string-at-position-x-of-another-string/](https://www.geeksforgeeks.org/javascript-insert-a-string-at-position-x-of-another-string/)
...
```
**参数:**
*   `start`: 此参数为必填项。它指定开始提取的位置。第一个字符从位置 0 开始。
*   `end`: 此参数为可选。它指定停止提取的位置(不包括它)。如果不使用，`slice()`将选择从开始位置到结束位置的所有字符。
...
```
**参数:**
*   `separator`: 此参数为可选。它指定要使用的分隔符。如果不使用，元素用逗号分隔
...
```
**参数:**
*   `start`: 此参数为必填项。它指定开始提取的位置。第一个字符位于索引 0 处。
    如果 `start` 为正数，并且大于或等于字符串的长度，则此方法返回一个空字符串。
    如果 `start` 为负，则该方法从末尾将其作为索引。
    如果 `start` 为负或大于字符串长度，`start` 将被用作 0。
*   `length`: 此参数可选。它指定要提取的字符数。如果不使用，它将提取整个字符串。

**返回值:**
它返回一个新的字符串，包含文本的提取部分。如果 `length` 为 0 或负数，则返回空字符串。

## 示例 1
本示例使用 `slice()` 和 `join()` 方法将一个字符串插入到另一个字符串中。
```html
<!DOCTYPE HTML>
<html>
<head>
    <title>
        JavaScript | Insert string at position X of another string.
    </title>
</head>
<body style="text-align:center;" id="body">
    ...
    <script>
        ...
        var b = 'For';
        var pos = 5;
        up.innerHTML = 'Str_1 = "' + a + '"<br>Str_2 = "' + b + '"';

        function GFG_Fun() {
            down.innerHTML = [a.slice(0, pos), b, a.slice(pos)].join('')
        }
    </script>
</body>
</html>
...
## 示例 2
本示例使用 `substr()` 方法将一个字符串插入到另一个字符串中。
```html
<!DOCTYPE HTML>
<html>
<head>
...
<body style="text-align:center;" id="body">
    ...
    <script>
        ...
        function GFG_Fun() {
            down.innerHTML = a.substr(0, pos) + b + a.substr(pos);
        }
    </script>
</body>
</html>
