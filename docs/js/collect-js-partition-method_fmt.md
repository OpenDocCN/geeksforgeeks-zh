...
`collect()`方法接受一个参数，该参数被转换为集合，然后对其应用`partition()`方法。`partition()`方法将回调函数作为参数保存。
