...
> 原文: [https://www.geeksforgeeks.org/javascript-errors-throw-and-try-to-catch/](https://www.geeksforgeeks.org/javascript-errors-throw-and-try-to-catch/)
...
> `throw` 语句让你自己犯错误。
> 最后 `finally` 语句让您在尝试和捕获之后执行代码。不管 `try`/`catch` 块的结果如何，`finally` 块都会运行。
...
当出现任何错误时，JavaScript 将停止并生成错误消息。`throw` 语句允许您创建自己的自定义错误。从技术上讲，您可以抛出您的自定义异常（抛出一个错误）。异常可以是 JavaScript 数字、字符串、布尔值或对象。通过使用 `throw` 和 `try`/`catch`，您可以轻松控制程序流并生成自定义错误消息。
...
`finally` 语句块还可以覆盖 `catch` 块的消息，所以使用时要小心。