...
### 参数:
这个关键字可以接受所有已经用来创建构造函数的参数。

下面的例子说明了超级关键字在 JavaScript 中的用法。

### 示例:
在 `FashionDesigner` 类的构造函数中，`super` 已经被用作函数。而在 `FashionDesigner` 类的 `doTasks()` 函数中，`super` 被用作对象。在 `FashionDesigner` 类的构造函数中，`super` 关键字被用作一个函数，通过将参数传递给 `FashionDesigner` 来调用父类的构造函数。这一步很重要，要确保 `FashionDesigner` 是 `Person` 的实例。在 `doTasks()` 函数中，`super` 用作引用父类 `Person` 实例的对象。这里，`super` 关键字用于显式调用父类 `Person` 的方法。
