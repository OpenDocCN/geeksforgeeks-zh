...
> 原文: [https://www.geeksforgeeks.org/subtract-days-from-date-object-in-javascript/](https://www.geeksforgeeks.org/subtract-days-from-date-object-in-javascript/)
...
**参数:**
*   `day`: 为必输参数。它指定表示一个月中某一天的整数。值预期值为 1-31，但也允许其他值。
    *   0 将导致前一个月的最后一天。
    *   -1 将导致前一个月最后一天的前一天。
...
**参数:**
*   `millisec`: 为必输参数。它指定要加/减的毫秒数，1970 年 1 月 1 日午夜。
...
### 例 1
本例使用 `setTime()` 和 `getTime()` 方法从 `today` 变量中减去 4 天。
...
### 例 2
本例使用 `setDate()` 和 `getDate()` 方法从 `today` 变量中减去 365 天。
