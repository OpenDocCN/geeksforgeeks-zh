...
> 原文: [https://www.geeksforgeeks.org/how-to-create-data-grid-for-millions-of-rows-in-javascript/](https://www.geeksforgeeks.org/how-to-create-data-grid-for-millions-of-rows-in-javascript/)

在 JavaScript 中，使用一个简单的循环最多可以显示 `10^5` 行。这也取决于浏览器运行如此复杂的大循环的能力。当如此庞大的任务交给浏览器时，页面就会崩溃。下面的代码是一个示例，说明我们如何填充一个包含 `10^5` 行和 5 列的表格。

`示例 1:` 这个示例将展示如何使用简单的 JavaScript 代码用 `10^5` 行填充一个表。
...
现在让我们对 100 万行，即 `10^6` 行执行相同的操作。下面的代码将实时加载 100 万行。
