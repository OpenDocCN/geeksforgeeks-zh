...
> 原文：[https://www.geeksforgeeks.org/how-to-check-a-key-exists-in-javascript-object/](https://www.geeksforgeeks.org/how-to-check-a-key-exists-in-javascript-object/)

主要有两种方法来检查 JavaScript 对象中是否存在一个键。第一种是使用 `in` 运算符，第二种是使用 `hasOwnProperty()` 方法。

## 方法 1：使用 `in` 运算符

如果指定的属性在对象中，`in` 运算符将返回一个布尔值。
...
本示例使用 `in` 运算符检查 JavaScript 对象中键的存在。
...
`hasOwnProperty()` 方法返回一个布尔值，该值指示对象是否具有指定的属性。可以在此函数中传递所需的键名，以检查它是否存在于对象中。
...
本示例使用 `hasOwnProperty()` 方法检查 JavaScript 对象中键的存在。
