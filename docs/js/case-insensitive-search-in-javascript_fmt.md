...
> 原文：[https://www.geeksforgeeks.org/case-insensitive-search-in-javascript/](https://www.geeksforgeeks.org/case-insensitive-search-in-javascript/)

**不区分大小写：** 表示对字母大写不敏感的文本或键入的输入，如“**Geeks**”和“**geeks**”在不区分大小写的搜索中必须同等对待。在 Javascript 中，我们使用 `string.match()` 函数搜索字符串中的正则表达式，`match()` 函数返回匹配项，作为 `Array` 对象。
...
**参数：** 该方法接受单参数 `regexp`，这是必需的。它用于将要搜索的值作为正则表达式传递。

**正则表达式（regexp）：** 是一种特殊的语法 `/模式/修饰符`；修饰符设置类型。例如 `/geeksforgeks/i`，其中 `i` 设置为不区分大小写。
