...
> 原文: [https://www.geeksforgeeks.org/what-is-object-equality-in-javascript/](https://www.geeksforgeeks.org/what-is-object-equality-in-javascript/)
...
我们可以说，当两个对象的指针相同时，或者当操作符是同一对象实例时，两个对象是引用相等的。

我们可以用三种方法检查引用相等性：

*   `===`（三重相等）运算符或严格相等运算符。严格意义上的平等是指两种价值观的平等。如果两个值具有相同的类型，则认为它们相等。
*   `==`（double equals）是松散相等运算符。它将这两个值转换为一个公共类型，然后检查是否相等。
*   [`Object.is()`](https://www.geeksforgeeks.org/object-is-in-javascript/) 功能。
