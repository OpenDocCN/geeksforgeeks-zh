...
> 原文：[`https://www.geeksforgeeks.org/javascript-reflect-deleteproperty-method/`](https://www.geeksforgeeks.org/javascript-reflect-deleteproperty-method/)

JavaScript 中的 `Reflect.deleteProperty()` 方法用于删除对象上的属性。它返回一个布尔值，该值指示属性是否被成功删除。
...
**参数：** 该方法接受两个参数，如下文所述：
*   `target`：此参数删除属性，是目标对象。
*   `propertyKey`：此参数是要删除的属性的名称。

**返回值：** 该方法返回一个布尔值，该值指示属性是否被成功删除。

**异常：** 当 `target` 不是构造函数时，类型错误是作为结果给出的异常。
