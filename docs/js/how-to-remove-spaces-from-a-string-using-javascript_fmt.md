...
> 原文: [https://www.geeksforgeeks.org/how-to-remove-spaces-from-a-string-using-javascript/](https://www.geeksforgeeks.org/how-to-remove-spaces-from-a-string-using-javascript/)

## 方法 1: 使用 `split()` 和 `join()` 方法

使用 `split()` 方法将一个字符串拆分为多个子字符串，并以数组的形式返回。可以将分隔符指定为参数，以便在字符串中找到该分隔符时拆分字符串。在此参数中指定了空格字符（`" "`），以便在出现空格时分隔字符串。
...
使用 `replace()` 方法将指定的字符串替换为另一个字符串。它需要两个参数，第一个是要替换的字符串，第二个参数是替换为的字符串。第二个字符串可以作为空字符串给出，以便空的空间被替换。
