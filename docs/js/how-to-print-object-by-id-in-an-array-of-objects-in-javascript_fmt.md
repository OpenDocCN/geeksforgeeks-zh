...
> 原文: [https://www.geeksforgeeks.org/how-to-print-object-by-id-in-an-array-of-objects-in-javascript/](https://www.geeksforgeeks.org/how-to-print-object-by-id-in-an-array-of-objects-in-javascript/)
...
*   使用 [`Array.filter()`](https://www.geeksforgeeks.org/javascript-array-filter-method/)
*   使用 [`Array.find()`](https://www.geeksforgeeks.org/javascript-array-find-method/)
*   使用 JavaScript 中的 [循环](https://www.geeksforgeeks.org/loops-in-javascript/)

## 使用 `Array.filter()`

`Array.filter()` 方法用于在应用一些条件后，从现有数组创建一个新数组。
...
使用 `Array.find()` 首先我们要搜索给定 `id` 存在于哪个对象中，然后我们从那个对象中提取 `name` 属性。
...
使用 `for` 循环首先，我们迭代数组并搜索给定 `id` 出现在哪个对象中，然后，我们打印我们想要的属性。
