...
新关键字用于创建用户定义的对象类型和构造函数的实例。它用于构造和返回构造函数的对象。
...
*   新关键字设置构造函数的内部原型属性。
*   `new` 关键字将 `this` 变量绑定到新创建的对象。
*   `new` 关键字返回新对象。

## 示例 1

在下面的示例中，我们将创建一个 `message` 函数来打印带有问候语的姓名，并且我们将创建一个 `message` 函数的实例。
...
在下面的示例中，我们将创建一个 `Student` 函数，并使用如下所示的 `new` 关键字创建该函数的实例。
