...
如果该运算符删除了一个属性，则返回 `true`。删除一个不存在的对象属性会返回一个 `true`，但不会影响对象。虽然试图删除一个变量或函数会返回一个 `false`。

## 示例

假设一个名为 `person` 的对象有三个键值对（即 `firstName`、`lastName` 和 `phone`）。现在，使用 `delete` 操作符删除 `phone` 属性将返回 `true`。
...
如上图所示，`delete person.phone` 返回 `true`，打印 `person` 对象表明 `phone` 属性已经不存在了。
...
因为 `delete` 操作符对变量或函数不起作用，所以返回 `false`，实际变量和函数保持不变。
...
输出显示 `delete` 操作符已经删除了属性，但是该值仍然存在于内存中。

## 异常

可以使用 `delete` 操作符删除全局变量。因为全局变量是 `window` 对象的属性，当 `delete` 处理对象时，它会删除变量。
...
不使用 `var`、`let` 或 `const` 关键字将变量设置为全局变量，它将作为对象属性工作。
...
删除 `toDelete` 返回 `true`，删除变量后试图访问该变量会抛出引用错误，因为该变量不再被定义。

## 使用 delete 删除数组值

JavaScript 数组毕竟是对象。所以，`delete` 符即可使用。但是这会导致一个问题，因为从数组中删除元素后，这个操作符会将位置显示为空，并且不会更新数组长度。
...
开发者还有其他的方法，比如将对象属性的值设置为 `null` 或者 `undefined`。但是该属性将仍然存在于对象上，并且一些像 `for in` 循环这样的操作符将仍然显示存在 `null` 或 `undefined` 的属性。

在循环中使用 `delete` 属性会显著降低程序速度。因此，只有在绝对有必要删除对象属性时，才应该使用此方法。