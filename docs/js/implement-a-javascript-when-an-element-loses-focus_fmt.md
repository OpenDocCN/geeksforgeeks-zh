# 当一个元素失去焦点时实现JavaScript功能

> 原文: [https://www.geeksforgeeks.org/implement-a-javascript-when-an-element-loses-focus/](https://www.geeksforgeeks.org/implement-a-javascript-when-an-element-loses-focus/)

给定一个文档，任务是在元素失去焦点时实现功能。我们有两个选项，一个是`onblur`事件，另一个是`onfocusout`事件。我们要讨论几个方法。
...
**示例 1:** 本示例向 `<input>` 元素添加一个`onblur`事件，当它发生时，指定的代码运行。
...
**示例 2:** 本示例向 `<input>` 元素添加一个 `onfocusout` 事件，当它发生时，指定的代码运行。
