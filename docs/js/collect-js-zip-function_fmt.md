...
下面的示例说明了`collect.js`中的`zip()`函数。在本例中，我们获取一个集合，然后使用`zip()`函数，使用`zip()`方法将它们与新集合的相应项目数组进行匹配。
...
`参考:` [https://collect.js.org/api/zip.html](https://collect.js.org/api/zip.html)