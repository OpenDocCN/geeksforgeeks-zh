...
> 原文：[https://www.geeksforgeeks.org/how-to-use-goto-in-javascript/](https://www.geeksforgeeks.org/how-to-use-goto-in-javascript/)
...
除了在 `switch` 语句中使用外，`break` 语句还可以用来提供“开化”形式的 `goto`。通过使用这种形式的中断，您可以中断一个或多个代码块。这些块不需要是某个循环或 `switch` 的一部分，只需要是任何代码块。您还可以精确地指定执行将在哪里继续，因为这种形式的中断与标签一起工作。所以结论是 `break` 和 `continue` 是用来给你 `goto` 的好处而没有它的弊端。
...
number++;
if (number < 100) goto start_position;
```
...
document.write("<br>Learnt something new");
```

**输出：**

```
123456789<br>Learnt something new
```
...
<p id="demo"></p>
<script>
    var cars = ["BMW", "Volvo", "Maruti", "Honda"];
    var text = "";
