...
> 原文: [`https://www.geeksforgeeks.org/how-to-write-a-program-that-takes-a-predicate-and-array-like-array-filter-but-only-keeps-x-if-predx-false-in-javascript/`](https://www.geeksforgeeks.org/how-to-write-a-program-that-takes-a-predicate-and-array-like-array-filter-but-only-keeps-x-if-predx-false-in-javascript/)
...
在本文中，让我们看看谓词在 JavaScript 中与函数一起传递时如何使用[**数组**](https://www.geeksforgeeks.org/array-data-structure/)。
...
以下函数接受一个数组和谓词，并返回满足 `pred(x) === false` 的结果数组。在这个函数中，谓词返回*真*，如果数组元素是偶数，则返回*假*。输出应该是奇数。
...
以下函数通过将参数作为数组和谓词传递给函数，使用谓词计算数组中 8 的频率。
