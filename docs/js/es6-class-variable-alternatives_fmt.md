...
`Class` 关键字允许用户创建一个类。类关键字对于在 JavaScript 中创建类是必不可少的。
...
<script>
    ...
    document.getElementById("gfg").innerHTML = Fru.fruit1;
</script>
</body>
</html>
```
...
每次创建类的实例时。它是使用 `构造函数()` 方法初始化的。如果用户没有定义 `构造函数()` 方法，那么 JavaScript 会自动定义它。
...
<script>
    ...
    document.getElementById("gfg").innerHTML = Fru.fruit1;
    document.getElementById("gfg1").innerHTML = Fru.present();
</script>
</body>
</html>
```
...
这个方法很简单，它们是在类上调用的，而不是在类的对象上调用的。
...
<script>
    ...
    document.getElementById("gfg1").innerHTML = Fruit.new();
</script>
</body>
</html>
```
...
继承允许子类继承父类的属性。关键字 `extends` 用于此目的。
...
<script>
    ...
    document.getElementById("gfg1").innerHTML = fru.show();
</script>
</body>
</html>
```
...
`getter` 允许获取 ES6 类变量的值，`Setters` 允许在适当的字段中分别设置这些值。
...
<script>
    ...
    document.getElementById("gfg1").innerHTML = Fru.fruitss;
</script>
</body>
</html>
```
