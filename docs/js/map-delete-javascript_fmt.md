...
*   `Map` 集合中的键和值可以是任何类型，如果使用集合中已存在的键将值添加到 `Map` 集合中，则新值将替换旧值。
*   `Map` 对象中元素的迭代是按照插入顺序进行的，并且 `for...of` 循环为每次迭代返回一个所有 `[key, value]` 对的数组。

## JavaScript 中对象和 `Map` 的区别

这两种数据结构在很多方面都是相似的，比如两者都使用键存储值，允许使用键检索那些值，删除键并验证键是否保存任何值。然而，在 JavaScript 中，对象和 `Map` 之间有很大的区别，这使得在许多情况下使用 `Map` 成为更好、更可取的选择。

*   `Map` 中使用的键可以是任何类型的值，如函数、对象等，而对象中的键仅限于符号和字符串。
*   使用 `size` 属性可以很容易地知道 `Map` 的大小，但是在处理对象时，必须手动确定大小。
*   在需要频繁添加和移除[键、值]对的情况下，应该首选 `Map`，因为 `Map` 是一种迭代数据类型，可以直接迭代，而迭代对象需要以特定方式获取其键。

## JavaScript 中的 `Map.delete()` 方法

JavaScript 中的 `Map.delete()` 方法用于删除 `Map` 中存在的所有元素中的指定元素。

`Map.delete()` 方法获取需要从映射中移除的键，从而移除与该键关联的元素并返回 `true`。如果键不存在，则返回 `false`。
...
**解释:** 键 `'3'` 出现在 `Map` 中，因此与之相关的元素被移除并返回 `true`。
...
*   如果传递给函数的键在 `Map` 中不存在，则返回 `false`。基本上，它既不会抛出任何异常，也不会有任何错误。

## `Map.clear()` 与 `Map.delete()` 的工作差异

`Map.clear()` 会移除 `Map` 的所有键值对，并将 `Map` 的大小减小到零。而 `Map.delete()` 移除指定的映射值，该值的键作为参数或迭代器在范围内传递以移除对。
