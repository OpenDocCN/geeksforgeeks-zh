...
> 原文: [https://www.geeksforgeeks.org/how-to-get-current-formatted-date-dd-mm-yyyy-in-javascript/](https://www.geeksforgeeks.org/how-to-get-current-formatted-date-dd-mm-yyyy-in-javascript/)
...
此方法返回定义日期的年份（对于公元 1000 年至 9999 年之间的日期为四位数）。
**语法:**
```
Date.getFullYear()
```
...
此方法获取字符串的一部分，并将提取的部分作为新字符串返回。它使用开始和结束参数来定义要提取的字符串部分。第一个字符从位置 0 开始，第二个字符位置为 1，依此类推。
**语法:**
```
...
### `replace()` 方法
此方法在字符串中搜索定义的值或正则表达式，并返回一个替换了定义值的新字符串。
**语法:**
```
...
## 示例 1
本示例通过同时检查日期和月份来设置日期的格式，如果日期和月份不在 2 位数内，则加零使其为 2 位数。
```html
<!DOCTYPE HTML>
<html>
...
## 示例 2
本示例首先从日期对象中分割日期部分，然后以 `dd/mm/yyyy` 格式设置日期。
```html
<!DOCTYPE HTML>
<html>
