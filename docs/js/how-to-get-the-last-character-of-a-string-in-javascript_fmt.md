...
> 原文：[https://www.geeksforgeeks.org/how-to-get-the-last-character-of-a-string-in-javascript/](https://www.geeksforgeeks.org/how-to-get-the-last-character-of-a-string-in-javascript/)
...
首先使用 `length` 属性计算给定字符串中的字符数。因为索引从 0 开始，所以使用 `str.charAt(str.length - 1)` 来获取字符串的最后一个字符。
...
## 方法 2：使用 `str.slice()` 函数

`string.slice()` 函数用于返回给定输入字符串的一部分或切片。
