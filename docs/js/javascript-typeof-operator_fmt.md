...
在这个示例中，我们使用了 `'==='` (严格相等比较运算符)，它比较值和类型，然后返回 `true` 或 `false`。例如——考虑第一个 `console.log()`，js 从左到右开始编译，它首先计算 `25` 的类型，即 `"number"`，然后将其与 `"number"` 进行比较，最后相应地返回 `true` 或 `false`。
