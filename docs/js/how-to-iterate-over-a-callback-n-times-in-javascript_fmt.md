...
> 原文：[https://www.geeksforgeeks.org/how-to-iterate-over-a-callback-n-times-in-javascript/](https://www.geeksforgeeks.org/how-to-iterate-over-a-callback-n-times-in-javascript/)

给定一个回调函数，我们必须迭代回调 n 次。回调是作为参数传递的函数。为了迭代回调函数，我们必须运行回调函数 n 次。

**方法 1：** 我们用递归迭代 n 次回调函数。

*   首先创建以 `n` 为参数的回调函数 `factor`。
*   `factor` 函数生成 n 长度的模式。
*   创建带有回调函数和 `n` 的 `test` 函数。
*   `test` 函数检查 `n` 的值是否等于 0。
*   如果 `n` 为 0，则返回终止 `test` 函数，否则调用打印模式的回调函数。
...
*   首先我们创建回调函数 `factor`，它生成数的阶乘。
*   用参数 `n` 和回调函数创建 `test` 函数。
