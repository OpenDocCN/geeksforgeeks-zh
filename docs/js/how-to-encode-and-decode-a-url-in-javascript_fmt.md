...
> 原文：[https://www.geeksforgeeks.org/how-to-encode-and-decode-a-url-in-javascript/](https://www.geeksforgeeks.org/how-to-encode-and-decode-a-url-in-javascript/)
...
`encodeURI()` 函数用于编码完整的 URI。该函数对除 `, / ? : @ & = + $ #` 外的字符进行编码。
...
**`encodeURIComponent()` 和 `encodeURI()` 对比：**

| 特性 | `encodeURIComponent()` | `encodeURI()` |
| :--- | :--- | :--- |
| **定义** | `encodeURIComponent()` 函数用于编码 URI 的某些部分或组件。 | `encodeURI()` 函数用于编码完整的 URI。 |
| **语法** | `encodeURIComponent(uri_string_component)` | `encodeURI(complete_uri_string)` |
| **特殊字符编码** | 此函数对特殊字符进行编码。此外，它还对以下字符进行编码：`, / ? : @ & = + $ #` | 此函数对 `, / ? : @ & = + $ #` 字符**不**进行编码。 |

### 3. `escape()` 函数

该函数将字符串作为单个参数，对可以通过支持 ASCII 字符的计算机网络传输的字符串进行编码。编码是将纯文本转换为密文的过程。
