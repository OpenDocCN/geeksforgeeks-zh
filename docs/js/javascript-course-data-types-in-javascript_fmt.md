...
> 原文：[https://www.geeksforgeeks.org/javascript-course-data-types-in-javascript/](https://www.geeksforgeeks.org/javascript-course-data-types-in-javascript/)
...
*   **空**：该类型只有一个值：`null`。
*   **未定义**：未赋值的变量为 `undefined`。
*   **对象**：它是最重要的数据类型，构成了现代 JavaScript 的构建模块。我们将在后续文章中详细了解这些数据类型。

## 数字

JavaScript 中的数字类型包含整数和浮点数。除了这些数字之外，我们在 javascript 中还有一些“特殊数字”，它们是：`Infinity`、`-Infinity` 和 `NaN`。`Infinity` 基本上代表数学上的“∞”。`NaN` 表示计算错误。
...
javascript 中的单引号和双引号没有区别。反引号（Backticks）提供了额外的功能，因为在它们的帮助下，我们可以在其中嵌入变量。
...
对象本质上并不原始，理解起来有点复杂。javascript 中的所有东西基本上都是一个对象，这就是为什么很好地理解它们是什么变得非常重要的原因。对象用于存储各种数据和更复杂实体的键控集合。
