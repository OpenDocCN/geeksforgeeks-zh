...
> 原文: [https://www.geeksforgeeks.org/how-to-check-if-a-variable-is-not-null-in-javascript/](https://www.geeksforgeeks.org/how-to-check-if-a-variable-is-not-null-in-javascript/)
...
根据上述条件，如果 `my_var` 为空，那么给定的 `if` 条件将不会执行，因为空在 JavaScript 中是一个 *falsy* 值，但是在 JavaScript 中有许多预定义的 falsy 值，比如
...
因此，如果 `my_var` 等于任何上述预定义的虚假值，那么 `if` 条件将不会执行，反之亦然。
