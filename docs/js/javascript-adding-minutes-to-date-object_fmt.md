...
> 原文: [https://www.geeksforgeeks.org/javascript-adding-minutes-to-date-object/](https://www.geeksforgeeks.org/javascript-adding-minutes-to-date-object/)

给定一个日期，任务是给它增加分钟。为了给 JavaScript 中的日期对象添加分钟，使用了下面列出的一些方法：

## JavaScript `getMinutes()` 方法

此方法返回提供的日期和时间的分钟数（从 0 到 59）。
...
*   `sec`: 可选参数。它指定表示秒的整数。预期值为 0-59，但也允许其他值。
*   `millisec`: 为可选参数。它指定表示毫秒的整数。预期值为 0-999，但也允许其他值。

**注意：** 前面三个参数除了它们的范围外都接受值，这些值的调整方式如下：

*   `min = -1`，表示前一小时的最后一分钟，其他参数相同。
*   如果 `min` 传递到 60，则表示下一小时的第一分钟，其他参数也是如此。
...
此方法通过向 1970 年 1 月 1 日午夜加/减定义的毫秒数来设置日期和时间。
...
**参数：** 包含需要的单参数 `millisec`。它指定要加/减的毫秒数，1970 年 1 月 1 日午夜。

**返回值：** 返回，表示日期对象和 1970 年 1 月 1 日午夜之间的毫秒数。

## 示例 1

本示例通过使用 `setTime()` 和 `getTime()` 方法，将 `today` 变量增加了 4 分钟。
...
本示例通过使用 `setMinutes()` 和 `getMinutes()` 方法，为 `today` 变量增加 6 分钟。
