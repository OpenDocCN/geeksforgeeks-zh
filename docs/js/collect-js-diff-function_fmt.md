...
`diff()`函数将主集合与给定集合进行比较，并返回原始集合中但不在给定集合中的值。在 JavaScript 中，首先将数组转换为集合，然后将函数应用于集合。
...
以下示例说明了`collect.js`中的`diff()`函数。

### 示例 1

在本例中，我们取一个集合，然后使用`diff()`函数并返回不在新集合中的值。
...
有一点需要注意，这个函数保存了一个集合，与主集合进行比较，但只返回主集合中多余的那些项。
...
**参考:** [https://collect.js.org/api/count.html](https://collect.js.org/api/count.html)