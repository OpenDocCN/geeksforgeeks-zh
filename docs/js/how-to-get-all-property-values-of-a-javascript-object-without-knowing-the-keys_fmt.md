...
## 方法 1:使用 `Object.values()` 方法

`Object.values()` 方法用于返回对象自身可枚举属性值的数组。可以使用 `for` 循环来循环数组，以获取对象的所有值。因此，获取所有属性值不需要知道这些键。
...
`Object.keys()` 方法用于返回对象自身可枚举属性名的数组。此数组使用 `forEach()` 方法来访问每个键。每个属性值都可以使用带有对象数组符号的键来访问。因此，获取所有属性值不需要事先知道密钥。
