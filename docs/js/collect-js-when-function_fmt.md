...
如果给定中的第一个参数被证明为真，则使用`when()`函数进行回调。在 JavaScript 中，首先将数组转换为集合，然后将函数应用于集合。
...
下面的例子说明了`collect.js`中的`when()`函数。

### 示例 1:

在这个例子中，我们取一个集合，然后使用`when()`函数修改这个集合。
...
**参考:** [https://collect.js.org/api/when.html](https://collect.js.org/api/when.html)