...
> 原文：[https://www.geeksforgeeks.org/how-to-trim-a-file-extension-from-string-using-javascript/](https://www.geeksforgeeks.org/how-to-trim-a-file-extension-from-string-using-javascript/)
...
This method adds the elements of an array into a string, and returns the string. The elements will be separated by a passed separator. The default separator is comma (, ).
...
本示例使用 `split()`、`slice()`和 `join()`方法获取文件名。
...
本示例通过使用**正则表达式**和 `replace()`方法来获取文件名。
