...
> 原文: [https://www.geeksforgeeks.org/javascript-multiline-property/](https://www.geeksforgeeks.org/javascript-multiline-property/)

JavaScript 中的 `multiline` 属性用于指定是否设置了 `"m"` 修饰符。如果设置了 `"m"` 修饰符，则该属性返回 `true`，否则返回 `false`。
...
如果设置了 `m` 修饰符，则返回真，否则返回假。

## 示例-1

本示例检查正则表达式是否包含 `m` 修饰符。
