# JavaScript 中`数组拼接()`方法的替代

> 原文：`https://www.geeksforgeeks.org/alternative-of-array-splice-method-in-javascript/`

`数组拼接()`方法是 JavaScript 的一种方法，在本文中，我们将讨论这种方法的替代方法。下面讨论两个例子。

`方法 1`：在这种方法中，`startIndex`（从哪里开始移除元素）和 `count`（要移除的元素数量）是变量。如果 `count` 未通过，则将其视为 1。运行 `while` 循环，直到 `count` 大于 0，并开始移除所需的元素并将其推入新数组。循环结束后返回这个新数组。
...
`方法 2`：在该方法中，`切片()`方法用于获取移除的元素，该方法也用于获取输出数组。这种方法还需要参数将新元素插入到数组中。
