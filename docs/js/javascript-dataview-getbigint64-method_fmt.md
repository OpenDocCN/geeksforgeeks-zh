# DataView.getBigInt64() 方法

> 原文: [https://www.geeksforgeeks.org/javascript-dataview-getbigint64-method/](https://www.geeksforgeeks.org/javascript-dataview-getbigint64-method/)
...
`getBigInt64()` 方法用于从数据视图开始的特定字节偏移量处获取有符号的 64 位整数（长整型）。
...
*   `byteOffset`: 此参数指定从视图开始读取数据的偏移量，以字节为单位。
...
如果字节偏移量超出了视图的结尾，则抛出 `RangeError`。

## 例 1:

在本例中，传递的偏移量为 0，因此打印的值为 0，因为没有设置任何内容。
...
在本例中，传递的偏移量为 3，因此打印的值为 10000，因为它是之前设置的。
