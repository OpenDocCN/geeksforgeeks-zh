...
> 原文：[https://www.geeksforgeeks.org/javascript-date-toutcstring-method/](https://www.geeksforgeeks.org/javascript-date-toutcstring-method/)

下面是 `toUTCString()` 方法的例子。
...
此方法不接受任何参数。它只是与使用 `Date()` 构造函数创建的 `Date` 对象一起使用，该构造函数的内容被转换为字符串。

## 返回值

根据世界时区 UTC 返回转换后的字符串。

## 注意

`dateObj` 是一个使用 `Date()` 构造函数创建的有效日期对象，其内容被转换为字符串。

上述方法的更多代码如下：

### 程序 1

这里在创建 `date` 对象的时候没有作为参数传递任何东西，但是 `toUTCString()` 方法根据世界时区 UTC 返回当前的日名称、月名称、日期、年份和时间。
...
当一些随机的值列表被传递时，`toUTCString()` 方法返回相应的生成字符串。
...
以下列出了 JavaScript `Date.toUTCString()` 方法支持的浏览器：

*   `Google Chrome` 1 及更高（1997 年）
*   `Edge` 12 及以上
*   `Firefox` 1 及更高（1997 年）
*   `Internet Explorer` 4 及以上
*   `Opera` 4 及以上
*   `Safari` 1