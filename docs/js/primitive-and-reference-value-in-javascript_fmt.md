...
JavaScript 提供了数字、字符串、布尔值、未定义、符号、BigInt 六种类型的原始值。 原始值的大小是固定，因此 JavaScript 将原始值存储在`调用堆栈(执行上下文)`中。
...
JavaScript 提供了 `Array`、`Object`、`Function` 三种类型的参考值。 参考值的大小是动态的，因此它存储在`堆`上。
