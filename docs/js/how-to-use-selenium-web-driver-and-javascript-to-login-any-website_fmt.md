...
> 原文: [https://www.geeksforgeeks.org/how-to-use-selenium-web-driver-and-javascript-to-login-any-website/](https://www.geeksforgeeks.org/how-to-use-selenium-web-driver-and-javascript-to-login-any-website/)
...
**步骤 4:** 为浏览器安装驱动程序，这些驱动程序将用于自动化浏览器。本文中使用的浏览器是 `Chrome`，因此需要安装 `chromedriver` 包。这可以使用下面的命令来完成。

```
npm install chromedriver
```

**步骤 5:** 创建一个 `JSON` 文件(如 `credentials.json`)来存储用户名/电子邮件和密码。以下格式可用于保存凭据。
...
**步骤 7:** 使用下面的命令启动包含脚本的 `JavaScript` 文件。
