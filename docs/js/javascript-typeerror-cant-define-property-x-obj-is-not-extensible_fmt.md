...
这个 JavaScript 异常`Cannot define property "x": "obj" is not extensible`发生在`Object.preventExtensions()`用在一个对象上使其不再可扩展的时候，所以现在不能给对象添加新的属性。
...
在对象上应用`Object.preventExtensions()`方法后，会向对象添加新属性，这是不允许的。

## 示例 1

在本示例中，在应用了`Object.preventExtensions()`方法后添加了新属性，因此出现了错误。
...
在本示例中，在应用了`Object.preventExtensions()`方法之后，正在使用`defineProperty()`方法添加新属性，因此出现了错误。
