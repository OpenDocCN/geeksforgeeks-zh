...
> 原文：[https://www.geeksforgeeks.org/how-to-flatten-a-given-array-up-to-the-specified-depth-in-javascript/](https://www.geeksforgeeks.org/how-to-flatten-a-given-array-up-to-the-specified-depth-in-javascript/)

在本文中，我们将学习如何在 JavaScript 中将给定的数组展平到指定的深度。

JavaScript 中的 `flat()` 方法用于将数组展平到所需的深度。它创建一个新的数组，并将原始数组的子数组递归连接到给定的深度。此方法接受的唯一参数是可选的 `depth` 参数（默认为 1）。此方法也可用于从数组中移除空元素。
