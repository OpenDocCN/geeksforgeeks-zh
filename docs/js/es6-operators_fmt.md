...
众所周知，这些是 JavaScript ES6 中可用的基本数学运算符。
...
| 操作符 | 功能 |
| :--- | :--- |
| 与(`&&`) | 如果与 `&&` 组合的所有关系语句都为真，则返回 `true`，否则返回 `false`。 |
| 或(`||`) | 如果与 `||` 组合的关系语句中至少有一个为 `true`，则返回 `true`，否则返回 `false`。 |
| 非(`!`) | 返回关系语句结果的倒数。 |
...
按位运算符是一种用于对位模式或二进制数字执行按位运算的运算符，涉及对单个位的操作。

| 操作符 | 功能 |
| :--- | :--- |
| 按位与(`&`) | 将第一个操作数的每个位与第二个操作数的相应位进行比较。如果两个位都为 1，则相应的结果位设置为 1，否则为 0。 |
| 按位或(`|`) | 将第一个操作数的每个位与第二个操作数的相应位进行比较。如果两个位都为 1，则相应的结果位设置为 1，否则为 0。 |
| 按位异或(`^`) | 反转其对应操作数的位。 |
| 左移(`<<`) | 将把“n”位数移到左侧，值为 0 的 n 位将在右侧填充。例: `x=2`，`t=4`，`x << t` 为了便于评估，它执行 `x*(2^n)=2*(2^4)`。 |
| 右移(`>>`) | 将把“n”位数移到右侧，值为 0 的“n”位将填充在左侧。示例: `x=2`，`t=4`，`x>>t`，为了便于评估，它执行 `x/(2^n)=2/(2^4)`。 |
...
赋值运算符是用于为变量、属性、事件或索引器元素赋值的运算符。

| 操作符 | 功能 |
| :--- | :--- |
| 简单赋值(`=`) | 将右操作数的值赋给左操作数。 |
| 加且赋值(`+=`) | 它将右操作数与左操作数相加，并将结果赋给左操作数。 |
| 减且赋值(`-=`) | 它从左操作数中减去右操作数，并将结果赋给左操作数。 |
| 乘且赋值(`*=`) | 它将右操作数与左操作数相乘，并将结果赋给左操作数。 |
| 除且赋值(`/=`) | 它将左操作数除以右操作数，并将结果赋给左操作数。 |
...
这些运算符在不同类型的场合使用时会做不同的运算。

| 操作符 | 功能 |
| :--- | :--- |
| 否定运算符(`-`) | 更改值的符号。 |
| 连接运算符(`+`) | 当应用于字符串时，它会追加字符串。 |
| 条件运算符(`?`) | 它可以用作三元运算符。 |
