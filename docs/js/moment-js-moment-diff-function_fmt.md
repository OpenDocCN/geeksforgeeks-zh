...
> 原文: [https://www.geeksforgeeks.org/moment-js-moment-diff-function/](https://www.geeksforgeeks.org/moment-js-moment-diff-function/)

`moment().diff()` 函数用于获取给定日期的差异（以毫秒为单位），这些日期作为参数传递给该函数。
...
这个函数有两个参数，第一个是 `Moment | String | Number | Date | Array` 类型的日期，第二个参数是 `boolean` 类型的可选参数，用于获取浮点数作为结果，而不是整数。
