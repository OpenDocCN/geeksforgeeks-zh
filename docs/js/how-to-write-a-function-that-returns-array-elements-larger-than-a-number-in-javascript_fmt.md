...
> 原文：[https://www.geeksforgeeks.org/how-to-write-a-function-that-returns-array-elements-larger-than-a-number-in-javascript/](https://www.geeksforgeeks.org/how-to-write-a-function-that-returns-array-elements-larger-than-a-number-in-javascript/)
...
在这种方法中，我们使用 `Array.filter` 方法。在每次迭代中，我们检查该值是否大于 `num`。
...
在这种方法中，我们使用 `map` 方法迭代数组，并且在每次迭代时，我们检查当前值是否大于 `num`？如果为真，那么我们就按原样返回值。如果为假，则用空字符串（或任何假值）替换该项。之后使用 `filter` 方法去除虚假值。
