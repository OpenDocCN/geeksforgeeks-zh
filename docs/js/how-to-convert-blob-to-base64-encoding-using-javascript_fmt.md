...
`Blob` 是 JavaScript 中的一种基本数据类型。`Blob` 代表**二进制大对象**，它是字节数据的表示。网络浏览器支持 `Blob` 数据类型来保存数据。`Blob` 是 `File` 对象和 `FileReader` 应用编程接口的底层数据结构。`Blob` 和普通文件一样有特定的大小和文件类型，可以从系统内存中存储和检索。`Blob` 也可以作为**缓冲区**进行转换和读取。缓冲区非常便于存储二进制数据，例如图像或文件的二进制数据。我们将使用 `FileReader` 应用编程接口将 `Blob` 转换为 JavaScript 中的 `Base64` 编码字符串。
...
使用 `FileReader` 应用编程接口将 `Blob` 转换为 `Base64` 编码字符串。`FileReader.readAsDataURL()` 读取指定 `Blob` 数据类型的内容，并将返回一个带有 `data:` 属性的 `Base64` 编码字符串。当数据读取成功完成或遇到错误时，触发 `FileReader.onloadend` 事件。我们已经使用 `Blob()` 构造函数创建了一个示例 `Blob`。构造函数接受 `string[]` 和由字符串 `type` 组成的对象中的值。
