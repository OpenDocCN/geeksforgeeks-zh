...
> 原文: [https://www.geeksforgeeks.org/javascript-regexp-r-metacharacter/](https://www.geeksforgeeks.org/javascript-regexp-r-metacharacter/)

JavaScript 中的 `RegExp \r` 元字符用于查找回车符（回车表示返回当前行的开头，不向下前进）。如果找到，它返回位置，否则返回 `-1`。
