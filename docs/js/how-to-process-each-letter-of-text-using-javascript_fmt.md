...
> 原文：[https://www.geeksforgeeks.org/how-to-process-each-letter-of-text-using-javascript/](https://www.geeksforgeeks.org/how-to-process-each-letter-of-text-using-javascript/)
...
*   **参数：** 该方法接受单参数 `string`，这是必需的。它指定要搜索的字符串。
...
*   **参数：** 该方法接受单参数 `index`，这是必须的。它指定表示要返回的字符索引的整数。
    **返回值：** 返回一个字符串，表示传递的索引处的字符，如果索引号无效，则返回一个空字符串。

**示例 1：** 本示例使用 `for` 循环逐字母处理字符串，并将每个字母分别转换为大写。
...
**示例 2：** 本示例使用 `for...in` 循环处理字符串字母，并分别提醒每个字母。
...
*   **点击按钮后：**
    ...
    ![](img/7e82df3aaebc3281f6f641aaf6ba4e6e.png)
    ![](img/1d986974731fe15a2051a0366c7d299a.png)
    ![](img/ade6aa94fe6027c346a3318a1d5cb04b.png)