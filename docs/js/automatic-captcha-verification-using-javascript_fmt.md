...
> 原文：[`https://www.geeksforgeeks.org/automatic-captcha-verification-using-javascript/`](https://www.geeksforgeeks.org/automatic-captcha-verification-using-javascript/)

验证码是一个用来通过僵尸工具保护网站的程序。它产生一些测试，人类可以阅读并通过，但目前的计算机做不到。例如，人类可以阅读扭曲的文本，但计算机不能阅读。验证码是完全自动化的公共图灵测试的缩写，用来区分计算机和人类。
