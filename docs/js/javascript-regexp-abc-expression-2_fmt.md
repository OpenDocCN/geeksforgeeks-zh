...
> 原文: `https://www.geeksforgeeks.org/javascript-regexp-abc-expression-2/`

JavaScript 中的 `RegExp [^abc]` 表达式用于搜索不在括号内的任何字符。括号内的字符可以是单个字符或一系列字符。
...
## 示例 1:
本示例搜索不在 `A-G` 之间的字符，即整个字符串中大写的 A 到大写的 G。
...
## 示例 2:
本示例搜索不在 `a-g` 之间的字符，即整个字符串中的小写 a 到小写 g。
