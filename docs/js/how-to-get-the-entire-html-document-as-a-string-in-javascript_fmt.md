...
> 原文：[https://www.geeksforgeeks.org/how-to-get-the-entire-html-document-as-a-string-in-javascript/](https://www.geeksforgeeks.org/how-to-get-the-entire-html-document-as-a-string-in-javascript/)

给定一个 HTML 文档，任务是使用 JavaScript 将整个文档作为字符串获取。这里讨论几个方法：

## `getElementsByTagName()` 方法

这个方法返回一组文档中所有元素的定义标签名，作为 `NodeList` 对象。该对象表示节点集合，通过索引号进行访问。索引从 0 开始。
...
本示例使用 `document.documentElement.innerHTML` 将整个文档作为字符串获取。
...
本示例通过首先选择标记名为 `'html'` 的元素，然后使用 `document.getElementsByTagName('html')[0]` 索引选择第一个元素来获取整个文档的 `innerHTML`。
