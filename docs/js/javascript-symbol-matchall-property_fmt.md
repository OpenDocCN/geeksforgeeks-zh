...
> 原文：`https://www.geeksforgeeks.org/javascript-symbol-matchall-property/`
...
`Symbol.matchAll` 属性返回与字符串匹配的正则表达式。`String.prototype.matchAll()` 方法调用此函数。属性的语法如下：
...
在本例中，`Symbol.matchAll` 属性返回一个迭代器，该迭代器返回与存储在 `result` 中的字符串 `"abcd"` 匹配的正则表达式 `/a/`。因此，匹配的元素是 `["a"]`。
...
在本例中，正则表达式的匹配元素是 `06`、`03` 和 `2021`。正则表达式 `[0-9]` 显示匹配的元素必须包含 0 到 9。`g` 表示进行全局搜索的全局标志。
