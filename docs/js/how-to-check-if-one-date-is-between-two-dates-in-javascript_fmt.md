...
> 原文：[https://www.geeksforgeeks.org/how-to-check-if-one-date-is-between-two-dates-in-javascript/](https://www.geeksforgeeks.org/how-to-check-if-one-date-is-between-two-dates-in-javascript/)

任务是确定给定的日期是否在给定的两个日期之间？下面是在 JavaScript 帮助下讨论的一些最常用的技术。

## 方法 1：使用 `.split()` 方法

在第一种方法中，我们将使用 `.split()` 方法和 `new Date()` 构造器。我们将使用 `.split()` 方法对“/”上的日期进行拆分，得到数组中的日、月、年。我们必须从上一步获得的数组中构造日期，因为我们将使用 `new Date()` 构造函数。因为这个方法返回 1970 年 1 月 1 日的秒数，所以比较日期变得很容易。
...
在第二种方法中，我们将使用 `.getTime()` 方法和 `new Date()` 构造函数。这里首先使用 `new Date()` 构造函数并传递其中的字符串，该字符串构成一个 Date 对象。`.getTime()` 方法返回的是 1970 年 1 月 1 日的秒数，可以很容易的比较秒数。
