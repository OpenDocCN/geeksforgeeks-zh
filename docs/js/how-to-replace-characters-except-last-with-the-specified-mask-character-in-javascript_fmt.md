...
> 原文: [https://www.geeksforgeeks.org/how-to-replace-characters-except-last-with-the-specified-mask-character-in-javascript/](https://www.geeksforgeeks.org/how-to-replace-characters-except-last-with-the-specified-mask-character-in-javascript/)
...
我们将制作一个可以屏蔽字符串字符的函数，除了最后一个元素，我们将把我们的数字作为字符串给出。

*   在这个函数中，我们使用三个参数，第一个参数是我们想要屏蔽的字符串，第二个参数将指定屏蔽字符，第三个参数是您想要跳过的字符数。
*   我们使用一些内置的 JavaScript 函数 `slice()` 来切片我们给定的字符串。这里我们写 `slice(0, -n)`，其中 `-n` 将跳过字符串的最后 `n` 个字符。
*   然后我们使用 `replace()` 功能，用指定的掩码替换字符。替换我们使用的所有字符 `RegExp(/./g)`，其中 `.` 将匹配任何字符，`g` 为全局标志。
*   现在我们必须将剩余的字符添加到掩码字符串中，因此我们再次使用 `slice()` 功能来获取 *剩余的 n 个字符*，并将其添加到掩码字符串中。
