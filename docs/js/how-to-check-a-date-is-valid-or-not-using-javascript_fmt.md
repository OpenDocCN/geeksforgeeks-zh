...
> 原文：[https://www.geeksforgeeks.org/how-to-check-a-date-is-valid-or-not-using-javascript/](https://www.geeksforgeeks.org/how-to-check-a-date-is-valid-or-not-using-javascript/)
...
*   将日期对象存储在变量中。
*   如果日期有效，那么 `getTime()` 将始终等于它自己。
*   如果日期无效，那么 `getTime()` 将返回不等于自身的 `NaN`。
*   `isValid()` 函数用于检查 `getTime()` 方法是否等于自身。
...
*   如果日期有效，那么 `getTime()` 方法将始终等于其本身。
*   如果日期无效，那么 `getTime()` 方法将返回不等于自身的 `NaN`。
*   在本例中，`isValid()` 方法正在检查 `getTime()` 是否等于自身。
