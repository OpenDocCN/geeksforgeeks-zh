...
> 原文：`https://www.geeksforgeeks.org/javascript-math-log2e-property/`

下面是 `Math.LOG2E` 属性的例子。
...
`Math.LOG2E` 是 JavaScript 中的一个属性，简单来说就是用来求 `e` 的基 2 对数的值，其中 `e` 是一个约等于 2.718 的无理数和超越数。
...
它只是返回 `e` 的以 2 为底的对数的值。

下面的例子说明了 JavaScript 中的 `Math.LOG2E` 属性。

### 示例：

这里简单地将 `e` 的以 2 为底的对数的值显示为输出。
...
这里我们考虑 `Math.LOG2E` 作为一个函数，但实际上它是一个属性，这就是为什么错误作为输出被显示。
...
每当我们需要求 `e` 的以 2 为底的对数的值时，那时候我们就借助这个性质。在数学中，它需要很多。
