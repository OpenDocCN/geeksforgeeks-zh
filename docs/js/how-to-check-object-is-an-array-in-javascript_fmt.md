...
> 原文: [https://www.geeksforgeeks.org/how-to-check-object-is-an-array-in-javascript/](https://www.geeksforgeeks.org/how-to-check-object-is-an-array-in-javascript/)

## 方法 1: 使用 `Array.isArray()` 函数

`Array.isArray()` 函数确定传递给该函数的值是否为数组。如果传递的参数是数组，则该函数返回 `true`，否则返回 `false`。
...
本示例使用 `Array.isArray()` 函数检查对象是否为数组。
...
在 JavaScript 中，`typeof` 运算符以字符串的形式返回其操作数的数据类型，其中操作数可以是任何对象、函数或变量。然而，这种方法的问题是它不适用于确定数组。
