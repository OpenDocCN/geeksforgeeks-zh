...
> 原文：[https://www.geeksforgeeks.org/check-whether-html-element-has-scrollbars-using-javascript/](https://www.geeksforgeeks.org/check-whether-html-element-has-scrollbars-using-javascript/)
...
1.  选择特定元素。
2.  获取元素的 `scrollWidth` 和 `clientWidth` 属性来检查水平滚动条。
3.  计算 `scrollWidth > clientWidth`。
4.  如果该值为真，则水平滚动条存在，否则不存在。
5.  对垂直滚动条执行同样的过程。
...
3.  如果这些值大于 0，则滚动条存在。
4.  如果这些是 0，那么首先将它们设置为 1，然后再次测试以了解是否得到 1 的结果。
5.  最后，将它们设置回 0。
