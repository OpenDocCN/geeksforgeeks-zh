...
> 原文: [https://www.geeksforgeeks.org/javascript-date-setutcmilliseconds-method/](https://www.geeksforgeeks.org/javascript-date-setutcmilliseconds-method/)

下面是`Date.setUTCMilliseconds()`方法的例子。
...
`Date.setUTCMilliseconds()`方法用于将根据世界时的毫秒设置为使用`Date()`构造函数创建的日期对象。
...
*   **milliseconds_Value:** 此参数保存毫秒的值，用于在`Date()`构造函数中设置。

## 返回值

返回新的，即更新的毫秒数，该毫秒数由`setUTCMilliseconds()`方法设置。

## 注意

`DateObj`是使用`Date()`构造函数创建的有效日期对象，我们希望在该构造函数中根据世界时设置毫秒。毫秒的值从 0 到 999。

上述方法的更多代码如下:

## 程序 1

如果在`Date()`构造函数中，我们在创建`Date`对象时没有给出毫秒，`setUTCMilliseconds()`方法仍然能够设置新的毫秒，该毫秒在创建的`Date`对象中作为参数给出。
...
如果`Date()`构造函数中没有给出参数，仍然可以设置毫秒，但是根据世界时，月份、年份、日期等仍然是当前的。
...
如果给定毫秒 1006 的值作为`setUTCMilliseconds()`方法的参数，它将设置 6 为毫秒，因为毫秒范围是从 0 到 999，因此 1006 - 1000 = 6，这里减去 1000，因为 0 到 999 是 1000。
...
`JavaScript Date.setUTCMilliseconds()`方法支持的浏览器如下:
