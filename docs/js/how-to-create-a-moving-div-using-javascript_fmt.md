...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-moving-div-using-javascript/](https://www.geeksforgeeks.org/how-to-create-a-moving-div-using-javascript/)
...
1.  我们必须创建一个 HTML `div`，并使用一个类 `ball` 给 `div` 添加一些 CSS。
2.  在 CSS 中，我们给身体添加一些 [`背景色`](https://www.geeksforgeeks.org/css-background-color-property/)，给 `div` 一些高度、宽度和颜色。
3.  现在我们将使用 JavaScript 在 `div` 中添加 [`左边距`](https://www.geeksforgeeks.org/css-margin-left-property/)。所以它会从左向右移动。
4.  在 JavaScript 中，我们使用 id 名称获取 `div`。经过一段时间后，我们将把 `左边距` 添加到 `div` 中。
