...
`arr.flat()` 方法在 ES2019 中引入。它用于展平一个数组，以减少一个数组的嵌套。

`flat()` 方法在 JavaScript 的函数式编程范式中大量使用。在 JavaScript 中引入 `flat()` 方法之前，主要使用了下划线.js 等各种库。
...
返回一个数组，即深度比原数组平坦，它根据深度去除嵌套。

上述功能的更多代码定义如下:

## 程序 1:

下面的代码片段展示了 `flat()` 方法是如何工作的。
