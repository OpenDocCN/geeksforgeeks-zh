...
> 原文：[https://www.geeksforgeeks.org/how-to-concatenate-regex-literals-in-javascript/](https://www.geeksforgeeks.org/how-to-concatenate-regex-literals-in-javascript/)
...
编程世界中 `Regex` 的串联可以理解为组合文本模式得到一个新的文本模式，比如“HelloWorld”就是 `/HelloWorld/`。每当调用 `RegExp()` 时，它都会创建一个新的 `RegExp` 对象。

## 示例 1

本示例创建一个表达式，但实际上没有使用 `Regex` 文字语法。这允许您在字符串成为 `Regex` 对象之前进行任意的字符串操作。
...
如果您有两个 `Regex` 文字，您可以使用一种技术将它们连接起来，该技术删除重复项，但保持唯一值的顺序，将两个 `Regex` 文字连接起来。
