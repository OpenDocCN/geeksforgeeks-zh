...
> 原文：[https://www.geeksforgeeks.org/disabling-sessions-in-passport-js/](https://www.geeksforgeeks.org/disabling-sessions-in-passport-js/)

在常见的 web 应用程序中，用于验证用户身份的凭据将仅在登录请求期间传输。Passport 将在身份验证成功后建立持久的登录会话。该会话通过用户浏览器中的 cookie 来维护。

但是，在某些情况下，不需要会话支持。例如，应用编程接口服务器为每个要验证的请求提供所需的凭据。在这种情况下，您可以禁用会话支持。您需要将 `session` 选项设置为 `false`。
...
*   我们可以使用 `uuid` 模块创建一个唯一的用户 id。

    ```bash
    npm i uuid
    ```

*   我们存储用户密码的散列，而不是直接存储用户输入的密码。我们可以使用 `bcrypt` 模块生成密码的散列。

    ```bash
    npm i bcrypt
    ```

*   我们需要安装 `passport` 模块来使用它的功能。

    ```bash
    npm i passport
    ```

*   护照提供了很多策略，这里我们将使用 `passport-local` 策略。
