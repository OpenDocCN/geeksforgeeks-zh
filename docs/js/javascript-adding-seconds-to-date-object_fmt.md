...
> 原文: [https://www.geeksforgeeks.org/javascript-adding-seconds-to-date-object/](https://www.geeksforgeeks.org/javascript-adding-seconds-to-date-object/)

给定一个日期，任务是给它增加秒。为了在 JavaScript 中添加秒到日期，我们将讨论一些技术。首先要知道的几个方法。

## JavaScript `getSeconds()`方法

此方法返回提供的日期和时间的秒数（从 0 到 59）。
...
本示例通过使用 `setTime()` 和 `getTime()` 方法，将 `today` 变量增加 100 秒。
...
本示例通过使用 `setSeconds()` 和 `getSeconds()` 方法，将 `today` 变量增加 10 秒。
