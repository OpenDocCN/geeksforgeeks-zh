...
> 原文：[https://www.geeksforgeeks.org/how-to-clone-a-given-regular-expression-in-javascript/](https://www.geeksforgeeks.org/how-to-clone-a-given-regular-expression-in-javascript/)
...
这里，`regExp`是要克隆的表达式，`flags`决定了克隆的标志。主要使用三种标志。

*   `g`：全局标志。使用此标志，搜索将查找全局匹配。
*   `i`：使用此标志时，搜索不区分大小写。
*   `m`：使用此标志，我们执行多行匹配。
