...
> 原文: [https://www.geeksforgeeks.org/javascript-course-loops-in-javascript/](https://www.geeksforgeeks.org/javascript-course-loops-in-javascript/)
...
**注:** Javascript 还包括 `for..in`、`for..of`、`for await...of`，虽然它们超出了本课程的范围。
...
上面的代码只是将“Hello world”打印到屏幕上，我们所做的就是初始化一个值为“0”的变量 `i`，然后我们说，直到这个变量 `i` 的值小于“5”为止，继续执行 while 循环的块内所写的任何操作。增加变量 `i` 的值很重要，否则它将进入“无限”循环。现在考虑这样一个场景，我们在圆括号内传递一个值不为真的东西，在这种情况下，代码块内的任何内容都不会被执行。
