...
> 原文:[https://www.geeksforgeeks.org/scroll-to-the-top-of-the-page-using-javascript-jquery/](https://www.geeksforgeeks.org/scroll-to-the-top-of-the-page-using-javascript-jquery/)

可滚动页面可以使用两种方法滚动到顶部:

## 方法一: 使用 `window.scrollTo()`

`window.scrollTo()` 方法可以滚动到页面上的指定位置。它接受两个参数，即要滚动到的页面的 x 和 y 坐标。将两个参数都作为 0 传递会将页面滚动到最顶端和最左端。
