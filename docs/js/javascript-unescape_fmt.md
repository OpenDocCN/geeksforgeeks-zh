...
## 先决条件
[JavaScript `escape()`函数](https://www.geeksforgeeks.org/javascript-escape-function/)
...
JavaScript 中的 `unescape()`函数将一个字符串作为参数，并用于解码由 `escape()`函数编码的字符串。当通过 `unescape()`解码时，字符串中的十六进制序列被它们所代表的字符替换。
