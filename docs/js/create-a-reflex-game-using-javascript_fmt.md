...
> 原文：[https://www.geeksforgeeks.org/create-a-reflex-game-using-javascript/](https://www.geeksforgeeks.org/create-a-reflex-game-using-javascript/)
...
下一个函数将是 `remark()` 函数。该功能将包括游戏结束时游戏将显示的备注。
...
按下停止按钮将执行下一个 `stopTest()` 功能。按下停止按钮可以执行 3 个选项：
第一，成功完成，告知响应时间。
