...
> 原文：[https://www.geeksforgeeks.org/how-to-hide-span-element-if-anchor-href-attribute-is-empty-using-javascript/](https://www.geeksforgeeks.org/how-to-hide-span-element-if-anchor-href-attribute-is-empty-using-javascript/)

当 `<a>` 标记的 `href` 属性不存在或无效时，任务是隐藏 `<span>` 标记。

让我们考虑一下我们有一个 `<a>` 标签里面有一个 `<span>` 标签。它看起来类似于这样：
...
现在，当 `<a>` 的 `href` 属性为空或无效时，我们必须隐藏 `<span>` 标记。
