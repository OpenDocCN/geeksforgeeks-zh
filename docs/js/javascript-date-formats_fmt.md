...
### 仅年份 (`Date("2019-03-06t 12:00:00Z")`)
...
JavaScript 短日期采用 `MM/DD/YYYY` 格式编写。
...
JavaScript 长日期采用 `MMM DD YYYY` 格式编写。
...
用于以英文返回给定日期对象的时间部分。日期对象是通过使用 `Date()` 构造函数创建的。此方法将中日期的时间部分转换为字符串。
...
`Date.parse()` 函数用于帮助计算从 1970 年 1 月 1 日午夜到我们提供的日期之间的确切毫秒数。它以毫秒为单位转换给它的有效日期字符串。
