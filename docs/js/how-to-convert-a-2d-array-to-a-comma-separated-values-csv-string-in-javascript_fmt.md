...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-a-2d-array-to-a-comma-separated-values-csv-string-in-javascript/](https://www.geeksforgeeks.org/how-to-convert-a-2d-array-to-a-comma-separated-values-csv-string-in-javascript/)
...
我们将使用 `map()` 函数和 `join()` 函数将每个 1D 行组合成一个字符串，中间用逗号分隔。然后使用 `join()` 函数用 `"\n"` 连接所有单个字符串。
