...
> 原文: [https://www.geeksforgeeks.org/explain-basic-code-snippet-of-jsx/](https://www.geeksforgeeks.org/explain-basic-code-snippet-of-jsx/)
...
在语法方面，react 中不强制写 JSX 代码。但是内置文件结构由编写简单 JSX 代码的组件组成。开发人员发现它很有用，因为 JSX 代码在 JavaScript 项目中使用用户界面时会显示更多有用的错误和警告消息。与使用 `React.createElement()` 和 `React.appendChild()` 方法创建的模板相比，JSX 代码看起来更简洁，更容易调试。

现在我们将在 react 应用程序中创建一个 JSX 组件，请看组件的详细解释。

## JSX 代码片段的解释

现在我们要学习 JSX 代码片段的基本属性。
...
*   **`className` 属性替换 `class`:** HTML 的常规 `class` 属性类似于 JavaScript 的 `class` 关键字。因此为了避免任何冲突，HTML 的 `class` 属性被重命名为 `className`。`className` 属性取代了 `class` 属性，并执行与 `class` 属性相同的工作。
...
现在我们要在 `App.js` 文件的 `App` 组件中创建一个 JSX 元素。下面讨论这些步骤。

*   清除 `App.js` 文件的 `App` 组件中的所有内容。
*   在 `App` 组件内创建一个名为 `jsxElement` 的新变量。
...
`App.css` 文件在顶部导入。反应项目结构是默认的 `App.css` 文件。创建 JSX 元素后，我们对其进行了修改，并在其中添加了我们自己的风格。
