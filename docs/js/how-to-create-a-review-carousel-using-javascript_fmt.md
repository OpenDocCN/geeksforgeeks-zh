...
> 原文: [https://www.geeksforgeeks.org/how-to-create-a-review-carousel-using-javascript/](https://www.geeksforgeeks.org/how-to-create-a-review-carousel-using-javascript/)

在本文中，我们使用 [JavaScript](https://www.geeksforgeeks.org/javascript-tutorial/) 创建了一个评审转盘。我们还使用了基本的 [HTML](https://www.geeksforgeeks.org/html-tutorials/) 和 [CSS](https://www.geeksforgeeks.org/css-tutorials/) 进行造型。轮播基本上是一种幻灯片放映，用于以循环方式显示图像、文本或两者。评论转盘用于显示评论。

**进场:**

*   在主体标签中，使用包含审阅者图像、名称和文本的类名创建嵌套的 [`div`](https://www.geeksforgeeks.org/div-tag-html/) 标签。
*   添加上一个和下一个按钮，分别检查上一个和下一个评论。
*   对于样式，在 [`style`](https://www.geeksforgeeks.org/html-style-tag/) 标签中添加一些 CSS 属性，比如对齐方式、字体大小、填充、边距等。
*   在 [`script`](https://www.geeksforgeeks.org/html-script-tag/) 标签中使用 JavaScript 创建一个 [`function`](https://www.geeksforgeeks.org/functions-in-javascript/)，一次只显示一条评论。

**示例:** 使用上述方法创建评审转盘。

**HTML 代码:** 和前两步一样，我们将在 [`body`](https://www.geeksforgeeks.org/html-body-tag/) 标签中创建一个嵌套的 `div` 标签和两个 [`button`](https://www.geeksforgeeks.org/html-button-tag/)。
...
现在，为了一次只显示一篇评论，我们将在 JavaScript 中创建一些 [`function`](https://www.geeksforgeeks.org/functions-in-javascript/)。
...
要显示指定的索引，首先，它将使用简单的 [`for` 循环](https://www.geeksforgeeks.org/javascript-for-loop/)将所有评论的显示设置为无，从而隐藏所有评论，对于特定的索引，它将通过将其显示设置为阻止来显示信息。

**注意:** [`if-else` 条件语句](https://www.geeksforgeeks.org/if-else-statement-in-javascript/)负责轮播的循环方式，如果参数为负会将参数设置到最后一个索引，如果参数大于等于最后一个索引会将参数设置到第一个索引。

**previousReview 函数:** 点击上一个按钮时会执行这个函数，它将变量减 1，然后传递给轮播函数。
