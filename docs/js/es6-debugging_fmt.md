...
> 原文：`https://www.geeksforgeeks.org/es6-debugging/`
...
为了避免每次都登录到控制台，ES6 提供了调试工具——`debugger`。`debugger`允许用户深入任何功能，并以循序渐进的方式监控一切。当 JavaScript Engine 和浏览器运行到 `debugger` 时，它会停止并为我们打开窗口。
下面的例子演示了`debugger`的使用。JavaScript 引擎在`debugger`处停止，并打开窗口，如下所示。该窗口提供有关程序中数组和累加器的信息。
