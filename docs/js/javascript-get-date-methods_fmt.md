...
*   `getDate()`: 用来取日为数字(1-31)。
*   `getFullYear()`: 用来取年份。
*   `getHours()`: 用于获取小时(0-23)。
*   `getMilliseconds()`: 用于获取毫秒(0-999)。
*   `getMinutes()`: 用于获取分钟(0-59)。
*   `getMonth()`: 用于取月份(0-11)。
*   `getSeconds()`: 用于获取秒(0-59)。
*   `getTime()`: 它用来返回 1970 年 1 月 1 日以来的毫秒数。
*   `getDay()`: 用于取工作日为数字(0-6)。
*   `Date.now()`: 用于返回自 1970 年 1 月 1 日 00:00:00 UTC 起经过的毫秒数。

我们将实现不同的日期方法，并通过示例了解它们的用法。

## getHours() 方法

JavaScript 中的 `getHours()` 方法用于以数字(0-23)的形式返回日期的小时数。

### 示例

本示例描述了 `getHours()` 方法，用于从 `Date` 对象检索指定日期到本地时间的小时。
...
JavaScript 中的 `getDate()` 方法用于以数字(1-31)的形式返回日期的日期。

### 示例

本示例描述了从 `Date` 对象中检索当月当前日期的 `getDate()` 方法。
...
JavaScript 中的 `getMonth()` 方法用于将日期的月份作为数字(0-11)返回。

### 示例

本示例描述了从 `Date` 对象中检索月份的 `getMonth()` 方法。
...
JavaScript 中的 `getFullYear()` 方法用于根据当地时间返回指定日期的年份(4 位数)。

### 示例

本示例描述了从 `Date` 对象中检索年份的 `getFullYear()` 方法。
...
JavaScript 中的 `getTime()` 方法用于返回毫秒数(0–999)。

### 示例

本示例描述了从 `Date` 对象中检索毫秒数的 `getTime()` 方法。
...
JavaScript 中的 `getSeconds()` 方法返回 `Date` 对象的秒数(0-59)。

### 示例

本示例描述了从 `Date` 对象中检索秒的 `getSeconds()` 方法。
