...
> 原文: [https://www.geeksforgeeks.org/javascript-check-the-existence-of-variable/](https://www.geeksforgeeks.org/javascript-check-the-existence-of-variable/)

JavaScript 有一个内置函数来检查变量是定义/初始化的还是未定义的。

**注:**

*   `typeof` 运算符将检查是否定义了变量。
*   当与未声明的变量一起使用时，`typeof` 运算符不会引发 `ReferenceError` 异常。
*   `typeof null` 将返回一个 `"object"`。所以，也需要检查 `null`。
...
**示例 3:** 前面的示例没有检查变量的空值。本示例还检查变量是否为 `null`。
