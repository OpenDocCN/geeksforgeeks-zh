...
> 原文：[https://www.geeksforgeeks.org/how-to-get-an-object-containing-parameters-of-current-url-in-javascript/](https://www.geeksforgeeks.org/how-to-get-an-object-containing-parameters-of-current-url-in-javascript/)
...
*   创建一个空对象。
*   使用 `String.match()` 方法提取所有由 `?` 和 `&` 分隔的查询参数。为了实现这一点我们使用了正则表达式 `/([^?=&]+)(=([^&]*))/g`。
*   `String.match()` 方法返回一个包含所有查询的数组。
*   使用 `for…each` 循环迭代数组，并在每次迭代中使用 `String.split()` 方法在 `=` 符号处拆分值。这个方法返回一个由 2 个字符串组成的数组，第 0 个字符串是 `=` 符号的左边部分，第 1 个字符串是 `=` 符号的右边部分。
...
<script>
    ...
    console.log(getAllParams("www.geeksforgeeks.org/search?name=john&age=27"))
