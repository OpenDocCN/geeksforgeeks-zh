...
> 原文: [https://www.geeksforgeeks.org/javascript-replace-multiple-strings-with-multiple-other-strings/](https://www.geeksforgeeks.org/javascript-replace-multiple-strings-with-multiple-other-strings/)

给定一个有多个字符串的句子。任务是用新字符串同时替换多个字符串，而不是使用 javascript 逐个替换。
下面是几个可以理解的方法:

### `replace()` 方法
此方法搜索一个字符串中定义的值或正则表达式，并返回一个替换了定义值的新字符串。
**语法:**
```
...
## 示例 1
本示例使用 `replace()` 方法，使用 RegExp 根据对象替换字符串。
```
<!DOCTYPE html>
<html>
<head>
...
## 示例 2
本示例首先创建一个正则表达式，然后使用 `replace()` 方法用新的关键字替换特定的关键字。
```
<!DOCTYPE HTML>
<html>
<head>
