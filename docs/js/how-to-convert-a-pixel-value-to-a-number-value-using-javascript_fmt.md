...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-a-pixel-value-to-a-number-value-using-javascript/](https://www.geeksforgeeks.org/how-to-convert-a-pixel-value-to-a-number-value-using-javascript/)

任务是借助 JavaScript 将包含`'px'`的字符串值转换为整数值。这里，很少讨论方法。

## 方法 1

使用 `parseInt()` 方法，该方法以字符串为第一个参数，返回整数值。
...
使用正则表达式将`'px'`替换为空字符串，然后使用 `Number()` 将结果转换为整数。
