...
> 原文：[https://www.geeksforgeeks.org/how-to-show-calendar-only-click-on-icon-using-javascript/](https://www.geeksforgeeks.org/how-to-show-calendar-only-click-on-icon-using-javascript/)
...
*   使用输入组文本类设置图标的跨度。
*   单击图标会触发 `setDatepicker()` 函数，`setDatepicker()` 函数接受当前事件作为参数。
...
第二种方法相对容易一些。它用更少的代码实现了目标。这段代码主要使用 jQuery。
