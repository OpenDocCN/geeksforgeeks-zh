...
> 原文: [https://www.geeksforgeeks.org/javascript-regexp-constructor/](https://www.geeksforgeeks.org/javascript-regexp-constructor/)

JavaScript 中的正则表达式构造函数用于返回创建正则表达式对象原型的函数，即对象的构造函数。它返回不同 JavaScript 类型的不同引用:

*   **正则表达式:** 构造函数属性为正则表达式返回 `function RegExp(){ [native code] }`。
*   **Numbers:** 构造函数属性返回 `function Number(){ [native code] }` 为 JavaScript 数字。
*   **Strings:** 构造函数属性返回 JavaScript 字符串的 `function String(){ [native code] }`。
