...
`first()`函数给出集合的第一个值。简单地说，它返回第一个值，或者`first()`方法返回集合中传递给定条件的第一个元素。在 JavaScript 中，首先将数组转换为集合，然后将函数应用于集合。
...
## 参数:
该函数接受如上所述的单个参数，如下所述:
* `e` : this parameter must be held through a set.的条件
...
## 示例 1:
在本例中，我们取一个集合，然后使用`first()`函数获取第一个值。
...
**参考:**`https://collect.js.org/api/first.html`