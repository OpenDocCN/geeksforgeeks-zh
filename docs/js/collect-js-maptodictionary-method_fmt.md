...
> 原文: [https://www.geeksforgeeks.org/collect-js-maptodictionary-method/](https://www.geeksforgeeks.org/collect-js-maptodictionary-method/)

`mapToDictionary()`方法用于在集合项上运行字典映射。给定的回调函数返回具有单个(键、值)对的关联数组。
...
`collect()`方法接受一个转换为集合的参数，然后对其应用`mapToDictionary()`方法。`mapToDictionary()`方法将回调函数作为参数保存。

## 返回值

这个方法返回一个具有单个(键，值)对的关联数组。
