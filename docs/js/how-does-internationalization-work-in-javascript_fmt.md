...
> 原文：[https://www.geeksforgeeks.org/how-does-internationalization-work-in-javascript/](https://www.geeksforgeeks.org/how-does-internationalization-work-in-javascript/)
...
`Intl` 对象是 `window` 对象上可用的对象，它充当国际化应用编程接口的命名空间。
...
在这个例子中，我使用 `DateTimeFormat` 构建器来使替换日期格式器错误地使用所需的地点（`"it-IT"`、`"en-US"` 和 `"en-GB"`）。
然后，我调用 `format()` 方法来格式化日期对象。
