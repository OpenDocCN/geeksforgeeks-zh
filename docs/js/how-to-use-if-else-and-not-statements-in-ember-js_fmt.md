...
> 原文：[https://www.geeksforgeks.org/how-to-use-if-else-and-not-statements-in-ember-js/](https://www.geeksforgeks.org/how-to-use-if-else-and-not-statements-in-ember-js/)
...
**第三步:** 这是我们的项目数据。现在让我们实现 `application.hbs` 文件。在 `application.hbs` 文件中，运行一个每个循环来列出每个项目。在这里，我们首先使用 `#each` 助手来迭代项目列表。然后我们对项目使用 `{{#if item.available}}` 来检查当前项目是否可用。如果可用，我们的项目将具有可用的 `else` 类，我们在代码的 `else` 部分使用了 `{{else}}` 标记。如果该项目不可用，我们的项目将具有“不可用”类别。接下来在 `app.css` 文件中，添加以下代码用于样式设计。
...
`unless` 和 `if` 正好相反。我们也可以在 `unless` 中使用 `ELSE`。因此，我们将在每个项目下放置一个文本，以告知该项目是否可用。
