...
> 原文：[https://www.geeksforgeeks.org/how-to-return-the-singular-plural-form-of-word-based-on-input-number-in-javascript/](https://www.geeksforgeeks.org/how-to-return-the-singular-plural-form-of-word-based-on-input-number-in-javascript/)

在本文中，我们将学习如何根据输入的数字返回单词的单数或复数形式。这可用于需要根据值动态更改单词的情况。这可以通过两种方法来实现。

## 方法 1

将单词的单数和复数形式传递给函数。在这种方法中，可以创建一个简单的函数，该函数接收单词的单数和复数形式以及计数值，并根据计数返回适当的单词。我们可以使用三元运算符（`?`）来检查所需的计数。
...
创建单词所有可能复数形式的词典。如果代码中重复出现大量单词，上述方法效率低下。我们可以通过创建一个单词复数值的字典来解决这个问题，并在检查计数后使用这个字典来选择合适的单词。
