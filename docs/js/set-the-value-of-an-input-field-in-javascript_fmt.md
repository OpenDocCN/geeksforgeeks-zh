...
> 原文: [https://www.geeksforgeeks.org/set-the-value-of-an-input-field-in-javascript/](https://www.geeksforgeeks.org/set-the-value-of-an-input-field-in-javascript/)

有时我们需要设置`<input>`元素的默认值，这个例子解释了这样做的方法。

## 文本值属性

该属性设置/返回文本字段的`value`属性的值。`value`属性包含默认值、用户键入的值或脚本设置的值。
...
此方法添加指定的属性到一个元素，并设置其指定值。如果属性已经存在，则其值会被设置/更改。
...
本示例通过`value`属性将输入元素的值设置为`"textValue"`。
...
本示例通过`setAttribute`方法将输入元素的值设置为`"defaultValue"`。
