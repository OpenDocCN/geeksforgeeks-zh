...
`Array.filter()` 是一个内置的方法，该方法创建一个新的数组，其元素遵循或通过给定的标准和条件。
...
该方法接受 2 个参数，上面提到过，下面描述：

*   **callback**: 函数是一个谓词，用来测试数组的每个元素。返回 `true` 以保留元素，否则返回 `false`。它接受三个论点：
