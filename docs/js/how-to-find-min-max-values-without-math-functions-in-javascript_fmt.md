...
> 原文: [https://www.geeksforgeeks.org/how-to-find-min-max-values-without-math-functions-in-javascript/](https://www.geeksforgeeks.org/how-to-find-min-max-values-without-math-functions-in-javascript/)
...
我们将继续使用循环检查数组中的所有元素。如果我们发现任何元素大于先前的 `max` 值，我们将该值设置为最大值。同样，我们继续检查任何小于 `min` 的元素。如果我们得到任何这样的元素，我们用一个较小的值替换 `min` 的值。因此，使用循环函数，我们可以得到在数组/列表中输入的最大值和最小值。
