...
> 原文：[https://www.geeksforgeeks.org/how-to-highlight-the-searched-string-result-using-javascript/](https://www.geeksforgeeks.org/how-to-highlight-the-searched-string-result-using-javascript/)
...
`mark.js` 中有很多内置函数，但是我们的需求使用了两个函数，分别是 `mark()` 和 `unmark()` 函数。这里，`mark()` 用于突出显示搜索文本，`unmark()` 用于取消突出显示之前突出显示的文本。
...
*   创建一个 `Mark` 对象，然后通过之前创建的 `obj` 调用 `mark()` 方法。
...
这几乎与上述技术相同，但唯一的细微差别是 `unmark()` 方法只带一个参数，即可选参数。此外，如果您想更改荧光笔的颜色和填充，那么我们需要对 `mark` 内部的 CSS 代码进行细微的更改，如下所示：
