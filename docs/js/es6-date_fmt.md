...
`ES6` 日期定义为自世界协调时 1970 年 1 月 1 日午夜以来经过的毫秒数。日期对象可以由 `new Date()` 构造器创建。在 JavaScript 中，日期和时间都由 `Date` 对象表示。

## 构造函数

1.  `Date()` – 为当前时间和日期创建日期对象。
...
2.  `Date(毫秒)` – 创建一个日期对象，其时间等于从世界协调时 1970 年 1 月 1 日起经过的毫秒数。
...
3.  `Date(字符串)` – 用指定的日期字符串创建一个日期对象，然后像那样自动解析。
...
4.  `Date(year, monthIndex [, day [, hours [, minutes [, seconds [, milliseconds]]]]])` – 使用本地时区中指定的日期创建一个日期对象。前两个参数是必需的，其余是可选的。
...
1.  `Date.prototype`：它代表了 `Date` 构造器的原型。`Date.prototype` 本身是一个普通的对象，而不是 `Date` 实例。它允许向日期对象添加属性。
2.  `Date.constructor`：它返回创建 `Date` 对象实例的数组函数的引用。

## 方法

1.  `Date.now()`：返回世界协调时 1970 年 1 月 1 日 00:00:00 后经过的毫秒数。
...
2.  `Date.parse()`：对于所提供的日期字符串表示形式，它返回自 1970 年 1 月 1 日 00:00:00 UTC 以来经过的毫秒数。

    ```
    console.log(Date.parse("2019-10-11")); // 1570800633000
    ```

3.  `Date.UTC()`：它返回提供的日期自 1970 年 1 月 1 日 00:00:00 UTC 起经过的毫秒数，用逗号分隔。
...
2.  `getDay()`：它根据给定日期对象的本地时间返回 0 到 6 的星期几。
...
5.  `getMilliseconds()`：它根据当地时间返回给定日期对象的毫秒数(0-999)。
...
6.  `getMinutes()`：它根据当地时间返回给定日期对象的分钟(0-59)。
...
7.  `getMonth()`：根据当地时间返回给定日期对象的月份(0-11)。
...
8.  `getSeconds()`：根据当地时间返回给定日期对象的秒(0-59)。
...
### 26. `setTime()`
用于将世界协调时 1970 年 1 月 1 日 00:00:00 后的毫秒设置为本地时间中的日期对象。
...
4. [`to localeDateString():`](https://www.geeksforgeeks.org/javascript-date-tolocaledatestring/) 它接受两个参数(可选)–区域设置和选项，并根据指定的区域设置返回日期部分的字符串表示。
