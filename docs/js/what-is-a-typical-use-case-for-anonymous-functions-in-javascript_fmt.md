...
> 原文：[https://www.geeksforgeeks.org/what-is-a-typical-use-case-for-anonymous-functions-in-javascript/](https://www.geeksforgeeks.org/what-is-a-typical-use-case-for-anonymous-functions-in-javascript/)
...
匿名函数是一个没有任何关联名称的函数。通常我们在函数名前使用 `function` 关键字来定义 JavaScript 中的函数，但是，在 JavaScript 中的匿名函数中，我们只使用 `function` 关键字而不使用函数名。匿名函数在初始创建后是不可访问的，它只能被一个变量访问，该变量作为一个值存储在 `function` 中。匿名函数也可以有多个参数，但只能有一个表达式。
...
| S 号 | Normal function | Anonymous function |
| :--- | :--- | :--- |
