...
> 原文: [https://www.geeksforgeeks.org/indexof-method-in-an-object-array-in-javascript/](https://www.geeksforgeeks.org/indexof-method-in-an-object-array-in-javascript/)

在本文中，我们将学习 Javascript 中对象数组中的 `indexOf()` 方法。要从具有对象值的数组中访问对象的索引，我们将使用几个方法。

*   通过使用 [`map()` 方法](https://www.geeksforgeeks.org/javascript-array-map-method/)
...
该方法创建一个单独的数组，并为每个数组元素调用一个函数。此方法为数组中的每个元素调用一次指定的函数，保持顺序不变。
...
*   `cValue`: 此参数为必选项。当前元素的值。
*   `ind`: 此参数可选。它指定当前元素的数组索引。
*   `Arr`: 此参数可选。它指定当前元素所属的数组对象。
*   `tValue`: 此参数可选。它指定要传递给函数的值作为其 `this` 值。如果留空，将使用 `undefined`。

**例 1:** 本例通过 `map()` 方法获取 `val_32` 的索引。
...
<body style="text-align:center;" id="body">
    ...
    <script>
        ...
        function gfg_Run() {
            ...
            el_down.innerHTML = "Index of 'val_32' value object is = " + pos;
        }
    </script>
</body>
...
该方法返回满足条件的数组中第一个元素的索引。如果此方法找到函数返回真值的数组元素，则此方法返回该数组元素的索引并停止，否则返回 `-1`。
...
**示例 3:** 这个示例没有得到值，所以返回 `-1` 并打印 `Not Found!`。
