...
> 原文: [https://www.geeksforgeeks.org/how-to-return-a-json-response-form-a-flask-api/](https://www.geeksforgeeks.org/how-to-return-a-json-response-form-a-flask-api/)

Flask 是设计 REST API 最广泛使用的 python 微框架之一。在本文中，我们将学习如何在 Flask 的帮助下创建一个简单的 REST API，该 API 返回一个简单的 JSON 对象。
...
我们将编写一个简单的 Flask API，使用两种方法返回一个 JSON 响应:
...
*   使用以下命令安装 Python `Flask` 库:

```
pip install Flask
```

*   使用以下命令安装 `Flask-RESTful` 库:
...
在这种方法中，我们将使用 Flask `jsonify` 方法返回一个 JSON 响应。在这种方法中，我们不打算使用 `flask-restful` 库。

*   创建一个名为 `main.py` 的新 Python 文件。
*   从 Flask 框架中导入 `Flask`、`jsonify` 和 `request`。
...
## 方法 2: 将 flask_restful 库与 Flask 一起使用

在这种方法中，我们将借助 `flask-restful` 库创建一个简单的 JSON 响应。下面讨论这些步骤:

*   创建一个名为 `main.py` 的新 Python 文件。
*   从 Flask 框架导入 `Flask`。
*   从 `flask_restful` 库中导入 `Api` 和 `Resource`。
*   使用以下语法将 web 应用注册到应用变量中。
