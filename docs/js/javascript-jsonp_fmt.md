...
**JSONP(带填充的 JSON):** 通过避免跨域问题来检索数据的一种方式。`script` 标签就是用来这样做的。
...
*   在 HTML 代码中，包含 `script` 标签。该脚本标记的来源将是从中检索数据的 URL。web 服务允许指定回调函数。在网址的最后包含回调参数。
    *   当浏览器遇到 `script` 元素时，它会向源 URL 发送 HTTP 请求。
