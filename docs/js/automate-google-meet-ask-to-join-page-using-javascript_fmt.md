...
> 原文：[https://www.geeksforgeeks.org/automate-google-meet-ask-to-join-page-using-javascript/](https://www.geeksforgeeks.org/automate-google-meet-ask-to-join-page-using-javascript/)
...
首先你需要为你的 Chrome 扩展创建一个 `manifest.json` 文件，让我们创建它。
...
现在让我们为 Chrome 扩展创建一个简单的前端。我们创建了一个名为 `popup.html` 的新的 HTML 文件。
...
<body>
    <h2>Joiner Active</h2>
    <p>
        You will automatically join the classes when this extension is ON.
    </p>
</body>
        ...
        try {
            for (i = 0; i < items.length; i++) {
                if (items[i].hasAttribute("aria-label")) {
                    if (items[i].getAttribute("aria-label").includes("microphone") ||
...
GitHub 仓库请点击：[https://github.com/Abhishek07Kalra/AutomaticClassJoiner](https://github.com/Abhishek07Kalra/AutomaticClassJoiner)