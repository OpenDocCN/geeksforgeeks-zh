...
## 不带 forEach()循环:
第一行用值 `[2，3，4，5，6]` 声明并初始化一个名为 `array_1` 的数字数组。为了使这个数组的每个元素都加倍，使用了一个 `for` 循环，它从零(因为数组的索引从零开始)到比数组长度小一的值。现在在第三行，从数组中提取每个元素，然后乘以 2，因此值会翻倍。
...
## 带 forEach()循环:
在 ES6 中，为数组引入了一种更容易理解和使用的方法，即 `forEach()`。让我们看看它是如何在上述情况下工作的。
...
### 程序 3:
这里我们只使用 `currentValue` 参数，使用该参数我们将名称数组中的每个值打印到控制台。
```html
<script>
