...
> 原文：`https://www.geeksforgeeks.org/difference-between-decodeuricomponent-and-decodeuri-functions-in-javascript/`
...
*   `decodeURI()`：它采用 `encodeURI(url)` 字符串，因此无法解码字符 `( , / ? : @ & = + $ # )`
*   `decodeURIComponent()`：它需要 `encodeURIComponent(url)` 字符串，以便能够解码这些字符。
