...
> 原文：[https://www.geeksforgeeks.org/dynamically-execute-javascript-in-electronjs/](https://www.geeksforgeeks.org/dynamically-execute-javascript-in-electronjs/)
...
在 Electron 中，每个 `BrowserWindow` 实例都可以被认为是应用程序中的单个网页。Electron 使用 `BrowserWindow` 对象和 `webContents` 属性创建和控制这些 `BrowserWindow` 实例。在传统的网络应用中，我们可以在浏览器的控制台内输入 `JavaScript` 代码，以便在网页上执行。为了通过脚本达到同样的效果，我们需要使用浏览器插件或扩展。在 Electron 中，`webContents` 属性为我们提供了某些实例方法，通过这些方法，我们可以在运行时在 `BrowserWindow` 实例中动态注入 JavaScript 代码。本教程将演示如何使用 `webContents` 属性的实例方法。
...
**示例：** 按照[给出的步骤](https://www.geeksforgeeks.org/introduction-to-electronjs/)，使用 Electron JS 构建桌面应用程序，以设置基本的 Electron 应用程序。复制文章中提供的 `main.js` 文件和 `index.html` 文件的样板代码。此外，对 `package.json` 文件进行必要的更改，以启动 Electron 应用程序。我们将继续使用相同的代码库构建我们的应用程序。
...
根据项目结构创建 `assets` 文件夹。在 `assets` 文件夹中创建 `sample.txt` 文件，用于演示目的。
...
**在 Electron 中动态注入 JS：** `BrowserWindow` 实例和 `webContents` 属性是主进程的一部分。要在渲染器进程中导入和使用 `BrowserWindow`，我们将使用 Electron `remote` 模块。

**index.html：** 在该文件中添加以下代码片段（HTML 部分）。
...
`Read sample.txt File` 和 `Print an Array` 按钮还没有任何相关功能。要进行更改，请在 `index.js` 文件中添加以下代码（JavaScript 部分）。
