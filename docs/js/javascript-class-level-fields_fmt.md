...
> 原文: [https://www.geeksforgeeks.org/javascript-class-level-fields/](https://www.geeksforgeeks.org/javascript-class-level-fields/)
...
**解释:** 要调用静态字段，我们需要使用构造函数类的名称。在上面的例子中，我们使用关键字 `static` 和 `#` 为私有字段创建了一个私有静态字段 `#name`，并将其初始化为一个空字符串。同样，我们正在创建一个私有静态字段 `#age`。现在，为了调用上面创建的字段，我们使用构造函数类 `User` 的名称作为 `User.#name` 和 `User.#age`。
