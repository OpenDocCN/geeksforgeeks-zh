...
> 原文：[https://www.geeksforgeeks.org/how-to-get-the-first-key-name-of-a-javascript-object/](https://www.geeksforgeeks.org/how-to-get-the-first-key-name-of-a-javascript-object/)

给定一个对象，任务是获取一个 JavaScript 对象的第一个键。由于 JavaScript 对象不包含编号索引，因此我们使用以下方法来获取对象的第一个键名。

## 方法 1

1.  首先将 JavaScript 对象放入一个变量中。
2.  使用 `object.keys(object name)` 方法访问对象的所有键。
3.  现在，我们可以使用像 `object.keys(object name)[0]` 这样的索引来获取对象的第一个元素的键。
...
1.  首先将 JavaScript 对象放入一个变量中。
2.  借助循环，开始访问 JavaScript 对象的所有键。
3.  运行一次后，打破它。然后我们将获得对象的第一个键。
