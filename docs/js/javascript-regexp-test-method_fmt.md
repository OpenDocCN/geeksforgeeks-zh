...
> 原文: [https://www.geeksforgeeks.org/javascript-regexp-test-method/](https://www.geeksforgeeks.org/javascript-regexp-test-method/)

JavaScript 中的`RegExp test()`方法用于测试字符串中的匹配。如果匹配，该方法返回`true`，否则返回`false`。
...
其中`str`是要搜索的字符串。这是必填字段。

## 示例 1

本示例在原始字符串中搜索字符串“computer”。
