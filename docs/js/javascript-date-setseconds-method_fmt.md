...
> 原文: [https://www.geeksforgeeks.org/javascript-date-setseconds-method/](https://www.geeksforgeeks.org/javascript-date-setseconds-method/)

下面是 `Date.setSeconds()` 方法的例子。
...
*   **seconds_Value:** 此参数保存秒的值，用于在 `Date()` 构造函数中设置。

## 返回值:

返回由 `setSeconds()` 方法设置的更新秒的新日期。
...
如果在 `Date()` 构造函数中我们没有给出任何秒，仍然 `setSeconds()` 方法设置新的秒作为它的参数。
...
如果 `Date()` 构造函数中没有给定参数，`setSeconds()` 方法仍然设置秒，但是月、年、日期等变成当前的。这里 42 是新的秒，3 是当月即 4 月，1 是当前日期，2018 是当前年份。
...
如果给定秒 66 的值作为 `setSeconds()` 函数的参数，它将设置 6 为秒，因为秒的范围是从 0 到 59 和 (66 % 60 = 6)。
...
JavaScript Date `setSeconds()` 方法支持的浏览器如下:
