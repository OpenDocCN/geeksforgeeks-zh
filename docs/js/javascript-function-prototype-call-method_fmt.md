...
> 原文：`https://www.geeksforgeeks.org/javascript-function-prototype-call-method/`

`call()`方法允许分配属于一个对象的函数调用，并为不同的对象调用。它为函数提供了`this`的新值。`call()`方法允许您一次性编写一个方法，并允许它在另一个对象中继承，而无需为新对象重写方法。
...
*   `thisArg`：调用`myFunc`函数时用作`this`的值。
*   `arg1, arg2, …, argN`：这些是上述函数的参数。

**注意：**在某些情况下，`thisArg`可能不是实际值。如果该方法处于非严格模式，原始值将被转换为对象并包装，`undefined`或`null`将被替换为全局对象。

**返回值：**作为函数调用的结果，它返回指定的`this`值和参数。

**示例 1：**以下示例演示了如何使用`call`来链接对象的构造函数。
...
const restau = new Restaurant('KFC', 1000);
...
**示例 3：**以下示例演示了使用`call`方法来调用函数，并为`this`指定上下文。
...
function display() {
    console.log('string value is %s ', this.str);
}
