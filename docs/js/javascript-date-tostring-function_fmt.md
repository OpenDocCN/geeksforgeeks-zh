...
> 原文: [https://www.geeksforgeeks.org/javascript-date-tostring-function/](https://www.geeksforgeeks.org/javascript-date-tostring-function/)
...
这里在创建日期对象时没有传递任何参数，但是 `toString()` 方法返回当前的日名称、月名称、日期、年和时间。
...
当一些随机的值列表被传递时，`toString()` 方法返回相应的产生的字符串。
...
JavaScript Date `toString()` 方法支持的浏览器如下:
