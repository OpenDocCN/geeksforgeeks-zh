...
它接受单个整数文字作为字符串，需要表示为 BigInt。

## 返回类型

该方法返回给定值作为 BigInt 数据类型。

## 示例 1

本示例使用 `BigInt()` 函数创建一个 BigInt。
...
BigInt 在某些方面类似于 Number，但是，它不能与内置 `Math` 对象的方法一起使用，也不能与 Number 的实例在操作中混合使用。
...
数组既可以保存基元数据类型，也可以保存 BigInts。这使得 `sort()` 方法可以在数组中同时存在正常的数字和大整数值时工作。
...
由于 BigInt 的实现，不建议将以下应用与 BigInt 一起使用：

1.  **强制**：Number 和 BigInt 之间强制会导致精度损失，建议只在合理预期值大于 2<sup>53</sup> 时使用 BigInt，不要在两种类型之间强制。
