...
> 原文：[https://www.geeksforge.org/javascriptes6-object-literal-enhancement/](https://www.geeksforgeeks.org/javascriptes6-object-literal-enhancement/)
...
**输出：** `name`、`color`、`age` 现在是 `dog` 对象的关键。
...
定义对象方法时，不再需要使用 `function` 关键字。对象文字增强允许我们将全局变量拉入对象，并通过使 `function` 关键字变得不必要来减少键入。
