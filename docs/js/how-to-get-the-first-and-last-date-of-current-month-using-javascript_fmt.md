...
> 原文：[https://www.geeksforgeeks.org/how-to-get-the-first-and-last-date-of-current-month-using-javascript/](https://www.geeksforgeeks.org/how-to-get-the-first-and-last-date-of-current-month-using-javascript/)
...
## 示例 1
本示例通过使用 `getFullYear()` 和 `getMonth()` 方法获取当前日期的年和月来设置 `var firstDay` 和 `lastDay` 的日期，然后获取该月的第一天和最后一天。
...
## 示例 2
本例与上例相似。此示例还通过使用 `getFullYear()` 和 `getMonth()` 方法获取当前日期的年和月来设置 `var` 的第一天和最后一天的日期，然后通过不同的方法获取该月的第一天和最后一天。
