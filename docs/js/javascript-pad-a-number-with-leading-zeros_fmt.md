...
> 原文: [https://www.geeksforgeeks.org/javascript-pad-a-number-with-leading-zeros/](https://www.geeksforgeeks.org/javascript-pad-a-number-with-leading-zeros/)

## `String.prototype`
`String.prototype` 属性用于用前导零填充数字。此属性允许向现有对象类型添加新的属性和方法。
...
## 返回值:
返回对 `String.prototype` 对象的引用。

## 示例 1:
本示例通过创建一个函数向一个数字添加前导零，在该函数中，如果该数字小于所提供的宽度，则添加前导零。
...
## 示例 2:
本示例通过创建原型 `pad` 将前导零添加到数字中。在这个例子中，我们可以向对象传递一个字符串，无论我们想用数字填充什么。
