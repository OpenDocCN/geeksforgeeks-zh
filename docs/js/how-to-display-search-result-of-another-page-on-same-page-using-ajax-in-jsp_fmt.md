...
> 原文：[https://www.geeksforgeeks.org/how-to-display-search-result-of-another-page-on-same-page-using-ajax-in-jsp/](https://www.geeksforgeeks.org/how-to-display-search-result-of-another-page-on-same-page-using-ajax-in-jsp/)
...
我们使用 JQuery ready 函数来确保我们的 JavaScript 在页面加载之前不会执行。在就绪功能中，我们执行一个简单的使用点击事件，它是提交按钮的 id，应该在该按钮上执行操作。
...
*   `$("#user_input").val()`: 从文本字段中获取输入值，其中 `user_input` 是 id。
...
*   `$("#msg").html(data)`: `msg` 是 div 标签的 id 意思，它只会在 div 标签里面显示结果。
...
<html>
<head>
    <title>search result of another page on same page using ajax in JSP</title>
    <meta charset="UTF-8">
...
<body>
    <center>
        <div>
            ...
            <input placeholder="Search" type="text" id="user_input"/>
