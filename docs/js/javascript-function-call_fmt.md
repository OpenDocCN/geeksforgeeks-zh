...
函数调用是预定义的 JavaScript 方法，用于为不同的对象编写方法。它调用方法，将所有者对象作为参数。 [这个](https://www.geeksforgeeks.org/this-in-javascript/) 关键词是指该功能或其所属对象的“所有者”。JavaScript 中的所有函数都被认为是对象方法。如果一个函数不被认为是一个 JavaScript 对象的方法，那么这个函数就是全局对象。
