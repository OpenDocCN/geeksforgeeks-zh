...
> 原文：[https://www.geeksforgeeks.org/how-to-compare-date-part-only-without-comparing-time-in-javascript/](https://www.geeksforgeeks.org/how-to-compare-date-part-only-without-comparing-time-in-javascript/)

为了在 JavaScript 中获取日期和时间，我们有 `Date()` 类。我们通过创建对象来访问 `Date()` 类。`Date()` 类返回日期和时间的总和。
...
因此 `Date()` 返回 `function Date(){[native code]}`，我们可以使用 `object.toString()` 将其转换为字符串。

## 方法 1

当对象被比较时，它会被与时间和日期进行比较。
...
这里我们将使用 `toDateString()` 来获取唯一的日期，但是当获取时它被转换为字符串，因为字符串的比较是不可能的，我们需要获取然后再次转换为日期对象，现在当将它们转换回对象时时间设置为 00:00:00（停止）。
