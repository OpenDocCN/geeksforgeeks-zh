...
## 简介
`Next.js` 基于 `react`、`webpack` 和 `babel`。它是创建 `web` 应用程序的一个很棒的工具，并以服务器端渲染而闻名。`Next.js` 由 [Zeit](https://github.com/zeit/next.js/) 打造。了解 `HTML`、`CSS`、`Java Script` 和 `React` 的开发人员可以轻松学习并切换到 `next.js`。
...
*   **第二步:** 现在为您的项目在桌面上创建一个文件夹，通过您的代码编辑器导航到该文件夹，并在终端上运行以下命令。
...
*   **第三步:** 在 `pages` 文件夹中添加一个文件 `index.js`，并在其中添加以下代码。
