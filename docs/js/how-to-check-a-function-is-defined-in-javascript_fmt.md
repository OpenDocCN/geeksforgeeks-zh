...
> 原文: [https://www.geeksforgeeks.org/how-to-check-a-function-is-defined-in-javascript/](https://www.geeksforgeeks.org/how-to-check-a-function-is-defined-in-javascript/)

工作是识别一个函数是否被定义。JavaScript `typeof` 运算符用于解决下面描述的问题:

## `typeof` 运算符

该运算符可用于查找 Javascript 变量的类型。此运算符返回变量或表达式的类型:
...
本示例检查函数的类型，如果是函数，则使用 `typeof` 运算符对其进行定义。
...
本示例检查函数的类型，如果是函数，则通过创建函数使用 `typeof` 运算符来定义，否则无法定义。
