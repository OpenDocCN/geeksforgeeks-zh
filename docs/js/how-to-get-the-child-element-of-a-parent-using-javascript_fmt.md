...
> 原文：[https://www.geeksforgeeks.org/how-to-get-the-child-element-of-a-parent-using-javascript/](https://www.geeksforgeeks.org/how-to-get-the-child-element-of-a-parent-using-javascript/)

在本文中，我们将学习使用 Javascript 获取父元素的子元素。给定一个 HTML 文档，任务是选择一个特定的元素，并在 JavaScript 的帮助下获取父元素的所有子元素。为此，有两种方法可以获取子元素：

*   通过使用 [`children`](https://www.geeksforgeeks.org/html-dom-children-property/) 属性
*   使用 [`querySelector()`](https://www.geeksforgeeks.org/html-dom-queryselector-method/) 方法

我们将讨论这两种方法，并通过示例了解它们的实现。

## 方法一：使用 `children` 属性

`children` 属性用于返回指定元素的所有子元素的 `HTMLCollection`。

### 步骤

1.  选择要选择其子元素的元素。
2.  使用 `children` 属性获取元素的所有子元素的访问权限。
3.  根据索引选择特定的子级。

### 示例 1

此示例实现了 `children` 属性获取指定元素的所有子元素的 `HTMLCollection`。
...
1.  选择要选择其子元素的父元素。
2.  使用父元素上的 `querySelector()` 方法。
3.  使用子对象的类名来选择特定的子对象。

### 示例 1

本例实现了 `querySelector()` 方法获取文档中指定 CSS 选择器要匹配的第一个元素。
