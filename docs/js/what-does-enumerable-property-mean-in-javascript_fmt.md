...
> 原文：[https://www.geeksforgeeks.org/what-does-enumerable-property-mean-in-javascript/](https://www.geeksforgeeks.org/what-does-enumerable-property-mean-in-javascript/)
...
由于所有属性都是由属性初始化器初始化的，因此默认情况下，它们都将可枚举设置为 `true`。要显式更改属性的内部可枚举属性，请使用 `Object.defineProperty()` 方法。另外，为了检查一个属性是否可枚举，我们使用函数 `propertyIsEnumerable()`。如果属性可枚举，则返回 `true`，否则返回 `false`。
