...
> 原文: [https://www.geeksforgeeks.org/javascript-trimend-and-trimright-method/](https://www.geeksforgeeks.org/javascript-trimend-and-trimright-method/)
...
`trimEnd()` 方法有一个别名，即 `trimRight()` 方法。它执行与 `trimEnd()`完全相同的功能。
...
以下列出了 `trimEnd()` 和 `trimRight()` 方法支持的浏览器:
