...
> 原文：[https://www.geeksforgeeks.org/how-to-check-caps-lock-is-on-off-using-javascript-jquery/](https://www.geeksforgeeks.org/how-to-check-caps-lock-is-on-off-using-javascript-jquery/)

工作是使用 JavaScript 和 jQuery 确定**大写锁定**是**开启**还是**关闭**。

## 使用 JavaScript 检查大写锁定是否打开/关闭

*   **`addEventListener()` 方法：** 此方法向文档添加一个事件处理程序。
...
这个示例向文档中添加了一个事件侦听器，当它发生时，它会调用一个匿名函数来处理这个事件。它通过使用按钮的**键码**来检查它是大写锁定键还是 SHIFT 键。
...
本示例向文档添加一个事件侦听器，当事件侦听器发生时，它会检查是否按下了 CAPS LOCK。
...
*   **jQuery `on()` 方法：** 此方法为选定的元素和子元素添加一个或多个事件处理程序。
...
这个示例向文档主体添加了一个事件侦听器，当它发生时，它会调用一个匿名函数来处理这个事件。它通过使用 `toUpperCase()` 和 `toLowerCase()` 来检查它是大写锁定键还是移位键。
...
该示例与前一示例的操作相同，但方法不同。向文档中添加一个事件侦听器，当它发生时，它会检查是否按下了大写锁定键。
