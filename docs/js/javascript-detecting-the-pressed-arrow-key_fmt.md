...
> 原文：[https://www.geeksforgeeks.org/javascript-detecting-the-pressed-arrow-key/](https://www.geeksforgeeks.org/javascript-detecting-the-pressed-arrow-key/)
...
*   **event**：此参数为必填项。它以字符串格式指定事件的名称。
*   **function**：此参数为必填项。它指定事件发生时要运行的函数。

当事件发生时，事件对象作为第一个参数传递给函数。事件对象的类型取决于指定的事件。

*   **captureBoolean**：此参数为可选参数。它指定一个布尔值，该值指示事件应该在捕获阶段还是在冒泡阶段执行。默认值为 `false`，事件在冒泡阶段执行；对于 `true`，事件在捕获阶段执行。

## 示例-1

本示例通过使用 `event.keyCode` 和 `onkeydown` 事件来检测箭头键。
...
<head>
    <title>
        JavaScript | Detecting arrow key presses.
    </title>
</head>
...
本示例通过使用 `event.key` 在 `body` 中添加 `addEventListener('keydown', ...)` 来检测箭头键。
