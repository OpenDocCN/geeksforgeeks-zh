...
`BigInt.asIntN()`方法是 JavaScript 中的一个内置方法，用于将 BigInt 值包装为-2 <sup>width-1</sup> 和 2 <sup>width-1</sup> -1 之间的有符号整数。
...
该方法将 bigint 模 2 <sup>宽度</sup>的值作为有符号整数返回。

以下示例说明了 JavaScript 中的`BigInt.asIntN()`方法:
