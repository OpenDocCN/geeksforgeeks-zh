...
> 原文: [https://www.geeksforgeeks.org/javascript-strip-all-non-numeric-characters-from-string/](https://www.geeksforgeeks.org/javascript-strip-all-non-numeric-characters-from-string/)
...
*   `newValue`：必选参数。它指定要由搜索值替换的值。

## 示例 1

本示例在正则表达式的帮助下，从字符串 `'1 gee2k for 345 geek 6'` 中去除所有非数字字符。
...
本示例从字符串 `'1 gee2ksfor 345 Gee67ks89'` 中删除所有非数字字符。在 `RegExp` 的帮助下，本示例保留浮点数。
