...
> 原文: [https://www.geeksforgeeks.org/how-to-replace-the-entire-html-node-using-javascript/](https://www.geeksforgeeks.org/how-to-replace-the-entire-html-node-using-javascript/)
...
*   使用 `html()` 方法在 HTML 元素上，并用新的 HTML 文档替换它（例如 `$('html').html(Str)`）。
...
<body style="text-align:center;">
    ...
    <script>
        ...
        function GFG_Fun() {
            var Str = '<!DOCTYPE HTML><html><head><title>Check if an element is a' +
...
*   将新文档作为字符串形式（例如 `Str = ""`）。
*   使用 `document.open()` 方法，该方法采用两个参数（第一个是 `"text/html"`，第二个是 `"replace"`）。如果我们不使用 `"replace"`，那么方法将调用添加页面历史。所以我们必须点击两次返回到上一页。所以，`replace` 是必要的参数通过。
...
<body style="text-align:center;">
    ...
    <script>
        ...
        function GFG_Fun() {
            var Str = '<!DOCTYPE HTML><html><head><title>Check if an element is a div' +
