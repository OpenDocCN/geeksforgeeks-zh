...
ES6 中有三种定形环。下面用例子来描述它们:

| 类型 | 描述 |
| --- | --- |
| `for(;;)` | 在终止条件下执行循环块指定次数。 |
| `for…in` | 通过对象的属性执行循环块。 |
| `for…of` | 执行循环块以迭代可迭代的而不是对象文字。 |
...
在每次迭代中，对象的一个属性被分配给变量 `variable_name`，这个循环一直持续到对象属性结束。它肯定会结束它的迭代，所以它会进入一个确定的循环。

### `for…of`

`for…of` 循环用于执行循环块以迭代可迭代对象而不是对象字面量。
...
在每次迭代中，iterable 的一个属性(数组、字符串等)被赋给变量 `variable_name`，这个循环一直持续到迭代结束，它肯定会结束它的迭代，所以，它进入了一个确定的循环。
...
ES6 中有两类不定环。下面用例子来描述它们:

| 类型 | 描述 |
| --- | --- |
| `while` | 在终止条件下执行循环块指定次数。 |
| `do…while` | 类似于 `while` 循环，但首先执行循环并计算。 |

### `while` 循环

这个循环属于不定循环，它可能会进入不确定或无穷大阶段。这个循环有两种类型。
...
`do…while` 循环类似于 `while` 循环，不同的是 `do…while` 循环不会在循环第一次执行时评估条件。不管终止条件如何，它至少执行一次循环。
...
为了中断流程的执行或控制执行流程，我们使用循环控制语句。

| 关键字 | 描述 |
| --- | --- |
| `break` | `break` 语句用于将控件从循环中取出。 |
| `continue` | `continue` 语句跳过当前迭代中的后续语句，并将控制带回到循环的开始。 |
| `return` | `return` 语句结束函数调用的执行，并“返回”结果 |
...
一个**标签**只是一个标识符，后跟一个冒号( `:` )，用于一条语句或一段代码。标签可以在 `break` 时使用，并继续更精确地控制流，但是标签的使用没有被广泛使用，因为它会导致时间复杂性问题，并且随着更多的内部循环而变得更加复杂。
