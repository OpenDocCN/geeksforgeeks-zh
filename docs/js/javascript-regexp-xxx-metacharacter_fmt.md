...
> 原文：`https://www.geeksforgeeks.org/javascript-regexp-xxx-metacharacter/`

JavaScript 中的 `RegExp \xxx` 元字符用于查找八进制数 `xxx` 指定的字符。如果找到匹配，则返回字符，否则返回空值。
...
本示例返回八进制数 `107` 对应的字符，即整个字符串中的 `G`。
