...
> 原文：[https://www.geeksforgeeks.org/next-js-swr-stale-while-revalidate-introduction/](https://www.geeksforgeeks.org/next-js-swr-stale-while-revalidate-introduction/)
...
除了自定义 API 调用 & REST API 集成外，Zeit 的 `SWR` 附带的内容描述如下。

*   **焦点重新验证：** 当您在浏览器中重新聚焦页面或在选项卡之间切换时，`SWR` 会自动重新验证数据。
*   **快速导航：** 一旦从缓存中渲染数据，`SWR` 就会自动从原点重新验证数据。
*   **在间隔上重新预取：** `SWR` 会给你自动获取数据的选项，其中预取只会发生在屏幕上与钩子相关的组件上。
...
`Next.js` 是一个基于 `React` 的框架。正是基于反应过来，`Webpack` 和 `Babel`。它以自动代码拆分、热代码重新加载（即一旦保存更改就重新加载）而闻名 & 最重要的是服务器端渲染。这将这个框架放在 `React` 文档中推荐的工具链之上。
...
*   **步骤 1：** 通过运行以下命令检查节点 & `npm` 版本
    ...
    $ node -v && npm -v
    ```
*   **步骤 2：** 创建目录，到达终端目标目录后执行
    ```
    $ npm install --save next react react-dom
    ```
*   **步骤 3：** 在 `pages` 文件夹（基本上是 `pages/index.js`）的 `index.js` 中创建一个文件，添加以下代码，运行 `npm start` 即可看到它在运行！
...
我们将使用 `SWR` & 与强大的同构执行数据提取，这是需要安装的两个依赖项（代码中给出的命令）。
