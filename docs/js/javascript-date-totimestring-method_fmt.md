...
> 原文: [https://www.geeksforgeeks.org/javascript-date-totimestring-method/](https://www.geeksforgeeks.org/javascript-date-totimestring-method/)
...
此功能不接受任何参数。它只是与使用 `Date()` 构造函数创建的 `Date` 对象一起使用，该构造函数的时间部分内容以类似英语的语言返回。
...
`DateObj` 是使用 `Date()` 构造函数创建的有效日期对象，其时间部分内容被返回。

上述方法的更多代码如下:

## 程序 1:

在这里，在创建日期对象时，没有作为参数传递任何东西，但 `toTimeString()` 方法仍然返回当前时间。
...
当 `Date()` 构造函数的参数没有给出时间部分时，它将时间返回为零(0)。
...
JavaScript Date toTimeString()方法支持的浏览器如下:
