...
> 原文：[`https://www.geeksforgeeks.org/javascript-object-fromentries-method/`](https://www.geeksforgeeks.org/javascript-object-fromentries-method/)

JavaScript 中的 `Object.fromEntries()` 方法是标准的内置对象，用于将键值对列表转换为对象。此方法返回一个新对象，该对象的属性由 `iterable` 的条目给出。
...
该方法接受单参数 `iterable`，它保存一个可迭代的，如数组或映射或其他实现可迭代协议的对象。

## 返回值

这个方法总是返回一个新的对象，它的属性由 `iterable` 的条目给出。
