...
*   松散地说，JavaScript 中的对象可以被定义为相关数据的无序集合，原始或引用类型，形式为“键:值”对。这些键可以是变量或函数，在对象的上下文中分别称为属性和方法。
...
在上例中`name`、`location`、`established`都是键，`"Vivekananda School"`、`"Delhi"`和`1971`分别是这些键的值。

这些键中的每一个都被称为对象的属性。JavaScript 中的一个对象也可以有一个成员函数，在这种情况下，它将被称为该对象的方法。
...
在上面的例子中，`displayInfo`是`school`对象的一个方法，用于处理存储在其属性中的对象数据。
...
像是数字的属性名一样，它们也必须使用括号符号来访问。例如，如果我们想访问“Vivekananda School”的“Vivekananda”，我们可以这样做:
...
*   **可配置:**如果为假，尝试删除属性、将属性更改为访问或属性、或更改其属性(除`[[Value]]`之外，或将`[[Writable]]`更改为假)将失败。
...
1.  **使用对象字面语法：** 对象字面语法使用`{…}`表示法来直接初始化一个对象及其方法/属性。
    让我们看一个使用此方法创建对象的例子：
...
2.  **对象构造函数：** 在 JavaScript 中创建对象的另一种方法涉及使用`Object`构造函数。`Object`构造函数为给定值创建一个对象包装器。这与`new`关键字结合使用，允许我们初始化新对象。
    例子：
...
注意功能 `Vehicle` 前“new”关键字的用法。在任何函数将其转换为构造函数之前，以这种方式使用“new”关键字。“`new Vehicle()`”实际上是:
...
在上面的例子中，`footballers`充当了创建对象“`footballer1`”的原型。
