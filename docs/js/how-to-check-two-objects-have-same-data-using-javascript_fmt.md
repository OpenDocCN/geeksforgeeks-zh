...
> 原文：[https://www.geeksforgeeks.org/how-to-check-two-objects-have-same-data-using-javascript/](https://www.geeksforgeeks.org/how-to-check-two-objects-have-same-data-using-javascript/)

在 JavaScript 中，我们不能通过[等式运算符](https://www.geeksforgeeks.org/javascript-vs-comparision-operator) (`==` 或 `===`) 直接比较两个对象，看它们是否相等。像这样比较两个对象会导致错误，即使它们有相同的数据。因为这是两个不同的对象实例，它们指的是两个不同的对象。
...
**注意：** 上述方法不适用于嵌套对象（对象内部的对象和数组）。在这种情况下，它需要根据嵌套对象运行。
