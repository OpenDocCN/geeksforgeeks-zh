...
> 原文: [https://www.geeksforgeeks.org/how-to-return-current-url-for-a-share-button-using-javascript/](https://www.geeksforgeeks.org/how-to-return-current-url-for-a-share-button-using-javascript/)

在这篇文章中，我们将学习网站上显示的共享按钮背后的逻辑，这些按钮可用于在其他社交媒体平台上共享一篇文章或网站的完整网址。我们将通过几个步骤来了解这一点。

## 第一步：创建 HTML 文件

首先，我们需要创建一个 HTML 文件，它将在浏览器上显示所有内容。
...
以上是 HTML 代码，我们将从这一点开始只编写 JavaScript 部分。

## 第二步：添加 JavaScript 代码以显示 URL

在这一步中，我们将添加 JavaScript 代码，该代码将以链接的形式显示当前网页的 URL。

*   这里的 `window` 是一个全局对象，它由各种属性组成，其中 `location` 是一个这样的属性，`href` 属性为我们提供了完整的网址以及使用的路径和协议。
...
我们不希望浏览器窗口一加载就显示网址，而是希望点击按钮就显示。因此，我们将向按钮添加一个事件侦听器，然后在单击按钮时显示 URL。
...
现在点击按钮就会显示链接，但是我们不需要当前页面的链接，而是在社交媒体平台上共享当前网页的链接。所以我们更新代码如下。
...
这就是我们如何获得共享网址。而不是显示共享网址，我们实际上可以在按钮内使用它来直接将用户重定向到特定的社交媒体平台。此外，我还使用了 Facebook 分享网址，几乎所有的社交媒体平台都有类似的网址，你可以随意尝试，并想出你自己的分享按钮。这篇文章已经涵盖了使用 JavaScript 共享当前网页网址的所有要点。