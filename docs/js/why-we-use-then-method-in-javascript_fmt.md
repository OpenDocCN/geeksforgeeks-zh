...
> 原文：`https://www.geeksforgeeks.org/why-we-use-then-method-in-javascript/`
...
该方法可以返回一个承诺（如果再调用另一个承诺），则()或者不返回任何承诺。
...
每个 `then()` 可以返回一个承诺（一个解决或一个拒绝），因此多个 `then()` 方法可以链接在一起。
