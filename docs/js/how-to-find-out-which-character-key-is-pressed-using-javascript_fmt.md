...
> 原文: [https://www.geeksforgeeks.org/how-to-find-out-which-character-key-is-pressed-using-javascript/](https://www.geeksforgeeks.org/how-to-find-out-which-character-key-is-pressed-using-javascript/)

在这里，任务是获得字符键，按下。在下面编写的代码中，当任何键被按下并调用一个函数时触发的事件，然后该函数识别它。

## 进场:

- 将事件附加到输入框。像`keypress`事件。
- 当该事件发生时调用一个函数，并在其中传递`event`参数。
- 在调用的函数中，识别按下的键。
