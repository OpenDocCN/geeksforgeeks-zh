...
> 原文：[https://www.geeksforgeeks.org/how-to-write-an-inline-if-statement-in-javascript/](https://www.geeksforgeeks.org/how-to-write-an-inline-if-statement-in-javascript/)

我们可以使用下面描述的方法用 `javascript` 编写一个内联 `IF` 语句。

## 方法 1

在这个方法中，我们只使用下面给出的语句编写一个内联 `IF`（无 `else`）语句。
...
如果 `condition` 为真，则 `value1` 将被分配给 `result` 变量，如果为假，则 `value2` 将被分配。
