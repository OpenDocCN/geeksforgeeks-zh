...
> 原文：[https://www.geeksforgeeks.org/how-to-create-video-slide-animation-using-css-and-javascript/](https://www.geeksforgeeks.org/how-to-create-video-slide-animation-using-css-and-javascript/)
...
- 在 HTML 文件的主体中创建一个 `container` 类。
- 在视频标签中使用 `slider` 类。
- 在视频标签中使用 `autoplay` `loop` `muted` 类（进行循环）。
- 使用 `li` 标签制作视频列表。
- 使用类给 HTML 元素赋予效果。
- 在视频中使用 `onClick` 事件。
...
现在我们将看到如何在任何网页上使用 HTML、CSS、JS 创建视频幻灯片动画。

## 分步实施

### 第一步

创建名为 `index.html` 的 HTML 文件，添加下面的代码。
...
创建名为 `style.css` 的 CSS 文件，添加下面的代码。
