...
由于 Pipeline Operator 是一个实验性特性，目前处于第 1 阶段提案中，因此不支持当前可用的浏览器，因此也不包含在 Node 中。然而，人们可以使用 Babel (JavaScript 编译器)来使用它。
...
*   安装 Babel 以便使用操作员。管道操作符目前不是 JavaScript 语言的一部分，Babel 用于编译代码，以便 Node 能够理解。

```
npm install @babel/cli @babel/core @babel/plugin-proposal-pipeline-operator
```

*   为了指导 Babel 正确编译代码，一个名为 `.babelrc` 的文件由以下线条创建:
...
*   添加一个 `start` 脚本到 `package.json` 文件，该文件将运行 Babel:
