...
> 原文: [https://www.geeksforgeeks.org/javascript-method-to-get-the-url-without-query-string/](https://www.geeksforgeeks.org/javascript-method-to-get-the-url-without-query-string/)
...
本示例首先使用 `href` 提取页面的所有 URL，然后通过设置 `索引= 0` 获取第一个 URL，然后删除 `之后的部分？` 采用 `分裂()法`。
...
本例通过 `替换()方法从` 位置`将`位置搜索`替换为`空字符串`。`
