...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-unix-timestamp-to-time-in-javascript/](https://www.geeksforgeeks.org/how-to-convert-unix-timestamp-to-time-in-javascript/)

UNIX 时间戳可以通过两种方法转换为时间：

## 方法 1：使用 `toUTCString()` 方法

由于 JavaScript 的工作时间是以毫秒为单位的，所以需要将时间乘以 1000 再转换成毫秒。然后将该值赋予 `Date()` 函数，以创建新的日期对象。

`toUTCString()` 方法用于将日期对象表示为世界协调时时间格式的字符串。从这个日期字符串开始的时间可以通过提取从字符串的第 11 个到最后一个到第 4 个到最后一个字符来找到。这是使用 `slice()` 功能提取的。该字符串是 UNIX 时间戳的时间表示。
...
由于 JavaScript 是以毫秒为单位工作的，在转换之前需要将时间乘以 1000 转换成毫秒。然后将该值赋予 `Date()` 函数，以创建新的日期对象。

时间的每一部分都是从日期对象中提取的。使用 `getUTCHours()` 方法从日期中提取以 UTC 表示的小时值。使用 `getUTCMinutes()` 方法从日期中提取以 UTC 表示的分钟值。使用 `getUTCSeconds()` 方法从日期中提取秒的 UTC 值。
