...
JavaScript 中的 `RegExp` `m` 修饰符用于执行多行匹配。当接管多行时，它将开始和结束字符(`^`和`$`)作为工作字符。它匹配每一行的开头或结尾。它区分大小写。
...
本示例搜索字符串中每行开头的单词“geeksforgeeks”。
...
本示例搜索字符串中每行开头的单词“geeksforgeeks”，并将其替换为“GEEKSFORGEEKS”。
