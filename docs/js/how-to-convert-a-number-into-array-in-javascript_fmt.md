...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-a-number-into-array-in-javascript/](https://www.geeksforgeeks.org/how-to-convert-a-number-into-array-in-javascript/)
...
`Array.from()` 方法从任何具有长度属性的对象或可迭代对象返回 Array 对象。
...
4.  The `myFunc` function will use the `Array.from()` method to iterate the returned parameters. We convert the numeric type to a string, so the parameter type is a string, but we will convert its type to an integer and return it.
...
JavaScript 中的 `map()` 方法通过对父数组中的每个元素调用特定的函数来创建数组。这是一种非突变方法。通常，`map()` 方法用于迭代数组，并对数组的每个元素调用一个函数。
