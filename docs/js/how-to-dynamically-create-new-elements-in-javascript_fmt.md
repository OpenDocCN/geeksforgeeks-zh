...
> 原文：[https://www.geeksforgeeks.org/how-to-dynamically-create-new-elements-in-javascript/](https://www.geeksforgeeks.org/how-to-dynamically-create-new-elements-in-javascript/)

借助 `createElement()` 方法，可以在 JavaScript 中动态创建新元素。可以使用 `setAttribute()` 方法设置创建元素的属性。下面给出的例子将演示这种方法。

## 示例 1
在此示例中，新创建的元素作为子元素添加到父元素中。指定要创建的元素的类型，并添加其值或指定元素的文本节点。
...
## 示例 2
在本例中，一个新创建的元素及其属性被添加到页面的主体元素中。只能使用两个参数输入属性，即属性类型和属性值。添加“子”项的 CSS 是为了方便。
