...
> 原文: [https://www.geeksforgeeks.org/explain-spread-operator-in-es6-with-an-example/](https://www.geeksforgeeks.org/explain-spread-operator-in-es6-with-an-example/)
...
遵循语法，我们可以使用任何可迭代对象(如数组)来实现 spread 运算符。
...
在本例中，我们将尝试首先使用 `concat()` 方法执行连接，然后使用更简单的方法，即使用 spread 运算符。
...
在本例中，我们将尝试将一个数组的值复制到另一个数组中，然后我们将尝试向新数组中添加更多的值，然后我们将进一步看到更改对以前和新数组元素的影响。
...
在本例中，我们将尝试使用 spread 运算符来查找元素数组中的最小元素。
