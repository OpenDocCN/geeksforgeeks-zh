...
> 原文: [https://www.geeksforgeeks.org/javascript-check-if-element-exists-in-the-visible-dom/](https://www.geeksforgeeks.org/javascript-check-if-element-exists-in-the-visible-dom/)

任务是找出一个元素是否存在于可见的 DOM 中。为此，我们使用了许多方法，但我们只看其中的几个。

## 示例-1

在此示例中，元素通过`document.getElementById('Id')`进行搜索，并在选择器之前使用`!!`以获得布尔结果。
...
在本例中，元素通过`document.getElementsByClassName('className')`进行搜索，并使用`length`属性来检查变量是否包含结果。
