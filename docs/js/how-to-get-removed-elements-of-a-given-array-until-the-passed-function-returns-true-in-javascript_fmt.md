...
> 原文：[https://www.geeksforgeeks.org/how-to-get-removed-elements-of-a-given-array-until-the-passed-function-returns-true-in-javascript/](https://www.geeksforgeeks.org/how-to-get-removed-elements-of-a-given-array-until-the-passed-function-returns-true-in-javascript/)
...
var retrieveRemoved = function (array, limiter) {
    ...
    for (i = 0; i < array.length; i++) {
        ...
        if (array[i] >= limiter) {
            // It takes the array from 0th index to i, excluding it
...
另一个数组可以用来检查条件。如果它不满足条件，这些是要删除的元素。我们将所有不满足条件的元素推入另一个数组，并返回结果数组。
...
var retrieveRemoved = function (array, limiter) {
    ...
    for (i = 0; i < array.length; i++) {
        if (array[i] < limiter) {
            // Push() method is used to values into the res[].
            res.push(array[i]);
        }
        else {
            s = i;
