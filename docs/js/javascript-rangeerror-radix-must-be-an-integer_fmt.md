...
> 原文: [https://www.geeksforgeeks.org/javascript-rangeerror-radix-must-be-an-integer/](https://www.geeksforgeeks.org/javascript-rangeerror-radix-must-be-an-integer/)
...
当 `Number.prototype.toString()` 或 `BigInt.prototype.toString()` 方法的可选基数参数被指定且不在 2 到 36 之间时，大于 10 的基数开始使用字母表字符作为不能大于 36 的数字（因为字母表只有 26 个字母）。

## 例 1

在本例中，传递的参数是 2，所以没有出现错误。
