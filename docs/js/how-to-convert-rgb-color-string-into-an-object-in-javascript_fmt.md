...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-rgb-color-string-into-an-object-in-javascript/](https://www.geeksforgeeks.org/how-to-convert-rgb-color-string-into-an-object-in-javascript/)
...
*   创建一个名为 `colorArr` 的变量，我们在其中存储输入 `rgb` 的颜色值。例如：`["255", "99", "71", 0.5]`，要实现这一点我们把 `rgb` 从 `(` 所在的地方 slice 到 `)` 出现的地方。现在你得到了字符串 `"255, 99, 71, 0.5"`。现在从 `,` 出现的地方分割数组。现在你得到数组 `["255", "99", "71", "0.5"]`。
*   现在创建一个空对象。
