...
> 原文: [https://www.geeksforgeeks.org/difference-between-var-let-and-const-keywords-in-javascript/](https://www.geeksforgeeks.org/difference-between-var-let-and-const-keywords-in-javascript/)之间的差异
...
**示例 4:** 如果用户在声明前使用 `var` 变量，它将使用 `undefined` 的值进行初始化。输出显示在控制台中。
...
## let 关键字在 JavaScript 中

这个 `let` 关键字是 `var` 关键字的改进版。

**作用域:** **块作用域:** 一个 `let` 变量的作用域只是块作用域。它不能在特定块 (`{block}`) 之外访问。让我们看看下面的例子。
