...
> 原文: [https://www.geeksforgeeks.org/javascript-symbol-split-property/](https://www.geeksforgeeks.org/javascript-symbol-split-property/)
...
接受不可写、不可枚举、不可配置的“字符串”。

## 返回值

返回从给定表达式拆分的字符串。

以下示例说明了 JavaScript 中的 `Symbol.split` 属性:
...
JavaScript `Symbol.split` 属性支持的浏览器如下:
...
[https://devdocs.io/javascript/global_objects/symbol/split](https://devdocs.io/javascript/global_objects/symbol/split)