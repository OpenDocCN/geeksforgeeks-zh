...
> 原文: [https://www.geeksforgeeks.org/check-two-given-strings-are-isomorphic-in-javascript/](https://www.geeksforgeeks.org/check-two-given-strings-are-isomorphic-in-javascript/)
...
要检查字符串是否同构，我们必须注意以下条件:

*   两个字符串的长度应该相等
*   两个字符串的当前字符不应该已经映射到其他字符。

我们将使用一个**散列表**来存储从 `str1` 到 `str2` 的字符之间的映射。我们还将使用**集合**来存储 `str2` 的已映射字符。
