...
> 原文: [https://www.geeksforgeeks.org/javascript-call-a-function-after-a-fixed-time/](https://www.geeksforgeeks.org/javascript-call-a-function-after-a-fixed-time/)
...
*   `fun`: 是必选参数。它保存要执行的函数。
*   `msec`: 是必需参数。多长时间后执行代码的时间间隔(毫秒)。如果该值小于 10，则使用 10。
*   `p1, p2, …`: 为可选参数。参数作为参数传递给函数。(IE9 及更早版本不支持)。

**`clearInterval()`方法:** 该方法清除由`setInterval()`方法设置的定时器。`setInterval()`方法返回的标识值用作该方法的参数。
...
**示例 1:** 本示例设置一个函数，该函数在 2 秒钟后将一个`<div>`元素连续追加到`<body>`中。
...
**示例 2:** 本示例设置一个函数，该函数在 2 秒钟后连续向`<body>`追加一个`<div>`元素，并在点击**停止**按钮时停止追加。
...
**示例 3:** 本示例设置一个函数，该函数在 1 秒钟后以与前一个不同的方式将`<div>`元素连续追加到`<body>`中，并在点击**停止**按钮时停止追加。
