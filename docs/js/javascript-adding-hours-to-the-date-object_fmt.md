...
> 原文: [https://www.geeksforgeeks.org/javascript-adding-hours-to-the-date-object/](https://www.geeksforgeeks.org/javascript-adding-hours-to-the-date-object/)
...
*   `hour`: 此参数为必填项。它指定表示小时的整数。预期值为 0-23，但也允许其他值。
*   `min`: 此参数为可选。它指定表示分钟的整数。预期值为 0-59，但也允许其他值。
*   `sec`: 此参数可选。它指定表示秒的整数。预期值为 0-59，但也允许其他值。
*   `millisec`: 此参数为可选。它指定表示毫秒的整数。预期值为 0-999，但也允许其他值。
...
本例通过使用 `setTime()` 和 `getTime()` 方法，将 **16 日** 增加 4 小时。
...
<head>
    <title>
        JavaScript | Adding hours to Date object.
    </title>
</head>
...
本例通过使用 `setHours()` 和 `getHours()` 方法，将 **16 日** 增加 6 小时。
