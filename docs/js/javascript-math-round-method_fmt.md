...
`Math.round()` 方法返回舍入到最接近整数的给定数字的值。

上述方法的更多代码如下：

### 程序 1

当作为参数传递给 `Math.round()` 方法时，该方法本身会舍入一个负数。要将负数舍入到其最接近的整数，应该以下列方式实现 `Math.round()` 方法：
...
下面的程序显示了当参数有小数点“.5”时，`Math.round()` 方法的结果。
