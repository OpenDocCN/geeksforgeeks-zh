...
> 原文: [https://www.geeksforgeeks.org/javascript-date-todatestring-method/](https://www.geeksforgeeks.org/javascript-date-todatestring-method/)

下面是 `Date.toDateString()` 方法的例子。
...
`Date.toDateString()` 方法用于将给定日期对象的日期部分内容转换为字符串。日期对象是使用 `Date()` 构造函数创建的。
...
此方法不接受任何参数。它只是与使用 `Date()` 构造函数创建的 `Date` 对象一起使用。

## 返回值

返回转换后的 `Date()` 构造函数内容的日期部分的字符串。

## 注意

`dateObj` 是使用 `Date()` 构造函数创建的有效日期对象，其日期部分的内容被转换为字符串。

以上方法的更多代码如下:

## 程序 1

这里在创建日期对象时没有传递任何参数，但是 `toDateString()` 方法返回当前的日名、月名、日期、年。
...
当一些随机的值列表被传递时，`toDateString()` 方法返回相应的生成字符串。
...
月、日期、小时、分钟、秒和毫秒应该在它们各自的范围内，月为 0 到 11，日期为 1 到 31，小时为 0 到 23，分钟为 0 到 59，秒为 0 到 59，毫秒为 0 到 999，否则 `toDateString()` 方法返回无效日期。
...
JavaScript `Date.toDateString()` 方法支持的浏览器如下:
