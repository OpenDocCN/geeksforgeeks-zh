...
> 原文: [https://www.geeksforgeeks.org/javascript-date-setminutes-method/](https://www.geeksforgeeks.org/javascript-date-setminutes-method/)
...
*   **Minutes_Value:** 此参数保存分钟值，用于在 `Date()` 构造函数中设置。

## 返回值:

返回用 `setMinutes()` 方法设置的更新分钟的新日期。

## 注意:

`DateObj` 是使用 `Date()` 构造函数创建的有效日期对象，我们希望在其中设置分钟。分钟的值从 0 到 59。

上述方法的更多代码如下:

### 程序 1:

如果在 `Date()` 构造函数中，我们在创建 `Date` 对象时没有给出任何分钟，`setMinutes()` 方法仍然设置新的分钟，该分钟作为其参数给出。
...
如果 `Date()` 构造函数中没有给定参数，`setMinutes()` 方法仍然设置分钟，但是月、年、日等变成当前的。这里 42 是新的分钟数，3 是当月即 4 月，1 是当前日期，2018 是当前年份。
...
如果在 `setMinutes()` 函数的参数中给定的分钟值是 66，它将设置 6 为分钟，因为分钟范围是从 0 到 59 和 (66 % 60 = 6)。
...
JavaScript Date `setMinutes()` 方法支持的浏览器如下:
