...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-custom-callback-in-javascript/](https://www.geeksforgeeks.org/how-to-create-a-custom-callback-in-javascript/)
...
使用`callback`关键字作为最后一个参数，可以创建自定义回调函数。然后可以通过在函数末尾调用`callback()`函数来调用它。`typeof`运算符可用于检查传递的参数是否实际上是一个函数。
