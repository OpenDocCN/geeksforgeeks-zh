...
> 原文: [https://www.geeksforgeeks.org/how-to-remove-duplicate-elements-from-array-in-javascript/](https://www.geeksforgeeks.org/how-to-remove-duplicate-elements-from-array-in-javascript/)

有各种不同的方法来查找数组中的副本。我们将讨论在数组中查找重复项的两种方法。

## 使用 Set

`Set` 对象允许您存储任何类型的唯一值，无论是基元值还是对象引用。这是移除重复元素并从数组中获取唯一元素的最简单方法。
...
我们使用 JavaScript `includes()` 函数，如果一个元素在数组中，该函数返回 `true`，如果它不存在，则返回 `false`。
