...
这里参数“对象”是可迭代对象。可迭代对象的所有元素都被添加到 `WeakSet` 中。

## 一些不同的 `WeakSet` 函数:

| 方法 | 描述 |
| --- | --- |
| `add(value)` | 一个新的对象被附加给 `weakset` 的给定值。`WeakSet_Object.add(value)` |
| `delete(value)` | 从 `WeakSet` 集合中删除该值。`WeakSet_Object.delete(value)` |
| `has(value)` | 如果该值存在于 `WeakSet` 集合中，则返回 `true`，否则返回 `false`。`WeakSet_Object.has(value)` |
| `length()` | 返回 `weakSetObject` 的长度。`WeakSet_Object.length()` |
