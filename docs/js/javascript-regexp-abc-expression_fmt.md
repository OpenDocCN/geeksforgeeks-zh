...
> 原文: [https://www.geeksforgeeks.org/javascript-regexp-abc-expression/](https://www.geeksforgeeks.org/javascript-regexp-abc-expression/)

JavaScript 中的正则表达式 `[abc]` 表达式用于搜索括号之间的任何字符。括号内的字符可以是单个字符或一系列字符。
...
## 示例 1:
本示例在整个字符串中搜索 `[A-G]` 之间的字符，即大写的 A 到大写的 G。
...
## 示例 2:
本示例搜索整个字符串中 `[a-g]` 之间的字符，即小写 a 到小写 g。
