...
## 方法 1: 使用 jQuery 的 `ajax()` 方法

使用 jQuery 的 `ajax()` 方法检查给定网址上是否存在文件。`ajax()` 方法用于触发异步 HTTP 请求。如果文件存在，`ajax()` 方法会依次调用 `ajaxSuccess()` 方法，否则会调用 **Error 函数**。
...
## 方法 2: 使用 `XMLHttpRequest()` 方法

使用 `XMLHttpRequest()` 方法触发 ajax 请求。如果 HTTP 状态为 200，则文件存在，否则文件不存在。
