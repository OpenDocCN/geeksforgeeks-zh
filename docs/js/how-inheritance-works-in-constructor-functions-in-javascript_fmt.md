...
> 原文：`https://www.geeksforgeeks.org/how-inheritance-works-in-constructor-functions-in-javascript/`

这里，我们将讨论在 JavaScript 中继承构造函数。构造函数定义了对象将包含的属性的原型。使用构造函数，我们可以在传递所需的参数后创建一个新的对象。

继承以前定义的构造函数意味着使用以前定义的函数的参数，同时向新定义的构造函数添加一些新参数。为此，我们需要使用 `call()` 函数，该函数允许我们调用在当前上下文中其他地方定义的函数。
...
*   `myFunction`：这是我们想要在新构造函数中继承参数的构造函数。
*   `this`：关键字 `this` 的参数值将在调用 `myFunction` 时使用。
*   `property1, property2, …, propertyN`：要在新构造函数中继承的参数。

## 返回类型

构造函数或继承了其属性的函数没有任何返回类型。它指定了对象将包含的属性的**原型**，这些属性是从该构造函数创建的。

## 示例

这里，我们创建一个 `Employee` 构造函数。创建了一个新的 `Developer` 构造函数，它将继承 `Employee` 的基本属性，并包含一些新属性。
...
我们可以注意到 `Developer` 构造函数继承了 `Employee` 构造函数的属性以及一个新参数 `specialization`。这里，我们使用 `call()` 函数调用了 `Employee` 函数，将所需的参数传递给 `Employee` 构造函数。
...
本示例描述了使用 `new` 关键字创建的对象，以创建具有构造函数的对象实例。
