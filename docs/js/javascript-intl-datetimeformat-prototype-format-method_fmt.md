...
> 原文：[https://www.geeksforgeeks.org/javascript-intl-datetimeformat-prototype-format-method/](https://www.geeksforgeeks.org/javascript-intl-datetimeformat-prototype-format-method/)

`Intl.DateTimeFormat.prototype.format()` 方法是 JavaScript 中的一个内置方法，用于根据此 `Intl.DateTimeFormat` 对象的区域设置和格式选项来格式化日期。
...
下面的例子说明了 `Intl.DateTimeFormat.prototype.format()` 方法在 JavaScript 中的使用：

## 示例 1

```javascript
const Geeks = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
