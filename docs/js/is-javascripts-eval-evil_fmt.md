...
使用 `eval()` 有几个问题，其中性能和代码注入被认为是最有问题的。
- **性能**：由于脚本编译器不能预编译 `eval()`，所以即使在运行时编译代码，它也会运行编译器。虽然不多，但这仍然会降低性能。
- **代码注入**：`eval()` 将字符串作为代码运行，黑客只需使用 `eval()` 就可以更容易地获取特权信息。当程序接受用户输入并在客户端运行时，这是一个很大的安全威胁。这反过来可以继续操纵原始程序本身。
- **更难调试**：虽然调试它需要双重工作。
