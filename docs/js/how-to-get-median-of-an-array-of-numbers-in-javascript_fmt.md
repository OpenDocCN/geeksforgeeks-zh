...
> 原文：[https://www.geeksforgeeks.org/how-to-get-median-of-an-array-of-numbers-in-javascript/](https://www.geeksforgeeks.org/how-to-get-median-of-an-array-of-numbers-in-javascript/)
...
这里我们先对数组进行排序，然后会求出数组长度。如果数组长度是偶数，那么中值将是 `arr[(arr . length)/2]+arr[((arr . length)/2)+1]`。如果数组长度是奇数，那么中间值将是一个中间元素。
...
这里我们先做了变量 `middle`，不管数组长度是奇数还是偶数，都有中间值。之后，我们通过避免突变对数组进行排序。突变意味着用另一个对象名改变对象名，或者将对象传递给另一个对象，称为突变。

可以用 `引用数据` 类型来完成，这些类型是 `数组`、`对象`。所以现在避免这个。之后，如果数组的长度是偶数，那么我们在数组中的位置 `arr` 处有两个值 `((arr . length)/2)+arr((arr . length)/2)+1)`。然后取这两个数的平均值，作为中位数返回。
