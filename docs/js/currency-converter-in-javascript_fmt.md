...
> 原文：[https://www.geeksforgeeks.org/currency-converter-in-javascript/](https://www.geeksforgeeks.org/currency-converter-in-javascript/)
...
*   选择币种，`折算`按钮显示折算金额。
*   `复位`按钮复位数据。
*   像 `addEventListener()` 一样，使用 JavaScript 函数和自定义方法实现货币转换。
*   脚本文件中使用了货币兑换 API。

## HTML 代码

下面的 HTML 代码实现了用户输入的图形用户界面，如金额和需要进行转换的货币。
...
以下是上述 HTML 文件中使用的文件 `style.css` 的内容。
...
以下是上述 HTML 代码中使用的文件 `script.js` 的内容。
