...
> 原文: [https://www.geeksforgeeks.org/how-to-run-a-given-array-of-promises-in-series-in-javascript/](https://www.geeksforgeeks.org/how-to-run-a-given-array-of-promises-in-series-in-javascript/)

给定一系列承诺，我们必须在一系列中运行。要完成这个任务，我们可以使用`then()`，在完成一个承诺后运行下一个承诺。

## 方法

`then()` 方法返回一个 Promise，帮助我们连锁承诺/方法。 `Promise.resolve()` 方法执行第一次回调，当该承诺实现后，传递到下一个函数 `callback1`，继续进行，直到所有承诺都实现。这样，我们就可以连续运行所有的承诺。
...
在本例中，我们将通过用 `then()` 方法链接三个承诺来执行它们。
...
如果数组中有更多的承诺，上述方法是不可行的。函数的链接会变得很累，并且会使代码很长。我们可以使用 `forEach()` 数组函数来执行承诺，方法是将结果存储在一个变量中，并在每次承诺时更新该变量。这将自动通过所有的承诺，并可以防止重复写 `then()` 声明。

## 示例 2

在本例中，我们将使用 `forEach()` 方法执行多个承诺。
