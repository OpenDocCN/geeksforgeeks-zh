...
> 原文：[https://www.geeksforgeeks.org/how-to-detect-adblocker-using-javascript/](https://www.geeksforgeeks.org/how-to-detect-adblocker-using-javascript/)
...
在这个方法中，你必须创建一个名为 `ad-zone` 的 `div`。使 `div` 为 1px 高度。现在写一个脚本，检查父级 `div` 的偏移 `offsetHeight` 是否为零，那么用户正在使用一个广告拦截器。adblocker 阻止了 `div` 渲染，因此偏移高度变为零。如果 `div` 的偏移高度保持不变，则浏览器中不存在 adblocker。原因是类名 `ad-zone` 是 ad blocker 用来检查哪个元素的类名看起来像 ad。

`easylist.text` 中有大量这样的类名。每个 adblocker 都有自己的一组将要被阻塞的类名，这就是为什么我们不应该只依赖一个类名。一些常见的类名是 `text-ads`、`banner-ads`、`banner_ads`、`ad-unit`、`ad-zone`、`ad-space`，它们被广告拦截器检测到用于广告。
...
您可以将此代码粘贴到任何浏览器控制台中，也可以在 `<script>` 标签中使用。
