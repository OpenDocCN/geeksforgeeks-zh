...
## 方法一：用 `replaceState()` 替换当前状态方法

浏览器的历史界面管理浏览器会话历史。它包括当前页面所在的选项卡或框架中所访问的页面。操作此状态可用于更改浏览器的网址，而无需重新加载页面。
...
## 方法二：用 `pushState()` 添加新状态方法

`pushState()` 方法用于添加新的历史条目，属性作为参数传递。这将在不重新加载页面的情况下，将当前网址更改为给定的新状态。
