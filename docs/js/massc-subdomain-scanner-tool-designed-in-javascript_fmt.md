...
> 原文: [https://www.geeksforgeeks.org/massc-subdomain-scanner-tool-designed-in-javascript/](https://www.geeksforgeeks.org/massc-subdomain-scanner-tool-designed-in-javascript/)
...
## 在 Kali Linux 操作系统上安装 Massc 工具

确保您已经在系统上安装了 Node.js，因为这是一个基于 JavaScript 的工具。关于 node.js 的安装，查看 [`在 Linux 上安装 node . js`](https://www.geeksforgeeks.org/installation-of-node-js-on-linux/)的文章。
...
### 步骤 2
打开你的 Kali Linux 终端，使用以下命令移动到桌面。
...
### 步骤 3
你在桌面上，现在使用以下命令创建一个名为 Massc 的新目录。在这个目录中，我们将安装 Massc 工具。
...
### 步骤 6
工具已在 Massc 目录下成功下载。现在使用下面的命令列出工具的内容。
...
### 步骤 7
您可以观察到，在我们安装 Massc 工具时，已经生成了该工具的新目录。现在使用下面的命令移动到该目录:
