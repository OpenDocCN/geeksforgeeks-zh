...
> 原文：[https://www.geeksforgeeks.org/how-to-find-the-longest-word-within-the-string-in-javascript/](https://www.geeksforgeeks.org/how-to-find-the-longest-word-within-the-string-in-javascript/)
...
## 方法 1：使用正则表达式和 for…循环

在这种方法中，我们使用正则表达式通过使用正则表达式`/[a-zA-Z0-9]+/gi`将字符串分割成一个单词数组，然后使用 `for` 循环迭代该数组并搜索最大的字符串。
...
## 方法 2：使用 `split()` 和 `sort()` 方法

在这种方法中，我们使用 `String.split()` 方法分割字符串，并使用 `array.sort()` 方法对数组进行排序。
...
## 方法 3：采用 `split()` 和 `reduce()` 的方法

在这种方法中，我们使用 `String.split()` 方法分割字符串，并使用 `reduce` 方法搜索数组中最大的元素，这是您最大的字符串。
