...
1.  发出警报，使用 `alert()` 功能完成任务。
2.  获得事件或输入的确认——`confirm()` 功能用于完成任务。
3.  从用户那里得到一个输入——`prompt()` 功能用来完成任务。

## 报警对话框

报警框用于向用户发出警告信息。警告框只有一个按钮“确定”来选择并继续下一个任务。例如，当某些输入字段是必需的，并且用户没有给该输入字段赋值时，可以使用警告框显示一个弹出窗口。
...
一个确认对话框用于接受用户的确认选项。它会显示一个包含两个按钮的对话框：确定和取消。例如，如果用户删除了一些数据，页面可以使用确认框进行确认，就好像该数据真的要被删除一样。如果用户单击“确定”按钮，该方法返回真值，而如果用户单击“取消”按钮，则 `confirm()` 方法返回假值。
...
弹出文本框获取用户输入时使用提示对话框。用户需要在文本框中输入，然后单击确定。该对话框有两个按钮：确定和取消。如果用户单击“确定”按钮，`prompt()` 会读取并返回用户输入的值。如果用户单击“取消”按钮，`prompt()` 将返回空值。
...
*   **示例：** 以下代码演示了提示对话框。这里，`message` 是要在文本框中显示的文本，而 `defstring` 是要在文本框中显示的默认字符串。
