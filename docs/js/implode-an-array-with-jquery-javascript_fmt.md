...
> 原文：[https://www.geeksforgeeks.org/implode-an-array-with-jquery-javascript/](https://www.geeksforgeeks.org/implode-an-array-with-jquery-javascript/)

给定一个元素数组，任务是内爆（连接数组的元素）数组元素。

## 方法 1：使用 `join()` 方法

`join()` 方法可用于连接带有分隔符的数组，并将其作为字符串返回。`join()` 方法接受分隔符的可选参数。如果未指定分隔符，则假定为逗号。如果数组只包含一个字符串，那么它将不带分隔符返回。
...
## 方法 2：遍历数组并创建一个包含所有字符串和连接分隔符的新字符串

通过遍历数组并将其连接到一个基本字符串，可以创建内爆函数。分隔符是在连接每个字符串后添加的，除非它是数组中的最后一个字符串。这可防止将分隔符添加到数组中最后一个字符串的末尾。
