...
> 原文：[https://www.geeksforgeeks.org/how-to-find-an-average-color-of-an-image-using-javascript/](https://www.geeksforgeeks.org/how-to-find-an-average-color-of-an-image-using-javascript/)

通过在画布元素上绘制图像，可以用 JavaScript 找到图像的平均颜色。要获得图像的平均颜色，必须执行以下步骤：

## 1. 在画布上绘制图像

首先使用画布 2D 应用编程接口的方法 `drawImage()` 在画布上绘制图像。该方法将图像和尺寸作为参数，并将其绘制到画布上。
...
使用 `getImageData()` 方法返回画布的图像数据。它返回一个 `ImageData` 对象，表示画布指定部分的像素数据。
...
然后，通过将所有颜色值分别相加并找到该颜色的平均值，用该图像数据计算平均红色、绿色和蓝色。
