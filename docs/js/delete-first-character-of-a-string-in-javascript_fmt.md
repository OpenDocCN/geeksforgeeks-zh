...
> 原文：[https://www.geeksforgeeks.org/delete-first-character-of-a-string-in-javascript/](https://www.geeksforgeeks.org/delete-first-character-of-a-string-in-javascript/)

在 JavaScript 中有很多方法可以删除字符串的第一个字符，下面讨论其中的一些：

## 方法 1：使用 `slice()` 方法

`slice()` 方法提取字符串的一部分，并将提取的部分返回到新字符串中。如果我们想删除字符串的第一个字符，那么可以通过指定需要从中提取字符串的起始索引来完成。我们还可以对最后一个元素进行切片。
...
`string.substring()` 是 JavaScript 中的一个内置函数，用于返回给定字符串从起始索引到结束索引的部分。索引从零（0）开始。
...
`string.replace()` 函数是 JavaScript 中的一个内置函数，用于用其他字符串或正则表达式替换给定字符串的一部分。原始字符串将保持不变。
