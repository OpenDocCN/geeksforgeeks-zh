...
> 原文：`https://www.geeksforgeeks.org/how-to-get-a-number-of-vowels-in-a-string-in-javascript/`
...
这里我们创建一个用户定义的函数，叫做 `getVowels()`，它读取一个字符串，并与元音列表进行比较。它将字符串的每个字符与元音进行比较。当元音匹配时，它将增加 `vowelsCount` 的值。
