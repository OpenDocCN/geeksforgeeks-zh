...
> 原文: [https://www.geeksforgeeks.org/javascript-string-includes-method/](https://www.geeksforgeeks.org/javascript-string-includes-method/)
...
*   `searchvalue`: 是进行搜索的字符串。
*   `start`: 这是搜索将被处理的位置（虽然如果没有提到，这个参数不是必需的，搜索将从字符串的开始开始）。
...
说明: 由于没有定义第二个参数，搜索将从起始索引开始。它将搜索 "Geeks"，因为它存在于字符串中，它将返回真。

```
Input: Welcome to GeeksforGeeks.
       str.includes("GeeksforGeeks.");

Output: false
```

**说明:** 即使在这种情况下也没有定义第二个参数，因此搜索将从起始索引开始。但是由于这个方法区分大小写，所以它会对两个字符串进行不同的处理，从而返回一个布尔值 `false`。因为区分大小写。
