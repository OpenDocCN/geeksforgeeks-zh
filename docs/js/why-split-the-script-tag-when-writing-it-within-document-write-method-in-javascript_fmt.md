...
## Note
This procedure have been used to include jQuery files only.

Below examples implements the above approach:

## Example 1
In this example, we will use `<script>` inside the `document.write` without splitting the tag.
...
## Example 2
In this example we will use `<script>` inside the `document.write` by splitting the tag.
