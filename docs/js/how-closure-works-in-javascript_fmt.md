...
> 原文: [https://www.geeksforgeeks.org/how-closure-works-in-javascript/](https://www.geeksforgeeks.org/how-closure-works-in-javascript/)
...
在这个例子中，我们将声明一个闭包，它将最终从外部函数访问外部变量`balance`。在内部函数中使用外部变量后，每次调用外部函数时，特定的闭包将帮助我们从中扣除 100。
...
闭包可以像下面的示例一样嵌套。在本例中，外部函数`outerfunction()`和内部函数`innerfunction()`都可以访问计数器变量，因此在调用`Counter()`时，外部函数`outerfunction()`和内部函数`innerfunction()`都会增加计数器的值。在这种情况下，我们可以说闭包可以访问所有的外部函数范围。
