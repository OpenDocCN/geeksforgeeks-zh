...
`DOM` 代表 **D**ocument **O**bject **M**odel。网页在浏览器中呈现。浏览器将网页中包含的所有元素从 web 服务器下载到内存中。完成后，浏览器将在浏览器窗口中显示这些对象，一旦完成，浏览器将无法再识别单个的 HTML 元素。启用 JavaScript 的浏览器能够在网页在浏览器中呈现后识别网页中的单个对象，因为启用 JavaScript 的浏览器能够识别并使用 `DOM`。因此，允许随意控制对象的功能。
...
*   **alinkColor:** 该属性定义激活链接的颜色。
    ...
    ```
*   **anchors[]:** 它是每个锚点对象的数组，当它出现在文档中时，每个锚点一个。
    ```
    Eg. document.anchors[0], document.anchors[1], ...
    ```
*   **applets[]:** 它是小程序对象的数组，当小程序出现在文档中时，每个小程序对应一个对象。
    ```
...
*   **cookie:** 该属性定义了具有特殊行为的值属性，允许设置与要查询的文档相关联的 Cookie。
    ...
    Eg. document.cookie
    ```
*   **domain:** 该属性定义了文档所属的域已经被用于安全目的。
    ```
    Eg. document.domain
    ```
*   **embeds[]:** `plugins[]` 的同义词。它是表示嵌入在文档中的数据的对象数组。
    ```
...
*   **images[]:** 它是表单对象的数组，每个元素对应一个表单中出现的带有 `<img>` 标签的元素。
    ...
    ```
*   **lastModified:** 该属性定义最近更新的日期。
    ```
    Eg. document.lastModified
    ```
*   **linkColor:** 该属性定义未访问链接的颜色，与 `vlinkColor` 相反。
    ```
...
*   **location:** 该属性保存文档的 URL。
    ...
    ```
*   **plugins[]:** 该属性是 `embeds[]` 的同义词。
    ```
    Eg. document.plugins[0], document.plugins[1], ...
    ```
*   **referrer:** 包含文档网址的字符串，如果它与任何链接。
    ```
...
*   **clear():** 清除文档内容，不返回任何内容。
    ```
    Eg. document.clear()
    ```
*   **close():** 关闭打开的 `document()`。
    ```
    Eg. document.close()
    ```
...
*   **defaultView:** 表示显示文档的窗口。
    ...
    ```
*   **documentElement:** 对文档 `<html>` 标签的引用。
    ```
    Eg. document.documentElement
    ```
*   **implementation:** 表示 `DOM` implementation 对象，该对象表示创建此文档的实现。
    ```
...
*   **createAttribute(name_of_attr):** 返回新创建的具有指定名称的 `Attr` 节点。
    ```
    Eg. document.createAttribute(name_of_attr)
    ```
*   **createComment(text):** 创建并返回包含指定文本的新注释节点。
    ```
...
*   **createDocumentFragment():** 创建并返回一个空的 `DocumentFragment` 节点。
    ...
    ```
*   **createElement(tagname_of_new_ele):** 用指定的标记名创建并返回一个新的元素节点。
    ```
    Eg. document.createElement(tagname_of_new_ele)
    ```
*   **createTextNode(text):** 创建并返回包含指定文本的新文本节点。
    ```
...
*   **charset:** 文档的字符集。
    ...
    Eg. document.charset
    ```
*   **children[]:** 数组，包含作为文档直接子级的 HTML 元素。
    ```
