...
| 量词 | 描述 |
| --- | --- |
| `pattern+` | 匹配具有至少一个或多个模式的字符串。 |
| `pattern*` | 匹配零个或多个模式的字符串。 |
| `pattern{n}` | 将字符串与“n”个模式匹配。 |
| `pattern{n1，n2}` | 匹配模式范围从 n1 到 n2(包括 n1 和 N2)的字符串。 |
| `pattern{n，}` | 匹配具有最小“n”个模式的字符串。 |
| `pattern$` | 匹配以模式作为结束序列的字符串。 |
| `pattern` | 匹配以模式作为开始序列的字符串。 |
...
**注意:** `RegExp` 本身没有方法和属性。但是它继承了原型的一些功能和属性。