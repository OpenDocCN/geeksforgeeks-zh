...
> 原文：[https://www.geeksforgeeks.org/how-to-check-whether-a-radio-button-is-selected-with-javascript/](https://www.geeksforgeeks.org/how-to-check-whether-a-radio-button-is-selected-with-javascript/)

给定一个包含单选按钮元素的表单，任务是在 JavaScript 的帮助下检查单选按钮是否被选中。有两种方法可以解决这个问题，讨论如下：

## 使用输入单选按钮 `checked` 属性

输入单选按钮 `checked` 属性用于返回输入单选按钮的选中状态。
使用 `document.getElementById().checked` 方法检查所选 `id` 的元素是否勾选。如果选中，则显示其相应的结果，否则检查下一条语句。如果没有选择任何单选按钮，则返回“没有选择任何人”。
...
## 使用 DOM `querySelector()` 方法

`querySelector()` 方法用于返回与文档中指定的 CSS 选择器匹配的第一个元素。
