...
> 原文: [https://www.geeksforgeeks.org/how-to-record-and-play-audio-in-javascript/](https://www.geeksforgeeks.org/how-to-record-and-play-audio-in-javascript/)
...
整个事情都是用一个 API 发生的，就是 `MediaRecorder` API。该应用编程接口提供记录媒体(如音频或视频)的功能。它是使用 `MediaRecorder()` 构造器创建的。

## 访问媒体设备

这里我们使用 JavaScript 的一个属性，即 `mediaDevices` 属性，用于访问麦克风、网络摄像头等连接的输入媒体设备。该属性使用方法 `getUserMedia()` 获取访问麦克风、网络摄像头等的权限。该方法向 `navigator` 对象返回一个 `Promise`。
...
*   `NotAllowedError`: 如果用户拒绝录制许可。
*   `NotFoundError`: 如果没有媒体轨道。
*   `NotReadableError`: 如果没有找到输入设备或者硬件不工作。
*   `OverconstrainedError`: 如果约束音频设置阻止。
*   `AbortError`: 如果发现一般的不明原因。
*   `TypeError`: 如果 `audio: false` 在 Javascript 代码中声明。
