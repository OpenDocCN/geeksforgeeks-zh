...
## HTML 文档包含 JavaScript

使用 HTML 标记将 JavaScript 文档包含在我们的 HTML 页面中。`<script>`标签用于指定外部 JavaScript 文件的来源。
...
### 示例 1: 了解如何在 DHTML 中使用 JavaScript
...
**解释:** 在上面的例子中，使用 `id` 更改段落的文本。`document` 是显示在当前窗口或 DOM 对象中的 HTML 对象。`getElementById(Id)` 给出了元素 `id`。`innerHTML` 定义了 `id` 元素中的内容。`id` 属性用于更改 HTML 文档及其属性。`document.getElementById("geeks").style.color = "blue"` 用于使用元素 `id` 设置段落颜色。
...
**解释:** 在本例中，创建一个将在点击按钮时被调用的函数 `Click()`，它改变文本的颜色并在屏幕上显示警报。`window` 是当前窗口的一个对象，它的内置方法 `alert()` 在 `Click()` 函数中被调用。
...
**说明:** 在本例中，创建一个 `Validate()` 函数，用于验证用户输入。如果用户输入的百分比大于 60，则选择，否则不选择。