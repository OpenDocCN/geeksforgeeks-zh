...
> 原文：[https://www.geeksforgeeks.org/why-javascript-is-a-single-thread-language-that-can-be-non-blocking/](https://www.geeksforgeeks.org/why-javascript-is-a-single-thread-language-that-can-be-non-blocking/)
...
### 内存堆
用于分配你的 `JavaScript` 程序使用的内存。记住内存堆和堆数据结构不一样，它们是完全不同的。这是你的操作系统内部的自由空间。

### 调用栈
在调用栈中，您的 `JS` 代码被一行行地读取和执行。
