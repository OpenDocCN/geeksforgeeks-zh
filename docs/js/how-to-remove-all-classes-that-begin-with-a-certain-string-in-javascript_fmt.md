...
> 原文：[https://www.geeksforgeeks.org/how-to-remove-all-classes-that-begin-with-a-certain-string-in-javascript/](https://www.geeksforgeeks.org/how-to-remove-all-classes-that-begin-with-a-certain-string-in-javascript/)
...
1.  选择特定元素。
2.  使用`.className`属性可以访问所有的类。
3.  使用`.replace()`方法用空格替换所有特定的类（这意味着类从元素中移除）。
4.  在本例中，正则表达式用于替换。
...
1.  选择特定元素。
2.  使用`.className`属性获取对所有类的访问。
3.  使用`.split()`方法获取所有类作为一个元素。
4.  使用`.filter()`函数过滤掉不以某个字符开头的类。
5.  最后，将这些类与元素放在一起。
