...
> 原文：[https://www.geeksforgeeks.org/how-to-dynamically-change-the-title-of-web-page-using-javascript/](https://www.geeksforgeeks.org/how-to-dynamically-change-the-title-of-web-page-using-javascript/)

给定一个包含页面标题的网页，任务是使用 JavaScript 动态更改网页的标题。

## 方法 1：使用 `document.title` 属性

`document.title` 属性用于设置或返回文档的当前标题。可以通过将新标题作为字符串分配给此属性来更改页面标题。这将把网站的标题改为首选标题。
...
## 方法 2：使用 `querySelector()` 方法

使用 `document.querySelector()` 方法选择文档中的元素。可以通过在选择器中将标题元素指定为参数来选择标题元素。这将返回页面的当前标题元素。
