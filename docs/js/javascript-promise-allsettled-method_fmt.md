...
> 原文: [https://www.geeksforgeeks.org/javascript-promise-allsettled-method/](https://www.geeksforgeeks.org/javascript-promise-allsettled-method/)

JavaScript 中的 [Promise](https://www.geeksforgeeks.org/javascript-promises/) 可以处于待定、履行或拒绝三种状态。Javascript 中的 `Promise.allSettled()` 方法用于在所有输入都已完成或被拒绝时获得一个承诺。
...
该方法接受单个参数 `iterable`，该参数取一个承诺的数组或一个包含一些对象的普通数组。

## 返回值

该方法返回以下值:
*   如果传递的参数为空，则返回一个已经解析的承诺。
*   对于所有其他情况，它返回一个待定承诺。
