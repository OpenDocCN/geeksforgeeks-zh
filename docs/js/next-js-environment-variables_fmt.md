...
> 原文：[https://www.geeksforgeeks.org/next-js-environment-variables/](https://www.geeksforgeeks.org/next-js-environment-variables/)
...
我们将创建一个 `.env.local` 文件来创建特定于我们本地机器的环境变量。您还可以使用其他环境变量文件类型，例如：
...
现在，您可以使用 `process.env.VARIABLE_NAME` 访问您的环境变量。为了尝试这一点，让我们从一个 NextJs 应用程序的主页访问我们的环境变量。
