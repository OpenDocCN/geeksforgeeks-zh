...
> 原文：[`https://www.geeksforgeeks.org/merge-sort-visualization-in-javascript/`](https://www.geeksforgeeks.org/merge-sort-visualization-in-javascript/)

GUI（图形用户界面）帮助用户更好地理解程序。在本文中，我们将使用 JavaScript 可视化合并排序。我们将看到数组在排序后如何被划分和合并，以获得最终的排序数组。
...
*   首先，我们将使用 [`Math.random()`](https://www.geeksforgeeks.org/javascript-math-random-method/) 功能生成一个随机数组。
*   [画布](https://www.geeksforgeeks.org/html-canvas-basics/)的属性用于制作矩形条和动画。
*   不同的颜色用于指示哪些数组被划分和合并。
*   使用 JavaScript `mergeSort()` 函数执行排序。
*   算法执行操作非常快，`timeout()` 功能已用于减慢进程。
...
下面是可视化[合并排序算法的程序。](https://www.geeksforgeeks.org/merge-sort/)
...
以下是上述 HTML 代码中使用的 `mergeSort.js` 文件的内容。
