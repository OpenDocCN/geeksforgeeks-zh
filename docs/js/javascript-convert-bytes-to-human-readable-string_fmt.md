...
> 原文: [https://www.geeksforgeeks.org/javascript-convert-bytes-to-human-readable-string/](https://www.geeksforgeeks.org/javascript-convert-bytes-to-human-readable-string/)

给定文件的大小（以字节为单位），任务是使用 JavaScript 将其转换为人类可读的形式。这里讨论几个方法。

## 示例 1

本示例将文件大小（以字节为单位）转换为人类可读的形式。它以十进制显示值，对于小于 1024 字节的值，它仍以字节为单位。
...
<head>
    <title>
        JavaScript | Converting bytes to human-readable string.
    </title>
</head>
...
本示例将文件大小（以字节为单位）转换为人类可读的形式。它以十进制显示值，对于小于 1024 字节的值，它仍以字节为单位。但是，用不同的方法。
