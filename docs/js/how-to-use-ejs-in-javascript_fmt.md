...
**注意：** 上面命令中的 `npm` 代表节点包管理器，一个安装所有依赖项的地方。`–save` 标志在 Node 5.0.0 版本后不再需要，因为我们现在安装的所有模块都将被添加到依赖项中。
...
现在要访问 `EJS` 文件或 `.js` 文件，您所需要做的就是将数据传递到另一个变量中，如下所示：
...
1.  创建完所有文件后，转到项目文件夹的根目录。
2.  在此目录中运行命令提示符。
3.  键入以下命令运行您的程序，并查看显示的输出。
