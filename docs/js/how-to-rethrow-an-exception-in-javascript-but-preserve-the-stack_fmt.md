...
> 原文: [https://www.geeksforgeeks.org/how-to-rethrow-an-exception-in-javascript-but-preserve-the-stack/](https://www.geeksforgeeks.org/how-to-rethrow-an-exception-in-javascript-but-preserve-the-stack/)
...
在一次班级测试中，拉德哈应该回答老师提出的问题。老师给了拉德哈一个数字，并问她给定的数字是否是有效的日数，如果是，则指定相应的日。如果给定的日数无效，拉德哈应该回答“无效日数”。如果我们运行代码，嵌入的页面将显示堆栈跟踪。它将显示错误起源于 `day()`，然后在 `month()` 被重新抛出，最后在 `year()` 被捕获。由于 8 超出界限，因此输出显示为无效日期。
...
数字 5 是有效的日数，因此不会抛出错误。因此，我们得到的输出是有效的一天。
