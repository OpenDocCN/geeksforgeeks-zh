...
> 原文: [https://www.geeksforgeeks.org/javascript-string-prototype-tolocaleuppercase-method/](https://www.geeksforgeeks.org/javascript-string-prototype-tolocaleuppercase-method/)

JavaScript 中的 `String.prototype.toLocaleUpperCase()` 方法是一个标准的内置对象，它返回基于主机当前区域设置转换为大写字母的调用字符串值。
...
*   `locale`: 这是一个可选参数，该区域设置参数表示根据任何特定于区域设置的大小写映射转换为大写的区域设置。
...
下面的例子说明了 JavaScript 中的 `String.prototype.toLocaleUpperCase()` 方法:
...
**支持的浏览器:** `String.prototype.toLocaleUpperCase()` 方法支持的浏览器如下:
