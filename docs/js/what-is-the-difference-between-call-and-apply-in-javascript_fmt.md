...
> 原文: [https://www.geeksforgeeks.org/what-is-the-difference-between-call-and-apply-in-javascript/](https://www.geeksforgeeks.org/what-is-the-difference-between-call-and-apply-in-javascript/)

## `call()` 方法

`call()` 方法调用方法，以所有者对象为参数。关键字 `this` 指的是函数或其所属对象的“所有者”。我们可以调用一个可以在不同对象上使用的方法。
...
*   `arguments`: `call()` 方法采用逗号分隔的参数。

## `apply()` 方法

`apply()` 方法用于编写方法，可以在不同的对象上使用。它不同于 `call()`，因为它将参数作为数组。
...
唯一的区别是 `call()` 方法采用逗号分隔的参数，而 `apply()` 方法采用参数数组。
