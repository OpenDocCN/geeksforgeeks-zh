...
在编程时，我们会遇到一些情况，我们希望声明一个变量，该变量的值在整个程序中保持不变。一个这样的例子是 pi 的值总是保持不变。ES6 支持一个这样的声明，可以使用单词 `const` 来声明。
...
在本例中，将抛出一个错误为“试图覆盖常量‘I’”。因此，这表明不能重写用 `const` 声明的变量。
...
下面的语句还将显示“语法错误”，因为 `const` 变量的值必须仅在声明时定义。
...
下面的语句还抛出了一个错误“未捕获语法错误”，因为我们试图声明一个使用已经声明为常量的变量名的。
...
常量对象的属性值可以被操作，但是您不能重新分配常量对象，如下例所示。
...
ES6 支持在 `for-of` 循环中使用 `Const`，其中为每次迭代创建新的绑定。
