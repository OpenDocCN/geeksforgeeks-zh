...
> 原文：[https://www.geeksforgeeks.org/how-to-get-value-of-a-string-after-last-slash-in-javascript/](https://www.geeksforgeeks.org/how-to-get-value-of-a-string-after-last-slash-in-javascript/)

任务是获取特定字符（`/`）后面的字符串。这里讨论了一些最常用的技术。我们将使用 JavaScript。

## 方法 1

*   使用 `.split()` 方法将字符串分割，并将其结果存入一个变量（数组）。
*   使用 `.length` 属性获取数组的长度。
*   从数组中返回位于索引 `length - 1` 的元素。
...
*   首先，使用 `.lastIndexOf()` 方法找到 `/` 的最后一个索引。
*   然后，使用 `.substring()` 方法获取最后一个斜杠后的字符串。
