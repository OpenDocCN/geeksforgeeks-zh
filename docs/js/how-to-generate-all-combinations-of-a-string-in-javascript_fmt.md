...
> 原文：[https://www.geeksforgeeks.org/how-to-generate-all-combinations-of-a-string-in-javascript/](https://www.geeksforgeeks.org/how-to-generate-all-combinations-of-a-string-in-javascript/)
...
*   进一步我们将使用 `.push()` 和 `.slice()` 方法将我们的结果添加到数组中。
...
*   然后我们将运行一个 `while` 循环，在这个 `while` 循环中，我们将使用 `str.charAt()` 方法存储当前索引值下的字符。
*   然后我们将声明一个临时数组，在其中存储获得的字符。
*   然后我们将运行一个 `for-in` 循环，在其中我们将 `.push()` 我们的结果，然后我们将使用 `.concat()` 方法将我们的结果添加到结果变量中，然后我们将增加当前的索引变量值。
