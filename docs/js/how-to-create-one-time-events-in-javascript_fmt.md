...
> 原文：[https://www.geeksforgeeks.org/how-to-create-one-time-events-in-javascript/](https://www.geeksforgeeks.org/how-to-create-one-time-events-in-javascript/)

在本文中，我们将学习 JavaScript 中的一次性事件以及如何创建它们。为了执行任务，我们通常在 JavaScript 中使用事件监听器。例如，如果我们需要计算两个变量的总和，则通过单击 DOM 中的一个按钮来完成。意思是我们用一个`click`事件监听器得到两个变量的和。

**一次性事件：** 这些事件只执行事件监听器一次。假设我们已经创建了一个按钮并添加了一个点击事件。如果一次性事件只执行一次，不管用户点击按钮几次，我们都会创建一个一次性事件。

假设我们已经创建了一个从 API 获取数据的按钮，如果我们没有在它上面添加一次性事件，那么它会在用户每次点击时从 API 获取数据，忽略之前的响应。正因为如此，它使我们的系统过载，降低了性能，也给用户带来了非常糟糕的体验。

**方法：** 我们[给一个按钮添加一个事件监听器](https://www.geeksforgeeks.org/javascript-addeventlistener-with-examples/)，然后使用`removeEventListener()`功能从该按钮中移除事件。这样，事件监听器只执行一次指令。
