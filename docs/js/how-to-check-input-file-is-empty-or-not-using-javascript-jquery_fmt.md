...
> 原文: [https://www.geeksforgeeks.org/how-to-check-input-file-is-empty-or-not-using-javascript-jquery/](https://www.geeksforgeeks.org/how-to-check-input-file-is-empty-or-not-using-javascript-jquery/)

给定一个包含输入元素的 HTML 文档，任务是在 JavaScript 的帮助下检查输入元素是否为空。

## 方法 1

使用 `element.files.length` 属性检查文件是否被选中。如果 `element.files.length` 属性返回 0，则不选择文件，否则选择文件。
...
使用 jQuery 中的 `element.files.length` 属性检查文件是否被选中。如果 `element.files.length` 属性返回 0，则不选择文件，否则选择文件。
