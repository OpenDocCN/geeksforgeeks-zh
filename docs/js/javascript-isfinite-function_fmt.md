...
JavaScript `isFinite()` 函数，用来检查一个数字是否是有限的合法数字。对于除 `+Infinity`、`-Infinity` 或 `NaN` 之外的所有值，它都返回 `true`。
...
*   `value`: 这是在 `isFinite()` 函数中传递的必需值。

## 返回值

返回布尔值，即如果值为 `+Infinity`、`-Infinity` 或 `NaN`，则返回 `false`，否则返回 `true`。
