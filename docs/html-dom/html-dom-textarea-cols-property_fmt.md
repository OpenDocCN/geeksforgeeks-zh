...
> 原文: [https://www.geeksforgeeks.org/html-dom-textarea-cols-property/](https://www.geeksforgeeks.org/html-dom-textarea-cols-property/)

**DOM Textarea cols 属性**用于**设置**或**返回** Textarea 字段的 `cols` 属性的值。`cols` 属性表示一行中应该容纳多少平均宽度字符。
...
HTML 程序来说明设置 **DOM Textarea cols 属性**。
...
HTML 程序来说明返回 **DOM Textarea cols Property**。
...
`textareaObject.cols` 和 `style.width` 工作原理相同。

## 支持的浏览器

`textarea` `cols` 属性支持的浏览器如下:
