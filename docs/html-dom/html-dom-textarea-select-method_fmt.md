...
> 原文: [https://www.geeksforgeeks.org/html-dom-textarea-select-method/](https://www.geeksforgeeks.org/html-dom-textarea-select-method/)

HTML DOM 中的 `select()` 方法用于选择 `<textarea>` 或包含文本字段的 `<input>` 元素的全部内容。
...
`DOM Textarea select()` 方法支持的浏览器如下:
