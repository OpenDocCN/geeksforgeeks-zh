# HTML DOM window.opener 属性

> 原文：[https://www.geeksforgeeks.org/html-dom-window-opener-properties/](https://www.geeksforgeeks.org/html-dom-window-opener-properties/)

HTML DOM 中的 `window.opener` 属性用于返回新创建窗口的引用。此属性用于返回源（父）窗口的详细信息。窗口使用 `window.open()` 方法打开，使用 `window.opener.close()` 方法关闭。
