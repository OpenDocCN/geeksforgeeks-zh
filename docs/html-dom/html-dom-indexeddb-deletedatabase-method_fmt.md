...
> 原文: [https://www.geeksforgeeks.org/html-dom-indexeddb-deletedatabase-method/](https://www.geeksforgeeks.org/html-dom-indexeddb-deletedatabase-method/)

`indexedDB.deleteDatabase()` 方法用于请求删除数据库。该方法立即返回一个 `IDBOpenDBRequest` 对象，并异步执行删除操作。
...
删除后的一个 `IDBOpenDBRequest` 对象。

## 示例

在本例中，我们将打开一个数据库，因此将使用此方法删除该数据库。
...
**按钮点击后:**
在控制台中，可以看到 `IDBOpenDBRequest` 对象，并有成功删除的消息。
