...
> 原文：[https://www.geeksforgeeks.org/html-dom-style-lineheight-property/](https://www.geeksforgeeks.org/html-dom-style-lineheight-property/)
...
返回一个字符串值，代表文本中各行之间的距离。

## 属性值

*   `normal`：用于定义正常高度。
*   `number`：用于定义一个将乘以当前字体大小的数字来设置行高。
*   `length`：用于定义以长度单位表示的行高。
*   `%`：用于定义当前字体大小的行高，单位为%。
...
<body>
    ...
    <script>
        function lineheight() {
            // Set lineHeight.
            document.getElementById("MyDiv").style.lineHeight = "2";
        }
    </script>
</body>
</html>
