...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-email-defaultvalue-property/](https://www.geeksforgeeks.org/html-dom-input-email-defaultvalue-property/)

HTML DOM 中的**输入邮件默认值属性**用于设置或返回邮件字段的默认值。此属性用于反映 HTML `value` 属性。默认值和 `value` 属性之间的主要区别在于，`defaultValue` 表示属性中指定的默认值，而该值在进行一些更改后包含当前值。当我们想知道电子邮件字段是否已被更改时，此属性很有用。
...
包含单个属性值 `value`，定义邮件字段的默认值。

## 返回值

返回一个代表电子邮件字段默认值的字符串值。
