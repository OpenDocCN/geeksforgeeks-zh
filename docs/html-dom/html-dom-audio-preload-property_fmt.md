...
> 原文: [https://www.geeksforgeeks.org/html-dom-audio-preload-property/](https://www.geeksforgeeks.org/html-dom-audio-preload-property/)

`音频预载属性`用于设置或返回音频预载属性的值。`preload`属性用于指定页面加载时作者认为应该加载音频的方式。音频预载属性允许作者向浏览器描述网站用户体验的实现方式。
...
2.  `metadata`: 用于指定页面加载时浏览器只加载元数据。
3.  `none`: 用于指定页面加载时浏览器不应加载视频。
