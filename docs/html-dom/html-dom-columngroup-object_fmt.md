...
HTML DOM 中的 `ColumnGroup` 对象用来表示 HTML [`<colgroup>`](https://www.geeksforgeeks.org/html-colgroup-tag/) 元素。
此标签用于设置或获取 `<colgroup>` 元素的属性。可以使用 `getElementById()` 方法访问。
...
**示例 1：** 使用 `document.getElementById("ColGroup_ID").id` 返回列组标识。
...
**示例 2：** 使用 `document.getElementById("myColGroup").span` 返回跨度元素的编号。
