...
> 原文: [https://www.geeksforgeeks.org/html-dom-option-label-property/](https://www.geeksforgeeks.org/html-dom-option-label-property/)

HTML DOM 中的选项标签属性用于设置或返回 `<option>` 元素中 `label` 属性的值。`label` 属性用于指定选项的标签。该属性包含代表选项短标签的文本值。
...
该属性包含单值 `text`，用于表示选项的短标签。

## 返回值

返回一个字符串值，代表选项元素或下拉列表中的最短标签。
