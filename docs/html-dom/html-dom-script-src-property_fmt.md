...
**DOM 脚本 src 属性**用于**设置**或**返回**一个 [`<script>`](https://www.geeksforgeeks.org/html-script-tag/) 元素的 `src` 属性的值。`src` 属性指定外部 Javascript 文件的网址。
...
## 返回值:
返回一个字符串值，代表外部 Javascript 文件的网址。
...
<body style="text-align:center;">
    ...
    <script>
        function Geeks() {
            var x = document.getElementById("myGeeks").src;
...
## 支持的浏览器:
下面列出了**HTML | DOM Script src Property**支持的浏览器:
