# HTML DOM Input DatetimeLocal stepUp() 方法

> 原文: [https://www.geeksforgeeks.org/html-dom-input-datetimelocal-stepup-method/](https://www.geeksforgeeks.org/html-dom-input-datetimelocal-stepup-method/)

HTML DOM 中的 `Input DatetimeLocal stepUp()` 方法用于将本地日期时间（Local datetime）字段的值增加一个指定的数字。`stepUp()` 方法只能用于分钟值。年、月、日、小时、秒或毫秒不受 `stepUp()` 方法的影响。
...
**参数:** 该方法接受单参数 `number`，用于指定本地日期时间字段应增加的分钟数。
...
**支持的浏览器:** 下面列出了 `Input DatetimeLocal` 支持的浏览器:
