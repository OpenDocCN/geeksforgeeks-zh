...
本例使用`getElementById()`方法访问`<input>`元素，元素类型为`type="email"`属性。
...
E-mail: <input type="email" id="email" value="careers@geeksforgeeks.org">
<button onclick="myGeeks()">
        Click Here!
    </button>
<p id="GFG"></p>
<!-- Script to access input element with type email attribute -->
<script>
    function myGeeks() {
...
本示例使用`document.createElement()`方法创建具有`type="email"`属性的`<input>`元素。
...
<!-- script to create input element of type email attribute -->
<script>
    function myGeeks() {
