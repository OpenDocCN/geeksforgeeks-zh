...
> 原文: [https://www.geeksforgeeks.org/html-dom-style-backgroundsize-property/](https://www.geeksforgeeks.org/html-dom-style-backgroundsize-property/)

HTML DOM Style `backgroundSize` 属性用于设置或返回背景图像的大小。
...
This is used to display the background image in its original size it’s default value.
...
<head>
    ...
    <style>
        .bg-img {
            ...
            width: 200px;
            border-style: solid;
            background-image: url('https://media.geeksforgeeks.org/wp-content/uploads/sample_image.png');
...
<body>
    ...
    <p>Click on the button to change the size of the background image</p>
...
This is used to set the height and width of the image. Both the values set the width and height respectively. If only one value is given, the other one is set to ‘auto’.
...
<body>
    ...
    <script>
        function changePos() {
            elem = document.querySelector('.bg-img');
            // Setting size to 200px in width and 50px in height
            elem.style.backgroundSize = '200px 50px';
        }
    </script>
</body>
...
This sets the width and height in terms of percentage of the parent element. Both the values set the width and height respectively. If only one value is given, the other one is set to ‘auto’.
...
This is used to scale the background image to cover the whole container element.
...
This is used to scale the background image as large as possible such that both the height and width fit inside the container area.
...
This is used to set this property to its default value.
