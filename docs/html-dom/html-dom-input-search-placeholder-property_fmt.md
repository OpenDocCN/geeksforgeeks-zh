...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-search-placeholder-property/](https://www.geeksforgeeks.org/html-dom-input-search-placeholder-property/)

HTML DOM 中的 `DOM 输入搜索占位符属性`用于设置或返回搜索字段的占位符属性值。占位符属性指定描述输入字段预期值的简短提示。在用户输入值之前，短提示会显示在字段中。
...
它包含单值文本，用于定义描述搜索字段预期值的简短提示。

## 返回值

它返回一个字符串值，表示描述搜索字段预期值的简短提示。
