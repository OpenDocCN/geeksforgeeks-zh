...
> 原文: [https://www.geeksforgeeks.org/html-dom-style-alignself-property/](https://www.geeksforgeeks.org/html-dom-style-alignself-property/)

DOM Style `alignSelf` 属性用于设置或返回柔性容器内选定项目的对齐方式。
...
*   `stretch`: 用于拉伸物品以适合容器。
*   `center`: 用于将容器中的物品居中。
*   `flex-start`: 用于将物品定位在容器的开始位置。
*   `flex-end`: 用于将物品定位在容器的末端。
*   `baseline`: 用于将物品定位在容器的基线。
