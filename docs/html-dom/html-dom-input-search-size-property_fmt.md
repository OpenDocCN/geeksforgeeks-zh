...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-search-size-property/](https://www.geeksforgeeks.org/html-dom-input-search-size-property/)

HTML DOM 中的 `DOM 输入搜索大小属性` 用于**设置**或**返回** `输入搜索字段` 的 `size` 属性值。`size` 属性用于定义搜索字段的宽度。它的默认值是 20。
...
**示例-1:** 本示例说明如何返回 Input 搜索 `size` 属性。
