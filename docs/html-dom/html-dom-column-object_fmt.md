...
HTML DOM 中的`列对象`用来表示 HTML [`<col>`](https://www.geeksforgeeks.org/html-col-tag/) 元素。
此标签用于设置或获取 `<col>` 元素的属性。可以使用 `getElementById()` 方法访问。
...
*   [`span`](https://www.geeksforgeeks.org/html-dom-column-span-property/)：用于设置或返回跨度属性。

## 示例-1：使用 `document.getElementById("col id").id;`
...
<body style="text-align:center;">
    ...
    <script>
        function myGeeks() {
            // access col element
            var x = document.getElementById("myCol").id;
...
## 示例-2：使用 `document.getElementById("col id").span;`
