...
> 原文: [https://www.geeksforgeeks.org/html-dom-script-charset-property/](https://www.geeksforgeeks.org/html-dom-script-charset-property/)

**HTML DOM Script 字符集属性**用于设置或返回`<script>`元素的字符集属性的值。`charset`属性用于指定外部脚本中使用的字符编码。
...
**属性值:** 它包含单个值 `charset`，用于指定外部脚本中使用的字符编码。
