...
> 原文: [https://www.geeksforgeeks.org/html-dom-form-method-property/](https://www.geeksforgeeks.org/html-dom-form-method-property/)

`DOM Form`方法属性用于设置或返回表单中方法属性的值。方法属性用于指定提交表单时用于发送数据的`HTTP`方法。`HTTP`方式有两种，分别是`GET`和`POST`。
...
返回一个字符串值，代表提交表单时用来发送数据的`HTTP`方法。
...
<script>
        function myGeeks() {
            ...
            document.getElementById("sudo").innerHTML = 
              "The value of the method attribute was changed to "
            + x;
        }
    </script>
