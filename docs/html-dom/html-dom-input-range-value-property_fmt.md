...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-range-value-property/](https://www.geeksforgeeks.org/html-dom-input-range-value-property/)

HTML DOM 中的 `DOM 输入范围值属性`用于**设置**或**返回**滑块控件或输入范围字段的值。该属性指定默认值或用户输入的值。
...
它包含一个值，即**数**，指定滑块控件的值。它的默认值是 50。

## 返回值

它返回一个字符串值，该值代表滑块控件值的数字。
