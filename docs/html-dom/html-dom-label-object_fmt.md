...
**DOM 标签对象**用于表示 [`<label>`](https://www.geeksforgeeks.org/html-label-tag/) 元素。标签元素由 `getElementById()` 访问。

## 属性

*   `control`: 用于返回标记的控制。
*   [`form`](https://www.geeksforgeeks.org/html-dom-label-form-property/?ref=rp): 用于返回包含标签的表单的地址。
*   [`htmlFor`](https://www.geeksforgeeks.org/html-dom-label-htmlfor-property/?ref=rp): 用于设置或返回标签的 For 属性的值。
...
可以使用 `document.createElement` 方法创建标签对象。
