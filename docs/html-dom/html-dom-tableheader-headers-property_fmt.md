...
> 原文：[https://www.geeksforgeeks.org/html-dom-tableheader-headers-property/](https://www.geeksforgeeks.org/html-dom-tableheader-headers-property/)

`HTML DOM TableHeader`的`headers`属性用于**设置**或**返回**`headers`属性的值。`headers`属性用于指定包含当前表头单元格表头信息的表格单元格。
...
它包含值，即`header_id`，指定一个或多个标题单元格的单独 id 列表。

## 返回值

它返回一个字符串值，代表标题列表标识的分离列表。
