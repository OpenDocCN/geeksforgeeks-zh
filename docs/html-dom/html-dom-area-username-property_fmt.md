# HTML DOM Area username 属性

> 原文：[`https://www.geeksforgeeks.org/html-dom-area-username-property/`](https://www.geeksforgeeks.org/html-dom-area-username-property/)

HTML DOM 中的 `area` `username` 属性用于设置或返回 `href` 属性的用户名部分的值。用户名部分用于定义用户输入的用户名。它在协议之后和密码部分之前指定。
例如：`https://manaschhabra:manaschhabra499@www.geeksforgeeks.org/`（`manachhabra` 是用户名，`manaschhabra499` 是密码）。

## 语法

它返回 `area` `username` 属性。

```html
areaObject.username
```

它用于设置 `area` `username` 属性。
