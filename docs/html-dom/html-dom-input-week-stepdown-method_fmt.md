# HTML DOM `stepDown()` 方法

> 原文：`https://www.geeksforgeeks.org/html-dom-input-week-stepdown-method/`

HTML DOM 中的 `stepDown()` 方法用于将周输入字段的值减少给定的数字。这种方法只会减少周数而不是年数。
...
接受单参数 `number`，为必输项。它指定要减少的周数。默认情况下，周数递减 1。
