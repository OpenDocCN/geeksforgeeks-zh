...
> 原文：[https://www.geeksforgeeks.org/html-dom-area-protocol-property/](https://www.geeksforgeeks.org/html-dom-area-protocol-property/)
...
该方法接受字符串类型的单值 `protocol`。用于设置 URL 的协议。协议的可能值是- `file:`，`ftp:`，`http:`，`https:`等。

## 返回值

返回包含当前网址协议部分的字符串，包括冒号(`:`)。
