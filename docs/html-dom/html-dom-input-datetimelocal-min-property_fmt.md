...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-datetimelocal-min-property/](https://www.geeksforgeeks.org/html-dom-input-datetimelocal-min-property/)
...
接受属性值 `YYYY-MM-DDThh:mm:ssTZD`。它用于指定允许的最小日期和时间。
...
返回一个字符串值，代表 `datetimeLocal` 字段的最小日期和时间。

下面的程序说明了 HTML DOM 中的 `DatetimeLocal` `min` 属性：

## 示例

本示例返回 `datetimeLocal` 字段允许的最小日期时间。
