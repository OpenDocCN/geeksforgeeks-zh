# HTML | DOM 样式 counterIncrement 属性

> 原文: [https://www.geeksforgeeks.org/html-dom-style-counterincrement-property/](https://www.geeksforgeeks.org/html-dom-style-counterincrement-property/)

HTML DOM 中的 `counterIncrement` 属性指定选择器每次出现时计数器应该增加多少。增量的默认值为 1。
...
*   `none`: 为默认值。该值不会重置计数器。
*   `number`: 每次元素出现时为指定的计数器设置一个增量。这个增量可以是零，甚至是负的。如果未指定，默认值为 1。
*   `initial`: 将元素设置为其初始位置。
*   `inherit`: 元素从父元素继承其属性。
...
以下是 `HTML | DOM Style counterIncrement` 属性支持的浏览器:
