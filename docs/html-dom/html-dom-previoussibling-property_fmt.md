...
> 原文：[https://www.geeksforgeeks.org/html-dom-previoussibling-property/](https://www.geeksforgeeks.org/html-dom-previoussibling-property/)

`previousSibling` 属性用于将指定节点的上一个节点作为 node 对象返回，如果指定节点是列表中的第一个节点，则返回 `null`。它是只读属性。
...
此属性返回指定节点的前一个同级，如果当前节点没有前一个同级，则返回 `null`。
...
不要在两个兄弟元素之间放空白，否则结果会是“未定义”。

## 支持的浏览器

由 `previousSibling` 属性支持的浏览器如下：
