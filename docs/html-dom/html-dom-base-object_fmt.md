...
HTML DOM 中的**基对象**用来表示 HTML [`<base>`](https://www.geeksforgeeks.org/html-base-tag/) 元素。此标签用于*设置*或*获取*`<base>`元素的属性。使用 `getElementById()` 方法可以访问该元素。
...
这个 `"Base_ID"` 被分配给 HTML `<base>` 元素。

**对象属性：**

*   [`href`](https://www.geeksforgeeks.org/html-dom-base-href-property/?ref=rp)：用于设置或返回基元素中的 `href` 属性。
*   [`target`](https://www.geeksforgeeks.org/html-dom-base-target-property/?ref=rp)：用于设置或返回一个基础元素中的 `target` 属性。
