...
> 原文：[https://www.geeksforgeeks.org/html-dom-image-naturalwidth-property/](https://www.geeksforgeeks.org/html-dom-image-naturalwidth-property/)

`naturalWidth` 属性用于获取图像的原始宽度。由于要在网页上显示的图像的宽度可以使用 `<img>` 标签的 `width` 属性来修改，因此在需要图像的原始宽度而不是定制宽度的情况下，`naturalWidth` 属性变得有用。它是只读属性。
