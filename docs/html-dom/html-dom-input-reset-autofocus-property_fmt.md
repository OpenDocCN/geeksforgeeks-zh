...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-reset-autofocus-property/](https://www.geeksforgeeks.org/html-dom-input-reset-autofocus-property/)

HTML DOM 中的**输入重置自动对焦属性**用于设置或返回页面加载时输入重置字段是否应该对焦。它反映了 HTML `autofocus` 属性。
...
*   **true:** 设置复位场的焦点。
*   **false:** 有默认值。它定义了重置字段不能获得焦点。

## 返回值

返回一个布尔值，代表复位字段是否自动对焦。
