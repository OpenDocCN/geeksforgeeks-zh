...
> 原文：[https://www.geeksforgeeks.org/html-dom-table-deletethead-method/](https://www.geeksforgeeks.org/html-dom-table-deletethead-method/)

`deleteTHead()` 方法用于从表格中删除一个 `<thead>` 元素及其内容。
只有当 `<thead>` 元素已经存在时才能使用。
...
<head>
    <title>Table deleteTHead() Method in HTML</title>
    <style>
        table,
        td {
...
<body>
    ...
    <script>
        function Add_Head() {
            ...
            MyCell.innerHTML = "<strong>GeeksforGeeks.</strong>";
        }

        function Delete_Head() {
            // Delete thead.
            document.getElementById("Courses").deleteTHead();
        }
    </script>
