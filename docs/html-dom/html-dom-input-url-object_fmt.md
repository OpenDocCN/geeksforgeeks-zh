...
*   `list`：返回包含 URL 字段的数据列表的引用。
*   `form`：返回包含 URL 字段的表单的引用。
*   `autocomplete`：用于设置或返回 URL 字段的 `autocomplete` 属性的值。
*   `autofocus`：用于设置或返回页面加载时 URL 字段是否应自动对焦。
*   `defaultValue`：用于设置或返回网址字段的默认值。
*   `disabled`：用于设置或返回网址字段是否禁用。
*   `maxLength`：用于设置或返回 URL 字段的 `maxLength` 属性的值。
*   `name`：用于设置或返回网址字段的 `name` 属性的值。
*   `pattern`：用于设置或返回 URL 字段的 `pattern` 属性的值。
*   `placeholder`：用于设置或返回 URL 字段的 `placeholder` 属性的值。
*   `readOnly`：用于设置或返回 URL 字段是否为只读。
*   `required`：用于设置或返回提交表单前是否必须填写网址字段。
*   `size`：用于设置或返回 URL 字段的 `size` 属性值。
*   `type`：返回 URL 字段所属的表单元素类型。
*   `value`：用于设置或返回网址字段的 `value` 属性的值。
