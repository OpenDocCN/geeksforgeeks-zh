...
> 原文: [https://www.geeksforgeeks.org/html-dom-style-opacity-property/](https://www.geeksforgeeks.org/html-dom-style-opacity-property/)

HTML DOM 中的样式`opacity`属性用于设置元素的不透明度级别。不透明度级别定义透明度级别，其中值 1 表示不透明，0.5 表示 50%透明度，0 表示完全透明。它还返回元素的不透明度。
...
`支持的浏览器:` 由 `DOM Style opacity`属性支持的浏览器如下:
