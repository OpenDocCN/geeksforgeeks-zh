...
HTML DOM 链接对象用于访问 **HTML `<link>` 元素**。
...
| 值 | 描述 |
| :--- | :--- |
| `charset` | Assign the character encoding of the linked document |
| `crossorigin` | CORS settings for assigning linked documents |
| `disabled` | Is the linked document disabled, yes or no |
| `href` | The URL used to set/return the linked document |
| `hreflang` | It assigns the language code of the linked document. |
| `media` | It assigns the media type of the linked element. |
| `rev` | It assigns the current reverse relationship from the linked document to the current document. |
| `sizes` | Returns the value of the size property of the linked resource. |
...
<body>
    ...
    <button onclick="gfg()">Get URL</button>
    ...
    <script>
        function gfg() {
            // Access link element.
            var NEW = document.getElementById("linkid").href;
            document.getElementById("pid").innerHTML = NEW;
        }
    </script>
...
<body>
    ...
    <script>
        function myFunction() {
            ...
            var NEW1 = document.getElementById("linkid").href;
            document.getElementById("pid").innerHTML = NEW1;
        }
    </script>
