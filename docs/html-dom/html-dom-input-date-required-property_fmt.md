# HTML DOM 输入日期（Input Date）的 required 属性

> 原文：[https://www.geeksforgeeks.org/html-dom-input-date-required-property/](https://www.geeksforgeeks.org/html-dom-input-date-required-property/)

`required` 输入日期属性用于设置或返回在提交表单之前是否必须填写日期字段。
HTML `required` 属性用于反映 `Input Date` 的 `required` 属性。
...
### 示例 2：下面的代码设置了 `date` 的 `required` 属性
...
<p>To set the date required property double-click the "set" button.</p>
