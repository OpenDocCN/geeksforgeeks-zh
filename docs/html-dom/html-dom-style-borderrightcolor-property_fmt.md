# HTML DOM 样式 `borderRightColor` 属性
...
`borderRightColor` 属性允许我们设置或获取元素右边框的颜色。

## 语法

`borderRightColor` 属性用于获取或设置右边框的颜色。
...
`borderRightColor` 属性返回元素右边框的颜色。

## 属性值

### `color`
指定对应元素的右边框颜色。黑色是默认颜色。
...
## 浏览器支持
DOM Style `borderRightColor` 属性支持的浏览器如下：
