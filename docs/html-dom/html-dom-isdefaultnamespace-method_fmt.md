...
> 原文：[https://www.geeksforgeeks.org/html-dom-isdefaultnamespace-method/](https://www.geeksforgeeks.org/html-dom-isdefaultnamespace-method/)
...
如果名称空间为默认值，则返回布尔值 `true`，否则返回 `false`。

## 示例

返回 DefaultNamespace 属性。
...
<body>
    ...
    <script>
        function space() {
            ...
            var x = d.isDefaultNamespace("https://ide.geeksforgeeks.org/tryit.php");
...
Internet Explorer 8 及更早版本不支持此方法。

## 浏览器支持

下面列出了 `DOM isDefaultNamespace()` 方法支持的浏览器：
