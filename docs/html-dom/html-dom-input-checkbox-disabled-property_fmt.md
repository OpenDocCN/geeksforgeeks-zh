...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-checkbox-disabled-property/](https://www.geeksforgeeks.org/html-dom-input-checkbox-disabled-property/)

HTML DOM 中的`输入复选框禁用属性`用于设置或返回是否必须禁用输入复选框字段。禁用的复选框不可勾选且不可用。它是一个布尔属性，用于反映 HTML `disabled` 属性。
