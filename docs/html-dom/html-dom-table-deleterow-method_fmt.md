# HTML DOM 表格 `deleteRow()` 方法

> 原文：[https://www.geeksforgeeks.org/html-dom-table-deleterow-method/](https://www.geeksforgeeks.org/html-dom-table-deleterow-method/)

`表格删除行()`方法用于从表格中删除一个`<tr>`元素。换句话说，`表格删除行()`方法用于删除表中指定索引处的行。
...
*   `index`：用于指定要删除的行的位置。值`0`导致删除第一行，而`-1`可用于删除最后一行。
...
![](img/ff29a2c6692c4c59243225466c4f5f86.png)
