...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-image-name-property/](https://www.geeksforgeeks.org/html-dom-input-image-name-property/)

**HTML DOM 输入图像名称属性**用于设置或返回输入图像的名称属性值。`name`属性用于在提交表单后引用表单数据或引用 JavaScript 中的元素。
...
```html
imageObject.name = string
```

## 属性值

接受单个参数`name`，并指定输入图像的名称。
