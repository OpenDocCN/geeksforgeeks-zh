...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-checkbox-property/](https://www.geeksforgeeks.org/html-dom-input-checkbox-property/)

HTML 中的输入复选框对象代表 HTML 表单中的一个复选框。
对于 HTML 表单中的 `<input>` 元素的每个实例，都会创建一个复选框对象。要访问 `checkbox` 对象，请使用索引相应表单的 `elements` 数组或使用 `getElementById()`。

## 创建复选框对象

我们可以通过 JavaScript 创建复选框对象。要创建 `<input type="checkbox">` 元素，请使用 `document.createElement()` 方法。创建后，使用 `appendChild()` 方法将其追加到特定元素（如 `div`）中以显示它。
...
我们可以使用 `getElementById()` 方法访问复选框对象。将复选框元素的 `id` 放在 `getElementById()` 中进行访问。
