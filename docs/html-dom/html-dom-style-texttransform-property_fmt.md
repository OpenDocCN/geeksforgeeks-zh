...
> 原文: [https://www.geeksforgeeks.org/html-dom-style-texttransform-property/](https://www.geeksforgeeks.org/html-dom-style-texttransform-property/)

HTML DOM 中的 `style` `textTransform` 属性用于设置或返回文本的大写。它可用于使所需文本大写、小写或大写每个单词的第一个字符。
...
<body>
    ...
    <!-- Script to capitalize first character of each word -->
    <script>
...
<style>
    .content {
        width: 500px;
        /* Set text-transform before to observe effect of 'none' */
...
<body>
    ...
    <!-- Script to convert lower case of each word -->
    <script>
        function setTransform() {
            elem = document.querySelector('.content');
            elem.style.textTransform = 'none';
...
<body>
    ...
    <!-- Script to change lower case of each word -->
    <script>
        function setTransform() {
            elem = document.querySelector('.content');
...
<head>
    ...
    <style>
        #parent {
            /* Set text-transform of parent to observe effect of 'inherit' */
            text-transform: uppercase;
        }
        .content {
            width: 500px;
            /* Set text-transform to initial to prevent auto inheritance of parent */
...
## 支持的浏览器:
*DOM Style textTransform 属性*支持的浏览器如下:
*   谷歌 Chrome
*   微软公司出品的 web 浏览器
*   火狐浏览器
*   歌剧
