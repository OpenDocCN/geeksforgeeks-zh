...
当一些内容粘贴到一个元素中时，就会出现 `HTML DOM onpaste` 事件。这个事件对每个元素都有效，就像在 `<p>` 元素中一样。如果 `contenteditable` 设置为 `true`，那么我们可以在 `<p>` 元素中粘贴内容。输入元素中最常用的 `HTML DOM onpaste` 事件。
