...
> 原文：[https://www.geeksforgeeks.org/html-dom-ins-datetime-property/](https://www.geeksforgeeks.org/html-dom-ins-datetime-property/)

`DOM ins DateTime` 属性用于设置或返回 `<ins>` 元素的 `DateTime` 属性的值。该属性用于指定插入文本的日期和时间。日期时间以 `YYYY-MM-DDThh:mm:ssTZD` 格式插入。
...
*   **YYYY-MM-DDThh:MM:ssTZD**：它指定文本插入或替换其他文本的日期和时间。
...
<html>
<head>
    <title>DOM ins dataTime Property</title>
    <style>
        del {
            color: red;
        }
...
<body>
    ...
    <p id="sudo">
        <script>
            function myGeeks() {
                <!-- Return dateTime -->
                var g = document.getElementById("GFG").dateTime = "2015-10-19T22:45:07Z";
