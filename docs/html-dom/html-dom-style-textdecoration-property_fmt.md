...
> 原文：[https://www.geeksforgeeks.org/html-dom-style-textdecoration-property/](https://www.geeksforgeeks.org/html-dom-style-textdecoration-property/)
...
*   `none`：用于定义正常文本。这是一个默认值。
*   `underline`：定义文本下的一行。
*   `overline`：它定义了文本上方的一条线。
*   `line-through`：它定义了一条穿越文本的线。
*   `initial`：将文本装饰属性设置为默认值。
...
**支持的浏览器：** `DOM Style textDecoration` 属性支持的浏览器如下：
