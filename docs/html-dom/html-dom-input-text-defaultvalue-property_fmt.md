...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-text-defaultvalue-property/](https://www.geeksforgeeks.org/html-dom-input-text-defaultvalue-property/)

HTML DOM 中的**输入文本默认值属性**用于设置或返回文本字段的默认值。此属性用于反映 HTML `value` 属性。默认值与值的主要区别在于，默认值表示默认值，值包含一些更改后的当前值。此属性对于确定文本字段是否已被更改非常有用。
