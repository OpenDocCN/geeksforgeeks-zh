...
> 原文：[https://www.geeksforgeeks.org/html-dom-style-border-property/](https://www.geeksforgeeks.org/html-dom-style-border-property/)
...
*   `none`：不创建边框，保持空白。
*   `hidden`：就像“无”一样，除非添加背景图像，否则它不会显示任何边框，然后边框顶部宽度将设置为 0，而与用户定义的值无关。
...
*   `inset`：显示一个 3D 内嵌边框，其效果取决于边框颜色值。
*   `outset`：显示一个 3D 外凸边框，其效果取决于边框颜色值。
...
<body>
    ...
    <script>
        function myGeeks() {
            ...
            document.getElementById("GFG").style.borderStyle = "dashed dotted double solid";
        }
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        function myGeeks() {
            ...
            document.getElementById("GFG").style.borderStyle = " dotted ";
        }
    </script>
</body>
</html>
