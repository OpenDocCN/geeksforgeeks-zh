...
> 原文：[https://www.geeksforgeeks.org/html-dom-keygen-disabled-property/](https://www.geeksforgeeks.org/html-dom-keygen-disabled-property/)

**HTML DOM Keygen disabled 属性**用于设置或返回一个布尔值，该值指示是否应该禁用 Keygen 元素。禁用的 keygen 字段不可用。该属性用于反映 HTML `disabled` 属性。
...
它返回一个布尔值，该值代表键根字段是否被禁用。

## 示例

下面的代码演示了如何启用和禁用 keygen 字段。
