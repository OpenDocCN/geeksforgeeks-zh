# HTML DOM history.go()方法

> 原文：[https://www.geeksforgeeks.org/html-dom-history-go-method/](https://www.geeksforgeeks.org/html-dom-history-go-method/)

HTML 中的 `history.go()` 方法用于从历史列表中加载特定的 URL。如果知道要从您的历史中加载的特定页面的号码或网址，这是 `history.back()` 和 `history.forward()` 方法的更好选择。
...
该方法接受单个参数 `number|URL`，该参数为必选项，用于指定网址在历史列表中的编号/位置或页面的网址。`-1` 用于后退一页，`1` 用于前进一页。

下面的程序用 HTML 说明了 `history.go()` 方法：
