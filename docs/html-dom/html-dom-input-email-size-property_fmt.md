...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-email-size-property/](https://www.geeksforgeeks.org/html-dom-input-email-size-property/)

HTML DOM 中的`输入邮件大小属性`用于设置或返回输入邮件字段的`size`属性值。`size`属性用于定义电子邮件字段的宽度。它的默认值是 20。
...
它包含单个值`数字`，用于根据字符数指定电子邮件字段的宽度。

## 返回值

返回代表电子邮件字段宽度的数值，以字符数表示。
