...
HTML DOM 中的`输入图像对象`用`type="image"`表示`HTML <input>`元素。
该标签用于访问或创建元素。可以使用`getElementById()`方法访问该元素。
...
| 值 | 描述 |
| :--- | :--- |
| `alt` | 设置/返回输入图像的 `alt` 属性值。 |
| `autofocus` | 如果页面加载时输入图像自动获得焦点，则设置/返回。 |
| `defaultValue` | 设置/返回输入图像的默认值。 |
| `disabled` | 设置/返回输入图像是否被禁用。 |
| `form` | 返回包含输入图像的窗体的引用。 |
| `formAction` | 设置/返回输入图像的 `formaction` 属性值。 |
| `formEnctype` | 设置/返回输入图像的 `formenctype` 属性值。 |
| `formMethod` | 设置/返回输入图像的 `formmethod` 属性值。 |
| `formNoValidate` | 提交时设置/返回表单数据是否有效。 |
| `formTarget` | 设置/返回输入图像的 `formtarget` 属性值。 |
| `height` | 设置/返回输入图像的 `height` 属性值。 |
| `name` | 设置/返回输入图像的 `name` 属性值。 |
| `src` | 设置/返回输入图像的 `src` 属性值。 |
| `type` | 返回输入元素的表单元素类型。 |
| `value` | 设置/返回输入图像的 `value` 属性值。 |
| `width` | 设置/返回输入图像的 `width` 属性值。 |
...
访问输入元素并返回 `formTarget`、`formEnctype` 和 `formAction`。
