...
> 原文: [https://www.geeksforgeeks.org/html-dom-removeeventlistener-method/](https://www.geeksforgeeks.org/html-dom-removeeventlistener-method/)
...
在下面的示例中，调用事件处理程序，该程序在鼠标移动到分部上时更改分部标记的颜色。按下“更改颜色”按钮后，与分部标记相关联的事件处理程序将被移除，并且在鼠标移动到分部上时不会出现颜色变化。
...
下面列出了 `removeEventListener()` 方法支持的浏览器:
