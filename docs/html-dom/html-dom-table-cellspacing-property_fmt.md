...
> 原文: [https://www.geeksforgeeks.org/html-dom-table-cellspacing-property/](https://www.geeksforgeeks.org/html-dom-table-cellspacing-property/)

``HTML DOM 表格单元格间距属性``用于设置或返回`<table>`元素的``单元格间距``属性的值。``单元格间距``属性用于定义单元格之间的间距。
...
<body>
    ...
    <script>
        function Table_Caption() {
            var w = document.getElementById("GFG").cellSpacing;
            document.getElementById("sudo").innerHTML = w + "PX";
        }
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        function Table_Caption() {
            // Pixel set
            var w = (document.getElementById("GFG").cellSpacing = "5");
            document.getElementById("sudo").innerHTML = w + "PX";
        }
    </script>
</body>
