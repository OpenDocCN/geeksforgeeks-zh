...
HTML DOM 中的 `borderRightWidth` 属性用于设置或返回元素右边框的宽度。
...
本示例使用样式 `borderRightWidth` 属性将右边框的宽度设置为 `20px`。
...
本示例使用样式 `borderRightWidth` 属性将右边框的宽度设置为 `"thin"`。
...
以下列出了 `DOM Style borderRightWidth` 属性支持的浏览器:
