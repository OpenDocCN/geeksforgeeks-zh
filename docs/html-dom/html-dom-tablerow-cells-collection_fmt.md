...
**属性:**
*   `length`: 用于返回集合中`<td>`和`<th>`元素的个数。

**方法:**
*   `[index]`: 用于从集合中返回带有指定索引的`<td>`和`<th>`元素。如果索引号超出范围，它将返回 `null`。
*   `item(index)`: 也用于从集合中返回带有指定索引的`<td>`和`<th>`元素。如果索引号超出范围，它将返回 `null`。
*   `namedItem(id)`: 也用于从集合中返回指定 id 的`<td>`和`<th>`元素。如果 id 不存在，则返回值为 `null`。
