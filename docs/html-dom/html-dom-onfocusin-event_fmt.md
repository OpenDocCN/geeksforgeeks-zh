...
如果想知道一个元素或其子元素是否获得焦点，应该使用 `onfocusin` 事件。`onfocusin` 事件与 `onfocusout` 事件相反。
...
<body>
    <center>
        ...
        <script>
            document.getElementById("fname").addEventListener("focusin", gfgFun);
...
## 支持的浏览器:
`HTML DOM onfocusin Event` 支持的浏览器如下:
