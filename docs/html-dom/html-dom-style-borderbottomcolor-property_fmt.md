...
> 原文: [https://www.geeksforgeeks.org/html-dom-style-borderbottomcolor-property/](https://www.geeksforgeeks.org/html-dom-style-borderbottomcolor-property/)

HTML DOM 中的 `Style borderBottomColor` 属性用于设置或返回元素下边框的颜色。
...
*   `color`: 指定对应元素的下边框颜色。默认颜色是黑色。
*   `transparent`: 将对应元素的下边框颜色设置为透明。
*   `initial`: 将 `borderBottomColor` 属性设置为默认值。
...
<body align="center">
    ...
    <!-- Script to use Style borderBottomColor Property -->
    <script>
        function myGeeks() {
...
下面列出了 `Style borderBottomColor` 属性支持的浏览器:
