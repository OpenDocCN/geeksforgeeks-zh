...
> 原文：[https://www.geeksforgeeks.org/html-dom-animationiteration-event/](https://www.geeksforgeeks.org/html-dom-animationiteration-event/)

当一个 CSS 动画被重复时，`animationiteration` 事件发生。

如果 CSS `animation-iteration-count` 属性设置为 `"1"`，动画将只播放一次。动画需要运行多次才能触发此事件。
