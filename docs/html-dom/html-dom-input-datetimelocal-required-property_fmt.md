...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-datetimelocal-required-property/](https://www.geeksforgeeks.org/html-dom-input-datetimelocal-required-property/)
...
它返回一个布尔值，指定本地日期时间字段是否是必需的。

下面的程序说明了所需的 `DatetimeLocal` 属性：

**在提交表单之前，确定是否必须填写 `datetimeLocal` 字段。**
