...
`DOM` `ol` `type` 属性用于设置或返回有序列表中的 `type` 属性。该属性定义了您想要在数字、字母或罗马数字列表中的顺序类型（`1`、`a`、`A`、`i` 和 `I`）。
...
*   `A`：用字母顺序的大写字母定义列表项。（A, B, C, D ...）
*   `i`：以小写罗马数字顺序定义列表项。（i, ii, iii, iv ...）
*   `I`：以大写罗马数字顺序定义列表项。（I, II, III, IV ...）

**返回值：** 返回一个字符串值，代表有序列表中使用的标记类型。

**示例 1：** 演示如何设置 `type` 属性的 HTML 程序。
