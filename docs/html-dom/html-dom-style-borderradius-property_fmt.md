...
`DOM` 样式 `borderRadius` 属性用于设置或返回一个元素的四个不同的 `borderRadius` 属性，如 `borderTopRightRadius`、`borderBottomRightRadius`、`borderBottomLeftRadius`。它用于在元素中添加圆角。
...
<body>
    <center>
        ...
        <script>
            function Geeks() {
                document.getElementById("GFG").style.borderRadius = "25px";
            }
        </script>
    </center>
</body>
...
`DOM Style borderRadius` 属性支持的浏览器如下：
