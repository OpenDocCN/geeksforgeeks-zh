...
| 属性 | 描述 |
| :--- | :--- |
| `autofocus` | 它说明当页面加载时，单选按钮是否应该自动获得焦点。 |
| `checked` | 它返回单选按钮的选中状态。 |
| `defaultChecked` | 它返回选中属性的默认值。 |
| `defaultValue` | 它返回单选按钮的默认值。 |
| `disabled` | 如果单选按钮被禁用或未被禁用，它将返回。 |
| `form` | 它返回对包含单选按钮的窗体的引用。 |
| `name` | 它返回单选按钮的名称属性值。 |
| `required` | 如果在提交表单之前必须选中单选按钮，则返回。 |
| `type` | 它返回单选按钮是哪种类型的表单元素。 |
| `value` | 它返回单选按钮的 `value` 属性值。 |
...
<body>
    ...
    <h2>HTML DOM Input Radio Object</h2>
    <p>Click to create a Radio Button.</p>
