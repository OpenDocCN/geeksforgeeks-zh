...
> 原文：[https://www.geeksforgeeks.org/html-dom-style-borderspacing-property/](https://www.geeksforgeeks.org/html-dom-style-borderspacing-property/)

DOM 样式 `borderSpacing` 属性用于设置或返回表格中单元格之间的间距。
...
返回一个字符串值，代表表格中单元格之间的间距。

## 属性值

*   `length`：用于以固定单位指定单元格之间的长度。不允许负值。默认值为 0。
*   `initial`：用于将属性设置为默认值。
