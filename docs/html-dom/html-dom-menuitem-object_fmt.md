...
**DOM 菜单项对象**用于表示一个 HTML `<menuitem>` 元素。这个标签定义了上下文菜单中每个菜单项的内容和动作。这也可以配置为某些类型的输入，如复选框和单选按钮。
...
*   `icon`: 该属性设置或返回可用于表示菜单项的图像。
*   `label`: 该属性设置或返回菜单项标签的值。标签是向用户显示的文本。
*   `type`: 该属性设置或返回菜单项类型的值。这允许将菜单项更改为复选框或单选按钮。
*   `checked`: 该属性设置或返回是否默认选中菜单项。
*   `command`: 该属性设置或返回菜单项的命令属性值。
*   `disabled`: 该属性设置或返回的是默认情况下应禁用的菜单项，使其不可选择。
*   `radiogroup`: 该属性设置或返回菜单项 `radiogroup` 的值。这用于区分单选按钮组。
*   `default`: 该属性设置或返回的是菜单项应有的默认命令。这可用于提交表单。
