...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-number-value-property/](https://www.geeksforgeeks.org/html-dom-input-number-value-property/)

HTML DOM 中的 **DOM 输入数字值属性**用于设置或返回数字字段的 `value` 属性的值。`value` 属性指定输入数字字段的初始值。它包含默认值或用户输入的类型。
...
<script> 
            function myFunction() {
                // Accessing input value 
                var x = document.getElementById("myNumber").valued = "20"; 
