...
> 原文: [https://www.geeksforgeeks.org/html-dom-select-value-property/](https://www.geeksforgeeks.org/html-dom-select-value-property/)

## 概述

Select value 属性用于设置或在下拉列表中返回所选选项的值。
...
*   `value`: 用于指定下拉列表中应被选中的 `<option>` 元素的值。

## 返回值

一个字符串，表示下拉列表中 `<option>` 元素的 `value` 属性的值。如果下拉列表允许多个选择，则返回第一个选择的选项。如果没有选定的选项，则不返回任何内容。

下面的程序说明了 Select value 属性:
