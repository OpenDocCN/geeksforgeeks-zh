...
> 原文：[https://www.geeksforgeeks.org/html-dom-optiongroup-disabled-property/](https://www.geeksforgeeks.org/html-dom-optiongroup-disabled-property/)

HTML DOM 中的**选项组禁用属性**用于设置或返回选项组禁用属性。该属性检查 `OptGroup` 是否被禁用。禁用的 `<optgroup>` 元素不可点击且不可用。它是一个布尔属性。
