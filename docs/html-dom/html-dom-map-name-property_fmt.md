# HTML DOM Map name 属性

> 原文: [https://www.geeksforgeeks.org/html-dom-map-name-property/](https://www.geeksforgeeks.org/html-dom-map-name-property/)

HTML DOM 中的 `地图名称属性` 用于设置或返回 `map` 元素的 `name` 属性值。此属性指定映射图像的名称。该属性与 `<img>` 的 `usemap` 属性相关联。它在 `<image>` 和 `<map>` 元素之间创建关系。
