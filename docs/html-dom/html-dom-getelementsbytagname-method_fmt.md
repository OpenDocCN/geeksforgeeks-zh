...
> 原文: [https://www.geeksforgeeks.org/html-dom-getelementsbytagname-method/](https://www.geeksforgeeks.org/html-dom-getelementsbytagname-method/)
...
*   `elements` 是所有找到的元素的集合，它们以给定的标签名出现的顺序排列。
*   `name` 是表示元素名称的字符串。特殊字符串 `"*"` 代表所有元素。
...
下面列出了 `getElementsByTagName()` 方法支持的浏览器:
