...
**DOM 主体对象**用于表示 HTML `<body>` 元素。主体元素由 `getElementsByTagName()` 访问。也可以使用 `document.body` 对象进行访问。

## 对象属性

*   `alink`: 用于设置或返回文档中活动链接的颜色。
*   `background`: 用于设置或返回文档的背景图像。
*   `bgColor`: 用于设置或返回文档的 backgroundColor。
*   `link`: 用于设置或返回文档中未访问链接的颜色。
*   `text`: 用于设置或返回文档中文本的颜色。
...
可以使用 `document.createElement()` 方法创建身体对象。
