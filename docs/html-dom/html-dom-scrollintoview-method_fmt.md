...
> 原文: [https://www.geeksforgeeks.org/html-dom-scrollintoview-method/](https://www.geeksforgeeks.org/html-dom-scrollintoview-method/)
...
*   `alignTo`: 这是一个布尔类型参数，包含 `true` 或 `false` 值。默认值设置为 `true`。
    *   `true`: 将元素滚动到其窗口顶部。
    *   `false`: 将元素滚动到其窗口底部。
...
决定滚动平滑度的`behavior`对象有“平滑”和“自动”两种模式。
`block`对象确定元素视图应该从块的哪个部分开始。
`inline`对象确定视图应该从元素的哪个对齐位置开始。
