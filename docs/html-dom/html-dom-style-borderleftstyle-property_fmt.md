...
> 原文：[https://www.geeksforgeeks.org/html-dom-style-borderleftstyle-property/](https://www.geeksforgeeks.org/html-dom-style-borderleftstyle-property/)

HTML DOM 中的 `borderLeftStyle` 属性用于设置或返回元素的左边框样式。
...
| --- | --- |
| `none` | 没有创建边框。这是默认值。 |
| `hidden` | 它与 `none` 属性相同，只是它在表元素的边界冲突解决过程中有所帮助。 |
| `dotted` | 圆点用作边框。 |
| `dashed` | 虚线用作边框。 |
| `solid` | 一条实线用作边框。 |
| `double` | 两条线用作边框。 |
...
由`DOM Style bordereftstyle 属性`支持的浏览器如下:
