...
> 原文: [https://www.geeksforgeeks.org/html-dom-style-animationdelay-property/](https://www.geeksforgeeks.org/html-dom-style-animationdelay-property/)

HTML DOM 中的 `animationDelay` 属性用于设置或返回延迟，在此之后动画应该开始。
...
*   `time`: 用于定义动画延迟的秒数或毫秒数。默认值为 0。
*   `initial`: 用于将属性设置为默认值。
*   `inherit`: 用于从其父代继承财产。

## 返回值

返回一个字符串值，代表元素的动画延迟属性。

## 示例-1

动画延迟 3 秒，使用 `time` 属性值。
...
以下列出了 `DOM Style animation delay Property` 支持的浏览器:
