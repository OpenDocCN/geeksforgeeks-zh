...
HTML DOM 中的 `OptionGroup` 对象用于表示 HTML `<optgroup>` 元素。使用 `getElementById()` 方法可以访问 `<optgroup>` 元素。

## 属性值

该对象包含两个属性值，如下所示:

*   `disabled`: 用于设置或返回选项组元素是否禁用。
*   `label`: 用于设置或返回选项组元素的标签属性的值。
...
可以使用 `document.createElement()` 方法创建选项组对象。
