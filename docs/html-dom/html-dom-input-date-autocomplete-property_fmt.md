...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-date-autocomplete-property/](https://www.geeksforgeeks.org/html-dom-input-date-autocomplete-property/)

HTML DOM 中的`输入日期自动完成属性`用于设置或返回输入日期字段的自动完成属性值。自动完成属性用于指定自动完成属性的值是“开”还是“关”。当`autocomplete`属性设置为“开”时，浏览器将根据用户之前输入的值自动完成。
...
<script>
        function My_Date() {
            ...
            document.getElementById("test").innerHTML = "The value was changed to " + d;
        }
    </script>
</body>
