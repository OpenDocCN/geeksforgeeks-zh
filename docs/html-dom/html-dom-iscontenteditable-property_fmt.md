...
> 原文：[https://www.geeksforgeeks.org/html-dom-iscontenteditable-property/](https://www.geeksforgeeks.org/html-dom-iscontenteditable-property/)

DOM 的 `isContentEditable` 属性用于返回布尔值，其中 `true` 表示元素的内容可编辑，`false` 表示内容不可编辑。此属性是只读的。
...
**支持的浏览器：** `isContentEditable` 属性支持的浏览器如下：
*   谷歌 Chrome 11.0
