...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-text-name-property/](https://www.geeksforgeeks.org/html-dom-input-text-name-property/)

HTML DOM 中的 `DOM输入文本名称属性` 用于设置或返回文本字段的名称属性值。每个输入字段都需要名称属性。如果没有在输入字段中指定 `name` 属性，则根本不会发送该字段的数据。
