...
> 原文：[https://www.geeksforgeeks.org/html-dom-location-origin-property/](https://www.geeksforgeeks.org/html-dom-location-origin-property/)
...
该方法返回一个字符串值，代表协议、域名（或 IP 地址）和端口号。具有`"文件://"`协议的 URL 可能会根据浏览器返回不同的值。
...
由 `DOM Location origin` 属性支持的浏览器如下：
