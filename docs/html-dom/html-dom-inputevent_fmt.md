...
当用户更改`<input>`元素、`<textarea>`元素值或`contenteditable`元素时，触发`input`事件。
当 HTML 文档中的一个元素从用户那里获得输入时，就会发生 DOM `InputEvent`。

## 输入事件属性

*   `data`: 返回插入的字符。
*   `dataTransfer`: 返回包含插入/删除数据信息的对象。
*   `getTargetRanges`: 返回一个数组，该数组包含将受插入/删除影响的目标范围。
*   `inputType`: 返回变更的类型(即“insertText”或“deleteContentBackward”)
*   `isComposing`: 返回事件的状态。
...
## 示例-1: 使用`event.inputType`访问输入类型
