...
> 原文：[https://www.geeksforgeeks.org/html-dom-output-name-property/](https://www.geeksforgeeks.org/html-dom-output-name-property/)

HTML DOM 中的**输出名称属性**用于设置或返回 `<output>` 元素的 `name` 属性的值。`name` 属性用于在提交表单后引用表单数据或引用 JavaScript 中的元素。
...
`HTML DOM Output name Property` 支持的浏览器如下：
