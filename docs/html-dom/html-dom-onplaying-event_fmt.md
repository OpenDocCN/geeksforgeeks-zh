...
`onplaying` 事件发生在音频/视频暂停并在缓冲区后播放时。
...
<body>
    <center>
        ...
        <video controls id="vidID" width="320" height="240">
            <source src="https://media.geeksforgeeks.org/wp-content/uploads/20190401140735/g4g2.mp4" type="video/mp4">
        </video>
        <script>
            document.getElementById("vidID").addEventListener("playing", GFGfun);
            function GFGfun() {
                alert("Video playing");
            }
        </script>
...
`HTML DOM onplaying Event` 支持的浏览器如下:
