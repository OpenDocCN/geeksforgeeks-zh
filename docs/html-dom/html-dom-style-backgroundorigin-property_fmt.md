# HTML DOM Style backgroundOrigin 属性

> 原文：[https://www.geeksforgeeks.org/html-dom-style-backgroundorigin-property/](https://www.geeksforgeeks.org/html-dom-style-backgroundorigin-property/)

HTML DOM 中的 `Style backgroundOrigin` 属性用于确定背景图像在位置上的相对位置。它可以相对于填充、边框或实际内容进行制作。
...
返回一个字符串值，代表元素的背景原点属性。

## 属性值

### 填充框
该值相对于左上角的填充边缘定位图像。这是默认值。

### 边框
该值相对于边框边缘定位图像，边框边缘是绝对的左上角。
...
由`DOM Style backgroundOrigin`属性支持的浏览器如下:
