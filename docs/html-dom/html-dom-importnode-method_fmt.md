...
`HTML | DOM importNode()`方法从另一个文档创建一个节点的副本，并将其导入到当前文档中。
...
*   `externalNode`: 我们需要从另一个文档导入的节点，可以是任何类型。
*   `deep`: 如果要复制节点及其属性和子节点，我们可以将其值设置为`"true"`；如果只想复制节点及其属性，我们可以设置为`"false"`。如果我们没有指定任何参数，那么`deep`的值默认为`"true"`。
...
Internet explorer 8 及更早版本不支持`importNode()`方法。

## 支持的浏览器:

支持`HTML|DOM importNode()`方法的浏览器如下。
