...
*   `type`：设置或返回嵌入对象的类型属性值。
*   `width`：设置或返回嵌入对象的宽度属性值。
...
其中 `ID` 是分配给 `<embed>` 标签的标识。

## 示例

### 示例 1：使用 getElementById() 方法

本示例描述了访问 `<embed>` 元素的 `getElementById()` 方法。
...
### 示例 2：使用 createElement() 方法

嵌入对象可以使用 `document.createElement()` 方法创建。
