...
HTML 中的 **DOM 脚本集合**用于返回 HTML 文档中所有 `<script>` 元素的集合。脚本元素按照源代码中的显示进行排序。
...
返回单值 `length`，即所有脚本元素的集合。

## 方法:

*   `[index]`: 用于返回所选索引的 `<script>` 元素。索引值以 0 开始。如果索引值超出范围，则返回空值。
*   `item(index)`: 用于返回所选索引的 `<script>` 元素。索引值以 0 开始。如果索引值超出范围，则返回空值。
*   `namedItem(id)`: 用于返回集合中给定 `id` 属性的 `<script>` 元素。如果 `id` 无效，它将返回 `NULL`。
