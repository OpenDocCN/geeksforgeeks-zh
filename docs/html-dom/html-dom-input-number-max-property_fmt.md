...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-number-max-property/](https://www.geeksforgeeks.org/html-dom-input-number-max-property/)

HTML DOM 中的 `DOM 输入数字最大属性` 用于设置或返回数字字段的 `max` 属性。`max` 属性定义了数字字段的最大值。
...
**属性值：** 它包含一个单一的值，即 `number`，该数字指定了允许用户为数字字段输入的最大值。
...
<script>
            function myFunction() {
                // Accessing input value
                var x = document.getElementById("myNumber").max = 15;
...
**支持的浏览器：** `DOM Input Number max Property` 支持的浏览器如下：
