...
> 原文：[https://www.geeksforgeeks.org/html-dom-geolocation-position-property/](https://www.geeksforgeeks.org/html-dom-geolocation-position-property/)

HTML DOM 中的地理位置属性用于返回设备在地球上的位置。返回的坐标对象可以用于各种目的，包括导航和跟踪设备的位置。
...
`getCurrentPosition()` 或 `watchPosition()` 之类的方法用于向函数传递回调，然后访问 `Position` 属性。
