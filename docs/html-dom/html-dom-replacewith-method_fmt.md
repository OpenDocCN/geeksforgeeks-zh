...
`replaceWith()` 方法用一组 `Node` 或 `DOMString` 对象替换其父节点的子节点列表中的一个节点。多字符串对象相当于文本节点。这里一个子元素被其他子元素代替。
...
*   在第 12 行，在应用 `replaceWith()` 方法之前，父元素的外部 HTML 将 `<p>` 元素作为子节点。
*   在第 16 行，应用此方法后，父元素将 `<div>` 元素替换为子节点。
