...
> 原文: [https://www.geeksforgeeks.org/html-dom-columngroup-span-property/](https://www.geeksforgeeks.org/html-dom-columngroup-span-property/)

HTML DOM 中的 `列组跨度属性` 用于设置或返回列组元素跨度属性的值。`跨度属性` 用于定义 `<colgroup>` 元素应跨越的列数。
...
包含单值 `number`，指定 `<colgroup>` 应跨越的数量。
