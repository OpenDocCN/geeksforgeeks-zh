...
> 原文：`https://www.geeksforgeeks.org/html-dom-onsearch-event/`

当用户使用带有`<输入>`元素进行搜索时，会发生 HTML DOM 中的`onsearch`事件。当用户按下“回车”或在搜索框中点击“x”时，该事件生效。
...
`HTML DOM onsearch Event`支持的浏览器如下：
