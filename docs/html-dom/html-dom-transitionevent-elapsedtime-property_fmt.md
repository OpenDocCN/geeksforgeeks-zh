...
> 原文：[https://www.geeksforgeeks.org/html-dom-transitionevent-elapsedtime-property/](https://www.geeksforgeeks.org/html-dom-transitionevent-elapsedtime-property/)

`TransitionEvent elapsedTime` 属性用于返回当 `transitionend` 事件发生时，过渡动画已经运行的秒数。
...
<head>
    <title>TransitionEvent elapsedTime Property</title>
...
<body>
    ...
    <p>Hover over the element to see the number of seconds the transition effect has been running. </p>

    <div id="div1"></div>

    <script>
        document.getElementById("div1").addEventListener("transitionend", myevent);

        function myevent(event) {
            // Return seconds.
            this.innerHTML = "Transition time is : " + event.elapsedTime + " seconds";
        }
    </script>
