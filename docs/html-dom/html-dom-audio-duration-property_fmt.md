...
> 原文: [https://www.geeksforgeeks.org/html-dom-audio-duration-property/](https://www.geeksforgeeks.org/html-dom-audio-duration-property/)

`音频持续时间属性`用于返回音频的长度。`音频持续时间属性`返回以秒为单位的值。
不同的浏览器返回不同的精度值，例如 safari 最多返回 14 位小数，opera 最多返回 9 位小数。
`音频持续时间属性`是只读属性。
如果没有设置视频，`音频持续时间`功能返回`"NAn"`，而如果音频是流式的并且没有预定义的长度，则返回`"Inf"`(无穷大)。
