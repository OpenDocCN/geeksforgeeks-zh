...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-submit-name-property/](https://www.geeksforgeeks.org/html-dom-input-submit-name-property/)

HTML DOM 中的**输入提交名称属性**用于设置或返回提交字段的名称属性值。每个输入字段都需要`name`属性。如果没有在输入字段中指定`name`属性，则根本不会发送该字段的数据。
...
## 属性值
它包含定义提交字段名称的单个值`name`。

## 返回值
返回代表提交字段名称的字符串值。

## 示例 1
本示例说明如何返回 Input 提交名称属性。
