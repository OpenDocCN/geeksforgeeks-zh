...
`DOM 字段集对象`用于表示 HTML `<fieldset>` 元素。通过 `getElementById()` 访问字段集元素。

## 属性

*   `disabled`: `disabled` 属性用于设置或返回字段集是否被禁用。
*   `form`: 用于返回对包含字段集的表单的引用。
*   `name`: 字段名称属性的值由 `name` 设置或返回。
*   `type`: 返回表单的类型。
...
**点击按钮后:**
![](img/82ece8d57c7c8cff522c0fb1f5643899.png)

## 示例-2

可以使用 `document.createElement` 方法创建字段集对象。
