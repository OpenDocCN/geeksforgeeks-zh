...
> 原文: [https://www.geeksforgeeks.org/html-dom-offsetparent-property/](https://www.geeksforgeeks.org/html-dom-offsetparent-property/)

在 HTML 中，`offsetParent` 属性用于返回元素的最近祖先。返回的祖先必须有一个非静态的位置。如果元素设置为 `display="none"`，则 `offsetParent` 属性返回空值。
...
本示例使用 `display` 属性隐藏 `offsetParent` 值。
