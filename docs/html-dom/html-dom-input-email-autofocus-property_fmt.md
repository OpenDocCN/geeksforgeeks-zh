...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-email-autofocus-property/](https://www.geeksforgeeks.org/html-dom-input-email-autofocus-property/)

HTML DOM 中的**输入邮件自动对焦属性**用于设置或返回页面加载时输入邮件字段是否应该对焦。它反映了 HTML `autofocus` 属性。
...
*   `true`: 设置邮件字段的焦点。
*   `false`: 它具有默认值。定义邮件字段不获取焦点。

## 返回值

返回一个布尔值，代表邮件字段是否自动对焦。

## 示例

本示例返回输入电子邮件 `autofocus` 属性。
