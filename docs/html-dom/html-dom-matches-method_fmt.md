...
`matches()`方法检查所提供的选择器字符串选择的元素是否是选择器。如果指定的元素是有效的选择器，那么它将返回`true`，否则`false`。
...
**返回值:** 返回布尔值（即选择器匹配其他时为`true`，`false`）。

**示例:** 在本例中，有一个列表，其中有一些元素的类选择器为`number`。因此，我们将列表存储在一个变量中，通过对列表进行迭代，如果选择器匹配`number`类，则相应地在文档上打印。
