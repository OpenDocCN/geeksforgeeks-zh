...
HTML 中的 `DOM onloadeddata` 事件发生在加载了当前帧数据，但下一帧的数据不足以播放音频/视频时。
...
<script>
            document.getElementById("VideoId").addEventListener("loadeddata", GFGFun);
...
**支持的浏览器:**
`HTML DOM onloadeddata` 事件支持的浏览器如下:
