...
`tabIndex` 属性用于返回元素的 `tabIndex` 属性的值。`tabindex` 属性指定元素的制表符顺序(当制表符按钮用于导航时使用。)
...
<body>
    ...
    <script>
        function gfgFun() {
            var gfgvar = document.getElementsByTagName("A")[0].tabIndex;
            document.getElementById("gfg").innerHTML = gfgvar;
        }
    </script>
</body>
