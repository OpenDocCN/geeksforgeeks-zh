...
> 原文：`https://www.geeksforgeeks.org/html-dom-execcommand-method/`
...
### 命令：
该参数保存在所选部分执行的命令的名称。HTML 中的命令有很多，其中有：`反白`、`加粗`、`复制`、`剪切`、`删除`等。
...
## 支持的浏览器：
下面列出了 `DOM execCommand()方法`支持的浏览器：
