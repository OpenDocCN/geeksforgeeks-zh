...
> 原文：[https://www.geeksforgeeks.org/html-dom-style-animationiterationcount-property/](https://www.geeksforgeeks.org/html-dom-style-animationiterationcount-property/)
...
*   `number`：用于设置动画播放的次数。它的默认值是 1。
*   `infinite`：它设置动画将播放无限次。
*   `initial`：它将动画迭代计数属性设置为默认值。
...
<body>
    <p>
        Click on the button to set animation iteration count!
    </p>
    ...
    <script>
        ...
        function myGeeks() {
            document.getElementById("GFG").style.WebkitAnimationIterationCount = "10";
...
<body>
    ...
    <script>
        function myGeeks() {
            /* For Chrome, Safari, and Opera browsers */
            document.getElementById("GFG").style.WebkitAnimationIterationCount = "infinite";
            document.getElementById("GFG").style.animationIterationCount = "infinite";
        }
    </script>
    <br>
    <div id="GFG">
...
由 `DOM Style animationIterationCount` 属性支持的浏览器如下：
