...
> 原文：`https://www.geeksforgeeks.org/html-dom-normalizedocument-method/`
...
该方法不被任何浏览器支持，但作为 [`DOM normalize()` 方法](https://www.geeksforgeeks.org/html-dom-normalize-method/)工作并显示输出。
...
<body>
    <script type="text/javascript">
        ...
        alert('Before Normalize : child nodes length: ' + parent.childNodes.length);
        parent.normalize();
        ...
        alert('After Normalize : child nodes length: ' + parent.childNodes.length);
    </script>
</body>
</html>
```
...
主要浏览器不支持 `DOM normalized document()` 方法。