...
> 原文：[https://www.geeksforgeeks.org/html-dom-tablerow-deletecell-method/](https://www.geeksforgeeks.org/html-dom-tablerow-deletecell-method/)

HTML DOM 中的 `TableRow.deleteCell()`方法用于从表格的当前行中删除一个单元格。它是 `TableRow` 对象的预定义方法。
...
*   **索引：**`index`。它包含一个从 0 开始的数值，该数值定义了要从当前行删除的单元格的位置。例如-值 0 表示要删除的第一个定位单元格。值 -1 表示删除最后定位单元格。
