...
**DOM 选项对象**用于表示 HTML [`<option>`](https://www.geeksforgeeks.org/html-option-tag/) 元素。通过 `getElementById()` 可以访问选项元素。

## 属性

*   `disabled`：该属性包含 `disabled` 的值，表示选项被禁用。
*   `label`：该属性包含代表选项短标签的文本值。
*   `selected`：该属性包含 `selected` 的值，该值代表浏览器加载时预先选定的项目。
*   `value`：该属性包含发送到服务器的值文本。
...
<body>
    ...
    <script>
        function geeks() {
            ...
            var g = document.getElementById("GFG").text;
...
选项对象可以使用 `document.createElement()` 方法创建。
...
<body>
    ...
    <script>
        function geeks() {
            ...
            var f = document.createTextNode("Choose an Option");
            g.appendChild(f);
