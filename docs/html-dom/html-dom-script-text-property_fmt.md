...
> 原文：[https://www.geeksforgeeks.org/html-dom-script-text-property/](https://www.geeksforgeeks.org/html-dom-script-text-property/)

`HTML DOM script.text` 属性用于**设置**或**返回** `<script>` 元素的内容。
...
包含值，即 `contents`，指定 `<script>` 元素的内容。

## 返回值

返回一个代表脚本元素内容的字符串值。即脚本的所有文本节点，但忽略注释。
...
`HTML DOM script.text` 属性支持的浏览器如下：
