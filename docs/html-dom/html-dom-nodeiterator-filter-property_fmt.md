...
> 原文：[https://www.geeksforgeeks.org/html-dom-nodeiterator-filter-property/](https://www.geeksforgeeks.org/html-dom-nodeiterator-filter-property/)

`节点迭代器过滤器`属性返回一个`NodeFilter`对象，该对象实现了一个`acceptNode( *node* )`方法，用于筛选节点。
...
在本例中，我们将创建一个节点迭代器，并将使用该属性获取`NodeFilter`对象。
