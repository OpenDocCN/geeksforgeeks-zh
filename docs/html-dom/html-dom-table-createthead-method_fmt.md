...
> 原文：[https://www.geeksforgeeks.org/html-dom-table-createthead-method/](https://www.geeksforgeeks.org/html-dom-table-createthead-method/)

`表格创建方法`用于创建一个空的 `<thead>` 元素，并将其添加到表格中。如果一个 `<thead>` 元素已经存在，它不会创建新的 `<thead>` 元素。在这种情况下，`createTHead()` 方法返回现有的一个。

`<thead>` 元素内部必须有一个或多个 `<tr>` 标签。
...
**返回值：** 返回新创建的或现有的 `<thead>` 元素。

下面的程序说明了表格 `createTHead()` 方法：
