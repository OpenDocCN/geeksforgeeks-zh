...
DOM 接口的 `outerHTML` 属性给出了该元素的 HTML 片段。它不仅给出了内容，还给出了元素的整个 HTML 结构。也可以用来替换元素的 HTML 结构。
...
获取外部 HTML 时，返回 HTML 字符串数据。

## 示例 1

这个示例展示了如何获取 `id = "gfg"` 元素的外部 HTML。
...
<body>
    <h1>GeeksforGeeks</h1>
    <div id="d">
        Click on Button to change the outerHTML.
    </div>
    <br>
    ...
    <script>
        function changeouter() {
            ...
            gfg.outerHTML = "<h3>Hey Geeks! outerHTML is changed</h3>";
        }
    </script>
</body>
