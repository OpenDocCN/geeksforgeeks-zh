...
HTML DOM 中的 `IFrame` 对象属性用于创建和访问文档中的 `<iframe>` 元素。内嵌框架用于在当前的 HTML 文档中嵌入另一个文档。

## 语法

*   用于访问 `<iframe>` 元素：`var x = document.getElementById("myframe");`
*   用于创建 `<iframe>` 元素：`var x = document.createElement("IFRAME");`
...
*   `height`: 用于设置或返回 iframe 中的高度属性。
*   `longDesc`: 用于设置或返回 iframe 中 `longDesc` 属性的值。HTML 5 不支持。
*   `marginHeight`: 用于设置或返回 iframe 中 `marginHeight` 属性的值。HTML 5 不支持。
*   `marginWidth`: 用于设置或返回 iframe 中 `marginWidth` 属性的值。HTML 5 不支持。
*   `name`: 用于设置或返回 iframe 中名称属性的值。
*   `sandbox`: 用于返回 iframe 中沙盒属性的值。
*   `scrolling`: 用于设置或返回 iframe 中滚动属性的值。HTML 5 不支持。
*   `seamless`: 用于设置或返回 iframe 是否应该看起来像是包含文档的一部分。
...
本示例描述了访问 `<iframe>` 元素的 `getElementById()` 方法。

```html
<!DOCTYPE html>
<html>
<head>
    <title>HTML DOM IFrame Object Property</title>
</head>
<body>
    <h1>GeeksforGeeks</h1>
    ...
    <br><br>
    <iframe id="GFGFrame" src="https://ide.geeksforgeeks.org/tryit.php" width="400" height="200"></iframe>
    <p id="GFG"></p>
    <script>
        function myGeeks() {
...
本示例描述了 `document.createElement()` 方法来创建 `<iframe>` 元素。
...
<body>
    ...
    <script>
        function myGeeks() {
            ...
            ifram.setAttribute("src", "https://ide.geeksforgeeks.org/tryit.php");
