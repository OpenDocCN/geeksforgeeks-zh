...
> 原文：[https://www.geeksforgeeks.org/html-dom-object-type-property/](https://www.geeksforgeeks.org/html-dom-object-type-property/)

`HTML | DOM 对象类型属性`用于**设置**或**返回** `<object>`元素的类型属性值。`type`属性用于指定对象的 MIME 类型的互联网类型。
...
包含指定嵌入文件的互联网媒体类型的值，即**媒体类型**。

## 返回值

返回一个字符串值，代表嵌入文件的互联网媒体类型。
