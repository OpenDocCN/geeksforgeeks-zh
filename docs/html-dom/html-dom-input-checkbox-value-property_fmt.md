...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-checkbox-value-property/](https://www.geeksforgeeks.org/html-dom-input-checkbox-value-property/)

HTML DOM 中的`输入复选框值属性`用于设置或返回输入复选框字段的`value`属性的值，但是`value`属性的内容不会显示给用户。当用户提交表单时，该值和其他信息被发送到服务器。但是当复选框处于未选中状态时，将不会发送任何信息。
...
包含单值`text`，用于指定与输入复选框字段关联的值。

## 返回值

返回一个字符串值，代表输入复选框字段的`value`属性的值。
...
`DOM Input Checkbox 值属性`支持的浏览器如下:
