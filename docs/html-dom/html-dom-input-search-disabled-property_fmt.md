...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-search-disabled-property/](https://www.geeksforgeeks.org/html-dom-input-search-disabled-property/)

HTML DOM 中的 `DOM 输入搜索禁用属性` 用于返回一个布尔值，该值表示搜索字段是否应被禁用。默认情况下，禁用的元素以灰色显示，不可用且不可点击。
...
返回布尔值，即如果搜索字段被禁用则为真，如果搜索字段未被禁用则为假。
