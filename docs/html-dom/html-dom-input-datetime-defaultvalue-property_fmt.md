...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-datetime-defaultvalue-property/](https://www.geeksforgeeks.org/html-dom-input-datetime-defaultvalue-property/)

HTML DOM 中的**输入日期时间默认值属性**用于设置或返回日期时间字段的默认值。此属性用于反映 HTML `value` 属性。默认值与值的主要区别在于，默认值表示属性的默认值，该值包含进行一些更改后的当前值。此属性用于查找日期字段是否已更改。
...
包含单个属性值 `value`，定义输入日期时间字段的默认值。

## 返回值

返回代表输入日期时间字段默认值的字符串值。

## 示例 1

本示例说明如何返回 Input Datetime `defaultValue` 属性。
...
本示例说明如何设置 Input datetime `defaultValue` 属性。
...
**HTML DOM Input Datetime defaultValue Property** 支持的浏览器如下：
