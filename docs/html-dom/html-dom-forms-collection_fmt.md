...
*   `item(索引)`: 用于返回指定索引的 `<表单>` 元素。索引值从 0 开始。如果索引值超出范围，则返回空值。此方法的执行类似于上述方法。
*   `namedItem(id)`: 用于从集合中返回与指定 id 匹配的 `<表单>` 元素。如果 id 不存在，则返回空值。

下面的程序说明了 `document.forms` 属性在 HTML 中的使用:

## 示例 1

本示例使用 `length` 属性计算集合中表单元素的数量。
