# HTML DOM 按钮 formAction 属性

> 原文: [https://www.geeksforgeeks.org/html-dom-button-formaction-property/](https://www.geeksforgeeks.org/html-dom-button-formaction-property/)

HTML DOM 中的 `button` 表单属性用于设置或返回按钮的 `formAction` 属性的值。提交表单后，会调用 `formAction` 属性。表单数据将在表单提交后发送到服务器。
...
这个示例描述了如何设置和获取 `formAction` 属性的值。
