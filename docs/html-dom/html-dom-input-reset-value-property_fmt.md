...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-reset-value-property/](https://www.geeksforgeeks.org/html-dom-input-reset-value-property/)

HTML DOM 中的**输入重置值属性**用于设置或返回输入重置字段的`value`属性的值。`value`属性指定重置字段中显示的文本。
...
<body style="text-align:center;">
    ...
    <script>
        function myGeeks() {
            var x = document.getElementById("GeekReset").value = "Reset Button";
            document.getElementById("Geek_p").innerHTML = x;
        }
    </script>
</body>
