...
> 原文: [https://www.geeksforgeeks.org/html-dom-indexeddb-open-method/](https://www.geeksforgeeks.org/html-dom-indexeddb-open-method/)

`indexedDB` 接口的 `open()` 方法请求打开到数据库的连接。该方法立即返回一个 `IDBOpenDBRequest` 对象，异步执行打开操作。
...
在本例中，我们将使用此方法打开一个名为“toDoList”的数据库。
...
在控制台中，可以看到 `IDBOpenDBRequest` 对象和数据库数组中的数据库“toDoList”。
