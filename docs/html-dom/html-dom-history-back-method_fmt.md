# HTML DOM `history.back()` 方法

> 原文：[https://www.geeksforgeeks.org/html-dom-history-back-method/](https://www.geeksforgeeks.org/html-dom-history-back-method/)

HTML 中的 `history.back()` 方法用于加载历史列表中的上一个 URL。它与我们的网络浏览器中的后退按钮具有相同的实际应用。如果上一页不存在，此方法将不起作用。此方法不包含任何参数。
...
这里这个按钮不会起作用，因为没有以前的 url，可以尝试复制 ide url 并打开任意站点粘贴 ide 并复制粘贴代码并运行。
