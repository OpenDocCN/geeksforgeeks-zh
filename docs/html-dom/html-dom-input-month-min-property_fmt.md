...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-month-min-property/](https://www.geeksforgeeks.org/html-dom-input-month-min-property/)

HTML DOM 中的 `DOM 输入月最小值` 属性用于设置或返回月字段的最小值属性的值。`min` 属性为“月”字段指定最小的月和年值。
...
在 Firefox 中，输入 `type="month"` 元素不显示任何日期字段或日历。