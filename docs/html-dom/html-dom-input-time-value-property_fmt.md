...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-time-value-property/](https://www.geeksforgeeks.org/html-dom-input-time-value-property/)

`DOM Input Time value Property` 用于设置或返回时间字段的 `value` 属性的值。`value` 属性用于定义时间字段的时间。
...
<body>
    <center>
        ...
        <script>
            function geeks() {
                var link = document.getElementById("gfg").value = "04:23:55";
                document.getElementById("GFG").innerHTML = "The value has changed to " + link;
            }
        </script>
    </center>
</body>
...
**支持的浏览器：** 以下所列浏览器支持 `DOM Input Time value Property`：
