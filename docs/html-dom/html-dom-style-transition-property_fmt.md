...
> 原文: [https://www.geeksforgeeks.org/html-dom-style-transition-property/](https://www.geeksforgeeks.org/html-dom-style-transition-property/)
...
| `transition-duration` | 完成过渡需要多少时间 |
| `transitionTimingFunction` | 转变速度 |
| `transition-delay` | 过渡起点 |
| `initial` | 设置为默认值 |
| `inherit` | 从其父元素继承 |

## 示例

在本例中，我们正在创建一个 `div` 标签，其 CSS 在样式标签中定义，当您在单击提交按钮后将鼠标悬停在 `div` 标签上时，CSS 将从 `myDIV` CSS 更改为 `myDIV:hover` CSS。
...
过渡时长只能为*非负数*，不能为零，否则不会显示交易效果。

*   **`none`**: 无属性获得交易效果
*   **`all`**: 这是默认值所有属性都会得到交易效果。
*   **`initial`**: 将该属性设置为默认值。
*   **`inherit`**: 从其父元素继承该属性。
