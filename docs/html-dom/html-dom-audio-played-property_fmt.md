...
> 原文: [https://www.geeksforgeeks.org/html-dom-audio-played-property/](https://www.geeksforgeeks.org/html-dom-audio-played-property/)

**音频播放属性**用于返回一个时间范围对象。“时间范围”对象用于表示用户已经播放的音频范围的情况。

播放范围是播放音频的时间范围。如果用户跳过音频，他可能会获得几个播放范围。`played`音频属性为只读属性。

时间范围对象属性包括:

*   `length` – 用于获取视频中播放范围的数量。
*   `start(index)` – 用于获取一个播放范围的开始位置。
