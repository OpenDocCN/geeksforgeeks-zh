...
> 原文：[https://www.geeksforgeeks.org/html-dom-form-elements-collection/](https://www.geeksforgeeks.org/html-dom-form-elements-collection/)

HTML DOM 中的**表单元素集合**用于设置或返回一个[表单元素](https://www.geeksforgeeks.org/html-forms/)内所有 [`<input>`](https://www.geeksforgeeks.org/html-input-tag/) 元素的集合。`<input>`元素在源代码中出现时进行排序。
...
**属性：** 返回`<form>`元素内的多个输入元素。

**方法：** DOM 表单元素集合包含以下列出的三种方法：

*   `[index]`：用于返回指定索引的`<form>`元素内的`<input>`元素。索引值从 0 开始。如果索引值超出范围，则返回空值。
*   `item(index)`：用于返回指定索引的`<form>`元素内的`<input>`元素。索引值从 0 开始。如果索引值超出范围，则返回空值。此方法的执行类似于上述方法。
*   `namedItem(id)`：用于从集合中返回符合指定 id 的`<input>`元素。如果 id 不存在，则返回空值。

**返回值：** 表单元素集合返回表单元素内所有`<input>`元素的集合。
