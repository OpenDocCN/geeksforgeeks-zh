...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-password-size-property/](https://www.geeksforgeeks.org/html-dom-input-password-size-property/)

**DOM 输入密码大小属性**用于设置或返回输入密码字段的 `size` 属性值。`size` 属性用于定义电子邮件字段的宽度。它的默认值是 20。
...
包含单个值 `数字`，用于根据字符数指定密码字段的宽度。

## 返回值

返回代表密码字段宽度的数值，以字符数表示。
