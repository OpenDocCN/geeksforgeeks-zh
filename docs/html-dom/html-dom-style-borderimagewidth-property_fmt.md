...
返回一个字符串值，代表元素的边框图像宽度属性。

## 属性值

*   **number:** 用于将宽度设置为对应 `border-width` 计算值的倍数。当设置为 1 时，这是默认值。
...
<html>
<head>
    <title>DOM Style borderImageWidth Property</title>
    <style>
        .item {
            height: 70px;
            border: 10px solid transparent;
            border-image: url('https://media.geeksforgeeks.org/wp-content/uploads/border-2.png');
...
<body>
    <h2>DOM Style borderImageWidth Property</h2>
    <p>Click on the button to change the width of border-image</p>
    <div class="item">GeeksforGeeks</div>
    <button onclick="changeWidth()">Click Here!</button>
    <script>
...
*   **percentage:** 用于以百分比设置宽度。百分比相对于边框图像区域的宽度（水平偏移）和高度（垂直偏移）。
...
*   **initial:** 用于将 `borderImageWidth` 属性设置为其默认值。
...
<body>
    ...
    <p>
        Click on the button to change the width of border-image
    </p>
    <div class = "Geeks">
        <div class = "item">
            GeeksforGeeks
