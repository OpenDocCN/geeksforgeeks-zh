...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-datetime-required-property/](https://www.geeksforgeeks.org/html-dom-input-datetime-required-property/)

输入`required`的日期时间属性用于在提交表单之前检查日期时间字段是否必须填写。
...
它返回一个布尔值，该值表示输入 Datetime 字段是否是必需的。
