...
> 原文: [https://www.geeksforgeeks.org/html-dom-touchevent-ctrlkey-property/](https://www.geeksforgeeks.org/html-dom-touchevent-ctrlkey-property/)

`TouchEvent` `ctrlKey` 属性是只读属性，用于返回一个布尔值，该值指示当触摸事件被触发时是否按下了 `CTRL` 键。它通常返回 `false`，因为一般来说，触摸设备没有 `ctrl` 键。
...
**返回值:** 按下 `ctrl` 键返回 `true`，否则返回 `false`。
