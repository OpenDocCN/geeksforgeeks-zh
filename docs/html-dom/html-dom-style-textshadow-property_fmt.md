...
> 原文：[https://www.geeksforgeeks.org/html-dom-style-textshadow-property/](https://www.geeksforgeeks.org/html-dom-style-textshadow-property/)

HTML DOM 中的 `style.textShadow` 属性用于设置文本的阴影效果。我们可以使用这个属性设置多个阴影效果。此属性还可以返回文本的阴影效果。
...
*   `none`：该属性使得不绘制阴影。这是默认值。
*   `h-shadow`：用于定位水平阴影，允许负值。这是必填字段。
*   `v-shadow`：用于定位垂直阴影，允许负值。这是必填字段。
*   `blur`：可选，指定模糊距离。
*   `color`：可选，用于指定阴影的颜色。
*   `initial`：它将 `textShadow` 属性设置为默认值。
    ...
    <center>
        ...
        <script>
            function geeks() {
                ...
                document.getElementById("gfg").style.textShadow = "5px 5px 1px green";
            }
        </script>
    </center>
</body>
...
<body>
    <center>
        ...
        <h2>DOM Style textShadow Property </h2>
        <p id="gfg">A Computer science portal for geeks</p>
        <button type="button" onclick="geeks()">Shadow effect</button>
        <script>
            function geeks() {
                // Set textShadow in magenta color.
                document.getElementById("gfg").style.textShadow = "-3px -5px 1px magenta";
            }
        </script>
    </center>
</body>
