...
`after()` 方法用于在 `父节点` 的子节点列表中，在指定的 `子节点` 之后插入一组 `Node` 对象或 `HTML DOMString` 对象。
...
*   `Node`: 它是一组 `Node` 对象或 `HTML DOMString` 对象，必须插入到子节点之后。
...
在输出中，可以看到点击按钮后，在 `<p>` 元素的子元素后插入了一个新节点。
