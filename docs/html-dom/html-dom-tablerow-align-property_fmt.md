...
> 原文: [https://www.geeksforgeeks.org/html-dom-tablerow-align-property/](https://www.geeksforgeeks.org/html-dom-tablerow-align-property/)
...
*   `left`: 设置左对齐到 `TableRow`。
*   `center`: 将居中对齐设置为 `TableRow`。这是默认值。
*   `right`: 设置右对齐 `TableRow`。
*   `justify`: 拉伸段落文本，使所有行的宽度相等。
*   `char`: 它将文本对齐设置为特定字符。

## 返回值

返回一个字符串值，代表 `TableRow` 元素的对齐方式。
...
`DOM TableRow align Property` 支持的浏览器如下:
