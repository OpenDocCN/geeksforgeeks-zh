...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-search-autocomplete-property/](https://www.geeksforgeeks.org/html-dom-input-search-autocomplete-property/)

`DOM 输入搜索自动完成属性`用于`设置`或`返回``输入搜索字段`的自动完成属性值。自动完成属性用于指定自动完成属性是`“开”`还是`“关”`值。当`autocomplete`属性设置为`"on"`时，浏览器将根据用户之前输入的值自动完成。
