...
> 原文：[https://www.geeksforgeeks.org/html-dom-style-cssfloat-property/](https://www.geeksforgeeks.org/html-dom-style-cssfloat-property/)
...
返回一个字符串值，代表元素的水平对齐方式。

## 属性值

*   `none`：为默认值。该值不会产生浮点。
*   `left`：将元素浮动到父体/容器的左侧。
*   `right`：将元素浮动到父体/容器的右侧。
*   `initial`：将元素设置为其初始位置。
*   `inherit`：元素从父元素继承其浮动属性。
...
<body>
    ...
    <script>
        function RightFloat() {
            document.getElementById("GFG").style.cssFloat = "right";
        }
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        ...
        function LeftFloat() {
            document.getElementById("GFG").style.cssFloat = "left";
        }
    </script>
</body>
</html>
