...
> 原文: [https://www.geeksforgeeks.org/html-dom-video-paused-property/](https://www.geeksforgeeks.org/html-dom-video-paused-property/)

HTML DOM 中的 `paused` 属性用于返回视频是否暂停。它是只读属性。
...
如果视频暂停，`paused` 属性返回布尔值 `true`，否则返回 `false`。

下面的程序说明了 HTML DOM 中的 `paused` 属性:
