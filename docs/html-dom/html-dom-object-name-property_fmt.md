...
> 原文: [https://www.geeksforgeeks.org/html-dom-object-name-property/](https://www.geeksforgeeks.org/html-dom-object-name-property/)

**HTML | DOM 对象名称属性**用于**设置**或**返回** `<object>` 元素的 `name` 属性值。`name` 属性用于指定嵌入文件的名称。
...
**属性值:** 包含用于指定嵌入文件名称的值，即 `name`。
**返回值:** 返回一个代表 `<object>` 元素名称的字符串值。
...
<body>
    ...
    <script>
        function Geeks() {
            // Accessing Object element.
            var x = document.getElementById("myobject").name = "Obj1";
