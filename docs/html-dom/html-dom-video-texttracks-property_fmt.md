...
> 原文: [https://www.geeksforgeeks.org/html-dom-video-texttracks-property/](https://www.geeksforgeeks.org/html-dom-video-texttracks-property/)

`视频文本跟踪属性`用于返回一个文本跟踪列表对象。
...
1.  **文本轨迹列表对象**: 表示视频可用的文本轨迹。
    *   `length`: 用于获取视频中可用的文本轨道数。
    *   `[index]`: 用于通过索引获取 `TextTrack` 对象。
2.  **TextTrack 对象**: 表示一个文本轨迹。
    *   `kind`: 用于获取文字轨迹的类型。
    *   `label`: 用于获取文本轨道的标签。
    *   `language`: 用于获取文本轨迹的语言。
    *   `mode`: 用于获取或设置轨道是否激活。
    *   `cues`: 用于获取一个作为 `TextTrackCueList` 对象的提示列表。
    *   `activeCues`: 用于获取当前活动的文本轨迹线索，作为 `TextTrackCueList` 对象。
    *   `addCue(cue)`: 用于向提示列表中添加提示。
    *   `removeCue(cue)`: 用于从提示列表中移除提示。
...
以下列出了 `HTML | DOM 视频文本跟踪属性`支持的浏览器:
