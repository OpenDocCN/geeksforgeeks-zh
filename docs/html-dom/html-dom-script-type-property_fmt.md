...
> 原文: [https://www.geeksforgeeks.org/html-dom-script-type-property/](https://www.geeksforgeeks.org/html-dom-script-type-property/)

**DOM 脚本类型属性**用于**设置**或**返回** `<script>` 元素的类型属性的值。类型属性用于指定脚本的 MIME 类型，并识别 `<script>` 标签的内容。它的默认值是 `"text/javascript"`。
