...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-date-stepup-method/](https://www.geeksforgeeks.org/html-dom-input-date-stepup-method/)

HTML DOM 中的 `inputDate.stepUp()` 方法用于将日期字段的值增加一个指定的数字。`stepUp()` 方法只能用于日，不能用于月和年。
...
该方法接受单参数 `number`，用于指定日期字段应增加的天数。

下面的程序说明了在超文本标记语言中输入日期的方法：

## 示例

本示例使用 `InputDate.stepUp()` 方法将日期字段的值增加 2 天。
