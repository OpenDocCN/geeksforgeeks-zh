...
> 原文: [https://www.geeksforgeeks.org/html-dom-ownerdocument-property/](https://www.geeksforgeeks.org/html-dom-ownerdocument-property/)
...
### `nodeName` 属性
返回指定节点的名称。如果节点是元素节点，它将返回标记名 else 如果节点是属性节点，它将返回属性名 else 对于不同的节点类型，将返回不同的名称。
...
### `nodeType` 属性
返回节点的类型，以数字表示。例如：1 表示元素节点，2 表示属性节点。
