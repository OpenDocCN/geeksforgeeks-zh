...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-datetimelocal-step-property/](https://www.geeksforgeeks.org/html-dom-input-datetimelocal-step-property/)

HTML DOM 中的 `输入日期时间本地步骤属性` 用于设置或返回本地日期时间字段的 `step` 属性的值。`step` 属性可用于在本地日期时间字段中指定秒或毫秒的合法数字间隔。`step` 属性不能用于影响天、月、年、小时或分钟。
...
包含单个属性值 `数字`，用于指定秒或毫秒的法定数字间隔。

## 返回值

返回一个数值，代表以秒或毫秒为单位的合法间隔数。

下面的程序说明了 HTML DOM 中的 `DatetimeLocal` `step` 属性：

## 示例

本示例更改了本地日期字段中的法定秒数间隔。
