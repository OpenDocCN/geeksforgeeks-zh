...
**DOM 跨度对象**用于表示 HTML `<span>` 元素。使用 `getElementById()` 方法可以访问 span 元素。
...
其中 `"id"` 是分配给 `<span>` 标签的标识。

## 示例

### 示例-1

在下面的程序中，span 元素中的内容被访问并附加到段落元素中。
...
可以使用 `document.createElement()` 方法创建跨度对象。在下面的程序中，点击按钮会创建一个 span 对象，并在其上附加一个文本节点。
