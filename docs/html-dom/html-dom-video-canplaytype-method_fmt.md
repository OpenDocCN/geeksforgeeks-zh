...
> 原文：[https://www.geeksforgeeks.org/html-dom-video-canplaytype-method/](https://www.geeksforgeeks.org/html-dom-video-canplaytype-method/)

`视频播放类型()`方法用于检查浏览器是否可以播放指定的视频类型。方法返回一个代表支持级别的字符串。
...
*   `type`：指定测试支持的视频类型（和可选编解码器）。

## 返回

`Video canPlayType()`方法通常返回以下值之一：
...
以下是 HTML DOM `Video canPlayType()` 方法支持的浏览器：
