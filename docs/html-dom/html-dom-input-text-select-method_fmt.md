# HTML DOM Input select()方法

> 原文：[https://www.geeksforgeeks.org/html-dom-input-text-select-method/](https://www.geeksforgeeks.org/html-dom-input-text-select-method/)

`DOM Input select()`方法选择包含文本字段的`textarea`或`input`元素的所有文本内容。
...
## 支持的浏览器
`HTML DOM Input select()`方法支持的浏览器如下:
