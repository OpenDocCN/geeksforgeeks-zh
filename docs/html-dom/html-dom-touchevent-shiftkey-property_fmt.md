...
> 原文: [https://www.geeksforgeeks.org/html-dom-touchevent-shiftkey-property/](https://www.geeksforgeeks.org/html-dom-touchevent-shiftkey-property/)

`TouchEvent shiftKey` 属性为只读属性，用于返回布尔值，该值指示触摸事件触发时是否按下了“移位”键。`touch event shiftKey` 属性通常返回 `false`，因为一般来说，触摸设备没有 `shift key`。
...
下面的程序说明了 `TouchEvent shiftKey` 属性：查找触摸屏上的 `shift` 键是否被按下。
