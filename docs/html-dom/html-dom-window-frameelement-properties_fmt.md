...
> 原文：[https://www.geeksforgeeks.org/html-dom-window-frameelement-properties/](https://www.geeksforgeeks.org/html-dom-window-frameelement-properties/)

HTML DOM `Window frameElement` 属性返回嵌入或存储窗口的 `iframe` 元素。如果没有存储，在这种情况下，它只返回一个空值。
...
`DOM Window frameElement` 属性支持的浏览器如下：
