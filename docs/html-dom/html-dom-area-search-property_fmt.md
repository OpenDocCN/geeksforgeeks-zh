...
> 原文：[https://www.geeksforgeeks.org/html-dom-area-search-property/](https://www.geeksforgeeks.org/html-dom-area-search-property/)

HTML DOM 中的 `Area` `search` 属性用于设置或返回 `href` 属性的查询字符串部分。它是网址中问号之后的部分。
...
包含单个值 `querystring`，指定网址的搜索部分。

## 返回值

返回一个字符串值，代表网址的查询字符串，包括 `?` 问号。

## 示例 1

本示例返回 `Area` `search` 属性。
...
<body>
    ...
    <map name="Geeks1">
        <area id="Geeks"
              ...
              coords="0, 0, 110, 100"
              alt="Geeks"
              href="https://manaschhabra:manaschhabra499@www.geeksforgeeks.org?id=Geeks/"
              target="_self">
    </map>
