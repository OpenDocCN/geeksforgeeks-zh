...
当一个元素失去焦点时，就会出现 **HTML DOM onfocusout 事件**。`onfocusout` 与 `onblur` 相同，唯一的区别是 `onblur` 事件不会冒泡。
...
<body>
    <center>
        ...
        <script>
            document.getElementById("email").addEventListener("focusout", gfgFun);
...
**HTML DOM onfocusout 事件** 支持的浏览器如下:
*   谷歌 Chrome
*   微软公司出品的 web 浏览器
*   Firefox 52.0
*   苹果 Safari
