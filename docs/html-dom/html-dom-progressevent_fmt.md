...
它返回一个字符串值，该值代表进度的可计算长度、加载长度或总度量。

## 属性

Progress event 的属性继承自其父事件。

*   `ProgressEvent.lengthComputable`: 它是一个只读属性，包含一个布尔标志，表示 progress event 所关注的资源有一个可以计算的长度。
