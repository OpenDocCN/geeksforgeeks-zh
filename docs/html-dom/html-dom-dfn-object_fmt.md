...
`DOM DFN` 对象用于表示 [HTML `<dfn>` 标签](https://www.geeksforgeeks.org/html-dfn-tag/)。`DFN` 元素由 `getElementById()` 访问。
...
可以使用 `document.createElement()` 方法创建 `DFN` 对象。
