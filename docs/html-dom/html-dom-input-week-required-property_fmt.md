...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-week-required-property/](https://www.geeksforgeeks.org/html-dom-input-week-required-property/)

HTML DOM 中的 `required` 属性用于设置或返回提交表单时是否需要填写 `<input type="week">` 字段。此属性用于反映 HTML 的 `required` 属性。

## 语法

*   它返回输入周元素的 `required` 属性。

```html
weekObject.required
```

*   它用于设置输入周元素的 `required` 属性。
...
返回一个布尔值，表示提交表单前必须填写或不填写周字段。

## 示例-1

本示例返回输入周元素的 `required` 属性。
...
在 Firefox 中，`<input type="week">` 元素不显示任何日期字段或日历。