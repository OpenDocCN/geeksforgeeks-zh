...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-email-maxlength-property/](https://www.geeksforgeeks.org/html-dom-input-email-maxlength-property/)

HTML DOM 中的`输入邮件最大长度属性`用于设置或返回邮件输入字段的最大长度属性值。它指定了电子邮件字段中允许的最大字符数。`输入电子邮件最大长度属性`的默认值为 524288。
...
**支持的浏览器:** `DOM 输入邮件最大长度属性`支持的浏览器如下:
