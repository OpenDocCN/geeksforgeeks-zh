...
> 原文: [https://www.geeksforgeks.org/html-dom-table-insertrow-method/](https://www.geeksforgeks.org/html-dom-table-insertrow-method/)

`表格插入行()`方法用于创建一个空的`<tr>`元素，该元素可以添加到表格中。通常用于在表中的指定索引处插入新行。一个`<tr>`元素包含至少一个`<th>`或`<td>`元素。
