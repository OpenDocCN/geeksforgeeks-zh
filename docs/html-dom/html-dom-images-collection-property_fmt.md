...
> 原文：[https://www.geeksforgeeks.org/html-dom-images-collection-property/](https://www.geeksforgeeks.org/html-dom-images-collection-property/)

HTML 中的**图像集合属性**用于返回文档中 `<img>` 元素的集合。它可以用于知道使用 `<img>` 标签插入到文档中的图像的数量。`type="image"` 的 `<input>` 元素不计入 `images` 属性。
...
*   **`item(index)`：** 用于返回所选索引的 `<img>` 元素。索引值以 0 开始。如果索引值超出范围，则返回空值。
*   **`namedItem(id)`：** 用于返回集合中给定 `id` 属性的 `<img>` 元素。如果 `id` 无效，它将返回 `NULL`。

**返回值：** 一个 `HTMLCollection` 对象，代表文档中的所有 `<img>` 元素。集合中的元素按照它们在源代码中出现的方式排序。

下面的程序说明了 HTML 中的 `document.images` 属性：

### 示例 1
使用 `length` 属性返回集合中 `<img>` 元素的个数。
...
<body>
    ...
    <img src="internships.png" alt="internships" width="150" height="150">
    <img src="coding.png" alt="Coding Practice time" width="150" height="150">
    <p>
...
### 示例 3
使用 `namedItem` 属性返回集合中 `<img>` 元素的 URL。
...
<body>
    ...
    <img src="home.png" alt="homepage" width="150" height="150">
    <img src="internships.png" alt="internships" width="150" height="150">
    <img id="coding.png" src="coding.png" width="150" height="150" alt="Coding Practice time">
    <p>
