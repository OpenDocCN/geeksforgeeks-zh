...
`HTML DOM IFrame src 属性`用于设置或返回 `<iframe>` 元素的 `src` 属性的值。此属性用于指定嵌入 iframe 元素的文档的 URL。
...
它返回一个字符串值，该值代表嵌入在 Iframe 元素中的文档的网址。

## 示例 1

本示例说明如何返回 Iframe `src` 属性。
...
`HTML DOM IFrame src Property`支持的浏览器如下:
