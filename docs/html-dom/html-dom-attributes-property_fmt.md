...
HTML DOM 中的`attributes`属性返回指定节点的`NamedNodeMap`对象。`NamedNodeMap`对象表示属性对象的集合，可以通过索引号进行访问。索引号从 0 开始。
...
返回节点集合的`NamedNodeMap`对象。

## 注意

在 Internet Explorer 8 和更低版本中，`attributes`属性将返回一个元素的所有可能属性的集合，这些属性可能会产生比预期值更高的值。
