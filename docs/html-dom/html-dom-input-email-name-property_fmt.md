...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-email-name-property/](https://www.geeksforgeeks.org/html-dom-input-email-name-property/)

HTML DOM 中的**输入邮件名称属性**用于设置或返回邮件字段的名称属性值。每个输入字段都需要`name`属性。如果没有在输入字段中指定`name`属性，则根本不会发送该字段的数据。
...
`DOM Input Email name`属性支持的浏览器如下:
