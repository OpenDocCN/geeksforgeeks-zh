...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-fileupload-multiple-property/](https://www.geeksforgeeks.org/html-dom-input-fileupload-multiple-property/)

HTML DOM 中的 `input` 文件上传 `multiple` 属性用于设置或返回用文件上传按钮选择的一个或多个文件。它包含一个布尔值，当允许用户选择多个文件时，该值为真。此属性用于反映 HTML `multiple` 属性。

## 语法

*   它返回输入文件上传 `multiple` 属性。

    ```html
    fileuploadObject.multiple
    ```

*   它用于设置输入文件上传 `multiple` 属性。
...
返回一个布尔值，指定是否允许用户使用文件上传按钮选择多个文件。
...
HTML DOM 输入文件 `multiple` 属性支持的浏览器如下：
