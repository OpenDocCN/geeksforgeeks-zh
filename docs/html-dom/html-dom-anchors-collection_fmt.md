...
**DOM 锚定集合**用于返回一个 HTML 文档中所有 `<a>` 元素的集合。它只计算那些具有 `name` 属性的 `<a>` 元素。在 HTML 5 中不支持锚点元素的 `name` 属性。集合中的元素按照源代码中显示的内容进行排序。

## 语法:
`document.anchors`

## 属性:
锚点集合属性包含一个值 `length`，该值返回文档中 `<a>` 元素的数量。

## 方法:
DOM `anchors` 集合包含以下三种方法:
*   **【索引】:** 用于返回所选索引的 `<a>` 元素。索引值以 0 开始。如果索引值超出范围，则返回空值。
*   `item(index)`: 用于返回所选索引的 `<a>` 元素。索引值以 0 开始。如果索引值超出范围，则返回空值。
*   `namedItem(id)`: 用于从集合中返回给定 `id` 属性的 `<a>` 元素。如果 `id` 无效，它将返回 `NULL`。

## 返回值:
一个 `HTMLCollection` 对象，表示文档中所有具有 `name` 属性的 `<a>` 元素。集合中的元素在源代码中出现时进行排序。
