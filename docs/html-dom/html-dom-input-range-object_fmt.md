...
HTML DOM 中的`输入范围对象`是用`type="range"`表示 HTML [`<input>`](https://www.geeksforgeeks.org/html5-date-attribute-in-input-tag/) 元素。

此标签用于`访问`或`创建` `<input>` 元素。可以使用 `getElementById()` 方法访问该元素。
...
## 示例-1：使用 `document.getElementById()` 从 `Geek_Range` id 返回 `<input>` 元素的名称
...
## 示例-2：使用 `document.getElementById()` 从 `Geek_Range` id 输入>元素的返回值
...
## 示例-3：创建用 `type = "range"` 的 `<input>` 元素
