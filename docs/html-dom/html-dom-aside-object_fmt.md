...
## 介绍
`DOM` 旁白对象用于表示 HTML `<aside>` 元素。`getElementById()` 可以访问 `aside` 元素。`aside` 元素在 HTML 5 中是新的。

## 语法
```html
document.getElementById("ID");
```
其中 `"id"` 是分配给 `<aside>` 标签的 ID。
...
## 示例-2
可以使用 `document.createElement` 方法创建侧边对象。
    ...
    <center>
        ...
        <script>
            function myGeeks() {
                ...
                var text2 = document.createTextNode("A Computer Science Portal for Geeks.");
