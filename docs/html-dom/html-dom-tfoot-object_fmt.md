...
`HTML DOM tfoot` 对象用来表示 HTML `<tfoot>` 元素。[`<tfoot>`](https://www.geeksforgeeks.org/html-tfoot-tag/) 对象用于对文档页脚部分的 HTML 表格内容进行分组。[`<tfoot>`](https://www.geeksforgeeks.org/html-tfoot-tag/) 对象与 HTML 文档的 [`<thead>`](https://www.geeksforgeeks.org/html-thead-tag/) 和 [`<tbody>`](https://www.geeksforgeeks.org/html-tbody-tag/) 元素一起使用。现在 `<tfoot>` 对象已被弃用，不再使用。
