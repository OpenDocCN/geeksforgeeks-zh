...
**DOM 画布对象**用于表示 HTML `<canvas>`元素。`<canvas>`标签用于使用 javascript 在文档中绘制图形。它在 HTML5 中是新的。画布元素由`getElementById()`访问。
...
## 示例-2:
可以使用`document.createElement`方法创建画布对象。
