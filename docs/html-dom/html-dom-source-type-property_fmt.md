...
> 原文：[https://www.geeksforgeeks.org/html-dom-source-type-property/](https://www.geeksforgeeks.org/html-dom-source-type-property/)

HTML DOM 中的 `source type` 属性用于设置或返回 `<source>` 元素中 `type` 属性的值。`type` 属性用于指定媒体资源的 MIME 类型。
...
它包含单个值 `MIME_type`，指定媒体资源的 `MIME_type`。很少有 MIME 类型是 `video/ogg`、`video/mp4`、`audio/ogg` 等。

## 返回值

返回一个字符串值，代表媒体资源的 MIME 类型。
...
<body>
    ...
    <p>Click the button to set the source type of the audio file.</p>
    ...
    <script>
        function myFunction() {
            ...
            document.getElementById("demo").innerHTML = "The source type has been changed to " + x;
        }
    </script>
