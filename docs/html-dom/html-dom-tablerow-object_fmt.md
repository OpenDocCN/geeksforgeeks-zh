...
*   `align`: 用于设置或返回表格行内内容的水平对齐。HTML 5 不支持。
...
*   `height`: 用于设置或返回表格行的高度。HTML 5 不支持。
*   `rowIndex`: 用于返回一个表的 `rows` 集合中的行的位置。
...
<p>
    Click on the button to delete cell of table
</p>
...
<p>
    Click on the button to create table element.
</p>
