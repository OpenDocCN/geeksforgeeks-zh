...
HTML DOM 中的输入周对象表示一个具有 `type = "week"` 属性的 `<input>` 元素。可以使用 `getElementById()` 方法访问该元素。
...
*   `list`：返回包含周字段的数据列表的引用。
*   `form`：返回包含周字段的表单的引用。
*   `autocomplete`：用于设置或返回一周字段的自动完成属性的值。
*   `autofocus`：用于设置或返回页面加载时周字段是否应自动对焦。
*   `defaultValue`：用于设置或返回一周字段的默认值。
*   `disabled`：用于设置或返回周字段是否禁用。
*   `max`：用于设置或返回一周字段的 `max` 属性的值。
*   `min`：用于设置或返回一周字段的 `min` 属性的值。
*   `name`：用于设置或返回周字段名称属性的值。
*   `readOnly`：用于设置或返回周字段是否只读。
*   `required`：用于设置或返回提交表单前是否必须填写周字段。
*   `size`：用于设置或返回周字段的 `size` 属性值。
*   `type`：返回周字段所属的表单元素类型。
*   `value`：用于设置或返回一周字段的 `value` 属性的值。

**示例 1：** 本示例描述了使用 `getElementById()` 方法访问具有 `type = "week"` 属性的 `<input>` 元素。
...
**示例 2：** 本示例描述了 `document.createElement()` 方法来创建具有 `type = "week"` 属性的 `<input>` 元素。
