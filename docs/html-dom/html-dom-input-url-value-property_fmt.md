...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-url-value-property/](https://www.geeksforgeeks.org/html-dom-input-url-value-property/)

`DOM 输入 URL 值属性`用于**设置**或**返回**`输入 URL 字段`的值。该属性指定默认值或用户类型值。
...
## 属性值
它包含一个值，即`网址`，指定通常指向另一个网站网址的绝对网址。

## 返回值
返回一个字符串值，代表代表网址的数字。

## 示例-1
本示例说明了如何**返回**`value`属性。
