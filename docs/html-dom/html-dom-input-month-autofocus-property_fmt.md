...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-month-autofocus-property/](https://www.geeksforgeeks.org/html-dom-input-month-autofocus-property/)

HTML DOM 中的 `autofocus` 属性用于设置或返回页面加载时 `<input type="month">` 字段是否应该自动获得焦点。它反映了 HTML `autofocus` 属性。
...
*   `true`: 设置月份字段获得焦点。
*   `false`: 有默认值。它定义了月份字段不能获得焦点。

## 返回值

返回一个布尔值，代表月份字段是否自动对焦。
