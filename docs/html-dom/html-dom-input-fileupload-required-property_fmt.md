...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-fileupload-required-property/](https://www.geeksforgeeks.org/html-dom-input-fileupload-required-property/)

`Input FileUpload required` 属性用于**设置**或**返回**是否在提交表单之前必须选择/上传文件上传字段中的文件。该属性反映了 HTML `required` 属性。
...
*   **true**：需要文件上传字段时返回 `true`。
*   **false**：不需要文件上传字段时返回 `false`。

## 返回值

显示文件上传状态的布尔值必填字段。

## 示例-1：返回文件上传 required 属性
