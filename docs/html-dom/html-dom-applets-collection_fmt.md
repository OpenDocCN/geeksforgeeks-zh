...
`小程序集合`将会返回一个在 HTML 文档对象模型中可用的所有可能元素的集合。最初，该元素用于将 Java 应用程序嵌入到 HTML DOM 中。在 Javascript 插件引入后不久，`applet`元素就被弃用了。
...
*   `length`：`length`属性值将返回指定集合中存在的元素总数。

**注意：** 该属性为绝对只读属性。

**方法：**

*   `[index]`：`[index]`方法将从集合中返回具有从 0 开始的已定义索引的元素。
...
**返回值：** `小程序集合`的返回值将是一个 HTML 集合对象，具有文档对象模型中所有元素的特征。集合中的元素按照它们在源代码中出现的方式排列。
**注意：** `小程序集合`元素与 HTML5 的最新版本不兼容。
...
<body>
    ...
    <script>
        function geeks() {
            ...
            document.getElementById("Geeks").innerHTML = g.toString();
        }
    </script>
</body>
</html>
