...
在 HTML 文档中，`className` 属性用于设置或返回元素的类属性的值。使用此属性，用户可以将元素的类更改为所需的类。
...
`className` 指定元素的类名。若要应用多个类，请使用空格将它们分开。例如，如果一个元素有两个类，那么我们将它们指定为 `"classname1 classname2"`，其中 `classname1` 和 `classname2` 是两个不同类的名称。`className` 属性返回一个字符串或一个以空格分隔的元素类列表。
...
*   **解释：** 使用 `className` 属性为 `<div>` 元素分配了一个值。
...
<body>
    <p>
        Click the button to change the value of the class attribute of div to "newstyle".
    </p>
