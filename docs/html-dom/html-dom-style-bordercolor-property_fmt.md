...
> 原文: [https://www.geeksforgeeks.org/html-dom-style-bordercolor-property/](https://www.geeksforgeeks.org/html-dom-style-bordercolor-property/)

DOM 样式边框颜色属性指定元素边框的颜色。它可以显式给定，从父级继承，或者默认情况下采用默认值。
...
*   `transparent`: 将对应元素的边框颜色设置为透明。
*   `inherit`: 当没有为此字段指定值时，从元素的父元素继承。如果没有父元素意味着这个元素是根元素，那么它采用初始值(或缺省值)。
*   `initial`: 此关键字将属性的初始值(或默认值)应用于元素。初始值不应与浏览器样式表指定的值混淆。当边框颜色设置为初始时，它显示为黑色(默认)。
