...
> 原文: [https://www.geeksforgeeks.org/html-dom-getattributenode-method/](https://www.geeksforgeeks.org/html-dom-getattributenode-method/)
...
该方法接受单参数 `attribute_name`，为必选项。此参数用于保存属性的名称。
...
本示例包含两个标题元素，任务是显示第二个标题元素的属性节点的值。
...
下面列出了 `DOM getAttributeNode()` 方法支持的浏览器:
