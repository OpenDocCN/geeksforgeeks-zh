# HTML DOM style transformStyle 属性

> 原文: [https://www.geeksforgeeks.org/html-dom-style-transformstyle-property/](https://www.geeksforgeeks.org/html-dom-style-transformstyle-property/)

`transformStyle` 属性用于设置或返回嵌套元素在 3D 空间中渲染的不同方式。
...
- **`preserve-3d`**: 它使子元素能够保留它们的 3D 位置。
- **`initial`**: 它将 `transformStyle` 设置为默认值。
- **`inherit`**: 继承父元素的 `transformStyle` 属性值。
...
`DOM style transformStyle` 属性支持的浏览器如下:
