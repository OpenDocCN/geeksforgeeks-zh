...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-text-size-property/](https://www.geeksforgeeks.org/html-dom-input-text-size-property/)

HTML DOM 中的输入文本大小属性用于设置或返回输入文本字段的 `size` 属性值。`size` 属性用于定义文本字段的宽度。它的默认值是 20。
...
它包含单个值 `number`，用于根据字符数指定文本字段的宽度。
