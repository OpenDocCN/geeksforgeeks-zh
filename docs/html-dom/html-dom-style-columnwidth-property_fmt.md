...
> 原文：[https://www.geeksforgeeks.org/html-dom-style-columnwidth-property/](https://www.geeksforgeeks.org/html-dom-style-columnwidth-property/)
...
*   `auto`：默认值。列宽将由浏览器决定。
*   `length`：指定列宽度的长度。
*   `initial`：将该属性设置为默认值。阅读关于 `initial`。
...
<body>
    <center>
        ...
        <div id="DIVid">
            ...
            which defines the structure of web pages. HTML is a markup language
            which is used by the browser to manipulate text, images and other
...
`HTML DOM Style columnWidth Property` 支持的浏览器如下：
