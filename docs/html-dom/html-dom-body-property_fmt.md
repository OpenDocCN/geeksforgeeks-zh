...
**HTML DOM Body 属性**用于设置文档 `<body>` 元素。它只返回存在于 `<body>` 标签中的内容。此属性用于更改 `<body>` 元素内的当前内容，并用新的指定内容设置它们。该属性不返回 `<html>` 元素。
...
**返回值:** 对身体对象的引用，表示 `<body>` 元素。

**属性值:** `document.body` 属性使用单值 `content`，用于在 `body` 元素中添加新内容。
