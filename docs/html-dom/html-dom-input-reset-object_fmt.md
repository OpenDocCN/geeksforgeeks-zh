...
HTML DOM 中的**输入重置对象**用来表示 HTML [`<input type="reset">`](https://www.geeksforgeeks.org/html5-date-attribute-in-input-tag/) 元素。
该标签用于**访问**或**创建** `<input>` 元素。可以使用 `getElementById()` 方法访问该元素。
...
## 示例-2: 使用 `document.getElementById()` 从 id 为 `GeekReset` 的 `<input>` 元素获取返回值。
...
`type="reset"` 用于使用**重置按钮**将所有值重置为其初始值。
