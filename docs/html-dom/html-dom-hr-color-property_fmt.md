...
`DOM HR` 颜色属性用于设置或返回 `<hr>` 元素的颜色属性值。
...
*   `color_name`：使用颜色名称保存文本颜色。例如：`"red"`。
*   `hex_number`：通过使用颜色十六进制编码来保存文本颜色。例如：`"#0000ff"`。
*   `rgb_number`：用 `rgb` 码保存文字颜色。例如：`"rgb(0, 153, 0)"`。

## 返回值

返回一个字符串值，代表 `HR` 元素的颜色。

## 示例 1

本示例使用 `DOM HR` 颜色属性返回 `HR` 颜色属性。
