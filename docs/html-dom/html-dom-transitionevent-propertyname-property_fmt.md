...
> 原文：[https://www.geeksforgeeks.org/html-dom-transitionevent-propertyname-property/](https://www.geeksforgeeks.org/html-dom-transitionevent-propertyname-property/)

## TransitionEvent propertyName 属性

`TransitionEvent propertyName` 属性是一个只读属性，用于在发生 `transitionend` 事件时返回与转换相关联的 CSS 属性的名称。
...
<head>
    <title>TransitionEvent propertyName Property</title>
    <style>
        #MyDiv {
            width: 100px;
            height: 20px;
...
<body>
    ...
    <p>Hover over the element to see the name of the CSS property the transition effect is for. </p>

    <div id="MyDiv"></div>

    <script>
        document.getElementById("MyDiv").addEventListener("transitionend", myevent);
