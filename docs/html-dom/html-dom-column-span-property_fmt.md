...
原文: [https://www.geeksforgeeks.org/html-dom-column-span-property/](https://www.geeksforgeeks.org/html-dom-column-span-property/)

`DOM 列跨度属性`用于设置或返回 `<col>` 元素 `span` 属性的值。HTML 中的 `colspan` 属性指定单元格应该跨越的列数。
...
<script>
    function myGeeks() {
        // access col element
        document.getElementById("gfg").span = 2;
