...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-date-autofocus-property/](https://www.geeksforgeeks.org/html-dom-input-date-autofocus-property/)

`autofocus` 属性用于设置或返回页面加载时日期字段是否自动对焦。
如果日期字段在页面加载时自动获得焦点，则返回 `true`，否则返回 `false`。
HTML `autofocus` 属性由 `Date` 的 `autofocus` 属性反映。
...
*   `true|false` : 用于指定页面加载时日期字段是否应该获得焦点。默认为 `false`。

## 返回值

返回一个布尔值，表示输入日期字段是否获得焦点。

下面的程序说明了 `autofocus` 属性:

### 示例-1: 找出日期字段是否在页面加载时自动聚焦
