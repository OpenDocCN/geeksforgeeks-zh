...
> 哎哎哎:# t0]https://www . geeksforgeeks . org/html-DOM-fullscreen-change 事件/
...
## 示例 1：使用 onfullscreenchange 属性

这个示例说明了 HTML DOM `fullscreenchange` 事件。
...
<body onfullscreenchange="function()">
    <center>
        ...
        <button onclick="closeFullscreen();">Close Fullscreen or press "Esc"</button>
    </center>
    <script>
...
## 示例 2：使用 JavaScript 处理事件

本示例使用 JavaScript 处理 `fullscreenchange` 事件。
...
本示例对 `fullscreenchange` 事件使用 `addEventListener()` 方法。
