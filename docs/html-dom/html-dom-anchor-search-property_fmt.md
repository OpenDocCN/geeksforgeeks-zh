...
> 原文：[https://www.geeksforgeeks.org/html-dom-anchor-search-property/](https://www.geeksforgeeks.org/html-dom-anchor-search-property/)

HTML DOM 中的`锚点搜索属性`用于设置或返回`href`属性的查询部分（问号`?`之后的部分）。它通常用于参数传递。
...
包含单值`查询字符串`，用于指定网址的搜索部分。

## 返回值

它返回一个字符串值，该值代表也包含问号的网址的查询字符串部分（`?`）。
...
<body>
    <center>
        ...
        <p>Welcome to
            <a href="https://contribute.geeksforgeeks.org/wp-admin/post.php?post=964437&action=edit"
               id="GFG" target="_self">
                GeeksforGeeks
            </a>
        </p>
