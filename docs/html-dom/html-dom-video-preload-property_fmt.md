...
> 原文：[https://www.geeksforgeeks.org/html-dom-video-preload-property/](https://www.geeksforgeeks.org/html-dom-video-preload-property/)
...
*   `metadata`：用于指定页面加载时浏览器只加载元数据。
*   `none`：用于指定页面加载时浏览器不应加载视频。

**返回值：** 返回一个字符串值，代表应该预加载哪些数据（如果有）。可能的返回值是`"auto"`、`"metadata"`或`"none"`。请参阅“属性值”了解这些值的含义。
