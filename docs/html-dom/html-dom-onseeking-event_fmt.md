...
当用户开始将媒体跳转到一个新位置时，就会发生 HTML DOM 中的 `onseeking 事件`。`onseeking 事件`和`onseeked 事件`正好相反。要获取媒体的当前位置，请使用`currentTime`。
...
<body>
    <center>
        ...
        <video controls id="videoID">
            <source src="https://media.geeksforgeeks.org/wp-content/uploads/20190723123920/secondneon.mp4" type="video/mp4">
        </video>
    </center>
    <script>
        document.getElementById("videoID").addEventListener("seeking", GFGfun);
...
## 支持的浏览器:
`HTML DOM onseeking Event`支持的浏览器如下:
