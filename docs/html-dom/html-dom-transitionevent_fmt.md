...
*   `propertyName`: 返回转换的名称。
*   `elapsedTime`: 返回转换运行的秒数。

## 事件类型

*   `transitionend`: 当 CSS 转换完成时，该事件发生。
*   `transitionstart`: 当 CSS 转换开始时，事件发生。
...
<body>
    <center>
        ...
        <script>
            ...
            document.getElementById("myDIV").addEventListener("webkitTransitionEnd", func);
            // TransitionEnd Event for other Standard Browsers.
            document.getElementById("myDIV").addEventListener("transitionend", func);
            function func() {
