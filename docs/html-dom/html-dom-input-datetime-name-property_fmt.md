...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-datetime-name-property/](https://www.geeksforgeeks.org/html-dom-input-datetime-name-property/)

输入日期时间名称属性用于设置或返回日期时间字段的`name`属性值。已经提交给服务器的表单数据可以通过`name`属性来识别。`name`属性还用于在客户端使用 Javascript 引用表单数据。
