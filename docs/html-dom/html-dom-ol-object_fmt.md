...
`DOM Ol` 对象用于表示 HTML `<ol>` 元素。`ol` 元素由 `getElementById()` 访问。要阅读更多关于列表的信息，请查看 [HTML | 列表](https://www.geeksforgeeks.org/html-lists/)。

## 属性

*   `compact`：用于设置或返回列表显示大小是否正常。
*   `reversed`：用于设置或返回列表项是降序显示还是升序显示。
*   `start`：用于设置或返回 `<ol>` 元素中 `start` 属性的值。
*   `type`：用于设置或返回 `<ol>` 元素中 `type` 属性的值。
...
<script>
        function Geeks() {
            // Accessing ol tag and starts with 101.
...
`Ol` 可以使用 `document.createElement` 方法创建对象。
