...
> 原文: [https://www.geeksforgeeks.org/html-dom-button-type-property/](https://www.geeksforgeeks.org/html-dom-button-type-property/)

**DOM 按钮类型**属性用于设置或返回 `<button>` 元素的 `type` 属性的值。
...
*   `submit`: 定义一个提交按钮。除了 Internet Explorer 之外，它对所有浏览器都有默认值。
*   `button`: 定义一个可点击的按钮。它有一个互联网浏览器的默认值。
*   `reset`: 定义了一个复位按钮，用于改变表单中之前的数据。

**返回值**: 返回代表按钮类型的字符串值。

## 示例-1

演示如何返回 `buttonObject.type` 属性的 HTML 程序。
...
说明如何设置 `buttonObject.type` 属性的示例。
