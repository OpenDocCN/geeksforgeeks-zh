...
> 原文: [https://www.geeksforgeeks.org/html-dom-input-week-autofocus-property/](https://www.geeksforgeeks.org/html-dom-input-week-autofocus-property/)

HTML DOM 中的 `DOM 输入周自动对焦` 属性用于设置或返回页面加载时输入周字段是否应该对焦。它反映了 HTML `autofocus` 属性。
...
返回一个布尔值，代表星期字段是否自动对焦。

## 示例-1

本示例返回输入周 `autofocus` 属性。
...
在 Firefox 中，输入 `type="week"` 元素不显示任何日期字段或日历。