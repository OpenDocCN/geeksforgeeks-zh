...
> 原文：`https://www.geeksforgeeks.org/html-dom-console-time-method/`

HTML 中的 `console.time()` 方法用于在控制台视图中启动定时器。`console.time()` 方法可用于计算各种测试目的的程序时间。标签作为参数发送到 `console.time()` 方法。
...
## 参数:
该方法接受单参数 `label`，该标签为可选，用于指定定时器的标签。
...
## 示例 2:
使用 `console.time()` 方法计算 while 循环所用的时间
