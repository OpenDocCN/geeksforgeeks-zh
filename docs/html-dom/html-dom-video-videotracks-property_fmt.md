...
> 原文: [https://www.geeksforgeeks.org/html-dom-video-videotracks-property/](https://www.geeksforgeeks.org/html-dom-video-videotracks-property/)

`视频跟踪属性`用于返回一个视频跟踪列表对象。视频跟踪列表对象通常用于表示视频可用的视频轨道。每个可用的视频轨道由一个单独的视频轨道对象表示。
...
1.  **视频轨道列表对象:** 表示视频的可用视频轨道。
    ...
    *   `videoTracks[index]`: 用于通过索引获取视频跟踪对象。
    *   `视频轨道列表对象`: 用于表示视频的可用视频轨道。
    *   `videoTracks.getTrackById(id)`: 用于按 id 获取 VideoTrack 对象。
2.  **视频轨道对象:** 表示视频轨道。
    *   `kind`: 用于获取视频轨道的类型。
    *   `label`: 用于获取视频轨道的标签。
    *   `language`: 用于获取视频文字轨迹的语言。
    *   `id`: 用于获取视频轨道的 id。
    *   `selected`: 用于获取或设置轨道是否激活。
