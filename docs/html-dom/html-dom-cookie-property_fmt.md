...
`document.cookie` 方法返回一个字符串，该字符串包含当前文档的所有 cookie (键=值对)的分号分隔列表。
...
可以为当前文档编写新的 Cookie，方法是提供一个字符串，该字符串包含由分号和空格分隔的 `name=value` 对，以及以下任意可选值：

*   `expires=日期`：其中日期为格林尼治标准时间格式。默认情况下，浏览器关闭时 cookie 会过期。
*   `path=路径`：指定计算机上存储 cookie 的目录。默认情况下，路径设置为当前文档位置的路径。
*   `max-age=秒`
*   `domain=域名`：指定 cookie 的域名。如果未指定，默认为当前页面的域名。
