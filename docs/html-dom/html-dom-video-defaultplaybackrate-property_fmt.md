...
> 原文: [https://www.geeksforgeeks.org/html-dom-video-defaultplaybackrate-property/](https://www.geeksforgeeks.org/html-dom-video-defaultplaybackrate-property/)

`defaultPlaybackRate` 属性用于**设置**或**返回**视频的默认播放速度。
视频的 `defaultPlaybackRate` 属性只改变默认播放速度，不改变当前播放速度。
...
支持 `HTML DOM Video defaultPlaybackRate` 属性的浏览器如下:
