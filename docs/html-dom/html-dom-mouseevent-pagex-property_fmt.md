...
> 原文: [https://www.geeksforgeeks.org/html-dom-mouseevent-pagex-property/](https://www.geeksforgeeks.org/html-dom-mouseevent-pagex-property/)

`MouseEvent` 的 `pageX` 属性是只读属性，用于在事件触发时返回鼠标指针的水平坐标。
...
返回一个数字，以像素为单位表示鼠标指针的水平坐标。

下面的程序说明了 `MouseEvent` `pageX` 属性：

## 示例

当鼠标按钮点击一个元素时，找出鼠标指针的水平坐标。
...
<body>
    ...
    <p onclick="coord(event)">
        ...
        mouse pointer when it was clicked.
    </p>
...
*   **点击按钮后:**
    ![](img/f7bb4b74e94129c03253b3d4a3435553.png)
