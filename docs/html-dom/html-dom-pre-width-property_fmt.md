...
*   **pixel:** 以像素为单位设置预格式化文本的宽度。
*   **%:** 以百分比(%)设置预格式化文本的宽度。

**返回值:** 返回一个代表 `<pre>` 元素宽度的数值。

**示例 1:** 本示例返回 pre 元素的宽度属性。
...
<body>
    <center>
        ...
        <script>
            function myGeeks() {
                // Accessing pre tag.
                var g = document.getElementById("GFG").width = "200px";
                document.getElementById("sudo").innerHTML = g;
            }
        </script>
    </center>
