...
原文: [https://www.geeksforgeeks.org/html-dom-style-borderbottomwidth-property/](https://www.geeksforgeeks.org/html-dom-style-borderbottomwidth-property/)
...
*   `medium`: 设置中等大小的下边框。这是默认值。
*   `thin`: 设置底部的薄边框。
*   `thick`: 设置粗底边框。
*   `length`: 设置边框的宽度。它不取负值。
*   `initial`: 将 `borderBottomWidth` 属性设置为默认值。
*   `inherit`: 用于从其父元素继承。

## 示例 1

本示例使用 `borderBottomWidth` 属性的厚值。
...
本示例使用 `borderBottomWidth` 属性的精简值。
...
下面列出了样式 `borderBottomWidth` 属性支持的浏览器:
