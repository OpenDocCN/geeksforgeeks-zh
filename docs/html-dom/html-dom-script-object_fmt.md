...
**DOM 脚本对象**用于表示 HTML [`<script>`](https://www.geeksforgeeks.org/html-script-tag/) 元素。*脚本元素*由 `getElementById()` 访问。

## 属性:

*   `async`: 用于指定脚本异步执行。
*   `charset`: 用于指定外部脚本文件中使用的字符编码。
*   `defer`: 用于指定页面解析完毕后执行脚本。
*   `src`: 用于指定外部脚本文件的 URL。
*   `type`: 用于指定脚本的媒体类型。
...
脚本对象可以使用 `document.createElement()` 方法创建。
...
<body>
    ...
    <script>
        function myGeeks() {
            ...
            var f = document.createTextNode("alert('Hello GeeksForGeeks')");
            g.appendChild(f);
