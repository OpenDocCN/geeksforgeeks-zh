...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-datetimelocal-disabled-property/](https://www.geeksforgeeks.org/html-dom-input-datetimelocal-disabled-property/)
...
该属性包含单个属性值 `true|false`。它用于指定是否应禁用本地日期时间字段。默认为 `false`。

### 返回值

返回一个布尔值，该值代表日期时间本地字段是否被禁用。

下面的程序说明了 HTML DOM 中的 `DatetimeLocal` `disabled` 属性：

### 示例

本示例使用 `Input` `DatetimeLocal` `disabled` 属性来禁用输入日期时间。
