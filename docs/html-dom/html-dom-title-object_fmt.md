...
HTML DOM 中的 `Title` 对象用来表示 HTML `<title>` 元素。使用 `getElementsByTagName()` 方法可以访问 `<title>` 元素。
...
`title` 对象包含单个属性 `text`，用于设置或返回文档标题的文本或内容。
...
可以使用 `document.createElement` 方法创建 `Title` 对象。
