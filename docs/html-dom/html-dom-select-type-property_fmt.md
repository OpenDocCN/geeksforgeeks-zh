# HTML DOM select type 属性

> 原文：[https://www.geeksforgeeks.org/html-dom-select-type-property/](https://www.geeksforgeeks.org/html-dom-select-type-property/)

`选择类型属性`用于`返回`表单元素类型下拉列表为。下拉列表可以有两种类型，分别是`"select-one"`或`"select-multiple"`。
...
一个字符串，表示下拉列表（`<select>`元素）的表单元素类型。
