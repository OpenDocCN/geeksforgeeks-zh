...
> 原文：[https://www.geeksforgeeks.org/html-dom-input-range-name-property/](https://www.geeksforgeeks.org/html-dom-input-range-name-property/)

HTML DOM 中的 `DOM 输入范围名称属性`用于**设置**或**返回**范围字段的 `name` 属性值。每个输入字段都需要 `name` 属性。如果没有在输入字段中指定 `name` 属性，则根本不会发送该字段的数据。
