...
HTML DOM 中的 `Area` 元素的 `host` 属性用于设置或返回 `href` 属性值的主机名和端口部分。
...
包含单个值 `hostname:port`，指定网址的主机名和端口号。

## 返回值

返回一个字符串值，代表网址的域名和端口号。
...
<body>
    ...
    <script>
        function GFG() {
            // Set host property.
            var x = document.getElementById("Geeks").host = "www.geeksforgeeks.org:111";
            document.getElementById("GEEK!").innerHTML = "The host has been changed to " + x;
        }
    </script>
</body>
</html>
