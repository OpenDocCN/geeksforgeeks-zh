...
> 原文: [https://www.geeksforgeeks.org/html-dom-customelements-define-method/](https://www.geeksforgeeks.org/html-dom-customelements-define-method/)

`自定义元素`的`define()`方法用于定义新的自定义元素。可以创建两种类型的自定义元素:
...
*   `name`: 指定新自定义元素的名称。自定义元素的名称必须包含连字符。
*   `constructor`: 指定新自定义元素的构造函数。
*   `options`: 指定控制元素定义方式的对象。这是一个可选参数。
...
在本例中，定义了一个自定义元素，名为`<gfg-custom-element>`，构造函数名为`CustomEl`，使用此方法。
