...
> 原文：[https://www.geeksforgeeks.org/html-dom-select-remove-method/](https://www.geeksforgeeks.org/html-dom-select-remove-method/)

HTML DOM 中的 `select remove()` 方法用于从下拉列表中移除一个选项。此方法接受索引号作为从所需位置移除选项的参数。
...
包含单参数 `index`，为必选项，用于指定待移除元素的索引位置。

下面的程序说明了在 HTML DOM 中 `select remove()` 方法：

## 示例

本示例使用 `select remove()` 方法从下拉列表中移除所选选项。
