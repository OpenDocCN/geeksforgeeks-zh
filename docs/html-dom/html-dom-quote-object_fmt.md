...
包含单属性值 [`cite`](https://www.geeksforgeeks.org/html-dom-quote-cite-property/?ref=rp)。该属性用于设置或返回一个 `<q>` 元素中引用属性的值。
...
可以使用 `document.createElement()` 方法创建报价对象。
