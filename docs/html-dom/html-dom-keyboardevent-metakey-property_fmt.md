...
> 原文：[https://www.geeksforgeeks.org/html-dom-keyboardevent-metakey-property/](https://www.geeksforgeeks.org/html-dom-keyboardevent-metakey-property/)

HTML DOM 中的 `KeyboardEvent.metaKey` 属性是一个只读属性，用于返回一个布尔值，该值用于检查 META 键是否被按下。如果按下了元键，`KeyboardEvent.metaKey` 属性返回 `true`，否则返回 `false`。
