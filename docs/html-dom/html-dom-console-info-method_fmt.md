...
HTML 中的 `console.info()` 方法用于在控制台中写入消息。它表示关于任何元素或对象的重要信息。该消息作为参数发送到 `console.info()` 方法。
...
**参数：** 该方法接受单参数 `message`，该消息是强制性的，用于指定要写入控制台的信息。
    ...
    <body>
        ...
        <script>
            function info_console() {
                console.info("GeeksforGeeks is a portal for geeks.");
            }
        </script>
    </body>
</html>
