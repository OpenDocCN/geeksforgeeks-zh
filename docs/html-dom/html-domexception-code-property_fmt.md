...
> 原文：[https://www.geeksforgeeks.org/html-domexception-code-property/](https://www.geeksforgeeks.org/html-domexception-code-property/)

`DOMException` 接口的 `code` 属性返回一个包含异常错误的错误代码的短数字，如果不匹配则返回 0。这是只读属性。
...
在本例中，我们将创建一个 `INVALID_CHARACTER_ERR`，然后显示其代码。
...
<script>
    function get(){
        try {
            ...
            document.querySelector(".p").textContent+=e.code;
        }   
        } 
</script>
</html>
