...
**DOM Pre 对象**用于表示 HTML [`<pre>`](https://www.geeksforgeeks.org/html-pre-tag/) 元素。可以通过 `getElementById()` 方法访问 `pre` 元素。

**属性：**

*   [`width`](https://www.geeksforgeeks.org/html-dom-pre-width-property/)：用于设置或返回 `pre` 元素的 `width` 属性的值。
...
可以使用 `document.createElement()` 方法创建 `pre` 对象。
    ...
    <center>
        ...
        <script>
            function myGeeks() {
                ...
                var f = document.createTextNode("GeeksForGeeks" + "A Computer Science Portal For Geeks.");
                g.appendChild(f);
