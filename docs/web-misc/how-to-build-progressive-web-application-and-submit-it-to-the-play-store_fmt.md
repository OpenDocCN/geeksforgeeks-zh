...
> 原文：[https://www.geeksforgeeks.org/how-to-build-progressive-web-application-and-submit-it-to-the-play-store/](https://www.geeksforgeeks.org/how-to-build-progressive-web-application-and-submit-it-to-the-play-store/)
...
在放置 `index.html` 文件的同一目录下创建一个 `manifest.json` 文件。这个文件包含应用名称，开始网址，主题颜色和一些更基本的 JSON 格式的应用信息。
...
创建 `serviceworker.js`，一个与 `manifest.json` 位于同一目录的 JavaScript 文件，负责 PWA 应用的安装和缓存。
...
使用 `index.html` 的 `<script>` 标签将 `serviceworker.js` 链接到 `index.html`。
...
在本教程中，我们使用 `django-blogiee.herokuapp.com` 网站。你可以查看它的 [`manifest.json`](https://django-blogiee.herokuapp.com/manifest.json) 和 `serviceworker.js`。

**有以下三种方法可以做到：**

*   如果你是安卓开发者，你可以自己构建，你不需要遵循这个教程（硬方法）
*   即使你不是安卓开发者，但有使用 `NodeJs`、`NPM`、`JDK` 和安卓命令行工具和命令行界面的经验，你可以使用 [Bubblewrap](https://github.com/GoogleChromeLabs/bubblewrap) 项目。（中等方式，但需要大量下载）
...
提取下载的压缩文件以获得 APK 文件，从而添加数字资产文件 `assetlinks.json`。请确保您的 `assetlinks.json` 文件可以在网址 `https://<your domain>/.well-known/assetlinks.json` 上找到。