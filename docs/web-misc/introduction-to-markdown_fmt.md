...
Markdown 文件的扩展名是 ``*.md*`` 或 ``*.markdown*``。要编译一个 Markdown 文件，你需要一个能够处理 Markdown 文件的应用程序，比如微软的 Word、``Dillinger`` 等。这些应用程序使用标记处理器或解析器，将标记文件转换为可打印的 HTML 代码。
...
5.  可以使用 ``Github Pages``、``blot.im`` 和 ``smallvictori.es`` 等工具将 Markdown 文件转换为网页。

如果你知道网页格式的基础知识，那将会非常有帮助，但是如果你不知道，也不用担心，我们会涵盖从业余到专业的所有内容。Markdown 有助于创建您在网页上看到的所有基本元素，如文本、列表、外部链接、图像等。

要开始编写 Markdown 代码，网上有很多可用的工具。正如你所知，学习的最好方法就是去做。你可以在任何像 ``Dillinger`` 这样的在线标记编辑器中开始执行如下所示的代码。
...
这篇文章是一个你可以用来做标记的备忘单，关于标记语法的详细教程见 [daringfireball.net](https://daringfireball.net/projects/markdown/syntax)。