...
> 原文:[https://www.geeksforgeeks.org/how-to-animate-the-drawing-on-a-web-page/](https://www.geeksforgeeks.org/how-to-animate-the-drawing-on-a-web-page/)
...
`@关键帧`规则用于指定动画规则。动画是通过使用可变的 `CSS` 样式创建的。在动画 `CSS` 期间，属性可以多次更改。
...
要定义动画，必须从`@关键帧`规则开始。`@关键帧`规则由关键字“`@关键帧`”组成，后跟一个给动画命名的标识符(将使用`动画名称`引用)，后跟一组样式规则(用大括号分隔)。然后，通过将标识符用作`动画名称`属性的值，将动画应用于元素。
