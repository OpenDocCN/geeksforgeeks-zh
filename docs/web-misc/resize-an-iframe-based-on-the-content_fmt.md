...
> 原文：[https://www.geeksforgeeks.org/resize-an-iframe-based-on-the-content/](https://www.geeksforgeeks.org/resize-an-iframe-based-on-the-content/)
...
下面是使用**内部 CSS** 调整内容大小的 HTML 代码的实现：
...
这里我们使用了一个内部样式表来将显示区域的尺寸固定为 `#target` 下的 `width` 和 `height`（以像素为单位）。CSS `overflow` 属性控制太大而无法放入区域的内容。

## 示例 2

下面是 HTML 代码调整内容大小的实现，使用的是 Javascript：

我们将使用 JavaScript `contentWindow` 属性，即 `iFrame` 会根据内容自动调整其高度，不会出现滚动条。

- 我们在 `script` 标记中选择 `iframe`：
