...
> 原文: [https://www.geeksforgeeks.org/how-to-use-protractor-to-check-if-an-element-is-visible/](https://www.geeksforgeeks.org/how-to-use-protractor-to-check-if-an-element-is-visible/)
...
**第三步:** 我们将创建 `test.js` 文件。在这个文件中，我们将访问上面的 HTML 文件，然后检查元素是否隐藏。`browser` 是由量角器创建的全局，用于浏览器级命令，如使用 `browser.get()` 方法导航。`describe` 和 `it` 语法来自 Jasmine 框架，其中 `describe` 是对您的测试的描述，而 `it` 定义了测试的步骤。
