...
`tabIndex` 选项用于指定在页面上选择表格控件的流程顺序。此选项可用于否决此流，并更改键盘与页面交互的方式。默认情况下，一个数据表将有一个已经分配给它的表格索引，这样就可以不用鼠标就可以访问控件。
...
在此示例中，第一个表的 `tabIndex` 设置为 `2`，第二个表的 `tabIndex` 设置为 `1`，因此第二个表的控件在第一个表之前被选中。
...
在本例中，第一个表的 `tabIndex` 设置为 `-1`，因此第一个表在文档流中不可选择。
