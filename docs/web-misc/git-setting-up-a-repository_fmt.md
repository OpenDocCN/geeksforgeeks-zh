...
## 什么是仓库？
简单来说，就是数据存储或管理的中心位置。一个`git`存储库虚拟存储一个特定的文件，并允许你保存和访问它的版本。
...
*   `--shared[=(false|true|umask|group|all|world|everybody|0xx)]`
    ...
    *   `group`(或`true`) :使存储库组可写。
    *   `all`(或`world`或`everybody`):-与组相同，但使存储库可供所有用户阅读。
...
其他选项有`--mirror`、`--bare`、`--origin`等等。

## 总结- `git init` VS `git clone`
区分它们非常简单的方法就是注意`git clone`依赖`git init`这一点。`git clone`用于创建现有存储库的副本，但是`git clone`本身首先调用`git init`来创建新的存储库。