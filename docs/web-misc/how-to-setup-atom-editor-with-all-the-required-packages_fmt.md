...
> 原文：[https://www.geeksforgeeks.org/how-to-setup-atom-editor-with-all-the-required-packages/](https://www.geeksforgeeks.org/how-to-setup-atom-editor-with-all-the-required-packages/)

要开始使用 Atom，首先，我们需要在您的系统上安装它。本节将讨论如何在您的系统上安装 Atom，以及如何使用 Web 开发人员所需的适当包从源代码构建 Atom 的基础知识。

## 下载和安装

Atom 编辑器可以从其官方网站 [atom.io](https://atom.io/) 下载。对于安装：

### 在 Windows 上

**第一步：** 点击按钮，下载 `.exe` 文件。

**步骤 2：** 运行 `.exe` 包，Atom 可以轻松地自行安装在 Windows 操作系统上。

### 在 Mac 上

**第一步：** 点击按钮，下载 `.exe` 文件。

**步骤 2：** 运行 `.exe` 包，Atom 可以轻松地自行安装在 MacOS 上。
...
*   在终端上输入 `apm install <package-name>`。显然，必须安装 Atom 包管理器 `apm`（您可以输入 `apm` 来验证安装）。
...
*   **`linter`**
    ...
    > **Note:** Click OK when installing `linter`. (It will require you to install the dependencies of the software package)

*   **`linter-jshint`**
    ![](img/775c72a6573c555e0837c8acf2095fe5.png)
    *   **步骤 1：** 安装完 `linter-jshint` 后，进入其设置。
    *   **步骤 2：** 在 `linter-jshint` 的设置页面上，向下滚动并启用“Lint 内联 Java 脚本”复选框。
        ![](img/a5c2c11c678805c945219b944b2d1af5.png)
*   **`pigments`**
