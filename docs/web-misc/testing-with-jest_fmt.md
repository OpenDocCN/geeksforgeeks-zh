...
*   **项目结构:**
    在项目根目录下，制作一个`tests`文件夹。该文件夹将存储所有测试文件。
...
我们将使用`expect`方法来测试我们的功能。也可以使用`describe` `it`测试功能。
...
当`npm run test`运行时， `index.test.js` 文件通过。然后运行测试添加功能，该功能位于`testFns`对象中。`toBe`用于`匹配`测试返回的响应与预期相符。这种“结果匹配”要么导致`fail`要么导致`pass`。
...
### 将“toBe”用于 JS 对象:
让我们考虑一个 JS 对象的每个字段都要被测试的情况。Jest 为我们提供了一种使用`toEqual`来做到这一点的方法。`toEqual`是一个深度匹配器(检查每个可能的字段和子字段)。
...
在这种情况下，使用 Jest 库中的`toBeCloseTo`匹配器就好了。
...
但是，如果上面的任何 `expect` 失败，Jest 将返回如下所示的有意义的错误消息。

**注意**：在上述情况下，如果任何 `expect` 失败，测试也完全失败。
...
但是，以下测试将失败，因为 `gfgUsers` 中不存在 "user4"。
...
Jest 也提供了扩展其匹配器功能的条款，这是使用 `expect.extend()` 关键字完成的。`extend()` 函数作为对象传递给匹配器。
...
Jest 也有在运行测试套件时跳过特定测试的规定。要实现它，只需使用 `test.skip` 关键字。例如，
...
与此相反的是只实现测试的子集，这是通过使用 `test.only` 关键字来实现的。例如，
