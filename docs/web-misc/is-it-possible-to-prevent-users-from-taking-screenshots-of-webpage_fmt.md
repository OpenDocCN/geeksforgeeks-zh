...
> 原文：[https://www.geeksforgeeks.org/is-it-possible-to-prevent-users-from-taking-screenshots-of-webpage/](https://www.geeksforgeeks.org/is-it-possible-to-prevent-users-from-taking-screenshots-of-webpage/)

截图是`Windows`中`OS`的一个功能。我们可以简单的按下键盘上的`Print Screen`（打印屏幕）进行截图。如果键盘没有`Prt Sc`键，我们可以使用`Fn + Windows logo键 + 空格键`。另外，我们可以使用`Snip & Sketch`（剪下&草图）或`Snipping Tool`（剪下工具）通过按下`Windows logo键 + Shift + S`并选择一个区域进行截图。

在`MacOS`中，我们可以使用`Command + Shift + 3`、`Command + Shift + 4`进行截图。
