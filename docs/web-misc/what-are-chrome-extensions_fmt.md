...
分机被压缩到`.crx`包，用户需要下载并安装该包。Chrome 扩展发布在 Chrome 网络商店中。
...
*   每个扩展名都需要一个清单文件。首先，创建一个`manifest.json`文件。
...
*   然后，为了演示，我们将在扩展中添加一个图标，点击该图标将打开我们创建的网页。将此添加到文件中。
