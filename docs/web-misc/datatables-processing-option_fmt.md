...
`processing` 选项用于指定加工指示器是否启用。当数据表忙于执行一些需要花费一些时间的操作时，会显示此指示器。这对于具有大量数据并且需要花费大量时间来执行任何操作的表非常有用。该指示器将能够通知用户操作正在进行并且没有卡住。一个 `true` 值显示指示器，一个 `false` 值删除指示器。
...
该选项具有如上所述的单一值，如下所述:
*   `value`: 这是一个布尔值，指定是否启用处理指示器。默认值为 `false`。

以下示例说明了该选项的使用。

## 示例 1

本示例启用在对列进行排序时可以看到的处理指示器。
