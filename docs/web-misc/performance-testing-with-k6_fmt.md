...
| 公制名称 | 描述 |
| :--- | :--- |
| `http_reqs` | k6 生成的 HTTP 请求总数。 |
| `http_req_blocked` | 在发起请求前阻塞（等待空闲 TCP 连接插槽）所花费的时间。float |
| `http_req_connecting` | 与远程主机建立 TCP 连接所花费的时间。float |
| `http_req_tls_handshaking` | 与远程主机进行 TLS 会话握手所花费的时间。 |
| `http_req_sending` | 向远程主机发送数据所花费的时间。float |
| `http_req_waiting` | 等待远程主机响应所花费的时间（也称为“首字节时间”或“TTFB”）。float |
| `http_req_receiving` | 从远程主机接收响应数据所花费的时间。float |
