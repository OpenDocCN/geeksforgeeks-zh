...
> 原文：[https://www.geeksforgeeks.org/how-to-pass-a-parameter-to-an-event-handler-or-callback/](https://www.geeksforgeeks.org/how-to-pass-a-parameter-to-an-event-handler-or-callback/)

`React js` 是当今最流行的 `JavaScript` 框架之一。由于框架的独特功能，有很多公司的网站都是建立在 `React` 上的。`React` `UI` 包含几个组件。用户可以在 `React` 用户界面上执行许多任务。每个组件都有不同的 `HTML` 元素，用户可以点击、悬停或触发任何形式的事件。
