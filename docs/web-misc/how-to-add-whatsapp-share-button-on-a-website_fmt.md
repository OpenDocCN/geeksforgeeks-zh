...
> 原文：[https://www.geeksforgeeks.org/how-to-add-whatsapp-share-button-on-a-website/](https://www.geeksforgeeks.org/how-to-add-whatsapp-share-button-on-a-website/)
...
设计一个简单的网页，上面有一个超链接。当用户点击此链接时，将完成共享。
...
这在台式机/笔记本电脑上是行不通的，所以我们添加 CSS 在大屏幕上隐藏。为此，使用了 CSS `@media` 查询。
...
该示例使用以上两个步骤实现。注意 `href` 属性指定了位置，这个请求被发送到 WhatsApp 应用程序。
...
在本例中，我们将从用户处获取输入，并使用 JavaScript 发送该消息。在这个例子中，我们定义了一个只在手机上显示内容的类。
