...
> 原文: [https://www.geeksforgeks.org/sweetalert-library-introduction-and-installation-with-the-example/](https://www.geeksforgeks.org/sweetalert-library-introduction-and-installation-with-the-example/)
...
**注意:** `swal()` 接受标题文本（字体大小比简单文本更粗更大）、简单文本和图标作为默认参数。您可以使用 JSON 来使用 Sweetalert 的任何其他功能。如果 `swal()` 中只给出一个文本，那么它将是简单文本。如果有两个文本，那么第一个将是标题文本，第二个将是简单文本。
...
1.  **标题:** 用于在警告框内写入标题。如果标题在 `Swal()` 中作为第一个参数给出，那么 JSON 属性 `title` 将覆盖该目标值。
...
*   **按钮和按钮:** `button` 用于创建单个按钮，而 `buttons` 用于创建多个按钮。默认情况下，在默认配置下，`buttons` 有两个按钮“取消”和“确定”。

## 例 4

可以给按钮和 `swal` 都添加 `className` 属性，改变按钮和对话框的样式，如下图所示:
...
*   **内容:** `content` 用于添加 `p` 元素、`h1` 元素、按钮、文本区、输入、`Div` 等任意元素。`content` 属性包含两件事：元素名称及其属性，属性表示元素的内部属性，如内联样式、宽度、高度、占位符、类名等。
...
**参考:** [https://sweetalert.js.org/](https://sweetalert.js.org/)