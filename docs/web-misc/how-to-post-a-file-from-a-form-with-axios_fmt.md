...
> 原文：[https://www.geeksforgeeks.org/how-to-post-a-file-from-a-form-with-axios/](https://www.geeksforgeeks.org/how-to-post-a-file-from-a-form-with-axios/)

在本文中，我们将讨论使用 Axios 库用表单数据发出 POST 请求。Axios 是一个基于 Promise 的 HTTP 客户端，可用于浏览器和 Node.js 开发。但是，在本文中，我们将严格参考 Axios 的客户端使用。
...
为了从表单中提取数据，我们将使用 `FormData()` 方法。`FormData` 方法以键值对的形式转换表单中的数据输入，以创建 `multipart/form-data` 对象。
...
2.  **用配置对象发布请求：**
    ...
    ```
    *   `method`：指定 HTTP 方法。
    *   `data`：一个包含 POST 数据的对象。
    *   `headers`（可选）：指定与请求相关联的头的对象。
