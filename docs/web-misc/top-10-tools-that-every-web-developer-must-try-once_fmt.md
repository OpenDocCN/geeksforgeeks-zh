...
> 原文:[https://www.geeksforgeeks.org/top-10-tools-that-every-web-developer-must-try-once/](https://www.geeksforgeeks.org/top-10-tools-that-every-web-developer-must-try-once/)
...
它是一个基于云的集成开发环境(`IDE`)，让您只需一个浏览器就可以编写、运行和调试各种流行的编程语言，如 `C`、`Python`、`JavaScript`、`PHP` 等。最初在 2010 年，`Cloud 9` 是一个开源软件，后来在 2016 年，亚马逊网络服务(`AWS`)收购了它。
...
*   `Atom` 可以跨各种操作系统工作，如 `OS X`、`视窗` 或 `Linux`。
...
*   它是跨平台的，因此可以在任何操作系统上运行，如 `视窗`、`Linux`、`苹果操作系统`、`Unix` 等。
*   它非常高效，而且超级轻便。
