...
> 原文: [https://www.geeksforgeeks.org/form-validation-using-the-jbvalidator-plugin/](https://www.geeksforgeeks.org/form-validation-using-the-jbvalidator-plugin/)

`jbvalidator` 是一个基于 `jQuery` 和 `Bootstrap` 的插件，支持客户端和服务器表单验证。提供的 `HTML` 数据属性易于使用和理解。该插件提供多语言功能以及自定义验证规则和消息。

从[官网](https://github.com/emretulek/jbvalidator)下载需要的预编译文件即可使用该插件。脚本文件可以包含在需要验证的页面中。

以下示例演示了不同类型的可用验证:

## 示例 1: 电子邮件和密码验证

以下代码演示了电子邮件 `id` 和密码的表单验证。
...
下面的代码片段演示了复选框的验证。请在 `form` 元素中使用上述 `HTML` 代码中的代码片段。
...
下面的代码片段演示了在用户表单元素中使用 `<textarea>` 元素。
