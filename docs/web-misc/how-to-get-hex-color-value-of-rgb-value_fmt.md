...
> 原文：[https://www.geeksforgeeks.org/how-to-get-hex-color-value-of-rgb-value/](https://www.geeksforgeeks.org/how-to-get-hex-color-value-of-rgb-value/)

给定 RGB 颜色值，任务是使用 JavaScript 获取该颜色的十六进制代码。

## 方法 1

*   调用用户定义函数 `convert()`，并使用 RGB 值作为参数。
...
本示例获取白色的 RGB 值，即 `rgb(255, 255, 255)`，并使用 JavaScript 将其转换为 `HexCode`。
...
该方法首先检查是否提供了十六进制值而不是 RGB 值。因为有些浏览器返回十六进制值来代替 RGB 值。本示例获取绿色的 RGB 值，即 `rgb(0, 128, 0)`，并通过创建函数将其转换为十六进制代码。
