...
> 原文：[https://www.geeksforgeeks.org/how-to-set-image-to-center-of-an-responsive-navbar/](https://www.geeksforgeeks.org/how-to-set-image-to-center-of-an-responsive-navbar/)
...
在这个方法中，我们使用自己的样式将图像居中放置在导航栏中。我们将把 `CSS` 代码嵌入到 `HTML` 代码中。在这里，我们已经将 `flex` 属性赋予了我们的品牌（图像或徽标）的宽度为 `100%`，并将内容调整为居中。这些属性将我们的徽标设置在导航栏的中心。
...
在这个方法中，我们可以省去自己编写额外的 `CSS` 代码。我们只需要添加一个带有类的 `div` 标签作为容器，并将 `navbar-brand`（图像或标志）放入这个 `div` 中。之后，我们只需要在 `navbar-brand` 类中添加类 `mx-auto`。`mx-auto` 类是 `Bootstrap` 类，只是调整左右两边的边距，使内容在中心对齐。
