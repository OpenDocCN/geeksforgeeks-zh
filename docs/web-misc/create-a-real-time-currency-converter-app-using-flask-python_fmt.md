...
> 原文: [https://www.geeksforgeeks.org/create-a-real-time-currency-converter-app-using-flask-python/](https://www.geeksforgeeks.org/create-a-real-time-currency-converter-app-using-flask-python/)
...
## 项目设置
创建一个新目录，并将其命名为`CURRENCY-CONVERTER`。
...
然后，创建一个新的文件夹名为`templates`，在里面创建一个 html 文件名为`home.html`。
...
### 2. 安装其他依赖项/python 包
这些是[`Flask`](https://pypi.org/project/Flask/)和[`requests`](https://pypi.org/project/requests/)。为此，打开终端并执行 Linux 命令。
...
之后可以运行网络服务器查看天气应用。为此，运行 `app.py` 文件。为此，在终端中编写以下 Linux 命令。

```
python app.py
```

然后打开任意网页浏览器，转到：`http://127.0.0.1:5000/`
