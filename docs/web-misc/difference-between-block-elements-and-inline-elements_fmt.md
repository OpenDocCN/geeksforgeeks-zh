...
> 原文: [https://www.geeksforgeeks.org/difference-between-block-elements-and-inline-elements/](https://www.geeksforgeeks.org/difference-between-block-elements-and-inline-elements/)
...
它们消耗可用的整个宽度，而不管它们是否足够。它们总是从新的一行开始，有顶部和底部的空白。它旁边不包含任何其他元素。
...
内联元素只占用足够的宽度，并允许它旁边的其他元素内联。内联元素不会从新行开始，也不会像块元素那样有上下边距。
...
从上面的输出来看，使用了三个不同的内联元素，分别是`<span>`、`<b>`、`<a>`。
