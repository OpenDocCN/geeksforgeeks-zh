...
> 原文: [https://www.geeksforgeeks.org/localstorage-and-sessionstorage-web-storage-apis/](https://www.geeksforgeeks.org/localstorage-and-sessionstorage-web-storage-apis/)

`SessionStorage`和`LocalStorage`被称为网络存储应用编程接口。通过使用这些 API，数据可以存储在客户端。
...
when we pass key in method, it will erase the complete data related to that key.
...
1.  Web 存储安全是一个大问题。强烈建议不要将敏感信息存储在网络存储中，因为它总是以纯文本格式存储数据，任何人都可以轻松窃取数据。所以千万不要用网络存储存储密码或支付凭证。
2.  Web 存储只能在客户端存储数据，只有客户端或我们的 javascript 可以处理这些数据。要在服务器端保存数据，[Cookies](https://www.geeksforgeeks.org/cookies-used-website/)是更好的选择。