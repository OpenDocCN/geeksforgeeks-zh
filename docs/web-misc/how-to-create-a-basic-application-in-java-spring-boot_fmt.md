...
> 原文: [https://www.geeksforgeeks.org/how-to-create-a-basic-application-in-java-spring-boot/](https://www.geeksforgeeks.org/how-to-create-a-basic-application-in-java-spring-boot/)
...
1.  转至 [Spring Initializr](https://start.spring.io/)。
2.  按照要求填写细节。对于本申请:
    *   项目: `Maven`
    *   语言: `Java`
    *   Spring Boot: `2.2.8`
...
4.  解压 zip 文件。现在打开一个合适的 IDE，然后转到 *File* -> *New* -> *Project from existing sources* -> *Spring-boot-app* 并选择 `pom.xml`。在提示时单击 import changes 并等待项目同步。
...
转到 *src* -> *main* -> *java* -> *com.gfg.spring.boot.app*，创建一个名为 `Controller` 的 Java 类作为 Controller，并添加注释 `@RestController`。现在创建一个 GET API，如下所示：

```java
@RestController
public class Controller {

    // One syntax to implement a GET method
    @GetMapping("/")
    public String home() {
...
此应用程序现在已准备就绪。运行 `SpringBootAppApplication` 类并等待 Tomcat 服务器启动。
...
现在进入浏览器，输入网址 `localhost:8080`。观察输出，现在对 `localhost:8080/gfg` 执行同样的操作。

**输出:**
运行上述应用程序时，会生成以下输出:

`https://media.geeksforgeeks.org/wp-content/uploads/20200526135420/output7.mp4`

**结论:** 按照上面的步骤，我们创建了一个简单的 [RESTful](https://www.geeksforgeeks.org/rest-api-introduction/) 路由，其中包含一些消息。为了使应用程序更加复杂，可以添加更多的 RESTful 路由来执行服务器上的 CRUD 操作。