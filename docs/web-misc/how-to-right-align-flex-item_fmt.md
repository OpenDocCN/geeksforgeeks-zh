...
## 示例 1: 使用 `flex-direction` 属性右对齐弯曲项
...
当伸缩方向设置为 `row-reverse` 时，它不仅会右对齐伸缩项目，还会反转项目的顺序。简单地说，flex-items 现在从右向左扩展，如给定的图所示。
...
## 示例 2: 使用 `justify-content` 属性来右对齐伸缩项
...
当调整内容设置为 `flex-end` 时，它会立即将所有伸缩项沿着主轴移动到伸缩容器的末端，即伸缩项右对齐。它与上面使用的方法的不同之处仅在于方向，因为在这种情况下，flex-item 将仅从左向右扩展。给定的数字会更清楚。
