...
> 原文: [https://www.geeksforgeks.org/datatables-displaystart-option/](https://www.geeksforgeks.org/datatables-displaystart-option/)

`DataTables` 是一个 jQuery 插件，可以用来为网页的 HTML 表格添加交互和高级控件。这也允许根据用户的需要搜索、排序和过滤表中的数据。数据表还公开了一个强大的应用编程接口，可以进一步用来修改数据的显示方式。

`displayStart` 选项用于指定数据表开始显示其行的行。当启用分页并且表必须从某一行开始时使用。指定一行将从该行对应页的表开始。
