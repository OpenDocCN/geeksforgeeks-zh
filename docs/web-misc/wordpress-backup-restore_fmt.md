...
要备份一个 WordPress 网站，我们需要安装一个插件。你可以安装任何相关的插件，你想要的功能备份网站。我们将安装一个名为 `UpdraftPlus` 的插件。让我们看看步骤：

### 第一步

进入 WordPress 登录面板，输入用户名和密码登录 WordPress 仪表盘。您可以通过在网站网址的末尾添加 `/wp-login.php` 来访问登录面板。在我的例子中，它是：`http://127.0.0.1/wordpress/wp-login.php`。
...
我们必须搜索名为 `UpdraftPlus` 的插件并安装它。为此：

在左侧，您将看到许多选项。转到 `插件` > `添加新插件`。

![](img/55b0e6309117033048cc50afadebb28e.png)

添加新插件

搜索一个名为 `UpdraftPlus` 的插件。这是我们将要用来备份我们的 WordPress 网站的插件。
...
成功安装并激活插件后，您会在左侧的设置中发现一个名为 `UpdraftPlus 备份` 的新选项。点击那个。
...
假设你让自己陷入了一个无法访问页面、功能或者整个网站的境地，你现在想要恢复网站：

### 第一步

点击左侧菜单栏中的 `设置` > `UpdraftPlus 备份`。
...
转到 `现有备份` 部分，在那里您会找到到目前为止所做的所有备份的列表。
...
进入下一页，进入 `选择要恢复的组件`，然后点击 `下一步`，建议选择所有选项：
