...
> 原文: [https://www.geeksforgeeks.org/how-to-check-whether-an-image-is-loaded-or-not/](https://www.geeksforgeeks.org/how-to-check-whether-an-image-is-loaded-or-not/)
...
*   `onload`: 加载并执行图像时，会触发加载事件
*   `onerror`: 如果在执行过程中出现错误，则会触发 `onerror` 事件
...
使用 HTML DOM 中的图像 `complete` 属性。如果加载了图像，此属性返回一个布尔值，否则返回 `false`。
...
这里，变量 `load` 检查图像是否被加载。如果加载了图像，则警告为 `true`，否则警告为 `false`。
