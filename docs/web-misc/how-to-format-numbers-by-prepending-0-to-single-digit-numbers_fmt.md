...
> 原文：[https://www.geeksforgeeks.org/how-to-format-numbers-by-prepending-0-to-single-digit-numbers/](https://www.geeksforgeeks.org/how-to-format-numbers-by-prepending-0-to-single-digit-numbers/)
...
`slice()` 方法用于从指定的开始和结束索引中提取字符串的部分。首先，数字前面加一个 `'0'` 字符，不管它是一个数字。这将使一位数变成两位数，但两位数将被转换成三位数，并增加 `'0'`。使用 `slice()` 方法提取结果编号的最后 2 位数字。
