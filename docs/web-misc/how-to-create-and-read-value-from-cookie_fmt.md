...
> 原文: [https://www.geeksforgeeks.org/how-to-create-and-read-value-from-cookie/](https://www.geeksforgeeks.org/how-to-create-and-read-value-from-cookie/)
...
该函数检索浏览器中存储的 cookie 数据。cookie 字符串在从服务器发送到浏览器时会自动编码。因此，在检索实际数据之前，需要对其进行解码。接下来，解码后的字符串被拆分成一个数组，以获得所有的 cookie 名称-值对。在数组中循环查找字段名和相应的字段值。如果找到 cookie，则返回值，否则函数返回空字符串。
...
加载网页后，将调用 `runApp()` 函数，该函数将检查浏览器中是否存在 cookie，并检索该 cookie，否则将为其创建新的 cookie。
