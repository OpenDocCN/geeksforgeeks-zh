...
标量类型表示 GraphQL 类型系统中的原始叶值。GraphQL 响应采用层次树的形式；这些树上的叶子是 GraphQL 标量。
...
GraphQL 赋予开发人员定义自定义标量的权力，适合他们自己的需求。我们可以如下所示声明一个自定义标量，并将其添加到模式中。要定义自己的标量，必须从头开始编写验证、序列化和解析器。还有一个叫做 [`graphql-scalars`](https://github.com/Urigo/graphql-scalars) 的很棒的包，包含可能需要的通用标量。
