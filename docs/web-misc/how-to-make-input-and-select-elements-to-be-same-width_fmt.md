...
> 原文: [https://www.geeksforgeeks.org/how-to-make-input-and-select-elements-to-be-same-width/](https://www.geeksforgeeks.org/how-to-make-input-and-select-elements-to-be-same-width/)

作为一个初学者，在使用`CSS`和`HTML`的时候，你可能注意到了像输入这样的表单元素有一个问题，选择当输出在浏览器中打开时，两个元素的宽度不一样。它看起来不太符合用户。
...
*   `box-sizing`: 定义如何计算元素的高度和宽度。
*   `-moz-box-sizing`: 仅在`Mozilla Firefox`浏览器中支持。
*   `-webkit-box-sizing`: 仅谷歌`Chrome`浏览器支持。
