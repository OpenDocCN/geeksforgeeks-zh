# Web Touch Event API | TouchEvent.changedTouches

> 哎哎哎:# t0]https://www . geeksforgeeks . org/web-touch event-API-touch event-changedtouche/

`TouchEvent.changedTouches` 是一个只读属性，它是一个触摸列表。代表直接参与此事件的每个接触点的接触列表：
- **`touchstart`：** 在 `touchstart` 事件中变为活动的接触点列表。
- **`touchmove`：** 在 `touchmove` 事件期间移动的触摸点列表。
- **`touchend`：** 在 `touchend` 事件中被停用的触摸点列表。
- **`touchenter`：** 在 `touchenter` 事件期间进入触摸表面的触摸点列表。
