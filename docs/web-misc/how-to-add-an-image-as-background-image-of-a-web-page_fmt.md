...
> 原文:[https://www . geeksforgeks . org/如何添加图片作为网页背景图片/](https://www.geeksforgeeks.org/how-to-add-an-image-as-background-image-of-a-web-page/)

在本文中，我们将添加一个图像作为网页的背景图像。背景图片用于使网站更具互动性和吸引力。它可以应用于许多风格。

**进场:**

*   在`body`标签中，通过传递图像的 URL 或位置路径，在`background`属性中指定一个背景图像。
*   添加`CSS`样式属性。
...
<body background="https://media.geeksforgeeks.org/wp-content/cdn-uploads/20190710102234/download3.png">
    <h1>GeeksforGeeks</h1>
...
**示例 2:** 在本例中，我们将使用`background-image`属性在 CSS 代码中指定图像的 URL 或路径。
...
**注意:**我们也可以在`style`标签下添加 CSS 代码，也可以单独创建文件。
...
**1. 背景-重复:**`background-repeat`属性用于在任何方向添加或删除背景图像的重复。
...
**2. 背景-附件和大小:**`background-attachment`属性用于指定背景图像相对于其容器的附件类型。`background-size`属性用于设置背景图像的尺寸。
