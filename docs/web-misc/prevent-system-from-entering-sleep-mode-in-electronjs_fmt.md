...
`powerSaveBlocker`模块是**主进程**的一部分。 要在**渲染器进程**中导入和使用此模块，我们将使用电子`remote`模块。 有关远程模块的更多详细信息，请参阅此[链接](https://www.electronjs.org/docs/api/remote)。
...
我们将按照给定的步骤为`powerSaveBlocker`模块的实现构建电子应用程序。
...
### Step 3
Create the `index.html` file within the `src` directory. We will also copy the boilerplate code for the `index.html` file from the above-mentioned link. We have modified the code to suit our project needs.
...
**输出：**此时，我们的应用程序已经设置好，我们可以启动该应用程序来检查 GUI 输出。 要启动电子应用程序，运行命令。
...
### Step 4
The `Prevent Display from Sleeping` button does not have any functionality associated with it yet.
...
*   **type: String** This value cannot be empty. It represents the type of Blocker that you want to implement. It can assume any of the following two values:
    *   `prevent-app-suspension`:此值防止应用程序被挂起。 它使系统保持活动状态，但可以关闭屏幕，如*Windows*中的**注销**操作。 此值可在流式传输音频或下载文件等时使用。
    *   `prevent-display-sleep`:此值阻止显示器进入休眠状态。 使系统和屏幕保持活动状态。 该值可以在流传输视频等时使用。

**注**-`prevent-display-sleep`始终优先于`prevent-app-suspension`。 如果多个渲染器进程实现不同的拦截器，则优先级较高的拦截器将始终有效，直到请求得到解决。
