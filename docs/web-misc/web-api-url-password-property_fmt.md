...
> 原文: [https://www.geeksforgeeks.org/web-api-url-password-property/](https://www.geeksforgeeks.org/web-api-url-password-property/)

`URL.password` 属性返回一个包含网址域名前指定密码的 `USVString`。`USVString` 是 Unicode 标量值的序列。
...
**返回值:** 该属性返回一个 `USVString`，包含在 URL 的域名前指定的密码。
