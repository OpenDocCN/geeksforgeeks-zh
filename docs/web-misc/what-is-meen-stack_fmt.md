...
`Ember` 是一个客户端框架，而 `Node`、`Express` 和 `Mongo` 在服务器端使用。`MongoDB` 是一个存储数据的 NoSQL 数据库。`Node` 是服务器端技术，而 `Express` 是基于 `Node` 的框架。

## MEEN 堆栈的组件

### 1. MongoDB
这是一个无 SQL 数据库，它有类似 JSON 的文档。`MongoDB` 是由 `MongoDB Inc.` 开发的。`MongoDB` 有一种称为 `BSON` 的二进制 JSON 格式，它提高了效率，可以快速索引和搜索，性能也更好。
...
### 2. ExpressJS
它是一个基于 `NodeJS` 的 web 框架，有助于服务器端任务和构建 web 应用。它是免费的开源软件。通过后端任务，可以用 `Express` 构建 `API`。
...
*   是客户端框架。
*   `EmberJS` 是开源的。
*   它提供了一种新的绑定语法，因为它使用了 `HTMLBars` 模板引擎。
*   此外，另一个引擎 `Glimmer` 渲染引擎有助于提高渲染速度。
...
*   `Ember` 是向后兼容的，因此提供了稳定性，旧版本运行将不会面临问题。
*   在 GitHub 上有 800 多个贡献者，在 StackOverflow 上也有很好的社区支持。
*   强大的附加组件。
*   拥有优秀的文档。
*   基于 `Glimmer` 渲染引擎构建，因此具有高性能。

### 4. NodeJS
它是一种用于在服务器上运行 `JS` 的服务器端技术。跨平台和开源。学习 `NodeJS` 的先决条件是首先学习 `JavaScript`。它运行在 `V8` 引擎上，有助于构建快速且可扩展的应用程序。
