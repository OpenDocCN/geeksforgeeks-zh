...
> 原文: [https://www.geeksforgeeks.org/ways-to-capture-the-backspace-and-delete-on-the-onkeydown-event/](https://www.geeksforgeeks.org/ways-to-capture-the-backspace-and-delete-on-the-onkeydown-event/)

给定 HTML 文档。任务是检测在按键事件中何时按下退格键和删除键。这里讨论了两种方法，一种使用 `event.key`，另一种在 JavaScript 的帮助下使用 `event.keyCode`。

## 方法 1

- 从输入元素获取输入，并使用 `onkeydown` 事件上的 `el.addEventListener()` 方法向输入元素添加一个事件侦听器。
