...
> 原文: [https://www.geeksforgeeks.org/difference-between-remove-and-detach-methods/](https://www.geeksforgeeks.org/difference-between-remove-and-detach-methods/)

在看 jQuery 这两种方法的区别之前，让我们先了解一下方法。

## `remove()`

`remove()` 方法从 DOM 中移除匹配的元素。当我们对任何元素应用 `remove()` 方法时，该元素内的所有内容和元素本身都将从 DOM 中移除。所有嵌套元素、事件处理程序或该元素中存在的任何类型的数据都将被移除。
...
`detach()` 方法和上面的 `remove()` 方法差不多。唯一的区别是 `detach()` 方法保留了与被移除元素相关的所有数据。我们所有的数据都是安全的，所以我们可以随时将它们重新插入 DOM。
...
我们将首先从 DOM 中移除一个元素，然后尝试将移除的元素重新插入到 DOM 中。
...
<body>
    ...
    <script>
        // A variable to store the data of removed element
        let rem;

        // Removing h3 tag first on clicking detach button
        $("#detach").click(function () {
            rem = $("h2").detach();
        })

        // Reinserting h2 tag on clicking attach button
