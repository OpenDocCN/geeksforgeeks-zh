...
*   [`Collect.js all()` 方法](https://www.geeksforgeeks.org/collect-js-all-method/)
*   [`Collect.js sortByDesc()` 方法](https://www.geeksforgeeks.org/collect-js-sortbydesc-method/)

## [`Collect.js all()` 方法](https://www.geeksforgeeks.org/collect-js-all-method/)

[`all()`](https://www.geeksforgeeks.org/collect-js-all-method/) 函数返回底层数组中集合所代表的所有元素。JavaScript 数组首先被转换成一个集合，然后该函数被应用于该集合。
...
## [`JavaScript sortByDesc()` 方法](https://www.geeksforgeeks.org/collect-js-sortbydesc-method/)

如果元素是对象，那么我们可以使用 [`sortByDesc()`](https://www.geeksforgeeks.org/collect-js-sortbydesc-method/) 方法进行排序，该方法使用一个键作为参数对元素集合进行降序排序。
...
<script>
    const collect = require('collect.js');
    const arr = ['Science', 'Welcome', 'Computer', 'GeeksforGeeks']
