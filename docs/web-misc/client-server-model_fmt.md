...
> 原文：`https://www.geeksforgeeks.org/client-server-model/`
...
当我们谈论`客户`这个词时，它指的是使用特定服务的个人或组织。类似地，在数字世界中，`客户端`是计算机（`主机`），即能够从服务提供商（`服务器`）接收信息或使用特定服务。

### 服务器

同样，当我们谈论`服务器`这个词时，它指的是提供某物的人或媒介。类似地，在这个数字世界中，`服务器`是一台提供信息（数据）或访问特定服务的远程计算机。
