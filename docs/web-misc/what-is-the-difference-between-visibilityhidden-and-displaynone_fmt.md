...
> 原文: [https://www.geeksforgeeks.org/what-is-the-difference-between-visibilityhidden-and-displaynone/](https://www.geeksforgeeks.org/what-is-the-difference-between-visibilityhidden-and-displaynone/)

两者的`可见性` & `显示`属性在 CSS 中相当有用。`visibility: "hidden"`属性用于指定一个元素在 web 文档中是否可见，但隐藏的元素会占用 web 文档中的空间。`可见性`是 CSS 中指定元素可见性行为的属性。CSS 中的`display`属性定义了组件(如`div`、超链接、标题等)将如何放置在网页上。`display: "none"`属性用于指定网站上是否存在元素。

## `可见性`属性

该属性用于指定一个元素在 web 文档中是否可见，但隐藏的元素会占用 web 文档中的空间。
...
CSS 中的`Display`属性定义了组件(`div`、超链接、标题等)将如何放置在网页上。
...
所以，`display: "none"`和`visibility: "hidden"`的区别从名字本身我们就可以看出：`display: "none"`完全摆脱标签，因为它从未存在于 HTML 页面中，而`visibility: "hidden"`只是使标签不可见，它仍然会在 HTML 页面上占据空间只是不可见。

## 示例

此示例说明了在 CSS 中使用`可见性`属性 & `显示`属性。
...
本示例说明了在 Javascript 中显示属性 & 可见性属性的使用。
