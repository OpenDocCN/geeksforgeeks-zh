...
现在从这个例子开始，我们将在命令行上使用下面的命令安装两个名为`express`和`chalk`的包的旧版本。
...
当使用`outdated`命令时，我们会得到以下输出。在输出中，第一列是包的名称，第二列显示为我们的项目安装的版本。第三列表示想要的版本，也就是我们可以安全升级到的版本，而不需要任何中断的更改。第四列代表该软件包的最新版本。

![](img/1ecbff7a34b180a2e011ed410596d236.png)

现在，当我们使用`npm update`命令时，两个包都被更新到最新的安全版本，我们得到以下输出。由于`chalk`和最新版本的`express`是一样的，所以更新到了最新版本。我们唯一剩下的过时的依赖是`chalk`。

![](img/ac80194793928124ca0721f202996dd6.png)

现在当我们使用`uninstall`命令卸载`chalk`包时如下所示:
