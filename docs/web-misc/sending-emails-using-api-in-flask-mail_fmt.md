...
> 原文: [https://www.geeksforgeeks.org/sending-emails-using-api-in-flask-mail/](https://www.geeksforgeeks.org/sending-emails-using-api-in-flask-mail/)
...
8) `MAIL_ASCII_ATTACHMENTS` : 如果设置为真，附件文件名转换为 ASCII。
9) `MAIL_DEFAULT_SENDER` : 设置默认发件人。
10) `MAIL_SUPPRESS_SEND` : 如果 `app.testing` 设置为 `true`。
11) `MAIL_MAX_EMAILS` : 设置最大发送邮件数。
