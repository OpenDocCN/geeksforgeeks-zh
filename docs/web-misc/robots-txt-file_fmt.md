...
这样做最重要的原因是为了保持一个网站的所有部分都是私有的，这样就没有机器人可以访问它。这也有助于防止搜索引擎索引某些文件。此外，它还指定了站点地图的位置。

## 如何创建 robots.txt 文件？

`robots.txt` 文件是放在你的网络服务器上的一个简单的文本文件，它告诉像 `Googlebot` 这样的网络爬虫是否应该访问一个文件。这个文件可以在记事本中创建。语法由下式给出:
...
这意味着 `Googlebot` 被允许以 20 毫秒的延迟抓取每个页面，除了那些仅在世界协调时 0200–0300 的时间间隔内不允许的网址。
