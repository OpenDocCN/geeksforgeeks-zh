...
> 原文: [https://www.geeksforgeeks.org/difference-between-domcontentloaded-and-load-events/](https://www.geeksforgeeks.org/difference-between-domcontentloaded-and-load-events/)
...
`load` 事件执行方式不同。一旦加载了所有组件，即 DOM 层次结构以及网页的相关功能，如 CSS 文件、JavaScript 文件、图像/图片和外部链接，此事件就完成了。所以基本上，`load` 事件有助于了解页面何时已经完全加载。
