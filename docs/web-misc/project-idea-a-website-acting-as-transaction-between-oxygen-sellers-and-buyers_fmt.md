...
要使该项目在您自己的本地系统上运行，请确保您安装了 `node.js` 和 `Django`。
...
2.  **在本地服务器上运行：**在终端/命令提示符中，导航到终端/命令提示符中的 `oxy-zone-front end/` 目录。然后运行以下命令：
...
**2. 为 `Reaction APP` 构建 `API`：**在 `covidproject` 的前端 `APP` 中，我们在 `Models.py` 和中创建了 2 个模型，分别是 `Sellers` 和 `Places`
...
在氧气危机时期，人们越来越需要了解氧气销售商，这款应用可以帮助人们。这个 `App` 的成功取决于注册和使用它的人。呆在家里，注意安全，接种疫苗！
