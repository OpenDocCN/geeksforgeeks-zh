...
在上面的语法中，`element_name` 是元素的名称，可以是任何名称。`attribute1`，`attribute2`，…是具有唯一属性名的 XML 属性。然后在内容部分，可以写入任何消息，最后，元素名称结束。
...
在上例中，XML 元素是 `text`，`category` 是属性名，`"message"` 是属性值，属性名及其值总是成对出现。使用属性名时不使用任何引号，但属性值使用单引号(`'`)或双引号(`" "`)。
