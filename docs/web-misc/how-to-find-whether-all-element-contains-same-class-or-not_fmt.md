...
> 原文：[https://www.geeksforgeeks.org/how-to-find-whether-all-element-contains-same-class-or-not/](https://www.geeksforgeeks.org/how-to-find-whether-all-element-contains-same-class-or-not/)
...
*   由于 `querySelectorAll()` 方法返回所有选定元素的节点列表，我们首先需要使用 `Array.from()` 方法将这个节点列表转换成一个数组，这样我们就可以在上面调用 `every()` 方法。
...
<head>
    ...
    <script>
        ...
        let check = buttons.every((btn) => btn.classList.contains("active"));
...
如果将 `active` CSS 类添加到所有按钮中，将显示以下输出。
