...
> 原文: [https://www.geeksforgeeks.org/new-features-in-ecmascript-2021-update/](https://www.geeksforgeeks.org/new-features-in-ecmascript-2021-update/)
...
逻辑赋值运算符引入了新的运算符，将逻辑运算符和赋值表达式结合在一起。
...
当值为假时进行赋值。赋值操作仅在 `x` 为假值时发生。如果 `x` 包含真值 `1`，则不会发生赋值。这里 `x` 包含 `0`，因此发生赋值。
...
符号 `??` 是 JavaScript 中的空值合并运算符。它检查一个值是否为 `null` 或 `undefined`。
...
如果我们想替换字符串中一个子字符串的所有实例，那么这个新方法 `replaceAll()` 非常有用。
...
`Promise.any()` 方法返回一个承诺，一旦其中一个承诺被解决，该承诺就会被解决。它是 [`Promise.all()`](https://www.geeksforgeeks.org/javascript-promise-all-method/) 方法的反面，等待所有承诺解决后再解决。

当所有承诺都被拒绝时会发生什么，该方法会抛出一个带有拒绝原因的 `AggregateError` 异常。我们已经在 `try-catch` 块中编写了代码。
...
### 6. Private Getters 和 Setters

就像 private 方法一样，现在我们可以制作 getter 和 setter，这样它们只能在类内部或者通过创建的实例来访问。
