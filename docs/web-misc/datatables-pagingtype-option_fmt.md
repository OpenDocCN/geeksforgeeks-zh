...
`pagingType` 选项用于指定将显示在数据表下方进行分页的控件类型。它接受一个字符串值，该值可以通过使用 6 种内置类型的可用控件来指定。这些由以下值指定。
...
在这种类型的控件中，仅显示“第一个”、“上一个”、“下一个”和“最后一个”按钮。
...
在这种类型的控件中，“第一个”、“上一个”、“下一个”和“最后一个”按钮与页码一起显示。
