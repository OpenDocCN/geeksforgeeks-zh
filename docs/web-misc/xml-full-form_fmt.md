...
*   **标记:** 标记以 `<标记>` 开始，以 `</标记>` 和空元素标记 `<换行 />` 结束。
...
*   **描述性:** `XML` 数据格式是一种描述性更强的格式，如果你熟悉 `HTML`，那么你就可以很容易地理解 `XML`，它在你看来就是一个普通的文本。
*   **可验证:** 验证出现在脑海中当您必须遵循数据的某些特定结构时，您可以准确描述 `XML` 数据文件在另一个 `XML` 文件中应该如何结构化。
