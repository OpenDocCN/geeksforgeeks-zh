...
> 原文：[https://www.geeksforgeeks.org/web-crawling-using-breadth-first-search-at-a-specified-depth/](https://www.geeksforgeeks.org/web-crawling-using-breadth-first-search-at-a-specified-depth/)
...
*   **美化程序：** 是一个从 HTML 和 XML 文件中提取数据的 python 库。我们可以将一个网页转换成一个 `BeautifulSoup` 对象，然后提取出 HTML 标签和它们的内容。
...
1.  首先，我们导入已安装的库。
2.  然后，我们创建两个称为 `links_intern` 和 `links_extern` 的空集合，它们将分别存储内部和外部链接，并确保它们不包含重复项。
3.  然后，我们创建一个名为 `level_crawler` 的方法，该方法获取一个输入 URL 并对其进行爬网，并使用以下步骤显示所有内部和外部链接–
    *   定义一个名为 `temp_urls` 的集合来临时存储这些 url。
    *   使用 `urlparse` 库提取 `input_url` 的域名。
    *   使用 HTML 解析器创建一个 `BeautifulSoup` 对象。
    *   从 `BeautifulSoup` 对象中提取所有锚点标签。
    *   从锚点标签中获取 `href` 标签，如果它们是空的，不要包含它们。
    *   使用 `urljoin` 方法，创建绝对 URL。
    *   检查网址的有效性。
    *   如果 `href` 有效，并且 `input_url` 的域不在 `href` 标签中，并且不在 `links_extern` 集中，请将其包含到 `links_extern` 集中。
    *   否则，如果不存在，将其添加到 `links_intern` 链接集，并打印并放入临时 `temp_urls` 集中。
    *   返回包含已访问内部链接的临时 `temp_urls` 集。这套将在以后使用。
4.  如果深度为 0，我们按原样打印 `input_url`。如果深度为 1，我们称之为上面定义的 `level_crawler` 方法。
5.  否则，我们执行广度优先搜索(BFS)遍历，将 URL 页面的形成视为树形结构。在第一层，我们有输入网址。在下一个级别，我们有输入网址中的所有网址，等等。
6.  我们创建一个队列 `queue`，并将输入 `input_url` 添加到其中。然后我们弹出一个 `url` 并将里面所有的 `urls` 插入队列。我们这样做，直到特定级别的所有 `url` 都没有被解析。我们重复该过程的次数与输入深度相同。
