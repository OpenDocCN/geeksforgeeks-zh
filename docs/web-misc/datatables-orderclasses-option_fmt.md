...
`orderClasses` 选项用于指定当前排序的列是否会应用一个类。默认情况下，数据表将类`sorting_1`、`sorting_2`和`sorting_3`应用于已排序的列。这将突出显示已排序的列，以便区分它们。类后的数字表示使用的排序级别，最大级别为 3，之后重复最后一个类。
...
*   `value`: 这是一个布尔值，用于指定是否使用预定义的类突出显示表中当前排序的列。默认值为`true`。

以下示例说明了该选项的使用。

## 示例 1

在此示例中，排序类被禁止添加到列中。三个排序类的背景色已经使用 CSS 进行了修改，以使差异更加明显。
...
在此示例中，相关类被启用并添加到列中。三个排序类的背景色已经使用 CSS 进行了修改，以使差异更加明显。
...
[https://datatables.net/reference/option/orderClasses](https://datatables.net/reference/option/orderClasses)