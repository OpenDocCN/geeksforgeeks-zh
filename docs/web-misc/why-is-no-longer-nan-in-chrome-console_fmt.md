...
> 原文：[https://www.geeksforgeeks.org/why-is-no-longer-nan-in-chrome-console/](https://www.geeksforgeeks.org/why-is-no-longer-nan-in-chrome-console/)
...
*   如果 `value1` 或 `value2` 是字符串，那么结果将是一个字符串，输出将是 `value1` 和 `value2` 的拼接；
*   否则，结果被转换为数字，并输出 `value1` 和 `value2` 的和。
...
这是一种使用 `{}` 括号创建对象的简单方法。对象可以在 `{}` 中包含逗号分隔的键-值对，其中键是属性或方法名，值是任何数据类型或函数的属性值。
...
**解释：** 因此，在上面的例子中，一个空对象被转换成字符串，产生以下结果。因此，通过连接 `""` 和 `"[object Object]"` 来创建结果。
...
现在，Chrome 开发者工具自动将所有以 `{` 开始、以 `}` 结束的内容包装在一对隐式括号中（[参见代码](https://chromium.googlesource.com/chromium/src.git/+/4fd348fdb9c0b3842829acdfb2b82c86dacd8e0a%5E!/#F2)），以强制将其求值为表达式。
