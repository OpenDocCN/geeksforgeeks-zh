...
> 原文：[https://www.geeksforgeeks.org/website-hosting-and-premium-domain-for-free/](https://www.geeksforgeeks.org/website-hosting-and-premium-domain-for-free/)
...
虽然有很多免费的托管平台，如 `000Webhost`、`Firebase` 等，但我们将使用 `GitHub` 来托管网站。在 [GitHub](https://github.com) 上托管一个网站也将允许我们通过 `GitHub Pages` 选项提供我们的网站。你可以关注[这篇 GFG 文章来托管你的](https://www.geeksforgeeks.org/using-github-to-host-a-free-static-website/)网站或者继续这篇文章。
...
现在你必须导航到 [**Azure 门户**](https://portal.azure.com/)。从那里，你可以看到一个搜索栏。你必须从那里搜索“**应用服务域**”。你也可以在搜索前看到。选择它，您将被带到另一个页面，您可以在那里添加新域。如果您已经有了任何域（或处于后期阶段），它将在这里列出。
...
你要再次点击“**+记录集**”。现在你要把“**类型**”从 **A** 换成上市的 **CNAME**。在“**名称**”部分，如果需要，您必须添加子域。可以是类似 `www` 这样的东西，也可以是你喜欢的任何东西。如果你要用 `www`，你的网站也可以通过 `www.domain_name.domain_extension` 进入。如果你用的是像 `developer` 这样的子域，你的网站可以通过 `developer.domain_name.domain_extension` 进入。

添加子域后，必须添加“**别名**”，这是您当前可以访问托管网站的链接。由于我们使用的是 `GitHub Pages`，您的网站将可以通过 `username.github.io` 访问。你必须在别名部分加上这个，并用一个**点（.）**。因此，别名应该类似于 `username.github.io.`。之后，可以点击“**确定**”。现在，`Azure` 上的所有工作都结束了。

![](img/6d524b5b3d02ce715eeecdcd14d30537.png)

添加 IP 地址

![](img/88dcd042a40029e3ffd31cb3b40713a5.png)

添加 CNAME

## 将域名连接到 GitHub

现在，您必须回到您的 `GitHub` 存储库。转到设置部分，然后转到 `GitHub Pages` 部分（您之前已经检查过）。如果朝底部查看，您将可以选择添加一个自定义“**域**”。您可以在该空间中添加您的裸域名（没有子域或协议）。如果域名是 `sultanistechgeek.com`，你可以在那里添加，就这样，然后点击“**保存**”。
