...
> 原文：[https://www.geeksforgeeks.org/how-to-set-unset-session-variable-in-codeigniter/](https://www.geeksforgeeks.org/how-to-set-unset-session-variable-in-codeigniter/)
...
可以使用键值对分配会话索引。可以使用赋值运算符将特定的键赋给一个值。该值可以是字符串、键，甚至是数组。
...
也可以使用 CodeIgniter 中的 `set_userdata()` 方法分配会话值。此方法将一个键作为第一个参数，并将接下来是要分配的值。
...
可以通过将会话变量赋给空值来取消设置。这会破坏存储在该键值中的值。
