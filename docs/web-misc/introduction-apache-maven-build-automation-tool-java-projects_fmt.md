...
> 原文：[https://www.geeksforgeeks.org/introduction-apache-maven-build-automation-tool-java-projects/](https://www.geeksforgeeks.org/introduction-apache-maven-build-automation-tool-java-projects/)
...
2.  **`modelVersion`**：`modelVersion` 表示您正在使用的 POM 模型的版本。maven 2 和 maven 3 使用 `4.0.0` 版本。
...
5.  **`version`**：`version` 元素包含项目的版本号。如果你的项目已经发布了不同的版本，那么给出你项目的版本是很有用的。
...
4.  **`scope`**：这个元素用于定义这个 maven 项目的范围，可以是编译、运行时、测试、提供的系统等。
5.  **`packaging`**：`packaging` 元素用于将我们的项目打包成输出类型，如 JAR、WAR 等。
...
**参考**：[Apache Maven 文档](https://maven.apache.org/guides/getting-started/index.html)