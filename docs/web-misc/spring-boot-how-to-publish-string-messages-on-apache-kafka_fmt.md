...
> 原文：[https://www.geeksforgeeks.org/spring-boot-how-to-publish-string-messages-on-apache-kafka/](https://www.geeksforgeeks.org/spring-boot-how-to-publish-string-messages-on-apache-kafka/)
...
2.  在集成开发环境中打开项目，并同步依赖项。现在用注解 `@RestController` 创建一个新的类 `Controller`。这个类处理所有 RESTful 路由。
...
4.  启动 Zookeeper 和 Kafka 服务器。现在我们需要创建一个名为 `gfg` 的新主题。为此，请打开一个新的命令提示符窗口，并将目录更改为 Kafka 目录。
...
6.  现在要实时查看 Kafka 服务器上的消息，请使用下面的命令：
...
> `localhost:8080/kafka/publish/{your_message}`
