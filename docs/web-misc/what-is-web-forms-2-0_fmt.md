...
*   HTML5 引入了一个名为 `<output>` 的新元素来表示不同类型脚本的输出，就像脚本编写的那样。
*   `for` 属性的输出元素用于指定该元素与文档中影响计算的其他元素之间的关系。
...
*   HTML5 引入了一个名为 `placeholder` 的新属性。
*   通过 `<input>` 和 `<textarea>` 元素上的 `placeholder` 属性，用户能够知道他们可以在字段中输入什么。
*   占位符文本不能包含换行符或回车。
...
*   `required` 的属性用于代替 Javascript 验证。
