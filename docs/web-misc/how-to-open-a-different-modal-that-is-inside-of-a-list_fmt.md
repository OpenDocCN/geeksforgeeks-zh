...
> 原文：[https://www.geeksforgeeks.org/how-to-open-a-different-modal-that-is-inside-of-a-list/](https://www.geeksforgeeks.org/how-to-open-a-different-modal-that-is-inside-of-a-list/)
...
列表和模态是 Bootstrap 4 中各种各样的预定义组件之一。模态是基于 HTML、CSS 和 JavaScript 的组合。模态位于文档中触发它的父元素之上，可以通过单击模态中的关闭选项来移除。另外，点击模态背景会自动关闭模态。Bootstrap 4 一次只支持一种模态。本文着重于打开一个列表中的不同模态。下面的示例演示了从列表项打开模态。

## 第一种方法

第一种方法处理包含完全不同内容的模态。在这种情况下，模态必须单独编码。
...
第二种方法处理内容几乎相似但差异很小的模态。JavaScript 函数是为根据需求改变文本而编写的。当调用列表项的 `show` 实例方法时，`show.bs.modal` 事件会立即触发。单击的元素可作为事件的`相关目标`属性。目标元素存储在`li`变量中。接下来，`data-whatever`目标元素的值存储在接收变量中。当前模态事件存储在`modal`变量中。这个`modal`变量调用`find()`方法找到模态“title”和模态“body”，并用需要的文本替换。
