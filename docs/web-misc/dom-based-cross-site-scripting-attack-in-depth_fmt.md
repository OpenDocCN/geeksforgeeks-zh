...
> 原文：[https://www.geeksforgeeks.org/dom-based-cross-site-scripting-attack-in-depth/](https://www.geeksforgeeks.org/dom-based-cross-site-scripting-attack-in-depth/)
...
*   基于 `DOM` 的 `XSS` 的工作方式类似于反射的 `XSS` ——攻击者操纵客户端的浏览器环境（文档对象模型）并将有效负载放入页面内容。主要区别在于，由于恶意负载存储在浏览器环境中，因此可能不会在服务器端发送。这样，所有与流量分析相关的保护机制都将失败。
...
1.  大多数 `DOM XSS` 漏洞可以使用 `Burp Suite` 的工具扫描程序或 `GitHub` 上可用的一些其他脚本快速有效地找到。
...
**注意：**
应避免以下来源属性，如 `URL`、`document URI`、`location`、`href`、`search`、`hash`。