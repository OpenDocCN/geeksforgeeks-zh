...
> 原文: [https://www.geeksforgeeks.org/add-comment-section-in-nextjs/](https://www.geeksforgeeks.org/add-comment-section-in-nextjs/)
...
要添加我们的评论部分，我们将使用 Disqus 平台。Disqus 是一个网络社区平台，被网络上成千上万的网站使用。首先，我们将在 Disqus 平台上创建一个帐户，并安装 Disqus 包，然后我们将创建一个新文件，在其中为注释部分编写代码。然后在我们的主页上，我们将导入我们的评论部分。
...
接下来我们要在 Disqus 中创建新账户。为此，请遵循以下步骤:
...
现在添加我们的评论区，我们将在 `gfg` 目录中创建一个名为 `components` 的新文件夹，在这个文件夹中，我们将创建一个名为 `Comment.js` 的新 JavaScript 文件。
...
在上面的代码中，我们首先从 `disqus-react` 导入 `DiscussionEmbed`。之后，我们创建了一个名为 `Comments` 的新组件，在这个组件中，我们存储了 `disqusShortname`、`disqusConfig` 等常量变量。然后我们返回我们的 `DiscussionEmbed` 函数。
