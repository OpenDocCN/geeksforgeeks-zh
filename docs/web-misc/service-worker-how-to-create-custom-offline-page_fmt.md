...
> 原文:[https://www.geeksforgeeks.org/service-worker-how-to-create-custom-offline-page/](https://www.geeksforgeeks.org/service-worker-how-to-create-custom-offline-page/)

`服务人员`允许拦截网络请求并决定加载什么（`fetch`）。当用户失去连接时，此功能可用于加载自定义缓存的离线页面，这可以改善他们的浏览体验。

## 了解应用生命周期

1.  当用户最初加载页面时，`服务人员`被安装和激活。然后自定义离线页面被存储到浏览器缓存中。
2.  当用户触发导致重新加载或导航到另一个页面的事件，但同时他不再连接到互联网时，`服务人员`拦截网络请求并返回离线缓存页面作为响应。
...
几乎每个浏览器都附带内置的开发工具。在大多数情况下，打开工具的快捷方式是`F12`或右键单击网页并选择检查元素。然后你可以去`网络`面板把`【在线】`改成`【离线】`刷新页面。
