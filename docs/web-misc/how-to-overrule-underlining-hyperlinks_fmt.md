...
在 CSS 中，超链接默认带有下划线。这是由于 HTML/CSS 强加的默认装饰。为了去掉这条下划线，我们需要使用 CSS 对锚点标签的默认装饰做以下更改。我们可以通过使用 [CSS 文本修饰属性](https://www.geeksforgeeks.org/css-text-decoration-property/)来去除底层超链接。如果此属性设置为无，则不会显示下划线超链接。
...
**示例:**在给定的示例中，HTML 文档中添加了两个超链接，这两个超链接指向`【geeksforgeeks.org】`网站。默认情况下，超链接带下划线。要删除下划线，CSS 将以下列方式在第二个超链接中使用。实施上述更改后，下划线将被删除。
