...
创建 Django 应用程序`users`。 在 `Models.py` 文件中的`users`应用程序目录下，编写以下代码。
...
1.  **Django 管理面板**：在管理面板中，您将看到`groups`以粗体字母显示，单击该组并创建 3 个不同的组，分别命名为 Level 0、Level 1、Levvel3。 另外，根据需要定义自定义权限。
...
现在，检查特定用户是否正在访问适当的功能，例如，限制`Level 0`不能访问`Level 1`用户或`Level 2`用户的功能，依此类推。 要执行此操作，请检查所创建的每个视图函数的权限。
...
2. [http://bradmontgomery.blogspot.in/2009/04/restricting-access-by-group-in-django.html](http://bradmontgomery.blogspot.in/2009/04/restricting-access-by-group-in-django.html)
