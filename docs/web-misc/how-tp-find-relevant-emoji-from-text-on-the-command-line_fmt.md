...
> 原文: [https://www.geeksforgeeks.org/how-to-find-relevant-emoji-from-text-on-the-command-line/](https://www.geeksforgeeks.org/how-to-find-relevant-emoji-from-text-on-the-command-line/)
...
我们的第一步应该是检查我们的计算机上是否安装了最新版本的 `Node.js`。要检查是否在计算机上打开终端或命令提示符，请键入以下命令：
...
在我们的第二步中，我们将使用下面的命令在我们的机器上下载并安装 `emoj` 包。
...
现在我们已经在机器上安装了我们的包，我们将尝试一些特定关键词的例子，这些关键词可以很容易地被提取并转换成表情符号。因此，要做到这一点，我们必须在“包含关键字或仅包含关键字的实际句子”之前使用 `emoj` 关键字。例如 `emoj tiger train` 等，按回车键。
