...
> 原文: [https://www.geeksforgeeks.org/create-responsive-modal-sign-form-website/](https://www.geeksforgeeks.org/create-responsive-modal-sign-form-website/)

按照步骤使用 CSS 创建一个响应的注册表单。

### 第一步：添加 HTML
使用`form`元素处理输入。然后为每个字段添加`input`（带有匹配的`label`）。

### 第二步：添加 CSS
添加所需 CSS 设计登录页面。尽量保持设计简洁。
