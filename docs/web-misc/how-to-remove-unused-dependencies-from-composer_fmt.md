...
> 原文: [https://www.geeksforgeeks.org/how-to-remove-unused-dependencies-from-composer/](https://www.geeksforgeeks.org/how-to-remove-unused-dependencies-from-composer/)

从 Composer 中移除未使用的依赖项非常容易。以下是两种方法：

## 1. 使用 Composer Remove

这是从 Composer 中移除未使用的依赖项的最简单命令。
...
首先决定你想从作曲者那里移除哪个依赖项。这里我们将移除 `stripe/stripe-php` 的依赖关系，如下所示，在 `composer.json` 中 `stripe` 文件夹也存在于目录中。
...
**作曲删除 `stripe/stripe-php`** 命令

### 输出

该命令删除了作曲者的 `stripe/stripe-php` 依赖关系，如下所示：

![](img/6a3dfe1fa5b6a3cbbe97f5ecc627685f.png)

命令后删除了条带文件夹。

因此，`composer remove` 是一个非常简单的命令，将从作曲者中删除未使用的依赖项。

## 2. 使用 Composer Update

这是从 Composer 中移除未使用的依赖项的另一种方式。
...
现在只需删除 `phpunit/phpunit:4.6.1` 依赖项，更新后的 `composer.json` 如下图所示：
