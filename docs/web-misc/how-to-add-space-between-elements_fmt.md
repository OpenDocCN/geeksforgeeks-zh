...
> 原文：[https://www.geeksforgeeks.org/how-to-add-space-between-elements/](https://www.geeksforgeeks.org/how-to-add-space-between-elements/)
...
下面给出了使用 `border-top` 属性给出元素间距的代码。
...
在下面给出的示例中，我们使用了 `::before` 选择器属性。这有助于在 `tbody` 元素之间留出余量，因此下面给出了相关代码。我们还在表中使用了 `overflow: hidden` 属性来防止 `before` 元素溢出。
