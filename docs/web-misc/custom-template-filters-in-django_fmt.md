...
> 原文：[https://www.geeksforgeeks.org/custom-template-filters-in-django/](https://www.geeksforgeeks.org/custom-template-filters-in-django/)
...
在 `settings.py` 中添加应用名称到 `INSTALLED_APPS` 中。

![](img/87cdf455b288de2508781e2409acedb4.png)

将此视图添加到您的 `views.py` 中
...
现在让我们在主 `main` 文件夹中创建 `templatetags` 目录。

![](img/50e8285138f57523403d25f93bd4c65a.png)

别忘了创建 `__init__.py` 文件在 `templatetags` 目录中。
...
在主目录中创建一个目录，并将其命名为 `templates`。

在 `templates` 目录中创建一个文件，并将其命名为 `home.html`
