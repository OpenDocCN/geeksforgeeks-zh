...
> 原文：[https://www.geeksforgeeks.org/how-to-implement-single-row-select-and-delete-using-datatables-plugin/](https://www.geeksforgeeks.org/how-to-implement-single-row-select-and-delete-using-datatables-plugin/)

`DataTables` 是一个现代的 jQuery 插件，用于为网页的 HTML 表格添加交互式和高级控件。这是一个非常简单易用的插件，可以根据应用程序的需要为开发人员的自定义更改提供各种选项。该插件的功能包括分页、排序、搜索和多列排序。

在本文中，我们将演示在使用 `DataTables` 插件选择行后删除单行的实现。

## 方法

在以下示例中，`DataTables` 使用来自 [HTML 表](https://www.geeksforgeeks.org/html-tables/) 的学生详细信息作为主要来源。表格中的每一行都显示了一名学生的详细信息。

- `DataTables` 被初始化。
- 开发人员可以根据需要设置分页或搜索功能，如代码脚本部分所示。
- 表格按照学生成绩降序设置，只是为了显示排序用法。
- 检查用户选择的行是否有类 `selected`，然后删除。
- 如果没有 `selected` 类，则从表中的所有其他行中删除该类，并将该类添加到正在选择的行中。
