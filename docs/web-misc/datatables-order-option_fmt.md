...
`order` 选项用于指定数据表中需要排序的行及其方向。它接受一个二维数组，该数组允许同时在多列中指定排序。排序时，首先定义的列将被赋予更高的优先级。默认值为 `[[0, 'asc']]` ，表示只有第一列按升序排序。
...
**参考链接:** [https://datatables.net/reference/option/order](https://datatables.net/reference/option/order)