...
> 原文: [https://www.geeksforgeeks.org/how-to-install-a-local-module-using-npm/](https://www.geeksforgeeks.org/how-to-install-a-local-module-using-npm/)
...
**示例:** 让 `local-dir` 是本地目录而 `project-dir` 是项目目录而 `local_module` 是您要安装的本地模块包。首先转到 `local-dir` 并键入 `npm link`，然后转到项目目录并键入 `npm link <local_module>`。这将把您的本地模块链接到您的项目。

### 2. 使用 `npm install`

我们需要为 `npm install` 提供 `<folder>` 参数，该参数应该是包所在的本地文件夹。`<path>` 是包含要安装的包 (`local_module`) 的文件夹的位置。
...
**示例:** 让本地模块存储的路径为 `C:\Users\engineer\Desktop\package`。现在进入项目目录，输入命令 `npm install C:\Users\engineer\Desktop\package`。

**注意:** 只提供包文件夹名称而不提供包名称，会自动链接到你的项目。

## 如何卸载使用 `npm link` 安装的软件包？

该程序可以使用与删除已安装软件包相同的 `uninstall` 或 `rm` 命令来卸载。唯一需要注意的是，必须全局卸载连接，必须定义全局标志。
