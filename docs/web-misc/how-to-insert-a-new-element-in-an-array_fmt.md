...
> 原文:[https://www . geeksforgeks . org/如何在数组中插入新元素/](https://www.geeksforgeeks.org/how-to-insert-a-new-element-in-an-array/)
...
该函数用于在数组的开头添加一个或多个元素。这些元素被插入到数组的开头。由于数组中插入了元素或项，数组的长度也会随着数组中插入或添加的元素数量而增加。插入元素后，我们可以使用`print_r()`函数打印数组信息。
...
在本例中，我们使用`array_unshift()`函数使用索引数组在数组的开头插入元素。
