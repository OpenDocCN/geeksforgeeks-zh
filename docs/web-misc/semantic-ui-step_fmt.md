...
*   将`completed`类添加到重新完成的步骤中。
*   将`active`类添加到正在进行的步骤中。
*   将`disabled`类添加到活动步骤之后的步骤中。

### 输出:
![](img/e0d63bd212362a475d8f1abcf64672b2.png)

要以垂直顺序显示这些步骤，请将`ordered`类替换为`vertical`类。
...
### 注意:
可以添加[<一个>标签](https://www.geeksforgeeks.org/html-a-tag/)使其链接
...
### 注:
*   您可以添加`stackable`类来使组步骤可堆叠。
*   您可以为步骤添加`ui tiny steps`类，以显示它们微小的另一种尺寸，只需将`tiny`替换为`small`、`large`、`big`。