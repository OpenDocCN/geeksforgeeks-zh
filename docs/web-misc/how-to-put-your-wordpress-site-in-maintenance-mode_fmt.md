...
> 原文：[https://www.geeksforgeeks.org/how-to-put-your-wordpress-site-in-maintenance-mode/](https://www.geeksforgeeks.org/how-to-put-your-wordpress-site-in-maintenance-mode/)
...
WordPress 维护模式要求你在不影响 SEO 和用户体验的情况下进行网站的变更。WordPress 维护模式的主要好处是不会丢失搜索引擎排名。当 WordPress 网站处于维护模式时，它会通知搜索引擎不要对其进行索引。
...
这个方法是针对那些想要激活 WordPress 维护模式而不接触一行代码的人。

你所需要做的就是下载并安装 WP 维护模式插件。激活后，您需要配置插件参数。为此，请转到右侧选项卡并选择 **设置 -> 维护模式**。在设置页面上，您将看到 5 个选项卡：常规、设计、模块、机器人管理和 GDPR。查看下面所有的解释：
...
第二种方法可能有点技术性。但别担心，没那么难。请记住，当你的网站处于维护模式时，WordPress 会自动安装默认的维护页面。您可以手动将 WordPress 置于维护模式，并在没有额外插件的情况下自定义页面。
...
你可以找到 `.htaccess` 在你的网站目录中。只需将当前的 `.htaccess` 文件重命名为 `.htaccess_old`。创建一个名为 `.htaccess` 的新文件，并添加以下代码片段：
...
现在在您的网站目录中创建一个带有维护消息文件的 `maintenance.html` 文件。

## 结束语

让 WordPress 进入维护模式的方法有很多。第一种方法是使用 WordPress 维护模式插件。这个插件将帮助你创建一个漂亮的闪屏。这种方法推荐给那些不熟悉编码的人。第二种方法需要向 `functions.php` 文件中添加一段 PHP 代码。您可以尝试的最后一种方法是将代码添加到 `.htaccess` 文件。
