...
> 原文: [https://www.geeksforgeeks.org/how-to-target-desktop-tablet-and-mobile-using-media-query/](https://www.geeksforgeeks.org/how-to-target-desktop-tablet-and-mobile-using-media-query/)
...
不同设备的屏幕分辨率如下:
*   手机(智能手机) `max-width`: 480 像素
*   低分辨率平板电脑和 ipads `max-width`: 767 像素
*   平板电脑 Ipads 纵向模式 `max-width`: 1024px
*   桌面 `max-width`: 1280 像素
*   大尺寸(更大的屏幕) `max-width`: 1281 像素或更大
...
当某个条件为真时，它使用 `@media` 规则来包含一个 CSS 属性块。某些媒体功能包括宽度(`max-width` 和 `min-width`)、纵横比、分辨率、方向等。
