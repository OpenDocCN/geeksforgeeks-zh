...
> 原文: [https://www.geeksforgeeks.org/how-to-use-margin-border-and-padding-to-fit-together-in-the-box-model/](https://www.geeksforgeeks.org/how-to-use-margin-border-and-padding-to-fit-together-in-the-box-model/)
...
CSS `padding` 用于在元素周围、任何定义的边框内创建空间。我们可以为各个侧面(顶部、右侧、底部、左侧)设置不同的填充。添加边框属性以实现填充属性非常重要。
...
*   `padding-top`: 指定内容上方框中的填充宽度
*   `padding-right`: 指定内容右侧框中的填充宽度
*   `padding-bottom`: 指定内容下方框中的填充宽度
*   `padding-left`: 指定内容左侧框中的填充宽度
...
*   `width`: 指定边框的厚度
*   `style`: 指定边框的外观，可以是实线、虚线、虚线、双点、凹槽、插图等。
*   `color`: 指定边框的颜色
...
*   `margin-top`: 指定框顶部的边距宽度
*   `margin-right`: 指定框右侧的边距宽度
*   `margin-bottom`: 指定框底部的边距宽度
*   `margin-left`: 指定框左侧的边距宽度
