...
GeeksforGeek 也有一个定制的错误页面。如果我们键入类似 `www.geeksforgeeks.org/ajneawnewiaiowjf` 这样的 URL，它将显示错误 404 页面，因为此 URL 不存在。但错误页面提供了漂亮的布局，帮助用户返回，甚至在特定的时间间隔后将他们带到主页。这就是为什么每个网站都需要自定义错误页面。

Flask 为我们提供了一种处理错误并返回自定义错误页面的方法。

为此，我们需要下载并导入 Flask。通过 CMD 上的以下命令下载 Flask。
...
**app.py**
Flask 允许我们创建一个 Python 文件来定义所有路由和函数。在 `app.py` 中，我们定义了到主页的路由(`/`)和错误处理函数，这是一个 Flask 函数，我们将 `404` 错误作为参数传递。
...
**404.html**
下面的代码从 `header.html` 中导出标题和导航栏。这两个文件都应根据 Flask 存放在 `Templates` 文件夹中。
...
此示例的 `app.py` 代码与上面相同。下面的代码显示了自定义 404 错误页，并开始了 5 秒的倒计时。完成 5 秒后，它会将用户重定向回主页。

**404.html**
下面的代码从 `header.html` 中导出标题和导航栏。这两个文件都应根据 Flask 存放在 `Templates` 文件夹中。5 秒后，用户将自动重定向到主页。
...
**sample header.html**
这是一个示例 `header.html`，其中包含如图所示的导航栏。它由引导程序组成。你也可以自己做一个。关于这一点，请参阅 [bootstrap 文档](https://getbootstrap.com/docs/4.0/components/navs/)。
...
**输出：**
输出将是用户导出的带有 `header.html` 的自定义错误页面。下面是我的自定义页眉、页脚和 `404.html` 文件的输出示例。
