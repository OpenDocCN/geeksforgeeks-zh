...
`autoWidth` 选项用于指定是否启用数据表中列宽的自动计算。该计算需要一点时间，当使用 `columns` 选项显式传递列宽时，该计算可能会被禁用。
...
*   `value`: 这是一个布尔值，表示是否启用列宽自动计算。默认值为 `true`。

以下示例说明了该选项的使用。

## 示例 1:

在本例中，`autoWidth` 选项设置为默认状态，这意味着列宽将自动计算。
...
在此示例中，`autoWidth` 选项设置为 `false`，列宽作为数组传递。
...
**参考:** [https://datatables.net/reference/option/autoWidth](https://datatables.net/reference/option/autoWidth)