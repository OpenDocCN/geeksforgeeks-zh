...
> 原文：[https://www.geeksforgeeks.org/how-to-export-default-constructors/](https://www.geeksforgeeks.org/how-to-export-default-constructors/)
...
*   在 `index.html` 添加 `script-src`（注意：因为我们正在导出模块，所以我们需要添加 `type="module"`）。

## 项目目录：

我们的项目目录会是这样的。

![项目目录结构](img/f7efb70e5940fe42433872ad652debda.png)

## 示例：

`index.html` 文件将包含导入模块的 `src` 到 `Profile.js`。`Profile.js` 文件将导入 `User.js` 并通过创建 `User` 类的对象来调用 `User.js` 的构造函数。`User.js` 文件将有一个构造函数，它接受参数并打印其值以及一些伪文本。
...
*   您将看到如下所示的本地服务器列表：
    ![服务的可用端口列表](img/6f9df4bc404b2d8b372bbb6fb2494427.png)

*   现在点击任何可用的本地服务器，我们将看到以下输出。
    ![输出结果](img/b07b8f92ab19e0d4d0c02a971c3b1fde.png)