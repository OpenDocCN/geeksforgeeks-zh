...
> 原文: [https://www.geeksforgeeks.org/get-and-post-method-using-fetch-api/](https://www.geeksforgeeks.org/get-and-post-method-using-fetch-api/)

`fetch()`方法用于在不刷新页面的情况下向服务器发送请求。它是`XMLHttpRequest`对象的替代。
`fetch()`请求的基本语法如下:
...
`XMLHttpRequest`和`fetch`的区别在于`fetch`使用[Promise](https://www.geeksforgeeks.org/javascript-promises/)，这在处理多个异步操作时很容易管理，回调会创建回调地狱，导致无法管理的代码。
...
POST 请求广泛用于向服务器提交表单。Fetch 还支持 POST 方法调用。要进行 POST 请求，我们需要在请求中指定附加参数，如`method`、`headers`等。
在这个例子中，我们将在同一个 JSONPlaceholder 上做一个 POST 请求，并在 POST 中添加一个帖子。然后，它将返回带有标识的相同帖子内容。
...
API 返回状态`201`，这是`Created`的 HTTP 状态代码。