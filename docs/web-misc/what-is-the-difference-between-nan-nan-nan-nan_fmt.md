...
> 原文: [https://www.geeksforgeeks.org/what-is-the-difference-between-nan-nan-nan-nan/](https://www.geeksforgeeks.org/what-is-the-difference-between-nan-nan-nan-nan/)
...
现在有两种方法可以检查所需的输入是否为 `NaN`。第一种检查方法是 `isNaN()`，另一种是 [`Number.isNaN()`](https://www.geeksforgeeks.org/number-isnan-javascript/)。第二种方法仅适用于数值数据类型。第一种方法在代码 2 中演示。字符串变量是非数字的，因此您将得到真实的输出（即它是 `NaN`）。

**区别 `NaN != NaN` 和 `NaN !== NaN`:**

*   主要区别不在 `NaN` 上，而是在运算符。`single = compare(NaN != NaN)` 只有对每个 `NaN` 不相等的值。
*   在 `NaN !== NaN` 比较值的类型，也因此区别现在很明显了。