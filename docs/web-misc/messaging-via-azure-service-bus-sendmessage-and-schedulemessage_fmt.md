...
> 原文：[https://www.geeksforgeeks.org/messaging-via-azure-service-bus-sendmessage-and-schedulemessage/](https://www.geeksforgeeks.org/messaging-via-azure-service-bus-sendmessage-and-schedulemessage/)
...
`send()` 函数调用将消息发送到当前发送者所连接的 Azure 服务总线。此方法进行非异步调用。您也可以使用异步版本来提高性能。
...
public static async sendMessage(content: Message): Promise<string> {
    ...
    const client = serviceConnection.createQueueClient("" + process.env.AZURE_SERVICEBUS_QUEUE);
    const sender = client.createSender();
    let response = "";

    try {
        const scheduledEnqueueTimeUtc = moment().utc().add(1, "m").toDate();
        await sender.send( {body: JSON.stringify(content), label: "MyTopic"});
        await client.close();
    } catch (error) {
    } finally {
...
此方法向调用发送方所连接的 Azure 服务总线发送一个基于计时器的消息。它将消息排入总线，以便在预定时间将消息传递给接收方。目前这是一个异步过程，以获得更好的性能。
...
```html
public static async sendScheduleMessage(content: Message): Promise<string> {
    const serviceConnection = AzureServiceBus.createConnection();
    const client = serviceConnection.createQueueClient("" + process.env.AZURE_SERVICEBUS_QUEUE);
    const sender = client.createSender();
    let response = "";
    ...
    try {
        ...
        const sequenceId = await sender.scheduleMessage(
            ...
            {body: JSON.stringify(content), label: "MyTopic"});
...
const sequenceId = await sender.scheduleMessage(
    scheduledEnqueueTimeUtc,
    {body: dt.encode(JSON.stringify(content)), label: "MyTopic"});
response = sequenceId.toString();
```
...
该方法使用 `scheduleMessage` 调用提前删除了放置在服务总线中的消息。我们需要将调用期间返回的 `sequenceNumber` 作为该方法调用的唯一参数发送。如果消息已经被传递，那么我们会收到一个需要在 `catch` 中处理的错误消息 `NotFound`。
