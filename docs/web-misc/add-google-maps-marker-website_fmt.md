...
> 原文: [https://www.geeksforgeeks.org/add-google-maps-marker-website/](https://www.geeksforgeeks.org/add-google-maps-marker-website/)
...
*   在代码中，`<script>` 标签从指定的网址加载 API。
*   `callback` 参数在 API 加载后执行 `initMap` 函数。
*   `async` 属性允许浏览器在 API 加载时继续呈现页面的其余部分。
*   `key` 参数包含 API 密钥。
*   在 `key=` 中输入你的 API 密钥。
![](img/2d9f452419087bbdc6133ed63f95cf97.png)

该代码包含 `initMap` 函数，该函数在网页加载时初始化并添加地图。`<script>` 标签可以用来添加你自己的 JavaScript。
![](img/bdf5d22fc9526f9bd6b69304d5484a9b.png)

*   该代码构建了一个新的 `google.maps.Map` 对象，并向地图添加了属性，包括中心和缩放级别。
*   在下面的代码中，`new google.maps.Map()` 创建了一个新的 Google maps 对象。
*   `center` 属性告诉 API 地图的中心位置。地图坐标按以下顺序设置: 纬度、经度。
*   `zoom` 属性指定地图的缩放级别。`zoom: 0` 是最低的缩放级别，显示整个地球。将 `zoom` 值设置得更高可以以更高的分辨率放大地球。
...
1.  转到下面提到的链接: [https://console.developers.google.com/flows/enableapi?apiid=maps_backend,geocoding_backend,directions_backend,distance_matrix_backend,elevation_backend,places_backend&reusekey=true](https://console.developers.google.com/flows/enableapi?apiid=maps_backend,geocoding_backend,directions_backend,distance_matrix_backend,elevation_backend,places_backend&reusekey=true)
2.  创建新项目或从现有项目中进行选择。
3.  单击“继续”启用 API。
4.  在“凭据”页面上，获取一个 API 密钥(并设置 API 密钥限制)。
5.  将 URL 中 `key` 参数的值替换为你自己的 API 密钥。
...
<html>
  ...
  <body>
    ...
    <script async defer
    src="https://maps.googleapis.com/maps/api/js?key=AIzaSyB2bXKNDezDf6YNVc-SauobynNHPo4RJb8&callback=initMap">
    </script>
  </body>
</html>
```
