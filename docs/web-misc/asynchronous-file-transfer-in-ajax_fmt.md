...
> 原文: [https://www.geeksforgeeks.org/asynchronous-file-transfer-in-ajax/](https://www.geeksforgeeks.org/asynchronous-file-transfer-in-ajax/)

要使用 Ajax 在 jQuery 中传输文件，我们遵循以下步骤和一个示例:

## 步骤说明

1.  使用 HTML5，我们可以使用表单上传文件。内容类型被指定为`multipart/form-data`。
2.  为要上传的文件创建一个输入字段，输入类型为`file`。
