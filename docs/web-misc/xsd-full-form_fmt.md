...
`XSD`s 也可以用来生成 `XML` 文档。它还检查文档的词汇。它不需要解析器进行处理。`XSD` 检查 `XML` 文件结构的正确性。`XSD` 于 2001 年首次出版，之后于 2004 年出版。`xsd` 是用 `XML` 编写的，因此不同于早期的模式，如 `DTD`、`SOX` 等。中间处理不需要解析器。
