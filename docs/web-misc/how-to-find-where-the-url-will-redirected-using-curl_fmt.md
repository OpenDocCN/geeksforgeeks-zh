...
> 原文：[https://www.geeksforgeeks.org/how-to-find-where-the-url-will-redirected-using-curl/](https://www.geeksforgeeks.org/how-to-find-where-the-url-will-redirected-using-curl/)

一般来说，cURL 代表“客户端 URL”，这里用大写字母书写的 URL 表示 cURL 处理网址。
...
**例 1：**
```bash
curl -Ls -w %{url_effective} -o /dev/null https://www.geeksforgeeks.org/php-curl/
```
...
```bash
curl -s -L -D - https://www.geeksforgeeks.org/php-curl/ -o /dev/null -w '%{url_effective}'
```

**输出：**
```bash
D:\mycurl\bin>curl -s -L -D - https://www.geeksforgeeks.org/php-curl/ -o /dev/null -w '%{url_effective}'
HTTP/2 404
server: Apache
