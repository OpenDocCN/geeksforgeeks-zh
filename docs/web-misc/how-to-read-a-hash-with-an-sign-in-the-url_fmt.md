...
> 原文：[https://www.geeksforgeeks.org/how-to-read-a-hash-with-an-sign-in-the-url/](https://www.geeksforgeeks.org/how-to-read-a-hash-with-an-sign-in-the-url/)
...
这里`url`是任意一个示例 URL 字符串。获取网址查询字符串参数。如果 URL 在查询字符串中有许多参数，下面的代码片段将指导您解析并存储在可访问的变量中。
...
要使用任何参数，用户可以通过参数名称访问该值。例如，如果网址包含查询字符串`?var1=姓名&var2=姓氏&var3=地址`，用户可以使用以下方式访问`"var1"`的值：
...
以下示例说明了上述方法。该示例读取 URL 参数，该参数包括给定 URL 中的哈希以及“&”符号。
...
<body>
    ...
    <script>
        var url = 'http://www.geeksforgeeks.com/search?var1=term&var2=english#results';
        $('#btn').click(function() {
            readHash(url);
        });
