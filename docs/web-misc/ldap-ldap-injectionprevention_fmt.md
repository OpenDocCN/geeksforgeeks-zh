...
> 原文: [https://www.geeksforgeeks.org/ldap-ldap-injectionprevention/](https://www.geeksforgeeks.org/ldap-ldap-injectionprevention/)
...
*   **Input Validation:** 所有用户端输入都必须经过净化。它应该不含可疑的字符和可能具有恶意性的字符串。存在 OWASP API 可以帮助防御这些漏洞，例如：
    ...
    *   C# AntiXSS 功能包括 `Encoder.LdapDistinguishedNameEncode(string)` 和 `Encoder.LdapDistinguishedNameEncode(string, bool, bool)`。
