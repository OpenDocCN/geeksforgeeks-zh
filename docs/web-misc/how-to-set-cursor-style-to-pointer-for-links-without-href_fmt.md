...
创建一个 CSS 类来改变光标属性。`cursor`属性用于指定当鼠标指向元素时要显示的鼠标光标。在此属性中使用`pointer`值会将光标更改为指示链接的“指针”。
...
`onmouseover`事件在鼠标指针移动到元素或其子元素上时触发。访问元素的`style`属性并更改`cursor`属性。
`cursor`属性用于指定鼠标光标指向元素时显示的光标。在此属性中使用`pointer`值会将光标更改为指示链接的指针。
