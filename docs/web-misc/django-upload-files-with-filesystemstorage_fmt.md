...
> 原文: [https://www.geeksforgeeks.org/django-upload-files-with-filesystemstorage/](https://www.geeksforgeeks.org/django-upload-files-with-filesystemstorage/)
...
这里，注意输入（通过它用户可以输入文件）具有名称 `document`。

### Step 2: 在 `views.py` 文件中编写视图

**视图**

首先，在文件顶部导入 `FileSystemStorage` 类。
...
函数 `FileSystemStorage.save` 接受 3 个参数：`name`、`content`（文件本身）和 `max_length`（默认值=`None`）。该函数将文件——`content` 存储在名称 `name` 下。如果存在同名文件，它会稍微修改文件名以生成唯一的名称。

在同一的 [Django 文档中阅读更多关于 `FileSystemStorage` 类的构造函数参数和方法](https://docs.djangoproject.com/en/2.2/ref/files/storage/#django.core.files.storage.FileSystemStorage)。
...
这将添加一个到媒体网址的路由，并在用户向 `media_url/(文件名)` 发出获取请求时从媒体根目录提供文件。完成所有这些后，上传的文件应该显示在 `MEDIA_ROOT` 常量中指定的目录中。
