...
> 原文：[https://www.geeksforgeeks.org/how-to-save-an-image-to-localstorage-and-display-it-on-the-next-page/](https://www.geeksforgeeks.org/how-to-save-an-image-to-localstorage-and-display-it-on-the-next-page/)
...
1.  `Setting item (key, value)`：用于保存数据到本地存储。
2.  `Remove item`：用于从 localStorage 删除数据。
3.  `getItem(key)`：它从 localStorage 读取数据。
4.  `clear()`：清除 localStorage(在域上)。
...
我们将实现一个名为 Pics Villa 的小图片发帖平台，有两个网页：
