...
Material-UI 是一个用户界面库，提供预定义和可定制的 React 组件，以实现更快、更简单的网络开发，这些 Material-UI 组件基于谷歌的 Material Design 之上。在这篇文章中，让我们来讨论一下材料用户界面库中的排版组件。
...
您可以使用以下代码从`@material-ui/core`导入`Typography`组件。
...
*   **对齐:** 用于对齐组件上的文字。示例: `inherit`、`left`、`center`、`right` 或 `justify`。
*   **颜色:** 用于设置组件的文字颜色。例如: `initial`、`inherit`、`primary`、`secondary`、`textPrimary`、`textSecondary` 或 `error`。
*   **变体:** 用于设置主题排版样式。示例: `h1`、`h2`、`h3`、`h4`、`h5`、`h6`、`subtitle1`、`subtitle2`、`body1`、`body2`、`caption`、`button`、`overline`、`srOnly` 或 `inherit`。
*   **类:** 覆盖样式是自定义 CSS 对象。
*   **段落:** 如果这是真的，文本将有一个底部边距。例子: `true` 或 `false`。

## 示例

使用 `App.js` 文件中的`<Typography />`实现 H1、H2、H3 造型。
