...
> 原文：[https://www.geeksforgeeks.org/how-to-skip-over-an-element-in-map/](https://www.geeksforgeeks.org/how-to-skip-over-an-element-in-map/)
...
<body>
    ...
    <script>
        ...
        var values = [1, -1, -2, 6, 7, 8]
        var filtered = values.map(display).filter(isPositive);
        function myFunction() {
            x = document.getElementById("demo")
            x.innerHTML = filtered;
        }
...
<body>
    ...
    <p>Given<br>images = [{src: 1}, {src: 2}, {src: 3}, {src: 4}]<br>Skip src=3</p>
    <button onclick="myFunction()">Skip</button>
    <p id="demo"><br></p>
    <script>
        let images = [{ src: 1 }, { src: 2 }, { src: 3 }, { src: 4 }];
        let sources = images.filter(img => img.src != 3).map(img => img.src);
        function myFunction() {
            x = document.getElementById("demo")
            x.innerHTML = sources;
        }
