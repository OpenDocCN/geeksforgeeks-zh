...
> 原文：[https://www.geeksforgeeks.org/difference-between-class-method-and-class-prototype-method/](https://www.geeksforgeeks.org/difference-between-class-method-and-class-prototype-method/)
...
`类方法`是静态的，与类的任何实例都没有关系。必须使用类名调用类方法。内存中只存在该函数的一个实例。
...
创建与对象实例相关的`类.原型.方法`。它是使用对象实例名调用的。类的每个实例都有自己的这个方法的副本。
...
<script>
    ...
    User.prototype.greet = function () {
        // can access object properties using 'this' keyword
    ...
    // Instance method being accessed using instance variable
    newUser.greet();
</script>
```
