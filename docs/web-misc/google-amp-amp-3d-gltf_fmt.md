...
| 序号 | 属性名 | 描述 |
| --- | --- | --- |
| 1. | `src` | 这是一个强制属性，因为它指定了要显示的 gltf 文件的位置。 |
| 2. | `alpha` | 这是一个可选属性，用于用布尔值指定画布背景的行为。 |
| 3. | `antialiasing` | 它是一个可选属性，用于启用抗锯齿。默认值为假。 |
| 4. | `clearcolor` | 它是一个可选属性，用于用 CSS 颜色填充画布上的空闲空间。 |
| 5. | `maxpixelratio` | 它是一个可选属性，用于设置像素比率渲染选项的上限。 |
| 6. | `autorotate` | 它是一个可选属性，用于启用围绕模型中心的自动旋转。 |
| 7. | `enablezoom` | 它是一个可选属性，用于禁用或启用放大和缩小模型。 |
