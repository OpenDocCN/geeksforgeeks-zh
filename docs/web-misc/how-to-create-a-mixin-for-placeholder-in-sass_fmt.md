...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-mixin-for-placeholder-in-sass/](https://www.geeksforgeeks.org/how-to-create-a-mixin-for-placeholder-in-sass/)
...
4.  在我们的处理过程中，`.scss` 文件，`@include` 替换为 mixin 代码，`@content` 替换为传递过来的 css 代码块，`&` 替换为父选择器，每次都是“输入”通过代码替换得到想要的 CSS 文件。

## SASS 文件

下面的代码片段演示了扩展名为 `.scss` 的 SASS 文件。
...
下面的代码是上面的 SASS 文件的结果，执行时转换为 `style.css` 文件。
