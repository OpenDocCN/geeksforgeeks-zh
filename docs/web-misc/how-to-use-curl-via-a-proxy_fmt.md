...
## 使用带有 PHP 的 cURL 函数的代理

要通过 cURL 向代理进行身份验证并发送 HTTP GET 请求，请按照下面给出的代码进行操作，并阅读注释中指定的说明。
...
在上面的代码中，我们连接到一个代理，该代理在发送一个简单的 GET 请求之前需要进行身份验证。如果代理不需要身份验证，那么您可以从代码中省略 `CURLOPT_PROXYUSERPWD` 行。
...
*   `“连接后收到来自代理的 HTTP 代码 407”` 您简单使用的 `CURLOPT_PROXYUSERPWD` 的用户名和密码组合是错误的。请确保用户名和密码是正确的，并使用冒号:字符分隔用户名和密码。