...
`orderCellsTop`选项用于指定订单事件处理程序将应用于表格中的哪个单元格。这可以在使用多个复杂标题的情况下使用。将选项设置为 `true` 会将订单事件处理程序附加到表格的顶部唯一单元格，而将选项设置为 `false` 会将其附加到表格的底部单元格，这是默认行为。
...
**参考:** [https://datatables.net/reference/option/orderCellsTop](https://datatables.net/reference/option/orderCellsTop)