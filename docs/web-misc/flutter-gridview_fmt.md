...
`GridView` 是一个类似于任何编程语言中的二维数组的小部件。顾名思义，当我们必须在网格上显示一些东西时，就要用到 `GridView` 小部件。我们可以在 `GridView` 上显示图像、文本、图标等。我们可以在 Flutter 中以各种方式实现 `GridView`：
...
*   **锚点:** 该属性取一个 `double` 值作为对象来控制零滚动效果。
*   **childrenDelegate:** `SliverChildDelegate` 是此属性的对象。它提供了一个代表，为 `GridView` 的孩子们服务。
*   **clipBehavior:** 该属性将 `Clip` 枚举作为对象来决定是否剪辑 `GridView` 中的内容。
*   **controller:** 该属性将 `ScrollController` 类作为对象来控制滚动视图的位置。
*   **dragStartBehavior:** 该属性以 `DragStartBehavior` 枚举为对象。它控制拖动行为的工作方式。
*   **gridDelegate:** `SliverGridDelegate` 类是此属性的对象。它负责在 `GridView` 中处理子部件布局的代表。
...
*   **crossAxisSpacing:** 定义沿交叉轴列出的每个子对象之间的像素数。
*   **mainAxisSpacing:** 定义沿主轴列出的每个子级之间的像素数。
*   **padding:** 它定义了围绕整个小部件列表的空间量。
*   **primary:** 如果为真，则是框架隐式获取的 `ScrollController`。
*   **scrollDirection:** 定义 `GridView` 上的项目移动的方向，默认为垂直。
*   **reverse:** 如果设置为 true，则只是沿主轴反向反转小部件列表。
*   **physics:** 它决定了当用户在滚动时到达小部件的末尾或开始时，小部件列表的行为。
