...
*   `<expression> + <expression>` 运算符将两个值或表达式相加，或者我们可以说将第一个值加到第二个值上。
*   `<expression> - <expression>` 运算符从第二个值中减去第一个值。这一点需要记住。
*   `<expression> * <expression>` 运算符将两个值相乘。
*   `<expression> / <expression>` 运算符将第一个值除以第二个值。
*   `<expression> % <expression>` 运算符返回第一个值除以第二个值的余数。它也被称为模运算符。
...
Sass 有很强的能力根据日常生活(现实生活)中的使用来操纵这些单元。这意味着当两个数相乘时，它们的单位与值相乘，在除法的情况下也会发生同样的情况。通过这些例子，你会了解更多:
...
你不需要提供显示速度。在每个组件中，你创建了一个 Sass 变量，它为你想要的每个组件提供了 CSS。
