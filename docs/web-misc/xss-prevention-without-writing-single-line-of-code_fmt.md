...
> 原文: [https://www.geeksforgeeks.org/xss-prevention-without-writing-single-line-of-code/](https://www.geeksforgeeks.org/xss-prevention-without-writing-single-line-of-code/)
...
`X-XSS-Protection` 头旨在防止 XSS 攻击，过滤器通常出现在各种现代浏览器中，但您需要强制使用它。它得到了 Internet Explorer 8+、Chrome 和 Firefox 等的支持。
...
这是它的外观检查，你可以使用 `Ctrl + Shift + I` --> go to network tab --> reload the page --> click on any HTML link。
...
- `0`: 表示禁用
- `1`: 表示启用
- `mode=block` 表示过滤开启，浏览器不会对页面进行消毒，而是会进行攻击。
- `report=<URI>` 如果检测到 XSS，它将发送检测到攻击的网址。

### 2. 内容安全策略

要启用 CSP，您需要配置您的 web 服务器以返回 `Content-Security-Policy` HTTP 头（有时您会看到提到 `X-Content-Security-Policy` 头，但这是一个旧版本，您不再需要指定它）。
