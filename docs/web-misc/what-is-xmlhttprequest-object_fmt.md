...
`XMLHTTPRequest` 对象是一个用于从服务器获取数据的应用编程接口。`XMLHTTPRequest` 基本上用于 `Ajax` 编程。它检索任何类型的数据，如 `json`、`xml`、文本等。它在后台请求数据并更新页面，而无需在客户端重新加载页面。`XMLHTTPRequest` 的一个对象用于客户端和服务器之间的异步通信。`$.ajax()` 方法用于创建 `XMLHTTPRequest` 对象。
...
下面我们将看到如何用 `$.ajax()` 方法创建 `XMLHTTPRequest` 对象。
...
这些是我们可以向服务器请求的数据类型。可用的数据类型有 `text`、`xml`、`html`、`script`、`jsonp` 和 `json`。基于这些数据类型，我们在将响应处理到 `XMLHTTPRequest` 对象的处理程序之前，先指定响应的预处理程序。以下是指定数据类型的预处理程序:
*   **文本:** 如果数据类型为 `text`，则不会对响应应用预处理程序。可以通过 `XMLHTTPRquest` 对象 `responseText` 属性来访问。
...
*   **字符串:** 如果是 `script`，首先运行脚本，然后以字符串的形式处理给处理程序。
*   **jsonp:** 在请求 `json` 的情况下，我们要指定 `$.ajax()` 的 `jsonpCallback` 属性。这将在将 `json` 对象传递给后续处理程序之前执行。
*   **json:** 在 `json` 的情况下，在将对象传递给处理程序之前，将响应解析为 `jQuery.parseJSON`。

## 属性

`XMLHTTPRequest` 对象有许多有用的类属性，这有助于灵活处理响应。`XMLHTTPRequest` 对象属性包括:
*   **readyState:** 该属性指示连接的状态。
*   **状态:** 包含来自服务器的 `http` 响应代码。
*   **状态文本:** 它包含来自服务器的 `http` 响应字符串。
*   **响应文本:** 它包含来自服务器的文本格式的响应。
...
正如我们所知，`XMLHTTPRequest` 进行异步通信，结果它返回承诺。我们有许多 `jQuery` `XMLHTTPRequest` 对象的承诺方法。可用的承诺方法有:
