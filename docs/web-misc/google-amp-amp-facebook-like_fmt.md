...
## 简介
一个真正流行的特色网页就像社交媒体平台上的按钮。类似 T2 和脸书的 T3 被用来在 T4 的网页上嵌入类似脸书的按钮。在这里，当一个类似脸书的按钮被嵌入时，你可以不去脸书就喜欢这个帖子。

## 必需脚本
将以下包含源代码的脚本添加到中，导入 `amp-facebook-like` 组件。

## 属性
1.  `data-href`: 要喜欢的页面的 URL。
2.  `data-action`: 要在按钮上显示的字符串。默认值为“喜欢”。
3.  `data-color-scheme`: 应该在按钮外部使用的配色方案。默认情况下，它是亮的，也可以保持暗。
4.  `data-size`: 用于改变按钮的大小。默认情况下，它很小，也可以设置为大。
5.  `data-layout`: 用于改变按钮的布局。有四种布局可供选择，它们是:
    *   标准(默认)
    *   按钮
    *   按钮计数
    *   箱数
