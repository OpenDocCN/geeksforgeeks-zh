...
> 原文: [https://www.geeksforgeeks.org/textdecoder-web-api-textdecoder-constructor/](https://www.geeksforgeeks.org/textdecoder-web-api-textdecoder-constructor/)

在 HTML 中有一个`TextDecoder`接口，我们可以为参数中指定的编码创建一个文本解码器对象。
...
该构造函数接受两个参数，上面提到了，下面描述:

*   `utf-Label`: 字符串格式的编码器标签。默认为“utf-8”。
*   `option`: `TextDecoderOptions`字典，它有一个`fatal`的属性(这是一个布尔标志，指示`TextDecoder.decode()`方法是否应该抛出一个异常。它的默认值是`false`。)

## 示例 1:

本示例创建一个以“iso-8859-2”为参数的文本解码器对象。
...
创建一个没有参数的`TextDecoder`对象。所以将采用默认参数“utf-8”。
