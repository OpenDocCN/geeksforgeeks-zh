...
> 原文: [https://www.geeksforgeeks.org/setting-up-google-cloud-sql-with-flask/](https://www.geeksforgeeks.org/setting-up-google-cloud-sql-with-flask/)
...
现在，您需要从市场上启用 `Cloud SQL Admin API`。你可以点击这个 [链接](https://console.cloud.google.com/marketplace/details/google/sqladmin.googleapis.com?q=Cloud%20SQL%20Admin%20API) 来完成。

## 创建 Cloud SQL 实例

现在，让我们跳转到我们的 GCP 仪表盘，然后搜索 `Cloud SQL`。您的 Cloud SQL 页面应该如下所示。
...
现在，您首先需要创建一个数据库。简单来说，点击左侧导航栏上的 `数据库` 选项，然后点击 `创建数据库`，然后提供数据库的名称。对于本教程，我们的数据库名称将是 `test`。完成后，您应该会看到您的数据库列在那里。设置部分到此为止。现在让我们看看如何从你的 flask 应用程序连接到这个数据库。
...
**注意:** 如果你在 Linux 机器上，那么你需要在安装 `mysqlclient` 之前安装 `libmysqlclient-dev`。要安装它，只需在你的终端上运行 `sudo apt-get install libmysqlclient-dev`。
...
*   `PUBLIC_IP_ADDRESS`: 您的 GCP 实例公共 IP 地址 (可在概览页面中找到)
*   `DBNAME`: 您稍后创建的数据库的名称 (对于本教程，它是 `test`)
*   `PROJECT_ID`: 您的 GCP 项目 ID
*   `INSTANCE_NAME`: 您的 Cloud SQL 实例名
...
在这里，点击公共 IP 下的 `添加网络` 按钮。在那里，你需要输入你的公共 IP 地址。如果你不知道你的公共 IP 地址，那就去这个 [链接](https://www.whatismyip.com/what-is-my-public-ip-address/)。输入你的公共 IP 地址后，你就可以走了。
现在，你的应用程序应该可以从你的系统连接到你的数据库。

## 初始化数据库表

现在，在你的项目目录中打开你的终端，在里面输入 `python`。这应该会打开 python 解释器。现在，只需在其中键入以下几行，就可以从 ORM 创建您的表。
...
仅此而已。现在，您可以测试您的应用程序了。继续使用任何 API 请求工具来检查它是否工作。你可以用著名的 Postman 来测试一下。
