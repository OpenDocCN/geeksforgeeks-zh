...
作为开发者，我们很多时候都想在网页中添加音频文件。以前，音频只能在网页上使用网页插件（如 Flash）播放。`audio`标签是一个内嵌元素，用于将声音文件嵌入到网页中。如果您想在网页上添加歌曲、采访等音频，这是一个非常有用的标签。要在放大器页面中嵌入音频，您必须使用放大器音频标签。
...
<head>
    ...
    <!-- Add the following boilerplate tag as it is. -->
...
<body>
    ...
    <amp-audio width="auto" height="50" src="GeeksForGeeks.mp3">
        <div fallback>
