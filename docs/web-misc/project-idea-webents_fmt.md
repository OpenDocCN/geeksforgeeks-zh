...
网站关注组织者和参与者。 登录页面将包含一个注册选项。 注册既可以作为组织者完成，也可以作为参与者完成。 注册人员后，可以使用以下功能集：
...
*   [`jQuery`](https://www.geeksforgeeks.org/jquery-tutorials/)：`JQuery` 是一个跨平台的 `JavaScript` 库，旨在简化 `HTML` 的客户端脚本编写。 `JQuery` 是当今使用的最流行的 `JavaScript` 库。
...
`Webents` 的目标不仅是针对某一特定学院，还包括将其扩展到其他组织。 对于资金，我们计划组织校园大使项目，通过广告等方式筹集资金。
