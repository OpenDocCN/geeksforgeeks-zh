...
> 原文：[https://www.geeksforgeeks.org/how-to-get-data-from-nasa-federal-agencies-using-the-nasa-public-apis/](https://www.geeksforgeeks.org/how-to-get-data-from-nasa-federal-agencies-using-the-nasa-public-apis/)
...
![API密钥生成示例](img/25c221a747ceb4c01b1bb8065c586fe1.png)
...
*   `date`：要检索的 `APOD` 图像的日期。为 *YYYY-MM-DD* 类型，默认值为*今日*。
*   `start_date`：请求日期范围的数据时，日期范围的开始。不能用于 `date`。它是 *YYYY-MM-DD* 类型，默认值为无。
*   `end_date`：日期范围的结束，与 `start_date` 一起使用。为 *YYYY-MM-DD* 型，默认值为*今*。
*   `count`：如果指定了这个 `count`，随机选择的图像将被返回。不能与 `date` 或 `start_date` 和 `end_date` 一起使用。为 *int* 类型，默认值为无。
*   `thumbs`：返回数据视频缩略图的 `URL`。如果 `APOD` 不是视频，此参数将被忽略。它属于 *boolean* 类型，默认值为假。
*   `api_key`：是扩展使用的 `api.nasa.gov` 密钥。为 *string* 类型，默认值为 `DEMO_KEY`。
...
![APOD应用输出示例](img/0fa5c63da3689febabb789a2d2a6a3fb.png)
