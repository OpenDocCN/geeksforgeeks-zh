...
使用 JavaScript 访问 IndexedDB。

1.  打开数据库（Open a Database） –
...
2.  在数据库中创建对象存储（Create an Object Store in the Database） –
