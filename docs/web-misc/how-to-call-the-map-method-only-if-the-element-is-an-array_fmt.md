...
> 原文：[https://www.geeksforgeeks.org/how-to-call-the-map-method-only-if-the-element-is-an-array/](https://www.geeksforgeeks.org/how-to-call-the-map-method-only-if-the-element-is-an-array/)
...
`Array filter()` 方法用于从给定的数组中创建一个新的数组，该数组只包含那些满足自变量函数设置的条件的元素。

JavaScript 中的 `Array map()` 方法通过对父数组中的每个元素调用特定的函数来创建一个数组。
...
**示例 1：** 从对象文字映射上述方法的问题在于，`filter` 和 `map` 方法都不能用于迭代对象。为了解决这个问题，应使用 `Object.values()`。`Object.values()` 将一个对象作为参数，并返回该对象中所有值的数组。现在我们可以在这个数组上应用 `Array map()` 和 `Array filter()` 方法。
...
**例 2：** 映射从一个数组，我们可以在这里直接使用 `Array map()` 和 `Array filter()` 方法。
