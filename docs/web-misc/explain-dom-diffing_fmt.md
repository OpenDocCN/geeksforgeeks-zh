...
在形式意义上，`DOM` 是文档的应用编程接口(`API`)。`DOM` 定义了文档的逻辑结构，并帮助开发人员访问和操作文档。它非常类似于文档模型。例如，考虑以下 `HTML` 列表。
...
每当 `UI` 元素的状态发生变化时，就会创建一个新的虚拟 `DOM`。然后将新的虚拟 `DOM` 和之前的虚拟 `DOM` 进行相互比较。这种比较叫做 `DOM diffing`。
...
更新 `UI` 很贵；批量更新真实的 `DOM` 提高了重新绘制 `UI` 的整体性能。