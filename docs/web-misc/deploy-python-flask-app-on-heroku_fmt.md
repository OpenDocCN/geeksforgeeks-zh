...
`Flask` 是一个用 `Python` 编写的网络应用框架。`Flask` 基于 `Werkzeug` `WSGI` 工具包和 `Jinja2` 模板引擎。两者都是 `Pocco` 项目。本文围绕如何在 `Heroku` 上部署 `flask` `app` 展开。为了演示这一点，我们首先要创建一个示例应用程序，以便更好地理解这个过程。
...
### STEP 1 : 用 `pipenv` 创建虚拟环境，安装 `flask` 和 `Gunicorn` 。

```
$ pipenv install flask gunicorn
```

### 第二步:创建一个 `Procfile` 并编写以下代码。
...
### 步骤 9 : 使用 `heroku login` 命令登录 `Heroku` 命令行界面。
