...
> 原文：[https://www.geeksforgeeks.org/how-to-get-_post-from-multiple-check-boxes/](https://www.geeksforgeeks.org/how-to-get-_post-from-multiple-check-boxes/)
...
**注意：** 复选框的`name`属性必须同名，并且必须用数组初始化。这可以通过在复选框的`name`属性末尾使用`[]`来完成。

## 示例

*   **代码：** 页面(`index.html`)包含一个表单，其中包含姓名、电子邮件、联系人和技能作为字段和方法`post`。请注意，对于技能，每个复选框输入的`name`属性中都有`skills[]`。
...
*   **代码：** 我们正在处理用户发送的数据的页面(`process.php`)。
...
*   提交表单后即 `process.php` ![After Submission](img/e03be815748b4c97f5bec56d878a220e.png)