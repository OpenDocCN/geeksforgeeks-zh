...
> 原文: [https://www.geeksforgeeks.org/how-to-center-a-div-within-another-div/](https://www.geeksforgeeks.org/how-to-center-a-div-within-another-div/)
...
正如我们所看到的，内部 `div` 容器占据了内部空间的左侧部分。要将内部 `div` 容器移动到父 `div` 的中心，我们必须使用 [`style`](https://www.geeksforgeeks.org/html-style-attribute/) 属性的 [`margin`](https://www.geeksforgeeks.org/css-margins-padding/) 属性。我们可以通过这个 `margin` 属性调整任何 HTML 元素周围的空间，只需向它提供所需的值。

现在这个属性在调整内部 `div` 中的作用来了。如果 `margin` 值设置为 `0`，即 `margin:0`，则告知浏览器 HTML 元素（此处为内部 `div`）的上下边距将为 `0`。此外，如果我们将 `margin` 的值写成 `margin:0 auto`，它会命令浏览器根据 HTML 元素的宽度自动将左右边距调整为相同的大小。
...
您也可以将第一个参数的任意值提供给 `margin` 值，以提供从顶部和底部到 HTML 元素的一些间隙。要将内部 `div` 调整到中心位置，只需在第二个参数中写入 `auto` 即可。
...
此示例显示了如何在 `margin` 属性的第一个参数为非零值的 `div` 内居中 `div`。
...
这个示例描述了我们如何并排放置 2 个 `div`，每个 `div` 内部都有一个中心对齐的 `div`。
