...
> 原文: [https://www.geeksforgeeks.org/adding-user-authentication-in-nextjs-using-auth0/](https://www.geeksforgeeks.org/adding-user-authentication-in-nextjs-using-auth0/)
...
为了在我们的项目中首先添加使用 auth0 的用户认证，我们将安装 auth0 模块。之后，我们将使用 `[...auth0].js` 名称创建一个动态文件。然后我们将在每个页面上添加用户提供者。之后，我们将在主页上添加一个登录和注销选项。
...
**步骤 7:** 在 `pages/api` 目录中创建一个名为 `auth` 的新文件夹。在这个文件夹中创建一个名为 `[...auth0].js` 的新动态路由文件并在文件内部添加以下内容。
