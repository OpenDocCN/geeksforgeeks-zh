...
> 原文：[https://www.geeksforgeeks.org/building-apps-with-apache-cordova/](https://www.geeksforgeeks.org/building-apps-with-apache-cordova/)
...
`com.example.name` 是项目 ID， `ProjectName` 是项目名称，`projectDirectory` 是现在为构建我们的 Cordova app 而创建的目录。将工作目录更改为您刚刚创建的项目。
