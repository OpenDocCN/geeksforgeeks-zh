...
全新的`.editorconfig`文件是在 VS-Code 中的编辑器配置扩展的帮助下生成的。
...
*   **键值对:**这些键值对是位于节标题下的规则(支持的属性)。例如，`indent_size = 4`将缩进大小设置为 4 个空格/制表符。你可以在他们的[维基页面](https://github.com/editorconfig/editorconfig/wiki/EditorConfig-Properties)上找到所有支持的属性。
...
这里 `root=true` 向编辑器指定它是最顶层的 EditorConfig 文件，在此部分标题下指定的任何规则 `[*]` 都将应用于所有文件，包括您的 `*.js`，`*.css` 等。您可以为特定文件设置特定的规则，例如 `[*.md]` 节头有一些规则，这些规则只适用于具有 `.md` 作为扩展的文件。