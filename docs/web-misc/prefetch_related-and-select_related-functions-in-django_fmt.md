...
*   `SELECT_RELATED()` "follows" the foreign key relationship, selecting additional related object data when executing the query.
*   `PREFETCH_RELATED()` performs a separate lookup for each relationship and performs a "join" in Python.

当要选择的对象是单个对象时，可以使用 `SELECT_RELATED`，即 `OneToOneField` 或 `ForeignKey`。 当您要获取一组对象时，可以使用 `PREFETCH_RELATED`，因此 `ManyToManyFields` 如您所述，或者反转 `ForeignKey`。 只是想澄清一下我所说的“反向外键”是什么意思。
...
这里我们可以看到 Django 使用了 `INNER JOIN`。 我想澄清的一件事是，`Optimize` 是我们应用程序的一个名称。 如果我们想获取湖北的所有城市名称，可以这样做：
