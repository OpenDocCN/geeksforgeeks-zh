...
一个 **clearfix** 是一种元素自动清除或固定其元素的方式，这样它就不需要添加额外的标记。它通常用在浮动布局中，在浮动布局中，元素被水平堆叠。如果元素比包含它的元素高，那么使用 CSS 的 [`overflow`](https://www.geeksforgeeks.org/css-overflow/) 属性，将其值设置为 `auto`，以克服&修复问题。
...
**示例 1：** 在下面的代码中，在不使用的 `overflow` 属性的情况下，问题得到了解决。
...
**示例 2：** 在本代码中，使用 [`overflow`](https://www.geeksforgeeks.org/css-overflow/) 属性修复了该问题。
