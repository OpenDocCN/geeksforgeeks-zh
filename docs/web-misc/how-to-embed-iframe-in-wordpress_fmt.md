...
> 原文：[https://www.geeksforgeeks.org/how-to-embed-iframe-in-wordpress/](https://www.geeksforgeeks.org/how-to-embed-iframe-in-wordpress/)
...
构建 `Iframe` 有几种方法，就像有许多情况下您希望使用 `Iframe` 一样。我们将通过一种最常见的方式将 `Iframe` 添加到您的 [WordPress](https://www.geeksforgeeks.org/introduction-wordpress/) 网站。

### 手动添加 iFrames 到 WordPress 页面

向页面添加一个 `Iframe` 只需要一行代码。需要打开和关闭 HTML 元素，以及网页的 URL。
...
*   **`allow`**：使用该参数，您可以指定您的 `Iframe` 是否应该具有某些默认行为或功能。
*   **`height`**：`height` 参数设置网页上 `Iframe` 窗口的像素大小。
*   **`width`**：该选项允许您选择 `Iframe` 的像素宽度。
...
保持网站平稳运行，同时还能保持在法律的正确一边，这一点至关重要。`Iframes` 不仅允许你合乎道德地分享他人的资料，还允许你展示视频和音频文件，而不必将它们存储在自己的网站上。