...
> 原文：[https://www.geeksforgeeks.org/elasticsearch-differences-between-queries-and-filters/](https://www.geeksforgeeks.org/elasticsearch-differences-between-queries-and-filters/)

`Elasticsearch` 是基于 Apache Lucene 的全文搜索和分析引擎。`Elasticsearch` 使对来自多个来源的数据执行数据聚合操作以及对存储的数据执行非结构化查询（如模糊搜索）变得更加容易。
...
在版本 2 之前，`Elasticsearch` 分别执行查询和过滤，之后它们将两者结合起来，产生更有效的搜索结果。因此，首先我们讨论这两个主题，然后，我们将它们相互比较。

## 查询

查询计算每个文档与查询的相关性，并为其分配相关性分数，该分数随后用于按相关性对匹配的文档进行排序。这种相关性的概念非常适合全文搜索，在全文搜索中很少有完全“正确”的答案。该查询还会提出如下问题：
...
大多数过滤器子句的输出是与过滤器匹配的文档的简单列表。它计算速度快，易于在内存中缓存，每个文档仅使用 1 位。这些缓存的过滤器可以为后续请求有效地重用。过滤器会询问每个文档的是或否问题，并用于包含精确值的字段：
