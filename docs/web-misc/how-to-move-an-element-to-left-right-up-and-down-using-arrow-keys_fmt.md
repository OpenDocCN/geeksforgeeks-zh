...
> 原文：[https://www.geeksforgeeks.org/how-to-move-an-element-to-left-right-up-and-down-using-arrow-keys/](https://www.geeksforgeeks.org/how-to-move-an-element-to-left-right-up-and-down-using-arrow-keys/)

任务是使用 jQuery 中的箭头键向左、向右、向上和向下移动一个元素。我们可以使用 jQuery 的 `keydown()` 方法以及 `.animate()` 方法。

每当用户按下键盘上的一个键时，`keydown()` 方法会触发 `keydown` 事件。
...
1.  `keydown()` 方法用于检查是否有任何按键被按下，并返回哪个按键被按下。
2.  检查哪个键被触发是由键码决定的。键码如下：
    *   左 = 37
    *   上 = 38
    *   右 = 39
    *   下 = 40
3.  根据按下的键，元素的移动通过 `.animate()` 方法完成。
