...
当我们对字段求和时，我们需要一个分组操作(`$group`)，后跟求和(`$sum`)，如下所示。
...
这里我们匹配 `address` 列，只有 `Chennai`。因此，与地址 `Chennai` 的值相匹配的任何文件仅被汇总。
...
我们需要使用以下语法将平均功能用于平均/最小功能，将最小/最大功能用于最大功能。
...
默认情况下，输出文档将使用 `_id` 字段。我们可以使用 `1` 在输出文档中包含一个字段，使用 `0` 隐藏。

## 结论

聚合查询非常强大，它们的功能类似于 SQL 中的 `GROUP BY` 子句(即 `$group`)和 `HAVING` 功能(即 `$match`)。MongoDB 是一个强大得多的 NoSQL document 类型的数据库，聚合查询在其中起着至关重要的作用。
