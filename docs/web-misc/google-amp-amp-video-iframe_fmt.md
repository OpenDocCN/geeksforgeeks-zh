...
## 简介
iframe 视频是**内嵌框架视频**，嵌入在 HTML 页面中。但是在 **AMP HTML** 中，我们使用了一个名为 `amp-video-iframe` 的组件将视频嵌入到页面中。使用 `amp-video-iframe`，我们可以使用 JavaScript 嵌入一个定制的视频播放器。

## 所需脚本
将 `amp-video-iframe` 组件导入标题。
...
*   `poster`: 视频加载时指向一个图片网址。这是必需的属性。
*   `implements-media-session`: 如果 iframe 应该实现媒体会话应用编程接口，那么应该指定这个属性。
*   `implements-rotate-to-fullscreen`: 当 iframe 内部的文档独立实现旋转到全屏时，该属性为 True。
*   `autoplay`: 如果设置了该属性，则视频一加载就开始播放。
