...
一旦观察者调用了主题的网络钩子，主题就可以对这个新推送的数据采取适当的行动。通常，网络钩子是通过特定网址上的发布请求来执行的。`POST` 请求允许向主题推送附加信息。此外，它还可以用来识别一组各种可能的事件，而不是为每个事件创建单独的网络钩子网址。
...
3.  处理 `POST` 数据，并向网络挂钩呼叫发起者返回响应以指示状态。该步骤可能存在，也可能不存在。
...
`disclock–只需给出我们希望我们的通知是什么的所有细节。`
