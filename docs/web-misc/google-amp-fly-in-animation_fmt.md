...
1.  `data-duration`:指定动画经过一段时间后变为静态的持续时间。
2.  `data-easing`:在整个持续时间内改变动画速度。
3.  `data-margin-start`/`data-margin-end`:指定动画应该从视口边距的多少%开始。
4.  `data-repeat`:滚动后即使满载也要重复动画。
5.  `fly-in-bottom`/`fly-in-top`:元素从顶部或底部飞入。
6.  `fly-in-left`/`fly-in-right`:元素从右侧或左侧飞入。
