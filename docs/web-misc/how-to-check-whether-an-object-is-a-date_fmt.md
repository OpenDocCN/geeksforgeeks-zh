...
> 原文: [https://www.geeksforgeeks.org/how-to-check-whether-an-object-is-a-date/](https://www.geeksforgeeks.org/how-to-check-whether-an-object-is-a-date/)

## 方法 1: 使用 `instanceof` 运算符

`instanceof` 运算符检查构造函数的 `prototype` 属性是否出现在对象的原型链中的任何位置。在这种情况下，它用于检查对象是否是日期对象的实例。真值意味着它确实与指定的对象匹配。
...
`Object.prototype.toString.call()` 方法用于以格式为 `"[object Type]"` 的字符串返回对象的内部 `Class` 属性。在创建任何对象的过程中，都会在内部分配此属性。可以通过将日期对象与字符串 `"[object Date]"` 进行比较来检查该属性。真值意味着它确实与指定的对象匹配。
日期对象中日期的有效性可以用 `!isNaN()` 函数。如果日期不是无效的，则返回 `true`。
