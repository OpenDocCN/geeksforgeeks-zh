...
> 原文: [https://www.geeksforgeeks.org/how-to-make-drawer-using-material-ui/](https://www.geeksforgeeks.org/how-to-make-drawer-using-material-ui/)
...
**1。临时抽屉:** 只有当真值传递给`open`道具时，该抽屉才会显示在网页上。抽屉以模态的形式出现在当前网页的顶部，为内容提供了一个阴暗的背景，如下例所示。

**2。持久抽屉:** 当真值传递给`open`道具时，该抽屉在网页上可见，但与临时抽屉不同，它通过调整网页内容来占据当前网页的一定空间。

**3。永久抽屉:** 在这里，用户始终可以看到抽屉。这里不需要将`open`道具传递给抽屉组件。
...
我们将设计一个主页，当点击该主页时，其左上角有一个菜单图标，显示如下所示的两个菜单项:
...
但是在我们继续之前，我们需要将以下包安装到我们的项目中。打开您的终端并键入以下命令:
...
上面所有的命令都应该在您的项目中对您的`package.json`文件进行如下更改:
...
**注意:** 这是一个临时抽屉的例子。它在左侧以模式显示列表(默认情况下)。在抽屉组件中添加一个`anchor`属性，并给出一些值来观察变化。同样，更改`variant`属性值，以检查持久抽屉和永久抽屉。
