...
工具栏不像其他材质用户界面组件那样独立工作，它与应用程序栏一起工作。工具栏组件设置子级的属性，使它们水平对齐。工具栏组件只是应用普通的 CSS flex(`display: 'flex'`)，通过 `alignItems: 'center'` 进行垂直居中，一些填充，以及由 `theme.mixins.toolbar` 提供的 `minHeight: 56px`。
...
*   `children`: 是工具栏子级，通常可以是 `IconButton`、`Button`、`Typography` 等的混合。
*   `component`: 用于根节点的组件。使用 HTML 元素的字符串或组件。例: `div`。
*   `disableGutters`: 用于启用或禁用侧边填充。例子: `true` 或 `false`。
*   `variant`: 用于选择工具栏的变体。例如: `regular`、`dense`。

**CSS 规则名称:**

*   `root`: 这将样式应用于根元素。
*   `gutters`: 如果 `disableGutters={false}`，这将对根元素应用样式。
*   `regular`: 如果 `variant="regular"`，这将对根元素应用样式。
*   `dense`: 如果 `variant="dense"`，这将对根元素应用样式。
