...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-snackbar-service/](https://www.geeksforgeeks.org/how-to-create-a-snackbar-service/)

## 任务栏

`SnackBar`是一个 Angular 指令，用于在移动设备上显示通知栏。这些类型的 UI 组件一般使用几次。所以为了避免代码重复，可以简单地创建一个服务，在不同的组件中使用 `SnackBar`。
...
导入`snackBarService`并将其注入到组件的构造函数中，在其中您要使用 `Snackbar`。这将创建一个服务实例，比如`snackBService`。现在，在`snackBService`的帮助下，在任何需要的地方调用`openSnackBar`函数。
...
通过重复这些步骤，我们可以在任何组件内部使用`snackBar`。
