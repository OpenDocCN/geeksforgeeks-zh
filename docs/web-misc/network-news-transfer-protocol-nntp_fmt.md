...
新闻组最初使用的是 `Unix 到 Unix 复制协议(UUCP)`。在这个协议中，服务器复制本地磁盘上的所有新闻，海报和读者登录这些服务器直接从本地磁盘访问文章。随着互联网使用的增加，有必要使个人计算机能够访问 UseNet。因此，NNTP 的设计思路类似于`简单邮件传输协议(SMTP)`。它是由包括布雷恩·坎特、菲尔·拉普利、桑特在内的多位投稿人制作的。奥·巴伯，埃里克·费尔。Brain Kantor 和 Phil Lapsley 于 1986 年 3 月编写了 RFC 977“网络新闻传输协议”。
