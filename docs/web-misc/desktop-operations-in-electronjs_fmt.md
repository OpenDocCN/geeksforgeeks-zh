...
> 原文: [https://www.geeksforgeeks.org/desktop-operations-in-electronjs/](https://www.geeksforgeeks.org/desktop-operations-in-electronjs/)
...
**1. 以系统默认方式打开外部网址。** 我们可以通过外部链接或邮件标识，它将根据提供的协议来解决。`shell.openExternal()` 返回 `Promise`。它接受以下参数，

*   `url`: `string` 要解析的外部网址。Windows 中最多允许 2081 个字符。将根据系统默认行为解析网址。
*   `options`: `object` (可选) 它是由以下参数组成的对象，
    *   `activate`: `boolean` 仅 **macOS** 支持。它用于将打开的应用程序带到前台。默认设置为 `true`。
...
**2. 解析给定的 `string` 文件路径，并在窗口/文件浏览器中显示该文件。** 如果可能，也选择该文件。此方法没有任何返回类型。
