...
打开终端，使用`cd`命令，更改存储 web 应用程序的目录。
...
在你的发行版中安装 Curl、OpenSSH-server 和 vim 编辑器，使用以下命令轻松设置。但在此之前，我会解释它们是什么以及为什么需要使用它们。
...
## 第 3 步: 复制 web 应用程序文件夹

现在在这一步我们将把我们的 web 应用程序文件夹复制到这个绝对路径 `/var/www`
...
现在我们将域名指向我们将托管此应用程序的服务器。即我们需要将您的服务器的 IP 地址添加到 DNS 设置中。假设你的服务器 IP 地址是 0.0.0.0，你的域名是 www.test.com，那么你的域名应该指向你的服务器 IP 地址。
...
与在远程服务器上一样，我们需要将我们的 web 应用程序文件夹添加到该服务器。为此，我们需要更改 web 应用程序文件夹的权限。(web 应用程序的文件夹名称可能会有所不同)
...
现在，我们将配置我们的 Nginx 服务器来服务我们的网站，让我们，首先，看到 Nginx 文件夹
