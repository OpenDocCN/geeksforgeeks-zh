...
那个！CSS 中重要属性用于提供比正常属性更多的权重(重要性)。在 CSS 中，该`!important`意味着“这很重要”，忽略所有后续规则，并应用`!important`规则和`!important`关键字必须放在行尾，分号之前。

*   在正常使用中，外部样式表中定义的规则被文档头中定义的样式否决，而文档头又被元素本身中的内联样式否决(假设选择器具有相同的特性)。
*   此属性用于将特定样式指定为要应用的关键样式。
*   如果用`!important`属性，则该特定样式将应用于元素，而不是任何其他同名样式。
*   如果`!important`属性添加到任何样式后，应用的样式具有更高的优先级。
