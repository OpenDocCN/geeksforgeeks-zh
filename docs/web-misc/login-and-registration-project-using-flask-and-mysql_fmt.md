...
> 原文: [https://www.geeksforgeeks.org/login-and-registration-project-using-flask-and-mysql/](https://www.geeksforgeeks.org/login-and-registration-project-using-flask-and-mysql/)
...
**先决条件:** `Python` 知识、`MySQL` 工作台和 `Flask` 框架基础。系统中应该安装 `Python` 和 `MySQL Workbench`。`Visual studio 代码` 或 `Spyder` 或任何代码编辑器来处理应用程序。
...
**步骤-1:** 安装 `MySQL` 工作台。
链接安装: [https://dev.mysql.com/downloads/workbench/](https://dev.mysql.com/downloads/workbench/)
了解详情: [https://www.mysql.com/products/workbench/](https://www.mysql.com/products/workbench/)
...
**步骤-4:** 编写以下代码。上面的 `SQL` 语句将创建我们的数据库 `geeklogin` 和表 `accounts`。
...
**步骤 2:** 现在打开你的代码编辑器，打开这个 `‘login’` 文件夹。
...
**第 4 步:** 创建文件夹 `templates`。在 `“模板”` 文件夹中创建文件 `“login.html”`、`“register.html”`、`“index.html”`。

**步骤-5:** 打开 `login.html` 文件，写出下面给出的代码。在 `“login.html”` 中，我们有两个字段，即用户名和密码。当用户输入正确的用户名和密码时，它会将您路由到索引页面，否则会显示 `“用户名/密码不正确”`。
...
**第 8 步:** 创建文件夹 `static`。在 `“静态”` 文件夹中创建文件 `style.css`，并粘贴给定的 `CSS` 代码。
...
**步骤-2:** 浏览网址`localhost:5000`。

**步骤-3:** 将显示输出网页。

### (5) 应用测试
**步骤-1:** 如果你是新用户，去报名页面填写详细信息。

**步骤-2:** 注册后，进入登录页面。输入您的用户名和密码并登录。
...
**如果登录成功，显示索引页面:**
![](img/6f278c55b725a7f7b4262ccfbd9fe845.png)
