...
> 原文：[https://www.geeksforgeeks.org/how-to-set-the-content-as-a-counter/](https://www.geeksforgeeks.org/how-to-set-the-content-as-a-counter/)

任务是将内容设置为计数器。[CSS 计数器](https://www.geeksforgeeks.org/css-counters/)是单位像`变量`一样由 CSS 维护，其值也根据 CSS 规则递增（以跟踪它们被使用的次数）。计数器允许您管理内容在文档中的位置所支持的内容外观。

## 将内容设置为计数器的方式

CSS 计数器面积单位像`变量`。变量值可以通过 CSS 规则来增加（可以跟踪它们被使用的百分比次数）。要使用 CSS 计数器，首先应该使用`counter-reset`来创建，为了使用 CSS 计数器，我们将使用后续属性：

*   `counter-reset`：创建或复位计数器
...
这里我们倾向于使用`counters()`执行在完全不同级别的嵌套计数器之间插入一个字符串：
...
这是一个设置计数器的例子，这里计数器的值用 1 来增加它的值，并将其与每个部分的开始相连接：
...
这是一个设置计数器的例子，这里计数器的值用 1 增加它的值，并将其与每个部分的开始相连接，并且它还增加了子部分的值：
