...
> 原文：`https://www.geeksforgeeks.org/introduction-to-aws-lambda/`

`AWS Lambda` 是一个新的计算系统，它提供了多种好处，比如实时数据处理和定制后端服务。
...
`AWS Lambda` 是一个亚马逊无服务器计算系统，运行代码并自动管理底层计算资源。它让一个人可以自动运行代码来响应多种类型的事件，比如来自亚马逊 `API` 网关的 `HTTP` 请求、亚马逊 `DynamoDB` 中的表更新以及状态转换。它还使人能够用定制逻辑扩展到其他 `AWS` 服务，甚至创建自己的后端服务。
...
如前所述，使用 `AWS Lambda` 用户只需为他所使用的内容付费，这考虑了请求数量和代码执行的持续时间。`Lambda` 将请求视为每次它开始执行以响应触发器（例如事件通知或调用卷）。

代码的持续时间从代码开始执行的时刻开始计算，直到它返回或终止。

如果不确定 `AWS Lambda` 是否是正确的选择，有一个免费等级选项可以尝试。该选项包括每月 `100` 万次免费请求，以及每月 `40` 万 `GB` 秒的计算时间。
