...
> 原文: [https://www.geeksforgeeks.org/how-to-match-username-and-password-in-database-in-sql/](https://www.geeksforgeeks.org/how-to-match-username-and-password-in-database-in-sql/)
...
- [HTML 基础](https://www.geeksforgeeks.org/html-tutorials/)
...
首先，我们必须从 XAMPP 控制面板启动 XAMPP 服务器。XAMPP 是捆绑软件。
...
打开自己选择的编辑器。创建一个名为 `connect.php` 的文件来建立数据库连接。连接对象返回到 `$conn` 变量。数据库名称为 `check`。

**connect.php:** `mysqli_connect()` 函数用于连接数据库。
...
创建一个名为 `register` 的数据库表。

![](img/91f74347bc8016e1a2f7f23e5ef3ff06.png)

## 步骤 4: 创建注册页面

创建另一个名为 `index.php` 的文件。
...
创建另一个文件 `login.php`。在这里，我们创建一个登录表单。当用户输入他们的 `username` 和 `password` 时，如果是正确的用户，跳转到另一个页面 `check.php`。
