# Elasticsearch 搜索引擎简介

> 原文：`https://www.geeksforgeeks.org/elasticsearch-search-engine-an-introduction/`
...
*   通过使用 `CRUD` 操作——`Create`、`Read`、`Update`、`Delete`——可以对持久存储中的数据进行有效操作。这些类似于关系数据库实现的 `CRUD`，可以通过 `RESTful APIs` 中的 `HTTP` 接口来执行。
