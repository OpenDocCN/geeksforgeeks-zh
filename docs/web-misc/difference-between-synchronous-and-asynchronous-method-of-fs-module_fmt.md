...
> 原文: [https://www.geeksforgeeks.org/difference-between-synchronous-and-asynchronous-method-of-fs-module/](https://www.geeksforgeeks.org/difference-between-synchronous-and-asynchronous-method-of-fs-module/)
...
同步函数阻止程序的执行，直到执行文件操作。这些函数也称为阻塞函数。同步方法将文件描述符作为最后一个参数。文件描述符是对打开文件的引用。它是使用 `fs` 模块的 `fs.open()` 方法打开文件后返回的文件的数字或引用 id。所有异步方法都可以通过在函数名后添加 `Sync` 来同步执行。NodeJS 中 `fs` 模块的一些同步方法有:
...
*   [`fs.statSync()`](https://www.geeksforgeeks.org/node-js-fs-statsync-method/)
*   [`fs.readdirSync()`](https://www.geeksforgeeks.org/node-js-fs-readdirsync-method/)
*   [`fs.existsSync()`](https://www.geeksforgeeks.org/node-js-fs-existssync-method/)
...
*   [`fs.appendFile()`](https://www.geeksforgeeks.org/node-js-fs-appendfile-method/)
...
| **序号** | **同步方法** | **异步方法** |
| :--- | :--- | :--- |
| 1. | 同步函数被称为阻塞函数。 | 异步函数被称为非阻塞函数。 |
| 2. | 它将阻止程序执行，直到文件操作完成。 | 不会阻止程序的执行。 |
