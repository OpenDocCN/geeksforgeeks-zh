...
> 原文: [https://www.geeksforgeeks.org/underscore-js-_-matcher-function/](https://www.geeksforgeeks.org/underscore-js-_-matcher-function/)

`下划线.js` 是一个 JavaScript 库，它提供了很多有用的函数，在很大程度上有助于编程，比如映射、过滤、调用等。即使不使用任何内置对象。

`_.matcher()`函数是 JavaScript 的 `下划线.js` 库中的一个内置函数，用于查找谓词函数，该函数可以通知您传入的对象是否包含所有在 `attrs` 参数中给出的键值属性。
...
**参考:** [https://underscorejs.org/#matcher](https://underscorejs.org/#matcher)