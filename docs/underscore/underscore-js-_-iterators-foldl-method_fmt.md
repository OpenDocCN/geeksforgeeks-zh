# 下划线.js _.iterators.foldl()方法

> 原文: [https://www.geeksforgeeks.org/underscore-js-_-iterators-foldl-method/](https://www.geeksforgeeks.org/underscore-js-_-iterators-foldl-method/)
...
要执行以下示例，您必须安装 `underscore-contrib` 库通过使用命令提示符我们必须执行以下命令。
...
在这个示例中，我们可以看到，通过使用 `_.iterators.foldl()` 方法，我们能够在调用迭代器时获得返回的单个值，因为它会将给定的值归结为一个。
