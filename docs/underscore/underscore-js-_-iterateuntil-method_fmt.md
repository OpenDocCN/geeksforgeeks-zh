...
> 原文: [https://www.geeksforgeeks.org/underscore-js-_-iterateuntil-method/](https://www.geeksforgeeks.org/underscore-js-_-iterateuntil-method/)

`_.iterateUntil()`方法采用两个函数，一个用作结果生成器，另一个用作停止检查和种子值 say n，并返回每个生成结果的数组。`_.iterateUntil()`方法的操作是这样的，结果生成器在开始时传递种子值，后续的每个结果将继续，直到结果未通过检查功能。
