...
> 原文: [https://www.geeksforgeeks.org/underscore-js-_-reject-function/](https://www.geeksforgeeks.org/underscore-js-_-reject-function/)

下划线.js 是一个 JavaScript 库，它提供了许多有用的函数，在很大程度上有助于编程，比如映射、过滤、调用等，甚至不使用任何内置对象。
`_.reject()` 函数用于给出与给定条件不匹配的答案。它用于从列表中找出那些不满足给定条件的元素，然后只对它们进行更改。当列表中的所有元素都被传递给函数/迭代，并且没有剩余的元素时，那么 `_.reject()` 循环结束。它与 `_.filter()` 函数相反，后者选择那些满足给定条件的项目。
...
该函数返回不满足条件的元素数组。`reject()` 功能。

## 将数字列表传递给 _.reject() 功能:

`_.reject()` 函数从列表中逐个取出元素，并对代码进行操作。下面是从列表中查找奇数元素的示例。
...
选择将应用条件的项目的一个属性。下面使用了“禁用”属性。因为“item2”没有禁用属性，所以它被拒绝并作为输出给出。
...
将不同的对象传递给每个 `_.reject()` 函数，然后通过使用像 `&&`、`||`、`!` 等逻辑运算符组合它们。这里，两个对象和两个数组列表都被拒绝，因为它们不满足给定的条件，所以它们作为输出给出。
