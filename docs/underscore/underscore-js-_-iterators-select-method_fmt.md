# Underscore.js 中的 `_.iterators.select()` 方法

> 原文: [https://www.geeksforgeeks.org/underscore-js-_-iterators-select-method/](https://www.geeksforgeeks.org/underscore-js-_-iterators-select-method/)
...
*   `iter`: 此参数保存数组的迭代器列表。
*   `unaryPredicateFn`: 此参数保存 `unaryPredicateFn` 功能键。
...
在这个示例中，我们可以看到，通过使用 `_.iterators.select()` 方法，每当一元谓词函数每次被调用时，我们都能够从迭代函数中获取值。
