...
> 原文: [https://www.geeksforgeeks.org/underscore-js-_-exists-method/](https://www.geeksforgeeks.org/underscore-js-_-exists-method/)

`_.exists()`方法检查给定值是否为“Existy”，对应返回一个布尔值。空值和未定义的都被认为是不存在的值。所有其他值都是“Existy”。
...
该方法返回一个布尔值（如果给定值为 Existy，则返回 `true`，否则返回 `false`）。
