...
> 原文: [https://www.geeksforgeeks.org/underscore-js-_-isset-function/](https://www.geeksforgeeks.org/underscore-js-_-isset-fucntion/)

`下划线.js` 是 JavaScript 中的一个库，使得对数组、字符串、对象的操作变得更加便捷。`_.isSet()` 函数用于检查给定对象是否是一个 JavaScript `Set`。链接下划线时，`"_"` 作为全局变量附加到浏览器。
...
*   `object`: 是数组、字符串、映射、`Set` 等任意 JavaScript 对象。

## 返回

返回布尔值。如果对象是一个 JavaScript `Set`，它将返回 `true`，否则函数将返回 `false`。
