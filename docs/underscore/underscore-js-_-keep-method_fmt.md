...
*   `array`: 用于创建 keep 数组的给定数组。
*   `function`: 包含要保留元素的条件的函数。
...
此方法在标准的 JavaScript 环境中无法直接工作，因为它需要安装 `underscore-contrib` 库。

可以使用以下命令安装 `underscore-contrib` 库：
```
