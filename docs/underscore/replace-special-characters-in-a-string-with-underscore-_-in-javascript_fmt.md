...
> 原文: [https://www.geeksforgeeks.org/replace-special-characters-in-a-string-with-underscore-_-in-javascript/](https://www.geeksforgeeks.org/replace-special-characters-in-a-string-with-underscore-_-in-javascript/)
...
本示例使用 `replace()`方法将**所有特殊字符**替换为 _(下划线)。
...
本示例将唯一的特殊字符替换为 _(下划线)。这个例子对每个字符进行检查，如果它是我们正在寻找的特殊字符，那么它将替换该字符。在本例中，唯一的字符是$(美元符号)。
