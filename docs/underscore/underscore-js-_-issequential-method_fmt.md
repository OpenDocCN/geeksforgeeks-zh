...
> 原文：[https://www.geeksforgeeks.org/underscore-js-_-issequential-method/](https://www.geeksforgeeks.org/underscore-js-_-issequential-method/)

`_.isSequential()`方法检查给定值是顺序复合类型值还是不是，例如数组、参数等。相应地返回一个布尔值。
...
该方法返回一个布尔值（如果给定值是顺序的，则返回`true`，否则返回`false`）。
