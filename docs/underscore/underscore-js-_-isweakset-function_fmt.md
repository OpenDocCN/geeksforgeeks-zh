...
> 原文: [https://www.geeksforgeeks.org/underscore-js-_-isweakset-function/](https://www.geeksforgeeks.org/underscore-js-_-isweakset-function/)

`underscore.js` 是一个 JavaScript 库，使得对数组、字符串、对象的操作变得更加容易和便捷。
`_.isWeakSet()` 函数的作用是：检查给定的对象是否是 JavaScript `WeakSet`。链接 `underscore.js` CDN 时，`"_"` 作为全局变量附加到浏览器。
...
返回布尔值。如果集合是弱集合，则返回 `true`，否则返回 `false`。
