...
*   **示例:**
    1.  在`resources/views`目录中创建一个名为`login.blade.php`的视图文件，并在该文件中写入以下代码。
...
2.  在`app/Http/Controllers`目录中创建一个名为`LoginController.php`的控制器。使用命令`php artisan make:controller LoginController`来创建此控制器。编写以下验证代码来验证表单提交的数据。
...
这里，`required`是当用户将字段留空时给用户一个错误。`min`代表最少字符，而`max`代表可输入的最大字符数。

3.  在`routes`目录下的`web.php`文件中写入以下代码。
...
当我们点击表单中的提交按钮时，我们得到如上所示的输出。它给出了空输入字段的这两行错误，因为我们已经使用**步骤 2** 中的`postValidate`功能中的`required`验证规则将这两个字段设置为必需的。
...
class LoginController extends Controller {
    ...
    public function postValidate(Request $request) {
        $request->validate([
            ...
            'password.min' => 'Password must contain at least 8 characters or more.',
...
在第一个数组中，`required`是当用户将字段留空时给用户一个错误。`min`代表最少字符，而`max`代表可输入的最大字符数。

在第二个数组中，我们指定了当验证规则被检查并被发现为真时要显示的错误消息。这意味着当`username`字段留空时，将显示`username.required`的错误信息，其他所有的都是一样的。因此，我们必须为我们指定的所有验证规则指定自定义错误消息。
...
这些是我们在`postValidate`中的**步骤 2** 中设置的`自定义错误信息`。

**参考:** [https://laravel.com/docs/6.x/validation](https://laravel.com/docs/6.x/validation)