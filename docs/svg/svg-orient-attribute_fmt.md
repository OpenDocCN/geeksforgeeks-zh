...
`orient`属性显示了当标记放置在形状上的位置时，它是如何旋转的。只有`<marker>`元素正在使用此属性。
...
`orient`属性接受上面提到的和下面描述的值

- `auto`: 它显示标记被定向，使得其正 x 轴指向相对于放置标记的位置处的路径的方向。
- `auto-start-reverse`: 如果此属性与标记启动一起使用，则标记的方向与指定`auto`时使用的方向相差 180°。
- `angle`: 指定的角度是形状的正 x 轴和标记之间的度量。
- `number`: 表示角度，单位为度。

以下示例说明了`orient`属性的使用。
