...
> 原文：`https://www.geeksforgeeks.org/svg-fegaussianblur-element/`

SVG 代表可缩放矢量图形。它可以用来制作像在 HTML 画布中的图形和动画。
`<feGaussianBlur>`元素基于输入图元中提供的标准偏差为图形添加平滑模糊。
...
*   `in`属性标识给定过滤器原语的输入。
*   `stdDeviation`：定义平滑模糊操作的标准差。默认值为 `0`。
*   `edgeMode`：表示输入图层边缘的额外像素。
