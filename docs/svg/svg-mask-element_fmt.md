...
`<mask>`元素定义了输入对象的透明度和可见性。它对输入对象应用掩码。它在屏幕上显示元素或图像的选定部分，同时隐藏其余部分。SVG代表可缩放矢量图形。它可以用来制作像在HTML画布中的图形和动画。
...
*   `width` — 对象的宽度。
*   `height` — 物体的高度。
*   `maskUnits` — 它有两个值`userSpaceOnUse`/`objectBoundingBox`。默认值为`objectBoundingBox`。`x`、`y`、`width`和`height`的坐标由`maskUnits`定义。
*   `maskContentUnits` — 它有两个值`userSpaceOnUse`/`objectBoundingBox`。默认值为`objectBoundingBox`。遮罩的子内容的坐标由遮罩定义。
