...
`pathLength`属性以用户单位定义路径的总长度。使用该属性的元素有：`<circle>`、`<ellipse>`、`<line>`、`<path>`、`<polygon>`、`<polyline>`和`<rect>`。
...
**属性值：** `pathLength`属性接受上面提到的和下面描述的值。

*   **number：** 可以是带小数的数，也可以是整数。

下面的例子说明了`pathLength`属性的使用。
