...
> 原文: [https://www.geeksforgeeks.org/svg-fecomponenttransfer-element/](https://www.geeksforgeeks.org/svg-fecomponenttransfer-element/)

*   SVG 代表可缩放矢量图形。它可以用来制作像在 HTML 画布中的图形和动画。
*   `<feComponentTransfer>` 元素分别在每个颜色通道上实现颜色操作。
*   这个元素的四个颜色通道分别是 `<feFuncR>`、`<feFuncG>`、`<feFuncB>` 和 `<feFuncA>`。
*   在执行颜色操作时，`<feComponentTransfer>` 元素应该每种类型只有一个子元素。
