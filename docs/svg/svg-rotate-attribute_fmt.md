...
`rotate`属性显示动画元素在`<animateMotion>`元素中沿指定路径移动时的旋转。
...
`rotate`属性接受上面提到的和下面描述的值:

*   **`auto`**: 该值允许动画元素沿路径移动时旋转动态变化。在这种情况下，元素会在当前运动方向上将其自身与右侧对齐。
*   **`auto-reverse`**: 该值允许动画元素沿路径移动时旋转动态变化。在这种情况下，元素会在当前运动方向上将其自身与左侧对齐。
*   **`number`**: 该值显示恒定的旋转，不会随着动画而改变。

以下示例说明了`rotate`属性的使用。
