...
`SVG FEDropShadow.dy` 属性返回对应于 `feOffset` 映射的 `dy` 元素 `dy` 成分的 `SVGAnimatedNumber` 对象。
...
这个属性返回对应于 `feOffset` 映射的 `dy` 元素 `dy` 成分的 `SVGAnimatedNumber` 对象。
...
[https://developer.mozilla.org/en-US/docs/Web/API/SVGFEOffsetElement](https://developer.mozilla.org/en-US/docs/Web/API/SVGFEOffsetElement)