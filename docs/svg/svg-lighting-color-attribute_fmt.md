...
> 原文: `https://www.geeksforgeeks.org/svg-lighting-color-attribute/`

`灯光颜色`属性表示灯光滤镜图元的光源颜色。使用该属性的元素包括`feDiffuseLighting`和`feSpecularLighting`。
...
## 属性值:
`lighting-color`属性接受上面提到的和下面描述的值

*   **颜色:** 代表元素的装饰和文字部分。

## 注意:
`lighting-color`的默认值为白色。

下面的例子说明了`lighting-color`属性的使用。
