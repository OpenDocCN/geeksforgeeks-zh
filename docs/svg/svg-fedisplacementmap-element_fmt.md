...
> 原文：[https://www.geeksforgeeks.org/svg-fedisplacementmap-element/](https://www.geeksforgeeks.org/svg-fedisplacementmap-element/)

SVG 代表可缩放矢量图形。它可以用来制作像在 HTML 画布中的图形和动画。

使用 `<feDisplacementMap>` SVG 过滤器图元来使用置换贴图在空间上置换图像内容。产生一个结果需要两个输入。要替换的图像内容在 `in2` 属性中指定。应用位移效果的内容保留在 `in` 属性中。
...
*   `scale` – 它定义了要在 `<feDisplacementMap>` 过滤器图元上使用的位移比例因子。
