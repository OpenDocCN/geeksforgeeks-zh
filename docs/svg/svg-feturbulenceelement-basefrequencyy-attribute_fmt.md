# SVG feturbulence element . baseFrequencyY 属性
...
`FETurbulenceElement.baseFrequencyY` 属性返回对应 `feturbulence` 元素 Y 分量的 `SVGAnimatedNumber` 对象。
...
## 返回值
此属性返回与 `feturbulence` 元素的 Y 分量对应的 `SVGAnimatedNumber` 对象。
