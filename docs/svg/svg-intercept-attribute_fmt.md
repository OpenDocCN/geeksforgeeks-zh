...
`intercept`属性用于指定颜色分量线性函数的截距。应该注意的是，`type`属性被设置为`linear`。使用该属性的元素包括：`<feFuncA>`、`<feFuncB>`、`<feFuncG>`和`<feFuncR>`。
...
**属性值:**
`intercept`属性接受上面提到的和下面描述的值。

*   **数字:** 这定义了一个整数或一个带有分数部分的数字。

**注意:** `intercept`属性的默认值为 0。

下面的例子说明了`intercept`属性的使用。
