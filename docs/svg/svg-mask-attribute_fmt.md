...
SVG `<mask>` 掩码属性用于将定义了该属性的元素与给定的 `<mask>` 元素绑定。它主要影响以下要素：`<a>`、`<circle>`、`<clipPath>`、`<ellipse>`、`<g>`、`<glyph>`、`<image>`、`<line>`、`<marker>`、`<mask>`、`<path>`、`<pattern>`、`<polygon>`、`<polyline>`、`<rect>`、`<svg>`、`<switch>`、`<text>` 和 `<use>`。
...
*   **全局值：** 该属性值包括 `inherit`、`initial` 和 `unset` 等值。
