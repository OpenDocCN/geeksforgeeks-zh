...
> 原文: [https://www.geeksforgeeks.org/svg-fespecularlighting-element/](https://www.geeksforgeeks.org/svg-fespecularlighting-element/)

`<feSpecularLighting>` SVG 滤镜图元使用 alpha 通道作为凹凸贴图来照亮源图形。
...
*   `specularConstant`: 控制镜面光的反射比。
*   `specularExponent`: 它控制光源的焦点。
*   `kernelUnitLength`: 根据其使用的上下文，它有两种含义。对于光照滤镜图元，它指示 x 和 y 坐标的预期距离，对于 `<feConvolveMatrix>`，它指示内核矩阵中连续的列和行之间的预期距离。
