...
> 原文：`https://www.geeksforgeeks.org/svg-textlength-attribute/`

`textLength` 属性允许您枚举文本所占空间的宽度。通过使用文本长度，您的 SVG 文本将以相同的宽度显示。使用该属性的元素包括：`<text>`、`<textPath>`、`<tref>` 和 `<tspan>`。
...
*   `length-percentage`：它枚举文本所占空间的宽度，以占据绝对长度或百分比。
*   `number`：是指当前坐标系单位的数值。

以下示例说明了 `textLength` 属性的使用。
