# SVG `feSpecularLighting` 的 `kernelUnitLengthX` 属性

> 原文：[https://www.geeksforgeeks.org/svg-fespecularlighting-kernelunitlengthx-property/](https://www.geeksforgeeks.org/svg-fespecularlighting-kernelunitlengthx-property/)

`feSpecularLighting` 元素的 `kernelUnitLengthX` 属性返回一个对应于其 `kernelUnitLengthX` 组件的 `SVGAnimatedNumber` 对象。
...
该属性返回一个对应于 `feSpecularLighting` 元素的 `kernelUnitLengthX` 组件的 `SVGAnimatedNumber` 对象。
