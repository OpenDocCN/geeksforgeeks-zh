...
> 原文: [https://www.geeksforgeeks.org/svg-fedropshadow-stddeviationy-property/](https://www.geeksforgeeks.org/svg-fedropshadow-stddeviationy-property/)

`SVG FedropShadow . stdDeviationY` 属性返回对应于 `Fe` 置换映射. `stdDeviationY` 元素的 `stddeviation` 成分的 `SVGAnimatedNumber` 对象。
...
此属性返回对应于 `fe` 置换映射. `stdDeviationY` 元素的 `stdDeviationY` 组件的 `SVGAnimatedNumber` 对象。
