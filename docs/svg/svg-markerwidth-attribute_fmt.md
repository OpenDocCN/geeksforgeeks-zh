...
`markerWidth`属性表示视口的宽度，在该视口中，`<marker>`根据保留范围比和视图框属性显示时将被调整。只有`<marker>`元素在使用这个属性。
...
`markerWidth`属性接受上面提到的和下面描述的值。

*   **长度百分比:**表示标记的相对或绝对宽度。
*   **数字:**表示标记的宽度，单位由`markerUnits`属性定义。

## 注意

`markerWidth`属性的默认值为 3。

## 示例 1

以下是说明`markerWidth`属性使用的示例。
