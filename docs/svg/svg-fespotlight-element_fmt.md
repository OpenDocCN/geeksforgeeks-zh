...
`<feSpotLight>` SVG 定义了一个产生聚光灯效果的光源。它定义了一个聚光灯，其光指向特定的点，并且只照亮有限的区域。
...
*   `pointsAtX`，`pointsAtY`，`pointsAtZ`: 光线照射的点的 X，Y，Z 位置。
*   `specularExponent`: 它告诉光在单个点上的焦点。默认值为 1。
*   `limitingConeAngle`: 表示两个直线之间的角度，即点光轴和点光锥。
