...
`keyTimes` 属性用于指定一个 0 到 1（包括 0 和 1）之间的浮点数（时间值）列表，该列表用于控制动画的速度。使用该属性的元素包括 `<animate>`、`<animatemotion>`、`<animatetransform>` 和 `<animatetransform>`。
...
**属性值:**
`keyTimes` 属性接受上面提到的和下面描述的值。

*   `[;<number>]*`: 这是一个由分号分隔的 0 到 1 之间的数字列表。

**注意:**
`keyTimes` 属性的默认值为 `none`。

下面的例子说明了 `keyTimes` 属性的使用。
