...
`markerHeight`属性表示视口的高度，当根据保留光谱比和视图框属性显示时，`<marker>`将被拟合到该视口中。只有`<marker>`元素在使用这个属性。
...
`markerHeight`属性接受上面提到的和下面描述的值。

*   **长度百分比:**表示标记的相对高度或绝对高度。
*   **数字:**表示标记的高度，单位由`markerUnits`属性定义。

## 注意

`markerHeight`属性默认值为 3。

## 示例 1

以下是说明`marker`属性使用的示例。
