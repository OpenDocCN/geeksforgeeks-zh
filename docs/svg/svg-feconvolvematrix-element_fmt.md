...
`<feConvolveMatrix>` SVG 滤镜原语会将输入图像中的像素与相邻像素进行更改，以生成结果图像。
...
*   `in` – 存储给定图元的输入。
*   `order` – 测试滤芯使用的基质大小。默认情况下是 3 X 3。
*   `kernelMatrix` – 它定义了构成内核矩阵的数字列表。
*   `divisor` – 定义核矩阵的值之和。默认设置为 `1`。
*   `bias` – 设置过滤器的范围。默认情况下，它设置为 0。
*   `targetX` – 水平移动卷积矩阵。(**范围** – 0 <= `targetX` < `order` x)。
*   `targetY` – 垂直移动卷积矩阵。(**范围** – 0 <= `targetY` < `order`)。
*   `kernelUnitLength` – 它表示内核矩阵中连续列和行之间的预期距离。预期距离以当前过滤器单位表示。默认值为 `1`。
*   `preserveAlpha` – 该值为真或假。它指示卷积是否仅适用于 alpha 和颜色通道。默认值为假。
