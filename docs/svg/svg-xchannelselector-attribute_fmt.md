...
> 原文: [https://www.geeksforgeeks.org/svg-xchannelselector-attribute/](https://www.geeksforgeeks.org/svg-xchannelselector-attribute/)

SVG 中的 `xChannelSelector` 属性用于指示从`in2`的颜色通道，该颜色通道用于沿 x 轴移动`in`中的像素。只有`<feDisplacementMap>`元素使用这个属性。
...
此示例使用“R”属性值说明了 `xChannelSelector` 属性的使用。
