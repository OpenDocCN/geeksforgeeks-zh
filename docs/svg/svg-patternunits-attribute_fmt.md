...
`patternUnits`属性指定哪个坐标系必须用于`<pattern>`元素的几何属性。只有`<pattern>`元素是使用这个属性的。
...
**属性值：**`patternUnits`属性接受上面提到的和下面描述的值：

*   **userSpaceOnUse：** 它显示当应用模式时，几何属性的所有坐标都参考用户坐标系。
*   **objectBoundingBox：** 它显示几何属性的所有坐标表示元素边界框的百分比或分数。

下面的例子说明了`patternUnits`属性的使用。
