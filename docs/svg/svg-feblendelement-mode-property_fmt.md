...
SVG `FEBlendElement.mode` 属性返回对应于 `FEBlendElement.mode` 元素的 `mode` 属性的 `SVGAnimatedEnumeration` 对象。
...
该属性返回与 `FEBlendElement.mode` 元素的 `mode` 属性对应的 `SVGAnimatedEnumeration` 对象。
