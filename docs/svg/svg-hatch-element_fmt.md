...
`<hatch>` SVG 元素用于描摹对象。它使用一个或多个预定义的路径来填充对象。它在指定方向上以固定间隔重复路径，以覆盖要绘制的区域。
...
*   `x` – 定义用户坐标系中的 x 轴坐标。
*   `y` – 定义用户坐标系中的 y 轴坐标。
*   `rotate` – 它指定动画元素沿着`<animateMotion>`元素中指定的路径移动时如何旋转。
*   `transform` – 它定义了应用于元素及其子元素的变换定义列表。
*   `href` – 它将资源链接定义为引用网址。
