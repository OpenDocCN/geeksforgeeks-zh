...
`<textPath>` SVG 元素用于沿着特定路径渲染文本。

要沿着某个路径渲染文本，请将文本放在一个 `<textPath>` 元素中，该元素具有一个引用了`<path>`元素的 `href` 属性。
...
*   `lengthAdjust`:应该对文本进行长度调整的地方。
*   `method`:沿路径渲染单个字形的方法。
*   `path`:文本要渲染的路径。
*   `side`:文本应该渲染到路径的哪一边。
*   `spacing`:字形之间的间距应该如何处理。
*   `startOffset` :文字的开头应该从路径的开头偏移多远。
*   `textLength`:文本将要渲染到的空间的宽度。
