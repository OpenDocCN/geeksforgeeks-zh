...
`repeatCount` 属性指定动画的持续时间。使用该属性的元素包括 `<animate>`、`<animateColor>`、`<animateMotion>`、`<animateTransform>` 和 `<set>`。
...
`repeatCount` 属性接受上面提到的和下面描述的值。

*   **number**: 表示迭代次数。这些值从大于 0 开始。
*   **indefinite**: 指定动画无限重复。

## 示例 1

以下示例说明了 `repeatCount` 属性的使用。
