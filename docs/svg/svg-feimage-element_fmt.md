...
SVG 代表可缩放矢量图形。它可以用来制作像在 HTML 画布中的图形和动画。`<feImage>` SVG 滤波器原语从外部源获取图像数据，并提供像素数据作为输出。
...
*   `x`：定义用户坐标系中的 x 轴坐标。
*   `y`：定义用户坐标系中的 y 轴坐标。
*   `width`：`foreignObject` 的宽度。
*   `height`：`foreignObject` 的高度。
*   `externalResourcesRequired`：表示当前单据是否需要外部资源。默认值为假。
*   `preserveAspectRatio`：它指示具有提供给定纵横比的视图框的元素必须如何适应具有不同纵横比的视口。
