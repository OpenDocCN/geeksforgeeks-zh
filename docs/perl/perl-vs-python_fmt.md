...
| **意见和文件** | 对于内联注释，我们在 Perl 中使用`#`号。例如`# Inline-Perl 中的注释` 而对于文档，我们使用`=pod`和`=cut` 例如`=pod Perl 中的文档 从这里开始，到这里结束。=cut` | Python 还使用`#`进行内联注释。例如`# Inline-Python 中的注释` 但是对于文档，我们使用`"""`，即三个引号 例如`"""Python 中的文档 从这里开始，到这里结束。"""` |
