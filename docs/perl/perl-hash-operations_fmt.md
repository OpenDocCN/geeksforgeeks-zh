...
[Perl 哈希](https://www.geeksforgeeks.org/Perl-Hashes/)

[Perl 哈希](https://www.geeksforgeeks.org/perl-hash/)
...
请注意，Perl 中的散列是**无序的**。这意味着当您迭代散列时，您可能无法按照插入值的相同顺序提取这些值。

存储在散列中的值可以是**整数、浮点、字符串、布尔、数组和散列**本身。
...
这里，键 `Traffic` 的数据类型是哈希，键 `AllVehicles` 的数据类型是数组。因此，输出分别是散列和数组的第一个元素的地址。
...
Perl 允许循环其哈希值。这意味着哈希是迭代类型，可以使用 `for` 循环和 `while` 循环迭代其键和值。在 Perl 中，哈希数据结构由 `keys()` 函数提供，类似于 Python 编程语言中的函数。这个 `keys()` 函数允许你在标量中获得一个散列的键列表，这个列表可以进一步用于迭代散列的各个键的值。
...
在上面的例子中，代码给出了数组第一个元素的地址和分别存储在关键字 `AllVehicles` 和 `Traffic` 中的散列。为了克服这一点，我们必须在数组内部循环并散列。
