...
> 原文: [https://www.geeksforgeeks.org/perl-eof-end-of-file-function/](https://www.geeksforgeeks.org/perl-eof-end-of-file-function/)
...
### 1. `eof(FileHandle)`
将 `FileHandle` 传递给 `eof()` 函数。如果文件为空则返回 1，否则返回 `undef`。
...
### 2. `eof()`
带空括号的 `eof` 指的是由作为命令行参数传递的文件形成的伪文件，并通过 `<>` 操作符访问。`eof()` 检查命令行参数中传递的所有文件的最后一个文件的结尾。
