...
Perl 中的 `given-when` 类似于 C/C++、Python 或者 PHP 的 `switch-case`。就像 `switch` 语句一样，它也用不同的情况替换多个 if 语句。
...
**break:** `break` 关键字用来 `break` 出一个 `when` 区块。在 Perl 中，不需要在每个 `when` 块后显式写 `break`。它已经被隐式定义了。
**continue:** `continue`，另一方面，移动到下一个 `when` 块如果第一个 `when` 块是正确的。
