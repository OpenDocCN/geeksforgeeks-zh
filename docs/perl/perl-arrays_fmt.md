...
在 Perl 中，数组是一种特殊类型的变量。数组用于存储值列表，列表中的每个对象被称为一个元素。元素可以是数字、字符串或任何类型的[标量](https://www.geeksforgeeks.org/perl-scalars/)数据，包括另一个变量。
...
在 Perl 编程中，每个数组变量都是使用变量名称前的`@`符号来声明的。单个数组也可以存储多种数据类型的元素。例如:
...
要访问数组元素，我们必须在数组变量名前加前缀`$`，后跟方括号内的索引。例如:
...
**注意:** 数组索引总是从零开始。要访问第一个元素，它必须给出 0 作为索引。我们也可以给一个负指数。但是给出负的索引将导致从结尾而不是从开头选择数组元素。
...
Perl 还提供了一个制作数字或字母顺序数组的快捷方式。这使得用户的任务变得容易。当数到 1000 或字母 A 到 Z 等时，使用顺序数字数组，用户可以跳过循环并键入每个元素。
...
数组的大小(数组的物理大小)可以通过在标量上下文中评估数组来找到。返回值将是数组中的元素数。一个数组可以在标量上下文中使用两种方式进行计算:
...
并且可以使用`$#array`找到数组的最大索引。所以`@array`和`scalar @array`总是用来求数组的大小。
...
我们可以通过查找数组的大小，然后运行一个从 0 到 size – 1 的 for 循环，然后访问数组的元素来实现。
...
我们可以使用 foreach 循环直接访问数组的元素，而不是运行一个范围循环再访问元素。
