...
`taint()`是`Scalar::Util`模块的一个函数，可用于检查变量是否被污染，以及使用哪个变量会触发“不安全依赖”消息。
...
Perl 认为这些变量是特殊的，因为 Perl 基本上认为，由于这些变量来自我们已经设置的正则表达式，我们已经明确检查了该正则表达式中数据的有效性。因此，`$1`和`$2`不被认为是有污点的。
