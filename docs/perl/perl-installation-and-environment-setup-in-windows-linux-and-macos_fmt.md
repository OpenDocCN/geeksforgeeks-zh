...
> 原文: [https://www.geeksforgeeks.org/perl-installation-and-environment-setup-in-windows-linux-and-macos/](https://www.geeksforgeeks.org/perl-installation-and-environment-setup-in-windows-linux-and-macos/)
...
要检查您的设备是否预装了 Perl，只需转到**命令行**（对于 **Windows** ，在运行对话框中搜索 `cmd`（`Win+R`）；对于 **Linux** 使用 `Ctrl+Alt+T` 打开终端；对于 **MacOS** 使用 `Control+Option+Shift+T`）。
...
在开始安装过程之前，您需要下载它。为此，适用于 Windows、Linux 和 MacOS 的所有 Perl 版本都可以在 [perl.org](https://www.perl.org/get.html) 上获得。下载 Perl 并按照进一步的说明安装 Perl。
