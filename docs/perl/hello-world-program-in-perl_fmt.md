...
*   **使用命令行:** 你也可以使用命令行选项来运行一个 Perl 程序。下面的步骤演示了如何在 Windows/Unix 操作系统的命令行上运行 Perl 程序：

### Windows

*   首先，打开一个文本编辑器，如记事本或 Notepad++。
    *   在文本编辑器中编写代码，并用 `.pl` 扩展名保存。
    *   确保您已经从 https://www.perl.org/get.html 下载并安装了最新版本的 Perl。
    *   打开命令行，运行命令 `perl -v` 来检查你的 perl 最新版本是否安装正确。
    *   编译代码，输入 `perl HelloWorld.pl`。如果您的代码没有错误，那么它将正确执行并显示输出。

### Unix/Linux

*   按照上面给出的步骤编写代码并保存文件，使用 `.pl` 扩展名。
*   打开你的 Unix/Linux 操作系统的终端，按照以下步骤下载并安装 Perl:
    ![](img/1aa0f862dc17ddbea9f2fde7003a9f79.png)
*   现在，运行命令 `perl -v` 来确定你的 perl 最新版本是否安装正确。
*   编译代码，输入 `perl hello.pl`。如果您的代码没有错误，那么它将正确执行并显示输出。