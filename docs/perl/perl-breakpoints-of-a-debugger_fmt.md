...
> 原文: [https://www.geeksforgeeks.org/perl-breakpoints-of-a-debugger/](https://www.geeksforgeeks.org/perl-breakpoints-of-a-debugger/)
...
这将在子例程 `value` 的第一条可执行语句中创建一个断点。

`b`命令也可用于仅在满足指定条件时暂停程序。
...
`c`命令用于指示调试器继续调试过程，直到到达断点或程序结束。

```perl
DB<15> c

main::(debugtest:12):                  $vardir =~ s/^\s+|\s+$//g;
...
当用户想跳过程序的几行，不想一次遍历程序一条语句而浪费执行时间时，`c`命令用于定义一个临时断点。使用 `c` 还可以确保不需要使用 `b` 定义断点，也不需要使用 `d` 命令进一步删除断点。
