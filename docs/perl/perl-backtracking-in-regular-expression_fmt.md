...
> 原文: [https://www.geeksforgeeks.org/perl-backtracking-in-regular-expression/](https://www.geeksforgeeks.org/perl-backtracking-in-regular-expression/)
...
*   最后，模式不仅可以指定某物在哪里，还可以指定它不在哪里。所以 `split` 运算符使用正则表达式来指定数据不在哪里。也就是说，正则表达式定义了分隔数据字段的分隔符。
    **示例：** 拆分运算符
