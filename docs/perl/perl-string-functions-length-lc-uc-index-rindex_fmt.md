# Perl 字符串函数（`length`、`lc`、`uc`、`index`、`rindex`）

> 原文：[https://www.geeksforgeeks.org/perl-string-functions-length-lc-uc-index-rindex/](https://www.geeksforgeeks.org/perl-string-functions-length-lc-uc-index-rindex/)
...
这个函数用于查找字符串中的字符数。这个函数返回字符串的长度。下面是说明这种方法的程序。
...
此方法将从字符串中的指定位置搜索子字符串，并返回子字符串在字符串中第一次出现的位置。如果位置被省略，它将从字符串的开头开始搜索。此方法将采用两个参数，即原始字符串和必须搜索的子字符串。
...
该函数与 `index()` 相同，只是它返回字符串中最后一次出现的文本。此外，可以给出第三个参数，该参数返回该位置之前或该位置处的位置。它从字符串的末尾开始搜索，而不是从开头开始搜索。下面是说明这种方法的程序。
