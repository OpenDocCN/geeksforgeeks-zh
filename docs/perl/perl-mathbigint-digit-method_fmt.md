...
`Math::BigInt` 模块的 `digit()` 方法用于从右数取给定数的第 n 位。要从左侧获取数字，我们需要将 n 指定为负数。

> **语法:** `Math::BigInt->digit($n)`
>
> **参数:**
> `$n`: 表示要提取的数字的整数值。
>
...
## 示例 2: 使用 `Math::BigInt->digit()` 方法从左侧获取第 n 个数字
...
## 示例 3: 使用 `Math::BigInt->digit()` 方法计算数字的位数总和
