...
运算符是任何编程语言的主要构件。运算符允许程序员对操作数执行不同类型的操作。在 Perl 中，运算符符号对于不同类型的操作数是不同的（比如标量和字符串）。一些运算符已经在 [Perl 运算符集–1](https://www.geeksforgeeks.org/perl-operators-set-1/) 中讨论过。本文将讨论其余的运算符：

*   [类似引号的运算符](#类似引号的运算符)
*   [串操纵操作器](#串操纵操作器)
*   [范围操作员](#范围运算符)
*   [自动递增&递减运算符](#自动递增和自动递减运算符)
*   [箭头操作器](#箭头运算符)
...
*   **串联运算符（`.`）**：Perl 字符串用一个点（`.`）符号。点（`.`）符号代替了 Perl 中的（`+`）符号。
...
在 Perl 中，range 运算符 `..` 用于创建指定元素的指定序列范围。所以这个操作符被用来创建一个指定的序列范围，其中开始和结束元素都包含在内。*例如* `7..10` 会产生一个类似 `7，8，9，10` 的序列。
...
此运算符 `->` 用于从类或对象中取消引用变量或方法。示例：`$ob->$x` 是从对象 `$ob` 访问变量 `$x` 的示例。这个操作符通常被用作散列或数组的引用，以访问散列或数组的元素。
**例：**
...
3.  **Perl Fixity：** 可以定义为运算符相对于其操作数的位置。
    ...
    *   **前缀运算符：** 这些运算符位于其操作数之前。比如，`!$g`，`–7`，这里，`!` 和 `–` 运算符位于操作数 `$a` 和 `3` 之前。
