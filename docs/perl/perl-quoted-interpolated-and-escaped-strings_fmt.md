...
> 原文: [https://www.geeksforgeeks.org/perl-quoted-interpolated-and-escaped-strings/](https://www.geeksforgeeks.org/perl-quoted-interpolated-and-escaped-strings/)
...
**双引号字符串:**
双引号字符串被内插，即变量名（标量、数组和哈希值）被其原始值和转义序列（如 `\t`、`\n` 等）替换，执行它们的工作。
也可以用 `qq` 操作符代替双引号字符串。
