...
| `\P{…}` | Unicode 字符类的补充 |
| `[:class:]` | 像 `digit`、`lower`、`ascii` 等字符类。 |
...
元字符用于匹配 Perl 正则表达式中的模式。所有的元字符必须是转义的。
...
| `a{2,6}` | 它检查“a”是否出现 2 到 6 次 |
| `a{2,}` | 它检查“a”是否出现 2 到无限次 |
| `a{2}` | 它检查“a”是否出现两次。 |
...
| `\x` | 它忽略了所有的空白。 |
| `(?#text)` | 它用于在代码中添加注释。 |
| `(?:pattern)` | 它用于匹配非捕获组的模式。 |
| `(?|pattern)` | 它用于匹配分支测试的模式。 |
| `(?=pattern)` | 它用于积极的前瞻断言。 |
| `(?!pattern)` | 它用于否定的前瞻断言。 |
| `(?<=pattern)` | 它用于断言后面的肯定的外观。 |
| `(?<!pattern)` | 它用于断言后面的否定的外观。 |
...
在正则表达式内部，这些组用 `\1` 表示，在正则表达式外部，这些组用 `$1` 表示。这些组可以通过变量赋值来获取，在列表上下文中称为捕获。分组结构 `(…)` 创建称为捕获缓冲区的捕获组。
