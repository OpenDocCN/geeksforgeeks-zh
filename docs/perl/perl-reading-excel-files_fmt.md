...
*   `Spreadsheet::ParseExcel` 读取 Excel 95-2003 文件，
*   `Spreadsheet::ParseXLSX` 读取 Excel 2007 打开 XML XLSX

## Excel 文件的创建

Excel 文件可以借助内置模块 `Excel::Writer::XLSX` 使用 Perl 创建，该模块用于创建 Excel 文件。
...
在 Perl 中读取 Excel 文件是通过使用 Perl 脚本中的 `Spreadsheet::Read` 模块来完成的。这个模块导出了许多函数，您可以在 Perl 代码脚本中导入或使用这些函数。 `ReadData()` 函数用于从 excel 文件中读取。
...
仅仅获取一行是远远不够的。为了高效编程，我们需要获取所有行。我们使用 `rows()` 函数来实现这一点。这个函数以一个工作表作为参数。它以矩阵(二维数组)的形式返回元素数组或引用数组。矩阵中的每个元素代表电子表格中的一行。
获取所有行的脚本如下:
...
{
    foreach my $j (1 .. scalar @{$rowsmulti[$i-1]}) 
    {
        say chr(64 + $i) . " $j " . 
               ($rowsmulti[$i - 1][$j - 1] // '');
    }
