...
## 先决条件
[Perl 中的递归](https://www.geeksforgeeks.org/recursion-in-perl/)

递归是指属于或使用可重复应用的规则或程序。它是通过算法的重复应用来定义函数或计算数字的过程。递归子例程是一种类型的[子例程](https://www.geeksforgeeks.org/perl-subroutines-or-functions/)，作为其执行的一部分调用自己，或者处于函数调用的潜在循环中。[Perl](https://www.geeksforgeeks.org/introduction-to-perl/) 为我们提供了迭代和递归使用子程序的灵活性。
