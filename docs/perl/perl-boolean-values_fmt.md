...
在 Perl 语言中，除零之外的任何非零数字都是真值。字符串常量，如 `"真"`、`"假"`、`" "`（以空格为字符的字符串）、`"00"`（2 个或更多个 0 字符）和 `"0\n"`（字符串中的一个零后跟一个换行符）等。还要考虑 Perl 中的真值。
...
空字符串或字符串包含单个数字 `0` 或 `undef` 值和零被认为是 Perl 中的假值。
...
对于用户必须比较两个不同变量的条件检查，如果它们不相等，则返回 `false` 否则返回 `true`。
