...
在 Perl 中，可以使用`m/`运算符构建模式。在这个操作符中，所需的模式简单地放在两个斜杠之间，绑定操作符用于搜索指定字符串中的模式。

## 使用 m// 和绑定操作符

绑定操作符大多与`m//`操作符一起使用，以便匹配出所需的模式。Regex 运算符用于将字符串与正则表达式进行匹配。语句的左侧将包含一个字符串，该字符串将与包含指定模式的右侧相匹配。否定正则表达式运算符用于检查字符串是否不等于右侧指定的正则表达式。
