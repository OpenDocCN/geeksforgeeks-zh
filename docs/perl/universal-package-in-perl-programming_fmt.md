...
> 原文: [https://www.geeksforgeeks.org/universal-package-in-perl-programming/](https://www.geeksforgeeks.org/universal-package-in-perl-programming/)

`UNIVERSAL` 是 [Perl](https://www.geeksforgeeks.org/introduction-to-perl/) 5 中的内置包。它也可以被认为是一个基类，派生类可以在其中隐式继承或重写。它提供了几种默认方法，如 `isa()`、`can()`、`VERSION()` 和 `DOES()`。通用类不能显示在包的 `@ISA` 变量中。其他方法也可以通过 Perl 或 XS 代码添加到 `UNIVERSAL` 类中。在程序中使用上述方法，不需要在代码中包含“`use UNIVERSAL`”语句。
