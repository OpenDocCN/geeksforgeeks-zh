...
1.  通过使用 Perl 对象接口，`Net::LDAP` 模块提供了一个接口，只需使用少量代码就可以对 LDAP 目录进行复杂的搜索。
2.  所有的 `Net::LDAP` 模块都是完全用 Perl 编写的，这使得它的库真正做到了跨平台兼容。
3.  正在积极开发。
