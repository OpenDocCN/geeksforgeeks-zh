...
> 原文：[https://www.geeksforgeeks.org/number-guessing-game-using-perl/](https://www.geeksforgeeks.org/number-guessing-game-using-perl/)
...
在这段代码中，最初，`rand` 函数选择一个随机数作为 `x`，函数 `rand k` 找到一个 0 到 `k` 之间的随机数，由于这个随机数是一个浮点数，所以用 `int` 将其显式转换为整数。`x` 存储整数。用户被给予特定数量的机会来猜测该数量，如果该机会超过用户猜测的，则用户将会失败。
...
在上面的程序中，用户可以修改 `rand` 函数的值来增加游戏中的数字范围，用户也可以通过增加机会变量的值来增加机会的数量。