...
> 原文: [https://www.geeksforgeeks.org/perl-scalar-context-sensitivity/](https://www.geeksforgeeks.org/perl-scalar-context-sensitivity/)
...
$x = Scalar;
```
在这里，`localtime()`以人类可读的格式显示时间，而在列表上下文中，这个函数显示时间的数字描述。
...
## 强制标量上下文
当 Perl 假定一个列表时，必须要求强制标量上下文。因此，在这种情况下，您可以使用`scalar()`函数，该函数生成标量上下文，因为该函数通知 Perl 为其参数赋予标量上下文。
...
## 在标量上下文中读取
为了将读行运算符（即`<STDIN>`）置于标量上下文中，需要将该运算符指定给标量变量。
