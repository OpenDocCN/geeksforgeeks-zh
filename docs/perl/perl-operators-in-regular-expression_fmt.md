...
> 原文: [https://www.geeksforgeeks.org/perl-operators-in-regular-expression/](https://www.geeksforgeeks.org/perl-operators-in-regular-expression/)

## 先决条件
[Perl 正则表达式](https://www.geeksforgeeks.org/perl-regular-expressions/)

## 正则表达式简介
**正则表达式**是一个字符串，它是提供文本字符串匹配的不同字符的组合。正则表达式也可以称为 regex 或 regexp。
...
### 1) 模式匹配或匹配正则表达式
匹配运算符`m//`用于将字符串或语句与正则表达式进行匹配。运算符中使用的正斜杠(`m//`)充当分隔符，该分隔符也可以像`m{}`、`m()`、`m<>`等。表达式写在运算符中使用的两个正斜杠之间。
...
### 2) 替代(搜索和替换)正则表达式
替代运算符`s///`用于搜索特定单词，然后用给定的正则表达式替换它。运算符(`s//`)中使用的正斜杠充当分隔符。
...
### 3) 全局字符音译正则表达式
翻译或音译运算符`tr///`或`y///`用于用给定的单个字符替换字符的所有出现。运算符(`tr///`和 `y///`)中使用的正斜杠充当分隔符。
...
**我们来看一些说明翻译正则表达式的例子:**
在下面的例子中，所有出现的`G`都被`g`替换成两个不同的运算符`tr///`和`y///`。
