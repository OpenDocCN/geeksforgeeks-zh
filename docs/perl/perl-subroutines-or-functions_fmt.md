...
> 原文: [https://www.geeksforgeeks.org/perl-subroutines-or-functions/](https://www.geeksforgeeks.org/perl-subroutines-or-functions/)
...
在 Perl 中，子程序可以通过将参数列表传递给它来调用，如下所示：

```perl
subroutine_name(aruguments_list);
```

上述调用子例程的方式仅适用于 Perl 5 及更高版本。在 Perl 5.0 之前，有另一种调用子例程的方法，但是不建议使用，因为它绕过了子例程原型。
...
这用于将值作为参数传递。这是使用特殊列表数组变量 `@_` 完成的。这将分配给函数 `$_[0]`，`$_[1]` 等等。
