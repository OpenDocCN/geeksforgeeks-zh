...
> 原文: [https://www.geeksforgeeks.org/perl-decision-making-if-if-else-nested-if-if-elsif-ladder-unless-unless-else-unless-elsif/](https://www.geeksforgeeks.org/perl-decision-making-if-if-else-nested-if-if-elsif-ladder-unless-unless-else-unless-elsif/)
...
在这种情况下，如果条件为假，那么语句将执行。数字 `0`、空字符串`""`、字符`"0"`、空列表`()`和 `undef` 在布尔上下文中都是假，所有其他值都是真。
...
`unless` 语句后面可以跟一个可选的 `else` 语句，该语句在布尔表达式为真时执行。
...
*   `unless` 语句可以有零到多个 `elsif`，并且所有这些都必须在 `else` 语句之前。
*   `unless` 语句可以有零个或一个 `else`，并且必须在任何 `elsif` 之后。
*   一旦 `elsif` 成功，剩下的 `elsif` 或 `else` 都不会被测试。
