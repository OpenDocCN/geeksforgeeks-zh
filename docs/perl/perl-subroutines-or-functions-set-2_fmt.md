...
> 原文: [https://www.geeksforgeeks.org/perl-subroutines-or-functions-set-2/](https://www.geeksforgeeks.org/perl-subroutines-or-functions-set-2/)
...
我们知道 `@_` 是一个函数或子程序内部的特殊数组变量，所以它用来将列表传递给子程序。Perl 有一种不同的方式来接受和解析数组和列表，这使得很难从 `@_` 中提取离散元素。为了将列表与其他标量参数一起传递，有必要将列表作为最后一个参数。
...
子程序也可以像 C、C++、Java 等其他编程语言一样返回值。如果用户不会从子程序手动返回值，那么子程序将自动返回值。在这种情况下，自动返回值将是子例程中执行的最后一次计算。返回值可以是标量、数组或散列。
...
默认情况下，Perl 程序中的所有变量都是全局变量。但是借助 `my` 关键字，你可以在一个块内创建本地或私有的变量。私有变量的作用域有限，比如在块之间(if、while、for、foreach 等)和方法等。在块或方法外部，这些变量不能使用。
...
Perl 没有为我们提供任何声明子程序参数的内置设施，这使得向函数传递任意个数的参数变得非常容易。
