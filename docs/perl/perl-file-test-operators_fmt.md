...
| `-R` | 检查文件是否可被真实 uid 读取 |
| `-W` | 检查文件是否可由真实 uid 写入 |
| `-X` | 检查文件是否可由真实 uid/gid 执行 |
| `-O` | 检查文件是否属于真实 uid |
| `-e` | 检查文件是否存在 |
| `-z` | 检查文件是否为空 |
| `-s` | 检查文件是否有非零大小(返回字节大小) |
| `-f` | 检查文件是否是纯文本文件 |
| `-d` | 检查文件是否是目录 |
| `-l` | 检查文件是否是符号链接 |
...
| `-k` | 检查文件是否设置了粘性位 |
| `-T` | 检查文件是否是 ASCII 文本文件(启发式猜测) |
| `-B` | 检查文件是否是“二进制”文件(与 `-T` 相反) |
...
# Applying multiple Test Operators on the File
