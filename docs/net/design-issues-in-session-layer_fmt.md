...
> 原文：[https://www.geeksforgeeks.org/design-issues-in-session-layer/](https://www.geeksforgeeks.org/design-issues-in-session-layer/)
...
2.  **管理令牌** –
    网络中的通信系统尝试执行一些关键操作，而会话层可以防止在执行这些操作时可能发生的冲突，否则这些冲突会导致丢失。
3.  **同步** –
    检查点是在数据流传输过程中，经过特定间隔后添加的中途标记。这些点也被称为同步点。会话层允许进程添加这些检查点。
