...
> 原文: [https://www.geeksforgeeks.org/how-to-setup-firewall-in-linux/](https://www.geeksforgeeks.org/how-to-setup-firewall-in-linux/)
...
**`-p protocol_name`：**
此选项用于匹配遵循 `protocol_name` 的数据包。

**`--dport port_number`：**
仅当您给出 `-p protocol_name` 选项时，此选项才可用。它指定查找发往端口“`port_number`”的数据包。
...
因此，来自 `192.168.1.3` 的任何数据包首先被检查它是否要到达端口 `22`，如果不是，则它通过链中的下一个规则运行。否则允许它通过防火墙。
...
### 4. 保存您的配置
如果您在不是服务器的个人计算机上实施，这一部分是不必要的，但是如果您在服务器上实施防火墙，那么您的服务器很有可能被破坏，并且您可能丢失所有数据。因此，保存您的配置总是更好。
