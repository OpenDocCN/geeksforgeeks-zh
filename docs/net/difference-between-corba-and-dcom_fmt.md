...
> 原文: [https://www.geeksforgeeks.org/difference-between-corba-and-dcom/](https://www.geeksforgeeks.org/difference-between-corba-and-dcom/)
...
| 基础 | 公共对象请求代理结构 (Common Object Request Broker Architecture) | DCOM |
| --- | --- | --- |
| 介绍人 | 它是由对象管理小组推出的 | 它是由微软推出的 |
| 关注 | 首先关注企业，然后是桌面 | 首先关注桌面，然后关注企业 |
| 平台 | 它在 `Unix`、`Windows` 和 `Macintosh` 上都有。 | 它可与 `Windows NT` 一起使用，并完全支持所有版本的 `Windows`、`Unix` 和 `Macintosh` |
...
| 继承 | 当每个接口都继承自 `CORBA` 对象时，支持接口级继承。 | 对象实现 `IUnknown` 接口时支持多重继承。 |
| 垃圾收集 | 它不提供通用的分布式垃圾收集 | 它通过 `ping` 线提供通用分布式垃圾收集。 |
| 异常处理 | 异常对象负责处理 | 异常被抛出到 `HRESULT` 中，对于其他异常必须实现 `ISupportErrorInfo` 接口 |
