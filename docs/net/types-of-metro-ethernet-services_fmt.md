...
电子线路服务也有变化。假设一个企业有一个中心站点和 10 个远程站点。如果企业想要使用电子线路服务，那么在 10 个远程站点和中心站点之间需要 10 条不同的电子线路。这听起来类似于 `E-tree` 服务，但有所不同。在 `E-tree` 实施中，所有这些链路都位于同一 `subnet` 中，但在 `E-line` 实施中，这 10 条链路位于 10 个 `subnet` 中。
