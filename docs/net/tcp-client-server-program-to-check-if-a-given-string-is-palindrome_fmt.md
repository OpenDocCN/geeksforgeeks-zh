...
> 原文: [https://www.geeksforgeeks.org/tcp-client-server-program-to-check-if-a-given-string-is-palindrome/](https://www.geeksforgeeks.org/tcp-client-server-program-to-check-if-a-given-string-is-palindrome/)

## 先决条件

*   [C/C++](https://www.geeksforgeeks.org/socket-programming-cc/)中的 Socket 编程，
*   [TCP 和 UDP 服务器使用 `select()`](https://www.geeksforgeeks.org/tcp-and-udp-server-using-select/)，
...
*   当连接建立时，客户端将通过 `send()` 系统调用向服务器发送用户输入字符串。
*   在服务器端，服务器将等待客户端发送的字符串。
*   服务器通过 `recv()` 系统调用读取字符串。
