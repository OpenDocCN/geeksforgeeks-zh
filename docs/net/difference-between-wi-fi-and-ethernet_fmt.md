...
> 原文: [https://www.geeksforgeeks.org/difference-between-wi-fi-and-ethernet/](https://www.geeksforgeeks.org/difference-between-wi-fi-and-ethernet/)
...
`以太网` 是使用最广泛的局域网技术，是在 `IEEE 标准 802.3` 下定义的。其广泛可用性背后的原因是以太网易于理解、实施、维护，并允许低成本的网络实施。此外，以太网在允许的拓扑方面提供了灵活性。以太网通常使用总线拓扑。以太网在现场视察模型的两层中运行，物理层和数据链路层。对于以太网，协议数据单元是帧，因为我们主要处理动态链接库。为了处理冲突，以太网中使用的访问控制机制是 `CSMA/光盘`。
