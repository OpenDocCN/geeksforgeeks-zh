...
网络中的每个顶点都有`2个变量`与之关联，分别是`高度变量(h)`和`过流(e)`。
...
重新标记到前端算法维护网络中的顶点列表。它从列表的开头开始，反复选择一个溢出的顶点`u`，并对其执行`放电操作`。
`放电操作`正在执行推送和重新标记操作，直到顶点`u`没有正的多余流量(`e`)。
...
运行于`O(V^3)`网络时间`G = (V，E)`。因此，它比在`O(V^2E)`时间运行的通用`推-重标记`算法更有效。

### 参考文献

[克利福德·斯坦、托马斯·h·科曼、查尔斯·e·雷瑟森、罗纳德·L·李维斯特《算法导论》第三版](http://www.euroinformatica.ro/documentation/programming/!!!Algorithms_CORMEN!!!/DDU0165.html)