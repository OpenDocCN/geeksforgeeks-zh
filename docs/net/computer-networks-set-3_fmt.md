...
(A)在`晚上 9:00`和`早上 5:00`期间阻止`HTTP`流量
(B)阻止所有`ICMP`流量
(C)阻止来自特定`IP`地址的传入流量，但允许来自同一`IP`的传出流量
(D)在`晚上 9:00`和`早上 5:00`期间阻止来自多用户系统上特定`IP`地址的特定用户的`TCP`流量
...
[邮局协议(`POP`)](http://en.wikipedia.org/wiki/Post_Office_Protocol)被客户用于接收邮件。
