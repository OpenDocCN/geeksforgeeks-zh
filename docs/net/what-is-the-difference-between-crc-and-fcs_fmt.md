...
> 原文：[https://www.geeksforgeeks.org/what-is-the-difference-between-crc-and-fcs/](https://www.geeksforgeeks.org/what-is-the-difference-between-crc-and-fcs/)
...
帧检查序列(`FCS`)是协议中添加到帧中的错误检测代码。帧用于将有效负载数据从源发送到目的地，也就是说，该帧用于将数据从源发送到目的地。
...
*注意-在`FCS`中，当`MSB`位为零时，我们插入一个长度与除数相同的零位序列，并执行和异或运算。*
