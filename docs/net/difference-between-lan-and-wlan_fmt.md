...
> 原文: [https://www.geeksforgeks.org/difference-between-lan-and-wlan/](https://www.geeksforgeks.org/difference-between-lan-and-wlan/)
...
| 有线局域网需要物理接入，比如将电线连接到`交换机`或`路由器`。 | 将电线连接到`交换机`和`路由器`的工作被忽略了。 |
| 在局域网中，设备通过`以太网电缆`本地连接。 | 对于无线局域网，`以太网电缆`不是必需的。 |
| 行动受限。 | 出色的机动性。 |
