...
配置一台 DHCP 服务器在一个子网上分发 IP 地址被称为 `DHCP池`。该地址池通常是单个 IP 子网内的一系列连续数字。如果需要阻止该范围内的任何地址，管理员可以执行此操作。子网掩码告诉设备它们所连接的子网有多大；从广播的角度来看，这是至关重要的。最后，默认网关是表示设备所属子网出口点的 IP 地址。
