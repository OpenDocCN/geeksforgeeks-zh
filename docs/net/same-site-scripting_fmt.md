...
1.  在共享 UNIX 系统上，攻击者监听非特权端口[0]，然后使用典型的 XSS 攻击向量(例如，`img src = …`)引诱受害者请求 `http://localhost.target.com:1024/cute_dogs.gif`。
2.  当受害者试图访问上述网址时，显示的响应类似于“加载图像失败”。
3.  然而，在后台，请求被记录。

## 作为管理员如何避免同站点脚本

1.  对于托管依赖 HTTP 状态管理的网站的域，应从名称服务器配置中删除非 FQ `localhost` 条目。
