...
> 原文：[https://www.geeksforgeeks.org/slammer-worm-in-information-security/](https://www.geeksforgeeks.org/slammer-worm-in-information-security/)
...
该蠕虫不会将自己保存在任何内存中，也不会在系统中创建或修改文件。它只影响那些在系统上运行 `Microsoft SQL Server 2000` 或 `MSDE 2000` 的系统。如果在系统中静默安装了 `MSDE 2000` 或 `Microsoft SQL Server 2000` 等软件，该蠕虫会影响设备。这种蠕虫花了 15 分钟传播到世界各地，感染了 30 多万人和 5.8 万人。由于该蠕虫消耗的带宽，许多 DNS 服务器宕机并经历了大量数据包丢失。
...
*   This worm can increase the traffic through `UDP port 1434`.
...
*   It enters the computer memory.
*   Then it loads some API functions to generate an Ip address and infect other machines.
*   It began to send multiple packets containing the worm code, and caused the port to be attacked by DDOS.
