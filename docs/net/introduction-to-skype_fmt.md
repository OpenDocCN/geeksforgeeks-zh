...
*   **语音连接–**
    找到搜索到的用户后，使用 [`TCP`（传输控制协议）](https://www.geeksforgeeks.org/tcp-ip-model/) 建立两者之间的语音连接，用于发出呼叫请求和呼叫结束的信号。对于语音呼叫的数据流，使用 `TCP` 或 [`UDP`（用户数据报协议）](https://www.geeksforgeeks.org/user-datagram-protocol-udp/)。一般来说，`UDP` 是首选的，因为快速的数据流。
*   **安全性–**
