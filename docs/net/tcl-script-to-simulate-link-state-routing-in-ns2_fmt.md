...
> 原文: [https://www.geeksforgeeks.org/tcl-script-to-simulate-link-state-routing-in-ns2/](https://www.geeksforgeeks.org/tcl-script-to-simulate-link-state-routing-in-ns2/)
...
我们接下来创建一个随机的节点数，比如说 7。我们使用 `Node` 实例如下创建这些节点。
...
下一步是在最终形成环的节点之间创建双工链路。这可以通过使用 `duplex-link` 实例并指定三个参数来实现:数据速率(1.5Mb)、延迟(10ms)和队列种类( `DropTail` )。
...
现在我们需要适当确定节点之间链接的方向，以获得适当的对齐。 `duplex-link-op` 实例也用于此目的。
...
下一步是在两个节点连接 TCP 代理(使用 `connect` 代理)，假设节点 1 和节点 4。我们可以通过创建源和接收器对象并使用 `connect` 实例连接它们来实现这一点。
...
最后一步是按要求的时间间隔调度 FTP 流量。我们还可以使用 `rtmodel-at` 实例在某个时间戳禁用任意一对节点之间的链接，然后在某个时间后启用它。这主要是为了测试的目的。这里我们已经禁用了节点 2 和 3 之间的链接。程序以 `run` 命令结束。
