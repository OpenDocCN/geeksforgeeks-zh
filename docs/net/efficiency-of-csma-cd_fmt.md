...
## 先决条件
- [以太网简介](https://www.geeksforgeeks.org/local-area-network-lan-technologies/)
- [CSMA/CD 基础知识](https://www.geeksforgeeks.org/computer-networks-gate-notes-set-1-data-link-layer/)

## 带碰撞检测的载波侦听多路访问 (CSMA/CD)
CSMA 方法没有告诉我们发生碰撞时该怎么办。带有冲突检测的载波侦听多路访问 (CSMA/CD) 增加了 CSMA 算法来处理冲突。在 CSMA/CD 中，帧的大小必须足够大，以便发送方在发送帧时可以检测到冲突。所以，帧传输延迟必须至少是*最大传播延迟的两倍*。
...
在 CSMA/CD 中，为了成功，只有一个站点应该传输，而其他站点不应该传输。设 `p` 为成功传输数据的概率。

```
P(success) = nC1 * p * (1-p)^(n-1) (by using Binomial distribution)
```

对于最大 `P(success)`，求 `P` 的微分并等于零（得到最大值和最小值）。
...
在效率公式 `Efficiency = Tt / (C * 2 * Tp + Tt + Tp)` 中输入 `a = Tt/Tp` 并除以 `Tt`。
