...
当消息以 `QoS` 等级 1 发布时，消息将被传递 `at least once`。
在此 `QoS` 等级中，发送方发布消息并存储它，直到收到接收方的确认。一旦收到确认，发送方将丢弃该消息。如果未及时收到确认，消息将再次发送。因此，在此 `QoS` 等级中，消息可以传递一次，有时甚至超过一次，以确保其成功传递。
...
当客户端以 `QoS` 等级 2 发布消息时，消息将被传递 `exactly once`。在此 `QoS` 等级中，发送方发布消息，存储它，并等待 `PUBREC`。接收方在收到消息后向发送方发送 `PUBREC`。`PUBREC` 表示消息已成功传递给接收方。因此，在收到 `PUBREC` 后，发送方将丢弃该消息。然后它向接收方发送 `PUBREL`。在收到 `PUBREL` 后，接收方丢弃保存的状态并发送 `PUBCOMP`。当发送方收到最终的 `PUBCOMP` 时，它也将丢弃先前保存的状态。因此，在 `QoS` 等级 2 中，用于通信的附加消息确保了消息的成功传递恰好一次。
