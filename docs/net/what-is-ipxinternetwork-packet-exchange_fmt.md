...
> 原文：[`https://www.geeksforgeks.org/what-is-ipxinternetwork-packet-exchange/`](https://www.geeksforgeks.org/what-is-ipxinternetwork-packet-exchange/)
...
*   `IPX` 的地址空间更大：`48位`，而不是 `IPv4` 中的 `32位`。
*   `IPX` 地址包含本地媒体访问控制地址：与 `IPv4` 的“地址分配”相比。
*   `IPX` 没有 `BootP` 或 `DHCP`。（`DHCP` 是从 `BootP` 发明的，因此 `IPv4` 可以像 `IPX` 那样允许“即插即用”的网络寻址。它后来被添加到 `IPv6` 中。）
