...
## OSI
`OSI`代表**开放系统互联**。它是一个参考模型，规定了通信协议的标准以及每一层的功能。

## 协议
协议是一组规则或算法，定义了两个实体如何通过网络通信，并且在 OSI 模型的每一层都定义了不同的协议。这样的协议很少有 `TCP`、`IP`、`UDP`、`ARP`、`DHCP`、`FTP` 等等。

## 网络的唯一标识符

### 主机名
网络中的每个设备都与一个称为主机名的唯一设备名相关联。
在命令提示符(管理员模式)中键入`hostname`，然后按“回车”，这将显示您机器的主机名。
