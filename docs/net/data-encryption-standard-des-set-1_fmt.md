...
> 原文: [https://www.geeksforgeeks.org/data-encryption-standard-des-set-1/](https://www.geeksforgeeks.org/data-encryption-standard-des-set-1/)
...
2.  初始置换在纯文本上执行。
3.  接下来，初始置换产生置换块的两半；表示左纯文本(`LPT`)和右纯文本(`RPT`)。
...
正如我们已经注意到的，初始排列(`IP`)只发生一次，而且发生在第一轮之前。它建议如何进行 `IP` 的转换，如图所示。
