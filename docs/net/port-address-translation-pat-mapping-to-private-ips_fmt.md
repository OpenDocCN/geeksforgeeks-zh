...
## 家庭网络中私有 IP 的使用:
[私有 IP](https://www.geeksforgeeks.org/difference-between-private-and-public-ip-addresses/) 未注册，因此不能用于在互联网上唯一地查找用户。私有 IP 通常在 `192.168.X.X` 和 `10.X.X.X` 的范围内。这些私有 IP 由路由器分配给连接到路由器的每个设备。
