...
> 原文: [https://www.geeksforgeeks.org/cisco-asa-redistribution-example/](https://www.geeksforgeeks.org/cisco-asa-redistribution-example/)

## 先决条件
– [自适应安全设备(ASA)](https://www.geeksforgeeks.org/computer-network-adaptive-security-appliance-asa-features/)
...
有 4 个路由器，即路由器 1 (IP 地址-`10.1.1.1/24`)、路由器 2 (IP 地址-`10.1.2.1/24`)、路由器 3 (IP 地址-`10.1.3.1/24`)、路由器 4(IP 地址-`10.1.4.1/24`)和 ASA(IP 地址-`10.1.1.2/24` 和名称-内部在 `e0`、`10.1.2.2/24` 和名称-外部在 `e1`、`10.1.3.2/24` 和名称-DMZ2 在 `e2`、`10.1.4.2/24` 和名称-DMZ1 在 `e3`)。在本任务中，我们将从内部到外部检查 ICMP。
...
这里，`outside` 是接口名称，`0 0` 表示任意 IP 任意掩码，`10.1.2.1` 是下一跳 IP 地址。
