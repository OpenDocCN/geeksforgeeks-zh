...
> 原文: [https://www.geeksforgeeks.org/difference-between-smtp-and-pop3/](https://www.geeksforgeeks.org/difference-between-smtp-and-pop3/)

对于发送和接收消息，我们使用两种协议，一种是 [`SMTP`](https://www.geeksforgeeks.org/simple-mail-transfer-protocol-smtp/)（简单邮件传输协议），另一种是 [`POP3`](https://practice.geeksforgeeks.org/problems/what-is-pop)（邮局协议版本 3）。它们也分别被称为 `PUSH` 和 `POP` 协议。它们分别是发送和检索消息的代理、消息传输代理和消息访问代理。
