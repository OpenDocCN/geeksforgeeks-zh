...
*   假设 `N` 个设备在网状拓扑中相互连接，那么连接它们所需的专用链路总数为 `N(N-1)/2`。在图 1 中，有 5 个设备相互连接，因此所需的链路总数为 `5*4/2 = 10`。
...
*   如果公共电缆出现故障，整个系统就会崩溃。
*   如果网络流量很大，会增加网络中的冲突。为了避免这种情况，在媒体访问控制层使用了各种协议，如纯 Aloha、时隙 Aloha、`CSMA/CD` 等。
*   安全性很低。
