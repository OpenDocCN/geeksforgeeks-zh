...
> 原文: [https://www.geeksforgeeks.org/logical-link-control-llc-protocol-data-unit/](https://www.geeksforgeeks.org/logical-link-control-llc-protocol-data-unit/)
...
*   **信息(`I`)**：一般包括 7 位序列号(`N(S)`)和一个捎带序列号(`N(R)`)。它用于携带数据或信息。
*   **监督(`S`)**：它通常包括一个确认序列号(`N(R)`)和一个 2 位`S`字段，用于三种不同的`PDU`格式，即`RNR`(接收未就绪)、`RR`(接收就绪)和`REJ`(拒绝)。它通常用于流量和错误控制。
*   **未编号(`U`)**：它通常是一个 5 位`M`位，用于指示`PDU`的类型。它用于各种协议`PDU`。
