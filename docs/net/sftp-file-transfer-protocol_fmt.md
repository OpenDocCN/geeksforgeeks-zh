...
SFTP(安全文件传输协议)是 FTP(文件传输协议)的高级版本，它确保在组织/计算机之间传输文件时的安全性。又名 [`SSH`](https://www.geeksforgeeks.org/introduction-to-sshsecure-shell-keys/) (安全壳)。它在 22 号端口工作，使用[客户端-服务器模型](https://www.geeksforgeeks.org/client-server-model/)。
