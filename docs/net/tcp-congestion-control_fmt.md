...
## 先决条件
[基本拥塞控制知识](https://www.geeksforgeeks.org/computer-networks-congestion-control/)
...
## 拥塞避免阶段: 附加增量
该阶段在阈值(也称为 `ssthresh`)之后开始。`cwnd` (拥堵窗口)的尺寸增加了添加剂。在每个 `RTT` 之后 `cwnd = cwnd + 1`。
...
## 拥塞检测阶段: 乘法递减
如果发生拥塞，拥塞窗口大小减小。发送方猜测拥塞发生的唯一方法是需要重新传输一个数据段。需要重传来恢复丢失的数据包，该数据包被认为是由于拥塞而被路由器丢弃的。重传可以发生在以下两种情况之一: 当实时操作系统定时器超时或收到三个重复确认时。
...
## 示例
假设一个 TCP 协议经历了慢启动行为。在第 5 轮传输中，阈值(`ssthresh`)值为 32 的传输进入拥塞避免阶段，并持续到第 10 轮传输。在第 10 轮传输中，接收器接收到 3 个重复确认，并进入加法增加模式。超时发生在第 16 轮传输。绘制传输周期(时间)与 TCP 段拥塞窗口大小的关系图。
