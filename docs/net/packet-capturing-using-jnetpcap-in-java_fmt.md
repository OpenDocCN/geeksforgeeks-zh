...
> 原文：[https://www.geeksforgeeks.org/packet-capturing-using-jnetpcap-in-java/](https://www.geeksforgeeks.org/packet-capturing-using-jnetpcap-in-java/)
...
6.  现在打开 Eclipse，创建项目。右键单击项目，转到属性，转到 java build 路径，单击添加外部 jar 并提供 `jnetpcap.jar` 的路径。
7.  写一个程序然后运行。
...
3.  安装 `g++` 和 `libpcap-dev`(从命令行安装，因为如果它不是更新的，它不会出现在软件中心)。
...
7.  现在打开 Eclipse，创建项目。右键单击项目，转到属性，转到 java build 路径，单击添加外部 jar 并提供 `jnetpcap.jar` 的路径。
8.  写一个程序然后运行。
...
> 输出：
> ```
> File : 1.pcap Number of Packets : 502.0
...
> 输出：
> ```
> File : tcp.pcap Number of Packets : 10.0
