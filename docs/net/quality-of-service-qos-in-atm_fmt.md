...
> 原文: [https://www.geeksforgeeks.org/quality-of-service-qos-in-atm/](https://www.geeksforgeeks.org/quality-of-service-qos-in-atm/)
...
ATM 中的服务质量基于以下内容：类别、用户相关属性和网络相关属性。这些解释如下。

## 类别

ATM 论坛定义了四种服务类别，解释如下：
...
2.  **可变比特率(VBR)**
    VBR 类分为两个子类：
    *   **实时(VBR-RT)**：需要音频和视频等实时传输服务的用户，他们也使用压缩技术来创建可变比特率，他们使用 VBR-RT 服务等级。
    *   **非实时(VBR-NRT)**：不需要实时传输服务但使用压缩技术创建可变比特率的用户，然后使用 VBR-NRT 服务类别。
...
1.  **持续细胞率(SCR)**
    SCR 是长时间间隔内的平均细胞率。原始电池速率可以小于或大于 `SCR` 值，但平均值必须等于或小于 `SCR` 值。

2.  **峰值细胞率(PCR)**
    PCR 定义为发送者的最大细胞率。只要保持 `SCR`，用户的小区速率就能达到这个峰值。

3.  **最小信元速率(MCR)**
    MCR 定义发送方可接受的最小信元速率。例如，如果 `MCR` 为 50,000，网络必须保证发送方每秒至少可以发送 50,000 个信元。
...
用于定义网络不同特征的属性称为网络相关属性。以下是一些网络相关属性：
