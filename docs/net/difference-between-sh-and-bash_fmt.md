...
> 原文:[https://www.geeksforgeeks.org/difference-between-sh-and-bash/](https://www.geeksforgeeks.org/difference-between-sh-and-bash/)
...
| **bash** | **sh** |
| :--- | :--- |
| `Bourne SHell again` | `Shell` |
| `Younaohu` | 由斯蒂芬·伯恩开发的 T3 |
| `嘘` | `The predecessor of Bashi` |
| `Bash` 是默认的 SHELL。 | `Sh` 不是默认的 SHEL |
| `#!/bin/bash` | `#!/bin/sh` |
| 升级后，功能更丰富。 | 功能较少。 |
| 支持作业控制。 | 不支持作业控制。 |
| `Bash` 不是有效的 POSIX 外壳。 | `嘘`是有效的 POSIX 外壳。 |
| 易于使用 | 不像 `bash` 那么容易 |
| 不如 `sh` 可移植。 | 比 `bash` 更可移植。 |
| 扩展语言 | 原始语言 |
| `Bash` 脚本是为 `Bash` 编写的脚本 | `Shell` 脚本是在任何 `Shell` 中编写的脚本 |
| 支持命令历史。 | 不支持命令历史。 |