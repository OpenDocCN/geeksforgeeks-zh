...
> 原文：[https://www.geeksforgeeks.org/maximum-data-rate-channel-capacity-for-noiseless-and-noisy-channels/](https://www.geeksforgeeks.org/maximum-data-rate-channel-capacity-for-noiseless-and-noisy-channels/)
...
奈奎斯特证明了如果一个任意信号已经通过了一个带宽的低通滤波器，那么通过每秒只做 `2 * Bandwidth`（精确）采样就可以完全重构出滤波后的信号。以每秒 `2 * Bandwidth` 的速度对线路进行采样是没有意义的，因为这种采样可以恢复的高频分量已经被滤除了。如果信号由 `L` 个离散电平组成，奈奎斯特定理指出：

```
BitRate = 2 * Bandwidth * log2(L) bits/sec
```

在上面的等式中，`Bandwidth` 是信道的带宽，`L` 是用于表示数据的信号电平数，`BitRate` 是以比特每秒为单位的比特率。

`Bandwidth` 是一个固定的量，所以不能改变。因此，数据速率与信号电平的数量成正比。
...
**输出 1：** 比特率 = `2 * 3000 * log2(2) = 6000 bps`

**输入 2：** 我们需要通过 20 kHz 带宽的无噪声信道发送 265 kbps。我们需要多少信号水平？
**输出 2：** `265000 = 2 * 20000 * log2(L)`
`log2(L) = 6.625`
`L = 2^6.625 = 98.7` 级
...
在上式中，`Bandwidth` 是信道的带宽，`SNR` 是信噪比，`Capacity` 是信道每秒比特数的容量。

`Bandwidth` 是一个固定的量，所以不能改变。因此，信道容量与信号功率成正比，`SNR = (信号功率) / (噪声功率)`。
...
**输出 1：** `C = 3000 * log2(1 + SNR) = 3000 * 11.62 = 34860 bps`

**输入 2：** 信噪比常以分贝给出。假设信噪比为 36 分贝，信道带宽为 2 兆赫。计算理论信道容量。
**输出 2：** `SNR(dB) = 10 * log10(SNR)`
`SNR = 10^(SNR(dB)/10)`
`SNR = 10^3.6 = 3981`
因此，`C = 2 * 10^6 * log2(3982) = 24 MHz`

**参考：**
[书籍——计算机网络：FOROUZAN 的自上而下方法](https://amzn.to/3hmRN6W)