...
`SSH` 端口转发是一种用于保护 `TCP/IP` 连接的方法。`TCP/IP` 数据包可以通过 `SSH` 链路进行隧道传输，使数据变得模糊，从而保护链路免受攻击。`SSH` 端口转发也可以看作是虚拟专用网络(`VPN`)的一种形式。
...
1.  **Local Port Forwarding:**
    ...
    这里的`-L`标志表示正在使用本地端口转发。因此，上述命令的作用是将本地端口`9090`上的数据转发给可以通过安全 `SSH` 连接访问`example.com`的服务器。请注意，`admin`是服务器上的用户。现在用你的浏览器访问`example.com`。
