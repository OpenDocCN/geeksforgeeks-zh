...
## 先决条件:
[`IP 地址`](https://www.geeksforgeeks.org/structure-and-types-of-ip-address/#:~:text=An%20IP%20address%20represents%20an,or%20through%20a%20local%20network.)、[`子网掩码`](https://www.geeksforgeeks.org/role-of-subnet-mask/)、[`DNS 服务器`](https://www.geeksforgeeks.org/domain-name-system-dns-in-application-layer/)。
...
## 要遵循的步骤:
1.  转到搜索栏并搜索`控制面板`。一旦它出现，然后双击它。将出现一个对话框。
