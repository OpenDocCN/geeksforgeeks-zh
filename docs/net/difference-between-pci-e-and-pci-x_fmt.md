...
> 原文：[https://www.geeksforgeeks.org/difference-between-pci-e-and-pci-x/](https://www.geeksforgeeks.org/difference-between-pci-e-and-pci-x/)

`1。外围组件互连扩展(PCI-X) :`
`PCI-X` 或外围组件互连扩展是一种与扩展卡一起提供的本地计算机总线。它是外围组件互连的扩展版本，具有更高的带宽。它于 1998 年由 `IBM`、`惠普`、`康柏`推出，具有支持更高时钟速度和热插拔的特性。它是并行设计的，宽度为 64 位。由于运行缓慢，后来被其他名为 `PCI-E` 的版本取代。
...
| 2. | 它是在 1998 年由 `IBM`、`惠普`和`康柏`推出的。 | 它于 2003 年由英特尔、戴尔、惠普和 `IBM` 推出。 |
