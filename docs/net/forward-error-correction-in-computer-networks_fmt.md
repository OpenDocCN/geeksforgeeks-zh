...
> 原文：[https://www.geeksforgeeks.org/forward-error-correction-in-computer-networks/](https://www.geeksforgeeks.org/forward-error-correction-in-computer-networks/)
...
```
如果对 `N` 个数据项应用 XOR 属性，我们可以通过对所有数据项进行异或运算来重新创建 `P_1` 到 `P_N` 的任何一个数据项，用之前操作的结果(`R`)替换要创建的数据项。在这种技术中，一个数据包被分成 `N` 个块，然后创建所有块的异或，然后发送 `N+1` 个块。如果任何数据块丢失或损坏，可以在接收端重新创建。
