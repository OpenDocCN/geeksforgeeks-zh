...
> 原文: `https://www.geeksforgeeks.org/difference-between-gprs-and-dgps/`
...
| S.NO | `GPRS` | `DGPS` |
| --- | --- | --- |
| 1. | `GPRS` 代表`通用分组无线业务`。 | 而 `DGPS` 代表`差分全球定位系统`。 |
| 2. | `GPRS` 不太贵。 | 而 `DGPS` 的成本比 `GPRS` 更高。 |
| 3. | 它不能在任何地方使用，只能在有限范围内在陆地上使用。 | 而 `DGPS` 的仪器范围是本地的。 |
| 4. | 在 `GPRS` 中，许多接收器被取代。 | 在`差分全球定位系统`中，有两个接收器。 |
