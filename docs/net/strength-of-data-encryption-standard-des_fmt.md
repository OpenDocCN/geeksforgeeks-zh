...
> 原文：[https://www.geeksforgeeks.org/strength-of-data-encryption-standard-des/](https://www.geeksforgeeks.org/strength-of-data-encryption-standard-des/)
...
关于所用算法的第一个问题是利用 `DES` 算法的特性来解决密码分析的可能性。更严重的问题是所用密钥的长度。可以有 `2^{56}`（大约 `7.2 × 10^{16}` 键）个可能的键，键长为 `56` 位。因此，暴力攻击似乎是不切实际的。
