...
> 原文：[https://www.geeksforgeeks.org/network-address-translation-nat/](https://www.geeksforgeeks.org/network-address-translation-nat/)
...
1.  **静态 NAT (`Static NAT`)** – 在这种方式中，一个未注册的(私有)IP 地址与一个合法注册的(公共)IP 地址进行映射，即本地地址和全局地址之间的一对一映射。这通常用于网站托管。这些在组织中不常用，因为会有许多设备需要互联网访问，而提供互联网访问需要一个公共 IP 地址。
    假设，如果有 3000 台设备需要接入互联网，组织必须购买 3000 个公共地址，这将非常昂贵。

2.  **动态 NAT (`Dynamic NAT`)** – 在这种类型的 NAT 中，一个未注册的 IP 地址被转换为一个注册的(公共)IP 地址，该地址来自一个公共 IP 地址池。如果地址池中的 IP 地址没有空闲，那么数据包将被丢弃，因为只有固定数量的私有 IP 地址可以被转换为公共地址。
