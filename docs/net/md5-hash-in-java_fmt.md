...
为了计算 Java 中的加密散列值，使用了 `java.security` 包下的 `MessageDigest` 类。
...
该算法在静态方法 `getInstance()` 中初始化。选择算法后，它计算 `digest()` 值，并以字节数组的形式返回结果。

使用 `BigInteger` 类，将结果字节数组转换为其符号幅度表示。
...
[消息摘要的 Java 文档](https://docs.oracle.com/javase/7/docs/api/java/security/MessageDigest.html)