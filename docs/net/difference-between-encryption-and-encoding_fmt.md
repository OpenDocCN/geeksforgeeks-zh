...
> 原文：[`https://www.geeksforgeeks.org/difference-between-encryption-and-encoding/`](https://www.geeksforgeeks.org/difference-between-encryption-and-encoding/)
...
这是一个用于将称为纯文本的简单可读数据转换为称为密文的不可读数据的过程，只有当用户知道加密密钥时，密文才能转换为纯文本。它基本上用于保护我们的数据安全。加密的主要目的是将我们的数据转换成对不知道加密密钥的人来说是垃圾的形式。它用于防止未经授权的访问。加密的反义词是解密，它用于从密文中获取纯文本。对于解密，我们必须知道加密密钥和加密算法。
...
| **算法示例** | `AES`、`RSA` 和 `河豚`。 | `ASCII`，`UNICODE`，URL 编码，`Base64`。 |
