...
> 原文: [https://www.geeksforgeeks.org/fixed-length-and-variable-length-subnet-mask-numericals/](https://www.geeksforgeeks.org/fixed-length-and-variable-length-subnet-mask-numericals/)
...
剩余可变位数 = `32 - 28 = 4` 位。因此，每个子网中的地址总数 = `2^4 = 16`。
...
4.  要查找最后一个地址，请将(子网中的地址数 - `1`)添加到该子网的第一个地址。在这种情况下，将(`16 - 1`)添加到每个子网的第一个地址。所以第 `1` 个子网的最后一个地址是：
...
1.  要找到第 `1` 个子网的第一个地址，在给定的地址和第 `1` 个子网的子网掩码之间执行 `AND` 操作。
    执行 `AND` 操作：
...
2.  要找到子网的最后一个地址，在子网的第一个地址和子网掩码的补码之间执行 `OR` 操作。
    第 `1` 个子网的第一个地址：
