...
> 原文: [https://www.geeksforgeeks.org/link-access-procedure-balanced-lapb-frame-types/](https://www.geeksforgeeks.org/link-access-procedure-balanced-lapb-frame-types/)
...
## 1. 信息帧
这些帧通常被称为信息帧。基本上是用来承载上层的信息或者数据。它包括两个不同的序列号，即用于发送序列号的 `N(S)` 和用于接收序列号的 `N(R)`，序列号 `N(S)` 通常在下一个将要接收的 I 帧中取的期望值。该帧还包括 X.25 PLP 数据包，并且总是命令。
