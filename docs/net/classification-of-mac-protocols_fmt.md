...
> 原文：[https://www.geeksforgeeks.org/classification-of-mac-protocols/](https://www.geeksforgeeks.org/classification-of-mac-protocols/)
...
*   接收器发起的协议：
    连接由接收器节点发起。比如 `RI-BTMA`，`MACA-BI`，`MARCH`。

## 2. 带有预留机制的基于竞争的协议

*   同步协议：
    所有节点保持同步。例如，`PRMA` 特区、`FAMA`、`HRMA`、`SRMA/PA`、`FPRP`。
*   异步协议：
...
这些 MAC 协议严格来说不属于以上任何一类。例如，`MMAC`，`MARCH`，`PCM`，`PCMA`。