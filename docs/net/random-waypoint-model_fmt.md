...
起始点为`(x1，y1)`。它正朝着特定的方向发展。当它到达`(x2，y2)`时，它必须再次改变方向，因为从起始点到`(x2，y2)`然后从`(x2，y2)`到`(x3，y3)`的角度不同。速度也可能不同。我们可以说，首先它以`10 米/秒`的速度运动，其次它以`5 米/秒`的速度运动。因此，对于特定的时间段，即暂停时间，比如说`x 米/秒`。它将在该时间内停止，然后将继续进一步处理。这是它的工作方式，直到最终目的地到达，即`(x9，y9)`。
