...
**1G** 代表**第一代**，**2G** 代表**第二代**，是两代手机技术。1G 是第一代将无线通信带到美国的手机。

1G 使用模拟信号进行通信，采用 FDMA 进行信道化。1G 主要用于语音通话，不提供数据传输服务。2G 是 1G 的升级，它使用数字信号进行通信。2G 采用时分多址（TDMA）和码分多址（CDMA）进行信道划分，提供语音和数字通信服务。
...
| 2. | 1G 使用电路交换。 | 而 2G 使用电路交换以及分组交换。 |
| 3. | 在 1G 中，不提供互联网。 | 而在 2G，则提供互联网。 |
| 4. | 1G 的语音信号是模拟信号。 | 而 2G 的语音信号是数字信号。 |
| 5. | 1G 中不提供数据服务。 | 而在 2G 中，除了语音，还提供数据服务（如视频）。 |
