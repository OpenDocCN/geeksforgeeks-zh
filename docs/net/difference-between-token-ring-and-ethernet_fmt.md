...
> 原文：[https://www.geeksforgeeks.org/difference-between-token-ring-and-ethernet/](https://www.geeksforgeeks.org/difference-between-token-ring-and-ethernet/)
...
`IEEE 802.3`定义了以太网。它使用`CSMA/CD`机制。意思是如果同时有很多站存在通话，所有站都会关闭。要恢复它们，请随机等待一段时间。与令牌环不同，它不采用任何优先级。而且比令牌环网便宜。
...
| S.NO | 令牌网 | 以太网 |
| --- | --- | --- |
| 1. | 在令牌环中，使用令牌传递机制。 | 而以太网使用`CSMA/CD`(载波侦听多路访问/冲突检测)机制。 |
