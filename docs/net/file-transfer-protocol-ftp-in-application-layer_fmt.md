...
> 原文: [https://www.geeksforgeeks.org/file-transfer-protocol-ftp-in-application-layer/](https://www.geeksforgeeks.org/file-transfer-protocol-ftp-in-application-layer/)
...
*   `LIST` – 发送显示目录中所有文件列表的请求。
*   `ABOR` – 该命令告诉服务器中止之前的 `FTP` 服务命令和任何相关的数据传输。
*   `QUIT` – 该命令终止用户，如果文件传输没有进行，服务器关闭控制连接。
...
*   它适用于简单的文件传输，例如在引导期间。
*   它使用 `UDP` 作为传输层协议。传输中的错误（数据包丢失、校验和错误）必须由 `TFTP` 服务器处理。
*   它只使用一个通过众所周知的端口 `69` 的连接。
*   `TFTP` 使用一个简单的锁步协议（每个数据包都需要确认）。因此吞吐量是有限的。

## 匿名 FTP

在一些文件可供公众访问的网站上启用了匿名 `FTP`。用户无需任何用户名或密码即可访问这些文件。相反，默认情况下，用户名设置为 `anonymous`，来宾密码设置为匿名。在这里，用户访问非常有限。例如，用户可以复制文件，但不能浏览目录。
