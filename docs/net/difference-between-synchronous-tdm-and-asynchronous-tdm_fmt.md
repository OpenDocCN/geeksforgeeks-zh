...
> 原文：`https://www.geeksforgeeks.org/difference-between-synchronous-tdm-and-asynchronous-tdm/`
...
| 2. | 每帧的时隙数等于输入线数，即如果它包含 n 条输入线，那么它在一帧中必须有 n 个时隙。 | 每帧的时隙数量少于输入线的数量，即，如果有 n 条输入线，则一帧中有 m 个时隙(m < n)。 |
