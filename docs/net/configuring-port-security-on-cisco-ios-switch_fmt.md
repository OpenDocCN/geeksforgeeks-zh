...
> 原文: [https://www.geeksforgeeks.org/configuring-port-security-on-cisco-ios-switch/](https://www.geeksforgeeks.org/configuring-port-security-on-cisco-ios-switch/)
...
#### 步骤-1:
首先，访问思科交换机的命令行接口，使用`接口千兆以太网 0/1`命令连接一个端口千兆以太网 0/1。然后，执行`switchport mode access`和`switchport port-security`命令，分别将端口模式更改为访问和启用安全性。无论何时连接任何其他端口，并且需要在其上启用端口安全，都必须执行这些命令。

![](img/8ab5d4cd19532c3761050447f639504b.png)

#### 步骤-2:
默认情况下，Cisco 交换机在单个端口上只允许 1 个 MAC 地址，如果任何其他设备尝试使用该端口进行连接，交换机会自动关闭其端口以限制未经授权的访问。为了增加单个端口上的用户数量，将执行`switchport port-security maximum 5`命令。这样，现在五个设备可以连接到一个端口并执行它们的功能。
...
#### 步骤-1:
要仅允许具有定义的媒体访问控制地址的设备与思科交换机连接，可以使用`switchport port-security mac-address`命令。成功执行此命令后，具有指定媒体访问控制地址的设备将仅被交换机授权通过可用的以太网端口进行连接。

![](img/c6b84d4c953973fbaab35cba80797f4d.png)

#### 步骤-2:
`mac-address`命令的另一种模式称为粘性。启用此模式可允许交换机学习新的媒体访问控制地址并将其存储在内存中。交换机自动收集并存储连接设备的媒体访问控制地址。
...
#### 步骤-1:
要检查和分析交换机上的端口安全配置，用户需要访问命令行界面的特权模式。执行`show port-security address`命令来检查当前端口安全状态。
...
#### 步骤-2:
如果用户想要检查交换机上可用的特定接口的端口安全性，那么可以使用`show port-security interface`命令。
