...
> 原文：[https://www.geeksforgeeks.org/cyber-security-attacking-through-command-and-control/](https://www.geeksforgeeks.org/cyber-security-attacking-through-command-and-control/)
...
*   连接的对等方或节点在应用程序中也被视为平等成员。
*   据说它们建立了一个共享的中心组织。
*   节点提供自己的一些资产，例如处理能力、网络储备或组织数据传输，直接给其他组织成员，而无需工作人员或稳定主机的协调。
*   它是资产的提供者和购买者，而不是传统的客户-服务器模型，在传统模型中资产的使用和优雅是分离的。
*   协作式 `P2P` 框架的发展正经历一个时期，朋友们同时共享资产并进行比较，并且他们正在寻找可以获取非凡资产和能力的不同合作伙伴，以便虚拟网络可以参与比单个朋友可以培养的更伟大的事业，同时对所有合作伙伴都有益。

**3. 随机模型：** 任意地理僵尸网络不依赖任何 `C&C` 主机；相反，所有僵尸网络订单都是合法发送的，从一个僵尸开始，然后发送到下一个，极有可能它们被一些不常见的方法“标记”，表明它们是从僵尸网络所有者或另一个经批准的客户开始的。

*   这种类型的僵尸网络具有极高的休眠性，并且会频繁地考虑到僵尸网络中的大量僵尸，因此分析师仅通过捕获一个僵尸网络就可以识别它。
*   在更复杂的 `C&C` 主机地理（例如在 `TDL-4` 僵尸网络中）中使用了开放组织中常见的异常类型的混淆机器人和机器人通信，以提供一种特别难以摧毁的僵尸网络。
