...
Telnet 是电信和网络的联合缩写，是 UNIX 平台最著名的网络协议。Telnet 使用端口 23，它是专门为局域网设计的。
...
1.  与 Telnet 相比，SSH 更安全。
2.  SSH 加密数据，而 Telnet 以纯文本形式发送数据。
3.  SSH 使用公钥进行身份验证，而 Telnet 不使用任何身份验证。
4.  与 Telnet 相比，SSH 给带宽增加了更多的开销。
5.  在几乎所有的应用中，Telnet 几乎都被 SSH 取代了。
6.  SSH 和 Telnet 通常服务于相同的目的。
