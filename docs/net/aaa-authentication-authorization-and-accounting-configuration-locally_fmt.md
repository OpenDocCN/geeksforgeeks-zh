...
> 原文: [https://www.geeksforgeeks.org/aaa-authentication-authorization-and-accounting-configuration-locally/](https://www.geeksforgeeks.org/aaa-authentication-authorization-and-accounting-configuration-locally/)
...
它由`aaa authentication login default local`命令启用。
...
我们可以通过命令`debug aaa authentication`看到 AAA 认证消息。
...
现在，我们将从路由器 2 (ip 地址–`10.1.1.2/24`)远程登录路由器 1 (ip 地址-`10.1.1.1/24`)，它将要求如图所示的凭据。
...
```
router1(config)#aaa authentication username-prompt "enter your username"
router1(config)#aaa authentication password-prompt "enter your password"
```
