...
> 原文：[https://www.geeksforgeeks.org/how-does-a-switch-learn-pc-mac-address-before-the-ping-process/](https://www.geeksforgeeks.org/how-does-a-switch-learn-pc-mac-address-before-the-ping-process/)
...
假设将 `IP` 地址 `192.168.0.1 255.255.255.0` 分配给 `PC1`。它会立即生成一个带有广播目的 `mac` 地址 `FFFF.FFFF` 和目的地 `IP` 地址及其自己的 `IP` 地址 `192.168.0.1` 的 `ARP` 数据包。
