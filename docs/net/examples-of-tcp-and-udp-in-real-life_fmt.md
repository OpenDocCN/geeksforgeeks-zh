...
> 原文: [https://www.geeksforgeeks.org/examples-of-tcp-and-udp-in-real-life/](https://www.geeksforgeeks.org/examples-of-tcp-and-udp-in-real-life/)
...
计算机网络是一组通过通信链路相互连接以共享数字信息的设备(节点)。通信链路可以是无线介质，也可以是有线介质。在当今时代，不可能想象一个没有计算机网络的世界。从和朋友聊天到打视频电话，由于计算机网络的存在，一切皆有可能。
...
`TCP/IP` 协议套件中的层与 `OSI` 模型中的层不完全匹配。最初的 `TCP/IP` 协议套件被定义为具有四层：主机到网络、互联网传输和应用。`TCP/IP` 参考模型是理论上的 `OSI` 模型的实际实现。

## 传输层

会话层下面的层称为传输层。该层中的数据称为数据段。它负责将消息从一个进程传递到另一个进程。它还负责通过流量控制、错误控制和分段进行可靠的通信。一些最重要的协议是传输控制协议(`TCP`)和用户数据报协议(`UDP`)。它进行两种类型的传输，即面向连接的传输和无连接的传输，分别由 `TCP` 和 `UDP` 实现。
...
*   使用端口号进行进程间或端到端通信。
*   将数据作为字节流从源传输到目标。
*   它将来自应用程序的数据分组称为段的数据包。它还为每个数据段添加一个报头，并将其发送到网络层。
*   它提供全双工通信。
...
它是一种传输层协议。这是一个不可靠的无连接协议。它比 `TCP` 更快、更简单、更高效。但是，它不会检查由于节省带宽而导致的错误。它广泛应用于视频游戏、语音或视频通信等实时服务。`UDP` 发送的数据包称为用户数据报。
...
1.  **文本通信 –**
    我们都知道文本通信在当下的重要性。发送方和接收方之间的任何短信差异都是不可容忍的。因此，`TCP` 因其可靠的传输、错误控制和有序接收数据的特性而被用于文本通信。
    示例：`Whatsapp`、`Instagram`、`Google Chat`、`iMessage`。
2.  **文件传输或 `FTP`–**
...
3.  **IP 语音(`VoIP`)–**
    ...
    例如：`Viber`、`Whatsapp` 语音通话、`Wi-Fi` 通话。
4.  **域名系统(`DNS`) –**
    它是一种用于将域名映射到其相应 `IP` 地址的服务。它被应用层使用。它也可以被视为一个具有分层名称服务器的分布式数据库。`DNS` 使用 `UDP` 来获取相应的 `IP` 地址，原因如下：
