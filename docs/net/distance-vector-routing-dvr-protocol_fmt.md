...
> 原文: [https://www.geeksforgeeks.org/distance-vector-routing-dvr-protocol/](https://www.geeksforgeeks.org/distance-vector-routing-dvr-protocol/)

距离矢量路由协议要求路由器定期通知其邻居拓扑变化。历史上称为老的 ARPANET 路由算法（或称为 Bellman-Ford 算法）。

## 贝尔曼-福特基础知识

每台路由器都维护一个距离向量表，其中包含自身与所有可能的目的节点之间的距离。基于所选度量的距离是使用来自邻居的距离向量的信息来计算的。
...
考虑 3 路由器 `X`、`Y` 和 `Z`，如图所示。每台路由器都有自己的路由表。每个路由表都会包含到目的节点的距离。
