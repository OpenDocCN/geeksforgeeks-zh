...
*   `请求` – 这个原语被逐层(`N+1`)传输或发送到第 `N` 层，只是请求服务。
*   `指示` – 该原语由第 `N` 层返回到第(`N+1`)层，仅通知激活正在请求的服务或由第 `N` 层的服务发起的动作。
*   `回应` – 这个原语是由 `Layer (N+1)` 简单提供的，作为对指示原语的回复。它可能会确认或完成先前由指示原语调用的操作。
*   `确认` – 这个原语由第 `N` 层返回到请求(`N+1`)层，以简单地确认或完成请求原语之前调用的动作。

## 服务原语参数

部分服务原语需要参数。这些内容如下:

*   `连接.请求` – 发起实体执行此连接。请求。它只是指定并确定我们要连接的机器、所需的服务类型以及连接中使用的数据包或消息的最大大小。
*   `连接.指示` – 接收方得到这个连接指示，它只是指定了我们想要使用的呼叫者身份服务，比如 `FTP` 和 `Telnet` 等。，以及交换的数据包的最大大小。
*   `连接.响应` – 它只是指定它是想接受还是简单地拒绝正在请求的连接。
*   `连接.确认` – 它只是使用发出初始连接的实体来发现或确定发生了什么。请求。
