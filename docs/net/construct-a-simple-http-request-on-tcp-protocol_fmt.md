...
> 原文: [https://www.geeksforgeeks.org/construct-a-simple-http-request-on-tcp-protocol/](https://www.geeksforgeeks.org/construct-a-simple-http-request-on-tcp-protocol/)
...
*   从上图中，`HTTP` 通信的不同部分是:
    ...
    *   状态行和响应标题(`HTTP` 响应)
    *   响应正文(`HTTP` 响应)
*   试着弄清楚这些响应头的含义&它们的用途是什么——首先，请参见 `Last-Modified`、`Content-Length`、`Content-Type`。
...
**注–**
我们还可以使用 `Wireshark` 分析网络数据包，这将由您完成。