...
> 原文：[https://www.geeksforgeeks.org/difference-between-box-and-sugarsync/](https://www.geeksforgeeks.org/difference-between-box-and-sugarsync/)
...
SugarSync 是由 `J2 Global` 提供的云服务和文件同步服务。它能够主动同步计算机系统和其他设备中的文件，以便进行备份、访问和共享。`Windows`、`macOS`、`iOS`、`Android` 操作系统都支持。它由 `J2 Global` 于 `2009` 年推出。它不提供免费存储空间。它为付费用户提供有限和无限的存储空间。

## Box 和 SugarSync 的区别

| Box | SugarSync |
| --- | --- |
| 它属于 `Box Incorporation`。 | 它归 `J2 Global` 所有。 |
| 它于 `2005` 年推出。 | 它于 `2009` 年推出。 |
| 它是由 `Aaron Levie` 和 `Dylan Smith` 开发的。 | 由 `J2 Global` 开发。 |
