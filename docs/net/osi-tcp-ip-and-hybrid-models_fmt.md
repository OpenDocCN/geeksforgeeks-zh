...
*   更快的开发(由于每一层都相互独立，因此与旧的专有模型相比，`OSI` 模型中的开发速度更快)。
...
`混合模型`:
在现实世界中，我们使用 `OSI` 模型和 `TCP/IP` 模型的混合，称为混合模型。在混合模型中，应用层是 `OSI` 模型(类似于 `TCP/IP` 模型)的第 `7` 层、第 `6` 层和第 `5` 层的组合。其余层(第 `1`、`2`、`3` 和 `4` 层)与 `OSI` 模型相同。
