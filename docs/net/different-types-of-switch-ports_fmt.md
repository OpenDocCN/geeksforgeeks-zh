...
> 原文：[https://www.geeksforgeeks.org/different-types-of-switch-ports/](https://www.geeksforgeeks.org/different-types-of-switch-ports/)

网络设备通过交换机端口连接到交换机。`switch port`是插入数据电缆以连接设备的物理开口。交换机端口类型应根据需求进行配置，并考虑网络架构、速度和功能等因素。
...
*   A hybrid port is a connection on a switch for transferring data between one or more virtual `LAN`s.
...
*   It can simultaneously receive frames from one or more virtual `LAN`s.
