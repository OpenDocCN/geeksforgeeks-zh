...
> 原文: [https://www.geeksforgeeks.org/dfd-based-threat-modelling-set-2/](https://www.geeksforgeeks.org/dfd-based-threat-modelling-set-2/)
...
DFD 的视觉表示本质上是迭代的。因此，系统建模涉及到 DFD 不同层次的构建。这意味着要准确地反映这个体系，DFD 必须以等级的方式组织起来。遵循 DFD 中使用的各种形状：
...
1.  **识别威胁**
    威胁识别是构建安全系统的一个非常重要的步骤。识别威胁是一个 3 步过程，第一步涉及分析每个入口/出口点，第二步涉及识别入口/出口点发生的关键处理的性质和类型，第三步也是最后一步涉及思考和描述入口/出口点可能如何被攻击。识别威胁不是一个容易的过程。它涉及提出以下问题：
...
2.  **调查和分析威胁**
    识别威胁后的下一步是对已识别的威胁进行深入分析，以确定易受攻击的区域和有效的攻击路径。
