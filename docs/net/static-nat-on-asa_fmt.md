...
先决条件–[自适应安全设备(ASA)](https://www.geeksforgeeks.org/computer-network-adaptive-security-appliance-asa-features/) 、[网络地址转换(NAT)](https://www.geeksforgeeks.com/computer-network-network-address-translation-nat/)
...
这将说明将应用 `NAT` 的主机。
### 步骤-4:创建静态 NAT 语句–
此步骤将指定 `NAT` 应该发生的方向以及应该将私有 `IP` 地址转换到哪个 `IP` 地址，例如 `nat (DMZ, OUTSIDE) static 111.1.1.1` 此步骤说明当流量从 `DMZ` 流向 `OUTSIDE` 时，将进行静态 `NAT` 操作，并将 `IP` 地址(在 `network object` 命令中指定)转换到 `111.1.1.1`
...
第一条语句声明访问列表 `traffic_out` 在向内的方向应用于 `OUTSIDE` 接口。第二种说法是访问列表 `traffic_dmz` 向内应用于 `dmz` 接口。
