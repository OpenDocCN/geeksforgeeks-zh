...
> 原文:[https://www.geeksforgeeks.org/supernetting-in-network-layer/](https://www.geeksforgeeks.org/supernetting-in-network-layer/)
...
3.  **First IP address exactly divisible by total size:** When a binary number is divided by `2^n` then last n bits are the remainder. Hence in order to prove that first IP address is exactly divisible by while size of Supernet Network. You can check that if last n bits are 0 or not.

在给定的示例中，第一个 IP 是 `200.1.0.0`，超网的整体大小是 `4*2^8 = 2^10`。如果第一个 IP 地址的最后 10 位是零，那么 IP 将是可分的。
