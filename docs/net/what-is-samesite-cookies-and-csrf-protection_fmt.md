...
> 原文: [https://www.geeksforgeeks.org/what-is-samesite-cookies-and-csrf-protection/](https://www.geeksforgeeks.org/what-is-samesite-cookies-and-csrf-protection/)
...
用 `SameSite: strict` 设置的 `cookie` 将禁止 `cookie` 发送到所有第三方网站。只有当域与设置 `cookie` 的路径相同时，才会发送 `cookie`。

```
SameSite : none
```

设置有 `SameSite: none` 的 `Cookies` 将禁用基于相同站点的保护。该网站可能使用自己的 `CSRF` 保护机制。

```
SameSite : Lax
```

用 `SameSite: Lax` 设置的 `cookie` 将允许发送 `cookie` 仅用于顶级导航。当您没有添加 `SameSite` 属性时，`Lax` 被认为是默认行为。

因此，如果您使用 `SameSite: strict` 设置会话 `cookie`，即使没有专用的 `CSRF` `cookie`，从第三方网站生成的到您网站的链接也不会包含会话 `cookie`。因此，不可能对他们进行 `CSRF` 袭击。
