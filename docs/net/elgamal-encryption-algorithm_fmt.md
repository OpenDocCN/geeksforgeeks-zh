...
这个密码系统是基于在循环群中寻找**离散对数**的难度，也就是说即使我们知道 `g^a` 和 `g^k` ，也极难计算 `g^ak` 。
...
1.  Bob 生成公钥和私钥:
    *   Bob 选择一个非常大的数字 `q` 和一个循环组 `F_q`。
    *   从循环组 `F_q` 中，他选择任意元素 `g` 和一个元素 `a` ，使得 `gcd(a, q) = 1`。
    *   然后他计算 `h = g^a` 。
    *   Bob 将 `F` 、 `h = g^a` 、 `q` 和 `g` 发布为自己的公钥，并将 `a` 保留为私钥。
2.  爱丽丝使用鲍勃的公钥加密数据:
    *   Alice 从循环组 `F` 中选择一个元素 `k` ，使得 `gcd(k, q) = 1`。
    *   然后她计算 `p = g^k` 和 `s = h^k = g^{ak}`。
    *   她是 `m` 的倍数。
    *   然后她发送 `(p, M*s) = (g^k, M*s)`。
3.  鲍勃解密了这条消息:
    *   鲍勃计算 `s' = p^a = g^{ak}`。
    *   他将 `M*s` 除以 `s'`，得到 `M` 为 `s = s'`。
...
在这个密码系统中，原始消息 `M` 通过将 `g^{ak}` 相乘来屏蔽。取下面具，以 `g^k` 的形式给出线索。除非有人知道 `a` ，否则他将无法取回 `M` 。这是因为在一个循环组中寻找离散对数比较困难，而且要简化知道 `g^a` 和 `g^k` 还不足以计算 `g^{ak}` 。