...
> 原文:[https://www.geeksforgeeks.org/shannon-fano-algorithm-for-data-compression/](https://www.geeksforgeeks.org/shannon-fano-algorithm-for-data-compression/)
...
## 插入符号到节点
将符号 `ch` 插入到节点 `p[i].sym` 中：
```
p[i].sym += ch
```

## 输入符号概率
定义概率列表 `x`，并循环输入每个符号的概率：
```
...
## 完成概率分配与排序
调整最后一个节点的概率，并调用 `sortByProbability(n, p)` 函数进行排序：
```
i+=1
p[i].pro = 1 - total
# Sorting the symbols based on
...
## 查找香农代码
调用 `shannon(0, n - 1, p)` 函数计算香农编码：
```
shannon(0, n - 1, p)
```

## 显示代码
调用 `display(n, p)` 函数显示最终结果：
```
display(n, p)
```
