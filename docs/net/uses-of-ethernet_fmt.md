...
**以太网：**
以太网在 20 世纪 90 年代只是互联网的另一个名字。`IEEE`(电气和电子工程师协会) `802.2` 和 `IEEE 802.3` 标准将其定义为使用最广泛的局域网技术之一。数据链路层和物理层都由以太网控制。你可能知道，在 `Wi-Fi` 出现在我们的社会之前，互联网完全依赖以太网，它提供了快速可靠的连接。
...
*   以太网支持 `10`、`100`、`1000`、`10000`、`40000` 和 `100000` 兆位每秒(`Mbps`)的数据带宽。当以太网最初被开发时，带宽是以每兆比特(`Mbps`)来衡量的，但现在它们是以每秒千兆比特(`Gbps`)来衡量的。
...
在过去的几年里，`Wi-Fi` 越来越受欢迎。技术进步带来了速度更快、覆盖范围更广的无线网络。`Wi-Fi` 信号在任何给定时刻只能处理一定数量的设备。因为有这么多支持 `Wi-Fi` 的小工具在争夺信号，服务可能会很弱或不一致。在旧建筑中，无线网络投放区经常出现。当无线信号通过障碍时，它们会变得很差。其他常见的家用物品也会干扰无线信号，如无线电话、电视卫星接收器、微波炉等。
