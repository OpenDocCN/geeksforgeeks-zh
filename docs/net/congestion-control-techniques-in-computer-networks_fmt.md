...
> 原文：[https://www.geeksforgeeks.org/congestion-control-techniques-in-computer-networks/](https://www.geeksforgeeks.org/congestion-control-techniques-in-computer-networks/)
...
1.  **Backpressure：**
    Backpressure 是一种技术，其中一个拥塞节点停止接收来自上游节点的数据包。这可能导致上游节点或节点变得拥塞并拒绝接收来自更上层节点的数据。Backpressure 是一种节点到节点的拥塞控制技术，其传播方向与数据流方向相反。Backpressure 技术只能应用于虚电路，其中每个节点都知晓其上层的上游节点。

    ![Backpressure](img/d9115f40b2ad1b607e5f359e1b9fdc4e.png)

    在上图中，第三个节点拥塞，并停止接收数据包，因此第二个节点可能会因输出数据流变慢而变得拥塞。类似地，第一个节点可能会拥塞，并通知源节点减速。
