...
## 2. CloudMe
`CloudMe` 是由 `CloudMe AB` 提供的文件托管和文件存储服务。它提供各种服务，如文件同步、云存储和客户端软件。它是由`丹尼尔·阿瑟森`于 2011 年创建的。它最初被命名为 `iCloud`，但后来苹果公司获得了这个名字。它提供 3 GB 的可用存储空间。它为高级帐户提供了有限的存储空间。它最大的竞争对手是 `Dropbox`。

## Box 和 CloudMe 的区别

| Box | CloudMe |
| --- | --- |
| 它属于`Box Incorporation`。 | 归 `CloudMe AB` 所有。 |
