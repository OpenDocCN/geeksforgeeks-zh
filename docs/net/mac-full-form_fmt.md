...
*   MAC 地址的总长度（以字节计）是`6`（或`48`位）。根据 IEEE 802 标准，此地址以三种常用格式书写：
    *   六个两位十六进制数字，用连字符(-)分隔，如`45-67-89-AB-12-CD`。
    *   六个两位十六进制数字，用冒号(:)分隔，如`45:67:89:AB:DE:23`。
    *   四个三位数的十六进制数字，用点(.)分隔，如`ABCD.4567.1238`。
...
*   第二层上的一些设备是网卡、网桥和交换机。该层还负责局域网传输下物理层上的无差错数据传输。如果参考我们的开放系统互连(`OSI`)网络模型，我们会发现媒体访问控制协议子层中的 MAC 地址使用的是`数据链路层`。
