...
> 原文: [https://www.geeksforgeeks.org/gateway-load-balancing-protocol-glbp/](https://www.geeksforgeeks.org/gateway-load-balancing-protocol-glbp/)
...
3.  **实际虚拟转发器(AVF)**: 这些路由器包括单个 `GLBP` 组中的 `AVG`。这些实际上是负责转发数据后，他们被 `AVG` 分配的任务。如果 `AVG` 倒下了，其中一个 `AVF` 可以变成 `AVG`。
...
为了检测网关故障，`GLBP` 成员通过 `hello` 消息相互通信，每 3 秒钟向组播地址 `224.0.0.102` 发送一次。如果 `AVG` 失败，那么具有最高优先级的 `AVF` 将成为 `AVG`，即负责提供自动增值服务的媒体访问控制地址。

### 虚拟转发器冗余

就像在 [`HSRP`](https://www.geeksforgeeks.org/computer-networks-hot-standby-router-protocol-hsrp/) 一样，如果 `AVF` 的一个出现故障，那么同一个 `GLBP` 组中的另一个 `AVF` 将负责转发数据包。一个 `GLBP` 组最多可以有 4 台路由器。
