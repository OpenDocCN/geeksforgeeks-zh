...
> 原文: [https://www.geeksforgeeks.org/cisco-switch-configuration-basic-commands/](https://www.geeksforgeeks.org/cisco-switch-configuration-basic-commands/)

## 先决条件 – [交换机功能](https://www.geeksforgeeks.org/switch-functions-at-layer-2/)
...
- **启用密码 (`enable password`)**：用于保护特权模式。

```
GfgSwitch(config)#enable password GFGGFG
```

- **启用秘密密码 (`enable secret`)**：这也是用于保护特权模式，但不同的是在配置文件上会显示为密文（`***`）。

```
GfgSwitch(config)#enable secret GFGGFG
```

- **线路控制台密码 (`line console`)**：当一个人通过控制台端口进行访问时，会询问该密码。
...
- **线路 VTY 密码 (`line VTY`)**：当一个人想通过 VTY 线路（`telnet` 或 `ssh`）访问路由器时，就会询问这个密码。
