...
> 原文:[https://www . geeksforgeks . org/WEP 和-wpa 之间的区别/](https://www.geeksforgeeks.org/difference-between-wep-and-wpa/)
...
## 2. 保护无线电脑网络安全系统(WPA)
保护无线电脑网络安全系统(`WPA`)是一种用于保护无线网络安全的安全协议，旨在取代 `WEP` 协议。它是由无线网络联盟在 2003 年开发的。它旨在取代 `WEP` 协议，并使用 Rivest 密码 4 (`RC4`)和临时密钥完整性协议(`TKIP`)进行加密。`WPA` 密钥是 `256` 位密钥。
