...
> 原文: [https://www.geeksforgeeks.org/performance-comparison-of-wap-and-i-mode/](https://www.geeksforgeeks.org/performance-comparison-of-wap-and-i-mode/)
...
传输系统由两个独立的网络组成，即现有的电路交换移动电话网络和专门为 `I-mode` 服务构建的新的分组交换网络。语音模式利用电路交换网络，其账单按分钟收费。`I-mode` 利用了分组交换网络，并且一直处于开启状态。在 `I-mode` 下，连接时间不计费，而是对发送的每个数据包收费。
...
| 没有。 | 无线应用协议 | `I-mode` |
| :--- | :--- | :--- |
| 1. | `WAP` 使用的是作为 `XML` 子集的 `WML`。 | `I-mode` 使用的是 `HTML`，它是超文本标记语言的子集。 |
| 2. | 这是一个协议栈 | 这是一项服务 |
