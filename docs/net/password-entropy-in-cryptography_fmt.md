...
> 原文: [https://www.geeksforgeeks.org/password-entropy-in-cryptography/](https://www.geeksforgeeks.org/password-entropy-in-cryptography/)

[密码熵](https://www.geeksforgeeks.org/passwords-entropy-cracking/)是对密码强度或给定密码强度的度量。这是对密码抵御猜测或暴力攻击的有效性的一种度量。它决定了输入的密码是否通用，是否容易破解。它是通过知道字符集（`a-z`、`A-Z`、`0-9`、符号等）和创建的密码长度来计算的。它用每个字符的熵位来表示。
