...
> 原文: [https://www.geeksforgeeks.org/functionality-of-computer-network/](https://www.geeksforgeeks.org/functionality-of-computer-network/)
...
1.  **Error control:**
    计算机网络具有某些责任，例如将数据从一个设备传输到另一个设备，以及将数据从发送应用程序端到端传输到接收应用程序涉及许多步骤，每个步骤都可能出错。通过使用错误控制过程，我们可以确信传输和接收的数据是相同的。数据在传输过程中可能会损坏。必须检测并纠正错误以实现可靠通信。
...
有些功能在计算机网络中是可选的，可选功能不需要总是在计算机网络中。下面给出了一些可选功能：

1.  **Encryption and decryption:**
    解密和加密是一种安全方法，其中信息被编码，使得只有授权用户才能读取它。一些网络使用加密算法来生成密文，只有在解密后才能读取。
