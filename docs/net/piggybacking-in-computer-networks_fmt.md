...
> 原文：[https://www.geeksforgeeks.org/piggybacking-in-computer-networks/](https://www.geeksforgeeks.org/piggybacking-in-computer-networks/)

在本文中，我们将涵盖网络通信的概述，并主要关注网络中携带的概念。我们还将讨论在网络中使用携带的优缺点。最后，我们会看到结论。我们一个一个来讨论。
...
这种发送确认被暂时延迟的技术被称为`携带`。

![](img/e3e8b3b7c4f6aea20afd1f4afa595adf.png)

正如我们在图中所看到的，我们可以看到在携带的情况下，一条消息(确认+数据)代替了两条单独的消息。携带提高了双向协议的效率。

## 携带优势

1.  携带的主要优势是更好地利用可用的信道带宽。这是因为确认帧不需要单独发送。
2.  降低使用成本。
3.  改善数据传输的延迟。
...
**注意：** 为了避免帧传输的延迟和重播，携带使用非常短的持续时间计时器。

## 结论

关于这是合法还是非法活动存在争议，但携带仍然是 Wi-Fi 的阴暗面。印度的网络恐怖袭击清楚地提醒我们，我们无法控制世界任何地方发生的事件，也无法控制不安全的无线网络。因此，所有者和管理员有责任保护他们的无线连接。