...
> 原文: [https://www.geeksforgeeks.org/lacp-vs-pagp-whats-the-difference/](https://www.geeksforgeeks.org/lacp-vs-pagp-whats-the-difference/)
...
| 参数 | `LACP` | `PAGP` |
| :--- | :--- | :--- |
| **引入** | `LACP` 是在 2000 年左右引进的。 | `PAGP` 是在 20 世纪 90 年代初引进的。 |
| **全格式** | `LACP` 代表链路聚合控制协议。 | `PAGP` 代表端口聚合协议。 |
| **IEEE 标准** | `LACP` 的 `IEEE` 标准是 `IEEE 802.3ad`。 | 以太信道。 |
| **支持的供应商** | `LACP` 是一个开放标准，也就是说，得到了大多数供应商的支持。 | `PAGP` 是思科专有的；即 `PAGP` 只能在思科设备之间使用。 |
| **配置/端口模式** | `LACP` 有两种模式，即主动模式（端口通过交换 `LACP` 数据包启动与其他端口的协商。）和被动模式（端口会回复收到的 `LACP` 数据包，但不会启动 `LACP`。）。 | `PAGP` 有两种模式，即自动模式（接口回复收到的 `PAGP` 数据包，但不启动 `PAGP` 协商。）和理想模式（接口通过发送 `PAGP` 数据包启动与其他接口的协商。）。 |
| **组播地址** | `LACP` 的组播地址是 `01-80-C2-00-00-02`。 | `PAGP` [组播](https://www.geeksforgeeks.org/multicasting-in-computer-network/)地址为 `01-00-0C-CC-CC-CC`。 |
| **由**支持 | `802.3ad` 和以太网通道支持 `LACP`。 | `PAGP` 由以太信道支持。 |
| **配置** | `switch(config-if-range)#channel-group 1 mode`（后接主动模式或被动模式）。 | `switch(config-if-range)#channel-group 1 mode`（后接自动模式或理想模式）。 |
| **跨栈支持** | `LACP` 可以支持跨栈。 | `PAGP` 不支持跨栈。 |