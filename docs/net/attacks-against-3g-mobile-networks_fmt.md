...
> 原文: [https://www.geeksforgeeks.org/attacks-against-3g-mobile-networks/](https://www.geeksforgeeks.org/attacks-against-3g-mobile-networks/)
...
*   `骷髅头木马` – 它被设计用来感染运行塞班移动操作系统的 `Series 60` 手机。
*   `Cabir 蠕虫` – 它是手机中最初已知的蠕虫；它攻击移动设备，然后扫描剩余的设备，寻找它的副本，启动移动设备，检测使用蓝牙。
*   `蚊子木马` – 蚊子木马是针对 `60` 系列手机的 `Mosquitos` 手机游戏的一个被破坏的变种。
*   `Brador 木马` – 通过在 `Windows` 的启动文件中制作 `svchost.exe` 文件感染 `Windows Mobile` 操作系统，让你可以完全访问手机。包括电子邮件附件在内的标准蠕虫传播媒介与此可执行文件兼容。
*   `Lasco Worm` – 最初发布于 `2005` 年，旨在攻击基于 `Symbian` 的 `PDA` 和手机。`Lasco` 是基于 `Cabir` 的代码构建的，并使用蓝牙技术进行复制。
