...
3.  **路由器实现或协议 (是否启用了 `QoS`):**
    固有地，路由器的内置协议将尝试为连接提供最佳用户体验所需的带宽。但如前所述，是正在执行的活动定义了带宽分配。遵循的一般趋势（尽管可能因其他因素而异）是：

    ```
    多媒体流 > `VOIP` > 下载 > 电子邮件 > 网页浏览
    ```
...
*   **选择正确的 `SSID` 名称/隐藏您的 `SSID`:**
    不要使用路由器附带的默认 `SSID` 名称。我们应该将其更改为不同的名称。黑客更有可能寻找带有默认 `SSID` 的网络，因为这表明用户没有花更多时间来保护他的网络。
...
*   **`MAC` 地址过滤:**
    转到 `Wireless->Wireless MAC Filtering` 页面，单击 `Add New` 按钮。键入您要允许或拒绝访问路由器的 `MAC` 地址，并为此项提供描述。状态应为 `Enabled`，最后，单击 `Save` 按钮。
