...
> 原文：[https://www.geeksforgeeks.org/uses-of-mobile-authenticators/](https://www.geeksforgeeks.org/uses-of-mobile-authenticators/)
...
3.  **物理位置在手机上–**
    `IMEI` 号码也可在设备上找到。它可能位于手机的电池、`SIM` 卡或内存卡槽 compartment 上。如果电池是可拆卸的，那么移除电池就会露出 `IMEI` 号或 `MEID` 号。

## 在手机上使用 IMEI、MEID 和 ESN 号码
...
4.  **购买二手手机–**
    这些号码也用于购买二手手机的情况。如果它们有“坏的 `MEID` 或坏的 `IMEI` 或坏的 `ESN` 号码”，这意味着这不能连接到任何类型的无线服务。这是非常重要的一点或注意事项，好像这样的设备是买来的，它不能用于任何一种互联网或通话。这款手机只能用于它所包含的硬件零件组件（可以重复使用）。
5.  **人为错误预防–**
