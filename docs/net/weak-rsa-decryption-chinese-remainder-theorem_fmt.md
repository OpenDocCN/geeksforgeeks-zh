...
> 原文: [https://www.geeksforgeeks.org/weak-rsa-decryption-chinese-remainder-theorem/](https://www.geeksforgeeks.org/weak-rsa-decryption-chinese-remainder-theorem/)
...
给定整数 `c`，`e`，`p` 和 `q`，求 `m` 使得 `c = pow(m, e) mod (p * q)` (弱整数的 RSA 解密)。
...
(1) 计算 `e` 的模逆。我们可以利用下面的等式，`d = e^(-1)(modλ(n))`，其中 `λ` 是 **Carmichael 全能性函数**。
**读取** : [卡迈克尔功能](https://en.wikipedia.org/wiki/Carmichael_function)
(2) 计算 `m = pow(c, d) mod(p * q)`
(3) 我们可以通过使用如下函数中定义的中国剩余定理来更快地执行该计算。关于中国剩余定理的进一步阅读可以在 [RSA(密码系统)](https://en.wikipedia.org/wiki/RSA_(cryptosystem)) 完成。
