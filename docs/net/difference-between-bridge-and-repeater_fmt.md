...
> 原文:[https://www . geeksforgeks . org/网桥和中继器之间的区别/](https://www.geeksforgeeks.org/difference-between-bridge-and-repeater/)
...
## 中继器:
中继器是一种电子设备。它是用于扩展局域网的硬件设备。中继器仅在物理层运行，即`现场视察模型`的第一层。它再生弱信号并增加网络的范围。通过使用中继器，网络的功能保持不变。交换机可以用作中继器，但集线器不能用作中继器。
...
| 没有。 | 桥 | 中继器 |
| --- | --- | --- |
| 1. | 网桥在`现场视察模型`的数据链路层运行。 | 中继器在`现场视察模型`的物理层运行。 |
