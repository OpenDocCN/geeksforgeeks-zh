...
> 原文:[https://www . geeksforgeks . org/wasabi 和-mimedia 的区别/](https://www.geeksforgeeks.org/difference-between-wasabi-and-mimedia/)

## 1. Wasabi
是由 `Wasabi` 技术公司提供的文件托管和云存储服务。它是一种对象存储服务。它于 2017 年由 `Wasabi` 技术公司推出。它不提供免费存储空间。它为付费用户提供无限的最大存储空间。它也被称为芥末热存储或芥末热云存储。
