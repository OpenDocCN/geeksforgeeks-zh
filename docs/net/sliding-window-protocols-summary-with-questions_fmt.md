...
> 原文: [https://www.geeksforgeeks.org/sliding-window-protocols-summary-with-questions/](https://www.geeksforgeeks.org/sliding-window-protocols-summary-with-questions/)
...
*   **例-1。** 在停止和等待协议中，每 4 个第个数据包丢失，我们总共需要发送 10 个数据包，那么发送所有数据包需要多少次传输？
*   **Explanation –**
...
*   **例-3。** 在 GBN 发送器窗口大小= 10，`T_p`= 49.5 毫秒 & `T_T`= 1 毫秒。给定带宽= 1000 bps，协议的效率和吞吐量是多少？
*   **解释–**
    效率= N/(1+2a)，N = 10(给定)，a = `T_p`/`T_T`= 49.5
...
*   **例-5。** 在 SR `W_s` = 5 中，我们发送了 10 个数据包，其中每丢失 5 个数据包，找到重传次数了吗？
*   **Explanation –**
...
**解释–**
    给定，K 位，对于 S&W `W_s`= 1 和 `W_r` = 1
    对于 GBN，`W_s` = 2<sup>K</sup> -1 和 `W_r` = 1
    对于 SR，`W_s` = 2<sup>K-1</sup> 和 `W_r`
