...
> 原文：[https://www.geeksforgeeks.org/transport-layer-security-tls-handshake/](https://www.geeksforgeeks.org/transport-layer-security-tls-handshake/)
...
2.  然后服务器用 `Server Hello` 消息(选择客户端支持的顶级域名系统的最高版本)进行响应，然后从 `ClientHello` 消息的列表中选择一个 `密码套件`。服务器还传输其 `数字证书` 和最终的 `服务器登录消息`。
...
4.  一旦接收到共享的秘密，服务器就用 `变更密码规范` 和加密的 `完成消息` 进行回复。

`会话密钥` 是 TLS 会话中使用的共享对称加密密钥，用于加密来回发送的数据。