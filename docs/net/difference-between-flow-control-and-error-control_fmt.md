...
> 原文: [https://www.geeksforgeeks.org/difference-between-flow-control-and-error-control/](https://www.geeksforgeeks.org/difference-between-flow-control-and-error-control/)

## 1. 流量控制

它是数据链路层的一个重要功能。它指的是在等待接收方确认之前，告知发送方可以传输多少数据的一套程序。
...
| 2. | 对于流量控制，有两种方法: 基于反馈的流量控制和基于速率的流量控制。 | 检测数据错误的方法有: 校验和、循环冗余校验和奇偶校验。纠正数据错误的方法有: 汉明码、二进制卷积码、里德-所罗门码、低密度奇偶校验码。 |
