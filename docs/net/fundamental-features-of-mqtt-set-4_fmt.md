...
> 原文: [https://www.geeksforgeeks.org/fundamental-features-of-mqtt-set-4/](https://www.geeksforgeeks.org/fundamental-features-of-mqtt-set-4/)
...
在上面的程序中，ID 为 1 的客户端以 5 秒的保活定时器连接到代理。因此，发布者客户端(ID:1)和代理之间将每 5 秒交换一次 `PINGREQ` 和 `PINGRESP`。
...
![Mosquitto Illustration of Keep-Alive Messages](img/0f1288e9790d07caf55730f3d1ef3197.png)

如果发布者客户端不发布任何消息，那么每 5 秒后，将只交换 `PINGREQ` 和 `PINGRESP`。这里如果订阅者客户端也指定了保活期，那么代理将与 ID 分别为 1 和 2 的两个客户端交换这些 `PING` 消息。

![Mosquitto Illustration of Keep-Alive Messages](img/398c02842fade295e8694e1e3a271193.png)