...
人们将以更相关的方式连接到互联网，并将生成数据并与设备交互，不仅通过 `Mobile’s/Tablet’s`、`PC’s` 和社交网络，还通过放置在人体皮肤上或缝入衣服的传感器，这些传感器将提供一个人的生命体征。这样，人们自己将成为互联网上的节点。

一个很好的例子是耐克的可穿戴健身带，它可以读取一个人的生命体征、运动服装和嵌入芯片的齿轮，这些芯片可以跟踪运动员的表现。

## Things

事物和物理项目，如传感器、工业设备、消费产品、企业资产，将被连接到互联网或相互连接，同时也从其周围环境获取信息，将更加情境感知、更加认知、更加智能，通常被称为物联网。
...
不仅仅是收集原始数据，这些连接的设备将向各自的服务器发送更高级别、经过更多处理的数据，以便进行更快的评估或更智能的决策。
