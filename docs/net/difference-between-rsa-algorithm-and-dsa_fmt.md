...
> 原文：[https://www.geeksforgeeks.org/difference-between-rsa-algorithm-and-dsa/](https://www.geeksforgeeks.org/difference-between-rsa-algorithm-and-dsa/)

## 1. Rivest-Shamir-Adleman (RSA) 算法

`RSA` 代表 **Rivest-Shamir-Adleman**。它是一种用于安全数据传输的密码系统。在 `RSA` 算法中，加密密钥是公共的，而解密密钥是私有的。该算法是基于这样一个数学事实，即分解两个大素数的乘积并不容易。它是由 **罗恩·瑞文斯特**、**阿迪·萨莫尔** 和 **伦纳德·阿德曼** 在 1977 年开发的。
...
| `密钥生成`速度较慢。 | 而与 `RSA` 相比，它在`密钥生成`方面更快。 |
