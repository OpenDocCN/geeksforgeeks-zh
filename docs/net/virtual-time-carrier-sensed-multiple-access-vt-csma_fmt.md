...
> 原文：[https://www.geeksforgeeks.org/virtual-time-carrier-sensed-multiple-access-vt-csma/](https://www.geeksforgeeks.org/virtual-time-carrier-sensed-multiple-access-vt-csma/)

`VT-CSMA` 主要用于硬实时系统。它用于在实时系统中同时有效地传输信号，以避免冲突。在实时系统中，不能使用 `CSMA`，但可以使用虚拟时间 `CSMA` 进行信号传输。
