...
> 原文：[https://www.geeksforgeeks.org/unicast-routing-link-state-routing/](https://www.geeksforgeeks.org/unicast-routing-link-state-routing/)
...
单播–单播是指从单个发送方到单个接收方的传输。它是发送方和接收方之间的点对点通信。有各种各样的单播协议，如 `TCP`、`HTTP` 等。
...
*   **步骤-4：** 节点重复步骤 2 和步骤 3，直到所有节点都添加到树中。
...
`OSPF` 是一个非常复杂的协议。它使用五种不同类型的消息。这些措施如下：
