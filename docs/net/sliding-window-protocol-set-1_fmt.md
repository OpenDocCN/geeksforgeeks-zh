...
> 原文:[https://www.geeksforgeeks.org/sliding-window-protocol-set-1/](https://www.geeksforgeeks.org/sliding-window-protocol-set-1/)
...
在下图中，发送方发送完数据包 0 后，将立即发送数据包 1、2、3。0 的确认将在 `2 * 1.5 = 3 毫秒`后到达。在停止和等待中，在时间 `1+2 * 1.5 = 4 毫秒`内，我们只传输了一个数据包。这里我们保留一个我们已经传输但尚未确认的数据包的窗口。
...
但是有时协议头中的位数是预先定义的。报头中序列号字段的大小也将决定我们在总周期时间内可以发送的最大数据包数量。如果 `N` 是报头中序列号字段的大小(以位为单位)，那么我们可以有 `2^N` 个序列号。
...
本文由`普伦珠尔·阿胡佳`供稿。如果您发现任何不正确的地方，或者您想分享更多关于上述主题的信息，请写评论