...
> 原文：[https://www.geeksforgeeks.org/digital-signatures-certificates/](https://www.geeksforgeeks.org/digital-signatures-certificates/)
...
`公钥`——大家都知道的密钥。A 的公钥是 7，这个信息大家都知道。
...
1.  通过对消息应用散列函数来计算消息摘要，然后使用发送者的私钥对消息摘要进行加密以形成数字签名。（数字签名 = `加密(发送者的私钥, 消息摘要)` 和 消息摘要 = `消息摘要算法(消息)`）。
