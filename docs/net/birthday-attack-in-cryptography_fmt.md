...
> 原文: [https://www.geeksforgeeks.org/birthday-attack-in-cryptography/](https://www.geeksforgeeks.org/birthday-attack-in-cryptography/)
...
让 **`H: M => {0,1}^n`** 成为一个哈希函数（ **`|M| >> 2^n`** ）。

下面是一个通用算法，用于在时间 **`O(2^(n/2))`** 哈希中查找冲突。

### 算法

1.  在 `M` 中选择 `2^(n/2)` 随机消息：`m_1, m_2, ..., m_(n/2)`
2.  对于 `i = 1, 2, ..., 2^(n/2)` 计算 `t_i = H(m_i) => {0,1}^n`
3.  寻找碰撞（`t_i = t_j`）。如果没有找到，返回第 1 步。
...
数字签名可能容易受到生日攻击。消息 `m` 通常通过首先计算 `H(m)` 来签名，其中 `H` 是加密散列函数，然后使用一些秘密密钥来签名 `H(m)`。

假设爱丽丝想骗鲍勃签一份欺诈性的合同。爱丽丝准备了一份公平合同 `m` 和一份欺诈合同 `m'`。然后，她找到了一些位置，在这些位置上 `m` 可以在不改变含义的情况下进行更改，例如在句子后面插入逗号、空行、一个空格对两个空格、替换同义词等。通过结合这些变化，她可以在 `m` 上创建大量的变体，这些变体都是公平的合同。
