...
> 原文: [https://www.geeksforgeeks.org/controlled-access-protocols-in-computer-network/](https://www.geeksforgeeks.org/controlled-access-protocols-in-computer-network/)
...
*   一般来说，第 `I` 站可以通过在第 `I` 时隙中插入 `1` 比特来宣布它有帧要发送。检查完所有 `N` 个时隙后，每个站都知道哪些站希望发送。
...
让 `T_poll` 为轮询时间，`T_t` 为数据传输所需时间。然后，
...
*   在令牌环中，令牌从一个站点传递到环中的另一个相邻站点，而在令牌总线的情况下，每个站点使用总线以某种预定义的顺序将令牌发送到下一个站点。
