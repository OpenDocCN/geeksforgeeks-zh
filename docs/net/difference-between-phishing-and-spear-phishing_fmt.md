...
> 原文：`https://www.geeksforgeeks.org/difference-between-phishing-and-spear-phishing/`

有几种类型的电子邮件攻击被攻击者用来从计算机系统或网络中窃取机密信息。机密信息包括登录凭据、银行卡详细信息或任何其他敏感数据。`网络钓鱼`和`鱼叉式网络钓鱼`也是这类电子邮件攻击。
...
`鱼叉式网络钓鱼`是一种针对特定个人或组织的电子邮件攻击。在`鱼叉式网络钓鱼`中，攻击者欺骗目标点击恶意链接，安装恶意代码，让攻击者从目标系统或网络中检索敏感信息。
