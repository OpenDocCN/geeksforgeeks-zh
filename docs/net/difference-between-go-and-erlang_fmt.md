...
> 原文：[https://www.geeksforgeeks.org/difference-between-go-and-erlang/](https://www.geeksforgeeks.org/difference-between-go-and-erlang/)
...
*   `Go` 得到了科技巨头谷歌的支持，`Go` 也被谷歌使用，这足以让人认为它不会很快走向任何地方。
*   程序可以直接在网上编辑和运行。

### 缺点

*   `GO` 没有泛型。这降低了代码的可重用性。
*   图书馆对 `Go` 的支持非常薄弱。这是因为语言很年轻。
*   `GO` 没有图形用户界面库。
*   `GO` 中的依赖管理器非常有问题。
...
| 03. | `Go` 的并发性相对较好。 | `Erlang` 的并发性不如 `GO`。 |
| 04. | `GO` 不是容错的首选。 | `Erlang` 因其容错性而成为首选。 |
| 05. | `Go` 是一门相对年轻的语言，因此它的社区很小。 | `Erlang` 有一个好社区支持了 20 多年。 |
| 06. | `Go` 很容易学。 | `Erlang` 有更大的代码行。 |
| 07. | `GO` 因其速度和语法而成为首选。 | `Erlang` 比 `GO` 慢。 |
| 08. | `GO` 用于服务器开发。 | `Erlang` 用于开发可扩展的系统。 |
| 09. | `GO` 是可移植的，生成的二进制文件很小。 | `Erlang` 不便携，体积大。 |