...
> 原文：[https://www.geeksforgeeks.org/types-of-digital-signature-attacks/](https://www.geeksforgeeks.org/types-of-digital-signature-attacks/)
...
在仅密钥攻击中，`A` 的公钥对每个人都可用，`C` 利用这一事实，试图重新创建 `A` 的签名，并对 `A` 不打算做的文档或消息进行数字签名。这将对不可否认的消息的认证造成很大威胁，因为 `A` 不能否认签署它。