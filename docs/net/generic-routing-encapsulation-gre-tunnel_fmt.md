...
> 原文: [https://www.geeksforgeeks.org/generic-routing-encapsulation-gre-tunnel/](https://www.geeksforgeeks.org/generic-routing-encapsulation-gre-tunnel/)

`通用路由封装`是一种将 IP 数据包封装在隐藏原始 IP 数据包的 GRE 报头中的方法。此外，一个名为传递头的新头被添加到包含新的源地址和目的地址的 GRE 头之上。
