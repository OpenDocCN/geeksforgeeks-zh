...
> 原文: [https://www.geeksforgeeks.org/implementation-diffie-hellman-algorithm/](https://www.geeksforgeeks.org/implementation-diffie-hellman-algorithm/)
...
| 可用公钥 = `P`, `G` | 可用公钥 = `P`, `G` |
| 所选私钥 = `a` | 选择私钥 = `b` |
| 生成的密钥 = `x = G^a mod P` | 生成的密钥 = `y = G^b mod P` |
| 交换生成的密钥 | |
| 收到密钥 = `y` | 收到的密钥 = `x` |
| 生成的密钥 = `k_a = y^a mod P` | 生成的密钥 = `k_b = x^b mod P` |
| 代数上，可以证明 `k_a = k_b` | |
| 用户现在有一个对称密钥要加密 | |
