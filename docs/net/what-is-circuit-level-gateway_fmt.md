...
**电路级网关**防火墙有助于提供 `UDP` 和使用该连接的 `TCP` 之间的安全性。它还充当可信客户端或服务器与不可信主机之间的握手设备，反之亦然。

![Circuit-level Gateway](img/828f9ca4a23ec546649f5bfe899c9183.png)

电路级网关

通常，这些电路级网关在 OSI 模型的会话层工作。电路级网关通过数据包之间的握手来确定会话请求是否被确认。
...
这个电路级网关设置可以借助两个 `TCP` 连接来完成，建立网关与内部主机 `TCP` 和 `TCP` 用户的外部主机之间的连接。
...
2.  它在 `OSI` 模型的会话层工作，或者在 `TCP/IP` 的应用层和传输层之间工作。
3.  它隐藏了他们保护的私有网络的信息。
4.  这是一个独立的系统。
