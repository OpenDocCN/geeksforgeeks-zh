...
> 原文: [https://www.geeksforgeeks.org/internet-control-message-protocol-icmp/](https://www.geeksforgeeks.org/internet-control-message-protocol-icmp/)
...
**例如：** 如果主机试图通过路由器发送数据，`R1` 和 `R1` 通过路由器 `R2` 发送数据，主机与 `R2` 之间有一条直接的路径。然后，`R1` 将发送重定向消息，通知主机有直接通过 `R2` 到达目的地的最佳途径。然后，主机将目的地的数据包直接发送到 `R2`。
