...
> 原文：[https://www.geeksforgeeks.org/difference-between-store-and-forward-switching-and-cut-through-switching/](https://www.geeksforgeeks.org/difference-between-store-and-forward-switching-and-cut-through-switching/)
...
| 特性 | 存储转发交换 | 直通交换 |
| :--- | :--- | :--- |
| **转发时机** | 交换设备在转发数据包之前等待接收整个帧。 | 一旦接收到目的地址，交换设备就转发数据包，而不等待整个帧被接收。 |
| **错误检查** | 它支持错误检查，在转发数据包之前会丢弃冲突/坏帧。 | 没有错误检查技术。 |
| **错误检测机制** | 它根据数据帧的 FCS 位检查错误。 | 它依赖更高级别的协议来检测错误。 |
| **帧存储** | 帧存储在交换设备的缓冲存储器中。 | 帧不存储在交换设备中。 |
| **类型** | 它没有进一步分为不同的类型。 | 直通交换进一步分为`快速帧转发`和`无碎片`。 |
| **延迟** | 它的延迟率很高，因为设备在将数据包发送到目的地之前会等待整个帧被接收。 | 它的延迟率较低，因为设备在将数据包发送到目的地之前不会等待整个帧被接收。 |
| **安全性** | 它更安全，因为它不会将损坏的帧转发到目的地。 | 它不太安全，因为它有时会将损坏的帧转发到目的地。 |
| **等待时间** | 它有很长的等待时间，因为交换设备在转发整个帧之前要等待整个帧被接收。 | 它的等待时间很短，因为它一收到目的地址就转发帧。 |
| **路由决策** | 它不限制交换机中的路由或仲裁决策。 | 它限制了源和目的地之间交换机的路由或仲裁决策。 |
| **存储需求** | 它有更多存储需求的网络。 | 它对存储的要求更低。 |
| **应用示例** | 例如：电信网络或需要高移动性的跨网络传输。 | 例如：通过光纤通道通信和用于 SCSI 传输的低延迟传输。 |