...
> 原文：[https://www.geeksforgeeks.org/multi-protocol-label-switching-mpls/](https://www.geeksforgeeks.org/multi-protocol-label-switching-mpls/)
...
1.  **`标签`**：该字段长 20 位，可以取值 `b/w 0&2^20–1`。
...
| `push`、`pop` 和 `swap` | `LSR` 分别添加、移除和交换标签的动作。 |

[多协议标签交换（MPLS）路由](https://www.geeksforgeeks.org/how-does-multiprotocol-label-switching-mpls-routing-work/)

## MPLS 中的转发

`LSR` 接收 `IP` 数据包 `CE`，并在第 3 层和第 2 层之间添加 MPLS 报头，这意味着它封装了链路层，即第 2 层帧。这个特性允许 `LSR` 支持接收包含来自不同协议（如帧中继、城域以太网等）的帧的数据包，这就是为什么它被称为多协议。
...
这个 `LSR` 再次重复了这个过程。这样，`LSR` 学习路线，并将此信息添加到 `标签转发信息库（LFIB）` 中。现在，如果任何一个对等体接收到目的地为子网 1 的数据包，那么通过查看标签和 `LFIB`，`LSR` 可以轻松地转发 `IP` 数据包。
