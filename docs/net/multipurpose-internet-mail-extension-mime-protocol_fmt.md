...
> 原文:[https://www . geesforgeks . org/多用-互联网-邮件-扩展名-mime-协议/](https://www.geeksforgeeks.org/multipurpose-internet-mail-extension-mime-protocol/)
...
2.  **`Content-Type`–**消息正文中使用的数据类型。它们有不同的类型，如文本数据(纯文本、HTML)、音频内容或视频内容。
3.  **`Content-Transfer-Encoding`–**它定义了用于编码消息的方法。比如 7 位编码、8 位编码等。
4.  **`Content-ID`–**用于唯一标识消息。
5.  **`Content-Description`–**定义身体实际上是图像、视频还是音频。