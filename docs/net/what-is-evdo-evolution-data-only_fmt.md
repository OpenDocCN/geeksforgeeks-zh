...
使用 EVDO，用户可以在笔记本电脑上获得高速互联网，即使他们正在旅行，并且没有连接到无线网络或局域网。EVDO 是一种用于基于 `CDMA` 的 3G 网络的技术，通过移动设备提供高速互联网服务。它成本相对较低，容量大。
...
*   `Release 0`，也称为 (`r0`)，这是最初在现实世界中部署的原始版本，提供高达 `2.4 Mbps` 的数据速率，平均为 `300-600 kbps`。
*   `Revision a`，也称为 (`RA`)，是一种为 `1xEV-DV Revision D` 标准设计的数据技术，它是一种更快的技术，有助于改善延迟。它支持 `VoIP` 和视频通话等功能。
