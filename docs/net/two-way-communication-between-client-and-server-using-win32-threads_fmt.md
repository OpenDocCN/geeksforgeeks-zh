...
> 原文: [https://www.geeksforgeeks.org/two-way-communication-between-client-and-server-using-win32-threads/](https://www.geeksforgeeks.org/two-way-communication-between-client-and-server-using-win32-threads/)
...
DWORD WINAPI clientReceive(LPVOID lpParam)
{
    ...
    while (true) {
        ...
        // Print the message given by server that was stored in buffer
...
DWORD WINAPI clientSend(LPVOID lpParam)
{
    ...
    while (true) {
        ...
        // Input message client wants to send to server
        gets(buffer);
