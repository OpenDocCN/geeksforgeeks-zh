...
> 原文：[https://www.geeksforgeeks.org/platform-as-a-service-paas-and-its-types/](https://www.geeksforgeeks.org/platform-as-a-service-paas-and-its-types/)

先决条件 – [基于云的服务](https://www.geeksforgeeks.org/cloud-based-services/)

**平台即服务（PaaS）** 是一种分布式计算模型，其中外部供应商通过互联网向客户分配设备和编程仪器。通常，这些是应用程序改进所必需的。PaaS 供应商在自己的框架上有设备和编程。因此，它将设计者从引入内部设备和编程来创建或运行另一个应用程序中解放出来。
...
对 PaaS 项目的内部更改也是一个预期问题。例如，如果平台即服务供应商停止支持特定的编程语言或限制使用其他高级设备，对客户的影响可能是麻烦和有问题的。客户应该遵循 `PaaS` 供应商的管理指南，了解供应商的安排如何影响他们的条件和能力。
...
### 1. Public PaaS
适用于公共云。`Public PaaS` 允许客户控制编程扩展，而云供应商处理应用程序所需的所有其他关键 IT 部分的分配，包括操作系统、数据库、服务器和存储框架系统。
...
### 2. Private PaaS
旨在提供公共通行证的敏捷性，同时保持安全、一致性、优势和私有安全社区的便利。一个私有通行证通常作为小工具或编程分布在客户的防火墙中，该防火墙通常在组织场所的服务器场中维护。私有 `PaaS` 可以在框架上开发并在组织特定的私有云中运行。

私有骰子允许一个组织更好地服务开发者，改进内部资产使用，并减少许多组织面临的昂贵云入口。同样，私有骰子允许开发者在遵守严格的安全和隐私要求的情况下运行和处理他们组织的应用程序。

### 3. Hybrid PaaS
结合了 `Public PaaS` 和 `Private PaaS` 的服务，具有 `Public PaaS` 提供的无边界限制的便利性以及 `Private PaaS` 拥有内部框架的成本效益。混合 `PaaS` 利用混合云。

### 4. Communication PaaS (CPaaS)
基于云的平台，允许开发者为其应用程序添加实时通信功能，而无需后端基础设施和接口。通常，实时通信发生在专门为此任务构建的应用程序中。模型是 `Skype`、`FaceTime`、`WhatsApp` 和传统电话。

`CPaaS` 提供了一个完全开发的框架，用于创建实时通信功能，而无需开发者构建自己的框架，包括基于标准的应用程序编程接口、编程工具、预构建应用程序和测试代码。

`CPaaS` 供应商通过提供支持和产品文档来帮助客户改进过程。一些供应商提供编程开发包以及库，帮助在各种桌面和移动平台上构建应用程序。选择使用 `CPaaS` 的开发团队可以节省基础设施、人力资源和市场的时间。

### 5. Mobile PaaS (MPaaS)
是一个用于移动应用程序配置的集成开发环境。在 `MPaaS` 中，不需要编码技能。`MPaaS` 通过互联网浏览器分发，主要支持公共云、私有云和本地存储。服务通常按月租用，具体取决于支持的设备和设施数量。
...
`PaaS` 的另一个用途是在 `DevOps` 仪器中。`PaaS` 提供针对公司项目改进排练定制的应用程序生命周期板亮点和明确亮点。`DevOps` 模型允许团队插入基于云的持久连接工具，包括刷新，而不会产生惰性时间。此外，遵循级联模式的公司可以利用日常维护中使用的类似支持来运行更新。
