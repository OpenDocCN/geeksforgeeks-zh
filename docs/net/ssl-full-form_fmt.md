...
2.  **第二子层–**
    `SSL` 协议栈的第二层和顶层协议包含了 `SSL Handshake Protocol`、`SSL Shift Cipher Protocol` 和 `SSL Warning Protocol`。`SSL` 协议栈的第二层建立在 `SSL` 记录协议之上，负责维护到应用协议（如 `HTTP`）的安全连接。这三个顶层协议提供客户端到服务器的会话控制、加密参数控制和安全的 `SSL` 消息传输。
