...
`ENQ/确认`是线路规程的程序，通常用于确定网络上的哪个设备能够发起或开始传输数据或消息，以及接收器是否准备好并能够接收数据。如果主机具有相似且相等的等级，那么主机或站中的任何一个都可以启动传输过程。
...
线路规程中的`轮询/选择`方法基本上适用于某些拓扑，其中一个设备被视为主站，而其他设备被视为次站。可以看到多点连接，而不是两个，即多点系统必须协调几个节点。
