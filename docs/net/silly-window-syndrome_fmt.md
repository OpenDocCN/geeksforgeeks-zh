...
**傻窗综合征**是由于`TCP`实现不佳而出现的问题。它降低了`TCP`的性能，使数据传输效率极低。之所以这么叫问题，是因为:
...
假设考虑接收器无法处理所有传入数据的情况。在这种情况下，接收器将广告小窗口大小。该过程继续，窗口大小变得越来越小。当一个阶段重复通告`1 字节`的窗口大小时，它就到达了。这使得接收过程缓慢且低效。这个问题的解决方案就是克拉克的解决方案。

**克拉克的解决方案建议:**

1.  接收器不应发送`1 字节`的窗口更新。
