...
这是一个小型拓扑，其中有主机 A (`192.168.1.1/24`)、主机 B (`192.168.1.2/24`)、主机 C (`192.168.1.3/24`)、主机 D (`192.168.1.4/24`)和两个交换机。它将通过从主机 A `ping` 主机 C 来了解交换机的媒体访问控制地址学习过程。最初，两台交换机的媒体访问控制表都只有另一台交换机的条目。
...
**注意 –** 交换机 0 已经有了交换机 1 的条目。交换机 0 将帧广播回交换机 1 和电脑。主机 B 将丢弃该帧，因为它的目的地不是它。
