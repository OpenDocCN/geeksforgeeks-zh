...
> 原文: [https://www.geeksforgeeks.org/differences-between-pop3-and-imap/](https://www.geeksforgeeks.org/differences-between-pop3-and-imap/)

`POP3`（邮局协议 3）和 `IMAP`（互联网消息访问协议）都是 `MAA`（消息访问代理），这两个协议都用于从邮件服务器检索消息到接收者系统。这两种协议都适用于垃圾邮件和病毒过滤器。`IMAP` 比 `POP3` 更灵活复杂。
