...
## Dropbox :
`Dropbox` 是由美国公司 `Dropbox Incorporation` 提供的文件托管服务。它提供云存储、文件同步、个人云和客户端软件。它成立于 2007 年。`Dropbox` 是由两位麻省理工学院的学生 `德鲁·休斯顿` 和 `阿拉斯·菲尔多西` 开发的。它仅提供 2 GB 的可用存储空间。它是最早的云存储服务之一。
