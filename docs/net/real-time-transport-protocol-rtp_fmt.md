...
> 原文: [https://www.geeksforgeks.org/real-time-transport-protocol-rtp/](https://www.geeksforgeks.org/real-time-transport-protocol-rtp/)

一种协议被设计用来处理互联网的实时流量(如音频和视频)，被称为**实时传输协议(RTP)**。`RTP`必须与`UDP`一起使用。它没有任何传送机制，如多播或端口号。`RTP`支持不同格式的文件，如`MPEG`和`MJPEG`。它对数据包延迟非常敏感，对数据包丢失不太敏感。

## RTP的历史

这个协议是由互联网工程任务组(`IETF`)的四个成员开发的:

1.  Henning Schulzrinne (哥伦比亚大学)
2.  Stephen Casner ( Packet Design )
3.  Frederick ( Blue Coat Systems )
4.  Van Jacobson ( Packet Design )
