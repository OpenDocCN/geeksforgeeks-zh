...
> 原文：[https://www.geeksforgeeks.org/difference-between-domain-and-workgroup/](https://www.geeksforgeeks.org/difference-between-domain-and-workgroup/)
...
| 6. | 域名由`域控制器`根据`IP 地址`提供。 | 在工作组中，分配名称不依赖于任何硬件组件和服务器。 |
| 7. | 数据可以从`集中存储`在域中恢复。 | 由于每个设备的本地存储，数据恢复在工作组中是不可能的。 |
