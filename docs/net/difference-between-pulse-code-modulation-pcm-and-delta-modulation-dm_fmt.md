...
## **脉码调制(PCM):**
是将模拟信号重新加工成数字信号的技术。`PCM`具有良好或可感知的信噪比。对于传输，脉冲编码调制需要高发射机带宽。`PCM`技术分为三个要素，最初是在提供端的传输，其次是在传输路径和接收端的再生。
...
| 2. | 在`PCM`中，反馈不存在于发射器或接收器中。 | 而在`DM`中，反馈存在于发射机中。 |
...
| 9. | 在`PCM`中，信号需要编码器和解码器。 | 在`DM`中，信号可以进行调制和解调。 |
| 10. | `PCM`主要用于视频电话和音频电话。 | `DM`主要用于演讲和图像。 |