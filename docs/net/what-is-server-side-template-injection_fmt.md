...
> 原文: [https://www.geeksforgeeks.org/what-is-server-side-template-injection/](https://www.geeksforgeeks.org/what-is-server-side-template-injection/)
...
然后这些模板引擎处理模板文件，模板文件支持将动态信息填充到网页中。当一个 `HTTP 请求`进来时，模板引擎生成 HTML 输出回复。
...
*   **代码上下文检测–**
    在这种情况下，有效载荷可用于获取错误响应。例如，可以使用 `HTTP GET` 参数在模板语句中插入变量。
