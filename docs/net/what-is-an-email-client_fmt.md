...
3.  **域类型**：它告诉给定的电子邮件地址是否像商业（`.com`）或教育类（`.edu`）和其他一些定制类型。

比如`joy@gmail.com`是邮箱地址，它有三部分，`joy`是用户名，`@gmail`显示域名，`.com`显示一个域类型。
...
4.  **对于有用数据的备份目的**：有用文件的数据备份一直是大家的大问题，但是现在把有用数据邮寄给自己就很容易保存了，这样的话就在服务器上，随时随地都可以访问。
