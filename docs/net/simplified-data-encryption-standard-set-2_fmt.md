...
> 原文: [https://www.geeksforgeeks.org/simplified-data-encryption-standard-set-2/](https://www.geeksforgeeks.org/simplified-data-encryption-standard-set-2/)
...
// 这个函数执行核心操作，如扩展
// 然后与所需密钥进行异或，接着是 S0 和 S1
// 替换、P4 置换，再次异或。我们在
// 加密过程中使用了这个函数 2 次（密钥-1 和 密钥-2），
// 在解密过程中使用了 2 次（密钥-2 和 密钥-1）。
...
public static void main(String[] args) {
    ...
    for (int i = 0; i < 8; i++) // 打印明文
        System.out.print(plaintext[i] + " ");

    int[] ciphertext = obj.encryption(plaintext);

    System.out.println();
    System.out.println("Your cipher Text is :"); // 打印密文
    for (int i = 0; i < 8; i++)
    ...
    System.out.println();
    System.out.println("Your decrypted Text is :"); // 打印解密文本
    for (int i = 0; i < 8; i++)
