...
> 原文：[https://www.geeksforgeeks.org/longest-prefix-matching-in-routers/](https://www.geeksforgeeks.org/longest-prefix-matching-in-routers/)
...
## 示例 1
路由器使用转发表条目转发数据包。传入数据包的网络地址可能匹配多个条目。路由器如何解决这个问题？
(A) 转发条目与传入数据包最长前缀匹配的路由器
(B) 转发数据包到所有网络地址匹配的路由器。
(C) 丢弃数据包。
...
## 示例 2
无类域间路由（CIDR）收到地址为 `131.23.151.76` 的数据包。路由器的路由表包含以下条目：（GATE CS 2015）
