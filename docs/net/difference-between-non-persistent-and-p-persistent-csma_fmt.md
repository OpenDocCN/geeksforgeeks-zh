...
> 原文: [https://www.geeksforgeeks.org/difference-between-non-persistent-and-p-persistent-csma/](https://www.geeksforgeeks.org/difference-between-non-persistent-and-p-persistent-csma/)

先决条件 – [载波侦听多路访问(CSMA)](https://www.geeksforgeeks.org/carrier-sense-multiple-access-csma/)

## 1. 非持久性 CSMA
在`非持久性 CSMA`中，有帧要发送的电台只对频道有感觉。在空闲信道的情况下，它会立即将帧发送到该信道。如果发现信道繁忙，它将等待一段固定的时间，并再次检测站的状态是空闲还是繁忙。在这种方法中，当该站检测到前一次传输结束时，它不会仅仅出于捕获的目的而立即感测该信道。这种方法减少了冲突的机会，但降低了网络的效率。
...
| 延迟低负载 | 它比`1-持久`长，因为它只在信道繁忙时随机检查。 | 当概率`p`较小时，它较大，因为站不会总是在信道空闲状态下发送。 |
