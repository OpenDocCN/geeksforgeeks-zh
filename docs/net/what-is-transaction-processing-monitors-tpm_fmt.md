...
> 原文：[https://www.geeksforgeeks.org/what-is-transaction-processing-monitors-tpm/](https://www.geeksforgeeks.org/what-is-transaction-processing-monitors-tpm/)
...
最古老的中间件形式之一来自 IBM，它旨在为在线事务处理应用程序提供丰富的运行时环境。然后，新版本的 `tp 监视器`出现了，并在客户端-服务器上工作，因为它们在当时是最好的，它们在今天的世界中仍然是相关的，例如银行交易的处理。
...
- 一旦事务完成，`TP 监视器`保证消息被完美地传递。
- 很多 `TP 监视器`都有锁定和恢复功能，让应用服务器自己实现 [`ACID` 属性](https://www.geeksforgeeks.org/acid-properties-in-dbms/)。
