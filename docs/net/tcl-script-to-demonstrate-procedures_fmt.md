...
> 原文：`https://www.geeksforgeeks.org/tcl-script-to-demonstrate-procedures/`
...
我们先来定义一下我们的程序。我们使用 `proc` 关键字来实现。创建程序 `addnumbers {}`、`subnumbers {}`、`mulnumbers {}`、`divnumbers {}` 和 `modnumbers {}` 来分别计算两个数的和、差、积、除和模。
