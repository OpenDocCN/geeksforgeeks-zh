...
> 原文：`https://www.geeksforgeeks.org/types-of-memory-on-a-cisco-device/`
...
闪存是一种非易失性存储存储器，可以以“扇区”或“块”为单位进行写入或编程。闪存是 `EEPROM`（电子可擦除可编程只读存储器），这意味着当电源移除时，它可以保留其内容，但是其内容可以通过施加更高电压的短脉冲在字节级快速擦除和重写。这被称为闪存擦除，因此得名。闪存目前既太贵又太慢，不能用作主存储器。它广泛用于汽车收音机、手机、数码相机、掌上电脑、固态硬盘、平板电脑和打印机。
