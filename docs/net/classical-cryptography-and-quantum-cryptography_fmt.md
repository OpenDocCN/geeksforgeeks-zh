...
> 原文：[https://www.geeksforgeeks.org/classical-cryptography-and-quantum-cryptography/](https://www.geeksforgeeks.org/classical-cryptography-and-quantum-cryptography/)
...
在对称密码学中，使用单个密钥对数据进行加密和解密。此加密密钥是私钥。这种加密技术的限制是，此私钥必须仅在授权的发送方和接收方之间分发。

![](img/60bf462105b2d9a0aad622a2d7e0f1c7.png)

### Asymmetric Cryptography

在非对称密码学中，使用一对密钥，即公钥和私钥，进行加密和解密。发送方可以使用其公钥加密数据，在接收端，接收方可以使用其私钥解密数据。这种技术克服了密钥分发的问题。
...
*   `QKD`支持的最大距离约为250公里，通过引导介质的速度为16 bps。