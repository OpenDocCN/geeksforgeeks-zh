...
> 原文：[https://www.geeksforgeeks.org/what-is-authentication-tokens-in-network-security/](https://www.geeksforgeeks.org/what-is-authentication-tokens-in-network-security/)
...
每个这样的设备（认证令牌）都预编程有一个唯一的数字，称为`随机种子`或`种子`。此种子确保身份验证令牌（设备）生成的输出是唯一的。身份验证令牌是双因素身份验证的一个示例，因为令牌本身受到某些个人识别码的保护。
