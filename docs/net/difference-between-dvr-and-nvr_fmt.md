...
> 原文：[https://www.geeksforgeeks.org/difference-between-dvr-and-nvr/](https://www.geeksforgeeks.org/difference-between-dvr-and-nvr/)
...
网络录像机（`NVR`）主要用于通过网络无线记录来自 `IP` 摄像机的数据。它与分辨率从 `2MP` 到 `12MP` 或更高的 `IP` 相机配对。在该系统中，无线或单根以太网电缆用于供电和数据。它提供远程服务，直到哪一个网络接入可用。在这种情况下，信号的稳定性取决于互联网的工作。
...
| 序号 | 数字视频录像机（`DVR`） | 网络录像机（`NVR`） |
| :--- | :--- | :--- |
| 01. | `DVR` 代表数字录像机。 | `NVR` 代表网络录像机。 |
