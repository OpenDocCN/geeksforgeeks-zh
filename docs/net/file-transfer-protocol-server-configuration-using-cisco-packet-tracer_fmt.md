...
> 原文：[https://www.geeksforgeeks.org/file-transfer-protocol-server-configuration-using-cisco-packet-tracer/](https://www.geeksforgeeks.org/file-transfer-protocol-server-configuration-using-cisco-packet-tracer/)

在本文中，我们将在`cisco packet tracer`中设计一个文件传输协议服务器配置，并检查从远程PC上传和下载文件的连通性。
...
2.  选择`Router0`并转到配置。
3.  通过将IP地址分配为`192.168.1.1`并将子网掩码分配为`255.255.255.0`来配置`GigabitEthernet0/0`，并打开端口状态。
4.  通过将IP地址分配为`10.0.0.1`并将子网掩码分配为`255.0.0.0`来配置`GigabitEthernet0/1`，并打开端口状态。
...
1.  从终端设备中选择两台`PC-PT`类型的个人电脑，并拖放到工作区。
2.  选择`PC0`，转到配置中的`FastEthernet0`，并为`PC0`分配IP地址和子网掩码，分别为`192.168.1.2`、`255.255.255.0`。
3.  选择`PC1`，转到配置中的`FastEthernet0`，并将`PC1`的IP地址和子网掩码分配为`192.168.1.3`、`255.255.255.0`。
...
3.  转到`FastEthernet0`，将IP地址和子网掩码分配为`10.0.0.2`、`255.0.0.0`。
4.  转到服务并打开文件传输协议服务。
5.  转到用户设置并创建用户名和密码。
...
2.  使用铜直通电缆将`PC0`的`FastEthernet0`端口连接到`Switch0`的`FastEthernet0/1`端口。
3.  使用铜质直通电缆将`PC1`的`FastEthernet0`端口连接到`Switch0`的`FastEthernet0/2`端口。
4.  使用铜直通电缆将`Switch0`的`FastEthernet0/3`端口连接到`Router0`的`GigabitEthernet0/0`端口。
5.  使用铜直通电缆将`Router0`的`GigabitEthernet0/1`端口连接到`Server0`的`FastEthernet0`。
