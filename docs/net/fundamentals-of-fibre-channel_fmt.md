...
> 原文：[https://www.geeksforgeeks.org/fundamentals-of-fibre-channel/](https://www.geeksforgeeks.org/fundamentals-of-fibre-channel/)
...
> **注**：拓扑是指网络中节点或其他设备的物理/逻辑排列。

## 基本端口详情

要了解所提到的拓扑，需要知道以下基本端口详情：

| **端口** | **类型** | **关联拓扑** |
| :--- | :--- | :--- |
| `N_port` | 节点 | 点对点或交换结构。 |
| `NL_port` | 节点 | 连接到仲裁环路的节点。 |
| `F_port` | 交换机 | 交换结构端口。 |
| `FL_port` | 交换机 | 连接到仲裁环路的交换机。 |
| `L_port` | 环路 | 仲裁环路上的集线器端口。 |
| `LC_port` | 环路 | 链路控制设施是一种硬件设施，它绑定到链路的每一端，并控制信息的传输和接收。 |
| `E_port` | 节点 | 此交换机间扩展端口用于连接到另一台交换机的 `E` 端口，以构建更大的交换结构。 |
| 光纤通道交换机 | 交换机 | 互连服务器和存储端口的网络交换机。 |
...
*   这是一种高速光纤通道拓扑，其中光纤通道端口/集线器使用仲裁来建立点对点电路，并防止多个端口/集线器同时发送帧。
...
*   这就是现在非常流行的拓扑结构。
*   光纤通道环境中的交换机网络称为交换结构。
*   一个节点上的端口可以与连接到同一交换结构的其他节点上的端口通信。使用交换结构拓扑，一次可以对许多连接发出警报。
*   交换结构提供的任意对任意连接服务和对等通信服务是光纤通道体系结构的基础。光纤通道可以同时支持通道和网络协议。