...
> 原文：[https://www.geeksforgeeks.org/classification-of-computer-network-on-basis-of-architecture/](https://www.geeksforgeeks.org/classification-of-computer-network-on-basis-of-architecture/)
...
在 `P2P` (`Peer-to-Peer`) 网络中，“`peers`”通常代表计算机系统。这些 `peers` 通过互联网相互连接。文件可以在这些网络上的系统之间直接共享，而不需要中央服务器。可以说，`P2P` 网络上的每台计算机通常既充当文件服务器，也充当客户端。在这种架构中，系统通常被分解为多个具有相同和等效能力、功能和责任的计算节点。
...
`CSN` (`Client/Server Network`) 是一种计算机网络，其中一个集中式且功能强大的计算机（通常称为服务器）是枢纽，许多功能较弱的个人计算机或工作站（通常称为客户端）都连接到它。这是一种客户端连接到服务器以共享或使用资源的系统类型。这些服务器通常被认为是系统的核心。
