...
> 原文:[https://www . geeksforgeks . org/广播和多播的区别/](https://www.geeksforgeeks.org/difference-between-broadcast-and-multicast/)
...
组播有一个/多个发送方和一个/多个接收方参与数据传输流量。在多播中，流量位于单播和广播的边界之间。它是服务器数据流的直接单一副本，然后被模拟并路由到请求它的主机。`IP`组播需要一些其他协议的支持，如互联网组管理协议、组播路由等。而且在有类`IP`寻址中，`D类`是为多播组保留的。
