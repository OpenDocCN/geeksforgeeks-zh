...
[校验和](https://www.geeksforgeeks.org/error-detection-code-checksum/)是检测数据错误的常用方法。这种方法比其他错误检测方法更可靠。这种方法在发送端使用`校验和生成器`，在接收端使用`校验和检查器`。

## 2. CRC

CRC 或循环冗余校验是检测错误的错误检测方法，该方法被上层协议使用。它包含发送方和接收方的`多项式生成器`。多项式发生器的类型为 `x^3 + x^2 + x + 1`。
