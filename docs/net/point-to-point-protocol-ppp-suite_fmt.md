...
> 原文: [https://www.geeksforgeeks.org/point-to-point-protocol-ppp-suite/](https://www.geeksforgeeks.org/point-to-point-protocol-ppp-suite/)
...
### 桥接控制协议(BCP)
`BCP` 主要负责配置、启用、禁用或协商以及控制或维护 `PP` 连接两端的桥接控制模块。它类似于 `IPCP`，但不是路由，而是初始化桥接。

### 互联网协议控制协议(IPCP)
该协议特别要求在连接的每一端配置、启用和禁用 `IP` 协议模块。路由器还交换 `IPCP`，只是为了协商特定于 `IP` 的选项。
