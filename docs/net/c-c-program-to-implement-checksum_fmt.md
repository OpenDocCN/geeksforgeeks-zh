...
> 原文: [https://www.geeksforgeeks.org/c-c-program-to-implement-checksum/](https://www.geeksforgeeks.org/c-c-program-to-implement-checksum/)
...
> **输入:** `send_message = “10101111”`，`rec_message = “10101101”`，`block_size = 8`
> **输出:** 错误
> **解释:** 由于 `send_message` 和 `rec_message` 中的第 7 位不同，最终校验和值不等于零，表示传输过程中出现了一些错误。
>
> **输入:** `send_message = “10000101100011100101001101101101”`，`rec_message = “100001011000111001010011101101”`，`block_size = 8`
> **输出:** 无错误

**方法:** 给定的问题可以分为以下两部分:

*   生成**发件人信息**的[校验和](https://www.geeksforgeeks.org/error-detection-in-computer-networks/)值可以通过以下步骤完成:
    *   将消息分成给定的 `block_size` 的[二进制字符串](https://www.geeksforgeeks.org/tag/binary-string/)。
...
*   检查**收到的消息**(即 `rec_message + senders_checksum`)的值是否等于 `0`。
