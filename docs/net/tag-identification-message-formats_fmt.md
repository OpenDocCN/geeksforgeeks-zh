...
> 原文：[https://www.geeksforgeeks.org/tag-identification-message-formats/](https://www.geeksforgeeks.org/tag-identification-message-formats/)
...
`q` 是这个命令最重要的参数。从 0 到 `2^Q - 1`，定义标签将响应的时隙范围。为了保护消息字段，循环冗余校验（`CRC`）是存在的。可以看出，在 5 位时，它比大多数 `CRC` 短。而查询消息也比大多数数据包短得多。
