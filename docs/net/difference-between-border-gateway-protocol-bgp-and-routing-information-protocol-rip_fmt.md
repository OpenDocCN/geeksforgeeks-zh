...
> 原文: [https://www.geeksforgeeks.org/difference-between-border-gateway-protocol-bgp-and-routing-information-protocol-rip/](https://www.geeksforgeeks.org/difference-between-border-gateway-protocol-bgp-and-routing-information-protocol-rip/)

## 边界网关协议(BGP)
在 `BGP` 中，使用传输控制协议。这是一种网状拓扑或设计。它通过两个独立的网络(自治系统或 `AS`)交换路由信息来工作。两台路由器同意交换如何到达特定 `IP` 范围的信息。`BGP` 使用[传输控制协议(`TCP`)](https://www.geeksforgeeks.org/tcp-ip-model/) 端口 `179`。
