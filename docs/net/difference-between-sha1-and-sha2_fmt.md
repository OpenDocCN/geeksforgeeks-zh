...
> 原文: [https://www.geeksforgeeks.org/difference-between-sha1-and-sha2/](https://www.geeksforgeeks.org/difference-between-sha1-and-sha2/)
...
`SHA2` 也是一个密码散列函数，由美国国家安全局设计。它是使用 `Merkle-Damgard` 结构从单向压缩函数构建的。所使用的压缩函数是使用分类分组密码的戴维斯-迈耶结构构建的。它于 2001 年首次出版。它是 `SH1` 的继承者。
