...
> 原文: [https://www.geeksforgeeks.org/difference-between-layer-2-and-layer-3-switches/](https://www.geeksforgeeks.org/difference-between-layer-2-and-layer-3-switches/)
...
第 2 层交换机工作在 `OSI` 模型的第 2 层，即数据链路层，并使用存储与目的端口相关联的设备的 `mac` 地址的 `MAC` 地址表向该端口发送“帧”。

## 第三层交换机

第三层交换机工作在 `OSI` 模型的第三层，即网络层，它通过使用 `IP` 地址来路由数据包，在虚拟局域网中被广泛使用。

## 对比表格

| 第 2 层交换机 | 第三层交换机 |
| --- | --- |
| 在 `OSI` 模型的第 2 层(数据链路)上运行。 | 在 `OSI` 模型的第 3 层(网络层)上运行。 |
