...
> 原文: [https://www.geeksforgeeks.org/hamming-code-implementation-in-c-cpp/](https://www.geeksforgeeks.org/hamming-code-implementation-in-c-cpp/)
...
**方法:** 思路是先用 `1` 初始化 `r` 后每次递增 `1` 找到可以找到的冗余比特数，而 `2^r` 小于 `(m + r + 1)` ，其中 `m` 为输入报文中的比特数。按照以下步骤解决问题:

*   通过 `1` 初始化 `r` ，并通过 `1` 递增，直到 `2^r` 小于 `m+r+1` 。
*   初始化一个大小为 `r + m` 的向量`hammingCode`，这将是输出消息的长度。
*   通过从 `i = 0` 遍历到 `r–1`并设置`hammingCode[2^i–1]=-1`，用 `-1` 初始化所有冗余位的位置。然后将输入信息位按 `0 <= j < (r + m)` 的顺序放置在`hammingCode[j]`不是 `-1` 的所有位置。
*   用 `0` 初始化一个变量`one_count`存储 1 的个数，然后从 `i = 0` 遍历到`(r+m–1)`。
*   如果当前位，即`hammingCode[i]`不是 `-1` ，则通过从 `j = i+2` 到 `r+m` 遍历`one_count`加 `1` 如果 `(j & (1 << log2(i+1)))`，则在`log2(i+1)`位置找到包含设置位的消息位。
*   如果对于索引 `i` ， `one_count` 为偶数，则设置`hammingCode[i] = 0` 否则设置`hammingCode[i] = 1` 。
*   遍历后，打印`hammingCode[]` 向量作为输出消息。
...
**时间复杂度:** `O((M + R)^2)` 其中 `M` 为输入消息中的位数，`R` 为冗余位数
**辅助空间:** `O(M + R)`