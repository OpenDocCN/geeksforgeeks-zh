...
答案 (D)
A 和 B 的 `IP` 地址最后八位是 `113` (`01110001`) 和 `91` (`01011011`)。选项 (D) 中的网络掩码在最后一个八位字节中设置了前三位。如果网络掩码设置了前 3 位，那么这些位在 A 和 B 中必须相同，但事实并非如此。简而言之，我们可以说选项 (D) 不是有效的网络掩码，因为用 A 和 B 的地址对其进行二进制 `&` 不会给出相同的网络地址。它必须是相同的地址，因为 A 和 B 在同一个网络上。详见[本](http://support.microsoft.com/kb/164015)。
