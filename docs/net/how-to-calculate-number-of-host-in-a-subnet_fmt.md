...
> 原文：[https://www.geeksforgeeks.org/how-to-calculate-number-of-host-in-a-subnet/](https://www.geeksforgeeks.org/how-to-calculate-number-of-host-in-a-subnet/)
...
`IPv4` 的每个 `IP` 地址由 32 位组成，它分为 4 个八位字节，1 个八位字节 = 8 位，看第一个八位字节找到给定 `IP` 地址的类别。下表给出了每个类别的范围。
...
**示例：** 如果给定的 `IP` 地址是 `64.0.0.8`，通过用各自的二进制值替换每个八位字节，将其转换为二进制格式，那么给定 `IP` 地址的二进制格式将是 `01000000.00000000.00000000.00001000`。

现在取 `IP` 地址所属的默认掩码值（来自上表），把那个默认掩码值转换成它的二进制格式，`A` 类的默认掩码是 `255.0.0.0` 转换成二进制格式会是 `11111111.00000000.00000000.00000000`。
现在执行 **与** **操作**：
...
| `B` | 16,384 | 65,536 | 65,534 |
| `C` | 2<sup>21</sup> | 256 | 254 |
| `D` | 没有网络 | 没有主机 | – |
| `E` | 没有网络 | 没有主机 | – |

**注：** `D` 类预留组播、群邮件/广播，`D` 类可能的 `IP` 地址为 2<sup>28</sup>。
`E` 类留作实验和研究/军事用途，`E` 类中可能的 `IP` 地址为 2<sup>28</sup>。
...
```
  ...
  -----------------------------------------------
  网络地址      => 00001001.00000000.00000000.00000000 => 9.0.0.0
```
给定 `IP` 地址的网络 `IP` 地址为 **`9.0.0.0`**。
**每个网络的主机数量为 2<sup>24</sup>–2**。
...
```
  ...
  -------------------------------------------------
  网络地址       => 11001001.00010100.00011110.00000000 => 201.20.30.0
```
