...
2.  `DREAD`
    `DREAD` 是为了威胁建模而提出的，但由于评级不一致，它在 2008 年被微软放弃。它目前被 OpenStack 和许多其他公司使用。它使用五个类别为风险评级安全威胁提供了一个助记符。这些类别是:
