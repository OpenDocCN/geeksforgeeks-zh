...
> 原文：[https://www.geeksforgeeks.org/nat-hole-punching-in-computer-network/](https://www.geeksforgeeks.org/nat-hole-punching-in-computer-network/)
...
1.  发送者请求连接到接收者
2.  `s` 将 `A` 的地址发送到 `B`，将 `B` 的地址发送到 `A`
3.  `A` 向 `B` 发送垃圾消息，`B` 向 `A` 发送垃圾消息(两者都被各自的 NAT 丢弃)
4.  重复步骤 3。
5.  连接已建立。
