...
> 原文: [https://www.geeksforgeeks.org/irda-infrared-data-association/](https://www.geeksforgeeks.org/irda-infrared-data-association/)
...
### 会话层
`IrOBEX`、`IrLAN`、`IrBUS`、`IrMC`、`IrTran`、`IrComm` 都在这一层执行不同的任务。
...
*   `IrComm` 协议通过模拟串行（例如 `RS232CCOM`）或并行端口。