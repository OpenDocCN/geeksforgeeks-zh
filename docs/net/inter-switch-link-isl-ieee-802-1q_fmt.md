...
> 原文:[https://www.geeksforgeeks.org/inter-switch-link-isl-ieee-802-1q/](https://www.geeksforgeeks.org/inter-switch-link-isl-ieee-802-1q/)
...
`802.1Q` 是 `IEEE` 标准的帧标记方法。如果用户想要在思科交换机链路和不同品牌交换机链路之间建立中继，则必须使用 `802.1Q` 封装才能使中继工作。
...
在 4 个字节中，最重要的是 12 位 `VLAN`。此字段说明帧所属的 VLAN 编号。VLAN 可以从 1 到 4094 不等，即支持 4094 个虚拟局域网，因为 `VLAN 0` 和 `VLAN 4095` 是保留的虚拟局域网。`802.1Q` 支持本地 `Vlan` 的概念，即流量将不会被标记到该 `Vlan`。
