...
**Chaumian Blinding** 是一种进行交易的技术，甚至不向批准交易的中心人物透露交易双方的所有人、交易时间或交易内容。Chaumian Blinding 是由一位名叫 **David Chaum** 的美国密码学家开发的。他在他的论文《无法追踪支付的 T4 盲签名》中发表了这项技术。
...
*   一个交换函数 `C` 和它的逆函数 `C'`，都只有提供者知道，在这个例子中，是阿德里安。通勤函数 `C` 及其逆 `C'` 的创建方式如下：`C'(S(C(x))) -> S(x)`
*   冗余检查谓词 `r`。
