...
> 原文：[https://www.geeksforgeeks.org/difference-between-unicast-broadcast-and-multicast-in-computer-network/](https://www.geeksforgeeks.org/difference-between-unicast-broadcast-and-multicast-in-computer-network/)
...
在多播中，一个或多个发送者和一个或多个接收者参与数据传输业务。在这种方法中，流量位于单播（一对一）和广播（一对一）的边界之间。多播允许服务器直接复制数据流，然后模拟并路由到请求它的主机。IP 组播的工作需要其他一些协议的支持，比如 `IGMP(互联网组管理协议)`，`组播路由`。同样在有类 IP 寻址中`D类`是为多播组保留的。
...
[单播、广播和组播的区别](https://serverfault.com/questions/279482/what-is-the-difference-between-unicast-anycast-broadcast-and-multicast-traffic)
