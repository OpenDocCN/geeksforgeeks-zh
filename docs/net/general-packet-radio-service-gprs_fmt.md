...
> 原文: [https://www.geeksforgeeks.org/general-packet-radio-service-gprs/](https://www.geeksforgeeks.org/general-packet-radio-service-gprs/)

`GPRS` 是一个扩展的全球移动通信系统。它基本上是 `2G` 和 `3G` 蜂窝通信网络的全球移动通信系统上面向分组的移动数据标准。由于之前的 `CDPD`，`GPRS` 是由欧洲电信标准协会(`ETSI`)建立的，并且 `I-模式` 分组交换小区取得了进展。
