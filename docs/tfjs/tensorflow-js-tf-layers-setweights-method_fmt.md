...
> 原文: [https://www.geeksforgeeks.org/tensorflow-js-tf-layers-setweights-method/](https://www.geeksforgeeks.org/tensorflow-js-tf-layers-setweights-method/)
...
*   `weights`: 是输入张量的指定列表。它是 `tf.Tensor[]` 类型。其中，数组的数量及其形状应等于所用层的规定权重的尺寸数量。换句话说，它必须等于 `getWeights()` 方法的结果。
...
这里，使用 `truncatedNormal()` 方法来创建 `tf.Tensor`，连同从截断正态分布采样的值一起，使用 `zeros()` 方法来创建 `tf.Tensor` 以及设置为 0 的所有元素，和 `getWeights()` 方法用于打印使用 `setWeights()` 方法设置的权重。
...
这里，使用 `ones()` 方法来创建 `tf.Tensor` 以及设置为 1 的所有元素。
