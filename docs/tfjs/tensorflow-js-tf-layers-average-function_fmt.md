...
> 原文: [https://www.geeksforgeeks.org/tensorflow-js-tf-layers-average-function/](https://www.geeksforgeeks.org/tensorflow-js-tf-layers-average-function/)
...
*   `inputShape`: 它是一个可选参数，用于创建输入图层，取值如数字和 null。
...
*   `trainable`: 这是一个可选参数，用于确定所提供的输入图层是否更新。它接受布尔值。
*   `weights`: 拥有图层的起始权重。它也是一个可选参数。
*   `inputDType`: 是用于输入数据类型的可选参数。像 `dtype` 一样，它也支持它的所有值。
...
**参考:** [https://js.tensorflow.org/api/latest/#layers.average](https://js.tensorflow.org/api/latest/#layers.average)