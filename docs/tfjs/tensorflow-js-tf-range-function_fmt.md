...
> 原文: [https://www.geeksforgeeks.org/tensorflow-js-tf-range-function/](https://www.geeksforgeeks.org/tensorflow-js-tf-range-function/)

TensorFlow.js 是谷歌开发的开源库，用于在浏览器或节点环境中运行机器学习模型和深度学习神经网络。

`tf.range()` 用于创建一个新的 `tf.Tensor1D`，填充了借助 `start`、`stop` 和 `step` 参数提供的范围内的数字。
...
*   `stop`: 是表示区间结束数的整数 `stop` 值，不包含该值。
*   `step`: 默认为 1 或 -1 的整数增量。这是一个可选参数。
...
**参考:** [https://js.tensorflow.org/api/latest/#range](https://js.tensorflow.org/api/latest/#range)