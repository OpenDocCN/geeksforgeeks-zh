...
*   `labels`: 指定的目标标签应该是有利于类的基于零的整数。它有形状`[numExamples]`。其中，`numExamples`是合并实例的度量。它可以是`tf.Tensor`1D、`TypedArray`或数组。
*   `predictions`: 所述的预测类应该是有利于类的基于零的整数。它应该具有与所述标签相当的形状。它可以是`tf.Tensor`1D、`TypedArray`或数组。
