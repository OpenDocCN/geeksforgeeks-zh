...
`tf.train.adagrad()` 函数我们用来创建一个使用自适应梯度算法的自适应梯度优化器。
...
*   `learningRate`: 指定自适应梯度下降算法将使用的学习速率。
*   `initialaccumulatorvalue`: 指定累加器的初始值。肯定是正面的。

**返回值:** 返回一个 `tf.AdagradOptimizer`。

**示例 1:** 通过学习系数 `x`，`y` 来拟合函数 `f(x, y) = x + y`。
...
**参考资料:** [https://js.tensorflow.org/api/1.0.0/#train.adagrad](https://js.tensorflow.org/api/1.0.0/#train.adagrad)