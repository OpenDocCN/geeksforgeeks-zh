...
*   `x`: 是要计算指数值减 1 的张量输入，可以是 `tf.Tensor`，或 `TypedArray`，或数组。
...
**参考:** https://js.tensorflow.org/api/latest/#expm1