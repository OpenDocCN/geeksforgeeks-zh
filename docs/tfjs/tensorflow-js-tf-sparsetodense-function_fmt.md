...
> 来源: [https://www.geeksforgeeks.org/tensorflow-js-tf-sparse-function/](https://www.geeksforgeeks.org/tensorflow-js-tf-sparse-function/)

TensorFlow.js 是谷歌开发的开源库，用于在浏览器或节点环境下运行机器学习模型和深度学习神经网络。

`tf.sparseToDense()` 函数用于将指定的稀疏表示转换为稠密张量。如果给定的索引重复出现，最终值将与该索引的所有值相加。
...
*   `sparseIndices`: 它是 `int32` 数据类型的 0 维、1 维或 2 维张量。在这里，`sparseValues[i]` 放在 `sparseIndices[i]` 处，其中 `i` 是索引值。
*   `sparseValues`: 它是一个 0 维或 1 维的值张量，对应于每一行 `sparseValues`。
*   `outputShape`: 是转换后的稠密输出张量的形状。
*   `defaultValue`: 是为稀疏字典中未指定的索引设置的值。它的默认值为零。这是一个可选参数。
...
[https://js.tensorflow.org/api/latest/#sparseToDense](https://js.tensorflow.org/api/latest/#sparseToDense)