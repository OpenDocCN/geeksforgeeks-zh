...
> 原文：[https://www.geeksforgeeks.org/tensorflow-js-tf-train-optimizer-class-minimize-method/](https://www.geeksforgeeks.org/tensorflow-js-tf-train-optimizer-class-minimize-method/)

TensorFlow.js 是谷歌开发的开源库，用于在浏览器或节点环境中运行机器学习模型和深度学习神经网络。

`.minimize()` 方法执行给定函数 `f()`，并试图通过计算 y 相对于由 `varList` 表示的给定可训练变量列表的梯度来最小化 `f()` 的标量输出。如果没有提供列表，它计算所有可训练变量的梯度。
...
*   `f (() => tf.Scalar)`: 它指定要执行的函数和要最小化其输出的函数。
*   `returnCost (布尔值)`: 指定是否返回执行 `f()` 产生的标量成本值。
*   `varList (tf.Variable[])`: 指定可训练变量列表。
...
**参考：**[https://js.tensorflow.org/api/latest/#tf.train.Optimizer.minimize](https://js.tensorflow.org/api/latest/#tf.train.Optimizer.minimize)