...
> 原文: [https://www.geeksforgeeks.org/tensorflow-js-tf-data-csvdataset-class/](https://www.geeksforgeeks.org/tensorflow-js-tf-data-csvdataset-class/)
...
`TF.data.CSVDataset` 类有一个预定义的方法，叫做 `columnNames()` 函数，帮助获取 CSV 文件的列名。每当数据中有列时，就使用 `columnNames()` 函数进行检索，否则当文件没有列名时会引发错误。可以解析为数字的值作为类型号发出，其他值作为字符串解析。
...
[https://js.tensorflow.org/api/latest/#class:data.CSVDataset](https://js.tensorflow.org/api/latest/#class:data.CSVDataset)