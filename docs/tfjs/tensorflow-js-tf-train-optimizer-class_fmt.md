...
> 原文：[https://www.geeksforgeeks.org/tensorflow-js-tf-train-optimizer-class/](https://www.geeksforgeeks.org/tensorflow-js-tf-train-optimizer-class/)

[TensorFlow.js](https://www.geeksforgeeks.org/introduction-to-tensorflow/) 是谷歌开发的开源库，用于在浏览器或节点环境下运行机器学习模型和深度学习神经网络。`tf.train.Optimizer()` 类用于扩展 `Serializable` 类。

`tf.train.Optimizer()` 类包含三个内置函数，如下所示。

*   `tf.train.Optimizer()` 类的 [`minimize()`](https://www.geeksforgeeks.org/tensorflow-js-tf-train-optimizer-class-minimize-method/) 函数
*   [`computeGradients()`](https://www.geeksforgeeks.org/tensorflow-js-tf-train-optimizer-class-computegradients-method/)
*   `tf.train.Optimizer()` 类的 [`applyGradients()`](https://www.geeksforgeeks.org/tensorflow-js-tf-train-optimizer-class-applygradients-method/) 函数

## tf.train.Optimizer().minimize()

`tf.train.Optimizer()` 类的 `minimize()` 函数用于执行给定的函数 `f()`，并通过计算 `y` 相对于由 `varList` 表示的给定可训练变量列表的梯度来最小化 `f()` 的标量输出。此外，如果没有提供列表，它会计算所有可训练变量的梯度。
...
`tf.train.Optimizer()` 类的 `computeGradients()` 函数用于执行 `f()`，并计算 `f()` 的标量输出相对于 `varList` 提供的可训练变量列表的梯度。此外，如果没有提供列表，它默认为所有可训练的变量。
...
`tf.train.Optimizer()` 类的 `applyGradients()` 函数的作用是：使用计算出的梯度来更新变量。
...
**参考：** [https://js.tensorflow.org/api/latest/#class:train.Optimizer](https://js.tensorflow.org/api/latest/#class:train.Optimizer)