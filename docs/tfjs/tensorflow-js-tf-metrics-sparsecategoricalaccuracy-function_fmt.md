...
> 原文: [https://www.geeksforgeeks.org/tensorflow-js-tf-metrics-sparsecategoricalaccuracy-function/](https://www.geeksforgeeks.org/tensorflow-js-tf-metrics-sparsecategoricalaccuracy-function/)

TensorFlow.js 是谷歌开发的开源库，用于在浏览器或节点环境中运行机器学习模型以及深度学习神经网络。

`tf.metrics.sparseCategoricalAccuracy()` 函数是一个稀疏分类精度度量函数，它使用索引和逻辑来返回 `tf.Tensor` 对象。
