...
> 原文: [https://www.geeksforgeeks.org/tensorflow-js-tf-memory-function/](https://www.geeksforgeeks.org/tensorflow-js-tf-memory-function/)

## 功能描述

Tensorflow.js 是谷歌开发的开源库，用于在浏览器或节点环境中运行机器学习模型和深度学习神经网络。

`tf.memory()` 函数用于获取程序当前时间的内存信息。此函数返回一个具有以下属性的 `memoryInfo` 对象：

*   `numBytes`: 指定当前分配的未公开字节数。
*   `numTensors`: 它指定分配的唯一张量的数量。
*   `numDataBuffers`: 指定当前时刻分配的未公开的唯一数据缓冲区个数，大于等于张量个数。
*   `unreliable`: `unreliable` 是只有当内存使用不可靠时才为真。
*   `reasons`: 指定一个字符串的数组，代表内存不可靠的原因。

仅当在 WebGL 上运行时，还包含以下属性：
...
**参考:** [https://js.tensorflow.org/api/latest/#memory](https://js.tensorflow.org/api/latest/#memory)