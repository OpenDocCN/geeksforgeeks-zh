...
> 原文：[https://www.geeksforgeeks.org/tensorflow-js-tf-layersmodel-class-summary-method/](https://www.geeksforgeeks.org/tensorflow-js-tf-layersmodel-class-summary-method/)

`tf.LayersModel` 是 TensorFlow.js 中用于训练、推断和评估层模型的类。它包含用于训练、评估、预测和保存层模型的方法。所以在这篇文章中，我们将了解 `model.summary()` 函数。
...
```
model_name.summary(line_length, position, print_function)
```

**参数：** 所有参数可选。

*   `line_length`：是一个自定义的行长，以若干字符为单位。
*   `position`：是一个显示每列宽度的数组，值可以是小数也可以是绝对值。
*   `print_function`：打印模型摘要的功能，默认功能为 `console.log()`。
...
**参考：** [https://js.tensorflow.org/api/latest/#tf.LayersModel.summary](https://js.tensorflow.org/api/latest/#tf.LayersModel.summary)