# TensorFlow.js
## tf.train.Optimizer.computeGradients()方法

原文: [https://www.geeksforgeeks.org/tensorflow-js-tf-train-optimizer-class-computegradients-method/](https://www.geeksforgeeks.org/tensorflow-js-tf-train-optimizer-class-computegradients-method/)

TensorFlow.js 是谷歌开发的开源库，用于在浏览器或节点环境中运行机器学习模型和深度学习神经网络。

执行 `f()` 并计算 `f()` 的标量输出相对于 `varList` 提供的可训练变量列表的梯度。如果没有提供列表，默认为所有可训练变量。
...
**参数:**
*   `f (() => tf.scalar)`: 要执行的函数，其输出用于计算变量的梯度。
*   `varList (tf.Variable[])`: 计算梯度的可选变量列表。如果指定，只有可训练变量是 `varList`，将根据计算梯度。默认为所有可训练变量。

**返回:** `{value: tf.scalar, gradients: {[name: string]: tf.Tensor}}`
...
参考: [https://js.tensorflow.org/api/latest/#tf.train.Optimizer.computeGradients](https://js.tensorflow.org/api/latest/#tf.train.Optimizer.computeGradients)