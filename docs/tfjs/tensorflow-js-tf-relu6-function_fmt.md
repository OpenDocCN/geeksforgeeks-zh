...
`.relu6()` 函数用于求整流后的线性 6，即最小值(最大值(x，0)，6)，并按元素进行。
...
*   `x`: 是陈述的张量输入，可以是 `tf.tensor`、`tf.Tensor` 或 `Array` 类型。此外，如果所述数据类型是 `boolean` 类型，那么输出数据类型将是 `int32` 类型。
...
[https://js . tensorlow . org/API/latest/# rel 6](https://js.tensorflow.org/api/latest/#relu6)