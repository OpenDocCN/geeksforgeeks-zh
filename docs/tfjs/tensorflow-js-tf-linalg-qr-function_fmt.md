...
`tf.linalg.qr()` 函数用于应用用户主变换，通过 n 矩阵计算参考 m 的 qr 分解。
...
*   `x`: 规定的 `tf.Tensor`。待二维码分解的张量。它的等级必须大于或等于 2。假设，它的形状为`[..., M, N]`。它是 `tf.Tensor` 类型的。
*   `fullMatrices`: 它是一个可选参数，属于布尔值类型，默认值为 `false`。如果是 `true`，那么它评估正常大小的 *Q*，否则它评估的只是最高的 *N* 列 *Q* 和 *R*。
...
**参考:**
*   https://www.geeksforgeeks.org/tensorflow-js-tf-linalg-qr-function/
