...
> 原文: [https://www.geeksforgeeks.org/tensorflow-js-tf-sequential-class/](https://www.geeksforgeeks.org/tensorflow-js-tf-sequential-class/)

TensorFlow.js 是谷歌开发的开源库，用于在浏览器或 Node 环境中运行机器学习模型和深度学习神经网络。

TensorFlow.js `tf.Sequential` 类是堆叠形式的层集合的模型。这些层连接到各自的相邻层。我们用 `tf.sequential()` 函数创建 `tf.Sequential` 类实例。`tf.Sequential` 类有许多用于实例的方法。
...
不同的方法返回不同的返回值，例如 `tf.Tensor` 对象等。

## 示例 1

在本例中，我们将看到 `add()` 方法，该方法用于在图层顶部添加图层。它以图层为参数，返回 void。
...
在本例中，我们将看到 `summary()` 方法，该方法用于打印图层实例的摘要。它采用的是行长度，即摘要的自定义长度，位置是摘要列的宽度，最后是打印功能，用于自定义摘要的输出。它返回空。
...
**参考:** [https://js.tensorflow.org/api/latest/#class:Sequential](https://js.tensorflow.org/api/latest/#class:Sequential)