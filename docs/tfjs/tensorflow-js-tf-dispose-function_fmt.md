# TensorFlow.js TF.dispose() 函数

> 哎哎哎:# t0]https://www.geeksforgeeks.org/tensorlow-js-TF-dispose-function/

TensorFlow.js 是 Google Brain 团队开发的开源库，用于在浏览器或 Node 环境中运行深度学习神经网络。

`dispose()` 函数用于释放一个张量所占用的内存。
...
**示例 2:** 在本例中，我们将创建一个秩为 2 的张量，并尝试使用 `tf.dispose` 函数在打印前后对其进行处理。
...
**参考:** https://js.tensorflow.org/api/latest/#dispose