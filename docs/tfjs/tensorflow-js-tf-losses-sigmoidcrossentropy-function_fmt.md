...
> 原文：[https://www.geeksforgeeks.org/tensorflow-js-tf-losses-sigmoidcrossentropy-function/](https://www.geeksforgeeks.org/tensorflow-js-tf-losses-sigmoidcrossentropy-function/)
...
*   `logits`：预测的是输出。
*   `weights`：这些是等级不是 0 就是 1 的张量，必须是宽可铸到勒贝尔。
*   `labelSmoothing`：如果该值大于 0，则表示将平滑标签。
*   `reduction`：适用于亏损的还原类型。它必须是缩减类型。

**注意：**`weights`、`labelSmoothing`和`reduction`为可选参数。
...
[https://js.tensorflow.org/api/latest/#losses.sigmoidCrossEntropy](https://js.tensorflow.org/api/latest/#losses.sigmoidCrossEntropy)