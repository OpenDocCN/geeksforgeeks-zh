...
*   `crops`: 一种二维数组形状，其[M，2]的所有值必须大于或等于 0。这里`crops[i] = [cropStart, cropEnd]`，它定义了要从输入维度 i + 1 裁剪的部分。
...
**参考资料:**[https://js.tensorflow.org/api/latest/#batchToSpaceND](https://js.tensorflow.org/api/latest/#batchToSpaceND)