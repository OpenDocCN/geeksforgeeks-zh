...
*   `equation`: 它是第一个输入张量元素，是描述收缩的字符串，格式与 `numpy.einsum` 相同。
*   `tensors`: 它是第二个输入张量元素，其中输入用于收缩(每个张量一个张量)，其形状应与方程一致。
