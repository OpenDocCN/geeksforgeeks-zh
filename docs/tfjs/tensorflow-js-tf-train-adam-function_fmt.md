...
> 原文：[https://www.geeksforgeeks.org/tensorflow-js-tf-train-adam-function/](https://www.geeksforgeeks.org/tensorflow-js-tf-train-adam-function/)
...
在 Tensorflow.js 中，`tf.train.adam()` 函数用于创建使用 Adam 算法的 `AdamOptimizer`。
...
*   **learningRate**：用于 Adam 梯度下降算法的学习率。它是*可选的*。
*   **beta1**：一阶矩估计的指数衰减率。它是*可选的*。
*   **beta2**：二阶矩估计的指数衰减率。它是*可选的*。
*   **epsilon**：一个数值稳定的小常数。它是*可选的*。
...
以 `x`，`y` 输入张量和 `A`，`b`，`c` 为随机系数定义二次函数。然后，我们计算预测的均方损失，并将其传递给 Adam 优化器，以最小化损失，并理想地更改系数。
...
下面是我们设计一个简单模型的代码，我们通过 `tf.train.adam` 定义一个学习率参数为 `0.01` 的优化器，并将其传递给模型编译。
...
**参考资料：**[https://js.tensorflow.org/api/3.6.0/#train.adam](https://js.tensorflow.org/api/3.6.0/#train.adam)