...
`tf.train.adadelta()` 函数我们用来创建一个使用 Adadelta 算法的 `AdadeltaOptimizer`。Adadelta 算法是梯度下降优化算法的扩展。它用于优化神经网络。
...
*   `learningRate`: 指定 Adadelta 梯度下降算法将使用的学习速率。
*   `rho`: 它指定每次更新的学习速率衰减。
*   `epsilon`: 它指定了一个常数 `epsilon`，用于改善梯度更新的条件。可选的。

**返回值:** 返回一个 `tf.AdadeltaOptimizer`。

## 示例 1

通过学习系数 `a` 和 `b`，使用 Adadelta 优化器拟合函数 `f = (a * x + b)`。
...
通过学习系数 `a`、`b` 和 `c`，使用 Adadelta 优化器拟合二次方程。优化器配置如下:
...
**参考资料:** [https://js.tensorflow.org/api/1.0.0/#train.adadelta](https://js.tensorflow.org/api/1.0.0/#train.adadelta)