...
`.clipByValue()` 函数用于查找所述张量输入的剪辑值，并且是按元素进行的。
...
*   `x`: 它是张量输入，其值将被剪切，并且它可以是类型 `tf.Tensor`，或`TypedArray`，或`Array`。
*   `clipValueMin`: 是要裁剪的指定范围的下限。
*   `clipValueMax`: 是要裁剪的指定范围的上限。
...
在这个示例中，我们定义了一个整数类型的输入张量，然后打印它的裁剪值。为了创建输入张量，我们使用 `.tensor1d()` 方法，为了打印输出，我们使用 `.print()` 方法。
...
在本例中，浮点类型值被视为张量输入，所有参数都直接传递给 `clipByValue` 函数。
