...
`.image.flipLeftRight()`功能用于将所述图像从左手侧翻转到右手侧。目前它可以在中央处理器、网易以及 WASM 后台访问。
...
*   `image`: 是所述的结构 4D 张量[批次，图像高度，图像宽度，深度]。它可以是 `tf.Tensor4D`、`TypedArray` 或数组。
...
在本例中，我们将看到在 `tf.image.flipLeftRight()` 函数中使用 4D 张量的用法。
...
// Printing output
tf.image.flipLeftRight(tf.tensor4d([[[[4, 7], [21, 9]], [[8, 9], [1, 5]]]])).print();
```
...
在本例中，我们将看到在 `tf.image.flipLeftRight()` 函数中使用一个浮点数组。
...
https://js.tensorflow.org/api/latest/#image.flipLeftRight