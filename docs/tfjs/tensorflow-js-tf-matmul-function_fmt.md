...
*   `a`: 这是点积运算中的第一个矩阵。
*   `b`: 这是点积运算中的第二个矩阵。
*   `transposeA`: 这是可选的，如果设置为真，那么 `a` 在乘法之前被转置。
*   `transposeB`: 这是可选的，如果设置为真，那么 `b` 在乘法之前被转置。
...
// Calling the .matMul() function over
...
// Calling the tf.matMul() function over
// the above tensors as its parameters and
// printing the result.
...
**参考资料:** [https://js.tensorflow.org/api/latest/#matMul](https://js.tensorflow.org/api/latest/#matMul)