...
> 原文: [https://www.geeksforgeeks.org/tensorflow-js-tf-layers-concatenate-function/](https://www.geeksforgeeks.org/tensorflow-js-tf-layers-concatenate-function/)
...
*   **轴 (数字):** 指定输入将连接的轴。它遵循基于 0 的索引，其值必须在 `[-rank, rank]` 范围内。它可以是正的，也可以是负的。这里，正轴的范围从 0 到 `rank(value)`，并指定第几个轴的尺寸。负值指定 `axis + rank(value) - ndim`。默认值为 0。
...
*   **权重:** 指定图层的初始权重值。
*   **InputType:** `"float32"` 或 `"int32"` 或 `"bool"` 或 `"complex64"` 或 `"string"`。
...
**参考:** [https://js.tensorflow.org/api/latest/#layers.concatenate](https://js.tensorflow.org/api/latest/#layers.concatenate)