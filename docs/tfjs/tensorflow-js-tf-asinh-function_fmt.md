...
*   `x`: 是要计算其 asinh 值的张量输入，可以是 `tf.Tensor` 类型，或 `TypedArray`，或 `Array`。

**返回值:** 返回 `tf.Tensor` 对象。

**示例 1:** 在本例中，我们定义了一个整数类型的输入张量，然后将其 asinh 值打印出来。为了创建输入张量，我们使用 `tensor1d()` 方法，为了打印输出，我们使用 `print()` 方法。
...
**参考:** https://js.tensorflow.org/api/latest/#asinh