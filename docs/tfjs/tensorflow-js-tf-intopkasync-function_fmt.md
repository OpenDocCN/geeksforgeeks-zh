...
`inTopKAsync()` 函数用于检查所述目标是否在给定的顶部 K *预测*中。
...
*   `predictions`: 它是所述的二维或上张量输入，其最后大小不小于 `k`，并且它可以是 `tf.tensor`、类型或数组。
*   `targets`: 它是所述的一维或上张量输入，并且它可以是 `tf.tensor`、类型或数组。
*   `k`: 是计算精度需要考虑的顶层元素的可选数量。默认值为 1，并且是数字类型。

## 返回值

返回 `Promise<tf.Tensor>` 对象。
