...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timestamp-round/](https://www.geeksforgeeks.org/python-pandas-timestamp-round/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas`就是其中一个包，让导入和分析数据变得容易多了。

`Timestamp.round()`函数用于将给定的时间戳舍入到指定的分辨率。
...
## 示例 #1
使用`Timestamp.round()`函数将给定的时间戳舍入为每日时间序列频率。
...
## 示例 #2
使用`Timestamp.round()`函数将给定的时间戳舍入到精确的时间序列频率。
