...
语法: `Series.median(axis=None, skipna=None, level=None, numeric_only=None, **kwargs)`
...
使用 `Series.median()` 函数找到给定序列对象中底层数据的中值。
...
使用 `Series.median()` 函数求给定序列对象中底层数据的中位数。给定的序列对象包含一些缺失的值。
