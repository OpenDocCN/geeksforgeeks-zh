# Python Pandas TimedeltaIndex.seconds属性

> 原文: [https://www.geeksforgeeks.org/python-pandas-timedeltaindex-seconds/](https://www.geeksforgeeks.org/python-pandas-timedeltaindex-seconds/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

Pandas `TimedeltaIndex.seconds` 属性返回时间增量索引对象中每个元素的秒数。返回值的范围从 0 到小于 1 天。

> **语法:** `TimedeltaIndex.seconds`
>
> **返回:** 索引对象
