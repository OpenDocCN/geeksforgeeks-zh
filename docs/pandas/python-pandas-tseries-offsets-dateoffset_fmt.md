...
> 原文链接: [https://www.geeksforgeeks.org/python-pandas-tseries-offsets-dateoffset/](https://www.geeksforgeeks.org/python-pandas-tseries-offsets-dateoffset/)
...
pandas `tseries.offsets.DateOffset` 用于创建一个日期范围的标准日期增量。
...
使用 `pandas.tseries.offsets.DateOffset` 功能创建 2 天的日期偏移。
...
使用 `pandas.tseries.offsets.DateOffset` 功能创建 10 天 2 小时的日期偏移。
