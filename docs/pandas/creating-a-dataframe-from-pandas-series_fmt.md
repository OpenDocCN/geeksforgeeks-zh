...
> 原文: [https://www.geeksforgeeks.org/creating-a-dataframe-from-pandas-series/](https://www.geeksforgeeks.org/creating-a-dataframe-from-pandas-series/)
...
**说明:**
我们正在合并两个系列`author`和`article`。创建一个字典，这样我们就可以组合系列的元数据。元数据是可以定义一系列值的数据的数据。把这个字典传给熊猫数据框，最后你可以看到两个系列的组合结果，即作者和文章数量。
...
**说明:**
我们又增加了一个外部命名为`age`的系列，然后直接在熊猫数据框中增加了这个系列。请记住一件事，如果缺少任何值，那么默认情况下，它将被转换为`NaN`值，即默认情况下为空。
