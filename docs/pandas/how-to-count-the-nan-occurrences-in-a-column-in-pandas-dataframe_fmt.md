...
> 原文：[https://www.geeksforgeeks.org/how-to-count-the-nan-occurrences-in-a-column-in-pandas-dataframe/](https://www.geeksforgeeks.org/how-to-count-the-nan-occurrences-in-a-column-in-pandas-dataframe/)

数据帧被分成单元，这些单元可以存储属于某个数据结构的值，也可以包含缺失值或空值。`pandas`包包含各种内置函数，用于检查数据帧单元中的值是否为 NA，并对这些 NA 值执行聚合。

## 方法 1：使用内置方法 `isna()` 和 `sum()`

在数据框上使用内置方法 `isna()` 和 `sum()`。
...
从数据帧的长度（即数据帧中的行数）中减去数据帧的任何特定列中包含的值的计数。`count()` 方法给出了指定列中非 NaN 值的总数，`len(data_frame)` 给出了数据帧的长度，即帧中的总行数。
