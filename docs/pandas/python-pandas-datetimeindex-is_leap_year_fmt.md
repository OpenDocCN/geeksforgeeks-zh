...
> 原文: [https://www.geeksforgeeks.org/python-pandas-datetimeindex-is_leap_year/](https://www.geeksforgeeks.org/python-pandas-datetimeindex-is_leap_year/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

如果日期属于闰年，`DatetimeIndex.is_leap_year` 属性返回一个布尔指示符。闰年是一年，有 366 天（而不是 365 天），包括 2 月 29 日作为闰日。闰年是四的倍数，但被 100 整除但不能被 400 整除的年份除外。
