...
`TimedeltaIndex.notna()` 函数检测给定时间增量索引对象的所有非缺失值。该功能的功能与 `TimedeltaIndex.isna()` 相反。

> **语法:** `TimedeltaIndex.notna()`
>
> **参数:** 无
>
> **返回:** 一个布尔数组，表示值是否为非空
...
使用 `TimedeltaIndex.notna()` 函数检测给定时间增量索引对象中所有未丢失的值。
...
使用 `TimedeltaIndex.notna()` 函数检测给定时间增量索引对象中所有未缺失的值。
