...
`Index.append()` 功能用于将单个或一组索引附加在一起。在收集索引的情况下，所有索引都按照传递给 `Index.append()` 函数的相同顺序追加到原始索引中。该函数返回一个附加索引。
...
## 返回值
追加: `bool` 或 `array_like` (如果指定了 `axis`)。单元素 `array_like` 可以转换为 `bool`。

## 示例 1
使用 `Index.append()` 函数将单个索引附加到给定的索引。
...
## 示例 2
使用 `Index.append()` 函数在给定索引的末尾追加一个索引集合。
