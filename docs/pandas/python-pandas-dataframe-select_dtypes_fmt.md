...
> 原文: [https://www.geeksforgeeks.org/python-pandas-dataframe-select_dtypes/](https://www.geeksforgeeks.org/python-pandas-dataframe-select_dtypes/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`DataFrame.select_dtypes()` 函数根据列数据类型返回数据框列的子集。此函数的参数可以设置为包括具有某些特定数据类型的所有列，也可以设置为排除具有某些特定数据类型的所有列。
...
## 示例 1
使用 `select_dtypes()` 功能选择所有具有浮动数据类型的列。
...
让我们使用 `select_dtypes()` 函数选择数据框中所有具有浮点数据类型的列。
...
## 示例 2
使用 `select_dtypes()` 功能选择数据框中的所有列，浮动数据类型的列除外。
