...
`TimedeltaIndex.notnull()` 函数检测给定时间增量索引对象的所有非缺失值。功能类似 `TimedeltaIndex.notna()`。
...
## 示例 1
使用 `TimedeltaIndex.notnull()` 函数检测给定时间增量索引对象中所有未丢失的值。
...
## 示例 2
使用 `TimedeltaIndex.notnull()` 函数检测给定时间增量索引对象中所有未缺失的值。
