...
> **语法**: `dataframe[~dataframe[column_name].isin(list)]`
>
> **其中**
>
> *   `dataframe` 是输入数据帧
...
现在，我们可以使用 `any()` 函数在多个列中进行筛选。此函数将检查存在于任何给定列中的值，列以逗号分隔的 `[[]]` 给出。

> **语法**: `dataframe[~dataframe[[columns]].isin(list).any(axis=1)]`
...
> **语法:** `dataframe[~numpy.isin(dataframe['column'], list)]`
