...
Pandas `Series.repeat()` 功能重复一个系列的元素。它返回一个新的序列，其中当前序列的每个元素被连续重复给定的次数。

> **语法:** `Series.repeat(repeats, axis=none)`
>
> **参数:**
> `repeats`: 每个元素的重复次数。
> `axis`: 无
...
使用 `Series.repeat()` 功能重复给定序列对象中的每个值 2 次。
...
使用 `Series.repeat()` 函数在给定的 Series 对象中重复每个值 3 次。
