# 使用 SQLAlchemy 读取 SQL 数据库表

## 将 SQL 数据库表读入 Pandas 数据框

> 原文: [https://www.geeksforgeeks.org/read-sql-database-table-into-a-pandas-dataframe-using-sqlalchemy/](https://www.geeksforgeeks.org/read-sql-database-table-into-a-pandas-dataframe-using-sqlalchemy/)

要仅使用表名将 SQL 表读入数据框，而不执行任何查询，我们在 Pandas 中使用 `read_sql_table()` 方法。此功能不支持 DBAPI 连接。

### read_sql_table()

> **语法:** `pandas.read_sql_table(table_name, con, schema=None, index_col=None, coerce_float=True, parse_dates=None, columns=None, chunksize=None)`

> **参数:**
>
> - `table_name`: (字符串) 数据库中 SQL 表的名称。
> - `con`: SQLAlchemy 可连接或 str。
> - `schema`: (str) 要查询的数据库中的 SQL 模式的名称(如果数据库风格支持)。默认值为 None。
> - `index_col`: 字符串或字符串的列表。要设置为索引的列(多索引)。默认值为 None。
> - `coerce_float`: (bool) 尝试转换非字符串、非数字对象(如 Decimal)的值到 float。默认值为 True。
> - `parse_dates`: (列表或字典)
>     - 要解析为日期的列名列表。
...
> - `columns`: 从 SQL 表中选择的列名列表。默认值为 None。
> - `chunksize`: (int) 如果指定，返回一个迭代器，其中 `chunksize` 是每个块中要包含的行数。默认值为 None。
