...
> 原文: [https://www.geeksforgeeks.org/highlight-the-minimum-value-in-each-column-in-pandas/](https://www.geeksforgeeks.org/highlight-the-minimum-value-in-each-column-in-pandas/)
...
> **参数:**
> *   `subset`: 要求最小值的列的名称。
> *   `color`: 要突出显示单元格的颜色的名称
> *   `axis`: {0 或 ‘index’, 1 或 ‘columns’} 基于您想要找到最小值的轴。
> **返回:** Styler 对象。
...
> *   `axis`: {0 或 ‘index’, 1 或 ‘columns’, None}, 默认 0。应用于每一列(`axis=0` 或 “index”)、每一行(`axis=1` 或 “columns”)，或一次应用于整个数据框，`axis=None`。
> *   `subset`: 要调用函数的一组列或行。
> *   `**kwargs`: 传递给 `func`。
> **返回:** Styler 对象。
