# Python pandas Index.argsort()详解

## 概述

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`pandas.Index.argsort()` 函数返回对索引进行排序的整数索引。默认情况下，排序顺序已设置为递增顺序。
...
**参数:**
*   `*args`: 传递给 `numpy.ndarray.argsort`。
*   `**kwargs`: 传递给 `numpy.ndarray.argsort`。
...
使用 `Index.argsort()` 函数找到给定索引排序的索引顺序。
