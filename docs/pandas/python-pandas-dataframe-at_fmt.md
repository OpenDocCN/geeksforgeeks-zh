...
`.at[]` 用于在经过的位置返回数据帧中的数据。传递的位置的格式为 `[位置，列名]`。此方法的工作方式与 Pandas `.loc[]` 类似，但 `.at[]` 用于返回唯一的单个值，因此比它更快。

> **语法:** `dataframe.at[位置， 标签]`
> **参数:**
> **位置:** 列中元素的位置
> **标签:** 要使用的列名
> **返回类型:** 单个元素在传递位置
...
**示例#1:**
在此示例中，通过将 csv 的 URL 传递给 Pandas 来创建数据帧。`.read_csv()` 方法。之后，使用 `.at[]` 方法返回“名称”列中的第二个值。
