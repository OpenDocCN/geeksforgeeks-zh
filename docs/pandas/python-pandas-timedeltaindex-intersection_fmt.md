...
`TimedeltaIndex.intersection()` 函数返回一个新的索引，其中包含两个索引共有的索引元素。这是时间增量索引对象的专用交集。它可能比索引交叉要快得多。

> **语法:** `TimedeltaIndex.intersection(other)`
> **参数:**
...
使用 `TimedeltaIndex.intersection()` 函数查找两个 `TimedeltaIndex` 对象的交集。
