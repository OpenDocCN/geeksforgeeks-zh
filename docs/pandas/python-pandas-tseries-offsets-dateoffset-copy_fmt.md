...
> 原文: [https://www.geeksforgeeks.org/python-pandas-tseries-offsets-dateoffset-copy/](https://www.geeksforgeeks.org/python-pandas-tseries-offsets-dateoffset-copy/)
...
pandas `tseries.offsets.DateOffset.copy()` 函数返回给定日期偏移对象的副本。
...
## 示例 #1
使用 `pandas.tseries.offsets.DateOffset.copy()` 函数返回给定日期偏移对象的副本。
...
现在，我们将向给定的时间戳对象添加日期偏移，以增加 datetime 值。我们还将返回给定日期偏移对象的副本。
...
正如我们在输出中看到的，该函数已经成功创建了给定 `DateOffset` 对象的副本。`False` 表示两个对象不相同。
