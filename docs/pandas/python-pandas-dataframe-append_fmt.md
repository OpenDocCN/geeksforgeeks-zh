...
`DataFrame.append()` 函数用于将其他数据帧的行追加到给定数据帧的末尾，返回一个新的数据帧对象。不在原始数据框中的列将作为新列添加，新单元格将填充 `NaN` 值。
...
- `other`: 数据框或 `Series`/类似字典的对象，或这些的列表。
- `ignore_index`: 如果为真，不要使用索引标签。
- `verify_integrity`: 如果为真，则在创建重复索引时引发值错误。
- `sort`: 如果自身和其他的列没有对齐，则排序列。默认排序已被弃用，并将在 pandas 的未来版本中更改为不排序。显式传递 `sort=True` 以消除警告并排序。显式传递 `sort=False` 以消除警告并不排序。
