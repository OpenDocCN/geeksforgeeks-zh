# Python | Pandas `isnull()` 和 `notnull()`

> 原文: [https://www.geeksforgeeks.org/python-pandas-isnull-and-notnull/](https://www.geeksforgeeks.org/python-pandas-isnull-and-notnull/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。
从 `csv` 文件制作数据框时，许多空白列作为空值导入到数据框中，这在以后操作该数据框时会产生问题。`Pandas` 的 `isnull()` 和 `notnull()` 方法用于检查和管理数据框中的空值。
