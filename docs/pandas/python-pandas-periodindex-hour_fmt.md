...
> 原文: [https://www.geeksforgeeks.org/python-pandas-periodindex-hour/](https://www.geeksforgeeks.org/python-pandas-periodindex-hour/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`熊猫`就是其中一个包，让导入和分析数据变得容易多了。

`PeriodIndex.hour`属性返回给定周期索引对象的周期小时数。
...
使用`PeriodIndex.hour`属性查找给定周期索引对象的周期小时值。
...
使用`PeriodIndex.hour`属性查找给定周期索引对象的周期小时数。
