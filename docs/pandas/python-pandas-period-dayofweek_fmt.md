...
> 原文: [https://www.geeksforgeeks.org/python-pandas-period-dayofweek/](https://www.geeksforgeeks.org/python-pandas-period-dayofweek/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Period.dayofweek` 属性返回给定周期对象的星期几。它返回一个整数值。
