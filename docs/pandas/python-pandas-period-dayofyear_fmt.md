# Python Pandas Period.dayofyear 属性

> 原文: [https://www.geeksforgeeks.org/python-pandas-period-dayofyear/](https://www.geeksforgeeks.org/python-pandas-period-dayofyear/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Period.dayofyear` 属性返回给定周期对象的一年中的某一天。它返回一个整数值。
...
使用 `Period.dayofyear` 属性在给定的 Period 对象中查找一年中的某一天。
