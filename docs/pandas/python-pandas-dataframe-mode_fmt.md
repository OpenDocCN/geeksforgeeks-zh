...
`pandas` 的 `DataFrame.mode()` 功能获取所选轴上每个元素的模式。为每个标签的每个模式添加一行，用 `nan` 填充空白。请注意，可能会为选定的轴返回多个值（当多个项目共享最大频率时），这就是返回数据框的原因。
...
- `axis`：获取各列的模式（0）或获取各行的模式（1）。
- `numeric_only`：如果为 `True`，则仅适用于数值列。

## 返回

返回一个已排序的 `DataFrame`，包含模式值。
