...
> 原文: [https://www.geeksforgeeks.org/python-pandas-dataframe-round/](https://www.geeksforgeeks.org/python-pandas-dataframe-round/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`dataframe.round()` 函数用于将数据帧舍入到可变的小数位数。该函数提供了根据不同位置舍入不同列的灵活性。

## 语法
`DataFrame.round(decimals=0, *args, **kwargs)`

## 参数
- `decimals`: 每列要舍入到的小数位数。如果给定一个 `int`，则将每列四舍五入到相同的位数。否则，`dict` 和 `Series` 舍入到可变的位数。如果 `decimals` 是类似字典的，列名应该在键中，如果 `decimals` 是 `Series`，列名应该在索引中。任何未包含在 `decimals` 中的列将保持原样。不是输入列的 `decimals` 元素将被忽略。

## 返回
数据框对象。

## 示例 1
使用 `round()` 函数将数据框中的所有列四舍五入到小数点后 3 位。
