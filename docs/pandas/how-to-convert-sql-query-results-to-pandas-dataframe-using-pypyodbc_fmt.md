...
> 原文: [https://www.geeksforgeeks.org/how-to-convert-sql-query-results-to-pandas-dataframe-using-pypyodbc/](https://www.geeksforgeeks.org/how-to-convert-sql-query-results-to-pandas-dataframe-using-pypyodbc/)
...
**pypyodbc:** 是纯 Python 跨平台 odbc 接口模块。在终端中使用此命令安装 `pypyodbc` 模块以访问 odbc 数据库。
...
### 步骤 3: 创建表格 `student_marks` 并在表格中添加行
...
使用 `pdb.connect()` 使用服务器名和数据库名连接到 MSSQL 服务器。然后使用 `pd.read_sql()` 读取 SQL 查询到熊猫数据框中并打印数据。
