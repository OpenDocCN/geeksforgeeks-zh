...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timedeltaindex-is_all_dates/](https://www.geeksforgeeks.org/python-pandas-timedeltaindex-is_all_dates/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`熊猫`就是其中一个包，让导入和分析数据变得容易多了。

`TimedeltaIndex.is_all_dates`属性返回一个布尔值。如果时间增量索引对象只包含日期，则返回`True`。
...
使用`TimedeltaIndex.is_all_dates`属性检查时间增量索引对象是否只包含日期。
...
使用`TimedeltaIndex.inferred_type`属性检查时间增量索引对象是否只包含日期。
