...
### 语法
`Series.min(轴=无，skipna =无，级别=无，numeric _ only =无，**kwargs)`
...
## 示例#1
使用 `Series.min()` 函数在给定序列对象的底层数据中寻找最小值。
...
```py
# return the minimum value in the series object
result = sr.min()
...
## 例 2
使用 `Series.min()` 函数在给定序列对象的底层数据中寻找最小值。给定的序列对象也包含一些缺失的值。
