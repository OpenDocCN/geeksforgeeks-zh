# Python 熊猫时间增量指数 dtype

> 哎哎哎:# t0]https://www . geeksforgeeks . orgpython-pandas-time-delta-index-dtype/
...
使用 `TimedeltaIndex.dtype` 属性查找给定时间增量索引对象的数据类型。
...
使用 `TimedeltaIndex.dtype` 属性查找给定 `TimedeltaIndex` 对象的数据类型。
