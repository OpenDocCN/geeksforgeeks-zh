...
在 Pandas 中，Panel 是一个非常重要的三维数据容器。三个轴的名称旨在为描述涉及面板数据的操作，特别是面板数据的计量经济学分析提供一些语义含义。

Pandas 中的 `Panel.rfloordiv()` 函数用于获取数列和数据框/面板的整数除法。

## 语法

**语法:** `Panel.rfloordiv(other, axis=0)`
