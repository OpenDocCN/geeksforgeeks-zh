...
> 原文: [https://www.geeksforgeeks.org/python-pandas-dataframe-product/](https://www.geeksforgeeks.org/python-pandas-dataframe-product/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`pandas` 的 `dataframe.product()` 功能返回所请求坐标轴的产品值。它在请求的轴上将所有元素相乘。默认情况下，选择索引轴。
...
使用 `product()` 函数查找数据框中任意轴的乘积。数据框包含 `NaN` 值。
