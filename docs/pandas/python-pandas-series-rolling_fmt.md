...
[pandas Series](https://www.geeksforgeeks.org/python-pandas-series/) 是带有轴标签的一维数组。标签不必是唯一的，但必须是可散列的类型。该对象支持整数索引和基于标签的索引，并提供了一系列方法来执行涉及索引的操作。

## Series.rolling()

`Series.rolling()` 功能是一个非常有用的功能。它对给定序列对象中的基础数据提供滚动窗口计算。

> **语法:** `Series.rolling(窗口, min_periods=None, center=False, win_type=None, on=None, axis=0, closed=None)`
>
> **参数:**
> **窗口:** 移动窗口的大小
...
使用 `Series.rolling()` 函数为给定的 Series 对象查找基础数据的滚动窗口总和。滚动窗口的大小应为 2，每个元素的权重应相同。
...
使用 `Series.rolling()` 函数查找给定 Series 对象的底层数据的滚动窗口和。滚动窗口的大小应为 2，滚动窗口类型应为“三角形”。
