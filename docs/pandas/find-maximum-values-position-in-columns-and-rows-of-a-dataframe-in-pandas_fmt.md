...
> **`level` :** 如果 `axis` 为 `MultiIndex`(分层)，则沿特定级别计数，折叠为 `Series`。如果没有，将尝试使用所有内容，然后只使用数字数据。不适用于系列。
> **返回:** 最大值: `Series` 或 `DataFrame`(如果指定了级别)
...
若要查找每行的最大值，请在参数 `axis= 1` 的 `DataFrame` 对象上调用 `max()`方法。
...
Pandas `DataFrame.idxmax()` 方法返回请求轴上最大值首次出现的索引。当在任何索引中查找最大值的索引时，所有 `NA/null` 值都被排除在外。
