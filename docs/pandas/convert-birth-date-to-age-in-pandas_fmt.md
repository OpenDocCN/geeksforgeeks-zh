...
**说明:**在上面的代码中，我们使用了`datetime`包，导入了`datetime`和`date`。我们使用`strptime`函数来识别存储在`born`变量中的日期，即我们将 2000 年 1 月 26 日识别为日期/月份/年份。然后我们用`today()`函数得到今天的日期。为了得到年龄，我们使用了公式“`today.year - born.year - ((today.month, today.day) < (born.month, born.day))`”。在这种情况下，我们从出生年份中减去一个当前年份，然后如果当前日期和月份没有超过出生日期和月份，我们减去一个，因为他/她的出生日期和月份还没有到来。
...
**说明:**在上面的代码中使用了`pandas`和`datetime`包。我们创建了一个包含三行不同日期的数据帧。为了计算年龄，我们创建了一个`age`函数，它使用`strptime`函数以日期/月/年的格式识别日期。然后我们用`today()`函数得到今天的日期。为了得到年龄，我们使用了公式“`today.year - born.year - ((today.month, today.day) < (born.month, born.day))`”。在这种情况下，我们从出生年份中减去一个当前年份，然后如果当前日期和月份没有超过出生日期和月份，我们减去一个，因为他/她的出生日期和月份还没有到来。我们在这个函数中返回年龄，它作为一个新行添加到`Age`列中。稍后我们显示数据框`df`。