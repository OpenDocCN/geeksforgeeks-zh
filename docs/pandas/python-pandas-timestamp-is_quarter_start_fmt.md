...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timestamp-is_quarter_start/](https://www.geeksforgeeks.org/python-pandas-timestamp-is_quarter_start/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Pandas` 的 `Timestamp.is_quarter_start` 属性返回一个布尔值。如果给定时间戳对象中的日期是季度的开始，则返回 `True`，否则返回 `False`。
...
使用 `Timestamp.is_quarter_start` 属性检查给定时间戳对象中的日期是否是季度的开始。
...
使用 `Timestamp.is_quarter_start` 属性检查给定时间戳对象中的日期是否是季度开始。
