...
> 原文：[https://www.geeksforgeeks.org/python-pandas-timedeltaindex-slice_indexer/](https://www.geeksforgeeks.org/python-pandas-timedeltaindex-slice_indexer/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`TimedeltaIndex.slice_indexer()` 函数计算输入标签的切片索引器和有序时间增量索引对象的步长。该函数假设数据已排序。
...
使用 `TimedeltaIndex.slice_indexer()` 函数计算给定时间增量索引对象中传递的标签的切片索引器。
