...
`DataFrame.eval()` 函数用于在调用 DataFrame 实例的上下文中计算表达式。表达式在数据框的列上计算。
...
**参数：**
- `expr`：要求值的表达式字符串。
- `inplace`：如果表达式包含赋值，是否在位执行操作并变异现有的 DataFrame。否则，返回新的 DataFrame。
- `kwargs`：有关 `query()` 接受的关键字参数的完整详细信息，请参见 `eval()` 的文档。

**返回：** ndarray、标量或 pandas 对象。

## 示例 1

使用 `eval()` 函数计算数据框中所有列元素的总和，并将结果列插入数据框中。
...
使用 `eval()` 函数计算数据框中任意两个列元素的总和，并将结果列插入数据框中。数据框有 `NaN` 值。
