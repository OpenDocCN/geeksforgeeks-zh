...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-str-contains/](https://www.geeksforgeeks.org/python-pandas-series-str-contains/)
...
- `pat`：字符序列或正则表达式。
- `case`：如果为真，区分大小写。
- `flags`：传递到 `re` 模块的标志，例如 `re.IGNORECASE`。
- `na`：为缺失值填充值。
- `regex`：如果为真，则假设 `pat` 是正则表达式。

## 返回
布尔值系列或索引。

## 示例 #1
使用 `Series.str.contains()` 函数来查找给定序列对象中基础数据的字符串中是否存在模式。
...
## 示例 #2
使用 `Series.str.contains()` 函数来查找给定序列对象中基础数据的字符串中是否存在模式。使用正则表达式查找字符串中的模式。
