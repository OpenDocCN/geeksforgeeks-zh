...
> 原文: [https://www.geeksforgeeks.org/python-pandas-dataframe-rename_axis/](https://www.geeksforgeeks.org/python-pandas-dataframe-rename_axis/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

## 概述

`DataFrame.rename_axis()` 用于重命名数据框中索引或列的轴。
...
- **`mapper`**: 【标量，列表式，可选】值设置轴名属性。
- **`axis`**: `int` 或 `string`，默认 `0`
- **`copy`**: 【boolean，默认 `True`】同时复制基础数据
- **`inplace`**: `boolean`，默认 `False`

## 返回

已重命名: 调用者类型，如果 `inplace=True`，则返回 `None`。

有关代码中使用的 CSV 文件的链接，请单击此处的。

## 示例 #1

在 `nba.csv` 文件中将球队“波士顿凯尔特人”替换为“欧米茄勇士”。
...
# Printing the first 10 rows of the dataframe for visualization
df[:10]
```
