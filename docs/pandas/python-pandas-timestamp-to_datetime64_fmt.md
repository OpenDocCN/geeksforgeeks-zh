...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timestamp-to_datetime64/](https://www.geeksforgeeks.org/python-pandas-timestamp-to_datetime64/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Pandas` 的 `Timestamp.to_datetime64()` 函数为给定的时间戳对象返回一个精度为“ns”的 `numpy.datetime64` 对象。
