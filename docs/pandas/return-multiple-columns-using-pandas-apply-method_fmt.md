...
> 原文：[https://www.geeksforgeeks.org/return-multiple-columns-using-pandas-apply-method/](https://www.geeksforgeeks.org/return-multiple-columns-using-pandas-apply-method/)

传递给`pandas.apply()`的对象是系列对象，其索引是数据框的索引(`axis=0`)或数据框的列(`axis=1`)。默认情况下(`result_type=None`)，最终的返回类型是从应用的函数的返回类型推断出来的。否则，它取决于`result_type`参数。
...
在函数内部返回`Series`类似于传递`result_type="expand"`。产生的列名将是系列索引。
