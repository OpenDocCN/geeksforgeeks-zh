...
> 原文: [https://www.geeksforgeeks.org/python-pandas-period-daysinmonth/](https://www.geeksforgeeks.org/python-pandas-period-daysinmonth/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Pandas` 的 `Period.daysinmonth` 属性返回给定周期对象中一个月中的天数。
...
使用 `Period.daysinmonth` 属性查找给定周期对象中当月的天数。
