# Python Pandas DataFrame.sort_values() 教程 - Set-1

> 原文: [https://www.geeksforgeeks.org/python-pandas-dataframe-sort_values-set-1/](https://www.geeksforgeeks.org/python-pandas-dataframe-sort_values-set-1/)
...
- `ascending`: 布尔值，如果为真，则按升序对数据框进行排序。
- `inplace`: 布尔值。如果为真，则对传递的数据框本身进行更改。
- `kind`: 字符串，可以有三个输入 (`'quicksort'`、`'mergesort'` 或 `'heapsort'`) 算法用于对数据框进行排序。
- `na_position`: 取两个字符串输入 `'last'` 或 `'first'` 来设置 Null 值的位置。默认值为 `'last'`。
