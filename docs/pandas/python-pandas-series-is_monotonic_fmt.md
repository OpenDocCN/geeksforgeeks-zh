...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-is_monotonic/](https://www.geeksforgeeks.org/python-pandas-series-is_monotonic/)
...
Pandas `Series.is_monotonic` 属性返回一个布尔值。如果给定序列对象中的数据单调递增，则返回 `True`，否则返回 `False`。
...
使用 `Series.is_monotonic` 属性检查给定 Series 对象中的基础数据是否单调增加。
