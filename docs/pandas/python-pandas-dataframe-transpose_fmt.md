...
> 原文：[https://www.geeksforgeeks.org/python-pandas-dataframe-transpose/](https://www.geeksforgeeks.org/python-pandas-dataframe-transpose/)

Pandas DataFrame 是一个二维可变大小、潜在异构的表格数据结构，带有标记轴（行和列）。算术运算在行标签和列标签上对齐。它可以被认为是 Series 对象的类似字典的容器。这是 Pandas 的主要数据结构。

Pandas `DataFrame.transpose()` 函数转置数据帧的索引和列。它通过将行写成列来反映主对角线上的数据帧，反之亦然。

## 语法
`DataFrame.transpose(*args, **kwargs)`

## 参数
- `copy`：如果为真，则复制底层数据。否则（默认），如果可能，不进行复制。
...
## 示例 1
使用 `DataFrame.transpose()` 函数查找给定数据帧的转置。
