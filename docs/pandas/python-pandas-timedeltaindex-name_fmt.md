...
`TimedeltaIndex.name`属性返回时间增量索引对象的名称(如果已设置)。如果没有为所考虑的对象设置名称，则返回无。

> **语法:**`TimedeltaIndex.name`
>
> **返回:**字符串对象

## 示例#1

使用`TimedeltaIndex.name`属性找出时间增量索引对象的名称。
...
使用`TimedeltaIndex.name`属性找出`TimedeltaIndex`对象的名称。
