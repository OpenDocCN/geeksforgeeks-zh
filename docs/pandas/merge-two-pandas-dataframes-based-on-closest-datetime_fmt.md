...
> 原文: [https://www.geeksforgeeks.org/merge-two-pandas-dataframes-based-on-closest-datetime/](https://www.geeksforgeeks.org/merge-two-pandas-dataframes-based-on-closest-datetime/)
...
*   要了解更多关于此功能，请参考 Python 中的文章 [`pandas.merge_asof()函数`](https://www.geeksforgeeks.org/pandas-merge_asof-function-in-python/)
*   数据帧必须按关键字排序。
...
在这一步中，我们必须使用函数 `pd.DataFrame()` 创建数据帧。在本例中，我们创建了两个数据框，一个命名为 `left`，另一个命名为 `right`，因为我们的最后一个目标是基于最接近的日期时间合并两个数据框。它可以写成:
