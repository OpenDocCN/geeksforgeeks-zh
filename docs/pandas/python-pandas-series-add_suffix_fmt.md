...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-add_suffix/](https://www.geeksforgeeks.org/python-pandas-series-add_suffix/)

Pandas Series 是带有轴标签的一维数组。标签不必是唯一的，但必须是可散列的类型。该对象支持基于整数和基于标签的索引，并提供了一系列方法来执行涉及索引的操作。

Pandas `Series.add_suffix()` 功能用于在给定 Series 对象的索引标签末尾添加后缀。

> **语法:** `Series.add_suffix(suffix)`
>
> **参数:**
> `suffix`: 每个标签后要添加的字符串。
>
...
**示例 #1:** 使用 `Series.add_suffix()` 函数在给定 Series 对象中的每个索引标签的末尾添加后缀。
...
现在我们将使用 `Series.add_suffix()` 函数在给定 Series 对象中的每个索引标签的末尾添加后缀 `'_IPL 2019'`。
...
正如我们在输出中看到的那样，`Series.add_suffix()` 函数已经成功地在给定 Series 对象中的每个索引标签的末尾添加了传递的后缀。

**示例 2:** 使用 `Series.add_suffix()` 函数在给定 Series 对象中的每个索引标签的末尾添加后缀。
...
现在我们将使用 `Series.add_suffix()` 函数在给定 Series 对象中的每个索引标签的末尾添加后缀 `'_Date'`。
