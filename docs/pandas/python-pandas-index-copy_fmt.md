...
`Pandas` 的 `Index.copy()` 功能复制这个对象。该函数还将新对象的名称和数据类型属性设置为原始对象的名称和数据类型属性。如果我们希望新对象有不同的数据类型，那么我们可以通过设置函数的 `dtype` 属性来实现。
...
**示例 #2:** 使用 `Index.copy()` 函数复制原对象。还要设置新对象的 `name` 属性，并将字符串数据类型转换为 `datetime` 类型。
