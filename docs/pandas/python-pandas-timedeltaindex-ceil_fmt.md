...
> 原文链接: https://www.geeksforgeeks.org/python-pandas-timedeltaindex-ceil/

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`TimedeltaIndex.ceil()` 函数将时间增量索引对象的索引增加到指定的频率。该函数将频率作为一个参数，我们希望对其取最大值。
...
使用 `TimedeltaIndex.ceil()` 函数将给定时间增量索引对象的值上限为指定频率。
