...
`Pandas` 的 `Index.contains()` 函数返回一个布尔值，指示提供的键是否在索引中。如果输入值出现在索引中，则返回 `True`，否则返回 `False`，表示输入值不出现在索引中。
...
使用 `Index.contains()` 功能检查给定日期是否出现在索引中。
...
使用 `Index.contains()` 功能检查索引中是否存在输入月份。
