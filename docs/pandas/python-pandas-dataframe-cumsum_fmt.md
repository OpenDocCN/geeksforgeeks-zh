...
`DataFrame.cumsum()`用于求任意轴上的累计和值。每个单元格都填充了迄今为止看到的值的累积总和。
...
使用`cumsum()`函数来查找到目前为止沿着列轴看到的值的累积和。
...
使用`cumsum()`函数查找数据帧中沿索引轴到目前为止看到的值的累积和，其中`NaN`值出现在数据帧中。
