...
`Pandas` 的 `Index.asof()` 函数返回从索引返回标签，或者，如果不存在，返回前一个标签。假设索引已排序，如果传递的索引标签在索引中，则返回传递的索引标签，如果传递的索引标签不在索引中，则返回上一个索引标签。

### 注意
该函数仅适用于排序索引。如果没有排序，它将返回错误。
...
### 返回值
如果在索引中，则返回传递的标签。如果传递的标签不在排序的索引中，则为前一个标签；如果没有这样的标签，则为 `NaN`。

## 示例 1
使用 `Index.asof()` 函数将最新的索引标签返回到传递的索引标签。
