...
> 原文: [https://www.geeksforgeeks.org/multiple-density-plots-with-pandas-in-python/](https://www.geeksforgeeks.org/multiple-density-plots-with-pandas-in-python/)

多密度图是比较数据中多个组分布的好方法。我们可以使用 Pandas 的 `plot.density()` 功能制作多个密度图。然而，如果我们使用密度函数，我们需要转换宽格式的数据。宽数据表示不同列中的不同组。我们使用 Pandas 的 `pivot()` 函数转换宽格式的数据。
...
让我们将数据按照国家分组在不同的列中，这样我们就可以应用 `density()` 函数绘制多个密度图。
...
我们也可以在 dataframe 上调用 `plot.kde()` 函数，用 Pandas 做多密度图。
...
现在应用 `pivot()` 函数使数据帧为宽格式，然后应用 `kde()` 得到多密度图。
