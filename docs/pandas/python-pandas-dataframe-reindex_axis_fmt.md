...
> 原文: [https://www.geeksforgeeks.org/python-pandas-dataframe-reindex_axis/](https://www.geeksforgeeks.org/python-pandas-dataframe-reindex_axis/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`DataFrame.reindex_axis()` 功能使输入对象符合新的索引。该函数在先前索引中没有值的位置填充 `NaN` 值。它还提供了一种填充数据框中缺失值的方法。除非新索引等同于当前索引并且 `copy=False`，否则将生成一个新对象。
...
- `labels`: 新标签/索引要符合。最好是一个 `Index` 对象，以避免重复数据。
- `axis`: {0 或 ‘index’, 1 或 ‘columns’}
- `method`: {None, ‘backfill’/’bfill’, ‘pad’/’ffill’, ‘nearest’}, 可选的。
- `copy`: 返回一个新的对象，即使传递的索引是相同的。
- `level`: 跨级别广播，匹配传递的多索引级别上的索引值。
- `limit`: 匹配位置的索引值最符合方程式 `abs(index[stepper] - target) <= tolerance`。
...
使用 `reindex_axis()` 函数在索引轴上重新索引数据框。默认情况下，新索引中在 dataframe 中没有相应记录的值被分配给 `NaN`。
