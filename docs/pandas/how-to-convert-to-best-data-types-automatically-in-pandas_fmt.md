...
> 原文: [https://www.geeksforgeeks.org/how-to-convert-to-best-data-types-automatically-in-pandas/](https://www.geeksforgeeks.org/how-to-convert-to-best-data-types-automatically-in-pandas/)

**先决条件:** `pandas`

默认情况下，`pandas`的数据类型是`int`、`float`和`object`。当我们在`pandas`中加载或创建任何`Series`或`DataFrame`时，`pandas`默认情况下会为列和系列分配必要的数据类型。

我们将使用`pandas`的`convert_dtypes()`函数自动将默认分配的数据类型转换为最佳数据类型。使用`convert_dtypes()`有一个很大的好处——它支持缺失值`pd.NA`和`NaN`一起。`pandas` 1.1.4 版本支持。
...
df = pd.DataFrame({"Column_1": pd.Series([1, 2, 3], dtype=np.dtype("int32")),
                   ...
                   # Column_2 datatype is object
                   "Column_3": pd.Series([True, False, np.nan], 
                                         dtype=np.dtype("object")),
                   # Column_3 datatype is object
