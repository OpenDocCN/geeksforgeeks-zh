...
> 原文：[https://www.geeksforgeeks.org/how-to-drop-columns-with-nan-values-in-pandas-dataframe/](https://www.geeksforgeeks.org/how-to-drop-columns-with-nan-values-in-pandas-dataframe/)

Nan（非数字）是一个浮点值，不能转换为其他数据类型，只能浮点。在数据分析中，Nan 是为了正确分析数据集而必须删除的不必要的值。在本文中，我们将讨论如何删除熊猫数据框中具有 Nan 值的列。我们有一个被称为 `DataFrame.dropna()` 的功能为具有 Nan 值的降列。
...
## 示例 4：使用 `subset` 属性删除某个标签索引下具有任何 NaN/NaT 值的所有列。
