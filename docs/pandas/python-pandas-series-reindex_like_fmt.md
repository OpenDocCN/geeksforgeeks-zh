...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-reindex_like/](https://www.geeksforgeeks.org/python-pandas-series-reindex_like/)
...
**参数:**
- `other`: 其行索引和列索引用于定义该对象的新索引。
- `method`: 用于填充重新索引的数据框中的孔的方法。
- `copy`: 返回新对象，即使传递的索引相同。
- `limit`: 为不精确匹配填充的连续标签的最大数量。
- `tolerance`: 不精确匹配的原始标签和新标签之间的最大距离。
...
使用 `Series.reindex_like()` 函数基于另一个对象重新索引给定的序列对象。
