# Python | `Series.argsort()`

> 原文: [https://www.geeksforgeeks.org/python-pandas-series-argsort-2/](https://www.geeksforgeeks.org/python-pandas-series-argsort-2/)

借助 `Series.argsort()`，可以对 pandas 中的 Series 元素进行排序。但是 pandas Series 的主要特点是，我们得到的输出是 Series 中排序元素的**索引值**。在后面的代码演示中，我们将解释如何将输出作为**排序索引值**。
...
## 参数
- `axis`: 对 numpy 有用。
- `kind`: {'mergesort', 'quicksort', 'heapsort'}，默认 'quicksort'
- `order`: 对 numpy 有用。

## 返回
argsorted Series，其中 `-1` 表示存在 `nan` 值的位置。
...
如上所述，如果我们想要处理丢失的值，那么在 `order` 的地方设置为 `None`，它将给出 `-1` 的输出。
