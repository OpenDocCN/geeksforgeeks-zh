...
> 原文: [https://www.geeksforgeeks.org/python-pandas-tseries-offsets-businesshour-rollforward/](https://www.geeksforgeeks.org/python-pandas-tseries-offsets-businesshour-rollforward/)
...
Pandas `tseries.offsets.BusinessHour.rollforward()` 功能仅在不偏移时提供向前到下一个偏移的日期滚动。
...
仅当不在偏移上时，使用 `pandas.tseries.offsets.BusinessHour.rollforward()` 功能将提供的日期向前滚动到下一个偏移。
