...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-autocorr/](https://www.geeksforgeeks.org/python-pandas-series-autocorr/)

Pandas Series 是带有轴标签的一维数组。标签不必是唯一的，但必须是可散列的类型。该对象支持基于整数和基于标签的索引，并提供了一系列方法来执行涉及索引的操作。

`Series.autocorr()` 函数计算滞后-N 自相关。这种方法计算数列与其移位自身之间的皮尔逊相关。
