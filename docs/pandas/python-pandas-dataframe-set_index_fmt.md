...
> 原文: [https://www.geeksforgeeks.org/python-pandas-dataframe-set_index/](https://www.geeksforgeeks.org/python-pandas-dataframe-set_index/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。
`DataFrame.set_index()` 是一种将列表、序列或数据框设置为数据框索引的方法。制作数据框时也可以设置索引列。但是有时一个数据帧是由两个或多个数据帧组成的，因此以后的索引可以使用这种方法来更改。
...
- `keys`: 列名或列名列表。
- `drop`: 布尔值，如果为真，则删除用于索引的列。
- `append`: 如果为真，则将该列追加到现有索引列。
- `inplace`: 如果为真，则在数据框中进行更改。
...
## 代码#3: 在 Pandas DataFrame 中设置单个浮点列作为索引
...
## 代码#4: 在 Pandas DataFrame 中将三列设置为多索引
...
在上例中，我们将列 `Name`、`City` 和 `ID` 设置为数据框的多索引。