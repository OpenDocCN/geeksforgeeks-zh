...
> 原文：[https://www.geeksforgeeks.org/split-a-text-column-into-two-columns-in-pandas-dataframe/](https://www.geeksforgeeks.org/split-a-text-column-into-two-columns-in-pandas-dataframe/)

让我们看看如何在熊猫数据框中将一个文本列分成两列。

## 方法#1：使用 `Series.str.split()` 功能

将 `Name` 列拆分为两个不同的列。默认情况下，通过 `str.split()` 功能在单个空间的基础上进行拆分。
...
将 `Name` 列分别拆分为 `First` 和 `Last` 列，并将其添加到现有的数据框中。
...
将 `Name` 列拆分为两个不同的列，分别命名为 `First` 和 `Last`，然后将其添加到现有的数据框中。
