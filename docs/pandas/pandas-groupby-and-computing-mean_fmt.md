...
> 原文：`https://www.geeksforgeeks.org/pandas-groupby-and-computing-mean/`

`pandas`是建立在`numpy`库之上的开源库。它是一个Python包，提供了各种数据结构和操作来操作数字数据和时间序列。它主要是流行的，因为导入和分析数据容易得多。`pandas`速度快，为用户带来高性能&生产力。
