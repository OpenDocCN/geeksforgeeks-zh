...
Pandas 的 `Index.all()` 功能检查索引中的所有元素是否为真。如果未指定轴，则返回一个布尔值。如果索引中的每个值都为真，则返回真。如果索引中的任何值不为真，则返回 false。
...
## 示例 2
使用 `Index.all()` 功能检查索引中的所有值是否都为真。在索引中，我们有一些 0 值。
