...
`DatetimeIndex.floor()`功能将数据落地到指定频率。该函数以目标频率为输入。它返回一个新的日期时间索引对象。
...
使用`DatetimeIndex.floor()`功能将 `DatetimeIndex` 对象的数据拖到指定的频率。
...
使用`DatetimeIndex.floor()`函数将 `DatetimeIndex` 对象的数据拖到指定的频率。
