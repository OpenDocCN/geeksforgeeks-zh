...
> 原文：[https://www.geeksforgeeks.org/creating-views-on-pandas-dataframe/](https://www.geeksforgeeks.org/creating-views-on-pandas-dataframe/)

很多时候，在进行数据分析时，我们处理的是一个庞大的数据集，有很多属性。所有的属性不一定都同等重要。因此，我们希望只处理数据框中的一组列。为此，让我们看看如何在数据框上创建视图，只选择那些我们需要的列，而留下其余的。

有关代码中使用的 CSV 文件的链接，请单击此处。

## 解决方案 #1

可以通过删除所有不需要的列来选择数据框中的一组列。
