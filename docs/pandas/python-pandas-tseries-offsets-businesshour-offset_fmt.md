...
> 原文: [https://www.geeksforgeeks.org/python-pandas-tseries-offsets-businesshour-offset/](https://www.geeksforgeeks.org/python-pandas-tseries-offsets-businesshour-offset/)
...
Pandas `tseries.offsets.BusinessHour.offset` 属性是 `self._offset` 的别名。它返回一个 `datetime.timedelta` 值。

> **语法:** `pandas.tseries.offsets.BusinessHour.offset`
>
> **参数:** 无
>
> **返回:** `datetime.timedelta`

**示例 #1:** 使用 `pandas.tseries.offsets.BusinessHour.offset` 属性返回给定偏移量的 `self._offset` 值。
...
正如我们在输出中看到的，我们已经成功地创建了一个偏移量，并将其添加到给定的时间戳中。我们也打印了 `self._offset` 值。

**示例 #2:** 使用 `pandas.tseries.offsets.BusinessHour.offset` 属性返回给定偏移量的 `self._offset` 值。
...
现在，我们将业务时间偏移量添加到给定的时间戳对象中，以增加日期时间值。我们将打印 `self._offset` 值。
