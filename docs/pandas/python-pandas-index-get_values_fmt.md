...
> 原文：[https://www.geeksforgeeks.org/python-pandas-index-get_values/](https://www.geeksforgeeks.org/python-pandas-index-get_values/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Pandas` 的 `Index.get_values()` 函数以数组形式返回索引数据。对于多索引数组，它返回一维数组。
