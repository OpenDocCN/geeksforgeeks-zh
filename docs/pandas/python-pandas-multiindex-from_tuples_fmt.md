...
`Pandas` 的 `MultiIndex.from_tuples()` 函数用于将元组列表转换为多索引。这是我们构建多索引的几种方法之一。

## 语法

`MultiIndex.from_tuples(元组，排序顺序=无，名称=无)`
...
使用 `MultiIndex.from_tuples()` 函数使用 python 元组构建多索引。
...
使用 `MultiIndex.from_tuples()` 函数使用 python 元组构建 `MultiIndex`。
