...
> 原文: [https://www.geeksforgeeks.org/python-pandas-dataframe-truediv/](https://www.geeksforgeeks.org/python-pandas-dataframe-truediv/)

## 概述
Pandas `DataFrame` 是一个二维可变大小、潜在异构的表格数据结构，带有标记轴（行和列）。算术运算在行标签和列标签上对齐。它可以被认为是 `Series` 对象的类似字典的容器。这是 pandas 的主要数据结构。

`DataFrame.truediv()` 方法执行数据帧与其他元素的浮点除法。它相当于 `dataframe / other`，但支持用 `fill_value` 替换其中一个输入中缺失的数据。
...
### 示例 1：标量除法
使用 `DataFrame.truediv()` 函数以标量元素方式对给定的数据帧进行除法。同时在所有缺失值的地方填入 100。
...
正如我们在输出中看到的那样，`DataFrame.truediv()` 函数已经成功地对给定的数据帧进行了标量除法。

### 示例 2：列表除法
使用 `DataFrame.truediv()` 功能使用列表对给定的数据帧进行除法。
...
现在我们将使用 `DataFrame.truediv()` 函数使用列表对给定的数据帧进行除法。
...
正如我们在输出中看到的那样，`DataFrame.truediv()` 函数已经成功地执行了给定数据帧的列表除法。