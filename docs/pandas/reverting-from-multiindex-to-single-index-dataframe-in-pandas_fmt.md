...
>
> **返回**:(数据框或无)带有新索引的数据框，如果 `inplace=True`，则返回无。
...
使用 `level` 关键字将值传递给函数，该关键字接受级别列表，我们希望从索引位置恢复该列表。正如我们所知，多索引形成了索引的层次结构，这就是为什么它们也被称为层次索引。在本数据框中，“地区”是 `level(0)` 指数或主要指数，“州”是 `level(1)` 指数，“个人”是 `level(2)` 指数。仅通过保留“状态”索引并恢复“区域”和“个人”索引，将数据框转换为单一索引。
...
**注意**:如果我们希望让 Dataframe 无索引，或者我们不想让任何列作为索引，那么在这种情况下，我们可以在 `reset_index()` 中传递所有索引或级别的名称，让 Dataframe 无索引。