...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-get_dtype_counts/](https://www.geeksforgeeks.org/python-pandas-series-get_dtype_counts/)
...
## 示例#1:
使用 `Series.get_dtype_counts()` 函数返回给定系列对象中唯一数据类型的计数。
...
## 示例 2:
使用 `Series.get_dtype_counts()` 函数返回给定序列对象中唯一数据类型的计数。
