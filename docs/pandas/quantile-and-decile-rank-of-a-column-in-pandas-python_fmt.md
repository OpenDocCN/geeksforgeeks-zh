...
> *   `labels`: 用作结果箱的标签。如果设置为假，它只返回容器的整数指示符。如果为真，则引发错误。默认情况下，它设置为“无”。
> *   `retbins`: (可选)它是一个布尔值，当设置为真时返回(bin，标签)。
> *   `precision`: (可选)存储和显示箱柜标签的精度。
> *   `duplicates`: (可选)如果箱边不唯一，则增加值错误或删除非唯一。
