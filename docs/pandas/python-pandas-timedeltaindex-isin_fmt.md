...
`TimedeltaIndex.isin()` 函数计算一个布尔数组，该数组表示每个索引值是否在传递的值集中找到。如果在传递的值中找到索引值，则标记为 `True`，否则标记为 `False`。
...
## 返回:
`is_contained` : ndarray(布尔数据类型)

## 示例#1:
使用 `TimedeltaIndex.isin()` 函数检查给定时间增量索引对象的元素是否存在于传递的值列表中。
