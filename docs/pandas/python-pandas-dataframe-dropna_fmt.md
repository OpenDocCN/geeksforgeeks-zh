...
- **subset**: 它是一个数组，将删除过程限制在通过列表传递的行/列。
- **inplace**: 是一个布尔值，如果为 `True`，则改变数据框本身。

有关代码中使用的 CSV 文件的链接，请单击[此处](https://drive.google.com/open?id=1Uzdua5o61cfXMz1K16NNcwLFqG91J6W4)。
...
制作了两个数据帧。所有值都=`None`的列将被添加到新的数据框中。验证列名以查看是否正确插入了空列。然后比较删除 `NaN` 值前后的列数。
