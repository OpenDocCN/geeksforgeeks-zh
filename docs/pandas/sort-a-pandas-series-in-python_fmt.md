...
> 原文: [https://www.geeksforgeeks.org/sort-a-pandas-series-in-python/](https://www.geeksforgeeks.org/sort-a-pandas-series-in-python/)

`Series` 是一维标记数组，能够保存整数、字符串、浮点、python 对象等类型的数据。轴标签统称为 `索引`。

现在，让我们来看一个对熊猫系列进行分类的程序。

对熊猫系列进行排序时，使用 `Series.sort_values()` 方法。
