...
> 原文: [https://www.geeksforgeeks.org/streamlined-data-ingestion-with-pandas/](https://www.geeksforgeeks.org/streamlined-data-ingestion-with-pandas/)
...
*   使用熊猫方法 `read_csv`
    ...
    *   参数–字符串格式，包含文件的路径及其名称，或远程服务器上的网址。它读取文件数据，并将其转换为有效的二维 dataframe 对象。此方法可用于读取数据，如 `.csv` 以及 `.txt` 文件格式。
...
*   使用熊猫方法 `read_excel`。
    ...
    *   参数–该方法接受字符串格式的文件路径及其名称作为参数。文件可以在远程服务器上，也可以在本地计算机上。它读取文件数据，并将其转换为有效的二维数据框对象。此方法可用于读取 `.xlsx` 以及 `.xls` 文件格式。
...
加载 JavaScript 对象符号文件 (`.json`) 我们将遵循以下步骤:
...
网页通常由 HTML 元素组成。根据数据显示的目的，在浏览器上有不同的 HTML 标签，如 `<head>`、`<title>`、`<table>`、`<div>`。我们可以将呈现在网页中的 `<table>` 元素之间的内容转移到熊猫数据框对象。遵循以下步骤–

*   选择在 `<table>` 开始和结束标签之间的中存在的所有元素。将其赋给一个 Python 变量。
*   使用熊猫方法 `read_html`。
    *   使用的方法–`read_html(在标记内的字符串)`
    *   参数–该方法接受字符串变量，包含出现在标记之间的元素。它读取元素，遍历 `<table>`、`<thead>` 和 `<tbody>` 标签，并将其转换为列表对象。列表对象的第一个元素是所需的 dataframe 对象。
...
*   使用“SQLite 数据库浏览器工具”或任何类似工具准备一个示例 SQLite 表。这些工具允许轻松创建与 SQLite 兼容的数据库文件。数据库文件有一个 `.db` 文件扩展名。在这个例子中，我们有 `novels.db` 文件，包含一个名为 `novels` 的表。此表包含有关小说的信息，如小说名称、价格、流派等。
...
*   使用熊猫方法 `read_sql_query`。
    ...
    *   参数–该方法接受以下参数
        ...
        *   连接对象–由 `connect` 方法返回的连接对象。`read_sql_query` 方法将查询的结果行转换为 dataframe 对象。
...
`novels` 表，如图所示，使用数据库浏览器的 SQLite 工具

编写以下代码来转换熊猫数据框对象中的 `novels` 表:
