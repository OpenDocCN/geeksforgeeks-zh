...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-kurtosis/](https://www.geeksforgeeks.org/python-pandas-series-kurtosis/)
...
- `level`: 如果轴是多索引(分层)，则沿着特定级别计数，折叠成标量。
...
使用 `Series.kurtosis()` 函数查找给定序列对象中底层数据的峰度。
...
使用 `Series.kurtosis()` 函数查找给定序列对象中底层数据的峰度。给定的序列对象包含一些缺失的值。
