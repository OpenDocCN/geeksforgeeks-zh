...
> 原文: [https://www.geeksforgeeks.org/python-pandas-datetimeindex-to_period/](https://www.geeksforgeeks.org/python-pandas-datetimeindex-to_period/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`DatetimeIndex.to_period()` 函数用于将给定的日期时间索引转换为特定频率的周期索引。该函数基本上将 `DatetimeIndex` 转换为 `PeriodIndex`。

> **语法:** `DatetimeIndex.to_period(freq=None)`
> **参数:**
> `freq`: Pandas 偏移字符串之一或偏移对象。将被默认推断。
> **返回:** `PeriodIndex`

## 示例 1

使用 `DatetimeIndex.to_period()` 函数将 `DatetimeIndex` 对象的数据转换为 `PeriodIndex`。
...
正如我们在输出中看到的，函数已经将 `DatetimeIndex` 对象转换为 `PeriodIndex` 对象。
