...
Pandas 的 `Index.fillna()` 功能用指定值填充 NA/NaN 值。它只需要一个标量值来填充索引中所有缺失的值。该函数返回一个新对象，该对象的缺失值由传递的值填充。
...
- `downcast`: 项的字典 -> 如果可能的话要向下转换的数据类型，或者尝试向下转换为适当的相等类型的字符串 `'infer'`(例如，如果可能的话从 `float64` 到 `int64`)

## 返回
已填充的 `%(klass)s`

## 示例 1
使用 `Index.fillna()` 函数填充索引中所有缺失的值。
...
## 示例 2
使用 `Index.fillna()` 函数填充索引中所有缺失的字符串。
