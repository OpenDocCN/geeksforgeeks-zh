...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timestamp-timetuple/](https://www.geeksforgeeks.org/python-pandas-timestamp-timetuple/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Pandas` 的 `Timestamp.timetuple()` 函数为给定的时间戳对象返回一个时间元组。返回的元组包含从年、月到小时和秒的值。时间元组与 `time.localtime()` 兼容。
...
## 示例 #1
使用 `Timestamp.timetuple()` 函数返回给定时间戳对象的时间元组。
