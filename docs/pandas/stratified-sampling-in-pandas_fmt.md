...
> 原文：[https://www.geeksforgeeks.org/stratified-sampling-in-pandas/](https://www.geeksforgeeks.org/stratified-sampling-in-pandas/)
...
**不成比例抽样：** 使用 `pandas` `groupby()` 方法，根据学生的年级将他们分成 A、B、C 组，并使用 `sample()` 函数从每个年级组中随机抽取 2 名学生
...
**比例抽样：** 采用 `pandas` `groupby()` 方法，根据学生的年级将学生分成 A、B、C 组，并根据人口比例从每组中随机抽取样本。总样本量为总体的 60%（`frac=0.6`）
...
在本例中，我们将从[训练](https://media.geeksforgeeks.org/wp-content/cdn-uploads/20210924200234/train.csv)数据集创建样本数据。泰坦尼克号是一艘英国客轮，撞上冰山后沉入北大西洋。该数据集包含所有登上泰坦尼克号的乘客的信息，一名乘客要么死亡，要么在事故中幸存，因此我们将使用 `Survived` 列作为我们的分层列。
...
检查死亡或幸存乘客的比例/百分比，这是死亡或存活乘客的数量 / 乘客总数 * 100
...
**第 4 步：** 按比例抽取 1%（`frac=0.01`）的乘客（0.6%死亡，0.4%存活）
