...
> 原文: [https://www.geeksforgeeks.org/replace-negative-values-with-latest-preceding-positive-value-in-pandas-dataframe/](https://www.geeksforgeeks.org/replace-negative-values-with-latest-preceding-positive-value-in-pandas-dataframe/)
...
熊猫 `dataframe.ffill()` 方法用于填充数据帧中缺失的值。这个方法中的“ffill”代表“向前填充”，它向前传播最后一次遇到的有效观察。`ffill()` 函数用于沿指定的索引轴填充缺失的值。此方法具有以下语法：
...
data_frame = data_frame.mask(data_frame.lt(0)).ffill().fillna(0).astype('int32')
