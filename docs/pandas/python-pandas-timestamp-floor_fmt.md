...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timestamp-floor/](https://www.geeksforgeeks.org/python-pandas-timestamp-floor/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Timestamp.floor()` 函数返回一个新的时间戳到这个分辨率。该函数将所需的时间序列频率作为输入。
...
使用 `Timestamp.floor()` 功能将给定的时间戳对象设定为每日时间序列频率。
...
使用 `Timestamp.floor()` 功能将给定的时间戳对象设定为精确的时间序列频率。
