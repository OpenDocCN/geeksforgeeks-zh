# Python Pandas TimedeltaIndex.empty

> 原文: [https://www.geeksforgeeks.org/python-pandas-timedeltaindex-empty/](https://www.geeksforgeeks.org/python-pandas-timedeltaindex-empty/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`TimedeltaIndex.empty` 属性返回一个布尔值。如果应用它的时间增量索引对象不为空，则返回 `False`。

> **语法**: `TimedeltaIndex.empty`
>
> **返回**: `bool`

## 示例 1

使用 `TimedeltaIndex.empty` 属性检查给定的时间增量索引对象是否为空。
