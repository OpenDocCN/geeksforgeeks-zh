...
> 原文: [https://www.geeksforgeeks.org/python-pandas-dataframe-drop_duplicates/](https://www.geeksforgeeks.org/python-pandas-dataframe-drop_duplicates/)
...
> **参数:**
> `subset`: `subset` 取列或列标签列表。它的默认值是 `None`。在传递列后，它将只考虑重复的列。
> `keep`: `keep` 是控制如何考虑重复值。它只有三个不同的值，默认值是 `'first'`。
> - 如果为 `'first'`，则认为第一个值是唯一的，其余相同的值是重复的。
> - 如果为 `'last'`，则认为最后一个值是唯一的，其余相同的值是重复的。
> - 如果为 `False`，则认为所有相同的值都是重复的。
> `inplace`: 布尔值，如果为 `True`，则删除重复的行。
