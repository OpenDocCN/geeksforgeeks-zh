...
> 原文: [https://www.geeksforgeeks.org/create-a-new-column-in-pandas-dataframe-based-on-the-existing-columns/](https://www.geeksforgeeks.org/create-a-new-column-in-pandas-dataframe-based-on-the-existing-columns/)
...
我们可以使用 `DataFrame.apply()` 功能来实现这个任务。
...
现在，我们将在对现有的 `Cost` 列应用 10%的折扣后，创建一个名为 `Discounted_Price` 的新列。
...
我们可以通过在所需的列元素上直接执行所需的操作来获得相同的结果。
