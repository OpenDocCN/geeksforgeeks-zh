...
> 原文：[https://www.geeksforgeeks.org/exporting-dta-file-using-pandas-dataframe-to_stata-function-in-python/](https://www.geeksforgeeks.org/exporting-dta-file-using-pandas-dataframe-to_stata-function-in-python/)

此方法用于将数据框写入 Stata 数据集文件。“dta”文件包含一个 Stata 数据集。DTA 文件是一个数据库文件，由 IWIS 链工程公司使用。

> 语法：`DataFrame.to_stata(path, convert_dates=None, write_index=True, timestamp=None)`
>
> 参数：
> *   `path`：字符串、缓冲区或路径对象
> *   `convert_dates`：字典
...
> *   `timestamp`：日期时间
>
> 返回：`DataFrame` 对象到 Stata dta 格式。意味着回归 .dta 文件。
