...
> 原文：[https://www.geeksforgeeks.org/how-to-filter-dataframe-rows-based-on-the-date-in-pandas/](https://www.geeksforgeeks.org/how-to-filter-dataframe-rows-based-on-the-date-in-pandas/)
...
要根据日期筛选行，首先将数据框中的日期格式化为 `datetime64` 类型。然后使用熊猫包中的 `DataFrame.loc[]` 和 `dataframe.query()` 功能指定过滤条件。因此，获取数据子集，即过滤后的数据帧。让我们看一些同样的例子。
...
使用 `DateTimeIndex.dt` 访问单独的日期时间属性，如年、月、日、工作日、小时、分钟、秒、微秒等。作为条件在 `loc[]` 功能如下。
...
使用 `dataframe.query()` 函数根据日期过滤数据，`query()` 函数过滤熊猫数据框，并通过在引号内指定条件来选择行。如下图，`query()` 里面的条件是选择日期在 8 月份的数据（指定日期范围）。默认情况下，数据框的列放在查询命名空间中，因此无需索引即可访问日期列（只需指定列名）。
