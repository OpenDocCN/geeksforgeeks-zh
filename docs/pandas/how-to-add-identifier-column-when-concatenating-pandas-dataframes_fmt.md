...
> 原文: [https://www.geeksforgeeks.org/how-to-add-identifier-column-when-concatenating-pandas-dataframes/](https://www.geeksforgeeks.org/how-to-add-identifier-column-when-concatenating-pandas-dataframes/)

在处理一些数据时，我们通常希望连接两个或更多的数据帧。因此，当我们连接这些数据帧时，我们实际上需要提供一个标识符列来标识连接的数据帧。在本文中，我们将借助示例来了解如何做到这一点。

## 例 1

要添加标识符列，我们需要将标识符指定为 `concat()` 函数中参数 `keys` 的列表，该函数创建一个新的多索引数据帧，其中两个数据帧连接在一起。现在我们将使用 `reset_index` 将多索引数据帧转换为常规熊猫数据帧。
