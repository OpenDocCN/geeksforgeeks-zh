...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-dt-tz_localize/](https://www.geeksforgeeks.org/python-pandas-series-dt-tz_localize/)
...
使用`Series.dt.tz_localize()`函数将系列中的 tz-naive 日期时间值本地化为 tz-aware。
...
现在我们将使用`Series.dt.tz_localize()`函数将给定的天真系列本地化为`'US/Eastern'`。
...
使用`Series.dt.tz_localize()`函数将给定的序列对象作为本机 python datetime 对象的数组返回。
...
现在我们将使用`Series.dt.tz_localize()`函数将给定的天真系列本地化为`'Europe/Berlin'`。
