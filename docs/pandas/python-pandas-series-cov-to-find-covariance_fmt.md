...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-cov-to-find-covariance/](https://www.geeksforgeeks.org/python-pandas-series-cov-to-find-covariance/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Series.cov()` 用于求两个序列的协方差。在下面的例子中，使用 Pandas 方法和手动方法找到协方差，然后比较答案。

要了解更多关于协方差的信息，请点击[这里](https://www.geeksforgeeks.org/program-find-covariance/)。
...
**返回类型:** 浮点值，返回调用方序列和传递序列的协方差。

## 示例

在本例中，使用 Pandas 创建了两个列表并将其转换为序列（`Series()` 方法）。计算了两个序列的平均值，并创建了一个函数来手动查找协方差。`Pandas.cov()` 也被应用，两种方式的结果都存储在变量中，并打印出来以比较输出。
