...
`DataFrame.rdiv()` 函数计算数据帧和其他元素的浮点除法（二进制运算符 `rtruediv`）。其他对象可以是标量、`pandas` 系列或 `pandas` 数据帧。该功能本质上与执行 `other / dataframe` 相同，但支持用 `fill_value` 替换其中一个输入中缺失的数据。
...
使用 `rdiv()` 函数将一个数据帧与另一个包含 `NaN` 值的数据帧相除。
