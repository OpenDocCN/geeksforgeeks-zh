...
> 原文: [https://www.geeksforgeeks.org/python-pandas-tseries-offsets-businessday-kwds/](https://www.geeksforgeeks.org/python-pandas-tseries-offsets-businessday-kwds/)

日期偏移量是 Pandas 中用于日期范围的一种标准的日期增量。就我们传递的关键字参数（`args`）而言，它的工作原理与 `relativedelta` 完全一样。日期偏移的工作方式如下，每个偏移指定一组符合日期偏移的日期。例如，`Bday` 将该集合定义为工作日（M-F）的日期集合。
...
## 示例 #1
使用 `pandas.tseries.offsets.BusinessDay.kwds` 属性返回应用于给定工作日偏移对象的关键字参数。
