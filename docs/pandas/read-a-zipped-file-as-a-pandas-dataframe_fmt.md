...
> 原文: [https://www.geeksforgeeks.org/read-a-zipped-file-as-a-pandas-dataframe/](https://www.geeksforgeeks.org/read-a-zipped-file-as-a-pandas-dataframe/)

在本文中，我们将尝试找出如何使用`pandas`数据框从`zip`文件中读取数据。
...
**方法#1:** 使用`pandas.read_csv()`方法中的`compression='zip'`参数。

通过将`read_csv()`方法中的`compression`参数指定为`zip`，`pandas`将首先解压缩`zip`，然后从压缩文件中的`csv`文件创建数据帧。
