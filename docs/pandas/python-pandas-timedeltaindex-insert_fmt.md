...
> 原文链接: [https://www.geeksforgeeks.org/python-pandas-timedeltaindex-insert/](https://www.geeksforgeeks.org/python-pandas-timedeltaindex-insert/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`TimedeltaIndex.insert()` 函数返回基础数据作为给定时间增量索引对象的数组。

> **语法:** `TimedeltaIndex.insert(loc, item)`
>
> **参数:**
> `loc` : int
> `item`: 如果不是 Python 日期时间或类似 numpy 的整数，返回的 Index 数据类型将是对象而不是日期时间。
>
> **返回:** `new_index` : Index

## 示例 1

使用 `TimedeltaIndex.insert()` 函数在时间增量索引对象中插入新值。它返回一个新对象，新元素被插入到所需的位置。
