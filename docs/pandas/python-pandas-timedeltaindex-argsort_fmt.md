# Pandas TimedeltaIndex.argsort() 用法详解

> 原文链接：[https://www.geeksforgeeks.org/python-pandas-timedeltaindex-argsort/](https://www.geeksforgeeks.org/python-pandas-timedeltaindex-argsort/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`TimedeltaIndex.argsort()` 函数返回对索引及其底层数据进行排序的索引。默认情况下，顺序是非递减的。

**语法：** `TimedeltaIndex.argsort(*args, **kwargs)`
...
使用 `TimedeltaIndex.argsort()` 函数查找给定时间增量索引对象的元素顺序，该顺序将对对象中的基础数据进行排序。
