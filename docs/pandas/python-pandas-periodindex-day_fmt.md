# Pandas PeriodIndex.day 属性详解

> 原文: [https://www.geeksforgeeks.org/python-pandas-periodindex-day/](https://www.geeksforgeeks.org/python-pandas-periodindex-day/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。
`Pandas` 的 `PeriodIndex.day` 属性返回一个 `Index` 对象，该对象包含给定 `PeriodIndex` 对象中每个元素的天数值。
