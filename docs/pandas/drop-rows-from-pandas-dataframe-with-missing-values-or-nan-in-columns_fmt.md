...
> 原文: [https://www.geeksforgeeks.org/drop-rows-from-pandas-dataframe-with-missing-values-or-nan-in-columns/](https://www.geeksforgeeks.org/drop-rows-from-pandas-dataframe-with-missing-values-or-nan-in-columns/)
...
熊猫认为`None`和`NaN`在表示缺失或空值时基本上可以互换。为了从数据框中删除空值，我们使用了`dropna()`函数。该函数以不同的方式删除具有空值的数据集的行/列。
...
**参数:**
*   `axis`: `axis`取行/列的 int 或 string 值。整数的输入可以是 0 或 1，字符串的输入可以是“index”或“columns”。
