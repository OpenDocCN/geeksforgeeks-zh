...
> 原文: [https://www.geeksforgeeks.org/python-pandas-datetimeindex-to_series/](https://www.geeksforgeeks.org/python-pandas-datetimeindex-to_series/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

## 函数描述

`DatetimeIndex.to_series()` 函数创建一个系列，其索引和值等于索引键，可用于映射，以返回基于索引的索引器。
...
**参数:**
- `keep_tz` : 返回保留时区的数据
- `index` : 结果序列的索引。如果为 `None`，默认为原始索引
- `name` : 结果系列的名称。如果为 `None`，则默认为原始索引的名称

**返回:** 系列

## 示例 #1

使用 `DatetimeIndex.to_series()` 函数从给定的日期时间索引对象创建一个序列对象。同时设置序列的索引值。
...
使用 `DatetimeIndex.to_series()` 函数从给定的 `DatetimeIndex` 对象创建一个系列对象。同时设置序列的索引值。
