...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timestamp-dayofyear/](https://www.geeksforgeeks.org/python-pandas-timestamp-dayofyear/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Timestamp.dayofyear` 属性返回给定时间戳对象的一年中的某一天。天数从 1 到 365。其中 1 是一年的第一天，365 是一年的最后一天。

> **语法:** `Timestamp.dayofyear`
> **参数:** 无
> **返回:** 一年中的某一天

## 示例 #1

使用 `Timestamp.dayofyear` 属性在给定的 `Timestamp` 对象中查找一年中的某一天。
