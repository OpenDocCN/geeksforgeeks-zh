...
Pandas 提供了两个非常有用的功能，我们可以用它们来对数据进行分组。

*   `resample()` — 该功能主要用于时间序列数据。这是一种方便的时间序列频率转换和重采样方法。对象必须具有类似日期时间的索引(日期时间索引、周期索引或时间增量索引)，或者将类似日期时间的值传递给 `on` 或 `level` 关键字。重采样根据实际数据生成唯一的采样分布。

> **语法:** `DataFrame.resample(rule, how=None, axis=0, fill_method=None, closed=None, label=None, convention='start', kind=None, loffset=None, limit=None, base=0, on=None, level=None)`
>
> **参数:**
>
...
> **语法:** `dataframe.groupby(pd.Grouper(key, level, freq, axis, sort, label, convention, base, loffset, origin, offset))`
>
> **参数:**
>
> *   **`key`:** Select the target column to be grouped.
> *   **`level`:** the level of the target index.
> *   **`freq`:** group by specify the frequency. If the target column is an object with similar date and time,
> *   **`axis`:** axis to use.
> *   **`Convention`:** If grouper is PeriodIndex and freq parameter is passed,
> *   **`base`:** only works when freq is passed.
> *   **`loffset`:** only works when freq is passed.
> *   **`Origin`:** Timestamp is based on
> *   **`Offset`:** The offset time increment is added to the origin
