...
`pandas`的`DataFrame.eq()`是用于灵活比较的包装。它提供了一种将`DataFrame`对象与常量、序列或其他`DataFrame`对象进行比较的便捷方法。

> `DataFrame.eq(其他，轴= '列'，级别=无)`
>
> **参数:**
> `其他`:系列、数据框或常量
> `轴`: {0，1，`索引`，`列`}
