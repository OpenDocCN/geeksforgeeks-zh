...
> 原文：[https://www.geeksforgeeks.org/count-values-in-pandas-dataframe/](https://www.geeksforgeeks.org/count-values-in-pandas-dataframe/)
...
> **参数：**
>
> *   **axis** {0 或 ‘index’， 1 或 ‘columns’}：如果 `axis=0` 或 `axis=‘index’`，则默认为每列生成 0 计数，如果 `axis=1` 或 `axis=‘columns’`，则为每行生成计数。
> *   **level** (int 或 str，可选)：如果轴是多索引，沿特定级别计数，折叠成数据框。字符串指定级别名称。
...
在这一步中，我们只是简单地使用 `count()` 函数对不同列的所有值进行计数。
...
如果我们想计算与行相关的所有值，那么我们必须传递 `axis=1` 或 `‘columns’`。
