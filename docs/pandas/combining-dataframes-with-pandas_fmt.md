...
> 原文: [https://www.geeksforgeeks.org/combining-dataframes-with-pandas/](https://www.geeksforgeeks.org/combining-dataframes-with-pandas/)

`pandas`数据框由三个主要组成部分组成，即数据、行和列。为了组合这些数据帧，`pandas`提供了多种功能，如`concat()`和`append()`。
...
现在，连接两个数据帧，我们将使用`concat()`来组合两个数据帧。如果`ignore_index=True`，`df`的索引将是连续的。
...
`dataframe.append()`方法执行合并两个数据帧的操作，类似于`concat()`方法。
