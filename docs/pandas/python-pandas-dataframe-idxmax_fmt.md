...
> 原文链接: https://www.geeksforgeeks.org/python-pandas-dataframe-idxmax/
...
使用 `idxmax()` 函数进行函数运算，沿指数轴找到最大值的指数。
...
如果我们查看 dataframe 中的值，我们可以验证函数返回的结果。该函数返回一个 pandas Series 对象，该对象包含每列中最大值的索引。

## 示例 2

使用 `idxmax()` 函数沿列轴查找最大值的索引。数据框包含 `NA` 值。
...
输出是 pandas Series，包含每一行的列标签，列标签具有最大值。