...
> 原文: [https://www.geeksforgeeks.org/extract-week-number-from-date-in-pandas-python/](https://www.geeksforgeeks.org/extract-week-number-from-date-in-pandas-python/)
...
我们也可以通过在‘Date’对象中添加更多的日期来对多个日期进行同样的操作。
...
使用 `date_range()` 和 `to_series()` 从多个日期的日期中提取周数。

*   **`pandas.date_range()`:** It generates all the dates from the start to end date
...
在本例中，我们将使用 `pd.Series()` 生成日期，并使用不同的方式将该系列转换为数据框。

**`pd.Series()`:** 用于创建带有轴标签的一维数组。
