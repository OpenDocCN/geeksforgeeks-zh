# Pandas Period.days_in_month 属性

## 蟒蛇|熊猫期.月 _ 日

> 原文: [https://www.geeksforgeeks.org/python-pandas-period-days_in_month/](https://www.geeksforgeeks.org/python-pandas-period-days_in_month/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Pandas` 的 `Period.days_in_month` 属性返回给定周期对象中一个月中的天数。
