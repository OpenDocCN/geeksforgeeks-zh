...
`Pandas` 的 `Index.equals()` 函数确定两个索引对象是否包含相同的元素。如果它们包含相同的元素，则函数返回 `True`，否则函数返回 `False`，表示两个索引中包含的值不同。
...
使用 `Index.equals()` 功能检查两个索引是否包含相同的元素。
