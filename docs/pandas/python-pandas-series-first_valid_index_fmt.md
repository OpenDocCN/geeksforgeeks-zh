...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-first_valid_index/](https://www.geeksforgeeks.org/python-pandas-series-first_valid_index/)

Pandas Series 是带有轴标签的一维数组。标签不必是唯一的，但必须是可散列的类型。该对象支持基于整数和基于标签的索引，并提供了一系列方法来执行涉及索引的操作。

Pandas `Series.first_valid_index()` 函数返回给定序列对象中第一个非 NA/null 值的索引。
