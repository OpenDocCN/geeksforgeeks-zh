...
Pandas `Series.apply()` 函数调用给定序列对象的每个元素上传递的函数。

> **语法:** `Series.apply(func, convert_dtype=True, args=(), **kwds)`
>
> **参数:**
> - `func`: Python 函数或 NumPy ufunc 来应用。
> - `convert_dtype`: 尝试为 elementwise 函数结果找到更好的 dtype。
> - `args`: 在序列值之后传递给 `func` 的位置参数。
...
如果城市是‘里约’，使用 `Series.apply()` 功能将城市名称更改为‘蒙特利尔’。
...
如果给定系列对象中的值大于 30，则使用 `Series.apply()` 函数返回真，否则返回假。
