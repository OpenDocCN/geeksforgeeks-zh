# Python Pandas Timestamp.days_in_month 属性详解

> 原文: [https://www.geeksforgeeks.org/python-pandas-timestamp-days_in_month/](https://www.geeksforgeeks.org/python-pandas-timestamp-days_in_month/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`pandas.Timestamp.days_in_month` 属性返回时间戳对象中给定日期的月份天数。
