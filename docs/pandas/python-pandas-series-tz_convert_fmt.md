...
## 示例 1
使用 `Series.tz_convert()` 功能将给定系列的时区感知指数转换为目标时区。
...
现在，我们将使用 `Series.tz_convert()` 功能将给定时区索引转换为时区感知索引，并转换为目标时区 `'US/Central'`。
...
## 示例 2
使用 `Series.tz_convert()` 功能将给定序列的时区感知索引转换为目标时区。
...
现在，我们将使用 `Series.tz_convert()` 功能将给定时区索引转换为时区感知索引，并转换为目标时区 `'Europe/Berlin'`。
