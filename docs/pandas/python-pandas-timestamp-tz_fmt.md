...
`pandas.Timestamp.tz` 属性用于检查给定时间戳对象的时区。如果未设置时区，则返回 `None`。
...
使用 `Timestamp.tz` 属性查找给定时间戳对象的时区。
...
正如我们在输出中看到的那样，`Timestamp.tz` 属性返回了 `'US/Central'`，表示给定时间戳对象中的时间基于 `'US/Central'` 时区。
...
正如我们在输出中看到的那样，`Timestamp.tz` 属性返回了 `'Europe/Berlin'`，表示给定 `Timestamp` 对象中的时间基于 `'Europe/Berlin'` 时区。