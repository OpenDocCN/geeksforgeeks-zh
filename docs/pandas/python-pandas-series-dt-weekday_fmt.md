...
`Series.dt`可用于访问系列的值，如 datetimelike，并返回几个属性。Pandas `Series.dt.weekday`属性返回星期几。假设一周从周一开始，用 0 表示，到周日结束，用 6 表示。
...
使用`Series.dt.weekday`属性返回给定序列对象的基础数据中给定日期时间的星期几。
...
使用`Series.dt.weekday`属性返回给定 Series 对象的基础数据中给定日期时间的星期几。
