...
> 原文: [https://www.geeksforgeeks.org/python-pandas-dataframe-bfill/](https://www.geeksforgeeks.org/python-pandas-dataframe-bfill/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`DataFrame.bfill()` 用于向后填充数据集中缺失的值。它将向后填充 Pandas 数据框中的 `NaN` 值。
...
- `limit`: 整数值，要填充的连续 `na` 单元格的数量。

## 示例 1：跨行填充

使用 `bfill()` 函数跨行填充数据框中缺失的 `na` 值。
