...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-transform/](https://www.geeksforgeeks.org/python-pandas-series-transform/)

Pandas Series 是带有轴标签的一维数组。标签不必是唯一的，但必须是可散列的类型。该对象支持基于整数和基于标签的索引，并提供了一系列方法来执行涉及索引的操作。

Pandas `Series.transform()` 函数调用函数（传递的函数）自行生成一个具有转换值的序列，该序列具有与自身相同的轴长。

> **语法:** `Series.transform(func, axis=0, *args, **kwargs)`
>
> **参数:**
> - `func`: 如果是一个函数，必须要么在传递给 Series 时工作，要么在传递给 Series 时工作。
> - `axis`: 与 DataFrame 兼容所需的参数。
...
使用 `Series.transform()` 函数变换给定系列对象的元素。在每个城市名称的末尾添加“_City”。
...
使用 `DataFrame.transform()` 函数转换给定数据帧的数据。每连门票费用增加 1000。
