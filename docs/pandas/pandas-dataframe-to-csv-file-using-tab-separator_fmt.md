...
> 原文：[https://www.geeksforgeeks.org/pandas-dataframe-to-csv-file-using-tab-separator/](https://www.geeksforgeeks.org/pandas-dataframe-to-csv-file-using-tab-separator/)

让我们看看如何使用标签分隔符将数据帧转换为 CSV 文件。我们将使用`to_csv()`方法将数据帧保存为 csv 文件。要保存带有制表符分隔符的 DataFrame，我们必须在`to_csv()`方法中传递`"\t"`作为`sep`参数。
