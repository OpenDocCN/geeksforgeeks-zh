...
> 原文: [https://www.geeksforgeeks.org/python-pandas-dataframe-series-tail-method/](https://www.geeksforgeeks.org/python-pandas-dataframe-series-tail-method/)

## 介绍
Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Pandas` 的 `tail()` 方法用于返回数据框或序列的底部 n（默认为 5）行。
...
## 示例数据
要下载下例使用的数据集，点击这里 [。](https://media.geeksforgeeks.org/wp-content/uploads/nba.csv)
