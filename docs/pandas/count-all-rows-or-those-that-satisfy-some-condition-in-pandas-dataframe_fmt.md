...
> 原文：[https://www.geeksforgeeks.org/count-all-rows-or-those-that-satisfy-some-condition-in-pandas-dataframe/](https://www.geeksforgeeks.org/count-all-rows-or-those-that-satisfy-some-condition-in-pandas-dataframe/)

让我们看看如何计算数据框中的所有行数或熊猫中满足条件的行数。

## 1) 使用`Dataframe.shape`计算熊猫数据框中的所有行
...
## 2) 使用`Dataframe.index`统计熊猫数据帧中的所有行
...
## 3) 使用`Dataframe.apply()`对熊猫数据框中满足条件的行进行计数
