...
> 原文: [https://www.geeksforgeeks.org/python-pandas-index-duplicated/](https://www.geeksforgeeks.org/python-pandas-index-duplicated/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Index.duplicated()` 函数表示重复的索引值。重复的值在结果数组中表示为 `True` 值。可以指示所有重复项，除第一个重复项外的所有重复项，或者除最后一个重复项外的所有重复项。
...
一组重复项中的一个或多个值标记为缺失。
- `'first'`: 除了第一次出现外，将重复项标记为真。
- `'last'`: 除最后一次出现外，将重复项标记为“真”。
- `False`: 将所有重复标记为真。
...
## 示例 1
使用 `Index.duplicated()` 函数指示索引中除第一个值之外的所有重复值。
...
## 示例 2
使用 `Index.duplicated()` 功能识别所有重复值。这里所有的重复值将被标记为 `True`。
...
该函数已将所有重复值标记为 `True`。它还将 `NaN` 值的单次出现视为唯一，并将其标记为 `False`。