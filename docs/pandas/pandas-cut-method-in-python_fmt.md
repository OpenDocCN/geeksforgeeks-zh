...
**语法:** `cut(x, bins, right=True, labels=None, retbins=False, precision=3, include_lowest=False, duplicates='raise')`

**参数:**

- `x`: 要入库的输入数组。必须是一维的。
- `bins`: 定义分割的面元边缘。
- `right`: (布尔值，默认为`True`) 指示箱是否包括最右边。如果`right == True`(默认值)，则箱`[1, 2, 3, 4]`指示`(1, 2]`, `(2, 3]`, `(3, 4]`。
- `labels`: (数组或布尔值，可选) 指定返回箱的标签。必须与结果箱的长度相同。如果为`False`，则仅返回容器的整数指示符。
- `retbins`: (bool，默认`False`) 是否返回箱。当以标量形式提供容器时很有用。
