...
> 原文: [https://www.geeksforgeeks.org/how-to-install-python-pandas-on-windows-and-linux/](https://www.geeksforgeeks.org/how-to-install-python-pandas-on-windows-and-linux/)
...
熊猫程序可以写在任何纯文本编辑器上，如`记事本`、`记事本++`或任何类似的东西上，并用`.py`分机。首先，编写熊猫代码并执行各种有趣而有用的操作，必须在他们的系统上安装 Python。这可以通过遵循下面提供的逐步说明来完成:

## 如果 Python 已经存在了呢？让我们检查一下

要检查您的设备是否预装了 Python，只需进入`命令行`(在运行对话框中搜索`cmd`(`Win`+`R`)。
现在运行以下命令:
...
`Anaconda` 是开源软件，包含 `Jupyter`、`spyder` 等，用于大型数据处理、数据分析、重型科学计算。如果你的系统没有预装 `Anaconda Navigator`，可以学习如何在 [Windows](https://www.geeksforgeeks.org/how-to-install-anaconda-on-windows/) 或者 [Linux](https://www.geeksforgeeks.org/how-to-install-anaconda-on-linux/) 上安装 `Anaconda Navigator`？
...
**第三步:** 给你的环境起一个名字，比如`pandas`，然后选择一个 `python` 版本在环境中运行。现在点击`创建`按钮创建熊猫环境。
![Anaconda-Installation-Pandas-13](img/651f5eedb7458c3f887ead9eab3dd9f6.png)

**第四步:** 现在点击`panda`环境创建激活它。
...
**第十步:** 现在打开 `Pandas` 环境，点击包名右侧的`绿色箭头`，选择你想要开始 `Pandas` 编程的控制台。
