...
> 原文：[https://www.geeksforgeeks.org/getting-unique-values-from-a-column-in-pandas-dataframe/](https://www.geeksforgeeks.org/getting-unique-values-from-a-column-in-pandas-dataframe/)
...
## 方法 1
从`record`中选择`continent`列，应用`unique()`函数得到我们想要的值。
...
## 方法 3
在这个方法中，您可以看到我们使用`pd.unique()`函数，将数据框`record['continent']`作为参数传入，尽管我们选择了与上面相同的列，因此我们得到了相同的输出。
