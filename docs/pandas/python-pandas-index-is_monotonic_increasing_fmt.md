...
> 原文: [https://www.geeksforgeeks.org/python-pandas-index-is_monotonic_increasing/](https://www.geeksforgeeks.org/python-pandas-index-is_monotonic_increasing/)

Pandas 索引是一个实现有序的、可切片的集合的不可变数组。它是存储所有 Pandas 对象的轴标签的基本对象。

Pandas `Index.is_monotonic_increasing` 属性返回 `True` 如果给定索引对象中的基础数据单调递增，则返回 `False`。
...
使用 `Index.is_monotonic_increasing` 属性找出给定索引对象中的底层数据是否单调增加。
