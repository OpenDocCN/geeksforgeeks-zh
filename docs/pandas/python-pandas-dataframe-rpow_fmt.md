...
`DataFrame.rpow()` 函数用于求数据帧和其他元素的指数幂(二进制运算符 rfloordiv)。该功能本质上与执行 `other ** dataframe` 相同，但支持替换其中一个输入中缺失的数据。
...
- `level` : 跨级别广播，匹配传递的 MultiIndex leve 上的索引值
...
使用 `rpow()` 函数将序列中的每个元素提升到列轴上数据框中的相应值。
...
使用 `rpow()` 函数将数据帧中的每个元素提升到其他数据帧中相应元素的幂
