...
Pandas 的 `Series.ne()` 方法用来比较调用者系列（Caller Series）和传递进来的系列的每个元素。对于每个不等于传递序列中对应元素的元素，它返回 `True`。

**注意:** 结果是在对比调用者**系列**与**其他系列**的基础上返回的！

> **语法:** `Series.ne(other, level=None, fill_value=None)`
>
> **参数:**
> - `other`: 要与之比较的其他系列。
> - `level`: int 或多级别情况下的级别名称。
...
**输出:**
如输出所示，如果调用者序列中的值不等于传递序列中的值，则返回 `True`。还可以看到，空值被 `5` 代替，使用该值进行比较。
