...
> 原文: [https://www.geeksforgeeks.org/pandas-scatter-plot-dataframe-plot-scatter/](https://www.geeksforgeeks.org/pandas-scatter-plot-dataframe-plot-scatter/)

散点图是一种显示两个数值变量之间关系的数据可视化技术。对于使用熊猫绘制散点图，有一个 `DataFrame` 类，该类有一个名为 `plot` 的成员。在 `plot` 成员上调用 `scatter()` 方法，在两个变量或两列熊猫数据框之间绘制一个绘图。
...
在本例中，我们将使用数据框绘制散点图，这里我们将创建数据框并使用不同的列绘制散点图。
