...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timestamp-dayofweek/](https://www.geeksforgeeks.org/python-pandas-timestamp-dayofweek/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Pandas` 的 `Timestamp.dayofweek` 属性返回给定时间戳对象中一周中某一天的值。日期从 0 到 6，其中 0 是星期一，6 是星期日。
...
使用 `Timestamp.dayofweek` 属性在给定的 `Timestamp` 对象中查找星期几。
...
使用 `Timestamp.dayofweek` 属性在给定的时间戳对象中查找一周中的某一天。
