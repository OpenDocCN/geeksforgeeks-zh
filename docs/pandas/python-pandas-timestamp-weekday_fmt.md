...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timestamp-weekday/](https://www.geeksforgeeks.org/python-pandas-timestamp-weekday/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Timestamp.weekday()` 函数返回给定 `Timestamp` 对象中的日期所代表的星期几。周一== 0 …周日== 6。
