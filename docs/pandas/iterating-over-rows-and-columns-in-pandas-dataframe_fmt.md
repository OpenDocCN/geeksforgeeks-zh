...
> 原文: [https://www.geeksforgeeks.org/iterating-over-rows-and-columns-in-pandas-dataframe/](https://www.geeksforgeeks.org/iterating-over-rows-and-columns-in-pandas-dataframe/)
...
为了遍历行，我们可以使用三个函数 `iteritems()`，`iterrows()`，`itertuples()`。这三个函数将有助于对行进行迭代。
...
现在，我们应用 `itertuples()` 来获取每一行的一个实例
