# Pandas Series.set_value() 方法详解

> 原文：[https://www.geeksforgeeks.org/python-pandas-series-set_value/](https://www.geeksforgeeks.org/python-pandas-series-set_value/)

Pandas Series 是带有轴标签的一维数组。标签不必是唯一的，但必须是可散列的类型。该对象支持基于整数和基于标签的索引，并提供了一系列方法来执行涉及索引的操作。

Pandas `Series.set_value()` 功能用于使用索引标签设置给定系列对象的值。

**语法：** `Series.set_value(标签, 值, takeable=False)`

**参数：**
- `标签`：不允许使用多索引进行部分索引
- `值`：标量值
- `takeable`：将索引解释为索引器，默认为 `False`

**返回：** Series

## 示例 1

使用 `Series.set_value()` 功能使用索引标签设置给定序列对象中的值。
