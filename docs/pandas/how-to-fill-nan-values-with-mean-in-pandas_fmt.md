...
> 原文：[https://www.geeksforgeeks.org/how-to-fill-nan-values-with-mean-in-pandas/](https://www.geeksforgeeks.org/how-to-fill-nan-values-with-mean-in-pandas/)
...
1.  使用熊猫库的 `DataFrame.fillna()`。
2.  使用 `sklearn.impute` 中的 `SimpleImputer()`（这仅在数据以 csv 文件的形式存在时有用）。

### 使用熊猫库的 `DataFrame.fillna()`

借助熊猫库中的 [`DataFrame.fillna()`](https://www.geeksforgeeks.org/python-pandas-series-fillna/)，我们可以轻松替换数据帧中的‘NaN’。

**程序：**

1.  为了计算 `mean()`，我们使用特定列的平均值函数。
...
> *   `strategy` 字符串：默认值=“均值”
> *   `fill_value` 字符串或数值：默认值=无
> *   `verbose`：整数，默认值=0
> *   `copy`：布尔值，默认值=真
> *   `add_indicator`：布尔值，默认值=False

**注：** 以下示例中使用的数据为[此处](https://github.com/dataoptimal/posts/blob/master/data%20cleaning%20with%20python%20and%20pandas/property%20data.csv)。
