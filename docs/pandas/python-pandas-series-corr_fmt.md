...
`Series.corr()` 函数计算与其他 Series 的相关性，排除缺失值。
...
- `other`: Series
- `method`: {‘pearson’, ‘kendall’, ‘spearman’} 或可调用的
...
使用 `Series.corr()` 函数找到给定 Series 对象与其他对象的相关性。
...
使用 `Series.corr()` 函数找出给定序列对象与其他序列对象的相关性。Series 对象包含一些缺失的值。
