...
> 原文：[https://www.geeksforgeeks.org/generating-random-integers-in-pandas-dataframe/](https://www.geeksforgeeks.org/generating-random-integers-in-pandas-dataframe/)

Pandas 是最流行的用于数据分析的 Python 库。它通过纯 C 或 Python 编写的后端源代码提供了高度优化的性能。

这里我们将看到如何在熊猫数据报中生成随机整数。我们将使用 `numpy.random.randint()` 方法生成随机整数。
