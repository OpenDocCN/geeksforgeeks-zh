...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-set_axis/](https://www.geeksforgeeks.org/python-pandas-series-set_axis/)

Pandas Series 是带有轴标签的一维数组。标签不必是唯一的，但必须是可散列的类型。该对象支持基于整数和基于标签的索引，并提供了一系列方法来执行涉及索引的操作。

Pandas `Series.set_axis()` 函数用于为给定的轴分配所需的索引。列或行标签的索引可以通过分配类似列表的或索引来更改。

> **语法:** `Series.set_axis(labels, axis=0, inplace=None)`
> **参数:**
> `labels`: 新索引的值。
> `axis`: 轴更新。值 `0` 标识行，`1` 标识列。
> `inplace`: 是否返回新的 Series 实例。
> **返回:** 重命名后的 Series
...
现在我们将使用 `Series.set_axis()` 函数来重置给定 Series 对象的索引。
