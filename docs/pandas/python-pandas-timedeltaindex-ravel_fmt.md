...
> 哎哎哎: https://www.geeksforgeeks.org/python-pandas-timedeltaindex-ravel/

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`TimedeltaIndex.ravel()` 函数返回时间增量索引对象的底层数据的展平值的数组。

> **语法:** `TimedeltaIndex.ravel()`
>
> **参数:** 无
>
> **返回:** `ndarray`
...
使用 `TimedeltaIndex.ravel()` 函数返回给定时间增量索引对象的数组。
