...
**语法:** `Series.drop(labels=None, axis=0, index=None, columns=None, level=None, inplace=False, errors='raise')`

### 参数:
`labels`: 指标标签要掉。
`axis`: 适用于系列产品。
`index`, `columns`: 对于在系列上的应用是多余的，但是可以使用索引来代替标签。
`level`: 对于多索引，将移除标签的级别。
`inplace`: 如果为真，在位进行操作，返回无。
`errors`: 如果‘忽略’，抑制错误，只丢弃现有标签。

### 返回:
下降:熊猫。`Series`

## 示例#1:
使用`Series.drop()`函数删除给定序列对象中与传递的索引标签相对应的值。
