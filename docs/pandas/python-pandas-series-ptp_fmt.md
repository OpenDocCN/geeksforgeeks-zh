...
熊猫 `Series.ptp()` 函数返回对象中最大值和最小值之间的差值。这相当于 `numpy.ndarray` 法 `ptp`。
...
使用 `Series.ptp()` 函数返回给定 Series 对象中基础数据的最大值和最小值之间的差值。
...
使用 `Series.ptp()` 函数返回给定 Series 对象中基础数据的最大值和最小值之间的差值。给定的序列对象中包含一些缺失的值。
