...
> 原文：[https://www.geeksforgeeks.org/calculate-the-number-of-characters-in-each-word-in-a-pandas-series/](https://www.geeksforgeeks.org/calculate-the-number-of-characters-in-each-word-in-a-pandas-series/)
...
另一种方法是通过使用 `len()` 函数（包裹在地图函数中）来查找字符数，这样给定的序列通过使用 `pandas.series()` 将一系列数据作为输入来替换长度值。
...
我们在 `Series.map()` 的帮助下输入单词并统计单词的每个字符，该系列使用名为 calc 的函数替换值并给出输出。
...
我们使用 `Series.map()` 输入一系列单词并替换值，使用 `Series.apply()` 获取单个值。
