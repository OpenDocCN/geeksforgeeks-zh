...
`熊猫`的`Series.diff()`用来寻找同一系列元素之间的差异。差异是连续的，取决于传递给`diff()`方法的周期参数。
...
在周期为 2 的第一次输出中，从第(i+2)个位置减去第 I 个位置的值，并存储在第(i+2)个位置。在第二次输出中，从第(i-1)个位置的值中减去第 I 个位置的值，并存储在第(i-1)个位置。

## 注意

输出序列中的第一个/最后一个 n 值是 NaN，取决于周期的符号。(如果周期为正，则为第一个；如果周期为负，则为最后一个，其中 n 为周期)。