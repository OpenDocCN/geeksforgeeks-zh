...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-rename_axis/](https://www.geeksforgeeks.org/python-pandas-series-rename_axis/)
...
**参数:**
- `mapper`: 值设置轴名属性。
- `index`, `columns`: 应用于该轴值的标量、列表、字典或函数转换。
- `axis`: 要重命名的轴。
- `copy`: 同样复制底层数据。
- `inplace`: 直接修改对象，而不是创建新的系列或数据帧。

**返回:** 序列、数据帧或无

## 示例 1

使用`Series.rename_axis()`功能重命名给定系列对象的轴。
...
使用`Series.rename_axis()`功能重命名给定序列对象的多索引轴。
