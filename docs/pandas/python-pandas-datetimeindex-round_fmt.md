...
> 原文: [https://www.geeksforgeeks.org/python-pandas-datetimeindex-round/](https://www.geeksforgeeks.org/python-pandas-datetimeindex-round/)
...
**参数:**
*   `freq`: 频率等级要四舍五入到指数。必须是固定频率，如`"S"`(秒)而不是`"ME"`(月末)。

**返回:**
日期时间索引或时间增量索引的相同类型的索引，或序列具有相同索引的序列。

## 示例 1

使用`DatetimeIndex.round()`函数将 DatetimeIndex 对象的数据舍入到指定的频率。
...
现在，我们要将 DatetimeIndex 对象的第二基于频率的频率转换为基于分钟的频率。
