...
> **语法:** `Series.where(cond, other=nan, inplace=False, axis=None, level=None, errors='raise', try_cast=False, raise_on_error=None)`
>
> **参数:**
> `cond`: 布尔 ndfram、类数组或可调用
> `other`: 标量、ndfram 或可调用
> `inplace`: 布尔，默认 False
> `axis`: int，默认 None
> `level`: int，默认 None
...
当传递的条件不满足时，使用`Series.where()`函数将给定序列对象中的值替换为其他值。
...
当传递的条件不满足时，使用`Series.where()`函数将给定 Series 对象中的值替换为其他值。
