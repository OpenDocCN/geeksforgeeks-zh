...
> 原文：[https://www.geeksforgeeks.org/ways-to-apply-an-if-condition-in-pandas-dataframe/](https://www.geeksforgeeks.org/ways-to-apply-an-if-condition-in-pandas-dataframe/)
...
> `df['新列名'] = df['列名'].apply(lambda x: "满足条件时的值" if 条件 else "不满足条件时的值")`
...
我们将处理只包含 5 个名称的字符串的数据帧：Hanah、Ria、Jay、Bholu、Sachin。条件是：如果名称等于“Ria”，则赋值为“Found”。否则，如果名称不是“Ria”，则赋值为“未找到”。但是这次我们将使用 lambdas 来处理它。
