# Python Pandas MultiIndex.nlevels 属性

> 哎哎哎:1230〔https://www.geeksforgeeks.org/python-pandas-multiindex-nlevels-investigation-/t1〕

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`MultiIndex.nlevels` 属性返回多索引中的整数级别。
