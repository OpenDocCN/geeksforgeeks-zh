...
> 原文: [https://www.geeksforgeeks.org/how-to-filter-dataframe-rows-based-on-the-date-in-pandas-2/](https://www.geeksforgeeks.org/how-to-filter-dataframe-rows-based-on-the-date-in-pandas-2/)
...
使用带有索引语法`[条件]`的`DataFrame.loc()`仅从`DataFrame`中选择满足条件的行。定义条件以检查数据框中的日期列是否在约束范围内。它将为所有行创建一个布尔数组。将只打印具有真实值的行。
