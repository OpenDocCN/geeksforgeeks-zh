...
> 原文：[https://www.geeksforgeeks.org/how-to-count-the-number-of-nan-values-in-pandas/](https://www.geeksforgeeks.org/how-to-count-the-number-of-nan-values-in-pandas/)

我们可能需要计算数据集中每个要素的 NaN 值的数量，以便决定如何处理它。例如，如果缺失值的数量很少，那么我们可以选择放弃那些观察值；或者可能有一列缺少大量条目，因此我们可以决定是否包含该变量。

## 方法 1: 使用 `describe()`

我们可以使用 `describe()` 方法返回一个包含数据集详细信息的表。`count` 属性直接给出每列中非 NaN 值的计数。所以，如果我们知道观察的总数，我们可以得到 NaN 值的计数。
...
`isnull()` 函数返回包含真值和假值的数据集。因为，真被视为 1，假被视为 0，所以调用 `isnull()` 系列上的 `sum()` 方法会返回真值的计数，该计数实际上对应于 NaN 值的数量。
...
可以使用 `loc` 或 `iloc` 选择行。然后我们像以前一样求出总和。
