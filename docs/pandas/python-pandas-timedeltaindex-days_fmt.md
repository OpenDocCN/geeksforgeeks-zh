...
> 哎哎哎: https://www.geeksforgeeks.org/python-pandas-timedeltaindex-days/

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`TimedeltaIndex.days` 属性返回时间增量索引对象中每个元素的天数。它返回一个索引对象。
...
使用 `TimedeltaIndex.days` 属性返回时间增量索引对象的每个条目中的天数。
...
使用 `TimedeltaIndex.days` 属性返回 `TimedeltaIndex` 对象每个条目中的天数。
