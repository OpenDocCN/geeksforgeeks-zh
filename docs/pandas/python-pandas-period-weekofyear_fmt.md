...
> 原文: [https://www.geeksforgeeks.org/python-pandas-period-weekofyear/](https://www.geeksforgeeks.org/python-pandas-period-weekofyear/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。
`pandas` 的 `Period.weekofyear` 属性返回一个整数值，该整数值是给定 `Period` 对象的一年中的周数。
...
## 示例#1
使用 `Period.weekofyear` 属性查找给定 `Period` 对象的周数。
