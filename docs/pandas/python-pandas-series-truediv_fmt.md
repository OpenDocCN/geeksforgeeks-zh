...
Pandas `Series.truediv()` 函数执行 Series 和其他元素的浮点除法（二进制运算符 truediv）。它相当于 `series / other`，但支持用 `fill_value` 替换其中一个输入中缺失的数据。

> **语法:** `Series.truediv(other, level=None, fill_value=None, axis=0)`
>
> **参数:**
> - **other:** Series 或标量值
> - **fill_value:** 填充现有缺失（NaN）值
> - **level:** 跨级别广播，在传递的多索引级别上匹配索引值
>
> **返回:** 结果 Series

## 示例 1

使用 `Series.truediv()` 函数执行给定 Series 对象与其他标量的浮点除法。
...
现在我们将使用 `Series.truediv()` 函数来执行给定 Series 对象与其他标量的浮点除法。
...
正如我们在输出中看到的那样，`Series.truediv()` 函数已经成功地对给定的 Series 对象执行了元素除法。

## 示例 2

使用 `Series.truediv()` 函数执行给定 Series 对象与其他标量的浮点除法。给定的 Series 对象包含一些缺失的值。处理缺失的值。
...
现在我们将使用 `Series.truediv()` 函数来执行给定 Series 对象与其他标量的浮点除法。将缺失的值填充 100。
...
正如我们在输出中看到的那样，`Series.truediv()` 函数已经成功地对给定的 Series 对象执行了元素除法。在执行除法之前，它还在缺失值的地方替换了 100。