# Python Pandas Series.to_frame() 方法详解

> 原文: [https://www.geeksforgeeks.org/python-pandas-series-to_frame/](https://www.geeksforgeeks.org/python-pandas-series-to_frame/)

Pandas Series 是带有轴标签的一维数组。标签不必是唯一的，但必须是可散列的类型。该对象支持基于整数和基于标签的索引，并提供了一系列方法来执行涉及索引的操作。

Pandas `Series.to_frame()` 功能用于将给定的序列对象转换为数据帧。

> **语法:** `Series.to_frame(name=None)`
>
> **参数:**
> `name`: 传递的名称应该替换系列名称(如果有)。
>
> **返回:** 数据帧: 数据帧

## 示例 #1

使用 `Series.to_frame()` 函数将给定的序列对象转换为数据帧。
