...
> 
> **参数:**
> `threshold`: 类似数值或列表，设置最大阈值，如果是列表，为调用者系列中的每个值设置单独的阈值(给定列表大小相同)
> `axis`: 0 或“索引”按行应用方法，1 或“列”按列应用方法。
...
## 示例 1: 应用于单值序列
在此示例中，最大阈值 26 作为参数传递给 `clip_upper()` 方法。此方法在数据框的“年龄”列上调用，新值存储在“年龄_新”列中。在执行任何操作之前，使用 `dropna()`。
...
在本例中，年龄列的前 10 行使用 `.head()` 方法提取并存储。之后，创建一个相同长度的列表，并将其传递给 `.clip_upper()` 方法的 `threshold` 参数，为串联的每个值设置单独的阈值。返回值存储在新列“clipped_values”中。
