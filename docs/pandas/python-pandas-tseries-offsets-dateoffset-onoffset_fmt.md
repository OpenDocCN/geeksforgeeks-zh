...
> 哎哎哎:# t0]https://www.geeksforgeeks.org/python-pandas-tseries-offsets-dateoffset-onoffset/
...
pandas `DateOffset.onOffset()` 函数返回一个布尔值。如果给定的日期偏移量从过去的日期开始设置，则返回 `True`，否则返回 `False`。
...
使用 `pandas.tseries.offsets.DateOffset.onOffset()` 功能检查对于给定的日期偏移量，过去的日期是否为“开”。
