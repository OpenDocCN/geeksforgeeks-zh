...
> 原文：[https://www.geeksforgeeks.org/difference-between-shallow-copy-vs-deep-copy-in-pandas-dataframes/](https://www.geeksforgeeks.org/difference-between-shallow-copy-vs-deep-copy-in-pandas-dataframes/)
...
这些操作是在库函数`pandas`的帮助下完成的。浅拷贝和深拷贝通过`DataFrame.copy(deep=False)`和`DataFrame.copy(deep=True)`实现，分别用于数据框和系列。
...
| 序号 | 浅拷贝 | 深度复制 |
| :--- | :--- | :--- |
| 1 | 是集合结构的副本，不是元素。 | 是集合的副本，原始集合中的所有元素都被复制。 |
| 2 | 影响初始数据帧。 | 不影响初始数据帧。 |
| 3 | 浅层复制不会复制子对象。 | 深度复制递归复制子对象。 |
| 4 | 与深度拷贝相比，创建浅拷贝速度更快。 | 与浅拷贝相比，创建深拷贝较慢。 |
| 5 | 副本依赖于原件。 | 副本并不完全依赖于原件。 |