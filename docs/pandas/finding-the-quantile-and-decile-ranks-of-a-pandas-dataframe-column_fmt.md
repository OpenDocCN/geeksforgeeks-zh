...
> 原文: [https://www.geeksforgeeks.org/finding-the-quantile-and-decile-ranks-of-a-pandas-dataframe-column/](https://www.geeksforgeeks.org/finding-the-quantile-and-decile-ranks-of-a-pandas-dataframe-column/)
...
其中 `q` 是分位数，低于你要找的 `I<sup>th</sup>` 值的比例。
...
第三步: 将任何百分比转换为“q”的小数。我们正在寻找一个数值，其中 25%的数值低于它，所以把它转换成 0.25。
...
现在，我们可以使用 pandas 函数 `qcut()` 找到分位数等级，方法是传递等级要考虑的列名，即表示分位数的参数 `q` 的值。十分位数为 10，四分位数为 4，等等。并且 `labels=False` 以整数形式返回容器。
...
现在，如果我们打印数据框架，我们可以看到新的列 `QuantileRank` 根据英语分数列对我们的数据进行排名。
