...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-slice_shift/](https://www.geeksforgeeks.org/python-pandas-series-slice_shift/)

Pandas Series 是带有轴标签的一维数组。标签不必是唯一的，但必须是可散列的类型。该对象支持基于整数和基于标签的索引，并提供了一系列方法来执行涉及索引的操作。

`Series.slice_shift()` 功能相当于不复制数据的移位。移位的数据将不包括丢失的周期，并且移位的轴将小于原始轴。
...
## 示例 1
使用 `Series.slice_shift()` 功能将给定序列对象的数据移动 2 个周期。
