...
`Pandas`的`DatetimeIndex.strftime()`函数使用指定的日期格式转换为索引。该函数返回由`date_format`指定的格式化字符串的索引，它支持与 python 标准库相同的字符串格式。
...
使用`DatetimeIndex.strftime()`函数将给定的日期时间索引对象转换为指定的格式。
...
使用`DatetimeIndex.strftime()`函数将给定的`DatetimeIndex`对象转换为指定的格式。
