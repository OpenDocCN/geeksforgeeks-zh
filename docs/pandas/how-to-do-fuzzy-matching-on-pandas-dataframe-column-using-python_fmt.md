...
> 原文：[https://www.geeksforgeeks.org/how-to-do-fuzzy-matching-on-pandas-dataframe-column-using-python/](https://www.geeksforgeeks.org/how-to-do-fuzzy-matching-on-pandas-dataframe-column-using-python/)

**先决条件：** [`fuzzywuzzy`](https://www.geeksforgeeks.org/fuzzywuzzy-python-library/)
...
*   `pd.DataFrame`：将 Python 字典转换为熊猫数据帧。
*   `dataframe['column_name'].tolist()`：用 Python 将熊猫数据框的特定列转换为项目列表。
*   `append()`：向列表追加项目。
*   `process.extract(query, choices, limit)`：模糊库的处理模块附带的一个函数，用于从 `choices` 列表中提取与给定的 `query` 匹配的项目。提取的最接近选项的数量由我们设置的 `limit` 决定。
*   `process.extractOne(query, choices, scorer)`：从 `choices` 列表中提取与给定的 `query` 和 `scorer` 匹配的唯一最接近的匹配。`scorer` 是可选参数，以使其使用特定的计分器，如 `fuzz.token_sort_ratio`，`fuzz.token_set_ratio`。
