...
> 原文链接：https://www.geeksforgeeks.org/python-pandas-dataframe-floordiv/

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`DataFrame.floordiv()` 函数用于对数据帧进行整数分割，可以是常量、序列或任何其他 `DataFrame`。如果其他是系列，则系列的尺寸必须与数据框的分割轴相匹配。如果另一个是数据帧，那么两个数据帧应该具有相同的维度。相当于 `dataframe/other`，但支持用 `fill_value` 替换其中一个输入中缺失的数据。

> **语法：** `DataFrame.floordiv(other, axis='columns', level=None, fill_value=None)`
>
> **参数：**
> - **other：** `Series`，`DataFrame`，或常量
> - **轴：** 对于 `Series` 输入，轴要与 `Series` 索引匹配
> - **fill_value：** 用该值填充缺失（`NaN`）值。如果两个数据帧位置都丢失，结果将丢失
