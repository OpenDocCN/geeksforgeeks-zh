...
`pandas.Series.idxmin()`功能返回最小值的行标签。如果多个值等于最小值，则返回具有该值的第一行标签。
...
- `axis`：用于与 `DataFrame.idxmin` 兼容。冗余用于系列上的应用。
...
使用 `Series.idxmin()` 函数在给定的序列对象中找到与最小值对应的索引标签。
...
使用 `Series.idxmin()` 函数，在给定的序列对象中找到与最小值对应的索引标签。
