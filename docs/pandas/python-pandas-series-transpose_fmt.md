...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-transpose/](https://www.geeksforgeeks.org/python-pandas-series-transpose/)

Pandas Series 是带有轴标签的一维数组。标签不必是唯一的，但必须是可散列的类型。该对象支持基于整数和基于标签的索引，并提供了一系列方法来执行涉及索引的操作。

Pandas `Series.transpose()` 函数返回转置，顾名思义就是自我。
...
## 示例 1
使用 `Series.transpose()` 函数找到给定 Series 对象的转置。
