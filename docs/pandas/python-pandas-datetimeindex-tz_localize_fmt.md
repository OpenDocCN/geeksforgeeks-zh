...
> 原文: [https://www.geeksforgeeks.org/python-pandas-datetimeindex-tz_localize/](https://www.geeksforgeeks.org/python-pandas-datetimeindex-tz_localize/)
...
>
> **参数:**
> **`tz` :** 时区将时间戳转换为 tz-aware DatetimeIndex。传递 `None` 将删除保留本地时间的时区信息。
> **`ambiguous` :** str {'infer', 'NaT', 'raise'} 或 bool 数组，默认为 'raise'
...
使用 `DatetimeIndex.tz_localize()` 函数将幼稚的 DatetimeIndex 对象转换为具有时区感知的对象。
...
使用 `DatetimeIndex.tz_localize()` 函数将幼稚的 DatetimeIndex 对象转换为具有时区意识的对象。
