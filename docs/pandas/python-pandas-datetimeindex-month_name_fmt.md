# Pandas DatetimeIndex.month_name() 函数详解

> 原文: [https://www.geeksforgeeks.org/python-pandas-datetimeindex-month_name/](https://www.geeksforgeeks.org/python-pandas-datetimeindex-month_name/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。Pandas 就是其中一个包，让导入和分析数据变得容易多了。

Pandas 的 `DatetimeIndex.month_name()` 函数返回指定地区的日期时间索引的月份名称。默认区域设置为 `None`，在这种情况下，将以英语返回名称。

> **语法:** `DatetimeIndex.month_name(locale=None)`
>
> **参数:**
> `locale`: 地区确定返回月份名称的语言
>
> **返回:** 月份名称索引

## 示例 1

使用 `DatetimeIndex.month_name()` 函数返回 `DatetimeIndex` 对象中每个条目的月份名称。返回法语地区的月份名称。
...
使用 `DatetimeIndex.month_name()` 函数返回 `DatetimeIndex` 对象中每个条目的月份名称。返回德语地区的月份名称。
