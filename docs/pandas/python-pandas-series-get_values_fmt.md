...
> 原文：[https://www.geeksforgeeks.org/python-pandas-series-get_values/](https://www.geeksforgeeks.org/python-pandas-series-get_values/)

Pandas Series 是带有轴标签的一维数组。标签不必是唯一的，但必须是可散列的类型。该对象支持基于整数和基于标签的索引，并提供了一系列方法来执行涉及索引的操作。

Pandas `Series.get_values()` 函数返回一个包含给定序列对象的底层数据的数组。
