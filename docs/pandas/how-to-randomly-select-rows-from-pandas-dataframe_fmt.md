...
> 原文：[https://www.geeksforgeeks.org/how-to-randomly-select-rows-from-pandas-dataframe/](https://www.geeksforgeeks.org/how-to-randomly-select-rows-from-pandas-dataframe/)

我们来讨论一下如何从熊猫`数据框`中随机选择行。从数据框中随机选择行可以通过不同的方式实现。
用列表字典创建一个简单的数据框架。
...
使用`sample(n)`或`sample(n=n)`随机选择 `n` 个行数。每次运行这个，你会得到 n 个不同的行。
