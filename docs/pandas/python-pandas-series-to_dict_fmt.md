...
`Series.to_dict()`函数用于将给定的 Series 对象转换为{label -> value} dict 或 dict 样对象。
...
`value_dict` : collections.绘图

### 示例#1

使用`Series.to_dict()`函数将给定的序列对象转换为字典。
...
使用`Series.to_dict()`功能将给定的序列对象转换为字典。
