...
`Series.dt`可用于访问系列的值，如 datetimelike，并返回几个属性。Pandas `Series.dt.normalize()`功能将时间转换为午夜。日期时间的时间部分转换为午夜，即 00:00:00。这在时间不重要的情况下很有用。长度不变。时区不受影响。
...
使用`Series.dt.normalize()`功能将给定系列对象中的时间转换为午夜。
