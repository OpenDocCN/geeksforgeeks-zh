...
- `method`: `{None, 'pad'/'ffill', 'backfill'/'bfill', 'nearest'}`，可选。
    - **默认:** 仅精确匹配。
    - `pad/ffill`: 如果没有精确匹配，则查找**前一个**索引值。
    - `backfill/bfill`: 如果没有精确匹配，则使用**下一个**索引值。
    - `nearest`: 如果没有精确匹配，则使用**最近的**索引值。通过选择较大的索引值来打破并列距离。

### 返回值

`loc`: 如果是唯一索引，则为 `int`；如果是单调索引，则为 `slice`；否则为 `mask`。

## 示例

### 示例 1

使用 `Index.get_loc()` 函数查找传递值的位置。
...
使用 `Index.get_loc()` 函数查找传递值的位置。如果传递的值不在索引中，则返回刚好小于传递值的前一个值的位置。
...
如我们所见，该函数返回了输出 `4`。由于索引中不存在 `33`，但在排序顺序中，略小于 `33` 的值为 `25`，其位置为 `4`。所以，输出是 `4`。如果我们不设置 `method` 参数，那么如果值不存在，就会导致错误。