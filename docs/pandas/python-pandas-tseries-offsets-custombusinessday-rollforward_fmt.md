...
> 原文: [https://www.geeksforgeeks.org/python-pandas-tseries-offsets-custombusinessday-rollforward/](https://www.geeksforgeeks.org/python-pandas-tseries-offsets-custombusinessday-rollforward/)
...
Pandas `tseries.offsets.CustomBusinessDay.rollforward()` 功能仅在不偏移时提供向前到下一个偏移的日期滚动。

**语法:**
`pandas.tseries.offsets.CustomBusinessDay.rollforward(dt)`
...
## 示例 #1
如果给定的时间戳不在偏移量上，使用 `pandas.tseries.offsets.CustomBusinessDay.rollforward()` 函数向前滚动。
