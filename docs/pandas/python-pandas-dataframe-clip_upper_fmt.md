...
> 原文: [https://www.geeksforgeeks.org/python-pandas-dataframe-clip_upper/](https://www.geeksforgeeks.org/python-pandas-dataframe-clip_upper/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`DataFrame.clip_upper()` 用于在指定的输入阈值下修剪值。我们使用此函数将输入值阈值以上的所有值修剪为指定的输入值。

## 语法
`DataFrame.clip_upper(threshold, axis=None, inplace=False)`

## 参数
- `threshold`: float 或 array_like
    - `float`: 将每个值与阈值进行比较。
...
## 示例 1
使用 `clip_upper()` 功能将数据帧的值修剪到给定阈值以上。
...
## 示例 2
使用 `clip_upper()` 函数将数据帧中的值与数据帧中每个单元格的特定值进行剪辑。
