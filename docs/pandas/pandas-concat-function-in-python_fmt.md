# pandas.concat()函数在Python中

> 原文: [https://www.geeksforgeeks.org/pandas-concat-function-in-python/](https://www.geeksforgeeks.org/pandas-concat-function-in-python/)
...
> - `objs`: 系列或数据框对象
> - `轴`: 轴连接；默认值= 0
> - `join`: 方式处理其他轴上的索引；默认值=“外部”
> - `ignore_index`: 如果为 True，则不要使用沿拼接轴的索引值；默认值=假
> - `key`: 向结果索引添加标识符的序列；默认值=无
> - `levels`: 用于构建多索引的特定级别(唯一值)；默认值=无
> - `names`: 结果分层索引中的级别名称；默认值=无
> - `verify_integrity`: 检查新级联轴是否包含重复项；默认值=假
> - `sort`: 当连接为“外部”时，如果非连接轴尚未对齐，则对其进行排序；默认值=假
> - `copy`: 如果为 False，不要进行不必要的数据复制；默认值=真
>
> 返回: 类型的对象(数据帧系列)
...
## 示例5：将2个数据帧连接起来，ignore_index=True，以便在连接的数据帧中显示新的索引值
