...
> 原文：`https://www.geeksforgeeks.org/python-pandas-multiindex-reorder_levels/`

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`MultiIndex.reorder_levels()` 功能用于使用输入顺序重新排列等级。它可能不会降低或复制级别。该函数将列表作为输入，该列表包含多索引级别的所需顺序。
...
使用 `MultiIndex.reorder_levels()` 函数对多索引的级别进行重新排序。
...
使用 `MultiIndex.reorder_levels()` 功能对多索引的级别进行重新排序。
