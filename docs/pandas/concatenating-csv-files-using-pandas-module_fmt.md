...
> 原文：[https://www.geeksforgeeks.org/concatenating-csv-files-using-pandas-module/](https://www.geeksforgeeks.org/concatenating-csv-files-using-pandas-module/)

使用`熊猫`，我们可以对CSV文件执行各种操作，如追加、更新、连接等。在本文中，我们将使用`熊猫`模块连接两个CSV文件。

假设我们有一个名为`Employee.csv`的csv文件，其中包含一些记录，如下所示：

![](img/cf13cc736c0d15f65bc5269f956f493e.png)

`Employee.csv`

还有一个名为`Updated.csv`的csv文件包含新记录以及来自`Employee.csv`文件的少量记录，但信息已更新。文件如下：

![](img/a9b00a6845549ef796326366777aeba9.png)

`Updated.csv`

我们可以看到`Updated.csv`中的前五条记录都是新的，其余都有更新的信息。比如`Louis`和`Diane`的工资有变化，`Joe`的`email_id`不一样等等。

本文旨在将`Updated.csv`文件中的新记录和现有记录的信息更新到`Employee.csv`中。

**注意：**没有两个员工可以拥有相同的`emp_id`。

**方法：**每当涉及到使用python操纵数据时，我们都会使用[数据帧](https://www.geeksforgeeks.org/python-pandas-dataframe/)。使用了以下方法。

*   阅读`Employee.csv`并创建一个数据框，比如说`employee_df`。
*   同样，从数据帧中读取`Updated.csv`，比如说，`updated_df`。
*   将`updated_df`连接到`employee_df`，并使用`emp_id`作为主键删除重复项。
*   创建名为`Updated_Employees.csv`的新csv文件，包含所有旧的、新的以及更新的记录。
...
`final_dataframe` - `Updated_Employees.csv`

下面是`Updated_Employee.csv`的图片，已经提供了。
