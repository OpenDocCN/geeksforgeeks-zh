...
> 原文：[https://www.geeksforgeeks.org/python-pandas-timedeltaindex-duplicated/](https://www.geeksforgeeks.org/python-pandas-timedeltaindex-duplicated/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Pandas` 的 `TimedeltaIndex.duplicated()` 函数检测给定时间增量索引对象中的重复值。它返回一个表示重复值的布尔值数组。除第一次出现外，所有重复出现的值都标记为 `True`。
...
## 示例 #1
使用 `TimedeltaIndex.duplicated()` 函数检查给定时间增量索引对象中所有重复出现的元素。
...
正如我们在输出中看到的，`TimedeltaIndex.duplicated()` 函数返回了一个包含 `tidx` 每个元素的布尔值的数组。如果元素没有被复制，则标记为 `False`，否则标记为 `True`。

## 示例 #2
使用 `TimedeltaIndex.duplicated()` 函数检查给定时间增量索引对象中元素的所有重复出现。
