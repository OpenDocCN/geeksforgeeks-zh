...
Pandas `Series.replace()`函数用于将`to_replace`中给出的值替换为`value`。序列的值会被其他值动态替换。
...
`value`: 值替换任何与`to_replace`匹配的值。
`inplace`: 如果为真，就位。
`limit`: 向前或向后填充的最大尺寸间隙。
`regex`: 是否将`to_replace`和/或`value`解释为正则表达式。
`method`: 当进行替换时使用的方法，当`to_replace`为标量、列表或元组且`value`为`None`时。

### 替换后返回:
对象。

## 示例#1
使用`Series.replace()`函数替换给定序列对象中的一些值。
