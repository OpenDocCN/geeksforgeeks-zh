...
> 原文: [https://www.geeksforgeeks.org/python-pandas-periodindex-asfreq/](https://www.geeksforgeeks.org/python-pandas-periodindex-asfreq/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`PeriodIndex.asfreq()` 函数将给定周期索引对象的每个元素转换为指定的频率。
...
## 示例 1
使用 `PeriodIndex.asfreq()` 功能将给定周期索引对象的频率转换为指定的时间序列频率。
...
## 示例 2
使用 `PeriodIndex.asfreq()` 函数将给定周期索引对象的频率转换为指定的时间序列频率。
