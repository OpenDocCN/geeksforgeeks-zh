...
> 哎哎哎:# t0]https://www . geeksforgeeks . orgpython-pandas-time-delta-index-append/

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`熊猫`就是其中一个包，让导入和分析数据变得容易多了。

`TimedeltaIndex.append()`函数将一组索引选项附加在一起。通过将索引作为 python 列表或元组传递，一次可以追加多个索引对象。
...
使用`TimedeltaIndex.append()`函数在给定对象的末尾追加一个时间增量索引对象。
...
使用`TimedeltaIndex.append()`函数在给定对象的末尾追加一个时间增量索引对象列表。
