...
> 原文：[https://www.geeksforgeeks.org/pandas-cut-continuous-to-categorical/](https://www.geeksforgeeks.org/pandas-cut-continuous-to-categorical/)
...
> `pandas.cut(x, bin, right=True, labels=None, retbins=False, precision=3, include_low=False, duplicates='raise', ordered=True)`

### 参数

*   `x`：输入数组。需要是一维的。
*   `bins`：表示分割的面元边界
*   `right`：表示是否应包含箱的最右边。布尔类型的值。默认值为真。
*   `labels`：定义返回的分段箱的标签。数组或布尔值
...
假设我们有一个由 15 个从 1 到 100 的随机数组成的数组 `Age`，我们希望将数据分成 4 个类别仓：
...
假设我们有一个 12 人的 `Height` 数组，从 150 厘米到 180 厘米随机开始，我们希望将数据分成 3 个类别。
