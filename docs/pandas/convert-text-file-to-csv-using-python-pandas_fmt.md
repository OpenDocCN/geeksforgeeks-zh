...
> 原文: [https://www.geeksforgeeks.org/convert-text-file-to-csv-using-python-pandas/](https://www.geeksforgeeks.org/convert-text-file-to-csv-using-python-pandas/)
...
**注意:** 数据框中的第一列是索引，这是读取文本文件时的默认设置。

一旦创建了数据帧，我们将使用 `DataFrame.to_csv()` 方法将该数据帧存储为 CSV 文件格式。
...
我们看到标题已成功添加，文件已从`.txt`格式化为`.csv`格式。

## 示例 3:

在本例中，文本文件中的字段由用户定义的分隔符`/`分隔。
...
在读取数据时，我们指定数据应该使用指定的分隔符进行标记化。在这种情况下是`/`。