...
> 原文: [https://www.geeksforgeeks.org/concatenate-strings-from-several-rows-using-pandas-groupby/](https://www.geeksforgeeks.org/concatenate-strings-from-several-rows-using-pandas-groupby/)

`DataFrame.groupby()` 方法用于根据一些标准将数据分成组。分组的抽象定义是提供标签到组名的映射。

要使用 `Dataframe.groupby()` 连接多行字符串，请执行以下步骤:

1.  使用 `Dataframe.groupby()` 方法对数据进行分组，您需要连接该方法的属性。
2.  使用连接函数连接字符串，并使用 [`lambda`](https://www.geeksforgeeks.org/python-lambda-anonymous-functions-filter-map-reduce/) 语句转换该列的值。
