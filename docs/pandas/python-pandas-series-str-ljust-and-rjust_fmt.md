...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-str-ljust-and-rjust/](https://www.geeksforgeeks.org/python-pandas-series-str-ljust-and-rjust/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Pandas` 的 `.ljust()` 和 `.rjust()` 是 `Series` 中用来处理文字数据的字符串方法。由于这些方法仅适用于字符串，每次调用这个方法之前都要加 `.str` 前缀。
...
- `width`: 输出字符串的最小宽度，如果宽度小于字符串长度，则不连接任何内容。
- `fillchar`: 字符串值，用传递的字符串填充 (`len(string) - width`) 个字符。
...
要下载以下示例中使用的数据集，请单击此处的[链接](https://media.geeksforgeeks.org/wp-content/uploads/employees.csv)。
...
在本例中，为 `Team` 列设置了 12 的最大宽度，并将 `"_"` 作为 `fillchar` 传递，用下划线填充剩余的空间。如果字符串长度小于宽度，则 `"_"` 将以字符串作为后缀。
...
**输出:**
如输出图像所示，`Team` 字符串现在在旧字符串后面加了 `"_"`。

![](img/8863791f20928b8f516ddce85fbd1fdc.png)

## 示例 2: 使用 `Series.str.rjust()`

在本例中，为 `Team` 列设置了 15 的最大宽度，并将 `"*"` 作为 `fillchar` 传递，以 `"*"` 填充剩余的空间。如果字符串长度小于宽度，则 `"*"` 将以字符串作为前缀。
