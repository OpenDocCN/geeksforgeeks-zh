...
> 原文: [https://www.geeksforgeeks.org/python-pandas-periodindex-dayofweek/](https://www.geeksforgeeks.org/python-pandas-periodindex-dayofweek/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。Pandas 就是其中一个包，让导入和分析数据变得容易多了。

`PeriodIndex.dayofweek` 属性返回一个 `Index` 对象，该对象给定 `PeriodIndex` 对象中每个元素的星期值。“0”是一周的第一天，“6”是一周的最后一天。

> **语法:** `PeriodIndex.dayofweek`
> **参数:** None
> **返回:** `Index`
