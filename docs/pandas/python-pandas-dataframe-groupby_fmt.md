# pandas DataFrame.groupby() 功能详解

> 原文链接: https://www.geeksforgeeks.org/python-pandas-dataframe-groupby/

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

[pandas groupby](https://www.geeksforgeeks.org/pandas-groupby/) 用于根据类别对数据进行分组，并对类别应用函数。它还有助于高效地聚合数据。

Pandas `dataframe.groupby()` 功能用于根据一些标准将数据分组。pandas 的物体可以在它们的任何一个轴上分开。分组的抽象定义是提供标签到组名的映射。
...
- `by` : 映射、函数、str 或可迭代
- `axis` : int，默认 0
- `level` : 如果轴是多索引(分层的)，则按特定的一个或多个级别
- `as_index` : 对于聚合输出，返回以组标签作为索引的对象。仅与数据帧输入相关。`as_index=False` 实际上是“SQL 风格”的分组输出
- `sort` : 排序组键。关闭此功能可以获得更好的性能。请注意，这不会影响各组内的观察顺序。groupby 保留每个组中的行顺序。
- `group_keys` : 调用 `apply` 时，将组合键添加到索引中以识别片段
- `squeeze` : 如果可能，降低返回类型的维度，否则返回一致的类型
...
使用 `groupby()` 功能基于多个类别形成组(即使用多个列执行拆分)。
