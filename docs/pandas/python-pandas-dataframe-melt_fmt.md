...
`DataFrame.melt()` 函数将数据框从宽格式取消固定为长格式，可选地保留标识符变量设置。该函数有助于将数据框消息转换为一种格式，其中一列或多列是标识符变量 (`id_vars`)，而所有其他被认为是测量变量 (`value_vars`) 的列被“取消固定”到行轴，只留下两个非标识符列，“变量”和“值”。

> **语法：** `DataFrame.melt(id_vars=None, value_vars=None, var_name=None, value_name='value', col_level=None)`
> **参数：**
> **frame：** DataFrame
> **id_vars：** 要用作标识符变量的列
...
**示例 2：** 使用 `melt()` 函数将列 “A” 设置为标识符变量，将列 “B” 和 “C” 设置为值变量。还可以自定义值列和变量列的名称。
