...
> 原文链接：https://www.geeksforgeeks.org/python-pandas-dateindex-hour/

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

Pandas 的 `DatetimeIndex.hour` 属性输出一个索引对象，该对象包含出现在 `DatetimeIndex` 对象的每个条目中的小时值。
...
使用 `DatetimeIndex.hour` 属性查找 `DatetimeIndex` 对象中的小时值。
...
使用 `DatetimeIndex.hour` 属性查找 `DatetimeIndex` 对象中存在的小时值。
