...
> 原文: [https://www.geeksforgeeks.org/python-pandas-panel-clip_lower/](https://www.geeksforgeeks.org/python-pandas-panel-clip_lower/)
...
- `threshold`: 允许的最小值。低于阈值的所有值都将设置为该值。
    - `float`: 将每个值与阈值进行比较。
    - `array-like`: 阈值的形状应该与其比较的对象相匹配。
- `axis`: 沿给定轴将自身与阈值对齐。
- `inplace`: 是否对数据执行到位操作。
