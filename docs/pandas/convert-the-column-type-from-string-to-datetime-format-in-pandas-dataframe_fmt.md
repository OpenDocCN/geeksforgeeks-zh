...
> 原文：[https://www.geeksforgeeks.org/convert-the-column-type-from-string-to-datetime-format-in-pandas-dataframe/](https://www.geeksforgeeks.org/convert-the-column-type-from-string-to-datetime-format-in-pandas-dataframe/)
...
## 代码#1：使用 `pd.to_datetime()` 函数

使用 `pd.to_datetime()` 函数将 Pandas dataframe 列类型从字符串转换为 datetime 格式。
...
## 代码#2：使用 `DataFrame.astype()` 函数

使用 `DataFrame.astype()` 函数将 Pandas dataframe 列类型从字符串转换为日期时间格式。
...
如果数据帧列为 `yymmdd` 格式，我们必须将其转换为 `yyyy-mm-dd` 格式。
...
在上例中，我们将“日期”列的数据类型从 `object` 更改为 `datetime64[ns]`，格式从 `yymmdd` 更改为 `yyyy-mm-dd`。

## 代码#4：转换多个列

使用 `pandas.to_datetime()` 将多个列从字符串转换为 datetime 格式。
...
在上例中，我们将列 `Treatment_start` 和 `Treatment_end` 的数据类型从 `object` 更改为 `datetime64[ns]` 类型。