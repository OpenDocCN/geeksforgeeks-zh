...
> 原文：[https://www.geeksforgeeks.org/how-to-read-text-files-with-pandas/](https://www.geeksforgeeks.org/how-to-read-text-files-with-pandas/)
...
> *   **`header`**：这是可选字段。默认情况下，它会将文本文件的第一行作为标题。如果我们使用 `header=None`，那么它将创建头。
> *   **`names`**：我们可以在导入文本文件时使用 `names` 参数分配列名。
...
在示例 2 中，我们将使 `header` 字段等于 `None`。这将在输出中创建一个默认标题。并将文本文件的第一行作为数据输入。创建的标题名称将是一个从 0 开始的数字。
...
在 `pandas` 中，我们可以使用 `read_table()` 从文本文件中读取数据。该函数将一般的分隔文件读取到数据框对象中。该函数本质上与 `read_csv()` 函数相同，但默认情况下使用 `delimiter='\t'`，而不是逗号。我们将使用 `read_table` 函数读取数据，使分隔符等于一个空格（`" "`）。
...
`read_fwf()` 函数中的 `fwf` 代表固定宽度的线条。我们可以使用这个函数从文件中加载数据帧。该功能还支持文本文件。我们将使用熊猫的 `read_fwf()` 函数从文本文件中读取数据。它还支持可选地迭代文件或将文件分成块。由于文本文件中的列是以固定的宽度分隔的，因此 `read_fwf()` 可以有效地将内容读入单独的列。
