...
熊猫`Series.notnull()`功能检测现有(非缺失)值。此函数返回一个与对象大小相同的布尔对象，指示值是否缺失。非缺失值映射到`True`。空字符串`""`或`numpy.inf`等字符不被视为数值(除非设置了`pandas.options.mode.use_inf_as_na = True`)。数值，如`None`或数值`NaN`，映射到`False`值。
...
使用`Series.notnull()`功能检测给定序列对象中的所有非缺失值。
