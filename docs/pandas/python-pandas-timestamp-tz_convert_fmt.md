...
> 原文:[https://www . geeksforgeks . org/python-pandas-timestamp-tz _ convert/](https://www.geeksforgeeks.org/python-pandas-timestamp-tz_convert/)
...
## 参数
**tz :** 时间戳将转换到的时区。`None`将删除保存世界协调时时间的时区。

## 返回
已转换:时间戳

## 示例#1
使用`Timestamp.tz_convert()`函数将给定的知晓时间戳对象转换为“亚洲/加尔各答”。
...
## 示例#2
使用`Timestamp.tz_convert()`函数将给定的知晓时间戳对象转换为“美国/太平洋”。
