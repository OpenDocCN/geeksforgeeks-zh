...
> 原文链接: https://www.geeksforgeeks.org/python-pandas-timedeltaindex-union/

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`TimedeltaIndex.union()` 函数是一个专门针对 `TimedeltaIndex` 对象的联合。它将重叠的范围与相同的日期偏移量相结合，将比 `Index.union` 快得多。

**语法:**
`TimedeltaIndex.union(other)`

**参数:**
*   `other`: `TimedeltaIndex` 或类似数组的

**返回:**
`Index` 或 `TimedeltaIndex`
