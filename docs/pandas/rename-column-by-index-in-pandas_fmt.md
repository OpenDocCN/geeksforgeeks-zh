...
> 原文: [https://www.geeksforgeeks.org/rename-column-by-index-in-pandas/](https://www.geeksforgeeks.org/rename-column-by-index-in-pandas/)
...
> **参数:**
>
> *   **`mapper`、`index` 和 `columns`:** 字典值，键指旧名称，值指新名称。一次只能使用这些参数中的一个。
> *   **`axis`:** 整数或字符串值，`0`/`'index'` 代表行，`1`/`'columns'` 代表列。
> *   **`copy`:** 如果为 `True`，则复制底层数据。
