# Pandas Series.describe() 函数详解

> 原文: [https://www.geeksforgeeks.org/python-pandas-series-describe/](https://www.geeksforgeeks.org/python-pandas-series-describe/)
...
- `include`: 要包含在结果中的数据类型的白名单。系列被忽略。
- `exclude`: 要从结果中省略的数据类型的黑名单。系列忽略
...
使用 `Series.describe()` 函数查找给定序列对象的汇总统计。
...
使用 `Series.describe()` 函数查找给定序列对象中底层数据的汇总统计。给定的序列对象包含一些缺失的值。
...
从输出中我们可以看到，`Series.describe()` 函数已经成功返回了给定序列对象的汇总统计。`NaN` 值在计算这些统计值时已被忽略。