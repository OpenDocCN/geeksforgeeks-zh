...
> 原文: [https://www.geeksforgeeks.org/python-pandas-multiindex-set_labels/](https://www.geeksforgeeks.org/python-pandas-multiindex-set_labels/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`MultiIndex.set_labels()` 功能在 `MultiIndex` 上设置新标签。默认返回新索引。
...
- `labels`：要应用的新标签
- `level`：要设置的级别（所有级别均为 `None`）
- `inplace`：如果为 `True`，则就地变异
- `verify_integrity`：如果为 `True`，则检查级别和标签是否兼容

## 示例 #1

使用 `MultiIndex.set_labels()` 函数重置 `MultiIndex` 的标签。
...
使用 `MultiIndex.set_labels()` 函数仅重置 `MultiIndex` 中的任何特定标签。
...
正如我们在输出中看到的，多索引的 `Char` 标签已经被重置为所需的值。