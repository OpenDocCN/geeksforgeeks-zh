...
在这篇文章中，我们将看到如何使用`modin`库来提高`pandas`的计算速度。`Modin`是一个与`pandas`非常相似的`python`库（在语法方面几乎完全相同），能够处理一个无法一次性装入`RAM`的巨大数据集。`pandas`在`MB`和几`GB`大小的数据集的执行速度方面已经足够好了，但是当我们处理非常大的数据集时，处理数据的速度成为瓶颈。

`pandas`库是为单核工作而设计的，因此随着现代计算能力的发展，每台个人笔记本电脑现在都至少有`2`个内核，`Modin`正好利用了这个机会，在所有可用的内核上执行操作，从而加快了整个过程。
...
`Append()`操作在`pandas`中非常常见，在下面的代码中，我们使用`pandas`和`Modin`运行了`10`次，并对其进行了计时，以查看加速差异。显然，莫迪打败了熊猫，因为它使用了我的系统上所有可用的内核。还用时间模块测量运算速度互相比较，原来`Modin`在这种情况下比`pandas`快`25`倍。
