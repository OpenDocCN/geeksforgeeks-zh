...
`DataFrame.info()` 功能用于获取数据帧的简明摘要。当对数据进行探索性分析时，它非常方便。为了快速了解数据集，我们使用 `DataFrame.info()` 函数。
...
> `max_cols`: 决定是打印全汇总还是短汇总。无符合 `display.max_info_columns` 设置。
...
> `null_counts`: 是否显示非空计数。如果为“无”，则仅当框架小于最大信息行数和最大信息列数时才显示。如果为真，则始终显示计数。如果为假，则从不显示计数。
