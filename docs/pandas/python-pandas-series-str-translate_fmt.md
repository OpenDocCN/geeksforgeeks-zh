...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-str-translate/](https://www.geeksforgeeks.org/python-pandas-series-str-translate/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Pandas` 的 `str.translate()` 是最重要也是最复杂的字符串方法之一。它使用翻译表根据翻译表翻译调用者的字符串序列。如果要翻译的值不止一个，字典会传递给 `maketrans()` 函数来创建翻译表。

> **语法:** `Series.str.translate(table, deletechars=None)`
>
> **参数:**
> **table:** 由 Python3 中的字典和 Python2 中的列表组成的翻译表。
> **deletechars:** 字符串类型，要删除的字符。此参数仅在 Python2 中正常工作(直到 Pandas 版本 0.23)
>
> **返回类型:** 一系列带翻译值的字符串

要下载下例使用的数据集，点击这里：[https://media.geeksforgeeks.org/wp-content/uploads/nba.csv](https://media.geeksforgeeks.org/wp-content/uploads/nba.csv)。
