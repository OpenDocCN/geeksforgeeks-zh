...
`pandas` 的 `Index.to_frame()` 函数从给定的索引创建一个数据帧，其中一列包含索引。默认情况下，原始索引在新数据帧中重复使用。为了加强新创建的数据框的新索引，我们将函数的 `index` 参数设置为 `False`。
...
使用 `Index.to_frame()` 函数将索引转换为数据帧，以便创建的数据帧使用新的索引值。
