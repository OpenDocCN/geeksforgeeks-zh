...
1.  从“团队”列中分组唯一值。
    ![](img/aacfc92246db88aa5ce56d9504414b1b.png)
2.  现在每组都有一个桶。
...
拆分是一个过程，在这个过程中，我们通过对数据集应用一些条件来将数据拆分成一个组。为了分割数据，我们对数据集应用了某些条件。为了拆分数据，我们使用`groupby()`函数，该函数用于根据一些标准将数据拆分成组。Pandas 对象可以在它们的任何一个轴上分开。分组的抽象定义是提供标签到组名的映射。Pandas 数据集可以分割成任何一个对象。有多种方法可以拆分数据，例如：
