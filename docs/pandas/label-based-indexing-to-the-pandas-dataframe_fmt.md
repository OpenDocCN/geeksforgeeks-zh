...
> 原文: [https://www.geeksforgeeks.org/label-based-indexing-to-the-pandas-dataframe/](https://www.geeksforgeeks.org/label-based-indexing-to-the-pandas-dataframe/)

索引在数据框中起着重要的作用。有时我们需要给熊猫数据框架一个基于标签的“花式索引”。为此，我们在熊猫中有一个功能被称为`pandas.DataFrame.lookup()`。花式索引的概念很简单，也就是说，我们必须通过一个索引数组来一次访问多个数组元素。

`pandas.DataFrame.lookup()` 函数将等长的行和列标签数组作为其属性，并返回对应于每个(行、列)对的值数组。
...
在上面的例子中，我们使用了基于标签的花式索引的概念来一次访问数据帧的多个元素，因此使用函数 `dataframe.lookup()` 创建了两个新列 `Age`、`Height` 和 `Date_of_Birth`。
