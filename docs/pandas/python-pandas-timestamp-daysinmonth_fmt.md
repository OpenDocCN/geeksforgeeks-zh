...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timestamp-daysinmonth/](https://www.geeksforgeeks.org/python-pandas-timestamp-daysinmonth/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Pandas` 的 `Timestamp.daysinmonth` 属性返回时间戳对象中给定日期的月份天数。
...
使用 `Timestamp.daysinmonth` 属性找出给定时间戳对象中的天数。
