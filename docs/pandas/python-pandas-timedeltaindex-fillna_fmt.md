...
`TimedeltaIndex.fillna()` 函数用指定的值填充给定时间增量索引对象中所有缺失的值。

> **语法:** `TimedeltaIndex.fillna(value=None, downcast=None)`
>
> **参数:**
> `value`: 用于填充孔洞的标量值(例如 0)。该值不能是喜欢的列表。
> `downcast`: 项的字典- >如果可能的话要向下转换的数据类型，或者尝试向下转换为适当的相等类型的字符串“推断”(例如，如果可能的话从 float64 到 int64)
>
> **返回:** 填充: `TimedeltaIndex`

## 示例 1

使用 `TimedeltaIndex.fillna()` 函数填充给定时间增量索引对象中所有缺失的值。
