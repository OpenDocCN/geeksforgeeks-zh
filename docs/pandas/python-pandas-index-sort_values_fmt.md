...
> 原文: [https://www.geeksforgeeks.org/python-pandas-index-sort_values/](https://www.geeksforgeeks.org/python-pandas-index-sort_values/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。***【熊猫】*** 就是其中一个包，让导入和分析数据变得容易多了。

`Index.sort_values()` 功能用于对索引值进行排序。函数返回索引的排序副本。除了对数值进行排序之外，该函数还可以对字符串类型的值进行排序。
...
- `ascending` : 索引值应该按升序排序。

**返回:**
- 索引的排序副本。
- `sorted_index` : `pandas.Index`
- `indexer` : `numpy.ndarray`，可选。索引本身排序所依据的索引。

## 示例 #1

使用 `Index.sort_values()` 函数对索引中的值进行排序。
...
使用 `Index.sort_values()` 功能对索引标签进行降序排序。
