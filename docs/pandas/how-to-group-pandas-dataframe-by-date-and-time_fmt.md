# 如何按日期和时间对Pandas数据框进行分组？

> 原文：[https://www.geeksforgeeks.org/how-to-group-pandas-dataframe-by-date-and-time/](https://www.geeksforgeeks.org/how-to-group-pandas-dataframe-by-date-and-time/)

在本文中，我们将讨论如何根据Pandas中的日期和时间对数据框进行分组。我们将看到按年、月、日等对时间序列数据帧进行分组的方法。此外，我们还将看到按时间对象（如分钟）分组的方法。
...
下面是一些例子，描述了如何使用Pandas的 `groupby` 方法根据日期和时间按数据帧分组。
...
在上面的示例中，数据框按 `Date` 列分组。由于我们提供了 `freq = 'M'`（意思是月），所以数据是按月分组的，直到每个月的最后一天，并提供了 `Price` 总和栏。我们没有提供所有月份的值，那么 `groupby` 函数也会显示所有月份的数据，并为其他月份分配值 0。
