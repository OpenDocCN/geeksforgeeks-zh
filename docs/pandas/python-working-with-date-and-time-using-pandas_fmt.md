...
`pandas.Series.dt.month`返回日期时间的月份。
`pandas.Series.dt.day`返回日期时间的当天。
`pandas.Series.dt.hour`返回日期时间的小时。
`pandas.Series.dt.minute`返回日期时间的分钟。
在此引用[中的所有`datetime`属性。](https://pandas.pydata.org/pandas-docs/stable/api.html#datetimelike-properties)
...
要获取当前时间，使用`Timestamp.now()`然后将`Timestamp`转换为`datetime`，直接访问年、月或日。
