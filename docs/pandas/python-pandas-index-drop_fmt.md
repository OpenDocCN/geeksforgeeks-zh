...
`Index.drop()`功能创建新的索引，删除传递的标签列表。该函数类似于`Index.delete()`，除了在该函数中我们传递标签名称而不是位置值。
...
> 如果'ignore'，抑制错误并删除现有标签。
> 
> **返回:** 下降: 指数
> 上升: 键错误。如果在所选轴中没有找到所有标签

## 示例 1

使用`Index.drop()`功能从索引中删除传递的标签。
...
使用`Index.drop()`函数在索引中删除包含日期时间数据的标签列表。
