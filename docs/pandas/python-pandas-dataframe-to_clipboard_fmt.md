...
> 原文: [https://www.geeksforgeeks.org/python-pandas-dataframe-to_clipboard/](https://www.geeksforgeeks.org/python-pandas-dataframe-to_clipboard/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`DataFrame.to_clipboard()` 功能复制对象到系统剪贴板。该函数将对象的文本表示写入系统剪贴板。这可以很容易地粘贴到 excel 或记事本++。
...
## 示例 1
使用 `to_clipboard()` 功能将对象以非 excel 格式复制到剪贴板。
...
## 示例 2
使用 `to_clipboard()` 功能将对象以 excel 格式复制到剪贴板。
