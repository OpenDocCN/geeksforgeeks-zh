...
> 原文: [https://www.geeksforgeeks.org/python-pandas-periodindex-freqstr/](https://www.geeksforgeeks.org/python-pandas-periodindex-freqstr/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`pandas` 的 `PeriodIndex.freqstr` 属性将频率对象作为字符串返回（如果设置了该属性），否则该函数将为给定的 `PeriodIndex` 对象返回 `None`。
