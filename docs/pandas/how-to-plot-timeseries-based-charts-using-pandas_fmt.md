...
> 原文: [https://www.geeksforgeeks.org/how-to-plot-timeseries-based-charts-using-pandas/](https://www.geeksforgeeks.org/how-to-plot-timeseries-based-charts-using-pandas/)
...
> **语法:** `plt.bar(x, height, width, bottom, align)`
...
它是一种常用的工具，用于检查数据集中的随机性。这种随机性是通过计算不同时滞下数据值的自相关来确定的。它显示了一种称为时间序列的数据类型的属性。大多数通用统计软件程序中都有这些图。可以使用 `pandas.plotting.autocorrelation_plot()` 来绘制。

> **语法:** `pandas.plotting.autocorrelation_plot(series, ax=None, **kwargs)`
>
> **参数:**
>
> *   `series`: 此参数是用于绘图的时间系列。
> *   `ax`: 该参数是 matplotlib axes 对象。其默认值为“无”。
>
> **返回:** 该函数返回 `matplotlib.axis.Axes` 类的对象
