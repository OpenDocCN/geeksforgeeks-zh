...
> 原文:[https://www . geeksforgeks . org/python-pandas-pants-panel-clip _ upper/](https://www.geeksforgeeks.org/python-pandas-panel-clip_upper/)

在熊猫中，面板是一个非常重要的三维数据容器。三个轴的名称旨在为描述涉及面板数据的操作，特别是面板数据的计量经济学分析提供一些语义含义。

`Panel.clip_upper()`函数用于返回给定值以上的值被截断的输入副本。

> **语法:** `Panel.clip_upper(threshold, axis=None, inplace=False)`
>
> **参数:**
> `threshold`: 像阈值一样的浮点数或数组。
> `axis`: 沿给定轴将对象与阈值对齐。
> `inplace`: 是否对数据执行到位操作。
>
> **返回:** 与输入类型相同。
