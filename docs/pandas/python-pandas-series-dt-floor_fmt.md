# Pandas Series.dt.floor() 方法详解

> 原文：[https://www.geeksforgeeks.org/python-pandas-series-dt-floor/](https://www.geeksforgeeks.org/python-pandas-series-dt-floor/)

`Series.dt`可用于访问系列的值，如 datetimelike，并返回几个属性。Pandas `Series.dt.floor()`功能对指定频率的数据进行楼层操作。
...
使用`Series.dt.floor()`函数将给定序列对象的日期时间数据拖到指定的频率。
