...
> 原文：[https://www.geeksforgeeks.org/add-a-new-column-in-pandas-data-frame-using-a-dictionary/](https://www.geeksforgeeks.org/add-a-new-column-in-pandas-data-frame-using-a-dictionary/)
...
现在使用上面写的方法，让我们尝试向它添加一个新的列。让我们添加名为`New_data_1`的新列。

**地图功能:** 通过给名为`data`的列赋予获取周名的功能，增加列`new_data_1`。调用`.map()`并传递`dict`，这将执行查找并返回该键的关联值。
