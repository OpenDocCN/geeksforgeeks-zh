...
> 原文：`https://www.geeksforgeeks.org/python-pandas-timestamp-isoweekday/`

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`pandas` 的 `Timestamp.isoweekday()` 功能返回日期代表的星期几。其中一周的天数从周一的 1 到周日的 7。
...
使用 `Timestamp.isoweekday()` 函数在给定的 `Timestamp` 对象中查找日期的星期几。
...
使用 `Timestamp.isoweekday()` 函数在给定的时间戳对象中查找日期的星期几。
