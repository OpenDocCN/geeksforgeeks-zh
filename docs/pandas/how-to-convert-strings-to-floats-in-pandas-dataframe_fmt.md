...
有时，我们可能没有用字符串表示的浮点值。因此，`pd.to_numeric()`函数将显示一个错误。要删除这个错误，我们可以使用 `errors= 'coerce'`，将这个位置的值转换为 `NaN`。
