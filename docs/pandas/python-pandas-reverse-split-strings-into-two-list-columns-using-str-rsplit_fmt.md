...
> 原文: [https://www.geeksforgeeks.org/python-pandas-reverse-split-strings-into-two-list-columns-using-str-rsplit/](https://www.geeksforgeeks.org/python-pandas-reverse-split-strings-into-two-list-columns-using-str-rsplit/)
...
要下载使用的 Csv 文件，点击这里的 [nba.csv](https://media.geeksforgeeks.org/wp-content/uploads/nba.csv)。
...
在本例中，“名称”列用空格(" ")分隔，`expand` 参数设置为 `True`，这意味着它将返回一个数据框，其中所有分隔的字符串都在不同的列中。然后使用数据框创建新列，并使用 `drop()` 方法删除旧的名称列。
