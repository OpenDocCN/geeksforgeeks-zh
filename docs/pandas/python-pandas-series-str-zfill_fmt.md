...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-str-zfill/](https://www.geeksforgeeks.org/python-pandas-series-str-zfill/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。Pandas 就是其中一个包，让导入和分析数据变得容易多了。

Pandas `zfill()` 方法是用零填充字符串的左侧。如果字符串的长度大于或等于宽度参数，则没有零作为前缀。由于这是一种字符串方法，所以只适用于系列字符串，每次调用这个方法之前都要加上 `.str` 前缀。
