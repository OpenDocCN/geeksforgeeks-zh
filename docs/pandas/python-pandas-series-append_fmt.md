...
Pandas `Series.append()` 功能用于连接两个或多个系列对象。

> **语法:** `Series.append(to_append, ignore_index=False, verify_integrity=False)`
>
> **参数:**
> - `to_append`: 系列或系列的列表/元组。
> - `ignore_index`: 如果为真，则不要使用索引标签。
> - `verify_integrity`: 如果为真，则在创建重复索引时引发异常。
>
> **返回:** 附加: 系列

## 示例 1

使用 `Series.append()` 函数将传递的序列对象追加到该序列对象的末尾。
...
使用 `Series.append()` 函数将传递的系列对象追加到该系列对象的末尾。忽略两个系列对象的原始索引。
