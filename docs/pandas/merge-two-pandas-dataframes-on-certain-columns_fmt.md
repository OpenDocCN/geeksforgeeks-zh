...
> 原文：[https://www.geeksforgeeks.org/merge-two-pandas-dataframes-on-certain-columns/](https://www.geeksforgeeks.org/merge-two-pandas-dataframes-on-certain-columns/)

通过简单地指定要合并的特定列，我们可以使用`merge()`函数合并特定列上的两个熊猫数据帧。
...
结果数据框包含`df1`的所有列，但`df2`的某些指定列带有键列`Name`，即结果列包含`Name`、`Marks`、`Grade`、`Rank`列。两个数据框的值数量不同，但合并后仅显示两个数据框中的公共值。

## 示例 2
在结果数据框中，`df2`的`Grade`列基于左侧合并类型的键列`Name`与`df1`合并，即显示左侧数据框(`df1`)的所有值。
...
## 示例 3
在这个例子中，我们已经合并了`df1`和`df2`。`df1`的`Marks`列与`df2`合并，此处仅显示两个数据框中基于键列`Name`的公共值。
