...
> 原文：[https://www.geeksforgeeks.org/dataframe-to_excel-method-in-pandas/](https://www.geeksforgeeks.org/dataframe-to_excel-method-in-pandas/)
...
| `sheet_name` | 字符串，默认 `'Sheet1'` | 将包含数据框的工作表的名称 |
| `columns` | 字符串序列或列表，可选 | 要写入的列 |
| `index` | 布尔，默认为 `True` | 写行名（索引） |
| `index_label` | 字符串或序列，可选 | 如果需要，索引列的列标签。如果未指定，并且 `header` 和 `index` 为真，则使用索引名称。如果数据框使用多索引，应该给出一个序列。 |

- 可以提供 excel 文件名或 `ExcelWriter` 对象。
- 默认情况下，工作表编号为 1，您可以通过输入参数 `sheet_name` 的值来更改它。
