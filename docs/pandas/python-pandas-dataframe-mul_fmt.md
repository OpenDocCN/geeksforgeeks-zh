...
`DataFrame.mul()`函数返回 `DataFrame` 和其他元素的乘法。这个函数本质上做了与 `DataFrame * other` 相同的事情，但是它提供了额外的支持来处理其中一个输入中的缺失值。
