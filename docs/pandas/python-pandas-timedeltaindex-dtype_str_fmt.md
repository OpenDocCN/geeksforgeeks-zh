# Python | Pandas 时间增量索引.dtype_str
...
`Pandas`的`TimedeltaIndex.dtype_str`属性以字符串形式返回时间增量索引对象的数据类型。

> **语法:**`TimedeltaIndex.dtype_str`
>
> **返回:**`dtype`

## 示例 1

使用`TimedeltaIndex.dtype_str`属性以字符串格式查找给定时间增量索引对象的数据类型。
...
使用`TimedeltaIndex.dtype_str`属性以字符串格式查找给定`TimedeltaIndex`对象的数据类型。
