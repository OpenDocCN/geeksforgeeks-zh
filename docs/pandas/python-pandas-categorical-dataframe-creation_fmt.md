# Python Pandas 分类 DataFrame 创建

> 原文：[https://www.geeksforgeeks.org/python-pandas-categorical-dataframe-creation/](https://www.geeksforgeeks.org/python-pandas-categorical-dataframe-creation/)

`pd.DataFrame(dtype="category")`：为了创建分类数据框，`DataFrame()` 方法将 `dtype` 属性设置为 `category`。
通过在数据框构造函数中指定 `dtype="category"`，数据框中的所有列都可以在构造期间或之后转换为分类类型。
