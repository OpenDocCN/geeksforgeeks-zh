...
> 原文: [https://www.geeksforgeeks.org/get-minimum-values-in-rows-or-columns-with-their-index-position-in-pandas-dataframe/](https://www.geeksforgeeks.org/get-minimum-values-in-rows-or-columns-with-their-index-position-in-pandas-dataframe/)

让我们讨论如何在数据框的行和列中找到最小值以及它们的索引位置。

## a) 求行和列中的最小值

`DataFrame.min()`：此函数返回给定对象中的最小值。如果输入是一个序列，该方法将返回一个标量，该标量将是该序列中值的最小值。如果输入是数据帧，则该方法将返回数据帧中指定轴上值最少的系列。默认情况下，该轴是索引轴。
...
### 2) 获取每一行的最小值
在具有`axis = 1`属性的数据框上使用`min()`函数来查找行轴上的最小值。
...
### 3) 获取每一列的最小值，不跳过无值
在数据帧上使用`min()`函数，该数据帧具有带`skipna = False`属性的 NA 值，以查找列轴上的最小值。
...
`DataFrame.idxmin()`：此函数返回请求轴上最小值的第一次出现的索引。当在任何索引中寻找最小值的索引时，所有的 NA/null 值都被排除。
...
### 2) 获取每行最小值的列名
使用带有`axis = 1`属性的`idxmin()`函数，沿列轴查找最小值的索引/标签。
