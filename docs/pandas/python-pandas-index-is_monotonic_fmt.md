...
> 原文: [https://www.geeksforgeeks.org/python-pandas-index-is_monotonic/](https://www.geeksforgeeks.org/python-pandas-index-is_monotonic/)

Pandas 索引是一个实现有序的、可切片的集合的不可变数组。它是存储所有 Pandas 对象的轴标签的基本对象。

Pandas `Index.is_monotonic` 属性是 `is_monotonic_increasing` 的别名。如果给定索引对象中的底层数据单调递增，则返回 `True`，否则返回 `False`。
...
使用 `Index.is_monotonic` 属性找出给定索引对象中的底层数据是否单调增加。
