...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-multiply/](https://www.geeksforgeeks.org/python-pandas-series-multiply/)
...
### 示例#1
使用 `Series.multiply()` 函数执行标量与给定序列对象的乘法运算。
...
### 示例#2
使用 `Series.multiply()` 函数执行标量与给定序列对象的乘法运算。给定的序列对象包含一些缺失的值。
