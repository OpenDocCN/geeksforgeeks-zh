...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timedeltaindex-get_duplicates/](https://www.geeksforgeeks.org/python-pandas-timedeltaindex-get_duplicates/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`TimedeltaIndex.get_duplicates()` 函数返回一个时间增量索引对象，该对象包含给定时间增量索引对象中的所有重复值。
...
**返回:** `TimedeltaIndex` 对象

## 示例 1

使用 `TimedeltaIndex.get_duplicates()` 函数查找给定时间增量索引对象中的所有重复值。
