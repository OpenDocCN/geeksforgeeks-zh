...
> 原文: `https://www.geeksforgeeks.org/python-pandas-timedeltaindex-to_frame/`

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`熊猫`就是其中一个包，让导入和分析数据变得容易多了。

`TimedeltaIndex.to_frame()`函数从给定的时间增量索引对象创建一个数据帧。时间增量索引对象的条目成为新构造的数据帧的列。

### 语法
`TimedeltaIndex.to_frame(index=True)`

### 参数
`index`: 将返回的数据帧的索引设置为原始索引（布尔值，默认为`True`）。

### 返回
`DataFrame`: 包含原始索引数据的数据帧。

## 示例 1
使用`TimedeltaIndex.to_frame()`函数从给定的时间增量索引对象构建数据帧。
