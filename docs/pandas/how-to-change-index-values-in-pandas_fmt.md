...
`索引`是用来唯一标识`数据框`中的一行。它只不过是一行的标签。如果我们在创建时没有为数据帧指定索引值，那么它将采用默认值，即从 0 到 n-1 的数字，其中 n 表示行数。
...
要更改索引值，我们需要使用`Pandas`中可用的`set_index()`方法来指定索引。

**语法**

> `dataframe.set_index("column_name_to_set_as_index", inplace=True/False)`
...
通过在位置中指定`inplace=False`（或不指定`inplace`参数，因为默认值为`False`）来临时更改索引。
...
**解释** – 由于我们没有在`set_index()`方法中指定`inplace`参数，默认为`False`，视为临时操作。
