...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timedeltaindex-sort_values/](https://www.geeksforgeeks.org/python-pandas-timedeltaindex-sort_values/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`TimedeltaIndex.sort_values()` 函数返回给定时间增量索引对象的排序副本。

> **语法:** `TimedeltaIndex.sort_values(return_indexer=False, ascending=True)`
>
> **参数:**
> **ascending:** 决定顺序
>
...
使用 `TimedeltaIndex.sort_values()` 函数以升序对给定的时间增量索引对象进行排序。
...
使用 `TimedeltaIndex.sort_values()` 函数按降序对给定的时间增量索引对象进行排序。
