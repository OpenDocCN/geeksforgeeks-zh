...
> 原文: [https://www.geeksforgeeks.org/python-pandas-dataframe-reindex/](https://www.geeksforgeeks.org/python-pandas-dataframe-reindex/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

## 语法与参数

`DataFrame.reindex()` 函数通过可选的填充逻辑使数据帧符合新索引，将 `NA`/`NaN` 放在先前索引中没有值的位置。除非新索引等同于当前索引并且 `copy=False`，否则将生成一个新对象。
...
>
> **参数:**
> - **labels:** 新标签/索引要符合‘轴’指定的轴到。
> - **index, columns:** 要符合的新标签/索引。最好是一个索引对象，以避免复制数据。
> - **axis:** 轴到目标。可以是轴名（`"index"`, `"columns"`）或数字（`0`, `1`）。
> - **method:** `{None, 'backfill'/'bfill', 'pad'/'ffill', 'nearest'}`, 可选。
> - **copy:** 返回新对象，即使传递的索引相同。
> - **level:** 跨级别广播，匹配传递的多索引级别上的索引值。
> - **fill_value:** 填充现有的缺失（`NaN`）值，以及成功的数据帧对齐所需的任何新元素，在计算前使用该值。如果两个相应数据框位置的数据都丢失，结果也将丢失。
> - **limit:** 向前或向后填充的连续元素的最大数量。
> - **tolerance:** 不精确匹配的原始标签和新标签之间的最大距离。匹配位置的索引值最符合方程式 `abs(index[stepper] - target) <= tolerance`。
>
> **返回:** 重新索引: 数据帧。

## 示例 1：重新索引行索引

使用 `reindex()` 功能重新索引数据帧。默认情况下，新索引中在 dataframe 中没有相应记录的值被分配给 `NaN`。
