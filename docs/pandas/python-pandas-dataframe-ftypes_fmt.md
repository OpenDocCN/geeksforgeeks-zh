# Pandas DataFrame.ftypes属性详解

> 哎哎哎:# t0]https://www . geeksforgeeks . org/python 熊猫 data frame-fttypes/

Pandas `DataFrame`是一个二维可变大小、潜在异构的表格数据结构，带有标记轴(行和列)。算术运算在行标签和列标签上对齐。它可以被认为是`Series`对象的类似字典的容器。这是熊猫的主要数据结构。

熊猫`DataFrame.ftypes`属性返回数据帧中的ftypes(稀疏/密集和dtype的指示)。它返回一个包含每列数据类型的`Series`。
...
现在我们将使用`DataFrame.ftypes`属性来检查给定数据帧中的列的ftype。
...
正如我们在输出中看到的，`DataFrame.ftypes`属性已经成功地返回了一个包含给定数据帧中每一列的ftypes的`Series`。
...
正如我们在输出中看到的，`DataFrame.ftypes`属性已经成功返回给定数据帧的ftype。