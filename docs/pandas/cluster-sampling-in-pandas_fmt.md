...
在这个输出中，我们看到，从具有随机数的 `N_numbers` 列中，顺便说一下，这是我们的随机集群，包含 4 个样本。借助`sample()`设置单个聚类呈现的样本数量。
...
正如您在我们的输出中看到的，我们收集了 4 个随机员工的数据，其中包含他们的 `employee_id` 和值。首先，我们创建一个 `samples` 变量，然后给它分配一个 4 个随机雇员的样本，作为我们的数据。