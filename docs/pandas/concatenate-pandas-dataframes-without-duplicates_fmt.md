...
> 原文: [https://www.geeksforgeeks.org/concatenate-pandas-dataframes-without-duplicates/](https://www.geeksforgeeks.org/concatenate-pandas-dataframes-without-duplicates/)

在本文中，我们将使用`pandas`模块连接两个数据帧。

为了执行两个数据帧的连接，我们将使用`pandas.concat().drop_duplicates()`方法。

## 分步方法

*   导入模块。
*   加载两个样本数据帧作为变量。
*   使用`pandas.concat().drop_duplicates()`连接数据帧。
*   显示生成的新数据帧。

以下是一些示例，描述如何使用`pandas`模块在两个数据帧之间执行无重复的拼接:
