...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-dataframe-any/](https://www.geeksforgeeks.org/python-pandas-series-dataframe-any/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

Pandas `any()` 方法适用于 `Series` 和 `DataFrame`。它检查调用者对象（`DataFrame` 或 `Series`）中的任何值是否不为 0，并为此返回真。如果所有值都为 0，它将返回 `False`。
...
- `bool_only`: 在 `DataFrame` 中检查 bool only 系列，如果没有找到，将只使用布尔值。此参数不适用于 `Series`，因为只有一列。
- `skipna`: 布尔值，如果为假，则为整个 NaN 列/行返回真。
- `level`: int 或 str，指定多级情况下的级别。
