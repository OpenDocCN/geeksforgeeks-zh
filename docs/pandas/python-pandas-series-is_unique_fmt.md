# Python Pandas Series.is_unique 属性详解

> 原文: [https://www.geeksforgeeks.org/python-pandas-series-is_unique/](https://www.geeksforgeeks.org/python-pandas-series-is_unique/)
...
Pandas `Series.is_unique` 属性返回一个布尔值。如果给定序列对象中的数据是唯一的，则返回 `True`，否则返回 `False`。
...
使用 `Series.is_unique` 属性检查给定序列对象中的基础数据是否唯一。
