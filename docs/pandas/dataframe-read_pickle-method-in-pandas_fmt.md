...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| `path` | `str`，`pathlib.Path` | 将加载腌制对象的文件路径。 |
| `compression` | `{ 'infer'，' gzip '，' bz2 '，' zip '，' xz '，' None ' }`，默认为`'infer'` | 用于磁盘数据的动态解压缩。如果`“infer”`，则使用 `gzip`、`bz2`、`xz` 或 `zip`(如果路径以`.gz`，`.bz2`，`.xz`，或`.zip`结尾)，否则不解压缩。设置为`None`表示不解压缩。 |
