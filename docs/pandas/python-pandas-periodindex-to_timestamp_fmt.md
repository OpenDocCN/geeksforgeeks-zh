...
> 原文: [https://www.geeksforgeeks.org/python-pandas-periodindex-to_timestamp/](https://www.geeksforgeeks.org/python-pandas-periodindex-to_timestamp/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`PeriodIndex.to_timestamp()` 函数将给定的周期索引对象转换为日期时间索引对象。

> **语法:** `PeriodIndex.to_timestamp(freq=None, how='start')`
>
> **参数:**
...
使用 `PeriodIndex.to_timestamp()` 函数将给定的周期索引对象转换为日期时间索引。
