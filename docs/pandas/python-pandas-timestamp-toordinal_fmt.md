...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timestamp-toordinal/](https://www.geeksforgeeks.org/python-pandas-timestamp-toordinal/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。Pandas 就是其中一个包，让导入和分析数据变得容易多了。

`Timestamp.toordinal()` 函数返回前视公历序数。第一年的 1 月 1 日是第一天。函数返回给定时间戳对象的序号值。

> **语法:** `Timestamp.toordinal()`
> **参数:** 无
> **返回:** 序数

## 示例 1

使用 `Timestamp.toordinal()` 函数返回给定时间戳对象的公历序数。
