# Python Pandas TimedeltaIndex.factorize() 方法详解

> 原文链接：【https://www.geeksforgeeks.org/python-pandas-timedeltaindex-factorize/】

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 Python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

## 方法介绍

Pandas 的 `TimedeltaIndex.factorize()` 函数将给定时间增量索引对象的元素编码为枚举类型或分类变量。
...
使用 `TimedeltaIndex.factorize()` 函数对给定时间增量索引对象的元素进行编码。
...
### 示例 2：检查两个索引对象的元素是否相同

使用 `TimedeltaIndex.factorize()` 函数检查两个给定时间增量索引对象中包含的元素是否相同。
