...
> 原文: [https://www.geeksforgeeks.org/how-to-do-a-vlookup-in-python-using-pandas/](https://www.geeksforgeeks.org/how-to-do-a-vlookup-in-python-using-pandas/)
...
> **参数:**
> **dataframe1:** 要合并的 `dataframe` 对象。
> **dataframe2:** 要合并的 `dataframe` 对象。
> **how:** `{left, right, inner, outer}` 指定合并将如何进行。
> **on:** 指定用于执行联接的列名或索引名。
> **suffixes:** 后缀用于重叠列。对于异常，使用值 `(False, False)`。
> **validate:** 如果指定，检查合并的种类。合并的类型可以是 `(one_to_one, one_to_many, many_to_one, many_to_many)`。
...
*   **内部连接:** 内部连接仅产生在两行中都满足条件的那些行的输出数据帧。要执行内部连接，您可以在 `how` 中指定 `inner` 作为关键字。
    **例:**
...
*   **右连接:** 右连接有点类似于左连接，在左连接中，输出数据帧将由第二个数据帧的所有行和第一个数据帧的匹配行组成。如果第一行中的行不匹配，则它们将被 `NaN` 替换。
