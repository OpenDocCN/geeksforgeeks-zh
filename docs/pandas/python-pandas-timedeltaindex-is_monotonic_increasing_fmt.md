...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timedeltaindex-is_monotonic_increasing/](https://www.geeksforgeeks.org/python-pandas-timedeltaindex-is_monotonic_increasing/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

## 语法与返回值

如果索引值单调递增，Pandas `TimedeltaIndex.is_monotonic_increasing` 属性返回 `True`。即使值相等，它也会返回 `True`。

> **语法:** `TimedeltaIndex.is_monotonic_increasing`
>
> **返回:** 布尔值

## 示例 1

使用 `TimedeltaIndex.is_monotonic_increasing` 属性检查时间增量索引对象是否单调增加。
