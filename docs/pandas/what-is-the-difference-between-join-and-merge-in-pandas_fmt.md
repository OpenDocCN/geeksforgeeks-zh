...
> 原文：[https://www.geeksforgeeks.org/what-is-the-difference-between-join-and-merge-in-pandas/](https://www.geeksforgeeks.org/what-is-the-difference-between-join-and-merge-in-pandas/)
...
`加入`方法获取两个数据帧，并在它们的索引上连接它们（从技术上讲，您可以为左边的数据帧选择要连接的列）。如果有重叠的列，联接将希望您从左侧数据框向重叠的列名添加后缀。我们的两个数据帧确实有一个重叠的列名 `P`。
...
请注意，索引被保留，我们有四列。我们还可以单独指定左侧数据框的特定列，并在 `on` 上使用参数作为连接键，但它仍将使用右侧的索引。
...
在基本层面上，`合并`或多或少做了和 `join` 一样的事情。这两种方法都用于将两个数据框组合在一起，但是 `merge` 功能更为多样，它需要将列指定为合并键。我们可以用 `on` 指定重叠列，也可以分别用 `left_on` 和 `right_on` 参数指定重叠列。
...
我们可以用 `left_index` 或 `right_index` 参数明确指定在索引的基础上进行合并。
