...
`TimedeltaIndex.equals()` 功能确定两个索引对象是否包含相同的元素。如果考虑的两个对象包含相同的元素，函数返回 `True`，否则函数返回 `False`。

> **语法:** `TimedeltaIndex.equals(other)`
>
> **参数:**
> **other:** 其他对象
>
...
使用 `TimedeltaIndex.equals()` 函数检查两个给定时间增量索引对象中包含的元素是否相同。
...
使用 `TimedeltaIndex.equals()` 功能检查两个给定的时间增量索引对象中包含的元素是否相同。
