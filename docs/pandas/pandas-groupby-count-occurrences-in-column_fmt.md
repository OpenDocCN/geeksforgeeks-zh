...
> 原文: [https://www.geeksforgeeks.org/pandas-groupby-count-occurrences-in-column/](https://www.geeksforgeeks.org/pandas-groupby-count-occurrences-in-column/)

使用`size()`或`count()`方法与`pandas.DataFrame.groupby()`将生成出现在dataframe特定列中的数据出现次数的计数。不过这个操作也可以用`pandas.Series.value_counts()`和`pandas.Index.value_counts()`。
