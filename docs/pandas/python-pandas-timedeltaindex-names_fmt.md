...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timedeltaindex-names/](https://www.geeksforgeeks.org/python-pandas-timedeltaindex-names/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`TimedeltaIndex.names` 属性返回一个包含时间增量索引对象名称的冻结列表（如果已设置）。如果没有为所考虑的对象设置名称，则返回 `None`。

> **语法:** `TimedeltaIndex.names`
>
> **返回:** `frozenlist`
...
**示例 2:** 使用 `TimedeltaIndex.names` 属性找出 `TimedeltaIndex` 对象的名称。
