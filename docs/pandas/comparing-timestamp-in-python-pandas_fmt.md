...
> 原文: [https://www.geeksforgeeks.org/comparing-timestamp-in-python-pandas/](https://www.geeksforgeeks.org/comparing-timestamp-in-python-pandas/)
...
给定时间可以使用 `pandas.Timestamp()` 方法转换为熊猫时间戳。这种方法可以采用各种形式的输入，如类似 `DateTime` 的字符串(如 `'2017-01-01T12'`)、以秒为单位的 Unix epoch(`1513393355.5`)等。这些值可以是年、月、日、小时、分钟、秒等。用逗号分隔或使用变量名。例如，如果我们想写 `2018/2/21 11:40:00`，我们可以提供 `(2018, 2, 21, 11, 40)` 作为 `Timestamp` 方法的参数，也可以写 `(year=2018, month=2, day=21, hour=11, minute=40)`。未提供的值将被视为零。下面的代码使用这种方法，使用提供的日期和时间信息创建时间戳列 `"new_time"`。
...
### 示例 1:
这里，比较 `"new_time"` 中的第一个和第二个时间戳，以知道其中最早的时间戳。
...
### 例 2:
这里，将 `'new_time'` 中的所有时间戳与 `Timestamp(2018-01-05 12:00:00)` 进行比较，返回该时间戳之前的条目。
...
### 示例 3:
这里我们再次将所有时间戳与 `Timestamp(2018-01-05 12:00:00)` 进行了比较，但将比较结果作为所有时间戳的布尔值(`True`/`False`)返回。
...
### 示例 4:
这里，`max()` 函数用于获取所有时间戳的最大值，即 `'new_time'` 列中的最近条目。
