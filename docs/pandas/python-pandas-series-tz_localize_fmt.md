...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-tz_localize/](https://www.geeksforgeeks.org/python-pandas-series-tz_localize/)
...
- **copy :** 同时复制底层数据
- **ambiguous:** 【推断】，`bool-n array`，【`NaT`】，默认“rail”
- **nonexistent:** `str`，默认“rail”
...
使用 `Series.tz_localize()` 函数将给定系列的时区初始索引定位到目标时区。
...
didx = pd.DatetimeIndex(start ='2014-08-01 10:00', freq ='W', periods = 6)
...
使用 `Series.tz_localize()` 函数将给定序列的时区初始索引定位到目标时区。
...
# Create the Datetime Index
didx = pd.DatetimeIndex(start ='2014-08-01 10:00', freq ='W', periods = 5)
