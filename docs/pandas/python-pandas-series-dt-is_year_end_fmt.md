...
> 原文: [https://www.geeksforgeeks.org/python-pandas-series-dt-is_year_end/](https://www.geeksforgeeks.org/python-pandas-series-dt-is_year_end/)

`Series.dt`可用于访问系列的值，如 datetimelike，并返回几个属性。Pandas `Series.dt.is_year_end`属性返回一个布尔值，指示日期是否是一年的最后一天。
...
## 示例 1
使用`Series.dt.is_year_end`属性检查给定序列对象的基础数据中的日期是否是一年的最后一天。
