...
> **`value`**: 要填充的静态、字典、数组、序列或数据框，而不是 NaN。
> **`method`**: 如果用户没有传递任何值，则使用`method`。pandas 有不同的方法，如`bfill`、`backfill`或`ffill`，分别在前进索引或上一个/后退中填入值。
> **`axis`**: 轴取行/列的 int 或 string 值。整数的输入可以是 0 或 1，字符串的输入可以是`index`或`columns`。
> **`inplace`**: 是一个布尔值，如果为真，则更改数据框本身。
> **`limit`**: 这是一个整数值，指定连续向前/向后 NaN 值填充的最大数量。
> **`downcast`**: 它需要一个字典来指定将哪个数据类型向下转换为哪个数据类型。就像 Float64 到 int64。
> **`**kwargs`**: 任何其他关键字参数。
...
在以下示例中，方法被设置为`ffill`，因此同一列中的值将替换空值。在这种情况下`Georgia`在第 4 行和第 5 行的学院列中替换了空值。
同样，也可以使用`pad`、`bfill`和`backfill`方法。
