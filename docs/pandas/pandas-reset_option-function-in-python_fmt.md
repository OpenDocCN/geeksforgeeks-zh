...
> 原文: [https://www.geeksforgeeks.org/pandas-reset_option-function-in-python/](https://www.geeksforgeeks.org/pandas-reset_option-function-in-python/)

Pandas 有一个选项系统，可以让我们定制它行为的某些方面，显示相关的选项是用户最有可能调整的。让我们看看如何将指定选项的值重置回默认值。
...
我们将使用 `pandas.get_option()` 更改选项的值，然后使用 `pandas.reset_option()` 将其重置回默认值。
...
我们可以通过在 `pandas.reset_option()` 函数中传递 `"all"` 作为参数，一次重置所有选项的值，而不是单独重置不同选项的值。
