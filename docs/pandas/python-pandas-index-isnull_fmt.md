...
`Pandas` 的 `Index.isnull()` 功能检测缺失值。它返回一个相同大小的布尔对象，指示值是否为“无”。数值，如 `None`、`numpy.NaN` 或 `pd.NaT`，映射到真值。其他一切都被映射到假值。空字符串 `""` 或 `numpy.inf` 等字符不被视为 NA 值（除非您设置 `pandas.options.mode.use_inf_as_na = True`）。
