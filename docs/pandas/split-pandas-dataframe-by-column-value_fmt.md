...
> 原文：[https://www.geeksforgeeks.org/split-pandas-dataframe-by-column-value/](https://www.geeksforgeeks.org/split-pandas-dataframe-by-column-value/)

有时，为了更准确地分析数据帧，我们需要将其分成 2 个或更多部分。Pandas 提供了根据列索引、行索引和列值等拆分数据帧的功能。

让我们看看如何在 Python 中按列值拆分[Pandas 数据帧](https://www.geeksforgeeks.org/python-pandas-dataframe/)？
...
在上面的例子中，基于列`Weight`的值，数据帧`df`被分成 2 个部分`df1`和`df2`。

## 方法二：使用[`DataFrame.groupby()`](https://www.geeksforgeeks.org/python-pandas-dataframe-groupby/)
...
在上面的例子中，数据帧`df`根据列`Salary`的值被分成两部分`df1`和`df2`。