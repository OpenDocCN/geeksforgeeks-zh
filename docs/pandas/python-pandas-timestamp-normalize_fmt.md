...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timestamp-normalize/](https://www.geeksforgeeks.org/python-pandas-timestamp-normalize/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

`Timestamp.normalize()` 功能用于将时间戳归一化为午夜。该函数在保存 tz 信息的同时执行该操作。
...
## 示例 1
使用 `Timestamp.normalize()` 函数将时间戳规范化为午夜值。
