...
> 原文: [https://www.geeksforgeeks.org/python-pandas-timestamp-month/](https://www.geeksforgeeks.org/python-pandas-timestamp-month/)

Python 是进行数据分析的优秀语言，主要是因为以数据为中心的 python 包的奇妙生态系统。`Pandas` 就是其中一个包，让导入和分析数据变得容易多了。

## 语法

`Timestamp.month` 属性用于在给定的 `Timestamp` 对象中查找月份值。
...
使用 `Timestamp.month` 属性在给定的 `Timestamp` 对象中查找月份值。
