...
因此，我们得到的学生表按照 `mark` 列按照学生的排名降序排列。

**注意:**在上面的查询中，注意 `select` 语句中列的顺序。如果 `mark` 列写在 `rank` 列之前，那么我们得不到想要的输出。因为每次 `@prev_mark` 被分配当前学生的分数，结果评价 `@prev_mark != mark` 为**假**。因此，每个学生的排名都显示为不递增，即它将保持为 0。