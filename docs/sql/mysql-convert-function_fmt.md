...
*   `input_value` – 用于指定输入值。
*   `data_type` – 用于指定需要转换的数据类型。
*   `character_set` – 用于指定需要转换的字符集。
...
实现 `CONVERT()`函数将值转换为 `UNSIGNED` 类型。
...
实现 `CONVERT()`函数，将字符串值转换为 `utf8` 字符集。
