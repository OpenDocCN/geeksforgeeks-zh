...
> 原文: [https://www.geeksforgeeks.org/how-to-pull-data-from-two-tables-sql-based-on-date/](https://www.geeksforgeeks.org/how-to-pull-data-from-two-tables-sql-based-on-date/)

在本文中，我们将看到如何编写 SQL 查询来根据日期从两个表中提取数据。我们可以通过对表执行 `UNION` 操作来执行上述功能。`UNION` 操作追加两个查询的结果，并将结果作为一个集合返回。

## 使用 `UNION` 关键字的方法

*   [`UNION ALL`](https://www.geeksforgeeks.org/union-and-union-all-in-ms-sql-server/): 包括重复项。
...
### 步骤 6: 基于日期从两个表中拉取数据的 SQL 查询

为了进行演示，我们将从 `'1990-01-01'` 年到 `'2000-01-01'` 年之间的数据提取出 `DOB`。
