...
PLSQL `INSTR2` 函数在字符串中搜索用户使用字符指定的子字符串，并返回字符串中指定出现的子字符串的第一个字符的位置。`INSTR2` 函数接受四个参数，即字符串、子字符串、起始位置和第 n 个外观。字符串和子字符串可以是任何数据类型，如 `CHAR`、`VARCHAR2`、`NCHAR`、`NVARCHAR2`、`CLOB` 或 `NCLOB`。
...
*   `start_position` – 这是一个可选参数，用于指定字符串中搜索开始的位置。默认值为 1。`INSTR2` 函数从字符串的末尾开始计数到 `start_position`，如果插入的值为负，则向字符串的开头搜索。
