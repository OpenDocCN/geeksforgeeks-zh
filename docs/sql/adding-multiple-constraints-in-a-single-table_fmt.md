...
> 原文: [https://www.geeksforgeeks.org/adding-multiple-constraints-in-a-single-table/](https://www.geeksforgeeks.org/adding-multiple-constraints-in-a-single-table/)
...
*   `company_type` 将包含以下值之一: “1C”、“2C”、“3C”
...
1.  **主键约束–**
    `Fd_master` 中的 `Sr_no`，其中 `pk` 是用户定义的给主键的名称。
2.  **外键约束–**
    `Fd_master` 中的 `Branch_no`，其中 `fk` 是引用 `Branch` 表的外键的名称。
3.  **检查约束–**
...
*   对默认值为 1 的 `ID` 具有默认约束的父级。给约束的名称是 `DF_ParentTable`。
*   带有 `ID` 列的外部表。
