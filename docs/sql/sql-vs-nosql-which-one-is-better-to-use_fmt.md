...
> 原文: [https://www.geeksforgeeks.org/sql-vs-nosql-which-one-is-better-to-use/](https://www.geeksforgeeks.org/sql-vs-nosql-which-one-is-better-to-use/)
...
但是选择数据库的决定并没有那么简单（到底是什么？！！）。SQL 和 NoSQL 数据库都有不同的结构和不同的数据存储方法。因此，在 SQL 和 NoSQL 之间的选择本质上归结为特定项目所需的数据库类型。
