...
MySQL 中的这个函数采用 IPv4 或 IPv6 地址的点状表示，并返回一个二进制字符串，该字符串以网络字节顺序表示地址的数值。如果输入地址不是有效的 IPv4 或 IPv6 地址，此函数返回空值。
...
在 `INET6_ATON` 函数的帮助下，检查以下地址的等效 `VARBINARY` 表示 `"2001:0db8:85a3:0000:0000:8a2e:0370:7334"`。因为它是一个有效的 IPv6 地址，所以我们会得到一个 `VARBINARY` 形式的结果。
