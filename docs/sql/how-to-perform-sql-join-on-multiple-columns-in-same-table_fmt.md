...
> 原文：[https://www.geeksforgeeks.org/how-to-perform-sql-join-on-multiple-columns-in-same-table/](https://www.geeksforgeeks.org/how-to-perform-sql-join-on-multiple-columns-in-same-table/)
...
## 步骤 3：创建表
在数据库 `GeeksForGeeks` 中创建一个表 `CORPORATE`。该表有 4 列，即 `E_NAME`、`E_ID`、`E_DEPT` 和 `E_LOC`，包含各种员工的姓名、ID、部门和地点。
...
## 步骤 7：检索特定员工
检索在至少 2 个部门和至少 2 个地点工作过的所有员工的详细信息。

**注：** 使用 `AS` 对表 `CORPORATE` 与 `C1` 和 `C2` 进行别名，以比较员工的身份证、部门和地点。
...
## 步骤 8：仅显示员工姓名
要仅显示在至少 2 个部门和至少 2 个地点工作过的员工的姓名，请在 `E_NAME` 列中使用 `SELECT`，并保持该列为 `DISTINCT` 以避免多余的行。
