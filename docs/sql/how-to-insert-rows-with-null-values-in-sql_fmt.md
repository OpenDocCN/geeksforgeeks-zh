...
> 原文: [https://www.geeksforgeeks.org/how-to-insert-rows-with-null-values-in-sql/](https://www.geeksforgeeks.org/how-to-insert-rows-with-null-values-in-sql/)
...
在数据库 `GeeksForGeeks` 中创建一个 `WORKER` 表。该表有 4 列，即学生身份、学生姓名、学生标准、英语、数学和物理，包含学生身份、学生姓名、标准和各科成绩。
...
## 步骤 6: 显示 WORKER 表的所有行

显示 `WORKER` 表的所有行，包括 0(零)值。
