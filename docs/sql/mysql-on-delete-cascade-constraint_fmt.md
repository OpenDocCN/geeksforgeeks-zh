...
## 第三步: 执行 `SELECT` 查询，检查 `Student` 表中的数据
...
## 第 6 步: 执行 `SELECT` 查询，查看 `Course` 表中的数据
...
## 第九步: 执行 `SELECT` 查询，查看 `Enroll` 表中的数据
...
## 第 10 步: 这里的父表是 `Student` 和 `Course`，子表是 `Enroll`。如果学生退出课程或课程从课程列表中删除，它也必然会影响子表。
