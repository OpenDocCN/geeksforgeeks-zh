...
> 原文: [https://www.geeksforgeeks.org/var-and-varp-function-in-ms-access/](https://www.geeksforgeeks.org/var-and-varp-function-in-ms-access/)
...
## 2. `VarP()` 函数
MS Access 中的 `VarP()` 函数用于估计一个群体的方差。`VarP()` 函数和 `Var()` 函数之间的主要区别是 `VarP` 评估总体，而 `Var` 函数评估总体样本。
