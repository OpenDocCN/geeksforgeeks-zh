...
**分部** (`/`): 对于分部请参考此链接 - [分部在 SQL](https://www.geeksforgeeks.org/sql-division/)
...
基本上模是用来检查一个数是偶数还是奇数。假设一个给定的数如果除以 2 给出 1 作为余数，那么它就是一个奇数或者如果除以 2 给出 0 作为余数，那么它就是一个偶数。

## 空的概念

如果我们对 `NULL` 进行任何算术运算，那么答案是总是 `null`。
...
在这里，输出总是为空，因为对空执行任何操作总是会产生一个空值。

**注意**: 确保空为不可用、未分配、未知。空不是与空格或零相同。
