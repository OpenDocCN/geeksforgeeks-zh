...
> 原文: [https://www.geeksforgeeks.org/how-to-custom-sort-in-sql-order-by-clause/](https://www.geeksforgeeks.org/how-to-custom-sort-in-sql-order-by-clause/)

默认情况下，`SQL ORDER BY`排序会按升序排列该列。当需要降序排序时，可以使用`ORDER BY DESC`。如果我们需要自定义排序，那么就需要使用`CASE`语句，在这里我们必须提到优先级来对列进行排序。

在本文中，让我们看看如何通过使用 MSSQL 作为服务器，使用`ORDER BY`在表中自定义排序。
...
## 步骤 1
我们正在创建一个数据库。为此，使用下面的命令创建一个名为 `GeeksforGeeks` 的数据库。
...
## 步骤 3
现在我们正在创建一个表。使用以下 SQL 查询创建一个包含 4 列的 `student_marks` 表。
...
## 步骤 5
向表中插入行的查询。使用以下 SQL 查询将行插入到 `student_marks` 表中。
...
根据 `stu_branch` 列进行自定义排序的查询，也显示了使用 `ORDER BY` 的默认排序与自定义排序之间的差异。
