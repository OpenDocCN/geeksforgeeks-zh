...
> 原文: [https://www.geeksforgeeks.org/difference-between-hadoop-and-sql-performance/](https://www.geeksforgeeks.org/difference-between-hadoop-and-sql-performance/)
...
| **垂直可扩展性** | 这意味着在现有机器上增加更多的硬件或 CPU | |
| **数据存储** | 数据可以以表格、键值对等形式存储 | 数据只能以表格形式存储。 |
| **完整性** | 低完整性 | 高完整性 |
| **数据多样性** | Hadoop 处理大数据并支持多种数据 | SQL 不支持多种数据 |
| **更新** | Hadoop 是写一次，读取多次。因此，数据更新实际上是不可能的 | SQL 是写一次，读和更新多次。因此数据更新非常容易完成 |
