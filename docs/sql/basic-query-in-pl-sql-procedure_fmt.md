...
> 原文: [https://www.geeksforgeeks.org/basic-query-in-pl-sql-procedure/](https://www.geeksforgeeks.org/basic-query-in-pl-sql-procedure/)
...
1.  **IN** – 它是只读参数。输入参数就像一个常数。在被调用的程序或函数中，它可以被引用。程序无法为输入参数指定新值。它们的值不能在子程序中更改。
2.  **OUT** – 用于获取子程序输出。它是子程序中的读写变量。它们的值可以在子程序中更改。
3.  **IN OUT** – 从子程序中获取输入输出，然后这个输入输出可以用来获取结果。它们的值可以在子程序中更改。