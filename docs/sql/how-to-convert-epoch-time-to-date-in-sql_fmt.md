...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-epoch-time-to-date-in-sql/](https://www.geeksforgeeks.org/how-to-convert-epoch-time-to-date-in-sql/)

`DATEADD()`函数的作用是：将一个时间或日期间隔加总到一个指定的日期，然后返回修改后的日期。`DATEADD()`有以下一些功能：
...
这里我们将看到，如何在 SQL Server 中使用`DATEADD()`函数将纪元时间转换为日期。为了演示，我们将在名为“geeks”的数据库中创建一个`EpochDB`表。
