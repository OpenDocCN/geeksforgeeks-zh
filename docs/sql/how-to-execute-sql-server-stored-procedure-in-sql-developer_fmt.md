...
> 原文：[https://www.geeksforgeeks.org/how-to-execute-sql-server-stored-procedure-in-sql-developer/](https://www.geeksforgeeks.org/how-to-execute-sql-server-stored-procedure-in-sql-developer/)
...
## 步骤 3：创建表
现在我们创建一个表。使用以下 SQL 查询创建一个包含 3 列的 `student_details` 表。
...
## 步骤 4：插入数据
向表中插入行的查询。使用以下 SQL 查询将行插入 `student_details` 表。
