...
| **系统角色** | **授予角色的特权** |
| :--- | :--- |
| `CONNECT` | `CREATE TABLE`、`CREATE VIEW`、`CREATE SYNONYM`、`CREATE SEQUENCE`、`CREATE SESSION` 等。 |
| `RESOURCE` | `CREATE PROCEDURE`、`CREATE SEQUENCE`、`CREATE TABLE`、`CREATE TRIGGER` 等。资源角色的主要用途是限制对数据库对象的访问。 |
| `DBA` | 所有系统权限 |
...
首先创建一个 `manager` 角色，然后允许 `manager` 创建表和视图。然后，它授予 `SAM` 和 `STARK` `manager` 的角色。现在 `SAM` 和 `STARK` 可以创建表格和视图。如果用户被授予多个角色，他们将获得与所有角色相关联的所有权限。然后使用 `REVOKE` 从角色 `manager` 中删除 `CREATE TABLE` 权限。使用 `DROP` 从数据库中删除该角色。