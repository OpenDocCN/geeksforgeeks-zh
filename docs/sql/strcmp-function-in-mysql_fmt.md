...
`STRCMP()` 函数用于比较两个字符串。如果两个字符串相同，则返回 0，如果根据定义的顺序，第一个参数小于第二个参数，则返回 -1，当第二个参数小于第一个参数时，则返回 1。
...
`STRCMP()` 函数比较两个相等的字符串。由于两个给定的字符串相等，它将返回 0。
...
`STRCMP()` 函数用于在第二个字符串小于第一个字符串时比较两个字符串。这里，返回值将是 1。
...
`STRCMP()` 函数在第二个字符串大于第一个字符串时比较两个字符串。由于第二个字符串大于第一个字符串，结果将为 -1。
...
`STRCMP()` 函数也可以用于列数据。为了演示，创建一个名为 `StudentDetails` 的表。
...
```sql
INSERT INTO
    StudentDetails(First_name, Last_name, Student_Class, TotalExamGiven)
VALUES
    ('Sayan', 'Jana', 'IX', 8),
    ('Nitin', 'Sharma', 'X', 5),
...
| Student_id | First_name | Last_name | Student_Class | TotalExamGiven |
| --- | --- | --- | --- | --- |
| 1 | Sayan | Jana | IX | 8 |
| 2 | Nitin | Sharma | X | 5 |
...
现在，我们将使用 `STRCMP` 函数来比较 `First_Name` 列和 `Last_Name` 列。
