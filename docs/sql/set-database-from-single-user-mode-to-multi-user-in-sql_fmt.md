...
> 原文: [https://www.geeksforgeeks.org/set-database-from-single-user-mode-to-multi-user-in-sql/](https://www.geeksforgeeks.org/set-database-from-single-user-mode-to-multi-user-in-sql/)
...
在这里，我们可以看到“单一用户”显示在 `geeks` 数据库名称旁边。
...
在这里，之前显示在 `geeks` 数据库旁边的“单用户”关键字现在被删除了。
...
*   选择数据库文件夹，并选择您的数据库(`geeks`，我们已经在这里使用)。
*   右键单击数据库并选择属性选项。
