...
> 原文: [https://www.geeksforgeeks.org/how-to-select-all-records-from-one-table-that-do-not-exist-in-another-table-in-sql/](https://www.geeksforgeeks.org/how-to-select-all-records-from-one-table-that-do-not-exist-in-another-table-in-sql/)
...
使用以下 SQL 查询将行插入 `employee_details` 表和 `employee_resigned` 表：
...
使用 `NOT EXISTS` 查询未辞职人员的 `emp_id` 和 `emp_name`：
