...
> 原文: [https://www.geeksforgeeks.org/sql-query-to-get-yesterday-and-tomorrow/](https://www.geeksforgeeks.org/sql-query-to-get-yesterday-and-tomorrow/)
...
为了得到当前日期的昨天和明天，我们可以使用 MySQL 中的 `CURDATE()` 函数，从中减去 1 得到昨天，再加上 1 得到明天。
...
> **语法:**
> `SELECT dates, DATE_SUB(dates, INTERVAL 1 DAY) AS some_name, DATE_ADD(dates, INTERVAL 1 DAY) AS some_name;`
...
> **语法:**
> `SELECT column_name, DATE_SUB(column_name, INTERVAL 1 DAY) AS some_name, DATE_ADD(column_name, INTERVAL 1 DAY) AS some_name, DAYNAME(CURRENT_DATE), DAYNAME(previous_date), DAYNAME(next_date);`
