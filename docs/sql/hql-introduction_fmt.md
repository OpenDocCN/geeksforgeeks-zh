...
*   子句(`from`、`select`、`in`、`order by`、`group by`)
*   关联和连接(`inner join`、`left outer join`、`right outer join`、`full join`)
*   聚合函数(`avg`、`sum`、`min`、`max`、`count` 等))
*   表达式(数学运算符、二进制比较、字符串连接、SQL 标量函数等。)
*   子查询(`any`、`all`、`some`、`in`)

## HQL 查询

查询中的查询称为子查询，用括号括起来。子查询的一个例子是子选择，其中 `select` 子句嵌入到另一个子句中，例如 `select`、`from`、和 `where` 子句。子查询在主查询执行之前执行。

例如，以下查询返回所有出价小于 100 的项目。

```
from Item where 100 > (select b.amount from Bid b)
```
