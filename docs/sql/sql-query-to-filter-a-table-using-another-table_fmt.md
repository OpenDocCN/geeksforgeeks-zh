...
> 原文：[https://www.geeksforgeeks.org/sql-query-to-filter-a-table-using-another-table/](https://www.geeksforgeeks.org/sql-query-to-filter-a-table-using-another-table/)

在本文中，我们将看到如何使用另一个表来过滤一个表。我们可以使用子查询代替 `WHERE` 子句中的条件来执行该函数。另一个查询内部的查询称为子查询。它也可以称为嵌套查询。一个 SQL 代码可以有一个或多个嵌套查询。
...
在 `geeks` 的数据库中，我们有两个名为 `demo_table1` 和 `demo_table2` 的表。
...
为了演示，我们将过滤 `demo_table1` 中收入大于 `demo_table2` 中最大收入的数据。
...
```sql
SELECT * FROM demo_table1 WHERE INCOME > (SELECT MAX(INCOME) FROM demo_table2);
```
...
从图中可以看到，`demo_table1` 中的数据被过滤掉，收入超过了 `650000`（`demo_table2` 中的最大收入值）。