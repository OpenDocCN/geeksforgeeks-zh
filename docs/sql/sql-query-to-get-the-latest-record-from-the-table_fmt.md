...
**步骤 1:** 要解决查询，首先[创建一个表](https://www.geeksforgeeks.org/sql-create/)，从中提取记录。这里，创建了一个名为`Student`的表，其中包含学生数据，如`StudentID`、`StudentFirstName`、`studentLastName`、`Age`和`Course`。
...
我们在表格顶部得到的数据是我们的最新数据，我们将使用 [`Order By`](https://www.geeksforgeeks.org/sql-order-by/) 降序得到我们的记录。
