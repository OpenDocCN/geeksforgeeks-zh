...
> 原文: [https://www.geeksforgeeks.org/sql-query-to-update-all-columns-in-a-table/](https://www.geeksforgeeks.org/sql-query-to-update-all-columns-in-a-table/)
...
## 步骤 3: 创建薪资表
在数据库 `GeeksForGeeks` 中创建一个薪资表。该表有两栏，即 `MONTHLY_SALARY` 和 `ANNUAL_SALARY`，包含工厂工人的月工资和年工资。
...
## 步骤 7: 更新表中所有列
更新表 `SALARY`。将 `MONTHLY_SALARY` 栏的所有条目值增加 `1000`，相应地，将 `ANNUAL_SALARY` 栏的所有条目值增加 `12*1000=12000`。
