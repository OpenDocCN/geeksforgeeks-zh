...
> 原文：[https://www.geeksforgeeks.org/compare-sql-server-results-of-two-queries/](https://www.geeksforgeeks.org/compare-sql-server-results-of-two-queries/)
...
在比较作者和 `Author2021` 表格内容时，对于像 `AuthorName`、`Age`、`skillset`、`NumberOfPosts` 这样的字段名，只有上面显示的数据是常见的，因此在输出中，我们看到了这一点。
