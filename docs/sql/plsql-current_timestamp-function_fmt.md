...
> 原文: [https://www.geeksforgeeks.org/plsql-current_timestamp-function/](https://www.geeksforgeeks.org/plsql-current_timestamp-function/)
...
**返回值:**
`CURRENT_TIMESTAMP` 函数返回 `TIMESTAMP WITH TIME ZONE` 数据类型中当前时间戳的值。
...
**优势:**
`CURRENT_TIMESTAMP` 函数返回带时区的时间戳值，而 `CURRENT_DATE` 函数返回不带时区数据的日期值。