...
> 原文: [https://www.geeksforgeeks.org/how-to-add-a-boolean-datatype-column-to-an-existing-table-in-sql/](https://www.geeksforgeeks.org/how-to-add-a-boolean-datatype-column-to-an-existing-table-in-sql/)

在 SQL Server 中，可以通过保留 `BIT` 数据类型来创建布尔数据类型。虽然它是一个数字数据类型，但它只能接受 `0` 或 `1` 或空值。因此，我们可以很容易地将假值赋给 `0`，将真值赋给 `1`。这将为数据类型提供布尔特性。

关于存储，如果表中的 `BIT` 数据少于 9 列，则存储为 1 字节。对于 9 到 16 位的列，它们的消耗是 2 字节，以此类推，它将保持不变。在本文中，让我们来看看如何向 SQL SERVER 中的现有表添加布尔数据类型，即 `BIT` 数据类型。
...
让我们向 `Authors` 表中添加一个 `BIT` 数据类型。`BIT` 数据类型有助于表示真(`1`)或假(`0`)的布尔性质，它们是 `BIT` 数据类型唯一允许的值。
...
因此，当一个 `BIT` 数据类型列被添加到一个现有的表中时，它的值将被填充为“空”值。
...
由于 `BIT` 列只支持 `0` 或 `1` 作为值，我们如上所述进行了填充。
...
这证明了 `BIT` 数据类型只接受“布尔值”，即只接受 `0` 或 `1`。如果给定一个非零值，它只被转换为 `1`。

**优势:**

*   如果预期值为 `0` 或 `1` 加空，这是一种很好的方法，可以在代码变得合乎逻辑时将列保持为 `BIT` 数据类型
*   关于存储，使用起来很经济，因为与其他数据类型相比，它占用了一点空间。

考虑到这些优点，`BIT` 数据类型是存储布尔数据的正确方法