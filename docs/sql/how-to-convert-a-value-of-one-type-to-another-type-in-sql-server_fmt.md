...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-a-value-of-one-type-to-another-type-in-sql-server/](https://www.geeksforgeeks.org/how-to-convert-a-value-of-one-type-to-another-type-in-sql-server/)
...
*   `target_type` – 是表达式要转换成的目标数据类型，例如：`INT`、`BIT`、`SQL_VARIANT` 等。
*   `length` – 它提供了 `target_type` 的长度。长度不是强制性的。默认长度设置为 30。
*   `expression` – 是任何将要被转换的表达式。
