...
> 原文: [https://www.geeksforgeeks.org/sql-query-to-select-all-records-from-employee-table-where-name-is-not-specified/](https://www.geeksforgeeks.org/sql-query-to-select-all-records-from-employee-table-where-name-is-not-specified/)
...
由于我们需要显示除帕拉德普之外的名称，因此我们可以在 `where` 子句中使用不等于 (`<>`) 运算符来执行所需的查询，在 `WHERE` 子句中，我们可以使用任何其他条件，也可以使用其他运算符如 `>`、`<`、`AND`、`OR`、`NOT` 等。
...
使用 `AND` 运算符我们可以在这里合并不同的条件，`AND` 用于执行以下查询。
