...
> 原文：[https://www.geeksforgeeks.org/how-to-find-duplicate-records-that-meet-certain-conditions-in-sql/](https://www.geeksforgeeks.org/how-to-find-duplicate-records-that-meet-certain-conditions-in-sql/)
...
我们来想象一下这样一种情况：由于某些原因，极客的表中出现了一个 bug，插入的副本很少。理想情况下，每一行都应该有一个唯一的 `GeekRank` 值，但是现在在我们的表中插入了具有重复等级的重复极客。
...
在上面新更新的表格中，我们可以看到有 2 条记录的极客等级都是 5。`Geek 107` 和 `Geek 108` 的等级相同，都是 5。现在，我们需要使用 SQL Query 找到这种重复。
...
使用 `GROUP BY` 和 `HAVING` 子句，我们可以在表数据中显示重复项。通过 `GROUP BY GeekRank` 的意思是，将只有该特定列（即 `GeekRank`）具有相同值的所有行放在一个组中。我们使用 `HAVING COUNT(GeekRank)>1` 来选择输出中极客等级超过 1 的列。这里使用了 SQL 的 `COUNT()` 函数来计算重复的行。在这里，我们将我们的新专栏命名为 `DuplicateRanks`，它计算重复的等级。
...
如我们所见，具有重复极客等级的行被分组在同一个极客等级下，它们对应的计数是重复行的极客等级的计数。`Geek ID 107` 和 `Geek ID 108` 具有相同的排名 5。因此，在上面的输出中，我们可以看到极客等级为 5（因为第 5 个等级被发现是重复的），并且由于两个极客标识具有相同的极客等级 5，因此重复记录的计数为 2。找到重复行后，您可以选择使用 `DELETE` 语句删除这些重复行。