...
SQL 中的 `RIGHT JOIN` 关键字返回所有匹配的记录(或行)和记录(或行)，它们出现在右表中但不在左表中。这意味着，如果某一行出现在右表中但不在左表中，结果将包括该行，但在左表的每一列中有一个空值。如果左表中的记录不在右表中，它将不会包含在结果中。
...
在本例中，我们将考虑两个表格员工表格，包含在特定部门工作的员工的详细信息，以及部门表格，包含该部门的详细信息
