...
如果是 `客户.CUSTOMERID = ACCOUNTS.CUSTOMERID` 在子查询中评估为真，返回一行&因此子查询评估为真&我们的外部查询(从客户中选择*)针对该特定客户执行。
...
| 5。 | Input operators cannot compare any values with null values. | The `EXISTS` clause can compare everything with `NULL`s. |
