...
2020 年 9 月 27 日使用`weekday()`函数求当前日期的 WEEKDAY 值。
...
当日期为空时，使用`weekday()`函数从给定的日期时间中查找工作日值。
...
在本例中，我们将查找每个工作日售出的产品数量。演示创建一个名为`Product`的表。
