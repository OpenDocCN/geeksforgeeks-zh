...
> 原文: [https://www.geeksforgeeks.org/selecting-multiple-columns-based-on-condition-in-sql/](https://www.geeksforgeeks.org/selecting-multiple-columns-based-on-condition-in-sql/)
...
为了更清楚，让我们以上面创建的 `EMPLOYEE` 表为例。现在，我们必须从给定的表中选择列 `FirstName`。
...
在带有或不带有任何条件的 SQL 中选择多列就像选择一列一样简单，不仅简单，而且相同。再来看我们前面的例子，这一次我们必须写一个查询来从 `employee` 表中获取 `FirstName` 和 `LastName`，这意味着我们必须提取 2 列，解决方案非常简单，我们必须用我们想要提取的 [`SELECT` 子句](https://www.geeksforgeeks.org/sql-select-query/#:~:text=The%20select%20clause%20is%20the,in%20the%20final%20result%20set.)然后是表名来写所有那些列名。其语法如下所示。
...
*   选择列名和姓，即选择 `FirstName`、`LastName`。
*   来自 `Employee` 表，即 `FROM Employee`。
*   现在，条件是棘手的部分，因为有两个条件，让我们逐一处理:
    *   工资必须是 10000，即 `Salary=10000`。
    *   姓氏应该是 `Chauhan`，即 `LastName='Chauhan'`。
