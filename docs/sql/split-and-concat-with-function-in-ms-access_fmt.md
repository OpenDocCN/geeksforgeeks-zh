...
> 原文: [https://www.geeksforgeeks.org/split-and-concat-with-function-in-ms-access/](https://www.geeksforgeeks.org/split-and-concat-with-function-in-ms-access/)
...
## 2. `&`连接符
在 MS Access 中，我们可以借助`&`将两个或两个以上的字符串组合成一个字符串。它会将所有字符串组合成一个字符串。我们可以使用`&`运算符将多个字符串串联成一个字符串。在 Concat 函数的帮助下，字符串之间也可以进行连接。`&`运算符没有参数或自变量。
