# MS Access 中的 SLN() 和 SYD() 函数

> 原文: [https://www.geeksforgeeks.org/sln-and-syd-function-function-in-ms-access/](https://www.geeksforgeeks.org/sln-and-syd-function-function-in-ms-access/)
...
**注:** `life` 和 `period` 参数必须用相同的单位表示。例如，如果 `life` 以月为单位，那么 `period` 也必须以月为单位。所有参数必须是正数。
