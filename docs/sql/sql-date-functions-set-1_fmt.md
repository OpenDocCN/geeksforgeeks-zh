...
10. `DATE_ADD()`：添加一定的时间/日期间隔后返回日期。
    ```sql
    SELECT DATE_ADD("2018-07-16", INTERVAL 10 DAY);
    ```
    **输出：** 2018-07-16

11. `DATE_FORMAT()`：按照格式掩码指定的格式设置日期。
    ```sql
    SELECT DATE_FORMAT("2018-06-15", "%Y");
    ```
    **输出：** 2018

12. `DATE_SUB()`：减去一定的时间/日期间隔后返回日期。
    ```sql
...
19. `FROM_DAYS()`：它从一天的数字表示中返回一个日期值。
    ```sql
