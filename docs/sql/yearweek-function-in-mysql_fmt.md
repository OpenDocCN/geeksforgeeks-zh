...
## 参数:
该方法接受两个参数，如上所述，如下所述:
*   **Date:** 我们想从中提取年和周的日期或日期时间。
*   **Mode:** 指定一周从哪一天开始。下表描述了 mode 参数的工作方式。

| mode | 一周的第一天 | 范围 | 第一周是…… |
| --- | --- | --- | --- |
| 0 | Sunday | 0-53 | 今年有星期日的一周。 |
| 3 | Monday | 1-53 | 今年有四天或更多天的一周。 |
| 4 | Sunday | 0-53 | 今年有四天或更多天的一周。 |
| 5 | Monday | 0-53 | 今年有四天或更多天的一周。 |
| 7 | Monday | 1-53 | 使用今年的星期一。 |

## 返回:
返回年和周的值。

## 示例-1 :
使用 2020 年 9 月 28 日的 `YEARWEEK()`函数查找当前年份和星期。
```sql
...
## 示例-3 :
当日期为空时，使用 `YEARWEEK()`函数从给定的日期时间中查找年和周。
```sql
