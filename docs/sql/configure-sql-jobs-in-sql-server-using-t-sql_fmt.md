...
> 原文: [https://www.geeksforgeeks.org/configure-sql-jobs-in-sql-server-using-t-sql/](https://www.geeksforgeeks.org/configure-sql-jobs-in-sql-server-using-t-sql/)
...
*   **`@notify_level_page`** –
    此参数用于发送 SQL 作业结果的寻呼机通知。参数的有效值与`@notify_level_eventlog` 参数值相同。
*   **`@delete_level`** –
...
| 子系统值 | 描述 |
| --- | --- |
| `CmdExec` 文件(`*.exe`，`*.bat`) | 操作系统命令或可执行文件 |
| `ANALYSIS` 查询 | SQL Server 分析服务查询，例如，`DMX` `MDX` |
| `ANALYSISQUERY` | SQL Server 分析服务命令，例如 `XMLA`。 |
| `SSIS` | SQL Server 集成服务包。 |
| `PowerShell` | 脚本上的 PowerShell 命令。 |
| `T-SQL` | 查询或存储过程 |
| `Distribution` | SQL Server 复制分发服务器代理。 |
| `Snapshot` | SQL Server 复制快照代理。 |
| `LogReader` | SQL Server 复制日志读取器代理。 |
...
*   **`@freq_type`** –
    表示执行 SQL 作业的时间。参数的数据类型为 `INT`，默认值为 0。有效值为以下任何一项:
