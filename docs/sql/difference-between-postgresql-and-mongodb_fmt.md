...
> 原文: [https://www.geeksforgeeks.org/difference-between-postgresql-and-mongodb/](https://www.geeksforgeeks.org/difference-between-postgresql-and-mongodb/)
...
| :--- | :--- | :--- |
| **数据库类型** | 非关系型数据库管理系统 | 关系型数据库管理系统 |
| **数据模型** | 面向文档 | 面向对象（关系型） |
| **开发语言** | 用 `C++` 编写 | 用 `C` 语言编写 |
| **起源** | 由 `10gen` 于 `2007` 年创立。它基于巨大的词创建了此产品。 | ... |
| **支持** | ... | `PostgreSQL` 为用户提供多种社区和商业支持选项。社区支持包括邮件列表和 `IRC`。用户还可以通过企业支持获得 `24×7` 的企业支持。 |
| **语言版本** | 仅提供英文版本 | 提供多语言版本 |

## 哪个最好，为什么？

**PostgreSQL 在以下情况下是最好的：** 您需要开箱即用的符合标准、事务性和 `ACID`（原子性、一致性、隔离性和持久性）的产品，这些产品也广泛支持 `NoSQL` 特性。

**MongoDB 在以下情况下是最好的：** 您需要实时分析的可伸缩性和缓存，但不是为事务性数据（即会计系统）而构建时。