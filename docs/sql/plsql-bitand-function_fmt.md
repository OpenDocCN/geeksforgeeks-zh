...
> 原文：`https://www.geeksforgeeks.org/plsql-bitand-function/`

`BITAND` 是 PLSQL 中内置的函数，用于返回一个整数值，该整数值通过对两个给定的输入十进制数进行“与”运算来计算。在内部，这些输入的十进制数被转换成二进制数，然后执行“与”运算，结果作为输出返回。
