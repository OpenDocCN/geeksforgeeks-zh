...
该功能用于检查给定的输入 json 字符串是否为 JSON 格式。如果是 JSON 格式，则返回 1 作为输出，否则返回 0。即它以 `INT` 格式返回 1 或 0。
...
*   `$` – 引用整个 JSON 对象
*   `$.example1` – 引用 JSON 对象中的 `example1`
...
该函数用于将 SQL Server 数据导出为 JSON 格式。这是一个将 SQL 数据导出为 JSON 格式的有用函数。`FOR JSON` 有两种选择：

*   **`AUTO`：** 由于是嵌套的，JSON 子数组是基于表层次结构创建的。
...
`SINGLE_BLOB`，它将文件读取为 `varbinary(max)`。`SINGLE_NCLOB`，将文件读取为 `nvarchar(max)` — 如果内容为非英语文本，如日语或中文等数据，我们需要按照这个模式走。我们使用了 `SINGLE_CLOB`，它将文件读取为 `varchar(max)`。
