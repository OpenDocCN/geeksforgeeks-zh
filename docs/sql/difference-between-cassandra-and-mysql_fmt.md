...
> 原文：[https://www.geeksforgeeks.org/difference-between-cassandra-and-mysql/](https://www.geeksforgeeks.org/difference-between-cassandra-and-mysql/)
...
| 5. | 卡珊德拉的读取性能非常高效，因为它需要 `O(1)` 时间。 | `MySQl` 需要使用 `JOIN` 从多个表中读取。因此，读取它需要 `O(log(n))` 的时间。 |
| 6. | 在卡珊德拉中的写入表现也是非常的高效。 | 用 `MySQL` 写入需要先搜索，这会降低写性能。 |
...
| 9. | 卡珊德拉的服务器操作系统是 `BSD`、`Linux`、`OS X`、`Windows`。 | `MySQL` 的服务器操作系统有 `FreeBSD`、`Linux`、`OS X`、`Solaris`、`Windows`。 |
