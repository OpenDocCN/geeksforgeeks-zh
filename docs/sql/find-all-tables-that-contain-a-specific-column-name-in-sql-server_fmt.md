...
> 原文: [https://www.geeksforgeeks.org/find-all-tables-that-contain-a-specific-column-name-in-sql-server/](https://www.geeksforgeeks.org/find-all-tables-that-contain-a-specific-column-name-in-sql-server/)
...
在下面的例子中，我们使用 `INFORMATION_SCHEMA.COLUMNS` 来获取列名类似于 `'%Err%'` 的表名。
...
| 表_目录 | 表_架构 | 表名 | 列名 | 组织位置 | 数据类型 | currentmodelogic 电流型逻辑的 |
| --- | --- | --- | --- | --- | --- | --- |
...
在本例中，我们使用 `sys.columns` 获取列信息，`sys.tables` 获取数据库表名。
...
| 表名 | 统计数据更新传输 |
| 数据库名 | 待定_统计_更新_传输_发布 |
| SQLInstanceName | 待定_统计_更新_传输_发布 |
| 统计时间 | 待定_统计_更新_传输_发布 |
| 表名 | 待定_统计_更新_传输_发布 |