...
在 SQL 中，有时我们需要从表中选择单独的列。为此，我们使用一种特定的查询，如下面的演示所示。在本文中，我们将使用微软的 SQL Server 作为我们的数据库和 `SELECT` 关键字。`SELECT` 是 SQL 中最常用的语句。SQL 中的 `SELECT` 语句用于从数据库中检索或获取数据。我们可以获取整个表，也可以根据特定的规则获取。返回的数据存储在结果表中。该结果表也称为结果集。
...
## 步骤 3: 在数据库 GeeksForGeeks 中创建一个居民表
该表有 3 列，即 `R_NAME`、`AGE` 和 `FLAT`，包含姓名、年龄和社会各种居民的固定数字。
...
## 步骤 7: 选择单个列
要从表中选择一个单独的列，我们需要在关键字 `SELECT` 后面加上要选择的列名，然后是查询的其余部分。例如，在下图中，列 `R_NAME` 是从表 `RESIDENTS` 中选择的单个列。
