...
> 原文：`https://www.geeksforgeeks.org/sql-query-to-compare-two-strings/`

SQL 代表结构化查询语言。它用于与数据库通信。有一些标准的 SQL 命令，如`SELECT`、`DELETE`、`ALTER`等。要在 SQL Server 中比较两个字符串，没有直接的方法。在本文中，我们将学习如何在 MS SQL 服务器中比较两个字符串，并提供一些示例。
