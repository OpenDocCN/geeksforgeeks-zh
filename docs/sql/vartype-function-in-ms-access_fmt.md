...
> 原文: [https://www.geeksforgeeks.org/vartype-function-in-ms-access/](https://www.geeksforgeeks.org/vartype-function-in-ms-access/)
...
| --- | --- | --- |
| `vbEmpty` | 空(未初始化) | Zero |
| `vbNull` | Null(无有效数据) | one |
| `vbInteger` | 整数 | Two |
| `vbLong` | 长整数 | three |
| `vbSingle` | 单精度浮点数 | four |
| `vbDouble` | 双精度浮点数 | five |
| `vbCurrency` | 币值 | six |
| `vbDate` | 日期值 | seven |
| `vbString` | 线 | eight |
| `vbObject` | 目标 | nine |
| `vbError` | 误差值 | Ten |
| `vbBoolean` | 布尔值 | Eleven |
| `vbVariant` | **变体**(仅用于变体阵列) | Twelve |
| `vbDataObject` | 数据访问对象 | Thirteen |
| `vbDecimal` | 十进制值 | Fourteen |
| `vbByte` | 字节值 | Seventeen |
| `vbUserDefinedType` | 包含用户定义类型的变体 | Thirty-six |
| `vbArray` | 排列 | Eight thousand one hundred and ninety-two |
