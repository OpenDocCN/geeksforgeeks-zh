...
## 步骤 3: 在数据库 GeeksForGeeks 中创建一个 BANDS 表
该表有 3 列，即乐队名称、表演成本和成员数量，包含乐队的名称、表演费用和成员数量。
...
## 步骤 7: 更新表 BANDS 中仅满足一个条件的所有记录
这里的条件是 `NUMBER_OF_MEMBERS` 的值应该小于 5。如果条件得到满足，那么列 `PERFORMING_COST` 的值将加倍。使用关键字 `UPDATE` 和 `WHERE` 来实现。
...
**注:** `PERFORMING_COST` 对于拥有 `NUMBER_OF_MEMBERS` <=5 的乐队来说已经翻倍了。

**输出:**

![](img/4e739b1456d994203b8063a3aa82103b.png)

## 步骤 9: 更新满足两个(多个)条件的表 BANDS 的所有记录
这里的条件是如果 `BAND_NAME` 是“METALLICA”，那么它的 `PERFORMING_COST` 设置为 90000，如果 `BAND_NAME` 是“BTS”，那么它的 `PERFORMING_COST` 设置为 200000。使用关键字 `UPDATE` 和 `CASE` 来实现这一点。该查询的行为类似于 if-else if-else 块。
...
**注:** `METALLICA` 和 `BTS` 的 `PERFORMING_COST` 已分别更新为 90000 和 200000。
