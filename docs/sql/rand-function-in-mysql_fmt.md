...
MySQL 中的 `RAND()`函数用于返回范围 `0 <= V < 1.0` 内的随机浮点值 `V`。如果我们想获得一个在 `i <= R < j` 范围内的随机整数 `R`，我们必须使用表达式: `FLOOR(i+RAND()*(j-i))`。
...
## 示例-3 :
使用 `RAND` 函数获取 **[5，10]** 范围内的随机值。这里，我们将使用表达式: `FLOOR(i+RAND()*(j-i))` 来生成随机数。在这里，`i` 为 5，`j` 为 10。
...
## 示例-4 :
使用 `RAND` 函数获取 **[5，10]** 范围内的随机值。这里，我们将使用表达式: `FLOOR(i+RAND()*(j - i + 1))` 来生成随机数。这里 `i` 为 5，`j` 为 10。
...
## 示例-5 :
使用 `RAND` 函数按随机顺序从类别表中返回行。为了演示，创建一个名为 `Student` 的表格。
