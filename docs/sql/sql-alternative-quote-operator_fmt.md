...
> 原文: [https://www.geeksforgeeks.org/sql-alternative-quote-operator/](https://www.geeksforgeeks.org/sql-alternative-quote-operator/)
...
为了**克服**上述问题，Oracle 引入了一个名为**替代报价操作符** (`q`)。
...
如上所述，`q'{` 表示我们的文字值的开始，然后我们使用 `}'` 表示我们的文字值的结束。所以看这里，我们很容易地在我们的文字值中使用了撇号（意味着我们很容易地在 `salary` 中使用 `has salary's`），没有任何错误，这就是为什么我们得到输出，因为 Rajat 有工资 `has salary's 50000`。
...
如上所述，`q'[` 表示我们的文字值的开始，我们使用 `]'` 表示我们的文字值的结束。所以看这里，我们在我们的文字值中很容易地使用了**撇号**（意味着我们在 `work` 中很容易地使用 `work's in`）没有任何错误，这就是为什么我们在执行部门得到的输出是 RR `work's in 'Executive department`。

在上面我们使用 `[` 作为分隔符，所以它**不是**使用分隔符的限制，意味着我们可以使用任何字符作为分隔符。
