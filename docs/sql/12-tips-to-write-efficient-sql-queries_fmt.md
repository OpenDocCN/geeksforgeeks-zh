...
> 原文: [https://www.geeksforgeeks.org/12-tips-to-write-efficient-sql-queries/](https://www.geeksforgeeks.org/12-tips-to-write-efficient-sql-queries/)
...
从上面的方法，你可以避免计算表上的每一项。当您使用 `EXISTS` 时，SQL 服务器会识别它并给出更快的结果。

## 8. 避免使用全局唯一标识符

对于表数据的排序，尽可能避免使用 `GUID`。`GUID` 可以很容易地快速打破你的桌子。您可以使用 `IDENTITY` 或 `DATE` 进行只需几分钟的戏剧性中断。
