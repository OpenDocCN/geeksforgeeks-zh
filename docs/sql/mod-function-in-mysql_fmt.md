...
MySQL 中的 `MOD()` 函数用来求一个数除以另一个数的余数。`MOD()` 函数返回被除数除以除数的余数。如果除数为零，则返回 `NULL`。
...
`MOD` 函数也可用于查找列数据的余数值。在这个例子中，我们将在 `MOD` 函数的帮助下找到一个学生是否出现了总共奇数次考试或者偶数次考试。为了演示，创建一个名为 `Student` 的表格。
