...
> 原文: [https://www.geeksforgeeks.org/insert-on-duplicate-key-update-in-mysql/](https://www.geeksforgeeks.org/insert-on-duplicate-key-update-in-mysql/)

在 MySQL 中，`ON DUPLICATE KEY UPDATE` 语句可以作为 `INSERT` 语句的扩展。每当一个新的行被插入到一个表中，以防该行在 `UNIQUE` 索引或 `PRIMARY KEY` 中引起重复条目，MySQL 将抛出一个错误。
