...
上面的查询将返回所提到的特定列的整个表，并且行将是随机的，并且每次运行查询时都会改变位置。要随机获得一行，我们可以使用 [`LIMIT` 子句](https://www.geeksforgeeks.org/sql-limit-clause/) 并设置为只有一行。查询中的 [`ORDER BY`](https://www.geeksforgeeks.org/sql-order-by/) 子句用于对行进行随机排序。
