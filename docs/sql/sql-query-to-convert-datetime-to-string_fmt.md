...
> 原文: [https://www.geeksforgeeks.org/sql-query-to-convert-datetime-to-string/](https://www.geeksforgeeks.org/sql-query-to-convert-datetime-to-string/)
...
*   `VARCHAR` – 表示字符串类型。
*   `datetime` – 可以是计算日期的表达式，也可以是要转换为字符串的日期时间值。
*   `style` – 指定日期的格式。它的值是由 SQL Server 预定义的。样式参数是可选的。
...
*   `length` – 表示结果数据类型的长度（可选）。

| 无世纪 (YY) | 有世纪 (YYYY) | 标准 | 格式 |
| :--- | :--- | :--- | :--- |
| – | 0 或 100 | 日期时间和 smalldatetime 的默认值 | `mon dd yyyy hh:miAM` (或 `PM`) |
| 1 | 101 | 美国 | `1 = mm/dd/yy` <br> `101 = mm/dd/yyyy` |
| 2 | 102 | ANSI | `2 = yy.mm.dd` <br> `102 = yyyy.mm.dd` |
| 3 | 103 | 英国/法国 | `3 = dd/mm/yy` <br> `103 = dd/mm/yyyy` |
| 4 | 104 | 德国 | `4 = dd.mm.yy` <br> `104 = dd.mm.yyyy` |
| 5 | 105 | 意大利 | `5 = dd-mm-yy` <br> `105 = dd-mm-yyyy` |
| 6 | 106 | – | `6 = dd mon yy` <br> `106 = dd mon yyyy` |
| 7 | 107 | – | `7 = mon dd, yy` <br> `107 = mon dd, yyyy` |
| 8 | 108 | – | `hh:mm:ss` |
