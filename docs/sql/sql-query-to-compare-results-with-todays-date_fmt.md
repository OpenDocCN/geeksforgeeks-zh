...
> 原文: [https://www.geeksforgeeks.org/sql-query-to-compare-results-with-todays-date/](https://www.geeksforgeeks.org/sql-query-to-compare-results-with-todays-date/)
...
此语句包含一个或多个条件及其对应的结果。它类似于 `IF_ELSE` 语句，一旦满足某个条件，它就停止读取并返回相应的结果。
...
为此，我们将返回一个名为 `"AFTER COMPARISON"` 的列，该列在将今天的日期与 `date` 列中的值进行比较后返回一个值。`"AFTER COMPARISON"` 列包含以下字符串:
