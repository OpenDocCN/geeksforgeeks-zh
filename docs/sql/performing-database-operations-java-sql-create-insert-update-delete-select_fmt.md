...
> 原文：[https://www.geeksforgeeks.org/performing-database-operations-java-sql-create-insert-update-delete-select/](https://www.geeksforgeeks.org/performing-database-operations-java-sql-create-insert-update-delete-select/)
...
为了更好的理解，在继续之前，你需要浏览一下[这篇文章](https://www.geeksforgeeks.org/establishing-jdbc-connection-in-java/)。
...
*   `Class.forName()`：这里我们在运行时将驱动程序的类文件加载到内存中。不需要使用 `new` 或创建的对象。
...
*   `Connection`：这个接口代表一个已经建立的数据库连接（session），我们可以从这个接口创建语句来执行查询和检索结果，获取关于数据库的元数据，关闭连接等。

```sql
Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:orcl", "login1", "pwd1");
```

*   `Statement` 和 `PreparedStatement`：这些接口分别用于执行静态 SQL 查询和参数化 SQL 查询。`Statement` 是 `PreparedStatement` 界面的超级界面。他们常用的方法有：
    1.  `boolean execute(String sql)`：执行一般的 `sql` 语句。如果查询返回 `ResultSet`，则返回 `真`，如果查询返回更新计数或不返回任何内容，则返回假。该方法只能与 `Statement` 一起使用。
    2.  `int executeUpdate(String SQL)`：执行 `INSERT`、`UPDATE` 或 `DELETE` 语句，并返回一个更新帐户，指示受影响的行数（例如，插入 1 行，或更新 2 行，或受影响 0 行）。
...
*   `ResultSet executeQuery(String sql)`：执行选择语句并返回一个 `ResultSet` 对象，该对象包含查询返回的结果。
...
*   `ResultSet`：包含 `SELECT` 查询返回的表数据。使用 `next()` 方法，使用此对象迭代结果集中的行。
*   `SQLException`：这个被检查的异常被声明为被上面所有的方法抛出，所以我们在调用上面类的方法的时候必须要显式的捕捉这个异常。

## 连接到数据库

Oracle 数据库服务器监听位于 `localhost` 的默认端口 `1521`。以下代码片段通过用户 `login1` 和密码 `pwd1` 连接到数据库名称 `orcl`。
...
**注：** 此处 `getConnection()` 方法中的数据库 URL 中的 `orcl` 指定 Oracle Database 的 SID。对于 oracle 数据库 11g 是 `orcl`，对于 Oracle 数据库 10g 是 `xe`。
