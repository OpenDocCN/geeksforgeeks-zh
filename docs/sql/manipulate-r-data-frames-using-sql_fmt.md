...
> 原文：[https://www.geeksforgeeks.org/manipulate-r-data-frames-using-sql/](https://www.geeksforgeeks.org/manipulate-r-data-frames-using-sql/)
...
*   [`crashes.csv`](https://drive.google.com/file/d/0B_5j9YRiO1GsUlRsMXgwVDdWSFE/view) 包含年份、道路、`N_Crashes` 和体积。
*   [`roads.csv`](https://drive.google.com/file/d/0B_5j9YRiO1GsWXFZOGVqSnVIYlU/view) 包含道路、区域和长度。
...
现在将数据加载到脚本中。为此，请使用 `setwd()` 函数将当前目录更改为包含 csv 文件 `crashes.csv` 和 `roads.csv` 的目录。
...
**解释：**
`crashes_join_roads` 是一个由 `sqldf` 语句创建的新数据帧，它存储了 join 操作的结果。`sqldf()` 函数或操作至少需要一个字符串和一个 SQL 操作。`stringsAsFactors` 参数用于将字符类分配给分类数据，而不是因子类。
...
在 `sqldf` 包中，聚合操作可以使用 `group by` 子句来执行。
...
`sqldf()` 函数可用于执行某些类型的数据操作。要克服这些限制，请使用 R 脚本中的 `plyr` 包。哈德利·韦翰的 `plyr` 软件包可用于执行高级计算和数据操作。让我们看看它是如何工作的。
