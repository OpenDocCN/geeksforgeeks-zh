...
| S.No | 方法 | 说明 |
| --- | --- | --- |
| 1. | `openDatabase` | 它可以用于创建新的数据库，也可以使用现有的数据库创建数据库对象。 |
| 2. | `transaction()` | 事务可以控制事务，并根据情况执行提交或回滚。 |
| 3. | `executeSql()` | 它用于执行真实的 SQL 查询。 |
...
我们可以从数据库实例中使用名为 `transaction` 的函数。
...
给你。`id` 和 `text` 是外部变量，`executeSql` 将数组参数中的每个项目映射到“？”南
