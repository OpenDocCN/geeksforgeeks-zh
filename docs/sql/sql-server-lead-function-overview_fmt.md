...
> 原文：[https://www.geeksforgeeks.org/sql-server-lead-function-overview/](https://www.geeksforgeeks.org/sql-server-lead-function-overview/)
...
2.  `offset` –
    偏移量是从当前行向前访问数据的行数。`offset` 应该是正整数。如果不定义，`offset` 的默认值是 1。
3.  `default` –
    如果偏移量超出分区范围，则 `LEAD()`函数将导致 `default` 值。如果未定义，`default` 值为 `NULL`。
4.  `PARTITION BY` 子句 –
