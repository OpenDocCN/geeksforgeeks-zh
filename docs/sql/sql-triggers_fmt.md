...
**说明**：我们想创建一个备份表，保存那些不再是机构员工的员工的价值。因此，我们创建了一个名为 `Backup` 的触发器，它将在从表 `employee` 中删除任何 Tuple 之前执行。在删除之前，表 `employee` 的所有属性值都将存储在表 `employee_backup` 中。
...
**说明**：我们想跟踪 `employee` 表中新元组的数量。为此，我们首先创建一个变量 `count`，并将其初始化为 0。之后，我们创建一个名为 `Count_tuples` 的触发器，在表 `employee` 中插入任何新的 Tuple 后，该触发器将增加 `Count` 的值。