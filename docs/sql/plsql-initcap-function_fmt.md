...
PLSQL 中的 `INITCAP` 函数用于将每个单词中的第一个字符设置为大写，其余字符设置为小写。单词由空格或非字母数字字符分隔。PLSQL 中的 `INITCAP` 函数可以接受任何数据类型的 `char` 参数，例如 `char`、`VARCHAR2`、`NCHAR` 或 `NVARCHAR2`。`INITCAP` 函数返回的值与 `char` 的数据类型相同。数据库根据为基础字符集定义的二进制映射设置初始字符的大小写。
