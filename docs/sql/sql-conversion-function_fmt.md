...
| --- | --- |
| `AM` 或 `PM` | 子午线指示器 |
| `A.M.` 或 `P.M.` | 带周期的子午线指示器 |
| `HH` 或 `HH12` 或 `HH24` | 一天中的小时，或小时(1-12)，或小时(0-23) |
| `MI` | 0-59 分钟 |
| `SS` | 第二个 0-59 |
| `SSSSS` | 午夜过后第二天 0-86399 |
...
| `TH` | 序数(例如第 4 位的 `DDTH`) |
| `SP` | 拼出数字(例如 `DDSP` 代表四) |
| `SPTH` 或 `THSP` | 拼出序数(例如第四位的 `DDSPTH`) |
...
这些是一些格式元素，您可以使用和 `TO_CHAR` 函数将数值显示为字符:
...
使用 `TO_DATE` 功能将字符串转换为日期格式:

```sql
TO_DATE(char[, 'format_model'])
```

这些函数有一个 `fx` 修改器。该修饰符指定了 `TO_DATE` 函数的字符参数和日期格式模型的精确匹配。
