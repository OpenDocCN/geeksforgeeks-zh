...
[PL/SQL](https://www.geeksforgeeks.org/plsql-introduction/) 中的字符串实际上是一个可选大小规格的字符序列。字符可以是数字、字母、空格、特殊字符或所有字符的组合。PLSQL 中的 `COMPOSE` 函数用于返回一个 Unicode 字符串。
...
`string` – 用于指定需要创建 Unicode 字符串的输入值。
