...
`SELECT SUM` 用于计算 SQL 中表达式的总值。这与在 SQL 中使用 [`SUM()`](https://www.geeksforgeeks.org/aggregate-functions-in-sql/) 聚合函数是一样的。
...
`col`: 列名
`TYPE`: 数据类型，如整数、可变字符等
`col_constraint`: SQL 中的约束，如 `PRIMARY KEY`、`NOT NULL`、`UNIQUE`、`REFERENCES` 等
