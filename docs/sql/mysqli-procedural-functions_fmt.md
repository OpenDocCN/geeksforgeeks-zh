...
当连接没有成功时，MySQL 函数会抛出一个错误，并且该函数会将该错误存储在之前对 `mysqli_connect()` 的调用中。如果没有遇到错误，它将返回空值。如果遇到任何错误，它将返回一条错误消息。

**注:**

*   要测试 `mysqli_connect_error()`，在 XAMPP 控制面板中停止 MySQL 服务器，然后调用上面的带有 `mysqli_connect()` 的 PHP 代码。

*   如果在 PHP 配置中启用了 `display_errors`，可以看到 `mysqli_connect_error()` 的一个错误，返回如下消息。
...
**注意:**
在良好的编程实践中，最好不要显示任何错误消息。出于故障排除的目的，使用 `mysqli_connect_error()` 记录以下代码中提到的错误。
