...
> 原文: [https://www.geeksforgeeks.org/rnd-and-round-function-in-ms-access/](https://www.geeksforgeeks.org/rnd-and-round-function-in-ms-access/)

## `Rnd()` 函数

函数的作用是：返回随机数。如果我们想生成一个 0 到 1 之间的数字，那么只使用 `Rnd`。否则，如果我们想要生成一个介于一个范围之间的数字，那么我们将使用公式 `Int((upper bound–lower bound+1)* Rnd+lower bound)`。
