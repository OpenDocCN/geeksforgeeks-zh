...
> 原文：`https://www.geeksforgeeks.org/difference-between-simple-and-complex-view-in-sql/`

先决条件–[`SQL |视图`](https://www.geeksforgeeks.org/sql-views/)

`SQL`中的视图是一个或多个表中数据的逻辑子集。视图用于限制数据访问。视图不包含自己的数据，而是包含类似的窗口，通过该窗口可以查看或更改表中的数据。视图所基于的表称为基表。
