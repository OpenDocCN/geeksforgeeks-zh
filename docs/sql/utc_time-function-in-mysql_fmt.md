...
`UTC_TIME()` 函数用于检查当前的协调世界时(UTC)时间值。它以“HH:MM:SS”或“HHMMSS”格式返回世界协调时时间值，具体取决于该函数是在字符串还是数字上下文中使用。
...
在这里，我们将使用 `UTC_DATE` 和 `UTC_TIME` 函数，当交付将完成。“交付时间”列中的值将是 `UTC_DATE` 函数给出的值，而“交付时间”列中的值将是 `UTC_TIME` 函数给出的值。
