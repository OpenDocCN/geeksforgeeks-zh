...
关系客户的 `customer_id` 足以区分一个元组和另一个元组。因此，`customer_id` 是一把超级钥匙。同样的，`customer_id` 和 `customer_name` 的组合对于关系客户来说也是超级关键。这里的`客户_姓名`不是超级钥匙，因为可能几个人同名。
