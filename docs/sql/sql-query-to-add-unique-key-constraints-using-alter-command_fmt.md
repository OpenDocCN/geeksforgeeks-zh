...
> 原文: [https://www.geeksforgeeks.org/sql-query-to-add-unique-key-constraints-using-alter-command/](https://www.geeksforgeeks.org/sql-query-to-add-unique-key-constraints-using-alter-command/)
...
现在让我们给列 `PHONE` 添加一个**唯一键**约束，因为电话号码应该是唯一的。
...
由于字符串 `'000000001'` 已经存在于 `PHONE` 列中，该列现在具有唯一的键约束，因此在执行上述查询时会导致错误。
