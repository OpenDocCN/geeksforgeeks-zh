...
> 原文：[https://www.geeksforgeeks.org/sql-query-to-add-an-agent-parameter-in-a-database/](https://www.geeksforgeeks.org/sql-query-to-add-an-agent-parameter-in-a-database/)

在本文中，我们将逐步详细了解如何在使用微软 SQL Server 中的 `ALTER 命令`创建表后添加额外的或新的参数：

## 步骤 1：创建数据库

使用以下 SQL 查询创建数据库 `sales`：
...
使用以下 SQL 查询创建一个包含 4 列的表 `sales_report`：
...
使用以下 SQL 查询向 `sales_report` 表中插入行：
