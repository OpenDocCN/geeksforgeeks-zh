...
> 原文: [https://www.geeksforgeeks.org/how-to-find-average-marks-of-each-subject-in-sql/](https://www.geeksforgeeks.org/how-to-find-average-marks-of-each-subject-in-sql/)
...
## 步骤 2: 使用数据库
使用 `GeeksForGeeks` 数据库。为此，请使用以下命令。
...
## 步骤 3: 创建表
在数据库 `GeeksForGeeks` 中创建一个 `STDMARKS` 表。该表有 4 栏，即卷号、学生姓名、科目和分数，包含学生的卷号和姓名以及学生在各自科目中的科目名称和分数。
...
## 步骤 7: 计算各科平均分
显示科目以及学生在该科目中从表 `STDMARKS` 中获得的平均分数。为了根据另一列获得一列的聚合值，我们需要使用 `GROUP BY` 子句以及聚合函数，如 `AVG()`。`GROUP BY` 子句后面提到的列是我们输出的基础。以这种方式获得学科平均分数。
