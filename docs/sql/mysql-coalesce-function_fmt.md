...
`COALESCE()` 函数的作用是：返回表达式列表中的第一个非 `NULL` 值。如果列表中的所有值都计算为 `NULL`，那么 `COALESCE()` 函数返回 `NULL`。

`COALESCE()` 函数接受一个参数，该参数是可以包含各种值的列表。MySQL `COALESCE()` 函数返回的值是表达式列表中的第一个非 `NULL` 值，如果列表中的所有值都为 `NULL`，则返回 `NULL`。
...
MySQL `COALESCE()` 函数返回表达式列表中的第一个非 `NULL` 值，如果列表中所有值都为 `NULL`，则返回 `NULL`。
