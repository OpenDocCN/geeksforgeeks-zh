...
> 原文: [https://www.geeksforgeeks.org/introduction-of-parallel-database/](https://www.geeksforgeeks.org/introduction-of-parallel-database/)
...
Where,
time_original = time required to execute the task using 1 processor
time_parallel = time required to execute the task using 'n' processors
```
...
图：一个 `CPU` 执行一个进程需要 3 分钟

图：‘n’ 个 `CPU` 需要 1 分钟来执行一个进程，通过划分成更小的任务
...
Where,
Volume_Parallel = volume executed in a given amount of time using 'n' processor
Volume_Original = volume executed in a given amount of time using 1 processor
```
