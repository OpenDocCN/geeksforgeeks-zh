...
> 原文: [https://www.geeksforgeeks.org/sql-query-to-return-rows-matching-a-certain-condition/](https://www.geeksforgeeks.org/sql-query-to-return-rows-matching-a-certain-condition/)

在 SQL 中，有时我们需要从表中选择匹配某个条件。我们将使用 `SELECT` 命令和 `WHERE` 子句在 SQL 中应用一个条件来实现这一点。为了这篇文章，我们将使用微软的服务器作为我们的数据库。
...
## 步骤 3: 在数据库 GeeksForGeeks 中创建一个 EMPLOYEE 表
该表有 6 列，即 `E_ID`、`E_NAME`、`E_SALARY`、`E_AGE`、`E_GENDER` 和 `E_DEPT`，包含各种员工的 ID、姓名、薪资、年龄、性别和部门。
...
## 步骤 7: 显示 EMPLOYEE 表中 20 岁以上 40 岁以下员工的所有详细信息
使用 `BETWEEN` 子句来比较员工的年龄属性。
...
## 步骤 8: 显示 EMPLOYEE 表中女性员工的所有详细信息
使用 `=`（等于）关系运算符，将员工的性别属性与 `'FEMALE'` 进行比较。
...
## 步骤 9: 从 EMPLOYEE 表中显示薪资大于 40000 且属于土建部门的员工的所有详细信息
我们将使用 `>`（大于）关系运算符来比较工资和 `=`（等于）关系运算符来检查部门。我们还将使用 `AND`（逻辑运算符），以便两个条件同时成立。
