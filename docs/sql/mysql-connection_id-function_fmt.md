...
MySQL `CONNECTION_ID()`函数用于返回 MySQL 中当前连接的连接标识。用于建立数据库连接的连接标识对于连接的客户端之间的每个连接都是唯一的。`CONNECTION_ID()`函数不需要任何参数或自变量。
...
## 使用的参数:
`CONNECTION_ID()`函数不需要任何参数或自变量。

## 返回值:
MySQL `CONNECTION_ID()`函数返回一个整数值，代表当前唯一的连接标识。
