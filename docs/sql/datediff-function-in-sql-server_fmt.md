...
> 原文: [https://www.geeksforgeeks.org/datediff-function-in-sql-server/](https://www.geeksforgeeks.org/datediff-function-in-sql-server/)
...
*   `interval`: 是要退回的指定部分。此外，区间的值可以如下给出:
    1.  `year`, `yyyy`, `yy` = Year，这是指定的年份。
    2.  `quarter`, `qq`, `q` = Quarter，这是指定的季度。
    3.  `month`, `mm`, `m` = 月，即指定的月份。
    4.  `dayofyear`, `dy`, `y` = 一年中的某一天，这是一年中指定的一天。
    5.  `day`, `dd`, `d` = Day，这是指定的日期。
    6.  `week`, `ww`, `wk` = Week，这是指定的一周。
    7.  `weekday`, `dw`, `w` = Weekday，这是指定的星期几。
    8.  `hour`, `hh` = 小时，这是指定的小时。
    9.  `minute`, `mi`, `n` = 分钟，这是指定的分钟。
    10. `second`, `ss`, `s` = Second，这是指定的秒。
    11. `millisecond`, `ms` = 毫秒，这是指定的毫秒。
*   `date1`, `date2`: 两个指定的日期，以便找出它们之间的区别。
