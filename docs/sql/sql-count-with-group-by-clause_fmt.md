...
> 原文: [https://www.geeksforgeeks.org/sql-count-with-group-by-clause/](https://www.geeksforgeeks.org/sql-count-with-group-by-clause/)

`count()`函数是一个聚合函数，用于查找满足固定条件的行数。带有`GROUP BY`子句的`count()`函数用于对根据表的特定属性分组的数据进行计数。
...
向表中插入行的查询。使用以下 SQL 查询将行插入 `student_marks` 表。
