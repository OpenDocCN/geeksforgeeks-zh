...
[MySQL](https://www.geeksforgeeks.org/mysql-common-mysql-queries/) 中的 `CURTIME()` 函数用于查看当前时间。它以 `'hh:mm:ss'` 或 `'hhmmss'` 格式的值返回当前时间，具体取决于该函数是在字符串还是数字上下文中使用。
...
**`fsp`** – 它指定返回的分数秒精度。如果用户省略了精度参数，结果将排除精度。
...
在这里，我们将使用 `CURDATE` 和 `CURTIME` 功能，当一个交付将完成。“交付时间”列中的值将是 `CURTIME` 给出的值，而“交付时间”列中的值将是 `CURDATE` 函数给出的值。
