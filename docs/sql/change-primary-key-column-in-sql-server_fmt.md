...
> 原文: [https://www.geeksforgeeks.org/change-primary-key-column-in-sql-server/](https://www.geeksforgeeks.org/change-primary-key-column-in-sql-server/)

主键是指表中唯一标识表中行的列。它包含唯一值，不能包含空值。为了演示的目的，我们将在名为`geeks`的数据库中为极客创建`geeks`表。
...
现在使用`SELECT * FROM INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE`命令来检查表的主键。
