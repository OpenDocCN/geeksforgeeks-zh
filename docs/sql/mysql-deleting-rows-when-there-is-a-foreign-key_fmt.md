...
> 原文：[https://www.geeksforgeeks.org/mysql-deleting-rows-when-there-is-a-foreign-key/](https://www.geeksforgeeks.org/mysql-deleting-rows-when-there-is-a-foreign-key/)
...
## 在MySQL中有外键时删除行的步骤
在这里，我们将借助示例来讨论在 MySQL 中实现有外键时删除行所需的步骤，以便更好地理解。

### 步骤-1：创建数据库
使用如下 SQL 查询创建数据库`student`。
...
![](img/3e1c1eb0e00e3e5fe580.png)

**注意：**
如果未使用`ON DELETE CASCADE`约束，则会出现引用错误。

### 步骤-7：有外键时删除行
查询删除`student_exam`表中`student_id`为`19102048`的学生，其中引用的表是`student_details`。
...
**输出：删除后**
两个表中都删除了`student_id`为`19102048`的行。
