...
> 原文: [https://www.geeksforgeeks.org/foreign-key-constraint-in-sql/](https://www.geeksforgeeks.org/foreign-key-constraint-in-sql/)

[外键](https://practice.geeksforgeeks.org/problems/what-is-a-foreign-key)是指其他表的主键/唯一键的列。因此，它展示了表之间的关系，并充当它们之间的交叉引用。定义了外键的表称为外键表/引用表。定义主键/唯一键并被外键引用的表称为主键表/主表/被引用表。它在`CREATE TABLE`/`ALTER TABLE`语句中定义。
...
此处`people`表应该具有类型为`INT`的主键。如果表中只有一个列主键，语法中的列名可以省略。所以以上两种语法都是正确的。
...
现在从`person`中删除记录将从子表`people`中删除所有相应的记录。
...
请注意`people`表中引用`person`表主键的字段`no`。删除`person`数据时，会在子表`people`中设置为`NULL`。但是记录不会被删除。