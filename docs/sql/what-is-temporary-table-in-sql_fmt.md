...
> 原文：[https://www.geeksforgeeks.org/what-is-temporary-table-in-sql/](https://www.geeksforgeeks.org/what-is-temporary-table-in-sql/)
...
此外，用户可以通过使用`DROP TABLE #EmpDetails`查询来删除这个临时表。将会有随机数被追加到表名的名称中。如果临时表是在存储过程中创建的，它会在存储过程执行完成时自动删除。
