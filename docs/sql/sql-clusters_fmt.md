...
`SQL` 中的[集群](https://www.geeksforgeeks.org/sql-queries-on-clustered-and-non-clustered-indexes/)用于将来自不同表的数据存储在相同的物理数据块中。如果经常一起查询这些表中的记录，则使用它们。通过存储相同的数据块，完全填充此类查询所需的数据库块读取次数减少，从而提高了性能。
