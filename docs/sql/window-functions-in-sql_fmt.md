...
*   `RANK() –`
    顾名思义，排名函数为每个分区内的所有行分配排名。分配的排名使得第一行获得排名 1，具有相同值的行被分配相同的排名。对于两个相同排名值之后的下一个排名，将跳过一个排名值。
*   `DENSE_RANK() –`
    它为分区内的每一行分配排名。就像排名函数一样，第一行被分配排名 1，具有相同值的行具有相同的排名。`RANK()` 和 `DENSE_RANK()` 之间的区别在于，在 `DENSE_RANK()` 中，两个相同排名之后的下一个排名使用连续整数，没有排名被跳过。
*   `ROW_NUMBER() –`
    它为分区内的所有行分配连续整数。在分区内，没有两行可以具有相同的行号。
