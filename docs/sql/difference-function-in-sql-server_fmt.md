...
> 原文：[https://www.geeksforgeeks.org/difference-function-in-sql-server/](https://www.geeksforgeeks.org/difference-function-in-sql-server/)
...
*   `string`：它是字符数据的字母数字表达。它可以是常量、变量或列。

## 返回

返回一个整数值，测量两个不同字符串的 `SOUNDEX()` 值之差。
