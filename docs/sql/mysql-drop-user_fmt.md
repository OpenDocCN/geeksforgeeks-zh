...
1.  **用户**: 是您要删除的用户帐户的用户名。
2.  **主机**: 是用户账户的主机服务器名称。用户名应采用以下格式 **“user_name”@“host_name”**。
...
### 使用 DROP USER 语句删除单个用户

要删除用户名为 `gfguser1` 的用户帐户，应执行如下 `DROP USER` 语句：

**语法:**
![](img/27ac501471d23fc12bfd1df8b573d9aa.png)

**输出:**
执行后的 `DROP USER` 语句后的表会如下:
![](img/948524e7f9c4c35477ba42a0edbfefaf.png)

### 使用 DROP USER 语句删除多个用户

`DROP USER` 语句可用于一次删除多个用户帐户。要从上面提到的表中删除两个用户帐户 `gfguser2` 和 `gfguser1`，应执行如下 `DROP USER` 语句：

**语法:**
![](img/a3a9111901e5f991d63f5127dc61b4e5.png)

**输出:**
执行完上面的 `DROP USER` 语句后的表会如下:
![](img/e3149f5bcf5a90652de8e44de5f6d0f2.png)

## 注意

如果在用户帐户的会话处于活动状态时，对该帐户执行 `DROP USER` 语句，则在该帐户的会话关闭之前，该语句不会影响该帐户。用户帐户将在其会话关闭后被删除，并且将无法再登录。
