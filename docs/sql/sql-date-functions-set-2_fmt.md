...
11. `STR_TO_DATE()`: 取一个字符串，返回格式掩码指定的日期。
...
13. `SUBTIME()`: 减去一定时间间隔后返回时间/日期时间值。

    ```sql
    SELECT SUBTIME("2018/07/18", INTERVAL 10 DAY);
    ```
...
16. `TIME_FORMAT()`: 它按照格式掩码指定的格式设置时间。
