...
> 原文: [https://www.geeksforgeeks.org/how-to-remove-the-last-character-from-a-table-in-sql/](https://www.geeksforgeeks.org/how-to-remove-the-last-character-from-a-table-in-sql/)

在这里，我们将看到如何删除 SQL 表中的最后一个字符。我们可以使用 `SUBSTRING()` 函数来完成这个任务。

## SUBSTRING() 函数说明

`SUBSTRING()` 函数用来从给定的位置找到给定字符串的一部分。它需要三个参数：
