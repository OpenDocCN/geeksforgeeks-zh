...
*   异常部分以 `EXCEPTIONS` 关键字开始。此部分是可选的，它包含在运行时出错时执行的语句。任何异常都可以在本节中处理。
...
*   `var1 INTEGER` : 是变量的声明，命名为 `var1` ，为整数类型。还有许多其他数据类型可以使用，如浮点、int、real、smallint、long 等。它还支持在 SQL 中使用的变量，如 `NUMBER(prec，scale)`，`varchar`，`varchar2` 等。
*   `PL/SQL procedure successfully completed.` : 代码编译执行成功时显示。
*   `END` 后斜线(`/`)； :斜线(`/`)告诉 SQL*Plus 执行该块。
...
2.  **显示输出** :
    输出通过使用 `DBMS_OUTPUT` 来显示，`DBMS_OUTPUT` 是一个内置的包，使用户能够显示输出、调试信息以及发送来自 PL/SQL 块、子程序、包和触发器的消息。
