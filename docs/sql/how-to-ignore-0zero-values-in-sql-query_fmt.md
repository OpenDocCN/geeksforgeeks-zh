...
> 原文: [https://www.geeksforgeeks.org/how-to-ignore-0zero-values-in-sql-query/](https://www.geeksforgeeks.org/how-to-ignore-0zero-values-in-sql-query/)
...
## 第 3 步: 在数据库 GeeksForGeeks 中创建一个 MARKS 表
该表有 4 栏，即 `ID`、`ENG`、`MATHS` 和 `PHYS`，包含学生身份证和各科成绩。
...
## 步骤 6: 显示 MARKS 表的所有行，忽略 0(零)值
这是通过 [`REPLACE`](https://www.geeksforgeeks.org/replace-string-in-sql-server/) 命令实现的，该命令将所有 0 替换为空白。
