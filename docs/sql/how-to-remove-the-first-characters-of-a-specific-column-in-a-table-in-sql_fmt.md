...
> 原文：[https://www.geeksforgeeks.org/how-to-remove-the-first-characters-of-a-specific-column-in-a-table-in-sql/](https://www.geeksforgeeks.org/how-to-remove-the-first-characters-of-a-specific-column-in-a-table-in-sql/)
...
这里，`字符串`表示字段，`2`表示字符串的起始位置，即第二个字符，`长度(字符串)`表示字符串的长度。
...
为了演示的目的，我们将在名为“geeks”的数据库中为极客创建 `geeksforgeeks` 表。
...
现在从字段中删除第一个字符，我们将使用 `geeksforgeeks` 表中的数据。下面是 `SUBSTRING()` 函数从字段中删除第一个字符的语法。
...
从 `geeksforgeeks` 表的 `FIRSTNAME` 列中删除第一个字符。我们使用下面给出的查询：
