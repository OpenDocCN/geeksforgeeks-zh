...
> 原文: [https://www.geeksforgeeks.org/how-to-find-first-value-from-any-table-in-sql-server/](https://www.geeksforgeeks.org/how-to-find-first-value-from-any-table-in-sql-server/)

我们可以在 [SQL Server](https://www.geeksforgeeks.org/introduction-of-ms-sql-server/) 中使用 `FIRST_VALUE()` 从任何表中查找第一个值。`FIRST_VALUE()` 函数是一种窗口函数，它在给定数据集的有序分区中产生第一个值。
...
*   `scalar_value` – 标量值是对所提供数据集的有序分区中第一行的值进行检查的值。
*   `PARTITION BY` – `PARTITION BY` 是可选的，它将提供的数据集的行与使用 `FIRST_VALUE()` 函数的分区不同。
*   `ORDER BY` – `ORDER BY` 表示使用 `FIRST_VALUE()` 函数的每个分区中的行的顺序。
...
### 示例-1 : 在没有 `PARTITION BY` 子句的情况下查找第一个值

要查找表 `geek_demo` 的第一个城市，请使用以下查询:
...
### 示例-2 : 使用 `PARTITION BY` 子句查找第一个值

要根据年份为表 `geek_demo` 找到第一个城市，请使用以下查询:
