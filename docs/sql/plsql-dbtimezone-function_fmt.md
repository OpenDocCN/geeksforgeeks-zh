...
`DBTIMEZONE` 函数用于返回数据库时区值。`DBTIMEZONE` 函数不需要传递任何参数。`DBTIMEZONE` 函数返回一个时区偏移量，它遵循的格式是“[+|-]TZH:TZM”，例如`-05:00`。它还可能返回时区区域名称，例如，某些设备上的`US/Michigan`。

`DBTIMEZONE` 函数返回的值取决于它在最近的 `CREATE DATABASE` 或 `ALTER DATABASE` 语句中的指定方式。
