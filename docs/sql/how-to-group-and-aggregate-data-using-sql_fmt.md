...
> 原文: [https://www.geeksforgeeks.org/how-to-group-and-aggregate-data-using-sql/](https://www.geeksforgeeks.org/how-to-group-and-aggregate-data-using-sql/)
...
我们将在 `Sales` 数据库中使用下面的 `Sales_Order` 表。
...
假设，我们想找出订单为 `In Process` 的客户的客户号。
...
因此，从这个输出中，我们可以很容易地看出 `C00001` 和 `C00006` 有 `1` 订单仍然 `In Process`。
...
您可以尝试使用 `AVG()`函数、`MIN()`函数、`MAX()`函数和 `GROUP BY` 函数来进行不同类型的查询。