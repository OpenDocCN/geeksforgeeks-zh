...
使用 `DEGREES` 函数求 3.141592653589793 弧度的度数值。
...
使用 `DEGREES` 函数求 -1.5707963267948966 弧度的度数值。
...
使用 `RADIANS` 函数将列数据的弧度转换为度数。为了演示，让我们创建一个名为 `Polygon` 的表。
...
我们可以看到内角和多边形的每个角都是以弧度给出的。现在我们将在 `DEGREES` 函数的帮助下把这些转换成度数。
