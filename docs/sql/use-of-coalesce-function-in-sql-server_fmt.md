# 在 SQL Server 中使用 `COALESCE()` 函数

> 原文: [https://www.geeksforgeeks.org/use-of-coalesce-function-in-sql-server/](https://www.geeksforgeeks.org/use-of-coalesce-function-in-sql-server/)

## 简介:
SQL Server `COALESCE()` 函数对于处理 `NULL` 值非常有用。在表达式值计算过程中，空值被用户给定的值替换。SQL Server 联合函数以确定的顺序计算表达式，并且总是首先从定义的表达式列表中得出非空值。
...
## 使用名为 `COALESCE()` 的 SQL server 函数来处理空值:
