...
> 原文：[https://www.geeksforgeeks.org/sequence-with-examples-in-sql-server/](https://www.geeksforgeeks.org/sequence-with-examples-in-sql-server/)
...
*   `序列名称` – 为序列定义一个在数据库中唯一的名称。
*   `AS integer_type` – 使用任意整数类型的序列为例；`TINYINT`、`INT` 或 `DECIMAL`。默认情况下，序列使用 `BIGINT`。
*   `START WITH start_value` – 定义序列的第一个值。
*   `INCREMENT BY increment_value` – 定义序列对象的 `increment_value`。调用 `NEXT VALUE FOR` 函数，`INCREMENT_VALUE` 不能为零。
*   `MINVALUE min_value` – 定义序列的下限值。
*   `MAXVALUE max_value` – 定义序列的上限值。
