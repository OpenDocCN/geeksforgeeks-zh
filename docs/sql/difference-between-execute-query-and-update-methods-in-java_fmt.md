...
> 原文: [https://www.geeksforgeeks.org/difference-between-execute-query-and-update-methods-in-java/](https://www.geeksforgeeks.org/difference-between-execute-query-and-update-methods-in-java/)

在开始之前，让我们先了解一下使用以下三个查询参数的参数，如下所示：

*   **`boolean execute(String SQL)`**：如果可以检索结果集对象，则返回布尔值 `true`；否则，它返回 `false`。使用此方法执行 SQL DDL 语句或当您需要使用真正动态的 SQL 时。
*   **`int executeUpdate(String SQL)`**：返回受 SQL 语句执行影响的行数。使用此方法来执行 SQL 语句，对于这些语句，您预计会有许多行受到影响，例如，`INSERT`、`UPDATE` 或 `DELETE` 语句。
*   **`ResultSet executeQuery(String SQL)`**：返回结果集对象。当您期望得到一个结果集时，使用这个方法，就像使用 `SELECT` 语句一样。
...
### 描述
用于所有类型的 SQL 语句的方法，即返回真或假的布尔值。

### 返回类型
这个方法返回一个布尔值。`true` 表示查询返回结果集对象，`false` 表示返回整数值或不返回任何内容。
...
### 描述
现在这个方法执行语句，通过从数据库中获取一些数据来返回一个结果集。

### 用法
此方法用于执行选择查询。

### 返回类型
该方法返回一个结果集对象，该对象包含查询返回的结果。
...
### 说明
此方法用于执行 DML 语句（`INSERT`、`UPDATE` 和 `DELETE`），返回 `int` 值，受影响行的计数。

### 用法
此方法用于执行非选择查询。该方法用于执行选择和非选择查询。

### 返回类型
表示受查询影响的行数的整数值。对于不返回任何内容的语句，这将是 `0`。
...
> 2.  **`executeQuery()`**：方法返回一个 `ResultSet`，`execute` 返回查询的结果，常用于执行查询。
