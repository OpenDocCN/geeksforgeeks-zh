...
*   它使得拼接字符串变得非常容易。它与拼接类似，但使用了分组。
*   该函数的特点是它还允许对拼接列表中的元素进行排序。
...
使用 `LISTAGG` 函数编写一个 SQL 查询，以逗号分隔的值在单个字段中输出主题名称。
...
借助 `LISTAGG` 函数，编写一个 SQL 查询，对每个主题进行分组，并显示每个主题在各自部门中的位置，用逗号分隔。
...
借助 `LISTAGG` 函数，编写一个 SQL 查询，显示按主题号(`SUBNO`)排序的属于各个部门的主题。
