...
> 原文：[https://www.geeksforgeeks.org/sql-query-to-show-top-5-selling-products/](https://www.geeksforgeeks.org/sql-query-to-show-top-5-selling-products/)
...
## 步骤 3：创建表
现在我们创建一个表。使用以下 SQL 查询创建一个包含 4 列的 `sales_details` 表。
...
## 步骤 5：插入数据
向表中插入行的查询。使用以下 SQL 查询将行插入 `sales_details` 表。
