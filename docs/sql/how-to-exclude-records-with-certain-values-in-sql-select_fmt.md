...
> 原文：[https://www.geeksforgeeks.org/how-to-exclude-records-with-certain-values-in-sql-select/](https://www.geeksforgeeks.org/how-to-exclude-records-with-certain-values-in-sql-select/)
...
我们还可以通过提供用`AND`或`OR`运算符分隔的条件来排除更多的记录。
...
现在看看`NOT`是如何工作的。在这里的例子中，我们提供了条件，当条件为真时，跟在`NOT`后面意味着查询将选择提供的条件不为真的所有行。

![](img/7a845c52834a583e00bbdf9086f138a9.png)

在上面的输出中，满足`GeekID > 104`的条件，并且由于`NOT`，所有小于 104 的行都被选择。
...
在这个查询中，我们排除了那些`GeekID`不在提供的列表中的记录（行）（即`GeekID`不应该是 104，101，102，107）。

因此，结果数据将包含不包括所提供的`GeekID`的记录。

![](img/a02db0ab29ed12afab7da7ad09d6e13e.png)

因此，我们可以对表的任何列应用任何条件，并排除那些使用`NOT`运算符的条件。
