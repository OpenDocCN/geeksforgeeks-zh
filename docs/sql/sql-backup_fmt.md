...
### 1. 使用微软 SQL Server 中的 SQL Server 管理工作室
...
在这里，`BACKUP DATABASE DatabaseName` 是指令。“到磁盘”选项指定备份应该写入驱动器，以及备份位置和文件名。
