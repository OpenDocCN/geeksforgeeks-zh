...
该函数将为分区内的每一行分配等级，没有间隙。基本上，等级是以连续的方式分配的，即如果值之间有联系，那么它们将被分配相同的等级，并且下一个等级值将比先前分配的等级大一个等级。

## rank()

该函数将为一个分区内的每一行分配等级，该分区带有间隙。这里，等级是以非连续的方式分配的，即如果值之间有联系，那么它们将被分配相同的等级，并且下一个等级值将是前一等级 + 对等体的数量（重复）。

## percent_rank()

它返回分区内一行的百分位数，范围从 0 到 1。它显示小于当前行中值的分区值的百分比，不包括最高值。

为了更好地理解这些功能，让我们考虑一张表 `result`：
...
这里，表是根据 `subjects` 来划分的。`order by` 子句用于按 `mark` 降序排列每个分区的行。`dense_rank()` 用于对各科学生进行排名。

**注意：** 对于科学学科来说，安基塔和普拉蒂巴之间是有联系的，所以他们都被分配了相同的等级。下一个等级值增加 1，即斯瓦纳增加到 2。
...
它的输出类似于 `dense_rank()` 函数。除此之外，在安基塔和普拉蒂巴打成平手的情况下，科学科目的下一个等级值增加 2，也就是斯沃那的 3。

### 查询 3：`percent_rank()` 函数
...
这里，`percent_rank()` 函数通过 `mark` 列按升序计算百分位数排名。
...
`rank` 是使用 `rank()` 函数得到的分区的每行的秩。`rows` 代表该分区的行数。
...
| 学科 | 名字 | 标记 | 排名-1 | total_rows-1 | 百分比等级 |
| --- | --- | --- | --- | --- | --- |
