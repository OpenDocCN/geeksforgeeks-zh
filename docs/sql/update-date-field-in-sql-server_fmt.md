...
> 原文: [https://www.geeksforgeeks.org/update-date-field-in-sql-server/](https://www.geeksforgeeks.org/update-date-field-in-sql-server/)
...
在下面的查询中，`SET` 语句用于为特定列设置新值，而 `WHERE` 子句用于选择需要更新列的行。如果我们没有使用 `WHERE` 子句，那么表中的列和所有行都将被更新。所以 `WHERE` 子句用于选择特定的行。
