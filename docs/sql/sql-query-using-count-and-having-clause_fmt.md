...
> 原文: [https://www.geeksforgeeks.org/sql-query-using-count-and-having-clause/](https://www.geeksforgeeks.org/sql-query-using-count-and-having-clause/)
...
之后我们使用 `HAVING MARKS > (SELECT AVG(MARKS) FROM STUDENT WHERE ADDRESS = 'NOIDA')`，它用于过滤结果，条件是分数必须大于 NOIDA 市学生的平均分数，即大于
```sql
(10+20+40) / 3 
= 23.3 
```
