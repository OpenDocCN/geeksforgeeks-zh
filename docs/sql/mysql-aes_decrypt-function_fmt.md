...
该函数的作用是解密一个加密的字符串，然后返回原始的字符串。它使用 AES（高级加密标准）算法执行解密。如果检测到无效数据，则返回解密后的字符串或空值。

`AES_DECRYPT` 函数返回的值是使用 `AES_ENCRYPT` 函数加密的原始明文字符串。该函数接受两个参数，即加密字符串和用于解密加密字符串的密钥字符串。
...
*   `encrypted_string`：用于指定加密字符串。
*   `key_string`：用于指定用于解密加密字符串的密钥字符串。
