...
> 原文: [https://www.geeksforgeeks.org/how-to-use-sql-query-to-rename-a-constraint/](https://www.geeksforgeeks.org/how-to-use-sql-query-to-rename-a-constraint/)
...
## 步骤 3: 创建 INFO 表
在数据库 `GeeksForGeeks` 中创建一个 `INFO` 表。该表有 3 列，即 `S_NAME`、`ROLL` 和 `BRANCH`，包含在工程学院学习的各种学生的姓名、ROLL 编号和 BRANCH。
...
## 步骤 4: 添加自定义检查约束
在 `INFO` 表中添加一个名为 `BRANCH_CHECK` 的自定义检查约束。该约束检查数据库中输入的学生分支是否在工程学院教授的 5 个分支中，即计算机科学与工程、电子与通信工程、土木工程、机械工程和电气工程。
...
## 步骤 6: 插入有效数据
在 `INFO` 表中插入 5 行，使分支在 `BRANCH_CHECK` 约束中定义的 5 个分支内。因此在添加这些行时不会引发错误。
...
## 步骤 7: 插入违反约束的数据
在 `INFO` 表中插入行，使分支在 `BRANCH_CHECK` 约束中定义的 5 个分支中不是。这将引发一个错误，因为它违反了约束。
...
## 步骤 10: 重命名约束
将自定义检查约束从 `BRANCH_CHECK` 重命名为 `CHECK_BRANCH`。
...
## 步骤 11: 验证重命名结果
显示为 `INFO` 表定义的所有约束。新名称 `CHECK_BRANCH` 现在必须可见，而不是旧名称 `BRANCH_CHECK`。
