...
假设我们要将学生数据存储在表 `Student_grade` 中，以便将学生的成绩存储在相应的列(高、中、低)中。我们使用优先级语句为 `ENUM` 列分配优先级。
...
*   除了使用枚举值，您还可以使用数字索引，以便将值插入表的 `ENUM` 列：
...
*   将其移植到其他关系数据库管理系统可能是一项艰巨的任务，因为 `ENUM` 不是 `SQL` 的标准数据类型，也没有多少数据库系统为其提供支持。
*   不可能向枚举列插入更多的值。假设，您有兴趣为每个优先级插入一个基于服务的协议，高(48 小时)、中(4-3 天)、低(1 周)，但它并不像 `ENUM` 数据类型看起来那么简单和实用。
