...
0xval  
```
`val` 代表任何十六进制数，即范围 (`0`, `1`, …, `d`, `E`, `F`) 内的任何数字。在 `0xval` 中，标记 `0x` 区分大小写，因此不能写成 `0Xval`。但是，以 `X` 或 `0x` 开头的数字的字母大小写不区分大小写。
...
### 1. COLLATE operation
十六进制文字的默认字符集可以通过使用可选的字符集引入符和 `COLLATE` 子句来更改，以将其转换为特定字符集和排序规则的字符串。
...
mysql> select _binary X'01' | X'02', hex(_binary X'01' | X'02')
      -> ♥ 03 
```
对于十六进制文字，位操作会产生一个 `BIGINT` 值。与上面的代码一样，在不使用 `_binary` 导入器的情况下，不显示包含零的最高有效位。因此，通过使用 `_binary` introducer，我们可以在二进制上下文中显式地指定它们。
...
### 4. HEX() / UNHEX() fun
我们可以使用内置函数 `HEX()` 将字符串或数字转换为十六进制字符串。
```sql
mysql> select hex('e')
      -> 65
mysql> select hex('table')
      -> 5461626C65
```
同样的，`UNHEX()` 函数用于将每对十六进制数字转换为对应的字符。
```sql
