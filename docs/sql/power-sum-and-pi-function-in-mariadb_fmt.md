...
> 原文: [https://www.geeksforgeeks.org/power-sum-and-pi-function-in-mariadb/](https://www.geeksforgeeks.org/power-sum-and-pi-function-in-mariadb/)
...
## `PI()` 函数
在马里亚数据库中，用于返回 π (pi) 值的 `PI()` 函数最多显示 6 位小数。在这个函数中，不会传递任何参数，它将返回 `pi` 的值。当将双精度值用于其他双精度值的计算时，它会使用双精度值。它最多只显示 6 位小数。
