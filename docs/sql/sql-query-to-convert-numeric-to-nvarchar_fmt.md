...
> 原文: [https://www.geeksforgeeks.org/sql-query-to-convert-numeric-to-nvarchar/](https://www.geeksforgeeks.org/sql-query-to-convert-numeric-to-nvarchar/)

这里我们将看到，如何使用 `CAST()`，`CONVERT()` 和 `FORMAT()` 函数将 MS SQL Server 数据库表中的 `NUMERIC` 数据转换为 `NVARCHAR` 数据。

我们将在一个名为“极客”的数据库中创建一个 `person` 表。
