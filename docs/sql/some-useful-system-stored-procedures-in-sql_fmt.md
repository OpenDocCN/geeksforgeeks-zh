...
> 原文: [https://www.geeksforgeeks.org/some-useful-system-stored-procedures-in-sql/](https://www.geeksforgeeks.org/some-useful-system-stored-procedures-in-sql/)
...
*   `sp_help`：这将显示存储过程名称、模式名称、创建日期和时间，或者如果有任何参数，则显示参数名称、数据类型、长度、精度、规模、排序规则等结果。
*   `sp_helptext`：这将显示结果存储过程的内容。
*   `sp_depends`：这将显示过程依赖的对象，如表名、函数名等。
