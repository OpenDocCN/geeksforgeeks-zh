...
> 原文: [https://www.geeksforgeeks.org/sql-query-to-display-first-50-percent-records-from-employee-table/](https://www.geeksforgeeks.org/sql-query-to-display-first-50-percent-records-from-employee-table/)

在这里，我们将看到如何在 `MS SQL server` 的数据库中显示 `Employee` 表中的前 50%的记录。

为了演示的目的，我们将在名为 `geeks` 的数据库中创建一个 `Employee` 表。
...
> **语法:**
> `SELECT TOP N PERCENT * FROM <table_name>;` /*给出数据库表中前 N %的记录*/
