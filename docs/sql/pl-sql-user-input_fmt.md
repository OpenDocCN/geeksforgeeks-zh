...
### 示例-1
考虑一下，表 `GFG`。让我们选择一个具有给定 `id` 的记录。(这里，`id` 是一个数值)
...
### 示例-1
考虑一下，表 `GFG`。让我们选择一个给定 `author` 的记录。(这里，`author` 是文本值)
...
### 示例-2
让我们用给定的 `author` 更新一条记录。(这里，`author` 是文本值)
