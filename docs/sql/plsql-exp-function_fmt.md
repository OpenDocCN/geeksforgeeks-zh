...
`EXP` 是 PLSQL 中内置的函数，用于返回一个被 `e` 提升到 `n` 次方的值。这里的“e”是一个数学常数，其值为 2.7182818，`n` 是输入数。
...
## 优势：
这个函数用来找出一个 `e` 升到第 `n` 次方的值。这里的“e”是一个数学常数，其值为 2.7182818，`n` 是输入数。