...
> 原文: [https://www.geeksforgeeks.org/sql-interview-questions-set-2/](https://www.geeksforgeeks.org/sql-interview-questions-set-2/)
...
## 3. 查找员工姓名，其中名字、中间名和姓氏在表中给出，但某些名字缺失

我们将使用 `COALESCE()` 函数，它将返回第一个非空值。
...
查找在过去 n 个月内雇佣的员工。这里我们使用 `TIMESTAMPDIFF()` MySQL 函数来获得所需的输出。
...
查找在过去 n 天内雇佣的员工。这里我们使用 `DATEDIFF()` MySQL 函数来获得所需的输出。
...
查找在过去 n 年内雇佣的员工。这里我们使用 `TIMESTAMPDIFF()` MySQL 函数来获得所需的输出。
