...
> 原文: [https://www.geeksforgeeks.org/sql-query-to-convert-month-number-to-month-name/](https://www.geeksforgeeks.org/sql-query-to-convert-month-number-to-month-name/)

SQL 代表`结构化查询语言`，在数据库中用来检索数据，更新和修改 MySql、Oracle 等关系数据库中的数据。查询是对数据库数据的一个问题或请求，也就是说，如果我们问某人任何问题，那么这个问题就是查询。类似地，当我们需要数据库中的任何数据时，我们用 SQL 编写查询来获取这些数据。在本文中，我们将讨论如何使用 SQL 将月号转换为月名。
...
要查看该表使用`desc table_name`查询，这里的查询是，
