...
PHP 代表超文本预处理器，这是一种服务器端脚本语言，也用于处理数据库操作。我们使用一个 PHP xampp 服务器来与数据库通信。使用的语言是 MySQL。MySQL 是一种用于管理数据库的数据库查询语言。它与 PHP 通信，并通过执行一些操作来管理数据库。
...
`(SELECT COLUMN_NAME ...n from TABLE_NAME WHERE ...)` 是内部/子查询。
