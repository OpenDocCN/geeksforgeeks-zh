...
现在，您需要从市场上启用`云 SQL 管理 API`。你可以点击这个链接[这里](https://console.cloud.google.com/marketplace/details/google/sqladmin.googleapis.com?q=Cloud%20SQL%20Admin%20API)来完成。
...
*   `PUBLIC_IP_ADDRESS`: 您的 GCP 实例公共 IP（可在概览页面中找到）
...
在这里，点击公共 IP 下的`添加网络`按钮。在那里，你需要输入你的公共 IP 地址。如果你不知道你的公共 IP，那就去这个[链接](https://www.whatismyip.com/what-is-my-public-ip-address/)。输入你的公共 IP 后，你就可以走了。
