...
> 原文：[https://www.geeksforgeeks.org/how-to-create-a-table-with-multiple-foreign-keys-in-sql/](https://www.geeksforgeeks.org/how-to-create-a-table-with-multiple-foreign-keys-in-sql/)
...
表 `student_details` 包含两个外键，这两个外键引用了表 `student_branch_details` 和 `student_address`。
...
使用以下 `SQL` 查询在插入行后查看表 `student_details`、`student_branch_details` 和 `student_address`：
