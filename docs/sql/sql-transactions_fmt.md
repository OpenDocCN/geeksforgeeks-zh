...
不完整的步骤会导致交易失败。根据定义，数据库事务必须是原子的、一致的、隔离的和持久的。这些通常被称为[酸性](https://en.wikipedia.org/wiki/ACID)属性。
...
如果单个事务中的所有语句都是有序的，所有的更改都会一起记录在数据库中，这称为**提交**。`COMMIT` 命令将自上次 `COMMIT` 或 `ROLLBACK` 命令以来的所有事务保存到数据库中。
...
下面是一个示例，它将从表中删除那些 `AGE = 20` 的记录，然后提交数据库中的更改。
...
如果任何 SQL 分组语句出现任何错误，则需要中止所有更改。逆转变化的过程称为**回退**。此命令只能用于撤消自上次发出 `COMMIT` 或 `ROLLBACK` 命令以来的事务。
...
**示例:**
从上面的示例中**示例表 1** 、
删除表中那些 `AGE = 20` 的记录，然后通过保留保存点来回滚数据库中的更改。
...
一旦保存点被释放，您就不能再使用 `ROLLBACK` 命令来撤消自上次保存点以来执行的事务。
