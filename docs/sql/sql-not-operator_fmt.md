...
## NOT 语法
`SELECT` 第 1 列、第 2 列、… `FROM` `table_name` `WHERE` `NOT` 条件；
...
```sql
SELECT * FROM Customers WHERE NOT Country = '英国';
```
...
例:
1. 以下 SQL 语句选择国家不是“美国”且不是“英国”的客户：
```sql
SELECT * FROM Customers WHERE NOT Country = '美国' AND NOT Country = '英国';
```
