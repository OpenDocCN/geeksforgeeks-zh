...
> 原文：`https://www.geeksforgeeks.org/plsql-trunc-function/`

`TRUNC` 函数是 PLSQL 中内置的函数，用于返回被截断到特定小数位数的数字。
...
*   `number` – 这是将要被截断为某个数字的输入数字。
*   `decimal_places` – 这也是一个输入数，指定小数点后最多有多少个数字是该函数的输出。
