...
> 原文：[https://www.geeksforgeeks.org/sql-query-to-convert-utc-to-local-time-zone/](https://www.geeksforgeeks.org/sql-query-to-convert-utc-to-local-time-zone/)
...
## 创建数据库和表
### 步骤 1
使用以下 SQL 查询创建数据库 `time_converter`，如下所示。
...
### 步骤 3
使用以下 SQL 查询创建包含 2 列的表 `times`，如下所示。让我们创建一个以索引和 `DATETIME` 作为数据类型的表。
...
### 查询 2：将 `times` 表中的所有时间从 UTC 转换为 IST 本地时间
#### 方法 1
...
#### 方法 2
在该方法中，当前日期时间 (`GETDATE()`) 与 UTC (`GETUTCDATE()`) 日期时间之间的差值被添加到 `date_time` 列中的日期。
