...
> 原文: [https://www.geeksforgeeks.org/sql-query-to-get-column-names-from-a-table/](https://www.geeksforgeeks.org/sql-query-to-get-column-names-from-a-table/)
...
数据表将有三个字段：名字、姓氏和年龄。使用下面的查询，我们将创建一个表。
...
我们将使用 `sys.columns` 来获取表中的列名。它是一个系统表，用于维护列信息。它包含以下有关列的信息：

*   `name` – 列的名称。
*   `object_id` – 列所属表的对象标识。
*   `column_id` – 列的标识。
*   `user_type_id` – 用户定义列类型的 ID。
