...
`PLSQL TRIM` 函数用于从字符串的开头或结尾删除所有指定的字符。
`TRIM` 函数接受三个参数，其中第一个参数可以有值 `LEADING`、`TRAILING`、`BOTH` 和 `Trim_character` 和 `input_string` 之一。

*   如果指定了 `LEADING`，则 Oracle 数据库将删除所有等于 `trim_character` 的前导字符。
*   如果指定了 `TRAILING`，则 Oracle 将删除所有等于 `trim_character` 的尾随字符。
*   如果 `BOTH` 指定或都不指定，则 Oracle 将删除等于 `trim_character` 的前导和尾随字符。
...
1.  `LEADING`: 此参数用于从 `input_string` 前面删除 `trim_character`。
2.  `TRAILING`: 此参数用于从 `input_string` 的末尾移除 `trim_character`。
...
使用 `LEADING` 参数从 `input_string` 前面删除 `trim_character`。
...
使用 `TRAILING` 参数从 `input_string` 的末尾移除 `trim_character`。
...
使用 `BOTH` 参数从 `input_string` 的前面和末尾移除 `trim_character`。
