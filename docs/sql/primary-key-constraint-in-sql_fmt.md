...
> 原文: [https://www.geeksforgeeks.org/primary-key-constraint-in-sql/](https://www.geeksforgeeks.org/primary-key-constraint-in-sql/)
...
如果 `PRIMARY KEY` 只包含一列，应该在列级定义。下面的代码在 `Person` 表上创建主键 `Id`。
...
这里添加了非空值，以确保 `Id` 应该具有唯一的值。如果未指定主键，SQL 将自动为其设置空值。
...
由于我们正在插入重复的值，将会抛出一个错误，因为主键 `Id` 只能包含唯一的值。
...
这里，一个表中只有一个主键，但它由多个列 (`Id`, `Name`) 组成。然而，以下是允许的。
...
作为主键添加的列必须包含唯一值，否则会被违反。如果一个 `Id` 包含重复值，则不能成为主键。它违反了主键的基本规则。更改表以添加 `Id` 作为可能包含重复值的主键会产生错误。
...
此外，使用 `ALTER` 语句作为主键添加的列不应有空值。更改表以添加可能包含空值的 `Id` 作为主键会产生错误。
