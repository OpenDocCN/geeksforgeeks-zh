...
> 原文: [https://www.geeksforgeeks.org/querying-multiple-tables-in-sql/](https://www.geeksforgeeks.org/querying-multiple-tables-in-sql/)

在这里，我们将看到如何在 SQL 中查询多个表。例如，在这里，我们将首先创建一个名为 `geeks` 的数据库，然后我们将在该数据库中创建 2 个表 `department` 和 `employee`。之后，我们将对表执行查询。
