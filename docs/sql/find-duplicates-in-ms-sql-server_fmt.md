...
> 原文: [https://www.geeksforgeeks.org/find-duplicates-in-ms-sql-server/](https://www.geeksforgeeks.org/find-duplicates-in-ms-sql-server/)
...
使用 `Geek` 表中的 `GROUP BY` 子句查找重复行的微软 SQL Server 查询。
...
使用 `Geek` 表中的 `ROW_NUMBER()` 函数查找重复行的 MS SQL Server 查询。
