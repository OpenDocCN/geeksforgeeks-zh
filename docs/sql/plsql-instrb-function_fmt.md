...
PLSQL `INSTRB` 函数接受四个参数，即 `string`、`substring`、`start_position` 和 `nth_appearance`。`string` 和 `substring` 可以是任何数据类型，如 `CHAR`、`VARCHAR2`、`NCHAR`、`NVARCHAR2`、`CLOB` 或 `NCLOB`。
...
*   `start_position`–这是一个可选参数，用于指定字符串中搜索开始的位置。默认值为 1。`INSTR` 函数从字符串的末尾开始计数到 `start_position`，如果插入的值为负，则向字符串的开头搜索。
