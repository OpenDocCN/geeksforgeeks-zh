...
> 原文: [https://www.geeksforgeeks.org/sql-query-to-remove-primary-key/](https://www.geeksforgeeks.org/sql-query-to-remove-primary-key/)
...
这里我们使用 `Drop` 约束从数据库中移除主键。`DROP` 用于删除整个数据库或一个表。`DROP` 语句像销毁现有的数据库、表、索引或视图一样销毁对象。SQL 中的 [`DROP`](https://www.geeksforgeeks.org/sql-drop-truncate/) 语句从关系数据库管理系统(`RDBMS`)中删除一个组件。
