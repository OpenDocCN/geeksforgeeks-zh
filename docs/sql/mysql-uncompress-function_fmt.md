...
MySQL `解压缩()` 函数用于解压缩字符串并将其恢复到原始状态。它用于已使用 `COMPRESS()` 函数压缩的字符串。
...
`string` – 用于指定要解压缩的压缩字符串。

## 返回值

MySQL 中的 `解压缩()` 函数将压缩后的字符串解压缩后返回原始字符串。
...
对空字符串实现 `COMPRESS` 函数，压缩后返回字符串长度。
