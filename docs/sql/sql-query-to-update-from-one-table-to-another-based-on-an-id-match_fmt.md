...
> 原文：[https://www.geeksforgeeks.org/sql-query-to-update-from-one-table-to-another-based-on-an-id-match/](https://www.geeksforgeeks.org/sql-query-to-update-from-one-table-to-another-based-on-an-id-match/)

在本文中，我们将看到，如何基于 ID 匹配从一个表更新到另一个表。我们可以使用 SQL 中的 `update` 语句来更新表。`update` 语句后面总是跟着 `SET` 命令。`SET` 命令用于指定表中哪些列和值需要更新。
...
`demo_table1` 和 `demo_table2` 都有 `NAME` 列。

*   `demo_table1.ID`：指定 `demo_table1` 的标识列。
*   `demo_table2.ID`：指定 `demo_table2` 的标识列。
