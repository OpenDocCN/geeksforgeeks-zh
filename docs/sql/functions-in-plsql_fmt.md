...
函数可以用作 SQL 表达式的一部分，也就是说，我们可以将它们与 `SELECT`/`UPDATE`/`MERGE` 命令一起使用。函数最重要的一个特征是，与过程不同，它必须返回值。
...
现在，让我们举一个例子来演示**声明、定义和调用**一个简单的 PL/SQL 函数，该函数将计算并返回一个数字的反转。
...
可以在函数中使用异常块进行异常处理，但不能使用 `try-catch` 块进行异常处理。
