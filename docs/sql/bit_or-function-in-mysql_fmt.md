...
> 原文: [https://www.geeksforgeeks.org/bit-or-function-in-mysql/](https://www.geeksforgeeks.org/bit-or-function-in-mysql/)

`BIT_OR()` 函数用于返回给定表达式中所有位的按位 OR。它首先将所有十进制值转换为二进制值，然后对这些二进制值执行按位“或”运算。
