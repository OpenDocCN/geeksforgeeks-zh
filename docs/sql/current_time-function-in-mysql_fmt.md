...
> 原文: [https://www.geeksforgeeks.org/current_time-function-in-mysql/](https://www.geeksforgeeks.org/current_time-function-in-mysql/)

`CURRENT_TIME()` 函数用于检查当前时间。它以 `'hh:mm:ss'` 或 `hhmmss` 格式的值返回当前时间，具体取决于该函数是在字符串还是数字上下文中使用。
...
*   `precision` – 它指定返回的分数秒精度。如果用户省略了精度参数，结果将排除精度。
...
`CURRENT_TIME` 函数可用于设置列的值。为了演示，创建一个名为 `DeliveryDetails` 的表。
