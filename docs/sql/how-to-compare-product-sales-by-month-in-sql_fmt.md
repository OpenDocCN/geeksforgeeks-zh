...
> 原文：[https://www.geeksforgeeks.org/how-to-compare-product-sales-by-month-in-sql/](https://www.geeksforgeeks.org/how-to-compare-product-sales-by-month-in-sql/)
...
创建一个名为 `Products` 的表，并添加这两列 `Order_date` 和 `Sales`。
...
在上面的查询中，我们使用了 `SUM()` 函数来计算每个月的总销售额。你也可以找出每个月的销售总数。为此，将求和功能替换为 `COUNT()` 功能。
