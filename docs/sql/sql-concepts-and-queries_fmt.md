...
让我们考虑现有的数据库，如 `information_schema`、`mysql`、`performance_schema`、`sakila`、`student`、`sys` 和 `world`。如果您想显示现有的数据库，那么我们将使用如下所示的 `show database` 查询。
...
`FULL OUTER JOIN` 或完全外部连接关键字返回两个表中的所有记录。
