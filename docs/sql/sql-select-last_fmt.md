...
我们可以使用命令 `FIRST()` 提取特定列的第一个条目，使用 `LAST()` 提取表中特定列的最后一个条目。如需了解更多信息，请访问**中的 [`FIRST()`和`LAST()`功能。**](https://www.geeksforgeeks.org/first-and-lastfunction-in-ms-access/)
...
我们可以使用 [`ORDER BY`](https://www.geeksforgeeks.org/sql-order-by/) 语句和 [`LIMIT`](https://www.geeksforgeeks.org/sql-limit-clause/#:~:text=The%20LIMIT%20clause%20is%20used,be%20a%20non%2Dnegative%20integer.) 子句来提取最后一个数据。基本思想是对表进行降序排序，然后我们将行数限制为 1。这样，我们将获得作为表的最后一行的输出。然后我们可以选择要检索的条目。
