...
此连接返回连接左侧表中的所有行，以及右侧表中的匹配行。对于右侧没有匹配行的行，结果集将包含 `null`。`LEFT JOIN` 也称为 `LEFT OUTER JOIN`。
...
`RIGHT JOIN` 类似于 `LEFT JOIN`。此连接返回连接右侧表中的所有行，以及左侧表中的匹配行。对于左侧没有匹配行的行，结果集将包含 `null`。`RIGHT JOIN` 也称为 `RIGHT OUTER JOIN`。
