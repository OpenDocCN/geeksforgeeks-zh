...
> 原文: [https://www.geeksforgeeks.org/difference-static-dynamic-SQL/](https://www.geeksforgeeks.org/difference-static-dynamic-SQL/)

`静态`或`嵌入式` SQL 是应用程序中的 SQL 语句，在运行时不会改变，因此可以硬编码到应用程序中。`动态` SQL 是运行时构造的 SQL 语句；例如，应用程序可以允许用户输入他们自己的查询。
