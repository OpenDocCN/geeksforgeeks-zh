...
> 原文：`https://www.geeksforgeeks.org/str-and-strcomp-function-in-ms-access/`
...
## `StrComp()`函数
在 MS Access 中，`StrComp()`函数比较两个字符串。它将采用两个参数`string1`、`string2`作为参数。如果两个字符串相同，则返回 0；如果第一个字符串大于第二个字符串，则返回 1；如果`string1 < string2`，则返回-1；如果`string1`或`string2`为 null，则返回 null。
