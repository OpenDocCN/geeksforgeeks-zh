...
> 原文：[https://www.geeksforgeeks.org/difference-between-view-and-cursor-in-sql/](https://www.geeksforgeeks.org/difference-between-view-and-cursor-in-sql/)
...
| 3. | We can perform CRUD operations on views, such as creating, inserting, deleting and updating. | 创建显式光标有一些步骤：<br>1. 在声明部分声明光标。<br>2. 在执行部分打开光标。<br>3. 使用光标将数据检索到 PL/SQL 变量中。<br>4. 关闭光标以释放分配的内存。 |
| 4. | There are two types of views, namely simple views (created from a single table) and complex views (created from multiple tables). | There are two types of cursors, namely implicit cursors (predefined) and explicit cursors (user-defined). |
...
| 6. | 创建视图的一般语法：<br>`Create view "VIEW_NAME" as "SQL statement";` | 创建光标的一般语法：<br>`CURSOR_name IS select_statement;` |