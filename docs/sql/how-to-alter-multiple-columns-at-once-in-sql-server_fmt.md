...
## 步骤 3: 在数据库 GeeksForGeeks 中创建一个公司表
该表有 4 列，即 `FIRST_NAME`、`LAST_NAME`、`SALARY` 和 `BONUS`，包含公司成员的名字、姓氏、薪水和奖金。
...
## 步骤 7: 通过同时向表格中添加两列来更改表格的多(2)列
两栏为 `JOINING_DATE` 和 `LEAVING_DATE`，包含会员加入日期和会员离开日期。使用关键字 [`ALTER`](https://www.geeksforgeeks.org/sql-alter-add-drop-modify/) [`ADD`](https://www.geeksforgeeks.org/sql-alter-add-drop-modify/) 来实现。
...
## 步骤 9: 通过将数据插入到 FIRM 表的 2 个新添加的列中来更新表
使用关键字 `UPDATE`。
...
## 步骤 11: 通过同时从表格中删除两列来更改表格的多(2)列
两栏为 `JOINING_DATE` 和 `LEAVING_DATE`，包含会员加入日期和会员离开日期。使用关键字 `ALTER` 和 `DROP` 来实现。
