...
> 原文：[https://www.geeksforgeeks.org/how-to-eliminate-duplicate-values-based-on-only-one-column-of-the-table-in-sql/](https://www.geeksforgeeks.org/how-to-eliminate-duplicate-values-based-on-only-one-column-of-the-table-in-sql/)
...
## 步骤 3：在数据库 GeeksForGeeks 中创建一个奖金表
该表有 3 列，即 `EMPLOYEE_ID`、`EMPLOYEE_NAME` 和 `EMPLOYEE_BONUS`，包含员工的 ID、员工的姓名和他/她的奖金。
...
## 步骤 7：从表 `BONUSES` 中删除在 `EMPLOYEE_BONUS` 栏中有重复条目的行
为了实现这一点，我们使用 `DELETE` 功能，通过自连接（在表的两个别名上使用 `JOIN` 功能，即 `B1` 和 `B2`）将表与其自身连接起来，并将 `EMPLOYEE_BONUS` 列的条目与 `EMPLOYEE_ID` 列的不同条目进行比较，因为 ID 对于每个员工都是唯一的。
...
**注意**：在 `EMPLOYEE_BONUS` 栏中没有重复的行条目。

**输出：**

![](img/9b1bb3319ed0cdff1d81a3e86f8abe46.png)

## 步骤 9：从表 `BONUSES` 中删除在 `EMPLOYEE_NAME` 栏中有重复条目的行
