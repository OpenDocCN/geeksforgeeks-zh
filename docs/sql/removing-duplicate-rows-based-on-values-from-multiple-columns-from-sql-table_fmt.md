...
## 步骤 3: 在数据库 GeeksForGeeks 中创建一个表 RESULT
该表有 5 列，即 `STUDENT_ID`、`PHYSICS_MARKS`、`CHEMISTRY_MARKS`、`MATHS_MARKS` 和 `TOTAL_MARKS`，包含学生的标识、他/她的物理、化学和数学标记，最后是他/她的总标记。
...
## 步骤 7: 从表 `RESULT` 中删除在列 `CHEMISTRY_MARKS` 和 `MATHS_MARKS` 中有重复条目的行
为了实现这一点，我们使用 `DELETE` 功能通过自连接(对表的两个别名，即 `R1` 和 `R2` 使用 `JOIN` 功能)将表与其自身连接，并对列 `CHEMISTRY_MARKS` 和 `MATHS_MARKS` 的条目进行比较，以获得列 `STUDENT_ID` 的不同条目，因为 ID 对于每个学生都是唯一的。
...
**注意:**在 `CHEMISTRY_MARKS` 和 `MATHS_MARKS` 列中没有重复的行条目。

**输出:**

![](img/4f6b6ca88f494ce9385c76a444484994.png)

## 步骤 9: 从表 `RESULT` 中删除在列 `TOTAL_MARKS` 和 `PHYSICS_MARKS` 中有重复条目的行
