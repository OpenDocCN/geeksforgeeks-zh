...
> 原文: [https://www.geeksforgeeks.org/datepart-function-in-sql-server/](https://www.geeksforgeeks.org/datepart-function-in-sql-server/)

## `DATEPART()`函数:
此函数在 `SQL Server` 中用于查找指定日期的给定部分。此外，它以整数形式返回输出值。

## 特征:
*   该函数用于查找指定日期的给定部分。
*   该功能属于日期功能。
*   该函数接受两个参数，即 `interval` 和 `date`。
*   该功能还可以在日期部分包括时间。
*   该函数以整数形式返回输出。
...
### `interval` –
是要退回的指定部分。此外，`interval` 的值可以如下给出。
