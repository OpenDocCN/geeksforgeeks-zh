...
### 1. 使用 `RENAME USER` 语句重命名单个用户

要使用用户名 `gfguser1` 重命名用户帐户，`RENAME USER` 语句应执行如下:
...
### 2. 使用 `RENAME USER` 语句重命名多个用户

`RENAME USER` 语句可用于一次重命名多个用户账户。要从上面提到的表中重命名两个用户帐户 `gfguser2` 和 `gfguser1`，`RENAME USER` 语句应该如下执行:
