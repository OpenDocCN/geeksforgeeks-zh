...
> 原文: [https://www.geeksforgeeks.org/character_length-function-in-mysql/](https://www.geeksforgeeks.org/character_length-function-in-mysql/)

`CHARACTER_LENGTH()` 这个函数在 MySQL 中帮助找到给定字符串的长度。长度是以字符的形式来衡量的。而且，`CHARACTER_LENGTH()` 也是 [`CHAR_LENGTH()` 函数](https://www.geeksforgeeks.org/char_length-function-in-mysql/)的同义词。
