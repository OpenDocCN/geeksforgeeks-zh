...
`TRANSLATE` 函数接受三个参数 `input_string`、`string_to_replace` 和 `replacement_string`。如果一个字符在 `string_to_replace` 中出现多次，则使用对应于第一次出现的 `replacement_string` 映射。`TRANSLATE` 函数返回一个字符串值。
...
3.  `replacement_string`–用于指定将被 `input_string` 中对应字符替换的字符。
