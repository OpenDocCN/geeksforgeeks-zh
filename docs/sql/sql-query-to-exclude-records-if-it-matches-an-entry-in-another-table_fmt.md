...
> 原文：[https://www.geeksforgeeks.org/sql-query-to-exclude-records-if-it-matches-an-entry-in-another-table/](https://www.geeksforgeeks.org/sql-query-to-exclude-records-if-it-matches-an-entry-in-another-table/)

在本文中，我们将看到如何编写 SQL Query 来排除与另一个表中的条目相匹配的记录。我们可以使用 SQL 中的 `NOT IN` 运算符来执行上述功能。为了获得值列表，我们可以编写子查询。
...
对于演示，从 `demo_table1` 中排除 `NAME` 列中的值与 `demo_table2` 的 `NAME` 列中的条目相匹配的数据。
