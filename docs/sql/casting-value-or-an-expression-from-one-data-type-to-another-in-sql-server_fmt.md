...
> 原文: [https://www.geeksforgeeks.org/casting-value-or-an-expression-from-one-data-type-to-another-in-sql-server/](https://www.geeksforgeeks.org/casting-value-or-an-expression-from-one-data-type-to-another-in-sql-server/)
...
*   `length` –
    `length` 是一个可选参数，用于指定 `targettype` 的长度，默认长度为 30。
...
### Example-1
下面的示例使用 `CAST()` 函数将 `decimal` 类型值转换为 `integer` 类型值。
...
### Example-2
下面的示例使用 `CAST()` 函数将 `decimal` 类型值转换为另一个零比例的 `decimal` 类型值。

```sql
SELECT CAST(5.95 AS DEC(3, 0)) AS Result;
```
...
### Example-3
使用 `CAST()` 函数将字符串转换为 `datetime` 值的示例。
下面的示例使用 `CAST()` 函数将字符串转换为 `datetime`。

```sql
SELECT CAST('2020-09-14' AS DATETIME) AS Result;
```
