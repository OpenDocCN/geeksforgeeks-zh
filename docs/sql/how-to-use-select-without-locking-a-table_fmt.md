...
> 原文: [https://www.geeksforgeeks.org/how-to-use-select-without-locking-a-table/](https://www.geeksforgeeks.org/how-to-use-select-without-locking-a-table/)
...
因此 **SQL Server 中的解决方案是** `NOLOCK` 表提示允许您指示查询优化器读取给定的表，而无需获得排他锁或共享锁。
...
## 第 7 步: 要运行阻塞查询，必须 `KILL` 或 `COMMIT` 或 `ROLLBACK` 事务。然而，这不是最可取的解决方案。这就是 `WITH (NOLOCK)` 进入画面的地方。
