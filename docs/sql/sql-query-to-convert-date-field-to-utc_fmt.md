...
> 原文：[https://www.geeksforgeeks.org/sql-query-to-convert-date-field-to-utc/](https://www.geeksforgeeks.org/sql-query-to-convert-date-field-to-utc/)
...
## 步骤 4：使用 `GETDATE()` 命令显示当前日期和时间

编写一个命令，以默认格式显示当前日期和时间。我们稍后会将其转换为世界协调时，即协调世界时。
...
## 步骤 5：使用 `GETUTCDATE()` 命令在 UTC 字段中显示当前日期和时间

编写一个类似的命令，在 UTC 字段中显示当前日期和时间。
...
## 步骤 6：同时运行命令以比较本地时区和 UTC 时间

现在同时编写和运行两个命令，根据本地时区和世界协调时查看当前日期和时间的值。
