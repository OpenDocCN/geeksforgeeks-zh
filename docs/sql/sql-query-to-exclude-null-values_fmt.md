...
> 原文: [https://www.geeksforgeeks.org/sql-query-to-exclude-null-values/](https://www.geeksforgeeks.org/sql-query-to-exclude-null-values/)
...
使用上面的查询在我们的数据库中创建`Student`表。`Student`表有三个字段`Name`、`Department`和`Roll_No`。要在表中插入值，我们必须使用`INSERT`查询。
...
使用上面的查询，我们已经将数据添加到了表中。我们使用`NULL`关键字插入空值。
...
为了从所有列中排除空值，我们使用了`AND`运算符。如果所有条件都为真，则`AND`运算符显示记录。
