...
> `INSERT Student(ROLL_NO, NAME, AGE) VALUES('5','PRATIK','19');`
...
- 你需要提供多个值列表，每个列表用逗号（`,`）分隔。每个值列表对应于表中每个新行要插入的值。
- 下一个列表中的值告诉我们要插入到表的下一行中的值。
