...
> 原文：[https://www.geeksforgeeks.org/how-to-print-prime-numbers-in-ms-sql-server/](https://www.geeksforgeeks.org/how-to-print-prime-numbers-in-ms-sql-server/)

在本文中，我们将使用 MS SQL 打印素数。这里我们将使用 2 个 `while` 循环语句来打印素数。

## 步骤 1

首先我们将声明一个初始值为 2 的变量 `@I`。
...
然后我们将声明一个初始值为 0 的变量 `@PRIME`（这将设置 `PRIME` 的值）。
...
现在我们将使用嵌套 `while` 循环，就像我们为质数编写程序一样。
...
*   `IF` 语句在那里，条件 `@I % @J = 0`，这意味着当 `@I`/`@J` 的余数为 0 时，`@PRIME` 将被设置为 0，`@J` 的值递减 1。
