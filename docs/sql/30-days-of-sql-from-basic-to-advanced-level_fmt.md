...
> 原文: [https://www.geeksforgeeks.org/30-days-of-sql-from-basic-to-advanced-level/](https://www.geeksforgeeks.org/30-days-of-sql-from-basic-to-advanced-level/)
...
*   [`INSERT INTO`](https://www.geeksforgeeks.org/sql-insert-statement/): 用于插入任务。
*   [`AND`/`OR` 运算符](https://www.geeksforgeeks.org/sql-and-and-or-operators/): 基于与或运算符选择数据。
*   [`DROP` 与 `TRUNCATE`](https://www.geeksforgeeks.org/sql-drop-truncate/): 它将根据条件删除或截断集合。
*   [`NOT` 运算符](https://www.geeksforgeeks.org/sql-not-operator/): 将根据给定条件选择**非**的数据。
...
*   [`UPDATE` 语句](https://www.geeksforgeeks.org/sql-update-statement/): 根据提供的条件更新某些数据项。
*   [`ALTER TABLE`](https://www.geeksforgeeks.org/sql-alter-add-drop-modify/): 根据给定条件添加、删除或修改表格。
*   [`LIKE` 子句](https://www.geeksforgeeks.org/sql-like/): 它将遵循搜索条件中给出的模式。
*   [`BETWEEN` 与 `IN` 运算符](https://www.geeksforgeeks.org/sql-between-in-operator/): 它将在给定条件下选择或中的数据范围。
*   [`CASE` 语句](https://www.geeksforgeeks.org/sql-case-statement/): 它会检查条件句，并根据各自的情况查询数据。
...
*   [`COUNT()` 函数](https://www.geeksforgeeks.org/aggregate-functions-in-sql/): 返回过滤数据的总计数。
*   [`SUM()` 函数](https://www.geeksforgeeks.org/aggregate-functions-in-sql/): 返回所有被查询数据的和。
*   [`AVG()` 函数](https://www.geeksforgeeks.org/aggregate-functions-in-sql/): 返回所有被查询数据的平均值。
*   [`MIN()` 函数](https://www.geeksforgeeks.org/aggregate-functions-in-sql/): 将从正在查询的全部数据中返回最小数据。
*   [`MAX()` 函数](https://www.geeksforgeeks.org/aggregate-functions-in-sql/): 将从被查询的全部数据中返回最大数据。
*   [`ORDER BY`](https://www.geeksforgeeks.org/sql-order-by/): 该语句将根据您的方便程度对查询到的数据进行升序或降序排序。
*   [`GROUP BY`](https://www.geeksforgeeks.org/sql-group-by/): 此语句将使用条件中给出的列对所有查询的数据进行分组。
*   [`ALL` 与 `ANY` 子句](https://www.geeksforgeeks.org/sql-all-and-any/): 它们是 SQL 中的逻辑运算符，结果返回布尔值。
...
*   [`INTERSECT` 子句](https://www.geeksforgeeks.org/sql-intersect-except-clause/): 它将在两个或多个表相交的地方连接它们。
*   [`ALIAS`](https://www.geeksforgeeks.org/sql-aliases/): 它会给表取一个别名，我们以后可以称之为。
*   [笛卡尔连接和自连接](https://www.geeksforgeeks.org/sql-join-cartesian-join-self-join/): 有时候为了查询出一些数据，我们不得不将表自连接到自身。
*   [`INNER JOIN`, `LEFT JOIN`, `RIGHT JOIN` 和 `FULL JOIN`](https://www.geeksforgeeks.org/sql-join-set-1-inner-left-right-and-full-joins/): 当我们必须将一个表与另一个表连接时，这四种类型的连接就会发挥作用。查看它们的语法并学习处理这些连接。
*   [`DIVISION` 子句](https://www.geeksforgeeks.org/sql-division/): 当您想要找出与一组不同类型实体的所有实体交互的实体时，通常需要除法。
...
*   [组合值](https://www.geeksforgeeks.org/combining-aggregate-and-non-aggregate-values-in-sql-using-joins-and-over-clause/): 使用 `JOIN` 和 `OVER` 子句组合 SQL 中的聚合值和非聚合值。
*   [`MINUS` 运算符](https://www.geeksforgeeks.org/sql-minus-operator/): 用作“除”，表示它将连接两个相交的表，并将减去一个表，以便只覆盖交集和另一个表。
