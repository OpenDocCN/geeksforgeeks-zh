...
> 原文：[https://www.geeksforgeeks.org/find-nth-highest-salary-table/](https://www.geeksforgeeks.org/find-nth-highest-salary-table/)
...
3.  作为一个分析函数，`DENSE_RANK` 根据 `order_by` 子句中 `value_exprs` 的值，计算查询返回的每一行相对于其他行的排名。
...
*   下一个外部查询是：“`SELECT MIN(sal) sal IN (Result_Set_of_Previous_Query) FROM Employee`”。这将返回以下结果：
...
*   接下来是最外层的查询，即：“`SELECT * FROM Employee WHERE sal = Result_of_Previous_Query`”。该查询将返回薪资第四高的员工的详细信息。
