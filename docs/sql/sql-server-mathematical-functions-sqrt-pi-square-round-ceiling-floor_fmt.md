...
> 原文: [https://www.geeksforgeeks.org/sql-server-mathematical-functions-sqrt-pi-square-round-ceiling-floor/](https://www.geeksforgeeks.org/sql-server-mathematical-functions-sqrt-pi-square-round-ceiling-floor/)
...
### 2. `PI()`
有些计算需要使用 PI。使用`pi()`函数，PI 的值可以在查询中的任何地方使用。
...
### 5. `CEILING()` 和 `FLOOR()`
**`CEILING()`:** `CEILING()`函数用于查找下一个最高值(整数)。
