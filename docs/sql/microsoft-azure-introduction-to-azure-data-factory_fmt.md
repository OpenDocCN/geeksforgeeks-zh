...
> 原文: [https://www.geeksforgeeks.org/microsoft-azure-introduction-to-azure-data-factory/](https://www.geeksforgeeks.org/microsoft-azure-introduction-to-azure-data-factory/)
...
随着世界各地的数据日益增加，许多企业和企业正在转向使用基于云的技术，以使其业务可扩展。由于云适应的增加，需要云中可靠的 `ETL` 工具来进行集成。与其他 `ETL` 工具相比，`Azure` 数据工厂因**功能**而脱颖而出，如**易用**、**经济高效的解决方案**、**强大智能的免费代码服务**。
...
*   **活动**：管道中的活动定义要对数据执行的操作。例如，`复制数据活动`可以从 `Blob` 存储的一个位置读取数据，并将其加载到 `Blob` 存储的另一个位置。
...
2.  **SQL Server 集成服务**：有助于轻松地将您现有的内部 `SQL Server` 集成服务项目移动到云中完全受管理的环境中。`SQL Server` 集成服务集成运行时节点的定价从 `₹60.498/小时` 开始。