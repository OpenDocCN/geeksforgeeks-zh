...
如果几个列具有相同的名称，但是数据类型不匹配，`自然连接`子句可以用`使用`子句来修改，以指定应该用于`等连接`的列。

*   当多列匹配时，`USING`子句仅用于匹配一列。
*   `自然连接`和`使用`条款是互斥的。
*   在引用的列中不应有限定符(表名或别名)。
*   `自然连接`使用所有具有匹配名称和数据类型的列来连接表。`USING`子句只能用于指定那些应该用于`EQUIJOIN`的列。
...
**说明:** 所示示例将`EMPLOYEES`和`DEPARTMENTS`表中的`DEPARTMENT_ID`列连接起来，从而显示了员工的工作地点。
...
**说明:** 所示示例将`LOCATIONS`和`COUNTRY`表中的`COUNTRY_ID`列连接起来，从而显示了所需的详细信息。
