...
> 原文：[https://www.geeksforgeeks.org/how-to-add-a-foreign-key-using-alter-in-mysql/](https://www.geeksforgeeks.org/how-to-add-a-foreign-key-using-alter-in-mysql/)
...
下面我们来看看如何在另一个表 `exam` 中，将学生表 `student` 中的主键 `student_id` 的一个属性添加为外键，如下所示。

### 步骤-1：创建数据库 university
...
**输出：**
在这里，正如你在描述中看到的，`student_id` 的 `Key` 列是 **PRI**，这意味着它是那个表 `student` 中的主键。
...
### 步骤-7：在 exam 表中添加另一列 student_id

在这里，您将看到如何在 `exam` 表中添加另一列 `student_id`，如下所示。
...
在这里，您将看到如何制作 `exam` 表中的 `student_id` 属性外键，它是 `student` 表中的主键，如下所示。
...
REFERENCES table_name(Referencing_column_name_in_table_name);
```
...
**输出：**
现在可以看到在表 `exam` 的描述中又增加了一列 `student_id`，在描述的 `Key` 列中，`student_id` 有 **MUL** 表示是外键。
