...
> 原文: [https://www.geeksforgeeks.org/how-to-create-daily-weekly-and-monthly-report-in-sql-server/](https://www.geeksforgeeks.org/how-to-create-daily-weekly-and-monthly-report-in-sql-server/)
...
这里我们检查“文章日期”列与当前日期值是否匹配。`CurrentDate` 可以通过 `GETDATE()` 函数获得。使用 `CONVERT`，我们对照“文章日期”值进行检查。
...
这里我们用的是 `BETWEEN` 子句。开始日期应该比当前日期早 30 天，因此按以下方式提供。
