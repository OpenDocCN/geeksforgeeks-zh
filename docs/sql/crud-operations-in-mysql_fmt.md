...
1.  **Alter Table 命令 –**
    这是 DDL 命令 (Data Definition Language) 用于改变表的结构。
2.  **Update Table 命令 –**
    这是 DML 命令 (Data Manipulating Language) 用于修改记录。
...
使用 `Update` 命令将姓名为 `ravi` 的学生的分数从 23 更新为 100 的更新命令:
