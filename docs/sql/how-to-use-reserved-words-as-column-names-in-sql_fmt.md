...
> 原文: [https://www.geeksforgeeks.org/how-to-use-reserved-words-as-column-names-in-sql/](https://www.geeksforgeeks.org/how-to-use-reserved-words-as-column-names-in-sql/)
...
## 步骤 3: 在数据库 GeeksForGeeks 中创建一个表
我们将表命名为 `BIKE`。它将包含 2 个名为 `SELECT` 和 `TABLE` 的列。这两个都是 SQL 中的关键词。这里的技巧是在方括号“[]”内键入列名，这样编译器就不会将它们作为保留字来读取。
...
**注意:** 这里 `SELECT` 列存储自行车名称，`TABLE` 列存储自行车费用。
