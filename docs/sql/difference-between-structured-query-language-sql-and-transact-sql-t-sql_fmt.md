...
> 原文: [https://www.geeksforgeeks.org/difference-between-structured-query-language-sql-and-transact-sql-t-sql/](https://www.geeksforgeeks.org/difference-between-structured-query-language-sql-and-transact-sql-t-sql/)
...
`Transact-SQL(T-SQL)` 是 `SQL` 的扩展。它被认为是过程语言，不像 `SQL` 服务器使用的 `SQL`。它有助于执行从单行获取数据、添加新行、获取多行等操作。语法不同于其他类似 `PL-SQL` 的语法。但是，它具有与其他语言相同的功能并生成类似的结果。这是 `SQL server` 结构化查询语言的微软实现。
