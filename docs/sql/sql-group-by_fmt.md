...
Group By single column means, to place all the rows with same value of only that particular column in one group. Consider the query as shown below:
...
这就是 `HAVING` 子句开始使用的地方。我们可以使用 `HAVING` 子句设置条件来决定哪个组将成为最终结果集的一部分。此外，我们不能使用像 `SUM()`，`COUNT()` 等聚合函数带 `WHERE` 子句。所以如果我们想在条件中使用这些函数，我们必须使用 `HAVING` 子句。
