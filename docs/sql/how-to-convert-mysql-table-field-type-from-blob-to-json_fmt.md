...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-mysql-table-field-type-from-blob-to-json/](https://www.geeksforgeeks.org/how-to-convert-mysql-table-field-type-from-blob-to-json/)
...
`ALTER TABLE` 是用于添加、删除或修改现有表格中的字段的命令。它还用于在现有表上添加和删除各种约束。我们有两个改变表格的命令，[`MODIFY`](https://www.geeksforgeeks.org/sql-alter-add-drop-modify/) 和 [`CHANGE`](https://www.geeksforgeeks.org/sql-alter-add-drop-modify/)。
...
我们将使用 `DESCRIBE` 命令来交叉检查我们的表是否创建了一个带有 `BLOB` 数据类型的字段。
...
## 步骤 5：将字段数据类型从 BLOB 更改为 JSON

`DOCS` 字段将被更改，因为它属于 `BLOB` 数据类型。
...
## 步骤 6：使用 DESCRIBE 命令查看对每个命令所做的更改：MODIFY 和 CHANGE
