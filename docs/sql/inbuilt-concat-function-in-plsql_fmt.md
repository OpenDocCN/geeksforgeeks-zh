...
> 原文: [https://www.geeksforgeeks.org/inbuilt-concat-function-in-plsql/](https://www.geeksforgeeks.org/inbuilt-concat-function-in-plsql/)
...
concatenation 的意思是将各种各样的字符串连接起来，形成一个新的字符串，或者我们也可以说是将序列中的某个东西链接在一起，或者链接成一个链。所以在 PLSQL 中，我们可以使用 `concat()` 函数将各种字符串连接成一个字符串。所以 `concat()` 函数接受两个输入作为参数，然后返回附加的字符串。我们还可以使用它来连接两个以上的字符串，我们可以在下面的示例中看到它的实现。
