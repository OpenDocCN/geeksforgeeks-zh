...
应用相同的逻辑来连接 2 个表，即连接 `n` 个表所需的最小 `JOIN` 语句数量为 `(n-1)`。
...
我们来看看创建的表：
*   `s_id` 是 `student` 表中的主键，是 `marks` 表中的外键。（`student`（家长）–`marks`（孩子））。
*   `school_id` 是 `marks` 表中的主键和 `details` 表中的外键。（`marks`（父项）–`details`（子项））。
