...
`PLSQL ASIN` 函数用于返回一个数的反正弦。`ASIN` 函数只有一个参数，这个参数是一个数字，参数个数必须在 `-1` 到 `1` 的范围内，函数返回一个在-pi/2 到 pi/2 范围内的值，用弧度表示。
...
在 `ASIN` 函数中使用一个不在-1 和 1 之间的数值作为参数。
