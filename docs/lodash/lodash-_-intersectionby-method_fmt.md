...
> 原文: [https://www.geeksforgeeks.org/lodash-_-intersectionby-method/](https://www.geeksforgeeks.org/lodash-_-intersectionby-method/)

`Lodash` 是一个工作在 `underscore.js` 之上的 JavaScript 库，有助于处理数组、字符串、对象、数字等。
`_.intersectionBy()` 用于根据在数组的每个元素上迭代的某个函数，获取数组与任意数量的数组的交集。它在数组相交后返回数组。
...
*   `arrays`: 要取交集的数组。
*   `iteratee=_.identity`: 它是迭代数组中要取交集的每个元素的函数。
...
在使用下面给出的代码之前，请通过 `npm install lodash` 安装 lodash 模块。

## 示例 1

当迭代函数未给出时，它的行为与 lodash 的 `intersection()` 函数相同。
...
当使用 `Math.ceil()` 函数在数组 1 上运行时，将数组 1 的所有值与下一个最近的较大整数进行比较，然后取交集。
...
当存在多个公共元素时，它只返回它们一次，并且在数组中不返回重复的元素。
