...
`_.isTypedArray()`方法用于查找给定值是否为类型化数组。如果给定值是类型化数组，则返回真。否则，它返回 `false`。
...
## 返回值:
如果值是类型化数组，该方法返回 `true`，否则返回 `false`。
...
**参考资料:** [https://lodash.com/docs/4.17.15#isTypedArray](https://lodash.com/docs/4.17.15#isTypedArray)