...
`_.zipWith()`方法用于通过在数组的相同索引处应用相同值的函数将数组组合成一个数组，首先将函数应用于数组的第一个值，然后将返回值添加到新的数组第一个值中，第二个、第三个也是如此，依此类推。如果你没有传递这个函数，它将简单地作为`zip`工作。
...
## 参数:
该方法接受两个或两个以上参数，如上所述，如下所述:
*   `arrays`: This parameter holds one or more arrays to be processed.
*   `[iteratee=_.identity]`: This parameter holds the function of combining grouping values.

## 返回值:
该方法在给定数组上应用函数后返回一个数组。
...
**参考:** [https://lodash.com/docs/4.17.15#zipWith](https://lodash.com/docs/4.17.15#zipWith)