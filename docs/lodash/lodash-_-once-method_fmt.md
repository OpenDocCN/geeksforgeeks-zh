...
**Lodash中的`_.once()`方法**用于创建一个只能调用该方法的`func`参数一次的函数。但是，对该函数的重复调用将返回第一次调用中返回的值。

**注意:** 这个方法的`func`参数和创建的函数的参数一起被这个绑定调用。
...
这里，每次调用`fetch`时，返回的值与第一次调用返回的值相同。

**参考:** [https://lodash.com/docs/4.17.15#once](https://lodash.com/docs/4.17.15#once)