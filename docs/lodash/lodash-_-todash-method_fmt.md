...
*   **`camelCaseString`**: This method takes a camelCaseString and converts it into a dotted string.

**返回值:**这个方法返回一个转换后的虚线字符串。

**注意:**这在正常的 JavaScript 中不会起作用，因为它需要安装 `lodash-contrib` 库。可以使用 `npm install lodash-contrib` 来安装 Lodash.js contrib 库。
