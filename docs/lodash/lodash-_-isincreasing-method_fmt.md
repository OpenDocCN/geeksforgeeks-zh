...
该方法返回一个布尔值（如果给定值为递增，则返回 `true`，否则返回 `false`）。

## 注意

这在正常的 JavaScript 中不会起作用，因为它需要安装 `lodash.js contrib` 库。可以使用 `npm install lodash-contrib` 来安装 Lodash.js contrib 库。
