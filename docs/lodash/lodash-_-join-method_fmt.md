...
*   `array`: 是要执行联接操作的原始数组。
*   `separator`: 分隔数组每个元素的字符串。如果使用默认值，数组元素用逗号(，)分隔。

## 返回值

该函数返回一个字符串，该字符串是通过使用分隔符连接数组的所有元素而创建的。

## 注意

在使用下面给出的代码之前，通过使用命令`npm install lodash`来安装lodash模块。

## 示例 1

在本例中，函数`join()`使用`'|'`将数组的元素连接到一个字符串中。
...
在本例中，函数`join()`使用`","`将数组的元素连接到一个字符串中，因为它是默认值。
...
在本例中，函数`join()`使用`''`(空字符串)将数组的元素连接在一起形成一个字符串。
