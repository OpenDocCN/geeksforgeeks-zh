# Lodash _.isAssociative() 方法

> 原文: [https://www.geeksforgeeks.org/lodash-_-isassociative-method/](https://www.geeksforgeeks.org/lodash-_-isassociative-method/)

Lodash `_.isAssociative()` 方法检查给定值是否关联。关联对象是指可以通过键或索引访问其元素的对象，例如数组。
...
该方法返回一个布尔值（如果给定值是关联的，则返回 `true`，否则返回 `false`）。
...
以下示例说明了 Lodash `_.isAssociative()` 方法在 JavaScript 中的使用：
