...
`_.reductions()`方法用于将元素数组转换为数组，在该数组中存储折叠操作中的每个中间值。这个方法是同`_.reduce()`方法，只是它返回一个数组。该方法传递一个数组、一个函数、和一个起始值，生成一个新的数组，对和数组进行运算。
...
这在正常的 JavaScript 中无法工作，因为它需要安装 Lodash contrib 库。 Lodash contrib 库可以使用`npm`安装 `Lodash-contrib–保存`。

## 示例 1

在此示例中，求和数组是以给定的起始值 0 生成的，该值在加法运算时更新。
...
在本例中，通过将的起始值设为 1 来生成乘法数组，该值在进一步乘法时更新。
