...
> 原文：`https://www.geeksforgeeks.org/lodash-_-cat-method/`

Lodash 是一个工作在下划线之上的 JavaScript 库。Lodash 有助于处理数组、字符串、对象、数字等。

`_.cat()` 方法用于将零个或多个参数连接成一个数组。它可以用来连接多种类型的参数。
...
这个方法在正常的 JavaScript 中无法工作，因为它需要安装 Lodash contrib 库。`lodash-contrib` 库可以使用 `npm install lodash-contrib --save` 来安装。
...
`_.cat()` 函数也将像处理数组一样处理 `arguments` 对象。
