...
`_.partition()` 方法创建一个分成两组的元素数组，第一组包含谓词返回 `true` 的元素，第二组包含谓词返回 `false` 的元素。
...
*   `predicate`: 此参数保存每次迭代调用的函数，并且用一个参数（值）调用。
...
let gfg = _.partition(users, function(o) { return o.active; });
