...
**Lodash** 是 Node.js 中的一个模块，工作在 underscore.js 之上，帮助处理数组、字符串、对象、数字等。
使用 `_.slice()` 函数从开始索引到结束索引对数组进行切片，这里结束索引是独占的，开始索引是包含的。
...
**注意:** 请在使用下面给出的代码之前，通过 `npm install lodash` 安装 lodash 模块。
