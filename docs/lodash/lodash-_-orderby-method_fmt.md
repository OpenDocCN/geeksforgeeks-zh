...
Lodash 是一个工作在下划线.js 之上的 JavaScript 库。Lodash 有助于处理数组、集合、字符串、对象、数字等。`_.orderBy()`方法类似于 `_.sortBy()`方法，除了它允许迭代的排序顺序作为排序依据。如果未指定顺序，则所有值都按升序排序，否则相应值的顺序指定“desc”为降序，或“asc”为升序排序。
...
这段代码在正常的 JavaScript 中无法工作，因为它需要安装库 lodash。