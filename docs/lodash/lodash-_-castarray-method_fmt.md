...
`_。castArray()`方法用于将值转换成数组，如果它不是数组的话。
...
此示例使用 String、null 和 undefined 作为参数。
...
这个例子使用了多个参数，它将只取第一个参数并且用一个数组，它将只返回同一个数组。
...
参考:[https://lodash.com/docs/4.17.15#castArray](https://lodash.com/docs/4.17.15#castArray)