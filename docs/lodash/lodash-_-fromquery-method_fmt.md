...
`_.fromQuery()` 方法用于将给定的 URL 查询字符串转换为等价的 JavaScript 对象。
...
*   `URL_Query`: 该方法取一个网址查询字符串转换成 JavaScript 对象。

## 返回值

这个方法返回等价的 JavaScript 对象。

## 注意

这在正常的 JavaScript 中不会起作用，因为它需要安装 `lodash-contrib` 库。可以使用 `npm install lodash-contrib --save` 命令安装。
