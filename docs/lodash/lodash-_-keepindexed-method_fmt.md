...
这在正常的 JavaScript 中不会起作用，因为它需要安装 `lodash-contrib` 库。可以使用 `npm install lodash-contrib --save` 安装 `lodash-contrib` 库。

## 示例 1

在本例中，我们将通过检查条件来使用此方法生成一个数组。数组的索引在函数中传递，该函数进一步用于获取值和进行比较。
