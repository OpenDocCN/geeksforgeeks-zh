...
`_.kebabCase()` 方法用于将给定字符串转换为 kebab 大小写字符串。[kebab-case](https://en.wikipedia.org/wiki/Letter_case#Special_case_styles) 是用连字符代替空格书写标识符的做法。
...
*   `string`: 此参数保存需要转换为 kebab 大小写字符串的字符串。

**返回值:** 该方法返回 kebab 大小写字符串。
