...
`_.remove()`方法用于从数组中移除谓词返回`True`并返回移除的元素的所有元素。
...
这里所有回归真的元素都是偶元素。在数组的所有元素(`n`)上调用该函数。
...
本示例从包含字母的数组中移除所有元音，并将它们存储在新数组中。
...
**参考:** [https://lodash.com/docs/4.17.15#remove](https://lodash.com/docs/4.17.15#remove)