...
`_.chunkAll()`方法是用来将给定的数组分成给定大小的小块。它是类似于的 `_.chunk()` 方法除了这个 方法永远不会从末尾掉短组块。
...
*   `array`:就是要拆分的阵。
*   `number`:是要形成的组块的大小。
*   `partitions`:它指定了如何从跳过的区域构建分区。这是一个可选参数。
...
这在正常的 JavaScript 中不会起作用，因为它需要安装 lodash-contrib 库。可以使用 `npm install lodash-contrib --save` 来安装 lodash-contrib 库
