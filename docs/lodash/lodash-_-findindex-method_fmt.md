...
Lodash 是 Node.js 中的一个模块，工作在 Underscore.js 之上。Lodash 有助于处理数组、字符串、对象、数字等。
使用 `findIndex()` 方法查找元素第一次出现的索引。它不同于 `indexOf`，因为它采用遍历数组每个元素的谓词函数。
...
*   `fromIndex`: 是要搜索元素的索引。如果默认情况下未给出 `fromIndex`，则它将为 0。
...
**示例 2:** 当元素在某个索引 `i` 之后。这里，元素出现在数组中，但输出仍然是 -1，因为它出现在索引 3 处，搜索从索引 5 开始。
