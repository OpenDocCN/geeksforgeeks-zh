...
`_.pullAll()`方法用于从第一个给定数组中移除第二个数组中给定的所有值。
...
*   `array`: 此参数存储要修改的数组。
*   `values`: 此参数保存需要从第一个数组中移除的值。

## 返回值:

返回一个数组，该数组包含从第一个数组中移除的所有值。

## 示例 1:

本示例从第一个数组中移除第二个数组元素，并返回剩余的元素。
...
**参考:** [https://lodash.com/docs/4.17.15#pullAll](https://lodash.com/docs/4.17.15#pullAll)