...
`_.zipObject()`方法用于将两个数组组合成一个对象，一个数组作为键，另一个作为值。
...
**示例 3:** 如果您传递了一个额外的键，但没有值，它将放入与该键相关联的`undefined`值。
...
**注意:** 这在正常的JavaScript中不会起作用，因为它需要安装库`lodash`。

参考: [https://lodash.com/docs/4.17.15#zipObject](https://lodash.com/docs/4.17.15#zipObject)