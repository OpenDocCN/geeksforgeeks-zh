...
`_.transform()`方法是`_.reduce()`方法将对象转换为一个新的累加器对象，这是通过迭代运行它自己的每个可枚举字符串键控属性的结果，每次调用都可能使累加器对象发生变化。如果不提供累加器，将使用具有相同原型的新对象。
...
*   `object`:它保存要迭代的对象。
*   `iteratee`:是方法每次迭代为每个元素调用的函数。
*   `accumulator`:保存自定义累加器值。
