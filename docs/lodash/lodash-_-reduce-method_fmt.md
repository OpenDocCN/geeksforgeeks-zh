...
`_。reduce()`方法将集合缩减为值，该值是通过迭代运行集合中每个元素的累积结果，其中每个连续的调用都被提供上一个调用的返回值。如果没有给定累加器，则集合的第一个元素用作初始值。有许多 lodash 方法被保护起来，作为像 `_`这样的方法的迭代。减少，`_。reduceRight` 和 `_.transform`。
