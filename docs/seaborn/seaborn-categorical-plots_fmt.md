...
图基本上用于可视化变量之间的关系。这些变量可以是完全数字的，也可以是一个类别，如组、类或部门。本文讨论了分类变量以及如何使用 Python 提供的 `Seaborn` 库来可视化它们。

`Seaborn` 除了是统计标绘库外，还提供了一些默认数据集。我们将使用一个名为“提示”的默认数据集。“小费”数据集包含了可能在餐馆吃过饭的人的信息，以及他们是否给服务员留过小费、他们的性别、他们是否吸烟等等。让我们来看看 `tips` 数据集。
...
**解释/分析**
`x` 取范畴列，`y` 为数值列。因此，我们可以看到每天花费的总账单。`hue` 参数用于进一步增加分类分离。通过看情节，我们可以说不吸烟的人在周五比吸烟的人有更高的账单。
...
*   带状图的一个问题是，你无法真正分辨出哪些点是堆叠在彼此之上的，因此我们使用 `jitter` 参数来添加一些随机噪声。
*   `jitter` 参数用于增加抖动量(仅沿分类轴),当您有许多点并且它们重叠时，这可能很有用，因此更容易看到分布。
*   `hue` 用于提供附加分类分离
*   设置 `dodge=True` 用于根据 `hue` 参数指定的类别绘制单独的带状图。

## 蜂群图 (Swarmplot)

除了调整点使其不重叠之外，它与 `stripplot` 非常相似。有些人也喜欢把 `violinplot` 和 `stripplot` 结合起来形成这个情节。使用 `swarmplot` 的一个缺点是，有时它们不能很好地扩展到非常大的数字，并且需要大量的计算来排列它们。所以如果我们想正确地想象一个群体图，我们可以把它画在一个 `violinplot` 的上面。
...
它是所有这些图中最通用的，并提供了一个名为 `kind` 的参数来选择我们想要的图的类型，从而免除了我们单独编写这些图的麻烦。`kind` 参数可以是 `bar`、`violin`、`swarm` 等。
