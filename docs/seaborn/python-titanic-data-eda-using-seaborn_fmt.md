...
> 原文: [https://www.geeksforgeeks.org/python-titanic-data-eda-using-seaborn/](https://www.geeksforgeeks.org/python-titanic-data-eda-using-seaborn/)
...
具有空值的列有: `Age`、`Cabin`、`Embarked`。稍后需要用适当的值填充它们。

## 特征:
泰坦尼克号数据集大致有以下几类特征:

*   **分类/名义**: 可以分为多个类别但没有顺序或优先级的变量。
    例如: `Embarked`（C = 瑟堡；Q = 皇后镇；S = 南安普顿）
...
*   **计数**: 它们代表一个变量的计数。
    例如 `SibSp`，`Parch`
*   **无用**: 它们对 ML 模型的最终结果没有贡献。在这里，`PassengerId`、`Name`、`Ticket` 和 `Cabin` 可能属于这一类。
...
这有助于确定高等级乘客的存活率是否高于低等级乘客，反之亦然。*1 级*乘客比*2 级和 3 级*乘客存活几率更高。这意味着*Pclass*对乘客的存活率有很大的贡献。
...
由于`Age`栏很重要，需要填写缺失的值，或者使用`Name`栏（根据称呼确定年龄-先生、女士等。）或通过使用回归器。
此步骤后，可创建另一列–`Age_Range`（基于`Age`列）并可再次分析数据。

### 代码: Family_Size（计数特征）和 Family_Size 的系数图。
...
**Family_Size** 表示乘客家庭的人数。通过对相应乘客的 `SibSp` 和 `Parch` 列求和来计算。此外，还增加了另一栏 `Alone`，以检查单独乘客与有家庭的乘客的生存机会。
...
**Fare** 表示乘客支付的票价。由于此栏中的值是连续的，因此需要将它们放在单独的容器中（如对`Age`功能所做的那样）以获得清晰的概念。可以得出结论，如果乘客支付了更高的票价，存活率就更高。

### 代码: Embarked 特征的分类计数图
...
*   可以删除的列有:
    *   `PassengerId`、`Name`、`Ticket`、`Cabin`: 它们是字符串，无法分类，对结果没有太大贡献。
    *   `Age`、`Fare`: 取而代之的是，保留相应的范围列。
