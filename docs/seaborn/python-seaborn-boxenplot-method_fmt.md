...
> 原文: [https://www.geeksforgeeks.org/python-seaborn-boxenplot-method/](https://www.geeksforgeeks.org/python-seaborn-boxenplot-method/)
...
`seaborn.boxenplot(参数)`

### 参数

*   `x, y, hue`: 用于绘制长格式数据的输入。
*   `data`: 用于绘图的数据集。
*   `order, hue_order`: 绘制分类级别的顺序，否则从数据对象推断级别。
*   `orient`: 地块的方向（垂直或水平）。
*   `color`: 所有元素的颜色，或者渐变调色板的种子。
*   `palette`: 用于色调变量不同级别的颜色。
*   `saturation`: 绘制颜色时原始饱和度的比例。
*   `width`: 不使用色调嵌套时整个元素的宽度，或主要分组变量一个级别的所有元素的宽度。
*   `dodge`: 使用色调嵌套时，元素是否要沿分类轴偏移。
*   `k_depth`: 框的数量，并通过延伸数的百分位数，来绘制。
*   `linewidth`: 构成绘图元素的灰色线条的宽度。
*   `scale`: 用于字母值框宽度的方法。
*   `outlier_prop`: 被认为是异常值的数据比例。
*   `showfliers`: 如果为假，抑制异常值的绘制。
*   `ax`: 绘制绘图的轴对象，否则使用当前轴。
*   `kwargs`: 其他关键字参数。
