...
> 原文：[https://www.geeksforgeeks.org/data-visualization-with-seaborn-line-plot/](https://www.geeksforgeeks.org/data-visualization-with-seaborn-line-plot/)
...
单线图使用连接数据点的线在 x-y 轴上显示数据。为了获得图形，Seaborn 附带了一个内置函数来绘制一个名为 `lineplot()` 的线图。

> **语法：** `lineplot(x, y, data)`
>
> 哪里，
>
> `x` – x 轴的数据变量
...
*   `font`
*   `font_size`：设置字体大小
*   `color_codes`：如果设置为真，则调色板被激活，颜色的简写符号可以从调色板中重新映射。
*   `rc`：超越上述参数的参数
