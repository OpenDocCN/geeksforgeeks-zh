...
> 原文: [https://www.geeksforgeeks.org/python-seaborn-pairgrid-method/](https://www.geeksforgeeks.org/python-seaborn-pairgrid-method/)
...
| 参数 | 描述 | 值 |
| :--- | :--- | :--- |
| `data` | 整齐的(长格式)数据框，其中每一列都是变量，每一行都是观察值。 | 数据帧 |
| `hue` | “数据”中的变量，用于将绘图方面映射到不同的颜色。 | 字符串(变量名)，可选 |
| `palette` | 用于映射“色调”变量的一组颜色。如果是字典，关键字应该是“色调”变量中的值。 | dict 或 seaborn 调色板 |
| `vars` | 要使用的“数据”中的变量，否则使用带有数字数据类型的每一列。 | 变量名列表，可选 |
| `dropna` | 绘图前删除数据中缺失的值。 | 布尔值，可选 |
