...
> 原文：[https://www.geeksforgeeks.org/how-to-make-ecdf-plot-with-seaborn-in-python/](https://www.geeksforgeeks.org/how-to-make-ecdf-plot-with-seaborn-in-python/)
...
此 ECDF 绘图和 `show()` 功能仅在 0.11.0 版或更高版本的 Seaborn 中可用。如果已经安装了 Seaborn，请编写以下命令进行升级。
...
| `y` | 进行绘图的数据或列名中的关键向量。 |
| `hue` | 确定绘图变量的颜色。 |
| `palette` | 此参数用于在映射色调时选择颜色。它可以是字符串、列表、字典。 |
| `kind` | 是 `displot()` 的参数，用来给出我们想要的那种剧情。 |
