...
> 原文：[https://www.geeksforgeeks.org/how-to-make-grouped-boxplot-with-seaborn-catplot/](https://www.geeksforgeeks.org/how-to-make-grouped-boxplot-with-seaborn-catplot/)
...
*   导入所需的包。
*   加载数据集。
*   现在使用 `catplot()` 方法，该方法在 `seaborn` 包中可用。让我们传递 `x` 和 `y` 变量，这里 `x` 轴上的变量是连续的，`y` 轴上的变量是分类的，还传递其他参数，如 `data`、`hue`、`height`、`aspect` 和 `kind="box"`。
