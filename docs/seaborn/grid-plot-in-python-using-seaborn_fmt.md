...
> 原文: [https://www.geeksforgeeks.org/grid-plot-in-python-using-seaborn/](https://www.geeksforgeeks.org/grid-plot-in-python-using-seaborn/)
...
我们可以通过调用 `map` 来映射到网格。把它画出来。在这里，我们称散点图为 `scatter`。现在所有的网格都是散点图。我们可以使用 `map` 定义网格中想要的绘图类型。
...
映射到我们称之为 `map_upper`、`map_lower` 和 `map_diag` 的上网格、下网格和对角网格。从 `PairGrid` 中移除 `map_diag`。在这里，我们可以看到对角线是 `hist` 型图，上网格是 `scatter` 型图，下网格是 `kde` 型图。
...
在这里，我们可以看到，由于在吸烟者列中有两种类型的值，即 `smoker=No` 和 `smoker=Yes`，因此这在网格中创建了两行，一行代表 `smoker=Yes`，另一行代表 `smoker=No`。对于列，由于在时间列中有两种类型的值，即 `time=Lunch` 和 `time=Dinner`，因此这在网格中创建了两列，一列代表 `time=Lunch`，另一列代表 `time=Dinner`。
...
在这里，我们将色调定义为性别，并绘制了散点图，其中 X 轴为总账单，Y 轴为小费。
