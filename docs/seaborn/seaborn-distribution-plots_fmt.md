...
Seaborn 是一个基于 Matplotlib 的 Python 数据可视化库。它提供了一个高级界面，用于绘制吸引人且信息丰富的统计图形。本文讨论了用于检验单变量和双变量分布的 Seaborn 分布图。在本文中，我们将讨论 4 种类型的分布图，即:
1.  联结图
2.  `distplot`
3.  配对图
4.  褶皱图
...
**说明:**
*   `KDE` 代表**核密度估计**，这是 Seaborn 的另一种图。
*   `bins` 用于设置您想要在绘图中使用的面元数量，它实际上取决于您的数据集。
*   `color` 用于指定绘图的颜色。
...
**说明:**
*   `kind` 是一个变量，它帮助我们处理数据可视化的问题。这有助于了解联结图内部的情况。默认值为 `scatter`，可以是 `hex`、`reg`(回归)或 `kde`。
*   `x` 和 `y` 是两个字符串，它们是列名，列包含的数据通过指定 `data` 参数来使用。
*   这里我们可以看到 y 轴上的小费和 x 轴上的总账单，以及两者之间的线性关系，这表明总账单随着小费的增加而增加。
...
它将数组中的数据点绘制成轴上的棒。就像 `distplot` 一样，它只需要一列。它不是绘制直方图，而是在整个图中创建虚线。如果你把它和 `jointplot` 进行比较，你可以看到 `jointplot` 的作用是计算破折号并显示为箱。
