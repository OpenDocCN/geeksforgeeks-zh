...
> 原文: [https://www.geeksforgeeks.org/python-seaborn-facetgrid-method/](https://www.geeksforgeeks.org/python-seaborn-facetgrid-method/)

## 先决条件: [海鸟编程基础知识](https://www.geeksforgeeks.org/plotting-graph-using-seaborn-python/)
...
`seaborn.FacetGrid` 使用许多参数作为输入，主要参数如下表所示:

| **参数** | **描述** | **值** |
| :--- | :--- | :--- |
| `data` | 整齐的(“长格式”)数据框，其中每列是一个变量，每行是一个观察值。 | 数据帧 |
| `row`、`col`、`hue` | 定义数据子集的变量，这些数据将绘制在网格中的不同面上。请参见`*_order`参数来控制此变量的级别顺序。 | 字符串 |
| `palette` | 用于`hue`变量不同级别的颜色。 | 调色板名称、列表或字典，可选 |
