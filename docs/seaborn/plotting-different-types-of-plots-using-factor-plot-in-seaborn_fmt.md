...
> 原文：[https://www.geeksforgeeks.org/plotting-different-types-of-plots-using-factor-plot-in-seaborn/](https://www.geeksforgeeks.org/plotting-different-types-of-plots-using-factor-plot-in-seaborn/)
...
`Factor Plot`用于绘制不同类型的`categorical plot`。显示的默认图是点图，但是我们可以通过使用`kind`参数绘制其他seaborn分类图，如方框图、小提琴图、条形图或条形图。
...
### 代码 4：使用 seaborn `factorplot()` 方法的箱线图
```py
# importing required library
import pandas as pd
import seaborn as sns
...
### 代码 5：使用 seaborn `factorplot()` 方法绘制带状图
```py
# importing required library
import pandas as pd
import seaborn as sns
...
### 代码 6：使用 seaborn `factorplot()` 方法计数图
```py
# importing required library
import pandas as pd
import seaborn as sns
