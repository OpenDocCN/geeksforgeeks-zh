# 带有边缘直方图的散点图，Python实现

> 原文：[https://www.geeksforgeeks.org/scatter-plot-with-marginal-histograms-in-python-with-seaborn/](https://www.geeksforgeeks.org/scatter-plot-with-marginal-histograms-in-python-with-seaborn/)

**先决条件**：[Seaborn](https://www.geeksforgeeks.org/introduction-to-seaborn-python/)
...
在Seaborn中，这可以通过`jointplot()`来实现。使用`scatterplot()`表示双变量分布，使用`histplot()`表示边际分布。
...
<seaborn.axisgrid.JointGrid at 0x...>

![](img/fd0ffdce33a3007da4ce95c0035d4eee.png)

jointplot_with_histogram

### 示例2

使用`kind="reg"`属性，可以添加线性回归拟合和单变量KDE曲线。
...
要向散点图添加条件颜色，可以使用`hue`属性，但它会在边缘轴上绘制单独的密度曲线（使用`kdeplot()`）。
