...
> 原文: [https://www.geeksforgeeks.org/grouped-boxplots-in-python-with-seaborn/](https://www.geeksforgeeks.org/grouped-boxplots-in-python-with-seaborn/)
...
示例 1: 让我们创建一个箱线图来了解`total_bill`在`tips`数据集的每个`day`上的分布。但我们也要根据`sex`属性来分组。因此，我们将绘制这三个属性的分组箱线图，其中`sex`和`day`是分类属性，`total_bill`是数字属性。
...
示例 2: 下一个示例与上一个示例类似。唯一的区别是作为`smoker`属性的`hue`参数的值。这里我们想知道`total_bill`和`day`组在一个人是否吸烟的基础上的分布。

此外，我们还学习了如何使用`boxplot()`的`palette`属性来更改 box plot 的颜色。
...
示例 3: 在本例中，我们将介绍如何绘制 2 个以上的分组箱线图。如果`hue`参数的值超过 2 个类别，那么我们可以绘制 2 个以上的分组方框图，如下所示。

这里，`hue=data['size']`有六个类别，所以我们可以用上面同样的方法看到 2 个以上的分组箱线图。
