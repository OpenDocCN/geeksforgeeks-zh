# 症状|分区。Python 中的 RGS_generalized()

> 原文: [https://www.geeksforgeeks.org/sympy-partition-rgs_generalized-in-python/](https://www.geeksforgeeks.org/sympy-partition-rgs_generalized-in-python/)

## 分区。RGS_generalized()

`RGS_generalized()` 是一个 sympy Python 库函数，它计算 **m + 1 个广义无限制增长字符串（其中 a[i] 是元素 i 出现的块的字符串）**，然后将其作为矩阵中的行返回。
