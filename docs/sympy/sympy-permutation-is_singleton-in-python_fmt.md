# Python 中的 SymPy | `Permutation.is_Singleton()`

> 原文: [https://www.geeksforgeeks.org/sympy-permutation-is_singleton-in-python/](https://www.geeksforgeeks.org/sympy-permutation-is_singleton-in-python/)

`Permutation.is_Singleton()` 是一个 SymPy Python 库函数，检查置换是否只包含一个单个元素。

### 语法
`sympy.combinatorics.permutations.Permutation.is_Singleton()`

### 返回
`true` – 如果置换只有一个元素；否则为 `false`
...
### 输出
> 排列 a – is_Singleton 形式: 假
> 排列 b – is_Singleton 形式: 真
