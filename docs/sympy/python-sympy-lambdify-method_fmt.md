...
## 参数
- `变量`–是数学表达式中的变量。
- `表达式`–是在给定的`库`中转换成各自名称的数学表达式。
- `库`–是`表达式`要转换到的Python库。

## 返回
返回一个可以计算数学表达式的`lambda`函数。

## 示例#1
在这个示例中，我们可以看到，通过使用`sympy.lambdify()`方法，我们可以从数学表达式中获得`lambda`函数。
...
## 例 2
我们可以通过`sympy`的`name:numeric_function`对的字典，对它不知道的数值库使用`lambdify`。
