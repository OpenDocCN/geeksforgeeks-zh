...
> 原文: [https://www.geeksforgeeks.org/node-js-mysql-locate-function/](https://www.geeksforgeeks.org/node-js-mysql-locate-function/)
...
`LOCATE()`函数接受三个参数，如下所述。

*   `pattern`: 要搜索的模式
*   `text`: 在这个文本模式中将被搜索
*   `starting_position` (可选): 搜索将从该位置开始。默认值为 1。

## 返回值

`LOCATE()`函数返回从特定位置搜索时文本中第一个模式出现的位置。如果出现问题，它将返回 0。
