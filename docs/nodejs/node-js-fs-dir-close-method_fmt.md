...
`fs.Dir.close()` 方法是类 `fs.Dir` 的内置应用编程接口，与 `fs`（文件系统）模块一起使用，用于异步关闭目录的底层资源句柄。
...
这个方法返回的承诺只是对回调函数 `stop()` 的一个增强。

下面的程序说明了 `fs.Dir.close()` 方法在 Node.js 中的使用。
...
**参考:** [https://nodejs.org/dist/latest-v12.x/docs/api/fs.html#fs_dir_close](https://nodejs.org/dist/latest-v12.x/docs/api/fs.html#fs_dir_close)