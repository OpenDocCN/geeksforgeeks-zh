# Node.js tlsSocket.getEphemeralKeyInfo() 方法

> 原文: [https://www.geeksforgeeks.org/node-js-tlssocket-getephemeralkeyinfo-method/](https://www.geeksforgeeks.org/node-js-tlssocket-getephemeralkeyinfo-method/)

`tlsSocket.getEphemeralKeyInfo()` 是 TLS 模块中类 `tlsSocket` 的内置应用程序编程接口，用于返回包含临时密钥交换信息的对象。

**语法:**

```javascript
const tlsSocket.getEphemeralKeyInfo()
```

**参数:** 此方法不接受任何参数。

**返回值:** 这个方法返回一个包含临时密钥交换信息的对象。

## 如何生成私钥和公钥证书？

1.  **私钥:** 打开记事本复制粘贴以下密钥:

```text
-----BEGIN RSA PRIVATE KEY-----

nUG9lYvH7DztWDTioXMrruPF7bdXfZOVJD8t0I4OUzvC
-----END RSA PRIVATE KEY-----
```

2.  将文件另存为 `private-key.pem`。
3.  **公共证书:** 打开记事本，复制粘贴以下密钥:

```text
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
```

4.  将文件保存为 `public-cert.pem`。

## 示例 1: 文件名 `index.js`

```javascript
// Node.js program to demonstrate the
// tlsSocket.getEphemeralKeyInfo() method

var tls = require('tls'),
    fs = require('fs'),

// Port and host address for server   
PORT = 1337,
HOST = '127.0.0.1',
value =null;

// Private key and public certificate for access
var options = {
  key: fs.readFileSync('private-key.pem'),
  cert: fs.readFileSync('public-cert.pem'),
  rejectUnauthorized: false
};

// Creating and initializing server
var server = tls.createServer(options,
        function(socket) {

// Print the data that we received
  socket.on('data', function(data) {
     console.log('\nReceived: %s ',
     data.toString().replace(/(\n)/gm, ""));
  });

// Stopping the server
  // by using the close() method
  server.close(() => {
     console.log("Server closed successfully");
  });
});

// Start listening on a specific port and address
// by using listen() method
server.listen(PORT, HOST, function() {
  console.log("I'm listening at %s, "
        + "on port %s", HOST, PORT);
});

// Creating and initializing client
var client = tls.connect(PORT,
    HOST, options, function() {

// Getting object containing information
  // regarding EphemeralKey by using
  // tlsSocket.getEphemeralKeyInfo() method
  value = client.getEphemeralKeyInfo();

client.write("EphemeralKey : " + value.name);

client.end(() => {
    console.log("Client closed successfully");
  });
});
```

使用以下命令运行 `index.js` 文件:

```bash
node index.js
```

**输出:**

```text
I'm listening at 127.0.0.1, on port 1337
Client closed successfully

Received: EphemeralKey : X25519
Server closed successfully
```

## 示例 2: 文件名 `index.js`

```javascript
// Node.js program to demonstrate the
// tlsSocket.getEphemeralKeyInfo()  API
var tls = require('tls'),
    fs = require('fs'),

// Port and host address for server   
PORT = 1337,
HOST = '127.0.0.1';

// Private key and public certificate for access
var options = {
  key: fs.readFileSync('private-key.pem'),
  cert: fs.readFileSync('public-cert.pem'),
  rejectUnauthorized: false
};

// Creating and initializing server
var server = tls.createServer(options,
                   function(socket) {

// Getting object containing information
  // regarding EphemeralKey by using
  // tlsSocket.getEphemeralKeyInfo() method
  value = socket.getEphemeralKeyInfo();

socket.write("EphemeralKey : " + value);

// Stopping the server
  // by using the close() method
  server.close(() => {
    console.log("Server closed successfully")
  });
});

// Start listening on a specific port
// and address by using listen() method
server.listen(PORT, HOST, function() {
  console.log("I'm listening at %s, "
        + "on port %s", HOST, PORT);
});

// Creating and initializing client
var client = tls.connect(PORT,
     HOST, options, function() {
   console.log("client is connected");
});

client.on("data", function(data) {

console.log('Received: %s',
  data.toString().replace(/(\n)/gm, ""));

// Close the connection after
  // receiving the message
  client.end(() => {
     console.log("client closed successfully")
  });
});
```

使用以下命令运行 `index.js` 文件:

```bash
node index.js
```

**输出:**

```text
I'm listening at 127.0.0.1, on port 1337
client is connected
Received: EphemeralKey : null
client closed successfully
Server closed successfully
```

**参考:** [https://nodejs.org/dist/latest-v12.x/docs/api/tls.html#tls_tlssocket_getephemeralkeyinfo](https://nodejs.org/dist/latest-v12.x/docs/api/tls.html#tls_tlssocket_getephemeralkeyinfo)