...
> 原文: [https://www.geeksforgeeks.org/node-js-tls-createserver-method/](https://www.geeksforgeeks.org/node-js-tls-createserver-method/)

`tls.createServer()` 是 `tls` 模块中类 `tls` 的内置应用程序编程接口，用于创建 TLS 服务器对象。
...
*   `options`: 可以根据需要使用 `enableTrace`、`host`、`port`、`path`、`socket`、`allowHalfOpen`、`rejectUnauthorized`、`pskCallback`、`ALPNProtocols` 等属性选项。
*   `secureConnectionListener`: 是控制服务器实例的功能。

## 返回值

该方法返回 `tls.Server` 的对象。
...
[https://nodejs.org/dist/latest-v12.x/docs/api/tls.html#tls_tls_createserver_options_secureconnectionlistener](https://nodejs.org/dist/latest-v12.x/docs/api/tls.html#tls_tls_createserver_options_secureconnectionlistener)