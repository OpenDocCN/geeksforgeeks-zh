...
> 原文: [https://www.geeksforgeeks.org/node-js-buffer-readuint32le-method/](https://www.geeksforgeeks.org/node-js-buffer-readuint32le-method/)

`Buffer.readUInt32LE()`方法是`Buffer`模块中类`Buffer`的内置应用编程接口，用于以小端格式从指定偏移量的已分配缓冲区中读取32位值。
...
下面的例子说明了在Node.js中使用`Buffer.readUInt32LE()`方法：
...
**参考:** [https://nodejs.org/api/buffer.html#buffer_buf_readuint32le_offset](https://nodejs.org/api/buffer.html#buffer_buf_readuint32le_offset)