# 如何在 Node.js 中整合 Express 速率限制？

> 原文：[https://www.geeksforgeeks.org/how-to-integrate-express-rate-limit-in-node-js/](https://www.geeksforgeeks.org/how-to-integrate-express-rate-limit-in-node-js/)

速率限制防止同一个 IP 发出太多请求，这将有助于我们防止暴力攻击。`express-rate-limit` 是一个用于限制用户请求的 npm 包。

## 项目设置

运行以下几组命令创建一个文件夹并初始化项目。
...
运行以下命令安装 `express` 和 `express-rate-limit` 模块：
