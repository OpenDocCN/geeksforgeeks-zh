...
`console.table()`方法是控制台模块的内置应用编程接口，用于将由其参数构建的表打印到控制台中。
...
2.  `properties`: Specify the attribute of the construction table.

## 返回值

这个方法不返回任何东西，只打印构造好的表并记录下来。如果它无法将参数解析到表中，那么它只是记录参数。
...
**参考:** [https://nodejs.org/api/console.html#console_console_table_tabulardata_properties](https://nodejs.org/api/console.html#console_console_table_tabulardata_properties)