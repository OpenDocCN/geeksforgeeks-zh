...
> 原文: [https://www.geeksforgeeks.org/node-js-fspromises-mkdtemp-method/](https://www.geeksforgeeks.org/node-js-fspromises-mkdtemp-method/)
...
*   `prefix`: 是一个表示文件路径的字符串。
*   `option`: 是一个字符串或一个对象。
    *   `encoding`: 是一个字符串，默认是 `utf8`。

## 返回值

它返回 *promise* 对象，该对象表示异步操作的最终完成(或失败)及其结果值。
...
可选选项参数可以是指定编码的字符串，也可以是具有指定要使用的字符编码的 `encoding` 属性的对象。
