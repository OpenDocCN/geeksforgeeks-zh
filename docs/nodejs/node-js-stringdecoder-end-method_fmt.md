...
> 原文：`https://www.geeksforgeeks.org/node-js-stringdecoder-end-method/`

`stringDecoder.end()`方法用于将存储在内部缓冲区中的所有剩余输入作为字符串返回。此方法确保任何不完整的 UTF-8 和 UTF-16 字符被替换为适合字符编码的替换字符。
...
*   `buffer`：它是一个包含必须解码的字节的缓冲区、类型数据区或数据视图。这是一个可选参数。
