...
Cookies 是存储在客户端的小数据，与服务器请求一起发送给客户端。Cookies 有各种各样的功能，它们可以用于维护会话和在你的 web 应用程序中添加用户特定的功能。为此，我们将使用 npm 的 `cookie-parser` 模块，该模块为 cookie 解析提供中间件。
...
如果我们有多个对象被推入 cookies，那么我们可以使用 `req.cookies.cookie_name` 访问特定的 cookie。
...
要销毁 cookie，请向以下链接发出获取请求：`localhost:3000/logout`
![](https://media.geeksforgeeks.org/wp-content/uploads/Screenshot-39-4.png)

要检查 cookie 是否被销毁，请向 `localhost:3000/getuser` 发出 get 请求，您将获得一个空的用户 cookie 对象。
![](img/bded5aa432826bddd68e1f88293aab17.png)

这是关于使用 `cookie-parser` 中间件的 HTTP cookies 的基本使用。Cookies 可以用于许多方面，例如维护会话，并根据每个用户以前在网站上的交易为他们提供不同的网站视图。