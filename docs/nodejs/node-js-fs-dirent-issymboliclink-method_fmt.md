...
> 原文: [`https://www.geeksforgeeks.org/node-js-fs-dirent-issymboliclink-method/`](https://www.geeksforgeeks.org/node-js-fs-dirent-issymboliclink-method/)

`fs.Dirent.isSymbolicLink()`方法是`fs`模块中`Dirent`类的内置应用编程接口，用于检查特定目录项是否描述了符号链接。
...
### 返回值:
如果特定目录项描述了一个符号链接，则该方法返回`true`，否则返回`false`。

下面的程序说明了`fs.Dirent.isSymbolicLink()`方法在Node.js中的使用。
...
**参考:** [`https://nodejs.org/dist/latest-v12.x/docs/api/fs.html#fs_dirent_issymboliclink`](https://nodejs.org/dist/latest-v12.x/docs/api/fs.html#fs_dirent_issymboliclink)