# Node.js NPM arraybuffer-to-string 模块

> 原文: [https://www.geeksforgeeks.org/node-js-npm-arraybuffer-to-string-module/](https://www.geeksforgeeks.org/node-js-npm-arraybuffer-to-string-module/)

NPM (Node Package Manager) 是 Node.js 包的包管理器。有一个名为 `arraybuffer-to-string` 的 NPM 包，用于将 `ArrayBuffer` 解码为实际的字符串。该包不仅将缓冲区转换为 `utf8` 字符串，还可以转换为多种形式，例如 Base64 编码字符串和十六进制编码字符串，这些在许多上下文中都会用到。
...
## 将 ArrayBuffer 转换为字符串的语法

```javascript
arrayBufferToString(buffer, encodingType)
```

**参数：** 需要两个参数 `buffer` 和 `encodingType`，我们要将 `ArrayBuffer` 转换为字符串，如下所示：

*   **`buffer`：** 是我们要转换为实际文本的 `ArrayBuffer`。Node.js 中有很多方法不直接返回字符串的实际内容，而是返回响应的缓冲形式。在这些上下文中，我们使用这个包将缓冲区转换为字符串。
*   **`encodingType`：** 为可选参数，默认值为 `'utf8'`。它表示我们要将缓冲区转换成的字符串的类型。可用的编码有 `utf8`、`binary`、`base64`、`hex`、`ascii`、`latin1`、`ucs2`、`utf16` 和许多其他编码。

## 示例 1

此示例说明了如何使用 `arraybuffer-to-string` 从 `ArrayBuffer` 转换为 `utf8` 字符串。
...
提交输入字符串，将其从字符串转换为 `ArrayBuffer`，再转换为 `utf8` 编码的字符串。
...
此示例说明如何使用 `arraybuffer-to-string` 从 `ArrayBuffer` 转换为 `base64` 编码字符串。
...
提交输入字符串，将其从字符串转换为 `ArrayBuffer`，再转换为 `base64` 编码的字符串。
