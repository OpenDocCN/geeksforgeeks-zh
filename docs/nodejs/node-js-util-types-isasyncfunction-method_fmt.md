...
> 原文：[https://www.geeksforgeeks.org/node-js-util-types-isasyncfunction-method/](https://www.geeksforgeeks.org/node-js-util-types-isasyncfunction-method/)

`util.types.isAsyncFunction()` 方法是 `util` 模块的一个内置 API，用于对 Node.js 中的异步函数进行类型检查。
...
它返回一个布尔值，如果从 JavaScript 引擎的角度来看，该值是异步函数，则为 `true`，否则为 `false`。
...
**参考：** [https://nodejs.org/api/util.html#util_util_types_isasyncfunction_value](https://nodejs.org/api/util.html#util_util_types_isasyncfunction_value)