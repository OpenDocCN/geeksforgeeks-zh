...
箭头函数不绑定自己的 `this` 值。相反，定义它的作用域的 `this` 值是可访问的。这使得箭头函数成为方法的不良候选者，因为 `this` 不会是对定义该方法的对象的引用。

对于方法，ES6 提供了一种新的方法定义语法。您可以在下面 `printGuestList` 方法的定义中看到这一点。该函数是一个标准函数，只是有一个允许删除冒号和函数关键字的简写语法。

因为箭头函数不绑定 `this`，所以除了方法之外，它们可以很好地工作。如下所示，传递给 `forEach` 的箭头函数能够正确地访问 `this.name`，因为它被定义为箭头函数，并且没有自己的绑定。如果你把箭头函数换成标准函数，这段代码就不能用了。
