...
`os.tmpdir()`方法是`os`模块内置的应用编程接口，用于获取操作系统临时文件的默认目录路径。
...
该方法返回一个字符串，指定操作系统临时文件默认目录的路径。

下面的例子说明了`os.tmpdir()`方法在Node.js中的使用：

## 例1

```js
// Node.js program to demonstrate the os.tmpdir() method
