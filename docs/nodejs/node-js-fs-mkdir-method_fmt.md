...
`fs.mkdir()`方法在 Node.js 中用于异步创建目录。
...
*   `path`: 该参数保存需要创建的目录的路径。
*   `mode`: 该参数保存递归布尔值。模式选项用于设置目录权限，默认为 `0777`。
*   `callback`: 此参数保存包含错误的回调函数。如果要创建的目录已经存在，递归选项如果设置为 `true` 将不会给出错误消息。

下面的例子说明了 `fs.mkdir()`方法在 Node.js 中的使用:
