...
> 原文：[https://www.geeksforgeeks.org/what-is-file-system-module-in-node-js/](https://www.geeksforgeeks.org/what-is-file-system-module-in-node-js/)

[Node.js](https://www.geeksforgeeks.org/nodejs-tutorials/) 是一个开源的 JavaScript 运行时，在浏览器之外运行 JavaScript。它是后端开发的流行选择。像其他编程语言一样，Node.js 也为我们提供了与系统或机器交互的内置库。在 Node.js 中，我们称这些库为模块。
...
文件系统是 Node.js 用来与文件和文件夹交互的核心模块。它提供了各种事件和方法来访问和操作我们机器中的文件和文件夹。
...
读取文件最简单的方法就是使用 [`fs.readFile()`](https://www.geeksforgeeks.org/node-js-fs-readfile-method/) 方法。这个方法负责打开和关闭文件，并将文件内容加载到内存中，供我们在程序中使用。
...
*   `path` – 我们要读取的文件的名称或路径
*   `options` – 这是一个可选参数，通常我们传递的编码是 `"utf-8"`
*   `callback` – 在我们读取文件后执行。它需要两个参数
    *   `error` – 如果出现任何错误
    *   `data` – 文件内容

#### 示例

创建一个文件名 `app.js` 并创建一个文本文件 `input.txt`。
...
我们将使用同步版本，即 [`fs.readFileSync()`](https://www.geeksforgeeks.org/node-js-fs-readfilesync-method/)。下面讨论的其他方法也有一个同步版本。要使用它，只需在函数名的末尾添加 `Sync`，并删除回调参数。
...
我们可以使用 [`fs.writeFile()`](https://www.geeksforgeeks.org/node-js-fs-writefile-method/#:~:text=The%20fs.,the%20functionality%20of%20the%20method.) 函数在文件中写入。该函数通过替换文件的现有内容将数据写入文件。此外，如果没有这样的文件，那么它会创建该文件并在其上写入数据。
...
创建一个空文本文件 – `output.txt` 以及 `app.js` 文件。
...
我们可以使用 [`fs.appendFile()`](https://www.geeksforgeeks.org/node-js-fs-appendfile-function/) 将数据追加到文件中。与 `fs.writeFile()` 不同，`fs.appendFile()` 不会替换文件的内容，而是将数据添加到其中。如果找不到该文件，则创建该文件并将数据添加到其中。
...
*   `path` – 文件的名称或路径。
*   `data` – 要添加到文件中的内容。
*   `options` – 我们通常指定编码 – `'utf-8'`
*   `callback` – 在我们追加文件后执行
    *   `error` – 如果有错误

#### 示例

创建一个文件 `app.js` 和一个空的 `output.txt` 文件。
...
`fs` 模块还有很多其他的方法用来对文件进行各种类型的操作 – [`fs.rename()`](https://www.geeksforgeeks.org/node-js-fs-rename-method/)、 [`fs.unlink()`](https://www.geeksforgeeks.org/node-js-fs-unlink-method/) 、 [`fs.open()`](https://www.geeksforgeeks.org/node-js-fs-open-method/)、 [`fs.close()`](https://www.geeksforgeeks.org/node-js-fs-close-method/) 等。

## 让我们使用文件系统模块创建一个离线裁判项目

一个离线判官的演示：假设你有一个 JSON 文件 `testcases.json` 包含一个问题的所有输入测试用例及其输出。
