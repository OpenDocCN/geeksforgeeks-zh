...
> 原文: [https://www.geeksforgeeks.org/node-js-assert-rejects-function/](https://www.geeksforgeeks.org/node-js-assert-rejects-function/)
...
* `error`: 该参数的类型可以是类、正则表达式、验证函数或每个属性将被测试的对象。这是一个可选参数。
* `message`: 如果`asyncFn`拒绝失败，该参数将是`AssertionError`提供的消息。这是一个可选参数。
...
## 参考:
[https://nodejs.org/dist/latest-v12.x/docs/api/assert.html#assert_assert_rejects_asyncfn_error_message](https://nodejs.org/dist/latest-v12.x/docs/api/assert.html#assert_assert_rejects_asyncfn_error_message)