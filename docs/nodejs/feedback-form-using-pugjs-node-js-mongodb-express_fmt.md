...
> 原文：[https://www.geeksforgeeks.org/feedback-form-using-pugjs-node-js-mongodb-express/](https://www.geeksforgeeks.org/feedback-form-using-pugjs-node-js-mongodb-express/)

我们可以使用 PugJs、NodeJS、MongoDB、Express 创建一个反馈表单，步骤如下。反馈表单接受用户的输入，并将数据发送到服务器进行处理，并相应地将其存储到数据库中。

## 步骤 1：创建项目文件夹
在任意目录下创建一个名为`反馈_表单`的文件夹。你可以取任何名字。
...
*   `static`：它包含你的服务器的静态文件。
*   `views`：它包含要渲染到客户端的 PugJS 文件。
