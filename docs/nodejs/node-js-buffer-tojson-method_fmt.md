...
**注:** `JSON.stringify()`是也可以用来返回 JSON 格式数据的方法。当我们调用`JSON.stringify()`方法时，它直接在后台调用`buffer.toJSON()`方法。
...
**参考:** [https://nodejs.org/api/buffer.html#buffer_buf_tojson](https://nodejs.org/api/buffer.html#buffer_buf_tojson)