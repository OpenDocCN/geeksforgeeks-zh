...
*   一个 `Console` 类，可用于写入任何 Node.js 流。例如: `console.log()`、`console.error()` 等。
*   无需导入 `console` 即可使用的全局控制台。例如: `process.stdout`、`process.stderr` 等。

`new Console()` (在 v8.0.0 中添加) 方法是一个内置的 `Console` 模块的应用程序编程接口，它创建了一个具有单个或多个可写流实例的新 `Console`，其中 `stdout` 是可写流，`stderr` 用于 `warn` 或 `error` 输出。如果没有提供 `stderr`，则 `stderr` 使用 `stdout`。它是一个控制台，其输出被发送到 `process.stdout` 和 `process.stderr`。
...
该功能接受如上所述和如下所述的对象/参数列表:

**`options`** < `Object` >: 它在 `options` 对象中可能有以下元素。

*   **`stdout`** < `stream.Writable` >: 接受从 `fs` 模块导入的写流。
*   **`stderr`** < `stream.Writable` >: 也接受从 `fs` 模块导入的写流。
*   **`ignoreErrors`** < `boolean` >: 传递的默认值为 `true`。它会忽略写入底层流时的错误。
*   **`colorMode`** < `boolean` > | < `string` >: 传递的默认值为 `"auto"`。它用于设置仅支持此控制台实例的颜色。根据设置的颜色模式，它可以设置为 `true`、`false` 或 `"auto"`。
*   **`inspectOptions`** < `Object` >: 它指定传递给 [`util.inspect()`](https://www.geeksforgeeks.org-node-js-util-inspect-method/) 方法的选项。
*   **`groupIndentation`** < `number` >: 默认值设置为 2。用于设置组缩进。

**返回值:** 其输出通过 `stream.Writable` 发送到由 `fs` 模块创建的 `process.stdout` 和 `process.stderr` 文件。

下面的例子说明了在 Node.js 中使用 `new Console(options)` 方法。
...
> **注意:** 上面的 node.js 示例将在 `index.js` 文件所在的同一个文件夹中创建日志文件(`output` & `error`)。
>
...
**参考:** [https://nodejs.org/api/console.html#console_new_console_options](https://nodejs.org/api/console.html#console_new_console_options)