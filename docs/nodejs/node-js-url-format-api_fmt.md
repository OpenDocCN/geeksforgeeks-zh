...
> 原文：`https://www.geeksforgeeks.org/node-js-url-format-api/`
...
*   `fragment` 如果为真，则应该包含片段，否则不包含。
*   `search` 如果为真，则提供搜索查询，否则不提供。
...
在本例中，我们首先在节点中导入`url`模块。然后为了生成或格式化随机网址，我们使用`url.format()`方法。
