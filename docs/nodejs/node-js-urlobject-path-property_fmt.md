...
> 原文: [https://www.geeksforgeeks.org/node-js-urlobject-path-property/](https://www.geeksforgeeks.org/node-js-urlobject-path-property/)
...
让我们考虑一个 `'http://user:pass@sub.example.com:8080/p/a/t/h?query=string#hash'` 的网址。
...
`urlObject.path` 返回由 `url.parse()` 函数返回后的 `'/p/a/t/h?query=string'`。
