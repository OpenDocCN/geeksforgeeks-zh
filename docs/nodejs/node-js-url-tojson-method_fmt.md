...
Node.js `url`模块中的`url.toJSON()`方法用于返回URL对象的序列化URL。此方法的返回值等效于`URL.href`和`url.toString()`方法。如果一个URL对象是使用`JSON.stringify()`方法序列化的，那么它将被自动调用。
...
下面的例子说明了Node.js中的`url.toJSON()`方法：
...
**参考:** [https://nodejs.org/api/url.html#url_url_tojson](https://nodejs.org/api/url.html#url_url_tojson)