...
> 原文：`https://www.geeksforgeeks.org/node-js-http-clientrequest-settimeout-method/`
...
**参数：** 该方法以 `timeout` 值为参数，单位为毫秒，第二个参数是给定超时值后执行的 `callback` 函数。
...
**参考：** `https://nodejs.org/dist/latest-v12.x/docs/api/http.html#http_request_settimeout_timeout_callback`