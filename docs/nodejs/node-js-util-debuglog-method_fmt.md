...
*   `section` <`字符串`>: 它接受一个<`字符串`>，标识正在为其创建 debuglog 函数的应用程序部分。
*   `callback` <`函数`>: 接受第一次调用日志记录函数时调用的回调，函数参数为更优化的日志记录函数。
...
**参考:** [https://nodejs.org/api/util.html#util_util_debuglog_section_callback](https://nodejs.org/api/util.html#util_util_debuglog_section_callback)