...
> 原文: [https://www.geeksforgeeks.org/node-js-response-write-method/](https://www.geeksforgeeks.org/node-js-response-write-method/)
...
*   `chunk` <`string`> | <`Buffer`>：它接受任何缓冲区或字符串数据。
*   `encoding` <`string`>：默认编码设置为 `utf8`。它接受字符串数据。
*   `callback` <`Function`>：接受回调功能。

## 返回值

<boolean>：如果整个数据被成功刷新到内核缓冲区，则返回 `true`，如果全部或部分数据在用户内存中排队，则返回 `false`。当缓冲器再次空闲时，将发出 `drain`。
...
**参考:** [https://nodejs.org/api/http.html#http_response_write_chunk_encoding_callback](https://nodejs.org/api/http.html#http_response_write_chunk_encoding_callback)