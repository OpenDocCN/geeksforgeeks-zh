...
> 原文：[https://www.geeksforgeeks.org/how-to-add-two-numbers-in-console-using-node-js/](https://www.geeksforgeeks.org/how-to-add-two-numbers-in-console-using-node-js/)

在本文中，我们将看到如何使用 NodeJS 在控制台中添加两个数字。为此，我们需要了解一个名为`提示`的节点包。它有助于从控制台获取用户输入。我们将使用它的方法`prompt.get()`来获取用户输入。
...
这就是如何在控制台中使用 nodeJs 添加两个数字。`提示`帮助我们接受用户的输入。它还支持输入的验证和默认值。