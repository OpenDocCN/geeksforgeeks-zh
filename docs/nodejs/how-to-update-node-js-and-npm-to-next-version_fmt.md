...
> 原文：[https://www.geeksforgeeks.org/how-to-update-node-js-and-npm-to-next-version/](https://www.geeksforgeeks.org/how-to-update-node-js-and-npm-to-next-version/)
...
`Node.js` 可以从官方 [Node.js](https://nodejs.org/en/download/) 网站更新，也可以使用 `Node Version Manager` (`nvm`) 通过命令行更新。`nvm` 最初是为 `Linux` 系统开发的，但是可以通过以下步骤为 `Windows` 系统单独安装 `nvm`：

1.  去这个[站点](https://github.com/coreybutler/nvm-windows/releases)
2.  安装并解压缩 `nvm-setup.zip` 文件
3.  从 `cmd` 输入 `nvm -v` 确保安装了 `nvm`。
...
## 在 Linux 系统中更新 Node.js 和 npm

下面是为 `Linux` 系统更新 `Node.js` 和 `npm` 版本的演示。
