...
`Buffer.readUInt16LE()`方法是 `Buffer` 模块中类 `Buffer` 的内置应用编程接口，用于以指定的小端格式从指定偏移量的缓冲区中读取无符号 16 位整数。
...
## 参数:
该方法接受单参数`offset`，表示开始从缓冲区读取之前要跳过的字节数。偏移量可以在范围 `0 <= offset <= buf.length - 2`内。偏移量的默认值为 0。
...
## 参考:
[https://nodejs . org/dist/latest-v 13 . x/docs/API/buffer . html # buffer _ buf _ readuint 16 le _ offset](https://nodejs.org/dist/latest-v13.x/docs/api/buffer.html#buffer_buf_readuint16le_offset)