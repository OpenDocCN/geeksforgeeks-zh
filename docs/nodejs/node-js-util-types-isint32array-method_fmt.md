...
> 原文：[https://www.geeksforgeeks.org/node-js-util-types-isint32array-method/](https://www.geeksforgeeks.org/node-js-util-types-isint32array-method/)

`util`模块的`util.types.isInt32Array()`方法主要是为了支持Node.js自身内部API的需求而设计的。它用于检查方法中传递的实例是否是内置的`Int32Array`实例。
...
**参数：** 该方法接受单个参数`value`，该值保存任何值，即任何模块的实例。

**返回值：** 该方法返回一个布尔值，即如果传递的值是`Int32Array`的实例，则返回`true`，否则返回`false`。

下面的例子说明了在Node.js中`util.types.isInt32Array()`方法的使用：
...
**参考：** [https://nodejs.org/api/util.html#util_util_types_isint32array_value](https://nodejs.org/api/util.html#util_util_types_isint32array_value)