...
> 原文: [https://www.geeksforgeeks.org/nodejs-process-nexttick-method/](https://www.geeksforgeeks.org/nodejs-process-nexttick-method/)

`process`对象是 NodeJS 核心 API 提供的少数全局对象之一。它可以从任何地方访问，因此它的方法也可以被访问。这就是一种叫做 `process.nextTick()` 的方法，实时应用中的开发人员每天都在使用这种方法，将函数的执行推迟到下一次事件循环迭代。
