...
`fs.readlink()`方法是`fs`模块的内置应用编程接口，用于异步返回符号链接的值，即链接到的路径。可选参数可用于指定链接路径的字符编码。
...
*   `path`: 它是一个字符串、缓冲区或网址，代表符号链接的路径。
*   `options`: 它是一个对象或字符串，可用于指定将影响输出的可选参数。它有一个可选参数:
    *   `encoding`: 是一个字符串值，指定返回链接路径的字符编码。默认值为`"utf8"`。
*   `callback`: 是执行方法时会调用的函数。
    *   `err`: 这是一个如果方法失败会抛出的错误。
    *   `linkString`: 它是包含符号链接字符串值的字符串或缓冲区对象。
...
**参考:** [https://nodejs.org/api/fs.html#fs_fs_readlink_path_options_callback](https://nodejs.org/api/fs.html#fs_fs_readlink_path_options_callback)