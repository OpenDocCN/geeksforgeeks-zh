...
> 原文: [https://www.geeksforgeeks.org/node-js-buffer-writeint16le-method/](https://www.geeksforgeeks.org/node-js-buffer-writeint16le-method/)

`Buffer.writeInt16LE()`方法是`Buffer`模块中类`Buffer`的内置应用编程接口，用于以指定的小端格式将有效的带符号 16 位整数写入指定偏移量处的缓冲区。
...
*   `value`: 要写入缓冲区的有效带符号 16 位整数。
*   `offset`: 表示在写入缓冲区之前要跳过的字节数。`offset`值的范围是 **0 <= offset <= buf.length–2**。`offset`的默认值为 0。

## 返回值:

以小端格式返回一个在指定偏移量插入值的缓冲区。
...
**参考:** [https://nodejs.org/dist/latest-v13.x/docs/api/buffer.html#buffer_buf_writeint16le_value_offset](https://nodejs.org/dist/latest-v13.x/docs/api/buffer.html#buffer_buf_writeint16le_value_offset)