# Node.js 中何时使用 `next()` 与 `return next()`？

> 原文：[https://www.geeksforgeeks.org/when-to-use-next-and-return-next-in-node-js/](https://www.geeksforgeeks.org/when-to-use-next-and-return-next-in-node-js/)
...
在函数 `app.use((req, res, next))` 中，我们有三个回调，即 `req`、`res` 和 `next`。
