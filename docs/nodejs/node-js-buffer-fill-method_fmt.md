...
`Buffer.fill()`方法将数据放入缓冲实例。如果没有给出偏移量和结束值，则整个缓冲区将被填满。
...
下面的例子说明了 `Buffer.fill()`方法在 Node.js 中的使用：
...
[https://nodejs.org/api/buffer.html#buffer_buf_fill_value_offset_end_encoding](https://nodejs.org/api/buffer.html#buffer_buf_fill_value_offset_end_encoding)