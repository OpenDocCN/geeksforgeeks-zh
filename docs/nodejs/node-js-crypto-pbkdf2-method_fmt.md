...
`crypto.pbkdf2()`方法给出了一个异步的**基于密码的密钥派生函数 2**，即(pbkdf2)实现。此外，由摘要定义的特定 HMAC 摘要算法被实现为从所述密码、salt 和迭代中导出所需字节长度的密钥(`keylen`)。
...
下面的例子说明了 `crypto.pbkdf2()`方法在 Node.js 中的使用:
