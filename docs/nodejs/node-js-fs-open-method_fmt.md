...
## 简介
创建文件、写入文件或读取文件使用 `fs.open()` 方法。`fs.readFile()` 仅用于读取文件，同样 `fs.writeFile()` 仅用于写入文件，而 `fs.open()` 方法对一个文件进行若干操作。首先我们需要加载 `fs` 类，它是访问物理文件系统的模块。为此要求采用方法。例如：`var fs = require('fs');`
