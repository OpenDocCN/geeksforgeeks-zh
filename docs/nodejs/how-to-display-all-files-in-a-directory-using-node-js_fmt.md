...
> 原文：[https://www.geeksforgeeks.org/how-to-display-all-files-in-a-directory-using-node-js/](https://www.geeksforgeeks.org/how-to-display-all-files-in-a-directory-using-node-js/)

目录中的文件可以使用 Node.js 中的两种方法来显示，这两种方法将在下面讨论：

## 方法 1：使用 `fs.readdirSync()` 方法

`fs.readdirSync()` 是 Node.js 的文件系统模块中提供的一种方法，用于读取目录的内容。它返回文件路径、缓冲区或文件系统的数组。

可以使用 `forEach()` 循环来循环返回的文件数组，并从中显示文件名。这样可以显示目录中的所有文件名。

方法 `fs.readdir()` 的异步变体也可以用来代替这个用文件而不是文件本身返回回调的函数。
...
## 方法 2：使用 `fs.opendirSync()` 方法

Node.js 的文件系统模块中有 `fs.opendirSync()` 方法，可以用来读取目录的内容。它返回一个代表给定目录的 `fs.Dir` 对象。
