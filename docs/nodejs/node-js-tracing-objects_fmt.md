...
### `tracing.categories`（在 v10.0.0 中添加）

对象是 `trace_events` 模块的内置应用程序编程接口，该接口返回 trace 事件类别，该类别是该 Tracing 对象所覆盖的逗号分隔列表。
...
### `tracing.disable()`（在 v10.0.0 中增加）

对象是 `trace_events` 模块的内置应用编程接口，用于禁用之前通过 `tracing.enable()` 方法启用的 Tracing 对象。
...
### `tracing.enable()`（在 v10.0.0 中添加）

对象是 `trace_events` 模块的内置应用程序编程接口，该接口为之前通过 `tracing.enable()` 方法启用的类别集启用 Tracing 对象。
...
### `tracing.enabled`（在 v10.0.0 中添加）

此对象跟踪 trace 事件是启用还是禁用，并相应地返回布尔值。
...
在谷歌 Chrome 中输入 URL `chrome://tracing`。现在，单击加载按钮，加载文件进行跟踪。

**参考：** [https://nodejs.org/api/tracing.html#tracing_tracing_object](https://nodejs.org/api/tracing.html#tracing_tracing_object)