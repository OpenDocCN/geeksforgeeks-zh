...
`os.arch()`方法是`os`模块的内置应用编程接口，用于获取当前 Node.js 所编译的计算机的 CPU 架构。
...
该方法返回当前 Node.js 编译所针对的操作系统 CPU 架构。可能的值为 `"x32"`、`"x64"`、`"arm"`、`"arm64"`、`"s390"`、`"s390x"`、`"mipsel"`、`"ia32"`、`"mips"`、`"ppc"` 和 `"ppc64"`。

下面的例子说明了`os.arch()`方法在 Node.js 中的使用。
...
**参考:** [https://nodejs.org/api/os.html#os_os_arch](https://nodejs.org/api/os.html#os_os_arch)