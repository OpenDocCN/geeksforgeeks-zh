...
*   `text`: 是搜索模式的给定文本。
*   `pattern`: 是用户想要在文本中搜索的模式。

## 返回值

返回文本中第一次出现模式的位置号。如果没有找到匹配项，它将返回 0。
...
db_con.connect((err) => {
    ...
    // Here is the query
    let query = "SELECT INSTR('GeeksForGeeks', 'geek') AS position";
...
db_con.connect((err) => {
    ...
    // Here is the query
    let query = "SELECT INSTR(name, 'n') AS position FROM publishers";
