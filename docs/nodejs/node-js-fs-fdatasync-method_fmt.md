...
*   **`fd`** <`整数`>：此参数接受<`整数`>类型值。
*   **`callback`** <`函数`>：此参数需要一个回调函数，谨防嵌套回调或回调地狱。
    *   **`err`** <`错误`>：如果回调函数处理不当，抛出错误。
...
[https://nodejs.org/api/fs.html#fs_fs_fdatasync_fd_callback](https://nodejs.org/api/fs.html#fs_fs_fdatasync_fd_callback)
