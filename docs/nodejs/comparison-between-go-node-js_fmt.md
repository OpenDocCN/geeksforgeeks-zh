...
| 03 | Go 的错误处理对开发者来说不是问题，因为它实现了显式的错误检查。 | NodeJS 中的错误处理更容易，因为它使用了抛出-捕获（throw-and-catch）错误处理的概念。 |
| 04 | Go 语言是一门相对较新的语言，所以开发者只需要对概念做少量研究，阅读后就能清楚地理解。 | 因为开发者非常了解 JavaScript，所以开始使用 NodeJS 对他们来说并不困难。JS 的基础知识将有助于清晰地理解 NodeJS。 |
| 05 | Go 语言中可以实现并发。 | NodeJS 无法实现并发。 |
| 06 | Go 语言的可扩展性比 NodeJS 更强大。 | 与 GO 语言相比，NodeJS 的可扩展性较差。 |
| 07 | 与 NodeJS 相比，供开发者使用的工具较少。 | 与 Golang 相比，有更多供开发者使用的工具、框架和库。 |
| 08 | Uber、Midsize、Intel、Google、Heroku 等许多公司都使用 Go 语言。 | Netflix、LinkedIn、PayPal、Walmart、eBay 等许多公司使用 NodeJS。 |