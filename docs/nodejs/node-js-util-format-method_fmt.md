...
*   `%s` (`字符串`): 它根据给定的格式转换除`对象`、 `BigInt` 和 `-0` 以外的所有值。没有自定义`toString`功能的对象使用`util.inspect()`和 `BigInt` 值用 `n` 表示。
*   `%c` (`CSS`): 如果有 `CSS` 通过，则跳过。通常，该说明符被忽略。
*   `%d` (`数字`): 除了`符号`和 `BigInt` 之外，它根据给定的格式转换所有的值。
*   `%i`(`parseInt()`): 解析一个`<字符串>`返回一个整数，用于除 `BigInt` 和`符号`以外的所有值。
...
## 参考:
[https://nodejs.org/api/util.html#util_util_format_args](https://nodejs.org/api/util.html#util_util_format_args)