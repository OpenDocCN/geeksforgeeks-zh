...
`http2.getUnpackedSettings()` 是 `http2` 模块中类 `HTTP2` 的内置应用程序编程接口，用于提供 HTTP/2 设置对象，该对象包含由 `http2.getPackedSettings()` 生成的来自给定缓冲区的反序列化设置。
...
**1. 文件名: 私钥** 打开记事本复制粘贴以下密钥，将文件保存为 `private-key`。

**2. 文件名: 公共证书** 打开记事本复制粘贴以下密钥，将文件保存为 `public-cert.pem`。
...
**参考:** [https://nodejs.org/dist/latest-v12.x/docs/api/http2.html#http2_http2_getunpackedsettings_buf](https://nodejs.org/dist/latest-v12.x/docs/api/http2.html#http2_http2_getunpackedsettings_buf)