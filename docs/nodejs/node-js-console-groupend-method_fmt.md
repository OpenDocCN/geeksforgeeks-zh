...
> 原文：[https://www.geeksforgeeks.org/node-js-console-groupend-method/](https://www.geeksforgeeks.org/node-js-console-groupend-method/)

`console.groupEnd()` 方法是 `console` 模块的内置 API，用于结束通过调用 `console.group()` 或 `console.groupCollapsed()` 启动的组。它标志着消息组的结束。
...
**返回值：** 这个方法不返回任何值，只是结束控制台上的组。

以下示例说明了 Node.js 中 `console.groupEnd()` 方法的使用。
...
**参考：** [https://nodejs.org/api/console.html#console_console_groupend](https://nodejs.org/api/console.html#console_console_groupend)