...
> 原文: [https://www.geeksforgeeks.org/how-to-update-single-and-multiple-documents-in-mongodb-using-node-js/](https://www.geeksforgeeks.org/how-to-update-single-and-multiple-documents-in-mongodb-using-node-js/)

`MongoDB` 是最受欢迎的 NoSQL 数据库，是一个开源的面向文档的数据库。术语“NoSQL”的意思是“非关系的”。这意味着 `MongoDB` 不是基于类似表的关系数据库结构，而是提供了一种完全不同的数据存储和检索机制。这种存储格式称为 `BSON`（类似于 `JSON` 格式）。参考[本](https://www.geeksforgeeks.org/mongodb-an-introduction/)篇。
