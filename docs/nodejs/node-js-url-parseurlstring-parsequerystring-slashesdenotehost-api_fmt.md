...
*   `parseQueryString`: 是布尔值。如果设置为 `true`，则 `query` 属性将被设置为由 `querystring` 模块的 `parse()` 方法返回的对象。如果设置为 `false`，则返回的 URL 对象的 `query` 属性将是一个未分析、未编码的字符串。它的默认值是 `false`。
*   `slashesDenoteHost`: 是布尔值。如果设置为 `true`，则文字字符串 `//` 之后、下一个 `/` 之前的第一个标记将被解释为主机。例如 `//geeksforgeeks.org/web-technology` 包含结果 `{ host: 'geeksforgeeks.org', pathname: '/web-technology' }` 而不是 `{ pathname: '//geeksforgeeks.org/web-technology' }`。它的默认值是 `false`。

**返回值:** `url.parse()` 方法返回一个对象，地址的每个部分都作为属性。

**注意:**

*   如果 `urlString` 不是字符串，则它会引发 `TypeError`。
...
**参考:** [https://nodejs.org/docs/latest/api/url.html#url_url_parse_urlstring_parsequerystring_slashesdenotehost](https://nodejs.org/docs/latest/api/url.html#url_url_parse_urlstring_parsequerystring_slashesdenotehost)