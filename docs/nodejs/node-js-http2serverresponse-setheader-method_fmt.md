...
> 原文: [https://www.geeksforgeeks.org/node-js-http2serverresponse-setheader-method/](https://www.geeksforgeeks.org/node-js-http2serverresponse-setheader-method/)

`Http2ServerResponse.setHeader()` 是 `http2` 模块内类 `Http2ServerResponse` 的内置应用程序编程接口，用于设置头名称和该数据头的特定值。
...
**第 1 步**: 打开记事本复制粘贴以下密钥，将文件另存为 `private-key.pem`。
...
**第 2 步**: 打开记事本复制粘贴以下密钥，将文件保存为 `public-cert.pem`。
...
[https://nodejs.org/dist/latest-v12.x/docs/api/http2.html#http2_response_setheader_name_value](https://nodejs.org/dist/latest-v12.x/docs/api/http2.html#http2_response_setheader_name_value)