...
> 原文: [https://www.geeksforgeeks.org/node-js-fspromises-symlink-method/](https://www.geeksforgeeks.org/node-js-fspromises-symlink-method/)
...
*   **Type:** is a string representing the `symbolic link` type to be created. It can be specified with `"file"`, `"dir"` or `"junction"`. If the target does not exist, the `file` will be used.

`type` 参数仅在 Windows 平台上使用，可以是 `目录`、`文件` 或 `交汇点` 之一。Windows 交叉点要求目标路径是绝对的。当使用 `'连接'` 时，`target` 参数将自动规范化为绝对路径。

## 示例

这个示例说明了 Node.js 中的 `fsPromises.symlink()` 方法:
...
[https://nodejs.org/api/fs.html#fs_fspromises_symlink_target_path_type](https://nodejs.org/api/fs.html#fs_fspromises_symlink_target_path_type)