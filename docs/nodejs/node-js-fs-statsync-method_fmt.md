...
`fs.statSync()`方法用于同步返回给定文件路径的信息。返回的`Stat`对象有几个字段和方法来获取文件的更多细节。
...
*   `path`:保存需要检查的文件的路径。它可以是字符串、缓冲区或网址。
*   `options`:是一个可以用来指定影响输出的可选参数的对象。它有一个可选参数:
    *   `bigint`: 它是一个布尔值，指定在`fs`中返回的数值。`Stats`对象是`bigint`。默认值为假。

**返回:**返回一个`Stats`对象，其中包含文件路径的详细信息。

以下示例说明了 Node.js 中的`fs.statSync()`方法:

**示例 1:** 本示例使用`fs.statSync()`方法获取路径的详细信息。
