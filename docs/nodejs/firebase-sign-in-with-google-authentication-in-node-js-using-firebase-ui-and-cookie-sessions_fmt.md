# Firebase（使用 Google 登录）：在 Node.js 中使用 Firebase UI 和 Cookie 会话进行身份验证

> 原文：[https://www.geeksforgeeks.org/firebase-sign-in-with-google-authentication-in-node-js-using-firebase-ui-and-cookie-sessions/](https://www.geeksforgeeks.org/firebase-sign-in-with-google-authentication-in-node-js-using-firebase-ui-and-cookie-sessions/)

Firebase 身份验证提供易于使用的 SDK 和现成的用户界面库后端服务，以向您的应用程序验证用户。

## 先决条件
节点、JavaScript、firebase 基础知识。

## 设置
首先我们需要创建一个 Firebase 项目，前往 [Firebase 控制台](https://console.firebase.google.com/) 并创建一个新项目。
...
现在进入认证选项卡，打开 **使用 Google 登录**。
> ![](img/39dbb7394ac14dee855fca590a075d18.png)

现在创建一个新项目，其中 `success.html`（带有一个简单的锚点标签指向“/”根）和 `login.html`（留下一个 id 为 `firebaseui-auth-container` 的空白区域，firebaseui 将被初始化的地方）。
...
**参考：** [https://firebase.google.com/docs/auth/admin/manage-cookies](https://firebase.google.com/docs/auth/admin/manage-cookies)