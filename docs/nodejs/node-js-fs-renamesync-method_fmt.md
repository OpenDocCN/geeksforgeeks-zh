...
`fs.renameSync()`方法用于将给定旧路径的文件同步重命名为给定新路径。如果目标文件已经存在，它将覆盖该文件。
...
*   `Old path`：要重命名的文件的路径。它可以是字符串、Buffer或Web地址。
*   `New Path`：重命名后文件的新路径。它可以是字符串、Buffer或Web地址。

下面的例子说明了Node.js中的`fs.renameSync()`方法：
...
**参考：** [https://nodejs.org/api/fs.html#fs_fs_renamesync_oldpath_newpath](https://nodejs.org/api/fs.html#fs_fs_renamesync_oldpath_newpath)