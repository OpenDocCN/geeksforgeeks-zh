...
> 原文: [https://www.geeksforgeeks.org/node-js-ecdh-getprivatekey-method/](https://www.geeksforgeeks.org/node-js-ecdh-getprivatekey-method/)

`ecdh.getPrivateKey()` 方法是加密模块内 `ecdh` 类的内置应用编程接口，用于获取椭圆曲线 Diffie-Hellman (ECDH) 对象的私钥。密钥的编码可以使用 `encoding` 参数指定，格式可以使用 `format` 参数指定。
...
*   `format`: 是指定按键格式的字符串。该值可以是“压缩”或“未压缩”。这是一个可选参数。
...
**参考资料:** [https://nodejs.org/api/crypto.html#crypto_ecdh_getprivatekey_encoding](https://nodejs.org/api/crypto.html#crypto_ecdh_getprivatekey_encoding)