...
> 原文：`https://www.geeksforgeeks.org/node-js-buffer-entries-method/`

`Buffer.entries()`方法用于从缓冲区的内容创建并返回`[index, byte]`对的迭代器。
...
该方法以`[index, byte]`格式返回一个`Iterator`对象对。

下面的例子说明了`buffer.entries()`方法在Node中的使用：
...
`https://nodejs.org/docs/latest-v11.x/api/buffer.html#buffer_buf_entries`