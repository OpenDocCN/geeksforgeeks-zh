...
> 原文: [https://www.geeksforgeeks.org/node-js-fs-dirent-isfile-method/](https://www.geeksforgeeks.org/node-js-fs-dirent-isfile-method/)

`fs.Dirent.isFile()` 方法是 `fs` 模块（文件系统）中 `Dirent` 类的一个内置 API，用于检查特定目录项是否描述一个文件。
...
## 返回值
如果特定目录项描述文件，则该方法返回 `true`，否则返回 `false`。

下面的程序说明了 `fs.dirent.isFile()` 方法在 Node.js 中的使用。
...
**参考:** [https://nodejs.org/dist/latest-v12.x/docs/api/fs.html#fs_dirent_isfile](https://nodejs.org/dist/latest-v12.x/docs/api/fs.html#fs_dirent_isfile)