...
> 原文: [https://www.geeksforgeeks.org/node-js-buffer-readint32be-method/](https://www.geeksforgeeks.org/node-js-buffer-readint32be-method/)

`Buffer.readInt32BE()`方法用于从给定偏移量的缓冲区对象中读取 32 位整数，并以大端字节顺序返回结果。
...
该方法接受单个参数`offset`，指定缓冲对象的位置。它表示开始读取前要跳过的字节数。偏移值在`0`到`缓冲区长度–4`的范围内。默认值为`0`。

## 返回值

该方法读取大端指定偏移量处的有符号 32 位整数。
