...
> 原文：[https://www.geeksforgeeks.org/node-js-fs-extra-pathexists-function/](https://www.geeksforgeeks.org/node-js-fs-extra-pathexists-function/)

`pathExists()`测试给定的文件路径是否存在。它在引擎盖下使用 [`fs.access()`](https://www.geeksforgeeks.org/node-js-fs-access-method/)。
...
*   `file`：是包含文件路径的字符串。
*   `callback`：函数执行后调用。它将导致一个错误或者一个名为 `exists` 的布尔值。我们也可以用承诺来代替回调函数。
