...
基于 Node.js 控制台的应用程序使用 Node.js 命令提示符运行。Node.js 中的 `console` 模块提供了一个简单的调试控制台。Node.js 是一个全局控制台，可用于同步和异步通信。`console.log()` 函数用于在控制台上显示输出。这个函数用换行符将输出打印到 `stdout`。
...
这里，`data` 是要在控制台上显示的内容。

### 示例 1

使用 Node.js 创建 Hello World 应用程序。创建包含以下代码的 `geeks.js` 文件:

```js
console.log('Hello World');
```

使用命令 `node geeks.js` (即 `node <file_name>`) 在 Node.js 命令提示符下运行文件。

**输出:**
![](img/ea328faba08b44343d1ffbbcee6bc3e8.png)

### 示例 2

创建接收用户输入的 Hello World 应用程序。创建一个包含以下代码的 `gfg.js` 文件。
...
## 基于 web 的 Node.js 应用程序

基于 Web 的 Node.js 应用程序由以下三个重要组件组成:
...
*   **创建服务器:** 创建一个服务器来监听客户端的请求。使用 `createServer()` 方法创建服务器实例。使用与服务器实例关联的 `listen` 方法将服务器绑定到端口 `8080`。
    **语法:**
...
*   **读取请求并返回响应:** 读取使用浏览器或控制台发出的客户端请求并返回响应。带有 `request` 和 `response` 参数的函数用于读取客户端请求并返回响应。
    **语法:**
...
本示例使用 Node.js 创建一个 Hello World 基于 web 的应用程序。
