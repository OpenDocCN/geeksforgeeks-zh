...
> 原文: [https://www.geeksforgeeks.org/node-js-process-memoryusage-method/](https://www.geeksforgeeks.org/node-js-process-memoryusage-method/)

`process.memoryUsage()` 方法是 `process` 模块的一种内置方法，它提供关于 Node.js 程序的当前进程或运行时的信息。`memoryUsage()` 方法返回一个对象，描述 Node.js 进程的内存使用情况（以字节为单位）。
