...
> 原文：[https://www.geeksforgeeks.org/how-to-convert-an-existing-callback-to-a-promise-in-node-js/](https://www.geeksforgeeks.org/how-to-convert-an-existing-callback-to-a-promise-in-node-js/)

## 回调

就 JavaScript 而言，回调是作为参数传递给另一个函数的函数。调用函数的结果一准备好，函数就执行。简单来说，它处理 JavaScript 的异步需求。

### 示例

演示这个的最佳示例是使用 `setTimeout()` 函数，该函数接受回调并延迟 JavaScript 代码的执行。一旦设置为函数的时间结束，回调函数就会执行。
...
非常类似于运行中的回调。但是，使用 Promises 的好处是提高代码的可读性，因为它将我们从回调的地狱中拯救出来。
...
*   以下列任何一种方式使用 `success` 和 `error` 方法。
*   为了更好地理解，请参见下面的代码片段。
...
这里，场景是我们有一个学生数组，`id` 和 `name` 作为值。我们需要获得拥有给定 `id` 的学生的姓名。我们得到了一个现有的回调，需要转换成一个承诺。
