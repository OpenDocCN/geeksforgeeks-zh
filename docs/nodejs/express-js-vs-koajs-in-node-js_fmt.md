...
> 原文: [https://www.geeksforgeeks.org/express-js-vs-koajs-in-node-js/](https://www.geeksforgeeks.org/express-js-vs-koajs-in-node-js/)
...
## Express 模块
为了使用 Express 模块，我们需要安装 `NPM`（`Node Package Manager`）和以下模块（在 `cmd` 上）。
...
## 发送和侦听响应
它与客户端和服务器通信请求和响应。它需要 `PORT`（`Number`）和 `IP`（`Number`）进行通信。
...
## KoaJS 模块
为了使用 `KoaJS` 模块，我们需要安装 `NPM`（`Node Package Manager`）和以下模块（在 `cmd` 上）。
...
## 创建服务器
上面的语法导入 `koa` 模块，并创建一个新的 Koa 应用程序，该应用程序存储在 `app` 变量中。
