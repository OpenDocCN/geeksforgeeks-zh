...
> 原文: [https://www.geeksforgeeks.org/node-js-http-validateheadername-method/](https://www.geeksforgeeks.org/node-js-http-validateheadername-method/)

`http.validateHeaderName()`（在 v14.3.0 中添加）是 `http` 模块的内置属性，该模块对所提供的名称执行低级验证，这些验证是在调用 `res.setHeader(name, value)` 时完成的。将非法值作为名称传递将导致抛出 `TypeError`，由代码标识：`ERR_INVALID_HTTP_TOKEN`。
...
下面的例子说明了 Node.js 中 `http.validateHeaderName()` 属性的使用。
...
> 在控制台中出现错误："ERR_INVALID_HTTP_TOKEN"
...
> 在控制台中，服务器正在端口 3000 处运行……
> 标头“Content-Type”验证为真……
> 标头“set-cookie”验证为真……
> 标头“alfa-beta”验证为真……
> 错误发生 ERR_INVALID_HTTP_TOKEN
> 标头名称必须是有效的 HTTP 令牌["@@wdjhgw"]
...
**参考：** [https://nodejs.org/api/http.html#http_http_validateheadername_name](https://nodejs.org/api/http.html#http_http_validateheadername_name)