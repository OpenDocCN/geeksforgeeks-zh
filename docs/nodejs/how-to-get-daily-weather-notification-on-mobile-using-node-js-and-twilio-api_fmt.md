...
> 原文：[https://www.geeksforgeeks.org/how-to-get-daily-weather-notification-on-mobile-using-node-js-and-twilio-api/](https://www.geeksforgeeks.org/how-to-get-daily-weather-notification-on-mobile-using-node-js-and-twilio-api/)

每天寻找天气信息是一项琐碎的任务。作为程序员，我们可以通过创建通知服务来简化这项任务。我们将使用 **Twilio 短信 API** 发送短信，使用 Node.js 编写每天特定时间运行的服务。

## 获取 Twilio 凭据

要使用 Twilio APIs，我们需要注册我们的应用程序，并从 Twilio 的控制台获取 API 密钥。我们需要得到`ACCOUNT SID`、`AUTH TOKEN`和`TRIAL NUMBER`。您可以在[这里](https://www.twilio.com/console/)获取所有这些凭证。
...
为了发送短信，我们可以使用 Twilio 的 Node.js 库，使用以下命令安装库：
...
获取最新天气信息，我们将使用[开放天气地图 API](http://openweathermap.org/)，免费提供。
