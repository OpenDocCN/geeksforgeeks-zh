...
`Buffer.swap64()`方法是`Buffer`模块中类`Buffer`的内置应用编程接口，用于就地交换缓冲字节顺序。交换是通过将缓冲区解释为64位数字的数组来执行的。
...
如果缓冲区长度(`buf.length`)不是8的倍数，则抛出`ERR_INVALID_BUFFER_SIZE`。

下面的例子说明了`Buffer.swap64()`方法在Node.js中的使用:
...
**参考:** [https://nodejs.org/dist/latest-v13.x/docs/api/buffer.html#buffer_buf_swap64](https://nodejs.org/dist/latest-v13.x/docs/api/buffer.html#buffer_buf_swap64)