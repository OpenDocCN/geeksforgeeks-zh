...
> 原文：[https://www.geeksforgeeks.org/node-js-mysql-order-by-clause/](https://www.geeksforgeeks.org/node-js-mysql-order-by-clause/)

## 简介
我们使用 SQL `ORDER BY` 子句按照升序或降序对某个列值的数据进行排序。

## 语法
```sql
SELECT * FROM users ORDER BY name;
```
这将按照 `name` 列的升序（默认）对所有输出行进行排序。

```sql
SELECT address FROM users ORDER BY name DESC;
```
这将按照 `name` 列降序对所有输出行进行排序，但返回 `address` 数据。
...
## 数据库
我们的 SQL `users` 表预览用样本数据如下所示：

![](img/28feecb0c4162e890d06e991d575dcce.png)

## 示例 1
从 `users` 表中选择所有列，但按 `name` 升序排序。
...
## 示例 2
仅从 `users` 表中选择 `address` 列，但按 `name` 降序排列。
