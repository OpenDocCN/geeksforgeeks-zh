...
> 原文: [https://www.geeksforgeeks.org/node-js-serverhttp2session-origin-method/](https://www.geeksforgeeks.org/node-js-serverhttp2session-origin-method/)

`serverhttp2session.origin()` 是 `http2` 模块内类 `ServerHttp2Session` 的内置 API，用于向连接的客户端提交一个 ORIGIN 帧（如 RFC 8336 所定义）来通告该组 ORIGIN。
