...
`path.resolve()`方法用于将一系列路径段解析为绝对路径。它的工作原理是从右到左处理路径序列，预先考虑每个路径，直到创建绝对路径。生成的路径被规范化，尾部斜线根据需要被删除。
...
下面的程序说明了 Node.js 中的 `path.resolve()`方法。

### 例 1:

```js
// Node.js program to demonstrate the path.resolve() Method
