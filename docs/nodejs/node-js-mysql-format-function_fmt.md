...
> 原文: [https://www.geeksforgeeks.org/node-js-mysql-format-function/](https://www.geeksforgeeks.org/node-js-mysql-format-function/)
...
*   `number`: 用于格式化的数字。
*   `number_of_decimal_places`: 是小数位数。
...
我们的 SQL `publishers` 表带有样本数据，预览如下所示：
...
db_con.connect((err) => {
    ...
    // Here is the query
    let query = "SELECT FORMAT(salary, 1) AS formatted_salary FROM publishers";
