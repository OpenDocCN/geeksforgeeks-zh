...
> 原文: [https://www.geeksforgeeks.org/node-js-buffer-readint32le-method/](https://www.geeksforgeeks.org/node-js-buffer-readint32le-method/)

`Buffer.readInt32LE()`方法用于从给定偏移量的缓冲区对象中读取 32 位整数，并以小端字节顺序返回结果。
...
该方法接受单个参数`offset`，指定缓冲对象的位置。它表示开始读取前要跳过的字节数。偏移值介于`0`到`buff.length–4`之间。默认值为`0`。

## 返回值:

该方法在小端字节序中指定的偏移量处读取一个带符号的 32 位整数。
...
[https://nodejs.org/api/buffer.html#buffer_buf_readint32le_offset](https://nodejs.org/api/buffer.html#buffer_buf_readint32le_offset)