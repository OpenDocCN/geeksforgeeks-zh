# Node.js fs-extra remove() 功能

> 原文: [https://www.geeksforgeeks.org/node-js-fs-extra-remove-function/](https://www.geeksforgeeks.org/node-js-fs-extra-remove-function/)

`remove()` 函数删除给定的文件或目录。目录中的所有文件都会被删除。如果给定的文件或目录不存在，该函数将什么也不做。
...
*   `path`: 是包含文件路径或目录路径的字符串。
*   `callback`: 函数执行后调用。我们也可以用承诺来代替回调函数。
