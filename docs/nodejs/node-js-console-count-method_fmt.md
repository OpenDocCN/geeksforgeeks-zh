...
`console.count()`方法是控制台模块的内置应用程序编程接口，用于通过维护特定标签的内部计数器，将传递给它的标签作为参数进行计数。
...
*   `Label`：是一个可选参数，指定要计数的标签。默认值是 `Default Value`。

## 返回值

此方法将使用指定标签调用的此函数的计数输出到 `stdout`。
...
[https://nodejs.org/api/console.html#console_console_count_label](https://nodejs.org/api/console.html#console_console_count_label)