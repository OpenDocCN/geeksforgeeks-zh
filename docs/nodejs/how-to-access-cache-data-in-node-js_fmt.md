...
> 原文: [https://www.geeksforgeeks.org/how-to-access-cache-data-in-node-js/](https://www.geeksforgeeks.org/how-to-access-cache-data-in-node-js/)
...
本文使用包 [node-cache](https://www.npmjs.com/package/node-cache) 演示了在 Node.js 中缓存的用法，`node-cache` 是一个内存缓存模块。

## 使用 node-cache 实现缓存演示
...
导航到所需的项目目录，并使用以下命令安装 `express` (用于创建服务器) 和 `node-cache` npm 包:
...
通过 `Ctrl+C` 停止服务器，通过 `node server.js` 再次启动服务器。
...
导入 [node-cache npm 模块](https://www.npmjs.com/package/node-cache) 并创建一个新的 `node-cache` 对象
...
*   `.get(key)`: 用于获取设置为指定键的值。如果密钥不存在，它将返回 `undefined`。
*   `.has(key)`: 用于检查缓存是否已经为指定键设置了某个值。如果存在则返回 `true`，否则返回 `false`。

**通过以下方式实现缓存:**

*   在 API 请求时，使用 `.has(key)` 函数检查缓存是否已经设置了键。
*   如果缓存有关键字，则通过 `.get(key)` 函数检索缓存的值并使用它，而不是再次执行操作。(这样可以节省时间)
*   如果缓存没有键，请执行所需的操作，并在发送响应之前使用 `.set(key, value)` 设置该键的值，以便可以通过缓存的数据直接响应进一步的请求。
