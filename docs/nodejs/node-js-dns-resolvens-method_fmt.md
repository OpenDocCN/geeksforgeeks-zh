...
`dns.resolveNs()`方法是 dns 模块的内置应用编程接口，用于使用 dns 协议解析指定主机名的 ns 或名称服务器记录。
...
[https://nodejs.org/api/dns.html#dns_dns_resolvens_hostname_callback](https://nodejs.org/api/dns.html#dns_dns_resolvens_hostname_callback)