...
*   `string`：是将要写入文件的字符串。
*   `position`：指定文件中要写入文本的位置。如果在方法中没有传递位置或者没有使用整数值来指定位置，那么它将从第 0 个位置开始写入。如果已经在该位置写入了字符串，则该方法将覆盖在该位置传递的新字符串。
*   `encoding`：是指定字符编码的字符串。默认情况下，它是 `utf8`。
*   `buffer`：包含缓冲区类型值，如 `Buffer`、`TypedArray`、`DataView`。
*   `offset`：是一个整数值，指定缓冲区中要写入文件的部分。
*   `length`：是一个整数值，指定写入文件的字节数。
...
*   [https://nodejs.org/api/fs.html#fs_fs_writesync_fd_string_position_encoding](https://nodejs.org/api/fs.html#fs_fs_writesync_fd_string_position_encoding)
*   [https://nodejs.org/api/fs.html#fs_fs_writesync_fd_buffer_offset_length_position](https://nodejs.org/api/fs.html#fs_fs_writesync_fd_buffer_offset_length_position)