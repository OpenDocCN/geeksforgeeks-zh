...
*   `"SIGTERM"`和`"SIGINT"`信号在非 Windows 平台上有默认的处理程序，在退出前会用代码 128 +信号号重置终端模式。如果其中一个信号安装了侦听器，它在 node.js 上的默认行为将被删除。
*   `'SIGHUP'`在控制台窗口关闭时生成。

## 返回值:

如果没有找到目标 `pid` 或者目标不存在，`process.kill()`方法会抛出一个错误。如果 pid 存在，该方法返回布尔值 `0` ，并且可以用作目标进程存在的测试。对于窗口用户来说，如果用 `pid` 杀死一组进程，这个方法也会抛出一个错误。
...
**参考:**[https://nodejs.org/api/process.html#process_process_kill_pid_signal](https://nodejs.org/api/process.html#process_process_kill_pid_signal)