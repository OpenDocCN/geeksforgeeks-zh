# `process.report.writeReport()` 方法

> 原文：[https://www.geeksforgeeks.org/node-js-process-report-writereport-method/](https://www.geeksforgeeks.org/node-js-process-report-writereport-method/)

`process.report.writeReport()` 方法将诊断报告写入文件。如果未提供文件名，默认文件名包括日期、时间、PID 和序列号。报告的 JavaScript 堆栈跟踪取自 `err`（如果有）。
