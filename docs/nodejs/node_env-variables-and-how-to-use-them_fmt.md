...
> 原文: [https://www.geeksforgeeks.org/node_env-variables-and-how-to-use-them/](https://www.geeksforgeeks.org/node_env-variables-and-how-to-use-them/)
...
因为不同的操作系统需要不同的命令，所以有一个名为 [`cross-env`](https://www.npmjs.com/package/cross-env) 的包可以让命令跨平台。

```js
npx cross-env NODE_ENV=production node app.js
```

上面的代码可以在任何平台上工作，只要 `cross-env` 包是作为开发人员依赖项安装的。
...
假设我们的数据库服务器是开发模式下的 `localhost`，我们将使用 `https://production-server.com` 进行生产。可以相应地设置代码。
