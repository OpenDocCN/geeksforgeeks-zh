...
> 原文：[https://www.geeksforgeeks.org/difference-between-npm-i-and-npm-ci-in-node-js/](https://www.geeksforgeeks.org/difference-between-npm-i-and-npm-ci-in-node-js/)
...
`NPM I`（或 `npm install`）用于安装来自 `package.json` 文件的所有依赖项或开发依赖项。
...
CI 代表持续集成，`npm ci` 用于从 `package-lock.json` 文件安装所有精确的版本依赖项或 `devDependencies`。
...
| 2. | 可能写入 `package.json` 或 `package-lock.json`。 | 从不写入 `package.json` 或 `package-lock.json`。 |
...
| 6. | If a `node_modules` already exists, this command will not change it. | If a `node_modules` already exists, it will be automatically removed before `npm ci` starts to install. |
