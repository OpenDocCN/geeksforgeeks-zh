...
> 原文: [https://www.geeksforgeeks.org/how-to-call-an-api-continuously-from-server-side-itself-in-node-js-express-js/](https://www.geeksforgeeks.org/how-to-call-an-api-continuously-from-server-side-itself-in-node-js-express-js/)
...
使用以下命令安装所需模块，即 `ExpressJS` 和 `Axios`。
...
**说明:** 在上面的例子中，NodeJS 每隔 2 秒调用一次 API 来获取数据。如果承诺被解决，那么 `.then()` 块将被执行并打印数据。如果承诺被拒绝，`.catch()` 块将被执行并打印错误信息。
