...
> 原文：[https://www.geeksforgeeks.org/how-to-drop-collection-in-mongodb-using-node-js/](https://www.geeksforgeeks.org/how-to-drop-collection-in-mongodb-using-node-js/)
...
`node.js` 的这个模块用于连接 `MongoDB` 数据库，也用于操作 `MongoDB` 中的集合和数据库。`mongodb.connect()` 方法用于连接您机器上特定服务器上运行的 `mongodb` 数据库。（参考[这篇文章](https://www.geeksforgeeks.org/how-to-connect-mongodb-server-with-node-js/)）。我们还可以在这个方法中使用 `promises` 来解析包含集合操作所需的所有方法和属性的对象，在 `reject` 中，错误发生在连接期间。
