...
`new URL()`（*新增于 v7.0.0，v6.13.0*）方法是 `URL` 模块的内置应用编程接口，通过解析相对于基底的输入来创建新的 `URL` 对象。如果基础作为字符串传递，将被解析为等同于 `new URL(base)`。
...
- **`input`** <`string`>: 用于解析绝对或相对输入网址的输入是 `string` 类型。如果输入是相对的，则需要基数；如果输入是绝对的，则忽略基数。
- **`base`** <`string`> | <`URL`>: 是 base URL，可以是 `string` 类型，也可以是 `URL` 类型，用于解析输入是否为绝对。

## 返回值

返回新生成的网址以及主机名、协议、路径名等一系列数据。
...
**参考:** [https://nodejs.org/api/url.html#url_new_url_input_base](https://nodejs.org/api/url.html#url_new_url_input_base)