...
原文:`https://www.geeksforgeeks.org/node-js-zlib-flush-method/`
...
下面的例子说明了`zlib.flush()`方法在Node.js中的使用:
...
`输出`:所以，当字节大小超过16384字节后，你需要回调flush方法，否则数据不会被完全刷新。
```js
Program Complete!
...
`参考资料`:`https://nodejs.org/api/zlib.html#zlib_flushing`