...
`fs.fchmodSync()`方法是`fs`模块的内置应用编程，用于同步更改给定文件描述符的权限。这些权限可以使用对应于各自文件模式的字符串常量或八进制数指定为参数。
...
*   `fd`: 是一个整数，表示需要更改权限的文件的文件描述符。
*   `mode`: 无论是字符串常量还是八进制常量，都表示要授予的权限。可以使用逻辑或运算符来分隔多个权限。
...
参考: [https://nodejs.org/api/fs.html#fs_fs_fchmodsync_fd_mode](https://nodejs.org/api/fs.html#fs_fs_fchmodsync_fd_mode)