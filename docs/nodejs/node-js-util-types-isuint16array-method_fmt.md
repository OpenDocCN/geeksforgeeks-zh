...
> 原文: [https://www.geeksforgeeks.org/node-js-util-types-isuint16array-method/](https://www.geeksforgeeks.org/node-js-util-types-isuint16array-method/)

`util.types.isUint16Array()` (在 v10.0.0 中添加) 方法是 `util` 模块的一个内置 API，用于检查传递的值在 Node.js 中是否属于类型 `Uint16Array`。如果传递的值属于类型 `Uint16Array`，则返回 `true`，否则返回 `false`。
...
<`boolean`>: 这返回一个布尔值。如果传递的值属于 `Uint16Array` 类型，则返回 `true`，否则返回 `false`。

下面的例子说明了在 Node.js 中 `util.types.isUint16Array()` 方法的使用。
...
**参考:** [https://nodejs.org/api/util.html#util_util_types_isuint16array_value](https://nodejs.org/api/util.html#util_util_types_isuint16array_value)