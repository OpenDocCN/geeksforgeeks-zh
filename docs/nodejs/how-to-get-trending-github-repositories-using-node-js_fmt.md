...
> 原文：[https://www.geeksforgeeks.org/how-to-get-trending-github-repositories-using-node-js/](https://www.geeksforgeeks.org/how-to-get-trending-github-repositories-using-node-js/)

## 进场

*   获取整个 HTML 页面，并使用 `request` 包将其存储为字符串。
*   将 HTML 加载到 `cheerio` 中，并找到 CSS 选择器来提取存储库细节。

## 使用 `request` 包

[`request` 包](https://www.npmjs.com/package/request)：请求被设计成最简单的 http 调用方式。默认情况下，它支持 HTTPS 并遵循重定向。
