...
> 原文：[https://www.geeksforgeeks.org/what-is-eventemitter-in-node-js/](https://www.geeksforgeeks.org/what-is-eventemitter-in-node-js/)
...
为了在事件发射器上执行操作，我们需要使用 `events` 模块创建一个引用，然后我们需要初始化 `EventEmitter` 的一个实例，以便我们可以进一步使用它。
...
**使用 `addListener` 创建监听事件：** 在我们发出事件之前，我们必须创建一个监听发出的回调的监听程序，然后监听程序将继续执行遵守事件所需的进一步操作。我们可以借助事件发射器的 `addListener` 属性来创建一个事件监听器。`addListener` 将事件追加到数组的末尾，这样我们就可以多次调用它来调用事件的多个实例，这非常有用，因为我们不需要多次编写事件。
...
**移除事件的一个实例：** 如果我们想从事件中移除一个特定的侦听器，我们可以使用两种方法来移除事件 `removeListener` 或者如果我们想从事件中移除所有侦听器的一个实例，我们可以使用 `removeAllListeners`。
