...
`Buffer.equals()`方法用于比较两个缓冲区对象，如果两个缓冲区对象相同，则返回真，否则返回假。
...
该方法接受单个参数 `otherBuffer`，保存另一个缓冲区与缓冲区对象进行比较。

## 返回值

如果两个缓冲区对象相等，该方法返回真，否则返回假。
...
[https://nodejs.org/api/buffer.html#buffer_buf_equals_otherbuffer](https://nodejs.org/api/buffer.html#buffer_buf_equals_otherbuffer)