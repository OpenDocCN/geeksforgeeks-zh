...
> 原文:[https://www.geeksforgeeks.org/node-js-filehandle-writefile-method-from-class-filehandle/](https://www.geeksforgeeks.org/node-js-filehandle-writefile-method-from-class-filehandle/)

`filehandle.writeFile()`方法是用来在Node.js的File System模块中定义的，File System模块基本上就是和用户电脑的硬盘进行交互。`fs.writeFile()`方法将数据异步写入文件，如果文件已经存在，则替换该文件。
...
*   `data`:是字符串、缓冲区或`Uint8Array`实例。它是将要写入文件的数据。
