...
`split()`函数是 [Node.js](https://www.geeksforgeeks.org/introduction-to-nodejs/) 的一个字符串函数，用于将字符串拆分为子字符串。该函数以数组形式返回输出。
...
该函数接受单参数`分隔符`，该分隔符保存要分割字符串的字符。

## 返回值

函数返回拆分后的字符串，并生成数组作为结果。
