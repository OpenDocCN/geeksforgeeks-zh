...
`swirl()` 函数是 GraphicsMagick 库中的一个内置函数，用于图像的中心。度数指定漩涡的紧密度。
...
该功能接受上面提到的和下面描述的单个参数:

*   `angle`: 该参数存储涡流的角度。

## 返回值

该函数返回 GraphicsMagick 漩涡图像。
...
*   [http://www.graphicsmagick.org/GraphicsMagick.html#details-swirl](http://www.graphicsmagick.org/GraphicsMagick.html#details-swirl)
