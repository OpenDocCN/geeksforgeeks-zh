...
> 原文：[https://www.geeksforgeeks.org/simple-task-manager-cli-using-node-js/](https://www.geeksforgeeks.org/simple-task-manager-cli-using-node-js/)

在使用 NodeJS 创建一个简单的任务管理器 CLI 之前，让我们通过以下简单的步骤运行我们的第一个基本 Node.js 应用程序：

**注意：** 您可以从 [https://nodejs.org/en/](https://nodejs.org/en/) 下载并安装 Node.js 应用程序。

## 第一步：创建目录

为我们的任务管理器 CLI 创建一个名为 `TODO-CLI` 的目录，您可以给出自己选择的名称。
...
在该目录中创建一个名为 `index.js` 的 JavaScript 文件，如下所示：
...
这个文件包含了一些运行程序的 JavaScript 代码，现在我们可以编写如下所示的一些示例代码：

![](img/9da9750ce6f07ee1ce95b3c36b484c6e.png)

## 第四步：运行文件

在 `index.js` 文件中写入代码后，使用以下命令运行该文件：
...
*   **lodash：** 一个现代的 JavaScript 实用程序库，帮助处理数组、数字、字符串、对象和字符串。
*   **yargs：** 它是一个交互式命令行模块，有助于读取用户输入。

## 第六步：创建核心逻辑文件

创建一个名为 `todo.js` 的新文件，并在其中写下以下代码：
