...
官方 Node.js 文档将流定义为“流是在 Node.js 中处理流数据的抽象接口”。`stream`模块提供了实现流接口的 API。Node.js 中的流对象的例子可以是对 HTTP 服务器和进程的请求。`stdout`都是流实例。简而言之，流是 Node.js 中的对象，它允许用户以连续的方式从源读取数据或将数据写入目标。
...
我们将从`inStream`读取数据，并使用`process.stdout`将其回显到标准输出。
...
在`outStream`中，我们简单地将块记录为一个字符串。我们还调用回调函数来指示没有任何错误的成功。我们将从`inStream`中读取数据，并使用`process.stdout`将其回显到标准输出。
