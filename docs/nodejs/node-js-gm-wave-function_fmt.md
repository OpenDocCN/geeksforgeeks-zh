...
`wave()` 函数是 `GraphicsMagick` 库中的一个内置函数，用于随着正弦波改变图像。该函数在成功时返回真值。
...
*   `amplitude`: 该参数存储正弦波的振幅值。
*   `wavelength`: 该参数存储正弦波的波长值。

## 返回值

该函数返回添加了图像的 `GraphicsMagick` 对象。
...
*   [http://www.graphicsmagick.org/GraphicsMagick.html#details-wave](http://www.graphicsmagick.org/GraphicsMagick.html#details-wave)
