...
> 原文: [https://www.geeksforgeeks.org/node-js-ecdh-setprivatekey-method/](https://www.geeksforgeeks.org/node-js-ecdh-setprivatekey-method/)

`ecdh.getPrivateKey()` 方法是加密模块内 `ecdh` 类的内置应用编程接口，用于设置椭圆曲线 Diffie-Hellman (ECDH) 对象的私钥。可以使用 `encoding` 参数指定密钥的编码。
...
*   `privateKey`: 这是需要设置的私钥。它可以以字符串、数组缓冲区、缓冲区、类型缓冲区或数据视图的格式给出。
*   `encoding`: 一个字符串值，指定返回值的编码。这是一个可选参数。

## 返回值

返回指定编码的椭圆曲线差分赫尔曼公钥。如果不提供编码，则作为缓冲区返回，否则返回字符串。
