...
该函数将数据作为参数来生成哈希，它可以是字符串或文件对象。除了数据，它还采用数据的编码类型，可以是 `utf-8`、`binary` 或 `ascii`。如果没有提供编码，并且数据是一个字符串，那么使用 `utf-8`。
...
[https://nodejs.org/api/crypto.html#crypto_hash_update_data_inputencoding](https://nodejs.org/api/crypto.html#crypto_hash_update_data_inputencoding)