...
> 原文：[https://www.geeksforgeeks.org/how-to-get-x-and-y-coordinates-of-an-element-in-protractor-node-js/](https://www.geeksforgeeks.org/how-to-get-x-and-y-coordinates-of-an-element-in-protractor-node-js/)
...
现在让我们创建我们的测试文件 `test.js`。在这个文件中，我们将访问一个 HTML 文件，然后获取 x 和 y 坐标，并将检查它是否如 HTML 文件中设置的那样。`browser` 是量角器创建的全局。

`Jasmine` 框架提供了 `describe()` 功能和 `it()` 功能，其中 `describe` 是对您的测试的描述，而 `it` 是测试的步骤。
