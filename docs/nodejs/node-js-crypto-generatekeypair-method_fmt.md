...
> 原文: [https://www.geeksforgeeks.org/node-js-crypto-generatekeypair-method/](https://www.geeksforgeeks.org/node-js-crypto-generatekeypair-method/)

`crypto.generateKeyPair()`方法是加密模块的内置应用编程接口，用于生成指定类型的新非对称密钥对。例如，目前支持的密钥类型有 `RSA`、`DSA`、`EC`、`Ed25519`、`Ed448`、`X25519`、`X448`、`DH`。此外，如果选项的`publicKeyEncoding`或`privateKeyEncoding`在此处被声明，则此函数的作用就像在其输出中调用了`keyObject.export()`一样。否则，密钥的特定部分作为`KeyObject`返回。
...
*   `type`: 它保存一个字符串，并且必须包含以下一个或多个算法：`'RSA'`、`'DSA'`、`'EC'`、`'ed25519'`、`'ed448'`、`'x25519'`、`'x448'`或`'DH'`。
*   `options`: 属于对象类型。它可以保存以下参数：
    1.  `modulusLength`: 它保存一个数字。它是以位为单位的密钥大小，仅适用于`RSA`和`DSA`算法。
    2.  `publicExponent`: 它保存一个数字。它是`RSA`算法的公开指数。其默认值为`0x10001`。
    3.  `divisorLength`: 它保存一个数字。这是数字减影算法中`q`的大小。
    4.  `namedCurve`: 它保存一个字符串。它是用于`EC`算法的曲线的名称。
    5.  `prime`: 它保存一个缓冲区。它是`DH`算法的首要参数。
    6.  `primeLength`: 它保存一个数字。它是`DH`算法的质数长度，以位为单位。
    7.  `generator`: 它保存一个数字。是`DH`算法的自定义生成器。它的默认值是`2`。
    8.  `groupName`: 保存字符串。是`DH`算法的Diffie-Hellman组名。
    9.  `publicKeyEncoding`: 它保存一个字符串。
    10. `privateKeyEncoding`: 它保存一个对象。
*   `callback`: 是一个函数，参数为`publicKey`，`privateKey`，`err`。
    1.  `err`: 保存错误。
    2.  `publicKey`: 它保存字符串、缓冲区或密钥对象。
...
下面的例子说明了`crypto.generateKeyPair()`方法在Node.js中的使用：
...
[https://nodejs.org/api/crypto.html#crypto_crypto_generatekeypair_type_options_callback](https://nodejs.org/api/crypto.html#crypto_crypto_generatekeypair_type_options_callback)