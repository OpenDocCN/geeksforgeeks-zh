...
> 原文: [https://www.geeksforgeeks.org/how-to-override-functions-of-module-in-node-js/](https://www.geeksforgeeks.org/how-to-override-functions-of-module-in-node-js/)
...
*   然后删除我们想要覆盖的模块功能。这里我们将覆盖模块的 `format()` 功能。

```js
delete url['format']
```

*   现在添加同名函数 `format`，为模块的 `format()` 函数提供新的定义。
...
现在让我们对 `app.js` 文件进行编码。在其中，我们按照上面提到的所有步骤来重写模块。完成这些步骤后，您将会成功地**覆盖**模块的 `format()` 功能。在下面找到完整的工作代码，其中显示了 `format()` 函数在覆盖之前和之后的行为。
