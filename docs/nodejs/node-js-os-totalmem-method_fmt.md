...
`os.totalmem()`方法是`os`模块的内置应用编程接口，用于获取系统内存总量(以字节为单位)。
...
## 返回值:
该方法返回一个整数值，以字节为单位指定系统内存总量。

下面的例子说明了`os.totalmem()`方法在Node.js中的使用:
...
**参考:** [https://nodejs.org/api/os.html#os_os_totalmem](https://nodejs.org/api/os.html#os_os_totalmem)