...
> 原文：[https://www.geeksforgeeks.org/node-js-util-isdeepstrictequal-method/](https://www.geeksforgeeks.org/node-js-util-isdeepstrictequal-method/)

`util` 模块提供用于调试目的的“实用”功能。为了访问这些函数，我们需要调用它们（通过 `require('util')`）。

`util.isDeepStrictEqual()`（在 v9.0.0 中添加）方法是 `util` 模块的内置应用程序编程接口，它是一个导出的类型函数，用于测试两个值的深度相等性，即实际（`val1`）和预期（`val2`）参数之间的相等性。深度相等是一种有助于通过一些规则递归地评估子对象的可枚举“拥有”属性的方法。
...
*   `val1` `<Any>`：Any 变量、类、函数、对象或 JavaScript 原语。
*   `val2` `<Any>`：Any 变量、类、函数、对象或 JavaScript 原语。

## 返回值

`<布尔>`：如果认为 `val1` 和 `val2` 相等，则返回 `true`，否则返回 `false`。
