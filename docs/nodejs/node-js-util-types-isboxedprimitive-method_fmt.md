...
> 原文: [https://www.geeksforgeeks.org/node-js-util-types-isboxedprimitive-method/](https://www.geeksforgeeks.org/node-js-util-types-isboxedprimitive-method/)

`util.types.isBoxedPrimitive()`方法是`util`模块的内置应用编程接口，主要设计用于支持 Node.js 自身内部 API 的需求。

`util.types.isBoxedPrimitive()`方法用于检查给定值是否为装箱的图元。
...
## 返回值:
如果传递的值是装箱的图元，则返回布尔值，即`true`，否则返回`false`。
...
## 参考:
[https://nodejs.org/api/util.html#util_util_types_isboxedprimitive_value](https://nodejs.org/api/util.html#util_util_types_isboxedprimitive_value)