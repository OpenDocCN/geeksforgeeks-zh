# 使用 Node.js 中的 Google-play-scraper 对安卓应用进行数据抓取

> 原文: [https://www.geeksforgeeks.org/data-scraping-for-android-apps-using-google-play-scraper-in-node-js/](https://www.geeksforgeeks.org/data-scraping-for-android-apps-using-google-play-scraper-in-node-js/)

最常见的刮网方法是我们在 Python 中使用的 selenium & beautifulsoup。尽管它可以帮助我们完成各种任务，但如果我们特别想提取谷歌 play 商店中已经存在的安卓应用的信息，用于研究甚至自用，还有另一种方法。

`google-play-scraper` 是一个 Node.js 模块，通过提供使我们的工作变得容易的方法来帮助从谷歌游戏商店抓取应用数据。
...
*   `appId`: 与 Google Play 上的应用相关联的 Id。这可以在 URL 的 `id=` 参数中找到。
*   `lang`: 是可选参数。英语的默认值为 `"en"`。如果我们想用不同的语言获取应用程序数据，我们可以为这个参数提供该语言的两个字母的代码。
*   `country`: 也是可选参数。美利坚合众国的默认值为 `"us"`。当我们正在查看的应用程序仅在特定国家/地区可用时，此参数非常有用。
...
[ { title: 'Learn DS & Algo, Programming Interview Preparation',
  description:
   'GeeksforGeeks is a one-stop destination for programmers. The app features 20000+ Programming Questions, 40, 000+....',
  descriptionHTML:
...
*   `country`: 也是可选参数。美利坚合众国的默认值为 `"us"`。当我们正在查看的应用程序仅在特定国家/地区可用时，此参数非常有用。
*   `price`: 我们可以通过 `"all"` 来同时退回免费和付费应用，`"free"` 退回免费应用，`"paid"` 退回付费应用。
...
[ { title: 'CNET: Best Tech News, Reviews, Videos & Deals',
    ...
    free: true,
    summary:
     'Breaking technology news and expert product how-tos, comparisons, advice & tips',
    scoreText: '4.0',
    score: 4 },
  { title: 'Tech Coach',
...
### 3. `suggest()`
它接受一个字符串输入，并返回一个包含五个建议的列表来完成我们的搜索查询。
...
*   `list()`: 它返回 Google Play 的一个集合中的应用程序列表。
*   `developer()`: 它按照给定的开发人员名称返回应用程序列表。
*   `reviews()`: 返回当前指定 app 的一整页评论。
*   `similar()`: 它返回与指定的应用程序相似的应用程序列表。
*   `categories()`: 返回谷歌 Play 商店上可用的类别列表。