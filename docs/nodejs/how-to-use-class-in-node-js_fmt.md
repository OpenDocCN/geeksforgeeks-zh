...
## 简介
在现代 JavaScript 中，有一个更高级的 `class` 构造，它引入了对面向对象编程有用的伟大的新特性。因为我们可以用两种方式定义函数，即函数表达式和函数声明。
...
## 类体和方法定义
类的体在花括号 `{}` 内，这里是定义类成员如方法或构造函数的地方。构造函数方法是创建和初始化用类创建的对象的特殊方法。构造函数可以使用 `super` 关键字来调用超类的构造函数。