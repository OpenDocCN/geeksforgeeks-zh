...
> 原文: [https://www.geeksforgeeks.org/node-js-crypto-pbkdf2sync-method/](https://www.geeksforgeeks.org/node-js-crypto-pbkdf2sync-method/)
...
*   **password**: 密码。类型为 `string`、`Buffer`、`TypedArray`、`DataView`。
*   **salt**: 盐值。应尽可能唯一。推荐的盐是任意的，但无论如何它至少应为 16 字节长。其类型为 `string`、`Buffer`、`TypedArray` 或 `DataView`。
*   **iterations**: 迭代次数。必须是一个数字，并应尽可能设置得高。因此，迭代次数越多，派生的密钥越安全，但在这种情况下，完成所需的时间也越长。类型为数字。
*   **keylen**: 所需的密钥字节长度。类型为数字。
*   **digest**: 摘要算法。是一个字符串类型的摘要算法。

## 返回类型:

返回派生键作为 `Buffer`。

下面的例子说明了 `crypto.pbkdf2Sync()` 方法在 Node.js 中的使用:
...
[https://nodejs.org/api/crypto.html#crypto_crypto_pbkdf2sync_password_salt_iterations_keylen_digest](https://nodejs.org/api/crypto.html#crypto_crypto_pbkdf2sync_password_salt_iterations_keylen_digest)