...
> 原文: [https://www.geeksforgeeks.org/node-js-buffer-writeuintbe-method/](https://www.geeksforgeeks.org/node-js-buffer-writeuintbe-method/)
...
*   `offset`: 指定在写入缓冲区之前要跳过的字节数。`offset` 的值位于 `0 <= offset <= buf.length - byteLength` 范围内。
*   `byteLength`: 指定写入缓冲区的字节数。`byteLength` 的值位于 `0 < byteLength <= 6` 范围内。
...
**参考:** [https://nodejs.org/api/buffer.html#buffer_buf_writeuintbe_value_offset_bytelength](https://nodejs.org/api/buffer.html#buffer_buf_writeuintbe_value_offset_bytelength)