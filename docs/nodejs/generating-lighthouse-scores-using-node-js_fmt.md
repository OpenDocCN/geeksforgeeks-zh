...
> 原文: [https://www.geeksforgeeks.org/generating-lighthouse-scores-using-node-js/](https://www.geeksforgeeks.org/generating-lighthouse-scores-using-node-js/)

**Lighthouse** 是一个开源工具，用来生成任何网页的性能报告。我们可以使用 CLI 或 Chrome 扩展为网页生成 Lighthouse 评分，但这里描述了如何为一个 CSV 文件中存在的多个 URL 生成 Lighthouse 评分，并为两个 **移动** 和 **桌面** 视图生成由 **性能、可访问性、最佳实践** 和 **SEO** 评分组成的 CSV 文件。

我们使用 npm 提供的 `lighthouse` 包，也使用 `chrome-launcher` 来生成数十个网站。我们可以下载该软件包并按如下方式使用它们:
...
导入这个第三方包后，我们准备通过使用 Node.js 提供的默认 `fs` 包的 `readFileSync` 方法来进一步导入 URL 文件，并将这些 URL 存储在一个数组中，我们将从该数组中遍历所有 URL，并为 Mobile 和 Desktop 策略生成分数。
...
6.  遍历数组中的所有 URL，并对每个 URL 执行以下操作:
    ...
    *   调用 `lighthouse` 函数，`url` 即 `array[i]` 作为第一个参数，`options` 对象作为第二个参数。
    *   返回的对象将存储调用 `lighthouse` 函数后收到的详细信息。
...
安装 Lighthouse 包后运行 `app.js` 文件。

## 输出

![](img/a9ff00fb62cc0d513edd8d49c12ded32.png)

Lighthouse 报告文件 (`lhreport.csv`) 在我们运行程序后生成。