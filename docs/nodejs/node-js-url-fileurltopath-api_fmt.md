...
> 哎哎哎:# t0]https://www.geeksforgeeks.org/node-js-URL-fileurltopath-api/
...
该函数接受单参数 `url`，保存要转换为路径的文件 url 字符串或对象。

## 返回值

返回一个字符串，代表完全解析的平台特定文件路径。

下面的程序说明了 `URL.fileURLToPath()` 方法在 Node.js 中的使用：
...
**注意:** 以上程序将使用 `node app.js` 命令编译运行。

**参考:** [https://nodejs.org/api/url.html#url_url_fileurltopath_url](https://nodejs.org/api/url.html#url_url_fileurltopath_url)