...
`os.freemem()`方法是`os`模块的内置应用编程接口，用于获取系统空闲内存量。
...
该方法返回一个整数值，以字节为单位指定可用系统内存量。

下面的例子说明了`os.freemem()`方法在Node.js中的使用：
...
**参考资料：** [https://nodejs.org/api/os.html#os_os_freemem](https://nodejs.org/api/os.html#os_os_freemem)