...
`zlib.gzip()`方法是 `zlib` 模块的内置应用编程接口，用于压缩一大块数据。
...
以下示例说明了 Node.js 中 `zlib.gzip()`方法的使用:

### 例 1:

```js
// Node.js program to demonstrate the gzip() method
...
[https://nodejs.org/api/zlib.html#zlib_zlib_gzip_buffer_options_callback](https://nodejs.org/api/zlib.html#zlib_zlib_gzip_buffer_options_callback)