...
> 原文：[https://www.geeksforgeeks.org/how-to-run-many-parallel-http-requests-using-node-js/](https://www.geeksforgeeks.org/how-to-run-many-parallel-http-requests-using-node-js/)
...
CPU 性能的最佳解决方案是 Worker Thread。这个模块用在 `Node.js` 中，因为它对执行繁重的 `JavaScript` 任务很有用。
...
**注意:** 我们必须使用`--experimental-worker`，因为工人线程模块仍然是实验性的。
