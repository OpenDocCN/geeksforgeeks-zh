...
方法是节点内网址模块的一个内置应用编程接口。JS。方法用于返回序列化的网址。返回值相当于 `url.href` 和 `url.toJSON()` 的值。
...
**参考:** [https://nodejs.org/api/url.html#url_url_tostring](https://nodejs.org/api/url.html#url_url_tostring)