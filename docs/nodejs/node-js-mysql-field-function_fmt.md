...
`FIELD()`函数是MySQL中的内置函数，用于获取一组表达式中某个值第一次出现的位置。对于字符串值，它不区分大小写。
...
返回一组表达式中某个值第一次出现的位置。如果没有发现，那么它将返回0。
...
db_con.connect((err) => {
    ...
    // Here is the query
    let query = "SELECT FIELD('geek', 'Geeek', 'gEEK', 'geeK') AS Output";
