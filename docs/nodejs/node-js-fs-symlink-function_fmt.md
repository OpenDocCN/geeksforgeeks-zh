...
`fs.symlink()`方法用于创建到指定路径的 symlink。这将创建一个使`path`指向`target`的链接。相对目标是相对于链接的父目录。
...
## 参考:
[https://nodejs.org/api/fs.html#fs_fs_symlink_target_path_type_callback](https://nodejs.org/api/fs.html#fs_fs_symlink_target_path_type_callback)