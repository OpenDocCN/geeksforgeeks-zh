...
> 原文: [https://www.geeksforgeeks.org/node-js-trace_events-getenabledcategories-method/](https://www.geeksforgeeks.org/node-js-trace_events-getenabledcategories-method/)

`trace_events.getEnabledCategories()`（在 v10.0.0 中添加）方法是 `trace_events` 模块的内置 API，用于显示由所有当前启用的跟踪对象的并集确定的当前启用的跟踪事件类别集。默认情况下，它的状态被 V8 运行时禁用，所有启用的类别都使用 `--trace-event-categories` 标志提取。
...
为了使用这个方法，我们需要使用 `trace_events.createTracing()` 方法创建跟踪事件，并且我们需要导入 `trace_events` 模块。
...
返回类型为 `<string>`：返回当前启用的所有跟踪事件类别列表，列表之间用逗号分隔。
...
>> false
>> Following tracing events are enabled myapp.category,node,node.async_hooks,node.perf.usertiming,node.perf.timerify
>> None of the Tracing events are enabled
```

## 参考

[https://nodejs.org/api/tracing.html#tracing_trace_events_getenabledcategories](https://nodejs.org/api/tracing.html#tracing_trace_events_getenabledcategories)