...
`stats.ctime` 属性是 `fs` 模块的一个内置应用编程接口，属于 `fs.Stats` 类，用于获取上次更改文件状态时的时间戳。
...
[https://nodejs.org/api/fs.html#fs_stats_ctime](https://nodejs.org/api/fs.html#fs_stats_ctime)