...
> 原文：[https://www.geeksforgeeks.org/node-js-crypto-randomfillsync-method/](https://www.geeksforgeeks.org/node-js-crypto-randomfillsync-method/)

`crypto.randomFillSync()`方法是加密模块的内置应用编程接口，用于返回作为`buffer`参数传递的对象。
...
*   `buffer`：该参数保存缓冲区、数据类型或数据视图类型的数据。
*   `offset`：是默认值为`0`的数字。
*   `size`：是一个数字，默认值为`(buffer.length - offset)`。

## 返回值

返回`buffer`、类型数据或数据视图类型的数据。

以下示例说明了`crypto.randomFillSync()`方法在Node.js中的使用。
...
这里，任何类型的`TypedArray`或`DataView`实例都作为缓冲区传递。

## 参考

[https://nodejs.org/api/crypto.html#crypto_crypto_randomfillsync_buffer_offset_size](https://nodejs.org/api/crypto.html#crypto_crypto_randomfillsync_buffer_offset_size)