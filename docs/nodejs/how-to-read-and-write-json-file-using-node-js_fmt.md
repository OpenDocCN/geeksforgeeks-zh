...
> 原文：[https://www.geeksforgeeks.org/how-to-read-and-write-json-file-using-node-js/](https://www.geeksforgeeks.org/how-to-read-and-write-json-file-using-node-js/)
...
读取 JSON 文件最简单的方法是使用 `require()` 方法在 node.js 文件中引入它。
...
我们也可以使用 node.js 的 `fs` 模块来读取文件。`fs` 模块以字符串格式返回文件内容，因此我们需要使用内置方法 `JSON.parse()` 将其转换为 JSON 格式。
...
我们可以使用 node.js 的 `fs` 模块将数据写入 JSON 文件。我们可以使用 `writeFile` 方法将数据写入文件。
...
1.  使用上述方法之一读取文件。
2.  使用 `.push()` 方法添加数据。
3.  使用 `JSON.stringify()` 方法将新数据写入文件，将数据转换为字符串。
