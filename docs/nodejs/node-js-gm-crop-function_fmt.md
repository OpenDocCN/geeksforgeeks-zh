...
`crop()`函数是 GraphicsMagick 库中的一个内置函数，用于通过指定坐标来移除图像中不需要的区域。该函数在成功时返回真值。
...
*   `width`: 此参数用于指定要裁剪图像的宽度。
*   `height`: 此参数用于指定要裁剪图像的高度。
*   `x`: 此参数用于指定裁剪图像的宽度。
*   `y`: 此参数用于指定裁剪图像的宽度。
*   `percentage`: 该参数用于指定布尔值，如果为真，则前面提到的所有值都被视为以像素为单位的百分比。
