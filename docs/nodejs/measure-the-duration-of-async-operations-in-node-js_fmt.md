...
> 原文：[https://www.geeksforgeeks.org/measure-the-duration-of-async-operations-in-node-js/](https://www.geeksforgeeks.org/measure-the-duration-of-async-operations-in-node-js/)
...
在下面提到的代码示例中，我们使用 [`setTimeout()`](geeksforgeeks.org/java-script-settimeout-setinterval-method/) 方法模拟了一个异步操作。我们在返回承诺的异步函数中执行加法操作。这个承诺在 2 秒钟后通过加法的结果被解决。最后，我们计算并显示在我们的[立即调用的函数表达式](https://www.geeksforgeeks.org/javascript-immediately-invoked-function-expressions-iife/)中执行加法操作所花费的时间。
...
我们可以在命令行上使用以下命令执行我们的 `app.js` 文件。
