...
> 原文: [https://www.geeksforgeeks.org/node-js-fspromises-truncate-method-2/](https://www.geeksforgeeks.org/node-js-fspromises-truncate-method-2/)
...
该方法返回一个承诺，该承诺将在成功时被无参数解析，或者在出错时被错误对象拒绝（例如给定路径是指向目录的路径，或者给定路径不存在）。
...
// Fetching contents before truncate
    const oldContents = await readFileContents('./test.txt')
...
// Fetching contents before truncate
    const newContents = await readFileContents('./test.txt')
...
[https://nodejs.org/dist/latest-v14.x/docs/api/fs.html#fs_fspromises_truncate_path_len](https://nodejs.org/dist/latest-v14.x/docs/api/fs.html#fs_fspromises_truncate_path_len)