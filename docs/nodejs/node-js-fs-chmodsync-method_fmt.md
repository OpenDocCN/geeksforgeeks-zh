...
`fs.chmodSync()`方法用于同步更改给定路径的权限。这些权限可以使用对应于各自文件模式的字符串常量或八进制数来指定。
...
*   `mode`: 是字符串或八进制整数常量，表示要授予的权限。逻辑或运算符可用于分隔多个权限。

以下示例说明了 Node.js 中的 `fs.chmodSync()`方法:

### 示例 1

此示例显示了使用字符串常量和 OR 运算符来赋予文件权限。
...
**参考:** [https://nodejs.org/api/fs.html#fs_fs_chmodsync_path_mode](https://nodejs.org/api/fs.html#fs_fs_chmodsync_path_mode)