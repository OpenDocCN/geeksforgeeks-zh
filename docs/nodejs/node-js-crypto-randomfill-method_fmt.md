...
> 原文: [https://www.geeksforgeeks.org/node-js-crypto-randomfill-method/](https://www.geeksforgeeks.org/node-js-crypto-randomfill-method/)

`crypto.randomFill()`方法与`crypto.randomBytes()`方法相同，但唯一的区别是这里的第一个参数是将被填充的`buffer`，它还有一个作为参数传递的回调函数。但是，如果回调函数不可用，则会引发错误。
...
这里，任何类型的`TypedArray`或`DataView`实例都作为缓冲区传递，在每次运行中，您将获得不同的输出。

参考: [https://nodejs.org/api/crypto.html#crypto_crypto_randomfill_buffer_offset_size_callback](https://nodejs.org/api/crypto.html#crypto_crypto_randomfill_buffer_offset_size_callback)