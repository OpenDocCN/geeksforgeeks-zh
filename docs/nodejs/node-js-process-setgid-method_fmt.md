...
> 原文: [https://www.geeksforgeeks.org/node-js-process-setgid-method/](https://www.geeksforgeeks.org/node-js-process-setgid-method/)

`process.setgid()`方法是流程模块的内置应用编程接口，用于设置 Node.js 流程的组标识。
...
这个功能只会在 POSIX 平台上工作。在 windows 或 android 平台上不可用，因此会导致错误，即 `TypeError`，`setgid` 不是函数。

以下示例说明了 `process.setgid()`方法在 Node.js 中的使用。
...
[https://nodejs.org/api/process.html#process_process_setgid_id](https://nodejs.org/api/process.html#process_process_setgid_id)