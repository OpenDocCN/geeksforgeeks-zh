...
> 原文: [https://www.geeksforgeeks.org/node-js-bot-senddocument-method/](https://www.geeksforgeeks.org/node-js-bot-senddocument-method/)

Node.js 电报机器人 API 中使用了 `Bot.sendDocument()` 方法。这个 Node.js 模块与官方的电报机器人 API 进行交互。此方法用于发送扩展名为 `.pdf`、`.docx`、`.txt` 等的文档。
...
1.  首先，通过电报从 BotFather 那里获得 **GET BOT_TOKEN**。只需在电报中搜索 **BotFather** 并选择验证过的，如下所示：
...
4.  现在只需从 BotFather 那里复制令牌。要删除令牌，只需在 BotFather 中搜索 `/deletebot`。
