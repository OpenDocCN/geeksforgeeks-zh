...
处理文件的创建、读取、删除等操作，Node.js 提供了一个名为 `FS`（文件系统）的内置模块。Node.js 通过提供标准 POSIX 函数的包装器来提供文件输入/输出的功能。根据用户要求，所有文件系统操作都可以有同步和异步形式。
...
*   **同步方法：** 它们被称为**阻塞函数**，因为它等待每个操作完成，只有在此之后，它才执行下一个操作，因此阻塞下一个命令的执行，即一个命令直到查询已经完成执行以获得来自先前命令的所有结果后才会被执行。
...
*   **异步和同步示例：** 创建一个名为 `input.txt` 的文本文件，内容如下：
...
`fs.open()` 方法用于创建、读取或写入文件。`fs.readFile()` 方法仅用于读取文件，`fs.writeFile()` 方法仅用于写入文件，而 `fs.open()` 方法对文件执行多个操作。首先，我们需要加载 `fs` 类，这是一个访问物理文件系统的模块。
...
*   **path：** 保存要读取的文件名，如果存储在其他位置，保存整个路径。
*   **flags：** 标志表示要打开的文件的行为。所有可能的值都是 (`r`, `r+`, `rs`, `rs+`, `w`, `wx`, `w+`, `wx+`, `a`, `ax`, `a+`, `ax+`)。
*   **mode：** 设置文件模式，即 `r` 读、`w` 写、`r+` 读写。它默认设置为读写。
*   **callback：** 它是读取文件后调用的回调函数。它需要两个参数：
    *   `err`：如果出现任何错误。
    *   `fd`：文件描述符。它在打开操作执行后被调用。
...
`fs.read()` 方法用于读取 `fd` 指定的文件。此方法将整个文件读入缓冲区。
...
*   **position：** 这是一个整数，指定从文件中的哪个位置开始读取。如果位置为空，将从当前文件位置读取数据。
*   **callback：** 是读取文件后调用的回调函数。它需要两个参数：
    *   `err`：如果出现任何错误。
    *   `bytesRead`：实际读取的字节数。
    *   `buffer`：缓冲区对象。
...
如果文件已经存在，此方法将覆盖文件。`fs.writeFile()` 方法用于将指定的数据异步写入文件。默认情况下，如果文件存在，它将被替换。“选项”参数可用于修改方法的功能。
...
*   **path：** 它是一个字符串、缓冲区、网址或文件描述整数，表示必须写入的文件路径。使用文件描述符将使其行为类似于 `fs.write()` 方法。
*   **data：** 它是将被写入文件的字符串、缓冲区、类型数据或数据视图。
*   **options：** 它是一个字符串或对象，可用于指定将影响输出的可选参数。它有三个可选参数：
    *   `encoding`：是指定文件编码的字符串值。默认值为 `"utf8"`。
    *   `mode`：指定文件模式的整数值。默认值为 `0o666`。
    *   `flags`：它是一个字符串值，指定写入文件时使用的标志。默认值为 `"w"`。
...
*   **filepath：** 是指定文件路径的字符串。
*   **data：** 是强制的，它包含您追加到文件中的数据。
*   **options：** 是指定编码/模式/标志的可选参数。
*   **callback：** 函数是必选项，在文件追加数据完成时调用。
...
`fs.close()` 方法用于异步关闭给定的文件描述符，从而清除与之关联的文件。这将允许文件描述符被其他文件重用。在文件描述符上执行其他操作时调用 `fs.close()` 可能会导致未定义的行为。
...
*   **path：** 它是一个字符串、缓冲区或网址，代表必须删除的文件或符号链接。
*   **callback：** 这是一个在执行方法时会被调用的函数。
    *   `err`： 这是一个如果方法失败会抛出的错误。
