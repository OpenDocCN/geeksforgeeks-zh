# 使用 EJS 进行 Node.js 服务器端渲染(SSR)

> 原文: [https://www.geeksforgeeks.org/node-js-server-side-rendering-ssr-using-ejs/](https://www.geeksforgeeks.org/node-js-server-side-rendering-ssr-using-ejs/)
...
要执行服务器端渲染，我们使用 EJS 模块的 `renderFile()` 方法，该方法帮助我们在服务器端渲染 `ejs` 文件。
...
2.  确保您已经使用以下命令安装了 `express` 和 `ejs` 模块：
...
4.  现在在浏览器中输入 `localhost:8000` 显示 `ejs` 页面，看到如下结果：
