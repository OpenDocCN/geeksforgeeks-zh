...
> 原文：[https://www.geeksforgeeks.org/convert-xml-data-into-json-using-node-js/](https://www.geeksforgeeks.org/convert-xml-data-into-json-using-node-js/)
...
[JSON:](https://www.geeksforgeeks.org/json-full-form/) JSON 代表 JavaScript 对象符号。它是一种基于文本的数据交换格式，用于维护数据的结构。JSON 是 XML 数据交换格式的替代。与 XML 相比，结构化数据很容易。它支持数组和对象等数据结构，以及在服务器上快速执行的 JSON 文档。它也是一种独立于语言的格式，是从 JavaScript 派生出来的。JSON 的官方媒体类型是 `application/json`，用于保存这些文件（`.json` 扩展名）。
