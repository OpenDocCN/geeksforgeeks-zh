...
`socket.setTTL()`方法是`dgram`模块内`Socket`类的内置应用编程接口，用于设置或清除`IP_TTL` socket选项，该选项有助于指定允许数据包通过指定路由器或网关的IP跳数。
...
**参数:** 该方法取整数值，表示允许数据包通过指定路由器或网关的IP跳数。
...
**参考:** [https://nodejs.org/dist/latest-v12.x/docs/api/dgram.html#dgram_socket_setttl_ttl](https://nodejs.org/dist/latest-v12.x/docs/api/dgram.html#dgram_socket_setttl_ttl)