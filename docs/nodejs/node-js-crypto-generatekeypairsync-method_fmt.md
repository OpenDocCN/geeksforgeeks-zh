...
> 原文: [https://www.geeksforgeeks.org/node-js-crypto-generatekeypairsync-method/](https://www.geeksforgeeks.org/node-js-crypto-generatekeypairsync-method/)

`crypto.generateKeyPairSync()` 方法是加密模块的内置 API，用于生成指定类型的新非对称密钥对。例如，当前支持的密钥类型有 RSA、DSA、EC、Ed25519、Ed448、X25519、X448 和 DH。此外，如果在这里声明了选项的 `publicKeyEncoding` 或 `privateKeyEncoding`，那么这个函数的行为就像在其输出中调用了 `keyObject.export()` 一样。否则，密钥的特定部分将作为密钥对象返回。
...
*   `options`：为对象类型。它可以保存以下参数：
    ...
    9.  `publicKeyEncoding`：它保存一个对象。
    10. `privateKeyEncoding`：它保存一个对象。

## 返回值

它返回一个给定类型的新的非对称密钥对，即它返回一个包含私钥和公钥的对象，公钥保存字符串、缓冲区和密钥对象。
...
[https://nodejs.org/api/crypto.html#crypto_crypto_generatekeypairsync_type_options](https://nodejs.org/api/crypto.html#crypto_crypto_generatekeypairsync_type_options)