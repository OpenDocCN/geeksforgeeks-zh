...
> 原文: [https://www.geeksforgeeks.org/node-js-fs-extra-move-function/](https://www.geeksforgeeks.org/node-js-fs-extra-move-function/)

`move()` 函数将文件或目录从源移动到用户指定的目标。如果你想将一个文件移动到一个已经存在同名文件的文件夹中，如果我们已经将覆盖选项设置为 `true`，该函数将**覆盖**该文件，否则它将抛出一个错误，即文件夹中已经存在一个**文件**。
...
*   `options`: 是属性 `overwrite` 的对象，可以是 `true` 也可以是 `false`。默认情况下，`overwrite: false`。如果设置为 `true`，则如果目标文件夹中存在同名文件，该文件将被覆盖。
*   `callback`: 是执行 `move()` 函数后会调用的函数。它要么导致错误，要么导致成功。这是一个可选参数，我们也可以用 promises 代替回调函数。
...
**注意:** 如果没有将 `overwrite` 属性设置为 `false`，上述程序将导致以下错误:
