# 在 Node.js 中使用 NEXMO API 发送短信

> 原文: [https://www.geeksforgeeks.org/sending-sms-using-nexmo-api-in-node-js/](https://www.geeksforgeeks.org/sending-sms-using-nexmo-api-in-node-js/)

## 简介
短信是手机之间发送短信的常用方式，但这些短信可以使用 Node.js 中的 API 发送，现在市场上有很多 API 向用户发送类似 Twilio、Exotel 等短信，但其中比较流行的是 Nexmo。

### 特性
在代码中集成这个模块非常简单高效。使用 Nexmo 模块，用户可以发送短信，也可以使用 Nexmo 语音 API 发送语音呼叫。
