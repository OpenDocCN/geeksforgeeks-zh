...
`path.join()`方法用于使用特定于平台的分隔符连接多个路径段，以形成单个路径。在连接发生后，最终路径被规范化。路径段是使用逗号分隔值指定的。
...
*   `Path`：这是一个以逗号分隔的路径序列，将被连接在一起以形成最终路径。
...
**参考：**[https://nodejs.org/api/path.html#path_path_join_paths](https://nodejs.org/api/path.html#path_path_join_paths)