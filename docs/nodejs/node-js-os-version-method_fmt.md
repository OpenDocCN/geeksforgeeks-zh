...
`os.version()`方法用于识别操作系统内核的版本。该信息通过在 POSIX 系统上调用`uname`并在 Windows 系统上调用`RtlGetVersion()`或`GetVersionExW()`来确定。
...
[https://nodejs.org/api/os.html#os_os_version](https://nodejs.org/api/os.html#os_os_version)