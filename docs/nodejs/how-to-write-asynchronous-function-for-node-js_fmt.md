...
> 原文：[https://www.geeksforgeeks.org/how-to-write-asynchronous-function-for-node-js/](https://www.geeksforgeeks.org/how-to-write-asynchronous-function-for-node-js/)
...
使用以下命令从 Node.js 中的 `npm` 安装 `async`：

```js
npm i async
```

使用 `require()` 方法在 Node.js 项目中使用 `async`。
...
## 示例 2：创建一个异步函数，使用 await 计算 Node.js 中两个数的和。

执行上面的过程来创建一个 Node.js 项目。
