...
`fs.chownSync()`方法用于同步更改给定路径的所有者和组。该功能接受用户 id 和组 id，可用于设置各自的所有者和组。它不返回任何东西。
...
**参考:** [https://nodejs.org/api/fs.html#fs_fs_chownsync_path_uid_gid](https://nodejs.org/api/fs.html#fs_fs_chownsync_path_uid_gid)