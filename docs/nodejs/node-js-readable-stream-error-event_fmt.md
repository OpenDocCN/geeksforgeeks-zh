...
> 原文：[https://www.geeksforgeeks.org/node-js-readable-stream-error-event/](https://www.geeksforgeeks.org/node-js-readable-stream-error-event/)

可读流中的`“错误”`事件可以随时发出。当隐藏流由于某种隐藏的内部故障而无法生成数据时，或者当流的实现推送无效的数据块时，就会发生这种情况。此外，单个`Error`对象作为参数传递给侦听器回调。
