...
`zlib.close()`方法是 `zlib` 模块的内置应用编程接口，用于关闭底层句柄。
...
下面的例子说明了 `zlib.close()`方法在 Node.js 中的使用:
...
## 参考:
[https://nodejs.org/api/zlib.html#zlib_zlib_close_callback](https://nodejs.org/api/zlib.html#zlib_zlib_close_callback)