...
> 原文：[https://www.geeksforgeeks.org/global-process-and-buffer-in-node-js/](https://www.geeksforgeeks.org/global-process-and-buffer-in-node-js/)

## 全局

`global` 对象在 Node.js 的所有模块中都是可用的，并且在本地作用于它们各自的模块。
...
`Process` 对象是一个全局对象，提供关于 Node.js process 的信息并对其进行控制。因为它是全局的，所以可以在项目中使用，而无需从任何模块导入它。
...
此事件在 Node.js 事件循环被万圣节化且没有安排额外工作之前触发。一般情况下，当没有剩余进程时，Node.js 进程将退出，但是在 `beforeExit` 上注册的侦听器可以进行异步调用。
...
此时无法阻止事件循环的退出，一旦所有 `exit` 侦听器都运行完，Node.js 进程将终止。
...
`Buffer` 类是用来处理原始二进制数据的。每个缓冲区对应于 V8 之外分配的一些原始内存。缓冲区的作用有点像整数数组，但不可调整大小，并且有一大堆专门针对二进制数据的方法。
