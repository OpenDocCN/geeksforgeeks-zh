...
> 原文：`https://www.geeksforgeeks.org/node-js-worker-threads/`

Node.js 中的工作线程对于执行繁重的 JavaScript 任务非常有用。在线程的帮助下，Worker 使并行运行 javascript 代码变得容易，从而使它变得更快、更高效。我们甚至可以在不干扰主线的情况下完成繁重的任务。旧版本的节点中没有引入工作线程。因此，首先更新你的 Node.js 来开始。
...
在这里，`workerData` 和 `parentPort` 是工人线程的一部分。`workerData` 用于从线程中获取数据，`parentPort` 用于操作线程。`postMessage()` 方法用于在控制台中发布给定的消息，方法是采用 `workerData` 获取的文件名。
