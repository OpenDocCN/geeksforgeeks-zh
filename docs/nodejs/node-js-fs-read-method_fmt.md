...
*   `length`: 指定要读取的字节数的整数。
*   `position`: 一个整数，指定从文件中的哪个位置开始读取。如果位置为空，则从当前文件位置读取数据。
*   `callback`: 回调函数接受三个参数即。(错误，字节前导，缓冲区)。

## 解释

`fs.open()` 方法打开文件并返回文件描述符。文件描述符是由内核存储在文件描述符表中的数字或索引，用于唯一标识计算机操作系统中打开的文件。`fs.read()` 方法使用文件描述符读取文件并存储在缓冲区中。缓冲区的内容作为输出打印出来。`fs.close()` 方法用于关闭文件。
