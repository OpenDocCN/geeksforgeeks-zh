...
> 原文：[https://www.geeksforgeeks.org/node-js-stream-writable-_write-method/](https://www.geeksforgeeks.org/node-js-stream-writable-_write-method/)

`writable._write()` 方法是 Stream 模块的内置 API，用于实现可写流。`_write()` 方法在定义它的类中加上了下划线。此外，用户程序不能直接调用它。此方法通过使用子类实现，并且仅由内部可写类方法调用。
...
*   `chunk`：是要写入的数据，可以是 Buffer、字符串或任何类型。
*   `encoding`：如果 `chunk` 是字符串类型，则使用的编码类型。
*   `callback`：是检查写操作完成还是失败而调用的方法。如果调用没有成功，传递给回调的第一个参数必须是 `Error` 对象，如果写入成功，则为 `null`。

下面的例子说明了 `writable._write()` 方法在 Node.js 中的用法。
...
**参考：**[https://nodejs.org/api/stream.html#stream_writable_write_chunk_encoding_callback_1](https://nodejs.org/api/stream.html#stream_writable_write_chunk_encoding_callback_1)