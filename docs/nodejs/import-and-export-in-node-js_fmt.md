...
模块是在 Node.js 中创建的 JavaScript 文件。每次有新文件 `.js` 扩展被创建，它就变成了一个模块。
...
我们需要导入模块，以便在另一个文件中使用导入模块中定义的函数。`require()` 返回的结果存储在一个变量中，该变量用于使用点表示法调用函数。
...
通过在 `lib.js` 前面加上目录名，在目录中导入 `lib.js` 文件。
