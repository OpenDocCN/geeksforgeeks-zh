...
> 原文: [https://www.geeksforgeeks.org/node-js-fs-appendfilesync-function/](https://www.geeksforgeeks.org/node-js-fs-appendfilesync-function/)

`fs.appendFileSync()`方法用于将给定数据同步追加到文件中。如果新文件不存在，则会创建一个新文件。可选选项参数可用于修改操作的行为。
...
## 示例 2:
该示例显示了可选参数的用法，用于更改操作的文件编码和标志。“w”标志代替文件的内容，而不是附加到它上面。
...
## 参考:
[https://nodejs.org/api/fs.html#fs_fs_appendfilesync_path_data_options](https://nodejs.org/api/fs.html#fs_fs_appendfilesync_path_data_options)