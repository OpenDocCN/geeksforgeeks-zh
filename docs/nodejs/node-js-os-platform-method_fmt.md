...
`os.platform()`方法是`os`模块的内置应用编程接口，用于获取操作系统平台。
...
这个方法返回一个代表操作系统平台的字符串。返回值可以是`"aix"`、`"android"`、`"darwin"`、`"freebsd"`、`"linux"`、`"openbsd"`、`"sunos"`和`"win32"`中的一个。该值在编译时设置。

以下示例说明了`os.platform()`方法在Node.js中的使用。

### 示例 1

```js
// Node.js program to demonstrate the os.platform() method
...
参考: [https://nodejs.org/api/os.html#os_os_platform](https://nodejs.org/api/os.html#os_os_platform)