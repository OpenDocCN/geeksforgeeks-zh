...
Node.js 电报机器人 API 中使用了 `Bot.sendPhoto()` 方法。这个 Node.js 模块与官方的电报机器人 API 进行交互。这种方法用于发送不同格式的照片。
...
1.  首先，通过电报从 BotFather 那里获得 **GET BOT_TOKEN** 。只需在电报中搜索 `BotFather`，并选择如下所示的验证过的:
