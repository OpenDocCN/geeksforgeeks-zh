...
`process.env` 属性是 `process` 模块的内置 API，用于获取用户环境。
...
下面的例子说明了 `process.env` 属性在 Node.js 中的使用:
...
**参考:** [https://nodejs.org/api/process.html#process_process_env](https://nodejs.org/api/process.html#process_process_env)