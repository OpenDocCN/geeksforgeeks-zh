...
节点js `Telegraf`模块使用`bot.hears()`方法。该模块提供各种功能与官方电报机器人应用编程接口进行交互。当给定的捕获消息与给定的关键字匹配时，此方法执行。
...
*   `keyword`:可以是数组、字符串，也可以是需要匹配的混合。
*   `callback`:该函数封装电报更新信息，当关键字匹配时执行。
...
**第一步:**首先，从电报中的BotFather处获取`GET BOT_TOKEN`。只需在电报中搜索`BotFather`，并选择如下所示的验证过的:
...
**步骤3:**现在输入机器人的名称，并且必须是唯一的。

![](img/cef7ef3daa2782f014c9b59a14b32547.png)

**第4步:**现在只需从BotFather那里复制令牌。要删除令牌，只需在BotFather中搜索`/deleteToken`。
