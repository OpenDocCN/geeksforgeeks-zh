...
> 原文：[https://www.geeksforgeeks.org/node-js-mysql-create-table-using-sequelize/](https://www.geeksforgeeks.org/node-js-mysql-create-table-using-sequelize/)
...
*   要使用 `Sequelize` 在 MySQL 和 Node.js 之间建立连接，请访问[如何在 Node.js 中使用 Sequelize](https://www.geeksforgeeks.org/how-to-use-sequelize-in-node-js/)。
*   成功连接后，我们需要三个文件：
    1.  `SequelizeDemo/app.js` 这是我们的根文件。
    2.  `SequelizeDemo/utils/database.js` 负责 MySql 连接。
    3.  `SequelizeDemo/models/user.js` 负责定义模型。

**配置 `database.js`：** `SequelizeDemo/utils/database.js`

*   建立连接是文章[中提到的方式如何在 Node.js 中使用 sequelize](https://www.geeksforgeeks.org/how-to-use-sequelize-in-node-js/)。现在，我们可以使用从 `SequelizeDemo/utils/database.js` 中导出的 `Sequelize` 对象来定义我们的模型。
...
*   使用 `SequelizeDemo/models/user.js` 文件定义模型和表之间的映射，使用 `define` 方法。
...
*   要了解更多关于 `Sequelize` 数据类型的信息，请访问[数据类型](https://sequelize.org/v5/manual/data-types.html)。
*   在 `SequelizeDemo/models/user.js` 文件中，我们已经定义了模型。
