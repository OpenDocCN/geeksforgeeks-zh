...
> 原文: [https://www.geeksforgeeks.org/node-js-crypto-publicdecrypt-method/](https://www.geeksforgeeks.org/node-js-crypto-publicdecrypt-method/)

`crypto.publicDecrypt()`方法用于使用密钥对缓冲区的内容进行解密，该密钥先前使用相应的私钥进行了加密，即`crypto.privateEncrypt()`方法。
...
*   `key`: 其类型是`object`、`string`、`Buffer`或`KeyObject`，包含两个参数，如下所示：
    1.  `passphrase`: 它是可选的私钥类型，可以是字符串或缓冲区。
    2.  `padding`: 是`crypto.constants`中定义的可选填充值，可以是`crypto.constants.RSA_NO_PADDING`或`crypto.constants.RSA_PKCS1_PADDING`。其类型是`password constant`。
*   `buffer`: 其类型是`Buffer`、`TypedArray`或`DataView`。

## 返回类型:

返回一个包含解密内容的新缓冲区。

下面的例子说明了`crypto.publicDecrypt()`方法在Node.js中的使用:
...
[https://nodejs.org/api/crypto.html#crypto_crypto_publicdecrypt_key_buffer](https://nodejs.org/api/crypto.html#crypto_crypto_publicdecrypt_key_buffer)