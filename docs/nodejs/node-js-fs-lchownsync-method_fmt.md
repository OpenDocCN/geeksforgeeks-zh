...
`fs.lchownSync()`方法用于同步更改给定路径的所有者和组，但是，如果路径是一个符号链接，它不尊重符号链接，这与`fs.chownSync()`方法不同，后者不将链接引用到它们的路径。
...
此示例显示了`chownSync()`和`lchownSync()`在取消符号链接引用方面的比较。
...
**参考:** [https://nodejs.org/api/fs.html#fs_fs_lchownsync_path_uid_gid](https://nodejs.org/api/fs.html#fs_fs_lchownsync_path_uid_gid)