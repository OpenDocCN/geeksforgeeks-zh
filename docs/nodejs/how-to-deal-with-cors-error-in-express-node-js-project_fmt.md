...
> 原文：[https://www.geeksforgeeks.org/how-to-deal-with-cors-error-in-express-node-js-project/](https://www.geeksforgeeks.org/how-to-deal-with-cors-error-in-express-node-js-project/)

CORS，也称为跨来源资源共享，应该启用，如果你想让你的客户端和服务器之间的请求时，他们在不同的网址。

让我们考虑客户端在 `http://localhost:5500` 上和服务器在 `http://localhost:5000` 上。现在，如果您尝试从客户端向服务器发出请求，您将收到一条错误消息，指出被 CORS 策略阻止。

![](img/6414014470ef6f460bfb87185d839fb5.png)

## 如何启用 CORS？

我们将使用 `cors`，一个 Node.js 包，在 Express Node.js 项目中启用 CORS。
...
*   **步骤 3:** 在根目录下创建 `client` 目录和 `server.js` 文件。然后在 `client` 目录下创建 `index.html` 和 `script.js`。
...
在 `index.html`、`script.js` 和 `server.js` 文件中写下以下代码。
...
如果您只想允许某个特定的源访问您的站点，那么 `corsOptions` 如下:
