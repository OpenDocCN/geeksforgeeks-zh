...
> 原文：[https://www.geeksforgeeks.org/how-to-send-attachments-and-email-using-nodemailer-in-node-js/](https://www.geeksforgeeks.org/how-to-send-attachments-and-email-using-nodemailer-in-node-js/)

为此，我们将使用名为`nodemailer`的包。这是一个让电子邮件发送变得非常容易的模块。要使用它，您需要使用以下命令进行安装：
```js
...
为了运行该文件，只需将 Bash Git 放入您的工作目录中，并键入以下命令：
```js
$ nodemon index.js
```
