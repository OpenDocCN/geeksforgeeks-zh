...
`dns.resolveMx()`方法是`dns`模块的内置应用编程接口，用于使用`dns`协议解析指定主机名的 Mx 或邮件交换记录。
...
*   `Callback`: 指定在主机名的 DNS 解析完成后要调用的函数。
    *   `Error`: 指定生成的错误。
    *   `Address`: 一个字符串数组，表示主机名的返回邮件交换记录。

## 返回值

此方法返回错误，通过回调函数寻址。这些数据作为参数传递给回调函数。
...
[https://nodejs.org/api/dns.html#dns_dns_resolvemx_hostname_callback](https://nodejs.org/api/dns.html#dns_dns_resolvemx_hostname_callback)