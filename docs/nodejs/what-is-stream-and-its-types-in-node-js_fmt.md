...
> 原文: [https://www.geeksforgeeks.org/what-is-stream-and-its-types-in-node-js/](https://www.geeksforgeeks.org/what-is-stream-and-its-types-in-node-js/)
...
3.  **双工流:** 是既可读又可写的流。因此，您可以一起发送和接收数据。比如 `net.Socket` 是一个 TCP 套接字。
...
管道（Piping）是一种操作或机制，我们将一个流（可读流，即源文件）的数据输出作为另一个流（可写流，即目标文件）的输入。它通常用于从一个流（即从源读取）获取数据，并将该流的输出传递到另一个流（即写入目标），而无需自己管理流程。这是消费流最简单的方法。管道操作没有限制。它用于以多种方式处理流式数据。例如，从一个文件读取并将其写入另一个文件。
...
链式流（Chaining）是一种通过将一个流的输出连接到另一个流来创建多个流操作链的机制。它通常与管道操作一起使用。例如，我们将使用管道和链式操作来首先压缩一个文件，然后解压缩同一个文件。
