# `util.formatWithOptions()` 方法

> 原文: [https://www.geeksforgeeks.org/node-js-util-formatwithoptions-method/](https://www.geeksforgeeks.org/node-js-util-formatwithoptions-method/)
...
*   `inspectOptions` <`对象`>: 接受函数、对象、回调、JSON 格式数据等。
*   `format` <`字符串`>: 由<字符串>类型的说明符组成，类似于 `printf` 格式字符串。
*   `args` <`字符串`>: 它接受<`字符串`>类型的列表或参数数组。
...
**参考:** [https://nodejs.org/api/util.html#util_util_formatwithoptions_inspectoptions_format_args](https://nodejs.org/api/util.html#util_util_formatwithoptions_inspectoptions_format_args)