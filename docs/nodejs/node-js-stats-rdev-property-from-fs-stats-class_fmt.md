# 来自`fs`的Node.js `stats.rdev`属性

> 原文：[https://www.geeksforgeeks.org/node-js-stats-rdev-property-from-fs-stats-class/](https://www.geeksforgeeks.org/node-js-stats-rdev-property-from-fs-stats-class/)

## 属性介绍

`stats.rdev`属性是`fs`内置应用编程接口`Stats`类的一个属性，用于获取存储文件的设备的数字（`number`或`bigint`）标识，其中文件被认为是“特殊的”。
...
它返回一个数字或`BigInt`值，表示文件存储在其中的设备的身份，该文件被认为是“特殊的”。

下面的例子说明了`stats.rdev`属性在Node.js中的使用：
...
[https://nodejs.org/api/fs.html#fs_stats_rdev](https://nodejs.org/api/fs.html#fs_stats_rdev)