...
> 原文: [https://www.geeksforgeeks.org/convert-json-file-into-csv-file-and-displaying-the-data-using-node-js/](https://www.geeksforgeeks.org/convert-json-file-into-csv-file-and-displaying-the-data-using-node-js/)
...
有一个 `csv-writer` 是一个用于将数据存入 CSV 的模块。
...
*   为它创建一个对象。
*   在常量变量中提及每一列的值。
*   通过 `csv_writer_object.writeRecords(constant variable)` 使用 `csvWriter.writeRecords(results)` 将数据写入 CSV。
...
有时候我们需要先在服务器上显示 JSON 文件，然后再将 JSON 转换成 CSV 文件来检查数据。

### 进场

1.  导入文件系统和 `csv-parser` 模块。
2.  为这两个对象(`fsdata` 和 `csvdata`)创建对象。
3.  通过传递 `csv-parser` 对象，使用名为*的方法创建数据流，使用管道方法创建读流*。
...
在第一种方法中，我们在脚本内部传递 JSON 数据，但是我们也可以附加已经创建的 JSON 文件。
