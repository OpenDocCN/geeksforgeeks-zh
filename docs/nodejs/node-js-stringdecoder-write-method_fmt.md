...
> 原文：[https://www.geeksforgeeks.org/node-js-stringdecoder-write-method/](https://www.geeksforgeeks.org/node-js-stringdecoder-write-method/)

`stringDecoder.write()`方法用于从给定的缓冲区返回解码后的字符串，类型为 `dArray` 或 `DataView`。此方法还确保从返回的字符串中省略缓冲区末尾的任何不完整的多字节字符。这些字符存储在内部缓冲区中，以便下次调用`stringDecoder.write()`或`stringDecoder.end()`方法时使用。
...
*   `Buffer`：它是一个缓冲区，类型为 `data` 或 `data view`，包含必须解码的字节。
...
[https://nodejs.org/api/string_decoder.html#string_decoder_stringdecoder_write_buffer](https://nodejs.org/api/string_decoder.html#string_decoder_stringdecoder_write_buffer)