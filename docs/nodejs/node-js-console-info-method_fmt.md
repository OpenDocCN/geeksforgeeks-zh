...
`console.info()`方法是控制台模块的内置应用编程接口，用于将消息以换行符打印到 `stdout`。它类似于 `console.log()`方法。
...
*   `data`：此参数指定要打印的数据。
*   `args`：是可选参数，指定在传递给 `data` 的消息中作为替代值传递。所有传递的参数都被发送到 `util.format()`。

## 返回值

这个方法不返回任何东西，而是用换行符将格式化的消息打印到 `stdout`。
...
**参考：** [https://nodejs.org/api/console.html#console_console_info_data_args](https://nodejs.org/api/console.html#console_console_info_data_args)