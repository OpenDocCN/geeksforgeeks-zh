...
> 原文: [https://www.geeksforgeeks.org/node-js-process-arch-property/](https://www.geeksforgeeks.org/node-js-process-arch-property/)

## 属性介绍

`process.arch` 属性是 `process` 模块的内置应用编程接口，用于获取当前 Node.js 所编译的计算机的 CPU 架构。
...
该属性返回为其编译当前 Node.js 的操作系统 CPU 架构。可能的值为 `"x32"`、`"x64"`、`"arm"`、`"arm64"`、`"s390"`、`"s390x"`、`"mipsel"`、`"ia32"`、`"mips"`、`"ppc"` 和 `"ppc64"`。

下面的例子说明了 `process.arch` 的使用。
