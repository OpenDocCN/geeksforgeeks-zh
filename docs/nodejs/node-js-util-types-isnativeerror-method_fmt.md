...
> 原文: [https://www.geeksforgeeks.org/node-js-util-types-isnativeerror-method/](https://www.geeksforgeeks.org/node-js-util-types-isnativeerror-method/)

`util.types.isNativeError()` (在 v10.0.0 中添加) 方法是 `util` 模块的内置应用程序编程接口，用于识别该值是否是 Node.js 中内置 `Error` 类型的实例。如果该实例具有内置的“Error”，则返回 `true`，否则返回 `false`。
...
*   `value` <任意> **:** 它是接受任何变量、类、函数、对象或 JavaScript 原语或任何数据类型的必需参数。

## 返回值

<布尔值> **:** 这将返回一个布尔值，如果实例具有内置的“Error”，则返回 `true`，否则返回 `false`。

下面的例子说明了在 Node.js 中使用 `util.types.isNativeError(value)` 方法。
...
**参考:** [https://nodejs.org/api/util.html#util_util_types_isnativeerror_value](https://nodejs.org/api/util.html#util_util_types_isnativeerror_value)