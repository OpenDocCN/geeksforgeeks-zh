...
本地模块必须写在单独的 JavaScript 文件中。在单独的文件中，我们可以用不同的属性和方法声明一个 JavaScript 对象。

### 步骤 1: 创建本地模块

用文件名`Welcome.js`创建一个本地模块。
...
**解释:**这里，我们用一个函数 `sayHello` 和两个变量 `currTime` 和 `companyName` 声明了一个对象‘welcome’。我们使用 `module.exports` 使该对象全局可用。

### 第二部分: 使用模块

在这一部分中，使用 `app.js` 文件中的上述模块。
