...
*   `path`：要删除的文件的路径。可以是字符串、Buffer或URL。
*   `options`：一个对象，可用于指定影响操作的可选参数，如下所示：
    *   `force`：布尔值。如果路径不存在，将忽略异常。
    *   `recursive`：布尔值，指定是否执行递归目录删除。在此模式下，如果找不到指定路径且操作因失败而重试，则不会报错。默认值为`false`。
*   `callback`：方法执行时将调用的函数。
    *   `err`：如果操作失败，将抛出此错误。

下面的例子说明了Node.js中的`fs.rm()`方法。
...
[https://nodejs.org/api/fs.html#fs_fs_rm_path_options_callback](https://nodejs.org/api/fs.html#fs_fs_rm_path_options_callback)