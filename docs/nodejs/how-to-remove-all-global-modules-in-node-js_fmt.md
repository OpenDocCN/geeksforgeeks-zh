...
> 原文: [https://www.geeksforgeeks.org/how-to-remove-all-global-modules-in-node-js/](https://www.geeksforgeeks.org/how-to-remove-all-global-modules-in-node-js/)
...
为了卸载全局安装的 `package_name` 包，可以使用以下命令（必要时使用 `sudo`，具体取决于您的设置和权限）。
...
Windows 用户只需删除以下目录的内容，即可全局删除所有 Node.js 模块:
...
2.  现在在文本框中输入 `%appdata%/npm` 并按回车键，如下所示:
