...
> 原文: [https://www.geeksforgeeks.org/how-to-parse-command-line-arguments-in-node-js/](https://www.geeksforgeeks.org/how-to-parse-command-line-arguments-in-node-js/)
...
## 方法 1: 使用 `process.argv`

借助 `process.argv` 数组接收参数是最简单的方法。Node.js 以 `process.argv` 的形式为每个正在运行的进程公开这个数组。`process.argv` 数组的第一个元素是指向 Node 可执行文件的文件系统路径。第二个元素是正在执行的 JavaScript 文件的路径，其余的数组元素是通过 `cmd` 传递的参数。
...
通过 `cmd` 传递参数当我们开始处理标志或者如果您的服务器需要大量参数时，就会变得很乏味。
...
*   `argv.time`、`argv.b` 等标志成为 `argv` 的属性。标志必须作为 `--flag` 传递。示例: `node`