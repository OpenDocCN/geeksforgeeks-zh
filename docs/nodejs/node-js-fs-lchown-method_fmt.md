...
`fs.lchown()`方法用于异步更改给定路径的所有者和组，但是，如果路径是一个符号链接，它不尊重符号链接，这与`fs.chown()`方法不同，后者不将链接引用到它们的路径。该功能接受用户 id 和组 id，可用于设置各自的所有者和组。它不返回任何东西。
...
此示例显示了`chown()`和`lchown()`在取消符号链接引用方面的比较。
...
[https://nodejs.org/api/fs.html#fs_fs_lchown_path_uid_gid_callback](https://nodejs.org/api/fs.html#fs_fs_lchown_path_uid_gid_callback)