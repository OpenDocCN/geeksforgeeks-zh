# Node.js HTTP/2 ServerRequest.complete 方法

> 原文：[https://www.geeksforgeeks.org/node-js-http2serverrequest-complete-method/](https://www.geeksforgeeks.org/node-js-http2serverrequest-complete-method/)

`http2ServerRequest.complete`是Http2模块内`Http2ServerRequest`类的内置API，用于检查该请求是否已经完成、中止或销毁。
...
当且仅当该请求已经完成、中止、销毁或未销毁时，该方法返回`true`。

## 如何生成私钥和公钥证书？

1.  **文件名：** `private-key.pem`
    打开记事本，复制粘贴以下密钥，将文件保存为`private-key.pem`。

2.  **文件名：** `public-cert.pem`
...
[https://nodejs.org/dist/latest-v12.x/docs/api/http2.html#http2_request_complete](https://nodejs.org/dist/latest-v12.x/docs/api/http2.html#http2_request_complete)