...
> 原文：[https://www.geeksforgeeks.org/how-to-add-new-functionalities-to-a-module-in-node-js/](https://www.geeksforgeeks.org/how-to-add-new-functionalities-to-a-module-in-node-js/)
...
**模块**是 Node.js 的组成部分。可以要求三种不同类型的模块：

*   NodeJS 提供的内置或默认模块。
*   可通过 `npm` 或 `yarn` 安装的开源模块。
*   私有模块是由程序员根据他们的需要定义的。

在本文中，我们将安装 NodeJS 的 `express` 模块，并为其添加新的功能。要添加新功能，首先导入模块，然后根据我们的需要添加功能。
...
在项目文件夹中创建一个 `app.js` 文件，并使用 `npm` 初始化项目。
...
创建一个 `script.js` 文件，并使用 `npm` 安装 `express` 包。
...
![项目结构](img/a50fa10b91b29e73f23711567023072c.png)

## 步骤 3

现在让我们为 `script.js` 文件编码。其中，我们需要 `express` `npm` 包，然后添加新功能，最后导出包。在其中，我们将添加一个变量、一个对象和两个函数用于演示目的。
...
现在我们将对 `app.js` 文件进行编码。在其中，我们需要从 `script.js` 文件中导出 `express` 模块。并使用该模块演示它的新旧功能。
...
![输出](img/11e69f90cdfca44bfdc50124ad27059f.png)