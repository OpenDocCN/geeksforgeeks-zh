...
`console.group()`方法是控制台模块内置的应用编程接口，用于分组打印传递的内容。它指示消息组的开始。要结束这个组，我们可以使用`console.groupEnd()`方法。
...
这个方法不返回任何东西，而是将控制台上的内容用标签分组(如果提供的话),否则就不用标签。

以下示例说明了`console.group()`方法在Node.js中的使用。
...
[https://nodejs.org/api/console.html#console_console_group_label](https://nodejs.org/api/console.html#console_console_group_label)