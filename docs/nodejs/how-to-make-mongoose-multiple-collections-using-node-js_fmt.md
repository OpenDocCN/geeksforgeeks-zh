...
> 原文：[https://www.geeksforgeeks.org/how-to-make-mongoose-multiple-collections-using-node-js/](https://www.geeksforgeeks.org/how-to-make-mongoose-multiple-collections-using-node-js/)
...
2.  **Model**：它是一个借助已定义的 `Schema` 创建的类，`MongoDB` 文档是模型的一个实例。因此，它充当 `MongoDB` 数据库的接口，用于创建、读取、更新和删除文档。
...
**数据库**：现在我们可以看到三个集合 `courses`、`students`、`teachers` 都是在我们的数据库 `GFG` 中创建的。
