...
> 原文：[https://www.geeksforgeeks.org/how-to-validate-data-using-hapi-joi-module-in-node-js/](https://www.geeksforgeeks.org/how-to-validate-data-using-hapi-joi-module-in-node-js/)验证数据

`Joi` 模块是一个流行的数据验证模块。该模块基于模式验证数据。有各种各样的功能，如 `optional()`，`required()`，`min()`，`max()` 等，使其易于使用，并有一个用户友好的模块来验证数据。
...
1.  可以访问安装 `joi` 模块链接。您可以使用此命令安装此软件包。
...
**注意：** 在上面的程序中，`abortEarly` 被设置为 `false`，这确保了如果有多个错误，那么所有错误都会显示在终端中。如果设置为 `true`，那么一旦遇到第一个错误，程序的执行将停止，并且只有该错误将显示在终端中。
...
4.  现在，如果没有错误发生，即用户数据有效，则将产生以下输出：
