...
> 原文: [https://www.geeksforgeeks.org/node-js-fspromises-appendfile-function/](https://www.geeksforgeeks.org/node-js-fspromises-appendfile-function/)
...
本示例使用名为 `example_file.txt` 的示例 txt 文件，其中包含 `Hello` 文本。
...
**注意:** 如果 `options` 是字符串，则指定编码。该路径可以被指定为一个 `文件句柄`，该文件句柄已经被打开以进行附加(使用 `fsPromises.open()` )。