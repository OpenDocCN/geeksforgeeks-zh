...
> 原文：[https://www.geeksforgeeks.org/which-is-first-argument-typically-passed-to-a-node-js-callback-handler/](https://www.geeksforgeeks.org/which-is-first-argument-typically-passed-to-a-node-js-callback-handler/)
...
*   在调用回调函数时，如果有错误，我们可以像`callback(err)`一样调用它，否则我们可以像`callback(null, result)`一样调用它。
...
在下面提到的代码示例中，我们创建了一个执行除法运算的函数。为了模拟异步操作，我们使用了 `setTimeout()` 方法，该方法在一秒钟后调用回调处理程序。当除数为零时，调用回调处理程序时，错误实例是唯一的参数，否则调用回调时，第一个参数为 `null`，第二个参数为除法结果。
