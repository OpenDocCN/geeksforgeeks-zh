...
> 原文: [https://www.geeksforgeeks.org/node-js-util-types-isstringobject-method/](https://www.geeksforgeeks.org/node-js-util-types-isstringobject-method/)

`util` 模块的 `util.types.isStringObject()` 方法主要是为了支持 Node.js 自己的内部 API 的需求而设计的。它用于检查方法中传递的实例是否是字符串对象。**字符串原语**和**字符串对象**是有区别的。在 JavaScript 中，字符串是一种原语数据类型，它没有方法，只是指向原始数据内存引用的指针。
...
如果传递的值是字符串对象，该方法返回一个布尔值，即 `true`，否则返回 `false`。

下面的例子说明了 `util.types.isStringObject()` 方法在 Node.js 中的使用:
...
参考: [https://nodejs.org/api/util.html#util_util_types_isstringobject_value](https://nodejs.org/api/util.html#util_util_types_isstringobject_value)