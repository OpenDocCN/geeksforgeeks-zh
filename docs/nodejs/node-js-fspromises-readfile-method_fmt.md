...
> 原文: [https://www.geeksforgeeks.org/node-js-fspromises-readfile-method/](https://www.geeksforgeeks.org/node-js-fspromises-readfile-method/)

使用`fsPromises.readFile()`方法读取文件。这个方法将整个文件读入缓冲区。要加载`fs`模块，我们使用`require()`方法。它异步读取文件的全部内容。
...
**参考:** [https://nodejs.org/api/fs.html#fs_fspromises_readfile_path_options](https://nodejs.org/api/fs.html#fs_fspromises_readfile_path_options)