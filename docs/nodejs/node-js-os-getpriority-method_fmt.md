...
> 原文：[https://www.geeksforgeeks.org/node-js-os-getpriority-method/](https://www.geeksforgeeks.org/node-js-os-getpriority-method/)

`os.getPriority()`方法是`os`模块内置的应用编程接口，用于获取`pid`指定的进程调度优先级。
...
下面的例子说明了`os.getPriority()`方法在 Node.js 中的使用：
