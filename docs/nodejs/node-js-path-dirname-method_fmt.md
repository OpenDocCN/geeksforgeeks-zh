...
`path.dirname()`方法用于获取给定路径的目录名。它忽略了各自平台的尾部目录分隔符。
...
*   `Path`: 是用于提取目录名的文件路径。如果此参数不是字符串值，将抛出类型错误。
...
**参考:** [https://nodejs.org/api/path.html#path_path_dirname_path](https://nodejs.org/api/path.html#path_path_dirname_path)