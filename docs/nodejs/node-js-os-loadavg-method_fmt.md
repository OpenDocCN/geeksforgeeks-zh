...
`os.loadavg()`方法是`os`模块的内置应用编程接口，用于获取负载平均值。负载平均值是系统活动的度量，用分数表示，由操作系统计算。
...
## 返回值:
该方法返回一个数组，该数组包含大小为 3 的小数，表示操作系统计算的 1、5 和 15 分钟平均负载。在 windows 上，它将返回`[0, 0, 0]`，因为平均负载是 Unix 特有的概念。

下面的例子说明了`os.loadavg()`方法在 Node.js 中的使用:
...
**参考:** [https://nodejs.org/api/os.html#os_os_loadavg](https://nodejs.org/api/os.html#os_os_loadavg)