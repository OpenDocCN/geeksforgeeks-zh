...
`console.time()`方法是 Node.js 的控制台类，用于启动一个计时器，该计时器用于计算一段代码或函数所花费的时间。方法`console.timeEnd()`用于停止计时器，并将经过的时间(以毫秒为单位)输出到 stdout。计时器可以精确到亚毫秒。
...
该方法接受单个参数`label`作为参数可以在方法中传递，如果标签没有传递`default`标签自动给方法。对于不同的功能或代码段，标签可以不同。
...
[https://nodejs.org/docs/latest-v11.x/api/console.html#console_console_time_label](https://nodejs.org/docs/latest-v11.x/api/console.html#console_console_time_label)