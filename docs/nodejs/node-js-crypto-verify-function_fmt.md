...
> 原文：[https://www.geeksforgeeks.org/node-js-crypto-verify-function/](https://www.geeksforgeeks.org/node-js-crypto-verify-function/)

`crypto.verify()`是node.js crypto内置模块的一种方法，用于验证使用不同类型的散列函数（如[SHA256算法](https://www.geeksforgeeks.org/difference-between-sha1-and-sha256/)等）散列的数据的签名。
...
该函数返回一个布尔值。如果签名通过验证，返回`真`否则返回`假`。
...
[https://nodejs.org/api/crypto.html#crypto_crypto_verify_algorithm_data_key_signature](https://nodejs.org/api/crypto.html#crypto_crypto_verify_algorithm_data_key_signature)