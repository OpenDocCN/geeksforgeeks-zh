...
> 原文: [https://www.geeksforgeeks.org/build-a-simple-beginner-app-with-node-js-bootstrap-and-mongodb/](https://www.geeksforgeeks.org/build-a-simple-beginner-app-with-node-js-bootstrap-and-mongodb/)
...
接下来在项目文件夹中创建一个 `index.js` 文件，这将是 app 的入口点。

## 步骤 3: 安装依赖项

现在使用 `npm` 命令安装依赖项 [express](https://www.geeksforgeeks.org/working-of-express-js-middleware-and-its-benefits/)、[mongoose](https://www.geeksforgeeks.org/mongoose-module-introduction/#:~:text=module%20is%20one%20of%20the,from%20MongoDB%20to%20the%20Node.) 和 [nodemon](https://www.geeksforgeeks.org/node-js-nodemon-module/)。
...
接下来我们打开 `index.js` 文件，用下面的代码创建一个简单的快递 app。
...
这段代码创建了一个简单的快速应用程序，启动一个服务器并监听端口 `80` 的连接。该应用以“Hello World”回应对 `/` 的请求。
...
然后打开浏览器，使用 `http://localhost`，或者简单的 `localhost` 从自身访问服务器。
...
我们现在需要添加 2 行代码，因为我们将需要 `express.json()` 和 `express.urlencoded()` 来处理 POST 和 PUT 请求。`Express` 为我们提供了中间件来处理请求体中传入的数据对象。在 POST 和 PUT 请求中，我们都向服务器发送数据对象，并要求服务器接受或存储该数据对象，该数据对象包含在该请求的 `req.body` 中。
...
在项目文件夹内创建一个名为 `public` 的文件夹。我们将在我们项目的 `public` 目录中创建所有静态 HTML 文件。
...
现在我们将使用以下代码连接到 mongoose 数据库。本项目数据库名称为 `projectDG`。
...
接下来，我们要定义一个 post 方法，将联系人表单的数据保存到我们的数据库中。我们定义我们的数据对象，并在此创建名为 `users` 的集合。成功插入数据后，我们将重定向至 `formSubmitted.html`。
...
现在我们将在 `public` 文件夹中创建 `index.html`、`formSubmitted.html` 和 `style.css` 文件。
