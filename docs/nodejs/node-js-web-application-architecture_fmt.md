...
> 原文: [https://www.geeksforgeeks.org/node-js-web-application-architecture/](https://www.geeksforgeeks.org/node-js-web-application-architecture/)
...
为了管理多个并发客户端，`Node.js` 采用了“单线程事件循环”设计。在 `Node.js` 处理模型中采用了基于 `JavaScript` 事件的模型和 `JavaScript` 回调机制。它采用了两个基本概念:
