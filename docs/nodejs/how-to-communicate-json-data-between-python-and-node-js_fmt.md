...
在下面的例子中，我们将为 Python 设置一个服务器，并从 node.js 发出请求。我们正在使用 `flask` 微框架，因为这是在 Python 中设置服务器并在 Node.js 中发出请求的最简单方法。我们将需要一个 `request-promise` 包。
...
*   使用以下命令为 Python 安装 `flask` 模块:
    ...
    pip install flask
    ```
*   使用以下命令为节点安装 `request-promise` 模块:
    ```bash
