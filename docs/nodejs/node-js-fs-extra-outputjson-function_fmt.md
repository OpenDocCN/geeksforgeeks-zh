...
> 原文: [https://www.geeksforgeeks.org/node-js-fs-extra-outputjson-function/](https://www.geeksforgeeks.org/node-js-fs-extra-outputjson-function/)
...
*   `file`: 是包含文件路径的字符串。
*   `object`: 是将要写入文件的对象。
*   `options`: 是一个包含可选参数的对象，可以传递到函数中。
    1.  `spaces`: 它是一个指定要缩进的空格数的数字，或者是一个用于缩进的字符串，如制表符的 `'\t'`。
