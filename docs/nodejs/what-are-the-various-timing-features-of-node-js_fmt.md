...
> 原文：[https://www.geeksforgeeks.org/what-are-the-various-timing-features-of-node-js/](https://www.geeksforgeeks.org/what-are-the-various-timing-features-of-node-js/)
...
## 3. setInterval()方法
`setInterval()`方法用于以指定的时间间隔(以毫秒为单位)调用函数。它仅用于在指定时间段后执行一次该功能。
...
**参数:**
*   `回调:` 该参数保存定时器到时要调用的函数。
*   `延迟:` 此参数保存调用 `callback` 函数之前等待的毫秒数。
*   `参数:` 此参数保存函数的可选参数。
