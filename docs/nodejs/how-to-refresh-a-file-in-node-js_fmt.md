...
> 原文：[https://www.geeksforgeeks.org/how-to-refresh-a-file-in-node-js/](https://www.geeksforgeeks.org/how-to-refresh-a-file-in-node-js/)
...
在所有这些工具中，我们想先从 `nodemon` 开始。基本上，`Nodemon` 是一个实用程序，它可以监视源中的任何更改，并自动重启服务器。
...
剩下的所谓 `forever` 的工具是一个 `node.js` 包，用于保持服务器活动，即使当它因为一些错误/异常而崩溃或停止时。`forever` 会自动重启。
