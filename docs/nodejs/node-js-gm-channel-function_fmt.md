...
`channel()` 函数是 GraphicsMagick 库中的一个内置函数，用于从图像中提取特定的通道。
...
该功能接受单参数 `channelType`，保存通道名称的类型。通道的类型有 `Red`、`Green`、`Blue`、`Opacity`、`Matte`、`Cyan`、`Magenta`、`Yellow`、`Black` 或 `Gray`。
...
*   [http://www.graphicsmagick.org/GraphicsMagick.html#details-channel](http://www.graphicsmagick.org/GraphicsMagick.html#details-channel)
