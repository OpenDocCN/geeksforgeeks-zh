...
> 原文: [https://www.geeksforgeeks.org/how-to-copy-folder-recursively-in-node-js/](https://www.geeksforgeeks.org/how-to-copy-folder-recursively-in-node-js/)
...
*   `options`
    ...
    *   `options.clobber`: 可以设置为真或假。如果设置为 `false`，`ncp` 不会覆盖目标中已经存在的文件。
    *   `options.stopOnErr`: 可以设置为真或假。默认情况下，它被设置为 `false`，即 `ncp` 继续复制，记录所有错误并返回一个数组。如果设置为 `true`，`ncp` 将在遇到第一个错误时停止。
*   `callback`: 给定任务完成时调用的函数，以错误为第一参数。如果出现错误，回调函数会在不阻止程序执行的情况下处理它。
...
*   `source`: 来源文件夹的文件路径。
*   `destination`: 目标文件夹的文件路径。
*   `callback`: 给定任务完成时调用的函数，以错误为第一参数。如果出现错误，回调函数会在不阻止程序执行的情况下处理它。如果回调未通过，它将返回承诺。
