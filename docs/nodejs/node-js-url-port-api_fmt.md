...
`url.port` 是 `url` 模块中 `url` 类的内置应用编程接口，用于获取和设置 URL 的端口部分。端口值可以是一个数字或包含 0 到 65535（含）范围内的数字的字符串。将该值设置为给定协议的 URL 对象的默认端口将导致端口值变成空字符串（`""`）。
...
为端口赋值后，该值将首先使用 `toString()` 转换为字符串。如果该字符串无效，但以数字开头，则前导数字被分配给端口。如果该数字位于上述范围之外，将被忽略。
...
上述程序将使用 `node myapp.js` 命令编译运行。

## 参考

[https://nodejs.org/api/url.html#url_url_port](https://nodejs.org/api/url.html#url_url_port)