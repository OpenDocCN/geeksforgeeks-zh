...
> 原文: [https://www.geeksforgeeks.org/node-js-dns-resolvetxt-method/](https://www.geeksforgeeks.org/node-js-dns-resolvetxt-method/)

`dns.resolveTxt()`方法是`dns`模块的内置应用编程接口，用于使用`dns`协议解析指定主机名的`Txt`或文本查询记录。
...
**参考:** [https://nodejs.org/api/dns.html#dns_dns_resolvetxt_hostname_callback](https://nodejs.org/api/dns.html#dns_dns_resolvetxt_hostname_callback)