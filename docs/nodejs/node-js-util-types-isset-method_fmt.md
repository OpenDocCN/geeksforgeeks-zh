...
> 原文：[https://www.geeksforgeeks.org/node-js-util-types-isset-method/](https://www.geeksforgeeks.org/node-js-util-types-isset-method/)

`util` 模块的 `util.types.isSet()` 方法主要是为了支持 Node.js 自带的内部 API 的需求而设计的。它用于检查方法中传递的实例是否是内置的 `Set` 实例。
...
如果传递的值是 `Set` 的内置实例，该方法返回一个布尔值，即 `true`，否则返回 `false`。

下面的例子说明了 `util.types.isSet()` 方法在 Node.js 中的使用：
...
**参考：** [https://nodejs.org/api/util.html#util_util_types_isset_value](https://nodejs.org/api/util.html#util_util_types_isset_value)