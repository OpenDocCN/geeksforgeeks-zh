...
`url.href` 是一个内置的应用编程接口类 `url`，在 `url` 模块中获取和设置序列化的 URL。获取 `href` 属性的值相当于调用 `url.toString()` 方法。将此属性的值设置为新值相当于使用 `new URL(值)` 创建新的 `URL` 对象。每个网址对象的属性都将被修改。
...
**参考:**
[https://nodejs.org/api/url.html#url_url_href](https://nodejs.org/api/url.html#url_url_href)