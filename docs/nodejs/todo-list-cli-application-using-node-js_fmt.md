...
> 原文: [https://www.geeksforgeeks.org/todo-list-cli-application-using-node-js/](https://www.geeksforgeeks.org/todo-list-cli-application-using-node-js/)
...
**说明:** 我们将在一个名为 `todo.txt` 的文件中写入待办任务，当这些任务完成时，它们将从 `todo.txt` 中删除，并被写入 `done.txt` 中。该报告将显示有多少待办事项已经完成，还有多少剩余。首先，我们将导入 `文件系统模块`，这将使我们能够访问文件系统。
