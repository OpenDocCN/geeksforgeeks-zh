...
> 原文: [https://www.geeksforgeeks.org/node-js-http2session-type-method/](https://www.geeksforgeeks.org/node-js-http2session-type-method/)

`http2session.type` 是 `http2` 模块内类 `http2session` 的内置应用编程接口，用于返回对等进程中使用的会话实例的类型。

## 语法

```js
const http2session.type
```
...
```js
This session is of NGHTTP2_SESSION_CLIENT type
...
[https://nodejs.org/dist/latest-v12.x/docs/api/http2.html#http2_http2session_type](https://nodejs.org/dist/latest-v12.x/docs/api/http2.html#http2_http2session_type)