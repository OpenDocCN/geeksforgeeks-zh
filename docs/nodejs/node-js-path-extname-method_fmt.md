...
`path.extname()`方法用于获取文件路径的扩展名部分。从句点（`.`）的最后一次出现返回的扩展字符串。）放在路径字符串的末尾。如果文件路径中没有句点，则返回一个空字符串。
...
该方法接受单参数`path`，该路径保存用于提取扩展名的文件路径。

## 返回值:

返回一个带有路径扩展部分的字符串。如果此参数不是字符串值，它将引发类型错误。
...
[https://nodejs.org/api/path.html#path_path_extname_path](https://nodejs.org/api/path.html#path_path_extname_path)