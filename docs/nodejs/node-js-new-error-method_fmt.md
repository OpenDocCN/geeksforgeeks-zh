...
`new Error()` 方法是 Node 模块的内置应用程序编程接口，在该接口中创建新的 Error 对象，并将 `error.message` 属性设置为所提供的文本消息。通过调用 `message.toString()` ，如果一个对象作为消息传递，就会生成文本消息。另一个属性 `error.stack` 表示代码中调用 `new Error()` 的点，这个属性的堆栈跟踪依赖于 `V8` 的堆栈跟踪 API。
...
*   `message` <`string`>: 接受抛出错误时控制台显示的消息。

## 返回值

<`Error`>: 返回作为参数传递的消息值中定义的特定错误。

下面的例子说明了在 Node.js 中使用 `new Error(message)` 方法
...
**参考:** [https://nodejs.org/api/errors.html#errors_new_error_message](https://nodejs.org/api/errors.html#errors_new_error_message)