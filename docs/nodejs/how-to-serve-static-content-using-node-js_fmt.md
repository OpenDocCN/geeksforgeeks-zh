...
> 原文: [https://www.geeksforgeeks.org/how-to-serve-static-content-using-node-js/](https://www.geeksforgeeks.org/how-to-serve-static-content-using-node-js/)
...
4.  要将这个文件夹作为静态文件夹，您需要在您的 `index.js` 中编写这个中间件，如下所示: `app.use(express.static(path.join(__dirname, 'public')))`，其中 `path` 是全局对象，`__dirname` 保存当前目录地址。视图是保存我们所有网页的文件夹。
