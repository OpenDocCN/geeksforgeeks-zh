...
> 原文: [https://www.geeksforgeeks.org/node-js-process-setgroups-method/](https://www.geeksforgeeks.org/node-js-process-setgroups-method/)

`process.setgroups()`方法是`process`模块的内置应用编程接口，用于设置补充组标识。
...
不返回任何东西。为了设置组标识，Node.js 进程需要根权限，因为它是一个特权操作。

## 注意

此功能仅在 POSIX 平台上有效。在 windows 或 android 平台上不可用，因此会导致错误，即类型错误，`setgroups`不是函数。
...
**注意:** 以上程序将使用`node filename.js`命令编译运行，仅在 POSIX 平台。

**参考:** [https://nodejs.org/api/process.html#process_process_setgroups_groups](https://nodejs.org/api/process.html#process_process_setgroups_groups)