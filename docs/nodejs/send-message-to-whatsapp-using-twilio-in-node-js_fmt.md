...
> 原文: [https://www.geeksforgeeks.org/send-message-to-whatsapp-using-twilio-in-node-js/](https://www.geeksforgeeks.org/send-message-to-whatsapp-using-twilio-in-node-js/)
...
创建一个 Twilio 账号，进入 WhatsApp 沙盒部分，你可以在 [WhatsApp 沙盒部分](https://www.twilio.com/console/sms/whatsapp/learn) 找到。

## 步骤 2: 链接你的 WhatsApp 号
...
一旦我们的号码成功链接，我们就可以开始发送 WhatsApp 消息。但是首先，我们需要安装 Twilio 模块。您可以在这里了解更多关于 [Twilio 模块](https://www.npmjs.com/package/twilio) 的信息。我们可以使用以下命令安装 Twilio 模块。
...
## 步骤 7: 从我们的 Twilio 账户获取授权令牌和 id

当我们成功链接我们的 WhatsApp 号码时，我们会获得身份验证令牌和 id。要找到自己的身份验证令牌和 id，请访问 Twilio 网站上 WhatsApp 的单向消息部分。

## 步骤 8: 发送 WhatsApp 消息

我们可以通过传递 id 和 auth token 创建一个客户端对象来发送 WhatsApp 消息，并使用 `message.create` 函数发送消息。(一个 JavaScript 承诺)。您可以在这里了解更多关于 [承诺](https://www.geeksforgeeks.org/javascript-promises/) 的信息。
