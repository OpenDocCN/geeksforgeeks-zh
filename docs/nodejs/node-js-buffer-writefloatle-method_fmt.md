...
> 原文: [https://www.geeksforgeeks.org/node-js-buffer-writefloatle-method/](https://www.geeksforgeeks.org/node-js-buffer-writefloatle-method/)

`Buffer.writeFloatLE()`方法是一个内置的类`Buffer`的应用编程接口，带有`in Buffer`模块，以指定的小端格式在指定的偏移量向缓冲区写入一个值。请注意，该值必须是有效的32位浮点值。
...
## 参考:
[https://nodejs.org/api/buffer.html#buffer_buf_writefloatle_value_offset](https://nodejs.org/api/buffer.html#buffer_buf_writefloatle_value_offset)