...
> 原文：[https://www.geeksforgeeks.org/node-js-process-hrtime-bigint-method/](https://www.geeksforgeeks.org/node-js-process-hrtime-bigint-method/)

`process.hrtime.bigint()` 方法返回当前高分辨率实时（以纳秒为单位）作为 NodeJS 中的 `bigint`。它不支持额外的时间参数，因为可以通过减去两个 `bigint` 变量来直接计算差值。
...
**参考：** [https://nodejs.org/api/process.html#process_process_hrtime_bigint](https://nodejs.org/api/process.html#process_process_hrtime_bigint)