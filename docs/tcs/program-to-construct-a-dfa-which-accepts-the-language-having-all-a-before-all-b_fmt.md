...
> 原文: [https://www.geeksforgeeks.org/program-to-construct-a-dfa-which-accepts-the-language-having-all-a-before-all-b/](https://www.geeksforgeeks.org/program-to-construct-a-dfa-which-accepts-the-language-having-all-a-before-all-b/)

给定一个字符串 `S`，他的任务是设计一个[确定性有限自动机(DFA)](https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-1/)用于接受语言 `L = { a^N b^M | N≥0，M ≥ 0，N+M ≥ 1}`，即常规语言 `L` 使得所有的“a”都出现在“b”{ a，ab，aab，bb…，}的第一个出现之前。如果给定字符串遵循给定语言 `L`，则打印 `【接受】`。否则，打印 `【不接受】`。
