...
这叫做左递归，其中 `S` 是任意非终结符，`a` 和 `b` 是任意一组终结符。
...
2.  引入一个新的非终结符，写在每个终结符的最后。我们生产一个新的非终结符 `S'`，并将新的产品写成，

    ```
    S-->cS' / dS'
    ```

3.  将新产生的非终结符写在左侧（LHS），在右侧（RHS）它可以产生 `ε`，或者产生新的产生式，其中跟在前一个左侧非终结符后面的终结符或非终结符在最后被新的非终结符替换。
