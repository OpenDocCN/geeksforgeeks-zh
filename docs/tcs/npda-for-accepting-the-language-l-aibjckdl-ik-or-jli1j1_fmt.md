...
> 原文: [https://www.geeksforgeeks.org/npda-for-accepting-the-language-l-aibjckdl-ik-or-jli1j1/](https://www.geeksforgeeks.org/npda-for-accepting-the-language-l-aibjckdl-ik-or-jli1j1/)
...
设计一个非确定性的 PDA 接受语言 `L = { a^i b^j c^k d^l : i==k 或 j==l，i >=1，j >=1 }`，即，
...
*   因为 `i==k`：每当 `a` 出现时，把它推入堆栈，如果 `a` 再次出现，也把它推入堆栈。之后，如果 `b` 来了，不要做任何操作。之后，当 `c` 出现时，每次从堆栈中弹出 `a`。之后，如果 `d` 来了就不要做任何操作。
...
所以，这就是我们所要求的非确定性 PDA 对于接受语言 `L = { a^i b^j c^k d^l : i==k 或 j==l，i >=1，j >=1 }`。