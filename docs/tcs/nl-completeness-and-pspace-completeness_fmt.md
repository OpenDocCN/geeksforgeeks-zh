...
> 原文：[https://www.geeksforgeeks.org/nl-completeness-and-pspace-completeness/](https://www.geeksforgeeks.org/nl-completeness-and-pspace-completeness/)
...
1.  “A”在`NL`中。（`A ∈ NL`）
2.  属于`NL`语言可以被日志空间还原为“A”（对于`NL`中的所有`B`，存在`B ≤L A`）。
...
1.  “A”在`PSPACE`中。（`A ∈ PSPACE`）
2.  属于`PSPACE`的语言可以在多项式时间内约化为“A”（对于`PSPACE`中的所有`B`，存在`B ≤L A`）。
...
## 为什么对数空间缩减用于NL-完备性，而PSPACE缩减不用于PSPACE-完备性？
