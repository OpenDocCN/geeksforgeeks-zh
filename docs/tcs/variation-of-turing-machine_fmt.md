...
## 1. 多磁道图灵机
*   A k-track Turing machine (for some k > 0) has k tracks and a read-write head, which can read and write all tracks one by one.
*   A K-track Turing machine can be simulated by a single-track Turing machine.

## 2. 双向无限带图灵机
*   The infinite tape of the bidirectional Turing machine is unbounded in both left and right directions.
...
*   Multi-dimensional Turing machine can be simulated by one-dimensional Turing machine.
