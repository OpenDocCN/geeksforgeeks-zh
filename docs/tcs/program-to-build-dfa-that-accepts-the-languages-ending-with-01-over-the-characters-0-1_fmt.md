# 构建接受以“01”结尾的语言的DFA程序

> 原文: [https://www.geeksforgeeks.org/program-to-build-dfa-that-accepts-the-languages-ending-with-01-over-the-characters-0-1/](https://www.geeksforgeeks.org/program-to-build-dfa-that-accepts-the-languages-ending-with-01-over-the-characters-0-1/)

给定一个二进制字符串 `str`，任务是构建一个接受以`“01”`结尾的语言而不是字符`{0，1}`的DFA。
...
解决上述问题，下面是需要的类状态和成员函数:

*   `Pair1`: 它处理第一种类型的输入，即 0，并将其与指向下一个状态的对象指针相链接。
*   `Pair2`: 它处理第二种类型的输入，即 1，并将其与指向下一个状态的对象指针相链接。
...
*   `check()`: 该功能检查输入结束后，结束状态是否为最终状态。如果它是最终的，则该字符串被接受，否则被拒绝。
