...
> 原文：[https://www.geeksforgeeks.org/dfa-for-exactly-one-of-a-and-at-least-one-of-b/](https://www.geeksforgeeks.org/dfa-for-exactly-one-of-a-and-at-least-one-of-b/)

[确定性有限自动机(DFA)](https://www.geeksforgeeks.org/introduction-of-finite-automata/) 定义为一个抽象的数学概念，用于解决不同软硬件中的各种具体问题。在这类问题中，我们有一些给定的参数，根据这些参数我们应该设计DFA。

本文给出了两个说明：
*   DFA应该恰好有一个`a`
*   DFA应该至少有一个`b`

该DFA应接受 **`ab`、`ba`、`abb`、`bab`、`bba`、`abb`、`babb`、`bbab`、`bbba`、`abbb`…等字符串**，但不接受 **`a`、`b`、`bb`、`bbb`、`aabb`、`aaabb`…等字符串**。
...
### 第三步
在状态`C`下，可以取任何可能数量的`'b'`，也可以取`'a'`作为输入字母。在字母`'b'`上，它保持相同的状态，但在输入`'a'`时，它进入状态`E`，该状态将设置为最终状态。
