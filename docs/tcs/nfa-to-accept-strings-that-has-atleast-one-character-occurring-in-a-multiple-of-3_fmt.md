...
> 原文: [https://www.geeksforgeeks.org/nfa-to-accept-strings-that-has-atleast-one-character-occurring-in-a-multiple-of-3/](https://www.geeksforgeeks.org/nfa-to-accept-strings-that-has-atleast-one-character-occurring-in-a-multiple-of-3/)
...
*   **情况 1: `a`的个数是 3 的倍数:**
    *   为了检查字符串中的`a`的个数是否是 3 的倍数，定义了一组单独的状态。定义为`Q2`、`Q3`、`Q4`的状态检查`a`的数量是否是 3 的倍数。如果这种情况在任何时候达到最终状态`Q2`，那么`a`的数是 3 的倍数。
*   **情况 2: `b`的个数是 3 的倍数:**
