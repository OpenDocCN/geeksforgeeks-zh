...
> 原文: [https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-9/](https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-9/)
...
### 问题-2: 构造接受{a，b}上一组字符串的最小 DFA，其中`{a²bwa² | wε{a，b}*}`。
...
这里我们可以看到，上面语言的每一个字符串都有一个不变的首字母，即`a²b`和最后的`a²`子串，但是“w”有一个超过{a，b}的子串，其字母范围从零到无穷大，但是下面的语言不被这个 DFA 接受，因为它的字符串不遵循字符串的格式。
