...
> 原文：[https://www.geeksforgeeks.org/computational-complexity-v-s-chomsky-hierarchy/](https://www.geeksforgeeks.org/computational-complexity-v-s-chomsky-hierarchy/)
...
### Type-2 或上下文无关语法
如果一个语法的所有产物都是以下形式的 `A ⇢ α`，其中 `A ∈ V` 和 `α ∈ (V ∪ T)*`，则该语法称为 type 2 或上下文无关语法。
`V` 是一组变量，`T` 是一组终端。

由类型 2 语法生成的语言称为**上下文无关语言**。

### Type-1 或上下文相关语法
如果一个语法的所有产物都是以下形式，则该语法被称为 Type-1 或上下文相关语法：`α ⇢ β`，其中 `β` 至少与 `α` 一样长。

### Type-0 或无限制语法
产生式可以不受任何限制地用无限制语法编写。如果产生了 `α ⇢ β`，那么 `α` 的长度可能大于 `β` 的长度。
