...
> 原文: [https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-6/](https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-6/)
...
## 问题-1
构造 `{a，b}` 上的最小 `DFA` 接受字符串集，其中 `a^n b^m` ，其中 `n` 和 `m` 大于或等于 `1`。
...
## 问题-2
构造 `{a，b}` 上的最小 `DFA` 接受字符串集，其中 `a^n b^m` ，其中 `n` 和 `m` 大于或等于 `0`。
