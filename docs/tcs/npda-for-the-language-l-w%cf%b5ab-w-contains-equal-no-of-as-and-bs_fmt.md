...
> 原文: [https://www.geeksforgeeks.org/npda-for-the-language-l-w%cf%b5ab-w-contains-equal-no-of-as-and-bs/](https://www.geeksforgeeks.org/npda-for-the-language-l-w%cf%b5ab-w-contains-equal-no-of-as-and-bs/)
...
## 问题
设计一个非确定性的 PDA 接受语言 L = {w ∈ {a, b}* | w 包含相等数量的 a 和 b}，即，
```
...
### 例子
我们取一个输入字符串: “aabbba”。PDA 接受不接受？

**解:**
1.  从左到右扫描字符串。
2.  在输入“a”和 STACK 字母 `Z` 时，将“a”作为: `(a, Z/aZ)` 推入 STACK，状态将为 `q0`。
3.  第二次输入“a”和 STACK 字母“a”，将“a”按如下方式推入 STACK: `(a, a/aa)`，状态为 `q0`。
4.  第三个输入“b”和 STACK 字母表“a”，从 STACK 弹出为: `(b, a/ε)`，状态为 `q0`。
5.  在输入“b”和 STACK 字母“a”时，从 STACK 弹出为: `(b, a/ε)`，状态为 `q0`。
6.  在输入“b”和 STACK 字母 `Z` 时，将“b”按如下方式推入 STACK: `(b, Z/bZ)`，状态将为 `q0`。
7.  在输入“a”和 STACK 字母“b”时，从 STACK 弹出为: `(a, b/ε)`，状态为 `q0`。
8.  在输入 `ε` 和 STACK 字母 `Z` 时，进入最终状态(`qf`)为: `(ε, Z/Z)`。
