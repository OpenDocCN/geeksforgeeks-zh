...
> 原文: [https://www.geeksforgeeks.org/ardens-theorem-and-challenging-applications-set-2/](https://www.geeksforgeeks.org/ardens-theorem-and-challenging-applications-set-2/)

已经掌握了如何绘制基本[有限状态机](https://www.geeksforgeeks.org/toc-finite-automata-introduction/) ( `DFA`，`NFA` 或 `$\epsilon$-NFA`) 的知识。我们将从提供的状态机中导出正则表达式。
...
`阿登定理`指出，如果 `P` 和 `Q` 是字母表 `$\Sigma$` 上的两个正则表达式，并且如果 `P` 不包含 `$\epsilon$`，那么由 `R = Q + RP` 给出的下列方程 `R` 有唯一解：`R = QP*`。
...
在状态 `A` 上，有一个 `$\epsilon$` 转换(作为开始状态，`$\epsilon$` 转换必须包括在内)，输入 `a` 上的自循环，当输入为 `b` 时，从 `B` 的转换。
