...
> 原文: [https://www.geeksforgeeks.org/turing-machine-for-subtraction-set-2/](https://www.geeksforgeeks.org/turing-machine-for-subtraction-set-2/)

## 先决条件
– [`图灵机`](https://www.geeksforgeeks.org/turing-machine/)、[用于减法的图灵机|集合 1](https://www.geeksforgeeks.org/turing-machine-for-subtraction/)
...
## 示例
– `(3–4)`或`(4–3)`将显示为 `0 0 0 c 0 0 0 0`
...
**步骤 2** – 继续忽略`0`，向右移动。忽略`c`，向右移动并转到步骤 3。
**步骤 3** – 继续忽略`X`，向右移动。将遇到的第一个`0`转换为`X`，然后向左移动并转到步骤 4。
**步骤 4** – 继续忽略左侧所有的`X`和`c`，转到第 5 步。
**步骤 5** – 继续向左移动，忽略`0`，当找到第一个`X`时，忽略它并向右移动，然后转到步骤 1。
**步骤 6** – 继续忽略所有的`X`，向右移动。忽略遇到的第一个`0`，向左移动，然后转到步骤 7。
