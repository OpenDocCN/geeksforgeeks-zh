...
> 原文: [https://www.geeksforgeeks.org/half-clique-np-complete-problem/](https://www.geeksforgeeks.org/half-clique-np-complete-problem/)

图中的半团是一组 `n/2` 个顶点，使得每个顶点与每隔一个顶点共享一条边，即图的 `k = n/2` 个顶点形成一个完整的图。

## 问题
给定一个图 `G(V，E)`，问题是确定该图是否包含大小至少为 `k = |V/2|` 的团。
