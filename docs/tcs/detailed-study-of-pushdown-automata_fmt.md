...
> 原文: [https://www.geeksforgeeks.org/detailed-study-of-pushdown-automata/](https://www.geeksforgeeks.org/detailed-study-of-pushdown-automata/)
...
我们看到前四个元组 `Q`、`$\epsilon$`、`Q0` 和 `F` 类似于有限自动机的情况。

让我们来谈谈 `$Z_o$`，现在我们知道下推自动机有一个堆栈机制来接受有限自动机中不可能的语言。当在推送操作之前我们需要检查溢出条件，或者在弹出操作之前我们检查下溢条件时，问题就出现了。

我们通过假设堆栈是无限的来解决这个问题，而在处理字符串之前，通过在堆栈中预先引入一个元素 `$Z_o$` 来克服空条件。
...
**伽马符号(`$\Gamma$`)** 用于表示所有的堆叠字母。每个输入字母(相同或不同)可以用不同的堆栈符号表示。它也是必要的，因为它将堆栈的最顶层元素传送到机器。

**Delta 函数(`$\delta$`)** 是过渡函数，通过仔细查看在 `Stack` 上完成的三个主要操作，其使用将变得更加清晰:-
