...
> 原文：`https://www.geeksforgeeks.org/turing-machine-for-multiplication/`
...
*   **第 5 步。**然后如果找到`B`，将其转换为`B`和右，然后如果`Y`转换为`0`和右，或者如果`C`转换为`C`和右，则转到步骤 3 并重复该过程，否则如果在第 4 步后找到`0`，则将其转换为`Y`和右，然后`Y`转换为`Y`和右，然后`C`转换为`C`和右，然后`0`转换为`0`或`X`转换为`X`和右，然后`C`转换为`C`和右，然后`0`转换为`0`和右然后`B`变成`0`然后左然后`0`变成`0`然后左然后`C`变成`C`然后左然后`0`变成`0`或者`X`变成`X`然后左然后`C`变成`C`然后左。
*   **第 6 步。**然后重复第五步。
