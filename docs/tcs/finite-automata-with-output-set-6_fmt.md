...
> 原文: [https://www.geeksforgeeks.org/finite-automata-with-output-set-6/](https://www.geeksforgeeks.org/finite-automata-with-output-set-6/)
...
## 问题
以 `{0, 1}` 上所有字符串的集合作为输入，如果输入包含 `101` 作为子串，或者输入字符串以 `101` 开头或以 `101` 结尾，则产生 `A` 作为输出的机器的构造。
...
因此最终米莱机器可以很容易地给出 `A` 作为输出，得到 `101` 作为输入子串。