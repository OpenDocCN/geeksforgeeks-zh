...
> 原文: [https://www.geeksforgeeks.org/design-finite-automata-from-regular-expressions/](https://www.geeksforgeeks.org/design-finite-automata-from-regular-expressions/)

**先决条件–** [有限自动机](https://www.geeksforgeeks.org/toc-finite-automata-introduction/)、[正则表达式、语法](https://www.geeksforgeeks.org/regular-expressions-regular-grammar-and-regular-languages/)和[语言](https://www.geeksforgeeks.org/regular-expressions-regular-grammar-and-regular-languages/)。
...
If `K<sub>1</sub>` > `K<sub>2</sub>` 状态数 = `K<sub>1</sub>` + 1
If `K<sub>1</sub>` < `K<sub>2</sub>` 状态数 = `K<sub>2</sub>` + 1
If `K<sub>1</sub>` = `K<sub>2</sub>` 状态数 = `K<sub>1</sub>` + 1
...
此处，残数即 `K<sub>2</sub>` =0，`K<sub>1</sub>` = `K<sub>2</sub>`。因此，状态数= K