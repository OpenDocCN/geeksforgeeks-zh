...
```
f : Σ1* → Σ2* such that For any w ∈ Σ1*, w ∈ A iff f(w) ∈ B
```
...
*   `∀x`，`x ∈ A`，如果 `f(x) ∈ B`；其中 `f` 是从 A 到 B 的多对一的缩减，表示为 `( A ≤m B )`。

![](img/4875507867f0a7051a5fb60bc45e41cc.png)

1.  **`A ≤ B`** – 问题 A 可简化为问题 B
2.  **`A ≤m B`** – 问题 A 是多对一，可简化为问题 B
3.  **`A ≤p B`** – 问题 A 可以多项式方式约简为问题 B

## 映射约简

*   函数 `f: Σ1* → Σ2*` 称为 A 到 B 的映射约简，如果对于任何 `w ∈ Σ1*`，`w ∈ A` iff `f(w) ∈ B`。
*   `f` 是一个可计算的函数。
...
1.  `A: t^4 - 1` — `B: t^2 - 1`，`C: t^2 + 1`
    在例 1 中，由于 A 是可解的，`B, C < A` 也是可解的，所以 B 和 C 也是可解的。

    ```
    since, ( t^4 - 1 ) = ( t^2 - 1 ) * ( C : t^2 + 1 )
    ```

2.  `A: L(D) = Σ*?` —> 问题 A 可以归结为 `B: L(D1) = Σ* - L(D2)` 吗？
    在示例中，B 是 A 的子集，因此 A 被简化为问题 B。

3.  `A: L(G) = 空吗？` —> 问题 A 可以化为 `B: L(G1)` 是 `L(G2)` 的子集吗？
    如果将上述问题 A 简化为形式更简单的问题 B，那么求解就很容易了。

4.  `A: a^3 + b^3 + 3a^2b + 3b^2a` —> A is reduced to `B: ( a + b )^3`
    If A reduces to B and B is “solvable,” then A is “solvable.”

5.  将 `L_D` 减少至 —> `0* 1*`
    `w_是 = 01`，`w_否 = 10`
    则减少的 `f(w)` 形式为: `f(w) = { 01 如果 w ∈ L_D, 10 如果 w ∉ L_D }`
...
8.  **给定问题 A : `L1 ≤ L2` 和 `L2 ≤ L3`** –
    1.  如果 `L2` 是可判定的，那么 –> `L1` 是可判定的，`L3` 是可判定的或不可判定的。
    2.  如果 `L2` 是不可判定的，那么 –> `L1` 要么是可判定的，要么是不可判定的，`L3` 是不可判定的。