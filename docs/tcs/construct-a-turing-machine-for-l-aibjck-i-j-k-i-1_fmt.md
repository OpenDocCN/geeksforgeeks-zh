...
在给定的语言中 `L = { aibjck | I<j<k；i≥ 1}`，每一串`a`，`b`和`c`都有一定数量的`a`，然后一定数量的`b`，然后一定数量的`c`。条件是第一个符号的计数至少应为 1。此后“b”和“c”可以有同样多，但`a`的计数小于“b”的计数，“b”的计数小于“c”的计数。假设字符串以“{content}”结尾。
...
## 注:
比较`|A|`、`|D|`、`|C|`，使用[图灵机作为比较器](https://www.geeksforgeeks.org/toc-turing-machine-as-comparator/)的概念。