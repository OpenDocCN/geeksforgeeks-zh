...
> 原文: [https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-5/](https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-5/)
...
在`{a，b}`上构造一个接受字符串集的最小 `DFA`，其中每个`"a"`后面跟一个`"bb"`。
...
在`{a，b}`上构造一个接受字符串集的最小 `DFA`，其中每个`"a"`后面都不跟`"bb"`。
