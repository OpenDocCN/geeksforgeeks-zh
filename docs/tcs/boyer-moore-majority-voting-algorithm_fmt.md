...
> 原文：[https://www.geeksforgeeks.org/boyer-moore-majority-voting-algorithm/](https://www.geeksforgeeks.org/boyer-moore-majority-voting-algorithm/)
...
*   初始化一个变量说 `i`，`votes = 0`，`candidate = -1`
*   使用 `for` 循环遍历数组
*   如果 `votes = 0`，选择 `candidate = arr[i]`，使 `votes = 1`。
*   否则，如果当前元素与候选增量票相同
*   否则会减少票数。

**第 2 步 –** 检查候选人的票数是否超过 `N/2` –

*   初始化一个变量 `count = 0`，如果它与候选值相同，则递增计数。
*   如果计数大于 `N/2`，则返回候选人。
*   否则返回 `-1`。
