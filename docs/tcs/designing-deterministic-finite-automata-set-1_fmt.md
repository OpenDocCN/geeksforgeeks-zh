...
> 原文: [https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-1/](https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-1/)
...
## 问题-1
为 `{a，b}` 上的字符串集构造一个 DFA，使得字符串的长度 `|w|=2`，即字符串的长度正好是 2。
...
这里，状态 `A` 表示长度为零(0)所有串的集合，状态 `B` 表示长度为一(1)的所有串的集合，状态 `C` 表示长度为二(2)的所有串的集合。状态 `C` 是最终状态，`D` 是死状态，因为在输入任何字母后，它永远不会进入最终状态。
...
## 问题-2
为 `{a，b}` 上的字符串集构造一个 DFA，使得字符串的长度 `|w| >= 2`，即字符串的长度至少应为 2。
...
这里，状态 `A` 表示长度为零(0)的所有刺的集合，状态 `B` 表示长度为一(1)的所有刺的集合，状态 `C` 表示长度为二(2)的所有刺的集合。
...
## 问题-3
为 `{a，b}` 上的字符串集构造一个 DFA，使得字符串的长度 `|w| <= 2`，即字符串的长度为 atmost 2。
...
这里，状态 `A` 表示长度为零(0)的所有刺的集合，状态 `B` 表示长度为一(1)的所有刺的集合，状态 `C` 表示长度为二(2)的所有刺的集合，状态 `A`、`B`、`C` 是最终状态，`D` 是死状态，之所以如此，是因为在获得任何字母表作为输入之后，它将永远不会进入最终状态。
