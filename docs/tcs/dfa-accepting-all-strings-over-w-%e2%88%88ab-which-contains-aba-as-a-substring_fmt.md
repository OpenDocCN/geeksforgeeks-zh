...
> 原文: [https://www.geeksforgeeks.org/dfa-accepting-all-strings-over-w-%e2%88%88ab-which-contains-aba-as-a-substring/](https://www.geeksforgeeks.org/dfa-accepting-all-strings-over-w-%e2%88%88ab-which-contains-aba-as-a-substring/)

给定一个二进制字符串 `S`，任务是为 DFA Machine 编写一个程序，该程序接受 `w∈(a,b)*` 上的一组所有字符串，其中包含“aba”作为子字符串。
...
下面是针对给定问题设计的 DFA 机器。为 DFA 状态构建一个转换表，并分析每个状态之间的转换。以下是步骤：
...
1.  首先，会有 4 种状态（`q0`，`q1`，`q2`，`q3`），其中 `q0` 为初始状态，`q3` 为最终状态。
2.  最初我们将处于 `q0` 状态，现在我们开始读取给定的字符串。
    *   当我们读“b”的时候，我们会保持同样的状态。
    *   如果我们读“a”，那么它会过渡到状态 `q1`。
3.  假设现在我们处于 `q1` 状态。
