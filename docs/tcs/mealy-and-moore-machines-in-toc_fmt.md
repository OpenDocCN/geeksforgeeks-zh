...
> 原文: [https://www.geeksforgeeks.org/mealy-and-moore-machines-in-toc/](https://www.geeksforgeeks.org/mealy-and-moore-machines-in-toc/)

## Moore 机器

Moore 机器是有输出值的有限状态机，其输出只依赖于当前状态。可以定义为 `(Q, q0, ∑, O, δ, λ)`，其中：
...
在图 1 所示的 Moore 机器中，输出由 `/` 分隔的每个输入状态表示。Moore 机器的输出长度比输入长度大 1。
...
Mealy 机器也是有输出值的有限状态机，其输出取决于当前状态和当前输入符号。可以定义为 `(Q, q0, ∑, O, δ, λ')`，其中：
...
在图 2 所示的机器中，输出用每个状态的每个输入符号表示，用 `/` 分隔。Mealy 机器的输出长度等于输入长度。
...
**第三步。** 用新生成的状态创建一个空的 Moore 机器。对于 Moore 机器，输出将与每个状态相关联，而与输入无关。
...
这是图 1 所示 Moore 机的过渡表。

## 从 Moore 机到 Mealy 机的转换

让我们以图 1 的 Moore 机器为例，其转换表如表 3 所示。

**第一步。** 如表 4 所示，使用 Moore 机器的所有状态构建一台空的 Mealy 设备。
...
**步骤 2:** 每个状态的下一个状态也可以直接从 Moore 机器转换表中找到，如下所示:
...
**第 3 步:** 正如我们在 Moore 机器转换表中可以看到的与每个输入对应的输出。用它来填充输出条目。例如: `q10`、`q11`、`q20`、`q21` 对应的输出分别为 `0`、`1`、`0`、`1`。
...
这是表 1 中所示的同一台 Mealy 机。因此，我们将 Mealy 转换为 Moore 机器，并将 Moore 转换回 Mealy。

**注意:** Mealy 机的状态数不能大于 Moore 机的状态数。

## 例

以下状态图描述的有限状态机，以 `A` 为起始状态，其中一个弧标号为 `x / y`，`x` 代表 1 位输入，`y` 代表 2 位输出？
