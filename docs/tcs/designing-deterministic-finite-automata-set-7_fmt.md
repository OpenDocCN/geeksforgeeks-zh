...
> 原文: [https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-7/](https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-7/)
...
## 问题-1
构造`{a, b}`上的最小 DFA 接受字符串集，其中 `a^n b^m c^l`，其中 `n`、`m` 和 `l` 大于或等于 0。
...
## 问题-2
构造`{a, b}`上的最小 DFA 接受字符串集，其中 `a^n b^m c^l`，其中 `n`、`m` 和 `l` 大于或等于 1。
