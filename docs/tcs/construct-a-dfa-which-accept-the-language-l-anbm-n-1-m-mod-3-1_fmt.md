# 构造一个接受语言 L = {a<sup>n</sup>b<sup>m</sup> | n >= 1, (m) mod 3 = 1} 的 DFA

> 原文: [https://www.geeksforgeeks.org/construct-a-dfa-which-accept-the-language-l-anbm-n-1-m-mod-3-1/](https://www.geeksforgeeks.org/construct-a-dfa-which-accept-the-language-l-anbm-n-1-m-mod-3-1/)
...
### 步骤-1
为 `a^+` 构建 FA 意味着拥有大于 1 的任意数量的 a。

### 步骤-2
为 `b` 构建 FA 意味着准确地拥有。

### 步骤-3
为 `(bbb)^*` 构建 FA 意味着 b 等于 3 的倍数。

### 步骤-4
将三个 FA 串联起来，做成单个 DFA。始终保持 a、b、c 的顺序。
```
ACCEPTED
```
