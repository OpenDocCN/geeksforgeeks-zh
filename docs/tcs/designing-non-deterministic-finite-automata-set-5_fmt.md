...
> 原文: [https://www.geeksforgeeks.org/designing-non-deterministic-finite-automata-set-5/](https://www.geeksforgeeks.org/designing-non-deterministic-finite-automata-set-5/)
...
## 问题-1
构造最小 `NFA`，接受 `{a，b}` 上的一组字符串，其中语言的每个字符串都包含 `“ab”` 作为子字符串。
...
## 问题-2
构造一个最小 `NFA`，接受 `{a，b}` 上的一组字符串，其中语言的每个字符串都不包含 `“ab”` 作为子字符串。
