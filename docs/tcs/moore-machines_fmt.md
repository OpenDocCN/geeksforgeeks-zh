...
**摩尔机器:** 一个[摩尔机器](https://www.geeksforgeeks.org/mealy-and-moore-machines-in-toc/)基本上是一个 [DFA](https://www.geeksforgeeks.org/designing-finite-automata-from-regular-expression-set-1/)，其输出与每个状态相关联。这些机器可以用于各种各样的任务，例如计算给定字符串中特定子串的出现次数，寻找二进制数的[2 补码，等等。](https://www.geeksforgeeks.org/1s-2s-complement-binary-number/)
...
然后，将此**结构**作为数据成员包含到我们的**类 State** 中。该类有三个数据成员:
*   **Input_1:** 这是上面定义的类型 `item` 的变量。它将第一种类型的输入 **‘a’** 映射到下一个状态。
*   **Input_2:** 这也是类型 `item` 的变量。它将第二种类型的输入 **‘b’** 映射到下一个状态。
*   **m_out:** 这是与[摩尔机器](https://www.geeksforgeeks.org/mealy-and-moore-machines-in-toc/)的每个状态相关的输出。
...
要首先实现这一点，请为此机器映射转换表。需要三个状态，即开始定义的类 `State` 的三个对象:
*   状态 1 => `Initialize({'0', &State1}, {'1', &State2}, '0')`
*   状态 2 => `Initialize({'0', &State2}, {'1', &State3}, '1')`
*   状态 3 => `Initialize({'0', &State2}, {'1', &State3}, '0')`
...
// Function to create states and produce output
## `mooreOut` Function
