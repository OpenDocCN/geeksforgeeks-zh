# 为 L = {a^i b^j | i<j, i>0 } 构建图灵机

> 原文: [https://www.geeksforgeeks.org/construct-turing-machine-for-l-ai-bj-ij-i0/](https://www.geeksforgeeks.org/construct-turing-machine-for-l-ai-bj-ij-i0/)

## 先决条件
[图灵机](https://www.geeksforgeeks.org/turing-machine-in-toc/)

## 任务
我们必须为 `a^i b^j` 设计一个图灵机，在那里 `i < j` 且 `i > 0`。
...
3.  当指针到达空白(`B`)时，它从右向左扫描输入，并将第一个‘b’替换为‘Y’。
...
6.  我们重复同样的步骤，直到把所有的 `a` 转换成 `X`。与 `a` 数量相等的 `b` 也会转换成‘Y’，剩下的 `b` 我们会剩下一些。
