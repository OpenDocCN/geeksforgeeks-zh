...
> 原文: [https://www.geeksforgeks.org/converting-context-free-grammar-greibach-normal-form/](https://www.geeksforgeks.org/converting-context-free-grammar-greibach-normal-form/)

先决条件 – [上下文无关语法](https://www.geeksforgeks.org/classification-of-context-free-grammars/)、[简化上下文无关语法](https://www.geeksforgeks.org/simplifying-context-free-grammars/)

如果所有生成规则满足以下条件之一，则上下文无关语法（CFG）处于格雷巴赫范式（GNF）中：
...
`G1` 的语法在 GNF，因为生产规则满足为 GNF 指定的规则。但是，语法 `G2` 不在 GNF，因为生产规则 `B-> ε` 和 `A-> ε` 不满足为 GNF 指定的规则（只有开始符号才能生成 `ε`）。
...
如果 CFG 包含左递归，消除它们。你可以参考以下文章来消除左递归: [解析 | 集合 1（介绍、歧义和解析器）](https://www.geeksforgeks.org/parsing-set-1-introduction-ambiguity-and-parsers/)
