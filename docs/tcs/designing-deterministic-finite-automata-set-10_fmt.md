...
> 原文: [https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-10/](https://www.geeksforgeeks.org/designing-deterministic-finite-automata-set-10/)
...
构造 `{a}` 上接受字符串的最小 DFA 集合，其中 `{a^n | n≥0, n≠2}`，即 ‘n’ 应大于 0 且不等于 2。
...
这里，`ε` 被视为字符串，因为 “n” 的值大于或等于零，其余字符串的 “a” 是任何正自然数的幂，但不是 2。
...
构造 `{a}` 上的最小 DFA 接受串集，其中 `{a^n | n≥0, n≠2, n≠4}`，即 ‘n’ 应大于 0 且不等于 2 和 4。
...
这里，`ε` 被视为字符串，因为 “n” 的值大于或等于零，其余字符串的 “a” 是任何正自然数的幂，但不是 2 和 4。
