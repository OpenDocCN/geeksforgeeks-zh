...
> 原文: [https://www.geeksforgeeks.org/moore-and-mealy-machines-to-produce-a-b-c-depends-on-inputs-that-end-with-10-or-with-11-else-other/](https://www.geeksforgeeks.org/moore-and-mealy-machines-to-produce-a-b-c-depends-on-inputs-that-end-with-10-or-with-11-else-other/)
...
## 问题
以`{0, 1}`上的一组所有字符串作为输入，如果输入以`"10"`结尾，则产生`"A"`作为输出的机器的构造，或者如果输入以`"11"`结尾，则产生`"B"`作为输出的机器的构造。
...
因此，如果输入以`"10"`结尾，上面的摩尔机器可以容易地产生`"A"`作为输出，或者如果输入以`"11"`结尾，则产生`"B"`作为输出，否则产生`"C"`。
...
### Step-1
Formation of State Transition Table of the above Moore machine-
...
### Step-2
Formation of Transition Table for Mealy machine from above Transition Table of Moore machine-
