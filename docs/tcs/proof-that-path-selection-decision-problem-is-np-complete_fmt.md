...
> 原文: [https://www.geeksforgeeks.org/proof-that-path-selection-decision-problem-is-np-complete/](https://www.geeksforgeeks.org/proof-that-path-selection-decision-problem-is-np-complete/)
...
我们创建一个新的图形 `G'`。对于 `G` 中的每个顶点 `v_i`:

1.  我们用 `P_i` 来表示路径，表示 `v_i`。
2.  让 `e_1, e_2, ..., e_r` 成为连接到 `v_i` 的边。我们在 `G'` 中为每个这样的边引入一个顶点。此外，我们在 `G'` 中引入一对顶点 `s_i, t_i`。
