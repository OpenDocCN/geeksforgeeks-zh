...
[正则表达式](https://www.geeksforgeeks.org/write-regular-expressions/)可以是从终端符号∅到两个正则表达式的并集(`R1 + R2`)、它们的交集(`R1 + R2`)或正则表达式的闭包(`R1*`)或 `a∈σ`，其中`σ`是输入符号的有限集合，也是语言`{a}`的正则表达式。

> 正则语言是一种可以用正则表达式表达的语言。

## 正则表达式示例

1.  恰好有一个 `a` 的所有字符串`σ= { a，b}`集合的正则表达式。

    ```regex
    b*ab* 
    ```

2.  以前缀 `ab` 开头的所有字符串`σ= { a，b}`集合的正则表达式。
