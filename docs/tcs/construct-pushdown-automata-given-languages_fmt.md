...
> 原文：[https://www.geeksforgeeks.org/construct-pushdown-automata-given-languages/](https://www.geeksforgeeks.org/construct-pushdown-automata-given-languages/)
...
*   `σ` 是一个有限集合，称为输入字母表
*   `γ` 是一个有限集合，称为栈字母表
*   `δ` 是 `Q X (σ∪{ε}) X γX Q X γ*` 的有限子集的过渡关系。
*   `q0 ∈ Q` 为启动状态
*   `ζ ∈ γ` 是初始堆栈符号
*   `F ⊆ Q` 是接受状态的集合
...
### q) 为语言构建一个 `PDA L = { 0^n1^m2^m3^n | n>=1, m>=1}`
...
### q) 为语言构建一个 `PDA L = { 0^n1^m | n>=1, m>=1, m>n+2}`
