...
> 原文：[https://www.geeksforgeeks.org/regular-expression-vs-context-free-grammar/](https://www.geeksforgeeks.org/regular-expression-vs-context-free-grammar/)
...
| 正则表达式对于描述词汇结构如标识符、常量等非常有用。 | 上下文无关文法在描述嵌套链结构或句法结构(如平衡括号、`if else` 等)时最有用。而这些不能用正则表达式来定义。 |