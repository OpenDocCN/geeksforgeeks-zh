# 接受语言的 DFA L = { a^n b^m | n+m =奇数}

> 原文: [https://www.geeksforgeeks.org/dfa-for-accepting-the-language-l-an-bm-nmodd/](https://www.geeksforgeeks.org/dfa-for-accepting-the-language-l-an-bm-nmodd/)

设计确定性有限自动机(DFA)接受语言 `L = {a^n b^m | m+n=odd}`。

为语言 `L = { a^n b^m | n+m =奇数}` 创建 DFA 使用初等数学，上面写着：
