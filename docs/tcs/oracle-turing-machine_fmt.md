...
甲骨文图灵机类似于标准图灵机，但增加了第二盘磁带，称为甲骨文磁带。在甲骨文磁带的单元格中可以找到空白(`B`)、`0` 或 `1`。给定集合 `A`，带有集合 `A` 的甲骨文图灵机将集合 `A` 的特征函数写到磁带上。甲骨文磁头在单元上以字母 `X_A(0)`开始。`X_A(1)`在这个右边的单元格中，依次递增。Oracle 磁带磁头开始的单元格左侧只有空白。
...
*   `OTMs` 可以被认为是利用子程序的计算。不管复杂程度如何，花在这些子程序上的时间只算一步。因此，`OTMs` 是一个虚拟的计算模型。
