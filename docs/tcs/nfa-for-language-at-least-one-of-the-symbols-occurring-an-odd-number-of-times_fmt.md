...
> 原文: [https://www.geeksforgeeks.org/nfa-for-language-at-least-one-of-the-symbols-occurring-an-odd-number-of-times/](https://www.geeksforgeeks.org/nfa-for-language-at-least-one-of-the-symbols-occurring-an-odd-number-of-times/)
...
### 步骤-1
创建一个初始状态`"A"`，它过渡到三个不同的状态，分别为`"B"`、`"C"`、`"D"`表示`空`。
...
### 步骤-4
在状态`"C"`上如果输入字母是`'b'`，那么进行`'b'`从状态`"C"`到`"Y"`的转换，如果状态`"C"`上的输入字母是`'a'`或`'c'`，那么在状态`"C"`本身上进行`'a'`和`'c'`的自循环，在状态`"Y"`上，输入字母`'a'`和`'c'`转换到状态`"Y"`本身，但是`'b'`的转换是从状态`"Y"`到状态`"C"`。

![](img/7e25c4c5650749969410bf5e149f54b8.png)

### 第 5 步
在状态`"D"`上如果输入字母是`'c'`，则进行`'c'`从状态`"D"`到`"Z"`的转换，如果状态`"D"`上的输入字母是`'a'`或`'b'`，则在状态`"D"`本身上进行`'a'`和`'b'`的自循环。在状态`"Z"`上，输入字母`'a'`和`'b'`本身转换到状态`"Z"`，但是`'c'`的转换从状态`"Z"`转换到状态`"D"`。
