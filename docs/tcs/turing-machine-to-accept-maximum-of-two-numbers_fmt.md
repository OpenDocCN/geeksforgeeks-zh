...
> 原文：[https://www.geeksforgeeks.org/turing-machine-to-accept-maximum-of-two-numbers/](https://www.geeksforgeeks.org/turing-machine-to-accept-maximum-of-two-numbers/)
...
2.  如果没有找到，则表示左侧的 `0` 比右侧多。那就是 `n1 > n2`。因此，将所有 `X` 更改为 `O`，并在最左侧停止。
3.  继续做第一步。如果恰好你指向 `C`，在最右边的转换之后（用右边的 `X` 代替 `0` 的值），那么就意味着 `n2 >= n1`。因此，将所有 `X` 转换为右侧的 `O` 并停止。
...
此处，`q0` 显示初始状态，`q1，q2，…..，q9` 为过渡状态，`q4，q9` 为最终状态。和 `0，1` 为机内数据，`X，Y，C` 为用于求最大值即比较数据的变量，`R，L` 为左右显示。