...
> 原文：[https://www.geeksforgeeks.org/undecidability-and-reducibility-in-toc/](https://www.geeksforgeeks.org/undecidability-and-reducibility-in-toc/)
...
*注：两个流行的不可判定问题是`TM`和`PCP`的停顿问题(后对应问题)。*
...
A. 如果`P2`可约化为`P3`，`P3`是不可判定的。
B. 如果`P3`可约化为`P2`的补语，`P3`是可判定的。
C. 如果`P3`可约化为`P2`，`P3`是不可判定的。
D. 如果`P1`可约化为`P3`，`P3`是可判定的。
