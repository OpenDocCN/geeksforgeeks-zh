# 为语言 L = {0^n1^n2^n | n≥1}构建图灵机

> 原文: [https://www.geeksforgeeks.org/construct-turing-machine-language-l-0n1n2n-n≥1/](https://www.geeksforgeeks.org/construct-turing-machine-language-l-0n1n2n-n≥1/)

先决条件 – [图灵机](https://www.geeksforgeeks.org/turing-machine/)

语言 `L = { 0^n1^n2^n | n≥1 }` 代表一种我们只使用 3 个字符的语言，即 `0`、`1` 和 `2`。在开始的语言中，有一些 `0` 后面跟有相等数量的 `1`，然后跟有相等数量的 `2`。任何属于这一类别的字符串都将被这种语言接受。字符串的开头和结尾用 `$` 符号标记。
