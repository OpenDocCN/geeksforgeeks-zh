...
> 原文: [https://www.geeksforgeeks.org/properties-of-regular-expressions/](https://www.geeksforgeeks.org/properties-of-regular-expressions/)
...
- 如果 `r1` 和 `r2` 是正则表达式，那么 `(r1)`、`r1.r2`、`r1+r2`、`r1*`、`r1+` 也是正则表达式。
...
两种常规语言 `L1` 和 `L2` 的串联，用 `L1.L2` 表示，也是正则的。它代表了一组字符串，这些字符串是由 `L1` 的任意字符串和 `L2` 的任意字符串连接而成的。

**例**
`L1 = {0, 1}` 和 `L2 = {00, 11}`，然后是 `L1.L2 = {000, 011, 100, 111}`。

### 3. 克莱尼闭包（Kleene Closure）

如果 `L1` 是一种正则语言，那么克莱尼闭包，即 `L1*` 也是正则的，并且表示通过从 `L1` 获取多个字符串而形成的一组字符串，并且同一字符串可以重复任意次并连接这些字符串。

**示例**
`L1 = {0, 1}`，那么 `L1* = {∈, 0, 1, 00, 01, 10, 11, …}`，是符号 `0` 和 `1` 的所有可能的字符串，包括空字符串。

## 正则表达式的代数性质

克莱尼闭包是一元运算符，以及 Union（`+`）和 concatenation 运算符（`.`）是二元运算符。
...
- `r+ = r.r* = r*r`，因为 `r* = ∑+r+rr+rrr…` 和 `r.r* = r+rr+rrr…`
- `r* = r+ + ∈`
...
在联合算子的情况下，如果 `r+x = r`，`x = ∅`，因为 `r ∪ ∅ = r`，那么 `∅` 就是 `+` 的同一性。
因此，`∅` 是联合运营商的身份要素。

在串联运算符的情况下，如果 `r.x = r`，对于 `x = ∈`，`r.∈ = r`，因此 `∈` 是串联运算符（`.`）的同一性。

### 5. 消灭者（Annihilator）

- 如果 `r+x = x`，则 `+` 没有零化子。
- 在串联运算符 `r.x = x` 的情况下，当 `x = ∅`，则 `r.∅ = ∅`，因此 `∅` 是（`.`）运算符的零化子。例如 `{a, aa, ab}.{ } = { }`。

### 6. 交换性质（Commutativity）

如果 `r1`、`r2` 是 RE，那么
- `r1+r2 = r2+r1`。例如，对于 `r1 = a` 和 `r2 = b`，则 RE `a+b` 和 `b+a` 相等。
- `r1.r2 ≠ r2.r1`。例如，对于 `r1 = a` 和 `r2 = b`，那么 RE `a.b` 不等于 `b.a`。

### 7. 分布式属性（Distributivity）
...
### 8. 幂等定律（Idempotent Laws）

- `r1+r1 = r1`，`r1∪r1 = r1`，因此并集算子满足幂等性质。
- `r.r ≠ r`，串联运算符不满足幂等属性。

### 9. 正则表达式的标识（Identities）
