...
> 原文：[https://www.geeksforgeeks.org/relationship-between-grammar-and-language-in-theory-of-computation/](https://www.geeksforgeeks.org/relationship-between-grammar-and-language-in-theory-of-computation/)
...
在这个语法中，使用 `S-> ε`，我们可以生成 `ε`。因此，`ε` 是 `L(G)` 的一部分。同样，使用 `S=>aSb=>ab`，生成 `ab`。同样，也可以生成 `aabb`。
因此，
...
使用 `S=> B=> b`，可以生成 `b`。
使用 `S=> B=> bB`，可以生成 `bB`。
使用 `S=> aS=> aB=> ab` 即可生成 `ab`。
使用 `S =>aS =>aB =>abB =>abb` 可以生成 `abb`。
我们可以看到，`a` 的个数可以是零或者更多，但是 `b` 的个数总是大于零。
因此，

```
L(G1) = {a^m b^n| m>=0 and n>0}
```

考虑语法 `G2`：
使用 `S=> aA=> a`，可以生成 `a`。
使用 `S=> bB=> b`，可以生成 `b`。
使用 `S=> aA=> aaA=> aa` 可以生成 `aa`。
使用 `S=> bB=> bbB=> bb` 即可生成 `bb`。
使用 `S =>aA =>aB =>abB =>abb` 可以生成 `abb`。

如我们所见，`a` 或 `b` 必须大于 0。
因此，

```
L(G2) = {a^m b^n| m>0 or n>0}
```
...
语言 `L(G)` 是一组字符串 `ε`，`ab`，`aabb`，`aaabbb`…
对于 `L(G)` 中的 `ε` 串，产生规则可以是 `S-> ε`。
