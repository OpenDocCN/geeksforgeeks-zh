# 接受语言的 DFA L = { a^n b^m | n+m =偶数}

> 原文: [https://www.geeksforgeeks.org/dfa-for-accepting-the-language-l-an-bm-nmeven/](https://www.geeksforgeeks.org/dfa-for-accepting-the-language-l-an-bm-nmeven/)

设计确定性有限自动机(DFA)接受语言 L = { a^n b^m | n+m=even}。为语言创建 DFA，l = { a^n b^m；n+m =偶数}使用初等数学，上面写着：
偶数+偶数=偶数，奇数+奇数=偶数。
