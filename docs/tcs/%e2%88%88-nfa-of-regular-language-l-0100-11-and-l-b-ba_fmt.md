# ∑-正则语言的 NFA：L =(0+1)*(00+11) 与 L = b+ ba*

> 原文: [https://www.geeksforgeeks.org/%e2%88%88-nfa-of-regular-language-l-0100-11-and-l-b-ba/](https://www.geeksforgeeks.org/%e2%88%88-nfa-of-regular-language-l-0100-11-and-l-b-ba/)

非确定性有限自动机和∑-非确定性有限自动机除了它们的转移函数和构造∑-NFA的一些特殊规则外，几乎是相同的。
...
这个结构是`a*`的，这意味着表达式中可以有任意数量的“a”，甚至是0。前面的结构只是修改了一点，这样即使没有输入符号，即如果输入符号为空，那么表达式也是有效的。

### ∑-NFA 为 a+b

![](img/432cb3aa5fbc59533d6136f8eb054575.png)
...
*   第一部分可以使用第三条规则和第二条规则绘制。`(0+1)`很容易遵循第三条规则绘制，并将`(0+1)`视为一个单元，`(0+1)*`也可以应用第二条规则绘制。

下面是第一部分。
![](img/776483ca2d3da2f34f91a62f23034e36.png)

*   第二部分可以在第四条规则的帮助下绘制。在第四条规则中，`a`和`b`都是`0`。这就是我们如何构造`00`。类似地，我们可以构造`11`。现在由于它们由‘+’号连接，将会有两条路径连接这两个结构。
...
![](img/3f6a6b7c8d9e0f1a2b3c4d5e6f7a8b9c.png)