...
> 原文: [https://www.geeksforgeeks.org/turing-machine-to-check-whether-given-string-is-even-palindrome-or-not/](https://www.geeksforgeeks.org/turing-machine-to-check-whether-given-string-is-even-palindrome-or-not/)
...
*   **Step-1:**
    我们从 `Q0` 状态开始，如果我们得到符号 `a` 作为输入，那么字符串末尾也应该有一个 `a`，这样字符串才是回文，我们需要验证这一点。我们首先将当前输入 `a` 变为 `B`（空白）并进入状态 `Q1`，向右移动以遍历字符串直到到达末尾。保持输入符号 `a` 或 `b`（无论途中遇到哪个）不变。
...
*   **步骤 3:**
    如果字符串是回文，那么将只剩下空白符号，因此我们在 `Q0` 测试它，如果我们得到空白，因此该字符串被接受并且是回文，在这一点上，还有一个条件被包括，它是空字符串或 `ε` 作为空字符串也是回文，因此被接受。