...
> 原文: [https://www.geeksforgeks.org/equivalence-of-f-s-a-finite-state-automata/](https://www.geeksforgeks.org/equivalence-of-f-s-a-finite-state-automata/)
...
**步骤 4** 但是对于在输入字母上操作的这对(`q2`, `q5`)，它们位于不同的状态。(即一个在 `F.S`，另一个在 `I.S`)。
...
`F.S` 代表 -> 最终状态。
`I.S` 代表 -> 中间状态(非最终状态)。
**步骤 3** 对于这对(`q1`, `q3`)，当在输入字母上操作时，它们处于不同的状态。(即一个在 `F.S`，另一个在 `I.S`)。
...
`F.S` 代表 -> 最终状态。
`I.S` 代表 -> 中间状态(非最终状态)。
**步骤 3** 对于每一对状态，合成状态或者位于 `F.S` 中，或者作为组合位于 `I.S` 中。
