...
> 原文: [https://www.geeksforgeeks.org/python-tensorflow-reciprocal-method/](https://www.geeksforgeeks.org/python-tensorflow-reciprocal-method/)

[Tensorflow](https://www.geeksforgeeks.org/introduction-to-tensorflow/) 是谷歌开发的开源机器学习库。其应用之一是开发深度神经网络。模块 `tensorflow.math` 为许多基本的数学运算提供支持。函数 `tf.reciprocal()` [别名 `tf.math.reciprocal`] 支持计算 Tensorflow 中输入的*倒数*。它期望以复数形式输入 $a+bi$，浮点数和整数。输入类型是张量，如果输入包含一个以上的元素，则计算元素方向的倒数，$y=1/x$。

## 语法

`tf.reciprocal(x, name=None)` 或 `tf.math.reciprocal(x, name=None)`

## 参数

- `x`: `bfloat16`、`half`、`float32`、`float64`、`int32`、`int64`、`complex64` 或 `complex128` 类型的张量。
- `名称` (可选): 操作的名称。
