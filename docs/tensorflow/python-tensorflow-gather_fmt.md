...
## 语法:
`tensorflow.gather(params, indices, validate_indices=None, axis=None, batch_dims=None, name=None)`

## 参数:
*   `params`: 是秩大于等于 `axis+1` 的张量。
*   `indices`: 它是 `int32` 或 `int64` 类型的张量。它的值应该在范围 `[0, params.shape[axis])` 内。
*   `axis`: 是 `int32` 或 `int64` 类型的张量。它定义了应该从中收集索引的轴。默认值为 0，并且必须大于或等于 `batch_dims`。
*   `batch_dims`: 表示批次维度数的整数。它必须小于或等于 `rank(indices)`。
*   `name`: 定义操作的名称。

## 返回:
它返回一个与 `params` 具有相同数据类型的张量。
