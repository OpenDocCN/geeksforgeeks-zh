...
> 原文: [https://www.geeksforgeeks.org/ml-training-image-classifier-using-tensorflow-object-detection-api/](https://www.geeksforgeeks.org/ml-training-image-classifier-using-tensorflow-object-detection-api/)
...
在开始冒险之前，让我们确保 Python 3 安装在您的系统中。安装 python 和 pip [请参考本网站](https://realpython.com/installing-python/)。
...
*   **创建一个 `PYTHONPATH` 变量:**
    必须创建一个 `PYTHONPATH` 变量，该变量指向 `\模型`、`\模型\研究` 和 `\模型\研究\slim` 目录。从任何目录以下列方式发出命令。就我而言，
...
要编译协议文件，首先需要得到 protobuf 编译器。你可以[在这里](https://github.com/protocolbuffers/protobuf/releases)下载。下载 windows 操作系统的 `protocol-3.8-win64.zip` 文件，其他操作系统下载相关的 zip 文件。将 bin 文件夹提取到研究目录。
...
安装完成后，将安装一个名为 `object_detection` 的包。
...
在这次冒险中，我将构建一个检测鞋子和水瓶的分类器。请记住，数据集是构建分类器中最重要的东西。这将是完成对象检测的分类器的基础。收集尽可能多的不同种类的图像组成的对象。在研究目录中创建一个名为 `images` 的目录。将 80%的图像存储到 `train` 目录中，将 20%的图像存储到 `images` 目录内的 `test` 目录中。我在 `train` 目录里收集了 50 张图片，在 `test` 目录里收集了 10 张图片。图像数量越多，分类器的精度就越好。
...
在 object_detection 目录内新建一个名为 `training` 的目录。

使用文本编辑器创建一个新文件，并将其保存为 `training` 目录中的 `labelmap.pbtxt` 。标签映射通过定义类名到类标识号的映射来告诉培训师每个对象是什么。
...
我们需要一个模型，即算法来训练我们的分类器。在这个项目中，我们将使用 `faster_rcnn_inception` 模型。Tensorflow 的对象检测 API 附带了大量的模型。导航至 `object_detection/samples/configs`。

在这个位置你可以找到很多 API 提供的所有模型的配置文件。您可以使用此链接下载模型。下载文件 `faster_rcnn_inception_v2_coco` 。下载完成后，将文件夹 `faster_rcnn_inception_v2_coco_2018_01_28` 解压到 object_detection 目录。要了解模型的工作原理，请参考本文。

由于我们在这个项目中使用的是 `faster_rcnn_inception_v2_coco` 模型，所以从 `object_detection/samples/configs` 中复制 `faster_rcnn_inception_v2_coco.config` 文件，并将其粘贴到之前创建的 `training` 目录中。

使用文本编辑器打开配置文件，并对文件进行以下更改。
**注意:** 路径必须用单个正斜杠(不是反斜杠)输入，否则 TensorFlow 在尝试训练模型时会给出文件路径错误！此外，路径必须使用双引号( `"` )，而不是单引号(`'`)。

*   **第 10 行:** 将 `num_classes` 值设置为分类器正在分类的对象数量。在我的例子中，当我对鞋子和瓶子进行分类时，应该是 `num_classes: 2`。
*   **第 107 行:** 给 `fine_tune_checkpoint` 参数提供 `model.ckpt` 文件的绝对路径。`model.ckpt` 文件位于 `object_detection/faster_rcnn_inception_v2_coco_2018_01_28`。在我的例子中，
    `fine_tune_checkpoint: "F:/Programming/geesforgeks_project/models-master/research/object_detection/faster_rcnn_inception_v2_coco_2018_01_28/model.ckpt"`
*   **`train_input_reader` 区间:** 你可以在第 120 行找到这个区间。在本节中，将 `input_path` 参数设置为 `train.record` 文件。在我这里是
...
*   **`eval_config` 部分:** 你可以在第 128 行找到这个部分。将 `num_examples` 参数设置为测试目录中存在的图像数量。就我而言，
    `num_examples: 10`
*   **`eval_input_reader` section:** You can find this section in the line 134. Similar to `train_input_reader` section, set the paths to `test.record` and `labelmap.pbtxt` files. In my case,
    `input_path: "F:/Programming/geeksforgeeks_project/models-master/research/object_detection/test.record"`
    `label_map_path: "F:/Programming/geesforgeks_project/models-master/research/object_detection/training/labelmap.pbtxt"`
...
Tensorflow 每 5 分钟创建一个检查点并存储它。您可以看到所有的检查点都保存在 `training` 目录中。
...
这将在 `object_detection/inference_graph` 文件夹中创建一个 `frozen_inference_graph.pb` 文件。那个 `.pb` 文件包含对象检测分类器。
...
给出第 17 行中要检测的对象所在图像的路径`PATH_TO_IMAGE`。
