# TensorFlow `math.add()` 方法详解

> 原文: [https://www.geeksforgeeks.org/python-tensorflow-math-add-method/](https://www.geeksforgeeks.org/python-tensorflow-math-add-method/)

`TensorFlow` 的 `math.add()` 方法返回传递输入的 `a + b`。运算是在 `a` 和 `b` 的表示上完成的，这种方法属于数学模块。
