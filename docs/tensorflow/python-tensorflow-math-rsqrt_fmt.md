...
> 原文链接: [https://www.geeksforgeeks.org/python-tensorflow-math-rsqrt/](https://www.geeksforgeeks.org/python-tensorflow-math-rsqrt/)

[TensorFlow](https://www.geeksforgeeks.org/introduction-to-tensorflow/) 是谷歌为开发机器学习模型和深度学习神经网络而设计的开源 Python 库。`tf.math.rsqrt()` 用于求 `x` 平方根的元素倒数。
...
*   `x`: 输入张量。这个张量允许的数据类型是 `bfloat16`，`half`，`float32`，`float64`。
*   `name` (可选): 定义操作的名称。
