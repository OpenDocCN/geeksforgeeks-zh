...
`gradients()` 用于得到 `ys` 中 `ys` w.r.t. `x` 之和的符号导数。当启用紧急执行时，它不起作用。

## Syntax

`tensorflow.gradients(ys, xs, grad_ys, name, gate_gradients, aggregation_method, stop_gradients, unconnected_gradients)`
...
*   `name` (可选): 一起用于组梯度运算。它的默认值是 `gradients`。
...
*   `unconnected_gradients` (可选): 指定给定输入张量未连接时返回的梯度值。接受的值是在 `UnconnectedGradients` 类中定义的常数。

## 返回

长度 `xs` 的列表，其中每个张量是 `y` 在 `ys` 中的和 `x` 在 `xs` 中的和 (`dy/dx`)。
