...
> 原文链接：https://www.geeksforgeeks.org/install-tensorflow-on-macos/
...
### 步骤 4: 创建新的虚拟环境后，创建一个`./pythonenv`目录保存
...
### 步骤 8: 在`/pythonenv`内部创建一个名为`tfdemo`的新文件夹。在`tfdemo`中创建一个名为`tfdemofile.py`的新文件
