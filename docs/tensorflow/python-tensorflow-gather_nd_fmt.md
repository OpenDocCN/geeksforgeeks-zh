...
`gather_nd()` 用于根据提供的索引从输入张量聚集切片。

> **语法:** `tensorflow.gather_nd(params, indices, batch_dims, name)`
>
> **参数:**
>
> *   `params`: 秩大于等于轴+1 的张量。
> *   `indices`: 它是 `int32` 或 `int64` 类型的张量。
> *   `batch_dims`: 表示批次维度数的整数。它必须小于 `rank(indices)`。
> *   `name`: 定义操作的名称。
>
> **返回:**
>
