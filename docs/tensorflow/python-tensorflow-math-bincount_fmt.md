# TensorFlow: tf.math.bincount()

> 哎哎哎:# t0]https://www.geeksforgeeks.org/python-tensorflow-math-bincount/

TensorFlow 是谷歌设计的开源 Python 库，用于开发机器学习模型和深度学习神经网络。`bincount()` 存在于 TensorFlow 的数学模块中。它用于计算整数数组中每个数字的出现次数。

**语法:** `tf.math.bincount(arr, weights, minlength, maxlength, dtype, name)`

**参数:**

*   `arr`: 是非负值的 dtype int32 的张量。
*   `weights` (可选): 这是一个与 `arr` 形状相同的张量。`arr` 中每一个值的计数按其对应的权重递增。
*   `minlength` (可选): 定义返回输出的最小长度。
*   `maxlength` (可选): 定义返回输出的最大长度。不计算 `arr` 中大于或等于 `maxlength` 的值的 Bin。
*   `dtype` (可选): 如果 `weights` 为无，则确定返回输出的数据类型。
*   `name` (可选): 这是一个可选参数，用于定义操作的名称。
