...
> 原文: [https://www.geeksforgeeks.org/python-tensorflow-math-accumulate_n-method/](https://www.geeksforgeeks.org/python-tensorflow-math-accumulate_n-method/)

TensorFlow 的 `math.accumulate_n()` 方法执行传递的张量列表的元素求和。结果是执行操作后的张量。运算是在输入张量上完成的，这种方法属于数学模块。

> **语法:** `tf.math.accumulate_n(inputs, shape=None, tensor_dtype=None, name=None)`
>
> **参数**
>
> *   `inputs`: 该参数取 Tensor 对象的列表，每个对象的形状和类型相同。
> *   `shape`: 这是可选参数，它定义了输入元素的预期形状。
> *   `tensor_dtype`: 这是可选参数，它定义了输入的预期数据类型。
> *   `name`: 这是可选参数，这是操作的名称。
>
> **返回值:** 它返回一个与输入元素具有相同形状和类型的张量。
