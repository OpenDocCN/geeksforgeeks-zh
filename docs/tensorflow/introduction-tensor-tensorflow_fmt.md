...
1.  **Import or generate data**
    ...
    我们所有的机器学习算法都将依赖于数据。实际上，我们要么生成数据，要么使用外部数据源。有时依赖生成的数据更好，因为我们知道预期的结果。此外，tensorflow 还预装了著名的数据集，如 MNIST、CIFAR-10 等。
...
张量流算法需要知道哪些对象是变量，哪些是常数。所以我们用 TensorFlow 函数 `tf.Variable()` 创建了一个变量。请注意，您不能运行 `sess.run(my_var)`，这将导致错误。因为 TensorFlow 使用计算图进行操作，所以我们必须创建一个变量初始化操作来评估变量。对于这个脚本，我们可以通过调用变量方法 `my_var.initializer` 一次初始化一个变量。
