...
> 原文:[https://www.geeksforgeeks.org/python-tensorflow-math-cumulative_logsumexp/](https://www.geeksforgeeks.org/python-tensorflow-math-cumulative_logsumexp/)

TensorFlow 是谷歌设计的开源 Python 库，用于开发机器学习模型和深度学习神经网络。

`cumulative_logsumexp()` 用于计算输入张量的累计 log-sum-exp。这个操作相当于 `tf.math.log(tf.math.cumsum(tf.math.exp(x)))` 但是数值上更稳定。
...
*   `axis` (可选): 是 `int32` 类型的张量。它的值应该在 `int32` 类型的张量范围内 (默认值: 0)。必须在 `[-rank(x), rank(x)]` 范围内。默认值为 0。
*   `exclusive` (可选): 属于 `bool` 类型。默认值为 `False`。
*   `reverse` (可选): 是 `bool` 类型。默认值为 `False`。
*   `name` (可选): 定义操作的名称。
