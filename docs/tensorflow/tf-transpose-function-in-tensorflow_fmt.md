...
> 原文: [https://www.geeksforgeeks.org/tf-transpose-function-in-tensorflow/](https://www.geeksforgeeks.org/tf-transpose-function-in-tensorflow/)
...
- `perm`: 该参数指定了 `input_tensor` 将被转置的置换。
  - 类型: 矢量
- `conjugate`: 如果输入张量类型为复数，该参数设置为 `True`。
  - 类型: 布尔型
...
形状是 `(4, 3, 2)`，因为我们的 `perm` 是 `[0, 2, 1]`。下面是从 `perm` 到输入张量形状的映射。
