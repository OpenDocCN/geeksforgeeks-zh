...
> 原文: [https://www.geeksforgeeks.org/deploying-a-tensorflow-2-1-cnn-model-on-the-web-with-flask/](https://www.geeksforgeeks.org/deploying-a-tensorflow-2-1-cnn-model-on-the-web-with-flask/)
...
然后，我们将不得不安装 Flask。Flask 是 Python 中的一个微框架，广泛用于在网络上部署 ML 模型，我们将不得不使用以下命令安装 flask。
...
然后，我们需要导入 `ImageDataGenerator`。现在，来看看 Flask 的代码。将下面的代码复制到一个 `Code.py` 文件中，放在根目录下。完整的 Python 代码如下:-
...
注意: 每次进行预测时，请在上传新图像之前删除 `upload/image` 中的图像，否则可能会出现内部服务器错误。