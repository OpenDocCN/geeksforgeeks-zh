...
> 原文：[https://www.geeksforgeeks.org/how-to-create-custom-model-for-android-using-tensorflow/](https://www.geeksforgeeks.org/how-to-create-custom-model-for-android-using-tensorflow/)

[TensorFlow](https://www.geeksforgeeks.org/introduction-to-tensorflow/) 是机器学习的开源库。在安卓系统中，我们的计算能力和资源都很有限。因此，我们正在使用 TensorFlow Lite，这是专门为在功率有限的设备上运行而设计的。在这篇文章中，我们将看到一个名为 iris 数据集的分类示例。数据集包含 3 个类，每个类 50 个实例，其中每个类指的是鸢尾植物的类型。
...
从[这个](https://archive.ics.uci.edu/ml/machine-learning-databases/iris/)（`https://archive.ics.uci.edu/ml/machine-learning-databases/iris/`）链接下载 iris 数据集（文件名：`iris.data`）。
...
执行完 `open('iris.tflite', 'wb').write(tfmodel)` 后，一个名为 `iris.tflite` 的新文件将在 `iris.data` 所在的目录中创建。

### 第四步

**A)** 打开 Android Studio。创建一个新的 kotlin-android 项目。（创建项目可以参考[这里](https://developer.android.com/studio)）。

**B)** 右键点击 `app` > `New` > `Other` > `TensorFlow Lite Model`。
...
**F)** 点击 `Finish` 后你的模型会是这个样子。（加载可能需要一些时间）。
