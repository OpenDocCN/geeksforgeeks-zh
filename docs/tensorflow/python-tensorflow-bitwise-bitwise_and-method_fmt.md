...
> 原文：[https://www.geeksforgeeks.org/python-tensorflow-bitwise-bitwise_and-method/](https://www.geeksforgeeks.org/python-tensorflow-bitwise-bitwise_and-method/)

TensorFlow `bitwise.bitwise_and()` 方法执行按位“与”运算，并返回在 `a` 和 `b` 中都设置为（1）的那些位集合。该运算是在 `a` 和 `b` 的二进制表示上完成的。
该方法属于按位模块。
